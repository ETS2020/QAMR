// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x03), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  aoi21  g08(.a(x04), .b(x02), .c(x00), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  aoi21  g10(.a(new_n30_), .b(new_n25_), .c(new_n34_), .O(z0));
  nand3  g11(.a(new_n29_), .b(x06), .c(x05), .O(new_n36_));
  nand4  g12(.a(new_n36_), .b(new_n33_), .c(new_n32_), .d(x03), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(z1));
  inv1   g14(.a(x00), .O(new_n39_));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x08), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g18(.a(new_n26_), .b(x02), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n40_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n45_), .c(new_n31_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n43_), .c(new_n42_), .O(z2));
  nand2  g26(.a(x04), .b(x02), .O(new_n51_));
  inv1   g27(.a(x12), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  oai21  g29(.a(new_n52_), .b(x00), .c(new_n53_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n51_), .O(z3));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(x07), .O(new_n57_));
  nor2   g33(.a(new_n41_), .b(new_n39_), .O(new_n58_));
  nand2  g34(.a(new_n53_), .b(new_n51_), .O(new_n59_));
  aoi21  g35(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(z4));
  nand4  g36(.a(new_n44_), .b(new_n33_), .c(x13), .d(new_n52_), .O(z5));
  xor2a  g37(.a(x09), .b(x03), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n32_), .O(new_n63_));
  inv1   g39(.a(new_n58_), .O(new_n64_));
  nand3  g40(.a(new_n29_), .b(x14), .c(new_n39_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n44_), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n63_), .c(new_n51_), .O(z6));
  nand3  g43(.a(new_n31_), .b(x09), .c(x03), .O(new_n68_));
  oai21  g44(.a(new_n41_), .b(x04), .c(x02), .O(new_n69_));
  oai21  g45(.a(new_n25_), .b(x01), .c(new_n41_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n44_), .O(z7));
  nand3  g47(.a(new_n51_), .b(new_n56_), .c(x00), .O(new_n72_));
  oai21  g48(.a(new_n68_), .b(x12), .c(new_n72_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n44_), .O(z8));
endmodule


