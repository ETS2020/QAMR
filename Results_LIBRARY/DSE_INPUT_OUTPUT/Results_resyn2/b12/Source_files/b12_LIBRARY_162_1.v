// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x06), .O(new_n26_));
  nand2  g02(.a(x03), .b(x01), .O(new_n27_));
  oai21  g03(.a(new_n27_), .b(x00), .c(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g05(.a(x00), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n30_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n29_), .O(z0));
  oai22  g11(.a(new_n31_), .b(new_n26_), .c(x04), .d(x03), .O(new_n36_));
  nor2   g12(.a(x06), .b(x02), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n27_), .O(new_n38_));
  nand2  g14(.a(x06), .b(new_n25_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(x00), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  aoi21  g17(.a(new_n36_), .b(x02), .c(new_n41_), .O(z1));
  aoi21  g18(.a(x06), .b(new_n25_), .c(new_n30_), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x01), .O(new_n47_));
  inv1   g23(.a(x09), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  oai21  g26(.a(x10), .b(x07), .c(x03), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  aoi22  g28(.a(new_n52_), .b(new_n37_), .c(new_n46_), .d(new_n43_), .O(z2));
  nand2  g29(.a(x12), .b(new_n30_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n39_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  nand2  g34(.a(x09), .b(new_n44_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n39_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n44_), .d(new_n30_), .O(new_n63_));
  and2   g39(.a(new_n63_), .b(new_n39_), .O(z5));
  inv1   g40(.a(x03), .O(new_n65_));
  nand2  g41(.a(new_n48_), .b(new_n65_), .O(new_n66_));
  nand2  g42(.a(x09), .b(x03), .O(new_n67_));
  nand2  g43(.a(new_n25_), .b(new_n47_), .O(new_n68_));
  aoi21  g44(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n30_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n58_), .c(new_n49_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n69_), .c(new_n39_), .O(z6));
  oai21  g48(.a(new_n67_), .b(x01), .c(new_n26_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n25_), .O(new_n74_));
  nand2  g50(.a(x03), .b(new_n47_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(x02), .c(new_n45_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n49_), .O(z7));
  inv1   g53(.a(x10), .O(new_n78_));
  inv1   g54(.a(x12), .O(new_n79_));
  nand4  g55(.a(new_n79_), .b(new_n78_), .c(x09), .d(new_n44_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n75_), .c(new_n26_), .O(new_n81_));
  nand3  g57(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n82_));
  inv1   g58(.a(new_n82_), .O(new_n83_));
  aoi21  g59(.a(new_n81_), .b(new_n25_), .c(new_n83_), .O(z8));
endmodule


