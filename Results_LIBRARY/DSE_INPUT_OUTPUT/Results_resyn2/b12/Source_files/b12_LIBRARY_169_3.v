// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g08(.a(x06), .b(x01), .O(new_n33_));
  oai21  g09(.a(new_n32_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(new_n33_), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(x00), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  inv1   g14(.a(x01), .O(new_n39_));
  inv1   g15(.a(x02), .O(new_n40_));
  oai21  g16(.a(new_n30_), .b(new_n39_), .c(new_n40_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x09), .O(new_n45_));
  nor2   g21(.a(new_n44_), .b(x03), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(x02), .O(new_n47_));
  oai21  g23(.a(x06), .b(new_n40_), .c(x01), .O(new_n48_));
  oai21  g24(.a(x08), .b(x07), .c(x00), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g26(.a(new_n47_), .b(new_n45_), .c(new_n50_), .O(z2));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n52_), .c(new_n33_), .O(z3));
  inv1   g31(.a(x07), .O(new_n56_));
  nand4  g32(.a(x09), .b(x08), .c(new_n56_), .d(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n52_), .c(new_n33_), .O(z4));
  nand4  g35(.a(new_n44_), .b(new_n36_), .c(x13), .d(new_n53_), .O(z5));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  nand2  g37(.a(x09), .b(new_n30_), .O(new_n62_));
  inv1   g38(.a(x09), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x03), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n62_), .c(new_n28_), .O(new_n65_));
  inv1   g41(.a(x00), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n66_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n65_), .c(new_n61_), .d(new_n44_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n33_), .O(z6));
  nand3  g45(.a(x03), .b(new_n40_), .c(new_n39_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x08), .O(new_n71_));
  nand3  g47(.a(new_n28_), .b(new_n63_), .c(x03), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n44_), .c(new_n35_), .O(z7));
  nand2  g50(.a(new_n53_), .b(x09), .O(new_n75_));
  oai22  g51(.a(new_n75_), .b(new_n70_), .c(x09), .d(new_n66_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n44_), .c(new_n35_), .O(z8));
endmodule


