// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_;
  nor2   g00(.a(x08), .b(x01), .O(new_n25_));
  inv1   g01(.a(new_n25_), .O(new_n26_));
  inv1   g02(.a(x05), .O(new_n27_));
  nand2  g03(.a(x02), .b(x01), .O(new_n28_));
  aoi21  g04(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g08(.a(x03), .O(new_n33_));
  aoi21  g09(.a(x04), .b(x02), .c(new_n33_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(x00), .c(new_n26_), .O(z0));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  oai21  g13(.a(new_n33_), .b(new_n30_), .c(new_n31_), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n33_), .c(x00), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n26_), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x02), .O(new_n44_));
  inv1   g20(.a(x00), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x08), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n30_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n44_), .O(z2));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  inv1   g31(.a(x12), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n26_), .O(z3));
  inv1   g34(.a(x08), .O(new_n59_));
  oai21  g35(.a(new_n46_), .b(new_n30_), .c(new_n59_), .O(new_n60_));
  nand3  g36(.a(x09), .b(new_n46_), .c(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n55_), .c(new_n60_), .O(z4));
  nand3  g39(.a(x13), .b(new_n56_), .c(new_n48_), .O(new_n64_));
  nor3   g40(.a(new_n64_), .b(x07), .c(x00), .O(new_n65_));
  nor2   g41(.a(new_n65_), .b(new_n25_), .O(z5));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  nor2   g43(.a(new_n59_), .b(x01), .O(new_n68_));
  oai21  g44(.a(new_n67_), .b(x02), .c(new_n68_), .O(new_n69_));
  nand2  g45(.a(x14), .b(x02), .O(new_n70_));
  aoi21  g46(.a(new_n70_), .b(x01), .c(x00), .O(new_n71_));
  nor2   g47(.a(x10), .b(x07), .O(new_n72_));
  nand2  g48(.a(new_n59_), .b(x01), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x00), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  aoi21  g51(.a(new_n71_), .b(new_n69_), .c(new_n75_), .O(z6));
  nand4  g52(.a(x09), .b(x03), .c(new_n31_), .d(new_n30_), .O(new_n77_));
  and2   g53(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n59_), .c(new_n73_), .O(z7));
  nand2  g55(.a(new_n47_), .b(x00), .O(new_n80_));
  nand2  g56(.a(new_n56_), .b(x08), .O(new_n81_));
  oai22  g57(.a(new_n81_), .b(new_n77_), .c(new_n80_), .d(new_n25_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n72_), .O(z8));
endmodule


