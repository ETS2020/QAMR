// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  inv1   g10(.a(x10), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g13(.a(new_n32_), .b(new_n27_), .c(new_n37_), .O(z0));
  nand3  g14(.a(x03), .b(new_n30_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n36_), .O(z1));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  nor2   g22(.a(x03), .b(x01), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(x02), .c(new_n46_), .O(new_n48_));
  aoi21  g24(.a(x08), .b(x00), .c(x07), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n29_), .c(new_n34_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(x10), .c(new_n48_), .O(z2));
  nand2  g27(.a(x12), .b(new_n33_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n36_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  and2   g31(.a(x08), .b(x00), .O(new_n56_));
  nand2  g32(.a(new_n53_), .b(new_n36_), .O(new_n57_));
  nor2   g33(.a(new_n34_), .b(x07), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n56_), .c(new_n57_), .O(z4));
  inv1   g35(.a(x07), .O(new_n60_));
  inv1   g36(.a(x12), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n35_), .c(x09), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n60_), .d(new_n33_), .O(z5));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n65_));
  nand3  g41(.a(x03), .b(new_n30_), .c(new_n29_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n49_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(x09), .c(x10), .O(z6));
  nand3  g44(.a(new_n66_), .b(x08), .c(new_n60_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n35_), .O(z7));
  inv1   g47(.a(new_n66_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n63_), .c(new_n60_), .O(z8));
endmodule


