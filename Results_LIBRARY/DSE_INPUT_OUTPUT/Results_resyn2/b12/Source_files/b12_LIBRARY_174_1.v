// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x13), .O(new_n26_));
  nand2  g02(.a(x03), .b(x01), .O(new_n27_));
  oai21  g03(.a(new_n27_), .b(x00), .c(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g05(.a(x00), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n30_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n29_), .O(z0));
  nor2   g11(.a(x13), .b(x02), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x03), .c(x01), .O(new_n37_));
  inv1   g13(.a(x06), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(new_n25_), .O(new_n41_));
  oai21  g17(.a(new_n31_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nor2   g21(.a(new_n45_), .b(x03), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g25(.a(new_n49_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n46_), .c(new_n44_), .O(new_n51_));
  inv1   g27(.a(x08), .O(new_n52_));
  nand2  g28(.a(x13), .b(new_n25_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(x00), .O(new_n54_));
  aoi21  g30(.a(new_n52_), .b(new_n47_), .c(new_n54_), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(new_n36_), .c(new_n55_), .O(z2));
  nand2  g32(.a(x12), .b(new_n30_), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n57_), .c(new_n53_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z3));
  inv1   g36(.a(x09), .O(new_n61_));
  nor2   g37(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g38(.a(new_n58_), .b(new_n53_), .O(new_n63_));
  nor2   g39(.a(new_n52_), .b(new_n30_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n62_), .c(new_n63_), .O(z4));
  inv1   g41(.a(x12), .O(new_n66_));
  nand3  g42(.a(new_n45_), .b(new_n66_), .c(new_n30_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x02), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x13), .O(z5));
  nand2  g45(.a(new_n61_), .b(new_n39_), .O(new_n70_));
  nand2  g46(.a(x09), .b(x03), .O(new_n71_));
  nand2  g47(.a(new_n25_), .b(new_n44_), .O(new_n72_));
  aoi21  g48(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(new_n73_));
  inv1   g49(.a(new_n64_), .O(new_n74_));
  nand4  g50(.a(x14), .b(x02), .c(x01), .d(new_n30_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n45_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n73_), .c(new_n53_), .O(z6));
  oai21  g53(.a(new_n71_), .b(x01), .c(new_n26_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n25_), .O(new_n79_));
  nand3  g55(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n52_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n79_), .c(new_n45_), .O(z7));
  nand2  g58(.a(new_n61_), .b(x00), .O(new_n83_));
  aoi21  g59(.a(x13), .b(new_n25_), .c(new_n83_), .O(new_n84_));
  nand3  g60(.a(new_n26_), .b(new_n66_), .c(x09), .O(new_n85_));
  nor2   g61(.a(new_n85_), .b(new_n80_), .O(new_n86_));
  oai21  g62(.a(new_n86_), .b(new_n84_), .c(new_n45_), .O(z8));
endmodule


