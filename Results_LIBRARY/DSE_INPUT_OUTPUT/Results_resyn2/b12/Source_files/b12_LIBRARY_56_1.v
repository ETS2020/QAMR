// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  aoi21  g04(.a(x04), .b(x02), .c(new_n28_), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  inv1   g06(.a(new_n30_), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand2  g08(.a(x05), .b(new_n32_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n27_), .c(x00), .O(z0));
  oai21  g11(.a(new_n25_), .b(x02), .c(x00), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n28_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(x02), .O(new_n41_));
  nor2   g17(.a(new_n25_), .b(x02), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x02), .b(new_n43_), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(x03), .c(new_n42_), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n41_), .c(new_n37_), .O(z1));
  inv1   g22(.a(x00), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  oai21  g26(.a(x10), .b(x07), .c(x03), .O(new_n51_));
  inv1   g27(.a(x09), .O(new_n52_));
  nor2   g28(.a(x10), .b(x07), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand4  g30(.a(new_n54_), .b(new_n51_), .c(new_n25_), .d(new_n43_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n32_), .c(new_n50_), .O(z2));
  inv1   g32(.a(x12), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n33_), .O(z3));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n52_), .b(x07), .O(new_n63_));
  nand2  g39(.a(new_n58_), .b(new_n33_), .O(new_n64_));
  aoi21  g40(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(z4));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  inv1   g42(.a(x10), .O(new_n67_));
  nand3  g43(.a(x13), .b(new_n57_), .c(new_n67_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n66_), .c(new_n42_), .O(z5));
  nand2  g46(.a(x09), .b(new_n28_), .O(new_n71_));
  nand2  g47(.a(new_n52_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n30_), .O(new_n73_));
  nand4  g49(.a(x14), .b(x02), .c(x01), .d(new_n47_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n73_), .c(new_n61_), .d(new_n53_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n33_), .O(z6));
  nand3  g52(.a(x09), .b(x03), .c(new_n43_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n25_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n32_), .O(new_n79_));
  nand3  g55(.a(x03), .b(new_n32_), .c(new_n43_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n49_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n79_), .c(new_n53_), .O(z7));
  nand3  g58(.a(new_n57_), .b(new_n25_), .c(new_n32_), .O(new_n83_));
  oai22  g59(.a(new_n83_), .b(new_n77_), .c(new_n36_), .d(x09), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n53_), .O(z8));
endmodule


