// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x03), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x00), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n29_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n28_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n35_), .c(x02), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n25_), .c(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(x09), .c(new_n26_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n25_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(new_n47_));
  nand2  g23(.a(new_n36_), .b(new_n25_), .O(new_n48_));
  nand4  g24(.a(new_n44_), .b(x09), .c(new_n42_), .d(new_n26_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n25_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n29_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n48_), .c(new_n47_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n48_), .O(z3));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  inv1   g34(.a(x09), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(x07), .O(new_n60_));
  nand2  g36(.a(new_n54_), .b(new_n48_), .O(new_n61_));
  aoi21  g37(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z4));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n53_), .d(new_n44_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n48_), .O(z5));
  oai21  g41(.a(new_n59_), .b(x01), .c(x03), .O(new_n66_));
  nor2   g42(.a(x10), .b(x07), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n29_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n57_), .c(new_n67_), .O(new_n69_));
  aoi21  g45(.a(new_n66_), .b(new_n25_), .c(new_n69_), .O(z6));
  nand2  g46(.a(new_n66_), .b(new_n25_), .O(new_n71_));
  oai21  g47(.a(x02), .b(x01), .c(new_n43_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z7));
  nand4  g49(.a(new_n67_), .b(new_n53_), .c(x09), .d(new_n26_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(x03), .O(new_n75_));
  nand3  g51(.a(new_n67_), .b(new_n59_), .c(x00), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  aoi21  g53(.a(new_n75_), .b(new_n25_), .c(new_n77_), .O(z8));
endmodule


