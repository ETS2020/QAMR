// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x02), .O(new_n30_));
  oai21  g06(.a(new_n26_), .b(x02), .c(new_n30_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x01), .O(new_n32_));
  nand2  g08(.a(x04), .b(x03), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x02), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(z0));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x02), .O(new_n37_));
  inv1   g13(.a(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n38_), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(new_n27_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n29_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g21(.a(x01), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n46_), .O(new_n51_));
  aoi22  g27(.a(new_n51_), .b(new_n38_), .c(new_n45_), .d(new_n29_), .O(z2));
  nand2  g28(.a(x12), .b(new_n25_), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n27_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nand4  g32(.a(x09), .b(x08), .c(new_n43_), .d(x00), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n27_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n27_), .d(x13), .O(z5));
  nand3  g38(.a(x14), .b(x01), .c(new_n25_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x02), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n26_), .O(new_n65_));
  oai21  g41(.a(x09), .b(new_n26_), .c(new_n46_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n38_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  oai21  g44(.a(new_n44_), .b(new_n25_), .c(new_n48_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n27_), .O(z6));
  oai21  g48(.a(new_n44_), .b(new_n26_), .c(x02), .O(new_n73_));
  oai21  g49(.a(new_n26_), .b(x01), .c(new_n44_), .O(new_n74_));
  nand4  g50(.a(x09), .b(x03), .c(new_n38_), .d(new_n46_), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n48_), .O(z7));
  oai22  g52(.a(new_n75_), .b(x12), .c(x09), .d(new_n25_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n48_), .c(new_n28_), .O(z8));
endmodule


