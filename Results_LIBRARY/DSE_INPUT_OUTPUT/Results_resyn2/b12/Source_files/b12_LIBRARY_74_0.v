// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  nand3  g02(.a(x03), .b(x01), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x08), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n29_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n25_), .c(x01), .O(new_n38_));
  oai21  g14(.a(new_n37_), .b(new_n36_), .c(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  nor2   g16(.a(x08), .b(x02), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g19(.a(x03), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(new_n44_), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x09), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n45_), .c(x01), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(x02), .c(new_n26_), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  nand2  g26(.a(x07), .b(x02), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n49_), .O(z2));
  nand2  g29(.a(x12), .b(new_n26_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n42_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand2  g33(.a(new_n55_), .b(new_n42_), .O(new_n58_));
  nor2   g34(.a(x07), .b(new_n26_), .O(new_n59_));
  and2   g35(.a(x09), .b(x08), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n42_), .d(x13), .O(z5));
  xor2a  g40(.a(x09), .b(x03), .O(new_n65_));
  inv1   g41(.a(x01), .O(new_n66_));
  nand2  g42(.a(new_n25_), .b(new_n66_), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n65_), .c(new_n26_), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n26_), .O(new_n69_));
  oai21  g45(.a(new_n46_), .b(new_n41_), .c(new_n69_), .O(new_n70_));
  aoi21  g46(.a(new_n68_), .b(x08), .c(new_n70_), .O(z6));
  nand4  g47(.a(x09), .b(x03), .c(new_n25_), .d(new_n66_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n46_), .c(x08), .O(z7));
  inv1   g49(.a(x12), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(x03), .c(new_n66_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n47_), .c(x08), .O(new_n76_));
  nor2   g52(.a(x09), .b(new_n26_), .O(new_n77_));
  aoi22  g53(.a(new_n77_), .b(new_n46_), .c(new_n76_), .d(new_n25_), .O(z8));
endmodule


