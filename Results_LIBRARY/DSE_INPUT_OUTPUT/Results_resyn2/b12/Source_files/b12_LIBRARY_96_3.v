// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nand2  g07(.a(x10), .b(x02), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  inv1   g11(.a(x10), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n37_), .c(new_n36_), .d(x02), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g18(.a(x09), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g21(.a(x07), .b(x03), .c(x01), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g23(.a(x08), .b(x07), .c(x00), .O(new_n48_));
  oai21  g24(.a(x03), .b(x02), .c(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g26(.a(new_n47_), .b(new_n34_), .c(new_n50_), .O(z2));
  inv1   g27(.a(x12), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  oai21  g29(.a(new_n52_), .b(x00), .c(new_n53_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n32_), .O(z3));
  inv1   g31(.a(x08), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n25_), .O(new_n57_));
  nor2   g33(.a(new_n43_), .b(x07), .O(new_n58_));
  nand2  g34(.a(new_n53_), .b(new_n32_), .O(new_n59_));
  aoi21  g35(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(z4));
  nand4  g36(.a(new_n44_), .b(x13), .c(new_n52_), .d(new_n25_), .O(z5));
  inv1   g37(.a(x01), .O(new_n62_));
  nand2  g38(.a(new_n43_), .b(x03), .O(new_n63_));
  nand2  g39(.a(x09), .b(new_n38_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n67_));
  aoi21  g43(.a(x08), .b(x00), .c(x07), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n67_), .c(x10), .O(new_n69_));
  aoi21  g45(.a(new_n66_), .b(new_n34_), .c(new_n69_), .O(z6));
  nand3  g46(.a(x03), .b(new_n34_), .c(new_n62_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g49(.a(new_n71_), .b(new_n56_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n44_), .O(z7));
  nand4  g51(.a(new_n52_), .b(x03), .c(new_n34_), .d(new_n62_), .O(new_n76_));
  inv1   g52(.a(x07), .O(new_n77_));
  oai21  g53(.a(x09), .b(x00), .c(new_n77_), .O(new_n78_));
  aoi21  g54(.a(new_n76_), .b(x09), .c(new_n78_), .O(new_n79_));
  nand2  g55(.a(x10), .b(new_n34_), .O(new_n80_));
  oai21  g56(.a(new_n79_), .b(x10), .c(new_n80_), .O(z8));
endmodule


