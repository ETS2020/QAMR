// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_;
  nand2  g00(.a(x13), .b(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x00), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  nor2   g04(.a(x02), .b(new_n28_), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nor2   g06(.a(x04), .b(new_n30_), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(new_n29_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(x04), .c(new_n28_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(x13), .c(x02), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n32_), .c(new_n27_), .O(z0));
  nand2  g12(.a(new_n29_), .b(x03), .O(new_n37_));
  inv1   g13(.a(x13), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(x03), .O(new_n40_));
  inv1   g16(.a(x04), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n39_), .c(new_n38_), .d(x02), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n37_), .c(x00), .O(z1));
  nor2   g20(.a(x08), .b(x07), .O(new_n45_));
  nor2   g21(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n28_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n30_), .c(new_n46_), .O(z2));
  inv1   g28(.a(x00), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  aoi21  g31(.a(x12), .b(new_n53_), .c(new_n55_), .O(z3));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  nor2   g34(.a(new_n47_), .b(x07), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(z4));
  nor2   g36(.a(x12), .b(x00), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n48_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n30_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x13), .O(z5));
  xor2a  g40(.a(x09), .b(x03), .O(new_n65_));
  nand2  g41(.a(new_n30_), .b(new_n28_), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n53_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n57_), .c(new_n48_), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n67_), .c(new_n25_), .O(z6));
  nand4  g46(.a(x09), .b(x03), .c(new_n30_), .d(new_n28_), .O(new_n71_));
  inv1   g47(.a(x08), .O(new_n72_));
  oai21  g48(.a(new_n40_), .b(x01), .c(new_n72_), .O(new_n73_));
  oai21  g49(.a(x13), .b(new_n72_), .c(x02), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n73_), .c(new_n71_), .d(new_n48_), .O(z7));
  oai22  g51(.a(new_n71_), .b(x12), .c(new_n26_), .d(x09), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n48_), .O(z8));
endmodule


