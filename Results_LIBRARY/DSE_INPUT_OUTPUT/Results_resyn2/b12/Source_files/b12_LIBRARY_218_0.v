// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nor2   g08(.a(x11), .b(x06), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand3  g12(.a(new_n34_), .b(new_n29_), .c(new_n36_), .O(new_n37_));
  and2   g13(.a(x04), .b(x02), .O(new_n38_));
  nand2  g14(.a(x06), .b(x05), .O(new_n39_));
  oai22  g15(.a(new_n39_), .b(new_n26_), .c(new_n38_), .d(x03), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(new_n37_), .O(z1));
  oai21  g17(.a(x08), .b(x07), .c(x00), .O(new_n42_));
  inv1   g18(.a(x02), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n44_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n42_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n34_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  aoi21  g30(.a(x11), .b(x07), .c(new_n33_), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  inv1   g33(.a(x06), .O(new_n58_));
  nor2   g34(.a(x07), .b(new_n58_), .O(new_n59_));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n46_), .c(new_n45_), .O(new_n61_));
  oai21  g37(.a(new_n59_), .b(x11), .c(new_n61_), .O(z4));
  nand3  g38(.a(x13), .b(new_n54_), .c(new_n47_), .O(new_n63_));
  nor3   g39(.a(new_n63_), .b(x07), .c(x00), .O(new_n64_));
  nor2   g40(.a(new_n64_), .b(new_n33_), .O(z5));
  nor2   g41(.a(x10), .b(x07), .O(new_n66_));
  xnor2a g42(.a(x09), .b(x03), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n69_));
  nand3  g44(.a(new_n67_), .b(new_n43_), .c(new_n44_), .O(new_n72_));
  nand4  g45(.a(new_n72_), .b(new_n69_), .c(new_n60_), .d(new_n66_), .O(new_n73_));
  nand2  g46(.a(new_n73_), .b(new_n34_), .O(z6));
  nand3  g47(.a(new_n28_), .b(x09), .c(x03), .O(new_n75_));
  inv1   g48(.a(new_n66_), .O(new_n76_));
  aoi21  g49(.a(new_n28_), .b(x03), .c(x08), .O(new_n77_));
  nor2   g50(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g51(.a(new_n78_), .b(new_n75_), .c(new_n33_), .O(z7));
  oai22  g52(.a(new_n75_), .b(x12), .c(x09), .d(new_n36_), .O(new_n80_));
  aoi21  g53(.a(new_n80_), .b(new_n66_), .c(new_n33_), .O(z8));
endmodule


