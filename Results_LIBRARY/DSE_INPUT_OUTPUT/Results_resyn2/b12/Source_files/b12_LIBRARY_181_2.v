// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x03), .b(x01), .c(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(z0));
  inv1   g06(.a(x02), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x01), .O(new_n32_));
  nand3  g08(.a(x06), .b(x05), .c(x01), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  aoi21  g10(.a(new_n28_), .b(new_n34_), .c(new_n26_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n32_), .O(z1));
  oai21  g13(.a(x10), .b(x07), .c(new_n34_), .O(new_n38_));
  nor2   g14(.a(x10), .b(x07), .O(new_n39_));
  inv1   g15(.a(x01), .O(new_n40_));
  nand2  g16(.a(new_n31_), .b(new_n40_), .O(new_n41_));
  aoi21  g17(.a(new_n39_), .b(x09), .c(new_n41_), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g21(.a(new_n31_), .b(x01), .c(new_n25_), .O(new_n46_));
  aoi22  g22(.a(new_n46_), .b(new_n45_), .c(new_n42_), .d(new_n38_), .O(z2));
  inv1   g23(.a(x12), .O(new_n48_));
  nand2  g24(.a(x11), .b(x07), .O(new_n49_));
  oai21  g25(.a(new_n48_), .b(x00), .c(new_n49_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n32_), .O(z3));
  nand2  g27(.a(x08), .b(x00), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  inv1   g29(.a(x09), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x07), .O(new_n55_));
  nand2  g31(.a(new_n49_), .b(new_n32_), .O(new_n56_));
  aoi21  g32(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(z4));
  inv1   g33(.a(x10), .O(new_n58_));
  nor2   g34(.a(x07), .b(x00), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n48_), .d(new_n58_), .O(new_n60_));
  and2   g36(.a(new_n60_), .b(new_n32_), .O(z5));
  nand2  g37(.a(x14), .b(new_n25_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(x02), .c(new_n40_), .O(new_n63_));
  xnor2a g39(.a(x09), .b(x03), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n31_), .O(new_n65_));
  nand3  g41(.a(new_n52_), .b(new_n58_), .c(new_n43_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(new_n63_), .O(z6));
  nand3  g45(.a(new_n39_), .b(new_n54_), .c(x03), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n40_), .O(new_n71_));
  nand3  g47(.a(new_n58_), .b(x08), .c(new_n43_), .O(new_n72_));
  aoi21  g48(.a(x03), .b(new_n31_), .c(new_n72_), .O(new_n73_));
  aoi21  g49(.a(new_n71_), .b(new_n31_), .c(new_n73_), .O(z7));
  nand2  g50(.a(x09), .b(x03), .O(new_n75_));
  nand3  g51(.a(new_n48_), .b(new_n58_), .c(new_n43_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n75_), .c(new_n40_), .O(new_n77_));
  nand3  g53(.a(new_n39_), .b(new_n54_), .c(x00), .O(new_n78_));
  inv1   g54(.a(new_n78_), .O(new_n79_));
  aoi21  g55(.a(new_n77_), .b(new_n31_), .c(new_n79_), .O(z8));
endmodule


