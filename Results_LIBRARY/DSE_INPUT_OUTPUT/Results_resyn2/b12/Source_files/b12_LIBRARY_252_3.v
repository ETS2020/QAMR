// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  inv1   g03(.a(x14), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x02), .O(new_n29_));
  inv1   g05(.a(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n26_), .c(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n28_), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  nand2  g11(.a(x14), .b(x02), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n27_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(x02), .O(new_n40_));
  inv1   g16(.a(new_n36_), .O(new_n41_));
  aoi21  g17(.a(new_n26_), .b(x03), .c(new_n41_), .O(new_n42_));
  aoi22  g18(.a(new_n42_), .b(new_n40_), .c(new_n36_), .d(x00), .O(z1));
  inv1   g19(.a(x02), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n36_), .b(x00), .O(new_n47_));
  aoi21  g23(.a(new_n46_), .b(new_n45_), .c(new_n47_), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g27(.a(new_n50_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(x03), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n51_), .c(new_n25_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n44_), .c(new_n48_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n36_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(new_n45_), .d(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n56_), .c(new_n36_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n63_), .c(new_n36_), .d(x13), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  nor3   g42(.a(new_n66_), .b(x02), .c(x01), .O(new_n67_));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n50_), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n67_), .c(new_n36_), .O(z6));
  aoi21  g46(.a(x03), .b(new_n25_), .c(x08), .O(new_n71_));
  nand3  g47(.a(x09), .b(x03), .c(new_n25_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n71_), .c(new_n44_), .O(new_n74_));
  aoi21  g50(.a(new_n46_), .b(x02), .c(new_n52_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n74_), .c(new_n41_), .O(z7));
  nand2  g52(.a(new_n57_), .b(new_n44_), .O(new_n77_));
  oai22  g53(.a(new_n77_), .b(new_n72_), .c(new_n47_), .d(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n50_), .O(z8));
endmodule


