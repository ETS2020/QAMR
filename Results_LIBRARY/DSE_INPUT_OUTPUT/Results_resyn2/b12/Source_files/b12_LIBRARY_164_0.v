// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  inv1   g10(.a(x10), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g13(.a(new_n32_), .b(new_n27_), .c(new_n37_), .O(z0));
  nand3  g14(.a(x03), .b(new_n30_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n36_), .O(z1));
  inv1   g21(.a(x03), .O(new_n46_));
  oai21  g22(.a(new_n35_), .b(new_n46_), .c(new_n29_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  oai21  g24(.a(new_n46_), .b(x02), .c(new_n33_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x07), .O(new_n50_));
  aoi22  g26(.a(new_n35_), .b(new_n34_), .c(x08), .d(x00), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z2));
  nand2  g29(.a(x12), .b(new_n33_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n36_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  inv1   g33(.a(x07), .O(new_n58_));
  oai21  g34(.a(new_n35_), .b(new_n58_), .c(new_n34_), .O(new_n59_));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(x07), .c(new_n55_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n59_), .O(z4));
  inv1   g38(.a(x12), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nor2   g40(.a(x10), .b(new_n34_), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(x13), .d(new_n63_), .O(z5));
  nand3  g42(.a(x03), .b(new_n30_), .c(new_n29_), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n68_));
  nand4  g44(.a(new_n68_), .b(new_n67_), .c(new_n60_), .d(new_n58_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(x09), .c(x10), .O(z6));
  nand3  g46(.a(new_n67_), .b(x08), .c(new_n58_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x09), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n35_), .O(z7));
  nand2  g49(.a(new_n63_), .b(new_n58_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n67_), .c(x09), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n35_), .O(z8));
endmodule


