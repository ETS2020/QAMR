// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  aoi21  g02(.a(new_n26_), .b(x04), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(x06), .b(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(x02), .c(new_n28_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n27_), .c(x01), .O(new_n33_));
  inv1   g09(.a(x01), .O(new_n34_));
  nand4  g10(.a(new_n29_), .b(x03), .c(x02), .d(new_n34_), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n33_), .c(x00), .O(z0));
  nand2  g12(.a(x04), .b(x02), .O(new_n37_));
  aoi21  g13(.a(x06), .b(x05), .c(new_n37_), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n32_), .c(x01), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n35_), .c(x00), .O(z1));
  nor2   g16(.a(x10), .b(x07), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x09), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(new_n43_));
  oai21  g19(.a(new_n41_), .b(x03), .c(new_n29_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n43_), .c(new_n34_), .O(new_n45_));
  nor2   g21(.a(x08), .b(x07), .O(new_n46_));
  nand2  g22(.a(x04), .b(new_n34_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x00), .O(new_n48_));
  nor2   g24(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  aoi21  g25(.a(new_n45_), .b(new_n25_), .c(new_n49_), .O(z2));
  inv1   g26(.a(x12), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(x00), .c(new_n52_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n47_), .O(z3));
  nand2  g30(.a(x08), .b(x00), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x07), .O(new_n58_));
  nand2  g34(.a(new_n52_), .b(new_n47_), .O(new_n59_));
  aoi21  g35(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n47_), .d(x13), .O(z5));
  nand2  g39(.a(new_n57_), .b(new_n28_), .O(new_n64_));
  nand2  g40(.a(x09), .b(x03), .O(new_n65_));
  nand2  g41(.a(new_n25_), .b(new_n34_), .O(new_n66_));
  aoi21  g42(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(new_n67_));
  inv1   g43(.a(x00), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n68_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n55_), .c(new_n41_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n67_), .c(new_n47_), .O(z6));
  oai21  g47(.a(new_n65_), .b(x02), .c(new_n29_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n34_), .O(new_n73_));
  inv1   g49(.a(x08), .O(new_n74_));
  nand3  g50(.a(x03), .b(new_n25_), .c(new_n34_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n73_), .c(new_n41_), .O(z7));
  nand3  g53(.a(new_n51_), .b(x09), .c(new_n29_), .O(new_n78_));
  oai22  g54(.a(new_n78_), .b(new_n75_), .c(new_n48_), .d(x09), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n41_), .O(z8));
endmodule


