// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  aoi21  g02(.a(x02), .b(new_n26_), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand3  g04(.a(x03), .b(new_n28_), .c(x01), .O(new_n29_));
  nand2  g05(.a(x04), .b(x01), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(x05), .c(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(z0));
  nand2  g08(.a(x06), .b(x05), .O(new_n33_));
  aoi21  g09(.a(x03), .b(new_n28_), .c(new_n33_), .O(new_n34_));
  nand2  g10(.a(x01), .b(new_n25_), .O(new_n35_));
  aoi21  g11(.a(x04), .b(x02), .c(x03), .O(new_n36_));
  nor3   g12(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(z1));
  inv1   g13(.a(x07), .O(new_n38_));
  inv1   g14(.a(x08), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(new_n38_), .O(new_n43_));
  oai21  g19(.a(x10), .b(x07), .c(x03), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n43_), .c(new_n26_), .O(new_n45_));
  aoi22  g21(.a(new_n45_), .b(new_n28_), .c(new_n40_), .d(new_n27_), .O(z2));
  nand2  g22(.a(x02), .b(new_n26_), .O(new_n47_));
  and2   g23(.a(x11), .b(x07), .O(new_n48_));
  inv1   g24(.a(x12), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(x00), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  nand4  g27(.a(x09), .b(x08), .c(new_n38_), .d(x00), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n48_), .c(new_n47_), .O(z4));
  nor2   g30(.a(x07), .b(x00), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(x13), .c(new_n49_), .d(new_n42_), .O(new_n56_));
  and2   g32(.a(new_n56_), .b(new_n47_), .O(z5));
  nand2  g33(.a(x09), .b(x03), .O(new_n58_));
  inv1   g34(.a(x03), .O(new_n59_));
  nand2  g35(.a(new_n41_), .b(new_n59_), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n58_), .c(x01), .O(new_n61_));
  nor2   g37(.a(x10), .b(x07), .O(new_n62_));
  nand2  g38(.a(x08), .b(x00), .O(new_n63_));
  nand4  g39(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n61_), .c(new_n47_), .O(z6));
  nand2  g42(.a(new_n58_), .b(new_n28_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n26_), .O(new_n68_));
  oai21  g44(.a(new_n59_), .b(x01), .c(new_n39_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n62_), .O(z7));
  nand3  g46(.a(new_n49_), .b(new_n42_), .c(new_n38_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n58_), .c(new_n28_), .O(new_n72_));
  nand3  g48(.a(new_n62_), .b(new_n41_), .c(x00), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  aoi21  g50(.a(new_n72_), .b(new_n26_), .c(new_n74_), .O(z8));
endmodule


