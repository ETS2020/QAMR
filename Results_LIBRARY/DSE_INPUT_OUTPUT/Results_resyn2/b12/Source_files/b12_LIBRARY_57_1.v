// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x07), .O(new_n34_));
  nand2  g10(.a(x14), .b(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n29_), .c(new_n36_), .O(z0));
  nand3  g13(.a(x03), .b(new_n26_), .c(x01), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  oai21  g16(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n33_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n35_), .O(z1));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand2  g22(.a(x07), .b(x03), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n33_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  oai21  g26(.a(x10), .b(new_n50_), .c(new_n34_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n49_), .c(new_n25_), .O(new_n52_));
  nand2  g28(.a(new_n46_), .b(x00), .O(new_n53_));
  inv1   g29(.a(x03), .O(new_n54_));
  nand3  g30(.a(x10), .b(new_n54_), .c(new_n25_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n26_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n52_), .c(new_n35_), .O(z2));
  inv1   g34(.a(x11), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(x07), .O(new_n60_));
  inv1   g36(.a(x14), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n34_), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(new_n60_), .c(x12), .d(new_n33_), .O(z3));
  nand2  g39(.a(x08), .b(x00), .O(new_n64_));
  nand2  g40(.a(new_n61_), .b(x09), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n64_), .c(new_n34_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n60_), .O(z4));
  nor2   g43(.a(new_n62_), .b(x10), .O(new_n68_));
  nor2   g44(.a(x12), .b(x00), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(x13), .O(z5));
  nand2  g46(.a(x09), .b(new_n54_), .O(new_n71_));
  nand2  g47(.a(new_n50_), .b(x03), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n71_), .c(new_n26_), .d(new_n25_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n68_), .c(new_n64_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  nand3  g51(.a(x03), .b(new_n26_), .c(new_n25_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x08), .O(new_n77_));
  oai21  g53(.a(new_n72_), .b(new_n27_), .c(new_n77_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n68_), .O(z7));
  oai21  g55(.a(new_n76_), .b(x12), .c(x09), .O(new_n80_));
  nand2  g56(.a(new_n50_), .b(new_n33_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(new_n68_), .O(z8));
endmodule


