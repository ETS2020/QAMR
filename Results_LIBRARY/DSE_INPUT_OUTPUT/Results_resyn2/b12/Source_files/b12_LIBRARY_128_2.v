// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  oai21  g05(.a(x04), .b(new_n29_), .c(new_n28_), .O(new_n30_));
  inv1   g06(.a(x03), .O(new_n31_));
  aoi21  g07(.a(x02), .b(x01), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(new_n27_), .O(new_n33_));
  nor2   g09(.a(x11), .b(new_n28_), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  oai21  g11(.a(new_n33_), .b(x00), .c(new_n35_), .O(z0));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  oai21  g13(.a(new_n31_), .b(new_n28_), .c(new_n29_), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n31_), .c(x00), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n35_), .O(z1));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x09), .O(new_n44_));
  nor2   g20(.a(new_n43_), .b(x03), .O(new_n45_));
  nor2   g21(.a(new_n45_), .b(x02), .O(new_n46_));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  inv1   g23(.a(x11), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n29_), .c(x01), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  aoi21  g26(.a(new_n46_), .b(new_n44_), .c(new_n50_), .O(z2));
  inv1   g27(.a(x00), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n52_), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n35_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nor2   g32(.a(x07), .b(x01), .O(new_n57_));
  nand3  g33(.a(x09), .b(x08), .c(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai22  g35(.a(new_n59_), .b(x07), .c(new_n57_), .d(x11), .O(z4));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x13), .O(new_n63_));
  inv1   g39(.a(new_n63_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n61_), .c(new_n34_), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  nand2  g42(.a(new_n29_), .b(new_n28_), .O(new_n67_));
  nor2   g43(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n52_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n43_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n68_), .c(new_n35_), .O(z6));
  nand4  g48(.a(x09), .b(x03), .c(new_n29_), .d(new_n28_), .O(new_n73_));
  inv1   g49(.a(x08), .O(new_n74_));
  oai21  g50(.a(new_n48_), .b(new_n74_), .c(x01), .O(new_n75_));
  oai21  g51(.a(new_n31_), .b(x02), .c(new_n74_), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n75_), .c(new_n73_), .d(new_n43_), .O(z7));
  inv1   g53(.a(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x00), .O(new_n79_));
  oai21  g55(.a(new_n73_), .b(x12), .c(new_n79_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n43_), .c(new_n34_), .O(z8));
endmodule


