// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  inv1   g03(.a(x08), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x01), .O(new_n32_));
  nor2   g08(.a(new_n27_), .b(new_n32_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n25_), .c(new_n33_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand4  g12(.a(new_n28_), .b(x03), .c(new_n27_), .d(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand2  g18(.a(x08), .b(new_n27_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(z1));
  nor2   g20(.a(x10), .b(x09), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  nand2  g22(.a(x10), .b(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n32_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n45_), .c(new_n28_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n27_), .O(new_n50_));
  inv1   g26(.a(x03), .O(new_n51_));
  nand3  g27(.a(x07), .b(new_n51_), .c(new_n32_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  nand3  g30(.a(new_n28_), .b(new_n46_), .c(x02), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n50_), .O(z2));
  inv1   g32(.a(x12), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n43_), .O(z3));
  nand3  g36(.a(x09), .b(new_n46_), .c(x00), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x02), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(x08), .c(x11), .d(x07), .O(z4));
  nor2   g39(.a(new_n28_), .b(x02), .O(new_n64_));
  inv1   g40(.a(x10), .O(new_n65_));
  nand3  g41(.a(x13), .b(new_n57_), .c(new_n65_), .O(new_n66_));
  nor3   g42(.a(new_n66_), .b(x07), .c(x00), .O(new_n67_));
  nor2   g43(.a(new_n67_), .b(new_n64_), .O(z5));
  nand2  g44(.a(new_n65_), .b(new_n46_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n43_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  nand2  g47(.a(x09), .b(new_n51_), .O(new_n72_));
  inv1   g48(.a(x09), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x03), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n72_), .c(new_n32_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n27_), .O(new_n76_));
  nand3  g52(.a(x14), .b(x01), .c(new_n36_), .O(new_n77_));
  aoi21  g53(.a(x08), .b(x00), .c(new_n27_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n77_), .c(new_n64_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n76_), .c(new_n71_), .O(z6));
  nand2  g56(.a(new_n27_), .b(new_n32_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n74_), .c(new_n28_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n70_), .O(z7));
  nand2  g59(.a(new_n57_), .b(x03), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(new_n81_), .c(x09), .O(new_n85_));
  oai21  g61(.a(x09), .b(new_n27_), .c(x08), .O(new_n86_));
  aoi21  g62(.a(new_n73_), .b(new_n36_), .c(new_n69_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z8));
endmodule


