// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(x02), .b(x01), .O(new_n33_));
  aoi21  g09(.a(new_n32_), .b(x04), .c(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n31_), .c(new_n25_), .O(new_n35_));
  nand2  g11(.a(x09), .b(x02), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(z0));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n39_), .c(new_n28_), .d(new_n26_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n36_), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(new_n46_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n47_), .c(new_n44_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n36_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n27_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n36_), .O(z3));
  inv1   g33(.a(x07), .O(new_n58_));
  oai21  g34(.a(x09), .b(new_n58_), .c(x02), .O(new_n59_));
  nand4  g35(.a(x09), .b(x08), .c(new_n58_), .d(x00), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n55_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n59_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n63_), .c(new_n36_), .d(x13), .O(z5));
  nand3  g41(.a(x14), .b(x01), .c(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x02), .O(new_n67_));
  nand3  g43(.a(x09), .b(x03), .c(new_n44_), .O(new_n68_));
  oai21  g44(.a(new_n26_), .b(x02), .c(new_n45_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n67_), .c(new_n28_), .O(new_n71_));
  inv1   g47(.a(x08), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n25_), .c(new_n46_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n36_), .O(z6));
  inv1   g52(.a(new_n36_), .O(new_n77_));
  inv1   g53(.a(new_n68_), .O(new_n78_));
  aoi21  g54(.a(x03), .b(new_n44_), .c(x08), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n78_), .c(new_n27_), .O(new_n80_));
  aoi21  g56(.a(new_n72_), .b(x02), .c(new_n48_), .O(new_n81_));
  aoi21  g57(.a(new_n81_), .b(new_n80_), .c(new_n77_), .O(z7));
  nand2  g58(.a(x03), .b(new_n27_), .O(new_n83_));
  nand3  g59(.a(new_n54_), .b(x09), .c(new_n44_), .O(new_n84_));
  oai22  g60(.a(new_n84_), .b(new_n83_), .c(x09), .d(new_n25_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n46_), .O(z8));
endmodule


