// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  nor2   g02(.a(new_n26_), .b(x08), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g08(.a(x04), .b(x02), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n32_), .c(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nor2   g11(.a(new_n31_), .b(new_n30_), .O(new_n36_));
  oai21  g12(.a(x05), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  nand3  g14(.a(x03), .b(new_n31_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(x03), .O(new_n41_));
  nand2  g17(.a(new_n35_), .b(new_n41_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n40_), .c(x02), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n39_), .c(new_n29_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  oai21  g21(.a(x10), .b(x00), .c(x08), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  nand2  g23(.a(new_n26_), .b(new_n45_), .O(new_n48_));
  nor2   g24(.a(x03), .b(x00), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  oai21  g28(.a(x08), .b(x07), .c(x00), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(x02), .c(new_n27_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n28_), .O(z3));
  inv1   g35(.a(x08), .O(new_n60_));
  oai21  g36(.a(x10), .b(new_n45_), .c(new_n60_), .O(new_n61_));
  nand3  g37(.a(x09), .b(new_n45_), .c(x00), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n56_), .c(new_n61_), .O(z4));
  nand3  g40(.a(x13), .b(new_n57_), .c(new_n45_), .O(new_n65_));
  nor3   g41(.a(new_n65_), .b(x10), .c(x00), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n27_), .O(z5));
  nand2  g43(.a(x14), .b(x02), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n26_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(x01), .c(new_n25_), .O(new_n70_));
  xnor2a g46(.a(x09), .b(x03), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n31_), .c(new_n30_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n70_), .c(new_n46_), .d(new_n45_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n28_), .O(z6));
  inv1   g50(.a(new_n48_), .O(new_n75_));
  oai21  g51(.a(new_n32_), .b(new_n41_), .c(new_n60_), .O(new_n76_));
  nand4  g52(.a(x09), .b(x03), .c(new_n31_), .d(new_n30_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(z7));
  oai22  g54(.a(new_n77_), .b(x12), .c(x09), .d(new_n25_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n75_), .c(new_n27_), .O(z8));
endmodule


