// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x00), .O(new_n25_));
  oai21  g01(.a(x02), .b(x01), .c(x03), .O(new_n26_));
  aoi21  g02(.a(x04), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(x02), .b(x01), .O(new_n29_));
  aoi21  g05(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n27_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x07), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(x02), .O(new_n35_));
  nand3  g11(.a(x03), .b(new_n35_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n37_), .c(x07), .d(x02), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n36_), .c(x00), .O(z1));
  nand2  g18(.a(x03), .b(new_n35_), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n25_), .c(new_n32_), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n32_), .O(new_n47_));
  nand2  g23(.a(x08), .b(x00), .O(new_n48_));
  aoi21  g24(.a(x10), .b(x03), .c(x01), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n35_), .c(new_n44_), .O(z2));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n33_), .c(new_n52_), .O(z3));
  nand2  g31(.a(new_n32_), .b(new_n35_), .O(new_n56_));
  nor3   g32(.a(new_n56_), .b(new_n48_), .c(new_n45_), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n52_), .O(z4));
  nor2   g34(.a(new_n56_), .b(x10), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n53_), .d(new_n25_), .O(z5));
  inv1   g36(.a(x01), .O(new_n61_));
  nand2  g37(.a(new_n45_), .b(x03), .O(new_n62_));
  nand2  g38(.a(x09), .b(new_n38_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n48_), .c(new_n46_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n35_), .c(x07), .O(z6));
  nand2  g42(.a(x03), .b(new_n61_), .O(new_n67_));
  and2   g43(.a(new_n67_), .b(x08), .O(new_n68_));
  nand3  g44(.a(new_n45_), .b(x03), .c(new_n61_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n68_), .c(new_n59_), .O(z7));
  oai21  g47(.a(new_n67_), .b(x12), .c(x09), .O(new_n72_));
  nand2  g48(.a(new_n45_), .b(new_n25_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n59_), .O(z8));
endmodule


