// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  nand2  g02(.a(x05), .b(x02), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(x03), .b(new_n28_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(x00), .O(new_n32_));
  and2   g08(.a(new_n32_), .b(new_n30_), .O(z0));
  nor2   g09(.a(new_n28_), .b(x01), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  inv1   g11(.a(x00), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x02), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  oai22  g14(.a(new_n25_), .b(new_n28_), .c(new_n38_), .d(new_n31_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n35_), .O(z1));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nor2   g18(.a(new_n42_), .b(x03), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(x09), .c(new_n44_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n43_), .c(new_n31_), .O(new_n48_));
  oai21  g24(.a(x08), .b(x07), .c(x00), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n35_), .O(new_n50_));
  aoi21  g26(.a(new_n48_), .b(new_n28_), .c(new_n50_), .O(z2));
  nand2  g27(.a(x12), .b(new_n36_), .O(new_n52_));
  and2   g28(.a(x11), .b(x07), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n34_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nand4  g32(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n53_), .c(new_n35_), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nand3  g36(.a(x13), .b(new_n60_), .c(new_n45_), .O(new_n61_));
  nor3   g37(.a(new_n61_), .b(x07), .c(x00), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n34_), .O(z5));
  nand2  g39(.a(x08), .b(x00), .O(new_n64_));
  inv1   g40(.a(x09), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(x03), .O(new_n66_));
  nand2  g42(.a(x09), .b(new_n38_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n31_), .O(new_n68_));
  nand3  g44(.a(new_n32_), .b(x14), .c(x02), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n68_), .c(new_n64_), .d(new_n42_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n35_), .O(z6));
  nand3  g47(.a(new_n42_), .b(new_n65_), .c(x03), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n28_), .O(new_n73_));
  nand2  g49(.a(x03), .b(new_n31_), .O(new_n74_));
  nand2  g50(.a(new_n42_), .b(x08), .O(new_n75_));
  inv1   g51(.a(new_n75_), .O(new_n76_));
  aoi22  g52(.a(new_n76_), .b(new_n74_), .c(new_n73_), .d(new_n31_), .O(z7));
  nand2  g53(.a(new_n65_), .b(x00), .O(new_n78_));
  nor2   g54(.a(new_n78_), .b(new_n34_), .O(new_n79_));
  nand3  g55(.a(new_n60_), .b(x09), .c(new_n28_), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n79_), .c(new_n42_), .O(z8));
endmodule


