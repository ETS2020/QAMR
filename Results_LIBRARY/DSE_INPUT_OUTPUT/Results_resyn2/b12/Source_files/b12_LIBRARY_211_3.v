// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  aoi21  g00(.a(x11), .b(x08), .c(x00), .O(new_n25_));
  inv1   g01(.a(new_n25_), .O(new_n26_));
  inv1   g02(.a(x05), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x04), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(x02), .c(x01), .O(new_n29_));
  nand2  g05(.a(x04), .b(x02), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n30_), .c(x03), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n29_), .c(new_n26_), .O(z0));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  oai21  g14(.a(new_n36_), .b(new_n31_), .c(new_n32_), .O(new_n39_));
  nand4  g15(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n25_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n31_), .O(new_n48_));
  inv1   g24(.a(x00), .O(new_n49_));
  oai21  g25(.a(x11), .b(x00), .c(x08), .O(new_n50_));
  oai21  g26(.a(new_n43_), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  aoi21  g27(.a(new_n48_), .b(new_n32_), .c(new_n51_), .O(z2));
  nand2  g28(.a(x11), .b(x08), .O(new_n53_));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(z3));
  inv1   g33(.a(x08), .O(new_n58_));
  nor2   g34(.a(x07), .b(new_n49_), .O(new_n59_));
  nand2  g35(.a(x09), .b(x08), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(x11), .O(new_n61_));
  aoi22  g37(.a(new_n61_), .b(new_n59_), .c(new_n54_), .d(new_n58_), .O(z4));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n55_), .d(new_n45_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n53_), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  nor2   g43(.a(x10), .b(x07), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n49_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n50_), .c(new_n68_), .O(new_n70_));
  nor2   g46(.a(new_n70_), .b(new_n67_), .O(z6));
  oai21  g47(.a(new_n33_), .b(new_n36_), .c(new_n58_), .O(new_n72_));
  nor2   g48(.a(x02), .b(x01), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(x09), .c(x03), .O(new_n74_));
  nand2  g50(.a(new_n68_), .b(new_n53_), .O(new_n75_));
  inv1   g51(.a(new_n75_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(z7));
  nand3  g53(.a(new_n73_), .b(new_n55_), .c(x03), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x09), .O(new_n79_));
  nand2  g55(.a(new_n44_), .b(new_n49_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(z8));
endmodule


