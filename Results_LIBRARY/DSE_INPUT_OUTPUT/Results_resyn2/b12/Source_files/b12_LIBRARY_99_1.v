// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n26_), .b(new_n25_), .c(x00), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  inv1   g04(.a(x10), .O(new_n29_));
  nand2  g05(.a(x04), .b(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  oai21  g10(.a(x04), .b(new_n34_), .c(new_n33_), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(new_n32_), .c(new_n27_), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(z0));
  nor2   g13(.a(new_n29_), .b(x05), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n34_), .O(new_n41_));
  nand4  g17(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(new_n27_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n39_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  nand3  g21(.a(new_n29_), .b(x09), .c(new_n45_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n34_), .c(new_n47_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n33_), .O(new_n51_));
  oai21  g27(.a(x08), .b(x07), .c(x00), .O(new_n52_));
  nand3  g28(.a(x10), .b(new_n34_), .c(new_n33_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n25_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n51_), .c(new_n39_), .O(z2));
  and2   g32(.a(x11), .b(x07), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n57_), .c(new_n39_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n45_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n39_), .O(z4));
  nand2  g39(.a(new_n29_), .b(new_n45_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(new_n65_));
  nand3  g41(.a(x13), .b(new_n58_), .c(new_n47_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n65_), .c(new_n38_), .O(z5));
  nand2  g44(.a(x09), .b(new_n34_), .O(new_n69_));
  nor2   g45(.a(x02), .b(x01), .O(new_n70_));
  inv1   g46(.a(x09), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(new_n73_));
  nand4  g49(.a(x14), .b(x02), .c(x01), .d(new_n47_), .O(new_n74_));
  nand3  g50(.a(new_n50_), .b(new_n29_), .c(new_n45_), .O(new_n75_));
  inv1   g51(.a(new_n75_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n39_), .O(z6));
  nand3  g54(.a(new_n70_), .b(x09), .c(x03), .O(new_n79_));
  inv1   g55(.a(x08), .O(new_n80_));
  nand3  g56(.a(x03), .b(new_n25_), .c(new_n33_), .O(new_n81_));
  aoi21  g57(.a(new_n81_), .b(new_n80_), .c(new_n64_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n79_), .c(new_n38_), .O(z7));
  nand2  g59(.a(new_n71_), .b(x00), .O(new_n84_));
  oai21  g60(.a(new_n79_), .b(x12), .c(new_n84_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n65_), .O(z8));
endmodule


