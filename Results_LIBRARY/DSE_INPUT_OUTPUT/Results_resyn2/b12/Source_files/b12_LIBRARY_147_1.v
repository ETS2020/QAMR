// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(x04), .b(x02), .O(new_n30_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x08), .O(new_n34_));
  nand2  g10(.a(x09), .b(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n29_), .c(new_n36_), .O(z0));
  nand3  g13(.a(new_n28_), .b(x06), .c(x05), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n26_), .c(new_n27_), .O(new_n40_));
  aoi21  g16(.a(new_n25_), .b(new_n39_), .c(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n35_), .O(z1));
  nand2  g19(.a(x07), .b(x02), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x09), .O(new_n45_));
  nand2  g21(.a(x07), .b(new_n39_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  aoi21  g26(.a(x09), .b(new_n34_), .c(x01), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n50_), .c(new_n45_), .O(new_n52_));
  nand3  g28(.a(x10), .b(new_n39_), .c(new_n26_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  nand2  g30(.a(x08), .b(x00), .O(new_n55_));
  oai21  g31(.a(x09), .b(x07), .c(new_n34_), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  oai21  g33(.a(new_n52_), .b(x00), .c(new_n57_), .O(z2));
  nand2  g34(.a(x12), .b(new_n33_), .O(new_n59_));
  nand2  g35(.a(x11), .b(x07), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n59_), .c(new_n35_), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(z3));
  oai21  g38(.a(x07), .b(new_n33_), .c(x08), .O(new_n63_));
  aoi22  g39(.a(new_n63_), .b(x09), .c(x11), .d(x07), .O(z4));
  inv1   g40(.a(x12), .O(new_n65_));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(x13), .c(new_n65_), .d(new_n48_), .O(new_n67_));
  and2   g43(.a(new_n67_), .b(new_n35_), .O(z5));
  xor2a  g44(.a(x09), .b(x03), .O(new_n69_));
  nor2   g45(.a(new_n69_), .b(new_n31_), .O(new_n70_));
  nor2   g46(.a(x10), .b(x07), .O(new_n71_));
  nand3  g47(.a(new_n28_), .b(x14), .c(new_n33_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n55_), .c(new_n71_), .d(new_n35_), .O(new_n73_));
  nor2   g49(.a(new_n73_), .b(new_n70_), .O(z6));
  oai21  g50(.a(new_n31_), .b(new_n39_), .c(new_n34_), .O(new_n75_));
  nand4  g51(.a(x09), .b(x03), .c(new_n27_), .d(new_n26_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n71_), .O(z7));
  nand2  g53(.a(new_n65_), .b(x08), .O(new_n78_));
  oai22  g54(.a(new_n78_), .b(new_n76_), .c(x09), .d(new_n33_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n71_), .O(z8));
endmodule


