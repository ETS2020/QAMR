// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n27_), .O(new_n32_));
  nor2   g08(.a(new_n25_), .b(x04), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  nand3  g11(.a(x06), .b(x02), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(x04), .c(new_n25_), .O(new_n37_));
  inv1   g13(.a(x00), .O(new_n38_));
  oai21  g14(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(new_n37_), .O(z1));
  inv1   g17(.a(x02), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n44_), .c(x03), .O(new_n46_));
  nand3  g22(.a(new_n45_), .b(x09), .c(new_n44_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n34_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n42_), .c(new_n51_), .O(z2));
  nand2  g28(.a(x12), .b(new_n38_), .O(new_n53_));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(new_n33_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand4  g33(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n54_), .c(new_n34_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n34_), .d(x13), .O(z5));
  nand2  g39(.a(x08), .b(x00), .O(new_n64_));
  xnor2a g40(.a(x09), .b(x03), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n38_), .O(new_n67_));
  nand3  g42(.a(new_n65_), .b(new_n42_), .c(new_n43_), .O(new_n70_));
  nand2  g43(.a(new_n45_), .b(new_n44_), .O(new_n71_));
  nor2   g44(.a(new_n71_), .b(new_n33_), .O(new_n72_));
  nand4  g45(.a(new_n72_), .b(new_n70_), .c(new_n67_), .d(new_n64_), .O(new_n73_));
  inv1   g46(.a(new_n73_), .O(z6));
  nand3  g47(.a(new_n30_), .b(x09), .c(x03), .O(new_n75_));
  inv1   g48(.a(x08), .O(new_n76_));
  nand2  g49(.a(new_n30_), .b(x03), .O(new_n77_));
  aoi21  g50(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(new_n78_));
  aoi21  g51(.a(new_n78_), .b(new_n75_), .c(new_n33_), .O(z7));
  oai21  g52(.a(new_n77_), .b(x12), .c(x09), .O(new_n80_));
  inv1   g53(.a(x09), .O(new_n81_));
  nand2  g54(.a(new_n81_), .b(new_n38_), .O(new_n82_));
  nand3  g55(.a(new_n82_), .b(new_n80_), .c(new_n72_), .O(z8));
endmodule


