// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  oai22  g02(.a(x08), .b(new_n25_), .c(x04), .d(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  aoi21  g04(.a(x02), .b(x01), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nor2   g08(.a(x08), .b(new_n26_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n32_), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g11(.a(x08), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x02), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n36_), .c(new_n25_), .O(new_n38_));
  aoi21  g14(.a(x04), .b(x02), .c(x03), .O(new_n39_));
  inv1   g15(.a(x00), .O(new_n40_));
  oai21  g16(.a(x02), .b(x01), .c(new_n40_), .O(new_n41_));
  nor3   g17(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z1));
  nand2  g18(.a(x10), .b(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n43_), .c(x02), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  aoi21  g24(.a(x08), .b(x00), .c(x01), .O(new_n49_));
  aoi21  g25(.a(x03), .b(new_n26_), .c(x00), .O(new_n50_));
  oai22  g26(.a(new_n50_), .b(new_n48_), .c(new_n49_), .d(new_n33_), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(new_n47_), .O(z2));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand2  g29(.a(x08), .b(x01), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi21  g31(.a(x12), .b(new_n40_), .c(new_n55_), .O(z3));
  nand3  g32(.a(x09), .b(new_n48_), .c(x00), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(x08), .c(x11), .d(x07), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(x13), .c(new_n48_), .d(new_n40_), .O(new_n61_));
  and2   g37(.a(new_n61_), .b(new_n54_), .O(z5));
  inv1   g38(.a(new_n45_), .O(new_n63_));
  nand2  g39(.a(new_n44_), .b(x03), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n28_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n26_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n49_), .O(new_n67_));
  nand3  g43(.a(x14), .b(x02), .c(new_n40_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n36_), .c(x01), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n67_), .c(new_n63_), .O(z6));
  oai21  g46(.a(new_n28_), .b(x02), .c(new_n36_), .O(new_n71_));
  nand3  g47(.a(x09), .b(x03), .c(new_n26_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n71_), .c(new_n45_), .d(new_n25_), .O(z7));
  inv1   g49(.a(x12), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n25_), .O(new_n75_));
  nand3  g51(.a(new_n54_), .b(new_n44_), .c(x00), .O(new_n76_));
  oai21  g52(.a(new_n75_), .b(new_n72_), .c(new_n76_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n45_), .O(z8));
endmodule


