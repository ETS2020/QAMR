// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_;
  nand2  g00(.a(x07), .b(x02), .O(new_n25_));
  nand2  g01(.a(x04), .b(x02), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand4  g05(.a(new_n29_), .b(new_n26_), .c(new_n25_), .d(x03), .O(new_n30_));
  inv1   g06(.a(x07), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n31_), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand3  g12(.a(x03), .b(new_n28_), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n25_), .O(z1));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g22(.a(x03), .b(x00), .c(x07), .O(new_n47_));
  aoi21  g23(.a(x10), .b(x03), .c(x01), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x07), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n28_), .c(new_n51_), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n28_), .c(new_n31_), .O(new_n54_));
  aoi21  g30(.a(x12), .b(new_n36_), .c(new_n54_), .O(z3));
  aoi21  g31(.a(new_n51_), .b(x09), .c(new_n54_), .O(z4));
  nor2   g32(.a(x12), .b(x00), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n45_), .c(x13), .O(z5));
  inv1   g34(.a(x03), .O(new_n59_));
  nand2  g35(.a(x09), .b(new_n59_), .O(new_n60_));
  nand2  g36(.a(new_n44_), .b(x03), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n28_), .d(new_n27_), .O(new_n62_));
  nand4  g38(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n50_), .d(new_n45_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z6));
  nand3  g41(.a(x03), .b(new_n28_), .c(new_n27_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x08), .O(new_n67_));
  oai21  g43(.a(new_n61_), .b(new_n29_), .c(new_n67_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n45_), .O(z7));
  oai21  g45(.a(new_n66_), .b(x12), .c(x09), .O(new_n70_));
  nand2  g46(.a(new_n44_), .b(new_n36_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n45_), .O(z8));
endmodule


