// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x02), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x01), .O(new_n28_));
  nor2   g04(.a(x03), .b(x01), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n28_), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x02), .c(new_n27_), .O(new_n31_));
  nor2   g07(.a(x11), .b(x03), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  oai21  g09(.a(new_n31_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x02), .O(new_n36_));
  nand2  g12(.a(new_n26_), .b(new_n36_), .O(new_n37_));
  nand4  g13(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand4  g17(.a(new_n41_), .b(new_n38_), .c(new_n37_), .d(new_n35_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n33_), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x09), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n39_), .c(new_n35_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g24(.a(x08), .b(x00), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n44_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n29_), .b(x10), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n50_), .c(new_n33_), .O(z2));
  nand2  g31(.a(x12), .b(new_n35_), .O(new_n56_));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n56_), .c(new_n33_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  nor2   g35(.a(x07), .b(new_n39_), .O(new_n60_));
  inv1   g36(.a(x07), .O(new_n61_));
  inv1   g37(.a(x09), .O(new_n62_));
  oai21  g38(.a(new_n49_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  oai21  g39(.a(new_n60_), .b(x11), .c(new_n63_), .O(z4));
  nor2   g40(.a(x12), .b(x10), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(x13), .c(new_n61_), .d(new_n35_), .O(new_n66_));
  and2   g42(.a(new_n66_), .b(new_n33_), .O(z5));
  xor2a  g43(.a(x09), .b(x03), .O(new_n68_));
  nand2  g44(.a(new_n36_), .b(new_n44_), .O(new_n69_));
  nor2   g45(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n35_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n49_), .c(new_n45_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n70_), .c(new_n33_), .O(z6));
  inv1   g49(.a(x08), .O(new_n74_));
  oai22  g50(.a(new_n69_), .b(new_n39_), .c(new_n32_), .d(new_n74_), .O(new_n75_));
  nand4  g51(.a(x09), .b(x03), .c(new_n36_), .d(new_n44_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n45_), .O(z7));
  nand2  g53(.a(new_n62_), .b(x00), .O(new_n78_));
  oai22  g54(.a(new_n78_), .b(new_n32_), .c(new_n76_), .d(x12), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n45_), .O(z8));
endmodule


