// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_,
    new_n75_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x08), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n32_), .b(x00), .c(new_n36_), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  inv1   g14(.a(new_n26_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(x06), .c(x05), .O(new_n40_));
  nand2  g16(.a(x04), .b(x02), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n30_), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n40_), .c(new_n29_), .d(new_n38_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n36_), .O(z1));
  inv1   g20(.a(x01), .O(new_n45_));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n46_), .c(new_n30_), .O(new_n50_));
  nor2   g26(.a(x10), .b(x07), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x09), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n45_), .O(new_n54_));
  aoi21  g30(.a(new_n46_), .b(x02), .c(new_n35_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n54_), .O(z2));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n36_), .O(new_n58_));
  aoi21  g34(.a(x12), .b(new_n38_), .c(new_n58_), .O(z3));
  inv1   g35(.a(new_n58_), .O(z4));
  inv1   g36(.a(x12), .O(new_n61_));
  nand3  g37(.a(x13), .b(new_n61_), .c(new_n38_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n51_), .c(new_n35_), .O(z5));
  nand2  g40(.a(new_n34_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n30_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n28_), .O(new_n67_));
  nand3  g43(.a(new_n34_), .b(x08), .c(x00), .O(new_n68_));
  nand3  g44(.a(new_n39_), .b(x14), .c(new_n38_), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n51_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n36_), .O(z6));
  oai21  g47(.a(new_n65_), .b(new_n29_), .c(new_n33_), .O(new_n72_));
  oai21  g48(.a(new_n51_), .b(x09), .c(new_n72_), .O(z7));
  nand4  g49(.a(new_n61_), .b(x09), .c(new_n33_), .d(x03), .O(new_n74_));
  oai22  g50(.a(new_n74_), .b(new_n29_), .c(x09), .d(new_n38_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n51_), .O(z8));
endmodule


