// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_;
  nand2  g00(.a(x04), .b(x02), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n25_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  nand2  g09(.a(new_n28_), .b(x03), .O(new_n34_));
  inv1   g10(.a(x00), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n35_), .c(new_n27_), .O(new_n37_));
  aoi21  g13(.a(x04), .b(x02), .c(x00), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n34_), .c(new_n37_), .O(z1));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x08), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n40_), .c(new_n35_), .O(new_n42_));
  inv1   g18(.a(x09), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x03), .O(new_n48_));
  nand4  g24(.a(new_n48_), .b(new_n45_), .c(new_n26_), .d(new_n35_), .O(new_n49_));
  oai21  g25(.a(new_n42_), .b(new_n27_), .c(new_n49_), .O(z2));
  nor2   g26(.a(x02), .b(new_n35_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n35_), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nand3  g32(.a(x09), .b(x08), .c(new_n40_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(x02), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(x00), .c(x11), .d(x07), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(x13), .c(new_n40_), .d(new_n35_), .O(new_n61_));
  and2   g37(.a(new_n61_), .b(new_n52_), .O(z5));
  nand2  g38(.a(x14), .b(x01), .O(new_n63_));
  xor2a  g39(.a(x09), .b(x03), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(x02), .c(new_n63_), .O(new_n65_));
  aoi21  g41(.a(new_n27_), .b(x01), .c(x00), .O(new_n66_));
  oai21  g42(.a(x08), .b(new_n27_), .c(x00), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n44_), .O(new_n68_));
  aoi21  g44(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(z6));
  inv1   g45(.a(x03), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(x01), .c(new_n41_), .O(new_n71_));
  nand4  g47(.a(x09), .b(x03), .c(new_n27_), .d(new_n26_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n35_), .O(new_n74_));
  aoi21  g50(.a(new_n41_), .b(x02), .c(new_n47_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n74_), .c(new_n51_), .O(z7));
  oai21  g52(.a(new_n72_), .b(x12), .c(new_n35_), .O(new_n77_));
  nand2  g53(.a(new_n43_), .b(x02), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(x00), .c(new_n47_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n77_), .O(z8));
endmodule


