// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x02), .O(new_n25_));
  nor2   g01(.a(x04), .b(new_n25_), .O(new_n26_));
  nand3  g02(.a(x08), .b(new_n25_), .c(x01), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n25_), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n29_), .c(x00), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n30_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n35_), .c(x02), .O(new_n38_));
  nand2  g14(.a(new_n28_), .b(x03), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g16(.a(x00), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x09), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n31_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n25_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  oai21  g26(.a(new_n42_), .b(new_n25_), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n49_), .O(z2));
  and2   g28(.a(x11), .b(x07), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x00), .O(new_n55_));
  nand2  g31(.a(new_n50_), .b(new_n25_), .O(new_n56_));
  oai21  g32(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(z3));
  nand3  g33(.a(x09), .b(new_n42_), .c(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n53_), .c(new_n51_), .O(z4));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n54_), .d(new_n44_), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n56_), .O(z5));
  nor2   g39(.a(new_n50_), .b(x02), .O(new_n64_));
  xor2a  g40(.a(x09), .b(x03), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(x01), .c(new_n64_), .O(new_n66_));
  nand2  g42(.a(x14), .b(x01), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(x02), .c(x00), .O(new_n68_));
  nor2   g44(.a(x10), .b(x07), .O(new_n69_));
  nand2  g45(.a(new_n50_), .b(x02), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x00), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  aoi21  g48(.a(new_n68_), .b(new_n66_), .c(new_n72_), .O(z6));
  nor2   g49(.a(new_n43_), .b(new_n36_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n64_), .c(new_n31_), .O(new_n75_));
  oai21  g51(.a(x10), .b(x07), .c(x08), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n70_), .O(z7));
  nand3  g53(.a(new_n56_), .b(new_n43_), .c(x00), .O(new_n78_));
  oai21  g54(.a(new_n75_), .b(x12), .c(new_n78_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n69_), .O(z8));
endmodule


