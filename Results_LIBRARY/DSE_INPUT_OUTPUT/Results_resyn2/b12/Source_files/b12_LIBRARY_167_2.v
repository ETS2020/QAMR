// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x12), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x09), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(x01), .O(new_n31_));
  nor2   g07(.a(x02), .b(x01), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  nand2  g09(.a(x04), .b(x02), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(new_n33_), .c(x03), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n31_), .c(new_n28_), .O(z0));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n34_), .b(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nor3   g16(.a(new_n40_), .b(new_n32_), .c(new_n28_), .O(z1));
  inv1   g17(.a(x01), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x09), .O(new_n44_));
  nand3  g20(.a(x07), .b(new_n38_), .c(new_n25_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g22(.a(x08), .b(x00), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n42_), .O(new_n48_));
  inv1   g24(.a(new_n27_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  inv1   g26(.a(x02), .O(new_n51_));
  nand3  g27(.a(x10), .b(new_n38_), .c(new_n42_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n50_), .c(new_n49_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n48_), .O(z2));
  nand2  g31(.a(x11), .b(x07), .O(new_n56_));
  nand2  g32(.a(x12), .b(new_n25_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n56_), .c(new_n27_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  inv1   g35(.a(x07), .O(new_n60_));
  oai21  g36(.a(x09), .b(new_n60_), .c(new_n26_), .O(new_n61_));
  nand2  g37(.a(x09), .b(new_n60_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n47_), .c(new_n56_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n61_), .O(z4));
  inv1   g40(.a(x09), .O(new_n65_));
  nand3  g41(.a(new_n43_), .b(x13), .c(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n26_), .O(z5));
  nand2  g44(.a(new_n65_), .b(x03), .O(new_n69_));
  aoi21  g45(.a(x09), .b(new_n38_), .c(new_n33_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n47_), .c(new_n43_), .d(new_n27_), .O(new_n72_));
  aoi21  g48(.a(new_n70_), .b(new_n69_), .c(new_n72_), .O(z6));
  nand3  g49(.a(x03), .b(new_n51_), .c(new_n42_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g51(.a(new_n32_), .b(new_n65_), .c(x03), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n43_), .c(new_n49_), .O(z7));
  nor2   g54(.a(x09), .b(new_n25_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n43_), .c(new_n49_), .O(z8));
endmodule


