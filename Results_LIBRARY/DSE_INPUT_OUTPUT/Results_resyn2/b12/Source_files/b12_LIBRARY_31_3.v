// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_;
  inv1   g00(.a(x09), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(x03), .b(new_n27_), .O(new_n28_));
  nand2  g04(.a(x05), .b(x02), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x03), .O(new_n31_));
  nor3   g07(.a(x04), .b(new_n31_), .c(new_n27_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n30_), .c(new_n25_), .O(new_n33_));
  nor2   g09(.a(new_n27_), .b(new_n26_), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x04), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n34_), .c(new_n31_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n33_), .c(x00), .O(z0));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  aoi21  g16(.a(x09), .b(x03), .c(x00), .O(new_n41_));
  oai21  g17(.a(new_n31_), .b(new_n26_), .c(new_n27_), .O(new_n42_));
  nand3  g18(.a(new_n34_), .b(x06), .c(x05), .O(new_n43_));
  nand4  g19(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n40_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n26_), .O(new_n48_));
  oai21  g24(.a(x09), .b(new_n27_), .c(x03), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g27(.a(new_n48_), .b(new_n27_), .c(new_n51_), .O(z2));
  nand2  g28(.a(x09), .b(x03), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(z3));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x07), .c(new_n31_), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(x09), .c(x11), .d(x07), .O(z4));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n61_), .c(x13), .O(new_n63_));
  and2   g39(.a(new_n63_), .b(new_n53_), .O(z5));
  inv1   g40(.a(x00), .O(new_n65_));
  nand3  g41(.a(new_n34_), .b(x14), .c(new_n65_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n58_), .c(new_n46_), .O(new_n67_));
  nand4  g43(.a(new_n25_), .b(new_n31_), .c(new_n27_), .d(new_n26_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n67_), .b(new_n53_), .c(new_n69_), .O(z6));
  inv1   g46(.a(x08), .O(new_n71_));
  oai21  g47(.a(new_n28_), .b(x01), .c(new_n71_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n46_), .c(new_n53_), .O(z7));
  nand3  g49(.a(new_n46_), .b(new_n25_), .c(x00), .O(z8));
endmodule


