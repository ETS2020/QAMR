// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nor2   g07(.a(x14), .b(x04), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(new_n27_), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x06), .c(x05), .O(new_n36_));
  inv1   g12(.a(x01), .O(new_n37_));
  inv1   g13(.a(x02), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n37_), .c(new_n38_), .O(new_n40_));
  inv1   g16(.a(x04), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n39_), .c(x00), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n40_), .c(new_n36_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n33_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n37_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n33_), .O(z2));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  inv1   g31(.a(x12), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n33_), .O(z3));
  nand4  g34(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n55_), .c(new_n33_), .O(z4));
  nor3   g37(.a(new_n32_), .b(x07), .c(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n56_), .d(new_n48_), .O(z5));
  nor2   g39(.a(x10), .b(x07), .O(new_n64_));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  nor2   g41(.a(x02), .b(x01), .O(new_n66_));
  nand2  g42(.a(x09), .b(new_n39_), .O(new_n67_));
  nand2  g43(.a(new_n47_), .b(x03), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand3  g45(.a(new_n35_), .b(x14), .c(new_n25_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n69_), .c(new_n65_), .d(new_n64_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n33_), .O(z6));
  nand2  g48(.a(new_n66_), .b(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x08), .O(new_n74_));
  nand3  g50(.a(new_n66_), .b(new_n47_), .c(x03), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n64_), .c(new_n32_), .O(z7));
  oai21  g53(.a(new_n73_), .b(x12), .c(x09), .O(new_n78_));
  inv1   g54(.a(new_n64_), .O(new_n79_));
  aoi21  g55(.a(new_n47_), .b(new_n25_), .c(new_n79_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n78_), .c(new_n32_), .O(z8));
endmodule


