// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x09), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x07), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g07(.a(x04), .b(x02), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n31_), .c(x03), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x04), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x02), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand3  g13(.a(x03), .b(new_n30_), .c(x01), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  oai21  g16(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n25_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n27_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g23(.a(x10), .b(x07), .c(x03), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n26_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n48_), .c(new_n29_), .O(new_n51_));
  aoi22  g27(.a(new_n51_), .b(new_n30_), .c(new_n47_), .d(new_n28_), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(x09), .c(new_n45_), .O(new_n54_));
  aoi21  g30(.a(x12), .b(new_n25_), .c(new_n54_), .O(z3));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  nor2   g33(.a(new_n26_), .b(x07), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nor2   g36(.a(x10), .b(x07), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n60_), .d(new_n25_), .O(z5));
  nand3  g38(.a(x03), .b(new_n30_), .c(new_n29_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n45_), .c(new_n26_), .O(new_n64_));
  inv1   g40(.a(x03), .O(new_n65_));
  nand4  g41(.a(new_n26_), .b(new_n65_), .c(new_n30_), .d(new_n29_), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(new_n56_), .d(new_n49_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n45_), .c(new_n64_), .O(z6));
  inv1   g45(.a(new_n63_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x09), .O(new_n71_));
  nand2  g47(.a(new_n63_), .b(new_n46_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n61_), .O(z7));
  nor2   g49(.a(x09), .b(x00), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(x10), .c(new_n45_), .O(new_n75_));
  nand3  g51(.a(new_n60_), .b(new_n45_), .c(x03), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n31_), .c(x09), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n75_), .O(z8));
endmodule


