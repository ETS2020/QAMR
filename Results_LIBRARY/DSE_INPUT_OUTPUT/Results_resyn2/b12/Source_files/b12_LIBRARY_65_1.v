// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x09), .O(new_n32_));
  nand2  g08(.a(x14), .b(new_n32_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(x02), .O(new_n35_));
  nand3  g11(.a(x03), .b(new_n35_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  oai21  g14(.a(x04), .b(x03), .c(x02), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n33_), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x03), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n47_), .c(new_n25_), .O(new_n48_));
  oai21  g24(.a(new_n46_), .b(new_n32_), .c(new_n48_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n43_), .O(new_n51_));
  oai21  g27(.a(x08), .b(x07), .c(x00), .O(new_n52_));
  nor2   g28(.a(x10), .b(x09), .O(new_n53_));
  nand2  g29(.a(new_n47_), .b(new_n43_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n53_), .c(new_n35_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n51_), .c(new_n33_), .O(z2));
  nand2  g33(.a(x12), .b(new_n25_), .O(new_n58_));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n33_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  oai21  g37(.a(x14), .b(new_n44_), .c(new_n32_), .O(new_n62_));
  oai21  g38(.a(new_n50_), .b(x07), .c(new_n59_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n62_), .O(z4));
  inv1   g40(.a(x12), .O(new_n65_));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(x13), .c(new_n65_), .d(new_n45_), .O(new_n67_));
  and2   g43(.a(new_n67_), .b(new_n33_), .O(z5));
  nor2   g44(.a(x10), .b(x07), .O(new_n69_));
  nand2  g45(.a(new_n32_), .b(x03), .O(new_n70_));
  nand2  g46(.a(x09), .b(new_n47_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n35_), .d(new_n43_), .O(new_n72_));
  inv1   g48(.a(new_n27_), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(x14), .c(x09), .d(new_n25_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n72_), .c(new_n50_), .d(new_n69_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n33_), .O(z6));
  nand3  g52(.a(x03), .b(new_n35_), .c(new_n43_), .O(new_n77_));
  inv1   g53(.a(x14), .O(new_n78_));
  oai21  g54(.a(new_n77_), .b(new_n46_), .c(new_n78_), .O(new_n79_));
  nand2  g55(.a(new_n69_), .b(x08), .O(new_n80_));
  inv1   g56(.a(new_n80_), .O(new_n81_));
  aoi22  g57(.a(new_n81_), .b(new_n77_), .c(new_n79_), .d(new_n32_), .O(z7));
  nand2  g58(.a(new_n69_), .b(x00), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n78_), .c(x09), .O(new_n84_));
  nand3  g60(.a(new_n69_), .b(new_n65_), .c(x09), .O(new_n85_));
  nor2   g61(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  nor2   g62(.a(new_n86_), .b(new_n84_), .O(z8));
endmodule


