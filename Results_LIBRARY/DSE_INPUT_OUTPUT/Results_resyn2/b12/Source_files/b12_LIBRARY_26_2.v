// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x03), .b(x01), .c(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(z0));
  inv1   g06(.a(x02), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x01), .O(new_n32_));
  nand3  g08(.a(x06), .b(x05), .c(x01), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  aoi21  g10(.a(new_n28_), .b(new_n34_), .c(new_n26_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n32_), .O(z1));
  nand2  g13(.a(x07), .b(new_n34_), .O(new_n38_));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x10), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n25_), .O(new_n43_));
  nor2   g19(.a(x08), .b(x07), .O(new_n44_));
  oai21  g20(.a(new_n40_), .b(x03), .c(new_n31_), .O(new_n45_));
  oai21  g21(.a(new_n44_), .b(new_n25_), .c(new_n45_), .O(new_n46_));
  inv1   g22(.a(x01), .O(new_n47_));
  nor2   g23(.a(x02), .b(new_n47_), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(new_n49_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n44_), .c(new_n48_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n46_), .c(new_n43_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n32_), .O(z3));
  nor2   g32(.a(new_n49_), .b(x07), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n54_), .b(new_n32_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n32_), .d(x13), .O(z5));
  nand2  g40(.a(new_n49_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n34_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n31_), .O(new_n67_));
  inv1   g43(.a(x14), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x00), .c(x02), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x01), .O(new_n70_));
  nor2   g46(.a(x10), .b(x07), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n58_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(z6));
  nand3  g51(.a(new_n71_), .b(new_n49_), .c(x03), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n47_), .O(new_n77_));
  nand2  g53(.a(x03), .b(new_n31_), .O(new_n78_));
  nand2  g54(.a(new_n71_), .b(x08), .O(new_n79_));
  inv1   g55(.a(new_n79_), .O(new_n80_));
  aoi22  g56(.a(new_n80_), .b(new_n78_), .c(new_n77_), .d(new_n31_), .O(z7));
  nand2  g57(.a(new_n49_), .b(x00), .O(new_n82_));
  nor2   g58(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  nand3  g59(.a(new_n53_), .b(x09), .c(new_n47_), .O(new_n84_));
  nor2   g60(.a(new_n84_), .b(new_n78_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n83_), .c(new_n71_), .O(z8));
endmodule


