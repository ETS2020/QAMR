// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nand2  g07(.a(x14), .b(x11), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(z0));
  inv1   g09(.a(new_n27_), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(x06), .c(x05), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n36_), .c(new_n37_), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n32_), .O(z1));
  oai21  g19(.a(x08), .b(x07), .c(x00), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n36_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n37_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n44_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n32_), .O(z2));
  inv1   g29(.a(x11), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  oai22  g31(.a(new_n55_), .b(x00), .c(new_n54_), .d(new_n45_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n32_), .O(z3));
  inv1   g33(.a(x14), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n45_), .c(new_n54_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(new_n25_), .O(new_n61_));
  nor2   g37(.a(new_n46_), .b(x07), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nor2   g40(.a(x07), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n32_), .d(x13), .O(z5));
  nor2   g42(.a(x02), .b(x01), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  and2   g44(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g45(.a(x10), .b(x07), .O(new_n70_));
  inv1   g46(.a(new_n61_), .O(new_n71_));
  nand3  g47(.a(new_n34_), .b(x14), .c(new_n25_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n69_), .c(new_n32_), .O(z6));
  nand3  g50(.a(new_n67_), .b(x09), .c(x03), .O(new_n75_));
  inv1   g51(.a(new_n70_), .O(new_n76_));
  nand2  g52(.a(new_n67_), .b(x03), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n60_), .c(new_n76_), .O(new_n78_));
  aoi22  g54(.a(new_n78_), .b(new_n75_), .c(x14), .d(x11), .O(z7));
  oai22  g55(.a(new_n75_), .b(x12), .c(x09), .d(new_n25_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n70_), .c(new_n32_), .O(z8));
endmodule


