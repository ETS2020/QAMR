// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_;
  inv1   g00(.a(x12), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  nand2  g10(.a(x02), .b(x01), .O(new_n35_));
  aoi21  g11(.a(new_n34_), .b(x04), .c(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n33_), .c(new_n27_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n26_), .O(z0));
  nand3  g14(.a(x03), .b(new_n29_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(new_n41_));
  nand4  g17(.a(new_n41_), .b(new_n40_), .c(x12), .d(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g23(.a(x10), .b(x07), .c(x03), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n44_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n25_), .b(x02), .c(new_n27_), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(new_n52_), .c(new_n49_), .d(new_n29_), .O(z2));
  inv1   g30(.a(x11), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(new_n26_), .c(x12), .d(new_n27_), .O(z3));
  nor2   g33(.a(new_n51_), .b(new_n27_), .O(new_n58_));
  nor2   g34(.a(new_n45_), .b(x07), .O(new_n59_));
  oai21  g35(.a(new_n55_), .b(new_n50_), .c(new_n26_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z4));
  nor2   g37(.a(x02), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n46_), .c(x13), .d(new_n25_), .O(z5));
  nand3  g39(.a(x14), .b(x01), .c(new_n27_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(x12), .c(x02), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n28_), .O(new_n66_));
  nand2  g42(.a(new_n45_), .b(x03), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n44_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n29_), .O(new_n69_));
  oai21  g45(.a(new_n51_), .b(new_n27_), .c(new_n46_), .O(new_n70_));
  aoi21  g46(.a(new_n69_), .b(new_n65_), .c(new_n70_), .O(z6));
  nand2  g47(.a(x03), .b(new_n44_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n51_), .O(new_n73_));
  oai21  g49(.a(new_n25_), .b(new_n51_), .c(x02), .O(new_n74_));
  nand4  g50(.a(x09), .b(x03), .c(new_n29_), .d(new_n44_), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n46_), .O(z7));
  inv1   g52(.a(new_n47_), .O(new_n77_));
  nand2  g53(.a(new_n46_), .b(x09), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n72_), .c(new_n29_), .O(new_n79_));
  aoi22  g55(.a(new_n79_), .b(new_n25_), .c(new_n77_), .d(x00), .O(z8));
endmodule


