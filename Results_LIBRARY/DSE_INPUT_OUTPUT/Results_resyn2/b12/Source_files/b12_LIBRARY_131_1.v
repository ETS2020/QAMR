// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  nand3  g03(.a(x03), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n27_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  nand2  g11(.a(x10), .b(new_n25_), .O(new_n36_));
  aoi21  g12(.a(x04), .b(x02), .c(x03), .O(new_n37_));
  nand4  g13(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n38_));
  nor2   g14(.a(x02), .b(x01), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(new_n27_), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n37_), .c(new_n36_), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  nand2  g20(.a(x09), .b(new_n44_), .O(new_n45_));
  inv1   g21(.a(x03), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n46_), .c(new_n27_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g24(.a(x08), .b(x00), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n43_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x02), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n50_), .c(new_n36_), .O(z2));
  nand2  g29(.a(x12), .b(new_n27_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n36_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  inv1   g33(.a(new_n45_), .O(new_n58_));
  inv1   g34(.a(new_n49_), .O(new_n59_));
  nand2  g35(.a(new_n55_), .b(new_n36_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z4));
  inv1   g37(.a(x12), .O(new_n62_));
  nor2   g38(.a(x10), .b(x07), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n62_), .d(new_n27_), .O(z5));
  nand3  g40(.a(x14), .b(x01), .c(new_n27_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n26_), .c(new_n25_), .O(new_n66_));
  inv1   g42(.a(x09), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x03), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n46_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n39_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n49_), .c(new_n44_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n26_), .c(new_n66_), .O(z6));
  inv1   g48(.a(new_n36_), .O(new_n73_));
  nand3  g49(.a(x03), .b(new_n25_), .c(new_n43_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n26_), .c(x08), .O(new_n75_));
  nand3  g51(.a(new_n39_), .b(new_n67_), .c(x03), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n44_), .c(new_n73_), .O(z7));
  nand3  g54(.a(new_n62_), .b(x03), .c(new_n43_), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n45_), .c(new_n26_), .O(new_n80_));
  nor2   g56(.a(x09), .b(new_n27_), .O(new_n81_));
  aoi22  g57(.a(new_n81_), .b(new_n63_), .c(new_n80_), .d(new_n25_), .O(z8));
endmodule


