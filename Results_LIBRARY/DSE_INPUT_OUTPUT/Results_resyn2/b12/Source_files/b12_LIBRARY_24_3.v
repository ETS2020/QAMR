// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x10), .b(x09), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  inv1   g08(.a(x04), .O(new_n33_));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g12(.a(x05), .b(new_n33_), .c(new_n36_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n32_), .c(new_n27_), .O(z0));
  nand3  g14(.a(x03), .b(new_n35_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n33_), .b(new_n29_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n39_), .c(new_n27_), .O(z1));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x07), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(x03), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n45_), .c(new_n34_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n26_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n35_), .c(new_n51_), .O(z2));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n26_), .O(new_n54_));
  aoi21  g30(.a(x12), .b(new_n25_), .c(new_n54_), .O(z3));
  nand2  g31(.a(new_n47_), .b(x00), .O(new_n56_));
  nand3  g32(.a(x09), .b(x08), .c(new_n46_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n26_), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nor2   g36(.a(x10), .b(x07), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n60_), .d(new_n25_), .O(z5));
  nand2  g38(.a(x09), .b(new_n29_), .O(new_n63_));
  nand2  g39(.a(new_n44_), .b(x03), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n63_), .c(new_n30_), .O(new_n65_));
  nand3  g41(.a(new_n36_), .b(x14), .c(new_n25_), .O(new_n66_));
  nand2  g42(.a(x08), .b(x00), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n61_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(z6));
  inv1   g45(.a(new_n26_), .O(new_n70_));
  nand3  g46(.a(x03), .b(new_n35_), .c(new_n34_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x08), .O(new_n72_));
  nand3  g48(.a(new_n30_), .b(new_n44_), .c(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n61_), .c(new_n70_), .O(z7));
  nand2  g51(.a(new_n56_), .b(new_n44_), .O(new_n76_));
  nand4  g52(.a(new_n60_), .b(x03), .c(new_n35_), .d(new_n34_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(x09), .c(x07), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x10), .c(new_n76_), .O(z8));
endmodule


