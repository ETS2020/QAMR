// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  oai21  g01(.a(x02), .b(x01), .c(x03), .O(new_n26_));
  aoi21  g02(.a(x04), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(x02), .b(x01), .O(new_n29_));
  aoi21  g05(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n27_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x10), .O(new_n32_));
  nand2  g08(.a(x11), .b(new_n32_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(x02), .O(new_n35_));
  nand3  g11(.a(x03), .b(new_n35_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n37_), .c(x02), .O(new_n41_));
  nand2  g17(.a(new_n33_), .b(new_n25_), .O(new_n42_));
  aoi21  g18(.a(new_n41_), .b(new_n36_), .c(new_n42_), .O(z1));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  nand3  g21(.a(new_n32_), .b(x09), .c(new_n45_), .O(new_n46_));
  nand3  g22(.a(x07), .b(new_n38_), .c(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g24(.a(x08), .b(x00), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n44_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand3  g27(.a(x10), .b(new_n38_), .c(new_n44_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n35_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n50_), .c(new_n33_), .O(z2));
  inv1   g31(.a(x11), .O(new_n56_));
  aoi21  g32(.a(x10), .b(new_n45_), .c(new_n56_), .O(new_n57_));
  aoi21  g33(.a(x12), .b(new_n25_), .c(new_n57_), .O(z3));
  inv1   g34(.a(new_n49_), .O(new_n59_));
  inv1   g35(.a(x09), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(x07), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n59_), .c(new_n57_), .O(z4));
  inv1   g38(.a(x12), .O(new_n63_));
  nand4  g39(.a(x13), .b(new_n63_), .c(new_n45_), .d(new_n25_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n56_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n32_), .O(z5));
  nor2   g42(.a(x02), .b(x01), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n69_));
  nor3   g45(.a(x11), .b(x10), .c(x07), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n49_), .O(new_n71_));
  aoi21  g47(.a(new_n68_), .b(new_n67_), .c(new_n71_), .O(z6));
  nand4  g48(.a(x09), .b(x03), .c(new_n35_), .d(new_n44_), .O(new_n73_));
  inv1   g49(.a(x08), .O(new_n74_));
  nand3  g50(.a(x03), .b(new_n35_), .c(new_n44_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z7));
  oai22  g53(.a(new_n73_), .b(x12), .c(x09), .d(new_n25_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n70_), .O(z8));
endmodule


