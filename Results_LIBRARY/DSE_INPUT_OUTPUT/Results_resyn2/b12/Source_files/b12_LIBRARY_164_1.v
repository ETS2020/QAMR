// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  nand2  g10(.a(x10), .b(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n27_), .c(new_n36_), .O(z0));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  inv1   g16(.a(x02), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n41_), .c(x01), .O(new_n42_));
  oai21  g18(.a(new_n40_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n35_), .O(z1));
  inv1   g21(.a(x01), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(x02), .O(new_n48_));
  nor2   g24(.a(x07), .b(x02), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(x10), .c(new_n34_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(new_n48_), .O(z2));
  nand2  g29(.a(x12), .b(new_n33_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n35_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  aoi21  g33(.a(x10), .b(new_n34_), .c(new_n55_), .O(new_n58_));
  nand2  g34(.a(x08), .b(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  nor2   g36(.a(new_n34_), .b(x07), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z4));
  inv1   g38(.a(x07), .O(new_n63_));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(x13), .c(new_n63_), .d(new_n33_), .O(z5));
  inv1   g41(.a(x10), .O(new_n66_));
  nand3  g42(.a(x03), .b(new_n41_), .c(new_n46_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n66_), .c(new_n34_), .O(new_n68_));
  nand3  g44(.a(new_n30_), .b(new_n34_), .c(new_n29_), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n69_), .c(new_n59_), .d(new_n63_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n66_), .c(new_n68_), .O(z6));
  nand2  g48(.a(new_n67_), .b(new_n66_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x09), .O(new_n74_));
  inv1   g50(.a(x08), .O(new_n75_));
  aoi21  g51(.a(new_n67_), .b(new_n75_), .c(x07), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(x10), .c(new_n74_), .O(z7));
  nor2   g53(.a(x09), .b(x00), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x07), .c(new_n66_), .O(new_n79_));
  nand3  g55(.a(new_n64_), .b(new_n30_), .c(x03), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x09), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n79_), .O(z8));
endmodule


