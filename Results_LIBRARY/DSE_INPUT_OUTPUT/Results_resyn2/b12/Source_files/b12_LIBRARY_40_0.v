// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  nand3  g02(.a(x03), .b(x01), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x09), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n29_), .O(z0));
  nor2   g10(.a(x09), .b(x02), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  nand2  g12(.a(x06), .b(x02), .O(new_n37_));
  aoi22  g13(.a(new_n37_), .b(x01), .c(new_n30_), .d(x02), .O(new_n38_));
  and2   g14(.a(x04), .b(x02), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(x03), .c(new_n26_), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(z1));
  oai21  g17(.a(x08), .b(x07), .c(x00), .O(new_n42_));
  oai21  g18(.a(new_n31_), .b(x02), .c(new_n42_), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(x08), .b(x00), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n43_), .c(new_n36_), .O(z2));
  nand2  g24(.a(x12), .b(new_n26_), .O(new_n49_));
  nand2  g25(.a(x11), .b(x07), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n36_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(z3));
  inv1   g28(.a(new_n46_), .O(new_n53_));
  nand2  g29(.a(new_n50_), .b(new_n36_), .O(new_n54_));
  inv1   g30(.a(x09), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x07), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n53_), .c(new_n54_), .O(z4));
  inv1   g33(.a(x07), .O(new_n58_));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n58_), .d(new_n26_), .O(new_n60_));
  and2   g36(.a(new_n60_), .b(new_n36_), .O(z5));
  nand3  g37(.a(new_n46_), .b(new_n45_), .c(new_n36_), .O(new_n62_));
  nand4  g38(.a(x14), .b(x02), .c(x01), .d(new_n26_), .O(new_n63_));
  nand3  g39(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor2   g41(.a(new_n65_), .b(new_n62_), .O(z6));
  nand2  g42(.a(new_n45_), .b(x08), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n64_), .c(new_n35_), .O(z7));
  nand2  g45(.a(x03), .b(new_n44_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(x12), .c(x09), .O(new_n71_));
  inv1   g47(.a(x10), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n58_), .O(new_n73_));
  nand2  g49(.a(new_n55_), .b(x00), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(x02), .c(new_n73_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n71_), .c(new_n35_), .O(z8));
endmodule


