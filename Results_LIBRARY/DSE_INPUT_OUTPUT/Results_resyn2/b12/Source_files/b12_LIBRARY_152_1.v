// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(x14), .b(new_n29_), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  nand4  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .d(x03), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  nand2  g13(.a(new_n28_), .b(new_n37_), .O(new_n38_));
  nand4  g14(.a(new_n38_), .b(new_n36_), .c(new_n32_), .d(new_n35_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n30_), .O(z1));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x08), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n41_), .c(new_n35_), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x14), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand4  g25(.a(new_n49_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n29_), .c(new_n43_), .O(z2));
  nand2  g27(.a(x12), .b(new_n35_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n30_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  nand2  g32(.a(x09), .b(new_n41_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n30_), .O(z4));
  nor2   g35(.a(new_n45_), .b(x02), .O(new_n60_));
  inv1   g36(.a(x10), .O(new_n61_));
  inv1   g37(.a(x12), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(x13), .c(new_n41_), .d(new_n35_), .O(z5));
  nand2  g41(.a(x01), .b(new_n35_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(x02), .c(new_n45_), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n31_), .O(new_n69_));
  nand3  g45(.a(new_n56_), .b(new_n61_), .c(new_n41_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nor2   g48(.a(new_n72_), .b(new_n67_), .O(z6));
  nand3  g49(.a(new_n31_), .b(x09), .c(x03), .O(new_n74_));
  nand2  g50(.a(new_n61_), .b(new_n41_), .O(new_n75_));
  nand3  g51(.a(x03), .b(new_n29_), .c(new_n44_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n42_), .c(new_n75_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n74_), .c(new_n60_), .O(z7));
  nand4  g54(.a(x09), .b(new_n41_), .c(x03), .d(new_n44_), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n63_), .c(new_n45_), .O(new_n80_));
  nand3  g56(.a(new_n48_), .b(new_n47_), .c(x00), .O(new_n81_));
  inv1   g57(.a(new_n81_), .O(new_n82_));
  aoi21  g58(.a(new_n80_), .b(new_n29_), .c(new_n82_), .O(z8));
endmodule


