// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x06), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(x01), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  oai21  g11(.a(new_n32_), .b(x00), .c(new_n35_), .O(z0));
  nand2  g12(.a(x05), .b(x02), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(x01), .c(new_n33_), .O(new_n38_));
  inv1   g14(.a(x00), .O(new_n39_));
  oai21  g15(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n38_), .O(z1));
  inv1   g18(.a(x02), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(new_n46_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n47_), .c(new_n44_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n35_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n43_), .c(new_n52_), .O(z2));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n54_), .c(new_n35_), .O(z3));
  inv1   g33(.a(x07), .O(new_n58_));
  nand4  g34(.a(x09), .b(x08), .c(new_n58_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n54_), .c(new_n35_), .O(z4));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n35_), .d(x13), .O(z5));
  xnor2a g40(.a(x09), .b(x03), .O(new_n65_));
  and2   g41(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand2  g42(.a(x08), .b(x00), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n39_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n46_), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n66_), .c(new_n35_), .O(z6));
  inv1   g46(.a(x08), .O(new_n71_));
  oai21  g47(.a(new_n29_), .b(x02), .c(new_n71_), .O(new_n72_));
  nand3  g48(.a(new_n30_), .b(x09), .c(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n33_), .O(new_n75_));
  aoi21  g51(.a(new_n71_), .b(x01), .c(new_n48_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(new_n34_), .O(z7));
  inv1   g53(.a(new_n47_), .O(new_n78_));
  nand2  g54(.a(x03), .b(new_n43_), .O(new_n79_));
  nand3  g55(.a(new_n63_), .b(x09), .c(new_n58_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n79_), .c(new_n33_), .O(new_n81_));
  aoi22  g57(.a(new_n81_), .b(new_n44_), .c(new_n78_), .d(x00), .O(z8));
endmodule


