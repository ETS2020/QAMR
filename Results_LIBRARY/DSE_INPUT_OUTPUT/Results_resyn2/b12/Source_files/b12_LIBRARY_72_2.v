// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x00), .O(new_n25_));
  oai21  g01(.a(x02), .b(x01), .c(x03), .O(new_n26_));
  aoi21  g02(.a(x04), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(x02), .b(x01), .O(new_n29_));
  aoi21  g05(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n27_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x09), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(x02), .O(new_n35_));
  nand3  g11(.a(x03), .b(new_n35_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n37_), .c(x09), .d(x02), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n36_), .c(x00), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n38_), .c(new_n43_), .O(new_n45_));
  oai21  g21(.a(new_n44_), .b(x02), .c(new_n32_), .O(new_n46_));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g24(.a(new_n45_), .b(new_n35_), .c(new_n48_), .O(z2));
  nand2  g25(.a(x12), .b(new_n25_), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n33_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  inv1   g29(.a(x07), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(x02), .c(new_n32_), .O(new_n55_));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(x07), .c(new_n51_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n33_), .d(x13), .O(z5));
  nand3  g37(.a(x14), .b(x01), .c(new_n25_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(x09), .c(x02), .O(new_n63_));
  nand2  g39(.a(x09), .b(new_n38_), .O(new_n64_));
  nand2  g40(.a(new_n32_), .b(x03), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n43_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  nand2  g43(.a(new_n56_), .b(new_n44_), .O(new_n68_));
  aoi21  g44(.a(new_n67_), .b(new_n63_), .c(new_n68_), .O(z6));
  nand3  g45(.a(x03), .b(new_n35_), .c(new_n43_), .O(new_n70_));
  oai21  g46(.a(x08), .b(new_n35_), .c(new_n70_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x09), .O(new_n72_));
  inv1   g48(.a(new_n44_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n33_), .O(new_n74_));
  nor2   g50(.a(x08), .b(x02), .O(new_n75_));
  oai21  g51(.a(new_n38_), .b(x01), .c(new_n75_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(z7));
  oai21  g53(.a(new_n70_), .b(x12), .c(x09), .O(new_n78_));
  nand3  g54(.a(new_n32_), .b(new_n35_), .c(new_n25_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n78_), .c(new_n74_), .O(z8));
endmodule


