// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand3  g03(.a(x11), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n27_), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n25_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(new_n28_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x03), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n25_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n36_), .c(x00), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x10), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n46_), .c(x01), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  inv1   g25(.a(x11), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x02), .O(new_n51_));
  inv1   g27(.a(x00), .O(new_n52_));
  nor2   g28(.a(x08), .b(x07), .O(new_n53_));
  nor2   g29(.a(x11), .b(x02), .O(new_n54_));
  nor3   g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(new_n49_), .c(new_n55_), .O(z2));
  inv1   g32(.a(x12), .O(new_n57_));
  aoi21  g33(.a(x11), .b(x07), .c(new_n54_), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(x00), .c(new_n58_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n42_), .d(x00), .O(new_n61_));
  and2   g37(.a(new_n61_), .b(new_n58_), .O(z4));
  inv1   g38(.a(new_n54_), .O(new_n63_));
  nand2  g39(.a(new_n47_), .b(new_n57_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n63_), .c(x13), .d(new_n52_), .O(z5));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  nand2  g43(.a(new_n27_), .b(new_n31_), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g45(.a(x08), .b(x00), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n52_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n47_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n69_), .c(new_n63_), .O(z6));
  nand3  g49(.a(x09), .b(x03), .c(new_n31_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(x11), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n27_), .O(new_n76_));
  inv1   g52(.a(x08), .O(new_n77_));
  nand3  g53(.a(x03), .b(new_n27_), .c(new_n31_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n77_), .c(new_n44_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n76_), .O(z7));
  oai21  g56(.a(new_n74_), .b(new_n64_), .c(x11), .O(new_n81_));
  nand3  g57(.a(new_n47_), .b(new_n46_), .c(x00), .O(new_n82_));
  inv1   g58(.a(new_n82_), .O(new_n83_));
  aoi21  g59(.a(new_n81_), .b(new_n27_), .c(new_n83_), .O(z8));
endmodule


