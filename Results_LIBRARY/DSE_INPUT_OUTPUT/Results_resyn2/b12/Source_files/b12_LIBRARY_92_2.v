// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x03), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(x05), .c(new_n26_), .O(new_n30_));
  inv1   g06(.a(x00), .O(new_n31_));
  nand2  g07(.a(x01), .b(new_n31_), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n30_), .O(z0));
  nand2  g09(.a(new_n26_), .b(x01), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n29_), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g13(.a(new_n29_), .O(new_n38_));
  inv1   g14(.a(x07), .O(new_n39_));
  inv1   g15(.a(x08), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(new_n31_), .O(new_n41_));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(x03), .O(new_n45_));
  nor2   g21(.a(x02), .b(x01), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n38_), .c(new_n41_), .O(z2));
  inv1   g24(.a(x12), .O(new_n49_));
  nand2  g25(.a(x11), .b(x07), .O(new_n50_));
  oai21  g26(.a(new_n49_), .b(x00), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n28_), .b(x02), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(z3));
  nand2  g29(.a(x08), .b(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  nor2   g31(.a(new_n42_), .b(x07), .O(new_n56_));
  nand2  g32(.a(new_n52_), .b(new_n50_), .O(new_n57_));
  aoi21  g33(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n52_), .d(x13), .O(z5));
  nand2  g37(.a(new_n42_), .b(x03), .O(new_n62_));
  nand2  g38(.a(x09), .b(new_n25_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n46_), .O(new_n64_));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n31_), .O(new_n65_));
  inv1   g41(.a(x10), .O(new_n66_));
  nand3  g42(.a(new_n54_), .b(new_n66_), .c(new_n39_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n65_), .c(new_n64_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n52_), .O(z6));
  oai21  g46(.a(new_n25_), .b(x01), .c(new_n40_), .O(new_n71_));
  oai21  g47(.a(new_n40_), .b(new_n28_), .c(x02), .O(new_n72_));
  inv1   g48(.a(x01), .O(new_n73_));
  nand4  g49(.a(x09), .b(x03), .c(new_n27_), .d(new_n73_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n72_), .c(new_n71_), .d(new_n43_), .O(z7));
  nor2   g51(.a(x04), .b(new_n27_), .O(new_n76_));
  nand2  g52(.a(new_n42_), .b(x00), .O(new_n77_));
  oai22  g53(.a(new_n77_), .b(new_n76_), .c(new_n74_), .d(x12), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n43_), .O(z8));
endmodule


