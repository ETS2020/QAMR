// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(new_n25_), .b(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g07(.a(x14), .b(new_n27_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n31_), .c(x03), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g11(.a(x03), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  nand3  g13(.a(x14), .b(new_n27_), .c(new_n26_), .O(new_n38_));
  inv1   g14(.a(x14), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(new_n37_), .O(new_n42_));
  aoi21  g18(.a(new_n33_), .b(new_n36_), .c(new_n42_), .O(z1));
  inv1   g19(.a(x00), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  nor2   g23(.a(new_n39_), .b(x01), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(x03), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n51_), .c(new_n48_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n27_), .c(new_n47_), .O(z2));
  nand2  g32(.a(x12), .b(new_n44_), .O(new_n57_));
  nor2   g33(.a(x14), .b(x02), .O(new_n58_));
  and2   g34(.a(x11), .b(x07), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(z3));
  nand4  g38(.a(x09), .b(x08), .c(new_n45_), .d(x00), .O(new_n63_));
  inv1   g39(.a(new_n63_), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(new_n59_), .c(new_n40_), .O(z4));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nor2   g42(.a(x12), .b(x10), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(new_n40_), .d(x13), .O(z5));
  nand3  g44(.a(x14), .b(x01), .c(new_n44_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x02), .O(new_n70_));
  inv1   g46(.a(new_n32_), .O(new_n71_));
  nand2  g47(.a(new_n49_), .b(x03), .O(new_n72_));
  nand2  g48(.a(x09), .b(new_n36_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n26_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai21  g51(.a(new_n46_), .b(new_n44_), .c(new_n50_), .O(new_n76_));
  aoi21  g52(.a(new_n75_), .b(new_n70_), .c(new_n76_), .O(z6));
  nand3  g53(.a(x03), .b(new_n27_), .c(new_n26_), .O(new_n78_));
  inv1   g54(.a(new_n78_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(x09), .O(new_n80_));
  aoi21  g56(.a(new_n78_), .b(new_n46_), .c(new_n53_), .O(new_n81_));
  aoi21  g57(.a(new_n81_), .b(new_n80_), .c(new_n58_), .O(z7));
  nand2  g58(.a(x03), .b(new_n26_), .O(new_n83_));
  inv1   g59(.a(x12), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n83_), .c(x14), .O(new_n86_));
  nand3  g62(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n87_));
  inv1   g63(.a(new_n87_), .O(new_n88_));
  aoi21  g64(.a(new_n86_), .b(new_n27_), .c(new_n88_), .O(z8));
endmodule


