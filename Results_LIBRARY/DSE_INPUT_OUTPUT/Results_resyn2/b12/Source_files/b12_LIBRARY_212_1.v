// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x04), .O(new_n25_));
  oai21  g01(.a(x02), .b(x01), .c(x03), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g05(.a(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x05), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(x00), .O(z0));
  nand2  g08(.a(new_n26_), .b(new_n25_), .O(new_n33_));
  nand3  g09(.a(new_n30_), .b(x06), .c(x05), .O(new_n34_));
  inv1   g10(.a(x00), .O(new_n35_));
  inv1   g11(.a(x02), .O(new_n36_));
  aoi21  g12(.a(x04), .b(new_n36_), .c(new_n35_), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n34_), .c(new_n33_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(z1));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x08), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g23(.a(x10), .b(x07), .c(x03), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n44_), .O(new_n49_));
  nor2   g25(.a(x04), .b(x02), .O(new_n50_));
  aoi22  g26(.a(new_n50_), .b(new_n49_), .c(new_n43_), .d(new_n37_), .O(z2));
  nand2  g27(.a(x04), .b(new_n36_), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n35_), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nand2  g32(.a(new_n54_), .b(new_n52_), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  nor2   g35(.a(new_n45_), .b(x07), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n41_), .d(new_n35_), .O(new_n63_));
  and2   g39(.a(new_n63_), .b(new_n52_), .O(z5));
  nand2  g40(.a(new_n45_), .b(x03), .O(new_n65_));
  inv1   g41(.a(x03), .O(new_n66_));
  nand2  g42(.a(x09), .b(new_n66_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n65_), .c(new_n44_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n25_), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n35_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n58_), .c(new_n46_), .O(new_n71_));
  aoi21  g47(.a(new_n69_), .b(new_n36_), .c(new_n71_), .O(z6));
  nand2  g48(.a(x03), .b(new_n44_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n45_), .c(new_n25_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n36_), .O(new_n75_));
  nand3  g51(.a(x03), .b(new_n36_), .c(new_n44_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n42_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n46_), .O(z7));
  inv1   g54(.a(x10), .O(new_n79_));
  inv1   g55(.a(x12), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(x09), .d(new_n41_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n73_), .c(new_n25_), .O(new_n82_));
  nand3  g58(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(new_n84_));
  aoi21  g60(.a(new_n82_), .b(new_n36_), .c(new_n84_), .O(z8));
endmodule


