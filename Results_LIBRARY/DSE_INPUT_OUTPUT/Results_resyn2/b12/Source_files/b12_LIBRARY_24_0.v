// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x09), .O(new_n26_));
  inv1   g02(.a(x10), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n30_), .c(new_n33_), .O(new_n34_));
  nand2  g10(.a(x04), .b(x02), .O(new_n35_));
  nand2  g11(.a(new_n32_), .b(new_n31_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x03), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  nand3  g14(.a(x03), .b(new_n32_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(x03), .O(new_n41_));
  nand2  g17(.a(new_n30_), .b(new_n41_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n40_), .c(x02), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n39_), .c(new_n29_), .O(z1));
  oai21  g20(.a(new_n27_), .b(new_n41_), .c(new_n31_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n32_), .O(new_n46_));
  oai21  g22(.a(new_n41_), .b(x02), .c(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x07), .O(new_n48_));
  aoi22  g24(.a(new_n27_), .b(new_n26_), .c(x08), .d(x00), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(z2));
  nand2  g27(.a(x12), .b(new_n25_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n28_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  inv1   g31(.a(x07), .O(new_n56_));
  oai21  g32(.a(new_n27_), .b(new_n56_), .c(new_n26_), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x07), .c(new_n53_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n57_), .O(z4));
  inv1   g36(.a(x12), .O(new_n61_));
  nor2   g37(.a(x10), .b(x07), .O(new_n62_));
  nor2   g38(.a(new_n26_), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(x13), .d(new_n61_), .O(z5));
  nand3  g40(.a(new_n62_), .b(new_n58_), .c(x09), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n66_));
  nand3  g42(.a(x03), .b(new_n32_), .c(new_n31_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(new_n65_), .O(z6));
  nand3  g45(.a(new_n67_), .b(x08), .c(new_n56_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x09), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n27_), .O(z7));
  nand3  g48(.a(new_n61_), .b(new_n56_), .c(x03), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n36_), .c(x09), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n27_), .O(z8));
endmodule


