// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x09), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x02), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(new_n28_), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n31_), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n30_), .c(x00), .O(z0));
  nand3  g11(.a(x03), .b(new_n25_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n37_), .c(new_n31_), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n36_), .c(x00), .O(z1));
  oai21  g18(.a(x10), .b(x07), .c(new_n38_), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x09), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(x00), .c(x01), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand2  g27(.a(new_n27_), .b(x00), .O(new_n52_));
  aoi21  g28(.a(new_n48_), .b(new_n47_), .c(new_n52_), .O(new_n53_));
  aoi21  g29(.a(new_n51_), .b(new_n25_), .c(new_n53_), .O(z2));
  nand2  g30(.a(x09), .b(x02), .O(new_n55_));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z3));
  oai21  g35(.a(x09), .b(new_n47_), .c(x02), .O(new_n60_));
  nand4  g36(.a(x09), .b(x08), .c(new_n47_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n56_), .c(new_n60_), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nor2   g40(.a(x07), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n55_), .d(x13), .O(z5));
  inv1   g42(.a(x00), .O(new_n67_));
  nand3  g43(.a(x14), .b(x01), .c(new_n67_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n31_), .O(new_n69_));
  inv1   g45(.a(x01), .O(new_n70_));
  nand2  g46(.a(x09), .b(new_n38_), .O(new_n71_));
  nand2  g47(.a(new_n27_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n25_), .O(new_n74_));
  oai21  g50(.a(new_n48_), .b(new_n67_), .c(new_n44_), .O(new_n75_));
  aoi21  g51(.a(new_n74_), .b(new_n69_), .c(new_n75_), .O(z6));
  nand3  g52(.a(x03), .b(new_n25_), .c(new_n70_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n48_), .O(new_n78_));
  nand3  g54(.a(x09), .b(x03), .c(new_n70_), .O(new_n79_));
  nand4  g55(.a(new_n79_), .b(new_n78_), .c(new_n55_), .d(new_n44_), .O(z7));
  oai21  g56(.a(new_n79_), .b(x12), .c(new_n52_), .O(new_n81_));
  aoi22  g57(.a(new_n81_), .b(new_n44_), .c(x09), .d(x02), .O(z8));
endmodule


