// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x01), .O(new_n31_));
  inv1   g07(.a(x04), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n28_), .c(new_n25_), .O(new_n35_));
  nand2  g11(.a(x06), .b(x02), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(z0));
  nand3  g13(.a(x03), .b(new_n30_), .c(x01), .O(new_n38_));
  nor2   g14(.a(x06), .b(new_n30_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(new_n39_), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(x00), .O(z1));
  inv1   g17(.a(x01), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nor2   g19(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n44_), .c(new_n42_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n30_), .c(new_n51_), .O(z2));
  inv1   g28(.a(new_n36_), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x00), .O(new_n55_));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  nor3   g32(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(z3));
  nand4  g33(.a(x09), .b(x08), .c(new_n45_), .d(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n56_), .c(new_n36_), .O(z4));
  nand3  g36(.a(x13), .b(new_n54_), .c(new_n46_), .O(new_n61_));
  nor3   g37(.a(new_n61_), .b(x07), .c(x00), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n53_), .O(z5));
  xor2a  g39(.a(x09), .b(x03), .O(new_n64_));
  nor3   g40(.a(new_n64_), .b(x02), .c(x01), .O(new_n65_));
  nand2  g41(.a(x08), .b(x00), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n43_), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n65_), .c(new_n36_), .O(z6));
  aoi21  g45(.a(x03), .b(new_n42_), .c(x08), .O(new_n70_));
  nand3  g46(.a(x09), .b(x03), .c(new_n42_), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n70_), .c(new_n30_), .O(new_n73_));
  inv1   g49(.a(new_n43_), .O(new_n74_));
  nor2   g50(.a(x08), .b(new_n30_), .O(new_n75_));
  nor2   g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n73_), .c(new_n53_), .O(z7));
  inv1   g53(.a(x09), .O(new_n78_));
  nand3  g54(.a(new_n36_), .b(new_n78_), .c(x00), .O(new_n79_));
  nand2  g55(.a(new_n54_), .b(new_n30_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n71_), .c(new_n79_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n43_), .O(z8));
endmodule


