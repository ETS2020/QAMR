// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x08), .O(new_n25_));
  oai21  g01(.a(new_n25_), .b(x04), .c(x02), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n26_), .c(x03), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  nand2  g10(.a(new_n29_), .b(x03), .O(new_n35_));
  nand2  g11(.a(x04), .b(x02), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  nor2   g13(.a(x08), .b(x01), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(x00), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  aoi21  g16(.a(new_n36_), .b(new_n35_), .c(new_n40_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  nand3  g18(.a(new_n25_), .b(new_n42_), .c(x01), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n39_), .c(x02), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g25(.a(x01), .b(x00), .O(new_n50_));
  nand4  g26(.a(new_n50_), .b(new_n49_), .c(new_n46_), .d(x08), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n45_), .O(z2));
  inv1   g28(.a(new_n38_), .O(new_n53_));
  inv1   g29(.a(x00), .O(new_n54_));
  nand2  g30(.a(x12), .b(new_n54_), .O(new_n55_));
  nand2  g31(.a(x11), .b(x07), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(z3));
  nand2  g34(.a(new_n56_), .b(new_n53_), .O(new_n59_));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nor2   g37(.a(new_n47_), .b(x07), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(x13), .c(new_n42_), .d(new_n54_), .O(new_n65_));
  and2   g41(.a(new_n65_), .b(new_n53_), .O(z5));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(x02), .c(x08), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n54_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n60_), .c(new_n48_), .O(new_n70_));
  aoi21  g46(.a(new_n68_), .b(new_n27_), .c(new_n70_), .O(z6));
  nand4  g47(.a(x09), .b(x03), .c(new_n28_), .d(new_n27_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n48_), .c(x08), .O(z7));
  nand2  g49(.a(new_n47_), .b(x00), .O(new_n74_));
  inv1   g50(.a(x12), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(x08), .O(new_n76_));
  oai22  g52(.a(new_n76_), .b(new_n72_), .c(new_n74_), .d(new_n38_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n48_), .O(z8));
endmodule


