// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x14), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x11), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n29_), .c(new_n32_), .O(new_n33_));
  inv1   g09(.a(x03), .O(new_n34_));
  nor2   g10(.a(x02), .b(x01), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g12(.a(new_n29_), .b(new_n31_), .c(new_n36_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand2  g14(.a(new_n29_), .b(new_n34_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n27_), .c(new_n25_), .O(new_n40_));
  oai21  g16(.a(new_n34_), .b(new_n30_), .c(new_n31_), .O(new_n41_));
  nand3  g17(.a(new_n32_), .b(x06), .c(x05), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nor2   g19(.a(new_n43_), .b(new_n40_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n30_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n31_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n45_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n27_), .O(z2));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  inv1   g31(.a(x12), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n27_), .O(z3));
  nand4  g34(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n55_), .c(new_n27_), .O(z4));
  nor2   g37(.a(x10), .b(x07), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n27_), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n56_), .d(new_n25_), .O(z5));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n34_), .O(new_n66_));
  nand2  g42(.a(new_n47_), .b(x03), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n35_), .O(new_n68_));
  nand3  g44(.a(new_n32_), .b(x14), .c(new_n25_), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n68_), .c(new_n65_), .d(new_n62_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n27_), .O(z6));
  inv1   g47(.a(x08), .O(new_n72_));
  nand2  g48(.a(new_n35_), .b(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g50(.a(new_n35_), .b(x09), .c(x03), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n63_), .O(z7));
  oai21  g52(.a(new_n73_), .b(x12), .c(x09), .O(new_n77_));
  nand2  g53(.a(new_n47_), .b(new_n25_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n63_), .O(z8));
endmodule


