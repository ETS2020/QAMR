// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  aoi21  g04(.a(x04), .b(x02), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(x05), .b(new_n30_), .O(new_n31_));
  nor2   g07(.a(x02), .b(x01), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nor2   g12(.a(new_n25_), .b(x02), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n28_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(x02), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x02), .b(new_n43_), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(x03), .c(new_n37_), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n42_), .c(new_n38_), .O(z1));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n47_), .c(new_n36_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  inv1   g26(.a(x09), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(new_n53_));
  nand4  g29(.a(new_n53_), .b(new_n50_), .c(new_n25_), .d(new_n43_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n30_), .c(new_n49_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n31_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(new_n47_), .d(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n56_), .c(new_n31_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n63_), .c(new_n31_), .d(x13), .O(z5));
  nor2   g41(.a(x10), .b(x07), .O(new_n66_));
  nand2  g42(.a(x08), .b(x00), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n28_), .O(new_n68_));
  nand2  g44(.a(new_n51_), .b(x03), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n32_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n67_), .d(new_n66_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n31_), .O(z6));
  nand2  g49(.a(x03), .b(new_n43_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n53_), .c(new_n25_), .O(new_n75_));
  nor2   g51(.a(new_n28_), .b(x01), .O(new_n76_));
  nand3  g52(.a(new_n52_), .b(x08), .c(new_n47_), .O(new_n77_));
  aoi21  g53(.a(new_n76_), .b(new_n30_), .c(new_n77_), .O(new_n78_));
  aoi21  g54(.a(new_n75_), .b(new_n30_), .c(new_n78_), .O(z7));
  nand3  g55(.a(new_n31_), .b(new_n51_), .c(x00), .O(new_n80_));
  nor2   g56(.a(x05), .b(x02), .O(new_n81_));
  nor2   g57(.a(x12), .b(new_n51_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n66_), .O(z8));
endmodule


