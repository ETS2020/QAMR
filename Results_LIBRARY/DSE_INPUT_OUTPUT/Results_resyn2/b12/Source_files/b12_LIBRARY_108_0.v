// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x05), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x00), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai22  g08(.a(new_n32_), .b(x04), .c(new_n31_), .d(new_n30_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n29_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n28_), .O(z0));
  nor2   g11(.a(new_n31_), .b(x02), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x03), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n37_), .c(x00), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x03), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n45_), .c(new_n30_), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand2  g27(.a(new_n31_), .b(new_n25_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(x00), .O(new_n53_));
  aoi21  g29(.a(new_n51_), .b(new_n46_), .c(new_n53_), .O(new_n54_));
  aoi21  g30(.a(new_n50_), .b(new_n36_), .c(new_n54_), .O(z2));
  inv1   g31(.a(x12), .O(new_n56_));
  nor2   g32(.a(x05), .b(x02), .O(new_n57_));
  and2   g33(.a(x11), .b(x07), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g35(.a(new_n56_), .b(x00), .c(new_n59_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  nand4  g37(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n58_), .c(new_n52_), .O(z4));
  nor2   g40(.a(x12), .b(x10), .O(new_n65_));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(new_n52_), .d(x13), .O(z5));
  xor2a  g43(.a(x09), .b(x03), .O(new_n68_));
  nor3   g44(.a(new_n68_), .b(x02), .c(x01), .O(new_n69_));
  nand2  g45(.a(x08), .b(x00), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n29_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n44_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n69_), .c(new_n52_), .O(z6));
  oai21  g49(.a(new_n39_), .b(x01), .c(new_n51_), .O(new_n74_));
  nand4  g50(.a(x09), .b(x03), .c(new_n25_), .d(new_n30_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x05), .O(new_n77_));
  inv1   g53(.a(new_n44_), .O(new_n78_));
  aoi21  g54(.a(new_n51_), .b(x02), .c(new_n78_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n77_), .c(new_n57_), .O(z7));
  nand2  g56(.a(x03), .b(new_n30_), .O(new_n81_));
  nand4  g57(.a(new_n56_), .b(x09), .c(x05), .d(new_n25_), .O(new_n82_));
  oai22  g58(.a(new_n82_), .b(new_n81_), .c(new_n53_), .d(x09), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n44_), .O(z8));
endmodule


