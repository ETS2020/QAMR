// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(new_n25_), .O(new_n26_));
  nand3  g02(.a(x05), .b(x04), .c(x02), .O(new_n27_));
  inv1   g03(.a(x00), .O(new_n28_));
  nand2  g04(.a(x01), .b(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n27_), .b(new_n26_), .c(new_n29_), .O(z0));
  nand2  g06(.a(x06), .b(x01), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n27_), .O(new_n32_));
  inv1   g08(.a(x04), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(x02), .c(new_n28_), .O(new_n34_));
  aoi21  g10(.a(x03), .b(x01), .c(x02), .O(new_n35_));
  nor3   g11(.a(new_n35_), .b(new_n34_), .c(new_n32_), .O(z1));
  inv1   g12(.a(x07), .O(new_n37_));
  inv1   g13(.a(x08), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g15(.a(x01), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  oai21  g19(.a(x10), .b(x07), .c(x03), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(new_n45_));
  aoi22  g21(.a(new_n45_), .b(new_n25_), .c(new_n39_), .d(new_n34_), .O(z2));
  nand2  g22(.a(new_n33_), .b(x02), .O(new_n47_));
  nand2  g23(.a(x12), .b(new_n28_), .O(new_n48_));
  nand2  g24(.a(x11), .b(x07), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(z3));
  nand2  g27(.a(new_n49_), .b(new_n47_), .O(new_n52_));
  nor2   g28(.a(new_n38_), .b(new_n28_), .O(new_n53_));
  nor2   g29(.a(new_n41_), .b(x07), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z4));
  nor2   g31(.a(x12), .b(x10), .O(new_n56_));
  nand4  g32(.a(new_n56_), .b(x13), .c(new_n37_), .d(new_n28_), .O(new_n57_));
  and2   g33(.a(new_n57_), .b(new_n47_), .O(z5));
  nand3  g34(.a(x14), .b(x01), .c(new_n28_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(x04), .c(x02), .O(new_n60_));
  inv1   g36(.a(x03), .O(new_n61_));
  nand2  g37(.a(x09), .b(new_n61_), .O(new_n62_));
  nand2  g38(.a(new_n41_), .b(x03), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n40_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n25_), .O(new_n65_));
  oai21  g41(.a(new_n38_), .b(new_n28_), .c(new_n42_), .O(new_n66_));
  aoi21  g42(.a(new_n65_), .b(new_n60_), .c(new_n66_), .O(z6));
  oai21  g43(.a(new_n38_), .b(new_n33_), .c(x02), .O(new_n68_));
  oai21  g44(.a(new_n61_), .b(x01), .c(new_n38_), .O(new_n69_));
  nor2   g45(.a(new_n61_), .b(x01), .O(new_n70_));
  nor2   g46(.a(new_n41_), .b(x02), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n69_), .c(new_n68_), .d(new_n42_), .O(z7));
  nand2  g49(.a(new_n34_), .b(new_n41_), .O(new_n74_));
  inv1   g50(.a(x12), .O(new_n75_));
  nand3  g51(.a(new_n71_), .b(new_n70_), .c(new_n75_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n42_), .O(z8));
endmodule


