// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nand2  g02(.a(x03), .b(x01), .O(new_n27_));
  nand3  g03(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n25_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(x02), .c(new_n29_), .O(z0));
  nand2  g10(.a(new_n27_), .b(new_n26_), .O(new_n35_));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g15(.a(new_n39_), .b(new_n36_), .c(new_n35_), .d(new_n25_), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(z1));
  oai21  g17(.a(x08), .b(x07), .c(x00), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x02), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(x03), .O(new_n51_));
  nand4  g27(.a(new_n51_), .b(new_n47_), .c(new_n44_), .d(new_n25_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n43_), .O(z2));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  nand2  g30(.a(new_n26_), .b(x00), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n54_), .c(x12), .d(new_n25_), .O(z3));
  oai21  g32(.a(new_n48_), .b(x00), .c(new_n26_), .O(new_n57_));
  nand4  g33(.a(x09), .b(x08), .c(new_n48_), .d(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n54_), .c(new_n57_), .O(z4));
  inv1   g36(.a(x12), .O(new_n61_));
  nand4  g37(.a(new_n46_), .b(x13), .c(new_n61_), .d(new_n25_), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n55_), .O(z5));
  nor3   g39(.a(x10), .b(x08), .c(x07), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(new_n26_), .c(x00), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n37_), .O(new_n66_));
  nand2  g42(.a(new_n45_), .b(x03), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(new_n26_), .d(new_n44_), .O(new_n68_));
  nand2  g44(.a(x14), .b(x01), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n25_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x02), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n68_), .c(new_n46_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n65_), .O(z6));
  nand3  g49(.a(x09), .b(x03), .c(new_n44_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n26_), .O(new_n76_));
  inv1   g52(.a(x08), .O(new_n77_));
  nand3  g53(.a(x03), .b(new_n26_), .c(new_n44_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n77_), .c(new_n50_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n76_), .O(z7));
  nand2  g56(.a(new_n61_), .b(new_n26_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n74_), .c(new_n25_), .O(new_n82_));
  nand2  g58(.a(new_n45_), .b(x02), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(x00), .c(new_n50_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n82_), .O(z8));
endmodule


