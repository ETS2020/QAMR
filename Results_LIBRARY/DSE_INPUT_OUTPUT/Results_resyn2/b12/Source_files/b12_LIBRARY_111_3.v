// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x03), .O(new_n25_));
  nor2   g01(.a(x02), .b(x01), .O(new_n26_));
  nor2   g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g03(.a(x06), .b(x02), .O(new_n28_));
  nand2  g04(.a(x04), .b(x02), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  inv1   g06(.a(x06), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n31_), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  oai21  g12(.a(new_n26_), .b(new_n25_), .c(new_n29_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n28_), .O(z1));
  inv1   g15(.a(x02), .O(new_n40_));
  inv1   g16(.a(x01), .O(new_n41_));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(x03), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  nand2  g24(.a(new_n28_), .b(x00), .O(new_n49_));
  aoi21  g25(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  aoi21  g26(.a(new_n46_), .b(new_n40_), .c(new_n50_), .O(z2));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand2  g28(.a(x12), .b(new_n36_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n28_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nand4  g31(.a(x09), .b(x08), .c(new_n47_), .d(x00), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n28_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n28_), .d(x13), .O(z5));
  nand3  g37(.a(x14), .b(x01), .c(new_n36_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n31_), .c(x02), .O(new_n63_));
  nand2  g39(.a(x09), .b(new_n25_), .O(new_n64_));
  nand2  g40(.a(new_n42_), .b(x03), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n41_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n40_), .O(new_n67_));
  oai21  g43(.a(new_n48_), .b(new_n36_), .c(new_n43_), .O(new_n68_));
  aoi21  g44(.a(new_n67_), .b(new_n63_), .c(new_n68_), .O(z6));
  inv1   g45(.a(new_n28_), .O(new_n70_));
  nand3  g46(.a(x03), .b(new_n40_), .c(new_n41_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x08), .O(new_n72_));
  nand3  g48(.a(new_n26_), .b(new_n42_), .c(x03), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n43_), .c(new_n70_), .O(z7));
  inv1   g51(.a(x12), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x09), .O(new_n77_));
  oai22  g53(.a(new_n77_), .b(new_n71_), .c(x09), .d(new_n36_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n43_), .c(new_n70_), .O(z8));
endmodule


