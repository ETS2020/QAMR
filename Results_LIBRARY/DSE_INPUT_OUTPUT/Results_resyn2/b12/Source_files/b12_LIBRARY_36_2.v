// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x01), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(x03), .b(new_n27_), .O(new_n28_));
  nand3  g04(.a(x05), .b(x04), .c(x02), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n28_), .c(new_n26_), .O(z0));
  inv1   g06(.a(x01), .O(new_n31_));
  nand3  g07(.a(x06), .b(x05), .c(x01), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  oai21  g09(.a(new_n28_), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n25_), .O(new_n35_));
  nor2   g11(.a(x04), .b(new_n27_), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n35_), .O(z1));
  nor2   g14(.a(x10), .b(x07), .O(new_n39_));
  nor2   g15(.a(new_n39_), .b(x03), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(x09), .c(new_n41_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n40_), .c(new_n31_), .O(new_n45_));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n37_), .O(new_n47_));
  aoi21  g23(.a(new_n45_), .b(new_n27_), .c(new_n47_), .O(z2));
  nand2  g24(.a(x12), .b(new_n25_), .O(new_n49_));
  and2   g25(.a(x11), .b(x07), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand4  g29(.a(x09), .b(x08), .c(new_n41_), .d(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n50_), .c(new_n37_), .O(z4));
  inv1   g32(.a(x12), .O(new_n57_));
  nor3   g33(.a(new_n36_), .b(x07), .c(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n57_), .d(new_n42_), .O(z5));
  xor2a  g35(.a(x09), .b(x03), .O(new_n60_));
  nor3   g36(.a(new_n60_), .b(x02), .c(x01), .O(new_n61_));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  nand4  g38(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n39_), .O(new_n64_));
  oai21  g40(.a(new_n64_), .b(new_n61_), .c(new_n37_), .O(z6));
  aoi21  g41(.a(x03), .b(new_n31_), .c(x08), .O(new_n66_));
  nand3  g42(.a(x09), .b(x03), .c(new_n31_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n66_), .c(new_n27_), .O(new_n69_));
  inv1   g45(.a(new_n39_), .O(new_n70_));
  nor2   g46(.a(x08), .b(new_n27_), .O(new_n71_));
  nor2   g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n69_), .c(new_n36_), .O(z7));
  nand2  g49(.a(new_n57_), .b(new_n27_), .O(new_n74_));
  oai22  g50(.a(new_n74_), .b(new_n67_), .c(x09), .d(new_n25_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n39_), .c(new_n36_), .O(z8));
endmodule


