// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g06(.a(x04), .b(x02), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x10), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x09), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n27_), .c(new_n36_), .O(z0));
  nand4  g13(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  nand2  g15(.a(new_n31_), .b(new_n39_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n38_), .c(new_n30_), .d(new_n33_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n35_), .O(z1));
  oai21  g18(.a(x08), .b(x07), .c(x00), .O(new_n43_));
  nand3  g19(.a(x10), .b(new_n39_), .c(new_n28_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n29_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nor2   g22(.a(x03), .b(x01), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x07), .c(new_n33_), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n46_), .c(new_n35_), .O(z2));
  inv1   g25(.a(x12), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(x00), .c(new_n51_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n35_), .O(z3));
  nand2  g29(.a(x08), .b(x00), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(x07), .c(x10), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(x09), .c(x11), .d(x07), .O(z4));
  nor3   g32(.a(x10), .b(x09), .c(x07), .O(new_n57_));
  nand4  g33(.a(new_n57_), .b(x13), .c(new_n50_), .d(new_n33_), .O(z5));
  nand2  g34(.a(new_n57_), .b(new_n54_), .O(new_n59_));
  nand2  g35(.a(new_n47_), .b(new_n29_), .O(new_n60_));
  nand4  g36(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n59_), .O(z6));
  inv1   g39(.a(x08), .O(new_n64_));
  nand3  g40(.a(x03), .b(new_n29_), .c(new_n28_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n64_), .c(x07), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(x09), .c(new_n34_), .O(z7));
  nand2  g43(.a(new_n57_), .b(x00), .O(z8));
endmodule


