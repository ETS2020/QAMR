// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_;
  inv1   g00(.a(x03), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  oai21  g02(.a(x05), .b(new_n26_), .c(x02), .O(new_n27_));
  nand2  g03(.a(x03), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  nor2   g05(.a(new_n29_), .b(x00), .O(new_n30_));
  inv1   g06(.a(new_n30_), .O(new_n31_));
  aoi22  g07(.a(new_n31_), .b(new_n28_), .c(new_n27_), .d(new_n25_), .O(z0));
  inv1   g08(.a(x02), .O(new_n33_));
  nand3  g09(.a(x03), .b(new_n33_), .c(x01), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  nor2   g11(.a(x03), .b(new_n33_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x04), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n34_), .c(x00), .O(z1));
  inv1   g14(.a(x09), .O(new_n39_));
  nor2   g15(.a(x10), .b(x07), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g17(.a(x10), .b(x07), .c(x03), .O(new_n42_));
  nand2  g18(.a(x08), .b(x00), .O(new_n43_));
  nand4  g19(.a(new_n43_), .b(new_n42_), .c(new_n41_), .d(new_n29_), .O(new_n44_));
  nor2   g20(.a(x08), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n25_), .b(x00), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi21  g23(.a(new_n44_), .b(new_n33_), .c(new_n47_), .O(z2));
  inv1   g24(.a(x12), .O(new_n49_));
  nand2  g25(.a(x11), .b(x07), .O(new_n50_));
  oai21  g26(.a(new_n49_), .b(x00), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n28_), .O(z3));
  inv1   g28(.a(new_n43_), .O(new_n53_));
  nor2   g29(.a(new_n39_), .b(x07), .O(new_n54_));
  nand2  g30(.a(new_n50_), .b(new_n28_), .O(new_n55_));
  aoi21  g31(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z4));
  nor2   g32(.a(x12), .b(x10), .O(new_n57_));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n57_), .c(new_n28_), .d(x13), .O(z5));
  nand3  g35(.a(new_n36_), .b(new_n30_), .c(x14), .O(new_n60_));
  nand2  g36(.a(new_n43_), .b(new_n40_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n28_), .O(new_n62_));
  xnor2a g38(.a(x09), .b(x03), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n33_), .c(new_n29_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(z6));
  oai21  g42(.a(new_n39_), .b(x01), .c(new_n33_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x03), .O(new_n68_));
  inv1   g44(.a(x08), .O(new_n69_));
  nand2  g45(.a(x03), .b(new_n29_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n68_), .c(new_n40_), .O(z7));
  nand3  g48(.a(new_n28_), .b(new_n39_), .c(x00), .O(new_n73_));
  nand3  g49(.a(new_n49_), .b(x09), .c(new_n33_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n70_), .c(new_n73_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n40_), .O(z8));
endmodule


