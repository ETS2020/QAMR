// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nand2  g07(.a(x13), .b(x07), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x03), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  aoi21  g12(.a(x13), .b(x07), .c(x00), .O(new_n37_));
  inv1   g13(.a(x01), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  oai21  g15(.a(new_n34_), .b(new_n38_), .c(new_n39_), .O(new_n40_));
  inv1   g16(.a(new_n27_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(x06), .c(x05), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n40_), .c(new_n37_), .d(new_n36_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  nand2  g20(.a(x07), .b(x03), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n32_), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x09), .O(new_n50_));
  nand2  g26(.a(x10), .b(new_n34_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  nand2  g30(.a(new_n32_), .b(x00), .O(new_n55_));
  aoi21  g31(.a(new_n54_), .b(new_n48_), .c(new_n55_), .O(new_n56_));
  aoi21  g32(.a(new_n53_), .b(new_n39_), .c(new_n56_), .O(z2));
  inv1   g33(.a(x12), .O(new_n58_));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  oai21  g35(.a(new_n58_), .b(x00), .c(new_n59_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n32_), .O(z3));
  nor2   g37(.a(new_n54_), .b(new_n25_), .O(new_n62_));
  inv1   g38(.a(x09), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(x07), .O(new_n64_));
  nor2   g40(.a(x13), .b(new_n48_), .O(new_n65_));
  aoi22  g41(.a(new_n65_), .b(x11), .c(new_n64_), .d(new_n62_), .O(z4));
  nor2   g42(.a(x10), .b(x07), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(x13), .c(new_n58_), .d(new_n25_), .O(z5));
  inv1   g44(.a(new_n62_), .O(new_n69_));
  nor2   g45(.a(x02), .b(x01), .O(new_n70_));
  nand2  g46(.a(x09), .b(new_n34_), .O(new_n71_));
  nand2  g47(.a(new_n63_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(new_n73_));
  nand3  g49(.a(new_n41_), .b(x14), .c(new_n25_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n73_), .c(new_n67_), .d(new_n69_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n32_), .O(z6));
  nand3  g52(.a(new_n70_), .b(x09), .c(x03), .O(new_n77_));
  nand2  g53(.a(new_n70_), .b(x03), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n54_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n77_), .c(new_n67_), .O(z7));
  inv1   g56(.a(new_n65_), .O(new_n81_));
  nand3  g57(.a(new_n70_), .b(new_n58_), .c(x03), .O(new_n82_));
  oai21  g58(.a(x09), .b(x00), .c(new_n49_), .O(new_n83_));
  aoi21  g59(.a(new_n82_), .b(x09), .c(new_n83_), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(x07), .c(new_n81_), .O(z8));
endmodule


