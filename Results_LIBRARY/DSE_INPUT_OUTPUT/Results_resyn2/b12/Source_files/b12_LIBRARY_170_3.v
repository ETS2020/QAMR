// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x02), .O(new_n25_));
  oai21  g01(.a(x05), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  oai21  g05(.a(new_n25_), .b(new_n28_), .c(new_n29_), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  nand2  g08(.a(x09), .b(x05), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n32_), .c(new_n30_), .d(new_n27_), .O(new_n34_));
  aoi21  g10(.a(new_n26_), .b(x04), .c(new_n34_), .O(z0));
  nand3  g11(.a(x03), .b(new_n25_), .c(x01), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  oai21  g14(.a(x04), .b(x03), .c(x02), .O(new_n39_));
  oai21  g15(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n27_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n33_), .O(z1));
  inv1   g18(.a(x09), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n28_), .O(new_n47_));
  oai21  g23(.a(x08), .b(x07), .c(x00), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  aoi21  g25(.a(new_n47_), .b(new_n25_), .c(new_n49_), .O(z2));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n33_), .O(new_n52_));
  aoi21  g28(.a(x12), .b(new_n27_), .c(new_n52_), .O(z3));
  inv1   g29(.a(x05), .O(new_n54_));
  nand2  g30(.a(x08), .b(x00), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(x07), .c(new_n54_), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(x09), .c(x11), .d(x07), .O(z4));
  inv1   g33(.a(x12), .O(new_n58_));
  inv1   g34(.a(new_n33_), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n44_), .b(new_n60_), .c(x13), .d(new_n58_), .O(z5));
  nand2  g37(.a(new_n43_), .b(x03), .O(new_n62_));
  nand2  g38(.a(x09), .b(new_n29_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n31_), .O(new_n64_));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n27_), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n55_), .d(new_n44_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n33_), .O(z6));
  nand3  g43(.a(x03), .b(new_n25_), .c(new_n28_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x08), .O(new_n69_));
  nand3  g45(.a(new_n31_), .b(new_n43_), .c(x03), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n44_), .c(new_n59_), .O(z7));
  nand2  g48(.a(new_n58_), .b(x09), .O(new_n73_));
  oai22  g49(.a(new_n73_), .b(new_n68_), .c(x09), .d(new_n27_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n44_), .c(new_n59_), .O(z8));
endmodule


