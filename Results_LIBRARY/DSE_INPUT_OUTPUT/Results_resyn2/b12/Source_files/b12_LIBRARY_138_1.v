// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_;
  nand2  g00(.a(x04), .b(x02), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n25_), .c(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  nand2  g10(.a(x10), .b(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n29_), .c(new_n36_), .O(z0));
  nand3  g13(.a(x03), .b(new_n27_), .c(x01), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  oai21  g16(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n33_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n35_), .O(z1));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  inv1   g21(.a(x03), .O(new_n46_));
  nand3  g22(.a(x10), .b(new_n46_), .c(new_n26_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n27_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  inv1   g29(.a(x10), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g31(.a(x07), .b(new_n46_), .c(new_n33_), .O(new_n56_));
  oai21  g32(.a(new_n55_), .b(new_n34_), .c(new_n56_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n49_), .c(new_n35_), .O(z2));
  nand2  g35(.a(x12), .b(new_n33_), .O(new_n60_));
  nand2  g36(.a(x11), .b(x07), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n60_), .c(new_n35_), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(z3));
  nand2  g39(.a(new_n61_), .b(new_n35_), .O(new_n64_));
  nor2   g40(.a(new_n34_), .b(x07), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n51_), .c(new_n64_), .O(z4));
  nor2   g42(.a(x12), .b(x00), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(x13), .c(new_n54_), .d(new_n53_), .O(z5));
  aoi21  g44(.a(x09), .b(new_n46_), .c(x02), .O(new_n69_));
  nand2  g45(.a(new_n34_), .b(x03), .O(new_n70_));
  oai21  g46(.a(new_n69_), .b(x10), .c(new_n70_), .O(new_n71_));
  nor2   g47(.a(new_n27_), .b(x00), .O(new_n72_));
  nand3  g48(.a(new_n50_), .b(new_n54_), .c(x01), .O(new_n73_));
  aoi21  g49(.a(new_n72_), .b(x14), .c(new_n73_), .O(new_n74_));
  aoi21  g50(.a(new_n71_), .b(new_n52_), .c(new_n74_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(x07), .c(new_n35_), .O(z6));
  nand3  g52(.a(x03), .b(new_n27_), .c(new_n26_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x09), .O(new_n79_));
  inv1   g55(.a(x08), .O(new_n80_));
  aoi21  g56(.a(new_n77_), .b(new_n80_), .c(x07), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(x10), .c(new_n79_), .O(z7));
  oai21  g58(.a(new_n77_), .b(x12), .c(x09), .O(new_n83_));
  aoi21  g59(.a(new_n34_), .b(new_n33_), .c(new_n55_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


