// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  inv1   g06(.a(x03), .O(new_n31_));
  aoi21  g07(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n28_), .c(new_n27_), .O(new_n33_));
  nand2  g09(.a(x10), .b(new_n25_), .O(new_n34_));
  oai21  g10(.a(new_n33_), .b(x00), .c(new_n34_), .O(z0));
  nor2   g11(.a(x02), .b(x01), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g14(.a(x06), .b(x05), .O(new_n39_));
  nand2  g15(.a(new_n28_), .b(new_n31_), .O(new_n40_));
  oai21  g16(.a(new_n39_), .b(new_n26_), .c(new_n40_), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n38_), .O(z1));
  inv1   g18(.a(x09), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(x03), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n45_), .c(new_n29_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n30_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x00), .O(new_n54_));
  nand2  g30(.a(x12), .b(new_n54_), .O(new_n55_));
  nor2   g31(.a(new_n47_), .b(x05), .O(new_n56_));
  and2   g32(.a(x11), .b(x07), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n46_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n34_), .O(z4));
  inv1   g39(.a(x12), .O(new_n64_));
  nand3  g40(.a(x13), .b(new_n64_), .c(new_n54_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(new_n44_), .c(new_n56_), .O(z5));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  xnor2a g44(.a(x09), .b(x03), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n54_), .O(new_n71_));
  nand3  g46(.a(new_n69_), .b(new_n30_), .c(new_n29_), .O(new_n74_));
  nand4  g47(.a(new_n74_), .b(new_n71_), .c(new_n68_), .d(new_n44_), .O(new_n75_));
  inv1   g48(.a(new_n75_), .O(z6));
  nand3  g49(.a(new_n36_), .b(x09), .c(x03), .O(new_n77_));
  inv1   g50(.a(x08), .O(new_n78_));
  nand2  g51(.a(new_n36_), .b(x03), .O(new_n79_));
  aoi21  g52(.a(new_n79_), .b(new_n78_), .c(new_n48_), .O(new_n80_));
  aoi21  g53(.a(new_n80_), .b(new_n77_), .c(new_n56_), .O(z7));
  nand2  g54(.a(x10), .b(x05), .O(new_n82_));
  nand3  g55(.a(new_n36_), .b(new_n64_), .c(x03), .O(new_n83_));
  oai21  g56(.a(x09), .b(x00), .c(new_n46_), .O(new_n84_));
  aoi21  g57(.a(new_n83_), .b(x09), .c(new_n84_), .O(new_n85_));
  oai21  g58(.a(new_n85_), .b(x10), .c(new_n82_), .O(z8));
endmodule


