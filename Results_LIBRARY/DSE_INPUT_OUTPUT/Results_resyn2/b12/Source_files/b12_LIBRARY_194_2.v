// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nor2   g04(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  inv1   g06(.a(new_n30_), .O(new_n31_));
  inv1   g07(.a(x03), .O(new_n32_));
  aoi21  g08(.a(x04), .b(x02), .c(new_n32_), .O(new_n33_));
  aoi22  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .d(new_n26_), .O(new_n34_));
  inv1   g10(.a(x10), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x05), .O(new_n36_));
  oai21  g12(.a(new_n34_), .b(x00), .c(new_n36_), .O(z0));
  nand3  g13(.a(x06), .b(x02), .c(x01), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(x10), .c(new_n25_), .O(new_n39_));
  aoi21  g15(.a(x03), .b(x01), .c(x02), .O(new_n40_));
  inv1   g16(.a(x00), .O(new_n41_));
  oai21  g17(.a(x04), .b(x03), .c(new_n41_), .O(new_n42_));
  nor3   g18(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nand3  g21(.a(new_n35_), .b(new_n45_), .c(new_n44_), .O(new_n46_));
  nand2  g22(.a(new_n35_), .b(new_n44_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x03), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n46_), .c(new_n27_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n36_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n28_), .c(new_n51_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  aoi21  g30(.a(new_n35_), .b(x05), .c(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n53_), .b(x00), .c(new_n55_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand4  g33(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n54_), .c(new_n36_), .O(z4));
  nand4  g36(.a(x13), .b(new_n53_), .c(new_n44_), .d(new_n41_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n25_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n35_), .O(z5));
  nand2  g39(.a(x09), .b(new_n32_), .O(new_n64_));
  aoi21  g40(.a(new_n45_), .b(x03), .c(new_n31_), .O(new_n65_));
  nand2  g41(.a(x08), .b(x00), .O(new_n66_));
  nand3  g42(.a(new_n29_), .b(x14), .c(new_n41_), .O(new_n67_));
  nor2   g43(.a(new_n47_), .b(x05), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g45(.a(new_n65_), .b(new_n64_), .c(new_n69_), .O(z6));
  nand3  g46(.a(new_n30_), .b(x09), .c(x03), .O(new_n71_));
  inv1   g47(.a(x08), .O(new_n72_));
  nand3  g48(.a(x03), .b(new_n28_), .c(new_n27_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n71_), .c(new_n44_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n25_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n35_), .O(z7));
  nand3  g53(.a(new_n30_), .b(new_n53_), .c(x03), .O(new_n78_));
  oai21  g54(.a(x09), .b(x00), .c(new_n44_), .O(new_n79_));
  aoi21  g55(.a(new_n78_), .b(x09), .c(new_n79_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(x05), .c(new_n35_), .O(z8));
endmodule


