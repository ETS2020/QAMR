// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x09), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  nor2   g04(.a(x04), .b(new_n28_), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(new_n27_), .c(x02), .O(new_n30_));
  nand2  g06(.a(new_n25_), .b(x02), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x03), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  nand4  g10(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  oai21  g13(.a(new_n28_), .b(new_n36_), .c(new_n37_), .O(new_n38_));
  nand2  g14(.a(x09), .b(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n28_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n38_), .c(new_n35_), .d(new_n34_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(z1));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x01), .O(new_n45_));
  oai21  g21(.a(new_n43_), .b(new_n28_), .c(new_n45_), .O(new_n46_));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  nor2   g23(.a(x09), .b(x03), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  aoi21  g26(.a(new_n46_), .b(new_n37_), .c(new_n50_), .O(z2));
  inv1   g27(.a(x12), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  oai21  g29(.a(new_n52_), .b(x00), .c(new_n53_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n49_), .O(z3));
  inv1   g31(.a(x08), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  nor2   g33(.a(new_n44_), .b(x07), .O(new_n58_));
  nand2  g34(.a(new_n53_), .b(new_n49_), .O(new_n59_));
  aoi21  g35(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(z4));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  inv1   g37(.a(x10), .O(new_n62_));
  nand3  g38(.a(x13), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  inv1   g39(.a(new_n63_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n61_), .c(new_n48_), .O(z5));
  inv1   g41(.a(new_n57_), .O(new_n66_));
  nand4  g42(.a(x09), .b(x03), .c(new_n37_), .d(new_n36_), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n68_));
  nand4  g44(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n43_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n49_), .O(z6));
  nand2  g46(.a(new_n37_), .b(new_n36_), .O(new_n71_));
  oai22  g47(.a(new_n71_), .b(new_n28_), .c(new_n48_), .d(new_n56_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n67_), .c(new_n43_), .O(z7));
  oai21  g49(.a(new_n71_), .b(x12), .c(x09), .O(new_n74_));
  nand2  g50(.a(new_n44_), .b(new_n34_), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n43_), .d(x03), .O(z8));
endmodule


