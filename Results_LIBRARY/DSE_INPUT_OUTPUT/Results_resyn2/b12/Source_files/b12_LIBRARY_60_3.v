// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x10), .b(x07), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n28_), .c(new_n31_), .O(new_n32_));
  nand2  g08(.a(x04), .b(x02), .O(new_n33_));
  nand2  g09(.a(new_n30_), .b(new_n29_), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(new_n33_), .c(x03), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n32_), .c(new_n27_), .O(z0));
  inv1   g12(.a(x03), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n28_), .b(new_n37_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n39_), .c(new_n27_), .O(z1));
  inv1   g19(.a(x10), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  aoi21  g21(.a(x03), .b(new_n30_), .c(x00), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n45_), .c(x01), .O(new_n48_));
  oai22  g24(.a(new_n48_), .b(x02), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nand2  g25(.a(x08), .b(x00), .O(new_n50_));
  nand2  g26(.a(x10), .b(x03), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n29_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n30_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n50_), .c(x07), .O(new_n54_));
  aoi21  g30(.a(new_n49_), .b(new_n44_), .c(new_n54_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n26_), .O(z3));
  inv1   g35(.a(new_n50_), .O(new_n60_));
  nor2   g36(.a(new_n47_), .b(x07), .O(new_n61_));
  aoi22  g37(.a(new_n61_), .b(new_n60_), .c(new_n56_), .d(new_n44_), .O(z4));
  nor2   g38(.a(x10), .b(x07), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n57_), .d(new_n25_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n26_), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n34_), .O(new_n67_));
  nand3  g43(.a(new_n31_), .b(x14), .c(new_n25_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n63_), .c(new_n50_), .O(new_n69_));
  nor2   g45(.a(new_n69_), .b(new_n67_), .O(z6));
  inv1   g46(.a(x08), .O(new_n71_));
  oai21  g47(.a(new_n34_), .b(new_n37_), .c(new_n71_), .O(new_n72_));
  nand3  g48(.a(new_n38_), .b(x09), .c(new_n29_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n63_), .O(z7));
  inv1   g50(.a(new_n63_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n26_), .O(new_n76_));
  aoi21  g52(.a(new_n47_), .b(x00), .c(x07), .O(new_n77_));
  oai21  g53(.a(new_n73_), .b(x12), .c(new_n77_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n76_), .O(z8));
endmodule


