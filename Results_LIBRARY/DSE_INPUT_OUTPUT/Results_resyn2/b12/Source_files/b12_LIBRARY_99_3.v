// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n27_));
  aoi21  g03(.a(new_n27_), .b(x01), .c(new_n25_), .O(new_n28_));
  inv1   g04(.a(x00), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nand2  g06(.a(x02), .b(x01), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  inv1   g09(.a(x02), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g11(.a(x10), .b(x05), .O(new_n36_));
  nand4  g12(.a(new_n36_), .b(new_n35_), .c(new_n32_), .d(new_n29_), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(new_n28_), .O(z0));
  nand4  g14(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n39_));
  oai21  g15(.a(new_n25_), .b(new_n34_), .c(new_n30_), .O(new_n40_));
  nand4  g16(.a(new_n40_), .b(new_n39_), .c(new_n35_), .d(new_n29_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n36_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(x09), .c(new_n43_), .O(new_n45_));
  nand3  g21(.a(x07), .b(new_n30_), .c(new_n29_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(x08), .b(x00), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n33_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand3  g26(.a(x10), .b(new_n30_), .c(new_n33_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n49_), .c(new_n36_), .O(z2));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  inv1   g31(.a(x12), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n36_), .O(z3));
  nand4  g34(.a(x09), .b(x08), .c(new_n43_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n55_), .c(new_n36_), .O(z4));
  inv1   g37(.a(new_n36_), .O(new_n62_));
  nand2  g38(.a(new_n44_), .b(new_n43_), .O(new_n63_));
  inv1   g39(.a(new_n63_), .O(new_n64_));
  nand3  g40(.a(x13), .b(new_n56_), .c(new_n29_), .O(new_n65_));
  inv1   g41(.a(new_n65_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(new_n64_), .c(new_n62_), .O(z5));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n29_), .O(new_n70_));
  nand3  g45(.a(new_n68_), .b(new_n34_), .c(new_n33_), .O(new_n73_));
  nand3  g46(.a(new_n48_), .b(new_n44_), .c(new_n43_), .O(new_n74_));
  inv1   g47(.a(new_n74_), .O(new_n75_));
  nand3  g48(.a(new_n75_), .b(new_n73_), .c(new_n70_), .O(new_n76_));
  nand2  g49(.a(new_n76_), .b(new_n36_), .O(z6));
  nand4  g50(.a(x09), .b(x03), .c(new_n34_), .d(new_n33_), .O(new_n78_));
  inv1   g51(.a(x08), .O(new_n79_));
  nand3  g52(.a(x03), .b(new_n34_), .c(new_n33_), .O(new_n80_));
  aoi21  g53(.a(new_n80_), .b(new_n79_), .c(new_n63_), .O(new_n81_));
  aoi21  g54(.a(new_n81_), .b(new_n78_), .c(new_n62_), .O(z7));
  nor2   g55(.a(x09), .b(new_n29_), .O(new_n83_));
  nor2   g56(.a(new_n78_), .b(x12), .O(new_n84_));
  oai21  g57(.a(new_n84_), .b(new_n83_), .c(new_n64_), .O(z8));
endmodule


