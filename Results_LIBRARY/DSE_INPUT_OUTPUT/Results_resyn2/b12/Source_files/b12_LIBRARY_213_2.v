// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand3  g01(.a(x03), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(x05), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  oai21  g06(.a(x04), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n26_), .c(x00), .O(z0));
  nand2  g09(.a(x06), .b(x01), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n34_), .c(new_n28_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n26_), .c(x00), .O(z1));
  inv1   g14(.a(x09), .O(new_n39_));
  nor2   g15(.a(x10), .b(x07), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  oai21  g17(.a(x10), .b(x07), .c(x03), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(new_n29_), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x00), .O(new_n47_));
  aoi21  g23(.a(new_n27_), .b(x02), .c(new_n47_), .O(new_n48_));
  aoi22  g24(.a(new_n48_), .b(new_n46_), .c(new_n43_), .d(new_n25_), .O(z2));
  nand2  g25(.a(new_n27_), .b(x02), .O(new_n50_));
  nand2  g26(.a(x12), .b(new_n47_), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(z3));
  nand2  g30(.a(new_n52_), .b(new_n50_), .O(new_n55_));
  nor2   g31(.a(new_n45_), .b(new_n47_), .O(new_n56_));
  nor2   g32(.a(new_n39_), .b(x07), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nand4  g35(.a(new_n59_), .b(x13), .c(new_n44_), .d(new_n47_), .O(new_n60_));
  and2   g36(.a(new_n60_), .b(new_n50_), .O(z5));
  nand3  g37(.a(x14), .b(x01), .c(new_n47_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n28_), .O(new_n63_));
  nand2  g39(.a(x09), .b(new_n30_), .O(new_n64_));
  nand2  g40(.a(new_n39_), .b(x03), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n29_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n25_), .O(new_n67_));
  oai21  g43(.a(new_n45_), .b(new_n47_), .c(new_n40_), .O(new_n68_));
  aoi21  g44(.a(new_n67_), .b(new_n63_), .c(new_n68_), .O(z6));
  nand2  g45(.a(new_n45_), .b(x02), .O(new_n70_));
  and2   g46(.a(new_n50_), .b(new_n40_), .O(new_n71_));
  oai21  g47(.a(new_n30_), .b(x01), .c(new_n45_), .O(new_n72_));
  nand4  g48(.a(x09), .b(x03), .c(new_n25_), .d(new_n29_), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(z7));
  nand3  g50(.a(new_n50_), .b(new_n39_), .c(x00), .O(new_n75_));
  oai21  g51(.a(new_n73_), .b(x12), .c(new_n75_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n40_), .O(z8));
endmodule


