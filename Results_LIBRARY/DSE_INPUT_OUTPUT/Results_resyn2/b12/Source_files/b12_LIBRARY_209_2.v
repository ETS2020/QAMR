// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x01), .O(new_n25_));
  nand2  g01(.a(x07), .b(x02), .O(new_n26_));
  oai22  g02(.a(new_n26_), .b(x04), .c(x02), .d(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x03), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand4  g06(.a(new_n30_), .b(x07), .c(x02), .d(x01), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g08(.a(x00), .O(new_n33_));
  nand3  g09(.a(x06), .b(x05), .c(x01), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  oai21  g11(.a(x04), .b(x03), .c(x02), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n37_), .c(x01), .O(new_n38_));
  oai21  g14(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  inv1   g16(.a(x07), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x02), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g22(.a(x10), .b(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n37_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand2  g25(.a(x07), .b(x03), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n37_), .c(x00), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nor2   g29(.a(x07), .b(x02), .O(new_n54_));
  nor2   g30(.a(x08), .b(x01), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(new_n54_), .c(new_n47_), .d(new_n46_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(z2));
  and2   g33(.a(x11), .b(x07), .O(new_n58_));
  inv1   g34(.a(x12), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(x00), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n42_), .c(new_n58_), .O(z3));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  nand2  g38(.a(new_n54_), .b(x09), .O(new_n63_));
  nor2   g39(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g40(.a(new_n64_), .b(new_n58_), .O(z4));
  nor3   g41(.a(x10), .b(x07), .c(x02), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(x13), .c(new_n59_), .d(new_n33_), .O(z5));
  nand2  g43(.a(new_n44_), .b(x03), .O(new_n68_));
  inv1   g44(.a(x03), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n69_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(new_n25_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n62_), .c(new_n45_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n37_), .c(x07), .O(z6));
  nand2  g49(.a(x03), .b(new_n25_), .O(new_n74_));
  and2   g50(.a(new_n74_), .b(x08), .O(new_n75_));
  nand3  g51(.a(new_n44_), .b(x03), .c(new_n25_), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(new_n75_), .c(new_n66_), .O(z7));
  oai21  g54(.a(new_n74_), .b(x12), .c(x09), .O(new_n79_));
  nand2  g55(.a(new_n44_), .b(new_n33_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n79_), .c(new_n66_), .O(z8));
endmodule


