// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_;
  nand2  g00(.a(x14), .b(x02), .O(new_n25_));
  nand2  g01(.a(x04), .b(x02), .O(new_n26_));
  nor2   g02(.a(x02), .b(x01), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  nand4  g04(.a(new_n28_), .b(new_n26_), .c(new_n25_), .d(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x04), .c(x14), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  nand4  g10(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n26_), .b(new_n36_), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n35_), .c(new_n28_), .d(new_n34_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n25_), .O(z1));
  inv1   g15(.a(x02), .O(new_n40_));
  inv1   g16(.a(x01), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nor2   g18(.a(new_n42_), .b(x03), .O(new_n43_));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(x09), .c(new_n44_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n43_), .c(new_n41_), .O(new_n48_));
  oai21  g24(.a(x08), .b(x07), .c(x00), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n25_), .O(new_n50_));
  aoi21  g26(.a(new_n48_), .b(new_n40_), .c(new_n50_), .O(z2));
  inv1   g27(.a(new_n25_), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  and2   g30(.a(x11), .b(x07), .O(new_n55_));
  nor3   g31(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(z3));
  nand4  g32(.a(x09), .b(x08), .c(new_n44_), .d(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n55_), .c(new_n25_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n25_), .d(x13), .O(z5));
  nand2  g38(.a(x08), .b(x00), .O(new_n63_));
  inv1   g39(.a(x09), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n36_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n27_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n63_), .c(new_n42_), .d(new_n25_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(z6));
  nand2  g45(.a(new_n27_), .b(x03), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(x08), .O(new_n71_));
  nand3  g47(.a(new_n27_), .b(new_n64_), .c(x03), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n42_), .c(new_n52_), .O(z7));
  nand2  g50(.a(new_n53_), .b(x09), .O(new_n75_));
  oai22  g51(.a(new_n75_), .b(new_n70_), .c(x09), .d(new_n34_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n42_), .c(new_n52_), .O(z8));
endmodule


