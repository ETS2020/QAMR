// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  oai21  g05(.a(x08), .b(x04), .c(x02), .O(new_n30_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  nand2  g10(.a(x08), .b(new_n26_), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(new_n25_), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  oai21  g14(.a(new_n36_), .b(new_n26_), .c(new_n27_), .O(new_n39_));
  nand4  g15(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(new_n38_), .O(z1));
  inv1   g18(.a(x09), .O(new_n43_));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g21(.a(x10), .b(x03), .c(x01), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(x02), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x08), .O(new_n49_));
  aoi21  g25(.a(x03), .b(new_n27_), .c(x00), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n49_), .O(new_n51_));
  nand3  g27(.a(x02), .b(x01), .c(new_n34_), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(new_n47_), .c(new_n52_), .O(z2));
  nand2  g29(.a(x12), .b(new_n34_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n35_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand3  g33(.a(x09), .b(new_n48_), .c(x00), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(x01), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(x08), .c(x11), .d(x07), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x13), .c(new_n48_), .d(new_n34_), .O(new_n62_));
  and2   g38(.a(new_n62_), .b(new_n35_), .O(z5));
  nand2  g39(.a(x09), .b(new_n36_), .O(new_n64_));
  aoi21  g40(.a(new_n43_), .b(x03), .c(new_n31_), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n66_));
  oai21  g42(.a(new_n26_), .b(x00), .c(x08), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n44_), .O(new_n68_));
  aoi21  g44(.a(new_n65_), .b(new_n64_), .c(new_n68_), .O(z6));
  nand2  g45(.a(x03), .b(new_n27_), .O(new_n70_));
  nand2  g46(.a(new_n43_), .b(new_n26_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n70_), .c(new_n49_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n44_), .c(new_n35_), .O(z7));
  inv1   g49(.a(x12), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(x09), .c(new_n49_), .d(new_n26_), .O(new_n75_));
  nand3  g51(.a(new_n35_), .b(new_n43_), .c(x00), .O(new_n76_));
  oai21  g52(.a(new_n75_), .b(new_n70_), .c(new_n76_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n44_), .O(z8));
endmodule


