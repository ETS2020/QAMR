// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_;
  inv1   g00(.a(x00), .O(new_n25_));
  nor2   g01(.a(x13), .b(x06), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n29_), .c(new_n32_), .O(new_n33_));
  nor2   g09(.a(x02), .b(x01), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  nand2  g11(.a(x04), .b(x02), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n35_), .c(x03), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n33_), .c(new_n28_), .O(z0));
  nand3  g14(.a(new_n35_), .b(new_n27_), .c(new_n25_), .O(new_n39_));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(new_n36_), .b(new_n40_), .O(new_n41_));
  nand3  g17(.a(new_n32_), .b(x06), .c(x05), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nor2   g19(.a(new_n43_), .b(new_n39_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(x03), .O(new_n47_));
  nand3  g23(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n47_), .c(new_n30_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n27_), .O(new_n52_));
  aoi21  g28(.a(new_n50_), .b(new_n31_), .c(new_n52_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n27_), .O(z3));
  inv1   g33(.a(x09), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x07), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(new_n25_), .O(new_n61_));
  nand2  g37(.a(new_n55_), .b(new_n27_), .O(new_n62_));
  aoi21  g38(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(z4));
  nand3  g39(.a(x13), .b(new_n54_), .c(new_n46_), .O(new_n64_));
  nor3   g40(.a(new_n64_), .b(x07), .c(x00), .O(new_n65_));
  nor2   g41(.a(new_n65_), .b(new_n26_), .O(z5));
  inv1   g42(.a(new_n61_), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n70_));
  nand3  g45(.a(new_n68_), .b(new_n31_), .c(new_n30_), .O(new_n73_));
  nand2  g46(.a(new_n46_), .b(new_n45_), .O(new_n74_));
  nor2   g47(.a(new_n74_), .b(new_n26_), .O(new_n75_));
  nand4  g48(.a(new_n75_), .b(new_n73_), .c(new_n70_), .d(new_n67_), .O(new_n76_));
  inv1   g49(.a(new_n76_), .O(z6));
  nand3  g50(.a(new_n34_), .b(x09), .c(x03), .O(new_n78_));
  oai21  g51(.a(new_n35_), .b(new_n40_), .c(new_n60_), .O(new_n79_));
  nand3  g52(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(z7));
  oai22  g53(.a(new_n78_), .b(x12), .c(x09), .d(new_n25_), .O(new_n81_));
  nand2  g54(.a(new_n81_), .b(new_n75_), .O(z8));
endmodule


