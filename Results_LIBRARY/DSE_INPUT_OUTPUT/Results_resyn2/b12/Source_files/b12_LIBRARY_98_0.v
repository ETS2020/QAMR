// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  nand3  g02(.a(x03), .b(x01), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x12), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nor2   g05(.a(new_n25_), .b(x00), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n29_), .O(z0));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  nand2  g13(.a(x04), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g15(.a(x02), .b(x01), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(new_n36_), .O(new_n42_));
  oai21  g18(.a(x12), .b(x02), .c(new_n42_), .O(z1));
  inv1   g19(.a(x12), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(x02), .O(new_n45_));
  inv1   g21(.a(x01), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n46_), .O(new_n51_));
  nor2   g27(.a(x12), .b(x02), .O(new_n52_));
  nor2   g28(.a(x08), .b(x07), .O(new_n53_));
  nor3   g29(.a(new_n53_), .b(new_n52_), .c(new_n26_), .O(new_n54_));
  aoi21  g30(.a(new_n51_), .b(new_n45_), .c(new_n54_), .O(z2));
  nand2  g31(.a(x11), .b(x07), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  aoi21  g33(.a(x12), .b(new_n26_), .c(new_n57_), .O(z3));
  inv1   g34(.a(x08), .O(new_n59_));
  nor2   g35(.a(new_n59_), .b(new_n26_), .O(new_n60_));
  nor3   g36(.a(new_n52_), .b(new_n47_), .c(x07), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(new_n57_), .O(z4));
  nand4  g38(.a(new_n48_), .b(new_n30_), .c(x13), .d(new_n44_), .O(z5));
  nand3  g39(.a(x14), .b(x01), .c(new_n26_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x02), .O(new_n65_));
  nand2  g41(.a(new_n47_), .b(x03), .O(new_n66_));
  nand2  g42(.a(x09), .b(new_n37_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n46_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n45_), .O(new_n69_));
  oai21  g45(.a(new_n59_), .b(new_n26_), .c(new_n48_), .O(new_n70_));
  aoi21  g46(.a(new_n69_), .b(new_n65_), .c(new_n70_), .O(z6));
  nand2  g47(.a(new_n48_), .b(new_n46_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n66_), .c(x12), .O(new_n73_));
  nand2  g49(.a(new_n48_), .b(x08), .O(new_n74_));
  aoi21  g50(.a(new_n40_), .b(x03), .c(new_n74_), .O(new_n75_));
  aoi21  g51(.a(new_n73_), .b(new_n25_), .c(new_n75_), .O(z7));
  inv1   g52(.a(new_n49_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(x00), .c(new_n52_), .O(z8));
endmodule


