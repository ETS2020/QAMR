// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x06), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  nand3  g03(.a(x03), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n27_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  aoi21  g11(.a(new_n31_), .b(x02), .c(new_n26_), .O(new_n36_));
  nand2  g12(.a(x04), .b(x02), .O(new_n37_));
  oai21  g13(.a(x02), .b(x01), .c(x03), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n27_), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(new_n36_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(new_n27_), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g27(.a(new_n51_), .b(new_n48_), .c(new_n26_), .d(new_n45_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n25_), .c(new_n44_), .O(z2));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nor2   g32(.a(new_n26_), .b(x02), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n56_), .O(z3));
  nor2   g35(.a(new_n43_), .b(new_n27_), .O(new_n60_));
  nor2   g36(.a(new_n49_), .b(x07), .O(new_n61_));
  nand2  g37(.a(new_n58_), .b(new_n55_), .O(new_n62_));
  aoi21  g38(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(z4));
  nor3   g39(.a(new_n57_), .b(x07), .c(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(x13), .c(new_n54_), .d(new_n46_), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(x01), .c(new_n26_), .O(new_n67_));
  inv1   g43(.a(new_n60_), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n27_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n50_), .O(new_n70_));
  aoi21  g46(.a(new_n67_), .b(new_n25_), .c(new_n70_), .O(z6));
  inv1   g47(.a(x03), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(x01), .c(new_n43_), .O(new_n73_));
  nand4  g49(.a(x09), .b(x03), .c(new_n25_), .d(new_n45_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n26_), .O(new_n76_));
  aoi21  g52(.a(new_n43_), .b(x02), .c(new_n47_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n76_), .c(new_n57_), .O(z7));
  nand2  g54(.a(x03), .b(new_n45_), .O(new_n79_));
  nand2  g55(.a(new_n49_), .b(x00), .O(new_n80_));
  nand4  g56(.a(new_n54_), .b(x09), .c(new_n26_), .d(new_n25_), .O(new_n81_));
  oai22  g57(.a(new_n81_), .b(new_n79_), .c(new_n80_), .d(new_n57_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n50_), .O(z8));
endmodule


