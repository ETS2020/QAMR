// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x11), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x09), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  aoi21  g10(.a(x03), .b(x01), .c(x02), .O(new_n35_));
  nor2   g11(.a(x04), .b(x03), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n33_), .c(new_n25_), .O(new_n38_));
  nor3   g14(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z1));
  inv1   g15(.a(x02), .O(new_n40_));
  inv1   g16(.a(x01), .O(new_n41_));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g20(.a(x10), .b(x07), .c(x03), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(new_n46_));
  oai21  g22(.a(x08), .b(x07), .c(x00), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n33_), .O(new_n48_));
  aoi21  g24(.a(new_n46_), .b(new_n40_), .c(new_n48_), .O(z2));
  nand2  g25(.a(x12), .b(new_n25_), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n50_), .c(new_n33_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand3  g29(.a(x09), .b(x08), .c(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(x07), .c(x11), .O(z4));
  inv1   g32(.a(x12), .O(new_n57_));
  inv1   g33(.a(new_n33_), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  nand4  g35(.a(new_n43_), .b(new_n59_), .c(x13), .d(new_n57_), .O(z5));
  nand2  g36(.a(new_n42_), .b(x03), .O(new_n61_));
  inv1   g37(.a(x03), .O(new_n62_));
  nand2  g38(.a(new_n40_), .b(new_n41_), .O(new_n63_));
  aoi21  g39(.a(x09), .b(new_n62_), .c(new_n63_), .O(new_n64_));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(new_n43_), .d(new_n33_), .O(new_n67_));
  aoi21  g43(.a(new_n64_), .b(new_n61_), .c(new_n67_), .O(z6));
  nand3  g44(.a(x03), .b(new_n40_), .c(new_n41_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x08), .O(new_n70_));
  nand4  g46(.a(new_n42_), .b(x03), .c(new_n40_), .d(new_n41_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n43_), .c(new_n58_), .O(z7));
  nand2  g49(.a(new_n57_), .b(x09), .O(new_n74_));
  oai22  g50(.a(new_n74_), .b(new_n69_), .c(x09), .d(new_n25_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n43_), .c(new_n58_), .O(z8));
endmodule


