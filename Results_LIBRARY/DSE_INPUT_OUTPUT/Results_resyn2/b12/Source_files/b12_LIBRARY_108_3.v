// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  nand2  g00(.a(x05), .b(x02), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(new_n26_), .c(x02), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nand3  g07(.a(x03), .b(new_n31_), .c(x01), .O(new_n32_));
  and2   g08(.a(new_n32_), .b(new_n25_), .O(new_n33_));
  aoi22  g09(.a(new_n33_), .b(new_n30_), .c(new_n25_), .d(x00), .O(z0));
  nor2   g10(.a(x05), .b(new_n31_), .O(new_n35_));
  oai21  g11(.a(x04), .b(x03), .c(new_n35_), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n32_), .c(x00), .O(z1));
  inv1   g13(.a(x07), .O(new_n38_));
  inv1   g14(.a(x08), .O(new_n39_));
  nand2  g15(.a(new_n25_), .b(x00), .O(new_n40_));
  aoi21  g16(.a(new_n39_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g20(.a(new_n43_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x03), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n44_), .c(new_n27_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n31_), .c(new_n41_), .O(z2));
  inv1   g24(.a(new_n25_), .O(new_n49_));
  inv1   g25(.a(x12), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x00), .O(new_n51_));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  nor3   g28(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(z3));
  nand4  g29(.a(x09), .b(x08), .c(new_n38_), .d(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n52_), .c(new_n25_), .O(z4));
  nor2   g32(.a(x12), .b(x10), .O(new_n57_));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n57_), .c(new_n25_), .d(x13), .O(z5));
  xor2a  g35(.a(x09), .b(x03), .O(new_n60_));
  nor3   g36(.a(new_n60_), .b(x02), .c(x01), .O(new_n61_));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  inv1   g38(.a(x00), .O(new_n63_));
  nand4  g39(.a(x14), .b(x02), .c(x01), .d(new_n63_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n62_), .c(new_n43_), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(new_n61_), .c(new_n25_), .O(z6));
  aoi21  g42(.a(x03), .b(new_n27_), .c(x08), .O(new_n67_));
  nand3  g43(.a(x09), .b(x03), .c(new_n27_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n67_), .c(new_n31_), .O(new_n70_));
  aoi21  g46(.a(new_n39_), .b(x02), .c(new_n45_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n70_), .c(new_n49_), .O(z7));
  nand2  g48(.a(new_n50_), .b(new_n31_), .O(new_n73_));
  oai22  g49(.a(new_n73_), .b(new_n68_), .c(new_n40_), .d(x09), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n43_), .O(z8));
endmodule


