// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  aoi21  g02(.a(x01), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x01), .O(new_n28_));
  aoi21  g04(.a(x03), .b(x02), .c(x01), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n28_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  oai21  g07(.a(new_n27_), .b(x02), .c(new_n31_), .O(z0));
  inv1   g08(.a(x02), .O(new_n33_));
  inv1   g09(.a(x04), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n26_), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n35_), .c(new_n33_), .O(new_n37_));
  inv1   g13(.a(x01), .O(new_n38_));
  nand2  g14(.a(new_n33_), .b(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n25_), .O(new_n40_));
  nand2  g16(.a(new_n26_), .b(new_n33_), .O(new_n41_));
  oai21  g17(.a(new_n40_), .b(new_n37_), .c(new_n41_), .O(z1));
  inv1   g18(.a(x10), .O(new_n43_));
  nand4  g19(.a(new_n43_), .b(x09), .c(x03), .d(new_n38_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(x07), .c(new_n33_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n25_), .O(new_n46_));
  nand2  g22(.a(new_n44_), .b(new_n33_), .O(new_n47_));
  nor2   g23(.a(x08), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n46_), .O(z2));
  nand2  g26(.a(x12), .b(new_n25_), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n41_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(z3));
  nand2  g30(.a(new_n52_), .b(new_n41_), .O(new_n55_));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  inv1   g33(.a(x09), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x07), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n41_), .d(x13), .O(z5));
  nand3  g39(.a(x09), .b(new_n33_), .c(new_n38_), .O(new_n64_));
  nor2   g40(.a(x10), .b(x07), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n56_), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  aoi21  g44(.a(new_n66_), .b(new_n41_), .c(new_n68_), .O(z6));
  oai21  g45(.a(new_n58_), .b(x01), .c(x03), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n33_), .O(new_n71_));
  inv1   g47(.a(x08), .O(new_n72_));
  nand2  g48(.a(new_n39_), .b(new_n72_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n71_), .c(new_n65_), .O(z7));
  nand3  g50(.a(new_n41_), .b(new_n58_), .c(x00), .O(new_n75_));
  inv1   g51(.a(x12), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x03), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(new_n64_), .c(new_n75_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n65_), .O(z8));
endmodule


