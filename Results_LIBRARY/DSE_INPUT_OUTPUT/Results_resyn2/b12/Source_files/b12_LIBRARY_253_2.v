// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x12), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x02), .O(new_n34_));
  oai21  g10(.a(new_n32_), .b(x00), .c(new_n34_), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  nand2  g13(.a(x04), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  nand4  g15(.a(new_n39_), .b(new_n37_), .c(new_n29_), .d(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n34_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x08), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n42_), .c(new_n36_), .O(new_n44_));
  nand2  g20(.a(x12), .b(x02), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n42_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(x03), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n48_), .c(new_n28_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n45_), .c(new_n44_), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n42_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n34_), .c(x12), .d(new_n36_), .O(z3));
  nor2   g31(.a(new_n43_), .b(new_n36_), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x07), .O(new_n58_));
  oai21  g34(.a(new_n53_), .b(new_n42_), .c(new_n34_), .O(new_n59_));
  aoi21  g35(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  inv1   g36(.a(x02), .O(new_n61_));
  nand3  g37(.a(new_n49_), .b(x13), .c(new_n36_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n33_), .O(z5));
  xor2a  g40(.a(x09), .b(x03), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(x01), .c(new_n61_), .O(new_n66_));
  nand3  g42(.a(x14), .b(x01), .c(new_n36_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(x12), .c(x02), .O(new_n68_));
  oai21  g44(.a(new_n43_), .b(new_n36_), .c(new_n49_), .O(new_n69_));
  aoi21  g45(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(z6));
  nand2  g46(.a(new_n34_), .b(x08), .O(new_n71_));
  inv1   g47(.a(x01), .O(new_n72_));
  nand2  g48(.a(x03), .b(new_n72_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(x02), .c(new_n71_), .O(new_n74_));
  nand3  g50(.a(new_n28_), .b(x09), .c(x03), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n49_), .O(z7));
  oai21  g52(.a(new_n73_), .b(new_n47_), .c(new_n61_), .O(new_n77_));
  nor2   g53(.a(x09), .b(new_n36_), .O(new_n78_));
  aoi22  g54(.a(new_n78_), .b(new_n49_), .c(new_n77_), .d(new_n33_), .O(z8));
endmodule


