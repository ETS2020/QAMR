// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n63_;
  inv1   g00(.a(x02), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x00), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  nor2   g03(.a(x03), .b(x01), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n27_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  inv1   g06(.a(x00), .O(new_n31_));
  nand3  g07(.a(x03), .b(x01), .c(new_n31_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x07), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n25_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  inv1   g11(.a(x03), .O(new_n36_));
  nand2  g12(.a(x04), .b(x02), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g14(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n39_));
  nand2  g15(.a(x07), .b(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  nand4  g17(.a(new_n41_), .b(new_n39_), .c(new_n38_), .d(new_n31_), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(z1));
  oai21  g19(.a(new_n28_), .b(x02), .c(new_n31_), .O(new_n44_));
  and2   g20(.a(x08), .b(x02), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(x07), .c(new_n44_), .O(z2));
  and2   g22(.a(x11), .b(x07), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n25_), .O(new_n49_));
  inv1   g25(.a(x12), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(x00), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(z3));
  nand3  g28(.a(x09), .b(new_n48_), .c(x00), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n45_), .c(new_n47_), .O(z4));
  nor2   g31(.a(x10), .b(x07), .O(new_n56_));
  nand4  g32(.a(new_n56_), .b(new_n26_), .c(x13), .d(new_n50_), .O(z5));
  nand3  g33(.a(x14), .b(x01), .c(new_n31_), .O(new_n58_));
  aoi21  g34(.a(x08), .b(x00), .c(x10), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(x02), .c(x07), .O(z6));
  nand2  g37(.a(new_n56_), .b(new_n45_), .O(z7));
  inv1   g38(.a(x09), .O(new_n63_));
  nand4  g39(.a(new_n56_), .b(new_n63_), .c(x02), .d(x00), .O(z8));
endmodule


