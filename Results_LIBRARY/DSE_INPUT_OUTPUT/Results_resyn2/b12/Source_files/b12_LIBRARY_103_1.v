// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x11), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  nand3  g03(.a(x03), .b(x01), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(new_n27_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n30_), .O(z0));
  nand2  g11(.a(x11), .b(new_n25_), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(x04), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g16(.a(x01), .O(new_n41_));
  nand2  g17(.a(new_n25_), .b(new_n41_), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n40_), .c(new_n37_), .d(new_n27_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n36_), .O(z1));
  nor2   g20(.a(x11), .b(x02), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nor2   g22(.a(new_n46_), .b(x03), .O(new_n47_));
  inv1   g23(.a(x07), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(new_n47_), .c(new_n41_), .O(new_n52_));
  inv1   g28(.a(x08), .O(new_n53_));
  nand2  g29(.a(new_n36_), .b(x00), .O(new_n54_));
  aoi21  g30(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  aoi21  g31(.a(new_n52_), .b(new_n45_), .c(new_n55_), .O(z2));
  nor2   g32(.a(new_n26_), .b(new_n48_), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n57_), .c(new_n36_), .O(z3));
  nand4  g36(.a(x09), .b(x08), .c(new_n48_), .d(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n36_), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nor2   g40(.a(x07), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n64_), .c(new_n36_), .d(x13), .O(z5));
  xor2a  g42(.a(x09), .b(x03), .O(new_n67_));
  nor2   g43(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n27_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n46_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n68_), .c(new_n36_), .O(z6));
  aoi21  g48(.a(x03), .b(new_n41_), .c(x11), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(x02), .c(new_n53_), .O(new_n74_));
  nand4  g50(.a(new_n45_), .b(x09), .c(x03), .d(new_n41_), .O(new_n75_));
  oai21  g51(.a(x10), .b(x07), .c(new_n36_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(z7));
  oai22  g53(.a(new_n75_), .b(x12), .c(new_n54_), .d(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n46_), .O(z8));
endmodule


