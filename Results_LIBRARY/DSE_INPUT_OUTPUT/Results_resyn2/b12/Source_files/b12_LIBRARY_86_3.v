// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(x04), .b(x02), .O(new_n30_));
  inv1   g06(.a(x03), .O(new_n31_));
  nor2   g07(.a(x02), .b(x01), .O(new_n32_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  inv1   g10(.a(x00), .O(new_n35_));
  nand2  g11(.a(x12), .b(x10), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi21  g13(.a(new_n34_), .b(new_n29_), .c(new_n37_), .O(z0));
  nand3  g14(.a(new_n28_), .b(x06), .c(x05), .O(new_n39_));
  oai21  g15(.a(new_n31_), .b(new_n26_), .c(new_n27_), .O(new_n40_));
  aoi21  g16(.a(new_n25_), .b(new_n31_), .c(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n36_), .O(z1));
  nor2   g19(.a(x10), .b(x07), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x09), .O(new_n45_));
  nand3  g21(.a(x07), .b(new_n31_), .c(new_n35_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(x08), .b(x00), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n26_), .O(new_n49_));
  inv1   g25(.a(new_n36_), .O(new_n50_));
  oai21  g26(.a(x08), .b(x07), .c(x00), .O(new_n51_));
  nand3  g27(.a(x10), .b(new_n31_), .c(new_n26_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n49_), .O(z2));
  inv1   g31(.a(x10), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(x00), .O(new_n57_));
  aoi22  g33(.a(new_n57_), .b(x12), .c(x11), .d(x07), .O(z3));
  inv1   g34(.a(new_n48_), .O(new_n59_));
  inv1   g35(.a(x09), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(x07), .O(new_n61_));
  nand2  g37(.a(x11), .b(x07), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n36_), .O(new_n63_));
  aoi21  g39(.a(new_n61_), .b(new_n59_), .c(new_n63_), .O(z4));
  nor2   g40(.a(x07), .b(x00), .O(new_n65_));
  inv1   g41(.a(x12), .O(new_n66_));
  nand3  g42(.a(x13), .b(new_n66_), .c(new_n56_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n65_), .c(new_n50_), .O(z5));
  nand2  g45(.a(x09), .b(new_n31_), .O(new_n70_));
  nand2  g46(.a(new_n60_), .b(x03), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n32_), .O(new_n72_));
  nand3  g48(.a(new_n28_), .b(x14), .c(new_n35_), .O(new_n73_));
  nand2  g49(.a(new_n48_), .b(new_n44_), .O(new_n74_));
  inv1   g50(.a(new_n74_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n36_), .O(z6));
  nand3  g53(.a(x03), .b(new_n27_), .c(new_n26_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x08), .O(new_n79_));
  nand3  g55(.a(new_n32_), .b(new_n60_), .c(x03), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g57(.a(new_n81_), .b(new_n44_), .c(new_n50_), .O(z7));
  oai21  g58(.a(new_n78_), .b(x12), .c(x09), .O(new_n83_));
  nand2  g59(.a(new_n60_), .b(new_n35_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(new_n44_), .O(z8));
endmodule


