// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x01), .O(new_n25_));
  nor2   g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x06), .b(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(x04), .O(new_n28_));
  oai21  g04(.a(new_n28_), .b(new_n26_), .c(x03), .O(new_n29_));
  inv1   g05(.a(new_n27_), .O(new_n30_));
  inv1   g06(.a(x05), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n30_), .c(x01), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g10(.a(x06), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  aoi21  g15(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  oai21  g16(.a(new_n31_), .b(new_n25_), .c(new_n40_), .O(new_n41_));
  nor2   g17(.a(x06), .b(new_n37_), .O(new_n42_));
  aoi21  g18(.a(new_n26_), .b(x03), .c(new_n42_), .O(new_n43_));
  aoi22  g19(.a(new_n43_), .b(new_n41_), .c(new_n36_), .d(x00), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  nand2  g22(.a(new_n36_), .b(x00), .O(new_n47_));
  aoi21  g23(.a(new_n46_), .b(new_n45_), .c(new_n47_), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nor2   g25(.a(new_n49_), .b(x03), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n45_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n50_), .c(new_n25_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n37_), .c(new_n48_), .O(z2));
  inv1   g31(.a(x00), .O(new_n56_));
  nand2  g32(.a(x12), .b(new_n56_), .O(new_n57_));
  and2   g33(.a(x11), .b(x07), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(new_n42_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  nand4  g37(.a(x09), .b(x08), .c(new_n45_), .d(x00), .O(new_n62_));
  inv1   g38(.a(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n58_), .c(new_n36_), .O(z4));
  nor2   g40(.a(x12), .b(x10), .O(new_n65_));
  nor2   g41(.a(x07), .b(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(new_n36_), .d(x13), .O(z5));
  nand3  g43(.a(x14), .b(x01), .c(new_n56_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n30_), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n38_), .O(new_n70_));
  inv1   g46(.a(x09), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x03), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n25_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n37_), .O(new_n74_));
  oai21  g50(.a(new_n46_), .b(new_n56_), .c(new_n49_), .O(new_n75_));
  aoi21  g51(.a(new_n74_), .b(new_n69_), .c(new_n75_), .O(z6));
  aoi21  g52(.a(x03), .b(new_n25_), .c(x08), .O(new_n77_));
  nand3  g53(.a(x09), .b(x03), .c(new_n25_), .O(new_n78_));
  inv1   g54(.a(new_n78_), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n77_), .c(new_n37_), .O(new_n80_));
  inv1   g56(.a(new_n49_), .O(new_n81_));
  aoi21  g57(.a(new_n46_), .b(x02), .c(new_n81_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n80_), .c(new_n42_), .O(z7));
  inv1   g59(.a(x12), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n37_), .O(new_n85_));
  oai22  g61(.a(new_n85_), .b(new_n78_), .c(x09), .d(new_n56_), .O(new_n86_));
  aoi21  g62(.a(new_n86_), .b(new_n49_), .c(new_n42_), .O(z8));
endmodule


