// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand4  g02(.a(new_n26_), .b(x08), .c(x02), .d(x01), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g06(.a(x08), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  nand2  g08(.a(x04), .b(x02), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n32_), .c(new_n30_), .d(x03), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n27_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n33_), .b(new_n38_), .O(new_n39_));
  nand4  g15(.a(new_n39_), .b(new_n37_), .c(new_n30_), .d(new_n36_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n32_), .O(z1));
  inv1   g17(.a(x07), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(x02), .c(new_n31_), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n28_), .O(new_n48_));
  aoi22  g24(.a(new_n48_), .b(new_n29_), .c(new_n43_), .d(x00), .O(z2));
  inv1   g25(.a(x12), .O(new_n50_));
  nand2  g26(.a(x11), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n50_), .b(x00), .c(new_n51_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n32_), .O(z3));
  nor2   g29(.a(x07), .b(new_n36_), .O(new_n54_));
  nor2   g30(.a(new_n44_), .b(new_n31_), .O(new_n55_));
  nand2  g31(.a(new_n51_), .b(new_n32_), .O(new_n56_));
  aoi21  g32(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(z4));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(x13), .c(new_n50_), .d(new_n45_), .O(new_n59_));
  and2   g35(.a(new_n59_), .b(new_n32_), .O(z5));
  nand3  g36(.a(x14), .b(x02), .c(x01), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n36_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x08), .O(new_n63_));
  nand2  g39(.a(new_n45_), .b(new_n42_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n32_), .O(new_n65_));
  nor2   g41(.a(x02), .b(x01), .O(new_n66_));
  nand2  g42(.a(new_n44_), .b(x03), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n38_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n65_), .c(new_n63_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(z6));
  nand3  g47(.a(new_n44_), .b(x03), .c(new_n28_), .O(new_n72_));
  nand2  g48(.a(x03), .b(new_n28_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x08), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n72_), .c(new_n29_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n65_), .O(z7));
  inv1   g52(.a(new_n64_), .O(new_n77_));
  nand2  g53(.a(new_n44_), .b(x00), .O(new_n78_));
  aoi21  g54(.a(new_n31_), .b(x02), .c(new_n78_), .O(new_n79_));
  nand3  g55(.a(new_n50_), .b(x09), .c(new_n29_), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(z8));
endmodule


