// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  inv1   g03(.a(x08), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x01), .O(new_n32_));
  nor2   g08(.a(new_n27_), .b(new_n32_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n25_), .c(new_n33_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(new_n29_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x03), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  nand2  g15(.a(new_n25_), .b(new_n39_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(x02), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n37_), .c(x00), .O(z1));
  nand2  g18(.a(x10), .b(x03), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor2   g22(.a(x07), .b(x01), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n43_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n28_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n27_), .O(new_n50_));
  inv1   g26(.a(x00), .O(new_n51_));
  nand3  g27(.a(x07), .b(new_n39_), .c(new_n32_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  nor2   g29(.a(x07), .b(new_n27_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n28_), .c(new_n53_), .d(new_n51_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n50_), .O(z2));
  nand2  g32(.a(x08), .b(new_n27_), .O(new_n57_));
  nand2  g33(.a(x12), .b(new_n51_), .O(new_n58_));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  inv1   g37(.a(x07), .O(new_n62_));
  nand2  g38(.a(x09), .b(new_n62_), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n51_), .c(x02), .O(new_n64_));
  aoi22  g40(.a(new_n64_), .b(x08), .c(x11), .d(x07), .O(z4));
  nor2   g41(.a(x12), .b(x10), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(x13), .c(new_n62_), .d(new_n51_), .O(new_n67_));
  and2   g43(.a(new_n67_), .b(new_n57_), .O(z5));
  nand2  g44(.a(new_n44_), .b(x03), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n39_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n32_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n28_), .O(new_n72_));
  nand4  g48(.a(x14), .b(x02), .c(x01), .d(new_n51_), .O(new_n73_));
  nor2   g49(.a(x10), .b(x07), .O(new_n74_));
  nand2  g50(.a(x08), .b(x00), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(new_n76_));
  aoi21  g52(.a(new_n72_), .b(new_n27_), .c(new_n76_), .O(z6));
  nand2  g53(.a(new_n27_), .b(new_n32_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n69_), .c(new_n28_), .O(new_n79_));
  aoi22  g55(.a(new_n79_), .b(new_n74_), .c(x08), .d(new_n27_), .O(z7));
  nand3  g56(.a(new_n66_), .b(x03), .c(new_n32_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(new_n63_), .c(new_n28_), .O(new_n82_));
  nand4  g58(.a(new_n45_), .b(new_n44_), .c(new_n62_), .d(x00), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(new_n84_));
  aoi21  g60(.a(new_n82_), .b(new_n27_), .c(new_n84_), .O(z8));
endmodule


