// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x10), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  nor2   g04(.a(x04), .b(new_n28_), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(new_n27_), .c(x02), .O(new_n30_));
  nand2  g06(.a(new_n25_), .b(x02), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x03), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g09(.a(x10), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n28_), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  inv1   g12(.a(x02), .O(new_n37_));
  oai21  g13(.a(new_n28_), .b(new_n36_), .c(new_n37_), .O(new_n38_));
  nand4  g14(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n28_), .c(x00), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n35_), .O(z1));
  oai21  g19(.a(x08), .b(x07), .c(x00), .O(new_n44_));
  nor2   g20(.a(x03), .b(x01), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(x02), .c(new_n44_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nand2  g23(.a(x08), .b(x00), .O(new_n48_));
  nand4  g24(.a(new_n48_), .b(x09), .c(new_n47_), .d(new_n36_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x03), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n34_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n46_), .O(z2));
  inv1   g28(.a(x12), .O(new_n53_));
  nand2  g29(.a(x11), .b(x07), .O(new_n54_));
  oai21  g30(.a(new_n53_), .b(x00), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n35_), .O(z3));
  inv1   g32(.a(new_n48_), .O(new_n57_));
  inv1   g33(.a(x09), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x07), .O(new_n59_));
  nand2  g35(.a(new_n54_), .b(new_n35_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  inv1   g37(.a(x00), .O(new_n62_));
  nand4  g38(.a(x13), .b(new_n53_), .c(new_n47_), .d(new_n62_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x03), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n34_), .O(z5));
  nand3  g41(.a(new_n34_), .b(new_n47_), .c(x03), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(new_n67_));
  nor2   g43(.a(x02), .b(x01), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x09), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n62_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n48_), .O(new_n72_));
  nor2   g48(.a(new_n72_), .b(new_n70_), .O(z6));
  inv1   g49(.a(x08), .O(new_n74_));
  inv1   g50(.a(new_n68_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n69_), .c(new_n67_), .O(z7));
  oai21  g53(.a(new_n75_), .b(x12), .c(x09), .O(new_n78_));
  aoi21  g54(.a(new_n58_), .b(new_n62_), .c(new_n66_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n78_), .O(z8));
endmodule


