// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  nand3  g02(.a(x03), .b(x01), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x12), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n29_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n25_), .c(x01), .O(new_n38_));
  oai21  g14(.a(new_n37_), .b(new_n36_), .c(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n26_), .O(new_n40_));
  inv1   g16(.a(x12), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n40_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  aoi21  g21(.a(new_n45_), .b(new_n44_), .c(new_n26_), .O(new_n46_));
  inv1   g22(.a(x01), .O(new_n47_));
  oai21  g23(.a(x10), .b(x07), .c(x03), .O(new_n48_));
  inv1   g24(.a(x09), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g27(.a(new_n51_), .b(new_n48_), .c(x12), .d(new_n47_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n25_), .c(new_n46_), .O(z2));
  inv1   g29(.a(x11), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(new_n44_), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(new_n42_), .c(x12), .d(new_n26_), .O(z3));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  nor2   g34(.a(new_n49_), .b(x07), .O(new_n59_));
  oai21  g35(.a(new_n54_), .b(new_n44_), .c(new_n42_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z4));
  nand3  g37(.a(new_n50_), .b(x13), .c(new_n26_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x02), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n41_), .O(z5));
  xor2a  g40(.a(x09), .b(x03), .O(new_n65_));
  oai21  g41(.a(new_n65_), .b(x01), .c(x12), .O(new_n66_));
  nand4  g42(.a(x14), .b(x02), .c(x01), .d(new_n26_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n57_), .c(new_n50_), .O(new_n68_));
  aoi21  g44(.a(new_n66_), .b(new_n25_), .c(new_n68_), .O(z6));
  nand2  g45(.a(x09), .b(x03), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(x01), .c(x12), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n25_), .O(new_n72_));
  nand3  g48(.a(x03), .b(new_n25_), .c(new_n47_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n72_), .c(new_n50_), .O(z7));
  inv1   g51(.a(new_n51_), .O(new_n76_));
  aoi22  g52(.a(new_n76_), .b(x00), .c(new_n41_), .d(new_n25_), .O(z8));
endmodule


