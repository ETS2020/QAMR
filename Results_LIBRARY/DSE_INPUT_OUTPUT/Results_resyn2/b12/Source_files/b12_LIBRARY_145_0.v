// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand3  g03(.a(x13), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n27_), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n25_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(new_n28_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x03), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n25_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n36_), .c(x00), .O(z1));
  inv1   g17(.a(x13), .O(new_n42_));
  nor2   g18(.a(new_n42_), .b(x02), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n46_), .c(new_n31_), .O(new_n51_));
  inv1   g27(.a(x00), .O(new_n52_));
  nor2   g28(.a(x08), .b(x07), .O(new_n53_));
  nor2   g29(.a(x13), .b(x02), .O(new_n54_));
  nor3   g30(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(new_n43_), .c(new_n55_), .O(z2));
  inv1   g32(.a(new_n54_), .O(new_n57_));
  inv1   g33(.a(x12), .O(new_n58_));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  oai21  g35(.a(new_n58_), .b(x00), .c(new_n59_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n57_), .O(z3));
  inv1   g37(.a(x08), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n52_), .O(new_n63_));
  nor2   g39(.a(new_n44_), .b(x07), .O(new_n64_));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(new_n65_));
  aoi21  g41(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(z4));
  nor2   g42(.a(x12), .b(x10), .O(new_n67_));
  nand3  g43(.a(x13), .b(new_n47_), .c(new_n52_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n67_), .c(new_n54_), .O(z5));
  nand3  g46(.a(x14), .b(x01), .c(new_n52_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x02), .O(new_n72_));
  nand2  g48(.a(new_n44_), .b(x03), .O(new_n73_));
  nand2  g49(.a(x09), .b(new_n38_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n31_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n43_), .O(new_n76_));
  oai21  g52(.a(new_n62_), .b(new_n52_), .c(new_n45_), .O(new_n77_));
  aoi21  g53(.a(new_n76_), .b(new_n72_), .c(new_n77_), .O(z6));
  nand3  g54(.a(x03), .b(new_n27_), .c(new_n31_), .O(new_n79_));
  inv1   g55(.a(new_n79_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x09), .O(new_n81_));
  aoi21  g57(.a(new_n79_), .b(new_n62_), .c(new_n49_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n81_), .c(new_n54_), .O(z7));
  nand2  g59(.a(x03), .b(new_n31_), .O(new_n84_));
  nand4  g60(.a(new_n58_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(x13), .O(new_n86_));
  nand3  g62(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n87_));
  inv1   g63(.a(new_n87_), .O(new_n88_));
  aoi21  g64(.a(new_n86_), .b(new_n27_), .c(new_n88_), .O(z8));
endmodule


