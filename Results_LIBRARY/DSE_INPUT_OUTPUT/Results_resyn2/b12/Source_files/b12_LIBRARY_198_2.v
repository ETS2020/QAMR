// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x09), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(x02), .O(new_n37_));
  inv1   g13(.a(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n38_), .c(x01), .O(new_n39_));
  oai21  g15(.a(new_n37_), .b(new_n36_), .c(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n25_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n33_), .O(z1));
  inv1   g18(.a(x01), .O(new_n43_));
  inv1   g19(.a(x03), .O(new_n44_));
  nor2   g20(.a(x10), .b(x07), .O(new_n45_));
  oai21  g21(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  oai21  g22(.a(new_n45_), .b(x02), .c(new_n32_), .O(new_n47_));
  oai21  g23(.a(x08), .b(x07), .c(x00), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g25(.a(new_n46_), .b(new_n38_), .c(new_n49_), .O(z2));
  nand2  g26(.a(x12), .b(new_n25_), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n33_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(z3));
  nand2  g30(.a(new_n52_), .b(new_n33_), .O(new_n55_));
  inv1   g31(.a(x08), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n25_), .O(new_n57_));
  nor2   g33(.a(new_n32_), .b(x07), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n57_), .c(new_n55_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n33_), .d(x13), .O(z5));
  nand3  g38(.a(x14), .b(x01), .c(new_n25_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x02), .O(new_n64_));
  nand2  g40(.a(new_n32_), .b(x03), .O(new_n65_));
  oai21  g41(.a(new_n32_), .b(x03), .c(new_n43_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n38_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  oai21  g44(.a(new_n56_), .b(new_n25_), .c(new_n45_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n33_), .O(z6));
  nand3  g48(.a(new_n45_), .b(x03), .c(new_n43_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n38_), .c(x09), .O(new_n74_));
  nor2   g50(.a(x02), .b(x01), .O(new_n75_));
  nand2  g51(.a(new_n45_), .b(x08), .O(new_n76_));
  aoi21  g52(.a(new_n75_), .b(x03), .c(new_n76_), .O(new_n77_));
  nor2   g53(.a(new_n77_), .b(new_n74_), .O(z7));
  inv1   g54(.a(x12), .O(new_n79_));
  nand4  g55(.a(new_n79_), .b(x09), .c(x03), .d(new_n43_), .O(new_n80_));
  oai21  g56(.a(x09), .b(new_n25_), .c(new_n80_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n45_), .c(new_n38_), .O(z8));
endmodule


