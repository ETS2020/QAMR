// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n65_, new_n66_;
  inv1   g00(.a(x02), .O(new_n25_));
  oai21  g01(.a(x05), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  oai21  g05(.a(new_n25_), .b(new_n28_), .c(new_n29_), .O(new_n30_));
  nand2  g06(.a(new_n25_), .b(new_n28_), .O(new_n31_));
  inv1   g07(.a(x07), .O(new_n32_));
  inv1   g08(.a(x09), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g10(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n27_), .O(new_n35_));
  aoi21  g11(.a(new_n26_), .b(x04), .c(new_n35_), .O(z0));
  nand3  g12(.a(x03), .b(new_n25_), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n27_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n34_), .O(z1));
  oai21  g19(.a(x10), .b(x07), .c(x03), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n28_), .O(new_n45_));
  oai21  g21(.a(x08), .b(x07), .c(x00), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  aoi21  g23(.a(new_n45_), .b(new_n25_), .c(new_n47_), .O(z2));
  nor2   g24(.a(new_n33_), .b(x07), .O(new_n49_));
  inv1   g25(.a(new_n49_), .O(new_n50_));
  inv1   g26(.a(x11), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x07), .O(new_n52_));
  aoi22  g28(.a(new_n52_), .b(new_n50_), .c(x12), .d(new_n27_), .O(z3));
  nand2  g29(.a(x08), .b(x00), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n52_), .O(z4));
  nor2   g32(.a(x12), .b(x10), .O(new_n57_));
  nand4  g33(.a(new_n57_), .b(new_n49_), .c(x13), .d(new_n27_), .O(z5));
  inv1   g34(.a(x10), .O(new_n59_));
  nand3  g35(.a(x03), .b(new_n25_), .c(new_n28_), .O(new_n60_));
  nand4  g36(.a(x14), .b(x02), .c(x01), .d(new_n27_), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n54_), .d(new_n59_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(x09), .c(x07), .O(z6));
  nand4  g39(.a(new_n60_), .b(new_n49_), .c(new_n59_), .d(x08), .O(z7));
  inv1   g40(.a(new_n57_), .O(new_n65_));
  oai21  g41(.a(new_n60_), .b(new_n65_), .c(x09), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n32_), .O(z8));
endmodule


