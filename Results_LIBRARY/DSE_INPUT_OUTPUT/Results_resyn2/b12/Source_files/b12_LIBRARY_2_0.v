// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x03), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x00), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n29_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n28_), .O(z0));
  inv1   g10(.a(x03), .O(new_n35_));
  nand2  g11(.a(x04), .b(x02), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g13(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n38_));
  nor2   g14(.a(x02), .b(x01), .O(new_n39_));
  nor2   g15(.a(new_n39_), .b(x00), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(z1));
  inv1   g18(.a(x10), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(x09), .c(new_n26_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nor2   g21(.a(x08), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(new_n35_), .b(new_n25_), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  nand4  g25(.a(new_n43_), .b(x09), .c(new_n49_), .d(new_n26_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n29_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n48_), .c(new_n47_), .O(z2));
  nand2  g29(.a(x12), .b(new_n29_), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n54_), .c(new_n48_), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(z3));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  nand2  g34(.a(x09), .b(new_n49_), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n48_), .O(z4));
  nor2   g37(.a(x12), .b(x10), .O(new_n62_));
  nor2   g38(.a(x07), .b(x00), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n48_), .d(x13), .O(z5));
  nor2   g40(.a(x10), .b(x07), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n29_), .O(new_n66_));
  nand3  g42(.a(x09), .b(new_n25_), .c(new_n26_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n66_), .c(new_n58_), .d(new_n65_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n48_), .O(z6));
  inv1   g45(.a(x09), .O(new_n70_));
  nand3  g46(.a(new_n65_), .b(new_n70_), .c(new_n26_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x03), .O(new_n72_));
  inv1   g48(.a(new_n65_), .O(new_n73_));
  oai21  g49(.a(x02), .b(x01), .c(x08), .O(new_n74_));
  nor2   g50(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g51(.a(new_n72_), .b(new_n25_), .c(new_n75_), .O(z7));
  nand3  g52(.a(new_n48_), .b(new_n70_), .c(x00), .O(new_n77_));
  inv1   g53(.a(x12), .O(new_n78_));
  nand4  g54(.a(new_n39_), .b(new_n78_), .c(x09), .d(x03), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n65_), .O(z8));
endmodule


