// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  nand3  g02(.a(x03), .b(x01), .c(new_n26_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x08), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n29_), .O(z0));
  nor2   g10(.a(x08), .b(x02), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  nand4  g12(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(x04), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g16(.a(x02), .b(x01), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n40_), .c(new_n37_), .d(new_n26_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n36_), .O(z1));
  inv1   g20(.a(x01), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n25_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n26_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  oai21  g30(.a(new_n46_), .b(new_n25_), .c(new_n54_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n53_), .O(z2));
  nand2  g32(.a(x12), .b(new_n26_), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n57_), .c(new_n36_), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(z3));
  nand2  g36(.a(new_n58_), .b(new_n36_), .O(new_n61_));
  nor2   g37(.a(x07), .b(new_n26_), .O(new_n62_));
  nor2   g38(.a(new_n47_), .b(new_n54_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z4));
  inv1   g40(.a(x12), .O(new_n65_));
  nand3  g41(.a(x13), .b(new_n65_), .c(new_n48_), .O(new_n66_));
  nor3   g42(.a(new_n66_), .b(x07), .c(x00), .O(new_n67_));
  nor2   g43(.a(new_n67_), .b(new_n35_), .O(z5));
  xor2a  g44(.a(x09), .b(x03), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n42_), .c(new_n26_), .O(new_n70_));
  nor2   g46(.a(x10), .b(x07), .O(new_n71_));
  nand4  g47(.a(x14), .b(x02), .c(x01), .d(new_n26_), .O(new_n72_));
  oai21  g48(.a(new_n71_), .b(new_n35_), .c(new_n72_), .O(new_n73_));
  aoi21  g49(.a(new_n70_), .b(x08), .c(new_n73_), .O(z6));
  nor2   g50(.a(new_n47_), .b(new_n38_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n41_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n71_), .c(x08), .O(z7));
  nand3  g53(.a(new_n36_), .b(new_n47_), .c(x00), .O(new_n78_));
  nand4  g54(.a(new_n75_), .b(new_n41_), .c(new_n65_), .d(x08), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n71_), .O(z8));
endmodule


