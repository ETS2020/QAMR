// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand3  g01(.a(x03), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(x08), .O(new_n27_));
  nand2  g03(.a(x05), .b(x01), .O(new_n28_));
  nor2   g04(.a(x03), .b(x01), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n28_), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(new_n27_), .c(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n26_), .c(x00), .O(z0));
  inv1   g08(.a(x00), .O(new_n33_));
  nand3  g09(.a(x06), .b(x05), .c(x01), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  oai21  g11(.a(x04), .b(x03), .c(x02), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n35_), .c(new_n26_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  nor2   g14(.a(new_n27_), .b(new_n25_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n38_), .O(z1));
  inv1   g17(.a(x03), .O(new_n42_));
  oai22  g18(.a(x08), .b(new_n33_), .c(new_n42_), .d(x02), .O(new_n43_));
  inv1   g19(.a(x01), .O(new_n44_));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(x10), .b(x03), .O(new_n48_));
  nand2  g24(.a(x08), .b(x00), .O(new_n49_));
  nand4  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n44_), .O(new_n50_));
  aoi22  g26(.a(new_n50_), .b(new_n25_), .c(new_n43_), .d(x07), .O(z2));
  inv1   g27(.a(x12), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  oai21  g29(.a(new_n52_), .b(x00), .c(new_n53_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n40_), .O(z3));
  inv1   g31(.a(x07), .O(new_n56_));
  nand3  g32(.a(x09), .b(new_n56_), .c(x00), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(x08), .c(x11), .d(x07), .O(z4));
  inv1   g35(.a(x10), .O(new_n60_));
  nand3  g36(.a(x13), .b(new_n52_), .c(new_n60_), .O(new_n61_));
  nor3   g37(.a(new_n61_), .b(x07), .c(x00), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n39_), .O(z5));
  nand2  g39(.a(new_n45_), .b(x03), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n42_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n44_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(new_n49_), .c(x02), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n67_), .c(new_n40_), .O(z6));
  nand3  g46(.a(x09), .b(x03), .c(new_n44_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n25_), .O(new_n73_));
  nand3  g49(.a(x03), .b(new_n25_), .c(new_n44_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n27_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n73_), .O(z7));
  oai21  g52(.a(new_n74_), .b(x12), .c(x09), .O(new_n77_));
  oai21  g53(.a(new_n39_), .b(new_n33_), .c(new_n45_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n46_), .O(z8));
endmodule


