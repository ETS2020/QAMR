// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  oai21  g05(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  nand4  g08(.a(new_n32_), .b(new_n30_), .c(new_n26_), .d(new_n25_), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x02), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n26_), .c(new_n27_), .O(new_n36_));
  nand2  g12(.a(new_n26_), .b(new_n29_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n32_), .c(new_n25_), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(new_n36_), .O(z1));
  nand2  g15(.a(x04), .b(x01), .O(new_n40_));
  oai21  g16(.a(x08), .b(x07), .c(x00), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x09), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  oai21  g21(.a(x10), .b(x07), .c(x03), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n27_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n28_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n40_), .O(z2));
  inv1   g26(.a(x12), .O(new_n51_));
  nand2  g27(.a(x11), .b(x07), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(x00), .c(new_n52_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n40_), .O(z3));
  inv1   g30(.a(x08), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(new_n25_), .O(new_n56_));
  nor2   g32(.a(new_n43_), .b(x07), .O(new_n57_));
  nand2  g33(.a(new_n52_), .b(new_n40_), .O(new_n58_));
  aoi21  g34(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z4));
  nor2   g35(.a(x12), .b(x10), .O(new_n60_));
  nor2   g36(.a(x07), .b(x00), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(new_n60_), .c(new_n40_), .d(x13), .O(z5));
  nor2   g38(.a(x10), .b(x07), .O(new_n63_));
  inv1   g39(.a(new_n56_), .O(new_n64_));
  nand2  g40(.a(new_n43_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n29_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n31_), .O(new_n67_));
  nand4  g43(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n68_));
  nand4  g44(.a(new_n68_), .b(new_n67_), .c(new_n64_), .d(new_n63_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n40_), .O(z6));
  oai21  g46(.a(new_n55_), .b(x04), .c(x01), .O(new_n71_));
  oai21  g47(.a(new_n29_), .b(x02), .c(new_n55_), .O(new_n72_));
  nand3  g48(.a(new_n31_), .b(x09), .c(x03), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n63_), .O(z7));
  nand3  g50(.a(new_n40_), .b(new_n43_), .c(x00), .O(new_n75_));
  oai21  g51(.a(new_n73_), .b(x12), .c(new_n75_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n63_), .O(z8));
endmodule


