// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand4  g06(.a(new_n30_), .b(new_n27_), .c(x01), .d(new_n25_), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(z0));
  nand4  g08(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n33_));
  nand2  g09(.a(x04), .b(x02), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n29_), .O(new_n35_));
  inv1   g11(.a(x01), .O(new_n36_));
  aoi21  g12(.a(x03), .b(new_n36_), .c(x00), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(new_n35_), .c(new_n33_), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(z1));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x09), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n29_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n36_), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n40_), .O(new_n46_));
  aoi21  g22(.a(x03), .b(new_n36_), .c(new_n25_), .O(new_n47_));
  aoi22  g23(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(new_n28_), .O(z2));
  nand2  g24(.a(x03), .b(new_n36_), .O(new_n49_));
  and2   g25(.a(x11), .b(x07), .O(new_n50_));
  inv1   g26(.a(x12), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(x00), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(z3));
  nand4  g29(.a(x09), .b(x08), .c(new_n40_), .d(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n50_), .c(new_n49_), .O(z4));
  nor2   g32(.a(x10), .b(x07), .O(new_n57_));
  nand4  g33(.a(new_n57_), .b(new_n37_), .c(x13), .d(new_n51_), .O(z5));
  oai21  g34(.a(new_n45_), .b(new_n25_), .c(new_n57_), .O(new_n59_));
  nand4  g35(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n60_));
  nand4  g36(.a(new_n41_), .b(new_n29_), .c(new_n28_), .d(new_n36_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g38(.a(new_n59_), .b(new_n49_), .c(new_n62_), .O(z6));
  nand3  g39(.a(new_n57_), .b(new_n49_), .c(x08), .O(z7));
  nand3  g40(.a(new_n47_), .b(new_n57_), .c(new_n41_), .O(z8));
endmodule


