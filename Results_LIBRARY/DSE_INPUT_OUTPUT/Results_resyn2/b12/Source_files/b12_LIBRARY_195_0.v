// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  oai21  g01(.a(x02), .b(x01), .c(x03), .O(new_n26_));
  aoi21  g02(.a(x04), .b(x02), .c(new_n26_), .O(new_n27_));
  inv1   g03(.a(x05), .O(new_n28_));
  nand2  g04(.a(x02), .b(x01), .O(new_n29_));
  aoi21  g05(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n27_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x07), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(z0));
  inv1   g10(.a(x01), .O(new_n35_));
  inv1   g11(.a(x02), .O(new_n36_));
  nand2  g12(.a(x03), .b(new_n36_), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .d(new_n35_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n33_), .O(z1));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n36_), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(x05), .c(x07), .O(new_n47_));
  inv1   g23(.a(x03), .O(new_n48_));
  oai21  g24(.a(new_n45_), .b(new_n48_), .c(new_n35_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  oai21  g26(.a(new_n48_), .b(x02), .c(new_n25_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x07), .O(new_n52_));
  inv1   g28(.a(x08), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n25_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n52_), .c(new_n50_), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n47_), .O(z2));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  aoi22  g35(.a(new_n59_), .b(new_n33_), .c(x11), .d(x07), .O(z3));
  nor2   g36(.a(new_n44_), .b(x07), .O(new_n61_));
  nand2  g37(.a(x11), .b(x07), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n33_), .O(new_n63_));
  aoi21  g39(.a(new_n61_), .b(new_n54_), .c(new_n63_), .O(z4));
  nor2   g40(.a(x07), .b(new_n28_), .O(new_n65_));
  nor2   g41(.a(x10), .b(x00), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(x13), .d(new_n58_), .O(z5));
  nor2   g43(.a(x02), .b(x01), .O(new_n68_));
  xnor2a g44(.a(x09), .b(x03), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n70_));
  nand3  g46(.a(new_n45_), .b(new_n32_), .c(x05), .O(new_n71_));
  inv1   g47(.a(new_n71_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n55_), .O(new_n73_));
  aoi21  g49(.a(new_n69_), .b(new_n68_), .c(new_n73_), .O(z6));
  nand4  g50(.a(x09), .b(x03), .c(new_n36_), .d(new_n35_), .O(new_n75_));
  oai21  g51(.a(new_n37_), .b(x01), .c(new_n53_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n72_), .O(z7));
  oai22  g53(.a(new_n75_), .b(x12), .c(x09), .d(new_n25_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n72_), .O(z8));
endmodule


