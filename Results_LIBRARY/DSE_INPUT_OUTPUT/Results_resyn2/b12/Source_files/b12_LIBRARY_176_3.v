// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  inv1   g07(.a(x07), .O(new_n32_));
  inv1   g08(.a(x10), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n31_), .O(z0));
  inv1   g12(.a(x02), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n37_), .c(x01), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  oai21  g16(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  oai21  g17(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n25_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n35_), .O(z1));
  aoi21  g20(.a(x03), .b(new_n37_), .c(x00), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n32_), .c(x01), .O(new_n47_));
  oai22  g23(.a(new_n47_), .b(x02), .c(new_n45_), .d(new_n32_), .O(new_n48_));
  nand2  g24(.a(x08), .b(x00), .O(new_n49_));
  inv1   g25(.a(x01), .O(new_n50_));
  nand2  g26(.a(x10), .b(x03), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n49_), .c(x07), .O(new_n54_));
  aoi21  g30(.a(new_n48_), .b(new_n33_), .c(new_n54_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n35_), .O(z3));
  inv1   g35(.a(new_n49_), .O(new_n60_));
  nor2   g36(.a(new_n46_), .b(x07), .O(new_n61_));
  aoi22  g37(.a(new_n61_), .b(new_n60_), .c(new_n56_), .d(new_n33_), .O(z4));
  nor2   g38(.a(x10), .b(x07), .O(new_n63_));
  nand3  g39(.a(x13), .b(new_n57_), .c(new_n25_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n63_), .c(new_n34_), .O(z5));
  nand2  g42(.a(new_n37_), .b(new_n50_), .O(new_n67_));
  xor2a  g43(.a(x09), .b(x03), .O(new_n68_));
  nor2   g44(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g45(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n63_), .c(new_n49_), .O(new_n71_));
  nor2   g47(.a(new_n71_), .b(new_n69_), .O(z6));
  inv1   g48(.a(x03), .O(new_n73_));
  inv1   g49(.a(x08), .O(new_n74_));
  oai21  g50(.a(new_n67_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand4  g51(.a(x09), .b(x03), .c(new_n37_), .d(new_n50_), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n75_), .c(new_n63_), .O(z7));
  aoi21  g53(.a(new_n46_), .b(x00), .c(x07), .O(new_n78_));
  oai21  g54(.a(new_n76_), .b(x12), .c(new_n78_), .O(new_n79_));
  oai21  g55(.a(new_n63_), .b(new_n34_), .c(new_n79_), .O(z8));
endmodule


