// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x02), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(x02), .b(x01), .O(new_n33_));
  aoi21  g09(.a(new_n32_), .b(x04), .c(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n31_), .c(new_n25_), .O(new_n35_));
  inv1   g11(.a(x09), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n35_), .O(z0));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  oai22  g18(.a(new_n42_), .b(new_n41_), .c(new_n28_), .d(new_n26_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n38_), .O(z1));
  oai21  g21(.a(x10), .b(x07), .c(new_n26_), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x09), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(x00), .c(x01), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nand2  g30(.a(new_n36_), .b(x00), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  aoi21  g32(.a(new_n54_), .b(new_n27_), .c(new_n56_), .O(z2));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n38_), .O(new_n59_));
  aoi21  g35(.a(x12), .b(new_n25_), .c(new_n59_), .O(z3));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(x07), .c(new_n27_), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(x09), .c(x11), .d(x07), .O(z4));
  nor2   g39(.a(x12), .b(x10), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(x13), .c(new_n50_), .d(new_n25_), .O(new_n65_));
  and2   g41(.a(new_n65_), .b(new_n38_), .O(z5));
  nand3  g42(.a(x14), .b(x01), .c(new_n25_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n36_), .c(new_n27_), .O(new_n68_));
  oai21  g44(.a(x09), .b(x02), .c(new_n26_), .O(new_n69_));
  aoi21  g45(.a(new_n36_), .b(x03), .c(x01), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n61_), .c(new_n47_), .O(new_n72_));
  nor2   g48(.a(new_n72_), .b(new_n68_), .O(z6));
  inv1   g49(.a(x01), .O(new_n74_));
  aoi21  g50(.a(x03), .b(new_n74_), .c(x08), .O(new_n75_));
  nand3  g51(.a(x09), .b(x03), .c(new_n74_), .O(new_n76_));
  inv1   g52(.a(new_n76_), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(new_n75_), .c(new_n27_), .O(new_n78_));
  nand2  g54(.a(new_n51_), .b(x02), .O(new_n79_));
  and2   g55(.a(new_n79_), .b(new_n47_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n78_), .c(new_n37_), .O(z7));
  oai21  g57(.a(new_n76_), .b(x12), .c(new_n55_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n47_), .c(new_n37_), .O(z8));
endmodule


