// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x13), .b(x07), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n28_), .c(new_n31_), .O(new_n32_));
  inv1   g08(.a(x03), .O(new_n33_));
  nor2   g09(.a(x02), .b(x01), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g11(.a(new_n28_), .b(new_n30_), .c(new_n35_), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n32_), .c(new_n27_), .O(z0));
  nand3  g13(.a(new_n31_), .b(x06), .c(x05), .O(new_n38_));
  oai21  g14(.a(new_n33_), .b(new_n29_), .c(new_n30_), .O(new_n39_));
  aoi21  g15(.a(new_n28_), .b(new_n33_), .c(x00), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n26_), .O(z1));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nor2   g19(.a(new_n43_), .b(x03), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n44_), .c(new_n29_), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  aoi22  g26(.a(new_n26_), .b(new_n25_), .c(new_n50_), .d(new_n45_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n30_), .c(new_n51_), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  inv1   g29(.a(x13), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(x07), .c(x12), .d(new_n25_), .O(z3));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x07), .O(new_n58_));
  nor2   g34(.a(new_n50_), .b(new_n25_), .O(new_n59_));
  nor2   g35(.a(x13), .b(new_n45_), .O(new_n60_));
  aoi22  g36(.a(new_n60_), .b(x11), .c(new_n59_), .d(new_n58_), .O(z4));
  inv1   g37(.a(x12), .O(new_n62_));
  nand4  g38(.a(new_n43_), .b(x13), .c(new_n62_), .d(new_n25_), .O(z5));
  inv1   g39(.a(new_n59_), .O(new_n64_));
  nand2  g40(.a(x09), .b(new_n33_), .O(new_n65_));
  nand2  g41(.a(new_n57_), .b(x03), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n34_), .O(new_n67_));
  nand3  g43(.a(new_n31_), .b(x14), .c(new_n25_), .O(new_n68_));
  nand4  g44(.a(new_n68_), .b(new_n67_), .c(new_n64_), .d(new_n43_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n26_), .O(z6));
  aoi21  g46(.a(new_n34_), .b(x03), .c(new_n50_), .O(new_n71_));
  nand3  g47(.a(new_n34_), .b(new_n57_), .c(x03), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n71_), .c(new_n43_), .O(z7));
  inv1   g50(.a(new_n60_), .O(new_n75_));
  nand3  g51(.a(new_n34_), .b(new_n62_), .c(x03), .O(new_n76_));
  oai21  g52(.a(x09), .b(x00), .c(new_n46_), .O(new_n77_));
  aoi21  g53(.a(new_n76_), .b(x09), .c(new_n77_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x07), .c(new_n75_), .O(z8));
endmodule


