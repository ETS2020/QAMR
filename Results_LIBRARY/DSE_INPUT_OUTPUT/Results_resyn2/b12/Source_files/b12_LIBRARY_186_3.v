// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand3  g01(.a(x03), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x01), .O(new_n28_));
  nor2   g04(.a(x03), .b(x01), .O(new_n29_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n28_), .O(new_n30_));
  aoi21  g06(.a(new_n30_), .b(x02), .c(new_n27_), .O(new_n31_));
  nand2  g07(.a(x14), .b(x04), .O(new_n32_));
  oai21  g08(.a(new_n31_), .b(x00), .c(new_n32_), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(x02), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n36_), .c(new_n26_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n32_), .O(z1));
  oai21  g16(.a(x08), .b(x07), .c(x00), .O(new_n41_));
  inv1   g17(.a(x01), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x09), .O(new_n44_));
  inv1   g20(.a(x10), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n42_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n25_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n32_), .O(z2));
  and2   g27(.a(x11), .b(x07), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(x00), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n52_), .c(new_n32_), .O(z3));
  nand4  g31(.a(x09), .b(x08), .c(new_n43_), .d(x00), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n52_), .c(new_n32_), .O(z4));
  nor2   g34(.a(x12), .b(x10), .O(new_n59_));
  nor2   g35(.a(x07), .b(x00), .O(new_n60_));
  nand4  g36(.a(new_n60_), .b(new_n59_), .c(new_n32_), .d(x13), .O(z5));
  xor2a  g37(.a(x09), .b(x03), .O(new_n62_));
  nor3   g38(.a(new_n62_), .b(x02), .c(x01), .O(new_n63_));
  nor2   g39(.a(x10), .b(x07), .O(new_n64_));
  nand2  g40(.a(x08), .b(x00), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n63_), .c(new_n32_), .O(z6));
  inv1   g44(.a(new_n32_), .O(new_n69_));
  nand4  g45(.a(x09), .b(x03), .c(new_n25_), .d(new_n42_), .O(new_n70_));
  inv1   g46(.a(x08), .O(new_n71_));
  inv1   g47(.a(new_n64_), .O(new_n72_));
  nand3  g48(.a(x03), .b(new_n25_), .c(new_n42_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n71_), .c(new_n72_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n70_), .c(new_n69_), .O(z7));
  oai22  g51(.a(new_n70_), .b(x12), .c(x09), .d(new_n34_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n64_), .c(new_n69_), .O(z8));
endmodule


