// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  nor2   g07(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g08(.a(new_n25_), .b(new_n27_), .c(new_n32_), .O(new_n33_));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x09), .O(new_n35_));
  inv1   g11(.a(x10), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  aoi21  g15(.a(new_n33_), .b(new_n29_), .c(new_n39_), .O(z0));
  nand3  g16(.a(new_n28_), .b(x06), .c(x05), .O(new_n41_));
  oai21  g17(.a(new_n30_), .b(new_n26_), .c(new_n27_), .O(new_n42_));
  aoi21  g18(.a(new_n25_), .b(new_n30_), .c(x00), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n38_), .O(z1));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand3  g23(.a(x09), .b(new_n47_), .c(new_n46_), .O(new_n48_));
  nand2  g24(.a(x07), .b(x03), .O(new_n49_));
  nand2  g25(.a(new_n35_), .b(new_n46_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n34_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n26_), .O(new_n53_));
  oai21  g29(.a(x08), .b(x07), .c(x00), .O(new_n54_));
  nand3  g30(.a(x10), .b(new_n30_), .c(new_n26_), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n27_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n54_), .c(new_n37_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n53_), .O(z2));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n38_), .O(new_n60_));
  aoi21  g36(.a(x12), .b(new_n34_), .c(new_n60_), .O(z3));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(x07), .c(new_n36_), .O(new_n63_));
  aoi22  g39(.a(new_n63_), .b(x09), .c(x11), .d(x07), .O(z4));
  nor2   g40(.a(x10), .b(x07), .O(new_n65_));
  nor2   g41(.a(x12), .b(x00), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(x13), .O(z5));
  nand2  g43(.a(new_n31_), .b(new_n30_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n36_), .c(x09), .O(new_n69_));
  nand3  g45(.a(new_n31_), .b(x09), .c(x03), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n62_), .d(new_n46_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n36_), .c(new_n69_), .O(z6));
  nand2  g49(.a(new_n31_), .b(x03), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  and2   g51(.a(new_n70_), .b(new_n65_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(new_n37_), .O(z7));
  oai22  g53(.a(new_n70_), .b(x12), .c(x09), .d(new_n34_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n65_), .O(z8));
endmodule


