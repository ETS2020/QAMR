// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand3  g01(.a(x03), .b(new_n25_), .c(x01), .O(new_n26_));
  inv1   g02(.a(x05), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  oai21  g06(.a(x04), .b(new_n30_), .c(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n26_), .c(x00), .O(z0));
  nand2  g09(.a(x06), .b(x01), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(new_n34_), .c(new_n28_), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(new_n26_), .c(x00), .O(z1));
  inv1   g14(.a(x09), .O(new_n39_));
  nor2   g15(.a(x10), .b(x07), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  inv1   g18(.a(x10), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x03), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n41_), .c(new_n29_), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  inv1   g24(.a(x00), .O(new_n49_));
  nor2   g25(.a(x05), .b(new_n25_), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi22  g27(.a(new_n51_), .b(new_n48_), .c(new_n46_), .d(new_n25_), .O(z2));
  inv1   g28(.a(new_n50_), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  oai21  g31(.a(new_n54_), .b(x00), .c(new_n55_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(z3));
  nor2   g33(.a(new_n47_), .b(new_n49_), .O(new_n58_));
  nor2   g34(.a(new_n39_), .b(x07), .O(new_n59_));
  nand2  g35(.a(new_n55_), .b(new_n53_), .O(new_n60_));
  aoi21  g36(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z4));
  nor3   g37(.a(new_n50_), .b(x07), .c(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n54_), .d(new_n43_), .O(z5));
  nand3  g39(.a(x14), .b(x01), .c(new_n49_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n28_), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n30_), .O(new_n66_));
  nand2  g42(.a(new_n39_), .b(x03), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n29_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n25_), .O(new_n69_));
  oai21  g45(.a(new_n47_), .b(new_n49_), .c(new_n40_), .O(new_n70_));
  aoi21  g46(.a(new_n69_), .b(new_n65_), .c(new_n70_), .O(z6));
  aoi21  g47(.a(x03), .b(new_n29_), .c(x08), .O(new_n72_));
  nand3  g48(.a(x09), .b(x03), .c(new_n29_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n72_), .c(new_n25_), .O(new_n75_));
  aoi21  g51(.a(new_n47_), .b(x02), .c(new_n44_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(new_n50_), .O(z7));
  nand2  g53(.a(new_n39_), .b(x00), .O(new_n78_));
  nand2  g54(.a(new_n54_), .b(new_n25_), .O(new_n79_));
  oai22  g55(.a(new_n79_), .b(new_n73_), .c(new_n78_), .d(new_n50_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n40_), .O(z8));
endmodule


