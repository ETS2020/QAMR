// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x03), .b(x01), .c(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(z0));
  nand3  g06(.a(x06), .b(x05), .c(x01), .O(new_n31_));
  inv1   g07(.a(x03), .O(new_n32_));
  aoi21  g08(.a(new_n28_), .b(new_n32_), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(z1));
  oai21  g11(.a(x08), .b(x07), .c(x00), .O(new_n36_));
  inv1   g12(.a(x07), .O(new_n37_));
  inv1   g13(.a(x10), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(x09), .c(new_n37_), .O(new_n39_));
  inv1   g15(.a(new_n39_), .O(new_n40_));
  inv1   g16(.a(x02), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(x03), .c(new_n41_), .O(new_n43_));
  oai21  g19(.a(new_n43_), .b(new_n40_), .c(new_n36_), .O(new_n44_));
  inv1   g20(.a(x01), .O(new_n45_));
  nor2   g21(.a(x02), .b(new_n45_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n44_), .O(z2));
  nand2  g24(.a(x12), .b(new_n25_), .O(new_n49_));
  and2   g25(.a(x11), .b(x07), .O(new_n50_));
  nor2   g26(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(z3));
  nand4  g29(.a(x09), .b(x08), .c(new_n37_), .d(x00), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n50_), .c(new_n47_), .O(z4));
  nor2   g32(.a(x12), .b(x10), .O(new_n57_));
  nor2   g33(.a(x07), .b(x00), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n57_), .c(new_n47_), .d(x13), .O(z5));
  nand2  g35(.a(x09), .b(x03), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n32_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n60_), .c(x02), .O(new_n63_));
  nand2  g39(.a(x08), .b(x00), .O(new_n64_));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n42_), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n63_), .c(new_n47_), .O(z6));
  nand2  g43(.a(new_n60_), .b(new_n45_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n41_), .O(new_n69_));
  inv1   g45(.a(x08), .O(new_n70_));
  nand2  g46(.a(x03), .b(new_n41_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n69_), .c(new_n42_), .O(z7));
  nand2  g49(.a(new_n61_), .b(x00), .O(new_n74_));
  nor2   g50(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  inv1   g51(.a(x12), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(x09), .c(new_n45_), .O(new_n77_));
  nor2   g53(.a(new_n77_), .b(new_n71_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n75_), .c(new_n42_), .O(z8));
endmodule


