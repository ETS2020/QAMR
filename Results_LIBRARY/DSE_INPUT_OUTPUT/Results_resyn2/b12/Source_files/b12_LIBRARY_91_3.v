// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x04), .O(new_n27_));
  inv1   g03(.a(x10), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(new_n27_), .c(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand4  g09(.a(new_n33_), .b(new_n28_), .c(x02), .d(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand3  g12(.a(x03), .b(new_n25_), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  nand2  g18(.a(x10), .b(x02), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(z1));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi21  g23(.a(x07), .b(x03), .c(x01), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  oai21  g26(.a(x03), .b(x02), .c(x10), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n49_), .b(new_n25_), .c(new_n52_), .O(z2));
  and2   g29(.a(x11), .b(x07), .O(new_n54_));
  inv1   g30(.a(x12), .O(new_n55_));
  nor2   g31(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(new_n54_), .c(new_n43_), .O(z3));
  inv1   g33(.a(x07), .O(new_n58_));
  nand4  g34(.a(x09), .b(x08), .c(new_n58_), .d(x00), .O(new_n59_));
  inv1   g35(.a(new_n59_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n54_), .c(new_n43_), .O(z4));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(x13), .c(new_n55_), .d(new_n28_), .O(new_n63_));
  and2   g39(.a(new_n63_), .b(new_n43_), .O(z5));
  inv1   g40(.a(x03), .O(new_n65_));
  nand2  g41(.a(new_n45_), .b(new_n65_), .O(new_n66_));
  nand2  g42(.a(x09), .b(x03), .O(new_n67_));
  inv1   g43(.a(x01), .O(new_n68_));
  nand2  g44(.a(new_n25_), .b(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n67_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand2  g46(.a(x08), .b(x00), .O(new_n71_));
  nand4  g47(.a(x14), .b(x02), .c(x01), .d(new_n36_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n46_), .O(new_n73_));
  nor2   g49(.a(new_n73_), .b(new_n70_), .O(z6));
  oai21  g50(.a(new_n67_), .b(x01), .c(new_n28_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n25_), .O(new_n76_));
  inv1   g52(.a(x08), .O(new_n77_));
  nand3  g53(.a(x03), .b(new_n25_), .c(new_n68_), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n77_), .c(x07), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(x10), .c(new_n76_), .O(z7));
  nand2  g56(.a(new_n55_), .b(x09), .O(new_n81_));
  oai22  g57(.a(new_n81_), .b(new_n78_), .c(x09), .d(new_n36_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n46_), .O(z8));
endmodule


