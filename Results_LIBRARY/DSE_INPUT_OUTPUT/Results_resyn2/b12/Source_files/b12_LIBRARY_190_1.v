// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x02), .O(new_n25_));
  inv1   g01(.a(x13), .O(new_n26_));
  nand2  g02(.a(x03), .b(x01), .O(new_n27_));
  oai21  g03(.a(new_n27_), .b(x00), .c(new_n26_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nor2   g05(.a(new_n25_), .b(x00), .O(new_n30_));
  nand2  g06(.a(x05), .b(x01), .O(new_n31_));
  nor2   g07(.a(x03), .b(x01), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(x04), .c(new_n31_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n29_), .O(z0));
  inv1   g11(.a(x06), .O(new_n36_));
  oai22  g12(.a(new_n31_), .b(new_n36_), .c(x04), .d(x03), .O(new_n37_));
  nor2   g13(.a(x13), .b(x02), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n27_), .O(new_n39_));
  nand2  g15(.a(x13), .b(new_n25_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(x00), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  aoi21  g18(.a(new_n37_), .b(x02), .c(new_n42_), .O(z1));
  inv1   g19(.a(x00), .O(new_n44_));
  aoi21  g20(.a(x13), .b(new_n25_), .c(new_n44_), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g24(.a(x01), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  nor2   g26(.a(x10), .b(x07), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  oai21  g28(.a(x10), .b(x07), .c(x03), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n38_), .c(new_n48_), .d(new_n45_), .O(z2));
  nand2  g31(.a(x12), .b(new_n44_), .O(new_n56_));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n56_), .c(new_n40_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  nand2  g35(.a(new_n57_), .b(new_n40_), .O(new_n60_));
  nand2  g36(.a(x08), .b(x00), .O(new_n61_));
  inv1   g37(.a(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n50_), .b(x07), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(z4));
  inv1   g40(.a(x12), .O(new_n65_));
  nand4  g41(.a(new_n51_), .b(new_n30_), .c(x13), .d(new_n65_), .O(z5));
  inv1   g42(.a(x03), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n67_), .O(new_n68_));
  nand2  g44(.a(new_n50_), .b(x03), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n68_), .c(new_n25_), .d(new_n49_), .O(new_n70_));
  nand3  g46(.a(new_n30_), .b(x14), .c(x01), .O(new_n71_));
  nand2  g47(.a(new_n61_), .b(new_n51_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n40_), .O(z6));
  inv1   g51(.a(new_n40_), .O(new_n76_));
  nand3  g52(.a(x03), .b(new_n25_), .c(new_n49_), .O(new_n77_));
  inv1   g53(.a(new_n77_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x09), .O(new_n79_));
  inv1   g55(.a(x10), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  aoi21  g57(.a(new_n77_), .b(new_n47_), .c(new_n81_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(z7));
  nand2  g59(.a(x03), .b(new_n49_), .O(new_n84_));
  nand4  g60(.a(new_n65_), .b(new_n80_), .c(x09), .d(new_n46_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(new_n26_), .O(new_n86_));
  nand3  g62(.a(new_n51_), .b(new_n50_), .c(x00), .O(new_n87_));
  inv1   g63(.a(new_n87_), .O(new_n88_));
  aoi21  g64(.a(new_n86_), .b(new_n25_), .c(new_n88_), .O(z8));
endmodule


