// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x05), .O(new_n26_));
  nand2  g02(.a(x02), .b(x01), .O(new_n27_));
  aoi21  g03(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  oai21  g04(.a(x02), .b(x01), .c(x03), .O(new_n29_));
  aoi21  g05(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(new_n28_), .c(new_n25_), .O(new_n31_));
  nand2  g07(.a(x11), .b(x09), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n34_), .c(new_n35_), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand4  g15(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n40_));
  inv1   g16(.a(new_n32_), .O(new_n41_));
  nor2   g17(.a(new_n41_), .b(x00), .O(new_n42_));
  nand4  g18(.a(new_n42_), .b(new_n40_), .c(new_n39_), .d(new_n37_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  inv1   g20(.a(x09), .O(new_n45_));
  nor2   g21(.a(x10), .b(x07), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g23(.a(x10), .b(x07), .c(x03), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n34_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n32_), .O(new_n51_));
  aoi21  g27(.a(new_n49_), .b(new_n35_), .c(new_n51_), .O(z2));
  inv1   g28(.a(x07), .O(new_n53_));
  nand2  g29(.a(new_n45_), .b(new_n53_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(x11), .c(x12), .d(new_n25_), .O(z3));
  inv1   g31(.a(x11), .O(new_n56_));
  nand2  g32(.a(x08), .b(x00), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(x07), .c(new_n56_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n54_), .c(new_n32_), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nand4  g36(.a(new_n46_), .b(new_n42_), .c(x13), .d(new_n60_), .O(z5));
  nand2  g37(.a(new_n45_), .b(x03), .O(new_n62_));
  nand2  g38(.a(new_n35_), .b(new_n34_), .O(new_n63_));
  aoi21  g39(.a(x09), .b(new_n36_), .c(new_n63_), .O(new_n64_));
  nand4  g40(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(new_n57_), .c(new_n46_), .d(new_n32_), .O(new_n66_));
  aoi21  g42(.a(new_n64_), .b(new_n62_), .c(new_n66_), .O(z6));
  nand3  g43(.a(x03), .b(new_n35_), .c(new_n34_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x08), .O(new_n69_));
  nand4  g45(.a(new_n45_), .b(x03), .c(new_n35_), .d(new_n34_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n46_), .c(new_n41_), .O(z7));
  nand2  g48(.a(new_n60_), .b(x09), .O(new_n73_));
  oai22  g49(.a(new_n73_), .b(new_n68_), .c(x09), .d(new_n25_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n46_), .c(new_n41_), .O(z8));
endmodule


