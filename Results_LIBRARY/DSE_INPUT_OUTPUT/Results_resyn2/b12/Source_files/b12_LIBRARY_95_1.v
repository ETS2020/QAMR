// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(x01), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x03), .O(new_n29_));
  nor2   g05(.a(x02), .b(x01), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  inv1   g08(.a(x00), .O(new_n33_));
  inv1   g09(.a(x09), .O(new_n34_));
  nand2  g10(.a(x10), .b(new_n34_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g12(.a(new_n32_), .b(new_n27_), .c(new_n36_), .O(z0));
  inv1   g13(.a(x02), .O(new_n38_));
  nand3  g14(.a(x03), .b(new_n38_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  inv1   g16(.a(new_n40_), .O(new_n41_));
  oai21  g17(.a(x04), .b(x03), .c(x02), .O(new_n42_));
  oai21  g18(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n35_), .O(z1));
  inv1   g21(.a(x01), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x09), .O(new_n48_));
  nand3  g24(.a(x07), .b(new_n29_), .c(new_n33_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g26(.a(x08), .O(new_n51_));
  nor2   g27(.a(new_n51_), .b(new_n33_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(new_n54_));
  oai21  g30(.a(x08), .b(x07), .c(x00), .O(new_n55_));
  nand3  g31(.a(x10), .b(new_n29_), .c(new_n46_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n38_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n54_), .c(new_n35_), .O(z2));
  inv1   g35(.a(x12), .O(new_n60_));
  nand2  g36(.a(x11), .b(x07), .O(new_n61_));
  oai21  g37(.a(new_n60_), .b(x00), .c(new_n61_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n35_), .O(z3));
  nor2   g39(.a(new_n34_), .b(x07), .O(new_n64_));
  nand2  g40(.a(new_n61_), .b(new_n35_), .O(new_n65_));
  aoi21  g41(.a(new_n64_), .b(new_n52_), .c(new_n65_), .O(z4));
  nand4  g42(.a(new_n47_), .b(x13), .c(new_n60_), .d(new_n33_), .O(z5));
  nand2  g43(.a(new_n34_), .b(x03), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n29_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n30_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n33_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n53_), .d(new_n47_), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(z6));
  inv1   g49(.a(x10), .O(new_n74_));
  nand3  g50(.a(x03), .b(new_n38_), .c(new_n46_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(x09), .O(new_n77_));
  aoi21  g53(.a(new_n75_), .b(new_n51_), .c(x07), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x10), .c(new_n77_), .O(z7));
  oai21  g55(.a(new_n75_), .b(x12), .c(x09), .O(new_n80_));
  nand2  g56(.a(new_n34_), .b(new_n33_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(new_n47_), .O(z8));
endmodule


