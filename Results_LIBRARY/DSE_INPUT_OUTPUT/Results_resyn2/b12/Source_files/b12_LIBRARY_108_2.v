// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x00), .O(new_n27_));
  inv1   g03(.a(new_n27_), .O(new_n28_));
  nand2  g04(.a(x03), .b(x01), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nor2   g06(.a(x04), .b(new_n30_), .O(new_n31_));
  inv1   g07(.a(x01), .O(new_n32_));
  nand2  g08(.a(x05), .b(new_n32_), .O(new_n33_));
  oai21  g09(.a(new_n33_), .b(new_n31_), .c(x02), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n29_), .c(new_n28_), .O(z0));
  aoi21  g11(.a(x06), .b(x01), .c(new_n25_), .O(new_n36_));
  oai21  g12(.a(x04), .b(x03), .c(new_n36_), .O(new_n37_));
  inv1   g13(.a(x00), .O(new_n38_));
  inv1   g14(.a(x02), .O(new_n39_));
  nand2  g15(.a(new_n29_), .b(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  aoi21  g17(.a(new_n37_), .b(x02), .c(new_n41_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(x03), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n48_), .c(new_n32_), .O(new_n52_));
  aoi22  g28(.a(new_n52_), .b(new_n39_), .c(new_n45_), .d(new_n28_), .O(z2));
  nand2  g29(.a(x12), .b(new_n38_), .O(new_n54_));
  nor2   g30(.a(x05), .b(new_n39_), .O(new_n55_));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g34(.a(new_n58_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(new_n43_), .d(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n56_), .c(new_n26_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nor2   g39(.a(x07), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n63_), .c(new_n26_), .d(x13), .O(z5));
  xor2a  g41(.a(x09), .b(x03), .O(new_n66_));
  nor3   g42(.a(new_n66_), .b(x02), .c(x01), .O(new_n67_));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n38_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n47_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n67_), .c(new_n26_), .O(z6));
  aoi21  g47(.a(x03), .b(new_n32_), .c(x08), .O(new_n72_));
  nand3  g48(.a(x09), .b(x03), .c(new_n32_), .O(new_n73_));
  inv1   g49(.a(new_n73_), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n72_), .c(new_n39_), .O(new_n75_));
  aoi21  g51(.a(new_n44_), .b(x02), .c(new_n50_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(new_n55_), .O(z7));
  inv1   g53(.a(x12), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n39_), .O(new_n79_));
  oai22  g55(.a(new_n79_), .b(new_n73_), .c(new_n27_), .d(x09), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n47_), .O(z8));
endmodule


