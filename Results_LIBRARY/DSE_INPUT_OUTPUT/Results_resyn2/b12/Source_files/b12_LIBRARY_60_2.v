// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x07), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(x01), .O(new_n31_));
  nand2  g07(.a(x04), .b(x02), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  inv1   g09(.a(x02), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(new_n32_), .c(x03), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n31_), .c(new_n28_), .O(z0));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n27_), .c(new_n25_), .O(new_n41_));
  oai21  g17(.a(new_n38_), .b(new_n33_), .c(new_n34_), .O(new_n42_));
  nand4  g18(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nor2   g20(.a(new_n44_), .b(new_n41_), .O(z1));
  nand2  g21(.a(x10), .b(x03), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nand2  g23(.a(new_n26_), .b(new_n47_), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n46_), .c(new_n33_), .O(new_n49_));
  aoi21  g25(.a(new_n26_), .b(x07), .c(x02), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  inv1   g27(.a(x08), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g29(.a(new_n26_), .b(x07), .c(new_n25_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n53_), .c(new_n50_), .d(new_n49_), .O(z2));
  and2   g31(.a(x11), .b(x07), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n27_), .O(z3));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nor2   g37(.a(new_n47_), .b(x07), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(new_n61_), .c(new_n56_), .d(x10), .O(z4));
  nand3  g39(.a(x13), .b(new_n57_), .c(new_n25_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n51_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n26_), .O(z5));
  nor2   g42(.a(x02), .b(x01), .O(new_n67_));
  xnor2a g43(.a(x09), .b(x03), .O(new_n68_));
  nand4  g44(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n69_));
  nor2   g45(.a(x10), .b(x07), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n60_), .O(new_n71_));
  aoi21  g47(.a(new_n68_), .b(new_n67_), .c(new_n71_), .O(z6));
  nand3  g48(.a(new_n67_), .b(x09), .c(x03), .O(new_n73_));
  oai21  g49(.a(new_n35_), .b(new_n38_), .c(new_n52_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(z7));
  nand4  g51(.a(new_n67_), .b(new_n57_), .c(x09), .d(x03), .O(new_n76_));
  aoi21  g52(.a(new_n47_), .b(x00), .c(x07), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n26_), .O(z8));
endmodule


