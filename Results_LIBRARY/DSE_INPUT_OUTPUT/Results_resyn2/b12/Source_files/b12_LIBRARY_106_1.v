// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_;
  nand2  g00(.a(x04), .b(x02), .O(new_n25_));
  or2    g01(.a(new_n25_), .b(x05), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(x00), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  inv1   g08(.a(new_n32_), .O(z0));
  nand3  g09(.a(x06), .b(x05), .c(x02), .O(new_n34_));
  nand2  g10(.a(new_n25_), .b(new_n30_), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(new_n34_), .c(new_n28_), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(z1));
  nor2   g13(.a(x10), .b(x07), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(x03), .O(new_n39_));
  inv1   g15(.a(x07), .O(new_n40_));
  inv1   g16(.a(x10), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(x09), .c(new_n40_), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(new_n43_));
  oai21  g19(.a(new_n43_), .b(new_n39_), .c(new_n27_), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(x02), .b(new_n27_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x00), .O(new_n47_));
  aoi21  g23(.a(new_n45_), .b(new_n40_), .c(new_n47_), .O(new_n48_));
  aoi21  g24(.a(new_n44_), .b(new_n29_), .c(new_n48_), .O(z2));
  inv1   g25(.a(x00), .O(new_n50_));
  nand2  g26(.a(x12), .b(new_n50_), .O(new_n51_));
  nor2   g27(.a(new_n29_), .b(x01), .O(new_n52_));
  and2   g28(.a(x11), .b(x07), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g31(.a(new_n55_), .O(z3));
  nand4  g32(.a(x09), .b(x08), .c(new_n40_), .d(x00), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n53_), .c(new_n46_), .O(z4));
  inv1   g35(.a(x12), .O(new_n60_));
  nand3  g36(.a(x13), .b(new_n60_), .c(new_n41_), .O(new_n61_));
  nor3   g37(.a(new_n61_), .b(x07), .c(x00), .O(new_n62_));
  nor2   g38(.a(new_n62_), .b(new_n52_), .O(z5));
  nand2  g39(.a(x09), .b(x03), .O(new_n64_));
  inv1   g40(.a(x09), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  nand2  g43(.a(x08), .b(x00), .O(new_n68_));
  nand3  g44(.a(new_n28_), .b(x14), .c(x02), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n38_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n67_), .c(new_n46_), .O(z6));
  nand2  g47(.a(new_n64_), .b(new_n29_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n27_), .O(new_n73_));
  nand2  g49(.a(x03), .b(new_n27_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n45_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n73_), .c(new_n38_), .O(z7));
  nand2  g52(.a(new_n65_), .b(x00), .O(new_n77_));
  nor2   g53(.a(new_n77_), .b(new_n52_), .O(new_n78_));
  nand3  g54(.a(new_n60_), .b(x09), .c(new_n29_), .O(new_n79_));
  nor2   g55(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n78_), .c(new_n38_), .O(z8));
endmodule


