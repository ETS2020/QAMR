// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x12), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nor2   g04(.a(new_n25_), .b(x00), .O(new_n29_));
  nand2  g05(.a(x05), .b(x01), .O(new_n30_));
  nor2   g06(.a(x03), .b(x01), .O(new_n31_));
  oai21  g07(.a(new_n31_), .b(x04), .c(new_n30_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n28_), .O(z0));
  inv1   g10(.a(x12), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x03), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  inv1   g15(.a(x04), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n38_), .c(x02), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n37_), .c(x00), .O(z1));
  inv1   g19(.a(x00), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x08), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  inv1   g23(.a(x01), .O(new_n48_));
  oai21  g24(.a(x10), .b(x07), .c(x03), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  nor2   g26(.a(x10), .b(x07), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g28(.a(new_n52_), .b(new_n49_), .c(x12), .d(new_n48_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n25_), .c(new_n47_), .O(z2));
  nor2   g30(.a(x12), .b(x02), .O(new_n55_));
  nand2  g31(.a(x11), .b(x07), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g33(.a(x12), .b(new_n44_), .c(new_n57_), .O(z3));
  nor2   g34(.a(new_n46_), .b(new_n44_), .O(new_n59_));
  nor3   g35(.a(new_n55_), .b(new_n50_), .c(x07), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n57_), .O(z4));
  nand4  g37(.a(new_n51_), .b(new_n29_), .c(x13), .d(new_n35_), .O(z5));
  nand3  g38(.a(x14), .b(x01), .c(new_n44_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x02), .O(new_n64_));
  nand2  g40(.a(new_n50_), .b(x03), .O(new_n65_));
  nand2  g41(.a(x09), .b(new_n39_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(new_n48_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n36_), .O(new_n68_));
  oai21  g44(.a(new_n46_), .b(new_n44_), .c(new_n51_), .O(new_n69_));
  aoi21  g45(.a(new_n68_), .b(new_n64_), .c(new_n69_), .O(z6));
  nand2  g46(.a(new_n51_), .b(new_n48_), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(new_n65_), .c(x12), .O(new_n72_));
  nor2   g48(.a(x02), .b(x01), .O(new_n73_));
  nand2  g49(.a(new_n51_), .b(x08), .O(new_n74_));
  aoi21  g50(.a(new_n73_), .b(x03), .c(new_n74_), .O(new_n75_));
  aoi21  g51(.a(new_n72_), .b(new_n25_), .c(new_n75_), .O(z7));
  nor2   g52(.a(new_n55_), .b(new_n44_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n51_), .c(new_n50_), .O(z8));
endmodule


