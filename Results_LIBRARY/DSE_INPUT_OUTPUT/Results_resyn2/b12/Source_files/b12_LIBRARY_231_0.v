// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x04), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x02), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand3  g03(.a(x12), .b(new_n27_), .c(x01), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  nor2   g07(.a(new_n27_), .b(new_n31_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n25_), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(x00), .O(z0));
  inv1   g10(.a(new_n28_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x03), .O(new_n36_));
  nand3  g12(.a(x06), .b(x05), .c(x01), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n25_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n37_), .c(x02), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n36_), .c(x00), .O(z1));
  inv1   g17(.a(x00), .O(new_n42_));
  nor2   g18(.a(x08), .b(x07), .O(new_n43_));
  nor2   g19(.a(x12), .b(x02), .O(new_n44_));
  nor3   g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  inv1   g21(.a(x09), .O(new_n46_));
  nor2   g22(.a(x10), .b(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x03), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n48_), .c(new_n31_), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x02), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z2));
  nand2  g32(.a(x11), .b(x07), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  aoi21  g34(.a(x12), .b(new_n42_), .c(new_n58_), .O(z3));
  nand2  g35(.a(x08), .b(x00), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nor3   g37(.a(new_n44_), .b(new_n46_), .c(x07), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(z4));
  nor2   g39(.a(new_n27_), .b(x00), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n47_), .c(x13), .d(new_n54_), .O(z5));
  inv1   g41(.a(new_n44_), .O(new_n66_));
  nand2  g42(.a(new_n46_), .b(new_n38_), .O(new_n67_));
  nand2  g43(.a(x09), .b(x03), .O(new_n68_));
  nand2  g44(.a(new_n27_), .b(new_n31_), .O(new_n69_));
  aoi21  g45(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n42_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n60_), .c(new_n47_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z6));
  oai21  g49(.a(new_n68_), .b(x01), .c(x12), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n27_), .O(new_n75_));
  inv1   g51(.a(x08), .O(new_n76_));
  nand3  g52(.a(x03), .b(new_n27_), .c(new_n31_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n76_), .c(new_n51_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n75_), .O(z7));
  nand4  g55(.a(new_n47_), .b(new_n66_), .c(new_n46_), .d(x00), .O(z8));
endmodule


