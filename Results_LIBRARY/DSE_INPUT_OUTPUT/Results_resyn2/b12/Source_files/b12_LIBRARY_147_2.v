// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_;
  inv1   g00(.a(x04), .O(new_n25_));
  inv1   g01(.a(x01), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nor2   g03(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n25_), .c(new_n28_), .O(new_n29_));
  nand2  g05(.a(x04), .b(x02), .O(new_n30_));
  nor2   g06(.a(x02), .b(x01), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(new_n30_), .c(x03), .O(new_n33_));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x09), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x08), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  aoi21  g13(.a(new_n33_), .b(new_n29_), .c(new_n37_), .O(z0));
  inv1   g14(.a(x08), .O(new_n39_));
  nor2   g15(.a(x09), .b(new_n39_), .O(new_n40_));
  nor2   g16(.a(new_n40_), .b(new_n34_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  inv1   g18(.a(x03), .O(new_n43_));
  nand2  g19(.a(new_n25_), .b(new_n43_), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n42_), .c(x02), .O(new_n45_));
  nor2   g21(.a(new_n43_), .b(x02), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(x01), .c(new_n40_), .O(new_n47_));
  aoi21  g23(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(z1));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n35_), .O(new_n50_));
  aoi21  g26(.a(x10), .b(x03), .c(x01), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n50_), .c(x02), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  oai21  g29(.a(new_n35_), .b(x00), .c(x08), .O(new_n54_));
  aoi21  g30(.a(x03), .b(new_n27_), .c(x00), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n53_), .c(new_n54_), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n52_), .O(z2));
  nand2  g33(.a(x12), .b(new_n34_), .O(new_n58_));
  nand2  g34(.a(x11), .b(x07), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n58_), .c(new_n36_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(z3));
  nand2  g37(.a(new_n54_), .b(new_n53_), .O(new_n62_));
  inv1   g38(.a(x11), .O(new_n63_));
  nand3  g39(.a(new_n36_), .b(new_n63_), .c(x07), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n62_), .O(z4));
  inv1   g41(.a(x12), .O(new_n66_));
  nand3  g42(.a(x13), .b(new_n66_), .c(new_n34_), .O(new_n67_));
  inv1   g43(.a(new_n67_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n49_), .c(new_n40_), .O(z5));
  xnor2a g45(.a(x09), .b(x03), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n34_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n54_), .c(new_n49_), .O(new_n72_));
  aoi21  g48(.a(new_n70_), .b(new_n31_), .c(new_n72_), .O(z6));
  nand3  g49(.a(x03), .b(new_n27_), .c(new_n26_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n39_), .O(new_n75_));
  nand3  g51(.a(new_n31_), .b(x09), .c(x03), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n75_), .c(new_n49_), .d(new_n36_), .O(z7));
  nand2  g53(.a(new_n66_), .b(x09), .O(new_n78_));
  nand3  g54(.a(new_n35_), .b(new_n39_), .c(x00), .O(new_n79_));
  oai21  g55(.a(new_n78_), .b(new_n74_), .c(new_n79_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n49_), .O(z8));
endmodule


