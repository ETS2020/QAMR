// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x10), .b(x09), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nor2   g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  oai21  g07(.a(x05), .b(new_n28_), .c(new_n31_), .O(new_n32_));
  inv1   g08(.a(x03), .O(new_n33_));
  nor2   g09(.a(x02), .b(x01), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g11(.a(new_n28_), .b(new_n30_), .c(new_n35_), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n32_), .c(new_n27_), .O(z0));
  aoi21  g13(.a(new_n28_), .b(new_n33_), .c(new_n27_), .O(new_n38_));
  oai21  g14(.a(new_n33_), .b(new_n29_), .c(new_n30_), .O(new_n39_));
  nand3  g15(.a(new_n31_), .b(x06), .c(x05), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(z1));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  nand3  g20(.a(x09), .b(new_n44_), .c(new_n43_), .O(new_n45_));
  nand2  g21(.a(x07), .b(x03), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n46_), .c(new_n25_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n29_), .O(new_n51_));
  inv1   g27(.a(new_n26_), .O(new_n52_));
  oai21  g28(.a(x08), .b(x07), .c(x00), .O(new_n53_));
  nand3  g29(.a(x10), .b(new_n33_), .c(new_n29_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n30_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n51_), .O(z2));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  aoi21  g35(.a(x12), .b(new_n25_), .c(new_n59_), .O(z3));
  inv1   g36(.a(x10), .O(new_n61_));
  nand2  g37(.a(x08), .b(x00), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(x07), .c(new_n61_), .O(new_n63_));
  aoi22  g39(.a(new_n63_), .b(x09), .c(x11), .d(x07), .O(z4));
  nor2   g40(.a(x10), .b(x07), .O(new_n65_));
  nor2   g41(.a(x12), .b(x00), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n65_), .c(x13), .O(z5));
  nand2  g43(.a(new_n34_), .b(new_n33_), .O(new_n68_));
  aoi21  g44(.a(new_n68_), .b(new_n61_), .c(x09), .O(new_n69_));
  nand3  g45(.a(new_n34_), .b(x09), .c(x03), .O(new_n70_));
  nand4  g46(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n70_), .c(new_n62_), .d(new_n43_), .O(new_n72_));
  aoi21  g48(.a(new_n72_), .b(new_n61_), .c(new_n69_), .O(z6));
  nand2  g49(.a(new_n34_), .b(x03), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n44_), .O(new_n75_));
  and2   g51(.a(new_n70_), .b(new_n65_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n75_), .c(new_n52_), .O(z7));
  oai22  g53(.a(new_n70_), .b(x12), .c(x09), .d(new_n25_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n65_), .O(z8));
endmodule


