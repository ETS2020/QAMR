// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(x03), .b(x01), .O(new_n26_));
  oai21  g02(.a(new_n26_), .b(x00), .c(x09), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g04(.a(x05), .b(x01), .O(new_n29_));
  nor2   g05(.a(x03), .b(x01), .O(new_n30_));
  oai21  g06(.a(new_n30_), .b(x04), .c(new_n29_), .O(new_n31_));
  nor2   g07(.a(new_n25_), .b(x00), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n28_), .O(z0));
  nand3  g10(.a(x06), .b(x05), .c(x01), .O(new_n35_));
  inv1   g11(.a(x03), .O(new_n36_));
  inv1   g12(.a(x04), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g14(.a(new_n38_), .b(new_n35_), .c(x02), .O(new_n39_));
  nand4  g15(.a(x09), .b(x03), .c(new_n25_), .d(x01), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n39_), .c(x00), .O(z1));
  inv1   g17(.a(x00), .O(new_n42_));
  inv1   g18(.a(x07), .O(new_n43_));
  inv1   g19(.a(x08), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  inv1   g21(.a(x01), .O(new_n46_));
  oai21  g22(.a(x10), .b(x07), .c(x03), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n25_), .c(new_n45_), .O(z2));
  nor2   g25(.a(x09), .b(x02), .O(new_n50_));
  inv1   g26(.a(new_n50_), .O(new_n51_));
  nand2  g27(.a(x12), .b(new_n42_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  inv1   g31(.a(x09), .O(new_n56_));
  oai21  g32(.a(new_n43_), .b(new_n25_), .c(new_n56_), .O(new_n57_));
  nand2  g33(.a(x08), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x07), .c(new_n53_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n57_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n51_), .d(x13), .O(z5));
  nand2  g39(.a(x03), .b(new_n46_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(x09), .c(new_n25_), .O(new_n65_));
  nand3  g41(.a(x14), .b(x01), .c(new_n42_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x02), .O(new_n67_));
  nor2   g43(.a(x10), .b(x07), .O(new_n68_));
  nand2  g44(.a(new_n58_), .b(new_n68_), .O(new_n69_));
  aoi21  g45(.a(new_n67_), .b(new_n65_), .c(new_n69_), .O(z6));
  nand3  g46(.a(x03), .b(new_n25_), .c(new_n46_), .O(new_n71_));
  nand2  g47(.a(new_n68_), .b(x08), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n71_), .c(new_n50_), .O(z7));
  inv1   g50(.a(new_n71_), .O(new_n75_));
  inv1   g51(.a(x10), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n43_), .c(x00), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(x02), .O(new_n78_));
  nor3   g54(.a(x12), .b(x10), .c(x07), .O(new_n79_));
  aoi22  g55(.a(new_n79_), .b(new_n75_), .c(new_n78_), .d(new_n56_), .O(z8));
endmodule


