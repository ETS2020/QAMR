// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x02), .b(new_n25_), .O(new_n26_));
  nand2  g02(.a(x05), .b(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x03), .b(x01), .c(new_n28_), .O(new_n29_));
  aoi21  g05(.a(new_n29_), .b(new_n27_), .c(new_n26_), .O(z0));
  nand3  g06(.a(x06), .b(x05), .c(x01), .O(new_n31_));
  inv1   g07(.a(x03), .O(new_n32_));
  aoi21  g08(.a(new_n28_), .b(new_n32_), .c(new_n26_), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(z1));
  nor2   g11(.a(x10), .b(x07), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x09), .O(new_n37_));
  nand2  g13(.a(x07), .b(new_n32_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n25_), .O(new_n40_));
  inv1   g16(.a(x08), .O(new_n41_));
  nand3  g17(.a(new_n36_), .b(x09), .c(new_n41_), .O(new_n42_));
  inv1   g18(.a(x01), .O(new_n43_));
  nor2   g19(.a(x02), .b(new_n43_), .O(new_n44_));
  oai21  g20(.a(x08), .b(x07), .c(x00), .O(new_n45_));
  inv1   g21(.a(x02), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(x03), .c(new_n46_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n42_), .c(new_n40_), .O(z2));
  inv1   g26(.a(new_n44_), .O(new_n51_));
  nand2  g27(.a(x12), .b(new_n25_), .O(new_n52_));
  nand2  g28(.a(x11), .b(x07), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(z3));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  inv1   g32(.a(x07), .O(new_n57_));
  nand2  g33(.a(x09), .b(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n51_), .O(z4));
  nor2   g36(.a(x12), .b(x10), .O(new_n61_));
  nor2   g37(.a(x07), .b(x00), .O(new_n62_));
  nand4  g38(.a(new_n62_), .b(new_n61_), .c(new_n51_), .d(x13), .O(z5));
  nand2  g39(.a(x14), .b(new_n25_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(x02), .c(new_n43_), .O(new_n65_));
  xnor2a g41(.a(x09), .b(x03), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n46_), .O(new_n67_));
  nand3  g43(.a(new_n56_), .b(new_n47_), .c(new_n57_), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nor2   g46(.a(new_n70_), .b(new_n65_), .O(z6));
  nand2  g47(.a(x09), .b(x03), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  oai21  g50(.a(new_n32_), .b(x02), .c(new_n41_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n74_), .c(new_n36_), .O(z7));
  inv1   g52(.a(x12), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n47_), .c(new_n57_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n72_), .c(new_n43_), .O(new_n79_));
  inv1   g55(.a(x09), .O(new_n80_));
  nand3  g56(.a(new_n36_), .b(new_n80_), .c(x00), .O(new_n81_));
  inv1   g57(.a(new_n81_), .O(new_n82_));
  aoi21  g58(.a(new_n79_), .b(new_n46_), .c(new_n82_), .O(z8));
endmodule


