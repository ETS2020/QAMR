// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x05), .O(new_n25_));
  nand2  g01(.a(x02), .b(x01), .O(new_n26_));
  aoi21  g02(.a(new_n25_), .b(x04), .c(new_n26_), .O(new_n27_));
  nor2   g03(.a(x02), .b(x01), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x03), .O(new_n30_));
  aoi21  g06(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand2  g08(.a(x14), .b(x09), .O(new_n33_));
  oai21  g09(.a(new_n32_), .b(x00), .c(new_n33_), .O(z0));
  inv1   g10(.a(x00), .O(new_n35_));
  nand4  g11(.a(x06), .b(x05), .c(x02), .d(x01), .O(new_n36_));
  nand2  g12(.a(x04), .b(x02), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  nand4  g14(.a(new_n38_), .b(new_n36_), .c(new_n29_), .d(new_n35_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n33_), .O(z1));
  inv1   g16(.a(x01), .O(new_n41_));
  nor2   g17(.a(x10), .b(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x09), .O(new_n43_));
  nand3  g19(.a(x07), .b(new_n30_), .c(new_n35_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g21(.a(x08), .b(x00), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n45_), .c(new_n41_), .O(new_n47_));
  inv1   g23(.a(new_n33_), .O(new_n48_));
  oai21  g24(.a(x08), .b(x07), .c(x00), .O(new_n49_));
  inv1   g25(.a(x02), .O(new_n50_));
  nand3  g26(.a(x10), .b(new_n30_), .c(new_n41_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n49_), .c(new_n48_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n47_), .O(z2));
  nand2  g30(.a(x11), .b(x07), .O(new_n55_));
  nand2  g31(.a(new_n55_), .b(new_n33_), .O(new_n56_));
  aoi21  g32(.a(x12), .b(new_n35_), .c(new_n56_), .O(z3));
  inv1   g33(.a(x07), .O(new_n58_));
  oai21  g34(.a(x09), .b(new_n58_), .c(x14), .O(new_n59_));
  nand2  g35(.a(x09), .b(new_n58_), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n46_), .c(new_n55_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n59_), .O(z4));
  nor2   g38(.a(x12), .b(x10), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(x13), .c(new_n58_), .d(new_n35_), .O(new_n64_));
  and2   g40(.a(new_n64_), .b(new_n33_), .O(z5));
  xnor2a g41(.a(x09), .b(x03), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n28_), .O(new_n67_));
  nor2   g43(.a(x09), .b(x00), .O(new_n68_));
  nand4  g44(.a(new_n68_), .b(x14), .c(x02), .d(x01), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n67_), .c(new_n46_), .d(new_n42_), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n33_), .O(z6));
  nand3  g47(.a(new_n28_), .b(x09), .c(x03), .O(new_n72_));
  inv1   g48(.a(x08), .O(new_n73_));
  inv1   g49(.a(new_n42_), .O(new_n74_));
  nand2  g50(.a(new_n28_), .b(x03), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n73_), .c(new_n74_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n72_), .c(new_n48_), .O(z7));
  oai21  g53(.a(new_n75_), .b(x12), .c(x09), .O(new_n78_));
  nor2   g54(.a(new_n68_), .b(new_n74_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n78_), .c(new_n48_), .O(z8));
endmodule


