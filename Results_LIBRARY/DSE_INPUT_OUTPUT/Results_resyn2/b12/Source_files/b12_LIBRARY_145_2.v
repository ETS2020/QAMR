// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_;
  inv1   g00(.a(x01), .O(new_n25_));
  nand2  g01(.a(x13), .b(x02), .O(new_n26_));
  oai22  g02(.a(new_n26_), .b(x04), .c(x02), .d(new_n25_), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x03), .O(new_n28_));
  inv1   g04(.a(new_n26_), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n29_), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n28_), .c(x00), .O(z0));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  nand3  g11(.a(x06), .b(x05), .c(x01), .O(new_n36_));
  inv1   g12(.a(x03), .O(new_n37_));
  inv1   g13(.a(x04), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n36_), .c(new_n29_), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n35_), .c(x00), .O(z1));
  inv1   g17(.a(x09), .O(new_n42_));
  nor2   g18(.a(x10), .b(x07), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x03), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n44_), .c(new_n25_), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  inv1   g27(.a(x00), .O(new_n52_));
  nor2   g28(.a(x13), .b(new_n34_), .O(new_n53_));
  nor2   g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(new_n51_), .c(new_n49_), .d(new_n34_), .O(z2));
  inv1   g31(.a(new_n53_), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nand2  g33(.a(x11), .b(x07), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n56_), .O(z3));
  nor2   g36(.a(new_n50_), .b(new_n52_), .O(new_n61_));
  nor2   g37(.a(new_n42_), .b(x07), .O(new_n62_));
  nand2  g38(.a(new_n58_), .b(new_n56_), .O(new_n63_));
  aoi21  g39(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(z4));
  nand3  g40(.a(x13), .b(new_n57_), .c(new_n46_), .O(new_n65_));
  nor3   g41(.a(new_n65_), .b(x07), .c(x00), .O(new_n66_));
  nor2   g42(.a(new_n66_), .b(new_n53_), .O(z5));
  nand3  g43(.a(x14), .b(x01), .c(new_n52_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n29_), .O(new_n69_));
  nand2  g45(.a(x09), .b(new_n37_), .O(new_n70_));
  nand2  g46(.a(new_n42_), .b(x03), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n25_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n34_), .O(new_n73_));
  oai21  g49(.a(new_n50_), .b(new_n52_), .c(new_n43_), .O(new_n74_));
  aoi21  g50(.a(new_n73_), .b(new_n69_), .c(new_n74_), .O(z6));
  aoi21  g51(.a(x03), .b(new_n25_), .c(x08), .O(new_n76_));
  nand3  g52(.a(x09), .b(x03), .c(new_n25_), .O(new_n77_));
  inv1   g53(.a(new_n77_), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(new_n76_), .c(new_n34_), .O(new_n79_));
  aoi21  g55(.a(new_n50_), .b(x02), .c(new_n47_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n79_), .c(new_n53_), .O(z7));
  nand2  g57(.a(new_n57_), .b(new_n34_), .O(new_n82_));
  oai22  g58(.a(new_n82_), .b(new_n77_), .c(x09), .d(new_n52_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n43_), .c(new_n53_), .O(z8));
endmodule


