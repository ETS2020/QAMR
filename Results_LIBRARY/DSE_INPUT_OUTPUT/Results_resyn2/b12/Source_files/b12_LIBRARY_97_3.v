// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x00), .O(new_n25_));
  nand2  g01(.a(x09), .b(x07), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g03(.a(x04), .b(x02), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n28_), .c(x03), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x04), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(x02), .c(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n32_), .c(new_n27_), .O(z0));
  nand3  g12(.a(x03), .b(new_n30_), .c(x01), .O(new_n37_));
  nand3  g13(.a(x06), .b(x05), .c(x01), .O(new_n38_));
  inv1   g14(.a(new_n38_), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(x02), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n26_), .O(z1));
  inv1   g19(.a(x07), .O(new_n44_));
  inv1   g20(.a(x08), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(x10), .b(x07), .c(x03), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(new_n29_), .O(new_n51_));
  aoi22  g27(.a(new_n51_), .b(new_n30_), .c(new_n46_), .d(new_n27_), .O(z2));
  inv1   g28(.a(x11), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n47_), .c(new_n44_), .O(new_n54_));
  aoi21  g30(.a(x12), .b(new_n25_), .c(new_n54_), .O(z3));
  nand2  g31(.a(x08), .b(x00), .O(new_n56_));
  inv1   g32(.a(new_n56_), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(x09), .c(new_n54_), .O(z4));
  nor2   g34(.a(x12), .b(x00), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n48_), .c(x13), .O(z5));
  inv1   g36(.a(x03), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n30_), .c(new_n29_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(new_n44_), .c(x09), .O(new_n63_));
  inv1   g39(.a(x10), .O(new_n64_));
  nand4  g40(.a(x09), .b(x03), .c(new_n30_), .d(new_n29_), .O(new_n65_));
  nand4  g41(.a(x14), .b(x02), .c(x01), .d(new_n25_), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n65_), .c(new_n56_), .d(new_n64_), .O(new_n67_));
  aoi21  g43(.a(new_n67_), .b(new_n44_), .c(new_n63_), .O(z6));
  nand3  g44(.a(x03), .b(new_n30_), .c(new_n29_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n45_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n65_), .c(new_n48_), .O(z7));
  inv1   g47(.a(new_n26_), .O(new_n72_));
  nand3  g48(.a(new_n47_), .b(new_n44_), .c(x00), .O(new_n73_));
  oai21  g49(.a(new_n65_), .b(x12), .c(new_n73_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n64_), .c(new_n72_), .O(z8));
endmodule


