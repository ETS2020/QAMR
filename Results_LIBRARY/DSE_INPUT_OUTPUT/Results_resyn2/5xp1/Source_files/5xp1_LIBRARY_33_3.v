// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_, new_n89_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  nand2  g05(.a(x6), .b(x5), .O(new_n23_));
  oai21  g06(.a(new_n22_), .b(new_n18_), .c(new_n23_), .O(z0));
  inv1   g07(.a(x2), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand3  g09(.a(new_n20_), .b(x4), .c(x1), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(new_n28_));
  inv1   g11(.a(x1), .O(new_n29_));
  nand3  g12(.a(new_n20_), .b(x4), .c(x3), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n26_), .c(new_n29_), .O(new_n31_));
  oai21  g14(.a(new_n31_), .b(new_n28_), .c(x0), .O(new_n32_));
  nand2  g15(.a(new_n21_), .b(new_n19_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x4), .O(new_n34_));
  aoi22  g17(.a(new_n34_), .b(x5), .c(x6), .d(x4), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n32_), .O(z1));
  nand2  g19(.a(new_n33_), .b(x5), .O(new_n37_));
  inv1   g20(.a(x3), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n25_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x1), .c(x0), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n37_), .c(new_n18_), .O(new_n41_));
  oai21  g24(.a(x3), .b(x2), .c(x0), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n29_), .c(new_n18_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n22_), .c(new_n26_), .O(new_n44_));
  oai21  g27(.a(x2), .b(x1), .c(x0), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x4), .O(new_n46_));
  nand2  g29(.a(new_n25_), .b(new_n29_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n18_), .c(x0), .O(new_n48_));
  nand4  g31(.a(new_n48_), .b(new_n46_), .c(x6), .d(new_n20_), .O(new_n49_));
  oai21  g32(.a(new_n44_), .b(new_n41_), .c(new_n49_), .O(z2));
  nand3  g33(.a(new_n20_), .b(x3), .c(x2), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n26_), .c(x0), .O(new_n52_));
  nor2   g35(.a(new_n42_), .b(new_n20_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n52_), .c(x1), .O(new_n54_));
  nand2  g37(.a(x5), .b(new_n19_), .O(new_n55_));
  nand3  g38(.a(new_n26_), .b(new_n20_), .c(x0), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g40(.a(new_n55_), .b(x2), .c(x3), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n29_), .c(new_n57_), .O(new_n59_));
  nand2  g42(.a(x3), .b(x2), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(x0), .c(new_n20_), .O(new_n61_));
  nand3  g44(.a(new_n20_), .b(new_n29_), .c(x0), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  aoi22  g46(.a(new_n63_), .b(new_n25_), .c(new_n61_), .d(x6), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n59_), .c(new_n54_), .O(z3));
  nand2  g48(.a(x6), .b(new_n20_), .O(new_n66_));
  nand2  g49(.a(new_n42_), .b(new_n60_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g51(.a(new_n42_), .b(new_n60_), .c(x6), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(x1), .O(new_n70_));
  oai21  g53(.a(x3), .b(x0), .c(x2), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  or2    g55(.a(new_n71_), .b(new_n26_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(new_n29_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n70_), .O(z4));
  nand2  g58(.a(new_n38_), .b(x2), .O(new_n76_));
  nand3  g59(.a(x3), .b(new_n25_), .c(x1), .O(new_n77_));
  nand3  g60(.a(new_n77_), .b(new_n76_), .c(new_n19_), .O(new_n78_));
  nand2  g61(.a(new_n77_), .b(new_n76_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x0), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n78_), .c(new_n23_), .O(new_n81_));
  inv1   g64(.a(new_n81_), .O(z5));
  nand2  g65(.a(x3), .b(new_n25_), .O(new_n83_));
  nand2  g66(.a(new_n77_), .b(new_n23_), .O(new_n84_));
  aoi21  g67(.a(new_n83_), .b(new_n29_), .c(new_n84_), .O(z6));
  inv1   g68(.a(new_n23_), .O(new_n86_));
  aoi21  g69(.a(new_n83_), .b(new_n76_), .c(new_n86_), .O(z7));
  nand2  g70(.a(new_n23_), .b(x3), .O(z8));
  nand4  g71(.a(new_n33_), .b(new_n26_), .c(x5), .d(x4), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z9));
endmodule


