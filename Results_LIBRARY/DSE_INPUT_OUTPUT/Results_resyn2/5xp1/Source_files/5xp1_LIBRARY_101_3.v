// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n105_, new_n106_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand3  g01(.a(x5), .b(x3), .c(x1), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x2), .O(new_n21_));
  inv1   g04(.a(x4), .O(new_n22_));
  inv1   g05(.a(x0), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n23_), .c(new_n18_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n22_), .O(new_n27_));
  nand2  g10(.a(new_n18_), .b(new_n23_), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(x5), .c(x4), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n27_), .c(new_n21_), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(z0));
  inv1   g14(.a(x5), .O(new_n32_));
  aoi21  g15(.a(x4), .b(x3), .c(x6), .O(new_n33_));
  nand2  g16(.a(x1), .b(x0), .O(new_n34_));
  oai22  g17(.a(new_n34_), .b(new_n33_), .c(new_n18_), .d(new_n22_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  nand3  g19(.a(x3), .b(x2), .c(x1), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  oai22  g21(.a(new_n38_), .b(new_n28_), .c(new_n25_), .d(x4), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x5), .O(new_n40_));
  nand2  g23(.a(new_n32_), .b(x0), .O(new_n41_));
  nand2  g24(.a(x4), .b(x1), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n41_), .c(new_n18_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x2), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n40_), .c(new_n36_), .O(z1));
  oai21  g28(.a(x5), .b(x1), .c(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  inv1   g30(.a(x2), .O(new_n48_));
  nor2   g31(.a(x5), .b(x3), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n23_), .c(new_n48_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n47_), .c(x4), .O(new_n51_));
  oai21  g34(.a(new_n38_), .b(x0), .c(x5), .O(new_n52_));
  oai21  g35(.a(x3), .b(x2), .c(x1), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x0), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n52_), .c(new_n22_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n51_), .c(new_n18_), .O(new_n57_));
  nand2  g40(.a(x5), .b(x1), .O(new_n58_));
  nand3  g41(.a(new_n46_), .b(new_n58_), .c(x4), .O(new_n59_));
  nor2   g42(.a(new_n18_), .b(x2), .O(new_n60_));
  nand2  g43(.a(new_n46_), .b(new_n58_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n22_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n57_), .O(z2));
  inv1   g47(.a(x3), .O(new_n65_));
  nand2  g48(.a(new_n32_), .b(new_n65_), .O(new_n66_));
  and2   g49(.a(x2), .b(x1), .O(new_n67_));
  nand2  g50(.a(x5), .b(new_n23_), .O(new_n68_));
  nand4  g51(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n41_), .O(new_n69_));
  nand3  g52(.a(new_n37_), .b(x5), .c(new_n23_), .O(new_n70_));
  nand3  g53(.a(new_n53_), .b(new_n32_), .c(x0), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n18_), .O(new_n73_));
  nand2  g56(.a(new_n32_), .b(new_n24_), .O(new_n74_));
  nor2   g57(.a(x6), .b(x3), .O(new_n75_));
  oai21  g58(.a(new_n75_), .b(new_n58_), .c(new_n74_), .O(new_n76_));
  aoi21  g59(.a(x6), .b(x1), .c(x5), .O(new_n77_));
  nand2  g60(.a(new_n58_), .b(new_n23_), .O(new_n78_));
  nor2   g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g62(.a(new_n76_), .b(x0), .c(new_n79_), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(x2), .c(new_n73_), .O(z3));
  oai21  g64(.a(x3), .b(x2), .c(x0), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x1), .O(new_n83_));
  oai21  g66(.a(new_n65_), .b(new_n48_), .c(new_n83_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n37_), .c(new_n18_), .O(new_n85_));
  nand3  g68(.a(new_n18_), .b(x2), .c(new_n24_), .O(new_n86_));
  nand2  g69(.a(x3), .b(new_n48_), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n18_), .c(new_n86_), .O(new_n88_));
  aoi22  g71(.a(new_n88_), .b(x0), .c(new_n60_), .d(new_n24_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n85_), .O(z4));
  inv1   g73(.a(new_n87_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(x1), .O(new_n92_));
  nand2  g75(.a(new_n75_), .b(x2), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n23_), .O(new_n95_));
  oai21  g78(.a(x6), .b(new_n65_), .c(x2), .O(new_n96_));
  nand3  g79(.a(new_n96_), .b(new_n92_), .c(x0), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n95_), .O(z5));
  inv1   g81(.a(new_n92_), .O(new_n99_));
  nand2  g82(.a(x6), .b(x2), .O(new_n100_));
  nor2   g83(.a(new_n91_), .b(x1), .O(new_n101_));
  aoi21  g84(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(z6));
  nand2  g85(.a(new_n93_), .b(new_n87_), .O(z7));
  aoi21  g86(.a(x6), .b(x2), .c(x3), .O(z8));
  oai21  g87(.a(new_n19_), .b(new_n22_), .c(new_n18_), .O(new_n105_));
  nand2  g88(.a(new_n105_), .b(x2), .O(new_n106_));
  nand2  g89(.a(new_n106_), .b(new_n29_), .O(z9));
endmodule


