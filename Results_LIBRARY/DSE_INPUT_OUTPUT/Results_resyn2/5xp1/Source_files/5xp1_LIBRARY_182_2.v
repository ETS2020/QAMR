// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x4), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand2  g08(.a(new_n19_), .b(x4), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n25_), .c(new_n18_), .O(z0));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(new_n21_), .O(new_n29_));
  nor2   g12(.a(new_n18_), .b(x5), .O(new_n30_));
  oai21  g13(.a(new_n29_), .b(x4), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n24_), .b(x6), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n20_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n31_), .O(z1));
  oai21  g17(.a(x3), .b(x2), .c(x0), .O(new_n35_));
  nand3  g18(.a(x5), .b(x3), .c(x2), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n35_), .c(x6), .O(new_n37_));
  inv1   g20(.a(x4), .O(new_n38_));
  nor2   g21(.a(x5), .b(x0), .O(new_n39_));
  nor2   g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n37_), .c(x1), .O(new_n41_));
  nand2  g24(.a(x5), .b(x0), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(x6), .c(new_n38_), .O(new_n43_));
  aoi21  g26(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(new_n44_));
  inv1   g27(.a(x2), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n21_), .c(x6), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  oai21  g30(.a(new_n29_), .b(x5), .c(new_n24_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(x6), .c(new_n38_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n47_), .c(new_n41_), .O(z2));
  inv1   g33(.a(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(x6), .c(x2), .O(new_n53_));
  nand3  g36(.a(new_n35_), .b(new_n23_), .c(new_n18_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x1), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  inv1   g39(.a(new_n42_), .O(new_n57_));
  nor2   g40(.a(new_n57_), .b(new_n39_), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nor2   g43(.a(x3), .b(x2), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(x0), .c(x5), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n44_), .c(new_n38_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n18_), .O(new_n64_));
  nand3  g47(.a(new_n58_), .b(new_n53_), .c(new_n22_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n64_), .c(new_n60_), .O(z3));
  aoi21  g49(.a(new_n35_), .b(new_n23_), .c(new_n18_), .O(new_n67_));
  nand4  g50(.a(new_n35_), .b(new_n23_), .c(new_n18_), .d(new_n38_), .O(new_n68_));
  inv1   g51(.a(new_n68_), .O(new_n69_));
  oai21  g52(.a(new_n69_), .b(new_n67_), .c(x1), .O(new_n70_));
  nand2  g53(.a(new_n52_), .b(x2), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n18_), .O(new_n72_));
  nor2   g55(.a(x6), .b(new_n38_), .O(new_n73_));
  inv1   g56(.a(new_n73_), .O(new_n74_));
  nand4  g57(.a(new_n74_), .b(new_n72_), .c(new_n53_), .d(new_n22_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n70_), .O(z4));
  nand2  g59(.a(new_n51_), .b(new_n45_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n23_), .O(new_n78_));
  oai21  g61(.a(new_n78_), .b(new_n28_), .c(x0), .O(new_n79_));
  nor2   g62(.a(new_n78_), .b(new_n28_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n21_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n79_), .c(new_n74_), .O(z5));
  nand2  g65(.a(x3), .b(new_n45_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n22_), .c(new_n74_), .O(new_n84_));
  aoi21  g67(.a(new_n83_), .b(new_n22_), .c(new_n84_), .O(z6));
  nor2   g68(.a(new_n78_), .b(new_n73_), .O(z7));
  nor2   g69(.a(new_n73_), .b(x3), .O(z8));
  nor2   g70(.a(new_n30_), .b(new_n38_), .O(z9));
endmodule


