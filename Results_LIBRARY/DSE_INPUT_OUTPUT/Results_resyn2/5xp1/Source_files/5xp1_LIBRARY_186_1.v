// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n85_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n18_), .O(new_n23_));
  inv1   g06(.a(x5), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  nand3  g08(.a(x3), .b(x2), .c(x1), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n19_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(new_n24_), .O(new_n29_));
  nor2   g12(.a(new_n25_), .b(x5), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  oai21  g14(.a(x6), .b(x4), .c(new_n31_), .O(new_n32_));
  aoi21  g15(.a(new_n29_), .b(new_n23_), .c(new_n32_), .O(z0));
  nand2  g16(.a(new_n22_), .b(x6), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x5), .c(new_n18_), .O(new_n35_));
  oai21  g18(.a(x3), .b(x2), .c(x1), .O(new_n36_));
  nand2  g19(.a(new_n24_), .b(x0), .O(new_n37_));
  nand3  g20(.a(new_n26_), .b(x5), .c(new_n19_), .O(new_n38_));
  oai21  g21(.a(new_n37_), .b(new_n36_), .c(new_n38_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n25_), .c(x4), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n35_), .O(z1));
  nand2  g24(.a(x5), .b(x0), .O(new_n42_));
  nand2  g25(.a(new_n21_), .b(new_n19_), .O(new_n43_));
  inv1   g26(.a(x2), .O(new_n44_));
  inv1   g27(.a(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g29(.a(new_n24_), .b(new_n19_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n46_), .c(new_n43_), .d(x1), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n42_), .c(new_n18_), .O(new_n49_));
  nand3  g32(.a(new_n48_), .b(new_n42_), .c(new_n18_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nand2  g34(.a(x6), .b(x5), .O(new_n52_));
  aoi21  g35(.a(new_n22_), .b(new_n18_), .c(new_n52_), .O(new_n53_));
  oai21  g36(.a(new_n22_), .b(new_n18_), .c(new_n53_), .O(new_n54_));
  oai21  g37(.a(new_n51_), .b(new_n49_), .c(new_n54_), .O(z2));
  inv1   g38(.a(new_n22_), .O(new_n56_));
  oai21  g39(.a(x2), .b(x1), .c(x6), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n36_), .c(new_n19_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n56_), .c(x5), .O(new_n59_));
  nand3  g42(.a(new_n36_), .b(new_n24_), .c(x0), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n38_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n25_), .O(new_n62_));
  inv1   g45(.a(new_n26_), .O(new_n63_));
  nand4  g46(.a(new_n63_), .b(new_n25_), .c(new_n24_), .d(new_n19_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n62_), .c(new_n59_), .O(z3));
  nand2  g48(.a(new_n46_), .b(x0), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n21_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand3  g51(.a(new_n66_), .b(new_n21_), .c(x6), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(x1), .O(new_n70_));
  oai21  g53(.a(x3), .b(x0), .c(x2), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n52_), .O(new_n72_));
  or2    g55(.a(new_n71_), .b(new_n25_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(new_n20_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n70_), .O(z4));
  nand2  g58(.a(new_n45_), .b(x2), .O(new_n76_));
  nand3  g59(.a(x3), .b(new_n44_), .c(x1), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g61(.a(new_n78_), .b(x0), .c(new_n30_), .O(new_n79_));
  oai21  g62(.a(new_n78_), .b(x0), .c(new_n79_), .O(new_n80_));
  inv1   g63(.a(new_n80_), .O(z5));
  inv1   g64(.a(new_n77_), .O(new_n82_));
  aoi21  g65(.a(x3), .b(new_n44_), .c(x1), .O(new_n83_));
  nor3   g66(.a(new_n83_), .b(new_n82_), .c(new_n30_), .O(z6));
  nand3  g67(.a(new_n46_), .b(new_n31_), .c(new_n21_), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z7));
  nor2   g69(.a(new_n30_), .b(x3), .O(z8));
  nor3   g70(.a(new_n28_), .b(new_n24_), .c(new_n18_), .O(z9));
endmodule


