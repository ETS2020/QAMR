// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n103_;
  inv1   g00(.a(x4), .O(new_n18_));
  nor2   g01(.a(x6), .b(x0), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x4), .O(new_n20_));
  aoi21  g03(.a(x3), .b(x2), .c(x0), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand3  g05(.a(x6), .b(new_n18_), .c(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(new_n24_));
  nand3  g07(.a(x3), .b(x2), .c(x1), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(x5), .O(new_n26_));
  oai21  g09(.a(x5), .b(new_n18_), .c(new_n26_), .O(z0));
  inv1   g10(.a(x5), .O(new_n28_));
  nand2  g11(.a(x6), .b(x4), .O(new_n29_));
  nand2  g12(.a(x6), .b(x2), .O(new_n30_));
  nor2   g13(.a(x3), .b(x2), .O(new_n31_));
  nand2  g14(.a(x4), .b(x1), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x0), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n29_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n28_), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n18_), .O(new_n38_));
  inv1   g21(.a(x0), .O(new_n39_));
  nand2  g22(.a(x3), .b(x2), .O(new_n40_));
  nor2   g23(.a(x6), .b(x1), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  aoi21  g26(.a(x6), .b(x4), .c(new_n28_), .O(new_n44_));
  nor2   g27(.a(x4), .b(new_n22_), .O(new_n45_));
  aoi21  g28(.a(new_n44_), .b(new_n43_), .c(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n36_), .O(z1));
  inv1   g30(.a(x2), .O(new_n48_));
  oai21  g31(.a(x5), .b(x3), .c(x0), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g33(.a(x5), .b(x0), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  nand2  g35(.a(x5), .b(x3), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n39_), .c(x6), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  aoi22  g39(.a(new_n53_), .b(new_n39_), .c(new_n51_), .d(new_n48_), .O(new_n57_));
  nor2   g40(.a(x5), .b(x0), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n22_), .c(x6), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n57_), .c(x4), .O(new_n60_));
  nand2  g43(.a(new_n57_), .b(x6), .O(new_n61_));
  nand2  g44(.a(new_n51_), .b(new_n37_), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n61_), .c(new_n18_), .d(new_n22_), .O(new_n63_));
  oai21  g46(.a(new_n60_), .b(new_n56_), .c(new_n63_), .O(z2));
  xor2a  g47(.a(x5), .b(x0), .O(new_n65_));
  nor2   g48(.a(new_n65_), .b(new_n37_), .O(new_n66_));
  oai21  g49(.a(new_n53_), .b(new_n39_), .c(x4), .O(new_n67_));
  oai21  g50(.a(new_n67_), .b(new_n66_), .c(x1), .O(new_n68_));
  inv1   g51(.a(x3), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n39_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(x6), .c(x2), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n65_), .c(new_n22_), .O(new_n72_));
  nand3  g55(.a(new_n40_), .b(x5), .c(new_n39_), .O(new_n73_));
  nand4  g56(.a(new_n28_), .b(new_n69_), .c(new_n48_), .d(x0), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n37_), .O(new_n76_));
  inv1   g59(.a(new_n51_), .O(new_n77_));
  oai21  g60(.a(x3), .b(x0), .c(x2), .O(new_n78_));
  nor2   g61(.a(new_n78_), .b(new_n41_), .O(new_n79_));
  oai21  g62(.a(new_n58_), .b(new_n77_), .c(new_n79_), .O(new_n80_));
  nand4  g63(.a(new_n80_), .b(new_n76_), .c(new_n72_), .d(new_n68_), .O(z3));
  nand2  g64(.a(new_n78_), .b(new_n37_), .O(new_n82_));
  aoi21  g65(.a(new_n82_), .b(new_n71_), .c(x1), .O(new_n83_));
  inv1   g66(.a(new_n21_), .O(new_n84_));
  inv1   g67(.a(new_n31_), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n84_), .c(x6), .O(new_n86_));
  nand2  g69(.a(new_n85_), .b(new_n84_), .O(new_n87_));
  aoi21  g70(.a(new_n87_), .b(new_n37_), .c(new_n32_), .O(new_n88_));
  aoi21  g71(.a(new_n88_), .b(new_n86_), .c(new_n83_), .O(z4));
  inv1   g72(.a(new_n45_), .O(new_n90_));
  nor2   g73(.a(x2), .b(x1), .O(new_n91_));
  inv1   g74(.a(new_n91_), .O(new_n92_));
  nand4  g75(.a(new_n92_), .b(new_n85_), .c(new_n40_), .d(x0), .O(new_n93_));
  nand2  g76(.a(new_n85_), .b(new_n40_), .O(new_n94_));
  oai21  g77(.a(new_n91_), .b(new_n94_), .c(new_n39_), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(new_n93_), .c(new_n90_), .O(new_n96_));
  inv1   g79(.a(new_n96_), .O(z5));
  nor2   g80(.a(new_n69_), .b(x2), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n32_), .O(new_n99_));
  oai21  g82(.a(new_n98_), .b(new_n22_), .c(new_n99_), .O(z6));
  nand2  g83(.a(new_n94_), .b(new_n90_), .O(z7));
  nor2   g84(.a(new_n45_), .b(x3), .O(z8));
  nand2  g85(.a(x5), .b(x4), .O(new_n103_));
  aoi21  g86(.a(new_n25_), .b(new_n19_), .c(new_n103_), .O(z9));
endmodule


