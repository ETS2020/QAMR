// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n84_, new_n85_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x4), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n18_), .c(x6), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  aoi21  g05(.a(x3), .b(x2), .c(x6), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n18_), .c(new_n22_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x5), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x4), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n21_), .O(z0));
  nor2   g10(.a(x3), .b(x2), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(new_n18_), .O(new_n29_));
  inv1   g12(.a(x6), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n19_), .O(new_n31_));
  aoi21  g14(.a(new_n29_), .b(x0), .c(new_n31_), .O(new_n32_));
  nand2  g15(.a(new_n19_), .b(x0), .O(new_n33_));
  aoi21  g16(.a(new_n33_), .b(x1), .c(new_n30_), .O(new_n34_));
  aoi21  g17(.a(new_n20_), .b(new_n30_), .c(new_n34_), .O(new_n35_));
  oai21  g18(.a(new_n32_), .b(new_n26_), .c(new_n35_), .O(z1));
  nand2  g19(.a(x5), .b(x0), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  oai21  g21(.a(x3), .b(x2), .c(x0), .O(new_n39_));
  nand3  g22(.a(x5), .b(x3), .c(x2), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n39_), .c(new_n18_), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n38_), .c(x4), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand2  g26(.a(new_n40_), .b(new_n39_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x1), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n37_), .c(new_n43_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n42_), .c(new_n30_), .O(new_n47_));
  nand2  g30(.a(new_n19_), .b(new_n22_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g32(.a(new_n19_), .b(x4), .c(new_n22_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(new_n49_), .c(x6), .d(x1), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n47_), .O(z2));
  nand2  g35(.a(x3), .b(x2), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n18_), .c(new_n22_), .O(new_n54_));
  oai22  g37(.a(new_n54_), .b(new_n19_), .c(new_n33_), .d(new_n29_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nor2   g39(.a(new_n48_), .b(new_n23_), .O(new_n57_));
  aoi21  g40(.a(new_n28_), .b(new_n30_), .c(new_n37_), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n57_), .c(x1), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n56_), .O(z3));
  aoi21  g43(.a(new_n39_), .b(new_n53_), .c(x6), .O(new_n61_));
  nand3  g44(.a(new_n39_), .b(new_n53_), .c(x6), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x1), .O(new_n63_));
  inv1   g46(.a(x3), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n22_), .O(new_n65_));
  nor2   g48(.a(x6), .b(x1), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n65_), .c(x2), .O(new_n67_));
  oai21  g50(.a(new_n63_), .b(new_n61_), .c(new_n67_), .O(z4));
  nand2  g51(.a(new_n64_), .b(x2), .O(new_n69_));
  inv1   g52(.a(x2), .O(new_n70_));
  nand3  g53(.a(x3), .b(new_n70_), .c(x1), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n69_), .c(new_n22_), .O(new_n72_));
  nor2   g55(.a(new_n30_), .b(x1), .O(new_n73_));
  inv1   g56(.a(new_n73_), .O(new_n74_));
  nand2  g57(.a(new_n71_), .b(new_n69_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x0), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  inv1   g60(.a(new_n77_), .O(z5));
  nand2  g61(.a(x3), .b(new_n70_), .O(new_n79_));
  inv1   g62(.a(new_n71_), .O(new_n80_));
  aoi21  g63(.a(new_n79_), .b(new_n66_), .c(new_n80_), .O(z6));
  aoi21  g64(.a(new_n79_), .b(new_n69_), .c(new_n73_), .O(z7));
  nor2   g65(.a(new_n73_), .b(x3), .O(z8));
  nor2   g66(.a(new_n19_), .b(new_n43_), .O(new_n84_));
  oai21  g67(.a(new_n54_), .b(x6), .c(new_n84_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n74_), .O(z9));
endmodule


