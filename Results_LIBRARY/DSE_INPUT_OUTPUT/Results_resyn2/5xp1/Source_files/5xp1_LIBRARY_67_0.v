// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n92_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n18_), .c(x4), .O(new_n24_));
  nand2  g07(.a(new_n20_), .b(new_n18_), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  inv1   g09(.a(x1), .O(new_n27_));
  aoi21  g10(.a(x3), .b(x2), .c(x0), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(x6), .c(x5), .d(new_n26_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n25_), .c(new_n24_), .O(z0));
  nand2  g14(.a(new_n21_), .b(new_n19_), .O(new_n32_));
  nand2  g15(.a(x3), .b(x2), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(x5), .c(new_n26_), .d(new_n27_), .O(new_n34_));
  and2   g17(.a(new_n34_), .b(x6), .O(new_n35_));
  oai21  g18(.a(x2), .b(x1), .c(x0), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n26_), .O(new_n37_));
  aoi22  g20(.a(new_n37_), .b(new_n18_), .c(new_n20_), .d(new_n26_), .O(new_n38_));
  oai21  g21(.a(new_n35_), .b(new_n32_), .c(new_n38_), .O(z1));
  inv1   g22(.a(x3), .O(new_n40_));
  nand3  g23(.a(x6), .b(x5), .c(x4), .O(new_n41_));
  nand3  g24(.a(new_n20_), .b(new_n26_), .c(x1), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nand3  g26(.a(x6), .b(x4), .c(x0), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(x2), .O(new_n46_));
  aoi21  g29(.a(x1), .b(x0), .c(x5), .O(new_n47_));
  oai21  g30(.a(x1), .b(x0), .c(x6), .O(new_n48_));
  nor2   g31(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n23_), .c(x4), .O(new_n50_));
  nand2  g33(.a(new_n33_), .b(new_n27_), .O(new_n51_));
  nand2  g34(.a(x6), .b(new_n19_), .O(new_n52_));
  inv1   g35(.a(x2), .O(new_n53_));
  nand2  g36(.a(new_n18_), .b(new_n53_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  and2   g38(.a(x6), .b(x0), .O(new_n56_));
  aoi21  g39(.a(x5), .b(new_n19_), .c(new_n56_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(new_n26_), .O(new_n58_));
  nand4  g41(.a(new_n58_), .b(new_n50_), .c(new_n46_), .d(new_n25_), .O(z2));
  aoi21  g42(.a(new_n51_), .b(x6), .c(new_n32_), .O(new_n60_));
  aoi21  g43(.a(x2), .b(x1), .c(x6), .O(new_n61_));
  nor2   g44(.a(new_n61_), .b(new_n36_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n60_), .c(x5), .O(new_n63_));
  nand3  g46(.a(x5), .b(x1), .c(x0), .O(new_n64_));
  nand2  g47(.a(new_n18_), .b(x2), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n52_), .c(new_n64_), .O(new_n66_));
  nand2  g49(.a(x6), .b(new_n18_), .O(new_n67_));
  aoi21  g50(.a(new_n27_), .b(new_n19_), .c(new_n67_), .O(new_n68_));
  aoi22  g51(.a(new_n68_), .b(new_n36_), .c(new_n66_), .d(x3), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n63_), .O(z3));
  nor2   g53(.a(x3), .b(x2), .O(new_n71_));
  nor2   g54(.a(new_n71_), .b(new_n28_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n20_), .O(new_n73_));
  oai21  g56(.a(new_n71_), .b(new_n28_), .c(x6), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n73_), .c(x1), .O(new_n75_));
  oai21  g58(.a(x3), .b(x0), .c(x2), .O(new_n76_));
  aoi21  g59(.a(new_n76_), .b(new_n20_), .c(x1), .O(new_n77_));
  oai21  g60(.a(new_n76_), .b(new_n20_), .c(new_n77_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n75_), .c(new_n25_), .O(z4));
  nand2  g62(.a(new_n40_), .b(x2), .O(new_n80_));
  nand3  g63(.a(x3), .b(new_n53_), .c(x1), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n80_), .c(new_n19_), .O(new_n82_));
  nand2  g65(.a(new_n81_), .b(new_n80_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(x0), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n82_), .c(new_n25_), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z5));
  nand2  g69(.a(x3), .b(new_n53_), .O(new_n87_));
  nand2  g70(.a(new_n81_), .b(new_n25_), .O(new_n88_));
  aoi21  g71(.a(new_n87_), .b(new_n27_), .c(new_n88_), .O(z6));
  aoi22  g72(.a(new_n87_), .b(new_n80_), .c(new_n20_), .d(new_n18_), .O(z7));
  nand2  g73(.a(new_n25_), .b(x3), .O(z8));
  nand3  g74(.a(new_n22_), .b(x5), .c(x4), .O(new_n92_));
  inv1   g75(.a(new_n92_), .O(z9));
endmodule


