// Benchmark "FAU" written by ABC on Thu Aug  6 21:27:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  aoi21  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  oai21  g05(.a(new_n19_), .b(x5), .c(new_n22_), .O(new_n23_));
  nand3  g06(.a(x3), .b(x2), .c(x1), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  nand2  g09(.a(x5), .b(x4), .O(new_n27_));
  aoi21  g10(.a(new_n26_), .b(new_n19_), .c(new_n27_), .O(z9));
  aoi21  g11(.a(new_n23_), .b(new_n18_), .c(z9), .O(z0));
  inv1   g12(.a(new_n21_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(x0), .c(x6), .O(new_n31_));
  inv1   g14(.a(x5), .O(new_n32_));
  nand2  g15(.a(new_n24_), .b(new_n19_), .O(new_n33_));
  aoi21  g16(.a(new_n33_), .b(x4), .c(new_n32_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nor2   g18(.a(x2), .b(x1), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(x5), .c(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x0), .O(new_n38_));
  nand3  g21(.a(x6), .b(new_n32_), .c(x4), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n38_), .c(new_n35_), .O(z1));
  nand2  g23(.a(new_n22_), .b(x4), .O(new_n41_));
  nand3  g24(.a(new_n25_), .b(new_n19_), .c(new_n18_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x5), .O(new_n44_));
  nand2  g27(.a(new_n19_), .b(x4), .O(new_n45_));
  oai21  g28(.a(x5), .b(x2), .c(x0), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(new_n21_), .c(x6), .d(new_n18_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n24_), .O(new_n49_));
  nand3  g32(.a(x6), .b(new_n18_), .c(new_n20_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n45_), .O(new_n51_));
  oai21  g34(.a(new_n36_), .b(new_n18_), .c(x6), .O(new_n52_));
  aoi22  g35(.a(new_n52_), .b(x0), .c(new_n51_), .d(new_n32_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(new_n44_), .O(z2));
  nand3  g37(.a(new_n33_), .b(new_n30_), .c(x5), .O(new_n55_));
  nand2  g38(.a(new_n33_), .b(new_n30_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n32_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n55_), .c(new_n20_), .O(new_n58_));
  nand2  g41(.a(new_n36_), .b(x5), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x0), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n37_), .c(new_n58_), .O(z3));
  xor2a  g44(.a(x6), .b(x1), .O(new_n62_));
  inv1   g45(.a(x3), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n20_), .O(new_n64_));
  nand2  g47(.a(new_n19_), .b(x0), .O(new_n65_));
  nand4  g48(.a(new_n65_), .b(new_n64_), .c(new_n62_), .d(x2), .O(new_n66_));
  nor2   g49(.a(x3), .b(x2), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(x6), .c(x1), .O(new_n68_));
  nand2  g51(.a(x3), .b(x2), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n20_), .O(new_n70_));
  or2    g53(.a(new_n70_), .b(new_n62_), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n68_), .c(new_n66_), .O(new_n72_));
  inv1   g55(.a(new_n72_), .O(z4));
  inv1   g56(.a(new_n69_), .O(new_n74_));
  nor2   g57(.a(new_n67_), .b(new_n74_), .O(new_n75_));
  nand2  g58(.a(new_n36_), .b(x3), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n20_), .O(new_n77_));
  nand3  g60(.a(new_n36_), .b(x3), .c(x0), .O(new_n78_));
  aoi21  g61(.a(new_n78_), .b(new_n77_), .c(new_n75_), .O(new_n79_));
  nand3  g62(.a(new_n78_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n65_), .O(new_n81_));
  nor2   g64(.a(new_n81_), .b(new_n79_), .O(z5));
  oai21  g65(.a(new_n63_), .b(x2), .c(x1), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n76_), .c(new_n65_), .O(z6));
  oai21  g67(.a(new_n67_), .b(new_n74_), .c(new_n65_), .O(z7));
  nand2  g68(.a(new_n65_), .b(x3), .O(z8));
endmodule


