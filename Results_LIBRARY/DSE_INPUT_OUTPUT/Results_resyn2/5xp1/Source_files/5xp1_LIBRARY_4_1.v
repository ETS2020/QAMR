// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n99_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nor2   g02(.a(x6), .b(x0), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x5), .c(new_n18_), .O(z0));
  inv1   g06(.a(x2), .O(new_n24_));
  oai21  g07(.a(x6), .b(x3), .c(x1), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  inv1   g09(.a(x0), .O(new_n27_));
  inv1   g10(.a(x6), .O(new_n28_));
  nand2  g11(.a(x4), .b(x1), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(new_n28_), .c(new_n27_), .O(new_n30_));
  inv1   g13(.a(x5), .O(new_n31_));
  nand2  g14(.a(x6), .b(x4), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  aoi21  g16(.a(new_n30_), .b(new_n26_), .c(new_n33_), .O(new_n34_));
  nand2  g17(.a(new_n22_), .b(x5), .O(new_n35_));
  nand2  g18(.a(x5), .b(new_n18_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nor2   g20(.a(new_n37_), .b(new_n34_), .O(z1));
  nand3  g21(.a(new_n28_), .b(new_n18_), .c(x2), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(new_n32_), .c(new_n19_), .O(new_n40_));
  nand2  g23(.a(x4), .b(x2), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n31_), .c(new_n28_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n40_), .c(x0), .O(new_n43_));
  oai21  g26(.a(x5), .b(x3), .c(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n24_), .O(new_n45_));
  oai21  g28(.a(new_n19_), .b(new_n27_), .c(new_n31_), .O(new_n46_));
  nand2  g29(.a(x3), .b(x1), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n27_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n28_), .c(x4), .O(new_n50_));
  inv1   g33(.a(x3), .O(new_n51_));
  nand4  g34(.a(new_n28_), .b(new_n18_), .c(x1), .d(x0), .O(new_n52_));
  nand3  g35(.a(x6), .b(x5), .c(x2), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g37(.a(x6), .b(x1), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(x4), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x5), .O(new_n57_));
  oai21  g40(.a(x2), .b(x1), .c(x0), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x6), .c(new_n18_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nor2   g43(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n50_), .c(new_n43_), .O(z2));
  xnor2a g45(.a(x5), .b(x0), .O(new_n63_));
  aoi21  g46(.a(x3), .b(x0), .c(x6), .O(new_n64_));
  nor2   g47(.a(x6), .b(x1), .O(new_n65_));
  oai21  g48(.a(x3), .b(x0), .c(x2), .O(new_n66_));
  oai22  g49(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n19_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nor2   g51(.a(new_n28_), .b(new_n24_), .O(new_n69_));
  nand3  g52(.a(x5), .b(new_n51_), .c(new_n27_), .O(new_n70_));
  oai21  g53(.a(new_n63_), .b(new_n69_), .c(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n19_), .O(new_n72_));
  nand2  g55(.a(new_n21_), .b(new_n20_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(x4), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(x5), .O(new_n75_));
  nand2  g58(.a(new_n51_), .b(new_n24_), .O(new_n76_));
  inv1   g59(.a(new_n76_), .O(new_n77_));
  nand4  g60(.a(new_n77_), .b(new_n28_), .c(new_n31_), .d(x0), .O(new_n78_));
  nand4  g61(.a(new_n78_), .b(new_n75_), .c(new_n72_), .d(new_n68_), .O(z3));
  nand2  g62(.a(new_n21_), .b(new_n27_), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n76_), .c(new_n28_), .O(new_n81_));
  nand2  g64(.a(new_n80_), .b(new_n76_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x6), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n81_), .c(x1), .O(new_n84_));
  aoi21  g67(.a(new_n66_), .b(new_n28_), .c(x1), .O(new_n85_));
  oai21  g68(.a(new_n66_), .b(new_n28_), .c(new_n85_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n84_), .c(new_n36_), .O(z4));
  nand2  g70(.a(new_n47_), .b(new_n24_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n21_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(x0), .O(new_n90_));
  nand3  g73(.a(new_n88_), .b(new_n21_), .c(new_n27_), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n90_), .c(new_n36_), .O(z5));
  nand2  g75(.a(x3), .b(new_n24_), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n19_), .c(new_n36_), .O(new_n94_));
  aoi21  g77(.a(new_n93_), .b(new_n19_), .c(new_n94_), .O(z6));
  nand3  g78(.a(new_n76_), .b(new_n36_), .c(new_n21_), .O(new_n96_));
  inv1   g79(.a(new_n96_), .O(z7));
  aoi21  g80(.a(x5), .b(new_n18_), .c(x3), .O(z8));
  nand3  g81(.a(new_n22_), .b(x5), .c(x4), .O(new_n99_));
  inv1   g82(.a(new_n99_), .O(z9));
endmodule


