// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:21 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n91_, new_n94_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x5), .O(new_n21_));
  nand3  g04(.a(x3), .b(x2), .c(x1), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  nand2  g08(.a(x3), .b(x2), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nor2   g10(.a(new_n19_), .b(new_n21_), .O(new_n28_));
  oai21  g11(.a(new_n27_), .b(x1), .c(new_n28_), .O(new_n29_));
  oai21  g12(.a(new_n25_), .b(new_n18_), .c(new_n29_), .O(z0));
  nand2  g13(.a(new_n19_), .b(new_n18_), .O(new_n31_));
  nor2   g14(.a(x6), .b(x1), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n26_), .c(new_n20_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g17(.a(x1), .O(new_n35_));
  nand2  g18(.a(new_n18_), .b(new_n35_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x6), .c(new_n21_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  oai21  g21(.a(x6), .b(x1), .c(x2), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  inv1   g23(.a(x3), .O(new_n41_));
  aoi21  g24(.a(new_n19_), .b(new_n41_), .c(new_n35_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n40_), .c(x0), .O(new_n43_));
  nor2   g26(.a(new_n19_), .b(new_n18_), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n31_), .c(new_n21_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(new_n38_), .O(z1));
  oai21  g30(.a(x3), .b(x2), .c(x0), .O(new_n48_));
  nor2   g31(.a(new_n48_), .b(new_n35_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n23_), .c(x4), .O(new_n50_));
  nand2  g33(.a(new_n48_), .b(new_n21_), .O(new_n51_));
  nand2  g34(.a(x5), .b(x0), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n35_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n51_), .c(new_n27_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n18_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n50_), .c(new_n19_), .O(new_n56_));
  nand2  g39(.a(new_n21_), .b(new_n20_), .O(new_n57_));
  oai21  g40(.a(x3), .b(x0), .c(x2), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n53_), .c(new_n57_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(x6), .c(new_n18_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n56_), .O(z2));
  nand2  g45(.a(new_n19_), .b(new_n41_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(x1), .c(x0), .O(new_n64_));
  nand2  g47(.a(x6), .b(x1), .O(new_n65_));
  inv1   g48(.a(new_n65_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n33_), .c(new_n64_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(x5), .O(new_n68_));
  aoi21  g51(.a(new_n65_), .b(new_n20_), .c(x5), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(new_n43_), .O(new_n70_));
  oai21  g53(.a(new_n57_), .b(new_n41_), .c(new_n52_), .O(new_n71_));
  aoi21  g54(.a(new_n71_), .b(new_n40_), .c(new_n44_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n70_), .c(new_n68_), .O(z3));
  aoi21  g56(.a(new_n58_), .b(new_n19_), .c(x1), .O(new_n74_));
  oai21  g57(.a(new_n58_), .b(new_n19_), .c(new_n74_), .O(new_n75_));
  inv1   g58(.a(x2), .O(new_n76_));
  nand2  g59(.a(new_n41_), .b(new_n76_), .O(new_n77_));
  nand3  g60(.a(new_n77_), .b(new_n27_), .c(x6), .O(new_n78_));
  nand3  g61(.a(new_n48_), .b(new_n26_), .c(new_n19_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(x1), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n75_), .c(new_n45_), .O(z4));
  oai21  g65(.a(new_n41_), .b(new_n35_), .c(new_n76_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n26_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x0), .O(new_n85_));
  nand3  g68(.a(new_n83_), .b(new_n26_), .c(new_n20_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n85_), .c(new_n45_), .O(z5));
  nand2  g70(.a(x3), .b(new_n76_), .O(new_n88_));
  oai21  g71(.a(new_n88_), .b(new_n35_), .c(new_n45_), .O(new_n89_));
  aoi21  g72(.a(new_n88_), .b(new_n35_), .c(new_n89_), .O(z6));
  nand2  g73(.a(new_n45_), .b(new_n26_), .O(new_n91_));
  aoi21  g74(.a(new_n41_), .b(new_n76_), .c(new_n91_), .O(z7));
  nand2  g75(.a(new_n45_), .b(x3), .O(z8));
  nand2  g76(.a(new_n25_), .b(x4), .O(new_n94_));
  inv1   g77(.a(new_n94_), .O(z9));
endmodule


