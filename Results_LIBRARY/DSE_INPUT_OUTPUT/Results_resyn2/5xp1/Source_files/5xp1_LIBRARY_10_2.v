// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_;
  inv1   g00(.a(x4), .O(new_n18_));
  nand2  g01(.a(x5), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  nor2   g07(.a(x5), .b(new_n18_), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x5), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  oai21  g12(.a(new_n24_), .b(new_n19_), .c(new_n29_), .O(z0));
  oai21  g13(.a(x2), .b(x1), .c(x0), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(new_n18_), .c(x5), .O(new_n32_));
  nor2   g15(.a(new_n23_), .b(new_n19_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n32_), .c(x6), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  inv1   g18(.a(x3), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x1), .O(new_n38_));
  nand2  g21(.a(new_n25_), .b(x0), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n38_), .c(new_n34_), .O(z1));
  nand2  g23(.a(new_n31_), .b(new_n18_), .O(new_n41_));
  inv1   g24(.a(x5), .O(new_n42_));
  nor2   g25(.a(x2), .b(x1), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n20_), .c(new_n42_), .O(new_n44_));
  nand3  g27(.a(x5), .b(x4), .c(x1), .O(new_n45_));
  oai21  g28(.a(new_n44_), .b(new_n41_), .c(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x6), .O(new_n47_));
  nor2   g30(.a(x6), .b(x5), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n18_), .O(new_n49_));
  nand2  g32(.a(x1), .b(x0), .O(new_n50_));
  nand2  g33(.a(x6), .b(x4), .O(new_n51_));
  nand2  g34(.a(x5), .b(x2), .O(new_n52_));
  oai22  g35(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(x3), .O(new_n54_));
  aoi21  g37(.a(new_n43_), .b(new_n42_), .c(new_n51_), .O(new_n55_));
  nand2  g38(.a(x2), .b(x1), .O(new_n56_));
  nor2   g39(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(x0), .O(new_n58_));
  nand4  g41(.a(x6), .b(new_n18_), .c(new_n21_), .d(new_n20_), .O(new_n59_));
  nand3  g42(.a(new_n48_), .b(x4), .c(new_n35_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g44(.a(new_n50_), .b(new_n48_), .c(x4), .O(new_n62_));
  inv1   g45(.a(new_n62_), .O(new_n63_));
  aoi21  g46(.a(new_n61_), .b(new_n36_), .c(new_n63_), .O(new_n64_));
  nand4  g47(.a(new_n64_), .b(new_n58_), .c(new_n54_), .d(new_n47_), .O(z2));
  oai21  g48(.a(new_n26_), .b(new_n21_), .c(new_n22_), .O(new_n66_));
  aoi21  g49(.a(new_n26_), .b(new_n21_), .c(x0), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g51(.a(new_n35_), .b(new_n21_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(x6), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n38_), .c(x0), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n42_), .O(new_n73_));
  nand2  g56(.a(new_n31_), .b(new_n23_), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(x6), .c(x5), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n73_), .O(z3));
  nor2   g59(.a(x3), .b(x2), .O(new_n77_));
  aoi21  g60(.a(x3), .b(x2), .c(x0), .O(new_n78_));
  nor2   g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n26_), .O(new_n80_));
  oai21  g63(.a(new_n78_), .b(new_n77_), .c(x6), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n80_), .c(x1), .O(new_n82_));
  oai21  g65(.a(x3), .b(x0), .c(x2), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n26_), .c(x1), .O(new_n84_));
  oai21  g67(.a(new_n83_), .b(new_n26_), .c(new_n84_), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n82_), .c(new_n27_), .O(z4));
  nand2  g69(.a(new_n37_), .b(new_n22_), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n43_), .c(x0), .O(new_n88_));
  nand4  g71(.a(new_n37_), .b(new_n69_), .c(new_n22_), .d(new_n20_), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n88_), .c(new_n27_), .O(z5));
  nand2  g73(.a(x3), .b(new_n35_), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n21_), .c(new_n27_), .O(new_n92_));
  aoi21  g75(.a(new_n91_), .b(new_n21_), .c(new_n92_), .O(z6));
  nand2  g76(.a(new_n87_), .b(new_n27_), .O(z7));
  nor2   g77(.a(new_n28_), .b(x3), .O(z8));
  aoi21  g78(.a(x6), .b(new_n18_), .c(new_n42_), .O(z9));
endmodule


