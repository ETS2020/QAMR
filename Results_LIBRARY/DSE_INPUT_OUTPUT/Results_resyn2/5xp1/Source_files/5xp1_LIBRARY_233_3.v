// Benchmark "FAU" written by ABC on Thu Aug  6 21:27:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n84_, new_n85_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nor2   g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  aoi21  g04(.a(new_n18_), .b(x4), .c(new_n21_), .O(new_n22_));
  nand3  g05(.a(x3), .b(x2), .c(x1), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x6), .c(x4), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  aoi21  g09(.a(x3), .b(x2), .c(x1), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x6), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n25_), .c(x5), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n22_), .O(z0));
  oai21  g15(.a(x3), .b(x2), .c(x1), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x6), .O(new_n35_));
  nand3  g18(.a(new_n29_), .b(new_n25_), .c(x5), .O(new_n36_));
  oai21  g19(.a(new_n35_), .b(new_n22_), .c(new_n36_), .O(z1));
  inv1   g20(.a(new_n21_), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  oai21  g22(.a(x3), .b(x2), .c(x0), .O(new_n40_));
  nand4  g23(.a(new_n20_), .b(x5), .c(x3), .d(x2), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  oai21  g25(.a(new_n27_), .b(new_n18_), .c(x6), .O(new_n43_));
  nand2  g26(.a(x5), .b(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n42_), .c(new_n26_), .O(new_n46_));
  aoi21  g29(.a(new_n23_), .b(new_n19_), .c(new_n18_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n34_), .c(new_n20_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n43_), .c(x4), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(new_n38_), .O(z2));
  nand3  g33(.a(new_n29_), .b(new_n23_), .c(new_n18_), .O(new_n51_));
  inv1   g34(.a(new_n23_), .O(new_n52_));
  nor2   g35(.a(new_n27_), .b(new_n20_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n52_), .c(x5), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n51_), .c(new_n19_), .O(new_n55_));
  nand2  g38(.a(new_n33_), .b(x5), .O(new_n56_));
  nand2  g39(.a(new_n20_), .b(x0), .O(new_n57_));
  nor2   g40(.a(new_n33_), .b(x5), .O(new_n58_));
  nor2   g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n55_), .O(z3));
  oai21  g44(.a(x3), .b(x0), .c(x2), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n39_), .O(new_n63_));
  nand2  g46(.a(x3), .b(x2), .O(new_n64_));
  inv1   g47(.a(new_n64_), .O(new_n65_));
  nor2   g48(.a(new_n65_), .b(x0), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n33_), .c(new_n63_), .O(new_n67_));
  inv1   g50(.a(new_n24_), .O(new_n68_));
  nand2  g51(.a(new_n53_), .b(new_n68_), .O(new_n69_));
  inv1   g52(.a(new_n69_), .O(new_n70_));
  aoi21  g53(.a(new_n67_), .b(new_n20_), .c(new_n70_), .O(z4));
  aoi21  g54(.a(x3), .b(x1), .c(x2), .O(new_n72_));
  nor2   g55(.a(new_n72_), .b(new_n65_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n19_), .O(new_n74_));
  oai21  g57(.a(new_n73_), .b(new_n57_), .c(new_n74_), .O(z5));
  inv1   g58(.a(x3), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(x2), .c(x1), .O(new_n77_));
  inv1   g60(.a(x2), .O(new_n78_));
  nand3  g61(.a(x3), .b(new_n78_), .c(new_n39_), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n77_), .c(new_n38_), .O(z6));
  nor2   g63(.a(x3), .b(x2), .O(new_n81_));
  oai21  g64(.a(new_n81_), .b(new_n65_), .c(new_n38_), .O(z7));
  nand2  g65(.a(new_n38_), .b(x3), .O(z8));
  nand2  g66(.a(new_n68_), .b(new_n20_), .O(new_n84_));
  nand4  g67(.a(new_n84_), .b(new_n38_), .c(x5), .d(x4), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z9));
endmodule


