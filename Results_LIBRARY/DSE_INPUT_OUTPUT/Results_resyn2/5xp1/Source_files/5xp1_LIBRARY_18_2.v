// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n89_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x4), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(x6), .c(x5), .d(new_n21_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n20_), .O(z0));
  oai21  g10(.a(new_n25_), .b(x4), .c(x6), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x5), .O(new_n29_));
  inv1   g12(.a(x2), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n23_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x6), .c(x0), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n21_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n29_), .O(z1));
  oai21  g18(.a(x3), .b(x2), .c(x0), .O(new_n36_));
  nand3  g19(.a(x5), .b(x3), .c(x2), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(new_n36_), .c(x6), .O(new_n38_));
  nor2   g21(.a(x5), .b(x0), .O(new_n39_));
  nor2   g22(.a(new_n39_), .b(new_n21_), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n38_), .c(x1), .O(new_n41_));
  nand2  g24(.a(new_n18_), .b(new_n30_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n37_), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x4), .O(new_n45_));
  nand2  g28(.a(x5), .b(x0), .O(new_n46_));
  nor2   g29(.a(new_n46_), .b(x6), .O(new_n47_));
  nand2  g30(.a(new_n24_), .b(new_n23_), .O(new_n48_));
  inv1   g31(.a(new_n39_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g33(.a(x6), .b(new_n21_), .O(new_n51_));
  aoi21  g34(.a(new_n42_), .b(x0), .c(new_n51_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n50_), .c(new_n47_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n45_), .c(new_n41_), .O(z2));
  oai21  g37(.a(x3), .b(x0), .c(x2), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  nand3  g39(.a(new_n36_), .b(new_n24_), .c(new_n19_), .O(new_n57_));
  nand2  g40(.a(new_n46_), .b(new_n49_), .O(new_n58_));
  oai21  g41(.a(new_n55_), .b(new_n19_), .c(new_n23_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g43(.a(new_n56_), .b(x6), .O(new_n61_));
  nand4  g44(.a(new_n61_), .b(new_n46_), .c(new_n49_), .d(new_n23_), .O(new_n62_));
  inv1   g45(.a(x3), .O(new_n63_));
  nand4  g46(.a(new_n18_), .b(new_n63_), .c(new_n30_), .d(x0), .O(new_n64_));
  nand3  g47(.a(new_n24_), .b(x5), .c(new_n22_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n64_), .c(new_n21_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n19_), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n62_), .c(new_n60_), .O(z3));
  nand3  g51(.a(new_n36_), .b(new_n24_), .c(new_n19_), .O(new_n69_));
  inv1   g52(.a(new_n69_), .O(new_n70_));
  aoi21  g53(.a(new_n36_), .b(new_n24_), .c(new_n19_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n70_), .c(x1), .O(new_n72_));
  nand2  g55(.a(new_n55_), .b(new_n19_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n61_), .c(new_n23_), .O(new_n74_));
  nor2   g57(.a(x6), .b(new_n21_), .O(new_n75_));
  inv1   g58(.a(new_n75_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(z4));
  xor2a  g60(.a(x3), .b(x2), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n31_), .c(x0), .O(new_n79_));
  nand2  g62(.a(new_n78_), .b(new_n31_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n22_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n79_), .c(new_n76_), .O(new_n82_));
  inv1   g65(.a(new_n82_), .O(z5));
  nand3  g66(.a(x3), .b(new_n30_), .c(new_n23_), .O(new_n84_));
  oai21  g67(.a(new_n63_), .b(x2), .c(x1), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n84_), .c(new_n76_), .O(z6));
  or2    g69(.a(new_n78_), .b(new_n75_), .O(z7));
  nand2  g70(.a(new_n76_), .b(x3), .O(z8));
  nand3  g71(.a(x6), .b(x5), .c(x4), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z9));
endmodule


