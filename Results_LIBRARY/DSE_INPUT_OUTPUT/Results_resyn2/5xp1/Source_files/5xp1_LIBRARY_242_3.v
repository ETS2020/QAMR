// Benchmark "FAU" written by ABC on Thu Aug  6 21:27:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n18_), .c(x4), .O(new_n24_));
  inv1   g07(.a(x1), .O(new_n25_));
  aoi21  g08(.a(x3), .b(x2), .c(x0), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g10(.a(x4), .O(new_n28_));
  nand2  g11(.a(x6), .b(new_n28_), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n27_), .c(x5), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n24_), .O(z0));
  nand2  g15(.a(new_n21_), .b(new_n19_), .O(new_n33_));
  aoi21  g16(.a(new_n33_), .b(x4), .c(x6), .O(new_n34_));
  nand2  g17(.a(x3), .b(x2), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n28_), .c(new_n25_), .d(new_n19_), .O(new_n36_));
  inv1   g19(.a(new_n36_), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n34_), .c(x5), .O(new_n38_));
  nand2  g21(.a(x6), .b(x4), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  inv1   g23(.a(x2), .O(new_n41_));
  oai21  g24(.a(x6), .b(x3), .c(x1), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g26(.a(x4), .b(x1), .c(x6), .O(new_n44_));
  nand2  g27(.a(new_n20_), .b(x0), .O(new_n45_));
  nor2   g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n43_), .c(new_n40_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n38_), .O(z1));
  nand2  g31(.a(new_n20_), .b(new_n19_), .O(new_n49_));
  oai21  g32(.a(x3), .b(x0), .c(x2), .O(new_n50_));
  aoi21  g33(.a(x5), .b(x0), .c(x1), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n30_), .O(new_n54_));
  oai21  g37(.a(x5), .b(x3), .c(x0), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n41_), .O(new_n56_));
  inv1   g39(.a(x3), .O(new_n57_));
  oai21  g40(.a(new_n20_), .b(new_n57_), .c(new_n19_), .O(new_n58_));
  nand2  g41(.a(x5), .b(x0), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n25_), .c(new_n28_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  inv1   g44(.a(new_n61_), .O(new_n62_));
  nor2   g45(.a(x3), .b(x2), .O(new_n63_));
  nand2  g46(.a(x1), .b(x0), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n63_), .c(new_n28_), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n22_), .c(new_n18_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n62_), .c(new_n54_), .O(z2));
  aoi21  g50(.a(new_n18_), .b(new_n25_), .c(new_n41_), .O(new_n68_));
  nand2  g51(.a(new_n42_), .b(x0), .O(new_n69_));
  nand3  g52(.a(x6), .b(x1), .c(new_n19_), .O(new_n70_));
  oai21  g53(.a(new_n69_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n20_), .O(new_n72_));
  oai21  g55(.a(new_n18_), .b(new_n25_), .c(new_n35_), .O(new_n73_));
  aoi21  g56(.a(new_n18_), .b(new_n25_), .c(x0), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n69_), .c(x5), .O(new_n76_));
  oai21  g59(.a(new_n49_), .b(new_n57_), .c(new_n59_), .O(new_n77_));
  aoi21  g60(.a(new_n77_), .b(new_n68_), .c(new_n40_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n76_), .c(new_n72_), .O(z3));
  oai21  g62(.a(new_n63_), .b(new_n26_), .c(x6), .O(new_n80_));
  nor2   g63(.a(new_n63_), .b(new_n26_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n80_), .c(x1), .O(new_n83_));
  nand2  g66(.a(new_n50_), .b(new_n29_), .O(new_n84_));
  or2    g67(.a(new_n50_), .b(new_n18_), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n84_), .c(new_n25_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n83_), .O(z4));
  oai21  g70(.a(new_n57_), .b(new_n25_), .c(new_n41_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n35_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(x0), .O(new_n90_));
  aoi21  g73(.a(new_n88_), .b(new_n26_), .c(new_n40_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n90_), .O(z5));
  nand2  g75(.a(x3), .b(new_n41_), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n25_), .c(new_n39_), .O(new_n94_));
  aoi21  g77(.a(new_n93_), .b(new_n25_), .c(new_n94_), .O(z6));
  nand2  g78(.a(new_n39_), .b(new_n35_), .O(new_n96_));
  nor2   g79(.a(new_n96_), .b(new_n63_), .O(z7));
  nand2  g80(.a(new_n39_), .b(x3), .O(z8));
  aoi21  g81(.a(new_n23_), .b(new_n18_), .c(new_n28_), .O(z9));
endmodule


