// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n88_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  aoi21  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  oai21  g05(.a(new_n19_), .b(x5), .c(new_n22_), .O(new_n23_));
  nand3  g06(.a(x3), .b(x2), .c(x1), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  nand2  g09(.a(x5), .b(x4), .O(new_n27_));
  aoi21  g10(.a(new_n26_), .b(new_n19_), .c(new_n27_), .O(z9));
  aoi21  g11(.a(new_n23_), .b(new_n18_), .c(z9), .O(z0));
  inv1   g12(.a(x5), .O(new_n30_));
  oai21  g13(.a(x2), .b(x1), .c(x0), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x6), .c(new_n30_), .O(new_n33_));
  inv1   g16(.a(x1), .O(new_n34_));
  nand2  g17(.a(x3), .b(x2), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x6), .O(new_n37_));
  nand2  g20(.a(new_n24_), .b(new_n19_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x4), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(new_n37_), .c(x5), .d(new_n20_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n33_), .O(z1));
  nand3  g24(.a(x6), .b(x3), .c(x2), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x4), .O(new_n44_));
  nand2  g27(.a(new_n19_), .b(x4), .O(new_n45_));
  nand2  g28(.a(x6), .b(new_n18_), .O(new_n46_));
  nand2  g29(.a(new_n35_), .b(new_n19_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x1), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x5), .O(new_n50_));
  oai21  g33(.a(new_n46_), .b(x0), .c(new_n45_), .O(new_n51_));
  inv1   g34(.a(new_n35_), .O(new_n52_));
  aoi21  g35(.a(new_n19_), .b(x4), .c(new_n34_), .O(new_n53_));
  oai21  g36(.a(new_n53_), .b(new_n52_), .c(x5), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  oai21  g38(.a(x2), .b(x1), .c(x4), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(x6), .c(new_n20_), .O(new_n57_));
  inv1   g40(.a(x2), .O(new_n58_));
  nand2  g41(.a(new_n30_), .b(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n46_), .c(new_n45_), .O(new_n60_));
  aoi21  g43(.a(new_n60_), .b(new_n34_), .c(new_n57_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n55_), .c(new_n50_), .O(z2));
  nand2  g45(.a(new_n38_), .b(new_n36_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n30_), .O(new_n64_));
  aoi21  g47(.a(new_n36_), .b(x5), .c(x0), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai22  g49(.a(new_n59_), .b(new_n20_), .c(x6), .d(new_n30_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n34_), .O(new_n68_));
  nand2  g51(.a(new_n47_), .b(new_n31_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(x5), .O(new_n70_));
  nand2  g53(.a(new_n19_), .b(x0), .O(new_n71_));
  nand4  g54(.a(new_n71_), .b(new_n70_), .c(new_n68_), .d(new_n66_), .O(z3));
  inv1   g55(.a(x3), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(x0), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n47_), .c(new_n42_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n71_), .c(x1), .O(new_n77_));
  aoi21  g60(.a(x2), .b(x0), .c(x1), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n47_), .c(new_n42_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n77_), .O(z4));
  oai21  g63(.a(x2), .b(x1), .c(x3), .O(new_n81_));
  xnor2a g64(.a(x2), .b(x0), .O(new_n82_));
  oai21  g65(.a(new_n82_), .b(new_n81_), .c(new_n71_), .O(new_n83_));
  aoi21  g66(.a(new_n82_), .b(new_n81_), .c(new_n83_), .O(z5));
  nand3  g67(.a(x3), .b(new_n58_), .c(new_n34_), .O(new_n85_));
  oai21  g68(.a(new_n73_), .b(x2), .c(x1), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n85_), .c(new_n71_), .O(z6));
  nand3  g70(.a(new_n74_), .b(new_n71_), .c(new_n35_), .O(new_n88_));
  inv1   g71(.a(new_n88_), .O(z7));
  nand2  g72(.a(new_n71_), .b(x3), .O(z8));
endmodule


