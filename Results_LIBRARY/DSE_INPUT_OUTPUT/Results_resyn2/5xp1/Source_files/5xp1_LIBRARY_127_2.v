// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:24 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  inv1   g04(.a(x5), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(x4), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(x6), .O(new_n24_));
  oai21  g07(.a(x6), .b(x0), .c(x5), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x1), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(x4), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n24_), .O(z0));
  aoi21  g12(.a(new_n20_), .b(new_n18_), .c(new_n26_), .O(new_n30_));
  oai21  g13(.a(x6), .b(x0), .c(x4), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x5), .c(new_n19_), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x0), .c(x4), .O(new_n35_));
  nand2  g18(.a(x6), .b(new_n22_), .O(new_n36_));
  oai22  g19(.a(new_n36_), .b(new_n35_), .c(new_n32_), .d(new_n30_), .O(z1));
  oai21  g20(.a(x5), .b(x0), .c(x6), .O(new_n38_));
  nand2  g21(.a(x5), .b(x0), .O(new_n39_));
  oai21  g22(.a(x3), .b(x0), .c(x2), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  nand2  g24(.a(new_n39_), .b(new_n26_), .O(new_n42_));
  oai21  g25(.a(x5), .b(x0), .c(x1), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n41_), .c(x4), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  inv1   g29(.a(new_n38_), .O(new_n47_));
  nand3  g30(.a(new_n40_), .b(new_n39_), .c(new_n19_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n42_), .c(new_n46_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n45_), .c(new_n27_), .O(z2));
  nand2  g34(.a(new_n20_), .b(new_n19_), .O(new_n52_));
  nor2   g35(.a(x5), .b(x0), .O(new_n53_));
  aoi21  g36(.a(new_n33_), .b(new_n19_), .c(new_n39_), .O(new_n54_));
  aoi21  g37(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  inv1   g38(.a(x3), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n18_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(x6), .c(x2), .O(new_n58_));
  nor2   g41(.a(new_n53_), .b(x1), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n58_), .c(new_n39_), .O(new_n60_));
  oai21  g43(.a(new_n55_), .b(new_n26_), .c(new_n60_), .O(z3));
  nor2   g44(.a(new_n40_), .b(new_n26_), .O(new_n62_));
  oai21  g45(.a(new_n56_), .b(new_n18_), .c(x1), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x6), .O(new_n64_));
  and2   g47(.a(new_n40_), .b(new_n27_), .O(new_n65_));
  aoi22  g48(.a(new_n65_), .b(new_n64_), .c(new_n62_), .d(new_n19_), .O(z4));
  oai21  g49(.a(new_n56_), .b(new_n19_), .c(new_n33_), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n20_), .c(new_n18_), .O(new_n68_));
  nand2  g51(.a(new_n67_), .b(new_n20_), .O(new_n69_));
  nand2  g52(.a(new_n69_), .b(x0), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n68_), .c(new_n27_), .O(z5));
  nor2   g54(.a(new_n56_), .b(x2), .O(new_n72_));
  nand2  g55(.a(x6), .b(x1), .O(new_n73_));
  oai22  g56(.a(new_n73_), .b(new_n72_), .c(new_n34_), .d(new_n56_), .O(z6));
  nor2   g57(.a(x3), .b(new_n33_), .O(new_n75_));
  nor2   g58(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  aoi21  g59(.a(new_n26_), .b(x1), .c(new_n76_), .O(z7));
  nand2  g60(.a(new_n27_), .b(x3), .O(z8));
  oai21  g61(.a(new_n31_), .b(new_n22_), .c(new_n27_), .O(z9));
endmodule


