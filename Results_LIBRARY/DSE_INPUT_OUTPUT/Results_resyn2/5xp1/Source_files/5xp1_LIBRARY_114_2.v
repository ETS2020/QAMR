// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n89_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(x5), .c(x4), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  inv1   g06(.a(x5), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n23_), .c(x6), .O(new_n25_));
  nor2   g08(.a(new_n25_), .b(new_n22_), .O(z0));
  oai21  g09(.a(x2), .b(x1), .c(x0), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(x6), .c(new_n24_), .O(new_n29_));
  nand2  g12(.a(new_n21_), .b(x6), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x5), .c(new_n23_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n29_), .O(z1));
  nand3  g15(.a(new_n27_), .b(new_n22_), .c(x6), .O(new_n33_));
  nand2  g16(.a(new_n24_), .b(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x4), .O(new_n35_));
  inv1   g18(.a(x6), .O(new_n36_));
  nand2  g19(.a(new_n20_), .b(new_n18_), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n34_), .c(new_n37_), .d(new_n36_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x1), .O(new_n43_));
  nand2  g26(.a(x5), .b(x0), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n23_), .c(x6), .O(new_n45_));
  nand2  g28(.a(x2), .b(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n24_), .O(new_n47_));
  aoi21  g30(.a(new_n20_), .b(new_n18_), .c(new_n23_), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n43_), .c(new_n33_), .O(z2));
  nand2  g33(.a(new_n19_), .b(new_n18_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n27_), .c(x6), .O(new_n52_));
  oai21  g35(.a(x3), .b(x2), .c(x1), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n36_), .c(new_n23_), .d(x0), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n24_), .O(new_n56_));
  oai21  g39(.a(new_n20_), .b(new_n36_), .c(new_n19_), .O(new_n57_));
  aoi21  g40(.a(x3), .b(x2), .c(x6), .O(new_n58_));
  nor2   g41(.a(new_n58_), .b(x0), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  oai21  g43(.a(x6), .b(x3), .c(x1), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x0), .O(new_n62_));
  nand2  g45(.a(new_n36_), .b(x4), .O(new_n63_));
  nand4  g46(.a(new_n63_), .b(new_n62_), .c(new_n60_), .d(x5), .O(new_n64_));
  oai21  g47(.a(new_n34_), .b(new_n39_), .c(new_n44_), .O(new_n65_));
  oai21  g48(.a(x4), .b(new_n19_), .c(new_n36_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n65_), .c(x2), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n64_), .c(new_n56_), .O(z3));
  nand2  g51(.a(new_n46_), .b(new_n20_), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(x6), .c(x1), .O(new_n70_));
  oai21  g53(.a(new_n69_), .b(x6), .c(new_n70_), .O(new_n71_));
  nor2   g54(.a(x3), .b(x2), .O(new_n72_));
  oai21  g55(.a(new_n72_), .b(new_n18_), .c(new_n58_), .O(new_n73_));
  nand3  g56(.a(new_n40_), .b(new_n37_), .c(x6), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x1), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n71_), .c(new_n63_), .O(z4));
  nor2   g60(.a(x2), .b(x1), .O(new_n78_));
  nand2  g61(.a(new_n40_), .b(new_n20_), .O(new_n79_));
  oai21  g62(.a(new_n79_), .b(new_n78_), .c(x0), .O(new_n80_));
  nor2   g63(.a(new_n79_), .b(new_n78_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n18_), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n80_), .c(new_n63_), .O(z5));
  nand2  g66(.a(x3), .b(new_n38_), .O(new_n84_));
  oai21  g67(.a(new_n84_), .b(new_n19_), .c(new_n63_), .O(new_n85_));
  aoi21  g68(.a(new_n84_), .b(new_n19_), .c(new_n85_), .O(z6));
  aoi21  g69(.a(new_n36_), .b(x4), .c(new_n79_), .O(z7));
  nand2  g70(.a(new_n63_), .b(x3), .O(z8));
  nand3  g71(.a(x6), .b(x5), .c(x4), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z9));
endmodule


