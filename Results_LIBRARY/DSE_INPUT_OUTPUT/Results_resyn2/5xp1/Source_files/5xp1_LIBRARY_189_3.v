// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x5), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(x6), .c(x4), .O(new_n23_));
  nand2  g06(.a(new_n20_), .b(new_n18_), .O(new_n24_));
  inv1   g07(.a(x5), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nor2   g09(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  oai21  g10(.a(new_n24_), .b(x1), .c(new_n27_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n23_), .O(z0));
  nor2   g12(.a(x6), .b(x4), .O(new_n30_));
  inv1   g13(.a(x4), .O(new_n31_));
  nand3  g14(.a(new_n20_), .b(new_n31_), .c(new_n19_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(x6), .c(new_n21_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n30_), .c(x5), .O(new_n34_));
  inv1   g17(.a(x3), .O(new_n35_));
  oai21  g18(.a(x6), .b(new_n35_), .c(x4), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x1), .O(new_n37_));
  oai21  g20(.a(x6), .b(new_n19_), .c(x4), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x2), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nor3   g23(.a(new_n30_), .b(x5), .c(new_n18_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n34_), .O(z1));
  inv1   g26(.a(x2), .O(new_n44_));
  nand2  g27(.a(new_n35_), .b(new_n44_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x0), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n19_), .c(new_n22_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  nor2   g31(.a(new_n25_), .b(new_n19_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n26_), .c(new_n31_), .O(new_n50_));
  nor2   g33(.a(new_n25_), .b(new_n35_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n38_), .c(x2), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  nand2  g36(.a(x6), .b(x4), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x5), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n39_), .c(new_n37_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n48_), .O(z2));
  nand2  g42(.a(new_n49_), .b(x6), .O(new_n60_));
  nand3  g43(.a(new_n25_), .b(new_n44_), .c(new_n19_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  nand2  g45(.a(new_n20_), .b(x5), .O(new_n63_));
  nand2  g46(.a(x6), .b(x1), .O(new_n64_));
  oai21  g47(.a(new_n25_), .b(new_n19_), .c(new_n18_), .O(new_n65_));
  aoi21  g48(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n62_), .c(new_n31_), .O(new_n67_));
  oai21  g50(.a(new_n35_), .b(new_n19_), .c(x5), .O(new_n68_));
  nand2  g51(.a(new_n45_), .b(new_n25_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(x0), .O(new_n70_));
  nand3  g53(.a(new_n20_), .b(x5), .c(new_n18_), .O(new_n71_));
  xor2a  g54(.a(x5), .b(x0), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n19_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n26_), .O(new_n75_));
  nand2  g58(.a(x6), .b(new_n31_), .O(new_n76_));
  oai21  g59(.a(x6), .b(new_n19_), .c(new_n76_), .O(new_n77_));
  oai21  g60(.a(x3), .b(x0), .c(x2), .O(new_n78_));
  nor2   g61(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n75_), .c(new_n67_), .O(z3));
  nand4  g64(.a(new_n45_), .b(new_n24_), .c(x6), .d(new_n31_), .O(new_n82_));
  nand3  g65(.a(new_n46_), .b(new_n20_), .c(new_n26_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x1), .O(new_n85_));
  nand2  g68(.a(new_n78_), .b(new_n76_), .O(new_n86_));
  or2    g69(.a(new_n78_), .b(new_n26_), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n86_), .c(new_n19_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n85_), .O(z4));
  inv1   g72(.a(new_n20_), .O(new_n90_));
  aoi21  g73(.a(x3), .b(x1), .c(x2), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n90_), .c(x0), .O(new_n92_));
  or2    g75(.a(new_n91_), .b(new_n24_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n92_), .c(new_n54_), .O(z5));
  nand2  g77(.a(x3), .b(new_n44_), .O(new_n95_));
  oai21  g78(.a(new_n95_), .b(new_n19_), .c(new_n54_), .O(new_n96_));
  aoi21  g79(.a(new_n95_), .b(new_n19_), .c(new_n96_), .O(z6));
  nand2  g80(.a(new_n45_), .b(new_n20_), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n54_), .O(z7));
  aoi21  g82(.a(x6), .b(x4), .c(x3), .O(z8));
  aoi21  g83(.a(new_n22_), .b(new_n26_), .c(new_n31_), .O(z9));
endmodule


