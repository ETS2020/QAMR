// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n103_,
    new_n104_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x5), .d(new_n19_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x5), .c(new_n18_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  nor2   g07(.a(x6), .b(x0), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(x4), .c(x1), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(x4), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(x6), .c(new_n27_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n24_), .O(z0));
  inv1   g14(.a(x1), .O(new_n32_));
  oai21  g15(.a(x3), .b(x2), .c(x0), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g17(.a(new_n21_), .b(x5), .c(new_n19_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n20_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n34_), .c(new_n23_), .O(new_n37_));
  nand3  g20(.a(new_n20_), .b(x5), .c(new_n18_), .O(new_n38_));
  nand3  g21(.a(x6), .b(new_n28_), .c(x0), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g23(.a(new_n40_), .b(new_n27_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n37_), .O(z1));
  inv1   g25(.a(x2), .O(new_n43_));
  oai21  g26(.a(x5), .b(x3), .c(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g28(.a(x5), .b(x3), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  aoi21  g30(.a(x5), .b(x0), .c(x1), .O(new_n48_));
  nor2   g31(.a(new_n48_), .b(x6), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(new_n50_));
  oai21  g33(.a(x3), .b(x0), .c(x2), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand2  g35(.a(new_n28_), .b(new_n19_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x6), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n50_), .c(x4), .O(new_n56_));
  nand3  g39(.a(x6), .b(new_n28_), .c(new_n19_), .O(new_n57_));
  nand3  g40(.a(new_n47_), .b(new_n45_), .c(new_n20_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g42(.a(x4), .b(new_n32_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n56_), .O(z2));
  xor2a  g45(.a(x5), .b(x0), .O(new_n63_));
  nor2   g46(.a(x3), .b(x0), .O(new_n64_));
  nand2  g47(.a(x6), .b(x2), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n64_), .c(new_n32_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  inv1   g50(.a(new_n63_), .O(new_n68_));
  inv1   g51(.a(new_n64_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(x6), .c(x2), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n67_), .c(x4), .O(new_n72_));
  nand3  g55(.a(new_n33_), .b(new_n21_), .c(new_n20_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n63_), .O(new_n74_));
  nand4  g57(.a(new_n68_), .b(new_n51_), .c(new_n46_), .d(new_n20_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n74_), .c(x1), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n72_), .O(z3));
  nand2  g60(.a(new_n33_), .b(new_n21_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(x6), .O(new_n79_));
  and2   g62(.a(new_n73_), .b(x1), .O(new_n80_));
  nand2  g63(.a(new_n51_), .b(new_n20_), .O(new_n81_));
  nand2  g64(.a(x4), .b(new_n32_), .O(new_n82_));
  aoi21  g65(.a(new_n81_), .b(new_n70_), .c(new_n82_), .O(new_n83_));
  aoi21  g66(.a(new_n80_), .b(new_n79_), .c(new_n83_), .O(z4));
  inv1   g67(.a(x3), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x2), .O(new_n86_));
  nand3  g69(.a(x3), .b(new_n43_), .c(x1), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n86_), .c(new_n19_), .O(new_n88_));
  nand2  g71(.a(new_n87_), .b(new_n86_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(x0), .O(new_n90_));
  nor2   g73(.a(new_n85_), .b(x2), .O(new_n91_));
  nand2  g74(.a(new_n18_), .b(new_n32_), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(new_n90_), .c(new_n88_), .O(new_n93_));
  inv1   g76(.a(new_n93_), .O(z5));
  inv1   g77(.a(new_n87_), .O(new_n95_));
  nor2   g78(.a(x4), .b(x1), .O(new_n96_));
  inv1   g79(.a(new_n96_), .O(new_n97_));
  nor2   g80(.a(new_n91_), .b(x1), .O(new_n98_));
  oai21  g81(.a(new_n98_), .b(new_n95_), .c(new_n97_), .O(z6));
  inv1   g82(.a(new_n91_), .O(new_n100_));
  aoi21  g83(.a(new_n100_), .b(new_n86_), .c(new_n96_), .O(z7));
  nand2  g84(.a(new_n97_), .b(x3), .O(z8));
  oai21  g85(.a(new_n21_), .b(new_n32_), .c(new_n25_), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(x5), .c(x4), .O(new_n104_));
  nand2  g87(.a(new_n104_), .b(new_n97_), .O(z9));
endmodule


