// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n94_,
    new_n97_, new_n98_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand3  g02(.a(x3), .b(x2), .c(x1), .O(new_n20_));
  nand4  g03(.a(new_n20_), .b(new_n19_), .c(x4), .d(new_n18_), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nor2   g06(.a(x1), .b(x0), .O(new_n24_));
  inv1   g07(.a(x4), .O(new_n25_));
  nand2  g08(.a(x6), .b(new_n25_), .O(new_n26_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n22_), .c(x5), .O(new_n28_));
  nor2   g11(.a(new_n19_), .b(x5), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x4), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n28_), .O(z0));
  oai21  g14(.a(x2), .b(x1), .c(x0), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n25_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  inv1   g17(.a(x1), .O(new_n35_));
  nand2  g18(.a(new_n23_), .b(new_n35_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(x4), .c(x6), .O(new_n37_));
  nand2  g20(.a(new_n20_), .b(new_n18_), .O(new_n38_));
  oai21  g21(.a(x6), .b(x4), .c(new_n38_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n37_), .c(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n34_), .O(z1));
  nand4  g24(.a(new_n23_), .b(new_n25_), .c(new_n35_), .d(new_n18_), .O(new_n42_));
  aoi21  g25(.a(x1), .b(x0), .c(x5), .O(new_n43_));
  oai21  g26(.a(x1), .b(x0), .c(x4), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x6), .O(new_n46_));
  inv1   g29(.a(x3), .O(new_n47_));
  nand3  g30(.a(x6), .b(x5), .c(x4), .O(new_n48_));
  nand3  g31(.a(new_n19_), .b(new_n25_), .c(x1), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand3  g33(.a(x6), .b(x4), .c(x0), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n50_), .c(x2), .O(new_n53_));
  nand2  g36(.a(new_n19_), .b(x0), .O(new_n54_));
  inv1   g37(.a(x5), .O(new_n55_));
  nand2  g38(.a(new_n32_), .b(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g40(.a(new_n19_), .b(new_n55_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n21_), .O(new_n59_));
  aoi21  g42(.a(new_n57_), .b(new_n25_), .c(new_n59_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n53_), .c(new_n46_), .O(z2));
  oai21  g44(.a(x3), .b(x2), .c(x1), .O(new_n62_));
  oai21  g45(.a(x2), .b(x1), .c(x6), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n62_), .c(x0), .O(new_n64_));
  nand2  g47(.a(new_n20_), .b(new_n19_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n36_), .c(new_n18_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g50(.a(new_n24_), .b(new_n23_), .O(new_n68_));
  nand2  g51(.a(new_n32_), .b(new_n68_), .O(new_n69_));
  aoi22  g52(.a(new_n69_), .b(new_n29_), .c(new_n67_), .d(x5), .O(z3));
  inv1   g53(.a(x2), .O(new_n71_));
  nand2  g54(.a(new_n47_), .b(new_n71_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(x0), .O(new_n73_));
  nand2  g56(.a(new_n19_), .b(x5), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n73_), .c(new_n23_), .O(new_n75_));
  nor2   g58(.a(x3), .b(x2), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(new_n18_), .c(new_n23_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n19_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n75_), .c(x1), .O(new_n79_));
  inv1   g62(.a(new_n74_), .O(new_n80_));
  oai21  g63(.a(x3), .b(x0), .c(x2), .O(new_n81_));
  aoi21  g64(.a(new_n81_), .b(new_n19_), .c(x1), .O(new_n82_));
  oai21  g65(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n79_), .O(z4));
  nand2  g67(.a(new_n71_), .b(new_n35_), .O(new_n85_));
  nand4  g68(.a(new_n72_), .b(new_n85_), .c(new_n23_), .d(x0), .O(new_n86_));
  nand3  g69(.a(new_n72_), .b(new_n85_), .c(new_n23_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n18_), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z5));
  nand2  g73(.a(x3), .b(new_n71_), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n35_), .c(new_n58_), .O(new_n92_));
  aoi21  g75(.a(new_n91_), .b(new_n35_), .c(new_n92_), .O(z6));
  nand2  g76(.a(new_n72_), .b(new_n23_), .O(new_n94_));
  aoi21  g77(.a(new_n19_), .b(new_n55_), .c(new_n94_), .O(z7));
  nand2  g78(.a(new_n58_), .b(x3), .O(z8));
  nand3  g79(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n97_));
  nand3  g80(.a(new_n97_), .b(x5), .c(x4), .O(new_n98_));
  nand2  g81(.a(new_n98_), .b(new_n58_), .O(z9));
endmodule


