// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_;
  inv1   g00(.a(x5), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  nand2  g09(.a(new_n18_), .b(x4), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n24_), .O(z0));
  inv1   g11(.a(x4), .O(new_n29_));
  nor2   g12(.a(x3), .b(x2), .O(new_n30_));
  nor3   g13(.a(new_n30_), .b(new_n29_), .c(new_n21_), .O(new_n31_));
  nor2   g14(.a(x2), .b(x1), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n29_), .c(new_n25_), .O(new_n33_));
  aoi21  g16(.a(x6), .b(x4), .c(x0), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x5), .O(new_n35_));
  oai21  g18(.a(new_n33_), .b(new_n31_), .c(new_n35_), .O(new_n36_));
  inv1   g19(.a(new_n23_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n26_), .O(z1));
  nand2  g22(.a(x6), .b(x4), .O(new_n40_));
  nand3  g23(.a(new_n25_), .b(new_n29_), .c(x2), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(new_n21_), .O(new_n42_));
  nand2  g25(.a(x6), .b(x2), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n18_), .c(new_n29_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n42_), .c(x0), .O(new_n45_));
  nand2  g28(.a(new_n25_), .b(x4), .O(new_n46_));
  inv1   g29(.a(x2), .O(new_n47_));
  nand2  g30(.a(x5), .b(x0), .O(new_n48_));
  nor2   g31(.a(new_n25_), .b(x4), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand2  g35(.a(new_n25_), .b(new_n29_), .O(new_n53_));
  nand2  g36(.a(x1), .b(x0), .O(new_n54_));
  nand3  g37(.a(x5), .b(x4), .c(x2), .O(new_n55_));
  oai21  g38(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x3), .O(new_n57_));
  nand4  g40(.a(x6), .b(new_n29_), .c(new_n21_), .d(new_n20_), .O(new_n58_));
  nand3  g41(.a(new_n25_), .b(x4), .c(new_n47_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n58_), .c(x3), .O(new_n60_));
  nand2  g43(.a(x4), .b(x1), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x6), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x5), .O(new_n63_));
  nand2  g46(.a(x6), .b(x5), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n53_), .c(new_n34_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nor2   g49(.a(new_n66_), .b(new_n60_), .O(new_n67_));
  nand4  g50(.a(new_n67_), .b(new_n57_), .c(new_n52_), .d(new_n45_), .O(z2));
  nor2   g51(.a(new_n32_), .b(new_n20_), .O(new_n69_));
  nor2   g52(.a(new_n69_), .b(new_n37_), .O(new_n70_));
  nand2  g53(.a(new_n22_), .b(new_n25_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(x1), .O(new_n72_));
  nand3  g55(.a(x6), .b(x3), .c(x2), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(new_n20_), .O(new_n74_));
  oai21  g57(.a(new_n30_), .b(new_n21_), .c(new_n25_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n69_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n18_), .O(new_n77_));
  oai21  g60(.a(new_n70_), .b(new_n64_), .c(new_n77_), .O(z3));
  aoi21  g61(.a(new_n22_), .b(new_n20_), .c(new_n30_), .O(new_n79_));
  nor2   g62(.a(x6), .b(x5), .O(new_n80_));
  nor2   g63(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g64(.a(new_n79_), .b(new_n25_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x1), .O(new_n83_));
  oai21  g66(.a(x3), .b(x0), .c(x2), .O(new_n84_));
  aoi21  g67(.a(new_n84_), .b(new_n25_), .c(x1), .O(new_n85_));
  oai21  g68(.a(new_n84_), .b(new_n80_), .c(new_n85_), .O(new_n86_));
  oai21  g69(.a(new_n83_), .b(new_n81_), .c(new_n86_), .O(z4));
  inv1   g70(.a(x3), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n47_), .O(new_n89_));
  inv1   g72(.a(new_n32_), .O(new_n90_));
  nand4  g73(.a(new_n90_), .b(new_n89_), .c(new_n22_), .d(x0), .O(new_n91_));
  nand2  g74(.a(new_n89_), .b(new_n22_), .O(new_n92_));
  oai21  g75(.a(new_n92_), .b(new_n32_), .c(new_n20_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n91_), .c(new_n26_), .O(new_n94_));
  inv1   g77(.a(new_n94_), .O(z5));
  nand3  g78(.a(x3), .b(new_n47_), .c(new_n21_), .O(new_n96_));
  oai21  g79(.a(new_n88_), .b(x2), .c(x1), .O(new_n97_));
  nand3  g80(.a(new_n97_), .b(new_n96_), .c(new_n26_), .O(z6));
  nand2  g81(.a(new_n92_), .b(new_n26_), .O(z7));
  nand2  g82(.a(new_n26_), .b(x3), .O(z8));
  nor2   g83(.a(new_n49_), .b(new_n18_), .O(z9));
endmodule


