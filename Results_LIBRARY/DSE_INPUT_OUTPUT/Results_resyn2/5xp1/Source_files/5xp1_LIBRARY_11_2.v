// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  aoi21  g02(.a(x3), .b(x2), .c(x0), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  inv1   g04(.a(x5), .O(new_n22_));
  inv1   g05(.a(x6), .O(new_n23_));
  nor2   g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  oai21  g07(.a(new_n21_), .b(x4), .c(new_n24_), .O(new_n25_));
  xor2a  g08(.a(new_n25_), .b(new_n18_), .O(z0));
  oai21  g09(.a(x2), .b(x1), .c(x0), .O(new_n27_));
  nor2   g10(.a(x5), .b(x4), .O(new_n28_));
  oai21  g11(.a(new_n27_), .b(new_n23_), .c(new_n28_), .O(new_n29_));
  xnor2a g12(.a(new_n29_), .b(new_n25_), .O(z1));
  inv1   g13(.a(x0), .O(new_n31_));
  nand2  g14(.a(new_n22_), .b(new_n31_), .O(new_n32_));
  oai21  g15(.a(x3), .b(x0), .c(x2), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  nand2  g17(.a(x5), .b(x0), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n18_), .O(new_n36_));
  aoi21  g19(.a(new_n34_), .b(new_n32_), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n27_), .b(new_n22_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n21_), .c(x4), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n37_), .c(x6), .O(new_n41_));
  inv1   g24(.a(new_n20_), .O(new_n42_));
  nor2   g25(.a(x3), .b(x2), .O(new_n43_));
  inv1   g26(.a(new_n43_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n42_), .c(x1), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n35_), .O(new_n46_));
  nor2   g29(.a(x6), .b(x4), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n32_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n41_), .O(z2));
  nand2  g32(.a(new_n47_), .b(new_n43_), .O(new_n50_));
  nand2  g33(.a(new_n23_), .b(x4), .O(new_n51_));
  aoi21  g34(.a(x6), .b(x2), .c(x1), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n50_), .c(new_n31_), .O(new_n54_));
  nand3  g37(.a(x6), .b(x1), .c(new_n31_), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n54_), .c(new_n22_), .O(new_n57_));
  nand2  g40(.a(x3), .b(x2), .O(new_n58_));
  oai21  g41(.a(new_n58_), .b(new_n23_), .c(new_n19_), .O(new_n59_));
  aoi21  g42(.a(x3), .b(x2), .c(x6), .O(new_n60_));
  nor2   g43(.a(new_n60_), .b(x0), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  oai21  g45(.a(x6), .b(x3), .c(x1), .O(new_n63_));
  oai21  g46(.a(x6), .b(new_n18_), .c(x5), .O(new_n64_));
  aoi21  g47(.a(new_n63_), .b(x0), .c(new_n64_), .O(new_n65_));
  inv1   g48(.a(x3), .O(new_n66_));
  oai21  g49(.a(new_n32_), .b(new_n66_), .c(new_n35_), .O(new_n67_));
  inv1   g50(.a(x2), .O(new_n68_));
  nand2  g51(.a(new_n18_), .b(x1), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(new_n23_), .c(new_n68_), .O(new_n70_));
  aoi22  g53(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n62_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n57_), .O(z3));
  oai21  g55(.a(new_n43_), .b(new_n31_), .c(new_n60_), .O(new_n73_));
  nand3  g56(.a(new_n44_), .b(new_n42_), .c(x6), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x1), .O(new_n76_));
  nand2  g59(.a(new_n34_), .b(x6), .O(new_n77_));
  oai21  g60(.a(new_n33_), .b(x1), .c(new_n47_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n76_), .O(z4));
  nor2   g63(.a(x2), .b(x1), .O(new_n81_));
  inv1   g64(.a(new_n81_), .O(new_n82_));
  nand4  g65(.a(new_n44_), .b(new_n82_), .c(new_n58_), .d(x0), .O(new_n83_));
  nand2  g66(.a(new_n44_), .b(new_n58_), .O(new_n84_));
  oai21  g67(.a(new_n84_), .b(new_n81_), .c(new_n31_), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n83_), .c(new_n51_), .O(new_n86_));
  inv1   g69(.a(new_n86_), .O(z5));
  inv1   g70(.a(new_n51_), .O(new_n88_));
  oai21  g71(.a(new_n66_), .b(x2), .c(x1), .O(new_n89_));
  nand3  g72(.a(x3), .b(new_n68_), .c(new_n19_), .O(new_n90_));
  aoi21  g73(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(z6));
  nand2  g74(.a(new_n84_), .b(new_n51_), .O(z7));
  nand2  g75(.a(new_n51_), .b(x3), .O(z8));
  aoi21  g76(.a(x6), .b(new_n22_), .c(new_n18_), .O(z9));
endmodule


