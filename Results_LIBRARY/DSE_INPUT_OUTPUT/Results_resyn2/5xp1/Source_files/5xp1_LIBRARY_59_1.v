// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n87_, new_n88_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nor2   g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  inv1   g05(.a(x0), .O(new_n23_));
  nand2  g06(.a(x6), .b(new_n23_), .O(new_n24_));
  nand3  g07(.a(x3), .b(x2), .c(x1), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x5), .c(x4), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n24_), .O(z9));
  aoi21  g11(.a(new_n22_), .b(new_n18_), .c(z9), .O(z0));
  oai21  g12(.a(x3), .b(x2), .c(x1), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n20_), .O(new_n31_));
  nor2   g14(.a(x2), .b(x1), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n20_), .c(new_n18_), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n31_), .c(x0), .O(new_n34_));
  nand2  g17(.a(new_n27_), .b(new_n22_), .O(new_n35_));
  aoi21  g18(.a(new_n34_), .b(new_n19_), .c(new_n35_), .O(z1));
  nand2  g19(.a(new_n26_), .b(x5), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x1), .c(x0), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n37_), .c(new_n18_), .O(new_n42_));
  aoi21  g25(.a(new_n25_), .b(new_n23_), .c(new_n19_), .O(new_n43_));
  inv1   g26(.a(x1), .O(new_n44_));
  oai21  g27(.a(x3), .b(x2), .c(x0), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n44_), .c(new_n18_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(new_n20_), .O(new_n47_));
  nand3  g30(.a(new_n32_), .b(new_n19_), .c(x4), .O(new_n48_));
  nand2  g31(.a(new_n32_), .b(new_n19_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n18_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(new_n48_), .c(x6), .d(x0), .O(new_n51_));
  oai21  g34(.a(new_n47_), .b(new_n42_), .c(new_n51_), .O(z2));
  aoi21  g35(.a(new_n45_), .b(new_n20_), .c(new_n19_), .O(new_n53_));
  nand4  g36(.a(new_n19_), .b(x3), .c(x2), .d(new_n23_), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n53_), .c(x1), .O(new_n56_));
  nor2   g39(.a(new_n19_), .b(x0), .O(new_n57_));
  nand4  g40(.a(new_n20_), .b(new_n19_), .c(new_n38_), .d(x0), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n57_), .c(new_n39_), .O(new_n60_));
  nand2  g43(.a(x6), .b(x2), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n19_), .c(x0), .O(new_n62_));
  inv1   g45(.a(new_n62_), .O(new_n63_));
  oai21  g46(.a(new_n63_), .b(new_n57_), .c(new_n44_), .O(new_n64_));
  oai21  g47(.a(new_n19_), .b(x2), .c(new_n20_), .O(new_n65_));
  aoi22  g48(.a(new_n65_), .b(new_n23_), .c(new_n21_), .d(x2), .O(new_n66_));
  nand4  g49(.a(new_n66_), .b(new_n64_), .c(new_n60_), .d(new_n56_), .O(z3));
  nand2  g50(.a(new_n31_), .b(x0), .O(new_n68_));
  nand3  g51(.a(new_n39_), .b(new_n44_), .c(new_n23_), .O(new_n69_));
  inv1   g52(.a(new_n25_), .O(new_n70_));
  nor2   g53(.a(new_n32_), .b(new_n70_), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(new_n72_));
  inv1   g55(.a(new_n32_), .O(new_n73_));
  nand4  g56(.a(new_n73_), .b(new_n30_), .c(x6), .d(x0), .O(new_n74_));
  inv1   g57(.a(new_n74_), .O(new_n75_));
  aoi21  g58(.a(new_n72_), .b(new_n20_), .c(new_n75_), .O(z4));
  oai21  g59(.a(new_n39_), .b(new_n44_), .c(new_n23_), .O(new_n77_));
  nand3  g60(.a(x3), .b(x1), .c(x0), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n77_), .c(new_n38_), .O(new_n79_));
  oai21  g62(.a(x3), .b(new_n38_), .c(new_n20_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n23_), .O(new_n81_));
  nand3  g64(.a(x3), .b(x2), .c(x0), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(z5));
  nand3  g66(.a(x3), .b(new_n38_), .c(new_n44_), .O(new_n84_));
  oai21  g67(.a(new_n39_), .b(x2), .c(x1), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n84_), .c(new_n24_), .O(z6));
  nand2  g69(.a(x3), .b(x2), .O(new_n87_));
  nand3  g70(.a(new_n40_), .b(new_n87_), .c(new_n24_), .O(new_n88_));
  inv1   g71(.a(new_n88_), .O(z7));
  aoi21  g72(.a(x6), .b(new_n23_), .c(x3), .O(z8));
endmodule


