// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n89_, new_n92_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x5), .O(new_n21_));
  nand3  g04(.a(x3), .b(x2), .c(x1), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(new_n23_));
  inv1   g06(.a(x4), .O(new_n24_));
  inv1   g07(.a(x1), .O(new_n25_));
  nand2  g08(.a(x3), .b(x2), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n20_), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(x6), .c(x5), .d(new_n24_), .O(new_n28_));
  oai21  g11(.a(new_n23_), .b(new_n19_), .c(new_n28_), .O(z0));
  nand2  g12(.a(x6), .b(x4), .O(new_n30_));
  nor2   g13(.a(x6), .b(x4), .O(new_n31_));
  inv1   g14(.a(new_n31_), .O(new_n32_));
  aoi21  g15(.a(new_n26_), .b(new_n25_), .c(x4), .O(new_n33_));
  nand3  g16(.a(new_n30_), .b(new_n22_), .c(new_n20_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x5), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  oai21  g20(.a(x6), .b(x3), .c(x1), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g22(.a(new_n24_), .b(new_n25_), .c(new_n18_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n21_), .d(x0), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n36_), .c(new_n30_), .O(z1));
  nand2  g25(.a(x5), .b(x0), .O(new_n43_));
  oai21  g26(.a(x3), .b(x2), .c(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n21_), .O(new_n45_));
  nand2  g28(.a(new_n44_), .b(new_n26_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n45_), .c(x1), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n31_), .O(new_n49_));
  inv1   g32(.a(new_n22_), .O(new_n50_));
  oai21  g33(.a(new_n33_), .b(new_n50_), .c(x5), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n32_), .c(new_n20_), .O(new_n52_));
  inv1   g35(.a(new_n30_), .O(new_n53_));
  nor2   g36(.a(x3), .b(x2), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n25_), .c(x4), .O(new_n55_));
  nand3  g38(.a(x6), .b(new_n37_), .c(new_n25_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n21_), .c(new_n53_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n52_), .c(new_n49_), .O(z2));
  aoi21  g42(.a(new_n18_), .b(new_n25_), .c(new_n37_), .O(new_n60_));
  nand2  g43(.a(new_n38_), .b(x0), .O(new_n61_));
  nand3  g44(.a(x6), .b(x1), .c(new_n20_), .O(new_n62_));
  oai21  g45(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n21_), .O(new_n64_));
  aoi21  g47(.a(new_n26_), .b(new_n25_), .c(x0), .O(new_n65_));
  oai21  g48(.a(new_n50_), .b(x6), .c(new_n65_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n61_), .c(x5), .O(new_n67_));
  nand3  g50(.a(new_n21_), .b(x3), .c(new_n20_), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n43_), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(new_n60_), .c(new_n53_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z3));
  nand2  g54(.a(x6), .b(new_n24_), .O(new_n72_));
  aoi21  g55(.a(new_n46_), .b(new_n72_), .c(new_n25_), .O(new_n73_));
  oai21  g56(.a(new_n46_), .b(new_n18_), .c(new_n73_), .O(new_n74_));
  oai21  g57(.a(x3), .b(x0), .c(x2), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  or2    g59(.a(new_n75_), .b(new_n18_), .O(new_n77_));
  nand3  g60(.a(new_n77_), .b(new_n76_), .c(new_n25_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n74_), .O(z4));
  inv1   g62(.a(x3), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n25_), .c(new_n37_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n26_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x0), .O(new_n83_));
  nand3  g66(.a(new_n81_), .b(new_n26_), .c(new_n20_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n83_), .c(new_n30_), .O(z5));
  oai21  g68(.a(new_n80_), .b(x2), .c(x1), .O(new_n86_));
  nand3  g69(.a(x3), .b(new_n37_), .c(new_n25_), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n86_), .c(new_n30_), .O(z6));
  nand2  g71(.a(new_n30_), .b(new_n26_), .O(new_n89_));
  nor2   g72(.a(new_n89_), .b(new_n54_), .O(z7));
  nand2  g73(.a(new_n30_), .b(x3), .O(z8));
  nand3  g74(.a(new_n23_), .b(new_n18_), .c(x4), .O(new_n92_));
  inv1   g75(.a(new_n92_), .O(z9));
endmodule


