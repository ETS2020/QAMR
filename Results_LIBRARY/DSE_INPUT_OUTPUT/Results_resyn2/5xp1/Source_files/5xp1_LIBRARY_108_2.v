// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n78_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  inv1   g03(.a(x6), .O(new_n21_));
  aoi21  g04(.a(new_n21_), .b(new_n19_), .c(new_n20_), .O(new_n22_));
  aoi21  g05(.a(x5), .b(new_n18_), .c(new_n21_), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x6), .c(x1), .O(new_n26_));
  oai22  g09(.a(new_n26_), .b(new_n23_), .c(new_n22_), .d(new_n18_), .O(z0));
  aoi21  g10(.a(x6), .b(x2), .c(x1), .O(new_n28_));
  oai22  g11(.a(new_n28_), .b(new_n19_), .c(new_n21_), .d(new_n18_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  inv1   g13(.a(x1), .O(new_n31_));
  nand3  g14(.a(new_n24_), .b(new_n31_), .c(new_n19_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x6), .O(new_n33_));
  oai21  g16(.a(x6), .b(x0), .c(x4), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(x5), .O(new_n35_));
  nand2  g18(.a(new_n21_), .b(x1), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n35_), .c(new_n30_), .O(z1));
  nand2  g20(.a(new_n20_), .b(new_n19_), .O(new_n38_));
  oai21  g21(.a(x3), .b(x0), .c(x2), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  nand2  g23(.a(x5), .b(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n42_), .O(new_n43_));
  oai21  g26(.a(x2), .b(x1), .c(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n32_), .c(x4), .O(new_n46_));
  inv1   g29(.a(new_n46_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n43_), .c(x6), .O(new_n48_));
  inv1   g31(.a(new_n41_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x4), .O(new_n50_));
  nor2   g33(.a(x6), .b(x1), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n42_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n48_), .O(z2));
  nor2   g36(.a(x2), .b(x1), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  aoi21  g38(.a(new_n24_), .b(new_n31_), .c(new_n38_), .O(new_n56_));
  aoi21  g39(.a(new_n55_), .b(new_n49_), .c(new_n56_), .O(new_n57_));
  inv1   g40(.a(x3), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n19_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(x6), .c(x2), .O(new_n60_));
  nand4  g43(.a(new_n60_), .b(new_n41_), .c(new_n38_), .d(new_n31_), .O(new_n61_));
  oai21  g44(.a(new_n57_), .b(new_n21_), .c(new_n61_), .O(z3));
  nor2   g45(.a(new_n21_), .b(new_n31_), .O(new_n63_));
  nor2   g46(.a(new_n63_), .b(new_n51_), .O(new_n64_));
  nor2   g47(.a(new_n31_), .b(new_n19_), .O(new_n65_));
  aoi22  g48(.a(new_n65_), .b(x3), .c(new_n39_), .d(new_n31_), .O(new_n66_));
  oai22  g49(.a(new_n66_), .b(new_n21_), .c(new_n64_), .d(new_n39_), .O(z4));
  inv1   g50(.a(x2), .O(new_n68_));
  nand2  g51(.a(new_n58_), .b(new_n68_), .O(new_n69_));
  nand4  g52(.a(new_n69_), .b(new_n55_), .c(new_n24_), .d(x0), .O(new_n70_));
  nand2  g53(.a(new_n69_), .b(new_n24_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n54_), .c(new_n19_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n70_), .c(new_n36_), .O(new_n73_));
  inv1   g56(.a(new_n73_), .O(z5));
  nand2  g57(.a(x3), .b(new_n68_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x1), .O(new_n76_));
  oai21  g59(.a(new_n75_), .b(new_n63_), .c(new_n76_), .O(z6));
  inv1   g60(.a(new_n36_), .O(new_n78_));
  nor2   g61(.a(new_n71_), .b(new_n78_), .O(z7));
  nor2   g62(.a(new_n78_), .b(x3), .O(z8));
  oai21  g63(.a(new_n34_), .b(new_n20_), .c(new_n36_), .O(z9));
endmodule


