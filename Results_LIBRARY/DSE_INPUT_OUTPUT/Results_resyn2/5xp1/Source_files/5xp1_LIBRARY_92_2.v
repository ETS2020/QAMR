// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x4), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  nand2  g04(.a(x5), .b(new_n21_), .O(new_n22_));
  inv1   g05(.a(x0), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  nand2  g07(.a(x3), .b(x2), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x6), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n22_), .c(new_n20_), .O(z0));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n21_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x6), .c(new_n18_), .O(new_n31_));
  nand3  g14(.a(new_n27_), .b(x5), .c(new_n21_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(z1));
  oai21  g16(.a(x3), .b(x2), .c(x0), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  nand2  g18(.a(new_n34_), .b(new_n25_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n35_), .c(x1), .O(new_n37_));
  aoi21  g20(.a(x5), .b(x0), .c(x6), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g22(.a(new_n29_), .b(new_n18_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n26_), .c(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(x4), .O(new_n42_));
  nand4  g25(.a(new_n40_), .b(new_n26_), .c(x6), .d(new_n21_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n42_), .c(new_n39_), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(z2));
  oai21  g28(.a(new_n25_), .b(new_n24_), .c(new_n19_), .O(new_n46_));
  aoi21  g29(.a(new_n25_), .b(new_n24_), .c(x0), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g31(.a(x6), .b(x3), .c(x1), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n19_), .b(x4), .O(new_n51_));
  nand4  g34(.a(new_n51_), .b(new_n50_), .c(new_n48_), .d(x5), .O(new_n52_));
  nand2  g35(.a(new_n24_), .b(new_n23_), .O(new_n53_));
  nand3  g36(.a(new_n29_), .b(new_n53_), .c(x6), .O(new_n54_));
  oai21  g37(.a(x3), .b(x2), .c(x1), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(new_n19_), .c(new_n21_), .d(x0), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  nand3  g41(.a(new_n18_), .b(x3), .c(new_n23_), .O(new_n59_));
  oai21  g42(.a(new_n18_), .b(new_n23_), .c(new_n59_), .O(new_n60_));
  oai21  g43(.a(x4), .b(new_n24_), .c(new_n19_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n60_), .c(x2), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n58_), .c(new_n52_), .O(z3));
  nand2  g46(.a(new_n36_), .b(new_n19_), .O(new_n64_));
  nand3  g47(.a(new_n34_), .b(new_n25_), .c(x6), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n64_), .c(x1), .O(new_n66_));
  oai21  g49(.a(x3), .b(x0), .c(x2), .O(new_n67_));
  aoi21  g50(.a(new_n67_), .b(new_n19_), .c(x1), .O(new_n68_));
  oai21  g51(.a(new_n67_), .b(new_n19_), .c(new_n68_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n66_), .c(new_n51_), .O(z4));
  inv1   g53(.a(x2), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n24_), .O(new_n72_));
  inv1   g55(.a(x3), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand4  g57(.a(new_n74_), .b(new_n72_), .c(new_n25_), .d(x0), .O(new_n75_));
  nand3  g58(.a(new_n74_), .b(new_n72_), .c(new_n25_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n23_), .O(new_n77_));
  nand3  g60(.a(new_n77_), .b(new_n75_), .c(new_n51_), .O(new_n78_));
  inv1   g61(.a(new_n78_), .O(z5));
  nand3  g62(.a(x3), .b(new_n71_), .c(new_n24_), .O(new_n80_));
  oai21  g63(.a(new_n73_), .b(x2), .c(x1), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n80_), .c(new_n51_), .O(z6));
  nand2  g65(.a(new_n74_), .b(new_n25_), .O(new_n83_));
  aoi21  g66(.a(new_n19_), .b(x4), .c(new_n83_), .O(z7));
  nand2  g67(.a(new_n51_), .b(x3), .O(z8));
  aoi21  g68(.a(x6), .b(new_n18_), .c(new_n21_), .O(z9));
endmodule


