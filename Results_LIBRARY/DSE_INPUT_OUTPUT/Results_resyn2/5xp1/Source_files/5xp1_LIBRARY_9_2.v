// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_;
  inv1   g00(.a(x4), .O(new_n18_));
  nand2  g01(.a(x5), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  inv1   g07(.a(x5), .O(new_n25_));
  nor2   g08(.a(x6), .b(new_n25_), .O(new_n26_));
  aoi21  g09(.a(new_n25_), .b(x4), .c(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n24_), .b(new_n19_), .c(new_n27_), .O(z0));
  inv1   g11(.a(x2), .O(new_n29_));
  oai21  g12(.a(x6), .b(x3), .c(x1), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g14(.a(x6), .O(new_n32_));
  nand2  g15(.a(x4), .b(x1), .O(new_n33_));
  aoi21  g16(.a(new_n33_), .b(new_n32_), .c(new_n20_), .O(new_n34_));
  aoi22  g17(.a(new_n34_), .b(new_n31_), .c(x6), .d(x4), .O(new_n35_));
  inv1   g18(.a(new_n19_), .O(new_n36_));
  aoi21  g19(.a(new_n24_), .b(new_n36_), .c(new_n26_), .O(new_n37_));
  oai21  g20(.a(new_n35_), .b(x5), .c(new_n37_), .O(z1));
  nand2  g21(.a(x6), .b(x4), .O(new_n39_));
  nand3  g22(.a(new_n32_), .b(new_n18_), .c(x2), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n39_), .c(new_n21_), .O(new_n41_));
  nand2  g24(.a(x6), .b(x2), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n25_), .c(new_n18_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n41_), .c(x0), .O(new_n44_));
  inv1   g27(.a(x3), .O(new_n45_));
  nand4  g28(.a(new_n32_), .b(new_n18_), .c(x1), .d(x0), .O(new_n46_));
  nand3  g29(.a(x5), .b(x4), .c(x2), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nand2  g31(.a(new_n33_), .b(x6), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x5), .O(new_n50_));
  xor2a  g33(.a(x6), .b(x4), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n19_), .c(new_n20_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nor2   g36(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand4  g37(.a(x6), .b(new_n18_), .c(new_n21_), .d(new_n20_), .O(new_n55_));
  nand3  g38(.a(new_n32_), .b(x4), .c(new_n29_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(x3), .O(new_n57_));
  nand2  g40(.a(x5), .b(x0), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n29_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n18_), .O(new_n60_));
  and2   g43(.a(new_n51_), .b(new_n21_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n60_), .c(new_n57_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n54_), .c(new_n44_), .O(z2));
  nand2  g46(.a(new_n25_), .b(new_n29_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(x6), .c(x1), .O(new_n65_));
  nand3  g48(.a(new_n32_), .b(new_n45_), .c(new_n29_), .O(new_n66_));
  oai21  g49(.a(x2), .b(x1), .c(x5), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g51(.a(new_n68_), .b(new_n65_), .c(x0), .O(new_n69_));
  nand2  g52(.a(new_n22_), .b(new_n21_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(x6), .c(x5), .O(new_n71_));
  nand2  g54(.a(new_n70_), .b(x6), .O(new_n72_));
  nand3  g55(.a(x3), .b(x2), .c(x1), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(new_n25_), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n71_), .c(new_n20_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n69_), .O(z3));
  nand2  g59(.a(new_n22_), .b(new_n20_), .O(new_n77_));
  nand2  g60(.a(new_n45_), .b(new_n29_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x6), .O(new_n80_));
  nand4  g63(.a(new_n78_), .b(new_n58_), .c(new_n77_), .d(new_n32_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n80_), .c(x1), .O(new_n82_));
  nand2  g65(.a(new_n26_), .b(new_n20_), .O(new_n83_));
  oai21  g66(.a(x3), .b(x0), .c(x2), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x6), .O(new_n85_));
  nand3  g68(.a(new_n64_), .b(new_n42_), .c(new_n77_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n21_), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n83_), .c(new_n82_), .O(z4));
  inv1   g72(.a(new_n26_), .O(new_n90_));
  nand2  g73(.a(new_n45_), .b(x2), .O(new_n91_));
  nand3  g74(.a(x3), .b(new_n29_), .c(x1), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(new_n91_), .c(new_n20_), .O(new_n93_));
  nand2  g76(.a(new_n92_), .b(new_n91_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(x0), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(new_n93_), .c(new_n90_), .O(new_n96_));
  inv1   g79(.a(new_n96_), .O(z5));
  inv1   g80(.a(new_n92_), .O(new_n98_));
  aoi21  g81(.a(x3), .b(new_n29_), .c(x1), .O(new_n99_));
  oai21  g82(.a(new_n99_), .b(new_n98_), .c(new_n90_), .O(z6));
  nand3  g83(.a(new_n78_), .b(new_n90_), .c(new_n22_), .O(new_n101_));
  inv1   g84(.a(new_n101_), .O(z7));
  nor2   g85(.a(new_n26_), .b(x3), .O(z8));
  aoi21  g86(.a(x6), .b(new_n18_), .c(new_n25_), .O(z9));
endmodule


