// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  or2    g05(.a(new_n22_), .b(x6), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(x5), .c(new_n18_), .O(z0));
  inv1   g07(.a(x5), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand2  g09(.a(new_n22_), .b(x4), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(z9));
  nor2   g11(.a(x2), .b(x1), .O(new_n29_));
  nor2   g12(.a(x3), .b(x2), .O(new_n30_));
  nand2  g13(.a(x4), .b(x1), .O(new_n31_));
  oai22  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .d(new_n26_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x0), .O(new_n33_));
  aoi21  g16(.a(x6), .b(x4), .c(x5), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n33_), .c(z9), .O(z1));
  nand2  g18(.a(new_n26_), .b(x0), .O(new_n36_));
  nand3  g19(.a(x5), .b(x3), .c(x2), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n30_), .c(new_n37_), .O(new_n38_));
  oai21  g21(.a(x2), .b(x1), .c(x0), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x6), .O(new_n40_));
  nand2  g23(.a(x5), .b(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g25(.a(new_n38_), .b(x1), .c(new_n42_), .O(new_n43_));
  inv1   g26(.a(x2), .O(new_n44_));
  oai21  g27(.a(x5), .b(x3), .c(x0), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g29(.a(x3), .O(new_n47_));
  oai21  g30(.a(new_n25_), .b(new_n47_), .c(new_n19_), .O(new_n48_));
  aoi21  g31(.a(new_n41_), .b(new_n20_), .c(x6), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(new_n50_));
  aoi21  g33(.a(new_n39_), .b(x6), .c(new_n18_), .O(new_n51_));
  nor2   g34(.a(new_n26_), .b(new_n25_), .O(new_n52_));
  aoi21  g35(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  oai21  g36(.a(new_n43_), .b(x4), .c(new_n53_), .O(z2));
  nor2   g37(.a(new_n41_), .b(new_n30_), .O(new_n55_));
  nand3  g38(.a(new_n25_), .b(x3), .c(x2), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n26_), .c(x0), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(x1), .O(new_n58_));
  aoi21  g41(.a(x5), .b(new_n19_), .c(new_n44_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(x3), .c(x1), .O(new_n60_));
  inv1   g43(.a(new_n41_), .O(new_n61_));
  aoi21  g44(.a(new_n36_), .b(new_n25_), .c(new_n61_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  oai21  g46(.a(new_n21_), .b(x0), .c(new_n25_), .O(new_n64_));
  oai21  g47(.a(x1), .b(new_n19_), .c(new_n25_), .O(new_n65_));
  nor2   g48(.a(new_n61_), .b(x2), .O(new_n66_));
  aoi22  g49(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(x6), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n63_), .c(new_n58_), .O(z3));
  aoi21  g51(.a(new_n21_), .b(new_n19_), .c(new_n30_), .O(new_n69_));
  nor2   g52(.a(new_n69_), .b(new_n26_), .O(new_n70_));
  nand2  g53(.a(x6), .b(new_n25_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(x1), .O(new_n73_));
  oai21  g56(.a(x3), .b(x0), .c(x2), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  or2    g58(.a(new_n74_), .b(new_n26_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n75_), .c(new_n20_), .O(new_n77_));
  oai21  g60(.a(new_n73_), .b(new_n70_), .c(new_n77_), .O(z4));
  inv1   g61(.a(new_n52_), .O(new_n79_));
  inv1   g62(.a(new_n29_), .O(new_n80_));
  nand2  g63(.a(new_n47_), .b(new_n44_), .O(new_n81_));
  nand4  g64(.a(new_n81_), .b(new_n80_), .c(new_n21_), .d(x0), .O(new_n82_));
  nand2  g65(.a(new_n81_), .b(new_n21_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n29_), .c(new_n19_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z5));
  nand2  g69(.a(x3), .b(new_n44_), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n20_), .c(new_n79_), .O(new_n88_));
  aoi21  g71(.a(new_n87_), .b(new_n20_), .c(new_n88_), .O(z6));
  nor2   g72(.a(new_n83_), .b(new_n52_), .O(z7));
  nand2  g73(.a(new_n79_), .b(x3), .O(z8));
endmodule


