// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x5), .O(new_n18_));
  nand2  g01(.a(x6), .b(x0), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(new_n20_));
  aoi21  g03(.a(new_n18_), .b(x4), .c(new_n20_), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  inv1   g05(.a(x6), .O(new_n23_));
  nand3  g06(.a(x3), .b(x2), .c(x1), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x4), .O(new_n26_));
  inv1   g09(.a(x4), .O(new_n27_));
  aoi21  g10(.a(x3), .b(x2), .c(x1), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x6), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n26_), .c(x5), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n21_), .O(z0));
  oai21  g16(.a(x3), .b(x2), .c(x1), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  nor2   g18(.a(new_n35_), .b(x6), .O(new_n36_));
  nand3  g19(.a(new_n30_), .b(new_n26_), .c(x5), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n21_), .c(new_n37_), .O(z1));
  inv1   g21(.a(x1), .O(new_n39_));
  oai21  g22(.a(x3), .b(x2), .c(x0), .O(new_n40_));
  nand4  g23(.a(new_n23_), .b(x5), .c(x3), .d(x2), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  oai21  g25(.a(new_n28_), .b(new_n18_), .c(x6), .O(new_n43_));
  nand2  g26(.a(x5), .b(x0), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n42_), .c(new_n27_), .O(new_n46_));
  aoi21  g29(.a(new_n24_), .b(new_n22_), .c(new_n18_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n35_), .c(new_n23_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n43_), .c(x4), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(new_n19_), .O(z2));
  oai21  g33(.a(new_n28_), .b(new_n23_), .c(new_n24_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x5), .O(new_n52_));
  nand3  g35(.a(new_n30_), .b(new_n24_), .c(new_n18_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n22_), .O(new_n54_));
  nand2  g37(.a(new_n34_), .b(x5), .O(new_n55_));
  nand2  g38(.a(new_n23_), .b(x0), .O(new_n56_));
  nor2   g39(.a(new_n34_), .b(x5), .O(new_n57_));
  nor2   g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n54_), .O(z3));
  xor2a  g43(.a(x6), .b(x1), .O(new_n61_));
  inv1   g44(.a(x3), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n19_), .c(x2), .O(new_n64_));
  or2    g47(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand2  g48(.a(x3), .b(x2), .O(new_n66_));
  nand3  g49(.a(new_n61_), .b(new_n66_), .c(new_n22_), .O(new_n67_));
  nor2   g50(.a(x3), .b(x2), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n23_), .c(x1), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(z4));
  nand2  g53(.a(new_n66_), .b(new_n22_), .O(new_n71_));
  aoi21  g54(.a(x3), .b(x1), .c(x2), .O(new_n72_));
  inv1   g55(.a(new_n66_), .O(new_n73_));
  nor2   g56(.a(new_n72_), .b(new_n73_), .O(new_n74_));
  oai22  g57(.a(new_n74_), .b(new_n56_), .c(new_n72_), .d(new_n71_), .O(z5));
  oai21  g58(.a(new_n62_), .b(x2), .c(x1), .O(new_n76_));
  inv1   g59(.a(x2), .O(new_n77_));
  nand3  g60(.a(x3), .b(new_n77_), .c(new_n39_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n76_), .c(new_n19_), .O(z6));
  nor3   g62(.a(new_n68_), .b(new_n73_), .c(new_n20_), .O(z7));
  nand2  g63(.a(new_n19_), .b(x3), .O(z8));
  oai21  g64(.a(new_n26_), .b(new_n18_), .c(new_n19_), .O(z9));
endmodule


