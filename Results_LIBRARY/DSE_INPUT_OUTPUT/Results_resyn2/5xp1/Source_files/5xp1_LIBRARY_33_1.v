// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n94_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x4), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n19_), .c(new_n20_), .O(new_n25_));
  nand3  g08(.a(x3), .b(x2), .c(x1), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n18_), .c(new_n21_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(x5), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x4), .O(new_n29_));
  oai21  g12(.a(new_n25_), .b(new_n18_), .c(new_n29_), .O(z0));
  nand4  g13(.a(new_n26_), .b(new_n18_), .c(x5), .d(new_n21_), .O(new_n31_));
  inv1   g14(.a(x2), .O(new_n32_));
  inv1   g15(.a(x3), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(x1), .c(x0), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(x5), .c(new_n31_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x4), .O(new_n37_));
  nand2  g20(.a(new_n24_), .b(x6), .O(new_n38_));
  nor2   g21(.a(new_n20_), .b(x4), .O(new_n39_));
  nor2   g22(.a(new_n18_), .b(x5), .O(new_n40_));
  aoi21  g23(.a(new_n39_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n37_), .O(z1));
  oai21  g25(.a(x3), .b(x2), .c(x1), .O(new_n43_));
  nor2   g26(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  aoi21  g27(.a(new_n26_), .b(new_n21_), .c(new_n20_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n44_), .c(x4), .O(new_n46_));
  nand2  g29(.a(new_n26_), .b(new_n21_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x5), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n35_), .c(new_n19_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(new_n18_), .O(new_n50_));
  inv1   g33(.a(new_n24_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x4), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n25_), .c(x6), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n50_), .O(z2));
  inv1   g37(.a(new_n23_), .O(new_n55_));
  nor2   g38(.a(new_n55_), .b(x1), .O(new_n56_));
  and2   g39(.a(new_n26_), .b(new_n18_), .O(new_n57_));
  nor2   g40(.a(new_n20_), .b(x0), .O(new_n58_));
  oai21  g41(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nor2   g42(.a(x5), .b(new_n21_), .O(new_n60_));
  aoi21  g43(.a(x2), .b(x0), .c(x3), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  oai21  g45(.a(new_n23_), .b(x5), .c(new_n21_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n62_), .c(x1), .O(new_n64_));
  nand2  g47(.a(x2), .b(x0), .O(new_n65_));
  aoi21  g48(.a(new_n65_), .b(x5), .c(new_n18_), .O(new_n66_));
  aoi21  g49(.a(new_n60_), .b(new_n43_), .c(new_n66_), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n64_), .c(new_n59_), .O(z3));
  nor2   g51(.a(x3), .b(x2), .O(new_n69_));
  oai21  g52(.a(new_n69_), .b(new_n21_), .c(new_n23_), .O(new_n70_));
  nand2  g53(.a(x6), .b(x5), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g55(.a(new_n34_), .b(x0), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n23_), .c(x6), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n72_), .c(x1), .O(new_n75_));
  oai21  g58(.a(x3), .b(x0), .c(x2), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  or2    g60(.a(new_n76_), .b(new_n18_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n77_), .c(new_n22_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n75_), .O(z4));
  inv1   g63(.a(new_n40_), .O(new_n81_));
  nand2  g64(.a(new_n33_), .b(x2), .O(new_n82_));
  nand3  g65(.a(x3), .b(new_n32_), .c(x1), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n82_), .c(new_n21_), .O(new_n84_));
  nand2  g67(.a(new_n83_), .b(new_n82_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x0), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n84_), .c(new_n81_), .O(new_n87_));
  inv1   g70(.a(new_n87_), .O(z5));
  inv1   g71(.a(new_n83_), .O(new_n89_));
  aoi21  g72(.a(x3), .b(new_n32_), .c(x1), .O(new_n90_));
  nor3   g73(.a(new_n90_), .b(new_n89_), .c(new_n40_), .O(z6));
  nor3   g74(.a(new_n40_), .b(new_n69_), .c(new_n55_), .O(z7));
  nand2  g75(.a(new_n81_), .b(x3), .O(z8));
  nand3  g76(.a(new_n27_), .b(x5), .c(x4), .O(new_n94_));
  inv1   g77(.a(new_n94_), .O(z9));
endmodule


