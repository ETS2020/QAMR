// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n91_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x4), .O(new_n19_));
  aoi21  g02(.a(x3), .b(x2), .c(x0), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  xor2a  g04(.a(x5), .b(x4), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(x6), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(z0));
  nand2  g07(.a(new_n21_), .b(x6), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  inv1   g09(.a(x5), .O(new_n27_));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  nand2  g11(.a(x6), .b(x0), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n28_), .c(new_n19_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n26_), .O(z1));
  inv1   g15(.a(x0), .O(new_n33_));
  nand2  g16(.a(new_n27_), .b(new_n33_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x4), .O(new_n35_));
  inv1   g18(.a(x6), .O(new_n36_));
  nand2  g19(.a(x3), .b(x2), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  inv1   g21(.a(x2), .O(new_n39_));
  inv1   g22(.a(x3), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n34_), .c(new_n38_), .d(new_n36_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x1), .O(new_n44_));
  oai21  g27(.a(x3), .b(x0), .c(x2), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  nand2  g30(.a(x5), .b(x0), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n47_), .c(x6), .O(new_n51_));
  nor2   g34(.a(x5), .b(x0), .O(new_n52_));
  nor2   g35(.a(new_n52_), .b(new_n19_), .O(new_n53_));
  aoi21  g36(.a(new_n27_), .b(new_n39_), .c(new_n20_), .O(new_n54_));
  aoi22  g37(.a(new_n54_), .b(new_n53_), .c(new_n49_), .d(new_n36_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n51_), .c(new_n44_), .O(z2));
  nand2  g39(.a(new_n48_), .b(new_n34_), .O(new_n57_));
  aoi21  g40(.a(new_n45_), .b(new_n18_), .c(new_n36_), .O(new_n58_));
  nand2  g41(.a(x3), .b(x0), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n45_), .c(new_n18_), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  nand2  g44(.a(new_n20_), .b(x5), .O(new_n62_));
  nand4  g45(.a(new_n27_), .b(new_n40_), .c(new_n39_), .d(x0), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n62_), .c(new_n19_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand2  g48(.a(new_n40_), .b(new_n33_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(x6), .c(x2), .O(new_n67_));
  nand4  g50(.a(new_n67_), .b(new_n48_), .c(new_n34_), .d(new_n18_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n65_), .c(new_n61_), .O(z3));
  nor2   g52(.a(x6), .b(x4), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n59_), .c(new_n45_), .O(new_n71_));
  nand3  g54(.a(new_n41_), .b(new_n38_), .c(x6), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(x1), .O(new_n74_));
  aoi21  g57(.a(new_n45_), .b(new_n36_), .c(x1), .O(new_n75_));
  oai21  g58(.a(new_n70_), .b(new_n45_), .c(new_n75_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n74_), .O(z4));
  oai21  g60(.a(new_n40_), .b(new_n18_), .c(new_n39_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n37_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x0), .O(new_n80_));
  nor2   g63(.a(x6), .b(new_n19_), .O(new_n81_));
  aoi21  g64(.a(new_n78_), .b(new_n20_), .c(new_n81_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n80_), .O(z5));
  inv1   g66(.a(new_n81_), .O(new_n84_));
  nand3  g67(.a(x3), .b(new_n39_), .c(new_n18_), .O(new_n85_));
  oai21  g68(.a(new_n40_), .b(x2), .c(x1), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z6));
  nand2  g70(.a(new_n41_), .b(new_n37_), .O(new_n88_));
  nor2   g71(.a(new_n88_), .b(new_n81_), .O(z7));
  nand2  g72(.a(new_n84_), .b(x3), .O(z8));
  nand3  g73(.a(x6), .b(x5), .c(x4), .O(new_n91_));
  inv1   g74(.a(new_n91_), .O(z9));
endmodule


