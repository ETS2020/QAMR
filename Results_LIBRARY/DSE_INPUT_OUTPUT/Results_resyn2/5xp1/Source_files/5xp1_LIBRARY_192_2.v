// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_;
  oai21  g00(.a(x6), .b(x0), .c(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x1), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n18_), .c(x4), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  inv1   g05(.a(x1), .O(new_n23_));
  nand2  g06(.a(x3), .b(x2), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n22_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  inv1   g09(.a(x4), .O(new_n27_));
  nand2  g10(.a(x6), .b(new_n27_), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n26_), .c(new_n21_), .O(z0));
  aoi21  g12(.a(new_n24_), .b(new_n22_), .c(new_n19_), .O(new_n30_));
  oai21  g13(.a(x6), .b(x0), .c(x4), .O(new_n31_));
  inv1   g14(.a(x5), .O(new_n32_));
  nor2   g15(.a(new_n32_), .b(x1), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n23_), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(x0), .c(x4), .O(new_n37_));
  nand2  g20(.a(x6), .b(new_n32_), .O(new_n38_));
  oai22  g21(.a(new_n38_), .b(new_n37_), .c(new_n34_), .d(new_n30_), .O(z1));
  nand2  g22(.a(new_n36_), .b(x0), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n26_), .c(x4), .O(new_n41_));
  oai21  g24(.a(x3), .b(x0), .c(x2), .O(new_n42_));
  aoi21  g25(.a(x5), .b(x0), .c(x1), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  aoi21  g27(.a(new_n32_), .b(new_n22_), .c(x4), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n44_), .c(new_n19_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n41_), .O(new_n47_));
  nand2  g30(.a(x5), .b(x0), .O(new_n48_));
  nand2  g31(.a(new_n19_), .b(new_n23_), .O(new_n49_));
  aoi21  g32(.a(new_n48_), .b(new_n27_), .c(new_n49_), .O(new_n50_));
  oai21  g33(.a(new_n48_), .b(new_n27_), .c(new_n50_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n47_), .O(z2));
  xor2a  g35(.a(x5), .b(x0), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  nor2   g37(.a(new_n42_), .b(new_n19_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(x1), .c(new_n54_), .O(new_n56_));
  nor2   g39(.a(x3), .b(x0), .O(new_n57_));
  nand2  g40(.a(new_n36_), .b(x6), .O(new_n58_));
  aoi22  g41(.a(new_n58_), .b(new_n53_), .c(new_n57_), .d(new_n33_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n56_), .O(z3));
  nand3  g43(.a(x3), .b(x1), .c(x0), .O(new_n61_));
  nand2  g44(.a(x6), .b(new_n23_), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n61_), .c(new_n42_), .d(new_n20_), .O(new_n63_));
  inv1   g46(.a(new_n63_), .O(new_n64_));
  aoi21  g47(.a(new_n55_), .b(new_n23_), .c(new_n64_), .O(z4));
  nand2  g48(.a(x3), .b(x1), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n24_), .O(new_n68_));
  inv1   g51(.a(new_n20_), .O(new_n69_));
  aoi21  g52(.a(new_n68_), .b(x0), .c(new_n69_), .O(new_n70_));
  oai21  g53(.a(new_n68_), .b(x0), .c(new_n70_), .O(z5));
  inv1   g54(.a(x3), .O(new_n72_));
  nor2   g55(.a(new_n72_), .b(x2), .O(new_n73_));
  nand2  g56(.a(x6), .b(x1), .O(new_n74_));
  oai22  g57(.a(new_n74_), .b(new_n73_), .c(new_n36_), .d(new_n72_), .O(z6));
  nor2   g58(.a(x3), .b(new_n35_), .O(new_n76_));
  nor2   g59(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nor2   g60(.a(new_n77_), .b(new_n69_), .O(z7));
  nand2  g61(.a(new_n20_), .b(x3), .O(z8));
  oai21  g62(.a(new_n31_), .b(new_n32_), .c(new_n20_), .O(z9));
endmodule


