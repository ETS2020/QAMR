// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n81_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(x5), .c(new_n19_), .O(new_n24_));
  inv1   g07(.a(x5), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x4), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n24_), .c(new_n18_), .O(z0));
  nand2  g10(.a(x5), .b(new_n19_), .O(new_n28_));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x6), .c(new_n25_), .O(new_n31_));
  and2   g14(.a(new_n23_), .b(x6), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n28_), .c(new_n31_), .O(z1));
  nand2  g16(.a(new_n29_), .b(new_n25_), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n23_), .c(x6), .d(new_n19_), .O(new_n35_));
  oai21  g18(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x5), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x1), .c(x0), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n37_), .c(new_n18_), .O(new_n42_));
  nand3  g25(.a(new_n34_), .b(new_n23_), .c(x6), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x4), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n42_), .c(new_n35_), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(z2));
  oai21  g29(.a(x3), .b(x2), .c(x0), .O(new_n47_));
  oai21  g30(.a(x3), .b(x0), .c(x2), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  nand3  g32(.a(new_n47_), .b(new_n22_), .c(new_n18_), .O(new_n50_));
  xnor2a g33(.a(x5), .b(x0), .O(new_n51_));
  nand2  g34(.a(new_n49_), .b(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nor2   g37(.a(new_n18_), .b(new_n38_), .O(new_n55_));
  nand3  g38(.a(x5), .b(new_n39_), .c(new_n20_), .O(new_n56_));
  oai21  g39(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n21_), .O(new_n58_));
  nand3  g41(.a(new_n22_), .b(x5), .c(new_n20_), .O(new_n59_));
  nand4  g42(.a(new_n25_), .b(new_n39_), .c(new_n38_), .d(x0), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n59_), .c(new_n19_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n18_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n58_), .c(new_n54_), .O(z3));
  nand3  g46(.a(new_n47_), .b(new_n22_), .c(new_n18_), .O(new_n64_));
  inv1   g47(.a(new_n64_), .O(new_n65_));
  aoi21  g48(.a(new_n47_), .b(new_n22_), .c(new_n18_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n65_), .c(x1), .O(new_n67_));
  nand2  g50(.a(new_n48_), .b(new_n18_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n52_), .c(new_n21_), .O(new_n69_));
  nand2  g52(.a(new_n18_), .b(x4), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n69_), .c(new_n67_), .O(z4));
  nand2  g54(.a(new_n38_), .b(new_n21_), .O(new_n72_));
  nand4  g55(.a(new_n40_), .b(new_n72_), .c(new_n22_), .d(x0), .O(new_n73_));
  nand3  g56(.a(new_n40_), .b(new_n72_), .c(new_n22_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n20_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n73_), .c(new_n70_), .O(new_n76_));
  inv1   g59(.a(new_n76_), .O(z5));
  nand3  g60(.a(x3), .b(new_n38_), .c(new_n21_), .O(new_n78_));
  oai21  g61(.a(new_n39_), .b(x2), .c(x1), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n78_), .c(new_n70_), .O(z6));
  nand2  g63(.a(new_n40_), .b(new_n22_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n70_), .O(z7));
  nand2  g65(.a(new_n70_), .b(x3), .O(z8));
  aoi21  g66(.a(x6), .b(new_n25_), .c(new_n19_), .O(z9));
endmodule


