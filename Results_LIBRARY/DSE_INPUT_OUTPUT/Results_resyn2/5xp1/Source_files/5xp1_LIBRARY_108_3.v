// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x1), .O(new_n18_));
  nand2  g01(.a(x3), .b(x2), .O(new_n19_));
  nor2   g02(.a(x6), .b(x0), .O(new_n20_));
  oai21  g03(.a(new_n19_), .b(new_n18_), .c(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x4), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  inv1   g06(.a(new_n19_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x0), .c(x6), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n22_), .c(x5), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  nand2  g11(.a(x6), .b(x1), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  aoi21  g13(.a(new_n28_), .b(x4), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n27_), .O(z0));
  nand2  g15(.a(x6), .b(x2), .O(new_n33_));
  oai21  g16(.a(x3), .b(x2), .c(x1), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n23_), .c(new_n33_), .O(new_n35_));
  aoi22  g18(.a(new_n35_), .b(x0), .c(x6), .d(x4), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  inv1   g21(.a(x0), .O(new_n39_));
  nor2   g22(.a(x6), .b(x1), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n19_), .c(new_n39_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  aoi21  g25(.a(x6), .b(x4), .c(new_n28_), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(new_n42_), .c(new_n30_), .O(new_n44_));
  oai21  g27(.a(new_n36_), .b(x5), .c(new_n44_), .O(z1));
  nand2  g28(.a(x5), .b(x0), .O(new_n46_));
  nand2  g29(.a(x5), .b(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n39_), .O(new_n48_));
  inv1   g31(.a(x2), .O(new_n49_));
  nand2  g32(.a(x3), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n48_), .c(x1), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n46_), .c(new_n23_), .O(new_n53_));
  nand3  g36(.a(new_n52_), .b(new_n46_), .c(new_n23_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  aoi22  g38(.a(new_n47_), .b(new_n39_), .c(new_n46_), .d(new_n49_), .O(new_n56_));
  nor2   g39(.a(new_n56_), .b(new_n23_), .O(new_n57_));
  nand2  g40(.a(new_n56_), .b(new_n23_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x6), .c(new_n18_), .O(new_n59_));
  oai22  g42(.a(new_n59_), .b(new_n57_), .c(new_n55_), .d(new_n53_), .O(z2));
  oai21  g43(.a(new_n50_), .b(new_n18_), .c(new_n41_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n29_), .c(x5), .O(new_n62_));
  nor2   g45(.a(x5), .b(new_n39_), .O(new_n63_));
  nand4  g46(.a(new_n63_), .b(new_n34_), .c(new_n33_), .d(new_n29_), .O(new_n64_));
  nand3  g47(.a(new_n28_), .b(x3), .c(new_n39_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  nor3   g49(.a(new_n40_), .b(new_n30_), .c(new_n49_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n64_), .c(new_n62_), .O(z3));
  oai21  g52(.a(x3), .b(x0), .c(x2), .O(new_n70_));
  aoi21  g53(.a(x3), .b(x0), .c(x6), .O(new_n71_));
  oai22  g54(.a(new_n71_), .b(new_n18_), .c(new_n70_), .d(new_n40_), .O(new_n72_));
  aoi21  g55(.a(new_n70_), .b(new_n40_), .c(new_n72_), .O(z4));
  inv1   g56(.a(x3), .O(new_n74_));
  nor2   g57(.a(new_n74_), .b(x2), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n37_), .c(x1), .O(new_n76_));
  nand3  g59(.a(new_n29_), .b(new_n74_), .c(x2), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n39_), .O(new_n79_));
  oai21  g62(.a(new_n75_), .b(x6), .c(x1), .O(new_n80_));
  aoi21  g63(.a(new_n74_), .b(x2), .c(new_n39_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n79_), .O(z5));
  inv1   g66(.a(new_n76_), .O(new_n84_));
  nor2   g67(.a(new_n75_), .b(x1), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n84_), .O(z6));
  nand2  g69(.a(new_n29_), .b(new_n19_), .O(new_n87_));
  aoi21  g70(.a(new_n74_), .b(new_n49_), .c(new_n87_), .O(z7));
  nor2   g71(.a(new_n30_), .b(x3), .O(z8));
  nand2  g72(.a(x5), .b(x4), .O(new_n90_));
  oai21  g73(.a(new_n90_), .b(new_n19_), .c(new_n37_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(x1), .O(new_n92_));
  oai21  g75(.a(new_n90_), .b(new_n20_), .c(new_n92_), .O(z9));
endmodule


