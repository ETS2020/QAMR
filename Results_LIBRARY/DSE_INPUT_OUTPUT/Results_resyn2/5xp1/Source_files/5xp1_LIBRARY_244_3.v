// Benchmark "FAU" written by ABC on Thu Aug  6 21:27:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n101_, new_n102_,
    new_n103_, new_n104_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nor2   g05(.a(x1), .b(x0), .O(new_n23_));
  inv1   g06(.a(x4), .O(new_n24_));
  nand2  g07(.a(x6), .b(new_n24_), .O(new_n25_));
  oai22  g08(.a(new_n25_), .b(new_n23_), .c(new_n22_), .d(new_n19_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x5), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  inv1   g11(.a(x2), .O(new_n29_));
  nor2   g12(.a(new_n18_), .b(new_n29_), .O(new_n30_));
  aoi21  g13(.a(new_n28_), .b(x4), .c(new_n30_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n27_), .O(z0));
  inv1   g15(.a(x1), .O(new_n33_));
  aoi21  g16(.a(x3), .b(x2), .c(x6), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g18(.a(x4), .b(x2), .c(x6), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n20_), .O(new_n37_));
  oai22  g20(.a(new_n37_), .b(new_n35_), .c(x6), .d(x4), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x5), .O(new_n39_));
  nand2  g22(.a(x1), .b(x0), .O(new_n40_));
  nand2  g23(.a(x6), .b(new_n29_), .O(new_n41_));
  aoi21  g24(.a(new_n40_), .b(new_n24_), .c(new_n41_), .O(new_n42_));
  inv1   g25(.a(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n29_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x1), .c(x0), .O(new_n45_));
  nor2   g28(.a(new_n45_), .b(new_n19_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n42_), .c(new_n28_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n39_), .O(z1));
  nor2   g31(.a(x5), .b(x1), .O(new_n49_));
  nand3  g32(.a(x5), .b(x3), .c(x1), .O(new_n50_));
  oai21  g33(.a(new_n49_), .b(new_n20_), .c(new_n50_), .O(new_n51_));
  nand2  g34(.a(new_n28_), .b(new_n43_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x0), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n51_), .c(x4), .O(new_n55_));
  nand2  g38(.a(new_n22_), .b(x5), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n45_), .c(new_n24_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n55_), .c(new_n18_), .O(new_n58_));
  oai21  g41(.a(x1), .b(x0), .c(x5), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n40_), .O(new_n60_));
  aoi21  g43(.a(new_n60_), .b(new_n24_), .c(new_n41_), .O(new_n61_));
  oai21  g44(.a(new_n60_), .b(new_n24_), .c(new_n61_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n58_), .O(z2));
  nand2  g46(.a(new_n28_), .b(new_n33_), .O(new_n64_));
  nand2  g47(.a(x5), .b(x1), .O(new_n65_));
  nor2   g48(.a(x6), .b(x3), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  oai21  g50(.a(new_n18_), .b(x5), .c(x1), .O(new_n68_));
  nor2   g51(.a(new_n49_), .b(x0), .O(new_n69_));
  aoi22  g52(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(x0), .O(new_n70_));
  nor2   g53(.a(new_n29_), .b(new_n33_), .O(new_n71_));
  xnor2a g54(.a(x5), .b(x0), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n71_), .c(new_n52_), .O(new_n73_));
  nand3  g56(.a(new_n21_), .b(x5), .c(new_n20_), .O(new_n74_));
  oai21  g57(.a(x3), .b(x2), .c(x1), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n28_), .c(x0), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n18_), .O(new_n78_));
  oai21  g61(.a(new_n70_), .b(x2), .c(new_n78_), .O(z3));
  nand2  g62(.a(new_n44_), .b(x0), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n34_), .c(x1), .O(new_n81_));
  oai22  g64(.a(new_n18_), .b(new_n43_), .c(new_n29_), .d(x1), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x0), .O(new_n83_));
  inv1   g66(.a(new_n34_), .O(new_n84_));
  oai21  g67(.a(new_n18_), .b(new_n29_), .c(x1), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n83_), .c(new_n81_), .O(z4));
  nor2   g70(.a(new_n43_), .b(x2), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(x1), .O(new_n89_));
  nand2  g72(.a(new_n66_), .b(x2), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n20_), .O(new_n92_));
  oai21  g75(.a(x6), .b(new_n43_), .c(x2), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n89_), .c(x0), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n92_), .O(z5));
  inv1   g78(.a(new_n88_), .O(new_n96_));
  inv1   g79(.a(new_n89_), .O(new_n97_));
  aoi21  g80(.a(new_n96_), .b(new_n85_), .c(new_n97_), .O(z6));
  nand2  g81(.a(new_n90_), .b(new_n96_), .O(z7));
  nor2   g82(.a(new_n30_), .b(x3), .O(z8));
  oai21  g83(.a(new_n50_), .b(new_n24_), .c(new_n18_), .O(new_n101_));
  nand2  g84(.a(new_n101_), .b(x2), .O(new_n102_));
  nand2  g85(.a(new_n18_), .b(new_n20_), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(x5), .c(x4), .O(new_n104_));
  nand2  g87(.a(new_n104_), .b(new_n102_), .O(z9));
endmodule


