// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n99_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand4  g05(.a(new_n22_), .b(x6), .c(x5), .d(new_n18_), .O(new_n23_));
  oai21  g06(.a(x6), .b(x0), .c(x5), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x3), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n24_), .c(x4), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n23_), .O(z0));
  inv1   g11(.a(x5), .O(new_n29_));
  nand2  g12(.a(x6), .b(x4), .O(new_n30_));
  oai21  g13(.a(x2), .b(x1), .c(x6), .O(new_n31_));
  inv1   g14(.a(x3), .O(z8));
  nand3  g15(.a(z8), .b(x2), .c(x1), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  nand2  g17(.a(new_n31_), .b(new_n18_), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n34_), .c(x0), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n29_), .O(new_n38_));
  nand3  g21(.a(new_n25_), .b(new_n18_), .c(z8), .O(new_n39_));
  oai21  g22(.a(new_n25_), .b(x2), .c(x3), .O(new_n40_));
  nand2  g23(.a(x6), .b(x1), .O(new_n41_));
  oai21  g24(.a(x6), .b(x3), .c(x4), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n19_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n39_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x5), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n38_), .O(z1));
  oai21  g29(.a(x2), .b(x1), .c(x0), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n18_), .O(new_n48_));
  aoi21  g31(.a(new_n22_), .b(x5), .c(new_n48_), .O(new_n49_));
  nand3  g32(.a(x5), .b(x4), .c(x1), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n49_), .c(x6), .O(new_n52_));
  nand2  g35(.a(x2), .b(x1), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  oai21  g37(.a(new_n25_), .b(x0), .c(x4), .O(new_n55_));
  aoi21  g38(.a(new_n54_), .b(x0), .c(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n25_), .b(x4), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n54_), .c(x0), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(z8), .O(new_n59_));
  nor2   g42(.a(x2), .b(x1), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n29_), .O(new_n61_));
  nand2  g44(.a(x5), .b(x2), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n19_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n61_), .c(x4), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(x6), .O(new_n65_));
  oai21  g48(.a(new_n59_), .b(new_n56_), .c(new_n65_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n52_), .O(z2));
  aoi21  g50(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n68_));
  nand4  g51(.a(new_n53_), .b(new_n31_), .c(new_n26_), .d(new_n29_), .O(new_n69_));
  inv1   g52(.a(new_n69_), .O(new_n70_));
  oai21  g53(.a(new_n70_), .b(new_n68_), .c(x0), .O(new_n71_));
  nand3  g54(.a(new_n41_), .b(new_n40_), .c(x5), .O(new_n72_));
  nand2  g55(.a(new_n21_), .b(new_n20_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(x6), .c(new_n29_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n19_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n71_), .O(z3));
  inv1   g60(.a(x2), .O(new_n78_));
  nand2  g61(.a(z8), .b(new_n78_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x1), .O(new_n80_));
  aoi21  g63(.a(new_n21_), .b(new_n19_), .c(new_n80_), .O(new_n81_));
  oai21  g64(.a(new_n81_), .b(new_n60_), .c(x6), .O(new_n82_));
  oai21  g65(.a(new_n78_), .b(new_n19_), .c(new_n25_), .O(new_n83_));
  or2    g66(.a(new_n83_), .b(x1), .O(new_n84_));
  nand2  g67(.a(new_n83_), .b(x1), .O(new_n85_));
  aoi21  g68(.a(x6), .b(x0), .c(x3), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n82_), .O(z4));
  nand3  g71(.a(x6), .b(x3), .c(new_n78_), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(new_n90_));
  aoi22  g73(.a(new_n90_), .b(x1), .c(z8), .d(x2), .O(new_n91_));
  nand2  g74(.a(new_n90_), .b(x1), .O(new_n92_));
  oai21  g75(.a(new_n25_), .b(z8), .c(new_n79_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n92_), .c(x0), .O(new_n94_));
  oai21  g77(.a(new_n91_), .b(x0), .c(new_n94_), .O(z5));
  nand2  g78(.a(new_n31_), .b(x3), .O(new_n96_));
  oai21  g79(.a(new_n90_), .b(new_n20_), .c(new_n96_), .O(z6));
  inv1   g80(.a(new_n79_), .O(new_n98_));
  nor2   g81(.a(new_n25_), .b(z8), .O(new_n99_));
  aoi21  g82(.a(new_n99_), .b(x2), .c(new_n98_), .O(z7));
  oai21  g83(.a(new_n24_), .b(new_n18_), .c(new_n26_), .O(z9));
endmodule


