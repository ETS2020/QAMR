// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_;
  inv1   g00(.a(x4), .O(new_n18_));
  nand2  g01(.a(x5), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  nor2   g07(.a(x5), .b(new_n18_), .O(new_n25_));
  inv1   g08(.a(x5), .O(new_n26_));
  nor2   g09(.a(x6), .b(new_n26_), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  oai21  g11(.a(new_n24_), .b(new_n19_), .c(new_n28_), .O(z0));
  oai21  g12(.a(x2), .b(x1), .c(x0), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n18_), .c(x5), .O(new_n31_));
  nor2   g14(.a(new_n23_), .b(new_n19_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(x6), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  inv1   g17(.a(x3), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n25_), .c(x1), .d(x0), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n33_), .O(z1));
  nand3  g21(.a(x6), .b(new_n34_), .c(new_n21_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n20_), .O(new_n41_));
  nand3  g24(.a(x6), .b(new_n21_), .c(new_n20_), .O(new_n42_));
  nand3  g25(.a(new_n26_), .b(new_n18_), .c(new_n34_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n35_), .O(new_n45_));
  nor2   g28(.a(x5), .b(x1), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(x6), .c(new_n18_), .O(new_n47_));
  nor2   g30(.a(x2), .b(x1), .O(new_n48_));
  nor2   g31(.a(x6), .b(new_n18_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n48_), .c(new_n26_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(new_n47_), .c(new_n45_), .d(new_n41_), .O(new_n51_));
  inv1   g34(.a(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x4), .O(new_n53_));
  nand2  g36(.a(x5), .b(x0), .O(new_n54_));
  nand4  g37(.a(new_n54_), .b(x6), .c(new_n18_), .d(new_n34_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand3  g40(.a(x6), .b(new_n26_), .c(new_n18_), .O(new_n58_));
  aoi21  g41(.a(new_n58_), .b(new_n53_), .c(x0), .O(new_n59_));
  nand3  g42(.a(new_n52_), .b(x4), .c(new_n34_), .O(new_n60_));
  oai21  g43(.a(new_n42_), .b(x4), .c(new_n60_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n35_), .c(new_n59_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n57_), .c(new_n51_), .O(z2));
  nand3  g46(.a(x3), .b(x2), .c(x1), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nand2  g48(.a(new_n22_), .b(new_n21_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n65_), .c(new_n20_), .O(new_n67_));
  oai21  g50(.a(x3), .b(x2), .c(x1), .O(new_n68_));
  oai21  g51(.a(x2), .b(x1), .c(x6), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(x0), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n26_), .O(new_n72_));
  nand2  g55(.a(new_n30_), .b(new_n23_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(x6), .c(x5), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n72_), .O(z3));
  nand2  g58(.a(new_n22_), .b(new_n20_), .O(new_n76_));
  nor2   g59(.a(x6), .b(x5), .O(new_n77_));
  aoi21  g60(.a(new_n36_), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  nand3  g61(.a(new_n36_), .b(new_n76_), .c(new_n52_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x1), .O(new_n80_));
  oai21  g63(.a(x3), .b(x0), .c(x2), .O(new_n81_));
  aoi21  g64(.a(new_n81_), .b(new_n52_), .c(x1), .O(new_n82_));
  oai21  g65(.a(new_n81_), .b(new_n77_), .c(new_n82_), .O(new_n83_));
  oai21  g66(.a(new_n80_), .b(new_n78_), .c(new_n83_), .O(z4));
  inv1   g67(.a(new_n27_), .O(new_n85_));
  inv1   g68(.a(new_n48_), .O(new_n86_));
  nand4  g69(.a(new_n36_), .b(new_n86_), .c(new_n22_), .d(x0), .O(new_n87_));
  nand2  g70(.a(new_n36_), .b(new_n22_), .O(new_n88_));
  oai21  g71(.a(new_n88_), .b(new_n48_), .c(new_n20_), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(new_n90_));
  inv1   g73(.a(new_n90_), .O(z5));
  nand2  g74(.a(x3), .b(new_n34_), .O(new_n92_));
  oai21  g75(.a(new_n92_), .b(new_n21_), .c(new_n85_), .O(new_n93_));
  aoi21  g76(.a(new_n92_), .b(new_n21_), .c(new_n93_), .O(z6));
  nor2   g77(.a(new_n88_), .b(new_n27_), .O(z7));
  nand2  g78(.a(new_n85_), .b(x3), .O(z8));
  aoi21  g79(.a(x6), .b(new_n18_), .c(new_n26_), .O(z9));
endmodule


