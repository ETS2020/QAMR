// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n87_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x5), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n19_), .c(new_n18_), .O(z9));
  inv1   g07(.a(x5), .O(new_n25_));
  aoi21  g08(.a(x3), .b(x2), .c(x1), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n20_), .c(new_n25_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(x6), .c(x4), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(z9), .O(z0));
  inv1   g12(.a(x1), .O(new_n30_));
  nand2  g13(.a(x3), .b(x2), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(x4), .c(x6), .O(new_n33_));
  nand2  g16(.a(new_n19_), .b(new_n18_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(new_n22_), .c(new_n25_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  oai21  g19(.a(x6), .b(x1), .c(x2), .O(new_n37_));
  oai21  g20(.a(x6), .b(x3), .c(x1), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g22(.a(new_n19_), .b(new_n18_), .O(new_n40_));
  inv1   g23(.a(new_n40_), .O(new_n41_));
  nor2   g24(.a(x5), .b(new_n20_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n39_), .d(new_n34_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n36_), .O(z1));
  inv1   g27(.a(x2), .O(new_n45_));
  inv1   g28(.a(x3), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(x1), .c(x0), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n23_), .c(new_n18_), .O(new_n49_));
  aoi21  g32(.a(new_n21_), .b(new_n20_), .c(new_n25_), .O(new_n50_));
  oai21  g33(.a(x3), .b(x2), .c(x0), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n30_), .c(new_n18_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n50_), .c(new_n19_), .O(new_n53_));
  nand2  g36(.a(new_n25_), .b(new_n20_), .O(new_n54_));
  oai21  g37(.a(x3), .b(x0), .c(x2), .O(new_n55_));
  nand2  g38(.a(x5), .b(x0), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n55_), .c(new_n30_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x6), .c(new_n18_), .O(new_n59_));
  oai21  g42(.a(new_n53_), .b(new_n49_), .c(new_n59_), .O(z2));
  nor2   g43(.a(new_n38_), .b(new_n20_), .O(new_n61_));
  aoi21  g44(.a(new_n32_), .b(x6), .c(new_n22_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n61_), .c(x5), .O(new_n63_));
  nand3  g46(.a(x6), .b(x1), .c(new_n20_), .O(new_n64_));
  oai21  g47(.a(new_n39_), .b(new_n20_), .c(new_n64_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  inv1   g49(.a(new_n37_), .O(new_n67_));
  oai21  g50(.a(new_n54_), .b(new_n46_), .c(new_n56_), .O(new_n68_));
  aoi21  g51(.a(new_n68_), .b(new_n67_), .c(new_n40_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z3));
  nand2  g53(.a(new_n51_), .b(new_n31_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n19_), .O(new_n72_));
  nand3  g55(.a(new_n51_), .b(new_n31_), .c(x6), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(x1), .O(new_n74_));
  aoi21  g57(.a(new_n55_), .b(new_n19_), .c(x1), .O(new_n75_));
  oai21  g58(.a(new_n55_), .b(new_n19_), .c(new_n75_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n41_), .O(z4));
  nand2  g60(.a(new_n46_), .b(x2), .O(new_n78_));
  nand3  g61(.a(x3), .b(new_n45_), .c(x1), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n20_), .O(new_n81_));
  nand3  g64(.a(new_n79_), .b(new_n78_), .c(x0), .O(new_n82_));
  aoi21  g65(.a(new_n82_), .b(new_n81_), .c(new_n40_), .O(z5));
  inv1   g66(.a(new_n79_), .O(new_n84_));
  aoi21  g67(.a(x3), .b(new_n45_), .c(x1), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n84_), .c(new_n41_), .O(z6));
  nand2  g69(.a(new_n47_), .b(new_n31_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n41_), .O(z7));
  nand2  g71(.a(new_n41_), .b(x3), .O(z8));
endmodule


