// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x4), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x6), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  nand2  g09(.a(x5), .b(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(new_n20_), .O(z0));
  nor2   g11(.a(x2), .b(x1), .O(new_n29_));
  nor2   g12(.a(new_n29_), .b(new_n21_), .O(new_n30_));
  nor2   g13(.a(new_n19_), .b(x5), .O(new_n31_));
  oai21  g14(.a(new_n30_), .b(x4), .c(new_n31_), .O(new_n32_));
  nand3  g15(.a(new_n25_), .b(x5), .c(new_n26_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(z1));
  aoi21  g17(.a(new_n18_), .b(new_n21_), .c(new_n26_), .O(new_n35_));
  nand2  g18(.a(new_n23_), .b(new_n21_), .O(new_n36_));
  nand2  g19(.a(new_n18_), .b(new_n21_), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n37_), .c(new_n36_), .d(new_n19_), .O(new_n41_));
  inv1   g24(.a(new_n41_), .O(new_n42_));
  oai21  g25(.a(new_n42_), .b(new_n35_), .c(x1), .O(new_n43_));
  oai21  g26(.a(new_n30_), .b(x5), .c(new_n24_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x6), .c(new_n26_), .O(new_n45_));
  nand2  g28(.a(x5), .b(x0), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n26_), .c(x6), .O(new_n47_));
  aoi22  g30(.a(new_n23_), .b(new_n21_), .c(new_n18_), .d(new_n38_), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n35_), .c(new_n47_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n45_), .c(new_n43_), .O(z2));
  nor2   g33(.a(x3), .b(x2), .O(new_n51_));
  nor2   g34(.a(x6), .b(x4), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g36(.a(new_n19_), .b(x4), .O(new_n54_));
  aoi21  g37(.a(x6), .b(x2), .c(x1), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n53_), .c(new_n21_), .O(new_n57_));
  nand3  g40(.a(x6), .b(x1), .c(new_n21_), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n57_), .c(new_n18_), .O(new_n60_));
  oai21  g43(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(new_n61_));
  aoi21  g44(.a(new_n23_), .b(new_n22_), .c(x0), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g46(.a(x6), .b(x3), .c(x1), .O(new_n64_));
  oai21  g47(.a(x6), .b(new_n26_), .c(x5), .O(new_n65_));
  aoi21  g48(.a(new_n64_), .b(x0), .c(new_n65_), .O(new_n66_));
  oai21  g49(.a(new_n37_), .b(new_n39_), .c(new_n46_), .O(new_n67_));
  nand2  g50(.a(new_n26_), .b(x1), .O(new_n68_));
  aoi21  g51(.a(new_n68_), .b(new_n19_), .c(new_n38_), .O(new_n69_));
  aoi22  g52(.a(new_n69_), .b(new_n67_), .c(new_n66_), .d(new_n63_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n60_), .O(z3));
  nand3  g54(.a(new_n40_), .b(new_n36_), .c(x6), .O(new_n72_));
  nor2   g55(.a(new_n51_), .b(new_n21_), .O(new_n73_));
  nand2  g56(.a(new_n52_), .b(new_n23_), .O(new_n74_));
  oai21  g57(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x1), .O(new_n76_));
  oai21  g59(.a(x3), .b(x0), .c(x2), .O(new_n77_));
  aoi21  g60(.a(new_n77_), .b(new_n19_), .c(x1), .O(new_n78_));
  oai21  g61(.a(new_n77_), .b(new_n52_), .c(new_n78_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n76_), .O(z4));
  inv1   g63(.a(new_n29_), .O(new_n81_));
  nand4  g64(.a(new_n40_), .b(new_n81_), .c(new_n23_), .d(x0), .O(new_n82_));
  nand2  g65(.a(new_n40_), .b(new_n23_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n29_), .c(new_n21_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n82_), .c(new_n54_), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z5));
  nand3  g69(.a(x3), .b(new_n38_), .c(new_n22_), .O(new_n87_));
  oai21  g70(.a(new_n39_), .b(x2), .c(x1), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n87_), .c(new_n54_), .O(z6));
  nand2  g72(.a(new_n83_), .b(new_n54_), .O(z7));
  aoi21  g73(.a(new_n19_), .b(x4), .c(x3), .O(z8));
  nor2   g74(.a(new_n31_), .b(new_n26_), .O(z9));
endmodule


