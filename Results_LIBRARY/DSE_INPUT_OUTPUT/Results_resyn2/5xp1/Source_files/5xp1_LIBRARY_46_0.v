// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  nand2  g02(.a(x6), .b(new_n19_), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x6), .O(new_n22_));
  nand3  g05(.a(x3), .b(x2), .c(x1), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x4), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x5), .O(new_n26_));
  inv1   g09(.a(x1), .O(new_n27_));
  nand2  g10(.a(x3), .b(x2), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(new_n27_), .c(new_n21_), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(x6), .c(x4), .O(new_n30_));
  oai22  g13(.a(new_n30_), .b(new_n26_), .c(new_n20_), .d(new_n18_), .O(z0));
  nand2  g14(.a(new_n23_), .b(new_n21_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x4), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  oai21  g17(.a(x2), .b(x1), .c(x0), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n18_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n19_), .O(new_n37_));
  nor2   g20(.a(new_n19_), .b(x4), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n28_), .c(new_n27_), .d(new_n21_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n37_), .c(new_n34_), .O(z1));
  nand2  g23(.a(new_n32_), .b(new_n22_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n18_), .O(new_n42_));
  nand2  g25(.a(new_n22_), .b(x4), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n29_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n24_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n42_), .c(x5), .O(new_n46_));
  nand2  g29(.a(new_n35_), .b(new_n19_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n29_), .c(new_n18_), .O(new_n48_));
  aoi21  g31(.a(new_n35_), .b(x4), .c(new_n22_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n46_), .O(z2));
  oai21  g34(.a(x3), .b(x2), .c(x1), .O(new_n52_));
  oai21  g35(.a(x2), .b(x1), .c(x6), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(x0), .O(new_n54_));
  nand2  g37(.a(new_n28_), .b(new_n27_), .O(new_n55_));
  nand2  g38(.a(new_n23_), .b(new_n22_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n55_), .c(new_n21_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  aoi21  g41(.a(new_n35_), .b(new_n29_), .c(new_n20_), .O(new_n59_));
  aoi21  g42(.a(new_n58_), .b(x5), .c(new_n59_), .O(z3));
  nor2   g43(.a(x3), .b(x2), .O(new_n61_));
  aoi21  g44(.a(x3), .b(x2), .c(x0), .O(new_n62_));
  nor2   g45(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n22_), .O(new_n64_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(x6), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n64_), .c(x1), .O(new_n66_));
  nor2   g49(.a(x6), .b(x5), .O(new_n67_));
  inv1   g50(.a(new_n67_), .O(new_n68_));
  oai21  g51(.a(x3), .b(x0), .c(x2), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(new_n22_), .c(x1), .O(new_n70_));
  oai21  g53(.a(new_n69_), .b(new_n22_), .c(new_n70_), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n68_), .c(new_n66_), .O(z4));
  nor2   g55(.a(x2), .b(x1), .O(new_n73_));
  inv1   g56(.a(new_n61_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n28_), .O(new_n75_));
  oai21  g58(.a(new_n75_), .b(new_n73_), .c(x0), .O(new_n76_));
  inv1   g59(.a(new_n73_), .O(new_n77_));
  nand4  g60(.a(new_n74_), .b(new_n77_), .c(new_n28_), .d(new_n21_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n76_), .c(new_n68_), .O(z5));
  inv1   g62(.a(x2), .O(new_n80_));
  nand3  g63(.a(x3), .b(new_n80_), .c(new_n27_), .O(new_n81_));
  inv1   g64(.a(x3), .O(new_n82_));
  oai21  g65(.a(new_n82_), .b(x2), .c(x1), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n81_), .c(new_n68_), .O(z6));
  nor2   g67(.a(new_n75_), .b(new_n67_), .O(z7));
  nand2  g68(.a(new_n68_), .b(x3), .O(z8));
  and2   g69(.a(new_n26_), .b(new_n20_), .O(z9));
endmodule


