// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  aoi21  g03(.a(x3), .b(x2), .c(x0), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  inv1   g05(.a(x5), .O(new_n23_));
  nand2  g06(.a(x6), .b(new_n23_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g08(.a(new_n19_), .b(x0), .O(new_n26_));
  nand2  g09(.a(x5), .b(x4), .O(new_n27_));
  aoi21  g10(.a(new_n26_), .b(new_n20_), .c(new_n27_), .O(z9));
  aoi21  g11(.a(new_n25_), .b(new_n18_), .c(z9), .O(z0));
  inv1   g12(.a(x0), .O(new_n30_));
  aoi21  g13(.a(x6), .b(x2), .c(x1), .O(new_n31_));
  oai22  g14(.a(new_n31_), .b(new_n30_), .c(new_n20_), .d(new_n18_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  oai21  g16(.a(new_n18_), .b(new_n30_), .c(new_n20_), .O(new_n34_));
  nand3  g17(.a(new_n21_), .b(new_n18_), .c(new_n19_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x5), .O(new_n37_));
  nand2  g20(.a(new_n20_), .b(x1), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n37_), .c(new_n33_), .O(z1));
  oai21  g22(.a(x5), .b(x0), .c(x6), .O(new_n40_));
  nand2  g23(.a(x5), .b(x0), .O(new_n41_));
  oai21  g24(.a(x3), .b(x0), .c(x2), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nand2  g26(.a(new_n41_), .b(new_n20_), .O(new_n44_));
  oai21  g27(.a(x5), .b(x0), .c(x1), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(x4), .O(new_n47_));
  nand2  g30(.a(new_n23_), .b(new_n30_), .O(new_n48_));
  nand3  g31(.a(new_n42_), .b(new_n41_), .c(new_n19_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n48_), .c(x6), .O(new_n50_));
  aoi21  g33(.a(new_n41_), .b(new_n20_), .c(x4), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n47_), .c(new_n38_), .O(z2));
  xor2a  g36(.a(x5), .b(x0), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n42_), .b(new_n20_), .c(new_n19_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor2   g40(.a(x3), .b(x0), .O(new_n58_));
  inv1   g41(.a(x2), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n19_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x6), .O(new_n61_));
  nor2   g44(.a(new_n23_), .b(x1), .O(new_n62_));
  aoi22  g45(.a(new_n62_), .b(new_n58_), .c(new_n61_), .d(new_n54_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n57_), .O(z3));
  xor2a  g47(.a(new_n42_), .b(x6), .O(new_n65_));
  inv1   g48(.a(new_n21_), .O(new_n66_));
  inv1   g49(.a(x3), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  nand2  g51(.a(x6), .b(x1), .O(new_n69_));
  aoi21  g52(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  aoi21  g53(.a(new_n65_), .b(new_n19_), .c(new_n70_), .O(z4));
  xor2a  g54(.a(x3), .b(x2), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n60_), .c(x0), .O(new_n73_));
  nand2  g56(.a(new_n72_), .b(new_n60_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n30_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n73_), .c(new_n38_), .O(new_n76_));
  inv1   g59(.a(new_n76_), .O(z5));
  nor2   g60(.a(new_n67_), .b(x2), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n19_), .O(new_n79_));
  oai21  g62(.a(new_n78_), .b(new_n69_), .c(new_n79_), .O(z6));
  and2   g63(.a(new_n72_), .b(new_n38_), .O(z7));
  aoi21  g64(.a(new_n20_), .b(x1), .c(x3), .O(z8));
endmodule


