// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n96_,
    new_n97_, new_n98_;
  inv1   g00(.a(x1), .O(new_n18_));
  nand2  g01(.a(x3), .b(x2), .O(new_n19_));
  nor2   g02(.a(x6), .b(x0), .O(new_n20_));
  oai21  g03(.a(new_n19_), .b(new_n18_), .c(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x4), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  inv1   g06(.a(x6), .O(new_n24_));
  aoi21  g07(.a(x3), .b(x2), .c(x0), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n22_), .c(x5), .O(new_n27_));
  inv1   g10(.a(x5), .O(new_n28_));
  nor2   g11(.a(new_n24_), .b(new_n18_), .O(new_n29_));
  aoi21  g12(.a(new_n28_), .b(x4), .c(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n27_), .O(z0));
  inv1   g14(.a(new_n29_), .O(new_n32_));
  aoi21  g15(.a(x2), .b(x0), .c(x4), .O(new_n33_));
  oai21  g16(.a(x3), .b(x2), .c(x0), .O(new_n34_));
  nand2  g17(.a(x4), .b(x1), .O(new_n35_));
  oai22  g18(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(new_n24_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n28_), .O(new_n37_));
  nor2   g20(.a(x6), .b(x4), .O(new_n38_));
  inv1   g21(.a(new_n19_), .O(new_n39_));
  nand2  g22(.a(new_n24_), .b(new_n18_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n39_), .c(x0), .O(new_n41_));
  aoi21  g24(.a(x6), .b(x4), .c(new_n28_), .O(new_n42_));
  oai21  g25(.a(new_n41_), .b(new_n38_), .c(new_n42_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n37_), .c(new_n32_), .O(z1));
  inv1   g27(.a(x0), .O(new_n45_));
  nor2   g28(.a(new_n28_), .b(new_n45_), .O(new_n46_));
  nand3  g29(.a(x5), .b(x3), .c(x2), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n34_), .c(new_n18_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n46_), .c(x4), .O(new_n49_));
  inv1   g32(.a(new_n46_), .O(new_n50_));
  nand2  g33(.a(new_n47_), .b(new_n34_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x1), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n50_), .c(new_n23_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(new_n24_), .O(new_n54_));
  inv1   g37(.a(new_n25_), .O(new_n55_));
  aoi21  g38(.a(x2), .b(x0), .c(x5), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n55_), .c(x4), .O(new_n58_));
  oai21  g41(.a(new_n56_), .b(new_n25_), .c(new_n23_), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(x6), .c(new_n18_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n54_), .O(z2));
  nand2  g45(.a(x5), .b(new_n45_), .O(new_n63_));
  nand2  g46(.a(new_n28_), .b(x0), .O(new_n64_));
  inv1   g47(.a(x2), .O(new_n65_));
  inv1   g48(.a(x3), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(new_n45_), .c(new_n65_), .O(new_n67_));
  nand4  g50(.a(new_n67_), .b(new_n64_), .c(new_n63_), .d(new_n40_), .O(new_n68_));
  nand2  g51(.a(new_n64_), .b(new_n63_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n24_), .c(new_n18_), .O(new_n70_));
  nand2  g53(.a(new_n65_), .b(x1), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n64_), .c(new_n63_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  nand3  g56(.a(x5), .b(x3), .c(x0), .O(new_n74_));
  aoi21  g57(.a(new_n74_), .b(new_n24_), .c(new_n18_), .O(new_n75_));
  oai21  g58(.a(new_n64_), .b(x1), .c(new_n63_), .O(new_n76_));
  aoi21  g59(.a(new_n76_), .b(new_n65_), .c(new_n75_), .O(new_n77_));
  nand4  g60(.a(new_n77_), .b(new_n73_), .c(new_n70_), .d(new_n68_), .O(z3));
  nand2  g61(.a(new_n67_), .b(new_n40_), .O(new_n79_));
  oai21  g62(.a(new_n67_), .b(x6), .c(new_n79_), .O(new_n80_));
  nor2   g63(.a(x3), .b(x2), .O(new_n81_));
  nor3   g64(.a(new_n81_), .b(new_n25_), .c(x6), .O(new_n82_));
  oai21  g65(.a(new_n82_), .b(new_n18_), .c(new_n80_), .O(z4));
  nand2  g66(.a(new_n66_), .b(x2), .O(new_n84_));
  nand3  g67(.a(x3), .b(new_n65_), .c(x1), .O(new_n85_));
  nand3  g68(.a(new_n85_), .b(new_n84_), .c(new_n45_), .O(new_n86_));
  nand2  g69(.a(new_n85_), .b(new_n84_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(x0), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n86_), .c(new_n32_), .O(new_n89_));
  inv1   g72(.a(new_n89_), .O(z5));
  inv1   g73(.a(new_n85_), .O(new_n91_));
  aoi22  g74(.a(new_n24_), .b(x1), .c(x3), .d(new_n65_), .O(new_n92_));
  nor2   g75(.a(new_n92_), .b(new_n91_), .O(z6));
  nor3   g76(.a(new_n81_), .b(new_n29_), .c(new_n39_), .O(z7));
  nand2  g77(.a(new_n32_), .b(x3), .O(z8));
  oai21  g78(.a(new_n47_), .b(new_n23_), .c(new_n24_), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(x1), .O(new_n97_));
  nand2  g80(.a(x5), .b(x4), .O(new_n98_));
  oai21  g81(.a(new_n98_), .b(new_n20_), .c(new_n97_), .O(z9));
endmodule


