// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n92_, new_n93_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(x3), .b(x2), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand4  g05(.a(new_n22_), .b(x6), .c(x5), .d(new_n18_), .O(new_n23_));
  inv1   g06(.a(x5), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x4), .O(new_n25_));
  nor2   g08(.a(x6), .b(x0), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n23_), .O(z0));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(new_n18_), .c(x5), .O(new_n30_));
  nand2  g13(.a(x5), .b(new_n18_), .O(new_n31_));
  nor2   g14(.a(new_n31_), .b(new_n22_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n30_), .c(x6), .O(new_n33_));
  oai21  g16(.a(x3), .b(x2), .c(x1), .O(new_n34_));
  inv1   g17(.a(x6), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x5), .O(new_n36_));
  oai22  g19(.a(new_n36_), .b(x4), .c(new_n34_), .d(new_n25_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x0), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n33_), .O(z1));
  inv1   g22(.a(x2), .O(new_n40_));
  nand3  g23(.a(x6), .b(new_n40_), .c(new_n20_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(x0), .c(x5), .O(new_n42_));
  nand2  g25(.a(new_n36_), .b(new_n22_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n42_), .c(new_n18_), .O(new_n44_));
  nand3  g27(.a(new_n34_), .b(new_n35_), .c(new_n24_), .O(new_n45_));
  nand2  g28(.a(new_n20_), .b(new_n19_), .O(new_n46_));
  nand2  g29(.a(x1), .b(x0), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n24_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(x6), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x4), .O(new_n51_));
  nand3  g34(.a(new_n35_), .b(new_n18_), .c(x1), .O(new_n52_));
  nand3  g35(.a(x6), .b(x4), .c(x0), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x2), .O(new_n55_));
  nand4  g38(.a(x6), .b(x5), .c(x4), .d(x2), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(x3), .c(new_n26_), .O(new_n58_));
  nand4  g41(.a(new_n58_), .b(new_n55_), .c(new_n51_), .d(new_n44_), .O(z2));
  aoi21  g42(.a(x2), .b(x1), .c(x6), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n29_), .c(new_n22_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x5), .O(new_n62_));
  nand3  g45(.a(new_n40_), .b(new_n20_), .c(x0), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x6), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n34_), .c(new_n24_), .O(new_n65_));
  oai21  g48(.a(x5), .b(new_n20_), .c(x6), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n19_), .O(new_n67_));
  nand2  g50(.a(new_n47_), .b(x5), .O(new_n68_));
  oai21  g51(.a(new_n40_), .b(x0), .c(new_n24_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n68_), .c(x3), .O(new_n70_));
  nand4  g53(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n62_), .O(z3));
  inv1   g54(.a(new_n21_), .O(new_n72_));
  nor2   g55(.a(new_n34_), .b(new_n35_), .O(new_n73_));
  oai21  g56(.a(new_n72_), .b(x0), .c(new_n73_), .O(new_n74_));
  inv1   g57(.a(x3), .O(new_n75_));
  nand3  g58(.a(x6), .b(new_n75_), .c(new_n19_), .O(new_n76_));
  nand3  g59(.a(new_n35_), .b(x2), .c(x0), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n20_), .O(new_n79_));
  nand3  g62(.a(new_n35_), .b(new_n75_), .c(x0), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(x1), .O(new_n81_));
  aoi21  g64(.a(new_n35_), .b(new_n20_), .c(x2), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n79_), .c(new_n74_), .O(z4));
  aoi21  g67(.a(x3), .b(x1), .c(x2), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n72_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(x6), .c(new_n19_), .O(new_n87_));
  oai21  g70(.a(new_n86_), .b(new_n19_), .c(new_n87_), .O(z5));
  nand3  g71(.a(x3), .b(new_n40_), .c(new_n20_), .O(new_n89_));
  oai21  g72(.a(new_n75_), .b(x2), .c(x1), .O(new_n90_));
  nand3  g73(.a(new_n90_), .b(new_n89_), .c(new_n27_), .O(z6));
  nand2  g74(.a(new_n75_), .b(new_n40_), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(new_n27_), .c(new_n21_), .O(new_n93_));
  inv1   g76(.a(new_n93_), .O(z7));
  nand2  g77(.a(new_n27_), .b(x3), .O(z8));
  nor3   g78(.a(new_n26_), .b(new_n24_), .c(new_n18_), .O(z9));
endmodule


