// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  nand3  g02(.a(x3), .b(x2), .c(x1), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x5), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n18_), .c(x4), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  nand2  g07(.a(x3), .b(x2), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n19_), .O(new_n26_));
  nor2   g09(.a(new_n18_), .b(x4), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(x5), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n23_), .O(z0));
  aoi21  g12(.a(new_n21_), .b(x4), .c(x6), .O(new_n30_));
  inv1   g13(.a(x4), .O(new_n31_));
  nand4  g14(.a(new_n25_), .b(new_n31_), .c(new_n24_), .d(new_n19_), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n30_), .c(x5), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  aoi21  g18(.a(x6), .b(new_n31_), .c(x3), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n24_), .c(new_n35_), .O(new_n37_));
  nand2  g20(.a(new_n18_), .b(x1), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x4), .O(new_n39_));
  inv1   g22(.a(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x0), .O(new_n41_));
  aoi21  g24(.a(new_n18_), .b(new_n31_), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n39_), .c(new_n37_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n34_), .O(z1));
  nand3  g27(.a(x5), .b(x3), .c(x1), .O(new_n45_));
  oai21  g28(.a(x5), .b(x1), .c(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g30(.a(x5), .b(x3), .c(x0), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n35_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(new_n47_), .c(x4), .O(new_n50_));
  inv1   g33(.a(x3), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n35_), .c(new_n19_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x1), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n22_), .c(new_n31_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n50_), .c(new_n18_), .O(new_n55_));
  oai21  g38(.a(x3), .b(x0), .c(x2), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  oai22  g40(.a(new_n57_), .b(x1), .c(x5), .d(x0), .O(new_n58_));
  nand2  g41(.a(x5), .b(x0), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n58_), .c(new_n27_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n55_), .O(z2));
  nor2   g44(.a(x5), .b(new_n24_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x6), .O(new_n63_));
  nand3  g46(.a(new_n25_), .b(x5), .c(new_n24_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n63_), .c(new_n19_), .O(new_n65_));
  aoi22  g48(.a(x6), .b(x1), .c(new_n40_), .d(new_n35_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n62_), .c(x0), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n65_), .c(new_n31_), .O(new_n68_));
  nand3  g51(.a(new_n40_), .b(new_n51_), .c(new_n35_), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(new_n45_), .c(new_n19_), .O(new_n70_));
  nand2  g53(.a(new_n25_), .b(new_n19_), .O(new_n71_));
  xnor2a g54(.a(x5), .b(x0), .O(new_n72_));
  aoi21  g55(.a(new_n71_), .b(x1), .c(new_n72_), .O(new_n73_));
  oai21  g56(.a(new_n73_), .b(new_n70_), .c(new_n18_), .O(new_n74_));
  oai21  g57(.a(new_n18_), .b(x4), .c(new_n38_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n72_), .c(new_n57_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n68_), .O(z3));
  oai21  g60(.a(new_n56_), .b(x1), .c(new_n18_), .O(new_n78_));
  oai21  g61(.a(new_n57_), .b(x1), .c(new_n27_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g63(.a(new_n51_), .b(new_n35_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(new_n71_), .c(x6), .O(new_n82_));
  nand2  g65(.a(new_n25_), .b(new_n18_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n52_), .c(new_n82_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x1), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n80_), .O(z4));
  oai21  g69(.a(new_n51_), .b(new_n24_), .c(new_n35_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n25_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(x0), .O(new_n89_));
  nand3  g72(.a(new_n87_), .b(new_n25_), .c(new_n19_), .O(new_n90_));
  nand2  g73(.a(x6), .b(x4), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z5));
  nand2  g75(.a(x3), .b(new_n35_), .O(new_n93_));
  oai21  g76(.a(new_n93_), .b(new_n24_), .c(new_n91_), .O(new_n94_));
  aoi21  g77(.a(new_n93_), .b(new_n24_), .c(new_n94_), .O(z6));
  nand2  g78(.a(new_n81_), .b(new_n25_), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n91_), .O(z7));
  nand2  g80(.a(new_n91_), .b(x3), .O(z8));
  aoi21  g81(.a(new_n22_), .b(new_n18_), .c(new_n31_), .O(z9));
endmodule


