// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_;
  inv1   g00(.a(x0), .O(new_n18_));
  nand2  g01(.a(x3), .b(x1), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(x6), .c(x5), .O(new_n21_));
  inv1   g04(.a(x2), .O(new_n22_));
  inv1   g05(.a(x6), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n21_), .c(x4), .O(new_n25_));
  aoi21  g08(.a(x3), .b(x2), .c(x1), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  nor2   g10(.a(new_n23_), .b(x4), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(new_n27_), .c(x5), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n25_), .O(z0));
  inv1   g13(.a(x5), .O(new_n31_));
  nand2  g14(.a(x6), .b(x4), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  oai21  g16(.a(new_n23_), .b(new_n22_), .c(new_n33_), .O(new_n34_));
  nand2  g17(.a(x4), .b(x2), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n23_), .c(new_n18_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n32_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n31_), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(new_n26_), .b(x6), .c(new_n40_), .O(new_n41_));
  nand4  g24(.a(new_n19_), .b(new_n23_), .c(x4), .d(x2), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(x0), .O(new_n43_));
  nor2   g26(.a(x6), .b(new_n22_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(x5), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n39_), .O(z1));
  nand2  g31(.a(x1), .b(x0), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n40_), .O(new_n50_));
  aoi21  g33(.a(new_n20_), .b(x5), .c(new_n50_), .O(new_n51_));
  aoi21  g34(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n40_), .c(new_n23_), .O(new_n53_));
  inv1   g36(.a(x3), .O(new_n54_));
  oai21  g37(.a(new_n31_), .b(new_n54_), .c(new_n18_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(x6), .c(x4), .O(new_n56_));
  oai21  g39(.a(new_n53_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x2), .O(new_n58_));
  nor2   g41(.a(x3), .b(x0), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n28_), .O(new_n60_));
  inv1   g43(.a(new_n60_), .O(new_n61_));
  nand2  g44(.a(x6), .b(new_n22_), .O(new_n62_));
  nand2  g45(.a(x5), .b(x0), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n32_), .O(new_n64_));
  aoi21  g47(.a(new_n62_), .b(new_n35_), .c(new_n64_), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n61_), .c(new_n33_), .O(new_n66_));
  nor2   g49(.a(x5), .b(x0), .O(new_n67_));
  inv1   g50(.a(new_n67_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n63_), .c(new_n33_), .O(new_n69_));
  nand2  g52(.a(new_n68_), .b(new_n40_), .O(new_n70_));
  aoi21  g53(.a(new_n67_), .b(x4), .c(new_n23_), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n66_), .c(new_n58_), .O(z2));
  inv1   g56(.a(new_n69_), .O(new_n74_));
  inv1   g57(.a(new_n59_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x2), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(new_n23_), .c(new_n74_), .O(new_n77_));
  nand3  g60(.a(new_n23_), .b(x5), .c(new_n54_), .O(new_n78_));
  nand2  g61(.a(new_n31_), .b(x1), .O(new_n79_));
  oai21  g62(.a(new_n79_), .b(new_n44_), .c(new_n78_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n18_), .O(new_n81_));
  nand2  g64(.a(new_n68_), .b(new_n63_), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n75_), .c(new_n34_), .O(new_n83_));
  nand4  g66(.a(new_n83_), .b(new_n81_), .c(new_n77_), .d(new_n24_), .O(z3));
  oai22  g67(.a(new_n49_), .b(new_n54_), .c(x2), .d(x1), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x6), .O(new_n86_));
  xor2a  g69(.a(x6), .b(x1), .O(new_n87_));
  or2    g70(.a(new_n87_), .b(new_n76_), .O(new_n88_));
  nand3  g71(.a(new_n87_), .b(new_n59_), .c(new_n24_), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(z4));
  nand2  g73(.a(new_n54_), .b(x2), .O(new_n91_));
  inv1   g74(.a(new_n19_), .O(new_n92_));
  inv1   g75(.a(new_n62_), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  and2   g77(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  aoi21  g78(.a(new_n94_), .b(new_n91_), .c(new_n18_), .O(new_n96_));
  nand2  g79(.a(new_n24_), .b(x0), .O(new_n97_));
  aoi21  g80(.a(new_n97_), .b(new_n95_), .c(new_n96_), .O(z5));
  aoi21  g81(.a(x6), .b(new_n54_), .c(x2), .O(new_n99_));
  nand4  g82(.a(x6), .b(x3), .c(new_n22_), .d(new_n33_), .O(new_n100_));
  oai21  g83(.a(new_n99_), .b(new_n33_), .c(new_n100_), .O(z6));
  inv1   g84(.a(new_n99_), .O(new_n102_));
  nand2  g85(.a(new_n102_), .b(new_n91_), .O(z7));
  nand2  g86(.a(new_n24_), .b(x3), .O(z8));
  oai21  g87(.a(new_n21_), .b(new_n40_), .c(new_n24_), .O(z9));
endmodule


