// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_;
  oai21  g00(.a(x6), .b(x0), .c(x5), .O(new_n18_));
  inv1   g01(.a(x2), .O(new_n19_));
  nor2   g02(.a(x6), .b(new_n19_), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nand3  g04(.a(new_n21_), .b(new_n18_), .c(x4), .O(new_n22_));
  inv1   g05(.a(x4), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  aoi21  g07(.a(x3), .b(x2), .c(x0), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand4  g09(.a(new_n26_), .b(x6), .c(x5), .d(new_n23_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n22_), .O(z0));
  inv1   g11(.a(x5), .O(new_n29_));
  inv1   g12(.a(x6), .O(new_n30_));
  oai21  g13(.a(x2), .b(x1), .c(x0), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(new_n23_), .c(new_n30_), .O(new_n32_));
  nand2  g15(.a(x3), .b(x0), .O(new_n33_));
  nand3  g16(.a(x4), .b(new_n19_), .c(x1), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n32_), .c(new_n29_), .O(new_n36_));
  nor2   g19(.a(x6), .b(x2), .O(new_n37_));
  inv1   g20(.a(new_n37_), .O(new_n38_));
  inv1   g21(.a(x3), .O(new_n39_));
  aoi21  g22(.a(x6), .b(new_n39_), .c(new_n19_), .O(new_n40_));
  nand2  g23(.a(new_n23_), .b(new_n24_), .O(new_n41_));
  oai21  g24(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(new_n42_));
  nand2  g25(.a(new_n37_), .b(new_n23_), .O(new_n43_));
  aoi21  g26(.a(new_n43_), .b(x0), .c(new_n29_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n36_), .O(z1));
  nand2  g29(.a(new_n31_), .b(new_n29_), .O(new_n47_));
  aoi21  g30(.a(new_n47_), .b(new_n26_), .c(new_n23_), .O(new_n48_));
  oai21  g31(.a(x3), .b(x0), .c(x2), .O(new_n49_));
  aoi21  g32(.a(x5), .b(x0), .c(x1), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g34(.a(x0), .O(new_n52_));
  aoi21  g35(.a(new_n29_), .b(new_n52_), .c(x4), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x6), .O(new_n55_));
  nand2  g38(.a(x3), .b(x1), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(x4), .c(x0), .O(new_n58_));
  aoi21  g41(.a(x3), .b(x1), .c(x5), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n52_), .c(new_n23_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n58_), .c(new_n37_), .O(new_n61_));
  oai21  g44(.a(new_n55_), .b(new_n48_), .c(new_n61_), .O(z2));
  nand2  g45(.a(new_n30_), .b(x5), .O(new_n63_));
  nand3  g46(.a(x6), .b(new_n29_), .c(x1), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g48(.a(x5), .b(x0), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(x6), .c(new_n19_), .O(new_n67_));
  aoi21  g50(.a(new_n65_), .b(new_n52_), .c(new_n67_), .O(new_n68_));
  nand2  g51(.a(new_n29_), .b(new_n52_), .O(new_n69_));
  nand3  g52(.a(x5), .b(x1), .c(x0), .O(new_n70_));
  oai21  g53(.a(new_n69_), .b(new_n19_), .c(new_n70_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(x3), .O(new_n72_));
  oai21  g55(.a(x5), .b(x3), .c(new_n30_), .O(new_n73_));
  nor2   g56(.a(new_n30_), .b(x1), .O(new_n74_));
  inv1   g57(.a(new_n74_), .O(new_n75_));
  aoi21  g58(.a(x6), .b(new_n29_), .c(new_n52_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(new_n77_));
  nand3  g60(.a(new_n69_), .b(new_n50_), .c(new_n49_), .O(new_n78_));
  nand4  g61(.a(new_n78_), .b(new_n77_), .c(new_n72_), .d(new_n68_), .O(z3));
  nand2  g62(.a(x6), .b(x3), .O(new_n80_));
  aoi21  g63(.a(new_n80_), .b(new_n19_), .c(new_n52_), .O(new_n81_));
  nand2  g64(.a(new_n33_), .b(new_n30_), .O(new_n82_));
  oai21  g65(.a(new_n39_), .b(new_n19_), .c(new_n82_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(new_n81_), .c(x1), .O(new_n84_));
  aoi21  g67(.a(new_n74_), .b(new_n49_), .c(new_n20_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n84_), .O(z4));
  nand3  g69(.a(new_n56_), .b(new_n19_), .c(x0), .O(new_n87_));
  nand2  g70(.a(new_n33_), .b(x6), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(x2), .O(new_n89_));
  nand2  g72(.a(new_n56_), .b(new_n19_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n25_), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(z5));
  nand2  g75(.a(x3), .b(new_n19_), .O(new_n93_));
  aoi21  g76(.a(new_n93_), .b(x1), .c(new_n20_), .O(new_n94_));
  oai21  g77(.a(new_n93_), .b(x1), .c(new_n94_), .O(z6));
  nand2  g78(.a(new_n80_), .b(x2), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n93_), .O(z7));
  nor2   g80(.a(new_n20_), .b(x3), .O(z8));
  oai21  g81(.a(new_n18_), .b(new_n23_), .c(new_n21_), .O(z9));
endmodule


