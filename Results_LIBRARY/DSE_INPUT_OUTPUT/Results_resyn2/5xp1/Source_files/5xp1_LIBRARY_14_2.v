// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n76_;
  oai21  g00(.a(x6), .b(x0), .c(x5), .O(new_n18_));
  inv1   g01(.a(x6), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x1), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n18_), .c(x4), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x0), .c(x6), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  nand2  g09(.a(x5), .b(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(new_n21_), .O(z0));
  inv1   g11(.a(x5), .O(new_n29_));
  inv1   g12(.a(x0), .O(new_n30_));
  aoi21  g13(.a(x6), .b(x2), .c(x1), .O(new_n31_));
  oai22  g14(.a(new_n31_), .b(new_n30_), .c(new_n19_), .d(new_n26_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  oai21  g16(.a(x6), .b(x0), .c(x4), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n25_), .c(x5), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n33_), .c(new_n20_), .O(z1));
  oai21  g19(.a(x5), .b(x0), .c(x6), .O(new_n37_));
  nand2  g20(.a(x5), .b(x0), .O(new_n38_));
  oai21  g21(.a(x3), .b(x0), .c(x2), .O(new_n39_));
  aoi21  g22(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g23(.a(new_n38_), .b(new_n19_), .O(new_n41_));
  oai21  g24(.a(x5), .b(x0), .c(x1), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n40_), .c(x4), .O(new_n44_));
  nor2   g27(.a(x5), .b(x0), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  nand3  g29(.a(new_n39_), .b(new_n38_), .c(new_n22_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(x6), .O(new_n48_));
  aoi21  g31(.a(new_n38_), .b(new_n19_), .c(x4), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n44_), .c(new_n20_), .O(z2));
  inv1   g34(.a(x2), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n22_), .c(new_n38_), .O(new_n53_));
  aoi21  g36(.a(new_n45_), .b(new_n24_), .c(new_n53_), .O(new_n54_));
  inv1   g37(.a(x3), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(x6), .c(x2), .O(new_n57_));
  aoi21  g40(.a(x5), .b(x0), .c(x1), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n57_), .c(new_n46_), .O(new_n59_));
  oai21  g42(.a(new_n54_), .b(new_n19_), .c(new_n59_), .O(z3));
  nand2  g43(.a(x3), .b(x1), .O(new_n61_));
  inv1   g44(.a(new_n61_), .O(new_n62_));
  aoi22  g45(.a(new_n62_), .b(x0), .c(new_n39_), .d(new_n22_), .O(new_n63_));
  nand2  g46(.a(x6), .b(new_n22_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n20_), .O(new_n65_));
  oai22  g48(.a(new_n65_), .b(new_n39_), .c(new_n63_), .d(new_n19_), .O(z4));
  nor2   g49(.a(x3), .b(new_n52_), .O(new_n67_));
  inv1   g50(.a(new_n67_), .O(new_n68_));
  nor2   g51(.a(new_n61_), .b(x2), .O(new_n69_));
  inv1   g52(.a(new_n69_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n68_), .c(x0), .O(new_n71_));
  oai21  g54(.a(new_n69_), .b(new_n67_), .c(new_n30_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n71_), .c(new_n20_), .O(z5));
  nand2  g56(.a(x3), .b(new_n52_), .O(new_n74_));
  aoi22  g57(.a(new_n74_), .b(new_n22_), .c(new_n69_), .d(x6), .O(z6));
  inv1   g58(.a(new_n20_), .O(new_n76_));
  aoi21  g59(.a(new_n74_), .b(new_n68_), .c(new_n76_), .O(z7));
  nor2   g60(.a(new_n76_), .b(x3), .O(z8));
  oai21  g61(.a(new_n34_), .b(new_n29_), .c(new_n20_), .O(z9));
endmodule


