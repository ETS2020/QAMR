// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand2  g01(.a(x5), .b(x4), .O(new_n19_));
  aoi21  g02(.a(new_n19_), .b(x3), .c(new_n18_), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x2), .O(new_n22_));
  nand2  g05(.a(x3), .b(x1), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n22_), .c(new_n21_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x5), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x4), .O(new_n26_));
  inv1   g09(.a(x5), .O(new_n27_));
  nor2   g10(.a(x2), .b(x1), .O(new_n28_));
  aoi21  g11(.a(new_n28_), .b(new_n21_), .c(new_n27_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(x4), .c(x6), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n26_), .c(new_n20_), .O(z0));
  nand2  g14(.a(x3), .b(x2), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n18_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(x1), .O(new_n34_));
  nand2  g17(.a(x3), .b(new_n22_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(x4), .c(x6), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n34_), .c(new_n21_), .O(new_n37_));
  oai21  g20(.a(x6), .b(x4), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x5), .O(new_n39_));
  nand3  g22(.a(x6), .b(x4), .c(x3), .O(new_n40_));
  nand2  g23(.a(x6), .b(x3), .O(new_n41_));
  nand3  g24(.a(new_n18_), .b(x4), .c(x1), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n41_), .c(new_n22_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  aoi21  g27(.a(new_n18_), .b(new_n44_), .c(new_n23_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n43_), .c(x0), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n27_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n39_), .O(z1));
  nand2  g32(.a(x5), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nor2   g34(.a(x3), .b(x2), .O(new_n52_));
  oai22  g35(.a(new_n52_), .b(new_n21_), .c(new_n32_), .d(new_n27_), .O(new_n53_));
  and2   g36(.a(new_n53_), .b(x1), .O(new_n54_));
  nand2  g37(.a(x2), .b(x1), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n21_), .O(new_n56_));
  nor2   g39(.a(x5), .b(x1), .O(new_n57_));
  nor2   g40(.a(x5), .b(x0), .O(new_n58_));
  nor3   g41(.a(new_n58_), .b(new_n57_), .c(new_n44_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n56_), .c(x6), .O(new_n60_));
  oai21  g43(.a(new_n54_), .b(new_n51_), .c(new_n60_), .O(new_n61_));
  nand2  g44(.a(new_n27_), .b(new_n22_), .O(new_n62_));
  aoi21  g45(.a(new_n62_), .b(x0), .c(x3), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x4), .O(new_n64_));
  nor2   g47(.a(new_n58_), .b(new_n28_), .O(new_n65_));
  inv1   g48(.a(new_n65_), .O(new_n66_));
  aoi21  g49(.a(new_n66_), .b(new_n50_), .c(new_n44_), .O(new_n67_));
  oai21  g50(.a(new_n65_), .b(new_n51_), .c(x3), .O(new_n68_));
  oai21  g51(.a(new_n68_), .b(new_n67_), .c(x6), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n64_), .c(new_n61_), .O(z2));
  inv1   g53(.a(new_n50_), .O(new_n71_));
  aoi21  g54(.a(x6), .b(x2), .c(x1), .O(new_n72_));
  nor2   g55(.a(x6), .b(x2), .O(new_n73_));
  aoi21  g56(.a(new_n73_), .b(new_n21_), .c(new_n72_), .O(new_n74_));
  nor2   g57(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  oai22  g58(.a(new_n75_), .b(new_n63_), .c(x5), .d(x0), .O(new_n76_));
  nand2  g59(.a(new_n58_), .b(new_n33_), .O(new_n77_));
  oai21  g60(.a(new_n52_), .b(new_n50_), .c(new_n77_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(x1), .O(new_n79_));
  oai21  g62(.a(new_n58_), .b(new_n71_), .c(x2), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(x3), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(x6), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(z3));
  inv1   g66(.a(x3), .O(z8));
  oai21  g67(.a(new_n73_), .b(z8), .c(new_n21_), .O(new_n85_));
  aoi21  g68(.a(x6), .b(x2), .c(new_n52_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(x1), .O(new_n88_));
  oai21  g71(.a(new_n72_), .b(z8), .c(new_n33_), .O(new_n89_));
  aoi21  g72(.a(new_n18_), .b(x1), .c(new_n21_), .O(new_n90_));
  inv1   g73(.a(x1), .O(new_n91_));
  aoi21  g74(.a(x6), .b(new_n91_), .c(new_n73_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(new_n89_), .c(new_n88_), .O(z4));
  aoi21  g77(.a(new_n22_), .b(x1), .c(z8), .O(new_n95_));
  inv1   g78(.a(new_n52_), .O(new_n96_));
  nor2   g79(.a(new_n96_), .b(x6), .O(new_n97_));
  oai21  g80(.a(new_n97_), .b(new_n95_), .c(x0), .O(new_n98_));
  nand2  g81(.a(x6), .b(z8), .O(new_n99_));
  nand3  g82(.a(new_n99_), .b(new_n96_), .c(new_n21_), .O(new_n100_));
  oai21  g83(.a(new_n100_), .b(new_n95_), .c(new_n98_), .O(z5));
  nand3  g84(.a(x3), .b(new_n22_), .c(new_n91_), .O(new_n102_));
  nand2  g85(.a(new_n35_), .b(x1), .O(new_n103_));
  nand3  g86(.a(new_n103_), .b(new_n102_), .c(new_n99_), .O(z6));
  aoi21  g87(.a(x3), .b(x2), .c(new_n97_), .O(z7));
  nand2  g88(.a(new_n55_), .b(new_n18_), .O(new_n106_));
  nand2  g89(.a(new_n106_), .b(x3), .O(new_n107_));
  nand2  g90(.a(new_n18_), .b(x0), .O(new_n108_));
  aoi21  g91(.a(new_n108_), .b(new_n107_), .c(new_n19_), .O(z9));
endmodule


