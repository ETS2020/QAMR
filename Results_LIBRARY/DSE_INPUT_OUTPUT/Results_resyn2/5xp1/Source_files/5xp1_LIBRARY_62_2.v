// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  nand3  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(x6), .c(new_n19_), .O(new_n24_));
  inv1   g07(.a(new_n22_), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x1), .c(x6), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x4), .c(new_n20_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n24_), .c(new_n18_), .O(z0));
  nand2  g11(.a(x5), .b(new_n20_), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n26_), .c(new_n19_), .O(z9));
  nand2  g14(.a(new_n23_), .b(x6), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x5), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x5), .O(new_n35_));
  nor2   g18(.a(x2), .b(x1), .O(new_n36_));
  nor2   g19(.a(new_n36_), .b(new_n20_), .O(new_n37_));
  aoi21  g20(.a(new_n37_), .b(new_n35_), .c(x4), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(new_n33_), .c(z9), .O(z1));
  nand2  g22(.a(x5), .b(x0), .O(new_n40_));
  nand2  g23(.a(new_n22_), .b(new_n20_), .O(new_n41_));
  inv1   g24(.a(x2), .O(new_n42_));
  inv1   g25(.a(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g27(.a(new_n18_), .b(new_n20_), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n44_), .c(new_n41_), .d(x1), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n40_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n36_), .b(new_n35_), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n47_), .c(new_n19_), .O(new_n50_));
  nand2  g33(.a(x5), .b(x4), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  oai21  g35(.a(x4), .b(x1), .c(x6), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n22_), .O(new_n54_));
  nor2   g37(.a(x5), .b(new_n19_), .O(new_n55_));
  nor2   g38(.a(x6), .b(x1), .O(new_n56_));
  aoi22  g39(.a(new_n56_), .b(x4), .c(x6), .d(new_n18_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(new_n58_));
  nand4  g41(.a(new_n23_), .b(x6), .c(x5), .d(x4), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  aoi21  g43(.a(new_n58_), .b(new_n20_), .c(new_n60_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n50_), .O(z2));
  nor2   g45(.a(new_n34_), .b(new_n42_), .O(new_n63_));
  xnor2a g46(.a(x5), .b(x0), .O(new_n64_));
  oai22  g47(.a(new_n64_), .b(new_n63_), .c(new_n29_), .d(x3), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n21_), .O(new_n66_));
  oai21  g49(.a(x3), .b(x0), .c(x2), .O(new_n67_));
  inv1   g50(.a(new_n67_), .O(new_n68_));
  inv1   g51(.a(new_n64_), .O(new_n69_));
  nor2   g52(.a(new_n69_), .b(new_n56_), .O(new_n70_));
  aoi21  g53(.a(new_n70_), .b(new_n68_), .c(new_n55_), .O(new_n71_));
  nand2  g54(.a(new_n18_), .b(x0), .O(new_n72_));
  oai22  g55(.a(new_n72_), .b(new_n44_), .c(new_n29_), .d(new_n25_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n34_), .O(new_n74_));
  oai21  g57(.a(new_n43_), .b(new_n20_), .c(new_n34_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n64_), .c(x1), .O(new_n76_));
  nand4  g59(.a(new_n76_), .b(new_n74_), .c(new_n71_), .d(new_n66_), .O(z3));
  nand3  g60(.a(new_n44_), .b(new_n41_), .c(x6), .O(new_n78_));
  oai21  g61(.a(new_n75_), .b(new_n68_), .c(new_n78_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x1), .O(new_n80_));
  nand2  g63(.a(new_n68_), .b(x6), .O(new_n81_));
  aoi21  g64(.a(new_n67_), .b(new_n34_), .c(x1), .O(new_n82_));
  aoi21  g65(.a(new_n82_), .b(new_n81_), .c(new_n55_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n80_), .O(z4));
  nand2  g67(.a(new_n44_), .b(new_n22_), .O(new_n85_));
  nor2   g68(.a(new_n85_), .b(new_n36_), .O(new_n86_));
  aoi21  g69(.a(new_n86_), .b(new_n20_), .c(new_n55_), .O(new_n87_));
  oai21  g70(.a(new_n86_), .b(new_n20_), .c(new_n87_), .O(z5));
  inv1   g71(.a(new_n55_), .O(new_n89_));
  nand3  g72(.a(x3), .b(new_n42_), .c(new_n21_), .O(new_n90_));
  oai21  g73(.a(new_n43_), .b(x2), .c(x1), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z6));
  nand2  g75(.a(new_n85_), .b(new_n89_), .O(z7));
  nor2   g76(.a(new_n55_), .b(x3), .O(z8));
endmodule


