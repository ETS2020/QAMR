// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n29_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n91_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  aoi21  g02(.a(x3), .b(x2), .c(x0), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n18_), .c(new_n19_), .O(new_n21_));
  inv1   g04(.a(x5), .O(new_n22_));
  inv1   g05(.a(x6), .O(new_n23_));
  nor2   g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  nand3  g08(.a(x6), .b(x5), .c(x4), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(z9));
  aoi21  g10(.a(new_n25_), .b(new_n18_), .c(z9), .O(z0));
  inv1   g11(.a(x2), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(x6), .c(x0), .O(new_n31_));
  nor2   g14(.a(x5), .b(x4), .O(new_n32_));
  aoi22  g15(.a(new_n32_), .b(new_n31_), .c(new_n24_), .d(new_n21_), .O(z1));
  inv1   g16(.a(x0), .O(new_n34_));
  nand2  g17(.a(new_n22_), .b(new_n34_), .O(new_n35_));
  oai21  g18(.a(x3), .b(x0), .c(x2), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n19_), .O(new_n37_));
  nand2  g20(.a(x5), .b(x0), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n18_), .O(new_n39_));
  aoi21  g22(.a(new_n37_), .b(new_n35_), .c(new_n39_), .O(new_n40_));
  nand2  g23(.a(new_n20_), .b(new_n19_), .O(new_n41_));
  oai21  g24(.a(x2), .b(x1), .c(x0), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n22_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(x4), .O(new_n44_));
  inv1   g27(.a(new_n44_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n40_), .c(x6), .O(new_n46_));
  nand2  g29(.a(x3), .b(x2), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nor2   g31(.a(x3), .b(x2), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n48_), .c(x1), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  nor2   g35(.a(x6), .b(x4), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n35_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n46_), .O(z2));
  nand2  g38(.a(new_n53_), .b(new_n49_), .O(new_n56_));
  nand2  g39(.a(new_n23_), .b(x4), .O(new_n57_));
  aoi21  g40(.a(x6), .b(x2), .c(x1), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n56_), .c(new_n34_), .O(new_n60_));
  nand3  g43(.a(x6), .b(x1), .c(new_n34_), .O(new_n61_));
  inv1   g44(.a(new_n61_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n60_), .c(new_n22_), .O(new_n63_));
  oai21  g46(.a(new_n47_), .b(new_n19_), .c(new_n23_), .O(new_n64_));
  aoi21  g47(.a(new_n47_), .b(new_n19_), .c(x0), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g49(.a(x6), .b(x3), .c(x1), .O(new_n67_));
  oai21  g50(.a(x6), .b(new_n18_), .c(x5), .O(new_n68_));
  aoi21  g51(.a(new_n67_), .b(x0), .c(new_n68_), .O(new_n69_));
  inv1   g52(.a(x3), .O(new_n70_));
  oai21  g53(.a(new_n35_), .b(new_n70_), .c(new_n38_), .O(new_n71_));
  nand2  g54(.a(new_n18_), .b(x1), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n23_), .c(new_n29_), .O(new_n73_));
  aoi22  g56(.a(new_n73_), .b(new_n71_), .c(new_n69_), .d(new_n66_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n63_), .O(z3));
  nand3  g58(.a(new_n50_), .b(new_n48_), .c(x6), .O(new_n76_));
  nor2   g59(.a(new_n49_), .b(new_n34_), .O(new_n77_));
  nand2  g60(.a(new_n53_), .b(new_n47_), .O(new_n78_));
  oai21  g61(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  nand2  g62(.a(new_n79_), .b(x1), .O(new_n80_));
  aoi21  g63(.a(new_n36_), .b(new_n23_), .c(x1), .O(new_n81_));
  oai21  g64(.a(new_n53_), .b(new_n36_), .c(new_n81_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n80_), .O(z4));
  nand3  g66(.a(new_n50_), .b(new_n30_), .c(new_n47_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x0), .O(new_n85_));
  or2    g68(.a(new_n84_), .b(x0), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n85_), .c(new_n57_), .O(z5));
  nand3  g70(.a(x3), .b(new_n29_), .c(new_n19_), .O(new_n88_));
  oai21  g71(.a(new_n70_), .b(x2), .c(x1), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n88_), .c(new_n57_), .O(z6));
  nand2  g73(.a(new_n50_), .b(new_n47_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n57_), .O(z7));
  nand2  g75(.a(new_n57_), .b(x3), .O(z8));
endmodule


