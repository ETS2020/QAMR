// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x3), .b(x2), .O(new_n20_));
  nor2   g03(.a(x6), .b(x0), .O(new_n21_));
  oai21  g04(.a(new_n20_), .b(new_n19_), .c(new_n21_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x5), .c(new_n18_), .O(new_n23_));
  inv1   g06(.a(x0), .O(new_n24_));
  inv1   g07(.a(x2), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n19_), .c(new_n24_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x6), .O(new_n27_));
  nand3  g10(.a(x5), .b(new_n18_), .c(x3), .O(new_n28_));
  nor2   g11(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  or2    g12(.a(new_n29_), .b(new_n23_), .O(z0));
  inv1   g13(.a(x6), .O(new_n31_));
  nor2   g14(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  oai21  g15(.a(x6), .b(x3), .c(x1), .O(new_n33_));
  aoi21  g16(.a(new_n33_), .b(new_n25_), .c(new_n24_), .O(new_n34_));
  nand2  g17(.a(x4), .b(x1), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n31_), .c(x5), .O(new_n36_));
  oai21  g19(.a(new_n34_), .b(new_n32_), .c(new_n36_), .O(new_n37_));
  inv1   g20(.a(x3), .O(z8));
  nand2  g21(.a(new_n18_), .b(z8), .O(new_n39_));
  nand2  g22(.a(new_n22_), .b(x4), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n27_), .c(x5), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n39_), .c(new_n37_), .O(z1));
  oai21  g25(.a(x5), .b(x2), .c(x0), .O(new_n43_));
  nand3  g26(.a(x5), .b(x3), .c(x2), .O(new_n44_));
  oai21  g27(.a(x5), .b(x0), .c(x1), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n44_), .c(new_n43_), .d(x6), .O(new_n46_));
  nand2  g29(.a(x5), .b(x2), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n24_), .O(new_n48_));
  nand2  g31(.a(x5), .b(x0), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n19_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n48_), .c(new_n31_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n46_), .c(x4), .O(new_n52_));
  oai22  g35(.a(x5), .b(x0), .c(x2), .d(x1), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(x6), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n51_), .c(x3), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n18_), .O(new_n56_));
  nand3  g39(.a(new_n43_), .b(new_n31_), .c(z8), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n56_), .c(new_n52_), .O(z2));
  nor2   g41(.a(x5), .b(x0), .O(new_n59_));
  nor2   g42(.a(x3), .b(x2), .O(new_n60_));
  nor2   g43(.a(new_n60_), .b(new_n24_), .O(new_n61_));
  xor2a  g44(.a(x5), .b(x0), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  oai22  g46(.a(new_n63_), .b(new_n61_), .c(new_n50_), .d(new_n59_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n31_), .O(new_n65_));
  nand2  g48(.a(x3), .b(x0), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n31_), .O(new_n67_));
  nor2   g50(.a(new_n62_), .b(new_n19_), .O(new_n68_));
  aoi22  g51(.a(new_n68_), .b(new_n67_), .c(new_n18_), .d(z8), .O(new_n69_));
  nor2   g52(.a(new_n50_), .b(new_n59_), .O(new_n70_));
  oai21  g53(.a(x3), .b(x0), .c(x2), .O(new_n71_));
  nand2  g54(.a(new_n59_), .b(x3), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n49_), .O(new_n73_));
  aoi21  g56(.a(new_n31_), .b(new_n19_), .c(new_n25_), .O(new_n74_));
  aoi22  g57(.a(new_n74_), .b(new_n73_), .c(new_n71_), .d(new_n70_), .O(new_n75_));
  nand3  g58(.a(new_n75_), .b(new_n69_), .c(new_n65_), .O(z3));
  aoi21  g59(.a(new_n71_), .b(x6), .c(x1), .O(new_n77_));
  oai21  g60(.a(new_n71_), .b(x6), .c(new_n77_), .O(new_n78_));
  nand2  g61(.a(z8), .b(new_n25_), .O(new_n79_));
  nand2  g62(.a(new_n20_), .b(new_n24_), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n79_), .c(new_n31_), .O(new_n81_));
  nand3  g64(.a(new_n71_), .b(new_n66_), .c(x6), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(x1), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n78_), .c(new_n39_), .O(new_n85_));
  inv1   g68(.a(new_n85_), .O(z4));
  nor2   g69(.a(z8), .b(x2), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(x1), .O(new_n88_));
  nand3  g71(.a(x4), .b(z8), .c(x2), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n24_), .O(new_n91_));
  aoi21  g74(.a(new_n25_), .b(x1), .c(z8), .O(new_n92_));
  nand2  g75(.a(new_n60_), .b(x4), .O(new_n93_));
  inv1   g76(.a(new_n93_), .O(new_n94_));
  oai21  g77(.a(new_n94_), .b(new_n92_), .c(x0), .O(new_n95_));
  nand2  g78(.a(new_n95_), .b(new_n91_), .O(z5));
  inv1   g79(.a(new_n87_), .O(new_n97_));
  nand2  g80(.a(new_n88_), .b(new_n39_), .O(new_n98_));
  aoi21  g81(.a(new_n97_), .b(new_n19_), .c(new_n98_), .O(z6));
  nand2  g82(.a(new_n89_), .b(new_n97_), .O(z7));
  aoi21  g83(.a(new_n18_), .b(x3), .c(new_n23_), .O(z9));
endmodule


