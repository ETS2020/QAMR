// Benchmark "FAU" written by ABC on Thu Aug  6 21:25:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  nand3  g02(.a(x3), .b(x2), .c(x1), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x5), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x4), .O(new_n23_));
  inv1   g06(.a(x1), .O(new_n24_));
  aoi21  g07(.a(x3), .b(x2), .c(x0), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(x5), .c(x4), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n18_), .c(new_n23_), .O(z0));
  nor2   g11(.a(x6), .b(x4), .O(new_n29_));
  inv1   g12(.a(x4), .O(new_n30_));
  nand2  g13(.a(x3), .b(x2), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n24_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n30_), .c(new_n21_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n29_), .c(x5), .O(new_n34_));
  nor2   g17(.a(new_n18_), .b(new_n30_), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  oai21  g20(.a(x6), .b(x3), .c(x1), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g22(.a(x4), .b(x1), .c(x6), .O(new_n40_));
  nor3   g23(.a(new_n40_), .b(x5), .c(new_n19_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n36_), .c(new_n34_), .O(z1));
  inv1   g26(.a(x5), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n19_), .O(new_n45_));
  oai21  g28(.a(x3), .b(x0), .c(x2), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n24_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  aoi21  g31(.a(x5), .b(x0), .c(x4), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n48_), .c(new_n18_), .O(new_n50_));
  oai21  g33(.a(x5), .b(x3), .c(x0), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n37_), .O(new_n52_));
  nand3  g35(.a(x5), .b(x3), .c(x1), .O(new_n53_));
  oai21  g36(.a(x5), .b(x1), .c(x0), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n52_), .c(x4), .O(new_n56_));
  inv1   g39(.a(x3), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n37_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x1), .c(x0), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n29_), .c(new_n22_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nor2   g44(.a(new_n61_), .b(new_n50_), .O(z2));
  nand3  g45(.a(new_n31_), .b(x5), .c(new_n24_), .O(new_n63_));
  nor2   g46(.a(x5), .b(new_n24_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(x6), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n63_), .c(new_n19_), .O(new_n66_));
  aoi22  g49(.a(x6), .b(x1), .c(new_n44_), .d(new_n37_), .O(new_n67_));
  oai21  g50(.a(new_n67_), .b(new_n64_), .c(x0), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n66_), .c(new_n30_), .O(new_n69_));
  nand3  g52(.a(new_n44_), .b(new_n57_), .c(new_n37_), .O(new_n70_));
  aoi21  g53(.a(new_n70_), .b(new_n53_), .c(new_n19_), .O(new_n71_));
  nand2  g54(.a(new_n31_), .b(new_n19_), .O(new_n72_));
  xnor2a g55(.a(x5), .b(x0), .O(new_n73_));
  aoi21  g56(.a(new_n72_), .b(x1), .c(new_n73_), .O(new_n74_));
  oai21  g57(.a(new_n74_), .b(new_n71_), .c(new_n18_), .O(new_n75_));
  inv1   g58(.a(new_n46_), .O(new_n76_));
  nand2  g59(.a(x6), .b(new_n30_), .O(new_n77_));
  oai21  g60(.a(x6), .b(new_n24_), .c(new_n77_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n73_), .c(new_n76_), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n75_), .c(new_n69_), .O(z3));
  aoi21  g63(.a(new_n76_), .b(new_n24_), .c(x6), .O(new_n81_));
  aoi21  g64(.a(new_n46_), .b(new_n24_), .c(new_n77_), .O(new_n82_));
  aoi21  g65(.a(new_n58_), .b(new_n72_), .c(new_n18_), .O(new_n83_));
  nand3  g66(.a(new_n58_), .b(new_n72_), .c(new_n18_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x1), .O(new_n85_));
  oai22  g68(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n81_), .O(z4));
  nand2  g69(.a(new_n57_), .b(x2), .O(new_n87_));
  nand3  g70(.a(x3), .b(new_n37_), .c(x1), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(new_n87_), .c(new_n19_), .O(new_n89_));
  nand2  g72(.a(new_n88_), .b(new_n87_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(x0), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n89_), .c(new_n36_), .O(new_n92_));
  inv1   g75(.a(new_n92_), .O(z5));
  inv1   g76(.a(new_n88_), .O(new_n94_));
  aoi21  g77(.a(x3), .b(new_n37_), .c(x1), .O(new_n95_));
  nor3   g78(.a(new_n95_), .b(new_n94_), .c(new_n35_), .O(z6));
  nand2  g79(.a(new_n58_), .b(new_n31_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n36_), .O(z7));
  nand2  g81(.a(new_n36_), .b(x3), .O(z8));
  aoi21  g82(.a(new_n22_), .b(new_n18_), .c(new_n30_), .O(z9));
endmodule


