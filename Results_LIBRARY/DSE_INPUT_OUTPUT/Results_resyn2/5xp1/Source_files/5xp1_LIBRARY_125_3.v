// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n81_, new_n84_;
  nand2  g00(.a(x6), .b(x5), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(new_n23_));
  inv1   g06(.a(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n19_), .c(x4), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n18_), .O(z0));
  inv1   g09(.a(x4), .O(new_n27_));
  inv1   g10(.a(x6), .O(new_n28_));
  oai21  g11(.a(new_n24_), .b(new_n27_), .c(new_n28_), .O(new_n29_));
  nor2   g12(.a(x3), .b(x2), .O(new_n30_));
  nor2   g13(.a(new_n30_), .b(new_n21_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x4), .c(x0), .O(new_n32_));
  oai21  g15(.a(x2), .b(x1), .c(x0), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  aoi21  g17(.a(new_n34_), .b(x6), .c(x5), .O(new_n35_));
  aoi22  g18(.a(new_n35_), .b(new_n32_), .c(new_n29_), .d(x5), .O(z1));
  inv1   g19(.a(x2), .O(new_n37_));
  oai21  g20(.a(x5), .b(x3), .c(x0), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g22(.a(x3), .O(new_n40_));
  oai21  g23(.a(new_n19_), .b(new_n40_), .c(new_n20_), .O(new_n41_));
  nand2  g24(.a(x5), .b(x0), .O(new_n42_));
  aoi21  g25(.a(new_n42_), .b(new_n21_), .c(x6), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n41_), .c(new_n39_), .O(new_n44_));
  nand2  g27(.a(new_n33_), .b(x6), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n44_), .c(x4), .O(new_n46_));
  nand2  g29(.a(new_n40_), .b(new_n37_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n28_), .c(x0), .O(new_n48_));
  inv1   g31(.a(new_n22_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x5), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n48_), .c(new_n21_), .O(new_n51_));
  nand2  g34(.a(new_n45_), .b(new_n42_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n51_), .c(new_n27_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n46_), .c(new_n18_), .O(z2));
  nand2  g37(.a(new_n31_), .b(x0), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n23_), .c(x5), .O(new_n56_));
  oai21  g39(.a(new_n31_), .b(new_n20_), .c(new_n19_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n56_), .c(new_n28_), .O(new_n58_));
  oai21  g41(.a(new_n49_), .b(x1), .c(x6), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n24_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n33_), .c(new_n19_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n58_), .O(z3));
  nand2  g45(.a(new_n22_), .b(new_n20_), .O(new_n63_));
  aoi21  g46(.a(new_n63_), .b(new_n47_), .c(new_n28_), .O(new_n64_));
  nand2  g47(.a(x6), .b(new_n19_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x1), .O(new_n67_));
  oai21  g50(.a(x3), .b(x0), .c(x2), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  or2    g52(.a(new_n68_), .b(new_n28_), .O(new_n70_));
  nand3  g53(.a(new_n70_), .b(new_n69_), .c(new_n21_), .O(new_n71_));
  oai21  g54(.a(new_n67_), .b(new_n64_), .c(new_n71_), .O(z4));
  oai21  g55(.a(new_n40_), .b(new_n21_), .c(new_n37_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n22_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(x0), .O(new_n75_));
  nand3  g58(.a(new_n73_), .b(new_n22_), .c(new_n20_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n75_), .c(new_n18_), .O(z5));
  nand2  g60(.a(x3), .b(new_n37_), .O(new_n78_));
  oai21  g61(.a(new_n78_), .b(new_n21_), .c(new_n18_), .O(new_n79_));
  aoi21  g62(.a(new_n78_), .b(new_n21_), .c(new_n79_), .O(z6));
  nand2  g63(.a(new_n22_), .b(new_n18_), .O(new_n81_));
  nor2   g64(.a(new_n81_), .b(new_n30_), .O(z7));
  nand2  g65(.a(new_n18_), .b(x3), .O(z8));
  nand4  g66(.a(new_n23_), .b(new_n28_), .c(x5), .d(x4), .O(new_n84_));
  inv1   g67(.a(new_n84_), .O(z9));
endmodule


