// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n92_, new_n93_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x0), .O(new_n19_));
  inv1   g02(.a(x6), .O(new_n20_));
  nand3  g03(.a(x3), .b(x2), .c(x1), .O(new_n21_));
  nand4  g04(.a(new_n21_), .b(new_n20_), .c(x5), .d(new_n19_), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(x5), .c(new_n18_), .O(z0));
  inv1   g06(.a(x5), .O(new_n24_));
  inv1   g07(.a(new_n22_), .O(new_n25_));
  inv1   g08(.a(x1), .O(new_n26_));
  oai21  g09(.a(x3), .b(x2), .c(x0), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n26_), .c(new_n20_), .O(new_n28_));
  aoi21  g11(.a(new_n28_), .b(new_n24_), .c(new_n25_), .O(new_n29_));
  oai21  g12(.a(x6), .b(x5), .c(new_n18_), .O(new_n30_));
  oai21  g13(.a(new_n29_), .b(new_n18_), .c(new_n30_), .O(z1));
  aoi21  g14(.a(x5), .b(x0), .c(x6), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  nand3  g16(.a(x5), .b(x3), .c(x2), .O(new_n34_));
  oai21  g17(.a(x5), .b(x2), .c(x0), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n34_), .c(new_n20_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n18_), .c(new_n33_), .O(new_n37_));
  nand3  g20(.a(x5), .b(x3), .c(x2), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(new_n27_), .c(x4), .O(new_n39_));
  aoi21  g22(.a(new_n24_), .b(new_n19_), .c(new_n20_), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n39_), .c(x1), .O(new_n41_));
  nand2  g24(.a(new_n34_), .b(new_n27_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x1), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n32_), .c(x4), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n41_), .c(new_n37_), .O(z2));
  nand3  g28(.a(x5), .b(x3), .c(x1), .O(new_n46_));
  inv1   g29(.a(x2), .O(new_n47_));
  inv1   g30(.a(x3), .O(new_n48_));
  nand3  g31(.a(new_n24_), .b(new_n48_), .c(new_n47_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n46_), .c(new_n19_), .O(new_n50_));
  xnor2a g33(.a(x5), .b(x0), .O(new_n51_));
  nand2  g34(.a(x3), .b(x2), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x5), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(x1), .c(new_n51_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n50_), .c(new_n20_), .O(new_n55_));
  nand3  g38(.a(x6), .b(x5), .c(x1), .O(new_n56_));
  nand3  g39(.a(new_n24_), .b(new_n47_), .c(new_n26_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n56_), .c(new_n19_), .O(new_n58_));
  nand2  g41(.a(x6), .b(x1), .O(new_n59_));
  nand2  g42(.a(x5), .b(x1), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n19_), .O(new_n61_));
  aoi21  g44(.a(new_n59_), .b(new_n53_), .c(new_n61_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n58_), .c(x4), .O(new_n63_));
  nor2   g46(.a(new_n20_), .b(x4), .O(new_n64_));
  inv1   g47(.a(new_n64_), .O(new_n65_));
  oai21  g48(.a(x3), .b(x0), .c(x2), .O(new_n66_));
  inv1   g49(.a(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n20_), .b(new_n26_), .O(new_n68_));
  nand4  g51(.a(new_n68_), .b(new_n67_), .c(new_n51_), .d(new_n65_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n63_), .c(new_n55_), .O(z3));
  nand3  g53(.a(new_n27_), .b(new_n52_), .c(x6), .O(new_n71_));
  nand2  g54(.a(new_n27_), .b(new_n52_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n20_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n71_), .c(x1), .O(new_n74_));
  aoi21  g57(.a(new_n66_), .b(new_n20_), .c(x1), .O(new_n75_));
  oai21  g58(.a(new_n66_), .b(new_n20_), .c(new_n75_), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n74_), .c(new_n65_), .O(z4));
  nand2  g60(.a(new_n48_), .b(x2), .O(new_n78_));
  nand3  g61(.a(x3), .b(new_n47_), .c(x1), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n78_), .c(new_n19_), .O(new_n80_));
  nand2  g63(.a(new_n79_), .b(new_n78_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(x0), .O(new_n82_));
  nand3  g65(.a(new_n82_), .b(new_n80_), .c(new_n65_), .O(new_n83_));
  inv1   g66(.a(new_n83_), .O(z5));
  inv1   g67(.a(new_n79_), .O(new_n85_));
  aoi21  g68(.a(x3), .b(new_n47_), .c(x1), .O(new_n86_));
  nor3   g69(.a(new_n86_), .b(new_n85_), .c(new_n64_), .O(z6));
  inv1   g70(.a(new_n52_), .O(new_n88_));
  nor2   g71(.a(x3), .b(x2), .O(new_n89_));
  nor3   g72(.a(new_n64_), .b(new_n89_), .c(new_n88_), .O(z7));
  nand2  g73(.a(new_n65_), .b(x3), .O(z8));
  nand2  g74(.a(new_n21_), .b(new_n19_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(x4), .O(new_n93_));
  aoi22  g76(.a(new_n93_), .b(new_n20_), .c(new_n24_), .d(x4), .O(z9));
endmodule


