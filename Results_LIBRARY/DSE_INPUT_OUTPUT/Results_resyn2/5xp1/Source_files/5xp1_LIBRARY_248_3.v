// Benchmark "FAU" written by ABC on Thu Aug  6 21:27:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n104_, new_n105_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand3  g01(.a(x5), .b(x3), .c(x1), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x2), .O(new_n21_));
  inv1   g04(.a(x4), .O(new_n22_));
  oai21  g05(.a(x1), .b(x0), .c(x5), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n18_), .c(new_n22_), .O(new_n24_));
  inv1   g07(.a(x0), .O(new_n25_));
  nand2  g08(.a(new_n18_), .b(new_n25_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(x5), .c(x4), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n24_), .c(new_n21_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z0));
  inv1   g12(.a(x1), .O(new_n30_));
  aoi21  g13(.a(x3), .b(x2), .c(x6), .O(new_n31_));
  nor2   g14(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g15(.a(x4), .b(x2), .c(x6), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n25_), .O(new_n34_));
  oai22  g17(.a(new_n34_), .b(new_n32_), .c(x6), .d(x4), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x5), .O(new_n36_));
  inv1   g19(.a(x5), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  nand2  g21(.a(x6), .b(new_n38_), .O(new_n39_));
  aoi21  g22(.a(x1), .b(x0), .c(x4), .O(new_n40_));
  inv1   g23(.a(x3), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  nand3  g26(.a(new_n18_), .b(x4), .c(x1), .O(new_n44_));
  oai22  g27(.a(new_n44_), .b(new_n43_), .c(new_n40_), .d(new_n39_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n37_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n36_), .O(z1));
  nand2  g30(.a(new_n37_), .b(new_n30_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x0), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n19_), .O(new_n50_));
  nand2  g33(.a(new_n37_), .b(new_n41_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x0), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n38_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n50_), .c(x4), .O(new_n54_));
  nand3  g37(.a(x3), .b(x2), .c(x1), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n25_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x5), .O(new_n57_));
  nand3  g40(.a(new_n42_), .b(x1), .c(x0), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n57_), .c(new_n22_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n54_), .c(new_n18_), .O(new_n60_));
  nand2  g43(.a(x1), .b(x0), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n23_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nand3  g46(.a(new_n61_), .b(new_n23_), .c(x4), .O(new_n64_));
  nand4  g47(.a(new_n64_), .b(new_n63_), .c(x6), .d(new_n38_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n60_), .O(z2));
  nand2  g49(.a(x5), .b(x1), .O(new_n67_));
  nor2   g50(.a(x6), .b(x3), .O(new_n68_));
  oai21  g51(.a(new_n68_), .b(new_n67_), .c(new_n48_), .O(new_n69_));
  nand2  g52(.a(new_n18_), .b(x1), .O(new_n70_));
  aoi21  g53(.a(x5), .b(x1), .c(x0), .O(new_n71_));
  nand3  g54(.a(new_n71_), .b(new_n48_), .c(new_n70_), .O(new_n72_));
  inv1   g55(.a(new_n72_), .O(new_n73_));
  aoi21  g56(.a(new_n69_), .b(x0), .c(new_n73_), .O(new_n74_));
  nor2   g57(.a(new_n38_), .b(new_n30_), .O(new_n75_));
  xnor2a g58(.a(x5), .b(x0), .O(new_n76_));
  nand3  g59(.a(new_n76_), .b(new_n75_), .c(new_n51_), .O(new_n77_));
  nand3  g60(.a(new_n55_), .b(x5), .c(new_n25_), .O(new_n78_));
  oai21  g61(.a(x3), .b(x2), .c(x1), .O(new_n79_));
  nand3  g62(.a(new_n79_), .b(new_n37_), .c(x0), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n18_), .O(new_n82_));
  oai21  g65(.a(new_n74_), .b(x2), .c(new_n82_), .O(z3));
  nand3  g66(.a(new_n43_), .b(new_n31_), .c(x1), .O(new_n84_));
  oai22  g67(.a(new_n18_), .b(new_n41_), .c(new_n38_), .d(x1), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x0), .O(new_n86_));
  inv1   g69(.a(new_n31_), .O(new_n87_));
  oai21  g70(.a(new_n18_), .b(new_n38_), .c(x1), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g72(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(z4));
  nor2   g73(.a(new_n41_), .b(x2), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(x1), .O(new_n92_));
  nand2  g75(.a(new_n68_), .b(x2), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n25_), .O(new_n95_));
  oai21  g78(.a(x6), .b(new_n41_), .c(x2), .O(new_n96_));
  nand3  g79(.a(new_n96_), .b(new_n92_), .c(x0), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n95_), .O(z5));
  inv1   g81(.a(new_n91_), .O(new_n99_));
  inv1   g82(.a(new_n92_), .O(new_n100_));
  aoi21  g83(.a(new_n99_), .b(new_n88_), .c(new_n100_), .O(z6));
  nand2  g84(.a(new_n93_), .b(new_n99_), .O(z7));
  aoi21  g85(.a(x6), .b(x2), .c(x3), .O(z8));
  oai21  g86(.a(new_n19_), .b(new_n22_), .c(new_n18_), .O(new_n104_));
  nand2  g87(.a(new_n104_), .b(x2), .O(new_n105_));
  nand2  g88(.a(new_n105_), .b(new_n27_), .O(z9));
endmodule


