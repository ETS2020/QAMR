// Benchmark "FAU" written by ABC on Thu Aug  6 21:26:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n100_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x5), .O(new_n21_));
  nand3  g04(.a(x3), .b(x2), .c(x1), .O(new_n22_));
  aoi21  g05(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(new_n23_));
  inv1   g06(.a(x4), .O(new_n24_));
  inv1   g07(.a(x1), .O(new_n25_));
  nand2  g08(.a(x3), .b(x2), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n20_), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(x6), .c(x5), .d(new_n24_), .O(new_n28_));
  oai21  g11(.a(new_n23_), .b(new_n19_), .c(new_n28_), .O(z0));
  nor2   g12(.a(x6), .b(x4), .O(new_n30_));
  nand2  g13(.a(new_n22_), .b(new_n20_), .O(new_n31_));
  nand3  g14(.a(new_n26_), .b(new_n24_), .c(new_n25_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(x6), .c(new_n31_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n30_), .c(x5), .O(new_n34_));
  inv1   g17(.a(x2), .O(new_n35_));
  aoi21  g18(.a(x6), .b(new_n24_), .c(x3), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n25_), .c(new_n35_), .O(new_n37_));
  nand2  g20(.a(new_n18_), .b(x1), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x4), .O(new_n39_));
  nor3   g22(.a(new_n30_), .b(x5), .c(new_n20_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n34_), .O(z1));
  oai21  g25(.a(x3), .b(x2), .c(x0), .O(new_n43_));
  nor2   g26(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(new_n23_), .c(new_n30_), .O(new_n45_));
  oai21  g28(.a(new_n24_), .b(x3), .c(x1), .O(new_n46_));
  nand2  g29(.a(x6), .b(x4), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x0), .O(new_n48_));
  aoi21  g31(.a(new_n46_), .b(new_n21_), .c(new_n48_), .O(new_n49_));
  nand2  g32(.a(x5), .b(x1), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x6), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n24_), .O(new_n52_));
  inv1   g35(.a(x3), .O(new_n53_));
  oai21  g36(.a(new_n21_), .b(new_n53_), .c(new_n20_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n39_), .c(x2), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n49_), .c(new_n45_), .O(z2));
  nand3  g40(.a(x6), .b(x5), .c(x1), .O(new_n58_));
  nand3  g41(.a(new_n21_), .b(new_n35_), .c(new_n25_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n58_), .c(new_n20_), .O(new_n60_));
  nand2  g43(.a(new_n26_), .b(x5), .O(new_n61_));
  nand2  g44(.a(x6), .b(x1), .O(new_n62_));
  nand2  g45(.a(new_n50_), .b(new_n20_), .O(new_n63_));
  aoi21  g46(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n60_), .c(new_n24_), .O(new_n65_));
  nand2  g48(.a(x3), .b(x1), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x5), .O(new_n67_));
  oai21  g50(.a(x3), .b(x2), .c(new_n21_), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n67_), .c(x0), .O(new_n69_));
  nand3  g52(.a(new_n26_), .b(x5), .c(new_n20_), .O(new_n70_));
  xor2a  g53(.a(x5), .b(x0), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n25_), .O(new_n72_));
  nand3  g55(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n18_), .O(new_n74_));
  oai21  g57(.a(new_n18_), .b(x4), .c(new_n38_), .O(new_n75_));
  oai21  g58(.a(x3), .b(x0), .c(x2), .O(new_n76_));
  nor2   g59(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(z3));
  aoi21  g62(.a(new_n76_), .b(new_n18_), .c(x1), .O(new_n80_));
  oai21  g63(.a(new_n76_), .b(new_n18_), .c(new_n80_), .O(new_n81_));
  nand2  g64(.a(new_n26_), .b(new_n20_), .O(new_n82_));
  nand2  g65(.a(new_n53_), .b(new_n35_), .O(new_n83_));
  nand3  g66(.a(new_n83_), .b(new_n82_), .c(x6), .O(new_n84_));
  nand3  g67(.a(new_n43_), .b(new_n26_), .c(new_n18_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(x1), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n81_), .c(new_n47_), .O(z4));
  nand2  g71(.a(new_n66_), .b(new_n35_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n26_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(x0), .O(new_n91_));
  nand3  g74(.a(new_n89_), .b(new_n26_), .c(new_n20_), .O(new_n92_));
  nand3  g75(.a(new_n92_), .b(new_n91_), .c(new_n47_), .O(z5));
  nand2  g76(.a(x3), .b(new_n35_), .O(new_n94_));
  oai21  g77(.a(new_n94_), .b(new_n25_), .c(new_n47_), .O(new_n95_));
  aoi21  g78(.a(new_n94_), .b(new_n25_), .c(new_n95_), .O(z6));
  nand2  g79(.a(new_n83_), .b(new_n26_), .O(new_n97_));
  aoi21  g80(.a(x6), .b(x4), .c(new_n97_), .O(z7));
  nand2  g81(.a(new_n47_), .b(x3), .O(z8));
  nand3  g82(.a(new_n23_), .b(new_n18_), .c(x4), .O(new_n100_));
  inv1   g83(.a(new_n100_), .O(z9));
endmodule


