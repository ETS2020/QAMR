// Benchmark "FAU" written by ABC on Sat Aug  8 23:36:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n135_, new_n136_, new_n138_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n146_, new_n147_,
    new_n148_, new_n150_, new_n151_, new_n153_, new_n154_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n168_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n173_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n185_, new_n186_, new_n187_,
    new_n188_, new_n190_, new_n191_, new_n192_, new_n193_, new_n195_,
    new_n196_, new_n197_, new_n198_, new_n200_, new_n201_, new_n202_,
    new_n203_, new_n205_, new_n206_, new_n207_, new_n208_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x07), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x04), .O(new_n85_));
  inv1   g002(.a(x10), .O(new_n86_));
  oai21  g003(.a(x11), .b(x07), .c(new_n86_), .O(new_n87_));
  nor2   g004(.a(new_n87_), .b(new_n85_), .O(z00));
  nand2  g005(.a(x10), .b(x08), .O(new_n89_));
  nor2   g006(.a(new_n84_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n86_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z01));
  nor2   g009(.a(new_n84_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n86_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z02));
  nor2   g012(.a(new_n84_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n86_), .O(new_n97_));
  nor2   g014(.a(new_n97_), .b(new_n96_), .O(z03));
  nor2   g015(.a(new_n84_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n86_), .O(new_n100_));
  nor2   g017(.a(new_n100_), .b(new_n99_), .O(z04));
  nor2   g018(.a(new_n84_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n86_), .O(new_n103_));
  nor2   g020(.a(new_n103_), .b(new_n102_), .O(z05));
  inv1   g021(.a(x17), .O(new_n105_));
  oai21  g022(.a(x18), .b(x10), .c(x08), .O(new_n106_));
  inv1   g023(.a(x08), .O(new_n107_));
  nand2  g024(.a(new_n86_), .b(new_n107_), .O(new_n108_));
  oai21  g025(.a(new_n108_), .b(new_n105_), .c(new_n106_), .O(z06));
  inv1   g026(.a(x18), .O(new_n110_));
  oai21  g027(.a(x19), .b(x10), .c(x08), .O(new_n111_));
  oai21  g028(.a(new_n108_), .b(new_n110_), .c(new_n111_), .O(z07));
  nor2   g029(.a(x20), .b(new_n107_), .O(new_n113_));
  oai21  g030(.a(x19), .b(x08), .c(new_n86_), .O(new_n114_));
  nor2   g031(.a(new_n114_), .b(new_n113_), .O(z08));
  inv1   g032(.a(x21), .O(new_n116_));
  oai21  g033(.a(x20), .b(x08), .c(new_n86_), .O(new_n117_));
  aoi21  g034(.a(new_n116_), .b(x08), .c(new_n117_), .O(z09));
  oai21  g035(.a(x22), .b(x10), .c(x08), .O(new_n119_));
  oai21  g036(.a(new_n108_), .b(new_n116_), .c(new_n119_), .O(z10));
  inv1   g037(.a(x22), .O(new_n121_));
  oai21  g038(.a(x23), .b(x10), .c(x08), .O(new_n122_));
  oai21  g039(.a(new_n108_), .b(new_n121_), .c(new_n122_), .O(z11));
  inv1   g040(.a(x24), .O(new_n124_));
  oai21  g041(.a(x23), .b(x08), .c(new_n86_), .O(new_n125_));
  aoi21  g042(.a(new_n124_), .b(x08), .c(new_n125_), .O(z12));
  oai21  g043(.a(x25), .b(x10), .c(x08), .O(new_n127_));
  oai21  g044(.a(new_n108_), .b(new_n124_), .c(new_n127_), .O(z13));
  inv1   g045(.a(x25), .O(new_n129_));
  oai21  g046(.a(x26), .b(x10), .c(x08), .O(new_n130_));
  oai21  g047(.a(new_n108_), .b(new_n129_), .c(new_n130_), .O(z14));
  inv1   g048(.a(x26), .O(new_n132_));
  oai21  g049(.a(x27), .b(x10), .c(x08), .O(new_n133_));
  oai21  g050(.a(new_n108_), .b(new_n132_), .c(new_n133_), .O(z15));
  inv1   g051(.a(x28), .O(new_n135_));
  oai21  g052(.a(x27), .b(x08), .c(new_n86_), .O(new_n136_));
  aoi21  g053(.a(new_n135_), .b(x08), .c(new_n136_), .O(z16));
  oai21  g054(.a(x29), .b(x10), .c(x08), .O(new_n138_));
  oai21  g055(.a(new_n108_), .b(new_n135_), .c(new_n138_), .O(z17));
  inv1   g056(.a(x29), .O(new_n140_));
  oai21  g057(.a(x30), .b(x10), .c(x08), .O(new_n141_));
  oai21  g058(.a(new_n108_), .b(new_n140_), .c(new_n141_), .O(z18));
  inv1   g059(.a(x00), .O(new_n143_));
  oai21  g060(.a(x30), .b(x08), .c(new_n86_), .O(new_n144_));
  aoi21  g061(.a(x08), .b(new_n143_), .c(new_n144_), .O(z19));
  inv1   g062(.a(x09), .O(new_n146_));
  nor2   g063(.a(x32), .b(new_n146_), .O(new_n147_));
  oai21  g064(.a(x31), .b(x09), .c(new_n86_), .O(new_n148_));
  nor2   g065(.a(new_n148_), .b(new_n147_), .O(z20));
  nor2   g066(.a(x33), .b(new_n146_), .O(new_n150_));
  oai21  g067(.a(x32), .b(x09), .c(new_n86_), .O(new_n151_));
  nor2   g068(.a(new_n151_), .b(new_n150_), .O(z21));
  nor2   g069(.a(x34), .b(new_n146_), .O(new_n153_));
  oai21  g070(.a(x33), .b(x09), .c(new_n86_), .O(new_n154_));
  nor2   g071(.a(new_n154_), .b(new_n153_), .O(z22));
  nor2   g072(.a(x35), .b(new_n146_), .O(new_n156_));
  oai21  g073(.a(x34), .b(x09), .c(new_n86_), .O(new_n157_));
  nor2   g074(.a(new_n157_), .b(new_n156_), .O(z23));
  nor2   g075(.a(x36), .b(new_n146_), .O(new_n159_));
  oai21  g076(.a(x35), .b(x09), .c(new_n86_), .O(new_n160_));
  nor2   g077(.a(new_n160_), .b(new_n159_), .O(z24));
  inv1   g078(.a(x37), .O(new_n162_));
  oai21  g079(.a(x36), .b(x09), .c(new_n86_), .O(new_n163_));
  aoi21  g080(.a(new_n162_), .b(x09), .c(new_n163_), .O(z25));
  nor2   g081(.a(x38), .b(new_n146_), .O(new_n165_));
  oai21  g082(.a(x37), .b(x09), .c(new_n86_), .O(new_n166_));
  oai21  g083(.a(new_n166_), .b(new_n165_), .c(new_n89_), .O(z26));
  inv1   g084(.a(x14), .O(new_n168_));
  inv1   g085(.a(x39), .O(new_n169_));
  nand2  g086(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand2  g087(.a(x14), .b(new_n143_), .O(new_n171_));
  nand2  g088(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  oai21  g089(.a(x38), .b(x09), .c(new_n86_), .O(new_n173_));
  aoi21  g090(.a(new_n172_), .b(x09), .c(new_n173_), .O(z27));
  inv1   g091(.a(x40), .O(new_n175_));
  nand3  g092(.a(new_n175_), .b(new_n168_), .c(x09), .O(new_n176_));
  oai21  g093(.a(x14), .b(new_n146_), .c(new_n169_), .O(new_n177_));
  nand3  g094(.a(new_n177_), .b(new_n176_), .c(new_n86_), .O(new_n178_));
  nand2  g095(.a(new_n178_), .b(new_n89_), .O(z28));
  inv1   g096(.a(x41), .O(new_n180_));
  nand3  g097(.a(new_n180_), .b(new_n168_), .c(x09), .O(new_n181_));
  oai21  g098(.a(x14), .b(new_n146_), .c(new_n175_), .O(new_n182_));
  nand3  g099(.a(new_n182_), .b(new_n181_), .c(new_n86_), .O(new_n183_));
  inv1   g100(.a(new_n183_), .O(z29));
  inv1   g101(.a(x42), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n168_), .c(x09), .O(new_n186_));
  oai21  g103(.a(x14), .b(new_n146_), .c(new_n180_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(new_n86_), .O(new_n188_));
  inv1   g105(.a(new_n188_), .O(z30));
  inv1   g106(.a(x43), .O(new_n190_));
  nand3  g107(.a(new_n190_), .b(new_n168_), .c(x09), .O(new_n191_));
  oai21  g108(.a(x14), .b(new_n146_), .c(new_n185_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n191_), .c(new_n86_), .O(new_n193_));
  inv1   g110(.a(new_n193_), .O(z31));
  inv1   g111(.a(x44), .O(new_n195_));
  nand3  g112(.a(new_n195_), .b(new_n168_), .c(x09), .O(new_n196_));
  oai21  g113(.a(x14), .b(new_n146_), .c(new_n190_), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n196_), .c(new_n86_), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(z32));
  inv1   g116(.a(x45), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n168_), .c(x09), .O(new_n201_));
  oai21  g118(.a(x14), .b(new_n146_), .c(new_n195_), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n201_), .c(new_n86_), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(new_n89_), .O(z33));
  inv1   g121(.a(x46), .O(new_n205_));
  nand3  g122(.a(new_n205_), .b(new_n168_), .c(x09), .O(new_n206_));
  oai21  g123(.a(x14), .b(new_n146_), .c(new_n200_), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n206_), .c(new_n86_), .O(new_n208_));
  nand2  g125(.a(new_n208_), .b(new_n89_), .O(z34));
  nand3  g126(.a(new_n168_), .b(x09), .c(new_n143_), .O(new_n210_));
  oai21  g127(.a(x14), .b(new_n146_), .c(new_n205_), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(new_n210_), .c(new_n86_), .O(new_n212_));
  nand2  g129(.a(new_n212_), .b(new_n89_), .O(z35));
endmodule


