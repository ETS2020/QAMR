// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n138_, new_n139_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n155_, new_n156_, new_n157_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n193_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n199_, new_n200_,
    new_n201_, new_n202_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  oai21  g009(.a(new_n92_), .b(new_n91_), .c(new_n85_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  oai21  g012(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  oai21  g015(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  oai21  g018(.a(new_n101_), .b(new_n100_), .c(new_n85_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  oai21  g021(.a(new_n104_), .b(new_n103_), .c(new_n85_), .O(z05));
  inv1   g022(.a(x08), .O(new_n106_));
  nor2   g023(.a(x18), .b(new_n106_), .O(new_n107_));
  oai21  g024(.a(x17), .b(x08), .c(new_n88_), .O(new_n108_));
  oai21  g025(.a(new_n108_), .b(new_n107_), .c(new_n85_), .O(z06));
  inv1   g026(.a(x19), .O(new_n110_));
  oai21  g027(.a(x18), .b(x08), .c(new_n88_), .O(new_n111_));
  aoi21  g028(.a(new_n110_), .b(x08), .c(new_n111_), .O(z07));
  inv1   g029(.a(x20), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(x08), .O(new_n114_));
  nand2  g031(.a(new_n110_), .b(new_n106_), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(new_n114_), .c(new_n88_), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n85_), .O(z08));
  nor2   g034(.a(x21), .b(new_n106_), .O(new_n118_));
  oai21  g035(.a(x20), .b(x08), .c(new_n88_), .O(new_n119_));
  oai21  g036(.a(new_n119_), .b(new_n118_), .c(new_n85_), .O(z09));
  inv1   g037(.a(x22), .O(new_n121_));
  oai21  g038(.a(x21), .b(x08), .c(new_n88_), .O(new_n122_));
  aoi21  g039(.a(new_n121_), .b(x08), .c(new_n122_), .O(z10));
  inv1   g040(.a(x23), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  nand2  g042(.a(new_n121_), .b(new_n106_), .O(new_n126_));
  nand3  g043(.a(new_n126_), .b(new_n125_), .c(new_n88_), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(new_n85_), .O(z11));
  nor2   g045(.a(x24), .b(new_n106_), .O(new_n129_));
  oai21  g046(.a(x23), .b(x08), .c(new_n88_), .O(new_n130_));
  oai21  g047(.a(new_n130_), .b(new_n129_), .c(new_n85_), .O(z12));
  inv1   g048(.a(x25), .O(new_n132_));
  oai21  g049(.a(x24), .b(x08), .c(new_n88_), .O(new_n133_));
  aoi21  g050(.a(new_n132_), .b(x08), .c(new_n133_), .O(z13));
  nor2   g051(.a(x26), .b(new_n106_), .O(new_n135_));
  oai21  g052(.a(x25), .b(x08), .c(new_n88_), .O(new_n136_));
  oai21  g053(.a(new_n136_), .b(new_n135_), .c(new_n85_), .O(z14));
  nor2   g054(.a(x27), .b(new_n106_), .O(new_n138_));
  oai21  g055(.a(x26), .b(x08), .c(new_n88_), .O(new_n139_));
  nor2   g056(.a(new_n139_), .b(new_n138_), .O(z15));
  nor2   g057(.a(x28), .b(new_n106_), .O(new_n141_));
  oai21  g058(.a(x27), .b(x08), .c(new_n88_), .O(new_n142_));
  nor2   g059(.a(new_n142_), .b(new_n141_), .O(z16));
  nor2   g060(.a(x29), .b(new_n106_), .O(new_n144_));
  oai21  g061(.a(x28), .b(x08), .c(new_n88_), .O(new_n145_));
  nor2   g062(.a(new_n145_), .b(new_n144_), .O(z17));
  inv1   g063(.a(x30), .O(new_n147_));
  oai21  g064(.a(x29), .b(x08), .c(new_n88_), .O(new_n148_));
  aoi21  g065(.a(new_n147_), .b(x08), .c(new_n148_), .O(z18));
  inv1   g066(.a(x00), .O(new_n150_));
  nand2  g067(.a(x08), .b(new_n150_), .O(new_n151_));
  nand2  g068(.a(new_n147_), .b(new_n106_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n151_), .c(new_n88_), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(new_n85_), .O(z19));
  inv1   g071(.a(x32), .O(new_n155_));
  nor2   g072(.a(x31), .b(x10), .O(new_n156_));
  nand2  g073(.a(new_n88_), .b(x09), .O(new_n157_));
  oai22  g074(.a(new_n157_), .b(new_n155_), .c(new_n156_), .d(x09), .O(z20));
  nor2   g075(.a(x33), .b(new_n84_), .O(new_n159_));
  oai21  g076(.a(x32), .b(x09), .c(new_n88_), .O(new_n160_));
  nor2   g077(.a(new_n160_), .b(new_n159_), .O(z21));
  inv1   g078(.a(x34), .O(new_n162_));
  nor2   g079(.a(x33), .b(x10), .O(new_n163_));
  oai22  g080(.a(new_n163_), .b(x09), .c(new_n157_), .d(new_n162_), .O(z22));
  nor2   g081(.a(x35), .b(new_n84_), .O(new_n165_));
  oai21  g082(.a(x34), .b(x09), .c(new_n88_), .O(new_n166_));
  nor2   g083(.a(new_n166_), .b(new_n165_), .O(z23));
  nor2   g084(.a(x36), .b(new_n84_), .O(new_n168_));
  oai21  g085(.a(x35), .b(x09), .c(new_n88_), .O(new_n169_));
  nor2   g086(.a(new_n169_), .b(new_n168_), .O(z24));
  inv1   g087(.a(x37), .O(new_n171_));
  nor2   g088(.a(x36), .b(x10), .O(new_n172_));
  oai22  g089(.a(new_n172_), .b(x09), .c(new_n157_), .d(new_n171_), .O(z25));
  nor2   g090(.a(x38), .b(new_n84_), .O(new_n174_));
  oai21  g091(.a(x37), .b(x09), .c(new_n88_), .O(new_n175_));
  nor2   g092(.a(new_n175_), .b(new_n174_), .O(z26));
  oai21  g093(.a(x38), .b(x10), .c(new_n84_), .O(new_n177_));
  nand2  g094(.a(x14), .b(new_n150_), .O(new_n178_));
  inv1   g095(.a(x14), .O(new_n179_));
  inv1   g096(.a(x39), .O(new_n180_));
  nand2  g097(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nand4  g098(.a(new_n181_), .b(new_n178_), .c(new_n88_), .d(x09), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(new_n177_), .O(z27));
  inv1   g100(.a(x40), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n179_), .c(x09), .O(new_n185_));
  oai21  g102(.a(x14), .b(new_n84_), .c(new_n180_), .O(new_n186_));
  nand3  g103(.a(new_n186_), .b(new_n185_), .c(new_n88_), .O(new_n187_));
  inv1   g104(.a(new_n187_), .O(z28));
  nand2  g105(.a(new_n179_), .b(x09), .O(new_n189_));
  oai21  g106(.a(x41), .b(x14), .c(new_n88_), .O(new_n190_));
  nor2   g107(.a(x40), .b(x10), .O(new_n191_));
  aoi22  g108(.a(new_n191_), .b(new_n189_), .c(new_n190_), .d(x09), .O(z29));
  inv1   g109(.a(x42), .O(new_n193_));
  nand3  g110(.a(new_n193_), .b(new_n179_), .c(x09), .O(new_n194_));
  inv1   g111(.a(x41), .O(new_n195_));
  oai21  g112(.a(x14), .b(new_n84_), .c(new_n195_), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n194_), .c(new_n88_), .O(new_n197_));
  inv1   g114(.a(new_n197_), .O(z30));
  inv1   g115(.a(x43), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n179_), .c(x09), .O(new_n200_));
  oai21  g117(.a(x14), .b(new_n84_), .c(new_n193_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n200_), .c(new_n88_), .O(new_n202_));
  inv1   g119(.a(new_n202_), .O(z31));
  oai21  g120(.a(x44), .b(x14), .c(new_n88_), .O(new_n204_));
  nor2   g121(.a(x43), .b(x10), .O(new_n205_));
  aoi22  g122(.a(new_n205_), .b(new_n189_), .c(new_n204_), .d(x09), .O(z32));
  inv1   g123(.a(x45), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n179_), .c(x09), .O(new_n208_));
  inv1   g125(.a(x44), .O(new_n209_));
  oai21  g126(.a(x14), .b(new_n84_), .c(new_n209_), .O(new_n210_));
  nand3  g127(.a(new_n210_), .b(new_n208_), .c(new_n88_), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(z33));
  oai21  g129(.a(x46), .b(x14), .c(new_n88_), .O(new_n213_));
  nor2   g130(.a(x45), .b(x10), .O(new_n214_));
  aoi22  g131(.a(new_n214_), .b(new_n189_), .c(new_n213_), .d(x09), .O(z34));
  nand3  g132(.a(new_n179_), .b(x09), .c(new_n150_), .O(new_n216_));
  inv1   g133(.a(x46), .O(new_n217_));
  oai21  g134(.a(x14), .b(new_n84_), .c(new_n217_), .O(new_n218_));
  nand3  g135(.a(new_n218_), .b(new_n216_), .c(new_n88_), .O(new_n219_));
  inv1   g136(.a(new_n219_), .O(z35));
endmodule


