// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n161_, new_n162_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n177_, new_n178_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n211_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n218_, new_n219_;
  inv1   g000(.a(x04), .O(new_n84_));
  inv1   g001(.a(x10), .O(new_n85_));
  oai21  g002(.a(x11), .b(x07), .c(new_n85_), .O(new_n86_));
  aoi21  g003(.a(x07), .b(new_n84_), .c(new_n86_), .O(z00));
  inv1   g004(.a(x05), .O(new_n88_));
  oai21  g005(.a(x12), .b(x07), .c(new_n85_), .O(new_n89_));
  aoi21  g006(.a(x07), .b(new_n88_), .c(new_n89_), .O(z01));
  inv1   g007(.a(x06), .O(new_n91_));
  oai21  g008(.a(x13), .b(x07), .c(new_n85_), .O(new_n92_));
  aoi21  g009(.a(x07), .b(new_n91_), .c(new_n92_), .O(z02));
  inv1   g010(.a(x01), .O(new_n94_));
  oai21  g011(.a(x14), .b(x07), .c(new_n85_), .O(new_n95_));
  aoi21  g012(.a(x07), .b(new_n94_), .c(new_n95_), .O(z03));
  inv1   g013(.a(x02), .O(new_n97_));
  oai21  g014(.a(x15), .b(x07), .c(new_n85_), .O(new_n98_));
  aoi21  g015(.a(x07), .b(new_n97_), .c(new_n98_), .O(z04));
  inv1   g016(.a(x03), .O(new_n100_));
  oai21  g017(.a(x16), .b(x07), .c(new_n85_), .O(new_n101_));
  aoi21  g018(.a(x07), .b(new_n100_), .c(new_n101_), .O(z05));
  inv1   g019(.a(x18), .O(new_n103_));
  oai21  g020(.a(x17), .b(x08), .c(new_n85_), .O(new_n104_));
  aoi21  g021(.a(new_n103_), .b(x08), .c(new_n104_), .O(z06));
  inv1   g022(.a(x09), .O(new_n106_));
  nand2  g023(.a(x10), .b(new_n106_), .O(new_n107_));
  inv1   g024(.a(x19), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(x08), .O(new_n109_));
  inv1   g026(.a(x08), .O(new_n110_));
  nand2  g027(.a(new_n103_), .b(new_n110_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n109_), .c(new_n85_), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(new_n107_), .O(z07));
  nor2   g030(.a(x20), .b(new_n110_), .O(new_n114_));
  oai21  g031(.a(x19), .b(x08), .c(new_n85_), .O(new_n115_));
  oai21  g032(.a(new_n115_), .b(new_n114_), .c(new_n107_), .O(z08));
  nor2   g033(.a(x21), .b(new_n110_), .O(new_n117_));
  oai21  g034(.a(x20), .b(x08), .c(new_n85_), .O(new_n118_));
  nor2   g035(.a(new_n118_), .b(new_n117_), .O(z09));
  inv1   g036(.a(x22), .O(new_n120_));
  oai21  g037(.a(x21), .b(x08), .c(new_n85_), .O(new_n121_));
  aoi21  g038(.a(new_n120_), .b(x08), .c(new_n121_), .O(z10));
  inv1   g039(.a(x23), .O(new_n123_));
  nand2  g040(.a(new_n123_), .b(x08), .O(new_n124_));
  nand2  g041(.a(new_n120_), .b(new_n110_), .O(new_n125_));
  nand3  g042(.a(new_n125_), .b(new_n124_), .c(new_n85_), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(new_n107_), .O(z11));
  inv1   g044(.a(x24), .O(new_n128_));
  nand2  g045(.a(new_n128_), .b(x08), .O(new_n129_));
  nand2  g046(.a(new_n123_), .b(new_n110_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n129_), .c(new_n85_), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(new_n107_), .O(z12));
  inv1   g049(.a(x25), .O(new_n133_));
  nand2  g050(.a(new_n133_), .b(x08), .O(new_n134_));
  nand2  g051(.a(new_n128_), .b(new_n110_), .O(new_n135_));
  nand3  g052(.a(new_n135_), .b(new_n134_), .c(new_n85_), .O(new_n136_));
  nand2  g053(.a(new_n136_), .b(new_n107_), .O(z13));
  inv1   g054(.a(x26), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(x08), .O(new_n139_));
  nand2  g056(.a(new_n133_), .b(new_n110_), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n85_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n107_), .O(z14));
  nor2   g059(.a(x27), .b(new_n110_), .O(new_n143_));
  oai21  g060(.a(x26), .b(x08), .c(new_n85_), .O(new_n144_));
  oai21  g061(.a(new_n144_), .b(new_n143_), .c(new_n107_), .O(z15));
  inv1   g062(.a(x28), .O(new_n146_));
  oai21  g063(.a(x27), .b(x08), .c(new_n85_), .O(new_n147_));
  aoi21  g064(.a(new_n146_), .b(x08), .c(new_n147_), .O(z16));
  nor2   g065(.a(x29), .b(new_n110_), .O(new_n149_));
  oai21  g066(.a(x28), .b(x08), .c(new_n85_), .O(new_n150_));
  oai21  g067(.a(new_n150_), .b(new_n149_), .c(new_n107_), .O(z17));
  nor2   g068(.a(x30), .b(new_n110_), .O(new_n152_));
  oai21  g069(.a(x29), .b(x08), .c(new_n85_), .O(new_n153_));
  nor2   g070(.a(new_n153_), .b(new_n152_), .O(z18));
  nor2   g071(.a(new_n110_), .b(x00), .O(new_n155_));
  oai21  g072(.a(x30), .b(x08), .c(new_n85_), .O(new_n156_));
  nor2   g073(.a(new_n156_), .b(new_n155_), .O(z19));
  nor2   g074(.a(x32), .b(new_n106_), .O(new_n158_));
  oai21  g075(.a(x31), .b(x09), .c(new_n85_), .O(new_n159_));
  nor2   g076(.a(new_n159_), .b(new_n158_), .O(z20));
  nor2   g077(.a(x33), .b(new_n106_), .O(new_n161_));
  oai21  g078(.a(x32), .b(x09), .c(new_n85_), .O(new_n162_));
  nor2   g079(.a(new_n162_), .b(new_n161_), .O(z21));
  nor2   g080(.a(x34), .b(new_n106_), .O(new_n164_));
  oai21  g081(.a(x33), .b(x09), .c(new_n85_), .O(new_n165_));
  nor2   g082(.a(new_n165_), .b(new_n164_), .O(z22));
  nor2   g083(.a(x35), .b(new_n106_), .O(new_n167_));
  oai21  g084(.a(x34), .b(x09), .c(new_n85_), .O(new_n168_));
  nor2   g085(.a(new_n168_), .b(new_n167_), .O(z23));
  inv1   g086(.a(x36), .O(new_n170_));
  nor2   g087(.a(x35), .b(x10), .O(new_n171_));
  nand2  g088(.a(new_n85_), .b(x09), .O(new_n172_));
  oai22  g089(.a(new_n172_), .b(new_n170_), .c(new_n171_), .d(x09), .O(z24));
  inv1   g090(.a(x37), .O(new_n174_));
  nor2   g091(.a(x36), .b(x10), .O(new_n175_));
  oai22  g092(.a(new_n175_), .b(x09), .c(new_n172_), .d(new_n174_), .O(z25));
  nor2   g093(.a(x38), .b(new_n106_), .O(new_n177_));
  oai21  g094(.a(x37), .b(x09), .c(new_n85_), .O(new_n178_));
  nor2   g095(.a(new_n178_), .b(new_n177_), .O(z26));
  nand2  g096(.a(x14), .b(x00), .O(new_n180_));
  inv1   g097(.a(x14), .O(new_n181_));
  aoi21  g098(.a(x39), .b(new_n181_), .c(new_n106_), .O(new_n182_));
  oai21  g099(.a(x38), .b(x09), .c(new_n85_), .O(new_n183_));
  aoi21  g100(.a(new_n182_), .b(new_n180_), .c(new_n183_), .O(z27));
  inv1   g101(.a(x40), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n181_), .c(x09), .O(new_n186_));
  inv1   g103(.a(x39), .O(new_n187_));
  oai21  g104(.a(x14), .b(new_n106_), .c(new_n187_), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n186_), .c(new_n85_), .O(new_n189_));
  inv1   g106(.a(new_n189_), .O(z28));
  nand2  g107(.a(new_n181_), .b(x09), .O(new_n191_));
  oai21  g108(.a(x41), .b(x14), .c(new_n85_), .O(new_n192_));
  nor2   g109(.a(x40), .b(x10), .O(new_n193_));
  aoi22  g110(.a(new_n193_), .b(new_n191_), .c(new_n192_), .d(x09), .O(z29));
  oai21  g111(.a(x42), .b(x14), .c(new_n85_), .O(new_n195_));
  nor2   g112(.a(x41), .b(x10), .O(new_n196_));
  aoi22  g113(.a(new_n196_), .b(new_n191_), .c(new_n195_), .d(x09), .O(z30));
  inv1   g114(.a(x43), .O(new_n198_));
  nand3  g115(.a(new_n198_), .b(new_n181_), .c(x09), .O(new_n199_));
  inv1   g116(.a(x42), .O(new_n200_));
  oai21  g117(.a(x14), .b(new_n106_), .c(new_n200_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n199_), .c(new_n85_), .O(new_n202_));
  inv1   g119(.a(new_n202_), .O(z31));
  oai21  g120(.a(x44), .b(x14), .c(new_n85_), .O(new_n204_));
  nor2   g121(.a(x43), .b(x10), .O(new_n205_));
  aoi22  g122(.a(new_n205_), .b(new_n191_), .c(new_n204_), .d(x09), .O(z32));
  inv1   g123(.a(x45), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n181_), .c(x09), .O(new_n208_));
  inv1   g125(.a(x44), .O(new_n209_));
  oai21  g126(.a(x14), .b(new_n106_), .c(new_n209_), .O(new_n210_));
  nand3  g127(.a(new_n210_), .b(new_n208_), .c(new_n85_), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(z33));
  inv1   g129(.a(x46), .O(new_n213_));
  nand3  g130(.a(new_n213_), .b(new_n181_), .c(x09), .O(new_n214_));
  oai21  g131(.a(x14), .b(new_n106_), .c(new_n207_), .O(new_n215_));
  nand3  g132(.a(new_n215_), .b(new_n214_), .c(new_n85_), .O(new_n216_));
  inv1   g133(.a(new_n216_), .O(z34));
  oai21  g134(.a(x14), .b(x00), .c(new_n85_), .O(new_n218_));
  nor2   g135(.a(x46), .b(x10), .O(new_n219_));
  aoi22  g136(.a(new_n219_), .b(new_n191_), .c(new_n218_), .d(x09), .O(z35));
endmodule


