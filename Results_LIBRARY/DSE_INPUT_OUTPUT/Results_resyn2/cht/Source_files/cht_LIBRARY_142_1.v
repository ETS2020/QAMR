// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  nor2   g009(.a(new_n92_), .b(new_n91_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  oai21  g012(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  nor2   g015(.a(new_n98_), .b(new_n97_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  nor2   g018(.a(new_n101_), .b(new_n100_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  nor2   g021(.a(new_n104_), .b(new_n103_), .O(z05));
  inv1   g022(.a(x08), .O(new_n106_));
  nor2   g023(.a(x18), .b(new_n106_), .O(new_n107_));
  oai21  g024(.a(x17), .b(x08), .c(new_n88_), .O(new_n108_));
  oai21  g025(.a(new_n108_), .b(new_n107_), .c(new_n85_), .O(z06));
  nor2   g026(.a(x19), .b(new_n106_), .O(new_n110_));
  oai21  g027(.a(x18), .b(x08), .c(new_n88_), .O(new_n111_));
  nor2   g028(.a(new_n111_), .b(new_n110_), .O(z07));
  inv1   g029(.a(x20), .O(new_n113_));
  oai21  g030(.a(x19), .b(x08), .c(new_n88_), .O(new_n114_));
  aoi21  g031(.a(new_n113_), .b(x08), .c(new_n114_), .O(z08));
  nor2   g032(.a(x21), .b(new_n106_), .O(new_n116_));
  oai21  g033(.a(x20), .b(x08), .c(new_n88_), .O(new_n117_));
  oai21  g034(.a(new_n117_), .b(new_n116_), .c(new_n85_), .O(z09));
  inv1   g035(.a(x22), .O(new_n119_));
  oai21  g036(.a(x21), .b(x08), .c(new_n88_), .O(new_n120_));
  aoi21  g037(.a(new_n119_), .b(x08), .c(new_n120_), .O(z10));
  nor2   g038(.a(x23), .b(new_n106_), .O(new_n122_));
  oai21  g039(.a(x22), .b(x08), .c(new_n88_), .O(new_n123_));
  oai21  g040(.a(new_n123_), .b(new_n122_), .c(new_n85_), .O(z11));
  nor2   g041(.a(x24), .b(new_n106_), .O(new_n125_));
  oai21  g042(.a(x23), .b(x08), .c(new_n88_), .O(new_n126_));
  nor2   g043(.a(new_n126_), .b(new_n125_), .O(z12));
  inv1   g044(.a(x25), .O(new_n128_));
  oai21  g045(.a(x24), .b(x08), .c(new_n88_), .O(new_n129_));
  aoi21  g046(.a(new_n128_), .b(x08), .c(new_n129_), .O(z13));
  inv1   g047(.a(x26), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(x08), .O(new_n132_));
  nand2  g049(.a(new_n128_), .b(new_n106_), .O(new_n133_));
  nand3  g050(.a(new_n133_), .b(new_n132_), .c(new_n88_), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(new_n85_), .O(z14));
  nor2   g052(.a(x27), .b(new_n106_), .O(new_n136_));
  oai21  g053(.a(x26), .b(x08), .c(new_n88_), .O(new_n137_));
  oai21  g054(.a(new_n137_), .b(new_n136_), .c(new_n85_), .O(z15));
  nor2   g055(.a(x28), .b(new_n106_), .O(new_n139_));
  oai21  g056(.a(x27), .b(x08), .c(new_n88_), .O(new_n140_));
  nor2   g057(.a(new_n140_), .b(new_n139_), .O(z16));
  inv1   g058(.a(x29), .O(new_n142_));
  oai21  g059(.a(x28), .b(x08), .c(new_n88_), .O(new_n143_));
  aoi21  g060(.a(new_n142_), .b(x08), .c(new_n143_), .O(z17));
  nor2   g061(.a(x30), .b(new_n106_), .O(new_n145_));
  oai21  g062(.a(x29), .b(x08), .c(new_n88_), .O(new_n146_));
  oai21  g063(.a(new_n146_), .b(new_n145_), .c(new_n85_), .O(z18));
  inv1   g064(.a(x00), .O(new_n148_));
  oai21  g065(.a(x30), .b(x08), .c(new_n88_), .O(new_n149_));
  aoi21  g066(.a(x08), .b(new_n148_), .c(new_n149_), .O(z19));
  nor2   g067(.a(x32), .b(new_n84_), .O(new_n151_));
  oai21  g068(.a(x31), .b(x09), .c(new_n88_), .O(new_n152_));
  nor2   g069(.a(new_n152_), .b(new_n151_), .O(z20));
  nor2   g070(.a(x33), .b(new_n84_), .O(new_n154_));
  oai21  g071(.a(x32), .b(x09), .c(new_n88_), .O(new_n155_));
  nor2   g072(.a(new_n155_), .b(new_n154_), .O(z21));
  inv1   g073(.a(x34), .O(new_n157_));
  nor2   g074(.a(x33), .b(x10), .O(new_n158_));
  nand2  g075(.a(new_n88_), .b(x09), .O(new_n159_));
  oai22  g076(.a(new_n159_), .b(new_n157_), .c(new_n158_), .d(x09), .O(z22));
  inv1   g077(.a(x35), .O(new_n161_));
  nor2   g078(.a(x34), .b(x10), .O(new_n162_));
  oai22  g079(.a(new_n162_), .b(x09), .c(new_n159_), .d(new_n161_), .O(z23));
  nor2   g080(.a(x36), .b(new_n84_), .O(new_n164_));
  oai21  g081(.a(x35), .b(x09), .c(new_n88_), .O(new_n165_));
  nor2   g082(.a(new_n165_), .b(new_n164_), .O(z24));
  nor2   g083(.a(x37), .b(new_n84_), .O(new_n167_));
  oai21  g084(.a(x36), .b(x09), .c(new_n88_), .O(new_n168_));
  nor2   g085(.a(new_n168_), .b(new_n167_), .O(z25));
  nor2   g086(.a(x38), .b(new_n84_), .O(new_n170_));
  oai21  g087(.a(x37), .b(x09), .c(new_n88_), .O(new_n171_));
  nor2   g088(.a(new_n171_), .b(new_n170_), .O(z26));
  oai21  g089(.a(x38), .b(x10), .c(new_n84_), .O(new_n173_));
  nand2  g090(.a(x14), .b(new_n148_), .O(new_n174_));
  inv1   g091(.a(x14), .O(new_n175_));
  inv1   g092(.a(x39), .O(new_n176_));
  nand2  g093(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand4  g094(.a(new_n177_), .b(new_n174_), .c(new_n88_), .d(x09), .O(new_n178_));
  nand2  g095(.a(new_n178_), .b(new_n173_), .O(z27));
  inv1   g096(.a(x40), .O(new_n180_));
  nand3  g097(.a(new_n180_), .b(new_n175_), .c(x09), .O(new_n181_));
  oai21  g098(.a(x14), .b(new_n84_), .c(new_n176_), .O(new_n182_));
  nand3  g099(.a(new_n182_), .b(new_n181_), .c(new_n88_), .O(new_n183_));
  inv1   g100(.a(new_n183_), .O(z28));
  nand2  g101(.a(new_n175_), .b(x09), .O(new_n185_));
  oai21  g102(.a(x41), .b(x14), .c(new_n88_), .O(new_n186_));
  nor2   g103(.a(x40), .b(x10), .O(new_n187_));
  aoi22  g104(.a(new_n187_), .b(new_n185_), .c(new_n186_), .d(x09), .O(z29));
  inv1   g105(.a(x42), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n175_), .c(x09), .O(new_n190_));
  inv1   g107(.a(x41), .O(new_n191_));
  oai21  g108(.a(x14), .b(new_n84_), .c(new_n191_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n190_), .c(new_n88_), .O(new_n193_));
  inv1   g110(.a(new_n193_), .O(z30));
  oai21  g111(.a(x43), .b(x14), .c(new_n88_), .O(new_n195_));
  nor2   g112(.a(x42), .b(x10), .O(new_n196_));
  aoi22  g113(.a(new_n196_), .b(new_n185_), .c(new_n195_), .d(x09), .O(z31));
  inv1   g114(.a(x44), .O(new_n198_));
  nand3  g115(.a(new_n198_), .b(new_n175_), .c(x09), .O(new_n199_));
  inv1   g116(.a(x43), .O(new_n200_));
  oai21  g117(.a(x14), .b(new_n84_), .c(new_n200_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n199_), .c(new_n88_), .O(new_n202_));
  inv1   g119(.a(new_n202_), .O(z32));
  oai21  g120(.a(x45), .b(x14), .c(new_n88_), .O(new_n204_));
  nor2   g121(.a(x44), .b(x10), .O(new_n205_));
  aoi22  g122(.a(new_n205_), .b(new_n185_), .c(new_n204_), .d(x09), .O(z33));
  oai21  g123(.a(x46), .b(x14), .c(new_n88_), .O(new_n207_));
  nor2   g124(.a(x45), .b(x10), .O(new_n208_));
  aoi22  g125(.a(new_n208_), .b(new_n185_), .c(new_n207_), .d(x09), .O(z34));
  oai21  g126(.a(x14), .b(x00), .c(new_n88_), .O(new_n210_));
  nor2   g127(.a(x46), .b(x10), .O(new_n211_));
  aoi22  g128(.a(new_n211_), .b(new_n185_), .c(new_n210_), .d(x09), .O(z35));
endmodule


