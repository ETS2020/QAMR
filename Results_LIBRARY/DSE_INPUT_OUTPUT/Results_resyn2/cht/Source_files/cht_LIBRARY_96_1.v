// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n203_, new_n204_, new_n206_, new_n207_, new_n209_,
    new_n210_, new_n212_, new_n213_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  oai21  g009(.a(new_n92_), .b(new_n91_), .c(new_n85_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  oai21  g012(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  oai21  g015(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  nor2   g018(.a(new_n101_), .b(new_n100_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  nor2   g021(.a(new_n104_), .b(new_n103_), .O(z05));
  inv1   g022(.a(x18), .O(new_n106_));
  nand2  g023(.a(new_n106_), .b(x08), .O(new_n107_));
  inv1   g024(.a(x08), .O(new_n108_));
  inv1   g025(.a(x17), .O(new_n109_));
  nand2  g026(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g027(.a(new_n110_), .b(new_n107_), .c(new_n88_), .O(new_n111_));
  nand2  g028(.a(new_n111_), .b(new_n85_), .O(z06));
  inv1   g029(.a(x19), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(x08), .O(new_n114_));
  nand2  g031(.a(new_n106_), .b(new_n108_), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(new_n114_), .c(new_n88_), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n85_), .O(z07));
  nor2   g034(.a(x20), .b(new_n108_), .O(new_n118_));
  oai21  g035(.a(x19), .b(x08), .c(new_n88_), .O(new_n119_));
  oai21  g036(.a(new_n119_), .b(new_n118_), .c(new_n85_), .O(z08));
  nor2   g037(.a(x21), .b(new_n108_), .O(new_n121_));
  oai21  g038(.a(x20), .b(x08), .c(new_n88_), .O(new_n122_));
  nor2   g039(.a(new_n122_), .b(new_n121_), .O(z09));
  inv1   g040(.a(x22), .O(new_n124_));
  oai21  g041(.a(x21), .b(x08), .c(new_n88_), .O(new_n125_));
  aoi21  g042(.a(new_n124_), .b(x08), .c(new_n125_), .O(z10));
  inv1   g043(.a(x23), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(x08), .O(new_n128_));
  nand2  g045(.a(new_n124_), .b(new_n108_), .O(new_n129_));
  nand3  g046(.a(new_n129_), .b(new_n128_), .c(new_n88_), .O(new_n130_));
  nand2  g047(.a(new_n130_), .b(new_n85_), .O(z11));
  inv1   g048(.a(x24), .O(new_n132_));
  nand2  g049(.a(new_n132_), .b(x08), .O(new_n133_));
  nand2  g050(.a(new_n127_), .b(new_n108_), .O(new_n134_));
  nand3  g051(.a(new_n134_), .b(new_n133_), .c(new_n88_), .O(new_n135_));
  nand2  g052(.a(new_n135_), .b(new_n85_), .O(z12));
  nor2   g053(.a(x25), .b(new_n108_), .O(new_n137_));
  oai21  g054(.a(x24), .b(x08), .c(new_n88_), .O(new_n138_));
  oai21  g055(.a(new_n138_), .b(new_n137_), .c(new_n85_), .O(z13));
  inv1   g056(.a(x26), .O(new_n140_));
  oai21  g057(.a(x25), .b(x08), .c(new_n88_), .O(new_n141_));
  aoi21  g058(.a(new_n140_), .b(x08), .c(new_n141_), .O(z14));
  nor2   g059(.a(x27), .b(new_n108_), .O(new_n143_));
  oai21  g060(.a(x26), .b(x08), .c(new_n88_), .O(new_n144_));
  oai21  g061(.a(new_n144_), .b(new_n143_), .c(new_n85_), .O(z15));
  nor2   g062(.a(x28), .b(new_n108_), .O(new_n146_));
  oai21  g063(.a(x27), .b(x08), .c(new_n88_), .O(new_n147_));
  nor2   g064(.a(new_n147_), .b(new_n146_), .O(z16));
  nor2   g065(.a(x29), .b(new_n108_), .O(new_n149_));
  oai21  g066(.a(x28), .b(x08), .c(new_n88_), .O(new_n150_));
  nor2   g067(.a(new_n150_), .b(new_n149_), .O(z17));
  nor2   g068(.a(x30), .b(new_n108_), .O(new_n152_));
  oai21  g069(.a(x29), .b(x08), .c(new_n88_), .O(new_n153_));
  nor2   g070(.a(new_n153_), .b(new_n152_), .O(z18));
  nor2   g071(.a(new_n108_), .b(x00), .O(new_n155_));
  oai21  g072(.a(x30), .b(x08), .c(new_n88_), .O(new_n156_));
  nor2   g073(.a(new_n156_), .b(new_n155_), .O(z19));
  aoi21  g074(.a(x32), .b(new_n88_), .c(new_n84_), .O(new_n158_));
  nor3   g075(.a(x31), .b(x10), .c(x09), .O(new_n159_));
  nor2   g076(.a(new_n159_), .b(new_n158_), .O(z20));
  nor2   g077(.a(x33), .b(new_n84_), .O(new_n161_));
  oai21  g078(.a(x32), .b(x09), .c(new_n88_), .O(new_n162_));
  nor2   g079(.a(new_n162_), .b(new_n161_), .O(z21));
  nor2   g080(.a(x34), .b(new_n84_), .O(new_n164_));
  oai21  g081(.a(x33), .b(x09), .c(new_n88_), .O(new_n165_));
  nor2   g082(.a(new_n165_), .b(new_n164_), .O(z22));
  nor2   g083(.a(x35), .b(new_n84_), .O(new_n167_));
  oai21  g084(.a(x34), .b(x09), .c(new_n88_), .O(new_n168_));
  nor2   g085(.a(new_n168_), .b(new_n167_), .O(z23));
  nor2   g086(.a(x36), .b(new_n84_), .O(new_n170_));
  oai21  g087(.a(x35), .b(x09), .c(new_n88_), .O(new_n171_));
  nor2   g088(.a(new_n171_), .b(new_n170_), .O(z24));
  nor2   g089(.a(x37), .b(new_n84_), .O(new_n173_));
  oai21  g090(.a(x36), .b(x09), .c(new_n88_), .O(new_n174_));
  nor2   g091(.a(new_n174_), .b(new_n173_), .O(z25));
  nor2   g092(.a(x38), .b(new_n84_), .O(new_n176_));
  oai21  g093(.a(x37), .b(x09), .c(new_n88_), .O(new_n177_));
  nor2   g094(.a(new_n177_), .b(new_n176_), .O(z26));
  nand2  g095(.a(x14), .b(x00), .O(new_n179_));
  inv1   g096(.a(x14), .O(new_n180_));
  aoi21  g097(.a(x39), .b(new_n180_), .c(new_n84_), .O(new_n181_));
  oai21  g098(.a(x38), .b(x09), .c(new_n88_), .O(new_n182_));
  aoi21  g099(.a(new_n181_), .b(new_n179_), .c(new_n182_), .O(z27));
  inv1   g100(.a(x40), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n180_), .c(x09), .O(new_n185_));
  inv1   g102(.a(x39), .O(new_n186_));
  oai21  g103(.a(x14), .b(new_n84_), .c(new_n186_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n185_), .c(new_n88_), .O(new_n188_));
  inv1   g105(.a(new_n188_), .O(z28));
  nand2  g106(.a(new_n180_), .b(x09), .O(new_n190_));
  oai21  g107(.a(x41), .b(x14), .c(new_n88_), .O(new_n191_));
  nor2   g108(.a(x40), .b(x10), .O(new_n192_));
  aoi22  g109(.a(new_n192_), .b(new_n190_), .c(new_n191_), .d(x09), .O(z29));
  oai21  g110(.a(x42), .b(x14), .c(new_n88_), .O(new_n194_));
  nor2   g111(.a(x41), .b(x10), .O(new_n195_));
  aoi22  g112(.a(new_n195_), .b(new_n190_), .c(new_n194_), .d(x09), .O(z30));
  inv1   g113(.a(x43), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n180_), .c(x09), .O(new_n198_));
  inv1   g115(.a(x42), .O(new_n199_));
  oai21  g116(.a(x14), .b(new_n84_), .c(new_n199_), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n198_), .c(new_n88_), .O(new_n201_));
  inv1   g118(.a(new_n201_), .O(z31));
  oai21  g119(.a(x44), .b(x14), .c(new_n88_), .O(new_n203_));
  nor2   g120(.a(x43), .b(x10), .O(new_n204_));
  aoi22  g121(.a(new_n204_), .b(new_n190_), .c(new_n203_), .d(x09), .O(z32));
  oai21  g122(.a(x45), .b(x14), .c(new_n88_), .O(new_n206_));
  nor2   g123(.a(x44), .b(x10), .O(new_n207_));
  aoi22  g124(.a(new_n207_), .b(new_n190_), .c(new_n206_), .d(x09), .O(z33));
  oai21  g125(.a(x46), .b(x14), .c(new_n88_), .O(new_n209_));
  nor2   g126(.a(x45), .b(x10), .O(new_n210_));
  aoi22  g127(.a(new_n210_), .b(new_n190_), .c(new_n209_), .d(x09), .O(z34));
  oai21  g128(.a(x14), .b(x00), .c(new_n88_), .O(new_n212_));
  nor2   g129(.a(x46), .b(x10), .O(new_n213_));
  aoi22  g130(.a(new_n213_), .b(new_n190_), .c(new_n212_), .d(x09), .O(z35));
endmodule


