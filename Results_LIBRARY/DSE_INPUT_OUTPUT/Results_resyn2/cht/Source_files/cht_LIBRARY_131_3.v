// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n173_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n208_, new_n209_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n217_, new_n218_, new_n219_, new_n220_, new_n222_,
    new_n223_;
  inv1   g000(.a(x07), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x04), .O(new_n85_));
  inv1   g002(.a(x10), .O(new_n86_));
  oai21  g003(.a(x11), .b(x07), .c(new_n86_), .O(new_n87_));
  nor2   g004(.a(new_n87_), .b(new_n85_), .O(z00));
  nand2  g005(.a(x10), .b(x09), .O(new_n89_));
  nor2   g006(.a(new_n84_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n86_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z01));
  nor2   g009(.a(new_n84_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n86_), .O(new_n94_));
  nor2   g011(.a(new_n94_), .b(new_n93_), .O(z02));
  nor2   g012(.a(new_n84_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n86_), .O(new_n97_));
  oai21  g014(.a(new_n97_), .b(new_n96_), .c(new_n89_), .O(z03));
  nor2   g015(.a(new_n84_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n86_), .O(new_n100_));
  oai21  g017(.a(new_n100_), .b(new_n99_), .c(new_n89_), .O(z04));
  nor2   g018(.a(new_n84_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n86_), .O(new_n103_));
  nor2   g020(.a(new_n103_), .b(new_n102_), .O(z05));
  inv1   g021(.a(x08), .O(new_n105_));
  nor2   g022(.a(x18), .b(new_n105_), .O(new_n106_));
  oai21  g023(.a(x17), .b(x08), .c(new_n86_), .O(new_n107_));
  oai21  g024(.a(new_n107_), .b(new_n106_), .c(new_n89_), .O(z06));
  nor2   g025(.a(x19), .b(new_n105_), .O(new_n109_));
  oai21  g026(.a(x18), .b(x08), .c(new_n86_), .O(new_n110_));
  nor2   g027(.a(new_n110_), .b(new_n109_), .O(z07));
  nor2   g028(.a(x20), .b(new_n105_), .O(new_n112_));
  oai21  g029(.a(x19), .b(x08), .c(new_n86_), .O(new_n113_));
  nor2   g030(.a(new_n113_), .b(new_n112_), .O(z08));
  nor2   g031(.a(x21), .b(new_n105_), .O(new_n115_));
  oai21  g032(.a(x20), .b(x08), .c(new_n86_), .O(new_n116_));
  nor2   g033(.a(new_n116_), .b(new_n115_), .O(z09));
  inv1   g034(.a(x22), .O(new_n118_));
  oai21  g035(.a(x21), .b(x08), .c(new_n86_), .O(new_n119_));
  aoi21  g036(.a(new_n118_), .b(x08), .c(new_n119_), .O(z10));
  inv1   g037(.a(x23), .O(new_n121_));
  nand2  g038(.a(new_n121_), .b(x08), .O(new_n122_));
  nand2  g039(.a(new_n118_), .b(new_n105_), .O(new_n123_));
  nand3  g040(.a(new_n123_), .b(new_n122_), .c(new_n86_), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(new_n89_), .O(z11));
  inv1   g042(.a(x24), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(x08), .O(new_n127_));
  nand2  g044(.a(new_n121_), .b(new_n105_), .O(new_n128_));
  nand3  g045(.a(new_n128_), .b(new_n127_), .c(new_n86_), .O(new_n129_));
  nand2  g046(.a(new_n129_), .b(new_n89_), .O(z12));
  inv1   g047(.a(x25), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(x08), .O(new_n132_));
  nand2  g049(.a(new_n126_), .b(new_n105_), .O(new_n133_));
  nand3  g050(.a(new_n133_), .b(new_n132_), .c(new_n86_), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(new_n89_), .O(z13));
  nor2   g052(.a(x26), .b(new_n105_), .O(new_n136_));
  oai21  g053(.a(x25), .b(x08), .c(new_n86_), .O(new_n137_));
  oai21  g054(.a(new_n137_), .b(new_n136_), .c(new_n89_), .O(z14));
  inv1   g055(.a(x27), .O(new_n139_));
  oai21  g056(.a(x26), .b(x08), .c(new_n86_), .O(new_n140_));
  aoi21  g057(.a(new_n139_), .b(x08), .c(new_n140_), .O(z15));
  inv1   g058(.a(x28), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(x08), .O(new_n143_));
  nand2  g060(.a(new_n139_), .b(new_n105_), .O(new_n144_));
  nand3  g061(.a(new_n144_), .b(new_n143_), .c(new_n86_), .O(new_n145_));
  nand2  g062(.a(new_n145_), .b(new_n89_), .O(z16));
  nor2   g063(.a(x29), .b(new_n105_), .O(new_n147_));
  oai21  g064(.a(x28), .b(x08), .c(new_n86_), .O(new_n148_));
  oai21  g065(.a(new_n148_), .b(new_n147_), .c(new_n89_), .O(z17));
  inv1   g066(.a(x30), .O(new_n150_));
  oai21  g067(.a(x29), .b(x08), .c(new_n86_), .O(new_n151_));
  aoi21  g068(.a(new_n150_), .b(x08), .c(new_n151_), .O(z18));
  nor2   g069(.a(new_n105_), .b(x00), .O(new_n153_));
  nand2  g070(.a(new_n150_), .b(new_n105_), .O(new_n154_));
  nand2  g071(.a(new_n154_), .b(new_n86_), .O(new_n155_));
  oai21  g072(.a(new_n155_), .b(new_n153_), .c(new_n89_), .O(z19));
  oai21  g073(.a(x32), .b(x10), .c(x09), .O(new_n157_));
  nor2   g074(.a(x10), .b(x09), .O(new_n158_));
  nand2  g075(.a(new_n158_), .b(x31), .O(new_n159_));
  nand2  g076(.a(new_n159_), .b(new_n157_), .O(z20));
  inv1   g077(.a(x33), .O(new_n161_));
  oai21  g078(.a(x32), .b(x09), .c(new_n86_), .O(new_n162_));
  aoi21  g079(.a(new_n161_), .b(x09), .c(new_n162_), .O(z21));
  inv1   g080(.a(new_n158_), .O(new_n164_));
  oai21  g081(.a(x34), .b(x10), .c(x09), .O(new_n165_));
  oai21  g082(.a(new_n164_), .b(new_n161_), .c(new_n165_), .O(z22));
  oai21  g083(.a(x35), .b(x10), .c(x09), .O(new_n167_));
  nand2  g084(.a(new_n158_), .b(x34), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(new_n167_), .O(z23));
  inv1   g086(.a(x36), .O(new_n170_));
  oai21  g087(.a(x35), .b(x09), .c(new_n86_), .O(new_n171_));
  aoi21  g088(.a(new_n170_), .b(x09), .c(new_n171_), .O(z24));
  oai21  g089(.a(x37), .b(x10), .c(x09), .O(new_n173_));
  oai21  g090(.a(new_n164_), .b(new_n170_), .c(new_n173_), .O(z25));
  oai21  g091(.a(x38), .b(x10), .c(x09), .O(new_n175_));
  nand2  g092(.a(new_n158_), .b(x37), .O(new_n176_));
  nand2  g093(.a(new_n176_), .b(new_n175_), .O(z26));
  nand2  g094(.a(new_n158_), .b(x38), .O(new_n178_));
  nand2  g095(.a(x14), .b(x00), .O(new_n179_));
  inv1   g096(.a(x14), .O(new_n180_));
  nand2  g097(.a(x39), .b(new_n180_), .O(new_n181_));
  nand3  g098(.a(new_n181_), .b(new_n179_), .c(new_n86_), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(x09), .O(new_n183_));
  nand2  g100(.a(new_n183_), .b(new_n178_), .O(z27));
  inv1   g101(.a(x40), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n180_), .c(x09), .O(new_n186_));
  inv1   g103(.a(x39), .O(new_n187_));
  nand2  g104(.a(new_n180_), .b(x09), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n186_), .c(new_n86_), .O(new_n190_));
  inv1   g107(.a(new_n190_), .O(z28));
  inv1   g108(.a(x41), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n180_), .c(x09), .O(new_n193_));
  nand2  g110(.a(new_n188_), .b(new_n185_), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n193_), .c(new_n86_), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(z29));
  inv1   g113(.a(x09), .O(new_n197_));
  nand2  g114(.a(x10), .b(new_n197_), .O(new_n198_));
  nand3  g115(.a(new_n198_), .b(new_n188_), .c(x41), .O(new_n199_));
  aoi21  g116(.a(x42), .b(new_n180_), .c(x10), .O(new_n200_));
  oai21  g117(.a(new_n200_), .b(new_n197_), .c(new_n199_), .O(z30));
  inv1   g118(.a(x43), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n180_), .c(x09), .O(new_n203_));
  inv1   g120(.a(x42), .O(new_n204_));
  nand2  g121(.a(new_n188_), .b(new_n204_), .O(new_n205_));
  nand3  g122(.a(new_n205_), .b(new_n203_), .c(new_n86_), .O(new_n206_));
  inv1   g123(.a(new_n206_), .O(z31));
  nand3  g124(.a(new_n198_), .b(new_n188_), .c(x43), .O(new_n208_));
  aoi21  g125(.a(x44), .b(new_n180_), .c(x10), .O(new_n209_));
  oai21  g126(.a(new_n209_), .b(new_n197_), .c(new_n208_), .O(z32));
  inv1   g127(.a(x45), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(new_n180_), .c(x09), .O(new_n212_));
  inv1   g129(.a(x44), .O(new_n213_));
  nand2  g130(.a(new_n188_), .b(new_n213_), .O(new_n214_));
  nand3  g131(.a(new_n214_), .b(new_n212_), .c(new_n86_), .O(new_n215_));
  inv1   g132(.a(new_n215_), .O(z33));
  inv1   g133(.a(x46), .O(new_n217_));
  nand3  g134(.a(new_n217_), .b(new_n180_), .c(x09), .O(new_n218_));
  nand2  g135(.a(new_n188_), .b(new_n211_), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n218_), .c(new_n86_), .O(new_n220_));
  inv1   g137(.a(new_n220_), .O(z34));
  nand3  g138(.a(new_n198_), .b(new_n188_), .c(x46), .O(new_n222_));
  aoi21  g139(.a(new_n180_), .b(x00), .c(x10), .O(new_n223_));
  oai21  g140(.a(new_n223_), .b(new_n197_), .c(new_n222_), .O(z35));
endmodule


