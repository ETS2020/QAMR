// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n169_, new_n170_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n181_, new_n182_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n202_, new_n203_, new_n204_, new_n205_,
    new_n207_, new_n208_, new_n210_, new_n211_, new_n212_, new_n213_,
    new_n214_, new_n216_, new_n217_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n223_, new_n225_, new_n226_;
  nand2  g000(.a(x10), .b(x09), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nor2   g002(.a(new_n85_), .b(x04), .O(new_n86_));
  inv1   g003(.a(x10), .O(new_n87_));
  oai21  g004(.a(x11), .b(x07), .c(new_n87_), .O(new_n88_));
  oai21  g005(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(z00));
  nor2   g006(.a(new_n85_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n87_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n84_), .O(z01));
  nor2   g009(.a(new_n85_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n87_), .O(new_n94_));
  nor2   g011(.a(new_n94_), .b(new_n93_), .O(z02));
  nor2   g012(.a(new_n85_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n87_), .O(new_n97_));
  oai21  g014(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z03));
  nor2   g015(.a(new_n85_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n87_), .O(new_n100_));
  nor2   g017(.a(new_n100_), .b(new_n99_), .O(z04));
  nor2   g018(.a(new_n85_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n87_), .O(new_n103_));
  nor2   g020(.a(new_n103_), .b(new_n102_), .O(z05));
  inv1   g021(.a(x18), .O(new_n105_));
  oai21  g022(.a(x17), .b(x08), .c(new_n87_), .O(new_n106_));
  aoi21  g023(.a(new_n105_), .b(x08), .c(new_n106_), .O(z06));
  inv1   g024(.a(x19), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(x08), .O(new_n109_));
  inv1   g026(.a(x08), .O(new_n110_));
  nand2  g027(.a(new_n105_), .b(new_n110_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n109_), .c(new_n87_), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(new_n84_), .O(z07));
  inv1   g030(.a(x20), .O(new_n114_));
  nand2  g031(.a(new_n114_), .b(x08), .O(new_n115_));
  nand2  g032(.a(new_n108_), .b(new_n110_), .O(new_n116_));
  nand3  g033(.a(new_n116_), .b(new_n115_), .c(new_n87_), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(new_n84_), .O(z08));
  inv1   g035(.a(x21), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(x08), .O(new_n120_));
  nand2  g037(.a(new_n114_), .b(new_n110_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n120_), .c(new_n87_), .O(new_n122_));
  nand2  g039(.a(new_n122_), .b(new_n84_), .O(z09));
  inv1   g040(.a(x22), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  nand2  g042(.a(new_n119_), .b(new_n110_), .O(new_n126_));
  nand3  g043(.a(new_n126_), .b(new_n125_), .c(new_n87_), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(new_n84_), .O(z10));
  nor2   g045(.a(x23), .b(new_n110_), .O(new_n129_));
  oai21  g046(.a(x22), .b(x08), .c(new_n87_), .O(new_n130_));
  oai21  g047(.a(new_n130_), .b(new_n129_), .c(new_n84_), .O(z11));
  nor2   g048(.a(x24), .b(new_n110_), .O(new_n132_));
  oai21  g049(.a(x23), .b(x08), .c(new_n87_), .O(new_n133_));
  nor2   g050(.a(new_n133_), .b(new_n132_), .O(z12));
  nor2   g051(.a(x25), .b(new_n110_), .O(new_n135_));
  oai21  g052(.a(x24), .b(x08), .c(new_n87_), .O(new_n136_));
  nor2   g053(.a(new_n136_), .b(new_n135_), .O(z13));
  inv1   g054(.a(x26), .O(new_n138_));
  oai21  g055(.a(x25), .b(x08), .c(new_n87_), .O(new_n139_));
  aoi21  g056(.a(new_n138_), .b(x08), .c(new_n139_), .O(z14));
  inv1   g057(.a(x27), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(x08), .O(new_n142_));
  nand2  g059(.a(new_n138_), .b(new_n110_), .O(new_n143_));
  nand3  g060(.a(new_n143_), .b(new_n142_), .c(new_n87_), .O(new_n144_));
  nand2  g061(.a(new_n144_), .b(new_n84_), .O(z15));
  inv1   g062(.a(x28), .O(new_n146_));
  nand2  g063(.a(new_n146_), .b(x08), .O(new_n147_));
  nand2  g064(.a(new_n141_), .b(new_n110_), .O(new_n148_));
  nand3  g065(.a(new_n148_), .b(new_n147_), .c(new_n87_), .O(new_n149_));
  nand2  g066(.a(new_n149_), .b(new_n84_), .O(z16));
  inv1   g067(.a(x29), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(x08), .O(new_n152_));
  nand2  g069(.a(new_n146_), .b(new_n110_), .O(new_n153_));
  nand3  g070(.a(new_n153_), .b(new_n152_), .c(new_n87_), .O(new_n154_));
  nand2  g071(.a(new_n154_), .b(new_n84_), .O(z17));
  nor2   g072(.a(x30), .b(new_n110_), .O(new_n156_));
  oai21  g073(.a(x29), .b(x08), .c(new_n87_), .O(new_n157_));
  oai21  g074(.a(new_n157_), .b(new_n156_), .c(new_n84_), .O(z18));
  nor2   g075(.a(new_n110_), .b(x00), .O(new_n159_));
  oai21  g076(.a(x30), .b(x08), .c(new_n87_), .O(new_n160_));
  nor2   g077(.a(new_n160_), .b(new_n159_), .O(z19));
  inv1   g078(.a(x09), .O(new_n162_));
  nor2   g079(.a(x32), .b(new_n162_), .O(new_n163_));
  oai21  g080(.a(x31), .b(x09), .c(new_n87_), .O(new_n164_));
  nor2   g081(.a(new_n164_), .b(new_n163_), .O(z20));
  oai21  g082(.a(x33), .b(x10), .c(x09), .O(new_n166_));
  nand3  g083(.a(x32), .b(new_n87_), .c(new_n162_), .O(new_n167_));
  nand2  g084(.a(new_n167_), .b(new_n166_), .O(z21));
  nor2   g085(.a(x34), .b(new_n162_), .O(new_n169_));
  oai21  g086(.a(x33), .b(x09), .c(new_n87_), .O(new_n170_));
  nor2   g087(.a(new_n170_), .b(new_n169_), .O(z22));
  nor2   g088(.a(x35), .b(new_n162_), .O(new_n172_));
  oai21  g089(.a(x34), .b(x09), .c(new_n87_), .O(new_n173_));
  nor2   g090(.a(new_n173_), .b(new_n172_), .O(z23));
  nor2   g091(.a(x36), .b(new_n162_), .O(new_n175_));
  oai21  g092(.a(x35), .b(x09), .c(new_n87_), .O(new_n176_));
  nor2   g093(.a(new_n176_), .b(new_n175_), .O(z24));
  nor2   g094(.a(x37), .b(new_n162_), .O(new_n178_));
  oai21  g095(.a(x36), .b(x09), .c(new_n87_), .O(new_n179_));
  nor2   g096(.a(new_n179_), .b(new_n178_), .O(z25));
  nor2   g097(.a(x38), .b(new_n162_), .O(new_n181_));
  oai21  g098(.a(x37), .b(x09), .c(new_n87_), .O(new_n182_));
  nor2   g099(.a(new_n182_), .b(new_n181_), .O(z26));
  nand3  g100(.a(x38), .b(new_n87_), .c(new_n162_), .O(new_n184_));
  nand2  g101(.a(x14), .b(x00), .O(new_n185_));
  inv1   g102(.a(x14), .O(new_n186_));
  nand2  g103(.a(x39), .b(new_n186_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n185_), .c(new_n87_), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(x09), .O(new_n189_));
  nand2  g106(.a(new_n189_), .b(new_n184_), .O(z27));
  nand2  g107(.a(new_n186_), .b(x09), .O(new_n191_));
  nand2  g108(.a(x10), .b(new_n162_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n191_), .c(x39), .O(new_n193_));
  aoi21  g110(.a(x40), .b(new_n186_), .c(x10), .O(new_n194_));
  oai21  g111(.a(new_n194_), .b(new_n162_), .c(new_n193_), .O(z28));
  inv1   g112(.a(x41), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n186_), .c(x09), .O(new_n197_));
  inv1   g114(.a(x40), .O(new_n198_));
  nand2  g115(.a(new_n191_), .b(new_n198_), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n197_), .c(new_n87_), .O(new_n200_));
  inv1   g117(.a(new_n200_), .O(z29));
  inv1   g118(.a(x42), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n186_), .c(x09), .O(new_n203_));
  nand2  g120(.a(new_n191_), .b(new_n196_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n203_), .c(new_n87_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z30));
  nand3  g123(.a(new_n192_), .b(new_n191_), .c(x42), .O(new_n207_));
  aoi21  g124(.a(x43), .b(new_n186_), .c(x10), .O(new_n208_));
  oai21  g125(.a(new_n208_), .b(new_n162_), .c(new_n207_), .O(z31));
  inv1   g126(.a(x44), .O(new_n210_));
  nand3  g127(.a(new_n210_), .b(new_n186_), .c(x09), .O(new_n211_));
  inv1   g128(.a(x43), .O(new_n212_));
  nand2  g129(.a(new_n191_), .b(new_n212_), .O(new_n213_));
  nand3  g130(.a(new_n213_), .b(new_n211_), .c(new_n87_), .O(new_n214_));
  inv1   g131(.a(new_n214_), .O(z32));
  nand3  g132(.a(new_n192_), .b(new_n191_), .c(x44), .O(new_n216_));
  aoi21  g133(.a(x45), .b(new_n186_), .c(x10), .O(new_n217_));
  oai21  g134(.a(new_n217_), .b(new_n162_), .c(new_n216_), .O(z33));
  inv1   g135(.a(x46), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n186_), .c(x09), .O(new_n220_));
  inv1   g137(.a(x45), .O(new_n221_));
  nand2  g138(.a(new_n191_), .b(new_n221_), .O(new_n222_));
  nand3  g139(.a(new_n222_), .b(new_n220_), .c(new_n87_), .O(new_n223_));
  inv1   g140(.a(new_n223_), .O(z34));
  nand3  g141(.a(new_n192_), .b(new_n191_), .c(x46), .O(new_n225_));
  aoi21  g142(.a(new_n186_), .b(x00), .c(x10), .O(new_n226_));
  oai21  g143(.a(new_n226_), .b(new_n162_), .c(new_n225_), .O(z35));
endmodule


