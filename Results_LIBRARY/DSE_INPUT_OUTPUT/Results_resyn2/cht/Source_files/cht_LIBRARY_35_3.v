// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n171_, new_n173_, new_n174_, new_n176_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n196_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n202_, new_n203_, new_n205_, new_n206_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n212_, new_n214_, new_n215_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  nand2  g000(.a(x10), .b(x09), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nor2   g002(.a(new_n85_), .b(x04), .O(new_n86_));
  inv1   g003(.a(x10), .O(new_n87_));
  oai21  g004(.a(x11), .b(x07), .c(new_n87_), .O(new_n88_));
  oai21  g005(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(z00));
  nor2   g006(.a(new_n85_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n87_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n84_), .O(z01));
  nor2   g009(.a(new_n85_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n87_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n84_), .O(z02));
  nor2   g012(.a(new_n85_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n87_), .O(new_n97_));
  oai21  g014(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z03));
  nor2   g015(.a(new_n85_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n87_), .O(new_n100_));
  oai21  g017(.a(new_n100_), .b(new_n99_), .c(new_n84_), .O(z04));
  nor2   g018(.a(new_n85_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n87_), .O(new_n103_));
  oai21  g020(.a(new_n103_), .b(new_n102_), .c(new_n84_), .O(z05));
  inv1   g021(.a(x08), .O(new_n105_));
  nor2   g022(.a(x18), .b(new_n105_), .O(new_n106_));
  oai21  g023(.a(x17), .b(x08), .c(new_n87_), .O(new_n107_));
  oai21  g024(.a(new_n107_), .b(new_n106_), .c(new_n84_), .O(z06));
  inv1   g025(.a(x19), .O(new_n109_));
  oai21  g026(.a(x18), .b(x08), .c(new_n87_), .O(new_n110_));
  aoi21  g027(.a(new_n109_), .b(x08), .c(new_n110_), .O(z07));
  inv1   g028(.a(x20), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(x08), .O(new_n113_));
  nand2  g030(.a(new_n109_), .b(new_n105_), .O(new_n114_));
  nand3  g031(.a(new_n114_), .b(new_n113_), .c(new_n87_), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(new_n84_), .O(z08));
  nor2   g033(.a(x21), .b(new_n105_), .O(new_n117_));
  oai21  g034(.a(x20), .b(x08), .c(new_n87_), .O(new_n118_));
  oai21  g035(.a(new_n118_), .b(new_n117_), .c(new_n84_), .O(z09));
  inv1   g036(.a(x22), .O(new_n120_));
  oai21  g037(.a(x21), .b(x08), .c(new_n87_), .O(new_n121_));
  aoi21  g038(.a(new_n120_), .b(x08), .c(new_n121_), .O(z10));
  inv1   g039(.a(x23), .O(new_n123_));
  nand2  g040(.a(new_n123_), .b(x08), .O(new_n124_));
  nand2  g041(.a(new_n120_), .b(new_n105_), .O(new_n125_));
  nand3  g042(.a(new_n125_), .b(new_n124_), .c(new_n87_), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(new_n84_), .O(z11));
  nor2   g044(.a(x24), .b(new_n105_), .O(new_n128_));
  oai21  g045(.a(x23), .b(x08), .c(new_n87_), .O(new_n129_));
  oai21  g046(.a(new_n129_), .b(new_n128_), .c(new_n84_), .O(z12));
  inv1   g047(.a(x25), .O(new_n131_));
  oai21  g048(.a(x24), .b(x08), .c(new_n87_), .O(new_n132_));
  aoi21  g049(.a(new_n131_), .b(x08), .c(new_n132_), .O(z13));
  nor2   g050(.a(x26), .b(new_n105_), .O(new_n134_));
  oai21  g051(.a(x25), .b(x08), .c(new_n87_), .O(new_n135_));
  oai21  g052(.a(new_n135_), .b(new_n134_), .c(new_n84_), .O(z14));
  nor2   g053(.a(x27), .b(new_n105_), .O(new_n137_));
  oai21  g054(.a(x26), .b(x08), .c(new_n87_), .O(new_n138_));
  nor2   g055(.a(new_n138_), .b(new_n137_), .O(z15));
  nor2   g056(.a(x28), .b(new_n105_), .O(new_n140_));
  oai21  g057(.a(x27), .b(x08), .c(new_n87_), .O(new_n141_));
  nor2   g058(.a(new_n141_), .b(new_n140_), .O(z16));
  nor2   g059(.a(x29), .b(new_n105_), .O(new_n143_));
  oai21  g060(.a(x28), .b(x08), .c(new_n87_), .O(new_n144_));
  nor2   g061(.a(new_n144_), .b(new_n143_), .O(z17));
  inv1   g062(.a(x30), .O(new_n146_));
  oai21  g063(.a(x29), .b(x08), .c(new_n87_), .O(new_n147_));
  aoi21  g064(.a(new_n146_), .b(x08), .c(new_n147_), .O(z18));
  inv1   g065(.a(x00), .O(new_n149_));
  nand2  g066(.a(x08), .b(new_n149_), .O(new_n150_));
  nand2  g067(.a(new_n146_), .b(new_n105_), .O(new_n151_));
  nand3  g068(.a(new_n151_), .b(new_n150_), .c(new_n87_), .O(new_n152_));
  nand2  g069(.a(new_n152_), .b(new_n84_), .O(z19));
  oai21  g070(.a(x32), .b(x10), .c(x09), .O(new_n154_));
  nor2   g071(.a(x10), .b(x09), .O(new_n155_));
  nand2  g072(.a(new_n155_), .b(x31), .O(new_n156_));
  nand2  g073(.a(new_n156_), .b(new_n154_), .O(z20));
  inv1   g074(.a(x33), .O(new_n158_));
  oai21  g075(.a(x32), .b(x09), .c(new_n87_), .O(new_n159_));
  aoi21  g076(.a(new_n158_), .b(x09), .c(new_n159_), .O(z21));
  inv1   g077(.a(new_n155_), .O(new_n161_));
  oai21  g078(.a(x34), .b(x10), .c(x09), .O(new_n162_));
  oai21  g079(.a(new_n161_), .b(new_n158_), .c(new_n162_), .O(z22));
  inv1   g080(.a(x09), .O(new_n164_));
  nor2   g081(.a(x35), .b(new_n164_), .O(new_n165_));
  oai21  g082(.a(x34), .b(x09), .c(new_n87_), .O(new_n166_));
  nor2   g083(.a(new_n166_), .b(new_n165_), .O(z23));
  inv1   g084(.a(x36), .O(new_n168_));
  oai21  g085(.a(x35), .b(x09), .c(new_n87_), .O(new_n169_));
  aoi21  g086(.a(new_n168_), .b(x09), .c(new_n169_), .O(z24));
  oai21  g087(.a(x37), .b(x10), .c(x09), .O(new_n171_));
  oai21  g088(.a(new_n161_), .b(new_n168_), .c(new_n171_), .O(z25));
  nor2   g089(.a(x38), .b(new_n164_), .O(new_n173_));
  oai21  g090(.a(x37), .b(x09), .c(new_n87_), .O(new_n174_));
  nor2   g091(.a(new_n174_), .b(new_n173_), .O(z26));
  nand2  g092(.a(new_n155_), .b(x38), .O(new_n176_));
  nand2  g093(.a(x14), .b(x00), .O(new_n177_));
  inv1   g094(.a(x14), .O(new_n178_));
  nand2  g095(.a(x39), .b(new_n178_), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n177_), .c(new_n87_), .O(new_n180_));
  nand2  g097(.a(new_n180_), .b(x09), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(new_n176_), .O(z27));
  inv1   g099(.a(x40), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n178_), .c(x09), .O(new_n184_));
  inv1   g101(.a(x39), .O(new_n185_));
  nand2  g102(.a(new_n178_), .b(x09), .O(new_n186_));
  nand2  g103(.a(new_n186_), .b(new_n185_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n184_), .c(new_n87_), .O(new_n188_));
  inv1   g105(.a(new_n188_), .O(z28));
  nand2  g106(.a(x10), .b(new_n164_), .O(new_n190_));
  nand3  g107(.a(new_n190_), .b(new_n186_), .c(x40), .O(new_n191_));
  aoi21  g108(.a(x41), .b(new_n178_), .c(x10), .O(new_n192_));
  oai21  g109(.a(new_n192_), .b(new_n164_), .c(new_n191_), .O(z29));
  inv1   g110(.a(x42), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n178_), .c(x09), .O(new_n195_));
  inv1   g112(.a(x41), .O(new_n196_));
  nand2  g113(.a(new_n186_), .b(new_n196_), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n195_), .c(new_n87_), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(z30));
  inv1   g116(.a(x43), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n178_), .c(x09), .O(new_n201_));
  nand2  g118(.a(new_n186_), .b(new_n194_), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n201_), .c(new_n87_), .O(new_n203_));
  inv1   g120(.a(new_n203_), .O(z31));
  nand3  g121(.a(new_n190_), .b(new_n186_), .c(x43), .O(new_n205_));
  aoi21  g122(.a(x44), .b(new_n178_), .c(x10), .O(new_n206_));
  oai21  g123(.a(new_n206_), .b(new_n164_), .c(new_n205_), .O(z32));
  inv1   g124(.a(x45), .O(new_n208_));
  nand3  g125(.a(new_n208_), .b(new_n178_), .c(x09), .O(new_n209_));
  inv1   g126(.a(x44), .O(new_n210_));
  nand2  g127(.a(new_n186_), .b(new_n210_), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(new_n209_), .c(new_n87_), .O(new_n212_));
  inv1   g129(.a(new_n212_), .O(z33));
  nand3  g130(.a(new_n190_), .b(new_n186_), .c(x45), .O(new_n214_));
  aoi21  g131(.a(x46), .b(new_n178_), .c(x10), .O(new_n215_));
  oai21  g132(.a(new_n215_), .b(new_n164_), .c(new_n214_), .O(z34));
  nand3  g133(.a(new_n178_), .b(x09), .c(new_n149_), .O(new_n217_));
  inv1   g134(.a(x46), .O(new_n218_));
  nand2  g135(.a(new_n186_), .b(new_n218_), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n217_), .c(new_n87_), .O(new_n220_));
  inv1   g137(.a(new_n220_), .O(z35));
endmodule


