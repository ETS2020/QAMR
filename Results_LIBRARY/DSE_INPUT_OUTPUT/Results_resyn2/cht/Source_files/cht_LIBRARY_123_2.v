// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n157_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n174_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n188_, new_n189_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n197_, new_n198_, new_n200_,
    new_n201_, new_n203_, new_n204_, new_n205_, new_n206_, new_n207_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n219_, new_n220_, new_n222_, new_n223_,
    new_n224_, new_n225_, new_n226_, new_n228_, new_n229_, new_n231_,
    new_n232_, new_n234_, new_n235_, new_n237_, new_n238_, new_n240_,
    new_n241_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  aoi21  g002(.a(x11), .b(new_n85_), .c(x05), .O(new_n86_));
  aoi21  g003(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nor2   g004(.a(x10), .b(x05), .O(new_n88_));
  nand3  g005(.a(new_n88_), .b(x12), .c(new_n85_), .O(new_n89_));
  inv1   g006(.a(new_n89_), .O(z01));
  inv1   g007(.a(x06), .O(new_n91_));
  nand2  g008(.a(x07), .b(new_n91_), .O(new_n92_));
  inv1   g009(.a(x13), .O(new_n93_));
  nand2  g010(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  nand3  g011(.a(new_n94_), .b(new_n92_), .c(new_n88_), .O(new_n95_));
  inv1   g012(.a(new_n95_), .O(z02));
  nand2  g013(.a(x07), .b(x01), .O(new_n97_));
  aoi21  g014(.a(x14), .b(new_n85_), .c(x05), .O(new_n98_));
  aoi21  g015(.a(new_n98_), .b(new_n97_), .c(x10), .O(z03));
  inv1   g016(.a(x02), .O(new_n100_));
  nand2  g017(.a(x07), .b(new_n100_), .O(new_n101_));
  inv1   g018(.a(x15), .O(new_n102_));
  nand2  g019(.a(new_n102_), .b(new_n85_), .O(new_n103_));
  nand3  g020(.a(new_n103_), .b(new_n101_), .c(new_n88_), .O(new_n104_));
  inv1   g021(.a(new_n104_), .O(z04));
  inv1   g022(.a(x03), .O(new_n106_));
  nand2  g023(.a(x07), .b(new_n106_), .O(new_n107_));
  inv1   g024(.a(x16), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(new_n85_), .O(new_n109_));
  nand3  g026(.a(new_n109_), .b(new_n107_), .c(new_n88_), .O(new_n110_));
  inv1   g027(.a(new_n110_), .O(z05));
  inv1   g028(.a(x18), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(x08), .O(new_n113_));
  inv1   g030(.a(x08), .O(new_n114_));
  inv1   g031(.a(x17), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g033(.a(new_n116_), .b(new_n113_), .c(new_n88_), .O(new_n117_));
  inv1   g034(.a(new_n117_), .O(z06));
  inv1   g035(.a(x19), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(x08), .O(new_n120_));
  nand2  g037(.a(new_n112_), .b(new_n114_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n120_), .c(new_n88_), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z07));
  nand2  g040(.a(x20), .b(x08), .O(new_n124_));
  aoi21  g041(.a(x19), .b(new_n114_), .c(x05), .O(new_n125_));
  aoi21  g042(.a(new_n125_), .b(new_n124_), .c(x10), .O(z08));
  inv1   g043(.a(x21), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(x08), .O(new_n128_));
  inv1   g045(.a(x20), .O(new_n129_));
  nand2  g046(.a(new_n129_), .b(new_n114_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n128_), .c(new_n88_), .O(new_n131_));
  inv1   g048(.a(new_n131_), .O(z09));
  inv1   g049(.a(x22), .O(new_n133_));
  nand2  g050(.a(new_n133_), .b(x08), .O(new_n134_));
  nand2  g051(.a(new_n127_), .b(new_n114_), .O(new_n135_));
  nand3  g052(.a(new_n135_), .b(new_n134_), .c(new_n88_), .O(new_n136_));
  inv1   g053(.a(new_n136_), .O(z10));
  inv1   g054(.a(x23), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(x08), .O(new_n139_));
  nand2  g056(.a(new_n133_), .b(new_n114_), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n88_), .O(new_n141_));
  inv1   g058(.a(new_n141_), .O(z11));
  inv1   g059(.a(x24), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(x08), .O(new_n144_));
  nand2  g061(.a(new_n138_), .b(new_n114_), .O(new_n145_));
  nand3  g062(.a(new_n145_), .b(new_n144_), .c(new_n88_), .O(new_n146_));
  inv1   g063(.a(new_n146_), .O(z12));
  nand2  g064(.a(x25), .b(x08), .O(new_n148_));
  aoi21  g065(.a(x24), .b(new_n114_), .c(x05), .O(new_n149_));
  aoi21  g066(.a(new_n149_), .b(new_n148_), .c(x10), .O(z13));
  inv1   g067(.a(x26), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(x08), .O(new_n152_));
  inv1   g069(.a(x25), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(new_n114_), .O(new_n154_));
  nand3  g071(.a(new_n154_), .b(new_n152_), .c(new_n88_), .O(new_n155_));
  inv1   g072(.a(new_n155_), .O(z14));
  inv1   g073(.a(x27), .O(new_n157_));
  nand2  g074(.a(new_n157_), .b(x08), .O(new_n158_));
  nand2  g075(.a(new_n151_), .b(new_n114_), .O(new_n159_));
  nand3  g076(.a(new_n159_), .b(new_n158_), .c(new_n88_), .O(new_n160_));
  inv1   g077(.a(new_n160_), .O(z15));
  nand2  g078(.a(x28), .b(x08), .O(new_n162_));
  aoi21  g079(.a(x27), .b(new_n114_), .c(x05), .O(new_n163_));
  aoi21  g080(.a(new_n163_), .b(new_n162_), .c(x10), .O(z16));
  inv1   g081(.a(x29), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(x08), .O(new_n166_));
  inv1   g083(.a(x28), .O(new_n167_));
  nand2  g084(.a(new_n167_), .b(new_n114_), .O(new_n168_));
  nand3  g085(.a(new_n168_), .b(new_n166_), .c(new_n88_), .O(new_n169_));
  inv1   g086(.a(new_n169_), .O(z17));
  inv1   g087(.a(x30), .O(new_n171_));
  nand2  g088(.a(new_n171_), .b(x08), .O(new_n172_));
  nand2  g089(.a(new_n165_), .b(new_n114_), .O(new_n173_));
  nand3  g090(.a(new_n173_), .b(new_n172_), .c(new_n88_), .O(new_n174_));
  inv1   g091(.a(new_n174_), .O(z18));
  inv1   g092(.a(x00), .O(new_n176_));
  nand2  g093(.a(x08), .b(new_n176_), .O(new_n177_));
  nand2  g094(.a(new_n171_), .b(new_n114_), .O(new_n178_));
  nand3  g095(.a(new_n178_), .b(new_n177_), .c(new_n88_), .O(new_n179_));
  inv1   g096(.a(new_n179_), .O(z19));
  inv1   g097(.a(x32), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(x09), .O(new_n182_));
  inv1   g099(.a(x09), .O(new_n183_));
  inv1   g100(.a(x31), .O(new_n184_));
  nand2  g101(.a(new_n184_), .b(new_n183_), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n182_), .c(new_n88_), .O(new_n186_));
  inv1   g103(.a(new_n186_), .O(z20));
  nand2  g104(.a(x33), .b(x09), .O(new_n188_));
  aoi21  g105(.a(x32), .b(new_n183_), .c(x05), .O(new_n189_));
  aoi21  g106(.a(new_n189_), .b(new_n188_), .c(x10), .O(z21));
  nand2  g107(.a(x34), .b(x09), .O(new_n191_));
  aoi21  g108(.a(x33), .b(new_n183_), .c(x05), .O(new_n192_));
  aoi21  g109(.a(new_n192_), .b(new_n191_), .c(x10), .O(z22));
  nand2  g110(.a(x35), .b(x09), .O(new_n194_));
  aoi21  g111(.a(x34), .b(new_n183_), .c(x05), .O(new_n195_));
  aoi21  g112(.a(new_n195_), .b(new_n194_), .c(x10), .O(z23));
  nand2  g113(.a(x36), .b(x09), .O(new_n197_));
  aoi21  g114(.a(x35), .b(new_n183_), .c(x05), .O(new_n198_));
  aoi21  g115(.a(new_n198_), .b(new_n197_), .c(x10), .O(z24));
  nand2  g116(.a(x37), .b(x09), .O(new_n200_));
  aoi21  g117(.a(x36), .b(new_n183_), .c(x05), .O(new_n201_));
  aoi21  g118(.a(new_n201_), .b(new_n200_), .c(x10), .O(z25));
  inv1   g119(.a(x38), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(x09), .O(new_n204_));
  inv1   g121(.a(x37), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(new_n183_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n204_), .c(new_n88_), .O(new_n207_));
  inv1   g124(.a(new_n207_), .O(z26));
  nand2  g125(.a(x14), .b(new_n176_), .O(new_n209_));
  oai21  g126(.a(x39), .b(x14), .c(new_n209_), .O(new_n210_));
  oai21  g127(.a(x38), .b(x09), .c(new_n88_), .O(new_n211_));
  aoi21  g128(.a(new_n210_), .b(x09), .c(new_n211_), .O(z27));
  nor2   g129(.a(x14), .b(new_n183_), .O(new_n213_));
  nand2  g130(.a(new_n213_), .b(x40), .O(new_n214_));
  inv1   g131(.a(x14), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(x09), .O(new_n216_));
  aoi21  g133(.a(new_n216_), .b(x39), .c(x05), .O(new_n217_));
  aoi21  g134(.a(new_n217_), .b(new_n214_), .c(x10), .O(z28));
  nand2  g135(.a(new_n213_), .b(x41), .O(new_n219_));
  aoi21  g136(.a(new_n216_), .b(x40), .c(x05), .O(new_n220_));
  aoi21  g137(.a(new_n220_), .b(new_n219_), .c(x10), .O(z29));
  inv1   g138(.a(x42), .O(new_n222_));
  nand3  g139(.a(new_n222_), .b(new_n215_), .c(x09), .O(new_n223_));
  inv1   g140(.a(x41), .O(new_n224_));
  nand2  g141(.a(new_n216_), .b(new_n224_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n223_), .c(new_n88_), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(z30));
  nand2  g144(.a(new_n213_), .b(x43), .O(new_n228_));
  aoi21  g145(.a(new_n216_), .b(x42), .c(x05), .O(new_n229_));
  aoi21  g146(.a(new_n229_), .b(new_n228_), .c(x10), .O(z31));
  nand2  g147(.a(new_n213_), .b(x44), .O(new_n231_));
  aoi21  g148(.a(new_n216_), .b(x43), .c(x05), .O(new_n232_));
  aoi21  g149(.a(new_n232_), .b(new_n231_), .c(x10), .O(z32));
  nand2  g150(.a(new_n213_), .b(x45), .O(new_n234_));
  aoi21  g151(.a(new_n216_), .b(x44), .c(x05), .O(new_n235_));
  aoi21  g152(.a(new_n235_), .b(new_n234_), .c(x10), .O(z33));
  nand2  g153(.a(new_n213_), .b(x46), .O(new_n237_));
  aoi21  g154(.a(new_n216_), .b(x45), .c(x05), .O(new_n238_));
  aoi21  g155(.a(new_n238_), .b(new_n237_), .c(x10), .O(z34));
  nand2  g156(.a(new_n213_), .b(x00), .O(new_n240_));
  aoi21  g157(.a(new_n216_), .b(x46), .c(x05), .O(new_n241_));
  aoi21  g158(.a(new_n241_), .b(new_n240_), .c(x10), .O(z35));
endmodule


