// Benchmark "FAU" written by ABC on Sat Aug  8 23:36:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n169_, new_n171_, new_n172_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n178_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n207_, new_n208_, new_n209_, new_n210_, new_n212_,
    new_n213_, new_n214_, new_n216_, new_n217_, new_n219_, new_n220_,
    new_n221_, new_n222_, new_n223_, new_n225_, new_n226_, new_n227_,
    new_n228_, new_n230_, new_n231_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  aoi21  g002(.a(x11), .b(new_n85_), .c(x33), .O(new_n86_));
  aoi21  g003(.a(new_n86_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g004(.a(x07), .b(x05), .O(new_n88_));
  aoi21  g005(.a(x12), .b(new_n85_), .c(x33), .O(new_n89_));
  aoi21  g006(.a(new_n89_), .b(new_n88_), .c(x10), .O(z01));
  nand2  g007(.a(x07), .b(x06), .O(new_n91_));
  aoi21  g008(.a(x13), .b(new_n85_), .c(x33), .O(new_n92_));
  aoi21  g009(.a(new_n92_), .b(new_n91_), .c(x10), .O(z02));
  inv1   g010(.a(x01), .O(new_n94_));
  nand2  g011(.a(x07), .b(new_n94_), .O(new_n95_));
  nor2   g012(.a(x33), .b(x10), .O(new_n96_));
  inv1   g013(.a(x14), .O(new_n97_));
  nand2  g014(.a(new_n97_), .b(new_n85_), .O(new_n98_));
  nand3  g015(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  inv1   g016(.a(new_n99_), .O(z03));
  nand2  g017(.a(x07), .b(x02), .O(new_n101_));
  aoi21  g018(.a(x15), .b(new_n85_), .c(x33), .O(new_n102_));
  aoi21  g019(.a(new_n102_), .b(new_n101_), .c(x10), .O(z04));
  nand2  g020(.a(x07), .b(x03), .O(new_n104_));
  aoi21  g021(.a(x16), .b(new_n85_), .c(x33), .O(new_n105_));
  aoi21  g022(.a(new_n105_), .b(new_n104_), .c(x10), .O(z05));
  nand2  g023(.a(x18), .b(x08), .O(new_n107_));
  inv1   g024(.a(x08), .O(new_n108_));
  aoi21  g025(.a(x17), .b(new_n108_), .c(x33), .O(new_n109_));
  aoi21  g026(.a(new_n109_), .b(new_n107_), .c(x10), .O(z06));
  nand2  g027(.a(x19), .b(x08), .O(new_n111_));
  aoi21  g028(.a(x18), .b(new_n108_), .c(x33), .O(new_n112_));
  aoi21  g029(.a(new_n112_), .b(new_n111_), .c(x10), .O(z07));
  inv1   g030(.a(x20), .O(new_n114_));
  nand2  g031(.a(new_n114_), .b(x08), .O(new_n115_));
  inv1   g032(.a(x19), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n108_), .O(new_n117_));
  nand3  g034(.a(new_n117_), .b(new_n115_), .c(new_n96_), .O(new_n118_));
  inv1   g035(.a(new_n118_), .O(z08));
  inv1   g036(.a(x21), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(x08), .O(new_n121_));
  nand2  g038(.a(new_n114_), .b(new_n108_), .O(new_n122_));
  nand3  g039(.a(new_n122_), .b(new_n121_), .c(new_n96_), .O(new_n123_));
  inv1   g040(.a(new_n123_), .O(z09));
  nand2  g041(.a(x22), .b(x08), .O(new_n125_));
  aoi21  g042(.a(x21), .b(new_n108_), .c(x33), .O(new_n126_));
  aoi21  g043(.a(new_n126_), .b(new_n125_), .c(x10), .O(z10));
  nand2  g044(.a(x23), .b(x08), .O(new_n128_));
  aoi21  g045(.a(x22), .b(new_n108_), .c(x33), .O(new_n129_));
  aoi21  g046(.a(new_n129_), .b(new_n128_), .c(x10), .O(z11));
  nand2  g047(.a(x24), .b(x08), .O(new_n131_));
  aoi21  g048(.a(x23), .b(new_n108_), .c(x33), .O(new_n132_));
  aoi21  g049(.a(new_n132_), .b(new_n131_), .c(x10), .O(z12));
  inv1   g050(.a(x25), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(x08), .O(new_n135_));
  inv1   g052(.a(x24), .O(new_n136_));
  nand2  g053(.a(new_n136_), .b(new_n108_), .O(new_n137_));
  nand3  g054(.a(new_n137_), .b(new_n135_), .c(new_n96_), .O(new_n138_));
  inv1   g055(.a(new_n138_), .O(z13));
  inv1   g056(.a(x26), .O(new_n140_));
  nand2  g057(.a(new_n140_), .b(x08), .O(new_n141_));
  nand2  g058(.a(new_n134_), .b(new_n108_), .O(new_n142_));
  nand3  g059(.a(new_n142_), .b(new_n141_), .c(new_n96_), .O(new_n143_));
  inv1   g060(.a(new_n143_), .O(z14));
  nand2  g061(.a(x27), .b(x08), .O(new_n145_));
  aoi21  g062(.a(x26), .b(new_n108_), .c(x33), .O(new_n146_));
  aoi21  g063(.a(new_n146_), .b(new_n145_), .c(x10), .O(z15));
  nand2  g064(.a(x28), .b(x08), .O(new_n148_));
  aoi21  g065(.a(x27), .b(new_n108_), .c(x33), .O(new_n149_));
  aoi21  g066(.a(new_n149_), .b(new_n148_), .c(x10), .O(z16));
  nand2  g067(.a(x29), .b(x08), .O(new_n151_));
  aoi21  g068(.a(x28), .b(new_n108_), .c(x33), .O(new_n152_));
  aoi21  g069(.a(new_n152_), .b(new_n151_), .c(x10), .O(z17));
  nand2  g070(.a(x30), .b(x08), .O(new_n154_));
  aoi21  g071(.a(x29), .b(new_n108_), .c(x33), .O(new_n155_));
  aoi21  g072(.a(new_n155_), .b(new_n154_), .c(x10), .O(z18));
  inv1   g073(.a(x00), .O(new_n157_));
  nand2  g074(.a(x08), .b(new_n157_), .O(new_n158_));
  inv1   g075(.a(x30), .O(new_n159_));
  nand2  g076(.a(new_n159_), .b(new_n108_), .O(new_n160_));
  nand3  g077(.a(new_n160_), .b(new_n158_), .c(new_n96_), .O(new_n161_));
  inv1   g078(.a(new_n161_), .O(z19));
  nand2  g079(.a(x32), .b(x09), .O(new_n163_));
  inv1   g080(.a(x09), .O(new_n164_));
  aoi21  g081(.a(x31), .b(new_n164_), .c(x33), .O(new_n165_));
  aoi21  g082(.a(new_n165_), .b(new_n163_), .c(x10), .O(z20));
  nand3  g083(.a(new_n96_), .b(x32), .c(new_n164_), .O(new_n167_));
  inv1   g084(.a(new_n167_), .O(z21));
  nand3  g085(.a(new_n96_), .b(x34), .c(x09), .O(new_n169_));
  inv1   g086(.a(new_n169_), .O(z22));
  nand2  g087(.a(x35), .b(x09), .O(new_n171_));
  aoi21  g088(.a(x34), .b(new_n164_), .c(x33), .O(new_n172_));
  aoi21  g089(.a(new_n172_), .b(new_n171_), .c(x10), .O(z23));
  inv1   g090(.a(x36), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(x09), .O(new_n175_));
  inv1   g092(.a(x35), .O(new_n176_));
  nand2  g093(.a(new_n176_), .b(new_n164_), .O(new_n177_));
  nand3  g094(.a(new_n177_), .b(new_n175_), .c(new_n96_), .O(new_n178_));
  inv1   g095(.a(new_n178_), .O(z24));
  inv1   g096(.a(x37), .O(new_n180_));
  nand2  g097(.a(new_n180_), .b(x09), .O(new_n181_));
  nand2  g098(.a(new_n174_), .b(new_n164_), .O(new_n182_));
  nand3  g099(.a(new_n182_), .b(new_n181_), .c(new_n96_), .O(new_n183_));
  inv1   g100(.a(new_n183_), .O(z25));
  inv1   g101(.a(x38), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(x09), .O(new_n186_));
  nand2  g103(.a(new_n180_), .b(new_n164_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(new_n96_), .O(new_n188_));
  inv1   g105(.a(new_n188_), .O(z26));
  nand2  g106(.a(x14), .b(new_n157_), .O(new_n190_));
  inv1   g107(.a(x39), .O(new_n191_));
  nand2  g108(.a(new_n191_), .b(new_n97_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n190_), .c(x09), .O(new_n193_));
  aoi21  g110(.a(x38), .b(new_n164_), .c(x33), .O(new_n194_));
  aoi21  g111(.a(new_n194_), .b(new_n193_), .c(x10), .O(z27));
  inv1   g112(.a(x40), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n97_), .c(x09), .O(new_n197_));
  nand2  g114(.a(new_n97_), .b(x09), .O(new_n198_));
  nand2  g115(.a(new_n198_), .b(new_n191_), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n197_), .c(new_n96_), .O(new_n200_));
  inv1   g117(.a(new_n200_), .O(z28));
  inv1   g118(.a(x41), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n97_), .c(x09), .O(new_n203_));
  nand2  g120(.a(new_n198_), .b(new_n196_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n203_), .c(new_n96_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z29));
  inv1   g123(.a(x42), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n97_), .c(x09), .O(new_n208_));
  nand2  g125(.a(new_n198_), .b(new_n202_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n208_), .c(new_n96_), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z30));
  nor2   g128(.a(x14), .b(new_n164_), .O(new_n212_));
  nand2  g129(.a(new_n212_), .b(x43), .O(new_n213_));
  aoi21  g130(.a(new_n198_), .b(x42), .c(x33), .O(new_n214_));
  aoi21  g131(.a(new_n214_), .b(new_n213_), .c(x10), .O(z31));
  nand2  g132(.a(new_n212_), .b(x44), .O(new_n216_));
  aoi21  g133(.a(new_n198_), .b(x43), .c(x33), .O(new_n217_));
  aoi21  g134(.a(new_n217_), .b(new_n216_), .c(x10), .O(z32));
  inv1   g135(.a(x45), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n97_), .c(x09), .O(new_n220_));
  inv1   g137(.a(x44), .O(new_n221_));
  nand2  g138(.a(new_n198_), .b(new_n221_), .O(new_n222_));
  nand3  g139(.a(new_n222_), .b(new_n220_), .c(new_n96_), .O(new_n223_));
  inv1   g140(.a(new_n223_), .O(z33));
  inv1   g141(.a(x46), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n97_), .c(x09), .O(new_n226_));
  nand2  g143(.a(new_n198_), .b(new_n219_), .O(new_n227_));
  nand3  g144(.a(new_n227_), .b(new_n226_), .c(new_n96_), .O(new_n228_));
  inv1   g145(.a(new_n228_), .O(z34));
  nand2  g146(.a(new_n212_), .b(x00), .O(new_n230_));
  aoi21  g147(.a(new_n198_), .b(x46), .c(x33), .O(new_n231_));
  aoi21  g148(.a(new_n231_), .b(new_n230_), .c(x10), .O(z35));
endmodule


