// Benchmark "FAU" written by ABC on Sat Aug  8 23:36:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n156_, new_n157_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n175_, new_n176_, new_n177_, new_n178_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n198_, new_n199_, new_n201_, new_n202_,
    new_n203_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  nor2   g009(.a(new_n92_), .b(new_n91_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  nor2   g012(.a(new_n95_), .b(new_n94_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  oai21  g015(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  oai21  g018(.a(new_n101_), .b(new_n100_), .c(new_n85_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  oai21  g021(.a(new_n104_), .b(new_n103_), .c(new_n85_), .O(z05));
  inv1   g022(.a(x08), .O(new_n106_));
  nor2   g023(.a(x18), .b(new_n106_), .O(new_n107_));
  oai21  g024(.a(x17), .b(x08), .c(new_n88_), .O(new_n108_));
  oai21  g025(.a(new_n108_), .b(new_n107_), .c(new_n85_), .O(z06));
  nor2   g026(.a(x19), .b(new_n106_), .O(new_n110_));
  oai21  g027(.a(x18), .b(x08), .c(new_n88_), .O(new_n111_));
  nor2   g028(.a(new_n111_), .b(new_n110_), .O(z07));
  inv1   g029(.a(x20), .O(new_n113_));
  oai21  g030(.a(x19), .b(x08), .c(new_n88_), .O(new_n114_));
  aoi21  g031(.a(new_n113_), .b(x08), .c(new_n114_), .O(z08));
  nor2   g032(.a(x21), .b(new_n106_), .O(new_n116_));
  oai21  g033(.a(x20), .b(x08), .c(new_n88_), .O(new_n117_));
  oai21  g034(.a(new_n117_), .b(new_n116_), .c(new_n85_), .O(z09));
  nor2   g035(.a(x22), .b(new_n106_), .O(new_n119_));
  oai21  g036(.a(x21), .b(x08), .c(new_n88_), .O(new_n120_));
  nor2   g037(.a(new_n120_), .b(new_n119_), .O(z10));
  nor2   g038(.a(x23), .b(new_n106_), .O(new_n122_));
  oai21  g039(.a(x22), .b(x08), .c(new_n88_), .O(new_n123_));
  nor2   g040(.a(new_n123_), .b(new_n122_), .O(z11));
  inv1   g041(.a(x24), .O(new_n125_));
  oai21  g042(.a(x23), .b(x08), .c(new_n88_), .O(new_n126_));
  aoi21  g043(.a(new_n125_), .b(x08), .c(new_n126_), .O(z12));
  inv1   g044(.a(x25), .O(new_n128_));
  nand2  g045(.a(new_n128_), .b(x08), .O(new_n129_));
  nand2  g046(.a(new_n125_), .b(new_n106_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n129_), .c(new_n88_), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(new_n85_), .O(z13));
  nor2   g049(.a(x26), .b(new_n106_), .O(new_n133_));
  oai21  g050(.a(x25), .b(x08), .c(new_n88_), .O(new_n134_));
  oai21  g051(.a(new_n134_), .b(new_n133_), .c(new_n85_), .O(z14));
  nor2   g052(.a(x27), .b(new_n106_), .O(new_n136_));
  oai21  g053(.a(x26), .b(x08), .c(new_n88_), .O(new_n137_));
  nor2   g054(.a(new_n137_), .b(new_n136_), .O(z15));
  nor2   g055(.a(x28), .b(new_n106_), .O(new_n139_));
  oai21  g056(.a(x27), .b(x08), .c(new_n88_), .O(new_n140_));
  nor2   g057(.a(new_n140_), .b(new_n139_), .O(z16));
  inv1   g058(.a(x29), .O(new_n142_));
  oai21  g059(.a(x28), .b(x08), .c(new_n88_), .O(new_n143_));
  aoi21  g060(.a(new_n142_), .b(x08), .c(new_n143_), .O(z17));
  inv1   g061(.a(x30), .O(new_n145_));
  nand2  g062(.a(new_n145_), .b(x08), .O(new_n146_));
  nand2  g063(.a(new_n142_), .b(new_n106_), .O(new_n147_));
  nand3  g064(.a(new_n147_), .b(new_n146_), .c(new_n88_), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(new_n85_), .O(z18));
  nor2   g066(.a(new_n106_), .b(x00), .O(new_n150_));
  oai21  g067(.a(x30), .b(x08), .c(new_n88_), .O(new_n151_));
  oai21  g068(.a(new_n151_), .b(new_n150_), .c(new_n85_), .O(z19));
  nor2   g069(.a(x32), .b(new_n84_), .O(new_n153_));
  oai21  g070(.a(x31), .b(x09), .c(new_n88_), .O(new_n154_));
  nor2   g071(.a(new_n154_), .b(new_n153_), .O(z20));
  nor2   g072(.a(x33), .b(new_n84_), .O(new_n156_));
  oai21  g073(.a(x32), .b(x09), .c(new_n88_), .O(new_n157_));
  nor2   g074(.a(new_n157_), .b(new_n156_), .O(z21));
  nor2   g075(.a(x34), .b(new_n84_), .O(new_n159_));
  oai21  g076(.a(x33), .b(x09), .c(new_n88_), .O(new_n160_));
  nor2   g077(.a(new_n160_), .b(new_n159_), .O(z22));
  inv1   g078(.a(x35), .O(new_n162_));
  nor2   g079(.a(x34), .b(x10), .O(new_n163_));
  nand2  g080(.a(new_n88_), .b(x09), .O(new_n164_));
  oai22  g081(.a(new_n164_), .b(new_n162_), .c(new_n163_), .d(x09), .O(z23));
  inv1   g082(.a(x36), .O(new_n166_));
  nor2   g083(.a(x35), .b(x10), .O(new_n167_));
  oai22  g084(.a(new_n167_), .b(x09), .c(new_n164_), .d(new_n166_), .O(z24));
  inv1   g085(.a(x37), .O(new_n169_));
  nor2   g086(.a(x36), .b(x10), .O(new_n170_));
  oai22  g087(.a(new_n170_), .b(x09), .c(new_n164_), .d(new_n169_), .O(z25));
  nor2   g088(.a(x38), .b(new_n84_), .O(new_n172_));
  oai21  g089(.a(x37), .b(x09), .c(new_n88_), .O(new_n173_));
  nor2   g090(.a(new_n173_), .b(new_n172_), .O(z26));
  nand2  g091(.a(x14), .b(x00), .O(new_n175_));
  inv1   g092(.a(x14), .O(new_n176_));
  aoi21  g093(.a(x39), .b(new_n176_), .c(new_n84_), .O(new_n177_));
  oai21  g094(.a(x38), .b(x09), .c(new_n88_), .O(new_n178_));
  aoi21  g095(.a(new_n177_), .b(new_n175_), .c(new_n178_), .O(z27));
  inv1   g096(.a(x40), .O(new_n180_));
  nand3  g097(.a(new_n180_), .b(new_n176_), .c(x09), .O(new_n181_));
  inv1   g098(.a(x39), .O(new_n182_));
  oai21  g099(.a(x14), .b(new_n84_), .c(new_n182_), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n181_), .c(new_n88_), .O(new_n184_));
  inv1   g101(.a(new_n184_), .O(z28));
  inv1   g102(.a(x41), .O(new_n186_));
  nand3  g103(.a(new_n186_), .b(new_n176_), .c(x09), .O(new_n187_));
  oai21  g104(.a(x14), .b(new_n84_), .c(new_n180_), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n187_), .c(new_n88_), .O(new_n189_));
  inv1   g106(.a(new_n189_), .O(z29));
  nand2  g107(.a(new_n176_), .b(x09), .O(new_n191_));
  oai21  g108(.a(x42), .b(x14), .c(new_n88_), .O(new_n192_));
  nor2   g109(.a(x41), .b(x10), .O(new_n193_));
  aoi22  g110(.a(new_n193_), .b(new_n191_), .c(new_n192_), .d(x09), .O(z30));
  oai21  g111(.a(x43), .b(x14), .c(new_n88_), .O(new_n195_));
  nor2   g112(.a(x42), .b(x10), .O(new_n196_));
  aoi22  g113(.a(new_n196_), .b(new_n191_), .c(new_n195_), .d(x09), .O(z31));
  oai21  g114(.a(x44), .b(x14), .c(new_n88_), .O(new_n198_));
  nor2   g115(.a(x43), .b(x10), .O(new_n199_));
  aoi22  g116(.a(new_n199_), .b(new_n191_), .c(new_n198_), .d(x09), .O(z32));
  inv1   g117(.a(x45), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n176_), .c(x09), .O(new_n202_));
  inv1   g119(.a(x44), .O(new_n203_));
  oai21  g120(.a(x14), .b(new_n84_), .c(new_n203_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n202_), .c(new_n88_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z33));
  oai21  g123(.a(x46), .b(x14), .c(new_n88_), .O(new_n207_));
  nor2   g124(.a(x45), .b(x10), .O(new_n208_));
  aoi22  g125(.a(new_n208_), .b(new_n191_), .c(new_n207_), .d(x09), .O(z34));
  oai21  g126(.a(x14), .b(x00), .c(new_n88_), .O(new_n210_));
  nor2   g127(.a(x46), .b(x10), .O(new_n211_));
  aoi22  g128(.a(new_n211_), .b(new_n191_), .c(new_n210_), .d(x09), .O(z35));
endmodule


