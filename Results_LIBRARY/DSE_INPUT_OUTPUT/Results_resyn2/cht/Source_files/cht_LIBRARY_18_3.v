// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_, new_n146_, new_n147_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_,
    new_n167_, new_n168_, new_n170_, new_n171_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n197_, new_n198_,
    new_n200_, new_n201_, new_n203_, new_n204_, new_n206_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n217_, new_n218_, new_n219_, new_n220_, new_n222_,
    new_n223_, new_n224_;
  inv1   g000(.a(x07), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x04), .O(new_n85_));
  inv1   g002(.a(x10), .O(new_n86_));
  oai21  g003(.a(x11), .b(x07), .c(new_n86_), .O(new_n87_));
  nor2   g004(.a(new_n87_), .b(new_n85_), .O(z00));
  nand2  g005(.a(x10), .b(x09), .O(new_n89_));
  nor2   g006(.a(new_n84_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n86_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z01));
  nor2   g009(.a(new_n84_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n86_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z02));
  nor2   g012(.a(new_n84_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n86_), .O(new_n97_));
  nor2   g014(.a(new_n97_), .b(new_n96_), .O(z03));
  nor2   g015(.a(new_n84_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n86_), .O(new_n100_));
  oai21  g017(.a(new_n100_), .b(new_n99_), .c(new_n89_), .O(z04));
  nor2   g018(.a(new_n84_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n86_), .O(new_n103_));
  oai21  g020(.a(new_n103_), .b(new_n102_), .c(new_n89_), .O(z05));
  inv1   g021(.a(x18), .O(new_n105_));
  nand2  g022(.a(new_n105_), .b(x08), .O(new_n106_));
  inv1   g023(.a(x08), .O(new_n107_));
  inv1   g024(.a(x17), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g026(.a(new_n109_), .b(new_n106_), .c(new_n86_), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n89_), .O(z06));
  inv1   g028(.a(x19), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(x08), .O(new_n113_));
  nand2  g030(.a(new_n105_), .b(new_n107_), .O(new_n114_));
  nand3  g031(.a(new_n114_), .b(new_n113_), .c(new_n86_), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(new_n89_), .O(z07));
  inv1   g033(.a(x20), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(x08), .O(new_n118_));
  nand2  g035(.a(new_n112_), .b(new_n107_), .O(new_n119_));
  nand3  g036(.a(new_n119_), .b(new_n118_), .c(new_n86_), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(new_n89_), .O(z08));
  nor2   g038(.a(x21), .b(new_n107_), .O(new_n122_));
  oai21  g039(.a(x20), .b(x08), .c(new_n86_), .O(new_n123_));
  oai21  g040(.a(new_n123_), .b(new_n122_), .c(new_n89_), .O(z09));
  inv1   g041(.a(x22), .O(new_n125_));
  oai21  g042(.a(x21), .b(x08), .c(new_n86_), .O(new_n126_));
  aoi21  g043(.a(new_n125_), .b(x08), .c(new_n126_), .O(z10));
  inv1   g044(.a(x23), .O(new_n128_));
  nand2  g045(.a(new_n128_), .b(x08), .O(new_n129_));
  nand2  g046(.a(new_n125_), .b(new_n107_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n129_), .c(new_n86_), .O(new_n131_));
  nand2  g048(.a(new_n131_), .b(new_n89_), .O(z11));
  inv1   g049(.a(x24), .O(new_n133_));
  nand2  g050(.a(new_n133_), .b(x08), .O(new_n134_));
  nand2  g051(.a(new_n128_), .b(new_n107_), .O(new_n135_));
  nand3  g052(.a(new_n135_), .b(new_n134_), .c(new_n86_), .O(new_n136_));
  nand2  g053(.a(new_n136_), .b(new_n89_), .O(z12));
  inv1   g054(.a(x25), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(x08), .O(new_n139_));
  nand2  g056(.a(new_n133_), .b(new_n107_), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n86_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n89_), .O(z13));
  nor2   g059(.a(x26), .b(new_n107_), .O(new_n143_));
  oai21  g060(.a(x25), .b(x08), .c(new_n86_), .O(new_n144_));
  oai21  g061(.a(new_n144_), .b(new_n143_), .c(new_n89_), .O(z14));
  nor2   g062(.a(x27), .b(new_n107_), .O(new_n146_));
  oai21  g063(.a(x26), .b(x08), .c(new_n86_), .O(new_n147_));
  nor2   g064(.a(new_n147_), .b(new_n146_), .O(z15));
  nor2   g065(.a(x28), .b(new_n107_), .O(new_n149_));
  oai21  g066(.a(x27), .b(x08), .c(new_n86_), .O(new_n150_));
  nor2   g067(.a(new_n150_), .b(new_n149_), .O(z16));
  nor2   g068(.a(x29), .b(new_n107_), .O(new_n152_));
  oai21  g069(.a(x28), .b(x08), .c(new_n86_), .O(new_n153_));
  nor2   g070(.a(new_n153_), .b(new_n152_), .O(z17));
  inv1   g071(.a(x30), .O(new_n155_));
  oai21  g072(.a(x29), .b(x08), .c(new_n86_), .O(new_n156_));
  aoi21  g073(.a(new_n155_), .b(x08), .c(new_n156_), .O(z18));
  inv1   g074(.a(x00), .O(new_n158_));
  nand2  g075(.a(x08), .b(new_n158_), .O(new_n159_));
  nand2  g076(.a(new_n155_), .b(new_n107_), .O(new_n160_));
  nand3  g077(.a(new_n160_), .b(new_n159_), .c(new_n86_), .O(new_n161_));
  nand2  g078(.a(new_n161_), .b(new_n89_), .O(z19));
  oai21  g079(.a(x32), .b(x10), .c(x09), .O(new_n163_));
  nor2   g080(.a(x10), .b(x09), .O(new_n164_));
  nand2  g081(.a(new_n164_), .b(x31), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(new_n163_), .O(z20));
  inv1   g083(.a(x33), .O(new_n167_));
  oai21  g084(.a(x32), .b(x09), .c(new_n86_), .O(new_n168_));
  aoi21  g085(.a(new_n167_), .b(x09), .c(new_n168_), .O(z21));
  inv1   g086(.a(new_n164_), .O(new_n170_));
  oai21  g087(.a(x34), .b(x10), .c(x09), .O(new_n171_));
  oai21  g088(.a(new_n170_), .b(new_n167_), .c(new_n171_), .O(z22));
  oai21  g089(.a(x35), .b(x10), .c(x09), .O(new_n173_));
  nand2  g090(.a(new_n164_), .b(x34), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(new_n173_), .O(z23));
  inv1   g092(.a(x36), .O(new_n176_));
  oai21  g093(.a(x35), .b(x09), .c(new_n86_), .O(new_n177_));
  aoi21  g094(.a(new_n176_), .b(x09), .c(new_n177_), .O(z24));
  oai21  g095(.a(x37), .b(x10), .c(x09), .O(new_n179_));
  oai21  g096(.a(new_n170_), .b(new_n176_), .c(new_n179_), .O(z25));
  inv1   g097(.a(x09), .O(new_n181_));
  nor2   g098(.a(x38), .b(new_n181_), .O(new_n182_));
  oai21  g099(.a(x37), .b(x09), .c(new_n86_), .O(new_n183_));
  nor2   g100(.a(new_n183_), .b(new_n182_), .O(z26));
  nand2  g101(.a(new_n164_), .b(x38), .O(new_n185_));
  nand2  g102(.a(x14), .b(x00), .O(new_n186_));
  inv1   g103(.a(x14), .O(new_n187_));
  nand2  g104(.a(x39), .b(new_n187_), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n186_), .c(new_n86_), .O(new_n189_));
  nand2  g106(.a(new_n189_), .b(x09), .O(new_n190_));
  nand2  g107(.a(new_n190_), .b(new_n185_), .O(z27));
  nand2  g108(.a(new_n187_), .b(x09), .O(new_n192_));
  nand2  g109(.a(x10), .b(new_n181_), .O(new_n193_));
  nand3  g110(.a(new_n193_), .b(new_n192_), .c(x39), .O(new_n194_));
  aoi21  g111(.a(x40), .b(new_n187_), .c(x10), .O(new_n195_));
  oai21  g112(.a(new_n195_), .b(new_n181_), .c(new_n194_), .O(z28));
  nand3  g113(.a(new_n193_), .b(new_n192_), .c(x40), .O(new_n197_));
  aoi21  g114(.a(x41), .b(new_n187_), .c(x10), .O(new_n198_));
  oai21  g115(.a(new_n198_), .b(new_n181_), .c(new_n197_), .O(z29));
  nand3  g116(.a(new_n193_), .b(new_n192_), .c(x41), .O(new_n200_));
  aoi21  g117(.a(x42), .b(new_n187_), .c(x10), .O(new_n201_));
  oai21  g118(.a(new_n201_), .b(new_n181_), .c(new_n200_), .O(z30));
  nand3  g119(.a(new_n193_), .b(new_n192_), .c(x42), .O(new_n203_));
  aoi21  g120(.a(x43), .b(new_n187_), .c(x10), .O(new_n204_));
  oai21  g121(.a(new_n204_), .b(new_n181_), .c(new_n203_), .O(z31));
  inv1   g122(.a(x44), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n187_), .c(x09), .O(new_n207_));
  inv1   g124(.a(x43), .O(new_n208_));
  nand2  g125(.a(new_n192_), .b(new_n208_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n207_), .c(new_n86_), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z32));
  inv1   g128(.a(x45), .O(new_n212_));
  nand3  g129(.a(new_n212_), .b(new_n187_), .c(x09), .O(new_n213_));
  nand2  g130(.a(new_n192_), .b(new_n206_), .O(new_n214_));
  nand3  g131(.a(new_n214_), .b(new_n213_), .c(new_n86_), .O(new_n215_));
  inv1   g132(.a(new_n215_), .O(z33));
  inv1   g133(.a(x46), .O(new_n217_));
  nand3  g134(.a(new_n217_), .b(new_n187_), .c(x09), .O(new_n218_));
  nand2  g135(.a(new_n192_), .b(new_n212_), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n218_), .c(new_n86_), .O(new_n220_));
  inv1   g137(.a(new_n220_), .O(z34));
  nand3  g138(.a(new_n187_), .b(x09), .c(new_n158_), .O(new_n222_));
  nand2  g139(.a(new_n192_), .b(new_n217_), .O(new_n223_));
  nand3  g140(.a(new_n223_), .b(new_n222_), .c(new_n86_), .O(new_n224_));
  inv1   g141(.a(new_n224_), .O(z35));
endmodule


