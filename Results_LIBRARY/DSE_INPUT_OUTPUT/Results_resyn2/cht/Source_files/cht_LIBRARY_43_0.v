// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n122_,
    new_n124_, new_n126_, new_n128_, new_n130_, new_n132_, new_n134_,
    new_n136_, new_n138_, new_n140_, new_n142_, new_n144_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n166_, new_n167_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n176_, new_n178_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n195_, new_n196_, new_n198_, new_n199_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n207_, new_n208_, new_n210_,
    new_n211_;
  inv1   g000(.a(x04), .O(new_n84_));
  nand2  g001(.a(x07), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x08), .O(new_n86_));
  nor2   g003(.a(x10), .b(new_n86_), .O(new_n87_));
  inv1   g004(.a(x07), .O(new_n88_));
  inv1   g005(.a(x11), .O(new_n89_));
  nand2  g006(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g007(.a(new_n90_), .b(new_n87_), .c(new_n85_), .O(new_n91_));
  inv1   g008(.a(new_n91_), .O(z00));
  inv1   g009(.a(x05), .O(new_n93_));
  nand2  g010(.a(x07), .b(new_n93_), .O(new_n94_));
  inv1   g011(.a(x12), .O(new_n95_));
  nand2  g012(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  nand3  g013(.a(new_n96_), .b(new_n94_), .c(new_n87_), .O(new_n97_));
  inv1   g014(.a(new_n97_), .O(z01));
  inv1   g015(.a(x06), .O(new_n99_));
  nand2  g016(.a(x07), .b(new_n99_), .O(new_n100_));
  inv1   g017(.a(x13), .O(new_n101_));
  nand2  g018(.a(new_n101_), .b(new_n88_), .O(new_n102_));
  nand3  g019(.a(new_n102_), .b(new_n100_), .c(new_n87_), .O(new_n103_));
  inv1   g020(.a(new_n103_), .O(z02));
  nand2  g021(.a(x07), .b(x01), .O(new_n105_));
  aoi21  g022(.a(x14), .b(new_n88_), .c(new_n86_), .O(new_n106_));
  aoi21  g023(.a(new_n106_), .b(new_n105_), .c(x10), .O(z03));
  inv1   g024(.a(x02), .O(new_n108_));
  nand2  g025(.a(x07), .b(new_n108_), .O(new_n109_));
  inv1   g026(.a(x15), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n109_), .c(new_n87_), .O(new_n112_));
  inv1   g029(.a(new_n112_), .O(z04));
  inv1   g030(.a(x03), .O(new_n114_));
  nand2  g031(.a(x07), .b(new_n114_), .O(new_n115_));
  inv1   g032(.a(x16), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  nand3  g034(.a(new_n117_), .b(new_n115_), .c(new_n87_), .O(new_n118_));
  inv1   g035(.a(new_n118_), .O(z05));
  nand2  g036(.a(new_n87_), .b(x18), .O(new_n120_));
  inv1   g037(.a(new_n120_), .O(z06));
  nand2  g038(.a(new_n87_), .b(x19), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z07));
  inv1   g040(.a(x20), .O(new_n124_));
  aoi21  g041(.a(new_n124_), .b(x08), .c(x10), .O(z08));
  nand2  g042(.a(new_n87_), .b(x21), .O(new_n126_));
  inv1   g043(.a(new_n126_), .O(z09));
  inv1   g044(.a(x22), .O(new_n128_));
  aoi21  g045(.a(new_n128_), .b(x08), .c(x10), .O(z10));
  nand2  g046(.a(new_n87_), .b(x23), .O(new_n130_));
  inv1   g047(.a(new_n130_), .O(z11));
  inv1   g048(.a(x24), .O(new_n132_));
  aoi21  g049(.a(new_n132_), .b(x08), .c(x10), .O(z12));
  nand2  g050(.a(new_n87_), .b(x25), .O(new_n134_));
  inv1   g051(.a(new_n134_), .O(z13));
  nand2  g052(.a(new_n87_), .b(x26), .O(new_n136_));
  inv1   g053(.a(new_n136_), .O(z14));
  inv1   g054(.a(x27), .O(new_n138_));
  aoi21  g055(.a(new_n138_), .b(x08), .c(x10), .O(z15));
  inv1   g056(.a(x28), .O(new_n140_));
  aoi21  g057(.a(new_n140_), .b(x08), .c(x10), .O(z16));
  inv1   g058(.a(x29), .O(new_n142_));
  aoi21  g059(.a(new_n142_), .b(x08), .c(x10), .O(z17));
  inv1   g060(.a(x30), .O(new_n144_));
  aoi21  g061(.a(new_n144_), .b(x08), .c(x10), .O(z18));
  inv1   g062(.a(x00), .O(new_n146_));
  aoi21  g063(.a(x08), .b(new_n146_), .c(x10), .O(z19));
  inv1   g064(.a(x32), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(x09), .O(new_n149_));
  inv1   g066(.a(x09), .O(new_n150_));
  inv1   g067(.a(x31), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n149_), .c(new_n87_), .O(new_n153_));
  inv1   g070(.a(new_n153_), .O(z20));
  inv1   g071(.a(x33), .O(new_n155_));
  nand2  g072(.a(new_n155_), .b(x09), .O(new_n156_));
  nand2  g073(.a(new_n148_), .b(new_n150_), .O(new_n157_));
  nand3  g074(.a(new_n157_), .b(new_n156_), .c(new_n87_), .O(new_n158_));
  inv1   g075(.a(new_n158_), .O(z21));
  nand2  g076(.a(x34), .b(x09), .O(new_n160_));
  aoi21  g077(.a(x33), .b(new_n150_), .c(new_n86_), .O(new_n161_));
  aoi21  g078(.a(new_n161_), .b(new_n160_), .c(x10), .O(z22));
  nand2  g079(.a(x35), .b(x09), .O(new_n163_));
  aoi21  g080(.a(x34), .b(new_n150_), .c(new_n86_), .O(new_n164_));
  aoi21  g081(.a(new_n164_), .b(new_n163_), .c(x10), .O(z23));
  nand2  g082(.a(x36), .b(x09), .O(new_n166_));
  aoi21  g083(.a(x35), .b(new_n150_), .c(new_n86_), .O(new_n167_));
  aoi21  g084(.a(new_n167_), .b(new_n166_), .c(x10), .O(z24));
  nand2  g085(.a(x37), .b(x09), .O(new_n169_));
  aoi21  g086(.a(x36), .b(new_n150_), .c(new_n86_), .O(new_n170_));
  aoi21  g087(.a(new_n170_), .b(new_n169_), .c(x10), .O(z25));
  inv1   g088(.a(x38), .O(new_n172_));
  nand2  g089(.a(new_n172_), .b(x09), .O(new_n173_));
  inv1   g090(.a(x37), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(new_n150_), .O(new_n175_));
  nand3  g092(.a(new_n175_), .b(new_n173_), .c(new_n87_), .O(new_n176_));
  inv1   g093(.a(new_n176_), .O(z26));
  nand2  g094(.a(x14), .b(new_n146_), .O(new_n178_));
  inv1   g095(.a(x14), .O(new_n179_));
  inv1   g096(.a(x39), .O(new_n180_));
  nand2  g097(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nand3  g098(.a(new_n181_), .b(new_n178_), .c(x09), .O(new_n182_));
  aoi21  g099(.a(x38), .b(new_n150_), .c(new_n86_), .O(new_n183_));
  aoi21  g100(.a(new_n183_), .b(new_n182_), .c(x10), .O(z27));
  inv1   g101(.a(x40), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n179_), .c(x09), .O(new_n186_));
  nand2  g103(.a(new_n179_), .b(x09), .O(new_n187_));
  nand2  g104(.a(new_n187_), .b(new_n180_), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n186_), .c(new_n87_), .O(new_n189_));
  inv1   g106(.a(new_n189_), .O(z28));
  nor2   g107(.a(x14), .b(new_n150_), .O(new_n191_));
  nand2  g108(.a(new_n191_), .b(x41), .O(new_n192_));
  aoi21  g109(.a(new_n187_), .b(x40), .c(new_n86_), .O(new_n193_));
  aoi21  g110(.a(new_n193_), .b(new_n192_), .c(x10), .O(z29));
  nand2  g111(.a(new_n191_), .b(x42), .O(new_n195_));
  aoi21  g112(.a(new_n187_), .b(x41), .c(new_n86_), .O(new_n196_));
  aoi21  g113(.a(new_n196_), .b(new_n195_), .c(x10), .O(z30));
  nand2  g114(.a(new_n191_), .b(x43), .O(new_n198_));
  aoi21  g115(.a(new_n187_), .b(x42), .c(new_n86_), .O(new_n199_));
  aoi21  g116(.a(new_n199_), .b(new_n198_), .c(x10), .O(z31));
  nand2  g117(.a(new_n191_), .b(x44), .O(new_n201_));
  aoi21  g118(.a(new_n187_), .b(x43), .c(new_n86_), .O(new_n202_));
  aoi21  g119(.a(new_n202_), .b(new_n201_), .c(x10), .O(z32));
  nand2  g120(.a(new_n191_), .b(x45), .O(new_n204_));
  aoi21  g121(.a(new_n187_), .b(x44), .c(new_n86_), .O(new_n205_));
  aoi21  g122(.a(new_n205_), .b(new_n204_), .c(x10), .O(z33));
  nand2  g123(.a(new_n191_), .b(x46), .O(new_n207_));
  aoi21  g124(.a(new_n187_), .b(x45), .c(new_n86_), .O(new_n208_));
  aoi21  g125(.a(new_n208_), .b(new_n207_), .c(x10), .O(z34));
  nand2  g126(.a(new_n191_), .b(x00), .O(new_n210_));
  aoi21  g127(.a(new_n187_), .b(x46), .c(new_n86_), .O(new_n211_));
  aoi21  g128(.a(new_n211_), .b(new_n210_), .c(x10), .O(z35));
endmodule


