// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n171_, new_n173_,
    new_n175_, new_n177_, new_n179_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n196_, new_n197_, new_n199_, new_n200_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n206_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x10), .O(new_n84_));
  inv1   g001(.a(x04), .O(new_n85_));
  nand2  g002(.a(x07), .b(new_n85_), .O(new_n86_));
  inv1   g003(.a(x07), .O(new_n87_));
  inv1   g004(.a(x11), .O(new_n88_));
  nand2  g005(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g006(.a(new_n89_), .b(new_n86_), .c(new_n84_), .d(x09), .O(new_n90_));
  inv1   g007(.a(new_n90_), .O(z00));
  nand2  g008(.a(x07), .b(x05), .O(new_n92_));
  inv1   g009(.a(x09), .O(new_n93_));
  aoi21  g010(.a(x12), .b(new_n87_), .c(new_n93_), .O(new_n94_));
  aoi21  g011(.a(new_n94_), .b(new_n92_), .c(x10), .O(z01));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  aoi21  g013(.a(x13), .b(new_n87_), .c(new_n93_), .O(new_n97_));
  aoi21  g014(.a(new_n97_), .b(new_n96_), .c(x10), .O(z02));
  nand2  g015(.a(x07), .b(x01), .O(new_n99_));
  aoi21  g016(.a(x14), .b(new_n87_), .c(new_n93_), .O(new_n100_));
  aoi21  g017(.a(new_n100_), .b(new_n99_), .c(x10), .O(z03));
  inv1   g018(.a(x02), .O(new_n102_));
  nand2  g019(.a(x07), .b(new_n102_), .O(new_n103_));
  inv1   g020(.a(x15), .O(new_n104_));
  nand2  g021(.a(new_n104_), .b(new_n87_), .O(new_n105_));
  nand4  g022(.a(new_n105_), .b(new_n103_), .c(new_n84_), .d(x09), .O(new_n106_));
  inv1   g023(.a(new_n106_), .O(z04));
  nand2  g024(.a(x07), .b(x03), .O(new_n108_));
  aoi21  g025(.a(x16), .b(new_n87_), .c(new_n93_), .O(new_n109_));
  aoi21  g026(.a(new_n109_), .b(new_n108_), .c(x10), .O(z05));
  nand2  g027(.a(x18), .b(x08), .O(new_n111_));
  inv1   g028(.a(x08), .O(new_n112_));
  aoi21  g029(.a(x17), .b(new_n112_), .c(new_n93_), .O(new_n113_));
  aoi21  g030(.a(new_n113_), .b(new_n111_), .c(x10), .O(z06));
  inv1   g031(.a(x19), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(x08), .O(new_n116_));
  inv1   g033(.a(x18), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  nand4  g035(.a(new_n118_), .b(new_n116_), .c(new_n84_), .d(x09), .O(new_n119_));
  inv1   g036(.a(new_n119_), .O(z07));
  nand2  g037(.a(x20), .b(x08), .O(new_n121_));
  aoi21  g038(.a(x19), .b(new_n112_), .c(new_n93_), .O(new_n122_));
  aoi21  g039(.a(new_n122_), .b(new_n121_), .c(x10), .O(z08));
  inv1   g040(.a(x21), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  inv1   g042(.a(x20), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(new_n112_), .O(new_n127_));
  nand4  g044(.a(new_n127_), .b(new_n125_), .c(new_n84_), .d(x09), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z09));
  nand2  g046(.a(x22), .b(x08), .O(new_n130_));
  aoi21  g047(.a(x21), .b(new_n112_), .c(new_n93_), .O(new_n131_));
  aoi21  g048(.a(new_n131_), .b(new_n130_), .c(x10), .O(z10));
  nand2  g049(.a(x23), .b(x08), .O(new_n133_));
  aoi21  g050(.a(x22), .b(new_n112_), .c(new_n93_), .O(new_n134_));
  aoi21  g051(.a(new_n134_), .b(new_n133_), .c(x10), .O(z11));
  nand2  g052(.a(x24), .b(x08), .O(new_n136_));
  aoi21  g053(.a(x23), .b(new_n112_), .c(new_n93_), .O(new_n137_));
  aoi21  g054(.a(new_n137_), .b(new_n136_), .c(x10), .O(z12));
  nand2  g055(.a(x25), .b(x08), .O(new_n139_));
  aoi21  g056(.a(x24), .b(new_n112_), .c(new_n93_), .O(new_n140_));
  aoi21  g057(.a(new_n140_), .b(new_n139_), .c(x10), .O(z13));
  nand2  g058(.a(x26), .b(x08), .O(new_n142_));
  aoi21  g059(.a(x25), .b(new_n112_), .c(new_n93_), .O(new_n143_));
  aoi21  g060(.a(new_n143_), .b(new_n142_), .c(x10), .O(z14));
  nand2  g061(.a(x27), .b(x08), .O(new_n145_));
  aoi21  g062(.a(x26), .b(new_n112_), .c(new_n93_), .O(new_n146_));
  aoi21  g063(.a(new_n146_), .b(new_n145_), .c(x10), .O(z15));
  inv1   g064(.a(x28), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(x08), .O(new_n149_));
  inv1   g066(.a(x27), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(new_n112_), .O(new_n151_));
  nand4  g068(.a(new_n151_), .b(new_n149_), .c(new_n84_), .d(x09), .O(new_n152_));
  inv1   g069(.a(new_n152_), .O(z16));
  inv1   g070(.a(x29), .O(new_n154_));
  nand2  g071(.a(new_n154_), .b(x08), .O(new_n155_));
  nand2  g072(.a(new_n148_), .b(new_n112_), .O(new_n156_));
  nand4  g073(.a(new_n156_), .b(new_n155_), .c(new_n84_), .d(x09), .O(new_n157_));
  inv1   g074(.a(new_n157_), .O(z17));
  nand2  g075(.a(x30), .b(x08), .O(new_n159_));
  aoi21  g076(.a(x29), .b(new_n112_), .c(new_n93_), .O(new_n160_));
  aoi21  g077(.a(new_n160_), .b(new_n159_), .c(x10), .O(z18));
  inv1   g078(.a(x00), .O(new_n162_));
  nand2  g079(.a(x08), .b(new_n162_), .O(new_n163_));
  inv1   g080(.a(x30), .O(new_n164_));
  nand2  g081(.a(new_n164_), .b(new_n112_), .O(new_n165_));
  nand4  g082(.a(new_n165_), .b(new_n163_), .c(new_n84_), .d(x09), .O(new_n166_));
  inv1   g083(.a(new_n166_), .O(z19));
  nor2   g084(.a(x10), .b(new_n93_), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(x32), .O(new_n169_));
  inv1   g086(.a(new_n169_), .O(z20));
  inv1   g087(.a(x33), .O(new_n171_));
  aoi21  g088(.a(new_n171_), .b(x09), .c(x10), .O(z21));
  inv1   g089(.a(x34), .O(new_n173_));
  aoi21  g090(.a(new_n173_), .b(x09), .c(x10), .O(z22));
  inv1   g091(.a(x35), .O(new_n175_));
  aoi21  g092(.a(new_n175_), .b(x09), .c(x10), .O(z23));
  inv1   g093(.a(x36), .O(new_n177_));
  aoi21  g094(.a(new_n177_), .b(x09), .c(x10), .O(z24));
  nand2  g095(.a(new_n168_), .b(x37), .O(new_n179_));
  inv1   g096(.a(new_n179_), .O(z25));
  nand2  g097(.a(new_n168_), .b(x38), .O(new_n181_));
  inv1   g098(.a(new_n181_), .O(z26));
  nand2  g099(.a(x14), .b(x00), .O(new_n183_));
  inv1   g100(.a(x14), .O(new_n184_));
  aoi21  g101(.a(x39), .b(new_n184_), .c(new_n93_), .O(new_n185_));
  aoi21  g102(.a(new_n185_), .b(new_n183_), .c(x10), .O(z27));
  nand2  g103(.a(x39), .b(x14), .O(new_n187_));
  aoi21  g104(.a(x40), .b(new_n184_), .c(new_n93_), .O(new_n188_));
  aoi21  g105(.a(new_n188_), .b(new_n187_), .c(x10), .O(z28));
  inv1   g106(.a(x40), .O(new_n190_));
  nand2  g107(.a(new_n190_), .b(x14), .O(new_n191_));
  inv1   g108(.a(x41), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(new_n184_), .O(new_n193_));
  nand4  g110(.a(new_n193_), .b(new_n191_), .c(new_n84_), .d(x09), .O(new_n194_));
  inv1   g111(.a(new_n194_), .O(z29));
  nand2  g112(.a(x41), .b(x14), .O(new_n196_));
  aoi21  g113(.a(x42), .b(new_n184_), .c(new_n93_), .O(new_n197_));
  aoi21  g114(.a(new_n197_), .b(new_n196_), .c(x10), .O(z30));
  nand2  g115(.a(x42), .b(x14), .O(new_n199_));
  aoi21  g116(.a(x43), .b(new_n184_), .c(new_n93_), .O(new_n200_));
  aoi21  g117(.a(new_n200_), .b(new_n199_), .c(x10), .O(z31));
  inv1   g118(.a(x43), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(x14), .O(new_n203_));
  inv1   g120(.a(x44), .O(new_n204_));
  nand2  g121(.a(new_n204_), .b(new_n184_), .O(new_n205_));
  nand4  g122(.a(new_n205_), .b(new_n203_), .c(new_n84_), .d(x09), .O(new_n206_));
  inv1   g123(.a(new_n206_), .O(z32));
  nand2  g124(.a(new_n204_), .b(x14), .O(new_n208_));
  inv1   g125(.a(x45), .O(new_n209_));
  nand2  g126(.a(new_n209_), .b(new_n184_), .O(new_n210_));
  nand4  g127(.a(new_n210_), .b(new_n208_), .c(new_n84_), .d(x09), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(z33));
  nand2  g129(.a(x45), .b(x14), .O(new_n213_));
  aoi21  g130(.a(x46), .b(new_n184_), .c(new_n93_), .O(new_n214_));
  aoi21  g131(.a(new_n214_), .b(new_n213_), .c(x10), .O(z34));
  inv1   g132(.a(x46), .O(new_n216_));
  nand2  g133(.a(new_n216_), .b(x14), .O(new_n217_));
  nand2  g134(.a(new_n184_), .b(new_n162_), .O(new_n218_));
  nand4  g135(.a(new_n218_), .b(new_n217_), .c(new_n84_), .d(x09), .O(new_n219_));
  inv1   g136(.a(new_n219_), .O(z35));
endmodule


