// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n156_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n173_, new_n174_, new_n175_,
    new_n176_, new_n177_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n184_, new_n185_, new_n186_, new_n187_, new_n189_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n208_, new_n209_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n227_, new_n228_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n236_, new_n237_,
    new_n238_, new_n239_, new_n241_, new_n242_, new_n243_, new_n244_,
    new_n246_, new_n247_, new_n249_, new_n250_, new_n251_, new_n252_;
  inv1   g000(.a(x04), .O(new_n84_));
  nand2  g001(.a(x07), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x33), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x10), .O(new_n87_));
  inv1   g004(.a(x07), .O(new_n88_));
  inv1   g005(.a(x11), .O(new_n89_));
  nand2  g006(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g007(.a(new_n90_), .b(new_n87_), .c(new_n85_), .O(new_n91_));
  inv1   g008(.a(new_n91_), .O(z00));
  inv1   g009(.a(x05), .O(new_n93_));
  nand2  g010(.a(x07), .b(new_n93_), .O(new_n94_));
  inv1   g011(.a(x12), .O(new_n95_));
  nand2  g012(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  nand3  g013(.a(new_n96_), .b(new_n94_), .c(new_n87_), .O(new_n97_));
  inv1   g014(.a(new_n97_), .O(z01));
  inv1   g015(.a(x06), .O(new_n99_));
  nand2  g016(.a(x07), .b(new_n99_), .O(new_n100_));
  inv1   g017(.a(x13), .O(new_n101_));
  nand2  g018(.a(new_n101_), .b(new_n88_), .O(new_n102_));
  nand3  g019(.a(new_n102_), .b(new_n100_), .c(new_n87_), .O(new_n103_));
  inv1   g020(.a(new_n103_), .O(z02));
  nand2  g021(.a(x07), .b(x01), .O(new_n105_));
  aoi21  g022(.a(x14), .b(new_n88_), .c(new_n86_), .O(new_n106_));
  aoi21  g023(.a(new_n106_), .b(new_n105_), .c(x10), .O(z03));
  inv1   g024(.a(x02), .O(new_n108_));
  nand2  g025(.a(x07), .b(new_n108_), .O(new_n109_));
  inv1   g026(.a(x15), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n109_), .c(new_n87_), .O(new_n112_));
  inv1   g029(.a(new_n112_), .O(z04));
  inv1   g030(.a(x03), .O(new_n114_));
  nand2  g031(.a(x07), .b(new_n114_), .O(new_n115_));
  inv1   g032(.a(x16), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n88_), .O(new_n117_));
  nand3  g034(.a(new_n117_), .b(new_n115_), .c(new_n87_), .O(new_n118_));
  inv1   g035(.a(new_n118_), .O(z05));
  nand2  g036(.a(x18), .b(x08), .O(new_n120_));
  inv1   g037(.a(x08), .O(new_n121_));
  aoi21  g038(.a(x17), .b(new_n121_), .c(new_n86_), .O(new_n122_));
  aoi21  g039(.a(new_n122_), .b(new_n120_), .c(x10), .O(z06));
  nand2  g040(.a(x19), .b(x08), .O(new_n124_));
  aoi21  g041(.a(x18), .b(new_n121_), .c(new_n86_), .O(new_n125_));
  aoi21  g042(.a(new_n125_), .b(new_n124_), .c(x10), .O(z07));
  nand2  g043(.a(x20), .b(x08), .O(new_n127_));
  aoi21  g044(.a(x19), .b(new_n121_), .c(new_n86_), .O(new_n128_));
  aoi21  g045(.a(new_n128_), .b(new_n127_), .c(x10), .O(z08));
  inv1   g046(.a(x21), .O(new_n130_));
  nand2  g047(.a(new_n130_), .b(x08), .O(new_n131_));
  inv1   g048(.a(x20), .O(new_n132_));
  nand2  g049(.a(new_n132_), .b(new_n121_), .O(new_n133_));
  nand3  g050(.a(new_n133_), .b(new_n131_), .c(new_n87_), .O(new_n134_));
  inv1   g051(.a(new_n134_), .O(z09));
  inv1   g052(.a(x22), .O(new_n136_));
  nand2  g053(.a(new_n136_), .b(x08), .O(new_n137_));
  nand2  g054(.a(new_n130_), .b(new_n121_), .O(new_n138_));
  nand3  g055(.a(new_n138_), .b(new_n137_), .c(new_n87_), .O(new_n139_));
  inv1   g056(.a(new_n139_), .O(z10));
  inv1   g057(.a(x23), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(x08), .O(new_n142_));
  nand2  g059(.a(new_n136_), .b(new_n121_), .O(new_n143_));
  nand3  g060(.a(new_n143_), .b(new_n142_), .c(new_n87_), .O(new_n144_));
  inv1   g061(.a(new_n144_), .O(z11));
  inv1   g062(.a(x24), .O(new_n146_));
  nand2  g063(.a(new_n146_), .b(x08), .O(new_n147_));
  nand2  g064(.a(new_n141_), .b(new_n121_), .O(new_n148_));
  nand3  g065(.a(new_n148_), .b(new_n147_), .c(new_n87_), .O(new_n149_));
  inv1   g066(.a(new_n149_), .O(z12));
  inv1   g067(.a(x25), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(x08), .O(new_n152_));
  nand2  g069(.a(new_n146_), .b(new_n121_), .O(new_n153_));
  nand3  g070(.a(new_n153_), .b(new_n152_), .c(new_n87_), .O(new_n154_));
  inv1   g071(.a(new_n154_), .O(z13));
  inv1   g072(.a(x26), .O(new_n156_));
  nand2  g073(.a(new_n156_), .b(x08), .O(new_n157_));
  nand2  g074(.a(new_n151_), .b(new_n121_), .O(new_n158_));
  nand3  g075(.a(new_n158_), .b(new_n157_), .c(new_n87_), .O(new_n159_));
  inv1   g076(.a(new_n159_), .O(z14));
  nand2  g077(.a(x27), .b(x08), .O(new_n161_));
  aoi21  g078(.a(x26), .b(new_n121_), .c(new_n86_), .O(new_n162_));
  aoi21  g079(.a(new_n162_), .b(new_n161_), .c(x10), .O(z15));
  inv1   g080(.a(x28), .O(new_n164_));
  nand2  g081(.a(new_n164_), .b(x08), .O(new_n165_));
  inv1   g082(.a(x27), .O(new_n166_));
  nand2  g083(.a(new_n166_), .b(new_n121_), .O(new_n167_));
  nand3  g084(.a(new_n167_), .b(new_n165_), .c(new_n87_), .O(new_n168_));
  inv1   g085(.a(new_n168_), .O(z16));
  nand2  g086(.a(x29), .b(x08), .O(new_n170_));
  aoi21  g087(.a(x28), .b(new_n121_), .c(new_n86_), .O(new_n171_));
  aoi21  g088(.a(new_n171_), .b(new_n170_), .c(x10), .O(z17));
  inv1   g089(.a(x30), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(x08), .O(new_n174_));
  inv1   g091(.a(x29), .O(new_n175_));
  nand2  g092(.a(new_n175_), .b(new_n121_), .O(new_n176_));
  nand3  g093(.a(new_n176_), .b(new_n174_), .c(new_n87_), .O(new_n177_));
  inv1   g094(.a(new_n177_), .O(z18));
  nand2  g095(.a(x08), .b(x00), .O(new_n179_));
  aoi21  g096(.a(x30), .b(new_n121_), .c(new_n86_), .O(new_n180_));
  aoi21  g097(.a(new_n180_), .b(new_n179_), .c(x10), .O(z19));
  inv1   g098(.a(x32), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(x09), .O(new_n183_));
  inv1   g100(.a(x09), .O(new_n184_));
  inv1   g101(.a(x31), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  nand3  g103(.a(new_n186_), .b(new_n183_), .c(new_n87_), .O(new_n187_));
  inv1   g104(.a(new_n187_), .O(z20));
  nor2   g105(.a(x32), .b(x09), .O(new_n189_));
  nor3   g106(.a(new_n189_), .b(new_n86_), .c(x10), .O(z21));
  inv1   g107(.a(x34), .O(new_n191_));
  nor2   g108(.a(new_n86_), .b(new_n184_), .O(new_n192_));
  aoi21  g109(.a(new_n192_), .b(new_n191_), .c(x10), .O(z22));
  inv1   g110(.a(x35), .O(new_n194_));
  nand2  g111(.a(new_n194_), .b(x09), .O(new_n195_));
  nand2  g112(.a(new_n191_), .b(new_n184_), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n195_), .c(new_n87_), .O(new_n197_));
  inv1   g114(.a(new_n197_), .O(z23));
  nand2  g115(.a(x36), .b(x09), .O(new_n199_));
  aoi21  g116(.a(x35), .b(new_n184_), .c(new_n86_), .O(new_n200_));
  aoi21  g117(.a(new_n200_), .b(new_n199_), .c(x10), .O(z24));
  inv1   g118(.a(x37), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(x09), .O(new_n203_));
  inv1   g120(.a(x36), .O(new_n204_));
  nand2  g121(.a(new_n204_), .b(new_n184_), .O(new_n205_));
  nand3  g122(.a(new_n205_), .b(new_n203_), .c(new_n87_), .O(new_n206_));
  inv1   g123(.a(new_n206_), .O(z25));
  nand2  g124(.a(x38), .b(x09), .O(new_n208_));
  aoi21  g125(.a(x37), .b(new_n184_), .c(new_n86_), .O(new_n209_));
  aoi21  g126(.a(new_n209_), .b(new_n208_), .c(x10), .O(z26));
  inv1   g127(.a(x00), .O(new_n211_));
  nand2  g128(.a(x14), .b(new_n211_), .O(new_n212_));
  oai21  g129(.a(x39), .b(x14), .c(new_n212_), .O(new_n213_));
  inv1   g130(.a(x38), .O(new_n214_));
  nand2  g131(.a(new_n214_), .b(new_n184_), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(new_n87_), .O(new_n216_));
  aoi21  g133(.a(new_n213_), .b(x09), .c(new_n216_), .O(z27));
  nor2   g134(.a(x14), .b(new_n184_), .O(new_n218_));
  nand2  g135(.a(new_n218_), .b(x40), .O(new_n219_));
  inv1   g136(.a(x14), .O(new_n220_));
  nand2  g137(.a(new_n220_), .b(x09), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(x39), .c(new_n86_), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n219_), .c(x10), .O(z28));
  nand2  g140(.a(new_n218_), .b(x41), .O(new_n224_));
  aoi21  g141(.a(new_n221_), .b(x40), .c(new_n86_), .O(new_n225_));
  aoi21  g142(.a(new_n225_), .b(new_n224_), .c(x10), .O(z29));
  nand2  g143(.a(new_n218_), .b(x42), .O(new_n227_));
  aoi21  g144(.a(new_n221_), .b(x41), .c(new_n86_), .O(new_n228_));
  aoi21  g145(.a(new_n228_), .b(new_n227_), .c(x10), .O(z30));
  inv1   g146(.a(x43), .O(new_n230_));
  nand3  g147(.a(new_n230_), .b(new_n220_), .c(x09), .O(new_n231_));
  inv1   g148(.a(x42), .O(new_n232_));
  nand2  g149(.a(new_n221_), .b(new_n232_), .O(new_n233_));
  nand3  g150(.a(new_n233_), .b(new_n231_), .c(new_n87_), .O(new_n234_));
  inv1   g151(.a(new_n234_), .O(z31));
  inv1   g152(.a(x44), .O(new_n236_));
  nand3  g153(.a(new_n236_), .b(new_n220_), .c(x09), .O(new_n237_));
  nand2  g154(.a(new_n221_), .b(new_n230_), .O(new_n238_));
  nand3  g155(.a(new_n238_), .b(new_n237_), .c(new_n87_), .O(new_n239_));
  inv1   g156(.a(new_n239_), .O(z32));
  inv1   g157(.a(x45), .O(new_n241_));
  nand3  g158(.a(new_n241_), .b(new_n220_), .c(x09), .O(new_n242_));
  nand2  g159(.a(new_n221_), .b(new_n236_), .O(new_n243_));
  nand3  g160(.a(new_n243_), .b(new_n242_), .c(new_n87_), .O(new_n244_));
  inv1   g161(.a(new_n244_), .O(z33));
  nand2  g162(.a(new_n218_), .b(x46), .O(new_n246_));
  aoi21  g163(.a(new_n221_), .b(x45), .c(new_n86_), .O(new_n247_));
  aoi21  g164(.a(new_n247_), .b(new_n246_), .c(x10), .O(z34));
  nand3  g165(.a(new_n220_), .b(x09), .c(new_n211_), .O(new_n249_));
  inv1   g166(.a(x46), .O(new_n250_));
  nand2  g167(.a(new_n221_), .b(new_n250_), .O(new_n251_));
  nand3  g168(.a(new_n251_), .b(new_n249_), .c(new_n87_), .O(new_n252_));
  inv1   g169(.a(new_n252_), .O(z35));
endmodule


