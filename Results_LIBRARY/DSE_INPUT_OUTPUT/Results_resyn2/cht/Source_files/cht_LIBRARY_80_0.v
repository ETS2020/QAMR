// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n157_, new_n158_, new_n159_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n166_, new_n167_, new_n169_,
    new_n170_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n191_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n200_, new_n201_,
    new_n202_, new_n203_, new_n204_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n219_, new_n220_, new_n221_, new_n223_,
    new_n224_, new_n225_, new_n226_, new_n227_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n234_, new_n235_, new_n237_, new_n238_,
    new_n239_, new_n240_, new_n241_, new_n243_, new_n244_, new_n245_,
    new_n246_, new_n248_, new_n249_;
  inv1   g000(.a(x04), .O(new_n84_));
  nand2  g001(.a(x07), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x01), .O(new_n86_));
  nor2   g003(.a(x10), .b(new_n86_), .O(new_n87_));
  inv1   g004(.a(x07), .O(new_n88_));
  inv1   g005(.a(x11), .O(new_n89_));
  nand2  g006(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g007(.a(new_n90_), .b(new_n87_), .c(new_n85_), .O(new_n91_));
  inv1   g008(.a(new_n91_), .O(z00));
  inv1   g009(.a(x05), .O(new_n93_));
  nand2  g010(.a(x07), .b(new_n93_), .O(new_n94_));
  inv1   g011(.a(x12), .O(new_n95_));
  nand2  g012(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  nand3  g013(.a(new_n96_), .b(new_n94_), .c(new_n87_), .O(new_n97_));
  inv1   g014(.a(new_n97_), .O(z01));
  nand2  g015(.a(x07), .b(x06), .O(new_n99_));
  aoi21  g016(.a(x13), .b(new_n88_), .c(new_n86_), .O(new_n100_));
  aoi21  g017(.a(new_n100_), .b(new_n99_), .c(x10), .O(z02));
  nor2   g018(.a(x14), .b(x07), .O(new_n102_));
  nor3   g019(.a(new_n102_), .b(x10), .c(new_n86_), .O(z03));
  inv1   g020(.a(x02), .O(new_n104_));
  nand2  g021(.a(x07), .b(new_n104_), .O(new_n105_));
  inv1   g022(.a(x15), .O(new_n106_));
  nand2  g023(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nand3  g024(.a(new_n107_), .b(new_n105_), .c(new_n87_), .O(new_n108_));
  inv1   g025(.a(new_n108_), .O(z04));
  nand2  g026(.a(x07), .b(x03), .O(new_n110_));
  aoi21  g027(.a(x16), .b(new_n88_), .c(new_n86_), .O(new_n111_));
  aoi21  g028(.a(new_n111_), .b(new_n110_), .c(x10), .O(z05));
  inv1   g029(.a(x18), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(x08), .O(new_n114_));
  inv1   g031(.a(x08), .O(new_n115_));
  inv1   g032(.a(x17), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g034(.a(new_n117_), .b(new_n114_), .c(new_n87_), .O(new_n118_));
  inv1   g035(.a(new_n118_), .O(z06));
  inv1   g036(.a(x19), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(x08), .O(new_n121_));
  nand2  g038(.a(new_n113_), .b(new_n115_), .O(new_n122_));
  nand3  g039(.a(new_n122_), .b(new_n121_), .c(new_n87_), .O(new_n123_));
  inv1   g040(.a(new_n123_), .O(z07));
  inv1   g041(.a(x20), .O(new_n125_));
  nand2  g042(.a(new_n125_), .b(x08), .O(new_n126_));
  nand2  g043(.a(new_n120_), .b(new_n115_), .O(new_n127_));
  nand3  g044(.a(new_n127_), .b(new_n126_), .c(new_n87_), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z08));
  inv1   g046(.a(x21), .O(new_n130_));
  nand2  g047(.a(new_n130_), .b(x08), .O(new_n131_));
  nand2  g048(.a(new_n125_), .b(new_n115_), .O(new_n132_));
  nand3  g049(.a(new_n132_), .b(new_n131_), .c(new_n87_), .O(new_n133_));
  inv1   g050(.a(new_n133_), .O(z09));
  inv1   g051(.a(x22), .O(new_n135_));
  nand2  g052(.a(new_n135_), .b(x08), .O(new_n136_));
  nand2  g053(.a(new_n130_), .b(new_n115_), .O(new_n137_));
  nand3  g054(.a(new_n137_), .b(new_n136_), .c(new_n87_), .O(new_n138_));
  inv1   g055(.a(new_n138_), .O(z10));
  nand2  g056(.a(x23), .b(x08), .O(new_n140_));
  aoi21  g057(.a(x22), .b(new_n115_), .c(new_n86_), .O(new_n141_));
  aoi21  g058(.a(new_n141_), .b(new_n140_), .c(x10), .O(z11));
  inv1   g059(.a(x24), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(x08), .O(new_n144_));
  inv1   g061(.a(x23), .O(new_n145_));
  nand2  g062(.a(new_n145_), .b(new_n115_), .O(new_n146_));
  nand3  g063(.a(new_n146_), .b(new_n144_), .c(new_n87_), .O(new_n147_));
  inv1   g064(.a(new_n147_), .O(z12));
  nand2  g065(.a(x25), .b(x08), .O(new_n149_));
  aoi21  g066(.a(x24), .b(new_n115_), .c(new_n86_), .O(new_n150_));
  aoi21  g067(.a(new_n150_), .b(new_n149_), .c(x10), .O(z13));
  nand2  g068(.a(x26), .b(x08), .O(new_n152_));
  aoi21  g069(.a(x25), .b(new_n115_), .c(new_n86_), .O(new_n153_));
  aoi21  g070(.a(new_n153_), .b(new_n152_), .c(x10), .O(z14));
  inv1   g071(.a(x27), .O(new_n155_));
  nand2  g072(.a(new_n155_), .b(x08), .O(new_n156_));
  inv1   g073(.a(x26), .O(new_n157_));
  nand2  g074(.a(new_n157_), .b(new_n115_), .O(new_n158_));
  nand3  g075(.a(new_n158_), .b(new_n156_), .c(new_n87_), .O(new_n159_));
  inv1   g076(.a(new_n159_), .O(z15));
  inv1   g077(.a(x28), .O(new_n161_));
  nand2  g078(.a(new_n161_), .b(x08), .O(new_n162_));
  nand2  g079(.a(new_n155_), .b(new_n115_), .O(new_n163_));
  nand3  g080(.a(new_n163_), .b(new_n162_), .c(new_n87_), .O(new_n164_));
  inv1   g081(.a(new_n164_), .O(z16));
  nand2  g082(.a(x29), .b(x08), .O(new_n166_));
  aoi21  g083(.a(x28), .b(new_n115_), .c(new_n86_), .O(new_n167_));
  aoi21  g084(.a(new_n167_), .b(new_n166_), .c(x10), .O(z17));
  nand2  g085(.a(x30), .b(x08), .O(new_n169_));
  aoi21  g086(.a(x29), .b(new_n115_), .c(new_n86_), .O(new_n170_));
  aoi21  g087(.a(new_n170_), .b(new_n169_), .c(x10), .O(z18));
  nand2  g088(.a(x08), .b(x00), .O(new_n172_));
  aoi21  g089(.a(x30), .b(new_n115_), .c(new_n86_), .O(new_n173_));
  aoi21  g090(.a(new_n173_), .b(new_n172_), .c(x10), .O(z19));
  nand2  g091(.a(x32), .b(x09), .O(new_n175_));
  inv1   g092(.a(x09), .O(new_n176_));
  aoi21  g093(.a(x31), .b(new_n176_), .c(new_n86_), .O(new_n177_));
  aoi21  g094(.a(new_n177_), .b(new_n175_), .c(x10), .O(z20));
  nand2  g095(.a(x33), .b(x09), .O(new_n179_));
  aoi21  g096(.a(x32), .b(new_n176_), .c(new_n86_), .O(new_n180_));
  aoi21  g097(.a(new_n180_), .b(new_n179_), .c(x10), .O(z21));
  inv1   g098(.a(x34), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(x09), .O(new_n183_));
  inv1   g100(.a(x33), .O(new_n184_));
  nand2  g101(.a(new_n184_), .b(new_n176_), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n183_), .c(new_n87_), .O(new_n186_));
  inv1   g103(.a(new_n186_), .O(z22));
  nand2  g104(.a(x35), .b(x09), .O(new_n188_));
  aoi21  g105(.a(x34), .b(new_n176_), .c(new_n86_), .O(new_n189_));
  aoi21  g106(.a(new_n189_), .b(new_n188_), .c(x10), .O(z23));
  inv1   g107(.a(x36), .O(new_n191_));
  nand2  g108(.a(new_n191_), .b(x09), .O(new_n192_));
  inv1   g109(.a(x35), .O(new_n193_));
  nand2  g110(.a(new_n193_), .b(new_n176_), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n192_), .c(new_n87_), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(z24));
  nand2  g113(.a(x37), .b(x09), .O(new_n197_));
  aoi21  g114(.a(x36), .b(new_n176_), .c(new_n86_), .O(new_n198_));
  aoi21  g115(.a(new_n198_), .b(new_n197_), .c(x10), .O(z25));
  inv1   g116(.a(x38), .O(new_n200_));
  nand2  g117(.a(new_n200_), .b(x09), .O(new_n201_));
  inv1   g118(.a(x37), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(new_n176_), .O(new_n203_));
  nand3  g120(.a(new_n203_), .b(new_n201_), .c(new_n87_), .O(new_n204_));
  inv1   g121(.a(new_n204_), .O(z26));
  inv1   g122(.a(x14), .O(new_n206_));
  inv1   g123(.a(x39), .O(new_n207_));
  nand2  g124(.a(new_n207_), .b(new_n206_), .O(new_n208_));
  oai21  g125(.a(new_n206_), .b(x00), .c(new_n208_), .O(new_n209_));
  nand2  g126(.a(new_n200_), .b(new_n176_), .O(new_n210_));
  nand2  g127(.a(new_n210_), .b(new_n87_), .O(new_n211_));
  aoi21  g128(.a(new_n209_), .b(x09), .c(new_n211_), .O(z27));
  inv1   g129(.a(x40), .O(new_n213_));
  nand3  g130(.a(new_n213_), .b(new_n206_), .c(x09), .O(new_n214_));
  nand2  g131(.a(new_n206_), .b(x09), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(new_n207_), .O(new_n216_));
  nand3  g133(.a(new_n216_), .b(new_n214_), .c(new_n87_), .O(new_n217_));
  inv1   g134(.a(new_n217_), .O(z28));
  nor2   g135(.a(x14), .b(new_n176_), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(x41), .O(new_n220_));
  aoi21  g137(.a(new_n215_), .b(x40), .c(new_n86_), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(new_n220_), .c(x10), .O(z29));
  inv1   g139(.a(x42), .O(new_n223_));
  nand3  g140(.a(new_n223_), .b(new_n206_), .c(x09), .O(new_n224_));
  inv1   g141(.a(x41), .O(new_n225_));
  nand2  g142(.a(new_n215_), .b(new_n225_), .O(new_n226_));
  nand3  g143(.a(new_n226_), .b(new_n224_), .c(new_n87_), .O(new_n227_));
  inv1   g144(.a(new_n227_), .O(z30));
  inv1   g145(.a(x43), .O(new_n229_));
  nand3  g146(.a(new_n229_), .b(new_n206_), .c(x09), .O(new_n230_));
  nand2  g147(.a(new_n215_), .b(new_n223_), .O(new_n231_));
  nand3  g148(.a(new_n231_), .b(new_n230_), .c(new_n87_), .O(new_n232_));
  inv1   g149(.a(new_n232_), .O(z31));
  nand2  g150(.a(new_n219_), .b(x44), .O(new_n234_));
  aoi21  g151(.a(new_n215_), .b(x43), .c(new_n86_), .O(new_n235_));
  aoi21  g152(.a(new_n235_), .b(new_n234_), .c(x10), .O(z32));
  inv1   g153(.a(x45), .O(new_n237_));
  nand3  g154(.a(new_n237_), .b(new_n206_), .c(x09), .O(new_n238_));
  inv1   g155(.a(x44), .O(new_n239_));
  nand2  g156(.a(new_n215_), .b(new_n239_), .O(new_n240_));
  nand3  g157(.a(new_n240_), .b(new_n238_), .c(new_n87_), .O(new_n241_));
  inv1   g158(.a(new_n241_), .O(z33));
  inv1   g159(.a(x46), .O(new_n243_));
  nand3  g160(.a(new_n243_), .b(new_n206_), .c(x09), .O(new_n244_));
  nand2  g161(.a(new_n215_), .b(new_n237_), .O(new_n245_));
  nand3  g162(.a(new_n245_), .b(new_n244_), .c(new_n87_), .O(new_n246_));
  inv1   g163(.a(new_n246_), .O(z34));
  nand2  g164(.a(new_n219_), .b(x00), .O(new_n248_));
  aoi21  g165(.a(new_n215_), .b(x46), .c(new_n86_), .O(new_n249_));
  aoi21  g166(.a(new_n249_), .b(new_n248_), .c(x10), .O(z35));
endmodule


