// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n162_, new_n163_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n173_, new_n174_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n199_,
    new_n200_, new_n201_, new_n202_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n209_, new_n210_, new_n211_, new_n212_, new_n214_,
    new_n215_, new_n216_, new_n217_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_;
  nand2  g000(.a(x36), .b(x10), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nor2   g002(.a(new_n85_), .b(x04), .O(new_n86_));
  inv1   g003(.a(x10), .O(new_n87_));
  oai21  g004(.a(x11), .b(x07), .c(new_n87_), .O(new_n88_));
  oai21  g005(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(z00));
  nor2   g006(.a(new_n85_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n87_), .O(new_n91_));
  nor2   g008(.a(new_n91_), .b(new_n90_), .O(z01));
  nor2   g009(.a(new_n85_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n87_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n84_), .O(z02));
  nor2   g012(.a(new_n85_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n87_), .O(new_n97_));
  oai21  g014(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z03));
  nor2   g015(.a(new_n85_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n87_), .O(new_n100_));
  nor2   g017(.a(new_n100_), .b(new_n99_), .O(z04));
  nor2   g018(.a(new_n85_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n87_), .O(new_n103_));
  oai21  g020(.a(new_n103_), .b(new_n102_), .c(new_n84_), .O(z05));
  inv1   g021(.a(x08), .O(new_n105_));
  nor2   g022(.a(x18), .b(new_n105_), .O(new_n106_));
  oai21  g023(.a(x17), .b(x08), .c(new_n87_), .O(new_n107_));
  nor2   g024(.a(new_n107_), .b(new_n106_), .O(z06));
  nor2   g025(.a(x19), .b(new_n105_), .O(new_n109_));
  oai21  g026(.a(x18), .b(x08), .c(new_n87_), .O(new_n110_));
  nor2   g027(.a(new_n110_), .b(new_n109_), .O(z07));
  inv1   g028(.a(x20), .O(new_n112_));
  oai21  g029(.a(x19), .b(x08), .c(new_n87_), .O(new_n113_));
  aoi21  g030(.a(new_n112_), .b(x08), .c(new_n113_), .O(z08));
  inv1   g031(.a(x21), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(x08), .O(new_n116_));
  nand2  g033(.a(new_n112_), .b(new_n105_), .O(new_n117_));
  nand3  g034(.a(new_n117_), .b(new_n116_), .c(new_n87_), .O(new_n118_));
  nand2  g035(.a(new_n118_), .b(new_n84_), .O(z09));
  nor2   g036(.a(x22), .b(new_n105_), .O(new_n120_));
  oai21  g037(.a(x21), .b(x08), .c(new_n87_), .O(new_n121_));
  oai21  g038(.a(new_n121_), .b(new_n120_), .c(new_n84_), .O(z10));
  nor2   g039(.a(x23), .b(new_n105_), .O(new_n123_));
  oai21  g040(.a(x22), .b(x08), .c(new_n87_), .O(new_n124_));
  nor2   g041(.a(new_n124_), .b(new_n123_), .O(z11));
  nor2   g042(.a(x24), .b(new_n105_), .O(new_n126_));
  oai21  g043(.a(x23), .b(x08), .c(new_n87_), .O(new_n127_));
  nor2   g044(.a(new_n127_), .b(new_n126_), .O(z12));
  inv1   g045(.a(x25), .O(new_n129_));
  oai21  g046(.a(x24), .b(x08), .c(new_n87_), .O(new_n130_));
  aoi21  g047(.a(new_n129_), .b(x08), .c(new_n130_), .O(z13));
  nor2   g048(.a(x26), .b(new_n105_), .O(new_n132_));
  oai21  g049(.a(x25), .b(x08), .c(new_n87_), .O(new_n133_));
  oai21  g050(.a(new_n133_), .b(new_n132_), .c(new_n84_), .O(z14));
  inv1   g051(.a(x27), .O(new_n135_));
  oai21  g052(.a(x26), .b(x08), .c(new_n87_), .O(new_n136_));
  aoi21  g053(.a(new_n135_), .b(x08), .c(new_n136_), .O(z15));
  inv1   g054(.a(x28), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(x08), .O(new_n139_));
  nand2  g056(.a(new_n135_), .b(new_n105_), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n87_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n84_), .O(z16));
  inv1   g059(.a(x29), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(x08), .O(new_n144_));
  nand2  g061(.a(new_n138_), .b(new_n105_), .O(new_n145_));
  nand3  g062(.a(new_n145_), .b(new_n144_), .c(new_n87_), .O(new_n146_));
  nand2  g063(.a(new_n146_), .b(new_n84_), .O(z17));
  inv1   g064(.a(x30), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(x08), .O(new_n149_));
  nand2  g066(.a(new_n143_), .b(new_n105_), .O(new_n150_));
  nand3  g067(.a(new_n150_), .b(new_n149_), .c(new_n87_), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(new_n84_), .O(z18));
  inv1   g069(.a(x00), .O(new_n153_));
  nand2  g070(.a(x08), .b(new_n153_), .O(new_n154_));
  nand2  g071(.a(new_n148_), .b(new_n105_), .O(new_n155_));
  nand3  g072(.a(new_n155_), .b(new_n154_), .c(new_n87_), .O(new_n156_));
  nand2  g073(.a(new_n156_), .b(new_n84_), .O(z19));
  inv1   g074(.a(x09), .O(new_n158_));
  nor2   g075(.a(x32), .b(new_n158_), .O(new_n159_));
  oai21  g076(.a(x31), .b(x09), .c(new_n87_), .O(new_n160_));
  nor2   g077(.a(new_n160_), .b(new_n159_), .O(z20));
  nor2   g078(.a(x33), .b(new_n158_), .O(new_n162_));
  oai21  g079(.a(x32), .b(x09), .c(new_n87_), .O(new_n163_));
  nor2   g080(.a(new_n163_), .b(new_n162_), .O(z21));
  inv1   g081(.a(x34), .O(new_n165_));
  oai21  g082(.a(x33), .b(x09), .c(new_n87_), .O(new_n166_));
  aoi21  g083(.a(new_n165_), .b(x09), .c(new_n166_), .O(z22));
  inv1   g084(.a(x35), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(x09), .O(new_n169_));
  nand2  g086(.a(new_n165_), .b(new_n158_), .O(new_n170_));
  nand3  g087(.a(new_n170_), .b(new_n169_), .c(new_n87_), .O(new_n171_));
  nand2  g088(.a(new_n171_), .b(new_n84_), .O(z23));
  nand2  g089(.a(new_n87_), .b(new_n158_), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(x36), .O(new_n174_));
  oai21  g091(.a(new_n173_), .b(new_n168_), .c(new_n174_), .O(z24));
  oai21  g092(.a(x10), .b(new_n158_), .c(x36), .O(new_n176_));
  nand3  g093(.a(x37), .b(new_n87_), .c(x09), .O(new_n177_));
  nand2  g094(.a(new_n177_), .b(new_n176_), .O(z25));
  nor2   g095(.a(x38), .b(new_n158_), .O(new_n179_));
  oai21  g096(.a(x37), .b(x09), .c(new_n87_), .O(new_n180_));
  oai21  g097(.a(new_n180_), .b(new_n179_), .c(new_n84_), .O(z26));
  inv1   g098(.a(x14), .O(new_n182_));
  inv1   g099(.a(x39), .O(new_n183_));
  nand2  g100(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nand2  g101(.a(x14), .b(new_n153_), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  oai21  g103(.a(x38), .b(x09), .c(new_n87_), .O(new_n187_));
  aoi21  g104(.a(new_n186_), .b(x09), .c(new_n187_), .O(z27));
  inv1   g105(.a(x40), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n182_), .c(x09), .O(new_n190_));
  oai21  g107(.a(x14), .b(new_n158_), .c(new_n183_), .O(new_n191_));
  nand3  g108(.a(new_n191_), .b(new_n190_), .c(new_n87_), .O(new_n192_));
  inv1   g109(.a(new_n192_), .O(z28));
  inv1   g110(.a(x41), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n182_), .c(x09), .O(new_n195_));
  oai21  g112(.a(x14), .b(new_n158_), .c(new_n189_), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n195_), .c(new_n87_), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(new_n84_), .O(z29));
  inv1   g115(.a(x42), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n182_), .c(x09), .O(new_n200_));
  oai21  g117(.a(x14), .b(new_n158_), .c(new_n194_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n200_), .c(new_n87_), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(new_n84_), .O(z30));
  inv1   g120(.a(x43), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n182_), .c(x09), .O(new_n205_));
  oai21  g122(.a(x14), .b(new_n158_), .c(new_n199_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n205_), .c(new_n87_), .O(new_n207_));
  inv1   g124(.a(new_n207_), .O(z31));
  inv1   g125(.a(x44), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n182_), .c(x09), .O(new_n210_));
  oai21  g127(.a(x14), .b(new_n158_), .c(new_n204_), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(new_n210_), .c(new_n87_), .O(new_n212_));
  nand2  g129(.a(new_n212_), .b(new_n84_), .O(z32));
  inv1   g130(.a(x45), .O(new_n214_));
  nand3  g131(.a(new_n214_), .b(new_n182_), .c(x09), .O(new_n215_));
  oai21  g132(.a(x14), .b(new_n158_), .c(new_n209_), .O(new_n216_));
  nand3  g133(.a(new_n216_), .b(new_n215_), .c(new_n87_), .O(new_n217_));
  nand2  g134(.a(new_n217_), .b(new_n84_), .O(z33));
  inv1   g135(.a(x46), .O(new_n219_));
  nand3  g136(.a(new_n219_), .b(new_n182_), .c(x09), .O(new_n220_));
  oai21  g137(.a(x14), .b(new_n158_), .c(new_n214_), .O(new_n221_));
  nand3  g138(.a(new_n221_), .b(new_n220_), .c(new_n87_), .O(new_n222_));
  inv1   g139(.a(new_n222_), .O(z34));
  nand3  g140(.a(new_n182_), .b(x09), .c(new_n153_), .O(new_n224_));
  oai21  g141(.a(x14), .b(new_n158_), .c(new_n219_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n224_), .c(new_n87_), .O(new_n226_));
  nand2  g143(.a(new_n226_), .b(new_n84_), .O(z35));
endmodule


