// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n179_, new_n180_, new_n181_, new_n182_, new_n183_, new_n184_,
    new_n185_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n197_, new_n198_, new_n199_,
    new_n201_, new_n202_, new_n203_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n210_, new_n212_, new_n213_, new_n215_,
    new_n216_, new_n217_, new_n218_, new_n219_, new_n221_, new_n222_;
  nand2  g000(.a(x14), .b(x10), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  nor2   g002(.a(new_n85_), .b(x04), .O(new_n86_));
  inv1   g003(.a(x10), .O(new_n87_));
  oai21  g004(.a(x11), .b(x07), .c(new_n87_), .O(new_n88_));
  oai21  g005(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(z00));
  nor2   g006(.a(new_n85_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n87_), .O(new_n91_));
  nor2   g008(.a(new_n91_), .b(new_n90_), .O(z01));
  nor2   g009(.a(new_n85_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n87_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n84_), .O(z02));
  nor2   g012(.a(new_n85_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n87_), .O(new_n97_));
  nor2   g014(.a(new_n97_), .b(new_n96_), .O(z03));
  nor2   g015(.a(new_n85_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n87_), .O(new_n100_));
  oai21  g017(.a(new_n100_), .b(new_n99_), .c(new_n84_), .O(z04));
  nor2   g018(.a(new_n85_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n87_), .O(new_n103_));
  nor2   g020(.a(new_n103_), .b(new_n102_), .O(z05));
  inv1   g021(.a(x18), .O(new_n105_));
  nand2  g022(.a(new_n105_), .b(x08), .O(new_n106_));
  inv1   g023(.a(x08), .O(new_n107_));
  inv1   g024(.a(x17), .O(new_n108_));
  nand2  g025(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g026(.a(new_n109_), .b(new_n106_), .c(new_n87_), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n84_), .O(z06));
  inv1   g028(.a(x19), .O(new_n112_));
  nand2  g029(.a(new_n112_), .b(x08), .O(new_n113_));
  nand2  g030(.a(new_n105_), .b(new_n107_), .O(new_n114_));
  nand3  g031(.a(new_n114_), .b(new_n113_), .c(new_n87_), .O(new_n115_));
  nand2  g032(.a(new_n115_), .b(new_n84_), .O(z07));
  inv1   g033(.a(x20), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(x08), .O(new_n118_));
  nand2  g035(.a(new_n112_), .b(new_n107_), .O(new_n119_));
  nand3  g036(.a(new_n119_), .b(new_n118_), .c(new_n87_), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(new_n84_), .O(z08));
  nor2   g038(.a(x21), .b(new_n107_), .O(new_n122_));
  oai21  g039(.a(x20), .b(x08), .c(new_n87_), .O(new_n123_));
  oai21  g040(.a(new_n123_), .b(new_n122_), .c(new_n84_), .O(z09));
  inv1   g041(.a(x22), .O(new_n125_));
  oai21  g042(.a(x21), .b(x08), .c(new_n87_), .O(new_n126_));
  aoi21  g043(.a(new_n125_), .b(x08), .c(new_n126_), .O(z10));
  nor2   g044(.a(x23), .b(new_n107_), .O(new_n128_));
  oai21  g045(.a(x22), .b(x08), .c(new_n87_), .O(new_n129_));
  oai21  g046(.a(new_n129_), .b(new_n128_), .c(new_n84_), .O(z11));
  nor2   g047(.a(x24), .b(new_n107_), .O(new_n131_));
  oai21  g048(.a(x23), .b(x08), .c(new_n87_), .O(new_n132_));
  nor2   g049(.a(new_n132_), .b(new_n131_), .O(z12));
  nor2   g050(.a(x25), .b(new_n107_), .O(new_n134_));
  oai21  g051(.a(x24), .b(x08), .c(new_n87_), .O(new_n135_));
  nor2   g052(.a(new_n135_), .b(new_n134_), .O(z13));
  inv1   g053(.a(x26), .O(new_n137_));
  oai21  g054(.a(x25), .b(x08), .c(new_n87_), .O(new_n138_));
  aoi21  g055(.a(new_n137_), .b(x08), .c(new_n138_), .O(z14));
  inv1   g056(.a(x27), .O(new_n140_));
  nand2  g057(.a(new_n140_), .b(x08), .O(new_n141_));
  nand2  g058(.a(new_n137_), .b(new_n107_), .O(new_n142_));
  nand3  g059(.a(new_n142_), .b(new_n141_), .c(new_n87_), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(new_n84_), .O(z15));
  nor2   g061(.a(x28), .b(new_n107_), .O(new_n145_));
  oai21  g062(.a(x27), .b(x08), .c(new_n87_), .O(new_n146_));
  oai21  g063(.a(new_n146_), .b(new_n145_), .c(new_n84_), .O(z16));
  inv1   g064(.a(x29), .O(new_n148_));
  oai21  g065(.a(x28), .b(x08), .c(new_n87_), .O(new_n149_));
  aoi21  g066(.a(new_n148_), .b(x08), .c(new_n149_), .O(z17));
  nor2   g067(.a(x30), .b(new_n107_), .O(new_n151_));
  oai21  g068(.a(x29), .b(x08), .c(new_n87_), .O(new_n152_));
  oai21  g069(.a(new_n152_), .b(new_n151_), .c(new_n84_), .O(z18));
  inv1   g070(.a(x00), .O(new_n154_));
  oai21  g071(.a(x30), .b(x08), .c(new_n87_), .O(new_n155_));
  aoi21  g072(.a(x08), .b(new_n154_), .c(new_n155_), .O(z19));
  inv1   g073(.a(x09), .O(new_n157_));
  nor2   g074(.a(x32), .b(new_n157_), .O(new_n158_));
  oai21  g075(.a(x31), .b(x09), .c(new_n87_), .O(new_n159_));
  nor2   g076(.a(new_n159_), .b(new_n158_), .O(z20));
  inv1   g077(.a(x33), .O(new_n161_));
  oai21  g078(.a(x32), .b(x09), .c(new_n87_), .O(new_n162_));
  aoi21  g079(.a(new_n161_), .b(x09), .c(new_n162_), .O(z21));
  nor2   g080(.a(x34), .b(new_n157_), .O(new_n164_));
  oai21  g081(.a(x33), .b(x09), .c(new_n87_), .O(new_n165_));
  oai21  g082(.a(new_n165_), .b(new_n164_), .c(new_n84_), .O(z22));
  inv1   g083(.a(x35), .O(new_n167_));
  oai21  g084(.a(x34), .b(x09), .c(new_n87_), .O(new_n168_));
  aoi21  g085(.a(new_n167_), .b(x09), .c(new_n168_), .O(z23));
  nor2   g086(.a(x36), .b(new_n157_), .O(new_n170_));
  oai21  g087(.a(x35), .b(x09), .c(new_n87_), .O(new_n171_));
  oai21  g088(.a(new_n171_), .b(new_n170_), .c(new_n84_), .O(z24));
  nor2   g089(.a(x37), .b(new_n157_), .O(new_n173_));
  oai21  g090(.a(x36), .b(x09), .c(new_n87_), .O(new_n174_));
  nor2   g091(.a(new_n174_), .b(new_n173_), .O(z25));
  nor2   g092(.a(x38), .b(new_n157_), .O(new_n176_));
  oai21  g093(.a(x37), .b(x09), .c(new_n87_), .O(new_n177_));
  nor2   g094(.a(new_n177_), .b(new_n176_), .O(z26));
  inv1   g095(.a(x14), .O(new_n179_));
  inv1   g096(.a(x39), .O(new_n180_));
  oai21  g097(.a(new_n180_), .b(x10), .c(new_n179_), .O(new_n181_));
  aoi21  g098(.a(x14), .b(new_n154_), .c(new_n157_), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nor2   g100(.a(x10), .b(x09), .O(new_n184_));
  aoi22  g101(.a(new_n184_), .b(x38), .c(x14), .d(x10), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(new_n183_), .O(z27));
  inv1   g103(.a(x40), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n179_), .c(x09), .O(new_n188_));
  oai21  g105(.a(x14), .b(new_n157_), .c(new_n180_), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n188_), .c(new_n87_), .O(new_n190_));
  inv1   g107(.a(new_n190_), .O(z28));
  inv1   g108(.a(x41), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n179_), .c(x09), .O(new_n193_));
  oai21  g110(.a(x14), .b(new_n157_), .c(new_n187_), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n193_), .c(new_n87_), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(z29));
  nand2  g113(.a(new_n179_), .b(x09), .O(new_n197_));
  oai21  g114(.a(x42), .b(new_n157_), .c(new_n87_), .O(new_n198_));
  nor2   g115(.a(x41), .b(x10), .O(new_n199_));
  aoi22  g116(.a(new_n199_), .b(new_n197_), .c(new_n198_), .d(new_n179_), .O(z30));
  inv1   g117(.a(x43), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n179_), .c(x09), .O(new_n202_));
  inv1   g119(.a(x42), .O(new_n203_));
  oai21  g120(.a(x14), .b(new_n157_), .c(new_n203_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n202_), .c(new_n87_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z31));
  inv1   g123(.a(x44), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n179_), .c(x09), .O(new_n208_));
  oai21  g125(.a(x14), .b(new_n157_), .c(new_n201_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n208_), .c(new_n87_), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z32));
  oai21  g128(.a(x45), .b(new_n157_), .c(new_n87_), .O(new_n212_));
  nor2   g129(.a(x44), .b(x10), .O(new_n213_));
  aoi22  g130(.a(new_n213_), .b(new_n197_), .c(new_n212_), .d(new_n179_), .O(z33));
  inv1   g131(.a(x46), .O(new_n215_));
  nand3  g132(.a(new_n215_), .b(new_n179_), .c(x09), .O(new_n216_));
  inv1   g133(.a(x45), .O(new_n217_));
  oai21  g134(.a(x14), .b(new_n157_), .c(new_n217_), .O(new_n218_));
  nand3  g135(.a(new_n218_), .b(new_n216_), .c(new_n87_), .O(new_n219_));
  inv1   g136(.a(new_n219_), .O(z34));
  oai21  g137(.a(new_n157_), .b(x00), .c(new_n87_), .O(new_n221_));
  nor2   g138(.a(x46), .b(x10), .O(new_n222_));
  aoi22  g139(.a(new_n222_), .b(new_n197_), .c(new_n221_), .d(new_n179_), .O(z35));
endmodule


