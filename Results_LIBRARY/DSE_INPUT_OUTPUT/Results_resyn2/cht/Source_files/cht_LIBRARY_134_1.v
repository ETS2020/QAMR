// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n184_, new_n185_, new_n187_, new_n188_, new_n189_, new_n190_,
    new_n191_, new_n192_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n200_, new_n201_, new_n203_, new_n204_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n212_, new_n213_,
    new_n215_, new_n216_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  oai21  g009(.a(new_n92_), .b(new_n91_), .c(new_n85_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  nor2   g012(.a(new_n95_), .b(new_n94_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  oai21  g015(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  nor2   g018(.a(new_n101_), .b(new_n100_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  nor2   g021(.a(new_n104_), .b(new_n103_), .O(z05));
  inv1   g022(.a(x18), .O(new_n106_));
  oai21  g023(.a(x17), .b(x08), .c(new_n88_), .O(new_n107_));
  aoi21  g024(.a(new_n106_), .b(x08), .c(new_n107_), .O(z06));
  inv1   g025(.a(x08), .O(new_n109_));
  nor2   g026(.a(x19), .b(new_n109_), .O(new_n110_));
  oai21  g027(.a(x18), .b(x08), .c(new_n88_), .O(new_n111_));
  oai21  g028(.a(new_n111_), .b(new_n110_), .c(new_n85_), .O(z07));
  nor2   g029(.a(x20), .b(new_n109_), .O(new_n113_));
  oai21  g030(.a(x19), .b(x08), .c(new_n88_), .O(new_n114_));
  nor2   g031(.a(new_n114_), .b(new_n113_), .O(z08));
  inv1   g032(.a(x21), .O(new_n116_));
  oai21  g033(.a(x20), .b(x08), .c(new_n88_), .O(new_n117_));
  aoi21  g034(.a(new_n116_), .b(x08), .c(new_n117_), .O(z09));
  inv1   g035(.a(x22), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(x08), .O(new_n120_));
  nand2  g037(.a(new_n116_), .b(new_n109_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n120_), .c(new_n88_), .O(new_n122_));
  nand2  g039(.a(new_n122_), .b(new_n85_), .O(z10));
  inv1   g040(.a(x23), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  nand2  g042(.a(new_n119_), .b(new_n109_), .O(new_n126_));
  nand3  g043(.a(new_n126_), .b(new_n125_), .c(new_n88_), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(new_n85_), .O(z11));
  inv1   g045(.a(x24), .O(new_n129_));
  nand2  g046(.a(new_n129_), .b(x08), .O(new_n130_));
  nand2  g047(.a(new_n124_), .b(new_n109_), .O(new_n131_));
  nand3  g048(.a(new_n131_), .b(new_n130_), .c(new_n88_), .O(new_n132_));
  nand2  g049(.a(new_n132_), .b(new_n85_), .O(z12));
  inv1   g050(.a(x25), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(x08), .O(new_n135_));
  nand2  g052(.a(new_n129_), .b(new_n109_), .O(new_n136_));
  nand3  g053(.a(new_n136_), .b(new_n135_), .c(new_n88_), .O(new_n137_));
  nand2  g054(.a(new_n137_), .b(new_n85_), .O(z13));
  inv1   g055(.a(x26), .O(new_n139_));
  nand2  g056(.a(new_n139_), .b(x08), .O(new_n140_));
  nand2  g057(.a(new_n134_), .b(new_n109_), .O(new_n141_));
  nand3  g058(.a(new_n141_), .b(new_n140_), .c(new_n88_), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(new_n85_), .O(z14));
  nor2   g060(.a(x27), .b(new_n109_), .O(new_n144_));
  oai21  g061(.a(x26), .b(x08), .c(new_n88_), .O(new_n145_));
  oai21  g062(.a(new_n145_), .b(new_n144_), .c(new_n85_), .O(z15));
  inv1   g063(.a(x28), .O(new_n147_));
  oai21  g064(.a(x27), .b(x08), .c(new_n88_), .O(new_n148_));
  aoi21  g065(.a(new_n147_), .b(x08), .c(new_n148_), .O(z16));
  inv1   g066(.a(x29), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(x08), .O(new_n151_));
  nand2  g068(.a(new_n147_), .b(new_n109_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n151_), .c(new_n88_), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(new_n85_), .O(z17));
  inv1   g071(.a(x30), .O(new_n155_));
  nand2  g072(.a(new_n155_), .b(x08), .O(new_n156_));
  nand2  g073(.a(new_n150_), .b(new_n109_), .O(new_n157_));
  nand3  g074(.a(new_n157_), .b(new_n156_), .c(new_n88_), .O(new_n158_));
  nand2  g075(.a(new_n158_), .b(new_n85_), .O(z18));
  inv1   g076(.a(x00), .O(new_n160_));
  nand2  g077(.a(x08), .b(new_n160_), .O(new_n161_));
  nand2  g078(.a(new_n155_), .b(new_n109_), .O(new_n162_));
  nand3  g079(.a(new_n162_), .b(new_n161_), .c(new_n88_), .O(new_n163_));
  nand2  g080(.a(new_n163_), .b(new_n85_), .O(z19));
  inv1   g081(.a(x32), .O(new_n165_));
  nor2   g082(.a(x31), .b(x10), .O(new_n166_));
  nand2  g083(.a(new_n88_), .b(x09), .O(new_n167_));
  oai22  g084(.a(new_n167_), .b(new_n165_), .c(new_n166_), .d(x09), .O(z20));
  nor2   g085(.a(x33), .b(new_n84_), .O(new_n169_));
  oai21  g086(.a(x32), .b(x09), .c(new_n88_), .O(new_n170_));
  nor2   g087(.a(new_n170_), .b(new_n169_), .O(z21));
  inv1   g088(.a(x34), .O(new_n172_));
  nor2   g089(.a(x33), .b(x10), .O(new_n173_));
  oai22  g090(.a(new_n173_), .b(x09), .c(new_n167_), .d(new_n172_), .O(z22));
  nor2   g091(.a(x35), .b(new_n84_), .O(new_n175_));
  oai21  g092(.a(x34), .b(x09), .c(new_n88_), .O(new_n176_));
  nor2   g093(.a(new_n176_), .b(new_n175_), .O(z23));
  nor2   g094(.a(x36), .b(new_n84_), .O(new_n178_));
  oai21  g095(.a(x35), .b(x09), .c(new_n88_), .O(new_n179_));
  nor2   g096(.a(new_n179_), .b(new_n178_), .O(z24));
  inv1   g097(.a(x37), .O(new_n181_));
  nor2   g098(.a(x36), .b(x10), .O(new_n182_));
  oai22  g099(.a(new_n182_), .b(x09), .c(new_n167_), .d(new_n181_), .O(z25));
  inv1   g100(.a(x38), .O(new_n184_));
  nor2   g101(.a(x37), .b(x10), .O(new_n185_));
  oai22  g102(.a(new_n185_), .b(x09), .c(new_n167_), .d(new_n184_), .O(z26));
  oai21  g103(.a(x38), .b(x10), .c(new_n84_), .O(new_n187_));
  nand2  g104(.a(x14), .b(new_n160_), .O(new_n188_));
  inv1   g105(.a(x14), .O(new_n189_));
  inv1   g106(.a(x39), .O(new_n190_));
  nand2  g107(.a(new_n190_), .b(new_n189_), .O(new_n191_));
  nand4  g108(.a(new_n191_), .b(new_n188_), .c(new_n88_), .d(x09), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(new_n187_), .O(z27));
  inv1   g110(.a(x40), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n189_), .c(x09), .O(new_n195_));
  oai21  g112(.a(x14), .b(new_n84_), .c(new_n190_), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n195_), .c(new_n88_), .O(new_n197_));
  inv1   g114(.a(new_n197_), .O(z28));
  nand2  g115(.a(new_n189_), .b(x09), .O(new_n199_));
  oai21  g116(.a(x41), .b(x14), .c(new_n88_), .O(new_n200_));
  nor2   g117(.a(x40), .b(x10), .O(new_n201_));
  aoi22  g118(.a(new_n201_), .b(new_n199_), .c(new_n200_), .d(x09), .O(z29));
  oai21  g119(.a(x42), .b(x14), .c(new_n88_), .O(new_n203_));
  nor2   g120(.a(x41), .b(x10), .O(new_n204_));
  aoi22  g121(.a(new_n204_), .b(new_n199_), .c(new_n203_), .d(x09), .O(z30));
  inv1   g122(.a(x43), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n189_), .c(x09), .O(new_n207_));
  inv1   g124(.a(x42), .O(new_n208_));
  oai21  g125(.a(x14), .b(new_n84_), .c(new_n208_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n207_), .c(new_n88_), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z31));
  oai21  g128(.a(x44), .b(x14), .c(new_n88_), .O(new_n212_));
  nor2   g129(.a(x43), .b(x10), .O(new_n213_));
  aoi22  g130(.a(new_n213_), .b(new_n199_), .c(new_n212_), .d(x09), .O(z32));
  oai21  g131(.a(x45), .b(x14), .c(new_n88_), .O(new_n215_));
  nor2   g132(.a(x44), .b(x10), .O(new_n216_));
  aoi22  g133(.a(new_n216_), .b(new_n199_), .c(new_n215_), .d(x09), .O(z33));
  inv1   g134(.a(x46), .O(new_n218_));
  nand3  g135(.a(new_n218_), .b(new_n189_), .c(x09), .O(new_n219_));
  inv1   g136(.a(x45), .O(new_n220_));
  oai21  g137(.a(x14), .b(new_n84_), .c(new_n220_), .O(new_n221_));
  nand3  g138(.a(new_n221_), .b(new_n219_), .c(new_n88_), .O(new_n222_));
  inv1   g139(.a(new_n222_), .O(z34));
  nand3  g140(.a(new_n189_), .b(x09), .c(new_n160_), .O(new_n224_));
  oai21  g141(.a(x14), .b(new_n84_), .c(new_n218_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n224_), .c(new_n88_), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(z35));
endmodule


