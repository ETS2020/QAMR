// Benchmark "FAU" written by ABC on Sat Aug  8 23:36:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n161_, new_n162_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n175_,
    new_n176_, new_n178_, new_n179_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n201_, new_n203_, new_n204_, new_n206_, new_n207_, new_n208_,
    new_n209_, new_n210_, new_n211_, new_n213_, new_n214_, new_n215_,
    new_n216_, new_n217_, new_n219_, new_n220_, new_n221_, new_n223_,
    new_n224_, new_n226_, new_n227_, new_n229_, new_n230_, new_n231_,
    new_n232_, new_n233_, new_n235_, new_n236_, new_n237_, new_n238_,
    new_n240_, new_n241_, new_n243_, new_n244_, new_n245_, new_n246_;
  inv1   g000(.a(x04), .O(new_n84_));
  nand2  g001(.a(x07), .b(new_n84_), .O(new_n85_));
  nor2   g002(.a(x15), .b(x10), .O(new_n86_));
  inv1   g003(.a(x07), .O(new_n87_));
  inv1   g004(.a(x11), .O(new_n88_));
  nand2  g005(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g006(.a(new_n89_), .b(new_n86_), .c(new_n85_), .O(new_n90_));
  inv1   g007(.a(new_n90_), .O(z00));
  inv1   g008(.a(x05), .O(new_n92_));
  nand2  g009(.a(x07), .b(new_n92_), .O(new_n93_));
  inv1   g010(.a(x12), .O(new_n94_));
  nand2  g011(.a(new_n94_), .b(new_n87_), .O(new_n95_));
  nand3  g012(.a(new_n95_), .b(new_n93_), .c(new_n86_), .O(new_n96_));
  inv1   g013(.a(new_n96_), .O(z01));
  inv1   g014(.a(x06), .O(new_n98_));
  nand2  g015(.a(x07), .b(new_n98_), .O(new_n99_));
  inv1   g016(.a(x13), .O(new_n100_));
  nand2  g017(.a(new_n100_), .b(new_n87_), .O(new_n101_));
  nand3  g018(.a(new_n101_), .b(new_n99_), .c(new_n86_), .O(new_n102_));
  inv1   g019(.a(new_n102_), .O(z02));
  inv1   g020(.a(x01), .O(new_n104_));
  nand2  g021(.a(x07), .b(new_n104_), .O(new_n105_));
  inv1   g022(.a(x14), .O(new_n106_));
  nand2  g023(.a(new_n106_), .b(new_n87_), .O(new_n107_));
  nand3  g024(.a(new_n107_), .b(new_n105_), .c(new_n86_), .O(new_n108_));
  inv1   g025(.a(new_n108_), .O(z03));
  aoi21  g026(.a(x07), .b(x02), .c(x15), .O(new_n110_));
  nor2   g027(.a(new_n110_), .b(x10), .O(z04));
  inv1   g028(.a(x03), .O(new_n112_));
  nand2  g029(.a(x07), .b(new_n112_), .O(new_n113_));
  inv1   g030(.a(x16), .O(new_n114_));
  nand2  g031(.a(new_n114_), .b(new_n87_), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(new_n113_), .c(new_n86_), .O(new_n116_));
  inv1   g033(.a(new_n116_), .O(z05));
  inv1   g034(.a(x18), .O(new_n118_));
  nand2  g035(.a(new_n118_), .b(x08), .O(new_n119_));
  inv1   g036(.a(x08), .O(new_n120_));
  inv1   g037(.a(x17), .O(new_n121_));
  nand2  g038(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g039(.a(new_n122_), .b(new_n119_), .c(new_n86_), .O(new_n123_));
  inv1   g040(.a(new_n123_), .O(z06));
  inv1   g041(.a(x19), .O(new_n125_));
  nand2  g042(.a(new_n125_), .b(x08), .O(new_n126_));
  nand2  g043(.a(new_n118_), .b(new_n120_), .O(new_n127_));
  nand3  g044(.a(new_n127_), .b(new_n126_), .c(new_n86_), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z07));
  nand2  g046(.a(x20), .b(x08), .O(new_n130_));
  aoi21  g047(.a(x19), .b(new_n120_), .c(x15), .O(new_n131_));
  aoi21  g048(.a(new_n131_), .b(new_n130_), .c(x10), .O(z08));
  inv1   g049(.a(x21), .O(new_n133_));
  nand2  g050(.a(new_n133_), .b(x08), .O(new_n134_));
  inv1   g051(.a(x20), .O(new_n135_));
  nand2  g052(.a(new_n135_), .b(new_n120_), .O(new_n136_));
  nand3  g053(.a(new_n136_), .b(new_n134_), .c(new_n86_), .O(new_n137_));
  inv1   g054(.a(new_n137_), .O(z09));
  inv1   g055(.a(x22), .O(new_n139_));
  nand2  g056(.a(new_n139_), .b(x08), .O(new_n140_));
  nand2  g057(.a(new_n133_), .b(new_n120_), .O(new_n141_));
  nand3  g058(.a(new_n141_), .b(new_n140_), .c(new_n86_), .O(new_n142_));
  inv1   g059(.a(new_n142_), .O(z10));
  inv1   g060(.a(x23), .O(new_n144_));
  nand2  g061(.a(new_n144_), .b(x08), .O(new_n145_));
  nand2  g062(.a(new_n139_), .b(new_n120_), .O(new_n146_));
  nand3  g063(.a(new_n146_), .b(new_n145_), .c(new_n86_), .O(new_n147_));
  inv1   g064(.a(new_n147_), .O(z11));
  nand2  g065(.a(x24), .b(x08), .O(new_n149_));
  aoi21  g066(.a(x23), .b(new_n120_), .c(x15), .O(new_n150_));
  aoi21  g067(.a(new_n150_), .b(new_n149_), .c(x10), .O(z12));
  inv1   g068(.a(x25), .O(new_n152_));
  nand2  g069(.a(new_n152_), .b(x08), .O(new_n153_));
  inv1   g070(.a(x24), .O(new_n154_));
  nand2  g071(.a(new_n154_), .b(new_n120_), .O(new_n155_));
  nand3  g072(.a(new_n155_), .b(new_n153_), .c(new_n86_), .O(new_n156_));
  inv1   g073(.a(new_n156_), .O(z13));
  nand2  g074(.a(x26), .b(x08), .O(new_n158_));
  aoi21  g075(.a(x25), .b(new_n120_), .c(x15), .O(new_n159_));
  aoi21  g076(.a(new_n159_), .b(new_n158_), .c(x10), .O(z14));
  inv1   g077(.a(x27), .O(new_n161_));
  nand2  g078(.a(new_n161_), .b(x08), .O(new_n162_));
  inv1   g079(.a(x26), .O(new_n163_));
  nand2  g080(.a(new_n163_), .b(new_n120_), .O(new_n164_));
  nand3  g081(.a(new_n164_), .b(new_n162_), .c(new_n86_), .O(new_n165_));
  inv1   g082(.a(new_n165_), .O(z15));
  inv1   g083(.a(x28), .O(new_n167_));
  nand2  g084(.a(new_n167_), .b(x08), .O(new_n168_));
  nand2  g085(.a(new_n161_), .b(new_n120_), .O(new_n169_));
  nand3  g086(.a(new_n169_), .b(new_n168_), .c(new_n86_), .O(new_n170_));
  inv1   g087(.a(new_n170_), .O(z16));
  nand2  g088(.a(x29), .b(x08), .O(new_n172_));
  aoi21  g089(.a(x28), .b(new_n120_), .c(x15), .O(new_n173_));
  aoi21  g090(.a(new_n173_), .b(new_n172_), .c(x10), .O(z17));
  nand2  g091(.a(x30), .b(x08), .O(new_n175_));
  aoi21  g092(.a(x29), .b(new_n120_), .c(x15), .O(new_n176_));
  aoi21  g093(.a(new_n176_), .b(new_n175_), .c(x10), .O(z18));
  nand2  g094(.a(x08), .b(x00), .O(new_n178_));
  aoi21  g095(.a(x30), .b(new_n120_), .c(x15), .O(new_n179_));
  aoi21  g096(.a(new_n179_), .b(new_n178_), .c(x10), .O(z19));
  nand2  g097(.a(x32), .b(x09), .O(new_n181_));
  inv1   g098(.a(x09), .O(new_n182_));
  aoi21  g099(.a(x31), .b(new_n182_), .c(x15), .O(new_n183_));
  aoi21  g100(.a(new_n183_), .b(new_n181_), .c(x10), .O(z20));
  nand2  g101(.a(x33), .b(x09), .O(new_n185_));
  aoi21  g102(.a(x32), .b(new_n182_), .c(x15), .O(new_n186_));
  aoi21  g103(.a(new_n186_), .b(new_n185_), .c(x10), .O(z21));
  nand2  g104(.a(x34), .b(x09), .O(new_n188_));
  aoi21  g105(.a(x33), .b(new_n182_), .c(x15), .O(new_n189_));
  aoi21  g106(.a(new_n189_), .b(new_n188_), .c(x10), .O(z22));
  nand2  g107(.a(x35), .b(x09), .O(new_n191_));
  aoi21  g108(.a(x34), .b(new_n182_), .c(x15), .O(new_n192_));
  aoi21  g109(.a(new_n192_), .b(new_n191_), .c(x10), .O(z23));
  nand2  g110(.a(x36), .b(x09), .O(new_n194_));
  aoi21  g111(.a(x35), .b(new_n182_), .c(x15), .O(new_n195_));
  aoi21  g112(.a(new_n195_), .b(new_n194_), .c(x10), .O(z24));
  inv1   g113(.a(x37), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(x09), .O(new_n198_));
  inv1   g115(.a(x36), .O(new_n199_));
  nand2  g116(.a(new_n199_), .b(new_n182_), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n198_), .c(new_n86_), .O(new_n201_));
  inv1   g118(.a(new_n201_), .O(z25));
  nand2  g119(.a(x38), .b(x09), .O(new_n203_));
  aoi21  g120(.a(x37), .b(new_n182_), .c(x15), .O(new_n204_));
  aoi21  g121(.a(new_n204_), .b(new_n203_), .c(x10), .O(z26));
  inv1   g122(.a(x00), .O(new_n206_));
  nand2  g123(.a(x14), .b(new_n206_), .O(new_n207_));
  inv1   g124(.a(x39), .O(new_n208_));
  nand2  g125(.a(new_n208_), .b(new_n106_), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n207_), .c(x09), .O(new_n210_));
  aoi21  g127(.a(x38), .b(new_n182_), .c(x15), .O(new_n211_));
  aoi21  g128(.a(new_n211_), .b(new_n210_), .c(x10), .O(z27));
  inv1   g129(.a(x40), .O(new_n213_));
  nand3  g130(.a(new_n213_), .b(new_n106_), .c(x09), .O(new_n214_));
  nand2  g131(.a(new_n106_), .b(x09), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(new_n208_), .O(new_n216_));
  nand3  g133(.a(new_n216_), .b(new_n214_), .c(new_n86_), .O(new_n217_));
  inv1   g134(.a(new_n217_), .O(z28));
  nor2   g135(.a(x14), .b(new_n182_), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(x41), .O(new_n220_));
  aoi21  g137(.a(new_n215_), .b(x40), .c(x15), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(new_n220_), .c(x10), .O(z29));
  nand2  g139(.a(new_n219_), .b(x42), .O(new_n223_));
  aoi21  g140(.a(new_n215_), .b(x41), .c(x15), .O(new_n224_));
  aoi21  g141(.a(new_n224_), .b(new_n223_), .c(x10), .O(z30));
  nand2  g142(.a(new_n219_), .b(x43), .O(new_n226_));
  aoi21  g143(.a(new_n215_), .b(x42), .c(x15), .O(new_n227_));
  aoi21  g144(.a(new_n227_), .b(new_n226_), .c(x10), .O(z31));
  inv1   g145(.a(x44), .O(new_n229_));
  nand3  g146(.a(new_n229_), .b(new_n106_), .c(x09), .O(new_n230_));
  inv1   g147(.a(x43), .O(new_n231_));
  nand2  g148(.a(new_n215_), .b(new_n231_), .O(new_n232_));
  nand3  g149(.a(new_n232_), .b(new_n230_), .c(new_n86_), .O(new_n233_));
  inv1   g150(.a(new_n233_), .O(z32));
  inv1   g151(.a(x45), .O(new_n235_));
  nand3  g152(.a(new_n235_), .b(new_n106_), .c(x09), .O(new_n236_));
  nand2  g153(.a(new_n215_), .b(new_n229_), .O(new_n237_));
  nand3  g154(.a(new_n237_), .b(new_n236_), .c(new_n86_), .O(new_n238_));
  inv1   g155(.a(new_n238_), .O(z33));
  nand2  g156(.a(new_n219_), .b(x46), .O(new_n240_));
  aoi21  g157(.a(new_n215_), .b(x45), .c(x15), .O(new_n241_));
  aoi21  g158(.a(new_n241_), .b(new_n240_), .c(x10), .O(z34));
  nand3  g159(.a(new_n106_), .b(x09), .c(new_n206_), .O(new_n243_));
  inv1   g160(.a(x46), .O(new_n244_));
  nand2  g161(.a(new_n215_), .b(new_n244_), .O(new_n245_));
  nand3  g162(.a(new_n245_), .b(new_n243_), .c(new_n86_), .O(new_n246_));
  inv1   g163(.a(new_n246_), .O(z35));
endmodule


