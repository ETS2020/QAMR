// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n166_,
    new_n167_, new_n168_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n185_, new_n187_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n195_, new_n196_,
    new_n198_, new_n200_, new_n202_, new_n204_, new_n206_, new_n208_,
    new_n210_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  inv1   g002(.a(x14), .O(new_n86_));
  aoi21  g003(.a(x11), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  aoi21  g004(.a(new_n87_), .b(new_n84_), .c(x10), .O(z00));
  nand2  g005(.a(x07), .b(x05), .O(new_n89_));
  aoi21  g006(.a(x12), .b(new_n85_), .c(new_n86_), .O(new_n90_));
  aoi21  g007(.a(new_n90_), .b(new_n89_), .c(x10), .O(z01));
  nand2  g008(.a(x07), .b(x06), .O(new_n92_));
  aoi21  g009(.a(x13), .b(new_n85_), .c(new_n86_), .O(new_n93_));
  aoi21  g010(.a(new_n93_), .b(new_n92_), .c(x10), .O(z02));
  nor2   g011(.a(new_n85_), .b(x01), .O(new_n95_));
  aoi21  g012(.a(new_n95_), .b(x14), .c(x10), .O(z03));
  nand2  g013(.a(x07), .b(x02), .O(new_n97_));
  aoi21  g014(.a(x15), .b(new_n85_), .c(new_n86_), .O(new_n98_));
  aoi21  g015(.a(new_n98_), .b(new_n97_), .c(x10), .O(z04));
  inv1   g016(.a(x10), .O(new_n100_));
  inv1   g017(.a(x03), .O(new_n101_));
  nand2  g018(.a(x07), .b(new_n101_), .O(new_n102_));
  inv1   g019(.a(x16), .O(new_n103_));
  nand2  g020(.a(new_n103_), .b(new_n85_), .O(new_n104_));
  nand4  g021(.a(new_n104_), .b(new_n102_), .c(x14), .d(new_n100_), .O(new_n105_));
  inv1   g022(.a(new_n105_), .O(z05));
  nand2  g023(.a(x18), .b(x08), .O(new_n107_));
  inv1   g024(.a(x08), .O(new_n108_));
  aoi21  g025(.a(x17), .b(new_n108_), .c(new_n86_), .O(new_n109_));
  aoi21  g026(.a(new_n109_), .b(new_n107_), .c(x10), .O(z06));
  inv1   g027(.a(x19), .O(new_n111_));
  nand2  g028(.a(new_n111_), .b(x08), .O(new_n112_));
  inv1   g029(.a(x18), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(new_n108_), .O(new_n114_));
  nand4  g031(.a(new_n114_), .b(new_n112_), .c(x14), .d(new_n100_), .O(new_n115_));
  inv1   g032(.a(new_n115_), .O(z07));
  nand2  g033(.a(x20), .b(x08), .O(new_n117_));
  aoi21  g034(.a(x19), .b(new_n108_), .c(new_n86_), .O(new_n118_));
  aoi21  g035(.a(new_n118_), .b(new_n117_), .c(x10), .O(z08));
  inv1   g036(.a(x21), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(x08), .O(new_n121_));
  inv1   g038(.a(x20), .O(new_n122_));
  nand2  g039(.a(new_n122_), .b(new_n108_), .O(new_n123_));
  nand4  g040(.a(new_n123_), .b(new_n121_), .c(x14), .d(new_n100_), .O(new_n124_));
  inv1   g041(.a(new_n124_), .O(z09));
  nand2  g042(.a(x22), .b(x08), .O(new_n126_));
  aoi21  g043(.a(x21), .b(new_n108_), .c(new_n86_), .O(new_n127_));
  aoi21  g044(.a(new_n127_), .b(new_n126_), .c(x10), .O(z10));
  nand2  g045(.a(x23), .b(x08), .O(new_n129_));
  aoi21  g046(.a(x22), .b(new_n108_), .c(new_n86_), .O(new_n130_));
  aoi21  g047(.a(new_n130_), .b(new_n129_), .c(x10), .O(z11));
  nand2  g048(.a(x24), .b(x08), .O(new_n132_));
  aoi21  g049(.a(x23), .b(new_n108_), .c(new_n86_), .O(new_n133_));
  aoi21  g050(.a(new_n133_), .b(new_n132_), .c(x10), .O(z12));
  nand2  g051(.a(x25), .b(x08), .O(new_n135_));
  aoi21  g052(.a(x24), .b(new_n108_), .c(new_n86_), .O(new_n136_));
  aoi21  g053(.a(new_n136_), .b(new_n135_), .c(x10), .O(z13));
  nand2  g054(.a(x26), .b(x08), .O(new_n138_));
  aoi21  g055(.a(x25), .b(new_n108_), .c(new_n86_), .O(new_n139_));
  aoi21  g056(.a(new_n139_), .b(new_n138_), .c(x10), .O(z14));
  nand2  g057(.a(x27), .b(x08), .O(new_n141_));
  aoi21  g058(.a(x26), .b(new_n108_), .c(new_n86_), .O(new_n142_));
  aoi21  g059(.a(new_n142_), .b(new_n141_), .c(x10), .O(z15));
  nand2  g060(.a(x28), .b(x08), .O(new_n144_));
  aoi21  g061(.a(x27), .b(new_n108_), .c(new_n86_), .O(new_n145_));
  aoi21  g062(.a(new_n145_), .b(new_n144_), .c(x10), .O(z16));
  nand2  g063(.a(x29), .b(x08), .O(new_n147_));
  aoi21  g064(.a(x28), .b(new_n108_), .c(new_n86_), .O(new_n148_));
  aoi21  g065(.a(new_n148_), .b(new_n147_), .c(x10), .O(z17));
  nand2  g066(.a(x30), .b(x08), .O(new_n150_));
  aoi21  g067(.a(x29), .b(new_n108_), .c(new_n86_), .O(new_n151_));
  aoi21  g068(.a(new_n151_), .b(new_n150_), .c(x10), .O(z18));
  inv1   g069(.a(x00), .O(new_n153_));
  nand2  g070(.a(x08), .b(new_n153_), .O(new_n154_));
  inv1   g071(.a(x30), .O(new_n155_));
  nand2  g072(.a(new_n155_), .b(new_n108_), .O(new_n156_));
  nand4  g073(.a(new_n156_), .b(new_n154_), .c(x14), .d(new_n100_), .O(new_n157_));
  inv1   g074(.a(new_n157_), .O(z19));
  nand2  g075(.a(x32), .b(x09), .O(new_n159_));
  inv1   g076(.a(x09), .O(new_n160_));
  aoi21  g077(.a(x31), .b(new_n160_), .c(new_n86_), .O(new_n161_));
  aoi21  g078(.a(new_n161_), .b(new_n159_), .c(x10), .O(z20));
  nand2  g079(.a(x33), .b(x09), .O(new_n163_));
  aoi21  g080(.a(x32), .b(new_n160_), .c(new_n86_), .O(new_n164_));
  aoi21  g081(.a(new_n164_), .b(new_n163_), .c(x10), .O(z21));
  inv1   g082(.a(x34), .O(new_n166_));
  nand2  g083(.a(new_n166_), .b(x09), .O(new_n167_));
  inv1   g084(.a(x33), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(new_n160_), .O(new_n169_));
  nand4  g086(.a(new_n169_), .b(new_n167_), .c(x14), .d(new_n100_), .O(new_n170_));
  inv1   g087(.a(new_n170_), .O(z22));
  inv1   g088(.a(x35), .O(new_n172_));
  nand2  g089(.a(new_n172_), .b(x09), .O(new_n173_));
  nand2  g090(.a(new_n166_), .b(new_n160_), .O(new_n174_));
  nand4  g091(.a(new_n174_), .b(new_n173_), .c(x14), .d(new_n100_), .O(new_n175_));
  inv1   g092(.a(new_n175_), .O(z23));
  inv1   g093(.a(x36), .O(new_n177_));
  nand2  g094(.a(new_n177_), .b(x09), .O(new_n178_));
  nand2  g095(.a(new_n172_), .b(new_n160_), .O(new_n179_));
  nand4  g096(.a(new_n179_), .b(new_n178_), .c(x14), .d(new_n100_), .O(new_n180_));
  inv1   g097(.a(new_n180_), .O(z24));
  inv1   g098(.a(x37), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(x09), .O(new_n183_));
  nand2  g100(.a(new_n177_), .b(new_n160_), .O(new_n184_));
  nand4  g101(.a(new_n184_), .b(new_n183_), .c(x14), .d(new_n100_), .O(new_n185_));
  inv1   g102(.a(new_n185_), .O(z25));
  inv1   g103(.a(x38), .O(new_n187_));
  nand2  g104(.a(new_n187_), .b(x09), .O(new_n188_));
  nand2  g105(.a(new_n182_), .b(new_n160_), .O(new_n189_));
  nand4  g106(.a(new_n189_), .b(new_n188_), .c(x14), .d(new_n100_), .O(new_n190_));
  inv1   g107(.a(new_n190_), .O(z26));
  nand2  g108(.a(x09), .b(x00), .O(new_n192_));
  aoi21  g109(.a(x38), .b(new_n160_), .c(new_n86_), .O(new_n193_));
  aoi21  g110(.a(new_n193_), .b(new_n192_), .c(x10), .O(z27));
  nor2   g111(.a(new_n86_), .b(x10), .O(new_n195_));
  nand2  g112(.a(new_n195_), .b(x39), .O(new_n196_));
  inv1   g113(.a(new_n196_), .O(z28));
  nand2  g114(.a(new_n195_), .b(x40), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(z29));
  nand2  g116(.a(new_n195_), .b(x41), .O(new_n200_));
  inv1   g117(.a(new_n200_), .O(z30));
  nand2  g118(.a(new_n195_), .b(x42), .O(new_n202_));
  inv1   g119(.a(new_n202_), .O(z31));
  nand2  g120(.a(new_n195_), .b(x43), .O(new_n204_));
  inv1   g121(.a(new_n204_), .O(z32));
  inv1   g122(.a(x44), .O(new_n206_));
  aoi21  g123(.a(new_n206_), .b(x14), .c(x10), .O(z33));
  nand2  g124(.a(new_n195_), .b(x45), .O(new_n208_));
  inv1   g125(.a(new_n208_), .O(z34));
  nand2  g126(.a(new_n195_), .b(x46), .O(new_n210_));
  inv1   g127(.a(new_n210_), .O(z35));
endmodule


