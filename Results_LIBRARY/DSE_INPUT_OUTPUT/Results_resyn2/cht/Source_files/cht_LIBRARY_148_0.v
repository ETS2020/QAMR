// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n167_, new_n168_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n180_, new_n181_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n192_, new_n193_,
    new_n194_, new_n195_, new_n197_, new_n198_, new_n199_, new_n200_,
    new_n202_, new_n203_, new_n204_, new_n205_, new_n207_, new_n208_,
    new_n209_, new_n211_, new_n212_, new_n213_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n221_, new_n222_, new_n224_,
    new_n225_, new_n227_, new_n228_, new_n229_, new_n230_, new_n231_,
    new_n233_, new_n234_, new_n235_, new_n236_, new_n238_, new_n239_,
    new_n241_, new_n242_, new_n243_, new_n244_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  inv1   g002(.a(x39), .O(new_n86_));
  aoi21  g003(.a(x11), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  aoi21  g004(.a(new_n87_), .b(new_n84_), .c(x10), .O(z00));
  inv1   g005(.a(x05), .O(new_n89_));
  nand2  g006(.a(x07), .b(new_n89_), .O(new_n90_));
  nor2   g007(.a(new_n86_), .b(x10), .O(new_n91_));
  inv1   g008(.a(x12), .O(new_n92_));
  nand2  g009(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nand3  g010(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  inv1   g011(.a(new_n94_), .O(z01));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  aoi21  g013(.a(x13), .b(new_n85_), .c(new_n86_), .O(new_n97_));
  aoi21  g014(.a(new_n97_), .b(new_n96_), .c(x10), .O(z02));
  inv1   g015(.a(x01), .O(new_n99_));
  nand2  g016(.a(x07), .b(new_n99_), .O(new_n100_));
  inv1   g017(.a(x14), .O(new_n101_));
  nand2  g018(.a(new_n101_), .b(new_n85_), .O(new_n102_));
  nand3  g019(.a(new_n102_), .b(new_n100_), .c(new_n91_), .O(new_n103_));
  inv1   g020(.a(new_n103_), .O(z03));
  nand2  g021(.a(x07), .b(x02), .O(new_n105_));
  aoi21  g022(.a(x15), .b(new_n85_), .c(new_n86_), .O(new_n106_));
  aoi21  g023(.a(new_n106_), .b(new_n105_), .c(x10), .O(z04));
  inv1   g024(.a(x03), .O(new_n108_));
  nand2  g025(.a(x07), .b(new_n108_), .O(new_n109_));
  inv1   g026(.a(x16), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n85_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n109_), .c(new_n91_), .O(new_n112_));
  inv1   g029(.a(new_n112_), .O(z05));
  nand2  g030(.a(x18), .b(x08), .O(new_n114_));
  inv1   g031(.a(x08), .O(new_n115_));
  aoi21  g032(.a(x17), .b(new_n115_), .c(new_n86_), .O(new_n116_));
  aoi21  g033(.a(new_n116_), .b(new_n114_), .c(x10), .O(z06));
  inv1   g034(.a(x19), .O(new_n118_));
  nand2  g035(.a(new_n118_), .b(x08), .O(new_n119_));
  inv1   g036(.a(x18), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(new_n115_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n119_), .c(new_n91_), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z07));
  nand2  g040(.a(x20), .b(x08), .O(new_n124_));
  aoi21  g041(.a(x19), .b(new_n115_), .c(new_n86_), .O(new_n125_));
  aoi21  g042(.a(new_n125_), .b(new_n124_), .c(x10), .O(z08));
  inv1   g043(.a(x21), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(x08), .O(new_n128_));
  inv1   g045(.a(x20), .O(new_n129_));
  nand2  g046(.a(new_n129_), .b(new_n115_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n128_), .c(new_n91_), .O(new_n131_));
  inv1   g048(.a(new_n131_), .O(z09));
  nand2  g049(.a(x22), .b(x08), .O(new_n133_));
  aoi21  g050(.a(x21), .b(new_n115_), .c(new_n86_), .O(new_n134_));
  aoi21  g051(.a(new_n134_), .b(new_n133_), .c(x10), .O(z10));
  nand2  g052(.a(x23), .b(x08), .O(new_n136_));
  aoi21  g053(.a(x22), .b(new_n115_), .c(new_n86_), .O(new_n137_));
  aoi21  g054(.a(new_n137_), .b(new_n136_), .c(x10), .O(z11));
  inv1   g055(.a(x24), .O(new_n139_));
  nand2  g056(.a(new_n139_), .b(x08), .O(new_n140_));
  inv1   g057(.a(x23), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n115_), .O(new_n142_));
  nand3  g059(.a(new_n142_), .b(new_n140_), .c(new_n91_), .O(new_n143_));
  inv1   g060(.a(new_n143_), .O(z12));
  inv1   g061(.a(x25), .O(new_n145_));
  nand2  g062(.a(new_n145_), .b(x08), .O(new_n146_));
  nand2  g063(.a(new_n139_), .b(new_n115_), .O(new_n147_));
  nand3  g064(.a(new_n147_), .b(new_n146_), .c(new_n91_), .O(new_n148_));
  inv1   g065(.a(new_n148_), .O(z13));
  inv1   g066(.a(x26), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(x08), .O(new_n151_));
  nand2  g068(.a(new_n145_), .b(new_n115_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n151_), .c(new_n91_), .O(new_n153_));
  inv1   g070(.a(new_n153_), .O(z14));
  nand2  g071(.a(x27), .b(x08), .O(new_n155_));
  aoi21  g072(.a(x26), .b(new_n115_), .c(new_n86_), .O(new_n156_));
  aoi21  g073(.a(new_n156_), .b(new_n155_), .c(x10), .O(z15));
  nand2  g074(.a(x28), .b(x08), .O(new_n158_));
  aoi21  g075(.a(x27), .b(new_n115_), .c(new_n86_), .O(new_n159_));
  aoi21  g076(.a(new_n159_), .b(new_n158_), .c(x10), .O(z16));
  inv1   g077(.a(x29), .O(new_n161_));
  nand2  g078(.a(new_n161_), .b(x08), .O(new_n162_));
  inv1   g079(.a(x28), .O(new_n163_));
  nand2  g080(.a(new_n163_), .b(new_n115_), .O(new_n164_));
  nand3  g081(.a(new_n164_), .b(new_n162_), .c(new_n91_), .O(new_n165_));
  inv1   g082(.a(new_n165_), .O(z17));
  nand2  g083(.a(x30), .b(x08), .O(new_n167_));
  aoi21  g084(.a(x29), .b(new_n115_), .c(new_n86_), .O(new_n168_));
  aoi21  g085(.a(new_n168_), .b(new_n167_), .c(x10), .O(z18));
  inv1   g086(.a(x00), .O(new_n170_));
  nand2  g087(.a(x08), .b(new_n170_), .O(new_n171_));
  inv1   g088(.a(x30), .O(new_n172_));
  nand2  g089(.a(new_n172_), .b(new_n115_), .O(new_n173_));
  nand3  g090(.a(new_n173_), .b(new_n171_), .c(new_n91_), .O(new_n174_));
  inv1   g091(.a(new_n174_), .O(z19));
  nand2  g092(.a(x32), .b(x09), .O(new_n176_));
  inv1   g093(.a(x09), .O(new_n177_));
  aoi21  g094(.a(x31), .b(new_n177_), .c(new_n86_), .O(new_n178_));
  aoi21  g095(.a(new_n178_), .b(new_n176_), .c(x10), .O(z20));
  nand2  g096(.a(x33), .b(x09), .O(new_n180_));
  aoi21  g097(.a(x32), .b(new_n177_), .c(new_n86_), .O(new_n181_));
  aoi21  g098(.a(new_n181_), .b(new_n180_), .c(x10), .O(z21));
  nand2  g099(.a(x34), .b(x09), .O(new_n183_));
  aoi21  g100(.a(x33), .b(new_n177_), .c(new_n86_), .O(new_n184_));
  aoi21  g101(.a(new_n184_), .b(new_n183_), .c(x10), .O(z22));
  inv1   g102(.a(x35), .O(new_n186_));
  nand2  g103(.a(new_n186_), .b(x09), .O(new_n187_));
  inv1   g104(.a(x34), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n177_), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n187_), .c(new_n91_), .O(new_n190_));
  inv1   g107(.a(new_n190_), .O(z23));
  inv1   g108(.a(x36), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(x09), .O(new_n193_));
  nand2  g110(.a(new_n186_), .b(new_n177_), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n193_), .c(new_n91_), .O(new_n195_));
  inv1   g112(.a(new_n195_), .O(z24));
  inv1   g113(.a(x37), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(x09), .O(new_n198_));
  nand2  g115(.a(new_n192_), .b(new_n177_), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n198_), .c(new_n91_), .O(new_n200_));
  inv1   g117(.a(new_n200_), .O(z25));
  inv1   g118(.a(x38), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(x09), .O(new_n203_));
  nand2  g120(.a(new_n197_), .b(new_n177_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n203_), .c(new_n91_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z26));
  nand2  g123(.a(new_n202_), .b(new_n177_), .O(new_n207_));
  nand3  g124(.a(x14), .b(x09), .c(new_n170_), .O(new_n208_));
  nand2  g125(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  aoi21  g126(.a(new_n209_), .b(x39), .c(x10), .O(z27));
  inv1   g127(.a(x40), .O(new_n211_));
  inv1   g128(.a(new_n91_), .O(new_n212_));
  nor2   g129(.a(x14), .b(new_n177_), .O(new_n213_));
  aoi21  g130(.a(new_n213_), .b(new_n211_), .c(new_n212_), .O(z28));
  inv1   g131(.a(x41), .O(new_n215_));
  nand3  g132(.a(new_n215_), .b(new_n101_), .c(x09), .O(new_n216_));
  nand2  g133(.a(new_n101_), .b(x09), .O(new_n217_));
  nand2  g134(.a(new_n217_), .b(new_n211_), .O(new_n218_));
  nand3  g135(.a(new_n218_), .b(new_n216_), .c(new_n91_), .O(new_n219_));
  inv1   g136(.a(new_n219_), .O(z29));
  nand2  g137(.a(new_n213_), .b(x42), .O(new_n221_));
  aoi21  g138(.a(new_n217_), .b(x41), .c(new_n86_), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n221_), .c(x10), .O(z30));
  nand2  g140(.a(new_n213_), .b(x43), .O(new_n224_));
  aoi21  g141(.a(new_n217_), .b(x42), .c(new_n86_), .O(new_n225_));
  aoi21  g142(.a(new_n225_), .b(new_n224_), .c(x10), .O(z31));
  inv1   g143(.a(x44), .O(new_n227_));
  nand3  g144(.a(new_n227_), .b(new_n101_), .c(x09), .O(new_n228_));
  inv1   g145(.a(x43), .O(new_n229_));
  nand2  g146(.a(new_n217_), .b(new_n229_), .O(new_n230_));
  nand3  g147(.a(new_n230_), .b(new_n228_), .c(new_n91_), .O(new_n231_));
  inv1   g148(.a(new_n231_), .O(z32));
  inv1   g149(.a(x45), .O(new_n233_));
  nand3  g150(.a(new_n233_), .b(new_n101_), .c(x09), .O(new_n234_));
  nand2  g151(.a(new_n217_), .b(new_n227_), .O(new_n235_));
  nand3  g152(.a(new_n235_), .b(new_n234_), .c(new_n91_), .O(new_n236_));
  inv1   g153(.a(new_n236_), .O(z33));
  nand2  g154(.a(new_n213_), .b(x46), .O(new_n238_));
  aoi21  g155(.a(new_n217_), .b(x45), .c(new_n86_), .O(new_n239_));
  aoi21  g156(.a(new_n239_), .b(new_n238_), .c(x10), .O(z34));
  inv1   g157(.a(x46), .O(new_n241_));
  oai21  g158(.a(x14), .b(new_n177_), .c(new_n241_), .O(new_n242_));
  nand3  g159(.a(new_n101_), .b(x09), .c(new_n170_), .O(new_n243_));
  nand2  g160(.a(new_n243_), .b(new_n242_), .O(new_n244_));
  aoi21  g161(.a(new_n244_), .b(x39), .c(x10), .O(z35));
endmodule


