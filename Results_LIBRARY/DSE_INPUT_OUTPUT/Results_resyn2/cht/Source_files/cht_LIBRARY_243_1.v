// Benchmark "FAU" written by ABC on Sat Aug  8 23:36:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n178_, new_n179_,
    new_n180_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n188_, new_n189_, new_n190_, new_n191_, new_n193_, new_n194_,
    new_n195_, new_n196_, new_n198_, new_n199_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n208_, new_n209_,
    new_n210_, new_n211_, new_n213_, new_n214_, new_n215_;
  inv1   g000(.a(x09), .O(new_n84_));
  nand2  g001(.a(x10), .b(new_n84_), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x04), .O(new_n87_));
  inv1   g004(.a(x10), .O(new_n88_));
  oai21  g005(.a(x11), .b(x07), .c(new_n88_), .O(new_n89_));
  oai21  g006(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z00));
  nor2   g007(.a(new_n86_), .b(x05), .O(new_n91_));
  oai21  g008(.a(x12), .b(x07), .c(new_n88_), .O(new_n92_));
  nor2   g009(.a(new_n92_), .b(new_n91_), .O(z01));
  nor2   g010(.a(new_n86_), .b(x06), .O(new_n94_));
  oai21  g011(.a(x13), .b(x07), .c(new_n88_), .O(new_n95_));
  oai21  g012(.a(new_n95_), .b(new_n94_), .c(new_n85_), .O(z02));
  nor2   g013(.a(new_n86_), .b(x01), .O(new_n97_));
  oai21  g014(.a(x14), .b(x07), .c(new_n88_), .O(new_n98_));
  oai21  g015(.a(new_n98_), .b(new_n97_), .c(new_n85_), .O(z03));
  nor2   g016(.a(new_n86_), .b(x02), .O(new_n100_));
  oai21  g017(.a(x15), .b(x07), .c(new_n88_), .O(new_n101_));
  nor2   g018(.a(new_n101_), .b(new_n100_), .O(z04));
  nor2   g019(.a(new_n86_), .b(x03), .O(new_n103_));
  oai21  g020(.a(x16), .b(x07), .c(new_n88_), .O(new_n104_));
  nor2   g021(.a(new_n104_), .b(new_n103_), .O(z05));
  inv1   g022(.a(x08), .O(new_n106_));
  nor2   g023(.a(x18), .b(new_n106_), .O(new_n107_));
  oai21  g024(.a(x17), .b(x08), .c(new_n88_), .O(new_n108_));
  oai21  g025(.a(new_n108_), .b(new_n107_), .c(new_n85_), .O(z06));
  inv1   g026(.a(x19), .O(new_n110_));
  oai21  g027(.a(x18), .b(x08), .c(new_n88_), .O(new_n111_));
  aoi21  g028(.a(new_n110_), .b(x08), .c(new_n111_), .O(z07));
  inv1   g029(.a(x20), .O(new_n113_));
  nand2  g030(.a(new_n113_), .b(x08), .O(new_n114_));
  nand2  g031(.a(new_n110_), .b(new_n106_), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(new_n114_), .c(new_n88_), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n85_), .O(z08));
  nor2   g034(.a(x21), .b(new_n106_), .O(new_n118_));
  oai21  g035(.a(x20), .b(x08), .c(new_n88_), .O(new_n119_));
  oai21  g036(.a(new_n119_), .b(new_n118_), .c(new_n85_), .O(z09));
  inv1   g037(.a(x22), .O(new_n121_));
  oai21  g038(.a(x21), .b(x08), .c(new_n88_), .O(new_n122_));
  aoi21  g039(.a(new_n121_), .b(x08), .c(new_n122_), .O(z10));
  nor2   g040(.a(x23), .b(new_n106_), .O(new_n124_));
  oai21  g041(.a(x22), .b(x08), .c(new_n88_), .O(new_n125_));
  oai21  g042(.a(new_n125_), .b(new_n124_), .c(new_n85_), .O(z11));
  inv1   g043(.a(x24), .O(new_n127_));
  oai21  g044(.a(x23), .b(x08), .c(new_n88_), .O(new_n128_));
  aoi21  g045(.a(new_n127_), .b(x08), .c(new_n128_), .O(z12));
  nor2   g046(.a(x25), .b(new_n106_), .O(new_n130_));
  oai21  g047(.a(x24), .b(x08), .c(new_n88_), .O(new_n131_));
  oai21  g048(.a(new_n131_), .b(new_n130_), .c(new_n85_), .O(z13));
  nor2   g049(.a(x26), .b(new_n106_), .O(new_n133_));
  oai21  g050(.a(x25), .b(x08), .c(new_n88_), .O(new_n134_));
  nor2   g051(.a(new_n134_), .b(new_n133_), .O(z14));
  inv1   g052(.a(x27), .O(new_n136_));
  oai21  g053(.a(x26), .b(x08), .c(new_n88_), .O(new_n137_));
  aoi21  g054(.a(new_n136_), .b(x08), .c(new_n137_), .O(z15));
  nor2   g055(.a(x28), .b(new_n106_), .O(new_n139_));
  oai21  g056(.a(x27), .b(x08), .c(new_n88_), .O(new_n140_));
  oai21  g057(.a(new_n140_), .b(new_n139_), .c(new_n85_), .O(z16));
  nor2   g058(.a(x29), .b(new_n106_), .O(new_n142_));
  oai21  g059(.a(x28), .b(x08), .c(new_n88_), .O(new_n143_));
  nor2   g060(.a(new_n143_), .b(new_n142_), .O(z17));
  nor2   g061(.a(x30), .b(new_n106_), .O(new_n145_));
  oai21  g062(.a(x29), .b(x08), .c(new_n88_), .O(new_n146_));
  nor2   g063(.a(new_n146_), .b(new_n145_), .O(z18));
  inv1   g064(.a(x00), .O(new_n148_));
  oai21  g065(.a(x30), .b(x08), .c(new_n88_), .O(new_n149_));
  aoi21  g066(.a(x08), .b(new_n148_), .c(new_n149_), .O(z19));
  inv1   g067(.a(x32), .O(new_n151_));
  nor2   g068(.a(x31), .b(x10), .O(new_n152_));
  nand2  g069(.a(new_n88_), .b(x09), .O(new_n153_));
  oai22  g070(.a(new_n153_), .b(new_n151_), .c(new_n152_), .d(x09), .O(z20));
  inv1   g071(.a(x33), .O(new_n155_));
  nor2   g072(.a(x32), .b(x10), .O(new_n156_));
  oai22  g073(.a(new_n156_), .b(x09), .c(new_n153_), .d(new_n155_), .O(z21));
  inv1   g074(.a(x34), .O(new_n158_));
  nor2   g075(.a(x33), .b(x10), .O(new_n159_));
  oai22  g076(.a(new_n159_), .b(x09), .c(new_n153_), .d(new_n158_), .O(z22));
  nor2   g077(.a(x35), .b(new_n84_), .O(new_n161_));
  oai21  g078(.a(x34), .b(x09), .c(new_n88_), .O(new_n162_));
  nor2   g079(.a(new_n162_), .b(new_n161_), .O(z23));
  inv1   g080(.a(x36), .O(new_n164_));
  nor2   g081(.a(x35), .b(x10), .O(new_n165_));
  oai22  g082(.a(new_n165_), .b(x09), .c(new_n153_), .d(new_n164_), .O(z24));
  inv1   g083(.a(x37), .O(new_n167_));
  nor2   g084(.a(x36), .b(x10), .O(new_n168_));
  oai22  g085(.a(new_n168_), .b(x09), .c(new_n153_), .d(new_n167_), .O(z25));
  inv1   g086(.a(x38), .O(new_n170_));
  nor2   g087(.a(x37), .b(x10), .O(new_n171_));
  oai22  g088(.a(new_n171_), .b(x09), .c(new_n153_), .d(new_n170_), .O(z26));
  nand2  g089(.a(x14), .b(x00), .O(new_n173_));
  inv1   g090(.a(x14), .O(new_n174_));
  aoi21  g091(.a(x39), .b(new_n174_), .c(new_n84_), .O(new_n175_));
  oai21  g092(.a(x38), .b(x09), .c(new_n88_), .O(new_n176_));
  aoi21  g093(.a(new_n175_), .b(new_n173_), .c(new_n176_), .O(z27));
  oai21  g094(.a(x40), .b(x14), .c(new_n88_), .O(new_n178_));
  nand2  g095(.a(new_n174_), .b(x09), .O(new_n179_));
  nor2   g096(.a(x39), .b(x10), .O(new_n180_));
  aoi22  g097(.a(new_n180_), .b(new_n179_), .c(new_n178_), .d(x09), .O(z28));
  inv1   g098(.a(x41), .O(new_n182_));
  nand3  g099(.a(new_n182_), .b(new_n174_), .c(x09), .O(new_n183_));
  inv1   g100(.a(x40), .O(new_n184_));
  oai21  g101(.a(x14), .b(new_n84_), .c(new_n184_), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n183_), .c(new_n88_), .O(new_n186_));
  inv1   g103(.a(new_n186_), .O(z29));
  inv1   g104(.a(x42), .O(new_n188_));
  nand3  g105(.a(new_n188_), .b(new_n174_), .c(x09), .O(new_n189_));
  oai21  g106(.a(x14), .b(new_n84_), .c(new_n182_), .O(new_n190_));
  nand3  g107(.a(new_n190_), .b(new_n189_), .c(new_n88_), .O(new_n191_));
  inv1   g108(.a(new_n191_), .O(z30));
  inv1   g109(.a(x43), .O(new_n193_));
  nand3  g110(.a(new_n193_), .b(new_n174_), .c(x09), .O(new_n194_));
  oai21  g111(.a(x14), .b(new_n84_), .c(new_n188_), .O(new_n195_));
  nand3  g112(.a(new_n195_), .b(new_n194_), .c(new_n88_), .O(new_n196_));
  inv1   g113(.a(new_n196_), .O(z31));
  inv1   g114(.a(x44), .O(new_n198_));
  nand3  g115(.a(new_n198_), .b(new_n174_), .c(x09), .O(new_n199_));
  oai21  g116(.a(x14), .b(new_n84_), .c(new_n193_), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n199_), .c(new_n88_), .O(new_n201_));
  inv1   g118(.a(new_n201_), .O(z32));
  inv1   g119(.a(x45), .O(new_n203_));
  nand3  g120(.a(new_n203_), .b(new_n174_), .c(x09), .O(new_n204_));
  oai21  g121(.a(x14), .b(new_n84_), .c(new_n198_), .O(new_n205_));
  nand3  g122(.a(new_n205_), .b(new_n204_), .c(new_n88_), .O(new_n206_));
  inv1   g123(.a(new_n206_), .O(z33));
  inv1   g124(.a(x46), .O(new_n208_));
  nand3  g125(.a(new_n208_), .b(new_n174_), .c(x09), .O(new_n209_));
  oai21  g126(.a(x14), .b(new_n84_), .c(new_n203_), .O(new_n210_));
  nand3  g127(.a(new_n210_), .b(new_n209_), .c(new_n88_), .O(new_n211_));
  inv1   g128(.a(new_n211_), .O(z34));
  nand3  g129(.a(new_n174_), .b(x09), .c(new_n148_), .O(new_n213_));
  oai21  g130(.a(x14), .b(new_n84_), .c(new_n208_), .O(new_n214_));
  nand3  g131(.a(new_n214_), .b(new_n213_), .c(new_n88_), .O(new_n215_));
  inv1   g132(.a(new_n215_), .O(z35));
endmodule


