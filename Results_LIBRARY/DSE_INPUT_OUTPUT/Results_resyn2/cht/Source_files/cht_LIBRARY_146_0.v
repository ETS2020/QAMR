// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n86_, new_n88_, new_n90_, new_n92_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_, new_n188_, new_n189_,
    new_n190_, new_n191_, new_n192_, new_n194_, new_n195_, new_n196_,
    new_n197_, new_n198_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n210_, new_n211_,
    new_n212_, new_n214_, new_n215_, new_n217_, new_n218_, new_n220_,
    new_n221_, new_n223_, new_n224_;
  inv1   g000(.a(x04), .O(new_n84_));
  aoi21  g001(.a(x07), .b(new_n84_), .c(x10), .O(z00));
  nand2  g002(.a(x07), .b(x05), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x10), .O(z01));
  inv1   g004(.a(x06), .O(new_n88_));
  aoi21  g005(.a(x07), .b(new_n88_), .c(x10), .O(z02));
  nand2  g006(.a(x07), .b(x01), .O(new_n90_));
  nor2   g007(.a(new_n90_), .b(x10), .O(z03));
  nand2  g008(.a(x07), .b(x02), .O(new_n92_));
  nor2   g009(.a(new_n92_), .b(x10), .O(z04));
  nand2  g010(.a(x07), .b(x03), .O(new_n94_));
  nor2   g011(.a(new_n94_), .b(x10), .O(z05));
  inv1   g012(.a(x18), .O(new_n96_));
  nand2  g013(.a(new_n96_), .b(x08), .O(new_n97_));
  inv1   g014(.a(x07), .O(new_n98_));
  nor2   g015(.a(x10), .b(new_n98_), .O(new_n99_));
  inv1   g016(.a(x08), .O(new_n100_));
  inv1   g017(.a(x17), .O(new_n101_));
  nand2  g018(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g019(.a(new_n102_), .b(new_n99_), .c(new_n97_), .O(new_n103_));
  inv1   g020(.a(new_n103_), .O(z06));
  inv1   g021(.a(x19), .O(new_n105_));
  nand2  g022(.a(new_n105_), .b(x08), .O(new_n106_));
  nand2  g023(.a(new_n96_), .b(new_n100_), .O(new_n107_));
  nand3  g024(.a(new_n107_), .b(new_n106_), .c(new_n99_), .O(new_n108_));
  inv1   g025(.a(new_n108_), .O(z07));
  inv1   g026(.a(x20), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(x08), .O(new_n111_));
  nand2  g028(.a(new_n105_), .b(new_n100_), .O(new_n112_));
  nand3  g029(.a(new_n112_), .b(new_n111_), .c(new_n99_), .O(new_n113_));
  inv1   g030(.a(new_n113_), .O(z08));
  nand2  g031(.a(x21), .b(x08), .O(new_n115_));
  aoi21  g032(.a(x20), .b(new_n100_), .c(new_n98_), .O(new_n116_));
  aoi21  g033(.a(new_n116_), .b(new_n115_), .c(x10), .O(z09));
  nand2  g034(.a(x22), .b(x08), .O(new_n118_));
  aoi21  g035(.a(x21), .b(new_n100_), .c(new_n98_), .O(new_n119_));
  aoi21  g036(.a(new_n119_), .b(new_n118_), .c(x10), .O(z10));
  nand2  g037(.a(x23), .b(x08), .O(new_n121_));
  aoi21  g038(.a(x22), .b(new_n100_), .c(new_n98_), .O(new_n122_));
  aoi21  g039(.a(new_n122_), .b(new_n121_), .c(x10), .O(z11));
  inv1   g040(.a(x24), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  inv1   g042(.a(x23), .O(new_n126_));
  nand2  g043(.a(new_n126_), .b(new_n100_), .O(new_n127_));
  nand3  g044(.a(new_n127_), .b(new_n125_), .c(new_n99_), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z12));
  nand2  g046(.a(x25), .b(x08), .O(new_n130_));
  aoi21  g047(.a(x24), .b(new_n100_), .c(new_n98_), .O(new_n131_));
  aoi21  g048(.a(new_n131_), .b(new_n130_), .c(x10), .O(z13));
  nand2  g049(.a(x26), .b(x08), .O(new_n133_));
  aoi21  g050(.a(x25), .b(new_n100_), .c(new_n98_), .O(new_n134_));
  aoi21  g051(.a(new_n134_), .b(new_n133_), .c(x10), .O(z14));
  nand2  g052(.a(x27), .b(x08), .O(new_n136_));
  aoi21  g053(.a(x26), .b(new_n100_), .c(new_n98_), .O(new_n137_));
  aoi21  g054(.a(new_n137_), .b(new_n136_), .c(x10), .O(z15));
  inv1   g055(.a(x28), .O(new_n139_));
  nand2  g056(.a(new_n139_), .b(x08), .O(new_n140_));
  inv1   g057(.a(x27), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n100_), .O(new_n142_));
  nand3  g059(.a(new_n142_), .b(new_n140_), .c(new_n99_), .O(new_n143_));
  inv1   g060(.a(new_n143_), .O(z16));
  nand2  g061(.a(x29), .b(x08), .O(new_n145_));
  aoi21  g062(.a(x28), .b(new_n100_), .c(new_n98_), .O(new_n146_));
  aoi21  g063(.a(new_n146_), .b(new_n145_), .c(x10), .O(z17));
  inv1   g064(.a(x30), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(x08), .O(new_n149_));
  inv1   g066(.a(x29), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(new_n100_), .O(new_n151_));
  nand3  g068(.a(new_n151_), .b(new_n149_), .c(new_n99_), .O(new_n152_));
  inv1   g069(.a(new_n152_), .O(z18));
  inv1   g070(.a(x00), .O(new_n154_));
  nand2  g071(.a(x08), .b(new_n154_), .O(new_n155_));
  nand2  g072(.a(new_n148_), .b(new_n100_), .O(new_n156_));
  nand3  g073(.a(new_n156_), .b(new_n155_), .c(new_n99_), .O(new_n157_));
  inv1   g074(.a(new_n157_), .O(z19));
  inv1   g075(.a(x32), .O(new_n159_));
  nand2  g076(.a(new_n159_), .b(x09), .O(new_n160_));
  inv1   g077(.a(x09), .O(new_n161_));
  inv1   g078(.a(x31), .O(new_n162_));
  nand2  g079(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nand3  g080(.a(new_n163_), .b(new_n160_), .c(new_n99_), .O(new_n164_));
  inv1   g081(.a(new_n164_), .O(z20));
  nand2  g082(.a(x33), .b(x09), .O(new_n166_));
  aoi21  g083(.a(x32), .b(new_n161_), .c(new_n98_), .O(new_n167_));
  aoi21  g084(.a(new_n167_), .b(new_n166_), .c(x10), .O(z21));
  nand2  g085(.a(x34), .b(x09), .O(new_n169_));
  aoi21  g086(.a(x33), .b(new_n161_), .c(new_n98_), .O(new_n170_));
  aoi21  g087(.a(new_n170_), .b(new_n169_), .c(x10), .O(z22));
  nand2  g088(.a(x35), .b(x09), .O(new_n172_));
  aoi21  g089(.a(x34), .b(new_n161_), .c(new_n98_), .O(new_n173_));
  aoi21  g090(.a(new_n173_), .b(new_n172_), .c(x10), .O(z23));
  nand2  g091(.a(x36), .b(x09), .O(new_n175_));
  aoi21  g092(.a(x35), .b(new_n161_), .c(new_n98_), .O(new_n176_));
  aoi21  g093(.a(new_n176_), .b(new_n175_), .c(x10), .O(z24));
  nand2  g094(.a(x37), .b(x09), .O(new_n178_));
  aoi21  g095(.a(x36), .b(new_n161_), .c(new_n98_), .O(new_n179_));
  aoi21  g096(.a(new_n179_), .b(new_n178_), .c(x10), .O(z25));
  inv1   g097(.a(x38), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(x09), .O(new_n182_));
  inv1   g099(.a(x37), .O(new_n183_));
  nand2  g100(.a(new_n183_), .b(new_n161_), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n182_), .c(new_n99_), .O(new_n185_));
  inv1   g102(.a(new_n185_), .O(z26));
  inv1   g103(.a(x14), .O(new_n187_));
  inv1   g104(.a(x39), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  oai21  g106(.a(new_n187_), .b(x00), .c(new_n189_), .O(new_n190_));
  nand2  g107(.a(new_n181_), .b(new_n161_), .O(new_n191_));
  nand2  g108(.a(new_n191_), .b(new_n99_), .O(new_n192_));
  aoi21  g109(.a(new_n190_), .b(x09), .c(new_n192_), .O(z27));
  inv1   g110(.a(x40), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n187_), .c(x09), .O(new_n195_));
  nand2  g112(.a(new_n187_), .b(x09), .O(new_n196_));
  nand2  g113(.a(new_n196_), .b(new_n188_), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n195_), .c(new_n99_), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(z28));
  inv1   g116(.a(x41), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n187_), .c(x09), .O(new_n201_));
  nand2  g118(.a(new_n196_), .b(new_n194_), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n201_), .c(new_n99_), .O(new_n203_));
  inv1   g120(.a(new_n203_), .O(z29));
  inv1   g121(.a(x42), .O(new_n205_));
  nand3  g122(.a(new_n205_), .b(new_n187_), .c(x09), .O(new_n206_));
  nand2  g123(.a(new_n196_), .b(new_n200_), .O(new_n207_));
  nand3  g124(.a(new_n207_), .b(new_n206_), .c(new_n99_), .O(new_n208_));
  inv1   g125(.a(new_n208_), .O(z30));
  nor2   g126(.a(x14), .b(new_n161_), .O(new_n210_));
  nand2  g127(.a(new_n210_), .b(x43), .O(new_n211_));
  aoi21  g128(.a(new_n196_), .b(x42), .c(new_n98_), .O(new_n212_));
  aoi21  g129(.a(new_n212_), .b(new_n211_), .c(x10), .O(z31));
  nand2  g130(.a(new_n210_), .b(x44), .O(new_n214_));
  aoi21  g131(.a(new_n196_), .b(x43), .c(new_n98_), .O(new_n215_));
  aoi21  g132(.a(new_n215_), .b(new_n214_), .c(x10), .O(z32));
  nand2  g133(.a(new_n210_), .b(x45), .O(new_n217_));
  aoi21  g134(.a(new_n196_), .b(x44), .c(new_n98_), .O(new_n218_));
  aoi21  g135(.a(new_n218_), .b(new_n217_), .c(x10), .O(z33));
  nand2  g136(.a(new_n210_), .b(x46), .O(new_n220_));
  aoi21  g137(.a(new_n196_), .b(x45), .c(new_n98_), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(new_n220_), .c(x10), .O(z34));
  nand2  g139(.a(new_n210_), .b(x00), .O(new_n223_));
  aoi21  g140(.a(new_n196_), .b(x46), .c(new_n98_), .O(new_n224_));
  aoi21  g141(.a(new_n224_), .b(new_n223_), .c(x10), .O(z35));
endmodule


