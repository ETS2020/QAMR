// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n165_, new_n166_, new_n167_, new_n168_, new_n169_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n178_,
    new_n179_, new_n181_, new_n183_, new_n185_, new_n187_, new_n189_,
    new_n191_, new_n192_, new_n193_, new_n194_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n206_, new_n207_, new_n209_, new_n210_, new_n212_, new_n213_,
    new_n214_, new_n215_, new_n216_, new_n218_, new_n219_, new_n221_,
    new_n222_, new_n224_, new_n225_, new_n226_, new_n227_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  inv1   g001(.a(x07), .O(new_n85_));
  inv1   g002(.a(x09), .O(new_n86_));
  aoi21  g003(.a(x11), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  aoi21  g004(.a(new_n87_), .b(new_n84_), .c(x10), .O(z00));
  inv1   g005(.a(x10), .O(new_n89_));
  inv1   g006(.a(x05), .O(new_n90_));
  nand2  g007(.a(x07), .b(new_n90_), .O(new_n91_));
  inv1   g008(.a(x12), .O(new_n92_));
  nand2  g009(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nand4  g010(.a(new_n93_), .b(new_n91_), .c(new_n89_), .d(x09), .O(new_n94_));
  inv1   g011(.a(new_n94_), .O(z01));
  inv1   g012(.a(x06), .O(new_n96_));
  nand2  g013(.a(x07), .b(new_n96_), .O(new_n97_));
  inv1   g014(.a(x13), .O(new_n98_));
  nand2  g015(.a(new_n98_), .b(new_n85_), .O(new_n99_));
  nand4  g016(.a(new_n99_), .b(new_n97_), .c(new_n89_), .d(x09), .O(new_n100_));
  inv1   g017(.a(new_n100_), .O(z02));
  inv1   g018(.a(x01), .O(new_n102_));
  nand2  g019(.a(x07), .b(new_n102_), .O(new_n103_));
  inv1   g020(.a(x14), .O(new_n104_));
  nand2  g021(.a(new_n104_), .b(new_n85_), .O(new_n105_));
  nand4  g022(.a(new_n105_), .b(new_n103_), .c(new_n89_), .d(x09), .O(new_n106_));
  inv1   g023(.a(new_n106_), .O(z03));
  inv1   g024(.a(x02), .O(new_n108_));
  nand2  g025(.a(x07), .b(new_n108_), .O(new_n109_));
  inv1   g026(.a(x15), .O(new_n110_));
  nand2  g027(.a(new_n110_), .b(new_n85_), .O(new_n111_));
  nand4  g028(.a(new_n111_), .b(new_n109_), .c(new_n89_), .d(x09), .O(new_n112_));
  inv1   g029(.a(new_n112_), .O(z04));
  nand2  g030(.a(x07), .b(x03), .O(new_n114_));
  aoi21  g031(.a(x16), .b(new_n85_), .c(new_n86_), .O(new_n115_));
  aoi21  g032(.a(new_n115_), .b(new_n114_), .c(x10), .O(z05));
  inv1   g033(.a(x18), .O(new_n117_));
  nand2  g034(.a(new_n117_), .b(x08), .O(new_n118_));
  inv1   g035(.a(x08), .O(new_n119_));
  inv1   g036(.a(x17), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g038(.a(new_n121_), .b(new_n118_), .c(new_n89_), .d(x09), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z06));
  inv1   g040(.a(x19), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(x08), .O(new_n125_));
  nand2  g042(.a(new_n117_), .b(new_n119_), .O(new_n126_));
  nand4  g043(.a(new_n126_), .b(new_n125_), .c(new_n89_), .d(x09), .O(new_n127_));
  inv1   g044(.a(new_n127_), .O(z07));
  nand2  g045(.a(x20), .b(x08), .O(new_n129_));
  aoi21  g046(.a(x19), .b(new_n119_), .c(new_n86_), .O(new_n130_));
  aoi21  g047(.a(new_n130_), .b(new_n129_), .c(x10), .O(z08));
  nand2  g048(.a(x21), .b(x08), .O(new_n132_));
  aoi21  g049(.a(x20), .b(new_n119_), .c(new_n86_), .O(new_n133_));
  aoi21  g050(.a(new_n133_), .b(new_n132_), .c(x10), .O(z09));
  nand2  g051(.a(x22), .b(x08), .O(new_n135_));
  aoi21  g052(.a(x21), .b(new_n119_), .c(new_n86_), .O(new_n136_));
  aoi21  g053(.a(new_n136_), .b(new_n135_), .c(x10), .O(z10));
  nand2  g054(.a(x23), .b(x08), .O(new_n138_));
  aoi21  g055(.a(x22), .b(new_n119_), .c(new_n86_), .O(new_n139_));
  aoi21  g056(.a(new_n139_), .b(new_n138_), .c(x10), .O(z11));
  inv1   g057(.a(x24), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(x08), .O(new_n142_));
  inv1   g059(.a(x23), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(new_n119_), .O(new_n144_));
  nand4  g061(.a(new_n144_), .b(new_n142_), .c(new_n89_), .d(x09), .O(new_n145_));
  inv1   g062(.a(new_n145_), .O(z12));
  nand2  g063(.a(x25), .b(x08), .O(new_n147_));
  aoi21  g064(.a(x24), .b(new_n119_), .c(new_n86_), .O(new_n148_));
  aoi21  g065(.a(new_n148_), .b(new_n147_), .c(x10), .O(z13));
  nand2  g066(.a(x26), .b(x08), .O(new_n150_));
  aoi21  g067(.a(x25), .b(new_n119_), .c(new_n86_), .O(new_n151_));
  aoi21  g068(.a(new_n151_), .b(new_n150_), .c(x10), .O(z14));
  nand2  g069(.a(x27), .b(x08), .O(new_n153_));
  aoi21  g070(.a(x26), .b(new_n119_), .c(new_n86_), .O(new_n154_));
  aoi21  g071(.a(new_n154_), .b(new_n153_), .c(x10), .O(z15));
  inv1   g072(.a(x28), .O(new_n156_));
  nand2  g073(.a(new_n156_), .b(x08), .O(new_n157_));
  inv1   g074(.a(x27), .O(new_n158_));
  nand2  g075(.a(new_n158_), .b(new_n119_), .O(new_n159_));
  nand4  g076(.a(new_n159_), .b(new_n157_), .c(new_n89_), .d(x09), .O(new_n160_));
  inv1   g077(.a(new_n160_), .O(z16));
  nand2  g078(.a(x29), .b(x08), .O(new_n162_));
  aoi21  g079(.a(x28), .b(new_n119_), .c(new_n86_), .O(new_n163_));
  aoi21  g080(.a(new_n163_), .b(new_n162_), .c(x10), .O(z17));
  inv1   g081(.a(x30), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(x08), .O(new_n166_));
  inv1   g083(.a(x29), .O(new_n167_));
  nand2  g084(.a(new_n167_), .b(new_n119_), .O(new_n168_));
  nand4  g085(.a(new_n168_), .b(new_n166_), .c(new_n89_), .d(x09), .O(new_n169_));
  inv1   g086(.a(new_n169_), .O(z18));
  inv1   g087(.a(x00), .O(new_n171_));
  nand2  g088(.a(x08), .b(new_n171_), .O(new_n172_));
  nand2  g089(.a(new_n165_), .b(new_n119_), .O(new_n173_));
  nand4  g090(.a(new_n173_), .b(new_n172_), .c(new_n89_), .d(x09), .O(new_n174_));
  inv1   g091(.a(new_n174_), .O(z19));
  inv1   g092(.a(x32), .O(new_n176_));
  aoi21  g093(.a(new_n176_), .b(x09), .c(x10), .O(z20));
  nor2   g094(.a(x10), .b(new_n86_), .O(new_n178_));
  nand2  g095(.a(new_n178_), .b(x33), .O(new_n179_));
  inv1   g096(.a(new_n179_), .O(z21));
  inv1   g097(.a(x34), .O(new_n181_));
  aoi21  g098(.a(new_n181_), .b(x09), .c(x10), .O(z22));
  inv1   g099(.a(x35), .O(new_n183_));
  aoi21  g100(.a(new_n183_), .b(x09), .c(x10), .O(z23));
  nand2  g101(.a(new_n178_), .b(x36), .O(new_n185_));
  inv1   g102(.a(new_n185_), .O(z24));
  nand2  g103(.a(new_n178_), .b(x37), .O(new_n187_));
  inv1   g104(.a(new_n187_), .O(z25));
  nand2  g105(.a(new_n178_), .b(x38), .O(new_n189_));
  inv1   g106(.a(new_n189_), .O(z26));
  nand2  g107(.a(x14), .b(new_n171_), .O(new_n191_));
  inv1   g108(.a(x39), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(new_n104_), .O(new_n193_));
  nand4  g110(.a(new_n193_), .b(new_n191_), .c(new_n89_), .d(x09), .O(new_n194_));
  inv1   g111(.a(new_n194_), .O(z27));
  nand2  g112(.a(new_n192_), .b(x14), .O(new_n196_));
  inv1   g113(.a(x40), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(new_n104_), .O(new_n198_));
  nand4  g115(.a(new_n198_), .b(new_n196_), .c(new_n89_), .d(x09), .O(new_n199_));
  inv1   g116(.a(new_n199_), .O(z28));
  nand2  g117(.a(new_n197_), .b(x14), .O(new_n201_));
  inv1   g118(.a(x41), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(new_n104_), .O(new_n203_));
  nand4  g120(.a(new_n203_), .b(new_n201_), .c(new_n89_), .d(x09), .O(new_n204_));
  inv1   g121(.a(new_n204_), .O(z29));
  nand2  g122(.a(x41), .b(x14), .O(new_n206_));
  aoi21  g123(.a(x42), .b(new_n104_), .c(new_n86_), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n206_), .c(x10), .O(z30));
  nand2  g125(.a(x42), .b(x14), .O(new_n209_));
  aoi21  g126(.a(x43), .b(new_n104_), .c(new_n86_), .O(new_n210_));
  aoi21  g127(.a(new_n210_), .b(new_n209_), .c(x10), .O(z31));
  inv1   g128(.a(x43), .O(new_n212_));
  nand2  g129(.a(new_n212_), .b(x14), .O(new_n213_));
  inv1   g130(.a(x44), .O(new_n214_));
  nand2  g131(.a(new_n214_), .b(new_n104_), .O(new_n215_));
  nand4  g132(.a(new_n215_), .b(new_n213_), .c(new_n89_), .d(x09), .O(new_n216_));
  inv1   g133(.a(new_n216_), .O(z32));
  nand2  g134(.a(x44), .b(x14), .O(new_n218_));
  aoi21  g135(.a(x45), .b(new_n104_), .c(new_n86_), .O(new_n219_));
  aoi21  g136(.a(new_n219_), .b(new_n218_), .c(x10), .O(z33));
  nand2  g137(.a(x45), .b(x14), .O(new_n221_));
  aoi21  g138(.a(x46), .b(new_n104_), .c(new_n86_), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n221_), .c(x10), .O(z34));
  inv1   g140(.a(x46), .O(new_n224_));
  nand2  g141(.a(new_n224_), .b(x14), .O(new_n225_));
  nand2  g142(.a(new_n104_), .b(new_n171_), .O(new_n226_));
  nand4  g143(.a(new_n226_), .b(new_n225_), .c(new_n89_), .d(x09), .O(new_n227_));
  inv1   g144(.a(new_n227_), .O(z35));
endmodule


