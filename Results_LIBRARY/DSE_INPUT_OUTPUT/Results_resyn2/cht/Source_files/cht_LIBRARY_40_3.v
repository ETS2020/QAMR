// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n150_, new_n151_, new_n152_, new_n154_,
    new_n155_, new_n157_, new_n158_, new_n160_, new_n161_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n169_, new_n170_, new_n172_,
    new_n173_, new_n174_, new_n175_, new_n176_, new_n177_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n189_, new_n190_, new_n191_, new_n192_, new_n194_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n202_, new_n204_, new_n205_, new_n206_, new_n207_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n214_, new_n215_, new_n216_;
  inv1   g000(.a(x07), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x04), .O(new_n85_));
  inv1   g002(.a(x10), .O(new_n86_));
  oai21  g003(.a(x11), .b(x07), .c(new_n86_), .O(new_n87_));
  nor2   g004(.a(new_n87_), .b(new_n85_), .O(z00));
  nand2  g005(.a(x35), .b(x10), .O(new_n89_));
  nor2   g006(.a(new_n84_), .b(x05), .O(new_n90_));
  oai21  g007(.a(x12), .b(x07), .c(new_n86_), .O(new_n91_));
  oai21  g008(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z01));
  nor2   g009(.a(new_n84_), .b(x06), .O(new_n93_));
  oai21  g010(.a(x13), .b(x07), .c(new_n86_), .O(new_n94_));
  oai21  g011(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z02));
  nor2   g012(.a(new_n84_), .b(x01), .O(new_n96_));
  oai21  g013(.a(x14), .b(x07), .c(new_n86_), .O(new_n97_));
  nor2   g014(.a(new_n97_), .b(new_n96_), .O(z03));
  nor2   g015(.a(new_n84_), .b(x02), .O(new_n99_));
  oai21  g016(.a(x15), .b(x07), .c(new_n86_), .O(new_n100_));
  oai21  g017(.a(new_n100_), .b(new_n99_), .c(new_n89_), .O(z04));
  nor2   g018(.a(new_n84_), .b(x03), .O(new_n102_));
  oai21  g019(.a(x16), .b(x07), .c(new_n86_), .O(new_n103_));
  oai21  g020(.a(new_n103_), .b(new_n102_), .c(new_n89_), .O(z05));
  inv1   g021(.a(x08), .O(new_n105_));
  nor2   g022(.a(x18), .b(new_n105_), .O(new_n106_));
  oai21  g023(.a(x17), .b(x08), .c(new_n86_), .O(new_n107_));
  nor2   g024(.a(new_n107_), .b(new_n106_), .O(z06));
  nor2   g025(.a(x19), .b(new_n105_), .O(new_n109_));
  oai21  g026(.a(x18), .b(x08), .c(new_n86_), .O(new_n110_));
  nor2   g027(.a(new_n110_), .b(new_n109_), .O(z07));
  nor2   g028(.a(x20), .b(new_n105_), .O(new_n112_));
  oai21  g029(.a(x19), .b(x08), .c(new_n86_), .O(new_n113_));
  nor2   g030(.a(new_n113_), .b(new_n112_), .O(z08));
  inv1   g031(.a(x21), .O(new_n115_));
  oai21  g032(.a(x20), .b(x08), .c(new_n86_), .O(new_n116_));
  aoi21  g033(.a(new_n115_), .b(x08), .c(new_n116_), .O(z09));
  nor2   g034(.a(x22), .b(new_n105_), .O(new_n118_));
  oai21  g035(.a(x21), .b(x08), .c(new_n86_), .O(new_n119_));
  oai21  g036(.a(new_n119_), .b(new_n118_), .c(new_n89_), .O(z10));
  nor2   g037(.a(x23), .b(new_n105_), .O(new_n121_));
  oai21  g038(.a(x22), .b(x08), .c(new_n86_), .O(new_n122_));
  nor2   g039(.a(new_n122_), .b(new_n121_), .O(z11));
  inv1   g040(.a(x24), .O(new_n124_));
  oai21  g041(.a(x23), .b(x08), .c(new_n86_), .O(new_n125_));
  aoi21  g042(.a(new_n124_), .b(x08), .c(new_n125_), .O(z12));
  inv1   g043(.a(x25), .O(new_n127_));
  nand2  g044(.a(new_n127_), .b(x08), .O(new_n128_));
  nand2  g045(.a(new_n124_), .b(new_n105_), .O(new_n129_));
  nand3  g046(.a(new_n129_), .b(new_n128_), .c(new_n86_), .O(new_n130_));
  nand2  g047(.a(new_n130_), .b(new_n89_), .O(z13));
  nor2   g048(.a(x26), .b(new_n105_), .O(new_n132_));
  oai21  g049(.a(x25), .b(x08), .c(new_n86_), .O(new_n133_));
  oai21  g050(.a(new_n133_), .b(new_n132_), .c(new_n89_), .O(z14));
  inv1   g051(.a(x27), .O(new_n135_));
  oai21  g052(.a(x26), .b(x08), .c(new_n86_), .O(new_n136_));
  aoi21  g053(.a(new_n135_), .b(x08), .c(new_n136_), .O(z15));
  nor2   g054(.a(x28), .b(new_n105_), .O(new_n138_));
  oai21  g055(.a(x27), .b(x08), .c(new_n86_), .O(new_n139_));
  oai21  g056(.a(new_n139_), .b(new_n138_), .c(new_n89_), .O(z16));
  nor2   g057(.a(x29), .b(new_n105_), .O(new_n141_));
  oai21  g058(.a(x28), .b(x08), .c(new_n86_), .O(new_n142_));
  nor2   g059(.a(new_n142_), .b(new_n141_), .O(z17));
  nor2   g060(.a(x30), .b(new_n105_), .O(new_n144_));
  oai21  g061(.a(x29), .b(x08), .c(new_n86_), .O(new_n145_));
  nor2   g062(.a(new_n145_), .b(new_n144_), .O(z18));
  inv1   g063(.a(x00), .O(new_n147_));
  oai21  g064(.a(x30), .b(x08), .c(new_n86_), .O(new_n148_));
  aoi21  g065(.a(x08), .b(new_n147_), .c(new_n148_), .O(z19));
  inv1   g066(.a(x09), .O(new_n150_));
  nor2   g067(.a(x32), .b(new_n150_), .O(new_n151_));
  oai21  g068(.a(x31), .b(x09), .c(new_n86_), .O(new_n152_));
  nor2   g069(.a(new_n152_), .b(new_n151_), .O(z20));
  nor2   g070(.a(x33), .b(new_n150_), .O(new_n154_));
  oai21  g071(.a(x32), .b(x09), .c(new_n86_), .O(new_n155_));
  nor2   g072(.a(new_n155_), .b(new_n154_), .O(z21));
  inv1   g073(.a(x34), .O(new_n157_));
  oai21  g074(.a(x33), .b(x09), .c(new_n86_), .O(new_n158_));
  aoi21  g075(.a(new_n157_), .b(x09), .c(new_n158_), .O(z22));
  nand2  g076(.a(new_n86_), .b(new_n150_), .O(new_n160_));
  nand2  g077(.a(new_n160_), .b(x35), .O(new_n161_));
  oai21  g078(.a(new_n160_), .b(new_n157_), .c(new_n161_), .O(z23));
  nor2   g079(.a(x36), .b(new_n150_), .O(new_n163_));
  oai21  g080(.a(x35), .b(x09), .c(new_n86_), .O(new_n164_));
  nor2   g081(.a(new_n164_), .b(new_n163_), .O(z24));
  nor2   g082(.a(x37), .b(new_n150_), .O(new_n166_));
  oai21  g083(.a(x36), .b(x09), .c(new_n86_), .O(new_n167_));
  nor2   g084(.a(new_n167_), .b(new_n166_), .O(z25));
  nor2   g085(.a(x38), .b(new_n150_), .O(new_n169_));
  oai21  g086(.a(x37), .b(x09), .c(new_n86_), .O(new_n170_));
  nor2   g087(.a(new_n170_), .b(new_n169_), .O(z26));
  inv1   g088(.a(x14), .O(new_n172_));
  inv1   g089(.a(x39), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand2  g091(.a(x14), .b(new_n147_), .O(new_n175_));
  nand2  g092(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  oai21  g093(.a(x38), .b(x09), .c(new_n86_), .O(new_n177_));
  aoi21  g094(.a(new_n176_), .b(x09), .c(new_n177_), .O(z27));
  inv1   g095(.a(x40), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n172_), .c(x09), .O(new_n180_));
  oai21  g097(.a(x14), .b(new_n150_), .c(new_n173_), .O(new_n181_));
  nand3  g098(.a(new_n181_), .b(new_n180_), .c(new_n86_), .O(new_n182_));
  inv1   g099(.a(new_n182_), .O(z28));
  inv1   g100(.a(x41), .O(new_n184_));
  nand3  g101(.a(new_n184_), .b(new_n172_), .c(x09), .O(new_n185_));
  oai21  g102(.a(x14), .b(new_n150_), .c(new_n179_), .O(new_n186_));
  nand3  g103(.a(new_n186_), .b(new_n185_), .c(new_n86_), .O(new_n187_));
  nand2  g104(.a(new_n187_), .b(new_n89_), .O(z29));
  inv1   g105(.a(x42), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n172_), .c(x09), .O(new_n190_));
  oai21  g107(.a(x14), .b(new_n150_), .c(new_n184_), .O(new_n191_));
  nand3  g108(.a(new_n191_), .b(new_n190_), .c(new_n86_), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(new_n89_), .O(z30));
  inv1   g110(.a(x43), .O(new_n194_));
  nand3  g111(.a(new_n194_), .b(new_n172_), .c(x09), .O(new_n195_));
  oai21  g112(.a(x14), .b(new_n150_), .c(new_n189_), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n195_), .c(new_n86_), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(new_n89_), .O(z31));
  inv1   g115(.a(x44), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(new_n172_), .c(x09), .O(new_n200_));
  oai21  g117(.a(x14), .b(new_n150_), .c(new_n194_), .O(new_n201_));
  nand3  g118(.a(new_n201_), .b(new_n200_), .c(new_n86_), .O(new_n202_));
  nand2  g119(.a(new_n202_), .b(new_n89_), .O(z32));
  inv1   g120(.a(x45), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n172_), .c(x09), .O(new_n205_));
  oai21  g122(.a(x14), .b(new_n150_), .c(new_n199_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n205_), .c(new_n86_), .O(new_n207_));
  nand2  g124(.a(new_n207_), .b(new_n89_), .O(z33));
  inv1   g125(.a(x46), .O(new_n209_));
  nand3  g126(.a(new_n209_), .b(new_n172_), .c(x09), .O(new_n210_));
  oai21  g127(.a(x14), .b(new_n150_), .c(new_n204_), .O(new_n211_));
  nand3  g128(.a(new_n211_), .b(new_n210_), .c(new_n86_), .O(new_n212_));
  inv1   g129(.a(new_n212_), .O(z34));
  nand3  g130(.a(new_n172_), .b(x09), .c(new_n147_), .O(new_n214_));
  oai21  g131(.a(x14), .b(new_n150_), .c(new_n209_), .O(new_n215_));
  nand3  g132(.a(new_n215_), .b(new_n214_), .c(new_n86_), .O(new_n216_));
  nand2  g133(.a(new_n216_), .b(new_n89_), .O(z35));
endmodule


