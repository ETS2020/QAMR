// Benchmark "FAU" written by ABC on Sat Aug  8 23:35:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n168_, new_n169_,
    new_n170_, new_n171_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n180_, new_n181_, new_n182_, new_n183_,
    new_n185_, new_n186_, new_n187_, new_n188_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n200_, new_n201_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n207_, new_n209_, new_n210_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n217_, new_n218_, new_n219_, new_n221_,
    new_n222_, new_n223_, new_n224_, new_n226_, new_n227_, new_n228_,
    new_n229_, new_n231_, new_n232_, new_n233_, new_n234_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n241_, new_n242_, new_n243_,
    new_n244_, new_n246_, new_n247_, new_n248_, new_n249_, new_n251_,
    new_n252_, new_n253_, new_n254_, new_n256_, new_n257_, new_n258_;
  inv1   g000(.a(x21), .O(new_n84_));
  inv1   g001(.a(x37), .O(new_n85_));
  nand2  g002(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g003(.a(x07), .O(new_n87_));
  nor2   g004(.a(new_n87_), .b(x04), .O(new_n88_));
  inv1   g005(.a(x10), .O(new_n89_));
  oai21  g006(.a(x11), .b(x07), .c(new_n89_), .O(new_n90_));
  oai21  g007(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(z00));
  nor2   g008(.a(new_n87_), .b(x05), .O(new_n92_));
  oai21  g009(.a(x12), .b(x07), .c(new_n89_), .O(new_n93_));
  oai21  g010(.a(new_n93_), .b(new_n92_), .c(new_n86_), .O(z01));
  nor2   g011(.a(new_n87_), .b(x06), .O(new_n95_));
  oai21  g012(.a(x13), .b(x07), .c(new_n89_), .O(new_n96_));
  oai21  g013(.a(new_n96_), .b(new_n95_), .c(new_n86_), .O(z02));
  nor2   g014(.a(new_n87_), .b(x01), .O(new_n98_));
  oai21  g015(.a(x14), .b(x07), .c(new_n89_), .O(new_n99_));
  oai21  g016(.a(new_n99_), .b(new_n98_), .c(new_n86_), .O(z03));
  aoi21  g017(.a(new_n85_), .b(new_n84_), .c(x10), .O(new_n101_));
  inv1   g018(.a(x15), .O(new_n102_));
  nand2  g019(.a(new_n102_), .b(new_n87_), .O(new_n103_));
  inv1   g020(.a(x02), .O(new_n104_));
  nand2  g021(.a(x07), .b(new_n104_), .O(new_n105_));
  nand3  g022(.a(new_n105_), .b(new_n103_), .c(new_n101_), .O(new_n106_));
  inv1   g023(.a(new_n106_), .O(z04));
  nor2   g024(.a(new_n87_), .b(x03), .O(new_n108_));
  oai21  g025(.a(x16), .b(x07), .c(new_n89_), .O(new_n109_));
  oai21  g026(.a(new_n109_), .b(new_n108_), .c(new_n86_), .O(z05));
  inv1   g027(.a(x18), .O(new_n111_));
  nand2  g028(.a(new_n111_), .b(x08), .O(new_n112_));
  inv1   g029(.a(x08), .O(new_n113_));
  inv1   g030(.a(x17), .O(new_n114_));
  nand2  g031(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(new_n112_), .c(new_n89_), .O(new_n116_));
  nand2  g033(.a(new_n116_), .b(new_n86_), .O(z06));
  nand2  g034(.a(new_n111_), .b(new_n113_), .O(new_n118_));
  inv1   g035(.a(x19), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(x08), .O(new_n120_));
  nand3  g037(.a(new_n120_), .b(new_n118_), .c(new_n101_), .O(new_n121_));
  inv1   g038(.a(new_n121_), .O(z07));
  nor2   g039(.a(x20), .b(new_n113_), .O(new_n123_));
  oai21  g040(.a(x19), .b(x08), .c(new_n89_), .O(new_n124_));
  oai21  g041(.a(new_n124_), .b(new_n123_), .c(new_n86_), .O(z08));
  aoi21  g042(.a(x37), .b(new_n113_), .c(x21), .O(new_n126_));
  oai21  g043(.a(x20), .b(x08), .c(new_n89_), .O(new_n127_));
  nor2   g044(.a(new_n127_), .b(new_n126_), .O(z09));
  nand2  g045(.a(new_n86_), .b(x22), .O(new_n129_));
  oai21  g046(.a(x21), .b(x08), .c(new_n89_), .O(new_n130_));
  aoi21  g047(.a(new_n129_), .b(x08), .c(new_n130_), .O(z10));
  inv1   g048(.a(x22), .O(new_n132_));
  nand2  g049(.a(new_n132_), .b(new_n113_), .O(new_n133_));
  inv1   g050(.a(x23), .O(new_n134_));
  nand2  g051(.a(new_n134_), .b(x08), .O(new_n135_));
  nand3  g052(.a(new_n135_), .b(new_n133_), .c(new_n101_), .O(new_n136_));
  inv1   g053(.a(new_n136_), .O(z11));
  inv1   g054(.a(x24), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(x08), .O(new_n139_));
  nand2  g056(.a(new_n134_), .b(new_n113_), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n89_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n86_), .O(z12));
  inv1   g059(.a(x25), .O(new_n143_));
  nand2  g060(.a(new_n143_), .b(x08), .O(new_n144_));
  nand2  g061(.a(new_n138_), .b(new_n113_), .O(new_n145_));
  nand3  g062(.a(new_n145_), .b(new_n144_), .c(new_n89_), .O(new_n146_));
  nand2  g063(.a(new_n146_), .b(new_n86_), .O(z13));
  inv1   g064(.a(x26), .O(new_n148_));
  nand2  g065(.a(new_n148_), .b(x08), .O(new_n149_));
  nand2  g066(.a(new_n143_), .b(new_n113_), .O(new_n150_));
  nand3  g067(.a(new_n150_), .b(new_n149_), .c(new_n89_), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(new_n86_), .O(z14));
  inv1   g069(.a(x27), .O(new_n153_));
  nand2  g070(.a(new_n153_), .b(x08), .O(new_n154_));
  nand2  g071(.a(new_n148_), .b(new_n113_), .O(new_n155_));
  nand3  g072(.a(new_n155_), .b(new_n154_), .c(new_n89_), .O(new_n156_));
  nand2  g073(.a(new_n156_), .b(new_n86_), .O(z15));
  nand2  g074(.a(new_n153_), .b(new_n113_), .O(new_n158_));
  inv1   g075(.a(x28), .O(new_n159_));
  nand2  g076(.a(new_n159_), .b(x08), .O(new_n160_));
  nand3  g077(.a(new_n160_), .b(new_n158_), .c(new_n101_), .O(new_n161_));
  inv1   g078(.a(new_n161_), .O(z16));
  inv1   g079(.a(x29), .O(new_n163_));
  nand2  g080(.a(new_n163_), .b(x08), .O(new_n164_));
  nand2  g081(.a(new_n159_), .b(new_n113_), .O(new_n165_));
  nand3  g082(.a(new_n165_), .b(new_n164_), .c(new_n89_), .O(new_n166_));
  nand2  g083(.a(new_n166_), .b(new_n86_), .O(z17));
  inv1   g084(.a(x30), .O(new_n168_));
  nand2  g085(.a(new_n168_), .b(x08), .O(new_n169_));
  nand2  g086(.a(new_n163_), .b(new_n113_), .O(new_n170_));
  nand3  g087(.a(new_n170_), .b(new_n169_), .c(new_n89_), .O(new_n171_));
  nand2  g088(.a(new_n171_), .b(new_n86_), .O(z18));
  inv1   g089(.a(x00), .O(new_n173_));
  nand2  g090(.a(x08), .b(new_n173_), .O(new_n174_));
  nand2  g091(.a(new_n168_), .b(new_n113_), .O(new_n175_));
  nand3  g092(.a(new_n175_), .b(new_n174_), .c(new_n89_), .O(new_n176_));
  nand2  g093(.a(new_n176_), .b(new_n86_), .O(z19));
  inv1   g094(.a(x09), .O(new_n178_));
  inv1   g095(.a(x31), .O(new_n179_));
  nand2  g096(.a(new_n179_), .b(new_n178_), .O(new_n180_));
  inv1   g097(.a(x32), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(x09), .O(new_n182_));
  nand3  g099(.a(new_n182_), .b(new_n180_), .c(new_n101_), .O(new_n183_));
  inv1   g100(.a(new_n183_), .O(z20));
  inv1   g101(.a(x33), .O(new_n185_));
  nand2  g102(.a(new_n185_), .b(x09), .O(new_n186_));
  nand2  g103(.a(new_n181_), .b(new_n178_), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(new_n89_), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n86_), .O(z21));
  inv1   g106(.a(x34), .O(new_n190_));
  nand2  g107(.a(new_n190_), .b(x09), .O(new_n191_));
  nand2  g108(.a(new_n185_), .b(new_n178_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n191_), .c(new_n89_), .O(new_n193_));
  nand2  g110(.a(new_n193_), .b(new_n86_), .O(z22));
  nand2  g111(.a(new_n190_), .b(new_n178_), .O(new_n195_));
  inv1   g112(.a(x35), .O(new_n196_));
  nand2  g113(.a(new_n196_), .b(x09), .O(new_n197_));
  nand3  g114(.a(new_n197_), .b(new_n195_), .c(new_n101_), .O(new_n198_));
  inv1   g115(.a(new_n198_), .O(z23));
  inv1   g116(.a(x36), .O(new_n200_));
  nand2  g117(.a(new_n200_), .b(x09), .O(new_n201_));
  nand2  g118(.a(new_n196_), .b(new_n178_), .O(new_n202_));
  nand3  g119(.a(new_n202_), .b(new_n201_), .c(new_n89_), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(new_n86_), .O(z24));
  nand2  g121(.a(new_n85_), .b(x09), .O(new_n205_));
  nand2  g122(.a(new_n200_), .b(new_n178_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n205_), .c(new_n89_), .O(new_n207_));
  nand2  g124(.a(new_n207_), .b(new_n86_), .O(z25));
  aoi21  g125(.a(x21), .b(x09), .c(x37), .O(new_n209_));
  oai21  g126(.a(x38), .b(new_n178_), .c(new_n89_), .O(new_n210_));
  nor2   g127(.a(new_n210_), .b(new_n209_), .O(z26));
  inv1   g128(.a(x14), .O(new_n212_));
  inv1   g129(.a(x39), .O(new_n213_));
  nand2  g130(.a(new_n213_), .b(new_n212_), .O(new_n214_));
  nand2  g131(.a(x14), .b(new_n173_), .O(new_n215_));
  nand2  g132(.a(new_n215_), .b(new_n214_), .O(new_n216_));
  inv1   g133(.a(x38), .O(new_n217_));
  nand2  g134(.a(new_n217_), .b(new_n178_), .O(new_n218_));
  nand3  g135(.a(new_n218_), .b(new_n86_), .c(new_n89_), .O(new_n219_));
  aoi21  g136(.a(new_n216_), .b(x09), .c(new_n219_), .O(z27));
  inv1   g137(.a(x40), .O(new_n221_));
  nand3  g138(.a(new_n221_), .b(new_n212_), .c(x09), .O(new_n222_));
  oai21  g139(.a(x14), .b(new_n178_), .c(new_n213_), .O(new_n223_));
  nand3  g140(.a(new_n223_), .b(new_n222_), .c(new_n89_), .O(new_n224_));
  nand2  g141(.a(new_n224_), .b(new_n86_), .O(z28));
  inv1   g142(.a(x41), .O(new_n226_));
  nand3  g143(.a(new_n226_), .b(new_n212_), .c(x09), .O(new_n227_));
  oai21  g144(.a(x14), .b(new_n178_), .c(new_n221_), .O(new_n228_));
  nand3  g145(.a(new_n228_), .b(new_n227_), .c(new_n89_), .O(new_n229_));
  nand2  g146(.a(new_n229_), .b(new_n86_), .O(z29));
  inv1   g147(.a(x42), .O(new_n231_));
  nand3  g148(.a(new_n231_), .b(new_n212_), .c(x09), .O(new_n232_));
  oai21  g149(.a(x14), .b(new_n178_), .c(new_n226_), .O(new_n233_));
  nand3  g150(.a(new_n233_), .b(new_n232_), .c(new_n89_), .O(new_n234_));
  nand2  g151(.a(new_n234_), .b(new_n86_), .O(z30));
  inv1   g152(.a(x43), .O(new_n236_));
  nand3  g153(.a(new_n236_), .b(new_n212_), .c(x09), .O(new_n237_));
  oai21  g154(.a(x14), .b(new_n178_), .c(new_n231_), .O(new_n238_));
  nand3  g155(.a(new_n238_), .b(new_n237_), .c(new_n101_), .O(new_n239_));
  inv1   g156(.a(new_n239_), .O(z31));
  inv1   g157(.a(x44), .O(new_n241_));
  nand3  g158(.a(new_n241_), .b(new_n212_), .c(x09), .O(new_n242_));
  oai21  g159(.a(x14), .b(new_n178_), .c(new_n236_), .O(new_n243_));
  nand3  g160(.a(new_n243_), .b(new_n242_), .c(new_n89_), .O(new_n244_));
  nand2  g161(.a(new_n244_), .b(new_n86_), .O(z32));
  inv1   g162(.a(x45), .O(new_n246_));
  nand3  g163(.a(new_n246_), .b(new_n212_), .c(x09), .O(new_n247_));
  oai21  g164(.a(x14), .b(new_n178_), .c(new_n241_), .O(new_n248_));
  nand3  g165(.a(new_n248_), .b(new_n247_), .c(new_n89_), .O(new_n249_));
  nand2  g166(.a(new_n249_), .b(new_n86_), .O(z33));
  inv1   g167(.a(x46), .O(new_n251_));
  nand3  g168(.a(new_n251_), .b(new_n212_), .c(x09), .O(new_n252_));
  oai21  g169(.a(x14), .b(new_n178_), .c(new_n246_), .O(new_n253_));
  nand3  g170(.a(new_n253_), .b(new_n252_), .c(new_n101_), .O(new_n254_));
  inv1   g171(.a(new_n254_), .O(z34));
  nand3  g172(.a(new_n212_), .b(x09), .c(new_n173_), .O(new_n256_));
  oai21  g173(.a(x14), .b(new_n178_), .c(new_n251_), .O(new_n257_));
  nand3  g174(.a(new_n257_), .b(new_n256_), .c(new_n89_), .O(new_n258_));
  nand2  g175(.a(new_n258_), .b(new_n86_), .O(z35));
endmodule


