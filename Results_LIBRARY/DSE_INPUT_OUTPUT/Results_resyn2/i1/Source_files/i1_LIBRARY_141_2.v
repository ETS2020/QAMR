// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n76_,
    new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x03), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x07), .O(new_n46_));
  nor3   g05(.a(x06), .b(x05), .c(x04), .O(new_n47_));
  nor3   g06(.a(x03), .b(x02), .c(x01), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(x00), .c(new_n43_), .O(z01));
  oai21  g10(.a(new_n43_), .b(x00), .c(x03), .O(new_n52_));
  nor3   g11(.a(x07), .b(x02), .c(x01), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n47_), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(x03), .O(new_n55_));
  inv1   g14(.a(x06), .O(new_n56_));
  nor2   g15(.a(x05), .b(x04), .O(new_n57_));
  nor2   g16(.a(x02), .b(x01), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n59_));
  inv1   g18(.a(x09), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(x08), .c(new_n46_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  oai21  g21(.a(new_n62_), .b(new_n54_), .c(x19), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n52_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  inv1   g24(.a(new_n44_), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n65_), .O(z03));
  inv1   g26(.a(x21), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n65_), .c(new_n66_), .O(z04));
  nand2  g28(.a(new_n53_), .b(new_n47_), .O(new_n70_));
  nand2  g29(.a(new_n44_), .b(x10), .O(new_n71_));
  inv1   g30(.a(x08), .O(new_n72_));
  nand3  g31(.a(x19), .b(new_n72_), .c(new_n55_), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n70_), .c(new_n71_), .O(z05));
  nand2  g33(.a(new_n43_), .b(new_n55_), .O(z06));
  nand2  g34(.a(x24), .b(x18), .O(new_n76_));
  nor2   g35(.a(new_n76_), .b(new_n66_), .O(z07));
  inv1   g36(.a(x11), .O(new_n78_));
  nand2  g37(.a(new_n44_), .b(new_n78_), .O(z08));
  inv1   g38(.a(x24), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(new_n44_), .O(z09));
  inv1   g40(.a(x14), .O(new_n82_));
  nand2  g41(.a(new_n44_), .b(new_n80_), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x22), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(z10));
  inv1   g45(.a(x22), .O(new_n87_));
  nand2  g46(.a(new_n80_), .b(x17), .O(new_n88_));
  oai21  g47(.a(new_n88_), .b(new_n87_), .c(new_n44_), .O(z11));
  nand2  g48(.a(new_n84_), .b(x23), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z12));
  inv1   g50(.a(x23), .O(new_n92_));
  oai21  g51(.a(new_n88_), .b(new_n92_), .c(new_n44_), .O(z13));
  inv1   g52(.a(x16), .O(new_n94_));
  nor2   g53(.a(new_n83_), .b(new_n94_), .O(z14));
  nor2   g54(.a(x13), .b(x12), .O(new_n96_));
  nor2   g55(.a(x15), .b(x14), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n66_), .O(z15));
endmodule


