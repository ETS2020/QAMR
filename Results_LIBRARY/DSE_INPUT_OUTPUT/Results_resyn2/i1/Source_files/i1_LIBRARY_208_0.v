// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n77_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n86_, new_n89_, new_n91_,
    new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x17), .b(x07), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  inv1   g04(.a(new_n43_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  nor2   g07(.a(x02), .b(x01), .O(new_n49_));
  nor2   g08(.a(x04), .b(x03), .O(new_n50_));
  nor2   g09(.a(x06), .b(x05), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n53_), .c(x17), .d(new_n48_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n47_), .c(new_n45_), .O(z01));
  nand2  g15(.a(new_n52_), .b(x17), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g18(.a(x08), .O(new_n60_));
  nor2   g19(.a(x09), .b(new_n60_), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n53_), .c(x17), .d(new_n48_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(new_n59_), .c(new_n45_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z03));
  inv1   g24(.a(x21), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n64_), .c(new_n43_), .O(z04));
  inv1   g26(.a(x10), .O(new_n68_));
  nand2  g27(.a(x19), .b(new_n60_), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(new_n52_), .c(x17), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n48_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n68_), .O(z05));
  nor2   g31(.a(new_n43_), .b(new_n45_), .O(z06));
  inv1   g32(.a(x18), .O(new_n74_));
  nand2  g33(.a(new_n46_), .b(x24), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n74_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nand2  g36(.a(new_n46_), .b(new_n77_), .O(z08));
  nor2   g37(.a(new_n75_), .b(new_n77_), .O(z09));
  inv1   g38(.a(x22), .O(new_n80_));
  inv1   g39(.a(x24), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x14), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n80_), .c(new_n46_), .O(z10));
  inv1   g42(.a(x17), .O(new_n84_));
  nor3   g43(.a(x24), .b(new_n80_), .c(new_n84_), .O(z11));
  inv1   g44(.a(x23), .O(new_n86_));
  nor3   g45(.a(new_n82_), .b(new_n43_), .c(new_n86_), .O(z12));
  nor3   g46(.a(x24), .b(new_n86_), .c(new_n84_), .O(z13));
  nand2  g47(.a(new_n81_), .b(x16), .O(new_n89_));
  nor2   g48(.a(new_n89_), .b(new_n43_), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n43_), .O(z15));
endmodule


