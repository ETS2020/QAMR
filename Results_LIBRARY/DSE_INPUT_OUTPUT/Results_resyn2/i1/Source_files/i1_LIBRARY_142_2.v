// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n63_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n90_;
  inv1   g00(.a(x19), .O(new_n42_));
  nand2  g01(.a(new_n42_), .b(x04), .O(new_n43_));
  and2   g02(.a(new_n43_), .b(x00), .O(z00));
  inv1   g03(.a(x07), .O(new_n45_));
  nor3   g04(.a(x05), .b(x04), .c(x03), .O(new_n46_));
  nor3   g05(.a(x06), .b(x02), .c(x01), .O(new_n47_));
  xnor2a g06(.a(x09), .b(x08), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x00), .c(new_n42_), .O(z01));
  inv1   g09(.a(x05), .O(new_n51_));
  nor2   g10(.a(x04), .b(x03), .O(new_n52_));
  nor2   g11(.a(x02), .b(x01), .O(new_n53_));
  nor2   g12(.a(x07), .b(x06), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x00), .O(new_n56_));
  inv1   g15(.a(x09), .O(new_n57_));
  inv1   g16(.a(x08), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x07), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(new_n56_), .c(new_n42_), .O(z02));
  and2   g20(.a(new_n43_), .b(x20), .O(z03));
  nor2   g21(.a(x21), .b(x20), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n43_), .O(z04));
  nand2  g23(.a(new_n43_), .b(x10), .O(new_n65_));
  nand2  g24(.a(x19), .b(new_n58_), .O(new_n66_));
  oai21  g25(.a(new_n66_), .b(new_n55_), .c(new_n65_), .O(z05));
  inv1   g26(.a(x04), .O(new_n68_));
  nand2  g27(.a(new_n42_), .b(new_n68_), .O(z06));
  inv1   g28(.a(x18), .O(new_n70_));
  inv1   g29(.a(x24), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n70_), .c(new_n43_), .O(z07));
  inv1   g31(.a(x11), .O(new_n73_));
  nand2  g32(.a(new_n43_), .b(new_n73_), .O(z08));
  inv1   g33(.a(new_n43_), .O(new_n75_));
  nor3   g34(.a(new_n75_), .b(new_n71_), .c(new_n73_), .O(z09));
  inv1   g35(.a(x22), .O(new_n77_));
  nand2  g36(.a(new_n71_), .b(x14), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n77_), .c(new_n43_), .O(z10));
  nand2  g38(.a(new_n43_), .b(new_n71_), .O(new_n80_));
  nand2  g39(.a(x22), .b(x17), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n80_), .O(z11));
  nand2  g41(.a(new_n43_), .b(x23), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n78_), .O(z12));
  nand3  g43(.a(new_n71_), .b(x23), .c(x17), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n43_), .O(z13));
  inv1   g45(.a(x16), .O(new_n87_));
  nor2   g46(.a(new_n80_), .b(new_n87_), .O(z14));
  nor2   g47(.a(x13), .b(x12), .O(new_n89_));
  nor2   g48(.a(x15), .b(x14), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n43_), .O(z15));
endmodule


