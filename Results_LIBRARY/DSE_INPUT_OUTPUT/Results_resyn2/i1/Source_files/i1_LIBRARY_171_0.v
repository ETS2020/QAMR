// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n76_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x19), .b(x08), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x07), .O(new_n46_));
  nor2   g05(.a(x06), .b(x05), .O(new_n47_));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  nor2   g07(.a(x04), .b(x03), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(x09), .b(new_n51_), .O(new_n52_));
  inv1   g11(.a(x19), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x08), .O(z06));
  inv1   g13(.a(x09), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x08), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(z06), .c(new_n52_), .O(new_n57_));
  aoi21  g16(.a(x19), .b(new_n42_), .c(new_n43_), .O(new_n58_));
  oai21  g17(.a(new_n57_), .b(new_n50_), .c(new_n58_), .O(z01));
  nor3   g18(.a(x07), .b(x06), .c(x05), .O(new_n60_));
  nand4  g19(.a(new_n56_), .b(new_n49_), .c(new_n48_), .d(new_n60_), .O(new_n61_));
  nand2  g20(.a(new_n50_), .b(new_n42_), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n61_), .c(x19), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nor2   g24(.a(new_n43_), .b(new_n65_), .O(z03));
  nor2   g25(.a(x21), .b(x20), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n44_), .O(z04));
  inv1   g27(.a(x10), .O(new_n69_));
  nand4  g28(.a(new_n49_), .b(new_n48_), .c(new_n60_), .d(new_n51_), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n43_), .O(z05));
  nand2  g30(.a(x24), .b(x18), .O(new_n72_));
  nor2   g31(.a(new_n72_), .b(new_n43_), .O(z07));
  inv1   g32(.a(x11), .O(new_n74_));
  nor2   g33(.a(new_n43_), .b(new_n74_), .O(z08));
  inv1   g34(.a(x24), .O(new_n76_));
  nor3   g35(.a(new_n43_), .b(new_n76_), .c(new_n74_), .O(z09));
  inv1   g36(.a(x22), .O(new_n78_));
  nand2  g37(.a(new_n76_), .b(x14), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n78_), .c(new_n44_), .O(z10));
  nand2  g39(.a(new_n76_), .b(x17), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n43_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x22), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z11));
  inv1   g43(.a(x23), .O(new_n85_));
  nor3   g44(.a(new_n79_), .b(new_n43_), .c(new_n85_), .O(z12));
  nand2  g45(.a(new_n82_), .b(x23), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z13));
  nand2  g47(.a(new_n76_), .b(x16), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n44_), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n43_), .O(z15));
endmodule


