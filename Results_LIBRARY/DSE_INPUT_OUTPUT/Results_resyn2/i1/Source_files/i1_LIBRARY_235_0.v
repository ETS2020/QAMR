// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n77_, new_n79_, new_n81_, new_n83_, new_n85_, new_n86_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x19), .b(x07), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x04), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nor3   g10(.a(x07), .b(x06), .c(x05), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n45_), .O(z01));
  inv1   g14(.a(x07), .O(new_n56_));
  inv1   g15(.a(x05), .O(new_n57_));
  inv1   g16(.a(x06), .O(new_n58_));
  inv1   g17(.a(x09), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x08), .c(new_n58_), .d(new_n57_), .O(new_n60_));
  oai21  g19(.a(new_n60_), .b(new_n50_), .c(x19), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  nor2   g21(.a(x04), .b(x03), .O(new_n63_));
  nand4  g22(.a(new_n52_), .b(new_n63_), .c(new_n47_), .d(new_n46_), .O(new_n64_));
  nand3  g23(.a(new_n64_), .b(x19), .c(x00), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n62_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  inv1   g26(.a(new_n43_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n67_), .O(z03));
  nor2   g28(.a(x21), .b(x20), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(new_n43_), .O(z04));
  nand2  g30(.a(new_n68_), .b(x10), .O(new_n72_));
  inv1   g31(.a(x08), .O(new_n73_));
  nand2  g32(.a(x19), .b(new_n73_), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n64_), .c(new_n72_), .O(z05));
  nand2  g34(.a(new_n45_), .b(x07), .O(z06));
  nand2  g35(.a(x24), .b(x18), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n43_), .O(z07));
  inv1   g37(.a(x11), .O(new_n79_));
  nor2   g38(.a(new_n43_), .b(new_n79_), .O(z08));
  inv1   g39(.a(x24), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n79_), .c(new_n68_), .O(z09));
  nand2  g41(.a(x22), .b(x14), .O(new_n83_));
  nor3   g42(.a(new_n83_), .b(new_n43_), .c(x24), .O(z10));
  inv1   g43(.a(x22), .O(new_n85_));
  nand2  g44(.a(new_n81_), .b(x17), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n68_), .O(z11));
  nand3  g46(.a(new_n81_), .b(x23), .c(x14), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n68_), .O(z12));
  inv1   g48(.a(x23), .O(new_n90_));
  nor3   g49(.a(new_n86_), .b(new_n43_), .c(new_n90_), .O(z13));
  nand2  g50(.a(new_n81_), .b(x16), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n68_), .O(z14));
  nor2   g52(.a(x13), .b(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(x14), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(new_n68_), .O(z15));
endmodule


