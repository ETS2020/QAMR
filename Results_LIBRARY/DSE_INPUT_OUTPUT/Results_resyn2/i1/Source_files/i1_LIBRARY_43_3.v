// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n81_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x04), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x05), .O(new_n50_));
  inv1   g09(.a(x06), .O(new_n51_));
  inv1   g10(.a(x07), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  xnor2a g13(.a(x09), .b(x08), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(new_n48_), .O(new_n56_));
  nor2   g15(.a(x04), .b(new_n42_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n56_), .c(new_n44_), .O(z01));
  nor2   g17(.a(x05), .b(x03), .O(new_n59_));
  nor2   g18(.a(x07), .b(x06), .O(new_n60_));
  inv1   g19(.a(x09), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(x08), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n60_), .c(new_n59_), .d(new_n48_), .O(new_n63_));
  nand3  g22(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n42_), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(x04), .O(z06));
  nand3  g25(.a(z06), .b(new_n65_), .c(new_n63_), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nand2  g28(.a(new_n46_), .b(new_n69_), .O(z03));
  nor2   g29(.a(x21), .b(x20), .O(new_n71_));
  nor2   g30(.a(new_n71_), .b(new_n45_), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  inv1   g32(.a(x08), .O(new_n74_));
  nand3  g33(.a(x19), .b(new_n74_), .c(new_n43_), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n54_), .c(new_n48_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n73_), .c(new_n45_), .O(z05));
  nand2  g37(.a(x24), .b(x18), .O(new_n79_));
  nor2   g38(.a(new_n79_), .b(new_n45_), .O(z07));
  inv1   g39(.a(x11), .O(new_n81_));
  nand2  g40(.a(new_n46_), .b(new_n81_), .O(z08));
  inv1   g41(.a(x24), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n81_), .c(new_n46_), .O(z09));
  inv1   g43(.a(x22), .O(new_n85_));
  nand2  g44(.a(new_n83_), .b(x14), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n46_), .O(z10));
  nand4  g46(.a(new_n46_), .b(new_n83_), .c(x22), .d(x17), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z11));
  inv1   g48(.a(x23), .O(new_n90_));
  oai21  g49(.a(new_n86_), .b(new_n90_), .c(new_n46_), .O(z12));
  nand3  g50(.a(new_n83_), .b(x23), .c(x17), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n46_), .O(z13));
  nand2  g52(.a(new_n83_), .b(x16), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n46_), .O(z14));
  nor2   g54(.a(x13), .b(x12), .O(new_n96_));
  nor2   g55(.a(x15), .b(x14), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n45_), .O(z15));
endmodule


