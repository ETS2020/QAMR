// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n78_, new_n80_, new_n82_, new_n84_, new_n86_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  nand2  g02(.a(x24), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x19), .O(new_n47_));
  nand2  g06(.a(new_n44_), .b(new_n42_), .O(new_n48_));
  nor2   g07(.a(x05), .b(x04), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x06), .O(new_n51_));
  inv1   g10(.a(x07), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g12(.a(x09), .b(new_n50_), .c(new_n53_), .O(new_n54_));
  nor3   g13(.a(x03), .b(x02), .c(x01), .O(new_n55_));
  inv1   g14(.a(x09), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(x08), .c(x24), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n55_), .c(new_n54_), .d(new_n49_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n48_), .c(new_n47_), .O(z01));
  inv1   g18(.a(x24), .O(new_n60_));
  nand2  g19(.a(new_n56_), .b(x08), .O(new_n61_));
  nor2   g20(.a(x07), .b(x06), .O(new_n62_));
  nor2   g21(.a(x03), .b(x02), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(new_n62_), .c(new_n49_), .d(x19), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n61_), .c(new_n60_), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n43_), .O(new_n66_));
  nand3  g25(.a(new_n55_), .b(new_n62_), .c(new_n49_), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(x19), .c(x00), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n66_), .O(z02));
  inv1   g28(.a(x20), .O(new_n70_));
  nand2  g29(.a(new_n44_), .b(new_n70_), .O(z03));
  or2    g30(.a(z03), .b(x21), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  oai21  g32(.a(new_n64_), .b(x08), .c(new_n60_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n43_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n73_), .O(z05));
  nor2   g35(.a(new_n45_), .b(new_n47_), .O(z06));
  nand3  g36(.a(x24), .b(x18), .c(x01), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n44_), .b(new_n80_), .O(z08));
  nand3  g40(.a(x24), .b(x11), .c(x01), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z09));
  nand2  g42(.a(x22), .b(x14), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(x24), .O(z10));
  nand2  g44(.a(x22), .b(x17), .O(new_n86_));
  nor2   g45(.a(new_n86_), .b(x24), .O(z11));
  nand2  g46(.a(x23), .b(x14), .O(new_n88_));
  nor2   g47(.a(new_n88_), .b(x24), .O(z12));
  nand3  g48(.a(new_n60_), .b(x23), .c(x17), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n44_), .O(z13));
  nand2  g50(.a(new_n60_), .b(x16), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n44_), .O(z14));
  nor2   g52(.a(x13), .b(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(x14), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(new_n44_), .O(z15));
endmodule


