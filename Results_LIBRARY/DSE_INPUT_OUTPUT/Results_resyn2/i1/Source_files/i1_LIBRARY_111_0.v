// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x19), .b(x02), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x02), .O(new_n45_));
  inv1   g04(.a(x03), .O(new_n46_));
  inv1   g05(.a(x04), .O(new_n47_));
  inv1   g06(.a(x05), .O(new_n48_));
  inv1   g07(.a(x06), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g09(.a(x09), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x08), .O(new_n52_));
  inv1   g11(.a(x08), .O(new_n53_));
  nand2  g12(.a(x09), .b(new_n53_), .O(new_n54_));
  nor2   g13(.a(x07), .b(x01), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n50_), .c(x19), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  nand2  g17(.a(x19), .b(new_n42_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g19(.a(new_n50_), .O(new_n61_));
  nand4  g20(.a(new_n55_), .b(new_n52_), .c(new_n61_), .d(new_n45_), .O(new_n62_));
  nand2  g21(.a(new_n55_), .b(new_n45_), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n50_), .c(new_n42_), .O(new_n64_));
  nand3  g23(.a(new_n64_), .b(new_n62_), .c(x19), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nor2   g26(.a(new_n43_), .b(new_n67_), .O(z03));
  inv1   g27(.a(x21), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n67_), .c(new_n43_), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  nand2  g30(.a(new_n55_), .b(new_n53_), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n50_), .c(x19), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n71_), .O(z05));
  inv1   g34(.a(x18), .O(new_n76_));
  inv1   g35(.a(x24), .O(new_n77_));
  inv1   g36(.a(new_n43_), .O(new_n78_));
  oai21  g37(.a(new_n77_), .b(new_n76_), .c(new_n78_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nor2   g39(.a(new_n43_), .b(new_n80_), .O(z08));
  oai21  g40(.a(new_n77_), .b(new_n80_), .c(new_n78_), .O(z09));
  inv1   g41(.a(x22), .O(new_n83_));
  nand2  g42(.a(new_n77_), .b(x14), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(z10));
  nand2  g44(.a(new_n78_), .b(new_n77_), .O(new_n86_));
  nand2  g45(.a(x22), .b(x17), .O(new_n87_));
  nor2   g46(.a(new_n87_), .b(new_n86_), .O(z11));
  inv1   g47(.a(x23), .O(new_n89_));
  nor3   g48(.a(new_n84_), .b(new_n43_), .c(new_n89_), .O(z12));
  nand3  g49(.a(new_n77_), .b(x23), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n78_), .O(z13));
  inv1   g51(.a(x16), .O(new_n93_));
  nor2   g52(.a(new_n86_), .b(new_n93_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n43_), .O(z15));
  buf    g56(.a(x19), .O(z06));
endmodule


