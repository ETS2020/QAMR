// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n68_, new_n69_, new_n70_, new_n73_, new_n74_, new_n77_, new_n79_,
    new_n81_, new_n83_, new_n85_, new_n87_, new_n89_, new_n90_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x08), .b(x07), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nor2   g04(.a(new_n44_), .b(x19), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nor2   g07(.a(x06), .b(x05), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  inv1   g10(.a(x09), .O(new_n52_));
  oai21  g11(.a(x08), .b(x07), .c(new_n52_), .O(new_n53_));
  inv1   g12(.a(x08), .O(new_n54_));
  nand2  g13(.a(x09), .b(new_n54_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(z00), .c(new_n46_), .O(z01));
  nand2  g16(.a(x19), .b(x00), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x07), .O(new_n60_));
  aoi21  g19(.a(new_n52_), .b(x08), .c(new_n50_), .O(new_n61_));
  nand2  g20(.a(new_n50_), .b(new_n42_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x19), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(new_n43_), .b(new_n65_), .O(z03));
  or2    g25(.a(z03), .b(x21), .O(z04));
  inv1   g26(.a(x07), .O(new_n68_));
  nand3  g27(.a(x19), .b(new_n54_), .c(new_n68_), .O(new_n69_));
  nor2   g28(.a(new_n44_), .b(x10), .O(new_n70_));
  oai21  g29(.a(new_n69_), .b(new_n50_), .c(new_n70_), .O(z05));
  inv1   g30(.a(new_n46_), .O(z06));
  nor2   g31(.a(new_n44_), .b(x24), .O(new_n73_));
  nor2   g32(.a(new_n44_), .b(x18), .O(new_n74_));
  nor2   g33(.a(new_n74_), .b(new_n73_), .O(z07));
  and2   g34(.a(new_n43_), .b(x11), .O(z08));
  nand2  g35(.a(z08), .b(x24), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z09));
  nand3  g37(.a(new_n73_), .b(x22), .c(x14), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z10));
  nand3  g39(.a(new_n73_), .b(x22), .c(x17), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z11));
  nand3  g41(.a(new_n73_), .b(x23), .c(x14), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z12));
  nand2  g43(.a(x23), .b(x17), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(x24), .c(new_n43_), .O(z13));
  nand2  g45(.a(new_n73_), .b(x16), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z14));
  nor2   g47(.a(x13), .b(x12), .O(new_n89_));
  nor2   g48(.a(x15), .b(x14), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n44_), .O(z15));
endmodule


