// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n71_, new_n73_, new_n75_, new_n77_, new_n79_, new_n81_,
    new_n83_, new_n85_, new_n86_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nor2   g02(.a(x24), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand2  g05(.a(x24), .b(x19), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  nor2   g07(.a(x06), .b(x05), .O(new_n49_));
  nor2   g08(.a(x02), .b(x01), .O(new_n50_));
  nor2   g09(.a(x04), .b(x03), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  xor2a  g11(.a(x09), .b(x08), .O(new_n53_));
  or2    g12(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n47_), .O(z01));
  nand2  g14(.a(new_n52_), .b(x00), .O(new_n56_));
  nor3   g15(.a(x07), .b(x06), .c(x05), .O(new_n57_));
  inv1   g16(.a(x08), .O(new_n58_));
  nor2   g17(.a(x09), .b(new_n58_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(new_n56_), .c(x24), .O(new_n61_));
  and2   g20(.a(new_n61_), .b(x19), .O(z02));
  inv1   g21(.a(x20), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z03));
  nor2   g23(.a(x21), .b(x20), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n44_), .O(z04));
  inv1   g25(.a(x10), .O(new_n67_));
  nand3  g26(.a(x24), .b(x19), .c(new_n58_), .O(new_n68_));
  oai22  g27(.a(new_n68_), .b(new_n52_), .c(new_n44_), .d(new_n67_), .O(z05));
  and2   g28(.a(x24), .b(x18), .O(z07));
  inv1   g29(.a(x11), .O(new_n71_));
  nor2   g30(.a(new_n44_), .b(new_n71_), .O(z08));
  inv1   g31(.a(x24), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n71_), .O(z09));
  nand2  g33(.a(x22), .b(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n43_), .c(x24), .O(z10));
  nand2  g35(.a(x22), .b(x17), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n43_), .c(x24), .O(z11));
  nand2  g37(.a(x23), .b(x14), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n43_), .c(x24), .O(z12));
  nand4  g39(.a(new_n73_), .b(x23), .c(new_n43_), .d(x17), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z13));
  nand3  g41(.a(new_n73_), .b(new_n43_), .c(x16), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z14));
  nor2   g43(.a(x13), .b(x12), .O(new_n85_));
  nor2   g44(.a(x15), .b(x14), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n85_), .c(new_n45_), .O(z15));
  buf    g46(.a(x19), .O(z06));
endmodule


