// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n85_, new_n87_, new_n89_,
    new_n91_, new_n92_;
  nand2  g00(.a(x19), .b(x06), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x06), .O(new_n44_));
  nand2  g03(.a(x19), .b(new_n44_), .O(new_n45_));
  nor2   g04(.a(x07), .b(x05), .O(new_n46_));
  inv1   g05(.a(x01), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x04), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(new_n52_), .c(new_n46_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n45_), .O(z01));
  inv1   g14(.a(x19), .O(new_n56_));
  nor2   g15(.a(x04), .b(x03), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n46_), .c(new_n48_), .d(new_n47_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g18(.a(x05), .O(new_n60_));
  inv1   g19(.a(x07), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(x08), .c(new_n61_), .d(new_n60_), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(new_n64_));
  aoi21  g23(.a(new_n64_), .b(new_n52_), .c(x06), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n59_), .c(new_n56_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  inv1   g26(.a(new_n42_), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n67_), .O(z03));
  inv1   g28(.a(x21), .O(new_n70_));
  nand3  g29(.a(new_n42_), .b(new_n70_), .c(new_n67_), .O(z04));
  nand2  g30(.a(new_n42_), .b(x10), .O(new_n72_));
  or2    g31(.a(new_n45_), .b(x08), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n58_), .c(new_n72_), .O(z05));
  nand2  g33(.a(x24), .b(x18), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n68_), .O(z07));
  and2   g35(.a(new_n42_), .b(x11), .O(z08));
  nand2  g36(.a(z08), .b(x24), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z09));
  inv1   g38(.a(x24), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x22), .c(x14), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n42_), .O(z10));
  nand4  g41(.a(new_n42_), .b(new_n80_), .c(x22), .d(x17), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z11));
  nand4  g43(.a(new_n42_), .b(new_n80_), .c(x23), .d(x14), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(z12));
  nand2  g45(.a(x23), .b(x17), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(x24), .c(new_n42_), .O(z13));
  inv1   g47(.a(x16), .O(new_n89_));
  oai21  g48(.a(x24), .b(new_n89_), .c(new_n42_), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n68_), .O(z15));
  buf    g52(.a(x19), .O(z06));
endmodule


