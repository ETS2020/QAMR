// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n80_, new_n82_, new_n83_, new_n85_, new_n87_, new_n89_, new_n91_,
    new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x12), .O(new_n43_));
  nand2  g02(.a(x19), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nor3   g07(.a(x07), .b(x06), .c(x05), .O(new_n49_));
  xnor2a g08(.a(x09), .b(x08), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(x19), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n43_), .O(z06));
  inv1   g12(.a(z06), .O(new_n54_));
  aoi21  g13(.a(new_n51_), .b(x00), .c(new_n54_), .O(z01));
  inv1   g14(.a(x09), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n58_));
  inv1   g17(.a(x07), .O(new_n59_));
  nor2   g18(.a(x06), .b(x05), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n48_), .c(new_n47_), .d(new_n59_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n58_), .c(z06), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z03));
  or2    g25(.a(z03), .b(x21), .O(z04));
  inv1   g26(.a(x10), .O(new_n68_));
  nor2   g27(.a(x08), .b(x07), .O(new_n69_));
  nand4  g28(.a(new_n69_), .b(new_n60_), .c(new_n48_), .d(new_n47_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x19), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n68_), .O(z05));
  inv1   g32(.a(x18), .O(new_n74_));
  inv1   g33(.a(x24), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n74_), .c(new_n44_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nand2  g36(.a(new_n44_), .b(new_n77_), .O(z08));
  nor3   g37(.a(new_n45_), .b(new_n75_), .c(new_n77_), .O(z09));
  nand2  g38(.a(x22), .b(x14), .O(new_n80_));
  nor3   g39(.a(new_n80_), .b(new_n45_), .c(x24), .O(z10));
  inv1   g40(.a(x22), .O(new_n82_));
  nand2  g41(.a(new_n75_), .b(x17), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n82_), .c(new_n44_), .O(z11));
  nand3  g43(.a(new_n75_), .b(x23), .c(x14), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n44_), .O(z12));
  nand2  g45(.a(new_n44_), .b(x23), .O(new_n87_));
  nor2   g46(.a(new_n87_), .b(new_n83_), .O(z13));
  nand2  g47(.a(new_n75_), .b(x16), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n44_), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n52_), .O(z15));
endmodule


