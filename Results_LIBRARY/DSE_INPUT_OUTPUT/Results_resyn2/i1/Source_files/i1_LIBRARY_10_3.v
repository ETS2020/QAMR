// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n78_, new_n79_, new_n80_, new_n82_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n91_, new_n93_, new_n95_, new_n97_, new_n98_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x09), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nor2   g07(.a(x06), .b(x05), .O(new_n49_));
  nor2   g08(.a(x08), .b(x07), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(new_n43_), .c(x00), .O(new_n52_));
  and2   g11(.a(new_n52_), .b(x19), .O(z01));
  inv1   g12(.a(x01), .O(new_n54_));
  inv1   g13(.a(x02), .O(new_n55_));
  inv1   g14(.a(x03), .O(new_n56_));
  inv1   g15(.a(x04), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n56_), .c(new_n55_), .d(new_n54_), .O(new_n58_));
  inv1   g17(.a(x07), .O(new_n59_));
  nand2  g18(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  oai21  g19(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  inv1   g20(.a(new_n58_), .O(new_n62_));
  inv1   g21(.a(x05), .O(new_n63_));
  inv1   g22(.a(x06), .O(new_n64_));
  nand4  g23(.a(x08), .b(new_n59_), .c(new_n64_), .d(new_n63_), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n62_), .c(x09), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n61_), .c(new_n44_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nor2   g28(.a(new_n45_), .b(new_n69_), .O(z03));
  inv1   g29(.a(x21), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n69_), .c(new_n45_), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  nand2  g32(.a(new_n51_), .b(new_n43_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x19), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n73_), .O(z05));
  nor2   g35(.a(new_n44_), .b(x09), .O(z06));
  inv1   g36(.a(x18), .O(new_n78_));
  inv1   g37(.a(x24), .O(new_n79_));
  inv1   g38(.a(new_n45_), .O(new_n80_));
  oai21  g39(.a(new_n79_), .b(new_n78_), .c(new_n80_), .O(z07));
  inv1   g40(.a(x11), .O(new_n82_));
  nand2  g41(.a(new_n80_), .b(new_n82_), .O(z08));
  nor3   g42(.a(new_n45_), .b(new_n79_), .c(new_n82_), .O(z09));
  inv1   g43(.a(x14), .O(new_n85_));
  nand2  g44(.a(new_n79_), .b(x22), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n80_), .O(z10));
  inv1   g46(.a(x17), .O(new_n88_));
  oai21  g47(.a(new_n86_), .b(new_n88_), .c(new_n80_), .O(z11));
  nor2   g48(.a(new_n45_), .b(x24), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(x23), .c(x14), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z12));
  nand3  g51(.a(new_n79_), .b(x23), .c(x17), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n80_), .O(z13));
  nand2  g53(.a(new_n90_), .b(x16), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z14));
  nor2   g55(.a(x13), .b(x12), .O(new_n97_));
  nor2   g56(.a(x15), .b(x14), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n45_), .O(z15));
endmodule


