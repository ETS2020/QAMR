// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x01), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x07), .O(new_n46_));
  nor3   g05(.a(x06), .b(x05), .c(x02), .O(new_n47_));
  nor3   g06(.a(x04), .b(x03), .c(x01), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(x00), .c(new_n43_), .O(z01));
  oai21  g10(.a(new_n43_), .b(x00), .c(x01), .O(new_n52_));
  nor3   g11(.a(x07), .b(x04), .c(x03), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n47_), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(x01), .O(new_n55_));
  inv1   g14(.a(x06), .O(new_n56_));
  nor2   g15(.a(x05), .b(x02), .O(new_n57_));
  nor2   g16(.a(x04), .b(x03), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n59_));
  inv1   g18(.a(x09), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(x08), .c(new_n46_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  oai21  g21(.a(new_n62_), .b(new_n54_), .c(x19), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n52_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z03));
  or2    g25(.a(z03), .b(x21), .O(z04));
  nand2  g26(.a(new_n53_), .b(new_n47_), .O(new_n68_));
  nand2  g27(.a(new_n44_), .b(x10), .O(new_n69_));
  inv1   g28(.a(x08), .O(new_n70_));
  nand3  g29(.a(x19), .b(new_n70_), .c(new_n55_), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n68_), .c(new_n69_), .O(z05));
  nand2  g31(.a(new_n43_), .b(new_n55_), .O(z06));
  inv1   g32(.a(new_n44_), .O(new_n74_));
  nand2  g33(.a(x24), .b(x18), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n74_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nor2   g36(.a(new_n74_), .b(new_n77_), .O(z08));
  inv1   g37(.a(x24), .O(new_n79_));
  nor3   g38(.a(new_n74_), .b(new_n79_), .c(new_n77_), .O(z09));
  inv1   g39(.a(x14), .O(new_n81_));
  nand2  g40(.a(new_n44_), .b(new_n79_), .O(new_n82_));
  nor2   g41(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x22), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z10));
  inv1   g44(.a(x22), .O(new_n86_));
  nand2  g45(.a(new_n79_), .b(x17), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n86_), .c(new_n44_), .O(z11));
  nand2  g47(.a(new_n83_), .b(x23), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z12));
  inv1   g49(.a(x23), .O(new_n91_));
  oai21  g50(.a(new_n87_), .b(new_n91_), .c(new_n44_), .O(z13));
  inv1   g51(.a(x16), .O(new_n93_));
  nor2   g52(.a(new_n82_), .b(new_n93_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n74_), .O(z15));
endmodule


