// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n70_, new_n71_, new_n72_, new_n76_, new_n78_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  nor2   g02(.a(x24), .b(new_n43_), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(new_n44_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nor3   g07(.a(x07), .b(x06), .c(x05), .O(new_n49_));
  nor2   g08(.a(x04), .b(x03), .O(new_n50_));
  nor2   g09(.a(x02), .b(x01), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n48_), .c(new_n46_), .O(z01));
  oai21  g13(.a(new_n46_), .b(new_n42_), .c(x24), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x01), .O(new_n56_));
  nor3   g15(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n49_), .c(new_n42_), .O(new_n58_));
  inv1   g17(.a(x07), .O(new_n59_));
  nor2   g18(.a(x06), .b(x05), .O(new_n60_));
  nand4  g19(.a(new_n51_), .b(new_n50_), .c(new_n60_), .d(new_n59_), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x08), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n58_), .c(x19), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n56_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nand2  g26(.a(new_n47_), .b(new_n67_), .O(z03));
  or2    g27(.a(z03), .b(x21), .O(z04));
  inv1   g28(.a(x08), .O(new_n70_));
  nand2  g29(.a(x19), .b(new_n70_), .O(new_n71_));
  nor2   g30(.a(new_n44_), .b(x10), .O(new_n72_));
  oai21  g31(.a(new_n71_), .b(new_n61_), .c(new_n72_), .O(z05));
  nor2   g32(.a(new_n44_), .b(new_n46_), .O(z06));
  and2   g33(.a(x24), .b(x18), .O(z07));
  inv1   g34(.a(x11), .O(new_n76_));
  nand2  g35(.a(new_n47_), .b(new_n76_), .O(z08));
  inv1   g36(.a(x24), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n76_), .O(z09));
  nand4  g38(.a(new_n78_), .b(x22), .c(x14), .d(new_n43_), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z10));
  nand4  g40(.a(new_n78_), .b(x22), .c(x17), .d(new_n43_), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z11));
  nand4  g42(.a(new_n78_), .b(x23), .c(x14), .d(new_n43_), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z12));
  nand2  g44(.a(x23), .b(x17), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n43_), .c(x24), .O(z13));
  inv1   g46(.a(x16), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n43_), .c(x24), .O(z14));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nor2   g49(.a(x15), .b(x14), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(new_n47_), .O(z15));
endmodule


