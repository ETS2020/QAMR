// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x24), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x10), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(new_n44_), .b(x19), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  nor2   g08(.a(x05), .b(x04), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  nor3   g11(.a(x07), .b(x06), .c(x03), .O(new_n53_));
  inv1   g12(.a(x09), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x08), .O(new_n55_));
  inv1   g14(.a(x08), .O(new_n56_));
  nand2  g15(.a(x09), .b(new_n56_), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n55_), .c(new_n53_), .d(new_n52_), .O(new_n58_));
  nor2   g17(.a(new_n44_), .b(new_n42_), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(z01));
  nand4  g19(.a(new_n53_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(x00), .O(new_n62_));
  inv1   g21(.a(x03), .O(new_n63_));
  inv1   g22(.a(x06), .O(new_n64_));
  inv1   g23(.a(x07), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nor2   g25(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n52_), .c(new_n44_), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n62_), .c(new_n47_), .O(z02));
  inv1   g28(.a(x20), .O(new_n70_));
  nor2   g29(.a(new_n44_), .b(new_n70_), .O(z03));
  nor2   g30(.a(x21), .b(x20), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n45_), .O(z04));
  inv1   g32(.a(x10), .O(new_n74_));
  nand3  g33(.a(new_n43_), .b(x19), .c(new_n56_), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n61_), .c(new_n74_), .O(z05));
  inv1   g35(.a(new_n47_), .O(z06));
  inv1   g36(.a(x18), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(x10), .c(new_n43_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n45_), .b(new_n80_), .O(z08));
  nand3  g40(.a(x24), .b(x11), .c(x10), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z09));
  inv1   g42(.a(x14), .O(new_n84_));
  nand2  g43(.a(new_n43_), .b(x22), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n84_), .c(new_n45_), .O(z10));
  inv1   g45(.a(x17), .O(new_n87_));
  oai21  g46(.a(new_n85_), .b(new_n87_), .c(new_n45_), .O(z11));
  nand2  g47(.a(x23), .b(x14), .O(new_n89_));
  nor2   g48(.a(new_n89_), .b(x24), .O(z12));
  nand3  g49(.a(new_n43_), .b(x23), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n45_), .O(z13));
  nand2  g51(.a(new_n43_), .b(x16), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n45_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n45_), .O(z15));
endmodule


