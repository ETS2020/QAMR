// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n82_, new_n85_, new_n87_, new_n88_;
  nand2  g00(.a(x24), .b(x06), .O(new_n42_));
  nand2  g01(.a(new_n42_), .b(x00), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  nor3   g04(.a(x07), .b(x06), .c(x05), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  aoi22  g09(.a(new_n50_), .b(z00), .c(new_n42_), .d(new_n45_), .O(z01));
  inv1   g10(.a(x07), .O(new_n52_));
  nor2   g11(.a(x06), .b(x05), .O(new_n53_));
  nand4  g12(.a(new_n48_), .b(new_n47_), .c(new_n53_), .d(new_n52_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  inv1   g14(.a(x09), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n58_));
  nand3  g17(.a(new_n58_), .b(new_n55_), .c(x19), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z02));
  inv1   g19(.a(x20), .O(new_n61_));
  nand2  g20(.a(new_n42_), .b(new_n61_), .O(z03));
  or2    g21(.a(z03), .b(x21), .O(z04));
  inv1   g22(.a(x08), .O(new_n64_));
  nand2  g23(.a(x19), .b(new_n64_), .O(new_n65_));
  inv1   g24(.a(new_n42_), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(x10), .O(new_n67_));
  oai21  g26(.a(new_n65_), .b(new_n54_), .c(new_n67_), .O(z05));
  nand2  g27(.a(new_n42_), .b(new_n45_), .O(z06));
  inv1   g28(.a(x06), .O(new_n70_));
  nand3  g29(.a(x24), .b(x18), .c(new_n70_), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z07));
  inv1   g31(.a(x11), .O(new_n73_));
  nor2   g32(.a(new_n66_), .b(new_n73_), .O(z08));
  inv1   g33(.a(x24), .O(new_n75_));
  aoi21  g34(.a(new_n73_), .b(new_n70_), .c(new_n75_), .O(z09));
  inv1   g35(.a(x14), .O(new_n77_));
  inv1   g36(.a(x22), .O(new_n78_));
  nor3   g37(.a(x24), .b(new_n78_), .c(new_n77_), .O(z10));
  nand2  g38(.a(new_n75_), .b(x17), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(new_n42_), .O(z11));
  inv1   g40(.a(x23), .O(new_n82_));
  nor3   g41(.a(x24), .b(new_n82_), .c(new_n77_), .O(z12));
  oai21  g42(.a(new_n80_), .b(new_n82_), .c(new_n42_), .O(z13));
  inv1   g43(.a(x16), .O(new_n85_));
  oai21  g44(.a(x24), .b(new_n85_), .c(new_n42_), .O(z14));
  nor2   g45(.a(x13), .b(x12), .O(new_n87_));
  nor2   g46(.a(x15), .b(x14), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n66_), .O(z15));
endmodule


