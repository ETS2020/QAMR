// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_,
    new_n87_, new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x07), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand2  g05(.a(x19), .b(new_n43_), .O(new_n47_));
  nor2   g06(.a(x06), .b(x05), .O(new_n48_));
  inv1   g07(.a(x01), .O(new_n49_));
  inv1   g08(.a(x02), .O(new_n50_));
  inv1   g09(.a(x03), .O(new_n51_));
  inv1   g10(.a(x04), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  xnor2a g13(.a(x09), .b(x08), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(new_n48_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(x00), .c(new_n47_), .O(z01));
  nor2   g16(.a(x04), .b(x03), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n48_), .c(new_n50_), .d(new_n49_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x00), .O(new_n60_));
  inv1   g19(.a(x05), .O(new_n61_));
  inv1   g20(.a(x06), .O(new_n62_));
  inv1   g21(.a(x09), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(x08), .c(new_n62_), .d(new_n61_), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n54_), .c(x07), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n60_), .c(new_n44_), .O(z02));
  inv1   g26(.a(x20), .O(new_n68_));
  nor2   g27(.a(new_n45_), .b(new_n68_), .O(z03));
  inv1   g28(.a(x21), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n68_), .c(new_n45_), .O(z04));
  inv1   g30(.a(new_n45_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x10), .O(new_n73_));
  or2    g32(.a(new_n47_), .b(x08), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n59_), .c(new_n73_), .O(z05));
  inv1   g34(.a(x24), .O(new_n76_));
  nor2   g35(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x18), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n72_), .b(new_n80_), .O(z08));
  nand2  g40(.a(new_n77_), .b(x11), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z09));
  inv1   g42(.a(x22), .O(new_n84_));
  nand2  g43(.a(new_n76_), .b(x14), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n84_), .c(new_n72_), .O(z10));
  nand4  g45(.a(new_n72_), .b(new_n76_), .c(x22), .d(x17), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z11));
  inv1   g47(.a(x23), .O(new_n89_));
  oai21  g48(.a(new_n85_), .b(new_n89_), .c(new_n72_), .O(z12));
  nand3  g49(.a(new_n76_), .b(x23), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n72_), .O(z13));
  inv1   g51(.a(x16), .O(new_n93_));
  oai21  g52(.a(x24), .b(new_n93_), .c(new_n72_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n72_), .O(z15));
  buf    g56(.a(x19), .O(z06));
endmodule


