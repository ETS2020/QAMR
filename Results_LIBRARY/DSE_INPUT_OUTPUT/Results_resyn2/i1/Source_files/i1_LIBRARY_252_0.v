// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n71_, new_n73_, new_n75_, new_n77_, new_n79_, new_n81_, new_n83_,
    new_n85_, new_n86_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x24), .b(x10), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nor2   g03(.a(new_n43_), .b(x19), .O(new_n45_));
  nor2   g04(.a(x02), .b(x01), .O(new_n46_));
  nor2   g05(.a(x05), .b(x04), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nor3   g08(.a(x07), .b(x06), .c(x03), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(z00), .c(new_n45_), .O(z01));
  inv1   g12(.a(x08), .O(new_n54_));
  nor2   g13(.a(x09), .b(new_n54_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n50_), .c(new_n49_), .O(new_n56_));
  inv1   g15(.a(x07), .O(new_n57_));
  nor2   g16(.a(x06), .b(x03), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n47_), .c(new_n46_), .d(new_n57_), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(x00), .c(new_n43_), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(new_n56_), .c(new_n45_), .O(z02));
  inv1   g20(.a(x20), .O(new_n62_));
  nor2   g21(.a(new_n43_), .b(new_n62_), .O(z03));
  nor2   g22(.a(x21), .b(x20), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n43_), .O(z04));
  inv1   g24(.a(x10), .O(new_n66_));
  nand3  g25(.a(x24), .b(x19), .c(new_n54_), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n59_), .c(new_n66_), .O(z05));
  inv1   g27(.a(new_n45_), .O(z06));
  and2   g28(.a(x24), .b(x18), .O(z07));
  inv1   g29(.a(x11), .O(new_n71_));
  oai21  g30(.a(x24), .b(x10), .c(new_n71_), .O(z08));
  inv1   g31(.a(x24), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n71_), .O(z09));
  nand4  g33(.a(new_n73_), .b(x22), .c(x14), .d(x10), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z10));
  nand2  g35(.a(x22), .b(x17), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(x10), .c(x24), .O(z11));
  nand4  g37(.a(new_n73_), .b(x23), .c(x14), .d(x10), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z12));
  nand2  g39(.a(x23), .b(x17), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(x10), .c(x24), .O(z13));
  nand2  g41(.a(x16), .b(x10), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(x24), .O(z14));
  nor2   g43(.a(x13), .b(x12), .O(new_n85_));
  nor2   g44(.a(x15), .b(x14), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n43_), .O(z15));
endmodule


