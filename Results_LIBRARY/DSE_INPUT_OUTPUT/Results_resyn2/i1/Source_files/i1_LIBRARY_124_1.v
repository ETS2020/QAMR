// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n80_, new_n82_, new_n84_, new_n86_,
    new_n88_, new_n90_, new_n92_, new_n94_, new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x17), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x06), .O(new_n47_));
  nor3   g06(.a(x05), .b(x04), .c(x03), .O(new_n48_));
  nor3   g07(.a(x07), .b(x02), .c(x01), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(x09), .b(new_n51_), .O(new_n52_));
  inv1   g11(.a(x09), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x08), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n52_), .c(x19), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n50_), .c(new_n43_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g16(.a(x19), .b(new_n42_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g18(.a(x19), .O(new_n60_));
  aoi21  g19(.a(new_n49_), .b(new_n48_), .c(x17), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(x06), .c(x00), .O(new_n62_));
  inv1   g21(.a(x05), .O(new_n63_));
  nor2   g22(.a(x04), .b(x03), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  or2    g24(.a(x02), .b(x01), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nor3   g26(.a(x17), .b(x07), .c(x06), .O(new_n68_));
  nand4  g27(.a(new_n68_), .b(new_n67_), .c(new_n53_), .d(x08), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n62_), .c(new_n60_), .O(z02));
  inv1   g29(.a(x20), .O(new_n71_));
  nor2   g30(.a(new_n44_), .b(new_n71_), .O(z03));
  inv1   g31(.a(x21), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n44_), .O(z04));
  inv1   g33(.a(new_n67_), .O(new_n75_));
  nand2  g34(.a(new_n45_), .b(x10), .O(new_n76_));
  nand3  g35(.a(new_n68_), .b(x19), .c(new_n51_), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n75_), .c(new_n76_), .O(z05));
  nor2   g37(.a(new_n44_), .b(new_n60_), .O(z06));
  nand2  g38(.a(x24), .b(x18), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n45_), .O(z07));
  inv1   g40(.a(x11), .O(new_n82_));
  nand2  g41(.a(new_n45_), .b(new_n82_), .O(z08));
  inv1   g42(.a(x24), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n82_), .c(new_n45_), .O(z09));
  nand3  g44(.a(new_n84_), .b(x22), .c(x14), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n45_), .O(z10));
  nand4  g46(.a(new_n84_), .b(x22), .c(x17), .d(x06), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z11));
  nand3  g48(.a(new_n84_), .b(x23), .c(x14), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n45_), .O(z12));
  nand4  g50(.a(new_n84_), .b(x23), .c(x17), .d(x06), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z13));
  nand2  g52(.a(new_n84_), .b(x16), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n45_), .O(z14));
  nor2   g54(.a(x13), .b(x12), .O(new_n96_));
  nor2   g55(.a(x15), .b(x14), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n44_), .O(z15));
endmodule


