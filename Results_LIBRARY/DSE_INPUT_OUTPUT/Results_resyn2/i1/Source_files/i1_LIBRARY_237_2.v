// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_, new_n76_,
    new_n78_, new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n92_, new_n93_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x10), .O(new_n43_));
  nor2   g02(.a(x24), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(new_n44_), .b(x19), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  nor2   g08(.a(x05), .b(x04), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g10(.a(x03), .O(new_n52_));
  inv1   g11(.a(x06), .O(new_n53_));
  inv1   g12(.a(x07), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g15(.a(x09), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x08), .O(new_n58_));
  inv1   g17(.a(x08), .O(new_n59_));
  nand2  g18(.a(x09), .b(new_n59_), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n42_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(new_n61_), .c(new_n47_), .O(z01));
  oai21  g22(.a(new_n55_), .b(new_n51_), .c(x00), .O(new_n64_));
  inv1   g23(.a(new_n51_), .O(new_n65_));
  nor2   g24(.a(new_n58_), .b(new_n55_), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n65_), .c(new_n44_), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n64_), .c(new_n47_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nor2   g28(.a(new_n44_), .b(new_n69_), .O(z03));
  nor2   g29(.a(x21), .b(x20), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n45_), .O(z04));
  nand3  g31(.a(new_n56_), .b(x19), .c(new_n59_), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n43_), .c(new_n44_), .O(z05));
  inv1   g33(.a(new_n47_), .O(z06));
  nand2  g34(.a(x24), .b(x18), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n45_), .O(z07));
  inv1   g36(.a(x11), .O(new_n78_));
  nand2  g37(.a(new_n45_), .b(new_n78_), .O(z08));
  inv1   g38(.a(x24), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n78_), .O(z09));
  nand2  g40(.a(x22), .b(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n43_), .c(x24), .O(z10));
  nand2  g42(.a(x22), .b(x17), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n43_), .c(x24), .O(z11));
  nand4  g44(.a(new_n80_), .b(x23), .c(x14), .d(new_n43_), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z12));
  nand2  g46(.a(x23), .b(x17), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n43_), .c(x24), .O(z13));
  inv1   g48(.a(x16), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n43_), .c(x24), .O(z14));
  nor2   g50(.a(x13), .b(x12), .O(new_n92_));
  nor2   g51(.a(x15), .b(x14), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n92_), .c(new_n45_), .O(z15));
endmodule


