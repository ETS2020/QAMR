// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n67_, new_n70_, new_n71_, new_n73_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x10), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x10), .O(new_n47_));
  nor2   g06(.a(new_n43_), .b(new_n47_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nor3   g08(.a(x07), .b(x06), .c(x05), .O(new_n50_));
  nor2   g09(.a(x02), .b(x01), .O(new_n51_));
  nor2   g10(.a(x04), .b(x03), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n49_), .O(z01));
  inv1   g14(.a(x09), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n58_));
  inv1   g17(.a(x07), .O(new_n59_));
  nor2   g18(.a(x06), .b(x05), .O(new_n60_));
  nand4  g19(.a(new_n52_), .b(new_n51_), .c(new_n60_), .d(new_n59_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n58_), .c(new_n48_), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z03));
  inv1   g25(.a(x21), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n65_), .c(new_n44_), .O(z04));
  nand2  g27(.a(new_n43_), .b(new_n47_), .O(z05));
  inv1   g28(.a(x18), .O(new_n70_));
  nand2  g29(.a(new_n45_), .b(x24), .O(new_n71_));
  nor2   g30(.a(new_n71_), .b(new_n70_), .O(z07));
  inv1   g31(.a(x11), .O(new_n73_));
  nand2  g32(.a(new_n45_), .b(new_n73_), .O(z08));
  nor2   g33(.a(new_n71_), .b(new_n73_), .O(z09));
  inv1   g34(.a(x24), .O(new_n76_));
  nand4  g35(.a(new_n45_), .b(new_n76_), .c(x22), .d(x14), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z10));
  inv1   g37(.a(x22), .O(new_n79_));
  nand2  g38(.a(new_n76_), .b(x17), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n45_), .O(z11));
  nand4  g40(.a(new_n45_), .b(new_n76_), .c(x23), .d(x14), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z12));
  inv1   g42(.a(x23), .O(new_n84_));
  nor3   g43(.a(new_n80_), .b(new_n44_), .c(new_n84_), .O(z13));
  inv1   g44(.a(x16), .O(new_n86_));
  nor3   g45(.a(new_n44_), .b(x24), .c(new_n86_), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n44_), .O(z15));
  buf    g49(.a(x19), .O(z06));
endmodule


