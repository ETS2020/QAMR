// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n67_,
    new_n68_, new_n70_, new_n73_, new_n75_, new_n77_, new_n79_, new_n81_,
    new_n83_, new_n85_, new_n86_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x24), .b(x19), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x05), .b(x04), .O(new_n48_));
  nor3   g07(.a(x07), .b(x06), .c(x03), .O(new_n49_));
  xnor2a g08(.a(x09), .b(x08), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nor2   g10(.a(x24), .b(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(new_n51_), .c(new_n46_), .O(z01));
  inv1   g12(.a(x24), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x19), .O(new_n55_));
  inv1   g14(.a(x07), .O(new_n56_));
  nor2   g15(.a(x06), .b(x03), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n48_), .c(new_n47_), .d(new_n56_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g18(.a(x08), .O(new_n60_));
  nor2   g19(.a(x09), .b(new_n60_), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(new_n59_), .c(new_n55_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z03));
  or2    g24(.a(z03), .b(x21), .O(z04));
  nand2  g25(.a(new_n43_), .b(x10), .O(new_n67_));
  nand3  g26(.a(new_n54_), .b(x19), .c(new_n60_), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n58_), .c(new_n67_), .O(z05));
  nand3  g28(.a(x24), .b(new_n46_), .c(x18), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z07));
  and2   g30(.a(new_n43_), .b(x11), .O(z08));
  nand3  g31(.a(x24), .b(new_n46_), .c(x11), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z09));
  nand2  g33(.a(x22), .b(x14), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(x24), .c(new_n43_), .O(z10));
  nand2  g35(.a(x22), .b(x17), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(x24), .O(z11));
  nand2  g37(.a(x23), .b(x14), .O(new_n79_));
  nor2   g38(.a(new_n79_), .b(x24), .O(z12));
  nand2  g39(.a(x23), .b(x17), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(x24), .c(new_n43_), .O(z13));
  inv1   g41(.a(x16), .O(new_n83_));
  oai21  g42(.a(x24), .b(new_n83_), .c(new_n43_), .O(z14));
  nor2   g43(.a(x13), .b(x12), .O(new_n85_));
  nor2   g44(.a(x15), .b(x14), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n44_), .O(z15));
  buf    g46(.a(x19), .O(z06));
endmodule


