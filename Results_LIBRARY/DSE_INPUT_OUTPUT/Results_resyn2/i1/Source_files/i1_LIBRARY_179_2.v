// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n69_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n87_, new_n89_,
    new_n91_, new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x19), .b(new_n42_), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  inv1   g03(.a(x07), .O(new_n45_));
  nor2   g04(.a(x02), .b(x01), .O(new_n46_));
  nor2   g05(.a(x04), .b(x03), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g07(.a(x08), .O(new_n49_));
  nand2  g08(.a(x09), .b(new_n49_), .O(new_n50_));
  inv1   g09(.a(x09), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x08), .O(new_n52_));
  nor2   g11(.a(x06), .b(x05), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(new_n48_), .c(x00), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x19), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n44_), .O(z01));
  nor3   g16(.a(x07), .b(x04), .c(x03), .O(new_n58_));
  nand2  g17(.a(new_n53_), .b(x19), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n52_), .c(new_n58_), .d(new_n46_), .O(new_n61_));
  oai21  g20(.a(new_n59_), .b(new_n48_), .c(new_n42_), .O(new_n62_));
  and2   g21(.a(new_n62_), .b(new_n61_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z03));
  nor2   g24(.a(x21), .b(x20), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n44_), .O(z04));
  nand2  g26(.a(new_n44_), .b(x10), .O(new_n68_));
  nand4  g27(.a(new_n60_), .b(new_n58_), .c(new_n46_), .d(new_n49_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n68_), .O(z05));
  inv1   g29(.a(x19), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n42_), .O(z06));
  inv1   g31(.a(x18), .O(new_n73_));
  inv1   g32(.a(x24), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(new_n44_), .O(z07));
  inv1   g34(.a(x11), .O(new_n76_));
  nand2  g35(.a(new_n44_), .b(new_n76_), .O(z08));
  nor3   g36(.a(new_n43_), .b(new_n74_), .c(new_n76_), .O(z09));
  nand2  g37(.a(new_n44_), .b(new_n74_), .O(new_n79_));
  nand2  g38(.a(x22), .b(x14), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n79_), .O(z10));
  inv1   g40(.a(x22), .O(new_n82_));
  nand2  g41(.a(new_n74_), .b(x17), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n82_), .c(new_n44_), .O(z11));
  nand3  g43(.a(new_n74_), .b(x23), .c(x14), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n44_), .O(z12));
  inv1   g45(.a(x23), .O(new_n87_));
  oai21  g46(.a(new_n83_), .b(new_n87_), .c(new_n44_), .O(z13));
  inv1   g47(.a(x16), .O(new_n89_));
  nor2   g48(.a(new_n79_), .b(new_n89_), .O(z14));
  nor2   g49(.a(x13), .b(x12), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n44_), .O(z15));
  buf    g52(.a(x00), .O(z00));
endmodule


