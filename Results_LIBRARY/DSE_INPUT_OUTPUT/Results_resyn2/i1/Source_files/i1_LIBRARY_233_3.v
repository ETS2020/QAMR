// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n87_,
    new_n90_, new_n92_, new_n93_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x01), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x01), .O(new_n45_));
  nand2  g04(.a(x19), .b(new_n45_), .O(new_n46_));
  nor2   g05(.a(x07), .b(x06), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x04), .O(new_n50_));
  inv1   g09(.a(x05), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n53_), .c(new_n47_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x00), .c(new_n46_), .O(z01));
  inv1   g15(.a(x19), .O(new_n57_));
  nor2   g16(.a(x05), .b(x04), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n47_), .c(new_n49_), .d(new_n48_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x00), .O(new_n60_));
  inv1   g19(.a(x06), .O(new_n61_));
  inv1   g20(.a(x07), .O(new_n62_));
  inv1   g21(.a(x09), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(x08), .c(new_n62_), .d(new_n61_), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n53_), .c(x01), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n60_), .c(new_n57_), .O(z02));
  inv1   g26(.a(x20), .O(new_n68_));
  inv1   g27(.a(new_n43_), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n68_), .O(z03));
  inv1   g29(.a(x21), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n68_), .c(new_n69_), .O(z04));
  nand2  g31(.a(new_n43_), .b(x10), .O(new_n73_));
  or2    g32(.a(new_n46_), .b(x08), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n59_), .c(new_n73_), .O(z05));
  nand2  g34(.a(x24), .b(x18), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n43_), .O(z07));
  and2   g36(.a(new_n43_), .b(x11), .O(z08));
  nand2  g37(.a(z08), .b(x24), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z09));
  inv1   g39(.a(x14), .O(new_n81_));
  inv1   g40(.a(x24), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x22), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n81_), .c(new_n43_), .O(z10));
  inv1   g43(.a(x17), .O(new_n85_));
  oai21  g44(.a(new_n83_), .b(new_n85_), .c(new_n43_), .O(z11));
  nand2  g45(.a(new_n82_), .b(x23), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n81_), .c(new_n43_), .O(z12));
  oai21  g47(.a(new_n87_), .b(new_n85_), .c(new_n43_), .O(z13));
  nand3  g48(.a(new_n43_), .b(new_n82_), .c(x16), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z14));
  nor2   g50(.a(x13), .b(x12), .O(new_n92_));
  nor2   g51(.a(x15), .b(x14), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n92_), .c(new_n43_), .O(z15));
  buf    g53(.a(x19), .O(z06));
endmodule


