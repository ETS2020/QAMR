// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n84_, new_n86_,
    new_n87_, new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x02), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(x05), .b(x04), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n53_), .c(new_n47_), .O(new_n55_));
  nand2  g14(.a(x19), .b(new_n43_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(x00), .c(new_n56_), .O(z01));
  inv1   g16(.a(new_n47_), .O(new_n58_));
  oai21  g17(.a(new_n52_), .b(new_n58_), .c(x00), .O(new_n59_));
  inv1   g18(.a(x04), .O(new_n60_));
  inv1   g19(.a(x05), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(x08), .c(new_n61_), .d(new_n60_), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(new_n64_));
  aoi21  g23(.a(new_n64_), .b(new_n53_), .c(x02), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n59_), .c(new_n44_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nor2   g26(.a(new_n45_), .b(new_n67_), .O(z03));
  inv1   g27(.a(x21), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n67_), .c(new_n45_), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  inv1   g30(.a(x08), .O(new_n72_));
  nand2  g31(.a(new_n47_), .b(new_n72_), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n52_), .c(new_n43_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x19), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(z05));
  inv1   g35(.a(x18), .O(new_n77_));
  inv1   g36(.a(x24), .O(new_n78_));
  inv1   g37(.a(new_n45_), .O(new_n79_));
  oai21  g38(.a(new_n78_), .b(new_n77_), .c(new_n79_), .O(z07));
  inv1   g39(.a(x11), .O(new_n81_));
  nand2  g40(.a(new_n79_), .b(new_n81_), .O(z08));
  oai21  g41(.a(new_n78_), .b(new_n81_), .c(new_n79_), .O(z09));
  nand3  g42(.a(new_n78_), .b(x22), .c(x14), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n79_), .O(z10));
  nor2   g44(.a(new_n45_), .b(x24), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x22), .c(x17), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z11));
  nand4  g47(.a(new_n79_), .b(new_n78_), .c(x23), .d(x14), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z12));
  nand3  g49(.a(new_n86_), .b(x23), .c(x17), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z13));
  nand2  g51(.a(new_n86_), .b(x16), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n45_), .O(z15));
  buf    g56(.a(x19), .O(z06));
endmodule


