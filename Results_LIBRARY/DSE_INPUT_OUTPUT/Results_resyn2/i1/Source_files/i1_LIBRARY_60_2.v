// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n77_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x24), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x07), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x19), .O(new_n47_));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  nor2   g07(.a(x04), .b(x03), .O(new_n49_));
  nor2   g08(.a(x06), .b(x05), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  xor2a  g11(.a(x09), .b(x08), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x07), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g14(.a(new_n46_), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(new_n55_), .c(new_n47_), .O(z01));
  nand2  g16(.a(x19), .b(x00), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x24), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x07), .O(new_n60_));
  inv1   g19(.a(x07), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(x08), .c(new_n61_), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n64_));
  nand2  g23(.a(new_n51_), .b(new_n42_), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(new_n64_), .c(x19), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n60_), .O(z02));
  inv1   g26(.a(x20), .O(new_n68_));
  nand2  g27(.a(new_n44_), .b(new_n68_), .O(z03));
  or2    g28(.a(z03), .b(x21), .O(z04));
  nand2  g29(.a(new_n44_), .b(x10), .O(new_n71_));
  inv1   g30(.a(x08), .O(new_n72_));
  nand3  g31(.a(x19), .b(new_n72_), .c(new_n61_), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n51_), .c(new_n71_), .O(z05));
  inv1   g33(.a(new_n47_), .O(z06));
  and2   g34(.a(x24), .b(x18), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nand2  g36(.a(new_n44_), .b(new_n77_), .O(z08));
  oai21  g37(.a(new_n43_), .b(new_n77_), .c(new_n44_), .O(z09));
  nand2  g38(.a(x22), .b(x14), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n61_), .c(x24), .O(z10));
  nand2  g40(.a(x22), .b(x17), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n61_), .c(x24), .O(z11));
  nand4  g42(.a(new_n43_), .b(x23), .c(x14), .d(new_n61_), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z12));
  nand2  g44(.a(x23), .b(x17), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n61_), .c(x24), .O(z13));
  nand3  g46(.a(new_n43_), .b(x16), .c(new_n61_), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nor2   g49(.a(x15), .b(x14), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n46_), .O(z15));
endmodule


