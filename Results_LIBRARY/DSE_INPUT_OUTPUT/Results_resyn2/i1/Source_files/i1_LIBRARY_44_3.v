// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n81_, new_n83_, new_n85_, new_n87_, new_n89_,
    new_n91_, new_n93_, new_n94_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x04), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x04), .O(new_n45_));
  nand2  g04(.a(x19), .b(new_n45_), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  xnor2a g12(.a(x09), .b(x08), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(new_n53_), .c(new_n47_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x00), .c(new_n46_), .O(z01));
  inv1   g15(.a(new_n46_), .O(z06));
  nor2   g16(.a(x05), .b(x03), .O(new_n58_));
  nor2   g17(.a(x07), .b(x06), .O(new_n59_));
  inv1   g18(.a(x09), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x08), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(new_n59_), .c(new_n58_), .d(new_n47_), .O(new_n62_));
  nand3  g21(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n42_), .O(new_n64_));
  nand3  g23(.a(new_n64_), .b(new_n62_), .c(z06), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nand2  g26(.a(new_n43_), .b(new_n67_), .O(z03));
  or2    g27(.a(z03), .b(x21), .O(z04));
  inv1   g28(.a(x10), .O(new_n70_));
  inv1   g29(.a(new_n43_), .O(new_n71_));
  inv1   g30(.a(x08), .O(new_n72_));
  nand3  g31(.a(x19), .b(new_n72_), .c(new_n45_), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n53_), .c(new_n47_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n70_), .c(new_n71_), .O(z05));
  nand2  g35(.a(x24), .b(x18), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n71_), .O(z07));
  inv1   g37(.a(x11), .O(new_n79_));
  nand2  g38(.a(new_n43_), .b(new_n79_), .O(z08));
  nor2   g39(.a(new_n71_), .b(x24), .O(new_n81_));
  aoi21  g40(.a(new_n43_), .b(new_n79_), .c(new_n81_), .O(z09));
  nand3  g41(.a(new_n81_), .b(x22), .c(x14), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z10));
  nand3  g43(.a(new_n81_), .b(x22), .c(x17), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(z11));
  nand3  g45(.a(new_n81_), .b(x23), .c(x14), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z12));
  nand2  g47(.a(x23), .b(x17), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(x24), .c(new_n43_), .O(z13));
  nand2  g49(.a(new_n81_), .b(x16), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z14));
  nor2   g51(.a(x13), .b(x12), .O(new_n93_));
  nor2   g52(.a(x15), .b(x14), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n71_), .O(z15));
endmodule


