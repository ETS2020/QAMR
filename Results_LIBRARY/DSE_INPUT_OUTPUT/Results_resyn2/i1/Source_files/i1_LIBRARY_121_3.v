// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n71_, new_n72_, new_n74_, new_n77_, new_n78_, new_n80_,
    new_n82_, new_n84_, new_n86_, new_n88_, new_n89_;
  nand2  g00(.a(x12), .b(x10), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x07), .O(new_n44_));
  nor2   g03(.a(x02), .b(x01), .O(new_n45_));
  nor2   g04(.a(x05), .b(x04), .O(new_n46_));
  nor2   g05(.a(x06), .b(x03), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  xor2a  g07(.a(x09), .b(x08), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x19), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n42_), .O(z01));
  nand2  g11(.a(new_n48_), .b(x00), .O(new_n53_));
  nor3   g12(.a(x07), .b(x06), .c(x03), .O(new_n54_));
  inv1   g13(.a(x08), .O(new_n55_));
  nor2   g14(.a(x09), .b(new_n55_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n54_), .c(new_n46_), .d(new_n45_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x19), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n42_), .O(z02));
  inv1   g19(.a(x20), .O(new_n61_));
  inv1   g20(.a(new_n42_), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g22(.a(x21), .O(new_n64_));
  aoi21  g23(.a(new_n64_), .b(new_n61_), .c(new_n62_), .O(z04));
  inv1   g24(.a(x10), .O(new_n66_));
  nand2  g25(.a(x19), .b(new_n55_), .O(new_n67_));
  or2    g26(.a(new_n67_), .b(new_n48_), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(z05));
  and2   g28(.a(new_n42_), .b(x19), .O(z06));
  inv1   g29(.a(x18), .O(new_n71_));
  inv1   g30(.a(x24), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(new_n42_), .O(z07));
  inv1   g32(.a(x11), .O(new_n74_));
  nand2  g33(.a(new_n42_), .b(new_n74_), .O(z08));
  oai21  g34(.a(new_n72_), .b(new_n74_), .c(new_n42_), .O(z09));
  nor2   g35(.a(new_n62_), .b(x24), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x22), .c(x14), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z10));
  nand3  g38(.a(new_n72_), .b(x22), .c(x17), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n42_), .O(z11));
  nand2  g40(.a(x23), .b(x14), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(x24), .c(new_n42_), .O(z12));
  nand4  g42(.a(new_n42_), .b(new_n72_), .c(x23), .d(x17), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z13));
  nand2  g44(.a(new_n77_), .b(x16), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(z15));
endmodule


