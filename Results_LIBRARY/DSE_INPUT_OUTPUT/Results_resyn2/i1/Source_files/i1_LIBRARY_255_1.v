// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_,
    new_n68_, new_n70_, new_n72_, new_n73_, new_n75_, new_n78_, new_n79_,
    new_n80_, new_n83_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x17), .O(new_n42_));
  nand2  g01(.a(x19), .b(new_n42_), .O(new_n43_));
  and2   g02(.a(new_n43_), .b(x00), .O(z00));
  inv1   g03(.a(x07), .O(new_n45_));
  nor3   g04(.a(x03), .b(x02), .c(x01), .O(new_n46_));
  nor3   g05(.a(x06), .b(x05), .c(x04), .O(new_n47_));
  xnor2a g06(.a(x09), .b(x08), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  nand2  g08(.a(x19), .b(x17), .O(new_n50_));
  aoi21  g09(.a(new_n49_), .b(x00), .c(new_n50_), .O(z01));
  inv1   g10(.a(x03), .O(new_n52_));
  nor2   g11(.a(x02), .b(x01), .O(new_n53_));
  nor2   g12(.a(x05), .b(x04), .O(new_n54_));
  nor2   g13(.a(x07), .b(x06), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g16(.a(x09), .O(new_n58_));
  inv1   g17(.a(x08), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x07), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n47_), .c(new_n46_), .d(new_n58_), .O(new_n61_));
  aoi21  g20(.a(new_n61_), .b(new_n57_), .c(new_n50_), .O(z02));
  inv1   g21(.a(x20), .O(new_n63_));
  inv1   g22(.a(new_n43_), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x21), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n63_), .c(new_n64_), .O(z04));
  nand2  g26(.a(new_n43_), .b(x10), .O(new_n68_));
  inv1   g27(.a(new_n50_), .O(z06));
  nand2  g28(.a(z06), .b(new_n59_), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n56_), .c(new_n68_), .O(z05));
  inv1   g30(.a(x18), .O(new_n72_));
  nand2  g31(.a(new_n43_), .b(x24), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n72_), .O(z07));
  inv1   g33(.a(x11), .O(new_n75_));
  nand2  g34(.a(new_n43_), .b(new_n75_), .O(z08));
  nor2   g35(.a(new_n73_), .b(new_n75_), .O(z09));
  inv1   g36(.a(x22), .O(new_n78_));
  inv1   g37(.a(x24), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x14), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(new_n43_), .O(z10));
  nor3   g40(.a(x24), .b(new_n78_), .c(new_n42_), .O(z11));
  inv1   g41(.a(x23), .O(new_n83_));
  oai21  g42(.a(new_n80_), .b(new_n83_), .c(new_n43_), .O(z12));
  nor3   g43(.a(x24), .b(new_n83_), .c(new_n42_), .O(z13));
  inv1   g44(.a(x16), .O(new_n86_));
  oai21  g45(.a(x24), .b(new_n86_), .c(new_n43_), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n88_), .c(new_n43_), .O(z15));
endmodule


