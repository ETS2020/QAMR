// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n79_, new_n81_, new_n82_;
  nand2  g00(.a(x19), .b(x08), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x09), .O(new_n44_));
  nor3   g03(.a(x07), .b(x06), .c(x05), .O(new_n45_));
  nor2   g04(.a(x02), .b(x01), .O(new_n46_));
  nor2   g05(.a(x04), .b(x03), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  inv1   g07(.a(x08), .O(new_n49_));
  nand2  g08(.a(x19), .b(new_n49_), .O(new_n50_));
  aoi21  g09(.a(new_n48_), .b(x00), .c(new_n50_), .O(z01));
  inv1   g10(.a(x07), .O(new_n52_));
  nor2   g11(.a(x06), .b(x05), .O(new_n53_));
  nand4  g12(.a(new_n47_), .b(new_n46_), .c(new_n53_), .d(new_n52_), .O(new_n54_));
  nand3  g13(.a(x19), .b(new_n49_), .c(x00), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  and2   g15(.a(new_n56_), .b(new_n54_), .O(z02));
  inv1   g16(.a(x20), .O(new_n58_));
  nand2  g17(.a(new_n42_), .b(new_n58_), .O(z03));
  or2    g18(.a(z03), .b(x21), .O(z04));
  inv1   g19(.a(x10), .O(new_n61_));
  nand2  g20(.a(new_n54_), .b(new_n49_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x19), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n61_), .O(z05));
  inv1   g23(.a(x18), .O(new_n65_));
  inv1   g24(.a(x24), .O(new_n66_));
  oai21  g25(.a(new_n66_), .b(new_n65_), .c(new_n42_), .O(z07));
  inv1   g26(.a(x11), .O(new_n68_));
  aoi21  g27(.a(x19), .b(x08), .c(new_n68_), .O(z08));
  oai21  g28(.a(new_n66_), .b(new_n68_), .c(new_n42_), .O(z09));
  nand3  g29(.a(new_n66_), .b(x22), .c(x14), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n42_), .O(z10));
  nand4  g31(.a(new_n42_), .b(new_n66_), .c(x22), .d(x17), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z11));
  nand4  g33(.a(new_n42_), .b(new_n66_), .c(x23), .d(x14), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  nand2  g35(.a(x23), .b(x17), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(x24), .c(new_n42_), .O(z13));
  inv1   g37(.a(x16), .O(new_n79_));
  oai21  g38(.a(x24), .b(new_n79_), .c(new_n42_), .O(z14));
  nor2   g39(.a(x13), .b(x12), .O(new_n81_));
  nor2   g40(.a(x15), .b(x14), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n81_), .c(new_n42_), .O(z15));
  buf    g42(.a(x19), .O(z06));
endmodule


