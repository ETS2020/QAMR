// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x07), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  nor2   g04(.a(x04), .b(x03), .O(new_n46_));
  inv1   g05(.a(x01), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(new_n52_), .c(new_n46_), .O(new_n54_));
  nor2   g13(.a(x07), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n54_), .c(new_n45_), .O(z01));
  inv1   g15(.a(new_n46_), .O(new_n57_));
  oai21  g16(.a(new_n51_), .b(new_n57_), .c(x00), .O(new_n58_));
  inv1   g17(.a(x03), .O(new_n59_));
  inv1   g18(.a(x04), .O(new_n60_));
  inv1   g19(.a(x09), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(x08), .c(new_n60_), .d(new_n59_), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n52_), .c(x07), .O(new_n64_));
  aoi21  g23(.a(new_n64_), .b(new_n58_), .c(new_n45_), .O(z02));
  inv1   g24(.a(x20), .O(new_n66_));
  nand2  g25(.a(new_n43_), .b(new_n66_), .O(z03));
  or2    g26(.a(z03), .b(x21), .O(z04));
  inv1   g27(.a(x10), .O(new_n69_));
  inv1   g28(.a(x07), .O(new_n70_));
  inv1   g29(.a(x08), .O(new_n71_));
  nand2  g30(.a(new_n46_), .b(new_n71_), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n51_), .c(new_n70_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x19), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n69_), .O(z05));
  nor2   g34(.a(new_n45_), .b(x07), .O(z06));
  inv1   g35(.a(x18), .O(new_n77_));
  inv1   g36(.a(x24), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n77_), .c(new_n43_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n43_), .b(new_n80_), .O(z08));
  inv1   g40(.a(new_n43_), .O(new_n82_));
  nor3   g41(.a(new_n82_), .b(new_n78_), .c(new_n80_), .O(z09));
  inv1   g42(.a(x14), .O(new_n84_));
  nand2  g43(.a(new_n78_), .b(x22), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n84_), .c(new_n43_), .O(z10));
  inv1   g45(.a(x17), .O(new_n87_));
  oai21  g46(.a(new_n85_), .b(new_n87_), .c(new_n43_), .O(z11));
  nand2  g47(.a(x23), .b(x14), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(x24), .c(new_n43_), .O(z12));
  nor2   g49(.a(new_n82_), .b(x24), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(x23), .c(x17), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z13));
  nand2  g52(.a(new_n91_), .b(x16), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z14));
  nor2   g54(.a(x13), .b(x12), .O(new_n96_));
  nor2   g55(.a(x15), .b(x14), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n82_), .O(z15));
endmodule


