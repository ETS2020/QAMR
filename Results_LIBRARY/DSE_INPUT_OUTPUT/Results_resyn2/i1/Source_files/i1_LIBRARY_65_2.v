// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n77_, new_n80_, new_n82_,
    new_n84_, new_n86_, new_n87_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x22), .O(new_n43_));
  nor2   g02(.a(x24), .b(new_n43_), .O(z11));
  nor2   g03(.a(z11), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(z11), .O(new_n47_));
  nor3   g06(.a(x07), .b(x06), .c(x05), .O(new_n48_));
  nor2   g07(.a(x02), .b(x01), .O(new_n49_));
  nor2   g08(.a(x04), .b(x03), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  aoi22  g11(.a(new_n52_), .b(z00), .c(new_n47_), .d(new_n46_), .O(z01));
  inv1   g12(.a(x09), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x08), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n56_));
  inv1   g15(.a(x07), .O(new_n57_));
  nor2   g16(.a(x06), .b(x05), .O(new_n58_));
  nand4  g17(.a(new_n50_), .b(new_n49_), .c(new_n58_), .d(new_n57_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n42_), .O(new_n60_));
  nor2   g19(.a(z11), .b(new_n46_), .O(new_n61_));
  nand3  g20(.a(new_n61_), .b(new_n60_), .c(new_n56_), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nand2  g23(.a(new_n47_), .b(new_n64_), .O(z03));
  nor2   g24(.a(x21), .b(x20), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(z11), .O(z04));
  inv1   g26(.a(x08), .O(new_n68_));
  nand2  g27(.a(x19), .b(new_n68_), .O(new_n69_));
  nor2   g28(.a(z11), .b(x10), .O(new_n70_));
  oai21  g29(.a(new_n69_), .b(new_n59_), .c(new_n70_), .O(z05));
  nand2  g30(.a(new_n47_), .b(new_n46_), .O(z06));
  nand2  g31(.a(x24), .b(x18), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n47_), .O(z07));
  inv1   g33(.a(x11), .O(new_n75_));
  nor2   g34(.a(z11), .b(new_n75_), .O(z08));
  inv1   g35(.a(x24), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n75_), .O(z09));
  nand4  g37(.a(new_n77_), .b(x23), .c(new_n43_), .d(x14), .O(new_n80_));
  inv1   g38(.a(new_n80_), .O(z12));
  nand2  g39(.a(x23), .b(x17), .O(new_n82_));
  aoi21  g40(.a(new_n82_), .b(new_n43_), .c(x24), .O(z13));
  inv1   g41(.a(x16), .O(new_n84_));
  aoi21  g42(.a(new_n43_), .b(new_n84_), .c(x24), .O(z14));
  nor2   g43(.a(x13), .b(x12), .O(new_n86_));
  nor2   g44(.a(x15), .b(x14), .O(new_n87_));
  nand3  g45(.a(new_n87_), .b(new_n86_), .c(new_n47_), .O(z15));
  zero   g46(.O(z10));
endmodule


