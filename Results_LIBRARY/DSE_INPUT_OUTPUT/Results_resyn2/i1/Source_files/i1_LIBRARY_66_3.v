// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x24), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  nor3   g06(.a(x07), .b(x06), .c(x05), .O(new_n48_));
  nor2   g07(.a(x02), .b(x01), .O(new_n49_));
  nor2   g08(.a(x04), .b(x03), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nand2  g11(.a(new_n46_), .b(x19), .O(new_n53_));
  aoi21  g12(.a(new_n52_), .b(x00), .c(new_n53_), .O(z01));
  inv1   g13(.a(x07), .O(new_n55_));
  nor2   g14(.a(x06), .b(x05), .O(new_n56_));
  nand4  g15(.a(new_n50_), .b(new_n49_), .c(new_n56_), .d(new_n55_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n42_), .O(new_n58_));
  inv1   g17(.a(x09), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x08), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n61_));
  nand3  g20(.a(new_n61_), .b(new_n58_), .c(x19), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n46_), .O(z02));
  nor2   g22(.a(x21), .b(x20), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n45_), .O(z04));
  inv1   g24(.a(x10), .O(new_n66_));
  inv1   g25(.a(x19), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(x08), .O(new_n68_));
  nand4  g27(.a(new_n68_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n66_), .c(new_n45_), .O(z05));
  nand2  g29(.a(new_n46_), .b(new_n67_), .O(z06));
  inv1   g30(.a(x18), .O(new_n72_));
  aoi21  g31(.a(new_n43_), .b(new_n72_), .c(new_n44_), .O(z07));
  inv1   g32(.a(x11), .O(new_n74_));
  nor2   g33(.a(new_n45_), .b(new_n74_), .O(z08));
  nand3  g34(.a(x24), .b(new_n43_), .c(x11), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z09));
  nand2  g36(.a(x22), .b(x14), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(x24), .O(z10));
  nand3  g38(.a(new_n44_), .b(x22), .c(x17), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n46_), .O(z11));
  nand3  g40(.a(new_n44_), .b(x23), .c(x14), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n46_), .O(z12));
  nand2  g42(.a(x23), .b(x17), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(x24), .O(z13));
  inv1   g44(.a(x16), .O(new_n86_));
  nor2   g45(.a(x24), .b(new_n86_), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n45_), .O(z15));
  buf    g49(.a(x20), .O(z03));
endmodule


