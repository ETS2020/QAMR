// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n90_, new_n92_, new_n94_, new_n95_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x07), .O(new_n45_));
  nor2   g04(.a(x02), .b(x01), .O(new_n46_));
  nor2   g05(.a(x04), .b(x03), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(x09), .b(new_n50_), .O(new_n51_));
  inv1   g10(.a(x09), .O(new_n52_));
  inv1   g11(.a(x05), .O(new_n53_));
  inv1   g12(.a(x06), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi21  g14(.a(new_n52_), .b(x08), .c(new_n55_), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(new_n51_), .c(new_n49_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x00), .c(new_n43_), .O(z01));
  nor3   g17(.a(x07), .b(x04), .c(x03), .O(new_n59_));
  nand2  g18(.a(new_n52_), .b(x08), .O(new_n60_));
  nand3  g19(.a(x19), .b(new_n54_), .c(new_n53_), .O(new_n61_));
  inv1   g20(.a(new_n61_), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n60_), .c(new_n59_), .d(new_n46_), .O(new_n63_));
  oai21  g22(.a(new_n61_), .b(new_n48_), .c(new_n42_), .O(new_n64_));
  and2   g23(.a(new_n64_), .b(new_n63_), .O(z02));
  inv1   g24(.a(x20), .O(new_n66_));
  nand2  g25(.a(new_n43_), .b(x00), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n66_), .O(z03));
  or2    g27(.a(z03), .b(x21), .O(z04));
  nand2  g28(.a(new_n67_), .b(x10), .O(new_n70_));
  nand3  g29(.a(new_n62_), .b(new_n49_), .c(new_n50_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n70_), .O(z05));
  inv1   g31(.a(new_n67_), .O(new_n73_));
  nand2  g32(.a(x24), .b(x18), .O(new_n74_));
  nor2   g33(.a(new_n74_), .b(new_n73_), .O(z07));
  inv1   g34(.a(x11), .O(new_n76_));
  nor2   g35(.a(new_n73_), .b(new_n76_), .O(z08));
  inv1   g36(.a(x24), .O(new_n78_));
  nor3   g37(.a(new_n73_), .b(new_n78_), .c(new_n76_), .O(z09));
  inv1   g38(.a(x14), .O(new_n80_));
  nand2  g39(.a(new_n67_), .b(new_n78_), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x22), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z10));
  inv1   g43(.a(x22), .O(new_n85_));
  nand2  g44(.a(new_n78_), .b(x17), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n67_), .O(z11));
  nand2  g46(.a(new_n82_), .b(x23), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z12));
  nand2  g48(.a(new_n67_), .b(x23), .O(new_n90_));
  nor2   g49(.a(new_n90_), .b(new_n86_), .O(z13));
  inv1   g50(.a(x16), .O(new_n92_));
  nor2   g51(.a(new_n81_), .b(new_n92_), .O(z14));
  nor2   g52(.a(x13), .b(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(x14), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n73_), .O(z15));
  buf    g55(.a(x19), .O(z06));
endmodule


