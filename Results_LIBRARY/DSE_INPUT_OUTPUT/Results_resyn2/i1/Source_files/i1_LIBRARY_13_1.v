// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n78_, new_n81_, new_n82_, new_n84_, new_n86_, new_n88_,
    new_n90_, new_n92_, new_n93_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x17), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nor3   g07(.a(x07), .b(x06), .c(x05), .O(new_n49_));
  xnor2a g08(.a(x09), .b(x08), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(x17), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n51_), .c(new_n43_), .O(z01));
  inv1   g13(.a(x09), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x08), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n57_));
  inv1   g16(.a(x07), .O(new_n58_));
  nor2   g17(.a(x06), .b(x05), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n48_), .c(new_n47_), .d(new_n58_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n42_), .O(new_n61_));
  nor2   g20(.a(new_n43_), .b(new_n52_), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n61_), .c(new_n57_), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z03));
  nor2   g25(.a(x21), .b(x20), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n44_), .O(z04));
  inv1   g27(.a(x10), .O(new_n69_));
  nor2   g28(.a(x08), .b(x07), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n59_), .c(new_n48_), .d(new_n47_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x17), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x19), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n69_), .O(z05));
  inv1   g33(.a(x18), .O(new_n75_));
  inv1   g34(.a(x24), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n75_), .c(new_n45_), .O(z07));
  inv1   g36(.a(x11), .O(new_n78_));
  nand2  g37(.a(new_n45_), .b(new_n78_), .O(z08));
  oai21  g38(.a(new_n76_), .b(new_n78_), .c(new_n45_), .O(z09));
  nor2   g39(.a(new_n44_), .b(x24), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x22), .c(x14), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z10));
  nand3  g42(.a(new_n76_), .b(x22), .c(x17), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n45_), .O(z11));
  nand3  g44(.a(new_n81_), .b(x23), .c(x14), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z12));
  nand2  g46(.a(x23), .b(x17), .O(new_n88_));
  nor2   g47(.a(new_n88_), .b(x24), .O(z13));
  nand2  g48(.a(new_n76_), .b(x16), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n45_), .O(z14));
  nor2   g50(.a(x13), .b(x12), .O(new_n92_));
  nor2   g51(.a(x15), .b(x14), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n44_), .O(z15));
  buf    g53(.a(x19), .O(z06));
endmodule


