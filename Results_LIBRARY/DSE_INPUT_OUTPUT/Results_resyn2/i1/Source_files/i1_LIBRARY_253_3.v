// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n75_, new_n77_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x23), .O(new_n43_));
  inv1   g02(.a(x24), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  inv1   g06(.a(x19), .O(new_n48_));
  nor3   g07(.a(x07), .b(x06), .c(x05), .O(new_n49_));
  nor2   g08(.a(x02), .b(x01), .O(new_n50_));
  nor2   g09(.a(x04), .b(x03), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor2   g12(.a(new_n45_), .b(new_n42_), .O(new_n54_));
  aoi22  g13(.a(new_n54_), .b(new_n53_), .c(new_n46_), .d(new_n48_), .O(z01));
  inv1   g14(.a(x09), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n58_));
  inv1   g17(.a(x07), .O(new_n59_));
  nor2   g18(.a(x06), .b(x05), .O(new_n60_));
  nand4  g19(.a(new_n51_), .b(new_n50_), .c(new_n60_), .d(new_n59_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n48_), .O(new_n63_));
  nand3  g22(.a(new_n63_), .b(new_n62_), .c(new_n58_), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z02));
  inv1   g24(.a(x20), .O(new_n66_));
  nand2  g25(.a(new_n46_), .b(new_n66_), .O(z03));
  nor2   g26(.a(x21), .b(x20), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n45_), .O(z04));
  inv1   g28(.a(x08), .O(new_n70_));
  nand2  g29(.a(x19), .b(new_n70_), .O(new_n71_));
  nor2   g30(.a(new_n45_), .b(x10), .O(new_n72_));
  oai21  g31(.a(new_n71_), .b(new_n61_), .c(new_n72_), .O(z05));
  nand2  g32(.a(new_n46_), .b(new_n48_), .O(z06));
  inv1   g33(.a(x18), .O(new_n75_));
  aoi21  g34(.a(new_n43_), .b(new_n75_), .c(new_n44_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nor2   g36(.a(new_n45_), .b(new_n77_), .O(z08));
  aoi21  g37(.a(new_n43_), .b(new_n77_), .c(new_n44_), .O(z09));
  nand2  g38(.a(x22), .b(x14), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(x24), .O(z10));
  nand3  g40(.a(new_n44_), .b(x22), .c(x17), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n46_), .O(z11));
  inv1   g42(.a(x14), .O(new_n84_));
  aoi21  g43(.a(new_n44_), .b(new_n84_), .c(new_n43_), .O(z12));
  inv1   g44(.a(x17), .O(new_n86_));
  aoi21  g45(.a(new_n44_), .b(new_n86_), .c(new_n43_), .O(z13));
  inv1   g46(.a(x16), .O(new_n88_));
  nor2   g47(.a(x24), .b(new_n88_), .O(z14));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nor2   g49(.a(x15), .b(x14), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n45_), .O(z15));
endmodule


