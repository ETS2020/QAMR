// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n78_, new_n80_, new_n82_, new_n84_, new_n85_, new_n87_,
    new_n89_, new_n91_, new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x06), .O(new_n43_));
  nor2   g02(.a(x08), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x19), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  nor3   g07(.a(x03), .b(x02), .c(x01), .O(new_n49_));
  nor3   g08(.a(x06), .b(x05), .c(x04), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(z00), .c(new_n47_), .O(z01));
  oai21  g12(.a(new_n47_), .b(new_n42_), .c(x08), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x06), .O(new_n55_));
  nor3   g14(.a(x07), .b(x05), .c(x04), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(new_n49_), .c(new_n42_), .O(new_n57_));
  inv1   g16(.a(x03), .O(new_n58_));
  nor2   g17(.a(x02), .b(x01), .O(new_n59_));
  nor2   g18(.a(x05), .b(x04), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n59_), .c(new_n43_), .d(new_n58_), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(x08), .c(new_n48_), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n57_), .c(x19), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n55_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nor2   g26(.a(new_n44_), .b(new_n67_), .O(z03));
  inv1   g27(.a(x21), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n67_), .c(new_n44_), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  inv1   g30(.a(x08), .O(new_n72_));
  nand3  g31(.a(new_n56_), .b(new_n49_), .c(x19), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n43_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(z05));
  nand2  g35(.a(new_n45_), .b(new_n47_), .O(z06));
  nand2  g36(.a(x24), .b(x18), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n44_), .O(z07));
  inv1   g38(.a(x11), .O(new_n80_));
  nand2  g39(.a(new_n45_), .b(new_n80_), .O(z08));
  inv1   g40(.a(x24), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n80_), .c(new_n45_), .O(z09));
  inv1   g42(.a(x22), .O(new_n84_));
  nand2  g43(.a(new_n82_), .b(x14), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n84_), .c(new_n45_), .O(z10));
  nand2  g45(.a(x22), .b(x17), .O(new_n87_));
  nor3   g46(.a(new_n87_), .b(new_n44_), .c(x24), .O(z11));
  inv1   g47(.a(x23), .O(new_n89_));
  oai21  g48(.a(new_n85_), .b(new_n89_), .c(new_n45_), .O(z12));
  nand3  g49(.a(new_n82_), .b(x23), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n45_), .O(z13));
  nand2  g51(.a(new_n82_), .b(x16), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n45_), .O(z14));
  nor2   g53(.a(x13), .b(x12), .O(new_n95_));
  nor2   g54(.a(x15), .b(x14), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n45_), .O(z15));
endmodule


