// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n56_, new_n58_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x08), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x09), .O(new_n45_));
  nor3   g04(.a(x07), .b(x06), .c(x05), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(x19), .b(new_n50_), .O(new_n51_));
  aoi21  g10(.a(new_n49_), .b(x00), .c(new_n51_), .O(z01));
  nand2  g11(.a(new_n48_), .b(new_n47_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  inv1   g13(.a(new_n51_), .O(z06));
  nand2  g14(.a(z06), .b(x00), .O(new_n56_));
  aoi21  g15(.a(new_n54_), .b(new_n46_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x20), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z03));
  or2    g18(.a(z03), .b(x21), .O(z04));
  inv1   g19(.a(x10), .O(new_n61_));
  inv1   g20(.a(x07), .O(new_n62_));
  nor2   g21(.a(x06), .b(x05), .O(new_n63_));
  nand4  g22(.a(new_n48_), .b(new_n47_), .c(new_n63_), .d(new_n62_), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(x19), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n61_), .O(z05));
  inv1   g26(.a(new_n43_), .O(new_n68_));
  nand2  g27(.a(x24), .b(x18), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n68_), .O(z07));
  and2   g29(.a(new_n43_), .b(x11), .O(z08));
  nand2  g30(.a(z08), .b(x24), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z09));
  inv1   g32(.a(x14), .O(new_n74_));
  inv1   g33(.a(x24), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x22), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n74_), .c(new_n43_), .O(z10));
  inv1   g36(.a(x17), .O(new_n78_));
  oai21  g37(.a(new_n76_), .b(new_n78_), .c(new_n43_), .O(z11));
  nand3  g38(.a(new_n43_), .b(new_n75_), .c(x23), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x14), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z12));
  nand2  g42(.a(new_n81_), .b(x17), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z13));
  inv1   g44(.a(x16), .O(new_n86_));
  oai21  g45(.a(x24), .b(new_n86_), .c(new_n43_), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n68_), .O(z15));
endmodule


