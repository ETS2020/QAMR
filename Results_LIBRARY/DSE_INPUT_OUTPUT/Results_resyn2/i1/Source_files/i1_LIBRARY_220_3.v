// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n70_, new_n73_, new_n74_, new_n77_, new_n79_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_;
  nand2  g00(.a(x10), .b(x08), .O(new_n42_));
  and2   g01(.a(new_n42_), .b(x00), .O(z00));
  inv1   g02(.a(x07), .O(new_n44_));
  nor2   g03(.a(x02), .b(x01), .O(new_n45_));
  nor2   g04(.a(x04), .b(x03), .O(new_n46_));
  nor2   g05(.a(x06), .b(x05), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  xor2a  g07(.a(x09), .b(x08), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x19), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n42_), .O(z01));
  inv1   g11(.a(x08), .O(new_n53_));
  inv1   g12(.a(x09), .O(new_n54_));
  inv1   g13(.a(x01), .O(new_n55_));
  inv1   g14(.a(x02), .O(new_n56_));
  inv1   g15(.a(x03), .O(new_n57_));
  inv1   g16(.a(x04), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n55_), .O(new_n59_));
  inv1   g18(.a(x05), .O(new_n60_));
  inv1   g19(.a(x06), .O(new_n61_));
  nand4  g20(.a(x19), .b(new_n44_), .c(new_n61_), .d(new_n60_), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n54_), .c(x10), .O(new_n64_));
  nand3  g23(.a(new_n48_), .b(x19), .c(x00), .O(new_n65_));
  oai21  g24(.a(new_n64_), .b(new_n53_), .c(new_n65_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nand2  g26(.a(new_n42_), .b(new_n67_), .O(z03));
  or2    g27(.a(z03), .b(x21), .O(z04));
  nor2   g28(.a(new_n63_), .b(x10), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(x08), .O(z05));
  and2   g30(.a(new_n42_), .b(x19), .O(z06));
  inv1   g31(.a(x18), .O(new_n73_));
  inv1   g32(.a(x24), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(new_n42_), .O(z07));
  and2   g34(.a(new_n42_), .b(x11), .O(z08));
  nand2  g35(.a(z08), .b(x24), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z09));
  inv1   g37(.a(x14), .O(new_n79_));
  nand2  g38(.a(new_n74_), .b(x22), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n42_), .O(z10));
  inv1   g40(.a(x17), .O(new_n82_));
  oai21  g41(.a(new_n80_), .b(new_n82_), .c(new_n42_), .O(z11));
  nand2  g42(.a(x23), .b(x14), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(x24), .c(new_n42_), .O(z12));
  nand4  g44(.a(new_n42_), .b(new_n74_), .c(x23), .d(x17), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z13));
  inv1   g46(.a(x16), .O(new_n88_));
  oai21  g47(.a(x24), .b(new_n88_), .c(new_n42_), .O(z14));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nor2   g49(.a(x15), .b(x14), .O(new_n91_));
  aoi22  g50(.a(new_n91_), .b(new_n90_), .c(x10), .d(x08), .O(z15));
endmodule


