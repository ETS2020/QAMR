// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n77_, new_n79_, new_n81_, new_n83_, new_n84_, new_n86_, new_n88_,
    new_n91_, new_n93_, new_n94_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x24), .b(x04), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  inv1   g04(.a(x07), .O(new_n46_));
  nor3   g05(.a(x06), .b(x05), .c(x03), .O(new_n47_));
  nor3   g06(.a(x04), .b(x02), .c(x01), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(z00), .c(new_n45_), .O(z01));
  inv1   g10(.a(x24), .O(new_n52_));
  oai21  g11(.a(new_n45_), .b(new_n42_), .c(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x04), .O(new_n54_));
  nor3   g13(.a(x07), .b(x02), .c(x01), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n47_), .c(new_n42_), .O(new_n56_));
  inv1   g15(.a(x04), .O(new_n57_));
  inv1   g16(.a(x06), .O(new_n58_));
  nor2   g17(.a(x05), .b(x03), .O(new_n59_));
  nor2   g18(.a(x02), .b(x01), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(x08), .c(new_n46_), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n56_), .c(x19), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n54_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nand2  g26(.a(new_n43_), .b(new_n67_), .O(z03));
  inv1   g27(.a(new_n43_), .O(new_n69_));
  nor2   g28(.a(x21), .b(x20), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(new_n69_), .O(z04));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x08), .b(x04), .O(new_n73_));
  nand4  g32(.a(new_n73_), .b(new_n55_), .c(new_n47_), .d(x19), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n72_), .c(new_n69_), .O(z05));
  nor2   g34(.a(new_n69_), .b(new_n45_), .O(z06));
  nand3  g35(.a(x24), .b(x18), .c(new_n57_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z07));
  inv1   g37(.a(x11), .O(new_n79_));
  nand2  g38(.a(new_n43_), .b(new_n79_), .O(z08));
  nand3  g39(.a(x24), .b(x11), .c(new_n57_), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(z09));
  inv1   g41(.a(x14), .O(new_n83_));
  inv1   g42(.a(x22), .O(new_n84_));
  nor3   g43(.a(x24), .b(new_n84_), .c(new_n83_), .O(z10));
  nand2  g44(.a(new_n52_), .b(x17), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n84_), .c(new_n43_), .O(z11));
  inv1   g46(.a(x23), .O(new_n88_));
  nor3   g47(.a(x24), .b(new_n88_), .c(new_n83_), .O(z12));
  oai21  g48(.a(new_n86_), .b(new_n88_), .c(new_n43_), .O(z13));
  inv1   g49(.a(x16), .O(new_n91_));
  oai21  g50(.a(x24), .b(new_n91_), .c(new_n43_), .O(z14));
  nor2   g51(.a(x13), .b(x12), .O(new_n93_));
  nor2   g52(.a(x15), .b(x14), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(new_n43_), .O(z15));
endmodule


