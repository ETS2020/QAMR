// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n70_, new_n71_, new_n74_, new_n75_, new_n77_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n88_,
    new_n90_, new_n92_, new_n94_, new_n95_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x06), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(new_n42_), .O(z00));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  nor2   g07(.a(x04), .b(x03), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(x09), .b(new_n51_), .O(new_n52_));
  inv1   g11(.a(x09), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x08), .O(new_n54_));
  nor2   g13(.a(x07), .b(x05), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(new_n52_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n50_), .c(x19), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n43_), .O(new_n58_));
  nand2  g17(.a(x19), .b(new_n42_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g19(.a(new_n50_), .O(new_n61_));
  nand4  g20(.a(new_n55_), .b(new_n54_), .c(new_n61_), .d(new_n43_), .O(new_n62_));
  nand4  g21(.a(new_n55_), .b(new_n49_), .c(new_n48_), .d(new_n43_), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n42_), .c(new_n44_), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nand2  g26(.a(new_n45_), .b(new_n67_), .O(z03));
  or2    g27(.a(z03), .b(x21), .O(z04));
  nand2  g28(.a(new_n45_), .b(x10), .O(new_n70_));
  nand2  g29(.a(x19), .b(new_n51_), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n63_), .c(new_n70_), .O(z05));
  nand2  g31(.a(new_n44_), .b(x06), .O(z06));
  inv1   g32(.a(x18), .O(new_n74_));
  nand2  g33(.a(new_n45_), .b(x24), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n74_), .O(z07));
  inv1   g35(.a(x11), .O(new_n77_));
  nand2  g36(.a(new_n45_), .b(new_n77_), .O(z08));
  nor2   g37(.a(new_n75_), .b(new_n77_), .O(z09));
  inv1   g38(.a(x22), .O(new_n80_));
  inv1   g39(.a(x24), .O(new_n81_));
  nand2  g40(.a(new_n45_), .b(new_n81_), .O(new_n82_));
  nor2   g41(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x14), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z10));
  nand2  g44(.a(new_n83_), .b(x17), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z11));
  nand3  g46(.a(new_n81_), .b(x23), .c(x14), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n45_), .O(z12));
  nand2  g48(.a(x23), .b(x17), .O(new_n90_));
  nor2   g49(.a(new_n90_), .b(new_n82_), .O(z13));
  inv1   g50(.a(x16), .O(new_n92_));
  nor2   g51(.a(new_n82_), .b(new_n92_), .O(z14));
  nor2   g52(.a(x13), .b(x12), .O(new_n94_));
  nor2   g53(.a(x15), .b(x14), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n46_), .O(z15));
endmodule


