// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n73_, new_n75_, new_n77_, new_n79_,
    new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x06), .O(new_n43_));
  inv1   g02(.a(x08), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  inv1   g06(.a(x19), .O(new_n48_));
  nor2   g07(.a(x02), .b(x01), .O(new_n49_));
  nor2   g08(.a(x04), .b(x03), .O(new_n50_));
  nor2   g09(.a(x07), .b(x05), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(new_n53_));
  nand2  g12(.a(x09), .b(new_n44_), .O(new_n54_));
  inv1   g13(.a(x09), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x08), .c(x06), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(new_n54_), .c(new_n53_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(z00), .c(new_n48_), .O(z01));
  oai21  g17(.a(new_n48_), .b(new_n42_), .c(new_n44_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x06), .O(new_n60_));
  aoi21  g19(.a(new_n55_), .b(x08), .c(new_n52_), .O(new_n61_));
  nand2  g20(.a(new_n52_), .b(new_n42_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x19), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nor2   g24(.a(new_n45_), .b(new_n65_), .O(z03));
  inv1   g25(.a(x21), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(new_n65_), .c(new_n45_), .O(z04));
  nand3  g27(.a(x19), .b(new_n44_), .c(new_n43_), .O(new_n69_));
  nor2   g28(.a(new_n45_), .b(x10), .O(new_n70_));
  oai21  g29(.a(new_n69_), .b(new_n52_), .c(new_n70_), .O(z05));
  nand2  g30(.a(new_n46_), .b(new_n48_), .O(z06));
  nand2  g31(.a(x24), .b(x18), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n45_), .O(z07));
  inv1   g33(.a(x11), .O(new_n75_));
  nand2  g34(.a(new_n46_), .b(new_n75_), .O(z08));
  inv1   g35(.a(x24), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n75_), .c(new_n46_), .O(z09));
  inv1   g37(.a(x22), .O(new_n79_));
  nand2  g38(.a(new_n77_), .b(x14), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n46_), .O(z10));
  nand4  g40(.a(new_n46_), .b(new_n77_), .c(x22), .d(x17), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z11));
  inv1   g42(.a(x23), .O(new_n84_));
  oai21  g43(.a(new_n80_), .b(new_n84_), .c(new_n46_), .O(z12));
  nand3  g44(.a(new_n77_), .b(x23), .c(x17), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n46_), .O(z13));
  nand2  g46(.a(new_n77_), .b(x16), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n46_), .O(z14));
  nor2   g48(.a(x13), .b(x12), .O(new_n90_));
  nor2   g49(.a(x15), .b(x14), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(new_n46_), .O(z15));
endmodule


