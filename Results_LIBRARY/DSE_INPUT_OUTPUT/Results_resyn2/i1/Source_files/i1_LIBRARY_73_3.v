// Benchmark "FAU" written by ABC on Tue Aug 11 19:27:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n79_, new_n81_, new_n82_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x08), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x09), .O(new_n45_));
  nor3   g04(.a(x07), .b(x06), .c(x05), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(x19), .b(new_n50_), .O(new_n51_));
  aoi21  g10(.a(new_n49_), .b(x00), .c(new_n51_), .O(z01));
  nand2  g11(.a(new_n48_), .b(new_n47_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  nand3  g13(.a(x19), .b(new_n50_), .c(x00), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n46_), .c(new_n55_), .O(z02));
  inv1   g15(.a(x20), .O(new_n57_));
  inv1   g16(.a(new_n43_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(new_n57_), .O(z03));
  inv1   g18(.a(x21), .O(new_n60_));
  nand3  g19(.a(new_n43_), .b(new_n60_), .c(new_n57_), .O(z04));
  inv1   g20(.a(x10), .O(new_n62_));
  nand4  g21(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x19), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n62_), .c(new_n58_), .O(z05));
  nand2  g23(.a(x24), .b(x18), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n58_), .O(z07));
  inv1   g25(.a(x11), .O(new_n67_));
  nor2   g26(.a(new_n58_), .b(new_n67_), .O(z08));
  inv1   g27(.a(x24), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(new_n67_), .c(new_n43_), .O(z09));
  nand3  g29(.a(new_n69_), .b(x22), .c(x14), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n43_), .O(z10));
  nand4  g31(.a(new_n43_), .b(new_n69_), .c(x22), .d(x17), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z11));
  nand4  g33(.a(new_n43_), .b(new_n69_), .c(x23), .d(x14), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  nand4  g35(.a(new_n43_), .b(new_n69_), .c(x23), .d(x17), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z13));
  nand3  g37(.a(new_n43_), .b(new_n69_), .c(x16), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z14));
  nor2   g39(.a(x13), .b(x12), .O(new_n81_));
  nor2   g40(.a(x15), .b(x14), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n81_), .c(new_n43_), .O(z15));
  buf    g42(.a(x19), .O(z06));
endmodule


