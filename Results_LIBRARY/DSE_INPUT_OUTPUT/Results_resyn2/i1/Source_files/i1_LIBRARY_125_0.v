// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n75_, new_n78_, new_n80_,
    new_n82_, new_n84_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x24), .b(x04), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(x04), .O(new_n47_));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  nor2   g07(.a(x05), .b(x03), .O(new_n49_));
  nor2   g08(.a(x07), .b(x06), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  inv1   g11(.a(x08), .O(new_n53_));
  nand2  g12(.a(x09), .b(new_n53_), .O(new_n54_));
  inv1   g13(.a(x09), .O(new_n55_));
  inv1   g14(.a(x24), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(x08), .c(new_n56_), .O(new_n57_));
  nand3  g16(.a(new_n57_), .b(new_n54_), .c(new_n52_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(z00), .c(new_n46_), .O(z01));
  nand3  g18(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n60_));
  nand3  g19(.a(x19), .b(new_n55_), .c(x08), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(new_n60_), .c(x24), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n47_), .O(new_n63_));
  nand3  g22(.a(new_n51_), .b(x19), .c(x00), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n63_), .O(z02));
  inv1   g24(.a(x20), .O(new_n66_));
  nand2  g25(.a(new_n44_), .b(new_n66_), .O(z03));
  nor2   g26(.a(x21), .b(x20), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n43_), .O(z04));
  inv1   g28(.a(x10), .O(new_n70_));
  nand3  g29(.a(x24), .b(x19), .c(new_n53_), .O(new_n71_));
  oai22  g30(.a(new_n71_), .b(new_n51_), .c(new_n43_), .d(new_n70_), .O(z05));
  nor2   g31(.a(new_n43_), .b(new_n46_), .O(z06));
  and2   g32(.a(x24), .b(x18), .O(z07));
  inv1   g33(.a(x11), .O(new_n75_));
  nand2  g34(.a(new_n44_), .b(new_n75_), .O(z08));
  nor2   g35(.a(new_n56_), .b(new_n75_), .O(z09));
  nand4  g36(.a(new_n56_), .b(x22), .c(x14), .d(x04), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z10));
  nand2  g38(.a(x22), .b(x17), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(x04), .c(x24), .O(z11));
  nand4  g40(.a(new_n56_), .b(x23), .c(x14), .d(x04), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z12));
  nand2  g42(.a(x23), .b(x17), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(x04), .c(x24), .O(z13));
  inv1   g44(.a(x16), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(x04), .c(x24), .O(z14));
  nor2   g46(.a(x13), .b(x12), .O(new_n88_));
  nor2   g47(.a(x15), .b(x14), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n88_), .c(new_n44_), .O(z15));
endmodule


