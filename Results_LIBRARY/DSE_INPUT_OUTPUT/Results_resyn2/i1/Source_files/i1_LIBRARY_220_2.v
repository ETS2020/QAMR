// Benchmark "FAU" written by ABC on Tue Aug 11 19:28:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n63_, new_n64_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n74_, new_n75_, new_n77_, new_n79_, new_n81_, new_n83_,
    new_n85_, new_n86_;
  inv1   g00(.a(x10), .O(new_n42_));
  nand2  g01(.a(new_n42_), .b(x08), .O(new_n43_));
  and2   g02(.a(new_n43_), .b(x00), .O(z00));
  inv1   g03(.a(x07), .O(new_n45_));
  nor2   g04(.a(x06), .b(x05), .O(new_n46_));
  nor2   g05(.a(x02), .b(x01), .O(new_n47_));
  nor2   g06(.a(x04), .b(x03), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  xor2a  g08(.a(x09), .b(x08), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x19), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n43_), .O(z01));
  inv1   g12(.a(x09), .O(new_n54_));
  nand2  g13(.a(x19), .b(new_n54_), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n49_), .c(x10), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n57_));
  nand3  g16(.a(new_n49_), .b(x19), .c(x00), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n57_), .O(z02));
  inv1   g18(.a(x20), .O(new_n60_));
  nand2  g19(.a(new_n43_), .b(new_n60_), .O(z03));
  or2    g20(.a(z03), .b(x21), .O(z04));
  inv1   g21(.a(x08), .O(new_n63_));
  nand2  g22(.a(x19), .b(new_n63_), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n49_), .c(new_n42_), .O(z05));
  and2   g24(.a(new_n43_), .b(x19), .O(z06));
  inv1   g25(.a(x18), .O(new_n67_));
  inv1   g26(.a(x24), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n67_), .c(new_n43_), .O(z07));
  inv1   g28(.a(x11), .O(new_n70_));
  inv1   g29(.a(new_n43_), .O(new_n71_));
  nor2   g30(.a(new_n71_), .b(new_n70_), .O(z08));
  nor3   g31(.a(new_n71_), .b(new_n68_), .c(new_n70_), .O(z09));
  inv1   g32(.a(x14), .O(new_n74_));
  nand2  g33(.a(new_n68_), .b(x22), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n74_), .c(new_n43_), .O(z10));
  inv1   g35(.a(x17), .O(new_n77_));
  oai21  g36(.a(new_n75_), .b(new_n77_), .c(new_n43_), .O(z11));
  nand3  g37(.a(new_n68_), .b(x23), .c(x14), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n43_), .O(z12));
  nand2  g39(.a(x23), .b(x17), .O(new_n81_));
  nor3   g40(.a(new_n81_), .b(new_n71_), .c(x24), .O(z13));
  nand2  g41(.a(new_n68_), .b(x16), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n43_), .O(z14));
  nor2   g43(.a(x13), .b(x12), .O(new_n85_));
  nor2   g44(.a(x15), .b(x14), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n71_), .O(z15));
endmodule


