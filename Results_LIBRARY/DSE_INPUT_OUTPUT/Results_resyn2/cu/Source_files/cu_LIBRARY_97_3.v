// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nor2   g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  oai21  g05(.a(new_n28_), .b(x04), .c(new_n30_), .O(new_n31_));
  oai21  g06(.a(new_n29_), .b(new_n26_), .c(new_n31_), .O(z00));
  nor2   g07(.a(new_n29_), .b(x03), .O(new_n33_));
  and2   g08(.a(new_n33_), .b(new_n31_), .O(z01));
  inv1   g09(.a(x04), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(x05), .c(new_n35_), .O(new_n37_));
  nand2  g12(.a(new_n26_), .b(new_n27_), .O(new_n38_));
  nor4   g13(.a(new_n38_), .b(new_n37_), .c(x01), .d(x00), .O(z02));
  inv1   g14(.a(x01), .O(new_n40_));
  nor3   g15(.a(x13), .b(x04), .c(x03), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(new_n27_), .c(new_n28_), .O(z03));
  inv1   g18(.a(x00), .O(new_n44_));
  nand3  g19(.a(new_n41_), .b(x01), .c(new_n44_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n27_), .c(new_n28_), .O(z04));
  nor4   g21(.a(new_n38_), .b(new_n37_), .c(new_n40_), .d(new_n44_), .O(z05));
  nand2  g22(.a(x04), .b(new_n26_), .O(new_n48_));
  inv1   g23(.a(x10), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n40_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n50_), .c(new_n44_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n40_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  nor2   g33(.a(x12), .b(new_n28_), .O(new_n59_));
  inv1   g34(.a(x07), .O(new_n60_));
  nand3  g35(.a(x13), .b(new_n60_), .c(new_n27_), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(new_n59_), .c(new_n58_), .d(new_n53_), .O(new_n63_));
  nand3  g38(.a(new_n36_), .b(new_n28_), .c(x02), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n63_), .c(new_n48_), .O(z06));
  nor2   g40(.a(new_n41_), .b(x02), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(new_n28_), .O(z07));
  oai21  g42(.a(new_n48_), .b(x13), .c(new_n28_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(x02), .O(new_n69_));
  nand4  g44(.a(new_n59_), .b(x13), .c(x04), .d(new_n26_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(z08));
  inv1   g46(.a(x06), .O(new_n72_));
  nor3   g47(.a(new_n29_), .b(new_n36_), .c(new_n72_), .O(z09));
  oai22  g48(.a(new_n72_), .b(x03), .c(new_n28_), .d(new_n27_), .O(z10));
endmodule


