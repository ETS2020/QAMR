// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_,
    new_n69_, new_n71_, new_n73_;
  nand2  g00(.a(x13), .b(x09), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  and2   g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n27_), .O(z00));
  nand2  g08(.a(new_n32_), .b(new_n26_), .O(new_n34_));
  inv1   g09(.a(new_n34_), .O(z01));
  inv1   g10(.a(x04), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nor2   g12(.a(x03), .b(x02), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(new_n37_), .c(x05), .d(new_n36_), .O(new_n39_));
  nor3   g14(.a(new_n39_), .b(x01), .c(x00), .O(z02));
  inv1   g15(.a(x01), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(x00), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n39_), .c(new_n26_), .O(z03));
  inv1   g18(.a(x00), .O(new_n44_));
  nand2  g19(.a(x01), .b(new_n44_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n39_), .c(new_n26_), .O(z04));
  nand2  g21(.a(x01), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n39_), .c(new_n26_), .O(z05));
  nand3  g23(.a(new_n37_), .b(new_n28_), .c(x02), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(x01), .c(new_n44_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(x00), .c(new_n41_), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g31(.a(new_n56_), .b(new_n54_), .c(new_n52_), .O(new_n57_));
  inv1   g32(.a(x02), .O(new_n58_));
  nand2  g33(.a(x05), .b(new_n58_), .O(new_n59_));
  nor2   g34(.a(x12), .b(x07), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x13), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  aoi21  g37(.a(new_n62_), .b(new_n57_), .c(new_n50_), .O(new_n63_));
  inv1   g38(.a(x03), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n64_), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n63_), .c(new_n26_), .O(z06));
  inv1   g41(.a(new_n39_), .O(z07));
  nor2   g42(.a(x12), .b(x09), .O(new_n68_));
  nand4  g43(.a(new_n68_), .b(x13), .c(x05), .d(new_n58_), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(new_n49_), .c(new_n65_), .O(z08));
  nor2   g45(.a(x09), .b(x06), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n37_), .O(z09));
  nand3  g47(.a(new_n31_), .b(new_n26_), .c(x06), .O(new_n73_));
  inv1   g48(.a(new_n73_), .O(z10));
endmodule


