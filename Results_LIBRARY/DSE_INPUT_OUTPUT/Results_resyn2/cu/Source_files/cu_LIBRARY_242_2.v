// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n62_, new_n63_, new_n65_, new_n67_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nor2   g03(.a(x04), .b(x03), .O(new_n29_));
  nand3  g04(.a(new_n29_), .b(x05), .c(new_n28_), .O(new_n30_));
  xnor2a g05(.a(new_n30_), .b(new_n27_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x01), .O(new_n33_));
  nand2  g08(.a(new_n28_), .b(new_n33_), .O(new_n34_));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nand3  g11(.a(new_n29_), .b(new_n36_), .c(x05), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n35_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n34_), .c(new_n27_), .O(z02));
  nand2  g15(.a(new_n38_), .b(x00), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n34_), .c(new_n27_), .O(z03));
  nand2  g17(.a(new_n28_), .b(x01), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n39_), .c(new_n27_), .O(z04));
  oai21  g19(.a(new_n43_), .b(new_n41_), .c(new_n27_), .O(z05));
  inv1   g20(.a(x04), .O(new_n46_));
  inv1   g21(.a(x10), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x01), .O(new_n48_));
  inv1   g23(.a(x08), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(new_n33_), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n48_), .c(new_n35_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n33_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  nor2   g31(.a(new_n36_), .b(x12), .O(new_n57_));
  nor3   g32(.a(x07), .b(x03), .c(x02), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n51_), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(x05), .c(new_n46_), .O(z06));
  oai21  g35(.a(new_n37_), .b(x02), .c(new_n27_), .O(z07));
  inv1   g36(.a(x03), .O(new_n62_));
  nand4  g37(.a(x05), .b(x04), .c(new_n62_), .d(new_n28_), .O(new_n63_));
  nor3   g38(.a(new_n63_), .b(new_n36_), .c(x12), .O(z08));
  nand2  g39(.a(new_n27_), .b(x06), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(new_n36_), .O(z09));
  oai21  g41(.a(new_n26_), .b(new_n28_), .c(new_n62_), .O(new_n67_));
  nor2   g42(.a(new_n67_), .b(new_n65_), .O(z10));
endmodule


