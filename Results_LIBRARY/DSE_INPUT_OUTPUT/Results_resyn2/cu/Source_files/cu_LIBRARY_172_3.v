// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand3  g01(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x11), .O(new_n30_));
  nand4  g05(.a(new_n30_), .b(x05), .c(new_n29_), .d(new_n28_), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g07(.a(z01), .O(z00));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(x01), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  inv1   g13(.a(new_n31_), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(new_n36_), .O(z02));
  nand2  g16(.a(new_n35_), .b(x00), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(new_n40_), .O(z03));
  nor3   g18(.a(x13), .b(x04), .c(x03), .O(new_n44_));
  nor2   g19(.a(x02), .b(new_n35_), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n44_), .c(new_n34_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n30_), .c(new_n26_), .O(z04));
  nand3  g22(.a(new_n45_), .b(new_n44_), .c(x00), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n30_), .c(new_n26_), .O(z05));
  nand3  g24(.a(new_n38_), .b(new_n26_), .c(x02), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(x00), .c(x01), .O(new_n52_));
  inv1   g27(.a(x08), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n35_), .c(new_n34_), .O(new_n54_));
  inv1   g29(.a(x09), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x00), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(new_n52_), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(x05), .d(new_n28_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(new_n60_));
  nor2   g35(.a(x11), .b(x07), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n60_), .c(new_n57_), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n37_), .O(new_n63_));
  aoi21  g38(.a(new_n62_), .b(new_n50_), .c(new_n63_), .O(z06));
  nand2  g39(.a(new_n44_), .b(new_n28_), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(new_n30_), .c(new_n26_), .O(z07));
  and2   g41(.a(new_n59_), .b(new_n50_), .O(new_n67_));
  nand2  g42(.a(x11), .b(x05), .O(new_n68_));
  oai21  g43(.a(new_n67_), .b(new_n63_), .c(new_n68_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n68_), .O(z09));
  nand2  g46(.a(new_n30_), .b(new_n28_), .O(new_n72_));
  nand2  g47(.a(x06), .b(new_n37_), .O(new_n73_));
  aoi21  g48(.a(new_n72_), .b(x05), .c(new_n73_), .O(z10));
endmodule


