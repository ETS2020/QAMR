// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x06), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nor2   g06(.a(x04), .b(x02), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  oai21  g08(.a(x05), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n30_), .c(new_n26_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x05), .O(new_n37_));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand3  g15(.a(new_n32_), .b(new_n40_), .c(new_n26_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(new_n39_), .c(new_n38_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n28_), .c(new_n37_), .O(z02));
  nor4   g19(.a(new_n41_), .b(new_n29_), .c(x01), .d(new_n38_), .O(z03));
  nand3  g20(.a(new_n42_), .b(x01), .c(new_n38_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n28_), .c(new_n37_), .O(z04));
  nand3  g22(.a(new_n42_), .b(x01), .c(x00), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n28_), .c(new_n37_), .O(z05));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x12), .O(new_n51_));
  nand3  g26(.a(x13), .b(new_n51_), .c(new_n31_), .O(new_n52_));
  nor2   g27(.a(new_n52_), .b(new_n29_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n39_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(new_n38_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n58_), .c(new_n53_), .d(new_n50_), .O(new_n64_));
  nand3  g39(.a(new_n40_), .b(new_n37_), .c(x02), .O(new_n65_));
  nand2  g40(.a(x04), .b(new_n26_), .O(new_n66_));
  aoi21  g41(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(z06));
  aoi21  g42(.a(new_n41_), .b(new_n28_), .c(new_n37_), .O(z07));
  inv1   g43(.a(new_n65_), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(new_n53_), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n66_), .O(z08));
  nand3  g46(.a(x13), .b(x06), .c(new_n37_), .O(new_n72_));
  inv1   g47(.a(new_n72_), .O(z09));
  aoi21  g48(.a(new_n37_), .b(x03), .c(new_n28_), .O(z10));
endmodule


