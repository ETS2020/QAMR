// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_,
    new_n69_, new_n71_, new_n73_;
  nor2   g00(.a(x13), .b(x05), .O(new_n26_));
  aoi21  g01(.a(x05), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nand2  g03(.a(x04), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  nor2   g09(.a(new_n34_), .b(new_n26_), .O(z00));
  oai21  g10(.a(x13), .b(x05), .c(new_n33_), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  inv1   g13(.a(x03), .O(new_n39_));
  nor2   g14(.a(x04), .b(x02), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(x05), .c(x13), .O(z02));
  nand4  g17(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x05), .c(x13), .O(z03));
  inv1   g19(.a(x13), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(x05), .c(new_n39_), .O(new_n46_));
  nand2  g21(.a(new_n40_), .b(x01), .O(new_n47_));
  nor3   g22(.a(new_n47_), .b(new_n46_), .c(x00), .O(z04));
  nor3   g23(.a(new_n47_), .b(new_n46_), .c(new_n37_), .O(z05));
  inv1   g24(.a(x07), .O(new_n50_));
  nor2   g25(.a(new_n30_), .b(x02), .O(new_n51_));
  nor2   g26(.a(new_n31_), .b(x03), .O(new_n52_));
  nor2   g27(.a(new_n45_), .b(x12), .O(new_n53_));
  nand4  g28(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  inv1   g34(.a(x10), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x01), .O(new_n61_));
  inv1   g36(.a(x08), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n38_), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(new_n61_), .c(new_n37_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(new_n54_), .O(z06));
  nor3   g41(.a(new_n46_), .b(x04), .c(x02), .O(z07));
  inv1   g42(.a(new_n52_), .O(new_n68_));
  nand2  g43(.a(new_n53_), .b(new_n51_), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(new_n68_), .O(z08));
  inv1   g45(.a(x06), .O(new_n71_));
  nor2   g46(.a(new_n45_), .b(new_n71_), .O(z09));
  nor2   g47(.a(new_n26_), .b(new_n71_), .O(new_n73_));
  and2   g48(.a(new_n73_), .b(new_n27_), .O(z10));
endmodule


