// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x11), .b(new_n27_), .O(new_n28_));
  inv1   g03(.a(new_n28_), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand3  g06(.a(x05), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand3  g07(.a(new_n27_), .b(x04), .c(x02), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n26_), .c(new_n29_), .O(z00));
  inv1   g10(.a(x11), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n27_), .c(x04), .d(x02), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n32_), .c(x03), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nor3   g15(.a(new_n32_), .b(x13), .c(x03), .O(z07));
  nand3  g16(.a(z07), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n28_), .O(z02));
  nand2  g18(.a(z07), .b(x00), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x01), .O(z03));
  nand3  g20(.a(z07), .b(x01), .c(new_n39_), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(new_n28_), .O(z04));
  nor2   g22(.a(new_n44_), .b(new_n40_), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  inv1   g24(.a(x10), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n40_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(new_n39_), .O(new_n54_));
  nand2  g29(.a(new_n36_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n40_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x12), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(x05), .d(new_n30_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n58_), .c(new_n54_), .d(new_n49_), .O(new_n62_));
  inv1   g37(.a(x13), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(new_n27_), .c(x02), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nor2   g40(.a(new_n31_), .b(x03), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n28_), .O(z06));
  inv1   g43(.a(new_n66_), .O(new_n69_));
  inv1   g44(.a(new_n64_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n36_), .O(new_n71_));
  aoi21  g46(.a(new_n71_), .b(new_n60_), .c(new_n69_), .O(z08));
  nand2  g47(.a(new_n28_), .b(x06), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n63_), .O(z09));
  oai21  g49(.a(new_n27_), .b(new_n30_), .c(new_n26_), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n73_), .O(z10));
endmodule


