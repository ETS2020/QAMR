// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n64_, new_n66_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(z00));
  nor2   g04(.a(new_n27_), .b(x03), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(z01));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand4  g10(.a(new_n28_), .b(new_n35_), .c(new_n34_), .d(new_n33_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(x05), .c(x03), .O(z02));
  nand4  g12(.a(new_n28_), .b(new_n35_), .c(x05), .d(new_n26_), .O(new_n38_));
  nor3   g13(.a(new_n38_), .b(x01), .c(new_n33_), .O(z03));
  nand4  g14(.a(new_n30_), .b(new_n28_), .c(new_n35_), .d(x01), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x00), .O(z04));
  nor2   g16(.a(new_n40_), .b(new_n33_), .O(z05));
  inv1   g17(.a(x10), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(x01), .O(new_n44_));
  inv1   g19(.a(x08), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(new_n44_), .c(new_n33_), .O(new_n47_));
  inv1   g22(.a(x11), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(x01), .O(new_n49_));
  inv1   g24(.a(x09), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(new_n34_), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n49_), .c(x00), .O(new_n52_));
  inv1   g27(.a(x02), .O(new_n53_));
  inv1   g28(.a(x12), .O(new_n54_));
  nand4  g29(.a(x13), .b(new_n54_), .c(x04), .d(new_n53_), .O(new_n55_));
  inv1   g30(.a(new_n55_), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(x05), .c(new_n26_), .O(new_n58_));
  inv1   g33(.a(new_n58_), .O(new_n59_));
  nand4  g34(.a(new_n59_), .b(new_n56_), .c(new_n52_), .d(new_n47_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(z06));
  inv1   g36(.a(new_n38_), .O(z07));
  aoi21  g37(.a(new_n55_), .b(x05), .c(x03), .O(z08));
  nand2  g38(.a(x13), .b(x06), .O(new_n64_));
  aoi21  g39(.a(new_n27_), .b(new_n26_), .c(new_n64_), .O(z09));
  nand2  g40(.a(x06), .b(new_n53_), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(x05), .c(x03), .O(z10));
endmodule


