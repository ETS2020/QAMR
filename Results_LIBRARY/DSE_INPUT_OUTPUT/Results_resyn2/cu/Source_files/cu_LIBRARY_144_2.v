// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x03), .O(new_n26_));
  nand2  g01(.a(x04), .b(x02), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(x05), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(x05), .O(new_n30_));
  inv1   g05(.a(new_n30_), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x09), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  oai21  g08(.a(new_n33_), .b(new_n28_), .c(new_n26_), .O(z00));
  oai21  g09(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n35_));
  inv1   g10(.a(x09), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(x05), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n35_), .O(z01));
  inv1   g13(.a(x05), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nor2   g16(.a(x13), .b(x03), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n29_), .c(new_n41_), .d(new_n40_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x09), .c(new_n39_), .O(z02));
  inv1   g19(.a(new_n42_), .O(new_n45_));
  nand2  g20(.a(new_n41_), .b(x00), .O(new_n46_));
  nor3   g21(.a(new_n46_), .b(new_n45_), .c(new_n32_), .O(z03));
  nor2   g22(.a(new_n45_), .b(x04), .O(new_n48_));
  nor2   g23(.a(x02), .b(new_n41_), .O(new_n49_));
  nand3  g24(.a(new_n49_), .b(new_n48_), .c(new_n40_), .O(new_n50_));
  aoi21  g25(.a(new_n50_), .b(x09), .c(new_n39_), .O(z04));
  nand3  g26(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(x09), .c(new_n39_), .O(z05));
  nand2  g28(.a(x10), .b(x01), .O(new_n54_));
  nand2  g29(.a(x08), .b(new_n41_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n54_), .c(new_n40_), .O(new_n56_));
  inv1   g31(.a(x11), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(x01), .c(x00), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  inv1   g35(.a(x12), .O(new_n61_));
  nand4  g36(.a(x13), .b(new_n61_), .c(x05), .d(new_n60_), .O(new_n62_));
  nor2   g37(.a(new_n62_), .b(x07), .O(new_n63_));
  inv1   g38(.a(x13), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(new_n39_), .c(x02), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  aoi21  g41(.a(new_n63_), .b(new_n59_), .c(new_n66_), .O(new_n67_));
  nand2  g42(.a(x04), .b(new_n26_), .O(new_n68_));
  oai21  g43(.a(new_n68_), .b(new_n67_), .c(new_n37_), .O(z06));
  nor2   g44(.a(new_n45_), .b(new_n32_), .O(z07));
  and2   g45(.a(new_n65_), .b(new_n62_), .O(new_n71_));
  oai21  g46(.a(new_n71_), .b(new_n68_), .c(new_n37_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n37_), .O(z09));
  nand2  g49(.a(x09), .b(new_n60_), .O(new_n75_));
  nand2  g50(.a(x06), .b(new_n26_), .O(new_n76_));
  aoi21  g51(.a(new_n75_), .b(x05), .c(new_n76_), .O(z10));
endmodule


