// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_, new_n71_, new_n73_, new_n75_;
  inv1   g00(.a(x09), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand3  g02(.a(new_n27_), .b(x04), .c(x02), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  aoi21  g04(.a(x05), .b(x02), .c(x03), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nand2  g06(.a(new_n27_), .b(new_n31_), .O(new_n32_));
  inv1   g07(.a(x02), .O(new_n33_));
  nand2  g08(.a(x04), .b(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n32_), .c(new_n30_), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n29_), .O(z00));
  nand2  g12(.a(new_n26_), .b(x05), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n35_), .O(z01));
  inv1   g14(.a(x01), .O(new_n40_));
  nand3  g15(.a(new_n31_), .b(new_n33_), .c(new_n40_), .O(new_n41_));
  nor2   g16(.a(x13), .b(x03), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(x09), .c(x05), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(new_n41_), .c(x00), .O(z02));
  inv1   g19(.a(x00), .O(new_n45_));
  nor3   g20(.a(new_n43_), .b(new_n41_), .c(new_n45_), .O(z03));
  nand3  g21(.a(new_n42_), .b(new_n31_), .c(new_n33_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(x01), .c(new_n45_), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x09), .c(new_n27_), .O(z04));
  nand2  g25(.a(x09), .b(x05), .O(new_n51_));
  nor4   g26(.a(new_n47_), .b(new_n51_), .c(new_n40_), .d(new_n45_), .O(z05));
  inv1   g27(.a(x07), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(x01), .c(x00), .O(new_n55_));
  nand2  g30(.a(x10), .b(x01), .O(new_n56_));
  nand2  g31(.a(x08), .b(new_n40_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n56_), .c(new_n45_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand3  g35(.a(x13), .b(new_n60_), .c(new_n33_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(new_n51_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n59_), .c(new_n53_), .O(new_n63_));
  inv1   g38(.a(x13), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(new_n27_), .c(x02), .O(new_n65_));
  inv1   g40(.a(x03), .O(new_n66_));
  nand2  g41(.a(x04), .b(new_n66_), .O(new_n67_));
  aoi21  g42(.a(new_n65_), .b(new_n63_), .c(new_n67_), .O(z06));
  aoi21  g43(.a(new_n47_), .b(x09), .c(new_n27_), .O(z07));
  inv1   g44(.a(new_n65_), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n62_), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n67_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  aoi21  g48(.a(new_n26_), .b(x05), .c(new_n73_), .O(z09));
  nand2  g49(.a(new_n30_), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n38_), .O(z10));
endmodule


