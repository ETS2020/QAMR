// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n76_,
    new_n78_;
  inv1   g00(.a(x10), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x09), .O(new_n27_));
  aoi21  g02(.a(x05), .b(x02), .c(x03), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(new_n34_));
  and2   g09(.a(new_n34_), .b(new_n27_), .O(z00));
  inv1   g10(.a(new_n34_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z01));
  nor2   g13(.a(new_n30_), .b(x02), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor2   g16(.a(x01), .b(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n41_), .c(new_n27_), .O(z02));
  inv1   g19(.a(x00), .O(new_n45_));
  nor2   g20(.a(x01), .b(new_n45_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n40_), .c(new_n39_), .d(new_n27_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z03));
  inv1   g23(.a(x01), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(x00), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n40_), .c(new_n39_), .d(new_n27_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(z04));
  nand2  g27(.a(x01), .b(x00), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(new_n41_), .c(new_n27_), .O(z05));
  inv1   g29(.a(x12), .O(new_n55_));
  nand4  g30(.a(x13), .b(new_n55_), .c(x05), .d(new_n32_), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  aoi21  g32(.a(new_n42_), .b(x08), .c(x07), .O(new_n58_));
  oai21  g33(.a(x01), .b(new_n45_), .c(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x09), .O(new_n60_));
  inv1   g35(.a(x11), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x00), .O(new_n62_));
  nand2  g37(.a(new_n26_), .b(new_n45_), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(new_n62_), .c(x01), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n60_), .c(new_n58_), .d(new_n57_), .O(new_n65_));
  inv1   g40(.a(x13), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(new_n30_), .c(x02), .O(new_n67_));
  inv1   g42(.a(new_n67_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n27_), .O(new_n69_));
  inv1   g44(.a(x03), .O(new_n70_));
  nand2  g45(.a(x04), .b(new_n70_), .O(new_n71_));
  aoi21  g46(.a(new_n69_), .b(new_n65_), .c(new_n71_), .O(z06));
  nand2  g47(.a(new_n41_), .b(new_n27_), .O(z07));
  nor2   g48(.a(new_n68_), .b(new_n57_), .O(new_n74_));
  oai21  g49(.a(new_n74_), .b(new_n71_), .c(new_n27_), .O(z08));
  nand2  g50(.a(x13), .b(x06), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n27_), .O(z09));
  nand2  g52(.a(new_n28_), .b(x06), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n27_), .O(z10));
endmodule


