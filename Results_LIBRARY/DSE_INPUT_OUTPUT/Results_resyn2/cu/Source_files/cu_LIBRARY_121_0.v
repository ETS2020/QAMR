// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n67_, new_n69_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(z00));
  nor2   g04(.a(new_n27_), .b(x03), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(z01));
  inv1   g07(.a(x13), .O(new_n33_));
  nand2  g08(.a(new_n30_), .b(new_n33_), .O(new_n34_));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  nand3  g11(.a(new_n28_), .b(new_n36_), .c(new_n35_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(new_n34_), .O(z02));
  nand3  g13(.a(new_n28_), .b(new_n36_), .c(x00), .O(new_n39_));
  nor2   g14(.a(new_n39_), .b(new_n34_), .O(z03));
  nand4  g15(.a(new_n28_), .b(new_n33_), .c(x01), .d(new_n35_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(x05), .c(x03), .O(z04));
  nand4  g17(.a(new_n28_), .b(new_n33_), .c(x01), .d(x00), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x05), .c(x03), .O(z05));
  inv1   g19(.a(x10), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g21(.a(x08), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n46_), .c(new_n35_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x02), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(x04), .d(new_n55_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(x05), .c(new_n26_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n58_), .c(new_n54_), .d(new_n49_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(z06));
  nand2  g38(.a(new_n28_), .b(new_n33_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(x05), .c(x03), .O(z07));
  aoi21  g40(.a(new_n57_), .b(x05), .c(x03), .O(z08));
  nand2  g41(.a(x13), .b(x06), .O(new_n67_));
  oai21  g42(.a(x05), .b(x03), .c(new_n67_), .O(z09));
  nand2  g43(.a(x06), .b(new_n55_), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(x05), .c(x03), .O(z10));
endmodule


