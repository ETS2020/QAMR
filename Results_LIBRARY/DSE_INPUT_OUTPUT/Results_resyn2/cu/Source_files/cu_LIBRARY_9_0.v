// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n73_, new_n75_, new_n76_,
    new_n78_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nor2   g05(.a(new_n27_), .b(new_n30_), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n29_), .c(new_n26_), .O(z00));
  nor2   g08(.a(x04), .b(x03), .O(new_n34_));
  nor2   g09(.a(new_n34_), .b(new_n27_), .O(new_n35_));
  inv1   g10(.a(x04), .O(new_n36_));
  nor2   g11(.a(new_n36_), .b(x03), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  nor2   g13(.a(new_n38_), .b(new_n35_), .O(z01));
  inv1   g14(.a(x01), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nor2   g16(.a(x03), .b(x02), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n41_), .c(x05), .d(new_n36_), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(x00), .O(new_n44_));
  nand2  g19(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(z02));
  inv1   g21(.a(x00), .O(new_n47_));
  nor3   g22(.a(new_n43_), .b(x01), .c(new_n47_), .O(z03));
  nand2  g23(.a(new_n44_), .b(x01), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z04));
  nand4  g25(.a(new_n34_), .b(new_n41_), .c(x01), .d(x00), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(x05), .c(x02), .O(z05));
  inv1   g27(.a(x07), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n40_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(new_n58_));
  inv1   g33(.a(x11), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n40_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x04), .d(new_n26_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n63_), .c(new_n58_), .d(new_n53_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(x05), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n30_), .O(new_n69_));
  nand3  g44(.a(new_n37_), .b(new_n41_), .c(new_n27_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(z06));
  inv1   g46(.a(new_n43_), .O(z07));
  oai21  g47(.a(new_n66_), .b(new_n27_), .c(new_n30_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n70_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n27_), .b(new_n30_), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n75_), .O(z09));
  nand2  g52(.a(x06), .b(new_n26_), .O(new_n78_));
  aoi21  g53(.a(new_n78_), .b(new_n76_), .c(new_n31_), .O(z10));
endmodule


