// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x05), .b(new_n27_), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  aoi21  g04(.a(x05), .b(x02), .c(x03), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  inv1   g09(.a(new_n28_), .O(new_n35_));
  nor3   g10(.a(x13), .b(x03), .c(x02), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n33_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z02));
  nand4  g13(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(x00), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z03));
  inv1   g15(.a(x05), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(x03), .O(new_n42_));
  nand4  g17(.a(new_n36_), .b(new_n35_), .c(x01), .d(new_n33_), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n42_), .O(z04));
  nand4  g19(.a(new_n36_), .b(new_n35_), .c(x01), .d(x00), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(new_n42_), .O(z05));
  inv1   g21(.a(x13), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(new_n41_), .c(x02), .O(new_n48_));
  inv1   g23(.a(x10), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n50_), .c(new_n33_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  nand2  g33(.a(x13), .b(new_n26_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n61_), .c(x05), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n60_), .c(new_n58_), .d(new_n53_), .O(new_n65_));
  inv1   g40(.a(x03), .O(new_n66_));
  nand2  g41(.a(x04), .b(new_n66_), .O(new_n67_));
  aoi21  g42(.a(new_n65_), .b(new_n48_), .c(new_n67_), .O(z06));
  nand2  g43(.a(new_n36_), .b(new_n35_), .O(new_n69_));
  nand2  g44(.a(new_n42_), .b(new_n69_), .O(z07));
  nand3  g45(.a(new_n62_), .b(x05), .c(new_n66_), .O(new_n71_));
  oai21  g46(.a(new_n71_), .b(new_n59_), .c(new_n48_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(x04), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n42_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n42_), .O(z09));
  nand2  g51(.a(new_n30_), .b(x06), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z10));
endmodule


