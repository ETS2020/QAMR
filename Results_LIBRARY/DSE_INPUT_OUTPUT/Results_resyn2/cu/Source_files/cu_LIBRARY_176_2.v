// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x00), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  nor2   g04(.a(x05), .b(new_n29_), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  aoi21  g06(.a(new_n26_), .b(new_n31_), .c(x03), .O(new_n32_));
  oai21  g07(.a(new_n30_), .b(new_n28_), .c(new_n32_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n27_), .O(z01));
  inv1   g09(.a(z01), .O(z00));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nor2   g12(.a(new_n26_), .b(x03), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n28_), .c(new_n37_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z07));
  nand2  g15(.a(z07), .b(new_n36_), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(x01), .c(new_n27_), .O(z02));
  inv1   g17(.a(x01), .O(new_n43_));
  inv1   g18(.a(x03), .O(new_n44_));
  nand4  g19(.a(new_n28_), .b(new_n37_), .c(new_n44_), .d(new_n43_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x05), .c(new_n36_), .O(z03));
  oai21  g21(.a(new_n41_), .b(new_n43_), .c(new_n27_), .O(z04));
  nor3   g22(.a(new_n39_), .b(new_n43_), .c(new_n36_), .O(z05));
  nand2  g23(.a(new_n30_), .b(new_n37_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  nand2  g25(.a(x10), .b(x01), .O(new_n51_));
  nand2  g26(.a(x08), .b(new_n43_), .O(new_n52_));
  nand4  g27(.a(new_n52_), .b(new_n51_), .c(x05), .d(new_n36_), .O(new_n53_));
  nand2  g28(.a(x11), .b(x01), .O(new_n54_));
  nand2  g29(.a(x09), .b(new_n43_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n54_), .c(x00), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(new_n57_), .d(new_n29_), .O(new_n59_));
  aoi21  g34(.a(new_n56_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  nor2   g35(.a(new_n31_), .b(x03), .O(new_n61_));
  oai21  g36(.a(new_n60_), .b(new_n50_), .c(new_n61_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n27_), .O(z06));
  nand4  g38(.a(x13), .b(new_n58_), .c(x05), .d(new_n29_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n49_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n61_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n27_), .O(z08));
  nand2  g42(.a(x13), .b(x06), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n27_), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n70_));
  nand3  g45(.a(new_n70_), .b(x06), .c(new_n44_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n27_), .O(z10));
endmodule


