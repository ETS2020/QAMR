// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_;
  inv1   g00(.a(x04), .O(new_n26_));
  nor2   g01(.a(x03), .b(x02), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  nor2   g04(.a(x05), .b(x03), .O(new_n30_));
  aoi22  g05(.a(new_n30_), .b(new_n29_), .c(new_n28_), .d(x05), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  inv1   g07(.a(x05), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(x03), .O(new_n34_));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  nor3   g11(.a(new_n28_), .b(x13), .c(new_n33_), .O(z07));
  nand3  g12(.a(z07), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n34_), .O(z02));
  nand3  g14(.a(z07), .b(new_n36_), .c(x00), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z03));
  nand3  g16(.a(z07), .b(x01), .c(new_n35_), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n34_), .O(z04));
  nand3  g18(.a(z07), .b(x01), .c(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(z05));
  inv1   g20(.a(x07), .O(new_n46_));
  nor2   g21(.a(x12), .b(new_n33_), .O(new_n47_));
  nand4  g22(.a(new_n47_), .b(new_n27_), .c(x13), .d(new_n46_), .O(new_n48_));
  inv1   g23(.a(x11), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  inv1   g25(.a(x09), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n36_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n50_), .c(x00), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n36_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(new_n35_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  inv1   g34(.a(x13), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n33_), .c(x02), .O(new_n61_));
  oai21  g36(.a(new_n59_), .b(new_n48_), .c(new_n61_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(x04), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n34_), .O(z06));
  nand3  g39(.a(new_n47_), .b(new_n27_), .c(x13), .O(new_n65_));
  nand2  g40(.a(new_n61_), .b(new_n65_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(x04), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n34_), .O(z08));
  inv1   g43(.a(x06), .O(new_n69_));
  oai21  g44(.a(new_n60_), .b(new_n69_), .c(new_n34_), .O(z09));
  inv1   g45(.a(new_n27_), .O(new_n71_));
  nor2   g46(.a(x06), .b(x03), .O(new_n72_));
  aoi21  g47(.a(new_n71_), .b(x05), .c(new_n72_), .O(z10));
endmodule


