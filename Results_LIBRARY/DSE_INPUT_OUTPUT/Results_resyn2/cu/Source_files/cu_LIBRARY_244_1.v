// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n72_, new_n74_, new_n76_, new_n77_;
  inv1   g00(.a(x09), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x07), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  oai21  g05(.a(new_n29_), .b(x02), .c(new_n30_), .O(new_n31_));
  aoi21  g06(.a(x05), .b(x04), .c(x03), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n31_), .c(new_n28_), .O(z00));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n28_), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  nor2   g11(.a(new_n29_), .b(x02), .O(new_n37_));
  nor3   g12(.a(x13), .b(x04), .c(x03), .O(new_n38_));
  nor2   g13(.a(new_n27_), .b(x01), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z02));
  nand4  g16(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z03));
  nand2  g18(.a(new_n38_), .b(new_n37_), .O(new_n44_));
  nand2  g19(.a(x01), .b(new_n36_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n44_), .c(new_n28_), .O(z04));
  nand2  g21(.a(x01), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n44_), .c(new_n28_), .O(z05));
  inv1   g23(.a(x03), .O(new_n49_));
  nand2  g24(.a(x04), .b(new_n49_), .O(new_n50_));
  inv1   g25(.a(x13), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n29_), .c(x02), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n28_), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(x01), .c(new_n36_), .O(new_n56_));
  inv1   g31(.a(x01), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  aoi21  g33(.a(new_n58_), .b(x00), .c(new_n57_), .O(new_n59_));
  inv1   g34(.a(x08), .O(new_n60_));
  nor2   g35(.a(new_n60_), .b(x00), .O(new_n61_));
  oai21  g36(.a(new_n61_), .b(new_n59_), .c(new_n56_), .O(new_n62_));
  inv1   g37(.a(x02), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x05), .d(new_n63_), .O(new_n65_));
  inv1   g40(.a(x07), .O(new_n66_));
  nand2  g41(.a(new_n26_), .b(new_n66_), .O(new_n67_));
  nor2   g42(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(new_n54_), .c(new_n50_), .O(z06));
  nand2  g45(.a(new_n44_), .b(new_n28_), .O(z07));
  and2   g46(.a(new_n65_), .b(new_n52_), .O(new_n72_));
  oai21  g47(.a(new_n72_), .b(new_n50_), .c(new_n28_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  nor2   g49(.a(new_n74_), .b(new_n27_), .O(z09));
  nand2  g50(.a(x05), .b(x02), .O(new_n76_));
  nand3  g51(.a(new_n76_), .b(x06), .c(new_n49_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n28_), .O(z10));
endmodule


