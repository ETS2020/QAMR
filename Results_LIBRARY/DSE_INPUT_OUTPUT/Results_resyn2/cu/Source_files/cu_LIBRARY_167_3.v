// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n70_;
  inv1   g00(.a(x01), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g03(.a(x04), .b(x02), .c(x05), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n27_), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(new_n28_), .O(z00));
  oai21  g07(.a(x02), .b(x01), .c(x05), .O(new_n33_));
  nand2  g08(.a(x05), .b(x04), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n33_), .c(new_n31_), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(z01));
  inv1   g11(.a(x02), .O(new_n37_));
  nand3  g12(.a(x05), .b(new_n37_), .c(new_n26_), .O(new_n38_));
  inv1   g13(.a(x03), .O(new_n39_));
  inv1   g14(.a(x04), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nor3   g17(.a(new_n42_), .b(new_n38_), .c(x00), .O(z02));
  nand4  g18(.a(new_n41_), .b(new_n40_), .c(new_n39_), .d(new_n37_), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x00), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n26_), .c(new_n27_), .O(z03));
  inv1   g22(.a(x07), .O(new_n50_));
  nand3  g23(.a(new_n41_), .b(new_n27_), .c(x02), .O(new_n51_));
  inv1   g24(.a(new_n51_), .O(new_n52_));
  inv1   g25(.a(x00), .O(new_n53_));
  inv1   g26(.a(x08), .O(new_n54_));
  nand2  g27(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g28(.a(x09), .O(new_n56_));
  nand2  g29(.a(new_n56_), .b(x00), .O(new_n57_));
  nand2  g30(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  aoi21  g31(.a(new_n58_), .b(new_n50_), .c(new_n52_), .O(new_n59_));
  inv1   g32(.a(x12), .O(new_n60_));
  nand2  g33(.a(x13), .b(new_n60_), .O(new_n61_));
  oai21  g34(.a(new_n61_), .b(new_n38_), .c(new_n51_), .O(new_n62_));
  nor2   g35(.a(new_n40_), .b(x03), .O(new_n63_));
  nand2  g36(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g37(.a(new_n64_), .b(new_n59_), .O(z06));
  aoi21  g38(.a(new_n44_), .b(new_n26_), .c(new_n27_), .O(z07));
  inv1   g39(.a(new_n64_), .O(z08));
  inv1   g40(.a(x06), .O(new_n68_));
  nor3   g41(.a(new_n28_), .b(new_n41_), .c(new_n68_), .O(z09));
  nor2   g42(.a(new_n68_), .b(x03), .O(new_n70_));
  and2   g43(.a(new_n70_), .b(new_n33_), .O(z10));
  zero   g44(.O(z04));
  zero   g45(.O(z05));
endmodule


