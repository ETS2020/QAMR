// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(x04), .b(x02), .O(new_n27_));
  oai21  g02(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  aoi21  g03(.a(x05), .b(x02), .c(x03), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(z00));
  inv1   g05(.a(z00), .O(z01));
  inv1   g06(.a(x00), .O(new_n32_));
  inv1   g07(.a(x02), .O(new_n33_));
  inv1   g08(.a(x03), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  inv1   g11(.a(x04), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(x05), .c(new_n37_), .d(new_n36_), .O(new_n39_));
  nor2   g14(.a(new_n39_), .b(new_n35_), .O(z02));
  nand3  g15(.a(new_n34_), .b(new_n33_), .c(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n39_), .O(z03));
  nand4  g17(.a(new_n38_), .b(x05), .c(new_n37_), .d(x01), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n35_), .O(z04));
  nor2   g19(.a(new_n43_), .b(new_n41_), .O(z05));
  nand2  g20(.a(x04), .b(new_n34_), .O(new_n46_));
  inv1   g21(.a(x10), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x01), .O(new_n48_));
  inv1   g23(.a(x08), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n48_), .c(new_n32_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  nor2   g31(.a(new_n38_), .b(new_n26_), .O(new_n57_));
  nor3   g32(.a(x12), .b(x07), .c(x02), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n51_), .O(new_n59_));
  nand3  g34(.a(new_n38_), .b(new_n26_), .c(x02), .O(new_n60_));
  aoi21  g35(.a(new_n60_), .b(new_n59_), .c(new_n46_), .O(z06));
  nand3  g36(.a(new_n38_), .b(new_n37_), .c(new_n34_), .O(new_n62_));
  aoi21  g37(.a(new_n62_), .b(new_n33_), .c(new_n26_), .O(z07));
  oai21  g38(.a(new_n46_), .b(x13), .c(new_n26_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(x02), .O(new_n65_));
  inv1   g40(.a(x12), .O(new_n66_));
  nand4  g41(.a(new_n57_), .b(new_n66_), .c(x04), .d(new_n34_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n65_), .O(z08));
  nand2  g43(.a(x13), .b(x06), .O(new_n69_));
  oai21  g44(.a(new_n26_), .b(new_n33_), .c(new_n69_), .O(z09));
  nand2  g45(.a(new_n29_), .b(x06), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
endmodule


