// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_;
  inv1   g00(.a(x04), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x03), .O(new_n27_));
  nor2   g02(.a(x04), .b(x03), .O(new_n28_));
  oai22  g03(.a(new_n28_), .b(x02), .c(new_n27_), .d(x05), .O(z00));
  inv1   g04(.a(z00), .O(z01));
  inv1   g05(.a(x00), .O(new_n31_));
  inv1   g06(.a(x13), .O(new_n32_));
  inv1   g07(.a(x05), .O(new_n33_));
  nor2   g08(.a(new_n33_), .b(x02), .O(new_n34_));
  nand4  g09(.a(new_n34_), .b(new_n28_), .c(new_n32_), .d(new_n31_), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(x01), .O(z02));
  nand4  g11(.a(new_n34_), .b(new_n28_), .c(new_n32_), .d(x00), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(x01), .O(z03));
  inv1   g13(.a(x01), .O(new_n39_));
  nor2   g14(.a(new_n35_), .b(new_n39_), .O(z04));
  nor2   g15(.a(new_n37_), .b(new_n39_), .O(z05));
  inv1   g16(.a(new_n27_), .O(new_n42_));
  inv1   g17(.a(x10), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(x01), .O(new_n44_));
  inv1   g19(.a(x08), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(new_n44_), .c(new_n31_), .O(new_n47_));
  inv1   g22(.a(x11), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(x01), .O(new_n49_));
  inv1   g24(.a(x09), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(new_n39_), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n49_), .c(x00), .O(new_n52_));
  nor2   g27(.a(x12), .b(new_n33_), .O(new_n53_));
  inv1   g28(.a(x02), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  nand3  g30(.a(x13), .b(new_n55_), .c(new_n54_), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  nand4  g32(.a(new_n57_), .b(new_n53_), .c(new_n52_), .d(new_n47_), .O(new_n58_));
  nand3  g33(.a(new_n32_), .b(new_n33_), .c(x02), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n58_), .c(new_n42_), .O(z06));
  inv1   g35(.a(new_n28_), .O(new_n61_));
  nand2  g36(.a(new_n34_), .b(new_n32_), .O(new_n62_));
  nor2   g37(.a(new_n62_), .b(new_n61_), .O(z07));
  aoi21  g38(.a(new_n27_), .b(new_n32_), .c(x05), .O(new_n64_));
  nand3  g39(.a(new_n53_), .b(new_n27_), .c(x13), .O(new_n65_));
  oai21  g40(.a(new_n64_), .b(new_n54_), .c(new_n65_), .O(z08));
  inv1   g41(.a(x06), .O(new_n67_));
  nand2  g42(.a(x05), .b(x02), .O(new_n68_));
  oai21  g43(.a(new_n32_), .b(new_n67_), .c(new_n68_), .O(z09));
  oai21  g44(.a(new_n67_), .b(x03), .c(new_n68_), .O(z10));
endmodule


