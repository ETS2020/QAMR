// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n73_, new_n75_;
  nand2  g00(.a(x12), .b(x07), .O(new_n26_));
  aoi21  g01(.a(x05), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n27_), .O(new_n33_));
  and2   g08(.a(new_n33_), .b(new_n26_), .O(z00));
  inv1   g09(.a(new_n33_), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  inv1   g12(.a(x01), .O(new_n38_));
  nor2   g13(.a(new_n29_), .b(x02), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n26_), .d(new_n38_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x00), .O(z02));
  inv1   g17(.a(x00), .O(new_n43_));
  nor2   g18(.a(new_n41_), .b(new_n43_), .O(z03));
  nand2  g19(.a(new_n40_), .b(new_n39_), .O(new_n45_));
  nand2  g20(.a(x01), .b(new_n43_), .O(new_n46_));
  oai21  g21(.a(new_n46_), .b(new_n45_), .c(new_n26_), .O(z04));
  nor2   g22(.a(new_n38_), .b(new_n43_), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n40_), .c(new_n39_), .d(new_n26_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z05));
  inv1   g25(.a(x13), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n29_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x07), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n38_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n38_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(new_n43_), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x05), .d(new_n31_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n63_), .c(new_n58_), .d(new_n53_), .O(new_n67_));
  inv1   g42(.a(x03), .O(new_n68_));
  nand3  g43(.a(new_n26_), .b(x04), .c(new_n68_), .O(new_n69_));
  aoi21  g44(.a(new_n67_), .b(new_n52_), .c(new_n69_), .O(z06));
  nand2  g45(.a(new_n45_), .b(new_n26_), .O(z07));
  aoi21  g46(.a(new_n65_), .b(new_n52_), .c(new_n69_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n26_), .O(z09));
  nand2  g49(.a(new_n27_), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n26_), .O(z10));
endmodule


