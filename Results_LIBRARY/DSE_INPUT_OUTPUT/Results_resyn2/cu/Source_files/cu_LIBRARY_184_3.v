// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n75_,
    new_n77_, new_n79_;
  nand2  g00(.a(x10), .b(x09), .O(new_n26_));
  aoi21  g01(.a(x05), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n27_), .O(new_n33_));
  and2   g08(.a(new_n33_), .b(new_n26_), .O(z00));
  inv1   g09(.a(new_n33_), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  nor2   g12(.a(new_n29_), .b(x02), .O(new_n38_));
  nor3   g13(.a(x13), .b(x04), .c(x03), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x01), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n40_), .c(new_n26_), .O(z02));
  nor2   g19(.a(x01), .b(new_n41_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n39_), .c(new_n38_), .d(new_n26_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  nor2   g22(.a(new_n42_), .b(x00), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n39_), .c(new_n38_), .d(new_n26_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z04));
  nand2  g25(.a(x01), .b(x00), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n40_), .c(new_n26_), .O(z05));
  inv1   g27(.a(x09), .O(new_n53_));
  nor2   g28(.a(x08), .b(x01), .O(new_n54_));
  oai21  g29(.a(new_n54_), .b(x00), .c(new_n53_), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nor2   g31(.a(x08), .b(x00), .O(new_n57_));
  oai21  g32(.a(new_n57_), .b(x01), .c(new_n56_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n31_), .O(new_n61_));
  inv1   g36(.a(x07), .O(new_n62_));
  inv1   g37(.a(x11), .O(new_n63_));
  oai21  g38(.a(new_n51_), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  inv1   g41(.a(x13), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(new_n29_), .c(x02), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n26_), .O(new_n70_));
  inv1   g45(.a(x03), .O(new_n71_));
  nand2  g46(.a(x04), .b(new_n71_), .O(new_n72_));
  aoi21  g47(.a(new_n70_), .b(new_n66_), .c(new_n72_), .O(z06));
  nand2  g48(.a(new_n40_), .b(new_n26_), .O(z07));
  and2   g49(.a(new_n68_), .b(new_n61_), .O(new_n75_));
  oai21  g50(.a(new_n75_), .b(new_n72_), .c(new_n26_), .O(z08));
  nand2  g51(.a(x13), .b(x06), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n26_), .O(z09));
  nand2  g53(.a(new_n27_), .b(x06), .O(new_n79_));
  nand2  g54(.a(new_n79_), .b(new_n26_), .O(z10));
endmodule


