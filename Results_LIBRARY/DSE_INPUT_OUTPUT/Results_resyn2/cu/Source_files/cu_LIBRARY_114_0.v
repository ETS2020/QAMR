// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x05), .O(new_n26_));
  inv1   g01(.a(x13), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n26_), .b(x02), .c(new_n29_), .O(new_n30_));
  and2   g05(.a(x05), .b(x04), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(x03), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(z00));
  nand2  g08(.a(new_n32_), .b(new_n30_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n28_), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  nor3   g12(.a(x04), .b(x03), .c(x02), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  aoi21  g14(.a(new_n39_), .b(x05), .c(x13), .O(z02));
  nor2   g15(.a(x13), .b(new_n26_), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n38_), .c(new_n37_), .d(x00), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z03));
  nand2  g18(.a(new_n41_), .b(new_n38_), .O(new_n44_));
  nand2  g19(.a(x01), .b(new_n36_), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n44_), .O(z04));
  nand4  g21(.a(new_n41_), .b(new_n38_), .c(x01), .d(x00), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  nor2   g24(.a(x03), .b(x02), .O(new_n50_));
  nor2   g25(.a(new_n27_), .b(x12), .O(new_n51_));
  nand4  g26(.a(new_n51_), .b(new_n50_), .c(new_n31_), .d(new_n49_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x09), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n37_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(x00), .O(new_n57_));
  inv1   g32(.a(x10), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g34(.a(x08), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n37_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n59_), .c(new_n36_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n52_), .c(new_n28_), .O(z06));
  nor2   g39(.a(new_n38_), .b(new_n26_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(x13), .O(z07));
  nand3  g41(.a(new_n51_), .b(new_n50_), .c(new_n31_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n28_), .O(z08));
  and2   g43(.a(x13), .b(x06), .O(z09));
  inv1   g44(.a(x03), .O(new_n70_));
  nand2  g45(.a(x05), .b(x02), .O(new_n71_));
  nand4  g46(.a(new_n71_), .b(new_n28_), .c(x06), .d(new_n70_), .O(new_n72_));
  inv1   g47(.a(new_n72_), .O(z10));
endmodule


