// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x10), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x05), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  nor2   g04(.a(x05), .b(new_n29_), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n31_), .c(x03), .O(new_n33_));
  oai21  g08(.a(new_n30_), .b(new_n28_), .c(new_n33_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n27_), .O(z01));
  inv1   g10(.a(z01), .O(z00));
  nor2   g11(.a(x01), .b(x00), .O(new_n37_));
  nor2   g12(.a(x13), .b(x03), .O(new_n38_));
  nor2   g13(.a(new_n32_), .b(x02), .O(new_n39_));
  nor2   g14(.a(new_n26_), .b(x04), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z02));
  nand4  g17(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(x01), .O(z03));
  inv1   g19(.a(x01), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(x00), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n40_), .c(new_n39_), .d(new_n38_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z04));
  nor2   g23(.a(new_n43_), .b(new_n45_), .O(z05));
  inv1   g24(.a(x13), .O(new_n50_));
  nand2  g25(.a(new_n30_), .b(new_n50_), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n37_), .b(new_n52_), .O(new_n53_));
  nand2  g28(.a(x11), .b(x01), .O(new_n54_));
  nand2  g29(.a(x09), .b(new_n45_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n54_), .c(x00), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(x05), .d(new_n29_), .O(new_n59_));
  inv1   g34(.a(x07), .O(new_n60_));
  nand2  g35(.a(x10), .b(new_n60_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  inv1   g38(.a(x03), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n64_), .O(new_n65_));
  aoi21  g40(.a(new_n63_), .b(new_n51_), .c(new_n65_), .O(z06));
  nand2  g41(.a(new_n38_), .b(new_n28_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(x10), .c(new_n32_), .O(z07));
  nand2  g43(.a(new_n59_), .b(new_n51_), .O(new_n69_));
  nand3  g44(.a(new_n69_), .b(x04), .c(new_n64_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n27_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n27_), .O(z09));
  nand2  g48(.a(x05), .b(x02), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x06), .c(new_n64_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n27_), .O(z10));
endmodule


