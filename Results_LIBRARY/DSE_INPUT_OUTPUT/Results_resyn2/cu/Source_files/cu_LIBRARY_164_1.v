// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(x13), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nor2   g05(.a(x05), .b(new_n30_), .O(new_n31_));
  or2    g06(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  inv1   g07(.a(x04), .O(new_n33_));
  aoi21  g08(.a(new_n26_), .b(new_n33_), .c(x03), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n32_), .c(new_n28_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  nor2   g13(.a(new_n26_), .b(x03), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n29_), .c(new_n38_), .d(new_n37_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x01), .O(z02));
  nand4  g16(.a(new_n39_), .b(new_n29_), .c(new_n38_), .d(x00), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(x01), .O(z03));
  inv1   g18(.a(x01), .O(new_n44_));
  oai21  g19(.a(new_n40_), .b(new_n44_), .c(new_n27_), .O(z04));
  oai21  g20(.a(new_n42_), .b(new_n44_), .c(new_n27_), .O(z05));
  inv1   g21(.a(x08), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n44_), .c(x00), .O(new_n48_));
  oai21  g23(.a(x10), .b(new_n44_), .c(new_n48_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(new_n44_), .c(new_n37_), .O(new_n53_));
  inv1   g28(.a(x07), .O(new_n54_));
  inv1   g29(.a(x12), .O(new_n55_));
  nand4  g30(.a(x13), .b(new_n55_), .c(new_n54_), .d(new_n30_), .O(new_n56_));
  aoi21  g31(.a(new_n53_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n49_), .c(new_n31_), .O(new_n58_));
  inv1   g33(.a(x03), .O(new_n59_));
  nand2  g34(.a(x04), .b(new_n59_), .O(new_n60_));
  oai21  g35(.a(new_n60_), .b(new_n58_), .c(new_n27_), .O(z06));
  nand3  g36(.a(new_n39_), .b(new_n29_), .c(new_n38_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n27_), .O(z07));
  nor2   g38(.a(x12), .b(x02), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(x13), .c(new_n31_), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n60_), .c(new_n27_), .O(z08));
  nand3  g41(.a(x13), .b(x06), .c(x05), .O(new_n67_));
  inv1   g42(.a(new_n67_), .O(z09));
  nand2  g43(.a(x05), .b(x02), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n27_), .c(x06), .d(new_n59_), .O(new_n70_));
  inv1   g45(.a(new_n70_), .O(z10));
endmodule


