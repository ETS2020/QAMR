// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n71_, new_n73_;
  inv1   g00(.a(x02), .O(new_n26_));
  nand2  g01(.a(x05), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  nand2  g05(.a(new_n28_), .b(x04), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  nand2  g07(.a(x05), .b(new_n32_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n30_), .O(z00));
  inv1   g10(.a(x03), .O(new_n36_));
  nand2  g11(.a(new_n32_), .b(new_n36_), .O(new_n37_));
  nand2  g12(.a(x04), .b(new_n36_), .O(new_n38_));
  aoi22  g13(.a(new_n38_), .b(x02), .c(new_n37_), .d(x05), .O(z01));
  inv1   g14(.a(x13), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n36_), .c(new_n26_), .O(new_n41_));
  nor4   g16(.a(new_n41_), .b(new_n33_), .c(x01), .d(x00), .O(z02));
  inv1   g17(.a(x01), .O(new_n43_));
  nor2   g18(.a(new_n37_), .b(x13), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n43_), .c(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x05), .c(x02), .O(z03));
  inv1   g21(.a(x00), .O(new_n47_));
  nand3  g22(.a(new_n44_), .b(x01), .c(new_n47_), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(x05), .c(x02), .O(z04));
  nand3  g24(.a(new_n44_), .b(x01), .c(x00), .O(new_n50_));
  aoi21  g25(.a(new_n50_), .b(x05), .c(x02), .O(z05));
  inv1   g26(.a(x07), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n43_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(new_n47_), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g34(.a(x09), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n43_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n59_), .c(x00), .O(new_n62_));
  inv1   g37(.a(x12), .O(new_n63_));
  nand4  g38(.a(x13), .b(new_n63_), .c(x05), .d(new_n26_), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(new_n62_), .c(new_n57_), .d(new_n52_), .O(new_n66_));
  nand3  g41(.a(new_n40_), .b(new_n28_), .c(x02), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n66_), .c(new_n38_), .O(z06));
  nor2   g43(.a(new_n41_), .b(new_n33_), .O(z07));
  aoi21  g44(.a(new_n67_), .b(new_n64_), .c(new_n38_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  aoi21  g46(.a(new_n28_), .b(new_n26_), .c(new_n71_), .O(z09));
  nand2  g47(.a(new_n30_), .b(x06), .O(new_n73_));
  inv1   g48(.a(new_n73_), .O(z10));
endmodule


