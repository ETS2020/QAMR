// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n40_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x05), .b(x02), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nor2   g04(.a(x04), .b(x02), .O(new_n30_));
  aoi21  g05(.a(new_n29_), .b(x04), .c(new_n30_), .O(new_n31_));
  or2    g06(.a(new_n31_), .b(new_n28_), .O(z00));
  nor3   g07(.a(new_n31_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g08(.a(x13), .O(new_n34_));
  nand4  g09(.a(new_n30_), .b(new_n34_), .c(x05), .d(new_n26_), .O(new_n35_));
  nor3   g10(.a(new_n35_), .b(x01), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n37_));
  nor3   g12(.a(new_n35_), .b(x01), .c(new_n37_), .O(z03));
  nor3   g13(.a(x13), .b(x04), .c(x03), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(x01), .c(new_n37_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(x05), .c(x02), .O(z04));
  nand3  g16(.a(new_n39_), .b(x01), .c(x00), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z05));
  nand3  g18(.a(new_n34_), .b(new_n29_), .c(x02), .O(new_n44_));
  inv1   g19(.a(x10), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g21(.a(x01), .O(new_n47_));
  inv1   g22(.a(x08), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g24(.a(new_n49_), .b(new_n46_), .c(new_n37_), .O(new_n50_));
  inv1   g25(.a(x11), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  nor2   g30(.a(new_n34_), .b(x12), .O(new_n56_));
  inv1   g31(.a(x02), .O(new_n57_));
  inv1   g32(.a(x07), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(x05), .c(new_n57_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(new_n60_));
  nand4  g35(.a(new_n60_), .b(new_n56_), .c(new_n55_), .d(new_n50_), .O(new_n61_));
  inv1   g36(.a(x04), .O(new_n62_));
  nor2   g37(.a(new_n62_), .b(x03), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  aoi21  g39(.a(new_n61_), .b(new_n44_), .c(new_n64_), .O(z06));
  inv1   g40(.a(new_n35_), .O(z07));
  aoi21  g41(.a(new_n63_), .b(new_n56_), .c(new_n29_), .O(new_n67_));
  nand3  g42(.a(new_n63_), .b(new_n34_), .c(new_n29_), .O(new_n68_));
  oai21  g43(.a(new_n67_), .b(x02), .c(new_n68_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n27_), .O(z09));
  nand2  g46(.a(x05), .b(x02), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x06), .c(new_n26_), .O(new_n73_));
  oai21  g48(.a(x05), .b(x02), .c(new_n73_), .O(z10));
endmodule


