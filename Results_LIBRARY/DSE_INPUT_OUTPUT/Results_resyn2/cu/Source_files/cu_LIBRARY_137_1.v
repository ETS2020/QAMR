// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n72_, new_n74_, new_n76_, new_n77_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(x12), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g06(.a(new_n26_), .b(x02), .O(new_n32_));
  inv1   g07(.a(x03), .O(new_n33_));
  oai21  g08(.a(x05), .b(x04), .c(new_n33_), .O(new_n34_));
  aoi21  g09(.a(new_n32_), .b(new_n31_), .c(new_n34_), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(new_n28_), .O(z00));
  inv1   g11(.a(x12), .O(new_n37_));
  nand3  g12(.a(x05), .b(new_n30_), .c(new_n29_), .O(new_n38_));
  inv1   g13(.a(new_n38_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n37_), .c(new_n35_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z01));
  inv1   g16(.a(x00), .O(new_n42_));
  inv1   g17(.a(x13), .O(new_n43_));
  nand4  g18(.a(new_n39_), .b(new_n43_), .c(new_n33_), .d(new_n42_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x01), .O(z02));
  nand3  g20(.a(new_n39_), .b(new_n43_), .c(new_n33_), .O(new_n46_));
  inv1   g21(.a(x01), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x00), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n46_), .c(new_n27_), .O(z03));
  nor2   g24(.a(new_n44_), .b(new_n47_), .O(z04));
  nor3   g25(.a(new_n46_), .b(new_n47_), .c(new_n42_), .O(z05));
  nand3  g26(.a(new_n43_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(new_n42_), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g34(.a(x09), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n59_), .c(x00), .O(new_n62_));
  nand3  g37(.a(x13), .b(x05), .c(new_n29_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nor2   g39(.a(x12), .b(x07), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(new_n64_), .c(new_n62_), .d(new_n57_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  nor2   g42(.a(new_n30_), .b(x03), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n27_), .O(z06));
  nand2  g45(.a(new_n46_), .b(new_n27_), .O(z07));
  nand2  g46(.a(new_n68_), .b(new_n37_), .O(new_n72_));
  aoi21  g47(.a(new_n63_), .b(new_n52_), .c(new_n72_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n27_), .O(z09));
  nand2  g50(.a(x05), .b(x02), .O(new_n76_));
  nand3  g51(.a(new_n76_), .b(x06), .c(new_n33_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n27_), .O(z10));
endmodule


