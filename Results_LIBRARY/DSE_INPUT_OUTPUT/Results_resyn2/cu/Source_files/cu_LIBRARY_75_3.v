// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_;
  nand2  g00(.a(x07), .b(x05), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  nand2  g04(.a(x05), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n30_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n29_), .c(new_n27_), .O(z00));
  nor2   g09(.a(x07), .b(x02), .O(new_n35_));
  oai21  g10(.a(new_n35_), .b(x04), .c(new_n33_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  inv1   g12(.a(x07), .O(new_n38_));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  inv1   g15(.a(x02), .O(new_n41_));
  nor2   g16(.a(x13), .b(x03), .O(new_n42_));
  nand3  g17(.a(new_n42_), .b(new_n28_), .c(new_n41_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n40_), .c(new_n39_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n38_), .c(new_n31_), .O(z02));
  nand2  g21(.a(new_n42_), .b(new_n35_), .O(new_n47_));
  nor4   g22(.a(new_n47_), .b(new_n30_), .c(x01), .d(new_n39_), .O(z03));
  nand3  g23(.a(new_n44_), .b(x01), .c(new_n39_), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n38_), .c(new_n31_), .O(z04));
  nand3  g25(.a(new_n44_), .b(x01), .c(x00), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n38_), .c(new_n31_), .O(z05));
  inv1   g27(.a(x03), .O(new_n53_));
  nand2  g28(.a(x04), .b(new_n53_), .O(new_n54_));
  inv1   g29(.a(x13), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n31_), .c(x02), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  aoi21  g33(.a(new_n58_), .b(new_n40_), .c(x00), .O(new_n59_));
  oai21  g34(.a(x10), .b(new_n40_), .c(new_n59_), .O(new_n60_));
  inv1   g35(.a(x11), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g37(.a(x09), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(new_n40_), .c(new_n39_), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand4  g40(.a(x13), .b(new_n65_), .c(x05), .d(new_n41_), .O(new_n66_));
  aoi21  g41(.a(new_n64_), .b(new_n62_), .c(new_n66_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n60_), .c(new_n57_), .O(new_n68_));
  oai21  g43(.a(new_n68_), .b(new_n54_), .c(new_n26_), .O(z06));
  aoi21  g44(.a(new_n43_), .b(new_n38_), .c(new_n31_), .O(z07));
  inv1   g45(.a(new_n66_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n38_), .O(new_n72_));
  aoi21  g47(.a(new_n72_), .b(new_n56_), .c(new_n54_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n26_), .O(z09));
  inv1   g50(.a(new_n35_), .O(new_n76_));
  nand2  g51(.a(x06), .b(new_n53_), .O(new_n77_));
  aoi21  g52(.a(new_n76_), .b(x05), .c(new_n77_), .O(z10));
endmodule


