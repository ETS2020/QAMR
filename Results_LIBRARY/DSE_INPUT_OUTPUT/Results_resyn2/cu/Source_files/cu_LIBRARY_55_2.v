// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_;
  nor2   g00(.a(x04), .b(x02), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x05), .O(new_n27_));
  inv1   g02(.a(x01), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand4  g04(.a(new_n29_), .b(x04), .c(x02), .d(new_n28_), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  inv1   g07(.a(x00), .O(new_n33_));
  nor2   g08(.a(x13), .b(x03), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n26_), .c(x05), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(new_n28_), .c(new_n33_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z02));
  nand2  g13(.a(new_n29_), .b(x01), .O(new_n39_));
  nand2  g14(.a(new_n28_), .b(x00), .O(new_n40_));
  oai21  g15(.a(new_n40_), .b(new_n35_), .c(new_n39_), .O(z03));
  nand3  g16(.a(new_n34_), .b(new_n26_), .c(new_n33_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(new_n28_), .O(z04));
  nor3   g18(.a(new_n35_), .b(new_n28_), .c(new_n33_), .O(z05));
  inv1   g19(.a(x13), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n29_), .c(x02), .O(new_n46_));
  inv1   g21(.a(x09), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(x05), .c(new_n28_), .O(new_n48_));
  inv1   g23(.a(x11), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  aoi21  g25(.a(new_n50_), .b(new_n48_), .c(new_n33_), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(x05), .c(new_n28_), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  aoi21  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  inv1   g31(.a(x02), .O(new_n57_));
  inv1   g32(.a(x07), .O(new_n58_));
  inv1   g33(.a(x12), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  oai21  g36(.a(new_n56_), .b(new_n51_), .c(new_n61_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  inv1   g38(.a(x04), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(x03), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n39_), .O(z06));
  nand2  g42(.a(new_n39_), .b(new_n35_), .O(z07));
  nand4  g43(.a(x13), .b(new_n59_), .c(x05), .d(new_n57_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n65_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n39_), .O(z08));
  nand2  g47(.a(new_n39_), .b(x06), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n45_), .O(z09));
  nor2   g49(.a(new_n29_), .b(new_n57_), .O(new_n75_));
  nor3   g50(.a(new_n75_), .b(new_n73_), .c(x03), .O(z10));
endmodule


