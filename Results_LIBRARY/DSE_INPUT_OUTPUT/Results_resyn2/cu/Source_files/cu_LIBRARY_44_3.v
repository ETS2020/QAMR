// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_,
    new_n77_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(x04), .b(x02), .O(new_n27_));
  oai21  g02(.a(new_n26_), .b(x02), .c(new_n27_), .O(new_n28_));
  aoi21  g03(.a(x05), .b(x04), .c(x03), .O(new_n29_));
  aoi22  g04(.a(new_n29_), .b(new_n28_), .c(x06), .d(x05), .O(z00));
  inv1   g05(.a(x04), .O(new_n31_));
  inv1   g06(.a(x06), .O(new_n32_));
  nor2   g07(.a(new_n26_), .b(x02), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  nand3  g09(.a(new_n26_), .b(x04), .c(x02), .O(new_n35_));
  aoi21  g10(.a(new_n35_), .b(new_n34_), .c(x03), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(new_n34_), .O(new_n38_));
  nor2   g13(.a(x13), .b(x03), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n42_));
  nor3   g17(.a(x13), .b(x04), .c(x03), .O(new_n43_));
  nor2   g18(.a(x02), .b(new_n37_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n32_), .c(new_n26_), .O(z03));
  nor2   g21(.a(new_n40_), .b(new_n42_), .O(z04));
  nand3  g22(.a(new_n44_), .b(new_n43_), .c(x01), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n32_), .c(new_n26_), .O(z05));
  nand2  g24(.a(x06), .b(x05), .O(new_n50_));
  inv1   g25(.a(x13), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x07), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(new_n37_), .O(new_n58_));
  inv1   g33(.a(x11), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x02), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand4  g40(.a(x13), .b(new_n65_), .c(x05), .d(new_n64_), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n63_), .c(new_n58_), .d(new_n53_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nor2   g44(.a(new_n31_), .b(x03), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n50_), .O(z06));
  nor3   g47(.a(new_n34_), .b(x13), .c(x03), .O(z07));
  inv1   g48(.a(new_n70_), .O(new_n74_));
  nand2  g49(.a(new_n67_), .b(new_n32_), .O(new_n75_));
  aoi21  g50(.a(new_n75_), .b(new_n52_), .c(new_n74_), .O(z08));
  nand3  g51(.a(x13), .b(x06), .c(new_n26_), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z09));
  aoi21  g53(.a(new_n26_), .b(x03), .c(new_n32_), .O(z10));
endmodule


