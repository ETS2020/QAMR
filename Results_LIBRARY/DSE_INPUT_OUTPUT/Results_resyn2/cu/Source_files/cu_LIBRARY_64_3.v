// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x09), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nor2   g06(.a(x04), .b(x02), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  oai21  g08(.a(x05), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n30_), .c(new_n26_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x13), .O(new_n37_));
  nand3  g12(.a(new_n32_), .b(new_n37_), .c(new_n26_), .O(new_n38_));
  nor4   g13(.a(new_n38_), .b(new_n29_), .c(x01), .d(x00), .O(z02));
  nor2   g14(.a(new_n38_), .b(new_n29_), .O(z07));
  nand2  g15(.a(z07), .b(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x01), .O(z03));
  inv1   g17(.a(x05), .O(new_n43_));
  inv1   g18(.a(x00), .O(new_n44_));
  inv1   g19(.a(new_n38_), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(x01), .c(new_n44_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n28_), .c(new_n43_), .O(z04));
  inv1   g22(.a(x01), .O(new_n48_));
  nor2   g23(.a(new_n41_), .b(new_n48_), .O(z05));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(x01), .c(new_n44_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(x00), .c(new_n48_), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x00), .O(new_n56_));
  oai21  g31(.a(new_n56_), .b(new_n54_), .c(new_n52_), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand3  g33(.a(x13), .b(new_n58_), .c(new_n31_), .O(new_n59_));
  nor2   g34(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n57_), .c(new_n50_), .O(new_n61_));
  nand3  g36(.a(new_n37_), .b(new_n43_), .c(x02), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n26_), .O(new_n63_));
  aoi21  g38(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(z06));
  inv1   g39(.a(new_n62_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(new_n63_), .O(z08));
  nand2  g42(.a(x13), .b(x06), .O(new_n68_));
  nand2  g43(.a(x09), .b(x05), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n68_), .O(z09));
  nand2  g45(.a(x05), .b(x02), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(x06), .c(new_n26_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n69_), .O(z10));
endmodule


