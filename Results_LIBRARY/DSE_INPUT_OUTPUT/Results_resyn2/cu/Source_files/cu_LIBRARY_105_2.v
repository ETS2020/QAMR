// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nor2   g03(.a(x03), .b(x02), .O(new_n29_));
  nand3  g04(.a(new_n29_), .b(x05), .c(new_n28_), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n27_), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  inv1   g07(.a(x03), .O(new_n33_));
  inv1   g08(.a(x13), .O(new_n34_));
  nand4  g09(.a(new_n34_), .b(x05), .c(new_n28_), .d(new_n33_), .O(new_n35_));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  inv1   g12(.a(x02), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n35_), .c(new_n27_), .O(z02));
  nand3  g15(.a(new_n38_), .b(new_n37_), .c(x00), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n35_), .c(new_n27_), .O(z03));
  nand4  g17(.a(new_n29_), .b(new_n34_), .c(x05), .d(new_n28_), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(new_n37_), .c(x00), .O(z04));
  nand2  g19(.a(x01), .b(x00), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n43_), .c(new_n27_), .O(z05));
  inv1   g21(.a(x07), .O(new_n47_));
  nor2   g22(.a(new_n26_), .b(new_n28_), .O(new_n48_));
  nor2   g23(.a(new_n34_), .b(x12), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n48_), .c(new_n29_), .d(new_n47_), .O(new_n50_));
  inv1   g25(.a(x11), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(new_n36_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n55_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(new_n50_), .O(z06));
  inv1   g37(.a(new_n43_), .O(z07));
  inv1   g38(.a(new_n29_), .O(new_n64_));
  nand2  g39(.a(new_n49_), .b(new_n48_), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(new_n64_), .O(z08));
  nand2  g41(.a(x13), .b(x06), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n27_), .O(z09));
  nand2  g43(.a(x05), .b(x02), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n27_), .c(x06), .d(new_n33_), .O(new_n70_));
  inv1   g45(.a(new_n70_), .O(z10));
endmodule


