// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n73_, new_n74_;
  inv1   g00(.a(x05), .O(new_n26_));
  inv1   g01(.a(x06), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g03(.a(x04), .b(x02), .c(x05), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n26_), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(new_n28_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  inv1   g08(.a(x03), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(new_n27_), .c(x05), .d(new_n34_), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  nor2   g12(.a(x04), .b(x02), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor3   g14(.a(new_n39_), .b(new_n36_), .c(x00), .O(z02));
  inv1   g15(.a(x00), .O(new_n41_));
  nor3   g16(.a(new_n39_), .b(new_n36_), .c(new_n41_), .O(z03));
  nand3  g17(.a(new_n38_), .b(new_n35_), .c(new_n34_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(x01), .c(new_n41_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n27_), .c(new_n26_), .O(z04));
  nand4  g21(.a(new_n27_), .b(x05), .c(x01), .d(x00), .O(new_n47_));
  nor2   g22(.a(new_n47_), .b(new_n43_), .O(z05));
  inv1   g23(.a(new_n28_), .O(new_n49_));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x08), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(new_n41_), .O(new_n55_));
  inv1   g30(.a(x11), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x09), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(x00), .O(new_n60_));
  inv1   g35(.a(x02), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(x05), .d(new_n61_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n60_), .c(new_n55_), .d(new_n50_), .O(new_n65_));
  nand3  g40(.a(new_n35_), .b(new_n26_), .c(x02), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  inv1   g42(.a(x04), .O(new_n68_));
  nor2   g43(.a(new_n68_), .b(x03), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n49_), .O(z06));
  aoi21  g46(.a(new_n43_), .b(new_n27_), .c(new_n26_), .O(z07));
  nand2  g47(.a(new_n66_), .b(new_n63_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n49_), .O(z08));
  aoi21  g50(.a(new_n35_), .b(new_n26_), .c(new_n27_), .O(z09));
  aoi21  g51(.a(new_n26_), .b(x03), .c(new_n27_), .O(z10));
endmodule


