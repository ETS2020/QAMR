// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x09), .b(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nor2   g05(.a(x04), .b(x02), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n29_), .c(new_n26_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  inv1   g10(.a(x13), .O(new_n36_));
  nand3  g11(.a(new_n31_), .b(new_n36_), .c(new_n26_), .O(new_n37_));
  nor4   g12(.a(new_n37_), .b(new_n28_), .c(x01), .d(x00), .O(z02));
  nor2   g13(.a(new_n37_), .b(new_n28_), .O(z07));
  nand2  g14(.a(z07), .b(x00), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x01), .O(z03));
  inv1   g16(.a(x05), .O(new_n42_));
  inv1   g17(.a(x00), .O(new_n43_));
  inv1   g18(.a(new_n37_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(x01), .c(new_n43_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x09), .c(new_n42_), .O(z04));
  inv1   g21(.a(x01), .O(new_n47_));
  nor2   g22(.a(new_n40_), .b(new_n47_), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  nand2  g24(.a(x10), .b(x01), .O(new_n50_));
  nand2  g25(.a(x08), .b(new_n47_), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n50_), .c(new_n43_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(x01), .c(x00), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand3  g31(.a(x13), .b(new_n56_), .c(new_n30_), .O(new_n57_));
  nor2   g32(.a(new_n57_), .b(new_n28_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n55_), .c(new_n49_), .O(new_n59_));
  nand3  g34(.a(new_n36_), .b(new_n42_), .c(x02), .O(new_n60_));
  nand2  g35(.a(x04), .b(new_n26_), .O(new_n61_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(z06));
  inv1   g37(.a(new_n60_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(new_n61_), .O(z08));
  nand2  g40(.a(x13), .b(x06), .O(new_n66_));
  inv1   g41(.a(x09), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(x05), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n66_), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n70_));
  nand3  g45(.a(new_n70_), .b(x06), .c(new_n26_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n68_), .O(z10));
endmodule


