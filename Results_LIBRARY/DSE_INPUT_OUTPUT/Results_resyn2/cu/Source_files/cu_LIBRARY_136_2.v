// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_,
    new_n78_;
  inv1   g00(.a(x09), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x07), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n27_), .O(z01));
  inv1   g09(.a(x03), .O(new_n35_));
  inv1   g10(.a(x04), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n36_), .d(new_n35_), .O(new_n38_));
  inv1   g13(.a(x02), .O(new_n39_));
  nand2  g14(.a(new_n27_), .b(new_n39_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(new_n38_), .O(z07));
  nor2   g16(.a(x01), .b(x00), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(z07), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(z02));
  inv1   g19(.a(new_n38_), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x00), .O(new_n46_));
  inv1   g21(.a(x01), .O(new_n47_));
  nand2  g22(.a(new_n39_), .b(new_n47_), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n46_), .c(new_n27_), .O(z03));
  inv1   g24(.a(x00), .O(new_n50_));
  nand3  g25(.a(new_n39_), .b(x01), .c(new_n50_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n38_), .c(new_n27_), .O(z04));
  nand2  g27(.a(new_n39_), .b(x01), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(new_n46_), .c(new_n27_), .O(z05));
  nand2  g29(.a(x04), .b(new_n35_), .O(new_n55_));
  nand3  g30(.a(new_n37_), .b(new_n28_), .c(x02), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n27_), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  inv1   g34(.a(x11), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x01), .O(new_n61_));
  nand2  g36(.a(new_n26_), .b(new_n47_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n61_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x10), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(x01), .O(new_n65_));
  inv1   g40(.a(x08), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n47_), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(new_n65_), .c(new_n50_), .O(new_n68_));
  inv1   g43(.a(x12), .O(new_n69_));
  nand4  g44(.a(x13), .b(new_n69_), .c(x05), .d(new_n39_), .O(new_n70_));
  inv1   g45(.a(new_n70_), .O(new_n71_));
  nand4  g46(.a(new_n71_), .b(new_n68_), .c(new_n63_), .d(new_n59_), .O(new_n72_));
  aoi21  g47(.a(new_n72_), .b(new_n58_), .c(new_n55_), .O(z06));
  nand3  g48(.a(new_n27_), .b(x04), .c(new_n35_), .O(new_n74_));
  aoi21  g49(.a(new_n70_), .b(new_n56_), .c(new_n74_), .O(z08));
  nand2  g50(.a(x13), .b(x06), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n27_), .O(z09));
  nand2  g52(.a(new_n31_), .b(x06), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n27_), .O(z10));
endmodule


