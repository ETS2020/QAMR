// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand3  g02(.a(new_n27_), .b(x04), .c(x02), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  inv1   g05(.a(x11), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x05), .O(new_n32_));
  oai21  g07(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n26_), .O(z00));
  oai21  g09(.a(new_n30_), .b(new_n27_), .c(new_n28_), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n26_), .O(new_n36_));
  nand2  g11(.a(x11), .b(x05), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(z01));
  inv1   g13(.a(x13), .O(new_n39_));
  nand3  g14(.a(new_n29_), .b(new_n39_), .c(new_n26_), .O(new_n40_));
  nor4   g15(.a(new_n40_), .b(new_n32_), .c(x01), .d(x00), .O(z02));
  nor2   g16(.a(new_n40_), .b(new_n32_), .O(z07));
  nand2  g17(.a(z07), .b(x00), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(x01), .O(z03));
  inv1   g19(.a(x00), .O(new_n45_));
  inv1   g20(.a(new_n40_), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(x01), .c(new_n45_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n31_), .c(new_n27_), .O(z04));
  inv1   g23(.a(x01), .O(new_n49_));
  nor2   g24(.a(new_n43_), .b(new_n49_), .O(z05));
  nand3  g25(.a(new_n39_), .b(new_n27_), .c(x02), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(x00), .c(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x00), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  nand2  g35(.a(x05), .b(new_n60_), .O(new_n61_));
  inv1   g36(.a(x07), .O(new_n62_));
  inv1   g37(.a(x12), .O(new_n63_));
  nand3  g38(.a(x13), .b(new_n63_), .c(new_n62_), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(new_n59_), .c(new_n52_), .O(new_n66_));
  nand2  g41(.a(x04), .b(new_n26_), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n66_), .c(new_n37_), .O(z06));
  nor2   g43(.a(x12), .b(x11), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(x13), .c(x05), .d(new_n60_), .O(new_n70_));
  aoi21  g45(.a(new_n70_), .b(new_n51_), .c(new_n67_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n37_), .O(z09));
  nand2  g48(.a(x05), .b(x02), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x06), .c(new_n26_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n37_), .O(z10));
endmodule


