// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x07), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  oai21  g05(.a(x05), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  aoi21  g06(.a(new_n26_), .b(new_n29_), .c(x03), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n31_), .c(new_n27_), .O(z00));
  nand3  g08(.a(new_n26_), .b(x04), .c(x02), .O(new_n34_));
  inv1   g09(.a(x07), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(x02), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(x05), .c(new_n29_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n34_), .c(x03), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nor2   g15(.a(x04), .b(x02), .O(new_n41_));
  nor2   g16(.a(x13), .b(x03), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(x07), .c(new_n26_), .O(z02));
  nand3  g19(.a(new_n42_), .b(new_n40_), .c(x00), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n37_), .O(z03));
  nor3   g21(.a(x13), .b(x04), .c(x03), .O(new_n47_));
  nor2   g22(.a(x02), .b(new_n40_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n47_), .c(new_n39_), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x07), .c(new_n26_), .O(z04));
  nand3  g25(.a(new_n48_), .b(new_n47_), .c(x00), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(x07), .c(new_n26_), .O(z05));
  inv1   g27(.a(new_n27_), .O(new_n53_));
  inv1   g28(.a(new_n42_), .O(new_n54_));
  oai21  g29(.a(new_n54_), .b(new_n34_), .c(new_n53_), .O(z06));
  nand2  g30(.a(new_n42_), .b(new_n41_), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(x07), .c(new_n26_), .O(z07));
  nor2   g32(.a(x12), .b(new_n26_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n36_), .O(new_n59_));
  inv1   g34(.a(x13), .O(new_n60_));
  oai21  g35(.a(x05), .b(new_n28_), .c(new_n60_), .O(new_n61_));
  nor2   g36(.a(new_n29_), .b(x03), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g38(.a(new_n59_), .b(x13), .c(new_n63_), .O(z08));
  nand2  g39(.a(x13), .b(x06), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n53_), .O(z09));
  nor2   g41(.a(new_n36_), .b(new_n26_), .O(new_n67_));
  inv1   g42(.a(x03), .O(new_n68_));
  nand2  g43(.a(x06), .b(new_n68_), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(new_n67_), .O(z10));
endmodule


