// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x00), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(x05), .O(new_n30_));
  nand3  g05(.a(new_n26_), .b(x04), .c(x02), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n30_), .c(x03), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n28_), .O(z00));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(new_n30_), .O(new_n35_));
  oai21  g10(.a(new_n35_), .b(new_n34_), .c(new_n32_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  nor2   g12(.a(x13), .b(x03), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n35_), .c(new_n34_), .O(new_n39_));
  nor2   g14(.a(new_n39_), .b(x01), .O(z02));
  inv1   g15(.a(x01), .O(new_n41_));
  nand3  g16(.a(new_n38_), .b(new_n29_), .c(new_n41_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(new_n34_), .O(z03));
  nor2   g18(.a(new_n39_), .b(new_n41_), .O(z04));
  nand3  g19(.a(new_n38_), .b(new_n29_), .c(x01), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x05), .c(new_n34_), .O(z05));
  inv1   g21(.a(x13), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(new_n26_), .c(x02), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  nand2  g24(.a(x10), .b(x01), .O(new_n50_));
  nand2  g25(.a(x08), .b(new_n41_), .O(new_n51_));
  nand4  g26(.a(new_n51_), .b(new_n50_), .c(x05), .d(new_n34_), .O(new_n52_));
  nand2  g27(.a(x11), .b(x01), .O(new_n53_));
  nand2  g28(.a(x09), .b(new_n41_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n53_), .c(x00), .O(new_n55_));
  inv1   g30(.a(x02), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n59_));
  aoi21  g34(.a(new_n55_), .b(new_n52_), .c(new_n59_), .O(new_n60_));
  inv1   g35(.a(x04), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x03), .O(new_n62_));
  oai21  g37(.a(new_n60_), .b(new_n49_), .c(new_n62_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n27_), .O(z06));
  nand2  g39(.a(new_n38_), .b(new_n35_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n27_), .O(z07));
  nand4  g41(.a(x13), .b(new_n58_), .c(x05), .d(new_n56_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n48_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n27_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n27_), .O(z09));
  inv1   g47(.a(x03), .O(new_n73_));
  nand2  g48(.a(x05), .b(x02), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x06), .c(new_n73_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n27_), .O(z10));
endmodule


