// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_, new_n76_,
    new_n78_, new_n79_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x10), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  oai21  g05(.a(x05), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  aoi21  g07(.a(new_n26_), .b(new_n32_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n31_), .c(new_n27_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  nor2   g10(.a(x01), .b(x00), .O(new_n36_));
  nor2   g11(.a(x13), .b(x03), .O(new_n37_));
  nand2  g12(.a(x05), .b(new_n28_), .O(new_n38_));
  inv1   g13(.a(new_n38_), .O(new_n39_));
  inv1   g14(.a(x10), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x04), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n39_), .c(new_n37_), .d(new_n36_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z02));
  inv1   g18(.a(x00), .O(new_n44_));
  nor2   g19(.a(x01), .b(new_n44_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n41_), .c(new_n39_), .d(new_n37_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z03));
  inv1   g22(.a(x01), .O(new_n48_));
  nor2   g23(.a(new_n48_), .b(x00), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n41_), .c(new_n39_), .d(new_n37_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nand4  g26(.a(new_n37_), .b(new_n29_), .c(x01), .d(x00), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(x10), .c(new_n26_), .O(z05));
  inv1   g28(.a(new_n27_), .O(new_n54_));
  inv1   g29(.a(x13), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n26_), .c(x02), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand2  g33(.a(new_n36_), .b(new_n58_), .O(new_n59_));
  nand2  g34(.a(x11), .b(x01), .O(new_n60_));
  nand2  g35(.a(x09), .b(new_n48_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n60_), .c(x00), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  inv1   g38(.a(x07), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand3  g40(.a(x13), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(new_n38_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n63_), .c(new_n57_), .O(new_n68_));
  inv1   g43(.a(x03), .O(new_n69_));
  nand2  g44(.a(x04), .b(new_n69_), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(new_n68_), .c(new_n54_), .O(z06));
  nand2  g46(.a(new_n37_), .b(new_n29_), .O(new_n72_));
  aoi21  g47(.a(new_n72_), .b(x10), .c(new_n26_), .O(z07));
  nand4  g48(.a(new_n39_), .b(x13), .c(new_n65_), .d(x10), .O(new_n74_));
  aoi21  g49(.a(new_n74_), .b(new_n56_), .c(new_n70_), .O(z08));
  nand2  g50(.a(x13), .b(x06), .O(new_n76_));
  nor2   g51(.a(new_n76_), .b(new_n27_), .O(z09));
  nand2  g52(.a(x05), .b(x02), .O(new_n78_));
  nand3  g53(.a(new_n78_), .b(x06), .c(new_n69_), .O(new_n79_));
  nand2  g54(.a(new_n79_), .b(new_n54_), .O(z10));
endmodule


