// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nand2  g03(.a(x05), .b(new_n28_), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  oai21  g05(.a(x05), .b(new_n28_), .c(x04), .O(new_n31_));
  inv1   g06(.a(x11), .O(new_n32_));
  nand2  g07(.a(x13), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n30_), .d(new_n26_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  inv1   g12(.a(new_n29_), .O(new_n38_));
  nor3   g13(.a(x13), .b(x04), .c(x03), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n36_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z02));
  nand2  g16(.a(new_n39_), .b(new_n38_), .O(new_n42_));
  nand2  g17(.a(new_n37_), .b(x00), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n42_), .c(new_n33_), .O(z03));
  nand4  g19(.a(new_n39_), .b(new_n38_), .c(x01), .d(new_n36_), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(new_n33_), .O(z04));
  nand2  g21(.a(x01), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n42_), .c(new_n33_), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  inv1   g24(.a(x09), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n37_), .c(x00), .O(new_n51_));
  nand2  g26(.a(x10), .b(x01), .O(new_n52_));
  nand2  g27(.a(x08), .b(new_n37_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n52_), .c(new_n36_), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand3  g31(.a(x13), .b(new_n56_), .c(x11), .O(new_n57_));
  nor2   g32(.a(new_n57_), .b(new_n29_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n55_), .c(new_n49_), .O(new_n59_));
  inv1   g34(.a(x05), .O(new_n60_));
  inv1   g35(.a(x13), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n60_), .c(x02), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n26_), .O(new_n63_));
  aoi21  g38(.a(new_n62_), .b(new_n59_), .c(new_n63_), .O(z06));
  nand2  g39(.a(new_n42_), .b(new_n33_), .O(z07));
  inv1   g40(.a(new_n62_), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  nor2   g42(.a(new_n67_), .b(new_n63_), .O(z08));
  inv1   g43(.a(x06), .O(new_n69_));
  aoi21  g44(.a(x11), .b(new_n69_), .c(new_n61_), .O(z09));
  nand2  g45(.a(x05), .b(x02), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(x06), .c(new_n26_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n33_), .O(z10));
endmodule


