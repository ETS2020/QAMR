// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n72_, new_n74_, new_n76_;
  nand2  g00(.a(x10), .b(x07), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  aoi21  g04(.a(x05), .b(x02), .c(x03), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(z00));
  nand2  g06(.a(new_n30_), .b(new_n29_), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n26_), .O(z01));
  nor2   g08(.a(new_n27_), .b(x04), .O(new_n34_));
  nor3   g09(.a(x13), .b(x03), .c(x02), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n36_), .c(new_n26_), .O(z02));
  aoi21  g15(.a(x10), .b(x07), .c(new_n37_), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n35_), .c(new_n34_), .d(new_n38_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z03));
  nand2  g18(.a(x01), .b(new_n37_), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n36_), .c(new_n26_), .O(z04));
  nand4  g20(.a(new_n41_), .b(new_n35_), .c(new_n34_), .d(x01), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z05));
  inv1   g22(.a(x13), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n27_), .c(x02), .O(new_n49_));
  inv1   g24(.a(x07), .O(new_n50_));
  inv1   g25(.a(x11), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n38_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(new_n37_), .O(new_n60_));
  inv1   g35(.a(x02), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(x05), .d(new_n61_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n60_), .c(new_n55_), .d(new_n50_), .O(new_n65_));
  inv1   g40(.a(x04), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(x03), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n26_), .O(new_n68_));
  aoi21  g43(.a(new_n65_), .b(new_n49_), .c(new_n68_), .O(z06));
  nand2  g44(.a(new_n36_), .b(new_n26_), .O(z07));
  nand2  g45(.a(new_n63_), .b(new_n49_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n26_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n26_), .O(z09));
  nand2  g50(.a(new_n30_), .b(x06), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n26_), .O(z10));
endmodule


