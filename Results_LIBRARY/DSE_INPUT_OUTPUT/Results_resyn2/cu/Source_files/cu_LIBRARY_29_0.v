// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(x05), .b(new_n28_), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nor2   g06(.a(new_n31_), .b(new_n27_), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n30_), .c(new_n26_), .O(z00));
  aoi21  g09(.a(new_n28_), .b(new_n26_), .c(new_n31_), .O(new_n35_));
  nor2   g10(.a(new_n28_), .b(x03), .O(new_n36_));
  nor2   g11(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(new_n35_), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nor3   g15(.a(x13), .b(x04), .c(x03), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z02));
  inv1   g18(.a(x13), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n26_), .c(new_n27_), .O(new_n45_));
  nor4   g20(.a(new_n45_), .b(new_n29_), .c(x01), .d(new_n39_), .O(z03));
  nand3  g21(.a(new_n41_), .b(x01), .c(new_n39_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x05), .c(x02), .O(z04));
  nand3  g23(.a(new_n41_), .b(x01), .c(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x05), .c(x02), .O(z05));
  inv1   g25(.a(x10), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x08), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n40_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(new_n39_), .O(new_n55_));
  inv1   g30(.a(x11), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x09), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n40_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(x00), .O(new_n60_));
  nor2   g35(.a(new_n44_), .b(x12), .O(new_n61_));
  inv1   g36(.a(x07), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(x04), .c(new_n26_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n61_), .c(new_n60_), .d(new_n55_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(x05), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n27_), .O(new_n67_));
  nand3  g42(.a(new_n36_), .b(new_n44_), .c(new_n31_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n67_), .O(z06));
  nor2   g44(.a(new_n45_), .b(new_n29_), .O(z07));
  nand2  g45(.a(new_n44_), .b(new_n31_), .O(new_n71_));
  nand2  g46(.a(new_n61_), .b(new_n27_), .O(new_n72_));
  nand2  g47(.a(new_n31_), .b(new_n27_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n36_), .O(new_n74_));
  aoi21  g49(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(z08));
  nand2  g50(.a(x13), .b(x06), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n73_), .O(z09));
  nand2  g52(.a(x06), .b(new_n26_), .O(new_n78_));
  aoi21  g53(.a(new_n78_), .b(new_n73_), .c(new_n32_), .O(z10));
endmodule


