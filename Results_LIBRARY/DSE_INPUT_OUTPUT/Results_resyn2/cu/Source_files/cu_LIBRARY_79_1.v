// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n65_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x02), .O(new_n26_));
  nor2   g01(.a(x05), .b(new_n26_), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nor2   g04(.a(x11), .b(x02), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(x05), .c(new_n29_), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n28_), .c(x03), .O(z01));
  inv1   g07(.a(z01), .O(z00));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(x01), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  inv1   g13(.a(new_n31_), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(new_n36_), .O(z02));
  nand2  g16(.a(new_n35_), .b(x00), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(new_n40_), .O(z03));
  nand4  g18(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(x01), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x00), .O(z04));
  nor2   g20(.a(new_n44_), .b(new_n34_), .O(z05));
  inv1   g21(.a(x07), .O(new_n47_));
  inv1   g22(.a(x10), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(x00), .c(x01), .O(new_n49_));
  inv1   g24(.a(x08), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n35_), .c(new_n34_), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x00), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n26_), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand3  g32(.a(x13), .b(new_n57_), .c(x05), .O(new_n58_));
  nor2   g33(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n54_), .c(new_n47_), .O(new_n60_));
  nand2  g35(.a(new_n27_), .b(new_n38_), .O(new_n61_));
  nand2  g36(.a(x04), .b(new_n37_), .O(new_n62_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(z06));
  inv1   g38(.a(new_n40_), .O(z07));
  inv1   g39(.a(new_n59_), .O(new_n65_));
  aoi21  g40(.a(new_n61_), .b(new_n65_), .c(new_n62_), .O(z08));
  nand2  g41(.a(x13), .b(x06), .O(new_n67_));
  oai21  g42(.a(new_n55_), .b(x02), .c(new_n67_), .O(z09));
  nor2   g43(.a(new_n30_), .b(new_n27_), .O(new_n69_));
  nand2  g44(.a(x06), .b(new_n37_), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n69_), .O(z10));
endmodule


