// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n73_, new_n74_, new_n77_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nor3   g03(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n29_));
  inv1   g04(.a(x06), .O(new_n30_));
  nand3  g05(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  oai21  g09(.a(new_n34_), .b(new_n29_), .c(new_n26_), .O(z00));
  oai21  g10(.a(new_n32_), .b(new_n29_), .c(new_n26_), .O(new_n36_));
  nand2  g11(.a(x06), .b(x05), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(z01));
  inv1   g13(.a(x05), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nor3   g16(.a(x13), .b(x04), .c(x03), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n27_), .c(new_n41_), .d(new_n40_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n30_), .c(new_n39_), .O(z02));
  inv1   g19(.a(x13), .O(new_n45_));
  nand4  g20(.a(new_n32_), .b(new_n45_), .c(new_n30_), .d(new_n26_), .O(new_n46_));
  nor3   g21(.a(new_n46_), .b(x01), .c(new_n40_), .O(z03));
  nand4  g22(.a(new_n42_), .b(new_n27_), .c(x01), .d(new_n40_), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n30_), .c(new_n39_), .O(z04));
  nor3   g24(.a(new_n46_), .b(new_n41_), .c(new_n40_), .O(z05));
  nand3  g25(.a(new_n45_), .b(new_n39_), .c(x02), .O(new_n51_));
  inv1   g26(.a(x10), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x08), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n41_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(new_n40_), .O(new_n56_));
  inv1   g31(.a(x11), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x09), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n41_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  nor2   g36(.a(new_n39_), .b(x02), .O(new_n62_));
  inv1   g37(.a(x07), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand3  g39(.a(x13), .b(new_n64_), .c(new_n63_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n62_), .c(new_n61_), .d(new_n56_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n51_), .O(new_n68_));
  nor2   g43(.a(new_n28_), .b(x03), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n37_), .O(z06));
  inv1   g46(.a(new_n46_), .O(z07));
  inv1   g47(.a(new_n69_), .O(new_n73_));
  nand4  g48(.a(new_n62_), .b(x13), .c(new_n64_), .d(new_n30_), .O(new_n74_));
  aoi21  g49(.a(new_n74_), .b(new_n51_), .c(new_n73_), .O(z08));
  aoi21  g50(.a(new_n45_), .b(new_n39_), .c(new_n30_), .O(z09));
  nand3  g51(.a(x06), .b(new_n39_), .c(new_n26_), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z10));
endmodule


