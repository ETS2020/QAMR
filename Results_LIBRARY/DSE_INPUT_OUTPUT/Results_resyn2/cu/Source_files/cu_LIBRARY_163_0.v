// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_;
  inv1   g00(.a(x02), .O(new_n26_));
  nand2  g01(.a(x05), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  nor2   g06(.a(x05), .b(new_n31_), .O(new_n32_));
  nor2   g07(.a(new_n28_), .b(x04), .O(new_n33_));
  oai21  g08(.a(new_n33_), .b(new_n32_), .c(new_n30_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(new_n26_), .c(new_n36_), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand3  g15(.a(new_n33_), .b(new_n40_), .c(new_n39_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n38_), .O(z02));
  nand3  g17(.a(new_n37_), .b(new_n26_), .c(x00), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n41_), .O(z03));
  nand3  g19(.a(new_n33_), .b(new_n40_), .c(x01), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n38_), .O(z04));
  nor2   g21(.a(new_n45_), .b(new_n43_), .O(z05));
  nand2  g22(.a(x04), .b(new_n37_), .O(new_n48_));
  nand3  g23(.a(new_n40_), .b(new_n28_), .c(x02), .O(new_n49_));
  inv1   g24(.a(x10), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n39_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(new_n36_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n39_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  nor2   g34(.a(new_n40_), .b(x12), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(x05), .c(new_n26_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(new_n54_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n49_), .c(new_n48_), .O(z06));
  nand3  g40(.a(new_n40_), .b(new_n31_), .c(new_n37_), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(x05), .c(x02), .O(z07));
  nor2   g42(.a(new_n31_), .b(x03), .O(new_n68_));
  aoi21  g43(.a(new_n60_), .b(new_n68_), .c(new_n28_), .O(new_n69_));
  nand3  g44(.a(new_n32_), .b(new_n40_), .c(new_n37_), .O(new_n70_));
  oai21  g45(.a(new_n69_), .b(x02), .c(new_n70_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  oai21  g47(.a(x05), .b(x02), .c(new_n72_), .O(z09));
  nand2  g48(.a(new_n30_), .b(x06), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z10));
endmodule


