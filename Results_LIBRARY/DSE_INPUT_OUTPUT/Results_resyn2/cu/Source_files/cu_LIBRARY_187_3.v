// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_;
  nand2  g00(.a(x12), .b(x05), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  inv1   g03(.a(x02), .O(new_n29_));
  nand2  g04(.a(x05), .b(new_n29_), .O(new_n30_));
  inv1   g05(.a(x03), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  inv1   g07(.a(x05), .O(new_n33_));
  oai21  g08(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(new_n34_));
  aoi21  g09(.a(new_n30_), .b(new_n28_), .c(new_n34_), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(new_n27_), .O(z00));
  nand2  g11(.a(new_n35_), .b(new_n26_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z01));
  inv1   g13(.a(x12), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  inv1   g16(.a(x13), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(new_n32_), .c(new_n31_), .d(new_n29_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n39_), .c(new_n33_), .O(z02));
  nand4  g21(.a(new_n44_), .b(new_n39_), .c(x05), .d(x00), .O(new_n47_));
  nor2   g22(.a(new_n47_), .b(x01), .O(z03));
  nand2  g23(.a(x01), .b(new_n40_), .O(new_n49_));
  nor4   g24(.a(new_n49_), .b(new_n43_), .c(x12), .d(new_n33_), .O(z04));
  nor2   g25(.a(new_n47_), .b(new_n41_), .O(z05));
  inv1   g26(.a(x08), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(new_n41_), .c(x00), .O(new_n53_));
  oai21  g28(.a(x10), .b(new_n41_), .c(new_n53_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n41_), .c(new_n40_), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(x05), .d(new_n29_), .O(new_n60_));
  aoi21  g35(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  nand3  g36(.a(new_n42_), .b(new_n33_), .c(x02), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  aoi21  g38(.a(new_n61_), .b(new_n54_), .c(new_n63_), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n31_), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n64_), .c(new_n26_), .O(z06));
  aoi21  g41(.a(new_n43_), .b(new_n39_), .c(new_n33_), .O(z07));
  nor2   g42(.a(new_n33_), .b(x02), .O(new_n68_));
  aoi21  g43(.a(new_n68_), .b(x13), .c(new_n63_), .O(new_n69_));
  oai21  g44(.a(new_n69_), .b(new_n65_), .c(new_n26_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n26_), .O(z09));
  nand2  g47(.a(x05), .b(x02), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x06), .c(new_n31_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n26_), .O(z10));
endmodule


