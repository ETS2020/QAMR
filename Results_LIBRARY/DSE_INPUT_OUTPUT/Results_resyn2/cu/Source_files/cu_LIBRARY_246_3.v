// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n70_, new_n72_, new_n73_;
  nand2  g00(.a(x07), .b(x06), .O(new_n26_));
  aoi21  g01(.a(x05), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(new_n27_), .c(new_n26_), .O(z00));
  nand2  g06(.a(new_n30_), .b(new_n27_), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n26_), .O(z01));
  inv1   g08(.a(x01), .O(new_n34_));
  nor2   g09(.a(new_n28_), .b(x04), .O(new_n35_));
  nor3   g10(.a(x13), .b(x03), .c(x02), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n35_), .c(new_n26_), .d(new_n34_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(x00), .O(z02));
  inv1   g13(.a(x00), .O(new_n39_));
  nor2   g14(.a(new_n37_), .b(new_n39_), .O(z03));
  nor2   g15(.a(new_n34_), .b(x00), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n36_), .c(new_n35_), .d(new_n26_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z04));
  nand2  g18(.a(new_n36_), .b(new_n35_), .O(new_n44_));
  nand2  g19(.a(x01), .b(x00), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n44_), .c(new_n26_), .O(z05));
  inv1   g21(.a(x13), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(new_n28_), .c(x02), .O(new_n48_));
  inv1   g23(.a(x07), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n39_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  inv1   g35(.a(x12), .O(new_n61_));
  nand4  g36(.a(x13), .b(new_n61_), .c(x05), .d(new_n60_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n59_), .c(new_n54_), .d(new_n49_), .O(new_n64_));
  inv1   g39(.a(x03), .O(new_n65_));
  nand3  g40(.a(new_n26_), .b(x04), .c(new_n65_), .O(new_n66_));
  aoi21  g41(.a(new_n64_), .b(new_n48_), .c(new_n66_), .O(z06));
  nand2  g42(.a(new_n44_), .b(new_n26_), .O(z07));
  aoi21  g43(.a(new_n62_), .b(new_n48_), .c(new_n66_), .O(z08));
  nand3  g44(.a(x13), .b(new_n49_), .c(x06), .O(new_n70_));
  inv1   g45(.a(new_n70_), .O(z09));
  inv1   g46(.a(x06), .O(new_n72_));
  nor2   g47(.a(new_n27_), .b(x07), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n72_), .O(z10));
endmodule


