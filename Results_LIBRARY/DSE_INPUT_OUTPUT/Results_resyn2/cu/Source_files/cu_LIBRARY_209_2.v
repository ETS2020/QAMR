// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand3  g02(.a(new_n27_), .b(x04), .c(x02), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  inv1   g06(.a(x11), .O(new_n32_));
  nand2  g07(.a(new_n28_), .b(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n31_), .c(new_n26_), .O(z00));
  nand2  g09(.a(new_n31_), .b(new_n26_), .O(new_n35_));
  nor2   g10(.a(x11), .b(new_n27_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n35_), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nand3  g16(.a(new_n29_), .b(new_n41_), .c(new_n26_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand3  g18(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x11), .c(new_n27_), .O(z02));
  nand3  g20(.a(new_n43_), .b(new_n40_), .c(x00), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(x11), .c(new_n27_), .O(z03));
  nand3  g22(.a(new_n43_), .b(x01), .c(new_n39_), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(x11), .c(new_n27_), .O(z04));
  nand4  g24(.a(x11), .b(x05), .c(x01), .d(x00), .O(new_n50_));
  nor2   g25(.a(new_n50_), .b(new_n42_), .O(z05));
  nand3  g26(.a(new_n41_), .b(new_n27_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n40_), .c(x00), .O(new_n54_));
  nand2  g29(.a(x10), .b(x01), .O(new_n55_));
  nand2  g30(.a(x08), .b(new_n40_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n55_), .c(new_n39_), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  inv1   g33(.a(x02), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n59_), .O(new_n61_));
  inv1   g36(.a(x07), .O(new_n62_));
  nand2  g37(.a(x11), .b(new_n62_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  nand2  g40(.a(x04), .b(new_n26_), .O(new_n66_));
  aoi21  g41(.a(new_n65_), .b(new_n52_), .c(new_n66_), .O(z06));
  aoi21  g42(.a(new_n42_), .b(x11), .c(new_n27_), .O(z07));
  and2   g43(.a(new_n61_), .b(new_n52_), .O(new_n69_));
  oai21  g44(.a(new_n69_), .b(new_n66_), .c(new_n37_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n36_), .O(z09));
  nand2  g47(.a(x05), .b(x02), .O(new_n73_));
  nand3  g48(.a(new_n73_), .b(x06), .c(new_n26_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n37_), .O(z10));
endmodule


