// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x04), .b(x02), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  aoi21  g03(.a(new_n28_), .b(x05), .c(x13), .O(z07));
  oai21  g04(.a(x04), .b(x02), .c(x05), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(x04), .b(x02), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n31_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n30_), .c(z07), .O(z00));
  nand2  g09(.a(new_n33_), .b(new_n30_), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n31_), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n35_), .O(z01));
  nand3  g13(.a(new_n36_), .b(x05), .c(new_n26_), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nand2  g15(.a(new_n27_), .b(new_n40_), .O(new_n41_));
  nor3   g16(.a(new_n41_), .b(new_n39_), .c(x00), .O(z02));
  inv1   g17(.a(x00), .O(new_n43_));
  nor3   g18(.a(new_n41_), .b(new_n39_), .c(new_n43_), .O(z03));
  nand4  g19(.a(new_n27_), .b(new_n26_), .c(x01), .d(new_n43_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x05), .c(x13), .O(z04));
  nand4  g21(.a(new_n27_), .b(new_n26_), .c(x01), .d(x00), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x05), .c(x13), .O(z05));
  inv1   g23(.a(x07), .O(new_n49_));
  nor2   g24(.a(new_n31_), .b(x03), .O(new_n50_));
  inv1   g25(.a(x04), .O(new_n51_));
  nor2   g26(.a(new_n51_), .b(x02), .O(new_n52_));
  nor2   g27(.a(new_n36_), .b(x12), .O(new_n53_));
  nand4  g28(.a(new_n53_), .b(new_n52_), .c(new_n50_), .d(new_n49_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n40_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  inv1   g34(.a(x10), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x01), .O(new_n61_));
  inv1   g36(.a(x08), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n40_), .O(new_n63_));
  nand3  g38(.a(new_n63_), .b(new_n61_), .c(new_n43_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n59_), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n54_), .c(new_n37_), .O(z06));
  nand3  g41(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n37_), .O(z08));
  and2   g43(.a(x13), .b(x06), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n70_));
  nand4  g45(.a(new_n70_), .b(new_n37_), .c(x06), .d(new_n26_), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
endmodule


