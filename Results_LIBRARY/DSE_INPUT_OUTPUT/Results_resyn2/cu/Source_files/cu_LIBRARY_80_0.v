// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n69_,
    new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x05), .b(new_n27_), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  inv1   g07(.a(x13), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g09(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n26_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  nor2   g13(.a(x03), .b(x02), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(new_n27_), .c(new_n38_), .d(new_n37_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(x05), .c(x13), .O(z02));
  nand4  g16(.a(new_n39_), .b(new_n27_), .c(new_n38_), .d(x00), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x13), .O(z03));
  nand3  g18(.a(new_n33_), .b(x05), .c(new_n27_), .O(new_n44_));
  nand2  g19(.a(new_n39_), .b(x01), .O(new_n45_));
  nor3   g20(.a(new_n45_), .b(new_n44_), .c(x00), .O(z04));
  nor3   g21(.a(new_n45_), .b(new_n44_), .c(new_n37_), .O(z05));
  inv1   g22(.a(x07), .O(new_n48_));
  nor2   g23(.a(new_n32_), .b(new_n27_), .O(new_n49_));
  nor2   g24(.a(new_n33_), .b(x12), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n49_), .c(new_n39_), .d(new_n48_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  inv1   g31(.a(x10), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x08), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n38_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(new_n37_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  nor2   g37(.a(new_n62_), .b(new_n51_), .O(z06));
  nand2  g38(.a(new_n39_), .b(new_n27_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(x05), .c(x13), .O(z07));
  inv1   g40(.a(new_n39_), .O(new_n66_));
  nand2  g41(.a(new_n50_), .b(new_n49_), .O(new_n67_));
  nor2   g42(.a(new_n67_), .b(new_n66_), .O(z08));
  nand2  g43(.a(x13), .b(x06), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n34_), .O(z09));
  nand3  g45(.a(new_n31_), .b(x06), .c(new_n26_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n34_), .O(z10));
endmodule


