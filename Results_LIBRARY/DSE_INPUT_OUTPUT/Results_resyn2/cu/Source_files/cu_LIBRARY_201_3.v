// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_, new_n71_, new_n73_, new_n75_;
  inv1   g00(.a(x09), .O(new_n26_));
  inv1   g01(.a(x11), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n30_), .O(new_n31_));
  aoi21  g06(.a(x05), .b(x02), .c(x03), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n31_), .c(new_n28_), .O(z00));
  inv1   g08(.a(z00), .O(z01));
  inv1   g09(.a(new_n28_), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nor2   g11(.a(new_n29_), .b(x04), .O(new_n37_));
  nor2   g12(.a(x03), .b(x02), .O(new_n38_));
  nand3  g13(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n39_), .c(new_n35_), .O(z02));
  nor2   g18(.a(x01), .b(new_n40_), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n39_), .c(new_n35_), .O(z03));
  nor2   g21(.a(new_n28_), .b(x13), .O(new_n47_));
  nor2   g22(.a(new_n41_), .b(x00), .O(new_n48_));
  nand4  g23(.a(new_n48_), .b(new_n47_), .c(new_n38_), .d(new_n37_), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z04));
  nand2  g25(.a(x01), .b(x00), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n39_), .c(new_n35_), .O(z05));
  inv1   g27(.a(x03), .O(new_n53_));
  nand2  g28(.a(x04), .b(new_n53_), .O(new_n54_));
  inv1   g29(.a(x02), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(x05), .d(new_n55_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  aoi21  g33(.a(new_n44_), .b(x09), .c(x07), .O(new_n59_));
  aoi21  g34(.a(new_n51_), .b(new_n26_), .c(new_n27_), .O(new_n60_));
  inv1   g35(.a(x10), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g37(.a(x08), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(new_n41_), .c(x00), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  nand3  g40(.a(new_n65_), .b(new_n59_), .c(new_n58_), .O(new_n66_));
  nand3  g41(.a(new_n47_), .b(new_n29_), .c(x02), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n66_), .c(new_n54_), .O(z06));
  nand2  g43(.a(new_n39_), .b(new_n35_), .O(z07));
  nand3  g44(.a(new_n36_), .b(new_n29_), .c(x02), .O(new_n70_));
  nand3  g45(.a(new_n35_), .b(x04), .c(new_n53_), .O(new_n71_));
  aoi21  g46(.a(new_n70_), .b(new_n57_), .c(new_n71_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n28_), .O(z09));
  nand2  g49(.a(new_n32_), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n35_), .O(z10));
endmodule


