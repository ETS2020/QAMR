// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x09), .b(new_n26_), .O(new_n27_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n28_));
  oai21  g03(.a(x04), .b(x02), .c(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  aoi21  g05(.a(new_n26_), .b(new_n30_), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  inv1   g08(.a(x00), .O(new_n34_));
  nor3   g09(.a(x13), .b(x04), .c(x03), .O(new_n35_));
  nor2   g10(.a(x02), .b(x01), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(x09), .c(new_n26_), .O(z02));
  nand3  g13(.a(new_n36_), .b(new_n35_), .c(x00), .O(new_n39_));
  aoi21  g14(.a(new_n39_), .b(x09), .c(new_n26_), .O(z03));
  inv1   g15(.a(x02), .O(new_n41_));
  nand2  g16(.a(x05), .b(new_n41_), .O(new_n42_));
  inv1   g17(.a(x03), .O(new_n43_));
  inv1   g18(.a(x13), .O(new_n44_));
  nand4  g19(.a(new_n44_), .b(x09), .c(new_n30_), .d(new_n43_), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n42_), .O(z07));
  nand3  g21(.a(z07), .b(x01), .c(new_n34_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z04));
  nand4  g23(.a(new_n35_), .b(new_n41_), .c(x01), .d(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x09), .c(new_n26_), .O(z05));
  inv1   g25(.a(new_n27_), .O(new_n51_));
  nand3  g26(.a(new_n44_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(x01), .c(x00), .O(new_n55_));
  nand2  g30(.a(x10), .b(x01), .O(new_n56_));
  inv1   g31(.a(x01), .O(new_n57_));
  nand2  g32(.a(x08), .b(new_n57_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n34_), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand3  g37(.a(x13), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(new_n42_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n60_), .c(new_n53_), .O(new_n65_));
  nand2  g40(.a(x04), .b(new_n43_), .O(new_n66_));
  oai21  g41(.a(new_n66_), .b(new_n65_), .c(new_n51_), .O(z06));
  inv1   g42(.a(new_n42_), .O(new_n68_));
  nand4  g43(.a(new_n68_), .b(x13), .c(new_n62_), .d(x09), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(new_n52_), .c(new_n66_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n27_), .O(z09));
  nand2  g47(.a(x09), .b(new_n41_), .O(new_n73_));
  nand2  g48(.a(x06), .b(new_n43_), .O(new_n74_));
  aoi21  g49(.a(new_n73_), .b(x05), .c(new_n74_), .O(z10));
endmodule


