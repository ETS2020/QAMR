// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_, new_n73_, new_n74_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor3   g01(.a(x04), .b(x03), .c(x02), .O(new_n27_));
  inv1   g02(.a(x03), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x13), .O(new_n32_));
  oai21  g07(.a(new_n27_), .b(new_n26_), .c(new_n32_), .O(z00));
  inv1   g08(.a(x02), .O(new_n34_));
  inv1   g09(.a(x04), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g11(.a(x13), .b(new_n26_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n36_), .c(new_n31_), .O(z01));
  nand3  g13(.a(new_n35_), .b(new_n28_), .c(new_n34_), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(x05), .c(new_n40_), .O(new_n42_));
  nor3   g17(.a(new_n42_), .b(new_n39_), .c(x00), .O(z02));
  nand3  g18(.a(new_n27_), .b(new_n40_), .c(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x05), .c(x13), .O(z03));
  inv1   g20(.a(x00), .O(new_n46_));
  nand3  g21(.a(new_n27_), .b(x01), .c(new_n46_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x05), .c(x13), .O(z04));
  nand3  g23(.a(new_n27_), .b(x01), .c(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x05), .c(x13), .O(z05));
  nand2  g25(.a(new_n41_), .b(new_n26_), .O(new_n51_));
  inv1   g26(.a(x07), .O(new_n52_));
  nor2   g27(.a(new_n26_), .b(x02), .O(new_n53_));
  nor2   g28(.a(new_n35_), .b(x03), .O(new_n54_));
  nor2   g29(.a(new_n41_), .b(x12), .O(new_n55_));
  nand4  g30(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n56_));
  inv1   g31(.a(x11), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x09), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n40_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  inv1   g36(.a(x10), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g38(.a(x08), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n40_), .O(new_n65_));
  nand3  g40(.a(new_n65_), .b(new_n63_), .c(new_n46_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n56_), .c(new_n51_), .O(z06));
  aoi21  g43(.a(new_n39_), .b(x05), .c(x13), .O(z07));
  nand3  g44(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n51_), .O(z08));
  and2   g46(.a(x13), .b(x06), .O(z09));
  inv1   g47(.a(new_n53_), .O(new_n73_));
  nand2  g48(.a(x06), .b(new_n28_), .O(new_n74_));
  aoi21  g49(.a(new_n73_), .b(new_n37_), .c(new_n74_), .O(z10));
endmodule


