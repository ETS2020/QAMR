// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x05), .b(new_n27_), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n30_), .c(new_n26_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x01), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(x05), .c(new_n27_), .d(new_n37_), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  nand3  g15(.a(new_n26_), .b(new_n31_), .c(new_n40_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n39_), .O(z02));
  nand3  g17(.a(new_n26_), .b(new_n31_), .c(x00), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n39_), .O(z03));
  nand3  g19(.a(new_n38_), .b(new_n27_), .c(new_n26_), .O(new_n45_));
  nand2  g20(.a(x01), .b(new_n40_), .O(new_n46_));
  or2    g21(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x05), .c(x02), .O(z04));
  nor4   g23(.a(new_n43_), .b(new_n28_), .c(x13), .d(new_n37_), .O(z05));
  nand3  g24(.a(new_n38_), .b(new_n32_), .c(x02), .O(new_n50_));
  inv1   g25(.a(x10), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x08), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(new_n40_), .O(new_n55_));
  inv1   g30(.a(x11), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x09), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(x00), .O(new_n60_));
  nor2   g35(.a(new_n38_), .b(x12), .O(new_n61_));
  inv1   g36(.a(x07), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(x05), .c(new_n31_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n61_), .c(new_n60_), .d(new_n55_), .O(new_n65_));
  nor2   g40(.a(new_n27_), .b(x03), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  aoi21  g42(.a(new_n65_), .b(new_n50_), .c(new_n67_), .O(z06));
  aoi21  g43(.a(new_n45_), .b(x05), .c(x02), .O(z07));
  aoi21  g44(.a(new_n66_), .b(new_n61_), .c(new_n32_), .O(new_n70_));
  nand3  g45(.a(new_n66_), .b(new_n38_), .c(new_n32_), .O(new_n71_));
  oai21  g46(.a(new_n70_), .b(x02), .c(new_n71_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  aoi21  g48(.a(new_n32_), .b(new_n31_), .c(new_n73_), .O(z09));
  nand2  g49(.a(new_n32_), .b(new_n31_), .O(new_n75_));
  nand2  g50(.a(x06), .b(new_n26_), .O(new_n76_));
  aoi21  g51(.a(new_n76_), .b(new_n75_), .c(new_n33_), .O(z10));
endmodule


