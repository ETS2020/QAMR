// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n75_,
    new_n77_, new_n79_;
  inv1   g00(.a(x09), .O(new_n26_));
  nand2  g01(.a(x10), .b(new_n26_), .O(new_n27_));
  aoi21  g02(.a(x05), .b(x02), .c(x03), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(new_n34_));
  and2   g09(.a(new_n34_), .b(new_n27_), .O(z00));
  inv1   g10(.a(new_n34_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z01));
  nor2   g13(.a(new_n30_), .b(x02), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  inv1   g16(.a(x00), .O(new_n42_));
  inv1   g17(.a(x01), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n41_), .c(new_n27_), .O(z02));
  nor2   g20(.a(x01), .b(new_n42_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n40_), .c(new_n39_), .d(new_n27_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z03));
  nor2   g23(.a(new_n43_), .b(x00), .O(new_n49_));
  nand4  g24(.a(new_n49_), .b(new_n40_), .c(new_n39_), .d(new_n27_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z04));
  nand2  g26(.a(x01), .b(x00), .O(new_n52_));
  oai21  g27(.a(new_n52_), .b(new_n41_), .c(new_n27_), .O(z05));
  inv1   g28(.a(x12), .O(new_n54_));
  nand4  g29(.a(x13), .b(new_n54_), .c(x05), .d(new_n32_), .O(new_n55_));
  inv1   g30(.a(x07), .O(new_n56_));
  nand3  g31(.a(x11), .b(x01), .c(x00), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nor2   g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  oai21  g34(.a(new_n43_), .b(x00), .c(x09), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x10), .O(new_n61_));
  nand2  g36(.a(new_n26_), .b(x00), .O(new_n62_));
  inv1   g37(.a(x08), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n42_), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(new_n62_), .c(new_n43_), .O(new_n65_));
  nand3  g40(.a(new_n65_), .b(new_n61_), .c(new_n59_), .O(new_n66_));
  inv1   g41(.a(x13), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(new_n30_), .c(x02), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n27_), .O(new_n70_));
  inv1   g45(.a(x03), .O(new_n71_));
  nand2  g46(.a(x04), .b(new_n71_), .O(new_n72_));
  aoi21  g47(.a(new_n70_), .b(new_n66_), .c(new_n72_), .O(z06));
  nand2  g48(.a(new_n41_), .b(new_n27_), .O(z07));
  and2   g49(.a(new_n68_), .b(new_n55_), .O(new_n75_));
  oai21  g50(.a(new_n75_), .b(new_n72_), .c(new_n27_), .O(z08));
  nand2  g51(.a(x13), .b(x06), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n27_), .O(z09));
  nand2  g53(.a(new_n28_), .b(x06), .O(new_n79_));
  nand2  g54(.a(new_n79_), .b(new_n27_), .O(z10));
endmodule


