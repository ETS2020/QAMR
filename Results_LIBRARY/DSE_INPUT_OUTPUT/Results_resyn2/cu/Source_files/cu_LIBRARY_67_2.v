// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n64_, new_n66_, new_n67_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand3  g02(.a(x05), .b(new_n27_), .c(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand4  g04(.a(x09), .b(new_n29_), .c(x04), .d(x02), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n28_), .c(x03), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  inv1   g07(.a(x09), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(x04), .O(new_n34_));
  inv1   g09(.a(x03), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(x05), .c(new_n27_), .d(new_n35_), .O(new_n37_));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  nand3  g14(.a(new_n26_), .b(new_n39_), .c(new_n38_), .O(new_n40_));
  oai21  g15(.a(new_n40_), .b(new_n37_), .c(new_n34_), .O(z02));
  nand3  g16(.a(new_n26_), .b(new_n39_), .c(x00), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n37_), .c(new_n34_), .O(z03));
  inv1   g18(.a(new_n37_), .O(new_n44_));
  nand2  g19(.a(new_n44_), .b(new_n26_), .O(new_n45_));
  nor3   g20(.a(new_n45_), .b(new_n39_), .c(x00), .O(z04));
  nand2  g21(.a(x01), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n45_), .c(new_n34_), .O(z05));
  inv1   g23(.a(x11), .O(new_n49_));
  nand3  g24(.a(new_n49_), .b(x01), .c(x00), .O(new_n50_));
  nand2  g25(.a(x10), .b(x01), .O(new_n51_));
  nand2  g26(.a(x08), .b(new_n39_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n51_), .c(new_n38_), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g29(.a(x12), .O(new_n55_));
  nand4  g30(.a(x13), .b(new_n55_), .c(x05), .d(new_n26_), .O(new_n56_));
  nor2   g31(.a(new_n56_), .b(x07), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand3  g33(.a(new_n36_), .b(new_n29_), .c(x02), .O(new_n59_));
  nand3  g34(.a(x09), .b(x04), .c(new_n35_), .O(new_n60_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z06));
  nand2  g36(.a(new_n45_), .b(new_n34_), .O(z07));
  aoi21  g37(.a(new_n59_), .b(new_n56_), .c(new_n60_), .O(z08));
  nand2  g38(.a(x13), .b(x06), .O(new_n64_));
  aoi21  g39(.a(new_n33_), .b(x04), .c(new_n64_), .O(z09));
  nand2  g40(.a(x05), .b(x02), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(x06), .c(new_n35_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n34_), .O(z10));
endmodule


