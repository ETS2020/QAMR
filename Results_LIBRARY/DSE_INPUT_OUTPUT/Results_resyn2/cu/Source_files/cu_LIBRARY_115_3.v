// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x04), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g02(.a(x03), .O(new_n28_));
  nand2  g03(.a(new_n26_), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  oai22  g05(.a(new_n30_), .b(x02), .c(new_n27_), .d(x05), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x02), .O(new_n33_));
  inv1   g08(.a(x05), .O(new_n34_));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  nor2   g11(.a(new_n29_), .b(x13), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(new_n33_), .c(new_n34_), .O(z02));
  inv1   g14(.a(x13), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(x05), .c(new_n33_), .d(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n29_), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(z03));
  nand3  g19(.a(new_n37_), .b(x01), .c(new_n35_), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n33_), .c(new_n34_), .O(z04));
  nand2  g21(.a(new_n42_), .b(x01), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z05));
  inv1   g23(.a(new_n27_), .O(new_n49_));
  inv1   g24(.a(x10), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(new_n35_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n36_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  nor2   g34(.a(x12), .b(new_n34_), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  nand3  g36(.a(x13), .b(new_n61_), .c(new_n33_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(new_n54_), .O(new_n64_));
  nand3  g39(.a(new_n40_), .b(new_n34_), .c(x02), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(new_n64_), .c(new_n49_), .O(z06));
  inv1   g41(.a(new_n37_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n33_), .c(new_n34_), .O(z07));
  aoi21  g43(.a(new_n27_), .b(new_n40_), .c(x05), .O(new_n69_));
  nand3  g44(.a(new_n60_), .b(new_n27_), .c(x13), .O(new_n70_));
  oai21  g45(.a(new_n69_), .b(new_n33_), .c(new_n70_), .O(z08));
  inv1   g46(.a(x06), .O(new_n72_));
  aoi21  g47(.a(x05), .b(x02), .c(new_n72_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(x13), .O(new_n74_));
  inv1   g49(.a(new_n74_), .O(z09));
  nand2  g50(.a(new_n73_), .b(new_n28_), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z10));
endmodule


