// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n46_, new_n47_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_;
  nor2   g00(.a(x12), .b(x11), .O(new_n26_));
  aoi21  g01(.a(x05), .b(x02), .c(x03), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x02), .O(new_n31_));
  nand2  g06(.a(x04), .b(new_n31_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n27_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  nor2   g09(.a(new_n34_), .b(new_n26_), .O(z00));
  inv1   g10(.a(new_n26_), .O(new_n36_));
  nand2  g11(.a(new_n34_), .b(new_n36_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z01));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nor2   g15(.a(x04), .b(x03), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(new_n40_), .c(x05), .d(new_n31_), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(new_n26_), .O(z07));
  nand2  g18(.a(z07), .b(new_n39_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x01), .O(z02));
  inv1   g20(.a(x01), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n42_), .c(new_n36_), .O(z03));
  nor2   g23(.a(new_n44_), .b(new_n46_), .O(z04));
  nand3  g24(.a(z07), .b(x01), .c(x00), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(z05));
  nand3  g26(.a(new_n40_), .b(new_n29_), .c(x02), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n46_), .c(x00), .O(new_n55_));
  nand2  g30(.a(x10), .b(x01), .O(new_n56_));
  nand2  g31(.a(x08), .b(new_n46_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n56_), .c(new_n39_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n31_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x07), .O(new_n62_));
  aoi21  g37(.a(new_n62_), .b(new_n59_), .c(new_n53_), .O(new_n63_));
  inv1   g38(.a(x03), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n64_), .O(new_n65_));
  oai21  g40(.a(new_n65_), .b(new_n63_), .c(new_n36_), .O(z06));
  nand3  g41(.a(new_n36_), .b(x04), .c(new_n64_), .O(new_n67_));
  aoi21  g42(.a(new_n61_), .b(new_n52_), .c(new_n67_), .O(z08));
  inv1   g43(.a(x06), .O(new_n69_));
  oai21  g44(.a(new_n40_), .b(new_n69_), .c(new_n36_), .O(z09));
  nor2   g45(.a(new_n26_), .b(new_n69_), .O(new_n71_));
  and2   g46(.a(new_n71_), .b(new_n27_), .O(z10));
endmodule


