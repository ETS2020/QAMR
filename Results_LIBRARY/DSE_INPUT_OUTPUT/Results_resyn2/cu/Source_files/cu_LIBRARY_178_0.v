// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_;
  inv1   g00(.a(x02), .O(new_n26_));
  nand2  g01(.a(x05), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  xor2a  g05(.a(x05), .b(x04), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  inv1   g08(.a(x01), .O(new_n34_));
  nor2   g09(.a(new_n28_), .b(x04), .O(new_n35_));
  nor2   g10(.a(x13), .b(x03), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n35_), .c(new_n26_), .d(new_n34_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(x00), .O(z02));
  inv1   g13(.a(x00), .O(new_n39_));
  nor2   g14(.a(new_n37_), .b(new_n39_), .O(z03));
  nor3   g15(.a(x13), .b(x04), .c(x03), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(x01), .c(new_n39_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z04));
  nand3  g18(.a(new_n41_), .b(x01), .c(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x05), .c(x02), .O(z05));
  inv1   g20(.a(x03), .O(new_n46_));
  nand2  g21(.a(x04), .b(new_n46_), .O(new_n47_));
  inv1   g22(.a(x13), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n28_), .c(x02), .O(new_n49_));
  inv1   g24(.a(x10), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(new_n39_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n34_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(x00), .O(new_n59_));
  nor2   g34(.a(new_n48_), .b(x12), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(x05), .c(new_n26_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand4  g38(.a(new_n63_), .b(new_n60_), .c(new_n59_), .d(new_n54_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n49_), .c(new_n47_), .O(z06));
  nor2   g40(.a(new_n41_), .b(new_n28_), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(x02), .O(z07));
  inv1   g42(.a(x12), .O(new_n68_));
  nand4  g43(.a(x13), .b(new_n68_), .c(x04), .d(new_n46_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(x05), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n26_), .O(new_n71_));
  nand2  g46(.a(new_n48_), .b(new_n28_), .O(new_n72_));
  oai21  g47(.a(new_n72_), .b(new_n47_), .c(new_n71_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  aoi21  g49(.a(new_n28_), .b(new_n26_), .c(new_n74_), .O(z09));
  nand2  g50(.a(new_n30_), .b(x06), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z10));
endmodule


