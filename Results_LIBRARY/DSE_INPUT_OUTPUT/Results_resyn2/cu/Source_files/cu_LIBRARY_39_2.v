// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x04), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  oai21  g03(.a(new_n28_), .b(x02), .c(x01), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  oai21  g05(.a(x05), .b(new_n30_), .c(new_n28_), .O(new_n31_));
  inv1   g06(.a(x03), .O(new_n32_));
  oai21  g07(.a(x04), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n31_), .c(new_n29_), .O(z00));
  nand2  g10(.a(new_n34_), .b(new_n31_), .O(new_n36_));
  nand2  g11(.a(new_n26_), .b(x01), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(z01));
  inv1   g13(.a(x01), .O(new_n39_));
  nand2  g14(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x13), .O(new_n42_));
  nand3  g17(.a(new_n27_), .b(new_n42_), .c(new_n32_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand2  g19(.a(new_n44_), .b(new_n41_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n40_), .c(new_n37_), .O(z02));
  nand2  g21(.a(new_n44_), .b(x00), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n40_), .c(new_n37_), .O(z03));
  nand2  g23(.a(new_n30_), .b(x01), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(new_n45_), .O(z04));
  nor2   g25(.a(new_n49_), .b(new_n47_), .O(z05));
  nand3  g26(.a(new_n42_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(x05), .c(new_n39_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(new_n54_), .c(new_n41_), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(x05), .c(new_n39_), .O(new_n59_));
  inv1   g34(.a(x10), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x01), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n59_), .c(x00), .O(new_n62_));
  inv1   g37(.a(x07), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(new_n63_), .d(new_n30_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  oai21  g41(.a(new_n62_), .b(new_n57_), .c(new_n66_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  inv1   g43(.a(x04), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(x03), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n37_), .O(z06));
  nor2   g47(.a(new_n43_), .b(x02), .O(z07));
  nand4  g48(.a(x13), .b(new_n64_), .c(x05), .d(new_n30_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n70_), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n37_), .O(z08));
  nand2  g52(.a(x13), .b(x06), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n37_), .O(z09));
  nand2  g54(.a(x05), .b(x02), .O(new_n80_));
  nand4  g55(.a(new_n80_), .b(new_n37_), .c(x06), .d(new_n32_), .O(new_n81_));
  inv1   g56(.a(new_n81_), .O(z10));
endmodule


