// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_;
  nand2  g00(.a(x13), .b(x12), .O(new_n26_));
  inv1   g01(.a(x03), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  oai21  g03(.a(x05), .b(new_n28_), .c(x04), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  nand2  g05(.a(x05), .b(new_n28_), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n26_), .O(z01));
  inv1   g09(.a(z01), .O(z00));
  inv1   g10(.a(x01), .O(new_n36_));
  nand2  g11(.a(new_n28_), .b(new_n36_), .O(new_n37_));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x13), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(x05), .c(new_n30_), .d(new_n27_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n37_), .c(new_n26_), .O(z02));
  nand2  g18(.a(new_n41_), .b(x00), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n37_), .c(new_n26_), .O(z03));
  nand2  g20(.a(new_n28_), .b(x01), .O(new_n46_));
  oai21  g21(.a(new_n46_), .b(new_n42_), .c(new_n26_), .O(z04));
  oai21  g22(.a(new_n46_), .b(new_n44_), .c(new_n26_), .O(z05));
  inv1   g23(.a(x05), .O(new_n49_));
  nand3  g24(.a(new_n39_), .b(new_n49_), .c(x02), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  aoi21  g27(.a(new_n52_), .b(new_n36_), .c(x00), .O(new_n53_));
  oai21  g28(.a(x10), .b(new_n36_), .c(new_n53_), .O(new_n54_));
  inv1   g29(.a(x11), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n36_), .c(new_n38_), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(x05), .d(new_n28_), .O(new_n60_));
  aoi21  g35(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n54_), .c(new_n51_), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n27_), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n62_), .c(new_n26_), .O(z06));
  oai21  g39(.a(new_n40_), .b(x02), .c(new_n26_), .O(z07));
  inv1   g40(.a(x12), .O(new_n66_));
  nand4  g41(.a(x13), .b(new_n66_), .c(x05), .d(new_n28_), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(new_n50_), .c(new_n63_), .O(z08));
  inv1   g43(.a(x06), .O(new_n69_));
  aoi21  g44(.a(new_n66_), .b(new_n69_), .c(new_n39_), .O(z09));
  nand2  g45(.a(x05), .b(x02), .O(new_n71_));
  nand4  g46(.a(new_n71_), .b(new_n26_), .c(x06), .d(new_n27_), .O(new_n72_));
  inv1   g47(.a(new_n72_), .O(z10));
endmodule


