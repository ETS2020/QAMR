// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  nand2  g00(.a(x10), .b(x05), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  nor2   g05(.a(x05), .b(new_n27_), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n28_), .c(x03), .O(new_n33_));
  oai21  g08(.a(new_n31_), .b(new_n30_), .c(new_n33_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n26_), .O(z01));
  inv1   g10(.a(z01), .O(z00));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x03), .O(new_n38_));
  inv1   g13(.a(x10), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(x05), .d(new_n38_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n37_), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(new_n29_), .c(x01), .O(z02));
  nor4   g19(.a(new_n41_), .b(new_n29_), .c(x01), .d(new_n37_), .O(z03));
  nand2  g20(.a(new_n30_), .b(x01), .O(new_n46_));
  nor2   g21(.a(new_n46_), .b(new_n43_), .O(z04));
  nor3   g22(.a(new_n46_), .b(new_n41_), .c(new_n37_), .O(z05));
  nand2  g23(.a(new_n31_), .b(new_n40_), .O(new_n49_));
  inv1   g24(.a(x01), .O(new_n50_));
  inv1   g25(.a(x09), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(new_n50_), .c(x00), .O(new_n52_));
  nor2   g27(.a(x08), .b(x00), .O(new_n53_));
  nand2  g28(.a(x11), .b(x00), .O(new_n54_));
  oai21  g29(.a(new_n53_), .b(x01), .c(new_n54_), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand4  g32(.a(x13), .b(new_n57_), .c(x05), .d(new_n27_), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  nand2  g34(.a(new_n39_), .b(new_n59_), .O(new_n60_));
  nor2   g35(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n56_), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n38_), .O(new_n63_));
  aoi21  g38(.a(new_n62_), .b(new_n49_), .c(new_n63_), .O(z06));
  nand3  g39(.a(new_n30_), .b(new_n40_), .c(new_n38_), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(new_n39_), .c(new_n32_), .O(z07));
  nand2  g41(.a(new_n58_), .b(new_n49_), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(x04), .c(new_n38_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n26_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n26_), .O(z09));
  nand2  g46(.a(x05), .b(x02), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x06), .c(new_n38_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n26_), .O(z10));
endmodule


