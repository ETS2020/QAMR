// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand2  g01(.a(x04), .b(x02), .O(new_n27_));
  oai21  g02(.a(new_n26_), .b(x04), .c(new_n27_), .O(new_n28_));
  aoi21  g03(.a(x05), .b(x02), .c(x03), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(new_n30_), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  inv1   g07(.a(x04), .O(new_n33_));
  nand3  g08(.a(x05), .b(new_n33_), .c(new_n32_), .O(new_n34_));
  inv1   g09(.a(new_n34_), .O(new_n35_));
  oai21  g10(.a(new_n35_), .b(x08), .c(new_n31_), .O(z00));
  nor2   g11(.a(x08), .b(x05), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n30_), .O(z01));
  inv1   g14(.a(x03), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nand4  g16(.a(new_n41_), .b(x05), .c(new_n33_), .d(new_n40_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n32_), .O(new_n44_));
  inv1   g19(.a(x00), .O(new_n45_));
  inv1   g20(.a(x01), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g22(.a(new_n47_), .b(new_n44_), .c(new_n38_), .O(z02));
  nand3  g23(.a(new_n32_), .b(new_n46_), .c(x00), .O(new_n49_));
  oai21  g24(.a(new_n49_), .b(new_n42_), .c(new_n38_), .O(z03));
  nor3   g25(.a(new_n44_), .b(new_n46_), .c(x00), .O(z04));
  nor4   g26(.a(new_n42_), .b(x02), .c(new_n46_), .d(new_n45_), .O(z05));
  nand3  g27(.a(new_n41_), .b(new_n26_), .c(x02), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(x01), .c(new_n45_), .O(new_n55_));
  nand2  g30(.a(x11), .b(x01), .O(new_n56_));
  nand2  g31(.a(x09), .b(new_n46_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n56_), .c(x00), .O(new_n58_));
  aoi21  g33(.a(new_n58_), .b(new_n55_), .c(new_n26_), .O(new_n59_));
  nor2   g34(.a(new_n47_), .b(x08), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(new_n61_), .d(new_n32_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(new_n64_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  nor2   g41(.a(new_n33_), .b(x03), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n38_), .O(z06));
  nand2  g44(.a(new_n44_), .b(new_n38_), .O(z07));
  nand4  g45(.a(x13), .b(new_n62_), .c(x05), .d(new_n32_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n38_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n37_), .O(z09));
  nand2  g51(.a(new_n29_), .b(x06), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n38_), .O(z10));
endmodule


