// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_;
  nand2  g00(.a(x07), .b(x05), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  nand2  g04(.a(x05), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n30_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n29_), .c(new_n27_), .O(z00));
  nor2   g09(.a(x07), .b(x02), .O(new_n35_));
  oai21  g10(.a(new_n35_), .b(x04), .c(new_n33_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  nor2   g12(.a(x13), .b(x03), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(new_n35_), .c(x05), .d(new_n28_), .O(new_n39_));
  nor3   g14(.a(new_n39_), .b(x01), .c(x00), .O(z02));
  inv1   g15(.a(x07), .O(new_n41_));
  inv1   g16(.a(x01), .O(new_n42_));
  inv1   g17(.a(x02), .O(new_n43_));
  nor3   g18(.a(x13), .b(x04), .c(x03), .O(new_n44_));
  nand4  g19(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n41_), .c(new_n31_), .O(z03));
  inv1   g21(.a(x00), .O(new_n47_));
  nand4  g22(.a(new_n44_), .b(new_n43_), .c(x01), .d(new_n47_), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n41_), .c(new_n31_), .O(z04));
  nor3   g24(.a(new_n39_), .b(new_n42_), .c(new_n47_), .O(z05));
  inv1   g25(.a(x03), .O(new_n51_));
  nand2  g26(.a(x04), .b(new_n51_), .O(new_n52_));
  inv1   g27(.a(x13), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n31_), .c(x02), .O(new_n54_));
  inv1   g29(.a(new_n54_), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(new_n42_), .c(x00), .O(new_n57_));
  oai21  g32(.a(x10), .b(new_n42_), .c(new_n57_), .O(new_n58_));
  inv1   g33(.a(x11), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n42_), .c(new_n47_), .O(new_n62_));
  inv1   g37(.a(x12), .O(new_n63_));
  nand4  g38(.a(x13), .b(new_n63_), .c(x05), .d(new_n43_), .O(new_n64_));
  aoi21  g39(.a(new_n62_), .b(new_n60_), .c(new_n64_), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(new_n58_), .c(new_n55_), .O(new_n66_));
  oai21  g41(.a(new_n66_), .b(new_n52_), .c(new_n26_), .O(z06));
  inv1   g42(.a(new_n39_), .O(z07));
  inv1   g43(.a(new_n64_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n41_), .O(new_n70_));
  aoi21  g45(.a(new_n70_), .b(new_n54_), .c(new_n52_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n26_), .O(z09));
  nor2   g48(.a(new_n35_), .b(new_n31_), .O(new_n74_));
  nand2  g49(.a(x06), .b(new_n51_), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n74_), .O(z10));
endmodule


