// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n44_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x11), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nor2   g03(.a(x05), .b(new_n28_), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  oai21  g05(.a(x04), .b(x02), .c(new_n30_), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  aoi21  g07(.a(new_n26_), .b(new_n32_), .c(x03), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n31_), .c(new_n27_), .O(z00));
  nand2  g09(.a(new_n29_), .b(x04), .O(new_n35_));
  nand4  g10(.a(x11), .b(x05), .c(new_n32_), .d(new_n28_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(new_n35_), .c(x03), .O(z01));
  inv1   g12(.a(x00), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n28_), .c(new_n39_), .d(new_n38_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(x11), .c(new_n26_), .O(z02));
  nor3   g17(.a(new_n36_), .b(x13), .c(x03), .O(z07));
  nand3  g18(.a(z07), .b(new_n39_), .c(x00), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(z03));
  nand4  g20(.a(new_n40_), .b(new_n28_), .c(x01), .d(new_n38_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(x11), .c(new_n26_), .O(z04));
  nand3  g22(.a(z07), .b(x01), .c(x00), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(z05));
  inv1   g24(.a(new_n27_), .O(new_n50_));
  inv1   g25(.a(x13), .O(new_n51_));
  nand2  g26(.a(new_n29_), .b(new_n51_), .O(new_n52_));
  inv1   g27(.a(new_n52_), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n39_), .c(x00), .O(new_n55_));
  nand2  g30(.a(x10), .b(x01), .O(new_n56_));
  nand2  g31(.a(x08), .b(new_n39_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n56_), .c(new_n38_), .O(new_n58_));
  nor2   g33(.a(x12), .b(x07), .O(new_n59_));
  nand4  g34(.a(new_n59_), .b(x13), .c(x05), .d(new_n28_), .O(new_n60_));
  aoi21  g35(.a(new_n58_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  nor2   g36(.a(new_n32_), .b(x03), .O(new_n62_));
  oai21  g37(.a(new_n61_), .b(new_n53_), .c(new_n62_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n50_), .O(z06));
  inv1   g39(.a(new_n62_), .O(new_n65_));
  inv1   g40(.a(x12), .O(new_n66_));
  nor2   g41(.a(new_n26_), .b(x02), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(x13), .c(new_n66_), .d(x11), .O(new_n68_));
  aoi21  g43(.a(new_n68_), .b(new_n52_), .c(new_n65_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n27_), .O(z09));
  inv1   g46(.a(x06), .O(new_n72_));
  aoi21  g47(.a(x11), .b(new_n28_), .c(new_n26_), .O(new_n73_));
  nor3   g48(.a(new_n73_), .b(new_n72_), .c(x03), .O(z10));
endmodule


