// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n50_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x07), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x05), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(x02), .O(new_n32_));
  inv1   g07(.a(x03), .O(new_n33_));
  oai21  g08(.a(x05), .b(x04), .c(new_n33_), .O(new_n34_));
  aoi21  g09(.a(new_n32_), .b(new_n30_), .c(new_n34_), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(new_n27_), .O(z00));
  nand3  g11(.a(x05), .b(new_n29_), .c(new_n28_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(new_n38_));
  oai21  g13(.a(new_n38_), .b(new_n26_), .c(new_n35_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z01));
  inv1   g15(.a(new_n27_), .O(new_n41_));
  inv1   g16(.a(x13), .O(new_n42_));
  nand4  g17(.a(new_n42_), .b(x05), .c(new_n29_), .d(new_n33_), .O(new_n43_));
  inv1   g18(.a(x00), .O(new_n44_));
  inv1   g19(.a(x01), .O(new_n45_));
  nand3  g20(.a(new_n28_), .b(new_n45_), .c(new_n44_), .O(new_n46_));
  oai21  g21(.a(new_n46_), .b(new_n43_), .c(new_n41_), .O(z02));
  nand3  g22(.a(new_n28_), .b(new_n45_), .c(x00), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n43_), .c(new_n41_), .O(z03));
  nand4  g24(.a(new_n38_), .b(new_n42_), .c(new_n33_), .d(x01), .O(new_n50_));
  nor2   g25(.a(new_n50_), .b(x00), .O(z04));
  nor2   g26(.a(new_n50_), .b(new_n44_), .O(z05));
  nand3  g27(.a(new_n42_), .b(new_n31_), .c(x02), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x10), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n45_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(new_n44_), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x05), .d(new_n28_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n63_), .c(new_n58_), .d(new_n26_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n53_), .O(new_n68_));
  nor2   g43(.a(new_n29_), .b(x03), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n41_), .O(z06));
  nand2  g46(.a(new_n42_), .b(new_n33_), .O(new_n72_));
  oai21  g47(.a(new_n72_), .b(new_n37_), .c(new_n41_), .O(z07));
  inv1   g48(.a(new_n69_), .O(new_n74_));
  inv1   g49(.a(new_n53_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n26_), .O(new_n76_));
  aoi21  g51(.a(new_n76_), .b(new_n65_), .c(new_n74_), .O(z08));
  nand2  g52(.a(x13), .b(x06), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n41_), .O(z09));
  nand2  g54(.a(x05), .b(x02), .O(new_n80_));
  nand3  g55(.a(new_n80_), .b(x06), .c(new_n33_), .O(new_n81_));
  nand2  g56(.a(new_n81_), .b(new_n41_), .O(z10));
endmodule


