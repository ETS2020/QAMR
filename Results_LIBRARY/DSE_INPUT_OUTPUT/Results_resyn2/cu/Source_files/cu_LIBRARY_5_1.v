// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x11), .O(new_n26_));
  nand2  g01(.a(x12), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n27_), .O(z01));
  inv1   g09(.a(x01), .O(new_n35_));
  nor2   g10(.a(new_n28_), .b(x04), .O(new_n36_));
  nor3   g11(.a(x13), .b(x03), .c(x02), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(new_n36_), .c(new_n27_), .d(new_n35_), .O(new_n38_));
  nor2   g13(.a(new_n38_), .b(x00), .O(z02));
  inv1   g14(.a(x00), .O(new_n40_));
  nor2   g15(.a(new_n38_), .b(new_n40_), .O(z03));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(new_n42_));
  nand2  g17(.a(x01), .b(new_n40_), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n42_), .c(new_n27_), .O(z04));
  nor2   g19(.a(new_n35_), .b(new_n40_), .O(new_n45_));
  nand4  g20(.a(new_n45_), .b(new_n37_), .c(new_n36_), .d(new_n27_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z05));
  inv1   g22(.a(x03), .O(new_n48_));
  nand2  g23(.a(x04), .b(new_n48_), .O(new_n49_));
  inv1   g24(.a(x13), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n28_), .c(x02), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n27_), .O(new_n53_));
  inv1   g28(.a(x07), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n35_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n40_), .O(new_n59_));
  nand2  g34(.a(new_n26_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n35_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x02), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand4  g40(.a(x13), .b(new_n65_), .c(x05), .d(new_n64_), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n63_), .c(new_n59_), .d(new_n54_), .O(new_n68_));
  aoi21  g43(.a(new_n68_), .b(new_n53_), .c(new_n49_), .O(z06));
  nand2  g44(.a(new_n42_), .b(new_n27_), .O(z07));
  nor2   g45(.a(new_n67_), .b(new_n52_), .O(new_n71_));
  oai21  g46(.a(new_n71_), .b(new_n49_), .c(new_n27_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n27_), .O(z09));
  inv1   g49(.a(new_n31_), .O(new_n75_));
  nand2  g50(.a(new_n27_), .b(x06), .O(new_n76_));
  nor2   g51(.a(new_n76_), .b(new_n75_), .O(z10));
endmodule


