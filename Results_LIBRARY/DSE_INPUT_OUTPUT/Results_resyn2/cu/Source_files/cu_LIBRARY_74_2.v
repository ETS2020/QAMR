// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_;
  inv1   g00(.a(x01), .O(new_n26_));
  nor2   g01(.a(x05), .b(new_n26_), .O(new_n27_));
  aoi21  g02(.a(x04), .b(x02), .c(x05), .O(new_n28_));
  inv1   g03(.a(x03), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nor2   g05(.a(x04), .b(x02), .O(new_n31_));
  oai21  g06(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nor2   g08(.a(new_n33_), .b(new_n27_), .O(z00));
  inv1   g09(.a(new_n27_), .O(new_n35_));
  nand2  g10(.a(new_n33_), .b(new_n35_), .O(new_n36_));
  inv1   g11(.a(new_n36_), .O(z01));
  inv1   g12(.a(x13), .O(new_n38_));
  nand4  g13(.a(new_n31_), .b(new_n38_), .c(x05), .d(new_n29_), .O(new_n39_));
  inv1   g14(.a(x00), .O(new_n40_));
  nand2  g15(.a(new_n26_), .b(new_n40_), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(z02));
  nor3   g17(.a(new_n39_), .b(x01), .c(new_n40_), .O(z03));
  or2    g18(.a(new_n39_), .b(new_n26_), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x00), .O(z04));
  nor2   g20(.a(new_n44_), .b(new_n40_), .O(z05));
  nand3  g21(.a(new_n38_), .b(new_n30_), .c(x02), .O(new_n47_));
  inv1   g22(.a(x09), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(x05), .c(new_n26_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n49_), .c(new_n40_), .O(new_n52_));
  inv1   g27(.a(x08), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(x05), .c(new_n26_), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(new_n54_), .c(x00), .O(new_n57_));
  inv1   g32(.a(x02), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(new_n62_));
  oai21  g37(.a(new_n57_), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  inv1   g39(.a(x04), .O(new_n65_));
  nor2   g40(.a(new_n65_), .b(x03), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n35_), .O(z06));
  nand2  g43(.a(new_n39_), .b(new_n35_), .O(z07));
  nand4  g44(.a(x13), .b(new_n60_), .c(x05), .d(new_n58_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n47_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n35_), .O(z08));
  nand2  g48(.a(x13), .b(x06), .O(new_n74_));
  nor2   g49(.a(new_n74_), .b(new_n27_), .O(z09));
  nand2  g50(.a(x05), .b(x02), .O(new_n76_));
  nand3  g51(.a(new_n76_), .b(x06), .c(new_n29_), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n35_), .O(z10));
endmodule


