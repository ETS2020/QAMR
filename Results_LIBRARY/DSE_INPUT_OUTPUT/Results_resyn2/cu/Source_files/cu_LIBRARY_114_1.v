// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand3  g03(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  inv1   g05(.a(x13), .O(new_n31_));
  nand4  g06(.a(new_n31_), .b(new_n30_), .c(x04), .d(x02), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n26_), .O(z00));
  inv1   g09(.a(new_n29_), .O(new_n35_));
  nor3   g10(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n36_));
  oai21  g11(.a(new_n36_), .b(new_n35_), .c(new_n26_), .O(new_n37_));
  nand2  g12(.a(x13), .b(new_n30_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(z01));
  nand3  g14(.a(new_n35_), .b(new_n31_), .c(new_n26_), .O(new_n40_));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x01), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n40_), .c(new_n38_), .O(z02));
  nor3   g19(.a(new_n40_), .b(x01), .c(new_n41_), .O(z03));
  nand4  g20(.a(new_n35_), .b(new_n31_), .c(new_n26_), .d(x01), .O(new_n46_));
  nor2   g21(.a(new_n46_), .b(x00), .O(z04));
  nor2   g22(.a(new_n46_), .b(new_n41_), .O(z05));
  inv1   g23(.a(x08), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n42_), .c(x00), .O(new_n50_));
  oai21  g25(.a(x10), .b(new_n42_), .c(new_n50_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n42_), .c(new_n41_), .O(new_n55_));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand4  g32(.a(x13), .b(new_n57_), .c(new_n56_), .d(new_n27_), .O(new_n58_));
  aoi21  g33(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  nor2   g34(.a(x05), .b(new_n27_), .O(new_n60_));
  aoi21  g35(.a(new_n59_), .b(new_n51_), .c(new_n60_), .O(new_n61_));
  nand2  g36(.a(x04), .b(new_n26_), .O(new_n62_));
  oai21  g37(.a(new_n62_), .b(new_n61_), .c(new_n38_), .O(z06));
  nand2  g38(.a(new_n40_), .b(new_n38_), .O(z07));
  nor2   g39(.a(x12), .b(x02), .O(new_n65_));
  aoi21  g40(.a(new_n65_), .b(x13), .c(new_n60_), .O(new_n66_));
  oai21  g41(.a(new_n66_), .b(new_n62_), .c(new_n38_), .O(z08));
  nand3  g42(.a(x13), .b(x06), .c(x05), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(z09));
  nand2  g44(.a(x05), .b(x02), .O(new_n70_));
  nand4  g45(.a(new_n70_), .b(new_n38_), .c(x06), .d(new_n26_), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
endmodule


