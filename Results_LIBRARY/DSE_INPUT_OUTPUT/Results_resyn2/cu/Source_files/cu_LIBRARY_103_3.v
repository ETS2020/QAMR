// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x05), .O(new_n26_));
  inv1   g01(.a(x10), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nand2  g04(.a(new_n26_), .b(new_n29_), .O(new_n30_));
  nor2   g05(.a(x04), .b(x02), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  nand2  g07(.a(new_n26_), .b(x02), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n32_), .c(x03), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n30_), .c(new_n28_), .O(z00));
  oai21  g10(.a(x10), .b(new_n26_), .c(new_n29_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  inv1   g12(.a(new_n37_), .O(z01));
  nor2   g13(.a(x13), .b(x03), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n27_), .c(x05), .O(new_n40_));
  nor4   g15(.a(new_n40_), .b(new_n32_), .c(x01), .d(x00), .O(z02));
  inv1   g16(.a(x01), .O(new_n42_));
  nand2  g17(.a(new_n39_), .b(new_n31_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n42_), .c(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n27_), .c(new_n26_), .O(z03));
  nor2   g21(.a(new_n42_), .b(x00), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n27_), .c(new_n26_), .O(z04));
  nand3  g24(.a(new_n44_), .b(x01), .c(x00), .O(new_n50_));
  aoi21  g25(.a(new_n50_), .b(new_n27_), .c(new_n26_), .O(z05));
  inv1   g26(.a(x13), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n26_), .c(x02), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n42_), .c(x00), .O(new_n55_));
  nor2   g30(.a(x08), .b(x00), .O(new_n56_));
  nand2  g31(.a(x11), .b(x00), .O(new_n57_));
  oai21  g32(.a(new_n56_), .b(x01), .c(new_n57_), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  inv1   g35(.a(x12), .O(new_n61_));
  nand4  g36(.a(x13), .b(new_n61_), .c(x05), .d(new_n60_), .O(new_n62_));
  inv1   g37(.a(x07), .O(new_n63_));
  nand2  g38(.a(new_n27_), .b(new_n63_), .O(new_n64_));
  nor2   g39(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  inv1   g41(.a(x03), .O(new_n67_));
  nand2  g42(.a(x04), .b(new_n67_), .O(new_n68_));
  aoi21  g43(.a(new_n66_), .b(new_n53_), .c(new_n68_), .O(z06));
  aoi21  g44(.a(new_n43_), .b(new_n27_), .c(new_n26_), .O(z07));
  aoi21  g45(.a(new_n62_), .b(new_n53_), .c(new_n68_), .O(new_n71_));
  or2    g46(.a(new_n71_), .b(new_n28_), .O(z08));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n28_), .O(z09));
  nand2  g49(.a(x05), .b(x02), .O(new_n75_));
  nand3  g50(.a(new_n75_), .b(x06), .c(new_n67_), .O(new_n76_));
  oai21  g51(.a(new_n27_), .b(new_n26_), .c(new_n76_), .O(z10));
endmodule


