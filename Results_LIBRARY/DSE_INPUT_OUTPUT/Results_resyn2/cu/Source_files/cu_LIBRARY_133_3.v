// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x03), .O(new_n26_));
  nand2  g01(.a(x10), .b(x08), .O(new_n27_));
  nand2  g02(.a(x05), .b(x02), .O(new_n28_));
  nand3  g03(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  inv1   g06(.a(x05), .O(new_n32_));
  nand2  g07(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g08(.a(x02), .O(new_n34_));
  nand2  g09(.a(x04), .b(new_n34_), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n33_), .c(new_n30_), .O(z00));
  inv1   g11(.a(z00), .O(z01));
  nor2   g12(.a(x01), .b(x00), .O(new_n38_));
  nor2   g13(.a(new_n32_), .b(x02), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n27_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z02));
  nand2  g17(.a(new_n40_), .b(new_n39_), .O(new_n43_));
  inv1   g18(.a(x01), .O(new_n44_));
  nand2  g19(.a(new_n44_), .b(x00), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n43_), .c(new_n27_), .O(z03));
  inv1   g21(.a(x00), .O(new_n47_));
  nand2  g22(.a(x01), .b(new_n47_), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n43_), .c(new_n27_), .O(z04));
  nor2   g24(.a(new_n44_), .b(new_n47_), .O(new_n50_));
  nand4  g25(.a(new_n50_), .b(new_n40_), .c(new_n39_), .d(new_n27_), .O(new_n51_));
  inv1   g26(.a(new_n51_), .O(z05));
  nand2  g27(.a(x04), .b(new_n26_), .O(new_n53_));
  inv1   g28(.a(x12), .O(new_n54_));
  nand4  g29(.a(x13), .b(new_n54_), .c(x05), .d(new_n34_), .O(new_n55_));
  inv1   g30(.a(new_n55_), .O(new_n56_));
  aoi21  g31(.a(new_n38_), .b(x08), .c(x07), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  oai21  g33(.a(new_n44_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x10), .O(new_n60_));
  inv1   g35(.a(x11), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g37(.a(x09), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n44_), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(new_n62_), .c(x00), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(new_n60_), .c(new_n57_), .d(new_n56_), .O(new_n66_));
  inv1   g41(.a(x13), .O(new_n67_));
  nand3  g42(.a(new_n67_), .b(new_n32_), .c(x02), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n27_), .O(new_n70_));
  aoi21  g45(.a(new_n70_), .b(new_n66_), .c(new_n53_), .O(z06));
  nand2  g46(.a(new_n43_), .b(new_n27_), .O(z07));
  nor2   g47(.a(new_n69_), .b(new_n56_), .O(new_n73_));
  oai21  g48(.a(new_n73_), .b(new_n53_), .c(new_n27_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n27_), .O(z09));
  nand2  g51(.a(new_n30_), .b(x06), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z10));
endmodule


