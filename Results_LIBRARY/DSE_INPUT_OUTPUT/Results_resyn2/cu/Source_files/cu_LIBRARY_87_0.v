// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_;
  nor2   g00(.a(x13), .b(x08), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  aoi21  g04(.a(x05), .b(x02), .c(x03), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  inv1   g09(.a(x02), .O(new_n35_));
  inv1   g10(.a(x03), .O(new_n36_));
  nor2   g11(.a(new_n27_), .b(x04), .O(new_n37_));
  nand3  g12(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  inv1   g13(.a(x13), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(x08), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(new_n38_), .O(z07));
  nand3  g16(.a(z07), .b(new_n34_), .c(new_n33_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z02));
  inv1   g18(.a(new_n38_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n34_), .c(x00), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(x08), .c(x13), .O(z03));
  nand3  g21(.a(new_n44_), .b(x01), .c(new_n33_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x08), .c(x13), .O(z04));
  nand3  g23(.a(z07), .b(x01), .c(x00), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(z05));
  inv1   g25(.a(new_n26_), .O(new_n51_));
  nand3  g26(.a(new_n39_), .b(new_n27_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(x01), .c(new_n33_), .O(new_n54_));
  nand2  g29(.a(x11), .b(x01), .O(new_n55_));
  nand2  g30(.a(x09), .b(new_n34_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n55_), .c(x00), .O(new_n57_));
  aoi21  g32(.a(new_n57_), .b(new_n54_), .c(new_n39_), .O(new_n58_));
  nor3   g33(.a(x08), .b(x01), .c(x00), .O(new_n59_));
  inv1   g34(.a(x07), .O(new_n60_));
  inv1   g35(.a(x12), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n60_), .c(x05), .d(new_n35_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(new_n63_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  inv1   g40(.a(x04), .O(new_n66_));
  nor2   g41(.a(new_n66_), .b(x03), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n51_), .O(z06));
  nand3  g44(.a(new_n61_), .b(x05), .c(new_n35_), .O(new_n70_));
  oai21  g45(.a(new_n70_), .b(new_n39_), .c(new_n52_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n51_), .O(z08));
  inv1   g48(.a(x06), .O(new_n74_));
  nor2   g49(.a(new_n39_), .b(new_n74_), .O(z09));
  inv1   g50(.a(new_n30_), .O(new_n76_));
  nor3   g51(.a(new_n76_), .b(new_n26_), .c(new_n74_), .O(z10));
endmodule


