// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_,
    new_n71_;
  inv1   g00(.a(x10), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x08), .O(new_n27_));
  aoi21  g02(.a(x05), .b(x02), .c(x03), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n32_), .O(new_n33_));
  nand4  g08(.a(new_n33_), .b(new_n31_), .c(new_n28_), .d(new_n27_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  inv1   g10(.a(x13), .O(new_n36_));
  nor2   g11(.a(x04), .b(x03), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(new_n36_), .c(x05), .d(new_n32_), .O(new_n38_));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nand3  g15(.a(new_n27_), .b(new_n40_), .c(new_n39_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n38_), .O(z02));
  nand2  g17(.a(new_n40_), .b(x00), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n38_), .c(new_n27_), .O(z03));
  nand2  g19(.a(x01), .b(new_n39_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n38_), .c(new_n27_), .O(z04));
  inv1   g21(.a(x08), .O(new_n47_));
  oai21  g22(.a(x10), .b(new_n47_), .c(x00), .O(new_n48_));
  nor3   g23(.a(new_n48_), .b(new_n38_), .c(new_n40_), .O(z05));
  inv1   g24(.a(x03), .O(new_n50_));
  nand2  g25(.a(x04), .b(new_n50_), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(x11), .b(x01), .O(new_n53_));
  oai21  g28(.a(new_n52_), .b(x01), .c(new_n53_), .O(new_n54_));
  nand2  g29(.a(x10), .b(x01), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n47_), .c(new_n39_), .O(new_n56_));
  oai21  g31(.a(new_n54_), .b(new_n48_), .c(new_n56_), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(x05), .d(new_n32_), .O(new_n59_));
  nor2   g34(.a(new_n59_), .b(x07), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g36(.a(new_n36_), .b(new_n30_), .c(x02), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n27_), .O(new_n64_));
  aoi21  g39(.a(new_n64_), .b(new_n61_), .c(new_n51_), .O(z06));
  nand2  g40(.a(new_n38_), .b(new_n27_), .O(z07));
  and2   g41(.a(new_n62_), .b(new_n59_), .O(new_n67_));
  oai21  g42(.a(new_n67_), .b(new_n51_), .c(new_n27_), .O(z08));
  nand2  g43(.a(x13), .b(x06), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n27_), .O(z09));
  nand3  g45(.a(new_n28_), .b(new_n27_), .c(x06), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
endmodule


