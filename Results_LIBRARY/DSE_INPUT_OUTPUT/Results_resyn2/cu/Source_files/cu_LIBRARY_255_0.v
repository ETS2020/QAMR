// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x05), .b(x02), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nor2   g04(.a(x04), .b(x02), .O(new_n30_));
  aoi21  g05(.a(new_n29_), .b(x04), .c(new_n30_), .O(new_n31_));
  or2    g06(.a(new_n31_), .b(new_n28_), .O(z00));
  nor3   g07(.a(new_n31_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nor2   g10(.a(new_n29_), .b(x03), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n30_), .c(new_n35_), .d(new_n34_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n39_));
  nor3   g14(.a(x13), .b(x04), .c(x03), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(x05), .c(x02), .O(z03));
  nor2   g17(.a(new_n37_), .b(new_n39_), .O(z04));
  nand3  g18(.a(new_n40_), .b(x01), .c(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x05), .c(x02), .O(z05));
  nand3  g20(.a(new_n35_), .b(new_n29_), .c(x02), .O(new_n46_));
  inv1   g21(.a(x10), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x01), .O(new_n48_));
  inv1   g23(.a(x08), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n48_), .c(new_n34_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n39_), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(x00), .O(new_n56_));
  nor2   g31(.a(new_n35_), .b(x12), .O(new_n57_));
  inv1   g32(.a(x02), .O(new_n58_));
  inv1   g33(.a(x07), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(x05), .c(new_n58_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n57_), .c(new_n56_), .d(new_n51_), .O(new_n62_));
  inv1   g37(.a(x04), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(x03), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(new_n65_));
  aoi21  g40(.a(new_n62_), .b(new_n46_), .c(new_n65_), .O(z06));
  nand2  g41(.a(new_n36_), .b(new_n35_), .O(new_n67_));
  nor3   g42(.a(new_n67_), .b(x04), .c(x02), .O(z07));
  aoi21  g43(.a(new_n64_), .b(new_n57_), .c(new_n29_), .O(new_n69_));
  nand3  g44(.a(new_n64_), .b(new_n35_), .c(new_n29_), .O(new_n70_));
  oai21  g45(.a(new_n69_), .b(x02), .c(new_n70_), .O(z08));
  inv1   g46(.a(new_n27_), .O(new_n72_));
  nand2  g47(.a(x13), .b(x06), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n72_), .O(z09));
  nand2  g49(.a(x05), .b(x02), .O(new_n75_));
  nand3  g50(.a(new_n75_), .b(x06), .c(new_n26_), .O(new_n76_));
  nand2  g51(.a(new_n76_), .b(new_n72_), .O(z10));
endmodule


