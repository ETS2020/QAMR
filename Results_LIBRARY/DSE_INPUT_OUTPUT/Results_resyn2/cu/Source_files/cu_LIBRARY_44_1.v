// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n50_, new_n51_, new_n53_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_;
  inv1   g00(.a(x06), .O(new_n26_));
  nor2   g01(.a(new_n26_), .b(x05), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  inv1   g03(.a(new_n28_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(x02), .O(new_n31_));
  inv1   g06(.a(x03), .O(new_n32_));
  oai21  g07(.a(x05), .b(x04), .c(new_n32_), .O(new_n33_));
  aoi21  g08(.a(new_n31_), .b(new_n29_), .c(new_n33_), .O(new_n34_));
  nor2   g09(.a(new_n34_), .b(new_n27_), .O(z00));
  inv1   g10(.a(x02), .O(new_n36_));
  inv1   g11(.a(x04), .O(new_n37_));
  nand3  g12(.a(x05), .b(new_n37_), .c(new_n36_), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(x06), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  inv1   g15(.a(new_n40_), .O(z01));
  inv1   g16(.a(x01), .O(new_n42_));
  nand2  g17(.a(new_n28_), .b(new_n42_), .O(new_n43_));
  inv1   g18(.a(x00), .O(new_n44_));
  inv1   g19(.a(x13), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(x05), .c(new_n32_), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nor2   g23(.a(new_n48_), .b(new_n43_), .O(z02));
  inv1   g24(.a(new_n27_), .O(new_n50_));
  nand2  g25(.a(new_n47_), .b(x00), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n43_), .c(new_n50_), .O(z03));
  nand2  g27(.a(new_n28_), .b(x01), .O(new_n53_));
  nor2   g28(.a(new_n53_), .b(new_n48_), .O(z04));
  oai21  g29(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(z05));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x10), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x08), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n42_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(new_n44_), .O(new_n61_));
  inv1   g36(.a(x11), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g38(.a(x09), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n42_), .O(new_n65_));
  nand3  g40(.a(new_n65_), .b(new_n63_), .c(x00), .O(new_n66_));
  inv1   g41(.a(x12), .O(new_n67_));
  nand4  g42(.a(x13), .b(new_n67_), .c(x05), .d(new_n36_), .O(new_n68_));
  inv1   g43(.a(new_n68_), .O(new_n69_));
  nand4  g44(.a(new_n69_), .b(new_n66_), .c(new_n61_), .d(new_n56_), .O(new_n70_));
  nand3  g45(.a(new_n45_), .b(new_n30_), .c(x02), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g47(.a(new_n37_), .b(x03), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n50_), .O(z06));
  nor2   g50(.a(new_n46_), .b(new_n29_), .O(z07));
  inv1   g51(.a(new_n73_), .O(new_n77_));
  inv1   g52(.a(new_n71_), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n26_), .O(new_n79_));
  aoi21  g54(.a(new_n79_), .b(new_n68_), .c(new_n77_), .O(z08));
  nand3  g55(.a(x13), .b(x06), .c(x05), .O(new_n81_));
  inv1   g56(.a(new_n81_), .O(z09));
  nand2  g57(.a(new_n32_), .b(new_n36_), .O(new_n83_));
  aoi21  g58(.a(new_n83_), .b(x05), .c(new_n26_), .O(z10));
endmodule


