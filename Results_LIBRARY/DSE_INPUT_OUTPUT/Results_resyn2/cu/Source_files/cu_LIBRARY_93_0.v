// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n66_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x04), .b(x03), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x03), .O(new_n29_));
  nand2  g04(.a(x04), .b(new_n29_), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(x02), .c(new_n28_), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  nor3   g09(.a(x13), .b(x04), .c(x03), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(x05), .c(x02), .O(z02));
  nand3  g12(.a(new_n35_), .b(new_n34_), .c(x00), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(x05), .c(x02), .O(z03));
  nand3  g14(.a(new_n35_), .b(x01), .c(new_n33_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(x05), .c(x02), .O(z04));
  nand3  g16(.a(new_n35_), .b(x01), .c(x00), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z05));
  inv1   g18(.a(x07), .O(new_n44_));
  inv1   g19(.a(x10), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g21(.a(x08), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n46_), .c(new_n33_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x02), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(x05), .d(new_n55_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(new_n54_), .c(new_n49_), .d(new_n44_), .O(new_n59_));
  inv1   g34(.a(x13), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n26_), .c(x02), .O(new_n61_));
  aoi21  g36(.a(new_n61_), .b(new_n59_), .c(new_n30_), .O(z06));
  nor2   g37(.a(new_n35_), .b(new_n26_), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(x02), .O(z07));
  aoi21  g39(.a(new_n61_), .b(new_n57_), .c(new_n30_), .O(z08));
  nand2  g40(.a(new_n26_), .b(new_n55_), .O(new_n66_));
  nand2  g41(.a(x13), .b(x06), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n66_), .O(z09));
  nand2  g43(.a(x06), .b(new_n29_), .O(new_n69_));
  nor2   g44(.a(new_n26_), .b(new_n55_), .O(new_n70_));
  aoi21  g45(.a(new_n69_), .b(new_n66_), .c(new_n70_), .O(z10));
endmodule


