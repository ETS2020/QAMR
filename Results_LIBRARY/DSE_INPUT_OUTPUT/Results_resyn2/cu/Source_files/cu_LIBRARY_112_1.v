// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand3  g02(.a(x05), .b(new_n27_), .c(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  inv1   g04(.a(x11), .O(new_n30_));
  nand4  g05(.a(new_n30_), .b(new_n29_), .c(x04), .d(x02), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n28_), .c(x03), .O(z01));
  inv1   g07(.a(z01), .O(z00));
  inv1   g08(.a(x00), .O(new_n34_));
  nor3   g09(.a(new_n28_), .b(x13), .c(x03), .O(z07));
  nand2  g10(.a(z07), .b(new_n34_), .O(new_n36_));
  nor2   g11(.a(new_n36_), .b(x01), .O(z02));
  nand2  g12(.a(x11), .b(new_n29_), .O(new_n38_));
  inv1   g13(.a(x01), .O(new_n39_));
  nand3  g14(.a(z07), .b(new_n39_), .c(x00), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n38_), .O(z03));
  oai21  g16(.a(new_n36_), .b(new_n39_), .c(new_n38_), .O(z04));
  nand3  g17(.a(z07), .b(x01), .c(x00), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n38_), .O(z05));
  inv1   g19(.a(x07), .O(new_n45_));
  inv1   g20(.a(x10), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(x01), .O(new_n47_));
  inv1   g22(.a(x08), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(new_n39_), .O(new_n49_));
  nand3  g24(.a(new_n49_), .b(new_n47_), .c(new_n34_), .O(new_n50_));
  nand2  g25(.a(new_n30_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n39_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x12), .O(new_n55_));
  nand4  g30(.a(x13), .b(new_n55_), .c(x05), .d(new_n26_), .O(new_n56_));
  inv1   g31(.a(new_n56_), .O(new_n57_));
  nand4  g32(.a(new_n57_), .b(new_n54_), .c(new_n50_), .d(new_n45_), .O(new_n58_));
  inv1   g33(.a(x13), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n29_), .c(x02), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nor2   g36(.a(new_n27_), .b(x03), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n38_), .O(z06));
  inv1   g39(.a(new_n62_), .O(new_n65_));
  nand4  g40(.a(new_n59_), .b(new_n30_), .c(new_n29_), .d(x02), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(new_n56_), .c(new_n65_), .O(z08));
  nand2  g42(.a(new_n38_), .b(x06), .O(new_n68_));
  nor2   g43(.a(new_n68_), .b(new_n59_), .O(z09));
  nor2   g44(.a(new_n29_), .b(new_n26_), .O(new_n70_));
  nor3   g45(.a(new_n70_), .b(new_n68_), .c(x03), .O(z10));
endmodule


