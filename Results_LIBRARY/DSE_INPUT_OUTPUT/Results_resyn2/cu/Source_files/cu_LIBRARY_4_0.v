// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_;
  nor2   g00(.a(x12), .b(x05), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x04), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nand2  g04(.a(x05), .b(new_n27_), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n29_), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g07(.a(new_n26_), .O(new_n33_));
  nand2  g08(.a(new_n31_), .b(new_n28_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n33_), .O(z01));
  inv1   g10(.a(x03), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n29_), .d(new_n36_), .O(new_n38_));
  inv1   g13(.a(x00), .O(new_n39_));
  inv1   g14(.a(x01), .O(new_n40_));
  nand3  g15(.a(new_n27_), .b(new_n40_), .c(new_n39_), .O(new_n41_));
  oai21  g16(.a(new_n41_), .b(new_n38_), .c(new_n33_), .O(z02));
  nand3  g17(.a(new_n27_), .b(new_n40_), .c(x00), .O(new_n43_));
  oai21  g18(.a(new_n43_), .b(new_n38_), .c(new_n33_), .O(z03));
  inv1   g19(.a(new_n38_), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n27_), .c(x01), .O(new_n46_));
  nor2   g21(.a(new_n46_), .b(x00), .O(z04));
  nor2   g22(.a(new_n46_), .b(new_n39_), .O(z05));
  inv1   g23(.a(x05), .O(new_n49_));
  nand3  g24(.a(new_n37_), .b(new_n49_), .c(x02), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x12), .O(new_n52_));
  inv1   g27(.a(new_n30_), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n40_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(new_n39_), .O(new_n58_));
  inv1   g33(.a(x11), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n40_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x07), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand3  g40(.a(x13), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n63_), .c(new_n58_), .d(new_n53_), .O(new_n68_));
  nand2  g43(.a(x04), .b(new_n36_), .O(new_n69_));
  aoi21  g44(.a(new_n68_), .b(new_n52_), .c(new_n69_), .O(z06));
  oai21  g45(.a(new_n38_), .b(x02), .c(new_n33_), .O(z07));
  nand3  g46(.a(x13), .b(new_n65_), .c(new_n27_), .O(new_n72_));
  and2   g47(.a(new_n72_), .b(new_n50_), .O(new_n73_));
  oai21  g48(.a(new_n73_), .b(new_n69_), .c(new_n33_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n26_), .O(z09));
  nand2  g51(.a(x05), .b(x02), .O(new_n77_));
  nand3  g52(.a(new_n77_), .b(x06), .c(new_n36_), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n33_), .O(z10));
endmodule


