// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n44_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  nor2   g00(.a(x12), .b(x05), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  oai21  g02(.a(x05), .b(new_n27_), .c(x04), .O(new_n28_));
  aoi21  g03(.a(x05), .b(new_n27_), .c(x04), .O(new_n29_));
  nor2   g04(.a(new_n29_), .b(x03), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(new_n26_), .O(new_n33_));
  inv1   g08(.a(x13), .O(new_n34_));
  nor2   g09(.a(x04), .b(x03), .O(new_n35_));
  nand4  g10(.a(new_n35_), .b(new_n34_), .c(x05), .d(new_n27_), .O(new_n36_));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  oai21  g14(.a(new_n39_), .b(new_n36_), .c(new_n33_), .O(z02));
  nor2   g15(.a(new_n36_), .b(new_n37_), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  inv1   g17(.a(new_n42_), .O(z03));
  nand2  g18(.a(x01), .b(new_n37_), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n36_), .c(new_n33_), .O(z04));
  nand2  g20(.a(new_n41_), .b(x01), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(new_n33_), .O(z05));
  inv1   g22(.a(x08), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(new_n38_), .c(x00), .O(new_n49_));
  oai21  g24(.a(x10), .b(new_n38_), .c(new_n49_), .O(new_n50_));
  inv1   g25(.a(x11), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  aoi21  g28(.a(new_n53_), .b(new_n38_), .c(new_n37_), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(new_n55_), .d(new_n27_), .O(new_n57_));
  aoi21  g32(.a(new_n54_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  inv1   g33(.a(x05), .O(new_n59_));
  nand3  g34(.a(new_n34_), .b(new_n59_), .c(x02), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  aoi21  g36(.a(new_n58_), .b(new_n50_), .c(new_n61_), .O(new_n62_));
  inv1   g37(.a(x03), .O(new_n63_));
  nand2  g38(.a(x04), .b(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n64_), .b(new_n62_), .c(new_n33_), .O(z06));
  inv1   g40(.a(new_n36_), .O(z07));
  nor2   g41(.a(x12), .b(x02), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(x13), .c(new_n61_), .O(new_n68_));
  oai21  g43(.a(new_n68_), .b(new_n64_), .c(new_n33_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n33_), .O(z09));
  nand2  g46(.a(x05), .b(x02), .O(new_n72_));
  nand3  g47(.a(new_n72_), .b(x06), .c(new_n63_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n33_), .O(z10));
endmodule


