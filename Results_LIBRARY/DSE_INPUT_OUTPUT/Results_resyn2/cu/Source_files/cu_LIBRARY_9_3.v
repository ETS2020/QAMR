// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  nand2  g04(.a(x05), .b(x02), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(new_n29_), .c(new_n26_), .O(z00));
  inv1   g06(.a(x02), .O(new_n32_));
  nand2  g07(.a(x04), .b(new_n26_), .O(new_n33_));
  nor2   g08(.a(x04), .b(x03), .O(new_n34_));
  inv1   g09(.a(new_n34_), .O(new_n35_));
  aoi22  g10(.a(new_n35_), .b(new_n32_), .c(new_n33_), .d(new_n27_), .O(z01));
  inv1   g11(.a(x01), .O(new_n37_));
  inv1   g12(.a(x04), .O(new_n38_));
  inv1   g13(.a(x13), .O(new_n39_));
  nor2   g14(.a(x03), .b(x02), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(x05), .d(new_n38_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x00), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n37_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(z02));
  inv1   g19(.a(x00), .O(new_n45_));
  nor3   g20(.a(new_n41_), .b(x01), .c(new_n45_), .O(z03));
  nand2  g21(.a(new_n42_), .b(x01), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z04));
  nand4  g23(.a(new_n34_), .b(new_n39_), .c(x01), .d(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n32_), .c(new_n27_), .O(z05));
  inv1   g25(.a(x08), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n37_), .c(x00), .O(new_n52_));
  oai21  g27(.a(x10), .b(new_n37_), .c(new_n52_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  aoi21  g31(.a(new_n56_), .b(new_n37_), .c(new_n45_), .O(new_n57_));
  inv1   g32(.a(x07), .O(new_n58_));
  inv1   g33(.a(x12), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(new_n58_), .d(x05), .O(new_n60_));
  aoi21  g35(.a(new_n57_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  nor2   g36(.a(x13), .b(new_n32_), .O(new_n62_));
  aoi21  g37(.a(new_n61_), .b(new_n53_), .c(new_n62_), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n33_), .c(new_n30_), .O(z06));
  inv1   g39(.a(new_n41_), .O(z07));
  nand3  g40(.a(x13), .b(new_n59_), .c(x05), .O(new_n66_));
  oai21  g41(.a(new_n33_), .b(x13), .c(new_n27_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(x02), .O(new_n68_));
  oai21  g43(.a(new_n66_), .b(new_n33_), .c(new_n68_), .O(z08));
  inv1   g44(.a(x06), .O(new_n70_));
  oai21  g45(.a(new_n39_), .b(new_n70_), .c(new_n30_), .O(z09));
  oai21  g46(.a(new_n70_), .b(x03), .c(new_n30_), .O(z10));
endmodule


