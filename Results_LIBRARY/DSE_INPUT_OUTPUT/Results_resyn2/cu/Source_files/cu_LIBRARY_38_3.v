// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n72_, new_n74_;
  inv1   g00(.a(x07), .O(new_n26_));
  inv1   g01(.a(x11), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(new_n28_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(x04), .b(x02), .O(new_n31_));
  oai21  g06(.a(new_n30_), .b(x04), .c(new_n31_), .O(new_n32_));
  aoi21  g07(.a(x05), .b(x02), .c(x03), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n32_), .c(new_n29_), .O(z00));
  nand2  g09(.a(new_n33_), .b(new_n32_), .O(new_n35_));
  nand2  g10(.a(new_n35_), .b(new_n29_), .O(z01));
  inv1   g11(.a(x01), .O(new_n37_));
  nor2   g12(.a(new_n30_), .b(x04), .O(new_n38_));
  nor3   g13(.a(x13), .b(x03), .c(x02), .O(new_n39_));
  nor2   g14(.a(new_n28_), .b(x00), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n37_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z02));
  nand2  g17(.a(new_n39_), .b(new_n38_), .O(new_n43_));
  nand2  g18(.a(new_n37_), .b(x00), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n43_), .c(new_n29_), .O(z03));
  nand4  g20(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(x01), .O(new_n46_));
  inv1   g21(.a(new_n46_), .O(z04));
  nand2  g22(.a(x01), .b(x00), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n43_), .c(new_n29_), .O(z05));
  inv1   g24(.a(x13), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n30_), .c(x02), .O(new_n51_));
  nand2  g26(.a(new_n27_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  inv1   g30(.a(x00), .O(new_n56_));
  inv1   g31(.a(x10), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x08), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n37_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  inv1   g36(.a(x02), .O(new_n62_));
  inv1   g37(.a(x12), .O(new_n63_));
  nand4  g38(.a(x13), .b(new_n63_), .c(x05), .d(new_n62_), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(new_n61_), .c(new_n55_), .d(new_n26_), .O(new_n66_));
  inv1   g41(.a(x03), .O(new_n67_));
  nand3  g42(.a(new_n29_), .b(x04), .c(new_n67_), .O(new_n68_));
  aoi21  g43(.a(new_n66_), .b(new_n51_), .c(new_n68_), .O(z06));
  nand2  g44(.a(new_n43_), .b(new_n29_), .O(z07));
  aoi21  g45(.a(new_n64_), .b(new_n51_), .c(new_n68_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nor2   g47(.a(new_n72_), .b(new_n28_), .O(z09));
  nand2  g48(.a(new_n33_), .b(x06), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n29_), .O(z10));
endmodule


