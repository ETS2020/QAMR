// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n73_;
  nor2   g00(.a(x04), .b(x03), .O(new_n26_));
  inv1   g01(.a(new_n26_), .O(new_n27_));
  inv1   g02(.a(x03), .O(new_n28_));
  nand2  g03(.a(x04), .b(new_n28_), .O(new_n29_));
  aoi22  g04(.a(new_n29_), .b(x02), .c(new_n27_), .d(x05), .O(z01));
  inv1   g05(.a(z01), .O(z00));
  inv1   g06(.a(x00), .O(new_n32_));
  inv1   g07(.a(x01), .O(new_n33_));
  inv1   g08(.a(x13), .O(new_n34_));
  nand2  g09(.a(new_n26_), .b(new_n34_), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(new_n36_));
  nand3  g11(.a(new_n36_), .b(new_n33_), .c(new_n32_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(x05), .c(x02), .O(z02));
  inv1   g13(.a(x02), .O(new_n39_));
  nand2  g14(.a(x05), .b(new_n39_), .O(new_n40_));
  nor4   g15(.a(new_n40_), .b(new_n35_), .c(x01), .d(new_n32_), .O(z03));
  nand3  g16(.a(new_n36_), .b(x01), .c(new_n32_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z04));
  nand3  g18(.a(new_n36_), .b(x01), .c(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x05), .c(x02), .O(z05));
  inv1   g20(.a(x10), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(x01), .O(new_n47_));
  inv1   g22(.a(x08), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nand3  g24(.a(new_n49_), .b(new_n47_), .c(new_n32_), .O(new_n50_));
  inv1   g25(.a(x11), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  nor2   g30(.a(new_n34_), .b(x12), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(x04), .c(new_n28_), .O(new_n58_));
  inv1   g33(.a(new_n58_), .O(new_n59_));
  nand4  g34(.a(new_n59_), .b(new_n56_), .c(new_n55_), .d(new_n50_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(x05), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  inv1   g37(.a(x05), .O(new_n63_));
  nand4  g38(.a(new_n34_), .b(new_n63_), .c(x04), .d(new_n28_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n62_), .O(z06));
  nor2   g40(.a(new_n40_), .b(new_n35_), .O(z07));
  nand3  g41(.a(new_n56_), .b(x05), .c(new_n39_), .O(new_n67_));
  nand3  g42(.a(new_n34_), .b(new_n63_), .c(x02), .O(new_n68_));
  aoi21  g43(.a(new_n68_), .b(new_n67_), .c(new_n29_), .O(z08));
  nand2  g44(.a(x13), .b(x06), .O(new_n70_));
  aoi21  g45(.a(new_n63_), .b(new_n39_), .c(new_n70_), .O(z09));
  nand2  g46(.a(new_n63_), .b(x02), .O(new_n72_));
  nand2  g47(.a(x06), .b(new_n28_), .O(new_n73_));
  aoi21  g48(.a(new_n72_), .b(new_n40_), .c(new_n73_), .O(z10));
endmodule


