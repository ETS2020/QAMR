// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x02), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nor2   g02(.a(x04), .b(x03), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nor2   g04(.a(new_n29_), .b(x03), .O(new_n30_));
  oai22  g05(.a(new_n30_), .b(new_n26_), .c(new_n28_), .d(new_n27_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  nor3   g09(.a(x13), .b(x04), .c(x03), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(x05), .c(x02), .O(z02));
  inv1   g12(.a(new_n35_), .O(new_n38_));
  nand2  g13(.a(x05), .b(new_n26_), .O(new_n39_));
  nand2  g14(.a(new_n34_), .b(x00), .O(new_n40_));
  nor3   g15(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(z03));
  nand3  g16(.a(new_n35_), .b(x01), .c(new_n33_), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x02), .O(z04));
  nand3  g18(.a(new_n35_), .b(x01), .c(x00), .O(new_n44_));
  aoi21  g19(.a(new_n44_), .b(x05), .c(x02), .O(z05));
  inv1   g20(.a(new_n30_), .O(new_n46_));
  inv1   g21(.a(x13), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(new_n27_), .c(x02), .O(new_n48_));
  inv1   g23(.a(x10), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n50_), .c(new_n33_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  nor2   g33(.a(new_n47_), .b(x12), .O(new_n59_));
  inv1   g34(.a(x07), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(x05), .c(new_n26_), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(new_n59_), .c(new_n58_), .d(new_n53_), .O(new_n63_));
  aoi21  g38(.a(new_n63_), .b(new_n48_), .c(new_n46_), .O(z06));
  nor2   g39(.a(new_n39_), .b(new_n38_), .O(z07));
  aoi21  g40(.a(new_n59_), .b(new_n30_), .c(new_n27_), .O(new_n66_));
  nand3  g41(.a(new_n30_), .b(new_n47_), .c(new_n27_), .O(new_n67_));
  oai21  g42(.a(new_n66_), .b(x02), .c(new_n67_), .O(z08));
  nand2  g43(.a(x13), .b(x06), .O(new_n69_));
  aoi21  g44(.a(new_n27_), .b(new_n26_), .c(new_n69_), .O(z09));
  nand2  g45(.a(new_n27_), .b(x02), .O(new_n71_));
  inv1   g46(.a(x03), .O(new_n72_));
  nand2  g47(.a(x06), .b(new_n72_), .O(new_n73_));
  aoi21  g48(.a(new_n71_), .b(new_n39_), .c(new_n73_), .O(z10));
endmodule


