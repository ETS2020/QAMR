// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n44_, new_n45_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n67_, new_n69_, new_n70_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  inv1   g02(.a(x13), .O(new_n28_));
  nand3  g03(.a(new_n28_), .b(new_n27_), .c(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(x04), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  oai21  g06(.a(new_n27_), .b(x02), .c(new_n31_), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n30_), .c(new_n26_), .O(z00));
  inv1   g08(.a(z00), .O(z01));
  nand2  g09(.a(x13), .b(new_n27_), .O(new_n35_));
  nand4  g10(.a(new_n28_), .b(x05), .c(new_n31_), .d(new_n26_), .O(new_n36_));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x01), .O(new_n38_));
  inv1   g13(.a(x02), .O(new_n39_));
  nand3  g14(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  oai21  g15(.a(new_n40_), .b(new_n36_), .c(new_n35_), .O(z02));
  nand3  g16(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n36_), .c(new_n35_), .O(z03));
  inv1   g18(.a(new_n36_), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n39_), .c(x01), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(x00), .O(z04));
  nor2   g21(.a(new_n45_), .b(new_n37_), .O(z05));
  inv1   g22(.a(x07), .O(new_n48_));
  inv1   g23(.a(x10), .O(new_n49_));
  nand2  g24(.a(new_n49_), .b(x01), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n50_), .c(new_n37_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n38_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x12), .O(new_n59_));
  nand4  g34(.a(x13), .b(new_n59_), .c(x05), .d(new_n39_), .O(new_n60_));
  inv1   g35(.a(new_n60_), .O(new_n61_));
  nand4  g36(.a(new_n61_), .b(new_n58_), .c(new_n53_), .d(new_n48_), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n26_), .O(new_n63_));
  aoi21  g38(.a(new_n62_), .b(new_n29_), .c(new_n63_), .O(z06));
  oai21  g39(.a(new_n36_), .b(x02), .c(new_n35_), .O(z07));
  aoi21  g40(.a(new_n60_), .b(new_n29_), .c(new_n63_), .O(z08));
  inv1   g41(.a(x06), .O(new_n67_));
  aoi21  g42(.a(new_n67_), .b(x05), .c(new_n28_), .O(z09));
  nand2  g43(.a(x05), .b(x02), .O(new_n69_));
  nand3  g44(.a(new_n69_), .b(x06), .c(new_n26_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n35_), .O(z10));
endmodule


