// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n72_;
  inv1   g00(.a(x09), .O(new_n26_));
  nand2  g01(.a(x11), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n27_), .O(z01));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x01), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(x04), .O(new_n38_));
  inv1   g13(.a(x13), .O(new_n39_));
  nand4  g14(.a(new_n39_), .b(x05), .c(new_n38_), .d(new_n37_), .O(new_n40_));
  inv1   g15(.a(x02), .O(new_n41_));
  nand2  g16(.a(new_n27_), .b(new_n41_), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(new_n40_), .O(z07));
  nand3  g18(.a(z07), .b(new_n36_), .c(new_n35_), .O(new_n44_));
  inv1   g19(.a(new_n44_), .O(z02));
  inv1   g20(.a(new_n40_), .O(new_n46_));
  nand2  g21(.a(new_n46_), .b(x00), .O(new_n47_));
  nand2  g22(.a(new_n41_), .b(new_n36_), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n47_), .c(new_n27_), .O(z03));
  nand3  g24(.a(new_n41_), .b(x01), .c(new_n35_), .O(new_n50_));
  oai21  g25(.a(new_n50_), .b(new_n40_), .c(new_n27_), .O(z04));
  nand2  g26(.a(new_n41_), .b(x01), .O(new_n52_));
  oai21  g27(.a(new_n52_), .b(new_n47_), .c(new_n27_), .O(z05));
  inv1   g28(.a(x11), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(x01), .c(new_n26_), .O(new_n55_));
  and2   g30(.a(x10), .b(x01), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  oai21  g32(.a(new_n57_), .b(x01), .c(new_n35_), .O(new_n58_));
  oai22  g33(.a(new_n58_), .b(new_n56_), .c(new_n55_), .d(new_n35_), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n41_), .O(new_n61_));
  nor2   g36(.a(new_n61_), .b(x07), .O(new_n62_));
  nand3  g37(.a(new_n39_), .b(new_n28_), .c(x02), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  aoi21  g39(.a(new_n62_), .b(new_n59_), .c(new_n64_), .O(new_n65_));
  nand2  g40(.a(x04), .b(new_n37_), .O(new_n66_));
  oai21  g41(.a(new_n66_), .b(new_n65_), .c(new_n27_), .O(z06));
  nand3  g42(.a(new_n27_), .b(x04), .c(new_n37_), .O(new_n68_));
  aoi21  g43(.a(new_n63_), .b(new_n61_), .c(new_n68_), .O(z08));
  nand2  g44(.a(new_n27_), .b(x06), .O(new_n70_));
  nor2   g45(.a(new_n70_), .b(new_n39_), .O(z09));
  inv1   g46(.a(new_n31_), .O(new_n72_));
  nor2   g47(.a(new_n70_), .b(new_n72_), .O(z10));
endmodule


