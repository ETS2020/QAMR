// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n40_, new_n42_,
    new_n44_, new_n46_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x11), .O(new_n26_));
  nor2   g01(.a(x04), .b(x02), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(x02), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n27_), .O(new_n33_));
  inv1   g08(.a(x03), .O(new_n34_));
  oai21  g09(.a(x05), .b(x04), .c(new_n34_), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n29_), .O(z00));
  nand2  g12(.a(new_n26_), .b(new_n30_), .O(new_n38_));
  oai21  g13(.a(new_n35_), .b(new_n33_), .c(new_n38_), .O(z01));
  inv1   g14(.a(x00), .O(new_n40_));
  nor3   g15(.a(new_n28_), .b(x13), .c(x03), .O(z07));
  nand2  g16(.a(z07), .b(new_n40_), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(x01), .O(z02));
  nand2  g18(.a(z07), .b(x00), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x01), .O(z03));
  inv1   g20(.a(x01), .O(new_n46_));
  oai21  g21(.a(new_n42_), .b(new_n46_), .c(new_n38_), .O(z04));
  nor2   g22(.a(new_n44_), .b(new_n46_), .O(z05));
  inv1   g23(.a(x13), .O(new_n49_));
  nand2  g24(.a(new_n32_), .b(new_n49_), .O(new_n50_));
  inv1   g25(.a(new_n50_), .O(new_n51_));
  nand2  g26(.a(new_n26_), .b(x01), .O(new_n52_));
  inv1   g27(.a(x09), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(x05), .c(new_n46_), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x00), .O(new_n56_));
  nand2  g31(.a(x08), .b(new_n46_), .O(new_n57_));
  nand2  g32(.a(x10), .b(x01), .O(new_n58_));
  nand4  g33(.a(new_n58_), .b(new_n57_), .c(x05), .d(new_n40_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n63_));
  aoi21  g38(.a(new_n59_), .b(new_n56_), .c(new_n63_), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n34_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  oai21  g41(.a(new_n64_), .b(new_n51_), .c(new_n66_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n38_), .O(z06));
  nand2  g43(.a(new_n51_), .b(x11), .O(new_n69_));
  nand4  g44(.a(x13), .b(new_n62_), .c(x05), .d(new_n60_), .O(new_n70_));
  aoi21  g45(.a(new_n70_), .b(new_n69_), .c(new_n65_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n38_), .O(z09));
  nand2  g48(.a(x05), .b(x02), .O(new_n74_));
  nand3  g49(.a(new_n74_), .b(x06), .c(new_n34_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n38_), .O(z10));
endmodule


