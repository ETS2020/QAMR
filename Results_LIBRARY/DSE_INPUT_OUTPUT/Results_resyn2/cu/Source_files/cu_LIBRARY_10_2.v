// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_;
  inv1   g00(.a(x01), .O(new_n26_));
  nor2   g01(.a(x05), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  nor2   g03(.a(x04), .b(x02), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  oai21  g05(.a(x05), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  inv1   g06(.a(x04), .O(new_n32_));
  inv1   g07(.a(x05), .O(new_n33_));
  aoi21  g08(.a(new_n33_), .b(new_n32_), .c(x03), .O(new_n34_));
  aoi21  g09(.a(new_n34_), .b(new_n31_), .c(new_n27_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(new_n27_), .O(new_n37_));
  inv1   g12(.a(x03), .O(new_n38_));
  inv1   g13(.a(x13), .O(new_n39_));
  nand4  g14(.a(new_n29_), .b(new_n39_), .c(x05), .d(new_n38_), .O(new_n40_));
  inv1   g15(.a(x00), .O(new_n41_));
  nand2  g16(.a(new_n26_), .b(new_n41_), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n40_), .c(new_n37_), .O(z02));
  nor3   g18(.a(new_n40_), .b(x01), .c(new_n41_), .O(z03));
  nor3   g19(.a(x13), .b(x04), .c(x03), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n28_), .c(new_n41_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(x05), .c(new_n26_), .O(z04));
  nand3  g22(.a(new_n45_), .b(new_n28_), .c(x00), .O(new_n48_));
  aoi21  g23(.a(new_n48_), .b(x05), .c(new_n26_), .O(z05));
  nand3  g24(.a(new_n39_), .b(new_n33_), .c(x02), .O(new_n50_));
  inv1   g25(.a(x09), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(x05), .c(new_n26_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n52_), .c(new_n41_), .O(new_n55_));
  inv1   g30(.a(x08), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(x05), .c(new_n26_), .O(new_n57_));
  inv1   g32(.a(x10), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n57_), .c(x00), .O(new_n60_));
  inv1   g35(.a(x07), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(new_n61_), .d(new_n28_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n60_), .b(new_n55_), .c(new_n64_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  nor2   g41(.a(new_n32_), .b(x03), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n37_), .O(z06));
  nand2  g44(.a(new_n40_), .b(new_n37_), .O(z07));
  nand4  g45(.a(x13), .b(new_n62_), .c(x05), .d(new_n28_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n50_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand2  g48(.a(new_n73_), .b(new_n37_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n27_), .O(z09));
  nand2  g51(.a(x05), .b(x02), .O(new_n77_));
  nand3  g52(.a(new_n77_), .b(x06), .c(new_n38_), .O(new_n78_));
  nand2  g53(.a(new_n78_), .b(new_n37_), .O(z10));
endmodule


