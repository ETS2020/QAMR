// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x03), .O(new_n26_));
  nand2  g01(.a(x05), .b(x02), .O(new_n27_));
  inv1   g02(.a(new_n27_), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  nand2  g04(.a(x05), .b(new_n29_), .O(new_n30_));
  nand2  g05(.a(x04), .b(x02), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  oai21  g07(.a(new_n28_), .b(new_n26_), .c(new_n32_), .O(z00));
  nand2  g08(.a(new_n27_), .b(new_n26_), .O(new_n34_));
  aoi21  g09(.a(new_n31_), .b(new_n30_), .c(new_n34_), .O(z01));
  inv1   g10(.a(x01), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(x05), .c(new_n29_), .d(new_n36_), .O(new_n38_));
  nor4   g13(.a(new_n38_), .b(x03), .c(x02), .d(x00), .O(z02));
  inv1   g14(.a(x02), .O(new_n40_));
  nand3  g15(.a(new_n26_), .b(new_n40_), .c(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(new_n38_), .O(z03));
  inv1   g17(.a(x05), .O(new_n43_));
  nand3  g18(.a(new_n37_), .b(new_n29_), .c(new_n26_), .O(new_n44_));
  inv1   g19(.a(x00), .O(new_n45_));
  nand2  g20(.a(x01), .b(new_n45_), .O(new_n46_));
  or2    g21(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n40_), .c(new_n43_), .O(z04));
  nor4   g23(.a(new_n41_), .b(new_n30_), .c(x13), .d(new_n36_), .O(z05));
  inv1   g24(.a(x08), .O(new_n50_));
  aoi21  g25(.a(new_n50_), .b(new_n36_), .c(x00), .O(new_n51_));
  oai21  g26(.a(x10), .b(new_n36_), .c(new_n51_), .O(new_n52_));
  inv1   g27(.a(x11), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x09), .O(new_n55_));
  aoi21  g30(.a(new_n55_), .b(new_n36_), .c(new_n45_), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(new_n57_), .d(x05), .O(new_n59_));
  aoi21  g34(.a(new_n56_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  nor2   g35(.a(x13), .b(new_n40_), .O(new_n61_));
  aoi21  g36(.a(new_n60_), .b(new_n52_), .c(new_n61_), .O(new_n62_));
  nand2  g37(.a(x04), .b(new_n26_), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n62_), .c(new_n27_), .O(z06));
  aoi21  g39(.a(new_n44_), .b(new_n40_), .c(new_n43_), .O(z07));
  nand3  g40(.a(x13), .b(new_n58_), .c(x05), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  nor2   g42(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nor3   g43(.a(new_n68_), .b(new_n63_), .c(new_n28_), .O(z08));
  inv1   g44(.a(x06), .O(new_n70_));
  nor3   g45(.a(new_n28_), .b(new_n37_), .c(new_n70_), .O(z09));
  oai21  g46(.a(new_n70_), .b(x03), .c(new_n27_), .O(z10));
endmodule


