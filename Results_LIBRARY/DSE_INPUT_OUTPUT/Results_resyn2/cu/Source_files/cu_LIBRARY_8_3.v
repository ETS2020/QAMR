// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand3  g01(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  nor2   g02(.a(x04), .b(x02), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x05), .O(new_n29_));
  aoi21  g04(.a(new_n29_), .b(new_n27_), .c(x03), .O(new_n30_));
  nand2  g05(.a(new_n27_), .b(x11), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(z00));
  inv1   g07(.a(new_n30_), .O(new_n33_));
  nand2  g08(.a(x11), .b(x05), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n33_), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  nand2  g12(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g13(.a(x03), .O(new_n39_));
  inv1   g14(.a(x11), .O(new_n40_));
  inv1   g15(.a(x13), .O(new_n41_));
  nand3  g16(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  or2    g17(.a(new_n42_), .b(new_n29_), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n38_), .O(z02));
  nand2  g19(.a(new_n37_), .b(x00), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n43_), .O(z03));
  nand3  g21(.a(new_n28_), .b(new_n41_), .c(new_n39_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(x01), .c(new_n36_), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n40_), .c(new_n26_), .O(z04));
  nor4   g25(.a(new_n42_), .b(new_n29_), .c(new_n37_), .d(new_n36_), .O(z05));
  nand3  g26(.a(new_n41_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(x00), .c(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n37_), .c(new_n36_), .O(new_n56_));
  inv1   g31(.a(x09), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(x00), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  inv1   g35(.a(x12), .O(new_n61_));
  nand4  g36(.a(x13), .b(new_n61_), .c(x05), .d(new_n60_), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  nor2   g38(.a(x11), .b(x07), .O(new_n64_));
  nand3  g39(.a(new_n64_), .b(new_n63_), .c(new_n59_), .O(new_n65_));
  nand2  g40(.a(x04), .b(new_n39_), .O(new_n66_));
  aoi21  g41(.a(new_n65_), .b(new_n52_), .c(new_n66_), .O(z06));
  aoi21  g42(.a(new_n47_), .b(new_n40_), .c(new_n26_), .O(z07));
  and2   g43(.a(new_n62_), .b(new_n52_), .O(new_n69_));
  oai21  g44(.a(new_n69_), .b(new_n66_), .c(new_n34_), .O(z08));
  nand2  g45(.a(x13), .b(x06), .O(new_n71_));
  aoi21  g46(.a(x11), .b(x05), .c(new_n71_), .O(z09));
  nand2  g47(.a(new_n40_), .b(new_n60_), .O(new_n73_));
  nand2  g48(.a(x06), .b(new_n39_), .O(new_n74_));
  aoi21  g49(.a(new_n73_), .b(x05), .c(new_n74_), .O(z10));
endmodule


