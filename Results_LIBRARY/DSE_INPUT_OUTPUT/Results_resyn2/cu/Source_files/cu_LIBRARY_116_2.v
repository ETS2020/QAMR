// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_;
  nor2   g00(.a(x04), .b(x02), .O(new_n26_));
  nand2  g01(.a(new_n26_), .b(x05), .O(new_n27_));
  inv1   g02(.a(x01), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand4  g04(.a(new_n29_), .b(x04), .c(x02), .d(new_n28_), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g06(.a(z01), .O(z00));
  nor2   g07(.a(x13), .b(x03), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n26_), .c(x05), .O(new_n34_));
  or2    g09(.a(new_n34_), .b(x00), .O(new_n35_));
  nor2   g10(.a(new_n35_), .b(x01), .O(z02));
  nand2  g11(.a(new_n29_), .b(x01), .O(new_n37_));
  nand2  g12(.a(new_n28_), .b(x00), .O(new_n38_));
  oai21  g13(.a(new_n38_), .b(new_n34_), .c(new_n37_), .O(z03));
  nor2   g14(.a(new_n35_), .b(new_n28_), .O(z04));
  nand3  g15(.a(new_n33_), .b(new_n26_), .c(x00), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(x05), .c(new_n28_), .O(z05));
  inv1   g17(.a(x13), .O(new_n43_));
  nand3  g18(.a(new_n43_), .b(new_n29_), .c(x02), .O(new_n44_));
  inv1   g19(.a(x00), .O(new_n45_));
  inv1   g20(.a(x09), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(x05), .c(new_n28_), .O(new_n47_));
  inv1   g22(.a(x11), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(x01), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand3  g26(.a(new_n51_), .b(x05), .c(new_n28_), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n52_), .c(x00), .O(new_n55_));
  inv1   g30(.a(x02), .O(new_n56_));
  inv1   g31(.a(x07), .O(new_n57_));
  inv1   g32(.a(x12), .O(new_n58_));
  nand4  g33(.a(x13), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(new_n60_));
  oai21  g35(.a(new_n55_), .b(new_n50_), .c(new_n60_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n44_), .O(new_n62_));
  inv1   g37(.a(x04), .O(new_n63_));
  nor2   g38(.a(new_n63_), .b(x03), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n37_), .O(z06));
  nand2  g41(.a(new_n37_), .b(new_n34_), .O(z07));
  nand4  g42(.a(x13), .b(new_n58_), .c(x05), .d(new_n56_), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n44_), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n37_), .O(z08));
  nand2  g46(.a(x13), .b(x06), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n37_), .O(z09));
  inv1   g48(.a(x03), .O(new_n74_));
  nand2  g49(.a(x05), .b(x02), .O(new_n75_));
  nand4  g50(.a(new_n75_), .b(new_n37_), .c(x06), .d(new_n74_), .O(new_n76_));
  inv1   g51(.a(new_n76_), .O(z10));
endmodule


