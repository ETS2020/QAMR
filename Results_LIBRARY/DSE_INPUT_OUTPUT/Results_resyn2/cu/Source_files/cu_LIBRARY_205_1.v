// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_,
    new_n71_;
  inv1   g00(.a(x08), .O(new_n26_));
  nand2  g01(.a(x10), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  nand3  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  nand2  g07(.a(new_n31_), .b(new_n30_), .O(new_n33_));
  nand2  g08(.a(new_n33_), .b(new_n27_), .O(z01));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x04), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nor2   g12(.a(x03), .b(x02), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(new_n37_), .c(x05), .d(new_n36_), .O(new_n39_));
  aoi21  g14(.a(x10), .b(new_n26_), .c(new_n39_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x01), .O(z02));
  inv1   g17(.a(x01), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(x00), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n39_), .c(new_n27_), .O(z03));
  nor2   g20(.a(new_n41_), .b(new_n43_), .O(z04));
  nand3  g21(.a(new_n40_), .b(x01), .c(x00), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z05));
  inv1   g23(.a(x10), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(x01), .c(new_n26_), .O(new_n50_));
  and2   g25(.a(x11), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  oai21  g27(.a(new_n52_), .b(x01), .c(x00), .O(new_n53_));
  oai22  g28(.a(new_n53_), .b(new_n51_), .c(new_n50_), .d(x00), .O(new_n54_));
  inv1   g29(.a(x02), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(x05), .d(new_n55_), .O(new_n57_));
  nor2   g32(.a(new_n57_), .b(x07), .O(new_n58_));
  nand3  g33(.a(new_n37_), .b(new_n28_), .c(x02), .O(new_n59_));
  inv1   g34(.a(new_n59_), .O(new_n60_));
  aoi21  g35(.a(new_n58_), .b(new_n54_), .c(new_n60_), .O(new_n61_));
  nor2   g36(.a(new_n36_), .b(x03), .O(new_n62_));
  inv1   g37(.a(new_n62_), .O(new_n63_));
  oai21  g38(.a(new_n63_), .b(new_n61_), .c(new_n27_), .O(z06));
  nand2  g39(.a(new_n39_), .b(new_n27_), .O(z07));
  nand2  g40(.a(new_n59_), .b(new_n57_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n27_), .O(z08));
  nand2  g43(.a(new_n27_), .b(x06), .O(new_n69_));
  nor2   g44(.a(new_n69_), .b(new_n37_), .O(z09));
  inv1   g45(.a(new_n31_), .O(new_n71_));
  nor2   g46(.a(new_n69_), .b(new_n71_), .O(z10));
endmodule


