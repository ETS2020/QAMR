// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_,
    new_n70_;
  inv1   g00(.a(x04), .O(new_n26_));
  nand2  g01(.a(x05), .b(new_n26_), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  inv1   g03(.a(x03), .O(new_n29_));
  nand2  g04(.a(x05), .b(x02), .O(new_n30_));
  nand2  g05(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  aoi21  g06(.a(new_n28_), .b(new_n27_), .c(new_n31_), .O(z01));
  inv1   g07(.a(z01), .O(z00));
  inv1   g08(.a(x01), .O(new_n34_));
  inv1   g09(.a(x13), .O(new_n35_));
  nor2   g10(.a(x03), .b(x02), .O(new_n36_));
  nand4  g11(.a(new_n36_), .b(new_n35_), .c(x05), .d(new_n26_), .O(new_n37_));
  nor2   g12(.a(new_n37_), .b(x00), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z02));
  inv1   g15(.a(x00), .O(new_n41_));
  nor2   g16(.a(new_n37_), .b(new_n41_), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  inv1   g18(.a(new_n43_), .O(z03));
  nand2  g19(.a(new_n38_), .b(x01), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(z04));
  nand2  g21(.a(new_n42_), .b(x01), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z05));
  inv1   g23(.a(x08), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n34_), .c(x00), .O(new_n50_));
  oai21  g25(.a(x10), .b(new_n34_), .c(new_n50_), .O(new_n51_));
  inv1   g26(.a(x11), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(x01), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  aoi21  g29(.a(new_n54_), .b(new_n34_), .c(new_n41_), .O(new_n55_));
  inv1   g30(.a(x07), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand4  g32(.a(x13), .b(new_n57_), .c(new_n56_), .d(x05), .O(new_n58_));
  aoi21  g33(.a(new_n55_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  inv1   g34(.a(x02), .O(new_n60_));
  nor2   g35(.a(x13), .b(new_n60_), .O(new_n61_));
  aoi21  g36(.a(new_n59_), .b(new_n51_), .c(new_n61_), .O(new_n62_));
  nor2   g37(.a(new_n26_), .b(x03), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  oai21  g39(.a(new_n64_), .b(new_n62_), .c(new_n30_), .O(z06));
  inv1   g40(.a(new_n37_), .O(z07));
  nand3  g41(.a(x13), .b(new_n57_), .c(x05), .O(new_n67_));
  aoi21  g42(.a(new_n63_), .b(new_n35_), .c(x05), .O(new_n68_));
  oai22  g43(.a(new_n68_), .b(new_n60_), .c(new_n64_), .d(new_n67_), .O(z08));
  inv1   g44(.a(x06), .O(new_n70_));
  oai21  g45(.a(new_n35_), .b(new_n70_), .c(new_n30_), .O(z09));
  oai21  g46(.a(new_n70_), .b(x03), .c(new_n30_), .O(z10));
endmodule


