// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x11), .O(new_n26_));
  inv1   g01(.a(x12), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  aoi21  g03(.a(x05), .b(x02), .c(x03), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  inv1   g05(.a(x05), .O(new_n31_));
  nand2  g06(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g07(.a(x02), .O(new_n33_));
  nand2  g08(.a(x04), .b(new_n33_), .O(new_n34_));
  nand3  g09(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(new_n36_));
  nor2   g11(.a(new_n36_), .b(new_n28_), .O(z00));
  inv1   g12(.a(new_n28_), .O(new_n38_));
  nand2  g13(.a(new_n36_), .b(new_n38_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(z01));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x03), .O(new_n42_));
  nand4  g17(.a(x05), .b(new_n30_), .c(new_n42_), .d(new_n33_), .O(new_n43_));
  nor3   g18(.a(new_n43_), .b(new_n28_), .c(x13), .O(z07));
  nand2  g19(.a(z07), .b(new_n41_), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(x01), .O(z02));
  inv1   g21(.a(x01), .O(new_n47_));
  inv1   g22(.a(x13), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n47_), .c(x00), .O(new_n49_));
  oai21  g24(.a(new_n49_), .b(new_n43_), .c(new_n38_), .O(z03));
  nor2   g25(.a(new_n45_), .b(new_n47_), .O(z04));
  nor2   g26(.a(new_n47_), .b(new_n41_), .O(new_n52_));
  and2   g27(.a(new_n52_), .b(z07), .O(z05));
  inv1   g28(.a(x07), .O(new_n54_));
  inv1   g29(.a(x10), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(x01), .O(new_n56_));
  inv1   g31(.a(x08), .O(new_n57_));
  nand2  g32(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  nand3  g33(.a(new_n58_), .b(new_n56_), .c(new_n41_), .O(new_n59_));
  nand2  g34(.a(new_n26_), .b(x01), .O(new_n60_));
  inv1   g35(.a(x09), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n60_), .c(x00), .O(new_n63_));
  nand4  g38(.a(x13), .b(new_n27_), .c(x05), .d(new_n33_), .O(new_n64_));
  inv1   g39(.a(new_n64_), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(new_n63_), .c(new_n59_), .d(new_n54_), .O(new_n66_));
  nand3  g41(.a(new_n48_), .b(new_n31_), .c(x02), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nor2   g43(.a(new_n30_), .b(x03), .O(new_n69_));
  nand2  g44(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n38_), .O(z06));
  inv1   g46(.a(new_n69_), .O(new_n72_));
  nand4  g47(.a(new_n38_), .b(new_n48_), .c(new_n31_), .d(x02), .O(new_n73_));
  aoi21  g48(.a(new_n73_), .b(new_n64_), .c(new_n72_), .O(z08));
  inv1   g49(.a(x06), .O(new_n75_));
  oai21  g50(.a(new_n48_), .b(new_n75_), .c(new_n38_), .O(z09));
  nand3  g51(.a(new_n29_), .b(new_n38_), .c(x06), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z10));
endmodule


