// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n40_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n71_, new_n73_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x05), .b(x04), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(x02), .O(new_n29_));
  oai21  g04(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(new_n30_), .b(new_n26_), .c(new_n27_), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x00), .O(new_n33_));
  inv1   g08(.a(x01), .O(new_n34_));
  nor3   g09(.a(x13), .b(x03), .c(x02), .O(new_n35_));
  nand3  g10(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  aoi21  g11(.a(new_n36_), .b(x05), .c(x04), .O(z02));
  nand3  g12(.a(new_n35_), .b(new_n34_), .c(x00), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(x05), .c(x04), .O(z03));
  nand3  g14(.a(new_n35_), .b(x01), .c(new_n33_), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(x05), .c(x04), .O(z04));
  nand3  g16(.a(new_n35_), .b(x01), .c(x00), .O(new_n42_));
  aoi21  g17(.a(new_n42_), .b(x05), .c(x04), .O(z05));
  inv1   g18(.a(new_n27_), .O(new_n44_));
  inv1   g19(.a(x10), .O(new_n45_));
  nand2  g20(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g21(.a(x08), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n46_), .c(new_n33_), .O(new_n49_));
  inv1   g24(.a(x11), .O(new_n50_));
  nand2  g25(.a(new_n50_), .b(x01), .O(new_n51_));
  inv1   g26(.a(x09), .O(new_n52_));
  nand2  g27(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand3  g28(.a(new_n53_), .b(new_n51_), .c(x00), .O(new_n54_));
  inv1   g29(.a(x02), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(x05), .d(new_n55_), .O(new_n57_));
  inv1   g32(.a(new_n57_), .O(new_n58_));
  inv1   g33(.a(x04), .O(new_n59_));
  nor2   g34(.a(x07), .b(new_n59_), .O(new_n60_));
  nand4  g35(.a(new_n60_), .b(new_n58_), .c(new_n54_), .d(new_n49_), .O(new_n61_));
  inv1   g36(.a(x13), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n28_), .c(x02), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n26_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n44_), .O(z06));
  nor2   g41(.a(new_n35_), .b(new_n28_), .O(new_n67_));
  nor2   g42(.a(new_n67_), .b(x04), .O(z07));
  nand2  g43(.a(x04), .b(new_n26_), .O(new_n69_));
  aoi21  g44(.a(new_n63_), .b(new_n57_), .c(new_n69_), .O(z08));
  nand2  g45(.a(new_n44_), .b(x06), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n62_), .O(z09));
  oai21  g47(.a(new_n28_), .b(new_n55_), .c(new_n26_), .O(new_n73_));
  nor2   g48(.a(new_n73_), .b(new_n71_), .O(z10));
endmodule


