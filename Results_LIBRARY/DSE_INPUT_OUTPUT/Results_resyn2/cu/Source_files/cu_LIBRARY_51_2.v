// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_;
  inv1   g00(.a(x11), .O(new_n26_));
  nor2   g01(.a(x13), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  nor2   g08(.a(x13), .b(x11), .O(new_n34_));
  inv1   g09(.a(x02), .O(new_n35_));
  inv1   g10(.a(x03), .O(new_n36_));
  inv1   g11(.a(x04), .O(new_n37_));
  nand4  g12(.a(x05), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n38_));
  inv1   g13(.a(new_n38_), .O(new_n39_));
  nand2  g14(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  inv1   g15(.a(x00), .O(new_n41_));
  inv1   g16(.a(x01), .O(new_n42_));
  nand2  g17(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n40_), .O(z02));
  nand2  g19(.a(new_n42_), .b(x00), .O(new_n45_));
  nor2   g20(.a(new_n45_), .b(new_n40_), .O(z03));
  nand3  g21(.a(new_n39_), .b(x01), .c(new_n41_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(new_n26_), .c(x13), .O(z04));
  nand3  g23(.a(new_n39_), .b(x01), .c(x00), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n26_), .c(x13), .O(z05));
  nand2  g25(.a(x04), .b(new_n36_), .O(new_n51_));
  inv1   g26(.a(x07), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n42_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(new_n41_), .O(new_n57_));
  nand2  g32(.a(new_n26_), .b(x01), .O(new_n58_));
  inv1   g33(.a(x09), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n42_), .O(new_n60_));
  nand3  g35(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n61_));
  inv1   g36(.a(x12), .O(new_n62_));
  nand4  g37(.a(x13), .b(new_n62_), .c(x05), .d(new_n35_), .O(new_n63_));
  inv1   g38(.a(new_n63_), .O(new_n64_));
  nand4  g39(.a(new_n64_), .b(new_n61_), .c(new_n57_), .d(new_n52_), .O(new_n65_));
  nand3  g40(.a(new_n34_), .b(new_n28_), .c(x02), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(new_n65_), .c(new_n51_), .O(z06));
  inv1   g42(.a(new_n40_), .O(z07));
  inv1   g43(.a(new_n27_), .O(new_n69_));
  inv1   g44(.a(x13), .O(new_n70_));
  nand3  g45(.a(new_n70_), .b(new_n28_), .c(x02), .O(new_n71_));
  and2   g46(.a(new_n71_), .b(new_n63_), .O(new_n72_));
  oai21  g47(.a(new_n72_), .b(new_n51_), .c(new_n69_), .O(z08));
  inv1   g48(.a(x06), .O(new_n74_));
  aoi21  g49(.a(x13), .b(new_n74_), .c(new_n34_), .O(z09));
  nor2   g50(.a(new_n27_), .b(new_n74_), .O(new_n76_));
  and2   g51(.a(new_n76_), .b(new_n31_), .O(z10));
endmodule


