// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n75_,
    new_n77_, new_n79_;
  inv1   g00(.a(x05), .O(new_n26_));
  inv1   g01(.a(x06), .O(new_n27_));
  nor2   g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g03(.a(new_n26_), .b(x02), .O(new_n29_));
  oai21  g04(.a(x04), .b(x02), .c(new_n29_), .O(new_n30_));
  inv1   g05(.a(x04), .O(new_n31_));
  aoi21  g06(.a(new_n26_), .b(new_n31_), .c(x03), .O(new_n32_));
  aoi21  g07(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(z00));
  inv1   g08(.a(z00), .O(z01));
  inv1   g09(.a(x00), .O(new_n35_));
  inv1   g10(.a(x03), .O(new_n36_));
  nor2   g11(.a(new_n26_), .b(x02), .O(new_n37_));
  nor2   g12(.a(x13), .b(x06), .O(new_n38_));
  nand2  g13(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n31_), .c(new_n36_), .d(new_n35_), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x01), .O(z02));
  inv1   g17(.a(x01), .O(new_n43_));
  nor3   g18(.a(x13), .b(x04), .c(x03), .O(new_n44_));
  nor2   g19(.a(x02), .b(new_n35_), .O(new_n45_));
  nand3  g20(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g21(.a(new_n46_), .b(new_n27_), .c(new_n26_), .O(z03));
  nor2   g22(.a(new_n41_), .b(new_n43_), .O(z04));
  nand3  g23(.a(new_n45_), .b(new_n44_), .c(x01), .O(new_n49_));
  aoi21  g24(.a(new_n49_), .b(new_n27_), .c(new_n26_), .O(z05));
  inv1   g25(.a(new_n28_), .O(new_n51_));
  inv1   g26(.a(x07), .O(new_n52_));
  inv1   g27(.a(x10), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(x01), .O(new_n54_));
  inv1   g29(.a(x08), .O(new_n55_));
  nand2  g30(.a(new_n55_), .b(new_n43_), .O(new_n56_));
  nand3  g31(.a(new_n56_), .b(new_n54_), .c(new_n35_), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g34(.a(x09), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n43_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n59_), .c(x00), .O(new_n62_));
  inv1   g37(.a(x02), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x05), .d(new_n63_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n62_), .c(new_n57_), .d(new_n52_), .O(new_n67_));
  or2    g42(.a(new_n29_), .b(x13), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g44(.a(new_n31_), .b(x03), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n51_), .O(z06));
  nor3   g47(.a(new_n39_), .b(x04), .c(x03), .O(z07));
  nand2  g48(.a(new_n68_), .b(new_n65_), .O(new_n74_));
  nand2  g49(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand2  g50(.a(new_n75_), .b(new_n51_), .O(z08));
  nand3  g51(.a(x13), .b(x06), .c(new_n26_), .O(new_n77_));
  inv1   g52(.a(new_n77_), .O(z09));
  nand3  g53(.a(x06), .b(new_n26_), .c(new_n36_), .O(new_n79_));
  inv1   g54(.a(new_n79_), .O(z10));
endmodule


