// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n56_, new_n57_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x05), .O(new_n26_));
  nor2   g01(.a(x07), .b(new_n26_), .O(new_n27_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n28_));
  oai21  g03(.a(x04), .b(x02), .c(new_n28_), .O(new_n29_));
  inv1   g04(.a(x04), .O(new_n30_));
  aoi21  g05(.a(new_n26_), .b(new_n30_), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(z00));
  nand3  g07(.a(new_n26_), .b(x04), .c(x02), .O(new_n33_));
  inv1   g08(.a(x07), .O(new_n34_));
  nor2   g09(.a(new_n34_), .b(x02), .O(new_n35_));
  nor2   g10(.a(new_n26_), .b(x04), .O(new_n36_));
  nand2  g11(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  aoi21  g12(.a(new_n37_), .b(new_n33_), .c(x03), .O(z01));
  nor2   g13(.a(x13), .b(x03), .O(new_n39_));
  nor2   g14(.a(x01), .b(x00), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(new_n39_), .c(new_n36_), .d(new_n35_), .O(new_n41_));
  inv1   g16(.a(new_n41_), .O(z02));
  inv1   g17(.a(x02), .O(new_n43_));
  nor3   g18(.a(x13), .b(x04), .c(x03), .O(new_n44_));
  inv1   g19(.a(x00), .O(new_n45_));
  nor2   g20(.a(x01), .b(new_n45_), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x07), .c(new_n26_), .O(z03));
  inv1   g23(.a(x01), .O(new_n49_));
  nor2   g24(.a(new_n49_), .b(x00), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n44_), .c(new_n43_), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(x07), .c(new_n26_), .O(z04));
  nor2   g27(.a(new_n49_), .b(new_n45_), .O(new_n53_));
  nand4  g28(.a(new_n53_), .b(new_n39_), .c(new_n36_), .d(new_n35_), .O(new_n54_));
  inv1   g29(.a(new_n54_), .O(z05));
  inv1   g30(.a(new_n27_), .O(new_n56_));
  inv1   g31(.a(new_n39_), .O(new_n57_));
  oai21  g32(.a(new_n57_), .b(new_n33_), .c(new_n56_), .O(z06));
  nor2   g33(.a(new_n57_), .b(new_n37_), .O(z07));
  nor2   g34(.a(x12), .b(new_n26_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n35_), .O(new_n61_));
  inv1   g36(.a(x13), .O(new_n62_));
  oai21  g37(.a(x05), .b(new_n43_), .c(new_n62_), .O(new_n63_));
  nor2   g38(.a(new_n30_), .b(x03), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g40(.a(new_n61_), .b(x13), .c(new_n65_), .O(z08));
  nand2  g41(.a(x13), .b(x06), .O(new_n67_));
  nand2  g42(.a(new_n67_), .b(new_n56_), .O(z09));
  nor2   g43(.a(new_n35_), .b(new_n26_), .O(new_n69_));
  inv1   g44(.a(x03), .O(new_n70_));
  nand2  g45(.a(x06), .b(new_n70_), .O(new_n71_));
  nor2   g46(.a(new_n71_), .b(new_n69_), .O(z10));
endmodule


