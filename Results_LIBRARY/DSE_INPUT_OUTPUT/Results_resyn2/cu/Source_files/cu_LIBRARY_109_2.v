// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n71_;
  inv1   g00(.a(x13), .O(new_n26_));
  inv1   g01(.a(x05), .O(new_n27_));
  nand2  g02(.a(x04), .b(x02), .O(new_n28_));
  oai21  g03(.a(new_n27_), .b(x04), .c(new_n28_), .O(new_n29_));
  aoi21  g04(.a(x05), .b(x02), .c(x03), .O(new_n30_));
  aoi22  g05(.a(new_n30_), .b(new_n29_), .c(new_n26_), .d(x06), .O(z00));
  inv1   g06(.a(z00), .O(z01));
  inv1   g07(.a(x06), .O(new_n33_));
  inv1   g08(.a(x00), .O(new_n34_));
  inv1   g09(.a(x03), .O(new_n35_));
  nor2   g10(.a(new_n27_), .b(x04), .O(new_n36_));
  nor2   g11(.a(x02), .b(x01), .O(new_n37_));
  nand4  g12(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  aoi21  g13(.a(new_n38_), .b(new_n33_), .c(x13), .O(z02));
  nand4  g14(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(x00), .O(new_n40_));
  aoi21  g15(.a(new_n40_), .b(new_n33_), .c(x13), .O(z03));
  nand2  g16(.a(new_n26_), .b(new_n33_), .O(new_n42_));
  nor2   g17(.a(new_n42_), .b(x02), .O(new_n43_));
  nand4  g18(.a(new_n43_), .b(new_n36_), .c(new_n35_), .d(x01), .O(new_n44_));
  nor2   g19(.a(new_n44_), .b(x00), .O(z04));
  nor2   g20(.a(new_n44_), .b(new_n34_), .O(z05));
  inv1   g21(.a(x07), .O(new_n47_));
  inv1   g22(.a(x10), .O(new_n48_));
  nand2  g23(.a(new_n48_), .b(x01), .O(new_n49_));
  inv1   g24(.a(x01), .O(new_n50_));
  inv1   g25(.a(x08), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n49_), .c(new_n34_), .O(new_n53_));
  inv1   g28(.a(x11), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x01), .O(new_n55_));
  inv1   g30(.a(x09), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nand3  g32(.a(new_n57_), .b(new_n55_), .c(x00), .O(new_n58_));
  inv1   g33(.a(x02), .O(new_n59_));
  inv1   g34(.a(x12), .O(new_n60_));
  nand4  g35(.a(x13), .b(new_n60_), .c(x05), .d(new_n59_), .O(new_n61_));
  inv1   g36(.a(new_n61_), .O(new_n62_));
  nand4  g37(.a(new_n62_), .b(new_n58_), .c(new_n53_), .d(new_n47_), .O(new_n63_));
  nand4  g38(.a(new_n26_), .b(new_n33_), .c(new_n27_), .d(x02), .O(new_n64_));
  nand2  g39(.a(x04), .b(new_n35_), .O(new_n65_));
  aoi21  g40(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(z06));
  nand2  g41(.a(new_n36_), .b(new_n35_), .O(new_n67_));
  nor3   g42(.a(new_n42_), .b(new_n67_), .c(x02), .O(z07));
  aoi21  g43(.a(new_n64_), .b(new_n61_), .c(new_n65_), .O(z08));
  nor2   g44(.a(new_n26_), .b(new_n33_), .O(z09));
  nand2  g45(.a(z09), .b(new_n30_), .O(new_n71_));
  inv1   g46(.a(new_n71_), .O(z10));
endmodule


