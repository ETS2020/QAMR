// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n74_, new_n76_,
    new_n78_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x05), .b(new_n27_), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  nand2  g04(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g05(.a(x05), .b(x02), .O(new_n31_));
  inv1   g06(.a(x07), .O(new_n32_));
  nor2   g07(.a(x11), .b(new_n32_), .O(new_n33_));
  inv1   g08(.a(new_n33_), .O(new_n34_));
  nand4  g09(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n26_), .O(z00));
  inv1   g10(.a(z00), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  inv1   g12(.a(x13), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(x05), .c(new_n27_), .d(new_n26_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  inv1   g16(.a(x01), .O(new_n42_));
  inv1   g17(.a(x02), .O(new_n43_));
  nand2  g18(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g19(.a(new_n44_), .b(new_n41_), .c(new_n34_), .O(z02));
  nor2   g20(.a(x01), .b(new_n37_), .O(new_n46_));
  nand4  g21(.a(new_n46_), .b(new_n40_), .c(new_n34_), .d(new_n43_), .O(new_n47_));
  inv1   g22(.a(new_n47_), .O(z03));
  nand2  g23(.a(new_n43_), .b(x01), .O(new_n49_));
  oai21  g24(.a(new_n49_), .b(new_n41_), .c(new_n34_), .O(z04));
  nand3  g25(.a(new_n43_), .b(x01), .c(x00), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n39_), .c(new_n34_), .O(z05));
  nand2  g27(.a(x04), .b(new_n26_), .O(new_n53_));
  inv1   g28(.a(x05), .O(new_n54_));
  nand3  g29(.a(new_n38_), .b(new_n54_), .c(x02), .O(new_n55_));
  inv1   g30(.a(new_n55_), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(new_n34_), .O(new_n57_));
  inv1   g32(.a(x11), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g34(.a(x09), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n42_), .O(new_n61_));
  nand3  g36(.a(new_n61_), .b(new_n59_), .c(x00), .O(new_n62_));
  inv1   g37(.a(x10), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g39(.a(x08), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n42_), .O(new_n66_));
  nand3  g41(.a(new_n66_), .b(new_n64_), .c(new_n37_), .O(new_n67_));
  inv1   g42(.a(x12), .O(new_n68_));
  nand4  g43(.a(x13), .b(new_n68_), .c(x05), .d(new_n43_), .O(new_n69_));
  inv1   g44(.a(new_n69_), .O(new_n70_));
  nand4  g45(.a(new_n70_), .b(new_n67_), .c(new_n62_), .d(new_n32_), .O(new_n71_));
  aoi21  g46(.a(new_n71_), .b(new_n57_), .c(new_n53_), .O(z06));
  nor3   g47(.a(new_n39_), .b(new_n33_), .c(x02), .O(z07));
  nor2   g48(.a(new_n70_), .b(new_n56_), .O(new_n74_));
  oai21  g49(.a(new_n74_), .b(new_n53_), .c(new_n34_), .O(z08));
  nand2  g50(.a(x13), .b(x06), .O(new_n76_));
  nor2   g51(.a(new_n76_), .b(new_n33_), .O(z09));
  nand4  g52(.a(new_n34_), .b(new_n31_), .c(x06), .d(new_n26_), .O(new_n78_));
  inv1   g53(.a(new_n78_), .O(z10));
endmodule


