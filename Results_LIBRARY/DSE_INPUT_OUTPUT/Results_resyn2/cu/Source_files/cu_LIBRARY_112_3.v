// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x05), .O(new_n26_));
  nand3  g01(.a(new_n26_), .b(x04), .c(x02), .O(new_n27_));
  inv1   g02(.a(x02), .O(new_n28_));
  inv1   g03(.a(x04), .O(new_n29_));
  inv1   g04(.a(x11), .O(new_n30_));
  nand4  g05(.a(new_n30_), .b(x05), .c(new_n29_), .d(new_n28_), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n27_), .c(x03), .O(z01));
  inv1   g07(.a(z01), .O(z00));
  nor2   g08(.a(x01), .b(x00), .O(new_n34_));
  nor3   g09(.a(new_n31_), .b(x13), .c(x03), .O(z07));
  and2   g10(.a(z07), .b(new_n34_), .O(z02));
  inv1   g11(.a(x01), .O(new_n37_));
  nor3   g12(.a(x13), .b(x04), .c(x03), .O(new_n38_));
  inv1   g13(.a(x00), .O(new_n39_));
  nor2   g14(.a(x02), .b(new_n39_), .O(new_n40_));
  nand3  g15(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(new_n41_));
  aoi21  g16(.a(new_n41_), .b(new_n30_), .c(new_n26_), .O(z03));
  nand4  g17(.a(new_n38_), .b(new_n28_), .c(x01), .d(new_n39_), .O(new_n43_));
  aoi21  g18(.a(new_n43_), .b(new_n30_), .c(new_n26_), .O(z04));
  nand3  g19(.a(new_n40_), .b(new_n38_), .c(x01), .O(new_n45_));
  aoi21  g20(.a(new_n45_), .b(new_n30_), .c(new_n26_), .O(z05));
  inv1   g21(.a(x13), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(new_n26_), .c(x02), .O(new_n48_));
  inv1   g23(.a(new_n48_), .O(new_n49_));
  inv1   g24(.a(x10), .O(new_n50_));
  oai21  g25(.a(new_n50_), .b(x00), .c(x01), .O(new_n51_));
  inv1   g26(.a(x08), .O(new_n52_));
  nand2  g27(.a(new_n34_), .b(new_n52_), .O(new_n53_));
  inv1   g28(.a(x09), .O(new_n54_));
  nand2  g29(.a(new_n54_), .b(x00), .O(new_n55_));
  nand3  g30(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(new_n56_));
  inv1   g31(.a(x12), .O(new_n57_));
  nand4  g32(.a(x13), .b(new_n57_), .c(x05), .d(new_n28_), .O(new_n58_));
  nor2   g33(.a(new_n58_), .b(x07), .O(new_n59_));
  aoi21  g34(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(new_n60_));
  inv1   g35(.a(x03), .O(new_n61_));
  nand2  g36(.a(x04), .b(new_n61_), .O(new_n62_));
  nand2  g37(.a(x11), .b(x05), .O(new_n63_));
  oai21  g38(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(z06));
  inv1   g39(.a(new_n58_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(new_n48_), .c(new_n62_), .O(z08));
  nand2  g42(.a(x13), .b(x06), .O(new_n68_));
  aoi21  g43(.a(x11), .b(x05), .c(new_n68_), .O(z09));
  nand2  g44(.a(new_n30_), .b(new_n28_), .O(new_n70_));
  nand2  g45(.a(x06), .b(new_n61_), .O(new_n71_));
  aoi21  g46(.a(new_n70_), .b(x05), .c(new_n71_), .O(z10));
endmodule


