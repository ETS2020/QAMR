// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n65_, new_n67_;
  inv1   g00(.a(x03), .O(new_n26_));
  nor2   g01(.a(x08), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nor2   g03(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g04(.a(new_n29_), .O(new_n30_));
  nand2  g05(.a(x04), .b(x02), .O(new_n31_));
  aoi21  g06(.a(x05), .b(x02), .c(x03), .O(new_n32_));
  inv1   g07(.a(new_n32_), .O(new_n33_));
  aoi21  g08(.a(new_n31_), .b(new_n30_), .c(new_n33_), .O(z01));
  nor2   g09(.a(z01), .b(new_n27_), .O(z00));
  inv1   g10(.a(x02), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand4  g12(.a(new_n29_), .b(new_n37_), .c(new_n26_), .d(new_n36_), .O(new_n38_));
  nor3   g13(.a(new_n38_), .b(x01), .c(x00), .O(z02));
  inv1   g14(.a(new_n27_), .O(new_n40_));
  inv1   g15(.a(x01), .O(new_n41_));
  nand2  g16(.a(new_n41_), .b(x00), .O(new_n42_));
  oai21  g17(.a(new_n42_), .b(new_n38_), .c(new_n40_), .O(z03));
  nor3   g18(.a(new_n38_), .b(new_n41_), .c(x00), .O(z04));
  inv1   g19(.a(x00), .O(new_n45_));
  nor3   g20(.a(new_n38_), .b(new_n41_), .c(new_n45_), .O(z05));
  inv1   g21(.a(x10), .O(new_n47_));
  nand3  g22(.a(new_n47_), .b(x01), .c(new_n45_), .O(new_n48_));
  nand2  g23(.a(x11), .b(x01), .O(new_n49_));
  nand2  g24(.a(x09), .b(new_n41_), .O(new_n50_));
  nand3  g25(.a(new_n50_), .b(new_n49_), .c(x00), .O(new_n51_));
  aoi21  g26(.a(new_n51_), .b(new_n48_), .c(x03), .O(new_n52_));
  nor3   g27(.a(x08), .b(x01), .c(x00), .O(new_n53_));
  inv1   g28(.a(x12), .O(new_n54_));
  nand4  g29(.a(x13), .b(new_n54_), .c(x05), .d(new_n36_), .O(new_n55_));
  nor2   g30(.a(new_n55_), .b(x07), .O(new_n56_));
  oai21  g31(.a(new_n53_), .b(new_n52_), .c(new_n56_), .O(new_n57_));
  nand3  g32(.a(new_n37_), .b(new_n28_), .c(x02), .O(new_n58_));
  inv1   g33(.a(new_n58_), .O(new_n59_));
  nand2  g34(.a(new_n59_), .b(new_n26_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x04), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n40_), .O(z06));
  nand2  g38(.a(new_n38_), .b(new_n40_), .O(z07));
  nand2  g39(.a(x04), .b(new_n26_), .O(new_n65_));
  aoi21  g40(.a(new_n58_), .b(new_n55_), .c(new_n65_), .O(z08));
  inv1   g41(.a(x06), .O(new_n67_));
  oai21  g42(.a(new_n37_), .b(new_n67_), .c(new_n40_), .O(z09));
  oai21  g43(.a(new_n33_), .b(new_n67_), .c(new_n40_), .O(z10));
endmodule


