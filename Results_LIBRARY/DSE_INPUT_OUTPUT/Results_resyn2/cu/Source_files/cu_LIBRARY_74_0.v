// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_;
  nor2   g00(.a(x05), .b(x01), .O(new_n26_));
  inv1   g01(.a(x02), .O(new_n27_));
  inv1   g02(.a(x04), .O(new_n28_));
  nand3  g03(.a(x05), .b(new_n28_), .c(new_n27_), .O(new_n29_));
  inv1   g04(.a(x05), .O(new_n30_));
  nand3  g05(.a(new_n30_), .b(x04), .c(x02), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n29_), .c(x03), .O(new_n32_));
  nor2   g07(.a(new_n32_), .b(new_n26_), .O(z00));
  inv1   g08(.a(new_n29_), .O(new_n34_));
  oai21  g09(.a(new_n34_), .b(x01), .c(new_n32_), .O(new_n35_));
  inv1   g10(.a(new_n35_), .O(z01));
  inv1   g11(.a(x00), .O(new_n37_));
  nor2   g12(.a(x13), .b(x03), .O(new_n38_));
  nand4  g13(.a(new_n38_), .b(new_n28_), .c(new_n27_), .d(new_n37_), .O(new_n39_));
  aoi21  g14(.a(new_n39_), .b(x05), .c(x01), .O(z02));
  nand3  g15(.a(new_n38_), .b(new_n34_), .c(x00), .O(new_n41_));
  nor2   g16(.a(new_n41_), .b(x01), .O(z03));
  nand2  g17(.a(x05), .b(x01), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n39_), .O(z04));
  inv1   g19(.a(x01), .O(new_n45_));
  nor2   g20(.a(new_n41_), .b(new_n45_), .O(z05));
  inv1   g21(.a(new_n26_), .O(new_n47_));
  inv1   g22(.a(x13), .O(new_n48_));
  nand3  g23(.a(new_n48_), .b(new_n30_), .c(x02), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  oai22  g25(.a(new_n43_), .b(x11), .c(x09), .d(x01), .O(new_n51_));
  nand2  g26(.a(new_n51_), .b(x00), .O(new_n52_));
  oai22  g27(.a(new_n43_), .b(x10), .c(x08), .d(x01), .O(new_n53_));
  nand2  g28(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x12), .O(new_n56_));
  nand4  g31(.a(x13), .b(new_n56_), .c(new_n55_), .d(new_n27_), .O(new_n57_));
  aoi21  g32(.a(new_n54_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  nor2   g33(.a(new_n28_), .b(x03), .O(new_n59_));
  oai21  g34(.a(new_n58_), .b(new_n50_), .c(new_n59_), .O(new_n60_));
  nand2  g35(.a(new_n60_), .b(new_n47_), .O(z06));
  nand2  g36(.a(new_n38_), .b(new_n34_), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n47_), .O(z07));
  nand4  g38(.a(x13), .b(new_n56_), .c(x05), .d(new_n27_), .O(new_n64_));
  nand2  g39(.a(new_n64_), .b(new_n49_), .O(new_n65_));
  nand2  g40(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand2  g41(.a(new_n66_), .b(new_n47_), .O(z08));
  nand2  g42(.a(x13), .b(x06), .O(new_n68_));
  nor2   g43(.a(new_n68_), .b(new_n26_), .O(z09));
  inv1   g44(.a(x03), .O(new_n70_));
  nand2  g45(.a(x05), .b(x02), .O(new_n71_));
  nand3  g46(.a(new_n71_), .b(x06), .c(new_n70_), .O(new_n72_));
  nand2  g47(.a(new_n72_), .b(new_n47_), .O(z10));
endmodule


