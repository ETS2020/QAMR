// Benchmark "FAU" written by ABC on Sat Aug  8 20:42:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x09), .O(new_n26_));
  inv1   g01(.a(x11), .O(new_n27_));
  nand2  g02(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  inv1   g03(.a(x05), .O(new_n29_));
  nand2  g04(.a(x04), .b(x02), .O(new_n30_));
  oai21  g05(.a(new_n29_), .b(x04), .c(new_n30_), .O(new_n31_));
  aoi21  g06(.a(x05), .b(x02), .c(x03), .O(new_n32_));
  nand3  g07(.a(new_n32_), .b(new_n31_), .c(new_n28_), .O(z00));
  nand2  g08(.a(new_n32_), .b(new_n31_), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(new_n28_), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x01), .O(new_n37_));
  inv1   g12(.a(x03), .O(new_n38_));
  inv1   g13(.a(x04), .O(new_n39_));
  inv1   g14(.a(x13), .O(new_n40_));
  nand4  g15(.a(new_n40_), .b(x05), .c(new_n39_), .d(new_n38_), .O(new_n41_));
  inv1   g16(.a(x02), .O(new_n42_));
  nand2  g17(.a(new_n28_), .b(new_n42_), .O(new_n43_));
  nor2   g18(.a(new_n43_), .b(new_n41_), .O(z07));
  nand3  g19(.a(z07), .b(new_n37_), .c(new_n36_), .O(new_n45_));
  inv1   g20(.a(new_n45_), .O(z02));
  inv1   g21(.a(new_n41_), .O(new_n47_));
  nand2  g22(.a(new_n47_), .b(x00), .O(new_n48_));
  nand2  g23(.a(new_n42_), .b(new_n37_), .O(new_n49_));
  oai21  g24(.a(new_n49_), .b(new_n48_), .c(new_n28_), .O(z03));
  nand3  g25(.a(new_n42_), .b(x01), .c(new_n36_), .O(new_n51_));
  oai21  g26(.a(new_n51_), .b(new_n41_), .c(new_n28_), .O(z04));
  nand2  g27(.a(new_n42_), .b(x01), .O(new_n53_));
  oai21  g28(.a(new_n53_), .b(new_n48_), .c(new_n28_), .O(z05));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(new_n36_), .O(new_n60_));
  nand2  g35(.a(new_n27_), .b(x01), .O(new_n61_));
  nand2  g36(.a(new_n26_), .b(new_n37_), .O(new_n62_));
  nand3  g37(.a(new_n62_), .b(new_n61_), .c(x00), .O(new_n63_));
  inv1   g38(.a(x12), .O(new_n64_));
  nand4  g39(.a(x13), .b(new_n64_), .c(x05), .d(new_n42_), .O(new_n65_));
  inv1   g40(.a(new_n65_), .O(new_n66_));
  nand4  g41(.a(new_n66_), .b(new_n63_), .c(new_n60_), .d(new_n55_), .O(new_n67_));
  nand3  g42(.a(new_n40_), .b(new_n29_), .c(x02), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g44(.a(new_n39_), .b(x03), .O(new_n70_));
  nand2  g45(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g46(.a(new_n71_), .b(new_n28_), .O(z06));
  nand2  g47(.a(new_n70_), .b(new_n28_), .O(new_n73_));
  aoi21  g48(.a(new_n68_), .b(new_n65_), .c(new_n73_), .O(z08));
  nand2  g49(.a(new_n28_), .b(x06), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n40_), .O(z09));
  inv1   g51(.a(new_n32_), .O(new_n77_));
  nor2   g52(.a(new_n75_), .b(new_n77_), .O(z10));
endmodule


