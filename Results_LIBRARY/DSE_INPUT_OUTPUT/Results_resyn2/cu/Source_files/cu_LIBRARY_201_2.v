// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n72_, new_n73_, new_n75_, new_n77_;
  inv1   g00(.a(x09), .O(new_n26_));
  nor2   g01(.a(x11), .b(new_n26_), .O(new_n27_));
  inv1   g02(.a(x05), .O(new_n28_));
  nand2  g03(.a(x04), .b(x02), .O(new_n29_));
  oai21  g04(.a(new_n28_), .b(x04), .c(new_n29_), .O(new_n30_));
  aoi21  g05(.a(x05), .b(x02), .c(x03), .O(new_n31_));
  aoi21  g06(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z00));
  inv1   g07(.a(z00), .O(z01));
  inv1   g08(.a(x11), .O(new_n34_));
  nand2  g09(.a(new_n34_), .b(x09), .O(new_n35_));
  inv1   g10(.a(x13), .O(new_n36_));
  inv1   g11(.a(x03), .O(new_n37_));
  inv1   g12(.a(x04), .O(new_n38_));
  nand3  g13(.a(x05), .b(new_n38_), .c(new_n37_), .O(new_n39_));
  inv1   g14(.a(new_n39_), .O(new_n40_));
  nand2  g15(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  inv1   g16(.a(x00), .O(new_n42_));
  inv1   g17(.a(x01), .O(new_n43_));
  inv1   g18(.a(x02), .O(new_n44_));
  nand3  g19(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  oai21  g20(.a(new_n45_), .b(new_n41_), .c(new_n35_), .O(z02));
  nand2  g21(.a(new_n44_), .b(new_n43_), .O(new_n47_));
  nand3  g22(.a(new_n40_), .b(new_n36_), .c(x00), .O(new_n48_));
  oai21  g23(.a(new_n48_), .b(new_n47_), .c(new_n35_), .O(z03));
  nand3  g24(.a(new_n44_), .b(x01), .c(new_n42_), .O(new_n50_));
  nor4   g25(.a(new_n50_), .b(new_n39_), .c(new_n27_), .d(x13), .O(z04));
  nand2  g26(.a(new_n44_), .b(x01), .O(new_n52_));
  oai21  g27(.a(new_n52_), .b(new_n48_), .c(new_n35_), .O(z05));
  nand2  g28(.a(x04), .b(new_n37_), .O(new_n54_));
  inv1   g29(.a(x07), .O(new_n55_));
  inv1   g30(.a(x10), .O(new_n56_));
  nand2  g31(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g32(.a(x08), .O(new_n58_));
  nand2  g33(.a(new_n58_), .b(new_n43_), .O(new_n59_));
  nand3  g34(.a(new_n59_), .b(new_n57_), .c(new_n42_), .O(new_n60_));
  nand2  g35(.a(x11), .b(new_n42_), .O(new_n61_));
  nand2  g36(.a(x11), .b(x01), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n26_), .O(new_n63_));
  nand2  g38(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  inv1   g39(.a(x12), .O(new_n65_));
  nand4  g40(.a(x13), .b(new_n65_), .c(x05), .d(new_n44_), .O(new_n66_));
  inv1   g41(.a(new_n66_), .O(new_n67_));
  nand4  g42(.a(new_n67_), .b(new_n64_), .c(new_n60_), .d(new_n55_), .O(new_n68_));
  nand4  g43(.a(new_n35_), .b(new_n36_), .c(new_n28_), .d(x02), .O(new_n69_));
  aoi21  g44(.a(new_n69_), .b(new_n68_), .c(new_n54_), .O(z06));
  oai21  g45(.a(new_n41_), .b(x02), .c(new_n35_), .O(z07));
  nand3  g46(.a(new_n36_), .b(new_n28_), .c(x02), .O(new_n72_));
  nand3  g47(.a(new_n35_), .b(x04), .c(new_n37_), .O(new_n73_));
  aoi21  g48(.a(new_n72_), .b(new_n66_), .c(new_n73_), .O(z08));
  nand2  g49(.a(x13), .b(x06), .O(new_n75_));
  nor2   g50(.a(new_n75_), .b(new_n27_), .O(z09));
  nand2  g51(.a(new_n31_), .b(x06), .O(new_n77_));
  nand2  g52(.a(new_n77_), .b(new_n35_), .O(z10));
endmodule


