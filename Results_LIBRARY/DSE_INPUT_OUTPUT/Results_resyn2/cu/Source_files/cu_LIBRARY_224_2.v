// Benchmark "FAU" written by ABC on Sat Aug  8 20:43:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10;
  wire new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n36_, new_n37_, new_n38_, new_n40_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x03), .O(new_n26_));
  inv1   g01(.a(x04), .O(new_n27_));
  nand2  g02(.a(x08), .b(x05), .O(new_n28_));
  nand2  g03(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  inv1   g04(.a(x02), .O(new_n30_));
  nor2   g05(.a(x04), .b(x02), .O(new_n31_));
  inv1   g06(.a(new_n31_), .O(new_n32_));
  oai21  g07(.a(x05), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand3  g08(.a(new_n33_), .b(new_n29_), .c(new_n26_), .O(z00));
  inv1   g09(.a(z00), .O(z01));
  inv1   g10(.a(x00), .O(new_n36_));
  inv1   g11(.a(x13), .O(new_n37_));
  nand3  g12(.a(new_n31_), .b(new_n37_), .c(new_n26_), .O(new_n38_));
  nor2   g13(.a(new_n38_), .b(new_n28_), .O(z07));
  nand2  g14(.a(z07), .b(new_n36_), .O(new_n40_));
  nor2   g15(.a(new_n40_), .b(x01), .O(z02));
  nor4   g16(.a(new_n38_), .b(new_n28_), .c(x01), .d(new_n36_), .O(z03));
  inv1   g17(.a(x01), .O(new_n43_));
  nor2   g18(.a(new_n40_), .b(new_n43_), .O(z04));
  inv1   g19(.a(x05), .O(new_n45_));
  inv1   g20(.a(new_n38_), .O(new_n46_));
  nand3  g21(.a(new_n46_), .b(x01), .c(x00), .O(new_n47_));
  aoi21  g22(.a(new_n47_), .b(x08), .c(new_n45_), .O(z05));
  nand3  g23(.a(new_n37_), .b(new_n45_), .c(x02), .O(new_n49_));
  inv1   g24(.a(new_n49_), .O(new_n50_));
  nand2  g25(.a(x11), .b(x01), .O(new_n51_));
  nand2  g26(.a(x09), .b(new_n43_), .O(new_n52_));
  nand3  g27(.a(new_n52_), .b(new_n51_), .c(x00), .O(new_n53_));
  inv1   g28(.a(x10), .O(new_n54_));
  nand3  g29(.a(new_n54_), .b(x01), .c(new_n36_), .O(new_n55_));
  nor2   g30(.a(x12), .b(x07), .O(new_n56_));
  nand4  g31(.a(new_n56_), .b(x13), .c(x05), .d(new_n30_), .O(new_n57_));
  aoi21  g32(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nor2   g33(.a(new_n27_), .b(x03), .O(new_n59_));
  oai21  g34(.a(new_n58_), .b(new_n50_), .c(new_n59_), .O(new_n60_));
  inv1   g35(.a(x08), .O(new_n61_));
  nand2  g36(.a(new_n61_), .b(x05), .O(new_n62_));
  nand2  g37(.a(new_n62_), .b(new_n60_), .O(z06));
  inv1   g38(.a(new_n59_), .O(new_n64_));
  nor2   g39(.a(x12), .b(new_n61_), .O(new_n65_));
  nand4  g40(.a(new_n65_), .b(x13), .c(x05), .d(new_n30_), .O(new_n66_));
  aoi21  g41(.a(new_n66_), .b(new_n49_), .c(new_n64_), .O(z08));
  nand2  g42(.a(x13), .b(x06), .O(new_n68_));
  nand2  g43(.a(new_n68_), .b(new_n62_), .O(z09));
  nand2  g44(.a(x08), .b(new_n30_), .O(new_n70_));
  nand2  g45(.a(x06), .b(new_n26_), .O(new_n71_));
  aoi21  g46(.a(new_n70_), .b(x05), .c(new_n71_), .O(z10));
endmodule


