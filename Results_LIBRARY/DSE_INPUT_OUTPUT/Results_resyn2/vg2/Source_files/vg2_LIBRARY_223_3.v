// Benchmark "FAU" written by ABC on Tue Aug 11 20:50:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  nand2  g001(.a(new_n34_), .b(x13), .O(new_n35_));
  nor2   g002(.a(x19), .b(x13), .O(new_n36_));
  nor2   g003(.a(new_n36_), .b(x05), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g005(.a(x13), .b(x05), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand4  g009(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(x00), .O(new_n43_));
  nand3  g010(.a(x24), .b(x07), .c(x05), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  inv1   g013(.a(x06), .O(new_n47_));
  nor3   g014(.a(x20), .b(x14), .c(x11), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  aoi21  g017(.a(new_n46_), .b(new_n38_), .c(new_n50_), .O(z0));
  nor2   g018(.a(x13), .b(x05), .O(new_n52_));
  nand4  g019(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n53_));
  nand3  g020(.a(x11), .b(x09), .c(x04), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g022(.a(x14), .b(x12), .O(new_n56_));
  nand4  g023(.a(x23), .b(x22), .c(x17), .d(x16), .O(new_n57_));
  nor2   g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g025(.a(x20), .b(x15), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n58_), .c(new_n55_), .O(new_n61_));
  nor2   g028(.a(x14), .b(x11), .O(new_n62_));
  nor2   g029(.a(x08), .b(x06), .O(new_n63_));
  inv1   g030(.a(x19), .O(new_n64_));
  nor2   g031(.a(x20), .b(new_n64_), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n63_), .c(new_n49_), .d(new_n62_), .O(new_n66_));
  inv1   g033(.a(x04), .O(new_n67_));
  inv1   g034(.a(x09), .O(new_n68_));
  nor2   g035(.a(x16), .b(x12), .O(new_n69_));
  nor3   g036(.a(x23), .b(x22), .c(x17), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  oai21  g038(.a(new_n71_), .b(new_n66_), .c(new_n61_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n52_), .O(new_n73_));
  nor3   g040(.a(x24), .b(x10), .c(x02), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n39_), .O(new_n75_));
  inv1   g042(.a(new_n75_), .O(new_n76_));
  nand3  g043(.a(new_n70_), .b(new_n69_), .c(new_n48_), .O(new_n77_));
  nor3   g044(.a(x09), .b(x08), .c(x04), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n49_), .c(new_n47_), .d(x00), .O(new_n79_));
  and2   g046(.a(x21), .b(x20), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n58_), .c(new_n55_), .O(new_n81_));
  oai21  g048(.a(new_n79_), .b(new_n77_), .c(new_n81_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n76_), .O(new_n83_));
  nand2  g050(.a(new_n39_), .b(x24), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  inv1   g052(.a(x18), .O(new_n86_));
  inv1   g053(.a(x20), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n58_), .c(new_n55_), .O(new_n89_));
  nand4  g056(.a(new_n78_), .b(new_n49_), .c(x07), .d(new_n47_), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n77_), .c(new_n89_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n83_), .c(new_n73_), .O(z1));
  inv1   g060(.a(x05), .O(new_n94_));
  nor2   g061(.a(new_n42_), .b(new_n86_), .O(new_n95_));
  nand3  g062(.a(x06), .b(x03), .c(x01), .O(new_n96_));
  nand2  g063(.a(x14), .b(x11), .O(new_n97_));
  nor3   g064(.a(new_n97_), .b(new_n96_), .c(new_n87_), .O(new_n98_));
  aoi21  g065(.a(new_n98_), .b(new_n95_), .c(x13), .O(new_n99_));
  nand2  g066(.a(new_n95_), .b(x13), .O(new_n100_));
  nand2  g067(.a(new_n52_), .b(x15), .O(new_n101_));
  nand2  g068(.a(new_n74_), .b(x21), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  oai21  g071(.a(new_n99_), .b(new_n94_), .c(new_n104_), .O(z2));
  inv1   g072(.a(new_n53_), .O(new_n106_));
  inv1   g073(.a(new_n97_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai21  g075(.a(new_n108_), .b(new_n59_), .c(new_n66_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n52_), .O(new_n110_));
  nand2  g077(.a(new_n47_), .b(x00), .O(new_n111_));
  inv1   g078(.a(x08), .O(new_n112_));
  nand3  g079(.a(new_n49_), .b(new_n48_), .c(new_n112_), .O(new_n113_));
  nand3  g080(.a(new_n107_), .b(new_n80_), .c(new_n106_), .O(new_n114_));
  oai21  g081(.a(new_n113_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n76_), .O(new_n116_));
  nand2  g083(.a(x07), .b(new_n47_), .O(new_n117_));
  nand3  g084(.a(new_n107_), .b(new_n88_), .c(new_n106_), .O(new_n118_));
  oai21  g085(.a(new_n113_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n85_), .O(new_n120_));
  nand3  g087(.a(new_n120_), .b(new_n116_), .c(new_n110_), .O(z3));
  inv1   g088(.a(x16), .O(new_n122_));
  inv1   g089(.a(x23), .O(new_n123_));
  oai21  g090(.a(new_n123_), .b(x04), .c(x17), .O(new_n124_));
  aoi21  g091(.a(new_n124_), .b(x22), .c(new_n68_), .O(new_n125_));
  oai21  g092(.a(new_n125_), .b(new_n122_), .c(x08), .O(new_n126_));
  inv1   g093(.a(new_n52_), .O(new_n127_));
  nand2  g094(.a(new_n95_), .b(new_n127_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n102_), .c(new_n101_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand2  g097(.a(new_n46_), .b(new_n38_), .O(z5));
  inv1   g098(.a(x22), .O(new_n132_));
  inv1   g099(.a(x17), .O(new_n133_));
  oai21  g100(.a(x23), .b(new_n67_), .c(new_n133_), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(new_n132_), .c(x09), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(x16), .c(new_n112_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(z5), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n130_), .c(new_n39_), .O(z4));
  inv1   g105(.a(x11), .O(new_n139_));
  aoi21  g106(.a(new_n87_), .b(x14), .c(new_n47_), .O(new_n140_));
  oai21  g107(.a(new_n140_), .b(new_n139_), .c(x03), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n129_), .O(new_n142_));
  inv1   g109(.a(new_n43_), .O(new_n143_));
  inv1   g110(.a(x14), .O(new_n144_));
  nand2  g111(.a(x20), .b(new_n144_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n47_), .c(x11), .O(new_n146_));
  aoi22  g113(.a(new_n127_), .b(new_n34_), .c(new_n36_), .d(new_n94_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n143_), .c(new_n146_), .d(x03), .O(new_n148_));
  nand3  g115(.a(new_n148_), .b(new_n142_), .c(new_n39_), .O(z6));
  nand4  g116(.a(new_n128_), .b(new_n102_), .c(new_n101_), .d(new_n39_), .O(z7));
endmodule


