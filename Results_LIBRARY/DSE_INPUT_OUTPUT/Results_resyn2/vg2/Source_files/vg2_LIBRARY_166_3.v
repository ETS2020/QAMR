// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  nand2  g000(.a(x24), .b(x13), .O(new_n34_));
  inv1   g001(.a(x06), .O(new_n35_));
  inv1   g002(.a(x14), .O(new_n36_));
  inv1   g003(.a(x20), .O(new_n37_));
  nand3  g004(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  inv1   g005(.a(new_n38_), .O(new_n39_));
  nor3   g006(.a(x11), .b(x03), .c(x01), .O(new_n40_));
  inv1   g007(.a(x13), .O(new_n41_));
  inv1   g008(.a(x19), .O(new_n42_));
  nand2  g009(.a(x24), .b(x07), .O(new_n43_));
  oai21  g010(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(x05), .O(new_n45_));
  inv1   g012(.a(x02), .O(new_n46_));
  inv1   g013(.a(x10), .O(new_n47_));
  inv1   g014(.a(x24), .O(new_n48_));
  nand4  g015(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x00), .O(new_n49_));
  nand2  g016(.a(new_n48_), .b(x13), .O(new_n50_));
  nor2   g017(.a(new_n42_), .b(x05), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g019(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n40_), .c(new_n39_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n34_), .O(z0));
  inv1   g022(.a(x05), .O(new_n56_));
  nand2  g023(.a(x15), .b(x13), .O(new_n57_));
  nand3  g024(.a(x21), .b(new_n47_), .c(new_n46_), .O(new_n58_));
  oai21  g025(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g026(.a(x03), .b(x01), .O(new_n60_));
  nand4  g027(.a(x09), .b(x08), .c(x06), .d(x04), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g029(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n63_));
  nand4  g030(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n64_));
  nor2   g031(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n62_), .c(new_n59_), .O(new_n66_));
  nand3  g033(.a(new_n47_), .b(new_n46_), .c(x00), .O(new_n67_));
  nand3  g034(.a(x19), .b(x13), .c(x05), .O(new_n68_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g036(.a(x09), .O(new_n70_));
  inv1   g037(.a(x11), .O(new_n71_));
  inv1   g038(.a(x12), .O(new_n72_));
  inv1   g039(.a(x16), .O(new_n73_));
  nand4  g040(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  inv1   g041(.a(x01), .O(new_n75_));
  inv1   g042(.a(x03), .O(new_n76_));
  inv1   g043(.a(x04), .O(new_n77_));
  inv1   g044(.a(x08), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  inv1   g047(.a(x17), .O(new_n81_));
  inv1   g048(.a(x22), .O(new_n82_));
  inv1   g049(.a(x23), .O(new_n83_));
  nand3  g050(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n38_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n80_), .c(new_n69_), .O(new_n86_));
  nand2  g053(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n48_), .O(new_n88_));
  nand2  g055(.a(x15), .b(new_n56_), .O(new_n89_));
  nand2  g056(.a(x24), .b(x18), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n56_), .c(new_n89_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n65_), .c(new_n62_), .O(new_n92_));
  nand3  g059(.a(x24), .b(x07), .c(x05), .O(new_n93_));
  oai21  g060(.a(new_n42_), .b(x05), .c(new_n93_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n85_), .c(new_n80_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n41_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n88_), .O(z1));
  inv1   g065(.a(new_n34_), .O(new_n99_));
  nand4  g066(.a(x20), .b(x14), .c(x11), .d(x06), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n60_), .O(new_n101_));
  nor2   g068(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand4  g069(.a(new_n48_), .b(x21), .c(new_n47_), .d(new_n46_), .O(new_n103_));
  nand3  g070(.a(x15), .b(new_n41_), .c(new_n56_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g072(.a(new_n90_), .b(new_n57_), .c(new_n56_), .O(new_n106_));
  nor3   g073(.a(new_n106_), .b(new_n105_), .c(new_n99_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n102_), .O(z2));
  nand3  g075(.a(new_n101_), .b(new_n59_), .c(x08), .O(new_n109_));
  nand4  g076(.a(new_n69_), .b(new_n40_), .c(new_n39_), .d(new_n78_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n48_), .O(new_n112_));
  nand3  g079(.a(new_n101_), .b(new_n91_), .c(x08), .O(new_n113_));
  nand4  g080(.a(new_n94_), .b(new_n40_), .c(new_n39_), .d(new_n78_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n41_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n112_), .O(z3));
  inv1   g084(.a(new_n53_), .O(new_n118_));
  aoi21  g085(.a(new_n83_), .b(x04), .c(x17), .O(new_n119_));
  oai21  g086(.a(new_n119_), .b(x22), .c(new_n70_), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(new_n73_), .c(x08), .O(new_n121_));
  aoi21  g088(.a(x23), .b(new_n77_), .c(new_n81_), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(new_n82_), .c(x09), .O(new_n123_));
  nand2  g090(.a(new_n34_), .b(x08), .O(new_n124_));
  aoi21  g091(.a(new_n123_), .b(x16), .c(new_n124_), .O(new_n125_));
  oai22  g092(.a(new_n125_), .b(new_n107_), .c(new_n121_), .d(new_n118_), .O(z4));
  nand2  g093(.a(new_n118_), .b(new_n34_), .O(z5));
  aoi21  g094(.a(x20), .b(new_n36_), .c(x06), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(x11), .c(new_n76_), .O(new_n129_));
  nand3  g096(.a(new_n44_), .b(new_n34_), .c(x05), .O(new_n130_));
  nand2  g097(.a(new_n51_), .b(new_n41_), .O(new_n131_));
  nand3  g098(.a(new_n131_), .b(new_n130_), .c(new_n49_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  aoi21  g100(.a(new_n37_), .b(x14), .c(new_n35_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n71_), .c(x03), .O(new_n135_));
  nand2  g102(.a(new_n106_), .b(new_n34_), .O(new_n136_));
  inv1   g103(.a(new_n136_), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(new_n105_), .c(new_n135_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n133_), .O(z6));
  inv1   g106(.a(new_n107_), .O(z7));
endmodule


