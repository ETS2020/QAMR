// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_;
  inv1   g000(.a(x24), .O(new_n34_));
  nand3  g001(.a(x19), .b(x13), .c(x05), .O(new_n35_));
  nor2   g002(.a(x10), .b(x02), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(x00), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  nand2  g006(.a(x24), .b(x07), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nor2   g008(.a(x03), .b(x01), .O(new_n42_));
  nor2   g009(.a(x20), .b(x14), .O(new_n43_));
  nor2   g010(.a(x11), .b(x06), .O(new_n44_));
  nand4  g011(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(new_n41_), .O(new_n45_));
  nor2   g012(.a(x13), .b(x05), .O(new_n46_));
  inv1   g013(.a(new_n46_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n45_), .O(z0));
  nand4  g015(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n49_));
  nand2  g016(.a(x09), .b(x04), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g018(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n52_));
  nand4  g019(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g021(.a(new_n54_), .b(new_n51_), .c(x18), .O(new_n55_));
  inv1   g022(.a(x16), .O(new_n56_));
  inv1   g023(.a(x17), .O(new_n57_));
  inv1   g024(.a(x22), .O(new_n58_));
  inv1   g025(.a(x23), .O(new_n59_));
  nand4  g026(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  inv1   g027(.a(new_n60_), .O(new_n61_));
  nor2   g028(.a(x06), .b(x04), .O(new_n62_));
  nor2   g029(.a(x11), .b(x09), .O(new_n63_));
  nand3  g030(.a(new_n63_), .b(new_n62_), .c(new_n42_), .O(new_n64_));
  inv1   g031(.a(new_n64_), .O(new_n65_));
  inv1   g032(.a(x08), .O(new_n66_));
  inv1   g033(.a(x12), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(x07), .O(new_n68_));
  inv1   g035(.a(new_n68_), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n65_), .c(new_n61_), .d(new_n43_), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n55_), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(x24), .O(new_n72_));
  nand3  g039(.a(new_n43_), .b(new_n67_), .c(new_n66_), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n65_), .c(new_n61_), .d(new_n38_), .O(new_n75_));
  nand3  g042(.a(x15), .b(x13), .c(x05), .O(new_n76_));
  nand2  g043(.a(new_n36_), .b(x21), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n54_), .c(new_n51_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n34_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n72_), .c(new_n47_), .O(z1));
  nand2  g049(.a(new_n77_), .b(new_n34_), .O(new_n83_));
  nor2   g050(.a(new_n34_), .b(x18), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g053(.a(x13), .b(x05), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(x24), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(x15), .O(new_n89_));
  oai21  g056(.a(new_n86_), .b(new_n46_), .c(new_n89_), .O(z7));
  nand3  g057(.a(x06), .b(x03), .c(x01), .O(new_n91_));
  nand3  g058(.a(x20), .b(x14), .c(x11), .O(new_n92_));
  nor2   g059(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(z7), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(z2));
  aoi21  g062(.a(new_n77_), .b(new_n34_), .c(new_n84_), .O(new_n96_));
  nor2   g063(.a(new_n92_), .b(new_n49_), .O(new_n97_));
  inv1   g064(.a(x02), .O(new_n98_));
  inv1   g065(.a(x10), .O(new_n99_));
  nand4  g066(.a(new_n34_), .b(new_n99_), .c(new_n98_), .d(x00), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n40_), .O(new_n101_));
  nand4  g068(.a(new_n44_), .b(new_n43_), .c(new_n42_), .d(new_n66_), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  aoi22  g070(.a(new_n103_), .b(new_n101_), .c(new_n97_), .d(new_n96_), .O(new_n104_));
  and2   g071(.a(new_n97_), .b(x15), .O(new_n105_));
  inv1   g072(.a(x19), .O(new_n106_));
  nor2   g073(.a(new_n102_), .b(new_n106_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n105_), .c(new_n88_), .O(new_n108_));
  oai21  g075(.a(new_n104_), .b(new_n46_), .c(new_n108_), .O(z3));
  nand2  g076(.a(new_n89_), .b(new_n86_), .O(new_n110_));
  inv1   g077(.a(x09), .O(new_n111_));
  oai21  g078(.a(new_n59_), .b(x04), .c(x17), .O(new_n112_));
  aoi21  g079(.a(new_n112_), .b(x22), .c(new_n111_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n56_), .c(x08), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand3  g082(.a(x24), .b(new_n66_), .c(x07), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n39_), .O(new_n117_));
  aoi21  g084(.a(new_n59_), .b(x04), .c(x17), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(x22), .c(new_n111_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n117_), .c(new_n56_), .O(new_n120_));
  nand2  g087(.a(new_n101_), .b(new_n47_), .O(new_n121_));
  inv1   g088(.a(new_n35_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n34_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n121_), .O(z5));
  aoi21  g091(.a(z5), .b(x08), .c(new_n46_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n120_), .c(new_n115_), .O(z4));
  inv1   g093(.a(x11), .O(new_n127_));
  inv1   g094(.a(x06), .O(new_n128_));
  inv1   g095(.a(x20), .O(new_n129_));
  aoi21  g096(.a(new_n129_), .b(x14), .c(new_n128_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n127_), .c(x03), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(z7), .O(new_n132_));
  inv1   g099(.a(x14), .O(new_n133_));
  nand2  g100(.a(x20), .b(new_n133_), .O(new_n134_));
  aoi21  g101(.a(new_n134_), .b(new_n128_), .c(x11), .O(new_n135_));
  oai21  g102(.a(new_n135_), .b(x03), .c(z5), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n132_), .O(z6));
endmodule


