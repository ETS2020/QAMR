// Benchmark "FAU" written by ABC on Tue Aug 11 20:50:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_;
  nand2  g000(.a(x13), .b(x05), .O(new_n34_));
  nor3   g001(.a(x24), .b(x10), .c(x02), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(x00), .O(new_n36_));
  nor2   g003(.a(x13), .b(x05), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x19), .O(new_n38_));
  inv1   g005(.a(new_n37_), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(x24), .c(x07), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n38_), .c(new_n36_), .O(new_n41_));
  nor2   g008(.a(x03), .b(x01), .O(new_n42_));
  nor2   g009(.a(x14), .b(x11), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  nor2   g012(.a(x20), .b(x06), .O(new_n46_));
  nand4  g013(.a(new_n46_), .b(new_n45_), .c(new_n41_), .d(new_n34_), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(z0));
  nand4  g015(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n49_));
  nand3  g016(.a(x11), .b(x09), .c(x04), .O(new_n50_));
  nor2   g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g018(.a(x17), .b(x16), .c(x14), .d(x12), .O(new_n52_));
  nand4  g019(.a(x23), .b(x22), .c(x20), .d(x15), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nor3   g022(.a(x04), .b(x03), .c(x01), .O(new_n56_));
  nor2   g023(.a(x12), .b(x09), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n56_), .c(new_n43_), .O(new_n58_));
  nor2   g025(.a(x17), .b(x16), .O(new_n59_));
  nor2   g026(.a(x23), .b(x22), .O(new_n60_));
  nor2   g027(.a(x20), .b(x08), .O(new_n61_));
  inv1   g028(.a(x19), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(x06), .O(new_n63_));
  nand4  g030(.a(new_n63_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n64_));
  oai21  g031(.a(new_n64_), .b(new_n58_), .c(new_n55_), .O(new_n65_));
  nand2  g032(.a(new_n65_), .b(new_n37_), .O(new_n66_));
  nand2  g033(.a(new_n35_), .b(new_n34_), .O(new_n67_));
  inv1   g034(.a(new_n67_), .O(new_n68_));
  nor2   g035(.a(x20), .b(x12), .O(new_n69_));
  nand4  g036(.a(new_n69_), .b(new_n60_), .c(new_n59_), .d(new_n43_), .O(new_n70_));
  inv1   g037(.a(x06), .O(new_n71_));
  nor2   g038(.a(x09), .b(x08), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n56_), .c(new_n71_), .d(x00), .O(new_n73_));
  nand4  g040(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  oai21  g043(.a(new_n73_), .b(new_n70_), .c(new_n76_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n68_), .O(new_n78_));
  nand2  g045(.a(new_n34_), .b(x24), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n37_), .O(new_n80_));
  nand4  g047(.a(x23), .b(x22), .c(x20), .d(x18), .O(new_n81_));
  nor2   g048(.a(new_n81_), .b(new_n52_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n51_), .O(new_n83_));
  inv1   g050(.a(x04), .O(new_n84_));
  inv1   g051(.a(x07), .O(new_n85_));
  nor2   g052(.a(new_n85_), .b(x06), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n72_), .c(new_n42_), .d(new_n84_), .O(new_n87_));
  oai21  g054(.a(new_n87_), .b(new_n70_), .c(new_n83_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n80_), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(new_n78_), .c(new_n66_), .O(z1));
  nand3  g057(.a(new_n35_), .b(new_n34_), .c(x21), .O(new_n91_));
  inv1   g058(.a(x13), .O(new_n92_));
  and2   g059(.a(x24), .b(x18), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n92_), .c(x05), .O(new_n94_));
  inv1   g061(.a(x05), .O(new_n95_));
  inv1   g062(.a(x15), .O(new_n96_));
  nand3  g063(.a(x24), .b(x18), .c(x13), .O(new_n97_));
  oai21  g064(.a(new_n96_), .b(x13), .c(new_n97_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n95_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n94_), .c(new_n91_), .O(new_n100_));
  inv1   g067(.a(new_n100_), .O(new_n101_));
  and2   g068(.a(x03), .b(x01), .O(new_n102_));
  inv1   g069(.a(x20), .O(new_n103_));
  nor2   g070(.a(new_n103_), .b(new_n71_), .O(new_n104_));
  nand4  g071(.a(new_n104_), .b(new_n102_), .c(x14), .d(x11), .O(new_n105_));
  nor2   g072(.a(new_n105_), .b(new_n101_), .O(z2));
  nand2  g073(.a(new_n63_), .b(new_n61_), .O(new_n107_));
  nor2   g074(.a(new_n107_), .b(new_n44_), .O(new_n108_));
  nand2  g075(.a(x20), .b(x15), .O(new_n109_));
  inv1   g076(.a(new_n49_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(x14), .c(x11), .O(new_n111_));
  nor2   g078(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n108_), .c(new_n37_), .O(new_n113_));
  nand2  g080(.a(x21), .b(x20), .O(new_n114_));
  nor2   g081(.a(new_n111_), .b(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n61_), .b(new_n43_), .O(new_n116_));
  nand3  g083(.a(new_n42_), .b(new_n71_), .c(x00), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(new_n115_), .c(new_n68_), .O(new_n119_));
  nand2  g086(.a(new_n86_), .b(new_n42_), .O(new_n120_));
  nor2   g087(.a(new_n116_), .b(new_n120_), .O(new_n121_));
  nand2  g088(.a(x20), .b(x18), .O(new_n122_));
  nor2   g089(.a(new_n111_), .b(new_n122_), .O(new_n123_));
  oai21  g090(.a(new_n123_), .b(new_n121_), .c(new_n80_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n119_), .c(new_n113_), .O(z3));
  nand2  g092(.a(new_n41_), .b(new_n34_), .O(new_n126_));
  inv1   g093(.a(x08), .O(new_n127_));
  inv1   g094(.a(x22), .O(new_n128_));
  inv1   g095(.a(x17), .O(new_n129_));
  aoi21  g096(.a(x23), .b(new_n84_), .c(new_n129_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n128_), .c(x09), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(x16), .c(new_n127_), .O(new_n132_));
  inv1   g099(.a(x16), .O(new_n133_));
  inv1   g100(.a(x09), .O(new_n134_));
  inv1   g101(.a(x23), .O(new_n135_));
  aoi21  g102(.a(new_n135_), .b(x04), .c(x17), .O(new_n136_));
  oai21  g103(.a(new_n136_), .b(x22), .c(new_n134_), .O(new_n137_));
  aoi21  g104(.a(new_n137_), .b(new_n133_), .c(x08), .O(new_n138_));
  oai22  g105(.a(new_n138_), .b(new_n126_), .c(new_n132_), .d(new_n101_), .O(z4));
  inv1   g106(.a(new_n126_), .O(z5));
  inv1   g107(.a(x11), .O(new_n141_));
  aoi21  g108(.a(new_n103_), .b(x14), .c(new_n71_), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n141_), .c(x03), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n100_), .O(new_n144_));
  oai21  g111(.a(new_n103_), .b(x14), .c(new_n71_), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(new_n141_), .c(x03), .O(new_n146_));
  oai21  g113(.a(new_n146_), .b(new_n126_), .c(new_n144_), .O(z6));
  nand2  g114(.a(new_n35_), .b(x21), .O(new_n148_));
  oai21  g115(.a(new_n93_), .b(x13), .c(x05), .O(new_n149_));
  nand2  g116(.a(new_n37_), .b(x15), .O(new_n150_));
  nand4  g117(.a(new_n150_), .b(new_n149_), .c(new_n148_), .d(new_n97_), .O(z7));
endmodule


