// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_;
  inv1   g000(.a(x13), .O(new_n34_));
  inv1   g001(.a(x19), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g003(.a(x24), .b(x07), .O(new_n37_));
  inv1   g004(.a(x24), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  inv1   g010(.a(x05), .O(new_n44_));
  nand2  g011(.a(new_n34_), .b(new_n44_), .O(new_n45_));
  nand3  g012(.a(new_n38_), .b(x13), .c(x05), .O(new_n46_));
  nand2  g013(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(x19), .O(new_n48_));
  nand3  g015(.a(new_n48_), .b(new_n43_), .c(new_n37_), .O(new_n49_));
  nand2  g016(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  inv1   g017(.a(x06), .O(new_n51_));
  inv1   g018(.a(x14), .O(new_n52_));
  inv1   g019(.a(x20), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  inv1   g021(.a(x01), .O(new_n55_));
  inv1   g022(.a(x03), .O(new_n56_));
  inv1   g023(.a(x11), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g026(.a(new_n59_), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n50_), .O(z0));
  nor2   g028(.a(x03), .b(x01), .O(new_n62_));
  nor2   g029(.a(x11), .b(x09), .O(new_n63_));
  nor2   g030(.a(x16), .b(x12), .O(new_n64_));
  nor2   g031(.a(x08), .b(x04), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  inv1   g033(.a(x17), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nor3   g037(.a(new_n70_), .b(new_n66_), .c(new_n54_), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n49_), .c(new_n36_), .O(new_n72_));
  nand2  g039(.a(x24), .b(x18), .O(new_n73_));
  nand3  g040(.a(new_n38_), .b(x15), .c(x13), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(x05), .O(new_n76_));
  nand3  g043(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n77_));
  inv1   g044(.a(new_n77_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n38_), .O(new_n79_));
  nor2   g046(.a(x13), .b(x05), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(x15), .O(new_n81_));
  inv1   g048(.a(new_n73_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(x13), .O(new_n83_));
  nand4  g050(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n76_), .O(new_n84_));
  nand4  g051(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n85_));
  inv1   g052(.a(new_n85_), .O(new_n86_));
  nand3  g053(.a(x20), .b(x14), .c(x11), .O(new_n87_));
  nand3  g054(.a(x23), .b(x22), .c(x17), .O(new_n88_));
  nand4  g055(.a(x16), .b(x12), .c(x09), .d(x04), .O(new_n89_));
  nor3   g056(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n86_), .c(new_n84_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n72_), .c(new_n36_), .O(z1));
  nor4   g059(.a(new_n87_), .b(new_n51_), .c(new_n56_), .d(new_n55_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n84_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n36_), .O(z2));
  inv1   g062(.a(x08), .O(new_n96_));
  oai21  g063(.a(new_n41_), .b(x24), .c(new_n37_), .O(new_n97_));
  nand3  g064(.a(new_n59_), .b(new_n97_), .c(new_n96_), .O(new_n98_));
  nor2   g065(.a(new_n87_), .b(new_n85_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n78_), .c(new_n38_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n36_), .O(new_n102_));
  nand3  g069(.a(x24), .b(x18), .c(x05), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n81_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  nand3  g072(.a(new_n59_), .b(new_n47_), .c(new_n96_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(x19), .O(new_n108_));
  nand3  g075(.a(new_n38_), .b(x15), .c(x05), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n73_), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n99_), .c(x13), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n108_), .c(new_n102_), .O(z3));
  inv1   g079(.a(x04), .O(new_n113_));
  oai21  g080(.a(x23), .b(new_n113_), .c(new_n67_), .O(new_n114_));
  aoi21  g081(.a(new_n114_), .b(new_n68_), .c(x09), .O(new_n115_));
  oai21  g082(.a(new_n115_), .b(x16), .c(new_n96_), .O(new_n116_));
  nand3  g083(.a(x19), .b(x13), .c(x05), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(new_n41_), .c(x24), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n45_), .c(new_n37_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  inv1   g088(.a(x16), .O(new_n122_));
  inv1   g089(.a(x09), .O(new_n123_));
  oai21  g090(.a(new_n69_), .b(x04), .c(x17), .O(new_n124_));
  aoi21  g091(.a(new_n124_), .b(x22), .c(new_n123_), .O(new_n125_));
  oai21  g092(.a(new_n125_), .b(new_n122_), .c(x08), .O(new_n126_));
  oai21  g093(.a(new_n45_), .b(x08), .c(new_n82_), .O(new_n127_));
  nand3  g094(.a(x15), .b(x13), .c(x05), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n77_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n38_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n126_), .O(new_n132_));
  inv1   g099(.a(x15), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(x05), .c(x19), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n34_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n132_), .c(new_n121_), .O(z4));
  oai21  g103(.a(new_n35_), .b(new_n44_), .c(new_n34_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n119_), .c(new_n37_), .O(z5));
  nand3  g105(.a(new_n129_), .b(new_n36_), .c(new_n38_), .O(new_n139_));
  nand2  g106(.a(new_n137_), .b(new_n82_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand2  g108(.a(new_n141_), .b(new_n56_), .O(new_n142_));
  nand2  g109(.a(new_n82_), .b(x03), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n139_), .O(new_n144_));
  nand2  g111(.a(new_n53_), .b(x14), .O(new_n145_));
  aoi21  g112(.a(new_n145_), .b(x06), .c(new_n57_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand2  g114(.a(new_n45_), .b(new_n37_), .O(new_n148_));
  nand2  g115(.a(x20), .b(new_n52_), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n51_), .c(x11), .O(new_n150_));
  oai22  g117(.a(new_n150_), .b(x03), .c(new_n148_), .d(new_n118_), .O(new_n151_));
  nand4  g118(.a(new_n151_), .b(new_n147_), .c(new_n142_), .d(new_n135_), .O(z6));
  nand3  g119(.a(new_n80_), .b(x19), .c(x15), .O(new_n153_));
  nand3  g120(.a(new_n153_), .b(new_n140_), .c(new_n139_), .O(z7));
endmodule


