// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x16), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x05), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  nand4  g008(.a(new_n36_), .b(new_n41_), .c(new_n40_), .d(x00), .O(new_n42_));
  nor2   g009(.a(x13), .b(x05), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x19), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g013(.a(new_n46_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n39_), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  nor2   g016(.a(x03), .b(x01), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nor3   g018(.a(new_n51_), .b(x14), .c(x11), .O(new_n52_));
  nand2  g019(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  aoi21  g020(.a(new_n53_), .b(new_n34_), .c(x20), .O(z0));
  nand3  g021(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n55_));
  nand3  g022(.a(x19), .b(x13), .c(x05), .O(new_n56_));
  nand2  g023(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g024(.a(x14), .O(new_n58_));
  inv1   g025(.a(x17), .O(new_n59_));
  inv1   g026(.a(x20), .O(new_n60_));
  nand3  g027(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  inv1   g028(.a(x08), .O(new_n62_));
  inv1   g029(.a(x09), .O(new_n63_));
  inv1   g030(.a(x22), .O(new_n64_));
  inv1   g031(.a(x23), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  inv1   g034(.a(x04), .O(new_n68_));
  nor2   g035(.a(x12), .b(x11), .O(new_n69_));
  nand2  g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor2   g037(.a(new_n70_), .b(new_n51_), .O(new_n71_));
  nand3  g038(.a(new_n71_), .b(new_n67_), .c(new_n57_), .O(new_n72_));
  nand3  g039(.a(x21), .b(new_n41_), .c(new_n40_), .O(new_n73_));
  nand3  g040(.a(x15), .b(x13), .c(x05), .O(new_n74_));
  nand2  g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand4  g042(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n76_));
  nand4  g043(.a(x23), .b(x22), .c(x14), .d(x12), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor2   g045(.a(new_n59_), .b(new_n34_), .O(new_n79_));
  nand3  g046(.a(x11), .b(x09), .c(x04), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  nand4  g048(.a(new_n81_), .b(new_n79_), .c(new_n78_), .d(new_n75_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n36_), .O(new_n84_));
  nor2   g051(.a(new_n43_), .b(new_n36_), .O(new_n85_));
  nor3   g052(.a(x20), .b(x17), .c(x14), .O(new_n86_));
  inv1   g053(.a(new_n66_), .O(new_n87_));
  nand2  g054(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand4  g055(.a(new_n81_), .b(new_n79_), .c(new_n78_), .d(x18), .O(new_n89_));
  nor3   g056(.a(x04), .b(x03), .c(x01), .O(new_n90_));
  nand4  g057(.a(new_n90_), .b(new_n69_), .c(x07), .d(new_n49_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(new_n88_), .c(new_n89_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nor2   g060(.a(x23), .b(x22), .O(new_n94_));
  inv1   g061(.a(x19), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(x09), .O(new_n96_));
  nor2   g063(.a(x08), .b(x06), .O(new_n97_));
  nand4  g064(.a(new_n97_), .b(new_n96_), .c(new_n69_), .d(new_n94_), .O(new_n98_));
  nand2  g065(.a(new_n90_), .b(new_n86_), .O(new_n99_));
  inv1   g066(.a(new_n76_), .O(new_n100_));
  inv1   g067(.a(new_n77_), .O(new_n101_));
  nand3  g068(.a(x17), .b(x16), .c(x15), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(new_n81_), .c(new_n101_), .d(new_n100_), .O(new_n104_));
  oai21  g071(.a(new_n99_), .b(new_n98_), .c(new_n104_), .O(new_n105_));
  nor2   g072(.a(x20), .b(new_n34_), .O(new_n106_));
  aoi21  g073(.a(new_n105_), .b(new_n43_), .c(new_n106_), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n93_), .c(new_n84_), .O(z1));
  inv1   g075(.a(x18), .O(new_n109_));
  nand3  g076(.a(new_n36_), .b(x15), .c(x13), .O(new_n110_));
  oai21  g077(.a(new_n36_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(x05), .O(new_n112_));
  nand4  g079(.a(new_n36_), .b(x21), .c(new_n41_), .d(new_n40_), .O(new_n113_));
  nand3  g080(.a(x24), .b(x18), .c(x13), .O(new_n114_));
  nand2  g081(.a(new_n43_), .b(x15), .O(new_n115_));
  nand3  g082(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  inv1   g083(.a(new_n116_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  inv1   g085(.a(new_n118_), .O(new_n119_));
  and2   g086(.a(x03), .b(x01), .O(new_n120_));
  nand3  g087(.a(x20), .b(x14), .c(x11), .O(new_n121_));
  inv1   g088(.a(new_n121_), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n120_), .c(x06), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n119_), .O(z2));
  inv1   g091(.a(x15), .O(new_n125_));
  nand4  g092(.a(new_n122_), .b(new_n120_), .c(x08), .d(x06), .O(new_n126_));
  nor2   g093(.a(x14), .b(x11), .O(new_n127_));
  nor2   g094(.a(x20), .b(x16), .O(new_n128_));
  nand4  g095(.a(new_n128_), .b(new_n97_), .c(new_n127_), .d(new_n50_), .O(new_n129_));
  oai22  g096(.a(new_n129_), .b(new_n95_), .c(new_n126_), .d(new_n125_), .O(new_n130_));
  nand2  g097(.a(new_n130_), .b(new_n43_), .O(new_n131_));
  aoi21  g098(.a(new_n74_), .b(new_n73_), .c(new_n126_), .O(new_n132_));
  aoi21  g099(.a(new_n56_), .b(new_n55_), .c(new_n129_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n132_), .c(new_n36_), .O(new_n134_));
  inv1   g101(.a(x07), .O(new_n135_));
  oai22  g102(.a(new_n129_), .b(new_n135_), .c(new_n126_), .d(new_n109_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n85_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n134_), .c(new_n131_), .O(z3));
  inv1   g105(.a(new_n106_), .O(new_n139_));
  oai21  g106(.a(new_n65_), .b(x04), .c(x17), .O(new_n140_));
  aoi21  g107(.a(new_n140_), .b(x22), .c(new_n63_), .O(new_n141_));
  oai21  g108(.a(new_n141_), .b(new_n34_), .c(x08), .O(new_n142_));
  nand2  g109(.a(new_n142_), .b(new_n118_), .O(new_n143_));
  oai21  g110(.a(x23), .b(new_n68_), .c(new_n59_), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n64_), .c(x09), .O(new_n145_));
  oai21  g112(.a(new_n145_), .b(x16), .c(new_n62_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n48_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n143_), .c(new_n139_), .O(z4));
  aoi21  g115(.a(new_n47_), .b(new_n39_), .c(new_n106_), .O(z5));
  inv1   g116(.a(x11), .O(new_n150_));
  aoi21  g117(.a(new_n60_), .b(x14), .c(new_n49_), .O(new_n151_));
  oai21  g118(.a(new_n151_), .b(new_n150_), .c(x03), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n118_), .O(new_n153_));
  inv1   g120(.a(x03), .O(new_n154_));
  aoi21  g121(.a(x20), .b(new_n58_), .c(x06), .O(new_n155_));
  oai21  g122(.a(new_n155_), .b(x11), .c(new_n154_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n48_), .O(new_n157_));
  nand3  g124(.a(new_n157_), .b(new_n153_), .c(new_n139_), .O(z6));
  nor2   g125(.a(new_n119_), .b(new_n106_), .O(z7));
endmodule


