// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(new_n38_), .O(new_n39_));
  inv1   g006(.a(x02), .O(new_n40_));
  inv1   g007(.a(x10), .O(new_n41_));
  nand4  g008(.a(new_n36_), .b(new_n41_), .c(new_n40_), .d(x00), .O(new_n42_));
  nor2   g009(.a(x13), .b(x05), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x19), .O(new_n44_));
  nand3  g011(.a(x24), .b(x13), .c(x07), .O(new_n45_));
  nand3  g012(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(new_n46_));
  inv1   g013(.a(new_n46_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n39_), .O(new_n48_));
  inv1   g015(.a(new_n48_), .O(new_n49_));
  nor2   g016(.a(x20), .b(x14), .O(new_n50_));
  nor2   g017(.a(x11), .b(x06), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g019(.a(new_n52_), .O(new_n53_));
  nor2   g020(.a(x03), .b(x01), .O(new_n54_));
  nand2  g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n49_), .O(z0));
  nand3  g023(.a(new_n41_), .b(new_n40_), .c(x00), .O(new_n57_));
  nand3  g024(.a(x19), .b(x13), .c(x05), .O(new_n58_));
  nand2  g025(.a(new_n54_), .b(new_n51_), .O(new_n59_));
  aoi21  g026(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nor3   g027(.a(x20), .b(x14), .c(x12), .O(new_n61_));
  inv1   g028(.a(x16), .O(new_n62_));
  inv1   g029(.a(x17), .O(new_n63_));
  inv1   g030(.a(x22), .O(new_n64_));
  inv1   g031(.a(x23), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  inv1   g033(.a(x04), .O(new_n67_));
  nor2   g034(.a(x09), .b(x08), .O(new_n68_));
  nand2  g035(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n61_), .c(new_n60_), .O(new_n71_));
  nand3  g038(.a(x23), .b(x22), .c(x17), .O(new_n72_));
  inv1   g039(.a(new_n72_), .O(new_n73_));
  nand3  g040(.a(x16), .b(x14), .c(x12), .O(new_n74_));
  nand3  g041(.a(x11), .b(x09), .c(x08), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand4  g043(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n77_));
  nand3  g044(.a(x21), .b(new_n41_), .c(new_n40_), .O(new_n78_));
  nand3  g045(.a(x15), .b(x13), .c(x05), .O(new_n79_));
  aoi21  g046(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nand4  g047(.a(new_n80_), .b(new_n76_), .c(new_n73_), .d(x20), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n36_), .O(new_n83_));
  nor2   g050(.a(new_n43_), .b(new_n36_), .O(new_n84_));
  nor2   g051(.a(new_n77_), .b(new_n75_), .O(new_n85_));
  nor2   g052(.a(new_n74_), .b(new_n72_), .O(new_n86_));
  nand4  g053(.a(new_n86_), .b(new_n85_), .c(x20), .d(x18), .O(new_n87_));
  inv1   g054(.a(new_n66_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n54_), .c(new_n67_), .O(new_n89_));
  nand4  g056(.a(new_n68_), .b(new_n61_), .c(new_n51_), .d(x07), .O(new_n90_));
  oai21  g057(.a(new_n90_), .b(new_n89_), .c(new_n87_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  and2   g059(.a(x20), .b(x15), .O(new_n93_));
  nand3  g060(.a(new_n93_), .b(new_n86_), .c(new_n85_), .O(new_n94_));
  inv1   g061(.a(x19), .O(new_n95_));
  nor2   g062(.a(new_n95_), .b(x12), .O(new_n96_));
  nand3  g063(.a(new_n96_), .b(new_n68_), .c(new_n53_), .O(new_n97_));
  oai21  g064(.a(new_n97_), .b(new_n89_), .c(new_n94_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n43_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n92_), .c(new_n83_), .O(z1));
  inv1   g067(.a(x20), .O(new_n101_));
  nand3  g068(.a(x06), .b(x03), .c(x01), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand2  g070(.a(x24), .b(x18), .O(new_n104_));
  nand3  g071(.a(new_n36_), .b(x15), .c(x13), .O(new_n105_));
  aoi21  g072(.a(new_n105_), .b(new_n104_), .c(new_n34_), .O(new_n106_));
  inv1   g073(.a(new_n106_), .O(new_n107_));
  nand4  g074(.a(new_n36_), .b(x21), .c(new_n41_), .d(new_n40_), .O(new_n108_));
  nand3  g075(.a(x24), .b(x18), .c(x13), .O(new_n109_));
  nand2  g076(.a(new_n43_), .b(x15), .O(new_n110_));
  nand3  g077(.a(new_n110_), .b(new_n109_), .c(new_n108_), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n107_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n103_), .c(x11), .O(new_n114_));
  aoi21  g081(.a(new_n114_), .b(x14), .c(new_n101_), .O(z2));
  nor2   g082(.a(x14), .b(x08), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n60_), .O(new_n117_));
  inv1   g084(.a(x08), .O(new_n118_));
  inv1   g085(.a(x11), .O(new_n119_));
  nor3   g086(.a(new_n102_), .b(new_n119_), .c(new_n118_), .O(new_n120_));
  aoi21  g087(.a(new_n79_), .b(new_n78_), .c(new_n101_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n117_), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n36_), .O(new_n124_));
  nand3  g091(.a(new_n120_), .b(x20), .c(x18), .O(new_n125_));
  inv1   g092(.a(new_n59_), .O(new_n126_));
  nand3  g093(.a(new_n116_), .b(new_n126_), .c(x07), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n84_), .O(new_n129_));
  nor2   g096(.a(new_n101_), .b(x14), .O(new_n130_));
  nand2  g097(.a(new_n120_), .b(new_n93_), .O(new_n131_));
  nand3  g098(.a(new_n116_), .b(new_n126_), .c(x19), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(new_n43_), .c(new_n130_), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n129_), .c(new_n124_), .O(z3));
  oai21  g102(.a(x23), .b(new_n67_), .c(new_n63_), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(new_n64_), .c(x09), .O(new_n137_));
  oai21  g104(.a(new_n137_), .b(x16), .c(new_n118_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n48_), .O(new_n139_));
  inv1   g106(.a(x09), .O(new_n140_));
  oai21  g107(.a(new_n65_), .b(x04), .c(x17), .O(new_n141_));
  aoi21  g108(.a(new_n141_), .b(x22), .c(new_n140_), .O(new_n142_));
  oai21  g109(.a(new_n142_), .b(new_n62_), .c(x08), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n113_), .O(new_n144_));
  aoi21  g111(.a(new_n144_), .b(new_n139_), .c(new_n130_), .O(z4));
  nor2   g112(.a(new_n130_), .b(new_n49_), .O(z5));
  inv1   g113(.a(new_n130_), .O(new_n147_));
  inv1   g114(.a(x03), .O(new_n148_));
  inv1   g115(.a(x06), .O(new_n149_));
  oai21  g116(.a(x11), .b(new_n149_), .c(new_n148_), .O(new_n150_));
  oai21  g117(.a(new_n46_), .b(new_n38_), .c(new_n150_), .O(new_n151_));
  oai21  g118(.a(new_n119_), .b(x06), .c(x03), .O(new_n152_));
  oai21  g119(.a(new_n111_), .b(new_n106_), .c(new_n152_), .O(new_n153_));
  nand2  g120(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g121(.a(new_n154_), .b(new_n147_), .O(new_n155_));
  nand4  g122(.a(new_n113_), .b(new_n101_), .c(x14), .d(x11), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n155_), .O(z6));
  aoi21  g124(.a(new_n112_), .b(new_n107_), .c(new_n130_), .O(z7));
endmodule


