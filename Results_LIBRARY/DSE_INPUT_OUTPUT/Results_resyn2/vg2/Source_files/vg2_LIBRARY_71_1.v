// Benchmark "FAU" written by ABC on Tue Aug 11 20:48:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_;
  inv1   g000(.a(x13), .O(new_n34_));
  nor2   g001(.a(new_n34_), .b(x05), .O(new_n35_));
  inv1   g002(.a(new_n35_), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x10), .O(new_n38_));
  nand3  g005(.a(new_n38_), .b(new_n37_), .c(x00), .O(new_n39_));
  nand2  g006(.a(x19), .b(x13), .O(new_n40_));
  aoi21  g007(.a(new_n40_), .b(new_n39_), .c(x24), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n36_), .O(new_n42_));
  inv1   g009(.a(x05), .O(new_n43_));
  nand2  g010(.a(new_n34_), .b(new_n43_), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  nand3  g012(.a(x24), .b(x07), .c(x05), .O(new_n46_));
  inv1   g013(.a(new_n46_), .O(new_n47_));
  aoi21  g014(.a(new_n45_), .b(x19), .c(new_n47_), .O(new_n48_));
  nor3   g015(.a(x20), .b(x14), .c(x06), .O(new_n49_));
  nor3   g016(.a(x11), .b(x03), .c(x01), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g018(.a(new_n48_), .b(new_n42_), .c(new_n51_), .O(z0));
  nand2  g019(.a(new_n48_), .b(new_n42_), .O(z5));
  nor2   g020(.a(x03), .b(x01), .O(new_n54_));
  nor2   g021(.a(x11), .b(x09), .O(new_n55_));
  nor2   g022(.a(x16), .b(x12), .O(new_n56_));
  nor2   g023(.a(x08), .b(x04), .O(new_n57_));
  nand4  g024(.a(new_n57_), .b(new_n56_), .c(new_n55_), .d(new_n54_), .O(new_n58_));
  inv1   g025(.a(x17), .O(new_n59_));
  inv1   g026(.a(x22), .O(new_n60_));
  inv1   g027(.a(x23), .O(new_n61_));
  nand4  g028(.a(new_n49_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n62_));
  nor2   g029(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g030(.a(new_n63_), .b(z5), .O(new_n64_));
  nand4  g031(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n65_));
  nand4  g032(.a(x12), .b(x11), .c(x09), .d(x04), .O(new_n66_));
  nor2   g033(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g034(.a(x16), .b(x14), .O(new_n68_));
  nand4  g035(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n69_));
  nor2   g036(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n67_), .c(x15), .O(new_n71_));
  nand2  g038(.a(new_n71_), .b(new_n34_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  inv1   g040(.a(x24), .O(new_n74_));
  nand2  g041(.a(x15), .b(x13), .O(new_n75_));
  nand3  g042(.a(x21), .b(new_n38_), .c(new_n37_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g044(.a(x24), .b(x18), .c(x05), .O(new_n78_));
  inv1   g045(.a(new_n78_), .O(new_n79_));
  aoi21  g046(.a(new_n77_), .b(new_n74_), .c(new_n79_), .O(new_n80_));
  inv1   g047(.a(new_n80_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n70_), .c(new_n67_), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n73_), .c(new_n64_), .O(z1));
  nand3  g050(.a(x06), .b(x03), .c(x01), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand3  g052(.a(new_n77_), .b(new_n36_), .c(new_n74_), .O(new_n86_));
  inv1   g053(.a(x15), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(x05), .O(new_n88_));
  aoi21  g055(.a(new_n88_), .b(new_n34_), .c(new_n79_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand3  g057(.a(x20), .b(x14), .c(x11), .O(new_n91_));
  inv1   g058(.a(new_n91_), .O(new_n92_));
  nand3  g059(.a(new_n92_), .b(new_n90_), .c(new_n85_), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(z2));
  inv1   g061(.a(new_n65_), .O(new_n95_));
  nand3  g062(.a(new_n92_), .b(new_n90_), .c(new_n95_), .O(new_n96_));
  inv1   g063(.a(x08), .O(new_n97_));
  nand4  g064(.a(new_n50_), .b(new_n49_), .c(x19), .d(new_n97_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n34_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nor2   g067(.a(new_n51_), .b(x08), .O(new_n101_));
  oai21  g068(.a(new_n47_), .b(new_n41_), .c(new_n101_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n100_), .c(new_n96_), .O(z3));
  inv1   g070(.a(x16), .O(new_n104_));
  inv1   g071(.a(x09), .O(new_n105_));
  oai21  g072(.a(new_n61_), .b(x04), .c(x17), .O(new_n106_));
  aoi21  g073(.a(new_n106_), .b(x22), .c(new_n105_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n104_), .c(x08), .O(new_n108_));
  inv1   g075(.a(new_n88_), .O(new_n109_));
  nand2  g076(.a(new_n109_), .b(new_n80_), .O(new_n110_));
  nand2  g077(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g078(.a(z5), .b(x08), .O(new_n112_));
  inv1   g079(.a(new_n39_), .O(new_n113_));
  nand3  g080(.a(new_n113_), .b(new_n74_), .c(new_n97_), .O(new_n114_));
  nand2  g081(.a(new_n74_), .b(x13), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n44_), .O(new_n116_));
  nand2  g083(.a(new_n74_), .b(x08), .O(new_n117_));
  nand3  g084(.a(new_n117_), .b(new_n116_), .c(x19), .O(new_n118_));
  nand3  g085(.a(new_n118_), .b(new_n114_), .c(new_n46_), .O(new_n119_));
  inv1   g086(.a(x04), .O(new_n120_));
  oai21  g087(.a(x23), .b(new_n120_), .c(new_n59_), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n60_), .O(new_n122_));
  aoi21  g089(.a(new_n122_), .b(new_n105_), .c(x16), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(new_n119_), .c(new_n35_), .O(new_n124_));
  nand3  g091(.a(new_n124_), .b(new_n112_), .c(new_n111_), .O(z4));
  inv1   g092(.a(x03), .O(new_n126_));
  inv1   g093(.a(x14), .O(new_n127_));
  aoi21  g094(.a(x20), .b(new_n127_), .c(x06), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(x11), .c(new_n126_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(z5), .O(new_n130_));
  inv1   g097(.a(x11), .O(new_n131_));
  inv1   g098(.a(x06), .O(new_n132_));
  inv1   g099(.a(x20), .O(new_n133_));
  aoi21  g100(.a(new_n133_), .b(x14), .c(new_n132_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n131_), .c(x03), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n90_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n130_), .O(z6));
  oai21  g104(.a(x15), .b(x13), .c(new_n43_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n80_), .O(z7));
endmodule


