// Benchmark "FAU" written by ABC on Tue Aug 11 20:50:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g004(.a(new_n37_), .b(x05), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  inv1   g008(.a(new_n41_), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x13), .c(x07), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  nor2   g012(.a(x13), .b(x05), .O(new_n46_));
  aoi21  g013(.a(new_n46_), .b(x19), .c(new_n45_), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n43_), .c(new_n38_), .O(new_n48_));
  inv1   g015(.a(new_n48_), .O(new_n49_));
  inv1   g016(.a(x06), .O(new_n50_));
  nor3   g017(.a(x20), .b(x14), .c(x11), .O(new_n51_));
  nor2   g018(.a(x03), .b(x01), .O(new_n52_));
  inv1   g019(.a(x17), .O(new_n53_));
  nand2  g020(.a(x23), .b(new_n53_), .O(new_n54_));
  nand4  g021(.a(new_n54_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n55_));
  nor2   g022(.a(new_n55_), .b(new_n49_), .O(z0));
  nand4  g023(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n57_));
  nand3  g024(.a(x11), .b(x09), .c(x04), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g026(.a(x23), .b(x22), .c(x20), .O(new_n60_));
  nand3  g027(.a(x16), .b(x14), .c(x12), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g029(.a(x15), .O(new_n63_));
  nand3  g030(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n64_));
  nand2  g031(.a(x13), .b(x05), .O(new_n65_));
  oai21  g032(.a(new_n65_), .b(new_n63_), .c(new_n64_), .O(new_n66_));
  nand3  g033(.a(new_n66_), .b(new_n62_), .c(new_n59_), .O(new_n67_));
  inv1   g034(.a(x16), .O(new_n68_));
  nor2   g035(.a(x12), .b(x11), .O(new_n69_));
  nor2   g036(.a(x22), .b(x20), .O(new_n70_));
  nand4  g037(.a(new_n70_), .b(new_n69_), .c(new_n53_), .d(new_n68_), .O(new_n71_));
  inv1   g038(.a(x14), .O(new_n72_));
  nor2   g039(.a(x09), .b(x08), .O(new_n73_));
  nor2   g040(.a(x06), .b(x04), .O(new_n74_));
  nand4  g041(.a(new_n74_), .b(new_n73_), .c(new_n52_), .d(new_n72_), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  inv1   g043(.a(x19), .O(new_n77_));
  oai21  g044(.a(new_n65_), .b(new_n77_), .c(new_n41_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n67_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n35_), .O(new_n81_));
  nand3  g048(.a(new_n62_), .b(new_n59_), .c(x18), .O(new_n82_));
  inv1   g049(.a(new_n82_), .O(new_n83_));
  inv1   g050(.a(x07), .O(new_n84_));
  nor3   g051(.a(new_n75_), .b(new_n71_), .c(new_n84_), .O(new_n85_));
  nor2   g052(.a(new_n46_), .b(new_n35_), .O(new_n86_));
  oai21  g053(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  inv1   g054(.a(new_n54_), .O(new_n88_));
  nand4  g055(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n89_));
  nor2   g056(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n59_), .O(new_n91_));
  nor2   g058(.a(x08), .b(x06), .O(new_n92_));
  nor2   g059(.a(x09), .b(x04), .O(new_n93_));
  nor2   g060(.a(new_n77_), .b(x14), .O(new_n94_));
  nand4  g061(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(new_n52_), .O(new_n95_));
  oai21  g062(.a(new_n95_), .b(new_n71_), .c(new_n91_), .O(new_n96_));
  aoi21  g063(.a(new_n96_), .b(new_n46_), .c(new_n88_), .O(new_n97_));
  nand3  g064(.a(new_n97_), .b(new_n87_), .c(new_n81_), .O(z1));
  and2   g065(.a(x03), .b(x01), .O(new_n99_));
  nand2  g066(.a(x24), .b(x18), .O(new_n100_));
  nand3  g067(.a(new_n35_), .b(x15), .c(x13), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(x05), .O(new_n103_));
  inv1   g070(.a(new_n64_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n35_), .O(new_n105_));
  inv1   g072(.a(new_n100_), .O(new_n106_));
  aoi22  g073(.a(new_n106_), .b(x13), .c(new_n46_), .d(x15), .O(new_n107_));
  nand3  g074(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nand3  g075(.a(x20), .b(x14), .c(x11), .O(new_n109_));
  inv1   g076(.a(new_n109_), .O(new_n110_));
  nand4  g077(.a(new_n110_), .b(new_n108_), .c(new_n99_), .d(x06), .O(new_n111_));
  nand2  g078(.a(new_n111_), .b(new_n54_), .O(z2));
  nor2   g079(.a(new_n109_), .b(new_n57_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n66_), .O(new_n114_));
  nand2  g081(.a(new_n92_), .b(new_n52_), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand3  g083(.a(new_n116_), .b(new_n78_), .c(new_n51_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n35_), .O(new_n119_));
  nand2  g086(.a(new_n113_), .b(x18), .O(new_n120_));
  nand4  g087(.a(new_n92_), .b(new_n52_), .c(new_n51_), .d(x07), .O(new_n121_));
  nand2  g088(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand2  g089(.a(new_n113_), .b(x15), .O(new_n123_));
  nand4  g090(.a(new_n92_), .b(new_n52_), .c(new_n51_), .d(x19), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi22  g092(.a(new_n125_), .b(new_n46_), .c(new_n122_), .d(new_n86_), .O(new_n126_));
  aoi21  g093(.a(new_n126_), .b(new_n119_), .c(new_n88_), .O(z3));
  inv1   g094(.a(x08), .O(new_n128_));
  inv1   g095(.a(x04), .O(new_n129_));
  aoi21  g096(.a(new_n53_), .b(new_n129_), .c(x22), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(x09), .c(new_n68_), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(new_n48_), .c(new_n108_), .O(new_n133_));
  inv1   g100(.a(x23), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(x04), .c(x17), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(x22), .O(new_n136_));
  aoi21  g103(.a(new_n136_), .b(x09), .c(new_n68_), .O(new_n137_));
  nand4  g104(.a(new_n47_), .b(new_n43_), .c(new_n38_), .d(x08), .O(new_n138_));
  oai21  g105(.a(new_n138_), .b(new_n137_), .c(new_n54_), .O(new_n139_));
  nor2   g106(.a(new_n139_), .b(new_n133_), .O(z4));
  nand2  g107(.a(new_n54_), .b(new_n49_), .O(z5));
  nand2  g108(.a(x20), .b(new_n72_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n50_), .c(x11), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(x03), .c(new_n48_), .O(new_n144_));
  inv1   g111(.a(x11), .O(new_n145_));
  inv1   g112(.a(x20), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x14), .c(new_n50_), .O(new_n147_));
  oai21  g114(.a(new_n147_), .b(new_n145_), .c(x03), .O(new_n148_));
  nand2  g115(.a(new_n148_), .b(new_n108_), .O(new_n149_));
  aoi21  g116(.a(new_n149_), .b(new_n144_), .c(new_n88_), .O(z6));
  or2    g117(.a(new_n108_), .b(new_n88_), .O(z7));
endmodule


