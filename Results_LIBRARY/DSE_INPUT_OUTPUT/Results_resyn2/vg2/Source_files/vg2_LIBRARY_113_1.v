// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_;
  inv1   g000(.a(x24), .O(new_n34_));
  inv1   g001(.a(x05), .O(new_n35_));
  nand2  g002(.a(x13), .b(new_n35_), .O(new_n36_));
  inv1   g003(.a(x13), .O(new_n37_));
  inv1   g004(.a(x19), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  oai21  g008(.a(new_n38_), .b(new_n37_), .c(new_n41_), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(new_n36_), .c(new_n34_), .O(new_n43_));
  nand3  g010(.a(x24), .b(x07), .c(x05), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  nor2   g012(.a(new_n38_), .b(x05), .O(new_n46_));
  aoi21  g013(.a(new_n46_), .b(new_n37_), .c(new_n45_), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  inv1   g016(.a(x11), .O(new_n50_));
  inv1   g017(.a(x14), .O(new_n51_));
  inv1   g018(.a(x20), .O(new_n52_));
  nand4  g019(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor3   g020(.a(new_n53_), .b(x03), .c(x01), .O(new_n54_));
  and2   g021(.a(new_n54_), .b(new_n48_), .O(z0));
  nand3  g022(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n56_));
  nor2   g023(.a(x17), .b(x16), .O(new_n57_));
  nor2   g024(.a(x23), .b(x22), .O(new_n58_));
  nor2   g025(.a(x09), .b(x04), .O(new_n59_));
  nor2   g026(.a(x12), .b(x11), .O(new_n60_));
  nand4  g027(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  inv1   g028(.a(x01), .O(new_n62_));
  inv1   g029(.a(x03), .O(new_n63_));
  inv1   g030(.a(x08), .O(new_n64_));
  nand3  g031(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(new_n65_));
  nor3   g032(.a(new_n65_), .b(new_n61_), .c(new_n56_), .O(new_n66_));
  nand2  g033(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  inv1   g034(.a(x15), .O(new_n68_));
  nand4  g035(.a(x16), .b(x12), .c(x11), .d(x09), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand4  g037(.a(x20), .b(x14), .c(x08), .d(x04), .O(new_n71_));
  inv1   g038(.a(new_n71_), .O(new_n72_));
  nand3  g039(.a(x06), .b(x03), .c(x01), .O(new_n73_));
  nand3  g040(.a(x23), .b(x22), .c(x17), .O(new_n74_));
  nor2   g041(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g042(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  oai21  g043(.a(new_n76_), .b(new_n68_), .c(new_n37_), .O(new_n77_));
  nand2  g044(.a(new_n77_), .b(new_n35_), .O(new_n78_));
  inv1   g045(.a(new_n76_), .O(new_n79_));
  nand3  g046(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n80_));
  oai21  g047(.a(new_n68_), .b(new_n37_), .c(new_n80_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n34_), .O(new_n82_));
  nand3  g049(.a(x24), .b(x18), .c(x05), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  nand3  g052(.a(new_n85_), .b(new_n78_), .c(new_n67_), .O(z1));
  nand3  g053(.a(x20), .b(x14), .c(x11), .O(new_n87_));
  nor3   g054(.a(new_n87_), .b(new_n73_), .c(new_n68_), .O(new_n88_));
  oai21  g055(.a(new_n88_), .b(x13), .c(new_n35_), .O(new_n89_));
  nor2   g056(.a(new_n87_), .b(new_n73_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n84_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n89_), .O(z2));
  nand3  g059(.a(new_n90_), .b(new_n81_), .c(x08), .O(new_n93_));
  nor2   g060(.a(new_n65_), .b(new_n53_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n42_), .O(new_n95_));
  nand2  g062(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand2  g063(.a(new_n96_), .b(new_n34_), .O(new_n97_));
  nand2  g064(.a(x15), .b(new_n35_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n83_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n90_), .c(x08), .O(new_n100_));
  oai21  g067(.a(new_n38_), .b(x05), .c(new_n44_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n94_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n100_), .c(new_n36_), .O(new_n103_));
  inv1   g070(.a(new_n103_), .O(new_n104_));
  nand2  g071(.a(new_n104_), .b(new_n97_), .O(z3));
  inv1   g072(.a(x22), .O(new_n106_));
  inv1   g073(.a(x04), .O(new_n107_));
  inv1   g074(.a(x17), .O(new_n108_));
  oai21  g075(.a(x23), .b(new_n107_), .c(new_n108_), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(new_n106_), .c(x09), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(x16), .c(new_n64_), .O(new_n111_));
  nand2  g078(.a(new_n42_), .b(new_n34_), .O(new_n112_));
  inv1   g079(.a(new_n101_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n111_), .O(new_n115_));
  inv1   g082(.a(x16), .O(new_n116_));
  inv1   g083(.a(x09), .O(new_n117_));
  inv1   g084(.a(x23), .O(new_n118_));
  oai21  g085(.a(new_n118_), .b(x04), .c(x17), .O(new_n119_));
  aoi21  g086(.a(new_n119_), .b(x22), .c(new_n117_), .O(new_n120_));
  oai21  g087(.a(new_n120_), .b(new_n116_), .c(x08), .O(new_n121_));
  nand3  g088(.a(new_n98_), .b(new_n83_), .c(new_n82_), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g090(.a(new_n123_), .b(new_n115_), .c(new_n36_), .O(z4));
  nand2  g091(.a(new_n38_), .b(new_n37_), .O(new_n125_));
  aoi21  g092(.a(new_n125_), .b(new_n35_), .c(new_n45_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n112_), .O(z5));
  aoi21  g094(.a(new_n52_), .b(x14), .c(new_n49_), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(new_n50_), .c(x03), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n122_), .O(new_n130_));
  aoi21  g097(.a(x20), .b(new_n51_), .c(x06), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(x11), .c(new_n63_), .O(new_n132_));
  nand2  g099(.a(new_n132_), .b(new_n114_), .O(new_n133_));
  nand3  g100(.a(new_n133_), .b(new_n130_), .c(new_n36_), .O(z6));
  oai21  g101(.a(x15), .b(x13), .c(new_n35_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n83_), .c(new_n82_), .O(z7));
endmodule


