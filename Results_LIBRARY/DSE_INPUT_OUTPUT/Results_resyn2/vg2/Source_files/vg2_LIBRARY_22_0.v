// Benchmark "FAU" written by ABC on Tue Aug 11 20:48:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_;
  nor2   g000(.a(x13), .b(x05), .O(new_n34_));
  inv1   g001(.a(new_n34_), .O(new_n35_));
  nand2  g002(.a(x24), .b(x07), .O(new_n36_));
  inv1   g003(.a(x02), .O(new_n37_));
  inv1   g004(.a(x10), .O(new_n38_));
  inv1   g005(.a(x24), .O(new_n39_));
  nand4  g006(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(x00), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nand2  g008(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand3  g009(.a(x19), .b(x13), .c(x05), .O(new_n43_));
  inv1   g010(.a(new_n43_), .O(new_n44_));
  nand2  g011(.a(new_n44_), .b(new_n39_), .O(new_n45_));
  nand2  g012(.a(new_n45_), .b(new_n42_), .O(z5));
  inv1   g013(.a(x11), .O(new_n47_));
  inv1   g014(.a(x14), .O(new_n48_));
  inv1   g015(.a(x20), .O(new_n49_));
  nand3  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nor4   g017(.a(new_n50_), .b(x06), .c(x03), .d(x01), .O(new_n51_));
  and2   g018(.a(new_n51_), .b(z5), .O(z0));
  inv1   g019(.a(x01), .O(new_n53_));
  inv1   g020(.a(x03), .O(new_n54_));
  inv1   g021(.a(x06), .O(new_n55_));
  inv1   g022(.a(x08), .O(new_n56_));
  nand4  g023(.a(new_n56_), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n57_));
  inv1   g024(.a(x04), .O(new_n58_));
  inv1   g025(.a(x09), .O(new_n59_));
  nand2  g026(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g028(.a(new_n40_), .b(new_n36_), .c(new_n50_), .O(new_n62_));
  nor2   g029(.a(x16), .b(x12), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  inv1   g031(.a(x17), .O(new_n65_));
  inv1   g032(.a(x22), .O(new_n66_));
  inv1   g033(.a(x23), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  nor2   g035(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n62_), .c(new_n61_), .O(new_n70_));
  nand2  g037(.a(x24), .b(x18), .O(new_n71_));
  nand4  g038(.a(new_n39_), .b(x21), .c(new_n38_), .d(new_n37_), .O(new_n72_));
  nand2  g039(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g040(.a(x20), .b(x14), .c(x11), .O(new_n74_));
  nand4  g041(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n75_));
  nor2   g042(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g043(.a(x23), .b(x22), .c(x17), .O(new_n77_));
  nand4  g044(.a(x16), .b(x12), .c(x09), .d(x04), .O(new_n78_));
  nor2   g045(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g046(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n35_), .O(new_n82_));
  inv1   g049(.a(x05), .O(new_n83_));
  inv1   g050(.a(x13), .O(new_n84_));
  nor3   g051(.a(x24), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  inv1   g052(.a(new_n57_), .O(new_n86_));
  nand3  g053(.a(new_n86_), .b(new_n59_), .c(new_n58_), .O(new_n87_));
  inv1   g054(.a(new_n50_), .O(new_n88_));
  inv1   g055(.a(new_n68_), .O(new_n89_));
  nand4  g056(.a(new_n89_), .b(new_n63_), .c(new_n88_), .d(x19), .O(new_n90_));
  nand3  g057(.a(new_n79_), .b(new_n76_), .c(x15), .O(new_n91_));
  oai21  g058(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n82_), .O(z1));
  nand2  g061(.a(new_n85_), .b(x15), .O(new_n95_));
  nand3  g062(.a(new_n95_), .b(new_n72_), .c(new_n71_), .O(new_n96_));
  nand3  g063(.a(x06), .b(x03), .c(x01), .O(new_n97_));
  nor2   g064(.a(new_n97_), .b(new_n74_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n35_), .O(z2));
  nand2  g067(.a(new_n76_), .b(new_n73_), .O(new_n101_));
  nand2  g068(.a(new_n62_), .b(new_n86_), .O(new_n102_));
  nand2  g069(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n35_), .O(new_n104_));
  and2   g071(.a(new_n76_), .b(x15), .O(new_n105_));
  inv1   g072(.a(x19), .O(new_n106_));
  nor3   g073(.a(new_n57_), .b(new_n50_), .c(new_n106_), .O(new_n107_));
  oai21  g074(.a(new_n107_), .b(new_n105_), .c(new_n85_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n104_), .O(z3));
  nand2  g076(.a(new_n38_), .b(new_n37_), .O(new_n110_));
  oai21  g077(.a(x13), .b(x05), .c(x00), .O(new_n111_));
  oai21  g078(.a(new_n111_), .b(new_n110_), .c(new_n43_), .O(new_n112_));
  nand2  g079(.a(new_n112_), .b(new_n39_), .O(new_n113_));
  nand3  g080(.a(x24), .b(new_n56_), .c(x07), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  oai21  g082(.a(x23), .b(new_n58_), .c(new_n65_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n66_), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(new_n59_), .c(x16), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  inv1   g086(.a(x16), .O(new_n120_));
  oai21  g087(.a(new_n67_), .b(x04), .c(x17), .O(new_n121_));
  aoi21  g088(.a(new_n121_), .b(x22), .c(new_n59_), .O(new_n122_));
  oai21  g089(.a(new_n122_), .b(new_n120_), .c(x08), .O(new_n123_));
  nand2  g090(.a(new_n123_), .b(new_n96_), .O(new_n124_));
  aoi21  g091(.a(z5), .b(x08), .c(new_n34_), .O(new_n125_));
  nand3  g092(.a(new_n125_), .b(new_n124_), .c(new_n119_), .O(z4));
  aoi21  g093(.a(x20), .b(new_n48_), .c(x06), .O(new_n127_));
  oai21  g094(.a(new_n127_), .b(x11), .c(new_n54_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(z5), .O(new_n129_));
  aoi21  g096(.a(new_n49_), .b(x14), .c(new_n55_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n47_), .c(x03), .O(new_n131_));
  nand2  g098(.a(new_n84_), .b(new_n83_), .O(new_n132_));
  nand3  g099(.a(new_n132_), .b(new_n131_), .c(new_n96_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n129_), .O(z6));
  inv1   g101(.a(new_n96_), .O(new_n135_));
  nand2  g102(.a(new_n135_), .b(new_n35_), .O(z7));
endmodule


