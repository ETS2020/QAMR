// Benchmark "FAU" written by ABC on Tue Aug 11 20:48:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x24), .b(x07), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  nand3  g003(.a(new_n36_), .b(x19), .c(x13), .O(new_n37_));
  aoi21  g004(.a(new_n37_), .b(new_n35_), .c(new_n34_), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand4  g007(.a(new_n36_), .b(new_n40_), .c(new_n39_), .d(x00), .O(new_n41_));
  nand3  g008(.a(x24), .b(x13), .c(x07), .O(new_n42_));
  nor2   g009(.a(x13), .b(x05), .O(new_n43_));
  nand2  g010(.a(new_n43_), .b(x19), .O(new_n44_));
  nand3  g011(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(new_n45_));
  nor2   g012(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nor2   g013(.a(x03), .b(x01), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  inv1   g015(.a(x06), .O(new_n49_));
  inv1   g016(.a(x20), .O(new_n50_));
  nor2   g017(.a(x14), .b(x11), .O(new_n51_));
  nand3  g018(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nor3   g019(.a(new_n52_), .b(new_n48_), .c(new_n46_), .O(z0));
  inv1   g020(.a(x17), .O(new_n54_));
  inv1   g021(.a(x22), .O(new_n55_));
  inv1   g022(.a(x23), .O(new_n56_));
  nand3  g023(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  inv1   g024(.a(x04), .O(new_n58_));
  inv1   g025(.a(x16), .O(new_n59_));
  nand3  g026(.a(new_n47_), .b(new_n59_), .c(new_n58_), .O(new_n60_));
  nor2   g027(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  inv1   g028(.a(x12), .O(new_n62_));
  nand3  g029(.a(new_n51_), .b(new_n50_), .c(new_n62_), .O(new_n63_));
  inv1   g030(.a(new_n63_), .O(new_n64_));
  nand3  g031(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n65_));
  inv1   g032(.a(x08), .O(new_n66_));
  inv1   g033(.a(x09), .O(new_n67_));
  nand3  g034(.a(new_n67_), .b(new_n66_), .c(new_n49_), .O(new_n68_));
  nand3  g035(.a(x19), .b(x13), .c(x05), .O(new_n69_));
  aoi21  g036(.a(new_n69_), .b(new_n65_), .c(new_n68_), .O(new_n70_));
  nand3  g037(.a(new_n70_), .b(new_n64_), .c(new_n61_), .O(new_n71_));
  nand3  g038(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n72_));
  nand3  g039(.a(x15), .b(x13), .c(x05), .O(new_n73_));
  nand2  g040(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g041(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n75_));
  nand2  g042(.a(x09), .b(x04), .O(new_n76_));
  nor2   g043(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand4  g044(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n78_));
  nand4  g045(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n79_));
  nor2   g046(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g047(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(new_n81_));
  nand2  g048(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand2  g049(.a(new_n82_), .b(new_n36_), .O(new_n83_));
  nor2   g050(.a(new_n43_), .b(new_n36_), .O(new_n84_));
  nor2   g051(.a(new_n78_), .b(new_n62_), .O(new_n85_));
  nand3  g052(.a(x18), .b(x16), .c(x14), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand4  g054(.a(new_n87_), .b(new_n85_), .c(new_n77_), .d(x11), .O(new_n88_));
  nand4  g055(.a(new_n67_), .b(new_n66_), .c(x07), .d(new_n49_), .O(new_n89_));
  inv1   g056(.a(new_n89_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n64_), .c(new_n61_), .O(new_n91_));
  nand2  g058(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n84_), .O(new_n93_));
  nand3  g060(.a(x16), .b(x15), .c(x14), .O(new_n94_));
  inv1   g061(.a(new_n94_), .O(new_n95_));
  nand4  g062(.a(new_n95_), .b(new_n85_), .c(new_n77_), .d(x11), .O(new_n96_));
  nand4  g063(.a(new_n50_), .b(x19), .c(new_n66_), .d(new_n49_), .O(new_n97_));
  nand3  g064(.a(new_n51_), .b(new_n62_), .c(new_n67_), .O(new_n98_));
  nor2   g065(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  nand2  g067(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n43_), .O(new_n102_));
  nand3  g069(.a(new_n102_), .b(new_n93_), .c(new_n83_), .O(z1));
  inv1   g070(.a(x14), .O(new_n104_));
  nand2  g071(.a(x24), .b(x18), .O(new_n105_));
  nand3  g072(.a(new_n36_), .b(x15), .c(x13), .O(new_n106_));
  aoi21  g073(.a(new_n106_), .b(new_n105_), .c(new_n34_), .O(new_n107_));
  inv1   g074(.a(new_n107_), .O(new_n108_));
  nand4  g075(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n109_));
  nand3  g076(.a(x24), .b(x18), .c(x13), .O(new_n110_));
  nand2  g077(.a(new_n43_), .b(x15), .O(new_n111_));
  nand3  g078(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  inv1   g079(.a(new_n112_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n108_), .O(new_n114_));
  nand4  g081(.a(x11), .b(x06), .c(x03), .d(x01), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand2  g083(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  aoi21  g084(.a(new_n117_), .b(x20), .c(new_n104_), .O(z2));
  nand3  g085(.a(x20), .b(x14), .c(x11), .O(new_n119_));
  nor2   g086(.a(new_n119_), .b(new_n75_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n74_), .O(new_n121_));
  nand2  g088(.a(new_n69_), .b(new_n65_), .O(new_n122_));
  nand2  g089(.a(new_n47_), .b(new_n66_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n52_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand2  g093(.a(new_n126_), .b(new_n36_), .O(new_n127_));
  and2   g094(.a(new_n120_), .b(x18), .O(new_n128_));
  inv1   g095(.a(x07), .O(new_n129_));
  nor3   g096(.a(new_n123_), .b(new_n52_), .c(new_n129_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n128_), .c(new_n84_), .O(new_n131_));
  and2   g098(.a(new_n120_), .b(x15), .O(new_n132_));
  nand2  g099(.a(new_n51_), .b(new_n47_), .O(new_n133_));
  nor2   g100(.a(new_n133_), .b(new_n97_), .O(new_n134_));
  oai21  g101(.a(new_n134_), .b(new_n132_), .c(new_n43_), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n131_), .c(new_n127_), .O(z3));
  inv1   g103(.a(new_n46_), .O(new_n137_));
  oai21  g104(.a(x23), .b(new_n58_), .c(new_n54_), .O(new_n138_));
  aoi21  g105(.a(new_n138_), .b(new_n55_), .c(x09), .O(new_n139_));
  oai21  g106(.a(new_n139_), .b(x16), .c(new_n66_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  oai21  g108(.a(new_n56_), .b(x04), .c(x17), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(x22), .c(new_n67_), .O(new_n143_));
  oai21  g110(.a(new_n143_), .b(new_n59_), .c(x08), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n114_), .O(new_n145_));
  nor2   g112(.a(x20), .b(new_n104_), .O(new_n146_));
  inv1   g113(.a(new_n146_), .O(new_n147_));
  nand3  g114(.a(new_n147_), .b(new_n145_), .c(new_n141_), .O(z4));
  nand2  g115(.a(new_n147_), .b(new_n46_), .O(z5));
  inv1   g116(.a(x03), .O(new_n150_));
  oai21  g117(.a(x11), .b(new_n49_), .c(new_n150_), .O(new_n151_));
  oai21  g118(.a(new_n45_), .b(new_n38_), .c(new_n151_), .O(new_n152_));
  inv1   g119(.a(x11), .O(new_n153_));
  oai21  g120(.a(new_n153_), .b(x06), .c(x03), .O(new_n154_));
  oai21  g121(.a(new_n112_), .b(new_n107_), .c(new_n154_), .O(new_n155_));
  nand2  g122(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n147_), .O(new_n157_));
  nand3  g124(.a(new_n51_), .b(new_n137_), .c(x20), .O(new_n158_));
  nand2  g125(.a(new_n158_), .b(new_n157_), .O(z6));
  aoi21  g126(.a(new_n113_), .b(new_n108_), .c(new_n146_), .O(z7));
endmodule


