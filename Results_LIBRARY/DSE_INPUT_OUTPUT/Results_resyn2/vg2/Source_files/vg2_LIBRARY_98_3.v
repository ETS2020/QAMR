// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n167_;
  nor2   g000(.a(x03), .b(x01), .O(new_n34_));
  nor2   g001(.a(x11), .b(x06), .O(new_n35_));
  nor2   g002(.a(x20), .b(x14), .O(new_n36_));
  nand2  g003(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g004(.a(new_n37_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  inv1   g006(.a(x05), .O(new_n40_));
  nand2  g007(.a(x24), .b(x07), .O(new_n41_));
  inv1   g008(.a(x24), .O(new_n42_));
  nand3  g009(.a(new_n42_), .b(x19), .c(x13), .O(new_n43_));
  aoi21  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  inv1   g011(.a(x02), .O(new_n45_));
  inv1   g012(.a(x10), .O(new_n46_));
  nand3  g013(.a(new_n46_), .b(new_n45_), .c(x00), .O(new_n47_));
  inv1   g014(.a(new_n47_), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nor2   g016(.a(x13), .b(x05), .O(new_n50_));
  nand2  g017(.a(new_n50_), .b(x19), .O(new_n51_));
  nand3  g018(.a(x24), .b(x13), .c(x07), .O(new_n52_));
  nand3  g019(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  nor2   g020(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  nand2  g021(.a(x21), .b(x13), .O(new_n55_));
  oai21  g022(.a(new_n54_), .b(new_n39_), .c(new_n55_), .O(z0));
  nand3  g023(.a(x11), .b(x09), .c(x08), .O(new_n57_));
  nand4  g024(.a(x06), .b(x04), .c(x03), .d(x01), .O(new_n58_));
  nor2   g025(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand4  g026(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n60_));
  nand3  g027(.a(x16), .b(x14), .c(x12), .O(new_n61_));
  nor2   g028(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g029(.a(x15), .b(x13), .O(new_n63_));
  nand3  g030(.a(x21), .b(new_n46_), .c(new_n45_), .O(new_n64_));
  oai21  g031(.a(new_n63_), .b(new_n40_), .c(new_n64_), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n62_), .c(new_n59_), .O(new_n66_));
  nand3  g033(.a(x19), .b(x13), .c(x05), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n47_), .O(new_n68_));
  inv1   g035(.a(x12), .O(new_n69_));
  nor2   g036(.a(x09), .b(x08), .O(new_n70_));
  nand2  g037(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nor2   g038(.a(new_n71_), .b(new_n37_), .O(new_n72_));
  inv1   g039(.a(x23), .O(new_n73_));
  nor2   g040(.a(x22), .b(x17), .O(new_n74_));
  nor2   g041(.a(x16), .b(x04), .O(new_n75_));
  nand4  g042(.a(new_n75_), .b(new_n74_), .c(new_n34_), .d(new_n73_), .O(new_n76_));
  inv1   g043(.a(new_n76_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n72_), .c(new_n68_), .O(new_n78_));
  nand2  g045(.a(new_n78_), .b(new_n66_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n42_), .O(new_n80_));
  nor2   g047(.a(new_n50_), .b(new_n42_), .O(new_n81_));
  nand3  g048(.a(new_n62_), .b(new_n59_), .c(x18), .O(new_n82_));
  nand3  g049(.a(new_n77_), .b(new_n72_), .c(x07), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  inv1   g052(.a(new_n55_), .O(new_n86_));
  nand4  g053(.a(x16), .b(x15), .c(x14), .d(x12), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n60_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n59_), .O(new_n89_));
  inv1   g056(.a(x19), .O(new_n90_));
  nor2   g057(.a(new_n90_), .b(x12), .O(new_n91_));
  nand4  g058(.a(new_n91_), .b(new_n70_), .c(new_n36_), .d(new_n35_), .O(new_n92_));
  oai21  g059(.a(new_n92_), .b(new_n76_), .c(new_n89_), .O(new_n93_));
  aoi21  g060(.a(new_n93_), .b(new_n50_), .c(new_n86_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n85_), .c(new_n80_), .O(z1));
  nand3  g062(.a(x06), .b(x03), .c(x01), .O(new_n96_));
  inv1   g063(.a(new_n96_), .O(new_n97_));
  inv1   g064(.a(x13), .O(new_n98_));
  inv1   g065(.a(x15), .O(new_n99_));
  nand4  g066(.a(new_n42_), .b(x21), .c(new_n46_), .d(new_n45_), .O(new_n100_));
  oai21  g067(.a(new_n99_), .b(x05), .c(new_n100_), .O(new_n101_));
  nand2  g068(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand2  g069(.a(x24), .b(x18), .O(new_n103_));
  inv1   g070(.a(x21), .O(new_n104_));
  nand2  g071(.a(new_n42_), .b(new_n104_), .O(new_n105_));
  oai22  g072(.a(new_n105_), .b(new_n63_), .c(new_n103_), .d(x13), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(x05), .O(new_n107_));
  inv1   g074(.a(new_n103_), .O(new_n108_));
  nand3  g075(.a(new_n108_), .b(new_n104_), .c(x13), .O(new_n109_));
  nand3  g076(.a(new_n109_), .b(new_n107_), .c(new_n102_), .O(new_n110_));
  nand3  g077(.a(x20), .b(x14), .c(x11), .O(new_n111_));
  inv1   g078(.a(new_n111_), .O(new_n112_));
  nand3  g079(.a(new_n112_), .b(new_n110_), .c(new_n97_), .O(new_n113_));
  inv1   g080(.a(new_n113_), .O(z2));
  nand2  g081(.a(new_n97_), .b(x08), .O(new_n115_));
  inv1   g082(.a(new_n115_), .O(new_n116_));
  nand3  g083(.a(new_n116_), .b(new_n112_), .c(new_n110_), .O(new_n117_));
  nand4  g084(.a(new_n55_), .b(new_n46_), .c(new_n45_), .d(x00), .O(new_n118_));
  inv1   g085(.a(new_n67_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n104_), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(new_n118_), .c(x24), .O(new_n121_));
  nand3  g088(.a(new_n81_), .b(new_n55_), .c(x07), .O(new_n122_));
  nand2  g089(.a(new_n122_), .b(new_n51_), .O(new_n123_));
  inv1   g090(.a(x08), .O(new_n124_));
  nand3  g091(.a(new_n38_), .b(new_n34_), .c(new_n124_), .O(new_n125_));
  inv1   g092(.a(new_n125_), .O(new_n126_));
  oai21  g093(.a(new_n123_), .b(new_n121_), .c(new_n126_), .O(new_n127_));
  nand2  g094(.a(new_n127_), .b(new_n117_), .O(z3));
  inv1   g095(.a(x16), .O(new_n129_));
  inv1   g096(.a(x09), .O(new_n130_));
  oai21  g097(.a(new_n73_), .b(x04), .c(x17), .O(new_n131_));
  aoi21  g098(.a(new_n131_), .b(x22), .c(new_n130_), .O(new_n132_));
  oai21  g099(.a(new_n132_), .b(new_n129_), .c(x08), .O(new_n133_));
  nand2  g100(.a(new_n108_), .b(x13), .O(new_n134_));
  nand2  g101(.a(new_n50_), .b(x15), .O(new_n135_));
  and2   g102(.a(new_n135_), .b(new_n100_), .O(new_n136_));
  oai21  g103(.a(new_n63_), .b(x24), .c(new_n103_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(x05), .O(new_n138_));
  nand3  g105(.a(new_n138_), .b(new_n136_), .c(new_n134_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n133_), .O(new_n140_));
  nand2  g107(.a(x13), .b(x08), .O(new_n141_));
  nand4  g108(.a(new_n141_), .b(new_n46_), .c(new_n45_), .d(x00), .O(new_n142_));
  nand4  g109(.a(x19), .b(x13), .c(new_n124_), .d(x05), .O(new_n143_));
  nand2  g110(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g111(.a(new_n144_), .b(new_n42_), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n122_), .c(new_n51_), .O(new_n146_));
  inv1   g113(.a(x22), .O(new_n147_));
  inv1   g114(.a(x04), .O(new_n148_));
  nor2   g115(.a(x23), .b(new_n148_), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(x17), .c(new_n147_), .O(new_n150_));
  aoi21  g117(.a(new_n150_), .b(new_n130_), .c(x16), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(new_n146_), .O(new_n152_));
  oai21  g119(.a(new_n123_), .b(new_n121_), .c(x08), .O(new_n153_));
  nand4  g120(.a(new_n153_), .b(new_n152_), .c(new_n140_), .d(new_n55_), .O(z4));
  or2    g121(.a(new_n123_), .b(new_n121_), .O(z5));
  inv1   g122(.a(x11), .O(new_n156_));
  inv1   g123(.a(x06), .O(new_n157_));
  inv1   g124(.a(x20), .O(new_n158_));
  aoi21  g125(.a(new_n158_), .b(x14), .c(new_n157_), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(new_n156_), .c(x03), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n139_), .O(new_n161_));
  inv1   g128(.a(x14), .O(new_n162_));
  nand2  g129(.a(x20), .b(new_n162_), .O(new_n163_));
  aoi21  g130(.a(new_n163_), .b(new_n157_), .c(x11), .O(new_n164_));
  oai22  g131(.a(new_n164_), .b(x03), .c(new_n53_), .d(new_n44_), .O(new_n165_));
  nand3  g132(.a(new_n165_), .b(new_n161_), .c(new_n55_), .O(z6));
  oai21  g133(.a(new_n108_), .b(x21), .c(x13), .O(new_n167_));
  nand3  g134(.a(new_n167_), .b(new_n138_), .c(new_n136_), .O(z7));
endmodule


