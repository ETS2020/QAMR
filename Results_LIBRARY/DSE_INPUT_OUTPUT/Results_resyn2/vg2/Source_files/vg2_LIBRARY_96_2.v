// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g000(.a(x24), .b(x07), .O(new_n34_));
  inv1   g001(.a(new_n34_), .O(new_n35_));
  nand2  g002(.a(new_n35_), .b(x13), .O(new_n36_));
  nand2  g003(.a(x19), .b(x13), .O(new_n37_));
  oai21  g004(.a(new_n37_), .b(x24), .c(new_n34_), .O(new_n38_));
  nand2  g005(.a(new_n38_), .b(x05), .O(new_n39_));
  nor2   g006(.a(x13), .b(x05), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(x19), .O(new_n41_));
  nor3   g008(.a(x24), .b(x10), .c(x02), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x00), .O(new_n43_));
  nand4  g010(.a(new_n43_), .b(new_n41_), .c(new_n39_), .d(new_n36_), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  inv1   g012(.a(x11), .O(new_n46_));
  inv1   g013(.a(x20), .O(new_n47_));
  nor2   g014(.a(x14), .b(x06), .O(new_n48_));
  nor2   g015(.a(x03), .b(x01), .O(new_n49_));
  nand4  g016(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g017(.a(x21), .O(new_n51_));
  nand2  g018(.a(new_n51_), .b(x13), .O(new_n52_));
  oai21  g019(.a(new_n50_), .b(new_n45_), .c(new_n52_), .O(z0));
  inv1   g020(.a(x02), .O(new_n54_));
  inv1   g021(.a(x10), .O(new_n55_));
  nand4  g022(.a(new_n52_), .b(new_n55_), .c(new_n54_), .d(x00), .O(new_n56_));
  nand4  g023(.a(x21), .b(x19), .c(x13), .d(x05), .O(new_n57_));
  aoi21  g024(.a(new_n57_), .b(new_n56_), .c(x24), .O(new_n58_));
  nor2   g025(.a(new_n40_), .b(new_n34_), .O(new_n59_));
  nand2  g026(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g027(.a(new_n60_), .b(new_n41_), .O(new_n61_));
  nand2  g028(.a(new_n48_), .b(new_n47_), .O(new_n62_));
  nor2   g029(.a(x11), .b(x09), .O(new_n63_));
  nor2   g030(.a(x16), .b(x12), .O(new_n64_));
  nor2   g031(.a(x08), .b(x04), .O(new_n65_));
  nand4  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n49_), .O(new_n66_));
  inv1   g033(.a(x17), .O(new_n67_));
  inv1   g034(.a(x22), .O(new_n68_));
  inv1   g035(.a(x23), .O(new_n69_));
  nand3  g036(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nor3   g037(.a(new_n70_), .b(new_n66_), .c(new_n62_), .O(new_n71_));
  oai21  g038(.a(new_n61_), .b(new_n58_), .c(new_n71_), .O(new_n72_));
  nand3  g039(.a(x24), .b(x18), .c(x13), .O(new_n73_));
  inv1   g040(.a(new_n73_), .O(new_n74_));
  oai21  g041(.a(new_n74_), .b(new_n42_), .c(x21), .O(new_n75_));
  inv1   g042(.a(x13), .O(new_n76_));
  nand3  g043(.a(x24), .b(x18), .c(new_n76_), .O(new_n77_));
  inv1   g044(.a(x24), .O(new_n78_));
  nand4  g045(.a(new_n78_), .b(x21), .c(x15), .d(x13), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g047(.a(new_n80_), .b(x05), .O(new_n81_));
  nand2  g048(.a(new_n40_), .b(x15), .O(new_n82_));
  nand3  g049(.a(new_n82_), .b(new_n81_), .c(new_n75_), .O(new_n83_));
  nand3  g050(.a(x06), .b(x03), .c(x01), .O(new_n84_));
  inv1   g051(.a(new_n84_), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(x08), .O(new_n86_));
  inv1   g053(.a(new_n86_), .O(new_n87_));
  nand3  g054(.a(x20), .b(x14), .c(x11), .O(new_n88_));
  nand3  g055(.a(x23), .b(x22), .c(x17), .O(new_n89_));
  nand4  g056(.a(x16), .b(x12), .c(x09), .d(x04), .O(new_n90_));
  nor3   g057(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  nand3  g058(.a(new_n91_), .b(new_n87_), .c(new_n83_), .O(new_n92_));
  nand2  g059(.a(new_n92_), .b(new_n72_), .O(z1));
  inv1   g060(.a(new_n88_), .O(new_n94_));
  nand3  g061(.a(new_n94_), .b(new_n85_), .c(new_n83_), .O(new_n95_));
  inv1   g062(.a(new_n95_), .O(z2));
  nand3  g063(.a(new_n94_), .b(new_n87_), .c(new_n83_), .O(new_n97_));
  nor2   g064(.a(new_n50_), .b(x08), .O(new_n98_));
  oai21  g065(.a(new_n61_), .b(new_n58_), .c(new_n98_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n97_), .O(z3));
  inv1   g067(.a(x08), .O(new_n101_));
  inv1   g068(.a(x04), .O(new_n102_));
  oai21  g069(.a(x23), .b(new_n102_), .c(new_n67_), .O(new_n103_));
  aoi21  g070(.a(new_n103_), .b(new_n68_), .c(x09), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(x16), .c(new_n101_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n44_), .O(new_n106_));
  inv1   g073(.a(x16), .O(new_n107_));
  inv1   g074(.a(x09), .O(new_n108_));
  oai21  g075(.a(new_n69_), .b(x04), .c(x17), .O(new_n109_));
  aoi21  g076(.a(new_n109_), .b(x22), .c(new_n108_), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(new_n107_), .c(x08), .O(new_n111_));
  nand2  g078(.a(new_n55_), .b(new_n54_), .O(new_n112_));
  nor2   g079(.a(x21), .b(x13), .O(new_n113_));
  nand3  g080(.a(x15), .b(x13), .c(x05), .O(new_n114_));
  oai21  g081(.a(new_n113_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n78_), .O(new_n116_));
  nand2  g083(.a(x24), .b(x18), .O(new_n117_));
  nor2   g084(.a(new_n117_), .b(new_n40_), .O(new_n118_));
  aoi21  g085(.a(new_n40_), .b(x15), .c(new_n118_), .O(new_n119_));
  nand2  g086(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  nand2  g087(.a(new_n120_), .b(new_n111_), .O(new_n121_));
  nand3  g088(.a(new_n121_), .b(new_n106_), .c(new_n52_), .O(z4));
  oai21  g089(.a(new_n35_), .b(new_n51_), .c(x13), .O(new_n123_));
  nand4  g090(.a(new_n123_), .b(new_n43_), .c(new_n41_), .d(new_n39_), .O(z5));
  inv1   g091(.a(x06), .O(new_n125_));
  inv1   g092(.a(x14), .O(new_n126_));
  nand2  g093(.a(x20), .b(new_n126_), .O(new_n127_));
  aoi21  g094(.a(new_n127_), .b(new_n125_), .c(x11), .O(new_n128_));
  oai21  g095(.a(new_n128_), .b(x03), .c(new_n44_), .O(new_n129_));
  aoi21  g096(.a(new_n47_), .b(x14), .c(new_n125_), .O(new_n130_));
  oai21  g097(.a(new_n130_), .b(new_n46_), .c(x03), .O(new_n131_));
  nand2  g098(.a(new_n131_), .b(new_n120_), .O(new_n132_));
  nand2  g099(.a(new_n43_), .b(x21), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(x13), .O(new_n134_));
  nand3  g101(.a(new_n134_), .b(new_n132_), .c(new_n129_), .O(z6));
  nand3  g102(.a(new_n119_), .b(new_n116_), .c(new_n52_), .O(z7));
endmodule


