// Benchmark "FAU" written by ABC on Tue Aug 11 20:50:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x24), .O(new_n34_));
  inv1   g001(.a(x05), .O(new_n35_));
  nand2  g002(.a(x13), .b(new_n35_), .O(new_n36_));
  nand2  g003(.a(x19), .b(x13), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  nand3  g006(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n40_));
  nand2  g007(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nand3  g008(.a(new_n41_), .b(new_n36_), .c(new_n34_), .O(new_n42_));
  inv1   g009(.a(x13), .O(new_n43_));
  nand3  g010(.a(x24), .b(x07), .c(x05), .O(new_n44_));
  inv1   g011(.a(new_n44_), .O(new_n45_));
  inv1   g012(.a(x19), .O(new_n46_));
  nor2   g013(.a(new_n46_), .b(x05), .O(new_n47_));
  aoi21  g014(.a(new_n47_), .b(new_n43_), .c(new_n45_), .O(new_n48_));
  nand2  g015(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  inv1   g016(.a(x06), .O(new_n50_));
  inv1   g017(.a(x11), .O(new_n51_));
  inv1   g018(.a(x14), .O(new_n52_));
  inv1   g019(.a(x20), .O(new_n53_));
  nand4  g020(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  nor3   g021(.a(new_n54_), .b(x03), .c(x01), .O(new_n55_));
  and2   g022(.a(new_n55_), .b(new_n49_), .O(z0));
  nand3  g023(.a(new_n53_), .b(new_n52_), .c(new_n50_), .O(new_n57_));
  nor2   g024(.a(x17), .b(x16), .O(new_n58_));
  nor2   g025(.a(x23), .b(x22), .O(new_n59_));
  nor2   g026(.a(x09), .b(x04), .O(new_n60_));
  nor2   g027(.a(x12), .b(x11), .O(new_n61_));
  nand4  g028(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n62_));
  inv1   g029(.a(x01), .O(new_n63_));
  inv1   g030(.a(x03), .O(new_n64_));
  inv1   g031(.a(x08), .O(new_n65_));
  nand3  g032(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  nor3   g033(.a(new_n66_), .b(new_n62_), .c(new_n57_), .O(new_n67_));
  nand2  g034(.a(new_n67_), .b(new_n49_), .O(new_n68_));
  nand3  g035(.a(x06), .b(x03), .c(x01), .O(new_n69_));
  inv1   g036(.a(new_n69_), .O(new_n70_));
  nand3  g037(.a(x12), .b(x11), .c(x09), .O(new_n71_));
  inv1   g038(.a(new_n71_), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n70_), .c(x08), .d(x04), .O(new_n73_));
  nand2  g040(.a(x20), .b(x14), .O(new_n74_));
  inv1   g041(.a(new_n74_), .O(new_n75_));
  nand4  g042(.a(x23), .b(x22), .c(x17), .d(x16), .O(new_n76_));
  inv1   g043(.a(new_n76_), .O(new_n77_));
  nand3  g044(.a(new_n77_), .b(new_n75_), .c(x15), .O(new_n78_));
  oai21  g045(.a(new_n78_), .b(new_n73_), .c(new_n43_), .O(new_n79_));
  nand2  g046(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  inv1   g047(.a(x15), .O(new_n81_));
  nand3  g048(.a(x21), .b(new_n39_), .c(new_n38_), .O(new_n82_));
  oai21  g049(.a(new_n81_), .b(new_n43_), .c(new_n82_), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n34_), .O(new_n84_));
  nand3  g051(.a(x24), .b(x18), .c(x05), .O(new_n85_));
  nand2  g052(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g053(.a(new_n77_), .b(new_n75_), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n73_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand3  g056(.a(new_n89_), .b(new_n80_), .c(new_n68_), .O(z1));
  nor4   g057(.a(new_n74_), .b(new_n69_), .c(new_n81_), .d(new_n51_), .O(new_n91_));
  oai21  g058(.a(new_n91_), .b(x13), .c(new_n35_), .O(new_n92_));
  nor3   g059(.a(new_n74_), .b(new_n69_), .c(new_n51_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  nand2  g061(.a(new_n94_), .b(new_n92_), .O(z2));
  nand3  g062(.a(new_n93_), .b(new_n83_), .c(x08), .O(new_n96_));
  nor2   g063(.a(new_n66_), .b(new_n54_), .O(new_n97_));
  nand2  g064(.a(new_n97_), .b(new_n41_), .O(new_n98_));
  nand2  g065(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g066(.a(new_n99_), .b(new_n34_), .O(new_n100_));
  oai21  g067(.a(new_n81_), .b(x05), .c(new_n85_), .O(new_n101_));
  nand3  g068(.a(new_n101_), .b(new_n93_), .c(x08), .O(new_n102_));
  oai21  g069(.a(new_n46_), .b(x05), .c(new_n44_), .O(new_n103_));
  nand2  g070(.a(new_n103_), .b(new_n97_), .O(new_n104_));
  nand3  g071(.a(new_n104_), .b(new_n102_), .c(new_n36_), .O(new_n105_));
  inv1   g072(.a(new_n105_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n100_), .O(z3));
  inv1   g074(.a(x16), .O(new_n108_));
  inv1   g075(.a(x09), .O(new_n109_));
  inv1   g076(.a(x23), .O(new_n110_));
  oai21  g077(.a(new_n110_), .b(x04), .c(x17), .O(new_n111_));
  aoi21  g078(.a(new_n111_), .b(x22), .c(new_n109_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n108_), .c(x08), .O(new_n113_));
  inv1   g080(.a(new_n101_), .O(new_n114_));
  nand2  g081(.a(new_n114_), .b(new_n84_), .O(new_n115_));
  nand2  g082(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  inv1   g083(.a(x22), .O(new_n117_));
  inv1   g084(.a(x04), .O(new_n118_));
  inv1   g085(.a(x17), .O(new_n119_));
  oai21  g086(.a(x23), .b(new_n118_), .c(new_n119_), .O(new_n120_));
  aoi21  g087(.a(new_n120_), .b(new_n117_), .c(x09), .O(new_n121_));
  oai21  g088(.a(new_n121_), .b(x16), .c(new_n65_), .O(new_n122_));
  nand2  g089(.a(new_n41_), .b(new_n34_), .O(new_n123_));
  inv1   g090(.a(new_n103_), .O(new_n124_));
  nand2  g091(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nand2  g092(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(new_n116_), .c(new_n36_), .O(z4));
  oai21  g094(.a(x19), .b(x13), .c(new_n35_), .O(new_n128_));
  nand3  g095(.a(new_n128_), .b(new_n44_), .c(new_n123_), .O(z5));
  nand2  g096(.a(x13), .b(new_n35_), .O(new_n130_));
  nand3  g097(.a(new_n130_), .b(new_n115_), .c(new_n64_), .O(new_n131_));
  inv1   g098(.a(new_n82_), .O(new_n132_));
  aoi21  g099(.a(new_n43_), .b(x05), .c(new_n81_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n132_), .c(x03), .O(new_n134_));
  nand2  g101(.a(new_n134_), .b(new_n34_), .O(new_n135_));
  nand3  g102(.a(x15), .b(new_n43_), .c(new_n35_), .O(new_n136_));
  aoi21  g103(.a(x18), .b(x05), .c(new_n34_), .O(new_n137_));
  oai21  g104(.a(x20), .b(new_n52_), .c(x06), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(x11), .O(new_n139_));
  aoi21  g106(.a(new_n137_), .b(new_n136_), .c(new_n139_), .O(new_n140_));
  nand2  g107(.a(new_n140_), .b(new_n135_), .O(new_n141_));
  nand2  g108(.a(x20), .b(new_n52_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n50_), .c(x11), .O(new_n143_));
  nand2  g110(.a(new_n36_), .b(new_n64_), .O(new_n144_));
  oai21  g111(.a(new_n144_), .b(new_n143_), .c(z5), .O(new_n145_));
  nand3  g112(.a(new_n145_), .b(new_n141_), .c(new_n131_), .O(z6));
  and2   g113(.a(new_n130_), .b(new_n115_), .O(z7));
endmodule


