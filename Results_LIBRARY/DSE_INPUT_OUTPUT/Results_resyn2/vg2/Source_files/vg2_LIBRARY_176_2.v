// Benchmark "FAU" written by ABC on Tue Aug 11 20:49:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_, new_n145_, new_n146_, new_n147_, new_n148_,
    new_n149_, new_n150_, new_n151_, new_n152_, new_n153_, new_n154_,
    new_n155_, new_n156_, new_n159_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n164_;
  inv1   g000(.a(x07), .O(new_n34_));
  inv1   g001(.a(x24), .O(new_n35_));
  nand3  g002(.a(new_n35_), .b(x19), .c(x13), .O(new_n36_));
  oai21  g003(.a(new_n35_), .b(new_n34_), .c(new_n36_), .O(new_n37_));
  inv1   g004(.a(x02), .O(new_n38_));
  inv1   g005(.a(x10), .O(new_n39_));
  nand4  g006(.a(new_n35_), .b(new_n39_), .c(new_n38_), .d(x00), .O(new_n40_));
  nand3  g007(.a(x24), .b(x13), .c(x07), .O(new_n41_));
  nor2   g008(.a(x13), .b(x05), .O(new_n42_));
  nand2  g009(.a(new_n42_), .b(x19), .O(new_n43_));
  nand3  g010(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(new_n44_));
  aoi21  g011(.a(new_n37_), .b(x05), .c(new_n44_), .O(new_n45_));
  nor2   g012(.a(x14), .b(x11), .O(new_n46_));
  nor2   g013(.a(x03), .b(x01), .O(new_n47_));
  nand2  g014(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g015(.a(new_n48_), .O(new_n49_));
  inv1   g016(.a(x17), .O(new_n50_));
  nor2   g017(.a(x23), .b(new_n50_), .O(new_n51_));
  inv1   g018(.a(new_n51_), .O(new_n52_));
  nor2   g019(.a(x20), .b(x06), .O(new_n53_));
  nand3  g020(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n45_), .O(z0));
  inv1   g022(.a(x22), .O(new_n56_));
  inv1   g023(.a(x23), .O(new_n57_));
  nand3  g024(.a(new_n57_), .b(new_n56_), .c(new_n50_), .O(new_n58_));
  inv1   g025(.a(x01), .O(new_n59_));
  inv1   g026(.a(x03), .O(new_n60_));
  inv1   g027(.a(x04), .O(new_n61_));
  inv1   g028(.a(x16), .O(new_n62_));
  nand4  g029(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n63_));
  nor2   g030(.a(new_n63_), .b(new_n58_), .O(new_n64_));
  inv1   g031(.a(x19), .O(new_n65_));
  nand3  g032(.a(new_n39_), .b(new_n38_), .c(x00), .O(new_n66_));
  nand2  g033(.a(x13), .b(x05), .O(new_n67_));
  oai21  g034(.a(new_n67_), .b(new_n65_), .c(new_n66_), .O(new_n68_));
  inv1   g035(.a(x11), .O(new_n69_));
  inv1   g036(.a(x12), .O(new_n70_));
  inv1   g037(.a(x14), .O(new_n71_));
  inv1   g038(.a(x20), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  inv1   g040(.a(x06), .O(new_n74_));
  inv1   g041(.a(x08), .O(new_n75_));
  inv1   g042(.a(x09), .O(new_n76_));
  nand3  g043(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nor2   g044(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g045(.a(new_n78_), .b(new_n68_), .c(new_n64_), .O(new_n79_));
  inv1   g046(.a(x15), .O(new_n80_));
  nand3  g047(.a(x21), .b(new_n39_), .c(new_n38_), .O(new_n81_));
  oai21  g048(.a(new_n67_), .b(new_n80_), .c(new_n81_), .O(new_n82_));
  nand4  g049(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n83_));
  nand3  g050(.a(x11), .b(x09), .c(x04), .O(new_n84_));
  nor2   g051(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g052(.a(x23), .b(x17), .c(x14), .d(x12), .O(new_n86_));
  nand3  g053(.a(x22), .b(x20), .c(x16), .O(new_n87_));
  nor2   g054(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g055(.a(new_n88_), .b(new_n85_), .c(new_n82_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n79_), .O(new_n90_));
  nand2  g057(.a(new_n90_), .b(new_n35_), .O(new_n91_));
  inv1   g058(.a(new_n58_), .O(new_n92_));
  inv1   g059(.a(new_n63_), .O(new_n93_));
  nand2  g060(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g061(.a(new_n71_), .b(new_n70_), .O(new_n95_));
  nor2   g062(.a(new_n56_), .b(new_n72_), .O(new_n96_));
  nand4  g063(.a(x23), .b(x18), .c(x17), .d(x16), .O(new_n97_));
  inv1   g064(.a(new_n97_), .O(new_n98_));
  nand4  g065(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n85_), .O(new_n99_));
  nor2   g066(.a(x20), .b(x12), .O(new_n100_));
  nor2   g067(.a(x08), .b(x06), .O(new_n101_));
  nor2   g068(.a(x09), .b(new_n34_), .O(new_n102_));
  nand4  g069(.a(new_n102_), .b(new_n101_), .c(new_n100_), .d(new_n46_), .O(new_n103_));
  oai21  g070(.a(new_n103_), .b(new_n94_), .c(new_n99_), .O(new_n104_));
  nor2   g071(.a(new_n42_), .b(new_n35_), .O(new_n105_));
  nand2  g072(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  inv1   g073(.a(new_n86_), .O(new_n107_));
  nor2   g074(.a(new_n62_), .b(new_n80_), .O(new_n108_));
  nand4  g075(.a(new_n108_), .b(new_n96_), .c(new_n107_), .d(new_n85_), .O(new_n109_));
  nor2   g076(.a(new_n65_), .b(x08), .O(new_n110_));
  nor2   g077(.a(x12), .b(x09), .O(new_n111_));
  nand4  g078(.a(new_n111_), .b(new_n110_), .c(new_n53_), .d(new_n46_), .O(new_n112_));
  oai21  g079(.a(new_n112_), .b(new_n94_), .c(new_n109_), .O(new_n113_));
  nand2  g080(.a(new_n113_), .b(new_n42_), .O(new_n114_));
  nand3  g081(.a(new_n114_), .b(new_n106_), .c(new_n91_), .O(z1));
  inv1   g082(.a(x05), .O(new_n116_));
  nand2  g083(.a(x24), .b(x18), .O(new_n117_));
  nand3  g084(.a(new_n35_), .b(x15), .c(x13), .O(new_n118_));
  aoi21  g085(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  nand4  g086(.a(new_n35_), .b(x21), .c(new_n39_), .d(new_n38_), .O(new_n120_));
  nand2  g087(.a(new_n42_), .b(x15), .O(new_n121_));
  nand3  g088(.a(x24), .b(x18), .c(x13), .O(new_n122_));
  nand3  g089(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nor2   g090(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand3  g091(.a(x20), .b(x14), .c(x11), .O(new_n125_));
  inv1   g092(.a(new_n125_), .O(new_n126_));
  nand4  g093(.a(new_n126_), .b(x06), .c(x03), .d(x01), .O(new_n127_));
  nor3   g094(.a(new_n127_), .b(new_n124_), .c(new_n51_), .O(z2));
  nor3   g095(.a(new_n125_), .b(new_n83_), .c(new_n80_), .O(new_n129_));
  nand2  g096(.a(new_n110_), .b(new_n53_), .O(new_n130_));
  nor2   g097(.a(new_n130_), .b(new_n48_), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(new_n129_), .c(new_n42_), .O(new_n132_));
  nor2   g099(.a(new_n125_), .b(new_n83_), .O(new_n133_));
  nand2  g100(.a(new_n133_), .b(new_n82_), .O(new_n134_));
  nor3   g101(.a(x20), .b(x08), .c(x06), .O(new_n135_));
  nand3  g102(.a(new_n135_), .b(new_n68_), .c(new_n49_), .O(new_n136_));
  nand2  g103(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand2  g104(.a(new_n137_), .b(new_n35_), .O(new_n138_));
  nand4  g105(.a(x20), .b(x18), .c(x14), .d(x11), .O(new_n139_));
  nand4  g106(.a(new_n72_), .b(new_n71_), .c(new_n69_), .d(x07), .O(new_n140_));
  nand2  g107(.a(new_n101_), .b(new_n47_), .O(new_n141_));
  oai22  g108(.a(new_n141_), .b(new_n140_), .c(new_n139_), .d(new_n83_), .O(new_n142_));
  aoi21  g109(.a(new_n142_), .b(new_n105_), .c(new_n51_), .O(new_n143_));
  nand3  g110(.a(new_n143_), .b(new_n138_), .c(new_n132_), .O(z3));
  nand2  g111(.a(new_n37_), .b(x05), .O(new_n145_));
  inv1   g112(.a(new_n44_), .O(new_n146_));
  nand2  g113(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  oai21  g114(.a(x23), .b(new_n61_), .c(new_n50_), .O(new_n148_));
  aoi21  g115(.a(new_n148_), .b(new_n56_), .c(x09), .O(new_n149_));
  oai21  g116(.a(new_n149_), .b(x16), .c(new_n75_), .O(new_n150_));
  nand3  g117(.a(new_n150_), .b(new_n52_), .c(new_n147_), .O(new_n151_));
  oai21  g118(.a(x22), .b(new_n76_), .c(x16), .O(new_n152_));
  nand3  g119(.a(x09), .b(x08), .c(x04), .O(new_n153_));
  aoi21  g120(.a(new_n153_), .b(x23), .c(new_n50_), .O(new_n154_));
  aoi21  g121(.a(new_n152_), .b(x08), .c(new_n154_), .O(new_n155_));
  oai21  g122(.a(new_n123_), .b(new_n119_), .c(new_n155_), .O(new_n156_));
  nand2  g123(.a(new_n156_), .b(new_n151_), .O(z4));
  nand2  g124(.a(new_n52_), .b(new_n45_), .O(z5));
  aoi21  g125(.a(x20), .b(new_n71_), .c(x06), .O(new_n159_));
  oai21  g126(.a(new_n159_), .b(x11), .c(new_n60_), .O(new_n160_));
  nand2  g127(.a(new_n160_), .b(new_n147_), .O(new_n161_));
  nand2  g128(.a(new_n72_), .b(x14), .O(new_n162_));
  aoi21  g129(.a(new_n162_), .b(x06), .c(new_n69_), .O(new_n163_));
  oai22  g130(.a(new_n163_), .b(new_n60_), .c(new_n123_), .d(new_n119_), .O(new_n164_));
  nand3  g131(.a(new_n164_), .b(new_n161_), .c(new_n52_), .O(z6));
  nor2   g132(.a(new_n124_), .b(new_n51_), .O(z7));
endmodule


