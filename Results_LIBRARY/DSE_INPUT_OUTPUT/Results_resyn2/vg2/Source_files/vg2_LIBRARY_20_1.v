// Benchmark "FAU" written by ABC on Tue Aug 11 20:48:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x05), .O(new_n34_));
  nand2  g001(.a(x19), .b(new_n34_), .O(new_n35_));
  inv1   g002(.a(x24), .O(new_n36_));
  inv1   g003(.a(x13), .O(new_n37_));
  inv1   g004(.a(x19), .O(new_n38_));
  inv1   g005(.a(x02), .O(new_n39_));
  inv1   g006(.a(x10), .O(new_n40_));
  nand3  g007(.a(new_n40_), .b(new_n39_), .c(x00), .O(new_n41_));
  oai21  g008(.a(new_n38_), .b(new_n37_), .c(new_n41_), .O(new_n42_));
  aoi21  g009(.a(new_n37_), .b(new_n34_), .c(new_n36_), .O(new_n43_));
  aoi22  g010(.a(new_n43_), .b(x07), .c(new_n42_), .d(new_n36_), .O(new_n44_));
  nor2   g011(.a(x20), .b(x11), .O(new_n45_));
  nor2   g012(.a(x03), .b(x01), .O(new_n46_));
  nor2   g013(.a(x14), .b(x06), .O(new_n47_));
  nand3  g014(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  oai21  g015(.a(new_n48_), .b(new_n44_), .c(new_n35_), .O(z0));
  nand4  g016(.a(x08), .b(x06), .c(x03), .d(x01), .O(new_n50_));
  nand2  g017(.a(x09), .b(x04), .O(new_n51_));
  nor2   g018(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand4  g019(.a(x23), .b(x22), .c(x20), .d(x17), .O(new_n53_));
  nand4  g020(.a(x16), .b(x14), .c(x12), .d(x11), .O(new_n54_));
  nor2   g021(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g022(.a(x21), .b(new_n40_), .c(new_n39_), .O(new_n56_));
  nand3  g023(.a(x15), .b(x13), .c(x05), .O(new_n57_));
  nand2  g024(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g025(.a(new_n58_), .b(new_n55_), .c(new_n52_), .O(new_n59_));
  nor2   g026(.a(x08), .b(x04), .O(new_n60_));
  nor2   g027(.a(x23), .b(x22), .O(new_n61_));
  nand3  g028(.a(new_n61_), .b(new_n60_), .c(new_n45_), .O(new_n62_));
  inv1   g029(.a(new_n62_), .O(new_n63_));
  inv1   g030(.a(x12), .O(new_n64_));
  inv1   g031(.a(x14), .O(new_n65_));
  inv1   g032(.a(x16), .O(new_n66_));
  inv1   g033(.a(x17), .O(new_n67_));
  nand4  g034(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  inv1   g035(.a(x01), .O(new_n69_));
  inv1   g036(.a(x03), .O(new_n70_));
  inv1   g037(.a(x06), .O(new_n71_));
  inv1   g038(.a(x09), .O(new_n72_));
  nand4  g039(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nor2   g040(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  nand3  g041(.a(new_n74_), .b(new_n63_), .c(new_n42_), .O(new_n75_));
  nand2  g042(.a(new_n75_), .b(new_n59_), .O(new_n76_));
  nand2  g043(.a(new_n76_), .b(new_n36_), .O(new_n77_));
  inv1   g044(.a(new_n68_), .O(new_n78_));
  nand2  g045(.a(new_n72_), .b(new_n71_), .O(new_n79_));
  nand3  g046(.a(x07), .b(new_n70_), .c(new_n69_), .O(new_n80_));
  nor2   g047(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g048(.a(new_n81_), .b(new_n78_), .c(new_n63_), .O(new_n82_));
  nand3  g049(.a(new_n55_), .b(new_n52_), .c(x18), .O(new_n83_));
  nand2  g050(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g051(.a(new_n84_), .b(new_n43_), .O(new_n85_));
  inv1   g052(.a(x15), .O(new_n86_));
  nor2   g053(.a(new_n86_), .b(x13), .O(new_n87_));
  nand3  g054(.a(new_n87_), .b(new_n55_), .c(new_n52_), .O(new_n88_));
  nand2  g055(.a(new_n88_), .b(new_n38_), .O(new_n89_));
  nand2  g056(.a(new_n89_), .b(new_n34_), .O(new_n90_));
  nand3  g057(.a(new_n90_), .b(new_n85_), .c(new_n77_), .O(z1));
  inv1   g058(.a(x18), .O(new_n92_));
  nor2   g059(.a(new_n36_), .b(new_n92_), .O(new_n93_));
  inv1   g060(.a(new_n93_), .O(new_n94_));
  nand3  g061(.a(new_n36_), .b(x15), .c(x13), .O(new_n95_));
  aoi21  g062(.a(new_n95_), .b(new_n94_), .c(new_n34_), .O(new_n96_));
  nand4  g063(.a(new_n36_), .b(x21), .c(new_n40_), .d(new_n39_), .O(new_n97_));
  nand2  g064(.a(new_n93_), .b(x13), .O(new_n98_));
  nand2  g065(.a(new_n87_), .b(new_n34_), .O(new_n99_));
  nand3  g066(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  nor2   g067(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nand3  g068(.a(x20), .b(x14), .c(x11), .O(new_n102_));
  inv1   g069(.a(new_n102_), .O(new_n103_));
  nand4  g070(.a(new_n103_), .b(x06), .c(x03), .d(x01), .O(new_n104_));
  oai21  g071(.a(new_n104_), .b(new_n101_), .c(new_n35_), .O(z2));
  nor2   g072(.a(new_n102_), .b(new_n50_), .O(new_n106_));
  nand2  g073(.a(new_n106_), .b(new_n87_), .O(new_n107_));
  nand2  g074(.a(new_n107_), .b(new_n38_), .O(new_n108_));
  nand2  g075(.a(new_n108_), .b(new_n34_), .O(new_n109_));
  nor3   g076(.a(new_n102_), .b(new_n50_), .c(new_n92_), .O(new_n110_));
  inv1   g077(.a(x08), .O(new_n111_));
  nand3  g078(.a(new_n47_), .b(new_n45_), .c(new_n111_), .O(new_n112_));
  nor2   g079(.a(new_n112_), .b(new_n80_), .O(new_n113_));
  oai21  g080(.a(new_n113_), .b(new_n110_), .c(new_n43_), .O(new_n114_));
  nand2  g081(.a(new_n106_), .b(new_n58_), .O(new_n115_));
  inv1   g082(.a(new_n48_), .O(new_n116_));
  nand3  g083(.a(new_n116_), .b(new_n42_), .c(new_n111_), .O(new_n117_));
  nand2  g084(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g085(.a(new_n118_), .b(new_n36_), .O(new_n119_));
  nand3  g086(.a(new_n119_), .b(new_n114_), .c(new_n109_), .O(z3));
  inv1   g087(.a(x22), .O(new_n121_));
  inv1   g088(.a(x04), .O(new_n122_));
  oai21  g089(.a(x23), .b(new_n122_), .c(new_n67_), .O(new_n123_));
  aoi21  g090(.a(new_n123_), .b(new_n121_), .c(x09), .O(new_n124_));
  oai21  g091(.a(new_n124_), .b(x16), .c(new_n111_), .O(new_n125_));
  oai21  g092(.a(x19), .b(new_n37_), .c(new_n34_), .O(new_n126_));
  nand3  g093(.a(new_n126_), .b(x24), .c(x07), .O(new_n127_));
  nand3  g094(.a(new_n42_), .b(new_n35_), .c(new_n36_), .O(new_n128_));
  nand2  g095(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g096(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  inv1   g097(.a(x23), .O(new_n131_));
  oai21  g098(.a(new_n131_), .b(x04), .c(x17), .O(new_n132_));
  aoi21  g099(.a(new_n132_), .b(x22), .c(new_n72_), .O(new_n133_));
  oai21  g100(.a(new_n133_), .b(new_n66_), .c(x08), .O(new_n134_));
  nand3  g101(.a(new_n58_), .b(new_n35_), .c(new_n36_), .O(new_n135_));
  nand2  g102(.a(new_n126_), .b(new_n93_), .O(new_n136_));
  nand3  g103(.a(new_n87_), .b(new_n38_), .c(new_n34_), .O(new_n137_));
  nand3  g104(.a(new_n137_), .b(new_n136_), .c(new_n135_), .O(new_n138_));
  nand2  g105(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g106(.a(new_n139_), .b(new_n130_), .O(z4));
  nand2  g107(.a(new_n42_), .b(new_n36_), .O(new_n141_));
  nand2  g108(.a(new_n43_), .b(x07), .O(new_n142_));
  nand3  g109(.a(new_n142_), .b(new_n141_), .c(new_n35_), .O(z5));
  inv1   g110(.a(x11), .O(new_n144_));
  inv1   g111(.a(x20), .O(new_n145_));
  nand2  g112(.a(new_n145_), .b(x14), .O(new_n146_));
  aoi21  g113(.a(new_n146_), .b(x06), .c(new_n144_), .O(new_n147_));
  oai22  g114(.a(new_n147_), .b(new_n70_), .c(new_n100_), .d(new_n96_), .O(new_n148_));
  aoi21  g115(.a(x20), .b(new_n65_), .c(x06), .O(new_n149_));
  aoi21  g116(.a(x19), .b(new_n34_), .c(x03), .O(new_n150_));
  oai21  g117(.a(new_n149_), .b(x11), .c(new_n150_), .O(new_n151_));
  nand2  g118(.a(new_n151_), .b(z5), .O(new_n152_));
  nand2  g119(.a(new_n152_), .b(new_n148_), .O(z6));
  nand2  g120(.a(new_n101_), .b(new_n35_), .O(z7));
endmodule


