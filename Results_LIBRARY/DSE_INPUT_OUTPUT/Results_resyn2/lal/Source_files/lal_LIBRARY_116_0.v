// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x20), .b(x19), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x22), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(x24), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  oai21  g12(.a(x25), .b(x23), .c(x19), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n56_), .d(new_n49_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  inv1   g18(.a(x24), .O(new_n64_));
  nand3  g19(.a(x22), .b(x21), .c(x20), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n53_), .c(new_n64_), .O(new_n66_));
  nor3   g21(.a(new_n66_), .b(new_n47_), .c(x25), .O(z03));
  inv1   g22(.a(new_n47_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n68_), .O(z06));
  nand3  g34(.a(new_n68_), .b(new_n46_), .c(x06), .O(z07));
  and2   g35(.a(new_n59_), .b(new_n56_), .O(z08));
  nor2   g36(.a(x15), .b(x07), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x05), .c(x04), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n68_), .O(z09));
  inv1   g39(.a(x17), .O(new_n85_));
  nand2  g40(.a(new_n82_), .b(new_n60_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n68_), .c(new_n85_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  inv1   g45(.a(x18), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n90_), .c(new_n87_), .d(new_n68_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(x19), .O(new_n96_));
  nand2  g51(.a(new_n90_), .b(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n95_), .c(new_n87_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n68_), .O(z12));
  inv1   g54(.a(new_n95_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n57_), .O(new_n101_));
  nand2  g56(.a(new_n86_), .b(new_n68_), .O(new_n102_));
  nand2  g57(.a(new_n95_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand2  g59(.a(new_n90_), .b(x21), .O(new_n105_));
  nor2   g60(.a(new_n90_), .b(x20), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n51_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x19), .O(new_n109_));
  nand2  g64(.a(x21), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n102_), .O(z14));
  nand3  g66(.a(new_n107_), .b(new_n68_), .c(new_n52_), .O(new_n112_));
  nand4  g67(.a(new_n100_), .b(x22), .c(new_n51_), .d(new_n57_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n87_), .O(z15));
  oai21  g70(.a(new_n107_), .b(x22), .c(x23), .O(new_n116_));
  nor3   g71(.a(x23), .b(x22), .c(x21), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n106_), .O(new_n118_));
  oai21  g73(.a(x23), .b(new_n57_), .c(new_n96_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n116_), .d(new_n87_), .O(z16));
  aoi21  g75(.a(new_n117_), .b(new_n106_), .c(x24), .O(new_n121_));
  nand3  g76(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n122_));
  nand3  g77(.a(x24), .b(x18), .c(x17), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n122_), .c(x19), .O(new_n124_));
  aoi22  g79(.a(new_n86_), .b(new_n68_), .c(x24), .d(x20), .O(new_n125_));
  oai21  g80(.a(new_n124_), .b(new_n121_), .c(new_n125_), .O(z17));
  nand3  g81(.a(new_n64_), .b(x18), .c(x17), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n122_), .c(x25), .O(new_n128_));
  nor2   g83(.a(x25), .b(x24), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n117_), .c(new_n106_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(x19), .O(new_n132_));
  aoi22  g87(.a(new_n86_), .b(new_n68_), .c(x25), .d(x20), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


