// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  inv1   g02(.a(x25), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(z02), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x18), .b(x17), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n53_), .c(new_n48_), .O(new_n59_));
  oai21  g14(.a(x23), .b(x20), .c(new_n48_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n60_), .b(x19), .c(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n59_), .O(z01));
  nand2  g20(.a(new_n60_), .b(x19), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n59_), .O(z03));
  inv1   g22(.a(new_n49_), .O(new_n68_));
  inv1   g23(.a(x08), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n68_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand2  g32(.a(new_n51_), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(new_n51_), .b(x14), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z06));
  aoi21  g36(.a(new_n69_), .b(x06), .c(new_n49_), .O(z07));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand3  g38(.a(new_n57_), .b(new_n83_), .c(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n48_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  inv1   g41(.a(x23), .O(new_n87_));
  oai21  g42(.a(new_n56_), .b(new_n54_), .c(new_n87_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n48_), .c(x24), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n86_), .O(z08));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n61_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n62_), .c(new_n68_), .O(z09));
  nand3  g48(.a(new_n62_), .b(new_n91_), .c(new_n61_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(x17), .c(new_n68_), .O(z10));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor4   g52(.a(new_n97_), .b(new_n94_), .c(new_n83_), .d(new_n49_), .O(z11));
  inv1   g53(.a(new_n94_), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(new_n47_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n68_), .O(z12));
  nand2  g58(.a(new_n94_), .b(new_n68_), .O(new_n104_));
  oai21  g59(.a(new_n97_), .b(x25), .c(x19), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n100_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n48_), .c(new_n54_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(z13));
  nand3  g64(.a(new_n54_), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x19), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x21), .O(new_n113_));
  inv1   g68(.a(x21), .O(new_n114_));
  nand4  g69(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n115_));
  nor2   g70(.a(new_n115_), .b(x25), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n113_), .c(new_n104_), .O(z14));
  nand2  g73(.a(new_n117_), .b(x22), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n110_), .c(new_n48_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(x19), .c(new_n94_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n119_), .O(z15));
  oai21  g79(.a(new_n121_), .b(new_n108_), .c(x23), .O(new_n125_));
  inv1   g80(.a(x22), .O(new_n126_));
  nand3  g81(.a(new_n87_), .b(new_n126_), .c(new_n114_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n110_), .c(new_n48_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x19), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n125_), .c(new_n99_), .O(z16));
  nand2  g85(.a(new_n129_), .b(x24), .O(new_n131_));
  nand3  g86(.a(new_n120_), .b(new_n53_), .c(new_n87_), .O(new_n132_));
  inv1   g87(.a(new_n132_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n116_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n131_), .c(new_n104_), .O(z17));
  nor2   g90(.a(new_n132_), .b(new_n115_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n94_), .c(new_n48_), .O(new_n137_));
  nand2  g92(.a(x25), .b(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n137_), .O(z18));
endmodule


