// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  inv1   g13(.a(x25), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(x25), .b(new_n58_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(new_n47_), .c(new_n64_), .O(new_n65_));
  oai21  g20(.a(new_n60_), .b(new_n57_), .c(new_n65_), .O(z01));
  and2   g21(.a(new_n48_), .b(x16), .O(z02));
  nand2  g22(.a(new_n61_), .b(new_n47_), .O(new_n68_));
  oai21  g23(.a(new_n60_), .b(new_n57_), .c(new_n68_), .O(z03));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  nor2   g32(.a(new_n70_), .b(x13), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor2   g34(.a(new_n70_), .b(new_n79_), .O(z06));
  nand3  g35(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g36(.a(new_n55_), .b(new_n51_), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n59_), .c(new_n58_), .O(z08));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n48_), .c(x05), .d(x04), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z09));
  nand4  g42(.a(new_n85_), .b(new_n63_), .c(new_n48_), .d(new_n52_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  xor2a  g44(.a(x18), .b(x17), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n85_), .c(new_n63_), .d(new_n48_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n94_));
  nand2  g49(.a(new_n85_), .b(new_n63_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n48_), .O(z12));
  and2   g53(.a(new_n85_), .b(new_n48_), .O(new_n99_));
  inv1   g54(.a(new_n93_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n51_), .O(new_n101_));
  nand2  g56(.a(new_n93_), .b(x20), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n101_), .c(new_n99_), .d(new_n63_), .O(z13));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n100_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n99_), .d(new_n63_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n100_), .c(new_n109_), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n108_), .c(new_n99_), .d(new_n63_), .O(z15));
  nand3  g66(.a(new_n110_), .b(x24), .c(x23), .O(new_n112_));
  nand2  g67(.a(new_n95_), .b(new_n48_), .O(new_n113_));
  nand4  g68(.a(new_n105_), .b(new_n100_), .c(new_n58_), .d(new_n109_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(z16));
  nand2  g70(.a(new_n110_), .b(x24), .O(new_n116_));
  nand4  g71(.a(new_n105_), .b(new_n100_), .c(new_n47_), .d(new_n109_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(new_n96_), .d(new_n58_), .O(z17));
  nand2  g73(.a(new_n117_), .b(x25), .O(new_n119_));
  nor2   g74(.a(x25), .b(x24), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n105_), .c(new_n100_), .d(new_n109_), .O(new_n121_));
  aoi21  g76(.a(new_n59_), .b(x24), .c(new_n58_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n95_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n119_), .O(z18));
endmodule


