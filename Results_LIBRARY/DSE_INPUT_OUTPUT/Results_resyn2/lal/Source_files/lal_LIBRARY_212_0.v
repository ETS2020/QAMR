// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n111_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x23), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n48_), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n55_), .b(x20), .c(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n53_), .c(new_n54_), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  nand2  g16(.a(new_n48_), .b(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n60_), .c(new_n52_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nor2   g20(.a(new_n62_), .b(new_n60_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n48_), .O(z04));
  nor3   g28(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g32(.a(new_n59_), .b(new_n53_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  nand2  g34(.a(new_n48_), .b(x25), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n79_), .O(z08));
  nor3   g36(.a(new_n47_), .b(x15), .c(x07), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n51_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z09));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n86_), .c(new_n48_), .O(z11));
  nand2  g44(.a(new_n82_), .b(new_n50_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  aoi21  g47(.a(x18), .b(x17), .c(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(z12));
  nor2   g49(.a(new_n91_), .b(x20), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(new_n91_), .b(x20), .c(new_n86_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n96_), .c(new_n47_), .O(z13));
  inv1   g53(.a(new_n90_), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(x21), .O(new_n100_));
  nand2  g55(.a(new_n95_), .b(new_n56_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(z14));
  nand2  g57(.a(new_n101_), .b(x22), .O(new_n103_));
  inv1   g58(.a(new_n86_), .O(new_n104_));
  nand3  g59(.a(new_n95_), .b(new_n57_), .c(new_n56_), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n103_), .c(new_n47_), .O(z15));
  oai21  g62(.a(new_n105_), .b(new_n86_), .c(x23), .O(new_n108_));
  nand2  g63(.a(x24), .b(new_n53_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n106_), .c(new_n108_), .O(z16));
  oai21  g65(.a(new_n105_), .b(x23), .c(x24), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n99_), .O(z17));
  aoi21  g67(.a(new_n104_), .b(new_n61_), .c(new_n47_), .O(z18));
endmodule


