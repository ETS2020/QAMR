// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n56_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x23), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(new_n48_), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n51_), .O(z03));
  inv1   g09(.a(z03), .O(z08));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(z08), .c(new_n50_), .O(z01));
  inv1   g12(.a(x16), .O(new_n58_));
  nor2   g13(.a(new_n48_), .b(new_n58_), .O(z02));
  xnor2a g14(.a(x12), .b(x03), .O(new_n60_));
  xnor2a g15(.a(x09), .b(x00), .O(new_n61_));
  xnor2a g16(.a(x11), .b(x02), .O(new_n62_));
  xnor2a g17(.a(x10), .b(x01), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n51_), .O(z04));
  nand2  g21(.a(new_n51_), .b(new_n46_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(x13), .O(z05));
  inv1   g23(.a(x14), .O(new_n69_));
  nor2   g24(.a(new_n67_), .b(new_n69_), .O(z06));
  nand3  g25(.a(new_n51_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g26(.a(x15), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n56_), .c(new_n51_), .O(z09));
  inv1   g29(.a(x17), .O(new_n75_));
  nand3  g30(.a(new_n56_), .b(new_n72_), .c(new_n50_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z10));
  xnor2a g34(.a(x18), .b(x17), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n76_), .c(new_n51_), .O(z11));
  aoi21  g36(.a(x18), .b(x17), .c(x19), .O(new_n82_));
  inv1   g37(.a(new_n76_), .O(new_n83_));
  nand3  g38(.a(x19), .b(x18), .c(x17), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n82_), .c(new_n51_), .O(z12));
  inv1   g41(.a(x20), .O(new_n87_));
  inv1   g42(.a(new_n84_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g44(.a(new_n84_), .b(x20), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n77_), .O(z13));
  nand2  g46(.a(new_n89_), .b(x21), .O(new_n92_));
  nor2   g47(.a(x21), .b(x20), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n88_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n92_), .c(new_n77_), .O(z14));
  nand2  g50(.a(new_n94_), .b(x22), .O(new_n96_));
  inv1   g51(.a(x22), .O(new_n97_));
  nand3  g52(.a(new_n93_), .b(new_n88_), .c(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n77_), .O(z15));
  inv1   g54(.a(x23), .O(new_n100_));
  nand4  g55(.a(new_n93_), .b(new_n88_), .c(new_n100_), .d(new_n97_), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(x23), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n77_), .O(z16));
  nand3  g58(.a(new_n101_), .b(new_n83_), .c(new_n47_), .O(z17));
  nand2  g59(.a(new_n101_), .b(new_n52_), .O(new_n105_));
  nand4  g60(.a(new_n93_), .b(new_n88_), .c(x25), .d(new_n97_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n47_), .O(new_n107_));
  aoi22  g62(.a(new_n76_), .b(new_n51_), .c(x25), .d(x23), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(z18));
endmodule


