// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x07), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  nor2   g07(.a(x24), .b(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(x25), .O(z01));
  inv1   g10(.a(x23), .O(new_n56_));
  nor3   g11(.a(x19), .b(x18), .c(x17), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n57_), .b(x20), .c(new_n59_), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n56_), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x24), .c(x25), .O(z03));
  nand2  g17(.a(new_n50_), .b(new_n46_), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z04));
  nor2   g25(.a(new_n63_), .b(x13), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  nor2   g27(.a(new_n63_), .b(new_n72_), .O(z06));
  aoi21  g28(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g29(.a(x25), .O(new_n75_));
  inv1   g30(.a(x19), .O(new_n76_));
  nor2   g31(.a(x18), .b(x17), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n78_));
  oai21  g33(.a(new_n58_), .b(new_n78_), .c(new_n56_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x24), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n75_), .c(new_n49_), .O(z08));
  inv1   g36(.a(x15), .O(new_n82_));
  inv1   g37(.a(new_n54_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n48_), .c(x07), .O(z09));
  inv1   g40(.a(x17), .O(new_n86_));
  nor2   g41(.a(new_n83_), .b(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n53_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  and2   g46(.a(x18), .b(x17), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n88_), .c(new_n77_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n92_), .b(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n88_), .O(z12));
  inv1   g52(.a(x07), .O(new_n98_));
  inv1   g53(.a(x20), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n94_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n87_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n98_), .O(z13));
  nand2  g59(.a(new_n100_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n95_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n89_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n106_), .b(new_n95_), .c(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n87_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n98_), .O(z15));
  nand2  g69(.a(new_n111_), .b(x23), .O(new_n115_));
  nor2   g70(.a(x23), .b(x22), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n106_), .c(new_n95_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n87_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n98_), .O(z16));
  and2   g75(.a(new_n117_), .b(new_n87_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(x24), .c(new_n98_), .O(z17));
  nand4  g77(.a(new_n116_), .b(new_n106_), .c(new_n95_), .d(new_n75_), .O(new_n123_));
  nand2  g78(.a(new_n117_), .b(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n87_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n48_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n98_), .O(z18));
endmodule


