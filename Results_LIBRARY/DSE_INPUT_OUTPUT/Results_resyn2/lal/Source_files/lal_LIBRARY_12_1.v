// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x06), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(x06), .c(new_n50_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x07), .c(new_n48_), .O(new_n64_));
  oai21  g19(.a(new_n61_), .b(x25), .c(new_n64_), .O(z01));
  and2   g20(.a(new_n48_), .b(x16), .O(z02));
  inv1   g21(.a(x25), .O(new_n67_));
  oai21  g22(.a(new_n60_), .b(new_n50_), .c(new_n67_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n48_), .O(z03));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  nor2   g32(.a(new_n70_), .b(x13), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor2   g34(.a(new_n70_), .b(new_n79_), .O(z06));
  nand2  g35(.a(new_n46_), .b(x06), .O(z07));
  inv1   g36(.a(new_n48_), .O(new_n82_));
  inv1   g37(.a(x23), .O(new_n83_));
  nand2  g38(.a(new_n59_), .b(new_n56_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n67_), .c(new_n82_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  and2   g43(.a(new_n88_), .b(new_n48_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n63_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n62_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n52_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nor2   g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n93_), .c(new_n97_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n88_), .b(new_n62_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n48_), .O(z12));
  nor2   g61(.a(new_n101_), .b(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n101_), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n93_), .O(z13));
  nand2  g65(.a(new_n108_), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n107_), .b(new_n57_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n93_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n107_), .b(new_n58_), .c(new_n57_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n93_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nand4  g72(.a(new_n107_), .b(new_n83_), .c(new_n58_), .d(new_n57_), .O(new_n118_));
  and2   g73(.a(new_n118_), .b(new_n104_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n117_), .c(new_n82_), .O(z16));
  nand3  g75(.a(new_n118_), .b(x24), .c(x06), .O(new_n121_));
  nand2  g76(.a(new_n103_), .b(new_n48_), .O(new_n122_));
  inv1   g77(.a(new_n115_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n50_), .c(new_n83_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(z17));
  nand4  g80(.a(new_n123_), .b(new_n67_), .c(new_n50_), .d(new_n83_), .O(new_n126_));
  nand2  g81(.a(new_n48_), .b(new_n67_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n126_), .c(new_n104_), .O(z18));
endmodule


