// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x05), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n47_), .c(new_n51_), .O(new_n61_));
  inv1   g16(.a(x05), .O(new_n62_));
  nand2  g17(.a(x25), .b(new_n47_), .O(new_n63_));
  inv1   g18(.a(x07), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n63_), .b(new_n62_), .c(new_n66_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n61_), .O(z01));
  inv1   g23(.a(x16), .O(new_n69_));
  nand2  g24(.a(new_n49_), .b(new_n69_), .O(z02));
  nand2  g25(.a(new_n61_), .b(new_n49_), .O(z03));
  nand2  g26(.a(new_n49_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor2   g36(.a(new_n72_), .b(new_n81_), .O(z06));
  nand3  g37(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(x23), .O(new_n84_));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n55_), .c(x20), .O(new_n86_));
  oai21  g41(.a(new_n58_), .b(new_n86_), .c(new_n84_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n62_), .c(x24), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n51_), .O(z08));
  nor2   g44(.a(x15), .b(x07), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n48_), .O(new_n91_));
  aoi21  g46(.a(new_n65_), .b(new_n49_), .c(new_n91_), .O(z09));
  inv1   g47(.a(x15), .O(new_n93_));
  nand3  g48(.a(new_n65_), .b(new_n93_), .c(new_n64_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(new_n48_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  inv1   g52(.a(new_n85_), .O(new_n98_));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n94_), .c(new_n49_), .O(z11));
  nand2  g56(.a(new_n99_), .b(new_n55_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(new_n94_), .c(new_n49_), .O(z12));
  nand2  g60(.a(new_n103_), .b(x20), .O(new_n106_));
  nor2   g61(.a(new_n103_), .b(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n95_), .O(z13));
  inv1   g64(.a(x21), .O(new_n110_));
  nor2   g65(.a(new_n107_), .b(new_n110_), .O(new_n111_));
  inv1   g66(.a(new_n103_), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n90_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n111_), .c(new_n49_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n65_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  inv1   g73(.a(x22), .O(new_n119_));
  nand3  g74(.a(new_n113_), .b(new_n112_), .c(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n95_), .O(z15));
  nand2  g76(.a(new_n120_), .b(x23), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n113_), .c(new_n112_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n90_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n49_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n65_), .O(z16));
  aoi21  g82(.a(new_n124_), .b(x24), .c(x04), .O(new_n128_));
  nand3  g83(.a(new_n123_), .b(new_n47_), .c(new_n110_), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(new_n107_), .c(new_n91_), .O(new_n131_));
  oai21  g86(.a(new_n128_), .b(new_n62_), .c(new_n131_), .O(z17));
  inv1   g87(.a(new_n94_), .O(new_n133_));
  oai21  g88(.a(new_n124_), .b(x25), .c(new_n47_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n62_), .O(new_n135_));
  nand4  g90(.a(new_n123_), .b(new_n107_), .c(new_n47_), .d(new_n110_), .O(new_n136_));
  xor2a  g91(.a(new_n136_), .b(x25), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(z18));
endmodule


