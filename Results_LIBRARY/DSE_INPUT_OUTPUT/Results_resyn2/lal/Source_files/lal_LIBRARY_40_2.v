// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n69_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x22), .O(new_n47_));
  nor2   g02(.a(x25), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(x22), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(x24), .c(x25), .O(z03));
  inv1   g08(.a(z03), .O(z08));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(x07), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(z08), .O(z01));
  inv1   g13(.a(x16), .O(new_n59_));
  nor2   g14(.a(new_n48_), .b(new_n59_), .O(z02));
  xnor2a g15(.a(x12), .b(x03), .O(new_n61_));
  xnor2a g16(.a(x11), .b(x02), .O(new_n62_));
  xnor2a g17(.a(x09), .b(x00), .O(new_n63_));
  xnor2a g18(.a(x10), .b(x01), .O(new_n64_));
  nand4  g19(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n61_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n49_), .O(z04));
  oai21  g22(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g23(.a(x14), .O(new_n69_));
  nor3   g24(.a(new_n48_), .b(new_n69_), .c(x08), .O(z06));
  aoi21  g25(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nor2   g26(.a(x15), .b(x07), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(new_n56_), .c(new_n49_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z09));
  inv1   g29(.a(x17), .O(new_n75_));
  nand2  g30(.a(new_n72_), .b(new_n55_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n48_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z10));
  nand2  g34(.a(x18), .b(x17), .O(new_n80_));
  inv1   g35(.a(x18), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n75_), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z11));
  aoi21  g39(.a(x18), .b(x17), .c(x19), .O(new_n85_));
  inv1   g40(.a(new_n76_), .O(new_n86_));
  nand3  g41(.a(x19), .b(x18), .c(x17), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n85_), .c(new_n49_), .O(z12));
  inv1   g44(.a(x20), .O(new_n90_));
  inv1   g45(.a(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g47(.a(new_n87_), .b(x20), .c(new_n76_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n92_), .c(new_n48_), .O(z13));
  nand2  g49(.a(new_n92_), .b(x21), .O(new_n95_));
  nor3   g50(.a(new_n87_), .b(x21), .c(x20), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n76_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n95_), .c(new_n48_), .O(z14));
  nand2  g53(.a(new_n96_), .b(new_n47_), .O(new_n99_));
  inv1   g54(.a(new_n96_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x22), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n77_), .O(z15));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n91_), .c(new_n51_), .O(new_n104_));
  nand2  g59(.a(new_n103_), .b(new_n51_), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n104_), .c(new_n47_), .O(new_n107_));
  nor2   g62(.a(new_n51_), .b(new_n47_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n76_), .c(new_n49_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n107_), .O(z16));
  nand3  g65(.a(new_n103_), .b(new_n91_), .c(new_n51_), .O(new_n111_));
  xor2a  g66(.a(new_n111_), .b(x24), .O(new_n112_));
  inv1   g67(.a(x24), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n76_), .c(new_n49_), .O(new_n115_));
  oai21  g70(.a(new_n112_), .b(x22), .c(new_n115_), .O(z17));
  inv1   g71(.a(x25), .O(new_n117_));
  nand3  g72(.a(new_n106_), .b(new_n117_), .c(new_n113_), .O(new_n118_));
  oai21  g73(.a(new_n111_), .b(x24), .c(x25), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n118_), .c(new_n86_), .d(new_n47_), .O(z18));
endmodule


