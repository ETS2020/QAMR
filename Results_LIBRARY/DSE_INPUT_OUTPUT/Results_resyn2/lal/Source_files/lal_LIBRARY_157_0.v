// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n74_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_;
  inv1   g00(.a(x16), .O(new_n46_));
  nor2   g01(.a(x24), .b(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  aoi21  g06(.a(x25), .b(x23), .c(x24), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nor3   g08(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g12(.a(new_n54_), .b(x20), .c(new_n57_), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n60_), .c(new_n53_), .d(new_n51_), .O(z01));
  nand2  g17(.a(new_n60_), .b(new_n53_), .O(z03));
  inv1   g18(.a(new_n49_), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g23(.a(x10), .b(x01), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z04));
  nor2   g26(.a(new_n64_), .b(x13), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  inv1   g28(.a(new_n47_), .O(new_n74_));
  oai21  g29(.a(new_n73_), .b(x08), .c(new_n74_), .O(z06));
  nand2  g30(.a(new_n49_), .b(x06), .O(z07));
  aoi21  g31(.a(new_n59_), .b(new_n58_), .c(new_n52_), .O(z08));
  nor2   g32(.a(x15), .b(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x05), .c(x04), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n74_), .O(z09));
  nand2  g35(.a(new_n78_), .b(new_n61_), .O(new_n81_));
  nor3   g36(.a(new_n81_), .b(new_n47_), .c(x17), .O(z10));
  xnor2a g37(.a(x18), .b(x17), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(new_n74_), .O(z11));
  inv1   g39(.a(new_n81_), .O(new_n85_));
  nand3  g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  inv1   g41(.a(x19), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n86_), .c(new_n85_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n74_), .O(z12));
  inv1   g46(.a(x20), .O(new_n92_));
  inv1   g47(.a(new_n86_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  aoi21  g49(.a(new_n86_), .b(x20), .c(new_n81_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n94_), .c(new_n47_), .O(z13));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  nand2  g52(.a(new_n55_), .b(new_n92_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n81_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n97_), .c(new_n47_), .O(z14));
  oai21  g56(.a(new_n98_), .b(new_n86_), .c(x22), .O(new_n102_));
  aoi21  g57(.a(new_n99_), .b(new_n56_), .c(new_n81_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n102_), .c(new_n47_), .O(z15));
  inv1   g59(.a(x23), .O(new_n105_));
  nand4  g60(.a(new_n99_), .b(x24), .c(new_n105_), .d(new_n56_), .O(new_n106_));
  nand4  g61(.a(new_n93_), .b(new_n56_), .c(new_n55_), .d(new_n92_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x23), .O(new_n108_));
  nand2  g63(.a(new_n81_), .b(new_n74_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(z16));
  oai21  g65(.a(new_n107_), .b(x23), .c(x24), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n109_), .O(z17));
  nand2  g67(.a(new_n74_), .b(x25), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n109_), .O(z18));
endmodule


