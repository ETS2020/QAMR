// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(x20), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  nand2  g06(.a(x22), .b(x21), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  aoi21  g08(.a(x05), .b(x04), .c(x07), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(x25), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  inv1   g17(.a(x25), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n62_), .c(new_n56_), .O(z01));
  and2   g20(.a(new_n48_), .b(x16), .O(z02));
  inv1   g21(.a(new_n52_), .O(new_n67_));
  aoi21  g22(.a(new_n62_), .b(new_n67_), .c(x23), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(new_n51_), .c(new_n63_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n48_), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  inv1   g34(.a(new_n48_), .O(new_n80_));
  nor3   g35(.a(new_n80_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n80_), .O(z07));
  nor2   g37(.a(new_n61_), .b(new_n52_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x23), .c(x24), .O(new_n84_));
  oai21  g39(.a(new_n52_), .b(new_n51_), .c(x15), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x20), .c(x25), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n84_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n57_), .c(x15), .O(z09));
  nand2  g45(.a(new_n54_), .b(new_n58_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n57_), .c(x15), .O(z10));
  nand2  g47(.a(new_n59_), .b(new_n58_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n54_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n57_), .c(x15), .O(z11));
  nand3  g51(.a(new_n54_), .b(new_n57_), .c(new_n47_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  aoi21  g54(.a(x18), .b(x17), .c(x19), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(z12));
  nand2  g56(.a(new_n98_), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n57_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(z13));
  inv1   g59(.a(x21), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  aoi21  g61(.a(new_n98_), .b(x21), .c(new_n97_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(z14));
  nand2  g63(.a(new_n106_), .b(x22), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n99_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n54_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n57_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(z15));
  nand2  g69(.a(new_n111_), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n110_), .b(new_n99_), .c(new_n50_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n54_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n57_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n47_), .O(z16));
  nand2  g74(.a(new_n116_), .b(x24), .O(new_n120_));
  nand4  g75(.a(new_n110_), .b(new_n99_), .c(new_n51_), .d(new_n50_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n54_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n57_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n47_), .O(z17));
  inv1   g79(.a(x22), .O(new_n125_));
  nand4  g80(.a(new_n63_), .b(new_n51_), .c(new_n50_), .d(new_n125_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n106_), .c(new_n54_), .O(new_n127_));
  aoi21  g82(.a(new_n121_), .b(x25), .c(new_n127_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(x20), .c(new_n47_), .O(z18));
endmodule


