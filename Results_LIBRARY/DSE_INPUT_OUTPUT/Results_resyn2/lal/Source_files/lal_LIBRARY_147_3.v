// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n80_,
    new_n81_, new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x14), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  inv1   g11(.a(x14), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n56_), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n49_), .O(z08));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(z08), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n47_), .b(new_n67_), .O(z02));
  inv1   g23(.a(z08), .O(z03));
  nand2  g24(.a(new_n47_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  or2    g32(.a(new_n70_), .b(x13), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  inv1   g34(.a(x24), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(x14), .c(new_n46_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  inv1   g37(.a(new_n47_), .O(new_n83_));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n83_), .O(z07));
  inv1   g39(.a(x15), .O(new_n85_));
  nand3  g40(.a(new_n47_), .b(new_n85_), .c(new_n62_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(new_n63_), .O(z09));
  nand3  g42(.a(new_n63_), .b(new_n85_), .c(new_n62_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g44(.a(new_n52_), .b(new_n51_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n88_), .c(new_n47_), .O(z11));
  inv1   g48(.a(new_n88_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n91_), .b(new_n53_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z12));
  inv1   g53(.a(new_n95_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(new_n100_));
  aoi21  g55(.a(new_n95_), .b(x20), .c(new_n88_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n100_), .c(new_n83_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  inv1   g60(.a(new_n86_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n63_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n103_), .O(z14));
  nand2  g64(.a(new_n105_), .b(x22), .O(new_n110_));
  nor3   g65(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n99_), .c(new_n88_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n110_), .c(new_n83_), .O(z15));
  inv1   g68(.a(x22), .O(new_n114_));
  nand2  g69(.a(new_n104_), .b(new_n114_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n95_), .c(x23), .O(new_n116_));
  nand3  g71(.a(new_n111_), .b(new_n99_), .c(new_n58_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n94_), .O(new_n118_));
  and2   g73(.a(new_n118_), .b(new_n47_), .O(z16));
  nand2  g74(.a(new_n117_), .b(x24), .O(new_n120_));
  inv1   g75(.a(new_n117_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n80_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n108_), .O(z17));
  nor2   g78(.a(new_n49_), .b(new_n80_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n88_), .c(new_n47_), .O(new_n125_));
  nand4  g80(.a(new_n111_), .b(new_n99_), .c(x25), .d(new_n58_), .O(new_n126_));
  nand2  g81(.a(new_n117_), .b(new_n49_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n80_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n125_), .O(z18));
endmodule


