// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n89_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(new_n48_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  and2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n58_), .b(new_n60_), .c(new_n50_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  inv1   g22(.a(x12), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(x03), .O(new_n69_));
  inv1   g24(.a(x02), .O(new_n70_));
  nand2  g25(.a(x11), .b(new_n70_), .O(new_n71_));
  inv1   g26(.a(x03), .O(new_n72_));
  nand2  g27(.a(x12), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x11), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x02), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n73_), .c(new_n71_), .d(new_n69_), .O(new_n76_));
  inv1   g31(.a(x00), .O(new_n77_));
  nand2  g32(.a(x09), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(x01), .O(new_n79_));
  nand2  g34(.a(x10), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(x09), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x00), .O(new_n82_));
  inv1   g37(.a(x10), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x01), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n82_), .c(new_n80_), .d(new_n78_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n76_), .c(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n60_), .O(z04));
  oai21  g42(.a(x13), .b(x08), .c(new_n60_), .O(z05));
  nand2  g43(.a(x14), .b(new_n46_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n60_), .O(z06));
  aoi21  g45(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g46(.a(x07), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(x05), .c(x04), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n47_), .c(x15), .O(z09));
  inv1   g49(.a(x15), .O(new_n95_));
  nand3  g50(.a(new_n61_), .b(new_n47_), .c(new_n95_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n52_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  nor2   g54(.a(x18), .b(x17), .O(new_n100_));
  nor2   g55(.a(new_n53_), .b(new_n52_), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n96_), .c(new_n100_), .O(z11));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n101_), .b(x19), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n104_), .c(new_n96_), .O(z12));
  nand2  g61(.a(new_n104_), .b(new_n51_), .O(new_n107_));
  nand2  g62(.a(new_n103_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n61_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n95_), .O(z13));
  nand2  g66(.a(new_n107_), .b(x21), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n104_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n97_), .O(z14));
  nand2  g70(.a(new_n114_), .b(x22), .O(new_n116_));
  inv1   g71(.a(x22), .O(new_n117_));
  nand3  g72(.a(new_n113_), .b(new_n104_), .c(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n61_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n47_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n95_), .O(z15));
  nand2  g76(.a(new_n118_), .b(new_n97_), .O(z16));
  nand2  g77(.a(new_n118_), .b(x24), .O(new_n123_));
  nor2   g78(.a(x24), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n113_), .c(new_n104_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n61_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n95_), .O(z17));
  aoi21  g83(.a(new_n125_), .b(x25), .c(new_n96_), .O(new_n129_));
  oai21  g84(.a(new_n125_), .b(x25), .c(new_n129_), .O(z18));
endmodule


