// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x19), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  oai21  g06(.a(x18), .b(x17), .c(new_n51_), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n52_), .c(x23), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(new_n49_), .c(new_n50_), .O(new_n56_));
  inv1   g11(.a(x15), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  nand3  g13(.a(new_n50_), .b(new_n58_), .c(new_n51_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(x19), .c(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n56_), .O(z01));
  and2   g19(.a(new_n47_), .b(x16), .O(z02));
  nand2  g20(.a(new_n60_), .b(x19), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n56_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n47_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g30(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z06));
  inv1   g32(.a(new_n47_), .O(new_n78_));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  inv1   g34(.a(x19), .O(new_n80_));
  nor2   g35(.a(x18), .b(x17), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n51_), .c(new_n53_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x23), .c(x24), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n47_), .c(new_n50_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n57_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand2  g43(.a(new_n61_), .b(new_n57_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n47_), .O(z10));
  and2   g45(.a(x18), .b(x17), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n89_), .c(new_n81_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nor2   g49(.a(new_n91_), .b(x19), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(z12));
  oai21  g51(.a(x20), .b(x15), .c(new_n93_), .O(new_n97_));
  nor2   g52(.a(new_n93_), .b(x20), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n62_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n97_), .c(new_n78_), .O(z13));
  inv1   g55(.a(new_n89_), .O(new_n101_));
  oai21  g56(.a(new_n93_), .b(x20), .c(x21), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(z14));
  nand2  g60(.a(new_n104_), .b(x22), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n101_), .O(z15));
  nand3  g64(.a(new_n107_), .b(new_n98_), .c(new_n58_), .O(new_n110_));
  nand2  g65(.a(new_n108_), .b(x23), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n101_), .O(z16));
  nand2  g67(.a(new_n110_), .b(x24), .O(new_n113_));
  nor2   g68(.a(x24), .b(x23), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n107_), .c(new_n98_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n101_), .O(z17));
  nand4  g71(.a(new_n114_), .b(new_n107_), .c(new_n98_), .d(new_n50_), .O(new_n117_));
  nand2  g72(.a(new_n115_), .b(x25), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n101_), .O(z18));
endmodule


