// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n77_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  aoi21  g06(.a(new_n51_), .b(new_n50_), .c(x20), .O(new_n52_));
  nand3  g07(.a(x24), .b(x22), .c(x21), .O(new_n53_));
  nor2   g08(.a(x25), .b(x23), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(z08));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x07), .c(new_n48_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(z08), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nand2  g15(.a(new_n48_), .b(new_n60_), .O(z02));
  inv1   g16(.a(x25), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x23), .c(new_n47_), .O(new_n63_));
  nand2  g18(.a(x22), .b(x21), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n52_), .c(new_n54_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n63_), .O(z03));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n48_), .O(z04));
  inv1   g28(.a(x23), .O(new_n74_));
  nor2   g29(.a(x24), .b(new_n74_), .O(new_n75_));
  nor3   g30(.a(new_n75_), .b(x13), .c(x08), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n48_), .O(z06));
  nand3  g33(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n75_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n57_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z09));
  inv1   g40(.a(x17), .O(new_n86_));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n56_), .c(new_n48_), .d(new_n86_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  xor2a  g44(.a(x18), .b(x17), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n87_), .c(new_n56_), .d(new_n48_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n93_), .c(new_n87_), .d(new_n56_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n48_), .O(z12));
  inv1   g52(.a(x20), .O(new_n98_));
  inv1   g53(.a(new_n93_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g55(.a(new_n87_), .b(new_n56_), .O(new_n101_));
  aoi21  g56(.a(new_n93_), .b(x20), .c(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n100_), .c(new_n75_), .O(z13));
  nand2  g58(.a(new_n100_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n99_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n83_), .d(new_n56_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n99_), .c(new_n101_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n75_), .O(z15));
  nand2  g66(.a(new_n109_), .b(new_n99_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(x24), .c(x23), .O(new_n113_));
  nand2  g68(.a(new_n101_), .b(new_n48_), .O(new_n114_));
  nand3  g69(.a(new_n109_), .b(new_n99_), .c(new_n74_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  nand4  g72(.a(new_n109_), .b(new_n99_), .c(new_n47_), .d(new_n74_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n114_), .O(z17));
  nand3  g74(.a(new_n109_), .b(new_n99_), .c(new_n47_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x25), .O(new_n121_));
  nand4  g76(.a(new_n109_), .b(new_n99_), .c(new_n62_), .d(new_n47_), .O(new_n122_));
  aoi21  g77(.a(new_n62_), .b(x24), .c(new_n74_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(new_n101_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(z18));
endmodule


