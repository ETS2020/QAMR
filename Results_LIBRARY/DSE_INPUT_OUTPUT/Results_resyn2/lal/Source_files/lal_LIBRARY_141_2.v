// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  nand3  g12(.a(new_n47_), .b(new_n57_), .c(new_n56_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  inv1   g18(.a(x24), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x23), .O(new_n65_));
  nor2   g20(.a(x24), .b(x19), .O(new_n66_));
  aoi21  g21(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g23(.a(x16), .O(new_n69_));
  nor2   g24(.a(new_n48_), .b(new_n69_), .O(z02));
  inv1   g25(.a(x23), .O(new_n71_));
  nand2  g26(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  aoi22  g27(.a(new_n72_), .b(x24), .c(new_n50_), .d(x25), .O(z03));
  nand2  g28(.a(new_n50_), .b(new_n46_), .O(new_n74_));
  xor2a  g29(.a(x12), .b(x03), .O(new_n75_));
  xor2a  g30(.a(x11), .b(x02), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  xor2a  g33(.a(x10), .b(x01), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z04));
  nor2   g36(.a(new_n74_), .b(x13), .O(z05));
  inv1   g37(.a(x14), .O(new_n83_));
  nor2   g38(.a(new_n74_), .b(new_n83_), .O(z06));
  nand3  g39(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g40(.a(x25), .O(new_n86_));
  nand2  g41(.a(new_n72_), .b(x24), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n50_), .c(new_n86_), .O(z08));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n52_), .c(new_n50_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n52_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n56_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nand2  g50(.a(new_n57_), .b(new_n56_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n93_), .c(new_n96_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  aoi21  g54(.a(x18), .b(x17), .c(x19), .O(new_n100_));
  inv1   g55(.a(new_n92_), .O(new_n101_));
  oai21  g56(.a(new_n97_), .b(new_n47_), .c(new_n101_), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(new_n100_), .c(new_n50_), .O(z12));
  aoi21  g58(.a(new_n97_), .b(x24), .c(new_n47_), .O(new_n104_));
  nand2  g59(.a(x24), .b(x19), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n97_), .c(x20), .O(new_n106_));
  aoi21  g61(.a(new_n89_), .b(new_n52_), .c(new_n48_), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai21  g63(.a(new_n104_), .b(new_n55_), .c(new_n108_), .O(z13));
  inv1   g64(.a(new_n107_), .O(new_n110_));
  nor2   g65(.a(new_n97_), .b(x20), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n64_), .c(x19), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x21), .O(new_n113_));
  nand4  g68(.a(new_n60_), .b(new_n55_), .c(x18), .d(x17), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x24), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n113_), .c(new_n110_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x19), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x22), .O(new_n120_));
  nand3  g75(.a(new_n115_), .b(x24), .c(new_n61_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n110_), .O(z15));
  nand3  g77(.a(new_n111_), .b(new_n61_), .c(new_n60_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(x24), .c(new_n47_), .O(new_n124_));
  nor3   g79(.a(x23), .b(x22), .c(x21), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n106_), .c(new_n107_), .O(new_n126_));
  oai21  g81(.a(new_n124_), .b(new_n71_), .c(new_n126_), .O(z16));
  nand3  g82(.a(new_n115_), .b(new_n71_), .c(new_n61_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x24), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n110_), .O(z17));
  nand2  g85(.a(new_n93_), .b(new_n86_), .O(z18));
endmodule


