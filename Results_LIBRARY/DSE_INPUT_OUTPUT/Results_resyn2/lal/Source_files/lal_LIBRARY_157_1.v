// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n69_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(new_n48_), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n53_), .O(z03));
  inv1   g11(.a(z03), .O(z08));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(z08), .c(new_n52_), .O(z01));
  inv1   g14(.a(new_n50_), .O(new_n60_));
  xor2a  g15(.a(x12), .b(x03), .O(new_n61_));
  xor2a  g16(.a(x11), .b(x02), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  xor2a  g18(.a(x09), .b(x00), .O(new_n64_));
  xor2a  g19(.a(x10), .b(x01), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(new_n60_), .O(z04));
  nor2   g22(.a(new_n60_), .b(x13), .O(z05));
  inv1   g23(.a(x14), .O(new_n69_));
  oai21  g24(.a(new_n69_), .b(x08), .c(new_n53_), .O(z06));
  nand2  g25(.a(new_n50_), .b(x06), .O(z07));
  inv1   g26(.a(x15), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n52_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n58_), .c(new_n53_), .O(z09));
  nand3  g29(.a(new_n58_), .b(new_n72_), .c(new_n52_), .O(new_n75_));
  nor3   g30(.a(new_n75_), .b(new_n48_), .c(x17), .O(z10));
  xnor2a g31(.a(x18), .b(x17), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n75_), .c(new_n53_), .O(z11));
  inv1   g33(.a(new_n75_), .O(new_n79_));
  nand3  g34(.a(x19), .b(x18), .c(x17), .O(new_n80_));
  inv1   g35(.a(x19), .O(new_n81_));
  nand2  g36(.a(x18), .b(x17), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n53_), .O(z12));
  inv1   g40(.a(x20), .O(new_n86_));
  inv1   g41(.a(new_n80_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g43(.a(new_n80_), .b(x20), .c(new_n75_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n88_), .c(new_n48_), .O(z13));
  nand2  g45(.a(new_n88_), .b(x21), .O(new_n91_));
  nor2   g46(.a(x21), .b(x20), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n87_), .c(new_n75_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n91_), .c(new_n48_), .O(z14));
  nand2  g49(.a(new_n92_), .b(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x22), .O(new_n96_));
  nor3   g51(.a(x22), .b(x21), .c(x20), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n87_), .c(new_n75_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n96_), .c(new_n48_), .O(z15));
  inv1   g54(.a(x23), .O(new_n100_));
  nand4  g55(.a(new_n97_), .b(new_n87_), .c(new_n47_), .d(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(new_n87_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x23), .O(new_n103_));
  nand2  g58(.a(new_n75_), .b(new_n53_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(z16));
  nand2  g60(.a(new_n79_), .b(new_n47_), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n101_), .c(new_n48_), .O(z17));
  inv1   g63(.a(x22), .O(new_n109_));
  nand4  g64(.a(new_n92_), .b(new_n87_), .c(new_n100_), .d(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n54_), .O(new_n111_));
  nand3  g66(.a(new_n97_), .b(new_n87_), .c(x25), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n47_), .O(new_n113_));
  aoi22  g68(.a(new_n75_), .b(new_n53_), .c(x25), .d(x23), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(z18));
endmodule


