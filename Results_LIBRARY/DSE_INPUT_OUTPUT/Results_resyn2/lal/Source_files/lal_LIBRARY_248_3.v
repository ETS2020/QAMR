// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n80_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(new_n52_), .c(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(x07), .O(new_n64_));
  oai21  g19(.a(new_n63_), .b(x07), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n50_), .b(new_n66_), .O(z02));
  nand3  g22(.a(new_n60_), .b(new_n50_), .c(new_n52_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z04));
  nor2   g31(.a(x13), .b(x08), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g37(.a(new_n60_), .b(new_n52_), .c(new_n49_), .O(z08));
  nor2   g38(.a(new_n62_), .b(x15), .O(new_n84_));
  nand2  g39(.a(new_n48_), .b(new_n47_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n50_), .c(new_n84_), .O(z09));
  nand3  g41(.a(new_n61_), .b(new_n48_), .c(new_n47_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n54_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nor2   g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n87_), .c(new_n91_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n56_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n88_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n50_), .O(z12));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n95_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n88_), .O(z13));
  inv1   g57(.a(x21), .O(new_n103_));
  nor2   g58(.a(new_n99_), .b(new_n103_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(new_n103_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n104_), .c(new_n47_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n64_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  aoi21  g64(.a(new_n99_), .b(new_n103_), .c(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n99_), .b(new_n109_), .c(new_n103_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n84_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n110_), .c(new_n47_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n64_), .O(z15));
  nand2  g69(.a(new_n111_), .b(x23), .O(new_n115_));
  nor2   g70(.a(new_n111_), .b(x23), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n88_), .O(z16));
  nand2  g73(.a(new_n117_), .b(x24), .O(new_n119_));
  inv1   g74(.a(x24), .O(new_n120_));
  nand2  g75(.a(new_n116_), .b(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n88_), .O(z17));
  nand3  g77(.a(new_n116_), .b(new_n52_), .c(new_n120_), .O(new_n123_));
  nand2  g78(.a(new_n121_), .b(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n88_), .O(z18));
endmodule


