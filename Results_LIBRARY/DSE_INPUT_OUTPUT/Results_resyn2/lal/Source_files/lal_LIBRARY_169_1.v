// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x09), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  inv1   g15(.a(x21), .O(new_n61_));
  inv1   g16(.a(x22), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(x23), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(x09), .c(new_n47_), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n49_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x25), .O(new_n69_));
  oai21  g24(.a(new_n64_), .b(new_n47_), .c(new_n69_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z03));
  inv1   g26(.a(x12), .O(new_n72_));
  inv1   g27(.a(x10), .O(new_n73_));
  aoi22  g28(.a(new_n72_), .b(x03), .c(new_n73_), .d(x01), .O(new_n74_));
  oai21  g29(.a(new_n72_), .b(x03), .c(new_n74_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  inv1   g31(.a(x00), .O(new_n77_));
  aoi21  g32(.a(new_n47_), .b(new_n77_), .c(x09), .O(new_n78_));
  inv1   g33(.a(x11), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x02), .O(new_n80_));
  nand2  g35(.a(x09), .b(new_n77_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai22  g37(.a(new_n79_), .b(x02), .c(new_n73_), .d(x01), .O(new_n83_));
  nor3   g38(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(new_n84_));
  aoi22  g39(.a(new_n84_), .b(new_n76_), .c(new_n49_), .d(x08), .O(z04));
  oai21  g40(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g41(.a(x14), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x08), .c(new_n49_), .O(z06));
  nand3  g43(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g44(.a(new_n64_), .b(x09), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x24), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n69_), .O(z08));
  nor2   g47(.a(x15), .b(x07), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n52_), .c(new_n49_), .O(z09));
  nand2  g50(.a(new_n93_), .b(new_n52_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n56_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  nand2  g54(.a(new_n57_), .b(new_n56_), .O(new_n100_));
  nand2  g55(.a(x18), .b(x17), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n97_), .c(new_n100_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  inv1   g58(.a(new_n96_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  nand2  g60(.a(new_n101_), .b(new_n58_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n49_), .O(z12));
  nor2   g63(.a(new_n105_), .b(x20), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n97_), .O(z13));
  nand2  g67(.a(new_n110_), .b(x21), .O(new_n113_));
  aoi21  g68(.a(new_n109_), .b(new_n61_), .c(new_n96_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(new_n48_), .O(z14));
  oai21  g70(.a(new_n110_), .b(x21), .c(x22), .O(new_n116_));
  nand3  g71(.a(new_n109_), .b(new_n62_), .c(new_n61_), .O(new_n117_));
  inv1   g72(.a(new_n117_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n96_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n116_), .c(new_n48_), .O(z15));
  nand2  g75(.a(new_n117_), .b(x23), .O(new_n121_));
  inv1   g76(.a(x23), .O(new_n122_));
  nand4  g77(.a(new_n109_), .b(new_n122_), .c(new_n62_), .d(new_n61_), .O(new_n123_));
  and2   g78(.a(new_n123_), .b(new_n104_), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n121_), .c(new_n48_), .O(z16));
  nand3  g80(.a(new_n123_), .b(x24), .c(x09), .O(new_n126_));
  nand2  g81(.a(new_n96_), .b(new_n49_), .O(new_n127_));
  nand3  g82(.a(new_n118_), .b(new_n47_), .c(new_n122_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(z17));
  nand4  g84(.a(new_n118_), .b(new_n69_), .c(new_n47_), .d(new_n122_), .O(new_n130_));
  nand2  g85(.a(new_n49_), .b(new_n69_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n130_), .c(new_n104_), .O(z18));
endmodule


