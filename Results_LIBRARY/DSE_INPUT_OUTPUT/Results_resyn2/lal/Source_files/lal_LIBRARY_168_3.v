// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  nand2  g04(.a(x24), .b(x08), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x08), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  inv1   g16(.a(x21), .O(new_n62_));
  inv1   g17(.a(x22), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(x23), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n55_), .c(new_n47_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n54_), .O(z01));
  nand2  g22(.a(new_n50_), .b(new_n46_), .O(z02));
  inv1   g23(.a(x25), .O(new_n69_));
  oai21  g24(.a(new_n65_), .b(new_n47_), .c(new_n69_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n50_), .O(z03));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n55_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(x08), .O(z06));
  nand2  g36(.a(new_n55_), .b(x06), .O(z07));
  nand2  g37(.a(new_n65_), .b(new_n55_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n69_), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n51_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n52_), .c(new_n50_), .O(z09));
  nand3  g43(.a(new_n52_), .b(new_n86_), .c(new_n51_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n57_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  nor2   g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n91_), .c(new_n95_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n59_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n90_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n50_), .O(z12));
  inv1   g57(.a(new_n50_), .O(new_n103_));
  inv1   g58(.a(new_n99_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n56_), .O(new_n105_));
  aoi21  g60(.a(new_n99_), .b(x20), .c(new_n89_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  nor2   g63(.a(new_n105_), .b(x21), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n92_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  nand2  g67(.a(new_n109_), .b(new_n63_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n92_), .O(z15));
  nand2  g69(.a(new_n113_), .b(x23), .O(new_n115_));
  inv1   g70(.a(x23), .O(new_n116_));
  nor2   g71(.a(new_n99_), .b(x20), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(new_n63_), .d(new_n62_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n92_), .O(z16));
  oai21  g74(.a(new_n118_), .b(x08), .c(x24), .O(new_n120_));
  nor2   g75(.a(x24), .b(x23), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n109_), .c(new_n63_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n90_), .O(z17));
  nand4  g78(.a(new_n121_), .b(new_n109_), .c(new_n69_), .d(new_n63_), .O(new_n124_));
  nand2  g79(.a(new_n50_), .b(new_n69_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n90_), .O(z18));
endmodule


