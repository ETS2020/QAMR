// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x22), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(x22), .c(x21), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n52_), .c(new_n51_), .O(new_n60_));
  inv1   g15(.a(x24), .O(new_n61_));
  nand2  g16(.a(x25), .b(new_n47_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n60_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nand2  g24(.a(new_n62_), .b(new_n61_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n60_), .O(z03));
  nand2  g26(.a(new_n49_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  nor2   g34(.a(new_n72_), .b(x13), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g38(.a(x21), .O(new_n84_));
  aoi21  g39(.a(new_n57_), .b(new_n53_), .c(new_n84_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n61_), .c(x22), .O(new_n86_));
  aoi21  g41(.a(x24), .b(x23), .c(x25), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(z08));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(x05), .c(x04), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n49_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n64_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g48(.a(new_n55_), .b(new_n54_), .O(new_n94_));
  nor2   g49(.a(new_n92_), .b(new_n48_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n56_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n92_), .c(new_n48_), .O(z12));
  inv1   g57(.a(new_n99_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n95_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor3   g62(.a(new_n99_), .b(x21), .c(x20), .O(new_n108_));
  nor2   g63(.a(new_n108_), .b(new_n92_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n49_), .O(z14));
  oai21  g65(.a(new_n108_), .b(new_n92_), .c(new_n47_), .O(new_n111_));
  aoi21  g66(.a(new_n89_), .b(new_n64_), .c(x21), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n103_), .c(new_n47_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n112_), .c(x24), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n111_), .O(z15));
  aoi21  g71(.a(new_n113_), .b(new_n103_), .c(new_n51_), .O(new_n117_));
  nand3  g72(.a(new_n51_), .b(new_n84_), .c(new_n53_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n99_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n117_), .c(new_n47_), .O(new_n120_));
  nor2   g75(.a(new_n51_), .b(new_n47_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n92_), .c(new_n49_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(z16));
  nand2  g78(.a(new_n92_), .b(new_n49_), .O(new_n124_));
  nand4  g79(.a(new_n108_), .b(new_n61_), .c(new_n51_), .d(new_n47_), .O(new_n125_));
  nand4  g80(.a(new_n113_), .b(new_n103_), .c(new_n51_), .d(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x24), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(z17));
  nand2  g83(.a(new_n52_), .b(new_n61_), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n113_), .c(new_n103_), .d(new_n51_), .O(new_n130_));
  oai21  g85(.a(new_n118_), .b(new_n99_), .c(new_n52_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n47_), .O(new_n132_));
  nand2  g87(.a(x25), .b(x24), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n132_), .c(new_n124_), .O(z18));
endmodule


