// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n81_, new_n83_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  and2   g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n60_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n50_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n60_), .b(new_n50_), .O(z03));
  nand2  g21(.a(new_n50_), .b(new_n46_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  or2    g29(.a(new_n67_), .b(x13), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g34(.a(new_n59_), .b(new_n52_), .c(new_n49_), .O(z08));
  nand3  g35(.a(new_n61_), .b(new_n48_), .c(new_n47_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand2  g37(.a(new_n62_), .b(new_n48_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n50_), .O(z10));
  and2   g39(.a(x18), .b(x17), .O(new_n85_));
  nor3   g40(.a(new_n85_), .b(new_n83_), .c(new_n55_), .O(z11));
  nand2  g41(.a(new_n85_), .b(x19), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nor2   g43(.a(new_n85_), .b(x19), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n88_), .c(new_n83_), .O(z12));
  nand2  g45(.a(new_n48_), .b(x07), .O(new_n91_));
  nand2  g46(.a(new_n87_), .b(x20), .O(new_n92_));
  nor2   g47(.a(new_n61_), .b(x15), .O(new_n93_));
  nand4  g48(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n47_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n91_), .O(z13));
  nand2  g52(.a(new_n94_), .b(x21), .O(new_n98_));
  nor2   g53(.a(new_n94_), .b(x21), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n98_), .O(z14));
  inv1   g56(.a(x21), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  nor2   g58(.a(x20), .b(new_n54_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n85_), .c(new_n103_), .d(new_n102_), .O(new_n105_));
  oai21  g60(.a(new_n94_), .b(x21), .c(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n93_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n91_), .O(z15));
  nand2  g64(.a(new_n105_), .b(x23), .O(new_n110_));
  nor2   g65(.a(x23), .b(x22), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n99_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n93_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n91_), .O(z16));
  nand2  g70(.a(new_n112_), .b(x24), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n117_), .c(new_n103_), .d(new_n102_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n94_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n83_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n116_), .O(z17));
  nand2  g77(.a(new_n120_), .b(new_n52_), .O(new_n123_));
  oai21  g78(.a(new_n119_), .b(new_n94_), .c(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n93_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n91_), .O(z18));
endmodule


