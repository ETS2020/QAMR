// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  aoi21  g02(.a(new_n47_), .b(new_n46_), .c(x16), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n47_), .c(new_n49_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nor2   g15(.a(new_n47_), .b(x16), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n47_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n58_), .b(x16), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g29(.a(new_n61_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  aoi21  g31(.a(new_n74_), .b(new_n71_), .c(new_n76_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n75_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n75_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n61_), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n53_), .c(x20), .O(new_n84_));
  oai21  g39(.a(new_n56_), .b(new_n84_), .c(new_n82_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n49_), .c(new_n61_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n60_), .c(new_n75_), .O(z09));
  nand3  g46(.a(new_n62_), .b(new_n60_), .c(new_n89_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x17), .O(z10));
  nor2   g48(.a(new_n52_), .b(new_n51_), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n92_), .c(new_n83_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n94_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n92_), .O(z12));
  nand4  g54(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n100_));
  nand3  g55(.a(new_n60_), .b(new_n89_), .c(new_n88_), .O(new_n101_));
  aoi21  g56(.a(new_n96_), .b(x20), .c(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n100_), .c(new_n61_), .O(z13));
  inv1   g58(.a(new_n92_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  inv1   g61(.a(new_n100_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n104_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  nand3  g66(.a(new_n107_), .b(new_n111_), .c(new_n106_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n104_), .O(z15));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n114_));
  nand2  g69(.a(new_n111_), .b(new_n106_), .O(new_n115_));
  nor2   g70(.a(new_n115_), .b(new_n100_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n82_), .c(new_n101_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n114_), .c(new_n61_), .O(z16));
  nand3  g73(.a(new_n116_), .b(new_n47_), .c(new_n82_), .O(new_n119_));
  nand2  g74(.a(new_n116_), .b(new_n82_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x24), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n104_), .O(z17));
  nand4  g77(.a(new_n116_), .b(new_n49_), .c(new_n47_), .d(new_n82_), .O(new_n123_));
  nand2  g78(.a(new_n120_), .b(x25), .O(new_n124_));
  nand2  g79(.a(new_n49_), .b(x16), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(x24), .c(new_n101_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z18));
endmodule


