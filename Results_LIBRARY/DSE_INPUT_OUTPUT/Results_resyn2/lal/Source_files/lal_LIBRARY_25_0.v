// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_;
  inv1   g00(.a(x16), .O(new_n46_));
  nor2   g01(.a(x25), .b(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  aoi21  g05(.a(x05), .b(x04), .c(x07), .O(new_n51_));
  or2    g06(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n55_), .b(x20), .c(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n53_), .c(new_n54_), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x15), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n59_), .c(new_n52_), .O(z01));
  nor2   g17(.a(new_n61_), .b(new_n59_), .O(z03));
  inv1   g18(.a(new_n49_), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g23(.a(x10), .b(x01), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z04));
  nor2   g26(.a(new_n64_), .b(x13), .O(z05));
  and2   g27(.a(new_n49_), .b(x14), .O(z06));
  nand2  g28(.a(new_n49_), .b(x06), .O(z07));
  inv1   g29(.a(x15), .O(new_n75_));
  inv1   g30(.a(x19), .O(new_n76_));
  nor2   g31(.a(x18), .b(x17), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n78_));
  oai21  g33(.a(new_n56_), .b(new_n78_), .c(new_n53_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x24), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n75_), .c(new_n60_), .O(z08));
  inv1   g36(.a(x07), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x05), .c(x04), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x25), .c(x15), .O(z09));
  inv1   g39(.a(x17), .O(new_n85_));
  nand2  g40(.a(new_n51_), .b(new_n85_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x25), .c(x15), .O(z10));
  inv1   g42(.a(new_n77_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n51_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x25), .c(x15), .O(z11));
  nand2  g46(.a(new_n89_), .b(new_n76_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n51_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(x25), .c(x15), .O(z12));
  inv1   g50(.a(x20), .O(new_n96_));
  inv1   g51(.a(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g53(.a(new_n51_), .b(x25), .c(new_n75_), .O(new_n99_));
  aoi21  g54(.a(new_n93_), .b(x20), .c(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n98_), .O(z13));
  nand2  g56(.a(new_n98_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n97_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n51_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x25), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n75_), .O(z14));
  nand2  g62(.a(new_n104_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n103_), .b(new_n97_), .c(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n51_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x25), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n75_), .O(z15));
  nand2  g68(.a(new_n110_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n103_), .c(new_n97_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n51_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x25), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n75_), .O(z16));
  nand2  g74(.a(new_n116_), .b(x24), .O(new_n120_));
  nand4  g75(.a(new_n115_), .b(new_n103_), .c(new_n97_), .d(new_n54_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n51_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x25), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n75_), .O(z17));
  or2    g79(.a(new_n121_), .b(new_n99_), .O(z18));
endmodule


