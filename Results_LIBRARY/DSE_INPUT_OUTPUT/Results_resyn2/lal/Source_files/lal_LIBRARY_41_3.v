// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x10), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(new_n47_), .b(new_n61_), .O(z02));
  nand2  g17(.a(new_n58_), .b(new_n47_), .O(z03));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(new_n64_));
  xor2a  g19(.a(x10), .b(x01), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z04));
  or2    g26(.a(new_n64_), .b(x13), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g30(.a(new_n47_), .O(new_n76_));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n76_), .O(z07));
  aoi21  g32(.a(new_n57_), .b(new_n49_), .c(new_n76_), .O(z08));
  nand2  g33(.a(x05), .b(x04), .O(new_n79_));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n79_), .c(new_n47_), .O(z09));
  nand3  g38(.a(new_n79_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x17), .c(new_n47_), .O(z10));
  nor2   g40(.a(x18), .b(x17), .O(new_n86_));
  inv1   g41(.a(new_n84_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n47_), .O(new_n88_));
  nor2   g43(.a(new_n52_), .b(new_n51_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nor2   g47(.a(new_n89_), .b(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n88_), .O(z12));
  inv1   g49(.a(new_n88_), .O(new_n95_));
  nor2   g50(.a(new_n91_), .b(x20), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n91_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  inv1   g55(.a(x21), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n95_), .O(z14));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n104_));
  inv1   g59(.a(x22), .O(new_n105_));
  nor3   g60(.a(new_n91_), .b(x21), .c(x20), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n84_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n104_), .c(new_n76_), .O(z15));
  oai21  g63(.a(new_n102_), .b(x22), .c(x23), .O(new_n109_));
  nor2   g64(.a(x23), .b(x22), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n106_), .c(new_n84_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n109_), .c(new_n76_), .O(z16));
  inv1   g67(.a(x24), .O(new_n113_));
  nand3  g68(.a(new_n110_), .b(new_n106_), .c(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n110_), .b(new_n96_), .c(new_n101_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(x24), .c(new_n84_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n114_), .c(new_n76_), .O(z17));
  nor2   g72(.a(new_n49_), .b(new_n113_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n84_), .c(new_n47_), .O(new_n119_));
  nor2   g74(.a(new_n115_), .b(new_n49_), .O(new_n120_));
  nand2  g75(.a(new_n115_), .b(new_n49_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n113_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n120_), .c(new_n119_), .O(z18));
endmodule


