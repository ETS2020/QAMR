// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand2  g16(.a(x22), .b(x21), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n61_), .c(x24), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n55_), .c(new_n54_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n53_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(x24), .b(new_n54_), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(z02));
  nor2   g24(.a(new_n55_), .b(x23), .O(new_n70_));
  nor2   g25(.a(x18), .b(x17), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n59_), .c(x20), .O(new_n72_));
  oai21  g27(.a(new_n62_), .b(new_n72_), .c(new_n54_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(x24), .c(new_n70_), .O(z03));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n48_), .O(z04));
  nand2  g36(.a(new_n48_), .b(new_n46_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(x13), .O(z05));
  inv1   g38(.a(x14), .O(new_n84_));
  nor2   g39(.a(new_n82_), .b(new_n84_), .O(z06));
  aoi21  g40(.a(new_n46_), .b(x06), .c(new_n68_), .O(z07));
  inv1   g41(.a(new_n70_), .O(new_n87_));
  aoi21  g42(.a(new_n60_), .b(new_n56_), .c(new_n62_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(x23), .c(x24), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n87_), .O(z08));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n51_), .c(new_n48_), .O(z09));
  nand3  g48(.a(new_n51_), .b(new_n91_), .c(new_n50_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(x17), .c(new_n48_), .O(z10));
  inv1   g50(.a(new_n71_), .O(new_n96_));
  nor2   g51(.a(new_n94_), .b(new_n68_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n59_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n94_), .c(new_n68_), .O(z12));
  inv1   g59(.a(new_n101_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n56_), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n97_), .O(z13));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n105_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n97_), .O(z14));
  nand2  g67(.a(new_n111_), .b(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand3  g69(.a(new_n110_), .b(new_n105_), .c(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n97_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nor3   g72(.a(x22), .b(x21), .c(x20), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n105_), .c(new_n54_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n97_), .O(z16));
  nand4  g75(.a(new_n118_), .b(new_n105_), .c(new_n47_), .d(new_n54_), .O(new_n121_));
  nand2  g76(.a(new_n94_), .b(new_n48_), .O(new_n122_));
  nand2  g77(.a(new_n119_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(z17));
  nand2  g79(.a(new_n55_), .b(new_n47_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n118_), .c(new_n105_), .O(new_n126_));
  nand2  g81(.a(new_n115_), .b(new_n55_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n54_), .O(new_n128_));
  aoi22  g83(.a(new_n94_), .b(new_n48_), .c(x25), .d(x24), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n128_), .O(z18));
endmodule


