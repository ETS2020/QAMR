// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x25), .b(new_n47_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  or2    g07(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  inv1   g09(.a(x24), .O(new_n55_));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n56_), .b(x20), .c(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n54_), .c(new_n55_), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n60_), .c(new_n53_), .O(z01));
  nor2   g18(.a(new_n62_), .b(new_n60_), .O(z03));
  inv1   g19(.a(new_n50_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  nor2   g27(.a(new_n65_), .b(x13), .O(z05));
  and2   g28(.a(new_n50_), .b(x14), .O(z06));
  nand2  g29(.a(new_n50_), .b(x06), .O(z07));
  inv1   g30(.a(x19), .O(new_n76_));
  nor2   g31(.a(x18), .b(x17), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n78_));
  oai21  g33(.a(new_n57_), .b(new_n78_), .c(new_n54_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x24), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x15), .c(new_n61_), .O(z08));
  inv1   g36(.a(new_n48_), .O(new_n82_));
  inv1   g37(.a(x07), .O(new_n83_));
  nand4  g38(.a(new_n47_), .b(new_n83_), .c(x05), .d(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n82_), .O(z09));
  nand2  g40(.a(new_n52_), .b(new_n47_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n82_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n86_), .c(new_n82_), .O(z11));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n76_), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n86_), .c(new_n82_), .O(z12));
  inv1   g49(.a(new_n86_), .O(new_n95_));
  inv1   g50(.a(x20), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(x19), .c(x18), .d(x17), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  inv1   g55(.a(new_n92_), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n86_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n100_), .c(new_n48_), .O(z14));
  nand2  g59(.a(x25), .b(x15), .O(new_n105_));
  inv1   g60(.a(x22), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n102_), .b(new_n101_), .c(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n52_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n107_), .c(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n105_), .O(z15));
  nand2  g66(.a(new_n108_), .b(x23), .O(new_n112_));
  nand4  g67(.a(new_n102_), .b(new_n101_), .c(new_n54_), .d(new_n106_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n52_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n105_), .O(z16));
  inv1   g71(.a(x21), .O(new_n117_));
  nand4  g72(.a(new_n55_), .b(new_n54_), .c(new_n106_), .d(new_n117_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n97_), .c(new_n52_), .O(new_n119_));
  aoi21  g74(.a(new_n113_), .b(x24), .c(new_n119_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(x15), .c(new_n105_), .O(z17));
  nor2   g76(.a(new_n118_), .b(new_n97_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n61_), .O(new_n123_));
  oai21  g78(.a(new_n118_), .b(new_n97_), .c(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n95_), .O(z18));
endmodule


