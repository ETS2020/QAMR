// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_;
  inv1   g00(.a(x05), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  aoi21  g05(.a(new_n49_), .b(x16), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n58_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n47_), .c(new_n52_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  oai21  g18(.a(x24), .b(x04), .c(x05), .O(new_n64_));
  and2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n62_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n49_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n62_), .b(new_n49_), .O(z03));
  inv1   g24(.a(new_n50_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g33(.a(new_n50_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand2  g35(.a(new_n50_), .b(x06), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n56_), .c(x20), .O(new_n84_));
  oai21  g39(.a(new_n59_), .b(new_n84_), .c(new_n82_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x05), .c(x24), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n52_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n47_), .c(new_n46_), .O(z09));
  and2   g45(.a(new_n88_), .b(new_n64_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n54_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(x05), .b(x04), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nor2   g50(.a(new_n55_), .b(new_n54_), .O(new_n96_));
  or2    g51(.a(new_n96_), .b(new_n83_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n95_), .c(new_n49_), .O(z11));
  nor2   g53(.a(new_n96_), .b(x19), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n94_), .c(new_n88_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n49_), .O(z12));
  inv1   g57(.a(new_n100_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n91_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor3   g62(.a(new_n100_), .b(x21), .c(x20), .O(new_n108_));
  oai21  g63(.a(new_n94_), .b(x24), .c(new_n88_), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n107_), .c(new_n48_), .O(z14));
  inv1   g66(.a(x22), .O(new_n112_));
  or2    g67(.a(new_n108_), .b(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(new_n112_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n91_), .O(z15));
  nand2  g70(.a(new_n114_), .b(x23), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n108_), .c(new_n109_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n116_), .c(new_n48_), .O(z16));
  nand3  g74(.a(new_n117_), .b(new_n108_), .c(new_n47_), .O(new_n120_));
  nor2   g75(.a(x21), .b(x20), .O(new_n121_));
  nand3  g76(.a(new_n117_), .b(new_n121_), .c(new_n103_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(x24), .c(new_n109_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n120_), .c(new_n48_), .O(z17));
  nand4  g79(.a(new_n117_), .b(new_n108_), .c(new_n52_), .d(new_n47_), .O(new_n125_));
  nand2  g80(.a(new_n120_), .b(x25), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n91_), .O(z18));
endmodule


