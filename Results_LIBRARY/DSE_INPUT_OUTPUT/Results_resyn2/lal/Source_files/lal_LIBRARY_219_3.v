// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x12), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n50_), .c(new_n51_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n47_), .c(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  inv1   g21(.a(new_n47_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n46_), .O(z02));
  inv1   g23(.a(x12), .O(new_n69_));
  nand2  g24(.a(new_n60_), .b(new_n69_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(x24), .c(x25), .O(z03));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  nand2  g28(.a(x11), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x09), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x00), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  xnor2a g32(.a(x12), .b(x03), .O(new_n78_));
  inv1   g33(.a(x00), .O(new_n79_));
  nand2  g34(.a(x09), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(x11), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x02), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nor3   g39(.a(new_n84_), .b(new_n67_), .c(x08), .O(z04));
  inv1   g40(.a(x08), .O(new_n86_));
  inv1   g41(.a(x13), .O(new_n87_));
  nand3  g42(.a(new_n47_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z05));
  inv1   g44(.a(x14), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g46(.a(new_n86_), .b(x06), .c(new_n67_), .O(z07));
  inv1   g47(.a(x23), .O(new_n93_));
  nor2   g48(.a(x18), .b(x17), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n55_), .c(x20), .O(new_n95_));
  oai21  g50(.a(new_n58_), .b(new_n95_), .c(new_n93_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(x12), .c(x24), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n51_), .O(z08));
  inv1   g53(.a(x15), .O(new_n99_));
  nand3  g54(.a(new_n47_), .b(new_n99_), .c(new_n62_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n63_), .O(z09));
  nand3  g56(.a(new_n63_), .b(new_n99_), .c(new_n62_), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(x17), .c(new_n47_), .O(z10));
  inv1   g58(.a(new_n100_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n63_), .O(new_n105_));
  nand2  g60(.a(x18), .b(x17), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nor3   g62(.a(new_n107_), .b(new_n105_), .c(new_n94_), .O(z11));
  nand2  g63(.a(new_n106_), .b(new_n55_), .O(new_n109_));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n102_), .c(new_n47_), .O(z12));
  inv1   g67(.a(new_n105_), .O(new_n113_));
  inv1   g68(.a(new_n110_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n52_), .O(new_n115_));
  nand2  g70(.a(new_n110_), .b(x20), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n113_), .O(z13));
  nand2  g72(.a(new_n115_), .b(x21), .O(new_n118_));
  nor3   g73(.a(new_n110_), .b(x21), .c(x20), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n113_), .O(z14));
  nand2  g76(.a(new_n120_), .b(x22), .O(new_n122_));
  inv1   g77(.a(x22), .O(new_n123_));
  nand2  g78(.a(new_n119_), .b(new_n123_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n113_), .O(z15));
  nand2  g80(.a(new_n124_), .b(x23), .O(new_n126_));
  inv1   g81(.a(new_n102_), .O(new_n127_));
  nor2   g82(.a(x21), .b(x20), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n114_), .c(new_n93_), .d(new_n123_), .O(new_n129_));
  and2   g84(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(new_n126_), .c(new_n67_), .O(z16));
  inv1   g86(.a(new_n129_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n50_), .O(new_n133_));
  aoi21  g88(.a(new_n129_), .b(x24), .c(new_n102_), .O(new_n134_));
  aoi21  g89(.a(new_n134_), .b(new_n133_), .c(new_n67_), .O(z17));
  nor2   g90(.a(new_n51_), .b(new_n50_), .O(new_n136_));
  oai21  g91(.a(new_n136_), .b(new_n102_), .c(new_n47_), .O(new_n137_));
  nand4  g92(.a(new_n119_), .b(x25), .c(new_n93_), .d(new_n123_), .O(new_n138_));
  aoi21  g93(.a(new_n129_), .b(new_n51_), .c(x24), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n138_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n137_), .O(z18));
endmodule


