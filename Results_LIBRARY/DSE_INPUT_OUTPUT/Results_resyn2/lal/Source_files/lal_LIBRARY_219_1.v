// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x12), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n47_), .c(new_n50_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n48_), .c(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n59_), .b(x12), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  inv1   g23(.a(new_n48_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  inv1   g26(.a(x02), .O(new_n72_));
  nand2  g27(.a(x11), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x09), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x00), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  xnor2a g31(.a(x12), .b(x03), .O(new_n77_));
  inv1   g32(.a(x00), .O(new_n78_));
  nand2  g33(.a(x09), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x11), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x02), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n76_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n70_), .O(z04));
  nor2   g39(.a(new_n70_), .b(x13), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n69_), .O(z06));
  aoi21  g42(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g43(.a(x12), .O(new_n89_));
  inv1   g44(.a(x23), .O(new_n90_));
  nor2   g45(.a(x18), .b(x17), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n54_), .c(x20), .O(new_n92_));
  oai21  g47(.a(new_n57_), .b(new_n92_), .c(new_n90_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n89_), .c(x24), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(z08));
  nor3   g50(.a(new_n48_), .b(x15), .c(x07), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n62_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z09));
  inv1   g53(.a(x07), .O(new_n99_));
  inv1   g54(.a(x15), .O(new_n100_));
  nand3  g55(.a(new_n61_), .b(new_n100_), .c(new_n99_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x17), .c(new_n69_), .O(z10));
  inv1   g57(.a(new_n91_), .O(new_n103_));
  nand2  g58(.a(x18), .b(x17), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n96_), .c(new_n61_), .d(new_n103_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z11));
  nand2  g61(.a(new_n104_), .b(new_n54_), .O(new_n107_));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n101_), .c(new_n69_), .O(z12));
  inv1   g65(.a(new_n108_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n51_), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(x20), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n112_), .c(new_n96_), .d(new_n61_), .O(z13));
  nand2  g69(.a(new_n112_), .b(x21), .O(new_n115_));
  nor3   g70(.a(new_n108_), .b(x21), .c(x20), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n115_), .c(new_n96_), .d(new_n61_), .O(z14));
  nand2  g73(.a(new_n117_), .b(x22), .O(new_n119_));
  inv1   g74(.a(x22), .O(new_n120_));
  nand2  g75(.a(new_n116_), .b(new_n120_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n119_), .c(new_n96_), .d(new_n61_), .O(z15));
  nand2  g77(.a(new_n121_), .b(x23), .O(new_n123_));
  inv1   g78(.a(new_n101_), .O(new_n124_));
  nor2   g79(.a(x21), .b(x20), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n111_), .c(new_n90_), .d(new_n120_), .O(new_n126_));
  and2   g81(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n123_), .c(new_n48_), .O(z16));
  inv1   g83(.a(new_n126_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  aoi21  g85(.a(new_n126_), .b(x24), .c(new_n101_), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(new_n130_), .c(new_n48_), .O(z17));
  nor2   g87(.a(new_n50_), .b(new_n47_), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(new_n101_), .c(new_n69_), .O(new_n134_));
  nand4  g89(.a(new_n116_), .b(x25), .c(new_n90_), .d(new_n120_), .O(new_n135_));
  aoi21  g90(.a(new_n126_), .b(new_n50_), .c(x24), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n134_), .O(z18));
endmodule


