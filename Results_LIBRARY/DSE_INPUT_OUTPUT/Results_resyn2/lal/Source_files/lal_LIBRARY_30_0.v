// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x23), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  aoi21  g06(.a(new_n51_), .b(new_n50_), .c(x20), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  oai21  g08(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  aoi21  g09(.a(x25), .b(x23), .c(x24), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  oai21  g14(.a(new_n54_), .b(x25), .c(new_n59_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(new_n47_), .b(new_n61_), .O(z02));
  nor2   g17(.a(new_n47_), .b(x25), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  aoi21  g19(.a(new_n54_), .b(x24), .c(new_n64_), .O(z03));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g26(.a(new_n47_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  aoi21  g28(.a(new_n71_), .b(new_n68_), .c(new_n73_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n72_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n72_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g33(.a(x24), .O(new_n79_));
  inv1   g34(.a(x20), .O(new_n80_));
  inv1   g35(.a(x17), .O(new_n81_));
  inv1   g36(.a(x18), .O(new_n82_));
  nand3  g37(.a(new_n50_), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  inv1   g39(.a(new_n53_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n84_), .c(x23), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n79_), .c(new_n63_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n72_), .O(z09));
  nand2  g45(.a(new_n88_), .b(new_n57_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n47_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n81_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  inv1   g49(.a(new_n51_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n91_), .c(new_n72_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n91_), .c(new_n47_), .O(z12));
  nor2   g57(.a(new_n99_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n99_), .b(x20), .c(new_n91_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n47_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  inv1   g62(.a(x21), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(new_n108_), .c(new_n91_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n107_), .c(new_n47_), .O(z14));
  oai21  g65(.a(new_n104_), .b(x21), .c(x22), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n103_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n92_), .O(z15));
  nand4  g69(.a(new_n112_), .b(new_n103_), .c(x24), .d(new_n49_), .O(new_n115_));
  nand2  g70(.a(new_n113_), .b(x23), .O(new_n116_));
  nand2  g71(.a(new_n91_), .b(new_n72_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(z16));
  oai21  g73(.a(new_n113_), .b(x23), .c(x24), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n117_), .O(z17));
  nand3  g75(.a(new_n88_), .b(new_n63_), .c(new_n57_), .O(z18));
endmodule


