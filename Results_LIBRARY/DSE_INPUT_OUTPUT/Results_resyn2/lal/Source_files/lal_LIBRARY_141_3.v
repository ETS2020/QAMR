// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  oai21  g12(.a(x18), .b(x17), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x21), .O(new_n59_));
  inv1   g14(.a(x22), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n58_), .c(x23), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n47_), .c(new_n48_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x25), .c(new_n56_), .O(z01));
  nor2   g19(.a(new_n49_), .b(new_n46_), .O(z02));
  inv1   g20(.a(x25), .O(new_n66_));
  oai21  g21(.a(new_n62_), .b(new_n48_), .c(new_n66_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n50_), .O(z03));
  nor2   g23(.a(new_n49_), .b(x08), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  and2   g29(.a(new_n74_), .b(new_n69_), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand2  g31(.a(new_n69_), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand2  g33(.a(new_n69_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand2  g35(.a(new_n69_), .b(x06), .O(z07));
  nand2  g36(.a(new_n62_), .b(new_n47_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n66_), .O(z08));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n54_), .c(new_n50_), .O(z09));
  inv1   g42(.a(x17), .O(new_n88_));
  nand2  g43(.a(new_n85_), .b(new_n54_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n50_), .c(new_n88_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  nor2   g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor4   g50(.a(new_n95_), .b(new_n89_), .c(new_n93_), .d(new_n49_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n94_), .b(new_n47_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n90_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(z12));
  nor2   g55(.a(new_n97_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n97_), .b(x20), .c(new_n89_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n102_), .c(new_n49_), .O(z13));
  nor2   g59(.a(new_n94_), .b(x20), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(x24), .c(x19), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x21), .O(new_n107_));
  nand4  g62(.a(new_n59_), .b(new_n57_), .c(x18), .d(x17), .O(new_n108_));
  nor3   g63(.a(new_n108_), .b(x24), .c(new_n47_), .O(new_n109_));
  nor2   g64(.a(new_n90_), .b(new_n49_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n107_), .O(z14));
  inv1   g67(.a(new_n110_), .O(new_n113_));
  inv1   g68(.a(new_n108_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(x24), .c(x19), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x22), .O(new_n116_));
  nand2  g71(.a(new_n109_), .b(new_n60_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n113_), .O(z15));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n105_), .c(x24), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n47_), .c(x23), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n114_), .c(new_n48_), .d(x19), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n113_), .O(z16));
  nor3   g79(.a(x23), .b(x22), .c(x21), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n101_), .c(new_n89_), .O(new_n126_));
  nand2  g81(.a(x24), .b(new_n47_), .O(new_n127_));
  oai21  g82(.a(new_n126_), .b(x24), .c(new_n127_), .O(z17));
  nand2  g83(.a(new_n122_), .b(new_n66_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n108_), .c(new_n48_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(x19), .O(new_n131_));
  nand2  g86(.a(new_n123_), .b(x25), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n90_), .O(z18));
endmodule


