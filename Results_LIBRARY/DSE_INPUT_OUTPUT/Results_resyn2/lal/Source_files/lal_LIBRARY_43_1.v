// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_;
  inv1   g00(.a(x23), .O(new_n46_));
  nand2  g01(.a(x24), .b(new_n46_), .O(new_n47_));
  and2   g02(.a(new_n47_), .b(x16), .O(z02));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(z08));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand4  g11(.a(new_n56_), .b(z08), .c(new_n47_), .d(new_n52_), .O(z01));
  nand2  g12(.a(z08), .b(new_n47_), .O(z03));
  xnor2a g13(.a(x11), .b(x02), .O(new_n59_));
  xnor2a g14(.a(x12), .b(x03), .O(new_n60_));
  xnor2a g15(.a(x09), .b(x00), .O(new_n61_));
  xnor2a g16(.a(x10), .b(x01), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x08), .c(new_n47_), .O(z04));
  oai21  g20(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  and2   g21(.a(new_n50_), .b(x14), .O(z06));
  nand2  g22(.a(new_n50_), .b(x06), .O(z07));
  nor2   g23(.a(x15), .b(x07), .O(new_n69_));
  and2   g24(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(x05), .c(x04), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z09));
  inv1   g27(.a(x17), .O(new_n73_));
  nand3  g28(.a(new_n70_), .b(new_n56_), .c(new_n73_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z10));
  nand2  g30(.a(new_n69_), .b(new_n56_), .O(new_n76_));
  xnor2a g31(.a(x18), .b(x17), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n76_), .c(new_n47_), .O(z11));
  inv1   g33(.a(new_n76_), .O(new_n79_));
  nand3  g34(.a(x19), .b(x18), .c(x17), .O(new_n80_));
  inv1   g35(.a(x19), .O(new_n81_));
  nand2  g36(.a(x18), .b(x17), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n80_), .c(new_n79_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n47_), .O(z12));
  nand2  g40(.a(new_n80_), .b(x20), .O(new_n86_));
  inv1   g41(.a(x20), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(x19), .c(x18), .d(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n86_), .c(new_n49_), .O(z13));
  inv1   g46(.a(x21), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n88_), .b(x21), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n93_), .c(new_n70_), .d(new_n56_), .O(z14));
  oai21  g50(.a(new_n88_), .b(x21), .c(x22), .O(new_n96_));
  nor2   g51(.a(x22), .b(x21), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n89_), .c(new_n76_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n96_), .c(new_n49_), .O(z15));
  nand2  g54(.a(new_n97_), .b(new_n89_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x23), .O(new_n101_));
  nand2  g56(.a(new_n76_), .b(new_n47_), .O(new_n102_));
  nand3  g57(.a(new_n97_), .b(new_n53_), .c(new_n46_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n89_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n102_), .c(new_n101_), .O(z16));
  inv1   g61(.a(new_n80_), .O(new_n107_));
  nor2   g62(.a(x23), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n97_), .c(new_n107_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n79_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n53_), .O(new_n111_));
  nand2  g66(.a(x24), .b(x23), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(z17));
  nand2  g68(.a(new_n97_), .b(new_n54_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n88_), .c(new_n53_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n46_), .O(new_n116_));
  oai21  g71(.a(new_n103_), .b(new_n88_), .c(x25), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n79_), .O(z18));
endmodule


