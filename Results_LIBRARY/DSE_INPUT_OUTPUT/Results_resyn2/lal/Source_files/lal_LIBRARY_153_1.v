// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(x24), .O(new_n55_));
  aoi21  g10(.a(x05), .b(x04), .c(x07), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nor2   g14(.a(x18), .b(x17), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(x20), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  oai22  g17(.a(new_n62_), .b(x25), .c(new_n58_), .d(new_n48_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n55_), .b(new_n49_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n62_), .c(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  nor3   g29(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  oai21  g33(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x24), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n51_), .c(new_n48_), .O(z08));
  nand2  g36(.a(x05), .b(x04), .O(new_n82_));
  nor4   g37(.a(new_n82_), .b(x20), .c(x15), .d(x07), .O(z09));
  inv1   g38(.a(x17), .O(new_n84_));
  nor3   g39(.a(x20), .b(x15), .c(x07), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n86_), .c(new_n60_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n87_), .O(z13));
  aoi21  g49(.a(new_n90_), .b(new_n59_), .c(z13), .O(z12));
  inv1   g50(.a(x15), .O(new_n96_));
  inv1   g51(.a(x21), .O(new_n97_));
  inv1   g52(.a(new_n93_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g54(.a(new_n93_), .b(x21), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n56_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n96_), .O(z14));
  nand2  g58(.a(new_n99_), .b(x22), .O(new_n104_));
  nor2   g59(.a(x22), .b(x21), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n87_), .O(z15));
  nand2  g62(.a(new_n106_), .b(x23), .O(new_n108_));
  nand3  g63(.a(new_n105_), .b(new_n98_), .c(new_n52_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n87_), .O(z16));
  nand2  g65(.a(new_n109_), .b(x24), .O(new_n111_));
  nor2   g66(.a(x24), .b(x23), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n105_), .c(new_n98_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n56_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n96_), .O(z17));
  aoi21  g71(.a(new_n113_), .b(x25), .c(new_n86_), .O(new_n117_));
  oai21  g72(.a(new_n113_), .b(x25), .c(new_n117_), .O(z18));
endmodule


