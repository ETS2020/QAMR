// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n81_, new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  oai21  g21(.a(new_n62_), .b(x25), .c(new_n66_), .O(z01));
  inv1   g22(.a(x25), .O(new_n68_));
  nand3  g23(.a(new_n60_), .b(new_n58_), .c(x24), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(new_n48_), .O(z08));
  inv1   g25(.a(z08), .O(z03));
  inv1   g26(.a(new_n49_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  nand2  g35(.a(new_n49_), .b(x14), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  inv1   g37(.a(x08), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g39(.a(x15), .O(new_n85_));
  nand3  g40(.a(new_n46_), .b(new_n85_), .c(new_n63_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(new_n64_), .O(z09));
  nand3  g42(.a(new_n64_), .b(new_n85_), .c(new_n63_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(x17), .c(new_n46_), .O(z10));
  xnor2a g44(.a(x18), .b(x17), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n88_), .c(new_n46_), .O(z11));
  inv1   g46(.a(new_n86_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  aoi21  g50(.a(x18), .b(x17), .c(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n93_), .O(z12));
  nor2   g52(.a(new_n94_), .b(x20), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  aoi21  g54(.a(new_n94_), .b(x20), .c(new_n88_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n48_), .O(z13));
  inv1   g56(.a(new_n93_), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  inv1   g58(.a(x21), .O(new_n104_));
  nand2  g59(.a(new_n98_), .b(new_n104_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n98_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n102_), .O(z15));
  nand2  g65(.a(new_n88_), .b(new_n46_), .O(new_n111_));
  nand3  g66(.a(new_n109_), .b(new_n52_), .c(x23), .O(new_n112_));
  nand3  g67(.a(new_n108_), .b(new_n98_), .c(new_n51_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(z16));
  nand2  g69(.a(new_n109_), .b(new_n52_), .O(new_n115_));
  nand3  g70(.a(new_n108_), .b(new_n98_), .c(x24), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n51_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n111_), .O(z17));
  inv1   g73(.a(new_n88_), .O(new_n119_));
  inv1   g74(.a(new_n113_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n68_), .c(new_n52_), .O(new_n121_));
  oai21  g76(.a(new_n109_), .b(x24), .c(x25), .O(new_n122_));
  oai21  g77(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n122_), .c(new_n121_), .d(new_n119_), .O(z18));
endmodule


