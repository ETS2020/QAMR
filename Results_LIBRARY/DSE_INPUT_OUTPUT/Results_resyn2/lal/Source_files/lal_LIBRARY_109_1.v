// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n47_), .c(new_n50_), .O(new_n60_));
  nand2  g15(.a(x25), .b(new_n47_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(new_n46_), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n60_), .O(z01));
  nand2  g21(.a(new_n59_), .b(x16), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  nor2   g23(.a(new_n47_), .b(x16), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(new_n70_));
  inv1   g25(.a(x08), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n70_), .O(z04));
  nand2  g33(.a(new_n70_), .b(new_n71_), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(x13), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor2   g36(.a(new_n79_), .b(new_n81_), .O(z06));
  nand3  g37(.a(new_n70_), .b(new_n71_), .c(x06), .O(z07));
  inv1   g38(.a(x23), .O(new_n84_));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n54_), .c(x20), .O(new_n86_));
  oai21  g41(.a(new_n57_), .b(new_n86_), .c(new_n84_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x24), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n50_), .c(new_n69_), .O(z08));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n63_), .c(new_n70_), .O(z09));
  nand3  g47(.a(new_n63_), .b(new_n90_), .c(new_n62_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(x17), .c(new_n70_), .O(z10));
  inv1   g49(.a(new_n85_), .O(new_n95_));
  nor2   g50(.a(new_n93_), .b(new_n69_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  aoi21  g55(.a(new_n97_), .b(new_n54_), .c(new_n93_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n70_), .O(z12));
  nor2   g58(.a(new_n100_), .b(x20), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  aoi21  g60(.a(new_n100_), .b(x20), .c(new_n93_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n69_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand2  g64(.a(new_n104_), .b(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n96_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n104_), .b(new_n113_), .c(new_n109_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n96_), .O(z15));
  nand2  g70(.a(new_n114_), .b(x23), .O(new_n116_));
  nand4  g71(.a(new_n104_), .b(new_n84_), .c(new_n113_), .d(new_n109_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n96_), .O(z16));
  inv1   g73(.a(new_n117_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n47_), .O(new_n120_));
  nand2  g75(.a(new_n117_), .b(x24), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n96_), .O(z17));
  nand3  g77(.a(new_n119_), .b(new_n50_), .c(new_n47_), .O(new_n123_));
  nand2  g78(.a(new_n117_), .b(x25), .O(new_n124_));
  nand2  g79(.a(new_n50_), .b(x16), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(x24), .c(new_n93_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z18));
  buf    g82(.a(x16), .O(z02));
endmodule


