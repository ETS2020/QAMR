// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_;
  inv1   g00(.a(x16), .O(new_n46_));
  nor2   g01(.a(x24), .b(x07), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n53_));
  inv1   g08(.a(x21), .O(new_n54_));
  inv1   g09(.a(x22), .O(new_n55_));
  nor2   g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g11(.a(new_n53_), .b(x20), .c(new_n56_), .O(new_n57_));
  nor2   g12(.a(x25), .b(x23), .O(new_n58_));
  and2   g13(.a(x05), .b(x04), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n52_), .c(new_n51_), .O(z01));
  inv1   g16(.a(x23), .O(new_n62_));
  nand2  g17(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  inv1   g18(.a(x25), .O(new_n64_));
  inv1   g19(.a(new_n47_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n63_), .b(x24), .c(new_n66_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  and2   g27(.a(new_n72_), .b(new_n49_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n65_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n65_), .O(z06));
  nand2  g31(.a(new_n49_), .b(x06), .O(z07));
  nand2  g32(.a(new_n63_), .b(x24), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n65_), .c(new_n64_), .O(z08));
  inv1   g34(.a(x15), .O(new_n80_));
  nand2  g35(.a(new_n59_), .b(new_n80_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x24), .c(x07), .O(z09));
  inv1   g37(.a(x17), .O(new_n83_));
  nor2   g38(.a(new_n59_), .b(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x07), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n84_), .b(x24), .c(new_n51_), .O(new_n88_));
  and2   g43(.a(x18), .b(x17), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z11));
  or2    g45(.a(new_n89_), .b(x19), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n84_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x24), .c(x07), .O(z12));
  nor2   g49(.a(new_n92_), .b(x20), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x24), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n51_), .O(z13));
  inv1   g55(.a(new_n88_), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(x21), .O(new_n102_));
  nand2  g57(.a(new_n95_), .b(new_n54_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z14));
  nand3  g59(.a(new_n95_), .b(new_n55_), .c(new_n54_), .O(new_n105_));
  nand2  g60(.a(new_n103_), .b(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n101_), .O(z15));
  inv1   g62(.a(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n62_), .O(new_n109_));
  nand2  g64(.a(new_n105_), .b(x23), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n101_), .O(z16));
  nand4  g66(.a(x24), .b(new_n62_), .c(new_n55_), .d(new_n54_), .O(new_n112_));
  inv1   g67(.a(x20), .O(new_n113_));
  nand3  g68(.a(new_n89_), .b(new_n113_), .c(x19), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n84_), .c(new_n51_), .O(z17));
  nand2  g71(.a(new_n101_), .b(new_n64_), .O(z18));
endmodule


