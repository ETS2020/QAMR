// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n47_), .c(new_n61_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(z01));
  nand2  g20(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g21(.a(x23), .O(new_n67_));
  nor2   g22(.a(x18), .b(x17), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n54_), .c(x20), .O(new_n69_));
  oai21  g24(.a(new_n57_), .b(new_n69_), .c(new_n67_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(x24), .c(x25), .O(z03));
  inv1   g26(.a(x08), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n47_), .O(z04));
  inv1   g34(.a(x13), .O(new_n80_));
  nand3  g35(.a(new_n47_), .b(new_n80_), .c(new_n72_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  inv1   g37(.a(x14), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g39(.a(new_n47_), .b(new_n72_), .c(x06), .O(z07));
  aoi22  g40(.a(new_n59_), .b(new_n50_), .c(x24), .d(x23), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n61_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n62_), .c(new_n47_), .O(z09));
  nand3  g44(.a(new_n62_), .b(new_n87_), .c(new_n61_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g46(.a(new_n64_), .b(new_n87_), .O(new_n92_));
  inv1   g47(.a(new_n68_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n92_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  aoi21  g52(.a(new_n94_), .b(new_n54_), .c(new_n90_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(z12));
  inv1   g55(.a(new_n92_), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(x20), .O(new_n102_));
  nand4  g57(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand2  g59(.a(new_n103_), .b(x21), .O(new_n105_));
  inv1   g60(.a(new_n97_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n101_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  nand3  g66(.a(new_n107_), .b(new_n106_), .c(new_n111_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n101_), .O(z15));
  nand2  g68(.a(new_n90_), .b(new_n47_), .O(new_n114_));
  inv1   g69(.a(x24), .O(new_n115_));
  nand3  g70(.a(new_n112_), .b(new_n115_), .c(x23), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n107_), .c(new_n106_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(z16));
  inv1   g74(.a(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n115_), .O(new_n121_));
  aoi21  g76(.a(new_n118_), .b(x24), .c(new_n90_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(z17));
  nand2  g78(.a(x25), .b(x23), .O(new_n124_));
  nor2   g79(.a(x25), .b(x21), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n117_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n103_), .c(new_n124_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n115_), .O(new_n128_));
  oai21  g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n90_), .O(new_n130_));
  nand2  g85(.a(new_n115_), .b(new_n111_), .O(new_n131_));
  nor2   g86(.a(new_n50_), .b(x23), .O(new_n132_));
  oai21  g87(.a(new_n131_), .b(new_n108_), .c(new_n132_), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n130_), .c(new_n128_), .O(z18));
endmodule


