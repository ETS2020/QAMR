// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n82_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n57_), .c(x24), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n51_), .c(new_n50_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x07), .c(new_n48_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n61_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nor2   g22(.a(new_n51_), .b(x23), .O(new_n68_));
  nor2   g23(.a(x18), .b(x17), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n55_), .c(x20), .O(new_n70_));
  oai21  g25(.a(new_n58_), .b(new_n70_), .c(new_n50_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(x24), .c(new_n68_), .O(z03));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n48_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g35(.a(new_n48_), .O(new_n81_));
  nand2  g36(.a(x14), .b(new_n46_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n81_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n81_), .O(z07));
  inv1   g39(.a(new_n68_), .O(new_n85_));
  aoi21  g40(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x23), .c(x24), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(z08));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n63_), .c(new_n48_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n62_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n48_), .O(z10));
  inv1   g48(.a(new_n69_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n92_), .c(new_n48_), .O(z11));
  nand3  g52(.a(new_n89_), .b(new_n62_), .c(new_n48_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(new_n55_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n98_), .O(z12));
  inv1   g57(.a(new_n98_), .O(new_n103_));
  inv1   g58(.a(new_n99_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand2  g60(.a(new_n99_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n104_), .c(new_n92_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n81_), .O(z14));
  nand2  g66(.a(new_n109_), .b(new_n104_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand3  g69(.a(new_n109_), .b(new_n104_), .c(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n103_), .O(z15));
  nand2  g71(.a(new_n92_), .b(new_n48_), .O(new_n117_));
  nand3  g72(.a(new_n115_), .b(x24), .c(x23), .O(new_n118_));
  inv1   g73(.a(new_n115_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n50_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(z16));
  inv1   g76(.a(new_n92_), .O(new_n122_));
  nand2  g77(.a(new_n115_), .b(x24), .O(new_n123_));
  nor2   g78(.a(x24), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n109_), .c(new_n104_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n123_), .c(new_n122_), .d(new_n50_), .O(z17));
  xor2a  g81(.a(new_n125_), .b(x25), .O(new_n127_));
  oai21  g82(.a(x25), .b(new_n47_), .c(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n122_), .O(z18));
endmodule


