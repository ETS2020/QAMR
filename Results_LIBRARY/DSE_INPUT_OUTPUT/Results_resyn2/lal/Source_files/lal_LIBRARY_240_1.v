// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_, new_n73_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  nand3  g06(.a(x22), .b(x21), .c(x20), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x24), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(x07), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nand2  g15(.a(new_n48_), .b(new_n60_), .O(z02));
  nand2  g16(.a(new_n55_), .b(new_n48_), .O(z03));
  xnor2a g17(.a(x12), .b(x03), .O(new_n63_));
  xnor2a g18(.a(x11), .b(x02), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n48_), .O(z04));
  oai21  g24(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g25(.a(x14), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n48_), .O(z06));
  inv1   g27(.a(new_n48_), .O(new_n73_));
  aoi21  g28(.a(new_n46_), .b(x06), .c(new_n73_), .O(z07));
  oai21  g29(.a(new_n53_), .b(new_n47_), .c(x24), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z08));
  nor2   g31(.a(x15), .b(x07), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n57_), .c(new_n48_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z09));
  inv1   g34(.a(x17), .O(new_n80_));
  nand4  g35(.a(new_n77_), .b(new_n56_), .c(new_n48_), .d(new_n80_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z10));
  nand2  g37(.a(new_n77_), .b(new_n56_), .O(new_n83_));
  xnor2a g38(.a(x18), .b(x17), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n83_), .c(new_n48_), .O(z11));
  nand3  g40(.a(new_n77_), .b(new_n56_), .c(new_n48_), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n47_), .O(new_n88_));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n86_), .O(z12));
  inv1   g46(.a(new_n83_), .O(new_n92_));
  inv1   g47(.a(x20), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  oai21  g49(.a(new_n73_), .b(x20), .c(new_n89_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z13));
  inv1   g51(.a(x21), .O(new_n97_));
  nor2   g52(.a(new_n89_), .b(x20), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g54(.a(new_n73_), .b(x21), .c(new_n94_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n92_), .O(z14));
  inv1   g56(.a(x24), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n77_), .b(new_n56_), .c(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n102_), .c(x19), .O(new_n105_));
  oai21  g60(.a(new_n94_), .b(x21), .c(x22), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n87_), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n107_), .c(new_n83_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n106_), .c(new_n105_), .O(z15));
  nor2   g65(.a(new_n51_), .b(x19), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n83_), .c(new_n48_), .O(new_n112_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(x23), .O(new_n113_));
  nand2  g68(.a(new_n103_), .b(new_n97_), .O(new_n114_));
  nand3  g69(.a(new_n93_), .b(x18), .c(x17), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n114_), .c(new_n51_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n113_), .c(x19), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n112_), .O(z16));
  nor2   g73(.a(x23), .b(x22), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n98_), .c(new_n97_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x24), .O(new_n121_));
  nor2   g76(.a(x24), .b(x23), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n108_), .c(new_n107_), .d(x19), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n92_), .O(z17));
  nand2  g79(.a(new_n94_), .b(new_n50_), .O(new_n125_));
  nand3  g80(.a(new_n107_), .b(new_n50_), .c(new_n51_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x19), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n102_), .O(new_n128_));
  oai21  g83(.a(x25), .b(x24), .c(new_n47_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n83_), .O(new_n130_));
  nand3  g85(.a(new_n122_), .b(new_n108_), .c(new_n107_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(x25), .c(x19), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n130_), .c(new_n128_), .O(z18));
endmodule


