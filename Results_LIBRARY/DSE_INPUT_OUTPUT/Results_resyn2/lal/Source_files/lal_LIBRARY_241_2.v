// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x23), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x23), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g16(.a(x25), .b(new_n59_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(new_n48_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n61_), .b(new_n58_), .c(new_n66_), .O(z01));
  nand2  g22(.a(new_n62_), .b(new_n48_), .O(new_n68_));
  oai21  g23(.a(new_n61_), .b(new_n58_), .c(new_n68_), .O(z03));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g32(.a(new_n49_), .O(new_n78_));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  nand2  g36(.a(new_n56_), .b(new_n52_), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(x24), .c(x22), .d(x21), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n60_), .c(new_n59_), .O(z08));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n49_), .c(x05), .d(x04), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z09));
  nand4  g42(.a(new_n85_), .b(new_n64_), .c(new_n49_), .d(new_n53_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  inv1   g44(.a(x15), .O(new_n90_));
  nand3  g45(.a(new_n64_), .b(new_n90_), .c(new_n63_), .O(new_n91_));
  nand2  g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n91_), .c(new_n49_), .O(z11));
  nand3  g50(.a(new_n85_), .b(new_n64_), .c(new_n49_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n93_), .b(new_n55_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n96_), .O(z12));
  nand4  g55(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  aoi21  g56(.a(new_n97_), .b(x20), .c(new_n91_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n78_), .O(z13));
  inv1   g58(.a(new_n96_), .O(new_n104_));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(x19), .c(x18), .d(x17), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  inv1   g64(.a(new_n97_), .O(new_n110_));
  nand3  g65(.a(new_n106_), .b(new_n110_), .c(new_n109_), .O(new_n111_));
  nand2  g66(.a(new_n107_), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n104_), .O(z15));
  inv1   g68(.a(new_n91_), .O(new_n114_));
  nand2  g69(.a(x24), .b(new_n109_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n107_), .c(x23), .O(new_n116_));
  nand4  g71(.a(new_n106_), .b(new_n110_), .c(new_n59_), .d(new_n109_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n114_), .O(z16));
  nand2  g73(.a(new_n117_), .b(x24), .O(new_n119_));
  nand2  g74(.a(new_n91_), .b(new_n49_), .O(new_n120_));
  inv1   g75(.a(new_n101_), .O(new_n121_));
  inv1   g76(.a(x21), .O(new_n122_));
  nand3  g77(.a(new_n48_), .b(new_n109_), .c(new_n122_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n121_), .c(new_n59_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n120_), .c(new_n119_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n101_), .c(x25), .O(new_n127_));
  nand3  g82(.a(new_n124_), .b(new_n121_), .c(new_n60_), .O(new_n128_));
  nand2  g83(.a(new_n60_), .b(x24), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(x23), .c(new_n91_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n128_), .c(new_n127_), .O(z18));
endmodule


