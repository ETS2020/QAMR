// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(x08), .O(new_n48_));
  nand2  g03(.a(new_n46_), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  nor3   g12(.a(x19), .b(x18), .c(x17), .O(new_n58_));
  and2   g13(.a(x22), .b(x21), .O(new_n59_));
  oai21  g14(.a(new_n58_), .b(x20), .c(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n56_), .c(new_n57_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x25), .c(new_n55_), .O(z01));
  nor2   g17(.a(new_n61_), .b(x25), .O(z03));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n49_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x08), .c(new_n46_), .O(z06));
  nand2  g28(.a(new_n50_), .b(x06), .O(z07));
  inv1   g29(.a(x25), .O(new_n75_));
  inv1   g30(.a(new_n46_), .O(new_n76_));
  inv1   g31(.a(x20), .O(new_n77_));
  inv1   g32(.a(new_n58_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g34(.a(new_n59_), .b(new_n79_), .c(x24), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n75_), .c(new_n76_), .O(z08));
  inv1   g36(.a(x15), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n52_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n53_), .c(new_n46_), .O(z09));
  nand3  g39(.a(new_n53_), .b(new_n82_), .c(new_n52_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n46_), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  inv1   g42(.a(new_n85_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n46_), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n89_), .c(new_n87_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g48(.a(x19), .O(new_n94_));
  nand2  g49(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n93_), .c(new_n88_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n46_), .O(z12));
  nor2   g52(.a(new_n93_), .b(x20), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  aoi21  g54(.a(new_n93_), .b(x20), .c(new_n85_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n76_), .O(z13));
  inv1   g56(.a(new_n89_), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  inv1   g58(.a(x21), .O(new_n104_));
  nand2  g59(.a(new_n98_), .b(new_n104_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n98_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n102_), .O(z15));
  nand3  g65(.a(new_n108_), .b(new_n98_), .c(new_n57_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x23), .O(new_n112_));
  nand3  g67(.a(new_n108_), .b(new_n98_), .c(new_n56_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n88_), .O(z16));
  aoi21  g69(.a(new_n113_), .b(x24), .c(new_n85_), .O(new_n115_));
  oai21  g70(.a(new_n113_), .b(x24), .c(new_n115_), .O(z17));
  inv1   g71(.a(new_n113_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n75_), .c(new_n57_), .O(new_n118_));
  nand2  g73(.a(new_n111_), .b(x25), .O(new_n119_));
  oai21  g74(.a(x25), .b(x24), .c(x23), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n119_), .c(new_n118_), .d(new_n88_), .O(z18));
endmodule


