// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n115_;
  nand2  g00(.a(x15), .b(x04), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(x08), .O(new_n48_));
  nand2  g03(.a(new_n46_), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(new_n46_), .O(new_n52_));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  or2    g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g09(.a(x23), .O(new_n55_));
  inv1   g10(.a(x24), .O(new_n56_));
  nor3   g11(.a(x19), .b(x18), .c(x17), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  oai21  g15(.a(new_n57_), .b(x20), .c(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n55_), .c(new_n56_), .O(new_n62_));
  inv1   g17(.a(x25), .O(new_n63_));
  nand2  g18(.a(new_n46_), .b(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n62_), .c(new_n54_), .O(z01));
  nor2   g20(.a(new_n64_), .b(new_n62_), .O(z03));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n49_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n46_), .O(z06));
  nand2  g31(.a(new_n50_), .b(x06), .O(z07));
  nand2  g32(.a(new_n61_), .b(new_n55_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n46_), .c(new_n63_), .O(z08));
  inv1   g35(.a(x04), .O(new_n81_));
  inv1   g36(.a(x15), .O(new_n82_));
  inv1   g37(.a(x07), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x05), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(z09));
  inv1   g40(.a(x17), .O(new_n86_));
  nand2  g41(.a(new_n53_), .b(new_n82_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xnor2a g45(.a(x18), .b(x17), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n87_), .c(new_n46_), .O(z11));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  xor2a  g48(.a(new_n93_), .b(x19), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n87_), .c(new_n46_), .O(z12));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(x20), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand2  g53(.a(new_n96_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n88_), .O(z13));
  nand2  g55(.a(new_n98_), .b(x21), .O(new_n101_));
  aoi21  g56(.a(new_n97_), .b(new_n58_), .c(new_n87_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n52_), .O(z14));
  oai21  g58(.a(new_n98_), .b(x21), .c(x22), .O(new_n104_));
  nand3  g59(.a(new_n97_), .b(new_n59_), .c(new_n58_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n88_), .O(z15));
  nand2  g61(.a(new_n105_), .b(x23), .O(new_n107_));
  inv1   g62(.a(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n55_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n88_), .O(z16));
  oai21  g65(.a(new_n105_), .b(x23), .c(x24), .O(new_n111_));
  nor2   g66(.a(x24), .b(x23), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n97_), .c(new_n59_), .d(new_n58_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n88_), .O(z17));
  aoi21  g69(.a(new_n113_), .b(x25), .c(new_n87_), .O(new_n115_));
  oai21  g70(.a(new_n113_), .b(x25), .c(new_n115_), .O(z18));
endmodule


