// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x07), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n58_), .b(new_n48_), .c(new_n50_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n48_), .O(z04));
  nand2  g29(.a(new_n48_), .b(new_n46_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(x13), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n75_), .b(new_n77_), .O(z06));
  nand3  g33(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  aoi22  g34(.a(new_n58_), .b(new_n50_), .c(new_n47_), .d(x07), .O(z08));
  inv1   g35(.a(x07), .O(new_n81_));
  nand3  g36(.a(new_n61_), .b(new_n47_), .c(new_n81_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand2  g38(.a(new_n60_), .b(new_n52_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n81_), .c(x15), .O(z10));
  nor2   g40(.a(x18), .b(x17), .O(new_n86_));
  nand2  g41(.a(new_n62_), .b(new_n47_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z11));
  nand2  g45(.a(new_n88_), .b(new_n54_), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n60_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n81_), .c(x15), .O(z12));
  inv1   g49(.a(new_n87_), .O(new_n95_));
  inv1   g50(.a(new_n92_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n51_), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n96_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n60_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n81_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(z14));
  nand2  g60(.a(new_n102_), .b(x22), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n96_), .c(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n60_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n81_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(z15));
  nand2  g66(.a(new_n108_), .b(x23), .O(new_n112_));
  nor2   g67(.a(x23), .b(x22), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n101_), .c(new_n96_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n60_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n81_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n47_), .O(z16));
  nand2  g72(.a(new_n114_), .b(x24), .O(new_n118_));
  inv1   g73(.a(x24), .O(new_n119_));
  nand4  g74(.a(new_n113_), .b(new_n101_), .c(new_n96_), .d(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n95_), .O(z17));
  inv1   g76(.a(new_n120_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n50_), .O(new_n123_));
  nand2  g78(.a(new_n120_), .b(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n95_), .O(z18));
endmodule


