// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_;
  nand2  g00(.a(x20), .b(x19), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  nand2  g06(.a(x22), .b(x21), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(x19), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nor2   g10(.a(x18), .b(x17), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n55_), .c(x20), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n54_), .c(new_n51_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nor2   g14(.a(x25), .b(x24), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  oai21  g18(.a(new_n58_), .b(x25), .c(new_n63_), .O(z01));
  inv1   g19(.a(x24), .O(new_n65_));
  nand2  g20(.a(new_n46_), .b(new_n65_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n58_), .c(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n46_), .O(z04));
  inv1   g29(.a(x13), .O(new_n75_));
  nand2  g30(.a(new_n49_), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x08), .c(new_n46_), .O(z06));
  nand2  g34(.a(new_n49_), .b(x06), .O(z07));
  inv1   g35(.a(x17), .O(new_n81_));
  inv1   g36(.a(x18), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x19), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n53_), .c(x23), .O(new_n85_));
  oai21  g40(.a(new_n52_), .b(new_n65_), .c(new_n55_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x20), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n85_), .b(new_n65_), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n59_), .c(new_n46_), .O(z09));
  nand3  g47(.a(new_n59_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n48_), .c(x17), .O(z10));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n83_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n93_), .c(new_n46_), .O(z11));
  aoi21  g52(.a(x18), .b(x17), .c(x19), .O(new_n98_));
  inv1   g53(.a(new_n93_), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n98_), .c(new_n46_), .O(z12));
  inv1   g57(.a(x20), .O(new_n103_));
  nand3  g58(.a(new_n100_), .b(new_n99_), .c(new_n103_), .O(z13));
  oai21  g59(.a(new_n100_), .b(x20), .c(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n103_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n105_), .c(new_n99_), .O(z14));
  inv1   g65(.a(new_n100_), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  aoi22  g67(.a(new_n112_), .b(new_n111_), .c(new_n107_), .d(x22), .O(new_n113_));
  aoi22  g68(.a(new_n93_), .b(new_n46_), .c(x22), .d(new_n55_), .O(new_n114_));
  oai21  g69(.a(new_n113_), .b(x20), .c(new_n114_), .O(z15));
  nand3  g70(.a(new_n112_), .b(x18), .c(x17), .O(new_n116_));
  nor3   g71(.a(x23), .b(x22), .c(x21), .O(new_n117_));
  aoi22  g72(.a(new_n117_), .b(new_n111_), .c(new_n116_), .d(x23), .O(new_n118_));
  aoi22  g73(.a(new_n93_), .b(new_n46_), .c(x23), .d(new_n55_), .O(new_n119_));
  oai21  g74(.a(new_n118_), .b(x20), .c(new_n119_), .O(z16));
  inv1   g75(.a(x22), .O(new_n121_));
  nand3  g76(.a(new_n51_), .b(new_n121_), .c(new_n106_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n95_), .c(x24), .O(new_n123_));
  nor2   g78(.a(new_n95_), .b(x24), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n117_), .c(x19), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n103_), .O(new_n127_));
  aoi22  g82(.a(new_n93_), .b(new_n46_), .c(x24), .d(new_n55_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n127_), .O(z17));
  nand3  g84(.a(new_n65_), .b(x18), .c(x17), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n122_), .c(x25), .O(new_n131_));
  nand3  g86(.a(new_n117_), .b(new_n111_), .c(new_n60_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n103_), .O(new_n134_));
  aoi22  g89(.a(new_n93_), .b(new_n46_), .c(x25), .d(new_n55_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n134_), .O(z18));
endmodule


