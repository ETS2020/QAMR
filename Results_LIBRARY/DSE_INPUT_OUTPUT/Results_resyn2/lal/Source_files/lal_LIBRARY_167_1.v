// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x07), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n47_), .c(new_n60_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n48_), .O(new_n67_));
  nand2  g22(.a(new_n59_), .b(new_n67_), .O(z03));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n67_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n67_), .O(z06));
  nand3  g34(.a(new_n67_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g35(.a(new_n58_), .b(new_n67_), .c(new_n50_), .O(z08));
  nand4  g36(.a(new_n47_), .b(new_n60_), .c(x05), .d(x04), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand2  g38(.a(new_n63_), .b(new_n52_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  nand2  g40(.a(new_n53_), .b(new_n52_), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n61_), .c(new_n86_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n47_), .c(x07), .O(z11));
  nand2  g44(.a(new_n87_), .b(new_n54_), .O(new_n90_));
  inv1   g45(.a(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x19), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n90_), .c(new_n61_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n47_), .c(x07), .O(z12));
  nand2  g49(.a(new_n92_), .b(x20), .O(new_n95_));
  nand4  g50(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n63_), .O(z13));
  nand2  g52(.a(new_n96_), .b(x21), .O(new_n98_));
  inv1   g53(.a(x21), .O(new_n99_));
  inv1   g54(.a(new_n96_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n98_), .c(new_n61_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n60_), .O(z14));
  inv1   g59(.a(x22), .O(new_n105_));
  nor2   g60(.a(x20), .b(new_n54_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n91_), .c(new_n105_), .d(new_n99_), .O(new_n107_));
  oai21  g62(.a(new_n96_), .b(x21), .c(x22), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n61_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n60_), .O(z15));
  nand2  g66(.a(new_n107_), .b(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  nand4  g68(.a(new_n100_), .b(new_n113_), .c(new_n105_), .d(new_n99_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n61_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n47_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n60_), .O(z16));
  nand2  g72(.a(new_n114_), .b(x24), .O(new_n118_));
  inv1   g73(.a(x24), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n113_), .c(new_n105_), .d(new_n99_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n96_), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(new_n62_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n118_), .O(z17));
  nand2  g78(.a(new_n121_), .b(new_n50_), .O(new_n124_));
  oai21  g79(.a(new_n120_), .b(new_n96_), .c(x25), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n61_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n47_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n60_), .O(z18));
endmodule


