// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n53_), .c(new_n54_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x25), .c(new_n52_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n47_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x25), .O(new_n68_));
  inv1   g23(.a(new_n47_), .O(new_n69_));
  nand3  g24(.a(new_n62_), .b(new_n60_), .c(x24), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n68_), .c(new_n69_), .O(z08));
  inv1   g26(.a(z08), .O(z03));
  nand2  g27(.a(new_n47_), .b(new_n46_), .O(new_n73_));
  xor2a  g28(.a(x12), .b(x03), .O(new_n74_));
  xor2a  g29(.a(x11), .b(x02), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  xor2a  g31(.a(x09), .b(x00), .O(new_n77_));
  xor2a  g32(.a(x10), .b(x01), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(z04));
  or2    g35(.a(new_n73_), .b(x13), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  inv1   g37(.a(x14), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g39(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g40(.a(x15), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n49_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n50_), .c(new_n47_), .O(z09));
  nand3  g43(.a(new_n50_), .b(new_n86_), .c(new_n49_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n56_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  xnor2a g49(.a(x18), .b(x17), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n89_), .c(new_n47_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  aoi21  g53(.a(x18), .b(x17), .c(x19), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n91_), .O(z12));
  nor2   g55(.a(new_n97_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n92_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  aoi21  g61(.a(new_n101_), .b(new_n106_), .c(new_n89_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n105_), .c(new_n69_), .O(z14));
  oai21  g63(.a(new_n102_), .b(x21), .c(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n101_), .b(new_n110_), .c(new_n106_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n92_), .O(z15));
  nand4  g67(.a(new_n101_), .b(new_n54_), .c(new_n110_), .d(new_n106_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x23), .O(new_n114_));
  nand4  g69(.a(new_n101_), .b(new_n53_), .c(new_n110_), .d(new_n106_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n90_), .O(z16));
  nand2  g71(.a(new_n89_), .b(new_n47_), .O(new_n117_));
  xor2a  g72(.a(new_n111_), .b(x24), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(x23), .c(new_n117_), .O(z17));
  inv1   g74(.a(new_n115_), .O(new_n120_));
  nor2   g75(.a(x25), .b(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  oai21  g77(.a(new_n121_), .b(new_n53_), .c(new_n90_), .O(new_n123_));
  aoi21  g78(.a(new_n113_), .b(x25), .c(new_n123_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n122_), .O(z18));
endmodule


