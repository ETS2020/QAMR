// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  inv1   g11(.a(x25), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nor2   g14(.a(x18), .b(x17), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(x22), .c(x21), .d(new_n59_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n58_), .c(new_n47_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n57_), .O(z08));
  nand2  g19(.a(z08), .b(new_n56_), .O(z01));
  nand2  g20(.a(new_n50_), .b(new_n46_), .O(z02));
  aoi21  g21(.a(new_n61_), .b(new_n58_), .c(new_n48_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x25), .c(new_n50_), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n69_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand3  g32(.a(new_n50_), .b(new_n77_), .c(new_n69_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(x14), .b(new_n69_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z06));
  nand3  g36(.a(new_n50_), .b(new_n69_), .c(x06), .O(z07));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n54_), .c(new_n50_), .O(z09));
  nand3  g40(.a(new_n54_), .b(new_n83_), .c(new_n53_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n50_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n86_), .c(new_n50_), .O(z11));
  inv1   g44(.a(new_n86_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n59_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(z12));
  oai21  g50(.a(new_n91_), .b(x24), .c(x20), .O(new_n96_));
  nor2   g51(.a(new_n91_), .b(x20), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n90_), .O(z13));
  nand2  g54(.a(new_n98_), .b(x21), .O(new_n100_));
  inv1   g55(.a(x21), .O(new_n101_));
  aoi21  g56(.a(new_n97_), .b(new_n101_), .c(new_n86_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n100_), .c(new_n49_), .O(z14));
  inv1   g58(.a(x22), .O(new_n104_));
  nand3  g59(.a(new_n97_), .b(new_n104_), .c(new_n101_), .O(new_n105_));
  oai22  g60(.a(new_n98_), .b(x21), .c(new_n49_), .d(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n90_), .O(z15));
  inv1   g62(.a(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n58_), .O(new_n109_));
  oai21  g64(.a(new_n49_), .b(x23), .c(new_n105_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n90_), .O(z16));
  nor2   g66(.a(new_n105_), .b(x23), .O(new_n112_));
  nand4  g67(.a(new_n48_), .b(new_n58_), .c(new_n104_), .d(new_n101_), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n97_), .c(new_n86_), .O(new_n115_));
  oai21  g70(.a(new_n112_), .b(new_n48_), .c(new_n115_), .O(z17));
  nand4  g71(.a(new_n108_), .b(new_n57_), .c(new_n48_), .d(new_n58_), .O(new_n117_));
  inv1   g72(.a(new_n91_), .O(new_n118_));
  nand2  g73(.a(new_n114_), .b(new_n118_), .O(new_n119_));
  oai21  g74(.a(x25), .b(x24), .c(x20), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n90_), .O(new_n121_));
  aoi21  g76(.a(new_n119_), .b(x25), .c(new_n121_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n117_), .O(z18));
endmodule


