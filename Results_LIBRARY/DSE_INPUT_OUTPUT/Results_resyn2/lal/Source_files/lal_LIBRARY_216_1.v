// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x11), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n47_), .c(new_n50_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(new_n48_), .b(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n48_), .O(new_n67_));
  nand2  g22(.a(new_n60_), .b(new_n67_), .O(z03));
  inv1   g23(.a(x11), .O(new_n69_));
  inv1   g24(.a(x02), .O(new_n70_));
  oai21  g25(.a(x08), .b(new_n70_), .c(new_n47_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  inv1   g27(.a(x09), .O(new_n73_));
  inv1   g28(.a(x12), .O(new_n74_));
  aoi22  g29(.a(new_n74_), .b(x03), .c(new_n73_), .d(x00), .O(new_n75_));
  oai21  g30(.a(new_n73_), .b(x00), .c(new_n75_), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  inv1   g32(.a(x03), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n78_), .O(new_n79_));
  nand2  g34(.a(x11), .b(new_n70_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n79_), .c(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n76_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n72_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n67_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n67_), .O(z06));
  nand3  g41(.a(new_n67_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nor2   g43(.a(x18), .b(x17), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n54_), .c(x20), .O(new_n90_));
  oai21  g45(.a(new_n57_), .b(new_n90_), .c(new_n88_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n69_), .c(x24), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z08));
  nor2   g48(.a(x15), .b(x07), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n61_), .c(new_n67_), .O(z09));
  nand2  g51(.a(new_n94_), .b(new_n61_), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n48_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n52_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z10));
  inv1   g55(.a(new_n89_), .O(new_n101_));
  nand2  g56(.a(x18), .b(x17), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n98_), .c(new_n101_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z11));
  inv1   g59(.a(new_n97_), .O(new_n105_));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n102_), .b(new_n54_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n67_), .O(z12));
  nor2   g64(.a(new_n106_), .b(x20), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n106_), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n98_), .O(z13));
  nand2  g68(.a(new_n111_), .b(x21), .O(new_n114_));
  inv1   g69(.a(x21), .O(new_n115_));
  nand2  g70(.a(new_n110_), .b(new_n115_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n98_), .O(z14));
  nand2  g72(.a(new_n116_), .b(x22), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n110_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n98_), .O(z15));
  nand2  g76(.a(new_n120_), .b(x23), .O(new_n122_));
  nand3  g77(.a(new_n119_), .b(new_n110_), .c(new_n88_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n98_), .O(z16));
  nand3  g79(.a(new_n123_), .b(x24), .c(x11), .O(new_n125_));
  nand2  g80(.a(new_n97_), .b(new_n67_), .O(new_n126_));
  nand4  g81(.a(new_n119_), .b(new_n110_), .c(new_n47_), .d(new_n88_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(z17));
  inv1   g83(.a(new_n127_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n50_), .O(new_n130_));
  oai21  g85(.a(new_n48_), .b(x25), .c(new_n127_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n105_), .O(z18));
endmodule


