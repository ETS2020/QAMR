// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n81_, new_n84_, new_n85_, new_n87_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n49_), .c(new_n50_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  oai21  g19(.a(new_n60_), .b(x25), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nand2  g21(.a(new_n47_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x25), .O(new_n68_));
  inv1   g23(.a(new_n47_), .O(new_n69_));
  nand3  g24(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n68_), .c(new_n69_), .O(z08));
  inv1   g26(.a(z08), .O(z03));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n47_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g35(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n69_), .O(z07));
  inv1   g38(.a(x15), .O(new_n84_));
  nand3  g39(.a(new_n47_), .b(new_n84_), .c(new_n61_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n62_), .O(z09));
  nand3  g41(.a(new_n62_), .b(new_n84_), .c(new_n61_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n47_), .O(z10));
  xnor2a g43(.a(x18), .b(x17), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n87_), .c(new_n47_), .O(z11));
  inv1   g45(.a(new_n85_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n62_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  aoi21  g49(.a(x18), .b(x17), .c(x19), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z12));
  inv1   g51(.a(new_n92_), .O(new_n97_));
  nor2   g52(.a(new_n93_), .b(x20), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n93_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(z13));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  aoi21  g58(.a(new_n98_), .b(new_n103_), .c(new_n87_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n102_), .c(new_n69_), .O(z14));
  oai21  g60(.a(new_n99_), .b(x21), .c(x22), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n97_), .O(z15));
  nand2  g64(.a(new_n87_), .b(new_n47_), .O(new_n110_));
  nand3  g65(.a(new_n108_), .b(new_n50_), .c(x23), .O(new_n111_));
  nand3  g66(.a(new_n107_), .b(new_n98_), .c(new_n49_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z16));
  aoi21  g68(.a(new_n112_), .b(x24), .c(new_n87_), .O(new_n114_));
  oai21  g69(.a(new_n112_), .b(x24), .c(new_n114_), .O(z17));
  inv1   g70(.a(new_n87_), .O(new_n116_));
  inv1   g71(.a(new_n112_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n68_), .c(new_n50_), .O(new_n118_));
  oai21  g73(.a(new_n108_), .b(x24), .c(x25), .O(new_n119_));
  oai21  g74(.a(x25), .b(x24), .c(x23), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n119_), .c(new_n118_), .d(new_n116_), .O(z18));
endmodule


