// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n73_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x04), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  aoi21  g04(.a(x05), .b(x04), .c(x07), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n50_), .c(new_n49_), .O(z01));
  and2   g16(.a(new_n47_), .b(x16), .O(z02));
  nand3  g17(.a(new_n59_), .b(new_n47_), .c(new_n51_), .O(z08));
  inv1   g18(.a(z08), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g30(.a(x04), .O(new_n76_));
  inv1   g31(.a(x15), .O(new_n77_));
  inv1   g32(.a(x07), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x05), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z09));
  nand2  g35(.a(new_n50_), .b(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x17), .c(new_n47_), .O(z10));
  xnor2a g37(.a(x18), .b(x17), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(new_n47_), .O(z11));
  nand3  g39(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  aoi21  g41(.a(x18), .b(x17), .c(x19), .O(new_n87_));
  nor3   g42(.a(new_n87_), .b(new_n86_), .c(new_n81_), .O(z12));
  nand2  g43(.a(x15), .b(new_n76_), .O(new_n89_));
  nor2   g44(.a(new_n85_), .b(x20), .O(new_n90_));
  nand2  g45(.a(new_n85_), .b(x20), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n90_), .c(new_n77_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n89_), .O(z13));
  inv1   g49(.a(x21), .O(new_n95_));
  nor2   g50(.a(new_n90_), .b(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n90_), .b(new_n95_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n50_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n96_), .c(new_n77_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n89_), .O(z14));
  inv1   g55(.a(new_n81_), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(x22), .O(new_n102_));
  nor2   g57(.a(x22), .b(x21), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n90_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  inv1   g61(.a(x23), .O(new_n107_));
  nand3  g62(.a(new_n103_), .b(new_n90_), .c(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n101_), .O(z16));
  nand2  g64(.a(new_n108_), .b(x24), .O(new_n110_));
  inv1   g65(.a(x24), .O(new_n111_));
  nand4  g66(.a(new_n103_), .b(new_n90_), .c(new_n111_), .d(new_n107_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n50_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n77_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n89_), .O(z17));
  inv1   g70(.a(new_n112_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n51_), .O(new_n117_));
  aoi21  g72(.a(new_n112_), .b(x25), .c(new_n81_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n117_), .c(new_n49_), .O(z18));
endmodule


