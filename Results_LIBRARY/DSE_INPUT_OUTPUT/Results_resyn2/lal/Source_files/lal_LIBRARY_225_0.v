// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n93_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x06), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  and2   g09(.a(x22), .b(x21), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(x23), .O(new_n58_));
  inv1   g13(.a(x06), .O(new_n59_));
  nor2   g14(.a(x24), .b(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  inv1   g16(.a(new_n47_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n62_), .O(new_n65_));
  oai21  g20(.a(new_n61_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n47_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x23), .O(new_n69_));
  nand2  g24(.a(new_n56_), .b(new_n69_), .O(new_n70_));
  aoi22  g25(.a(new_n70_), .b(x24), .c(new_n62_), .d(x25), .O(z03));
  inv1   g26(.a(x12), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x03), .O(new_n73_));
  inv1   g28(.a(x02), .O(new_n74_));
  nand2  g29(.a(x11), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x03), .O(new_n76_));
  nand2  g31(.a(x12), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(x11), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x02), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n73_), .O(new_n80_));
  inv1   g35(.a(x00), .O(new_n81_));
  nand2  g36(.a(x09), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x01), .O(new_n83_));
  nand2  g38(.a(x10), .b(new_n83_), .O(new_n84_));
  inv1   g39(.a(x09), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x00), .O(new_n86_));
  inv1   g41(.a(x10), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x01), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n80_), .c(new_n46_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n62_), .O(z04));
  oai21  g46(.a(x13), .b(x08), .c(new_n62_), .O(z05));
  nand2  g47(.a(x14), .b(new_n46_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n62_), .O(z06));
  aoi21  g49(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g50(.a(x25), .O(new_n96_));
  nand2  g51(.a(new_n70_), .b(x24), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n62_), .c(new_n96_), .O(z08));
  inv1   g53(.a(x07), .O(new_n99_));
  inv1   g54(.a(x15), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n64_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z09));
  nand2  g59(.a(new_n102_), .b(new_n63_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n50_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(z10));
  nand3  g63(.a(new_n63_), .b(new_n100_), .c(new_n99_), .O(new_n109_));
  nand2  g64(.a(new_n51_), .b(new_n50_), .O(new_n110_));
  nand2  g65(.a(x18), .b(x17), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n109_), .c(new_n62_), .O(z11));
  nand3  g68(.a(x19), .b(x18), .c(x17), .O(new_n114_));
  aoi21  g69(.a(new_n111_), .b(new_n52_), .c(new_n109_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n62_), .O(z12));
  nor2   g72(.a(new_n114_), .b(x20), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n114_), .b(x20), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n106_), .O(z13));
  nand2  g76(.a(new_n119_), .b(x21), .O(new_n122_));
  inv1   g77(.a(x21), .O(new_n123_));
  nand2  g78(.a(new_n118_), .b(new_n123_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n106_), .O(z14));
  nand2  g80(.a(new_n124_), .b(x22), .O(new_n126_));
  nor2   g81(.a(x22), .b(x21), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n118_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n126_), .c(new_n106_), .O(z15));
  nand2  g84(.a(new_n128_), .b(x23), .O(new_n130_));
  nand3  g85(.a(new_n127_), .b(new_n118_), .c(new_n69_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n106_), .O(z16));
  nand2  g87(.a(new_n131_), .b(x24), .O(new_n133_));
  nand4  g88(.a(new_n127_), .b(new_n118_), .c(new_n57_), .d(new_n69_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n133_), .c(new_n106_), .O(z17));
  xor2a  g90(.a(new_n134_), .b(x25), .O(new_n136_));
  nand2  g91(.a(new_n96_), .b(x24), .O(new_n137_));
  aoi21  g92(.a(new_n137_), .b(new_n59_), .c(new_n109_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n136_), .O(z18));
endmodule


