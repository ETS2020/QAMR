// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n47_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z01));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g20(.a(x23), .O(new_n66_));
  nor2   g21(.a(x18), .b(x17), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n54_), .c(x20), .O(new_n68_));
  oai21  g23(.a(new_n57_), .b(new_n68_), .c(new_n66_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(x24), .c(x25), .O(z03));
  inv1   g25(.a(x08), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n47_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand3  g34(.a(new_n47_), .b(new_n79_), .c(new_n71_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g38(.a(new_n71_), .b(x06), .c(new_n62_), .O(z07));
  aoi21  g39(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x23), .c(x24), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n50_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand3  g44(.a(new_n47_), .b(new_n89_), .c(new_n88_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n61_), .O(z09));
  inv1   g46(.a(new_n90_), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n61_), .c(new_n52_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  nand2  g49(.a(new_n92_), .b(new_n61_), .O(new_n95_));
  inv1   g50(.a(new_n67_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n95_), .O(z11));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(new_n54_), .O(new_n101_));
  nand3  g56(.a(new_n61_), .b(new_n89_), .c(new_n88_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(z12));
  nand4  g60(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  aoi21  g61(.a(new_n100_), .b(x20), .c(new_n102_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n62_), .O(z13));
  inv1   g63(.a(new_n95_), .O(new_n109_));
  nand2  g64(.a(new_n106_), .b(x21), .O(new_n110_));
  inv1   g65(.a(new_n100_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n110_), .c(new_n109_), .O(z14));
  inv1   g69(.a(x22), .O(new_n115_));
  nand3  g70(.a(new_n112_), .b(new_n111_), .c(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n113_), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n109_), .O(z15));
  inv1   g73(.a(x24), .O(new_n119_));
  nand4  g74(.a(new_n112_), .b(new_n111_), .c(new_n119_), .d(new_n115_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x23), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n112_), .c(new_n111_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n103_), .O(z16));
  inv1   g79(.a(new_n123_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n119_), .O(new_n126_));
  aoi21  g81(.a(new_n123_), .b(x24), .c(new_n102_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z17));
  nand2  g83(.a(x25), .b(x23), .O(new_n129_));
  nor2   g84(.a(x25), .b(x21), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n122_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n106_), .c(new_n129_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n119_), .O(new_n133_));
  oai21  g88(.a(x25), .b(x24), .c(x23), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n102_), .O(new_n135_));
  nand3  g90(.a(new_n120_), .b(x25), .c(new_n66_), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n135_), .c(new_n133_), .O(z18));
endmodule


