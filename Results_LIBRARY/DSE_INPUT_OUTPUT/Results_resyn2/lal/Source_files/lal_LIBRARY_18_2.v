// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x12), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  inv1   g13(.a(x24), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x23), .O(new_n60_));
  nor2   g15(.a(x24), .b(x12), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  inv1   g17(.a(new_n48_), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x07), .c(new_n63_), .O(new_n66_));
  oai21  g21(.a(new_n62_), .b(x25), .c(new_n66_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n48_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n62_), .b(x25), .O(z03));
  oai21  g25(.a(x08), .b(x03), .c(x24), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x12), .O(new_n72_));
  inv1   g27(.a(x09), .O(new_n73_));
  inv1   g28(.a(x02), .O(new_n74_));
  aoi22  g29(.a(x11), .b(new_n74_), .c(new_n73_), .d(x00), .O(new_n75_));
  oai21  g30(.a(new_n73_), .b(x00), .c(new_n75_), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  inv1   g32(.a(x11), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x02), .O(new_n79_));
  nand2  g34(.a(new_n47_), .b(x03), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n79_), .c(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n76_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n72_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n63_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n63_), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g42(.a(x25), .O(new_n88_));
  aoi21  g43(.a(new_n54_), .b(new_n50_), .c(new_n56_), .O(new_n89_));
  inv1   g44(.a(x23), .O(new_n90_));
  nand2  g45(.a(new_n88_), .b(new_n90_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n89_), .c(x24), .O(new_n92_));
  oai21  g47(.a(new_n88_), .b(x12), .c(new_n92_), .O(z08));
  nor2   g48(.a(x15), .b(x07), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n65_), .c(new_n63_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand2  g51(.a(new_n94_), .b(new_n64_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(x17), .c(new_n63_), .O(z10));
  nand2  g53(.a(new_n52_), .b(new_n51_), .O(new_n99_));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n97_), .c(new_n63_), .O(z11));
  nand3  g57(.a(new_n94_), .b(new_n64_), .c(new_n63_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(new_n53_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nor2   g61(.a(new_n106_), .b(new_n103_), .O(z12));
  inv1   g62(.a(new_n103_), .O(new_n108_));
  nor2   g63(.a(new_n104_), .b(x20), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(z13));
  nand2  g67(.a(new_n110_), .b(x21), .O(new_n113_));
  inv1   g68(.a(x21), .O(new_n114_));
  nand2  g69(.a(new_n109_), .b(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n108_), .O(z14));
  nand2  g71(.a(new_n115_), .b(x22), .O(new_n117_));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n109_), .c(new_n97_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n117_), .c(new_n48_), .O(z15));
  nand2  g75(.a(new_n118_), .b(new_n109_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x23), .O(new_n122_));
  nand3  g77(.a(new_n118_), .b(new_n109_), .c(new_n90_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n108_), .O(z16));
  nand2  g79(.a(new_n123_), .b(x24), .O(new_n125_));
  nand4  g80(.a(new_n118_), .b(new_n109_), .c(new_n59_), .d(new_n90_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n108_), .O(z17));
  xor2a  g82(.a(new_n126_), .b(x25), .O(new_n128_));
  aoi21  g83(.a(new_n88_), .b(x24), .c(new_n47_), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n97_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(z18));
endmodule


