// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  aoi21  g01(.a(x24), .b(x16), .c(new_n46_), .O(z00));
  inv1   g02(.a(x23), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n49_), .c(new_n48_), .O(new_n58_));
  inv1   g13(.a(x24), .O(new_n59_));
  oai21  g14(.a(new_n49_), .b(x08), .c(new_n59_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n60_), .c(new_n58_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(x24), .b(new_n46_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(z02));
  nand2  g22(.a(new_n60_), .b(new_n58_), .O(z03));
  inv1   g23(.a(new_n66_), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n69_), .O(z04));
  nor2   g31(.a(x13), .b(x08), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x08), .O(z06));
  nand2  g34(.a(new_n46_), .b(x06), .O(z07));
  aoi21  g35(.a(new_n56_), .b(new_n55_), .c(x23), .O(new_n81_));
  nor2   g36(.a(new_n66_), .b(x25), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n59_), .c(new_n82_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n69_), .c(new_n62_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n84_), .b(new_n61_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n51_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(new_n52_), .b(new_n51_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n87_), .c(new_n69_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n53_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n87_), .c(new_n66_), .O(z12));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  aoi21  g55(.a(new_n95_), .b(x20), .c(new_n87_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n100_), .c(new_n66_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  inv1   g58(.a(new_n95_), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n103_), .c(new_n88_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n104_), .c(new_n87_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n66_), .O(z15));
  nand3  g66(.a(new_n109_), .b(new_n104_), .c(new_n48_), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n105_), .b(new_n104_), .c(new_n113_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(x23), .c(new_n87_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n112_), .c(new_n66_), .O(z16));
  nand2  g71(.a(new_n112_), .b(x24), .O(new_n117_));
  nand2  g72(.a(new_n87_), .b(new_n69_), .O(new_n118_));
  nor2   g73(.a(x24), .b(x23), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n109_), .c(new_n104_), .d(new_n46_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(z17));
  nor2   g76(.a(x22), .b(x21), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n119_), .c(new_n99_), .d(new_n49_), .O(new_n123_));
  nand3  g78(.a(new_n122_), .b(new_n119_), .c(new_n99_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x25), .O(new_n125_));
  aoi21  g80(.a(new_n49_), .b(x24), .c(new_n46_), .O(new_n126_));
  nor2   g81(.a(new_n126_), .b(new_n87_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n123_), .O(z18));
endmodule


