// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x12), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nor2   g08(.a(x18), .b(x17), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n51_), .c(new_n50_), .O(new_n61_));
  aoi21  g16(.a(x25), .b(new_n47_), .c(x24), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n62_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  aoi21  g23(.a(new_n54_), .b(new_n53_), .c(x20), .O(new_n69_));
  inv1   g24(.a(new_n59_), .O(new_n70_));
  inv1   g25(.a(x24), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x23), .O(new_n72_));
  oai21  g27(.a(new_n70_), .b(new_n69_), .c(new_n72_), .O(new_n73_));
  nand2  g28(.a(new_n71_), .b(new_n47_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n73_), .c(x25), .O(z03));
  inv1   g30(.a(new_n48_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  xor2a  g32(.a(x11), .b(x02), .O(new_n78_));
  xor2a  g33(.a(x09), .b(x00), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  xor2a  g35(.a(x10), .b(x01), .O(new_n81_));
  xor2a  g36(.a(x12), .b(x03), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(z04));
  nor2   g39(.a(new_n77_), .b(x13), .O(z05));
  inv1   g40(.a(x14), .O(new_n86_));
  nor2   g41(.a(new_n77_), .b(new_n86_), .O(z06));
  nand3  g42(.a(new_n76_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g43(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n89_));
  nor2   g44(.a(new_n48_), .b(x25), .O(new_n90_));
  oai21  g45(.a(new_n89_), .b(new_n71_), .c(new_n90_), .O(z08));
  nor2   g46(.a(x15), .b(x07), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n64_), .c(new_n76_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  inv1   g49(.a(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n63_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  and2   g54(.a(x18), .b(x17), .O(new_n100_));
  or2    g55(.a(new_n100_), .b(new_n54_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n96_), .c(new_n76_), .O(z11));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n100_), .b(x19), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  and2   g61(.a(new_n106_), .b(new_n97_), .O(z12));
  nor2   g62(.a(new_n103_), .b(x20), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand2  g64(.a(new_n103_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n97_), .O(z13));
  nand2  g66(.a(new_n109_), .b(x21), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(new_n57_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n97_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n108_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n97_), .O(z15));
  nand2  g73(.a(new_n117_), .b(x23), .O(new_n119_));
  nand4  g74(.a(new_n50_), .b(new_n58_), .c(new_n57_), .d(new_n52_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n104_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n119_), .c(new_n97_), .O(z16));
  nand2  g78(.a(new_n122_), .b(x24), .O(new_n124_));
  nand2  g79(.a(new_n96_), .b(new_n76_), .O(new_n125_));
  nand4  g80(.a(new_n121_), .b(new_n104_), .c(new_n71_), .d(new_n47_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(z17));
  oai21  g82(.a(new_n120_), .b(new_n103_), .c(x25), .O(new_n128_));
  nor3   g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n116_), .c(new_n108_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n47_), .O(new_n132_));
  aoi22  g87(.a(new_n96_), .b(new_n76_), .c(x25), .d(x24), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


