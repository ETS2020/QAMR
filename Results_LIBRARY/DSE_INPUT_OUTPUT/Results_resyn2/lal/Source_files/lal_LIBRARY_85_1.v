// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n56_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n67_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x23), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n48_), .O(z08));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand4  g11(.a(new_n56_), .b(z08), .c(new_n50_), .d(new_n53_), .O(z01));
  nand2  g12(.a(z08), .b(new_n50_), .O(z03));
  xnor2a g13(.a(x11), .b(x02), .O(new_n59_));
  xnor2a g14(.a(x12), .b(x03), .O(new_n60_));
  xnor2a g15(.a(x09), .b(x00), .O(new_n61_));
  xnor2a g16(.a(x10), .b(x01), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n59_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n46_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n50_), .O(z04));
  oai21  g20(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g21(.a(x14), .b(new_n46_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n49_), .O(z06));
  aoi21  g23(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nor2   g24(.a(x15), .b(x07), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(x05), .c(x04), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n50_), .O(z09));
  inv1   g27(.a(x17), .O(new_n73_));
  nand2  g28(.a(new_n70_), .b(new_n56_), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z10));
  nand2  g32(.a(x18), .b(x17), .O(new_n78_));
  inv1   g33(.a(x18), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n78_), .c(new_n75_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z11));
  xor2a  g37(.a(new_n78_), .b(x19), .O(new_n83_));
  nor3   g38(.a(new_n83_), .b(new_n74_), .c(new_n49_), .O(z12));
  nand3  g39(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g41(.a(x20), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(x19), .c(x18), .d(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n86_), .c(new_n75_), .O(z13));
  inv1   g44(.a(x21), .O(new_n90_));
  inv1   g45(.a(new_n85_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n87_), .O(new_n92_));
  aoi21  g47(.a(new_n88_), .b(x21), .c(new_n74_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n92_), .c(new_n49_), .O(z14));
  inv1   g49(.a(x22), .O(new_n95_));
  nor3   g50(.a(new_n85_), .b(x21), .c(x20), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(x22), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n75_), .O(z15));
  oai21  g54(.a(new_n92_), .b(x22), .c(x23), .O(new_n100_));
  nand2  g55(.a(new_n74_), .b(new_n50_), .O(new_n101_));
  nor2   g56(.a(x23), .b(x22), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n48_), .c(new_n90_), .O(new_n103_));
  or2    g58(.a(new_n103_), .b(new_n88_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(z16));
  inv1   g60(.a(new_n74_), .O(new_n106_));
  nand2  g61(.a(new_n102_), .b(new_n96_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n48_), .O(z17));
  inv1   g63(.a(x23), .O(new_n109_));
  nand4  g64(.a(new_n54_), .b(new_n95_), .c(new_n90_), .d(new_n87_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n85_), .c(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  oai21  g67(.a(new_n103_), .b(new_n88_), .c(x25), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n106_), .O(z18));
endmodule


