// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n83_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  oai21  g21(.a(new_n62_), .b(x25), .c(new_n66_), .O(z01));
  inv1   g22(.a(x25), .O(new_n68_));
  nand3  g23(.a(new_n60_), .b(new_n58_), .c(x24), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(new_n48_), .O(z08));
  inv1   g25(.a(z08), .O(z03));
  inv1   g26(.a(x08), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(z04));
  inv1   g34(.a(x13), .O(new_n80_));
  nand2  g35(.a(new_n49_), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(new_n49_), .b(x14), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z06));
  aoi21  g39(.a(new_n72_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g40(.a(x15), .O(new_n86_));
  nand3  g41(.a(new_n46_), .b(new_n86_), .c(new_n63_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n64_), .O(z09));
  nand3  g43(.a(new_n64_), .b(new_n86_), .c(new_n63_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n46_), .O(z10));
  xnor2a g45(.a(x18), .b(x17), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n89_), .c(new_n46_), .O(z11));
  inv1   g47(.a(new_n87_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(x18), .b(x17), .c(x19), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n94_), .O(z12));
  inv1   g53(.a(new_n94_), .O(new_n99_));
  nor2   g54(.a(new_n95_), .b(x20), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n95_), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(z13));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n104_));
  inv1   g59(.a(x21), .O(new_n105_));
  aoi21  g60(.a(new_n100_), .b(new_n105_), .c(new_n89_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n104_), .c(new_n48_), .O(z14));
  oai21  g62(.a(new_n101_), .b(x21), .c(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n100_), .b(new_n109_), .c(new_n105_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n99_), .O(z15));
  inv1   g66(.a(new_n89_), .O(new_n112_));
  nand4  g67(.a(new_n100_), .b(new_n52_), .c(new_n109_), .d(new_n105_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x23), .O(new_n114_));
  nand4  g69(.a(new_n100_), .b(new_n51_), .c(new_n109_), .d(new_n105_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(z16));
  nand2  g71(.a(new_n89_), .b(new_n46_), .O(new_n117_));
  xor2a  g72(.a(new_n110_), .b(x24), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(x23), .c(new_n117_), .O(z17));
  inv1   g74(.a(new_n115_), .O(new_n120_));
  nor2   g75(.a(x25), .b(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  oai21  g77(.a(new_n121_), .b(new_n51_), .c(new_n112_), .O(new_n123_));
  aoi21  g78(.a(new_n113_), .b(x25), .c(new_n123_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n122_), .O(z18));
endmodule


