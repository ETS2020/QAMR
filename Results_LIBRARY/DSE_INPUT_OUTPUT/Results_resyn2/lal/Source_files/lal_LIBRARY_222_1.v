// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x00), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n49_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n49_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n49_), .O(z03));
  inv1   g20(.a(x12), .O(new_n66_));
  inv1   g21(.a(x10), .O(new_n67_));
  aoi22  g22(.a(new_n66_), .b(x03), .c(new_n67_), .d(x01), .O(new_n68_));
  oai21  g23(.a(new_n66_), .b(x03), .c(new_n68_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(new_n70_));
  inv1   g25(.a(x09), .O(new_n71_));
  aoi21  g26(.a(new_n47_), .b(new_n71_), .c(x00), .O(new_n72_));
  inv1   g27(.a(x11), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x02), .O(new_n74_));
  nand2  g29(.a(new_n71_), .b(x00), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  oai22  g31(.a(new_n73_), .b(x02), .c(new_n67_), .d(x01), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(new_n78_));
  aoi22  g33(.a(new_n78_), .b(new_n70_), .c(new_n49_), .d(x08), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x08), .c(new_n49_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g38(.a(new_n59_), .b(new_n51_), .c(new_n48_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n85_), .c(new_n49_), .O(z09));
  nand3  g44(.a(new_n85_), .b(new_n87_), .c(new_n86_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nor2   g47(.a(new_n90_), .b(new_n48_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  inv1   g51(.a(new_n90_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n94_), .b(new_n55_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n49_), .O(z12));
  nor2   g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n93_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  aoi21  g62(.a(new_n102_), .b(new_n107_), .c(new_n90_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n106_), .c(new_n48_), .O(z14));
  oai21  g64(.a(new_n103_), .b(x21), .c(x22), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  inv1   g66(.a(new_n98_), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n111_), .c(new_n107_), .d(new_n52_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n110_), .c(new_n93_), .O(z15));
  nand2  g69(.a(new_n113_), .b(x23), .O(new_n115_));
  inv1   g70(.a(x23), .O(new_n116_));
  nand4  g71(.a(new_n102_), .b(new_n116_), .c(new_n111_), .d(new_n107_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n97_), .O(new_n118_));
  and2   g73(.a(new_n118_), .b(new_n49_), .O(z16));
  nand2  g74(.a(new_n90_), .b(new_n49_), .O(new_n120_));
  nand3  g75(.a(new_n117_), .b(x24), .c(x00), .O(new_n121_));
  inv1   g76(.a(new_n113_), .O(new_n122_));
  nor2   g77(.a(x24), .b(x23), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n121_), .c(new_n120_), .O(z17));
  nand3  g80(.a(new_n123_), .b(new_n122_), .c(new_n51_), .O(new_n126_));
  nand2  g81(.a(new_n49_), .b(new_n51_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n126_), .c(new_n97_), .O(z18));
endmodule


