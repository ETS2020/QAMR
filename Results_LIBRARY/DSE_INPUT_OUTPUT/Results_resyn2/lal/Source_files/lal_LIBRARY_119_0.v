// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n96_, new_n98_, new_n100_, new_n102_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x20), .b(x15), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n59_), .b(new_n48_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n48_), .O(z04));
  nor3   g28(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g32(.a(new_n58_), .b(new_n50_), .c(new_n47_), .O(z08));
  inv1   g33(.a(x07), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x05), .c(x04), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x20), .c(x15), .O(z09));
  inv1   g36(.a(x15), .O(new_n82_));
  nand3  g37(.a(new_n60_), .b(x20), .c(new_n82_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x17), .O(z10));
  nand2  g39(.a(new_n53_), .b(new_n52_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n60_), .c(new_n85_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x20), .c(x15), .O(z11));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(new_n86_), .b(new_n54_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n60_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(x20), .c(x15), .O(z12));
  oai21  g47(.a(new_n89_), .b(new_n61_), .c(x20), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n82_), .O(z13));
  inv1   g49(.a(x21), .O(new_n95_));
  inv1   g50(.a(new_n83_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(z14));
  oai21  g52(.a(new_n61_), .b(x22), .c(x20), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n82_), .O(z15));
  inv1   g54(.a(x23), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(new_n100_), .O(z16));
  inv1   g56(.a(x24), .O(new_n102_));
  nand2  g57(.a(new_n96_), .b(new_n102_), .O(z17));
  nand2  g58(.a(new_n96_), .b(new_n50_), .O(z18));
endmodule


