// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(new_n47_), .b(new_n61_), .O(z02));
  nand2  g17(.a(new_n58_), .b(new_n47_), .O(z03));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g23(.a(x10), .b(x01), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z04));
  inv1   g26(.a(x13), .O(new_n72_));
  inv1   g27(.a(new_n64_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z05));
  nand2  g30(.a(new_n73_), .b(x14), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z06));
  nand2  g32(.a(new_n73_), .b(x06), .O(z07));
  aoi22  g33(.a(new_n57_), .b(new_n49_), .c(x15), .d(x07), .O(z08));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g37(.a(x05), .b(x04), .c(x15), .O(new_n83_));
  aoi21  g38(.a(new_n82_), .b(new_n47_), .c(new_n83_), .O(z09));
  nand2  g39(.a(new_n59_), .b(new_n81_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g41(.a(new_n52_), .b(new_n51_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n85_), .c(new_n47_), .O(z11));
  inv1   g45(.a(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(x19), .O(new_n92_));
  nand2  g47(.a(new_n88_), .b(new_n53_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(new_n85_), .O(z12));
  inv1   g50(.a(new_n85_), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(x20), .O(new_n97_));
  nand4  g52(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(z13));
  nand2  g54(.a(new_n81_), .b(x07), .O(new_n100_));
  nand2  g55(.a(new_n98_), .b(x21), .O(new_n101_));
  inv1   g56(.a(x21), .O(new_n102_));
  inv1   g57(.a(new_n98_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n101_), .c(new_n83_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n80_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n100_), .O(z14));
  inv1   g62(.a(x22), .O(new_n108_));
  nor2   g63(.a(x20), .b(new_n53_), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n91_), .c(new_n108_), .d(new_n102_), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(x22), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n96_), .O(z15));
  nand2  g67(.a(new_n110_), .b(x23), .O(new_n113_));
  inv1   g68(.a(x23), .O(new_n114_));
  nand4  g69(.a(new_n103_), .b(new_n114_), .c(new_n108_), .d(new_n102_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n83_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n80_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n100_), .O(z16));
  nand2  g73(.a(new_n115_), .b(x24), .O(new_n119_));
  inv1   g74(.a(x24), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n114_), .c(new_n108_), .d(new_n102_), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(new_n98_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n85_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n119_), .O(z17));
  nand2  g79(.a(new_n122_), .b(new_n49_), .O(new_n125_));
  oai21  g80(.a(new_n121_), .b(new_n98_), .c(x25), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n83_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n80_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n100_), .O(z18));
endmodule


