// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x19), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n47_), .c(new_n58_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g18(.a(x19), .O(new_n64_));
  nand2  g19(.a(new_n56_), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(x24), .c(x25), .O(z03));
  aoi21  g21(.a(x24), .b(x19), .c(x08), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  and2   g27(.a(new_n72_), .b(new_n67_), .O(z04));
  inv1   g28(.a(x13), .O(new_n74_));
  nand2  g29(.a(new_n67_), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z05));
  nand2  g31(.a(new_n67_), .b(x14), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z06));
  nand2  g33(.a(new_n67_), .b(x06), .O(z07));
  inv1   g34(.a(x23), .O(new_n80_));
  inv1   g35(.a(x17), .O(new_n81_));
  inv1   g36(.a(x18), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n81_), .c(x20), .O(new_n83_));
  oai21  g38(.a(new_n54_), .b(new_n83_), .c(new_n80_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x19), .c(x24), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n51_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n58_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n59_), .c(new_n47_), .O(z09));
  nand3  g44(.a(new_n59_), .b(new_n87_), .c(new_n58_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g46(.a(new_n82_), .b(new_n81_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n47_), .O(z11));
  nand2  g50(.a(new_n50_), .b(x19), .O(new_n96_));
  inv1   g51(.a(new_n90_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  aoi21  g54(.a(new_n96_), .b(new_n93_), .c(new_n99_), .O(z12));
  nor2   g55(.a(new_n93_), .b(x20), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x24), .c(x19), .O(new_n102_));
  oai21  g57(.a(new_n96_), .b(new_n93_), .c(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n97_), .O(z13));
  nand3  g59(.a(new_n52_), .b(x18), .c(x17), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(x21), .c(new_n50_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x19), .O(new_n107_));
  oai21  g62(.a(new_n105_), .b(new_n96_), .c(x21), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n97_), .O(z14));
  nand2  g64(.a(new_n90_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(x22), .O(new_n111_));
  inv1   g66(.a(x21), .O(new_n112_));
  inv1   g67(.a(new_n98_), .O(new_n113_));
  nor2   g68(.a(x24), .b(x22), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(new_n52_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n111_), .c(new_n110_), .O(z15));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n101_), .c(new_n112_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n50_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x19), .O(new_n120_));
  aoi21  g75(.a(new_n115_), .b(x23), .c(new_n90_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(z16));
  nand2  g77(.a(new_n97_), .b(new_n50_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  oai21  g79(.a(new_n118_), .b(new_n64_), .c(new_n124_), .O(z17));
  oai21  g80(.a(new_n118_), .b(new_n96_), .c(x25), .O(new_n126_));
  nand3  g81(.a(new_n117_), .b(new_n51_), .c(new_n112_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n105_), .c(new_n50_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x19), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n126_), .c(new_n97_), .O(z18));
endmodule


