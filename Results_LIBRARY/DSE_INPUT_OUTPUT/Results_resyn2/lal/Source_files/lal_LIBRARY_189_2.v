// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x07), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(new_n50_), .O(z01));
  and2   g19(.a(new_n48_), .b(x16), .O(z02));
  aoi21  g20(.a(new_n59_), .b(new_n51_), .c(new_n50_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  nand2  g22(.a(new_n48_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  nor2   g30(.a(new_n68_), .b(x13), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n48_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n50_), .O(z07));
  inv1   g34(.a(x07), .O(new_n80_));
  nand3  g35(.a(new_n62_), .b(new_n47_), .c(new_n80_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand2  g37(.a(new_n61_), .b(new_n53_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n80_), .c(x15), .O(z10));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n62_), .O(new_n85_));
  oai21  g40(.a(new_n54_), .b(new_n53_), .c(new_n85_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n80_), .c(x15), .O(z11));
  nand2  g42(.a(new_n63_), .b(new_n47_), .O(new_n88_));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  aoi21  g45(.a(x18), .b(x17), .c(x19), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(z12));
  inv1   g47(.a(new_n88_), .O(new_n93_));
  nor2   g48(.a(new_n89_), .b(x20), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand2  g50(.a(new_n89_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n93_), .O(z13));
  inv1   g52(.a(x21), .O(new_n98_));
  nor2   g53(.a(new_n94_), .b(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(new_n98_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n61_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n80_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(z14));
  nand2  g58(.a(new_n100_), .b(x22), .O(new_n104_));
  inv1   g59(.a(x22), .O(new_n105_));
  nand3  g60(.a(new_n94_), .b(new_n105_), .c(new_n98_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n93_), .O(z15));
  nor2   g62(.a(x23), .b(x22), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n94_), .c(new_n98_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n61_), .O(new_n110_));
  aoi21  g65(.a(new_n106_), .b(x23), .c(new_n110_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(x07), .c(new_n47_), .O(z16));
  nand2  g67(.a(new_n109_), .b(x24), .O(new_n113_));
  inv1   g68(.a(x24), .O(new_n114_));
  inv1   g69(.a(new_n109_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n113_), .c(new_n93_), .O(z17));
  nand3  g72(.a(new_n115_), .b(new_n51_), .c(new_n114_), .O(new_n118_));
  nand2  g73(.a(new_n116_), .b(x25), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n93_), .O(z18));
endmodule


