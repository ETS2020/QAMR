// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand3  g09(.a(x24), .b(x22), .c(x21), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(new_n54_), .c(new_n51_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand4  g12(.a(new_n57_), .b(new_n56_), .c(new_n47_), .d(new_n50_), .O(z01));
  inv1   g13(.a(new_n47_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n46_), .O(z02));
  nand2  g15(.a(new_n56_), .b(new_n47_), .O(z03));
  xnor2a g16(.a(x12), .b(x03), .O(new_n62_));
  xnor2a g17(.a(x09), .b(x00), .O(new_n63_));
  xnor2a g18(.a(x11), .b(x02), .O(new_n64_));
  xnor2a g19(.a(x10), .b(x01), .O(new_n65_));
  nand4  g20(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x08), .c(new_n47_), .O(z04));
  inv1   g23(.a(x13), .O(new_n69_));
  nor2   g24(.a(new_n59_), .b(x08), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z05));
  nand2  g27(.a(new_n70_), .b(x14), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z06));
  nand2  g29(.a(new_n70_), .b(x06), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  inv1   g31(.a(x20), .O(new_n77_));
  inv1   g32(.a(x17), .O(new_n78_));
  inv1   g33(.a(x18), .O(new_n79_));
  nand3  g34(.a(new_n52_), .b(new_n79_), .c(new_n78_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  inv1   g36(.a(new_n55_), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n51_), .c(new_n59_), .O(z08));
  inv1   g39(.a(x15), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n57_), .c(new_n47_), .O(z09));
  nand3  g42(.a(new_n57_), .b(new_n85_), .c(new_n50_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n47_), .c(new_n78_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  inv1   g46(.a(new_n53_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n88_), .c(new_n47_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(new_n52_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n89_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(z12));
  nand2  g54(.a(new_n89_), .b(new_n47_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n77_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand2  g57(.a(new_n96_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(new_n96_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n101_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nor3   g65(.a(new_n96_), .b(x21), .c(x20), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(x22), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n101_), .O(z15));
  inv1   g69(.a(x24), .O(new_n115_));
  nand4  g70(.a(new_n107_), .b(new_n106_), .c(new_n115_), .d(new_n110_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n107_), .c(new_n106_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n89_), .O(z16));
  nand3  g75(.a(new_n118_), .b(new_n111_), .c(new_n115_), .O(new_n121_));
  aoi21  g76(.a(new_n119_), .b(x24), .c(new_n88_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(z17));
  nand2  g78(.a(x25), .b(x23), .O(new_n124_));
  inv1   g79(.a(x21), .O(new_n125_));
  nand4  g80(.a(new_n51_), .b(new_n76_), .c(new_n110_), .d(new_n125_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n102_), .c(new_n124_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n115_), .O(new_n128_));
  oai21  g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n88_), .O(new_n130_));
  nand3  g85(.a(new_n116_), .b(x25), .c(new_n76_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n128_), .O(z18));
endmodule


