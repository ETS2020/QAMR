// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n80_, new_n83_, new_n85_, new_n86_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand3  g13(.a(x24), .b(x22), .c(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  nor2   g15(.a(x25), .b(x23), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n60_), .c(new_n53_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n48_), .b(new_n64_), .O(z02));
  inv1   g20(.a(x25), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x23), .c(new_n47_), .O(new_n67_));
  nand2  g22(.a(x22), .b(x21), .O(new_n68_));
  aoi21  g23(.a(new_n58_), .b(new_n54_), .c(new_n68_), .O(new_n69_));
  oai21  g24(.a(new_n69_), .b(new_n62_), .c(new_n67_), .O(z03));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g33(.a(new_n48_), .O(new_n79_));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(new_n79_), .O(z06));
  nand3  g36(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  and2   g37(.a(new_n58_), .b(new_n54_), .O(new_n83_));
  oai21  g38(.a(new_n59_), .b(new_n83_), .c(new_n61_), .O(z08));
  inv1   g39(.a(x15), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n51_), .c(new_n48_), .O(z09));
  nand3  g42(.a(new_n51_), .b(new_n85_), .c(new_n50_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(x17), .c(new_n48_), .O(z10));
  nand2  g44(.a(new_n56_), .b(new_n55_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n88_), .c(new_n48_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(new_n57_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n88_), .c(new_n79_), .O(z12));
  nor2   g52(.a(new_n88_), .b(new_n79_), .O(new_n98_));
  nand4  g53(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z13));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  inv1   g57(.a(new_n94_), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n102_), .c(new_n98_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  nand3  g63(.a(new_n104_), .b(new_n103_), .c(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n98_), .O(z15));
  nand2  g65(.a(new_n88_), .b(new_n48_), .O(new_n111_));
  nand3  g66(.a(new_n109_), .b(x24), .c(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  nand4  g68(.a(new_n104_), .b(new_n103_), .c(new_n113_), .d(new_n108_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(z16));
  nand2  g70(.a(new_n114_), .b(x24), .O(new_n116_));
  inv1   g71(.a(new_n99_), .O(new_n117_));
  inv1   g72(.a(x21), .O(new_n118_));
  nand3  g73(.a(new_n47_), .b(new_n108_), .c(new_n118_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n117_), .c(new_n113_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n116_), .c(new_n111_), .O(z17));
  oai21  g77(.a(new_n119_), .b(new_n99_), .c(x25), .O(new_n123_));
  nand3  g78(.a(new_n120_), .b(new_n117_), .c(new_n66_), .O(new_n124_));
  nand2  g79(.a(new_n66_), .b(x24), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(x23), .c(new_n88_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z18));
endmodule


