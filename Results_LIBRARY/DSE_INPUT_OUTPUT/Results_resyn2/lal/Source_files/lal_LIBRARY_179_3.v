// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n144_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x20), .b(x04), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x04), .O(new_n60_));
  inv1   g15(.a(x05), .O(new_n61_));
  aoi21  g16(.a(new_n51_), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(new_n47_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n46_), .O(z02));
  nand3  g21(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  inv1   g24(.a(x12), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x03), .O(new_n71_));
  inv1   g26(.a(x02), .O(new_n72_));
  nand2  g27(.a(x11), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  nand2  g29(.a(x12), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x11), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x02), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n75_), .c(new_n73_), .d(new_n71_), .O(new_n78_));
  inv1   g33(.a(x00), .O(new_n79_));
  nand2  g34(.a(x09), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(x01), .O(new_n81_));
  nand2  g36(.a(x10), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x09), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x00), .O(new_n84_));
  inv1   g39(.a(x10), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x01), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n84_), .c(new_n82_), .d(new_n80_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n78_), .c(new_n69_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n47_), .O(z04));
  inv1   g44(.a(x13), .O(new_n90_));
  nand3  g45(.a(new_n47_), .b(new_n90_), .c(new_n69_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z05));
  nand3  g47(.a(new_n47_), .b(x14), .c(new_n69_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z06));
  aoi21  g49(.a(new_n69_), .b(x06), .c(new_n65_), .O(z07));
  nor2   g50(.a(new_n56_), .b(new_n55_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(x23), .c(x24), .O(new_n97_));
  inv1   g52(.a(x24), .O(new_n98_));
  oai21  g53(.a(new_n56_), .b(new_n98_), .c(new_n60_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(x20), .c(x25), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n97_), .O(z08));
  nor2   g56(.a(x15), .b(x07), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x05), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n51_), .c(new_n60_), .O(z09));
  inv1   g59(.a(new_n102_), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n62_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n52_), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(z10));
  nand2  g63(.a(new_n53_), .b(new_n52_), .O(new_n109_));
  nand2  g64(.a(x05), .b(x04), .O(new_n110_));
  nand2  g65(.a(x18), .b(x17), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n102_), .d(new_n109_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n47_), .O(z11));
  nand3  g68(.a(x19), .b(x18), .c(x17), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand2  g70(.a(new_n111_), .b(new_n54_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n102_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n115_), .c(new_n47_), .O(z12));
  nand2  g73(.a(new_n115_), .b(new_n51_), .O(new_n119_));
  aoi21  g74(.a(new_n114_), .b(x20), .c(new_n105_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n119_), .c(new_n65_), .O(z13));
  inv1   g76(.a(x21), .O(new_n122_));
  inv1   g77(.a(new_n114_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n51_), .O(new_n124_));
  oai21  g79(.a(new_n114_), .b(x20), .c(x21), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n106_), .O(z14));
  oai21  g81(.a(new_n114_), .b(x21), .c(x22), .O(new_n127_));
  inv1   g82(.a(x22), .O(new_n128_));
  nand3  g83(.a(new_n123_), .b(new_n128_), .c(new_n122_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n110_), .O(new_n130_));
  oai21  g85(.a(new_n128_), .b(new_n51_), .c(new_n102_), .O(new_n131_));
  aoi21  g86(.a(new_n130_), .b(new_n51_), .c(new_n131_), .O(new_n132_));
  nor2   g87(.a(new_n132_), .b(new_n65_), .O(z15));
  oai21  g88(.a(new_n129_), .b(x20), .c(x23), .O(new_n134_));
  inv1   g89(.a(x23), .O(new_n135_));
  nor3   g90(.a(new_n114_), .b(x22), .c(x21), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n135_), .c(new_n51_), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n134_), .c(new_n106_), .O(z16));
  nand2  g93(.a(new_n137_), .b(x24), .O(new_n139_));
  nor2   g94(.a(x24), .b(x23), .O(new_n140_));
  nand3  g95(.a(new_n140_), .b(new_n136_), .c(new_n51_), .O(new_n141_));
  nand3  g96(.a(new_n141_), .b(new_n139_), .c(new_n106_), .O(z17));
  nand4  g97(.a(new_n140_), .b(new_n136_), .c(new_n50_), .d(new_n51_), .O(new_n143_));
  nand2  g98(.a(new_n141_), .b(x25), .O(new_n144_));
  nand3  g99(.a(new_n144_), .b(new_n143_), .c(new_n106_), .O(z18));
endmodule


