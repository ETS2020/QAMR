// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x07), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n58_), .c(x23), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n51_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n50_), .O(z01));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g20(.a(new_n61_), .b(new_n50_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(x24), .c(x25), .O(z03));
  inv1   g22(.a(x08), .O(new_n68_));
  nand2  g23(.a(new_n47_), .b(new_n68_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  or2    g31(.a(new_n69_), .b(x13), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand3  g33(.a(new_n47_), .b(x14), .c(new_n68_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  inv1   g35(.a(new_n47_), .O(new_n81_));
  aoi21  g36(.a(new_n68_), .b(x06), .c(new_n81_), .O(z07));
  inv1   g37(.a(x23), .O(new_n83_));
  nor2   g38(.a(x18), .b(x17), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n56_), .c(x20), .O(new_n85_));
  oai21  g40(.a(new_n59_), .b(new_n85_), .c(new_n83_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x24), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n52_), .c(new_n81_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n50_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n63_), .c(new_n47_), .O(z09));
  nand3  g46(.a(new_n63_), .b(new_n89_), .c(new_n50_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n54_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n92_), .c(new_n84_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n97_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n92_), .O(z12));
  nand2  g57(.a(new_n51_), .b(x07), .O(new_n103_));
  nor2   g58(.a(new_n99_), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(x20), .O(new_n105_));
  aoi21  g60(.a(x05), .b(x04), .c(x15), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n104_), .c(new_n50_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n103_), .O(z13));
  inv1   g64(.a(x21), .O(new_n110_));
  aoi21  g65(.a(new_n104_), .b(new_n110_), .c(new_n92_), .O(new_n111_));
  oai21  g66(.a(new_n104_), .b(new_n110_), .c(new_n111_), .O(z14));
  inv1   g67(.a(x22), .O(new_n113_));
  aoi21  g68(.a(new_n104_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  and2   g70(.a(new_n115_), .b(new_n104_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n114_), .c(new_n50_), .O(new_n117_));
  inv1   g72(.a(new_n106_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n50_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n103_), .O(z15));
  aoi21  g75(.a(new_n115_), .b(new_n104_), .c(new_n83_), .O(new_n121_));
  nand3  g76(.a(new_n115_), .b(new_n104_), .c(new_n83_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n106_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n121_), .c(new_n50_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n103_), .O(z16));
  nand2  g80(.a(new_n122_), .b(new_n50_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n51_), .O(new_n127_));
  nand3  g82(.a(new_n122_), .b(x24), .c(new_n50_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n119_), .O(z17));
  oai21  g84(.a(new_n122_), .b(x25), .c(new_n50_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n51_), .O(new_n131_));
  nor2   g86(.a(x24), .b(x23), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n115_), .c(new_n100_), .d(new_n53_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(x25), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n106_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n50_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n131_), .O(z18));
endmodule


