// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  nand2  g02(.a(x15), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n59_), .b(new_n51_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x15), .c(new_n47_), .O(z01));
  and2   g18(.a(new_n48_), .b(x16), .O(z02));
  nand3  g19(.a(new_n59_), .b(new_n48_), .c(new_n51_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(new_n73_));
  aoi21  g28(.a(new_n72_), .b(new_n69_), .c(new_n73_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g33(.a(new_n59_), .b(new_n51_), .c(new_n49_), .O(z08));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n61_), .b(new_n80_), .c(new_n47_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand3  g37(.a(new_n60_), .b(new_n80_), .c(new_n47_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n53_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  nand2  g41(.a(new_n54_), .b(new_n53_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n60_), .c(new_n87_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n80_), .c(x07), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  aoi21  g47(.a(new_n88_), .b(new_n55_), .c(new_n92_), .O(z12));
  nor2   g48(.a(new_n91_), .b(x20), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand2  g50(.a(new_n91_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n84_), .O(z13));
  nand2  g52(.a(new_n95_), .b(x21), .O(new_n98_));
  inv1   g53(.a(x21), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n98_), .c(new_n84_), .O(z14));
  nand2  g56(.a(new_n100_), .b(x22), .O(new_n102_));
  or2    g57(.a(new_n100_), .b(x22), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n84_), .O(z15));
  nand2  g59(.a(new_n103_), .b(x23), .O(new_n105_));
  nor2   g60(.a(x23), .b(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n94_), .c(new_n99_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n84_), .O(z16));
  nand2  g63(.a(new_n107_), .b(x24), .O(new_n109_));
  inv1   g64(.a(x24), .O(new_n110_));
  nand4  g65(.a(new_n106_), .b(new_n94_), .c(new_n110_), .d(new_n99_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n60_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n80_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(z17));
  aoi21  g69(.a(new_n111_), .b(x25), .c(new_n83_), .O(new_n115_));
  oai21  g70(.a(new_n111_), .b(x25), .c(new_n115_), .O(z18));
endmodule


