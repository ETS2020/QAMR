// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x18), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(x24), .O(new_n56_));
  aoi21  g11(.a(x05), .b(x04), .c(x07), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x20), .O(new_n60_));
  inv1   g15(.a(x15), .O(new_n61_));
  inv1   g16(.a(x18), .O(new_n62_));
  nor2   g17(.a(x19), .b(x17), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n61_), .c(new_n62_), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n53_), .c(new_n60_), .O(new_n65_));
  oai22  g20(.a(new_n65_), .b(x25), .c(new_n59_), .d(new_n48_), .O(z01));
  nand2  g21(.a(new_n56_), .b(new_n49_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n48_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  oai21  g34(.a(new_n54_), .b(new_n60_), .c(new_n53_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x24), .O(new_n81_));
  inv1   g36(.a(new_n54_), .O(new_n82_));
  nand4  g37(.a(new_n63_), .b(new_n82_), .c(x24), .d(new_n62_), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n81_), .c(new_n52_), .O(new_n84_));
  and2   g39(.a(new_n84_), .b(new_n49_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x05), .c(x04), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x18), .c(x15), .O(z09));
  nand3  g43(.a(new_n57_), .b(x18), .c(new_n61_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x17), .O(z10));
  inv1   g45(.a(x17), .O(new_n91_));
  nand2  g46(.a(new_n57_), .b(new_n91_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x18), .c(x15), .O(z11));
  nand2  g48(.a(x19), .b(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n89_), .c(new_n63_), .O(z12));
  nand2  g51(.a(new_n94_), .b(x20), .O(new_n97_));
  nand2  g52(.a(new_n95_), .b(new_n60_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n57_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x18), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n61_), .O(z13));
  inv1   g56(.a(new_n89_), .O(new_n102_));
  nand2  g57(.a(new_n98_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(x19), .c(x17), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  nand3  g63(.a(new_n104_), .b(new_n95_), .c(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n57_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x18), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n61_), .O(z15));
  nand2  g67(.a(new_n109_), .b(x23), .O(new_n113_));
  nor2   g68(.a(x23), .b(x22), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n104_), .c(x19), .d(x17), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n102_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  nor2   g72(.a(x24), .b(x21), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n114_), .c(new_n95_), .d(new_n60_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n57_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x18), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n61_), .O(z17));
  inv1   g77(.a(x24), .O(new_n123_));
  nand2  g78(.a(new_n52_), .b(new_n123_), .O(new_n124_));
  aoi21  g79(.a(new_n119_), .b(x25), .c(new_n89_), .O(new_n125_));
  oai21  g80(.a(new_n124_), .b(new_n115_), .c(new_n125_), .O(z18));
endmodule


