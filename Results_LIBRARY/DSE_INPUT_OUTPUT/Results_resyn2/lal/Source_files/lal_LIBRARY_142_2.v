// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n72_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x15), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  aoi21  g04(.a(x05), .b(x04), .c(x07), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x15), .c(new_n47_), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nor2   g08(.a(x18), .b(x17), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n53_), .c(x20), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  nor2   g11(.a(x25), .b(x23), .O(new_n57_));
  oai21  g12(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n52_), .c(new_n50_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nand2  g15(.a(new_n48_), .b(new_n60_), .O(z02));
  nand2  g16(.a(new_n58_), .b(new_n52_), .O(z03));
  xnor2a g17(.a(x12), .b(x03), .O(new_n63_));
  xnor2a g18(.a(x11), .b(x02), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n48_), .O(z04));
  nand2  g24(.a(new_n48_), .b(new_n46_), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(x13), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  nor2   g27(.a(new_n70_), .b(new_n72_), .O(z06));
  aoi22  g28(.a(new_n47_), .b(x15), .c(new_n46_), .d(x06), .O(z07));
  inv1   g29(.a(z03), .O(z08));
  inv1   g30(.a(x07), .O(new_n76_));
  inv1   g31(.a(x15), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(x05), .d(x04), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z09));
  nand2  g34(.a(new_n50_), .b(new_n77_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x17), .c(new_n48_), .O(z10));
  inv1   g36(.a(x17), .O(new_n82_));
  inv1   g37(.a(x18), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  or2    g39(.a(new_n84_), .b(new_n54_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n80_), .c(new_n48_), .O(z11));
  inv1   g41(.a(new_n80_), .O(new_n87_));
  nand2  g42(.a(new_n84_), .b(x19), .O(new_n88_));
  oai21  g43(.a(new_n83_), .b(new_n82_), .c(new_n53_), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n48_), .O(z12));
  nand2  g46(.a(new_n88_), .b(x20), .O(new_n92_));
  inv1   g47(.a(x20), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n92_), .c(new_n87_), .O(z13));
  nand2  g50(.a(x24), .b(x15), .O(new_n96_));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  nand4  g53(.a(new_n84_), .b(new_n98_), .c(new_n93_), .d(x19), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n96_), .O(z14));
  oai21  g57(.a(new_n94_), .b(x21), .c(x22), .O(new_n103_));
  nor2   g58(.a(x22), .b(x21), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n84_), .c(new_n93_), .d(x19), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n50_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n77_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n96_), .O(z15));
  nand2  g63(.a(new_n105_), .b(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  inv1   g65(.a(new_n94_), .O(new_n111_));
  nand3  g66(.a(new_n104_), .b(new_n111_), .c(new_n110_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n109_), .c(new_n87_), .O(z16));
  nand2  g68(.a(new_n112_), .b(x24), .O(new_n114_));
  nor2   g69(.a(x24), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n104_), .c(new_n111_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n87_), .O(z17));
  nand4  g72(.a(new_n115_), .b(new_n104_), .c(new_n111_), .d(new_n51_), .O(new_n118_));
  nand2  g73(.a(new_n116_), .b(x25), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n87_), .O(z18));
endmodule


