// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n92_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x06), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x06), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n48_), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  and2   g21(.a(new_n48_), .b(x16), .O(z02));
  inv1   g22(.a(x25), .O(new_n68_));
  oai21  g23(.a(new_n61_), .b(new_n51_), .c(new_n68_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n48_), .O(z03));
  inv1   g25(.a(x12), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x03), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  nand2  g28(.a(x11), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x03), .O(new_n75_));
  nand2  g30(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .d(new_n72_), .O(new_n79_));
  inv1   g34(.a(x00), .O(new_n80_));
  nand2  g35(.a(x09), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(x01), .O(new_n82_));
  nand2  g37(.a(x10), .b(new_n82_), .O(new_n83_));
  inv1   g38(.a(x09), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x00), .O(new_n85_));
  inv1   g40(.a(x10), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x01), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n85_), .c(new_n83_), .d(new_n81_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n79_), .c(new_n46_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n48_), .O(z04));
  oai21  g45(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g46(.a(x14), .b(new_n46_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n48_), .O(z06));
  aoi21  g48(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand2  g49(.a(new_n61_), .b(x06), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x24), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n68_), .O(z08));
  nor2   g52(.a(x15), .b(x07), .O(new_n98_));
  and2   g53(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n64_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z09));
  nand2  g56(.a(new_n99_), .b(new_n63_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n53_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z10));
  nand2  g60(.a(new_n98_), .b(new_n63_), .O(new_n106_));
  nand2  g61(.a(new_n54_), .b(new_n53_), .O(new_n107_));
  nand2  g62(.a(x18), .b(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n106_), .c(new_n48_), .O(z11));
  inv1   g65(.a(new_n106_), .O(new_n111_));
  nand3  g66(.a(x19), .b(x18), .c(x17), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(new_n55_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n48_), .O(z12));
  inv1   g70(.a(new_n112_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  nand2  g72(.a(new_n112_), .b(x20), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n103_), .O(z13));
  nand2  g74(.a(new_n117_), .b(x21), .O(new_n120_));
  inv1   g75(.a(new_n117_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n58_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n103_), .O(z14));
  nand2  g78(.a(new_n122_), .b(x22), .O(new_n124_));
  nand2  g79(.a(new_n59_), .b(new_n58_), .O(new_n125_));
  nor2   g80(.a(new_n125_), .b(new_n117_), .O(new_n126_));
  inv1   g81(.a(new_n126_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n124_), .c(new_n103_), .O(z15));
  nand2  g83(.a(new_n127_), .b(x23), .O(new_n129_));
  inv1   g84(.a(x23), .O(new_n130_));
  nand2  g85(.a(new_n126_), .b(new_n130_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n129_), .c(new_n103_), .O(z16));
  nand3  g87(.a(new_n126_), .b(new_n51_), .c(new_n130_), .O(new_n133_));
  nand2  g88(.a(new_n131_), .b(x24), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n133_), .c(new_n103_), .O(z17));
  nand4  g90(.a(new_n126_), .b(new_n68_), .c(new_n51_), .d(new_n130_), .O(new_n136_));
  nand2  g91(.a(new_n48_), .b(new_n68_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g93(.a(new_n138_), .b(new_n136_), .c(new_n111_), .O(z18));
endmodule


