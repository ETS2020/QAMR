// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x19), .O(new_n53_));
  nor2   g08(.a(x18), .b(x17), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n53_), .c(x20), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  nor2   g11(.a(x25), .b(x23), .O(new_n57_));
  oai21  g12(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand4  g14(.a(new_n59_), .b(new_n58_), .c(x24), .d(new_n48_), .O(z01));
  nor2   g15(.a(new_n49_), .b(x23), .O(new_n61_));
  oai21  g16(.a(new_n56_), .b(new_n55_), .c(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n49_), .b(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(x25), .O(z03));
  nand2  g19(.a(new_n50_), .b(new_n46_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  nor2   g27(.a(new_n65_), .b(x13), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n65_), .b(new_n74_), .O(z06));
  aoi22  g30(.a(new_n49_), .b(new_n48_), .c(new_n46_), .d(x06), .O(z07));
  nand2  g31(.a(new_n58_), .b(x24), .O(new_n77_));
  nand2  g32(.a(x25), .b(x07), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(z08));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(x05), .c(x04), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x24), .c(x07), .O(z09));
  aoi21  g37(.a(x05), .b(x04), .c(x15), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x24), .c(new_n48_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(x17), .O(z10));
  and2   g40(.a(x18), .b(x17), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n84_), .c(new_n54_), .O(z11));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n86_), .b(x19), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n89_), .c(new_n84_), .O(z12));
  inv1   g46(.a(x20), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n88_), .b(x20), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n83_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x24), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n48_), .O(z13));
  nand2  g52(.a(new_n93_), .b(x21), .O(new_n98_));
  nor2   g53(.a(x21), .b(x20), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n89_), .c(new_n84_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n98_), .O(z14));
  inv1   g56(.a(x22), .O(new_n102_));
  aoi21  g57(.a(new_n99_), .b(new_n89_), .c(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(new_n102_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(new_n88_), .c(new_n83_), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(new_n103_), .c(x24), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(z15));
  inv1   g62(.a(x23), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n89_), .c(new_n108_), .O(new_n110_));
  nor2   g65(.a(x23), .b(x22), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n99_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n88_), .c(new_n83_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(x24), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n48_), .O(z16));
  inv1   g70(.a(new_n83_), .O(new_n116_));
  nand3  g71(.a(new_n111_), .b(new_n99_), .c(new_n89_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n116_), .c(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n48_), .O(z17));
  oai21  g74(.a(new_n116_), .b(x25), .c(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n48_), .O(z18));
endmodule


