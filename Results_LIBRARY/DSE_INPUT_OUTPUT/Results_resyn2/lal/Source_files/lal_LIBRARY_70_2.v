// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n88_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x14), .O(new_n47_));
  nor2   g02(.a(x15), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n48_), .b(new_n63_), .O(z02));
  nand3  g19(.a(new_n59_), .b(new_n50_), .c(new_n51_), .O(z08));
  inv1   g20(.a(z08), .O(z03));
  inv1   g21(.a(x12), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(x03), .O(new_n68_));
  inv1   g23(.a(x02), .O(new_n69_));
  nand2  g24(.a(x11), .b(new_n69_), .O(new_n70_));
  inv1   g25(.a(x03), .O(new_n71_));
  nand2  g26(.a(x12), .b(new_n71_), .O(new_n72_));
  inv1   g27(.a(x11), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x02), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n72_), .c(new_n70_), .d(new_n68_), .O(new_n75_));
  inv1   g30(.a(x00), .O(new_n76_));
  nand2  g31(.a(x09), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(x01), .O(new_n78_));
  nand2  g33(.a(x10), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x09), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x00), .O(new_n81_));
  inv1   g36(.a(x10), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x01), .O(new_n83_));
  nand4  g38(.a(new_n83_), .b(new_n81_), .c(new_n79_), .d(new_n77_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n75_), .c(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(z04));
  oai21  g41(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g42(.a(x15), .b(x14), .c(new_n46_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z06));
  aoi21  g44(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g45(.a(x07), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(x05), .c(x04), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n47_), .c(x15), .O(z09));
  nand2  g48(.a(new_n61_), .b(new_n53_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x15), .O(z10));
  nor2   g50(.a(x18), .b(x17), .O(new_n96_));
  inv1   g51(.a(x15), .O(new_n97_));
  nand3  g52(.a(new_n61_), .b(new_n97_), .c(new_n47_), .O(new_n98_));
  nor2   g53(.a(new_n54_), .b(new_n53_), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n96_), .O(z11));
  oai21  g55(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n101_));
  nand2  g56(.a(new_n99_), .b(x19), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n61_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n47_), .c(x15), .O(z12));
  nand2  g59(.a(new_n102_), .b(x20), .O(new_n105_));
  nand4  g60(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n61_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n97_), .O(z13));
  nand2  g64(.a(new_n106_), .b(x21), .O(new_n110_));
  inv1   g65(.a(x21), .O(new_n111_));
  inv1   g66(.a(new_n106_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n110_), .c(new_n61_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n97_), .O(z14));
  inv1   g71(.a(new_n98_), .O(new_n117_));
  inv1   g72(.a(x22), .O(new_n118_));
  nor2   g73(.a(new_n106_), .b(x21), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n113_), .b(x22), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n117_), .O(z15));
  inv1   g77(.a(x23), .O(new_n123_));
  nand3  g78(.a(new_n119_), .b(new_n123_), .c(new_n118_), .O(new_n124_));
  nand2  g79(.a(new_n120_), .b(x23), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n117_), .O(z16));
  nand2  g81(.a(new_n124_), .b(x24), .O(new_n127_));
  inv1   g82(.a(x24), .O(new_n128_));
  nand4  g83(.a(new_n128_), .b(new_n123_), .c(new_n118_), .d(new_n111_), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(new_n112_), .c(new_n98_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n127_), .O(z17));
  nand3  g87(.a(new_n130_), .b(new_n112_), .c(new_n51_), .O(new_n133_));
  oai21  g88(.a(new_n129_), .b(new_n106_), .c(x25), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n133_), .c(new_n61_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n47_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n97_), .O(z18));
endmodule


