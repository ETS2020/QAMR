// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n111_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x15), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g09(.a(new_n51_), .b(x20), .c(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n50_), .c(new_n49_), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n50_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n56_), .b(x23), .c(new_n60_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  inv1   g18(.a(x23), .O(new_n64_));
  nand2  g19(.a(new_n55_), .b(new_n64_), .O(new_n65_));
  inv1   g20(.a(new_n47_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n50_), .O(new_n67_));
  aoi21  g22(.a(new_n65_), .b(x24), .c(new_n67_), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n66_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n66_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n66_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  nand2  g34(.a(new_n65_), .b(x24), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n66_), .c(new_n50_), .O(z08));
  inv1   g36(.a(x07), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(x05), .c(x04), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x23), .c(x15), .O(z09));
  nand3  g39(.a(new_n58_), .b(x23), .c(new_n49_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(x17), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  and2   g42(.a(x18), .b(x17), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n85_), .c(new_n87_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n88_), .b(x19), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n91_), .c(new_n85_), .O(z12));
  nand2  g48(.a(new_n90_), .b(x20), .O(new_n94_));
  inv1   g49(.a(x20), .O(new_n95_));
  nand2  g50(.a(new_n91_), .b(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n94_), .c(new_n58_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x23), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n49_), .O(z13));
  inv1   g54(.a(new_n85_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x21), .O(new_n101_));
  nand3  g56(.a(new_n91_), .b(new_n52_), .c(new_n95_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(z14));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n104_));
  nand4  g59(.a(new_n91_), .b(new_n53_), .c(new_n52_), .d(new_n95_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n58_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x23), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n49_), .O(z15));
  inv1   g63(.a(new_n105_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n100_), .O(z16));
  oai21  g65(.a(new_n59_), .b(x24), .c(x23), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n49_), .O(z17));
  nand2  g67(.a(new_n100_), .b(new_n50_), .O(z18));
endmodule


