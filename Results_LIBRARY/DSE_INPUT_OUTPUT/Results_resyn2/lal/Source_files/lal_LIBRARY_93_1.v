// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nand2  g03(.a(x23), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  nor2   g06(.a(x25), .b(x24), .O(new_n52_));
  inv1   g07(.a(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(new_n52_), .c(new_n49_), .O(new_n56_));
  inv1   g11(.a(x23), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nor2   g13(.a(x18), .b(x17), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(x20), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(new_n60_), .c(new_n57_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x25), .c(new_n56_), .O(z01));
  inv1   g18(.a(x24), .O(new_n64_));
  nand2  g19(.a(new_n49_), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n62_), .c(x25), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  nand2  g28(.a(new_n49_), .b(new_n46_), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x13), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor2   g31(.a(new_n74_), .b(new_n76_), .O(z06));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nor2   g33(.a(new_n64_), .b(new_n57_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x25), .c(x15), .O(new_n80_));
  inv1   g35(.a(x20), .O(new_n81_));
  nand2  g36(.a(new_n59_), .b(new_n58_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g38(.a(x24), .b(x22), .c(x21), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n83_), .c(x25), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x23), .c(new_n80_), .O(z08));
  nor4   g42(.a(new_n54_), .b(x23), .c(x15), .d(x07), .O(z09));
  inv1   g43(.a(x17), .O(new_n89_));
  nand4  g44(.a(new_n54_), .b(new_n57_), .c(new_n48_), .d(new_n53_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  inv1   g48(.a(new_n55_), .O(new_n94_));
  and2   g49(.a(x18), .b(x17), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n57_), .c(x15), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n95_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n90_), .O(z12));
  nand2  g57(.a(new_n99_), .b(x20), .O(new_n103_));
  nand4  g58(.a(new_n81_), .b(x19), .c(x18), .d(x17), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n94_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n57_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(z13));
  nand2  g62(.a(new_n104_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n58_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n95_), .c(new_n109_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n108_), .c(new_n91_), .O(z14));
  oai21  g67(.a(new_n104_), .b(x21), .c(x22), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n110_), .c(new_n95_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n94_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n57_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n48_), .O(z15));
  nand2  g73(.a(new_n115_), .b(new_n91_), .O(z16));
  inv1   g74(.a(x22), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n109_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n104_), .c(x24), .O(new_n122_));
  nand4  g77(.a(new_n114_), .b(new_n100_), .c(new_n64_), .d(new_n81_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n94_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n57_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n48_), .O(z17));
  nand4  g81(.a(new_n64_), .b(new_n120_), .c(new_n109_), .d(new_n81_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n99_), .c(x25), .O(new_n128_));
  nand4  g83(.a(new_n114_), .b(new_n110_), .c(new_n95_), .d(new_n52_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n128_), .c(new_n94_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n57_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n48_), .O(z18));
endmodule


