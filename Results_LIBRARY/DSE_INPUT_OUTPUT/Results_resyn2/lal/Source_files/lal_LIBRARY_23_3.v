// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n84_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n47_), .c(new_n61_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(z01));
  inv1   g20(.a(new_n47_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n46_), .O(z02));
  inv1   g22(.a(x23), .O(new_n68_));
  nor2   g23(.a(x18), .b(x17), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n54_), .c(x20), .O(new_n70_));
  oai21  g25(.a(new_n57_), .b(new_n70_), .c(new_n68_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(x24), .c(x25), .O(z03));
  inv1   g27(.a(x08), .O(new_n73_));
  nand2  g28(.a(new_n47_), .b(new_n73_), .O(new_n74_));
  xor2a  g29(.a(x12), .b(x03), .O(new_n75_));
  xor2a  g30(.a(x11), .b(x02), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  xor2a  g33(.a(x10), .b(x01), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z04));
  or2    g36(.a(new_n74_), .b(x13), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z05));
  inv1   g38(.a(x14), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g40(.a(new_n73_), .b(x06), .c(new_n66_), .O(z07));
  aoi21  g41(.a(new_n59_), .b(new_n50_), .c(new_n66_), .O(z08));
  inv1   g42(.a(x15), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n61_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n62_), .c(new_n47_), .O(z09));
  nand3  g45(.a(new_n62_), .b(new_n88_), .c(new_n61_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g47(.a(new_n64_), .b(new_n88_), .O(new_n93_));
  inv1   g48(.a(new_n69_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n93_), .O(z11));
  inv1   g52(.a(new_n91_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(new_n54_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(z12));
  inv1   g57(.a(new_n93_), .O(new_n103_));
  xor2a  g58(.a(new_n99_), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(z13));
  oai21  g60(.a(new_n99_), .b(x20), .c(x21), .O(new_n106_));
  inv1   g61(.a(new_n99_), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n107_), .c(new_n91_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n106_), .c(new_n66_), .O(z14));
  nand2  g65(.a(new_n108_), .b(new_n107_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n108_), .b(new_n107_), .c(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n103_), .O(z15));
  nand2  g70(.a(new_n91_), .b(new_n47_), .O(new_n116_));
  inv1   g71(.a(x24), .O(new_n117_));
  nand3  g72(.a(new_n114_), .b(new_n117_), .c(x23), .O(new_n118_));
  nor3   g73(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n107_), .c(new_n68_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n116_), .O(z16));
  nand2  g76(.a(new_n114_), .b(new_n117_), .O(new_n122_));
  nand3  g77(.a(new_n119_), .b(new_n107_), .c(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n68_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n116_), .O(z17));
  nor2   g80(.a(x25), .b(x24), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n119_), .c(new_n107_), .d(new_n68_), .O(new_n127_));
  oai21  g82(.a(new_n114_), .b(x24), .c(x25), .O(new_n128_));
  oai21  g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n128_), .c(new_n127_), .d(new_n98_), .O(z18));
endmodule


