// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n58_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n58_), .b(new_n47_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g27(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z06));
  inv1   g29(.a(new_n47_), .O(new_n75_));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n75_), .O(z07));
  oai21  g31(.a(new_n56_), .b(x23), .c(x24), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(z08));
  inv1   g33(.a(x07), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n79_), .c(x05), .d(x04), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n47_), .O(z09));
  nand2  g37(.a(new_n59_), .b(new_n80_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g39(.a(new_n52_), .b(new_n51_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n83_), .c(new_n47_), .O(z11));
  nand2  g43(.a(new_n86_), .b(new_n53_), .O(new_n89_));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n83_), .c(new_n47_), .O(z12));
  inv1   g47(.a(new_n90_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n50_), .O(new_n94_));
  aoi21  g49(.a(new_n90_), .b(x20), .c(new_n83_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n94_), .c(new_n75_), .O(z13));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  nor2   g52(.a(x21), .b(x20), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n93_), .c(new_n83_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n97_), .c(new_n75_), .O(z14));
  inv1   g55(.a(new_n83_), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n93_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x22), .O(new_n103_));
  inv1   g58(.a(x22), .O(new_n104_));
  nand3  g59(.a(new_n98_), .b(new_n93_), .c(new_n104_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n103_), .c(new_n101_), .d(new_n47_), .O(z15));
  inv1   g61(.a(x24), .O(new_n107_));
  nand4  g62(.a(new_n98_), .b(new_n93_), .c(new_n107_), .d(new_n104_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  nand4  g65(.a(new_n98_), .b(new_n93_), .c(new_n110_), .d(new_n104_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n101_), .O(z16));
  nand2  g67(.a(new_n83_), .b(new_n47_), .O(new_n113_));
  xor2a  g68(.a(new_n105_), .b(x24), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(x23), .c(new_n113_), .O(z17));
  inv1   g70(.a(new_n111_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n49_), .c(new_n107_), .O(new_n117_));
  nand2  g72(.a(new_n108_), .b(x25), .O(new_n118_));
  oai21  g73(.a(x25), .b(x24), .c(x23), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n101_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n118_), .c(new_n117_), .O(z18));
endmodule


