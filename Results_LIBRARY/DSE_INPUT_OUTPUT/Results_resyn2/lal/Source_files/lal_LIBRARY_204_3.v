// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_;
  nand2  g00(.a(x24), .b(x10), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(x08), .O(new_n48_));
  nand2  g03(.a(new_n46_), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x10), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n52_), .c(new_n53_), .O(new_n64_));
  inv1   g19(.a(x07), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  oai21  g23(.a(new_n64_), .b(x25), .c(new_n68_), .O(z01));
  nor2   g24(.a(new_n64_), .b(x25), .O(z03));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x12), .b(x03), .O(new_n74_));
  xor2a  g29(.a(x09), .b(x00), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n49_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  nand2  g33(.a(new_n50_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  inv1   g35(.a(new_n46_), .O(new_n81_));
  aoi21  g36(.a(new_n48_), .b(x06), .c(new_n81_), .O(z07));
  inv1   g37(.a(x25), .O(new_n83_));
  inv1   g38(.a(x23), .O(new_n84_));
  nand2  g39(.a(new_n62_), .b(new_n59_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x24), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n83_), .c(new_n81_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand3  g44(.a(new_n46_), .b(new_n89_), .c(new_n65_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n66_), .O(z09));
  inv1   g46(.a(new_n90_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n66_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n55_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nand3  g51(.a(new_n66_), .b(new_n89_), .c(new_n65_), .O(new_n97_));
  xnor2a g52(.a(x18), .b(x17), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n97_), .c(new_n46_), .O(z11));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  aoi21  g56(.a(x18), .b(x17), .c(x19), .O(new_n102_));
  nor3   g57(.a(new_n102_), .b(new_n101_), .c(new_n93_), .O(z12));
  nand2  g58(.a(new_n101_), .b(new_n54_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n94_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n104_), .b(x21), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n94_), .O(z14));
  nand2  g65(.a(new_n109_), .b(x22), .O(new_n111_));
  nand2  g66(.a(new_n108_), .b(new_n61_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n94_), .O(z15));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n114_));
  nor2   g69(.a(new_n100_), .b(x20), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n84_), .c(new_n61_), .d(new_n60_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n94_), .O(z16));
  inv1   g72(.a(new_n97_), .O(new_n118_));
  oai21  g73(.a(new_n116_), .b(x10), .c(x24), .O(new_n119_));
  nor2   g74(.a(x24), .b(x23), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n108_), .c(new_n61_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(z17));
  nand4  g77(.a(new_n120_), .b(new_n108_), .c(new_n83_), .d(new_n61_), .O(new_n123_));
  nand2  g78(.a(new_n46_), .b(new_n83_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n118_), .O(z18));
endmodule


