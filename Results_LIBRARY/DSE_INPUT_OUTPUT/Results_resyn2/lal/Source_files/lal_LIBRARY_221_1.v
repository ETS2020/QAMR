// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x20), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n48_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n50_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n61_), .O(z01));
  nand3  g19(.a(new_n60_), .b(new_n50_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g27(.a(new_n50_), .b(new_n46_), .O(new_n73_));
  aoi21  g28(.a(new_n72_), .b(new_n69_), .c(new_n73_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g33(.a(new_n60_), .b(new_n53_), .c(new_n49_), .O(z08));
  nand2  g34(.a(x05), .b(x04), .O(new_n80_));
  nor4   g35(.a(new_n80_), .b(x20), .c(x15), .d(x07), .O(z09));
  nor3   g36(.a(x20), .b(x15), .c(x07), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n54_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nor2   g42(.a(new_n55_), .b(new_n54_), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n83_), .c(new_n87_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  oai21  g47(.a(new_n88_), .b(x19), .c(new_n92_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n48_), .c(x15), .O(z12));
  inv1   g49(.a(x15), .O(new_n95_));
  oai21  g50(.a(new_n92_), .b(x20), .c(new_n95_), .O(z13));
  inv1   g51(.a(x21), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(x19), .c(x18), .d(x17), .O(new_n98_));
  nand2  g53(.a(new_n90_), .b(x21), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n84_), .O(z14));
  nand2  g55(.a(new_n98_), .b(x22), .O(new_n101_));
  inv1   g56(.a(x22), .O(new_n102_));
  inv1   g57(.a(new_n90_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n97_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n101_), .c(new_n84_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  inv1   g61(.a(x23), .O(new_n107_));
  nand4  g62(.a(new_n103_), .b(new_n107_), .c(new_n102_), .d(new_n97_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n62_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n95_), .O(z16));
  nand2  g66(.a(new_n108_), .b(x24), .O(new_n112_));
  nor2   g67(.a(x24), .b(x23), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n103_), .c(new_n102_), .d(new_n97_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n84_), .O(z17));
  nor2   g70(.a(x25), .b(x22), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n98_), .c(new_n62_), .O(new_n118_));
  aoi21  g73(.a(new_n114_), .b(x25), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x20), .c(new_n95_), .O(z18));
endmodule


