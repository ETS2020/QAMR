// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n56_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n68_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x23), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(z03));
  inv1   g09(.a(z03), .O(z08));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(z08), .c(new_n51_), .O(z01));
  inv1   g12(.a(x16), .O(new_n58_));
  nand2  g13(.a(new_n49_), .b(new_n58_), .O(z02));
  xnor2a g14(.a(x12), .b(x03), .O(new_n60_));
  xnor2a g15(.a(x11), .b(x02), .O(new_n61_));
  xnor2a g16(.a(x09), .b(x00), .O(new_n62_));
  xnor2a g17(.a(x10), .b(x01), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n60_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n49_), .O(z04));
  oai21  g21(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g22(.a(x14), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n49_), .O(z06));
  nand3  g24(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g25(.a(new_n56_), .O(new_n71_));
  nor3   g26(.a(new_n48_), .b(x15), .c(x07), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z09));
  inv1   g29(.a(x15), .O(new_n75_));
  nand3  g30(.a(new_n56_), .b(new_n75_), .c(new_n51_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x17), .c(new_n49_), .O(z10));
  xnor2a g32(.a(x18), .b(x17), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z11));
  nand3  g34(.a(x19), .b(x18), .c(x17), .O(new_n80_));
  inv1   g35(.a(x19), .O(new_n81_));
  nand2  g36(.a(x18), .b(x17), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n49_), .O(z12));
  inv1   g40(.a(x20), .O(new_n86_));
  inv1   g41(.a(new_n80_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g43(.a(new_n80_), .b(x20), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n88_), .c(new_n72_), .d(new_n56_), .O(z13));
  nand2  g45(.a(new_n88_), .b(x21), .O(new_n91_));
  nor2   g46(.a(x21), .b(x20), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n87_), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n91_), .c(new_n72_), .d(new_n56_), .O(z14));
  nand2  g49(.a(new_n93_), .b(x22), .O(new_n95_));
  inv1   g50(.a(x21), .O(new_n96_));
  inv1   g51(.a(x22), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n86_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n80_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n76_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n95_), .c(new_n48_), .O(z15));
  oai21  g56(.a(new_n98_), .b(new_n80_), .c(x23), .O(new_n102_));
  inv1   g57(.a(x23), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(new_n103_), .c(new_n76_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n102_), .c(new_n48_), .O(z16));
  nand2  g60(.a(new_n104_), .b(new_n47_), .O(z17));
  nand4  g61(.a(new_n92_), .b(new_n87_), .c(new_n103_), .d(new_n97_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n52_), .O(new_n108_));
  nand2  g63(.a(new_n99_), .b(x25), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n47_), .O(new_n110_));
  aoi22  g65(.a(new_n76_), .b(new_n49_), .c(x25), .d(x23), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n110_), .O(z18));
endmodule


