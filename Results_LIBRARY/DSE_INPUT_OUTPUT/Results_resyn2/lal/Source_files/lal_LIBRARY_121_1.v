// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x03), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x03), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n48_), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  and2   g21(.a(new_n48_), .b(x16), .O(z02));
  inv1   g22(.a(x25), .O(new_n68_));
  oai21  g23(.a(new_n61_), .b(new_n51_), .c(new_n68_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n48_), .O(z03));
  nand2  g25(.a(new_n48_), .b(new_n46_), .O(new_n71_));
  xor2a  g26(.a(x12), .b(x03), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  xor2a  g29(.a(x11), .b(x02), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(z04));
  nor2   g33(.a(new_n71_), .b(x13), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor2   g35(.a(new_n71_), .b(new_n80_), .O(z06));
  nand3  g36(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g37(.a(new_n61_), .b(x03), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n68_), .O(z08));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  and2   g41(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n64_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand2  g44(.a(new_n87_), .b(new_n63_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n53_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n86_), .b(new_n63_), .O(new_n94_));
  nand2  g49(.a(new_n54_), .b(new_n53_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n94_), .c(new_n48_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  aoi21  g54(.a(new_n96_), .b(new_n55_), .c(new_n94_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(z12));
  inv1   g57(.a(new_n99_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n91_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n104_), .b(x21), .O(new_n108_));
  nor2   g63(.a(new_n108_), .b(new_n94_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n107_), .c(new_n49_), .O(z14));
  oai21  g65(.a(new_n104_), .b(x21), .c(x22), .O(new_n111_));
  aoi21  g66(.a(new_n108_), .b(new_n59_), .c(new_n94_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n111_), .c(new_n49_), .O(z15));
  nand2  g68(.a(new_n108_), .b(new_n59_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x23), .O(new_n115_));
  nor2   g70(.a(new_n99_), .b(x20), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n58_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n91_), .O(z16));
  inv1   g74(.a(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n51_), .O(new_n121_));
  aoi21  g76(.a(new_n118_), .b(x24), .c(new_n94_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n121_), .c(new_n49_), .O(z17));
  nor2   g78(.a(new_n68_), .b(new_n51_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n94_), .c(new_n48_), .O(new_n125_));
  nor2   g80(.a(new_n118_), .b(new_n68_), .O(new_n126_));
  oai21  g81(.a(new_n120_), .b(x25), .c(new_n51_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(z18));
endmodule


