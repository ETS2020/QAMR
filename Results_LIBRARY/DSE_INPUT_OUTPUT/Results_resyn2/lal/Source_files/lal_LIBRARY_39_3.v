// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n102_, new_n104_;
  inv1   g00(.a(x17), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  aoi21  g05(.a(new_n49_), .b(x16), .c(new_n50_), .O(z00));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x19), .b(x18), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n52_), .c(new_n53_), .O(new_n59_));
  inv1   g14(.a(x23), .O(new_n60_));
  nand3  g15(.a(new_n53_), .b(new_n60_), .c(new_n54_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(x17), .c(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n59_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nand2  g24(.a(new_n62_), .b(x17), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n59_), .O(z03));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x08), .c(new_n49_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand2  g34(.a(new_n50_), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  nand2  g36(.a(new_n50_), .b(x14), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  nand2  g38(.a(new_n50_), .b(x06), .O(z07));
  inv1   g39(.a(x19), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n47_), .c(new_n46_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n54_), .c(new_n56_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x23), .c(x24), .O(new_n88_));
  nor2   g43(.a(new_n48_), .b(x25), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(z08));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n64_), .c(new_n49_), .O(z09));
  nand2  g48(.a(new_n91_), .b(new_n64_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n46_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  nor2   g52(.a(x18), .b(x17), .O(new_n98_));
  aoi21  g53(.a(new_n94_), .b(new_n49_), .c(new_n98_), .O(z11));
  nor4   g54(.a(new_n65_), .b(new_n48_), .c(new_n85_), .d(x15), .O(z12));
  aoi21  g55(.a(new_n95_), .b(new_n54_), .c(new_n48_), .O(z13));
  inv1   g56(.a(x21), .O(new_n102_));
  nand3  g57(.a(new_n95_), .b(new_n49_), .c(new_n102_), .O(z14));
  inv1   g58(.a(x22), .O(new_n104_));
  nand3  g59(.a(new_n95_), .b(new_n49_), .c(new_n104_), .O(z15));
  aoi21  g60(.a(new_n95_), .b(new_n60_), .c(new_n48_), .O(z16));
  aoi21  g61(.a(new_n95_), .b(new_n52_), .c(new_n48_), .O(z17));
  nand2  g62(.a(new_n95_), .b(new_n89_), .O(z18));
endmodule


