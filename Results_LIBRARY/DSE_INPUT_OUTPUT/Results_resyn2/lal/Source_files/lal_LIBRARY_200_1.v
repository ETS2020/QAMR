// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(new_n46_), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n58_), .c(new_n47_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n58_), .b(new_n47_), .O(z03));
  inv1   g19(.a(x10), .O(new_n65_));
  inv1   g20(.a(x00), .O(new_n66_));
  aoi22  g21(.a(new_n65_), .b(x01), .c(x09), .d(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n65_), .b(x01), .c(new_n67_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(new_n69_));
  inv1   g24(.a(x12), .O(new_n70_));
  inv1   g25(.a(x24), .O(new_n71_));
  oai21  g26(.a(new_n70_), .b(x03), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x11), .O(new_n73_));
  oai22  g28(.a(new_n73_), .b(x02), .c(x09), .d(new_n66_), .O(new_n74_));
  nand2  g29(.a(new_n70_), .b(x03), .O(new_n75_));
  nand2  g30(.a(new_n73_), .b(x02), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n69_), .c(x08), .O(z04));
  aoi21  g34(.a(new_n71_), .b(x13), .c(x08), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  aoi21  g36(.a(new_n71_), .b(new_n81_), .c(x08), .O(z06));
  oai21  g37(.a(x24), .b(x06), .c(new_n46_), .O(z07));
  inv1   g38(.a(new_n47_), .O(new_n84_));
  aoi21  g39(.a(new_n57_), .b(new_n49_), .c(new_n84_), .O(z08));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n60_), .c(new_n47_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand3  g43(.a(new_n86_), .b(new_n59_), .c(new_n47_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n51_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  nand2  g47(.a(new_n86_), .b(new_n59_), .O(new_n93_));
  nand2  g48(.a(new_n52_), .b(new_n51_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n93_), .c(new_n47_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  aoi21  g53(.a(new_n95_), .b(new_n53_), .c(new_n93_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n47_), .O(z12));
  nor2   g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  aoi21  g58(.a(new_n98_), .b(x20), .c(new_n93_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n84_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  aoi21  g62(.a(new_n102_), .b(new_n107_), .c(new_n93_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n106_), .c(new_n84_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n102_), .b(new_n110_), .c(new_n107_), .O(new_n111_));
  oai21  g66(.a(new_n103_), .b(x21), .c(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n90_), .O(z15));
  nand2  g68(.a(new_n111_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n102_), .c(new_n107_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n90_), .O(z16));
  inv1   g72(.a(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n71_), .O(new_n119_));
  aoi21  g74(.a(new_n116_), .b(x24), .c(new_n93_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n119_), .c(new_n84_), .O(z17));
  nor2   g76(.a(new_n49_), .b(new_n71_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n93_), .c(new_n47_), .O(new_n123_));
  nor2   g78(.a(new_n116_), .b(new_n49_), .O(new_n124_));
  nand2  g79(.a(new_n116_), .b(new_n49_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n71_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z18));
endmodule


