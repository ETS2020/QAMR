// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n74_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_;
  inv1   g00(.a(x16), .O(new_n46_));
  nor2   g01(.a(x15), .b(x11), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(z01));
  aoi21  g17(.a(new_n59_), .b(new_n51_), .c(new_n47_), .O(z08));
  inv1   g18(.a(z08), .O(z03));
  inv1   g19(.a(new_n49_), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  xor2a  g21(.a(x10), .b(x01), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  nor2   g27(.a(new_n65_), .b(x13), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  oai22  g29(.a(x15), .b(x11), .c(new_n74_), .d(x08), .O(z06));
  nand2  g30(.a(new_n49_), .b(x06), .O(z07));
  inv1   g31(.a(x07), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(x05), .c(x04), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(x11), .c(x15), .O(z09));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n61_), .b(new_n80_), .c(x11), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(x17), .O(z10));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(x18), .b(x17), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nor3   g40(.a(new_n85_), .b(new_n81_), .c(new_n83_), .O(z11));
  nand2  g41(.a(new_n84_), .b(new_n55_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n87_), .c(new_n61_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x11), .c(x15), .O(z12));
  inv1   g45(.a(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n52_), .O(new_n92_));
  aoi21  g47(.a(new_n88_), .b(x20), .c(new_n81_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(z13));
  nand2  g49(.a(new_n92_), .b(x21), .O(new_n95_));
  nor2   g50(.a(x21), .b(x20), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n91_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n95_), .c(new_n61_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x11), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n80_), .O(z14));
  inv1   g55(.a(x22), .O(new_n101_));
  nor3   g56(.a(new_n88_), .b(x21), .c(x20), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n81_), .O(new_n103_));
  oai21  g58(.a(new_n102_), .b(new_n101_), .c(new_n103_), .O(z15));
  oai21  g59(.a(new_n97_), .b(x22), .c(x23), .O(new_n105_));
  nor2   g60(.a(x23), .b(x22), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n96_), .c(new_n91_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n61_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x11), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n80_), .O(z16));
  nand2  g65(.a(new_n107_), .b(x24), .O(new_n111_));
  inv1   g66(.a(x24), .O(new_n112_));
  nand3  g67(.a(new_n106_), .b(new_n102_), .c(new_n112_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n61_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x11), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n80_), .O(z17));
  aoi21  g71(.a(new_n113_), .b(x25), .c(new_n81_), .O(new_n117_));
  oai21  g72(.a(new_n113_), .b(x25), .c(new_n117_), .O(z18));
endmodule


