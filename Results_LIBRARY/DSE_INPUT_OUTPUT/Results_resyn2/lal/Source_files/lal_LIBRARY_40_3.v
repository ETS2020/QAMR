// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n76_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x25), .b(x22), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  inv1   g13(.a(x07), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n47_), .c(new_n59_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(z01));
  and2   g18(.a(new_n47_), .b(x16), .O(z02));
  nand2  g19(.a(new_n58_), .b(new_n47_), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n47_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g28(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z06));
  inv1   g30(.a(new_n47_), .O(new_n76_));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n76_), .O(z07));
  aoi21  g32(.a(new_n57_), .b(new_n49_), .c(new_n76_), .O(z08));
  nor4   g33(.a(new_n60_), .b(new_n76_), .c(x15), .d(x07), .O(z09));
  inv1   g34(.a(x15), .O(new_n80_));
  nand2  g35(.a(new_n62_), .b(new_n80_), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(x17), .O(z10));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nor2   g38(.a(new_n52_), .b(new_n51_), .O(new_n84_));
  nor3   g39(.a(new_n84_), .b(new_n81_), .c(new_n83_), .O(z11));
  nor2   g40(.a(new_n84_), .b(x19), .O(new_n86_));
  nand3  g41(.a(new_n60_), .b(new_n80_), .c(new_n59_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n86_), .c(new_n47_), .O(z12));
  inv1   g46(.a(new_n89_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(new_n93_));
  aoi21  g48(.a(new_n89_), .b(x20), .c(new_n87_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n93_), .c(new_n76_), .O(z13));
  nand2  g50(.a(new_n93_), .b(x21), .O(new_n96_));
  nor2   g51(.a(x21), .b(x20), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n92_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n87_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n96_), .c(new_n76_), .O(z14));
  inv1   g56(.a(x22), .O(new_n102_));
  nand3  g57(.a(new_n97_), .b(new_n92_), .c(new_n102_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(new_n81_), .O(new_n105_));
  oai21  g60(.a(new_n99_), .b(new_n102_), .c(new_n105_), .O(z15));
  inv1   g61(.a(x23), .O(new_n107_));
  nand2  g62(.a(new_n104_), .b(new_n107_), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(x23), .c(new_n87_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n108_), .c(new_n76_), .O(z16));
  inv1   g65(.a(x24), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n102_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n87_), .c(new_n47_), .O(new_n113_));
  nand3  g68(.a(new_n99_), .b(x24), .c(new_n107_), .O(new_n114_));
  oai21  g69(.a(new_n98_), .b(x23), .c(new_n111_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n102_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n113_), .O(z17));
  nand4  g72(.a(new_n104_), .b(new_n49_), .c(new_n111_), .d(new_n107_), .O(new_n118_));
  nand2  g73(.a(new_n111_), .b(new_n107_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n103_), .c(x25), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n88_), .O(z18));
endmodule


