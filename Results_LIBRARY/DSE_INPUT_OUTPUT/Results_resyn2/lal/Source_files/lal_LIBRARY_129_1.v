// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x12), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n59_), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n48_), .c(new_n53_), .O(new_n63_));
  aoi21  g18(.a(x05), .b(x04), .c(x07), .O(new_n64_));
  and2   g19(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n63_), .O(z01));
  nand2  g21(.a(new_n62_), .b(x12), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  nand2  g23(.a(new_n50_), .b(new_n46_), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  inv1   g25(.a(x02), .O(new_n71_));
  nand2  g26(.a(x11), .b(new_n71_), .O(new_n72_));
  inv1   g27(.a(x09), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(x00), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(new_n75_));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  inv1   g31(.a(x00), .O(new_n77_));
  nand2  g32(.a(x09), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(x11), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x02), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(new_n75_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n69_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor2   g40(.a(new_n69_), .b(new_n85_), .O(z06));
  nand3  g41(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x12), .O(new_n88_));
  inv1   g43(.a(x23), .O(new_n89_));
  nor2   g44(.a(x18), .b(x17), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n57_), .c(x20), .O(new_n91_));
  oai21  g46(.a(new_n60_), .b(new_n91_), .c(new_n89_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n88_), .c(x24), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n53_), .O(z08));
  nor2   g49(.a(x15), .b(x07), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n50_), .c(x05), .d(x04), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z09));
  inv1   g52(.a(x15), .O(new_n98_));
  nand2  g53(.a(new_n64_), .b(new_n98_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g55(.a(new_n99_), .b(new_n49_), .O(new_n101_));
  nor2   g56(.a(new_n56_), .b(new_n55_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n90_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z11));
  inv1   g60(.a(new_n99_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n50_), .O(new_n107_));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nor2   g64(.a(new_n102_), .b(x19), .O(new_n110_));
  nor3   g65(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(z12));
  nand4  g66(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  aoi21  g67(.a(new_n108_), .b(x20), .c(new_n99_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n112_), .c(new_n49_), .O(z13));
  nand2  g69(.a(new_n112_), .b(x21), .O(new_n115_));
  inv1   g70(.a(x21), .O(new_n116_));
  inv1   g71(.a(new_n112_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n116_), .c(new_n99_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n115_), .c(new_n49_), .O(z14));
  oai21  g74(.a(new_n112_), .b(x21), .c(x22), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand3  g76(.a(new_n117_), .b(new_n121_), .c(new_n116_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n101_), .O(z15));
  nand2  g78(.a(new_n122_), .b(x23), .O(new_n124_));
  nand2  g79(.a(new_n121_), .b(new_n116_), .O(new_n125_));
  nor2   g80(.a(new_n125_), .b(new_n112_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n89_), .c(new_n99_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n124_), .c(new_n49_), .O(z16));
  nand2  g83(.a(new_n99_), .b(new_n50_), .O(new_n129_));
  nand2  g84(.a(new_n126_), .b(new_n89_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(x24), .c(x12), .O(new_n131_));
  nand3  g86(.a(new_n126_), .b(new_n48_), .c(new_n89_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n129_), .O(z17));
  nand4  g88(.a(new_n126_), .b(new_n53_), .c(new_n48_), .d(new_n89_), .O(new_n134_));
  nand2  g89(.a(new_n50_), .b(new_n53_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n134_), .c(new_n106_), .O(z18));
endmodule


