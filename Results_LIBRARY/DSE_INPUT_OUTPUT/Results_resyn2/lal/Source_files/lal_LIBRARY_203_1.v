// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  aoi21  g02(.a(new_n47_), .b(new_n46_), .c(x16), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n47_), .c(new_n49_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nor2   g15(.a(new_n47_), .b(x16), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n47_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n58_), .b(x16), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  inv1   g23(.a(new_n61_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  nor2   g32(.a(new_n70_), .b(x13), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n69_), .O(z06));
  nand3  g35(.a(new_n69_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x23), .O(new_n82_));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n53_), .c(x20), .O(new_n84_));
  oai21  g39(.a(new_n56_), .b(new_n84_), .c(new_n82_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n49_), .c(new_n61_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n60_), .c(new_n69_), .O(z09));
  nand3  g46(.a(new_n62_), .b(new_n60_), .c(new_n89_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(x17), .O(z10));
  nand3  g48(.a(new_n60_), .b(new_n89_), .c(new_n88_), .O(new_n94_));
  inv1   g49(.a(new_n83_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n94_), .c(new_n69_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  aoi21  g54(.a(new_n96_), .b(new_n53_), .c(new_n94_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n69_), .O(z12));
  nand4  g57(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(x20), .c(new_n94_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n61_), .O(z13));
  inv1   g60(.a(new_n92_), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  inv1   g62(.a(new_n103_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g64(.a(new_n103_), .b(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(z14));
  nand2  g66(.a(new_n109_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n108_), .b(new_n113_), .c(new_n107_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n106_), .O(z15));
  nand2  g70(.a(new_n114_), .b(x23), .O(new_n116_));
  nand2  g71(.a(new_n113_), .b(new_n107_), .O(new_n117_));
  nor2   g72(.a(new_n117_), .b(new_n103_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n82_), .c(new_n94_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n116_), .c(new_n61_), .O(z16));
  nand3  g75(.a(new_n118_), .b(new_n47_), .c(new_n82_), .O(new_n121_));
  nand2  g76(.a(new_n118_), .b(new_n82_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n106_), .O(z17));
  nand4  g79(.a(new_n118_), .b(new_n49_), .c(new_n47_), .d(new_n82_), .O(new_n125_));
  nand2  g80(.a(new_n122_), .b(x25), .O(new_n126_));
  nand2  g81(.a(new_n49_), .b(x16), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(x24), .c(new_n94_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n126_), .c(new_n125_), .O(z18));
endmodule


