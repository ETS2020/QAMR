// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x25), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x22), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  aoi21  g14(.a(new_n47_), .b(x23), .c(x22), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n60_), .c(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n60_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n59_), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor3   g32(.a(new_n48_), .b(new_n77_), .c(x08), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g34(.a(x22), .O(new_n80_));
  aoi21  g35(.a(new_n57_), .b(x24), .c(x25), .O(new_n81_));
  nand3  g36(.a(new_n47_), .b(x24), .c(x23), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n62_), .c(new_n49_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n84_), .b(new_n61_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n49_), .c(new_n53_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(new_n54_), .b(new_n53_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n88_), .c(new_n91_), .d(new_n49_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  aoi21  g49(.a(x18), .b(x17), .c(x19), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n95_), .c(new_n49_), .O(z12));
  nand2  g53(.a(new_n96_), .b(x20), .O(new_n99_));
  nand4  g54(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n87_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n99_), .c(new_n48_), .O(z13));
  nand2  g58(.a(new_n100_), .b(x21), .O(new_n104_));
  nor3   g59(.a(new_n96_), .b(x21), .c(x20), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n104_), .c(new_n48_), .O(z14));
  oai21  g62(.a(new_n105_), .b(x25), .c(new_n80_), .O(new_n108_));
  inv1   g63(.a(new_n105_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n88_), .O(z15));
  nand2  g66(.a(new_n108_), .b(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  nand4  g68(.a(new_n47_), .b(new_n113_), .c(new_n80_), .d(new_n52_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n100_), .O(new_n115_));
  aoi21  g70(.a(new_n84_), .b(new_n61_), .c(new_n48_), .O(new_n116_));
  nor2   g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n112_), .O(z16));
  inv1   g73(.a(x24), .O(new_n119_));
  inv1   g74(.a(new_n96_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n113_), .c(new_n52_), .d(new_n51_), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n47_), .c(x22), .O(new_n122_));
  nand4  g77(.a(new_n119_), .b(new_n113_), .c(new_n80_), .d(new_n52_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(new_n100_), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n47_), .c(new_n116_), .O(new_n125_));
  oai21  g80(.a(new_n122_), .b(new_n119_), .c(new_n125_), .O(z17));
  nand2  g81(.a(new_n88_), .b(new_n47_), .O(new_n127_));
  or2    g82(.a(new_n127_), .b(new_n124_), .O(z18));
endmodule


