// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_;
  inv1   g00(.a(x16), .O(new_n46_));
  nor2   g01(.a(x24), .b(x14), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  aoi21  g06(.a(x25), .b(x14), .c(x24), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  nor2   g12(.a(x25), .b(x23), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n53_), .d(new_n51_), .O(z01));
  nand2  g16(.a(new_n59_), .b(new_n53_), .O(z03));
  xnor2a g17(.a(x12), .b(x03), .O(new_n63_));
  xnor2a g18(.a(x11), .b(x02), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  and2   g22(.a(new_n67_), .b(new_n49_), .O(z04));
  inv1   g23(.a(new_n47_), .O(new_n69_));
  oai21  g24(.a(x13), .b(x08), .c(new_n69_), .O(z05));
  inv1   g25(.a(x14), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x08), .c(new_n69_), .O(z06));
  nand2  g27(.a(new_n49_), .b(x06), .O(z07));
  inv1   g28(.a(z03), .O(z08));
  inv1   g29(.a(x15), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(new_n60_), .c(new_n69_), .O(z09));
  nand3  g32(.a(new_n60_), .b(new_n75_), .c(new_n51_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x17), .c(new_n69_), .O(z10));
  and2   g34(.a(x18), .b(x17), .O(new_n80_));
  or2    g35(.a(new_n80_), .b(new_n55_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n78_), .c(new_n69_), .O(z11));
  nor2   g37(.a(new_n78_), .b(new_n47_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand3  g39(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nor2   g41(.a(new_n80_), .b(x19), .O(new_n87_));
  nor3   g42(.a(new_n87_), .b(new_n86_), .c(new_n84_), .O(z12));
  nor2   g43(.a(new_n85_), .b(x20), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  aoi21  g45(.a(new_n85_), .b(x20), .c(new_n78_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n90_), .c(new_n47_), .O(z13));
  nand2  g47(.a(new_n90_), .b(x21), .O(new_n93_));
  inv1   g48(.a(x21), .O(new_n94_));
  aoi21  g49(.a(new_n89_), .b(new_n94_), .c(new_n78_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n93_), .c(new_n47_), .O(z14));
  oai21  g51(.a(new_n90_), .b(x21), .c(x22), .O(new_n97_));
  nor2   g52(.a(x22), .b(x21), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n89_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n83_), .O(z15));
  nand2  g55(.a(new_n99_), .b(x23), .O(new_n101_));
  inv1   g56(.a(x20), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  inv1   g58(.a(x23), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n103_), .c(new_n94_), .d(new_n102_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n86_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n101_), .c(new_n83_), .O(z16));
  inv1   g63(.a(x24), .O(new_n109_));
  nand3  g64(.a(new_n106_), .b(new_n86_), .c(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(x24), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n83_), .O(z17));
  oai21  g67(.a(new_n105_), .b(new_n85_), .c(x25), .O(new_n113_));
  nand4  g68(.a(new_n98_), .b(new_n89_), .c(new_n58_), .d(new_n109_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x14), .O(new_n116_));
  aoi22  g71(.a(new_n78_), .b(new_n69_), .c(x25), .d(x24), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(z18));
endmodule


