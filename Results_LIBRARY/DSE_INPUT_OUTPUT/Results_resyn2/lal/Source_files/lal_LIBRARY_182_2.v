// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x19), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  and2   g16(.a(new_n61_), .b(new_n49_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n60_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n48_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n60_), .b(new_n49_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor3   g30(.a(new_n48_), .b(new_n75_), .c(x08), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nor2   g32(.a(x18), .b(x17), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n57_), .c(new_n47_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n55_), .O(new_n81_));
  inv1   g36(.a(x23), .O(new_n82_));
  oai21  g37(.a(new_n57_), .b(new_n52_), .c(new_n82_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x24), .c(x25), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n81_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n47_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand2  g43(.a(new_n61_), .b(new_n47_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n49_), .O(z10));
  nor2   g45(.a(new_n54_), .b(new_n53_), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n89_), .c(new_n78_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nor2   g49(.a(new_n91_), .b(x19), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n94_), .c(new_n89_), .O(z12));
  aoi21  g51(.a(new_n91_), .b(new_n52_), .c(x15), .O(new_n97_));
  oai21  g52(.a(new_n94_), .b(new_n52_), .c(new_n61_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(new_n99_));
  oai21  g54(.a(new_n97_), .b(new_n55_), .c(new_n99_), .O(z13));
  inv1   g55(.a(new_n89_), .O(new_n101_));
  oai21  g56(.a(new_n93_), .b(x20), .c(x21), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  nor2   g58(.a(new_n93_), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n102_), .c(new_n101_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n101_), .O(z15));
  nand2  g65(.a(new_n109_), .b(x23), .O(new_n111_));
  nand3  g66(.a(new_n108_), .b(new_n104_), .c(new_n82_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n101_), .O(z16));
  nand2  g68(.a(new_n112_), .b(x24), .O(new_n114_));
  nor2   g69(.a(x24), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n108_), .c(new_n104_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n101_), .O(z17));
  aoi21  g72(.a(new_n116_), .b(x25), .c(new_n89_), .O(new_n118_));
  oai21  g73(.a(new_n116_), .b(x25), .c(new_n118_), .O(z18));
endmodule


