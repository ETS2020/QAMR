// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x20), .b(x06), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nor3   g16(.a(new_n61_), .b(new_n47_), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n47_), .b(new_n64_), .O(z02));
  nand2  g20(.a(new_n59_), .b(new_n48_), .O(z03));
  nand2  g21(.a(new_n48_), .b(new_n46_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor2   g31(.a(new_n67_), .b(new_n76_), .O(z06));
  nand2  g32(.a(new_n46_), .b(x06), .O(z07));
  aoi21  g33(.a(new_n58_), .b(new_n50_), .c(new_n47_), .O(z08));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand4  g36(.a(new_n61_), .b(new_n48_), .c(new_n81_), .d(new_n80_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand2  g38(.a(new_n62_), .b(new_n81_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(x17), .O(z10));
  nand2  g40(.a(new_n53_), .b(new_n52_), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n84_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand2  g45(.a(new_n87_), .b(new_n54_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n84_), .O(z12));
  nor2   g48(.a(new_n90_), .b(x20), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n60_), .b(new_n81_), .c(new_n80_), .O(new_n96_));
  aoi21  g51(.a(new_n90_), .b(x20), .c(new_n96_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n95_), .c(new_n47_), .O(z13));
  inv1   g53(.a(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(x21), .O(new_n100_));
  inv1   g55(.a(x21), .O(new_n101_));
  nand2  g56(.a(new_n94_), .b(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n48_), .O(z14));
  oai21  g58(.a(new_n90_), .b(x21), .c(x22), .O(new_n104_));
  nor2   g59(.a(x22), .b(x21), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n94_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x06), .O(new_n108_));
  aoi22  g63(.a(new_n96_), .b(new_n48_), .c(x22), .d(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z15));
  inv1   g65(.a(x06), .O(new_n111_));
  oai21  g66(.a(new_n106_), .b(new_n111_), .c(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  inv1   g68(.a(new_n90_), .O(new_n114_));
  nand3  g69(.a(new_n105_), .b(new_n114_), .c(new_n113_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x06), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n51_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n112_), .c(new_n99_), .O(z16));
  nand2  g73(.a(new_n115_), .b(x24), .O(new_n119_));
  nor2   g74(.a(x24), .b(x23), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n105_), .c(new_n94_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x06), .O(new_n123_));
  aoi22  g78(.a(new_n96_), .b(new_n48_), .c(x24), .d(x20), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n123_), .O(z17));
  xor2a  g80(.a(new_n121_), .b(x25), .O(new_n126_));
  nand2  g81(.a(new_n50_), .b(x20), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n111_), .c(new_n96_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n126_), .O(z18));
endmodule


