// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x12), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x19), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n51_), .c(x20), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(new_n53_), .c(new_n47_), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nor2   g11(.a(x25), .b(x24), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(x07), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n49_), .O(new_n60_));
  oai21  g15(.a(new_n55_), .b(x25), .c(new_n60_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n48_), .b(new_n62_), .O(z02));
  inv1   g18(.a(x24), .O(new_n64_));
  nand2  g19(.a(new_n49_), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n55_), .c(x25), .O(z03));
  aoi21  g21(.a(new_n46_), .b(x03), .c(x23), .O(new_n67_));
  inv1   g22(.a(x00), .O(new_n68_));
  nand2  g23(.a(x09), .b(new_n68_), .O(new_n69_));
  inv1   g24(.a(x02), .O(new_n70_));
  nand2  g25(.a(x11), .b(new_n70_), .O(new_n71_));
  inv1   g26(.a(x09), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x00), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(new_n71_), .c(new_n69_), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  inv1   g30(.a(x11), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x02), .O(new_n77_));
  inv1   g32(.a(x03), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n78_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n74_), .c(new_n46_), .O(new_n81_));
  oai21  g36(.a(new_n67_), .b(x12), .c(new_n81_), .O(z04));
  nand2  g37(.a(new_n49_), .b(new_n46_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x13), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor2   g40(.a(new_n83_), .b(new_n85_), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nor2   g42(.a(new_n64_), .b(new_n47_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(x25), .c(x12), .O(new_n89_));
  inv1   g44(.a(x20), .O(new_n90_));
  nand2  g45(.a(new_n52_), .b(new_n51_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g47(.a(x24), .b(x22), .c(x21), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n92_), .c(x25), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(x23), .c(new_n89_), .O(z08));
  inv1   g51(.a(x07), .O(new_n97_));
  inv1   g52(.a(x15), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(new_n56_), .c(new_n49_), .O(z09));
  nand3  g55(.a(new_n56_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x17), .c(new_n49_), .O(z10));
  nor2   g57(.a(new_n101_), .b(new_n48_), .O(new_n103_));
  and2   g58(.a(x18), .b(x17), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(z11));
  nor2   g62(.a(new_n104_), .b(x19), .O(new_n108_));
  inv1   g63(.a(new_n101_), .O(new_n109_));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(new_n49_), .O(z12));
  inv1   g67(.a(new_n110_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n90_), .O(new_n114_));
  nand2  g69(.a(new_n110_), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n103_), .O(z13));
  nand2  g71(.a(new_n114_), .b(x21), .O(new_n117_));
  nor2   g72(.a(x21), .b(x20), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n113_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n103_), .O(z14));
  nand2  g75(.a(new_n119_), .b(x22), .O(new_n121_));
  inv1   g76(.a(x22), .O(new_n122_));
  nand3  g77(.a(new_n118_), .b(new_n113_), .c(new_n122_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n103_), .O(z15));
  nand4  g79(.a(new_n118_), .b(new_n113_), .c(new_n47_), .d(new_n122_), .O(new_n125_));
  aoi21  g80(.a(new_n123_), .b(x23), .c(new_n101_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n125_), .c(new_n48_), .O(z16));
  inv1   g82(.a(x21), .O(new_n128_));
  nand4  g83(.a(new_n64_), .b(new_n122_), .c(new_n128_), .d(new_n90_), .O(new_n129_));
  inv1   g84(.a(new_n129_), .O(new_n130_));
  aoi22  g85(.a(new_n130_), .b(new_n113_), .c(new_n123_), .d(x24), .O(new_n131_));
  oai21  g86(.a(new_n101_), .b(new_n88_), .c(new_n49_), .O(new_n132_));
  oai21  g87(.a(new_n131_), .b(x23), .c(new_n132_), .O(z17));
  oai21  g88(.a(new_n129_), .b(new_n110_), .c(x25), .O(new_n134_));
  nand4  g89(.a(new_n118_), .b(new_n113_), .c(new_n57_), .d(new_n122_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand2  g91(.a(new_n136_), .b(new_n47_), .O(new_n137_));
  and2   g92(.a(x25), .b(x23), .O(new_n138_));
  oai21  g93(.a(new_n138_), .b(new_n101_), .c(new_n49_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(new_n137_), .O(z18));
endmodule


