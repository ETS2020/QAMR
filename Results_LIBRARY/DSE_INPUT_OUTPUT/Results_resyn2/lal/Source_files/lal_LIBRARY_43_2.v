// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  inv1   g15(.a(x24), .O(new_n61_));
  nand2  g16(.a(x25), .b(new_n47_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n60_), .b(new_n58_), .c(new_n66_), .O(z01));
  nand2  g22(.a(new_n62_), .b(new_n61_), .O(new_n68_));
  oai21  g23(.a(new_n60_), .b(new_n58_), .c(new_n68_), .O(z03));
  inv1   g24(.a(new_n48_), .O(new_n70_));
  inv1   g25(.a(x08), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n71_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n70_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n70_), .O(z05));
  and2   g34(.a(new_n50_), .b(x14), .O(z06));
  nand2  g35(.a(new_n50_), .b(x06), .O(z07));
  nand2  g36(.a(new_n58_), .b(x24), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n59_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n70_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand4  g41(.a(new_n84_), .b(new_n64_), .c(new_n70_), .d(new_n53_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand2  g43(.a(new_n84_), .b(new_n64_), .O(new_n89_));
  nand2  g44(.a(new_n54_), .b(new_n53_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n70_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(new_n55_), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n94_), .c(new_n84_), .d(new_n64_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n70_), .O(z12));
  inv1   g52(.a(new_n94_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n52_), .O(new_n99_));
  aoi21  g54(.a(new_n94_), .b(x20), .c(new_n89_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n48_), .O(z13));
  inv1   g56(.a(new_n84_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  nand4  g61(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n64_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n98_), .c(new_n89_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n48_), .O(z15));
  nand2  g66(.a(new_n109_), .b(new_n98_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(x24), .c(x23), .O(new_n113_));
  nand2  g68(.a(new_n89_), .b(new_n70_), .O(new_n114_));
  nand3  g69(.a(new_n109_), .b(new_n98_), .c(new_n47_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  nand4  g72(.a(new_n109_), .b(new_n98_), .c(new_n61_), .d(new_n47_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n114_), .O(z17));
  nand3  g74(.a(new_n109_), .b(new_n98_), .c(new_n61_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x25), .O(new_n121_));
  inv1   g76(.a(x25), .O(new_n122_));
  nand4  g77(.a(new_n109_), .b(new_n98_), .c(new_n122_), .d(new_n61_), .O(new_n123_));
  aoi21  g78(.a(new_n122_), .b(x24), .c(new_n47_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n89_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n121_), .O(z18));
endmodule


