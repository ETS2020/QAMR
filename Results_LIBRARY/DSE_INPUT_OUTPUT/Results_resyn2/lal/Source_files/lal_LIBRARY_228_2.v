// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(x23), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n47_), .c(new_n52_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x07), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(new_n51_), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  aoi21  g23(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n69_));
  nand2  g24(.a(new_n50_), .b(new_n47_), .O(new_n70_));
  aoi21  g25(.a(new_n69_), .b(x24), .c(new_n70_), .O(z03));
  inv1   g26(.a(new_n48_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  xor2a  g28(.a(x12), .b(x03), .O(new_n74_));
  xor2a  g29(.a(x11), .b(x02), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  xor2a  g31(.a(x09), .b(x00), .O(new_n77_));
  xor2a  g32(.a(x10), .b(x01), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(z04));
  nor2   g35(.a(new_n73_), .b(x13), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  nor2   g37(.a(new_n73_), .b(new_n82_), .O(z06));
  nand3  g38(.a(new_n72_), .b(new_n46_), .c(x06), .O(z07));
  oai21  g39(.a(new_n69_), .b(x23), .c(x24), .O(new_n85_));
  oai21  g40(.a(new_n50_), .b(x23), .c(new_n85_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n64_), .c(new_n72_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand2  g44(.a(new_n87_), .b(new_n63_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n72_), .O(z10));
  xnor2a g46(.a(x18), .b(x17), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n90_), .c(new_n72_), .O(z11));
  nand3  g48(.a(new_n87_), .b(new_n63_), .c(new_n72_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(x18), .b(x17), .c(x19), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n94_), .O(z12));
  inv1   g53(.a(new_n94_), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n53_), .O(new_n100_));
  nand2  g55(.a(new_n95_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n96_), .c(new_n90_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n103_), .c(new_n48_), .O(z14));
  nand2  g61(.a(new_n104_), .b(new_n96_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x22), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n96_), .c(new_n90_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n48_), .O(z15));
  nand2  g66(.a(new_n109_), .b(new_n96_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x23), .O(new_n113_));
  nand3  g68(.a(new_n109_), .b(new_n96_), .c(new_n47_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n99_), .O(z16));
  nand2  g70(.a(new_n90_), .b(new_n72_), .O(new_n116_));
  inv1   g71(.a(new_n114_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n52_), .O(new_n118_));
  nand2  g73(.a(new_n114_), .b(x24), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n116_), .O(z17));
  nand2  g75(.a(new_n50_), .b(new_n52_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n109_), .c(new_n96_), .O(new_n122_));
  nand2  g77(.a(new_n112_), .b(new_n50_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n47_), .O(new_n124_));
  nand2  g79(.a(x25), .b(x24), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n116_), .O(z18));
endmodule


