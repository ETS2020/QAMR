// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x20), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x24), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  oai21  g08(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nor2   g10(.a(x18), .b(x17), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n55_), .c(x20), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  oai21  g14(.a(new_n58_), .b(new_n57_), .c(new_n59_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n60_), .c(new_n54_), .d(new_n50_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n54_), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n48_), .O(z04));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(x13), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor2   g30(.a(new_n73_), .b(new_n75_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g32(.a(new_n58_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(x24), .c(x25), .O(new_n79_));
  nor3   g34(.a(x19), .b(x18), .c(x17), .O(new_n80_));
  inv1   g35(.a(new_n59_), .O(new_n81_));
  aoi21  g36(.a(new_n78_), .b(new_n80_), .c(new_n81_), .O(new_n82_));
  oai22  g37(.a(new_n82_), .b(new_n52_), .c(new_n79_), .d(new_n51_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(x05), .c(x04), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n48_), .O(z09));
  inv1   g41(.a(x17), .O(new_n87_));
  nand2  g42(.a(new_n84_), .b(new_n61_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n47_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  inv1   g46(.a(new_n56_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n89_), .c(new_n92_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(new_n55_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n88_), .c(new_n47_), .O(z12));
  nor3   g54(.a(new_n96_), .b(new_n52_), .c(x20), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(x20), .O(new_n102_));
  aoi21  g57(.a(new_n84_), .b(new_n61_), .c(new_n47_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(z13));
  inv1   g60(.a(new_n96_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n52_), .c(new_n51_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand2  g64(.a(new_n100_), .b(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n104_), .O(z14));
  oai21  g66(.a(new_n96_), .b(x21), .c(x24), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n51_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x22), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n100_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n104_), .O(z15));
  aoi21  g72(.a(new_n115_), .b(new_n106_), .c(new_n52_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(x20), .c(x23), .O(new_n119_));
  nor2   g74(.a(new_n96_), .b(x20), .O(new_n120_));
  inv1   g75(.a(x23), .O(new_n121_));
  nand3  g76(.a(new_n115_), .b(x24), .c(new_n121_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n120_), .c(new_n103_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n119_), .O(z16));
  inv1   g80(.a(new_n88_), .O(new_n126_));
  oai21  g81(.a(new_n122_), .b(new_n96_), .c(new_n51_), .O(new_n127_));
  nand2  g82(.a(x24), .b(x20), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(z17));
  nand2  g84(.a(new_n89_), .b(new_n53_), .O(z18));
endmodule


