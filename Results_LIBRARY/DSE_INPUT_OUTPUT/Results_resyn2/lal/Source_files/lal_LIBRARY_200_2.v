// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  aoi21  g01(.a(x24), .b(x16), .c(new_n46_), .O(z00));
  inv1   g02(.a(x24), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  oai21  g04(.a(new_n49_), .b(x08), .c(new_n48_), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n51_), .c(x20), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  nor2   g09(.a(x25), .b(x23), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x07), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n56_), .c(new_n50_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(x24), .b(new_n46_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n61_), .O(z02));
  nand2  g18(.a(new_n56_), .b(new_n50_), .O(z03));
  inv1   g19(.a(new_n62_), .O(new_n65_));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n65_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n65_), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n65_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n62_), .O(z07));
  inv1   g31(.a(z03), .O(z08));
  nor2   g32(.a(x15), .b(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n65_), .c(new_n58_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  inv1   g35(.a(x17), .O(new_n81_));
  nand2  g36(.a(new_n78_), .b(new_n57_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n62_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  xnor2a g40(.a(x18), .b(x17), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n82_), .c(new_n65_), .O(z11));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n51_), .c(new_n82_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n65_), .O(z12));
  nor2   g47(.a(new_n88_), .b(x20), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  aoi21  g49(.a(new_n88_), .b(x20), .c(new_n82_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n94_), .c(new_n62_), .O(z13));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(new_n98_), .c(new_n82_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n97_), .c(new_n62_), .O(z14));
  oai21  g55(.a(new_n94_), .b(x21), .c(x22), .O(new_n101_));
  nor2   g56(.a(x22), .b(x21), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n83_), .O(z15));
  nand2  g59(.a(new_n103_), .b(x23), .O(new_n105_));
  inv1   g60(.a(new_n88_), .O(new_n106_));
  inv1   g61(.a(x20), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  inv1   g63(.a(x23), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n108_), .c(new_n98_), .d(new_n107_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n106_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n105_), .c(new_n83_), .O(z16));
  nand4  g68(.a(new_n111_), .b(new_n106_), .c(new_n48_), .d(new_n46_), .O(new_n114_));
  nand2  g69(.a(new_n82_), .b(new_n65_), .O(new_n115_));
  nand2  g70(.a(new_n112_), .b(x24), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(z17));
  oai21  g72(.a(new_n110_), .b(new_n88_), .c(x25), .O(new_n118_));
  nand4  g73(.a(new_n102_), .b(new_n93_), .c(new_n55_), .d(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n46_), .O(new_n121_));
  aoi22  g76(.a(new_n82_), .b(new_n65_), .c(x25), .d(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(z18));
endmodule


