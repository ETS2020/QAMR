// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n81_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n49_), .b(new_n62_), .O(z02));
  aoi21  g18(.a(new_n58_), .b(new_n50_), .c(new_n49_), .O(z08));
  inv1   g19(.a(z08), .O(z03));
  inv1   g20(.a(x10), .O(new_n66_));
  inv1   g21(.a(x00), .O(new_n67_));
  aoi22  g22(.a(new_n66_), .b(x01), .c(x09), .d(new_n67_), .O(new_n68_));
  oai21  g23(.a(new_n66_), .b(x01), .c(new_n68_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(new_n70_));
  inv1   g25(.a(x12), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x03), .c(new_n48_), .O(new_n72_));
  inv1   g27(.a(x11), .O(new_n73_));
  oai22  g28(.a(new_n73_), .b(x02), .c(x09), .d(new_n67_), .O(new_n74_));
  nand2  g29(.a(new_n71_), .b(x03), .O(new_n75_));
  nand2  g30(.a(new_n73_), .b(x02), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n70_), .c(x08), .O(z04));
  nor3   g34(.a(x15), .b(x13), .c(x08), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  aoi21  g36(.a(new_n48_), .b(new_n81_), .c(x08), .O(z06));
  nand3  g37(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(x07), .O(new_n84_));
  nand4  g39(.a(new_n48_), .b(new_n84_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n60_), .b(new_n48_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n52_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nor2   g45(.a(x18), .b(x17), .O(new_n91_));
  nor2   g46(.a(new_n53_), .b(new_n52_), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n87_), .c(new_n91_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n92_), .b(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n87_), .O(z12));
  nand2  g52(.a(x15), .b(x08), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(new_n51_), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n60_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n98_), .O(z13));
  nand2  g58(.a(new_n99_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n95_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n88_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nor3   g64(.a(new_n94_), .b(x21), .c(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n108_), .c(new_n60_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n98_), .O(z15));
  nand2  g69(.a(new_n111_), .b(x23), .O(new_n115_));
  nor2   g70(.a(x23), .b(x22), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n105_), .c(new_n95_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n88_), .O(z16));
  nand2  g73(.a(new_n117_), .b(x24), .O(new_n119_));
  inv1   g74(.a(x24), .O(new_n120_));
  nand3  g75(.a(new_n116_), .b(new_n110_), .c(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n60_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n98_), .O(z17));
  aoi21  g79(.a(new_n121_), .b(x25), .c(new_n87_), .O(new_n125_));
  oai21  g80(.a(new_n121_), .b(x25), .c(new_n125_), .O(z18));
endmodule


