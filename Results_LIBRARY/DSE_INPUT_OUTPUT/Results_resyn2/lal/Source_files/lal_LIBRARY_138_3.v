// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x20), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n48_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n61_), .O(z01));
  nor2   g23(.a(new_n49_), .b(new_n46_), .O(z02));
  nand2  g24(.a(new_n61_), .b(new_n50_), .O(z03));
  nor2   g25(.a(new_n49_), .b(x08), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  and2   g31(.a(new_n76_), .b(new_n71_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(new_n71_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand2  g35(.a(new_n71_), .b(x06), .O(z07));
  aoi21  g36(.a(new_n60_), .b(new_n53_), .c(new_n49_), .O(z08));
  nand2  g37(.a(new_n47_), .b(new_n62_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n63_), .c(new_n50_), .O(z09));
  nand2  g39(.a(new_n65_), .b(new_n47_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n54_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand2  g43(.a(new_n55_), .b(new_n54_), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n85_), .c(new_n50_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n90_), .b(new_n56_), .O(new_n94_));
  nand4  g49(.a(new_n94_), .b(new_n93_), .c(new_n65_), .d(new_n47_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z12));
  inv1   g51(.a(new_n93_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(x15), .c(new_n48_), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(x20), .c(new_n64_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n98_), .c(new_n49_), .O(z13));
  oai21  g55(.a(new_n93_), .b(x20), .c(x21), .O(new_n101_));
  inv1   g56(.a(x21), .O(new_n102_));
  nand3  g57(.a(new_n97_), .b(new_n102_), .c(new_n48_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n86_), .O(z14));
  nand2  g59(.a(new_n103_), .b(x22), .O(new_n105_));
  inv1   g60(.a(x22), .O(new_n106_));
  nand4  g61(.a(new_n97_), .b(new_n106_), .c(new_n102_), .d(new_n48_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n86_), .O(z15));
  nand2  g63(.a(new_n107_), .b(x23), .O(new_n109_));
  nor2   g64(.a(x23), .b(x22), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n97_), .c(new_n102_), .d(new_n48_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n86_), .O(z16));
  nor2   g67(.a(x24), .b(x21), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n93_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(x15), .c(new_n48_), .O(new_n116_));
  aoi21  g71(.a(new_n111_), .b(x24), .c(new_n64_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(x15), .c(new_n116_), .O(z17));
  nand3  g73(.a(new_n115_), .b(new_n53_), .c(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n115_), .b(new_n48_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x25), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n86_), .O(z18));
endmodule


