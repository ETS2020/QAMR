// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n78_, new_n80_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x20), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(z02));
  nand3  g19(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  inv1   g21(.a(x08), .O(new_n67_));
  nand2  g22(.a(new_n47_), .b(new_n67_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  or2    g30(.a(new_n68_), .b(x13), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  nand3  g32(.a(new_n47_), .b(x14), .c(new_n67_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z06));
  inv1   g34(.a(new_n47_), .O(new_n80_));
  aoi21  g35(.a(new_n67_), .b(x06), .c(new_n80_), .O(z07));
  aoi21  g36(.a(new_n58_), .b(new_n50_), .c(new_n80_), .O(z08));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n83_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n60_), .b(new_n84_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n47_), .O(z10));
  xnor2a g43(.a(x18), .b(x17), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n87_), .c(new_n47_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  aoi21  g47(.a(x18), .b(x17), .c(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n87_), .O(z12));
  inv1   g49(.a(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n51_), .O(new_n96_));
  nand2  g51(.a(new_n91_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(z13));
  inv1   g53(.a(x21), .O(new_n99_));
  nand2  g54(.a(new_n92_), .b(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n51_), .O(new_n102_));
  inv1   g57(.a(new_n60_), .O(new_n103_));
  aoi21  g58(.a(new_n96_), .b(x21), .c(new_n103_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(x15), .c(new_n102_), .O(z14));
  inv1   g60(.a(x22), .O(new_n106_));
  nor3   g61(.a(new_n91_), .b(x21), .c(x20), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n87_), .O(new_n108_));
  oai21  g63(.a(new_n107_), .b(new_n106_), .c(new_n108_), .O(z15));
  nand2  g64(.a(new_n107_), .b(new_n106_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x23), .O(new_n111_));
  nor2   g66(.a(x23), .b(x22), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n92_), .c(new_n99_), .d(new_n51_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n95_), .O(z16));
  nor2   g69(.a(x24), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n112_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n91_), .c(new_n84_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n51_), .O(new_n118_));
  aoi21  g73(.a(new_n113_), .b(x24), .c(new_n103_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x15), .c(new_n118_), .O(z17));
  nor2   g75(.a(x25), .b(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n112_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n100_), .c(new_n84_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n51_), .O(new_n124_));
  nand4  g79(.a(new_n115_), .b(new_n112_), .c(new_n92_), .d(new_n51_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(x25), .c(new_n103_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(x15), .c(new_n124_), .O(z18));
endmodule


