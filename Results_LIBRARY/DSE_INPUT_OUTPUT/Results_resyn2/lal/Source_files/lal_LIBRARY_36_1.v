// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n65_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nor2   g06(.a(x25), .b(x24), .O(z03));
  nor2   g07(.a(z03), .b(x07), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z01));
  and2   g09(.a(new_n48_), .b(x16), .O(z02));
  xnor2a g10(.a(x09), .b(x00), .O(new_n56_));
  xnor2a g11(.a(x11), .b(x02), .O(new_n57_));
  xnor2a g12(.a(x12), .b(x03), .O(new_n58_));
  xnor2a g13(.a(x10), .b(x01), .O(new_n59_));
  nand4  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n56_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n46_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n48_), .O(z04));
  nand2  g17(.a(new_n48_), .b(new_n46_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x13), .O(z05));
  inv1   g19(.a(x14), .O(new_n65_));
  nor2   g20(.a(new_n63_), .b(new_n65_), .O(z06));
  nand3  g21(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nor2   g22(.a(z03), .b(new_n49_), .O(z08));
  nor2   g23(.a(x15), .b(x07), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n48_), .c(x05), .d(x04), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(z09));
  inv1   g26(.a(x17), .O(new_n72_));
  nand3  g27(.a(new_n69_), .b(new_n51_), .c(new_n48_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z10));
  nand2  g31(.a(x18), .b(x17), .O(new_n77_));
  inv1   g32(.a(x18), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n74_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z11));
  nand3  g36(.a(x19), .b(x18), .c(x17), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  aoi21  g38(.a(x18), .b(x17), .c(x19), .O(new_n84_));
  nor3   g39(.a(new_n84_), .b(new_n83_), .c(new_n73_), .O(z12));
  nand2  g40(.a(new_n82_), .b(x20), .O(new_n86_));
  nand2  g41(.a(new_n69_), .b(new_n51_), .O(new_n87_));
  inv1   g42(.a(x20), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(x19), .c(x18), .d(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n86_), .c(new_n49_), .O(z13));
  inv1   g47(.a(x21), .O(new_n93_));
  aoi21  g48(.a(new_n90_), .b(new_n93_), .c(new_n73_), .O(new_n94_));
  oai21  g49(.a(new_n90_), .b(new_n93_), .c(new_n94_), .O(z14));
  oai21  g50(.a(new_n89_), .b(x21), .c(x22), .O(new_n96_));
  nor2   g51(.a(x22), .b(x21), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n90_), .c(new_n87_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n96_), .c(new_n49_), .O(z15));
  inv1   g54(.a(new_n87_), .O(new_n100_));
  nand3  g55(.a(new_n97_), .b(new_n90_), .c(new_n47_), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(new_n90_), .O(new_n102_));
  inv1   g57(.a(x24), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n101_), .c(new_n100_), .O(z16));
  nand2  g61(.a(x24), .b(x23), .O(new_n107_));
  nor2   g62(.a(x23), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n97_), .c(new_n83_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n100_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n107_), .O(z17));
  inv1   g67(.a(x25), .O(new_n113_));
  nand2  g68(.a(new_n97_), .b(new_n113_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n89_), .c(new_n103_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n47_), .O(new_n116_));
  nand3  g71(.a(new_n97_), .b(new_n103_), .c(new_n47_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n89_), .c(x25), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n100_), .O(z18));
endmodule


