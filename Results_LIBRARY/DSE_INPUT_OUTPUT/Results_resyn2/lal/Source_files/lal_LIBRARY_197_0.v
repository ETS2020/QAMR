// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n116_, new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x22), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g18(.a(x22), .b(x21), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n63_), .c(x24), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n57_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n52_), .O(z01));
  nor2   g23(.a(x18), .b(x17), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n61_), .c(x20), .O(new_n70_));
  oai21  g25(.a(new_n64_), .b(new_n70_), .c(new_n53_), .O(new_n71_));
  nor2   g26(.a(new_n48_), .b(x25), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  aoi21  g28(.a(new_n71_), .b(x24), .c(new_n73_), .O(z03));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n49_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n48_), .O(z06));
  nand3  g39(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g40(.a(new_n62_), .b(new_n58_), .c(new_n64_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x23), .c(x24), .O(new_n87_));
  nand2  g42(.a(new_n72_), .b(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(x05), .c(x04), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x22), .c(x15), .O(z09));
  inv1   g46(.a(x15), .O(new_n92_));
  nand3  g47(.a(new_n52_), .b(x22), .c(new_n92_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(x17), .O(z10));
  inv1   g49(.a(new_n69_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n52_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(x22), .c(x15), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n61_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n93_), .O(z12));
  nand2  g57(.a(new_n99_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n99_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n58_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n52_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x22), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n92_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  nand3  g64(.a(new_n104_), .b(new_n109_), .c(new_n58_), .O(new_n110_));
  oai21  g65(.a(new_n99_), .b(x20), .c(x21), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n52_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x22), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n92_), .O(z14));
  or2    g69(.a(new_n110_), .b(new_n93_), .O(z15));
  inv1   g70(.a(new_n93_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n53_), .O(z16));
  nand2  g72(.a(new_n52_), .b(new_n54_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x22), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n92_), .O(z17));
  nand2  g75(.a(new_n116_), .b(new_n55_), .O(z18));
endmodule


