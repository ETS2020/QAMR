// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g10(.a(new_n55_), .b(x24), .c(x22), .d(x21), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n47_), .c(new_n58_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n57_), .b(new_n47_), .O(z03));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  or2    g27(.a(new_n65_), .b(x13), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g31(.a(new_n47_), .O(new_n77_));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n77_), .O(z07));
  nand2  g33(.a(x22), .b(x21), .O(new_n79_));
  aoi21  g34(.a(new_n54_), .b(new_n50_), .c(new_n79_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x23), .c(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z08));
  inv1   g37(.a(x15), .O(new_n83_));
  nand3  g38(.a(new_n47_), .b(new_n83_), .c(new_n58_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n59_), .O(z09));
  inv1   g40(.a(new_n84_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n59_), .c(new_n51_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand3  g43(.a(new_n59_), .b(new_n83_), .c(new_n58_), .O(new_n89_));
  xnor2a g44(.a(x18), .b(x17), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n89_), .c(new_n47_), .O(z11));
  nand2  g46(.a(new_n86_), .b(new_n59_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  aoi21  g49(.a(x18), .b(x17), .c(x19), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z12));
  nand2  g51(.a(new_n94_), .b(new_n50_), .O(new_n97_));
  aoi21  g52(.a(new_n93_), .b(x20), .c(new_n89_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n97_), .c(new_n77_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n94_), .c(new_n89_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n100_), .c(new_n77_), .O(z14));
  nand2  g58(.a(new_n101_), .b(new_n94_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x22), .O(new_n105_));
  inv1   g60(.a(x22), .O(new_n106_));
  nand3  g61(.a(new_n101_), .b(new_n94_), .c(new_n106_), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n105_), .c(new_n86_), .d(new_n59_), .O(z15));
  inv1   g63(.a(x24), .O(new_n109_));
  nand3  g64(.a(new_n107_), .b(new_n109_), .c(x23), .O(new_n110_));
  inv1   g65(.a(x23), .O(new_n111_));
  nand4  g66(.a(new_n101_), .b(new_n94_), .c(new_n111_), .d(new_n106_), .O(new_n112_));
  nand2  g67(.a(new_n89_), .b(new_n47_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(z16));
  nand2  g69(.a(new_n107_), .b(new_n109_), .O(new_n115_));
  nand4  g70(.a(new_n101_), .b(new_n94_), .c(x24), .d(new_n106_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n111_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n113_), .O(z17));
  nand2  g73(.a(new_n112_), .b(new_n49_), .O(new_n119_));
  nor2   g74(.a(new_n49_), .b(x23), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  oai21  g77(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nand4  g78(.a(new_n101_), .b(new_n94_), .c(new_n109_), .d(new_n106_), .O(new_n124_));
  aoi22  g79(.a(new_n124_), .b(new_n120_), .c(new_n123_), .d(new_n89_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n122_), .O(z18));
endmodule


