// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x12), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n63_));
  inv1   g18(.a(x12), .O(new_n64_));
  aoi21  g19(.a(x25), .b(new_n64_), .c(x24), .O(new_n65_));
  nand2  g20(.a(x05), .b(x04), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nor3   g22(.a(new_n67_), .b(new_n65_), .c(x07), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n63_), .O(z01));
  nor2   g24(.a(x18), .b(x17), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n57_), .c(x20), .O(new_n71_));
  nor2   g26(.a(new_n48_), .b(x23), .O(new_n72_));
  oai21  g27(.a(new_n60_), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nand2  g28(.a(new_n48_), .b(new_n64_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n73_), .c(x25), .O(z03));
  nand2  g30(.a(new_n49_), .b(new_n46_), .O(new_n76_));
  xor2a  g31(.a(x11), .b(x02), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  xor2a  g34(.a(x10), .b(x01), .O(new_n80_));
  xor2a  g35(.a(x12), .b(x03), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor2   g40(.a(new_n76_), .b(new_n85_), .O(z06));
  nand3  g41(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g42(.a(new_n61_), .b(new_n59_), .c(x23), .O(new_n88_));
  inv1   g43(.a(new_n49_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x25), .O(new_n90_));
  oai21  g45(.a(new_n88_), .b(new_n48_), .c(new_n90_), .O(z08));
  nor2   g46(.a(x15), .b(x07), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n67_), .c(new_n49_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  nand2  g49(.a(new_n92_), .b(new_n66_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(x17), .c(new_n49_), .O(z10));
  nand3  g51(.a(new_n92_), .b(new_n66_), .c(new_n49_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nor2   g53(.a(new_n56_), .b(new_n55_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n70_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z11));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n99_), .b(x19), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n104_), .c(new_n97_), .O(z12));
  nor2   g61(.a(new_n103_), .b(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(x20), .c(new_n95_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n108_), .c(new_n89_), .O(z13));
  nand2  g65(.a(new_n108_), .b(x21), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n104_), .c(new_n95_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n111_), .c(new_n89_), .O(z14));
  nand2  g69(.a(new_n112_), .b(new_n104_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x22), .O(new_n116_));
  inv1   g71(.a(x22), .O(new_n117_));
  nand3  g72(.a(new_n112_), .b(new_n104_), .c(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n98_), .O(z15));
  nor3   g74(.a(x22), .b(x21), .c(x20), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n104_), .c(new_n52_), .O(new_n121_));
  aoi21  g76(.a(new_n118_), .b(x23), .c(new_n95_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n121_), .c(new_n89_), .O(z16));
  nand2  g78(.a(new_n95_), .b(new_n49_), .O(new_n124_));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  nor2   g80(.a(x24), .b(x23), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n120_), .c(new_n104_), .d(new_n64_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(z17));
  nor2   g83(.a(x22), .b(x21), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n126_), .c(new_n107_), .d(new_n53_), .O(new_n130_));
  nand3  g85(.a(new_n129_), .b(new_n126_), .c(new_n107_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(x25), .O(new_n132_));
  aoi21  g87(.a(new_n53_), .b(x24), .c(new_n64_), .O(new_n133_));
  nor2   g88(.a(new_n133_), .b(new_n95_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n132_), .c(new_n130_), .O(z18));
endmodule


