// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n93_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n105_, new_n107_, new_n110_, new_n112_, new_n115_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x17), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  and2   g05(.a(x22), .b(x21), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x23), .c(x24), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  inv1   g08(.a(x07), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor3   g11(.a(x25), .b(x23), .c(x20), .O(new_n57_));
  nor2   g12(.a(x19), .b(x18), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n53_), .c(new_n48_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n48_), .b(new_n62_), .O(z02));
  inv1   g18(.a(x15), .O(new_n64_));
  oai21  g19(.a(new_n57_), .b(new_n64_), .c(x17), .O(new_n65_));
  inv1   g20(.a(x23), .O(new_n66_));
  inv1   g21(.a(x24), .O(new_n67_));
  oai21  g22(.a(new_n58_), .b(x20), .c(new_n51_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  oai21  g24(.a(new_n69_), .b(x25), .c(new_n65_), .O(z03));
  inv1   g25(.a(new_n48_), .O(new_n71_));
  inv1   g26(.a(x00), .O(new_n72_));
  nand2  g27(.a(x09), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x02), .O(new_n74_));
  nand2  g29(.a(x11), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x03), .O(new_n76_));
  nand2  g31(.a(x12), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(x11), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x02), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n73_), .O(new_n80_));
  inv1   g35(.a(x01), .O(new_n81_));
  nand2  g36(.a(x10), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x09), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x00), .O(new_n84_));
  inv1   g39(.a(x10), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x01), .O(new_n86_));
  inv1   g41(.a(x12), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x03), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n80_), .c(new_n46_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n71_), .O(z04));
  nor3   g46(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g47(.a(x14), .b(new_n46_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n71_), .O(z06));
  nand3  g49(.a(new_n71_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g50(.a(x20), .O(new_n96_));
  nand2  g51(.a(x22), .b(x21), .O(new_n97_));
  aoi21  g52(.a(new_n59_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(x23), .c(x24), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n71_), .c(new_n50_), .O(z08));
  nor3   g55(.a(x17), .b(x15), .c(x07), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n55_), .O(z09));
  aoi21  g58(.a(new_n56_), .b(new_n47_), .c(x15), .O(z10));
  nand2  g59(.a(new_n55_), .b(x18), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n102_), .O(z11));
  nand2  g61(.a(new_n55_), .b(x19), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n102_), .O(z12));
  nand3  g63(.a(new_n101_), .b(new_n55_), .c(new_n96_), .O(z13));
  inv1   g64(.a(x21), .O(new_n110_));
  nand3  g65(.a(new_n101_), .b(new_n55_), .c(new_n110_), .O(z14));
  inv1   g66(.a(x22), .O(new_n112_));
  nand3  g67(.a(new_n101_), .b(new_n55_), .c(new_n112_), .O(z15));
  nand3  g68(.a(new_n101_), .b(new_n55_), .c(new_n66_), .O(z16));
  oai21  g69(.a(new_n56_), .b(x24), .c(new_n47_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n64_), .O(z17));
  nand3  g71(.a(new_n101_), .b(new_n55_), .c(new_n50_), .O(z18));
endmodule


