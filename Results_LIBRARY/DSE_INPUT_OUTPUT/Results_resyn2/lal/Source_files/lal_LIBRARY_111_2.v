// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x00), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  and2   g15(.a(x22), .b(x21), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g17(.a(x24), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x23), .O(new_n64_));
  nor2   g19(.a(x24), .b(x00), .O(new_n65_));
  aoi21  g20(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n66_), .b(x25), .O(z03));
  oai21  g25(.a(x09), .b(x08), .c(x24), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x00), .O(new_n72_));
  nand2  g27(.a(x09), .b(new_n47_), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  inv1   g29(.a(x10), .O(new_n75_));
  aoi22  g30(.a(x12), .b(new_n74_), .c(new_n75_), .d(x01), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  inv1   g32(.a(x01), .O(new_n78_));
  nand2  g33(.a(x10), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x12), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x03), .O(new_n81_));
  xnor2a g36(.a(x11), .b(x02), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n77_), .c(new_n46_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n72_), .O(z04));
  nor3   g40(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g41(.a(x14), .b(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n49_), .O(z06));
  nand3  g43(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g44(.a(x25), .O(new_n90_));
  inv1   g45(.a(x23), .O(new_n91_));
  nand2  g46(.a(new_n62_), .b(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(x24), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n49_), .c(new_n90_), .O(z08));
  inv1   g49(.a(x15), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n51_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n52_), .c(new_n49_), .O(z09));
  nand3  g52(.a(new_n52_), .b(new_n95_), .c(new_n51_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n56_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z10));
  nand2  g56(.a(new_n57_), .b(new_n56_), .O(new_n102_));
  nand2  g57(.a(x18), .b(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(new_n98_), .c(new_n49_), .O(z11));
  nand3  g60(.a(x19), .b(x18), .c(x17), .O(new_n106_));
  aoi21  g61(.a(new_n103_), .b(new_n58_), .c(new_n98_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n49_), .O(z12));
  nor2   g64(.a(new_n106_), .b(x20), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n106_), .b(x20), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n99_), .O(z13));
  nand2  g68(.a(new_n111_), .b(x21), .O(new_n114_));
  inv1   g69(.a(x21), .O(new_n115_));
  aoi21  g70(.a(new_n110_), .b(new_n115_), .c(new_n98_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n114_), .c(new_n48_), .O(z14));
  oai21  g72(.a(new_n111_), .b(x21), .c(x22), .O(new_n118_));
  nor2   g73(.a(x22), .b(x21), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n110_), .c(new_n98_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(z15));
  nand2  g76(.a(new_n119_), .b(new_n110_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n119_), .b(new_n110_), .c(new_n91_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n99_), .O(z16));
  nand2  g80(.a(new_n124_), .b(x24), .O(new_n126_));
  nand4  g81(.a(new_n119_), .b(new_n110_), .c(new_n63_), .d(new_n91_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n99_), .O(z17));
  xor2a  g83(.a(new_n127_), .b(x25), .O(new_n129_));
  nand2  g84(.a(new_n90_), .b(x24), .O(new_n130_));
  aoi21  g85(.a(new_n130_), .b(x00), .c(new_n98_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n129_), .O(z18));
endmodule


