// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n91_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n55_), .c(x24), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(x20), .c(x25), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  inv1   g14(.a(x23), .O(new_n60_));
  inv1   g15(.a(x24), .O(new_n61_));
  nor3   g16(.a(x19), .b(x18), .c(x17), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n60_), .c(new_n61_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n59_), .c(new_n54_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nor2   g22(.a(new_n64_), .b(new_n59_), .O(z03));
  inv1   g23(.a(x12), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(x03), .O(new_n70_));
  inv1   g25(.a(x02), .O(new_n71_));
  nand2  g26(.a(x11), .b(new_n71_), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  nand2  g28(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x11), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x02), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n74_), .c(new_n72_), .d(new_n70_), .O(new_n77_));
  inv1   g32(.a(x00), .O(new_n78_));
  nand2  g33(.a(x09), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x01), .O(new_n80_));
  nand2  g35(.a(x10), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(x09), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x00), .O(new_n83_));
  inv1   g38(.a(x10), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x01), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n83_), .c(new_n81_), .d(new_n79_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n77_), .c(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n50_), .O(z04));
  nand2  g43(.a(new_n50_), .b(new_n46_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x13), .O(z05));
  inv1   g45(.a(x14), .O(new_n91_));
  nor2   g46(.a(new_n89_), .b(new_n91_), .O(z06));
  aoi21  g47(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nor2   g48(.a(new_n56_), .b(new_n55_), .O(new_n94_));
  aoi21  g49(.a(new_n62_), .b(new_n94_), .c(x23), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n61_), .c(new_n58_), .O(z08));
  nor2   g51(.a(x15), .b(x07), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n52_), .c(new_n50_), .O(z09));
  inv1   g54(.a(x17), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(new_n52_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z10));
  inv1   g59(.a(x18), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n100_), .O(new_n106_));
  nand2  g61(.a(x18), .b(x17), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n102_), .c(new_n106_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(z11));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  aoi21  g66(.a(x18), .b(x17), .c(x19), .O(new_n112_));
  nor4   g67(.a(new_n112_), .b(new_n111_), .c(new_n101_), .d(new_n48_), .O(z12));
  nand3  g68(.a(new_n110_), .b(x24), .c(x20), .O(new_n114_));
  nand2  g69(.a(new_n101_), .b(new_n50_), .O(new_n115_));
  nand2  g70(.a(new_n111_), .b(new_n47_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(z13));
  nand3  g72(.a(new_n111_), .b(new_n55_), .c(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n116_), .b(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n102_), .O(z14));
  inv1   g75(.a(new_n101_), .O(new_n121_));
  nor2   g76(.a(new_n110_), .b(x21), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n56_), .c(new_n47_), .O(new_n123_));
  oai21  g78(.a(new_n48_), .b(x22), .c(new_n118_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(z15));
  aoi21  g80(.a(new_n122_), .b(new_n56_), .c(new_n60_), .O(new_n126_));
  nand4  g81(.a(new_n55_), .b(x19), .c(x18), .d(x17), .O(new_n127_));
  nor2   g82(.a(x23), .b(x22), .O(new_n128_));
  inv1   g83(.a(new_n128_), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n126_), .c(new_n47_), .O(new_n131_));
  nor2   g86(.a(new_n60_), .b(new_n47_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n101_), .c(new_n50_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n131_), .O(z16));
  oai21  g89(.a(new_n129_), .b(new_n127_), .c(x24), .O(new_n135_));
  nand3  g90(.a(new_n128_), .b(new_n122_), .c(new_n61_), .O(new_n136_));
  nand4  g91(.a(new_n136_), .b(new_n135_), .c(new_n121_), .d(new_n47_), .O(z17));
  inv1   g92(.a(x25), .O(new_n138_));
  nand2  g93(.a(new_n136_), .b(new_n138_), .O(new_n139_));
  nand3  g94(.a(new_n128_), .b(new_n122_), .c(x25), .O(new_n140_));
  nand3  g95(.a(new_n140_), .b(new_n139_), .c(new_n47_), .O(new_n141_));
  aoi22  g96(.a(new_n101_), .b(new_n50_), .c(x25), .d(x24), .O(new_n142_));
  nand2  g97(.a(new_n142_), .b(new_n141_), .O(z18));
endmodule


