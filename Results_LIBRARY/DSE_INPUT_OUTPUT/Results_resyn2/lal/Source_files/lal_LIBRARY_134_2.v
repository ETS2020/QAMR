// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x19), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x18), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n48_), .c(new_n52_), .O(new_n58_));
  inv1   g13(.a(x23), .O(new_n59_));
  nand3  g14(.a(new_n52_), .b(new_n59_), .c(new_n53_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x24), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(x19), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n58_), .O(z01));
  nand2  g21(.a(new_n61_), .b(x19), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n58_), .O(z03));
  nand2  g23(.a(new_n49_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi22  g34(.a(new_n48_), .b(x19), .c(new_n46_), .d(x06), .O(z07));
  inv1   g35(.a(x19), .O(new_n81_));
  nor2   g36(.a(x18), .b(x17), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n53_), .c(new_n55_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x23), .c(x24), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n49_), .c(new_n52_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n63_), .c(new_n49_), .O(z09));
  nand3  g44(.a(new_n63_), .b(new_n87_), .c(new_n62_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  or2    g48(.a(new_n93_), .b(new_n82_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n49_), .O(z11));
  oai21  g50(.a(new_n93_), .b(new_n48_), .c(x19), .O(new_n96_));
  aoi21  g51(.a(new_n92_), .b(new_n81_), .c(new_n90_), .O(new_n97_));
  and2   g52(.a(new_n97_), .b(new_n96_), .O(z12));
  nand2  g53(.a(new_n90_), .b(new_n49_), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(x20), .O(new_n100_));
  nor2   g55(.a(new_n48_), .b(new_n81_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n93_), .c(new_n53_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(z13));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n93_), .c(new_n48_), .O(new_n105_));
  aoi21  g60(.a(new_n102_), .b(x21), .c(new_n90_), .O(new_n106_));
  oai21  g61(.a(new_n105_), .b(new_n81_), .c(new_n106_), .O(z14));
  inv1   g62(.a(new_n90_), .O(new_n108_));
  oai21  g63(.a(new_n102_), .b(x21), .c(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(new_n110_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n92_), .c(x24), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x19), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n109_), .c(new_n108_), .O(z15));
  nor3   g69(.a(x22), .b(x21), .c(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n93_), .c(new_n59_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(x24), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x19), .O(new_n118_));
  nand3  g73(.a(new_n115_), .b(new_n101_), .c(new_n93_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(x23), .c(new_n90_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n118_), .O(z16));
  oai21  g76(.a(new_n116_), .b(new_n81_), .c(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n99_), .O(z17));
  nand3  g78(.a(new_n108_), .b(new_n49_), .c(new_n52_), .O(z18));
endmodule


