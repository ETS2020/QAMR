// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x09), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n50_), .O(z01));
  nand3  g19(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  aoi21  g21(.a(new_n46_), .b(x00), .c(x15), .O(new_n67_));
  inv1   g22(.a(x09), .O(new_n68_));
  inv1   g23(.a(x03), .O(new_n69_));
  inv1   g24(.a(x10), .O(new_n70_));
  aoi22  g25(.a(x12), .b(new_n69_), .c(new_n70_), .d(x01), .O(new_n71_));
  oai21  g26(.a(new_n68_), .b(x00), .c(new_n71_), .O(new_n72_));
  inv1   g27(.a(x01), .O(new_n73_));
  nand2  g28(.a(x10), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x12), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x03), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(new_n72_), .c(new_n46_), .O(new_n79_));
  oai21  g34(.a(new_n67_), .b(x09), .c(new_n79_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g36(.a(x14), .b(new_n46_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n49_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g39(.a(new_n61_), .b(new_n53_), .c(new_n49_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n48_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand2  g43(.a(new_n63_), .b(new_n48_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n50_), .O(z10));
  nand2  g45(.a(new_n56_), .b(new_n55_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n89_), .c(new_n50_), .O(z11));
  inv1   g49(.a(new_n92_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x19), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(new_n57_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n89_), .c(new_n50_), .O(z12));
  inv1   g54(.a(new_n89_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n95_), .b(new_n54_), .c(x19), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(z13));
  inv1   g58(.a(x21), .O(new_n104_));
  inv1   g59(.a(new_n102_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n89_), .O(new_n106_));
  oai21  g61(.a(new_n105_), .b(new_n104_), .c(new_n106_), .O(z14));
  nand2  g62(.a(x15), .b(x09), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n57_), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n95_), .c(new_n109_), .d(new_n104_), .O(new_n111_));
  oai21  g66(.a(new_n102_), .b(x21), .c(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n63_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n48_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n108_), .O(z15));
  nand2  g70(.a(new_n111_), .b(x23), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n110_), .c(new_n95_), .d(new_n104_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n63_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n48_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n108_), .O(z16));
  nand2  g76(.a(new_n118_), .b(x24), .O(new_n122_));
  nor2   g77(.a(x24), .b(x21), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n117_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n110_), .c(new_n95_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n122_), .c(new_n100_), .O(z17));
  nand3  g82(.a(new_n125_), .b(new_n105_), .c(new_n53_), .O(new_n128_));
  aoi21  g83(.a(new_n126_), .b(x25), .c(new_n89_), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(new_n128_), .c(new_n49_), .O(z18));
endmodule


