// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x13), .O(new_n47_));
  nor2   g02(.a(x23), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x25), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(x25), .b(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  and2   g18(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n60_), .c(new_n48_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  inv1   g22(.a(new_n62_), .O(new_n68_));
  oai21  g23(.a(new_n59_), .b(x13), .c(new_n50_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n68_), .O(z03));
  inv1   g25(.a(new_n48_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  nor2   g34(.a(x13), .b(x08), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n71_), .O(z06));
  nand3  g37(.a(new_n71_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g38(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x23), .c(x24), .O(new_n85_));
  nor2   g40(.a(new_n48_), .b(x25), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n61_), .c(new_n71_), .O(z09));
  nand3  g46(.a(new_n61_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n71_), .O(z10));
  nand2  g48(.a(new_n53_), .b(new_n52_), .O(new_n94_));
  nor2   g49(.a(new_n92_), .b(new_n48_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  aoi21  g55(.a(x18), .b(x17), .c(x19), .O(new_n101_));
  nor4   g56(.a(new_n101_), .b(new_n100_), .c(new_n92_), .d(new_n48_), .O(z12));
  nand2  g57(.a(new_n100_), .b(new_n51_), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n95_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n100_), .c(new_n92_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n106_), .c(new_n48_), .O(z14));
  nand2  g64(.a(new_n107_), .b(new_n100_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x22), .O(new_n111_));
  nor3   g66(.a(x22), .b(x21), .c(x20), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n100_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n95_), .O(z15));
  nand3  g69(.a(new_n112_), .b(new_n100_), .c(new_n50_), .O(new_n115_));
  inv1   g70(.a(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n47_), .O(new_n117_));
  inv1   g72(.a(new_n92_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n48_), .O(new_n119_));
  aoi21  g74(.a(new_n113_), .b(x23), .c(new_n119_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n117_), .O(z16));
  inv1   g76(.a(x22), .O(new_n122_));
  inv1   g77(.a(x24), .O(new_n123_));
  nand4  g78(.a(new_n107_), .b(new_n100_), .c(new_n123_), .d(new_n122_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  oai21  g80(.a(x24), .b(new_n50_), .c(new_n125_), .O(new_n126_));
  aoi21  g81(.a(new_n115_), .b(x24), .c(new_n92_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z17));
  nand2  g83(.a(new_n124_), .b(x25), .O(new_n129_));
  nand4  g84(.a(new_n112_), .b(new_n100_), .c(new_n62_), .d(new_n50_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n47_), .O(new_n132_));
  aoi21  g87(.a(x25), .b(x23), .c(new_n119_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


