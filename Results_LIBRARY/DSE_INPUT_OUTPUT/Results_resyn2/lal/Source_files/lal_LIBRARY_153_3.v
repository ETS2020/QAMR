// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x20), .b(x15), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  nand2  g06(.a(x22), .b(x21), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x24), .O(new_n54_));
  aoi21  g09(.a(x05), .b(x04), .c(x07), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n54_), .b(new_n50_), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n51_), .c(new_n58_), .O(new_n63_));
  oai22  g18(.a(new_n63_), .b(x25), .c(new_n57_), .d(new_n49_), .O(z01));
  and2   g19(.a(new_n47_), .b(x16), .O(z02));
  nand2  g20(.a(new_n54_), .b(new_n47_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n47_), .O(z04));
  inv1   g29(.a(x13), .O(new_n75_));
  nand3  g30(.a(new_n47_), .b(new_n75_), .c(new_n46_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g35(.a(new_n62_), .b(new_n58_), .c(new_n52_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x23), .c(x24), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n50_), .c(new_n49_), .O(z08));
  inv1   g38(.a(x07), .O(new_n84_));
  inv1   g39(.a(x15), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(new_n84_), .c(x05), .d(x04), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z09));
  nand2  g42(.a(new_n55_), .b(new_n85_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n59_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  nor2   g46(.a(x18), .b(x17), .O(new_n92_));
  nor2   g47(.a(new_n60_), .b(new_n59_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n88_), .c(new_n92_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nor2   g51(.a(new_n93_), .b(x19), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n88_), .O(z12));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n95_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n89_), .O(z13));
  inv1   g57(.a(x21), .O(new_n103_));
  nand2  g58(.a(new_n96_), .b(new_n103_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n85_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n58_), .O(new_n106_));
  aoi21  g61(.a(new_n100_), .b(x21), .c(new_n56_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(x15), .c(new_n106_), .O(z14));
  oai21  g63(.a(new_n104_), .b(x20), .c(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n99_), .b(new_n110_), .c(new_n103_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n89_), .O(z15));
  nand2  g67(.a(new_n111_), .b(x23), .O(new_n113_));
  nor3   g68(.a(x23), .b(x22), .c(x21), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n99_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n89_), .O(z16));
  nand3  g71(.a(new_n51_), .b(new_n110_), .c(new_n103_), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(x19), .c(x18), .d(x17), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n117_), .c(new_n85_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n58_), .O(new_n121_));
  aoi21  g76(.a(new_n115_), .b(x24), .c(new_n56_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(x15), .c(new_n121_), .O(z17));
  nand4  g78(.a(new_n114_), .b(new_n96_), .c(new_n118_), .d(new_n58_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n50_), .O(new_n126_));
  aoi21  g81(.a(new_n124_), .b(x25), .c(new_n88_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z18));
endmodule


