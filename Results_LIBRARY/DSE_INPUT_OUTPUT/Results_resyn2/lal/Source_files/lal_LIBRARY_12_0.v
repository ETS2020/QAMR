// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n134_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x06), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  nand2  g05(.a(x22), .b(x21), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(new_n52_));
  oai21  g07(.a(new_n50_), .b(x20), .c(new_n52_), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(x23), .O(new_n55_));
  inv1   g10(.a(x06), .O(new_n56_));
  nor2   g11(.a(x24), .b(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x07), .c(new_n48_), .O(new_n61_));
  oai21  g16(.a(new_n58_), .b(x25), .c(new_n61_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n47_), .b(new_n63_), .O(z02));
  inv1   g19(.a(x23), .O(new_n65_));
  inv1   g20(.a(x25), .O(new_n66_));
  nand3  g21(.a(new_n53_), .b(new_n66_), .c(new_n65_), .O(new_n67_));
  aoi22  g22(.a(new_n67_), .b(x24), .c(x25), .d(x06), .O(z03));
  nand2  g23(.a(new_n48_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n69_), .b(new_n78_), .O(z06));
  nand2  g34(.a(new_n46_), .b(x06), .O(z07));
  inv1   g35(.a(x20), .O(new_n81_));
  inv1   g36(.a(x19), .O(new_n82_));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n81_), .c(new_n51_), .O(new_n85_));
  nand2  g40(.a(new_n66_), .b(new_n65_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n85_), .c(x24), .O(new_n87_));
  oai21  g42(.a(new_n66_), .b(new_n56_), .c(new_n87_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n47_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n60_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  inv1   g49(.a(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n59_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  inv1   g54(.a(new_n83_), .O(new_n100_));
  nand2  g55(.a(x18), .b(x17), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n92_), .c(new_n59_), .d(new_n100_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z11));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  nand3  g59(.a(new_n59_), .b(new_n90_), .c(new_n89_), .O(new_n105_));
  aoi21  g60(.a(new_n101_), .b(new_n82_), .c(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n48_), .O(z12));
  nand2  g63(.a(new_n104_), .b(x20), .O(new_n109_));
  inv1   g64(.a(new_n104_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n81_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n97_), .O(z13));
  nand2  g67(.a(new_n111_), .b(x21), .O(new_n113_));
  nor3   g68(.a(new_n104_), .b(x21), .c(x20), .O(new_n114_));
  inv1   g69(.a(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n97_), .O(z14));
  inv1   g71(.a(x22), .O(new_n117_));
  nand2  g72(.a(new_n114_), .b(new_n117_), .O(new_n118_));
  nand2  g73(.a(new_n115_), .b(x22), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n97_), .O(z15));
  nand2  g75(.a(new_n118_), .b(x23), .O(new_n121_));
  nor2   g76(.a(x23), .b(x22), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n114_), .c(new_n105_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n121_), .c(new_n47_), .O(z16));
  nand2  g79(.a(new_n122_), .b(new_n114_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(x24), .O(new_n126_));
  nand2  g81(.a(new_n105_), .b(new_n48_), .O(new_n127_));
  nor2   g82(.a(x21), .b(x20), .O(new_n128_));
  nand4  g83(.a(new_n122_), .b(new_n128_), .c(new_n110_), .d(new_n57_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n126_), .O(z17));
  nand4  g85(.a(new_n122_), .b(new_n114_), .c(new_n66_), .d(new_n54_), .O(new_n131_));
  oai21  g86(.a(new_n125_), .b(x24), .c(x25), .O(new_n132_));
  nand2  g87(.a(new_n66_), .b(x24), .O(new_n133_));
  aoi21  g88(.a(new_n133_), .b(new_n56_), .c(new_n105_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(z18));
endmodule


