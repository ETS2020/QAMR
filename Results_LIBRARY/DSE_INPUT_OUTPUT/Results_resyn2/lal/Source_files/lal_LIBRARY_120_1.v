// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n99_, new_n101_,
    new_n103_, new_n106_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nand2  g03(.a(x18), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(new_n49_), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x23), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(x24), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n60_));
  inv1   g15(.a(x17), .O(new_n61_));
  inv1   g16(.a(x18), .O(new_n62_));
  inv1   g17(.a(x19), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nor2   g19(.a(x23), .b(x20), .O(new_n65_));
  nand4  g20(.a(new_n65_), .b(new_n64_), .c(new_n49_), .d(new_n53_), .O(new_n66_));
  oai21  g21(.a(new_n60_), .b(new_n52_), .c(new_n66_), .O(z01));
  nand3  g22(.a(new_n65_), .b(new_n64_), .c(new_n49_), .O(new_n68_));
  nand2  g23(.a(new_n57_), .b(new_n49_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n68_), .c(x25), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n52_), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n52_), .O(z07));
  inv1   g36(.a(x24), .O(new_n82_));
  nand3  g37(.a(x22), .b(x21), .c(x20), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n54_), .c(new_n82_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x25), .c(new_n49_), .O(new_n85_));
  nand3  g40(.a(x24), .b(x22), .c(x21), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n64_), .c(new_n85_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n62_), .c(x15), .O(z09));
  nand3  g45(.a(new_n58_), .b(new_n62_), .c(new_n48_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  nand2  g49(.a(new_n58_), .b(x17), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n62_), .c(x15), .O(z11));
  nand2  g51(.a(new_n92_), .b(x19), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z12));
  oai21  g53(.a(new_n59_), .b(x20), .c(new_n62_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n48_), .O(z13));
  inv1   g55(.a(x21), .O(new_n101_));
  nand2  g56(.a(new_n92_), .b(new_n101_), .O(z14));
  oai21  g57(.a(new_n59_), .b(x22), .c(new_n62_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n48_), .O(z15));
  nand2  g59(.a(new_n92_), .b(new_n54_), .O(z16));
  oai21  g60(.a(new_n59_), .b(x24), .c(new_n62_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(z17));
  nand2  g62(.a(new_n92_), .b(new_n53_), .O(z18));
endmodule


