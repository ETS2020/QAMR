// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n56_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n68_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x23), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(new_n48_), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n47_), .O(z08));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand4  g09(.a(new_n54_), .b(z08), .c(new_n51_), .d(new_n50_), .O(z01));
  inv1   g10(.a(x16), .O(new_n56_));
  nor2   g11(.a(new_n48_), .b(new_n56_), .O(z02));
  inv1   g12(.a(z08), .O(z03));
  xor2a  g13(.a(x12), .b(x03), .O(new_n59_));
  xor2a  g14(.a(x11), .b(x02), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  xor2a  g16(.a(x09), .b(x00), .O(new_n62_));
  xor2a  g17(.a(x10), .b(x01), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g19(.a(new_n51_), .b(new_n46_), .O(new_n65_));
  aoi21  g20(.a(new_n64_), .b(new_n61_), .c(new_n65_), .O(z04));
  oai21  g21(.a(x13), .b(x08), .c(new_n51_), .O(z05));
  nand2  g22(.a(x14), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n51_), .O(z06));
  aoi21  g24(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g25(.a(x15), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n50_), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(new_n54_), .c(new_n51_), .O(z09));
  inv1   g28(.a(x17), .O(new_n74_));
  nand3  g29(.a(new_n54_), .b(new_n71_), .c(new_n50_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z10));
  xnor2a g33(.a(x18), .b(x17), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n75_), .c(new_n51_), .O(z11));
  inv1   g35(.a(x19), .O(new_n81_));
  and2   g36(.a(x18), .b(x17), .O(new_n82_));
  xor2a  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor3   g38(.a(new_n83_), .b(new_n75_), .c(new_n48_), .O(z12));
  nand3  g39(.a(x19), .b(x18), .c(x17), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g41(.a(x20), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(x19), .c(x18), .d(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n75_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n86_), .c(new_n48_), .O(z13));
  inv1   g46(.a(new_n75_), .O(new_n92_));
  nand2  g47(.a(new_n88_), .b(x21), .O(new_n93_));
  inv1   g48(.a(x21), .O(new_n94_));
  nand4  g49(.a(new_n82_), .b(new_n94_), .c(new_n87_), .d(x19), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  and2   g51(.a(new_n96_), .b(new_n51_), .O(z14));
  inv1   g52(.a(x22), .O(new_n98_));
  nand3  g53(.a(new_n89_), .b(new_n98_), .c(new_n94_), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(x22), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n76_), .O(z15));
  oai21  g56(.a(new_n95_), .b(x22), .c(x23), .O(new_n102_));
  nand2  g57(.a(new_n75_), .b(new_n51_), .O(new_n103_));
  inv1   g58(.a(x23), .O(new_n104_));
  nor2   g59(.a(x22), .b(x21), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n47_), .c(new_n104_), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n89_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n103_), .c(new_n102_), .O(z16));
  inv1   g64(.a(new_n85_), .O(new_n110_));
  nor2   g65(.a(x23), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n105_), .c(new_n110_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n92_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  nand2  g69(.a(x24), .b(x23), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(z17));
  nand2  g71(.a(new_n105_), .b(new_n52_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n88_), .c(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n104_), .O(new_n119_));
  oai21  g74(.a(new_n106_), .b(new_n88_), .c(x25), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n92_), .O(z18));
endmodule


