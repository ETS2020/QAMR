// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x22), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(x21), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n50_), .c(new_n49_), .O(new_n58_));
  inv1   g13(.a(x24), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n50_), .b(new_n59_), .c(new_n62_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n58_), .c(new_n47_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n47_), .b(new_n65_), .O(z02));
  nor2   g21(.a(new_n47_), .b(new_n50_), .O(new_n67_));
  nand3  g22(.a(new_n56_), .b(x22), .c(x21), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(x24), .c(new_n67_), .O(z03));
  inv1   g25(.a(new_n47_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  nor2   g34(.a(new_n72_), .b(x13), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n71_), .O(z06));
  nand3  g37(.a(new_n71_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(new_n67_), .O(new_n84_));
  inv1   g39(.a(x21), .O(new_n85_));
  aoi21  g40(.a(new_n55_), .b(new_n51_), .c(new_n85_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x22), .c(x23), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n59_), .c(new_n84_), .O(z08));
  inv1   g43(.a(new_n61_), .O(new_n89_));
  nor2   g44(.a(x15), .b(x07), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n71_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand2  g47(.a(new_n90_), .b(new_n52_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n89_), .c(new_n47_), .O(z10));
  inv1   g49(.a(x15), .O(new_n95_));
  nand3  g50(.a(new_n61_), .b(new_n95_), .c(new_n60_), .O(new_n96_));
  nand2  g51(.a(new_n53_), .b(new_n52_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(new_n96_), .c(new_n71_), .O(z11));
  nand2  g55(.a(new_n98_), .b(new_n54_), .O(new_n101_));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  oai21  g58(.a(new_n103_), .b(new_n96_), .c(new_n71_), .O(z12));
  nor2   g59(.a(new_n102_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(x20), .c(new_n96_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n47_), .O(z13));
  nor2   g63(.a(new_n89_), .b(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n106_), .b(x21), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(new_n85_), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n90_), .O(z14));
  nand2  g67(.a(new_n96_), .b(new_n71_), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand3  g69(.a(new_n105_), .b(x23), .c(new_n85_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g71(.a(new_n105_), .b(x22), .c(new_n85_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n113_), .O(z15));
  inv1   g74(.a(new_n96_), .O(new_n120_));
  nand2  g75(.a(x23), .b(x22), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n116_), .c(new_n120_), .O(z16));
  nor2   g77(.a(new_n96_), .b(new_n47_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n59_), .O(z17));
  nand2  g79(.a(new_n123_), .b(new_n50_), .O(z18));
endmodule


