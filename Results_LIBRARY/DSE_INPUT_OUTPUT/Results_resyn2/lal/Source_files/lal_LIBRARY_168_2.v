// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand3  g01(.a(x24), .b(new_n46_), .c(x08), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(z00));
  inv1   g03(.a(x08), .O(new_n49_));
  nor2   g04(.a(x24), .b(new_n49_), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  inv1   g06(.a(x07), .O(new_n52_));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  and2   g16(.a(x22), .b(x21), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g18(.a(x24), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x23), .O(new_n65_));
  nor2   g20(.a(x24), .b(x08), .O(new_n66_));
  aoi21  g21(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x25), .c(new_n55_), .O(z01));
  nand2  g23(.a(new_n51_), .b(new_n46_), .O(z02));
  inv1   g24(.a(x23), .O(new_n70_));
  nand2  g25(.a(new_n63_), .b(new_n70_), .O(new_n71_));
  aoi22  g26(.a(new_n71_), .b(x24), .c(new_n51_), .d(x25), .O(z03));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n51_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n51_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(x08), .O(z06));
  nand2  g37(.a(new_n49_), .b(x06), .O(z07));
  inv1   g38(.a(x25), .O(new_n84_));
  nand2  g39(.a(new_n71_), .b(x24), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n51_), .c(new_n84_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n53_), .c(new_n51_), .O(z09));
  nand2  g44(.a(new_n87_), .b(new_n53_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n57_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n58_), .b(new_n57_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n91_), .c(new_n94_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  aoi21  g53(.a(new_n95_), .b(new_n59_), .c(new_n90_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n51_), .O(z12));
  nor2   g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  aoi21  g58(.a(new_n98_), .b(x20), .c(new_n90_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n50_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n91_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  nor2   g65(.a(x22), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n102_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n91_), .O(z15));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n114_));
  nand3  g69(.a(new_n111_), .b(new_n102_), .c(new_n70_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n91_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  nand4  g72(.a(new_n111_), .b(new_n102_), .c(new_n64_), .d(new_n70_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n91_), .O(z17));
  xor2a  g74(.a(new_n118_), .b(x25), .O(new_n120_));
  aoi21  g75(.a(new_n84_), .b(x24), .c(new_n49_), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(new_n90_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(z18));
endmodule


