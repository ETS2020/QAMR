// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n82_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x16), .b(new_n46_), .O(z00));
  nor2   g02(.a(x15), .b(x08), .O(new_n48_));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n58_), .c(new_n48_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(new_n48_), .b(new_n61_), .O(z02));
  aoi21  g17(.a(new_n57_), .b(new_n49_), .c(new_n48_), .O(z08));
  inv1   g18(.a(z08), .O(z03));
  inv1   g19(.a(x10), .O(new_n65_));
  inv1   g20(.a(x00), .O(new_n66_));
  aoi22  g21(.a(new_n65_), .b(x01), .c(x09), .d(new_n66_), .O(new_n67_));
  oai21  g22(.a(new_n65_), .b(x01), .c(new_n67_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(new_n69_));
  inv1   g24(.a(x12), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x03), .c(x15), .O(new_n71_));
  inv1   g26(.a(x11), .O(new_n72_));
  oai22  g27(.a(new_n72_), .b(x02), .c(x09), .d(new_n66_), .O(new_n73_));
  nand2  g28(.a(new_n70_), .b(x03), .O(new_n74_));
  nand2  g29(.a(new_n72_), .b(x02), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nor3   g31(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n69_), .c(x08), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand3  g34(.a(x15), .b(new_n79_), .c(new_n46_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  aoi21  g37(.a(x15), .b(new_n82_), .c(x08), .O(z06));
  nand3  g38(.a(x15), .b(new_n46_), .c(x06), .O(z07));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x08), .c(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n85_), .O(z09));
  inv1   g44(.a(new_n88_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n85_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x17), .O(z10));
  nor2   g47(.a(x18), .b(x17), .O(new_n93_));
  nor2   g48(.a(new_n52_), .b(new_n51_), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n91_), .c(new_n93_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n94_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n91_), .O(z12));
  nand2  g54(.a(new_n97_), .b(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n59_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x08), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n87_), .O(z13));
  inv1   g59(.a(new_n91_), .O(new_n105_));
  nand2  g60(.a(new_n100_), .b(x21), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n97_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  nor3   g66(.a(new_n96_), .b(x21), .c(x20), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n110_), .c(new_n59_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x08), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n87_), .O(z15));
  nand2  g71(.a(new_n113_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n107_), .c(new_n97_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n105_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  inv1   g76(.a(x24), .O(new_n122_));
  nand3  g77(.a(new_n118_), .b(new_n112_), .c(new_n122_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n59_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x08), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n87_), .O(z17));
  aoi21  g81(.a(new_n123_), .b(x25), .c(new_n91_), .O(new_n127_));
  oai21  g82(.a(new_n123_), .b(x25), .c(new_n127_), .O(z18));
endmodule


