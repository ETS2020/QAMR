// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(x24), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  inv1   g16(.a(x24), .O(new_n62_));
  oai21  g17(.a(new_n60_), .b(x23), .c(new_n62_), .O(new_n63_));
  aoi21  g18(.a(x05), .b(x04), .c(x07), .O(new_n64_));
  and2   g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  oai21  g20(.a(new_n61_), .b(new_n59_), .c(new_n65_), .O(z01));
  oai21  g21(.a(new_n61_), .b(new_n59_), .c(new_n63_), .O(z03));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n50_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n49_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand2  g33(.a(new_n59_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n60_), .c(new_n48_), .O(z08));
  nor2   g35(.a(x15), .b(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x05), .c(x04), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z09));
  nand2  g38(.a(x05), .b(x04), .O(new_n84_));
  nand2  g39(.a(new_n81_), .b(new_n84_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n49_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n54_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand2  g43(.a(new_n55_), .b(new_n54_), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n86_), .c(new_n89_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z11));
  and2   g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x19), .O(new_n94_));
  nand2  g49(.a(new_n90_), .b(new_n56_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n85_), .c(new_n49_), .O(z12));
  nand2  g52(.a(new_n94_), .b(x20), .O(new_n98_));
  nand4  g53(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n86_), .O(z13));
  inv1   g55(.a(new_n85_), .O(new_n101_));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  nand4  g58(.a(new_n93_), .b(new_n103_), .c(new_n53_), .d(x19), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n50_), .O(z14));
  nand2  g61(.a(new_n104_), .b(x22), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n93_), .c(new_n53_), .d(x19), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n86_), .O(z15));
  nand3  g65(.a(new_n109_), .b(x24), .c(x23), .O(new_n111_));
  nand2  g66(.a(new_n85_), .b(new_n50_), .O(new_n112_));
  inv1   g67(.a(new_n99_), .O(new_n113_));
  nand3  g68(.a(new_n108_), .b(new_n113_), .c(new_n48_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(z16));
  nand2  g70(.a(new_n109_), .b(x24), .O(new_n116_));
  nand3  g71(.a(new_n108_), .b(new_n113_), .c(new_n62_), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(new_n101_), .d(new_n48_), .O(z17));
  oai21  g73(.a(new_n109_), .b(x24), .c(x25), .O(new_n119_));
  nand4  g74(.a(new_n108_), .b(new_n113_), .c(new_n60_), .d(new_n62_), .O(new_n120_));
  oai21  g75(.a(x25), .b(new_n62_), .c(x23), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(new_n101_), .O(z18));
endmodule


