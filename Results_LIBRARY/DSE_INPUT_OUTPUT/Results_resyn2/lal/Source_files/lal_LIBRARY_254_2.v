// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x23), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  aoi21  g16(.a(x25), .b(new_n52_), .c(x24), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n62_), .c(x07), .O(new_n65_));
  oai21  g20(.a(new_n61_), .b(new_n54_), .c(new_n65_), .O(z01));
  aoi21  g21(.a(new_n61_), .b(x24), .c(new_n54_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  inv1   g29(.a(new_n49_), .O(new_n75_));
  nor3   g30(.a(new_n75_), .b(x13), .c(x08), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n75_), .O(z07));
  inv1   g34(.a(z03), .O(z08));
  nor2   g35(.a(x15), .b(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n64_), .c(new_n49_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand3  g38(.a(new_n81_), .b(new_n63_), .c(new_n49_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n56_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n85_), .c(new_n88_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z11));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n58_), .O(new_n93_));
  nand2  g48(.a(new_n81_), .b(new_n63_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n49_), .O(z12));
  inv1   g52(.a(new_n92_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n55_), .O(new_n99_));
  aoi21  g54(.a(new_n92_), .b(x20), .c(new_n94_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n75_), .O(z13));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n85_), .O(z14));
  nand2  g60(.a(new_n104_), .b(x22), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  nand3  g62(.a(new_n103_), .b(new_n98_), .c(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n85_), .O(z15));
  aoi21  g64(.a(new_n108_), .b(x23), .c(new_n84_), .O(new_n110_));
  oai21  g65(.a(new_n108_), .b(x23), .c(new_n110_), .O(z16));
  nand4  g66(.a(new_n103_), .b(new_n98_), .c(new_n48_), .d(new_n107_), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(x24), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n112_), .c(new_n95_), .d(new_n52_), .O(z17));
  nand2  g69(.a(new_n112_), .b(new_n53_), .O(new_n115_));
  nand4  g70(.a(new_n103_), .b(new_n98_), .c(x25), .d(new_n107_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n52_), .O(new_n117_));
  aoi22  g72(.a(new_n94_), .b(new_n49_), .c(x25), .d(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n117_), .O(z18));
endmodule


