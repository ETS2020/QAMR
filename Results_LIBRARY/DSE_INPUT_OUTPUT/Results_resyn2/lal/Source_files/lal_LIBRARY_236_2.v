// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n86_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x02), .O(new_n48_));
  nor2   g03(.a(x23), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x25), .c(new_n48_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nor2   g17(.a(x25), .b(x24), .O(new_n63_));
  inv1   g18(.a(x07), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n62_), .O(z01));
  inv1   g23(.a(new_n63_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n62_), .O(z03));
  oai21  g25(.a(x11), .b(x08), .c(x23), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x02), .O(new_n72_));
  inv1   g27(.a(x09), .O(new_n73_));
  inv1   g28(.a(x12), .O(new_n74_));
  aoi22  g29(.a(new_n74_), .b(x03), .c(new_n73_), .d(x00), .O(new_n75_));
  oai21  g30(.a(new_n73_), .b(x00), .c(new_n75_), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  inv1   g32(.a(x03), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n78_), .O(new_n79_));
  nand2  g34(.a(x11), .b(new_n48_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n79_), .c(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n76_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n72_), .O(z04));
  nand2  g38(.a(new_n50_), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(x13), .O(z05));
  inv1   g40(.a(x14), .O(new_n86_));
  nor2   g41(.a(new_n84_), .b(new_n86_), .O(z06));
  nand3  g42(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  oai21  g43(.a(new_n60_), .b(x23), .c(x24), .O(new_n89_));
  nor2   g44(.a(new_n49_), .b(x25), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(z08));
  inv1   g46(.a(x15), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n64_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n65_), .c(new_n50_), .O(z09));
  nand3  g49(.a(new_n65_), .b(new_n92_), .c(new_n64_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n49_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n55_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z10));
  xnor2a g53(.a(x18), .b(x17), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(new_n95_), .c(new_n50_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nor4   g58(.a(new_n103_), .b(new_n102_), .c(new_n95_), .d(new_n49_), .O(z12));
  nand2  g59(.a(new_n102_), .b(new_n54_), .O(new_n105_));
  aoi21  g60(.a(new_n101_), .b(x20), .c(new_n95_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n49_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  nor3   g63(.a(new_n101_), .b(x21), .c(x20), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n96_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  aoi21  g68(.a(new_n109_), .b(new_n113_), .c(new_n95_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n112_), .c(new_n49_), .O(z15));
  oai21  g70(.a(new_n110_), .b(x22), .c(x23), .O(new_n116_));
  nand3  g71(.a(new_n109_), .b(new_n53_), .c(new_n113_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n96_), .O(z16));
  inv1   g73(.a(new_n95_), .O(new_n119_));
  nand2  g74(.a(new_n117_), .b(x24), .O(new_n120_));
  nor2   g75(.a(x21), .b(x20), .O(new_n121_));
  nor2   g76(.a(x24), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n102_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n48_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n53_), .O(new_n125_));
  nand2  g80(.a(x24), .b(x02), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n120_), .d(new_n119_), .O(z17));
  nand2  g82(.a(new_n123_), .b(x25), .O(new_n128_));
  nor3   g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n121_), .c(new_n102_), .d(new_n113_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n48_), .O(new_n132_));
  aoi22  g87(.a(new_n95_), .b(new_n50_), .c(x25), .d(x23), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


