// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x20), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x23), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(x24), .O(new_n55_));
  aoi21  g10(.a(x05), .b(x04), .c(x07), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nor2   g17(.a(x23), .b(x20), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n47_), .O(new_n64_));
  oai22  g19(.a(new_n64_), .b(x25), .c(new_n58_), .d(new_n48_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nand2  g22(.a(new_n55_), .b(new_n49_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n64_), .c(x25), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z04));
  nor3   g31(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  oai21  g36(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n82_));
  inv1   g37(.a(x20), .O(new_n83_));
  aoi21  g38(.a(new_n53_), .b(new_n52_), .c(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n82_), .b(new_n47_), .c(new_n84_), .O(new_n85_));
  nand2  g40(.a(new_n49_), .b(x25), .O(new_n86_));
  oai21  g41(.a(new_n85_), .b(new_n81_), .c(new_n86_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand4  g43(.a(new_n47_), .b(new_n88_), .c(x05), .d(x04), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  nand2  g45(.a(new_n56_), .b(new_n47_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n59_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  nor2   g49(.a(x18), .b(x17), .O(new_n95_));
  nor2   g50(.a(new_n60_), .b(new_n59_), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n91_), .c(new_n95_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nor2   g54(.a(new_n96_), .b(x19), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n99_), .c(new_n91_), .O(z12));
  nor2   g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n92_), .O(z13));
  inv1   g60(.a(x21), .O(new_n106_));
  nand2  g61(.a(new_n102_), .b(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n106_), .b(new_n47_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n103_), .c(new_n57_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n107_), .c(new_n48_), .O(z14));
  nand2  g65(.a(new_n107_), .b(x22), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n102_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n92_), .O(z15));
  nand2  g69(.a(new_n113_), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n112_), .b(new_n99_), .c(new_n63_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n92_), .O(z16));
  inv1   g72(.a(x22), .O(new_n118_));
  nand4  g73(.a(new_n52_), .b(new_n118_), .c(new_n106_), .d(new_n83_), .O(new_n119_));
  nand2  g74(.a(new_n81_), .b(new_n47_), .O(new_n120_));
  oai21  g75(.a(new_n119_), .b(new_n98_), .c(new_n120_), .O(new_n121_));
  nand4  g76(.a(new_n112_), .b(new_n99_), .c(new_n63_), .d(new_n81_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n56_), .O(new_n123_));
  and2   g78(.a(new_n123_), .b(new_n49_), .O(z17));
  aoi21  g79(.a(new_n122_), .b(x25), .c(new_n91_), .O(new_n125_));
  oai21  g80(.a(new_n122_), .b(x25), .c(new_n125_), .O(z18));
endmodule


