// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  or2    g07(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nor3   g08(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  and2   g09(.a(x22), .b(x21), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(x20), .c(new_n55_), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(x23), .O(new_n58_));
  inv1   g13(.a(x15), .O(new_n59_));
  nor2   g14(.a(x24), .b(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n58_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x25), .c(new_n53_), .O(z01));
  inv1   g17(.a(x23), .O(new_n63_));
  nand2  g18(.a(new_n56_), .b(new_n63_), .O(new_n64_));
  aoi22  g19(.a(new_n64_), .b(x24), .c(new_n49_), .d(x25), .O(z03));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor2   g30(.a(new_n66_), .b(new_n75_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g32(.a(x25), .O(new_n78_));
  nand2  g33(.a(new_n64_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n49_), .c(new_n78_), .O(z08));
  inv1   g35(.a(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x05), .c(x04), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x24), .c(x15), .O(z09));
  inv1   g38(.a(x17), .O(new_n84_));
  nand2  g39(.a(new_n52_), .b(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x15), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n52_), .b(x24), .c(new_n59_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(z11));
  inv1   g46(.a(x19), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n92_), .O(new_n93_));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n52_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(x24), .c(x15), .O(z12));
  inv1   g51(.a(new_n88_), .O(new_n97_));
  nand2  g52(.a(new_n94_), .b(x20), .O(new_n98_));
  nor2   g53(.a(new_n94_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z13));
  nand2  g56(.a(new_n100_), .b(x21), .O(new_n102_));
  inv1   g57(.a(x21), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n97_), .O(z14));
  aoi21  g60(.a(new_n104_), .b(x22), .c(new_n88_), .O(new_n106_));
  oai21  g61(.a(new_n104_), .b(x22), .c(new_n106_), .O(z15));
  oai21  g62(.a(new_n104_), .b(x22), .c(x23), .O(new_n108_));
  nor2   g63(.a(x23), .b(x22), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n99_), .c(new_n103_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n97_), .O(z16));
  nand4  g66(.a(new_n109_), .b(new_n99_), .c(new_n52_), .d(new_n103_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x24), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n59_), .O(z17));
  nand2  g69(.a(new_n97_), .b(new_n78_), .O(z18));
endmodule


