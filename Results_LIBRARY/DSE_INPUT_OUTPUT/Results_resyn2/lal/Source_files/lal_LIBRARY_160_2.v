// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  aoi21  g01(.a(x24), .b(x16), .c(new_n46_), .O(z00));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  oai21  g05(.a(new_n50_), .b(x08), .c(new_n49_), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand4  g13(.a(new_n58_), .b(new_n57_), .c(new_n51_), .d(new_n48_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nor2   g15(.a(x24), .b(new_n46_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n60_), .O(z02));
  nand2  g18(.a(new_n57_), .b(new_n51_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n62_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n62_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(x08), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n61_), .O(z07));
  inv1   g30(.a(z03), .O(z08));
  inv1   g31(.a(x15), .O(new_n77_));
  inv1   g32(.a(new_n58_), .O(new_n78_));
  nand4  g33(.a(new_n62_), .b(new_n78_), .c(new_n77_), .d(new_n48_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  nand3  g35(.a(new_n58_), .b(new_n77_), .c(new_n48_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x17), .c(new_n62_), .O(z10));
  and2   g37(.a(x18), .b(x17), .O(new_n83_));
  or2    g38(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n81_), .c(new_n62_), .O(z11));
  inv1   g40(.a(new_n81_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n62_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n83_), .b(x19), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n89_), .c(new_n87_), .O(z12));
  inv1   g46(.a(new_n87_), .O(new_n92_));
  nor2   g47(.a(new_n88_), .b(x20), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n88_), .b(x20), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z13));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  nand2  g53(.a(new_n93_), .b(new_n98_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n92_), .O(z14));
  nand2  g55(.a(new_n99_), .b(x22), .O(new_n101_));
  nor2   g56(.a(x22), .b(x21), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n92_), .O(z15));
  nand2  g59(.a(new_n103_), .b(x23), .O(new_n105_));
  inv1   g60(.a(x20), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  inv1   g62(.a(x23), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n98_), .d(new_n106_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n89_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n105_), .c(new_n92_), .O(z16));
  nand3  g67(.a(new_n110_), .b(new_n89_), .c(new_n49_), .O(new_n113_));
  nand2  g68(.a(new_n111_), .b(x24), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n92_), .O(z17));
  oai21  g70(.a(new_n109_), .b(new_n88_), .c(x25), .O(new_n116_));
  nand4  g71(.a(new_n102_), .b(new_n93_), .c(new_n56_), .d(new_n49_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n46_), .O(new_n119_));
  aoi22  g74(.a(new_n81_), .b(new_n62_), .c(x25), .d(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(z18));
endmodule


