// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n79_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n117_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n48_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n48_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n48_), .O(z04));
  nand2  g26(.a(new_n48_), .b(new_n46_), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(x13), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n72_), .b(new_n74_), .O(z06));
  inv1   g30(.a(new_n48_), .O(new_n76_));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n76_), .O(z07));
  aoi21  g32(.a(new_n58_), .b(new_n50_), .c(new_n76_), .O(z08));
  nand2  g33(.a(x05), .b(x04), .O(new_n79_));
  nor4   g34(.a(new_n79_), .b(x24), .c(x15), .d(x07), .O(z09));
  inv1   g35(.a(x24), .O(new_n81_));
  nand2  g36(.a(new_n60_), .b(new_n52_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n81_), .c(x15), .O(z10));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n84_));
  nand2  g39(.a(x18), .b(x17), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n60_), .c(new_n84_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n81_), .c(x15), .O(z11));
  nand2  g42(.a(new_n85_), .b(new_n54_), .O(new_n88_));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n60_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n81_), .c(x15), .O(z12));
  inv1   g46(.a(new_n89_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n51_), .O(new_n93_));
  inv1   g48(.a(x07), .O(new_n94_));
  nand4  g49(.a(new_n79_), .b(new_n81_), .c(new_n47_), .d(new_n94_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n89_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(z13));
  nand2  g53(.a(new_n93_), .b(x21), .O(new_n99_));
  inv1   g54(.a(x21), .O(new_n100_));
  nor2   g55(.a(new_n89_), .b(x20), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n99_), .c(new_n60_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n81_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(z14));
  inv1   g60(.a(x22), .O(new_n106_));
  aoi21  g61(.a(new_n101_), .b(new_n100_), .c(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n106_), .c(new_n100_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n60_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n107_), .c(new_n81_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(z15));
  nand2  g66(.a(new_n108_), .b(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  nand4  g68(.a(new_n101_), .b(new_n113_), .c(new_n106_), .d(new_n100_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n96_), .O(z16));
  nand2  g70(.a(new_n114_), .b(new_n96_), .O(z17));
  aoi21  g71(.a(new_n114_), .b(x25), .c(new_n95_), .O(new_n117_));
  oai21  g72(.a(new_n114_), .b(x25), .c(new_n117_), .O(z18));
endmodule


