// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x13), .b(x12), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n47_), .c(new_n60_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z02));
  nand3  g20(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  nor2   g22(.a(x08), .b(x03), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x13), .c(x12), .O(new_n69_));
  inv1   g24(.a(x08), .O(new_n70_));
  inv1   g25(.a(x09), .O(new_n71_));
  inv1   g26(.a(x02), .O(new_n72_));
  aoi22  g27(.a(x11), .b(new_n72_), .c(new_n71_), .d(x00), .O(new_n73_));
  oai21  g28(.a(new_n71_), .b(x00), .c(new_n73_), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  inv1   g30(.a(x11), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x02), .O(new_n77_));
  inv1   g32(.a(x12), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x03), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n74_), .c(new_n70_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n69_), .O(z04));
  nor2   g37(.a(x13), .b(x08), .O(z05));
  nand3  g38(.a(new_n47_), .b(x14), .c(new_n70_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z06));
  nand3  g40(.a(new_n47_), .b(new_n70_), .c(x06), .O(z07));
  inv1   g41(.a(new_n47_), .O(new_n87_));
  aoi21  g42(.a(new_n58_), .b(new_n50_), .c(new_n87_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand3  g44(.a(new_n47_), .b(new_n89_), .c(new_n60_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(new_n61_), .O(z09));
  nand3  g46(.a(new_n61_), .b(new_n89_), .c(new_n60_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g48(.a(new_n53_), .b(new_n52_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n92_), .c(new_n47_), .O(z11));
  nand2  g52(.a(new_n95_), .b(new_n54_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n92_), .c(new_n47_), .O(z12));
  inv1   g56(.a(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n51_), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(x20), .c(new_n92_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n87_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nor3   g61(.a(new_n99_), .b(x21), .c(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  inv1   g63(.a(new_n90_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n61_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n108_), .c(new_n106_), .O(z14));
  nand2  g67(.a(new_n108_), .b(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand2  g69(.a(new_n107_), .b(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n107_), .c(new_n92_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n117_), .c(new_n87_), .O(z16));
  nand2  g75(.a(new_n118_), .b(new_n107_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x24), .O(new_n122_));
  inv1   g77(.a(new_n92_), .O(new_n123_));
  inv1   g78(.a(x24), .O(new_n124_));
  nor2   g79(.a(x21), .b(x20), .O(new_n125_));
  nand4  g80(.a(new_n118_), .b(new_n125_), .c(new_n102_), .d(new_n124_), .O(new_n126_));
  and2   g81(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n122_), .c(new_n87_), .O(z17));
  aoi21  g83(.a(new_n126_), .b(x25), .c(new_n110_), .O(new_n129_));
  oai21  g84(.a(new_n126_), .b(x25), .c(new_n129_), .O(z18));
endmodule


