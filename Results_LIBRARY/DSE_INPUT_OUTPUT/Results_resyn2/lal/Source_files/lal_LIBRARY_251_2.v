// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x25), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  inv1   g14(.a(x24), .O(new_n60_));
  oai21  g15(.a(new_n58_), .b(x23), .c(new_n60_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  oai21  g18(.a(new_n59_), .b(new_n57_), .c(new_n63_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  oai21  g21(.a(new_n59_), .b(new_n57_), .c(new_n61_), .O(z03));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z06));
  nand3  g32(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g33(.a(new_n57_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n58_), .c(new_n47_), .O(z08));
  nand2  g35(.a(x05), .b(x04), .O(new_n81_));
  inv1   g36(.a(x07), .O(new_n82_));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n81_), .c(new_n49_), .O(z09));
  nand3  g40(.a(new_n81_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n48_), .c(x17), .O(z10));
  nand2  g42(.a(new_n53_), .b(new_n52_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n86_), .c(new_n49_), .O(z11));
  inv1   g46(.a(new_n86_), .O(new_n92_));
  inv1   g47(.a(new_n89_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x19), .O(new_n94_));
  nand2  g49(.a(new_n89_), .b(new_n54_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n49_), .O(z12));
  nand2  g52(.a(new_n94_), .b(x20), .O(new_n98_));
  nand4  g53(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n86_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n98_), .c(new_n48_), .O(z13));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  inv1   g58(.a(x21), .O(new_n104_));
  aoi21  g59(.a(new_n100_), .b(new_n104_), .c(new_n86_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n103_), .c(new_n48_), .O(z14));
  oai21  g61(.a(new_n99_), .b(x21), .c(x22), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n100_), .c(new_n86_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n107_), .c(new_n48_), .O(z15));
  nand4  g65(.a(new_n108_), .b(new_n93_), .c(new_n51_), .d(x19), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n92_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n47_), .O(new_n113_));
  nor2   g68(.a(new_n60_), .b(new_n47_), .O(new_n114_));
  oai21  g69(.a(new_n111_), .b(new_n86_), .c(new_n114_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n113_), .O(z16));
  nand2  g71(.a(new_n111_), .b(x24), .O(new_n117_));
  nand3  g72(.a(new_n108_), .b(new_n100_), .c(new_n60_), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n117_), .c(new_n92_), .d(new_n47_), .O(z17));
  nand2  g74(.a(new_n118_), .b(x25), .O(new_n120_));
  nand4  g75(.a(new_n108_), .b(new_n100_), .c(new_n58_), .d(new_n60_), .O(new_n121_));
  nand2  g76(.a(new_n58_), .b(x24), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(x23), .c(new_n86_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(z18));
endmodule


