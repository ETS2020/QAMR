// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n93_, new_n94_, new_n96_, new_n98_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x17), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x07), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x19), .b(x18), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n47_), .c(new_n58_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n48_), .b(new_n63_), .O(z02));
  inv1   g19(.a(x23), .O(new_n65_));
  nor2   g20(.a(x19), .b(x18), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n47_), .c(x20), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(new_n54_), .c(new_n65_), .O(new_n68_));
  inv1   g23(.a(new_n48_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n51_), .O(new_n70_));
  aoi21  g25(.a(new_n68_), .b(x24), .c(new_n70_), .O(z03));
  nand2  g26(.a(new_n69_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n69_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor2   g36(.a(new_n72_), .b(new_n81_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g38(.a(x18), .O(new_n84_));
  inv1   g39(.a(x19), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n52_), .c(new_n54_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x23), .c(x24), .O(new_n88_));
  nand3  g43(.a(new_n51_), .b(new_n65_), .c(new_n52_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x07), .c(new_n47_), .O(new_n90_));
  aoi21  g45(.a(new_n88_), .b(new_n51_), .c(new_n90_), .O(z08));
  nor4   g46(.a(new_n59_), .b(x17), .c(x15), .d(x07), .O(z09));
  inv1   g47(.a(x15), .O(new_n93_));
  nand2  g48(.a(new_n61_), .b(new_n93_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nand3  g50(.a(new_n61_), .b(x18), .c(new_n93_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nand3  g52(.a(new_n61_), .b(x19), .c(new_n93_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z12));
  nand2  g54(.a(z10), .b(new_n52_), .O(z13));
  inv1   g55(.a(x21), .O(new_n101_));
  nand3  g56(.a(new_n59_), .b(new_n101_), .c(new_n93_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n58_), .O(z14));
  inv1   g59(.a(x22), .O(new_n105_));
  nand3  g60(.a(new_n59_), .b(new_n105_), .c(new_n93_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n58_), .O(z15));
  nand3  g63(.a(new_n59_), .b(new_n65_), .c(new_n93_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n58_), .O(z16));
  nand2  g66(.a(z10), .b(new_n50_), .O(z17));
  nand2  g67(.a(z10), .b(new_n51_), .O(z18));
endmodule


