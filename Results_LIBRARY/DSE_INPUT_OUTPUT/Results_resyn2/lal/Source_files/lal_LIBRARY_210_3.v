// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(z02), .O(z00));
  inv1   g07(.a(new_n49_), .O(new_n53_));
  inv1   g08(.a(x07), .O(new_n54_));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g18(.a(x22), .b(x21), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n47_), .c(new_n48_), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x25), .c(new_n57_), .O(z01));
  inv1   g23(.a(x25), .O(new_n69_));
  nand3  g24(.a(new_n65_), .b(new_n63_), .c(x24), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(new_n49_), .O(z08));
  inv1   g26(.a(z08), .O(z03));
  inv1   g27(.a(x08), .O(new_n73_));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x09), .b(x00), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n73_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n53_), .O(z04));
  inv1   g35(.a(x13), .O(new_n81_));
  nand2  g36(.a(new_n51_), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z05));
  nand2  g38(.a(new_n51_), .b(x14), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z06));
  aoi21  g40(.a(new_n73_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n54_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n55_), .c(new_n53_), .O(z09));
  nand3  g44(.a(new_n55_), .b(new_n87_), .c(new_n54_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n53_), .O(z10));
  nor2   g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n90_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n53_), .O(new_n94_));
  nor2   g49(.a(new_n60_), .b(new_n59_), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nor2   g53(.a(new_n95_), .b(x19), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n94_), .O(z12));
  inv1   g55(.a(new_n94_), .O(new_n101_));
  nor2   g56(.a(new_n97_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n97_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n101_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  inv1   g65(.a(x22), .O(new_n111_));
  nand3  g66(.a(new_n102_), .b(new_n111_), .c(new_n107_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n101_), .O(z15));
  nand4  g68(.a(new_n102_), .b(new_n48_), .c(new_n111_), .d(new_n107_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x23), .O(new_n115_));
  or2    g70(.a(new_n112_), .b(x23), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n93_), .O(z16));
  nand2  g72(.a(new_n90_), .b(new_n53_), .O(new_n118_));
  xor2a  g73(.a(new_n112_), .b(x24), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x23), .c(new_n118_), .O(z17));
  oai21  g75(.a(new_n112_), .b(x23), .c(new_n69_), .O(new_n121_));
  nor2   g76(.a(new_n69_), .b(x23), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(x24), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  oai21  g79(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  aoi22  g80(.a(new_n125_), .b(new_n90_), .c(new_n122_), .d(new_n114_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n124_), .O(z18));
endmodule


