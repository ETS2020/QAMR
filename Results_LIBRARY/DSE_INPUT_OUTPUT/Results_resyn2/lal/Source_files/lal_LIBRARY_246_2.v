// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n91_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n142_, new_n143_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x22), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x24), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n56_), .c(new_n57_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n55_), .c(x22), .O(new_n63_));
  aoi21  g18(.a(x24), .b(x23), .c(x25), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n54_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(new_n65_), .O(z03));
  inv1   g24(.a(x12), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x03), .O(new_n71_));
  inv1   g26(.a(x02), .O(new_n72_));
  nand2  g27(.a(x11), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(x03), .O(new_n74_));
  nand2  g29(.a(x12), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x11), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x02), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n75_), .c(new_n73_), .d(new_n71_), .O(new_n78_));
  inv1   g33(.a(x00), .O(new_n79_));
  nand2  g34(.a(x09), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(x01), .O(new_n81_));
  nand2  g36(.a(x10), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x09), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x00), .O(new_n84_));
  inv1   g39(.a(x10), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x01), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n84_), .c(new_n82_), .d(new_n80_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n78_), .c(new_n46_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n50_), .O(z04));
  nor3   g44(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g45(.a(x14), .b(new_n46_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(z06));
  aoi21  g47(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g48(.a(x25), .O(new_n94_));
  inv1   g49(.a(x23), .O(new_n95_));
  nand2  g50(.a(new_n94_), .b(new_n95_), .O(new_n96_));
  aoi21  g51(.a(new_n62_), .b(x22), .c(new_n96_), .O(new_n97_));
  oai22  g52(.a(new_n97_), .b(new_n55_), .c(new_n94_), .d(x22), .O(z08));
  inv1   g53(.a(x15), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n51_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n52_), .c(new_n50_), .O(z09));
  nand3  g56(.a(new_n52_), .b(new_n99_), .c(new_n51_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n50_), .c(new_n58_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z10));
  nand2  g60(.a(new_n59_), .b(new_n58_), .O(new_n106_));
  nand2  g61(.a(x18), .b(x17), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n102_), .c(new_n50_), .O(z11));
  nand3  g64(.a(x19), .b(x18), .c(x17), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(new_n60_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n103_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n50_), .O(z12));
  inv1   g68(.a(new_n110_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n56_), .O(new_n115_));
  nand2  g70(.a(new_n110_), .b(x20), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n115_), .c(new_n103_), .d(new_n50_), .O(z13));
  nand2  g72(.a(new_n115_), .b(x21), .O(new_n118_));
  nor2   g73(.a(new_n110_), .b(x20), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n57_), .c(new_n102_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(z14));
  nand2  g76(.a(new_n119_), .b(new_n57_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(x24), .c(x22), .O(new_n123_));
  nand2  g78(.a(new_n102_), .b(new_n50_), .O(new_n124_));
  inv1   g79(.a(new_n122_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n47_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z15));
  aoi21  g82(.a(new_n119_), .b(new_n57_), .c(new_n95_), .O(new_n128_));
  nand2  g83(.a(new_n95_), .b(new_n57_), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n115_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n128_), .c(new_n47_), .O(new_n131_));
  nor2   g86(.a(new_n95_), .b(new_n47_), .O(new_n132_));
  oai21  g87(.a(new_n132_), .b(new_n102_), .c(new_n50_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n131_), .O(z16));
  nand4  g89(.a(new_n119_), .b(new_n95_), .c(new_n47_), .d(new_n57_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(x24), .O(new_n136_));
  nand4  g91(.a(new_n125_), .b(new_n55_), .c(new_n95_), .d(new_n47_), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n136_), .c(new_n124_), .O(z17));
  nand2  g93(.a(new_n94_), .b(new_n55_), .O(new_n139_));
  nand4  g94(.a(new_n139_), .b(new_n119_), .c(new_n95_), .d(new_n57_), .O(new_n140_));
  oai21  g95(.a(new_n129_), .b(new_n115_), .c(new_n94_), .O(new_n141_));
  nand3  g96(.a(new_n141_), .b(new_n140_), .c(new_n47_), .O(new_n142_));
  nand2  g97(.a(x25), .b(x24), .O(new_n143_));
  nand3  g98(.a(new_n143_), .b(new_n142_), .c(new_n124_), .O(z18));
endmodule


