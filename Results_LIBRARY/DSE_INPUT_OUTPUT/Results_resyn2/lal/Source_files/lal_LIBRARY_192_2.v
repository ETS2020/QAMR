// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n77_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nor3   g05(.a(x19), .b(x18), .c(x17), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  oai21  g09(.a(new_n51_), .b(x20), .c(new_n54_), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  aoi21  g13(.a(x25), .b(new_n58_), .c(x24), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n60_), .c(new_n57_), .d(new_n50_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n57_), .O(z03));
  nand2  g20(.a(new_n48_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  inv1   g31(.a(new_n48_), .O(new_n77_));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n77_), .O(z07));
  aoi21  g33(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(z08));
  nor2   g34(.a(x15), .b(x07), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n48_), .c(x05), .d(x04), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand2  g37(.a(new_n80_), .b(new_n61_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g39(.a(x18), .b(x17), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n83_), .c(new_n48_), .O(z11));
  nand3  g41(.a(new_n80_), .b(new_n61_), .c(new_n48_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  aoi21  g44(.a(x18), .b(x17), .c(x19), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n89_), .c(new_n87_), .O(z12));
  inv1   g46(.a(x20), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(new_n92_), .O(new_n93_));
  aoi21  g48(.a(new_n88_), .b(x20), .c(new_n83_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n93_), .c(new_n77_), .O(z13));
  inv1   g50(.a(new_n87_), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(x21), .O(new_n97_));
  nand3  g52(.a(new_n89_), .b(new_n52_), .c(new_n92_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(z14));
  nand2  g54(.a(new_n98_), .b(x22), .O(new_n100_));
  nand2  g55(.a(new_n52_), .b(new_n92_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n88_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n53_), .c(new_n83_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n100_), .c(new_n77_), .O(z15));
  inv1   g59(.a(new_n83_), .O(new_n105_));
  nand3  g60(.a(new_n102_), .b(x24), .c(new_n53_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x23), .O(new_n107_));
  nand3  g62(.a(new_n102_), .b(new_n58_), .c(new_n53_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n105_), .O(z16));
  nand2  g64(.a(new_n83_), .b(new_n48_), .O(new_n110_));
  nand4  g65(.a(new_n102_), .b(new_n47_), .c(new_n58_), .d(new_n53_), .O(new_n111_));
  nand2  g66(.a(new_n108_), .b(x24), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z17));
  inv1   g68(.a(x25), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x24), .O(new_n115_));
  nand2  g70(.a(x25), .b(new_n47_), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n115_), .c(new_n102_), .d(new_n53_), .O(new_n117_));
  oai21  g72(.a(new_n98_), .b(x22), .c(x25), .O(new_n118_));
  aoi21  g73(.a(new_n115_), .b(x23), .c(new_n83_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(z18));
endmodule


