// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x09), .b(x05), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  oai21  g15(.a(x09), .b(x04), .c(x05), .O(new_n61_));
  and2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(new_n47_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n46_), .O(z02));
  nand3  g20(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  nor2   g22(.a(x08), .b(x00), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x05), .c(x09), .O(new_n69_));
  inv1   g24(.a(x08), .O(new_n70_));
  inv1   g25(.a(x09), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x00), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  inv1   g28(.a(x10), .O(new_n74_));
  aoi22  g29(.a(x12), .b(new_n73_), .c(new_n74_), .d(x01), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  inv1   g31(.a(x01), .O(new_n77_));
  nand2  g32(.a(x10), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(x12), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x03), .O(new_n80_));
  xnor2a g35(.a(x11), .b(x02), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n76_), .c(new_n70_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n69_), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g40(.a(new_n47_), .b(x14), .c(new_n70_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z06));
  nand3  g42(.a(new_n47_), .b(new_n70_), .c(x06), .O(z07));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n71_), .b(x05), .c(x04), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n90_), .O(z09));
  nand2  g47(.a(new_n89_), .b(new_n61_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n52_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nand2  g51(.a(new_n53_), .b(new_n52_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n94_), .c(new_n97_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n102_), .c(new_n93_), .O(z12));
  nand2  g59(.a(new_n101_), .b(x20), .O(new_n105_));
  nor2   g60(.a(new_n101_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n94_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  nor3   g64(.a(new_n101_), .b(x21), .c(x20), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n94_), .O(z14));
  inv1   g67(.a(x22), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(new_n113_), .O(new_n114_));
  nand2  g69(.a(new_n111_), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n94_), .O(z15));
  nand2  g71(.a(new_n114_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand2  g73(.a(new_n91_), .b(new_n89_), .O(new_n119_));
  aoi21  g74(.a(new_n118_), .b(new_n110_), .c(new_n119_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n117_), .c(new_n64_), .O(z16));
  nand2  g76(.a(new_n118_), .b(new_n110_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x24), .O(new_n123_));
  nor2   g78(.a(x24), .b(x21), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n118_), .c(new_n106_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n94_), .O(z17));
  nand4  g81(.a(new_n124_), .b(new_n118_), .c(new_n106_), .d(new_n50_), .O(new_n127_));
  aoi21  g82(.a(new_n125_), .b(x25), .c(new_n119_), .O(new_n128_));
  aoi21  g83(.a(new_n128_), .b(new_n127_), .c(new_n64_), .O(z18));
endmodule


