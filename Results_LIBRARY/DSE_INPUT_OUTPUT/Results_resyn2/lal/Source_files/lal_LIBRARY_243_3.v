// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x02), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(z02), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(z01));
  aoi21  g19(.a(new_n61_), .b(new_n53_), .c(new_n49_), .O(z08));
  inv1   g20(.a(z08), .O(z03));
  nor2   g21(.a(x11), .b(x08), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(x15), .c(x02), .O(new_n68_));
  inv1   g23(.a(x09), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(x00), .O(new_n70_));
  inv1   g25(.a(x00), .O(new_n71_));
  inv1   g26(.a(x03), .O(new_n72_));
  oai22  g27(.a(x12), .b(new_n72_), .c(x09), .d(new_n71_), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand2  g29(.a(x12), .b(new_n72_), .O(new_n75_));
  nand2  g30(.a(x11), .b(new_n47_), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nor3   g32(.a(new_n77_), .b(new_n73_), .c(new_n70_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x08), .c(new_n68_), .O(z04));
  inv1   g34(.a(x13), .O(new_n80_));
  nand2  g35(.a(new_n51_), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(new_n51_), .b(x14), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z06));
  nand2  g39(.a(new_n51_), .b(x06), .O(z07));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n48_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  inv1   g43(.a(new_n49_), .O(new_n89_));
  nand2  g44(.a(new_n63_), .b(new_n48_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n89_), .O(z10));
  nand2  g46(.a(new_n56_), .b(new_n55_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n89_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(new_n57_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n90_), .c(new_n89_), .O(z12));
  inv1   g54(.a(new_n90_), .O(new_n100_));
  nor2   g55(.a(new_n96_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n96_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z13));
  nand2  g59(.a(x15), .b(new_n47_), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nor2   g61(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n101_), .b(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n63_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n107_), .c(new_n48_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n105_), .O(z14));
  inv1   g66(.a(x22), .O(new_n112_));
  aoi21  g67(.a(new_n101_), .b(new_n106_), .c(new_n112_), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n101_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n63_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n113_), .c(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n105_), .O(z15));
  nand2  g73(.a(new_n115_), .b(x23), .O(new_n119_));
  inv1   g74(.a(x23), .O(new_n120_));
  nand3  g75(.a(new_n114_), .b(new_n101_), .c(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n100_), .O(z16));
  nand2  g77(.a(new_n121_), .b(x24), .O(new_n123_));
  nor2   g78(.a(x24), .b(x23), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n114_), .c(new_n101_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n100_), .O(z17));
  or2    g81(.a(new_n125_), .b(x25), .O(new_n127_));
  aoi21  g82(.a(new_n125_), .b(x25), .c(new_n90_), .O(new_n128_));
  aoi21  g83(.a(new_n128_), .b(new_n127_), .c(new_n49_), .O(z18));
endmodule


