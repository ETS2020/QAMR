// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x18), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  nand3  g08(.a(x22), .b(x21), .c(x20), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(x18), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(x24), .c(x25), .O(z03));
  inv1   g11(.a(x07), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(new_n49_), .O(new_n59_));
  or2    g14(.a(new_n59_), .b(z03), .O(z01));
  nand2  g15(.a(new_n50_), .b(new_n46_), .O(new_n61_));
  xor2a  g16(.a(x12), .b(x03), .O(new_n62_));
  xor2a  g17(.a(x11), .b(x02), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  xor2a  g19(.a(x09), .b(x00), .O(new_n65_));
  xor2a  g20(.a(x10), .b(x01), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n64_), .c(new_n61_), .O(z04));
  nor2   g23(.a(new_n61_), .b(x13), .O(z05));
  inv1   g24(.a(x14), .O(new_n70_));
  nor2   g25(.a(new_n61_), .b(new_n70_), .O(z06));
  aoi21  g26(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g27(.a(x25), .O(new_n73_));
  nand2  g28(.a(new_n54_), .b(new_n53_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x24), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n73_), .c(new_n49_), .O(z08));
  inv1   g31(.a(x15), .O(new_n77_));
  nor2   g32(.a(new_n58_), .b(new_n49_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n57_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  nand3  g35(.a(new_n58_), .b(new_n77_), .c(new_n57_), .O(new_n81_));
  nor3   g36(.a(new_n81_), .b(new_n49_), .c(x17), .O(z10));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(x18), .b(x17), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n50_), .O(new_n85_));
  nor3   g40(.a(new_n85_), .b(new_n83_), .c(new_n81_), .O(z11));
  inv1   g41(.a(new_n81_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(x19), .O(new_n89_));
  nand2  g44(.a(new_n84_), .b(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(z12));
  oai21  g47(.a(new_n49_), .b(x20), .c(new_n88_), .O(new_n93_));
  inv1   g48(.a(x20), .O(new_n94_));
  and2   g49(.a(x19), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x18), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n93_), .c(new_n87_), .O(z13));
  nand2  g54(.a(new_n96_), .b(x21), .O(new_n100_));
  inv1   g55(.a(x21), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n94_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n88_), .O(new_n103_));
  aoi21  g58(.a(new_n48_), .b(new_n101_), .c(x18), .O(new_n104_));
  nor3   g59(.a(new_n104_), .b(new_n103_), .c(new_n81_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n100_), .O(z14));
  inv1   g61(.a(x22), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n95_), .c(new_n107_), .O(new_n109_));
  nand3  g64(.a(new_n107_), .b(x19), .c(x17), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n102_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n109_), .c(x18), .O(new_n112_));
  inv1   g67(.a(x18), .O(new_n113_));
  aoi21  g68(.a(x22), .b(new_n113_), .c(new_n81_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n112_), .c(new_n49_), .O(z15));
  oai21  g70(.a(new_n110_), .b(new_n102_), .c(x23), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n103_), .O(new_n118_));
  nand2  g73(.a(new_n48_), .b(new_n53_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n113_), .c(new_n81_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n116_), .O(z16));
  nand2  g76(.a(new_n118_), .b(x24), .O(new_n122_));
  nand3  g77(.a(new_n117_), .b(new_n48_), .c(new_n101_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(new_n96_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x18), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n122_), .c(new_n87_), .O(z17));
  nand3  g81(.a(new_n124_), .b(new_n73_), .c(x18), .O(new_n127_));
  oai21  g82(.a(new_n123_), .b(new_n96_), .c(x25), .O(new_n128_));
  nand2  g83(.a(new_n73_), .b(new_n48_), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(new_n113_), .c(new_n81_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n128_), .c(new_n127_), .O(z18));
endmodule


