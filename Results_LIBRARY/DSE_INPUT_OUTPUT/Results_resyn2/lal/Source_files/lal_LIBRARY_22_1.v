// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x09), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(x09), .c(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x07), .c(new_n48_), .O(new_n63_));
  oai21  g18(.a(new_n60_), .b(x25), .c(new_n63_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(x25), .O(new_n67_));
  oai21  g22(.a(new_n59_), .b(new_n51_), .c(new_n67_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n48_), .O(z03));
  inv1   g24(.a(x12), .O(new_n70_));
  inv1   g25(.a(x10), .O(new_n71_));
  aoi22  g26(.a(new_n70_), .b(x03), .c(new_n71_), .d(x01), .O(new_n72_));
  oai21  g27(.a(new_n70_), .b(x03), .c(new_n72_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  inv1   g29(.a(x00), .O(new_n75_));
  aoi21  g30(.a(new_n51_), .b(new_n75_), .c(x09), .O(new_n76_));
  inv1   g31(.a(x02), .O(new_n77_));
  oai22  g32(.a(x11), .b(new_n77_), .c(new_n47_), .d(x00), .O(new_n78_));
  inv1   g33(.a(x11), .O(new_n79_));
  oai22  g34(.a(new_n79_), .b(x02), .c(new_n71_), .d(x01), .O(new_n80_));
  nor3   g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  aoi22  g36(.a(new_n81_), .b(new_n74_), .c(new_n48_), .d(x08), .O(z04));
  oai21  g37(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g38(.a(x14), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n49_), .O(z06));
  aoi21  g40(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g41(.a(x23), .O(new_n87_));
  nand2  g42(.a(new_n58_), .b(new_n57_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(x24), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n67_), .c(new_n49_), .O(z08));
  nor2   g46(.a(x15), .b(x07), .O(new_n92_));
  and2   g47(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n62_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand2  g50(.a(new_n93_), .b(new_n61_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n53_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  nand2  g54(.a(new_n92_), .b(new_n61_), .O(new_n100_));
  xnor2a g55(.a(x18), .b(x17), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n100_), .c(new_n48_), .O(z11));
  inv1   g57(.a(new_n100_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  oai21  g59(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(z12));
  nor2   g62(.a(new_n104_), .b(x20), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand2  g64(.a(new_n104_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n97_), .O(z13));
  nand2  g66(.a(new_n109_), .b(x21), .O(new_n112_));
  inv1   g67(.a(x21), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n97_), .O(z14));
  nand2  g70(.a(new_n114_), .b(x22), .O(new_n116_));
  nor2   g71(.a(x22), .b(x21), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n108_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n97_), .O(z15));
  nand2  g74(.a(new_n118_), .b(x23), .O(new_n120_));
  nand3  g75(.a(new_n117_), .b(new_n108_), .c(new_n87_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n100_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n120_), .c(new_n49_), .O(z16));
  nand2  g79(.a(new_n122_), .b(new_n51_), .O(new_n125_));
  nand2  g80(.a(new_n121_), .b(x24), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n97_), .O(z17));
  nor2   g82(.a(new_n67_), .b(new_n51_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n100_), .c(new_n48_), .O(new_n129_));
  nor2   g84(.a(new_n121_), .b(new_n67_), .O(new_n130_));
  oai21  g85(.a(new_n122_), .b(x25), .c(new_n51_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(z18));
endmodule


