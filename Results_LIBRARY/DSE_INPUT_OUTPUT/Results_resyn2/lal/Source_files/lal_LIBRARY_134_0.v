// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n140_, new_n141_,
    new_n142_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  nand3  g10(.a(new_n48_), .b(new_n55_), .c(new_n54_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  aoi21  g15(.a(x25), .b(x19), .c(x24), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  oai21  g20(.a(new_n60_), .b(new_n58_), .c(new_n65_), .O(z01));
  inv1   g21(.a(new_n61_), .O(new_n67_));
  oai21  g22(.a(new_n60_), .b(new_n58_), .c(new_n67_), .O(z03));
  nand2  g23(.a(new_n50_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z06));
  aoi22  g34(.a(new_n49_), .b(new_n48_), .c(new_n46_), .d(x06), .O(z07));
  nor2   g35(.a(x18), .b(x17), .O(new_n81_));
  inv1   g36(.a(new_n57_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n81_), .c(new_n49_), .O(new_n83_));
  inv1   g38(.a(x23), .O(new_n84_));
  nand3  g39(.a(x22), .b(x21), .c(x20), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x24), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n83_), .b(x19), .c(new_n87_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n62_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n63_), .c(new_n50_), .O(z09));
  nand3  g46(.a(new_n63_), .b(new_n89_), .c(new_n62_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n50_), .O(z10));
  inv1   g48(.a(new_n81_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n92_), .c(new_n50_), .O(z11));
  nand4  g52(.a(new_n63_), .b(new_n50_), .c(new_n89_), .d(new_n62_), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(new_n48_), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n98_), .O(z12));
  nand2  g57(.a(new_n95_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n53_), .b(x18), .c(x17), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x19), .O(new_n106_));
  nor2   g61(.a(new_n53_), .b(x19), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n92_), .c(new_n50_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n106_), .O(z13));
  inv1   g64(.a(new_n92_), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(x21), .O(new_n111_));
  inv1   g66(.a(new_n100_), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  oai21  g69(.a(new_n49_), .b(x21), .c(new_n48_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n114_), .c(new_n111_), .d(new_n110_), .O(z14));
  nand3  g71(.a(new_n113_), .b(x18), .c(x17), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x22), .O(new_n118_));
  inv1   g73(.a(x22), .O(new_n119_));
  inv1   g74(.a(new_n95_), .O(new_n120_));
  nand4  g75(.a(new_n113_), .b(new_n120_), .c(new_n119_), .d(x19), .O(new_n121_));
  oai21  g76(.a(new_n49_), .b(x22), .c(new_n48_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n121_), .c(new_n118_), .d(new_n110_), .O(z15));
  nor2   g78(.a(x23), .b(x22), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n113_), .c(new_n112_), .O(new_n125_));
  nand2  g80(.a(new_n119_), .b(x19), .O(new_n126_));
  oai21  g81(.a(x24), .b(x19), .c(new_n84_), .O(new_n127_));
  oai21  g82(.a(new_n126_), .b(new_n117_), .c(new_n127_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n125_), .c(new_n110_), .O(z16));
  nand2  g84(.a(new_n92_), .b(new_n50_), .O(new_n130_));
  nand2  g85(.a(new_n125_), .b(x24), .O(new_n131_));
  nor2   g86(.a(new_n95_), .b(x20), .O(new_n132_));
  nor2   g87(.a(x24), .b(x22), .O(new_n133_));
  nor2   g88(.a(x23), .b(x21), .O(new_n134_));
  nand4  g89(.a(new_n134_), .b(new_n133_), .c(new_n132_), .d(x19), .O(new_n135_));
  nand3  g90(.a(new_n135_), .b(new_n131_), .c(new_n130_), .O(z17));
  nand2  g91(.a(new_n134_), .b(new_n133_), .O(new_n137_));
  oai21  g92(.a(new_n137_), .b(new_n104_), .c(x25), .O(new_n138_));
  nand4  g93(.a(new_n133_), .b(new_n113_), .c(new_n120_), .d(new_n59_), .O(new_n139_));
  inv1   g94(.a(x25), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(x24), .O(new_n141_));
  aoi21  g96(.a(new_n141_), .b(new_n48_), .c(new_n92_), .O(new_n142_));
  nand3  g97(.a(new_n142_), .b(new_n139_), .c(new_n138_), .O(z18));
endmodule


