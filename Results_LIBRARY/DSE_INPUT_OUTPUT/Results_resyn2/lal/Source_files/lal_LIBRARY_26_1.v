// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n66_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x23), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(z01));
  inv1   g09(.a(x16), .O(new_n55_));
  nor2   g10(.a(new_n48_), .b(new_n55_), .O(z02));
  nand2  g11(.a(new_n52_), .b(new_n49_), .O(z03));
  xnor2a g12(.a(x11), .b(x02), .O(new_n58_));
  xnor2a g13(.a(x12), .b(x03), .O(new_n59_));
  xnor2a g14(.a(x09), .b(x00), .O(new_n60_));
  xnor2a g15(.a(x10), .b(x01), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n49_), .O(z04));
  oai21  g19(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g20(.a(x14), .b(new_n46_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n49_), .O(z06));
  nand3  g22(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g23(.a(z03), .O(z08));
  nor2   g24(.a(x15), .b(x07), .O(new_n70_));
  nand3  g25(.a(new_n70_), .b(x05), .c(x04), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z09));
  inv1   g27(.a(x17), .O(new_n73_));
  nand2  g28(.a(x05), .b(x04), .O(new_n74_));
  nand2  g29(.a(new_n70_), .b(new_n74_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z10));
  and2   g33(.a(x18), .b(x17), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(new_n80_));
  inv1   g35(.a(x18), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n73_), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n80_), .c(new_n76_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z11));
  inv1   g39(.a(new_n75_), .O(new_n85_));
  nand3  g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  inv1   g41(.a(x19), .O(new_n87_));
  nand2  g42(.a(new_n80_), .b(new_n87_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n49_), .O(z12));
  nand2  g45(.a(new_n86_), .b(x20), .O(new_n91_));
  inv1   g46(.a(x20), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(x19), .c(x18), .d(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n91_), .c(new_n76_), .O(z13));
  nand2  g49(.a(new_n93_), .b(x21), .O(new_n95_));
  inv1   g50(.a(x21), .O(new_n96_));
  nand4  g51(.a(new_n79_), .b(new_n96_), .c(new_n92_), .d(x19), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n95_), .c(new_n85_), .O(new_n98_));
  and2   g53(.a(new_n98_), .b(new_n49_), .O(z14));
  inv1   g54(.a(x22), .O(new_n100_));
  inv1   g55(.a(new_n93_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n96_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x22), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n76_), .O(z15));
  oai21  g59(.a(new_n97_), .b(x22), .c(x23), .O(new_n105_));
  nand2  g60(.a(new_n75_), .b(new_n49_), .O(new_n106_));
  inv1   g61(.a(x23), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n47_), .c(new_n107_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n101_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n106_), .c(new_n105_), .O(z16));
  nand2  g67(.a(x24), .b(x23), .O(new_n113_));
  inv1   g68(.a(new_n86_), .O(new_n114_));
  nor2   g69(.a(x23), .b(x20), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n108_), .c(new_n114_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n85_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n113_), .O(z17));
  nand2  g74(.a(new_n108_), .b(new_n51_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n93_), .c(new_n47_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n107_), .O(new_n122_));
  oai21  g77(.a(new_n109_), .b(new_n93_), .c(x25), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n85_), .O(z18));
endmodule


