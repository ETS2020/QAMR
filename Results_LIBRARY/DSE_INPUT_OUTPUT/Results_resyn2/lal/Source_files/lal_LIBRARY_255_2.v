// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x24), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  or2    g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(x20), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x24), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x23), .O(new_n59_));
  nor2   g14(.a(x24), .b(x15), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x25), .c(new_n54_), .O(z01));
  nor2   g17(.a(new_n61_), .b(x25), .O(z03));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x11), .b(x02), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n50_), .O(z04));
  nand2  g25(.a(new_n50_), .b(new_n46_), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x13), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor2   g28(.a(new_n71_), .b(new_n73_), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g30(.a(x25), .O(new_n76_));
  inv1   g31(.a(x23), .O(new_n77_));
  nand2  g32(.a(new_n57_), .b(new_n77_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n50_), .c(new_n76_), .O(z08));
  inv1   g35(.a(x07), .O(new_n81_));
  nand4  g36(.a(new_n48_), .b(new_n81_), .c(x05), .d(x04), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z09));
  nand2  g38(.a(new_n53_), .b(new_n48_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g40(.a(x18), .b(x17), .O(new_n86_));
  and2   g41(.a(x18), .b(x17), .O(new_n87_));
  nor3   g42(.a(new_n87_), .b(new_n84_), .c(new_n86_), .O(z11));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor2   g45(.a(new_n87_), .b(x19), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n90_), .c(new_n84_), .O(z12));
  nand2  g47(.a(x24), .b(x15), .O(new_n93_));
  inv1   g48(.a(x20), .O(new_n94_));
  nand2  g49(.a(new_n90_), .b(new_n94_), .O(new_n95_));
  nand2  g50(.a(new_n89_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n53_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n48_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n93_), .O(z13));
  nand2  g54(.a(new_n95_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n53_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n93_), .O(z14));
  inv1   g60(.a(x22), .O(new_n106_));
  aoi21  g61(.a(new_n101_), .b(new_n90_), .c(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n90_), .c(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n53_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n107_), .c(new_n48_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n93_), .O(z15));
  nor2   g66(.a(x23), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n101_), .c(new_n90_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(x23), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n53_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n48_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n93_), .O(z16));
  oai21  g72(.a(new_n113_), .b(x15), .c(x24), .O(new_n118_));
  inv1   g73(.a(new_n113_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n60_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n54_), .O(z17));
  inv1   g76(.a(new_n84_), .O(new_n122_));
  nand3  g77(.a(new_n119_), .b(new_n76_), .c(new_n58_), .O(new_n123_));
  oai21  g78(.a(new_n113_), .b(x24), .c(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(z18));
endmodule


