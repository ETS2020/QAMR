// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x05), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  nand2  g08(.a(x25), .b(x05), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g14(.a(new_n56_), .b(x20), .c(new_n59_), .O(new_n60_));
  nor2   g15(.a(x25), .b(x23), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand4  g18(.a(new_n63_), .b(new_n62_), .c(new_n55_), .d(new_n52_), .O(z01));
  nand2  g19(.a(new_n62_), .b(new_n55_), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z04));
  nor3   g27(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor3   g29(.a(new_n48_), .b(new_n74_), .c(x08), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi22  g31(.a(new_n61_), .b(new_n60_), .c(new_n54_), .d(new_n53_), .O(z08));
  nor2   g32(.a(x15), .b(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x05), .c(x04), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  or2    g35(.a(new_n78_), .b(new_n48_), .O(new_n81_));
  nand2  g36(.a(new_n49_), .b(x17), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n81_), .c(new_n63_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z10));
  or2    g39(.a(x18), .b(x17), .O(new_n85_));
  nand2  g40(.a(new_n78_), .b(new_n63_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n87_), .c(new_n85_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(x19), .O(new_n92_));
  aoi21  g47(.a(new_n88_), .b(new_n92_), .c(new_n86_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n49_), .O(z12));
  inv1   g50(.a(x20), .O(new_n96_));
  inv1   g51(.a(new_n91_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g53(.a(new_n91_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n87_), .O(z13));
  nand2  g55(.a(new_n98_), .b(x21), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n97_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n87_), .O(z14));
  aoi21  g59(.a(new_n102_), .b(new_n97_), .c(new_n58_), .O(new_n105_));
  nand2  g60(.a(new_n102_), .b(new_n58_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n91_), .c(new_n78_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n105_), .c(new_n49_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n63_), .O(z15));
  inv1   g64(.a(x23), .O(new_n110_));
  nor3   g65(.a(x22), .b(x21), .c(x20), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n97_), .c(new_n110_), .O(new_n112_));
  nand4  g67(.a(new_n110_), .b(new_n58_), .c(new_n57_), .d(new_n96_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n91_), .c(new_n78_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n112_), .c(new_n49_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n63_), .O(z16));
  inv1   g71(.a(x04), .O(new_n117_));
  nor2   g72(.a(x24), .b(x23), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n111_), .c(new_n97_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x05), .O(new_n121_));
  oai21  g76(.a(new_n113_), .b(new_n91_), .c(x24), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n81_), .O(z17));
  nand2  g78(.a(new_n119_), .b(x25), .O(new_n124_));
  inv1   g79(.a(new_n119_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n54_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n87_), .O(z18));
endmodule


