// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x18), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n51_), .c(new_n52_), .O(new_n58_));
  inv1   g13(.a(x23), .O(new_n59_));
  nand3  g14(.a(new_n52_), .b(new_n59_), .c(new_n53_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x24), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(x19), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n58_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n61_), .b(x19), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n58_), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g35(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x17), .O(new_n82_));
  inv1   g37(.a(x18), .O(new_n83_));
  nand3  g38(.a(new_n47_), .b(new_n83_), .c(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n53_), .c(new_n55_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x23), .c(x24), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n49_), .c(new_n52_), .O(z08));
  inv1   g42(.a(new_n63_), .O(new_n88_));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n49_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n63_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g48(.a(new_n83_), .b(new_n82_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n92_), .c(new_n49_), .O(z11));
  inv1   g52(.a(new_n95_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x19), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n89_), .c(new_n63_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n49_), .O(z12));
  nor2   g57(.a(new_n95_), .b(x20), .O(new_n103_));
  oai21  g58(.a(new_n103_), .b(new_n51_), .c(x19), .O(new_n104_));
  nor2   g59(.a(new_n51_), .b(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n98_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(x20), .c(new_n92_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n104_), .O(z13));
  aoi21  g63(.a(new_n89_), .b(new_n63_), .c(new_n48_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(x21), .O(new_n111_));
  inv1   g66(.a(x21), .O(new_n112_));
  nand3  g67(.a(new_n105_), .b(new_n103_), .c(new_n112_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(z14));
  aoi21  g69(.a(new_n103_), .b(new_n112_), .c(new_n51_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n47_), .c(x22), .O(new_n116_));
  nand3  g71(.a(new_n53_), .b(x18), .c(x17), .O(new_n117_));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n105_), .c(new_n109_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n116_), .O(z15));
  aoi21  g77(.a(new_n118_), .b(new_n103_), .c(new_n51_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n47_), .c(x23), .O(new_n124_));
  nand3  g79(.a(new_n118_), .b(new_n59_), .c(new_n53_), .O(new_n125_));
  nor2   g80(.a(new_n125_), .b(new_n100_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(x24), .c(new_n109_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n124_), .O(z16));
  oai21  g83(.a(new_n126_), .b(new_n51_), .c(new_n110_), .O(z17));
  oai21  g84(.a(new_n48_), .b(new_n52_), .c(new_n110_), .O(z18));
endmodule


