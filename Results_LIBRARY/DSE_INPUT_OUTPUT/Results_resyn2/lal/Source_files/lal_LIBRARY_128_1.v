// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x12), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n59_), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n48_), .c(new_n53_), .O(new_n63_));
  aoi21  g18(.a(x05), .b(x04), .c(x07), .O(new_n64_));
  and2   g19(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n63_), .O(z01));
  nand2  g21(.a(new_n63_), .b(new_n50_), .O(z03));
  inv1   g22(.a(x12), .O(new_n68_));
  inv1   g23(.a(x03), .O(new_n69_));
  oai21  g24(.a(x08), .b(new_n69_), .c(new_n48_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  inv1   g26(.a(x09), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  aoi22  g28(.a(x11), .b(new_n73_), .c(new_n72_), .d(x00), .O(new_n74_));
  oai21  g29(.a(new_n72_), .b(x00), .c(new_n74_), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n69_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n75_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n71_), .O(z04));
  nand2  g37(.a(new_n50_), .b(new_n46_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x13), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor2   g40(.a(new_n83_), .b(new_n85_), .O(z06));
  nand3  g41(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nor2   g43(.a(x18), .b(x17), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n57_), .c(x20), .O(new_n90_));
  oai21  g45(.a(new_n60_), .b(new_n90_), .c(new_n88_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n68_), .c(x24), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n53_), .O(z08));
  inv1   g48(.a(x07), .O(new_n94_));
  inv1   g49(.a(x15), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n94_), .c(x05), .d(x04), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n50_), .O(z09));
  nand2  g52(.a(new_n64_), .b(new_n95_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g54(.a(new_n56_), .b(new_n55_), .O(new_n100_));
  or2    g55(.a(new_n100_), .b(new_n89_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n98_), .c(new_n50_), .O(z11));
  nand3  g57(.a(new_n64_), .b(new_n50_), .c(new_n95_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nor2   g60(.a(new_n100_), .b(x19), .O(new_n106_));
  nor3   g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z12));
  nor2   g62(.a(new_n104_), .b(x20), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  aoi21  g64(.a(new_n104_), .b(x20), .c(new_n98_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n109_), .c(new_n49_), .O(z13));
  inv1   g66(.a(new_n103_), .O(new_n112_));
  nand2  g67(.a(new_n109_), .b(x21), .O(new_n113_));
  inv1   g68(.a(x21), .O(new_n114_));
  nand2  g69(.a(new_n108_), .b(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(z14));
  inv1   g71(.a(x22), .O(new_n117_));
  nand3  g72(.a(new_n108_), .b(new_n117_), .c(new_n114_), .O(new_n118_));
  nand2  g73(.a(new_n115_), .b(x22), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n112_), .O(z15));
  nand2  g75(.a(new_n118_), .b(x23), .O(new_n121_));
  nor3   g76(.a(x23), .b(x22), .c(x21), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n108_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n112_), .O(z16));
  nand3  g79(.a(new_n122_), .b(new_n108_), .c(new_n48_), .O(new_n125_));
  nand2  g80(.a(new_n123_), .b(x24), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n112_), .O(z17));
  nor2   g82(.a(new_n53_), .b(new_n48_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n98_), .c(new_n50_), .O(new_n129_));
  nand3  g84(.a(new_n122_), .b(new_n108_), .c(x25), .O(new_n130_));
  nand2  g85(.a(new_n123_), .b(new_n53_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n48_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n129_), .O(z18));
endmodule


