// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x11), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nor2   g08(.a(x18), .b(x17), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n52_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n49_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n49_), .O(z03));
  aoi21  g19(.a(new_n46_), .b(x02), .c(x15), .O(new_n65_));
  inv1   g20(.a(x09), .O(new_n66_));
  inv1   g21(.a(x12), .O(new_n67_));
  aoi22  g22(.a(new_n67_), .b(x03), .c(new_n66_), .d(x00), .O(new_n68_));
  oai21  g23(.a(new_n66_), .b(x00), .c(new_n68_), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  inv1   g25(.a(x03), .O(new_n71_));
  nand2  g26(.a(x12), .b(new_n71_), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  nand2  g28(.a(x11), .b(new_n73_), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(new_n69_), .c(new_n46_), .O(new_n76_));
  oai21  g31(.a(new_n65_), .b(x11), .c(new_n76_), .O(z04));
  nor3   g32(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g35(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand3  g36(.a(new_n58_), .b(new_n49_), .c(new_n51_), .O(z08));
  inv1   g37(.a(x07), .O(new_n83_));
  nand4  g38(.a(new_n47_), .b(new_n83_), .c(x05), .d(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n49_), .O(z09));
  nand2  g40(.a(new_n60_), .b(new_n47_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n49_), .O(z10));
  and2   g42(.a(x18), .b(x17), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n86_), .c(new_n54_), .O(z11));
  inv1   g44(.a(new_n86_), .O(new_n90_));
  or2    g45(.a(new_n88_), .b(x19), .O(new_n91_));
  nand2  g46(.a(new_n88_), .b(x19), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n49_), .O(z12));
  nand2  g49(.a(x15), .b(x11), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n88_), .b(new_n52_), .c(x19), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n60_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n47_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n95_), .O(z13));
  nand2  g55(.a(new_n97_), .b(x21), .O(new_n101_));
  inv1   g56(.a(x21), .O(new_n102_));
  nor2   g57(.a(x20), .b(new_n53_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n88_), .c(new_n102_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n101_), .c(new_n90_), .O(z14));
  inv1   g60(.a(x22), .O(new_n106_));
  nand4  g61(.a(new_n103_), .b(new_n88_), .c(new_n106_), .d(new_n102_), .O(new_n107_));
  nand2  g62(.a(new_n104_), .b(x22), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n90_), .O(z15));
  nand2  g64(.a(new_n107_), .b(x23), .O(new_n110_));
  nor2   g65(.a(x23), .b(x22), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n103_), .c(new_n88_), .d(new_n102_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n60_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n95_), .O(z16));
  inv1   g70(.a(x24), .O(new_n116_));
  nand3  g71(.a(new_n111_), .b(new_n116_), .c(new_n102_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n97_), .c(new_n60_), .O(new_n118_));
  aoi21  g73(.a(new_n112_), .b(x24), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x15), .c(new_n95_), .O(z17));
  inv1   g75(.a(new_n117_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n103_), .c(new_n88_), .d(new_n51_), .O(new_n122_));
  nand3  g77(.a(new_n121_), .b(new_n103_), .c(new_n88_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(x25), .c(new_n86_), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(new_n122_), .c(new_n48_), .O(z18));
endmodule


