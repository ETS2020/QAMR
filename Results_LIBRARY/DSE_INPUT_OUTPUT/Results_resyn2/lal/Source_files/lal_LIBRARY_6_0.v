// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  aoi21  g01(.a(x24), .b(new_n46_), .c(x16), .O(z00));
  nor3   g02(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  inv1   g03(.a(x21), .O(new_n49_));
  inv1   g04(.a(x22), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g06(.a(new_n48_), .b(x20), .c(new_n51_), .O(new_n52_));
  nor2   g07(.a(x25), .b(x23), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g09(.a(x25), .b(x16), .c(x24), .O(new_n55_));
  inv1   g10(.a(x07), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n54_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(x24), .b(new_n61_), .O(z02));
  inv1   g17(.a(x19), .O(new_n63_));
  nor2   g18(.a(x18), .b(x17), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n63_), .c(x20), .O(new_n65_));
  inv1   g20(.a(new_n51_), .O(new_n66_));
  inv1   g21(.a(x24), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(x23), .O(new_n68_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n68_), .O(new_n69_));
  nand2  g24(.a(new_n67_), .b(x16), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n69_), .c(x25), .O(z03));
  xor2a  g26(.a(x12), .b(x03), .O(new_n72_));
  xor2a  g27(.a(x11), .b(x02), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  xor2a  g29(.a(x09), .b(x00), .O(new_n75_));
  xor2a  g30(.a(x10), .b(x01), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nor2   g32(.a(x24), .b(x16), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  aoi21  g35(.a(new_n77_), .b(new_n74_), .c(new_n80_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n79_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n79_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  aoi21  g40(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x05), .c(x04), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n79_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand2  g45(.a(new_n87_), .b(new_n57_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n78_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  inv1   g49(.a(new_n64_), .O(new_n95_));
  nand2  g50(.a(x18), .b(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n92_), .c(new_n95_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(new_n63_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n91_), .c(new_n78_), .O(z12));
  nor2   g57(.a(new_n99_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n99_), .b(x20), .c(new_n91_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n78_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  inv1   g62(.a(x20), .O(new_n108_));
  inv1   g63(.a(new_n99_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n49_), .c(new_n108_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n107_), .c(new_n92_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  nand2  g67(.a(new_n49_), .b(new_n108_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n99_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n50_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n112_), .c(new_n92_), .O(z15));
  oai21  g71(.a(new_n110_), .b(x22), .c(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n114_), .c(new_n91_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n117_), .c(new_n78_), .O(z16));
  nand3  g75(.a(new_n118_), .b(new_n114_), .c(new_n67_), .O(new_n121_));
  nand2  g76(.a(new_n118_), .b(new_n114_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n92_), .O(z17));
  inv1   g79(.a(x25), .O(new_n125_));
  nor2   g80(.a(x24), .b(x21), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n118_), .c(new_n103_), .d(new_n125_), .O(new_n127_));
  nand3  g82(.a(new_n126_), .b(new_n118_), .c(new_n103_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x25), .O(new_n129_));
  aoi21  g84(.a(new_n125_), .b(x24), .c(x16), .O(new_n130_));
  nor2   g85(.a(new_n130_), .b(new_n91_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n129_), .c(new_n127_), .O(z18));
endmodule


