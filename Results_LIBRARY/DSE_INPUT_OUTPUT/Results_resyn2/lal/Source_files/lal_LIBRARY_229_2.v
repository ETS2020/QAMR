// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n113_, new_n114_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(x24), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  or2    g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n55_), .b(x20), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x24), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x23), .O(new_n61_));
  nor2   g16(.a(x24), .b(x15), .O(new_n62_));
  aoi21  g17(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g19(.a(x23), .O(new_n65_));
  nand2  g20(.a(new_n59_), .b(new_n65_), .O(new_n66_));
  aoi22  g21(.a(new_n66_), .b(x24), .c(new_n50_), .d(x25), .O(z03));
  nand2  g22(.a(new_n50_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n68_), .b(new_n77_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g34(.a(x25), .O(new_n80_));
  nand2  g35(.a(new_n66_), .b(x24), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n50_), .c(new_n80_), .O(z08));
  inv1   g37(.a(x07), .O(new_n83_));
  nand4  g38(.a(new_n48_), .b(new_n83_), .c(x05), .d(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n50_), .O(z09));
  nand2  g40(.a(new_n53_), .b(new_n48_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n86_), .c(new_n88_), .O(z11));
  xor2a  g46(.a(new_n89_), .b(x19), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n86_), .c(new_n50_), .O(z12));
  inv1   g48(.a(new_n86_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(x20), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n95_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n94_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(new_n56_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n94_), .O(z14));
  nand3  g57(.a(new_n96_), .b(new_n57_), .c(new_n56_), .O(new_n103_));
  nand2  g58(.a(new_n101_), .b(x22), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n94_), .O(z15));
  nand2  g60(.a(new_n103_), .b(x23), .O(new_n106_));
  nor3   g61(.a(x23), .b(x22), .c(x21), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n96_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n94_), .O(z16));
  nand4  g64(.a(new_n107_), .b(new_n96_), .c(new_n60_), .d(new_n48_), .O(new_n110_));
  oai21  g65(.a(new_n108_), .b(x15), .c(x24), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n54_), .O(z17));
  nand4  g67(.a(new_n107_), .b(new_n96_), .c(new_n80_), .d(new_n60_), .O(new_n113_));
  oai21  g68(.a(new_n108_), .b(x24), .c(x25), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n94_), .O(z18));
endmodule


