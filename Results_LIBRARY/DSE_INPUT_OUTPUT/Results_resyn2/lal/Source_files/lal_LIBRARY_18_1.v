// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x12), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x12), .c(new_n51_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n48_), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(x25), .c(new_n65_), .O(z01));
  and2   g21(.a(new_n48_), .b(x16), .O(z02));
  nor2   g22(.a(new_n62_), .b(x25), .O(z03));
  aoi21  g23(.a(new_n46_), .b(x03), .c(x24), .O(new_n69_));
  inv1   g24(.a(x09), .O(new_n70_));
  inv1   g25(.a(x02), .O(new_n71_));
  aoi22  g26(.a(x11), .b(new_n71_), .c(new_n70_), .d(x00), .O(new_n72_));
  oai21  g27(.a(new_n70_), .b(x00), .c(new_n72_), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  inv1   g29(.a(x11), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x02), .O(new_n76_));
  inv1   g31(.a(x03), .O(new_n77_));
  nand2  g32(.a(x12), .b(new_n77_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n73_), .c(new_n46_), .O(new_n80_));
  oai21  g35(.a(new_n69_), .b(x12), .c(new_n80_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n48_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g40(.a(x25), .O(new_n86_));
  inv1   g41(.a(x23), .O(new_n87_));
  nand2  g42(.a(new_n60_), .b(new_n57_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(x24), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n86_), .c(new_n49_), .O(z08));
  nor2   g46(.a(x15), .b(x07), .O(new_n92_));
  and2   g47(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n64_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand2  g50(.a(new_n92_), .b(new_n63_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g52(.a(x18), .b(x17), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n96_), .c(new_n48_), .O(z11));
  nand2  g54(.a(new_n93_), .b(new_n63_), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z12));
  inv1   g59(.a(new_n100_), .O(new_n105_));
  nand2  g60(.a(new_n102_), .b(new_n52_), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n105_), .O(z13));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n109_));
  nand3  g64(.a(new_n102_), .b(new_n58_), .c(new_n52_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n105_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  nand2  g67(.a(new_n59_), .b(new_n58_), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n96_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n112_), .c(new_n49_), .O(z15));
  oai21  g71(.a(new_n113_), .b(new_n106_), .c(x23), .O(new_n117_));
  nand2  g72(.a(new_n114_), .b(new_n87_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n105_), .O(z16));
  nand3  g74(.a(new_n114_), .b(new_n51_), .c(new_n87_), .O(new_n120_));
  nand2  g75(.a(new_n118_), .b(x24), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n105_), .O(z17));
  inv1   g77(.a(new_n96_), .O(new_n123_));
  nand4  g78(.a(new_n114_), .b(new_n86_), .c(new_n51_), .d(new_n87_), .O(new_n124_));
  nand2  g79(.a(new_n48_), .b(new_n86_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n120_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(z18));
endmodule


