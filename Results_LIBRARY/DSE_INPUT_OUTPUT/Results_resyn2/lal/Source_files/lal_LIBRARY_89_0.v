// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x20), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n49_), .O(new_n52_));
  nor3   g07(.a(x19), .b(x18), .c(x17), .O(new_n53_));
  and2   g08(.a(x22), .b(x21), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(x20), .c(new_n54_), .O(new_n55_));
  inv1   g10(.a(x24), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(x23), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  nor2   g13(.a(x24), .b(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n57_), .b(new_n55_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x25), .c(new_n52_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nor2   g18(.a(new_n60_), .b(x25), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z04));
  nor3   g26(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  nand2  g27(.a(x14), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g30(.a(new_n54_), .b(x24), .c(x25), .O(new_n76_));
  inv1   g31(.a(x23), .O(new_n77_));
  inv1   g32(.a(x25), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n54_), .b(new_n53_), .c(new_n79_), .O(new_n80_));
  oai22  g35(.a(new_n80_), .b(new_n56_), .c(new_n76_), .d(new_n58_), .O(z08));
  nor2   g36(.a(x15), .b(x07), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n51_), .c(new_n49_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z09));
  inv1   g39(.a(x17), .O(new_n85_));
  nand3  g40(.a(new_n82_), .b(new_n50_), .c(new_n49_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  inv1   g44(.a(x18), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n85_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n87_), .c(new_n91_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  aoi21  g49(.a(x18), .b(x17), .c(x19), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n82_), .c(new_n50_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n95_), .c(new_n49_), .O(z12));
  nand2  g53(.a(new_n82_), .b(new_n50_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n49_), .O(new_n100_));
  inv1   g55(.a(new_n96_), .O(new_n101_));
  nor2   g56(.a(new_n56_), .b(x20), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g58(.a(new_n96_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n100_), .O(z13));
  oai21  g60(.a(new_n101_), .b(new_n56_), .c(new_n58_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n96_), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n102_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n100_), .O(z14));
  oai21  g65(.a(new_n108_), .b(new_n56_), .c(new_n58_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n108_), .b(new_n102_), .c(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n100_), .O(z15));
  aoi21  g70(.a(new_n108_), .b(new_n113_), .c(new_n56_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(x20), .c(x23), .O(new_n117_));
  nand4  g72(.a(new_n108_), .b(new_n77_), .c(new_n113_), .d(new_n58_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x24), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n117_), .c(new_n100_), .O(z16));
  oai21  g76(.a(new_n119_), .b(new_n56_), .c(new_n100_), .O(z17));
  nand2  g77(.a(new_n87_), .b(new_n78_), .O(z18));
endmodule


