// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(new_n46_), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n55_), .c(x23), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(x08), .c(new_n49_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x07), .c(new_n47_), .O(new_n61_));
  oai21  g16(.a(new_n58_), .b(x25), .c(new_n61_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  inv1   g18(.a(x25), .O(new_n64_));
  oai21  g19(.a(new_n57_), .b(new_n49_), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n47_), .O(z03));
  inv1   g21(.a(x10), .O(new_n67_));
  inv1   g22(.a(x00), .O(new_n68_));
  aoi22  g23(.a(new_n67_), .b(x01), .c(x09), .d(new_n68_), .O(new_n69_));
  oai21  g24(.a(new_n67_), .b(x01), .c(new_n69_), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(new_n71_));
  inv1   g26(.a(x12), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x03), .c(new_n49_), .O(new_n73_));
  inv1   g28(.a(x11), .O(new_n74_));
  oai22  g29(.a(new_n74_), .b(x02), .c(x09), .d(new_n68_), .O(new_n75_));
  nand2  g30(.a(new_n72_), .b(x03), .O(new_n76_));
  nand2  g31(.a(new_n74_), .b(x02), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor3   g33(.a(new_n78_), .b(new_n75_), .c(new_n73_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n71_), .c(x08), .O(z04));
  aoi21  g35(.a(new_n49_), .b(x13), .c(x08), .O(z05));
  nand3  g36(.a(new_n49_), .b(x14), .c(new_n46_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  oai21  g38(.a(x24), .b(x06), .c(new_n46_), .O(z07));
  nand2  g39(.a(new_n57_), .b(x08), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n64_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  and2   g43(.a(new_n88_), .b(new_n47_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n59_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n51_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nand2  g50(.a(new_n88_), .b(new_n59_), .O(new_n96_));
  nand2  g51(.a(new_n52_), .b(new_n51_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(new_n96_), .c(new_n47_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n53_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nor2   g58(.a(new_n103_), .b(new_n92_), .O(z12));
  nor2   g59(.a(new_n101_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n93_), .O(z13));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n109_));
  inv1   g64(.a(x21), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n93_), .O(z14));
  inv1   g67(.a(new_n47_), .O(new_n113_));
  nand2  g68(.a(new_n111_), .b(x22), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n105_), .c(new_n96_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z15));
  nand2  g72(.a(new_n115_), .b(new_n105_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x23), .O(new_n119_));
  inv1   g74(.a(x23), .O(new_n120_));
  nand3  g75(.a(new_n115_), .b(new_n105_), .c(new_n120_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(new_n96_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n119_), .c(new_n113_), .O(z16));
  nand2  g79(.a(new_n122_), .b(new_n49_), .O(new_n125_));
  nand2  g80(.a(new_n121_), .b(x24), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n93_), .O(z17));
  nor2   g82(.a(new_n64_), .b(new_n49_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n96_), .c(new_n47_), .O(new_n129_));
  nor2   g84(.a(new_n121_), .b(new_n64_), .O(new_n130_));
  oai21  g85(.a(new_n122_), .b(x25), .c(new_n49_), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(new_n130_), .c(new_n129_), .O(z18));
endmodule


