// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x24), .O(new_n59_));
  nand2  g14(.a(x25), .b(new_n47_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g18(.a(new_n60_), .b(new_n59_), .c(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n58_), .b(new_n51_), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  aoi21  g22(.a(new_n58_), .b(x24), .c(new_n51_), .O(z03));
  nand2  g23(.a(new_n59_), .b(x23), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  nor2   g32(.a(new_n70_), .b(x13), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n69_), .O(z06));
  nand3  g35(.a(new_n69_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g36(.a(new_n58_), .b(x24), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n50_), .c(new_n47_), .O(z08));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n61_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n48_), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x05), .c(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand3  g43(.a(new_n86_), .b(new_n62_), .c(new_n53_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  xor2a  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n86_), .c(new_n62_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nor2   g48(.a(x15), .b(x07), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n62_), .c(new_n69_), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  aoi21  g52(.a(x18), .b(x17), .c(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z12));
  nand2  g54(.a(new_n97_), .b(new_n52_), .O(new_n100_));
  nand2  g55(.a(new_n94_), .b(new_n62_), .O(new_n101_));
  aoi21  g56(.a(new_n96_), .b(x20), .c(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n100_), .c(new_n48_), .O(z13));
  inv1   g58(.a(new_n95_), .O(new_n104_));
  nand2  g59(.a(new_n100_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand3  g61(.a(new_n97_), .b(new_n106_), .c(new_n52_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  nor3   g64(.a(x22), .b(x21), .c(x20), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n97_), .c(new_n101_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n109_), .c(new_n48_), .O(z15));
  nand3  g67(.a(new_n110_), .b(new_n97_), .c(new_n47_), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n106_), .c(new_n52_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n96_), .c(x23), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n113_), .c(new_n104_), .O(z16));
  nand4  g72(.a(new_n110_), .b(new_n97_), .c(new_n59_), .d(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n101_), .b(new_n69_), .O(new_n119_));
  nand2  g74(.a(new_n113_), .b(x24), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(z17));
  nand2  g76(.a(new_n50_), .b(new_n59_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n110_), .c(new_n97_), .O(new_n123_));
  oai21  g78(.a(new_n115_), .b(new_n96_), .c(new_n50_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n47_), .O(new_n125_));
  aoi22  g80(.a(new_n101_), .b(new_n69_), .c(x25), .d(x24), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n125_), .O(z18));
endmodule


