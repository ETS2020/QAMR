// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x12), .O(new_n48_));
  nand2  g03(.a(x13), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  inv1   g16(.a(new_n49_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(new_n62_), .c(x07), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  nand3  g21(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  inv1   g23(.a(x03), .O(new_n69_));
  nor2   g24(.a(x08), .b(new_n69_), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x13), .c(new_n48_), .O(new_n71_));
  inv1   g26(.a(x09), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  aoi22  g28(.a(x11), .b(new_n73_), .c(new_n72_), .d(x00), .O(new_n74_));
  oai21  g29(.a(new_n72_), .b(x00), .c(new_n74_), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n69_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n75_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n71_), .O(z04));
  nor2   g37(.a(x13), .b(x08), .O(z05));
  nand2  g38(.a(x14), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n62_), .O(z06));
  nand3  g40(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g41(.a(new_n60_), .b(new_n52_), .c(new_n62_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand4  g44(.a(new_n64_), .b(new_n49_), .c(new_n89_), .d(new_n88_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand3  g46(.a(new_n63_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g48(.a(new_n55_), .b(new_n54_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n92_), .c(new_n49_), .O(z11));
  nand2  g52(.a(new_n95_), .b(new_n56_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n92_), .c(new_n49_), .O(z12));
  inv1   g56(.a(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(x20), .c(new_n92_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n103_), .c(new_n62_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nor3   g61(.a(new_n99_), .b(x21), .c(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n63_), .b(new_n49_), .c(new_n89_), .d(new_n88_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand2  g68(.a(new_n107_), .b(new_n113_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(z15));
  nand2  g70(.a(new_n114_), .b(x23), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n107_), .c(new_n92_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n116_), .c(new_n62_), .O(z16));
  nand2  g74(.a(new_n117_), .b(new_n107_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x24), .O(new_n121_));
  inv1   g76(.a(new_n92_), .O(new_n122_));
  inv1   g77(.a(x24), .O(new_n123_));
  nor2   g78(.a(x21), .b(x20), .O(new_n124_));
  nand4  g79(.a(new_n117_), .b(new_n124_), .c(new_n102_), .d(new_n123_), .O(new_n125_));
  and2   g80(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n121_), .c(new_n62_), .O(z17));
  aoi21  g82(.a(new_n125_), .b(x25), .c(new_n109_), .O(new_n128_));
  oai21  g83(.a(new_n125_), .b(x25), .c(new_n128_), .O(z18));
endmodule


