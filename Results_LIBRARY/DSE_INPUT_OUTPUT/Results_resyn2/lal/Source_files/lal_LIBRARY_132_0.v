// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x21), .b(x20), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x17), .O(new_n59_));
  inv1   g14(.a(x18), .O(new_n60_));
  inv1   g15(.a(x19), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g18(.a(x22), .b(x21), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n63_), .c(x24), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n57_), .O(new_n67_));
  nand2  g22(.a(x05), .b(x04), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n67_), .c(new_n52_), .O(z01));
  nor2   g24(.a(x18), .b(x17), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n61_), .c(x20), .O(new_n71_));
  oai21  g26(.a(new_n64_), .b(new_n71_), .c(new_n53_), .O(new_n72_));
  nor2   g27(.a(new_n48_), .b(x25), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  aoi21  g29(.a(new_n72_), .b(x24), .c(new_n74_), .O(z03));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x09), .b(x00), .O(new_n78_));
  xnor2a g33(.a(x10), .b(x01), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z04));
  oai21  g37(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g38(.a(x14), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n48_), .O(z06));
  aoi21  g40(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g41(.a(new_n62_), .b(new_n58_), .c(new_n64_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x23), .c(x24), .O(new_n88_));
  nand2  g43(.a(new_n73_), .b(new_n88_), .O(z08));
  nor2   g44(.a(x15), .b(x07), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(x05), .c(x04), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n49_), .O(z09));
  nand2  g47(.a(new_n90_), .b(new_n68_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(x17), .c(new_n49_), .O(z10));
  nor2   g49(.a(new_n93_), .b(new_n48_), .O(new_n95_));
  nor2   g50(.a(new_n60_), .b(new_n59_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n70_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  inv1   g54(.a(new_n93_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x19), .O(new_n101_));
  oai21  g56(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n49_), .O(z12));
  nand3  g59(.a(new_n96_), .b(new_n58_), .c(x19), .O(new_n105_));
  nand2  g60(.a(new_n101_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n95_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n95_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  aoi21  g65(.a(new_n100_), .b(new_n110_), .c(new_n48_), .O(z15));
  nand2  g66(.a(new_n95_), .b(new_n53_), .O(z16));
  aoi21  g67(.a(new_n100_), .b(new_n54_), .c(new_n48_), .O(z17));
  nand2  g68(.a(new_n95_), .b(new_n55_), .O(z18));
endmodule


