// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n117_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x22), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(x22), .c(x21), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n53_), .c(new_n54_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  and2   g18(.a(new_n63_), .b(new_n50_), .O(new_n64_));
  oai21  g19(.a(new_n62_), .b(x25), .c(new_n64_), .O(z01));
  nor3   g20(.a(new_n62_), .b(new_n49_), .c(x25), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n49_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x15), .O(new_n78_));
  inv1   g33(.a(x21), .O(new_n79_));
  aoi21  g34(.a(new_n59_), .b(new_n55_), .c(new_n79_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(x24), .c(new_n78_), .O(new_n81_));
  aoi21  g36(.a(x24), .b(x23), .c(x25), .O(new_n82_));
  oai21  g37(.a(new_n81_), .b(new_n48_), .c(new_n82_), .O(z08));
  inv1   g38(.a(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(x05), .c(x04), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n48_), .c(x15), .O(z09));
  nand3  g41(.a(new_n63_), .b(new_n48_), .c(new_n78_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n56_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(new_n57_), .b(new_n56_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n63_), .c(new_n91_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n48_), .c(x15), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(x18), .b(x17), .c(x19), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n87_), .O(z12));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(x20), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n63_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n78_), .O(z13));
  nor2   g58(.a(new_n99_), .b(new_n79_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(new_n79_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n63_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n104_), .c(new_n48_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n78_), .O(z14));
  nand2  g63(.a(new_n105_), .b(new_n88_), .O(z15));
  nand2  g64(.a(new_n105_), .b(x23), .O(new_n110_));
  nand3  g65(.a(new_n99_), .b(new_n53_), .c(new_n79_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n88_), .O(z16));
  nand4  g67(.a(new_n99_), .b(new_n54_), .c(new_n53_), .d(new_n79_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n63_), .O(new_n114_));
  aoi21  g69(.a(new_n111_), .b(x24), .c(new_n114_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(x22), .c(new_n78_), .O(z17));
  aoi21  g71(.a(new_n113_), .b(x25), .c(new_n87_), .O(new_n117_));
  oai21  g72(.a(new_n113_), .b(x25), .c(new_n117_), .O(z18));
endmodule


