// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x13), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n57_), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n50_), .c(new_n51_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n47_), .c(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  nand2  g21(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g22(.a(new_n61_), .b(new_n47_), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  nand2  g24(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g33(.a(new_n47_), .b(x14), .c(new_n69_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  inv1   g35(.a(new_n47_), .O(new_n81_));
  aoi21  g36(.a(new_n69_), .b(x06), .c(new_n81_), .O(z07));
  inv1   g37(.a(x23), .O(new_n83_));
  nor2   g38(.a(x18), .b(x17), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n55_), .c(x20), .O(new_n85_));
  oai21  g40(.a(new_n58_), .b(new_n85_), .c(new_n83_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x13), .c(x24), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n51_), .O(z08));
  nor4   g43(.a(new_n63_), .b(new_n81_), .c(x15), .d(x07), .O(z09));
  inv1   g44(.a(x15), .O(new_n90_));
  nand3  g45(.a(new_n63_), .b(new_n90_), .c(new_n62_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g47(.a(new_n65_), .b(new_n90_), .O(new_n93_));
  nor2   g48(.a(new_n54_), .b(new_n53_), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n93_), .c(new_n84_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n94_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n93_), .O(z12));
  inv1   g54(.a(new_n93_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x20), .O(new_n101_));
  nand4  g56(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(z13));
  nand2  g58(.a(new_n102_), .b(x21), .O(new_n104_));
  inv1   g59(.a(x21), .O(new_n105_));
  inv1   g60(.a(new_n102_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n91_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n104_), .c(new_n81_), .O(z14));
  oai21  g63(.a(new_n102_), .b(x21), .c(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n106_), .b(new_n110_), .c(new_n105_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n100_), .O(z15));
  nand2  g67(.a(new_n111_), .b(x23), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(new_n105_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n102_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n83_), .c(new_n91_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n113_), .c(new_n81_), .O(z16));
  nand2  g72(.a(new_n91_), .b(new_n47_), .O(new_n118_));
  nor2   g73(.a(new_n50_), .b(x13), .O(new_n119_));
  oai21  g74(.a(new_n111_), .b(x23), .c(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n115_), .b(new_n50_), .c(new_n83_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n118_), .O(z17));
  inv1   g77(.a(new_n91_), .O(new_n123_));
  nand2  g78(.a(new_n47_), .b(new_n51_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand4  g80(.a(new_n115_), .b(new_n51_), .c(new_n50_), .d(new_n83_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(z18));
endmodule


