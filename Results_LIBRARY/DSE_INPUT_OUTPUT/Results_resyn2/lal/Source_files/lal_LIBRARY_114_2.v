// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x20), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  aoi21  g06(.a(x25), .b(new_n51_), .c(x24), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  nor2   g12(.a(x25), .b(x23), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n53_), .d(new_n50_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n48_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n53_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n48_), .O(z04));
  nand2  g26(.a(new_n48_), .b(new_n46_), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(x13), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n72_), .b(new_n74_), .O(z06));
  inv1   g30(.a(new_n48_), .O(new_n76_));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n76_), .O(z07));
  inv1   g32(.a(x17), .O(new_n78_));
  inv1   g33(.a(x18), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x19), .c(new_n51_), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x22), .c(x21), .O(new_n82_));
  aoi21  g37(.a(new_n58_), .b(new_n82_), .c(new_n52_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(x05), .c(x04), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n48_), .O(z09));
  nand2  g41(.a(new_n84_), .b(new_n60_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n76_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n78_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n88_), .c(new_n80_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(new_n54_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n87_), .c(new_n76_), .O(z12));
  nand2  g52(.a(new_n94_), .b(x20), .O(new_n98_));
  nor2   g53(.a(new_n94_), .b(x20), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(new_n87_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n98_), .c(new_n76_), .O(z13));
  inv1   g56(.a(x21), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(new_n102_), .O(new_n103_));
  inv1   g58(.a(new_n94_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n51_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(x21), .c(new_n87_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n103_), .c(new_n76_), .O(z14));
  inv1   g62(.a(x22), .O(new_n108_));
  aoi21  g63(.a(new_n104_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nor3   g64(.a(new_n94_), .b(x22), .c(x21), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n109_), .c(new_n51_), .O(new_n111_));
  nor2   g66(.a(new_n108_), .b(new_n51_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n87_), .c(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n111_), .O(z15));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(x19), .c(x18), .d(x17), .O(new_n116_));
  xor2a  g71(.a(new_n116_), .b(x23), .O(new_n117_));
  and2   g72(.a(x23), .b(x20), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n87_), .c(new_n48_), .O(new_n119_));
  oai21  g74(.a(new_n117_), .b(x20), .c(new_n119_), .O(z16));
  inv1   g75(.a(new_n87_), .O(new_n121_));
  oai21  g76(.a(new_n116_), .b(x23), .c(x24), .O(new_n122_));
  nor2   g77(.a(x24), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n115_), .c(new_n104_), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n122_), .c(new_n121_), .d(new_n51_), .O(z17));
  xor2a  g80(.a(new_n124_), .b(x25), .O(new_n126_));
  oai21  g81(.a(x25), .b(new_n47_), .c(x20), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n121_), .O(z18));
endmodule


