// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x07), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nand2  g17(.a(new_n48_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n48_), .O(z03));
  nand2  g19(.a(new_n48_), .b(new_n46_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  nor2   g27(.a(new_n65_), .b(x13), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n65_), .b(new_n74_), .O(z06));
  nand3  g30(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  aoi22  g31(.a(new_n58_), .b(new_n50_), .c(new_n47_), .d(x07), .O(z08));
  nor2   g32(.a(new_n60_), .b(x15), .O(z09));
  inv1   g33(.a(x07), .O(new_n79_));
  nand2  g34(.a(x05), .b(x04), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n52_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n79_), .c(x15), .O(z10));
  nand2  g37(.a(new_n53_), .b(new_n52_), .O(new_n83_));
  nand2  g38(.a(x18), .b(x17), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n80_), .c(new_n83_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n79_), .c(x15), .O(z11));
  nand2  g41(.a(new_n60_), .b(new_n47_), .O(new_n87_));
  inv1   g42(.a(new_n84_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(x19), .O(new_n89_));
  nand2  g44(.a(new_n84_), .b(new_n54_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n87_), .O(z12));
  inv1   g47(.a(new_n87_), .O(new_n93_));
  nand2  g48(.a(new_n89_), .b(x20), .O(new_n94_));
  nand4  g49(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(z13));
  nand2  g51(.a(new_n95_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  inv1   g53(.a(new_n95_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n97_), .c(new_n80_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n79_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(z14));
  inv1   g58(.a(x22), .O(new_n104_));
  nor2   g59(.a(x20), .b(new_n54_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n88_), .c(new_n104_), .d(new_n98_), .O(new_n106_));
  nand2  g61(.a(new_n100_), .b(x22), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n93_), .O(z15));
  nand2  g63(.a(new_n106_), .b(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  nand4  g65(.a(new_n99_), .b(new_n110_), .c(new_n104_), .d(new_n98_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n80_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n79_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n47_), .O(z16));
  nand2  g69(.a(new_n111_), .b(x24), .O(new_n115_));
  inv1   g70(.a(x24), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n110_), .c(new_n104_), .d(new_n98_), .O(new_n117_));
  nor2   g72(.a(new_n117_), .b(new_n95_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n87_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n115_), .O(z17));
  nand2  g75(.a(new_n118_), .b(new_n50_), .O(new_n121_));
  oai21  g76(.a(new_n117_), .b(new_n95_), .c(x25), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n80_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n79_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(z18));
endmodule


