// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n92_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x13), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  inv1   g17(.a(x23), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(x13), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n62_), .c(x24), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n55_), .O(z08));
  nand2  g21(.a(z08), .b(new_n54_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n48_), .b(new_n68_), .O(z02));
  inv1   g24(.a(z08), .O(z03));
  inv1   g25(.a(x12), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(x03), .O(new_n72_));
  inv1   g27(.a(x02), .O(new_n73_));
  nand2  g28(.a(x11), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x03), .O(new_n75_));
  nand2  g30(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .d(new_n72_), .O(new_n79_));
  inv1   g34(.a(x00), .O(new_n80_));
  nand2  g35(.a(x09), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(x01), .O(new_n82_));
  nand2  g37(.a(x10), .b(new_n82_), .O(new_n83_));
  inv1   g38(.a(x09), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x00), .O(new_n85_));
  inv1   g40(.a(x10), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x01), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n85_), .c(new_n83_), .d(new_n81_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n79_), .c(new_n46_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n50_), .O(z04));
  aoi21  g45(.a(new_n47_), .b(x08), .c(x13), .O(z05));
  nand2  g46(.a(x14), .b(new_n46_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n50_), .O(z06));
  nand3  g48(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  nor2   g49(.a(x15), .b(x07), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n52_), .c(new_n50_), .O(z09));
  nand2  g52(.a(new_n95_), .b(new_n52_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n57_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z10));
  nor2   g56(.a(x18), .b(x17), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(x18), .b(x17), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n99_), .c(new_n103_), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(z11));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  aoi21  g62(.a(new_n104_), .b(new_n59_), .c(new_n98_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n50_), .O(z12));
  nand4  g65(.a(new_n56_), .b(x19), .c(x18), .d(x17), .O(new_n111_));
  aoi21  g66(.a(new_n107_), .b(x20), .c(new_n98_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n111_), .c(new_n48_), .O(z13));
  nand2  g68(.a(new_n111_), .b(x21), .O(new_n114_));
  inv1   g69(.a(x21), .O(new_n115_));
  inv1   g70(.a(new_n111_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n114_), .c(new_n99_), .O(z14));
  nand2  g73(.a(new_n117_), .b(x22), .O(new_n119_));
  inv1   g74(.a(x22), .O(new_n120_));
  nand3  g75(.a(new_n116_), .b(new_n120_), .c(new_n115_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n99_), .O(z15));
  nand2  g77(.a(new_n121_), .b(x23), .O(new_n123_));
  nand2  g78(.a(new_n120_), .b(new_n115_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n111_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n63_), .c(new_n98_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n123_), .c(new_n48_), .O(z16));
  nand3  g82(.a(new_n125_), .b(new_n47_), .c(new_n63_), .O(new_n128_));
  nand2  g83(.a(new_n125_), .b(new_n63_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x24), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n128_), .c(new_n99_), .O(z17));
  nand4  g86(.a(new_n125_), .b(new_n55_), .c(new_n47_), .d(new_n63_), .O(new_n132_));
  nand2  g87(.a(new_n129_), .b(x25), .O(new_n133_));
  aoi21  g88(.a(new_n55_), .b(x13), .c(new_n47_), .O(new_n134_));
  nor2   g89(.a(new_n134_), .b(new_n98_), .O(new_n135_));
  nand3  g90(.a(new_n135_), .b(new_n133_), .c(new_n132_), .O(z18));
endmodule


