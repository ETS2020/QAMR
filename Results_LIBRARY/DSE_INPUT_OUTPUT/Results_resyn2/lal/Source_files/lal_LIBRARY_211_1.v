// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n72_,
    new_n76_, new_n77_, new_n79_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x20), .O(new_n48_));
  nand2  g03(.a(x24), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  inv1   g10(.a(x24), .O(new_n56_));
  aoi21  g11(.a(x22), .b(x21), .c(x23), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(x20), .c(new_n56_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x25), .c(new_n55_), .O(z01));
  inv1   g14(.a(x25), .O(new_n60_));
  oai21  g15(.a(new_n57_), .b(new_n56_), .c(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n49_), .O(z03));
  xnor2a g17(.a(x11), .b(x02), .O(new_n63_));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n49_), .O(z04));
  nand2  g24(.a(new_n49_), .b(new_n46_), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(x13), .O(z05));
  nand2  g26(.a(x14), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z06));
  nand3  g28(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  or2    g29(.a(new_n58_), .b(x25), .O(z08));
  inv1   g30(.a(x15), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(new_n53_), .c(new_n49_), .O(z09));
  nand3  g33(.a(new_n53_), .b(new_n76_), .c(new_n52_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x17), .c(new_n49_), .O(z10));
  xnor2a g35(.a(x18), .b(x17), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n79_), .c(new_n49_), .O(z11));
  and2   g37(.a(x18), .b(x17), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x19), .O(new_n84_));
  inv1   g39(.a(new_n79_), .O(new_n85_));
  nand3  g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n84_), .c(new_n49_), .O(z12));
  inv1   g43(.a(new_n86_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x24), .c(new_n48_), .O(new_n90_));
  aoi21  g45(.a(new_n86_), .b(x20), .c(new_n79_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(z13));
  nand2  g47(.a(new_n79_), .b(new_n49_), .O(new_n93_));
  nand2  g48(.a(new_n90_), .b(x21), .O(new_n94_));
  inv1   g49(.a(x21), .O(new_n95_));
  nor2   g50(.a(x24), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n89_), .c(new_n95_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n94_), .c(new_n93_), .O(z14));
  inv1   g53(.a(x22), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n95_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n86_), .c(new_n56_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x22), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n85_), .O(z15));
  nand4  g59(.a(new_n99_), .b(new_n95_), .c(x18), .d(x17), .O(new_n105_));
  inv1   g60(.a(x23), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x19), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n105_), .c(new_n56_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n48_), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n96_), .c(new_n89_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(x23), .c(new_n79_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n109_), .O(z16));
  inv1   g68(.a(x19), .O(new_n114_));
  nor2   g69(.a(x23), .b(new_n114_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n110_), .c(new_n83_), .d(new_n48_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n85_), .c(new_n56_), .O(z17));
  nand3  g72(.a(new_n60_), .b(new_n106_), .c(x19), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n105_), .c(new_n56_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n48_), .O(new_n120_));
  nand2  g75(.a(new_n110_), .b(new_n96_), .O(new_n121_));
  nand2  g76(.a(new_n115_), .b(new_n83_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n121_), .c(x25), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n120_), .c(new_n85_), .O(z18));
endmodule


