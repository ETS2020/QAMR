// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n83_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x02), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  inv1   g16(.a(x21), .O(new_n62_));
  inv1   g17(.a(x22), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n61_), .c(x23), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n47_), .c(new_n48_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n55_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n50_), .b(new_n68_), .O(z02));
  inv1   g24(.a(x25), .O(new_n70_));
  oai21  g25(.a(new_n65_), .b(new_n48_), .c(new_n70_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n50_), .O(z03));
  nand2  g27(.a(new_n50_), .b(new_n46_), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  xor2a  g31(.a(x12), .b(x03), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(z04));
  or2    g35(.a(new_n73_), .b(x13), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand2  g40(.a(new_n65_), .b(new_n47_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x24), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n70_), .O(z08));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n53_), .c(new_n50_), .O(z09));
  nand3  g46(.a(new_n53_), .b(new_n89_), .c(new_n52_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g48(.a(x18), .b(x17), .O(new_n94_));
  inv1   g49(.a(new_n92_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n50_), .O(new_n96_));
  nor2   g51(.a(new_n58_), .b(new_n57_), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n94_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n97_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n96_), .O(z12));
  nand2  g57(.a(new_n100_), .b(new_n56_), .O(new_n103_));
  aoi21  g58(.a(new_n99_), .b(x20), .c(new_n96_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nor2   g61(.a(new_n99_), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n62_), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(new_n92_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n106_), .c(new_n49_), .O(z14));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  aoi21  g67(.a(new_n109_), .b(new_n63_), .c(new_n92_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n112_), .c(new_n49_), .O(z15));
  oai21  g69(.a(new_n108_), .b(x22), .c(x23), .O(new_n115_));
  nor2   g70(.a(x23), .b(x22), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n107_), .c(new_n62_), .O(new_n117_));
  inv1   g72(.a(new_n117_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n115_), .c(new_n49_), .O(z16));
  nand2  g75(.a(new_n118_), .b(new_n48_), .O(new_n121_));
  aoi21  g76(.a(new_n117_), .b(x24), .c(new_n92_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n121_), .c(new_n49_), .O(z17));
  nor2   g78(.a(new_n70_), .b(new_n48_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n92_), .c(new_n50_), .O(new_n125_));
  nor2   g80(.a(new_n117_), .b(new_n70_), .O(new_n126_));
  oai21  g81(.a(new_n118_), .b(x25), .c(new_n48_), .O(new_n127_));
  oai21  g82(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(z18));
endmodule


