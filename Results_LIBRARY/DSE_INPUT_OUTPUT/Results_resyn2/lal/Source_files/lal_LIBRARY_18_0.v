// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x12), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n49_), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  inv1   g16(.a(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x23), .O(new_n63_));
  inv1   g18(.a(x12), .O(new_n64_));
  nor2   g19(.a(x24), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n52_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n47_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n66_), .b(x25), .O(z03));
  inv1   g25(.a(x03), .O(new_n71_));
  oai21  g26(.a(x08), .b(new_n71_), .c(x24), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n64_), .O(new_n73_));
  inv1   g28(.a(x09), .O(new_n74_));
  inv1   g29(.a(x02), .O(new_n75_));
  aoi22  g30(.a(x11), .b(new_n75_), .c(new_n74_), .d(x00), .O(new_n76_));
  oai21  g31(.a(new_n74_), .b(x00), .c(new_n76_), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  inv1   g33(.a(x11), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x02), .O(new_n80_));
  nand2  g35(.a(x12), .b(new_n71_), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n77_), .c(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n73_), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n49_), .O(z06));
  aoi21  g42(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g43(.a(x25), .O(new_n89_));
  aoi21  g44(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n90_));
  inv1   g45(.a(x23), .O(new_n91_));
  nand2  g46(.a(new_n89_), .b(new_n91_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n90_), .c(x24), .O(new_n93_));
  oai21  g48(.a(new_n89_), .b(new_n64_), .c(new_n93_), .O(z08));
  nor3   g49(.a(new_n47_), .b(x15), .c(x07), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n51_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z09));
  nor2   g52(.a(x15), .b(x07), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n50_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(x17), .c(new_n49_), .O(z10));
  xnor2a g55(.a(x18), .b(x17), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n49_), .O(z11));
  nand2  g57(.a(new_n95_), .b(new_n50_), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  aoi21  g60(.a(x18), .b(x17), .c(x19), .O(new_n106_));
  nor3   g61(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z12));
  inv1   g62(.a(new_n103_), .O(new_n108_));
  nor2   g63(.a(new_n104_), .b(x20), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n104_), .b(x20), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(z13));
  nand2  g67(.a(new_n110_), .b(x21), .O(new_n113_));
  inv1   g68(.a(x21), .O(new_n114_));
  nand2  g69(.a(new_n109_), .b(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n108_), .O(z14));
  nand2  g71(.a(new_n115_), .b(x22), .O(new_n117_));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n109_), .c(new_n99_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n117_), .c(new_n47_), .O(z15));
  nand2  g75(.a(new_n118_), .b(new_n109_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x23), .O(new_n122_));
  nand3  g77(.a(new_n118_), .b(new_n109_), .c(new_n91_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n108_), .O(z16));
  nand2  g79(.a(new_n123_), .b(x24), .O(new_n125_));
  nand4  g80(.a(new_n118_), .b(new_n109_), .c(new_n62_), .d(new_n91_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n108_), .O(z17));
  xor2a  g82(.a(new_n126_), .b(x25), .O(new_n128_));
  aoi21  g83(.a(new_n89_), .b(x24), .c(x12), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n99_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(z18));
endmodule


