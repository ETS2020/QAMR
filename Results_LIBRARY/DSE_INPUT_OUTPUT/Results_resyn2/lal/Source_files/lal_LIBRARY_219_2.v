// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x12), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x19), .c(new_n53_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n52_), .c(new_n51_), .O(new_n62_));
  inv1   g17(.a(x12), .O(new_n63_));
  aoi21  g18(.a(x25), .b(new_n63_), .c(x24), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nor3   g21(.a(new_n66_), .b(new_n64_), .c(x07), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n62_), .O(z01));
  and2   g23(.a(new_n48_), .b(x16), .O(z02));
  inv1   g24(.a(x19), .O(new_n70_));
  nor2   g25(.a(x18), .b(x17), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n70_), .c(x20), .O(new_n72_));
  inv1   g27(.a(new_n60_), .O(new_n73_));
  nor2   g28(.a(new_n47_), .b(x23), .O(new_n74_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(new_n74_), .O(new_n75_));
  nand2  g30(.a(new_n47_), .b(new_n63_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n75_), .c(x25), .O(z03));
  nand2  g32(.a(new_n48_), .b(new_n46_), .O(new_n78_));
  xor2a  g33(.a(x11), .b(x02), .O(new_n79_));
  xor2a  g34(.a(x09), .b(x00), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  xor2a  g36(.a(x10), .b(x01), .O(new_n82_));
  xor2a  g37(.a(x12), .b(x03), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n81_), .c(new_n78_), .O(z04));
  nor2   g40(.a(new_n78_), .b(x13), .O(z05));
  nand2  g41(.a(x14), .b(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n48_), .O(z06));
  aoi21  g43(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g44(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n90_));
  nor2   g45(.a(new_n49_), .b(x25), .O(new_n91_));
  oai21  g46(.a(new_n90_), .b(new_n47_), .c(new_n91_), .O(z08));
  nor2   g47(.a(x15), .b(x07), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n66_), .c(new_n48_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand2  g50(.a(new_n93_), .b(new_n65_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(x17), .c(new_n48_), .O(z10));
  nand3  g52(.a(new_n93_), .b(new_n65_), .c(new_n48_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n56_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z11));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  aoi21  g58(.a(new_n100_), .b(new_n70_), .c(new_n96_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n48_), .O(z12));
  nor2   g61(.a(new_n103_), .b(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n103_), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n99_), .O(z13));
  nand2  g65(.a(new_n108_), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n107_), .b(new_n58_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n99_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n107_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n99_), .O(z15));
  nand2  g72(.a(new_n116_), .b(x23), .O(new_n118_));
  nand4  g73(.a(new_n51_), .b(new_n59_), .c(new_n58_), .d(new_n53_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n103_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n96_), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n118_), .c(new_n49_), .O(z16));
  nand2  g77(.a(new_n96_), .b(new_n48_), .O(new_n123_));
  nand3  g78(.a(new_n120_), .b(new_n47_), .c(new_n63_), .O(new_n124_));
  oai21  g79(.a(new_n119_), .b(new_n103_), .c(x24), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n123_), .O(z17));
  oai21  g81(.a(new_n119_), .b(new_n103_), .c(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n115_), .c(new_n107_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n63_), .O(new_n131_));
  aoi22  g86(.a(new_n96_), .b(new_n48_), .c(x25), .d(x24), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n131_), .O(z18));
endmodule


