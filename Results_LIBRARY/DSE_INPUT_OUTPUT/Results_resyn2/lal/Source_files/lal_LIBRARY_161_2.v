// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n80_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x22), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x19), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n51_), .c(x20), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  nor2   g09(.a(x25), .b(x23), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  inv1   g12(.a(x25), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  and2   g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n56_), .c(new_n48_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n49_), .b(new_n63_), .O(z02));
  inv1   g19(.a(x20), .O(new_n65_));
  nand2  g20(.a(new_n52_), .b(new_n51_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n65_), .c(new_n54_), .O(new_n67_));
  nand2  g22(.a(x24), .b(x23), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  aoi22  g24(.a(new_n69_), .b(new_n49_), .c(new_n67_), .d(x24), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  nand2  g32(.a(new_n49_), .b(new_n46_), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x13), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor2   g35(.a(new_n78_), .b(new_n80_), .O(z06));
  nand3  g36(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g37(.a(new_n69_), .b(new_n49_), .O(new_n83_));
  nand2  g38(.a(new_n67_), .b(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n83_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n47_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n49_), .O(z09));
  nand2  g43(.a(new_n60_), .b(new_n47_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n49_), .O(z10));
  xnor2a g45(.a(x18), .b(x17), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n89_), .c(new_n49_), .O(z11));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n51_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n89_), .c(new_n49_), .O(z12));
  nand2  g52(.a(x22), .b(x15), .O(new_n98_));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n95_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n65_), .c(new_n60_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n98_), .O(z13));
  oai21  g58(.a(new_n95_), .b(x20), .c(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n100_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n60_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n98_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n105_), .b(new_n100_), .c(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n60_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n47_), .O(new_n113_));
  oai21  g68(.a(new_n106_), .b(x15), .c(x22), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(z15));
  inv1   g70(.a(new_n89_), .O(new_n116_));
  nand2  g71(.a(new_n111_), .b(x23), .O(new_n117_));
  nor2   g72(.a(x23), .b(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n105_), .c(new_n100_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  nor3   g76(.a(new_n95_), .b(x21), .c(x20), .O(new_n122_));
  nand3  g77(.a(new_n118_), .b(new_n122_), .c(new_n57_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n116_), .O(z17));
  oai21  g79(.a(new_n119_), .b(x24), .c(x25), .O(new_n125_));
  nand4  g80(.a(new_n118_), .b(new_n122_), .c(new_n58_), .d(new_n57_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n116_), .O(z18));
endmodule


