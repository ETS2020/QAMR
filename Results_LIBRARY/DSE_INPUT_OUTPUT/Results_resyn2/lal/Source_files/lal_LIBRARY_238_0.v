// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x07), .O(new_n48_));
  inv1   g03(.a(x24), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x23), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  oai21  g18(.a(new_n62_), .b(new_n59_), .c(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(x24), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n48_), .O(z01));
  oai21  g21(.a(new_n61_), .b(new_n48_), .c(new_n49_), .O(new_n67_));
  oai21  g22(.a(new_n62_), .b(new_n59_), .c(new_n67_), .O(z03));
  nand2  g23(.a(new_n50_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n69_), .b(new_n78_), .O(z06));
  nand3  g34(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  oai21  g35(.a(new_n59_), .b(x23), .c(x24), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n50_), .c(new_n61_), .O(z08));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n63_), .O(z09));
  inv1   g40(.a(new_n63_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n54_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x24), .c(x07), .O(z10));
  nand2  g44(.a(new_n55_), .b(new_n54_), .O(new_n90_));
  nor2   g45(.a(new_n55_), .b(new_n54_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n87_), .c(new_n90_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(x24), .c(x07), .O(z11));
  inv1   g49(.a(new_n84_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n63_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nor2   g53(.a(new_n91_), .b(x19), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n96_), .O(z12));
  inv1   g55(.a(new_n96_), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n53_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  nor2   g60(.a(x21), .b(x20), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n101_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n106_), .b(new_n98_), .c(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(x22), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n87_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x24), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n48_), .O(z15));
  nand2  g69(.a(new_n110_), .b(x23), .O(new_n115_));
  nand4  g70(.a(new_n106_), .b(new_n98_), .c(new_n60_), .d(new_n109_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n87_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n48_), .O(z16));
  nor3   g74(.a(new_n86_), .b(new_n49_), .c(new_n56_), .O(new_n120_));
  nand3  g75(.a(new_n106_), .b(new_n60_), .c(new_n109_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n120_), .c(new_n91_), .d(new_n83_), .O(z17));
  nand2  g78(.a(new_n101_), .b(new_n61_), .O(z18));
endmodule


