// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x13), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  inv1   g14(.a(new_n48_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(x25), .b(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(new_n65_));
  oai21  g20(.a(new_n59_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x24), .O(new_n69_));
  inv1   g24(.a(x25), .O(new_n70_));
  aoi21  g25(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(new_n69_), .c(new_n70_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n60_), .O(z03));
  nand2  g28(.a(new_n60_), .b(new_n46_), .O(new_n74_));
  xor2a  g29(.a(x12), .b(x03), .O(new_n75_));
  xor2a  g30(.a(x11), .b(x02), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  xor2a  g33(.a(x10), .b(x01), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z04));
  nor3   g36(.a(x23), .b(x13), .c(x08), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n60_), .O(z06));
  nand3  g39(.a(new_n60_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g40(.a(new_n59_), .b(x24), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n60_), .c(new_n70_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n61_), .c(new_n60_), .O(z09));
  nand3  g46(.a(new_n61_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n60_), .O(z10));
  nor2   g48(.a(new_n92_), .b(new_n48_), .O(new_n94_));
  nor2   g49(.a(x18), .b(x17), .O(new_n95_));
  nor2   g50(.a(new_n52_), .b(new_n51_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  inv1   g54(.a(new_n94_), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nor2   g57(.a(new_n96_), .b(x19), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z12));
  nand2  g59(.a(new_n102_), .b(new_n50_), .O(new_n105_));
  nand2  g60(.a(new_n101_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n94_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n102_), .c(new_n92_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n48_), .O(z14));
  nand2  g66(.a(new_n109_), .b(new_n102_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  nand3  g69(.a(new_n109_), .b(new_n102_), .c(new_n114_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n94_), .O(z15));
  oai21  g71(.a(new_n115_), .b(new_n92_), .c(x13), .O(new_n117_));
  inv1   g72(.a(new_n92_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n47_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  aoi22  g75(.a(new_n120_), .b(new_n115_), .c(new_n117_), .d(x23), .O(z16));
  inv1   g76(.a(new_n115_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n69_), .c(new_n47_), .O(new_n123_));
  oai22  g78(.a(new_n115_), .b(x23), .c(new_n48_), .d(x24), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n118_), .O(z17));
  nor2   g80(.a(new_n70_), .b(new_n47_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n92_), .c(new_n60_), .O(new_n127_));
  nor2   g82(.a(x24), .b(x22), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n109_), .c(new_n102_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x25), .O(new_n130_));
  nand4  g85(.a(new_n109_), .b(new_n102_), .c(new_n62_), .d(new_n114_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n47_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n127_), .O(z18));
endmodule


