// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x15), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n53_), .c(new_n52_), .O(new_n63_));
  inv1   g18(.a(x15), .O(new_n64_));
  aoi21  g19(.a(x25), .b(new_n64_), .c(x24), .O(new_n65_));
  aoi21  g20(.a(x05), .b(x04), .c(x07), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n63_), .O(z01));
  nor2   g24(.a(x18), .b(x17), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n57_), .c(x20), .O(new_n71_));
  nor2   g26(.a(new_n48_), .b(x23), .O(new_n72_));
  oai21  g27(.a(new_n60_), .b(new_n71_), .c(new_n72_), .O(new_n73_));
  nand2  g28(.a(new_n48_), .b(new_n64_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n73_), .c(x25), .O(z03));
  xnor2a g30(.a(x12), .b(x03), .O(new_n76_));
  xnor2a g31(.a(x11), .b(x02), .O(new_n77_));
  xnor2a g32(.a(x09), .b(x00), .O(new_n78_));
  xnor2a g33(.a(x10), .b(x01), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n78_), .c(new_n77_), .d(new_n76_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z04));
  oai21  g37(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g38(.a(x14), .b(new_n46_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n49_), .O(z06));
  nand3  g40(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g41(.a(new_n61_), .b(new_n59_), .c(x23), .O(new_n87_));
  aoi21  g42(.a(new_n48_), .b(x15), .c(x25), .O(new_n88_));
  oai21  g43(.a(new_n87_), .b(new_n48_), .c(new_n88_), .O(z08));
  nand2  g44(.a(x05), .b(x04), .O(new_n90_));
  inv1   g45(.a(x07), .O(new_n91_));
  nand2  g46(.a(new_n64_), .b(new_n91_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n90_), .c(new_n49_), .O(z09));
  nand2  g48(.a(new_n66_), .b(new_n64_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n55_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  inv1   g52(.a(new_n70_), .O(new_n98_));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n94_), .c(new_n49_), .O(z11));
  nand2  g56(.a(new_n99_), .b(new_n57_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(new_n94_), .c(new_n49_), .O(z12));
  nor2   g60(.a(new_n103_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n103_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n95_), .O(z13));
  nand2  g64(.a(new_n107_), .b(x21), .O(new_n110_));
  inv1   g65(.a(x21), .O(new_n111_));
  nand2  g66(.a(new_n106_), .b(new_n111_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n95_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n106_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n95_), .O(z15));
  nand3  g72(.a(new_n115_), .b(new_n106_), .c(new_n52_), .O(new_n118_));
  nand2  g73(.a(new_n116_), .b(x23), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n95_), .O(z16));
  nor2   g75(.a(x23), .b(x15), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n115_), .c(new_n106_), .d(new_n48_), .O(new_n122_));
  nand2  g77(.a(new_n67_), .b(new_n49_), .O(new_n123_));
  nand3  g78(.a(new_n121_), .b(new_n115_), .c(new_n106_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x24), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n122_), .O(z17));
  nor2   g81(.a(x24), .b(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n115_), .c(new_n106_), .d(new_n53_), .O(new_n128_));
  nand3  g83(.a(new_n127_), .b(new_n115_), .c(new_n106_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x25), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n128_), .c(new_n95_), .O(z18));
endmodule


