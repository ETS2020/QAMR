// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x22), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(x21), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n47_), .c(new_n51_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  and2   g15(.a(new_n60_), .b(new_n49_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n49_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n59_), .b(new_n49_), .O(z03));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  nor2   g28(.a(new_n66_), .b(x13), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g32(.a(x22), .O(new_n78_));
  inv1   g33(.a(x21), .O(new_n79_));
  inv1   g34(.a(x23), .O(new_n80_));
  nor2   g35(.a(x18), .b(x17), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n55_), .c(x20), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n78_), .c(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n51_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n86_), .c(x05), .d(x04), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n49_), .O(z09));
  nand2  g44(.a(new_n60_), .b(new_n87_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n49_), .O(z10));
  nor2   g46(.a(new_n90_), .b(new_n48_), .O(new_n92_));
  nor2   g47(.a(new_n54_), .b(new_n53_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(new_n81_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  inv1   g51(.a(new_n90_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n93_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(z12));
  nand2  g57(.a(new_n99_), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n100_), .b(new_n52_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n92_), .O(z13));
  nand2  g60(.a(new_n104_), .b(x21), .O(new_n106_));
  nand3  g61(.a(new_n100_), .b(new_n79_), .c(new_n52_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n92_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  nor3   g64(.a(x24), .b(x22), .c(x21), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n100_), .c(new_n52_), .O(new_n111_));
  nand2  g66(.a(new_n90_), .b(new_n49_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n109_), .O(z15));
  aoi21  g68(.a(new_n107_), .b(new_n47_), .c(x22), .O(new_n114_));
  nor3   g69(.a(new_n99_), .b(x23), .c(x20), .O(new_n115_));
  aoi22  g70(.a(new_n115_), .b(new_n110_), .c(new_n90_), .d(new_n49_), .O(new_n116_));
  oai21  g71(.a(new_n114_), .b(new_n80_), .c(new_n116_), .O(z16));
  nand4  g72(.a(new_n100_), .b(new_n80_), .c(new_n79_), .d(new_n52_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n78_), .O(new_n120_));
  nand2  g75(.a(new_n97_), .b(new_n47_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n120_), .c(new_n48_), .O(z17));
  nor3   g78(.a(x25), .b(x22), .c(x21), .O(new_n124_));
  aoi22  g79(.a(new_n124_), .b(new_n115_), .c(new_n118_), .d(x25), .O(new_n125_));
  aoi22  g80(.a(new_n90_), .b(new_n49_), .c(x25), .d(x22), .O(new_n126_));
  oai21  g81(.a(new_n125_), .b(x24), .c(new_n126_), .O(z18));
endmodule


