// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n83_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x04), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x04), .O(new_n50_));
  nor2   g05(.a(x25), .b(x24), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n50_), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(x25), .b(x23), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(new_n58_), .c(new_n62_), .O(new_n63_));
  inv1   g18(.a(x07), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(new_n63_), .c(x24), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n52_), .O(z01));
  and2   g23(.a(new_n48_), .b(x16), .O(z02));
  inv1   g24(.a(new_n63_), .O(new_n70_));
  inv1   g25(.a(x25), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x04), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x24), .c(new_n70_), .O(z03));
  xnor2a g28(.a(x12), .b(x03), .O(new_n74_));
  xnor2a g29(.a(x11), .b(x02), .O(new_n75_));
  xnor2a g30(.a(x09), .b(x00), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  nand4  g32(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(new_n74_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n48_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  inv1   g37(.a(new_n48_), .O(new_n83_));
  nor3   g38(.a(new_n83_), .b(new_n82_), .c(x08), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n83_), .O(z07));
  oai22  g40(.a(new_n63_), .b(new_n47_), .c(new_n71_), .d(x04), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x05), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x24), .c(new_n50_), .O(z09));
  oai21  g44(.a(new_n47_), .b(x05), .c(x04), .O(new_n90_));
  and2   g45(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n54_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n87_), .b(new_n65_), .O(new_n94_));
  xnor2a g49(.a(x18), .b(x17), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n94_), .c(new_n48_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n98_));
  nand4  g53(.a(new_n98_), .b(new_n97_), .c(new_n87_), .d(new_n65_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n48_), .O(z12));
  nor2   g55(.a(new_n97_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n91_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n91_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n101_), .c(new_n94_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n109_), .c(new_n83_), .O(z15));
  nand2  g67(.a(new_n110_), .b(new_n101_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x23), .O(new_n114_));
  inv1   g69(.a(x23), .O(new_n115_));
  nand3  g70(.a(new_n110_), .b(new_n101_), .c(new_n115_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n91_), .O(z16));
  nand2  g72(.a(new_n116_), .b(x24), .O(new_n118_));
  nand4  g73(.a(new_n110_), .b(new_n101_), .c(new_n47_), .d(new_n115_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n91_), .O(z17));
  oai21  g75(.a(new_n113_), .b(new_n62_), .c(new_n50_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  oai21  g77(.a(x25), .b(x05), .c(x04), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n87_), .O(new_n124_));
  aoi21  g79(.a(new_n119_), .b(x25), .c(new_n124_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n122_), .O(z18));
endmodule


