// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n113_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nor2   g02(.a(x20), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n51_), .c(new_n52_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n50_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nor2   g16(.a(new_n48_), .b(new_n61_), .O(z02));
  nor2   g17(.a(new_n58_), .b(new_n48_), .O(z03));
  inv1   g18(.a(x00), .O(new_n64_));
  nand2  g19(.a(x09), .b(new_n64_), .O(new_n65_));
  inv1   g20(.a(x02), .O(new_n66_));
  nand2  g21(.a(x11), .b(new_n66_), .O(new_n67_));
  inv1   g22(.a(x03), .O(new_n68_));
  nand2  g23(.a(x12), .b(new_n68_), .O(new_n69_));
  inv1   g24(.a(x11), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x02), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(new_n72_));
  inv1   g27(.a(x01), .O(new_n73_));
  nand2  g28(.a(x10), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x09), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x00), .O(new_n76_));
  inv1   g31(.a(x10), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x01), .O(new_n78_));
  inv1   g33(.a(x12), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n72_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(z06));
  nand3  g41(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nand2  g43(.a(new_n56_), .b(new_n54_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x24), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n52_), .c(new_n48_), .O(z08));
  nor2   g47(.a(x15), .b(x07), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n50_), .c(x05), .d(x04), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  inv1   g50(.a(x15), .O(new_n96_));
  nand2  g51(.a(new_n59_), .b(new_n96_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g53(.a(new_n97_), .b(new_n48_), .O(new_n99_));
  inv1   g54(.a(x17), .O(new_n100_));
  nand2  g55(.a(new_n47_), .b(new_n100_), .O(new_n101_));
  nand2  g56(.a(x18), .b(x17), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z11));
  xor2a  g59(.a(new_n102_), .b(x19), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n97_), .c(new_n48_), .O(z12));
  nand2  g61(.a(new_n97_), .b(new_n50_), .O(new_n107_));
  inv1   g62(.a(x19), .O(new_n108_));
  oai21  g63(.a(new_n102_), .b(new_n108_), .c(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n107_), .O(z13));
  nand2  g65(.a(new_n50_), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n107_), .O(z14));
  nand2  g67(.a(new_n50_), .b(x22), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n107_), .O(z15));
  nand2  g69(.a(new_n99_), .b(new_n88_), .O(z16));
  nand2  g70(.a(new_n99_), .b(new_n51_), .O(z17));
  oai21  g71(.a(new_n48_), .b(new_n52_), .c(new_n107_), .O(z18));
endmodule


