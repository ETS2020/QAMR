// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x07), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  aoi21  g18(.a(new_n61_), .b(new_n53_), .c(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x15), .c(new_n52_), .O(z01));
  aoi21  g20(.a(new_n61_), .b(new_n53_), .c(new_n48_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  inv1   g22(.a(new_n48_), .O(new_n68_));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x08), .c(new_n68_), .O(z04));
  inv1   g30(.a(new_n50_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(x13), .O(z05));
  and2   g32(.a(new_n50_), .b(x14), .O(z06));
  nand2  g33(.a(new_n50_), .b(x06), .O(z07));
  aoi21  g34(.a(new_n62_), .b(new_n47_), .c(x07), .O(z09));
  nand3  g35(.a(new_n62_), .b(new_n47_), .c(new_n52_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n55_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z10));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  nor2   g40(.a(new_n56_), .b(new_n55_), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n81_), .c(new_n85_), .O(z11));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n86_), .b(x19), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n89_), .c(new_n81_), .O(z12));
  nor2   g46(.a(new_n88_), .b(x20), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n88_), .b(x20), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n82_), .O(z13));
  inv1   g50(.a(x21), .O(new_n96_));
  nor2   g51(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(new_n96_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n62_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(new_n97_), .c(new_n47_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n52_), .O(z14));
  nand2  g56(.a(new_n98_), .b(x22), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n92_), .b(new_n103_), .c(new_n96_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n82_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  nor2   g61(.a(new_n104_), .b(x23), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n82_), .O(z16));
  nand2  g64(.a(new_n108_), .b(x24), .O(new_n110_));
  inv1   g65(.a(x24), .O(new_n111_));
  nand2  g66(.a(new_n107_), .b(new_n111_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n82_), .O(z17));
  nand3  g68(.a(new_n107_), .b(new_n53_), .c(new_n111_), .O(new_n114_));
  nand2  g69(.a(new_n112_), .b(x25), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n82_), .O(z18));
endmodule


