// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n130_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x19), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(z02), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(x23), .c(x24), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor3   g14(.a(x25), .b(x23), .c(x20), .O(new_n60_));
  nor2   g15(.a(x18), .b(x17), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n48_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n60_), .c(new_n59_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n57_), .c(new_n49_), .O(z01));
  oai21  g19(.a(new_n60_), .b(x15), .c(x19), .O(new_n65_));
  inv1   g20(.a(x23), .O(new_n66_));
  inv1   g21(.a(x24), .O(new_n67_));
  oai21  g22(.a(new_n61_), .b(x20), .c(new_n55_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n66_), .c(new_n67_), .O(new_n69_));
  oai21  g24(.a(new_n69_), .b(x25), .c(new_n65_), .O(z03));
  inv1   g25(.a(new_n49_), .O(new_n71_));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x08), .c(new_n71_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand2  g34(.a(new_n51_), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  nand2  g36(.a(new_n51_), .b(x14), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  nand2  g38(.a(new_n51_), .b(x06), .O(z07));
  inv1   g39(.a(x20), .O(new_n85_));
  aoi21  g40(.a(new_n62_), .b(new_n85_), .c(new_n54_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x23), .c(x24), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n71_), .c(new_n53_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  nand4  g44(.a(new_n47_), .b(new_n89_), .c(x05), .d(x04), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n58_), .b(new_n47_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x17), .c(new_n71_), .O(z10));
  and2   g48(.a(x18), .b(x17), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n92_), .c(new_n61_), .O(z11));
  nor2   g50(.a(new_n94_), .b(x19), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n96_), .c(new_n92_), .O(z12));
  oai21  g54(.a(x20), .b(x15), .c(new_n48_), .O(new_n100_));
  nor2   g55(.a(new_n94_), .b(new_n85_), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n94_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n58_), .O(new_n104_));
  oai21  g59(.a(new_n104_), .b(new_n101_), .c(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n100_), .O(z13));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n103_), .c(new_n59_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n108_), .c(new_n49_), .O(z14));
  inv1   g67(.a(x22), .O(new_n113_));
  nor3   g68(.a(new_n97_), .b(x21), .c(x20), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  aoi21  g70(.a(new_n108_), .b(x22), .c(new_n92_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n115_), .O(z15));
  nand2  g72(.a(new_n115_), .b(x23), .O(new_n118_));
  nor2   g73(.a(x23), .b(x22), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n114_), .c(new_n92_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n118_), .O(z16));
  oai21  g76(.a(x24), .b(x15), .c(new_n48_), .O(new_n122_));
  nand4  g77(.a(new_n119_), .b(new_n107_), .c(x18), .d(x17), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x24), .O(new_n124_));
  nor2   g79(.a(x24), .b(x21), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n119_), .c(new_n102_), .d(new_n94_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n58_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n47_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n122_), .O(z17));
  aoi21  g84(.a(new_n126_), .b(x25), .c(new_n92_), .O(new_n130_));
  oai21  g85(.a(new_n126_), .b(x25), .c(new_n130_), .O(z18));
endmodule


