// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x11), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  inv1   g05(.a(x08), .O(new_n51_));
  inv1   g06(.a(new_n49_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(z02), .O(z00));
  inv1   g10(.a(x25), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(x22), .b(x21), .O(new_n62_));
  aoi21  g17(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x23), .c(x24), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n56_), .O(new_n65_));
  aoi21  g20(.a(x05), .b(x04), .c(x07), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n65_), .c(new_n49_), .O(z01));
  aoi21  g22(.a(new_n64_), .b(new_n56_), .c(new_n49_), .O(z08));
  inv1   g23(.a(z08), .O(z03));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n53_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand2  g32(.a(new_n54_), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(x14), .b(new_n51_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n52_), .O(z06));
  nand2  g36(.a(new_n54_), .b(x06), .O(z07));
  inv1   g37(.a(x07), .O(new_n83_));
  nand4  g38(.a(new_n48_), .b(new_n83_), .c(x05), .d(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n52_), .O(z09));
  nand2  g40(.a(new_n66_), .b(new_n48_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n58_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  nor2   g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n86_), .c(new_n90_), .O(z11));
  nand2  g48(.a(new_n91_), .b(new_n60_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n86_), .c(new_n52_), .O(z12));
  inv1   g52(.a(new_n95_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n57_), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n87_), .O(z13));
  nand2  g56(.a(x15), .b(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n98_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n66_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n102_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  nor3   g64(.a(new_n95_), .b(x21), .c(x20), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n109_), .c(new_n86_), .O(new_n111_));
  oai21  g66(.a(new_n110_), .b(new_n109_), .c(new_n111_), .O(z15));
  oai21  g67(.a(new_n105_), .b(x22), .c(x23), .O(new_n113_));
  nor2   g68(.a(x23), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n104_), .c(new_n98_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n66_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n102_), .O(z16));
  nand2  g73(.a(new_n115_), .b(x24), .O(new_n119_));
  inv1   g74(.a(x24), .O(new_n120_));
  nand3  g75(.a(new_n114_), .b(new_n110_), .c(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n66_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n102_), .O(z17));
  aoi21  g79(.a(new_n121_), .b(x25), .c(new_n86_), .O(new_n125_));
  oai21  g80(.a(new_n121_), .b(x25), .c(new_n125_), .O(z18));
endmodule


