// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x22), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x24), .O(new_n53_));
  nor3   g08(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  oai21  g09(.a(new_n54_), .b(x20), .c(x21), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n52_), .c(new_n53_), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  oai21  g12(.a(x25), .b(x23), .c(x24), .O(new_n58_));
  inv1   g13(.a(x07), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g16(.a(new_n58_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n56_), .b(x25), .c(new_n62_), .O(z01));
  oai21  g18(.a(new_n55_), .b(new_n57_), .c(new_n52_), .O(new_n64_));
  inv1   g19(.a(x25), .O(new_n65_));
  nand2  g20(.a(new_n49_), .b(new_n65_), .O(new_n66_));
  aoi21  g21(.a(new_n64_), .b(x24), .c(new_n66_), .O(z03));
  nand2  g22(.a(new_n49_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n68_), .b(new_n77_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g34(.a(x20), .O(new_n80_));
  inv1   g35(.a(x21), .O(new_n81_));
  inv1   g36(.a(x19), .O(new_n82_));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n80_), .c(new_n81_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x22), .c(x23), .O(new_n86_));
  nand2  g41(.a(new_n49_), .b(x25), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(new_n53_), .c(new_n87_), .O(z08));
  inv1   g43(.a(new_n60_), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n59_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n48_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  nand3  g49(.a(new_n60_), .b(new_n90_), .c(new_n59_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g51(.a(new_n92_), .b(new_n60_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  xor2a  g53(.a(x18), .b(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  aoi21  g58(.a(x18), .b(x17), .c(x19), .O(new_n104_));
  nor3   g59(.a(new_n104_), .b(new_n103_), .c(new_n97_), .O(z12));
  nor2   g60(.a(new_n102_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n98_), .O(z13));
  nand2  g64(.a(new_n107_), .b(x21), .O(new_n110_));
  nand2  g65(.a(new_n106_), .b(new_n81_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n98_), .O(z14));
  nand2  g67(.a(new_n95_), .b(new_n49_), .O(new_n113_));
  nand2  g68(.a(new_n111_), .b(x22), .O(new_n114_));
  nand4  g69(.a(new_n106_), .b(x24), .c(new_n57_), .d(new_n81_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(z15));
  nand3  g71(.a(new_n106_), .b(new_n52_), .c(new_n81_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(x24), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n57_), .O(new_n119_));
  aoi21  g74(.a(new_n115_), .b(x23), .c(new_n95_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(z16));
  oai21  g76(.a(new_n117_), .b(x22), .c(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n98_), .O(z17));
  nand2  g78(.a(new_n113_), .b(new_n87_), .O(z18));
endmodule


