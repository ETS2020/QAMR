// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  aoi21  g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x20), .O(new_n49_));
  inv1   g04(.a(x17), .O(new_n50_));
  inv1   g05(.a(x18), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nand3  g07(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  and2   g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x23), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n46_), .c(new_n47_), .O(new_n57_));
  nand2  g12(.a(x24), .b(x08), .O(new_n58_));
  inv1   g13(.a(x07), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  oai21  g17(.a(new_n57_), .b(x25), .c(new_n62_), .O(z01));
  and2   g18(.a(new_n58_), .b(x16), .O(z02));
  inv1   g19(.a(x25), .O(new_n65_));
  oai21  g20(.a(new_n56_), .b(new_n47_), .c(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n58_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n58_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n58_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(x08), .O(z06));
  inv1   g32(.a(new_n58_), .O(new_n78_));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  nand2  g34(.a(new_n56_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n65_), .O(z08));
  inv1   g37(.a(x15), .O(new_n83_));
  nand3  g38(.a(new_n58_), .b(new_n83_), .c(new_n59_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n60_), .O(z09));
  inv1   g40(.a(new_n84_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n60_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n50_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand3  g45(.a(new_n60_), .b(new_n83_), .c(new_n59_), .O(new_n91_));
  nand2  g46(.a(new_n51_), .b(new_n50_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n91_), .c(new_n58_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(new_n52_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n87_), .O(z12));
  nor2   g54(.a(new_n96_), .b(x20), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n88_), .O(z13));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n104_));
  inv1   g59(.a(x21), .O(new_n105_));
  nand2  g60(.a(new_n100_), .b(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n88_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  nor2   g63(.a(x22), .b(x21), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n100_), .c(new_n91_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n78_), .O(z15));
  nand2  g66(.a(new_n109_), .b(new_n100_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x23), .O(new_n113_));
  inv1   g68(.a(new_n91_), .O(new_n114_));
  inv1   g69(.a(x23), .O(new_n115_));
  nand3  g70(.a(new_n109_), .b(new_n100_), .c(new_n115_), .O(new_n116_));
  and2   g71(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n113_), .c(new_n78_), .O(z16));
  aoi21  g73(.a(new_n116_), .b(x24), .c(new_n87_), .O(new_n119_));
  oai21  g74(.a(new_n116_), .b(x24), .c(new_n119_), .O(z17));
  nor2   g75(.a(new_n65_), .b(new_n47_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n91_), .c(new_n58_), .O(new_n122_));
  nor2   g77(.a(new_n116_), .b(new_n65_), .O(new_n123_));
  nand2  g78(.a(new_n116_), .b(new_n65_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n123_), .c(new_n122_), .O(z18));
endmodule


