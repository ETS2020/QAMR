// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n75_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x13), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  or2    g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x23), .c(x24), .O(new_n62_));
  nor2   g17(.a(new_n49_), .b(x25), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(z08));
  nand2  g19(.a(z08), .b(new_n54_), .O(z01));
  inv1   g20(.a(z08), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n49_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g32(.a(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x05), .c(x04), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n48_), .c(x15), .O(z09));
  inv1   g35(.a(x15), .O(new_n81_));
  nand3  g36(.a(new_n53_), .b(new_n81_), .c(new_n48_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n56_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  nor2   g40(.a(x18), .b(x17), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n82_), .c(new_n86_), .O(z11));
  nand2  g44(.a(new_n87_), .b(new_n58_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n53_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n48_), .c(x15), .O(z12));
  nand4  g48(.a(new_n55_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(x20), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n83_), .O(z13));
  nand2  g51(.a(new_n94_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  nor2   g53(.a(new_n91_), .b(x20), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n97_), .c(new_n53_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n81_), .O(z14));
  nand2  g58(.a(new_n100_), .b(x22), .O(new_n104_));
  nor2   g59(.a(x22), .b(x21), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n99_), .c(new_n82_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n104_), .O(z15));
  inv1   g62(.a(x23), .O(new_n108_));
  aoi21  g63(.a(new_n105_), .b(new_n99_), .c(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n99_), .c(new_n108_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n53_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n109_), .c(new_n48_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n81_), .O(z16));
  nand2  g68(.a(new_n110_), .b(x24), .O(new_n114_));
  nor2   g69(.a(x24), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n105_), .c(new_n99_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n83_), .O(z17));
  inv1   g72(.a(x25), .O(new_n118_));
  nand4  g73(.a(new_n115_), .b(new_n105_), .c(new_n99_), .d(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n115_), .b(new_n105_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n94_), .c(x25), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n53_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n81_), .O(z18));
endmodule


