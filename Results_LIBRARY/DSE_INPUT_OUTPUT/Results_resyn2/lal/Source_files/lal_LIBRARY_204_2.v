// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n87_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x10), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  inv1   g14(.a(x24), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x23), .O(new_n61_));
  nor2   g16(.a(x24), .b(x10), .O(new_n62_));
  aoi21  g17(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x07), .c(new_n49_), .O(new_n66_));
  oai21  g21(.a(new_n63_), .b(x25), .c(new_n66_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n48_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n63_), .b(x25), .O(z03));
  nand2  g25(.a(new_n49_), .b(new_n46_), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  nand2  g28(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x09), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x00), .O(new_n76_));
  nand3  g31(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  inv1   g33(.a(x00), .O(new_n79_));
  nand2  g34(.a(x09), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(x12), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(x03), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n71_), .O(z04));
  oai21  g40(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g41(.a(x14), .O(new_n87_));
  nor2   g42(.a(new_n71_), .b(new_n87_), .O(z06));
  aoi21  g43(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g44(.a(x25), .O(new_n90_));
  aoi21  g45(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n91_));
  inv1   g46(.a(x23), .O(new_n92_));
  nand2  g47(.a(new_n90_), .b(new_n92_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n91_), .c(x24), .O(new_n94_));
  oai21  g49(.a(new_n90_), .b(x10), .c(new_n94_), .O(z08));
  nor2   g50(.a(x15), .b(x07), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n65_), .c(new_n49_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z09));
  nand3  g53(.a(new_n96_), .b(new_n64_), .c(new_n49_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n52_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(z10));
  nand2  g57(.a(new_n96_), .b(new_n64_), .O(new_n103_));
  nand2  g58(.a(new_n53_), .b(new_n52_), .O(new_n104_));
  nand2  g59(.a(x18), .b(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n103_), .c(new_n49_), .O(z11));
  nand3  g62(.a(x19), .b(x18), .c(x17), .O(new_n108_));
  nand2  g63(.a(new_n105_), .b(new_n54_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n99_), .O(z12));
  nor2   g66(.a(new_n108_), .b(x20), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(x20), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n100_), .O(z13));
  nand2  g70(.a(new_n113_), .b(x21), .O(new_n116_));
  inv1   g71(.a(x21), .O(new_n117_));
  nand2  g72(.a(new_n112_), .b(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n100_), .O(z14));
  nand2  g74(.a(new_n118_), .b(x22), .O(new_n120_));
  nor2   g75(.a(x22), .b(x21), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n112_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n100_), .O(z15));
  nand2  g78(.a(new_n122_), .b(x23), .O(new_n124_));
  nand3  g79(.a(new_n121_), .b(new_n112_), .c(new_n92_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n100_), .O(z16));
  nand2  g81(.a(new_n125_), .b(x24), .O(new_n127_));
  nand4  g82(.a(new_n121_), .b(new_n112_), .c(new_n60_), .d(new_n92_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n100_), .O(z17));
  xor2a  g84(.a(new_n128_), .b(x25), .O(new_n130_));
  aoi21  g85(.a(new_n90_), .b(x24), .c(new_n47_), .O(new_n131_));
  nor2   g86(.a(new_n131_), .b(new_n103_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n130_), .O(z18));
endmodule


