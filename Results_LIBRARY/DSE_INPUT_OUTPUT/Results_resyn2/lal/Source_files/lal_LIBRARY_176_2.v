// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x23), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(x23), .c(new_n48_), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  nor2   g12(.a(x25), .b(x23), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n53_), .O(z01));
  nand2  g16(.a(new_n59_), .b(new_n53_), .O(z03));
  xnor2a g17(.a(x12), .b(x03), .O(new_n63_));
  xnor2a g18(.a(x11), .b(x02), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n49_), .O(z04));
  oai21  g24(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g25(.a(x14), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z06));
  inv1   g27(.a(x23), .O(new_n73_));
  nor2   g28(.a(x24), .b(new_n73_), .O(new_n74_));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n74_), .O(z07));
  inv1   g30(.a(x20), .O(new_n76_));
  inv1   g31(.a(x17), .O(new_n77_));
  inv1   g32(.a(x18), .O(new_n78_));
  nand3  g33(.a(new_n54_), .b(new_n78_), .c(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n76_), .c(new_n57_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n58_), .O(z08));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(x05), .c(x04), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n49_), .O(z09));
  nand2  g40(.a(x05), .b(x04), .O(new_n86_));
  nand2  g41(.a(new_n83_), .b(new_n86_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n77_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  inv1   g45(.a(new_n55_), .O(new_n91_));
  inv1   g46(.a(new_n87_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand4  g48(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(new_n49_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n93_), .b(new_n54_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n92_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n49_), .O(z12));
  inv1   g54(.a(new_n96_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n76_), .O(new_n101_));
  aoi21  g56(.a(new_n96_), .b(x20), .c(new_n87_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n74_), .O(z13));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n100_), .c(new_n87_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n104_), .c(new_n74_), .O(z14));
  nand2  g62(.a(new_n105_), .b(new_n100_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x22), .O(new_n109_));
  nor3   g64(.a(x22), .b(x21), .c(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n100_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n88_), .O(z15));
  nand3  g67(.a(new_n111_), .b(x24), .c(x23), .O(new_n113_));
  nand3  g68(.a(new_n110_), .b(new_n100_), .c(new_n73_), .O(new_n114_));
  nand2  g69(.a(new_n87_), .b(new_n49_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(z16));
  nand4  g71(.a(new_n110_), .b(new_n100_), .c(new_n48_), .d(new_n73_), .O(new_n117_));
  nand2  g72(.a(new_n114_), .b(x24), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n115_), .O(z17));
  nand2  g74(.a(new_n52_), .b(new_n48_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n110_), .c(new_n100_), .O(new_n121_));
  inv1   g76(.a(x21), .O(new_n122_));
  inv1   g77(.a(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n76_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n96_), .c(new_n52_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n121_), .c(new_n73_), .O(new_n126_));
  aoi22  g81(.a(new_n87_), .b(new_n49_), .c(x25), .d(x24), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z18));
endmodule


