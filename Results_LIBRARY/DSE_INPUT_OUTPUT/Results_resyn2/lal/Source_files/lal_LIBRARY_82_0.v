// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x18), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  aoi21  g04(.a(x25), .b(x18), .c(x24), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n57_), .c(new_n51_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n57_), .b(new_n51_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n48_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor3   g28(.a(new_n47_), .b(new_n73_), .c(x08), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  nor3   g30(.a(x19), .b(x18), .c(x17), .O(new_n76_));
  inv1   g31(.a(new_n55_), .O(new_n77_));
  oai21  g32(.a(new_n76_), .b(x20), .c(new_n77_), .O(new_n78_));
  aoi21  g33(.a(new_n56_), .b(new_n78_), .c(new_n50_), .O(z08));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand4  g36(.a(new_n59_), .b(new_n48_), .c(new_n81_), .d(new_n80_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand3  g38(.a(new_n58_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  nor3   g39(.a(new_n84_), .b(new_n47_), .c(x17), .O(z10));
  xnor2a g40(.a(x18), .b(x17), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n84_), .c(new_n48_), .O(z11));
  inv1   g42(.a(new_n84_), .O(new_n88_));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n48_), .O(z12));
  oai21  g48(.a(new_n47_), .b(x20), .c(new_n89_), .O(new_n94_));
  inv1   g49(.a(x20), .O(new_n95_));
  inv1   g50(.a(new_n89_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n94_), .c(new_n88_), .O(z13));
  nand2  g53(.a(new_n97_), .b(x21), .O(new_n99_));
  nor2   g54(.a(x21), .b(x20), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n96_), .c(new_n84_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n99_), .c(new_n47_), .O(z14));
  inv1   g57(.a(x22), .O(new_n103_));
  and2   g58(.a(x19), .b(x17), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n100_), .c(new_n103_), .d(x18), .O(new_n105_));
  nand3  g60(.a(new_n100_), .b(x19), .c(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x18), .O(new_n108_));
  inv1   g63(.a(x24), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(x22), .c(new_n108_), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n107_), .c(new_n105_), .d(new_n88_), .O(z15));
  inv1   g66(.a(new_n106_), .O(new_n112_));
  nor2   g67(.a(x23), .b(x22), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(x18), .O(new_n114_));
  oai21  g69(.a(new_n106_), .b(x22), .c(x23), .O(new_n115_));
  inv1   g70(.a(x23), .O(new_n116_));
  nand2  g71(.a(x24), .b(new_n116_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n108_), .c(new_n84_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n114_), .O(z16));
  nand4  g74(.a(new_n113_), .b(new_n104_), .c(new_n100_), .d(new_n109_), .O(new_n120_));
  nand3  g75(.a(new_n113_), .b(new_n104_), .c(new_n100_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x24), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n120_), .c(new_n88_), .d(x18), .O(z17));
  nand2  g78(.a(new_n120_), .b(x25), .O(new_n124_));
  nor2   g79(.a(x25), .b(x24), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n113_), .c(new_n112_), .O(new_n126_));
  inv1   g81(.a(x25), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x24), .O(new_n128_));
  aoi21  g83(.a(new_n128_), .b(new_n108_), .c(new_n84_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n126_), .c(new_n124_), .O(z18));
endmodule


