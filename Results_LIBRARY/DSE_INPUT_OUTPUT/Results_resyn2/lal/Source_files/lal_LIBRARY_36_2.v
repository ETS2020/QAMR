// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x23), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand4  g13(.a(new_n58_), .b(x24), .c(x22), .d(x21), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n52_), .c(new_n51_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x07), .c(new_n48_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  and2   g19(.a(new_n48_), .b(x16), .O(z02));
  inv1   g20(.a(new_n60_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n48_), .O(z04));
  nand2  g28(.a(new_n48_), .b(new_n46_), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(x13), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor2   g31(.a(new_n74_), .b(new_n76_), .O(z06));
  nand3  g32(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g33(.a(x22), .b(x21), .O(new_n79_));
  aoi21  g34(.a(new_n57_), .b(new_n53_), .c(new_n79_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x23), .c(x24), .O(new_n81_));
  nand2  g36(.a(x25), .b(new_n51_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n62_), .c(new_n48_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand3  g41(.a(new_n84_), .b(new_n61_), .c(new_n48_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n54_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nand2  g45(.a(new_n55_), .b(new_n54_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n88_), .c(new_n91_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(x18), .b(x17), .c(x19), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n96_), .c(new_n87_), .O(z12));
  nand2  g53(.a(new_n96_), .b(new_n53_), .O(new_n99_));
  nand2  g54(.a(new_n84_), .b(new_n61_), .O(new_n100_));
  aoi21  g55(.a(new_n95_), .b(x20), .c(new_n100_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n99_), .c(new_n49_), .O(z13));
  nand2  g57(.a(new_n99_), .b(x21), .O(new_n103_));
  inv1   g58(.a(x21), .O(new_n104_));
  nand3  g59(.a(new_n96_), .b(new_n104_), .c(new_n53_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n88_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  nand2  g63(.a(new_n104_), .b(new_n53_), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(new_n95_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n100_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n107_), .c(new_n49_), .O(z15));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nand3  g68(.a(new_n110_), .b(x24), .c(new_n108_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x23), .O(new_n115_));
  nand3  g70(.a(new_n110_), .b(new_n51_), .c(new_n108_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n113_), .O(z16));
  nand4  g72(.a(new_n110_), .b(new_n47_), .c(new_n51_), .d(new_n108_), .O(new_n118_));
  nand2  g73(.a(new_n100_), .b(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n116_), .b(x24), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(z17));
  nand2  g76(.a(new_n52_), .b(x24), .O(new_n122_));
  nand2  g77(.a(x25), .b(new_n47_), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n122_), .c(new_n110_), .d(new_n108_), .O(new_n124_));
  oai21  g79(.a(new_n105_), .b(x22), .c(x25), .O(new_n125_));
  aoi21  g80(.a(new_n122_), .b(x23), .c(new_n100_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n124_), .O(z18));
endmodule


