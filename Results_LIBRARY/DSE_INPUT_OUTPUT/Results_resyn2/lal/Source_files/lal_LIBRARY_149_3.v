// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  and2   g17(.a(x05), .b(x04), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z01));
  nor2   g20(.a(new_n49_), .b(new_n46_), .O(z02));
  nand3  g21(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(z08));
  inv1   g22(.a(z08), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  nand2  g24(.a(new_n50_), .b(new_n69_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  inv1   g32(.a(x13), .O(new_n78_));
  inv1   g33(.a(new_n70_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  nand2  g36(.a(x14), .b(new_n69_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n50_), .O(z06));
  nand2  g38(.a(new_n79_), .b(x06), .O(z07));
  nand3  g39(.a(new_n63_), .b(new_n48_), .c(new_n47_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n64_), .b(new_n48_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n50_), .O(z10));
  nand2  g43(.a(new_n56_), .b(new_n55_), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n87_), .c(new_n50_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n90_), .b(new_n57_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n87_), .O(z12));
  nand2  g51(.a(new_n48_), .b(x07), .O(new_n97_));
  nor2   g52(.a(new_n93_), .b(x20), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n93_), .b(x20), .O(new_n100_));
  nor2   g55(.a(new_n63_), .b(x15), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n97_), .O(z13));
  inv1   g59(.a(x21), .O(new_n105_));
  nor2   g60(.a(new_n98_), .b(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n98_), .b(new_n105_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n101_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n106_), .c(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n97_), .O(z14));
  inv1   g65(.a(x22), .O(new_n111_));
  aoi21  g66(.a(new_n98_), .b(new_n105_), .c(new_n111_), .O(new_n112_));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n98_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n101_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(new_n47_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n97_), .O(z15));
  nand2  g72(.a(new_n114_), .b(x23), .O(new_n118_));
  inv1   g73(.a(x23), .O(new_n119_));
  nand3  g74(.a(new_n113_), .b(new_n98_), .c(new_n119_), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n118_), .c(new_n64_), .d(new_n48_), .O(z16));
  nor2   g76(.a(x24), .b(x23), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n113_), .c(new_n98_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n101_), .O(new_n124_));
  aoi21  g79(.a(new_n120_), .b(x24), .c(new_n124_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(x07), .c(new_n97_), .O(z17));
  nand4  g81(.a(new_n122_), .b(new_n113_), .c(new_n98_), .d(new_n53_), .O(new_n127_));
  nand2  g82(.a(new_n123_), .b(x25), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n101_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n97_), .O(z18));
endmodule


