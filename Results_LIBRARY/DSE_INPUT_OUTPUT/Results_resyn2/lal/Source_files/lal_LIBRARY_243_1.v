// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x02), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  inv1   g04(.a(x08), .O(new_n50_));
  inv1   g05(.a(new_n48_), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(z02), .O(z00));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x23), .c(x24), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  aoi21  g19(.a(x05), .b(x04), .c(x07), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(z01));
  aoi21  g21(.a(new_n63_), .b(new_n55_), .c(new_n48_), .O(z08));
  inv1   g22(.a(z08), .O(z03));
  aoi21  g23(.a(x11), .b(new_n50_), .c(x15), .O(new_n69_));
  inv1   g24(.a(x09), .O(new_n70_));
  inv1   g25(.a(x12), .O(new_n71_));
  aoi22  g26(.a(new_n71_), .b(x03), .c(new_n70_), .d(x00), .O(new_n72_));
  oai21  g27(.a(new_n70_), .b(x00), .c(new_n72_), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  inv1   g29(.a(x03), .O(new_n75_));
  nand2  g30(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x11), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x02), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n73_), .c(new_n50_), .O(new_n80_));
  oai21  g35(.a(new_n69_), .b(x02), .c(new_n80_), .O(z04));
  nor2   g36(.a(new_n52_), .b(x13), .O(z05));
  inv1   g37(.a(x14), .O(new_n83_));
  nor2   g38(.a(new_n52_), .b(new_n83_), .O(z06));
  nand2  g39(.a(new_n53_), .b(x06), .O(z07));
  inv1   g40(.a(x07), .O(new_n86_));
  nand4  g41(.a(new_n47_), .b(new_n86_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand2  g43(.a(new_n65_), .b(new_n47_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n51_), .O(z10));
  nand2  g45(.a(new_n58_), .b(new_n57_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n89_), .c(new_n51_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n59_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n89_), .c(new_n51_), .O(z12));
  inv1   g53(.a(new_n89_), .O(new_n99_));
  nor2   g54(.a(new_n95_), .b(x20), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n95_), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n99_), .O(z13));
  nand2  g58(.a(x15), .b(x02), .O(new_n104_));
  inv1   g59(.a(x21), .O(new_n105_));
  nor2   g60(.a(new_n100_), .b(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n100_), .b(new_n105_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n65_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n106_), .c(new_n47_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n104_), .O(z14));
  inv1   g65(.a(x22), .O(new_n111_));
  aoi21  g66(.a(new_n100_), .b(new_n105_), .c(new_n111_), .O(new_n112_));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n100_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n65_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(new_n47_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n104_), .O(z15));
  nand2  g72(.a(new_n114_), .b(x23), .O(new_n118_));
  inv1   g73(.a(x23), .O(new_n119_));
  nand3  g74(.a(new_n113_), .b(new_n100_), .c(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n99_), .O(z16));
  nand2  g76(.a(new_n120_), .b(x24), .O(new_n122_));
  nor2   g77(.a(x24), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n113_), .c(new_n100_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n99_), .O(z17));
  or2    g80(.a(new_n124_), .b(x25), .O(new_n126_));
  aoi21  g81(.a(new_n124_), .b(x25), .c(new_n89_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(new_n126_), .c(new_n48_), .O(z18));
endmodule


