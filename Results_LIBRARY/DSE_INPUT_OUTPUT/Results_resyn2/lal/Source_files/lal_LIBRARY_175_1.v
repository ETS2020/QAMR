// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n94_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n107_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x17), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x15), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x19), .b(x18), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  nand3  g13(.a(new_n51_), .b(new_n58_), .c(new_n52_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(x15), .c(new_n47_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n57_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n60_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n57_), .O(z03));
  inv1   g23(.a(new_n48_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n69_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor2   g34(.a(new_n70_), .b(new_n79_), .O(z06));
  nand3  g35(.a(new_n69_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x18), .O(new_n82_));
  inv1   g37(.a(x19), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n84_));
  oai21  g39(.a(new_n54_), .b(new_n84_), .c(new_n58_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n51_), .c(new_n60_), .O(z08));
  inv1   g42(.a(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n47_), .c(x15), .O(z09));
  inv1   g45(.a(x15), .O(new_n91_));
  nand3  g46(.a(new_n61_), .b(new_n47_), .c(new_n91_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n61_), .b(x18), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n47_), .c(x15), .O(z11));
  nand2  g50(.a(new_n61_), .b(x19), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n47_), .c(x15), .O(z12));
  oai21  g52(.a(new_n62_), .b(x20), .c(new_n47_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n91_), .O(z13));
  inv1   g54(.a(x21), .O(new_n100_));
  nand2  g55(.a(z10), .b(new_n100_), .O(z14));
  oai21  g56(.a(new_n62_), .b(x22), .c(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n91_), .O(z15));
  nand2  g58(.a(z10), .b(new_n58_), .O(z16));
  oai21  g59(.a(new_n62_), .b(x24), .c(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n91_), .O(z17));
  oai21  g61(.a(new_n62_), .b(x25), .c(new_n47_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n91_), .O(z18));
endmodule


