// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x17), .O(new_n48_));
  nor2   g03(.a(x21), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  oai21  g10(.a(x19), .b(x18), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n53_), .c(new_n54_), .O(new_n60_));
  inv1   g15(.a(x23), .O(new_n61_));
  nand3  g16(.a(new_n54_), .b(new_n61_), .c(new_n55_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(x21), .O(new_n63_));
  inv1   g18(.a(x07), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi21  g21(.a(new_n63_), .b(x17), .c(new_n66_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n60_), .O(z01));
  nand2  g23(.a(new_n63_), .b(x17), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n60_), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g33(.a(x14), .b(new_n46_), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n49_), .O(z06));
  nand3  g35(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g36(.a(x18), .O(new_n82_));
  inv1   g37(.a(x19), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n82_), .c(new_n48_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n55_), .c(new_n57_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x23), .c(x24), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n50_), .c(new_n54_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n65_), .c(new_n50_), .O(z09));
  nand2  g45(.a(new_n88_), .b(new_n65_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g47(.a(new_n91_), .b(new_n49_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand2  g49(.a(new_n82_), .b(new_n48_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  xor2a  g52(.a(new_n94_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n91_), .c(new_n49_), .O(z12));
  inv1   g54(.a(x21), .O(new_n100_));
  nor2   g55(.a(new_n83_), .b(new_n82_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n100_), .c(x17), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n91_), .b(new_n50_), .O(new_n104_));
  nand4  g59(.a(new_n101_), .b(x21), .c(new_n55_), .d(x17), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(z13));
  inv1   g61(.a(new_n91_), .O(new_n107_));
  oai21  g62(.a(x21), .b(x17), .c(new_n105_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  aoi21  g65(.a(new_n107_), .b(new_n110_), .c(new_n49_), .O(z15));
  aoi21  g66(.a(new_n107_), .b(new_n61_), .c(new_n49_), .O(z16));
  nand2  g67(.a(new_n93_), .b(new_n53_), .O(z17));
  aoi21  g68(.a(new_n107_), .b(new_n54_), .c(new_n49_), .O(z18));
endmodule


