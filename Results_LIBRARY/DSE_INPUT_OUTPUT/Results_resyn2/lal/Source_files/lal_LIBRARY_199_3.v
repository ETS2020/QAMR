// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x13), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n47_), .O(z01));
  nand2  g16(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g17(.a(new_n59_), .b(new_n47_), .O(z03));
  inv1   g18(.a(x08), .O(new_n64_));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n64_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n47_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g29(.a(new_n47_), .O(new_n75_));
  aoi21  g30(.a(new_n64_), .b(x06), .c(new_n75_), .O(z07));
  aoi21  g31(.a(new_n58_), .b(new_n50_), .c(new_n75_), .O(z08));
  nand2  g32(.a(x05), .b(x04), .O(new_n78_));
  inv1   g33(.a(x07), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n47_), .b(new_n80_), .c(new_n79_), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(new_n78_), .O(z09));
  nand3  g37(.a(new_n78_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n47_), .O(z10));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  inv1   g40(.a(new_n81_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n78_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n87_), .c(new_n85_), .O(z11));
  inv1   g45(.a(new_n83_), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand2  g47(.a(new_n88_), .b(new_n54_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n47_), .O(z12));
  nand4  g50(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n96_));
  aoi21  g51(.a(new_n92_), .b(x20), .c(new_n83_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n96_), .c(new_n75_), .O(z13));
  nand2  g53(.a(new_n96_), .b(x21), .O(new_n99_));
  inv1   g54(.a(x21), .O(new_n100_));
  nor2   g55(.a(new_n92_), .b(x20), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n100_), .c(new_n83_), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n99_), .c(new_n75_), .O(z14));
  oai21  g58(.a(new_n96_), .b(x21), .c(x22), .O(new_n104_));
  nor2   g59(.a(x22), .b(x21), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n101_), .c(new_n87_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n104_), .O(z15));
  inv1   g62(.a(new_n105_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n96_), .c(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  nand3  g65(.a(new_n105_), .b(new_n101_), .c(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n91_), .O(new_n112_));
  and2   g67(.a(new_n112_), .b(new_n47_), .O(z16));
  aoi21  g68(.a(new_n111_), .b(x24), .c(new_n87_), .O(new_n114_));
  oai21  g69(.a(new_n111_), .b(x24), .c(new_n114_), .O(z17));
  inv1   g70(.a(x24), .O(new_n116_));
  nor2   g71(.a(new_n50_), .b(new_n116_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n83_), .c(new_n47_), .O(new_n118_));
  nor2   g73(.a(new_n111_), .b(new_n50_), .O(new_n119_));
  nand2  g74(.a(new_n111_), .b(new_n50_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(z18));
endmodule


