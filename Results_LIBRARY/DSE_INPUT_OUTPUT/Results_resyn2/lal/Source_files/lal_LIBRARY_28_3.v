// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x19), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  oai21  g06(.a(x18), .b(x17), .c(new_n51_), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n52_), .c(x23), .O(new_n55_));
  oai21  g10(.a(new_n55_), .b(new_n49_), .c(new_n50_), .O(new_n56_));
  inv1   g11(.a(x07), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n47_), .c(new_n57_), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(z01));
  and2   g16(.a(new_n47_), .b(x16), .O(z02));
  nand2  g17(.a(new_n56_), .b(new_n47_), .O(z03));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x11), .b(x02), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n47_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g28(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g29(.a(x23), .O(new_n75_));
  inv1   g30(.a(x17), .O(new_n76_));
  inv1   g31(.a(x18), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n76_), .c(x20), .O(new_n78_));
  oai21  g33(.a(new_n53_), .b(new_n78_), .c(new_n75_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x19), .c(x24), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z08));
  nand3  g36(.a(new_n47_), .b(x05), .c(x04), .O(new_n82_));
  nor3   g37(.a(new_n82_), .b(x15), .c(x07), .O(z09));
  inv1   g38(.a(x15), .O(new_n84_));
  nand3  g39(.a(new_n58_), .b(new_n84_), .c(new_n57_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g41(.a(new_n77_), .b(new_n76_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n85_), .c(new_n47_), .O(z11));
  inv1   g45(.a(new_n88_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x19), .O(new_n92_));
  inv1   g47(.a(new_n85_), .O(new_n93_));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n92_), .c(new_n47_), .O(z12));
  oai21  g51(.a(new_n94_), .b(x24), .c(x20), .O(new_n97_));
  nor2   g52(.a(new_n88_), .b(x20), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(x24), .c(x19), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n93_), .O(z13));
  nand2  g55(.a(new_n85_), .b(new_n47_), .O(new_n101_));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  inv1   g57(.a(new_n94_), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n49_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n102_), .c(new_n101_), .O(z14));
  or2    g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x19), .O(new_n108_));
  aoi21  g63(.a(new_n104_), .b(new_n91_), .c(x24), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n108_), .c(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n107_), .c(new_n101_), .O(z15));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n98_), .c(x24), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n108_), .c(x23), .O(new_n114_));
  nor3   g69(.a(x23), .b(x22), .c(x21), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n98_), .c(x19), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n93_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n114_), .O(z16));
  nand2  g74(.a(new_n117_), .b(new_n49_), .O(new_n120_));
  nand2  g75(.a(x24), .b(new_n108_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(z17));
  nand2  g77(.a(new_n116_), .b(new_n50_), .O(new_n123_));
  nand3  g78(.a(new_n115_), .b(new_n98_), .c(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n49_), .O(new_n125_));
  nand2  g80(.a(x25), .b(new_n108_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n101_), .O(z18));
endmodule


