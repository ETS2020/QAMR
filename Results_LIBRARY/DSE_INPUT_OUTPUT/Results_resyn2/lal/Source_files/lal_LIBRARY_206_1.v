// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x14), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(x14), .c(new_n50_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x07), .c(new_n48_), .O(new_n64_));
  oai21  g19(.a(new_n61_), .b(x25), .c(new_n64_), .O(z01));
  and2   g20(.a(new_n48_), .b(x16), .O(z02));
  nor2   g21(.a(new_n61_), .b(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n48_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  oai21  g30(.a(new_n47_), .b(x08), .c(new_n48_), .O(z06));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x25), .O(new_n78_));
  inv1   g33(.a(new_n48_), .O(new_n79_));
  inv1   g34(.a(x23), .O(new_n80_));
  nand2  g35(.a(new_n59_), .b(new_n56_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n78_), .c(new_n79_), .O(z08));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  and2   g40(.a(new_n85_), .b(new_n48_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n63_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  nand2  g43(.a(new_n86_), .b(new_n62_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  nand2  g47(.a(new_n85_), .b(new_n62_), .O(new_n93_));
  xnor2a g48(.a(x18), .b(x17), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n93_), .c(new_n48_), .O(z11));
  inv1   g50(.a(new_n93_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  oai21  g52(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n48_), .O(z12));
  nor2   g55(.a(new_n97_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n90_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  aoi21  g60(.a(new_n101_), .b(new_n57_), .c(new_n93_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n79_), .O(z14));
  oai21  g62(.a(new_n102_), .b(x21), .c(x22), .O(new_n108_));
  nand3  g63(.a(new_n101_), .b(new_n58_), .c(new_n57_), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n93_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n108_), .c(new_n79_), .O(z15));
  nand2  g67(.a(new_n109_), .b(x23), .O(new_n113_));
  nand4  g68(.a(new_n101_), .b(new_n80_), .c(new_n58_), .d(new_n57_), .O(new_n114_));
  and2   g69(.a(new_n114_), .b(new_n96_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n113_), .c(new_n79_), .O(z16));
  nand3  g71(.a(new_n114_), .b(x24), .c(x14), .O(new_n117_));
  nand2  g72(.a(new_n93_), .b(new_n48_), .O(new_n118_));
  nand3  g73(.a(new_n110_), .b(new_n50_), .c(new_n80_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(z17));
  nand4  g75(.a(new_n110_), .b(new_n78_), .c(new_n50_), .d(new_n80_), .O(new_n121_));
  nand2  g76(.a(new_n48_), .b(new_n78_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n96_), .O(z18));
endmodule


