// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  aoi21  g05(.a(x05), .b(x04), .c(x07), .O(new_n51_));
  aoi21  g06(.a(x25), .b(x15), .c(x24), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  nor2   g12(.a(x25), .b(x23), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n53_), .c(new_n51_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(new_n49_), .b(new_n61_), .O(z02));
  nand2  g17(.a(new_n59_), .b(new_n53_), .O(z03));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x11), .b(x02), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n49_), .O(z04));
  nand2  g25(.a(new_n49_), .b(new_n46_), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x13), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor2   g28(.a(new_n71_), .b(new_n73_), .O(z06));
  aoi22  g29(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x06), .O(z07));
  inv1   g30(.a(x20), .O(new_n76_));
  inv1   g31(.a(x17), .O(new_n77_));
  inv1   g32(.a(x18), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x19), .c(new_n76_), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(x22), .c(x21), .O(new_n81_));
  aoi21  g36(.a(new_n58_), .b(new_n81_), .c(new_n52_), .O(z08));
  nand2  g37(.a(x05), .b(x04), .O(new_n83_));
  inv1   g38(.a(x07), .O(new_n84_));
  nand3  g39(.a(x24), .b(new_n47_), .c(new_n84_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n83_), .O(z09));
  nand2  g41(.a(new_n51_), .b(new_n77_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x24), .c(x15), .O(z10));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n79_), .c(new_n51_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x24), .c(x15), .O(z11));
  nand2  g46(.a(new_n89_), .b(new_n54_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n51_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(x24), .c(x15), .O(z12));
  inv1   g50(.a(new_n93_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n76_), .O(new_n97_));
  nand2  g52(.a(new_n93_), .b(x20), .O(new_n98_));
  inv1   g53(.a(new_n85_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n98_), .c(new_n97_), .O(z13));
  oai21  g57(.a(new_n93_), .b(x20), .c(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n96_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n51_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x24), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  aoi21  g64(.a(new_n104_), .b(new_n96_), .c(new_n109_), .O(new_n110_));
  inv1   g65(.a(x21), .O(new_n111_));
  nand3  g66(.a(new_n109_), .b(new_n111_), .c(new_n76_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n93_), .c(new_n51_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n110_), .c(x24), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n47_), .O(z15));
  oai21  g70(.a(new_n112_), .b(new_n93_), .c(x23), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  nor2   g72(.a(new_n112_), .b(new_n93_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n116_), .c(new_n101_), .O(z16));
  nand3  g75(.a(new_n118_), .b(new_n101_), .c(new_n117_), .O(z17));
  inv1   g76(.a(x25), .O(new_n122_));
  nand2  g77(.a(new_n101_), .b(new_n122_), .O(z18));
endmodule


