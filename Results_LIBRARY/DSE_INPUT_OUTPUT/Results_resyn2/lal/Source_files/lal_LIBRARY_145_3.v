// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x23), .b(x06), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  aoi21  g06(.a(new_n51_), .b(new_n50_), .c(x20), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  oai21  g08(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(new_n54_));
  inv1   g09(.a(x07), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  inv1   g11(.a(x24), .O(new_n57_));
  inv1   g12(.a(x25), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n56_), .c(new_n55_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  oai21  g16(.a(new_n54_), .b(x25), .c(new_n61_), .O(z01));
  and2   g17(.a(new_n47_), .b(x16), .O(z02));
  nand2  g18(.a(new_n47_), .b(new_n57_), .O(new_n64_));
  aoi21  g19(.a(new_n64_), .b(new_n54_), .c(x25), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n47_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g28(.a(new_n47_), .b(x14), .c(new_n46_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z06));
  oai21  g30(.a(x23), .b(new_n46_), .c(x06), .O(z07));
  inv1   g31(.a(x06), .O(new_n77_));
  nor2   g32(.a(new_n57_), .b(new_n49_), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x25), .c(new_n77_), .O(new_n79_));
  inv1   g34(.a(x20), .O(new_n80_));
  nand2  g35(.a(new_n51_), .b(new_n50_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g37(.a(x24), .b(x22), .c(x21), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n82_), .c(x25), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x23), .c(new_n79_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n55_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n56_), .c(new_n47_), .O(z09));
  inv1   g44(.a(x17), .O(new_n90_));
  nand3  g45(.a(new_n56_), .b(new_n87_), .c(new_n55_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n47_), .c(new_n90_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  and2   g49(.a(x18), .b(x17), .O(new_n95_));
  or2    g50(.a(new_n95_), .b(new_n51_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n91_), .c(new_n47_), .O(z11));
  nand2  g52(.a(new_n92_), .b(new_n47_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n95_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(z12));
  inv1   g57(.a(new_n47_), .O(new_n103_));
  nand2  g58(.a(new_n100_), .b(new_n80_), .O(new_n104_));
  aoi21  g59(.a(new_n99_), .b(x20), .c(new_n91_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n103_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n100_), .c(new_n91_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n107_), .c(new_n103_), .O(z14));
  nand2  g65(.a(new_n108_), .b(new_n100_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand3  g68(.a(new_n108_), .b(new_n100_), .c(new_n113_), .O(new_n114_));
  and2   g69(.a(new_n114_), .b(new_n92_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n112_), .c(new_n103_), .O(z15));
  nor2   g71(.a(new_n49_), .b(x06), .O(new_n117_));
  oai21  g72(.a(new_n114_), .b(new_n91_), .c(new_n117_), .O(new_n118_));
  oai21  g73(.a(new_n115_), .b(x23), .c(new_n118_), .O(z16));
  xor2a  g74(.a(new_n114_), .b(x24), .O(new_n120_));
  oai21  g75(.a(new_n91_), .b(new_n78_), .c(new_n47_), .O(new_n121_));
  oai21  g76(.a(new_n120_), .b(x23), .c(new_n121_), .O(z17));
  oai21  g77(.a(new_n114_), .b(x24), .c(x25), .O(new_n123_));
  nor2   g78(.a(new_n59_), .b(x23), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n108_), .c(new_n100_), .d(new_n113_), .O(new_n125_));
  oai21  g80(.a(x25), .b(x06), .c(x23), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n123_), .d(new_n92_), .O(z18));
endmodule


