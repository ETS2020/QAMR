// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x18), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n49_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n59_), .b(new_n49_), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z04));
  oai21  g26(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g27(.a(x14), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z06));
  nand3  g29(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g30(.a(new_n58_), .b(new_n50_), .c(new_n47_), .O(z08));
  nand2  g31(.a(x05), .b(x04), .O(new_n77_));
  inv1   g32(.a(x07), .O(new_n78_));
  inv1   g33(.a(x15), .O(new_n79_));
  nand3  g34(.a(x18), .b(new_n79_), .c(new_n78_), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(new_n77_), .O(z09));
  inv1   g36(.a(new_n80_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n77_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  nor2   g41(.a(x19), .b(x17), .O(new_n87_));
  and2   g42(.a(x19), .b(x17), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n83_), .c(new_n87_), .O(z12));
  nand2  g44(.a(x19), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(x20), .O(new_n91_));
  nand3  g46(.a(new_n51_), .b(x19), .c(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n60_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x18), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n79_), .O(z13));
  inv1   g50(.a(x21), .O(new_n96_));
  nand4  g51(.a(new_n96_), .b(new_n51_), .c(x19), .d(x17), .O(new_n97_));
  nand2  g52(.a(new_n92_), .b(x21), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n84_), .O(z14));
  nand2  g54(.a(new_n97_), .b(x22), .O(new_n100_));
  nor2   g55(.a(new_n90_), .b(x20), .O(new_n101_));
  nor2   g56(.a(x22), .b(x21), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n100_), .c(new_n60_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x18), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n79_), .O(z15));
  inv1   g61(.a(x23), .O(new_n107_));
  aoi21  g62(.a(new_n102_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n102_), .b(new_n88_), .c(new_n107_), .d(new_n51_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n60_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n108_), .c(x18), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n79_), .O(z16));
  nand2  g67(.a(new_n109_), .b(x24), .O(new_n113_));
  inv1   g68(.a(x24), .O(new_n114_));
  nand4  g69(.a(new_n102_), .b(new_n101_), .c(new_n114_), .d(new_n107_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n60_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(x18), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n79_), .O(z17));
  inv1   g73(.a(x22), .O(new_n119_));
  nand4  g74(.a(new_n50_), .b(new_n114_), .c(new_n107_), .d(new_n119_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n97_), .c(new_n60_), .O(new_n121_));
  aoi21  g76(.a(new_n115_), .b(x25), .c(new_n121_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n53_), .c(new_n79_), .O(z18));
  inv1   g78(.a(new_n85_), .O(z11));
endmodule


