// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n71_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x20), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x07), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  aoi21  g06(.a(x22), .b(x21), .c(x25), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nand4  g12(.a(new_n57_), .b(new_n56_), .c(new_n53_), .d(new_n49_), .O(z01));
  inv1   g13(.a(x16), .O(new_n59_));
  nor2   g14(.a(new_n47_), .b(new_n59_), .O(z02));
  nand2  g15(.a(new_n56_), .b(new_n53_), .O(z03));
  inv1   g16(.a(new_n47_), .O(new_n62_));
  xnor2a g17(.a(x11), .b(x02), .O(new_n63_));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x09), .b(x00), .O(new_n65_));
  xnor2a g20(.a(x10), .b(x01), .O(new_n66_));
  nand4  g21(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n62_), .O(z04));
  oai21  g24(.a(x13), .b(x08), .c(new_n62_), .O(z05));
  nand2  g25(.a(x14), .b(new_n46_), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n47_), .O(z06));
  aoi21  g27(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g28(.a(x22), .b(x21), .c(x23), .O(new_n74_));
  nor2   g29(.a(new_n47_), .b(x25), .O(new_n75_));
  oai21  g30(.a(new_n74_), .b(new_n54_), .c(new_n75_), .O(z08));
  inv1   g31(.a(x15), .O(new_n77_));
  nor2   g32(.a(new_n57_), .b(new_n47_), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n49_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z09));
  nand3  g35(.a(new_n57_), .b(new_n77_), .c(new_n49_), .O(new_n81_));
  nor3   g36(.a(new_n81_), .b(new_n47_), .c(x17), .O(z10));
  xnor2a g37(.a(x18), .b(x17), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(new_n62_), .O(z11));
  aoi21  g39(.a(x18), .b(x17), .c(x19), .O(new_n85_));
  inv1   g40(.a(new_n81_), .O(new_n86_));
  nand3  g41(.a(x19), .b(x18), .c(x17), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n85_), .c(new_n62_), .O(z12));
  nand2  g44(.a(new_n87_), .b(x23), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nand2  g46(.a(new_n87_), .b(x20), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n86_), .O(z13));
  nand2  g48(.a(new_n91_), .b(x21), .O(new_n94_));
  nand2  g49(.a(new_n81_), .b(new_n62_), .O(new_n95_));
  inv1   g50(.a(x21), .O(new_n96_));
  inv1   g51(.a(new_n87_), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(x23), .c(new_n96_), .d(new_n50_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n95_), .c(new_n94_), .O(z14));
  inv1   g54(.a(x22), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n87_), .c(x23), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n50_), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(x22), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n86_), .O(z15));
  nand4  g60(.a(new_n97_), .b(new_n100_), .c(new_n96_), .d(new_n50_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x23), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n95_), .O(z16));
  aoi21  g63(.a(new_n86_), .b(new_n54_), .c(new_n47_), .O(z17));
  aoi21  g64(.a(new_n86_), .b(new_n55_), .c(new_n47_), .O(z18));
endmodule


