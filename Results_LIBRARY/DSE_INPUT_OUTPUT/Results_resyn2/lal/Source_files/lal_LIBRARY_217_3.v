// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n131_, new_n132_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x19), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x15), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nor2   g14(.a(x23), .b(x20), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n58_), .c(new_n59_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n57_), .O(z01));
  nand2  g21(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g22(.a(x17), .O(new_n68_));
  inv1   g23(.a(x18), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x19), .c(new_n60_), .O(new_n71_));
  inv1   g26(.a(x23), .O(new_n72_));
  aoi21  g27(.a(new_n54_), .b(new_n72_), .c(new_n50_), .O(new_n73_));
  nand2  g28(.a(new_n47_), .b(new_n51_), .O(new_n74_));
  aoi21  g29(.a(new_n73_), .b(new_n71_), .c(new_n74_), .O(z03));
  inv1   g30(.a(x08), .O(new_n76_));
  nand2  g31(.a(new_n47_), .b(new_n76_), .O(new_n77_));
  xor2a  g32(.a(x12), .b(x03), .O(new_n78_));
  xor2a  g33(.a(x11), .b(x02), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  xor2a  g35(.a(x09), .b(x00), .O(new_n81_));
  xor2a  g36(.a(x10), .b(x01), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n80_), .c(new_n77_), .O(z04));
  or2    g39(.a(new_n77_), .b(x13), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z05));
  nand3  g41(.a(new_n47_), .b(x14), .c(new_n76_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z06));
  aoi22  g43(.a(x19), .b(x15), .c(new_n76_), .d(x06), .O(z07));
  aoi21  g44(.a(new_n70_), .b(new_n52_), .c(new_n54_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x23), .c(x24), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n51_), .c(new_n62_), .O(z08));
  inv1   g47(.a(x07), .O(new_n93_));
  nand4  g48(.a(new_n58_), .b(new_n93_), .c(x05), .d(x04), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand2  g50(.a(new_n63_), .b(new_n58_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n68_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  nand2  g54(.a(x18), .b(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n70_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n96_), .O(z11));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nand3  g58(.a(x19), .b(x18), .c(x17), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n103_), .c(new_n96_), .O(z12));
  oai21  g61(.a(x20), .b(x15), .c(new_n59_), .O(new_n107_));
  nor2   g62(.a(new_n104_), .b(x20), .O(new_n108_));
  nand2  g63(.a(new_n100_), .b(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n63_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n108_), .c(new_n58_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n107_), .O(z13));
  oai21  g67(.a(x21), .b(x15), .c(new_n59_), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n105_), .O(new_n115_));
  oai21  g70(.a(new_n100_), .b(x20), .c(x21), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n63_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n58_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n113_), .O(z14));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n108_), .O(new_n121_));
  nand2  g76(.a(new_n115_), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n97_), .O(z15));
  nand3  g78(.a(new_n120_), .b(new_n108_), .c(new_n72_), .O(new_n124_));
  nand2  g79(.a(new_n121_), .b(x23), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n97_), .O(z16));
  nand2  g81(.a(new_n124_), .b(x24), .O(new_n127_));
  nor2   g82(.a(x24), .b(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n120_), .c(new_n108_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n97_), .O(z17));
  nand4  g85(.a(new_n128_), .b(new_n120_), .c(new_n108_), .d(new_n51_), .O(new_n131_));
  nand2  g86(.a(new_n129_), .b(x25), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n97_), .O(z18));
endmodule


