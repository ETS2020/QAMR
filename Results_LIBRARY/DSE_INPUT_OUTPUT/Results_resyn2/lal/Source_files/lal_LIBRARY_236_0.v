// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n90_, new_n91_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x23), .b(x02), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x25), .c(x02), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nor2   g16(.a(x25), .b(x24), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n61_), .O(z01));
  inv1   g22(.a(new_n62_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n61_), .O(z03));
  inv1   g24(.a(x02), .O(new_n70_));
  inv1   g25(.a(x11), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x08), .c(x23), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  inv1   g28(.a(x09), .O(new_n74_));
  inv1   g29(.a(x12), .O(new_n75_));
  aoi22  g30(.a(new_n75_), .b(x03), .c(new_n74_), .d(x00), .O(new_n76_));
  oai21  g31(.a(new_n74_), .b(x00), .c(new_n76_), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  inv1   g33(.a(x03), .O(new_n79_));
  nand2  g34(.a(x12), .b(new_n79_), .O(new_n80_));
  nand2  g35(.a(new_n71_), .b(x02), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n77_), .c(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n73_), .O(z04));
  nand2  g39(.a(new_n49_), .b(new_n46_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(x13), .O(z05));
  inv1   g41(.a(x14), .O(new_n87_));
  nor2   g42(.a(new_n85_), .b(new_n87_), .O(z06));
  nand3  g43(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  oai21  g44(.a(new_n59_), .b(x23), .c(x24), .O(new_n90_));
  nor2   g45(.a(new_n48_), .b(x25), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(z08));
  inv1   g47(.a(x15), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n63_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n64_), .c(new_n49_), .O(z09));
  nand3  g50(.a(new_n64_), .b(new_n93_), .c(new_n63_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n54_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z10));
  xnor2a g54(.a(x18), .b(x17), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n96_), .c(new_n49_), .O(z11));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  aoi21  g58(.a(x18), .b(x17), .c(x19), .O(new_n104_));
  nor4   g59(.a(new_n104_), .b(new_n103_), .c(new_n96_), .d(new_n48_), .O(z12));
  nand2  g60(.a(new_n103_), .b(new_n53_), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(x20), .c(new_n96_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n48_), .O(z13));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n97_), .O(z14));
  nand2  g67(.a(new_n111_), .b(x22), .O(new_n113_));
  inv1   g68(.a(x22), .O(new_n114_));
  inv1   g69(.a(new_n111_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n114_), .c(new_n96_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n113_), .c(new_n48_), .O(z15));
  oai21  g72(.a(new_n111_), .b(x22), .c(x23), .O(new_n118_));
  nand3  g73(.a(new_n115_), .b(new_n52_), .c(new_n114_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n97_), .O(z16));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n121_));
  nor2   g76(.a(x24), .b(x22), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n110_), .c(new_n103_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x02), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n52_), .O(new_n125_));
  aoi21  g80(.a(x24), .b(new_n70_), .c(new_n96_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n121_), .O(z17));
  nand2  g82(.a(new_n123_), .b(x25), .O(new_n128_));
  nor3   g83(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n110_), .c(new_n103_), .d(new_n114_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(x02), .O(new_n132_));
  aoi22  g87(.a(new_n96_), .b(new_n49_), .c(x25), .d(x23), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


