// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n78_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n47_), .c(new_n60_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g20(.a(new_n59_), .b(new_n47_), .O(z03));
  inv1   g21(.a(x08), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n67_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n47_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g30(.a(new_n47_), .b(x14), .c(new_n67_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z06));
  inv1   g32(.a(new_n47_), .O(new_n78_));
  aoi21  g33(.a(new_n67_), .b(x06), .c(new_n78_), .O(z07));
  oai21  g34(.a(new_n57_), .b(x23), .c(x24), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n50_), .O(z08));
  inv1   g36(.a(x15), .O(new_n82_));
  nand3  g37(.a(new_n47_), .b(new_n82_), .c(new_n60_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n61_), .O(z09));
  inv1   g39(.a(new_n83_), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n61_), .c(new_n52_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  nand3  g42(.a(new_n61_), .b(new_n82_), .c(new_n60_), .O(new_n88_));
  xnor2a g43(.a(x18), .b(x17), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n88_), .c(new_n47_), .O(z11));
  nand2  g45(.a(new_n85_), .b(new_n61_), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  aoi21  g48(.a(x18), .b(x17), .c(x19), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n93_), .c(new_n91_), .O(z12));
  nand2  g50(.a(new_n93_), .b(new_n51_), .O(new_n96_));
  aoi21  g51(.a(new_n92_), .b(x20), .c(new_n88_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n96_), .c(new_n78_), .O(z13));
  inv1   g53(.a(new_n91_), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n93_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(z14));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n104_));
  inv1   g59(.a(x22), .O(new_n105_));
  nand3  g60(.a(new_n101_), .b(new_n93_), .c(new_n105_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n99_), .O(z15));
  inv1   g62(.a(new_n88_), .O(new_n108_));
  inv1   g63(.a(x24), .O(new_n109_));
  nand4  g64(.a(new_n101_), .b(new_n93_), .c(new_n109_), .d(new_n105_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x23), .O(new_n111_));
  inv1   g66(.a(x23), .O(new_n112_));
  nand4  g67(.a(new_n101_), .b(new_n93_), .c(new_n112_), .d(new_n105_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n108_), .O(z16));
  nand2  g69(.a(new_n88_), .b(new_n47_), .O(new_n115_));
  xor2a  g70(.a(new_n106_), .b(x24), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(x23), .c(new_n115_), .O(z17));
  inv1   g72(.a(new_n113_), .O(new_n118_));
  nor2   g73(.a(x25), .b(x24), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g75(.a(new_n110_), .b(x25), .O(new_n121_));
  oai21  g76(.a(new_n119_), .b(new_n112_), .c(new_n108_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(z18));
endmodule


