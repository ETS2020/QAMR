// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_;
  nand2  g00(.a(x15), .b(x05), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(x08), .O(new_n48_));
  nand2  g03(.a(new_n46_), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(new_n46_), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(new_n52_), .O(z01));
  nand2  g19(.a(new_n62_), .b(new_n46_), .O(z03));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n49_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x08), .c(new_n46_), .O(z06));
  nand2  g30(.a(new_n50_), .b(x06), .O(z07));
  nand3  g31(.a(new_n61_), .b(new_n46_), .c(new_n53_), .O(z08));
  inv1   g32(.a(x05), .O(new_n78_));
  inv1   g33(.a(x15), .O(new_n79_));
  inv1   g34(.a(x07), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x04), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(z09));
  nand2  g37(.a(new_n63_), .b(new_n79_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n46_), .O(z10));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n83_), .c(new_n46_), .O(z11));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(new_n86_), .b(new_n57_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n83_), .c(new_n46_), .O(z12));
  inv1   g47(.a(new_n83_), .O(new_n93_));
  inv1   g48(.a(new_n89_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n54_), .O(new_n95_));
  nand2  g50(.a(new_n89_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n93_), .O(z13));
  nand2  g52(.a(new_n95_), .b(x21), .O(new_n98_));
  nor2   g53(.a(x21), .b(x20), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n94_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n98_), .c(new_n93_), .O(z14));
  nand2  g56(.a(x15), .b(new_n78_), .O(new_n102_));
  nand2  g57(.a(new_n100_), .b(x22), .O(new_n103_));
  inv1   g58(.a(x22), .O(new_n104_));
  nor3   g59(.a(new_n89_), .b(x21), .c(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n103_), .c(new_n63_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n79_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n102_), .O(z15));
  nor2   g64(.a(x23), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n99_), .c(new_n94_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n63_), .O(new_n112_));
  aoi21  g67(.a(new_n106_), .b(x23), .c(new_n112_), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(x15), .c(new_n102_), .O(z16));
  nand2  g69(.a(new_n111_), .b(x24), .O(new_n115_));
  inv1   g70(.a(x24), .O(new_n116_));
  nand3  g71(.a(new_n110_), .b(new_n105_), .c(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n63_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n79_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n102_), .O(z17));
  aoi21  g75(.a(new_n117_), .b(x25), .c(new_n83_), .O(new_n121_));
  oai21  g76(.a(new_n117_), .b(x25), .c(new_n121_), .O(z18));
endmodule


