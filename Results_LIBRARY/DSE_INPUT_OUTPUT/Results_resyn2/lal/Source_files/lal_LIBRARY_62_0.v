// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x22), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x22), .O(new_n60_));
  oai21  g15(.a(x25), .b(x23), .c(x24), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(new_n60_), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n59_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nand2  g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n61_), .b(new_n60_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n59_), .O(z03));
  nand2  g25(.a(new_n48_), .b(new_n46_), .O(new_n71_));
  xor2a  g26(.a(x12), .b(x03), .O(new_n72_));
  xor2a  g27(.a(x11), .b(x02), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  xor2a  g29(.a(x09), .b(x00), .O(new_n75_));
  xor2a  g30(.a(x10), .b(x01), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(z04));
  nor2   g33(.a(new_n71_), .b(x13), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n48_), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g37(.a(x24), .O(new_n83_));
  aoi21  g38(.a(new_n57_), .b(x22), .c(x23), .O(new_n84_));
  nor2   g39(.a(new_n47_), .b(x25), .O(new_n85_));
  oai21  g40(.a(new_n84_), .b(new_n83_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n63_), .c(new_n48_), .O(z09));
  nand3  g44(.a(new_n63_), .b(new_n87_), .c(new_n62_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n48_), .O(z10));
  nand2  g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nor2   g47(.a(new_n90_), .b(new_n47_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n94_), .b(new_n55_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n90_), .c(new_n47_), .O(z12));
  inv1   g55(.a(new_n97_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n51_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n93_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  nor2   g60(.a(new_n97_), .b(x20), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n52_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n93_), .O(z14));
  inv1   g63(.a(new_n107_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(x24), .c(new_n60_), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(x22), .O(new_n111_));
  nand2  g66(.a(new_n90_), .b(new_n48_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z15));
  aoi21  g68(.a(new_n106_), .b(new_n52_), .c(new_n83_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(x22), .c(x23), .O(new_n115_));
  inv1   g70(.a(x23), .O(new_n116_));
  nand4  g71(.a(new_n109_), .b(x24), .c(new_n116_), .d(new_n60_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n112_), .O(z16));
  nand2  g73(.a(new_n116_), .b(new_n60_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n107_), .c(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n112_), .O(z17));
  inv1   g76(.a(new_n90_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n50_), .c(new_n47_), .O(z18));
endmodule


