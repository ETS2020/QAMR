// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  aoi21  g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n47_), .c(new_n49_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(new_n47_), .b(new_n46_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n59_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n62_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n63_), .b(new_n59_), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n63_), .O(z04));
  nor2   g31(.a(x13), .b(x08), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x08), .O(z06));
  nand2  g34(.a(new_n46_), .b(x06), .O(z07));
  inv1   g35(.a(x23), .O(new_n81_));
  nor2   g36(.a(x18), .b(x17), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n53_), .c(x20), .O(new_n83_));
  oai21  g38(.a(new_n56_), .b(new_n83_), .c(new_n81_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x08), .c(x24), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n49_), .O(z08));
  nor4   g41(.a(new_n62_), .b(new_n61_), .c(x15), .d(x07), .O(z09));
  inv1   g42(.a(x15), .O(new_n88_));
  nand3  g43(.a(new_n61_), .b(new_n88_), .c(new_n60_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n63_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n51_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  nor2   g49(.a(new_n52_), .b(new_n51_), .O(new_n95_));
  or2    g50(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n89_), .c(new_n63_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nor2   g54(.a(new_n95_), .b(x19), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n99_), .c(new_n91_), .O(z12));
  nand4  g56(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  aoi21  g57(.a(new_n98_), .b(x20), .c(new_n89_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n102_), .c(new_n62_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand3  g61(.a(new_n99_), .b(new_n106_), .c(new_n50_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n92_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n102_), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(new_n89_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n109_), .c(new_n62_), .O(z15));
  oai21  g69(.a(new_n111_), .b(new_n102_), .c(x23), .O(new_n115_));
  aoi21  g70(.a(new_n112_), .b(new_n81_), .c(new_n89_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n115_), .c(new_n62_), .O(z16));
  nand2  g72(.a(new_n112_), .b(new_n81_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(x24), .c(new_n46_), .O(new_n119_));
  nand2  g74(.a(new_n89_), .b(new_n63_), .O(new_n120_));
  nand3  g75(.a(new_n112_), .b(new_n47_), .c(new_n81_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(z17));
  nand4  g77(.a(new_n112_), .b(new_n49_), .c(new_n47_), .d(new_n81_), .O(new_n123_));
  nand2  g78(.a(new_n63_), .b(new_n49_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n90_), .O(z18));
endmodule


