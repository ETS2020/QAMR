// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n98_, new_n100_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x19), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n47_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n51_), .c(new_n52_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(new_n48_), .b(new_n58_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(z01));
  and2   g16(.a(new_n48_), .b(x16), .O(z02));
  nor2   g17(.a(new_n57_), .b(new_n49_), .O(z03));
  nand2  g18(.a(new_n48_), .b(new_n46_), .O(new_n64_));
  xor2a  g19(.a(x12), .b(x03), .O(new_n65_));
  xor2a  g20(.a(x11), .b(x02), .O(new_n66_));
  nor2   g21(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  xor2a  g22(.a(x09), .b(x00), .O(new_n68_));
  xor2a  g23(.a(x10), .b(x01), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n67_), .c(new_n64_), .O(z04));
  nor2   g26(.a(new_n64_), .b(x13), .O(z05));
  nand2  g27(.a(x14), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n48_), .O(z06));
  nand3  g29(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  nand2  g31(.a(new_n55_), .b(new_n53_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n52_), .c(new_n49_), .O(z08));
  nand2  g35(.a(x05), .b(x04), .O(new_n81_));
  inv1   g36(.a(x15), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n58_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(new_n48_), .O(z09));
  nand2  g39(.a(new_n60_), .b(new_n82_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(x17), .O(z10));
  xnor2a g41(.a(x18), .b(x17), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n85_), .O(z11));
  and2   g43(.a(x18), .b(x17), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(x19), .O(new_n90_));
  nand2  g45(.a(new_n89_), .b(x19), .O(new_n91_));
  nand3  g46(.a(new_n81_), .b(new_n82_), .c(new_n58_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n48_), .O(z12));
  nand2  g50(.a(new_n91_), .b(x20), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n93_), .c(new_n49_), .O(z13));
  inv1   g52(.a(x21), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(new_n98_), .c(new_n49_), .O(z14));
  inv1   g54(.a(x22), .O(new_n100_));
  nand3  g55(.a(new_n60_), .b(new_n100_), .c(new_n82_), .O(z15));
  nand3  g56(.a(new_n60_), .b(new_n76_), .c(new_n82_), .O(z16));
  aoi21  g57(.a(new_n93_), .b(new_n51_), .c(new_n49_), .O(z17));
  aoi21  g58(.a(new_n93_), .b(new_n52_), .c(new_n49_), .O(z18));
endmodule


