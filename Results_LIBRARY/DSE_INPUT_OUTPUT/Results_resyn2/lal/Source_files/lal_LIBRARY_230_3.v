// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g19(.a(x23), .O(new_n65_));
  nor2   g20(.a(x18), .b(x17), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n54_), .c(x20), .O(new_n67_));
  oai21  g22(.a(new_n56_), .b(new_n67_), .c(new_n65_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(x24), .c(x25), .O(z03));
  aoi21  g24(.a(x24), .b(x23), .c(x08), .O(new_n70_));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  and2   g30(.a(new_n75_), .b(new_n70_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand2  g32(.a(new_n70_), .b(x14), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z06));
  nand2  g34(.a(new_n70_), .b(x06), .O(z07));
  nand2  g35(.a(new_n68_), .b(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n50_), .O(z08));
  inv1   g37(.a(x07), .O(new_n83_));
  inv1   g38(.a(x15), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n83_), .c(x05), .d(x04), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n47_), .O(z09));
  nand2  g41(.a(new_n60_), .b(new_n84_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n47_), .O(z10));
  inv1   g43(.a(new_n87_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n47_), .O(new_n90_));
  inv1   g45(.a(new_n66_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(new_n90_), .O(z11));
  nand2  g49(.a(new_n92_), .b(new_n54_), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n87_), .c(new_n47_), .O(z12));
  inv1   g53(.a(new_n90_), .O(new_n99_));
  xor2a  g54(.a(new_n96_), .b(x20), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(z13));
  oai21  g56(.a(new_n96_), .b(x20), .c(x21), .O(new_n102_));
  inv1   g57(.a(new_n96_), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n102_), .c(new_n99_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  nand3  g63(.a(new_n104_), .b(new_n103_), .c(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n99_), .O(z15));
  inv1   g65(.a(x24), .O(new_n111_));
  nand4  g66(.a(new_n104_), .b(new_n103_), .c(new_n111_), .d(new_n108_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x23), .O(new_n113_));
  nand4  g68(.a(new_n104_), .b(new_n103_), .c(new_n65_), .d(new_n108_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n89_), .O(z16));
  nand2  g70(.a(new_n87_), .b(new_n47_), .O(new_n116_));
  xor2a  g71(.a(new_n109_), .b(x24), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(x23), .c(new_n116_), .O(z17));
  inv1   g73(.a(new_n114_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n50_), .c(new_n111_), .O(new_n120_));
  nand2  g75(.a(new_n112_), .b(x25), .O(new_n121_));
  oai21  g76(.a(x25), .b(x24), .c(x23), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n89_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n121_), .c(new_n120_), .O(z18));
endmodule


