// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x14), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n48_), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  inv1   g16(.a(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x23), .O(new_n63_));
  inv1   g18(.a(x14), .O(new_n64_));
  nor2   g19(.a(x24), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n52_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n47_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n66_), .b(x25), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  oai21  g33(.a(new_n64_), .b(x08), .c(new_n48_), .O(z06));
  nand3  g34(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g35(.a(x25), .O(new_n81_));
  aoi21  g36(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n82_));
  inv1   g37(.a(x23), .O(new_n83_));
  nand2  g38(.a(new_n81_), .b(new_n83_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n82_), .c(x24), .O(new_n85_));
  oai21  g40(.a(new_n81_), .b(new_n64_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  inv1   g42(.a(x15), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n47_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n51_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand2  g47(.a(new_n90_), .b(new_n50_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n54_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nand3  g51(.a(new_n50_), .b(new_n88_), .c(new_n87_), .O(new_n97_));
  nand2  g52(.a(new_n55_), .b(new_n54_), .O(new_n98_));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n97_), .c(new_n48_), .O(z11));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  aoi21  g57(.a(new_n99_), .b(new_n56_), .c(new_n97_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n48_), .O(z12));
  nand2  g60(.a(new_n102_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n102_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n53_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n94_), .O(z13));
  nand2  g64(.a(new_n108_), .b(x21), .O(new_n110_));
  nor3   g65(.a(new_n102_), .b(x21), .c(x20), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n97_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n110_), .c(new_n47_), .O(z14));
  inv1   g68(.a(x22), .O(new_n114_));
  nand2  g69(.a(new_n111_), .b(new_n114_), .O(new_n115_));
  nor2   g70(.a(x21), .b(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(x22), .c(new_n97_), .O(new_n118_));
  aoi21  g73(.a(new_n118_), .b(new_n115_), .c(new_n47_), .O(z15));
  nand2  g74(.a(new_n115_), .b(x23), .O(new_n120_));
  nor2   g75(.a(x23), .b(x22), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n111_), .c(new_n97_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n120_), .c(new_n47_), .O(z16));
  nand2  g78(.a(new_n121_), .b(new_n111_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x24), .O(new_n125_));
  nand2  g80(.a(new_n97_), .b(new_n48_), .O(new_n126_));
  nand4  g81(.a(new_n121_), .b(new_n116_), .c(new_n107_), .d(new_n65_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(z17));
  nand4  g83(.a(new_n121_), .b(new_n111_), .c(new_n81_), .d(new_n62_), .O(new_n129_));
  oai21  g84(.a(new_n124_), .b(x24), .c(x25), .O(new_n130_));
  nand2  g85(.a(new_n81_), .b(x24), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(new_n64_), .c(new_n97_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n130_), .c(new_n129_), .O(z18));
endmodule


