// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n78_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_;
  nand2  g00(.a(x20), .b(x06), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n46_), .c(new_n61_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(z01));
  nand2  g20(.a(new_n60_), .b(new_n46_), .O(z03));
  inv1   g21(.a(new_n49_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  nand2  g30(.a(new_n49_), .b(x14), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z06));
  inv1   g32(.a(x08), .O(new_n78_));
  nand3  g33(.a(new_n52_), .b(new_n78_), .c(x06), .O(z07));
  aoi21  g34(.a(new_n59_), .b(new_n51_), .c(new_n48_), .O(z08));
  nor4   g35(.a(new_n62_), .b(new_n48_), .c(x15), .d(x07), .O(z09));
  inv1   g36(.a(x15), .O(new_n82_));
  nand2  g37(.a(new_n64_), .b(new_n82_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x17), .O(z10));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  nor2   g40(.a(new_n54_), .b(new_n53_), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n83_), .c(new_n85_), .O(z11));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n86_), .b(x19), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n89_), .c(new_n83_), .O(z12));
  nor2   g46(.a(new_n88_), .b(x20), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand3  g48(.a(new_n62_), .b(new_n82_), .c(new_n61_), .O(new_n94_));
  aoi21  g49(.a(new_n88_), .b(x20), .c(new_n94_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n93_), .c(new_n48_), .O(z13));
  inv1   g51(.a(new_n94_), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  nand2  g53(.a(new_n92_), .b(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n93_), .b(x21), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n99_), .c(new_n97_), .d(new_n46_), .O(z14));
  nand2  g56(.a(new_n99_), .b(x22), .O(new_n102_));
  nor2   g57(.a(x22), .b(x21), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n92_), .c(new_n94_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n102_), .c(new_n48_), .O(z15));
  inv1   g60(.a(x23), .O(new_n106_));
  nand3  g61(.a(new_n103_), .b(new_n92_), .c(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n103_), .b(new_n89_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x23), .O(new_n109_));
  oai21  g64(.a(x23), .b(x06), .c(x20), .O(new_n110_));
  nand4  g65(.a(new_n110_), .b(new_n109_), .c(new_n107_), .d(new_n97_), .O(z16));
  nand3  g66(.a(new_n103_), .b(new_n89_), .c(new_n106_), .O(new_n112_));
  xor2a  g67(.a(new_n112_), .b(x24), .O(new_n113_));
  and2   g68(.a(x24), .b(x20), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n94_), .c(new_n46_), .O(new_n115_));
  oai21  g70(.a(new_n113_), .b(x20), .c(new_n115_), .O(z17));
  oai21  g71(.a(new_n112_), .b(x24), .c(x25), .O(new_n117_));
  nor3   g72(.a(x25), .b(x24), .c(x23), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n103_), .c(new_n92_), .O(new_n119_));
  oai21  g74(.a(x25), .b(x06), .c(x20), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n119_), .c(new_n117_), .d(new_n97_), .O(z18));
endmodule


