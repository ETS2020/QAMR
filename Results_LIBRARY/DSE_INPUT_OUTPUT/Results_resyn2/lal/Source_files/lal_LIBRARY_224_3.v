// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n80_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  inv1   g01(.a(new_n46_), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x08), .O(new_n48_));
  aoi21  g03(.a(new_n46_), .b(x16), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(z08));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(z08), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n46_), .b(new_n65_), .O(z02));
  nand2  g21(.a(new_n57_), .b(x24), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n50_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n46_), .O(z04));
  inv1   g31(.a(x13), .O(new_n77_));
  nand2  g32(.a(new_n48_), .b(new_n77_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x08), .c(new_n46_), .O(z06));
  nand2  g36(.a(new_n48_), .b(x06), .O(z07));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n61_), .O(z09));
  inv1   g40(.a(new_n84_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n52_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z10));
  nor2   g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n87_), .c(new_n91_), .O(z11));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(new_n54_), .O(new_n96_));
  nand2  g51(.a(new_n83_), .b(new_n61_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n46_), .O(z12));
  nor2   g55(.a(new_n95_), .b(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(new_n95_), .b(x20), .c(new_n97_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n102_), .c(new_n47_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n88_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n101_), .c(new_n97_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n109_), .c(new_n47_), .O(z15));
  inv1   g67(.a(x24), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(new_n101_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(x23), .O(new_n115_));
  nand2  g70(.a(new_n97_), .b(new_n46_), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  nand3  g72(.a(new_n110_), .b(new_n101_), .c(new_n117_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(z16));
  nand2  g74(.a(new_n114_), .b(new_n113_), .O(new_n120_));
  nand3  g75(.a(new_n110_), .b(new_n101_), .c(x24), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n117_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n116_), .O(z17));
  inv1   g78(.a(new_n118_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n50_), .c(new_n113_), .O(new_n125_));
  oai21  g80(.a(new_n114_), .b(x24), .c(x25), .O(new_n126_));
  oai21  g81(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n125_), .d(new_n98_), .O(z18));
endmodule


