// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n112_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x20), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n51_), .c(x07), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  inv1   g12(.a(x23), .O(new_n58_));
  oai21  g13(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g14(.a(new_n52_), .b(x20), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n59_), .c(new_n55_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  nand2  g18(.a(new_n49_), .b(new_n52_), .O(new_n64_));
  aoi21  g19(.a(new_n59_), .b(x24), .c(new_n64_), .O(z03));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g26(.a(new_n49_), .b(new_n46_), .O(new_n72_));
  aoi21  g27(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g32(.a(z03), .O(z08));
  inv1   g33(.a(new_n50_), .O(new_n79_));
  nor2   g34(.a(x15), .b(x07), .O(new_n80_));
  nand3  g35(.a(new_n80_), .b(new_n79_), .c(new_n49_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  inv1   g37(.a(x17), .O(new_n83_));
  nand2  g38(.a(new_n80_), .b(new_n83_), .O(new_n84_));
  nor3   g39(.a(new_n84_), .b(new_n79_), .c(new_n47_), .O(z10));
  nand2  g40(.a(new_n80_), .b(new_n50_), .O(new_n86_));
  xnor2a g41(.a(x18), .b(x17), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n86_), .c(new_n49_), .O(z11));
  aoi21  g43(.a(x18), .b(x17), .c(x19), .O(new_n89_));
  inv1   g44(.a(new_n86_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n49_), .O(z12));
  nand2  g48(.a(new_n91_), .b(x20), .O(new_n94_));
  inv1   g49(.a(x20), .O(new_n95_));
  inv1   g50(.a(new_n91_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(x23), .c(new_n95_), .O(new_n97_));
  nand2  g52(.a(new_n86_), .b(new_n49_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n94_), .O(z13));
  nand4  g54(.a(new_n56_), .b(x19), .c(x18), .d(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x23), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n95_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x21), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n90_), .O(z14));
  nand2  g59(.a(new_n102_), .b(x22), .O(new_n105_));
  nand3  g60(.a(x23), .b(new_n57_), .c(new_n95_), .O(new_n106_));
  nor2   g61(.a(new_n106_), .b(new_n100_), .O(new_n107_));
  aoi21  g62(.a(new_n86_), .b(new_n49_), .c(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n105_), .O(z15));
  nor2   g64(.a(x23), .b(new_n95_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n107_), .c(new_n90_), .O(z16));
  nor2   g66(.a(new_n79_), .b(new_n47_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n80_), .c(new_n51_), .O(z17));
  nand3  g68(.a(new_n112_), .b(new_n80_), .c(new_n52_), .O(z18));
endmodule


