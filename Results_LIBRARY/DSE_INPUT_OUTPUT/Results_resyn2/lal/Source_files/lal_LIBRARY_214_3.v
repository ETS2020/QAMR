// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  and2   g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand3  g12(.a(x24), .b(x22), .c(x21), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n57_), .c(new_n51_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n46_), .O(z01));
  nand2  g16(.a(new_n59_), .b(new_n46_), .O(z03));
  inv1   g17(.a(new_n49_), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n46_), .O(z05));
  nand2  g26(.a(new_n49_), .b(x14), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z06));
  inv1   g28(.a(x08), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  aoi21  g31(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n51_), .c(new_n48_), .O(z08));
  nand2  g34(.a(x05), .b(x04), .O(new_n80_));
  inv1   g35(.a(x07), .O(new_n81_));
  inv1   g36(.a(x15), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n80_), .c(new_n46_), .O(z09));
  nand3  g39(.a(new_n80_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n46_), .O(z10));
  xnor2a g41(.a(x18), .b(x17), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n85_), .c(new_n46_), .O(z11));
  inv1   g43(.a(new_n85_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n46_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  aoi21  g47(.a(x18), .b(x17), .c(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(z12));
  nand2  g49(.a(new_n92_), .b(new_n52_), .O(new_n95_));
  aoi21  g50(.a(new_n91_), .b(x20), .c(new_n85_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n95_), .c(new_n48_), .O(z13));
  inv1   g52(.a(new_n90_), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(x21), .O(new_n99_));
  nor2   g54(.a(x21), .b(x20), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n92_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(z14));
  nand2  g57(.a(new_n101_), .b(x22), .O(new_n103_));
  inv1   g58(.a(x22), .O(new_n104_));
  nand3  g59(.a(new_n100_), .b(new_n92_), .c(new_n104_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n98_), .O(z15));
  inv1   g61(.a(x24), .O(new_n107_));
  nand4  g62(.a(new_n100_), .b(new_n92_), .c(new_n107_), .d(new_n104_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x23), .O(new_n109_));
  nand4  g64(.a(new_n100_), .b(new_n92_), .c(new_n76_), .d(new_n104_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n89_), .O(z16));
  nand2  g66(.a(new_n85_), .b(new_n46_), .O(new_n112_));
  xor2a  g67(.a(new_n105_), .b(x24), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(x23), .c(new_n112_), .O(z17));
  nand2  g69(.a(new_n110_), .b(new_n51_), .O(new_n115_));
  nor2   g70(.a(new_n51_), .b(x23), .O(new_n116_));
  nor2   g71(.a(new_n116_), .b(x24), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  oai21  g73(.a(x25), .b(x24), .c(x23), .O(new_n119_));
  aoi22  g74(.a(new_n119_), .b(new_n85_), .c(new_n116_), .d(new_n108_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n118_), .O(z18));
endmodule


