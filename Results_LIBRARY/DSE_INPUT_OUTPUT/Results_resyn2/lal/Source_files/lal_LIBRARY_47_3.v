// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x24), .O(new_n52_));
  nor2   g07(.a(x19), .b(x17), .O(new_n53_));
  and2   g08(.a(x22), .b(x21), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(x20), .c(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n48_), .c(new_n52_), .O(new_n56_));
  oai21  g11(.a(x25), .b(x20), .c(new_n48_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g15(.a(new_n57_), .b(x18), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n56_), .b(x25), .c(new_n61_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n49_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n50_), .b(new_n52_), .O(new_n65_));
  inv1   g20(.a(new_n54_), .O(new_n66_));
  aoi21  g21(.a(new_n53_), .b(new_n47_), .c(x20), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(new_n66_), .c(new_n48_), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n65_), .c(x25), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g32(.a(new_n50_), .b(x14), .c(new_n46_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand2  g35(.a(new_n68_), .b(x24), .O(new_n81_));
  nor2   g36(.a(new_n49_), .b(x25), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(z08));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n59_), .c(new_n50_), .O(z09));
  nand3  g41(.a(new_n59_), .b(new_n84_), .c(new_n58_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n50_), .O(z10));
  xnor2a g43(.a(x18), .b(x17), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n87_), .c(new_n50_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  aoi21  g47(.a(x18), .b(x17), .c(x19), .O(new_n93_));
  nor4   g48(.a(new_n93_), .b(new_n92_), .c(new_n87_), .d(new_n49_), .O(z12));
  inv1   g49(.a(new_n87_), .O(new_n95_));
  oai21  g50(.a(new_n91_), .b(x23), .c(x20), .O(new_n96_));
  inv1   g51(.a(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(x19), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x18), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n96_), .c(new_n95_), .O(z13));
  nand2  g56(.a(new_n87_), .b(new_n50_), .O(new_n102_));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n92_), .c(new_n48_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z14));
  oai21  g61(.a(new_n98_), .b(x21), .c(new_n48_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x18), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x22), .O(new_n109_));
  inv1   g64(.a(x22), .O(new_n110_));
  nand4  g65(.a(new_n104_), .b(new_n92_), .c(new_n48_), .d(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n102_), .O(z15));
  nand3  g67(.a(new_n104_), .b(new_n92_), .c(new_n110_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n95_), .c(new_n48_), .O(z16));
  nand2  g69(.a(new_n111_), .b(x24), .O(new_n115_));
  inv1   g70(.a(x21), .O(new_n116_));
  nand3  g71(.a(new_n52_), .b(new_n110_), .c(new_n116_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n98_), .c(new_n48_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x18), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n115_), .c(new_n95_), .O(z17));
  oai21  g75(.a(new_n117_), .b(new_n98_), .c(x25), .O(new_n121_));
  nor2   g76(.a(x25), .b(x20), .O(new_n122_));
  nor2   g77(.a(x22), .b(x21), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n92_), .c(new_n122_), .d(new_n52_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n121_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n48_), .O(new_n126_));
  nand2  g81(.a(x25), .b(new_n47_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n102_), .O(z18));
endmodule


