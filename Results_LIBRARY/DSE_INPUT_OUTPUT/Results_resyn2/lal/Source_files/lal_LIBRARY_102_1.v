// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x20), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x14), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n48_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nor2   g17(.a(new_n49_), .b(x07), .O(new_n63_));
  and2   g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n61_), .O(z01));
  nand3  g20(.a(new_n60_), .b(new_n50_), .c(new_n53_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  nand2  g22(.a(new_n50_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  nor2   g30(.a(new_n68_), .b(x13), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n50_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nor2   g34(.a(new_n58_), .b(new_n57_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x23), .c(x24), .O(new_n81_));
  inv1   g36(.a(x24), .O(new_n82_));
  oai21  g37(.a(new_n58_), .b(new_n82_), .c(x14), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(x20), .c(x25), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n81_), .O(z08));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n62_), .c(new_n50_), .O(z09));
  nand2  g43(.a(new_n86_), .b(new_n62_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n49_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n54_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  nand2  g47(.a(new_n55_), .b(new_n54_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n90_), .c(new_n93_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  inv1   g51(.a(new_n89_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n94_), .b(new_n56_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n50_), .O(z12));
  nand2  g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n98_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n90_), .O(z13));
  nor2   g60(.a(new_n98_), .b(x21), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  aoi21  g62(.a(new_n104_), .b(x21), .c(new_n89_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n107_), .c(new_n49_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n106_), .b(new_n110_), .c(new_n48_), .O(new_n111_));
  aoi21  g66(.a(new_n107_), .b(x22), .c(new_n89_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n111_), .c(new_n49_), .O(z15));
  inv1   g68(.a(x23), .O(new_n114_));
  nor3   g69(.a(new_n98_), .b(x22), .c(x21), .O(new_n115_));
  xor2a  g70(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nor2   g71(.a(new_n114_), .b(new_n48_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n89_), .c(new_n50_), .O(new_n118_));
  oai21  g73(.a(new_n116_), .b(x20), .c(new_n118_), .O(z16));
  nand2  g74(.a(new_n115_), .b(new_n114_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x24), .O(new_n121_));
  nor3   g76(.a(x24), .b(x23), .c(x20), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n115_), .O(new_n123_));
  aoi21  g78(.a(new_n82_), .b(x14), .c(new_n48_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n89_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n121_), .O(z17));
  nand3  g81(.a(new_n122_), .b(new_n115_), .c(new_n53_), .O(new_n127_));
  oai21  g82(.a(new_n49_), .b(x25), .c(new_n123_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n97_), .O(z18));
endmodule


