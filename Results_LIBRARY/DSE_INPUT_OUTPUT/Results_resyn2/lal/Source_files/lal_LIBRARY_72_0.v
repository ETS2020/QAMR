// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n74_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x13), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  or2    g07(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nor2   g16(.a(new_n48_), .b(x25), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n61_), .O(z08));
  nand2  g18(.a(z08), .b(new_n53_), .O(z01));
  inv1   g19(.a(z08), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z04));
  aoi21  g27(.a(x15), .b(x08), .c(x13), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n48_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g31(.a(x07), .O(new_n77_));
  nand3  g32(.a(new_n77_), .b(x05), .c(x04), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(x13), .c(x15), .O(z09));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n52_), .b(new_n80_), .c(x13), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(x17), .O(z10));
  nor2   g37(.a(x18), .b(x17), .O(new_n83_));
  nand2  g38(.a(x18), .b(x17), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nor3   g40(.a(new_n85_), .b(new_n81_), .c(new_n83_), .O(z11));
  nand2  g41(.a(new_n84_), .b(new_n57_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n87_), .c(new_n52_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(x13), .c(x15), .O(z12));
  inv1   g45(.a(new_n81_), .O(new_n91_));
  nand4  g46(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n92_));
  nand2  g47(.a(new_n88_), .b(x20), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(z13));
  nand2  g49(.a(new_n92_), .b(x21), .O(new_n95_));
  inv1   g50(.a(x21), .O(new_n96_));
  nor2   g51(.a(new_n88_), .b(x20), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n95_), .c(new_n52_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(x13), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n80_), .O(z14));
  nand2  g56(.a(new_n98_), .b(x22), .O(new_n102_));
  nor2   g57(.a(x22), .b(x21), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n97_), .c(new_n81_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n102_), .O(z15));
  inv1   g60(.a(x23), .O(new_n106_));
  aoi21  g61(.a(new_n103_), .b(new_n97_), .c(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n103_), .b(new_n97_), .c(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n52_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n107_), .c(x13), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n80_), .O(z16));
  nand2  g66(.a(new_n108_), .b(x24), .O(new_n112_));
  nor2   g67(.a(x24), .b(x23), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n103_), .c(new_n97_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n112_), .c(new_n91_), .O(z17));
  inv1   g70(.a(x25), .O(new_n116_));
  nand4  g71(.a(new_n113_), .b(new_n103_), .c(new_n97_), .d(new_n116_), .O(new_n117_));
  nand2  g72(.a(new_n113_), .b(new_n103_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n92_), .c(x25), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n52_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x13), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n80_), .O(z18));
endmodule


