// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x18), .O(new_n48_));
  nor2   g03(.a(x24), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nor3   g12(.a(x19), .b(x18), .c(x17), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x20), .c(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n53_), .c(new_n54_), .O(new_n60_));
  inv1   g15(.a(x25), .O(new_n61_));
  nand2  g16(.a(new_n50_), .b(new_n61_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n50_), .O(new_n65_));
  oai21  g20(.a(new_n62_), .b(new_n60_), .c(new_n65_), .O(z01));
  nor2   g21(.a(new_n62_), .b(new_n60_), .O(z03));
  nand2  g22(.a(new_n50_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  nor2   g30(.a(new_n68_), .b(x13), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n68_), .b(new_n77_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  nand3  g34(.a(x24), .b(x22), .c(x21), .O(new_n80_));
  nor3   g35(.a(new_n80_), .b(x19), .c(x17), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x25), .c(new_n48_), .O(new_n82_));
  nand2  g37(.a(new_n57_), .b(x20), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n61_), .c(new_n53_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x24), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n82_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n64_), .c(new_n50_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand2  g44(.a(new_n87_), .b(new_n63_), .O(new_n90_));
  nor3   g45(.a(new_n90_), .b(new_n49_), .c(x17), .O(z10));
  inv1   g46(.a(x17), .O(new_n92_));
  nand2  g47(.a(x24), .b(x18), .O(new_n93_));
  inv1   g48(.a(new_n90_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi21  g51(.a(new_n93_), .b(new_n92_), .c(new_n96_), .O(z11));
  aoi21  g52(.a(x18), .b(x17), .c(x19), .O(new_n98_));
  inv1   g53(.a(x19), .O(new_n99_));
  oai21  g54(.a(new_n95_), .b(new_n99_), .c(new_n94_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n98_), .c(new_n50_), .O(z12));
  inv1   g56(.a(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(x19), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x24), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x18), .O(new_n105_));
  nand4  g60(.a(x24), .b(x19), .c(x18), .d(x17), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(x20), .c(new_n90_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n105_), .O(z13));
  oai21  g63(.a(new_n103_), .b(x21), .c(x24), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x18), .O(new_n110_));
  oai21  g65(.a(new_n103_), .b(new_n93_), .c(x21), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n94_), .O(z14));
  nand2  g67(.a(new_n110_), .b(x22), .O(new_n113_));
  nand2  g68(.a(new_n90_), .b(new_n50_), .O(new_n114_));
  nor2   g69(.a(new_n103_), .b(new_n93_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n56_), .c(new_n55_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(z15));
  nand2  g72(.a(new_n116_), .b(x23), .O(new_n118_));
  nand3  g73(.a(new_n53_), .b(new_n56_), .c(new_n55_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n103_), .c(x24), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(x18), .c(new_n90_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n118_), .O(z16));
  nand2  g77(.a(new_n54_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n93_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n120_), .c(new_n94_), .O(z17));
  nand3  g80(.a(new_n94_), .b(new_n50_), .c(new_n61_), .O(z18));
endmodule


