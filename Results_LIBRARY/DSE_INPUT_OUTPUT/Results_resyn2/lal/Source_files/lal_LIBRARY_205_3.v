// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  oai21  g09(.a(x18), .b(x17), .c(new_n54_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n55_), .c(x23), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n48_), .c(new_n53_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n50_), .O(z01));
  nor2   g16(.a(new_n49_), .b(new_n46_), .O(z02));
  nand2  g17(.a(new_n58_), .b(new_n47_), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(x24), .c(x25), .O(z03));
  nor2   g19(.a(new_n49_), .b(x08), .O(new_n65_));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  and2   g25(.a(new_n70_), .b(new_n65_), .O(z04));
  inv1   g26(.a(x13), .O(new_n72_));
  nand2  g27(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n50_), .O(z06));
  nand2  g31(.a(new_n65_), .b(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  inv1   g33(.a(x17), .O(new_n79_));
  inv1   g34(.a(x18), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n81_));
  oai21  g36(.a(new_n56_), .b(new_n81_), .c(new_n78_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n53_), .c(new_n49_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n85_), .O(z09));
  nand2  g43(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n50_), .O(z10));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  oai21  g47(.a(x18), .b(x17), .c(new_n85_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n87_), .O(z11));
  nor2   g49(.a(new_n92_), .b(x19), .O(new_n95_));
  inv1   g50(.a(new_n89_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n95_), .c(new_n50_), .O(z12));
  nor2   g54(.a(new_n91_), .b(x20), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(x24), .c(x19), .O(new_n101_));
  nand2  g56(.a(new_n48_), .b(x19), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(new_n91_), .c(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n96_), .O(z13));
  inv1   g59(.a(x21), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n54_), .c(x18), .d(x17), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x19), .O(new_n108_));
  nor3   g63(.a(new_n97_), .b(x24), .c(x20), .O(new_n109_));
  inv1   g64(.a(new_n109_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x21), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n108_), .c(new_n96_), .O(z14));
  nand2  g67(.a(new_n108_), .b(x22), .O(new_n113_));
  aoi22  g68(.a(new_n86_), .b(new_n85_), .c(x24), .d(x19), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n109_), .c(new_n114_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n113_), .O(z15));
  inv1   g72(.a(x22), .O(new_n118_));
  nand3  g73(.a(new_n100_), .b(new_n118_), .c(new_n105_), .O(new_n119_));
  aoi21  g74(.a(new_n119_), .b(new_n48_), .c(new_n47_), .O(new_n120_));
  nor3   g75(.a(x23), .b(x22), .c(x21), .O(new_n121_));
  aoi21  g76(.a(new_n121_), .b(new_n109_), .c(new_n114_), .O(new_n122_));
  oai21  g77(.a(new_n120_), .b(new_n78_), .c(new_n122_), .O(z16));
  nand3  g78(.a(new_n121_), .b(new_n100_), .c(x19), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n96_), .c(new_n48_), .O(z17));
  nand2  g80(.a(new_n124_), .b(new_n53_), .O(new_n126_));
  nand3  g81(.a(new_n121_), .b(new_n100_), .c(x25), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n126_), .c(new_n48_), .O(new_n128_));
  aoi21  g83(.a(x25), .b(new_n47_), .c(new_n114_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n128_), .O(z18));
endmodule


