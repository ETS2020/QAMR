// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x09), .O(new_n48_));
  nor2   g03(.a(x15), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n62_), .c(new_n50_), .O(z01));
  nand3  g19(.a(new_n61_), .b(new_n50_), .c(new_n53_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  oai21  g21(.a(x08), .b(x00), .c(x15), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(x09), .O(new_n68_));
  nand2  g23(.a(new_n48_), .b(x00), .O(new_n69_));
  inv1   g24(.a(x03), .O(new_n70_));
  inv1   g25(.a(x10), .O(new_n71_));
  aoi22  g26(.a(x12), .b(new_n70_), .c(new_n71_), .d(x01), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  inv1   g28(.a(x01), .O(new_n74_));
  nand2  g29(.a(x10), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x12), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(x03), .O(new_n77_));
  xnor2a g32(.a(x11), .b(x02), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n75_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n73_), .c(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n68_), .O(z04));
  oai21  g36(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n49_), .O(z06));
  aoi21  g39(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g40(.a(new_n61_), .b(new_n53_), .c(new_n49_), .O(z08));
  nand2  g41(.a(x05), .b(x04), .O(new_n87_));
  nor3   g42(.a(x15), .b(x09), .c(x07), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor2   g44(.a(new_n89_), .b(new_n87_), .O(z09));
  nand2  g45(.a(new_n63_), .b(new_n55_), .O(new_n91_));
  aoi21  g46(.a(new_n91_), .b(new_n48_), .c(x15), .O(z10));
  nand2  g47(.a(new_n56_), .b(new_n55_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n63_), .c(new_n93_), .O(new_n95_));
  aoi21  g50(.a(new_n95_), .b(new_n48_), .c(x15), .O(z11));
  nand2  g51(.a(new_n94_), .b(new_n57_), .O(new_n97_));
  inv1   g52(.a(new_n94_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x19), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n97_), .c(new_n63_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n48_), .c(x15), .O(z12));
  nand2  g56(.a(new_n99_), .b(x20), .O(new_n102_));
  nand4  g57(.a(new_n54_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n102_), .c(new_n88_), .d(new_n87_), .O(z13));
  nand2  g59(.a(new_n103_), .b(x21), .O(new_n105_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(new_n106_));
  nor2   g61(.a(new_n103_), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n105_), .O(z14));
  inv1   g64(.a(x15), .O(new_n110_));
  inv1   g65(.a(x21), .O(new_n111_));
  inv1   g66(.a(x22), .O(new_n112_));
  nor2   g67(.a(x20), .b(new_n57_), .O(new_n113_));
  nand4  g68(.a(new_n113_), .b(new_n98_), .c(new_n112_), .d(new_n111_), .O(new_n114_));
  oai21  g69(.a(new_n103_), .b(x21), .c(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n63_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n110_), .O(z15));
  nand2  g73(.a(new_n114_), .b(x23), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n107_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n63_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n48_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n110_), .O(z16));
  nand2  g79(.a(new_n121_), .b(x24), .O(new_n125_));
  inv1   g80(.a(x23), .O(new_n126_));
  inv1   g81(.a(x24), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n126_), .c(new_n112_), .d(new_n111_), .O(new_n128_));
  nor2   g83(.a(new_n128_), .b(new_n103_), .O(new_n129_));
  nor2   g84(.a(new_n129_), .b(new_n106_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n125_), .O(z17));
  nand2  g86(.a(new_n129_), .b(new_n53_), .O(new_n132_));
  oai21  g87(.a(new_n128_), .b(new_n103_), .c(x25), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n132_), .c(new_n63_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n48_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n110_), .O(z18));
endmodule


