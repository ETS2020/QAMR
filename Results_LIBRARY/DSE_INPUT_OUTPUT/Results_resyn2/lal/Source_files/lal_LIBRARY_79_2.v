// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n91_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g000(.a(x08), .O(new_n46_));
  inv1   g001(.a(x24), .O(new_n47_));
  nand2  g002(.a(new_n47_), .b(x19), .O(new_n48_));
  inv1   g003(.a(new_n48_), .O(new_n49_));
  nor3   g004(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g005(.a(x25), .O(new_n51_));
  inv1   g006(.a(x20), .O(new_n52_));
  oai21  g007(.a(x18), .b(x17), .c(new_n52_), .O(new_n53_));
  nand2  g008(.a(x22), .b(x21), .O(new_n54_));
  inv1   g009(.a(new_n54_), .O(new_n55_));
  aoi21  g010(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g011(.a(new_n56_), .b(new_n47_), .c(new_n51_), .O(new_n57_));
  nor2   g012(.a(x25), .b(x23), .O(new_n58_));
  nand2  g013(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  nand2  g014(.a(new_n59_), .b(x24), .O(new_n60_));
  inv1   g015(.a(x07), .O(new_n61_));
  nand2  g016(.a(x05), .b(x04), .O(new_n62_));
  nand2  g017(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g018(.a(new_n60_), .b(x19), .c(new_n63_), .O(new_n64_));
  nand2  g019(.a(new_n64_), .b(new_n57_), .O(z01));
  and2   g020(.a(new_n48_), .b(x16), .O(z02));
  nand2  g021(.a(new_n60_), .b(x19), .O(new_n67_));
  nand2  g022(.a(new_n67_), .b(new_n57_), .O(z03));
  inv1   g023(.a(x00), .O(new_n69_));
  nand2  g024(.a(x09), .b(new_n69_), .O(new_n70_));
  inv1   g025(.a(x02), .O(new_n71_));
  nand2  g026(.a(x11), .b(new_n71_), .O(new_n72_));
  inv1   g027(.a(x03), .O(new_n73_));
  nand2  g028(.a(x12), .b(new_n73_), .O(new_n74_));
  inv1   g029(.a(x11), .O(new_n75_));
  nand2  g030(.a(new_n75_), .b(x02), .O(new_n76_));
  nand4  g031(.a(new_n76_), .b(new_n74_), .c(new_n72_), .d(new_n70_), .O(new_n77_));
  inv1   g032(.a(x01), .O(new_n78_));
  nand2  g033(.a(x10), .b(new_n78_), .O(new_n79_));
  inv1   g034(.a(x09), .O(new_n80_));
  nand2  g035(.a(new_n80_), .b(x00), .O(new_n81_));
  inv1   g036(.a(x10), .O(new_n82_));
  nand2  g037(.a(new_n82_), .b(x01), .O(new_n83_));
  inv1   g038(.a(x12), .O(new_n84_));
  nand2  g039(.a(new_n84_), .b(x03), .O(new_n85_));
  nand4  g040(.a(new_n85_), .b(new_n83_), .c(new_n81_), .d(new_n79_), .O(new_n86_));
  oai21  g041(.a(new_n86_), .b(new_n77_), .c(new_n46_), .O(new_n87_));
  nand2  g042(.a(new_n87_), .b(new_n48_), .O(z04));
  nand2  g043(.a(new_n48_), .b(new_n46_), .O(new_n89_));
  nor2   g044(.a(new_n89_), .b(x13), .O(z05));
  inv1   g045(.a(x14), .O(new_n91_));
  nor2   g046(.a(new_n89_), .b(new_n91_), .O(z06));
  aoi21  g047(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  oai21  g048(.a(new_n54_), .b(new_n52_), .c(new_n58_), .O(new_n94_));
  nand2  g049(.a(new_n94_), .b(x24), .O(new_n95_));
  inv1   g050(.a(x19), .O(new_n96_));
  inv1   g051(.a(x17), .O(new_n97_));
  inv1   g052(.a(x18), .O(new_n98_));
  nand3  g053(.a(x24), .b(new_n98_), .c(new_n97_), .O(new_n99_));
  oai21  g054(.a(new_n99_), .b(new_n54_), .c(new_n51_), .O(new_n100_));
  nand2  g055(.a(new_n100_), .b(new_n96_), .O(new_n101_));
  nand2  g056(.a(new_n101_), .b(new_n95_), .O(z08));
  inv1   g057(.a(new_n62_), .O(new_n103_));
  nor2   g058(.a(x15), .b(x07), .O(new_n104_));
  nand3  g059(.a(new_n104_), .b(new_n103_), .c(new_n48_), .O(new_n105_));
  inv1   g060(.a(new_n105_), .O(z09));
  nand2  g061(.a(new_n104_), .b(new_n62_), .O(new_n107_));
  oai21  g062(.a(new_n107_), .b(x17), .c(new_n48_), .O(z10));
  nand2  g063(.a(new_n98_), .b(new_n97_), .O(new_n109_));
  nand3  g064(.a(new_n104_), .b(new_n62_), .c(new_n48_), .O(new_n110_));
  inv1   g065(.a(new_n110_), .O(new_n111_));
  nand2  g066(.a(x18), .b(x17), .O(new_n112_));
  nand3  g067(.a(new_n112_), .b(new_n111_), .c(new_n109_), .O(new_n113_));
  inv1   g068(.a(new_n113_), .O(z11));
  inv1   g069(.a(new_n112_), .O(new_n115_));
  nor2   g070(.a(new_n47_), .b(new_n96_), .O(new_n116_));
  nor2   g071(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  inv1   g072(.a(new_n107_), .O(new_n118_));
  nand2  g073(.a(new_n115_), .b(x19), .O(new_n119_));
  nand2  g074(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nor2   g075(.a(new_n120_), .b(new_n117_), .O(z12));
  nand3  g076(.a(new_n52_), .b(x18), .c(x17), .O(new_n122_));
  inv1   g077(.a(new_n122_), .O(new_n123_));
  oai21  g078(.a(new_n123_), .b(new_n47_), .c(x19), .O(new_n124_));
  nand2  g079(.a(new_n116_), .b(new_n115_), .O(new_n125_));
  nand2  g080(.a(new_n125_), .b(x20), .O(new_n126_));
  nand3  g081(.a(new_n126_), .b(new_n124_), .c(new_n118_), .O(z13));
  nand2  g082(.a(new_n107_), .b(new_n48_), .O(new_n128_));
  nand2  g083(.a(new_n124_), .b(x21), .O(new_n129_));
  inv1   g084(.a(x21), .O(new_n130_));
  nand3  g085(.a(new_n123_), .b(new_n116_), .c(new_n130_), .O(new_n131_));
  nand3  g086(.a(new_n131_), .b(new_n129_), .c(new_n128_), .O(z14));
  inv1   g087(.a(x22), .O(new_n133_));
  nand2  g088(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  oai21  g089(.a(new_n134_), .b(new_n122_), .c(x24), .O(new_n135_));
  nand2  g090(.a(new_n135_), .b(x19), .O(new_n136_));
  nand2  g091(.a(new_n131_), .b(x22), .O(new_n137_));
  nand3  g092(.a(new_n137_), .b(new_n136_), .c(new_n118_), .O(z15));
  nand2  g093(.a(new_n136_), .b(x23), .O(new_n139_));
  inv1   g094(.a(x23), .O(new_n140_));
  nand4  g095(.a(new_n140_), .b(new_n133_), .c(new_n130_), .d(new_n52_), .O(new_n141_));
  nor2   g096(.a(new_n141_), .b(new_n119_), .O(new_n142_));
  nand2  g097(.a(new_n142_), .b(x24), .O(new_n143_));
  nand3  g098(.a(new_n143_), .b(new_n139_), .c(new_n128_), .O(z16));
  oai21  g099(.a(new_n142_), .b(new_n47_), .c(new_n128_), .O(z17));
  nand2  g100(.a(new_n111_), .b(new_n51_), .O(z18));
endmodule


