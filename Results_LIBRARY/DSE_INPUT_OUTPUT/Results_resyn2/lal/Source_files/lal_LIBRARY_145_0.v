// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x23), .b(x06), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  aoi21  g06(.a(new_n51_), .b(new_n50_), .c(x20), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  inv1   g08(.a(x06), .O(new_n54_));
  nor2   g09(.a(x23), .b(new_n54_), .O(new_n55_));
  oai21  g10(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nor2   g12(.a(x25), .b(x24), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x07), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  oai21  g16(.a(new_n56_), .b(x25), .c(new_n61_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n47_), .b(new_n63_), .O(z02));
  inv1   g19(.a(x24), .O(new_n65_));
  nand2  g20(.a(new_n48_), .b(new_n65_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n56_), .c(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n48_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor3   g31(.a(new_n47_), .b(new_n76_), .c(x08), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  inv1   g34(.a(x20), .O(new_n80_));
  inv1   g35(.a(x17), .O(new_n81_));
  inv1   g36(.a(x18), .O(new_n82_));
  nand3  g37(.a(new_n50_), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand3  g39(.a(x24), .b(x22), .c(x21), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(new_n84_), .c(x25), .O(new_n87_));
  oai22  g42(.a(new_n87_), .b(new_n54_), .c(new_n58_), .d(new_n79_), .O(z08));
  inv1   g43(.a(x07), .O(new_n89_));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n57_), .c(new_n48_), .O(z09));
  nand3  g47(.a(new_n57_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n48_), .c(new_n81_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nor2   g51(.a(new_n82_), .b(new_n81_), .O(new_n97_));
  or2    g52(.a(new_n97_), .b(new_n51_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n93_), .c(new_n48_), .O(z11));
  nand2  g54(.a(new_n94_), .b(new_n48_), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nor2   g57(.a(new_n97_), .b(x19), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z12));
  nand4  g59(.a(new_n80_), .b(x19), .c(x18), .d(x17), .O(new_n105_));
  aoi21  g60(.a(new_n101_), .b(x20), .c(new_n93_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n105_), .c(new_n47_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand3  g64(.a(new_n102_), .b(new_n109_), .c(new_n80_), .O(new_n110_));
  and2   g65(.a(new_n110_), .b(new_n94_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n108_), .c(new_n47_), .O(z14));
  nand2  g67(.a(new_n110_), .b(x22), .O(new_n113_));
  nor3   g68(.a(x22), .b(x21), .c(x20), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n102_), .c(new_n93_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n113_), .c(new_n47_), .O(z15));
  nand3  g71(.a(new_n114_), .b(new_n102_), .c(new_n55_), .O(new_n117_));
  nand2  g72(.a(new_n93_), .b(new_n48_), .O(new_n118_));
  nand2  g73(.a(new_n114_), .b(new_n102_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x23), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(z16));
  aoi21  g76(.a(new_n114_), .b(new_n102_), .c(new_n65_), .O(new_n122_));
  inv1   g77(.a(x22), .O(new_n123_));
  nand4  g78(.a(new_n65_), .b(new_n79_), .c(new_n123_), .d(new_n109_), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n105_), .O(new_n125_));
  oai21  g80(.a(new_n125_), .b(new_n122_), .c(x06), .O(new_n126_));
  aoi22  g81(.a(new_n93_), .b(new_n48_), .c(x24), .d(x23), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(z17));
  inv1   g83(.a(x25), .O(new_n129_));
  xor2a  g84(.a(new_n125_), .b(new_n129_), .O(new_n130_));
  nand2  g85(.a(new_n129_), .b(x23), .O(new_n131_));
  aoi21  g86(.a(new_n131_), .b(new_n54_), .c(new_n93_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n130_), .O(z18));
endmodule


