// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n80_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x04), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nor2   g15(.a(x25), .b(x23), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n60_), .c(x05), .O(new_n62_));
  inv1   g17(.a(x25), .O(new_n63_));
  aoi22  g18(.a(new_n49_), .b(x07), .c(new_n63_), .d(new_n47_), .O(new_n64_));
  oai21  g19(.a(new_n62_), .b(new_n51_), .c(new_n64_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  aoi21  g22(.a(new_n56_), .b(new_n52_), .c(new_n58_), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x23), .c(x24), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z03));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n49_), .O(z04));
  oai21  g33(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g34(.a(x14), .O(new_n80_));
  nor3   g35(.a(new_n48_), .b(new_n80_), .c(x08), .O(z06));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g37(.a(new_n69_), .b(new_n63_), .c(new_n48_), .O(z08));
  nand2  g38(.a(x05), .b(x04), .O(new_n84_));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n84_), .c(new_n49_), .O(z09));
  nand2  g42(.a(new_n85_), .b(new_n84_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n48_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n53_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  xnor2a g46(.a(x18), .b(x17), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n88_), .c(new_n49_), .O(z11));
  aoi21  g48(.a(x18), .b(x17), .c(x19), .O(new_n94_));
  inv1   g49(.a(new_n88_), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n94_), .c(new_n49_), .O(z12));
  inv1   g53(.a(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n52_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x20), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n89_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n99_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n89_), .O(z14));
  inv1   g61(.a(x22), .O(new_n107_));
  aoi21  g62(.a(new_n104_), .b(new_n99_), .c(new_n107_), .O(new_n108_));
  nor3   g63(.a(x22), .b(x21), .c(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n99_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n85_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n108_), .c(new_n49_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n84_), .O(z15));
  nand2  g68(.a(new_n110_), .b(x23), .O(new_n114_));
  inv1   g69(.a(x23), .O(new_n115_));
  nand3  g70(.a(new_n109_), .b(new_n99_), .c(new_n115_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n89_), .O(z16));
  oai21  g72(.a(new_n116_), .b(new_n51_), .c(x24), .O(new_n118_));
  nand4  g73(.a(new_n109_), .b(new_n99_), .c(new_n47_), .d(new_n115_), .O(new_n119_));
  nand2  g74(.a(new_n47_), .b(new_n51_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(x05), .c(new_n86_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(z17));
  xor2a  g77(.a(new_n119_), .b(x25), .O(new_n123_));
  nand3  g78(.a(new_n109_), .b(new_n99_), .c(new_n61_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n51_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n123_), .c(new_n95_), .O(z18));
endmodule


