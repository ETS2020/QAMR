// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand4  g11(.a(new_n56_), .b(x24), .c(x22), .d(x21), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  inv1   g13(.a(x07), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n47_), .c(new_n59_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(z01));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g19(.a(new_n58_), .b(new_n47_), .O(z03));
  inv1   g20(.a(x08), .O(new_n66_));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n66_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n47_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g31(.a(new_n47_), .O(new_n77_));
  aoi21  g32(.a(new_n66_), .b(x06), .c(new_n77_), .O(z07));
  nand2  g33(.a(x22), .b(x21), .O(new_n79_));
  aoi21  g34(.a(new_n55_), .b(new_n51_), .c(new_n79_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(x23), .c(x24), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n50_), .O(z08));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n59_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n60_), .c(new_n47_), .O(z09));
  nand3  g40(.a(new_n60_), .b(new_n83_), .c(new_n59_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n47_), .c(new_n52_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  nor2   g44(.a(x18), .b(x17), .O(new_n90_));
  nor2   g45(.a(new_n53_), .b(new_n52_), .O(new_n91_));
  nor4   g46(.a(new_n91_), .b(new_n86_), .c(new_n90_), .d(new_n77_), .O(z11));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n47_), .O(z12));
  inv1   g51(.a(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n51_), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(x20), .c(new_n86_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n98_), .c(new_n77_), .O(z13));
  nand2  g55(.a(new_n98_), .b(x21), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n97_), .c(new_n86_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n101_), .c(new_n77_), .O(z14));
  nand2  g59(.a(new_n102_), .b(new_n97_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x22), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  nand3  g62(.a(new_n102_), .b(new_n97_), .c(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n87_), .d(new_n47_), .O(z15));
  inv1   g64(.a(x24), .O(new_n110_));
  nand3  g65(.a(new_n108_), .b(new_n110_), .c(x23), .O(new_n111_));
  inv1   g66(.a(x23), .O(new_n112_));
  nand4  g67(.a(new_n102_), .b(new_n97_), .c(new_n112_), .d(new_n107_), .O(new_n113_));
  nand2  g68(.a(new_n86_), .b(new_n47_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n111_), .O(z16));
  nand2  g70(.a(new_n108_), .b(new_n110_), .O(new_n116_));
  nand4  g71(.a(new_n102_), .b(new_n97_), .c(x24), .d(new_n107_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n112_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n114_), .O(z17));
  nand2  g74(.a(new_n113_), .b(new_n50_), .O(new_n120_));
  nor2   g75(.a(new_n50_), .b(x23), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  oai21  g78(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  nand4  g79(.a(new_n102_), .b(new_n97_), .c(new_n110_), .d(new_n107_), .O(new_n125_));
  aoi22  g80(.a(new_n125_), .b(new_n121_), .c(new_n124_), .d(new_n86_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n123_), .O(z18));
endmodule


