// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x13), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(x07), .c(new_n50_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  inv1   g14(.a(x21), .O(new_n60_));
  inv1   g15(.a(x22), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n59_), .c(x23), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(x13), .c(new_n47_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x25), .c(new_n53_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nor2   g22(.a(new_n64_), .b(x25), .O(z03));
  nand2  g23(.a(new_n50_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor3   g31(.a(x24), .b(x13), .c(x08), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z06));
  nand3  g34(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g35(.a(x25), .O(new_n81_));
  inv1   g36(.a(x23), .O(new_n82_));
  nand2  g37(.a(new_n62_), .b(new_n59_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x24), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n81_), .c(new_n48_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n52_), .c(new_n50_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand2  g44(.a(new_n87_), .b(new_n51_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g46(.a(x18), .b(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nor2   g48(.a(new_n90_), .b(new_n48_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(new_n57_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n90_), .c(new_n48_), .O(z12));
  inv1   g56(.a(new_n98_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n54_), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n94_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nand3  g61(.a(new_n102_), .b(new_n60_), .c(new_n54_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n94_), .O(z14));
  nand2  g63(.a(new_n107_), .b(x22), .O(new_n109_));
  nand2  g64(.a(new_n61_), .b(new_n60_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n103_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n90_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n109_), .c(new_n48_), .O(z15));
  oai21  g68(.a(new_n110_), .b(new_n103_), .c(x23), .O(new_n114_));
  nand2  g69(.a(new_n111_), .b(new_n82_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n94_), .O(z16));
  nand3  g71(.a(new_n111_), .b(new_n47_), .c(new_n82_), .O(new_n117_));
  nand2  g72(.a(new_n115_), .b(x24), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n94_), .O(z17));
  inv1   g74(.a(new_n90_), .O(new_n120_));
  nand2  g75(.a(new_n50_), .b(new_n81_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand4  g77(.a(new_n111_), .b(new_n81_), .c(new_n47_), .d(new_n82_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n120_), .O(z18));
endmodule


