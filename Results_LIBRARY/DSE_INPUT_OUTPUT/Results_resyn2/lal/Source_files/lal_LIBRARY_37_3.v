// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n81_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x05), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x05), .O(new_n49_));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  and2   g11(.a(x22), .b(x21), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n50_), .c(new_n49_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(x25), .c(x24), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x07), .c(new_n47_), .O(new_n62_));
  oai21  g17(.a(new_n59_), .b(x25), .c(new_n62_), .O(z01));
  and2   g18(.a(new_n47_), .b(x16), .O(z02));
  inv1   g19(.a(x24), .O(new_n65_));
  inv1   g20(.a(x25), .O(new_n66_));
  aoi21  g21(.a(new_n57_), .b(new_n56_), .c(x23), .O(new_n67_));
  oai21  g22(.a(new_n67_), .b(new_n65_), .c(new_n66_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n47_), .O(z03));
  nand2  g24(.a(new_n47_), .b(new_n46_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g35(.a(new_n47_), .O(new_n81_));
  aoi21  g36(.a(new_n46_), .b(x06), .c(new_n81_), .O(z07));
  nand2  g37(.a(new_n59_), .b(x24), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n66_), .O(z08));
  nor2   g39(.a(x15), .b(x07), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand3  g41(.a(new_n65_), .b(x05), .c(x04), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n86_), .O(z09));
  nand3  g43(.a(new_n85_), .b(new_n60_), .c(new_n52_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n47_), .O(z10));
  nand2  g45(.a(new_n53_), .b(new_n52_), .O(new_n91_));
  oai21  g46(.a(x24), .b(x04), .c(x05), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n91_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z11));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  aoi21  g54(.a(x18), .b(x17), .c(x19), .O(new_n100_));
  nor3   g55(.a(new_n100_), .b(new_n99_), .c(new_n93_), .O(z12));
  nand2  g56(.a(new_n99_), .b(new_n51_), .O(new_n102_));
  nand2  g57(.a(new_n98_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n94_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  nor3   g60(.a(new_n98_), .b(x21), .c(x20), .O(new_n106_));
  nand2  g61(.a(new_n87_), .b(new_n85_), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n105_), .c(new_n81_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand2  g65(.a(new_n106_), .b(new_n110_), .O(new_n111_));
  nor2   g66(.a(x21), .b(x20), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n99_), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(x22), .c(new_n107_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n111_), .c(new_n81_), .O(z15));
  nand2  g70(.a(new_n111_), .b(x23), .O(new_n116_));
  nand4  g71(.a(new_n112_), .b(new_n99_), .c(new_n50_), .d(new_n110_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n94_), .O(z16));
  nand4  g73(.a(new_n106_), .b(new_n65_), .c(new_n50_), .d(new_n110_), .O(new_n119_));
  aoi21  g74(.a(new_n117_), .b(x24), .c(new_n107_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n119_), .c(new_n81_), .O(z17));
  inv1   g76(.a(new_n117_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n66_), .c(new_n65_), .O(new_n123_));
  nand2  g78(.a(new_n119_), .b(x25), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n94_), .O(z18));
endmodule


