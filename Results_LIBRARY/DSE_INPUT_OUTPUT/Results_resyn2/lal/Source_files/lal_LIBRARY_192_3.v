// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  and2   g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand3  g10(.a(x24), .b(x22), .c(x21), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n55_), .c(new_n49_), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n47_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nand2  g15(.a(new_n47_), .b(new_n60_), .O(z02));
  nand2  g16(.a(new_n57_), .b(new_n47_), .O(z03));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g28(.a(new_n47_), .O(new_n74_));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n74_), .O(z07));
  inv1   g30(.a(x23), .O(new_n76_));
  aoi21  g31(.a(new_n54_), .b(new_n50_), .c(new_n56_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n49_), .c(new_n74_), .O(z08));
  nand2  g34(.a(x05), .b(x04), .O(new_n80_));
  inv1   g35(.a(x07), .O(new_n81_));
  inv1   g36(.a(x15), .O(new_n82_));
  nand3  g37(.a(new_n47_), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(new_n80_), .O(z09));
  nand3  g39(.a(new_n80_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n47_), .O(z10));
  xnor2a g41(.a(x18), .b(x17), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n85_), .c(new_n47_), .O(z11));
  inv1   g43(.a(new_n83_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n80_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  aoi21  g47(.a(x18), .b(x17), .c(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(z12));
  nand2  g49(.a(new_n92_), .b(new_n50_), .O(new_n95_));
  aoi21  g50(.a(new_n91_), .b(x20), .c(new_n85_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n95_), .c(new_n74_), .O(z13));
  nand2  g52(.a(new_n95_), .b(x21), .O(new_n98_));
  nor2   g53(.a(x21), .b(x20), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n92_), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n98_), .c(new_n89_), .d(new_n80_), .O(z14));
  nand2  g56(.a(new_n100_), .b(x22), .O(new_n102_));
  inv1   g57(.a(new_n85_), .O(new_n103_));
  inv1   g58(.a(x22), .O(new_n104_));
  nand3  g59(.a(new_n99_), .b(new_n92_), .c(new_n104_), .O(new_n105_));
  and2   g60(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n102_), .c(new_n74_), .O(z15));
  inv1   g62(.a(x24), .O(new_n108_));
  nand4  g63(.a(new_n99_), .b(new_n92_), .c(new_n108_), .d(new_n104_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x23), .O(new_n110_));
  nand4  g65(.a(new_n99_), .b(new_n92_), .c(new_n76_), .d(new_n104_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n103_), .O(z16));
  nand2  g67(.a(new_n85_), .b(new_n47_), .O(new_n113_));
  xor2a  g68(.a(new_n105_), .b(x24), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(x23), .c(new_n113_), .O(z17));
  inv1   g70(.a(new_n111_), .O(new_n116_));
  nor2   g71(.a(x25), .b(x24), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n109_), .b(x25), .O(new_n119_));
  oai21  g74(.a(new_n117_), .b(new_n76_), .c(new_n103_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(z18));
endmodule


