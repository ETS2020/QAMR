// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n75_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nand2  g02(.a(x25), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(z08));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  and2   g15(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(z08), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n48_), .b(new_n63_), .O(z02));
  inv1   g19(.a(z08), .O(z03));
  nand2  g20(.a(new_n48_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n48_), .O(z06));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g32(.a(x05), .b(x04), .O(new_n78_));
  nor4   g33(.a(new_n78_), .b(x25), .c(x15), .d(x07), .O(z09));
  nor3   g34(.a(x25), .b(x15), .c(x07), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n52_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z10));
  nand2  g39(.a(new_n53_), .b(new_n52_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n60_), .c(new_n85_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n50_), .c(x15), .O(z11));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  aoi21  g45(.a(x18), .b(x17), .c(x19), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n90_), .c(new_n81_), .O(z12));
  nand2  g47(.a(new_n90_), .b(new_n51_), .O(new_n93_));
  nand2  g48(.a(new_n89_), .b(x20), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n82_), .O(z13));
  nand2  g50(.a(new_n93_), .b(x21), .O(new_n96_));
  nor2   g51(.a(x21), .b(x20), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n90_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n60_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n47_), .O(z14));
  nand2  g56(.a(new_n98_), .b(x22), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n97_), .b(new_n90_), .c(new_n103_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n82_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  nor2   g61(.a(x23), .b(x22), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n97_), .c(new_n90_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n60_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n50_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(z16));
  nand2  g66(.a(new_n108_), .b(x24), .O(new_n112_));
  or2    g67(.a(new_n108_), .b(x24), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n82_), .O(z17));
  nand2  g69(.a(new_n113_), .b(new_n82_), .O(z18));
endmodule


