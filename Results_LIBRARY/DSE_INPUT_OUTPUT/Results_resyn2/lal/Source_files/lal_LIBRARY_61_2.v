// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  inv1   g04(.a(x08), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(x23), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(z02), .O(z00));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  inv1   g17(.a(x25), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n47_), .O(new_n64_));
  nand2  g19(.a(x25), .b(new_n47_), .O(new_n65_));
  inv1   g20(.a(x07), .O(new_n66_));
  nand2  g21(.a(x05), .b(x04), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g23(.a(new_n65_), .b(new_n51_), .c(new_n68_), .O(new_n69_));
  oai21  g24(.a(new_n64_), .b(new_n62_), .c(new_n69_), .O(z01));
  nand2  g25(.a(new_n65_), .b(new_n51_), .O(new_n71_));
  oai21  g26(.a(new_n64_), .b(new_n62_), .c(new_n71_), .O(z03));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n53_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n52_), .O(z05));
  inv1   g35(.a(x14), .O(new_n81_));
  nor2   g36(.a(new_n53_), .b(new_n81_), .O(z06));
  nand2  g37(.a(new_n54_), .b(x06), .O(z07));
  nand2  g38(.a(new_n60_), .b(new_n56_), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(x24), .c(x22), .d(x21), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n63_), .c(new_n47_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n66_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n67_), .c(new_n52_), .O(z09));
  nand3  g44(.a(new_n67_), .b(new_n87_), .c(new_n66_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n52_), .O(z10));
  nand2  g46(.a(new_n58_), .b(new_n57_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n52_), .O(z11));
  inv1   g50(.a(new_n90_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n93_), .b(new_n59_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n52_), .O(z12));
  nand4  g55(.a(new_n56_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nor2   g56(.a(new_n90_), .b(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  inv1   g60(.a(new_n97_), .O(new_n106_));
  nor2   g61(.a(x21), .b(x20), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n90_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n105_), .c(new_n48_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n107_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  nand4  g66(.a(new_n107_), .b(x19), .c(x18), .d(x17), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(x22), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n102_), .O(z15));
  nand2  g69(.a(x24), .b(new_n110_), .O(new_n115_));
  oai21  g70(.a(new_n115_), .b(new_n112_), .c(x23), .O(new_n116_));
  nand4  g71(.a(new_n107_), .b(new_n106_), .c(new_n47_), .d(new_n110_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n96_), .O(z16));
  nand2  g73(.a(new_n90_), .b(new_n52_), .O(new_n119_));
  nand2  g74(.a(new_n117_), .b(x24), .O(new_n120_));
  inv1   g75(.a(new_n101_), .O(new_n121_));
  inv1   g76(.a(x21), .O(new_n122_));
  nand3  g77(.a(new_n51_), .b(new_n110_), .c(new_n122_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n121_), .c(new_n47_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n120_), .c(new_n119_), .O(z17));
  oai21  g81(.a(new_n123_), .b(new_n101_), .c(x25), .O(new_n127_));
  nand3  g82(.a(new_n124_), .b(new_n121_), .c(new_n63_), .O(new_n128_));
  nand2  g83(.a(new_n63_), .b(x24), .O(new_n129_));
  aoi21  g84(.a(new_n129_), .b(x23), .c(new_n90_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n128_), .c(new_n127_), .O(z18));
endmodule


