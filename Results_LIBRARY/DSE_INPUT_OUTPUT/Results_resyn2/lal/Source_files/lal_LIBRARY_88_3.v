// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x20), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nor2   g12(.a(x18), .b(x17), .O(new_n58_));
  nand4  g13(.a(new_n58_), .b(x22), .c(x21), .d(new_n57_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n56_), .c(new_n55_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n54_), .O(z08));
  nand2  g17(.a(z08), .b(new_n53_), .O(z01));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g19(.a(x24), .O(new_n65_));
  aoi21  g20(.a(new_n59_), .b(new_n56_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n47_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n47_), .O(z04));
  inv1   g29(.a(x13), .O(new_n75_));
  inv1   g30(.a(new_n47_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z05));
  nand2  g34(.a(new_n77_), .b(x14), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z06));
  nand2  g36(.a(new_n77_), .b(x06), .O(z07));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n51_), .c(new_n47_), .O(z09));
  inv1   g40(.a(x17), .O(new_n86_));
  nand3  g41(.a(new_n51_), .b(new_n83_), .c(new_n50_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n47_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n86_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  and2   g47(.a(x18), .b(x17), .O(new_n93_));
  or2    g48(.a(new_n93_), .b(new_n58_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n87_), .c(new_n47_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n93_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n89_), .O(z12));
  nand3  g54(.a(new_n96_), .b(new_n65_), .c(x20), .O(new_n100_));
  nand2  g55(.a(new_n87_), .b(new_n47_), .O(new_n101_));
  nor2   g56(.a(new_n96_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(z13));
  nand2  g59(.a(new_n103_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nand2  g61(.a(new_n102_), .b(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n90_), .O(z14));
  oai21  g63(.a(new_n76_), .b(x22), .c(new_n107_), .O(new_n109_));
  nor2   g64(.a(x22), .b(x21), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n102_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n88_), .O(z15));
  nand3  g67(.a(new_n110_), .b(new_n102_), .c(new_n56_), .O(new_n113_));
  oai21  g68(.a(new_n76_), .b(x23), .c(new_n111_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n88_), .O(z16));
  nand3  g70(.a(new_n110_), .b(new_n97_), .c(new_n56_), .O(new_n116_));
  xor2a  g71(.a(new_n116_), .b(x24), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(x20), .c(new_n101_), .O(z17));
  nand2  g73(.a(new_n111_), .b(new_n54_), .O(new_n119_));
  nand2  g74(.a(new_n54_), .b(new_n56_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n55_), .c(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  oai21  g77(.a(x25), .b(x24), .c(x20), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n87_), .O(new_n124_));
  nor2   g79(.a(new_n54_), .b(x20), .O(new_n125_));
  oai21  g80(.a(new_n116_), .b(x24), .c(new_n125_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n124_), .c(new_n122_), .O(z18));
endmodule


