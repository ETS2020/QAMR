// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n80_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x17), .b(x15), .O(z10));
  nor3   g02(.a(z10), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x17), .O(new_n49_));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(new_n54_));
  nor2   g09(.a(x19), .b(x18), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(x24), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n52_), .c(x15), .O(new_n57_));
  inv1   g12(.a(x20), .O(new_n58_));
  inv1   g13(.a(x23), .O(new_n59_));
  oai21  g14(.a(new_n53_), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(x24), .c(x25), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  aoi21  g17(.a(new_n57_), .b(new_n49_), .c(new_n62_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(z10), .b(new_n64_), .O(z02));
  inv1   g20(.a(z10), .O(new_n66_));
  inv1   g21(.a(x24), .O(new_n67_));
  nor3   g22(.a(x19), .b(x18), .c(x17), .O(new_n68_));
  oai21  g23(.a(new_n68_), .b(x20), .c(new_n54_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n59_), .c(new_n67_), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x25), .c(new_n66_), .O(z03));
  xnor2a g26(.a(x12), .b(x03), .O(new_n72_));
  xnor2a g27(.a(x11), .b(x02), .O(new_n73_));
  xnor2a g28(.a(x09), .b(x00), .O(new_n74_));
  xnor2a g29(.a(x10), .b(x01), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n66_), .O(z04));
  nor3   g33(.a(z10), .b(x13), .c(x08), .O(z05));
  nand2  g34(.a(x14), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n66_), .O(z06));
  nand3  g36(.a(new_n66_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g37(.a(new_n56_), .b(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n49_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n61_), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  nand3  g41(.a(x17), .b(new_n86_), .c(new_n50_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n51_), .O(z09));
  inv1   g43(.a(x18), .O(new_n89_));
  inv1   g44(.a(new_n87_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n51_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z11));
  and2   g49(.a(x19), .b(x18), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n91_), .c(new_n55_), .O(z12));
  nand2  g51(.a(x19), .b(x18), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x20), .O(new_n98_));
  nor2   g53(.a(new_n97_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n98_), .c(new_n92_), .O(z13));
  inv1   g56(.a(x21), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n100_), .b(x21), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n92_), .O(z14));
  nand2  g60(.a(new_n103_), .b(x22), .O(new_n106_));
  nor2   g61(.a(x22), .b(x21), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n99_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n92_), .O(z15));
  nand2  g64(.a(new_n108_), .b(x23), .O(new_n110_));
  nand4  g65(.a(new_n107_), .b(new_n95_), .c(new_n59_), .d(new_n58_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n92_), .O(z16));
  inv1   g67(.a(new_n52_), .O(new_n113_));
  nand2  g68(.a(new_n111_), .b(x24), .O(new_n114_));
  nand4  g69(.a(new_n107_), .b(new_n99_), .c(new_n67_), .d(new_n59_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(x17), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n86_), .O(z17));
  aoi21  g73(.a(new_n115_), .b(x25), .c(new_n91_), .O(new_n119_));
  oai21  g74(.a(new_n115_), .b(x25), .c(new_n119_), .O(z18));
endmodule


