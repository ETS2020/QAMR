// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(x07), .c(new_n49_), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(x24), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n56_), .c(x23), .O(new_n62_));
  nor2   g17(.a(x24), .b(x20), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(new_n62_), .c(new_n54_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n53_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  inv1   g22(.a(x23), .O(new_n68_));
  inv1   g23(.a(new_n55_), .O(new_n69_));
  nand2  g24(.a(new_n61_), .b(new_n69_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  aoi22  g26(.a(new_n71_), .b(x24), .c(new_n49_), .d(x25), .O(z03));
  nand2  g27(.a(new_n49_), .b(new_n46_), .O(new_n73_));
  xor2a  g28(.a(x12), .b(x03), .O(new_n74_));
  xor2a  g29(.a(x11), .b(x02), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  xor2a  g31(.a(x09), .b(x00), .O(new_n77_));
  xor2a  g32(.a(x10), .b(x01), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(z04));
  oai21  g35(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  nor2   g37(.a(new_n73_), .b(new_n82_), .O(z06));
  aoi21  g38(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nor2   g39(.a(new_n60_), .b(new_n55_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x23), .c(x24), .O(new_n86_));
  aoi21  g41(.a(new_n56_), .b(x20), .c(x25), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(z08));
  nor2   g43(.a(x15), .b(x07), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n52_), .c(new_n49_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand2  g46(.a(new_n89_), .b(new_n51_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n57_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nand2  g50(.a(new_n58_), .b(new_n57_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n93_), .c(new_n96_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  aoi21  g54(.a(x18), .b(x17), .c(x19), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n89_), .c(new_n51_), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(new_n100_), .c(new_n49_), .O(z12));
  nand3  g58(.a(new_n101_), .b(x24), .c(x20), .O(new_n104_));
  nand2  g59(.a(new_n92_), .b(new_n49_), .O(new_n105_));
  inv1   g60(.a(new_n101_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(z13));
  nand2  g63(.a(new_n107_), .b(x21), .O(new_n109_));
  inv1   g64(.a(x21), .O(new_n110_));
  nand3  g65(.a(new_n106_), .b(new_n110_), .c(new_n47_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n93_), .O(z14));
  inv1   g67(.a(x22), .O(new_n113_));
  aoi21  g68(.a(new_n106_), .b(new_n110_), .c(new_n113_), .O(new_n114_));
  nand2  g69(.a(new_n113_), .b(new_n110_), .O(new_n115_));
  nor2   g70(.a(new_n115_), .b(new_n101_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n114_), .c(new_n47_), .O(new_n117_));
  nand3  g72(.a(x24), .b(x22), .c(x20), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n105_), .O(z15));
  nand4  g74(.a(new_n106_), .b(new_n68_), .c(new_n113_), .d(new_n110_), .O(new_n120_));
  oai21  g75(.a(new_n116_), .b(new_n68_), .c(new_n120_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand3  g77(.a(x24), .b(x23), .c(x20), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n105_), .O(z16));
  oai21  g79(.a(new_n120_), .b(x20), .c(x24), .O(new_n125_));
  inv1   g80(.a(new_n120_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n63_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n105_), .O(z17));
  oai21  g83(.a(new_n120_), .b(x24), .c(x25), .O(new_n129_));
  inv1   g84(.a(x24), .O(new_n130_));
  nand3  g85(.a(new_n126_), .b(new_n54_), .c(new_n130_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n129_), .c(new_n93_), .O(z18));
endmodule


