// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n85_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n47_), .c(new_n60_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(new_n47_), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n46_), .O(z02));
  inv1   g21(.a(x23), .O(new_n67_));
  nor2   g22(.a(x18), .b(x17), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n54_), .c(x20), .O(new_n69_));
  oai21  g24(.a(new_n56_), .b(new_n69_), .c(new_n67_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(x24), .c(x25), .O(z03));
  nor2   g26(.a(new_n65_), .b(x08), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  and2   g32(.a(new_n77_), .b(new_n72_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nand2  g34(.a(new_n72_), .b(new_n79_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z05));
  inv1   g36(.a(x14), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x08), .c(new_n47_), .O(z06));
  nand2  g38(.a(new_n72_), .b(x06), .O(z07));
  nand2  g39(.a(new_n70_), .b(x24), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(z08));
  nor4   g41(.a(new_n61_), .b(new_n65_), .c(x15), .d(x07), .O(z09));
  inv1   g42(.a(x15), .O(new_n88_));
  nand3  g43(.a(new_n61_), .b(new_n88_), .c(new_n60_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n47_), .c(new_n52_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z10));
  nand2  g47(.a(new_n90_), .b(new_n47_), .O(new_n93_));
  nor2   g48(.a(new_n53_), .b(new_n52_), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n93_), .c(new_n68_), .O(z11));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  nor2   g52(.a(new_n94_), .b(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n93_), .O(z12));
  nand2  g54(.a(new_n97_), .b(new_n51_), .O(new_n100_));
  aoi21  g55(.a(new_n96_), .b(x20), .c(new_n89_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n100_), .c(new_n65_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  nor2   g58(.a(x21), .b(x20), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n97_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n103_), .c(new_n90_), .d(new_n47_), .O(z14));
  nand2  g61(.a(new_n105_), .b(x22), .O(new_n107_));
  inv1   g62(.a(x22), .O(new_n108_));
  nand3  g63(.a(new_n104_), .b(new_n97_), .c(new_n108_), .O(new_n109_));
  and2   g64(.a(new_n109_), .b(new_n90_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n107_), .c(new_n65_), .O(z15));
  inv1   g66(.a(x24), .O(new_n112_));
  nand4  g67(.a(new_n104_), .b(new_n97_), .c(new_n112_), .d(new_n108_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x23), .O(new_n114_));
  nand4  g69(.a(new_n104_), .b(new_n97_), .c(new_n67_), .d(new_n108_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n90_), .O(z16));
  nand2  g71(.a(new_n89_), .b(new_n47_), .O(new_n117_));
  xor2a  g72(.a(new_n109_), .b(x24), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(x23), .c(new_n117_), .O(z17));
  nand2  g74(.a(new_n115_), .b(new_n50_), .O(new_n120_));
  nor2   g75(.a(new_n50_), .b(x23), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  oai21  g78(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  aoi22  g79(.a(new_n124_), .b(new_n89_), .c(new_n121_), .d(new_n113_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n123_), .O(z18));
endmodule


