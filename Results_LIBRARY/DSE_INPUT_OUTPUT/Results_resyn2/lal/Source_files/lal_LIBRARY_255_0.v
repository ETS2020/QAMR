// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n73_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x24), .b(x15), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nor3   g09(.a(x19), .b(x18), .c(x17), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(x20), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x24), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x23), .O(new_n59_));
  inv1   g14(.a(x15), .O(new_n60_));
  nor2   g15(.a(x24), .b(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n59_), .b(new_n57_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x25), .c(new_n54_), .O(z01));
  nor2   g18(.a(new_n62_), .b(x25), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z04));
  nor3   g26(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor3   g28(.a(new_n48_), .b(new_n73_), .c(x08), .O(z06));
  aoi21  g29(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g30(.a(x25), .O(new_n76_));
  inv1   g31(.a(x23), .O(new_n77_));
  nand2  g32(.a(new_n57_), .b(new_n77_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x24), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n49_), .c(new_n76_), .O(z08));
  inv1   g35(.a(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x05), .c(x04), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(x24), .c(x15), .O(z09));
  inv1   g38(.a(x17), .O(new_n84_));
  nand2  g39(.a(new_n52_), .b(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x15), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n52_), .b(x24), .c(new_n60_), .O(new_n88_));
  and2   g43(.a(x18), .b(x17), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nor2   g47(.a(new_n89_), .b(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n88_), .O(z12));
  nand2  g49(.a(new_n91_), .b(x20), .O(new_n95_));
  inv1   g50(.a(x20), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n95_), .c(new_n52_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x24), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n60_), .O(z13));
  nand2  g55(.a(new_n97_), .b(x21), .O(new_n101_));
  nor2   g56(.a(x21), .b(x20), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n92_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n52_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x24), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n60_), .O(z14));
  nor2   g61(.a(new_n91_), .b(x20), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g64(.a(new_n103_), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n52_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x24), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n60_), .O(z15));
  aoi21  g68(.a(new_n108_), .b(new_n107_), .c(new_n77_), .O(new_n114_));
  nand3  g69(.a(new_n108_), .b(new_n107_), .c(new_n77_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n52_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n114_), .c(x24), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n60_), .O(z16));
  oai21  g73(.a(new_n115_), .b(new_n53_), .c(x24), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n60_), .O(z17));
  inv1   g75(.a(new_n88_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n76_), .O(z18));
endmodule


