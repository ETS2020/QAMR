// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n98_, new_n100_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x18), .O(new_n48_));
  nor2   g03(.a(x20), .b(new_n48_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x24), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  oai21  g10(.a(x19), .b(x17), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  aoi21  g13(.a(new_n58_), .b(new_n56_), .c(x23), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n53_), .c(new_n54_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n49_), .c(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  nor2   g19(.a(new_n60_), .b(new_n49_), .O(z03));
  nand2  g20(.a(new_n50_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  nor2   g28(.a(new_n66_), .b(x13), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n50_), .O(z06));
  nand3  g31(.a(new_n50_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g32(.a(x23), .O(new_n78_));
  nand2  g33(.a(new_n58_), .b(new_n56_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x24), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n54_), .c(new_n49_), .O(z08));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n62_), .c(new_n50_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z09));
  nand2  g40(.a(new_n83_), .b(new_n61_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n50_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n86_), .c(new_n50_), .O(z11));
  and2   g44(.a(x18), .b(x17), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x19), .O(new_n91_));
  inv1   g46(.a(new_n86_), .O(new_n92_));
  nand2  g47(.a(new_n90_), .b(x19), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n91_), .c(new_n50_), .O(z12));
  nand2  g50(.a(new_n93_), .b(x20), .O(new_n96_));
  oai21  g51(.a(new_n92_), .b(new_n49_), .c(new_n96_), .O(z13));
  inv1   g52(.a(x21), .O(new_n98_));
  aoi21  g53(.a(new_n92_), .b(new_n98_), .c(new_n49_), .O(z14));
  inv1   g54(.a(x22), .O(new_n100_));
  aoi21  g55(.a(new_n92_), .b(new_n100_), .c(new_n49_), .O(z15));
  aoi21  g56(.a(new_n92_), .b(new_n78_), .c(new_n49_), .O(z16));
  nand3  g57(.a(new_n92_), .b(new_n50_), .c(new_n53_), .O(z17));
  aoi21  g58(.a(new_n92_), .b(new_n54_), .c(new_n49_), .O(z18));
endmodule


