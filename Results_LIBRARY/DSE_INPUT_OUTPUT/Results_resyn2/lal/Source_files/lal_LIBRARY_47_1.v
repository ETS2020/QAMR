// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g12(.a(new_n57_), .b(new_n52_), .c(new_n53_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  inv1   g15(.a(x07), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n49_), .c(new_n61_), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n48_), .b(new_n66_), .O(z02));
  nand3  g22(.a(new_n59_), .b(new_n49_), .c(new_n51_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z03));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor3   g33(.a(new_n48_), .b(new_n78_), .c(x08), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  nand2  g36(.a(new_n56_), .b(new_n54_), .O(new_n82_));
  nor3   g37(.a(new_n82_), .b(new_n53_), .c(new_n81_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x23), .c(new_n55_), .O(new_n84_));
  oai21  g39(.a(new_n53_), .b(new_n52_), .c(new_n47_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x25), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n84_), .O(z08));
  inv1   g42(.a(x15), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n61_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n62_), .c(new_n49_), .O(z09));
  nand3  g45(.a(new_n62_), .b(new_n88_), .c(new_n61_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x17), .c(new_n49_), .O(z10));
  inv1   g47(.a(new_n91_), .O(new_n93_));
  nand2  g48(.a(new_n55_), .b(new_n54_), .O(new_n94_));
  nor2   g49(.a(new_n55_), .b(new_n54_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n49_), .O(z11));
  aoi21  g53(.a(new_n95_), .b(x19), .c(x15), .O(new_n99_));
  oai21  g54(.a(new_n95_), .b(x19), .c(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n63_), .O(z12));
  nand4  g56(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n102_));
  nand2  g57(.a(x19), .b(x17), .O(new_n103_));
  oai22  g58(.a(new_n103_), .b(new_n55_), .c(new_n48_), .d(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n93_), .O(z13));
  or2    g60(.a(new_n102_), .b(x21), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(x21), .c(new_n91_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n48_), .O(z14));
  inv1   g63(.a(x22), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(x18), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n91_), .c(new_n49_), .O(new_n111_));
  inv1   g66(.a(x21), .O(new_n112_));
  nor2   g67(.a(new_n103_), .b(x20), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n112_), .c(x22), .O(new_n114_));
  nand3  g69(.a(new_n113_), .b(x22), .c(new_n112_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x18), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n114_), .c(new_n111_), .O(z15));
  nor2   g72(.a(x22), .b(x21), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n102_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n47_), .c(new_n91_), .O(new_n121_));
  oai21  g76(.a(new_n120_), .b(new_n47_), .c(new_n121_), .O(z16));
  nand3  g77(.a(new_n120_), .b(new_n81_), .c(new_n47_), .O(new_n123_));
  nand2  g78(.a(new_n113_), .b(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n118_), .b(x18), .O(new_n125_));
  oai22  g80(.a(new_n125_), .b(new_n124_), .c(new_n48_), .d(x24), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n123_), .c(new_n93_), .O(z17));
  nand3  g82(.a(new_n118_), .b(new_n51_), .c(new_n81_), .O(new_n128_));
  oai22  g83(.a(new_n128_), .b(new_n102_), .c(new_n51_), .d(x18), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  aoi21  g85(.a(new_n51_), .b(new_n47_), .c(x18), .O(new_n131_));
  nand4  g86(.a(new_n118_), .b(new_n113_), .c(new_n81_), .d(new_n47_), .O(new_n132_));
  aoi21  g87(.a(new_n132_), .b(x25), .c(new_n91_), .O(new_n133_));
  oai21  g88(.a(new_n133_), .b(new_n131_), .c(new_n130_), .O(z18));
endmodule


