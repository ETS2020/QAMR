// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_;
  nand2  g00(.a(x24), .b(x23), .O(new_n46_));
  and2   g01(.a(new_n46_), .b(x16), .O(z02));
  inv1   g02(.a(new_n46_), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(z02), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n57_), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n46_), .c(new_n62_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n61_), .O(z01));
  inv1   g21(.a(x23), .O(new_n67_));
  nor2   g22(.a(x18), .b(x17), .O(new_n68_));
  aoi21  g23(.a(new_n68_), .b(new_n55_), .c(x20), .O(new_n69_));
  oai21  g24(.a(new_n58_), .b(new_n69_), .c(new_n67_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(x24), .c(x25), .O(z03));
  inv1   g26(.a(x08), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n46_), .O(z04));
  inv1   g34(.a(x13), .O(new_n80_));
  nand2  g35(.a(new_n49_), .b(new_n80_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(new_n49_), .b(x14), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z06));
  aoi21  g39(.a(new_n72_), .b(x06), .c(new_n48_), .O(z07));
  aoi21  g40(.a(new_n60_), .b(new_n51_), .c(new_n48_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n63_), .c(new_n46_), .O(z09));
  nand3  g44(.a(new_n63_), .b(new_n87_), .c(new_n62_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n46_), .O(z10));
  nor2   g46(.a(new_n54_), .b(new_n53_), .O(new_n92_));
  nor4   g47(.a(new_n92_), .b(new_n64_), .c(new_n68_), .d(x15), .O(z11));
  inv1   g48(.a(new_n90_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  oai21  g50(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n46_), .O(z12));
  nor2   g53(.a(new_n95_), .b(x20), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  aoi21  g55(.a(new_n95_), .b(x20), .c(new_n90_), .O(new_n101_));
  aoi21  g56(.a(new_n101_), .b(new_n100_), .c(new_n48_), .O(z13));
  nand2  g57(.a(new_n100_), .b(x21), .O(new_n103_));
  inv1   g58(.a(new_n95_), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n90_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n103_), .c(new_n48_), .O(z14));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n99_), .O(new_n109_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x22), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n109_), .c(new_n65_), .d(new_n87_), .O(z15));
  nor2   g67(.a(x24), .b(x22), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n105_), .c(new_n104_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x23), .O(new_n115_));
  nor3   g70(.a(x23), .b(x22), .c(x21), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n99_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n94_), .O(z16));
  inv1   g73(.a(x24), .O(new_n119_));
  nand3  g74(.a(new_n116_), .b(new_n99_), .c(new_n119_), .O(new_n120_));
  nand2  g75(.a(new_n117_), .b(x24), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n94_), .O(z17));
  nand4  g77(.a(new_n116_), .b(new_n99_), .c(new_n51_), .d(new_n119_), .O(new_n123_));
  nand2  g78(.a(new_n114_), .b(x25), .O(new_n124_));
  oai21  g79(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n124_), .c(new_n123_), .d(new_n94_), .O(z18));
endmodule


