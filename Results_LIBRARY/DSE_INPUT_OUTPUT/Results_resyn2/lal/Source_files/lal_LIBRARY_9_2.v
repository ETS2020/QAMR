// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  inv1   g12(.a(x24), .O(new_n58_));
  inv1   g13(.a(x25), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(new_n58_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n60_), .c(new_n57_), .d(new_n51_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n49_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n57_), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z04));
  nor3   g27(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  and2   g31(.a(new_n60_), .b(new_n57_), .O(z08));
  nor2   g32(.a(x15), .b(x07), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(x05), .c(x04), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n49_), .O(z09));
  inv1   g35(.a(x17), .O(new_n81_));
  nand2  g36(.a(new_n78_), .b(new_n61_), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z10));
  inv1   g40(.a(new_n53_), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n83_), .c(new_n86_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand2  g45(.a(new_n87_), .b(new_n52_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n82_), .c(new_n48_), .O(z12));
  inv1   g48(.a(x20), .O(new_n94_));
  inv1   g49(.a(new_n90_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g51(.a(new_n90_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n83_), .O(z13));
  nand2  g53(.a(new_n96_), .b(x21), .O(new_n99_));
  inv1   g54(.a(x21), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n94_), .O(new_n101_));
  nor2   g56(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n82_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n99_), .c(new_n48_), .O(z14));
  oai21  g59(.a(new_n101_), .b(new_n90_), .c(x22), .O(new_n105_));
  inv1   g60(.a(x22), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(new_n106_), .c(new_n82_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n105_), .c(new_n48_), .O(z15));
  nand3  g63(.a(new_n102_), .b(new_n47_), .c(new_n106_), .O(new_n109_));
  nand2  g64(.a(new_n102_), .b(new_n106_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x23), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n83_), .O(z16));
  nand2  g67(.a(new_n110_), .b(x24), .O(new_n113_));
  nor2   g68(.a(x21), .b(x20), .O(new_n114_));
  nand4  g69(.a(new_n114_), .b(new_n95_), .c(new_n58_), .d(new_n106_), .O(new_n115_));
  nand3  g70(.a(new_n78_), .b(new_n61_), .c(new_n47_), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(z17));
  xor2a  g73(.a(new_n115_), .b(x25), .O(new_n119_));
  aoi21  g74(.a(new_n59_), .b(x24), .c(new_n47_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n119_), .O(z18));
endmodule


