// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n79_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x20), .b(x15), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  nand2  g18(.a(new_n47_), .b(new_n46_), .O(z02));
  nand3  g19(.a(new_n58_), .b(new_n47_), .c(new_n50_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  inv1   g27(.a(x08), .O(new_n73_));
  nand2  g28(.a(new_n47_), .b(new_n73_), .O(new_n74_));
  aoi21  g29(.a(new_n72_), .b(new_n69_), .c(new_n74_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g33(.a(new_n47_), .O(new_n79_));
  aoi21  g34(.a(new_n73_), .b(x06), .c(new_n79_), .O(z07));
  aoi21  g35(.a(new_n58_), .b(new_n50_), .c(new_n79_), .O(z08));
  inv1   g36(.a(x07), .O(new_n82_));
  inv1   g37(.a(x15), .O(new_n83_));
  nand4  g38(.a(new_n83_), .b(new_n82_), .c(x05), .d(x04), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z09));
  nand2  g40(.a(new_n60_), .b(new_n83_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n52_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z10));
  nor2   g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n86_), .c(new_n90_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n83_), .O(new_n95_));
  oai21  g50(.a(new_n92_), .b(x19), .c(new_n60_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n95_), .c(new_n47_), .O(z12));
  nand2  g52(.a(new_n95_), .b(new_n51_), .O(new_n98_));
  inv1   g53(.a(new_n60_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nand3  g55(.a(new_n94_), .b(x20), .c(new_n83_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(z13));
  oai21  g57(.a(new_n94_), .b(x20), .c(x21), .O(new_n103_));
  inv1   g58(.a(x21), .O(new_n104_));
  nor2   g59(.a(new_n94_), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n103_), .c(new_n87_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n109_), .c(new_n104_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n87_), .O(z15));
  inv1   g66(.a(new_n94_), .O(new_n112_));
  nor3   g67(.a(x23), .b(x22), .c(x21), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n112_), .c(x15), .O(new_n114_));
  aoi21  g69(.a(new_n110_), .b(x23), .c(new_n99_), .O(new_n115_));
  oai22  g70(.a(new_n115_), .b(x15), .c(new_n114_), .d(x20), .O(z16));
  oai21  g71(.a(new_n110_), .b(x23), .c(x24), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand3  g73(.a(new_n113_), .b(new_n105_), .c(new_n118_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n87_), .O(z17));
  inv1   g75(.a(x23), .O(new_n121_));
  nand4  g76(.a(new_n118_), .b(new_n121_), .c(new_n109_), .d(new_n104_), .O(new_n122_));
  nand2  g77(.a(new_n112_), .b(new_n50_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n122_), .c(new_n83_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n51_), .O(new_n125_));
  aoi21  g80(.a(new_n119_), .b(x25), .c(new_n99_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(x15), .c(new_n125_), .O(z18));
endmodule


