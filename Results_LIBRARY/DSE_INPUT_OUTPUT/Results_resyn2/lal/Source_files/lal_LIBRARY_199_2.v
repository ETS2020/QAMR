// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n76_, new_n79_, new_n80_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x13), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(x13), .c(new_n48_), .O(new_n53_));
  nor3   g08(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  inv1   g09(.a(x21), .O(new_n55_));
  inv1   g10(.a(x22), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  oai21  g12(.a(new_n54_), .b(x20), .c(new_n57_), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g18(.a(new_n63_), .b(new_n60_), .c(new_n53_), .O(z01));
  nand2  g19(.a(new_n60_), .b(new_n53_), .O(z03));
  xnor2a g20(.a(x12), .b(x03), .O(new_n66_));
  xnor2a g21(.a(x11), .b(x02), .O(new_n67_));
  xnor2a g22(.a(x09), .b(x00), .O(new_n68_));
  xnor2a g23(.a(x10), .b(x01), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n49_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g28(.a(x14), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n49_), .O(z06));
  inv1   g30(.a(new_n49_), .O(new_n76_));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n76_), .O(z07));
  and2   g32(.a(new_n60_), .b(new_n53_), .O(z08));
  nor2   g33(.a(x15), .b(x07), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n62_), .c(new_n49_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z09));
  nand2  g36(.a(new_n79_), .b(new_n61_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x17), .c(new_n49_), .O(z10));
  or2    g38(.a(x18), .b(x17), .O(new_n84_));
  nand3  g39(.a(new_n79_), .b(new_n61_), .c(new_n49_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand2  g41(.a(x18), .b(x17), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n84_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  inv1   g45(.a(x19), .O(new_n91_));
  aoi21  g46(.a(new_n87_), .b(new_n91_), .c(new_n82_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n49_), .O(z12));
  nor2   g49(.a(new_n90_), .b(x20), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  aoi21  g51(.a(new_n90_), .b(x20), .c(new_n82_), .O(new_n97_));
  aoi21  g52(.a(new_n97_), .b(new_n96_), .c(new_n76_), .O(z13));
  nand2  g53(.a(new_n96_), .b(x21), .O(new_n99_));
  aoi21  g54(.a(new_n95_), .b(new_n55_), .c(new_n82_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n76_), .O(z14));
  oai21  g56(.a(new_n96_), .b(x21), .c(x22), .O(new_n102_));
  nor2   g57(.a(x22), .b(x21), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n95_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n86_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  inv1   g61(.a(x20), .O(new_n107_));
  inv1   g62(.a(x23), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n56_), .c(new_n55_), .d(new_n107_), .O(new_n109_));
  nor2   g64(.a(new_n109_), .b(new_n90_), .O(new_n110_));
  nor2   g65(.a(new_n110_), .b(new_n82_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n106_), .c(new_n76_), .O(z16));
  nand2  g67(.a(new_n110_), .b(new_n48_), .O(new_n113_));
  oai21  g68(.a(new_n109_), .b(new_n90_), .c(x24), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n86_), .O(z17));
  inv1   g70(.a(x13), .O(new_n116_));
  oai21  g71(.a(new_n109_), .b(new_n90_), .c(x25), .O(new_n117_));
  nand4  g72(.a(new_n103_), .b(new_n95_), .c(new_n59_), .d(new_n48_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n116_), .O(new_n120_));
  aoi22  g75(.a(new_n82_), .b(new_n49_), .c(x25), .d(x24), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n120_), .O(z18));
endmodule


