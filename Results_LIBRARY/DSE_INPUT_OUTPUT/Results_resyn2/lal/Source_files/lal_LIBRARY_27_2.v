// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x25), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x19), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x20), .O(new_n50_));
  oai21  g05(.a(x18), .b(x17), .c(new_n50_), .O(new_n51_));
  inv1   g06(.a(x21), .O(new_n52_));
  inv1   g07(.a(x22), .O(new_n53_));
  nor2   g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n51_), .c(x23), .O(new_n55_));
  inv1   g10(.a(x24), .O(new_n56_));
  nor2   g11(.a(new_n56_), .b(x19), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(new_n55_), .c(new_n47_), .O(z08));
  aoi21  g14(.a(x05), .b(x04), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(z08), .O(z01));
  and2   g16(.a(new_n48_), .b(x16), .O(z02));
  inv1   g17(.a(z08), .O(z03));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x11), .b(x02), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n46_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n48_), .O(z04));
  nand2  g25(.a(new_n48_), .b(new_n46_), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(x13), .O(z05));
  inv1   g27(.a(x14), .O(new_n73_));
  nor2   g28(.a(new_n71_), .b(new_n73_), .O(z06));
  aoi22  g29(.a(new_n47_), .b(x19), .c(new_n46_), .d(x06), .O(z07));
  nand2  g30(.a(x05), .b(x04), .O(new_n76_));
  inv1   g31(.a(x07), .O(new_n77_));
  inv1   g32(.a(x15), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n76_), .c(new_n48_), .O(z09));
  nand3  g35(.a(new_n76_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x17), .c(new_n48_), .O(z10));
  nand2  g37(.a(x18), .b(x17), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  oai21  g39(.a(x18), .b(x17), .c(new_n48_), .O(new_n85_));
  nor3   g40(.a(new_n85_), .b(new_n84_), .c(new_n81_), .O(z11));
  inv1   g41(.a(new_n81_), .O(new_n87_));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  inv1   g43(.a(x19), .O(new_n89_));
  nand2  g44(.a(new_n83_), .b(new_n89_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n48_), .O(z12));
  oai21  g47(.a(new_n84_), .b(new_n47_), .c(x19), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x20), .O(new_n94_));
  nand2  g49(.a(new_n81_), .b(new_n48_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nor3   g51(.a(new_n88_), .b(new_n47_), .c(x20), .O(new_n97_));
  nor2   g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n94_), .O(z13));
  nand3  g54(.a(new_n50_), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x25), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(x19), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x21), .O(new_n103_));
  inv1   g58(.a(new_n88_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(x25), .c(new_n52_), .d(new_n50_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n95_), .O(z14));
  nand2  g61(.a(new_n53_), .b(new_n52_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n100_), .c(x25), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x19), .O(new_n109_));
  nand2  g64(.a(new_n105_), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n87_), .O(z15));
  oai21  g66(.a(new_n105_), .b(x22), .c(x23), .O(new_n112_));
  inv1   g67(.a(x23), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n53_), .c(new_n52_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n100_), .c(x25), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(x19), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n112_), .c(new_n87_), .O(z16));
  nand2  g72(.a(new_n116_), .b(x24), .O(new_n118_));
  nand4  g73(.a(new_n56_), .b(new_n113_), .c(new_n53_), .d(new_n52_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n97_), .c(new_n96_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n118_), .O(z17));
  nand2  g77(.a(new_n104_), .b(new_n50_), .O(new_n123_));
  oai21  g78(.a(new_n119_), .b(new_n123_), .c(x25), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n95_), .O(z18));
endmodule


