// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n136_, new_n138_, new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x19), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n47_), .O(z01));
  inv1   g14(.a(new_n47_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(new_n46_), .O(z02));
  nand2  g16(.a(new_n57_), .b(new_n47_), .O(z03));
  inv1   g17(.a(x08), .O(new_n63_));
  inv1   g18(.a(x12), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(x03), .O(new_n65_));
  inv1   g20(.a(x02), .O(new_n66_));
  nand2  g21(.a(x11), .b(new_n66_), .O(new_n67_));
  inv1   g22(.a(x03), .O(new_n68_));
  nand2  g23(.a(x12), .b(new_n68_), .O(new_n69_));
  inv1   g24(.a(x11), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x02), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(new_n72_));
  inv1   g27(.a(x00), .O(new_n73_));
  nand2  g28(.a(x09), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x01), .O(new_n75_));
  nand2  g30(.a(x10), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x09), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x00), .O(new_n78_));
  inv1   g33(.a(x10), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x01), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n72_), .c(new_n63_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n47_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g41(.a(new_n47_), .b(new_n63_), .c(x06), .O(z07));
  inv1   g42(.a(x17), .O(new_n88_));
  inv1   g43(.a(x18), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n52_), .c(new_n54_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(x23), .c(x24), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n51_), .c(new_n60_), .O(z08));
  nand2  g48(.a(x05), .b(x04), .O(new_n94_));
  inv1   g49(.a(x07), .O(new_n95_));
  inv1   g50(.a(x15), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n94_), .c(new_n47_), .O(z09));
  nand3  g53(.a(new_n94_), .b(new_n96_), .c(new_n95_), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g55(.a(x18), .b(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  oai21  g57(.a(new_n102_), .b(new_n99_), .c(new_n47_), .O(z11));
  inv1   g58(.a(new_n99_), .O(new_n104_));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  inv1   g60(.a(x19), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n47_), .O(z12));
  nand2  g64(.a(new_n99_), .b(new_n47_), .O(new_n110_));
  aoi21  g65(.a(x18), .b(x17), .c(x24), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n106_), .c(x20), .O(new_n112_));
  nand4  g67(.a(new_n52_), .b(x19), .c(x18), .d(x17), .O(new_n113_));
  inv1   g68(.a(new_n113_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n50_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n112_), .c(new_n110_), .O(z13));
  nand3  g71(.a(new_n52_), .b(x18), .c(x17), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n50_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(x19), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x21), .O(new_n120_));
  inv1   g75(.a(x21), .O(new_n121_));
  nand3  g76(.a(new_n114_), .b(new_n50_), .c(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n110_), .O(z14));
  nand2  g78(.a(new_n122_), .b(x22), .O(new_n124_));
  inv1   g79(.a(x22), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n117_), .c(new_n50_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x19), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n124_), .c(new_n104_), .O(z15));
  nand2  g84(.a(new_n128_), .b(x23), .O(new_n130_));
  inv1   g85(.a(x23), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n125_), .c(new_n121_), .O(new_n132_));
  nor2   g87(.a(new_n132_), .b(new_n113_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n50_), .O(new_n134_));
  nand3  g89(.a(new_n134_), .b(new_n130_), .c(new_n110_), .O(z16));
  nand2  g90(.a(new_n104_), .b(new_n50_), .O(new_n136_));
  or2    g91(.a(new_n136_), .b(new_n133_), .O(z17));
  nand4  g92(.a(new_n51_), .b(new_n52_), .c(x18), .d(x17), .O(new_n138_));
  oai21  g93(.a(new_n138_), .b(new_n132_), .c(new_n50_), .O(new_n139_));
  nand2  g94(.a(new_n139_), .b(x19), .O(new_n140_));
  nand2  g95(.a(new_n134_), .b(x25), .O(new_n141_));
  nand3  g96(.a(new_n141_), .b(new_n140_), .c(new_n104_), .O(z18));
endmodule


