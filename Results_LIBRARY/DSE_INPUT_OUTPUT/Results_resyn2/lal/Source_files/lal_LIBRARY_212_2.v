// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g12(.a(new_n57_), .b(x24), .c(x22), .d(x21), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n51_), .c(new_n47_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x07), .c(new_n49_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n59_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n49_), .b(new_n64_), .O(z02));
  inv1   g20(.a(new_n59_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  nor3   g28(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z06));
  nand3  g31(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g32(.a(x22), .b(x21), .O(new_n78_));
  aoi21  g33(.a(new_n56_), .b(new_n52_), .c(new_n78_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x23), .c(x24), .O(new_n80_));
  nand2  g35(.a(x25), .b(new_n47_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(z08));
  nor2   g37(.a(x15), .b(x07), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n61_), .c(new_n49_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(z09));
  nand2  g40(.a(new_n83_), .b(new_n60_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g42(.a(new_n54_), .b(new_n53_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(new_n86_), .c(new_n49_), .O(z11));
  nand3  g46(.a(new_n83_), .b(new_n60_), .c(new_n49_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n89_), .b(new_n55_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n92_), .O(z12));
  inv1   g51(.a(new_n93_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n52_), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(x20), .c(new_n86_), .O(new_n99_));
  aoi21  g54(.a(new_n99_), .b(new_n98_), .c(new_n48_), .O(z13));
  inv1   g55(.a(new_n92_), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n97_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(z14));
  nand2  g60(.a(new_n104_), .b(x22), .O(new_n106_));
  nor3   g61(.a(x22), .b(x21), .c(x20), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n97_), .c(new_n86_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n106_), .c(new_n48_), .O(z15));
  nand2  g64(.a(new_n86_), .b(new_n49_), .O(new_n110_));
  nand2  g65(.a(new_n107_), .b(new_n97_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(x24), .c(x23), .O(new_n112_));
  nand3  g67(.a(new_n107_), .b(new_n97_), .c(new_n47_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(z16));
  inv1   g69(.a(new_n86_), .O(new_n115_));
  nand2  g70(.a(new_n111_), .b(x24), .O(new_n116_));
  inv1   g71(.a(x22), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand4  g73(.a(new_n103_), .b(new_n97_), .c(new_n118_), .d(new_n117_), .O(new_n119_));
  nand4  g74(.a(new_n119_), .b(new_n116_), .c(new_n115_), .d(new_n47_), .O(z17));
  nand2  g75(.a(new_n119_), .b(new_n51_), .O(new_n121_));
  nand3  g76(.a(new_n107_), .b(new_n97_), .c(x25), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n47_), .O(new_n123_));
  aoi22  g78(.a(new_n86_), .b(new_n49_), .c(x25), .d(x24), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n123_), .O(z18));
endmodule


