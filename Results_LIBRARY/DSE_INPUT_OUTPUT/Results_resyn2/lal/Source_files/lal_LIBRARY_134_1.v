// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n83_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x19), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x23), .O(new_n55_));
  nand3  g10(.a(x22), .b(x21), .c(x20), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n50_), .d(new_n53_), .O(z01));
  nand2  g16(.a(new_n59_), .b(new_n50_), .O(z03));
  nand2  g17(.a(new_n50_), .b(new_n46_), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z04));
  nor2   g25(.a(new_n63_), .b(x13), .O(z05));
  nand2  g26(.a(x14), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z06));
  aoi21  g28(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g29(.a(x19), .O(new_n75_));
  oai21  g30(.a(new_n57_), .b(new_n75_), .c(x24), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n54_), .O(z08));
  inv1   g32(.a(x15), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n60_), .c(new_n50_), .O(z09));
  nand3  g35(.a(new_n60_), .b(new_n78_), .c(new_n53_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(x17), .c(new_n50_), .O(z10));
  xnor2a g37(.a(x18), .b(x17), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(new_n50_), .O(z11));
  nand2  g39(.a(x18), .b(x17), .O(new_n85_));
  xor2a  g40(.a(new_n85_), .b(x19), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n81_), .c(new_n49_), .O(z12));
  nand2  g42(.a(new_n81_), .b(new_n50_), .O(new_n88_));
  nand2  g43(.a(new_n85_), .b(x20), .O(new_n89_));
  inv1   g44(.a(x20), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(x19), .O(new_n93_));
  nor2   g48(.a(new_n90_), .b(x19), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n50_), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n93_), .c(new_n88_), .O(z13));
  inv1   g51(.a(x21), .O(new_n97_));
  nand4  g52(.a(new_n97_), .b(new_n90_), .c(x18), .d(x17), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(x19), .O(new_n99_));
  nand3  g54(.a(new_n48_), .b(new_n97_), .c(new_n75_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g56(.a(new_n91_), .b(x21), .c(new_n81_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(z14));
  inv1   g58(.a(new_n81_), .O(new_n104_));
  nor2   g59(.a(new_n98_), .b(x22), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x19), .O(new_n106_));
  nand2  g61(.a(new_n98_), .b(x22), .O(new_n107_));
  oai21  g62(.a(x24), .b(x22), .c(new_n75_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n104_), .O(z15));
  nand3  g64(.a(new_n105_), .b(new_n55_), .c(x19), .O(new_n110_));
  oai21  g65(.a(new_n98_), .b(x22), .c(x23), .O(new_n111_));
  nor2   g66(.a(x24), .b(x23), .O(new_n112_));
  nor2   g67(.a(new_n112_), .b(x19), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n81_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(z16));
  nor2   g70(.a(x21), .b(x20), .O(new_n116_));
  nor2   g71(.a(x23), .b(x22), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(x18), .d(x17), .O(new_n118_));
  xor2a  g73(.a(new_n118_), .b(x24), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n75_), .c(new_n88_), .O(z17));
  oai21  g75(.a(new_n118_), .b(x24), .c(x25), .O(new_n121_));
  nand4  g76(.a(new_n112_), .b(new_n105_), .c(new_n54_), .d(x19), .O(new_n122_));
  nand2  g77(.a(new_n54_), .b(new_n48_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n75_), .c(new_n81_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n121_), .O(z18));
endmodule


