// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g18(.a(x23), .O(new_n64_));
  nor2   g19(.a(x18), .b(x17), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n54_), .c(x20), .O(new_n66_));
  oai21  g21(.a(new_n57_), .b(new_n66_), .c(new_n64_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  aoi21  g23(.a(x24), .b(x23), .c(x08), .O(new_n69_));
  xnor2a g24(.a(x12), .b(x03), .O(new_n70_));
  xnor2a g25(.a(x11), .b(x02), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  xnor2a g27(.a(x10), .b(x01), .O(new_n73_));
  nand4  g28(.a(new_n73_), .b(new_n72_), .c(new_n71_), .d(new_n70_), .O(new_n74_));
  and2   g29(.a(new_n74_), .b(new_n69_), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand2  g31(.a(new_n69_), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand2  g33(.a(new_n69_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand2  g35(.a(new_n69_), .b(x06), .O(z07));
  aoi21  g36(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x23), .c(x24), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n47_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n85_), .O(z09));
  nand2  g44(.a(new_n61_), .b(new_n87_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n47_), .O(z10));
  inv1   g46(.a(new_n88_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  inv1   g48(.a(new_n65_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n93_), .O(z11));
  nand2  g52(.a(new_n95_), .b(new_n54_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n90_), .c(new_n47_), .O(z12));
  inv1   g56(.a(new_n93_), .O(new_n102_));
  nor2   g57(.a(new_n99_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  inv1   g62(.a(new_n99_), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n107_), .c(new_n102_), .O(z14));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n103_), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n102_), .O(z15));
  nand2  g70(.a(new_n90_), .b(new_n47_), .O(new_n116_));
  inv1   g71(.a(x24), .O(new_n117_));
  nand3  g72(.a(new_n113_), .b(new_n117_), .c(x23), .O(new_n118_));
  nand3  g73(.a(new_n112_), .b(new_n103_), .c(new_n64_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n116_), .O(z16));
  inv1   g75(.a(new_n90_), .O(new_n121_));
  nand4  g76(.a(new_n112_), .b(new_n103_), .c(new_n117_), .d(new_n64_), .O(new_n122_));
  nand2  g77(.a(new_n119_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(z17));
  nor2   g79(.a(x25), .b(x24), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n112_), .c(new_n103_), .d(new_n64_), .O(new_n126_));
  inv1   g81(.a(x22), .O(new_n127_));
  nand2  g82(.a(new_n117_), .b(new_n127_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n110_), .c(x25), .O(new_n129_));
  oai21  g84(.a(x25), .b(x24), .c(x23), .O(new_n130_));
  nand4  g85(.a(new_n130_), .b(new_n129_), .c(new_n126_), .d(new_n121_), .O(z18));
endmodule


