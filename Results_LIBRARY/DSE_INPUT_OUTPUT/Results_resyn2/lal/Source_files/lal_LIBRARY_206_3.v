// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x14), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x14), .O(new_n49_));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  inv1   g11(.a(x21), .O(new_n57_));
  inv1   g12(.a(x22), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(new_n56_), .c(x23), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n49_), .c(new_n50_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n47_), .O(new_n65_));
  oai21  g20(.a(new_n61_), .b(x25), .c(new_n65_), .O(z01));
  and2   g21(.a(new_n47_), .b(x16), .O(z02));
  nor2   g22(.a(new_n61_), .b(x25), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n47_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  aoi21  g31(.a(new_n50_), .b(x08), .c(new_n49_), .O(z06));
  nand3  g32(.a(new_n47_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g33(.a(x25), .O(new_n79_));
  inv1   g34(.a(new_n47_), .O(new_n80_));
  inv1   g35(.a(x23), .O(new_n81_));
  nand2  g36(.a(new_n59_), .b(new_n56_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n79_), .c(new_n80_), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  nand3  g41(.a(new_n47_), .b(new_n86_), .c(new_n62_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n63_), .O(z09));
  inv1   g43(.a(new_n87_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n63_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n52_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand3  g48(.a(new_n63_), .b(new_n86_), .c(new_n62_), .O(new_n94_));
  xnor2a g49(.a(x18), .b(x17), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n94_), .c(new_n47_), .O(z11));
  inv1   g51(.a(new_n94_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  oai21  g53(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n97_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n47_), .O(z12));
  nor2   g56(.a(new_n98_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n98_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n91_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  aoi21  g61(.a(new_n102_), .b(new_n57_), .c(new_n94_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n80_), .O(z14));
  oai21  g63(.a(new_n103_), .b(x21), .c(x22), .O(new_n109_));
  nand3  g64(.a(new_n102_), .b(new_n58_), .c(new_n57_), .O(new_n110_));
  inv1   g65(.a(new_n110_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n94_), .O(new_n112_));
  aoi21  g67(.a(new_n112_), .b(new_n109_), .c(new_n80_), .O(z15));
  nand2  g68(.a(new_n110_), .b(x23), .O(new_n114_));
  nand4  g69(.a(new_n102_), .b(new_n81_), .c(new_n58_), .d(new_n57_), .O(new_n115_));
  and2   g70(.a(new_n115_), .b(new_n97_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n114_), .c(new_n80_), .O(z16));
  nand3  g72(.a(new_n115_), .b(x24), .c(new_n49_), .O(new_n118_));
  nand2  g73(.a(new_n94_), .b(new_n47_), .O(new_n119_));
  nand3  g74(.a(new_n111_), .b(new_n50_), .c(new_n81_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(z17));
  nand4  g76(.a(new_n111_), .b(new_n79_), .c(new_n50_), .d(new_n81_), .O(new_n122_));
  nand2  g77(.a(new_n47_), .b(new_n79_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n97_), .O(z18));
endmodule


