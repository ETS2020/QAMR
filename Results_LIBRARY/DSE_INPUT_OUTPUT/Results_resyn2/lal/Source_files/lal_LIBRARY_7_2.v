// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n95_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(x23), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x19), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  nand4  g07(.a(new_n52_), .b(x22), .c(x21), .d(new_n51_), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n50_), .c(x20), .O(new_n54_));
  nor2   g09(.a(x25), .b(x24), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(new_n57_));
  nor3   g12(.a(new_n57_), .b(new_n55_), .c(x07), .O(new_n58_));
  oai21  g13(.a(new_n54_), .b(x23), .c(new_n58_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nor2   g15(.a(new_n48_), .b(new_n60_), .O(z02));
  oai22  g16(.a(new_n54_), .b(x23), .c(x25), .d(x24), .O(z03));
  inv1   g17(.a(new_n48_), .O(new_n63_));
  inv1   g18(.a(x00), .O(new_n64_));
  nand2  g19(.a(x09), .b(new_n64_), .O(new_n65_));
  inv1   g20(.a(x02), .O(new_n66_));
  nand2  g21(.a(x11), .b(new_n66_), .O(new_n67_));
  inv1   g22(.a(x03), .O(new_n68_));
  nand2  g23(.a(x12), .b(new_n68_), .O(new_n69_));
  inv1   g24(.a(x11), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(x02), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n69_), .c(new_n67_), .d(new_n65_), .O(new_n72_));
  inv1   g27(.a(x01), .O(new_n73_));
  nand2  g28(.a(x10), .b(new_n73_), .O(new_n74_));
  inv1   g29(.a(x09), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(x00), .O(new_n76_));
  inv1   g31(.a(x10), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x01), .O(new_n78_));
  inv1   g33(.a(x12), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x03), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n78_), .c(new_n76_), .d(new_n74_), .O(new_n81_));
  oai21  g36(.a(new_n81_), .b(new_n72_), .c(new_n46_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n63_), .O(z04));
  oai21  g38(.a(x13), .b(x08), .c(new_n63_), .O(z05));
  nand2  g39(.a(x14), .b(new_n46_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n48_), .O(z06));
  aoi21  g41(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g42(.a(x23), .O(new_n88_));
  nand2  g43(.a(new_n53_), .b(new_n88_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(x24), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n63_), .c(new_n50_), .O(z08));
  nor2   g46(.a(x15), .b(x07), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n57_), .c(new_n63_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  nand2  g49(.a(new_n92_), .b(new_n56_), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n48_), .c(x17), .O(z10));
  xnor2a g51(.a(x18), .b(x17), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n95_), .c(new_n63_), .O(z11));
  inv1   g53(.a(new_n95_), .O(new_n99_));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(x18), .b(x17), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n51_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n63_), .O(z12));
  nand2  g59(.a(new_n100_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n100_), .O(new_n106_));
  oai21  g61(.a(new_n88_), .b(new_n47_), .c(new_n106_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n99_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  nor2   g64(.a(new_n106_), .b(new_n109_), .O(new_n110_));
  nor2   g65(.a(new_n100_), .b(x21), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n110_), .c(new_n47_), .O(new_n112_));
  nand3  g67(.a(x23), .b(x21), .c(x20), .O(new_n113_));
  nand2  g68(.a(new_n95_), .b(new_n63_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(z14));
  inv1   g70(.a(x22), .O(new_n116_));
  nor3   g71(.a(new_n100_), .b(x21), .c(x20), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g73(.a(new_n111_), .b(new_n47_), .O(new_n119_));
  oai21  g74(.a(new_n48_), .b(x22), .c(new_n119_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n99_), .O(z15));
  nand2  g76(.a(new_n118_), .b(x23), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n117_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n122_), .c(new_n114_), .O(z16));
  nand2  g80(.a(new_n116_), .b(new_n109_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n100_), .c(x24), .O(new_n127_));
  inv1   g82(.a(x24), .O(new_n128_));
  nor2   g83(.a(x22), .b(x21), .O(new_n129_));
  nand4  g84(.a(new_n129_), .b(new_n106_), .c(new_n128_), .d(new_n88_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n47_), .O(new_n132_));
  nand2  g87(.a(x24), .b(x23), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n132_), .c(new_n114_), .O(z17));
  nand3  g89(.a(new_n128_), .b(new_n116_), .c(new_n109_), .O(new_n135_));
  oai21  g90(.a(new_n135_), .b(new_n100_), .c(x25), .O(new_n136_));
  nand3  g91(.a(new_n123_), .b(new_n111_), .c(new_n55_), .O(new_n137_));
  nand2  g92(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  aoi22  g94(.a(new_n95_), .b(new_n63_), .c(x25), .d(x23), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n139_), .O(z18));
endmodule


