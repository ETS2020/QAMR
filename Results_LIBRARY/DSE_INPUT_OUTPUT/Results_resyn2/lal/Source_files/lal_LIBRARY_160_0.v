// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n82_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  aoi21  g05(.a(x25), .b(x08), .c(x24), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nor2   g08(.a(x18), .b(x17), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n53_), .c(x20), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  nor2   g11(.a(x25), .b(x23), .O(new_n57_));
  oai21  g12(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand4  g14(.a(new_n59_), .b(new_n58_), .c(new_n52_), .d(new_n50_), .O(z01));
  inv1   g15(.a(x16), .O(new_n61_));
  nand2  g16(.a(new_n48_), .b(new_n61_), .O(z02));
  nand2  g17(.a(new_n58_), .b(new_n52_), .O(z03));
  inv1   g18(.a(x01), .O(new_n64_));
  nand2  g19(.a(x10), .b(new_n64_), .O(new_n65_));
  inv1   g20(.a(x10), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(x01), .O(new_n67_));
  inv1   g22(.a(x00), .O(new_n68_));
  nand2  g23(.a(x09), .b(new_n68_), .O(new_n69_));
  nand3  g24(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(new_n71_));
  inv1   g26(.a(x12), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x03), .c(x24), .O(new_n73_));
  inv1   g28(.a(x11), .O(new_n74_));
  oai22  g29(.a(new_n74_), .b(x02), .c(x09), .d(new_n68_), .O(new_n75_));
  nand2  g30(.a(new_n72_), .b(x03), .O(new_n76_));
  nand2  g31(.a(new_n74_), .b(x02), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nor3   g33(.a(new_n78_), .b(new_n75_), .c(new_n73_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n71_), .c(x08), .O(z04));
  aoi21  g35(.a(x24), .b(x13), .c(x08), .O(z05));
  nand3  g36(.a(x24), .b(x14), .c(new_n46_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z06));
  oai21  g38(.a(new_n47_), .b(x06), .c(new_n46_), .O(z07));
  inv1   g39(.a(z03), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  inv1   g41(.a(new_n59_), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n48_), .c(new_n86_), .d(new_n50_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand3  g44(.a(new_n59_), .b(new_n86_), .c(new_n50_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n48_), .O(z10));
  and2   g46(.a(x18), .b(x17), .O(new_n92_));
  or2    g47(.a(new_n92_), .b(new_n54_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n90_), .c(new_n48_), .O(z11));
  inv1   g49(.a(new_n90_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n48_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nor2   g53(.a(new_n92_), .b(x19), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n96_), .O(z12));
  inv1   g55(.a(new_n96_), .O(new_n101_));
  nor2   g56(.a(new_n97_), .b(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n97_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n101_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  inv1   g61(.a(x21), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n101_), .O(z14));
  nand2  g64(.a(new_n108_), .b(x22), .O(new_n110_));
  nor2   g65(.a(x22), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n102_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n101_), .O(z15));
  nand2  g68(.a(new_n112_), .b(x23), .O(new_n114_));
  inv1   g69(.a(x20), .O(new_n115_));
  inv1   g70(.a(x22), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  nand4  g72(.a(new_n117_), .b(new_n116_), .c(new_n107_), .d(new_n115_), .O(new_n118_));
  inv1   g73(.a(new_n118_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n98_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n114_), .c(new_n101_), .O(z16));
  nand3  g76(.a(new_n119_), .b(new_n98_), .c(new_n47_), .O(new_n122_));
  nand2  g77(.a(new_n120_), .b(x24), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n101_), .O(z17));
  oai21  g79(.a(new_n118_), .b(new_n97_), .c(x25), .O(new_n125_));
  nand4  g80(.a(new_n111_), .b(new_n102_), .c(new_n57_), .d(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x08), .O(new_n128_));
  aoi22  g83(.a(new_n90_), .b(new_n48_), .c(x25), .d(x24), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n128_), .O(z18));
endmodule


