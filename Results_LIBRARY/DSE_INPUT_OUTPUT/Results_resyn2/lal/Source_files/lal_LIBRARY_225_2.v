// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n93_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x06), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  and2   g10(.a(x22), .b(x21), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g12(.a(x24), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(x23), .O(new_n59_));
  nor2   g14(.a(x24), .b(x06), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  inv1   g16(.a(new_n48_), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x07), .c(new_n62_), .O(new_n65_));
  oai21  g20(.a(new_n61_), .b(x25), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  inv1   g23(.a(x23), .O(new_n69_));
  nand2  g24(.a(new_n57_), .b(new_n69_), .O(new_n70_));
  aoi22  g25(.a(new_n70_), .b(x24), .c(new_n62_), .d(x25), .O(z03));
  inv1   g26(.a(x12), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x03), .O(new_n73_));
  inv1   g28(.a(x02), .O(new_n74_));
  nand2  g29(.a(x11), .b(new_n74_), .O(new_n75_));
  inv1   g30(.a(x03), .O(new_n76_));
  nand2  g31(.a(x12), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(x11), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x02), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .d(new_n73_), .O(new_n80_));
  inv1   g35(.a(x00), .O(new_n81_));
  nand2  g36(.a(x09), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x01), .O(new_n83_));
  nand2  g38(.a(x10), .b(new_n83_), .O(new_n84_));
  inv1   g39(.a(x09), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x00), .O(new_n86_));
  inv1   g41(.a(x10), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x01), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n80_), .c(new_n46_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n62_), .O(z04));
  oai21  g46(.a(x13), .b(x08), .c(new_n62_), .O(z05));
  nand2  g47(.a(x14), .b(new_n46_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n62_), .O(z06));
  oai21  g49(.a(new_n58_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g50(.a(x25), .O(new_n96_));
  nand2  g51(.a(new_n70_), .b(x24), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n62_), .c(new_n96_), .O(z08));
  nor2   g53(.a(x15), .b(x07), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n64_), .c(new_n62_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z09));
  nand3  g56(.a(new_n99_), .b(new_n63_), .c(new_n62_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n51_), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(z10));
  nand2  g60(.a(new_n99_), .b(new_n63_), .O(new_n106_));
  nand2  g61(.a(new_n52_), .b(new_n51_), .O(new_n107_));
  nand2  g62(.a(x18), .b(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n106_), .c(new_n62_), .O(z11));
  nand3  g65(.a(x19), .b(x18), .c(x17), .O(new_n111_));
  aoi21  g66(.a(new_n108_), .b(new_n53_), .c(new_n106_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n62_), .O(z12));
  nor2   g69(.a(new_n111_), .b(x20), .O(new_n115_));
  inv1   g70(.a(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n111_), .b(x20), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n103_), .O(z13));
  nand2  g73(.a(new_n116_), .b(x21), .O(new_n119_));
  inv1   g74(.a(x21), .O(new_n120_));
  nand2  g75(.a(new_n115_), .b(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n103_), .O(z14));
  nand2  g77(.a(new_n121_), .b(x22), .O(new_n123_));
  nor2   g78(.a(x22), .b(x21), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n115_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n103_), .O(z15));
  nand2  g81(.a(new_n125_), .b(x23), .O(new_n127_));
  nand3  g82(.a(new_n124_), .b(new_n115_), .c(new_n69_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n103_), .O(z16));
  nand2  g84(.a(new_n128_), .b(x24), .O(new_n130_));
  nand4  g85(.a(new_n124_), .b(new_n115_), .c(new_n58_), .d(new_n69_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n130_), .c(new_n103_), .O(z17));
  xor2a  g87(.a(new_n131_), .b(x25), .O(new_n133_));
  aoi21  g88(.a(new_n96_), .b(x24), .c(new_n47_), .O(new_n134_));
  nor2   g89(.a(new_n134_), .b(new_n106_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n133_), .O(z18));
endmodule


