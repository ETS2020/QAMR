// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nand2  g03(.a(x24), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x07), .O(new_n52_));
  nor2   g07(.a(x25), .b(x24), .O(z03));
  inv1   g08(.a(z03), .O(z08));
  nand2  g09(.a(x05), .b(x04), .O(new_n55_));
  nand4  g10(.a(new_n55_), .b(z08), .c(new_n49_), .d(new_n52_), .O(z01));
  xnor2a g11(.a(x12), .b(x03), .O(new_n57_));
  xnor2a g12(.a(x11), .b(x02), .O(new_n58_));
  xnor2a g13(.a(x09), .b(x00), .O(new_n59_));
  xnor2a g14(.a(x10), .b(x01), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n49_), .O(z04));
  inv1   g18(.a(new_n49_), .O(new_n64_));
  nor3   g19(.a(new_n64_), .b(x13), .c(x08), .O(z05));
  nand2  g20(.a(x14), .b(new_n46_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n49_), .O(z06));
  aoi21  g22(.a(new_n46_), .b(x06), .c(new_n64_), .O(z07));
  nor2   g23(.a(x15), .b(x07), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n49_), .c(x05), .d(x04), .O(new_n70_));
  inv1   g25(.a(new_n70_), .O(z09));
  inv1   g26(.a(x17), .O(new_n72_));
  nand3  g27(.a(new_n69_), .b(new_n55_), .c(new_n49_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z10));
  nand2  g31(.a(x18), .b(x17), .O(new_n77_));
  inv1   g32(.a(x18), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n74_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z11));
  nand2  g36(.a(new_n69_), .b(new_n55_), .O(new_n82_));
  xor2a  g37(.a(new_n77_), .b(x19), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n82_), .c(new_n49_), .O(z12));
  inv1   g39(.a(x20), .O(new_n85_));
  nand3  g40(.a(x19), .b(x18), .c(x17), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  aoi21  g43(.a(new_n86_), .b(x20), .c(new_n82_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n88_), .c(new_n64_), .O(z13));
  nand2  g45(.a(new_n88_), .b(x21), .O(new_n91_));
  nor2   g46(.a(x21), .b(x20), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n87_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n91_), .c(new_n74_), .O(z14));
  nand2  g49(.a(new_n93_), .b(x22), .O(new_n95_));
  inv1   g50(.a(x22), .O(new_n96_));
  nand3  g51(.a(new_n92_), .b(new_n87_), .c(new_n96_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n95_), .c(new_n74_), .O(z15));
  nand4  g53(.a(new_n92_), .b(new_n87_), .c(new_n48_), .d(new_n96_), .O(new_n99_));
  nand2  g54(.a(new_n97_), .b(x23), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n74_), .O(z16));
  inv1   g56(.a(x24), .O(new_n102_));
  inv1   g57(.a(new_n82_), .O(new_n103_));
  nand3  g58(.a(new_n99_), .b(new_n103_), .c(new_n102_), .O(z17));
  inv1   g59(.a(x25), .O(new_n105_));
  nand2  g60(.a(new_n99_), .b(new_n105_), .O(new_n106_));
  nand4  g61(.a(new_n92_), .b(new_n87_), .c(x25), .d(new_n96_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n102_), .O(new_n108_));
  aoi22  g63(.a(new_n82_), .b(new_n49_), .c(x25), .d(x23), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z18));
endmodule


