// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x24), .b(x07), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x24), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  oai21  g05(.a(new_n50_), .b(x07), .c(new_n49_), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  and2   g12(.a(x22), .b(x21), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nor2   g14(.a(x25), .b(x23), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  aoi21  g17(.a(new_n60_), .b(new_n59_), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x07), .c(new_n51_), .O(z01));
  and2   g19(.a(new_n47_), .b(x16), .O(z02));
  aoi21  g20(.a(new_n58_), .b(new_n57_), .c(x23), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(new_n49_), .c(new_n50_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n47_), .O(z03));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  or2    g31(.a(new_n69_), .b(x13), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x08), .c(new_n47_), .O(z06));
  aoi22  g35(.a(x24), .b(x07), .c(new_n46_), .d(x06), .O(z07));
  inv1   g36(.a(x07), .O(new_n82_));
  inv1   g37(.a(x23), .O(new_n83_));
  nand3  g38(.a(new_n59_), .b(new_n83_), .c(new_n82_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(x24), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n50_), .O(z08));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n62_), .b(new_n87_), .c(new_n82_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand2  g44(.a(new_n61_), .b(new_n87_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n82_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n53_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z10));
  nor2   g50(.a(x18), .b(x17), .O(new_n96_));
  nor2   g51(.a(new_n54_), .b(new_n53_), .O(new_n97_));
  nor3   g52(.a(new_n97_), .b(new_n92_), .c(new_n96_), .O(z11));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n97_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n92_), .O(z12));
  nand2  g57(.a(new_n49_), .b(x07), .O(new_n103_));
  nand2  g58(.a(new_n100_), .b(new_n52_), .O(new_n104_));
  nand2  g59(.a(new_n99_), .b(x20), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n91_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n82_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n103_), .O(z13));
  nand2  g63(.a(new_n104_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n100_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n91_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n82_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n103_), .O(z14));
  nand2  g69(.a(new_n111_), .b(x22), .O(new_n115_));
  inv1   g70(.a(x22), .O(new_n116_));
  nand3  g71(.a(new_n110_), .b(new_n100_), .c(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n91_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n82_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n103_), .O(z15));
  nand2  g75(.a(new_n117_), .b(x23), .O(new_n121_));
  nand4  g76(.a(new_n110_), .b(new_n100_), .c(new_n83_), .d(new_n116_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n91_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n82_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n103_), .O(z16));
  oai21  g80(.a(new_n122_), .b(new_n90_), .c(new_n82_), .O(new_n126_));
  nand3  g81(.a(new_n91_), .b(new_n49_), .c(new_n82_), .O(new_n127_));
  inv1   g82(.a(new_n127_), .O(new_n128_));
  aoi22  g83(.a(new_n128_), .b(new_n122_), .c(new_n126_), .d(x24), .O(z17));
  inv1   g84(.a(new_n122_), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n50_), .c(new_n49_), .O(new_n131_));
  oai21  g86(.a(new_n122_), .b(x24), .c(x25), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n93_), .O(z18));
endmodule


