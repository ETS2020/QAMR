// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n83_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x19), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  and2   g07(.a(x22), .b(x21), .O(new_n53_));
  oai21  g08(.a(new_n52_), .b(x20), .c(new_n53_), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n50_), .c(new_n51_), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  nor2   g11(.a(x25), .b(x23), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(x24), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g17(.a(new_n59_), .b(x19), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n55_), .b(x25), .c(new_n63_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n53_), .O(new_n67_));
  aoi21  g22(.a(new_n52_), .b(new_n47_), .c(x20), .O(new_n68_));
  nor2   g23(.a(new_n51_), .b(x23), .O(new_n69_));
  oai21  g24(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand2  g25(.a(new_n51_), .b(new_n47_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n70_), .c(x25), .O(z03));
  inv1   g27(.a(new_n48_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  xor2a  g29(.a(x12), .b(x03), .O(new_n75_));
  xor2a  g30(.a(x11), .b(x02), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  xor2a  g33(.a(x10), .b(x01), .O(new_n79_));
  nor2   g34(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n77_), .c(new_n74_), .O(z04));
  nor2   g36(.a(new_n74_), .b(x13), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n73_), .O(z06));
  nand3  g39(.a(new_n73_), .b(new_n46_), .c(x06), .O(z07));
  oai21  g40(.a(new_n67_), .b(new_n56_), .c(new_n57_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x24), .O(new_n87_));
  inv1   g42(.a(x25), .O(new_n88_));
  nand3  g43(.a(new_n53_), .b(new_n52_), .c(x24), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n87_), .O(z08));
  inv1   g47(.a(new_n61_), .O(new_n93_));
  nor2   g48(.a(x15), .b(x07), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n73_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z09));
  nand2  g51(.a(new_n94_), .b(new_n61_), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(x17), .c(new_n73_), .O(z10));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor4   g55(.a(new_n100_), .b(new_n97_), .c(new_n52_), .d(new_n48_), .O(z11));
  inv1   g56(.a(new_n97_), .O(new_n102_));
  nand2  g57(.a(new_n99_), .b(new_n102_), .O(new_n103_));
  nand2  g58(.a(new_n100_), .b(new_n102_), .O(new_n104_));
  nor2   g59(.a(new_n51_), .b(new_n47_), .O(new_n105_));
  aoi22  g60(.a(new_n105_), .b(new_n103_), .c(new_n104_), .d(new_n47_), .O(z12));
  nor2   g61(.a(new_n99_), .b(x20), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(new_n51_), .c(x19), .O(new_n108_));
  nand2  g63(.a(new_n105_), .b(new_n100_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n102_), .O(z13));
  nand3  g66(.a(new_n56_), .b(x18), .c(x17), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(x21), .c(x24), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x19), .O(new_n114_));
  oai21  g69(.a(new_n109_), .b(x20), .c(x21), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n102_), .O(z14));
  nand2  g71(.a(new_n97_), .b(new_n73_), .O(new_n117_));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  inv1   g73(.a(x21), .O(new_n119_));
  inv1   g74(.a(x22), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nor2   g76(.a(new_n121_), .b(new_n112_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n105_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n118_), .c(new_n117_), .O(z15));
  nor2   g79(.a(x22), .b(x21), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n107_), .c(new_n51_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n47_), .c(x23), .O(new_n127_));
  nand3  g82(.a(new_n122_), .b(new_n105_), .c(new_n50_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n117_), .O(z16));
  nand2  g84(.a(new_n128_), .b(new_n71_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n102_), .O(z17));
  aoi21  g86(.a(new_n102_), .b(new_n88_), .c(new_n48_), .O(z18));
endmodule


