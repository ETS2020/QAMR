// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x21), .b(x17), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x19), .b(x18), .c(new_n52_), .O(new_n53_));
  inv1   g08(.a(x21), .O(new_n54_));
  inv1   g09(.a(x22), .O(new_n55_));
  nor2   g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n53_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n50_), .c(new_n51_), .O(new_n58_));
  aoi21  g13(.a(x05), .b(x04), .c(x07), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(z01));
  nand2  g15(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g16(.a(new_n58_), .b(new_n47_), .O(z03));
  inv1   g17(.a(x08), .O(new_n63_));
  xnor2a g18(.a(x12), .b(x03), .O(new_n64_));
  xnor2a g19(.a(x11), .b(x02), .O(new_n65_));
  xnor2a g20(.a(x09), .b(x00), .O(new_n66_));
  xnor2a g21(.a(x10), .b(x01), .O(new_n67_));
  nand4  g22(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n47_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  nand3  g26(.a(new_n47_), .b(x14), .c(new_n63_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(z06));
  nand3  g28(.a(new_n47_), .b(new_n63_), .c(x06), .O(z07));
  nor2   g29(.a(new_n50_), .b(new_n55_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n53_), .c(x17), .O(new_n76_));
  aoi21  g31(.a(x24), .b(x23), .c(x25), .O(new_n77_));
  oai21  g32(.a(new_n76_), .b(new_n54_), .c(new_n77_), .O(z08));
  inv1   g33(.a(x07), .O(new_n79_));
  inv1   g34(.a(x15), .O(new_n80_));
  nand4  g35(.a(new_n80_), .b(new_n79_), .c(x05), .d(x04), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n47_), .O(z09));
  nand2  g37(.a(new_n59_), .b(new_n80_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x17), .c(new_n47_), .O(z10));
  inv1   g39(.a(new_n83_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  xnor2a g41(.a(x18), .b(x17), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n86_), .O(z11));
  inv1   g43(.a(x19), .O(new_n89_));
  and2   g44(.a(x18), .b(x17), .O(new_n90_));
  xor2a  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n86_), .O(z12));
  inv1   g47(.a(x17), .O(new_n93_));
  aoi21  g48(.a(x19), .b(x18), .c(x21), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  nand2  g50(.a(new_n83_), .b(new_n47_), .O(new_n96_));
  nor2   g51(.a(x20), .b(new_n89_), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n90_), .c(new_n54_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n95_), .O(z13));
  nand2  g54(.a(new_n97_), .b(new_n90_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n85_), .c(new_n54_), .O(z14));
  nand3  g56(.a(new_n52_), .b(x19), .c(x18), .O(new_n102_));
  and2   g57(.a(new_n102_), .b(x22), .O(new_n103_));
  nand4  g58(.a(new_n55_), .b(new_n52_), .c(x19), .d(x18), .O(new_n104_));
  nor2   g59(.a(new_n104_), .b(new_n93_), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(new_n103_), .c(new_n54_), .O(new_n106_));
  nand2  g61(.a(x22), .b(new_n93_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n96_), .O(z15));
  nand2  g63(.a(new_n104_), .b(x23), .O(new_n109_));
  nor2   g64(.a(x23), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n97_), .c(new_n90_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n54_), .O(new_n113_));
  nand2  g68(.a(x23), .b(new_n93_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n96_), .O(z16));
  inv1   g70(.a(new_n110_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n98_), .c(x24), .O(new_n117_));
  inv1   g72(.a(x23), .O(new_n118_));
  nand3  g73(.a(new_n50_), .b(new_n118_), .c(new_n55_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n102_), .c(new_n54_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x17), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n117_), .c(new_n85_), .O(z17));
  oai21  g77(.a(new_n119_), .b(new_n102_), .c(x25), .O(new_n123_));
  nor3   g78(.a(x24), .b(x23), .c(x22), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n97_), .c(new_n90_), .d(new_n51_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n54_), .O(new_n127_));
  aoi22  g82(.a(new_n83_), .b(new_n47_), .c(x25), .d(new_n93_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n127_), .O(z18));
endmodule


