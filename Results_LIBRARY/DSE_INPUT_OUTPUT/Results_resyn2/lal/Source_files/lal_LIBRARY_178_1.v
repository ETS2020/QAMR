// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n75_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x05), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(new_n48_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n53_), .c(new_n52_), .O(z08));
  nand2  g17(.a(new_n53_), .b(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n63_), .c(z08), .O(z01));
  nand2  g20(.a(new_n61_), .b(new_n52_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n53_), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  and2   g27(.a(new_n72_), .b(new_n50_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n53_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  oai21  g30(.a(new_n75_), .b(x08), .c(new_n53_), .O(z06));
  nand2  g31(.a(new_n50_), .b(x06), .O(z07));
  inv1   g32(.a(x07), .O(new_n78_));
  nand2  g33(.a(new_n47_), .b(new_n78_), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(new_n64_), .c(new_n53_), .O(z09));
  oai21  g35(.a(x15), .b(x04), .c(x05), .O(new_n81_));
  and2   g36(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n55_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n53_), .O(z10));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n82_), .c(new_n85_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n53_), .O(z11));
  nand3  g43(.a(x19), .b(x18), .c(x17), .O(new_n89_));
  nand2  g44(.a(new_n86_), .b(new_n57_), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n89_), .c(new_n82_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n53_), .O(z12));
  nor2   g47(.a(new_n89_), .b(x20), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand3  g49(.a(new_n64_), .b(new_n47_), .c(new_n78_), .O(new_n95_));
  aoi21  g50(.a(new_n89_), .b(x20), .c(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n94_), .O(z13));
  inv1   g52(.a(x21), .O(new_n98_));
  aoi21  g53(.a(new_n93_), .b(new_n98_), .c(new_n95_), .O(new_n99_));
  oai21  g54(.a(new_n93_), .b(new_n98_), .c(new_n99_), .O(z14));
  inv1   g55(.a(x22), .O(new_n101_));
  aoi21  g56(.a(new_n93_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  inv1   g57(.a(new_n89_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n98_), .d(new_n54_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n78_), .O(new_n105_));
  oai21  g60(.a(new_n105_), .b(new_n102_), .c(new_n47_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n81_), .O(z15));
  nand2  g62(.a(new_n104_), .b(x23), .O(new_n108_));
  nor2   g63(.a(x23), .b(x22), .O(new_n109_));
  nand4  g64(.a(new_n109_), .b(new_n103_), .c(new_n98_), .d(new_n54_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n78_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n47_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n81_), .O(z16));
  nand2  g68(.a(new_n110_), .b(x24), .O(new_n114_));
  inv1   g69(.a(x24), .O(new_n115_));
  nand4  g70(.a(new_n109_), .b(new_n93_), .c(new_n115_), .d(new_n98_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n78_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n47_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n81_), .O(z17));
  aoi21  g74(.a(new_n116_), .b(x25), .c(new_n95_), .O(new_n120_));
  oai21  g75(.a(new_n116_), .b(x25), .c(new_n120_), .O(z18));
endmodule


