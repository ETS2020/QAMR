// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  nor2   g02(.a(x15), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x07), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n58_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n48_), .b(new_n63_), .O(z02));
  inv1   g19(.a(new_n48_), .O(new_n65_));
  nand3  g20(.a(new_n57_), .b(new_n65_), .c(new_n50_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  nand2  g22(.a(new_n65_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n65_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n68_), .b(new_n77_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n60_), .b(new_n80_), .c(new_n47_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  inv1   g37(.a(x17), .O(new_n83_));
  nand2  g38(.a(new_n59_), .b(new_n83_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n47_), .c(x15), .O(z10));
  nand2  g40(.a(new_n61_), .b(new_n80_), .O(new_n86_));
  and2   g41(.a(x18), .b(x17), .O(new_n87_));
  nor3   g42(.a(new_n87_), .b(new_n86_), .c(new_n53_), .O(z11));
  aoi21  g43(.a(new_n87_), .b(x19), .c(new_n60_), .O(new_n89_));
  oai21  g44(.a(new_n87_), .b(x19), .c(new_n89_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n47_), .c(x15), .O(z12));
  inv1   g46(.a(new_n86_), .O(new_n92_));
  nand2  g47(.a(new_n87_), .b(x19), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(x20), .O(new_n94_));
  nand4  g49(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(z13));
  nand2  g51(.a(new_n95_), .b(x21), .O(new_n97_));
  inv1   g52(.a(x21), .O(new_n98_));
  inv1   g53(.a(new_n95_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n97_), .c(new_n59_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n80_), .O(z14));
  inv1   g58(.a(x22), .O(new_n104_));
  nor2   g59(.a(x20), .b(new_n52_), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n87_), .c(new_n104_), .d(new_n98_), .O(new_n106_));
  nand2  g61(.a(new_n100_), .b(x22), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n92_), .O(z15));
  nand2  g63(.a(new_n106_), .b(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  nand4  g65(.a(new_n99_), .b(new_n110_), .c(new_n104_), .d(new_n98_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n59_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n47_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n80_), .O(z16));
  nand2  g69(.a(new_n111_), .b(x24), .O(new_n115_));
  inv1   g70(.a(x24), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n110_), .c(new_n104_), .d(new_n98_), .O(new_n117_));
  nor2   g72(.a(new_n117_), .b(new_n95_), .O(new_n118_));
  nor2   g73(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n115_), .O(z17));
  nand2  g75(.a(new_n118_), .b(new_n50_), .O(new_n121_));
  oai21  g76(.a(new_n117_), .b(new_n95_), .c(x25), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n121_), .c(new_n59_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n80_), .O(z18));
endmodule


