// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(x17), .b(new_n47_), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  inv1   g14(.a(x07), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(new_n48_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n59_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nor2   g20(.a(new_n48_), .b(new_n65_), .O(z02));
  inv1   g21(.a(new_n48_), .O(new_n67_));
  nand2  g22(.a(new_n59_), .b(new_n67_), .O(z03));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n67_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n69_), .b(new_n78_), .O(z06));
  nand3  g34(.a(new_n67_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g35(.a(new_n58_), .b(new_n50_), .c(new_n48_), .O(z08));
  nand2  g36(.a(new_n47_), .b(new_n60_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n61_), .c(new_n67_), .O(z09));
  inv1   g38(.a(new_n62_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n47_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n52_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  oai21  g43(.a(new_n52_), .b(new_n47_), .c(new_n84_), .O(new_n89_));
  nand2  g44(.a(new_n53_), .b(new_n52_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n89_), .c(new_n67_), .O(z11));
  nand2  g48(.a(new_n91_), .b(new_n54_), .O(new_n94_));
  nand3  g49(.a(x19), .b(x18), .c(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n89_), .c(new_n67_), .O(z12));
  nand2  g52(.a(x19), .b(x18), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(x20), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(x15), .c(x17), .O(new_n100_));
  aoi21  g55(.a(new_n95_), .b(x20), .c(new_n62_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(x15), .c(new_n100_), .O(z13));
  nand4  g57(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(x21), .O(new_n104_));
  inv1   g59(.a(x21), .O(new_n105_));
  nand3  g60(.a(new_n99_), .b(new_n105_), .c(x17), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n86_), .O(z14));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n51_), .c(x19), .d(x18), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(x17), .O(new_n111_));
  aoi21  g66(.a(new_n106_), .b(x22), .c(new_n62_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(x15), .c(new_n111_), .O(z15));
  nand2  g68(.a(new_n108_), .b(new_n51_), .O(new_n114_));
  oai21  g69(.a(new_n114_), .b(new_n95_), .c(x23), .O(new_n115_));
  inv1   g70(.a(x23), .O(new_n116_));
  inv1   g71(.a(new_n95_), .O(new_n117_));
  nand4  g72(.a(new_n108_), .b(new_n117_), .c(new_n116_), .d(new_n51_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n86_), .O(z16));
  aoi21  g74(.a(new_n118_), .b(x24), .c(new_n62_), .O(new_n120_));
  inv1   g75(.a(x24), .O(new_n121_));
  nand3  g76(.a(new_n108_), .b(new_n121_), .c(new_n116_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n99_), .c(x15), .O(new_n124_));
  oai22  g79(.a(new_n124_), .b(new_n52_), .c(new_n120_), .d(x15), .O(z17));
  nand3  g80(.a(new_n50_), .b(new_n121_), .c(new_n116_), .O(new_n126_));
  oai21  g81(.a(new_n126_), .b(new_n109_), .c(new_n47_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x17), .O(new_n128_));
  oai21  g83(.a(new_n122_), .b(new_n103_), .c(x25), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n84_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n47_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n128_), .O(z18));
endmodule


