// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x05), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(new_n49_), .b(new_n62_), .O(new_n63_));
  aoi21  g18(.a(x05), .b(x04), .c(new_n63_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n61_), .O(z01));
  nand2  g20(.a(new_n61_), .b(new_n49_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  nor3   g28(.a(new_n48_), .b(x13), .c(x08), .O(z05));
  nand2  g29(.a(x14), .b(new_n46_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n49_), .O(z06));
  aoi21  g31(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand3  g32(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(z08));
  inv1   g33(.a(x15), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n62_), .c(x05), .d(x04), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z09));
  nor2   g36(.a(x07), .b(x04), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n79_), .c(x05), .O(new_n83_));
  nor2   g38(.a(new_n83_), .b(x17), .O(z10));
  nor2   g39(.a(x18), .b(x17), .O(new_n85_));
  nor2   g40(.a(new_n55_), .b(new_n54_), .O(new_n86_));
  nor3   g41(.a(new_n86_), .b(new_n83_), .c(new_n85_), .O(z11));
  oai21  g42(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n88_));
  nand2  g43(.a(new_n86_), .b(x19), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n82_), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(x05), .c(x15), .O(z12));
  inv1   g46(.a(new_n83_), .O(new_n92_));
  nand2  g47(.a(new_n89_), .b(x20), .O(new_n93_));
  nand4  g48(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(z13));
  nand2  g50(.a(new_n94_), .b(x21), .O(new_n96_));
  inv1   g51(.a(x21), .O(new_n97_));
  inv1   g52(.a(new_n94_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n96_), .c(new_n82_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x05), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n79_), .O(z14));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n98_), .b(new_n103_), .c(new_n97_), .O(new_n104_));
  oai21  g59(.a(new_n94_), .b(x21), .c(x22), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n82_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x05), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n79_), .O(z15));
  nand2  g63(.a(new_n104_), .b(x23), .O(new_n109_));
  nor2   g64(.a(x20), .b(new_n56_), .O(new_n110_));
  nor2   g65(.a(x23), .b(x22), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n86_), .d(new_n97_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n109_), .c(new_n92_), .O(z16));
  inv1   g68(.a(x05), .O(new_n114_));
  inv1   g69(.a(x23), .O(new_n115_));
  inv1   g70(.a(x24), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n115_), .c(new_n103_), .d(new_n97_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n94_), .c(new_n82_), .O(new_n118_));
  aoi21  g73(.a(new_n112_), .b(x24), .c(new_n118_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n114_), .c(new_n79_), .O(z17));
  inv1   g75(.a(new_n117_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n98_), .c(new_n52_), .O(new_n122_));
  oai21  g77(.a(new_n117_), .b(new_n94_), .c(x25), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n122_), .c(new_n82_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(x05), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n79_), .O(z18));
endmodule


