// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(x25), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x19), .O(new_n51_));
  nor2   g06(.a(x18), .b(x17), .O(new_n52_));
  aoi21  g07(.a(new_n52_), .b(new_n51_), .c(x20), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  nor2   g09(.a(x25), .b(x23), .O(new_n55_));
  oai21  g10(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n56_));
  nand2  g11(.a(x05), .b(x04), .O(new_n57_));
  nor2   g12(.a(new_n47_), .b(x07), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(z01));
  and2   g14(.a(new_n48_), .b(x16), .O(z02));
  nand2  g15(.a(new_n56_), .b(x24), .O(z03));
  inv1   g16(.a(x12), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(x03), .O(new_n63_));
  inv1   g18(.a(x11), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(x02), .O(new_n65_));
  inv1   g20(.a(x02), .O(new_n66_));
  nand2  g21(.a(x11), .b(new_n66_), .O(new_n67_));
  inv1   g22(.a(x03), .O(new_n68_));
  nand2  g23(.a(x12), .b(new_n68_), .O(new_n69_));
  nand4  g24(.a(new_n69_), .b(new_n67_), .c(new_n65_), .d(new_n63_), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  xnor2a g26(.a(x09), .b(x00), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n70_), .c(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n48_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n48_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g34(.a(x20), .O(new_n80_));
  inv1   g35(.a(x17), .O(new_n81_));
  inv1   g36(.a(x18), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x19), .c(new_n80_), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(x22), .c(x21), .O(new_n85_));
  aoi21  g40(.a(new_n55_), .b(new_n85_), .c(new_n47_), .O(z08));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n48_), .c(x05), .d(x04), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  nand3  g44(.a(new_n87_), .b(new_n57_), .c(new_n48_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n81_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n87_), .b(new_n57_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n83_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n94_), .c(new_n48_), .O(z11));
  inv1   g52(.a(new_n94_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n95_), .b(new_n51_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n48_), .O(z12));
  nand2  g57(.a(new_n99_), .b(x20), .O(new_n103_));
  nand4  g58(.a(new_n80_), .b(x19), .c(x18), .d(x17), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nor2   g60(.a(new_n105_), .b(new_n94_), .O(new_n106_));
  aoi21  g61(.a(new_n106_), .b(new_n103_), .c(new_n49_), .O(z13));
  nand2  g62(.a(new_n104_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  aoi21  g64(.a(new_n105_), .b(new_n109_), .c(new_n94_), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n108_), .c(new_n49_), .O(z14));
  oai21  g66(.a(new_n104_), .b(x21), .c(x22), .O(new_n112_));
  inv1   g67(.a(x22), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  nor2   g69(.a(new_n114_), .b(new_n104_), .O(new_n115_));
  nor2   g70(.a(new_n115_), .b(new_n90_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n112_), .O(z15));
  oai21  g72(.a(new_n114_), .b(new_n104_), .c(x23), .O(new_n118_));
  inv1   g73(.a(new_n99_), .O(new_n119_));
  inv1   g74(.a(x23), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n113_), .c(new_n109_), .d(new_n80_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  aoi21  g77(.a(new_n122_), .b(new_n119_), .c(new_n94_), .O(new_n123_));
  aoi21  g78(.a(new_n123_), .b(new_n118_), .c(new_n49_), .O(z16));
  nand3  g79(.a(new_n122_), .b(new_n119_), .c(new_n47_), .O(new_n125_));
  oai21  g80(.a(new_n121_), .b(new_n99_), .c(x24), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n91_), .O(z17));
  inv1   g82(.a(x25), .O(new_n128_));
  nand3  g83(.a(new_n115_), .b(new_n47_), .c(new_n120_), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n98_), .c(new_n128_), .O(z18));
endmodule


