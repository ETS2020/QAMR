// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x02), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n50_));
  inv1   g05(.a(x21), .O(new_n51_));
  inv1   g06(.a(x22), .O(new_n52_));
  nor2   g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g08(.a(new_n50_), .b(x20), .c(new_n53_), .O(new_n54_));
  nor2   g09(.a(x25), .b(x23), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g11(.a(x25), .b(x02), .c(x24), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor3   g14(.a(new_n59_), .b(new_n57_), .c(x07), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  inv1   g18(.a(x23), .O(new_n64_));
  nand3  g19(.a(new_n54_), .b(x24), .c(new_n64_), .O(new_n65_));
  inv1   g20(.a(x24), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(x02), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n65_), .c(x25), .O(z03));
  aoi21  g23(.a(x11), .b(new_n46_), .c(new_n66_), .O(new_n69_));
  inv1   g24(.a(x00), .O(new_n70_));
  nand2  g25(.a(x09), .b(new_n70_), .O(new_n71_));
  inv1   g26(.a(x12), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x03), .O(new_n73_));
  inv1   g28(.a(x09), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(x00), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  xnor2a g31(.a(x10), .b(x01), .O(new_n77_));
  inv1   g32(.a(x03), .O(new_n78_));
  nand2  g33(.a(x12), .b(new_n78_), .O(new_n79_));
  inv1   g34(.a(x11), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(x02), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n76_), .c(new_n46_), .O(new_n83_));
  oai21  g38(.a(new_n69_), .b(x02), .c(new_n83_), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g40(.a(x14), .b(new_n46_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n48_), .O(z06));
  nand3  g42(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g43(.a(new_n55_), .b(new_n54_), .c(new_n57_), .O(z08));
  inv1   g44(.a(x07), .O(new_n90_));
  inv1   g45(.a(x15), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n47_), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n59_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z09));
  nand3  g50(.a(new_n58_), .b(new_n91_), .c(new_n90_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g52(.a(x18), .b(x17), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n96_), .c(new_n48_), .O(z11));
  nand2  g54(.a(new_n93_), .b(new_n58_), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nor3   g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z12));
  nor2   g59(.a(new_n101_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  aoi21  g61(.a(new_n101_), .b(x20), .c(new_n96_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n47_), .O(z13));
  inv1   g63(.a(new_n100_), .O(new_n109_));
  nand2  g64(.a(new_n106_), .b(x21), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(new_n51_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(z14));
  nand2  g67(.a(new_n111_), .b(x22), .O(new_n113_));
  nor2   g68(.a(x22), .b(x21), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n105_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n109_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  inv1   g72(.a(x20), .O(new_n118_));
  nand4  g73(.a(new_n64_), .b(new_n52_), .c(new_n51_), .d(new_n118_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n102_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n117_), .c(new_n109_), .O(z16));
  nand3  g77(.a(new_n120_), .b(new_n102_), .c(new_n66_), .O(new_n123_));
  nand2  g78(.a(new_n121_), .b(x24), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n109_), .O(z17));
  oai21  g80(.a(new_n119_), .b(new_n101_), .c(x25), .O(new_n126_));
  nand4  g81(.a(new_n114_), .b(new_n105_), .c(new_n55_), .d(new_n66_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(x02), .O(new_n129_));
  aoi22  g84(.a(new_n96_), .b(new_n48_), .c(x25), .d(x24), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n129_), .O(z18));
endmodule


