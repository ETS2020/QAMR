// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x22), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x24), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(x22), .c(x21), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n53_), .c(new_n54_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n50_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(new_n68_));
  oai21  g23(.a(new_n62_), .b(x25), .c(new_n68_), .O(z01));
  nand2  g24(.a(new_n50_), .b(new_n46_), .O(z02));
  nor3   g25(.a(new_n62_), .b(new_n49_), .c(x25), .O(z03));
  inv1   g26(.a(x08), .O(new_n72_));
  xnor2a g27(.a(x12), .b(x03), .O(new_n73_));
  xnor2a g28(.a(x11), .b(x02), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  xnor2a g30(.a(x10), .b(x01), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(new_n73_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n50_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand3  g35(.a(new_n50_), .b(x14), .c(new_n72_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z06));
  nand3  g37(.a(new_n50_), .b(new_n72_), .c(x06), .O(z07));
  inv1   g38(.a(x21), .O(new_n84_));
  aoi21  g39(.a(new_n59_), .b(new_n55_), .c(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(x24), .c(x15), .O(new_n86_));
  aoi21  g41(.a(x24), .b(x23), .c(x25), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(new_n48_), .c(new_n87_), .O(z08));
  nand2  g43(.a(new_n47_), .b(new_n63_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(new_n64_), .c(new_n50_), .O(z09));
  nand2  g45(.a(new_n66_), .b(new_n47_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n56_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z10));
  xnor2a g49(.a(x18), .b(x17), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n91_), .c(new_n50_), .O(z11));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  aoi21  g53(.a(x18), .b(x17), .c(x19), .O(new_n99_));
  nor3   g54(.a(new_n99_), .b(new_n98_), .c(new_n91_), .O(z12));
  nand2  g55(.a(new_n48_), .b(x15), .O(new_n101_));
  nand2  g56(.a(new_n98_), .b(new_n55_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n66_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n101_), .O(z13));
  nand2  g61(.a(new_n102_), .b(x21), .O(new_n107_));
  nor2   g62(.a(new_n97_), .b(x20), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n84_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n66_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n101_), .O(z14));
  nand2  g67(.a(new_n109_), .b(x22), .O(new_n113_));
  inv1   g68(.a(new_n109_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n48_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n92_), .O(z15));
  nand2  g71(.a(new_n115_), .b(x23), .O(new_n117_));
  nand4  g72(.a(new_n108_), .b(new_n53_), .c(new_n48_), .d(new_n84_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n92_), .O(z16));
  nor2   g74(.a(x24), .b(x23), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n114_), .c(x15), .O(new_n121_));
  aoi21  g76(.a(new_n118_), .b(x24), .c(new_n65_), .O(new_n122_));
  oai22  g77(.a(new_n122_), .b(x15), .c(new_n121_), .d(x22), .O(z17));
  inv1   g78(.a(x25), .O(new_n124_));
  nand4  g79(.a(new_n120_), .b(new_n114_), .c(new_n124_), .d(new_n48_), .O(new_n125_));
  nand3  g80(.a(new_n120_), .b(new_n114_), .c(new_n48_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x25), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n125_), .c(new_n92_), .O(z18));
endmodule


