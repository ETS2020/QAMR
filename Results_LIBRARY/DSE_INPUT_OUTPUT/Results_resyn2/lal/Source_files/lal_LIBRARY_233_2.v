// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x15), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(new_n48_), .O(new_n50_));
  aoi21  g05(.a(x05), .b(x04), .c(x07), .O(new_n51_));
  or2    g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nor2   g16(.a(new_n47_), .b(x23), .O(new_n62_));
  nor2   g17(.a(x24), .b(x15), .O(new_n63_));
  aoi21  g18(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(x25), .c(new_n52_), .O(z01));
  and2   g20(.a(new_n48_), .b(x16), .O(z02));
  nor2   g21(.a(new_n64_), .b(x25), .O(z03));
  nand2  g22(.a(new_n48_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n48_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n50_), .O(z07));
  inv1   g34(.a(x25), .O(new_n80_));
  aoi21  g35(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n81_));
  inv1   g36(.a(x23), .O(new_n82_));
  nand2  g37(.a(new_n80_), .b(new_n82_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(new_n81_), .c(x24), .O(new_n84_));
  oai21  g39(.a(new_n80_), .b(x15), .c(new_n84_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand4  g42(.a(new_n87_), .b(new_n86_), .c(x05), .d(x04), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n48_), .O(z09));
  nand2  g44(.a(new_n51_), .b(new_n87_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n54_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nand2  g48(.a(new_n55_), .b(new_n54_), .O(new_n94_));
  nand2  g49(.a(x18), .b(x17), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n90_), .c(new_n48_), .O(z11));
  nand2  g52(.a(new_n95_), .b(new_n56_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n90_), .c(new_n48_), .O(z12));
  inv1   g56(.a(new_n99_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(x20), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n91_), .O(z13));
  nand2  g60(.a(new_n103_), .b(x21), .O(new_n106_));
  nor3   g61(.a(new_n99_), .b(x21), .c(x20), .O(new_n107_));
  nor2   g62(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n106_), .c(new_n50_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  aoi21  g65(.a(new_n107_), .b(new_n110_), .c(new_n90_), .O(new_n111_));
  oai21  g66(.a(new_n107_), .b(new_n110_), .c(new_n111_), .O(z15));
  nand2  g67(.a(new_n107_), .b(new_n110_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x21), .b(x20), .O(new_n115_));
  nor2   g70(.a(x23), .b(x22), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n102_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n114_), .c(new_n91_), .O(z16));
  nand3  g73(.a(new_n116_), .b(new_n107_), .c(new_n47_), .O(new_n119_));
  aoi21  g74(.a(new_n117_), .b(x24), .c(new_n90_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n119_), .O(z17));
  oai21  g76(.a(x25), .b(x15), .c(x24), .O(new_n122_));
  nor3   g77(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  nand4  g78(.a(new_n123_), .b(new_n115_), .c(new_n102_), .d(new_n110_), .O(new_n124_));
  nand2  g79(.a(new_n117_), .b(x25), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n51_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n87_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(new_n122_), .O(z18));
endmodule


