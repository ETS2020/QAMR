// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  inv1   g04(.a(x07), .O(new_n50_));
  nand2  g05(.a(x05), .b(x04), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  and2   g14(.a(x22), .b(x21), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g16(.a(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x23), .O(new_n63_));
  inv1   g18(.a(x15), .O(new_n64_));
  nor2   g19(.a(x24), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n53_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n47_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n66_), .b(x25), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  nor3   g32(.a(new_n47_), .b(x13), .c(x08), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n47_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(x23), .O(new_n82_));
  nor2   g37(.a(new_n47_), .b(x25), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(new_n62_), .c(new_n83_), .O(z08));
  nand3  g39(.a(x24), .b(new_n64_), .c(new_n50_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n51_), .O(z09));
  inv1   g41(.a(new_n52_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n55_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x24), .c(x15), .O(z10));
  nand2  g44(.a(new_n56_), .b(new_n55_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n87_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(x24), .c(x15), .O(z11));
  inv1   g48(.a(new_n85_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n51_), .O(new_n95_));
  nand3  g50(.a(x19), .b(x18), .c(x17), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(new_n97_));
  aoi21  g52(.a(x18), .b(x17), .c(x19), .O(new_n98_));
  nor3   g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z12));
  inv1   g54(.a(new_n95_), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(new_n54_), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(x20), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(z13));
  nand2  g58(.a(new_n101_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n97_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n87_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x24), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n64_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand3  g65(.a(new_n105_), .b(new_n97_), .c(new_n110_), .O(new_n111_));
  nand2  g66(.a(new_n106_), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n87_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(x24), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n64_), .O(z15));
  nand2  g70(.a(new_n111_), .b(x23), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  nand4  g72(.a(new_n105_), .b(new_n97_), .c(new_n117_), .d(new_n110_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n87_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(x24), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n64_), .O(z16));
  inv1   g76(.a(new_n118_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n100_), .O(z17));
  oai21  g78(.a(new_n52_), .b(x25), .c(x24), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n64_), .O(z18));
endmodule


