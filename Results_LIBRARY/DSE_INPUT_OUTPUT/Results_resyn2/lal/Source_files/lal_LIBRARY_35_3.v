// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n77_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  nand2  g01(.a(x15), .b(x04), .O(new_n47_));
  oai21  g02(.a(x16), .b(new_n46_), .c(new_n47_), .O(z00));
  inv1   g03(.a(x25), .O(new_n49_));
  inv1   g04(.a(x20), .O(new_n50_));
  inv1   g05(.a(x17), .O(new_n51_));
  inv1   g06(.a(x18), .O(new_n52_));
  inv1   g07(.a(x19), .O(new_n53_));
  nand3  g08(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  aoi21  g10(.a(new_n54_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(x23), .c(x24), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n49_), .O(new_n58_));
  inv1   g13(.a(x05), .O(new_n59_));
  inv1   g14(.a(x15), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(x04), .c(x07), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n58_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nand2  g19(.a(new_n47_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n57_), .b(new_n47_), .c(new_n49_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  nand2  g22(.a(new_n47_), .b(new_n46_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  oai21  g32(.a(new_n77_), .b(x08), .c(new_n47_), .O(z06));
  inv1   g33(.a(new_n47_), .O(new_n79_));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n79_), .O(z07));
  inv1   g35(.a(x07), .O(new_n81_));
  nand4  g36(.a(new_n60_), .b(new_n81_), .c(x05), .d(x04), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  aoi21  g38(.a(x05), .b(x04), .c(x07), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n60_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n51_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand2  g43(.a(new_n52_), .b(new_n51_), .O(new_n89_));
  nor2   g44(.a(new_n52_), .b(new_n51_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n86_), .c(new_n89_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n47_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n90_), .b(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n85_), .O(z12));
  inv1   g52(.a(x04), .O(new_n98_));
  nand2  g53(.a(x15), .b(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(x20), .O(new_n100_));
  nand4  g55(.a(new_n50_), .b(x19), .c(x18), .d(x17), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n84_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n60_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n99_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n53_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n90_), .c(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n86_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand4  g65(.a(new_n107_), .b(new_n90_), .c(new_n110_), .d(new_n106_), .O(new_n111_));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n86_), .O(z15));
  nand2  g68(.a(new_n111_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n107_), .c(new_n90_), .d(new_n106_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n86_), .O(z16));
  nor2   g72(.a(x24), .b(x21), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n101_), .c(new_n84_), .O(new_n120_));
  aoi21  g75(.a(new_n116_), .b(x24), .c(new_n120_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(x15), .c(new_n99_), .O(z17));
  nor2   g77(.a(new_n119_), .b(new_n101_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n49_), .O(new_n124_));
  nand4  g79(.a(new_n118_), .b(new_n115_), .c(new_n107_), .d(new_n90_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(x25), .c(new_n85_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n124_), .c(new_n79_), .O(z18));
endmodule


