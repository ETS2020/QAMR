// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n105_, new_n106_, new_n110_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x22), .b(x15), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  aoi21  g04(.a(x05), .b(x04), .c(x07), .O(new_n50_));
  oai21  g05(.a(x25), .b(x24), .c(new_n50_), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  inv1   g07(.a(x23), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n54_), .c(x20), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n56_), .c(new_n53_), .O(new_n58_));
  nor2   g13(.a(new_n47_), .b(x25), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n58_), .c(new_n52_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  nor2   g17(.a(new_n47_), .b(new_n62_), .O(z02));
  aoi21  g18(.a(new_n58_), .b(x24), .c(new_n60_), .O(z03));
  nand2  g19(.a(new_n49_), .b(new_n46_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  oai21  g27(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g28(.a(x14), .O(new_n74_));
  nor2   g29(.a(new_n65_), .b(new_n74_), .O(z06));
  aoi21  g30(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g31(.a(x24), .O(new_n77_));
  inv1   g32(.a(x20), .O(new_n78_));
  inv1   g33(.a(x17), .O(new_n79_));
  inv1   g34(.a(x18), .O(new_n80_));
  nand3  g35(.a(new_n54_), .b(new_n80_), .c(new_n79_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  inv1   g37(.a(new_n57_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n82_), .c(x23), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n77_), .c(new_n59_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(x05), .c(x04), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(x22), .c(x15), .O(z09));
  inv1   g43(.a(x15), .O(new_n89_));
  nand3  g44(.a(new_n50_), .b(x22), .c(new_n89_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x17), .O(z10));
  nor2   g46(.a(new_n80_), .b(new_n79_), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n90_), .c(new_n55_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor2   g50(.a(new_n92_), .b(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n90_), .O(z12));
  nand2  g52(.a(new_n94_), .b(x20), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(new_n78_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n50_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(x22), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n89_), .O(z13));
  aoi21  g57(.a(new_n99_), .b(x21), .c(new_n90_), .O(new_n103_));
  oai21  g58(.a(new_n99_), .b(x21), .c(new_n103_), .O(z14));
  inv1   g59(.a(new_n90_), .O(new_n105_));
  nor2   g60(.a(new_n99_), .b(x21), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n105_), .O(z15));
  nand2  g62(.a(new_n105_), .b(new_n53_), .O(z16));
  nand2  g63(.a(new_n105_), .b(new_n77_), .O(z17));
  inv1   g64(.a(x25), .O(new_n110_));
  nand2  g65(.a(new_n105_), .b(new_n110_), .O(z18));
endmodule


