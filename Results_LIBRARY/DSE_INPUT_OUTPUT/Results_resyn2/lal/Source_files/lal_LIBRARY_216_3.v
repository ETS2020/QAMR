// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n86_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x11), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n61_), .b(new_n59_), .c(x23), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(new_n48_), .c(new_n53_), .O(new_n63_));
  inv1   g18(.a(x07), .O(new_n64_));
  nand2  g19(.a(x05), .b(x04), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(new_n50_), .c(new_n64_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n63_), .O(z01));
  nor2   g23(.a(new_n49_), .b(new_n46_), .O(z02));
  nand2  g24(.a(new_n63_), .b(new_n50_), .O(z03));
  nor2   g25(.a(x08), .b(x02), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(x24), .c(x11), .O(new_n72_));
  inv1   g27(.a(x08), .O(new_n73_));
  inv1   g28(.a(x09), .O(new_n74_));
  inv1   g29(.a(x12), .O(new_n75_));
  aoi22  g30(.a(new_n75_), .b(x03), .c(new_n74_), .d(x00), .O(new_n76_));
  oai21  g31(.a(new_n74_), .b(x00), .c(new_n76_), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  inv1   g33(.a(x03), .O(new_n79_));
  nand2  g34(.a(x12), .b(new_n79_), .O(new_n80_));
  nand2  g35(.a(new_n47_), .b(x02), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n77_), .c(new_n73_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n72_), .O(z04));
  oai21  g39(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g40(.a(x14), .b(new_n73_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n50_), .O(z06));
  nand3  g42(.a(new_n50_), .b(new_n73_), .c(x06), .O(z07));
  inv1   g43(.a(x23), .O(new_n89_));
  nor2   g44(.a(x18), .b(x17), .O(new_n90_));
  aoi21  g45(.a(new_n90_), .b(new_n57_), .c(x20), .O(new_n91_));
  oai21  g46(.a(new_n60_), .b(new_n91_), .c(new_n89_), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(x11), .c(x24), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n53_), .O(z08));
  inv1   g49(.a(x15), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n64_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n65_), .c(new_n50_), .O(z09));
  nand3  g52(.a(new_n65_), .b(new_n95_), .c(new_n64_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n50_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n55_), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(z10));
  nand2  g58(.a(x18), .b(x17), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n100_), .c(new_n90_), .O(z11));
  nand3  g61(.a(x19), .b(x18), .c(x17), .O(new_n107_));
  nand2  g62(.a(new_n104_), .b(new_n57_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n99_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n50_), .O(z12));
  nor2   g65(.a(new_n107_), .b(x20), .O(new_n111_));
  inv1   g66(.a(new_n111_), .O(new_n112_));
  nand2  g67(.a(new_n107_), .b(x20), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n101_), .O(z13));
  nand2  g69(.a(new_n112_), .b(x21), .O(new_n115_));
  inv1   g70(.a(x21), .O(new_n116_));
  nand2  g71(.a(new_n111_), .b(new_n116_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n101_), .O(z14));
  nand2  g73(.a(new_n117_), .b(x22), .O(new_n119_));
  nor2   g74(.a(x22), .b(x21), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n111_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n101_), .O(z15));
  nand2  g77(.a(new_n121_), .b(x23), .O(new_n123_));
  nand3  g78(.a(new_n120_), .b(new_n111_), .c(new_n89_), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n101_), .O(z16));
  nand3  g80(.a(new_n124_), .b(x24), .c(new_n47_), .O(new_n126_));
  nand2  g81(.a(new_n98_), .b(new_n50_), .O(new_n127_));
  nand4  g82(.a(new_n120_), .b(new_n111_), .c(new_n48_), .d(new_n89_), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(z17));
  inv1   g84(.a(new_n128_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n53_), .O(new_n131_));
  oai21  g86(.a(new_n49_), .b(x25), .c(new_n128_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n131_), .c(new_n99_), .O(z18));
endmodule


