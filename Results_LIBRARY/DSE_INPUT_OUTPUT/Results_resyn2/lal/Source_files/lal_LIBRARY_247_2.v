// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n89_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x02), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x07), .O(new_n51_));
  nand2  g06(.a(x05), .b(x04), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  and2   g15(.a(x22), .b(x21), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  inv1   g17(.a(x24), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x23), .O(new_n64_));
  nor2   g19(.a(x24), .b(x02), .O(new_n65_));
  aoi21  g20(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nand2  g23(.a(new_n49_), .b(new_n68_), .O(z02));
  inv1   g24(.a(x23), .O(new_n70_));
  nand2  g25(.a(new_n62_), .b(new_n70_), .O(new_n71_));
  aoi22  g26(.a(new_n71_), .b(x24), .c(new_n49_), .d(x25), .O(z03));
  nand2  g27(.a(new_n49_), .b(new_n46_), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  inv1   g29(.a(x03), .O(new_n75_));
  nand2  g30(.a(x12), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x09), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x00), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  xnor2a g34(.a(x11), .b(x02), .O(new_n80_));
  inv1   g35(.a(x00), .O(new_n81_));
  nand2  g36(.a(x09), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(x12), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x03), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n82_), .c(new_n80_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  nor2   g41(.a(new_n86_), .b(new_n73_), .O(z04));
  nor2   g42(.a(new_n73_), .b(x13), .O(z05));
  nand2  g43(.a(x14), .b(new_n46_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n49_), .O(z06));
  aoi21  g45(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  nand2  g46(.a(new_n71_), .b(x24), .O(new_n92_));
  nor2   g47(.a(new_n48_), .b(x25), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(z08));
  inv1   g49(.a(x15), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n51_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n52_), .c(new_n49_), .O(z09));
  nand3  g52(.a(new_n52_), .b(new_n95_), .c(new_n51_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g54(.a(new_n57_), .b(new_n56_), .O(new_n100_));
  nor2   g55(.a(new_n98_), .b(new_n48_), .O(new_n101_));
  nand2  g56(.a(x18), .b(x17), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(z11));
  nand3  g59(.a(x19), .b(x18), .c(x17), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  aoi21  g61(.a(x18), .b(x17), .c(x19), .O(new_n107_));
  nor4   g62(.a(new_n107_), .b(new_n106_), .c(new_n98_), .d(new_n48_), .O(z12));
  nand2  g63(.a(new_n106_), .b(new_n55_), .O(new_n109_));
  nand2  g64(.a(new_n105_), .b(x20), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n101_), .O(z13));
  nand2  g66(.a(new_n109_), .b(x21), .O(new_n112_));
  nor2   g67(.a(x21), .b(x20), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  inv1   g69(.a(new_n114_), .O(new_n115_));
  nor2   g70(.a(new_n115_), .b(new_n98_), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n112_), .c(new_n48_), .O(z14));
  nand2  g72(.a(new_n114_), .b(x22), .O(new_n118_));
  inv1   g73(.a(x22), .O(new_n119_));
  aoi21  g74(.a(new_n115_), .b(new_n119_), .c(new_n98_), .O(new_n120_));
  aoi21  g75(.a(new_n120_), .b(new_n118_), .c(new_n48_), .O(z15));
  oai21  g76(.a(new_n114_), .b(x22), .c(x23), .O(new_n122_));
  nor2   g77(.a(x23), .b(x22), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n113_), .c(new_n106_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  nor2   g80(.a(new_n125_), .b(new_n98_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(new_n122_), .c(new_n48_), .O(z16));
  nand2  g82(.a(new_n98_), .b(new_n49_), .O(new_n128_));
  nand2  g83(.a(new_n125_), .b(new_n65_), .O(new_n129_));
  nand2  g84(.a(new_n124_), .b(x24), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(z17));
  nand2  g86(.a(new_n124_), .b(x25), .O(new_n132_));
  nor3   g87(.a(x25), .b(x24), .c(x23), .O(new_n133_));
  nand4  g88(.a(new_n133_), .b(new_n113_), .c(new_n106_), .d(new_n119_), .O(new_n134_));
  nand2  g89(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g90(.a(new_n135_), .b(new_n47_), .O(new_n136_));
  nand2  g91(.a(x25), .b(x24), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n136_), .c(new_n128_), .O(z18));
endmodule


