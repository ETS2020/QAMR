// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x23), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x23), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g16(.a(x25), .b(new_n59_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(new_n48_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n61_), .b(new_n58_), .c(new_n66_), .O(z01));
  aoi21  g22(.a(new_n58_), .b(x24), .c(new_n61_), .O(z03));
  nand2  g23(.a(new_n49_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n69_), .b(new_n78_), .O(z06));
  nand3  g34(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g35(.a(new_n56_), .b(new_n52_), .O(new_n81_));
  nand4  g36(.a(new_n81_), .b(x24), .c(x22), .d(x21), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n60_), .c(new_n59_), .O(z08));
  nor2   g38(.a(x15), .b(x07), .O(new_n84_));
  nand4  g39(.a(new_n84_), .b(new_n49_), .c(x05), .d(x04), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z09));
  nand2  g41(.a(new_n84_), .b(new_n64_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n49_), .O(z10));
  xor2a  g43(.a(x18), .b(x17), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(new_n84_), .c(new_n64_), .d(new_n49_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z11));
  inv1   g46(.a(new_n87_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  oai21  g48(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n49_), .O(z12));
  and2   g51(.a(new_n84_), .b(new_n49_), .O(new_n97_));
  inv1   g52(.a(new_n93_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n52_), .O(new_n99_));
  nand2  g54(.a(new_n93_), .b(x20), .O(new_n100_));
  nand4  g55(.a(new_n100_), .b(new_n99_), .c(new_n97_), .d(new_n64_), .O(z13));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand4  g59(.a(new_n104_), .b(new_n102_), .c(new_n97_), .d(new_n64_), .O(z14));
  nand2  g60(.a(new_n104_), .b(x22), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  nand3  g62(.a(new_n103_), .b(new_n98_), .c(new_n107_), .O(new_n108_));
  nand4  g63(.a(new_n108_), .b(new_n106_), .c(new_n97_), .d(new_n64_), .O(z15));
  nand2  g64(.a(new_n87_), .b(new_n49_), .O(new_n110_));
  nand3  g65(.a(new_n108_), .b(x24), .c(x23), .O(new_n111_));
  nand4  g66(.a(new_n103_), .b(new_n98_), .c(new_n59_), .d(new_n107_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z16));
  nand2  g68(.a(new_n108_), .b(x24), .O(new_n114_));
  nand4  g69(.a(new_n103_), .b(new_n98_), .c(new_n48_), .d(new_n107_), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n114_), .c(new_n92_), .d(new_n59_), .O(z17));
  nand2  g71(.a(new_n115_), .b(x25), .O(new_n117_));
  nor2   g72(.a(x25), .b(x24), .O(new_n118_));
  nand4  g73(.a(new_n118_), .b(new_n103_), .c(new_n98_), .d(new_n107_), .O(new_n119_));
  aoi21  g74(.a(new_n60_), .b(x24), .c(new_n59_), .O(new_n120_));
  nor2   g75(.a(new_n120_), .b(new_n87_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(z18));
endmodule


