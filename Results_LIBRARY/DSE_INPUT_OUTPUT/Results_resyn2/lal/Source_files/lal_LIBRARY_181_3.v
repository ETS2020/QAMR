// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x23), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  nor3   g06(.a(x19), .b(x18), .c(x17), .O(new_n52_));
  inv1   g07(.a(x21), .O(new_n53_));
  inv1   g08(.a(x22), .O(new_n54_));
  nor2   g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g10(.a(new_n52_), .b(x20), .c(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  oai21  g16(.a(new_n57_), .b(x25), .c(new_n61_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g18(.a(new_n57_), .b(x25), .O(z03));
  xnor2a g19(.a(x12), .b(x03), .O(new_n65_));
  xnor2a g20(.a(x11), .b(x02), .O(new_n66_));
  xnor2a g21(.a(x09), .b(x00), .O(new_n67_));
  xnor2a g22(.a(x10), .b(x01), .O(new_n68_));
  nand4  g23(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n69_));
  inv1   g24(.a(new_n69_), .O(new_n70_));
  oai21  g25(.a(new_n70_), .b(x08), .c(new_n47_), .O(z04));
  inv1   g26(.a(x13), .O(new_n72_));
  inv1   g27(.a(new_n47_), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z05));
  nand2  g31(.a(new_n74_), .b(x14), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z06));
  nand2  g33(.a(new_n74_), .b(x06), .O(z07));
  inv1   g34(.a(x25), .O(new_n80_));
  inv1   g35(.a(x20), .O(new_n81_));
  inv1   g36(.a(new_n52_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g38(.a(new_n55_), .b(new_n83_), .c(x24), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n80_), .c(new_n73_), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  nand3  g41(.a(new_n47_), .b(new_n86_), .c(new_n58_), .O(new_n87_));
  nor2   g42(.a(new_n87_), .b(new_n59_), .O(z09));
  nand3  g43(.a(new_n59_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n47_), .O(z10));
  nor2   g45(.a(x18), .b(x17), .O(new_n91_));
  inv1   g46(.a(new_n87_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n59_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  nor3   g50(.a(new_n95_), .b(new_n93_), .c(new_n91_), .O(z11));
  inv1   g51(.a(new_n89_), .O(new_n97_));
  nand3  g52(.a(x19), .b(x18), .c(x17), .O(new_n98_));
  inv1   g53(.a(x19), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(new_n99_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n47_), .O(z12));
  nor2   g57(.a(new_n98_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n98_), .b(x20), .c(new_n89_), .O(new_n105_));
  aoi21  g60(.a(new_n105_), .b(new_n104_), .c(new_n73_), .O(z13));
  nand2  g61(.a(new_n104_), .b(x21), .O(new_n107_));
  aoi21  g62(.a(new_n103_), .b(new_n53_), .c(new_n89_), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n107_), .c(new_n73_), .O(z14));
  inv1   g64(.a(new_n93_), .O(new_n110_));
  oai21  g65(.a(new_n104_), .b(x21), .c(x22), .O(new_n111_));
  nand3  g66(.a(new_n103_), .b(new_n54_), .c(new_n53_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(z15));
  nand4  g68(.a(new_n103_), .b(new_n51_), .c(new_n54_), .d(new_n53_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x23), .O(new_n115_));
  nand4  g70(.a(new_n103_), .b(new_n50_), .c(new_n54_), .d(new_n53_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n97_), .O(z16));
  nand2  g72(.a(new_n89_), .b(new_n47_), .O(new_n118_));
  xor2a  g73(.a(new_n112_), .b(x24), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(x23), .c(new_n118_), .O(z17));
  inv1   g75(.a(new_n116_), .O(new_n121_));
  nor2   g76(.a(x25), .b(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  oai21  g78(.a(new_n122_), .b(new_n50_), .c(new_n97_), .O(new_n124_));
  aoi21  g79(.a(new_n114_), .b(x25), .c(new_n124_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n123_), .O(z18));
endmodule


