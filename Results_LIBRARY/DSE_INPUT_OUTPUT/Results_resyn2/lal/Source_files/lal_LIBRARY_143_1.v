// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x09), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x05), .O(new_n48_));
  nor3   g03(.a(new_n48_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(x22), .b(x21), .O(new_n56_));
  aoi21  g11(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(x23), .c(x24), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  and2   g14(.a(x05), .b(x04), .O(new_n60_));
  nor3   g15(.a(new_n60_), .b(new_n48_), .c(x07), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nor2   g18(.a(new_n48_), .b(new_n63_), .O(z02));
  inv1   g19(.a(new_n48_), .O(new_n65_));
  nand3  g20(.a(new_n58_), .b(new_n65_), .c(new_n50_), .O(z08));
  inv1   g21(.a(z08), .O(z03));
  oai21  g22(.a(x08), .b(x00), .c(x05), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(x09), .O(new_n69_));
  nand2  g24(.a(new_n47_), .b(x00), .O(new_n70_));
  inv1   g25(.a(x03), .O(new_n71_));
  inv1   g26(.a(x10), .O(new_n72_));
  aoi22  g27(.a(x12), .b(new_n71_), .c(new_n72_), .d(x01), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  inv1   g29(.a(x01), .O(new_n75_));
  nand2  g30(.a(x10), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(x12), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x03), .O(new_n78_));
  xnor2a g33(.a(x11), .b(x02), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  oai21  g35(.a(new_n80_), .b(new_n74_), .c(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n69_), .O(z04));
  oai21  g37(.a(x13), .b(x08), .c(new_n65_), .O(z05));
  nand2  g38(.a(x14), .b(new_n46_), .O(new_n84_));
  nor2   g39(.a(new_n84_), .b(new_n48_), .O(z06));
  nand3  g40(.a(new_n65_), .b(new_n46_), .c(x06), .O(z07));
  nor2   g41(.a(x15), .b(x07), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n60_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(z09));
  inv1   g44(.a(x15), .O(new_n90_));
  nand2  g45(.a(new_n61_), .b(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(x17), .O(z10));
  nand2  g47(.a(new_n53_), .b(new_n52_), .O(new_n93_));
  nand2  g48(.a(x18), .b(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g50(.a(new_n95_), .b(new_n91_), .O(z11));
  inv1   g51(.a(new_n94_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(x19), .O(new_n98_));
  nand2  g53(.a(new_n94_), .b(new_n54_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g55(.a(new_n100_), .b(new_n91_), .O(z12));
  inv1   g56(.a(new_n91_), .O(new_n102_));
  nand4  g57(.a(new_n51_), .b(x19), .c(x18), .d(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  aoi21  g59(.a(new_n98_), .b(x20), .c(new_n104_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n102_), .O(z13));
  inv1   g61(.a(x21), .O(new_n107_));
  nand2  g62(.a(new_n104_), .b(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n103_), .b(x21), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n102_), .O(z14));
  inv1   g65(.a(x22), .O(new_n111_));
  nor2   g66(.a(x20), .b(new_n54_), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n97_), .c(new_n111_), .d(new_n107_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n102_), .O(z15));
  inv1   g70(.a(new_n60_), .O(new_n116_));
  nand2  g71(.a(new_n113_), .b(x23), .O(new_n117_));
  inv1   g72(.a(x23), .O(new_n118_));
  nand4  g73(.a(new_n104_), .b(new_n118_), .c(new_n111_), .d(new_n107_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n87_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n65_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n116_), .O(z16));
  nand2  g77(.a(new_n119_), .b(x24), .O(new_n123_));
  inv1   g78(.a(x24), .O(new_n124_));
  nand4  g79(.a(new_n124_), .b(new_n118_), .c(new_n111_), .d(new_n107_), .O(new_n125_));
  inv1   g80(.a(new_n125_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n104_), .O(new_n127_));
  nand3  g82(.a(new_n127_), .b(new_n123_), .c(new_n102_), .O(z17));
  nand3  g83(.a(new_n126_), .b(new_n104_), .c(new_n50_), .O(new_n129_));
  oai21  g84(.a(new_n125_), .b(new_n103_), .c(x25), .O(new_n130_));
  nand3  g85(.a(new_n130_), .b(new_n129_), .c(new_n87_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n65_), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n116_), .O(z18));
endmodule


