// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n84_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x20), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x09), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n46_), .O(z02));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n47_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  nand2  g15(.a(x05), .b(x04), .O(new_n61_));
  inv1   g16(.a(new_n61_), .O(new_n62_));
  nor3   g17(.a(new_n62_), .b(new_n48_), .c(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n60_), .O(z01));
  inv1   g19(.a(new_n48_), .O(new_n65_));
  nand3  g20(.a(new_n59_), .b(new_n65_), .c(new_n52_), .O(new_n66_));
  inv1   g21(.a(new_n66_), .O(z03));
  inv1   g22(.a(new_n50_), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  inv1   g24(.a(x03), .O(new_n70_));
  nand2  g25(.a(x12), .b(new_n70_), .O(new_n71_));
  inv1   g26(.a(x10), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(x01), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(new_n71_), .c(new_n69_), .O(new_n74_));
  xnor2a g29(.a(x09), .b(x00), .O(new_n75_));
  inv1   g30(.a(x01), .O(new_n76_));
  nand2  g31(.a(x10), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(x12), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(x03), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  nor2   g35(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(new_n68_), .O(z04));
  nor2   g37(.a(new_n68_), .b(x13), .O(z05));
  inv1   g38(.a(x14), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x08), .c(new_n65_), .O(z06));
  nand2  g40(.a(new_n50_), .b(x06), .O(z07));
  aoi21  g41(.a(new_n59_), .b(new_n52_), .c(new_n48_), .O(z08));
  nor3   g42(.a(new_n48_), .b(x15), .c(x07), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n62_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  inv1   g45(.a(x07), .O(new_n91_));
  inv1   g46(.a(x15), .O(new_n92_));
  nand3  g47(.a(new_n61_), .b(new_n92_), .c(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(x17), .c(new_n65_), .O(z10));
  nand2  g49(.a(new_n88_), .b(new_n61_), .O(new_n95_));
  nand2  g50(.a(new_n54_), .b(new_n53_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g53(.a(new_n98_), .b(new_n95_), .O(z11));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(new_n55_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g57(.a(new_n102_), .b(new_n95_), .O(z12));
  inv1   g58(.a(new_n93_), .O(new_n104_));
  inv1   g59(.a(new_n100_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand2  g61(.a(new_n100_), .b(x20), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n106_), .c(new_n104_), .d(new_n65_), .O(z13));
  inv1   g63(.a(x21), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n109_), .c(new_n47_), .O(new_n110_));
  nand2  g65(.a(new_n106_), .b(x21), .O(new_n111_));
  nand4  g66(.a(new_n111_), .b(new_n110_), .c(new_n104_), .d(new_n65_), .O(z14));
  or2    g67(.a(new_n110_), .b(x22), .O(new_n113_));
  aoi21  g68(.a(new_n110_), .b(x22), .c(new_n93_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(new_n48_), .O(z15));
  inv1   g70(.a(x22), .O(new_n116_));
  inv1   g71(.a(x23), .O(new_n117_));
  nor2   g72(.a(new_n100_), .b(x21), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n116_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n47_), .O(new_n121_));
  nand2  g76(.a(new_n118_), .b(new_n116_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(x23), .O(new_n123_));
  nand2  g78(.a(new_n117_), .b(x09), .O(new_n124_));
  aoi21  g79(.a(new_n124_), .b(x20), .c(new_n93_), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n123_), .c(new_n121_), .O(z16));
  nand2  g81(.a(new_n119_), .b(x24), .O(new_n127_));
  nor3   g82(.a(x24), .b(x23), .c(x22), .O(new_n128_));
  nand2  g83(.a(new_n128_), .b(new_n118_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  nand2  g85(.a(x20), .b(x09), .O(new_n131_));
  oai21  g86(.a(new_n131_), .b(x24), .c(new_n130_), .O(new_n132_));
  nand3  g87(.a(new_n132_), .b(new_n127_), .c(new_n104_), .O(z17));
  nor2   g88(.a(new_n52_), .b(new_n47_), .O(new_n134_));
  oai21  g89(.a(new_n134_), .b(new_n93_), .c(new_n65_), .O(new_n135_));
  nand3  g90(.a(new_n128_), .b(new_n118_), .c(x25), .O(new_n136_));
  nand2  g91(.a(new_n129_), .b(new_n52_), .O(new_n137_));
  nand3  g92(.a(new_n137_), .b(new_n136_), .c(new_n47_), .O(new_n138_));
  nand2  g93(.a(new_n138_), .b(new_n135_), .O(z18));
endmodule


