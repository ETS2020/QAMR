// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x15), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  oai21  g07(.a(x25), .b(x24), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x15), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(x22), .c(x21), .O(new_n62_));
  nand3  g17(.a(new_n62_), .b(new_n48_), .c(new_n55_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x25), .c(new_n54_), .O(z01));
  inv1   g19(.a(x24), .O(new_n65_));
  nand2  g20(.a(new_n49_), .b(new_n65_), .O(new_n66_));
  aoi21  g21(.a(new_n66_), .b(new_n63_), .c(x25), .O(z03));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n46_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n49_), .O(z04));
  nand2  g29(.a(new_n49_), .b(new_n46_), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(x13), .O(z05));
  inv1   g31(.a(x14), .O(new_n77_));
  nor2   g32(.a(new_n75_), .b(new_n77_), .O(z06));
  nand3  g33(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g34(.a(x22), .b(x21), .O(new_n80_));
  aoi21  g35(.a(new_n60_), .b(new_n56_), .c(new_n80_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(x24), .c(x25), .O(new_n82_));
  oai21  g37(.a(x25), .b(x24), .c(x23), .O(new_n83_));
  oai21  g38(.a(new_n82_), .b(x15), .c(new_n83_), .O(z08));
  inv1   g39(.a(x07), .O(new_n85_));
  nand4  g40(.a(new_n55_), .b(new_n85_), .c(x05), .d(x04), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z09));
  nand2  g42(.a(new_n52_), .b(new_n55_), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n57_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z10));
  xnor2a g46(.a(x18), .b(x17), .O(new_n92_));
  oai21  g47(.a(new_n92_), .b(new_n88_), .c(new_n49_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(new_n95_));
  aoi21  g50(.a(x18), .b(x17), .c(x19), .O(new_n96_));
  nor3   g51(.a(new_n96_), .b(new_n95_), .c(new_n88_), .O(z12));
  nand2  g52(.a(x23), .b(x15), .O(new_n98_));
  nand4  g53(.a(new_n56_), .b(x19), .c(x18), .d(x17), .O(new_n99_));
  nand2  g54(.a(new_n94_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n99_), .c(new_n52_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n55_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n98_), .O(z13));
  nand2  g58(.a(new_n99_), .b(x21), .O(new_n104_));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n95_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n89_), .O(z14));
  nand2  g62(.a(new_n106_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n105_), .b(new_n95_), .c(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n52_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n55_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n98_), .O(z15));
  aoi21  g68(.a(new_n110_), .b(x23), .c(new_n88_), .O(new_n114_));
  oai21  g69(.a(new_n110_), .b(x23), .c(new_n114_), .O(z16));
  oai21  g70(.a(x24), .b(x15), .c(x23), .O(new_n116_));
  inv1   g71(.a(x21), .O(new_n117_));
  nand4  g72(.a(new_n65_), .b(new_n48_), .c(new_n109_), .d(new_n117_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n99_), .c(new_n52_), .O(new_n119_));
  aoi21  g74(.a(new_n110_), .b(x24), .c(new_n119_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(x15), .c(new_n116_), .O(z17));
  oai21  g76(.a(x25), .b(x15), .c(x23), .O(new_n122_));
  inv1   g77(.a(x25), .O(new_n123_));
  nor2   g78(.a(new_n94_), .b(x20), .O(new_n124_));
  nor2   g79(.a(x24), .b(x22), .O(new_n125_));
  nor2   g80(.a(x23), .b(x21), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n123_), .O(new_n127_));
  nand2  g82(.a(new_n125_), .b(new_n105_), .O(new_n128_));
  oai21  g83(.a(new_n128_), .b(new_n94_), .c(x25), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n127_), .c(new_n52_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n55_), .O(new_n131_));
  nand2  g86(.a(new_n131_), .b(new_n122_), .O(z18));
endmodule


