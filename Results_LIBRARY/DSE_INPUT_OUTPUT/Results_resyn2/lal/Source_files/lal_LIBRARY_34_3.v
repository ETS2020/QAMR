// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x11), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  oai21  g05(.a(x16), .b(new_n46_), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nor2   g09(.a(x18), .b(x17), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n53_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n52_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n50_), .O(z01));
  inv1   g17(.a(x16), .O(new_n63_));
  nand2  g18(.a(new_n50_), .b(new_n63_), .O(z02));
  nand2  g19(.a(new_n60_), .b(new_n50_), .O(z03));
  nor2   g20(.a(x08), .b(x02), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x15), .c(x11), .O(new_n67_));
  inv1   g22(.a(x09), .O(new_n68_));
  inv1   g23(.a(x12), .O(new_n69_));
  aoi22  g24(.a(new_n69_), .b(x03), .c(new_n68_), .d(x00), .O(new_n70_));
  oai21  g25(.a(new_n68_), .b(x00), .c(new_n70_), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  nand2  g28(.a(x12), .b(new_n73_), .O(new_n74_));
  nand2  g29(.a(new_n47_), .b(x02), .O(new_n75_));
  nand3  g30(.a(new_n75_), .b(new_n74_), .c(new_n72_), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(new_n71_), .c(new_n46_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n67_), .O(z04));
  inv1   g33(.a(x13), .O(new_n79_));
  nor2   g34(.a(new_n49_), .b(x08), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z05));
  nand2  g37(.a(x14), .b(new_n46_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z06));
  nand2  g39(.a(new_n80_), .b(x06), .O(z07));
  nand3  g40(.a(new_n59_), .b(new_n50_), .c(new_n52_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand4  g42(.a(new_n48_), .b(new_n87_), .c(x05), .d(x04), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n50_), .O(z09));
  nand2  g44(.a(new_n61_), .b(new_n48_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n50_), .O(z10));
  and2   g46(.a(x18), .b(x17), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n90_), .c(new_n55_), .O(z11));
  inv1   g48(.a(new_n90_), .O(new_n94_));
  or2    g49(.a(new_n92_), .b(x19), .O(new_n95_));
  nand2  g50(.a(new_n92_), .b(x19), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n94_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n50_), .O(z12));
  nand2  g53(.a(x15), .b(new_n47_), .O(new_n99_));
  nand2  g54(.a(new_n96_), .b(x20), .O(new_n100_));
  nand3  g55(.a(new_n92_), .b(new_n53_), .c(x19), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n100_), .c(new_n61_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n99_), .O(z13));
  nand2  g59(.a(new_n101_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nor2   g61(.a(x20), .b(new_n54_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n92_), .c(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n94_), .O(z14));
  inv1   g64(.a(x22), .O(new_n110_));
  nand4  g65(.a(new_n107_), .b(new_n92_), .c(new_n110_), .d(new_n106_), .O(new_n111_));
  nand2  g66(.a(new_n108_), .b(x22), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n94_), .O(z15));
  nand2  g68(.a(new_n111_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(new_n107_), .c(new_n92_), .d(new_n106_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n61_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n48_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n99_), .O(z16));
  inv1   g74(.a(x24), .O(new_n120_));
  nand3  g75(.a(new_n115_), .b(new_n120_), .c(new_n106_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n101_), .c(new_n61_), .O(new_n122_));
  aoi21  g77(.a(new_n116_), .b(x24), .c(new_n122_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(x15), .c(new_n99_), .O(z17));
  inv1   g79(.a(new_n121_), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n107_), .c(new_n92_), .d(new_n52_), .O(new_n126_));
  nand3  g81(.a(new_n125_), .b(new_n107_), .c(new_n92_), .O(new_n127_));
  aoi21  g82(.a(new_n127_), .b(x25), .c(new_n90_), .O(new_n128_));
  aoi21  g83(.a(new_n128_), .b(new_n126_), .c(new_n49_), .O(z18));
endmodule


