// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n85_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x12), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(x23), .O(new_n49_));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g10(.a(x21), .O(new_n56_));
  inv1   g11(.a(x22), .O(new_n57_));
  nor2   g12(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n50_), .c(new_n49_), .O(new_n60_));
  aoi21  g15(.a(x25), .b(x12), .c(x24), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor3   g18(.a(new_n63_), .b(new_n61_), .c(x07), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n60_), .O(z01));
  inv1   g20(.a(x16), .O(new_n66_));
  nor2   g21(.a(new_n47_), .b(new_n66_), .O(z02));
  aoi21  g22(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n68_));
  inv1   g23(.a(new_n58_), .O(new_n69_));
  inv1   g24(.a(x24), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(x23), .O(new_n71_));
  oai21  g26(.a(new_n69_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  nand2  g27(.a(new_n70_), .b(x12), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n72_), .c(x25), .O(z03));
  inv1   g29(.a(new_n47_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  xor2a  g31(.a(x11), .b(x02), .O(new_n77_));
  xor2a  g32(.a(x09), .b(x00), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  xor2a  g34(.a(x10), .b(x01), .O(new_n80_));
  xor2a  g35(.a(x12), .b(x03), .O(new_n81_));
  nor2   g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(z04));
  nor2   g38(.a(new_n76_), .b(x13), .O(z05));
  inv1   g39(.a(x14), .O(new_n85_));
  nor2   g40(.a(new_n76_), .b(new_n85_), .O(z06));
  nand3  g41(.a(new_n75_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g42(.a(new_n58_), .b(new_n55_), .c(x23), .O(new_n88_));
  nor2   g43(.a(new_n47_), .b(x25), .O(new_n89_));
  oai21  g44(.a(new_n88_), .b(new_n70_), .c(new_n89_), .O(z08));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n63_), .c(new_n75_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  inv1   g48(.a(x17), .O(new_n94_));
  nand3  g49(.a(new_n91_), .b(new_n62_), .c(new_n75_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(z10));
  nand2  g53(.a(new_n91_), .b(new_n62_), .O(new_n99_));
  and2   g54(.a(x18), .b(x17), .O(new_n100_));
  or2    g55(.a(new_n100_), .b(new_n53_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n99_), .c(new_n75_), .O(z11));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  inv1   g58(.a(new_n103_), .O(new_n104_));
  nor2   g59(.a(new_n100_), .b(x19), .O(new_n105_));
  nor3   g60(.a(new_n105_), .b(new_n104_), .c(new_n95_), .O(z12));
  nor2   g61(.a(new_n103_), .b(x20), .O(new_n107_));
  inv1   g62(.a(new_n107_), .O(new_n108_));
  nand2  g63(.a(new_n103_), .b(x20), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n96_), .O(z13));
  nand2  g65(.a(new_n108_), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n107_), .b(new_n56_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n96_), .O(z14));
  nand2  g68(.a(new_n112_), .b(x22), .O(new_n114_));
  nor2   g69(.a(x22), .b(x21), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n107_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n96_), .O(z15));
  nand2  g72(.a(new_n116_), .b(x23), .O(new_n118_));
  nand4  g73(.a(new_n49_), .b(new_n57_), .c(new_n56_), .d(new_n51_), .O(new_n119_));
  inv1   g74(.a(new_n119_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n104_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n118_), .c(new_n96_), .O(z16));
  nand2  g77(.a(new_n121_), .b(x24), .O(new_n123_));
  nand2  g78(.a(new_n99_), .b(new_n75_), .O(new_n124_));
  nand4  g79(.a(new_n120_), .b(new_n104_), .c(new_n70_), .d(x12), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n124_), .c(new_n123_), .O(z17));
  oai21  g81(.a(new_n119_), .b(new_n103_), .c(x25), .O(new_n127_));
  nor3   g82(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  nand3  g83(.a(new_n128_), .b(new_n115_), .c(new_n107_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(x12), .O(new_n131_));
  aoi22  g86(.a(new_n99_), .b(new_n75_), .c(x25), .d(x24), .O(new_n132_));
  nand2  g87(.a(new_n132_), .b(new_n131_), .O(z18));
endmodule


