// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x18), .O(new_n47_));
  nor2   g02(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  oai21  g08(.a(x19), .b(x17), .c(new_n53_), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(new_n56_));
  aoi21  g11(.a(new_n56_), .b(new_n54_), .c(x23), .O(new_n57_));
  oai21  g12(.a(new_n57_), .b(new_n51_), .c(new_n52_), .O(new_n58_));
  nor2   g13(.a(x25), .b(x23), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(x24), .O(new_n61_));
  inv1   g16(.a(x07), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g19(.a(new_n61_), .b(x18), .c(new_n64_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n58_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  nand2  g23(.a(new_n61_), .b(x18), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n58_), .O(z03));
  xnor2a g25(.a(x12), .b(x03), .O(new_n71_));
  xnor2a g26(.a(x11), .b(x02), .O(new_n72_));
  xnor2a g27(.a(x09), .b(x00), .O(new_n73_));
  xnor2a g28(.a(x10), .b(x01), .O(new_n74_));
  nand4  g29(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  nor3   g34(.a(new_n48_), .b(new_n79_), .c(x08), .O(z06));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g36(.a(x17), .O(new_n82_));
  inv1   g37(.a(x19), .O(new_n83_));
  nand3  g38(.a(x24), .b(new_n83_), .c(new_n82_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n55_), .c(new_n52_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  oai21  g41(.a(new_n55_), .b(new_n53_), .c(new_n59_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(x24), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n86_), .O(z08));
  inv1   g44(.a(new_n63_), .O(new_n90_));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n49_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z09));
  nand2  g48(.a(new_n91_), .b(new_n82_), .O(new_n94_));
  nor3   g49(.a(new_n94_), .b(new_n90_), .c(new_n48_), .O(z10));
  nand2  g50(.a(new_n91_), .b(new_n63_), .O(new_n96_));
  xnor2a g51(.a(x18), .b(x17), .O(new_n97_));
  oai21  g52(.a(new_n97_), .b(new_n96_), .c(new_n49_), .O(z11));
  nor2   g53(.a(new_n47_), .b(new_n82_), .O(new_n99_));
  nor2   g54(.a(new_n99_), .b(x19), .O(new_n100_));
  inv1   g55(.a(new_n96_), .O(new_n101_));
  nand2  g56(.a(new_n99_), .b(x19), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  oai21  g58(.a(new_n103_), .b(new_n100_), .c(new_n49_), .O(z12));
  nand4  g59(.a(x24), .b(x19), .c(x18), .d(x17), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(x20), .O(new_n106_));
  nand2  g61(.a(x19), .b(x17), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(x20), .c(x24), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(x18), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n106_), .c(new_n101_), .O(z13));
  nand2  g65(.a(new_n109_), .b(x21), .O(new_n111_));
  nand2  g66(.a(new_n96_), .b(new_n49_), .O(new_n112_));
  inv1   g67(.a(x21), .O(new_n113_));
  inv1   g68(.a(new_n105_), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n53_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n112_), .c(new_n111_), .O(z14));
  nand2  g71(.a(new_n115_), .b(x22), .O(new_n117_));
  inv1   g72(.a(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n113_), .c(new_n53_), .O(new_n119_));
  nor2   g74(.a(new_n119_), .b(new_n107_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n51_), .c(x18), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n117_), .c(new_n101_), .O(z15));
  inv1   g77(.a(x23), .O(new_n123_));
  aoi21  g78(.a(new_n120_), .b(new_n123_), .c(new_n51_), .O(new_n124_));
  nor3   g79(.a(x22), .b(x21), .c(x20), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n114_), .O(new_n126_));
  aoi21  g81(.a(new_n126_), .b(x23), .c(new_n96_), .O(new_n127_));
  oai21  g82(.a(new_n124_), .b(new_n47_), .c(new_n127_), .O(z16));
  nor2   g83(.a(new_n90_), .b(new_n48_), .O(new_n129_));
  nand4  g84(.a(new_n125_), .b(new_n123_), .c(x19), .d(x17), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(x24), .O(new_n131_));
  nand2  g86(.a(x24), .b(new_n47_), .O(new_n132_));
  nand4  g87(.a(new_n132_), .b(new_n131_), .c(new_n129_), .d(new_n91_), .O(z17));
  nand3  g88(.a(new_n129_), .b(new_n91_), .c(new_n52_), .O(z18));
endmodule


