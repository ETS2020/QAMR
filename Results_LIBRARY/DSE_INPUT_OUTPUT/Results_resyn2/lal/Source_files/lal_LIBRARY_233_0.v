// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(new_n49_), .O(new_n51_));
  aoi21  g06(.a(x05), .b(x04), .c(x07), .O(new_n52_));
  or2    g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nor2   g17(.a(new_n48_), .b(x23), .O(new_n63_));
  nor2   g18(.a(x24), .b(new_n47_), .O(new_n64_));
  aoi21  g19(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x25), .c(new_n53_), .O(z01));
  and2   g21(.a(new_n49_), .b(x16), .O(z02));
  nor2   g22(.a(new_n65_), .b(x25), .O(z03));
  nand2  g23(.a(new_n49_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n51_), .O(z07));
  inv1   g35(.a(x25), .O(new_n81_));
  aoi21  g36(.a(new_n58_), .b(new_n54_), .c(new_n60_), .O(new_n82_));
  inv1   g37(.a(x23), .O(new_n83_));
  nand2  g38(.a(new_n81_), .b(new_n83_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n82_), .c(x24), .O(new_n85_));
  oai21  g40(.a(new_n81_), .b(new_n47_), .c(new_n85_), .O(z08));
  inv1   g41(.a(x07), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(x05), .c(x04), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(x24), .c(x15), .O(z09));
  nand3  g44(.a(new_n52_), .b(x24), .c(new_n47_), .O(new_n90_));
  nor2   g45(.a(new_n90_), .b(x17), .O(z10));
  nand2  g46(.a(new_n56_), .b(new_n55_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n52_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(x24), .c(x15), .O(z11));
  nand2  g50(.a(new_n93_), .b(new_n57_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n52_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(x24), .c(x15), .O(z12));
  inv1   g54(.a(new_n90_), .O(new_n100_));
  inv1   g55(.a(new_n97_), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n54_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z13));
  nand2  g59(.a(new_n102_), .b(x21), .O(new_n105_));
  inv1   g60(.a(x21), .O(new_n106_));
  nor2   g61(.a(new_n97_), .b(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n52_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x24), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n47_), .O(z14));
  inv1   g66(.a(x22), .O(new_n112_));
  nand3  g67(.a(new_n107_), .b(new_n112_), .c(new_n106_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n100_), .O(z15));
  nand2  g70(.a(new_n113_), .b(x23), .O(new_n116_));
  inv1   g71(.a(new_n113_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n83_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n100_), .O(z16));
  nand3  g74(.a(new_n117_), .b(new_n100_), .c(new_n83_), .O(z17));
  nand2  g75(.a(new_n52_), .b(new_n81_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x24), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n47_), .O(z18));
endmodule


