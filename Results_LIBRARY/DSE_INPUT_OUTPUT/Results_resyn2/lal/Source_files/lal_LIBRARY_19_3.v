// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n78_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x15), .b(x07), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(new_n47_), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  and2   g15(.a(x05), .b(x04), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x07), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(z01));
  nor2   g18(.a(new_n50_), .b(new_n46_), .O(z02));
  aoi21  g19(.a(new_n59_), .b(new_n51_), .c(new_n50_), .O(z08));
  inv1   g20(.a(z08), .O(z03));
  inv1   g21(.a(x08), .O(new_n67_));
  nand2  g22(.a(new_n47_), .b(new_n67_), .O(new_n68_));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(new_n71_), .c(new_n68_), .O(z04));
  or2    g30(.a(new_n68_), .b(x13), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x08), .c(new_n47_), .O(z06));
  aoi21  g34(.a(new_n67_), .b(x06), .c(new_n50_), .O(z07));
  inv1   g35(.a(x07), .O(new_n81_));
  inv1   g36(.a(x15), .O(new_n82_));
  nand3  g37(.a(new_n61_), .b(new_n82_), .c(new_n81_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z09));
  nand2  g39(.a(new_n62_), .b(new_n82_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n53_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z10));
  nand2  g43(.a(new_n54_), .b(new_n53_), .O(new_n89_));
  nand2  g44(.a(x18), .b(x17), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  oai21  g46(.a(new_n91_), .b(new_n85_), .c(new_n47_), .O(z11));
  nand2  g47(.a(new_n90_), .b(new_n55_), .O(new_n93_));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(new_n85_), .c(new_n47_), .O(z12));
  nor2   g51(.a(new_n94_), .b(x20), .O(new_n97_));
  inv1   g52(.a(new_n97_), .O(new_n98_));
  nand2  g53(.a(new_n94_), .b(x20), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n98_), .c(new_n86_), .O(z13));
  inv1   g55(.a(x21), .O(new_n101_));
  aoi21  g56(.a(new_n97_), .b(new_n101_), .c(new_n85_), .O(new_n102_));
  oai21  g57(.a(new_n97_), .b(new_n101_), .c(new_n102_), .O(z14));
  nand2  g58(.a(new_n82_), .b(x07), .O(new_n104_));
  inv1   g59(.a(x22), .O(new_n105_));
  aoi21  g60(.a(new_n97_), .b(new_n101_), .c(new_n105_), .O(new_n106_));
  nor2   g61(.a(new_n61_), .b(x15), .O(new_n107_));
  nor2   g62(.a(x22), .b(x21), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n97_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n106_), .c(new_n81_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n104_), .O(z15));
  nand2  g67(.a(new_n109_), .b(x23), .O(new_n113_));
  or2    g68(.a(new_n109_), .b(x23), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n86_), .O(z16));
  oai21  g70(.a(new_n109_), .b(x23), .c(x24), .O(new_n116_));
  nor2   g71(.a(x24), .b(x23), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n108_), .c(new_n97_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n116_), .c(new_n86_), .O(z17));
  nand4  g74(.a(new_n117_), .b(new_n108_), .c(new_n97_), .d(new_n51_), .O(new_n120_));
  nand2  g75(.a(new_n118_), .b(x25), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n107_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n81_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n104_), .O(z18));
endmodule


