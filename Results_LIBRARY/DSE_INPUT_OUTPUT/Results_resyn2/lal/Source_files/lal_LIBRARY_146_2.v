// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  oai21  g05(.a(new_n50_), .b(x23), .c(new_n47_), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  inv1   g13(.a(new_n58_), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(x07), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n57_), .c(new_n51_), .O(z01));
  inv1   g16(.a(x16), .O(new_n62_));
  inv1   g17(.a(x23), .O(new_n63_));
  nor2   g18(.a(x24), .b(new_n63_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(new_n62_), .O(z02));
  nand2  g20(.a(new_n57_), .b(new_n51_), .O(z03));
  nand2  g21(.a(new_n48_), .b(new_n46_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  nor2   g29(.a(new_n67_), .b(x13), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n64_), .O(z07));
  inv1   g33(.a(x20), .O(new_n79_));
  inv1   g34(.a(x17), .O(new_n80_));
  inv1   g35(.a(x18), .O(new_n81_));
  nand3  g36(.a(new_n52_), .b(new_n81_), .c(new_n80_), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n79_), .c(new_n55_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n56_), .O(z08));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n64_), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n59_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z09));
  nand3  g46(.a(new_n89_), .b(new_n58_), .c(new_n80_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z10));
  nor2   g48(.a(x15), .b(x07), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n58_), .O(new_n95_));
  inv1   g50(.a(new_n53_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n95_), .c(new_n48_), .O(z11));
  nand3  g54(.a(new_n94_), .b(new_n58_), .c(new_n48_), .O(new_n100_));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  nand2  g56(.a(new_n97_), .b(new_n52_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nor2   g58(.a(new_n103_), .b(new_n100_), .O(z12));
  inv1   g59(.a(new_n101_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n79_), .O(new_n106_));
  aoi21  g61(.a(new_n101_), .b(x20), .c(new_n95_), .O(new_n107_));
  aoi21  g62(.a(new_n107_), .b(new_n106_), .c(new_n64_), .O(z13));
  nand2  g63(.a(new_n106_), .b(x21), .O(new_n109_));
  nor2   g64(.a(x21), .b(x20), .O(new_n110_));
  aoi21  g65(.a(new_n110_), .b(new_n105_), .c(new_n95_), .O(new_n111_));
  aoi21  g66(.a(new_n111_), .b(new_n109_), .c(new_n64_), .O(z14));
  inv1   g67(.a(new_n100_), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(new_n105_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x22), .O(new_n115_));
  nor3   g70(.a(x22), .b(x21), .c(x20), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n105_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(z15));
  nand3  g73(.a(new_n117_), .b(x24), .c(x23), .O(new_n119_));
  nand3  g74(.a(new_n116_), .b(new_n105_), .c(new_n63_), .O(new_n120_));
  nand2  g75(.a(new_n95_), .b(new_n48_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(z16));
  nand4  g77(.a(new_n116_), .b(new_n105_), .c(new_n47_), .d(new_n63_), .O(new_n123_));
  nand2  g78(.a(new_n120_), .b(x24), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n121_), .O(z17));
  nand2  g80(.a(new_n50_), .b(new_n47_), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n116_), .c(new_n105_), .O(new_n127_));
  inv1   g82(.a(x21), .O(new_n128_));
  inv1   g83(.a(x22), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n128_), .c(new_n79_), .O(new_n130_));
  oai21  g85(.a(new_n130_), .b(new_n101_), .c(new_n50_), .O(new_n131_));
  nand3  g86(.a(new_n131_), .b(new_n127_), .c(new_n63_), .O(new_n132_));
  aoi22  g87(.a(new_n95_), .b(new_n48_), .c(x25), .d(x24), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n132_), .O(z18));
endmodule


