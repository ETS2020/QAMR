// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x18), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x18), .O(new_n50_));
  inv1   g05(.a(x24), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x19), .b(x17), .c(new_n52_), .O(new_n53_));
  and2   g08(.a(x22), .b(x21), .O(new_n54_));
  aoi21  g09(.a(new_n54_), .b(new_n53_), .c(x23), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n50_), .c(new_n51_), .O(new_n56_));
  inv1   g11(.a(x07), .O(new_n57_));
  nand2  g12(.a(x05), .b(x04), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  oai21  g15(.a(new_n56_), .b(x25), .c(new_n60_), .O(z01));
  nand2  g16(.a(new_n47_), .b(new_n46_), .O(z02));
  nor2   g17(.a(new_n56_), .b(x25), .O(z03));
  inv1   g18(.a(x08), .O(new_n64_));
  nand2  g19(.a(new_n47_), .b(new_n64_), .O(new_n65_));
  xor2a  g20(.a(x12), .b(x03), .O(new_n66_));
  xor2a  g21(.a(x11), .b(x02), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  xor2a  g23(.a(x09), .b(x00), .O(new_n69_));
  xor2a  g24(.a(x10), .b(x01), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  aoi21  g26(.a(new_n71_), .b(new_n68_), .c(new_n65_), .O(z04));
  or2    g27(.a(new_n65_), .b(x13), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(z05));
  nand3  g29(.a(new_n47_), .b(x14), .c(new_n64_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(z06));
  inv1   g31(.a(new_n47_), .O(new_n77_));
  aoi21  g32(.a(new_n64_), .b(x06), .c(new_n77_), .O(z07));
  inv1   g33(.a(x25), .O(new_n79_));
  inv1   g34(.a(x23), .O(new_n80_));
  nand2  g35(.a(new_n54_), .b(new_n53_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g37(.a(new_n82_), .b(x24), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n79_), .c(new_n77_), .O(z08));
  nor4   g39(.a(new_n58_), .b(new_n77_), .c(x15), .d(x07), .O(z09));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n58_), .O(new_n87_));
  nor3   g42(.a(new_n87_), .b(new_n77_), .c(x17), .O(z10));
  inv1   g43(.a(new_n87_), .O(new_n89_));
  inv1   g44(.a(x17), .O(new_n90_));
  nand2  g45(.a(new_n50_), .b(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n50_), .b(new_n90_), .O(new_n92_));
  nor2   g47(.a(new_n92_), .b(new_n77_), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n91_), .c(new_n89_), .O(new_n94_));
  inv1   g49(.a(new_n94_), .O(z11));
  nor2   g50(.a(new_n92_), .b(x19), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(x19), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n89_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n96_), .c(new_n47_), .O(z12));
  oai21  g54(.a(new_n97_), .b(x24), .c(x20), .O(new_n100_));
  inv1   g55(.a(x19), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x17), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n51_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(x18), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n100_), .c(new_n89_), .O(z13));
  inv1   g61(.a(x21), .O(new_n107_));
  nand4  g62(.a(new_n107_), .b(new_n52_), .c(x19), .d(x17), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n51_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x18), .O(new_n110_));
  nand3  g65(.a(new_n102_), .b(new_n92_), .c(new_n51_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(x21), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n89_), .O(z14));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  nand2  g69(.a(new_n87_), .b(new_n47_), .O(new_n115_));
  nor2   g70(.a(x22), .b(x21), .O(new_n116_));
  nand4  g71(.a(new_n116_), .b(new_n102_), .c(new_n92_), .d(new_n51_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z15));
  nor3   g73(.a(x23), .b(x22), .c(x21), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n102_), .c(x17), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n51_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(x18), .O(new_n122_));
  aoi21  g77(.a(new_n117_), .b(x23), .c(new_n87_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n122_), .O(z16));
  nand4  g79(.a(new_n119_), .b(new_n92_), .c(new_n52_), .d(x19), .O(new_n125_));
  nand3  g80(.a(new_n125_), .b(new_n89_), .c(new_n51_), .O(z17));
  oai21  g81(.a(new_n120_), .b(x25), .c(new_n51_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x18), .O(new_n128_));
  oai21  g83(.a(new_n117_), .b(x23), .c(x25), .O(new_n129_));
  nand3  g84(.a(new_n129_), .b(new_n128_), .c(new_n89_), .O(z18));
endmodule


