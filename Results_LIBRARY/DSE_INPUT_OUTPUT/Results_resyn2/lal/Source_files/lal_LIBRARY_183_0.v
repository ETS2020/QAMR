// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  nor2   g02(.a(x15), .b(x01), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n61_), .c(new_n48_), .O(z01));
  nand3  g18(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n64_));
  inv1   g19(.a(new_n64_), .O(z03));
  nand2  g20(.a(new_n49_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x10), .b(x01), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor2   g30(.a(new_n66_), .b(new_n75_), .O(z06));
  nand3  g31(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g32(.a(new_n60_), .b(new_n52_), .c(new_n48_), .O(z08));
  nand2  g33(.a(x05), .b(x04), .O(new_n79_));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n80_), .c(x01), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n79_), .O(z09));
  inv1   g38(.a(new_n82_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n79_), .O(new_n85_));
  nor2   g40(.a(new_n85_), .b(x17), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nor2   g42(.a(new_n55_), .b(new_n54_), .O(new_n88_));
  nor3   g43(.a(new_n88_), .b(new_n85_), .c(new_n87_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n88_), .b(x19), .O(new_n92_));
  nor3   g47(.a(new_n92_), .b(new_n91_), .c(new_n85_), .O(z12));
  nand2  g48(.a(new_n91_), .b(new_n53_), .O(new_n94_));
  nand2  g49(.a(new_n90_), .b(x20), .O(new_n95_));
  nand3  g50(.a(new_n95_), .b(new_n94_), .c(new_n62_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(x01), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n81_), .O(z13));
  nand2  g53(.a(new_n94_), .b(x21), .O(new_n99_));
  nor2   g54(.a(x21), .b(x20), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n91_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n62_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(x01), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n81_), .O(z14));
  nand2  g59(.a(new_n101_), .b(x22), .O(new_n105_));
  inv1   g60(.a(x22), .O(new_n106_));
  nor3   g61(.a(new_n90_), .b(x21), .c(x20), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n105_), .c(new_n62_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(x01), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n81_), .O(z15));
  inv1   g66(.a(new_n85_), .O(new_n112_));
  nand2  g67(.a(new_n108_), .b(x23), .O(new_n113_));
  nor2   g68(.a(x23), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n100_), .c(new_n91_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(z16));
  nand2  g71(.a(new_n115_), .b(x24), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand3  g73(.a(new_n114_), .b(new_n107_), .c(new_n118_), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n117_), .c(new_n62_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(x01), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n81_), .O(z17));
  aoi21  g77(.a(new_n119_), .b(x25), .c(new_n85_), .O(new_n123_));
  oai21  g78(.a(new_n119_), .b(x25), .c(new_n123_), .O(z18));
endmodule


