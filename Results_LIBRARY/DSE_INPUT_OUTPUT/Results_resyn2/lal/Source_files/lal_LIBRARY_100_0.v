// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x13), .O(new_n47_));
  nor3   g02(.a(new_n47_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g03(.a(new_n47_), .O(new_n49_));
  nand2  g04(.a(x05), .b(x04), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  oai21  g06(.a(new_n51_), .b(x07), .c(new_n49_), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  inv1   g14(.a(new_n59_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  inv1   g16(.a(x24), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x23), .O(new_n63_));
  inv1   g18(.a(x13), .O(new_n64_));
  nor2   g19(.a(x24), .b(new_n64_), .O(new_n65_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x25), .c(new_n52_), .O(z01));
  inv1   g22(.a(x16), .O(new_n68_));
  nor2   g23(.a(new_n47_), .b(new_n68_), .O(z02));
  nor2   g24(.a(new_n66_), .b(x25), .O(z03));
  nand2  g25(.a(new_n49_), .b(new_n46_), .O(new_n71_));
  xor2a  g26(.a(x12), .b(x03), .O(new_n72_));
  xor2a  g27(.a(x11), .b(x02), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  xor2a  g29(.a(x09), .b(x00), .O(new_n75_));
  xor2a  g30(.a(x10), .b(x01), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g32(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(z04));
  nand3  g33(.a(x24), .b(new_n64_), .c(new_n46_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z06));
  nand3  g37(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  inv1   g38(.a(x25), .O(new_n84_));
  aoi21  g39(.a(new_n57_), .b(new_n53_), .c(new_n59_), .O(new_n85_));
  inv1   g40(.a(x23), .O(new_n86_));
  nand2  g41(.a(new_n84_), .b(new_n86_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n85_), .c(x24), .O(new_n88_));
  oai21  g43(.a(new_n84_), .b(new_n64_), .c(new_n88_), .O(z08));
  nor2   g44(.a(x15), .b(x07), .O(new_n90_));
  nand3  g45(.a(new_n90_), .b(new_n51_), .c(new_n49_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand2  g47(.a(new_n90_), .b(new_n50_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g49(.a(new_n55_), .b(new_n54_), .O(new_n95_));
  nor2   g50(.a(new_n93_), .b(new_n47_), .O(new_n96_));
  nand2  g51(.a(x18), .b(x17), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(z11));
  nand3  g54(.a(x19), .b(x18), .c(x17), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(new_n101_));
  aoi21  g56(.a(x18), .b(x17), .c(x19), .O(new_n102_));
  nor4   g57(.a(new_n102_), .b(new_n101_), .c(new_n93_), .d(new_n47_), .O(z12));
  nor2   g58(.a(new_n100_), .b(x20), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nand2  g60(.a(new_n100_), .b(x20), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n105_), .c(new_n96_), .O(z13));
  nand2  g62(.a(new_n105_), .b(x21), .O(new_n108_));
  inv1   g63(.a(x21), .O(new_n109_));
  nand2  g64(.a(new_n104_), .b(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n96_), .O(z14));
  nand2  g66(.a(new_n110_), .b(x22), .O(new_n112_));
  nor2   g67(.a(x22), .b(x21), .O(new_n113_));
  aoi21  g68(.a(new_n113_), .b(new_n104_), .c(new_n93_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n112_), .c(new_n47_), .O(z15));
  nand2  g70(.a(new_n113_), .b(new_n104_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(x23), .O(new_n117_));
  nand3  g72(.a(new_n113_), .b(new_n104_), .c(new_n86_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n117_), .c(new_n96_), .O(z16));
  nand2  g74(.a(new_n118_), .b(x24), .O(new_n120_));
  nand4  g75(.a(new_n113_), .b(new_n104_), .c(new_n62_), .d(new_n86_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n96_), .O(z17));
  xor2a  g77(.a(new_n121_), .b(x25), .O(new_n123_));
  aoi21  g78(.a(new_n84_), .b(x24), .c(x13), .O(new_n124_));
  nor2   g79(.a(new_n124_), .b(new_n93_), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n123_), .O(z18));
endmodule


