// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x20), .b(x14), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x23), .O(new_n51_));
  nand2  g06(.a(x22), .b(x21), .O(new_n52_));
  nand2  g07(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x24), .O(new_n54_));
  inv1   g09(.a(x07), .O(new_n55_));
  nand2  g10(.a(x05), .b(x04), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g12(.a(new_n54_), .b(new_n50_), .c(new_n57_), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nor2   g14(.a(x18), .b(x17), .O(new_n60_));
  aoi21  g15(.a(new_n60_), .b(new_n59_), .c(x20), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(new_n50_), .c(new_n51_), .d(x14), .O(new_n62_));
  oai21  g17(.a(new_n58_), .b(new_n47_), .c(new_n62_), .O(z01));
  inv1   g18(.a(x16), .O(new_n64_));
  nor2   g19(.a(new_n47_), .b(new_n64_), .O(z02));
  nand3  g20(.a(new_n61_), .b(new_n51_), .c(x14), .O(new_n66_));
  nand2  g21(.a(new_n54_), .b(new_n48_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(new_n66_), .c(x25), .O(z03));
  nand2  g23(.a(new_n48_), .b(new_n46_), .O(new_n69_));
  xor2a  g24(.a(x12), .b(x03), .O(new_n70_));
  xor2a  g25(.a(x11), .b(x02), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  xor2a  g27(.a(x09), .b(x00), .O(new_n73_));
  xor2a  g28(.a(x10), .b(x01), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g30(.a(new_n75_), .b(new_n72_), .c(new_n69_), .O(z04));
  nor2   g31(.a(new_n69_), .b(x13), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n78_), .b(x08), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  inv1   g35(.a(x24), .O(new_n81_));
  inv1   g36(.a(new_n52_), .O(new_n82_));
  inv1   g37(.a(x20), .O(new_n83_));
  inv1   g38(.a(x17), .O(new_n84_));
  inv1   g39(.a(x18), .O(new_n85_));
  nand3  g40(.a(new_n59_), .b(new_n85_), .c(new_n84_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n82_), .c(x23), .O(new_n88_));
  nor2   g43(.a(new_n47_), .b(x25), .O(new_n89_));
  oai21  g44(.a(new_n88_), .b(new_n81_), .c(new_n89_), .O(z08));
  nor2   g45(.a(x15), .b(x07), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(x05), .c(x04), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n48_), .O(z09));
  nand2  g48(.a(new_n91_), .b(new_n56_), .O(new_n94_));
  nor2   g49(.a(new_n94_), .b(new_n47_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n84_), .O(new_n96_));
  inv1   g51(.a(new_n96_), .O(z10));
  inv1   g52(.a(new_n60_), .O(new_n98_));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n95_), .c(new_n98_), .O(new_n100_));
  inv1   g55(.a(new_n100_), .O(z11));
  nand3  g56(.a(x19), .b(x18), .c(x17), .O(new_n102_));
  aoi21  g57(.a(new_n99_), .b(new_n59_), .c(new_n94_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n48_), .O(z12));
  nor2   g60(.a(new_n102_), .b(x20), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  nand2  g62(.a(new_n102_), .b(x20), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n107_), .c(new_n95_), .O(z13));
  inv1   g64(.a(x21), .O(new_n110_));
  inv1   g65(.a(new_n102_), .O(new_n111_));
  nor2   g66(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nor3   g67(.a(new_n102_), .b(x21), .c(x20), .O(new_n113_));
  oai21  g68(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  aoi22  g69(.a(new_n94_), .b(new_n48_), .c(x21), .d(x20), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n114_), .O(z14));
  nor2   g71(.a(new_n113_), .b(x22), .O(new_n117_));
  nand2  g72(.a(x22), .b(new_n110_), .O(new_n118_));
  oai21  g73(.a(new_n118_), .b(new_n102_), .c(x14), .O(new_n119_));
  aoi22  g74(.a(new_n94_), .b(new_n48_), .c(x22), .d(x20), .O(new_n120_));
  oai21  g75(.a(new_n119_), .b(new_n117_), .c(new_n120_), .O(z15));
  inv1   g76(.a(x22), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n110_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n102_), .c(x23), .O(new_n124_));
  nor2   g79(.a(x21), .b(x20), .O(new_n125_));
  nand4  g80(.a(new_n125_), .b(new_n111_), .c(new_n51_), .d(new_n122_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g82(.a(new_n127_), .b(x14), .O(new_n128_));
  aoi22  g83(.a(new_n94_), .b(new_n48_), .c(x23), .d(x20), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(new_n128_), .O(z16));
  nand2  g85(.a(new_n126_), .b(x24), .O(new_n131_));
  nor2   g86(.a(x22), .b(x21), .O(new_n132_));
  nor2   g87(.a(x24), .b(x23), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n132_), .c(new_n106_), .O(new_n134_));
  aoi21  g89(.a(new_n81_), .b(x20), .c(x14), .O(new_n135_));
  nor2   g90(.a(new_n135_), .b(new_n94_), .O(new_n136_));
  nand3  g91(.a(new_n136_), .b(new_n134_), .c(new_n131_), .O(z17));
  xor2a  g92(.a(new_n134_), .b(x25), .O(new_n138_));
  aoi21  g93(.a(new_n50_), .b(x20), .c(x14), .O(new_n139_));
  nor2   g94(.a(new_n139_), .b(new_n94_), .O(new_n140_));
  nand2  g95(.a(new_n140_), .b(new_n138_), .O(z18));
endmodule


