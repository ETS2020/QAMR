// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n76_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x04), .O(new_n47_));
  nor2   g02(.a(x15), .b(new_n47_), .O(z09));
  nor2   g03(.a(z09), .b(new_n46_), .O(z02));
  nor2   g04(.a(z09), .b(x08), .O(new_n50_));
  nor2   g05(.a(new_n50_), .b(z02), .O(z00));
  inv1   g06(.a(z09), .O(new_n52_));
  inv1   g07(.a(x07), .O(new_n53_));
  nand2  g08(.a(x05), .b(x04), .O(new_n54_));
  nand2  g09(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  inv1   g11(.a(x20), .O(new_n57_));
  inv1   g12(.a(x17), .O(new_n58_));
  inv1   g13(.a(x18), .O(new_n59_));
  inv1   g14(.a(x19), .O(new_n60_));
  nand3  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g16(.a(x22), .b(x21), .O(new_n62_));
  aoi21  g17(.a(new_n61_), .b(new_n57_), .c(new_n62_), .O(new_n63_));
  oai21  g18(.a(new_n63_), .b(x23), .c(x24), .O(new_n64_));
  nor2   g19(.a(z09), .b(x25), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n64_), .O(z08));
  nand2  g21(.a(z08), .b(new_n56_), .O(z01));
  inv1   g22(.a(z08), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  and2   g28(.a(new_n73_), .b(new_n50_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n52_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x08), .c(new_n52_), .O(z06));
  nand2  g32(.a(new_n50_), .b(x06), .O(z07));
  nor3   g33(.a(x15), .b(x07), .c(x04), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n58_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z10));
  nand2  g36(.a(new_n59_), .b(new_n58_), .O(new_n82_));
  nand2  g37(.a(x18), .b(x17), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n82_), .c(new_n53_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n47_), .c(x15), .O(z11));
  nand2  g40(.a(new_n83_), .b(new_n60_), .O(new_n86_));
  nand3  g41(.a(x19), .b(x18), .c(x17), .O(new_n87_));
  nand3  g42(.a(new_n87_), .b(new_n86_), .c(new_n53_), .O(new_n88_));
  aoi21  g43(.a(new_n88_), .b(new_n47_), .c(x15), .O(z12));
  inv1   g44(.a(new_n87_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n57_), .O(new_n91_));
  nand2  g46(.a(new_n87_), .b(x20), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n79_), .O(z13));
  inv1   g48(.a(x15), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(x21), .O(new_n95_));
  inv1   g50(.a(x21), .O(new_n96_));
  nor2   g51(.a(new_n87_), .b(x20), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n95_), .c(new_n53_), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n94_), .O(z14));
  nand2  g56(.a(new_n98_), .b(x22), .O(new_n102_));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n97_), .b(new_n103_), .c(new_n96_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n79_), .O(z15));
  nand2  g60(.a(new_n104_), .b(x23), .O(new_n106_));
  inv1   g61(.a(x23), .O(new_n107_));
  inv1   g62(.a(new_n104_), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n106_), .c(new_n79_), .O(z16));
  oai21  g65(.a(new_n104_), .b(x23), .c(x24), .O(new_n111_));
  nor2   g66(.a(x24), .b(x23), .O(new_n112_));
  nand4  g67(.a(new_n112_), .b(new_n97_), .c(new_n103_), .d(new_n96_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n79_), .O(z17));
  inv1   g69(.a(x25), .O(new_n115_));
  nand3  g70(.a(new_n112_), .b(new_n108_), .c(new_n115_), .O(new_n116_));
  nand2  g71(.a(new_n113_), .b(x25), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n79_), .O(z18));
endmodule


