// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n87_, new_n88_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x22), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand3  g05(.a(new_n50_), .b(new_n46_), .c(x08), .O(new_n51_));
  inv1   g06(.a(new_n51_), .O(z00));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  inv1   g14(.a(x25), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n48_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(x24), .O(new_n63_));
  aoi21  g18(.a(new_n60_), .b(new_n63_), .c(x07), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  oai21  g21(.a(new_n61_), .b(new_n59_), .c(new_n66_), .O(z01));
  nor2   g22(.a(new_n49_), .b(new_n46_), .O(z02));
  nand2  g23(.a(new_n57_), .b(new_n53_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(x21), .c(new_n47_), .O(new_n70_));
  oai21  g25(.a(x23), .b(x22), .c(x24), .O(new_n71_));
  oai21  g26(.a(new_n71_), .b(new_n70_), .c(new_n60_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n50_), .O(z03));
  nor2   g28(.a(new_n49_), .b(x08), .O(new_n74_));
  xnor2a g29(.a(x12), .b(x03), .O(new_n75_));
  xnor2a g30(.a(x11), .b(x02), .O(new_n76_));
  xnor2a g31(.a(x09), .b(x00), .O(new_n77_));
  xnor2a g32(.a(x10), .b(x01), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n75_), .O(new_n79_));
  and2   g34(.a(new_n79_), .b(new_n74_), .O(z04));
  inv1   g35(.a(x13), .O(new_n81_));
  nand2  g36(.a(new_n74_), .b(new_n81_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z05));
  inv1   g38(.a(x14), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x08), .c(new_n50_), .O(z06));
  nand2  g40(.a(new_n74_), .b(x06), .O(z07));
  aoi21  g41(.a(new_n59_), .b(x24), .c(x25), .O(new_n87_));
  aoi21  g42(.a(x24), .b(x23), .c(x25), .O(new_n88_));
  oai22  g43(.a(new_n88_), .b(x22), .c(new_n87_), .d(x23), .O(z08));
  nor2   g44(.a(x15), .b(x07), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n62_), .O(z09));
  nand3  g47(.a(new_n90_), .b(new_n62_), .c(new_n50_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n54_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z10));
  nand2  g51(.a(new_n90_), .b(new_n62_), .O(new_n97_));
  nand2  g52(.a(new_n55_), .b(new_n54_), .O(new_n98_));
  nand2  g53(.a(x18), .b(x17), .O(new_n99_));
  nand2  g54(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g55(.a(new_n100_), .b(new_n97_), .c(new_n50_), .O(z11));
  inv1   g56(.a(new_n97_), .O(new_n102_));
  nand3  g57(.a(x19), .b(x18), .c(x17), .O(new_n103_));
  nand2  g58(.a(new_n99_), .b(new_n56_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n102_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n50_), .O(z12));
  inv1   g61(.a(new_n103_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n53_), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(x20), .c(new_n97_), .O(new_n109_));
  aoi21  g64(.a(new_n109_), .b(new_n108_), .c(new_n49_), .O(z13));
  nand2  g65(.a(new_n108_), .b(x21), .O(new_n111_));
  inv1   g66(.a(x21), .O(new_n112_));
  nand3  g67(.a(new_n107_), .b(new_n112_), .c(new_n53_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n111_), .c(new_n94_), .O(z14));
  nand2  g69(.a(new_n113_), .b(x22), .O(new_n115_));
  nor3   g70(.a(new_n103_), .b(x21), .c(x20), .O(new_n116_));
  aoi21  g71(.a(new_n116_), .b(new_n47_), .c(new_n97_), .O(new_n117_));
  aoi21  g72(.a(new_n117_), .b(new_n115_), .c(new_n49_), .O(z15));
  oai21  g73(.a(new_n113_), .b(x22), .c(x23), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand2  g75(.a(new_n116_), .b(new_n120_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n102_), .O(z16));
  nand3  g77(.a(new_n116_), .b(new_n120_), .c(new_n63_), .O(new_n123_));
  nand2  g78(.a(new_n48_), .b(new_n112_), .O(new_n124_));
  oai21  g79(.a(new_n124_), .b(new_n108_), .c(x24), .O(new_n125_));
  nand2  g80(.a(new_n63_), .b(new_n48_), .O(new_n126_));
  nand2  g81(.a(new_n126_), .b(x22), .O(new_n127_));
  nand4  g82(.a(new_n127_), .b(new_n125_), .c(new_n123_), .d(new_n102_), .O(z17));
  nand4  g83(.a(new_n116_), .b(new_n120_), .c(new_n60_), .d(new_n63_), .O(new_n129_));
  oai21  g84(.a(new_n126_), .b(new_n113_), .c(x25), .O(new_n130_));
  nand2  g85(.a(new_n61_), .b(x22), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n130_), .c(new_n129_), .d(new_n102_), .O(z18));
endmodule


