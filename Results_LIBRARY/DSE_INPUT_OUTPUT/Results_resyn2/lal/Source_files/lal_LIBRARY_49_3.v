// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n83_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x07), .O(new_n46_));
  inv1   g01(.a(x15), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n50_));
  aoi21  g05(.a(new_n49_), .b(x16), .c(new_n50_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  and2   g16(.a(x05), .b(x04), .O(new_n62_));
  nor2   g17(.a(new_n62_), .b(x07), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n61_), .O(z01));
  inv1   g19(.a(x16), .O(new_n65_));
  nand2  g20(.a(new_n49_), .b(new_n65_), .O(z02));
  nand3  g21(.a(new_n60_), .b(new_n49_), .c(new_n52_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  inv1   g28(.a(new_n73_), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(x08), .c(new_n49_), .O(z04));
  inv1   g30(.a(x13), .O(new_n76_));
  nand2  g31(.a(new_n50_), .b(new_n76_), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(z05));
  nand2  g33(.a(new_n50_), .b(x14), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z06));
  nand2  g35(.a(new_n50_), .b(x06), .O(z07));
  aoi21  g36(.a(new_n60_), .b(new_n52_), .c(new_n48_), .O(z08));
  nand3  g37(.a(new_n62_), .b(new_n47_), .c(new_n46_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(z09));
  nand2  g39(.a(new_n63_), .b(new_n47_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x17), .c(new_n49_), .O(z10));
  nor2   g41(.a(x18), .b(x17), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  inv1   g43(.a(new_n88_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n85_), .c(new_n87_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(new_n88_), .b(new_n56_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n85_), .c(new_n49_), .O(z12));
  inv1   g49(.a(new_n85_), .O(new_n95_));
  inv1   g50(.a(new_n91_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n53_), .O(new_n97_));
  nand2  g52(.a(new_n91_), .b(x20), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(z13));
  nand2  g54(.a(new_n47_), .b(x07), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(x21), .O(new_n101_));
  nor2   g56(.a(new_n62_), .b(x15), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n96_), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n46_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n100_), .O(z14));
  nand2  g62(.a(new_n104_), .b(x22), .O(new_n108_));
  inv1   g63(.a(x22), .O(new_n109_));
  nand3  g64(.a(new_n103_), .b(new_n96_), .c(new_n109_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n102_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n46_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n100_), .O(z15));
  nand2  g68(.a(new_n110_), .b(x23), .O(new_n114_));
  nor2   g69(.a(x23), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n103_), .c(new_n96_), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n114_), .c(new_n102_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n46_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n100_), .O(z16));
  nand2  g74(.a(new_n116_), .b(x24), .O(new_n120_));
  inv1   g75(.a(x24), .O(new_n121_));
  nand4  g76(.a(new_n115_), .b(new_n103_), .c(new_n96_), .d(new_n121_), .O(new_n122_));
  nand3  g77(.a(new_n122_), .b(new_n120_), .c(new_n95_), .O(z17));
  inv1   g78(.a(new_n122_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n52_), .O(new_n125_));
  nand2  g80(.a(new_n122_), .b(x25), .O(new_n126_));
  nand3  g81(.a(new_n126_), .b(new_n125_), .c(new_n95_), .O(z18));
endmodule


