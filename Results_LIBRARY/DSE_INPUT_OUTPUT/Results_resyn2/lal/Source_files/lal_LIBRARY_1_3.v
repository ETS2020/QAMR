// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n79_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x23), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x25), .O(new_n50_));
  inv1   g05(.a(x20), .O(new_n51_));
  inv1   g06(.a(x17), .O(new_n52_));
  inv1   g07(.a(x18), .O(new_n53_));
  inv1   g08(.a(x19), .O(new_n54_));
  nand3  g09(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  inv1   g12(.a(new_n57_), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n56_), .c(x24), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  aoi21  g15(.a(x05), .b(x04), .c(x07), .O(new_n61_));
  nand3  g16(.a(new_n61_), .b(new_n60_), .c(new_n47_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  inv1   g18(.a(x23), .O(new_n64_));
  nor2   g19(.a(x18), .b(x17), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(new_n54_), .c(x20), .O(new_n66_));
  oai21  g21(.a(new_n57_), .b(new_n66_), .c(new_n64_), .O(new_n67_));
  aoi21  g22(.a(new_n67_), .b(x24), .c(x25), .O(z03));
  inv1   g23(.a(x08), .O(new_n69_));
  nand2  g24(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  xor2a  g25(.a(x12), .b(x03), .O(new_n71_));
  xor2a  g26(.a(x11), .b(x02), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  xor2a  g28(.a(x09), .b(x00), .O(new_n74_));
  xor2a  g29(.a(x10), .b(x01), .O(new_n75_));
  nor2   g30(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g31(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z04));
  oai21  g32(.a(x13), .b(x08), .c(new_n47_), .O(z05));
  inv1   g33(.a(x14), .O(new_n79_));
  oai21  g34(.a(new_n79_), .b(x08), .c(new_n47_), .O(z06));
  nand3  g35(.a(new_n47_), .b(new_n69_), .c(x06), .O(z07));
  aoi21  g36(.a(new_n55_), .b(new_n51_), .c(new_n57_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x23), .c(x24), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n50_), .O(z08));
  nand2  g39(.a(x05), .b(x04), .O(new_n85_));
  inv1   g40(.a(x07), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n47_), .b(new_n87_), .c(new_n86_), .O(new_n88_));
  nor2   g43(.a(new_n88_), .b(new_n85_), .O(z09));
  nand2  g44(.a(new_n61_), .b(new_n87_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n47_), .O(z10));
  inv1   g46(.a(new_n65_), .O(new_n92_));
  nand2  g47(.a(x18), .b(x17), .O(new_n93_));
  nand2  g48(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n90_), .c(new_n47_), .O(z11));
  nand2  g50(.a(new_n93_), .b(new_n54_), .O(new_n96_));
  nand3  g51(.a(x19), .b(x18), .c(x17), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  oai21  g53(.a(new_n98_), .b(new_n90_), .c(new_n47_), .O(z12));
  inv1   g54(.a(new_n88_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n85_), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n97_), .b(x20), .O(new_n103_));
  inv1   g58(.a(new_n97_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n51_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(z13));
  nand2  g61(.a(new_n105_), .b(x21), .O(new_n107_));
  nor2   g62(.a(x21), .b(x20), .O(new_n108_));
  aoi21  g63(.a(new_n108_), .b(new_n104_), .c(new_n90_), .O(new_n109_));
  aoi22  g64(.a(new_n109_), .b(new_n107_), .c(x24), .d(x23), .O(z14));
  nor2   g65(.a(new_n97_), .b(x20), .O(new_n111_));
  nor2   g66(.a(x22), .b(x21), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g68(.a(new_n108_), .b(new_n104_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x22), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n113_), .c(new_n102_), .O(z15));
  nand2  g71(.a(new_n90_), .b(new_n47_), .O(new_n117_));
  inv1   g72(.a(x24), .O(new_n118_));
  nand3  g73(.a(new_n113_), .b(new_n118_), .c(x23), .O(new_n119_));
  nand3  g74(.a(new_n112_), .b(new_n111_), .c(new_n64_), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n119_), .c(new_n117_), .O(z16));
  inv1   g76(.a(new_n90_), .O(new_n122_));
  nand4  g77(.a(new_n112_), .b(new_n111_), .c(new_n118_), .d(new_n64_), .O(new_n123_));
  nand2  g78(.a(new_n120_), .b(x24), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n123_), .c(new_n122_), .O(z17));
  nor2   g80(.a(x25), .b(x24), .O(new_n126_));
  nand4  g81(.a(new_n126_), .b(new_n112_), .c(new_n111_), .d(new_n64_), .O(new_n127_));
  inv1   g82(.a(x22), .O(new_n128_));
  nand2  g83(.a(new_n118_), .b(new_n128_), .O(new_n129_));
  oai21  g84(.a(new_n129_), .b(new_n114_), .c(x25), .O(new_n130_));
  oai21  g85(.a(x25), .b(x24), .c(x23), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n130_), .c(new_n127_), .d(new_n122_), .O(z18));
endmodule


