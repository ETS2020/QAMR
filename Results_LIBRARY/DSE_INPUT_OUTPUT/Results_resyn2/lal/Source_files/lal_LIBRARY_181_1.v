// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x23), .O(new_n48_));
  nand2  g03(.a(x24), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(new_n49_), .O(new_n52_));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nor2   g08(.a(x25), .b(x24), .O(z03));
  nor2   g09(.a(z03), .b(x07), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(z01));
  xnor2a g11(.a(x11), .b(x02), .O(new_n57_));
  xnor2a g12(.a(x12), .b(x03), .O(new_n58_));
  xnor2a g13(.a(x09), .b(x00), .O(new_n59_));
  xnor2a g14(.a(x10), .b(x01), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n49_), .O(z04));
  nand2  g18(.a(new_n49_), .b(new_n46_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x13), .O(z05));
  inv1   g20(.a(x14), .O(new_n66_));
  nor2   g21(.a(new_n64_), .b(new_n66_), .O(z06));
  nand3  g22(.a(new_n49_), .b(new_n46_), .c(x06), .O(z07));
  nor2   g23(.a(z03), .b(new_n52_), .O(z08));
  nor2   g24(.a(x15), .b(x07), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n49_), .c(x05), .d(x04), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(z09));
  nand2  g27(.a(new_n70_), .b(new_n53_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x17), .c(new_n49_), .O(z10));
  nand3  g29(.a(new_n70_), .b(new_n53_), .c(new_n49_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  and2   g31(.a(x18), .b(x17), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(new_n78_));
  or2    g33(.a(x18), .b(x17), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  inv1   g35(.a(new_n80_), .O(z11));
  inv1   g36(.a(new_n73_), .O(new_n82_));
  nand3  g37(.a(x19), .b(x18), .c(x17), .O(new_n83_));
  inv1   g38(.a(x19), .O(new_n84_));
  nand2  g39(.a(new_n78_), .b(new_n84_), .O(new_n85_));
  nand3  g40(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n49_), .O(z12));
  nand2  g42(.a(new_n83_), .b(x20), .O(new_n88_));
  inv1   g43(.a(x20), .O(new_n89_));
  nand4  g44(.a(new_n89_), .b(x19), .c(x18), .d(x17), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(new_n91_));
  nor2   g46(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  aoi21  g47(.a(new_n92_), .b(new_n88_), .c(new_n52_), .O(z13));
  nand2  g48(.a(new_n90_), .b(x21), .O(new_n94_));
  inv1   g49(.a(x21), .O(new_n95_));
  nand4  g50(.a(new_n77_), .b(new_n95_), .c(new_n89_), .d(x19), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n94_), .c(new_n82_), .O(new_n97_));
  and2   g52(.a(new_n97_), .b(new_n49_), .O(z14));
  nand2  g53(.a(new_n96_), .b(x22), .O(new_n99_));
  nor2   g54(.a(x22), .b(x21), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n91_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n76_), .O(z15));
  nand3  g57(.a(new_n100_), .b(new_n91_), .c(new_n48_), .O(new_n103_));
  nand2  g58(.a(new_n101_), .b(x23), .O(new_n104_));
  nand3  g59(.a(new_n104_), .b(new_n103_), .c(new_n76_), .O(z16));
  nand2  g60(.a(x24), .b(x23), .O(new_n106_));
  inv1   g61(.a(x24), .O(new_n107_));
  inv1   g62(.a(new_n83_), .O(new_n108_));
  nor2   g63(.a(x21), .b(x20), .O(new_n109_));
  nor2   g64(.a(x23), .b(x22), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n108_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n82_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n107_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n106_), .O(z17));
  inv1   g69(.a(x25), .O(new_n115_));
  nand2  g70(.a(new_n100_), .b(new_n115_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(new_n90_), .c(new_n107_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n48_), .O(new_n118_));
  nand3  g73(.a(new_n100_), .b(new_n107_), .c(new_n48_), .O(new_n119_));
  oai21  g74(.a(new_n119_), .b(new_n90_), .c(x25), .O(new_n120_));
  nand3  g75(.a(new_n120_), .b(new_n118_), .c(new_n82_), .O(z18));
endmodule


