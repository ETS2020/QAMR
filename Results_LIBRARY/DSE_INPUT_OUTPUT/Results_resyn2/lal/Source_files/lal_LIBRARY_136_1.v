// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n123_,
    new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x22), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(new_n58_), .b(new_n54_), .O(new_n59_));
  aoi21  g14(.a(new_n59_), .b(x21), .c(x23), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n48_), .c(new_n53_), .O(new_n61_));
  aoi21  g16(.a(x05), .b(x04), .c(x07), .O(new_n62_));
  and2   g17(.a(new_n62_), .b(new_n50_), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n61_), .O(z01));
  nand2  g19(.a(new_n60_), .b(x22), .O(new_n65_));
  aoi21  g20(.a(new_n65_), .b(x24), .c(x25), .O(z03));
  nand2  g21(.a(new_n50_), .b(new_n46_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  oai21  g29(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  nor2   g31(.a(new_n67_), .b(new_n76_), .O(z06));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g33(.a(x21), .O(new_n79_));
  inv1   g34(.a(x23), .O(new_n80_));
  nor2   g35(.a(x18), .b(x17), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n57_), .c(x20), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(x24), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n53_), .c(new_n49_), .O(z08));
  nor2   g40(.a(x15), .b(x07), .O(new_n86_));
  nand4  g41(.a(new_n86_), .b(new_n50_), .c(x05), .d(x04), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(z09));
  inv1   g43(.a(x15), .O(new_n89_));
  nand2  g44(.a(new_n62_), .b(new_n89_), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x17), .c(new_n50_), .O(z10));
  nor2   g46(.a(new_n90_), .b(new_n49_), .O(new_n92_));
  nor2   g47(.a(new_n56_), .b(new_n55_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(new_n81_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(z11));
  inv1   g51(.a(new_n90_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n50_), .O(new_n98_));
  nand3  g53(.a(x19), .b(x18), .c(x17), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor2   g55(.a(new_n93_), .b(x19), .O(new_n101_));
  nor3   g56(.a(new_n101_), .b(new_n100_), .c(new_n98_), .O(z12));
  nand2  g57(.a(new_n99_), .b(x20), .O(new_n103_));
  nor2   g58(.a(new_n99_), .b(x20), .O(new_n104_));
  inv1   g59(.a(new_n104_), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n103_), .c(new_n92_), .O(z13));
  nand2  g61(.a(new_n105_), .b(x21), .O(new_n107_));
  nor3   g62(.a(new_n99_), .b(x21), .c(x20), .O(new_n108_));
  inv1   g63(.a(new_n108_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n92_), .O(z14));
  nand2  g65(.a(new_n90_), .b(new_n50_), .O(new_n111_));
  inv1   g66(.a(x22), .O(new_n112_));
  nand3  g67(.a(new_n108_), .b(new_n48_), .c(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n109_), .b(x22), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n111_), .O(z15));
  nand4  g70(.a(new_n100_), .b(new_n80_), .c(new_n79_), .d(new_n54_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n48_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n112_), .O(new_n118_));
  nand2  g73(.a(new_n113_), .b(x23), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n118_), .c(new_n97_), .O(z16));
  nand3  g75(.a(new_n108_), .b(new_n80_), .c(new_n112_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n97_), .c(new_n48_), .O(z17));
  nand4  g77(.a(new_n53_), .b(new_n80_), .c(new_n112_), .d(new_n79_), .O(new_n123_));
  inv1   g78(.a(new_n123_), .O(new_n124_));
  aoi22  g79(.a(new_n124_), .b(new_n104_), .c(new_n116_), .d(x25), .O(new_n125_));
  aoi22  g80(.a(new_n90_), .b(new_n50_), .c(x25), .d(x22), .O(new_n126_));
  oai21  g81(.a(new_n125_), .b(x24), .c(new_n126_), .O(z18));
endmodule


