// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n114_;
  inv1   g00(.a(x08), .O(new_n46_));
  nor2   g01(.a(x24), .b(x23), .O(new_n47_));
  inv1   g02(.a(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  aoi21  g04(.a(x25), .b(x23), .c(x24), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(new_n51_));
  inv1   g06(.a(x19), .O(new_n52_));
  nor2   g07(.a(x18), .b(x17), .O(new_n53_));
  aoi21  g08(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nand2  g09(.a(x22), .b(x21), .O(new_n55_));
  nor2   g10(.a(x25), .b(x23), .O(new_n56_));
  oai21  g11(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  aoi21  g12(.a(x05), .b(x04), .c(x07), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n51_), .O(z01));
  inv1   g14(.a(x16), .O(new_n60_));
  nor2   g15(.a(new_n47_), .b(new_n60_), .O(z02));
  nand2  g16(.a(new_n57_), .b(new_n51_), .O(z03));
  nand2  g17(.a(new_n48_), .b(new_n46_), .O(new_n63_));
  xor2a  g18(.a(x12), .b(x03), .O(new_n64_));
  xor2a  g19(.a(x11), .b(x02), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  xor2a  g21(.a(x09), .b(x00), .O(new_n67_));
  xor2a  g22(.a(x10), .b(x01), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g24(.a(new_n69_), .b(new_n66_), .c(new_n63_), .O(z04));
  oai21  g25(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g26(.a(x14), .O(new_n72_));
  nor2   g27(.a(new_n63_), .b(new_n72_), .O(z06));
  aoi21  g28(.a(new_n46_), .b(x06), .c(new_n47_), .O(z07));
  nor3   g29(.a(x19), .b(x18), .c(x17), .O(new_n75_));
  inv1   g30(.a(new_n55_), .O(new_n76_));
  oai21  g31(.a(new_n75_), .b(x20), .c(new_n76_), .O(new_n77_));
  aoi21  g32(.a(new_n56_), .b(new_n77_), .c(new_n50_), .O(z08));
  nand2  g33(.a(x05), .b(x04), .O(new_n79_));
  inv1   g34(.a(x07), .O(new_n80_));
  inv1   g35(.a(x15), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(new_n79_), .c(new_n48_), .O(z09));
  nand3  g38(.a(new_n79_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g40(.a(x18), .b(x17), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(new_n84_), .c(new_n48_), .O(z11));
  nand3  g42(.a(x19), .b(x18), .c(x17), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand2  g44(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nor3   g46(.a(new_n91_), .b(new_n84_), .c(new_n47_), .O(z12));
  inv1   g47(.a(x20), .O(new_n93_));
  inv1   g48(.a(new_n88_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi21  g50(.a(new_n88_), .b(x20), .c(new_n84_), .O(new_n96_));
  aoi21  g51(.a(new_n96_), .b(new_n95_), .c(new_n47_), .O(z13));
  nor2   g52(.a(new_n84_), .b(new_n47_), .O(new_n98_));
  nand2  g53(.a(new_n95_), .b(x21), .O(new_n99_));
  nor2   g54(.a(x21), .b(x20), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n94_), .O(new_n101_));
  nand3  g56(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(z14));
  inv1   g57(.a(x22), .O(new_n103_));
  nand3  g58(.a(new_n100_), .b(new_n94_), .c(new_n103_), .O(new_n104_));
  nand2  g59(.a(new_n101_), .b(x22), .O(new_n105_));
  nand3  g60(.a(new_n105_), .b(new_n104_), .c(new_n98_), .O(z15));
  inv1   g61(.a(x23), .O(new_n107_));
  nand4  g62(.a(new_n100_), .b(new_n94_), .c(new_n107_), .d(new_n103_), .O(new_n108_));
  nand2  g63(.a(new_n104_), .b(x23), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n98_), .O(z16));
  nand2  g65(.a(new_n84_), .b(new_n48_), .O(new_n111_));
  nand2  g66(.a(new_n108_), .b(x24), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n111_), .O(z17));
  nand2  g68(.a(new_n48_), .b(x25), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n111_), .O(z18));
endmodule


