// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n78_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  aoi21  g07(.a(x05), .b(x04), .c(x07), .O(new_n53_));
  or2    g08(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  inv1   g09(.a(x25), .O(new_n55_));
  inv1   g10(.a(x20), .O(new_n56_));
  inv1   g11(.a(x17), .O(new_n57_));
  inv1   g12(.a(x18), .O(new_n58_));
  inv1   g13(.a(x19), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  nand2  g15(.a(x22), .b(x21), .O(new_n61_));
  aoi21  g16(.a(new_n60_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  inv1   g17(.a(x23), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(x15), .O(new_n64_));
  oai21  g19(.a(new_n64_), .b(new_n62_), .c(x24), .O(new_n65_));
  nand2  g20(.a(new_n65_), .b(new_n55_), .O(z08));
  nand2  g21(.a(z08), .b(new_n54_), .O(z01));
  inv1   g22(.a(z08), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n50_), .O(z04));
  nand2  g30(.a(new_n50_), .b(new_n46_), .O(new_n76_));
  nor2   g31(.a(new_n76_), .b(x13), .O(z05));
  inv1   g32(.a(x14), .O(new_n78_));
  nor2   g33(.a(new_n76_), .b(new_n78_), .O(z06));
  aoi21  g34(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  inv1   g35(.a(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x05), .c(x04), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n48_), .c(x15), .O(z09));
  nand2  g38(.a(new_n53_), .b(new_n57_), .O(new_n84_));
  aoi21  g39(.a(new_n84_), .b(new_n48_), .c(x15), .O(z10));
  nor2   g40(.a(x18), .b(x17), .O(new_n86_));
  inv1   g41(.a(x15), .O(new_n87_));
  nand3  g42(.a(new_n53_), .b(new_n48_), .c(new_n87_), .O(new_n88_));
  nor2   g43(.a(new_n58_), .b(new_n57_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(z11));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(new_n92_));
  nor2   g47(.a(new_n89_), .b(x19), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n92_), .c(new_n88_), .O(z12));
  nand2  g49(.a(new_n92_), .b(new_n56_), .O(new_n95_));
  nand2  g50(.a(new_n91_), .b(x20), .O(new_n96_));
  nand3  g51(.a(new_n96_), .b(new_n95_), .c(new_n53_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n48_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n87_), .O(z13));
  nand2  g54(.a(new_n95_), .b(x21), .O(new_n100_));
  nor2   g55(.a(x21), .b(x20), .O(new_n101_));
  nand2  g56(.a(new_n101_), .b(new_n92_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n100_), .c(new_n53_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n87_), .O(z14));
  nand2  g60(.a(new_n102_), .b(x22), .O(new_n106_));
  inv1   g61(.a(x22), .O(new_n107_));
  nand3  g62(.a(new_n101_), .b(new_n92_), .c(new_n107_), .O(new_n108_));
  nand3  g63(.a(new_n108_), .b(new_n106_), .c(new_n53_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n48_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n87_), .O(z15));
  nand2  g66(.a(new_n108_), .b(x23), .O(new_n112_));
  nand4  g67(.a(new_n101_), .b(new_n92_), .c(new_n63_), .d(new_n107_), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n112_), .c(new_n53_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n48_), .O(new_n115_));
  nand2  g70(.a(new_n115_), .b(new_n87_), .O(z16));
  nand2  g71(.a(new_n113_), .b(new_n53_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n48_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n87_), .O(z17));
  aoi21  g74(.a(new_n113_), .b(x25), .c(new_n88_), .O(new_n120_));
  oai21  g75(.a(new_n113_), .b(x25), .c(new_n120_), .O(z18));
endmodule


