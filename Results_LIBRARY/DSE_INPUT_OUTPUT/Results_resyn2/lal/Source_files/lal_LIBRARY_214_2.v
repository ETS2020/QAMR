// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(x23), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x19), .O(new_n50_));
  nor2   g05(.a(x18), .b(x17), .O(new_n51_));
  aoi21  g06(.a(new_n51_), .b(new_n50_), .c(x20), .O(new_n52_));
  nand2  g07(.a(x22), .b(x21), .O(new_n53_));
  nor2   g08(.a(x25), .b(x23), .O(new_n54_));
  oai21  g09(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  inv1   g10(.a(x23), .O(new_n56_));
  aoi21  g11(.a(x25), .b(new_n56_), .c(x24), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n55_), .O(z01));
  and2   g17(.a(new_n48_), .b(x16), .O(z02));
  inv1   g18(.a(new_n57_), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n55_), .O(z03));
  nand2  g20(.a(new_n48_), .b(new_n46_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  oai21  g28(.a(x13), .b(x08), .c(new_n48_), .O(z05));
  inv1   g29(.a(x14), .O(new_n75_));
  nor2   g30(.a(new_n66_), .b(new_n75_), .O(z06));
  inv1   g31(.a(new_n48_), .O(new_n77_));
  aoi21  g32(.a(new_n46_), .b(x06), .c(new_n77_), .O(z07));
  nor3   g33(.a(x19), .b(x18), .c(x17), .O(new_n79_));
  inv1   g34(.a(new_n53_), .O(new_n80_));
  oai21  g35(.a(new_n79_), .b(x20), .c(new_n80_), .O(new_n81_));
  aoi21  g36(.a(new_n54_), .b(new_n81_), .c(new_n57_), .O(z08));
  inv1   g37(.a(x15), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n58_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(new_n59_), .c(new_n48_), .O(z09));
  nand3  g40(.a(new_n59_), .b(new_n83_), .c(new_n58_), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g42(.a(x18), .b(x17), .O(new_n88_));
  oai21  g43(.a(new_n88_), .b(new_n86_), .c(new_n48_), .O(z11));
  nand3  g44(.a(x19), .b(x18), .c(x17), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n50_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n86_), .c(new_n77_), .O(z12));
  inv1   g49(.a(x20), .O(new_n95_));
  inv1   g50(.a(new_n90_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  aoi21  g52(.a(new_n90_), .b(x20), .c(new_n86_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n97_), .c(new_n77_), .O(z13));
  nor2   g54(.a(new_n86_), .b(new_n77_), .O(new_n100_));
  nand2  g55(.a(new_n97_), .b(x21), .O(new_n101_));
  inv1   g56(.a(x21), .O(new_n102_));
  nand3  g57(.a(new_n96_), .b(new_n102_), .c(new_n95_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(z14));
  nand2  g59(.a(new_n103_), .b(x22), .O(new_n105_));
  nor3   g60(.a(x22), .b(x21), .c(x20), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n96_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n100_), .O(z15));
  nand2  g63(.a(new_n107_), .b(x23), .O(new_n109_));
  nand3  g64(.a(new_n106_), .b(new_n96_), .c(new_n56_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n109_), .c(new_n100_), .O(z16));
  nand2  g66(.a(new_n86_), .b(new_n48_), .O(new_n112_));
  nand4  g67(.a(new_n106_), .b(new_n96_), .c(new_n47_), .d(new_n56_), .O(new_n113_));
  nand2  g68(.a(new_n110_), .b(x24), .O(new_n114_));
  nand3  g69(.a(new_n114_), .b(new_n113_), .c(new_n112_), .O(z17));
  inv1   g70(.a(x25), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n47_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n106_), .c(new_n96_), .O(new_n118_));
  inv1   g73(.a(x22), .O(new_n119_));
  nand3  g74(.a(new_n119_), .b(new_n102_), .c(new_n95_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n90_), .c(new_n116_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n118_), .c(new_n56_), .O(new_n122_));
  aoi22  g77(.a(new_n86_), .b(new_n48_), .c(x25), .d(x24), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(new_n122_), .O(z18));
endmodule


