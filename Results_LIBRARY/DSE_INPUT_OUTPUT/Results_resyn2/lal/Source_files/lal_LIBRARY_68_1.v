// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n77_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(x07), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nor2   g06(.a(z02), .b(new_n46_), .O(z00));
  inv1   g07(.a(x07), .O(new_n53_));
  inv1   g08(.a(x25), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  oai21  g16(.a(new_n61_), .b(x23), .c(x24), .O(new_n62_));
  nand2  g17(.a(x05), .b(x04), .O(new_n63_));
  inv1   g18(.a(new_n63_), .O(new_n64_));
  aoi21  g19(.a(new_n62_), .b(new_n54_), .c(new_n64_), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x15), .c(new_n53_), .O(z01));
  nand3  g21(.a(new_n62_), .b(new_n50_), .c(new_n54_), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z03));
  xnor2a g23(.a(x12), .b(x03), .O(new_n69_));
  xnor2a g24(.a(x11), .b(x02), .O(new_n70_));
  xnor2a g25(.a(x09), .b(x00), .O(new_n71_));
  xnor2a g26(.a(x10), .b(x01), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n71_), .c(new_n70_), .d(new_n69_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n50_), .O(z04));
  oai21  g30(.a(x13), .b(x08), .c(new_n50_), .O(z05));
  nand2  g31(.a(x14), .b(new_n46_), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n49_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n49_), .O(z07));
  aoi21  g34(.a(new_n62_), .b(new_n54_), .c(new_n49_), .O(z08));
  nand3  g35(.a(new_n64_), .b(new_n48_), .c(new_n53_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand3  g37(.a(new_n63_), .b(new_n48_), .c(new_n53_), .O(new_n83_));
  inv1   g38(.a(new_n83_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n56_), .O(new_n85_));
  inv1   g40(.a(new_n85_), .O(z10));
  nand2  g41(.a(new_n57_), .b(new_n56_), .O(new_n87_));
  nand2  g42(.a(x18), .b(x17), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n63_), .c(new_n87_), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n48_), .c(x07), .O(z11));
  nand2  g45(.a(new_n88_), .b(new_n58_), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  nand3  g47(.a(new_n92_), .b(new_n91_), .c(new_n63_), .O(new_n93_));
  aoi21  g48(.a(new_n93_), .b(new_n48_), .c(x07), .O(z12));
  nor2   g49(.a(new_n92_), .b(x20), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n84_), .O(z13));
  inv1   g53(.a(x21), .O(new_n99_));
  nor2   g54(.a(new_n95_), .b(new_n99_), .O(new_n100_));
  oai21  g55(.a(new_n96_), .b(x21), .c(new_n63_), .O(new_n101_));
  oai21  g56(.a(new_n101_), .b(new_n100_), .c(new_n48_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n53_), .O(z14));
  inv1   g58(.a(x22), .O(new_n104_));
  aoi21  g59(.a(new_n95_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nor2   g60(.a(x22), .b(x21), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n95_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n63_), .O(new_n108_));
  oai21  g63(.a(new_n108_), .b(new_n105_), .c(new_n48_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n53_), .O(z15));
  and2   g65(.a(new_n107_), .b(x23), .O(new_n111_));
  oai21  g66(.a(new_n107_), .b(x23), .c(new_n63_), .O(new_n112_));
  oai21  g67(.a(new_n112_), .b(new_n111_), .c(new_n48_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n53_), .O(z16));
  oai21  g69(.a(new_n107_), .b(x23), .c(x24), .O(new_n115_));
  nor2   g70(.a(x24), .b(x23), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n106_), .c(new_n95_), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n115_), .c(new_n63_), .O(new_n118_));
  nand2  g73(.a(new_n118_), .b(new_n48_), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n53_), .O(z17));
  nor2   g75(.a(new_n117_), .b(x25), .O(new_n121_));
  nand2  g76(.a(new_n117_), .b(x25), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n63_), .O(new_n123_));
  oai21  g78(.a(new_n123_), .b(new_n121_), .c(new_n48_), .O(new_n124_));
  nand2  g79(.a(new_n124_), .b(new_n53_), .O(z18));
endmodule


