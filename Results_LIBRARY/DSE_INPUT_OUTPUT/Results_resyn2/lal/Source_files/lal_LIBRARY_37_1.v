// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n81_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x24), .O(new_n47_));
  nor2   g02(.a(new_n47_), .b(x05), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  oai21  g04(.a(x16), .b(new_n46_), .c(new_n49_), .O(z00));
  inv1   g05(.a(x05), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g12(.a(x21), .O(new_n58_));
  inv1   g13(.a(x22), .O(new_n59_));
  nor2   g14(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nor2   g16(.a(x25), .b(x23), .O(new_n62_));
  aoi21  g17(.a(new_n62_), .b(new_n61_), .c(x04), .O(new_n63_));
  inv1   g18(.a(x25), .O(new_n64_));
  aoi22  g19(.a(new_n49_), .b(x07), .c(new_n64_), .d(new_n47_), .O(new_n65_));
  oai21  g20(.a(new_n63_), .b(new_n51_), .c(new_n65_), .O(z01));
  inv1   g21(.a(x16), .O(new_n67_));
  nor2   g22(.a(new_n48_), .b(new_n67_), .O(z02));
  aoi21  g23(.a(new_n60_), .b(new_n57_), .c(x23), .O(new_n69_));
  oai21  g24(.a(new_n69_), .b(new_n47_), .c(new_n64_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n49_), .O(z03));
  nand2  g26(.a(new_n49_), .b(new_n46_), .O(new_n72_));
  xor2a  g27(.a(x12), .b(x03), .O(new_n73_));
  xor2a  g28(.a(x11), .b(x02), .O(new_n74_));
  nor2   g29(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  xor2a  g30(.a(x09), .b(x00), .O(new_n76_));
  xor2a  g31(.a(x10), .b(x01), .O(new_n77_));
  nor2   g32(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g33(.a(new_n78_), .b(new_n75_), .c(new_n72_), .O(z04));
  oai21  g34(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g35(.a(x14), .b(new_n46_), .O(new_n81_));
  nand2  g36(.a(new_n81_), .b(new_n49_), .O(z06));
  aoi21  g37(.a(new_n46_), .b(x06), .c(new_n48_), .O(z07));
  inv1   g38(.a(x23), .O(new_n84_));
  nand3  g39(.a(new_n61_), .b(new_n84_), .c(x05), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(x24), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n64_), .O(z08));
  nor2   g42(.a(x15), .b(x07), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(x05), .c(x04), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(z09));
  nor2   g45(.a(new_n88_), .b(new_n48_), .O(new_n91_));
  nand2  g46(.a(x05), .b(x04), .O(new_n92_));
  oai21  g47(.a(new_n48_), .b(new_n53_), .c(new_n92_), .O(new_n93_));
  nor2   g48(.a(new_n93_), .b(new_n91_), .O(z10));
  nand2  g49(.a(new_n54_), .b(new_n53_), .O(new_n95_));
  nand2  g50(.a(new_n88_), .b(new_n92_), .O(new_n96_));
  nor2   g51(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  nand2  g52(.a(x18), .b(x17), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n95_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(z11));
  nand3  g55(.a(x19), .b(x18), .c(x17), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  aoi21  g57(.a(x18), .b(x17), .c(x19), .O(new_n103_));
  nor4   g58(.a(new_n103_), .b(new_n102_), .c(new_n96_), .d(new_n48_), .O(z12));
  nor2   g59(.a(new_n101_), .b(x20), .O(new_n105_));
  inv1   g60(.a(new_n105_), .O(new_n106_));
  nand2  g61(.a(new_n101_), .b(x20), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n106_), .c(new_n97_), .O(z13));
  nor2   g63(.a(new_n105_), .b(new_n58_), .O(new_n109_));
  nand3  g64(.a(new_n102_), .b(new_n58_), .c(new_n52_), .O(new_n110_));
  nand2  g65(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  oai21  g66(.a(new_n111_), .b(new_n109_), .c(new_n49_), .O(new_n112_));
  nand2  g67(.a(new_n112_), .b(new_n92_), .O(z14));
  nand2  g68(.a(new_n110_), .b(x22), .O(new_n114_));
  nand4  g69(.a(new_n102_), .b(new_n59_), .c(new_n58_), .d(new_n52_), .O(new_n115_));
  nand3  g70(.a(new_n115_), .b(new_n114_), .c(new_n88_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n49_), .O(new_n117_));
  nand2  g72(.a(new_n117_), .b(new_n92_), .O(z15));
  nand2  g73(.a(new_n115_), .b(x23), .O(new_n119_));
  nor2   g74(.a(x23), .b(x22), .O(new_n120_));
  nand4  g75(.a(new_n120_), .b(new_n102_), .c(new_n58_), .d(new_n52_), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n119_), .c(new_n97_), .O(z16));
  aoi21  g77(.a(new_n121_), .b(x24), .c(x04), .O(new_n123_));
  nand3  g78(.a(new_n120_), .b(new_n47_), .c(new_n58_), .O(new_n124_));
  inv1   g79(.a(new_n124_), .O(new_n125_));
  aoi21  g80(.a(new_n125_), .b(new_n105_), .c(new_n91_), .O(new_n126_));
  oai21  g81(.a(new_n123_), .b(new_n51_), .c(new_n126_), .O(z17));
  inv1   g82(.a(new_n96_), .O(new_n128_));
  inv1   g83(.a(new_n62_), .O(new_n129_));
  oai21  g84(.a(new_n115_), .b(new_n129_), .c(new_n47_), .O(new_n130_));
  nand2  g85(.a(new_n130_), .b(new_n51_), .O(new_n131_));
  nand4  g86(.a(new_n120_), .b(new_n105_), .c(new_n47_), .d(new_n58_), .O(new_n132_));
  xor2a  g87(.a(new_n132_), .b(x25), .O(new_n133_));
  nand3  g88(.a(new_n133_), .b(new_n131_), .c(new_n128_), .O(z18));
endmodule


