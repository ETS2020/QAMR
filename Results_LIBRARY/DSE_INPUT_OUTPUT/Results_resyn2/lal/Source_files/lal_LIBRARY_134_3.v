// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n76_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_;
  inv1   g00(.a(x16), .O(new_n46_));
  nand2  g01(.a(x24), .b(x19), .O(new_n47_));
  nand3  g02(.a(new_n47_), .b(new_n46_), .c(x08), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(z00));
  inv1   g04(.a(x24), .O(new_n50_));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  oai21  g07(.a(x18), .b(x17), .c(new_n52_), .O(new_n53_));
  nand2  g08(.a(x22), .b(x21), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  aoi21  g10(.a(new_n55_), .b(new_n53_), .c(x23), .O(new_n56_));
  oai21  g11(.a(new_n56_), .b(new_n50_), .c(new_n51_), .O(new_n57_));
  inv1   g12(.a(x07), .O(new_n58_));
  nand2  g13(.a(x05), .b(x04), .O(new_n59_));
  nand3  g14(.a(new_n59_), .b(new_n47_), .c(new_n58_), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n57_), .O(z01));
  nand2  g17(.a(new_n47_), .b(new_n46_), .O(z02));
  nand2  g18(.a(new_n57_), .b(new_n47_), .O(z03));
  inv1   g19(.a(x08), .O(new_n65_));
  nand2  g20(.a(new_n47_), .b(new_n65_), .O(new_n66_));
  xor2a  g21(.a(x12), .b(x03), .O(new_n67_));
  xor2a  g22(.a(x11), .b(x02), .O(new_n68_));
  nor2   g23(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  xor2a  g24(.a(x09), .b(x00), .O(new_n70_));
  xor2a  g25(.a(x10), .b(x01), .O(new_n71_));
  nor2   g26(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(new_n69_), .c(new_n66_), .O(z04));
  or2    g28(.a(new_n66_), .b(x13), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(z05));
  inv1   g30(.a(x14), .O(new_n76_));
  oai21  g31(.a(new_n76_), .b(x08), .c(new_n47_), .O(z06));
  aoi22  g32(.a(x24), .b(x19), .c(new_n65_), .d(x06), .O(z07));
  inv1   g33(.a(x23), .O(new_n79_));
  inv1   g34(.a(x17), .O(new_n80_));
  inv1   g35(.a(x18), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n82_));
  oai21  g37(.a(new_n54_), .b(new_n82_), .c(new_n79_), .O(new_n83_));
  oai21  g38(.a(new_n83_), .b(x19), .c(x24), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n51_), .O(z08));
  inv1   g40(.a(x15), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n58_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(new_n59_), .c(new_n47_), .O(z09));
  nand3  g43(.a(new_n59_), .b(new_n86_), .c(new_n58_), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x17), .c(new_n47_), .O(z10));
  nand2  g45(.a(new_n81_), .b(new_n80_), .O(new_n91_));
  nand2  g46(.a(x18), .b(x17), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(new_n89_), .c(new_n47_), .O(z11));
  inv1   g49(.a(new_n92_), .O(new_n95_));
  oai21  g50(.a(new_n95_), .b(x24), .c(x19), .O(new_n96_));
  inv1   g51(.a(x19), .O(new_n97_));
  aoi21  g52(.a(new_n92_), .b(new_n97_), .c(new_n89_), .O(new_n98_));
  and2   g53(.a(new_n98_), .b(new_n96_), .O(z12));
  nand2  g54(.a(new_n89_), .b(new_n47_), .O(new_n100_));
  nand2  g55(.a(new_n96_), .b(x20), .O(new_n101_));
  nor2   g56(.a(x20), .b(new_n97_), .O(new_n102_));
  nand3  g57(.a(new_n102_), .b(new_n95_), .c(new_n50_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .O(z13));
  nor2   g59(.a(x21), .b(x20), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n95_), .O(new_n106_));
  inv1   g61(.a(new_n106_), .O(new_n107_));
  oai21  g62(.a(new_n107_), .b(x24), .c(x19), .O(new_n108_));
  aoi21  g63(.a(new_n103_), .b(x21), .c(new_n89_), .O(new_n109_));
  nand2  g64(.a(new_n109_), .b(new_n108_), .O(z14));
  inv1   g65(.a(new_n89_), .O(new_n111_));
  inv1   g66(.a(x22), .O(new_n112_));
  nand3  g67(.a(new_n105_), .b(new_n95_), .c(new_n112_), .O(new_n113_));
  nand2  g68(.a(new_n113_), .b(new_n50_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(x19), .O(new_n115_));
  nor2   g70(.a(x24), .b(new_n97_), .O(new_n116_));
  inv1   g71(.a(new_n116_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n106_), .c(x22), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n115_), .c(new_n111_), .O(z15));
  oai21  g74(.a(new_n117_), .b(new_n113_), .c(x23), .O(new_n120_));
  nor2   g75(.a(x23), .b(x22), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n105_), .c(new_n95_), .O(new_n122_));
  nand2  g77(.a(new_n122_), .b(new_n50_), .O(new_n123_));
  nand2  g78(.a(new_n123_), .b(x19), .O(new_n124_));
  nand3  g79(.a(new_n124_), .b(new_n120_), .c(new_n111_), .O(z16));
  nand3  g80(.a(new_n116_), .b(new_n107_), .c(new_n112_), .O(new_n126_));
  oai21  g81(.a(new_n89_), .b(x24), .c(new_n47_), .O(new_n127_));
  oai21  g82(.a(new_n126_), .b(x23), .c(new_n127_), .O(z17));
  oai21  g83(.a(new_n122_), .b(x25), .c(new_n50_), .O(new_n129_));
  nand2  g84(.a(new_n129_), .b(x19), .O(new_n130_));
  nor3   g85(.a(x23), .b(x22), .c(x21), .O(new_n131_));
  nand4  g86(.a(new_n131_), .b(new_n102_), .c(new_n95_), .d(new_n50_), .O(new_n132_));
  aoi21  g87(.a(new_n132_), .b(x25), .c(new_n89_), .O(new_n133_));
  nand2  g88(.a(new_n133_), .b(new_n130_), .O(z18));
endmodule


