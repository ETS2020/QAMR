// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_;
  inv1   g00(.a(x16), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  inv1   g02(.a(x15), .O(new_n48_));
  nor2   g03(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z02));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n51_));
  nor2   g06(.a(new_n51_), .b(z02), .O(z00));
  inv1   g07(.a(x25), .O(new_n53_));
  inv1   g08(.a(x20), .O(new_n54_));
  inv1   g09(.a(x17), .O(new_n55_));
  inv1   g10(.a(x18), .O(new_n56_));
  inv1   g11(.a(x19), .O(new_n57_));
  nand3  g12(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g13(.a(x22), .b(x21), .O(new_n59_));
  aoi21  g14(.a(new_n58_), .b(new_n54_), .c(new_n59_), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(x23), .c(x24), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  aoi21  g17(.a(x05), .b(x04), .c(x07), .O(new_n63_));
  aoi21  g18(.a(new_n63_), .b(new_n62_), .c(new_n49_), .O(z01));
  aoi21  g19(.a(new_n61_), .b(new_n53_), .c(new_n49_), .O(z08));
  inv1   g20(.a(z08), .O(z03));
  inv1   g21(.a(new_n49_), .O(new_n67_));
  xnor2a g22(.a(x12), .b(x03), .O(new_n68_));
  xnor2a g23(.a(x11), .b(x02), .O(new_n69_));
  xnor2a g24(.a(x09), .b(x00), .O(new_n70_));
  xnor2a g25(.a(x10), .b(x01), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(new_n68_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n67_), .O(z04));
  inv1   g29(.a(x13), .O(new_n75_));
  nand2  g30(.a(new_n51_), .b(new_n75_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z05));
  nand2  g32(.a(new_n51_), .b(x14), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(z06));
  nand2  g34(.a(new_n51_), .b(x06), .O(z07));
  nand2  g35(.a(new_n48_), .b(new_n47_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n67_), .c(new_n63_), .O(z09));
  nand2  g37(.a(x05), .b(x04), .O(new_n83_));
  nand3  g38(.a(new_n83_), .b(new_n48_), .c(new_n47_), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  nand2  g40(.a(new_n85_), .b(new_n55_), .O(new_n86_));
  inv1   g41(.a(new_n86_), .O(z10));
  nor2   g42(.a(x18), .b(x17), .O(new_n88_));
  nor2   g43(.a(new_n56_), .b(new_n55_), .O(new_n89_));
  nor3   g44(.a(new_n89_), .b(new_n84_), .c(new_n88_), .O(z11));
  nor2   g45(.a(new_n89_), .b(x19), .O(new_n91_));
  nand3  g46(.a(x19), .b(x18), .c(x17), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(new_n93_));
  nor3   g48(.a(new_n93_), .b(new_n91_), .c(new_n84_), .O(z12));
  nor2   g49(.a(new_n92_), .b(x20), .O(new_n95_));
  inv1   g50(.a(new_n95_), .O(new_n96_));
  nand2  g51(.a(new_n92_), .b(x20), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(new_n96_), .c(new_n85_), .O(z13));
  nand2  g53(.a(new_n96_), .b(x21), .O(new_n99_));
  inv1   g54(.a(x21), .O(new_n100_));
  nand2  g55(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  nand4  g56(.a(new_n101_), .b(new_n99_), .c(new_n63_), .d(new_n48_), .O(new_n102_));
  xor2a  g57(.a(new_n102_), .b(new_n49_), .O(z14));
  nand2  g58(.a(new_n101_), .b(x22), .O(new_n104_));
  inv1   g59(.a(x22), .O(new_n105_));
  nand3  g60(.a(new_n95_), .b(new_n105_), .c(new_n100_), .O(new_n106_));
  nand3  g61(.a(new_n106_), .b(new_n104_), .c(new_n85_), .O(z15));
  nand2  g62(.a(new_n106_), .b(x23), .O(new_n108_));
  inv1   g63(.a(x23), .O(new_n109_));
  nand4  g64(.a(new_n95_), .b(new_n109_), .c(new_n105_), .d(new_n100_), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n108_), .c(new_n85_), .O(z16));
  nand2  g66(.a(new_n110_), .b(x24), .O(new_n112_));
  nor2   g67(.a(new_n110_), .b(x24), .O(new_n113_));
  nor2   g68(.a(new_n113_), .b(new_n84_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n112_), .O(z17));
  nand2  g70(.a(new_n113_), .b(new_n53_), .O(new_n116_));
  oai21  g71(.a(new_n110_), .b(x24), .c(x25), .O(new_n117_));
  nand3  g72(.a(new_n117_), .b(new_n116_), .c(new_n85_), .O(z18));
endmodule


