// Benchmark "FAU" written by ABC on Tue Aug 11 20:07:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n53_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x23), .O(new_n47_));
  nand2  g02(.a(x24), .b(new_n47_), .O(new_n48_));
  oai21  g03(.a(x16), .b(new_n46_), .c(new_n48_), .O(z00));
  inv1   g04(.a(x07), .O(new_n50_));
  nor2   g05(.a(x25), .b(x24), .O(z03));
  inv1   g06(.a(z03), .O(z08));
  nand2  g07(.a(x05), .b(x04), .O(new_n53_));
  nand4  g08(.a(new_n53_), .b(z08), .c(new_n48_), .d(new_n50_), .O(z01));
  inv1   g09(.a(x16), .O(new_n55_));
  nand2  g10(.a(new_n48_), .b(new_n55_), .O(z02));
  xnor2a g11(.a(x11), .b(x02), .O(new_n57_));
  xnor2a g12(.a(x12), .b(x03), .O(new_n58_));
  xnor2a g13(.a(x09), .b(x00), .O(new_n59_));
  xnor2a g14(.a(x10), .b(x01), .O(new_n60_));
  nand4  g15(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n48_), .O(z04));
  nand2  g18(.a(new_n48_), .b(new_n46_), .O(new_n64_));
  nor2   g19(.a(new_n64_), .b(x13), .O(z05));
  nand2  g20(.a(x14), .b(new_n46_), .O(new_n66_));
  nand2  g21(.a(new_n66_), .b(new_n48_), .O(z06));
  nand3  g22(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  nand2  g23(.a(new_n48_), .b(new_n50_), .O(new_n69_));
  nor3   g24(.a(new_n53_), .b(new_n69_), .c(x15), .O(z09));
  inv1   g25(.a(x15), .O(new_n71_));
  nand3  g26(.a(new_n53_), .b(new_n71_), .c(new_n50_), .O(new_n72_));
  oai21  g27(.a(new_n72_), .b(x17), .c(new_n48_), .O(z10));
  xnor2a g28(.a(x18), .b(x17), .O(new_n74_));
  oai21  g29(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z11));
  nand4  g30(.a(new_n53_), .b(new_n48_), .c(new_n71_), .d(new_n50_), .O(new_n76_));
  nand3  g31(.a(x19), .b(x18), .c(x17), .O(new_n77_));
  inv1   g32(.a(new_n77_), .O(new_n78_));
  aoi21  g33(.a(x18), .b(x17), .c(x19), .O(new_n79_));
  nor3   g34(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(z12));
  inv1   g35(.a(new_n76_), .O(new_n81_));
  inv1   g36(.a(x20), .O(new_n82_));
  nand4  g37(.a(new_n82_), .b(x19), .c(x18), .d(x17), .O(new_n83_));
  nand2  g38(.a(new_n77_), .b(x20), .O(new_n84_));
  nand3  g39(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(z13));
  nand2  g40(.a(new_n83_), .b(x21), .O(new_n86_));
  inv1   g41(.a(x21), .O(new_n87_));
  nand3  g42(.a(new_n78_), .b(new_n87_), .c(new_n82_), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n86_), .c(new_n81_), .O(z14));
  inv1   g44(.a(x22), .O(new_n90_));
  nor3   g45(.a(new_n77_), .b(x21), .c(x20), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g47(.a(new_n88_), .b(x22), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n81_), .O(z15));
  nand2  g49(.a(new_n72_), .b(new_n48_), .O(new_n95_));
  oai21  g50(.a(new_n88_), .b(x22), .c(x23), .O(new_n96_));
  inv1   g51(.a(x24), .O(new_n97_));
  nor2   g52(.a(x23), .b(x22), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n97_), .c(new_n87_), .O(new_n99_));
  or2    g54(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  nand3  g55(.a(new_n100_), .b(new_n96_), .c(new_n95_), .O(z16));
  inv1   g56(.a(new_n72_), .O(new_n102_));
  nand2  g57(.a(new_n98_), .b(new_n91_), .O(new_n103_));
  nand3  g58(.a(new_n103_), .b(new_n102_), .c(new_n97_), .O(z17));
  inv1   g59(.a(x25), .O(new_n105_));
  nand4  g60(.a(new_n105_), .b(new_n90_), .c(new_n87_), .d(new_n82_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n77_), .c(new_n97_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(new_n108_));
  oai21  g63(.a(new_n99_), .b(new_n83_), .c(x25), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n102_), .O(z18));
endmodule


