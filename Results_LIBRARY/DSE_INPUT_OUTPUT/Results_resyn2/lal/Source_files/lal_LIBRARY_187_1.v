// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x07), .O(new_n47_));
  nand2  g02(.a(x15), .b(new_n47_), .O(new_n48_));
  inv1   g03(.a(new_n48_), .O(new_n49_));
  nor3   g04(.a(new_n49_), .b(x16), .c(new_n46_), .O(z00));
  inv1   g05(.a(x25), .O(new_n51_));
  inv1   g06(.a(x20), .O(new_n52_));
  inv1   g07(.a(x17), .O(new_n53_));
  inv1   g08(.a(x18), .O(new_n54_));
  inv1   g09(.a(x19), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g11(.a(x22), .b(x21), .O(new_n57_));
  aoi21  g12(.a(new_n56_), .b(new_n52_), .c(new_n57_), .O(new_n58_));
  oai21  g13(.a(new_n58_), .b(x23), .c(x24), .O(new_n59_));
  nand2  g14(.a(x05), .b(x04), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(new_n61_));
  aoi21  g16(.a(new_n59_), .b(new_n51_), .c(new_n61_), .O(new_n62_));
  oai21  g17(.a(new_n62_), .b(x15), .c(new_n47_), .O(z01));
  and2   g18(.a(new_n48_), .b(x16), .O(z02));
  nand3  g19(.a(new_n59_), .b(new_n48_), .c(new_n51_), .O(new_n65_));
  inv1   g20(.a(new_n65_), .O(z03));
  nand2  g21(.a(new_n48_), .b(new_n46_), .O(new_n67_));
  xor2a  g22(.a(x12), .b(x03), .O(new_n68_));
  xor2a  g23(.a(x11), .b(x02), .O(new_n69_));
  nor2   g24(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  xor2a  g25(.a(x09), .b(x00), .O(new_n71_));
  xor2a  g26(.a(x10), .b(x01), .O(new_n72_));
  nor2   g27(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g28(.a(new_n73_), .b(new_n70_), .c(new_n67_), .O(z04));
  nor2   g29(.a(new_n67_), .b(x13), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n48_), .O(z06));
  nand3  g32(.a(new_n48_), .b(new_n46_), .c(x06), .O(z07));
  aoi21  g33(.a(new_n59_), .b(new_n51_), .c(new_n49_), .O(z08));
  inv1   g34(.a(x15), .O(new_n80_));
  nand3  g35(.a(new_n61_), .b(new_n80_), .c(new_n47_), .O(new_n81_));
  inv1   g36(.a(new_n81_), .O(z09));
  nand2  g37(.a(new_n60_), .b(new_n53_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n80_), .c(x07), .O(z10));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n85_));
  nand2  g40(.a(x18), .b(x17), .O(new_n86_));
  nand3  g41(.a(new_n86_), .b(new_n60_), .c(new_n85_), .O(new_n87_));
  aoi21  g42(.a(new_n87_), .b(new_n80_), .c(x07), .O(z11));
  nand3  g43(.a(new_n60_), .b(new_n80_), .c(new_n47_), .O(new_n89_));
  inv1   g44(.a(new_n89_), .O(new_n90_));
  nand3  g45(.a(x19), .b(x18), .c(x17), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g47(.a(new_n86_), .b(new_n55_), .c(new_n92_), .O(z12));
  nor2   g48(.a(new_n91_), .b(x20), .O(new_n94_));
  nand2  g49(.a(new_n91_), .b(x20), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n60_), .O(new_n96_));
  oai21  g51(.a(new_n96_), .b(new_n94_), .c(new_n80_), .O(new_n97_));
  nand2  g52(.a(new_n97_), .b(new_n47_), .O(z13));
  inv1   g53(.a(x21), .O(new_n99_));
  aoi21  g54(.a(new_n94_), .b(new_n99_), .c(new_n89_), .O(new_n100_));
  oai21  g55(.a(new_n94_), .b(new_n99_), .c(new_n100_), .O(z14));
  inv1   g56(.a(x22), .O(new_n102_));
  aoi21  g57(.a(new_n94_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  nor2   g58(.a(x22), .b(x21), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n94_), .O(new_n105_));
  nand2  g60(.a(new_n105_), .b(new_n60_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n103_), .c(new_n80_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(new_n47_), .O(z15));
  nand2  g63(.a(new_n105_), .b(x23), .O(new_n109_));
  inv1   g64(.a(x23), .O(new_n110_));
  nand3  g65(.a(new_n104_), .b(new_n94_), .c(new_n110_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n109_), .c(new_n90_), .O(z16));
  nor2   g67(.a(x24), .b(x23), .O(new_n113_));
  nand3  g68(.a(new_n113_), .b(new_n104_), .c(new_n94_), .O(new_n114_));
  nand2  g69(.a(new_n114_), .b(new_n60_), .O(new_n115_));
  aoi21  g70(.a(new_n111_), .b(x24), .c(new_n115_), .O(new_n116_));
  oai21  g71(.a(new_n116_), .b(x15), .c(new_n47_), .O(z17));
  nor2   g72(.a(new_n114_), .b(x25), .O(new_n118_));
  nand2  g73(.a(new_n114_), .b(x25), .O(new_n119_));
  nand2  g74(.a(new_n119_), .b(new_n60_), .O(new_n120_));
  oai21  g75(.a(new_n120_), .b(new_n118_), .c(new_n80_), .O(new_n121_));
  nand2  g76(.a(new_n121_), .b(new_n47_), .O(z18));
endmodule


