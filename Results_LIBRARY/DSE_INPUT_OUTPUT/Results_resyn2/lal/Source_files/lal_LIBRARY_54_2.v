// Benchmark "FAU" written by ABC on Tue Aug 11 20:06:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n78_, new_n80_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x24), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x23), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x23), .O(new_n52_));
  inv1   g07(.a(x25), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g09(.a(x20), .O(new_n55_));
  inv1   g10(.a(x17), .O(new_n56_));
  inv1   g11(.a(x18), .O(new_n57_));
  inv1   g12(.a(x19), .O(new_n58_));
  nand3  g13(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  nand2  g14(.a(x22), .b(x21), .O(new_n60_));
  aoi21  g15(.a(new_n59_), .b(new_n55_), .c(new_n60_), .O(new_n61_));
  nand2  g16(.a(x25), .b(new_n52_), .O(new_n62_));
  inv1   g17(.a(x07), .O(new_n63_));
  nand2  g18(.a(x05), .b(x04), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g20(.a(new_n62_), .b(new_n48_), .c(new_n65_), .O(new_n66_));
  oai21  g21(.a(new_n61_), .b(new_n54_), .c(new_n66_), .O(z01));
  aoi21  g22(.a(new_n61_), .b(x24), .c(new_n54_), .O(z03));
  xor2a  g23(.a(x12), .b(x03), .O(new_n69_));
  xor2a  g24(.a(x11), .b(x02), .O(new_n70_));
  nor2   g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  xor2a  g26(.a(x09), .b(x00), .O(new_n72_));
  xor2a  g27(.a(x10), .b(x01), .O(new_n73_));
  nor2   g28(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g29(.a(new_n49_), .b(new_n46_), .O(new_n75_));
  aoi21  g30(.a(new_n74_), .b(new_n71_), .c(new_n75_), .O(z04));
  oai21  g31(.a(x13), .b(x08), .c(new_n49_), .O(z05));
  nand2  g32(.a(x14), .b(new_n46_), .O(new_n78_));
  nand2  g33(.a(new_n78_), .b(new_n49_), .O(z06));
  nor2   g34(.a(x24), .b(new_n52_), .O(new_n80_));
  aoi21  g35(.a(new_n46_), .b(x06), .c(new_n80_), .O(z07));
  nand2  g36(.a(new_n61_), .b(x24), .O(new_n82_));
  nand3  g37(.a(new_n82_), .b(new_n53_), .c(new_n52_), .O(z08));
  inv1   g38(.a(x15), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n63_), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(new_n64_), .c(new_n49_), .O(z09));
  nand3  g41(.a(new_n64_), .b(new_n84_), .c(new_n63_), .O(new_n87_));
  oai21  g42(.a(new_n87_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g43(.a(new_n57_), .b(new_n56_), .O(new_n89_));
  nor2   g44(.a(new_n87_), .b(new_n80_), .O(new_n90_));
  nand2  g45(.a(x18), .b(x17), .O(new_n91_));
  nand3  g46(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  inv1   g47(.a(new_n92_), .O(z11));
  nand3  g48(.a(x19), .b(x18), .c(x17), .O(new_n94_));
  aoi21  g49(.a(new_n91_), .b(new_n58_), .c(new_n87_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n49_), .O(z12));
  inv1   g52(.a(new_n94_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n55_), .O(new_n99_));
  aoi21  g54(.a(new_n94_), .b(x20), .c(new_n87_), .O(new_n100_));
  aoi21  g55(.a(new_n100_), .b(new_n99_), .c(new_n80_), .O(z13));
  nand2  g56(.a(new_n99_), .b(x21), .O(new_n102_));
  nor2   g57(.a(x21), .b(x20), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n98_), .c(new_n87_), .O(new_n104_));
  aoi21  g59(.a(new_n104_), .b(new_n102_), .c(new_n80_), .O(z14));
  nand2  g60(.a(new_n103_), .b(new_n98_), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(x22), .O(new_n107_));
  nor3   g62(.a(x22), .b(x21), .c(x20), .O(new_n108_));
  nand2  g63(.a(new_n108_), .b(new_n98_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n107_), .c(new_n90_), .O(z15));
  nand2  g65(.a(new_n109_), .b(x23), .O(new_n111_));
  nand3  g66(.a(new_n108_), .b(new_n98_), .c(new_n52_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n111_), .c(new_n90_), .O(z16));
  nand2  g68(.a(new_n87_), .b(new_n49_), .O(new_n114_));
  nand4  g69(.a(new_n108_), .b(new_n98_), .c(new_n48_), .d(new_n52_), .O(new_n115_));
  nand2  g70(.a(new_n112_), .b(x24), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(new_n115_), .c(new_n114_), .O(z17));
  nand2  g72(.a(new_n53_), .b(new_n48_), .O(new_n118_));
  nand3  g73(.a(new_n118_), .b(new_n108_), .c(new_n98_), .O(new_n119_));
  inv1   g74(.a(x21), .O(new_n120_));
  inv1   g75(.a(x22), .O(new_n121_));
  nand3  g76(.a(new_n121_), .b(new_n120_), .c(new_n55_), .O(new_n122_));
  oai21  g77(.a(new_n122_), .b(new_n94_), .c(new_n53_), .O(new_n123_));
  nand3  g78(.a(new_n123_), .b(new_n119_), .c(new_n52_), .O(new_n124_));
  aoi22  g79(.a(new_n87_), .b(new_n49_), .c(x25), .d(x24), .O(new_n125_));
  nand2  g80(.a(new_n125_), .b(new_n124_), .O(z18));
endmodule


