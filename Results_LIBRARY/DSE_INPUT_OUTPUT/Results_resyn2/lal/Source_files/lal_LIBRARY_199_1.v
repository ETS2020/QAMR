// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n76_, new_n78_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x08), .O(new_n46_));
  inv1   g01(.a(x16), .O(new_n47_));
  inv1   g02(.a(x13), .O(new_n48_));
  nand2  g03(.a(x24), .b(new_n48_), .O(new_n49_));
  nand2  g04(.a(new_n49_), .b(new_n47_), .O(z02));
  nor2   g05(.a(z02), .b(new_n46_), .O(z00));
  inv1   g06(.a(x25), .O(new_n52_));
  inv1   g07(.a(x20), .O(new_n53_));
  inv1   g08(.a(x17), .O(new_n54_));
  inv1   g09(.a(x18), .O(new_n55_));
  inv1   g10(.a(x19), .O(new_n56_));
  nand3  g11(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g12(.a(x22), .b(x21), .O(new_n58_));
  aoi21  g13(.a(new_n57_), .b(new_n53_), .c(new_n58_), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(x23), .c(x24), .O(new_n60_));
  nand2  g15(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand2  g16(.a(x05), .b(x04), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(new_n63_));
  nor2   g18(.a(new_n63_), .b(x07), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(new_n61_), .c(new_n49_), .O(z01));
  nand2  g20(.a(new_n61_), .b(new_n49_), .O(z03));
  xnor2a g21(.a(x12), .b(x03), .O(new_n67_));
  xnor2a g22(.a(x11), .b(x02), .O(new_n68_));
  xnor2a g23(.a(x09), .b(x00), .O(new_n69_));
  xnor2a g24(.a(x10), .b(x01), .O(new_n70_));
  nand4  g25(.a(new_n70_), .b(new_n69_), .c(new_n68_), .d(new_n67_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n46_), .O(new_n72_));
  nand2  g27(.a(new_n72_), .b(new_n49_), .O(z04));
  inv1   g28(.a(x24), .O(new_n74_));
  aoi21  g29(.a(new_n74_), .b(x08), .c(x13), .O(z05));
  nand2  g30(.a(x14), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n49_), .O(z06));
  inv1   g32(.a(new_n49_), .O(new_n78_));
  aoi21  g33(.a(new_n46_), .b(x06), .c(new_n78_), .O(z07));
  aoi21  g34(.a(new_n60_), .b(new_n52_), .c(new_n78_), .O(z08));
  nor2   g35(.a(x15), .b(x07), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(new_n63_), .c(new_n49_), .O(new_n82_));
  inv1   g37(.a(new_n82_), .O(z09));
  nand2  g38(.a(new_n81_), .b(new_n62_), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x17), .c(new_n49_), .O(z10));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(new_n86_));
  nand3  g41(.a(new_n81_), .b(new_n62_), .c(new_n49_), .O(new_n87_));
  inv1   g42(.a(new_n87_), .O(new_n88_));
  nand2  g43(.a(x18), .b(x17), .O(new_n89_));
  nand3  g44(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(new_n90_));
  inv1   g45(.a(new_n90_), .O(z11));
  inv1   g46(.a(new_n84_), .O(new_n92_));
  nand3  g47(.a(x19), .b(x18), .c(x17), .O(new_n93_));
  nand2  g48(.a(new_n89_), .b(new_n56_), .O(new_n94_));
  nand3  g49(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n49_), .O(z12));
  nand4  g51(.a(new_n53_), .b(x19), .c(x18), .d(x17), .O(new_n97_));
  aoi21  g52(.a(new_n93_), .b(x20), .c(new_n84_), .O(new_n98_));
  aoi21  g53(.a(new_n98_), .b(new_n97_), .c(new_n78_), .O(z13));
  nand2  g54(.a(new_n97_), .b(x21), .O(new_n100_));
  inv1   g55(.a(x21), .O(new_n101_));
  nor2   g56(.a(new_n93_), .b(x20), .O(new_n102_));
  aoi21  g57(.a(new_n102_), .b(new_n101_), .c(new_n84_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n100_), .c(new_n78_), .O(z14));
  oai21  g59(.a(new_n97_), .b(x21), .c(x22), .O(new_n105_));
  nor2   g60(.a(x22), .b(x21), .O(new_n106_));
  nand2  g61(.a(new_n106_), .b(new_n102_), .O(new_n107_));
  nand3  g62(.a(new_n107_), .b(new_n105_), .c(new_n88_), .O(z15));
  inv1   g63(.a(new_n106_), .O(new_n109_));
  oai21  g64(.a(new_n109_), .b(new_n97_), .c(x23), .O(new_n110_));
  inv1   g65(.a(x23), .O(new_n111_));
  nand3  g66(.a(new_n106_), .b(new_n102_), .c(new_n111_), .O(new_n112_));
  nand3  g67(.a(new_n112_), .b(new_n110_), .c(new_n92_), .O(new_n113_));
  and2   g68(.a(new_n113_), .b(new_n49_), .O(z16));
  aoi21  g69(.a(new_n112_), .b(x24), .c(new_n87_), .O(new_n115_));
  oai21  g70(.a(new_n112_), .b(x24), .c(new_n115_), .O(z17));
  nor2   g71(.a(new_n52_), .b(new_n74_), .O(new_n117_));
  oai21  g72(.a(new_n117_), .b(new_n84_), .c(new_n49_), .O(new_n118_));
  nor2   g73(.a(new_n112_), .b(new_n52_), .O(new_n119_));
  nand2  g74(.a(new_n112_), .b(new_n52_), .O(new_n120_));
  nand2  g75(.a(new_n120_), .b(new_n74_), .O(new_n121_));
  oai21  g76(.a(new_n121_), .b(new_n119_), .c(new_n118_), .O(z18));
endmodule


