// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(x14), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  inv1   g08(.a(x09), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x08), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(x18), .c(new_n29_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n36_), .c(new_n31_), .O(z0));
  nand2  g12(.a(new_n38_), .b(new_n29_), .O(new_n41_));
  inv1   g13(.a(x08), .O(new_n42_));
  nor2   g14(.a(new_n30_), .b(new_n42_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x00), .O(new_n44_));
  oai21  g16(.a(new_n41_), .b(x11), .c(new_n44_), .O(z1));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n30_), .O(new_n47_));
  oai21  g19(.a(new_n46_), .b(new_n41_), .c(new_n47_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(new_n38_), .b(new_n32_), .c(new_n29_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n30_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(x09), .b(new_n42_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n32_), .c(new_n29_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand4  g28(.a(x14), .b(new_n29_), .c(x09), .d(new_n42_), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n32_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n56_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n32_), .b(new_n62_), .O(new_n63_));
  nand2  g35(.a(new_n33_), .b(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n63_), .c(new_n58_), .O(new_n65_));
  oai21  g37(.a(new_n54_), .b(new_n62_), .c(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n53_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x04), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(z5));
  nand2  g41(.a(new_n43_), .b(x05), .O(new_n70_));
  inv1   g42(.a(new_n41_), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(x15), .b(x14), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n32_), .c(new_n72_), .O(new_n74_));
  nand3  g46(.a(x16), .b(x15), .c(x14), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n33_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n70_), .O(z6));
  nand2  g51(.a(new_n43_), .b(x06), .O(new_n80_));
  nand3  g52(.a(new_n76_), .b(new_n33_), .c(x17), .O(new_n81_));
  inv1   g53(.a(x17), .O(new_n82_));
  nand2  g54(.a(new_n77_), .b(new_n82_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n81_), .c(new_n71_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n80_), .O(z7));
  nand2  g57(.a(new_n43_), .b(x07), .O(new_n86_));
  oai21  g58(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n87_));
  nor2   g59(.a(x18), .b(new_n82_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n76_), .c(new_n33_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n71_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n86_), .O(z8));
endmodule


