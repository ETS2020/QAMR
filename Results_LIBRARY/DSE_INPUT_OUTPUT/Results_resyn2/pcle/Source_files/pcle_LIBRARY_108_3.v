// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x17), .b(x16), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(new_n32_), .c(x15), .O(new_n34_));
  nand3  g06(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(x18), .O(new_n37_));
  oai22  g09(.a(new_n37_), .b(new_n34_), .c(new_n30_), .d(new_n29_), .O(z0));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(x09), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(x08), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(x10), .O(z1));
  oai21  g16(.a(x10), .b(x01), .c(x08), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n35_), .c(new_n45_), .O(z2));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(x12), .b(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n42_), .c(new_n49_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n48_), .c(x10), .O(z3));
  oai21  g26(.a(x10), .b(x03), .c(x08), .O(new_n55_));
  inv1   g27(.a(new_n49_), .O(new_n56_));
  nor2   g28(.a(new_n56_), .b(x14), .O(new_n57_));
  nand2  g29(.a(new_n36_), .b(new_n31_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n57_), .c(new_n55_), .O(z4));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nor2   g33(.a(new_n31_), .b(new_n61_), .O(new_n62_));
  inv1   g34(.a(new_n62_), .O(new_n63_));
  nand2  g35(.a(new_n31_), .b(new_n61_), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n42_), .c(new_n63_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n60_), .c(x10), .O(z5));
  nand2  g38(.a(x08), .b(x05), .O(new_n67_));
  nand2  g39(.a(new_n62_), .b(x16), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n31_), .b(new_n61_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n42_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n67_), .c(x10), .O(z6));
  oai21  g44(.a(x10), .b(x06), .c(x08), .O(new_n73_));
  aoi21  g45(.a(new_n62_), .b(x16), .c(x17), .O(new_n74_));
  nand2  g46(.a(new_n36_), .b(new_n34_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z7));
  oai21  g48(.a(x10), .b(x07), .c(x08), .O(new_n77_));
  nand3  g49(.a(new_n33_), .b(new_n62_), .c(x18), .O(new_n78_));
  inv1   g50(.a(x18), .O(new_n79_));
  nand2  g51(.a(new_n34_), .b(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n36_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n77_), .O(z8));
endmodule


