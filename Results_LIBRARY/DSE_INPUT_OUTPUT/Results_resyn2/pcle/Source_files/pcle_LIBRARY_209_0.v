// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nor2   g00(.a(x18), .b(x14), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  inv1   g07(.a(x14), .O(new_n36_));
  inv1   g08(.a(x18), .O(new_n37_));
  nand3  g09(.a(x17), .b(x16), .c(x15), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n30_), .O(z0));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  inv1   g15(.a(new_n33_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n42_), .c(new_n29_), .O(z1));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  inv1   g20(.a(x12), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n48_), .c(new_n44_), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n47_), .c(new_n29_), .O(z2));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand2  g26(.a(new_n48_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n34_), .c(new_n44_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n53_), .c(new_n29_), .O(z3));
  oai21  g29(.a(new_n35_), .b(new_n37_), .c(new_n36_), .O(new_n58_));
  nand3  g30(.a(new_n34_), .b(new_n44_), .c(x14), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(z4));
  nand3  g33(.a(new_n30_), .b(x08), .c(x04), .O(new_n62_));
  inv1   g34(.a(new_n34_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(x14), .c(x15), .O(new_n64_));
  nor2   g36(.a(new_n33_), .b(new_n29_), .O(new_n65_));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n64_), .c(new_n62_), .O(z5));
  nand3  g42(.a(new_n30_), .b(x08), .c(x05), .O(new_n71_));
  nand3  g43(.a(new_n67_), .b(new_n63_), .c(x16), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(new_n68_), .b(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n72_), .c(new_n65_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n71_), .O(z6));
  inv1   g48(.a(x17), .O(new_n77_));
  nand2  g49(.a(new_n72_), .b(new_n77_), .O(new_n78_));
  nor2   g50(.a(new_n38_), .b(new_n34_), .O(new_n79_));
  nor2   g51(.a(new_n79_), .b(new_n33_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand4  g53(.a(x17), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x18), .O(new_n83_));
  and2   g55(.a(x08), .b(x06), .O(new_n84_));
  aoi21  g56(.a(new_n83_), .b(new_n36_), .c(new_n84_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n81_), .O(z7));
  nand2  g58(.a(new_n79_), .b(x18), .O(new_n87_));
  oai21  g59(.a(new_n38_), .b(new_n34_), .c(new_n37_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n44_), .O(new_n89_));
  oai21  g61(.a(new_n44_), .b(new_n37_), .c(new_n36_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z8));
endmodule


