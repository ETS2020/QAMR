// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_;
  inv1   g00(.a(x18), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x11), .O(new_n31_));
  inv1   g03(.a(x09), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(x08), .O(new_n33_));
  aoi21  g05(.a(new_n33_), .b(new_n31_), .c(x18), .O(new_n34_));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(x10), .c(new_n35_), .O(z1));
  inv1   g08(.a(x08), .O(new_n37_));
  nor2   g09(.a(z0), .b(new_n37_), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x01), .O(new_n39_));
  inv1   g11(.a(x10), .O(new_n40_));
  nand3  g12(.a(new_n33_), .b(new_n29_), .c(new_n40_), .O(new_n41_));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(new_n41_), .c(new_n39_), .O(z2));
  nand2  g15(.a(new_n38_), .b(x02), .O(new_n44_));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  inv1   g17(.a(new_n41_), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(z3));
  nand4  g21(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n33_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n40_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z4));
  nand2  g29(.a(new_n38_), .b(x04), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n50_), .b(new_n59_), .O(new_n60_));
  inv1   g32(.a(new_n50_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x15), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n46_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n58_), .O(z5));
  inv1   g36(.a(x16), .O(new_n65_));
  oai21  g37(.a(new_n50_), .b(new_n59_), .c(new_n65_), .O(new_n66_));
  nand2  g38(.a(x09), .b(new_n37_), .O(new_n67_));
  nor2   g39(.a(new_n65_), .b(new_n59_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n61_), .c(new_n67_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n66_), .c(x18), .O(new_n70_));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(x10), .c(new_n71_), .O(z6));
  nand2  g44(.a(new_n38_), .b(x06), .O(new_n73_));
  aoi21  g45(.a(new_n68_), .b(new_n61_), .c(x17), .O(new_n74_));
  nand3  g46(.a(new_n68_), .b(new_n61_), .c(x17), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n74_), .c(new_n73_), .O(z7));
  oai21  g49(.a(new_n75_), .b(new_n67_), .c(new_n29_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n40_), .O(new_n79_));
  nand2  g51(.a(x08), .b(x07), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(z8));
endmodule


