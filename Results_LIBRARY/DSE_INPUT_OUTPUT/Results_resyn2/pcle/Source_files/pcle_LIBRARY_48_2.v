// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand2  g02(.a(x16), .b(x15), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(new_n30_), .c(x18), .d(x17), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(x00), .O(new_n37_));
  nor2   g09(.a(x01), .b(new_n37_), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n36_), .c(new_n33_), .O(z0));
  inv1   g11(.a(x01), .O(new_n40_));
  oai21  g12(.a(x08), .b(new_n40_), .c(x00), .O(new_n41_));
  oai21  g13(.a(new_n36_), .b(x11), .c(new_n41_), .O(z1));
  aoi21  g14(.a(new_n34_), .b(new_n37_), .c(x01), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n44_), .c(new_n35_), .d(x09), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n34_), .c(new_n43_), .O(z2));
  inv1   g19(.a(new_n36_), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n44_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n38_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n49_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n48_), .c(new_n29_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n38_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nor2   g32(.a(new_n29_), .b(new_n60_), .O(new_n61_));
  oai21  g33(.a(new_n30_), .b(x15), .c(new_n48_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n38_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(z5));
  nand2  g36(.a(x08), .b(x05), .O(new_n65_));
  nand2  g37(.a(new_n32_), .b(new_n30_), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  oai21  g39(.a(new_n29_), .b(new_n60_), .c(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n48_), .c(new_n66_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n65_), .c(new_n38_), .O(z6));
  nand2  g42(.a(x08), .b(x06), .O(new_n71_));
  nand3  g43(.a(new_n32_), .b(new_n30_), .c(x17), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  oai21  g45(.a(new_n31_), .b(new_n29_), .c(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n48_), .c(new_n72_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n71_), .c(new_n38_), .O(z7));
  nor2   g48(.a(new_n31_), .b(new_n29_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(x17), .c(x18), .O(new_n78_));
  nand2  g50(.a(new_n48_), .b(new_n33_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x07), .c(new_n38_), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n78_), .c(new_n80_), .O(z8));
endmodule


