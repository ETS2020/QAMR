// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x16), .b(x15), .c(x14), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n34_), .c(new_n32_), .d(new_n30_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  inv1   g10(.a(x11), .O(new_n39_));
  inv1   g11(.a(x08), .O(new_n40_));
  nand3  g12(.a(new_n30_), .b(x09), .c(new_n40_), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  inv1   g15(.a(x18), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x17), .O(new_n45_));
  nand2  g17(.a(x08), .b(x00), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n43_), .O(z1));
  nor2   g19(.a(x18), .b(new_n29_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  inv1   g22(.a(x12), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n39_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n50_), .c(new_n32_), .d(new_n30_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n48_), .O(z2));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  inv1   g27(.a(x13), .O(new_n56_));
  nand2  g28(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(new_n33_), .c(new_n32_), .d(new_n30_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z3));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand4  g32(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  inv1   g33(.a(x14), .O(new_n62_));
  nand2  g34(.a(new_n33_), .b(new_n62_), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n61_), .c(new_n32_), .d(new_n30_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n60_), .c(new_n48_), .O(z4));
  inv1   g37(.a(x15), .O(new_n66_));
  nor2   g38(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n61_), .b(new_n66_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n32_), .c(new_n30_), .O(new_n69_));
  aoi22  g41(.a(new_n44_), .b(x17), .c(x08), .d(x04), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(z5));
  nand2  g43(.a(new_n36_), .b(new_n34_), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(x15), .b(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n33_), .c(new_n73_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n72_), .c(new_n42_), .O(new_n76_));
  aoi21  g48(.a(x08), .b(x05), .c(new_n48_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(z6));
  nand3  g50(.a(new_n45_), .b(x08), .c(x06), .O(new_n79_));
  oai22  g51(.a(new_n35_), .b(new_n33_), .c(new_n44_), .d(new_n29_), .O(new_n80_));
  nand3  g52(.a(new_n36_), .b(new_n34_), .c(x17), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n80_), .c(new_n42_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(z7));
  nand3  g55(.a(new_n45_), .b(x08), .c(x07), .O(new_n84_));
  nand3  g56(.a(new_n81_), .b(new_n42_), .c(x18), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


