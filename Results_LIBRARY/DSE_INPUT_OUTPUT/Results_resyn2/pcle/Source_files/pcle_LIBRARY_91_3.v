// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  nand3  g03(.a(x17), .b(x16), .c(x15), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand3  g05(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  oai21  g08(.a(new_n30_), .b(new_n29_), .c(new_n36_), .O(z0));
  oai21  g09(.a(x10), .b(x00), .c(x08), .O(new_n38_));
  oai21  g10(.a(new_n34_), .b(x11), .c(new_n38_), .O(z1));
  oai21  g11(.a(x10), .b(x01), .c(x08), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n34_), .c(new_n40_), .O(z2));
  oai21  g14(.a(x10), .b(x02), .c(x08), .O(new_n43_));
  nand3  g15(.a(x13), .b(x12), .c(x11), .O(new_n44_));
  inv1   g16(.a(x13), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n44_), .c(new_n35_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n44_), .b(new_n51_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n29_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n50_), .c(x10), .O(z4));
  oai21  g26(.a(x10), .b(x04), .c(x08), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  nand2  g28(.a(new_n31_), .b(new_n56_), .O(new_n57_));
  nor2   g29(.a(new_n31_), .b(new_n56_), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n35_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n55_), .O(z5));
  oai21  g33(.a(x10), .b(x05), .c(x08), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(x16), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n31_), .b(new_n56_), .c(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n35_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n62_), .O(z6));
  oai21  g39(.a(x10), .b(x06), .c(x08), .O(new_n68_));
  aoi21  g40(.a(new_n58_), .b(x16), .c(x17), .O(new_n69_));
  inv1   g41(.a(new_n31_), .O(new_n70_));
  inv1   g42(.a(new_n32_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g44(.a(new_n35_), .b(new_n72_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n69_), .c(new_n68_), .O(z7));
  oai21  g46(.a(x10), .b(x07), .c(x08), .O(new_n75_));
  nand2  g47(.a(new_n33_), .b(x18), .O(new_n76_));
  inv1   g48(.a(x18), .O(new_n77_));
  oai21  g49(.a(new_n32_), .b(new_n31_), .c(new_n77_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n76_), .c(new_n35_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n75_), .O(z8));
endmodule


