// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nand2  g00(.a(x15), .b(x14), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(new_n30_), .c(x16), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x18), .c(x17), .O(new_n38_));
  oai22  g10(.a(new_n38_), .b(new_n33_), .c(x18), .d(x16), .O(z0));
  nor2   g11(.a(x18), .b(x16), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand2  g14(.a(new_n37_), .b(new_n42_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n40_), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(new_n36_), .c(new_n46_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand2  g20(.a(new_n37_), .b(new_n31_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n40_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n48_), .c(new_n50_), .O(z3));
  nor2   g23(.a(new_n32_), .b(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n40_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nor2   g29(.a(new_n53_), .b(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n53_), .b(new_n57_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n37_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x04), .c(new_n40_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z5));
  inv1   g34(.a(x18), .O(new_n63_));
  nor2   g35(.a(new_n31_), .b(new_n29_), .O(new_n64_));
  aoi21  g36(.a(new_n37_), .b(new_n64_), .c(new_n63_), .O(new_n65_));
  nand2  g37(.a(new_n32_), .b(new_n30_), .O(new_n66_));
  and2   g38(.a(x08), .b(x05), .O(new_n67_));
  nand4  g39(.a(x16), .b(new_n35_), .c(x09), .d(new_n34_), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n66_), .c(new_n67_), .O(new_n70_));
  oai21  g42(.a(new_n65_), .b(x16), .c(new_n70_), .O(z6));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n33_), .b(new_n72_), .O(new_n73_));
  aoi21  g45(.a(new_n64_), .b(x17), .c(new_n36_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  oai21  g48(.a(new_n36_), .b(new_n72_), .c(x18), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x06), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(z7));
  nand3  g52(.a(new_n64_), .b(x18), .c(x17), .O(new_n81_));
  nand3  g53(.a(x17), .b(x15), .c(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n31_), .c(new_n63_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n81_), .c(new_n37_), .O(new_n84_));
  oai21  g56(.a(new_n37_), .b(new_n63_), .c(new_n76_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z8));
endmodule


