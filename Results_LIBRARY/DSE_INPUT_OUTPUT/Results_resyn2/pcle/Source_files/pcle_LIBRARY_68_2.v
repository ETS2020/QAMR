// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x01), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(x08), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nor3   g06(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n31_), .c(x18), .d(x17), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  aoi21  g10(.a(new_n31_), .b(new_n38_), .c(x01), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  oai21  g12(.a(new_n39_), .b(x10), .c(new_n40_), .O(z1));
  nand3  g13(.a(x10), .b(x08), .c(x01), .O(new_n42_));
  inv1   g14(.a(x10), .O(new_n43_));
  nand3  g15(.a(new_n31_), .b(new_n43_), .c(new_n29_), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(new_n33_), .b(new_n31_), .c(new_n43_), .O(new_n48_));
  aoi22  g20(.a(new_n43_), .b(x01), .c(x08), .d(x02), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(z3));
  inv1   g22(.a(x08), .O(new_n51_));
  aoi21  g23(.a(new_n43_), .b(x01), .c(new_n51_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x03), .O(new_n53_));
  inv1   g25(.a(new_n44_), .O(new_n54_));
  inv1   g26(.a(new_n33_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x14), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n33_), .b(new_n57_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n53_), .O(z4));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand2  g34(.a(x09), .b(new_n51_), .O(new_n63_));
  inv1   g35(.a(new_n34_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n55_), .c(new_n63_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n62_), .c(x01), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n67_), .O(z5));
  nand3  g40(.a(new_n64_), .b(new_n55_), .c(x16), .O(new_n69_));
  oai21  g41(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n31_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n29_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  nand2  g47(.a(new_n52_), .b(x06), .O(new_n76_));
  nor2   g48(.a(new_n35_), .b(x17), .O(new_n77_));
  nand4  g49(.a(new_n64_), .b(new_n55_), .c(x17), .d(x16), .O(new_n78_));
  nand2  g50(.a(new_n54_), .b(new_n78_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z7));
  nand2  g52(.a(new_n52_), .b(x07), .O(new_n81_));
  nand3  g53(.a(new_n35_), .b(x18), .c(x17), .O(new_n82_));
  inv1   g54(.a(x18), .O(new_n83_));
  nand2  g55(.a(new_n78_), .b(new_n83_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n54_), .c(new_n82_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n81_), .O(z8));
endmodule


