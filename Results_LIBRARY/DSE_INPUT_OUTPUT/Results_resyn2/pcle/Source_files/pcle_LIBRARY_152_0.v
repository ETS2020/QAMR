// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  nor2   g07(.a(x18), .b(x17), .O(new_n36_));
  nand2  g08(.a(x08), .b(x00), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(new_n34_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n37_), .c(new_n36_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n39_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n42_), .c(new_n36_), .O(z2));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  inv1   g20(.a(x13), .O(new_n49_));
  nand2  g21(.a(new_n43_), .b(new_n49_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n39_), .c(new_n30_), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n48_), .c(new_n36_), .O(z3));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n30_), .b(new_n54_), .O(new_n55_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n39_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n53_), .c(new_n36_), .O(z4));
  nand2  g30(.a(x08), .b(x04), .O(new_n59_));
  inv1   g31(.a(new_n56_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x15), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n56_), .b(new_n62_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n39_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n59_), .c(new_n36_), .O(z5));
  nand3  g37(.a(new_n60_), .b(x16), .c(x15), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n61_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n39_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n36_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  nand2  g43(.a(x16), .b(x15), .O(new_n72_));
  nor2   g44(.a(new_n56_), .b(new_n72_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n39_), .c(new_n29_), .O(new_n74_));
  inv1   g46(.a(new_n43_), .O(new_n75_));
  inv1   g47(.a(new_n72_), .O(new_n76_));
  and2   g48(.a(x14), .b(x13), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n75_), .O(new_n78_));
  and2   g50(.a(x08), .b(x06), .O(new_n79_));
  nand4  g51(.a(x17), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n78_), .c(new_n79_), .O(new_n82_));
  oai21  g54(.a(new_n74_), .b(x17), .c(new_n82_), .O(z7));
  oai21  g55(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n84_));
  inv1   g56(.a(x17), .O(new_n85_));
  nor2   g57(.a(x18), .b(new_n85_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n60_), .c(new_n76_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n39_), .O(new_n89_));
  inv1   g61(.a(new_n36_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(x08), .c(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n89_), .O(z8));
endmodule


