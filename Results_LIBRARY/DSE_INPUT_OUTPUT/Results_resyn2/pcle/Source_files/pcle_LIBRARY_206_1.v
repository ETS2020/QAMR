// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x16), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x13), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g10(.a(x18), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(x13), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  inv1   g14(.a(new_n32_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x01), .c(new_n40_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n46_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n43_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n40_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z3));
  inv1   g29(.a(x14), .O(new_n58_));
  oai21  g30(.a(new_n32_), .b(new_n58_), .c(new_n39_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  nand2  g32(.a(x08), .b(x03), .O(new_n61_));
  nand3  g33(.a(x14), .b(x12), .c(x11), .O(new_n62_));
  nand2  g34(.a(new_n52_), .b(new_n58_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n43_), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n61_), .c(new_n60_), .O(z4));
  nand2  g37(.a(x18), .b(new_n53_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(x08), .c(x04), .O(new_n67_));
  nor2   g39(.a(new_n33_), .b(new_n53_), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand4  g41(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n66_), .c(new_n43_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n68_), .c(new_n67_), .O(z5));
  oai21  g45(.a(new_n33_), .b(new_n53_), .c(new_n29_), .O(new_n74_));
  nand2  g46(.a(new_n34_), .b(x16), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n43_), .O(new_n76_));
  oai21  g48(.a(new_n32_), .b(new_n29_), .c(new_n39_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x05), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(z6));
  nor2   g52(.a(new_n40_), .b(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x06), .O(new_n82_));
  aoi21  g54(.a(x18), .b(new_n53_), .c(new_n32_), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand2  g56(.a(x16), .b(x15), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n70_), .c(new_n84_), .O(new_n86_));
  inv1   g58(.a(new_n70_), .O(new_n87_));
  inv1   g59(.a(new_n85_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(x17), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n82_), .O(z7));
  nand2  g63(.a(new_n81_), .b(x07), .O(new_n92_));
  inv1   g64(.a(new_n62_), .O(new_n93_));
  nand3  g65(.a(x17), .b(x16), .c(x15), .O(new_n94_));
  inv1   g66(.a(new_n94_), .O(new_n95_));
  nand3  g67(.a(new_n95_), .b(new_n93_), .c(x18), .O(new_n96_));
  oai21  g68(.a(new_n94_), .b(new_n62_), .c(new_n39_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n96_), .c(new_n43_), .d(x13), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(new_n92_), .O(z8));
endmodule


