// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x18), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g03(.a(x10), .O(new_n33_));
  inv1   g04(.a(x09), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x08), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(new_n31_), .c(new_n33_), .O(new_n36_));
  oai22  g07(.a(new_n36_), .b(x11), .c(new_n32_), .d(new_n30_), .O(z1));
  nor2   g08(.a(x12), .b(x11), .O(new_n38_));
  nand2  g09(.a(x12), .b(x11), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n38_), .c(new_n31_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n33_), .O(new_n42_));
  nand2  g13(.a(x08), .b(x01), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n42_), .O(z2));
  aoi21  g15(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  nand3  g16(.a(x13), .b(x12), .c(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n35_), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n45_), .c(new_n31_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nand2  g20(.a(x08), .b(x02), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n49_), .O(z3));
  nand4  g22(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  inv1   g23(.a(x14), .O(new_n53_));
  nand2  g24(.a(new_n46_), .b(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(new_n35_), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n31_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n33_), .O(new_n57_));
  nand2  g28(.a(x08), .b(x03), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n57_), .O(z4));
  inv1   g30(.a(x04), .O(new_n60_));
  inv1   g31(.a(new_n36_), .O(new_n61_));
  inv1   g32(.a(x15), .O(new_n62_));
  nand2  g33(.a(new_n52_), .b(new_n62_), .O(new_n63_));
  inv1   g34(.a(new_n52_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x15), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n63_), .c(new_n61_), .O(new_n66_));
  oai21  g37(.a(new_n32_), .b(new_n60_), .c(new_n66_), .O(z5));
  inv1   g38(.a(x05), .O(new_n68_));
  inv1   g39(.a(x16), .O(new_n69_));
  nor2   g40(.a(new_n65_), .b(new_n69_), .O(new_n70_));
  nand2  g41(.a(new_n65_), .b(new_n69_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n61_), .O(new_n72_));
  oai22  g43(.a(new_n72_), .b(new_n70_), .c(new_n32_), .d(new_n68_), .O(z6));
  inv1   g44(.a(x17), .O(new_n74_));
  nand2  g45(.a(new_n52_), .b(new_n74_), .O(new_n75_));
  inv1   g46(.a(x08), .O(new_n76_));
  nand2  g47(.a(x09), .b(new_n76_), .O(new_n77_));
  nand2  g48(.a(x16), .b(x15), .O(new_n78_));
  xor2a  g49(.a(new_n78_), .b(x17), .O(new_n79_));
  aoi21  g50(.a(new_n79_), .b(new_n64_), .c(new_n77_), .O(new_n80_));
  aoi21  g51(.a(new_n80_), .b(new_n75_), .c(x18), .O(new_n81_));
  nand2  g52(.a(x08), .b(x06), .O(new_n82_));
  oai21  g53(.a(new_n81_), .b(x10), .c(new_n82_), .O(z7));
  inv1   g54(.a(new_n78_), .O(new_n84_));
  nand4  g55(.a(new_n84_), .b(new_n64_), .c(new_n35_), .d(x17), .O(new_n85_));
  nand2  g56(.a(new_n85_), .b(new_n31_), .O(new_n86_));
  nand2  g57(.a(new_n86_), .b(new_n33_), .O(new_n87_));
  nand2  g58(.a(x08), .b(x07), .O(new_n88_));
  nand2  g59(.a(new_n88_), .b(new_n87_), .O(z8));
  zero   g60(.O(z0));
endmodule


