// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x18), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n35_), .c(x17), .d(x16), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n33_), .O(z0));
  inv1   g11(.a(x16), .O(new_n40_));
  nor2   g12(.a(x18), .b(new_n40_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n32_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n47_), .c(new_n32_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n41_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand2  g24(.a(new_n34_), .b(new_n32_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n41_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(z3));
  nor2   g27(.a(new_n35_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n35_), .b(x14), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n32_), .O(new_n58_));
  aoi21  g30(.a(x08), .b(x03), .c(new_n41_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nor2   g34(.a(new_n36_), .b(new_n34_), .O(new_n63_));
  nor2   g35(.a(new_n63_), .b(new_n31_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x04), .c(new_n41_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(z5));
  nor2   g39(.a(new_n63_), .b(x16), .O(new_n68_));
  nand2  g40(.a(new_n63_), .b(x16), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n32_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n41_), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(new_n68_), .c(new_n71_), .O(z6));
  aoi21  g44(.a(new_n63_), .b(x16), .c(x17), .O(new_n73_));
  inv1   g45(.a(new_n41_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n38_), .c(new_n32_), .O(new_n75_));
  nand3  g47(.a(new_n74_), .b(x08), .c(x06), .O(new_n76_));
  oai21  g48(.a(new_n75_), .b(new_n73_), .c(new_n76_), .O(z7));
  inv1   g49(.a(new_n38_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x07), .c(new_n41_), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n33_), .c(new_n79_), .O(z8));
endmodule


