// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x11), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  and2   g04(.a(x13), .b(x12), .O(new_n33_));
  nand4  g05(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n33_), .c(new_n32_), .d(x18), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(new_n30_), .O(z0));
  inv1   g09(.a(x08), .O(new_n38_));
  aoi21  g10(.a(x11), .b(x10), .c(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x00), .O(new_n40_));
  nand3  g12(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(z1));
  oai21  g14(.a(x11), .b(x10), .c(x12), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n30_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n43_), .c(new_n32_), .O(new_n46_));
  aoi22  g18(.a(x11), .b(x10), .c(x08), .d(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g20(.a(new_n39_), .b(x02), .O(new_n49_));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  inv1   g23(.a(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n29_), .b(x09), .c(new_n38_), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  inv1   g26(.a(new_n54_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n50_), .c(new_n49_), .O(z3));
  nand3  g28(.a(new_n33_), .b(x09), .c(new_n38_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(x14), .c(new_n29_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x11), .O(new_n59_));
  aoi22  g31(.a(new_n54_), .b(x14), .c(x08), .d(x03), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x14), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n57_), .c(new_n29_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x11), .O(new_n65_));
  nand2  g37(.a(x08), .b(x04), .O(new_n66_));
  inv1   g38(.a(new_n53_), .O(new_n67_));
  nand2  g39(.a(new_n52_), .b(x14), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n67_), .c(x15), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n66_), .c(new_n65_), .O(z5));
  nand2  g42(.a(new_n39_), .b(x05), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(x15), .b(x14), .O(new_n73_));
  oai21  g45(.a(new_n51_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nand3  g46(.a(x16), .b(x15), .c(x14), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n74_), .c(new_n67_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n71_), .O(z6));
  inv1   g51(.a(x17), .O(new_n80_));
  nand4  g52(.a(new_n76_), .b(new_n33_), .c(new_n32_), .d(new_n80_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x11), .O(new_n83_));
  nand4  g55(.a(x17), .b(new_n29_), .c(x09), .d(new_n38_), .O(new_n84_));
  inv1   g56(.a(new_n84_), .O(new_n85_));
  aoi22  g57(.a(new_n85_), .b(new_n77_), .c(x08), .d(x06), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n83_), .O(z7));
  nand2  g59(.a(new_n39_), .b(x07), .O(new_n88_));
  oai21  g60(.a(new_n51_), .b(new_n34_), .c(x18), .O(new_n89_));
  inv1   g61(.a(x18), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(x17), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n77_), .c(new_n89_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(new_n67_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n88_), .O(z8));
endmodule


