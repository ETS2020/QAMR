// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand2  g02(.a(new_n30_), .b(x15), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n35_), .c(new_n31_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(new_n35_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  inv1   g12(.a(x12), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x10), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n40_), .O(z1));
  and2   g16(.a(new_n42_), .b(x08), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  nand2  g19(.a(new_n41_), .b(new_n38_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n39_), .c(new_n47_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n46_), .O(z2));
  nand2  g22(.a(new_n45_), .b(x02), .O(new_n51_));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n39_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n51_), .O(z3));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  nand3  g30(.a(x14), .b(x13), .c(x11), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n39_), .O(new_n60_));
  nand2  g32(.a(new_n34_), .b(x14), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n32_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n41_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x03), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n63_), .c(new_n60_), .O(z4));
  nand2  g37(.a(new_n45_), .b(x04), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand2  g39(.a(new_n29_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n39_), .c(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n66_), .O(z5));
  nand2  g42(.a(new_n45_), .b(x05), .O(new_n71_));
  nand3  g43(.a(new_n30_), .b(x16), .c(x15), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(new_n31_), .b(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n72_), .c(new_n39_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n71_), .O(z6));
  nand2  g48(.a(new_n45_), .b(x06), .O(new_n77_));
  nor2   g49(.a(new_n73_), .b(new_n67_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n30_), .c(x17), .O(new_n79_));
  inv1   g51(.a(x17), .O(new_n80_));
  nand2  g52(.a(new_n78_), .b(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n79_), .c(new_n39_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(z7));
  nand2  g56(.a(new_n45_), .b(x07), .O(new_n85_));
  nand4  g57(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n52_), .c(x18), .O(new_n87_));
  nor2   g59(.a(x18), .b(new_n80_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n78_), .c(new_n30_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n39_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n85_), .O(z8));
endmodule


