// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand4  g01(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n33_), .c(new_n30_), .d(new_n29_), .O(z0));
  nor2   g07(.a(x18), .b(x17), .O(new_n36_));
  aoi21  g08(.a(x08), .b(x00), .c(new_n36_), .O(new_n37_));
  oai21  g09(.a(new_n33_), .b(x11), .c(new_n37_), .O(z1));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  inv1   g11(.a(new_n33_), .O(new_n40_));
  nand2  g12(.a(x12), .b(x11), .O(new_n41_));
  or2    g13(.a(x12), .b(x11), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n36_), .O(z2));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  inv1   g17(.a(x13), .O(new_n46_));
  nand2  g18(.a(new_n41_), .b(new_n46_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n34_), .c(new_n40_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n45_), .c(new_n36_), .O(z3));
  nand4  g21(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n34_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n40_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x03), .c(new_n36_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z4));
  nand2  g27(.a(x08), .b(x04), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x15), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n40_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n56_), .c(new_n36_), .O(z5));
  aoi21  g35(.a(new_n58_), .b(x15), .c(x16), .O(new_n64_));
  nand2  g36(.a(x16), .b(x15), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n58_), .b(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n40_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n36_), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(new_n64_), .c(new_n69_), .O(z6));
  inv1   g42(.a(new_n50_), .O(new_n71_));
  nor2   g43(.a(new_n33_), .b(new_n65_), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n71_), .c(new_n29_), .O(new_n73_));
  and2   g45(.a(x08), .b(x06), .O(new_n74_));
  nand4  g46(.a(x17), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n67_), .c(new_n74_), .O(new_n77_));
  oai21  g49(.a(new_n73_), .b(x17), .c(new_n77_), .O(z7));
  oai21  g50(.a(new_n34_), .b(new_n30_), .c(x18), .O(new_n79_));
  inv1   g51(.a(x17), .O(new_n80_));
  nor2   g52(.a(x18), .b(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n58_), .c(new_n66_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n40_), .O(new_n84_));
  inv1   g56(.a(new_n36_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(x08), .c(x07), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n84_), .O(z8));
endmodule


