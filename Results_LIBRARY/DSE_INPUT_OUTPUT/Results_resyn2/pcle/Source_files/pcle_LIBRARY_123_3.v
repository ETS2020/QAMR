// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_;
  nand2  g00(.a(x18), .b(x14), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  aoi21  g05(.a(x08), .b(x00), .c(z0), .O(new_n34_));
  oai21  g06(.a(new_n33_), .b(x11), .c(new_n34_), .O(z1));
  xnor2a g07(.a(x12), .b(x11), .O(new_n36_));
  aoi21  g08(.a(x08), .b(x01), .c(z0), .O(new_n37_));
  oai21  g09(.a(new_n36_), .b(new_n33_), .c(new_n37_), .O(z2));
  aoi21  g10(.a(x12), .b(x11), .c(x13), .O(new_n39_));
  inv1   g11(.a(new_n33_), .O(new_n40_));
  nand3  g12(.a(x13), .b(x12), .c(x11), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g14(.a(x08), .b(x02), .c(z0), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(new_n39_), .c(new_n43_), .O(z3));
  nor2   g16(.a(z0), .b(new_n31_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x03), .O(new_n46_));
  inv1   g18(.a(new_n41_), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(x14), .O(new_n48_));
  nand2  g20(.a(new_n47_), .b(x14), .O(new_n49_));
  nand4  g21(.a(new_n29_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n50_));
  inv1   g22(.a(new_n50_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n48_), .c(new_n46_), .O(z4));
  inv1   g25(.a(x15), .O(new_n54_));
  nand2  g26(.a(new_n49_), .b(new_n54_), .O(new_n55_));
  nand4  g27(.a(x15), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  inv1   g28(.a(new_n56_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(x14), .c(new_n33_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x04), .c(z0), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z5));
  nand2  g33(.a(new_n58_), .b(x16), .O(new_n62_));
  inv1   g34(.a(x18), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n56_), .c(new_n63_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x14), .O(new_n67_));
  nand2  g39(.a(x08), .b(x05), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n67_), .c(new_n62_), .O(z6));
  nand2  g41(.a(new_n45_), .b(x06), .O(new_n70_));
  nand3  g42(.a(x16), .b(x15), .c(x14), .O(new_n71_));
  nor2   g43(.a(new_n71_), .b(new_n41_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x17), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  oai21  g46(.a(new_n71_), .b(new_n41_), .c(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n73_), .c(new_n51_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n70_), .O(z7));
  nand2  g49(.a(new_n45_), .b(x07), .O(new_n78_));
  aoi21  g50(.a(new_n72_), .b(x17), .c(x18), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n50_), .c(new_n78_), .O(z8));
endmodule


