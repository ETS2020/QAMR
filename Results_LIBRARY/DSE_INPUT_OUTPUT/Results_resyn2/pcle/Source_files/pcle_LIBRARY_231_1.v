// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x13), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x18), .c(x17), .d(x16), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g08(.a(x17), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x13), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n32_), .b(x11), .c(new_n39_), .O(z1));
  nand2  g12(.a(x08), .b(x01), .O(new_n41_));
  inv1   g13(.a(new_n32_), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n41_), .c(new_n38_), .O(z2));
  nor2   g18(.a(new_n38_), .b(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x02), .O(new_n48_));
  nor2   g20(.a(new_n38_), .b(new_n32_), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g22(.a(new_n43_), .b(new_n29_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n48_), .O(z3));
  nand2  g25(.a(new_n47_), .b(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n50_), .b(new_n55_), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n49_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n54_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand2  g33(.a(new_n43_), .b(x13), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(x15), .c(x14), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n42_), .O(new_n64_));
  oai21  g36(.a(new_n32_), .b(new_n60_), .c(new_n37_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n29_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(z5));
  inv1   g40(.a(new_n38_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  nand4  g42(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n34_), .c(new_n70_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand3  g45(.a(new_n37_), .b(x16), .c(new_n29_), .O(new_n74_));
  inv1   g46(.a(x16), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x15), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n57_), .c(new_n74_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n42_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n73_), .O(z6));
  nand2  g51(.a(new_n47_), .b(x06), .O(new_n80_));
  nor2   g52(.a(new_n33_), .b(new_n75_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x17), .O(new_n82_));
  oai21  g54(.a(new_n33_), .b(new_n75_), .c(new_n37_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n82_), .c(new_n42_), .d(x13), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n80_), .O(z7));
  nand2  g57(.a(new_n47_), .b(x07), .O(new_n86_));
  aoi21  g58(.a(new_n81_), .b(x17), .c(x18), .O(new_n87_));
  nand2  g59(.a(new_n49_), .b(new_n35_), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z8));
endmodule


