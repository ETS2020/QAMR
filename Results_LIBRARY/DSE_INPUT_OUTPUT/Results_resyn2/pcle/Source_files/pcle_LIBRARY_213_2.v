// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x13), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(x12), .b(x11), .O(new_n34_));
  nand3  g06(.a(x17), .b(x16), .c(x15), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n33_), .c(x14), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g10(.a(x18), .b(new_n29_), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n32_), .b(x11), .c(new_n40_), .O(z1));
  or2    g13(.a(x12), .b(x11), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n34_), .c(new_n33_), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x01), .c(new_n39_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(z2));
  inv1   g17(.a(new_n39_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x08), .c(x02), .O(new_n47_));
  aoi22  g19(.a(x18), .b(x13), .c(x12), .d(x11), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n33_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n32_), .c(x18), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x13), .O(new_n55_));
  nand3  g27(.a(new_n49_), .b(new_n33_), .c(x14), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z4));
  nand4  g30(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(x14), .c(x12), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n59_), .c(x18), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x13), .O(new_n63_));
  nand4  g35(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  nand4  g36(.a(x15), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  aoi22  g38(.a(new_n66_), .b(new_n64_), .c(x08), .d(x04), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(z5));
  nand3  g40(.a(new_n46_), .b(x08), .c(x05), .O(new_n69_));
  inv1   g41(.a(x16), .O(new_n70_));
  nand4  g42(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x18), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(x13), .c(new_n70_), .O(new_n73_));
  nand3  g45(.a(x18), .b(new_n70_), .c(x15), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n64_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n73_), .c(new_n33_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n69_), .O(z6));
  inv1   g49(.a(x18), .O(new_n78_));
  inv1   g50(.a(new_n59_), .O(new_n79_));
  nand2  g51(.a(x16), .b(x15), .O(new_n80_));
  nand2  g52(.a(x14), .b(x12), .O(new_n81_));
  nor3   g53(.a(new_n81_), .b(new_n80_), .c(x17), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(new_n79_), .c(new_n78_), .O(new_n83_));
  and2   g55(.a(x08), .b(x06), .O(new_n84_));
  inv1   g56(.a(new_n80_), .O(new_n85_));
  inv1   g57(.a(new_n64_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(x18), .O(new_n87_));
  nand4  g59(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n88_));
  inv1   g60(.a(new_n88_), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n87_), .c(new_n84_), .O(new_n90_));
  oai21  g62(.a(new_n83_), .b(new_n29_), .c(new_n90_), .O(z7));
  nor2   g63(.a(new_n64_), .b(new_n35_), .O(new_n92_));
  nand2  g64(.a(new_n33_), .b(x18), .O(new_n93_));
  aoi21  g65(.a(x08), .b(x07), .c(new_n39_), .O(new_n94_));
  oai21  g66(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z8));
endmodule


