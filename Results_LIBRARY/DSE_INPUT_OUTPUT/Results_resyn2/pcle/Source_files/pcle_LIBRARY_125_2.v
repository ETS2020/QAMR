// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  inv1   g00(.a(x17), .O(new_n29_));
  nand2  g01(.a(new_n29_), .b(x10), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(x16), .b(x15), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n35_));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x17), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n35_), .c(new_n30_), .O(z0));
  inv1   g13(.a(x00), .O(new_n42_));
  nand2  g14(.a(new_n30_), .b(x08), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(new_n42_), .c(new_n38_), .d(x11), .O(z1));
  inv1   g16(.a(x01), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(new_n38_), .c(new_n43_), .d(new_n45_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  inv1   g22(.a(new_n30_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n51_), .O(new_n52_));
  oai21  g24(.a(new_n50_), .b(new_n48_), .c(new_n52_), .O(z3));
  inv1   g25(.a(x03), .O(new_n54_));
  inv1   g26(.a(new_n49_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n39_), .b(new_n31_), .O(new_n57_));
  oai22  g29(.a(new_n57_), .b(new_n56_), .c(new_n43_), .d(new_n54_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n31_), .b(new_n59_), .O(new_n60_));
  oai21  g32(.a(new_n32_), .b(x15), .c(new_n39_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x04), .c(new_n51_), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(z5));
  nor2   g35(.a(new_n60_), .b(x16), .O(new_n64_));
  oai21  g36(.a(new_n33_), .b(new_n31_), .c(new_n39_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x05), .c(new_n51_), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(new_n64_), .c(new_n66_), .O(z6));
  nand3  g39(.a(new_n30_), .b(x08), .c(x06), .O(new_n68_));
  nor2   g40(.a(new_n33_), .b(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x17), .O(new_n70_));
  oai21  g42(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n39_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n68_), .O(z7));
  aoi21  g45(.a(new_n69_), .b(x17), .c(x18), .O(new_n74_));
  nand2  g46(.a(new_n39_), .b(new_n35_), .O(new_n75_));
  nand3  g47(.a(x18), .b(x09), .c(new_n36_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n37_), .O(new_n77_));
  and2   g49(.a(x08), .b(x07), .O(new_n78_));
  aoi21  g50(.a(new_n77_), .b(new_n29_), .c(new_n78_), .O(new_n79_));
  oai21  g51(.a(new_n75_), .b(new_n74_), .c(new_n79_), .O(z8));
endmodule


