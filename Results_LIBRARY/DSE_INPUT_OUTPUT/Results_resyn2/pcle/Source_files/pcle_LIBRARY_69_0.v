// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x18), .b(x17), .c(x16), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n33_), .c(x15), .d(x14), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nor2   g09(.a(x18), .b(x13), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(new_n31_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n39_), .c(new_n38_), .O(z1));
  nor2   g15(.a(new_n38_), .b(new_n29_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nor2   g18(.a(new_n38_), .b(new_n31_), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n40_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n45_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand2  g24(.a(new_n32_), .b(new_n41_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n38_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(z3));
  inv1   g27(.a(x13), .O(new_n56_));
  nand4  g28(.a(x14), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x18), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  inv1   g32(.a(x14), .O(new_n61_));
  nand2  g33(.a(new_n32_), .b(new_n61_), .O(new_n62_));
  nand3  g34(.a(x14), .b(x12), .c(x11), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n41_), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n60_), .c(new_n59_), .O(z4));
  nand2  g37(.a(new_n44_), .b(x04), .O(new_n66_));
  nand3  g38(.a(new_n33_), .b(x15), .c(x14), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  oai21  g40(.a(new_n32_), .b(new_n61_), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n47_), .c(new_n67_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n66_), .O(z5));
  nand2  g43(.a(new_n44_), .b(x05), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(x15), .b(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n32_), .c(new_n73_), .O(new_n75_));
  nand3  g47(.a(x16), .b(x15), .c(x14), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n33_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n75_), .c(new_n47_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n72_), .O(z6));
  inv1   g52(.a(x17), .O(new_n81_));
  oai21  g53(.a(new_n76_), .b(new_n32_), .c(new_n81_), .O(new_n82_));
  nand3  g54(.a(x17), .b(x12), .c(x11), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n82_), .c(new_n41_), .O(new_n86_));
  nand4  g58(.a(x17), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(x18), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n56_), .O(new_n89_));
  nand2  g61(.a(x08), .b(x06), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n89_), .c(new_n86_), .O(z7));
  nand2  g63(.a(new_n44_), .b(x07), .O(new_n92_));
  inv1   g64(.a(x18), .O(new_n93_));
  oai21  g65(.a(new_n83_), .b(new_n76_), .c(new_n93_), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(new_n47_), .c(new_n36_), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(new_n92_), .O(z8));
endmodule


