// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  inv1   g04(.a(x15), .O(new_n33_));
  inv1   g05(.a(x16), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n32_), .c(new_n30_), .d(x18), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x17), .c(x10), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  oai21  g10(.a(x17), .b(x10), .c(x08), .O(new_n39_));
  inv1   g11(.a(x10), .O(new_n40_));
  nand3  g12(.a(new_n32_), .b(x17), .c(new_n40_), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(x11), .c(new_n39_), .d(new_n38_), .O(z1));
  inv1   g14(.a(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai22  g16(.a(new_n44_), .b(new_n41_), .c(new_n39_), .d(new_n43_), .O(z2));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  nand2  g19(.a(new_n32_), .b(new_n47_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n46_), .c(x17), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n40_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  inv1   g25(.a(new_n41_), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n47_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n54_), .c(new_n29_), .O(new_n57_));
  oai21  g29(.a(new_n39_), .b(new_n53_), .c(new_n57_), .O(z4));
  inv1   g30(.a(x17), .O(new_n59_));
  nand2  g31(.a(new_n30_), .b(x15), .O(new_n60_));
  inv1   g32(.a(x08), .O(new_n61_));
  nand2  g33(.a(x09), .b(new_n61_), .O(new_n62_));
  aoi21  g34(.a(new_n29_), .b(new_n33_), .c(new_n62_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(x10), .c(new_n65_), .O(z5));
  nand2  g38(.a(x15), .b(x14), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n47_), .c(new_n34_), .O(new_n68_));
  nand3  g40(.a(x16), .b(x15), .c(x14), .O(new_n69_));
  nor2   g41(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  nor2   g42(.a(new_n70_), .b(new_n62_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n68_), .c(new_n59_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x05), .O(new_n73_));
  oai21  g45(.a(new_n72_), .b(x10), .c(new_n73_), .O(z6));
  inv1   g46(.a(x06), .O(new_n75_));
  oai22  g47(.a(new_n70_), .b(new_n41_), .c(new_n39_), .d(new_n75_), .O(z7));
  inv1   g48(.a(new_n47_), .O(new_n77_));
  inv1   g49(.a(new_n69_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(x18), .O(new_n79_));
  inv1   g51(.a(x18), .O(new_n80_));
  oai21  g52(.a(new_n69_), .b(new_n47_), .c(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n79_), .c(new_n32_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x17), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n40_), .O(new_n84_));
  nand2  g56(.a(x08), .b(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


