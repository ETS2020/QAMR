// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x15), .O(new_n32_));
  inv1   g04(.a(x16), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n31_), .c(x17), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  inv1   g08(.a(x09), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x08), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n36_), .c(x05), .O(new_n39_));
  nor3   g11(.a(new_n39_), .b(new_n35_), .c(new_n29_), .O(z0));
  inv1   g12(.a(x05), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  aoi21  g14(.a(new_n38_), .b(new_n42_), .c(new_n41_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(x10), .c(new_n44_), .O(z1));
  inv1   g17(.a(x08), .O(new_n46_));
  aoi21  g18(.a(new_n36_), .b(new_n41_), .c(new_n46_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x01), .O(new_n48_));
  xnor2a g20(.a(x12), .b(x11), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n39_), .c(new_n48_), .O(z2));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand2  g24(.a(new_n38_), .b(new_n52_), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n51_), .c(x05), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n36_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x02), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z3));
  nand2  g29(.a(new_n47_), .b(x03), .O(new_n58_));
  inv1   g30(.a(new_n39_), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n52_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n30_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n58_), .O(z4));
  nand2  g35(.a(new_n30_), .b(new_n32_), .O(new_n64_));
  nand2  g36(.a(x09), .b(new_n46_), .O(new_n65_));
  aoi21  g37(.a(new_n31_), .b(x15), .c(new_n65_), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n64_), .c(new_n41_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x04), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(x10), .c(new_n68_), .O(z5));
  oai21  g41(.a(new_n30_), .b(new_n32_), .c(new_n33_), .O(new_n70_));
  aoi21  g42(.a(new_n34_), .b(new_n31_), .c(new_n37_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n70_), .c(new_n41_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x05), .O(new_n73_));
  oai21  g45(.a(new_n72_), .b(x10), .c(new_n73_), .O(z6));
  nand2  g46(.a(new_n47_), .b(x06), .O(new_n75_));
  nor3   g47(.a(new_n30_), .b(new_n33_), .c(new_n32_), .O(new_n76_));
  nor2   g48(.a(new_n76_), .b(x17), .O(new_n77_));
  nand2  g49(.a(new_n59_), .b(new_n35_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n77_), .c(new_n75_), .O(z7));
  nand2  g51(.a(new_n47_), .b(x07), .O(new_n80_));
  nand3  g52(.a(new_n76_), .b(x18), .c(x17), .O(new_n81_));
  aoi21  g53(.a(new_n35_), .b(new_n29_), .c(new_n39_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n80_), .O(z8));
endmodule


