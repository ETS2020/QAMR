// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_;
  inv1   g00(.a(x09), .O(new_n29_));
  nor2   g01(.a(x10), .b(x08), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand2  g03(.a(x15), .b(x14), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x17), .c(x16), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x18), .c(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  oai21  g10(.a(x18), .b(new_n29_), .c(x08), .O(new_n39_));
  nand3  g11(.a(new_n30_), .b(x18), .c(x09), .O(new_n40_));
  oai22  g12(.a(new_n40_), .b(x11), .c(new_n39_), .d(new_n38_), .O(z1));
  nor2   g13(.a(x12), .b(x11), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n30_), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n42_), .c(x18), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x09), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  inv1   g20(.a(new_n39_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x02), .O(new_n50_));
  inv1   g22(.a(new_n40_), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n43_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n31_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n50_), .O(z3));
  nand2  g27(.a(new_n49_), .b(x03), .O(new_n56_));
  inv1   g28(.a(new_n31_), .O(new_n57_));
  nor2   g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand2  g30(.a(new_n57_), .b(x14), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n51_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(z4));
  nand2  g33(.a(new_n49_), .b(x04), .O(new_n62_));
  inv1   g34(.a(new_n59_), .O(new_n63_));
  nor2   g35(.a(new_n40_), .b(new_n33_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(x15), .c(new_n64_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n62_), .O(z5));
  inv1   g38(.a(new_n32_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n57_), .c(x16), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n32_), .b(new_n31_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n30_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x18), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  nand2  g47(.a(new_n49_), .b(x06), .O(new_n76_));
  nand2  g48(.a(new_n51_), .b(new_n34_), .O(new_n77_));
  aoi21  g49(.a(new_n33_), .b(x16), .c(x17), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(z7));
  nand2  g51(.a(new_n49_), .b(x07), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n77_), .O(z8));
endmodule


