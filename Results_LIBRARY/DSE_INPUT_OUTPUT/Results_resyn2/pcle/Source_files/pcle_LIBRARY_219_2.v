// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  and2   g03(.a(x17), .b(x16), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nand2  g06(.a(x15), .b(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n33_), .c(new_n29_), .O(z0));
  inv1   g10(.a(x02), .O(new_n39_));
  nor2   g11(.a(x18), .b(new_n39_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n43_));
  inv1   g15(.a(new_n43_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n47_), .c(new_n44_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x01), .c(new_n40_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  oai21  g24(.a(new_n29_), .b(x08), .c(x02), .O(new_n53_));
  aoi21  g25(.a(x12), .b(x11), .c(x13), .O(new_n54_));
  nand2  g26(.a(new_n34_), .b(new_n44_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z3));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  inv1   g29(.a(new_n34_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(x14), .c(new_n43_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(x14), .c(new_n59_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n57_), .c(new_n40_), .O(z4));
  aoi21  g33(.a(new_n58_), .b(x14), .c(x15), .O(new_n62_));
  nand2  g34(.a(new_n37_), .b(new_n44_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n40_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n62_), .c(new_n64_), .O(z5));
  inv1   g37(.a(new_n35_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n58_), .c(x16), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  oai21  g40(.a(new_n36_), .b(x16), .c(new_n44_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n40_), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n68_), .c(new_n70_), .O(z6));
  aoi21  g43(.a(new_n36_), .b(new_n32_), .c(new_n43_), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n67_), .b(new_n73_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x06), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n75_), .c(new_n40_), .O(z7));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  nand3  g50(.a(new_n66_), .b(new_n58_), .c(new_n29_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n33_), .c(new_n78_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n39_), .O(new_n81_));
  inv1   g53(.a(new_n78_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n72_), .c(x18), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n81_), .O(z8));
endmodule


