// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand2  g01(.a(x12), .b(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand3  g03(.a(x15), .b(x14), .c(x13), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(new_n31_), .c(x16), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand4  g08(.a(x17), .b(new_n36_), .c(x09), .d(new_n35_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  nand3  g10(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n39_));
  nor2   g11(.a(new_n29_), .b(x14), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  inv1   g15(.a(new_n39_), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n30_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n43_), .c(new_n40_), .O(z2));
  nor2   g19(.a(new_n31_), .b(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n40_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x14), .O(new_n53_));
  oai21  g25(.a(new_n49_), .b(new_n39_), .c(new_n29_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g27(.a(new_n49_), .b(new_n44_), .c(x14), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n49_), .b(new_n59_), .O(new_n60_));
  nand4  g32(.a(x15), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n60_), .c(new_n44_), .d(x14), .O(new_n62_));
  oai21  g34(.a(new_n39_), .b(new_n59_), .c(new_n29_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n53_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(z5));
  inv1   g38(.a(new_n40_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n32_), .b(new_n30_), .c(new_n69_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n67_), .c(new_n44_), .d(new_n34_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n68_), .O(z6));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n34_), .b(new_n73_), .O(new_n74_));
  inv1   g46(.a(new_n61_), .O(new_n75_));
  nand2  g47(.a(x17), .b(x16), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n75_), .c(new_n39_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand2  g51(.a(new_n37_), .b(new_n29_), .O(new_n80_));
  and2   g52(.a(x08), .b(x06), .O(new_n81_));
  aoi21  g53(.a(new_n80_), .b(new_n53_), .c(new_n81_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(z7));
  nand3  g55(.a(new_n67_), .b(x08), .c(x07), .O(new_n84_));
  nor3   g56(.a(new_n76_), .b(new_n61_), .c(new_n29_), .O(new_n85_));
  oai21  g57(.a(new_n76_), .b(new_n61_), .c(new_n29_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n44_), .c(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(z8));
endmodule


