// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  nand2  g03(.a(x08), .b(x00), .O(new_n32_));
  inv1   g04(.a(x11), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n32_), .c(z0), .O(z1));
  xnor2a g11(.a(x12), .b(x11), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x01), .c(z0), .O(new_n41_));
  oai21  g13(.a(new_n40_), .b(new_n36_), .c(new_n41_), .O(z2));
  nand2  g14(.a(x08), .b(x02), .O(new_n43_));
  inv1   g15(.a(x13), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n37_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n43_), .c(z0), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  nand4  g22(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n51_));
  inv1   g23(.a(x14), .O(new_n52_));
  nand2  g24(.a(new_n47_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n37_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n50_), .c(z0), .O(z4));
  nand2  g27(.a(x08), .b(x04), .O(new_n56_));
  inv1   g28(.a(new_n51_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x15), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n51_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n37_), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n56_), .c(z0), .O(z5));
  aoi21  g34(.a(new_n57_), .b(x15), .c(x16), .O(new_n63_));
  nand3  g35(.a(new_n57_), .b(x16), .c(x15), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n37_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x05), .c(z0), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(z6));
  nand4  g39(.a(new_n57_), .b(x17), .c(x16), .d(x15), .O(new_n68_));
  nand2  g40(.a(new_n64_), .b(new_n29_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n68_), .c(new_n37_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x06), .c(z0), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z7));
  nand2  g44(.a(new_n68_), .b(new_n30_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n37_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x07), .c(z0), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z8));
endmodule


