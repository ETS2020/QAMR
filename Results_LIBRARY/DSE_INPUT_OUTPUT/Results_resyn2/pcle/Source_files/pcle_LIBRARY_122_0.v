// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand2  g03(.a(x16), .b(x15), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x14), .c(x13), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(x12), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n37_), .O(new_n41_));
  nor3   g13(.a(new_n41_), .b(new_n34_), .c(new_n31_), .O(z0));
  nor2   g14(.a(x18), .b(x15), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x00), .c(new_n43_), .O(new_n44_));
  oai21  g16(.a(new_n31_), .b(x11), .c(new_n44_), .O(z1));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n43_), .O(new_n47_));
  oai21  g19(.a(new_n46_), .b(new_n31_), .c(new_n47_), .O(z2));
  nor2   g20(.a(new_n40_), .b(x13), .O(new_n49_));
  inv1   g21(.a(new_n31_), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n43_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(z3));
  inv1   g26(.a(new_n51_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  aoi21  g30(.a(x08), .b(x03), .c(new_n43_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  nor2   g32(.a(new_n43_), .b(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x04), .O(new_n62_));
  inv1   g34(.a(x15), .O(new_n63_));
  nor2   g35(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n57_), .b(new_n63_), .O(new_n65_));
  nor2   g37(.a(new_n43_), .b(new_n31_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n64_), .c(new_n62_), .O(z5));
  nand2  g40(.a(new_n61_), .b(x05), .O(new_n69_));
  nor2   g41(.a(new_n64_), .b(x16), .O(new_n70_));
  inv1   g42(.a(new_n57_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n33_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n66_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n70_), .c(new_n69_), .O(z6));
  nand2  g46(.a(new_n61_), .b(x06), .O(new_n75_));
  nand3  g47(.a(new_n71_), .b(new_n33_), .c(x17), .O(new_n76_));
  oai21  g48(.a(new_n57_), .b(new_n32_), .c(new_n35_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n66_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n75_), .O(z7));
  nand2  g51(.a(x16), .b(x14), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n55_), .c(x17), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n36_), .O(new_n83_));
  nor2   g55(.a(new_n80_), .b(new_n51_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n37_), .c(new_n31_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g58(.a(new_n31_), .b(x18), .O(new_n87_));
  aoi22  g59(.a(new_n87_), .b(new_n63_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(z8));
endmodule


