// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x04), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  nand4  g07(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g11(.a(x18), .b(x04), .O(new_n40_));
  inv1   g12(.a(new_n40_), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n33_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n34_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n44_), .c(new_n41_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n35_), .b(new_n34_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n41_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  nand3  g24(.a(new_n40_), .b(x08), .c(x03), .O(new_n53_));
  nand4  g25(.a(new_n40_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n54_));
  xor2a  g26(.a(new_n35_), .b(x14), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z4));
  nand3  g28(.a(new_n30_), .b(x08), .c(x04), .O(new_n57_));
  inv1   g29(.a(new_n35_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(x14), .c(x15), .O(new_n59_));
  inv1   g31(.a(new_n54_), .O(new_n60_));
  and2   g32(.a(x15), .b(x14), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n59_), .c(new_n57_), .O(z5));
  nand3  g36(.a(new_n61_), .b(new_n58_), .c(x16), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n62_), .b(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n34_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n41_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z6));
  nand2  g42(.a(x08), .b(x06), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  inv1   g45(.a(new_n36_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n58_), .c(new_n33_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n71_), .c(new_n41_), .O(z7));
  nand3  g49(.a(new_n40_), .b(x08), .c(x07), .O(new_n78_));
  nand2  g50(.a(new_n37_), .b(x18), .O(new_n79_));
  oai21  g51(.a(new_n36_), .b(new_n35_), .c(new_n30_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(new_n60_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n78_), .O(z8));
endmodule


