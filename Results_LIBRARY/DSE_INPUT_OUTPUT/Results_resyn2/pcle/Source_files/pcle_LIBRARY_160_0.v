// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  nor2   g07(.a(x18), .b(x13), .O(new_n36_));
  nand2  g08(.a(x08), .b(x00), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(new_n34_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n37_), .c(new_n36_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n39_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n42_), .c(new_n36_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand2  g20(.a(new_n39_), .b(new_n30_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n36_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n48_), .c(new_n50_), .O(z3));
  nor2   g23(.a(new_n36_), .b(new_n32_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x03), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n30_), .b(new_n54_), .O(new_n55_));
  nor2   g27(.a(new_n36_), .b(new_n34_), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n53_), .O(z4));
  nand2  g31(.a(new_n52_), .b(x04), .O(new_n60_));
  nand4  g32(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n61_));
  inv1   g33(.a(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x13), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand2  g36(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n56_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n60_), .O(z5));
  inv1   g39(.a(x13), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n61_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  nand2  g42(.a(new_n62_), .b(x16), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n39_), .O(new_n72_));
  oai21  g44(.a(new_n34_), .b(new_n69_), .c(x18), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x05), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n72_), .O(z6));
  nand2  g48(.a(new_n52_), .b(x06), .O(new_n77_));
  inv1   g49(.a(new_n57_), .O(new_n78_));
  nor2   g50(.a(new_n69_), .b(new_n64_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n78_), .c(x17), .O(new_n80_));
  nand3  g52(.a(new_n79_), .b(new_n78_), .c(x17), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n56_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(z7));
  nand2  g55(.a(new_n52_), .b(x07), .O(new_n84_));
  oai21  g56(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n85_));
  inv1   g57(.a(x17), .O(new_n86_));
  nor2   g58(.a(x18), .b(new_n86_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n79_), .c(new_n78_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n39_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n84_), .O(z8));
endmodule


