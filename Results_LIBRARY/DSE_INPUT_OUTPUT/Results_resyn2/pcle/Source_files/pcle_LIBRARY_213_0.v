// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nor2   g00(.a(x18), .b(x13), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x15), .b(x14), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(new_n32_), .c(x16), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(x18), .c(x17), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n34_), .c(new_n30_), .O(z0));
  aoi21  g12(.a(x08), .b(x00), .c(new_n29_), .O(new_n41_));
  oai21  g13(.a(new_n37_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n38_), .c(new_n43_), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z2));
  nand3  g19(.a(new_n30_), .b(x08), .c(x02), .O(new_n48_));
  nor2   g20(.a(new_n37_), .b(new_n29_), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n43_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n31_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n48_), .O(z3));
  inv1   g25(.a(x14), .O(new_n54_));
  oai21  g26(.a(new_n37_), .b(new_n54_), .c(x18), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand2  g29(.a(new_n31_), .b(new_n54_), .O(new_n58_));
  nand3  g30(.a(x14), .b(x12), .c(x11), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n38_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n57_), .c(new_n56_), .O(z4));
  inv1   g33(.a(new_n59_), .O(new_n62_));
  oai21  g34(.a(x15), .b(new_n50_), .c(new_n62_), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  aoi21  g36(.a(new_n59_), .b(new_n64_), .c(new_n37_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  oai21  g38(.a(new_n37_), .b(new_n64_), .c(x18), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n50_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x04), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z5));
  nand3  g42(.a(new_n30_), .b(x08), .c(x05), .O(new_n71_));
  aoi21  g43(.a(new_n33_), .b(new_n32_), .c(x16), .O(new_n72_));
  nand2  g44(.a(new_n49_), .b(new_n34_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(z6));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n34_), .b(new_n75_), .O(new_n76_));
  nand3  g48(.a(x17), .b(x16), .c(x15), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(new_n37_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand4  g52(.a(x17), .b(new_n36_), .c(x09), .d(new_n35_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x18), .O(new_n82_));
  and2   g54(.a(x08), .b(x06), .O(new_n83_));
  aoi21  g55(.a(new_n82_), .b(new_n50_), .c(new_n83_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n80_), .O(z7));
  nand2  g57(.a(new_n78_), .b(x18), .O(new_n86_));
  inv1   g58(.a(x18), .O(new_n87_));
  oai21  g59(.a(new_n77_), .b(new_n59_), .c(new_n87_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n86_), .c(new_n38_), .O(new_n89_));
  oai21  g61(.a(new_n38_), .b(new_n87_), .c(new_n50_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(z8));
endmodule


