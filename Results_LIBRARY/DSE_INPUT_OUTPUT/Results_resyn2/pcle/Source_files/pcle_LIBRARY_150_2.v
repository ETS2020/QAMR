// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_;
  inv1   g00(.a(x07), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  inv1   g06(.a(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n36_), .c(x18), .d(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n33_), .O(z0));
  inv1   g12(.a(x11), .O(new_n41_));
  aoi21  g13(.a(new_n32_), .b(new_n41_), .c(x07), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(x10), .c(new_n43_), .O(z1));
  nor2   g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(x08), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n45_), .c(new_n29_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n30_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x01), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  aoi21  g24(.a(new_n30_), .b(x07), .c(new_n46_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x02), .O(new_n54_));
  aoi21  g26(.a(x12), .b(x11), .c(x13), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(new_n32_), .c(new_n30_), .d(new_n29_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z3));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n56_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n37_), .c(new_n32_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n29_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n30_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x03), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z4));
  nand2  g37(.a(new_n53_), .b(x04), .O(new_n66_));
  inv1   g38(.a(new_n33_), .O(new_n67_));
  nand2  g39(.a(new_n37_), .b(new_n34_), .O(new_n68_));
  nand2  g40(.a(new_n38_), .b(x15), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n66_), .O(z5));
  nand2  g43(.a(new_n53_), .b(x05), .O(new_n72_));
  nand2  g44(.a(new_n38_), .b(new_n36_), .O(new_n73_));
  nand2  g45(.a(new_n69_), .b(new_n35_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n67_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n72_), .O(z6));
  nand2  g48(.a(new_n53_), .b(x06), .O(new_n77_));
  nand3  g49(.a(new_n38_), .b(new_n36_), .c(x17), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nand2  g51(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n67_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n77_), .O(z7));
  inv1   g54(.a(x18), .O(new_n83_));
  nand3  g55(.a(x17), .b(x16), .c(x15), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n37_), .c(new_n83_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n39_), .c(new_n67_), .O(new_n86_));
  nand3  g58(.a(x10), .b(x08), .c(x07), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n86_), .O(z8));
endmodule


