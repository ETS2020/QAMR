// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand2  g01(.a(x15), .b(x14), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g03(.a(x09), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(x08), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n31_), .c(x18), .d(x16), .O(new_n34_));
  aoi21  g06(.a(new_n34_), .b(x17), .c(x10), .O(z0));
  inv1   g07(.a(x11), .O(new_n36_));
  inv1   g08(.a(x17), .O(new_n37_));
  aoi21  g09(.a(new_n33_), .b(new_n36_), .c(new_n37_), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  oai21  g11(.a(new_n38_), .b(x10), .c(new_n39_), .O(z1));
  inv1   g12(.a(x10), .O(new_n41_));
  nor2   g13(.a(x12), .b(x11), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(new_n33_), .b(new_n43_), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n42_), .c(x17), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n41_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  inv1   g20(.a(x02), .O(new_n49_));
  oai21  g21(.a(x17), .b(x10), .c(x08), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n43_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n33_), .b(x17), .c(new_n41_), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n29_), .O(new_n55_));
  oai21  g27(.a(new_n50_), .b(new_n49_), .c(new_n55_), .O(z3));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n29_), .b(new_n57_), .O(new_n58_));
  inv1   g30(.a(new_n29_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x14), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n33_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x17), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n41_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x03), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z4));
  inv1   g37(.a(x15), .O(new_n66_));
  nand2  g38(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  inv1   g39(.a(x08), .O(new_n68_));
  nand2  g40(.a(x09), .b(new_n68_), .O(new_n69_));
  nor2   g41(.a(new_n69_), .b(new_n31_), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n67_), .c(new_n37_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x04), .O(new_n72_));
  oai21  g44(.a(new_n71_), .b(x10), .c(new_n72_), .O(z5));
  inv1   g45(.a(x05), .O(new_n74_));
  or2    g46(.a(new_n50_), .b(new_n74_), .O(new_n75_));
  nor2   g47(.a(new_n31_), .b(x16), .O(new_n76_));
  nand3  g48(.a(x16), .b(x15), .c(x14), .O(new_n77_));
  inv1   g49(.a(new_n77_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n59_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n54_), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n76_), .c(new_n75_), .O(z6));
  aoi21  g53(.a(new_n79_), .b(new_n33_), .c(new_n37_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x06), .O(new_n83_));
  oai21  g55(.a(new_n82_), .b(x10), .c(new_n83_), .O(z7));
  nand3  g56(.a(new_n78_), .b(new_n59_), .c(x18), .O(new_n85_));
  inv1   g57(.a(x18), .O(new_n86_));
  oai21  g58(.a(new_n77_), .b(new_n29_), .c(new_n86_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n85_), .c(new_n33_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x17), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n41_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z8));
endmodule


