// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x16), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  nand3  g04(.a(x18), .b(x17), .c(x15), .O(new_n33_));
  nor3   g05(.a(new_n33_), .b(new_n32_), .c(new_n31_), .O(z0));
  inv1   g06(.a(x17), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x15), .O(new_n36_));
  nand2  g08(.a(x08), .b(x00), .O(new_n37_));
  inv1   g09(.a(x11), .O(new_n38_));
  nand3  g10(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  aoi21  g13(.a(new_n41_), .b(new_n37_), .c(new_n36_), .O(z1));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n43_), .c(new_n40_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n36_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n40_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n36_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n50_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n32_), .c(new_n40_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n36_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  oai21  g31(.a(new_n32_), .b(new_n39_), .c(new_n35_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand4  g33(.a(x15), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n62_));
  inv1   g34(.a(new_n62_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n32_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(z5));
  inv1   g38(.a(new_n32_), .O(new_n67_));
  oai21  g39(.a(x16), .b(new_n59_), .c(new_n67_), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  aoi21  g41(.a(new_n32_), .b(new_n69_), .c(new_n39_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g43(.a(new_n31_), .b(new_n35_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n59_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z6));
  inv1   g47(.a(new_n36_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x08), .c(x06), .O(new_n77_));
  inv1   g49(.a(new_n50_), .O(new_n78_));
  and2   g50(.a(x16), .b(x14), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n78_), .c(x17), .O(new_n80_));
  nand3  g52(.a(new_n79_), .b(new_n78_), .c(x17), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n63_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n80_), .c(new_n77_), .O(z7));
  nand3  g55(.a(new_n76_), .b(x08), .c(x07), .O(new_n84_));
  nand4  g56(.a(new_n79_), .b(new_n78_), .c(x18), .d(x17), .O(new_n85_));
  inv1   g57(.a(x18), .O(new_n86_));
  nand2  g58(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nor2   g59(.a(new_n36_), .b(new_n39_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n85_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n84_), .O(z8));
endmodule


