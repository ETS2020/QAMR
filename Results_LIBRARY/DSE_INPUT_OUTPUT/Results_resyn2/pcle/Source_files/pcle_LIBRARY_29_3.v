// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_;
  inv1   g00(.a(x16), .O(new_n30_));
  inv1   g01(.a(x18), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g03(.a(x08), .b(x00), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  inv1   g05(.a(x08), .O(new_n35_));
  inv1   g06(.a(x10), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  aoi21  g10(.a(new_n39_), .b(new_n33_), .c(new_n32_), .O(z1));
  nand2  g11(.a(x08), .b(x01), .O(new_n41_));
  nand2  g12(.a(x12), .b(x11), .O(new_n42_));
  inv1   g13(.a(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n34_), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(new_n41_), .c(new_n32_), .O(z2));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n38_), .O(new_n49_));
  aoi21  g20(.a(x08), .b(x02), .c(new_n32_), .O(new_n50_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g22(.a(new_n48_), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(x14), .O(new_n53_));
  nand2  g24(.a(new_n52_), .b(x14), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n32_), .O(new_n56_));
  oai21  g27(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(z4));
  aoi21  g28(.a(new_n52_), .b(x14), .c(x15), .O(new_n58_));
  nand2  g29(.a(x15), .b(x14), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n52_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n38_), .O(new_n62_));
  aoi21  g33(.a(x08), .b(x04), .c(new_n32_), .O(new_n63_));
  oai21  g34(.a(new_n62_), .b(new_n58_), .c(new_n63_), .O(z5));
  nor2   g35(.a(new_n59_), .b(new_n48_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x16), .O(new_n66_));
  nand2  g37(.a(new_n61_), .b(new_n30_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n66_), .c(new_n38_), .O(new_n68_));
  aoi21  g39(.a(x08), .b(x05), .c(new_n32_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(z6));
  aoi21  g41(.a(new_n65_), .b(x16), .c(x17), .O(new_n71_));
  inv1   g42(.a(new_n32_), .O(new_n72_));
  nand4  g43(.a(new_n60_), .b(new_n52_), .c(x17), .d(x16), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(new_n38_), .c(new_n72_), .O(new_n74_));
  nand3  g45(.a(new_n72_), .b(x08), .c(x06), .O(new_n75_));
  oai21  g46(.a(new_n74_), .b(new_n71_), .c(new_n75_), .O(z7));
  nand2  g47(.a(new_n73_), .b(new_n31_), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n38_), .O(new_n78_));
  aoi21  g49(.a(x08), .b(x07), .c(new_n32_), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n78_), .O(z8));
  zero   g51(.O(z0));
endmodule


