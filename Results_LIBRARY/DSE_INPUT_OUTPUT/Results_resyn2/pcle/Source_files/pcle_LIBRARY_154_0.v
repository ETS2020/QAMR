// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x16), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nand2  g06(.a(x15), .b(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x18), .c(x17), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n33_), .O(z0));
  nor2   g10(.a(x16), .b(x05), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand2  g13(.a(new_n32_), .b(new_n41_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n41_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n45_), .c(new_n32_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n39_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand2  g22(.a(new_n34_), .b(new_n32_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n39_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(z3));
  inv1   g25(.a(new_n34_), .O(new_n54_));
  nor2   g26(.a(new_n54_), .b(x14), .O(new_n55_));
  nand2  g27(.a(new_n54_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n32_), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x03), .c(new_n39_), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(new_n55_), .c(new_n58_), .O(z4));
  inv1   g31(.a(new_n56_), .O(new_n60_));
  nor2   g32(.a(new_n36_), .b(new_n31_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(x15), .c(new_n61_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n39_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z5));
  inv1   g36(.a(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n30_), .c(x09), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n36_), .c(x08), .O(new_n69_));
  oai22  g41(.a(new_n69_), .b(new_n65_), .c(new_n36_), .d(new_n33_), .O(z6));
  nand2  g42(.a(new_n66_), .b(new_n30_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x06), .O(new_n72_));
  nand3  g44(.a(x17), .b(x09), .c(new_n29_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n71_), .c(new_n72_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(x05), .O(new_n75_));
  inv1   g47(.a(new_n35_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n54_), .c(x17), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  oai21  g50(.a(new_n35_), .b(new_n34_), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n32_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n75_), .O(z7));
  inv1   g55(.a(x18), .O(new_n84_));
  oai21  g56(.a(new_n77_), .b(new_n66_), .c(new_n84_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n37_), .c(new_n32_), .O(new_n86_));
  oai21  g58(.a(new_n31_), .b(new_n84_), .c(x05), .O(new_n87_));
  aoi22  g59(.a(new_n87_), .b(new_n66_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(z8));
endmodule


