// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x16), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(x18), .c(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n34_), .O(z0));
  nor2   g12(.a(x18), .b(x12), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n37_), .b(x11), .c(new_n42_), .O(z1));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n41_), .O(new_n45_));
  oai21  g17(.a(new_n44_), .b(new_n37_), .c(new_n45_), .O(z2));
  nor2   g18(.a(new_n41_), .b(new_n35_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x02), .O(new_n48_));
  nor2   g20(.a(new_n41_), .b(new_n37_), .O(new_n49_));
  inv1   g21(.a(new_n49_), .O(new_n50_));
  nand2  g22(.a(x12), .b(x11), .O(new_n51_));
  xor2a  g23(.a(new_n51_), .b(x13), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(z3));
  inv1   g25(.a(x12), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  oai21  g27(.a(new_n37_), .b(new_n55_), .c(x18), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  inv1   g30(.a(x13), .O(new_n59_));
  oai21  g31(.a(new_n51_), .b(new_n59_), .c(new_n55_), .O(new_n60_));
  nand3  g32(.a(x14), .b(x13), .c(x11), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n60_), .c(new_n38_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n58_), .c(new_n57_), .O(z4));
  nand2  g35(.a(new_n47_), .b(x04), .O(new_n64_));
  nand2  g36(.a(new_n32_), .b(new_n29_), .O(new_n65_));
  nand2  g37(.a(new_n33_), .b(x15), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n49_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n64_), .O(z5));
  nand2  g40(.a(new_n47_), .b(x05), .O(new_n69_));
  nand2  g41(.a(new_n66_), .b(new_n30_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n49_), .c(new_n34_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n69_), .O(z6));
  nand2  g44(.a(new_n47_), .b(x06), .O(new_n73_));
  inv1   g45(.a(new_n61_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n31_), .c(x17), .d(x12), .O(new_n75_));
  inv1   g47(.a(x17), .O(new_n76_));
  nand3  g48(.a(new_n74_), .b(new_n31_), .c(x12), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n75_), .c(new_n49_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n73_), .O(z7));
  nand2  g52(.a(new_n47_), .b(x07), .O(new_n81_));
  nand4  g53(.a(new_n33_), .b(new_n31_), .c(x18), .d(x17), .O(new_n82_));
  inv1   g54(.a(x18), .O(new_n83_));
  nand3  g55(.a(new_n33_), .b(new_n31_), .c(x17), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n82_), .c(new_n38_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n81_), .O(z8));
endmodule


