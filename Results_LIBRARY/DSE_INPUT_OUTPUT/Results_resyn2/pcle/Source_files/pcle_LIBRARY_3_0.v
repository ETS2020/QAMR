// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x11), .O(new_n29_));
  inv1   g01(.a(x17), .O(new_n30_));
  nand2  g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g03(.a(x13), .b(x12), .O(new_n32_));
  nand3  g04(.a(x16), .b(x15), .c(x14), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  and2   g10(.a(x17), .b(x11), .O(new_n39_));
  nand4  g11(.a(new_n39_), .b(new_n38_), .c(new_n34_), .d(x18), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n31_), .O(z0));
  oai21  g13(.a(new_n38_), .b(new_n30_), .c(new_n29_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n42_), .O(z1));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(new_n31_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n46_), .O(new_n47_));
  oai21  g19(.a(new_n45_), .b(new_n37_), .c(new_n47_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  inv1   g21(.a(new_n32_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n46_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  oai21  g27(.a(new_n37_), .b(new_n55_), .c(x17), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand2  g30(.a(new_n51_), .b(new_n55_), .O(new_n59_));
  aoi21  g31(.a(new_n50_), .b(x14), .c(new_n37_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(z4));
  nand3  g34(.a(new_n31_), .b(x08), .c(x04), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  inv1   g38(.a(new_n65_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x15), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n66_), .c(new_n38_), .d(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n63_), .O(z5));
  inv1   g42(.a(x16), .O(new_n71_));
  oai21  g43(.a(new_n65_), .b(new_n64_), .c(new_n71_), .O(new_n72_));
  nor2   g44(.a(new_n37_), .b(new_n34_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g46(.a(new_n37_), .b(new_n71_), .c(x17), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n29_), .O(new_n76_));
  nand2  g48(.a(x08), .b(x05), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(z6));
  oai21  g50(.a(new_n33_), .b(new_n32_), .c(x17), .O(new_n79_));
  inv1   g51(.a(new_n33_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n50_), .c(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n38_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x06), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(new_n42_), .O(z7));
  nand3  g57(.a(new_n31_), .b(x08), .c(x07), .O(new_n86_));
  nand4  g58(.a(new_n34_), .b(x18), .c(x17), .d(x11), .O(new_n87_));
  nand3  g59(.a(new_n39_), .b(new_n80_), .c(new_n50_), .O(new_n88_));
  nand2  g60(.a(new_n31_), .b(x18), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n87_), .c(new_n38_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n86_), .O(z8));
endmodule


