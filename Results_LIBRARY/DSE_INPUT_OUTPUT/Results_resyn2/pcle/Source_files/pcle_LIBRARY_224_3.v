// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n33_), .c(x18), .d(new_n31_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n30_), .c(new_n29_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  oai21  g10(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n39_));
  nand3  g11(.a(new_n30_), .b(x09), .c(new_n31_), .O(new_n40_));
  oai22  g12(.a(new_n40_), .b(x11), .c(new_n39_), .d(new_n38_), .O(z1));
  inv1   g13(.a(x01), .O(new_n42_));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(new_n40_), .c(new_n39_), .d(new_n42_), .O(z2));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n45_), .c(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x09), .O(new_n48_));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z3));
  inv1   g22(.a(x03), .O(new_n51_));
  inv1   g23(.a(new_n40_), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n32_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n53_), .c(new_n52_), .O(new_n56_));
  oai21  g28(.a(new_n39_), .b(new_n51_), .c(new_n56_), .O(z4));
  inv1   g29(.a(new_n53_), .O(new_n58_));
  nor2   g30(.a(new_n58_), .b(x15), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  oai21  g32(.a(new_n53_), .b(new_n60_), .c(new_n31_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n59_), .c(new_n30_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x09), .O(new_n63_));
  nand2  g35(.a(x08), .b(x04), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z5));
  inv1   g37(.a(x05), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nor3   g39(.a(new_n53_), .b(new_n67_), .c(new_n60_), .O(new_n68_));
  oai21  g40(.a(new_n53_), .b(new_n60_), .c(new_n67_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n52_), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n68_), .c(new_n39_), .d(new_n66_), .O(z6));
  nand3  g43(.a(x16), .b(x15), .c(x14), .O(new_n72_));
  inv1   g44(.a(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n33_), .c(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  oai21  g47(.a(new_n72_), .b(new_n32_), .c(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n31_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n30_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(x09), .O(new_n79_));
  nand2  g51(.a(x08), .b(x06), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(z7));
  oai21  g53(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n82_));
  nor2   g54(.a(x18), .b(new_n75_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n73_), .c(new_n33_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n82_), .c(x08), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(x10), .c(x09), .O(new_n86_));
  nand2  g58(.a(x08), .b(x07), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n86_), .O(z8));
endmodule


