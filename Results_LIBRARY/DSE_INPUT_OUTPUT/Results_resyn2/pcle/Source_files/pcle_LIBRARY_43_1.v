// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  inv1   g00(.a(x18), .O(new_n29_));
  nor2   g01(.a(x10), .b(x08), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x16), .b(x15), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n32_), .c(new_n30_), .d(x17), .O(new_n34_));
  aoi21  g06(.a(new_n34_), .b(x09), .c(new_n29_), .O(z0));
  inv1   g07(.a(x00), .O(new_n36_));
  oai21  g08(.a(new_n29_), .b(x09), .c(x08), .O(new_n37_));
  nand2  g09(.a(new_n30_), .b(x09), .O(new_n38_));
  oai22  g10(.a(new_n38_), .b(x11), .c(new_n37_), .d(new_n36_), .O(z1));
  inv1   g11(.a(x01), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(new_n38_), .c(new_n37_), .d(new_n40_), .O(z2));
  aoi21  g14(.a(x12), .b(x11), .c(x13), .O(new_n43_));
  inv1   g15(.a(new_n38_), .O(new_n44_));
  nand3  g16(.a(x13), .b(x12), .c(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor2   g18(.a(new_n29_), .b(x09), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x02), .c(new_n47_), .O(new_n48_));
  oai21  g20(.a(new_n46_), .b(new_n43_), .c(new_n48_), .O(z3));
  inv1   g21(.a(x14), .O(new_n50_));
  nand2  g22(.a(new_n45_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n44_), .c(new_n31_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x03), .c(new_n47_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z4));
  inv1   g26(.a(x15), .O(new_n55_));
  nor2   g27(.a(new_n31_), .b(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n31_), .b(new_n55_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  aoi21  g30(.a(x08), .b(x04), .c(new_n47_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z5));
  nor2   g32(.a(new_n56_), .b(x16), .O(new_n61_));
  nand2  g33(.a(new_n33_), .b(new_n32_), .O(new_n62_));
  nand2  g34(.a(new_n44_), .b(new_n62_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x05), .c(new_n47_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(z6));
  inv1   g37(.a(new_n37_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x06), .O(new_n67_));
  nand3  g39(.a(new_n33_), .b(new_n32_), .c(x17), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  nand2  g41(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n44_), .c(new_n68_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n67_), .O(z7));
  nand2  g44(.a(new_n66_), .b(x07), .O(new_n73_));
  nand4  g45(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n45_), .c(x18), .O(new_n75_));
  nor2   g47(.a(x18), .b(new_n69_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n33_), .c(new_n32_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n44_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n73_), .O(z8));
endmodule


