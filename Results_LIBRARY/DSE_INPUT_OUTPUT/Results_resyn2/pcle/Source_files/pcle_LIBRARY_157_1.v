// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x14), .O(new_n36_));
  inv1   g08(.a(x16), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n35_), .c(new_n33_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x15), .c(new_n29_), .O(z0));
  nor2   g12(.a(new_n29_), .b(x15), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand3  g15(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n44_));
  inv1   g16(.a(new_n44_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  inv1   g21(.a(x12), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n48_), .c(new_n41_), .O(z2));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  inv1   g26(.a(x13), .O(new_n55_));
  nand2  g27(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n34_), .c(new_n45_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n41_), .O(z3));
  nor2   g30(.a(new_n35_), .b(x14), .O(new_n59_));
  oai21  g31(.a(new_n34_), .b(new_n36_), .c(new_n45_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x03), .c(new_n41_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(z4));
  nor2   g34(.a(new_n41_), .b(new_n30_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x04), .O(new_n64_));
  inv1   g36(.a(new_n41_), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  oai21  g38(.a(new_n34_), .b(new_n36_), .c(new_n66_), .O(new_n67_));
  nand2  g39(.a(x15), .b(x14), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n35_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n67_), .c(new_n65_), .d(new_n45_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n64_), .O(z5));
  nand2  g44(.a(new_n63_), .b(x05), .O(new_n73_));
  oai21  g45(.a(new_n68_), .b(new_n34_), .c(new_n37_), .O(new_n74_));
  nand3  g46(.a(new_n69_), .b(new_n35_), .c(x16), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n74_), .c(new_n65_), .d(new_n45_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n73_), .O(z6));
  inv1   g49(.a(x17), .O(new_n78_));
  nand2  g50(.a(new_n75_), .b(new_n78_), .O(new_n79_));
  nand3  g51(.a(x17), .b(x16), .c(x14), .O(new_n80_));
  nor2   g52(.a(new_n80_), .b(new_n34_), .O(new_n81_));
  nor2   g53(.a(new_n81_), .b(new_n44_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g55(.a(new_n32_), .b(new_n29_), .O(new_n84_));
  and2   g56(.a(x08), .b(x06), .O(new_n85_));
  aoi21  g57(.a(new_n84_), .b(new_n66_), .c(new_n85_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n83_), .O(z7));
  nand2  g59(.a(new_n63_), .b(x07), .O(new_n88_));
  nand2  g60(.a(new_n81_), .b(x18), .O(new_n89_));
  oai21  g61(.a(new_n80_), .b(new_n34_), .c(new_n29_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n89_), .c(new_n45_), .d(x15), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n88_), .O(z8));
endmodule


