// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  nand2  g00(.a(x18), .b(x17), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  and2   g06(.a(x16), .b(x15), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n34_), .c(new_n32_), .d(new_n30_), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x08), .O(new_n38_));
  nand3  g10(.a(new_n30_), .b(x09), .c(new_n38_), .O(new_n39_));
  nor2   g11(.a(x18), .b(x17), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(new_n39_), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n40_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n43_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n44_), .c(new_n50_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n40_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n44_), .c(new_n31_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n40_), .O(z4));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  aoi21  g33(.a(new_n31_), .b(new_n61_), .c(new_n39_), .O(new_n62_));
  oai21  g34(.a(new_n31_), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n60_), .c(new_n40_), .O(z5));
  inv1   g36(.a(x16), .O(new_n65_));
  oai21  g37(.a(new_n31_), .b(new_n61_), .c(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n35_), .b(new_n32_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n44_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x05), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n68_), .c(new_n40_), .O(z6));
  nand3  g42(.a(new_n67_), .b(new_n44_), .c(x17), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n36_), .b(x18), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x06), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(z7));
  nand2  g48(.a(new_n67_), .b(new_n29_), .O(new_n77_));
  nand3  g49(.a(new_n35_), .b(new_n32_), .c(x18), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n44_), .O(new_n79_));
  nand2  g51(.a(new_n39_), .b(x18), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x07), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(z8));
endmodule


