// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x17), .O(new_n30_));
  nand3  g02(.a(x16), .b(x15), .c(x14), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x12), .b(x11), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  and2   g07(.a(x18), .b(x13), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n35_), .c(new_n33_), .d(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g10(.a(new_n35_), .b(new_n29_), .O(new_n39_));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(x11), .c(new_n41_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x01), .c(new_n40_), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(new_n39_), .c(new_n44_), .O(z2));
  inv1   g17(.a(new_n40_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x08), .c(x02), .O(new_n47_));
  inv1   g19(.a(new_n39_), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  or2    g21(.a(new_n33_), .b(x13), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n47_), .O(z3));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n49_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n53_), .c(new_n48_), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x03), .c(new_n40_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n53_), .b(new_n59_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n35_), .c(new_n29_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n40_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(z5));
  nand3  g36(.a(new_n46_), .b(x08), .c(x05), .O(new_n65_));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  nor2   g38(.a(new_n49_), .b(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x16), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n49_), .b(new_n66_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n65_), .O(z6));
  inv1   g44(.a(new_n35_), .O(new_n73_));
  inv1   g45(.a(new_n49_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n32_), .O(new_n75_));
  nand3  g47(.a(new_n30_), .b(x16), .c(new_n29_), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  aoi22  g49(.a(new_n77_), .b(new_n67_), .c(new_n75_), .d(x17), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x06), .c(new_n40_), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n73_), .c(new_n79_), .O(z7));
  nor2   g52(.a(new_n49_), .b(new_n31_), .O(new_n81_));
  inv1   g53(.a(x08), .O(new_n82_));
  inv1   g54(.a(x18), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x09), .c(new_n82_), .O(new_n84_));
  inv1   g56(.a(new_n84_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n81_), .c(x10), .O(new_n86_));
  nand3  g58(.a(new_n74_), .b(new_n32_), .c(x17), .O(new_n87_));
  nand4  g59(.a(x18), .b(new_n29_), .c(x09), .d(new_n82_), .O(new_n88_));
  inv1   g60(.a(new_n88_), .O(new_n89_));
  and2   g61(.a(x08), .b(x07), .O(new_n90_));
  aoi21  g62(.a(new_n89_), .b(new_n87_), .c(new_n90_), .O(new_n91_));
  oai21  g63(.a(new_n86_), .b(new_n30_), .c(new_n91_), .O(z8));
endmodule


