// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x16), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n35_), .c(x17), .O(new_n38_));
  nor4   g10(.a(new_n38_), .b(new_n33_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g11(.a(x05), .O(new_n40_));
  nor2   g12(.a(x16), .b(new_n40_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  inv1   g15(.a(new_n33_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  inv1   g20(.a(x12), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n43_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n48_), .c(new_n44_), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n47_), .c(new_n41_), .O(z2));
  aoi21  g24(.a(x12), .b(x11), .c(x13), .O(new_n53_));
  nand2  g25(.a(new_n34_), .b(new_n44_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n41_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z3));
  nor2   g28(.a(new_n35_), .b(x14), .O(new_n57_));
  nand2  g29(.a(new_n35_), .b(x14), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n44_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n41_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  aoi21  g33(.a(new_n35_), .b(x14), .c(x15), .O(new_n62_));
  nor2   g34(.a(new_n36_), .b(new_n34_), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x04), .c(new_n41_), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(new_n62_), .c(new_n66_), .O(z5));
  nor2   g39(.a(new_n63_), .b(x16), .O(new_n68_));
  nand2  g40(.a(new_n63_), .b(x16), .O(new_n69_));
  nor2   g41(.a(new_n41_), .b(new_n33_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g43(.a(x16), .b(x08), .c(x05), .O(new_n72_));
  oai21  g44(.a(new_n71_), .b(new_n68_), .c(new_n72_), .O(z6));
  nand2  g45(.a(x08), .b(x06), .O(new_n74_));
  nand2  g46(.a(x17), .b(new_n29_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n33_), .c(new_n74_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n40_), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  oai21  g50(.a(new_n36_), .b(new_n34_), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n38_), .c(new_n44_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x16), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n77_), .O(z7));
  oai21  g55(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(new_n84_));
  nand3  g56(.a(new_n63_), .b(x18), .c(x17), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(new_n44_), .O(new_n86_));
  oai21  g58(.a(new_n33_), .b(new_n30_), .c(new_n40_), .O(new_n87_));
  aoi22  g59(.a(new_n87_), .b(new_n29_), .c(x08), .d(x07), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(z8));
endmodule


