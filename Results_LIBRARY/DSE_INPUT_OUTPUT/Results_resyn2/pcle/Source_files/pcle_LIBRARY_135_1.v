// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand2  g02(.a(x15), .b(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand4  g06(.a(x17), .b(new_n34_), .c(x09), .d(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x16), .c(new_n29_), .O(z0));
  nor2   g10(.a(new_n29_), .b(x16), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand3  g13(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n42_));
  inv1   g14(.a(new_n42_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n41_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n43_), .c(new_n46_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x01), .c(new_n39_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand2  g24(.a(new_n43_), .b(new_n30_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n39_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(z3));
  inv1   g27(.a(new_n30_), .O(new_n56_));
  nor2   g28(.a(new_n56_), .b(x14), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n43_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n39_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nor2   g34(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand2  g35(.a(new_n58_), .b(new_n62_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n43_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x04), .c(new_n39_), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(z5));
  nand2  g39(.a(x16), .b(x15), .O(new_n68_));
  nor2   g40(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  oai21  g41(.a(new_n32_), .b(x16), .c(new_n43_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n39_), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(new_n69_), .c(new_n71_), .O(z6));
  nand3  g44(.a(x17), .b(x15), .c(x14), .O(new_n73_));
  inv1   g45(.a(new_n73_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n56_), .c(new_n42_), .O(new_n75_));
  oai21  g47(.a(new_n69_), .b(x17), .c(new_n75_), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  oai21  g49(.a(new_n36_), .b(x18), .c(new_n77_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x06), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(z7));
  oai21  g52(.a(new_n73_), .b(new_n30_), .c(x18), .O(new_n81_));
  nand4  g53(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n58_), .c(new_n81_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n43_), .O(new_n84_));
  aoi21  g56(.a(x08), .b(x07), .c(new_n39_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


