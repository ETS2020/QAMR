// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g07(.a(x00), .O(new_n36_));
  oai21  g08(.a(x10), .b(x09), .c(x08), .O(new_n37_));
  oai22  g09(.a(new_n37_), .b(new_n36_), .c(new_n34_), .d(x11), .O(z1));
  inv1   g10(.a(x09), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(x12), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nor2   g14(.a(new_n41_), .b(new_n40_), .O(new_n43_));
  nor2   g15(.a(new_n43_), .b(x08), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n39_), .O(new_n45_));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(x10), .c(new_n46_), .O(z2));
  nor2   g19(.a(new_n43_), .b(x13), .O(new_n48_));
  nand2  g20(.a(new_n30_), .b(new_n32_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n48_), .c(x09), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n33_), .O(new_n51_));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z3));
  nand4  g25(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  aoi21  g27(.a(new_n30_), .b(new_n55_), .c(x08), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n54_), .c(new_n39_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(x10), .c(new_n58_), .O(z4));
  inv1   g31(.a(new_n37_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x04), .O(new_n61_));
  inv1   g33(.a(new_n54_), .O(new_n62_));
  inv1   g34(.a(new_n30_), .O(new_n63_));
  nand2  g35(.a(x15), .b(x14), .O(new_n64_));
  inv1   g36(.a(new_n64_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n63_), .c(new_n34_), .O(new_n66_));
  oai21  g38(.a(new_n62_), .b(x15), .c(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n61_), .O(z5));
  nand3  g40(.a(new_n65_), .b(new_n63_), .c(x16), .O(new_n69_));
  inv1   g41(.a(x16), .O(new_n70_));
  oai21  g42(.a(new_n64_), .b(new_n30_), .c(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n32_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n33_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x05), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z6));
  nand2  g48(.a(new_n60_), .b(x06), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  aoi21  g50(.a(new_n69_), .b(new_n78_), .c(new_n34_), .O(new_n79_));
  oai21  g51(.a(new_n69_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n77_), .O(z7));
  oai21  g53(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n82_));
  nor2   g54(.a(new_n78_), .b(new_n70_), .O(new_n83_));
  inv1   g55(.a(x15), .O(new_n84_));
  nor2   g56(.a(x18), .b(new_n84_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n62_), .c(new_n83_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n82_), .c(x08), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n39_), .c(new_n33_), .O(new_n88_));
  nand2  g60(.a(x08), .b(x07), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z8));
endmodule


