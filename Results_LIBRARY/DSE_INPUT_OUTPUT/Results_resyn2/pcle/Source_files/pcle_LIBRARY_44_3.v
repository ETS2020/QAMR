// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand4  g03(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  inv1   g07(.a(x09), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(x08), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n34_), .c(new_n29_), .O(z0));
  inv1   g11(.a(x15), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n38_), .b(x11), .c(new_n42_), .O(z1));
  inv1   g15(.a(x01), .O(new_n44_));
  oai21  g16(.a(new_n40_), .b(new_n35_), .c(x08), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(new_n38_), .c(new_n45_), .d(new_n44_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(new_n37_), .b(new_n30_), .c(new_n35_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n41_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n48_), .c(new_n50_), .O(z3));
  inv1   g23(.a(x03), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  oai21  g26(.a(new_n31_), .b(x14), .c(new_n35_), .O(new_n55_));
  oai22  g27(.a(new_n55_), .b(new_n54_), .c(new_n45_), .d(new_n52_), .O(z4));
  nand2  g28(.a(new_n54_), .b(new_n35_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x15), .O(new_n58_));
  nand2  g30(.a(x08), .b(x04), .O(new_n59_));
  inv1   g31(.a(new_n38_), .O(new_n60_));
  inv1   g32(.a(new_n53_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n60_), .c(new_n40_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z5));
  inv1   g35(.a(x05), .O(new_n64_));
  nand2  g36(.a(x16), .b(x15), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n40_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n65_), .c(new_n53_), .O(new_n68_));
  nand2  g40(.a(new_n53_), .b(new_n66_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n37_), .c(new_n35_), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n68_), .c(new_n45_), .d(new_n64_), .O(z6));
  inv1   g43(.a(x08), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(x16), .c(x09), .d(new_n72_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n53_), .c(new_n35_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x15), .O(new_n76_));
  inv1   g48(.a(new_n65_), .O(new_n77_));
  nand2  g49(.a(new_n61_), .b(new_n77_), .O(new_n78_));
  nand4  g50(.a(x17), .b(new_n35_), .c(x09), .d(new_n72_), .O(new_n79_));
  inv1   g51(.a(new_n79_), .O(new_n80_));
  aoi22  g52(.a(new_n80_), .b(new_n78_), .c(x08), .d(x06), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n76_), .O(z7));
  inv1   g54(.a(new_n45_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(x07), .O(new_n84_));
  oai21  g56(.a(new_n32_), .b(new_n30_), .c(x18), .O(new_n85_));
  nor2   g57(.a(x18), .b(new_n73_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n61_), .c(new_n77_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n60_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n84_), .O(z8));
endmodule


