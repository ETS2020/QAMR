// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x16), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(x18), .c(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n34_), .O(z0));
  nor2   g12(.a(x18), .b(x12), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n38_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nor2   g17(.a(new_n41_), .b(new_n35_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x01), .O(new_n47_));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  nor2   g20(.a(new_n41_), .b(new_n37_), .O(new_n49_));
  inv1   g21(.a(x12), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n47_), .O(z2));
  inv1   g25(.a(new_n49_), .O(new_n54_));
  nand2  g26(.a(new_n46_), .b(x02), .O(new_n55_));
  xor2a  g27(.a(new_n48_), .b(x13), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n54_), .c(new_n55_), .O(z3));
  inv1   g29(.a(x14), .O(new_n58_));
  oai21  g30(.a(new_n37_), .b(new_n58_), .c(x18), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n50_), .O(new_n60_));
  nand2  g32(.a(x08), .b(x03), .O(new_n61_));
  inv1   g33(.a(x13), .O(new_n62_));
  oai21  g34(.a(new_n48_), .b(new_n62_), .c(new_n58_), .O(new_n63_));
  nand3  g35(.a(x14), .b(x13), .c(x11), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n63_), .c(new_n38_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n61_), .c(new_n60_), .O(z4));
  nand2  g38(.a(new_n46_), .b(x04), .O(new_n67_));
  nand2  g39(.a(new_n32_), .b(new_n29_), .O(new_n68_));
  nand2  g40(.a(new_n33_), .b(x15), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n68_), .c(new_n49_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n67_), .O(z5));
  nand2  g43(.a(new_n46_), .b(x05), .O(new_n72_));
  nand2  g44(.a(new_n69_), .b(new_n30_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n49_), .c(new_n34_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n72_), .O(z6));
  nand2  g47(.a(new_n46_), .b(x06), .O(new_n76_));
  inv1   g48(.a(new_n64_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n31_), .c(x17), .d(x12), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nand3  g51(.a(new_n77_), .b(new_n31_), .c(x12), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n78_), .c(new_n49_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n76_), .O(z7));
  nand2  g55(.a(new_n46_), .b(x07), .O(new_n84_));
  nand4  g56(.a(new_n33_), .b(new_n31_), .c(x18), .d(x17), .O(new_n85_));
  inv1   g57(.a(x18), .O(new_n86_));
  nand3  g58(.a(new_n33_), .b(new_n31_), .c(x17), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n85_), .c(new_n38_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n84_), .O(z8));
endmodule


