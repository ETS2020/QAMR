// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  nor2   g07(.a(new_n29_), .b(x12), .O(new_n36_));
  aoi21  g08(.a(x08), .b(x00), .c(new_n36_), .O(new_n37_));
  oai21  g09(.a(new_n34_), .b(x11), .c(new_n37_), .O(z1));
  inv1   g10(.a(x01), .O(new_n39_));
  inv1   g11(.a(new_n36_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x08), .O(new_n41_));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  inv1   g14(.a(new_n34_), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  inv1   g16(.a(x12), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n40_), .c(new_n43_), .d(new_n42_), .O(new_n47_));
  oai21  g19(.a(new_n41_), .b(new_n39_), .c(new_n47_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n43_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n36_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z3));
  inv1   g25(.a(x03), .O(new_n54_));
  nand3  g26(.a(x14), .b(x13), .c(x11), .O(new_n55_));
  inv1   g27(.a(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n50_), .b(new_n58_), .O(new_n59_));
  nand4  g31(.a(new_n59_), .b(new_n57_), .c(new_n40_), .d(new_n43_), .O(new_n60_));
  oai21  g32(.a(new_n41_), .b(new_n54_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n55_), .b(new_n62_), .O(new_n63_));
  nand4  g35(.a(x15), .b(x14), .c(x13), .d(x11), .O(new_n64_));
  nand2  g36(.a(x14), .b(new_n45_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n43_), .O(new_n66_));
  oai21  g38(.a(new_n34_), .b(new_n62_), .c(new_n29_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n45_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x04), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z5));
  inv1   g42(.a(new_n64_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x05), .O(new_n72_));
  nand4  g44(.a(x16), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n71_), .c(new_n72_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n40_), .O(new_n75_));
  nand3  g47(.a(new_n29_), .b(x16), .c(new_n45_), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n50_), .c(new_n76_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n43_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n75_), .O(z6));
  nand2  g53(.a(x16), .b(x15), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n55_), .c(x17), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(x16), .c(x15), .d(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n50_), .c(new_n83_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n43_), .O(new_n87_));
  oai21  g59(.a(new_n34_), .b(new_n84_), .c(new_n29_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n45_), .O(new_n89_));
  nand2  g61(.a(x08), .b(x06), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n89_), .c(new_n87_), .O(z7));
  nand2  g63(.a(x13), .b(x11), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n31_), .c(x18), .O(new_n93_));
  inv1   g65(.a(new_n30_), .O(new_n94_));
  inv1   g66(.a(new_n31_), .O(new_n95_));
  nand3  g67(.a(new_n95_), .b(new_n94_), .c(new_n29_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n43_), .O(new_n98_));
  aoi21  g70(.a(x08), .b(x07), .c(new_n36_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n98_), .O(z8));
endmodule


