// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n33_), .c(x18), .d(new_n31_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(new_n30_), .O(z0));
  oai21  g09(.a(x09), .b(x00), .c(x08), .O(new_n38_));
  nor2   g10(.a(x10), .b(new_n30_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(x11), .c(new_n38_), .O(z1));
  nor2   g13(.a(x09), .b(new_n29_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x01), .O(new_n43_));
  nand3  g15(.a(new_n31_), .b(x09), .c(new_n29_), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(z2));
  nand2  g18(.a(new_n42_), .b(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  inv1   g20(.a(new_n44_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n32_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  oai21  g23(.a(x09), .b(x03), .c(x08), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n32_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n53_), .c(new_n39_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n52_), .O(z4));
  oai21  g29(.a(x09), .b(x04), .c(x08), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n53_), .b(new_n59_), .O(new_n60_));
  inv1   g32(.a(new_n53_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(x15), .c(new_n39_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z5));
  nand2  g35(.a(new_n42_), .b(x05), .O(new_n64_));
  nand2  g36(.a(x16), .b(x15), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n59_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n65_), .c(new_n53_), .O(new_n68_));
  oai21  g40(.a(new_n61_), .b(x16), .c(new_n49_), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(z6));
  oai21  g42(.a(x09), .b(x06), .c(x08), .O(new_n71_));
  nand3  g43(.a(x16), .b(x15), .c(x14), .O(new_n72_));
  inv1   g44(.a(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n33_), .c(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  oai21  g47(.a(new_n72_), .b(new_n32_), .c(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n39_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n71_), .O(z7));
  oai21  g50(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n75_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n73_), .c(new_n33_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n49_), .O(new_n83_));
  nand2  g55(.a(new_n42_), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


