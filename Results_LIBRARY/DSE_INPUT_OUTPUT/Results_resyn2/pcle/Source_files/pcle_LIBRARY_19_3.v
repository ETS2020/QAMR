// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x02), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  inv1   g09(.a(x16), .O(new_n38_));
  inv1   g10(.a(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g12(.a(new_n40_), .b(new_n37_), .c(new_n34_), .O(new_n41_));
  aoi21  g13(.a(new_n41_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g14(.a(new_n30_), .b(new_n29_), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x00), .c(new_n43_), .O(new_n44_));
  oai21  g16(.a(new_n33_), .b(x11), .c(new_n44_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  or2    g19(.a(x12), .b(x11), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n34_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n46_), .c(new_n43_), .O(z2));
  oai21  g22(.a(x18), .b(x08), .c(x02), .O(new_n51_));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand2  g24(.a(new_n35_), .b(new_n34_), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(new_n35_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n35_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n34_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n55_), .c(new_n43_), .O(z4));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  inv1   g34(.a(new_n37_), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  oai21  g36(.a(new_n35_), .b(new_n58_), .c(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n34_), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n62_), .c(new_n43_), .O(z5));
  nand2  g39(.a(x08), .b(x05), .O(new_n68_));
  inv1   g40(.a(new_n36_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n56_), .c(x16), .O(new_n70_));
  oai21  g42(.a(new_n36_), .b(new_n35_), .c(new_n38_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n34_), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n68_), .c(new_n43_), .O(z6));
  nand3  g45(.a(new_n37_), .b(x17), .c(x16), .O(new_n74_));
  nand2  g46(.a(new_n70_), .b(new_n39_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n34_), .O(new_n76_));
  aoi21  g48(.a(x08), .b(x06), .c(new_n43_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(z7));
  aoi22  g50(.a(new_n40_), .b(new_n37_), .c(x18), .d(new_n29_), .O(new_n79_));
  nand3  g51(.a(new_n40_), .b(new_n37_), .c(x18), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n34_), .O(new_n81_));
  inv1   g53(.a(new_n43_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x08), .c(x07), .O(new_n83_));
  oai21  g55(.a(new_n81_), .b(new_n79_), .c(new_n83_), .O(z8));
endmodule


