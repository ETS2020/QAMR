// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x18), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n29_), .c(new_n33_), .O(new_n36_));
  oai22  g08(.a(new_n36_), .b(x11), .c(new_n32_), .d(new_n31_), .O(z1));
  inv1   g09(.a(x01), .O(new_n38_));
  xnor2a g10(.a(x12), .b(x11), .O(new_n39_));
  oai22  g11(.a(new_n39_), .b(new_n36_), .c(new_n32_), .d(new_n38_), .O(z2));
  inv1   g12(.a(x02), .O(new_n41_));
  aoi21  g13(.a(x12), .b(x11), .c(x13), .O(new_n42_));
  inv1   g14(.a(new_n36_), .O(new_n43_));
  nand3  g15(.a(x13), .b(x12), .c(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai22  g17(.a(new_n45_), .b(new_n42_), .c(new_n32_), .d(new_n41_), .O(z3));
  nand4  g18(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n47_));
  inv1   g19(.a(x14), .O(new_n48_));
  nand2  g20(.a(new_n44_), .b(new_n48_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n47_), .c(new_n35_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n29_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n33_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z4));
  inv1   g26(.a(x04), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  nand2  g28(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  inv1   g29(.a(new_n47_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x15), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n43_), .O(new_n60_));
  oai21  g32(.a(new_n32_), .b(new_n55_), .c(new_n60_), .O(z5));
  inv1   g33(.a(x05), .O(new_n62_));
  inv1   g34(.a(x16), .O(new_n63_));
  nor2   g35(.a(new_n59_), .b(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n59_), .b(new_n63_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n43_), .O(new_n66_));
  oai22  g38(.a(new_n66_), .b(new_n64_), .c(new_n32_), .d(new_n62_), .O(z6));
  inv1   g39(.a(x17), .O(new_n68_));
  nand2  g40(.a(new_n47_), .b(new_n68_), .O(new_n69_));
  inv1   g41(.a(x08), .O(new_n70_));
  nand2  g42(.a(x09), .b(new_n70_), .O(new_n71_));
  nand2  g43(.a(x16), .b(x15), .O(new_n72_));
  xor2a  g44(.a(new_n72_), .b(x17), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n58_), .c(new_n71_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n69_), .c(x18), .O(new_n75_));
  nand2  g47(.a(x08), .b(x06), .O(new_n76_));
  oai21  g48(.a(new_n75_), .b(x10), .c(new_n76_), .O(z7));
  inv1   g49(.a(new_n72_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(new_n58_), .c(new_n35_), .d(x17), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n29_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n33_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x07), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z8));
endmodule


