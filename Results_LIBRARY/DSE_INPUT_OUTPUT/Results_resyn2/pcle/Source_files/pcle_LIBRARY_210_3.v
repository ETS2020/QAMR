// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_;
  inv1   g00(.a(x05), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(x17), .b(x16), .c(x15), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n36_), .c(new_n32_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g12(.a(new_n30_), .b(new_n29_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n36_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n36_), .c(new_n47_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n41_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n36_), .b(new_n53_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n41_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z3));
  inv1   g28(.a(new_n53_), .O(new_n57_));
  nor2   g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand2  g30(.a(new_n36_), .b(new_n31_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n41_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(z4));
  nor2   g33(.a(new_n32_), .b(x15), .O(new_n62_));
  nand2  g34(.a(x15), .b(x14), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n57_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  aoi21  g38(.a(x08), .b(x04), .c(new_n41_), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(new_n62_), .c(new_n67_), .O(z5));
  oai21  g40(.a(x18), .b(x08), .c(x05), .O(new_n69_));
  nor2   g41(.a(new_n63_), .b(new_n53_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  oai21  g44(.a(new_n63_), .b(new_n53_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n36_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n69_), .O(z6));
  aoi21  g47(.a(new_n70_), .b(x16), .c(x17), .O(new_n76_));
  and2   g48(.a(x17), .b(x16), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n64_), .c(new_n57_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n36_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x06), .c(new_n41_), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n76_), .c(new_n80_), .O(z7));
  nand4  g53(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n82_));
  inv1   g54(.a(new_n82_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n57_), .c(x18), .O(new_n84_));
  oai21  g56(.a(new_n82_), .b(new_n53_), .c(new_n30_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(new_n36_), .O(new_n86_));
  aoi21  g58(.a(x08), .b(x07), .c(new_n41_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n86_), .O(z8));
endmodule


