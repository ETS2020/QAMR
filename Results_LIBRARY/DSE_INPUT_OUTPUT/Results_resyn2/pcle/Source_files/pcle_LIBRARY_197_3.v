// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x04), .O(new_n29_));
  inv1   g01(.a(x17), .O(new_n30_));
  nand2  g02(.a(x12), .b(x11), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g07(.a(x16), .b(x15), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  and2   g09(.a(x14), .b(x13), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n37_), .c(new_n35_), .d(x18), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g12(.a(x17), .b(x04), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  aoi21  g14(.a(x08), .b(x00), .c(new_n42_), .O(new_n43_));
  oai21  g15(.a(new_n34_), .b(x11), .c(new_n43_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  inv1   g17(.a(new_n34_), .O(new_n46_));
  or2    g18(.a(x12), .b(x11), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n31_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n45_), .c(new_n42_), .O(z2));
  and2   g21(.a(x12), .b(x11), .O(new_n50_));
  nor2   g22(.a(new_n50_), .b(x13), .O(new_n51_));
  nand2  g23(.a(new_n50_), .b(x13), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n42_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n51_), .c(new_n54_), .O(z3));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n52_), .b(new_n56_), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  nand2  g30(.a(new_n41_), .b(new_n46_), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n57_), .O(new_n61_));
  nand3  g33(.a(new_n41_), .b(x08), .c(x03), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z4));
  nand3  g35(.a(new_n30_), .b(x08), .c(x04), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand2  g37(.a(new_n58_), .b(new_n65_), .O(new_n66_));
  inv1   g38(.a(new_n58_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x15), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n60_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n64_), .O(z5));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n68_), .b(new_n72_), .O(new_n73_));
  nor2   g45(.a(new_n58_), .b(new_n36_), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n34_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n71_), .c(new_n42_), .O(z6));
  aoi21  g49(.a(new_n74_), .b(x17), .c(new_n34_), .O(new_n78_));
  oai21  g50(.a(new_n74_), .b(x17), .c(new_n78_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x06), .c(new_n42_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(z7));
  nand2  g53(.a(new_n78_), .b(x18), .O(new_n82_));
  nand4  g54(.a(new_n50_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n83_));
  inv1   g55(.a(x18), .O(new_n84_));
  nand3  g56(.a(new_n38_), .b(new_n37_), .c(new_n84_), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n83_), .c(new_n29_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(x17), .O(new_n87_));
  nand2  g59(.a(x08), .b(x07), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n82_), .O(z8));
endmodule


