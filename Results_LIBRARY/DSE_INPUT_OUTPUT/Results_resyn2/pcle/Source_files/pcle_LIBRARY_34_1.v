// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_;
  inv1   g00(.a(x14), .O(new_n29_));
  inv1   g01(.a(x15), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g03(.a(new_n31_), .b(x18), .c(x17), .d(x16), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n33_), .c(new_n32_), .O(z0));
  inv1   g09(.a(x18), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(x13), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n36_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(new_n36_), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n42_), .c(new_n39_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  inv1   g20(.a(new_n39_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x08), .O(new_n50_));
  nand2  g22(.a(new_n49_), .b(new_n44_), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n43_), .b(new_n52_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  oai22  g26(.a(new_n54_), .b(new_n51_), .c(new_n50_), .d(new_n48_), .O(z3));
  oai21  g27(.a(new_n36_), .b(new_n29_), .c(new_n38_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand2  g30(.a(new_n33_), .b(new_n29_), .O(new_n59_));
  nand3  g31(.a(x14), .b(x12), .c(x11), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n44_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(z4));
  inv1   g34(.a(x04), .O(new_n63_));
  nand4  g35(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n30_), .O(new_n65_));
  inv1   g37(.a(new_n33_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n31_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n65_), .c(new_n49_), .d(new_n44_), .O(new_n68_));
  oai21  g40(.a(new_n50_), .b(new_n63_), .c(new_n68_), .O(z5));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  nand4  g42(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  nand4  g44(.a(x16), .b(new_n35_), .c(x09), .d(new_n34_), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand2  g48(.a(new_n67_), .b(new_n76_), .O(new_n77_));
  oai21  g49(.a(x18), .b(x13), .c(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n44_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n75_), .O(z6));
  nand2  g52(.a(x16), .b(x15), .O(new_n81_));
  nor2   g53(.a(new_n81_), .b(new_n60_), .O(new_n82_));
  nand4  g54(.a(x17), .b(new_n35_), .c(x09), .d(new_n34_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x06), .O(new_n84_));
  oai21  g56(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n49_), .O(new_n86_));
  nand3  g58(.a(new_n38_), .b(x17), .c(new_n52_), .O(new_n87_));
  inv1   g59(.a(x17), .O(new_n88_));
  inv1   g60(.a(new_n60_), .O(new_n89_));
  inv1   g61(.a(new_n81_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x13), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(new_n44_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n86_), .O(z7));
  nand2  g66(.a(x17), .b(x16), .O(new_n95_));
  oai21  g67(.a(new_n71_), .b(new_n95_), .c(x18), .O(new_n96_));
  inv1   g68(.a(new_n64_), .O(new_n97_));
  nor2   g69(.a(x18), .b(new_n88_), .O(new_n98_));
  nand3  g70(.a(new_n98_), .b(new_n90_), .c(new_n97_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n44_), .O(new_n101_));
  aoi21  g73(.a(x08), .b(x07), .c(new_n39_), .O(new_n102_));
  nand2  g74(.a(new_n102_), .b(new_n101_), .O(z8));
endmodule


