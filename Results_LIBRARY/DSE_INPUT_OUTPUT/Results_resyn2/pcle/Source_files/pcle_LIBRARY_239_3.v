// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  nand3  g04(.a(x16), .b(x15), .c(x14), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x18), .c(x17), .d(new_n31_), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(new_n30_), .c(new_n29_), .O(z0));
  aoi21  g08(.a(x10), .b(x09), .c(new_n31_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(x00), .O(new_n38_));
  nand3  g10(.a(new_n30_), .b(x09), .c(new_n31_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(x11), .c(new_n38_), .O(z1));
  nand2  g12(.a(new_n37_), .b(x01), .O(new_n41_));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(new_n39_), .c(new_n41_), .O(z2));
  nand2  g15(.a(new_n37_), .b(x02), .O(new_n44_));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  inv1   g17(.a(new_n39_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n32_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n45_), .c(new_n44_), .O(z3));
  nand4  g20(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n49_));
  inv1   g21(.a(x14), .O(new_n50_));
  aoi21  g22(.a(new_n32_), .b(new_n50_), .c(x08), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n49_), .c(x10), .O(new_n52_));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n29_), .c(new_n53_), .O(z4));
  nand2  g26(.a(new_n37_), .b(x04), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  nor2   g28(.a(new_n49_), .b(new_n56_), .O(new_n57_));
  nand2  g29(.a(new_n49_), .b(new_n56_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n46_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(z5));
  inv1   g32(.a(x16), .O(new_n61_));
  oai21  g33(.a(new_n49_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  inv1   g34(.a(new_n32_), .O(new_n63_));
  inv1   g35(.a(new_n33_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n63_), .c(x08), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n62_), .c(x10), .O(new_n66_));
  nand2  g38(.a(x08), .b(x05), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(new_n29_), .c(new_n67_), .O(z6));
  nand2  g40(.a(new_n37_), .b(x06), .O(new_n69_));
  nor2   g41(.a(new_n34_), .b(x17), .O(new_n70_));
  nand2  g42(.a(new_n34_), .b(x17), .O(new_n71_));
  nand2  g43(.a(new_n46_), .b(new_n71_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(z7));
  nand2  g45(.a(new_n37_), .b(x07), .O(new_n74_));
  aoi21  g46(.a(new_n34_), .b(x17), .c(x18), .O(new_n75_));
  inv1   g47(.a(x18), .O(new_n76_));
  oai21  g48(.a(new_n71_), .b(new_n76_), .c(new_n46_), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(z8));
endmodule


