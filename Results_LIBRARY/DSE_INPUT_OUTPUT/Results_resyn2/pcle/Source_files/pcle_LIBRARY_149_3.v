// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x12), .b(x11), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x18), .O(new_n34_));
  nand4  g06(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  aoi21  g10(.a(x13), .b(x10), .c(new_n31_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x00), .O(new_n40_));
  nand3  g12(.a(new_n29_), .b(x09), .c(new_n31_), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g14(.a(new_n39_), .b(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n41_), .c(new_n43_), .O(z2));
  nand2  g17(.a(new_n39_), .b(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  inv1   g19(.a(new_n41_), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n47_), .c(new_n46_), .O(z3));
  nand2  g23(.a(new_n39_), .b(x03), .O(new_n52_));
  inv1   g24(.a(new_n49_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(x14), .c(new_n41_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n52_), .O(z4));
  nand2  g28(.a(new_n54_), .b(x15), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n32_), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x13), .O(new_n61_));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n61_), .c(new_n57_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x15), .c(x14), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n32_), .c(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x13), .O(new_n67_));
  nand3  g39(.a(new_n53_), .b(x15), .c(x14), .O(new_n68_));
  nand4  g40(.a(x16), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  aoi22  g42(.a(new_n70_), .b(new_n68_), .c(x08), .d(x05), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n67_), .O(z6));
  nand2  g44(.a(new_n39_), .b(x06), .O(new_n73_));
  nand3  g45(.a(x16), .b(x15), .c(x14), .O(new_n74_));
  inv1   g46(.a(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n53_), .c(x17), .O(new_n76_));
  inv1   g48(.a(x17), .O(new_n77_));
  oai21  g49(.a(new_n74_), .b(new_n49_), .c(new_n77_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n73_), .O(z7));
  nand2  g52(.a(new_n39_), .b(x07), .O(new_n81_));
  oai21  g53(.a(new_n49_), .b(new_n35_), .c(x18), .O(new_n82_));
  nand4  g54(.a(new_n75_), .b(new_n53_), .c(new_n34_), .d(x17), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n48_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n81_), .O(z8));
endmodule


