// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nand4  g06(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n34_), .c(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  inv1   g10(.a(x08), .O(new_n39_));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  nand2  g14(.a(new_n34_), .b(new_n29_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(x11), .c(new_n42_), .O(z1));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n40_), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(new_n43_), .c(new_n46_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(new_n34_), .b(new_n31_), .c(new_n29_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n40_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n48_), .c(new_n50_), .O(z3));
  oai21  g23(.a(new_n31_), .b(x10), .c(new_n30_), .O(new_n52_));
  nand2  g24(.a(new_n32_), .b(x14), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n52_), .c(new_n34_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n40_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(x09), .c(new_n39_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n31_), .c(new_n29_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x14), .O(new_n60_));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  inv1   g33(.a(new_n43_), .O(new_n62_));
  nand3  g34(.a(new_n53_), .b(new_n62_), .c(x15), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n60_), .O(z5));
  nand2  g36(.a(new_n41_), .b(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(x15), .b(x14), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n31_), .c(new_n66_), .O(new_n68_));
  nand3  g40(.a(x16), .b(x15), .c(x14), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n32_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n68_), .c(new_n62_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n65_), .O(z6));
  nand2  g45(.a(new_n41_), .b(x06), .O(new_n74_));
  nand3  g46(.a(new_n70_), .b(new_n32_), .c(x17), .O(new_n75_));
  inv1   g47(.a(x17), .O(new_n76_));
  nand2  g48(.a(new_n71_), .b(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n62_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n74_), .O(z7));
  nand2  g51(.a(new_n41_), .b(x07), .O(new_n80_));
  nand4  g52(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n31_), .c(x18), .O(new_n82_));
  nor2   g54(.a(x18), .b(new_n76_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n70_), .c(new_n32_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n62_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n80_), .O(z8));
endmodule


