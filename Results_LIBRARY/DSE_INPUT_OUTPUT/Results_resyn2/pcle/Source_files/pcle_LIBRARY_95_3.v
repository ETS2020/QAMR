// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x11), .O(new_n32_));
  nor2   g04(.a(x10), .b(x08), .O(new_n33_));
  aoi21  g05(.a(new_n33_), .b(new_n32_), .c(x18), .O(new_n34_));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(new_n29_), .c(new_n35_), .O(z1));
  nor2   g08(.a(x12), .b(x11), .O(new_n37_));
  nand2  g09(.a(x12), .b(x11), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n33_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n37_), .c(new_n30_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x09), .O(new_n41_));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z2));
  inv1   g15(.a(x02), .O(new_n44_));
  oai21  g16(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n45_));
  nand3  g17(.a(x13), .b(x12), .c(x11), .O(new_n46_));
  nand3  g18(.a(new_n33_), .b(new_n30_), .c(x09), .O(new_n47_));
  inv1   g19(.a(new_n47_), .O(new_n48_));
  inv1   g20(.a(x13), .O(new_n49_));
  nand2  g21(.a(new_n38_), .b(new_n49_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(new_n51_));
  oai21  g23(.a(new_n45_), .b(new_n44_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  nand4  g25(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n46_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n54_), .c(new_n48_), .O(new_n57_));
  oai21  g29(.a(new_n45_), .b(new_n53_), .c(new_n57_), .O(z4));
  inv1   g30(.a(x04), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n54_), .b(new_n60_), .O(new_n61_));
  inv1   g33(.a(new_n54_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x15), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(new_n64_));
  oai21  g36(.a(new_n45_), .b(new_n59_), .c(new_n64_), .O(z5));
  inv1   g37(.a(x05), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nor2   g39(.a(new_n63_), .b(new_n67_), .O(new_n68_));
  nand2  g40(.a(new_n63_), .b(new_n67_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n48_), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n68_), .c(new_n45_), .d(new_n66_), .O(z6));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n54_), .b(new_n72_), .O(new_n73_));
  inv1   g45(.a(new_n33_), .O(new_n74_));
  nand2  g46(.a(x16), .b(x15), .O(new_n75_));
  xor2a  g47(.a(new_n75_), .b(x17), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n62_), .c(new_n74_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n73_), .c(x18), .O(new_n78_));
  nand2  g50(.a(x08), .b(x06), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n29_), .c(new_n79_), .O(z7));
  inv1   g52(.a(new_n75_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n33_), .c(x17), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n54_), .c(new_n30_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(x09), .O(new_n84_));
  nand2  g56(.a(x08), .b(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


