// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_;
  nand2  g00(.a(x16), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g05(.a(x18), .b(x17), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n34_), .c(new_n33_), .O(z0));
  nor2   g10(.a(x18), .b(x13), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n37_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(new_n37_), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n42_), .c(new_n39_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  inv1   g20(.a(new_n39_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nor2   g23(.a(new_n39_), .b(new_n37_), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n43_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  oai21  g27(.a(new_n50_), .b(new_n48_), .c(new_n55_), .O(z3));
  inv1   g28(.a(x14), .O(new_n57_));
  oai21  g29(.a(new_n37_), .b(new_n57_), .c(x18), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand2  g32(.a(new_n51_), .b(new_n57_), .O(new_n61_));
  nand3  g33(.a(x14), .b(x12), .c(x11), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n61_), .c(new_n44_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z4));
  nand3  g36(.a(new_n49_), .b(x08), .c(x04), .O(new_n65_));
  nor2   g37(.a(new_n32_), .b(x15), .O(new_n66_));
  nand2  g38(.a(new_n32_), .b(x15), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z5));
  nand3  g41(.a(new_n49_), .b(x08), .c(x05), .O(new_n70_));
  aoi21  g42(.a(new_n32_), .b(x15), .c(x16), .O(new_n71_));
  nand2  g43(.a(new_n52_), .b(new_n33_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z6));
  nand2  g45(.a(x08), .b(x06), .O(new_n74_));
  nor2   g46(.a(new_n62_), .b(new_n29_), .O(new_n75_));
  nand4  g47(.a(x17), .b(new_n36_), .c(x09), .d(new_n35_), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n49_), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n29_), .c(new_n34_), .O(new_n79_));
  nand2  g51(.a(x17), .b(x13), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(new_n44_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n78_), .O(z7));
  nand3  g54(.a(new_n75_), .b(x18), .c(x17), .O(new_n83_));
  inv1   g55(.a(x18), .O(new_n84_));
  inv1   g56(.a(new_n62_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n30_), .c(x17), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n83_), .c(new_n44_), .O(new_n88_));
  nand2  g60(.a(new_n37_), .b(x18), .O(new_n89_));
  aoi22  g61(.a(new_n89_), .b(new_n53_), .c(x08), .d(x07), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n88_), .O(z8));
endmodule


