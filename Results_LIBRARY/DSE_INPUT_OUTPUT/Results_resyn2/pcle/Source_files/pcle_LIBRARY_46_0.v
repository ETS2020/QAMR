// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nor2   g00(.a(x18), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand4  g03(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n32_));
  or2    g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(x18), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n33_), .c(new_n30_), .O(z0));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand2  g13(.a(new_n37_), .b(new_n41_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n29_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n41_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n37_), .c(new_n45_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n29_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand2  g22(.a(new_n37_), .b(new_n31_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n31_), .b(new_n55_), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nor2   g30(.a(new_n58_), .b(new_n36_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n54_), .c(new_n29_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  oai21  g34(.a(new_n57_), .b(new_n36_), .c(x18), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g36(.a(new_n59_), .b(x15), .O(new_n65_));
  nand2  g37(.a(x08), .b(x04), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z5));
  inv1   g39(.a(x16), .O(new_n68_));
  oai21  g40(.a(new_n57_), .b(new_n62_), .c(new_n68_), .O(new_n69_));
  nand2  g41(.a(new_n58_), .b(x16), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n37_), .O(new_n71_));
  oai21  g43(.a(new_n36_), .b(new_n68_), .c(x18), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n62_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x15), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n58_), .c(x16), .O(new_n78_));
  nand2  g50(.a(new_n70_), .b(new_n76_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n37_), .O(new_n80_));
  oai21  g52(.a(new_n36_), .b(new_n76_), .c(x18), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n62_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x06), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z7));
  oai21  g56(.a(new_n32_), .b(new_n31_), .c(x18), .O(new_n85_));
  nand2  g57(.a(x17), .b(x15), .O(new_n86_));
  nor2   g58(.a(new_n86_), .b(x18), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n58_), .c(x16), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n37_), .O(new_n90_));
  nand3  g62(.a(new_n30_), .b(x08), .c(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z8));
endmodule


