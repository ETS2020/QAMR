// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x18), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  nand3  g07(.a(x16), .b(x15), .c(x14), .O(new_n36_));
  nand3  g08(.a(x13), .b(x12), .c(x11), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  and2   g10(.a(new_n38_), .b(new_n34_), .O(z0));
  inv1   g11(.a(x11), .O(new_n40_));
  nand2  g12(.a(new_n32_), .b(new_n40_), .O(new_n41_));
  inv1   g13(.a(x18), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x11), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z1));
  aoi21  g17(.a(new_n42_), .b(x11), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n43_), .b(new_n32_), .O(new_n48_));
  xnor2a g20(.a(x12), .b(x11), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(z2));
  nand2  g22(.a(new_n46_), .b(x02), .O(new_n51_));
  nand2  g23(.a(x12), .b(x11), .O(new_n52_));
  xor2a  g24(.a(new_n52_), .b(x13), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(x13), .c(x12), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n31_), .c(x18), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x11), .O(new_n58_));
  nand2  g30(.a(x08), .b(x03), .O(new_n59_));
  nand3  g31(.a(new_n37_), .b(new_n32_), .c(x14), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(z4));
  nand3  g33(.a(x14), .b(x13), .c(x12), .O(new_n62_));
  nor3   g34(.a(new_n62_), .b(new_n31_), .c(x15), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n42_), .c(x11), .O(new_n64_));
  inv1   g36(.a(new_n62_), .O(new_n65_));
  nand4  g37(.a(x15), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n66_));
  aoi21  g38(.a(new_n65_), .b(x11), .c(new_n66_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n67_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n64_), .O(z5));
  inv1   g41(.a(x16), .O(new_n70_));
  nand2  g42(.a(new_n65_), .b(new_n70_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n66_), .c(x18), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x11), .O(new_n73_));
  nand3  g45(.a(new_n65_), .b(x15), .c(x11), .O(new_n74_));
  nand4  g46(.a(x16), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  aoi22  g48(.a(new_n76_), .b(new_n74_), .c(x08), .d(x05), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n73_), .O(z6));
  nand2  g50(.a(new_n46_), .b(x06), .O(new_n79_));
  nand2  g51(.a(x16), .b(x15), .O(new_n80_));
  oai21  g52(.a(new_n62_), .b(new_n80_), .c(x18), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(x11), .c(new_n35_), .O(new_n82_));
  nand2  g54(.a(x13), .b(x12), .O(new_n83_));
  nand3  g55(.a(x18), .b(new_n35_), .c(x11), .O(new_n84_));
  nor3   g56(.a(new_n84_), .b(new_n83_), .c(new_n36_), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n82_), .c(new_n32_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n79_), .O(z7));
  nand2  g59(.a(new_n46_), .b(x07), .O(new_n88_));
  oai21  g60(.a(new_n38_), .b(new_n33_), .c(new_n88_), .O(z8));
endmodule


