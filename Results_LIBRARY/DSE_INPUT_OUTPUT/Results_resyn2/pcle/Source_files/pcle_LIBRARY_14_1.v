// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  and2   g07(.a(x17), .b(x11), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n35_), .c(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x12), .c(new_n29_), .O(z0));
  nor2   g10(.a(new_n29_), .b(x12), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n32_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  inv1   g15(.a(x12), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n42_), .c(new_n33_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n39_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n42_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n33_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n39_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  oai21  g27(.a(new_n32_), .b(new_n55_), .c(new_n29_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n44_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand2  g30(.a(new_n49_), .b(new_n55_), .O(new_n59_));
  nand3  g31(.a(x14), .b(x13), .c(x11), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n33_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(z4));
  inv1   g34(.a(x15), .O(new_n63_));
  oai21  g35(.a(new_n32_), .b(new_n63_), .c(new_n29_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n44_), .O(new_n65_));
  nand2  g37(.a(new_n60_), .b(x15), .O(new_n66_));
  and2   g38(.a(x14), .b(x13), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n42_), .c(new_n66_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n33_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x04), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n65_), .O(z5));
  inv1   g44(.a(new_n39_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x08), .c(x05), .O(new_n74_));
  nor3   g46(.a(new_n34_), .b(new_n44_), .c(new_n43_), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nor2   g48(.a(new_n39_), .b(new_n76_), .O(new_n77_));
  nor3   g49(.a(new_n60_), .b(new_n63_), .c(new_n44_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n77_), .c(new_n33_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n75_), .c(new_n74_), .O(z6));
  nand3  g52(.a(new_n73_), .b(x08), .c(x06), .O(new_n81_));
  and2   g53(.a(x16), .b(x15), .O(new_n82_));
  nand3  g54(.a(new_n36_), .b(new_n82_), .c(new_n67_), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  oai21  g56(.a(new_n34_), .b(new_n43_), .c(new_n84_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n83_), .c(x12), .O(new_n86_));
  nor2   g58(.a(x18), .b(new_n84_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n44_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n33_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n81_), .O(z7));
  nand2  g63(.a(new_n83_), .b(x18), .O(new_n92_));
  nand4  g64(.a(new_n87_), .b(new_n35_), .c(x12), .d(x11), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(new_n33_), .O(new_n95_));
  aoi21  g67(.a(x08), .b(x07), .c(new_n39_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n95_), .O(z8));
endmodule


