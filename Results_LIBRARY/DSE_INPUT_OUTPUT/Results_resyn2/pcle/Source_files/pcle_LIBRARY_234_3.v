// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  nand2  g00(.a(x10), .b(x06), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n32_), .c(x18), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n29_), .O(z0));
  inv1   g10(.a(new_n29_), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n35_), .b(x11), .c(new_n40_), .O(z1));
  inv1   g13(.a(x01), .O(new_n42_));
  nand2  g14(.a(new_n29_), .b(x08), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai22  g16(.a(new_n44_), .b(new_n35_), .c(new_n43_), .d(new_n42_), .O(z2));
  inv1   g17(.a(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand2  g19(.a(new_n36_), .b(new_n30_), .O(new_n48_));
  oai22  g20(.a(new_n48_), .b(new_n47_), .c(new_n43_), .d(new_n46_), .O(z3));
  inv1   g21(.a(x03), .O(new_n50_));
  inv1   g22(.a(new_n30_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x14), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(new_n30_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n36_), .O(new_n55_));
  oai21  g27(.a(new_n43_), .b(new_n50_), .c(new_n55_), .O(z4));
  aoi21  g28(.a(new_n51_), .b(x14), .c(x15), .O(new_n57_));
  and2   g29(.a(x15), .b(x14), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n51_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n36_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x04), .c(new_n39_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n57_), .c(new_n61_), .O(z5));
  nand3  g34(.a(new_n58_), .b(new_n51_), .c(x16), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(new_n59_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n36_), .O(new_n66_));
  aoi21  g38(.a(x08), .b(x05), .c(new_n39_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(z6));
  nand2  g40(.a(x08), .b(x06), .O(new_n69_));
  inv1   g41(.a(x17), .O(new_n70_));
  nand2  g42(.a(new_n63_), .b(new_n70_), .O(new_n71_));
  inv1   g43(.a(new_n31_), .O(new_n72_));
  nand2  g44(.a(x09), .b(new_n33_), .O(new_n73_));
  aoi21  g45(.a(new_n72_), .b(new_n51_), .c(new_n73_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n69_), .c(x10), .O(z7));
  nand2  g48(.a(new_n32_), .b(x18), .O(new_n77_));
  inv1   g49(.a(x18), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n30_), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n36_), .O(new_n80_));
  aoi21  g52(.a(x08), .b(x07), .c(new_n39_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z8));
endmodule


