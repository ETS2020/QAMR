// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x00), .O(new_n29_));
  nand2  g01(.a(x12), .b(x11), .O(new_n30_));
  nand3  g02(.a(x15), .b(x14), .c(x13), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x16), .O(new_n33_));
  inv1   g05(.a(x17), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  inv1   g07(.a(x09), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(x08), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n35_), .c(new_n32_), .d(x18), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g11(.a(x08), .O(new_n40_));
  nand2  g12(.a(x09), .b(new_n40_), .O(new_n41_));
  inv1   g13(.a(x10), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(x08), .c(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n41_), .c(new_n43_), .O(z1));
  nor2   g18(.a(x12), .b(x11), .O(new_n47_));
  nand3  g19(.a(new_n30_), .b(x09), .c(new_n40_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n47_), .c(new_n29_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x01), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  aoi21  g24(.a(new_n42_), .b(x00), .c(new_n40_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x02), .O(new_n54_));
  inv1   g26(.a(new_n30_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x13), .O(new_n56_));
  nand3  g28(.a(new_n37_), .b(new_n42_), .c(new_n29_), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  inv1   g30(.a(x13), .O(new_n59_));
  nand2  g31(.a(new_n30_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n54_), .O(z3));
  nand2  g34(.a(new_n53_), .b(x03), .O(new_n63_));
  nand4  g35(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  inv1   g36(.a(x14), .O(new_n65_));
  nand2  g37(.a(new_n56_), .b(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(z4));
  inv1   g40(.a(x15), .O(new_n69_));
  nand2  g41(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  inv1   g42(.a(new_n31_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n55_), .c(new_n41_), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n70_), .c(x00), .O(new_n73_));
  nand2  g45(.a(x08), .b(x04), .O(new_n74_));
  oai21  g46(.a(new_n73_), .b(x10), .c(new_n74_), .O(z5));
  nand2  g47(.a(new_n53_), .b(x05), .O(new_n76_));
  oai21  g48(.a(new_n31_), .b(new_n30_), .c(new_n33_), .O(new_n77_));
  nand2  g49(.a(new_n32_), .b(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n58_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n76_), .O(z6));
  nand2  g52(.a(new_n53_), .b(x06), .O(new_n81_));
  nand3  g53(.a(new_n32_), .b(x17), .c(x16), .O(new_n82_));
  nand2  g54(.a(new_n78_), .b(new_n34_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n58_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(z7));
  nand2  g57(.a(new_n53_), .b(x07), .O(new_n86_));
  nand3  g58(.a(new_n35_), .b(new_n32_), .c(x18), .O(new_n87_));
  inv1   g59(.a(x18), .O(new_n88_));
  nand2  g60(.a(new_n35_), .b(new_n32_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n87_), .c(new_n58_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n86_), .O(z8));
endmodule


