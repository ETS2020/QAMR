// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  and2   g03(.a(x17), .b(x16), .O(new_n32_));
  nand2  g04(.a(x15), .b(x14), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n32_), .c(new_n31_), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n29_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x18), .O(new_n39_));
  oai22  g11(.a(new_n39_), .b(new_n35_), .c(x09), .d(new_n29_), .O(z0));
  inv1   g12(.a(x09), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(x00), .c(x08), .O(new_n42_));
  oai21  g14(.a(new_n37_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nor2   g18(.a(x10), .b(x08), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n41_), .O(z2));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n45_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n47_), .c(new_n30_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n50_), .c(new_n41_), .O(z3));
  oai21  g26(.a(new_n41_), .b(x03), .c(x08), .O(new_n55_));
  nor2   g27(.a(new_n31_), .b(x14), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n38_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z4));
  oai21  g31(.a(new_n41_), .b(x04), .c(x08), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n34_), .b(new_n31_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n38_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n60_), .O(z5));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n63_), .b(new_n67_), .O(new_n68_));
  nor2   g40(.a(new_n33_), .b(new_n30_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x16), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n47_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n66_), .c(new_n41_), .O(z6));
  oai21  g44(.a(new_n41_), .b(x06), .c(x08), .O(new_n73_));
  aoi21  g45(.a(new_n69_), .b(x16), .c(x17), .O(new_n74_));
  nand2  g46(.a(new_n38_), .b(new_n35_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z7));
  nand4  g48(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n30_), .c(x18), .O(new_n78_));
  inv1   g50(.a(new_n57_), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n61_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(new_n32_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n47_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n83_), .c(new_n41_), .O(z8));
endmodule


