// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  nor2   g00(.a(x01), .b(x00), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  nand3  g03(.a(x17), .b(x16), .c(x15), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n33_), .c(x18), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n30_), .O(z0));
  aoi21  g11(.a(x08), .b(x00), .c(new_n29_), .O(new_n40_));
  oai21  g12(.a(new_n36_), .b(x11), .c(new_n40_), .O(z1));
  and2   g13(.a(x12), .b(x11), .O(new_n42_));
  inv1   g14(.a(new_n42_), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n37_), .c(new_n43_), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z2));
  nor2   g19(.a(new_n42_), .b(x13), .O(new_n48_));
  nand2  g20(.a(new_n42_), .b(x13), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n29_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  aoi21  g24(.a(new_n42_), .b(x13), .c(x14), .O(new_n53_));
  nand2  g25(.a(new_n37_), .b(new_n31_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n29_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  aoi21  g29(.a(new_n31_), .b(new_n57_), .c(new_n36_), .O(new_n58_));
  oai21  g30(.a(new_n31_), .b(new_n57_), .c(new_n58_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x04), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z5));
  nand2  g33(.a(x08), .b(x05), .O(new_n62_));
  and2   g34(.a(x14), .b(x13), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n42_), .c(x16), .d(x15), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  oai21  g37(.a(new_n31_), .b(new_n57_), .c(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n37_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n62_), .c(new_n29_), .O(z6));
  nand2  g40(.a(x08), .b(x06), .O(new_n69_));
  inv1   g41(.a(x17), .O(new_n70_));
  nand2  g42(.a(new_n64_), .b(new_n70_), .O(new_n71_));
  nor2   g43(.a(new_n36_), .b(new_n33_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n69_), .c(new_n29_), .O(z7));
  nand2  g46(.a(new_n33_), .b(x18), .O(new_n75_));
  inv1   g47(.a(x18), .O(new_n76_));
  oai21  g48(.a(new_n32_), .b(new_n31_), .c(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n37_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x07), .c(new_n29_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z8));
endmodule


