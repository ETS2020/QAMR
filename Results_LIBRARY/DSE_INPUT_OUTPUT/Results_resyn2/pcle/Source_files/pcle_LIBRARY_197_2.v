// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x04), .O(new_n29_));
  nor2   g01(.a(x17), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(x17), .O(new_n36_));
  nand3  g08(.a(x13), .b(x12), .c(x11), .O(new_n37_));
  nand2  g09(.a(x15), .b(x14), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(x18), .c(x16), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n36_), .c(new_n31_), .O(z0));
  aoi21  g13(.a(x08), .b(x00), .c(new_n30_), .O(new_n42_));
  oai21  g14(.a(new_n34_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n35_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n44_), .c(new_n30_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n37_), .b(new_n35_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n30_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  inv1   g24(.a(new_n37_), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(x14), .O(new_n54_));
  nand2  g26(.a(new_n53_), .b(x14), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n35_), .c(new_n31_), .O(new_n56_));
  nand3  g28(.a(new_n31_), .b(x08), .c(x03), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(z4));
  nand3  g30(.a(x17), .b(x08), .c(x04), .O(new_n59_));
  aoi21  g31(.a(new_n53_), .b(x14), .c(x15), .O(new_n60_));
  inv1   g32(.a(new_n38_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n35_), .c(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n60_), .c(new_n59_), .O(z5));
  nand2  g36(.a(x08), .b(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n62_), .b(new_n66_), .O(new_n67_));
  aoi21  g39(.a(new_n39_), .b(x16), .c(new_n34_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n65_), .c(new_n30_), .O(z6));
  inv1   g42(.a(x17), .O(new_n71_));
  nand3  g43(.a(new_n61_), .b(new_n53_), .c(x16), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n34_), .c(new_n29_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  and2   g46(.a(x08), .b(x06), .O(new_n75_));
  aoi21  g47(.a(new_n68_), .b(x17), .c(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n74_), .O(z7));
  inv1   g49(.a(x18), .O(new_n78_));
  nand2  g50(.a(new_n72_), .b(new_n78_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n40_), .c(new_n35_), .d(x17), .O(new_n80_));
  oai21  g52(.a(new_n34_), .b(new_n78_), .c(new_n29_), .O(new_n81_));
  aoi22  g53(.a(new_n81_), .b(new_n71_), .c(x08), .d(x07), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n80_), .O(z8));
endmodule


