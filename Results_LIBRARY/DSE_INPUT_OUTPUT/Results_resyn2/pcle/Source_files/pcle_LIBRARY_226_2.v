// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x03), .O(new_n29_));
  nor2   g01(.a(x18), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  and2   g05(.a(x12), .b(x11), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  and2   g09(.a(x18), .b(x13), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n37_), .c(x17), .d(x16), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n35_), .c(new_n31_), .O(z0));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n43_));
  inv1   g15(.a(new_n43_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n41_), .c(new_n30_), .O(z1));
  inv1   g18(.a(new_n34_), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n44_), .c(new_n47_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x01), .c(new_n30_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand2  g26(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n44_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n53_), .c(new_n30_), .O(z3));
  nand3  g30(.a(x18), .b(x08), .c(x03), .O(new_n59_));
  inv1   g31(.a(new_n56_), .O(new_n60_));
  nor2   g32(.a(new_n60_), .b(x14), .O(new_n61_));
  nand2  g33(.a(new_n60_), .b(x14), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n44_), .c(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n61_), .c(new_n59_), .O(z4));
  inv1   g36(.a(x15), .O(new_n65_));
  nor2   g37(.a(new_n62_), .b(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n62_), .b(new_n65_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n44_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x04), .c(new_n30_), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(z5));
  inv1   g42(.a(x16), .O(new_n71_));
  oai21  g43(.a(new_n56_), .b(new_n36_), .c(new_n71_), .O(new_n72_));
  nor3   g44(.a(new_n56_), .b(new_n36_), .c(new_n71_), .O(new_n73_));
  nor2   g45(.a(new_n73_), .b(new_n43_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  aoi21  g47(.a(x08), .b(x05), .c(new_n30_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(z6));
  nand4  g49(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n78_));
  inv1   g50(.a(new_n78_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n60_), .c(new_n43_), .O(new_n80_));
  oai21  g52(.a(new_n73_), .b(x17), .c(new_n80_), .O(new_n81_));
  aoi21  g53(.a(x08), .b(x06), .c(new_n30_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z7));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nor2   g56(.a(x18), .b(new_n54_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n79_), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n35_), .c(new_n84_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n29_), .O(new_n88_));
  inv1   g60(.a(new_n84_), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n80_), .c(x18), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n88_), .O(z8));
endmodule


