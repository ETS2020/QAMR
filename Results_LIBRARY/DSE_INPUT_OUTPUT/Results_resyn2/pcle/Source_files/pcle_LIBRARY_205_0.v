// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand4  g01(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(x01), .b(x00), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n35_), .c(new_n31_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(z0));
  inv1   g12(.a(x11), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(new_n33_), .c(x09), .O(new_n42_));
  aoi21  g14(.a(new_n32_), .b(x01), .c(x00), .O(new_n43_));
  aoi21  g15(.a(new_n42_), .b(new_n32_), .c(new_n43_), .O(z1));
  aoi21  g16(.a(new_n32_), .b(x00), .c(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n41_), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n46_), .c(new_n33_), .d(x09), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n32_), .c(new_n45_), .O(z2));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n35_), .b(new_n29_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n37_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(z3));
  inv1   g26(.a(new_n29_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n35_), .O(new_n58_));
  aoi21  g30(.a(x08), .b(x03), .c(new_n37_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  inv1   g33(.a(new_n57_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand2  g36(.a(new_n57_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n35_), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n61_), .c(new_n37_), .O(z5));
  inv1   g39(.a(x16), .O(new_n68_));
  nand2  g40(.a(new_n63_), .b(new_n68_), .O(new_n69_));
  nor2   g41(.a(new_n68_), .b(new_n64_), .O(new_n70_));
  nand2  g42(.a(new_n62_), .b(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n35_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x05), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n72_), .c(new_n37_), .O(z6));
  inv1   g46(.a(x17), .O(new_n75_));
  nor2   g47(.a(new_n71_), .b(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n71_), .b(new_n75_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n35_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x06), .c(new_n37_), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(z7));
  oai21  g52(.a(new_n30_), .b(new_n29_), .c(x18), .O(new_n81_));
  nor2   g53(.a(x18), .b(new_n75_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n62_), .c(new_n70_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n35_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n85_), .c(new_n37_), .O(z8));
endmodule


