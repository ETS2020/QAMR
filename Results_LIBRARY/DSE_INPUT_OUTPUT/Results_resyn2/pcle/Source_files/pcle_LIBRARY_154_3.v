// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  nand2  g04(.a(x15), .b(x14), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x05), .O(new_n36_));
  nand4  g08(.a(x18), .b(x17), .c(x16), .d(new_n36_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n35_), .c(new_n31_), .O(z0));
  and2   g10(.a(x16), .b(x05), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(new_n31_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n41_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(new_n39_), .O(z2));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n32_), .b(new_n42_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n39_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(z3));
  inv1   g26(.a(new_n32_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n55_), .b(x14), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n42_), .O(new_n58_));
  aoi21  g30(.a(x08), .b(x03), .c(new_n39_), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n59_), .O(z4));
  aoi21  g32(.a(new_n55_), .b(x14), .c(x15), .O(new_n61_));
  nand2  g33(.a(new_n35_), .b(new_n42_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n39_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(z5));
  aoi22  g36(.a(new_n34_), .b(new_n42_), .c(x08), .d(x05), .O(new_n65_));
  nand4  g37(.a(new_n35_), .b(new_n42_), .c(x16), .d(new_n36_), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(x16), .c(new_n66_), .O(z6));
  nand2  g39(.a(x08), .b(x06), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  inv1   g41(.a(new_n33_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n55_), .c(x16), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g44(.a(new_n34_), .b(x17), .c(x16), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n42_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n68_), .c(new_n39_), .O(z7));
  inv1   g47(.a(x09), .O(new_n76_));
  nor2   g48(.a(new_n76_), .b(x08), .O(new_n77_));
  nor2   g49(.a(x18), .b(new_n69_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(new_n34_), .c(new_n77_), .d(new_n30_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n36_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x16), .O(new_n81_));
  nand4  g53(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n82_));
  inv1   g54(.a(new_n82_), .O(new_n83_));
  aoi22  g55(.a(new_n83_), .b(new_n73_), .c(x08), .d(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(z8));
endmodule


