// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  nand2  g00(.a(x18), .b(x09), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  inv1   g02(.a(x09), .O(new_n31_));
  inv1   g03(.a(x11), .O(new_n32_));
  nor2   g04(.a(x10), .b(x08), .O(new_n33_));
  aoi21  g05(.a(new_n33_), .b(new_n32_), .c(x18), .O(new_n34_));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(new_n31_), .c(new_n35_), .O(z1));
  inv1   g08(.a(x01), .O(new_n37_));
  nand2  g09(.a(new_n29_), .b(x08), .O(new_n38_));
  inv1   g10(.a(x18), .O(new_n39_));
  nand3  g11(.a(new_n33_), .b(new_n39_), .c(x09), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(new_n40_), .c(new_n38_), .d(new_n37_), .O(z2));
  aoi21  g14(.a(x12), .b(x11), .c(x13), .O(new_n43_));
  nand3  g15(.a(x13), .b(x12), .c(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n33_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n43_), .c(new_n39_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x09), .O(new_n47_));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z3));
  inv1   g21(.a(x03), .O(new_n50_));
  inv1   g22(.a(new_n40_), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(new_n44_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  oai21  g27(.a(new_n38_), .b(new_n50_), .c(new_n55_), .O(z4));
  inv1   g28(.a(x04), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nor2   g30(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  nand2  g31(.a(new_n52_), .b(new_n58_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n51_), .O(new_n61_));
  oai22  g33(.a(new_n61_), .b(new_n59_), .c(new_n38_), .d(new_n57_), .O(z5));
  inv1   g34(.a(x05), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nor3   g36(.a(new_n52_), .b(new_n64_), .c(new_n58_), .O(new_n65_));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n44_), .c(new_n64_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n51_), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(new_n65_), .c(new_n38_), .d(new_n63_), .O(z6));
  inv1   g41(.a(x06), .O(new_n70_));
  nand3  g42(.a(x17), .b(x16), .c(x15), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  oai21  g44(.a(new_n64_), .b(new_n58_), .c(new_n72_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n71_), .c(new_n52_), .O(new_n74_));
  nand2  g46(.a(new_n52_), .b(new_n72_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  oai22  g48(.a(new_n76_), .b(new_n74_), .c(new_n38_), .d(new_n70_), .O(z7));
  inv1   g49(.a(new_n52_), .O(new_n78_));
  inv1   g50(.a(new_n71_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n33_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n39_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x09), .O(new_n82_));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


