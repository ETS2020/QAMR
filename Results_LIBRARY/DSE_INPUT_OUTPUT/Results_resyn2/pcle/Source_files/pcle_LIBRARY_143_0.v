// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  oai21  g07(.a(x11), .b(x08), .c(x09), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n37_), .O(z1));
  inv1   g11(.a(x01), .O(new_n40_));
  oai21  g12(.a(x10), .b(x09), .c(x08), .O(new_n41_));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n34_), .c(new_n41_), .d(new_n40_), .O(z2));
  inv1   g15(.a(new_n41_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x02), .O(new_n45_));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  inv1   g18(.a(new_n34_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z3));
  nand2  g21(.a(new_n44_), .b(x03), .O(new_n50_));
  inv1   g22(.a(new_n30_), .O(new_n51_));
  nor2   g23(.a(new_n51_), .b(x14), .O(new_n52_));
  nand2  g24(.a(new_n51_), .b(x14), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(z4));
  nand2  g27(.a(new_n44_), .b(x04), .O(new_n56_));
  aoi21  g28(.a(new_n51_), .b(x14), .c(x15), .O(new_n57_));
  nand3  g29(.a(new_n51_), .b(x15), .c(x14), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(z5));
  nand2  g32(.a(new_n44_), .b(x05), .O(new_n61_));
  inv1   g33(.a(x16), .O(new_n62_));
  aoi21  g34(.a(new_n58_), .b(new_n62_), .c(new_n34_), .O(new_n63_));
  oai21  g35(.a(new_n58_), .b(new_n62_), .c(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n61_), .O(z6));
  nand3  g37(.a(x16), .b(x15), .c(x14), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n51_), .c(x17), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  oai21  g41(.a(new_n66_), .b(new_n30_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n32_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x09), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n33_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x06), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z7));
  inv1   g47(.a(x09), .O(new_n76_));
  oai21  g48(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n77_));
  nand4  g49(.a(new_n67_), .b(new_n51_), .c(new_n29_), .d(x17), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n77_), .c(x08), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n76_), .c(new_n33_), .O(new_n80_));
  nand2  g52(.a(x08), .b(x07), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z8));
endmodule


