// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand4  g01(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n30_));
  or2    g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x09), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n33_), .c(new_n38_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  oai21  g13(.a(new_n40_), .b(x08), .c(new_n41_), .O(z1));
  inv1   g14(.a(x12), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n39_), .O(new_n44_));
  nor2   g16(.a(new_n43_), .b(new_n39_), .O(new_n45_));
  nor2   g17(.a(new_n45_), .b(x10), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n44_), .c(new_n38_), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  oai21  g20(.a(new_n47_), .b(x08), .c(new_n48_), .O(z2));
  nor2   g21(.a(new_n45_), .b(x13), .O(new_n50_));
  nand2  g22(.a(new_n29_), .b(new_n33_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n50_), .c(x09), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n32_), .O(new_n53_));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z3));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand4  g28(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n29_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n35_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n56_), .O(z4));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  inv1   g34(.a(new_n57_), .O(new_n63_));
  nand2  g35(.a(x15), .b(x14), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n29_), .O(new_n65_));
  nor2   g37(.a(new_n65_), .b(new_n34_), .O(new_n66_));
  oai21  g38(.a(new_n63_), .b(x15), .c(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n62_), .O(z5));
  nand2  g40(.a(new_n65_), .b(x16), .O(new_n69_));
  inv1   g41(.a(x16), .O(new_n70_));
  oai21  g42(.a(new_n64_), .b(new_n29_), .c(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n33_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n32_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x05), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z6));
  nand2  g48(.a(x08), .b(x06), .O(new_n77_));
  aoi21  g49(.a(new_n65_), .b(x16), .c(x17), .O(new_n78_));
  nand2  g50(.a(new_n35_), .b(new_n31_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(z7));
  oai21  g52(.a(new_n30_), .b(new_n29_), .c(x18), .O(new_n81_));
  and2   g53(.a(x17), .b(x16), .O(new_n82_));
  inv1   g54(.a(x15), .O(new_n83_));
  nor2   g55(.a(x18), .b(new_n83_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n63_), .c(new_n82_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n81_), .c(x10), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n38_), .c(new_n32_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x07), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(z8));
endmodule


