// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x15), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(x10), .b(new_n33_), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n32_), .c(x06), .O(new_n35_));
  nor4   g07(.a(new_n35_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g08(.a(x00), .O(new_n37_));
  oai21  g09(.a(x10), .b(x06), .c(x08), .O(new_n38_));
  oai22  g10(.a(new_n38_), .b(new_n37_), .c(new_n35_), .d(x11), .O(z1));
  inv1   g11(.a(x01), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(new_n35_), .c(new_n38_), .d(new_n40_), .O(z2));
  inv1   g14(.a(x10), .O(new_n43_));
  aoi21  g15(.a(x12), .b(x11), .c(x13), .O(new_n44_));
  nand3  g16(.a(x13), .b(x12), .c(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x09), .c(new_n32_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n44_), .c(x06), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z3));
  inv1   g22(.a(x03), .O(new_n51_));
  inv1   g23(.a(new_n45_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  inv1   g25(.a(new_n35_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n30_), .O(new_n55_));
  oai22  g27(.a(new_n55_), .b(new_n53_), .c(new_n38_), .d(new_n51_), .O(z4));
  inv1   g28(.a(x06), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(new_n30_), .b(new_n58_), .O(new_n59_));
  inv1   g31(.a(new_n30_), .O(new_n60_));
  nand2  g32(.a(x09), .b(new_n32_), .O(new_n61_));
  aoi21  g33(.a(new_n60_), .b(x15), .c(new_n61_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n59_), .c(new_n57_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x04), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(x10), .c(new_n64_), .O(z5));
  inv1   g37(.a(x16), .O(new_n66_));
  oai21  g38(.a(new_n30_), .b(new_n58_), .c(new_n66_), .O(new_n67_));
  nor2   g39(.a(new_n66_), .b(new_n58_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n60_), .c(new_n61_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n67_), .c(new_n57_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(x10), .c(new_n71_), .O(z6));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n68_), .b(new_n60_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand4  g47(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n52_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n75_), .c(new_n34_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n32_), .c(new_n57_), .O(z7));
  oai21  g52(.a(new_n76_), .b(new_n45_), .c(x18), .O(new_n81_));
  inv1   g53(.a(new_n31_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n60_), .c(new_n29_), .O(new_n83_));
  aoi21  g55(.a(new_n83_), .b(new_n81_), .c(new_n61_), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n57_), .c(new_n43_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n85_), .O(z8));
endmodule


