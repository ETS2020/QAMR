// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x17), .c(x16), .d(x15), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n31_), .O(z0));
  inv1   g07(.a(x00), .O(new_n36_));
  inv1   g08(.a(x09), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x08), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  inv1   g11(.a(x15), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n30_), .c(x08), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(new_n36_), .c(new_n39_), .d(x11), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  nor2   g15(.a(new_n40_), .b(new_n30_), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n44_), .O(new_n45_));
  oai21  g17(.a(new_n43_), .b(new_n39_), .c(new_n45_), .O(z2));
  inv1   g18(.a(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n38_), .c(new_n30_), .O(new_n50_));
  oai22  g22(.a(new_n50_), .b(new_n48_), .c(new_n41_), .d(new_n47_), .O(z3));
  inv1   g23(.a(new_n49_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand3  g25(.a(new_n32_), .b(new_n38_), .c(new_n30_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n44_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z4));
  oai21  g28(.a(new_n32_), .b(x10), .c(new_n40_), .O(new_n57_));
  nand2  g29(.a(new_n33_), .b(x15), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n57_), .c(new_n38_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x04), .c(new_n44_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z5));
  inv1   g33(.a(x16), .O(new_n62_));
  nand3  g34(.a(new_n33_), .b(new_n38_), .c(new_n62_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n30_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x15), .O(new_n65_));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  inv1   g38(.a(new_n39_), .O(new_n67_));
  nand3  g39(.a(new_n58_), .b(new_n67_), .c(x16), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z6));
  inv1   g41(.a(new_n41_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x06), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(x16), .b(x15), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n32_), .c(new_n72_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n34_), .c(new_n67_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n71_), .O(z7));
  nand2  g48(.a(x17), .b(x16), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(x18), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n33_), .c(new_n38_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n30_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x15), .O(new_n81_));
  inv1   g53(.a(new_n31_), .O(new_n82_));
  and2   g54(.a(x08), .b(x07), .O(new_n83_));
  aoi21  g55(.a(new_n34_), .b(new_n82_), .c(new_n83_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(z8));
endmodule


