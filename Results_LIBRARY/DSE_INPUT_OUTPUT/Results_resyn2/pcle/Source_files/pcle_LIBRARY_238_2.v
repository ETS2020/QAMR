// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x01), .O(new_n29_));
  nor2   g01(.a(x18), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(x13), .b(x12), .c(x11), .O(new_n36_));
  nand2  g08(.a(x15), .b(x14), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  and2   g10(.a(x17), .b(x16), .O(new_n39_));
  nand4  g11(.a(new_n39_), .b(new_n38_), .c(new_n35_), .d(x18), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n31_), .O(z0));
  aoi21  g13(.a(x08), .b(x00), .c(new_n30_), .O(new_n42_));
  oai21  g14(.a(new_n34_), .b(x11), .c(new_n42_), .O(z1));
  inv1   g15(.a(x18), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(x08), .c(x01), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n34_), .c(new_n45_), .O(z2));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  inv1   g20(.a(x13), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n36_), .c(new_n35_), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n48_), .c(new_n30_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand4  g26(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n36_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n35_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n54_), .c(new_n30_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nor2   g32(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  nand2  g33(.a(new_n55_), .b(new_n60_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n35_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n30_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(z5));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  oai21  g39(.a(new_n55_), .b(new_n60_), .c(new_n67_), .O(new_n68_));
  inv1   g40(.a(new_n36_), .O(new_n69_));
  inv1   g41(.a(new_n37_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(x16), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n68_), .c(new_n35_), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n66_), .c(new_n30_), .O(z6));
  aoi21  g45(.a(new_n39_), .b(new_n38_), .c(new_n34_), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n71_), .b(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x06), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n77_), .c(new_n30_), .O(z7));
  nand3  g51(.a(new_n39_), .b(new_n70_), .c(new_n69_), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n34_), .c(new_n29_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n44_), .O(new_n82_));
  and2   g54(.a(x08), .b(x07), .O(new_n83_));
  aoi21  g55(.a(new_n74_), .b(x18), .c(new_n83_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n82_), .O(z8));
endmodule


