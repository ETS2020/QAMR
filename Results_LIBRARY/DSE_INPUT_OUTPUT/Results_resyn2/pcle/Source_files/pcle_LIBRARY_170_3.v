// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x12), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  aoi21  g06(.a(x08), .b(x00), .c(z0), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(x11), .c(new_n35_), .O(z1));
  xnor2a g08(.a(x12), .b(x11), .O(new_n37_));
  aoi21  g09(.a(x08), .b(x01), .c(z0), .O(new_n38_));
  oai21  g10(.a(new_n37_), .b(new_n34_), .c(new_n38_), .O(z2));
  inv1   g11(.a(z0), .O(new_n40_));
  nand3  g12(.a(new_n40_), .b(x08), .c(x02), .O(new_n41_));
  inv1   g13(.a(new_n34_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  xor2a  g16(.a(new_n44_), .b(x13), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n43_), .c(new_n41_), .O(z3));
  nand4  g18(.a(x11), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n47_));
  inv1   g19(.a(x14), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(x13), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n30_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x12), .O(new_n51_));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand4  g24(.a(x14), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  aoi22  g26(.a(new_n54_), .b(new_n52_), .c(x08), .d(x03), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n51_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(x14), .c(x13), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n47_), .c(new_n30_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x12), .O(new_n60_));
  nand4  g32(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  nand4  g33(.a(x15), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n62_));
  inv1   g34(.a(new_n62_), .O(new_n63_));
  aoi22  g35(.a(new_n63_), .b(new_n61_), .c(x08), .d(x04), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n60_), .O(z5));
  inv1   g37(.a(x16), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(x15), .c(x14), .d(x13), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n47_), .c(new_n30_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x12), .O(new_n69_));
  inv1   g41(.a(new_n61_), .O(new_n70_));
  nand4  g42(.a(x16), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n71_));
  aoi21  g43(.a(new_n70_), .b(x15), .c(new_n71_), .O(new_n72_));
  aoi21  g44(.a(x08), .b(x05), .c(new_n72_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n69_), .O(z6));
  inv1   g46(.a(new_n47_), .O(new_n75_));
  nand2  g47(.a(x16), .b(x15), .O(new_n76_));
  inv1   g48(.a(x17), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(x14), .c(x13), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n75_), .c(x18), .O(new_n80_));
  and2   g52(.a(x08), .b(x06), .O(new_n81_));
  nand3  g53(.a(new_n70_), .b(x16), .c(x15), .O(new_n82_));
  nand4  g54(.a(x17), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  oai21  g57(.a(new_n80_), .b(new_n29_), .c(new_n85_), .O(z7));
  nand3  g58(.a(new_n40_), .b(x08), .c(x07), .O(new_n87_));
  nor2   g59(.a(new_n76_), .b(new_n61_), .O(new_n88_));
  nor2   g60(.a(x18), .b(new_n77_), .O(new_n89_));
  aoi22  g61(.a(new_n89_), .b(new_n88_), .c(x18), .d(new_n29_), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n34_), .c(new_n87_), .O(z8));
endmodule


