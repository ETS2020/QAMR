// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(x16), .O(new_n33_));
  inv1   g05(.a(x17), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x18), .c(x15), .d(x05), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n32_), .c(new_n31_), .O(z0));
  nor2   g09(.a(new_n33_), .b(x05), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(new_n31_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n39_), .c(new_n38_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n38_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n38_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z3));
  inv1   g26(.a(new_n51_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n32_), .b(new_n41_), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x03), .c(new_n38_), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nor2   g32(.a(new_n32_), .b(new_n60_), .O(new_n61_));
  inv1   g33(.a(new_n32_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(x15), .c(new_n41_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n38_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n61_), .c(new_n64_), .O(z5));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  nor2   g38(.a(new_n66_), .b(new_n51_), .O(new_n67_));
  nand3  g39(.a(x16), .b(new_n30_), .c(x09), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n67_), .c(new_n29_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x05), .O(new_n70_));
  nand3  g42(.a(new_n67_), .b(new_n41_), .c(new_n33_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  nand2  g44(.a(x08), .b(x06), .O(new_n73_));
  inv1   g45(.a(new_n66_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n55_), .c(x16), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n34_), .O(new_n76_));
  aoi21  g48(.a(new_n67_), .b(new_n35_), .c(new_n31_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n73_), .c(new_n38_), .O(z7));
  inv1   g51(.a(x09), .O(new_n80_));
  nor2   g52(.a(new_n80_), .b(x08), .O(new_n81_));
  nand2  g53(.a(x17), .b(x15), .O(new_n82_));
  nor2   g54(.a(new_n82_), .b(x18), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n62_), .c(new_n81_), .d(new_n30_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x05), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(x16), .O(new_n86_));
  and2   g58(.a(x08), .b(x07), .O(new_n87_));
  aoi21  g59(.a(new_n77_), .b(x18), .c(new_n87_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(z8));
endmodule


