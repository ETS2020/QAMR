// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x18), .O(new_n29_));
  nor2   g01(.a(x10), .b(x08), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand2  g03(.a(x15), .b(x14), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  and2   g05(.a(x17), .b(x16), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n33_), .c(new_n30_), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(x09), .c(new_n29_), .O(z0));
  nand2  g08(.a(new_n30_), .b(x09), .O(new_n37_));
  nor2   g09(.a(new_n29_), .b(x09), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n37_), .b(x11), .c(new_n39_), .O(z1));
  nand2  g12(.a(x12), .b(x11), .O(new_n41_));
  inv1   g13(.a(new_n37_), .O(new_n42_));
  or2    g14(.a(x12), .b(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n38_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z2));
  inv1   g18(.a(x08), .O(new_n47_));
  nor2   g19(.a(new_n38_), .b(new_n47_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(x02), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n41_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n42_), .c(new_n31_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n49_), .O(z3));
  nand2  g25(.a(new_n48_), .b(x03), .O(new_n54_));
  inv1   g26(.a(new_n31_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x14), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n31_), .b(new_n57_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n42_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n54_), .O(z4));
  nand2  g32(.a(new_n48_), .b(x04), .O(new_n61_));
  inv1   g33(.a(new_n32_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n55_), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand2  g36(.a(new_n56_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n42_), .c(new_n63_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n61_), .O(z5));
  nand2  g39(.a(new_n48_), .b(x05), .O(new_n68_));
  nand2  g40(.a(new_n33_), .b(x16), .O(new_n69_));
  inv1   g41(.a(x16), .O(new_n70_));
  nand2  g42(.a(new_n63_), .b(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n42_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n68_), .O(z6));
  aoi21  g45(.a(new_n33_), .b(x16), .c(x17), .O(new_n74_));
  nand3  g46(.a(new_n34_), .b(new_n62_), .c(new_n55_), .O(new_n75_));
  nand2  g47(.a(new_n42_), .b(new_n75_), .O(new_n76_));
  aoi21  g48(.a(x08), .b(x06), .c(new_n38_), .O(new_n77_));
  oai21  g49(.a(new_n76_), .b(new_n74_), .c(new_n77_), .O(z7));
  nand2  g50(.a(new_n29_), .b(x09), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n34_), .c(new_n33_), .O(new_n80_));
  nand2  g52(.a(new_n75_), .b(new_n29_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n80_), .c(new_n30_), .O(new_n82_));
  aoi21  g54(.a(x08), .b(x07), .c(new_n38_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


