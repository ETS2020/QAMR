// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_;
  nand3  g00(.a(x15), .b(x14), .c(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x18), .b(x17), .c(x16), .d(x09), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(x13), .b(x12), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nor2   g06(.a(x10), .b(x08), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n34_), .c(new_n32_), .d(new_n30_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  inv1   g10(.a(x09), .O(new_n39_));
  oai21  g11(.a(x18), .b(new_n39_), .c(x08), .O(new_n40_));
  nand3  g12(.a(new_n35_), .b(x18), .c(x09), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(x11), .c(new_n40_), .d(new_n38_), .O(z1));
  nor2   g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n35_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n43_), .c(x18), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x09), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  inv1   g21(.a(x02), .O(new_n50_));
  inv1   g22(.a(new_n41_), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n44_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n55_));
  oai21  g27(.a(new_n40_), .b(new_n50_), .c(new_n55_), .O(z3));
  inv1   g28(.a(new_n54_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n35_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x18), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x09), .O(new_n63_));
  nand2  g35(.a(x08), .b(x03), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z4));
  inv1   g37(.a(x04), .O(new_n66_));
  aoi21  g38(.a(new_n57_), .b(x14), .c(x15), .O(new_n67_));
  nand2  g39(.a(new_n34_), .b(new_n30_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n51_), .O(new_n69_));
  oai22  g41(.a(new_n69_), .b(new_n67_), .c(new_n40_), .d(new_n66_), .O(z5));
  nand3  g42(.a(new_n34_), .b(new_n30_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  oai21  g44(.a(new_n33_), .b(new_n29_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n35_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(x18), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x09), .O(new_n76_));
  nand2  g48(.a(x08), .b(x05), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(z6));
  inv1   g50(.a(x18), .O(new_n79_));
  inv1   g51(.a(x17), .O(new_n80_));
  nand2  g52(.a(new_n68_), .b(new_n80_), .O(new_n81_));
  inv1   g53(.a(new_n35_), .O(new_n82_));
  nor2   g54(.a(new_n33_), .b(new_n29_), .O(new_n83_));
  xnor2a g55(.a(x17), .b(x16), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n81_), .c(new_n79_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x06), .O(new_n87_));
  oai21  g59(.a(new_n86_), .b(new_n39_), .c(new_n87_), .O(z7));
  inv1   g60(.a(x07), .O(new_n89_));
  nand2  g61(.a(x17), .b(x16), .O(new_n90_));
  nor2   g62(.a(new_n68_), .b(new_n90_), .O(new_n91_));
  oai22  g63(.a(new_n91_), .b(new_n41_), .c(new_n40_), .d(new_n89_), .O(z8));
endmodule


