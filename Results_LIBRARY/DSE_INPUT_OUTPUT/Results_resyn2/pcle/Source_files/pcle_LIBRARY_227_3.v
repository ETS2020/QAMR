// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_;
  inv1   g00(.a(x17), .O(new_n30_));
  inv1   g01(.a(x18), .O(new_n31_));
  nor2   g02(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g03(.a(x08), .b(x00), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  inv1   g05(.a(x08), .O(new_n35_));
  inv1   g06(.a(x10), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  aoi21  g10(.a(new_n39_), .b(new_n33_), .c(new_n32_), .O(z1));
  xnor2a g11(.a(x12), .b(x11), .O(new_n41_));
  aoi21  g12(.a(x08), .b(x01), .c(new_n32_), .O(new_n42_));
  oai21  g13(.a(new_n41_), .b(new_n37_), .c(new_n42_), .O(z2));
  nand2  g14(.a(x08), .b(x02), .O(new_n44_));
  inv1   g15(.a(x13), .O(new_n45_));
  nand2  g16(.a(x12), .b(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n47_), .c(new_n38_), .O(new_n49_));
  aoi21  g20(.a(new_n49_), .b(new_n44_), .c(new_n32_), .O(z3));
  inv1   g21(.a(x14), .O(new_n51_));
  aoi21  g22(.a(new_n48_), .b(new_n51_), .c(new_n37_), .O(new_n52_));
  oai21  g23(.a(new_n48_), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  aoi21  g24(.a(x08), .b(x03), .c(new_n32_), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n53_), .O(z4));
  nand2  g26(.a(x08), .b(x04), .O(new_n56_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(x15), .O(new_n59_));
  inv1   g30(.a(x15), .O(new_n60_));
  nand2  g31(.a(new_n57_), .b(new_n60_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n59_), .c(new_n38_), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(new_n56_), .c(new_n32_), .O(z5));
  inv1   g34(.a(x16), .O(new_n64_));
  nor2   g35(.a(new_n59_), .b(new_n64_), .O(new_n65_));
  oai21  g36(.a(new_n57_), .b(new_n60_), .c(new_n64_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n38_), .O(new_n67_));
  aoi21  g38(.a(x08), .b(x05), .c(new_n32_), .O(new_n68_));
  oai21  g39(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(z6));
  nand2  g40(.a(x16), .b(x15), .O(new_n70_));
  nor2   g41(.a(new_n70_), .b(new_n57_), .O(new_n71_));
  xor2a  g42(.a(new_n71_), .b(new_n30_), .O(new_n72_));
  aoi21  g43(.a(x08), .b(x06), .c(new_n32_), .O(new_n73_));
  oai21  g44(.a(new_n72_), .b(new_n37_), .c(new_n73_), .O(z7));
  nor2   g45(.a(new_n71_), .b(new_n30_), .O(new_n75_));
  inv1   g46(.a(new_n32_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x08), .c(x07), .O(new_n77_));
  nand2  g48(.a(new_n31_), .b(new_n30_), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(new_n38_), .c(new_n76_), .O(new_n79_));
  oai21  g50(.a(new_n79_), .b(new_n75_), .c(new_n77_), .O(z8));
  zero   g51(.O(z0));
endmodule


