// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand2  g02(.a(x15), .b(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x16), .O(new_n33_));
  nor2   g05(.a(x10), .b(x08), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x17), .c(x09), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n33_), .c(new_n29_), .O(z0));
  inv1   g08(.a(x00), .O(new_n37_));
  inv1   g09(.a(x09), .O(new_n38_));
  oai21  g10(.a(x17), .b(new_n38_), .c(x08), .O(new_n39_));
  oai22  g11(.a(new_n39_), .b(new_n37_), .c(new_n35_), .d(x11), .O(z1));
  inv1   g12(.a(x01), .O(new_n41_));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n35_), .c(new_n39_), .d(new_n41_), .O(z2));
  aoi21  g15(.a(x12), .b(x11), .c(x13), .O(new_n44_));
  nand2  g16(.a(new_n34_), .b(new_n30_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n44_), .c(x17), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x09), .O(new_n47_));
  nand2  g19(.a(x08), .b(x02), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z3));
  inv1   g21(.a(x14), .O(new_n50_));
  nand2  g22(.a(new_n30_), .b(new_n50_), .O(new_n51_));
  inv1   g23(.a(new_n30_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n34_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x17), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x09), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z4));
  inv1   g30(.a(x04), .O(new_n59_));
  inv1   g31(.a(new_n53_), .O(new_n60_));
  nor2   g32(.a(new_n35_), .b(new_n32_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(x15), .c(new_n61_), .O(new_n62_));
  oai21  g34(.a(new_n39_), .b(new_n59_), .c(new_n62_), .O(z5));
  inv1   g35(.a(x05), .O(new_n64_));
  nor2   g36(.a(new_n32_), .b(x16), .O(new_n65_));
  inv1   g37(.a(new_n35_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  oai22  g39(.a(new_n67_), .b(new_n65_), .c(new_n39_), .d(new_n64_), .O(z6));
  inv1   g40(.a(x06), .O(new_n69_));
  oai21  g41(.a(new_n39_), .b(new_n69_), .c(new_n67_), .O(z7));
  nand3  g42(.a(x16), .b(x15), .c(x14), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n52_), .c(x18), .O(new_n73_));
  oai21  g45(.a(new_n71_), .b(new_n30_), .c(new_n29_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n34_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x17), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x09), .O(new_n77_));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n77_), .O(z8));
endmodule


