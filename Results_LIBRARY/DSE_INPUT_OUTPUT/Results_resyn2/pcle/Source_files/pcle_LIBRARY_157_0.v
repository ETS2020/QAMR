// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_;
  nor2   g00(.a(x18), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand4  g04(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x18), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n35_), .c(new_n30_), .O(z0));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n39_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n29_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n39_), .c(new_n47_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n29_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n39_), .c(new_n31_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n52_), .c(new_n29_), .O(z3));
  nor2   g28(.a(new_n32_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  oai21  g30(.a(new_n31_), .b(new_n58_), .c(new_n39_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n29_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x04), .O(new_n62_));
  nand2  g34(.a(new_n30_), .b(x08), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  oai21  g36(.a(new_n31_), .b(new_n58_), .c(new_n64_), .O(new_n65_));
  nor2   g37(.a(new_n38_), .b(new_n29_), .O(new_n66_));
  nand3  g38(.a(new_n32_), .b(x15), .c(x14), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(new_n68_));
  oai21  g40(.a(new_n63_), .b(new_n62_), .c(new_n68_), .O(z5));
  inv1   g41(.a(x05), .O(new_n70_));
  nand2  g42(.a(x16), .b(x14), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n32_), .c(x15), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  nand2  g46(.a(x15), .b(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n31_), .c(new_n74_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n73_), .c(new_n66_), .O(new_n77_));
  oai21  g49(.a(new_n63_), .b(new_n70_), .c(new_n77_), .O(z6));
  inv1   g50(.a(x17), .O(new_n79_));
  nand2  g51(.a(new_n73_), .b(new_n79_), .O(new_n80_));
  nor2   g52(.a(new_n71_), .b(new_n31_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(x17), .c(new_n38_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand4  g55(.a(x17), .b(new_n37_), .c(x09), .d(new_n36_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x18), .O(new_n85_));
  and2   g57(.a(x08), .b(x06), .O(new_n86_));
  aoi21  g58(.a(new_n85_), .b(new_n64_), .c(new_n86_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n83_), .O(z7));
  inv1   g60(.a(x07), .O(new_n89_));
  nand3  g61(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n90_));
  inv1   g62(.a(x18), .O(new_n91_));
  oai21  g63(.a(new_n33_), .b(new_n31_), .c(new_n91_), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n90_), .c(new_n39_), .O(new_n93_));
  oai21  g65(.a(new_n63_), .b(new_n89_), .c(new_n93_), .O(z8));
endmodule


