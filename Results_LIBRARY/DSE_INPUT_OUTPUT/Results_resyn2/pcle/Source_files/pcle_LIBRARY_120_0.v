// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  oai21  g07(.a(x11), .b(x08), .c(x09), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n37_), .O(z1));
  inv1   g11(.a(x01), .O(new_n40_));
  oai21  g12(.a(x10), .b(x09), .c(x08), .O(new_n41_));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n34_), .c(new_n41_), .d(new_n40_), .O(z2));
  inv1   g15(.a(x02), .O(new_n44_));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  inv1   g17(.a(new_n34_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n30_), .O(new_n47_));
  oai22  g19(.a(new_n47_), .b(new_n45_), .c(new_n41_), .d(new_n44_), .O(z3));
  inv1   g20(.a(x03), .O(new_n49_));
  inv1   g21(.a(new_n30_), .O(new_n50_));
  nor2   g22(.a(new_n50_), .b(x14), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  oai22  g25(.a(new_n53_), .b(new_n51_), .c(new_n41_), .d(new_n49_), .O(z4));
  inv1   g26(.a(new_n52_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x15), .O(new_n56_));
  inv1   g28(.a(x15), .O(new_n57_));
  oai21  g29(.a(new_n52_), .b(new_n57_), .c(new_n32_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n56_), .c(x09), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n33_), .O(new_n60_));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n60_), .O(z5));
  inv1   g34(.a(x05), .O(new_n63_));
  aoi21  g35(.a(new_n55_), .b(x15), .c(x16), .O(new_n64_));
  nand2  g36(.a(x16), .b(x15), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  nand2  g38(.a(new_n55_), .b(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n46_), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(new_n64_), .c(new_n41_), .d(new_n63_), .O(z6));
  nand3  g41(.a(new_n55_), .b(new_n66_), .c(x17), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  oai21  g43(.a(new_n52_), .b(new_n65_), .c(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n32_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x09), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n33_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x06), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(z7));
  inv1   g49(.a(x09), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n71_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n55_), .c(new_n66_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n79_), .c(x08), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n78_), .c(new_n33_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


