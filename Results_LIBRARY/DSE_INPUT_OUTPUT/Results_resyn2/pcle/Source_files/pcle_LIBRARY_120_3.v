// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g07(.a(x09), .O(new_n36_));
  inv1   g08(.a(x11), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n32_), .c(x10), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  oai21  g11(.a(new_n38_), .b(new_n36_), .c(new_n39_), .O(z1));
  inv1   g12(.a(x01), .O(new_n41_));
  oai21  g13(.a(new_n33_), .b(new_n36_), .c(x08), .O(new_n42_));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(new_n34_), .c(new_n42_), .d(new_n41_), .O(z2));
  inv1   g16(.a(x02), .O(new_n45_));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  inv1   g18(.a(new_n34_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  oai22  g20(.a(new_n48_), .b(new_n46_), .c(new_n42_), .d(new_n45_), .O(z3));
  inv1   g21(.a(x03), .O(new_n50_));
  inv1   g22(.a(new_n30_), .O(new_n51_));
  nor2   g23(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  oai22  g26(.a(new_n54_), .b(new_n52_), .c(new_n42_), .d(new_n50_), .O(z4));
  inv1   g27(.a(new_n53_), .O(new_n56_));
  nor2   g28(.a(new_n56_), .b(x15), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  oai21  g30(.a(new_n53_), .b(new_n58_), .c(new_n32_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n57_), .c(new_n33_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x09), .O(new_n61_));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z5));
  inv1   g35(.a(x05), .O(new_n64_));
  aoi21  g36(.a(new_n56_), .b(x15), .c(x16), .O(new_n65_));
  nand2  g37(.a(x16), .b(x15), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n56_), .b(new_n67_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n47_), .O(new_n69_));
  oai22  g41(.a(new_n69_), .b(new_n65_), .c(new_n42_), .d(new_n64_), .O(z6));
  nand3  g42(.a(new_n56_), .b(new_n67_), .c(x17), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  oai21  g44(.a(new_n53_), .b(new_n66_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n32_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n33_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x09), .O(new_n76_));
  nand2  g48(.a(x08), .b(x06), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(z7));
  oai21  g50(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n72_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n56_), .c(new_n67_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n79_), .c(x08), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(x10), .c(x09), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


