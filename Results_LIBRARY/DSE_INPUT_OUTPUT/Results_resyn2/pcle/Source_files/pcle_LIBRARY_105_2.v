// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x01), .O(new_n29_));
  nand2  g01(.a(new_n29_), .b(x00), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(x15), .b(x14), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  and2   g06(.a(x17), .b(x16), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n34_), .c(new_n32_), .O(new_n36_));
  inv1   g08(.a(x08), .O(new_n37_));
  inv1   g09(.a(x10), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(x09), .c(new_n37_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x18), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n36_), .c(new_n30_), .O(z0));
  inv1   g14(.a(x00), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  inv1   g16(.a(x11), .O(new_n45_));
  nand2  g17(.a(new_n30_), .b(new_n45_), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(new_n39_), .c(new_n44_), .d(new_n43_), .O(z1));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  inv1   g20(.a(x12), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n40_), .c(new_n48_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n44_), .c(new_n30_), .O(z2));
  aoi21  g24(.a(x12), .b(x11), .c(x13), .O(new_n53_));
  nand2  g25(.a(new_n40_), .b(new_n31_), .O(new_n54_));
  inv1   g26(.a(new_n30_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g28(.a(new_n54_), .b(new_n53_), .c(new_n56_), .O(z3));
  nor2   g29(.a(new_n32_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  oai21  g31(.a(new_n31_), .b(new_n59_), .c(new_n40_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x03), .c(new_n55_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z4));
  nand2  g34(.a(x08), .b(x04), .O(new_n63_));
  nand2  g35(.a(new_n34_), .b(new_n32_), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  oai21  g37(.a(new_n31_), .b(new_n59_), .c(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n40_), .c(new_n64_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n63_), .c(new_n55_), .O(z5));
  nand2  g40(.a(x08), .b(x05), .O(new_n69_));
  nand3  g41(.a(new_n34_), .b(new_n32_), .c(x16), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  oai21  g43(.a(new_n33_), .b(new_n31_), .c(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n40_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n69_), .c(new_n55_), .O(z6));
  nand4  g46(.a(new_n34_), .b(new_n32_), .c(x17), .d(x16), .O(new_n75_));
  inv1   g47(.a(x17), .O(new_n76_));
  nand2  g48(.a(new_n70_), .b(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n40_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x06), .c(new_n55_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z7));
  nand4  g52(.a(new_n35_), .b(new_n34_), .c(new_n32_), .d(x18), .O(new_n81_));
  inv1   g53(.a(x18), .O(new_n82_));
  nand2  g54(.a(new_n36_), .b(new_n82_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n81_), .c(new_n40_), .O(new_n84_));
  aoi21  g56(.a(x08), .b(x07), .c(new_n55_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


