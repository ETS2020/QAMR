// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  and2   g00(.a(x12), .b(x11), .O(new_n29_));
  and2   g01(.a(x14), .b(x13), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(new_n29_), .c(x15), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n34_), .c(new_n31_), .O(z0));
  inv1   g08(.a(x18), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x17), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  inv1   g12(.a(x09), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(x08), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n33_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n42_), .c(new_n46_), .d(new_n33_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(new_n38_), .O(z2));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n53_), .c(new_n42_), .d(new_n33_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n51_), .c(new_n38_), .O(z3));
  inv1   g28(.a(new_n54_), .O(new_n57_));
  nor2   g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand4  g30(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  nand3  g31(.a(new_n42_), .b(new_n59_), .c(new_n33_), .O(new_n60_));
  inv1   g32(.a(x17), .O(new_n61_));
  aoi22  g33(.a(x18), .b(new_n61_), .c(x08), .d(x03), .O(new_n62_));
  oai21  g34(.a(new_n60_), .b(new_n58_), .c(new_n62_), .O(z4));
  inv1   g35(.a(new_n34_), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand2  g37(.a(new_n59_), .b(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n31_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n38_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(z5));
  nand4  g41(.a(new_n30_), .b(new_n29_), .c(x16), .d(x15), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  oai21  g43(.a(new_n59_), .b(new_n65_), .c(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n64_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x05), .c(new_n38_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  and2   g47(.a(x16), .b(x15), .O(new_n76_));
  nand2  g48(.a(new_n37_), .b(new_n61_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n76_), .c(new_n30_), .d(new_n29_), .O(new_n78_));
  nand2  g50(.a(x16), .b(x15), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n59_), .c(new_n61_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n64_), .O(new_n81_));
  inv1   g53(.a(new_n38_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x08), .c(x06), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n81_), .O(z7));
  nand2  g56(.a(new_n37_), .b(x17), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n76_), .c(new_n30_), .d(new_n29_), .O(new_n86_));
  oai21  g58(.a(new_n79_), .b(new_n59_), .c(new_n37_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n64_), .O(new_n88_));
  aoi21  g60(.a(x08), .b(x07), .c(new_n38_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z8));
endmodule


