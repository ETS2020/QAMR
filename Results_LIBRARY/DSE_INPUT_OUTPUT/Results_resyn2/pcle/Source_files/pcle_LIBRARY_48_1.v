// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x01), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x00), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(x16), .b(x15), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n35_), .c(x18), .d(x17), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n33_), .c(new_n30_), .O(z0));
  inv1   g11(.a(new_n30_), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(new_n33_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(z1));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n41_), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n46_), .c(new_n42_), .d(new_n40_), .O(new_n49_));
  oai21  g21(.a(new_n44_), .b(new_n29_), .c(new_n49_), .O(z2));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n42_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n30_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n51_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n34_), .c(new_n42_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n57_), .c(new_n30_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nor2   g34(.a(new_n34_), .b(new_n62_), .O(new_n63_));
  oai21  g35(.a(new_n35_), .b(x15), .c(new_n42_), .O(new_n64_));
  aoi21  g36(.a(x08), .b(x04), .c(new_n30_), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(new_n63_), .c(new_n65_), .O(z5));
  nand2  g38(.a(x08), .b(x05), .O(new_n67_));
  nand2  g39(.a(new_n37_), .b(new_n35_), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n34_), .b(new_n62_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(new_n42_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n67_), .c(new_n30_), .O(z6));
  nand2  g44(.a(x08), .b(x06), .O(new_n73_));
  nand3  g45(.a(new_n37_), .b(new_n35_), .c(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  oai21  g47(.a(new_n36_), .b(new_n34_), .c(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n42_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n73_), .c(new_n30_), .O(z7));
  nor2   g50(.a(new_n36_), .b(new_n34_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(x17), .c(x18), .O(new_n80_));
  nand2  g52(.a(new_n38_), .b(new_n42_), .O(new_n81_));
  aoi21  g53(.a(x08), .b(x07), .c(new_n30_), .O(new_n82_));
  oai21  g54(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z8));
endmodule


