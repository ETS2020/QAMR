// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand2  g00(.a(x17), .b(x16), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(new_n30_), .b(x18), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(x13), .b(x12), .c(x11), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n31_), .O(z0));
  inv1   g11(.a(x11), .O(new_n40_));
  nand2  g12(.a(new_n35_), .b(new_n40_), .O(new_n41_));
  inv1   g13(.a(x15), .O(new_n42_));
  nand2  g14(.a(x18), .b(new_n42_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n40_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n35_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n49_), .c(new_n43_), .O(z2));
  inv1   g23(.a(new_n43_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand2  g26(.a(new_n46_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n36_), .c(new_n35_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(z3));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand4  g30(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n36_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n35_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n58_), .c(new_n52_), .O(z4));
  nand2  g35(.a(new_n59_), .b(new_n42_), .O(new_n64_));
  nand3  g36(.a(new_n37_), .b(x15), .c(x14), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n64_), .c(new_n43_), .d(new_n35_), .O(new_n66_));
  nand3  g38(.a(new_n43_), .b(x08), .c(x04), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(z5));
  inv1   g40(.a(new_n59_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(x16), .c(x15), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  oai21  g43(.a(new_n59_), .b(new_n42_), .c(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n35_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x05), .c(new_n52_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x15), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n69_), .c(x16), .O(new_n78_));
  oai21  g50(.a(new_n59_), .b(new_n71_), .c(new_n76_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n35_), .O(new_n80_));
  inv1   g52(.a(x18), .O(new_n81_));
  nand4  g53(.a(x17), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n42_), .O(new_n84_));
  nand2  g56(.a(x08), .b(x06), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(new_n80_), .O(z7));
  nand2  g58(.a(new_n81_), .b(x15), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n69_), .c(new_n30_), .O(new_n88_));
  oai21  g60(.a(new_n59_), .b(new_n29_), .c(new_n81_), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(new_n88_), .c(new_n35_), .O(new_n90_));
  aoi21  g62(.a(x08), .b(x07), .c(new_n52_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z8));
endmodule


