// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  nand4  g02(.a(x15), .b(x14), .c(x12), .d(x11), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(new_n29_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n34_), .c(new_n32_), .d(x16), .O(new_n37_));
  oai21  g09(.a(x13), .b(new_n29_), .c(new_n37_), .O(z0));
  inv1   g10(.a(x08), .O(new_n39_));
  aoi21  g11(.a(new_n30_), .b(x10), .c(new_n39_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x00), .O(new_n41_));
  nand2  g13(.a(new_n34_), .b(new_n29_), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g15(.a(new_n40_), .b(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n42_), .c(new_n44_), .O(z2));
  nand2  g18(.a(new_n40_), .b(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n34_), .c(new_n29_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n48_), .c(new_n47_), .O(z3));
  nand2  g23(.a(new_n40_), .b(x03), .O(new_n52_));
  inv1   g24(.a(new_n42_), .O(new_n53_));
  nand3  g25(.a(x14), .b(x12), .c(x11), .O(new_n54_));
  inv1   g26(.a(new_n54_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x13), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n49_), .b(new_n57_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n52_), .O(z4));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n53_), .c(new_n31_), .O(new_n63_));
  nand2  g35(.a(new_n34_), .b(x15), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n29_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n63_), .O(z5));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n31_), .b(new_n30_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n55_), .b(x16), .c(x15), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n53_), .O(new_n72_));
  nand2  g44(.a(new_n34_), .b(x16), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n29_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n30_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x05), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n75_), .c(new_n72_), .O(z6));
  nand2  g49(.a(new_n40_), .b(x06), .O(new_n78_));
  nand3  g50(.a(x16), .b(x15), .c(x13), .O(new_n79_));
  inv1   g51(.a(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n55_), .c(x17), .O(new_n81_));
  inv1   g53(.a(x17), .O(new_n82_));
  oai21  g54(.a(new_n79_), .b(new_n54_), .c(new_n82_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n81_), .c(new_n53_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n78_), .O(z7));
  nand2  g57(.a(new_n40_), .b(x07), .O(new_n86_));
  nand4  g58(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n49_), .c(x18), .O(new_n88_));
  nor2   g60(.a(x18), .b(new_n82_), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(new_n80_), .c(new_n55_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n53_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(new_n86_), .O(z8));
endmodule


