// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_;
  and2   g00(.a(x12), .b(x11), .O(new_n29_));
  and2   g01(.a(x14), .b(x13), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(new_n29_), .c(x15), .O(new_n31_));
  inv1   g03(.a(x09), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(x08), .O(new_n33_));
  inv1   g05(.a(x16), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x10), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n33_), .c(x18), .d(x17), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x08), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x18), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  nand3  g14(.a(new_n33_), .b(x18), .c(new_n39_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(x11), .c(new_n42_), .O(z1));
  nor2   g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n33_), .b(new_n46_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n45_), .c(x18), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n39_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g23(.a(new_n41_), .b(x02), .O(new_n52_));
  nor2   g24(.a(new_n29_), .b(x13), .O(new_n53_));
  inv1   g25(.a(new_n43_), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n53_), .c(new_n52_), .O(z3));
  inv1   g29(.a(new_n55_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n33_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x18), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n39_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x03), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z4));
  inv1   g38(.a(x15), .O(new_n67_));
  nand4  g39(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n33_), .c(new_n31_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x18), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n39_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x04), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(z5));
  nand2  g46(.a(new_n41_), .b(x05), .O(new_n75_));
  nand2  g47(.a(new_n31_), .b(new_n34_), .O(new_n76_));
  nor2   g48(.a(new_n68_), .b(new_n67_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n76_), .c(new_n54_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n75_), .O(z6));
  nand2  g52(.a(new_n41_), .b(x06), .O(new_n81_));
  aoi21  g53(.a(new_n77_), .b(x16), .c(x17), .O(new_n82_));
  nand3  g54(.a(new_n77_), .b(x17), .c(x16), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n54_), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(z7));
  nand2  g57(.a(new_n41_), .b(x07), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n84_), .O(z8));
endmodule


