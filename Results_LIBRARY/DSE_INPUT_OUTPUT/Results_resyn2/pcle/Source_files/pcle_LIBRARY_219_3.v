// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  inv1   g05(.a(x02), .O(new_n34_));
  nand2  g06(.a(x18), .b(new_n34_), .O(new_n35_));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n35_), .c(new_n33_), .O(z0));
  nand2  g11(.a(x18), .b(x02), .O(new_n40_));
  inv1   g12(.a(new_n40_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  inv1   g15(.a(new_n38_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(z1));
  xnor2a g18(.a(x12), .b(x11), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n41_), .O(new_n48_));
  oai21  g20(.a(new_n47_), .b(new_n38_), .c(new_n48_), .O(z2));
  oai21  g21(.a(x18), .b(x08), .c(x02), .O(new_n50_));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n44_), .b(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand2  g26(.a(new_n30_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n29_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n44_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n54_), .c(new_n41_), .O(z4));
  aoi21  g31(.a(new_n30_), .b(x14), .c(x15), .O(new_n60_));
  and2   g32(.a(x15), .b(x14), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n30_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n41_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n60_), .c(new_n64_), .O(z5));
  nand3  g37(.a(new_n61_), .b(new_n30_), .c(x16), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n41_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  nand2  g43(.a(x08), .b(x06), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n66_), .b(new_n73_), .O(new_n74_));
  aoi21  g46(.a(new_n32_), .b(new_n30_), .c(new_n38_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n72_), .c(new_n41_), .O(z7));
  nand2  g49(.a(new_n35_), .b(new_n33_), .O(new_n78_));
  nand3  g50(.a(new_n32_), .b(new_n30_), .c(x18), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n44_), .O(new_n80_));
  nand3  g52(.a(new_n40_), .b(x08), .c(x07), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z8));
endmodule


