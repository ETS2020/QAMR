// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x18), .O(new_n29_));
  and2   g01(.a(x16), .b(x15), .O(new_n30_));
  nand3  g02(.a(x14), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n32_), .c(new_n30_), .d(x17), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x13), .c(new_n29_), .O(z0));
  nor2   g10(.a(new_n29_), .b(x13), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n35_), .b(x11), .c(new_n40_), .O(z1));
  inv1   g13(.a(new_n39_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(x08), .c(x01), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n42_), .c(new_n36_), .d(new_n44_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n43_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n36_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n39_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x13), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  oai21  g26(.a(new_n35_), .b(new_n54_), .c(new_n29_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand2  g29(.a(new_n49_), .b(new_n54_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n36_), .c(new_n31_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand3  g32(.a(new_n42_), .b(x08), .c(x04), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand4  g34(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n63_));
  nor2   g35(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g36(.a(new_n63_), .b(new_n62_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n42_), .c(new_n36_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n64_), .c(new_n61_), .O(z5));
  nand2  g39(.a(new_n32_), .b(new_n30_), .O(new_n68_));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n63_), .b(new_n62_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n36_), .c(new_n68_), .O(new_n71_));
  oai21  g43(.a(new_n35_), .b(new_n69_), .c(new_n29_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  aoi21  g48(.a(new_n32_), .b(new_n30_), .c(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n76_), .b(x16), .c(x15), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(new_n63_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n36_), .O(new_n80_));
  oai21  g52(.a(new_n35_), .b(new_n76_), .c(new_n29_), .O(new_n81_));
  and2   g53(.a(x08), .b(x06), .O(new_n82_));
  aoi21  g54(.a(new_n81_), .b(new_n53_), .c(new_n82_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n80_), .O(z7));
  nand3  g56(.a(x17), .b(x16), .c(x15), .O(new_n85_));
  nor2   g57(.a(new_n85_), .b(new_n31_), .O(new_n86_));
  oai21  g58(.a(x18), .b(new_n53_), .c(new_n86_), .O(new_n87_));
  oai21  g59(.a(new_n85_), .b(new_n31_), .c(new_n29_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n36_), .O(new_n89_));
  aoi21  g61(.a(x08), .b(x07), .c(new_n39_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n89_), .O(z8));
endmodule


