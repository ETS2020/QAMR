// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x10), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand3  g02(.a(x16), .b(x15), .c(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x18), .c(x17), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(new_n29_), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(x09), .c(x08), .O(z0));
  inv1   g08(.a(x08), .O(new_n37_));
  oai21  g09(.a(x11), .b(x10), .c(x09), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n39_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand3  g14(.a(new_n29_), .b(x09), .c(new_n37_), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  inv1   g19(.a(new_n43_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(z3));
  inv1   g22(.a(x09), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  aoi21  g25(.a(new_n30_), .b(new_n53_), .c(x10), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(x08), .c(new_n56_), .O(z4));
  inv1   g29(.a(x15), .O(new_n58_));
  and2   g30(.a(new_n52_), .b(new_n58_), .O(new_n59_));
  oai21  g31(.a(new_n52_), .b(new_n58_), .c(new_n29_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n59_), .c(x09), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n37_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x04), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z5));
  nand2  g36(.a(x08), .b(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nor3   g38(.a(new_n52_), .b(new_n66_), .c(new_n58_), .O(new_n67_));
  oai21  g39(.a(new_n52_), .b(new_n58_), .c(new_n66_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n48_), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(z6));
  inv1   g42(.a(new_n30_), .O(new_n71_));
  inv1   g43(.a(new_n31_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n71_), .c(x17), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  oai21  g46(.a(new_n31_), .b(new_n30_), .c(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n73_), .c(new_n29_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x09), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n37_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x06), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z7));
  nand2  g52(.a(x08), .b(x07), .O(new_n81_));
  aoi21  g53(.a(new_n32_), .b(x17), .c(x18), .O(new_n82_));
  nand2  g54(.a(new_n48_), .b(new_n33_), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n82_), .c(new_n81_), .O(z8));
endmodule


