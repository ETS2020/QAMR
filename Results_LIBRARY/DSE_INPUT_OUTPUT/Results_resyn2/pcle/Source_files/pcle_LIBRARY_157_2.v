// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x11), .O(new_n34_));
  nand4  g06(.a(x16), .b(x14), .c(x13), .d(x12), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g10(.a(x18), .b(new_n29_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  nand3  g12(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(new_n39_), .O(z2));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(new_n42_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n51_), .c(new_n39_), .O(z3));
  inv1   g28(.a(new_n54_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  oai21  g31(.a(new_n57_), .b(x14), .c(new_n42_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x03), .c(new_n39_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(z4));
  nor2   g34(.a(new_n39_), .b(new_n30_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x04), .O(new_n64_));
  nand4  g36(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(x18), .c(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n65_), .b(new_n29_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n66_), .c(new_n64_), .O(z5));
  nand2  g41(.a(new_n63_), .b(x05), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  nand2  g43(.a(new_n65_), .b(x18), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(x15), .c(new_n71_), .O(new_n73_));
  nand4  g45(.a(x18), .b(new_n71_), .c(x15), .d(x14), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n54_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n73_), .c(new_n42_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n70_), .O(z6));
  inv1   g49(.a(x18), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(x11), .O(new_n80_));
  nor2   g52(.a(new_n80_), .b(new_n35_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n42_), .c(new_n78_), .O(new_n82_));
  nand4  g54(.a(new_n57_), .b(x16), .c(x15), .d(x14), .O(new_n83_));
  and2   g55(.a(x08), .b(x06), .O(new_n84_));
  aoi21  g56(.a(new_n83_), .b(new_n33_), .c(new_n84_), .O(new_n85_));
  oai21  g57(.a(new_n82_), .b(new_n29_), .c(new_n85_), .O(z7));
  nand2  g58(.a(new_n63_), .b(x07), .O(new_n87_));
  nor2   g59(.a(new_n83_), .b(new_n79_), .O(new_n88_));
  nand2  g60(.a(new_n42_), .b(x18), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z8));
endmodule


