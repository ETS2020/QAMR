// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x17), .b(x16), .c(x15), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nor2   g09(.a(x18), .b(x13), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n31_), .b(x11), .c(new_n39_), .O(z1));
  inv1   g12(.a(x01), .O(new_n41_));
  inv1   g13(.a(new_n38_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x08), .O(new_n43_));
  inv1   g15(.a(new_n31_), .O(new_n44_));
  inv1   g16(.a(x11), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nor2   g18(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g19(.a(new_n47_), .O(new_n48_));
  nand2  g20(.a(new_n46_), .b(new_n45_), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n42_), .c(new_n48_), .d(new_n44_), .O(new_n50_));
  oai21  g22(.a(new_n43_), .b(new_n41_), .c(new_n50_), .O(z2));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n48_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n38_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z3));
  inv1   g29(.a(x03), .O(new_n58_));
  inv1   g30(.a(new_n52_), .O(new_n59_));
  nor2   g31(.a(new_n59_), .b(x14), .O(new_n60_));
  nand3  g32(.a(new_n42_), .b(new_n32_), .c(new_n44_), .O(new_n61_));
  oai22  g33(.a(new_n61_), .b(new_n60_), .c(new_n43_), .d(new_n58_), .O(z4));
  and2   g34(.a(x15), .b(x14), .O(new_n63_));
  oai21  g35(.a(new_n47_), .b(new_n53_), .c(new_n63_), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  aoi21  g37(.a(new_n32_), .b(new_n65_), .c(new_n31_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  oai21  g39(.a(new_n31_), .b(new_n65_), .c(x18), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x04), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n67_), .O(z5));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n63_), .b(new_n59_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g46(.a(x16), .b(x15), .O(new_n75_));
  nand3  g47(.a(x14), .b(x12), .c(x11), .O(new_n76_));
  or2    g48(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n74_), .c(new_n44_), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n72_), .c(x18), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n53_), .O(new_n80_));
  nand2  g52(.a(x08), .b(x05), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(z6));
  oai21  g54(.a(new_n76_), .b(new_n75_), .c(x17), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand4  g56(.a(new_n63_), .b(new_n59_), .c(new_n84_), .d(x16), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n44_), .O(new_n87_));
  oai21  g59(.a(new_n31_), .b(new_n84_), .c(x18), .O(new_n88_));
  and2   g60(.a(x08), .b(x06), .O(new_n89_));
  aoi21  g61(.a(new_n88_), .b(new_n53_), .c(new_n89_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n87_), .O(z7));
  inv1   g63(.a(x07), .O(new_n92_));
  aoi21  g64(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n93_));
  nand2  g65(.a(new_n36_), .b(new_n44_), .O(new_n94_));
  oai22  g66(.a(new_n94_), .b(new_n93_), .c(new_n43_), .d(new_n92_), .O(z8));
endmodule


