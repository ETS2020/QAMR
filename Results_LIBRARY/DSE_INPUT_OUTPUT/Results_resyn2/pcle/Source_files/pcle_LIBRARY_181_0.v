// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  and2   g02(.a(x16), .b(x15), .O(new_n31_));
  nand2  g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x18), .c(x17), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n32_), .O(z0));
  nor2   g10(.a(x18), .b(x17), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand2  g13(.a(new_n36_), .b(new_n41_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(x09), .O(new_n46_));
  nor2   g18(.a(new_n46_), .b(x08), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n47_), .c(new_n45_), .d(new_n34_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n44_), .c(new_n39_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n45_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(new_n54_), .c(new_n47_), .d(new_n34_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n52_), .c(new_n39_), .O(z3));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n55_), .b(new_n59_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n47_), .c(new_n29_), .d(new_n34_), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n58_), .c(new_n39_), .O(z4));
  nand2  g34(.a(new_n30_), .b(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand2  g36(.a(new_n29_), .b(new_n64_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n36_), .O(new_n66_));
  aoi21  g38(.a(x08), .b(x04), .c(new_n39_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(z5));
  inv1   g40(.a(x16), .O(new_n69_));
  oai21  g41(.a(new_n29_), .b(new_n64_), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n36_), .c(new_n32_), .O(new_n71_));
  aoi21  g43(.a(x08), .b(x05), .c(new_n39_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(z6));
  inv1   g45(.a(x17), .O(new_n74_));
  nand2  g46(.a(x18), .b(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n31_), .c(new_n30_), .O(new_n76_));
  nand2  g48(.a(x16), .b(x15), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n29_), .c(new_n74_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n76_), .c(new_n36_), .O(new_n79_));
  inv1   g51(.a(new_n39_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(x08), .c(x06), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(z7));
  nand2  g54(.a(x18), .b(x17), .O(new_n83_));
  oai21  g55(.a(new_n77_), .b(new_n29_), .c(new_n83_), .O(new_n84_));
  and2   g56(.a(x12), .b(x11), .O(new_n85_));
  and2   g57(.a(x14), .b(x13), .O(new_n86_));
  nand4  g58(.a(new_n31_), .b(new_n86_), .c(new_n85_), .d(x18), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n84_), .c(new_n36_), .O(new_n88_));
  nand2  g60(.a(new_n35_), .b(x18), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n74_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(z8));
endmodule


