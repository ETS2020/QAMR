// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(x18), .O(new_n34_));
  nand4  g06(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x16), .c(x15), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  inv1   g10(.a(x14), .O(new_n39_));
  nor2   g11(.a(x18), .b(new_n39_), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n32_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n33_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n43_), .c(new_n40_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n33_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n40_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(new_n49_), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(x14), .O(new_n54_));
  nand2  g26(.a(new_n35_), .b(new_n33_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x03), .c(new_n40_), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(z4));
  nor2   g29(.a(new_n36_), .b(x15), .O(new_n58_));
  nand3  g30(.a(new_n53_), .b(x15), .c(x14), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n33_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x04), .c(new_n40_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z5));
  inv1   g34(.a(new_n40_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(x08), .c(x05), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  nand2  g37(.a(new_n59_), .b(new_n65_), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(new_n63_), .c(new_n37_), .d(new_n33_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n64_), .O(z6));
  inv1   g40(.a(x18), .O(new_n69_));
  nand3  g41(.a(new_n29_), .b(x16), .c(x15), .O(new_n70_));
  nor2   g42(.a(new_n70_), .b(new_n32_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n53_), .c(new_n69_), .O(new_n72_));
  and2   g44(.a(x08), .b(x06), .O(new_n73_));
  nand4  g45(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n74_));
  inv1   g46(.a(new_n74_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n37_), .c(new_n73_), .O(new_n76_));
  oai21  g48(.a(new_n72_), .b(new_n39_), .c(new_n76_), .O(z7));
  nor2   g49(.a(new_n37_), .b(new_n29_), .O(new_n78_));
  nand3  g50(.a(new_n63_), .b(x08), .c(x07), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n34_), .c(new_n79_), .O(z8));
endmodule


