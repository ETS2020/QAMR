// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x03), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  nand2  g05(.a(x09), .b(new_n33_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n32_), .c(x18), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g09(.a(x10), .O(new_n38_));
  oai21  g10(.a(new_n34_), .b(x11), .c(new_n29_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(z1));
  nor2   g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  nand2  g16(.a(new_n35_), .b(new_n44_), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n43_), .c(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n38_), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  aoi21  g21(.a(new_n38_), .b(x03), .c(new_n33_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x02), .O(new_n51_));
  nand3  g23(.a(new_n35_), .b(new_n38_), .c(new_n29_), .O(new_n52_));
  xor2a  g24(.a(new_n44_), .b(x13), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(z3));
  oai21  g26(.a(new_n38_), .b(x08), .c(x03), .O(new_n55_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n30_), .b(new_n57_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n56_), .c(new_n35_), .d(new_n38_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n55_), .O(z4));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  inv1   g34(.a(new_n30_), .O(new_n63_));
  nor2   g35(.a(new_n61_), .b(new_n57_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n63_), .c(new_n34_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n62_), .c(x03), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n67_), .O(z5));
  nand2  g40(.a(new_n50_), .b(x05), .O(new_n69_));
  nor3   g41(.a(new_n30_), .b(new_n61_), .c(new_n57_), .O(new_n70_));
  nor2   g42(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g43(.a(new_n52_), .O(new_n72_));
  nand2  g44(.a(new_n70_), .b(x16), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(z6));
  nand2  g47(.a(new_n50_), .b(x06), .O(new_n76_));
  nand3  g48(.a(new_n70_), .b(x17), .c(x16), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  nand2  g50(.a(new_n73_), .b(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n72_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(z7));
  oai21  g53(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n82_));
  and2   g54(.a(x17), .b(x16), .O(new_n83_));
  inv1   g55(.a(new_n56_), .O(new_n84_));
  nor2   g56(.a(x18), .b(new_n61_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n82_), .c(new_n34_), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(x03), .c(new_n38_), .O(new_n88_));
  nand2  g60(.a(x08), .b(x07), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z8));
endmodule


