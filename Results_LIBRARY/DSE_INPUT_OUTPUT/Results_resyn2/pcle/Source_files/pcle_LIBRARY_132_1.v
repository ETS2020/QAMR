// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  nand2  g00(.a(x17), .b(x16), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(new_n30_), .b(x18), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(x13), .b(x12), .c(x11), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n31_), .O(z0));
  inv1   g11(.a(x18), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(x15), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n35_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n47_), .c(new_n35_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n41_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n47_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n36_), .c(new_n35_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n52_), .c(new_n41_), .O(z3));
  nor2   g28(.a(new_n37_), .b(x14), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n35_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n41_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  inv1   g35(.a(new_n58_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x15), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n35_), .O(new_n66_));
  aoi21  g38(.a(x08), .b(x04), .c(new_n41_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n66_), .O(z5));
  nand3  g40(.a(new_n64_), .b(x16), .c(x15), .O(new_n69_));
  inv1   g41(.a(x16), .O(new_n70_));
  oai21  g42(.a(new_n58_), .b(new_n62_), .c(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n35_), .O(new_n72_));
  aoi21  g44(.a(x08), .b(x05), .c(new_n41_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(z6));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x15), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n64_), .c(x16), .O(new_n77_));
  oai21  g49(.a(new_n58_), .b(new_n70_), .c(new_n75_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n35_), .O(new_n79_));
  nand4  g51(.a(x17), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n40_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n62_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x06), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n79_), .O(z7));
  nand2  g56(.a(new_n40_), .b(x15), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n64_), .c(new_n30_), .O(new_n86_));
  oai21  g58(.a(new_n58_), .b(new_n29_), .c(new_n40_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n35_), .O(new_n88_));
  aoi21  g60(.a(x08), .b(x07), .c(new_n41_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z8));
endmodule


