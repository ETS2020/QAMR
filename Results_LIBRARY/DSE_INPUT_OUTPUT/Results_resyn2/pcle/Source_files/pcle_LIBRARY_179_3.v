// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_;
  nand2  g00(.a(x18), .b(x09), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  inv1   g02(.a(x09), .O(new_n31_));
  inv1   g03(.a(x11), .O(new_n32_));
  nor2   g04(.a(x10), .b(x08), .O(new_n33_));
  aoi21  g05(.a(new_n33_), .b(new_n32_), .c(x18), .O(new_n34_));
  nand2  g06(.a(x08), .b(x00), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(new_n31_), .c(new_n35_), .O(z1));
  inv1   g08(.a(x01), .O(new_n37_));
  nand2  g09(.a(new_n29_), .b(x08), .O(new_n38_));
  inv1   g10(.a(x18), .O(new_n39_));
  nand3  g11(.a(new_n33_), .b(new_n39_), .c(x09), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(new_n40_), .c(new_n38_), .d(new_n37_), .O(z2));
  inv1   g14(.a(new_n38_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x02), .O(new_n44_));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  inv1   g17(.a(new_n40_), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(z3));
  nand4  g21(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n33_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n39_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x09), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z4));
  nand2  g29(.a(new_n43_), .b(x04), .O(new_n58_));
  inv1   g30(.a(new_n50_), .O(new_n59_));
  nor2   g31(.a(new_n59_), .b(x15), .O(new_n60_));
  nand2  g32(.a(new_n59_), .b(x15), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(z5));
  nand2  g35(.a(new_n43_), .b(x05), .O(new_n64_));
  aoi21  g36(.a(new_n59_), .b(x15), .c(x16), .O(new_n65_));
  nand3  g37(.a(new_n59_), .b(x16), .c(x15), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n46_), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(z6));
  nand2  g40(.a(new_n43_), .b(x06), .O(new_n69_));
  nand4  g41(.a(new_n59_), .b(x17), .c(x16), .d(x15), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  nand2  g43(.a(x16), .b(x15), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n50_), .c(new_n71_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n70_), .c(new_n46_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n69_), .O(z7));
  nand2  g47(.a(new_n43_), .b(x07), .O(new_n76_));
  oai21  g48(.a(new_n70_), .b(new_n40_), .c(new_n76_), .O(z8));
endmodule


