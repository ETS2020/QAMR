// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_;
  nand2  g00(.a(x08), .b(x00), .O(new_n30_));
  inv1   g01(.a(x08), .O(new_n31_));
  inv1   g02(.a(x18), .O(new_n32_));
  inv1   g03(.a(x09), .O(new_n33_));
  nor2   g04(.a(x10), .b(new_n33_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n32_), .c(new_n31_), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(x11), .c(new_n30_), .O(z1));
  nand2  g07(.a(x08), .b(x01), .O(new_n37_));
  xnor2a g08(.a(x12), .b(x11), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n35_), .c(new_n37_), .O(z2));
  nand2  g10(.a(x08), .b(x02), .O(new_n40_));
  aoi21  g11(.a(x12), .b(x11), .c(x13), .O(new_n41_));
  inv1   g12(.a(new_n35_), .O(new_n42_));
  nand3  g13(.a(x13), .b(x12), .c(x11), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n41_), .c(new_n40_), .O(z3));
  nand2  g16(.a(x08), .b(x03), .O(new_n46_));
  nand4  g17(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n47_));
  inv1   g18(.a(x14), .O(new_n48_));
  nand2  g19(.a(new_n43_), .b(new_n48_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n47_), .c(new_n42_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n46_), .O(z4));
  inv1   g22(.a(new_n47_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x15), .O(new_n53_));
  inv1   g24(.a(x15), .O(new_n54_));
  nand2  g25(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  and2   g26(.a(new_n55_), .b(new_n34_), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(new_n53_), .c(x18), .O(new_n57_));
  nand2  g28(.a(x08), .b(x04), .O(new_n58_));
  oai21  g29(.a(new_n57_), .b(x08), .c(new_n58_), .O(z5));
  inv1   g30(.a(x16), .O(new_n60_));
  nand2  g31(.a(x15), .b(x14), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n43_), .c(new_n60_), .O(new_n62_));
  oai21  g33(.a(new_n61_), .b(new_n43_), .c(new_n60_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n34_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n62_), .c(new_n32_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n31_), .O(new_n66_));
  nand2  g37(.a(x08), .b(x05), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n66_), .O(z6));
  xnor2a g39(.a(x17), .b(x16), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n52_), .c(x15), .O(new_n70_));
  inv1   g41(.a(x17), .O(new_n71_));
  oai21  g42(.a(new_n61_), .b(new_n43_), .c(new_n71_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n70_), .c(new_n34_), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n32_), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n31_), .O(new_n75_));
  nand2  g46(.a(x08), .b(x06), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n75_), .O(z7));
  nand2  g48(.a(x08), .b(x07), .O(new_n78_));
  nand3  g49(.a(new_n42_), .b(x17), .c(x16), .O(new_n79_));
  oai21  g50(.a(new_n79_), .b(new_n53_), .c(new_n78_), .O(z8));
  zero   g51(.O(z0));
endmodule


