// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  and2   g06(.a(x15), .b(x05), .O(new_n35_));
  nand3  g07(.a(x18), .b(x17), .c(x16), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n35_), .c(new_n34_), .d(new_n30_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(z0));
  inv1   g11(.a(x18), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(x05), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n34_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n34_), .c(new_n47_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n41_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand2  g26(.a(new_n47_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n34_), .c(new_n53_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n52_), .c(new_n41_), .O(z3));
  inv1   g29(.a(new_n53_), .O(new_n58_));
  nor2   g30(.a(new_n58_), .b(x14), .O(new_n59_));
  nand2  g31(.a(new_n34_), .b(new_n29_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x03), .c(new_n41_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n59_), .c(new_n61_), .O(z4));
  nor2   g34(.a(new_n30_), .b(x15), .O(new_n63_));
  nand2  g35(.a(x15), .b(x14), .O(new_n64_));
  inv1   g36(.a(new_n64_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n58_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n34_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n41_), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(new_n63_), .c(new_n68_), .O(z5));
  nor2   g41(.a(new_n64_), .b(new_n53_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  oai21  g44(.a(new_n64_), .b(new_n53_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n34_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x05), .c(new_n41_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z6));
  aoi21  g48(.a(new_n70_), .b(x16), .c(x17), .O(new_n77_));
  and2   g49(.a(x17), .b(x16), .O(new_n78_));
  nand3  g50(.a(new_n65_), .b(new_n78_), .c(new_n58_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n34_), .O(new_n80_));
  aoi21  g52(.a(x08), .b(x06), .c(new_n41_), .O(new_n81_));
  oai21  g53(.a(new_n80_), .b(new_n77_), .c(new_n81_), .O(z7));
  nand4  g54(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n58_), .c(x18), .O(new_n85_));
  oai21  g57(.a(new_n83_), .b(new_n53_), .c(new_n40_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n34_), .O(new_n87_));
  aoi21  g59(.a(x08), .b(x07), .c(new_n41_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(z8));
endmodule


