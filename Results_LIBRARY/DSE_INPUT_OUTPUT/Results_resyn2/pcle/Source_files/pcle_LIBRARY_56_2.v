// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x06), .O(new_n29_));
  nor2   g01(.a(x09), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(x16), .b(x15), .c(x14), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n36_), .c(x18), .d(x17), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n34_), .c(new_n31_), .O(z0));
  aoi21  g12(.a(x08), .b(x00), .c(new_n30_), .O(new_n41_));
  oai21  g13(.a(new_n34_), .b(x11), .c(new_n41_), .O(z1));
  inv1   g14(.a(new_n34_), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n35_), .b(new_n43_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n30_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  nand2  g25(.a(new_n31_), .b(x08), .O(new_n54_));
  nand2  g26(.a(new_n36_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n35_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n43_), .O(new_n58_));
  oai21  g30(.a(new_n54_), .b(new_n53_), .c(new_n58_), .O(z4));
  inv1   g31(.a(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  oai21  g33(.a(new_n35_), .b(new_n56_), .c(new_n61_), .O(new_n62_));
  nand2  g34(.a(x15), .b(x14), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n62_), .c(new_n43_), .O(new_n66_));
  oai21  g38(.a(new_n54_), .b(new_n60_), .c(new_n66_), .O(z5));
  inv1   g39(.a(x16), .O(new_n68_));
  nor2   g40(.a(new_n65_), .b(new_n68_), .O(new_n69_));
  oai21  g41(.a(new_n63_), .b(new_n35_), .c(new_n68_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n43_), .O(new_n71_));
  aoi21  g43(.a(x08), .b(x05), .c(new_n30_), .O(new_n72_));
  oai21  g44(.a(new_n71_), .b(new_n69_), .c(new_n72_), .O(z6));
  inv1   g45(.a(x09), .O(new_n74_));
  nand2  g46(.a(x08), .b(x06), .O(new_n75_));
  nand3  g47(.a(new_n38_), .b(new_n36_), .c(x17), .O(new_n76_));
  inv1   g48(.a(x17), .O(new_n77_));
  oai21  g49(.a(new_n37_), .b(new_n35_), .c(new_n77_), .O(new_n78_));
  nor2   g50(.a(x10), .b(x08), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  aoi21  g52(.a(new_n80_), .b(new_n75_), .c(new_n74_), .O(z7));
  nor2   g53(.a(new_n37_), .b(new_n35_), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(x17), .c(x18), .O(new_n83_));
  nand2  g55(.a(new_n39_), .b(new_n43_), .O(new_n84_));
  aoi21  g56(.a(x08), .b(x07), .c(new_n30_), .O(new_n85_));
  oai21  g57(.a(new_n84_), .b(new_n83_), .c(new_n85_), .O(z8));
endmodule


