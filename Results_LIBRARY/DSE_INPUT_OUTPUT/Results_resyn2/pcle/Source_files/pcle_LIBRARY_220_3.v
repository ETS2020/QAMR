// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x06), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand3  g03(.a(x17), .b(x16), .c(x15), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n33_), .c(x18), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g11(.a(new_n30_), .b(new_n29_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand2  g14(.a(new_n37_), .b(new_n42_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n37_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(new_n40_), .O(z2));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n37_), .b(new_n31_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n40_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(z3));
  nor2   g26(.a(new_n40_), .b(new_n34_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(x03), .O(new_n56_));
  aoi21  g28(.a(new_n31_), .b(new_n29_), .c(new_n30_), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(x14), .c(new_n37_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand2  g33(.a(new_n55_), .b(x04), .O(new_n62_));
  nand2  g34(.a(new_n57_), .b(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  inv1   g36(.a(new_n31_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x14), .c(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n63_), .c(new_n37_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n62_), .O(z5));
  nand3  g41(.a(new_n65_), .b(x15), .c(x14), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n37_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(x15), .c(x11), .d(new_n35_), .O(new_n73_));
  nand4  g45(.a(x13), .b(x12), .c(x09), .d(new_n34_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n73_), .c(new_n29_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x14), .O(new_n76_));
  nand2  g48(.a(x08), .b(x05), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z6));
  aoi22  g50(.a(new_n37_), .b(x17), .c(x08), .d(x06), .O(new_n79_));
  and2   g51(.a(x16), .b(x15), .O(new_n80_));
  inv1   g52(.a(x17), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x14), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n59_), .c(new_n80_), .O(new_n83_));
  nand2  g55(.a(new_n59_), .b(new_n80_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n83_), .c(new_n37_), .d(new_n29_), .O(new_n86_));
  oai21  g58(.a(new_n79_), .b(x14), .c(new_n86_), .O(z7));
  oai21  g59(.a(new_n32_), .b(new_n31_), .c(x18), .O(new_n88_));
  inv1   g60(.a(x18), .O(new_n89_));
  and2   g61(.a(x17), .b(x14), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n59_), .c(new_n80_), .d(new_n89_), .O(new_n91_));
  aoi21  g63(.a(new_n91_), .b(new_n88_), .c(x06), .O(new_n92_));
  nor2   g64(.a(new_n89_), .b(x14), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n92_), .c(new_n37_), .O(new_n94_));
  nand2  g66(.a(new_n55_), .b(x07), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(new_n94_), .O(z8));
endmodule


