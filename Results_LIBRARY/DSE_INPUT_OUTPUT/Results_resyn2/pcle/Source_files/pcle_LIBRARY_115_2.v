// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x18), .O(new_n29_));
  and2   g01(.a(x15), .b(x14), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand2  g03(.a(x17), .b(x16), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n34_), .c(new_n29_), .O(z0));
  inv1   g10(.a(x04), .O(new_n39_));
  nor2   g11(.a(x18), .b(new_n39_), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n37_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  inv1   g16(.a(new_n37_), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n43_), .c(new_n40_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n45_), .b(new_n31_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n40_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  inv1   g24(.a(new_n40_), .O(new_n53_));
  inv1   g25(.a(new_n31_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n31_), .b(new_n56_), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(new_n55_), .c(new_n53_), .d(new_n45_), .O(new_n58_));
  nand3  g30(.a(new_n53_), .b(x08), .c(x03), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n58_), .O(z4));
  nand3  g32(.a(x18), .b(x08), .c(x04), .O(new_n61_));
  aoi21  g33(.a(new_n54_), .b(x14), .c(x15), .O(new_n62_));
  nand2  g34(.a(new_n54_), .b(new_n30_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n53_), .c(new_n45_), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(z5));
  aoi21  g37(.a(new_n54_), .b(new_n30_), .c(x16), .O(new_n66_));
  nand3  g38(.a(new_n54_), .b(new_n30_), .c(x16), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n45_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n40_), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(new_n66_), .c(new_n69_), .O(z6));
  aoi21  g42(.a(new_n33_), .b(new_n30_), .c(new_n37_), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n67_), .b(new_n72_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x06), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n74_), .c(new_n40_), .O(z7));
  nand2  g48(.a(x08), .b(x07), .O(new_n77_));
  inv1   g49(.a(new_n77_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n71_), .c(x18), .O(new_n79_));
  inv1   g51(.a(x09), .O(new_n80_));
  nor2   g52(.a(new_n80_), .b(x08), .O(new_n81_));
  nor2   g53(.a(x18), .b(x10), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(new_n81_), .c(new_n33_), .d(new_n30_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n79_), .O(z8));
endmodule


