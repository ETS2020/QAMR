// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand2  g00(.a(x16), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  inv1   g06(.a(x09), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x08), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n34_), .c(x07), .O(new_n37_));
  nand2  g09(.a(x18), .b(x17), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n37_), .c(new_n33_), .O(z0));
  inv1   g11(.a(x07), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  aoi21  g13(.a(new_n36_), .b(new_n41_), .c(new_n40_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(x10), .c(new_n43_), .O(z1));
  nor2   g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n36_), .b(new_n46_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n45_), .c(x07), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n34_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n52_), .c(x07), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n34_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x02), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z3));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n36_), .b(new_n31_), .O(new_n60_));
  aoi21  g32(.a(new_n53_), .b(new_n59_), .c(new_n60_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n40_), .c(new_n34_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x03), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z4));
  inv1   g36(.a(x04), .O(new_n65_));
  oai21  g37(.a(x10), .b(x07), .c(x08), .O(new_n66_));
  nor2   g38(.a(new_n32_), .b(x15), .O(new_n67_));
  inv1   g39(.a(new_n37_), .O(new_n68_));
  nand2  g40(.a(new_n32_), .b(x15), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(z5));
  inv1   g43(.a(x05), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(new_n69_), .b(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n68_), .c(new_n33_), .O(new_n75_));
  oai21  g47(.a(new_n66_), .b(new_n72_), .c(new_n75_), .O(z6));
  nand3  g48(.a(new_n32_), .b(new_n30_), .c(x17), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  oai21  g50(.a(new_n31_), .b(new_n29_), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n77_), .c(new_n36_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x07), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n34_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x06), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z7));
  nand4  g56(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n53_), .c(x18), .O(new_n86_));
  nor2   g58(.a(x18), .b(new_n78_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n32_), .c(new_n30_), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(new_n86_), .c(new_n35_), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n40_), .c(new_n34_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z8));
endmodule


