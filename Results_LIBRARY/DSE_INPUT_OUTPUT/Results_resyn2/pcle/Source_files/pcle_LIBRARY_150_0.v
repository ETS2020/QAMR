// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  nand3  g01(.a(x17), .b(x16), .c(x15), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(x10), .b(new_n33_), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n32_), .c(x07), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n31_), .c(x18), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n29_), .O(z0));
  inv1   g10(.a(x07), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nor2   g12(.a(new_n33_), .b(x08), .O(new_n41_));
  aoi21  g13(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(x10), .c(new_n43_), .O(z1));
  inv1   g16(.a(x10), .O(new_n45_));
  nor2   g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  nand2  g19(.a(new_n41_), .b(new_n47_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n46_), .c(x07), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand2  g22(.a(x08), .b(x01), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z2));
  inv1   g24(.a(x02), .O(new_n53_));
  oai21  g25(.a(x10), .b(x07), .c(x08), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  inv1   g27(.a(x13), .O(new_n56_));
  nand2  g28(.a(new_n47_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n36_), .c(new_n55_), .O(new_n58_));
  oai21  g30(.a(new_n54_), .b(new_n53_), .c(new_n58_), .O(z3));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n41_), .c(new_n29_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x07), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n45_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x03), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z4));
  inv1   g38(.a(x04), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand2  g40(.a(new_n29_), .b(new_n68_), .O(new_n69_));
  inv1   g41(.a(new_n29_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x15), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n69_), .c(new_n36_), .O(new_n72_));
  oai21  g44(.a(new_n54_), .b(new_n67_), .c(new_n72_), .O(z5));
  inv1   g45(.a(x05), .O(new_n74_));
  inv1   g46(.a(x16), .O(new_n75_));
  nand2  g47(.a(new_n71_), .b(new_n75_), .O(new_n76_));
  nor2   g48(.a(new_n75_), .b(new_n68_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n76_), .c(new_n36_), .O(new_n79_));
  oai21  g51(.a(new_n54_), .b(new_n74_), .c(new_n79_), .O(z6));
  inv1   g52(.a(x06), .O(new_n81_));
  nand3  g53(.a(new_n77_), .b(new_n70_), .c(x17), .O(new_n82_));
  inv1   g54(.a(x17), .O(new_n83_));
  nand2  g55(.a(new_n78_), .b(new_n83_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n82_), .c(new_n36_), .O(new_n85_));
  oai21  g57(.a(new_n54_), .b(new_n81_), .c(new_n85_), .O(z7));
  nand4  g58(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n55_), .c(x18), .O(new_n88_));
  inv1   g60(.a(x18), .O(new_n89_));
  nand3  g61(.a(new_n31_), .b(new_n70_), .c(new_n89_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n34_), .O(new_n92_));
  aoi21  g64(.a(new_n92_), .b(new_n32_), .c(new_n39_), .O(z8));
endmodule


