// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x11), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand3  g04(.a(x14), .b(x13), .c(x12), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x17), .b(x16), .c(x15), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n34_), .c(x18), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g10(.a(new_n32_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(x18), .c(new_n29_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(x18), .O(new_n44_));
  nor2   g16(.a(new_n44_), .b(x11), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n45_), .O(new_n46_));
  oai21  g18(.a(new_n43_), .b(new_n32_), .c(new_n46_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n45_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x14), .O(new_n53_));
  oai21  g25(.a(new_n32_), .b(new_n53_), .c(new_n44_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n29_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand2  g28(.a(new_n49_), .b(new_n53_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n33_), .c(new_n39_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x14), .O(new_n61_));
  oai22  g33(.a(new_n61_), .b(new_n49_), .c(new_n34_), .d(new_n60_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n39_), .O(new_n63_));
  oai21  g35(.a(new_n32_), .b(new_n60_), .c(new_n44_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n29_), .O(new_n65_));
  nand2  g37(.a(x08), .b(x04), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n63_), .O(z5));
  inv1   g39(.a(x16), .O(new_n68_));
  oai21  g40(.a(new_n32_), .b(new_n68_), .c(new_n44_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n29_), .O(new_n70_));
  nand2  g42(.a(x15), .b(x11), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n33_), .c(new_n68_), .O(new_n72_));
  nand3  g44(.a(new_n34_), .b(x16), .c(x15), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n39_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x05), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n70_), .O(z6));
  nand2  g48(.a(x16), .b(x15), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n33_), .c(x17), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(x16), .c(x15), .d(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n49_), .c(new_n78_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  oai21  g54(.a(new_n32_), .b(new_n79_), .c(new_n44_), .O(new_n83_));
  and2   g55(.a(x08), .b(x06), .O(new_n84_));
  aoi21  g56(.a(new_n83_), .b(new_n29_), .c(new_n84_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n82_), .O(z7));
  oai21  g58(.a(new_n35_), .b(new_n33_), .c(new_n44_), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(new_n37_), .c(new_n39_), .d(x11), .O(new_n88_));
  nand2  g60(.a(x08), .b(x07), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n45_), .c(new_n88_), .O(z8));
endmodule


