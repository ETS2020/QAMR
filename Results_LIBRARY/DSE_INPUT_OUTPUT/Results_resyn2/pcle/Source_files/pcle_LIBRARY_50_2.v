// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x00), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x15), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n32_), .c(x18), .O(new_n37_));
  oai21  g09(.a(x01), .b(new_n29_), .c(new_n37_), .O(z0));
  inv1   g10(.a(x01), .O(new_n39_));
  oai21  g11(.a(x08), .b(new_n39_), .c(x00), .O(new_n40_));
  oai21  g12(.a(new_n35_), .b(x11), .c(new_n40_), .O(z1));
  aoi21  g13(.a(new_n33_), .b(new_n29_), .c(x01), .O(new_n42_));
  and2   g14(.a(x12), .b(x11), .O(new_n43_));
  inv1   g15(.a(new_n43_), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n44_), .c(new_n34_), .d(x09), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n33_), .c(new_n42_), .O(z2));
  nand2  g19(.a(new_n43_), .b(x13), .O(new_n48_));
  inv1   g20(.a(x13), .O(new_n49_));
  nand2  g21(.a(new_n44_), .b(new_n49_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n48_), .c(new_n36_), .O(new_n51_));
  nor2   g23(.a(x01), .b(new_n29_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n52_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n36_), .c(new_n30_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n52_), .O(z4));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n30_), .b(new_n61_), .O(new_n62_));
  and2   g34(.a(x14), .b(x13), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n43_), .c(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n62_), .c(new_n36_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n60_), .c(new_n52_), .O(z5));
  nand4  g38(.a(new_n63_), .b(new_n43_), .c(x16), .d(x15), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  nand2  g40(.a(new_n64_), .b(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n36_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n52_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  nand2  g44(.a(x08), .b(x06), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nand2  g46(.a(new_n67_), .b(new_n74_), .O(new_n75_));
  nor2   g47(.a(new_n35_), .b(new_n32_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n73_), .c(new_n52_), .O(z7));
  nand2  g50(.a(x08), .b(x07), .O(new_n79_));
  nand2  g51(.a(new_n32_), .b(x18), .O(new_n80_));
  inv1   g52(.a(x18), .O(new_n81_));
  oai21  g53(.a(new_n31_), .b(new_n30_), .c(new_n81_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n80_), .c(new_n36_), .O(new_n83_));
  aoi21  g55(.a(new_n83_), .b(new_n79_), .c(new_n52_), .O(z8));
endmodule


