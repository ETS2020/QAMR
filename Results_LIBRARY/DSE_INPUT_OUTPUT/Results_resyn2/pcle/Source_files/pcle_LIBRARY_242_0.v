// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(x08), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x18), .c(new_n29_), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  and2   g06(.a(x15), .b(x14), .O(new_n35_));
  and2   g07(.a(x17), .b(x16), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n32_), .O(z0));
  inv1   g10(.a(x00), .O(new_n39_));
  oai21  g11(.a(x18), .b(x10), .c(x08), .O(new_n40_));
  oai22  g12(.a(new_n40_), .b(new_n39_), .c(new_n32_), .d(x11), .O(z1));
  nor2   g13(.a(x12), .b(x11), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n31_), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n42_), .c(x18), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n29_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n33_), .b(new_n31_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n49_), .c(x18), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n29_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n33_), .b(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n34_), .b(x14), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n31_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x18), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  nand2  g32(.a(x08), .b(x03), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n60_), .O(z4));
  inv1   g34(.a(x04), .O(new_n63_));
  inv1   g35(.a(new_n32_), .O(new_n64_));
  nand2  g36(.a(new_n35_), .b(new_n34_), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  nand2  g38(.a(new_n57_), .b(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(new_n68_));
  oai21  g40(.a(new_n40_), .b(new_n63_), .c(new_n68_), .O(z5));
  inv1   g41(.a(x05), .O(new_n70_));
  nand3  g42(.a(new_n35_), .b(new_n34_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n71_), .c(new_n64_), .O(new_n74_));
  oai21  g46(.a(new_n40_), .b(new_n70_), .c(new_n74_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nand3  g48(.a(x16), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n33_), .c(new_n76_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n37_), .c(new_n31_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(x18), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n29_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x06), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z7));
  inv1   g55(.a(x18), .O(new_n84_));
  aoi21  g56(.a(new_n37_), .b(new_n31_), .c(new_n84_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  oai21  g58(.a(new_n85_), .b(x10), .c(new_n86_), .O(z8));
endmodule


