// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x17), .b(x16), .c(x15), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nor2   g09(.a(x14), .b(x09), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n29_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x00), .O(new_n40_));
  oai21  g12(.a(new_n31_), .b(x11), .c(new_n40_), .O(z1));
  xnor2a g13(.a(x12), .b(x11), .O(new_n42_));
  aoi21  g14(.a(x08), .b(x01), .c(new_n38_), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(new_n31_), .c(new_n43_), .O(z2));
  aoi21  g16(.a(x12), .b(x11), .c(x13), .O(new_n45_));
  inv1   g17(.a(new_n31_), .O(new_n46_));
  nand3  g18(.a(x13), .b(x12), .c(x11), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g20(.a(x08), .b(x02), .c(new_n38_), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(new_n45_), .c(new_n49_), .O(z3));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n30_), .b(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n47_), .c(x09), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g26(.a(new_n47_), .b(new_n46_), .c(x14), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z4));
  nand2  g29(.a(new_n39_), .b(x04), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n32_), .b(new_n59_), .O(new_n60_));
  oai21  g32(.a(new_n33_), .b(x15), .c(new_n46_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(z5));
  nand2  g34(.a(new_n39_), .b(x05), .O(new_n63_));
  nand2  g35(.a(new_n60_), .b(x16), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  oai21  g37(.a(new_n32_), .b(new_n59_), .c(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n46_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(z6));
  inv1   g40(.a(x17), .O(new_n69_));
  inv1   g41(.a(new_n47_), .O(new_n70_));
  and2   g42(.a(x16), .b(x15), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(new_n72_));
  nand3  g44(.a(new_n69_), .b(x16), .c(x15), .O(new_n73_));
  nor2   g45(.a(new_n73_), .b(new_n32_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n72_), .c(new_n46_), .O(new_n75_));
  oai21  g47(.a(new_n52_), .b(new_n69_), .c(x09), .O(new_n76_));
  and2   g48(.a(x08), .b(x06), .O(new_n77_));
  aoi21  g49(.a(new_n76_), .b(new_n51_), .c(new_n77_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n75_), .O(z7));
  nand2  g51(.a(new_n39_), .b(x07), .O(new_n80_));
  aoi21  g52(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n81_));
  nand2  g53(.a(new_n36_), .b(new_n46_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(z8));
endmodule


