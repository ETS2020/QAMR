// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x06), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(x08), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nor3   g06(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n31_), .c(x18), .d(x17), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  aoi21  g10(.a(new_n31_), .b(new_n38_), .c(x06), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  oai21  g12(.a(new_n39_), .b(x10), .c(new_n40_), .O(z1));
  inv1   g13(.a(x01), .O(new_n42_));
  oai21  g14(.a(x10), .b(new_n29_), .c(x08), .O(new_n43_));
  inv1   g15(.a(x10), .O(new_n44_));
  nand3  g16(.a(new_n31_), .b(new_n44_), .c(new_n29_), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(new_n45_), .c(new_n43_), .d(new_n42_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  inv1   g20(.a(new_n33_), .O(new_n49_));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  or2    g22(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai22  g23(.a(new_n51_), .b(new_n45_), .c(new_n43_), .d(new_n48_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  inv1   g25(.a(new_n45_), .O(new_n54_));
  nand2  g26(.a(new_n49_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n33_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(new_n58_));
  oai21  g30(.a(new_n43_), .b(new_n53_), .c(new_n58_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  inv1   g33(.a(x08), .O(new_n62_));
  nand2  g34(.a(x09), .b(new_n62_), .O(new_n63_));
  inv1   g35(.a(new_n34_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n49_), .c(new_n63_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n61_), .c(x06), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n67_), .O(z5));
  nand3  g40(.a(new_n64_), .b(new_n49_), .c(x16), .O(new_n69_));
  oai21  g41(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n31_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n29_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n44_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  nand3  g47(.a(x10), .b(x08), .c(x06), .O(new_n76_));
  nor2   g48(.a(new_n35_), .b(x17), .O(new_n77_));
  nand4  g49(.a(new_n64_), .b(new_n49_), .c(x17), .d(x16), .O(new_n78_));
  nand2  g50(.a(new_n54_), .b(new_n78_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(z7));
  nand3  g52(.a(new_n35_), .b(x18), .c(x17), .O(new_n81_));
  inv1   g53(.a(x18), .O(new_n82_));
  nand2  g54(.a(new_n78_), .b(new_n82_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n81_), .c(new_n31_), .d(new_n44_), .O(new_n84_));
  aoi22  g56(.a(new_n44_), .b(x06), .c(x08), .d(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z8));
endmodule


