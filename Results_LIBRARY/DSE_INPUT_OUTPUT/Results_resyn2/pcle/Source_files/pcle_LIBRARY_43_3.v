// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x08), .O(new_n32_));
  nor2   g04(.a(z0), .b(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(x00), .O(new_n34_));
  nor2   g06(.a(x10), .b(x08), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n30_), .c(x09), .O(new_n36_));
  oai21  g08(.a(new_n36_), .b(x11), .c(new_n34_), .O(z1));
  nand2  g09(.a(new_n33_), .b(x01), .O(new_n38_));
  xnor2a g10(.a(x12), .b(x11), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n36_), .c(new_n38_), .O(z2));
  aoi21  g12(.a(x12), .b(x11), .c(x13), .O(new_n41_));
  nand3  g13(.a(x13), .b(x12), .c(x11), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n41_), .c(new_n30_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x09), .O(new_n45_));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z3));
  nand4  g19(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n48_));
  inv1   g20(.a(x14), .O(new_n49_));
  nand2  g21(.a(new_n42_), .b(new_n49_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n48_), .c(new_n35_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x09), .O(new_n53_));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z4));
  inv1   g27(.a(x15), .O(new_n56_));
  and2   g28(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  oai21  g29(.a(new_n48_), .b(new_n56_), .c(new_n35_), .O(new_n58_));
  oai21  g30(.a(new_n58_), .b(new_n57_), .c(new_n30_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x09), .O(new_n60_));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n60_), .O(z5));
  inv1   g34(.a(x16), .O(new_n63_));
  oai21  g35(.a(new_n48_), .b(new_n56_), .c(new_n63_), .O(new_n64_));
  inv1   g36(.a(x10), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n32_), .O(new_n66_));
  inv1   g38(.a(new_n48_), .O(new_n67_));
  nor2   g39(.a(new_n63_), .b(new_n56_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n64_), .c(x18), .O(new_n70_));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(new_n29_), .c(new_n71_), .O(z6));
  nand2  g44(.a(new_n33_), .b(x06), .O(new_n73_));
  nand3  g45(.a(new_n68_), .b(new_n67_), .c(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n68_), .b(new_n67_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n75_), .c(new_n36_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n73_), .O(z7));
  nand2  g51(.a(new_n33_), .b(x07), .O(new_n80_));
  oai21  g52(.a(new_n74_), .b(new_n36_), .c(new_n80_), .O(z8));
endmodule


