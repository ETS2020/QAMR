// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_;
  nand3  g00(.a(x14), .b(x13), .c(x12), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  and2   g02(.a(x15), .b(x11), .O(new_n31_));
  and2   g03(.a(x17), .b(x16), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(x18), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n33_), .O(z0));
  nor2   g11(.a(x18), .b(x11), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n34_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  oai21  g14(.a(new_n38_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(new_n41_), .b(x01), .O(new_n44_));
  nor2   g16(.a(new_n40_), .b(new_n36_), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n44_), .O(z2));
  inv1   g23(.a(new_n45_), .O(new_n52_));
  nand2  g24(.a(new_n41_), .b(x02), .O(new_n53_));
  xor2a  g25(.a(new_n46_), .b(x13), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n52_), .c(new_n53_), .O(z3));
  inv1   g27(.a(x14), .O(new_n56_));
  oai21  g28(.a(new_n36_), .b(new_n56_), .c(x18), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  nand2  g30(.a(x08), .b(x03), .O(new_n59_));
  inv1   g31(.a(x13), .O(new_n60_));
  oai21  g32(.a(new_n46_), .b(new_n60_), .c(new_n56_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n37_), .c(new_n29_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z4));
  inv1   g35(.a(x15), .O(new_n64_));
  oai21  g36(.a(new_n36_), .b(new_n64_), .c(x18), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nand2  g38(.a(x14), .b(x13), .O(new_n67_));
  nand3  g39(.a(new_n64_), .b(x12), .c(x11), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(new_n67_), .c(new_n30_), .d(new_n64_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n37_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x04), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n70_), .c(new_n66_), .O(z5));
  nand2  g44(.a(new_n41_), .b(x05), .O(new_n73_));
  aoi21  g45(.a(new_n31_), .b(new_n30_), .c(x16), .O(new_n74_));
  nand4  g46(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x12), .c(x11), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n45_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(z6));
  inv1   g51(.a(x17), .O(new_n80_));
  nand2  g52(.a(new_n77_), .b(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n76_), .b(x17), .c(x12), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n37_), .O(new_n83_));
  oai21  g55(.a(new_n36_), .b(new_n80_), .c(x18), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n47_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x06), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n83_), .O(z7));
  nand2  g59(.a(new_n41_), .b(x07), .O(new_n88_));
  nor2   g60(.a(new_n75_), .b(new_n46_), .O(new_n89_));
  nor2   g61(.a(x18), .b(new_n80_), .O(new_n90_));
  aoi22  g62(.a(new_n90_), .b(new_n89_), .c(new_n33_), .d(x18), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n36_), .c(new_n88_), .O(z8));
endmodule


