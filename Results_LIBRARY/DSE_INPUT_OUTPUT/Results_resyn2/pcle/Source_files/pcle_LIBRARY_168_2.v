// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  and2   g03(.a(x17), .b(x16), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nand2  g06(.a(x15), .b(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n33_), .c(new_n29_), .O(z0));
  nand3  g10(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n39_));
  inv1   g11(.a(x02), .O(new_n40_));
  nor2   g12(.a(x18), .b(new_n40_), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n39_), .b(x11), .c(new_n42_), .O(z1));
  inv1   g15(.a(new_n39_), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n41_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  oai21  g21(.a(new_n29_), .b(x08), .c(x02), .O(new_n50_));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n34_), .b(new_n44_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n34_), .b(new_n55_), .O(new_n56_));
  inv1   g28(.a(new_n34_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n44_), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n54_), .c(new_n41_), .O(z4));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(new_n62_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n37_), .c(new_n44_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n61_), .c(new_n41_), .O(z5));
  nand2  g37(.a(new_n36_), .b(x16), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n37_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n44_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n41_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  aoi21  g43(.a(new_n36_), .b(new_n32_), .c(new_n39_), .O(new_n72_));
  inv1   g44(.a(new_n72_), .O(new_n73_));
  aoi21  g45(.a(new_n36_), .b(x16), .c(x17), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x06), .c(new_n41_), .O(new_n75_));
  oai21  g47(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(z7));
  nand2  g48(.a(x08), .b(x07), .O(new_n77_));
  inv1   g49(.a(new_n35_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n57_), .c(new_n29_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n33_), .c(new_n77_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n40_), .O(new_n81_));
  inv1   g53(.a(new_n77_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n72_), .c(x18), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n81_), .O(z8));
endmodule


