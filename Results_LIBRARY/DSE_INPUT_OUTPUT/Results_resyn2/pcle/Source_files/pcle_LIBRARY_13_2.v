// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand3  g04(.a(x16), .b(x15), .c(x14), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n34_), .c(x17), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g10(.a(x11), .O(new_n39_));
  nor2   g11(.a(x18), .b(new_n39_), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n32_), .b(x11), .c(new_n41_), .O(z1));
  inv1   g14(.a(new_n32_), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  inv1   g16(.a(x12), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n39_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n40_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  and2   g22(.a(x13), .b(x12), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n43_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n40_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n50_), .c(new_n54_), .O(z3));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(x13), .b(x12), .O(new_n57_));
  nor2   g29(.a(new_n57_), .b(new_n32_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n56_), .c(new_n29_), .O(new_n59_));
  and2   g31(.a(x08), .b(x03), .O(new_n60_));
  nand4  g32(.a(x14), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n61_));
  inv1   g33(.a(new_n61_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n52_), .c(new_n60_), .O(new_n63_));
  oai21  g35(.a(new_n59_), .b(new_n39_), .c(new_n63_), .O(z4));
  nor2   g36(.a(x15), .b(new_n56_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n58_), .c(new_n29_), .O(new_n66_));
  nand4  g38(.a(x15), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n51_), .b(x14), .c(x11), .O(new_n69_));
  and2   g41(.a(x08), .b(x04), .O(new_n70_));
  aoi21  g42(.a(new_n69_), .b(new_n68_), .c(new_n70_), .O(new_n71_));
  oai21  g43(.a(new_n66_), .b(new_n39_), .c(new_n71_), .O(z5));
  inv1   g44(.a(x16), .O(new_n73_));
  nand3  g45(.a(new_n51_), .b(new_n73_), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n67_), .c(x18), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x11), .O(new_n76_));
  nand3  g48(.a(new_n36_), .b(x15), .c(x14), .O(new_n77_));
  nand4  g49(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n78_));
  inv1   g50(.a(new_n78_), .O(new_n79_));
  aoi22  g51(.a(new_n79_), .b(new_n77_), .c(x08), .d(x05), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(z6));
  nor2   g53(.a(new_n33_), .b(x17), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(new_n58_), .c(new_n29_), .O(new_n83_));
  and2   g55(.a(x08), .b(x06), .O(new_n84_));
  nand2  g56(.a(new_n36_), .b(new_n34_), .O(new_n85_));
  nand4  g57(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n86_));
  inv1   g58(.a(new_n86_), .O(new_n87_));
  aoi21  g59(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(new_n88_));
  oai21  g60(.a(new_n83_), .b(new_n39_), .c(new_n88_), .O(z7));
  nand3  g61(.a(new_n37_), .b(new_n43_), .c(x18), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n40_), .c(new_n90_), .O(z8));
endmodule


