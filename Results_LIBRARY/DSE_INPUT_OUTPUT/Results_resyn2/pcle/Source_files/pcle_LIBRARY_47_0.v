// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x16), .b(x15), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x17), .b(x14), .c(x13), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nor2   g09(.a(x18), .b(x14), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n31_), .b(x11), .c(new_n39_), .O(z1));
  nand2  g12(.a(x08), .b(x01), .O(new_n41_));
  inv1   g13(.a(new_n31_), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n41_), .c(new_n38_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n42_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n38_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  oai21  g24(.a(new_n48_), .b(new_n31_), .c(x18), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g26(.a(new_n48_), .b(new_n42_), .c(x14), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z4));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(new_n48_), .b(new_n58_), .O(new_n59_));
  inv1   g31(.a(new_n48_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x15), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n59_), .c(new_n42_), .d(x14), .O(new_n62_));
  oai21  g34(.a(new_n31_), .b(new_n58_), .c(x18), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(z5));
  inv1   g38(.a(new_n38_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(x08), .c(x05), .O(new_n68_));
  and2   g40(.a(x14), .b(x13), .O(new_n69_));
  nand3  g41(.a(x15), .b(x12), .c(x11), .O(new_n70_));
  inv1   g42(.a(new_n70_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n69_), .c(x16), .O(new_n72_));
  nand3  g44(.a(new_n33_), .b(x14), .c(x13), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n67_), .c(new_n42_), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n72_), .c(new_n68_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nand4  g48(.a(new_n71_), .b(new_n69_), .c(new_n76_), .d(x16), .O(new_n77_));
  inv1   g49(.a(x13), .O(new_n78_));
  oai21  g50(.a(new_n32_), .b(new_n78_), .c(x17), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n42_), .O(new_n81_));
  oai21  g53(.a(new_n31_), .b(new_n76_), .c(x18), .O(new_n82_));
  and2   g54(.a(x08), .b(x06), .O(new_n83_));
  aoi21  g55(.a(new_n82_), .b(new_n52_), .c(new_n83_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(z7));
  nand3  g57(.a(new_n67_), .b(x08), .c(x07), .O(new_n86_));
  aoi21  g58(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n87_));
  nand2  g59(.a(new_n36_), .b(new_n42_), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(z8));
endmodule


