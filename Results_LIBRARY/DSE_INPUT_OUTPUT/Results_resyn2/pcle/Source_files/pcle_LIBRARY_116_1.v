// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n33_), .c(new_n32_), .d(new_n29_), .O(z0));
  nor2   g07(.a(new_n29_), .b(x12), .O(new_n36_));
  aoi21  g08(.a(x08), .b(x00), .c(new_n36_), .O(new_n37_));
  oai21  g09(.a(new_n32_), .b(x11), .c(new_n37_), .O(z1));
  inv1   g10(.a(new_n32_), .O(new_n39_));
  nand2  g11(.a(x12), .b(x11), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x12), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n36_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z2));
  nor2   g18(.a(new_n36_), .b(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x02), .O(new_n48_));
  nor2   g20(.a(new_n36_), .b(new_n32_), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n40_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n48_), .O(z3));
  nand2  g26(.a(new_n47_), .b(x03), .O(new_n55_));
  inv1   g27(.a(new_n50_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n34_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n55_), .O(z4));
  nand2  g33(.a(new_n47_), .b(x04), .O(new_n62_));
  inv1   g34(.a(x15), .O(new_n63_));
  nand2  g35(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nand3  g36(.a(new_n56_), .b(x15), .c(x14), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n64_), .c(new_n49_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n62_), .O(z5));
  inv1   g39(.a(new_n36_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x05), .O(new_n69_));
  nand4  g41(.a(x15), .b(x14), .c(x13), .d(x11), .O(new_n70_));
  inv1   g42(.a(new_n70_), .O(new_n71_));
  nand4  g43(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n71_), .c(new_n69_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n68_), .O(new_n74_));
  nand3  g46(.a(new_n29_), .b(x16), .c(new_n42_), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n34_), .c(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n39_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n74_), .O(z6));
  oai21  g52(.a(new_n70_), .b(new_n76_), .c(x17), .O(new_n81_));
  inv1   g53(.a(x17), .O(new_n82_));
  nand4  g54(.a(x16), .b(x15), .c(x12), .d(x11), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  nor2   g56(.a(new_n58_), .b(new_n51_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(new_n82_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n81_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n39_), .O(new_n88_));
  oai21  g60(.a(new_n32_), .b(new_n82_), .c(new_n29_), .O(new_n89_));
  and2   g61(.a(x08), .b(x06), .O(new_n90_));
  aoi21  g62(.a(new_n89_), .b(new_n42_), .c(new_n90_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n88_), .O(z7));
  nand2  g64(.a(x13), .b(x11), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n33_), .c(x18), .O(new_n94_));
  nor2   g66(.a(new_n82_), .b(new_n58_), .O(new_n95_));
  nor2   g67(.a(x18), .b(new_n51_), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n84_), .c(new_n95_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(new_n39_), .O(new_n99_));
  aoi21  g71(.a(x08), .b(x07), .c(new_n36_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n99_), .O(z8));
endmodule


