// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x17), .O(new_n29_));
  nand2  g01(.a(x13), .b(x12), .O(new_n30_));
  nand3  g02(.a(x16), .b(x15), .c(x14), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n32_), .c(x18), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x11), .c(new_n29_), .O(z0));
  inv1   g10(.a(x11), .O(new_n39_));
  oai21  g11(.a(new_n36_), .b(x17), .c(new_n39_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  nor2   g15(.a(new_n29_), .b(x11), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n44_), .O(new_n45_));
  oai21  g17(.a(new_n43_), .b(new_n35_), .c(new_n45_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n36_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n44_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  oai21  g24(.a(new_n35_), .b(new_n52_), .c(new_n29_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n39_), .O(new_n54_));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  nand2  g27(.a(new_n48_), .b(new_n52_), .O(new_n56_));
  inv1   g28(.a(new_n30_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n36_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n55_), .c(new_n54_), .O(z4));
  inv1   g32(.a(new_n44_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(x08), .c(x04), .O(new_n62_));
  inv1   g34(.a(new_n48_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(x15), .c(x14), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand4  g37(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n66_));
  oai21  g38(.a(new_n44_), .b(new_n65_), .c(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n64_), .c(new_n36_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n62_), .O(z5));
  inv1   g41(.a(x16), .O(new_n70_));
  oai21  g42(.a(new_n66_), .b(new_n65_), .c(new_n70_), .O(new_n71_));
  nor2   g43(.a(new_n35_), .b(new_n32_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  oai21  g45(.a(new_n35_), .b(new_n70_), .c(new_n29_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n39_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x05), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(z6));
  oai21  g49(.a(new_n31_), .b(new_n30_), .c(x17), .O(new_n78_));
  inv1   g50(.a(new_n31_), .O(new_n79_));
  nand3  g51(.a(new_n63_), .b(new_n79_), .c(new_n29_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n36_), .O(new_n82_));
  aoi21  g54(.a(x08), .b(x06), .c(new_n44_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z7));
  nand3  g56(.a(new_n61_), .b(x08), .c(x07), .O(new_n85_));
  aoi21  g57(.a(new_n32_), .b(x17), .c(x18), .O(new_n86_));
  nand4  g58(.a(new_n79_), .b(new_n57_), .c(x18), .d(x17), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n61_), .c(new_n36_), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(z8));
endmodule


