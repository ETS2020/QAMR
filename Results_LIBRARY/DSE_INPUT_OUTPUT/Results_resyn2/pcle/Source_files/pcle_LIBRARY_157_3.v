// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  nand2  g03(.a(x08), .b(x00), .O(new_n32_));
  inv1   g04(.a(x11), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n32_), .c(z0), .O(z1));
  nand2  g11(.a(x08), .b(x01), .O(new_n40_));
  nand2  g12(.a(x12), .b(x11), .O(new_n41_));
  inv1   g13(.a(x12), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n33_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n37_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n40_), .c(z0), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  aoi21  g19(.a(new_n41_), .b(new_n47_), .c(new_n36_), .O(new_n48_));
  oai21  g20(.a(new_n41_), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n46_), .c(z0), .O(z3));
  and2   g22(.a(x12), .b(x11), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(x13), .c(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n37_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(z0), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z4));
  nor2   g28(.a(z0), .b(new_n34_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x04), .O(new_n58_));
  aoi21  g30(.a(new_n53_), .b(new_n30_), .c(new_n29_), .O(new_n59_));
  inv1   g31(.a(new_n53_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(x15), .c(new_n37_), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n59_), .c(new_n58_), .O(z5));
  nand2  g34(.a(new_n57_), .b(x05), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(new_n53_), .b(new_n30_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(x15), .c(new_n64_), .O(new_n66_));
  and2   g38(.a(x14), .b(x13), .O(new_n67_));
  nor2   g39(.a(x18), .b(x16), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n67_), .c(new_n51_), .d(x15), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n66_), .c(new_n37_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n63_), .O(z6));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x16), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n36_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n60_), .c(x18), .O(new_n76_));
  and2   g48(.a(x08), .b(x06), .O(new_n77_));
  nor2   g49(.a(x18), .b(new_n64_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n60_), .c(x15), .O(new_n79_));
  nand4  g51(.a(x17), .b(new_n35_), .c(x09), .d(new_n34_), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n79_), .c(new_n77_), .O(new_n82_));
  oai21  g54(.a(new_n76_), .b(new_n29_), .c(new_n82_), .O(z7));
  nand2  g55(.a(new_n57_), .b(x07), .O(new_n84_));
  nand2  g56(.a(x18), .b(new_n29_), .O(new_n85_));
  nand4  g57(.a(new_n78_), .b(new_n60_), .c(x17), .d(x15), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n37_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n84_), .O(z8));
endmodule


