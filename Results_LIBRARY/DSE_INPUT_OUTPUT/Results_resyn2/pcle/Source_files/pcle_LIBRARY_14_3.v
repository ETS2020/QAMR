// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x12), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  aoi21  g06(.a(x08), .b(x00), .c(z0), .O(new_n35_));
  oai21  g07(.a(new_n34_), .b(x11), .c(new_n35_), .O(z1));
  xnor2a g08(.a(x12), .b(x11), .O(new_n37_));
  aoi21  g09(.a(x08), .b(x01), .c(z0), .O(new_n38_));
  oai21  g10(.a(new_n37_), .b(new_n34_), .c(new_n38_), .O(z2));
  aoi21  g11(.a(x12), .b(x11), .c(x13), .O(new_n40_));
  inv1   g12(.a(new_n34_), .O(new_n41_));
  nand3  g13(.a(x13), .b(x12), .c(x11), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x02), .c(z0), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(new_n40_), .c(new_n44_), .O(z3));
  inv1   g17(.a(x14), .O(new_n46_));
  nand2  g18(.a(x13), .b(x11), .O(new_n47_));
  nor2   g19(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n46_), .c(x18), .O(new_n49_));
  nand4  g21(.a(x14), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n50_));
  inv1   g22(.a(new_n50_), .O(new_n51_));
  aoi22  g23(.a(new_n51_), .b(new_n42_), .c(x08), .d(x03), .O(new_n52_));
  oai21  g24(.a(new_n49_), .b(new_n29_), .c(new_n52_), .O(z4));
  nor2   g25(.a(x15), .b(new_n46_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n48_), .c(x18), .O(new_n55_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  nand4  g28(.a(x15), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  aoi22  g30(.a(new_n58_), .b(new_n56_), .c(x08), .d(x04), .O(new_n59_));
  oai21  g31(.a(new_n55_), .b(new_n29_), .c(new_n59_), .O(z5));
  inv1   g32(.a(z0), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(x08), .c(x05), .O(new_n62_));
  inv1   g34(.a(x16), .O(new_n63_));
  nand4  g35(.a(x15), .b(x14), .c(x13), .d(x11), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n30_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(x12), .c(new_n63_), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand2  g39(.a(new_n30_), .b(new_n63_), .O(new_n68_));
  nor3   g40(.a(new_n68_), .b(new_n56_), .c(new_n67_), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n66_), .c(new_n41_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n62_), .O(z6));
  nand3  g43(.a(new_n61_), .b(x08), .c(x06), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  oai21  g45(.a(new_n64_), .b(new_n63_), .c(new_n30_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(x12), .c(new_n73_), .O(new_n75_));
  nand3  g47(.a(new_n30_), .b(new_n73_), .c(x16), .O(new_n76_));
  nor3   g48(.a(new_n76_), .b(new_n56_), .c(new_n67_), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n75_), .c(new_n41_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n72_), .O(z7));
  nand4  g51(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n42_), .c(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n41_), .O(new_n82_));
  aoi21  g54(.a(x08), .b(x07), .c(z0), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


