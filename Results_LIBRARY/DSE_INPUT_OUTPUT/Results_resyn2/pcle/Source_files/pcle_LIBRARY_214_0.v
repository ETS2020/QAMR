// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  inv1   g04(.a(x17), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  inv1   g08(.a(x14), .O(new_n37_));
  inv1   g09(.a(x15), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n36_), .c(new_n34_), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n31_), .O(z0));
  nor2   g13(.a(x18), .b(x05), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand3  g16(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n45_));
  inv1   g17(.a(new_n45_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n43_), .c(new_n42_), .O(z1));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  inv1   g22(.a(x12), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n44_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n50_), .c(new_n46_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n42_), .O(z2));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  inv1   g27(.a(x13), .O(new_n56_));
  nand2  g28(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n35_), .c(new_n46_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n42_), .O(z3));
  nand4  g31(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  nand2  g32(.a(new_n35_), .b(new_n37_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n60_), .c(new_n46_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x03), .c(new_n42_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z4));
  nor2   g36(.a(new_n60_), .b(new_n38_), .O(new_n65_));
  inv1   g37(.a(new_n60_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(x15), .c(new_n46_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n42_), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(z5));
  nand3  g41(.a(new_n39_), .b(new_n36_), .c(x16), .O(new_n70_));
  nand2  g42(.a(new_n39_), .b(new_n36_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n32_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n46_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x05), .c(new_n42_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z6));
  nor2   g47(.a(x17), .b(x16), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n34_), .c(new_n65_), .O(new_n77_));
  nand2  g49(.a(new_n71_), .b(new_n33_), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n46_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x06), .c(new_n42_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n79_), .O(z7));
  inv1   g53(.a(x05), .O(new_n82_));
  nand3  g54(.a(x17), .b(x16), .c(x15), .O(new_n83_));
  nor2   g55(.a(new_n83_), .b(new_n45_), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n66_), .c(new_n82_), .O(new_n85_));
  inv1   g57(.a(new_n31_), .O(new_n86_));
  and2   g58(.a(x08), .b(x07), .O(new_n87_));
  aoi21  g59(.a(new_n40_), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  oai21  g60(.a(new_n85_), .b(x18), .c(new_n88_), .O(z8));
endmodule


