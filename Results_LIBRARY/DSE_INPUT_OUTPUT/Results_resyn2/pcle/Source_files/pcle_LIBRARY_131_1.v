// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x04), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  inv1   g08(.a(x08), .O(new_n37_));
  nand3  g09(.a(new_n29_), .b(x09), .c(new_n37_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x18), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n36_), .c(new_n31_), .O(z0));
  aoi21  g13(.a(x08), .b(x00), .c(new_n30_), .O(new_n42_));
  oai21  g14(.a(new_n38_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n39_), .c(new_n44_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n39_), .b(new_n32_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n30_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n49_), .c(new_n51_), .O(z3));
  nor2   g24(.a(new_n30_), .b(new_n37_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x03), .O(new_n54_));
  nor2   g26(.a(new_n33_), .b(x14), .O(new_n55_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n39_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z4));
  inv1   g30(.a(new_n56_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x15), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nand2  g33(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n39_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n30_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z5));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n60_), .b(new_n66_), .O(new_n67_));
  nor2   g39(.a(new_n66_), .b(new_n61_), .O(new_n68_));
  nand2  g40(.a(new_n59_), .b(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n39_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n30_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  nand2  g44(.a(new_n53_), .b(x06), .O(new_n73_));
  nand3  g45(.a(new_n59_), .b(new_n68_), .c(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n69_), .b(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n39_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n73_), .O(z7));
  nand2  g50(.a(new_n53_), .b(x07), .O(new_n79_));
  oai21  g51(.a(new_n34_), .b(new_n32_), .c(x18), .O(new_n80_));
  nor2   g52(.a(x18), .b(new_n75_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n59_), .c(new_n68_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n39_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n79_), .O(z8));
endmodule


