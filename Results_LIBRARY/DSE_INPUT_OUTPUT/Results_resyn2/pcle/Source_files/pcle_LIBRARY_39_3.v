// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  and2   g07(.a(x17), .b(x16), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  inv1   g10(.a(x00), .O(new_n39_));
  oai21  g11(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n40_));
  inv1   g12(.a(x08), .O(new_n41_));
  nand3  g13(.a(new_n29_), .b(x09), .c(new_n41_), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(x11), .c(new_n40_), .d(new_n39_), .O(z1));
  inv1   g15(.a(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai22  g17(.a(new_n45_), .b(new_n42_), .c(new_n40_), .d(new_n44_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  inv1   g19(.a(new_n42_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nor2   g21(.a(new_n30_), .b(new_n29_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n50_), .O(new_n51_));
  oai21  g23(.a(new_n49_), .b(new_n47_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  inv1   g25(.a(new_n33_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n33_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n48_), .O(new_n58_));
  oai21  g30(.a(new_n40_), .b(new_n53_), .c(new_n58_), .O(z4));
  aoi21  g31(.a(new_n54_), .b(x14), .c(x15), .O(new_n60_));
  inv1   g32(.a(new_n34_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n54_), .O(new_n62_));
  nand2  g34(.a(new_n48_), .b(new_n62_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n50_), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(new_n60_), .c(new_n64_), .O(z5));
  nand2  g37(.a(new_n35_), .b(x16), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n50_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  aoi21  g43(.a(new_n35_), .b(x16), .c(x17), .O(new_n72_));
  nand3  g44(.a(new_n36_), .b(new_n61_), .c(new_n54_), .O(new_n73_));
  nand2  g45(.a(new_n48_), .b(new_n73_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x06), .c(new_n50_), .O(new_n75_));
  oai21  g47(.a(new_n74_), .b(new_n72_), .c(new_n75_), .O(z7));
  nand2  g48(.a(new_n30_), .b(new_n29_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n36_), .c(new_n35_), .O(new_n78_));
  nand2  g50(.a(new_n73_), .b(new_n30_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n32_), .O(new_n80_));
  aoi21  g52(.a(x08), .b(x07), .c(new_n50_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z8));
endmodule


