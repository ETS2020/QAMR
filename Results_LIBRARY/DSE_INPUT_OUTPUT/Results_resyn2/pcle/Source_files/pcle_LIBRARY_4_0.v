// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nor2   g00(.a(x18), .b(x17), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand3  g04(.a(x17), .b(x16), .c(x15), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x18), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n35_), .c(new_n30_), .O(z0));
  aoi21  g13(.a(x08), .b(x00), .c(new_n29_), .O(new_n42_));
  oai21  g14(.a(new_n38_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n39_), .c(new_n45_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n44_), .c(new_n29_), .O(z2));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n45_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n39_), .c(new_n50_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n29_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n50_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n39_), .c(new_n31_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n29_), .O(z4));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  nand2  g32(.a(new_n32_), .b(x15), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(new_n31_), .b(new_n62_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n61_), .c(new_n39_), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n60_), .c(new_n29_), .O(z5));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  nor2   g39(.a(new_n66_), .b(new_n62_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n32_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n39_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n29_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  nand3  g44(.a(new_n30_), .b(x08), .c(x06), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nand2  g46(.a(x18), .b(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n68_), .c(new_n32_), .O(new_n76_));
  nand2  g48(.a(new_n69_), .b(new_n74_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n39_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n73_), .O(z7));
  nand3  g51(.a(new_n30_), .b(x08), .c(x07), .O(new_n80_));
  oai21  g52(.a(new_n33_), .b(new_n31_), .c(x18), .O(new_n81_));
  nand2  g53(.a(x17), .b(x16), .O(new_n82_));
  nor2   g54(.a(new_n82_), .b(x18), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n32_), .c(x15), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n39_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n80_), .O(z8));
endmodule


