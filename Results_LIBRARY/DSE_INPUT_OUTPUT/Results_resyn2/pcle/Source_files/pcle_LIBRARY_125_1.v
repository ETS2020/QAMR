// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x17), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x08), .O(new_n31_));
  nor2   g03(.a(z0), .b(new_n31_), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x00), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  inv1   g06(.a(x09), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x08), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n29_), .c(new_n34_), .O(new_n37_));
  oai21  g09(.a(new_n37_), .b(x11), .c(new_n33_), .O(z1));
  nand2  g10(.a(new_n32_), .b(x01), .O(new_n39_));
  xnor2a g11(.a(x12), .b(x11), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n37_), .c(new_n39_), .O(z2));
  aoi21  g13(.a(x12), .b(x11), .c(x13), .O(new_n42_));
  nand3  g14(.a(x13), .b(x12), .c(x11), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n36_), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n42_), .c(new_n29_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n34_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x02), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z3));
  nand2  g20(.a(new_n32_), .b(x03), .O(new_n49_));
  inv1   g21(.a(new_n37_), .O(new_n50_));
  nand4  g22(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n51_));
  inv1   g23(.a(x14), .O(new_n52_));
  nand2  g24(.a(new_n43_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n49_), .O(z4));
  inv1   g27(.a(new_n51_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x15), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(x09), .b(new_n31_), .O(new_n59_));
  aoi21  g31(.a(new_n51_), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n57_), .c(x17), .O(new_n61_));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(x10), .c(new_n62_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(new_n51_), .b(new_n64_), .O(new_n65_));
  xnor2a g37(.a(x16), .b(x15), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n56_), .c(new_n59_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n65_), .c(x17), .O(new_n68_));
  nand2  g40(.a(x08), .b(x05), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(x10), .c(new_n69_), .O(z6));
  nand2  g42(.a(new_n32_), .b(x06), .O(new_n71_));
  nand4  g43(.a(new_n56_), .b(new_n50_), .c(x16), .d(x15), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(z7));
  aoi21  g45(.a(new_n36_), .b(x18), .c(x17), .O(new_n74_));
  nand2  g46(.a(x08), .b(x07), .O(new_n75_));
  oai21  g47(.a(new_n74_), .b(x10), .c(new_n75_), .O(z8));
endmodule


