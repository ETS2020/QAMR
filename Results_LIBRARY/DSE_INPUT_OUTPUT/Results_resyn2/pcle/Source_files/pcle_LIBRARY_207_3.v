// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_;
  nand2  g00(.a(x18), .b(x09), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  nand2  g03(.a(new_n29_), .b(x08), .O(new_n32_));
  inv1   g04(.a(x18), .O(new_n33_));
  nor2   g05(.a(x10), .b(x08), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(new_n33_), .c(x09), .O(new_n35_));
  oai22  g07(.a(new_n35_), .b(x11), .c(new_n32_), .d(new_n31_), .O(z1));
  nor2   g08(.a(x12), .b(x11), .O(new_n37_));
  nand2  g09(.a(x12), .b(x11), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n34_), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n37_), .c(new_n33_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x09), .O(new_n41_));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z2));
  inv1   g15(.a(new_n32_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x02), .O(new_n45_));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  inv1   g18(.a(new_n35_), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(z3));
  nand2  g22(.a(new_n44_), .b(x03), .O(new_n51_));
  inv1   g23(.a(new_n48_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand2  g25(.a(new_n52_), .b(x14), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(z4));
  nand2  g28(.a(new_n44_), .b(x04), .O(new_n57_));
  inv1   g29(.a(new_n54_), .O(new_n58_));
  nand2  g30(.a(x15), .b(x14), .O(new_n59_));
  nor2   g31(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  nor2   g32(.a(new_n60_), .b(new_n35_), .O(new_n61_));
  oai21  g33(.a(new_n58_), .b(x15), .c(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n57_), .O(z5));
  inv1   g35(.a(new_n59_), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n52_), .c(x16), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  oai21  g38(.a(new_n59_), .b(new_n48_), .c(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n34_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  nand2  g44(.a(new_n44_), .b(x06), .O(new_n73_));
  nand3  g45(.a(new_n60_), .b(x17), .c(x16), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n65_), .b(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n73_), .O(z7));
  nand2  g50(.a(new_n44_), .b(x07), .O(new_n79_));
  oai21  g51(.a(new_n74_), .b(new_n35_), .c(new_n79_), .O(z8));
endmodule


