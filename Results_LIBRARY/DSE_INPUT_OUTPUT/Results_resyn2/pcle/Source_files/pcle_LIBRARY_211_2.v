// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x11), .O(new_n29_));
  nand2  g01(.a(new_n29_), .b(x10), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand4  g04(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand2  g06(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  inv1   g07(.a(x08), .O(new_n36_));
  inv1   g08(.a(x10), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(x09), .c(new_n36_), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x18), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n35_), .c(new_n30_), .O(z0));
  aoi21  g13(.a(new_n29_), .b(x10), .c(new_n36_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x00), .O(new_n43_));
  oai21  g15(.a(new_n38_), .b(x11), .c(new_n43_), .O(z1));
  nand2  g16(.a(new_n42_), .b(x01), .O(new_n45_));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n38_), .c(new_n45_), .O(z2));
  nand2  g19(.a(new_n42_), .b(x02), .O(new_n48_));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n39_), .b(new_n31_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  nand2  g24(.a(new_n31_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(x14), .b(x13), .c(x12), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(new_n39_), .O(new_n55_));
  nand3  g27(.a(x14), .b(x09), .c(new_n36_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n37_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n29_), .O(new_n58_));
  nand2  g30(.a(x08), .b(x03), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(z4));
  nand2  g32(.a(new_n42_), .b(x04), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand4  g34(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n63_));
  nor2   g35(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g36(.a(new_n63_), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(x15), .c(new_n39_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n64_), .c(new_n61_), .O(z5));
  nand2  g39(.a(new_n42_), .b(x05), .O(new_n68_));
  nand2  g40(.a(x15), .b(x14), .O(new_n69_));
  nor2   g41(.a(new_n69_), .b(new_n31_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand3  g44(.a(new_n32_), .b(x15), .c(x14), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n71_), .c(new_n39_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n68_), .O(z6));
  nand2  g48(.a(new_n42_), .b(x06), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(new_n72_), .O(new_n79_));
  nor2   g51(.a(x17), .b(x16), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n79_), .c(new_n70_), .O(new_n81_));
  nand2  g53(.a(new_n73_), .b(new_n78_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n39_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(z7));
  nand2  g56(.a(new_n42_), .b(x07), .O(new_n85_));
  oai21  g57(.a(new_n33_), .b(new_n31_), .c(x18), .O(new_n86_));
  nor2   g58(.a(x18), .b(new_n62_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n65_), .c(new_n79_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n39_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n85_), .O(z8));
endmodule


