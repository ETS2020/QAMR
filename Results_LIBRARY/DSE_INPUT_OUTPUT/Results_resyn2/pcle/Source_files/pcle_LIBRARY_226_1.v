// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x15), .O(new_n36_));
  inv1   g08(.a(x16), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n35_), .c(new_n33_), .d(x17), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x03), .c(new_n29_), .O(z0));
  nor2   g12(.a(new_n29_), .b(x03), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n33_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n43_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n33_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x01), .c(new_n41_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n46_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(x13), .b(x12), .c(x11), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n54_), .c(new_n33_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n52_), .c(new_n41_), .O(z3));
  inv1   g29(.a(x03), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n55_), .b(new_n59_), .O(new_n60_));
  oai21  g32(.a(x18), .b(x08), .c(new_n58_), .O(new_n61_));
  inv1   g33(.a(x09), .O(new_n62_));
  nor2   g34(.a(x10), .b(new_n62_), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n61_), .c(new_n60_), .d(new_n34_), .O(new_n64_));
  oai21  g36(.a(new_n30_), .b(new_n58_), .c(new_n64_), .O(z4));
  nand2  g37(.a(new_n34_), .b(new_n36_), .O(new_n66_));
  nand2  g38(.a(new_n35_), .b(x15), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n33_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x04), .c(new_n41_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z5));
  nor2   g42(.a(new_n34_), .b(new_n36_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(x16), .O(new_n72_));
  nand2  g44(.a(new_n67_), .b(new_n37_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n33_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x05), .c(new_n41_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z6));
  aoi21  g48(.a(new_n71_), .b(x16), .c(x17), .O(new_n77_));
  nand3  g49(.a(new_n38_), .b(new_n35_), .c(x17), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n33_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x06), .c(new_n41_), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n77_), .c(new_n80_), .O(z7));
  nand4  g53(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n82_));
  and2   g54(.a(x18), .b(x03), .O(new_n83_));
  oai21  g55(.a(new_n82_), .b(new_n55_), .c(new_n83_), .O(new_n84_));
  inv1   g56(.a(x17), .O(new_n85_));
  nor2   g57(.a(x18), .b(new_n85_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n38_), .c(new_n35_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n33_), .O(new_n89_));
  inv1   g61(.a(new_n41_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(x08), .c(x07), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n89_), .O(z8));
endmodule


