// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x18), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n29_), .c(new_n33_), .O(new_n36_));
  oai22  g08(.a(new_n36_), .b(x11), .c(new_n32_), .d(new_n31_), .O(z1));
  inv1   g09(.a(x01), .O(new_n38_));
  xnor2a g10(.a(x12), .b(x11), .O(new_n39_));
  oai22  g11(.a(new_n39_), .b(new_n36_), .c(new_n32_), .d(new_n38_), .O(z2));
  aoi21  g12(.a(x12), .b(x11), .c(x13), .O(new_n41_));
  nand3  g13(.a(x13), .b(x12), .c(x11), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n41_), .c(new_n29_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n33_), .O(new_n45_));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z3));
  inv1   g19(.a(x03), .O(new_n48_));
  nand4  g20(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n49_));
  inv1   g21(.a(x14), .O(new_n50_));
  aoi21  g22(.a(new_n42_), .b(new_n50_), .c(new_n36_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  oai21  g24(.a(new_n32_), .b(new_n48_), .c(new_n52_), .O(z4));
  inv1   g25(.a(new_n49_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x15), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  inv1   g28(.a(x08), .O(new_n57_));
  nand2  g29(.a(x09), .b(new_n57_), .O(new_n58_));
  aoi21  g30(.a(new_n49_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n55_), .c(x18), .O(new_n60_));
  nand2  g32(.a(x08), .b(x04), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(x10), .c(new_n61_), .O(z5));
  inv1   g34(.a(x16), .O(new_n63_));
  nor3   g35(.a(new_n49_), .b(new_n63_), .c(new_n56_), .O(new_n64_));
  nand2  g36(.a(x15), .b(x14), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n42_), .c(new_n63_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n35_), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n64_), .c(new_n29_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n33_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n49_), .b(new_n72_), .O(new_n73_));
  nand2  g45(.a(x16), .b(x15), .O(new_n74_));
  xor2a  g46(.a(new_n74_), .b(x17), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n54_), .c(new_n58_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n73_), .c(x18), .O(new_n77_));
  nand2  g49(.a(x08), .b(x06), .O(new_n78_));
  oai21  g50(.a(new_n77_), .b(x10), .c(new_n78_), .O(z7));
  inv1   g51(.a(new_n74_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n54_), .c(new_n35_), .d(x17), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n33_), .O(new_n83_));
  nand2  g55(.a(x08), .b(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


