// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  nor2   g00(.a(x18), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  nand4  g03(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n33_), .c(x18), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n30_), .O(z0));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand2  g13(.a(new_n37_), .b(new_n41_), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n29_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  and2   g16(.a(x12), .b(x11), .O(new_n45_));
  inv1   g17(.a(new_n45_), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n41_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n37_), .c(new_n46_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n44_), .c(new_n29_), .O(z2));
  nor2   g22(.a(new_n45_), .b(x13), .O(new_n51_));
  nand2  g23(.a(new_n37_), .b(new_n31_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n29_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n31_), .b(new_n56_), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  nor2   g31(.a(new_n59_), .b(new_n36_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n55_), .c(new_n29_), .O(z4));
  inv1   g34(.a(x15), .O(new_n63_));
  oai21  g35(.a(new_n58_), .b(new_n36_), .c(x18), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g37(.a(new_n60_), .b(x15), .O(new_n66_));
  nand2  g38(.a(x08), .b(x04), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n65_), .O(z5));
  nand3  g40(.a(new_n30_), .b(x08), .c(x05), .O(new_n69_));
  and2   g41(.a(x14), .b(x13), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n45_), .c(x16), .d(x15), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  oai21  g44(.a(new_n58_), .b(new_n63_), .c(new_n72_), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(new_n71_), .c(new_n37_), .d(new_n30_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n69_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nand2  g48(.a(new_n71_), .b(new_n76_), .O(new_n77_));
  nor2   g49(.a(new_n76_), .b(new_n72_), .O(new_n78_));
  aoi21  g50(.a(new_n59_), .b(new_n78_), .c(new_n36_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g52(.a(new_n36_), .b(new_n76_), .c(x18), .O(new_n81_));
  and2   g53(.a(x08), .b(x06), .O(new_n82_));
  aoi21  g54(.a(new_n81_), .b(new_n63_), .c(new_n82_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n80_), .O(z7));
  nand3  g56(.a(new_n30_), .b(x08), .c(x07), .O(new_n85_));
  nand2  g57(.a(new_n33_), .b(x18), .O(new_n86_));
  inv1   g58(.a(x18), .O(new_n87_));
  oai21  g59(.a(new_n32_), .b(new_n31_), .c(new_n87_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n86_), .c(new_n37_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n85_), .O(z8));
endmodule


