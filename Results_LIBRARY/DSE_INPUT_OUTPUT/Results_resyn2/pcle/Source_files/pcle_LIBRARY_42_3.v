// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand4  g01(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x11), .O(new_n31_));
  or2    g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g09(.a(x17), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n33_), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n36_), .b(x11), .c(new_n40_), .O(z1));
  inv1   g13(.a(x08), .O(new_n42_));
  nor2   g14(.a(new_n39_), .b(new_n42_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n36_), .c(new_n44_), .O(z2));
  nand2  g18(.a(new_n43_), .b(x02), .O(new_n47_));
  inv1   g19(.a(new_n36_), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(x12), .b(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n49_), .c(new_n48_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n47_), .O(z3));
  nand2  g26(.a(new_n43_), .b(x03), .O(new_n55_));
  nand4  g27(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand2  g29(.a(new_n49_), .b(new_n57_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n55_), .O(z4));
  inv1   g32(.a(x15), .O(new_n61_));
  nor2   g33(.a(new_n56_), .b(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n56_), .b(new_n61_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n35_), .c(new_n33_), .O(new_n64_));
  aoi21  g36(.a(x08), .b(x04), .c(new_n39_), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(new_n62_), .c(new_n65_), .O(z5));
  nand2  g38(.a(new_n43_), .b(x05), .O(new_n67_));
  and2   g39(.a(x15), .b(x14), .O(new_n68_));
  nand3  g40(.a(x13), .b(x12), .c(x11), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n68_), .c(x16), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n70_), .b(new_n68_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n71_), .c(new_n48_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n67_), .O(z6));
  nand2  g48(.a(x16), .b(x11), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n30_), .c(x17), .O(new_n78_));
  nand3  g50(.a(new_n38_), .b(x16), .c(new_n33_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n73_), .c(new_n78_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n35_), .O(new_n81_));
  aoi21  g53(.a(x08), .b(x06), .c(new_n39_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z7));
  nand2  g55(.a(new_n43_), .b(x07), .O(new_n84_));
  oai21  g56(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n85_));
  nand4  g57(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n56_), .c(new_n85_), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n48_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n84_), .O(z8));
endmodule


