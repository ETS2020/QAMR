// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x11), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  nand4  g03(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(x17), .b(x16), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  nand3  g10(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(x18), .c(new_n38_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z1));
  aoi21  g15(.a(x18), .b(new_n38_), .c(new_n29_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x01), .O(new_n45_));
  nand2  g17(.a(x18), .b(new_n38_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  xnor2a g19(.a(x12), .b(x11), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(z2));
  nand2  g21(.a(new_n44_), .b(x02), .O(new_n50_));
  nand2  g22(.a(x12), .b(x11), .O(new_n51_));
  xor2a  g23(.a(new_n51_), .b(x13), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g25(.a(x14), .O(new_n54_));
  inv1   g26(.a(x18), .O(new_n55_));
  oai21  g27(.a(new_n39_), .b(new_n54_), .c(new_n55_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n38_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x03), .O(new_n58_));
  nand3  g30(.a(x14), .b(x13), .c(x12), .O(new_n59_));
  inv1   g31(.a(x13), .O(new_n60_));
  oai21  g32(.a(new_n51_), .b(new_n60_), .c(new_n54_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n40_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n58_), .c(new_n57_), .O(z4));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n32_), .c(new_n40_), .O(new_n67_));
  oai21  g39(.a(new_n39_), .b(new_n64_), .c(new_n55_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n38_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x04), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n69_), .c(new_n67_), .O(z5));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x11), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n33_), .O(new_n74_));
  nand2  g46(.a(new_n32_), .b(new_n72_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n40_), .O(new_n76_));
  oai21  g48(.a(new_n39_), .b(new_n72_), .c(new_n55_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n38_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x05), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(z6));
  nand2  g52(.a(new_n44_), .b(x06), .O(new_n81_));
  aoi21  g53(.a(x18), .b(new_n38_), .c(new_n39_), .O(new_n82_));
  inv1   g54(.a(x17), .O(new_n83_));
  nand2  g55(.a(x16), .b(x15), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n65_), .c(new_n83_), .O(new_n85_));
  inv1   g57(.a(new_n65_), .O(new_n86_));
  inv1   g58(.a(new_n84_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(x17), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n85_), .c(new_n82_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n81_), .O(z7));
  nand2  g62(.a(new_n44_), .b(x07), .O(new_n91_));
  inv1   g63(.a(new_n31_), .O(new_n92_));
  oai21  g64(.a(new_n34_), .b(new_n32_), .c(new_n55_), .O(new_n93_));
  nand3  g65(.a(new_n93_), .b(new_n36_), .c(new_n92_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(new_n91_), .O(z8));
endmodule


