// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_;
  nand2  g00(.a(x01), .b(x00), .O(new_n29_));
  nand4  g01(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n30_));
  nand3  g02(.a(x17), .b(x16), .c(x15), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(new_n32_), .c(x18), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n29_), .O(z0));
  oai21  g10(.a(x08), .b(x01), .c(x00), .O(new_n39_));
  oai21  g11(.a(new_n35_), .b(x11), .c(new_n39_), .O(z1));
  inv1   g12(.a(x00), .O(new_n41_));
  nand3  g13(.a(x08), .b(x01), .c(new_n41_), .O(new_n42_));
  inv1   g14(.a(new_n29_), .O(new_n43_));
  and2   g15(.a(x12), .b(x11), .O(new_n44_));
  nor2   g16(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g17(.a(x12), .b(x11), .c(new_n45_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n35_), .c(new_n42_), .O(z2));
  nand2  g19(.a(new_n44_), .b(x13), .O(new_n48_));
  or2    g20(.a(new_n44_), .b(x13), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n48_), .c(new_n36_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n43_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(z3));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  nand2  g26(.a(new_n48_), .b(new_n54_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(new_n36_), .c(new_n30_), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n53_), .c(new_n43_), .O(z4));
  nand2  g29(.a(x08), .b(x04), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n30_), .b(new_n59_), .O(new_n60_));
  and2   g32(.a(x14), .b(x13), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n44_), .c(x15), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n36_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n58_), .c(new_n43_), .O(z5));
  nand4  g36(.a(new_n61_), .b(new_n44_), .c(x16), .d(x15), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n62_), .b(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n36_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n43_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z6));
  nand2  g42(.a(x08), .b(x06), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  nor2   g45(.a(new_n35_), .b(new_n32_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n71_), .c(new_n43_), .O(z7));
  nand2  g48(.a(x08), .b(x07), .O(new_n77_));
  nand2  g49(.a(new_n32_), .b(x18), .O(new_n78_));
  inv1   g50(.a(x18), .O(new_n79_));
  oai21  g51(.a(new_n31_), .b(new_n30_), .c(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n36_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n77_), .c(new_n43_), .O(z8));
endmodule


