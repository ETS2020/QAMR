// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x16), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g09(.a(x18), .b(new_n29_), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand3  g12(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(new_n38_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n42_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n38_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n50_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n34_), .c(new_n42_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n38_), .O(z4));
  nand2  g30(.a(new_n34_), .b(new_n29_), .O(new_n59_));
  nand2  g31(.a(new_n35_), .b(x15), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n42_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x04), .c(new_n38_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n34_), .c(x18), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x15), .O(new_n67_));
  nand4  g39(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  aoi22  g41(.a(new_n69_), .b(new_n60_), .c(x08), .d(x05), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n67_), .O(z6));
  inv1   g43(.a(x18), .O(new_n72_));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x16), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n41_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n35_), .c(new_n72_), .O(new_n76_));
  nand3  g48(.a(new_n35_), .b(x16), .c(x15), .O(new_n77_));
  and2   g49(.a(x08), .b(x06), .O(new_n78_));
  aoi21  g50(.a(new_n77_), .b(new_n33_), .c(new_n78_), .O(new_n79_));
  oai21  g51(.a(new_n76_), .b(new_n29_), .c(new_n79_), .O(z7));
  nor2   g52(.a(new_n77_), .b(new_n73_), .O(new_n81_));
  nand2  g53(.a(new_n42_), .b(x18), .O(new_n82_));
  inv1   g54(.a(new_n38_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x08), .c(x07), .O(new_n84_));
  oai21  g56(.a(new_n82_), .b(new_n81_), .c(new_n84_), .O(z8));
endmodule


