// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_;
  inv1   g00(.a(x09), .O(new_n29_));
  nor2   g01(.a(x10), .b(new_n29_), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  and2   g04(.a(x15), .b(x14), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n32_), .c(x17), .d(x16), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x18), .c(x08), .O(z0));
  inv1   g09(.a(x08), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  nand2  g11(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x18), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n38_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n42_), .O(z1));
  nor2   g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n30_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n45_), .c(x18), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n38_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand2  g25(.a(new_n46_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n30_), .b(x18), .c(new_n38_), .O(new_n55_));
  inv1   g27(.a(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n54_), .c(new_n31_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n52_), .O(z3));
  nand2  g30(.a(x08), .b(x03), .O(new_n59_));
  nand2  g31(.a(new_n32_), .b(x14), .O(new_n60_));
  inv1   g32(.a(x14), .O(new_n61_));
  nand2  g33(.a(new_n31_), .b(new_n61_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n59_), .O(z4));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand2  g37(.a(new_n33_), .b(new_n32_), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand2  g39(.a(new_n60_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n56_), .c(new_n66_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n65_), .O(z5));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  nand3  g43(.a(new_n33_), .b(new_n32_), .c(x16), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(new_n66_), .b(new_n73_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n56_), .c(new_n72_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n71_), .O(z6));
  inv1   g48(.a(x17), .O(new_n77_));
  nand3  g49(.a(x16), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n31_), .c(new_n77_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n34_), .c(new_n30_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x18), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n38_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x06), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z7));
  nand2  g56(.a(x08), .b(x07), .O(new_n85_));
  oai21  g57(.a(new_n55_), .b(new_n35_), .c(new_n85_), .O(z8));
endmodule


