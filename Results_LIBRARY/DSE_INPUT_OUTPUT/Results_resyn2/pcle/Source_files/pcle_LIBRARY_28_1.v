// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nand4  g00(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  inv1   g06(.a(x05), .O(new_n35_));
  inv1   g07(.a(x15), .O(new_n36_));
  nand3  g08(.a(x18), .b(x17), .c(x16), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  nand3  g10(.a(new_n38_), .b(new_n34_), .c(new_n30_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(z0));
  inv1   g12(.a(x18), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(x05), .O(new_n42_));
  aoi21  g14(.a(x08), .b(x00), .c(new_n42_), .O(new_n43_));
  oai21  g15(.a(new_n33_), .b(x11), .c(new_n43_), .O(z1));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n34_), .c(new_n45_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n42_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n34_), .b(new_n51_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n42_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z3));
  nand2  g26(.a(x08), .b(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n34_), .c(new_n29_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(new_n42_), .O(z4));
  nand2  g31(.a(new_n29_), .b(new_n36_), .O(new_n60_));
  nand2  g32(.a(x15), .b(x14), .O(new_n61_));
  nor2   g33(.a(new_n61_), .b(new_n51_), .O(new_n62_));
  inv1   g34(.a(new_n62_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n60_), .c(new_n34_), .O(new_n64_));
  aoi21  g36(.a(x08), .b(x04), .c(new_n42_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z5));
  nand2  g38(.a(new_n62_), .b(x16), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  oai21  g40(.a(new_n61_), .b(new_n51_), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n34_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n42_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  aoi21  g44(.a(new_n62_), .b(x16), .c(x17), .O(new_n73_));
  inv1   g45(.a(new_n51_), .O(new_n74_));
  and2   g46(.a(x17), .b(x16), .O(new_n75_));
  inv1   g47(.a(new_n61_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n34_), .O(new_n78_));
  aoi21  g50(.a(x08), .b(x06), .c(new_n42_), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(new_n73_), .c(new_n79_), .O(z7));
  nand4  g52(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n81_));
  inv1   g53(.a(new_n81_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n74_), .c(x18), .O(new_n83_));
  oai21  g55(.a(new_n81_), .b(new_n51_), .c(new_n41_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(new_n34_), .O(new_n85_));
  aoi21  g57(.a(x08), .b(x07), .c(new_n42_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n85_), .O(z8));
endmodule


