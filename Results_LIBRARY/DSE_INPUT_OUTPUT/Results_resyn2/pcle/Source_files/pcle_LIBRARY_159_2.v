// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(x18), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  nand4  g04(.a(x18), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(x17), .b(x16), .c(x15), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n33_), .c(new_n31_), .O(z0));
  inv1   g11(.a(x00), .O(new_n40_));
  nand3  g12(.a(new_n29_), .b(x09), .c(new_n32_), .O(new_n41_));
  nand2  g13(.a(new_n31_), .b(x08), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n40_), .c(new_n41_), .d(x11), .O(z1));
  inv1   g15(.a(new_n41_), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n30_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n44_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n30_), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z3));
  inv1   g26(.a(x03), .O(new_n55_));
  inv1   g27(.a(new_n51_), .O(new_n56_));
  nor2   g28(.a(new_n56_), .b(x14), .O(new_n57_));
  nand2  g29(.a(new_n34_), .b(new_n44_), .O(new_n58_));
  oai22  g30(.a(new_n58_), .b(new_n57_), .c(new_n42_), .d(new_n55_), .O(z4));
  inv1   g31(.a(x15), .O(new_n60_));
  nor2   g32(.a(new_n34_), .b(new_n60_), .O(new_n61_));
  oai21  g33(.a(new_n35_), .b(x15), .c(new_n44_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n30_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(z5));
  inv1   g36(.a(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(x15), .b(x14), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n51_), .c(new_n66_), .O(new_n68_));
  nand3  g40(.a(x16), .b(x15), .c(x14), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n56_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n68_), .c(new_n44_), .O(new_n72_));
  oai21  g44(.a(new_n42_), .b(new_n65_), .c(new_n72_), .O(z6));
  inv1   g45(.a(x06), .O(new_n74_));
  aoi21  g46(.a(new_n70_), .b(new_n56_), .c(x17), .O(new_n75_));
  nand2  g47(.a(new_n38_), .b(new_n44_), .O(new_n76_));
  oai22  g48(.a(new_n76_), .b(new_n75_), .c(new_n42_), .d(new_n74_), .O(z7));
  inv1   g49(.a(x18), .O(new_n78_));
  inv1   g50(.a(x09), .O(new_n79_));
  nor2   g51(.a(new_n79_), .b(x08), .O(new_n80_));
  nand3  g52(.a(new_n37_), .b(new_n35_), .c(new_n80_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  inv1   g55(.a(new_n33_), .O(new_n84_));
  aoi22  g56(.a(new_n38_), .b(new_n84_), .c(x08), .d(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n83_), .O(z8));
endmodule


