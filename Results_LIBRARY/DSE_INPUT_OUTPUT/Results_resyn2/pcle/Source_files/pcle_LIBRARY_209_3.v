// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_;
  nand2  g00(.a(x18), .b(x14), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  nand2  g02(.a(x08), .b(x00), .O(new_n31_));
  inv1   g03(.a(x11), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n31_), .c(z0), .O(z1));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  nand2  g11(.a(x12), .b(x11), .O(new_n40_));
  inv1   g12(.a(x12), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n36_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(z0), .O(z2));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  nand3  g17(.a(x13), .b(x12), .c(x11), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  nand2  g19(.a(new_n40_), .b(new_n47_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n36_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(z0), .O(z3));
  inv1   g22(.a(new_n46_), .O(new_n51_));
  nor2   g23(.a(new_n51_), .b(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n36_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(z0), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z4));
  nand3  g28(.a(new_n29_), .b(x08), .c(x04), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n51_), .b(x15), .c(x14), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n59_), .c(new_n36_), .d(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n57_), .O(z5));
  nand3  g34(.a(new_n29_), .b(x08), .c(x05), .O(new_n63_));
  nand2  g35(.a(x16), .b(x15), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(x18), .c(x14), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n60_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n36_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n63_), .O(z6));
  inv1   g42(.a(x14), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(new_n34_), .c(x09), .d(new_n33_), .O(new_n73_));
  inv1   g45(.a(new_n73_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n65_), .c(x18), .O(new_n75_));
  and2   g47(.a(x08), .b(x06), .O(new_n76_));
  inv1   g48(.a(new_n64_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n51_), .c(x14), .O(new_n78_));
  nand4  g50(.a(x17), .b(new_n34_), .c(x09), .d(new_n33_), .O(new_n79_));
  inv1   g51(.a(new_n79_), .O(new_n80_));
  aoi21  g52(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  oai21  g53(.a(new_n75_), .b(new_n71_), .c(new_n81_), .O(z7));
  nor2   g54(.a(new_n64_), .b(new_n53_), .O(new_n83_));
  aoi21  g55(.a(new_n83_), .b(x17), .c(x18), .O(new_n84_));
  aoi21  g56(.a(x08), .b(x07), .c(z0), .O(new_n85_));
  oai21  g57(.a(new_n84_), .b(new_n35_), .c(new_n85_), .O(z8));
endmodule


