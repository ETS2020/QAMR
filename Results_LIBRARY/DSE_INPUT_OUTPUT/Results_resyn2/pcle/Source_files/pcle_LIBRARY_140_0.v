// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand4  g03(.a(x16), .b(x15), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x17), .b(x14), .c(x13), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nor2   g09(.a(x18), .b(x14), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n31_), .b(x11), .c(new_n39_), .O(z1));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x01), .c(new_n38_), .O(new_n42_));
  oai21  g14(.a(new_n41_), .b(new_n31_), .c(new_n42_), .O(z2));
  aoi21  g15(.a(x12), .b(x11), .c(x13), .O(new_n44_));
  inv1   g16(.a(new_n31_), .O(new_n45_));
  nand3  g17(.a(x13), .b(x12), .c(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x02), .c(new_n38_), .O(new_n48_));
  oai21  g20(.a(new_n47_), .b(new_n44_), .c(new_n48_), .O(z3));
  inv1   g21(.a(x14), .O(new_n50_));
  oai21  g22(.a(new_n46_), .b(new_n31_), .c(x18), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g24(.a(new_n46_), .b(new_n45_), .c(x14), .O(new_n53_));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z4));
  inv1   g27(.a(x15), .O(new_n56_));
  nand2  g28(.a(new_n46_), .b(new_n56_), .O(new_n57_));
  inv1   g29(.a(new_n46_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x15), .O(new_n59_));
  nand4  g31(.a(new_n59_), .b(new_n57_), .c(new_n45_), .d(x14), .O(new_n60_));
  oai21  g32(.a(new_n31_), .b(new_n56_), .c(x18), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n50_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x04), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(z5));
  inv1   g36(.a(new_n38_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x08), .c(x05), .O(new_n66_));
  and2   g38(.a(x14), .b(x13), .O(new_n67_));
  nand3  g39(.a(x15), .b(x12), .c(x11), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n67_), .c(x16), .O(new_n70_));
  nand3  g42(.a(new_n33_), .b(x14), .c(x13), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(new_n65_), .c(new_n45_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n70_), .c(new_n66_), .O(z6));
  inv1   g45(.a(x17), .O(new_n74_));
  nand4  g46(.a(new_n69_), .b(new_n67_), .c(new_n74_), .d(x16), .O(new_n75_));
  inv1   g47(.a(x13), .O(new_n76_));
  oai21  g48(.a(new_n32_), .b(new_n76_), .c(x17), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  oai21  g51(.a(new_n31_), .b(new_n74_), .c(x18), .O(new_n80_));
  and2   g52(.a(x08), .b(x06), .O(new_n81_));
  aoi21  g53(.a(new_n80_), .b(new_n50_), .c(new_n81_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(z7));
  nand3  g55(.a(new_n65_), .b(x08), .c(x07), .O(new_n84_));
  aoi21  g56(.a(new_n35_), .b(new_n33_), .c(x18), .O(new_n85_));
  nand2  g57(.a(new_n36_), .b(new_n45_), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z8));
endmodule


