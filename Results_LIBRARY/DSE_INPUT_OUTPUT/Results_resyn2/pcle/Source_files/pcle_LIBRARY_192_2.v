// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x02), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(x08), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nor3   g06(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n31_), .c(x18), .d(x17), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g09(.a(x08), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x02), .c(new_n38_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x00), .O(new_n41_));
  nand3  g13(.a(new_n31_), .b(new_n39_), .c(new_n29_), .O(new_n42_));
  oai21  g14(.a(new_n42_), .b(x11), .c(new_n41_), .O(z1));
  nor2   g15(.a(x12), .b(x11), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n31_), .O(new_n46_));
  oai21  g18(.a(new_n46_), .b(new_n44_), .c(new_n29_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n39_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z2));
  oai21  g22(.a(new_n39_), .b(x08), .c(x02), .O(new_n51_));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand3  g24(.a(new_n33_), .b(new_n31_), .c(new_n39_), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n33_), .b(new_n55_), .O(new_n56_));
  inv1   g28(.a(new_n33_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n31_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n39_), .O(new_n61_));
  nand2  g33(.a(x08), .b(x03), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z4));
  nand2  g35(.a(new_n40_), .b(x04), .O(new_n64_));
  inv1   g36(.a(new_n34_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n57_), .O(new_n66_));
  inv1   g38(.a(new_n42_), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand2  g40(.a(new_n58_), .b(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n64_), .O(z5));
  nand3  g43(.a(new_n65_), .b(new_n57_), .c(x16), .O(new_n72_));
  oai21  g44(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n31_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n29_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n39_), .O(new_n76_));
  nand2  g48(.a(x08), .b(x05), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n76_), .O(z6));
  nand2  g50(.a(new_n40_), .b(x06), .O(new_n79_));
  nor2   g51(.a(new_n35_), .b(x17), .O(new_n80_));
  nand4  g52(.a(new_n65_), .b(new_n57_), .c(x17), .d(x16), .O(new_n81_));
  nand2  g53(.a(new_n67_), .b(new_n81_), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(z7));
  nand2  g55(.a(new_n40_), .b(x07), .O(new_n84_));
  nand3  g56(.a(new_n35_), .b(x18), .c(x17), .O(new_n85_));
  inv1   g57(.a(x18), .O(new_n86_));
  nand2  g58(.a(new_n81_), .b(new_n86_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n67_), .c(new_n85_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n84_), .O(z8));
endmodule


