// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_;
  inv1   g00(.a(x14), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x17), .O(new_n34_));
  nand4  g06(.a(x16), .b(x15), .c(x13), .d(x12), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  nand3  g10(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n39_));
  nor2   g11(.a(x18), .b(new_n29_), .O(new_n40_));
  aoi21  g12(.a(x08), .b(x00), .c(new_n40_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(x11), .c(new_n41_), .O(z1));
  nand2  g14(.a(x08), .b(x01), .O(new_n43_));
  inv1   g15(.a(new_n39_), .O(new_n44_));
  nand2  g16(.a(x12), .b(x11), .O(new_n45_));
  or2    g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n43_), .c(new_n40_), .O(z2));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n45_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n51_), .c(new_n44_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n40_), .O(z3));
  inv1   g26(.a(new_n40_), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(x08), .c(x03), .O(new_n56_));
  aoi21  g28(.a(new_n52_), .b(x18), .c(new_n29_), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(x14), .c(new_n44_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand3  g33(.a(new_n55_), .b(x08), .c(x04), .O(new_n62_));
  inv1   g34(.a(x15), .O(new_n63_));
  nand2  g35(.a(new_n52_), .b(x18), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(x14), .c(new_n63_), .O(new_n65_));
  nand3  g37(.a(x18), .b(new_n63_), .c(x14), .O(new_n66_));
  nor2   g38(.a(new_n66_), .b(new_n58_), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n65_), .c(new_n44_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n62_), .O(z5));
  inv1   g41(.a(x16), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(x15), .c(x13), .d(x12), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n32_), .c(x18), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x14), .O(new_n73_));
  inv1   g45(.a(new_n52_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(x15), .c(x14), .O(new_n75_));
  nand4  g47(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  aoi22  g49(.a(new_n77_), .b(new_n75_), .c(x08), .d(x05), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n73_), .O(z6));
  inv1   g51(.a(x18), .O(new_n80_));
  nor2   g52(.a(new_n35_), .b(x17), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n33_), .c(new_n80_), .O(new_n82_));
  and2   g54(.a(x08), .b(x06), .O(new_n83_));
  nand3  g55(.a(x16), .b(x15), .c(x14), .O(new_n84_));
  inv1   g56(.a(new_n84_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n59_), .O(new_n86_));
  nand4  g58(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n87_));
  inv1   g59(.a(new_n87_), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(new_n86_), .c(new_n83_), .O(new_n89_));
  oai21  g61(.a(new_n82_), .b(new_n29_), .c(new_n89_), .O(z7));
  nor2   g62(.a(new_n86_), .b(new_n34_), .O(new_n91_));
  nand2  g63(.a(new_n44_), .b(x18), .O(new_n92_));
  aoi21  g64(.a(x08), .b(x07), .c(new_n40_), .O(new_n93_));
  oai21  g65(.a(new_n92_), .b(new_n91_), .c(new_n93_), .O(z8));
endmodule


