// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  inv1   g04(.a(x17), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  inv1   g08(.a(x14), .O(new_n37_));
  inv1   g09(.a(x15), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n36_), .c(new_n34_), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n31_), .O(z0));
  nand3  g13(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n42_));
  nor2   g14(.a(x18), .b(x05), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x00), .c(new_n43_), .O(new_n44_));
  oai21  g16(.a(new_n42_), .b(x11), .c(new_n44_), .O(z1));
  xnor2a g17(.a(x12), .b(x11), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n43_), .O(new_n47_));
  oai21  g19(.a(new_n46_), .b(new_n42_), .c(new_n47_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  inv1   g21(.a(new_n42_), .O(new_n50_));
  nand2  g22(.a(new_n35_), .b(new_n50_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n43_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand4  g26(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  nand2  g27(.a(new_n35_), .b(new_n37_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n50_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n43_), .O(z4));
  nor2   g30(.a(new_n55_), .b(new_n38_), .O(new_n59_));
  inv1   g31(.a(new_n55_), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(x15), .c(new_n50_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x04), .c(new_n43_), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(new_n59_), .c(new_n62_), .O(z5));
  nand3  g35(.a(new_n39_), .b(new_n36_), .c(x16), .O(new_n64_));
  nand2  g36(.a(new_n39_), .b(new_n36_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n32_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n50_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x05), .c(new_n43_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(z6));
  nor2   g41(.a(x17), .b(x16), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n34_), .c(new_n59_), .O(new_n71_));
  nand2  g43(.a(new_n65_), .b(new_n33_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n71_), .c(new_n50_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x06), .c(new_n43_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z7));
  inv1   g47(.a(x05), .O(new_n76_));
  nand3  g48(.a(x17), .b(x16), .c(x15), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(new_n42_), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n60_), .c(new_n76_), .O(new_n79_));
  inv1   g51(.a(new_n31_), .O(new_n80_));
  and2   g52(.a(x08), .b(x07), .O(new_n81_));
  aoi21  g53(.a(new_n40_), .b(new_n80_), .c(new_n81_), .O(new_n82_));
  oai21  g54(.a(new_n79_), .b(x18), .c(new_n82_), .O(z8));
endmodule


