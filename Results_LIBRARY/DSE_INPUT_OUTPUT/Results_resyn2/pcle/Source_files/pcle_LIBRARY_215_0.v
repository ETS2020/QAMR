// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x16), .b(x15), .c(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x17), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  nand3  g09(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n38_));
  nor2   g10(.a(x18), .b(x09), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n29_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x00), .O(new_n41_));
  oai21  g13(.a(new_n38_), .b(x11), .c(new_n41_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x01), .c(new_n39_), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(new_n38_), .c(new_n44_), .O(z2));
  nand2  g17(.a(new_n40_), .b(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  inv1   g19(.a(new_n38_), .O(new_n48_));
  nand2  g20(.a(new_n32_), .b(new_n48_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(z3));
  nor2   g22(.a(new_n33_), .b(x14), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x03), .c(new_n39_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n51_), .c(new_n54_), .O(z4));
  inv1   g27(.a(x15), .O(new_n56_));
  nor2   g28(.a(new_n52_), .b(new_n56_), .O(new_n57_));
  nand2  g29(.a(new_n52_), .b(new_n56_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x04), .c(new_n39_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z5));
  nand2  g33(.a(new_n40_), .b(x05), .O(new_n62_));
  nor2   g34(.a(new_n34_), .b(new_n32_), .O(new_n63_));
  inv1   g35(.a(new_n63_), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n32_), .c(new_n65_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n64_), .c(new_n48_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n62_), .O(z6));
  nand2  g41(.a(new_n40_), .b(x06), .O(new_n70_));
  nor2   g42(.a(new_n63_), .b(x17), .O(new_n71_));
  nand2  g43(.a(new_n36_), .b(new_n48_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z7));
  inv1   g45(.a(x09), .O(new_n74_));
  nand3  g46(.a(x17), .b(new_n30_), .c(new_n29_), .O(new_n75_));
  inv1   g47(.a(new_n75_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n63_), .c(new_n74_), .O(new_n77_));
  inv1   g49(.a(new_n31_), .O(new_n78_));
  and2   g50(.a(x08), .b(x07), .O(new_n79_));
  aoi21  g51(.a(new_n36_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  oai21  g52(.a(new_n77_), .b(x18), .c(new_n80_), .O(z8));
endmodule


