// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x00), .O(new_n29_));
  inv1   g01(.a(x01), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  oai21  g06(.a(new_n30_), .b(new_n29_), .c(new_n34_), .O(new_n35_));
  nand3  g07(.a(x13), .b(x12), .c(x11), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand4  g09(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n37_), .c(x18), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n35_), .O(z0));
  nand3  g13(.a(x08), .b(new_n30_), .c(x00), .O(new_n42_));
  oai21  g14(.a(new_n35_), .b(x11), .c(new_n42_), .O(z1));
  nand3  g15(.a(x08), .b(x01), .c(new_n29_), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n35_), .c(new_n44_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand2  g19(.a(new_n36_), .b(new_n34_), .O(new_n48_));
  nor2   g20(.a(new_n30_), .b(new_n29_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n49_), .O(new_n50_));
  oai21  g22(.a(new_n48_), .b(new_n47_), .c(new_n50_), .O(z3));
  nor2   g23(.a(new_n37_), .b(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n34_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n49_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z4));
  nand2  g28(.a(x08), .b(x04), .O(new_n57_));
  inv1   g29(.a(x15), .O(new_n58_));
  nand2  g30(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  and2   g31(.a(x15), .b(x14), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n37_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n34_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n57_), .c(new_n49_), .O(z5));
  nand2  g35(.a(x08), .b(x05), .O(new_n64_));
  nand3  g36(.a(new_n60_), .b(new_n37_), .c(x16), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n34_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n64_), .c(new_n49_), .O(z6));
  nand4  g41(.a(new_n60_), .b(new_n37_), .c(x17), .d(x16), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  nand2  g43(.a(new_n65_), .b(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n34_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x06), .c(new_n49_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n73_), .O(z7));
  oai21  g47(.a(new_n38_), .b(new_n36_), .c(x18), .O(new_n76_));
  nor2   g48(.a(new_n71_), .b(new_n66_), .O(new_n77_));
  inv1   g49(.a(new_n53_), .O(new_n78_));
  nor2   g50(.a(x18), .b(new_n58_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n34_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  aoi21  g55(.a(new_n83_), .b(new_n82_), .c(new_n49_), .O(z8));
endmodule


