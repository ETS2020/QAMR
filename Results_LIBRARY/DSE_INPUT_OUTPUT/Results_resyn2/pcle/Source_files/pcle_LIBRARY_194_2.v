// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  inv1   g03(.a(x15), .O(new_n32_));
  nand4  g04(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x18), .c(x17), .d(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n31_), .O(z0));
  inv1   g08(.a(x09), .O(new_n37_));
  oai21  g09(.a(new_n37_), .b(x00), .c(x08), .O(new_n38_));
  oai21  g10(.a(new_n31_), .b(x11), .c(new_n38_), .O(z1));
  oai21  g11(.a(new_n37_), .b(x01), .c(x08), .O(new_n40_));
  xnor2a g12(.a(x12), .b(x11), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n31_), .c(new_n40_), .O(z2));
  oai21  g14(.a(new_n37_), .b(x02), .c(x08), .O(new_n43_));
  and2   g15(.a(x12), .b(x11), .O(new_n44_));
  nor2   g16(.a(new_n44_), .b(x13), .O(new_n45_));
  inv1   g17(.a(new_n31_), .O(new_n46_));
  nand2  g18(.a(new_n44_), .b(x13), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n45_), .c(new_n43_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand2  g23(.a(new_n47_), .b(new_n51_), .O(new_n52_));
  nor2   g24(.a(x10), .b(x08), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n52_), .c(new_n33_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n50_), .c(new_n37_), .O(z4));
  nand2  g27(.a(x08), .b(x04), .O(new_n56_));
  inv1   g28(.a(new_n34_), .O(new_n57_));
  nand2  g29(.a(new_n33_), .b(new_n32_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n53_), .c(new_n57_), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n56_), .c(new_n37_), .O(z5));
  oai21  g32(.a(new_n37_), .b(x05), .c(x08), .O(new_n61_));
  nor2   g33(.a(new_n34_), .b(x16), .O(new_n62_));
  and2   g34(.a(x14), .b(x13), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(new_n44_), .c(x16), .d(x15), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(z6));
  nand2  g38(.a(x08), .b(x06), .O(new_n67_));
  nand3  g39(.a(new_n34_), .b(x17), .c(x16), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  nand2  g41(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n53_), .c(new_n68_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n67_), .c(new_n37_), .O(z7));
  oai21  g44(.a(new_n37_), .b(x07), .c(x08), .O(new_n73_));
  inv1   g45(.a(x18), .O(new_n74_));
  nand2  g46(.a(new_n68_), .b(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n35_), .c(new_n46_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n73_), .O(z8));
endmodule


