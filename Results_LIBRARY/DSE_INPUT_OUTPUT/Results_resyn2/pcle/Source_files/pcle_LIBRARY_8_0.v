// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x16), .b(x15), .c(x14), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  oai21  g08(.a(new_n36_), .b(new_n31_), .c(x18), .O(new_n37_));
  nor2   g09(.a(x18), .b(x17), .O(new_n38_));
  inv1   g10(.a(new_n38_), .O(new_n39_));
  nand2  g11(.a(x18), .b(x17), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  and2   g13(.a(new_n41_), .b(new_n37_), .O(z0));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  inv1   g16(.a(new_n31_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n43_), .c(new_n38_), .O(z1));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  inv1   g21(.a(x12), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n48_), .c(new_n38_), .O(z2));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  inv1   g26(.a(x13), .O(new_n55_));
  nand2  g27(.a(new_n49_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n32_), .c(new_n45_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n38_), .O(z3));
  nand2  g30(.a(x08), .b(x03), .O(new_n59_));
  nand4  g31(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  inv1   g32(.a(x14), .O(new_n61_));
  nand2  g33(.a(new_n32_), .b(new_n61_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n45_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n59_), .c(new_n38_), .O(z4));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand3  g37(.a(new_n33_), .b(x15), .c(x14), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand2  g39(.a(new_n60_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n45_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n65_), .c(new_n38_), .O(z5));
  nand2  g42(.a(x08), .b(x05), .O(new_n71_));
  nor2   g43(.a(new_n34_), .b(new_n32_), .O(new_n72_));
  nor2   g44(.a(new_n72_), .b(new_n31_), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  oai21  g46(.a(new_n60_), .b(new_n67_), .c(new_n74_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n71_), .c(new_n38_), .O(z6));
  inv1   g49(.a(x17), .O(new_n78_));
  nand2  g50(.a(new_n37_), .b(new_n78_), .O(new_n79_));
  and2   g51(.a(x08), .b(x06), .O(new_n80_));
  aoi21  g52(.a(new_n73_), .b(x17), .c(new_n80_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(z7));
  nand2  g54(.a(new_n40_), .b(new_n36_), .O(new_n83_));
  aoi21  g55(.a(new_n72_), .b(x18), .c(new_n31_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g57(.a(new_n31_), .b(x18), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n78_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x07), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n85_), .O(z8));
endmodule


