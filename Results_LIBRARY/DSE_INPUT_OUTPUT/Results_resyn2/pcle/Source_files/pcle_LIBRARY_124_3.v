// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_;
  nand2  g00(.a(x18), .b(x17), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  nand2  g02(.a(x08), .b(x00), .O(new_n31_));
  inv1   g03(.a(x11), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n31_), .c(z0), .O(z1));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  nand2  g11(.a(x12), .b(x11), .O(new_n40_));
  inv1   g12(.a(x12), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n36_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(z0), .O(z2));
  nand2  g16(.a(x08), .b(x02), .O(new_n45_));
  inv1   g17(.a(x13), .O(new_n46_));
  nand2  g18(.a(new_n40_), .b(new_n46_), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n36_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(z0), .O(z3));
  nand2  g22(.a(x08), .b(x03), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(new_n48_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n36_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n51_), .c(z0), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nand2  g29(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  inv1   g30(.a(new_n52_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x15), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n36_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x04), .c(z0), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n52_), .b(new_n57_), .c(new_n64_), .O(new_n65_));
  nand2  g37(.a(x16), .b(x15), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n65_), .c(new_n36_), .O(new_n69_));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n69_), .c(z0), .O(z6));
  nand3  g43(.a(new_n29_), .b(x08), .c(x06), .O(new_n72_));
  nor2   g44(.a(new_n66_), .b(new_n52_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  oai22  g47(.a(new_n66_), .b(new_n52_), .c(x18), .d(new_n75_), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(new_n74_), .c(new_n36_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n72_), .O(z7));
  aoi21  g50(.a(new_n73_), .b(x17), .c(x18), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x07), .c(z0), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n35_), .c(new_n80_), .O(z8));
endmodule


