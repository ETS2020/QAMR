// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x16), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  and2   g07(.a(x15), .b(x14), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n35_), .c(new_n33_), .d(x17), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g10(.a(x18), .b(new_n29_), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n32_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n33_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n42_), .c(new_n39_), .O(z2));
  nand2  g18(.a(x08), .b(x02), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand2  g20(.a(new_n43_), .b(new_n48_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n34_), .c(new_n33_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n47_), .c(new_n39_), .O(z3));
  nor2   g23(.a(new_n35_), .b(x14), .O(new_n52_));
  nand2  g24(.a(new_n35_), .b(x14), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n39_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z4));
  aoi21  g28(.a(new_n35_), .b(x14), .c(x15), .O(new_n57_));
  nand2  g29(.a(new_n36_), .b(new_n35_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n33_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x04), .c(new_n39_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z5));
  nand3  g33(.a(new_n36_), .b(new_n35_), .c(x16), .O(new_n62_));
  nand2  g34(.a(new_n58_), .b(new_n29_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n33_), .O(new_n64_));
  aoi21  g36(.a(x08), .b(x05), .c(new_n39_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z6));
  inv1   g38(.a(new_n39_), .O(new_n67_));
  nand4  g39(.a(new_n36_), .b(new_n35_), .c(x17), .d(x16), .O(new_n68_));
  inv1   g40(.a(x17), .O(new_n69_));
  nand2  g41(.a(new_n62_), .b(new_n69_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(new_n33_), .O(new_n71_));
  nand3  g43(.a(new_n67_), .b(x08), .c(x06), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n71_), .O(z7));
  nand3  g45(.a(new_n68_), .b(new_n33_), .c(x18), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x07), .c(new_n39_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z8));
endmodule


