// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand2  g01(.a(x15), .b(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  and2   g06(.a(x17), .b(x16), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n34_), .c(x18), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n30_), .c(new_n29_), .O(z0));
  nor2   g09(.a(x18), .b(x00), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n33_), .b(x11), .c(new_n39_), .O(z1));
  nand2  g12(.a(x08), .b(x01), .O(new_n41_));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  or2    g14(.a(x12), .b(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n34_), .c(new_n42_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n41_), .c(new_n38_), .O(z2));
  aoi21  g17(.a(x12), .b(x11), .c(x13), .O(new_n46_));
  nand2  g18(.a(new_n34_), .b(new_n29_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x02), .c(new_n38_), .O(new_n48_));
  oai21  g20(.a(new_n47_), .b(new_n46_), .c(new_n48_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  inv1   g22(.a(new_n29_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x14), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(new_n29_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n34_), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n50_), .c(new_n38_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nor2   g29(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n52_), .b(new_n57_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n34_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x04), .c(new_n38_), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z5));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  inv1   g35(.a(new_n30_), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n51_), .c(x16), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  oai21  g38(.a(new_n30_), .b(new_n29_), .c(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n65_), .c(new_n34_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n63_), .c(new_n38_), .O(z6));
  nor2   g41(.a(new_n30_), .b(new_n29_), .O(new_n70_));
  aoi21  g42(.a(new_n35_), .b(new_n70_), .c(new_n33_), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g46(.a(x08), .b(x06), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n74_), .c(new_n38_), .O(z7));
  nand3  g48(.a(new_n35_), .b(new_n64_), .c(new_n51_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x07), .O(new_n78_));
  inv1   g50(.a(x18), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n77_), .c(new_n78_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(x00), .O(new_n82_));
  inv1   g54(.a(new_n78_), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n71_), .c(x18), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n82_), .O(z8));
endmodule


