// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  nand4  g02(.a(x18), .b(x17), .c(x16), .d(x15), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  inv1   g06(.a(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x10), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n34_), .c(new_n32_), .d(new_n30_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(z0));
  inv1   g10(.a(x11), .O(new_n39_));
  inv1   g11(.a(x15), .O(new_n40_));
  aoi21  g12(.a(new_n34_), .b(new_n39_), .c(new_n40_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  oai21  g14(.a(new_n41_), .b(x10), .c(new_n42_), .O(z1));
  oai21  g15(.a(x15), .b(x10), .c(x08), .O(new_n44_));
  inv1   g16(.a(new_n44_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g18(.a(x10), .O(new_n47_));
  nand3  g19(.a(new_n34_), .b(x15), .c(new_n47_), .O(new_n48_));
  xnor2a g20(.a(x12), .b(x11), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(z2));
  aoi21  g22(.a(x12), .b(x11), .c(x13), .O(new_n51_));
  nand2  g23(.a(new_n34_), .b(new_n29_), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n51_), .c(x15), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand2  g28(.a(new_n45_), .b(x03), .O(new_n57_));
  inv1   g29(.a(new_n48_), .O(new_n58_));
  nand2  g30(.a(new_n29_), .b(new_n35_), .O(new_n59_));
  nand4  g31(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n57_), .O(z4));
  aoi21  g34(.a(new_n60_), .b(new_n34_), .c(new_n40_), .O(new_n63_));
  nand2  g35(.a(x08), .b(x04), .O(new_n64_));
  oai21  g36(.a(new_n63_), .b(x10), .c(new_n64_), .O(z5));
  nand2  g37(.a(new_n45_), .b(x05), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nor2   g39(.a(new_n60_), .b(new_n67_), .O(new_n68_));
  nand2  g40(.a(new_n60_), .b(new_n67_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n68_), .c(new_n66_), .O(z6));
  inv1   g43(.a(x17), .O(new_n72_));
  nor3   g44(.a(new_n60_), .b(new_n72_), .c(new_n67_), .O(new_n73_));
  nand2  g45(.a(x16), .b(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n29_), .c(new_n72_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n34_), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n73_), .c(x15), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n47_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x06), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z7));
  nand2  g52(.a(new_n45_), .b(x07), .O(new_n81_));
  nand2  g53(.a(new_n73_), .b(x18), .O(new_n82_));
  inv1   g54(.a(x18), .O(new_n83_));
  nand2  g55(.a(new_n68_), .b(x17), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n82_), .c(new_n58_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n81_), .O(z8));
endmodule


