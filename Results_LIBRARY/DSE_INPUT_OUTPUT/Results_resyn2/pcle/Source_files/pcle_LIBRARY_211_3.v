// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x11), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x13), .b(x12), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(x16), .b(x15), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x14), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  aoi21  g10(.a(x11), .b(x10), .c(new_n31_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x00), .O(new_n40_));
  nand3  g12(.a(new_n29_), .b(x09), .c(new_n31_), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g14(.a(new_n39_), .b(x01), .O(new_n43_));
  xnor2a g15(.a(x12), .b(x11), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n41_), .c(new_n43_), .O(z2));
  nand2  g17(.a(new_n39_), .b(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  inv1   g20(.a(new_n48_), .O(new_n49_));
  nor2   g21(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  inv1   g22(.a(new_n50_), .O(new_n51_));
  oai21  g23(.a(new_n51_), .b(new_n47_), .c(new_n46_), .O(z3));
  oai21  g24(.a(new_n32_), .b(x14), .c(new_n29_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g26(.a(new_n50_), .b(x14), .O(new_n55_));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(z4));
  nand2  g29(.a(new_n39_), .b(x04), .O(new_n58_));
  inv1   g30(.a(x15), .O(new_n59_));
  nand4  g31(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n60_));
  nor2   g32(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  inv1   g33(.a(new_n41_), .O(new_n62_));
  nand2  g34(.a(new_n60_), .b(new_n59_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n61_), .c(new_n58_), .O(z5));
  nand2  g37(.a(new_n39_), .b(x05), .O(new_n66_));
  nand2  g38(.a(new_n61_), .b(x16), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  oai21  g40(.a(new_n60_), .b(new_n59_), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n62_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n66_), .O(z6));
  nand2  g43(.a(new_n39_), .b(x06), .O(new_n72_));
  inv1   g44(.a(new_n34_), .O(new_n73_));
  inv1   g45(.a(new_n60_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n73_), .c(x17), .O(new_n75_));
  nand3  g47(.a(x17), .b(x16), .c(x15), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n62_), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n75_), .c(new_n72_), .O(z7));
  nand2  g52(.a(new_n39_), .b(x07), .O(new_n81_));
  nand3  g53(.a(new_n77_), .b(new_n74_), .c(x18), .O(new_n82_));
  inv1   g54(.a(x18), .O(new_n83_));
  oai21  g55(.a(new_n76_), .b(new_n60_), .c(new_n83_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n82_), .c(new_n62_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n81_), .O(z8));
endmodule


