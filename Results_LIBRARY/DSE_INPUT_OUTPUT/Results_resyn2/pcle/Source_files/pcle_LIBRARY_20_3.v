// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(x10), .b(new_n33_), .O(new_n34_));
  and2   g06(.a(x13), .b(x12), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n34_), .c(new_n32_), .d(x18), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(new_n30_), .O(z0));
  oai21  g09(.a(x11), .b(x00), .c(x08), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(x09), .c(new_n29_), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(x11), .c(new_n38_), .O(z1));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  aoi21  g16(.a(x12), .b(new_n29_), .c(x11), .O(new_n45_));
  aoi22  g17(.a(new_n45_), .b(new_n44_), .c(new_n43_), .d(new_n29_), .O(z2));
  nand3  g18(.a(new_n30_), .b(x08), .c(x02), .O(new_n47_));
  inv1   g19(.a(new_n40_), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n42_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n47_), .O(z3));
  aoi22  g25(.a(new_n48_), .b(x14), .c(x08), .d(x03), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  oai21  g27(.a(new_n42_), .b(new_n50_), .c(new_n55_), .O(new_n56_));
  nand3  g28(.a(x14), .b(x13), .c(x12), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(new_n48_), .O(new_n58_));
  oai21  g30(.a(new_n54_), .b(x11), .c(new_n58_), .O(z4));
  oai21  g31(.a(x11), .b(x04), .c(x08), .O(new_n60_));
  nand4  g32(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  nand2  g33(.a(x15), .b(new_n29_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  inv1   g35(.a(new_n61_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x15), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n63_), .c(new_n34_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n60_), .O(z5));
  oai21  g39(.a(x11), .b(x05), .c(x08), .O(new_n68_));
  aoi22  g40(.a(new_n64_), .b(x15), .c(x16), .d(new_n29_), .O(new_n69_));
  nand2  g41(.a(x16), .b(x15), .O(new_n70_));
  oai21  g42(.a(new_n70_), .b(new_n61_), .c(new_n34_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(z6));
  inv1   g44(.a(x17), .O(new_n73_));
  nand2  g45(.a(x08), .b(x06), .O(new_n74_));
  oai21  g46(.a(new_n40_), .b(new_n73_), .c(new_n74_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n30_), .O(new_n76_));
  nor2   g48(.a(new_n70_), .b(new_n57_), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nor3   g50(.a(new_n70_), .b(new_n61_), .c(x17), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n78_), .c(new_n48_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n76_), .O(z7));
  oai21  g53(.a(x11), .b(x07), .c(x08), .O(new_n82_));
  inv1   g54(.a(new_n49_), .O(new_n83_));
  nand2  g55(.a(x18), .b(new_n29_), .O(new_n84_));
  aoi21  g56(.a(new_n83_), .b(new_n32_), .c(new_n84_), .O(new_n85_));
  inv1   g57(.a(x18), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(x17), .c(x16), .d(x15), .O(new_n87_));
  nor2   g59(.a(new_n87_), .b(new_n61_), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n85_), .c(new_n34_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n82_), .O(z8));
endmodule


