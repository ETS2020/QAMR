// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x18), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x16), .b(x15), .c(x14), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n32_), .c(new_n29_), .O(z0));
  nand3  g10(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n39_));
  oai21  g11(.a(new_n29_), .b(x08), .c(x00), .O(new_n40_));
  oai21  g12(.a(new_n39_), .b(x11), .c(new_n40_), .O(z1));
  inv1   g13(.a(x00), .O(new_n42_));
  nor2   g14(.a(x17), .b(new_n42_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  inv1   g16(.a(new_n39_), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  or2    g18(.a(x12), .b(x11), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n43_), .O(z2));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand2  g22(.a(new_n33_), .b(new_n45_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n43_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n54_));
  nand2  g26(.a(new_n34_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n33_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n55_), .c(new_n45_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n54_), .c(new_n43_), .O(z4));
  aoi21  g31(.a(new_n34_), .b(x14), .c(x15), .O(new_n60_));
  nand3  g32(.a(new_n34_), .b(x15), .c(x14), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n45_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n43_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(z5));
  inv1   g36(.a(x16), .O(new_n65_));
  nand2  g37(.a(new_n61_), .b(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n37_), .c(new_n45_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x05), .c(new_n43_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(z6));
  nor2   g41(.a(new_n35_), .b(new_n33_), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n45_), .c(x00), .O(new_n71_));
  and2   g43(.a(x08), .b(x06), .O(new_n72_));
  nand4  g44(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n73_));
  inv1   g45(.a(new_n73_), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n37_), .c(new_n72_), .O(new_n75_));
  oai21  g47(.a(new_n71_), .b(x17), .c(new_n75_), .O(z7));
  inv1   g48(.a(x18), .O(new_n77_));
  nand2  g49(.a(new_n37_), .b(new_n77_), .O(new_n78_));
  aoi21  g50(.a(new_n70_), .b(x18), .c(new_n73_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g52(.a(new_n32_), .b(new_n42_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z8));
endmodule


