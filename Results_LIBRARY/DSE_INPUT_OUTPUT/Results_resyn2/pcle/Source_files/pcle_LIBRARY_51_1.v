// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x15), .O(new_n36_));
  inv1   g08(.a(x16), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n35_), .c(new_n33_), .d(x17), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x01), .c(new_n29_), .O(z0));
  nor2   g12(.a(new_n29_), .b(x01), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n32_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  or2    g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n33_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n41_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  nand2  g20(.a(x08), .b(x02), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand2  g22(.a(new_n44_), .b(new_n50_), .O(new_n51_));
  nand3  g23(.a(x13), .b(x12), .c(x11), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(new_n51_), .c(new_n33_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n49_), .c(new_n41_), .O(z3));
  inv1   g26(.a(x14), .O(new_n55_));
  aoi21  g27(.a(new_n52_), .b(new_n55_), .c(new_n32_), .O(new_n56_));
  oai21  g28(.a(new_n52_), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x03), .c(new_n41_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n57_), .O(z4));
  nand2  g31(.a(x08), .b(x04), .O(new_n60_));
  nand2  g32(.a(new_n35_), .b(x15), .O(new_n61_));
  nand2  g33(.a(new_n34_), .b(new_n36_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n61_), .c(new_n33_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n60_), .c(new_n41_), .O(z5));
  nor2   g36(.a(new_n34_), .b(new_n36_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x16), .O(new_n66_));
  nand2  g38(.a(new_n61_), .b(new_n37_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n33_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n41_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z6));
  aoi21  g42(.a(new_n65_), .b(x16), .c(x17), .O(new_n71_));
  nand3  g43(.a(new_n38_), .b(new_n35_), .c(x17), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n33_), .O(new_n73_));
  aoi21  g45(.a(x08), .b(x06), .c(new_n41_), .O(new_n74_));
  oai21  g46(.a(new_n73_), .b(new_n71_), .c(new_n74_), .O(z7));
  nand4  g47(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n76_));
  and2   g48(.a(x18), .b(x01), .O(new_n77_));
  oai21  g49(.a(new_n76_), .b(new_n52_), .c(new_n77_), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nor2   g51(.a(x18), .b(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n38_), .c(new_n35_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n33_), .O(new_n83_));
  inv1   g55(.a(new_n41_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(x08), .c(x07), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n83_), .O(z8));
endmodule


