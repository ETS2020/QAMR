// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x03), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  nand4  g04(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n33_));
  or2    g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  nand3  g07(.a(new_n29_), .b(x09), .c(new_n35_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(x18), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(new_n34_), .c(new_n31_), .O(z0));
  aoi21  g11(.a(x08), .b(x00), .c(new_n30_), .O(new_n40_));
  oai21  g12(.a(new_n36_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  or2    g14(.a(x12), .b(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n37_), .c(new_n42_), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n30_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z2));
  nor2   g18(.a(new_n30_), .b(new_n35_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x02), .O(new_n48_));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand2  g21(.a(new_n37_), .b(new_n32_), .O(new_n50_));
  oai21  g22(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  nand2  g24(.a(new_n32_), .b(new_n52_), .O(new_n53_));
  nand4  g25(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n53_), .c(new_n37_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x03), .c(new_n30_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n55_), .O(z4));
  inv1   g29(.a(new_n54_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x15), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  nand2  g32(.a(new_n54_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n37_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n30_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z5));
  nand2  g36(.a(new_n47_), .b(x05), .O(new_n65_));
  inv1   g37(.a(x16), .O(new_n66_));
  nand2  g38(.a(new_n59_), .b(new_n66_), .O(new_n67_));
  nor2   g39(.a(new_n66_), .b(new_n60_), .O(new_n68_));
  nand2  g40(.a(new_n58_), .b(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n37_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n65_), .O(z6));
  nand2  g43(.a(new_n47_), .b(x06), .O(new_n72_));
  nand3  g44(.a(new_n58_), .b(new_n68_), .c(x17), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nand2  g46(.a(new_n69_), .b(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n73_), .c(new_n37_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n72_), .O(z7));
  oai21  g49(.a(new_n33_), .b(new_n32_), .c(x18), .O(new_n78_));
  nor2   g50(.a(x18), .b(new_n74_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n58_), .c(new_n68_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n37_), .O(new_n82_));
  aoi21  g54(.a(x08), .b(x07), .c(new_n30_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


