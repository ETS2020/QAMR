// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand4  g03(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n31_), .c(x18), .d(x09), .O(new_n34_));
  aoi21  g06(.a(new_n34_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g07(.a(x10), .O(new_n36_));
  oai21  g08(.a(new_n36_), .b(x00), .c(x08), .O(new_n37_));
  nand2  g09(.a(new_n36_), .b(x09), .O(new_n38_));
  oai21  g10(.a(new_n38_), .b(x11), .c(new_n37_), .O(z1));
  oai21  g11(.a(new_n36_), .b(x01), .c(x08), .O(new_n40_));
  nor2   g12(.a(x12), .b(x11), .O(new_n41_));
  and2   g13(.a(x12), .b(x11), .O(new_n42_));
  or2    g14(.a(new_n38_), .b(new_n42_), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z2));
  oai21  g16(.a(new_n36_), .b(x02), .c(x08), .O(new_n45_));
  nor2   g17(.a(new_n42_), .b(x13), .O(new_n46_));
  inv1   g18(.a(new_n38_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z3));
  nor2   g21(.a(new_n31_), .b(x14), .O(new_n50_));
  nand2  g22(.a(new_n31_), .b(x14), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(new_n47_), .c(new_n29_), .O(new_n52_));
  nand3  g24(.a(x10), .b(x08), .c(x03), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z4));
  oai21  g26(.a(new_n36_), .b(x04), .c(x08), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  nand2  g28(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x15), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n57_), .c(new_n47_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n55_), .O(z5));
  oai21  g34(.a(new_n36_), .b(x05), .c(x08), .O(new_n63_));
  nand3  g35(.a(new_n59_), .b(x16), .c(x15), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  nand2  g37(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n47_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n63_), .O(z6));
  oai21  g40(.a(new_n36_), .b(x06), .c(x08), .O(new_n69_));
  nor2   g41(.a(new_n65_), .b(new_n56_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n59_), .c(x17), .O(new_n71_));
  inv1   g43(.a(x17), .O(new_n72_));
  nand2  g44(.a(new_n70_), .b(new_n59_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n71_), .c(new_n47_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n69_), .O(z7));
  oai21  g48(.a(new_n36_), .b(x07), .c(x08), .O(new_n77_));
  oai21  g49(.a(new_n32_), .b(new_n30_), .c(x18), .O(new_n78_));
  nor2   g50(.a(x18), .b(new_n72_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n70_), .c(new_n59_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n47_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n77_), .O(z8));
endmodule


