// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand4  g02(.a(x18), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x17), .b(x16), .c(x15), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n31_), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  nand3  g10(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n39_));
  oai21  g11(.a(x18), .b(new_n30_), .c(x08), .O(new_n40_));
  oai22  g12(.a(new_n40_), .b(new_n38_), .c(new_n39_), .d(x11), .O(z1));
  inv1   g13(.a(new_n39_), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nor2   g17(.a(x18), .b(new_n30_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x01), .c(new_n46_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n45_), .O(z2));
  aoi21  g20(.a(x12), .b(x11), .c(x13), .O(new_n49_));
  nand3  g21(.a(x13), .b(x12), .c(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n42_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n46_), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z3));
  inv1   g25(.a(new_n50_), .O(new_n54_));
  nor2   g26(.a(new_n54_), .b(x14), .O(new_n55_));
  nand2  g27(.a(new_n32_), .b(new_n42_), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x03), .c(new_n46_), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(z4));
  inv1   g30(.a(new_n40_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nor2   g33(.a(new_n32_), .b(new_n61_), .O(new_n62_));
  oai21  g34(.a(new_n33_), .b(x15), .c(new_n42_), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(z5));
  nand2  g36(.a(new_n59_), .b(x05), .O(new_n65_));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  nor2   g38(.a(new_n66_), .b(new_n50_), .O(new_n67_));
  nand3  g39(.a(x16), .b(x15), .c(x14), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n54_), .c(new_n39_), .O(new_n70_));
  oai21  g42(.a(new_n67_), .b(x16), .c(new_n70_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n65_), .O(z6));
  aoi21  g44(.a(new_n69_), .b(new_n54_), .c(x17), .O(new_n73_));
  nand2  g45(.a(new_n36_), .b(new_n42_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x06), .c(new_n46_), .O(new_n75_));
  oai21  g47(.a(new_n74_), .b(new_n73_), .c(new_n75_), .O(z7));
  inv1   g48(.a(x18), .O(new_n77_));
  inv1   g49(.a(x09), .O(new_n78_));
  nor2   g50(.a(new_n78_), .b(x08), .O(new_n79_));
  nand3  g51(.a(new_n35_), .b(new_n33_), .c(new_n79_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n30_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  inv1   g54(.a(new_n31_), .O(new_n83_));
  aoi22  g55(.a(new_n36_), .b(new_n83_), .c(x08), .d(x07), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n82_), .O(z8));
endmodule


