// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g07(.a(x11), .O(new_n36_));
  inv1   g08(.a(new_n34_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  inv1   g10(.a(x09), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x07), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(new_n40_), .c(new_n38_), .O(z1));
  inv1   g14(.a(x01), .O(new_n43_));
  nand2  g15(.a(new_n40_), .b(x08), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai22  g17(.a(new_n45_), .b(new_n34_), .c(new_n44_), .d(new_n43_), .O(z2));
  inv1   g18(.a(x02), .O(new_n47_));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand2  g20(.a(new_n37_), .b(new_n30_), .O(new_n49_));
  oai22  g21(.a(new_n49_), .b(new_n48_), .c(new_n44_), .d(new_n47_), .O(z3));
  inv1   g22(.a(x03), .O(new_n51_));
  nand4  g23(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand2  g25(.a(new_n30_), .b(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n52_), .c(new_n37_), .O(new_n55_));
  oai21  g27(.a(new_n44_), .b(new_n51_), .c(new_n55_), .O(z4));
  inv1   g28(.a(x15), .O(new_n57_));
  nand2  g29(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  inv1   g30(.a(new_n30_), .O(new_n59_));
  nor2   g31(.a(new_n57_), .b(new_n53_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n58_), .c(new_n37_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x04), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n62_), .c(new_n40_), .O(z5));
  inv1   g36(.a(new_n44_), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x05), .O(new_n66_));
  nand3  g38(.a(new_n60_), .b(new_n59_), .c(x16), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  nand2  g40(.a(new_n61_), .b(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n67_), .c(new_n37_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n66_), .O(z6));
  nand2  g43(.a(new_n65_), .b(x06), .O(new_n72_));
  nand4  g44(.a(new_n60_), .b(new_n59_), .c(x17), .d(x16), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nand2  g46(.a(new_n67_), .b(new_n74_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n73_), .c(new_n37_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n72_), .O(z7));
  oai21  g49(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n78_));
  nor2   g50(.a(new_n74_), .b(new_n68_), .O(new_n79_));
  inv1   g51(.a(new_n52_), .O(new_n80_));
  nor2   g52(.a(x18), .b(new_n57_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nor2   g55(.a(x10), .b(x08), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n85_), .c(new_n39_), .O(z8));
endmodule


