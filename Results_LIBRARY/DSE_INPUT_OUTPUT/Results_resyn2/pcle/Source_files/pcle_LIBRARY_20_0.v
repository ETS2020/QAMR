// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  nand3  g00(.a(x14), .b(x13), .c(x12), .O(new_n29_));
  nand2  g01(.a(x16), .b(x15), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand2  g04(.a(new_n32_), .b(x09), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(new_n31_), .c(x18), .d(x17), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(x11), .c(x08), .O(z0));
  inv1   g08(.a(x11), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(x08), .O(new_n38_));
  inv1   g10(.a(x08), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(x00), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(new_n38_), .O(z1));
  oai21  g13(.a(new_n33_), .b(x12), .c(x11), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  nand2  g18(.a(new_n38_), .b(new_n34_), .O(new_n47_));
  xnor2a g19(.a(x13), .b(x12), .O(new_n48_));
  oai21  g20(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(z3));
  nand2  g21(.a(x08), .b(x03), .O(new_n50_));
  aoi21  g22(.a(x13), .b(x12), .c(x14), .O(new_n51_));
  inv1   g23(.a(new_n47_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n29_), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(z4));
  inv1   g26(.a(x15), .O(new_n55_));
  nand2  g27(.a(new_n29_), .b(new_n55_), .O(new_n56_));
  inv1   g28(.a(new_n29_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x15), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(new_n56_), .c(new_n34_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x11), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n39_), .O(new_n61_));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  nand2  g36(.a(new_n58_), .b(new_n64_), .O(new_n65_));
  nor2   g37(.a(new_n33_), .b(new_n31_), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n65_), .c(new_n37_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x05), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(x08), .c(new_n68_), .O(z6));
  nand2  g41(.a(x08), .b(x06), .O(new_n70_));
  nand2  g42(.a(new_n31_), .b(x17), .O(new_n71_));
  or2    g43(.a(new_n31_), .b(x17), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n71_), .c(new_n52_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n70_), .O(z7));
  inv1   g46(.a(new_n30_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n57_), .c(x18), .d(x17), .O(new_n76_));
  inv1   g48(.a(x18), .O(new_n77_));
  nand3  g49(.a(x17), .b(x16), .c(x15), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n29_), .c(new_n77_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n76_), .c(new_n34_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x11), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n39_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z8));
endmodule


