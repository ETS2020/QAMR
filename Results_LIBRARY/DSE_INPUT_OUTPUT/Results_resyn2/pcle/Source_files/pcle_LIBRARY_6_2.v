// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x11), .O(new_n34_));
  nand4  g06(.a(x16), .b(x14), .c(x13), .d(x12), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g10(.a(x18), .b(new_n29_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  nand3  g12(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n45_), .c(new_n39_), .O(z2));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n51_), .c(new_n42_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n39_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  inv1   g29(.a(new_n51_), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n51_), .b(new_n60_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n59_), .c(new_n42_), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n57_), .c(new_n39_), .O(z4));
  nand4  g35(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  nor2   g36(.a(new_n64_), .b(new_n29_), .O(new_n65_));
  nand2  g37(.a(new_n64_), .b(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n42_), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n39_), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(new_n65_), .c(new_n68_), .O(z5));
  inv1   g41(.a(new_n39_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(x08), .c(x05), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand2  g44(.a(new_n64_), .b(x18), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(x15), .c(new_n72_), .O(new_n74_));
  nand4  g46(.a(x18), .b(new_n72_), .c(x15), .d(x14), .O(new_n75_));
  nor2   g47(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n74_), .c(new_n42_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n71_), .O(z6));
  inv1   g50(.a(x18), .O(new_n79_));
  inv1   g51(.a(x17), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x11), .O(new_n81_));
  nor2   g53(.a(new_n81_), .b(new_n35_), .O(new_n82_));
  aoi21  g54(.a(new_n82_), .b(new_n42_), .c(new_n79_), .O(new_n83_));
  nand4  g55(.a(new_n58_), .b(x16), .c(x15), .d(x14), .O(new_n84_));
  and2   g56(.a(x08), .b(x06), .O(new_n85_));
  aoi21  g57(.a(new_n84_), .b(new_n33_), .c(new_n85_), .O(new_n86_));
  oai21  g58(.a(new_n83_), .b(new_n29_), .c(new_n86_), .O(z7));
  nand3  g59(.a(new_n70_), .b(x08), .c(x07), .O(new_n88_));
  nor2   g60(.a(new_n84_), .b(new_n80_), .O(new_n89_));
  nand2  g61(.a(new_n42_), .b(x18), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(z8));
endmodule


