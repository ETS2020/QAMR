// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nand4  g02(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  inv1   g06(.a(x16), .O(new_n35_));
  inv1   g07(.a(x17), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n34_), .c(new_n32_), .d(x15), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  inv1   g11(.a(x08), .O(new_n40_));
  nand3  g12(.a(new_n29_), .b(x09), .c(new_n40_), .O(new_n41_));
  nand2  g13(.a(x18), .b(x10), .O(new_n42_));
  inv1   g14(.a(new_n42_), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x00), .c(new_n43_), .O(new_n44_));
  oai21  g16(.a(new_n41_), .b(x11), .c(new_n44_), .O(z1));
  nand3  g17(.a(new_n42_), .b(x08), .c(x01), .O(new_n46_));
  xnor2a g18(.a(x12), .b(x11), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n41_), .c(new_n46_), .O(z2));
  nand3  g20(.a(new_n42_), .b(x08), .c(x02), .O(new_n49_));
  aoi21  g21(.a(x12), .b(x11), .c(x13), .O(new_n50_));
  nand3  g22(.a(x13), .b(x12), .c(x11), .O(new_n51_));
  inv1   g23(.a(new_n41_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n50_), .c(new_n49_), .O(z3));
  inv1   g26(.a(new_n51_), .O(new_n55_));
  nor2   g27(.a(new_n55_), .b(x14), .O(new_n56_));
  nand2  g28(.a(new_n52_), .b(new_n31_), .O(new_n57_));
  aoi21  g29(.a(x08), .b(x03), .c(new_n43_), .O(new_n58_));
  oai21  g30(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(z4));
  nand3  g31(.a(new_n42_), .b(x08), .c(x04), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nor2   g33(.a(new_n31_), .b(new_n61_), .O(new_n62_));
  nand2  g34(.a(new_n31_), .b(new_n61_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(z5));
  nand2  g37(.a(x15), .b(x14), .O(new_n66_));
  nor3   g38(.a(new_n66_), .b(new_n51_), .c(new_n35_), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(new_n51_), .c(new_n35_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n43_), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n67_), .c(new_n70_), .O(z6));
  nand3  g43(.a(new_n42_), .b(x08), .c(x06), .O(new_n72_));
  xnor2a g44(.a(x17), .b(x16), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n32_), .c(x15), .O(new_n74_));
  oai21  g46(.a(new_n66_), .b(new_n51_), .c(new_n36_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n52_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n72_), .O(z7));
  nand4  g49(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n78_));
  inv1   g50(.a(new_n78_), .O(new_n79_));
  nand2  g51(.a(new_n30_), .b(new_n29_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(new_n55_), .O(new_n81_));
  oai21  g53(.a(new_n78_), .b(new_n51_), .c(new_n30_), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n34_), .O(new_n83_));
  aoi21  g55(.a(x08), .b(x07), .c(new_n43_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n83_), .O(z8));
endmodule


