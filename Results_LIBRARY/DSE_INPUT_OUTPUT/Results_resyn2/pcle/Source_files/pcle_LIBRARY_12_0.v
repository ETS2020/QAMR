// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand3  g02(.a(new_n30_), .b(x09), .c(new_n29_), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand3  g04(.a(x13), .b(x12), .c(x11), .O(new_n33_));
  nand3  g05(.a(x16), .b(x15), .c(x14), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n32_), .c(x18), .d(x17), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(z0));
  nor2   g09(.a(x18), .b(x17), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand2  g12(.a(new_n32_), .b(new_n40_), .O(new_n41_));
  aoi21  g13(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z1));
  xnor2a g14(.a(x12), .b(x11), .O(new_n43_));
  aoi21  g15(.a(x08), .b(x01), .c(new_n38_), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(new_n31_), .c(new_n44_), .O(z2));
  nand2  g17(.a(x08), .b(x02), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n33_), .c(new_n32_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n46_), .c(new_n38_), .O(z3));
  nand2  g23(.a(x08), .b(x03), .O(new_n52_));
  inv1   g24(.a(new_n33_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x14), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  nand2  g27(.a(new_n33_), .b(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n54_), .c(new_n32_), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n52_), .c(new_n38_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  nor2   g32(.a(new_n59_), .b(new_n55_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n53_), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(new_n60_), .c(new_n32_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(new_n38_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z5));
  nand3  g37(.a(new_n61_), .b(new_n53_), .c(x16), .O(new_n66_));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(new_n62_), .b(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n66_), .c(new_n32_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x05), .c(new_n38_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n69_), .O(z6));
  inv1   g43(.a(x18), .O(new_n72_));
  aoi21  g44(.a(new_n35_), .b(new_n32_), .c(new_n72_), .O(new_n73_));
  inv1   g45(.a(new_n34_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  and2   g47(.a(x08), .b(x06), .O(new_n76_));
  nand4  g48(.a(x17), .b(new_n30_), .c(x09), .d(new_n29_), .O(new_n77_));
  inv1   g49(.a(new_n77_), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n75_), .c(new_n76_), .O(new_n79_));
  oai21  g51(.a(new_n73_), .b(x17), .c(new_n79_), .O(z7));
  inv1   g52(.a(x17), .O(new_n81_));
  oai22  g53(.a(new_n34_), .b(new_n33_), .c(new_n72_), .d(new_n81_), .O(new_n82_));
  nand2  g54(.a(new_n35_), .b(x18), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n32_), .O(new_n84_));
  oai21  g56(.a(new_n32_), .b(new_n72_), .c(new_n81_), .O(new_n85_));
  nand2  g57(.a(x08), .b(x07), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(z8));
endmodule


