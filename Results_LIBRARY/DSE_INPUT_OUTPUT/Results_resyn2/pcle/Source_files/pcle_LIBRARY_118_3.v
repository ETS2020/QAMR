// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_;
  inv1   g00(.a(x15), .O(new_n29_));
  inv1   g01(.a(x18), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  nand2  g03(.a(x08), .b(x00), .O(new_n32_));
  inv1   g04(.a(x11), .O(new_n33_));
  inv1   g05(.a(x10), .O(new_n34_));
  inv1   g06(.a(x09), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(x08), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n33_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n32_), .c(z0), .O(z1));
  nand2  g12(.a(x08), .b(x01), .O(new_n41_));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  inv1   g14(.a(x12), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n41_), .c(z0), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n38_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(z0), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g23(.a(new_n48_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand4  g25(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n38_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x03), .c(z0), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(z4));
  nor2   g29(.a(new_n54_), .b(new_n29_), .O(new_n58_));
  nand2  g30(.a(new_n54_), .b(new_n29_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n38_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x04), .c(z0), .O(new_n61_));
  oai21  g33(.a(new_n60_), .b(new_n58_), .c(new_n61_), .O(z5));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n54_), .b(new_n29_), .c(new_n64_), .O(new_n65_));
  nand2  g37(.a(new_n58_), .b(x16), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n65_), .c(new_n38_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n63_), .c(z0), .O(z6));
  nand3  g40(.a(new_n66_), .b(new_n38_), .c(x17), .O(new_n69_));
  nand2  g41(.a(x16), .b(x14), .O(new_n70_));
  nor2   g42(.a(new_n70_), .b(x17), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n52_), .c(new_n36_), .d(new_n34_), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(new_n30_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x15), .O(new_n74_));
  nand2  g46(.a(x08), .b(x06), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z7));
  nand4  g48(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n77_));
  inv1   g49(.a(new_n77_), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n52_), .c(x18), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x07), .c(z0), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n37_), .c(new_n80_), .O(z8));
endmodule


