// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  inv1   g00(.a(x16), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n33_), .c(x17), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x18), .c(new_n29_), .O(z0));
  nor2   g09(.a(x18), .b(new_n29_), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand3  g12(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n41_));
  inv1   g13(.a(new_n41_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n39_), .c(new_n38_), .O(z1));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n38_), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(new_n41_), .c(new_n46_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n42_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n38_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(new_n49_), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(x14), .O(new_n54_));
  nand4  g26(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n42_), .O(new_n56_));
  aoi21  g28(.a(x08), .b(x03), .c(new_n38_), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n54_), .c(new_n57_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nor2   g31(.a(new_n55_), .b(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n55_), .b(new_n59_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  aoi21  g34(.a(x08), .b(x04), .c(new_n38_), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(new_n60_), .c(new_n63_), .O(z5));
  nand2  g36(.a(x15), .b(x14), .O(new_n65_));
  oai21  g37(.a(new_n49_), .b(new_n65_), .c(new_n29_), .O(new_n66_));
  nand3  g38(.a(x16), .b(x15), .c(x14), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n66_), .c(new_n42_), .O(new_n70_));
  aoi21  g42(.a(x08), .b(x05), .c(new_n38_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z6));
  inv1   g44(.a(x18), .O(new_n73_));
  nor2   g45(.a(new_n34_), .b(x17), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n33_), .c(new_n73_), .O(new_n75_));
  nand3  g47(.a(new_n68_), .b(new_n53_), .c(x18), .O(new_n76_));
  nand4  g48(.a(x17), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n77_));
  inv1   g49(.a(new_n77_), .O(new_n78_));
  and2   g50(.a(x08), .b(x06), .O(new_n79_));
  aoi21  g51(.a(new_n78_), .b(new_n76_), .c(new_n79_), .O(new_n80_));
  oai21  g52(.a(new_n75_), .b(new_n29_), .c(new_n80_), .O(z7));
  nand3  g53(.a(new_n68_), .b(new_n53_), .c(x17), .O(new_n82_));
  nand4  g54(.a(x18), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n83_));
  inv1   g55(.a(new_n83_), .O(new_n84_));
  and2   g56(.a(x08), .b(x07), .O(new_n85_));
  aoi21  g57(.a(new_n84_), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  oai21  g58(.a(new_n75_), .b(new_n29_), .c(new_n86_), .O(z8));
endmodule


