// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(x13), .b(x12), .c(x11), .O(new_n34_));
  nand3  g06(.a(x17), .b(x16), .c(x15), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x14), .c(new_n29_), .O(z0));
  nor2   g10(.a(new_n29_), .b(x14), .O(new_n39_));
  aoi21  g11(.a(x08), .b(x00), .c(new_n39_), .O(new_n40_));
  oai21  g12(.a(new_n32_), .b(x11), .c(new_n40_), .O(z1));
  nand2  g13(.a(x08), .b(x01), .O(new_n42_));
  nand2  g14(.a(x12), .b(x11), .O(new_n43_));
  or2    g15(.a(x12), .b(x11), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n43_), .c(new_n33_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n42_), .c(new_n39_), .O(z2));
  nand2  g18(.a(x08), .b(x02), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand2  g20(.a(new_n43_), .b(new_n48_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n34_), .c(new_n33_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n47_), .c(new_n39_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  aoi21  g24(.a(new_n29_), .b(new_n52_), .c(new_n34_), .O(new_n53_));
  inv1   g25(.a(new_n34_), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(x14), .c(new_n33_), .O(new_n55_));
  inv1   g27(.a(new_n39_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x08), .c(x03), .O(new_n57_));
  oai21  g29(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(z4));
  inv1   g30(.a(x15), .O(new_n59_));
  nand2  g31(.a(new_n34_), .b(new_n59_), .O(new_n60_));
  nand4  g32(.a(x15), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n60_), .c(new_n33_), .d(x14), .O(new_n62_));
  oai21  g34(.a(new_n32_), .b(new_n59_), .c(new_n29_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n52_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(z5));
  inv1   g38(.a(new_n61_), .O(new_n67_));
  nand4  g39(.a(x16), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x05), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n56_), .O(new_n71_));
  nand3  g43(.a(new_n29_), .b(x16), .c(new_n52_), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n61_), .c(new_n72_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n33_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n71_), .O(z6));
  and2   g49(.a(x16), .b(x15), .O(new_n78_));
  inv1   g50(.a(x17), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(x14), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n54_), .O(new_n81_));
  nand2  g53(.a(new_n78_), .b(new_n54_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n81_), .c(new_n33_), .O(new_n84_));
  oai21  g56(.a(new_n32_), .b(new_n79_), .c(new_n29_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x06), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n84_), .O(z7));
  oai21  g60(.a(new_n35_), .b(new_n34_), .c(x18), .O(new_n89_));
  and2   g61(.a(x17), .b(x14), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n78_), .c(new_n54_), .d(new_n29_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(new_n33_), .O(new_n93_));
  aoi21  g65(.a(x08), .b(x07), .c(new_n39_), .O(new_n94_));
  nand2  g66(.a(new_n94_), .b(new_n93_), .O(z8));
endmodule


