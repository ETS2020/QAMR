// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_;
  nor2   g00(.a(x18), .b(x12), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x15), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(x09), .c(new_n34_), .O(new_n36_));
  inv1   g08(.a(new_n36_), .O(new_n37_));
  inv1   g09(.a(x16), .O(new_n38_));
  inv1   g10(.a(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n37_), .c(new_n33_), .d(x18), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n30_), .O(z0));
  aoi21  g14(.a(x08), .b(x00), .c(new_n29_), .O(new_n43_));
  oai21  g15(.a(new_n36_), .b(x11), .c(new_n43_), .O(z1));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x01), .c(new_n29_), .O(new_n46_));
  oai21  g18(.a(new_n45_), .b(new_n36_), .c(new_n46_), .O(z2));
  aoi21  g19(.a(x12), .b(x11), .c(x13), .O(new_n48_));
  nand3  g20(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  nand2  g21(.a(new_n37_), .b(new_n49_), .O(new_n50_));
  aoi21  g22(.a(x08), .b(x02), .c(new_n29_), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x12), .O(new_n53_));
  inv1   g25(.a(x14), .O(new_n54_));
  oai21  g26(.a(new_n36_), .b(new_n54_), .c(x18), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand2  g29(.a(new_n49_), .b(new_n54_), .O(new_n58_));
  nand3  g30(.a(x14), .b(x13), .c(x11), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(new_n37_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n57_), .c(new_n56_), .O(z4));
  nand2  g33(.a(new_n32_), .b(new_n31_), .O(new_n62_));
  inv1   g34(.a(new_n59_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(new_n62_), .c(new_n37_), .O(new_n65_));
  oai21  g37(.a(new_n36_), .b(new_n31_), .c(x18), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n53_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x04), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(z5));
  nand3  g41(.a(new_n30_), .b(x08), .c(x05), .O(new_n70_));
  nand2  g42(.a(new_n33_), .b(x16), .O(new_n71_));
  nor2   g43(.a(new_n36_), .b(new_n29_), .O(new_n72_));
  oai21  g44(.a(new_n32_), .b(new_n31_), .c(new_n38_), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n70_), .O(z6));
  nand3  g47(.a(new_n30_), .b(x08), .c(x06), .O(new_n76_));
  and2   g48(.a(x16), .b(x15), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n63_), .c(x17), .d(x12), .O(new_n78_));
  nand3  g50(.a(new_n77_), .b(new_n63_), .c(x12), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n39_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n72_), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n76_), .O(z7));
  nand4  g54(.a(new_n77_), .b(new_n63_), .c(x18), .d(x17), .O(new_n83_));
  inv1   g55(.a(x18), .O(new_n84_));
  nand3  g56(.a(new_n77_), .b(new_n63_), .c(x17), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g58(.a(new_n86_), .b(new_n83_), .c(new_n37_), .O(new_n87_));
  nand2  g59(.a(new_n36_), .b(x18), .O(new_n88_));
  aoi22  g60(.a(new_n88_), .b(new_n53_), .c(x08), .d(x07), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n87_), .O(z8));
endmodule


