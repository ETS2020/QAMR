// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_;
  nand3  g00(.a(x13), .b(x12), .c(x11), .O(new_n29_));
  nand4  g01(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  inv1   g08(.a(x00), .O(new_n37_));
  nor2   g09(.a(x01), .b(new_n37_), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n35_), .c(new_n31_), .O(new_n40_));
  inv1   g12(.a(new_n40_), .O(z0));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand2  g15(.a(new_n35_), .b(new_n43_), .O(new_n44_));
  aoi21  g16(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(z1));
  aoi21  g17(.a(new_n32_), .b(new_n37_), .c(x01), .O(new_n46_));
  nand2  g18(.a(x12), .b(x11), .O(new_n47_));
  inv1   g19(.a(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n43_), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n47_), .c(new_n33_), .d(x09), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n32_), .c(new_n46_), .O(z2));
  aoi21  g23(.a(x12), .b(x11), .c(x13), .O(new_n52_));
  nand2  g24(.a(new_n35_), .b(new_n29_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n38_), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n52_), .c(new_n54_), .O(z3));
  inv1   g27(.a(new_n29_), .O(new_n56_));
  nor2   g28(.a(new_n56_), .b(x14), .O(new_n57_));
  nand4  g29(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n35_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n38_), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(new_n57_), .c(new_n60_), .O(z4));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  inv1   g34(.a(new_n58_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x15), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand2  g37(.a(new_n58_), .b(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n64_), .c(new_n35_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n62_), .c(new_n38_), .O(z5));
  inv1   g40(.a(x16), .O(new_n69_));
  nand2  g41(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  nor2   g42(.a(new_n69_), .b(new_n65_), .O(new_n71_));
  nand2  g43(.a(new_n63_), .b(new_n71_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n70_), .c(new_n35_), .O(new_n73_));
  nand2  g45(.a(x08), .b(x05), .O(new_n74_));
  aoi21  g46(.a(new_n74_), .b(new_n73_), .c(new_n38_), .O(z6));
  inv1   g47(.a(x17), .O(new_n76_));
  nor2   g48(.a(new_n72_), .b(new_n76_), .O(new_n77_));
  nand2  g49(.a(new_n72_), .b(new_n76_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n35_), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x06), .c(new_n38_), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(new_n77_), .c(new_n80_), .O(z7));
  oai21  g53(.a(new_n30_), .b(new_n29_), .c(x18), .O(new_n82_));
  nor2   g54(.a(x18), .b(new_n76_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n63_), .c(new_n71_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n35_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x07), .O(new_n87_));
  aoi21  g59(.a(new_n87_), .b(new_n86_), .c(new_n38_), .O(z8));
endmodule


