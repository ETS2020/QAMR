// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_;
  inv1   g00(.a(x14), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x09), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(x08), .O(new_n35_));
  nand3  g07(.a(new_n35_), .b(new_n29_), .c(new_n33_), .O(new_n36_));
  oai22  g08(.a(new_n36_), .b(x11), .c(new_n32_), .d(new_n31_), .O(z1));
  nor2   g09(.a(x12), .b(x11), .O(new_n38_));
  inv1   g10(.a(x08), .O(new_n39_));
  nand2  g11(.a(x12), .b(x11), .O(new_n40_));
  nand3  g12(.a(new_n40_), .b(x09), .c(new_n39_), .O(new_n41_));
  oai21  g13(.a(new_n41_), .b(new_n38_), .c(new_n29_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n33_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n43_), .O(z2));
  inv1   g17(.a(x02), .O(new_n46_));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n35_), .c(new_n29_), .d(new_n33_), .O(new_n49_));
  oai22  g21(.a(new_n49_), .b(new_n47_), .c(new_n32_), .d(new_n46_), .O(z3));
  inv1   g22(.a(new_n48_), .O(new_n51_));
  aoi21  g23(.a(new_n51_), .b(new_n35_), .c(x14), .O(new_n52_));
  nand2  g24(.a(x08), .b(x03), .O(new_n53_));
  oai21  g25(.a(new_n52_), .b(x10), .c(new_n53_), .O(z4));
  inv1   g26(.a(x04), .O(new_n55_));
  inv1   g27(.a(x15), .O(new_n56_));
  oai22  g28(.a(new_n36_), .b(new_n56_), .c(new_n32_), .d(new_n55_), .O(z5));
  aoi21  g29(.a(new_n35_), .b(x16), .c(x14), .O(new_n58_));
  nand2  g30(.a(x08), .b(x05), .O(new_n59_));
  oai21  g31(.a(new_n58_), .b(x10), .c(new_n59_), .O(z6));
  aoi21  g32(.a(new_n35_), .b(x17), .c(x14), .O(new_n61_));
  nand2  g33(.a(x08), .b(x06), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(x10), .c(new_n62_), .O(z7));
  inv1   g35(.a(x07), .O(new_n64_));
  inv1   g36(.a(x18), .O(new_n65_));
  oai22  g37(.a(new_n36_), .b(new_n65_), .c(new_n32_), .d(new_n64_), .O(z8));
endmodule


