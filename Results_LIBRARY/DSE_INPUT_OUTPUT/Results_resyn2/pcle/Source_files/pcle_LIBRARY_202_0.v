// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_;
  nor2   g00(.a(x18), .b(x01), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(x09), .c(new_n31_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand3  g06(.a(x13), .b(x12), .c(x11), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  and2   g09(.a(x17), .b(x16), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n37_), .c(new_n34_), .d(x18), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n30_), .O(z0));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand2  g14(.a(new_n34_), .b(new_n42_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n29_), .O(z1));
  aoi21  g16(.a(x18), .b(new_n31_), .c(x01), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n42_), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n46_), .c(new_n32_), .d(x09), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n31_), .c(new_n45_), .O(z2));
  nand2  g22(.a(x08), .b(x02), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand2  g24(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n35_), .c(new_n34_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n51_), .c(new_n29_), .O(z3));
  nand2  g27(.a(x08), .b(x03), .O(new_n56_));
  inv1   g28(.a(new_n35_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x14), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand2  g31(.a(new_n35_), .b(new_n59_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n58_), .c(new_n34_), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n56_), .c(new_n29_), .O(z4));
  aoi21  g34(.a(new_n57_), .b(x14), .c(x15), .O(new_n63_));
  inv1   g35(.a(new_n37_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  aoi21  g37(.a(x08), .b(x04), .c(new_n29_), .O(new_n66_));
  oai21  g38(.a(new_n65_), .b(new_n63_), .c(new_n66_), .O(z5));
  inv1   g39(.a(new_n36_), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n57_), .c(x16), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  oai21  g42(.a(new_n37_), .b(x16), .c(new_n34_), .O(new_n71_));
  aoi21  g43(.a(x08), .b(x05), .c(new_n29_), .O(new_n72_));
  oai21  g44(.a(new_n71_), .b(new_n70_), .c(new_n72_), .O(z6));
  aoi21  g45(.a(new_n38_), .b(new_n37_), .c(new_n33_), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n69_), .b(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x06), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n77_), .c(new_n29_), .O(z7));
  inv1   g51(.a(x18), .O(new_n80_));
  nand3  g52(.a(new_n38_), .b(new_n68_), .c(new_n57_), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n33_), .c(x01), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  and2   g55(.a(x08), .b(x07), .O(new_n84_));
  aoi21  g56(.a(new_n74_), .b(x18), .c(new_n84_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n83_), .O(z8));
endmodule


