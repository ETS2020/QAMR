// Benchmark "FAU" written by ABC on Tue Aug 11 19:39:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x18), .O(new_n34_));
  nand2  g06(.a(x16), .b(x15), .O(new_n35_));
  nand2  g07(.a(x13), .b(x12), .O(new_n36_));
  nor3   g08(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  nand2  g09(.a(new_n37_), .b(new_n33_), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(x14), .c(new_n29_), .O(z0));
  nand3  g11(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n40_));
  nor2   g12(.a(new_n29_), .b(x14), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n41_), .O(new_n42_));
  oai21  g14(.a(new_n40_), .b(x11), .c(new_n42_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  inv1   g16(.a(new_n40_), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  or2    g18(.a(x12), .b(x11), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n44_), .c(new_n41_), .O(z2));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand2  g23(.a(new_n46_), .b(new_n51_), .O(new_n52_));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n52_), .c(new_n45_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n50_), .c(new_n41_), .O(z3));
  inv1   g27(.a(new_n41_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x08), .c(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  inv1   g31(.a(new_n53_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x14), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n59_), .c(new_n56_), .d(new_n45_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n57_), .O(z4));
  nand4  g35(.a(x15), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  nand4  g36(.a(x14), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand3  g39(.a(x13), .b(x12), .c(x11), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n66_), .c(new_n64_), .O(new_n70_));
  oai21  g42(.a(new_n40_), .b(new_n67_), .c(new_n29_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n58_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x04), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(z5));
  nand2  g46(.a(new_n64_), .b(x16), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n53_), .c(new_n75_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  oai21  g51(.a(new_n40_), .b(new_n76_), .c(new_n29_), .O(new_n80_));
  aoi22  g52(.a(new_n80_), .b(new_n58_), .c(x08), .d(x05), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n79_), .O(z6));
  nand3  g54(.a(new_n56_), .b(x08), .c(x06), .O(new_n83_));
  inv1   g55(.a(new_n35_), .O(new_n84_));
  inv1   g56(.a(new_n68_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n84_), .c(x17), .O(new_n86_));
  oai21  g58(.a(new_n68_), .b(new_n35_), .c(new_n29_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n66_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n83_), .O(z7));
  nor3   g61(.a(new_n36_), .b(new_n35_), .c(x18), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n33_), .c(new_n58_), .O(new_n91_));
  nand4  g63(.a(x18), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n92_));
  inv1   g64(.a(new_n92_), .O(new_n93_));
  and2   g65(.a(x08), .b(x07), .O(new_n94_));
  aoi21  g66(.a(new_n93_), .b(new_n86_), .c(new_n94_), .O(new_n95_));
  oai21  g67(.a(new_n91_), .b(new_n29_), .c(new_n95_), .O(z8));
endmodule


