// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_;
  nand4  g00(.a(x15), .b(x14), .c(x13), .d(x11), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  inv1   g04(.a(x12), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x10), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n34_), .c(new_n32_), .d(new_n30_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(z0));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(x09), .c(new_n40_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n39_), .O(new_n44_));
  nand2  g16(.a(x08), .b(x00), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z1));
  inv1   g18(.a(x01), .O(new_n47_));
  inv1   g19(.a(new_n34_), .O(new_n48_));
  oai21  g20(.a(x12), .b(x10), .c(x08), .O(new_n49_));
  oai22  g21(.a(new_n49_), .b(new_n47_), .c(new_n42_), .d(new_n48_), .O(z2));
  inv1   g22(.a(x02), .O(new_n51_));
  nand2  g23(.a(new_n34_), .b(new_n32_), .O(new_n52_));
  xnor2a g24(.a(x13), .b(x11), .O(new_n53_));
  oai22  g25(.a(new_n53_), .b(new_n52_), .c(new_n49_), .d(new_n51_), .O(z3));
  aoi21  g26(.a(x13), .b(x11), .c(x14), .O(new_n55_));
  nand3  g27(.a(x14), .b(x13), .c(x11), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x09), .c(new_n40_), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n55_), .c(x12), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n39_), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z4));
  inv1   g33(.a(x04), .O(new_n62_));
  inv1   g34(.a(x15), .O(new_n63_));
  nand2  g35(.a(new_n56_), .b(new_n63_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n34_), .c(new_n32_), .d(new_n29_), .O(new_n65_));
  oai21  g37(.a(new_n49_), .b(new_n62_), .c(new_n65_), .O(z5));
  inv1   g38(.a(x05), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  nor2   g40(.a(new_n29_), .b(new_n68_), .O(new_n69_));
  nand2  g41(.a(new_n29_), .b(new_n68_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n34_), .c(new_n32_), .O(new_n71_));
  oai22  g43(.a(new_n71_), .b(new_n69_), .c(new_n49_), .d(new_n67_), .O(z6));
  inv1   g44(.a(x06), .O(new_n73_));
  inv1   g45(.a(x17), .O(new_n74_));
  nor3   g46(.a(new_n29_), .b(new_n74_), .c(new_n68_), .O(new_n75_));
  nand2  g47(.a(x16), .b(x15), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n56_), .c(new_n74_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n34_), .c(new_n32_), .O(new_n78_));
  oai22  g50(.a(new_n78_), .b(new_n75_), .c(new_n49_), .d(new_n73_), .O(z7));
  inv1   g51(.a(x07), .O(new_n80_));
  inv1   g52(.a(x18), .O(new_n81_));
  nand2  g53(.a(x17), .b(x16), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g55(.a(new_n83_), .b(new_n35_), .c(new_n29_), .O(new_n84_));
  nand2  g56(.a(new_n29_), .b(new_n81_), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(new_n34_), .c(new_n32_), .O(new_n86_));
  oai22  g58(.a(new_n86_), .b(new_n84_), .c(new_n49_), .d(new_n80_), .O(z8));
endmodule


