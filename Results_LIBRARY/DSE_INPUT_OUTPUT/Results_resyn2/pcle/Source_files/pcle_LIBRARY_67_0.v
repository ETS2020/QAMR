// Benchmark "FAU" written by ABC on Tue Aug 11 19:37:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n82_;
  nor2   g00(.a(x18), .b(x17), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(new_n30_));
  inv1   g02(.a(x15), .O(new_n31_));
  nand4  g03(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x17), .c(x16), .O(new_n34_));
  inv1   g06(.a(x08), .O(new_n35_));
  inv1   g07(.a(x10), .O(new_n36_));
  nand3  g08(.a(new_n36_), .b(x09), .c(new_n35_), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x18), .O(new_n39_));
  oai21  g11(.a(new_n39_), .b(new_n34_), .c(new_n30_), .O(z0));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand2  g14(.a(new_n38_), .b(new_n42_), .O(new_n43_));
  aoi21  g15(.a(new_n43_), .b(new_n41_), .c(new_n29_), .O(z1));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nor2   g18(.a(new_n46_), .b(new_n42_), .O(new_n47_));
  inv1   g19(.a(new_n47_), .O(new_n48_));
  nand2  g20(.a(new_n46_), .b(new_n42_), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(new_n38_), .c(new_n48_), .O(new_n50_));
  aoi21  g22(.a(new_n50_), .b(new_n45_), .c(new_n29_), .O(z2));
  nor2   g23(.a(new_n47_), .b(x13), .O(new_n52_));
  nand2  g24(.a(new_n47_), .b(x13), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n29_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n52_), .c(new_n55_), .O(z3));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n38_), .c(new_n32_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n57_), .c(new_n29_), .O(z4));
  nand2  g33(.a(x08), .b(x04), .O(new_n62_));
  nand2  g34(.a(new_n32_), .b(new_n31_), .O(new_n63_));
  nor2   g35(.a(new_n37_), .b(new_n33_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n62_), .c(new_n29_), .O(z5));
  aoi21  g38(.a(new_n33_), .b(x16), .c(new_n37_), .O(new_n67_));
  oai21  g39(.a(new_n33_), .b(x16), .c(new_n67_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x05), .c(new_n29_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z6));
  inv1   g42(.a(x17), .O(new_n71_));
  nand3  g43(.a(new_n38_), .b(new_n33_), .c(x16), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x18), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  and2   g46(.a(x08), .b(x06), .O(new_n75_));
  aoi21  g47(.a(new_n67_), .b(x17), .c(new_n75_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n74_), .O(z7));
  nand4  g49(.a(new_n33_), .b(x18), .c(x17), .d(x16), .O(new_n78_));
  inv1   g50(.a(x18), .O(new_n79_));
  nand2  g51(.a(new_n34_), .b(new_n79_), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n78_), .c(new_n38_), .O(new_n81_));
  nand3  g53(.a(new_n30_), .b(x08), .c(x07), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(new_n81_), .O(z8));
endmodule


