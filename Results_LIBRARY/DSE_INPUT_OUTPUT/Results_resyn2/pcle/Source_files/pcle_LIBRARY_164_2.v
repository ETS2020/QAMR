// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x06), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  inv1   g02(.a(x09), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x08), .O(new_n32_));
  nand3  g04(.a(new_n32_), .b(new_n30_), .c(new_n29_), .O(new_n33_));
  nand4  g05(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x15), .O(new_n36_));
  inv1   g08(.a(x16), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n35_), .c(x18), .d(x17), .O(new_n39_));
  nor2   g11(.a(new_n39_), .b(new_n33_), .O(z0));
  inv1   g12(.a(x00), .O(new_n41_));
  oai21  g13(.a(x10), .b(new_n29_), .c(x08), .O(new_n42_));
  oai22  g14(.a(new_n42_), .b(new_n41_), .c(new_n33_), .d(x11), .O(z1));
  inv1   g15(.a(x01), .O(new_n44_));
  xnor2a g16(.a(x12), .b(x11), .O(new_n45_));
  oai22  g17(.a(new_n45_), .b(new_n33_), .c(new_n42_), .d(new_n44_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n32_), .O(new_n49_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n29_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n30_), .O(new_n51_));
  nand2  g23(.a(x08), .b(x02), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z3));
  inv1   g25(.a(x03), .O(new_n54_));
  inv1   g26(.a(new_n33_), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand2  g28(.a(new_n48_), .b(new_n56_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n34_), .c(new_n55_), .O(new_n58_));
  oai21  g30(.a(new_n42_), .b(new_n54_), .c(new_n58_), .O(z4));
  nand2  g31(.a(new_n34_), .b(new_n36_), .O(new_n60_));
  inv1   g32(.a(x08), .O(new_n61_));
  nand2  g33(.a(x09), .b(new_n61_), .O(new_n62_));
  aoi21  g34(.a(new_n35_), .b(x15), .c(new_n62_), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n60_), .c(x06), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(x10), .c(new_n65_), .O(z5));
  oai21  g38(.a(new_n34_), .b(new_n36_), .c(new_n37_), .O(new_n67_));
  aoi21  g39(.a(new_n38_), .b(new_n35_), .c(new_n62_), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n67_), .c(x06), .O(new_n69_));
  nand2  g41(.a(x08), .b(x05), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(x10), .c(new_n70_), .O(z6));
  nand3  g43(.a(x10), .b(x08), .c(x06), .O(new_n72_));
  nor3   g44(.a(new_n34_), .b(new_n37_), .c(new_n36_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x17), .O(new_n74_));
  inv1   g46(.a(x17), .O(new_n75_));
  nand2  g47(.a(new_n38_), .b(new_n35_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n74_), .c(new_n55_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n72_), .O(z7));
  aoi21  g51(.a(new_n73_), .b(x17), .c(x18), .O(new_n80_));
  nand3  g52(.a(new_n39_), .b(new_n32_), .c(new_n30_), .O(new_n81_));
  aoi22  g53(.a(new_n30_), .b(x06), .c(x08), .d(x07), .O(new_n82_));
  oai21  g54(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z8));
endmodule


