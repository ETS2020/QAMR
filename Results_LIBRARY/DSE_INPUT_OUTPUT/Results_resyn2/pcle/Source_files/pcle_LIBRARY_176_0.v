// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x18), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nand4  g02(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n34_));
  nor4   g06(.a(new_n34_), .b(new_n31_), .c(new_n30_), .d(new_n29_), .O(z0));
  inv1   g07(.a(x00), .O(new_n36_));
  oai21  g08(.a(x10), .b(x09), .c(x08), .O(new_n37_));
  oai22  g09(.a(new_n37_), .b(new_n36_), .c(new_n34_), .d(x11), .O(z1));
  inv1   g10(.a(x09), .O(new_n39_));
  or2    g11(.a(x12), .b(x11), .O(new_n40_));
  and2   g12(.a(x12), .b(x11), .O(new_n41_));
  nor2   g13(.a(new_n41_), .b(x08), .O(new_n42_));
  aoi21  g14(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(x10), .c(new_n44_), .O(z2));
  nor2   g17(.a(new_n41_), .b(x13), .O(new_n46_));
  nand2  g18(.a(new_n30_), .b(new_n32_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n46_), .c(x09), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z3));
  inv1   g23(.a(x14), .O(new_n52_));
  nand4  g24(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n32_), .O(new_n54_));
  aoi21  g26(.a(new_n30_), .b(new_n52_), .c(new_n54_), .O(new_n55_));
  oai21  g27(.a(new_n55_), .b(new_n39_), .c(new_n33_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n56_), .O(z4));
  inv1   g30(.a(x04), .O(new_n59_));
  inv1   g31(.a(x15), .O(new_n60_));
  aoi21  g32(.a(new_n53_), .b(new_n60_), .c(new_n34_), .O(new_n61_));
  oai21  g33(.a(new_n53_), .b(new_n60_), .c(new_n61_), .O(new_n62_));
  oai21  g34(.a(new_n37_), .b(new_n59_), .c(new_n62_), .O(z5));
  inv1   g35(.a(x16), .O(new_n64_));
  oai21  g36(.a(new_n53_), .b(new_n60_), .c(new_n64_), .O(new_n65_));
  nand2  g37(.a(x16), .b(x15), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  inv1   g39(.a(new_n53_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g41(.a(new_n69_), .b(new_n65_), .c(new_n32_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x09), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n33_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x05), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(new_n72_), .O(z6));
  nand3  g46(.a(new_n68_), .b(new_n67_), .c(x17), .O(new_n75_));
  inv1   g47(.a(x17), .O(new_n76_));
  oai21  g48(.a(new_n53_), .b(new_n66_), .c(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n75_), .c(new_n32_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(x09), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n33_), .O(new_n80_));
  nand2  g52(.a(x08), .b(x06), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z7));
  oai21  g54(.a(new_n31_), .b(new_n30_), .c(x18), .O(new_n83_));
  nor2   g55(.a(x18), .b(new_n76_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n68_), .c(new_n67_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n83_), .c(x08), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n39_), .c(new_n33_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x07), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(z8));
endmodule


