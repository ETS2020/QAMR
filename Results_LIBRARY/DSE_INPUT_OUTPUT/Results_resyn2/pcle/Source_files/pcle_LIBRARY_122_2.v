// Benchmark "FAU" written by ABC on Tue Aug 11 19:38:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand3  g03(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n32_));
  nand4  g04(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x17), .c(x16), .d(x15), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n32_), .c(new_n29_), .O(z0));
  inv1   g08(.a(x15), .O(new_n37_));
  nor2   g09(.a(x18), .b(new_n37_), .O(new_n38_));
  aoi21  g10(.a(x08), .b(x00), .c(new_n38_), .O(new_n39_));
  oai21  g11(.a(new_n32_), .b(x11), .c(new_n39_), .O(z1));
  inv1   g12(.a(new_n32_), .O(new_n41_));
  nand2  g13(.a(x12), .b(x11), .O(new_n42_));
  or2    g14(.a(x12), .b(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  aoi21  g16(.a(x08), .b(x01), .c(new_n38_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n44_), .O(z2));
  aoi21  g18(.a(x12), .b(x11), .c(x13), .O(new_n47_));
  nand3  g19(.a(x13), .b(x12), .c(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n41_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x02), .c(new_n38_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(z3));
  inv1   g23(.a(new_n48_), .O(new_n52_));
  nor2   g24(.a(new_n52_), .b(x14), .O(new_n53_));
  nand2  g25(.a(new_n33_), .b(new_n41_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x03), .c(new_n38_), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z4));
  nor2   g28(.a(new_n38_), .b(new_n30_), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(x04), .O(new_n58_));
  nand2  g30(.a(new_n33_), .b(x18), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x15), .O(new_n60_));
  inv1   g32(.a(new_n60_), .O(new_n61_));
  oai21  g33(.a(new_n34_), .b(x15), .c(new_n41_), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n61_), .c(new_n58_), .O(z5));
  nand2  g35(.a(new_n57_), .b(x05), .O(new_n64_));
  inv1   g36(.a(x16), .O(new_n65_));
  nand3  g37(.a(x18), .b(new_n65_), .c(x15), .O(new_n66_));
  nor2   g38(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  aoi21  g39(.a(new_n60_), .b(x16), .c(new_n67_), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(new_n32_), .c(new_n64_), .O(z6));
  nand2  g41(.a(new_n57_), .b(x06), .O(new_n70_));
  inv1   g42(.a(x17), .O(new_n71_));
  nand2  g43(.a(x16), .b(x15), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n33_), .c(new_n71_), .O(new_n73_));
  nor2   g45(.a(new_n38_), .b(new_n32_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(new_n35_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n70_), .O(z7));
  nand3  g48(.a(new_n35_), .b(new_n41_), .c(x18), .O(new_n77_));
  aoi21  g49(.a(x08), .b(x07), .c(new_n38_), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n77_), .O(z8));
endmodule


