// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n57_, new_n60_, new_n61_, new_n64_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x15), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  nand2  g05(.a(x20), .b(x15), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n46_), .c(x14), .O(z01));
  nand2  g07(.a(new_n44_), .b(x14), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(x12), .b(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z02));
  nand2  g12(.a(z01), .b(x12), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z03));
  nand2  g14(.a(new_n44_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nand2  g16(.a(new_n44_), .b(new_n57_), .O(z05));
  inv1   g17(.a(new_n47_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  inv1   g19(.a(new_n44_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(z07));
  and2   g21(.a(new_n44_), .b(x16), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n61_), .O(z09));
  and2   g24(.a(new_n64_), .b(new_n44_), .O(z10));
  inv1   g25(.a(x14), .O(new_n67_));
  nand2  g26(.a(new_n44_), .b(new_n67_), .O(z11));
  nand2  g27(.a(x13), .b(x12), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(x20), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x15), .O(new_n71_));
  nand2  g30(.a(new_n46_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n50_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(z12));
  and2   g35(.a(x10), .b(x08), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n50_), .c(x01), .O(new_n78_));
  oai21  g37(.a(new_n46_), .b(new_n50_), .c(new_n67_), .O(new_n79_));
  nand4  g38(.a(new_n79_), .b(new_n78_), .c(new_n44_), .d(x12), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z13));
  nand2  g40(.a(new_n77_), .b(x02), .O(new_n82_));
  oai21  g41(.a(new_n46_), .b(new_n50_), .c(x12), .O(new_n83_));
  aoi21  g42(.a(new_n82_), .b(new_n47_), .c(new_n83_), .O(z14));
  inv1   g43(.a(x12), .O(new_n85_));
  nand3  g44(.a(new_n77_), .b(new_n50_), .c(x03), .O(new_n86_));
  nand4  g45(.a(new_n51_), .b(new_n44_), .c(x16), .d(new_n67_), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z15));
  nand2  g47(.a(z06), .b(x17), .O(new_n89_));
  inv1   g48(.a(x04), .O(new_n90_));
  nand3  g49(.a(x10), .b(x08), .c(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n46_), .b(new_n60_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n50_), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n89_), .c(new_n85_), .O(z16));
  nand2  g53(.a(x18), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x20), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n46_), .b(x18), .O(new_n98_));
  nand4  g57(.a(new_n50_), .b(x10), .c(x08), .d(x05), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n97_), .O(z17));
  nand2  g61(.a(z06), .b(x19), .O(new_n103_));
  inv1   g62(.a(x06), .O(new_n104_));
  nand3  g63(.a(x10), .b(x08), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n46_), .b(new_n57_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n105_), .c(new_n50_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n103_), .c(new_n85_), .O(z18));
  inv1   g67(.a(x07), .O(new_n109_));
  nand3  g68(.a(new_n77_), .b(new_n50_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n51_), .b(new_n43_), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(new_n110_), .c(x12), .O(new_n112_));
  inv1   g71(.a(new_n112_), .O(z19));
endmodule


