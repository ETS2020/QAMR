// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n60_,
    new_n62_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x15), .O(new_n42_));
  nand2  g01(.a(x19), .b(new_n42_), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n43_), .O(z01));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x15), .c(new_n47_), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n43_), .O(z02));
  oai21  g16(.a(new_n50_), .b(new_n54_), .c(new_n43_), .O(z03));
  nand2  g17(.a(new_n43_), .b(x18), .O(z04));
  nor2   g18(.a(x19), .b(x15), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z07));
  and2   g22(.a(new_n43_), .b(x16), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n43_), .O(z09));
  nand3  g25(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nand2  g26(.a(new_n43_), .b(new_n47_), .O(z11));
  inv1   g27(.a(x19), .O(new_n69_));
  nand2  g28(.a(x12), .b(x00), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n52_), .c(new_n69_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n42_), .O(new_n72_));
  nand3  g31(.a(new_n42_), .b(x10), .c(x08), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(x13), .c(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n72_), .O(z12));
  inv1   g34(.a(x01), .O(new_n76_));
  oai21  g35(.a(new_n52_), .b(new_n76_), .c(new_n69_), .O(new_n77_));
  nand2  g36(.a(new_n48_), .b(new_n47_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  aoi21  g38(.a(new_n77_), .b(new_n42_), .c(new_n79_), .O(z13));
  inv1   g39(.a(x02), .O(new_n81_));
  oai21  g40(.a(new_n52_), .b(new_n81_), .c(new_n42_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n48_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n43_), .O(z14));
  nand2  g43(.a(x12), .b(x03), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n52_), .c(new_n69_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n42_), .O(new_n87_));
  nand4  g46(.a(new_n73_), .b(x16), .c(new_n47_), .d(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(z15));
  aoi21  g48(.a(x10), .b(x08), .c(x19), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(x15), .c(x17), .O(new_n91_));
  nand4  g50(.a(new_n60_), .b(x10), .c(x08), .d(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n54_), .O(z16));
  nand2  g52(.a(x12), .b(x05), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n52_), .c(new_n69_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n42_), .O(new_n96_));
  nand3  g55(.a(new_n73_), .b(x18), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(z17));
  nand4  g57(.a(new_n42_), .b(x10), .c(x08), .d(x06), .O(new_n99_));
  nand2  g58(.a(new_n43_), .b(x12), .O(new_n100_));
  aoi21  g59(.a(new_n99_), .b(new_n69_), .c(new_n100_), .O(z18));
  nand2  g60(.a(x12), .b(x07), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n52_), .c(new_n69_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n42_), .O(new_n104_));
  nand3  g63(.a(new_n73_), .b(x20), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z19));
  buf    g65(.a(x19), .O(z05));
endmodule


