// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x10), .O(new_n44_));
  nand2  g03(.a(x15), .b(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x08), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n47_), .c(x10), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(z01));
  nand2  g11(.a(new_n45_), .b(x14), .O(new_n53_));
  nand3  g12(.a(new_n49_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z02));
  nand3  g15(.a(new_n47_), .b(x12), .c(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x10), .c(new_n49_), .O(z03));
  nand2  g17(.a(new_n45_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  inv1   g19(.a(new_n45_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n61_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand3  g25(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  inv1   g27(.a(new_n53_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  inv1   g29(.a(x00), .O(new_n71_));
  inv1   g30(.a(x13), .O(new_n72_));
  nand2  g31(.a(x10), .b(x08), .O(new_n73_));
  oai22  g32(.a(new_n73_), .b(new_n71_), .c(new_n72_), .d(x10), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  nand2  g34(.a(new_n49_), .b(x08), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x13), .c(x10), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n70_), .O(z12));
  oai21  g37(.a(new_n47_), .b(new_n70_), .c(new_n49_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n44_), .O(new_n80_));
  aoi21  g39(.a(x08), .b(x01), .c(new_n47_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n50_), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n80_), .O(z13));
  aoi21  g42(.a(x08), .b(x02), .c(x15), .O(new_n84_));
  nand2  g43(.a(x12), .b(x10), .O(new_n85_));
  nor3   g44(.a(new_n85_), .b(new_n84_), .c(new_n50_), .O(z14));
  nand3  g45(.a(x10), .b(x08), .c(x03), .O(new_n87_));
  nand3  g46(.a(x16), .b(new_n47_), .c(new_n44_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n49_), .O(new_n90_));
  nand4  g49(.a(new_n76_), .b(x16), .c(new_n47_), .d(x10), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n70_), .O(z15));
  inv1   g51(.a(x04), .O(new_n93_));
  oai22  g52(.a(new_n73_), .b(new_n93_), .c(new_n63_), .d(x10), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n49_), .O(new_n95_));
  nand3  g54(.a(new_n76_), .b(x17), .c(x10), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n70_), .O(z16));
  nand2  g56(.a(x18), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n49_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n44_), .O(new_n100_));
  oai21  g59(.a(x15), .b(new_n48_), .c(x18), .O(new_n101_));
  nand4  g60(.a(new_n49_), .b(x10), .c(x08), .d(x05), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n100_), .O(z17));
  oai21  g64(.a(new_n60_), .b(new_n70_), .c(new_n49_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n44_), .O(new_n107_));
  oai21  g66(.a(x15), .b(new_n48_), .c(x19), .O(new_n108_));
  nand4  g67(.a(new_n49_), .b(x10), .c(x08), .d(x06), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n107_), .O(z18));
  inv1   g71(.a(x07), .O(new_n113_));
  oai22  g72(.a(new_n73_), .b(new_n113_), .c(new_n43_), .d(x10), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n49_), .O(new_n115_));
  nand3  g74(.a(new_n76_), .b(x20), .c(x10), .O(new_n116_));
  aoi21  g75(.a(new_n116_), .b(new_n115_), .c(new_n70_), .O(z19));
  buf    g76(.a(x15), .O(z06));
endmodule


