// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x09), .O(new_n42_));
  nor2   g01(.a(new_n42_), .b(x01), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(new_n43_), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(z01));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  inv1   g13(.a(x12), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x11), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n46_), .O(z02));
  oai21  g17(.a(new_n50_), .b(new_n55_), .c(new_n46_), .O(z03));
  nand2  g18(.a(new_n46_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n46_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n46_), .b(new_n52_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n43_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x01), .O(new_n68_));
  oai21  g27(.a(x08), .b(new_n68_), .c(x09), .O(z10));
  inv1   g28(.a(z10), .O(z09));
  nand2  g29(.a(new_n46_), .b(new_n47_), .O(z11));
  inv1   g30(.a(x00), .O(new_n72_));
  nand4  g31(.a(new_n52_), .b(x10), .c(x08), .d(new_n72_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  nand2  g33(.a(new_n53_), .b(new_n74_), .O(new_n75_));
  aoi21  g34(.a(x09), .b(new_n68_), .c(new_n55_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  aoi21  g37(.a(x14), .b(x12), .c(x09), .O(new_n79_));
  nand2  g38(.a(x10), .b(x08), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n47_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n53_), .c(x12), .O(new_n82_));
  oai21  g41(.a(new_n79_), .b(x01), .c(new_n82_), .O(z13));
  inv1   g42(.a(x02), .O(new_n84_));
  oai21  g43(.a(new_n80_), .b(new_n84_), .c(new_n52_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n48_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n46_), .O(z14));
  inv1   g46(.a(x03), .O(new_n88_));
  nand4  g47(.a(new_n52_), .b(x10), .c(x08), .d(new_n88_), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n47_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n53_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n89_), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n46_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand4  g53(.a(new_n52_), .b(x10), .c(x08), .d(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n53_), .b(new_n64_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n46_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  nand4  g58(.a(new_n52_), .b(x10), .c(x08), .d(new_n99_), .O(new_n100_));
  inv1   g59(.a(x18), .O(new_n101_));
  nand2  g60(.a(new_n53_), .b(new_n101_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(new_n100_), .c(new_n76_), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z17));
  inv1   g63(.a(x06), .O(new_n105_));
  nand4  g64(.a(new_n52_), .b(x10), .c(x08), .d(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n53_), .b(new_n61_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(new_n106_), .c(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n46_), .O(z18));
  inv1   g68(.a(x07), .O(new_n110_));
  nand4  g69(.a(new_n52_), .b(x10), .c(x08), .d(new_n110_), .O(new_n111_));
  inv1   g70(.a(x20), .O(new_n112_));
  nand2  g71(.a(new_n53_), .b(new_n112_), .O(new_n113_));
  nand3  g72(.a(new_n113_), .b(new_n111_), .c(x12), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n46_), .O(z19));
endmodule


