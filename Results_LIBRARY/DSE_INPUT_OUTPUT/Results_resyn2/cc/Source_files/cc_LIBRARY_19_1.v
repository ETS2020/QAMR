// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x18), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x14), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  nor2   g05(.a(x18), .b(x14), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(z11));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(z11), .O(z01));
  inv1   g09(.a(x14), .O(new_n51_));
  nand2  g10(.a(x18), .b(new_n51_), .O(new_n52_));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x15), .c(new_n51_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n52_), .O(z02));
  inv1   g15(.a(new_n49_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n44_), .c(x14), .O(z03));
  nand2  g18(.a(x18), .b(x14), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n52_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x15), .O(new_n63_));
  nand2  g22(.a(new_n52_), .b(new_n63_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nor2   g24(.a(new_n45_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n52_), .b(new_n67_), .O(z08));
  nand3  g27(.a(new_n52_), .b(x09), .c(x08), .O(z10));
  inv1   g28(.a(z10), .O(z09));
  inv1   g29(.a(x00), .O(new_n71_));
  nand4  g30(.a(new_n63_), .b(x10), .c(x08), .d(new_n71_), .O(new_n72_));
  inv1   g31(.a(x12), .O(new_n73_));
  aoi21  g32(.a(x18), .b(new_n51_), .c(new_n73_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  oai21  g34(.a(new_n53_), .b(x15), .c(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  nand4  g37(.a(new_n63_), .b(x10), .c(x08), .d(x01), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x14), .O(new_n80_));
  nand4  g39(.a(new_n44_), .b(x15), .c(x10), .d(x08), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n73_), .O(z13));
  nand3  g41(.a(x10), .b(x08), .c(x02), .O(new_n83_));
  and2   g42(.a(new_n83_), .b(new_n63_), .O(new_n84_));
  nand2  g43(.a(new_n74_), .b(new_n49_), .O(new_n85_));
  nor2   g44(.a(new_n85_), .b(new_n84_), .O(z14));
  nand3  g45(.a(x10), .b(x08), .c(x03), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(new_n52_), .c(new_n63_), .O(new_n89_));
  nand3  g48(.a(new_n63_), .b(x10), .c(x08), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n47_), .c(x16), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n89_), .c(new_n73_), .O(z15));
  inv1   g51(.a(x04), .O(new_n93_));
  nand4  g52(.a(new_n63_), .b(x10), .c(x08), .d(new_n93_), .O(new_n94_));
  oai21  g53(.a(new_n53_), .b(x15), .c(new_n65_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n52_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  nand4  g57(.a(new_n63_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  oai21  g58(.a(new_n53_), .b(x15), .c(new_n44_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n74_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand4  g62(.a(new_n63_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  oai21  g63(.a(new_n53_), .b(x15), .c(new_n61_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n52_), .O(z18));
  inv1   g66(.a(x07), .O(new_n108_));
  nand4  g67(.a(new_n63_), .b(x10), .c(x08), .d(new_n108_), .O(new_n109_));
  oai21  g68(.a(new_n53_), .b(x15), .c(new_n43_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(new_n109_), .c(new_n74_), .O(new_n111_));
  inv1   g70(.a(new_n111_), .O(z19));
endmodule


