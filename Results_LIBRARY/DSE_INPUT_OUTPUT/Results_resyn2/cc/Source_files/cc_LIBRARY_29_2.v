// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x18), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x15), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  nor2   g08(.a(new_n44_), .b(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n47_), .O(z01));
  nand2  g11(.a(new_n45_), .b(x14), .O(new_n53_));
  inv1   g12(.a(new_n47_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand2  g14(.a(x12), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(new_n53_), .c(new_n56_), .O(z02));
  nand3  g16(.a(new_n54_), .b(new_n48_), .c(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x18), .c(new_n49_), .O(z03));
  nor2   g18(.a(x18), .b(x15), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  aoi21  g20(.a(new_n44_), .b(x15), .c(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  nand3  g27(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g28(.a(new_n53_), .O(z11));
  nand2  g29(.a(x13), .b(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x18), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x15), .O(new_n73_));
  nand2  g32(.a(new_n47_), .b(x13), .O(new_n74_));
  nand4  g33(.a(new_n49_), .b(x10), .c(x08), .d(x00), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n73_), .O(z12));
  inv1   g37(.a(x12), .O(new_n79_));
  nand2  g38(.a(new_n47_), .b(new_n48_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  nand3  g40(.a(x10), .b(x08), .c(x01), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n49_), .c(x14), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n81_), .c(new_n79_), .O(z13));
  nand3  g43(.a(new_n54_), .b(new_n49_), .c(x02), .O(new_n85_));
  nand3  g44(.a(new_n47_), .b(x18), .c(x15), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n79_), .O(z14));
  nand3  g46(.a(x16), .b(new_n48_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x18), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand3  g49(.a(new_n47_), .b(x16), .c(new_n48_), .O(new_n91_));
  nand4  g50(.a(new_n49_), .b(x10), .c(x08), .d(x03), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  nand2  g54(.a(new_n50_), .b(x17), .O(new_n96_));
  inv1   g55(.a(x04), .O(new_n97_));
  nand3  g56(.a(x10), .b(x08), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n47_), .b(new_n63_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(new_n49_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n79_), .O(z16));
  nand2  g60(.a(new_n47_), .b(x18), .O(new_n102_));
  nand3  g61(.a(x10), .b(x08), .c(x05), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n102_), .c(new_n49_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n45_), .O(z17));
  nand2  g65(.a(new_n50_), .b(x19), .O(new_n107_));
  inv1   g66(.a(x06), .O(new_n108_));
  nand3  g67(.a(x10), .b(x08), .c(new_n108_), .O(new_n109_));
  nand2  g68(.a(new_n47_), .b(new_n61_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(new_n109_), .c(new_n49_), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n107_), .c(new_n79_), .O(z18));
  nand2  g71(.a(new_n50_), .b(x20), .O(new_n113_));
  inv1   g72(.a(x07), .O(new_n114_));
  nand3  g73(.a(x10), .b(x08), .c(new_n114_), .O(new_n115_));
  nand2  g74(.a(new_n47_), .b(new_n43_), .O(new_n116_));
  nand3  g75(.a(new_n116_), .b(new_n115_), .c(new_n49_), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n113_), .c(new_n79_), .O(z19));
  buf    g77(.a(x15), .O(z06));
endmodule


