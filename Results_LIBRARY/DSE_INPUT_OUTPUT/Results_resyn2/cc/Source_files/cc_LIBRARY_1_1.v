// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x12), .O(new_n43_));
  nand2  g02(.a(x15), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x12), .c(new_n46_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  nor2   g11(.a(new_n43_), .b(x11), .O(new_n53_));
  oai21  g12(.a(new_n52_), .b(x14), .c(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n44_), .O(z02));
  inv1   g14(.a(new_n44_), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nor2   g17(.a(new_n56_), .b(new_n58_), .O(z05));
  nor2   g18(.a(new_n46_), .b(new_n43_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n56_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n56_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n56_), .O(z09));
  and2   g25(.a(new_n65_), .b(new_n44_), .O(z10));
  nor2   g26(.a(new_n56_), .b(new_n47_), .O(z11));
  inv1   g27(.a(x00), .O(new_n69_));
  nand3  g28(.a(new_n48_), .b(new_n46_), .c(new_n69_), .O(new_n70_));
  inv1   g29(.a(x13), .O(new_n71_));
  oai21  g30(.a(new_n51_), .b(x15), .c(new_n71_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n70_), .c(x12), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n51_), .b(new_n75_), .c(x14), .O(new_n76_));
  nand2  g35(.a(new_n48_), .b(x15), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n43_), .O(z13));
  nand3  g37(.a(x12), .b(x10), .c(x08), .O(new_n79_));
  aoi21  g38(.a(new_n46_), .b(x02), .c(new_n79_), .O(new_n80_));
  aoi21  g39(.a(new_n79_), .b(new_n46_), .c(new_n80_), .O(z14));
  oai21  g40(.a(new_n63_), .b(x14), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x15), .O(new_n83_));
  nand2  g42(.a(new_n46_), .b(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n48_), .O(new_n85_));
  oai21  g44(.a(new_n63_), .b(x14), .c(new_n51_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n85_), .c(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n83_), .O(z15));
  inv1   g47(.a(x04), .O(new_n89_));
  nand3  g48(.a(new_n48_), .b(new_n46_), .c(new_n89_), .O(new_n90_));
  oai21  g49(.a(new_n51_), .b(x15), .c(new_n61_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(x12), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z16));
  nand2  g52(.a(new_n48_), .b(new_n46_), .O(new_n94_));
  oai21  g53(.a(new_n51_), .b(x05), .c(x12), .O(new_n95_));
  nor2   g54(.a(x18), .b(new_n43_), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n94_), .c(new_n95_), .d(new_n46_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand3  g57(.a(new_n48_), .b(new_n46_), .c(new_n98_), .O(new_n99_));
  oai21  g58(.a(new_n51_), .b(x15), .c(new_n58_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand3  g62(.a(new_n48_), .b(new_n46_), .c(new_n103_), .O(new_n104_));
  inv1   g63(.a(x20), .O(new_n105_));
  oai21  g64(.a(new_n51_), .b(x15), .c(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n104_), .c(x12), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z19));
  aoi21  g67(.a(new_n49_), .b(x12), .c(new_n46_), .O(z03));
endmodule


