// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n58_, new_n61_, new_n63_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x15), .b(x12), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x12), .O(z06));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x14), .O(new_n49_));
  and2   g08(.a(new_n49_), .b(z06), .O(z01));
  inv1   g09(.a(x12), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n48_), .b(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n42_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n46_), .c(new_n51_), .O(z02));
  inv1   g14(.a(new_n44_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z05));
  and2   g18(.a(new_n44_), .b(x17), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nand2  g20(.a(new_n44_), .b(new_n61_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n44_), .O(z09));
  nand3  g23(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g24(.a(new_n44_), .b(new_n52_), .O(z11));
  nand3  g25(.a(x10), .b(x08), .c(x00), .O(new_n67_));
  aoi21  g26(.a(new_n48_), .b(x13), .c(x15), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n67_), .c(new_n51_), .O(z12));
  inv1   g28(.a(x01), .O(new_n70_));
  oai21  g29(.a(new_n48_), .b(new_n70_), .c(x14), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n46_), .c(new_n51_), .O(z13));
  nand3  g31(.a(x10), .b(x08), .c(x02), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n46_), .c(new_n51_), .O(z14));
  nand3  g33(.a(new_n48_), .b(x16), .c(new_n52_), .O(new_n75_));
  nand3  g34(.a(x10), .b(x08), .c(x03), .O(new_n76_));
  and2   g35(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n51_), .O(z15));
  nand3  g37(.a(x10), .b(x08), .c(x04), .O(new_n79_));
  aoi21  g38(.a(new_n48_), .b(x17), .c(x15), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n51_), .O(z16));
  inv1   g40(.a(x05), .O(new_n82_));
  nand3  g41(.a(x10), .b(x08), .c(new_n82_), .O(new_n83_));
  inv1   g42(.a(x18), .O(new_n84_));
  nand2  g43(.a(new_n48_), .b(new_n84_), .O(new_n85_));
  nor2   g44(.a(x15), .b(new_n51_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n85_), .c(new_n83_), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z17));
  nand3  g47(.a(x10), .b(x08), .c(x06), .O(new_n89_));
  aoi21  g48(.a(new_n48_), .b(x19), .c(x15), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n51_), .O(z18));
  inv1   g50(.a(x07), .O(new_n92_));
  nand3  g51(.a(x10), .b(x08), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n48_), .b(new_n43_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(new_n86_), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z19));
endmodule


