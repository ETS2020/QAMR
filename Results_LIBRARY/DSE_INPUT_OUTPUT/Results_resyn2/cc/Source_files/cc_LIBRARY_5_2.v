// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x06), .O(new_n44_));
  nor2   g03(.a(x18), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x15), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor4   g08(.a(new_n49_), .b(new_n45_), .c(new_n48_), .d(x14), .O(z01));
  inv1   g09(.a(x14), .O(new_n51_));
  nand3  g10(.a(new_n48_), .b(x10), .c(x08), .O(new_n52_));
  nand3  g11(.a(new_n46_), .b(x12), .c(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n52_), .b(new_n51_), .c(new_n53_), .O(z02));
  nor2   g13(.a(new_n45_), .b(new_n48_), .O(z06));
  inv1   g14(.a(new_n49_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(z06), .c(new_n51_), .d(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z03));
  nor2   g17(.a(x18), .b(x06), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n45_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n46_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n45_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n45_), .O(z10));
  nor2   g27(.a(new_n45_), .b(new_n51_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  nor2   g29(.a(new_n45_), .b(new_n70_), .O(new_n71_));
  inv1   g30(.a(x00), .O(new_n72_));
  nor2   g31(.a(new_n49_), .b(x15), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  nand2  g34(.a(new_n52_), .b(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n74_), .c(new_n71_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n49_), .b(new_n79_), .c(x14), .O(new_n80_));
  nand2  g39(.a(new_n56_), .b(x15), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n46_), .O(z13));
  aoi21  g43(.a(new_n56_), .b(x02), .c(x15), .O(new_n85_));
  nand2  g44(.a(new_n81_), .b(new_n71_), .O(new_n86_));
  nor2   g45(.a(new_n86_), .b(new_n85_), .O(z14));
  nand3  g46(.a(new_n52_), .b(x16), .c(new_n51_), .O(new_n88_));
  nand3  g47(.a(new_n56_), .b(new_n48_), .c(x03), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n46_), .O(z15));
  inv1   g51(.a(x04), .O(new_n93_));
  nand2  g52(.a(new_n73_), .b(new_n93_), .O(new_n94_));
  nand2  g53(.a(new_n52_), .b(new_n62_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(new_n71_), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  nand2  g57(.a(new_n73_), .b(new_n98_), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  nand2  g59(.a(new_n52_), .b(new_n100_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n99_), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n46_), .O(z17));
  aoi21  g62(.a(new_n73_), .b(x12), .c(new_n100_), .O(new_n104_));
  nand3  g63(.a(new_n52_), .b(x19), .c(x12), .O(new_n105_));
  oai21  g64(.a(new_n104_), .b(new_n44_), .c(new_n105_), .O(z18));
  inv1   g65(.a(x07), .O(new_n107_));
  nand2  g66(.a(new_n73_), .b(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n52_), .b(new_n43_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n108_), .c(x12), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n46_), .O(z19));
endmodule


