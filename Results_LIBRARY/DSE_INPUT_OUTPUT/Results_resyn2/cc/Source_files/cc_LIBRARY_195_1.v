// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_, new_n62_,
    new_n65_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x03), .O(new_n42_));
  nand2  g01(.a(x15), .b(new_n42_), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  inv1   g05(.a(x15), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(new_n42_), .O(z06));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x14), .O(new_n50_));
  and2   g09(.a(new_n50_), .b(z06), .O(z01));
  nor2   g10(.a(new_n49_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n43_), .O(z02));
  nand3  g15(.a(new_n50_), .b(z06), .c(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z03));
  nor2   g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z07));
  and2   g22(.a(new_n43_), .b(x16), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n43_), .O(z09));
  nand3  g25(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(x14), .O(new_n68_));
  nor2   g27(.a(new_n44_), .b(new_n68_), .O(z11));
  nand2  g28(.a(new_n52_), .b(x00), .O(new_n70_));
  nand3  g29(.a(new_n47_), .b(x10), .c(x08), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n43_), .c(x13), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n70_), .c(new_n53_), .O(z12));
  nand3  g32(.a(x15), .b(x10), .c(x08), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n68_), .O(new_n75_));
  nand4  g34(.a(new_n47_), .b(x10), .c(x08), .d(x01), .O(new_n76_));
  nand4  g35(.a(new_n76_), .b(new_n75_), .c(new_n43_), .d(x12), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z13));
  inv1   g37(.a(x02), .O(new_n79_));
  oai21  g38(.a(new_n49_), .b(new_n79_), .c(new_n47_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n74_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n43_), .O(z14));
  nand3  g41(.a(x16), .b(new_n68_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x15), .O(new_n85_));
  nand4  g44(.a(new_n47_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand3  g45(.a(new_n49_), .b(x16), .c(new_n68_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n85_), .O(z15));
  oai21  g49(.a(new_n62_), .b(new_n53_), .c(x03), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x15), .O(new_n92_));
  nand2  g51(.a(new_n49_), .b(x17), .O(new_n93_));
  nand4  g52(.a(new_n47_), .b(x10), .c(x08), .d(x04), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n92_), .O(z16));
  nand2  g56(.a(x18), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x03), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x15), .O(new_n100_));
  nand2  g59(.a(new_n49_), .b(x18), .O(new_n101_));
  nand4  g60(.a(new_n47_), .b(x10), .c(x08), .d(x05), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n100_), .O(z17));
  oai21  g64(.a(new_n60_), .b(new_n53_), .c(x03), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x15), .O(new_n107_));
  nand2  g66(.a(new_n49_), .b(x19), .O(new_n108_));
  nand4  g67(.a(new_n47_), .b(x10), .c(x08), .d(x06), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n107_), .O(z18));
  nand2  g71(.a(z06), .b(x20), .O(new_n113_));
  inv1   g72(.a(x07), .O(new_n114_));
  nand3  g73(.a(x10), .b(x08), .c(new_n114_), .O(new_n115_));
  inv1   g74(.a(x20), .O(new_n116_));
  nand2  g75(.a(new_n49_), .b(new_n116_), .O(new_n117_));
  nand3  g76(.a(new_n117_), .b(new_n115_), .c(new_n47_), .O(new_n118_));
  aoi21  g77(.a(new_n118_), .b(new_n113_), .c(new_n53_), .O(z19));
endmodule


