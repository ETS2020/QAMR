// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x06), .O(new_n44_));
  nor2   g03(.a(x15), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  and2   g07(.a(x10), .b(x08), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x15), .c(new_n48_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(z01));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x15), .c(new_n48_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nor2   g15(.a(new_n50_), .b(new_n56_), .O(z03));
  nor2   g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n45_), .b(new_n59_), .O(z05));
  nor2   g19(.a(x15), .b(x06), .O(new_n61_));
  inv1   g20(.a(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n46_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n45_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n45_), .O(z10));
  nor2   g28(.a(new_n45_), .b(new_n48_), .O(z11));
  nand2  g29(.a(x15), .b(x13), .O(new_n71_));
  inv1   g30(.a(x15), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n49_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  aoi21  g34(.a(new_n52_), .b(new_n75_), .c(x06), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n71_), .c(new_n56_), .O(z12));
  oai21  g37(.a(new_n49_), .b(x14), .c(x15), .O(new_n79_));
  nand2  g38(.a(new_n49_), .b(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x14), .c(new_n44_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n79_), .c(new_n56_), .O(z13));
  aoi21  g41(.a(new_n61_), .b(x02), .c(new_n52_), .O(new_n83_));
  oai21  g42(.a(new_n49_), .b(x15), .c(x12), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(new_n83_), .O(z14));
  nand3  g44(.a(x16), .b(x15), .c(new_n48_), .O(new_n86_));
  nand2  g45(.a(new_n72_), .b(x03), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n49_), .O(new_n88_));
  oai21  g47(.a(new_n65_), .b(x14), .c(new_n52_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n88_), .c(new_n44_), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n86_), .c(new_n56_), .O(z15));
  nand2  g50(.a(x17), .b(x15), .O(new_n92_));
  nand2  g51(.a(new_n72_), .b(x04), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n49_), .O(new_n94_));
  aoi21  g53(.a(new_n52_), .b(new_n63_), .c(x06), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n92_), .c(new_n56_), .O(z16));
  nand2  g56(.a(x18), .b(x15), .O(new_n98_));
  nand2  g57(.a(new_n72_), .b(x05), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n49_), .O(new_n100_));
  inv1   g59(.a(x18), .O(new_n101_));
  aoi21  g60(.a(new_n52_), .b(new_n101_), .c(x06), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n98_), .c(new_n56_), .O(z17));
  aoi21  g63(.a(new_n52_), .b(new_n44_), .c(x15), .O(new_n105_));
  nor3   g64(.a(new_n105_), .b(new_n59_), .c(new_n56_), .O(z18));
  nand2  g65(.a(x20), .b(x15), .O(new_n107_));
  nand2  g66(.a(new_n72_), .b(x07), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n49_), .O(new_n109_));
  aoi21  g68(.a(new_n52_), .b(new_n43_), .c(x06), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n107_), .c(new_n56_), .O(z19));
endmodule


