// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n57_, new_n61_, new_n63_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n91_, new_n92_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x12), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n46_), .c(new_n45_), .O(z02));
  inv1   g13(.a(new_n43_), .O(z03));
  nand2  g14(.a(new_n43_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nand2  g16(.a(new_n43_), .b(new_n57_), .O(z05));
  nor2   g17(.a(new_n46_), .b(x12), .O(z06));
  and2   g18(.a(new_n43_), .b(x17), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(z03), .O(z09));
  and2   g23(.a(new_n63_), .b(new_n43_), .O(z10));
  nand2  g24(.a(new_n43_), .b(new_n47_), .O(z11));
  inv1   g25(.a(x13), .O(new_n67_));
  nand2  g26(.a(new_n51_), .b(new_n67_), .O(new_n68_));
  inv1   g27(.a(x00), .O(new_n69_));
  nand3  g28(.a(x10), .b(x08), .c(new_n69_), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n68_), .c(new_n46_), .d(x12), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z12));
  inv1   g31(.a(x01), .O(new_n73_));
  oai21  g32(.a(new_n51_), .b(new_n73_), .c(x14), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n46_), .c(new_n45_), .O(z13));
  nand3  g34(.a(x10), .b(x08), .c(x02), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n46_), .c(new_n45_), .O(z14));
  nand3  g36(.a(new_n51_), .b(x16), .c(new_n47_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x03), .O(new_n79_));
  and2   g38(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n78_), .c(new_n45_), .O(z15));
  nand3  g40(.a(x10), .b(x08), .c(x04), .O(new_n82_));
  aoi21  g41(.a(new_n51_), .b(x17), .c(x15), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n45_), .O(z16));
  nand3  g43(.a(x10), .b(x08), .c(x05), .O(new_n85_));
  aoi21  g44(.a(new_n51_), .b(x18), .c(x15), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n45_), .O(z17));
  nand3  g46(.a(x10), .b(x08), .c(x06), .O(new_n88_));
  aoi21  g47(.a(new_n51_), .b(x19), .c(x15), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n45_), .O(z18));
  nand3  g49(.a(x10), .b(x08), .c(x07), .O(new_n91_));
  aoi21  g50(.a(new_n51_), .b(x20), .c(x15), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n45_), .O(z19));
endmodule


