// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x18), .O(z04));
  nand2  g02(.a(z04), .b(x05), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x05), .O(new_n47_));
  nor2   g06(.a(x18), .b(new_n47_), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x10), .c(x08), .O(new_n50_));
  nor3   g09(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(z01));
  nand3  g10(.a(new_n46_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n44_), .O(z02));
  nand2  g16(.a(x10), .b(x08), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x14), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n44_), .c(x15), .d(x12), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z03));
  inv1   g20(.a(x19), .O(new_n62_));
  nor2   g21(.a(new_n48_), .b(new_n62_), .O(z05));
  nor2   g22(.a(new_n48_), .b(new_n46_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nor2   g24(.a(new_n48_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n44_), .b(new_n67_), .O(z08));
  nand2  g27(.a(x09), .b(x08), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n48_), .O(z09));
  and2   g29(.a(new_n69_), .b(new_n44_), .O(z10));
  nor2   g30(.a(new_n48_), .b(new_n49_), .O(z11));
  inv1   g31(.a(x00), .O(new_n73_));
  nand4  g32(.a(new_n46_), .b(x10), .c(x08), .d(new_n73_), .O(new_n74_));
  aoi21  g33(.a(z04), .b(x05), .c(new_n54_), .O(new_n75_));
  inv1   g34(.a(x13), .O(new_n76_));
  nand2  g35(.a(new_n52_), .b(new_n76_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z12));
  inv1   g38(.a(new_n75_), .O(new_n80_));
  inv1   g39(.a(x01), .O(new_n81_));
  oai21  g40(.a(new_n58_), .b(new_n81_), .c(x14), .O(new_n82_));
  nand3  g41(.a(x15), .b(x10), .c(x08), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z13));
  inv1   g43(.a(x02), .O(new_n85_));
  oai21  g44(.a(new_n58_), .b(new_n85_), .c(new_n46_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n83_), .c(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n44_), .O(z14));
  inv1   g47(.a(x03), .O(new_n89_));
  nand4  g48(.a(new_n46_), .b(x10), .c(x08), .d(new_n89_), .O(new_n90_));
  nand2  g49(.a(x16), .b(new_n49_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n52_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n90_), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n44_), .O(z15));
  inv1   g53(.a(x04), .O(new_n95_));
  nand4  g54(.a(new_n46_), .b(x10), .c(x08), .d(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n52_), .b(new_n65_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(new_n75_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z16));
  nand2  g58(.a(new_n52_), .b(x18), .O(new_n100_));
  aoi22  g59(.a(new_n100_), .b(new_n47_), .c(x18), .d(new_n54_), .O(z17));
  inv1   g60(.a(x06), .O(new_n102_));
  nand4  g61(.a(new_n46_), .b(x10), .c(x08), .d(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n52_), .b(new_n62_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(new_n103_), .c(new_n75_), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z18));
  inv1   g65(.a(x07), .O(new_n107_));
  nand4  g66(.a(new_n46_), .b(x10), .c(x08), .d(new_n107_), .O(new_n108_));
  inv1   g67(.a(x20), .O(new_n109_));
  nand2  g68(.a(new_n52_), .b(new_n109_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(new_n108_), .c(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n44_), .O(z19));
endmodule


