// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x02), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(x14), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x15), .c(new_n49_), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n45_), .O(z02));
  nand2  g14(.a(z01), .b(x12), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n45_), .O(z03));
  nand2  g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n45_), .b(new_n59_), .O(z05));
  nand2  g19(.a(new_n44_), .b(x02), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  aoi21  g21(.a(new_n44_), .b(new_n43_), .c(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n45_), .O(z09));
  nand3  g26(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n45_), .b(new_n49_), .O(z11));
  nand2  g28(.a(x15), .b(x13), .O(new_n70_));
  and2   g29(.a(x10), .b(x08), .O(new_n71_));
  nand2  g30(.a(new_n44_), .b(x00), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  aoi21  g33(.a(new_n50_), .b(new_n74_), .c(new_n43_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n70_), .c(new_n52_), .O(z12));
  inv1   g36(.a(x01), .O(new_n78_));
  oai21  g37(.a(new_n50_), .b(new_n78_), .c(x02), .O(new_n79_));
  nand2  g38(.a(new_n47_), .b(new_n49_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  aoi21  g40(.a(new_n79_), .b(new_n44_), .c(new_n81_), .O(z13));
  nor2   g41(.a(new_n71_), .b(x15), .O(new_n83_));
  nand2  g42(.a(new_n47_), .b(x12), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n83_), .c(new_n45_), .O(z14));
  nand4  g44(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x02), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n44_), .O(new_n88_));
  nand2  g47(.a(x16), .b(x12), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n51_), .c(new_n88_), .O(z15));
  nand2  g49(.a(x17), .b(x15), .O(new_n91_));
  nand2  g50(.a(new_n44_), .b(x04), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n71_), .O(new_n93_));
  aoi21  g52(.a(new_n50_), .b(new_n62_), .c(new_n43_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n91_), .c(new_n52_), .O(z16));
  nand2  g55(.a(x18), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n44_), .b(x05), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n71_), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  aoi21  g59(.a(new_n50_), .b(new_n100_), .c(new_n43_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n97_), .c(new_n52_), .O(z17));
  nand2  g62(.a(x19), .b(x15), .O(new_n104_));
  nand2  g63(.a(new_n44_), .b(x06), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n71_), .O(new_n106_));
  aoi21  g65(.a(new_n50_), .b(new_n59_), .c(new_n43_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n104_), .c(new_n52_), .O(z18));
  nand4  g68(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x02), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n44_), .O(new_n112_));
  nand2  g71(.a(new_n71_), .b(new_n44_), .O(new_n113_));
  nand3  g72(.a(new_n113_), .b(x20), .c(x12), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n112_), .O(z19));
endmodule


