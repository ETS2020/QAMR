// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x17), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x15), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(x14), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x15), .c(new_n49_), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n45_), .O(z02));
  nand2  g14(.a(z01), .b(x12), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n45_), .O(z03));
  nor2   g16(.a(new_n44_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n45_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z06));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n45_), .O(z09));
  inv1   g25(.a(z09), .O(z10));
  nor2   g26(.a(new_n44_), .b(new_n49_), .O(z11));
  inv1   g27(.a(x13), .O(new_n69_));
  nand2  g28(.a(x12), .b(x00), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n50_), .c(new_n43_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n61_), .O(new_n72_));
  oai21  g31(.a(new_n50_), .b(x15), .c(x12), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n69_), .c(new_n72_), .O(z12));
  nand3  g33(.a(x10), .b(x08), .c(x01), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x14), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n47_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n45_), .O(z13));
  and2   g38(.a(x10), .b(x08), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(x02), .c(x15), .O(new_n81_));
  nand2  g40(.a(new_n47_), .b(x12), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n81_), .c(new_n45_), .O(z14));
  nand2  g42(.a(x12), .b(x03), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n50_), .c(new_n43_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n61_), .O(new_n86_));
  nand2  g45(.a(x16), .b(new_n49_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n73_), .c(new_n86_), .O(z15));
  nand4  g47(.a(new_n61_), .b(x10), .c(x08), .d(x04), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n43_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n45_), .O(z16));
  inv1   g51(.a(x18), .O(new_n93_));
  nand2  g52(.a(x12), .b(x05), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n50_), .c(new_n43_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n61_), .O(new_n96_));
  oai21  g55(.a(new_n73_), .b(new_n93_), .c(new_n96_), .O(z17));
  nand2  g56(.a(x12), .b(x06), .O(new_n98_));
  oai21  g57(.a(new_n98_), .b(new_n50_), .c(new_n43_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n61_), .O(new_n100_));
  oai21  g59(.a(new_n73_), .b(new_n59_), .c(new_n100_), .O(z18));
  nand2  g60(.a(x20), .b(x15), .O(new_n102_));
  nand2  g61(.a(new_n61_), .b(x07), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n80_), .O(new_n104_));
  inv1   g63(.a(x20), .O(new_n105_));
  aoi21  g64(.a(new_n50_), .b(new_n105_), .c(x17), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n102_), .c(new_n52_), .O(z19));
  buf    g67(.a(x17), .O(z07));
endmodule


