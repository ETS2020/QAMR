// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  inv1   g03(.a(x16), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n43_), .b(new_n42_), .c(new_n47_), .O(z00));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(x15), .b(x10), .c(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n46_), .O(z01));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x15), .c(new_n49_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(new_n45_), .c(new_n44_), .O(z02));
  aoi21  g16(.a(new_n52_), .b(new_n45_), .c(new_n44_), .O(z03));
  nand2  g17(.a(new_n47_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n46_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x15), .O(new_n62_));
  nor2   g21(.a(new_n46_), .b(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n46_), .b(new_n64_), .O(z07));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n47_), .O(z09));
  inv1   g26(.a(z09), .O(z10));
  nor2   g27(.a(new_n46_), .b(new_n49_), .O(z11));
  nor2   g28(.a(new_n54_), .b(x15), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x00), .O(new_n71_));
  nand3  g30(.a(new_n62_), .b(x10), .c(x08), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(x13), .c(x16), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n44_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n54_), .b(new_n75_), .c(x14), .O(new_n76_));
  nor2   g35(.a(new_n51_), .b(x16), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n44_), .O(z13));
  nand3  g37(.a(x10), .b(x08), .c(x02), .O(new_n79_));
  nor2   g38(.a(x16), .b(new_n44_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  aoi21  g40(.a(new_n79_), .b(new_n62_), .c(new_n81_), .O(z14));
  nand2  g41(.a(new_n80_), .b(x03), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n72_), .O(z15));
  inv1   g43(.a(x04), .O(new_n85_));
  nand4  g44(.a(new_n62_), .b(x10), .c(x08), .d(new_n85_), .O(new_n86_));
  nand2  g45(.a(new_n72_), .b(new_n64_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n86_), .c(new_n80_), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z16));
  nand2  g48(.a(new_n70_), .b(x05), .O(new_n90_));
  aoi21  g49(.a(new_n72_), .b(x18), .c(x16), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n44_), .O(z17));
  inv1   g51(.a(x06), .O(new_n93_));
  nand4  g52(.a(new_n62_), .b(x10), .c(x08), .d(new_n93_), .O(new_n94_));
  nand2  g53(.a(new_n72_), .b(new_n60_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(new_n80_), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z18));
  inv1   g56(.a(x07), .O(new_n98_));
  nand4  g57(.a(new_n62_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n72_), .b(new_n43_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n80_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z19));
  buf    g61(.a(x16), .O(z08));
endmodule


