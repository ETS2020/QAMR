// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x18), .O(new_n42_));
  nor2   g01(.a(new_n42_), .b(x15), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(new_n43_), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n47_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n46_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n46_), .O(z02));
  nor2   g15(.a(new_n49_), .b(new_n53_), .O(z03));
  nand2  g16(.a(x18), .b(x15), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n43_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n43_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n46_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n46_), .O(z09));
  nand3  g25(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nand2  g26(.a(new_n46_), .b(new_n47_), .O(z11));
  inv1   g27(.a(x13), .O(new_n69_));
  inv1   g28(.a(x15), .O(new_n70_));
  nand2  g29(.a(x12), .b(x00), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n51_), .c(new_n42_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g32(.a(new_n51_), .b(x15), .c(x12), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n69_), .c(new_n73_), .O(z12));
  oai21  g34(.a(new_n48_), .b(x14), .c(x15), .O(new_n76_));
  nand2  g35(.a(new_n48_), .b(x01), .O(new_n77_));
  nor2   g36(.a(x18), .b(new_n47_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n53_), .O(z13));
  nand2  g39(.a(new_n51_), .b(x15), .O(new_n81_));
  nand3  g40(.a(new_n52_), .b(new_n42_), .c(x02), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n53_), .O(z14));
  nand2  g42(.a(x12), .b(x03), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n51_), .c(new_n42_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n70_), .O(new_n86_));
  nand2  g45(.a(x16), .b(new_n47_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n74_), .c(new_n86_), .O(z15));
  aoi21  g47(.a(x10), .b(x08), .c(x18), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(x15), .c(x17), .O(new_n90_));
  nand3  g49(.a(new_n52_), .b(new_n42_), .c(x04), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n53_), .O(z16));
  nand4  g51(.a(new_n70_), .b(x10), .c(x08), .d(x05), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n42_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n46_), .O(z17));
  nand2  g55(.a(x12), .b(x06), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n51_), .c(new_n42_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n70_), .O(new_n99_));
  oai21  g58(.a(new_n74_), .b(new_n59_), .c(new_n99_), .O(z18));
  nand2  g59(.a(x20), .b(x15), .O(new_n101_));
  nand2  g60(.a(new_n70_), .b(x07), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  inv1   g62(.a(x20), .O(new_n104_));
  aoi21  g63(.a(new_n51_), .b(new_n104_), .c(x18), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n101_), .c(new_n53_), .O(z19));
  buf    g66(.a(x15), .O(z06));
endmodule


