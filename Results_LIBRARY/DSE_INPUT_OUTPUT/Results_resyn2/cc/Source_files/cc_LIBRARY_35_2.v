// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x14), .O(new_n43_));
  nor2   g02(.a(x15), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n43_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n45_), .O(z01));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x15), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  oai21  g12(.a(new_n51_), .b(x14), .c(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n45_), .O(z02));
  nor2   g14(.a(new_n48_), .b(new_n52_), .O(z03));
  nand2  g15(.a(new_n45_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n58_), .O(z05));
  nor2   g18(.a(x15), .b(x14), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n44_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g27(.a(x15), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n43_), .O(z11));
  inv1   g29(.a(x13), .O(new_n71_));
  nand2  g30(.a(x12), .b(x00), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n50_), .c(new_n43_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  oai21  g33(.a(new_n50_), .b(x15), .c(x12), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n71_), .c(new_n74_), .O(z12));
  nand2  g35(.a(new_n47_), .b(x15), .O(new_n77_));
  aoi22  g36(.a(new_n77_), .b(new_n43_), .c(x15), .d(new_n52_), .O(z13));
  aoi21  g37(.a(new_n60_), .b(x02), .c(new_n50_), .O(new_n79_));
  oai21  g38(.a(new_n47_), .b(x15), .c(x12), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n79_), .O(z14));
  inv1   g40(.a(x03), .O(new_n82_));
  nand2  g41(.a(new_n51_), .b(new_n82_), .O(new_n83_));
  oai21  g42(.a(new_n50_), .b(x15), .c(new_n64_), .O(new_n84_));
  nor2   g43(.a(x14), .b(new_n52_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z15));
  nand2  g46(.a(x17), .b(x15), .O(new_n88_));
  nand2  g47(.a(new_n69_), .b(x04), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n47_), .O(new_n90_));
  aoi21  g49(.a(new_n50_), .b(new_n62_), .c(x14), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n88_), .c(new_n52_), .O(z16));
  inv1   g52(.a(x18), .O(new_n94_));
  nand2  g53(.a(x12), .b(x05), .O(new_n95_));
  oai21  g54(.a(new_n95_), .b(new_n50_), .c(new_n43_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n69_), .O(new_n97_));
  oai21  g56(.a(new_n75_), .b(new_n94_), .c(new_n97_), .O(z17));
  nand2  g57(.a(x12), .b(x06), .O(new_n99_));
  oai21  g58(.a(new_n99_), .b(new_n50_), .c(new_n43_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n69_), .O(new_n101_));
  oai21  g60(.a(new_n75_), .b(new_n58_), .c(new_n101_), .O(z18));
  nand2  g61(.a(x20), .b(x15), .O(new_n103_));
  nand2  g62(.a(new_n69_), .b(x07), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  inv1   g64(.a(x20), .O(new_n106_));
  aoi21  g65(.a(new_n50_), .b(new_n106_), .c(x14), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n103_), .c(new_n52_), .O(z19));
endmodule


