// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n57_, new_n60_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x19), .b(x18), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(new_n44_), .b(new_n46_), .O(z11));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(z11), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand3  g10(.a(new_n44_), .b(x12), .c(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n51_), .b(new_n46_), .c(new_n52_), .O(z02));
  nand2  g12(.a(new_n46_), .b(x12), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(new_n48_), .c(new_n44_), .O(z03));
  inv1   g14(.a(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x18), .O(z05));
  nand2  g17(.a(new_n44_), .b(new_n50_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n44_), .b(new_n60_), .O(z07));
  and2   g20(.a(new_n44_), .b(x16), .O(z08));
  nand3  g21(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  inv1   g22(.a(z10), .O(z09));
  inv1   g23(.a(x12), .O(new_n65_));
  aoi21  g24(.a(x19), .b(x18), .c(new_n65_), .O(new_n66_));
  inv1   g25(.a(x00), .O(new_n67_));
  nand4  g26(.a(new_n50_), .b(x10), .c(x08), .d(new_n67_), .O(new_n68_));
  inv1   g27(.a(x13), .O(new_n69_));
  nand2  g28(.a(x10), .b(x08), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(x15), .c(new_n69_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n68_), .c(new_n66_), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  nand2  g32(.a(new_n44_), .b(x12), .O(new_n74_));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n70_), .b(new_n75_), .c(x14), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n48_), .c(new_n74_), .O(z13));
  inv1   g36(.a(x02), .O(new_n78_));
  oai21  g37(.a(new_n70_), .b(new_n78_), .c(new_n50_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n48_), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n44_), .O(z14));
  nand3  g40(.a(new_n51_), .b(x16), .c(new_n46_), .O(new_n82_));
  nand4  g41(.a(new_n50_), .b(x10), .c(x08), .d(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n74_), .O(z15));
  inv1   g43(.a(x04), .O(new_n85_));
  nand4  g44(.a(new_n50_), .b(x10), .c(x08), .d(new_n85_), .O(new_n86_));
  oai21  g45(.a(new_n70_), .b(x15), .c(new_n60_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n86_), .c(new_n66_), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z16));
  inv1   g48(.a(x05), .O(new_n90_));
  nand4  g49(.a(new_n50_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n70_), .b(x15), .c(z04), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n66_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z17));
  inv1   g53(.a(x06), .O(new_n95_));
  nand4  g54(.a(new_n50_), .b(x10), .c(x08), .d(new_n95_), .O(new_n96_));
  oai21  g55(.a(new_n70_), .b(x15), .c(new_n57_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(new_n66_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z18));
  inv1   g58(.a(x07), .O(new_n100_));
  nand4  g59(.a(new_n50_), .b(x10), .c(x08), .d(new_n100_), .O(new_n101_));
  oai21  g60(.a(new_n70_), .b(x15), .c(new_n43_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(new_n101_), .c(new_n66_), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z19));
endmodule


