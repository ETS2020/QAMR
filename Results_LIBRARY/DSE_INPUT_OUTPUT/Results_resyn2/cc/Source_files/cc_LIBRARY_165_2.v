// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x05), .O(new_n44_));
  nor2   g03(.a(x18), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n45_), .O(z01));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  inv1   g12(.a(x18), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x05), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n53_), .b(new_n47_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  oai21  g17(.a(new_n50_), .b(new_n58_), .c(new_n55_), .O(z03));
  nor2   g18(.a(x18), .b(x05), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n55_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n55_), .b(new_n52_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z08));
  nand3  g26(.a(new_n55_), .b(x09), .c(x08), .O(z10));
  inv1   g27(.a(z10), .O(z09));
  nor2   g28(.a(new_n45_), .b(new_n47_), .O(z11));
  aoi21  g29(.a(new_n54_), .b(x05), .c(new_n58_), .O(new_n71_));
  inv1   g30(.a(x00), .O(new_n72_));
  nand4  g31(.a(new_n52_), .b(x10), .c(x08), .d(new_n72_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(x15), .c(new_n74_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  nand2  g37(.a(new_n55_), .b(x12), .O(new_n79_));
  inv1   g38(.a(x01), .O(new_n80_));
  oai21  g39(.a(new_n75_), .b(new_n80_), .c(x14), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n48_), .c(new_n79_), .O(z13));
  inv1   g41(.a(x02), .O(new_n83_));
  oai21  g42(.a(new_n75_), .b(new_n83_), .c(new_n52_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(new_n48_), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n55_), .O(z14));
  nand3  g45(.a(new_n53_), .b(x16), .c(new_n47_), .O(new_n87_));
  nand4  g46(.a(new_n52_), .b(x10), .c(x08), .d(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n79_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand4  g49(.a(new_n52_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n75_), .b(x15), .c(new_n64_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n71_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  nor2   g53(.a(new_n54_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n53_), .b(x18), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n44_), .c(new_n95_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand4  g57(.a(new_n52_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  oai21  g58(.a(new_n75_), .b(x15), .c(new_n61_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n71_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand4  g62(.a(new_n52_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  oai21  g63(.a(new_n75_), .b(x15), .c(new_n43_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n55_), .O(z19));
endmodule


