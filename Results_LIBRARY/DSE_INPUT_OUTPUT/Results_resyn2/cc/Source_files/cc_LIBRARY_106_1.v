// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x17), .O(new_n42_));
  nor2   g01(.a(new_n42_), .b(x15), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(new_n43_), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x15), .O(new_n50_));
  inv1   g09(.a(x12), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x11), .O(new_n52_));
  oai21  g11(.a(new_n50_), .b(x14), .c(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n48_), .O(z02));
  nand2  g13(.a(z01), .b(x12), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n48_), .O(z03));
  nor2   g15(.a(new_n43_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n48_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x15), .O(new_n60_));
  nor2   g19(.a(new_n42_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n43_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n48_), .O(z09));
  nand3  g24(.a(new_n48_), .b(x09), .c(x08), .O(z10));
  inv1   g25(.a(x14), .O(new_n67_));
  nor2   g26(.a(new_n43_), .b(new_n67_), .O(z11));
  aoi21  g27(.a(x10), .b(x08), .c(x17), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(x15), .c(x13), .O(new_n70_));
  nand3  g29(.a(new_n50_), .b(new_n42_), .c(x00), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n70_), .c(new_n51_), .O(z12));
  nand3  g31(.a(x10), .b(x08), .c(x01), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x14), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n48_), .O(z13));
  nand2  g36(.a(new_n49_), .b(x15), .O(new_n78_));
  nand3  g37(.a(new_n50_), .b(new_n42_), .c(x02), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n51_), .O(z14));
  nor2   g39(.a(new_n62_), .b(x14), .O(new_n81_));
  oai21  g40(.a(new_n69_), .b(x15), .c(new_n81_), .O(new_n82_));
  nand3  g41(.a(new_n50_), .b(new_n42_), .c(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n51_), .O(z15));
  nand4  g43(.a(new_n60_), .b(x10), .c(x08), .d(x04), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n42_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n48_), .O(z16));
  nand2  g47(.a(x18), .b(x15), .O(new_n89_));
  and2   g48(.a(x10), .b(x08), .O(new_n90_));
  nand2  g49(.a(new_n60_), .b(x05), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g51(.a(x18), .O(new_n93_));
  aoi21  g52(.a(new_n49_), .b(new_n93_), .c(x17), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n89_), .c(new_n51_), .O(z17));
  nand2  g55(.a(x19), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n60_), .b(x06), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n90_), .O(new_n99_));
  aoi21  g58(.a(new_n49_), .b(new_n58_), .c(x17), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n97_), .c(new_n51_), .O(z18));
  nand2  g61(.a(x12), .b(x07), .O(new_n103_));
  oai21  g62(.a(new_n103_), .b(new_n49_), .c(new_n42_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n60_), .O(new_n105_));
  nand2  g64(.a(x20), .b(x12), .O(new_n106_));
  oai21  g65(.a(new_n106_), .b(new_n50_), .c(new_n105_), .O(z19));
  buf    g66(.a(x15), .O(z06));
endmodule


