// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n55_, new_n58_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x12), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  aoi21  g08(.a(x10), .b(x08), .c(x14), .O(new_n50_));
  nand2  g09(.a(new_n46_), .b(x12), .O(new_n51_));
  nor3   g10(.a(new_n51_), .b(new_n50_), .c(x11), .O(z02));
  inv1   g11(.a(new_n43_), .O(z03));
  nor2   g12(.a(z03), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nor2   g14(.a(z03), .b(new_n55_), .O(z05));
  nor2   g15(.a(new_n46_), .b(x12), .O(z06));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z07));
  and2   g18(.a(new_n43_), .b(x16), .O(z08));
  nand3  g19(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g20(.a(z10), .O(z09));
  nand2  g21(.a(new_n43_), .b(new_n47_), .O(z11));
  nand3  g22(.a(x10), .b(x08), .c(x00), .O(new_n64_));
  nand2  g23(.a(x10), .b(x08), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(x13), .c(x15), .O(new_n66_));
  aoi21  g25(.a(new_n66_), .b(new_n64_), .c(new_n45_), .O(z12));
  inv1   g26(.a(x01), .O(new_n68_));
  oai21  g27(.a(new_n65_), .b(new_n68_), .c(x14), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n46_), .c(new_n45_), .O(z13));
  nand3  g29(.a(x10), .b(x08), .c(x02), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n46_), .c(new_n45_), .O(z14));
  nand3  g31(.a(x10), .b(x08), .c(x03), .O(new_n73_));
  nand2  g32(.a(new_n50_), .b(x16), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n73_), .c(new_n51_), .O(z15));
  nor2   g34(.a(x15), .b(new_n45_), .O(new_n76_));
  inv1   g35(.a(x04), .O(new_n77_));
  nand3  g36(.a(x10), .b(x08), .c(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n65_), .b(new_n58_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z16));
  inv1   g40(.a(x05), .O(new_n82_));
  nand3  g41(.a(x10), .b(x08), .c(new_n82_), .O(new_n83_));
  inv1   g42(.a(x18), .O(new_n84_));
  nand2  g43(.a(new_n65_), .b(new_n84_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n83_), .c(new_n76_), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z17));
  inv1   g46(.a(x06), .O(new_n88_));
  nand3  g47(.a(x10), .b(x08), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n65_), .b(new_n55_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n76_), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z18));
  nand3  g51(.a(x10), .b(x08), .c(x07), .O(new_n93_));
  aoi21  g52(.a(new_n65_), .b(x20), .c(x15), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n45_), .O(z19));
endmodule


