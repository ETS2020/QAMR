// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x17), .b(x16), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(x16), .O(new_n46_));
  inv1   g05(.a(x17), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(x15), .b(x10), .c(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n48_), .O(z01));
  inv1   g12(.a(x15), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x10), .c(x08), .O(new_n55_));
  nand3  g14(.a(new_n48_), .b(x12), .c(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(new_n49_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  oai21  g17(.a(new_n52_), .b(new_n58_), .c(new_n48_), .O(z03));
  nand2  g18(.a(new_n48_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n48_), .b(new_n61_), .O(z05));
  nor2   g21(.a(new_n44_), .b(new_n54_), .O(z06));
  nand2  g22(.a(new_n47_), .b(x16), .O(z07));
  nand2  g23(.a(x17), .b(new_n46_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n48_), .O(z09));
  nand3  g26(.a(new_n48_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n48_), .b(new_n49_), .O(z11));
  nor2   g28(.a(new_n44_), .b(new_n58_), .O(new_n70_));
  inv1   g29(.a(x00), .O(new_n71_));
  nand4  g30(.a(new_n54_), .b(x10), .c(x08), .d(new_n71_), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(x15), .c(new_n73_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z12));
  nand2  g36(.a(new_n50_), .b(new_n49_), .O(new_n78_));
  nand4  g37(.a(new_n54_), .b(x10), .c(x08), .d(x01), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n78_), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n48_), .O(z13));
  inv1   g40(.a(x02), .O(new_n82_));
  oai21  g41(.a(new_n74_), .b(new_n82_), .c(new_n54_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n50_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n48_), .O(z14));
  nand3  g44(.a(new_n55_), .b(x16), .c(new_n49_), .O(new_n86_));
  and2   g45(.a(x10), .b(x08), .O(new_n87_));
  nand4  g46(.a(new_n87_), .b(new_n48_), .c(new_n54_), .d(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n86_), .c(new_n58_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand4  g49(.a(new_n54_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n74_), .b(x15), .c(new_n47_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n48_), .O(z16));
  inv1   g53(.a(x05), .O(new_n95_));
  nand4  g54(.a(new_n54_), .b(x10), .c(x08), .d(new_n95_), .O(new_n96_));
  inv1   g55(.a(x18), .O(new_n97_));
  oai21  g56(.a(new_n74_), .b(x15), .c(new_n97_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(new_n96_), .c(new_n70_), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z17));
  inv1   g59(.a(x06), .O(new_n101_));
  nand4  g60(.a(new_n54_), .b(x10), .c(x08), .d(new_n101_), .O(new_n102_));
  oai21  g61(.a(new_n74_), .b(x15), .c(new_n61_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n102_), .c(new_n70_), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z18));
  inv1   g64(.a(x07), .O(new_n106_));
  nand4  g65(.a(new_n54_), .b(x10), .c(x08), .d(new_n106_), .O(new_n107_));
  oai21  g66(.a(new_n74_), .b(x15), .c(new_n43_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(new_n70_), .O(new_n109_));
  inv1   g68(.a(new_n109_), .O(z19));
endmodule


