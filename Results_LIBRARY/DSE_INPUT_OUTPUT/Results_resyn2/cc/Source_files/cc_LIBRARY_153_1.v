// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n59_, new_n62_,
    new_n64_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x08), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x10), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x08), .c(new_n44_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n50_), .O(z02));
  inv1   g13(.a(x08), .O(new_n55_));
  inv1   g14(.a(x12), .O(new_n56_));
  nor4   g15(.a(new_n48_), .b(new_n44_), .c(new_n56_), .d(new_n55_), .O(z03));
  nor2   g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n50_), .b(new_n59_), .O(z05));
  nor2   g19(.a(new_n44_), .b(new_n55_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n50_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n50_), .b(new_n64_), .O(z08));
  and2   g24(.a(x09), .b(x08), .O(z09));
  nor2   g25(.a(z09), .b(new_n45_), .O(z10));
  aoi21  g26(.a(x15), .b(new_n55_), .c(new_n47_), .O(z11));
  inv1   g27(.a(x13), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(new_n56_), .c(new_n44_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n55_), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  nand4  g32(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n74_));
  oai21  g33(.a(new_n73_), .b(new_n69_), .c(new_n74_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n71_), .O(z12));
  inv1   g36(.a(new_n51_), .O(new_n78_));
  nand3  g37(.a(x15), .b(x10), .c(x08), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(z11), .c(x12), .O(new_n81_));
  aoi21  g40(.a(new_n78_), .b(x01), .c(new_n81_), .O(z13));
  nand2  g41(.a(x15), .b(new_n72_), .O(new_n83_));
  nand2  g42(.a(new_n73_), .b(x02), .O(new_n84_));
  nand2  g43(.a(x12), .b(x08), .O(new_n85_));
  aoi21  g44(.a(new_n84_), .b(new_n83_), .c(new_n85_), .O(z14));
  nand2  g45(.a(x16), .b(new_n47_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n56_), .c(new_n44_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n55_), .O(new_n89_));
  nand4  g48(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  oai21  g49(.a(new_n87_), .b(new_n73_), .c(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n89_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand2  g53(.a(x10), .b(x08), .O(new_n95_));
  oai22  g54(.a(new_n95_), .b(new_n94_), .c(new_n62_), .d(x08), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n44_), .O(new_n97_));
  nand2  g56(.a(new_n44_), .b(x10), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(x17), .c(x08), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n97_), .c(new_n56_), .O(z16));
  inv1   g59(.a(x05), .O(new_n101_));
  inv1   g60(.a(x18), .O(new_n102_));
  oai22  g61(.a(new_n95_), .b(new_n101_), .c(new_n102_), .d(x08), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n44_), .O(new_n104_));
  nand3  g63(.a(new_n98_), .b(x18), .c(x08), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n56_), .O(z17));
  inv1   g65(.a(x06), .O(new_n107_));
  oai22  g66(.a(new_n95_), .b(new_n107_), .c(new_n59_), .d(x08), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n44_), .O(new_n109_));
  nand3  g68(.a(new_n98_), .b(x19), .c(x08), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n56_), .O(z18));
  inv1   g70(.a(x07), .O(new_n112_));
  oai22  g71(.a(new_n95_), .b(new_n112_), .c(new_n43_), .d(x08), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n44_), .O(new_n114_));
  nand3  g73(.a(new_n98_), .b(x20), .c(x08), .O(new_n115_));
  aoi21  g74(.a(new_n115_), .b(new_n114_), .c(new_n56_), .O(z19));
endmodule


