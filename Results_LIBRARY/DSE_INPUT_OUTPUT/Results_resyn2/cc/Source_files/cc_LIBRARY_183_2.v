// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x01), .O(new_n44_));
  nor2   g03(.a(x10), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(new_n45_), .O(new_n47_));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(x14), .c(new_n47_), .O(z01));
  nand2  g08(.a(new_n47_), .b(x14), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nor2   g10(.a(x15), .b(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x10), .O(new_n53_));
  nand2  g12(.a(x12), .b(new_n42_), .O(new_n54_));
  aoi21  g13(.a(new_n53_), .b(new_n50_), .c(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nor3   g15(.a(new_n48_), .b(x14), .c(new_n56_), .O(z03));
  nor2   g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n45_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  nor2   g20(.a(new_n45_), .b(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n47_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n47_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n45_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n45_), .O(z10));
  inv1   g28(.a(x14), .O(new_n70_));
  nand2  g29(.a(new_n47_), .b(new_n70_), .O(z11));
  nor2   g30(.a(x10), .b(x01), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x13), .O(new_n73_));
  inv1   g32(.a(x00), .O(new_n74_));
  nand3  g33(.a(new_n61_), .b(x08), .c(new_n74_), .O(new_n75_));
  inv1   g34(.a(x13), .O(new_n76_));
  nand2  g35(.a(new_n61_), .b(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n75_), .c(x10), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n73_), .c(new_n56_), .O(z12));
  inv1   g39(.a(new_n48_), .O(new_n81_));
  aoi21  g40(.a(x08), .b(x01), .c(new_n70_), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n81_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n47_), .O(z13));
  nand2  g43(.a(x15), .b(new_n51_), .O(new_n85_));
  nand3  g44(.a(new_n61_), .b(x08), .c(x02), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x10), .O(new_n88_));
  nand2  g47(.a(new_n72_), .b(x15), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n56_), .O(z14));
  inv1   g49(.a(x10), .O(new_n91_));
  nand2  g50(.a(x16), .b(new_n70_), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n56_), .c(new_n44_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand4  g53(.a(new_n61_), .b(x10), .c(x08), .d(x03), .O(new_n95_));
  oai21  g54(.a(new_n92_), .b(new_n52_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n94_), .O(z15));
  oai21  g57(.a(new_n63_), .b(new_n56_), .c(new_n44_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n91_), .O(new_n100_));
  nand4  g59(.a(new_n61_), .b(x10), .c(x08), .d(x04), .O(new_n101_));
  oai21  g60(.a(new_n52_), .b(new_n63_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n100_), .O(z16));
  nand2  g63(.a(new_n72_), .b(x18), .O(new_n105_));
  inv1   g64(.a(x05), .O(new_n106_));
  nand3  g65(.a(new_n61_), .b(x08), .c(new_n106_), .O(new_n107_));
  inv1   g66(.a(x18), .O(new_n108_));
  nand2  g67(.a(new_n77_), .b(new_n108_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n107_), .c(x10), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n105_), .c(new_n56_), .O(z17));
  nand2  g70(.a(new_n72_), .b(x19), .O(new_n112_));
  inv1   g71(.a(x06), .O(new_n113_));
  nand3  g72(.a(new_n61_), .b(x08), .c(new_n113_), .O(new_n114_));
  nand2  g73(.a(new_n77_), .b(new_n59_), .O(new_n115_));
  nand3  g74(.a(new_n115_), .b(new_n114_), .c(x10), .O(new_n116_));
  aoi21  g75(.a(new_n116_), .b(new_n112_), .c(new_n56_), .O(z18));
  oai21  g76(.a(new_n43_), .b(new_n56_), .c(new_n44_), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  nand4  g78(.a(new_n61_), .b(x10), .c(x08), .d(x07), .O(new_n120_));
  oai21  g79(.a(new_n52_), .b(new_n43_), .c(new_n120_), .O(new_n121_));
  nand2  g80(.a(new_n121_), .b(x12), .O(new_n122_));
  nand2  g81(.a(new_n122_), .b(new_n119_), .O(z19));
endmodule


