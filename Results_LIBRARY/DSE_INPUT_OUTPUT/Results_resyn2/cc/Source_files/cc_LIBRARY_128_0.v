// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n61_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x16), .b(x15), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(x14), .c(new_n46_), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nor2   g08(.a(new_n44_), .b(new_n49_), .O(z11));
  inv1   g09(.a(z11), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x16), .c(new_n52_), .O(new_n55_));
  nand2  g14(.a(x12), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(new_n51_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nor3   g17(.a(new_n47_), .b(x14), .c(new_n58_), .O(z03));
  nand2  g18(.a(new_n46_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n46_), .b(new_n61_), .O(z05));
  nand2  g21(.a(x16), .b(new_n52_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n46_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(x15), .O(z08));
  aoi21  g26(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g27(.a(z10), .O(z09));
  nand2  g28(.a(new_n53_), .b(x16), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n52_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n54_), .b(x16), .c(new_n52_), .d(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n72_), .c(new_n58_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n53_), .b(new_n75_), .c(x14), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n47_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n46_), .O(z13));
  nand2  g38(.a(new_n53_), .b(x15), .O(new_n80_));
  nand4  g39(.a(new_n54_), .b(x16), .c(new_n52_), .d(x02), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n58_), .O(z14));
  nand2  g41(.a(x12), .b(x03), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n53_), .c(x16), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nand2  g44(.a(new_n54_), .b(new_n52_), .O(new_n86_));
  nand4  g45(.a(new_n86_), .b(x16), .c(new_n49_), .d(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n85_), .O(z15));
  nand2  g47(.a(x17), .b(x15), .O(new_n89_));
  nand2  g48(.a(new_n52_), .b(x04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n54_), .O(new_n91_));
  aoi21  g50(.a(new_n53_), .b(new_n64_), .c(new_n66_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n89_), .c(new_n58_), .O(z16));
  nand2  g53(.a(x18), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n52_), .b(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n54_), .O(new_n97_));
  inv1   g56(.a(x18), .O(new_n98_));
  aoi21  g57(.a(new_n53_), .b(new_n98_), .c(new_n66_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n95_), .c(new_n58_), .O(z17));
  nand2  g60(.a(x12), .b(x06), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n53_), .c(x16), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n52_), .O(new_n104_));
  nand3  g63(.a(new_n86_), .b(x19), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z18));
  nand2  g65(.a(x20), .b(x15), .O(new_n107_));
  nand2  g66(.a(new_n52_), .b(x07), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n54_), .O(new_n109_));
  aoi21  g68(.a(new_n53_), .b(new_n43_), .c(new_n66_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n107_), .c(new_n58_), .O(z19));
endmodule


