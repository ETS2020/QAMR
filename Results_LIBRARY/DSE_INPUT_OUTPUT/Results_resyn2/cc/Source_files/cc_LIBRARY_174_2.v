// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x02), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n46_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x15), .c(new_n46_), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n44_), .O(z02));
  oai21  g14(.a(new_n48_), .b(new_n52_), .c(new_n44_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x02), .O(new_n60_));
  nand2  g19(.a(new_n43_), .b(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  aoi21  g21(.a(new_n43_), .b(x02), .c(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n44_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n44_), .O(z09));
  nand3  g26(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n44_), .b(new_n46_), .O(z11));
  nand2  g28(.a(x15), .b(x13), .O(new_n70_));
  nand2  g29(.a(new_n43_), .b(x00), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n47_), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  aoi21  g32(.a(new_n50_), .b(new_n73_), .c(x02), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n70_), .c(new_n52_), .O(z12));
  oai21  g35(.a(new_n47_), .b(x14), .c(x15), .O(new_n77_));
  nand2  g36(.a(new_n47_), .b(x01), .O(new_n78_));
  nor2   g37(.a(new_n46_), .b(x02), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n77_), .c(new_n52_), .O(z13));
  nand3  g40(.a(new_n50_), .b(x15), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n44_), .O(z14));
  nand4  g42(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n60_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n43_), .O(new_n86_));
  nand2  g45(.a(x16), .b(x12), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n51_), .c(new_n86_), .O(z15));
  nand2  g47(.a(x17), .b(x15), .O(new_n89_));
  nand2  g48(.a(new_n43_), .b(x04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n47_), .O(new_n91_));
  aoi21  g50(.a(new_n50_), .b(new_n62_), .c(x02), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n89_), .c(new_n52_), .O(z16));
  nand2  g53(.a(x18), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n43_), .b(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n47_), .O(new_n97_));
  inv1   g56(.a(x18), .O(new_n98_));
  aoi21  g57(.a(new_n50_), .b(new_n98_), .c(x02), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n95_), .c(new_n52_), .O(z17));
  nand2  g60(.a(x19), .b(x15), .O(new_n102_));
  nand2  g61(.a(new_n43_), .b(x06), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n47_), .O(new_n104_));
  aoi21  g63(.a(new_n50_), .b(new_n58_), .c(x02), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n102_), .c(new_n52_), .O(z18));
  nand4  g66(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n60_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n43_), .O(new_n110_));
  nand2  g69(.a(new_n47_), .b(new_n43_), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(x20), .c(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n110_), .O(z19));
endmodule


