// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  inv1   g03(.a(x18), .O(z04));
  nor2   g04(.a(z04), .b(new_n44_), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n43_), .b(new_n42_), .c(new_n47_), .O(z00));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x15), .c(new_n49_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(z01));
  inv1   g12(.a(x15), .O(new_n54_));
  nand2  g13(.a(new_n51_), .b(new_n54_), .O(new_n55_));
  nor2   g14(.a(x18), .b(new_n44_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  aoi21  g16(.a(new_n55_), .b(new_n49_), .c(new_n57_), .O(z02));
  aoi21  g17(.a(new_n52_), .b(z04), .c(new_n44_), .O(z03));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n47_), .b(new_n60_), .O(z05));
  nor2   g20(.a(new_n46_), .b(new_n54_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n46_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n47_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n47_), .O(z09));
  nand3  g27(.a(new_n47_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n47_), .b(new_n49_), .O(z11));
  inv1   g29(.a(x00), .O(new_n71_));
  nand4  g30(.a(new_n54_), .b(x10), .c(x08), .d(new_n71_), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  oai21  g32(.a(new_n50_), .b(x15), .c(new_n73_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n72_), .c(new_n56_), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  nand2  g35(.a(new_n51_), .b(x15), .O(new_n77_));
  inv1   g36(.a(new_n56_), .O(new_n78_));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n50_), .b(new_n79_), .c(x14), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n77_), .c(new_n78_), .O(z13));
  nand3  g40(.a(new_n51_), .b(new_n54_), .c(x02), .O(new_n82_));
  aoi21  g41(.a(new_n50_), .b(x15), .c(x18), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n44_), .O(z14));
  nand2  g43(.a(x16), .b(new_n49_), .O(new_n85_));
  inv1   g44(.a(x03), .O(new_n86_));
  nand4  g45(.a(new_n54_), .b(x10), .c(x08), .d(new_n86_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n56_), .O(new_n88_));
  aoi21  g47(.a(new_n85_), .b(new_n55_), .c(new_n88_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand4  g49(.a(new_n54_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n50_), .b(x15), .c(new_n63_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n56_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  nand3  g53(.a(new_n51_), .b(new_n54_), .c(x05), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(z04), .c(new_n44_), .O(z17));
  inv1   g55(.a(x06), .O(new_n97_));
  nand4  g56(.a(new_n54_), .b(x10), .c(x08), .d(new_n97_), .O(new_n98_));
  oai21  g57(.a(new_n50_), .b(x15), .c(new_n60_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(new_n56_), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z18));
  inv1   g60(.a(x07), .O(new_n102_));
  nand4  g61(.a(new_n54_), .b(x10), .c(x08), .d(new_n102_), .O(new_n103_));
  oai21  g62(.a(new_n50_), .b(x15), .c(new_n43_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(new_n103_), .c(new_n56_), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z19));
endmodule


