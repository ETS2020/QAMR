// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n63_, new_n66_, new_n68_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x10), .O(new_n42_));
  nor2   g01(.a(x18), .b(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x18), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(x14), .O(z01));
  inv1   g08(.a(new_n43_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x14), .O(new_n51_));
  nor2   g10(.a(new_n48_), .b(x15), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x18), .O(new_n53_));
  inv1   g12(.a(x11), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n54_), .O(new_n55_));
  aoi21  g14(.a(new_n53_), .b(new_n51_), .c(new_n55_), .O(z02));
  inv1   g15(.a(x14), .O(new_n57_));
  nand3  g16(.a(x15), .b(x12), .c(x08), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(x18), .c(new_n42_), .O(z03));
  nor2   g20(.a(x18), .b(x10), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nand2  g22(.a(new_n50_), .b(new_n63_), .O(z05));
  nand2  g23(.a(new_n50_), .b(new_n46_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nand2  g27(.a(new_n50_), .b(new_n68_), .O(z08));
  nand3  g28(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  inv1   g29(.a(z10), .O(z09));
  inv1   g30(.a(new_n51_), .O(z11));
  inv1   g31(.a(x12), .O(new_n73_));
  aoi21  g32(.a(new_n46_), .b(x08), .c(new_n47_), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n42_), .c(x13), .O(new_n75_));
  nand3  g34(.a(new_n52_), .b(x18), .c(x00), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n75_), .c(new_n73_), .O(z12));
  oai21  g36(.a(new_n59_), .b(new_n47_), .c(x10), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x14), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n78_), .O(z13));
  nand4  g40(.a(new_n46_), .b(x12), .c(x08), .d(x02), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x18), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x10), .O(new_n84_));
  nand3  g43(.a(new_n48_), .b(x15), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(z14));
  nor2   g45(.a(new_n68_), .b(x14), .O(new_n87_));
  oai21  g46(.a(new_n74_), .b(new_n42_), .c(new_n87_), .O(new_n88_));
  nand3  g47(.a(new_n52_), .b(x18), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n73_), .O(z15));
  oai21  g49(.a(new_n74_), .b(new_n42_), .c(x17), .O(new_n91_));
  nand3  g50(.a(new_n52_), .b(x18), .c(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n73_), .O(z16));
  inv1   g52(.a(x05), .O(new_n94_));
  nand2  g53(.a(new_n52_), .b(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x12), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(x18), .c(z04), .O(z17));
  nand4  g56(.a(new_n46_), .b(x12), .c(x08), .d(x06), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x18), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x10), .O(new_n100_));
  nand2  g59(.a(x19), .b(x12), .O(new_n101_));
  oai21  g60(.a(new_n101_), .b(new_n52_), .c(new_n100_), .O(z18));
  inv1   g61(.a(x08), .O(new_n103_));
  oai21  g62(.a(x15), .b(new_n103_), .c(x20), .O(new_n104_));
  nand4  g63(.a(new_n46_), .b(x10), .c(x08), .d(x07), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x18), .O(new_n107_));
  nand2  g66(.a(x20), .b(new_n42_), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n73_), .O(z19));
endmodule


