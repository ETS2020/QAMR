// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x15), .b(x10), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n44_), .O(z01));
  inv1   g09(.a(x08), .O(new_n51_));
  nor2   g10(.a(x15), .b(new_n51_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x14), .O(new_n53_));
  inv1   g12(.a(x11), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n54_), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n53_), .c(new_n44_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nor2   g16(.a(new_n49_), .b(new_n57_), .O(z03));
  nor2   g17(.a(new_n43_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n44_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x15), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x10), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n43_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n44_), .O(z09));
  nand3  g28(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n44_), .b(new_n46_), .O(z11));
  nand2  g30(.a(x13), .b(new_n51_), .O(new_n72_));
  nand3  g31(.a(new_n62_), .b(x08), .c(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x10), .O(new_n75_));
  nand2  g34(.a(x15), .b(x13), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n75_), .c(new_n57_), .O(z12));
  nand2  g36(.a(x08), .b(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x10), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n62_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x14), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n47_), .c(new_n57_), .O(z13));
  nor2   g41(.a(new_n43_), .b(x12), .O(new_n83_));
  nand2  g42(.a(new_n52_), .b(x02), .O(new_n84_));
  inv1   g43(.a(x10), .O(new_n85_));
  aoi21  g44(.a(x15), .b(new_n51_), .c(new_n85_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(z14));
  nand2  g46(.a(new_n62_), .b(x08), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(x16), .c(new_n46_), .O(new_n89_));
  aoi21  g48(.a(new_n52_), .b(x03), .c(new_n43_), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n83_), .O(z15));
  nand3  g50(.a(new_n62_), .b(x08), .c(x04), .O(new_n92_));
  nand2  g51(.a(x17), .b(new_n51_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x10), .O(new_n95_));
  nand2  g54(.a(x17), .b(x15), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n57_), .O(z16));
  nand3  g56(.a(new_n62_), .b(x08), .c(x05), .O(new_n98_));
  nand2  g57(.a(x18), .b(new_n51_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x10), .O(new_n101_));
  nand2  g60(.a(x18), .b(x15), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n57_), .O(z17));
  nand3  g62(.a(x12), .b(x08), .c(x06), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x10), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n62_), .O(new_n106_));
  nand3  g65(.a(new_n88_), .b(x19), .c(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(z18));
  nand3  g67(.a(x12), .b(x08), .c(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x10), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n62_), .O(new_n111_));
  nand3  g70(.a(new_n88_), .b(x20), .c(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n111_), .O(z19));
endmodule


