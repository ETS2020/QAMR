// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_,
    new_n63_, new_n67_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_;
  nand2  g00(.a(x18), .b(x12), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n42_), .b(new_n47_), .O(z11));
  nor2   g07(.a(z11), .b(new_n46_), .O(z01));
  inv1   g08(.a(x12), .O(new_n50_));
  inv1   g09(.a(x18), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x15), .c(new_n47_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(z02));
  inv1   g15(.a(new_n46_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n51_), .c(new_n50_), .O(z03));
  nand2  g18(.a(x18), .b(new_n50_), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n42_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x15), .O(new_n63_));
  nor2   g22(.a(new_n43_), .b(new_n63_), .O(z06));
  and2   g23(.a(new_n42_), .b(x17), .O(z07));
  and2   g24(.a(new_n42_), .b(x16), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n42_), .O(z09));
  nand3  g27(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  inv1   g28(.a(x00), .O(new_n70_));
  inv1   g29(.a(new_n53_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n63_), .c(new_n70_), .O(new_n72_));
  nor2   g31(.a(x18), .b(new_n50_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  nand3  g33(.a(new_n63_), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n73_), .c(new_n72_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n53_), .b(new_n79_), .c(x14), .O(new_n80_));
  nor2   g39(.a(new_n57_), .b(x18), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n50_), .O(z13));
  aoi21  g41(.a(new_n71_), .b(x02), .c(x15), .O(new_n83_));
  nand2  g42(.a(new_n73_), .b(new_n46_), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(new_n83_), .O(z14));
  nand2  g44(.a(new_n51_), .b(x12), .O(new_n86_));
  nand3  g45(.a(new_n75_), .b(x16), .c(new_n47_), .O(new_n87_));
  nor2   g46(.a(new_n53_), .b(x15), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n87_), .c(new_n86_), .O(z15));
  nand2  g49(.a(new_n88_), .b(x04), .O(new_n91_));
  aoi21  g50(.a(new_n75_), .b(x17), .c(x18), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n50_), .O(z16));
  inv1   g52(.a(x05), .O(new_n94_));
  nor3   g53(.a(new_n86_), .b(new_n75_), .c(new_n94_), .O(z17));
  inv1   g54(.a(x06), .O(new_n96_));
  nand3  g55(.a(new_n71_), .b(new_n63_), .c(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n75_), .b(new_n61_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(new_n97_), .c(new_n73_), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z18));
  nand2  g59(.a(new_n88_), .b(x07), .O(new_n101_));
  aoi21  g60(.a(new_n75_), .b(x20), .c(x18), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n50_), .O(z19));
endmodule


