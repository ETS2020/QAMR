// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x14), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n46_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  nand3  g08(.a(x14), .b(x12), .c(new_n42_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(z02));
  nand2  g10(.a(new_n47_), .b(x12), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(x15), .c(x14), .O(z03));
  inv1   g12(.a(new_n44_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nand2  g15(.a(new_n54_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nor2   g17(.a(new_n44_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n54_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(new_n44_), .O(z09));
  aoi21  g22(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nand2  g23(.a(x15), .b(new_n46_), .O(z11));
  inv1   g24(.a(x15), .O(new_n66_));
  nand4  g25(.a(x12), .b(x10), .c(x08), .d(x00), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(x14), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g28(.a(new_n47_), .b(new_n66_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(x13), .c(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n69_), .O(z12));
  inv1   g31(.a(x12), .O(new_n73_));
  nand2  g32(.a(new_n47_), .b(x15), .O(new_n74_));
  inv1   g33(.a(x01), .O(new_n75_));
  nand2  g34(.a(x10), .b(x08), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n75_), .c(x14), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n74_), .c(new_n73_), .O(z13));
  aoi21  g37(.a(new_n47_), .b(x02), .c(x15), .O(new_n79_));
  nand2  g38(.a(new_n74_), .b(x12), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n54_), .O(z14));
  nand3  g40(.a(x16), .b(x15), .c(new_n46_), .O(new_n82_));
  nand4  g41(.a(new_n47_), .b(new_n66_), .c(x14), .d(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n73_), .O(z15));
  nand4  g43(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x14), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  nand3  g46(.a(new_n70_), .b(x17), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(z16));
  nand4  g48(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x14), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n66_), .O(new_n92_));
  nand3  g51(.a(new_n70_), .b(x18), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z17));
  nand4  g53(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x14), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n66_), .O(new_n97_));
  nand3  g56(.a(new_n70_), .b(x19), .c(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(z18));
  nand2  g58(.a(x20), .b(x15), .O(new_n100_));
  nand2  g59(.a(new_n66_), .b(x07), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  aoi21  g61(.a(new_n76_), .b(new_n43_), .c(new_n46_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n100_), .c(new_n73_), .O(z19));
  buf    g64(.a(x15), .O(z06));
endmodule


