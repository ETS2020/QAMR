// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n62_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  inv1   g03(.a(x18), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nor3   g05(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g06(.a(new_n46_), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x15), .c(new_n49_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n48_), .O(z01));
  inv1   g12(.a(x15), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x10), .c(x08), .O(new_n55_));
  nor2   g14(.a(x18), .b(new_n44_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n42_), .O(new_n57_));
  aoi21  g16(.a(new_n55_), .b(new_n49_), .c(new_n57_), .O(z02));
  inv1   g17(.a(new_n56_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n52_), .O(z03));
  nand2  g19(.a(x18), .b(new_n44_), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nor2   g21(.a(new_n46_), .b(new_n62_), .O(z05));
  nand2  g22(.a(new_n48_), .b(new_n54_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nor2   g24(.a(new_n46_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n48_), .b(new_n67_), .O(z08));
  nand2  g27(.a(x09), .b(x08), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n48_), .O(z09));
  nand3  g29(.a(new_n48_), .b(x09), .c(x08), .O(z10));
  nand2  g30(.a(new_n48_), .b(new_n49_), .O(z11));
  nand2  g31(.a(new_n55_), .b(x13), .O(new_n73_));
  inv1   g32(.a(x00), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n74_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n51_), .c(x18), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n73_), .c(new_n44_), .O(z12));
  nand2  g36(.a(new_n51_), .b(x15), .O(new_n78_));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n50_), .b(new_n79_), .c(x14), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n78_), .c(new_n59_), .O(z13));
  nor2   g40(.a(new_n50_), .b(x15), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x02), .O(new_n83_));
  aoi21  g42(.a(new_n50_), .b(x15), .c(x18), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n44_), .O(z14));
  nand3  g44(.a(new_n55_), .b(x16), .c(new_n49_), .O(new_n86_));
  inv1   g45(.a(x03), .O(new_n87_));
  nor2   g46(.a(x15), .b(new_n87_), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n51_), .c(x18), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n86_), .c(new_n44_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  nand2  g50(.a(new_n82_), .b(new_n91_), .O(new_n92_));
  nand2  g51(.a(new_n55_), .b(new_n65_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n92_), .c(new_n56_), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z16));
  nand2  g54(.a(new_n82_), .b(x05), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n45_), .c(new_n44_), .O(z17));
  nand2  g56(.a(new_n55_), .b(x19), .O(new_n98_));
  inv1   g57(.a(x06), .O(new_n99_));
  nor2   g58(.a(x15), .b(new_n99_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n51_), .c(x18), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n98_), .c(new_n44_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand2  g62(.a(new_n82_), .b(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n55_), .b(new_n43_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(new_n56_), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z19));
endmodule


