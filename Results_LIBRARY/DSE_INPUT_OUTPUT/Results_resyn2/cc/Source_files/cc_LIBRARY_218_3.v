// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x17), .b(x16), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand3  g03(.a(x15), .b(x10), .c(x08), .O(new_n45_));
  oai21  g04(.a(new_n45_), .b(x14), .c(new_n43_), .O(z01));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(x15), .O(new_n48_));
  inv1   g07(.a(x12), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x11), .O(new_n50_));
  oai21  g09(.a(new_n48_), .b(x14), .c(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n43_), .O(z02));
  nand2  g11(.a(new_n43_), .b(x15), .O(new_n53_));
  inv1   g12(.a(x14), .O(new_n54_));
  and2   g13(.a(x10), .b(x08), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(x12), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(new_n53_), .O(z03));
  inv1   g16(.a(new_n43_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n58_), .b(new_n60_), .O(z05));
  inv1   g20(.a(new_n53_), .O(z06));
  nand3  g21(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g22(.a(z10), .O(z09));
  nor2   g23(.a(new_n58_), .b(new_n54_), .O(z11));
  inv1   g24(.a(x00), .O(new_n66_));
  inv1   g25(.a(x15), .O(new_n67_));
  nand3  g26(.a(new_n55_), .b(new_n67_), .c(new_n66_), .O(new_n68_));
  aoi21  g27(.a(x17), .b(x16), .c(new_n49_), .O(new_n69_));
  inv1   g28(.a(x13), .O(new_n70_));
  nand3  g29(.a(new_n67_), .b(x10), .c(x08), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n69_), .c(new_n68_), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  nand3  g33(.a(x10), .b(x08), .c(x01), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x14), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n45_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n43_), .O(z13));
  nand2  g38(.a(new_n55_), .b(x02), .O(new_n80_));
  nand3  g39(.a(new_n45_), .b(new_n43_), .c(x12), .O(new_n81_));
  aoi21  g40(.a(new_n80_), .b(new_n67_), .c(new_n81_), .O(z14));
  inv1   g41(.a(x17), .O(new_n83_));
  nand4  g42(.a(new_n71_), .b(new_n83_), .c(x16), .d(new_n54_), .O(new_n84_));
  nand3  g43(.a(new_n48_), .b(new_n43_), .c(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n49_), .O(z15));
  nand2  g45(.a(new_n71_), .b(new_n83_), .O(new_n87_));
  inv1   g46(.a(x04), .O(new_n88_));
  nand3  g47(.a(new_n55_), .b(new_n67_), .c(new_n88_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n87_), .c(new_n69_), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z16));
  inv1   g50(.a(x05), .O(new_n92_));
  nand3  g51(.a(new_n55_), .b(new_n67_), .c(new_n92_), .O(new_n93_));
  inv1   g52(.a(x18), .O(new_n94_));
  nand2  g53(.a(new_n71_), .b(new_n94_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n93_), .c(new_n69_), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand3  g57(.a(new_n55_), .b(new_n67_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n71_), .b(new_n60_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n43_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand3  g62(.a(new_n55_), .b(new_n67_), .c(new_n103_), .O(new_n104_));
  inv1   g63(.a(x20), .O(new_n105_));
  nand2  g64(.a(new_n71_), .b(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n104_), .c(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n43_), .O(z19));
  buf    g67(.a(x17), .O(z07));
  buf    g68(.a(x16), .O(z08));
endmodule


