// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x08), .O(new_n44_));
  nand2  g03(.a(x15), .b(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x10), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n47_), .c(x08), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(z01));
  nand2  g11(.a(new_n49_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n44_), .c(new_n47_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n45_), .O(z02));
  nand3  g15(.a(new_n47_), .b(x12), .c(x10), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x08), .c(new_n49_), .O(z03));
  inv1   g17(.a(new_n45_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n45_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n59_), .b(new_n63_), .O(z07));
  and2   g23(.a(new_n45_), .b(x16), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(z10));
  inv1   g25(.a(z10), .O(z09));
  nor2   g26(.a(new_n59_), .b(new_n47_), .O(z11));
  nand2  g27(.a(x13), .b(x12), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n49_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n44_), .O(new_n71_));
  oai21  g30(.a(x15), .b(new_n48_), .c(x13), .O(new_n72_));
  nand4  g31(.a(new_n49_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(z12));
  nand2  g35(.a(x10), .b(x01), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x14), .O(new_n78_));
  nor2   g37(.a(new_n50_), .b(new_n44_), .O(new_n79_));
  nand2  g38(.a(new_n47_), .b(new_n44_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n45_), .c(x12), .O(new_n81_));
  aoi21  g40(.a(new_n79_), .b(new_n78_), .c(new_n81_), .O(z13));
  aoi21  g41(.a(x10), .b(x02), .c(x15), .O(new_n83_));
  nand2  g42(.a(x12), .b(x08), .O(new_n84_));
  nor3   g43(.a(new_n84_), .b(new_n83_), .c(new_n50_), .O(z14));
  inv1   g44(.a(x12), .O(new_n86_));
  nand3  g45(.a(x10), .b(x08), .c(x03), .O(new_n87_));
  nand3  g46(.a(x16), .b(new_n47_), .c(new_n44_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n49_), .O(new_n90_));
  nand4  g49(.a(new_n53_), .b(x16), .c(new_n47_), .d(x08), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n86_), .O(z15));
  inv1   g51(.a(x04), .O(new_n93_));
  nand2  g52(.a(x10), .b(x08), .O(new_n94_));
  oai22  g53(.a(new_n94_), .b(new_n93_), .c(new_n63_), .d(x08), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n49_), .O(new_n96_));
  nand3  g55(.a(new_n53_), .b(x17), .c(x08), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n86_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  oai22  g59(.a(new_n94_), .b(new_n99_), .c(new_n100_), .d(x08), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n49_), .O(new_n102_));
  nand3  g61(.a(new_n53_), .b(x18), .c(x08), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n86_), .O(z17));
  inv1   g63(.a(x06), .O(new_n105_));
  oai22  g64(.a(new_n94_), .b(new_n105_), .c(new_n61_), .d(x08), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n49_), .O(new_n107_));
  nand3  g66(.a(new_n53_), .b(x19), .c(x08), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n86_), .O(z18));
  oai21  g68(.a(new_n43_), .b(new_n86_), .c(new_n49_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n44_), .O(new_n111_));
  oai21  g70(.a(x15), .b(new_n48_), .c(x20), .O(new_n112_));
  nand4  g71(.a(new_n49_), .b(x10), .c(x08), .d(x07), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x12), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n111_), .O(z19));
  buf    g75(.a(x15), .O(z06));
endmodule


