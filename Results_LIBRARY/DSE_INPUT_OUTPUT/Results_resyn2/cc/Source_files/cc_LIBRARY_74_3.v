// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n45_, new_n46_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x18), .O(z04));
  nand3  g01(.a(x20), .b(z04), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  nand2  g04(.a(x18), .b(x11), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n45_), .O(z11));
  nand3  g06(.a(x15), .b(x10), .c(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(z11), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand2  g11(.a(x12), .b(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n45_), .c(new_n53_), .O(z02));
  nand2  g13(.a(new_n45_), .b(x12), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n48_), .c(new_n46_), .O(z03));
  inv1   g15(.a(x19), .O(new_n57_));
  nand2  g16(.a(new_n46_), .b(new_n57_), .O(z05));
  inv1   g17(.a(new_n46_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n50_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n59_), .b(new_n61_), .O(z07));
  and2   g21(.a(new_n46_), .b(x16), .O(z08));
  nand3  g22(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(z10), .O(z09));
  inv1   g24(.a(x00), .O(new_n66_));
  nand4  g25(.a(new_n50_), .b(x10), .c(x08), .d(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  nand2  g27(.a(new_n51_), .b(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n67_), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n46_), .O(z12));
  nand3  g30(.a(x10), .b(x08), .c(x01), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x14), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n48_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n46_), .O(z13));
  nand3  g35(.a(x10), .b(x08), .c(x02), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n50_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n48_), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n46_), .O(z14));
  inv1   g39(.a(x03), .O(new_n81_));
  nand4  g40(.a(new_n50_), .b(x10), .c(x08), .d(new_n81_), .O(new_n82_));
  nand2  g41(.a(x16), .b(new_n45_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n51_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(new_n82_), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n46_), .O(z15));
  inv1   g45(.a(x04), .O(new_n87_));
  nand4  g46(.a(new_n50_), .b(x10), .c(x08), .d(new_n87_), .O(new_n88_));
  and2   g47(.a(new_n46_), .b(x12), .O(new_n89_));
  nand2  g48(.a(new_n51_), .b(new_n61_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n88_), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z16));
  inv1   g51(.a(x05), .O(new_n93_));
  nand4  g52(.a(new_n50_), .b(x10), .c(x08), .d(new_n93_), .O(new_n94_));
  nand2  g53(.a(new_n51_), .b(z04), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n46_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand4  g57(.a(new_n50_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n51_), .b(new_n57_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n89_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand4  g62(.a(new_n50_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  inv1   g63(.a(x20), .O(new_n105_));
  nand2  g64(.a(new_n51_), .b(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n104_), .c(new_n89_), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z19));
endmodule


