// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n59_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x18), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n45_), .b(new_n47_), .O(z11));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(z11), .O(z01));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand3  g11(.a(new_n45_), .b(x12), .c(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n52_), .b(new_n47_), .c(new_n53_), .O(z02));
  nand2  g13(.a(new_n47_), .b(x12), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n49_), .c(new_n45_), .O(z03));
  inv1   g15(.a(x18), .O(z04));
  nand2  g16(.a(new_n45_), .b(new_n51_), .O(z06));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n45_), .b(new_n59_), .O(z07));
  and2   g19(.a(new_n45_), .b(x16), .O(z08));
  nand3  g20(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g21(.a(z10), .O(z09));
  inv1   g22(.a(x12), .O(new_n64_));
  aoi21  g23(.a(new_n44_), .b(x18), .c(new_n64_), .O(new_n65_));
  inv1   g24(.a(x00), .O(new_n66_));
  nand4  g25(.a(new_n51_), .b(x10), .c(x08), .d(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  nand2  g27(.a(x10), .b(x08), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(x15), .c(new_n68_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n67_), .c(new_n65_), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z12));
  nand2  g31(.a(new_n45_), .b(x12), .O(new_n73_));
  inv1   g32(.a(x01), .O(new_n74_));
  oai21  g33(.a(new_n69_), .b(new_n74_), .c(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n49_), .c(new_n73_), .O(z13));
  inv1   g35(.a(x02), .O(new_n77_));
  oai21  g36(.a(new_n69_), .b(new_n77_), .c(new_n51_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n49_), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n45_), .O(z14));
  nand3  g39(.a(new_n52_), .b(x16), .c(new_n47_), .O(new_n81_));
  nand4  g40(.a(new_n51_), .b(x10), .c(x08), .d(x03), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n73_), .O(z15));
  inv1   g42(.a(x04), .O(new_n84_));
  nand4  g43(.a(new_n51_), .b(x10), .c(x08), .d(new_n84_), .O(new_n85_));
  oai21  g44(.a(new_n69_), .b(x15), .c(new_n59_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n85_), .c(new_n65_), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z16));
  inv1   g47(.a(x05), .O(new_n89_));
  nand4  g48(.a(new_n51_), .b(x10), .c(x08), .d(new_n89_), .O(new_n90_));
  oai21  g49(.a(new_n69_), .b(x15), .c(z04), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(new_n65_), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z17));
  nand4  g52(.a(z04), .b(new_n51_), .c(x10), .d(x08), .O(new_n94_));
  inv1   g53(.a(x06), .O(new_n95_));
  nand4  g54(.a(new_n51_), .b(x10), .c(x08), .d(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x12), .O(new_n97_));
  aoi21  g56(.a(new_n94_), .b(new_n44_), .c(new_n97_), .O(z18));
  inv1   g57(.a(x07), .O(new_n99_));
  nand4  g58(.a(new_n51_), .b(x10), .c(x08), .d(new_n99_), .O(new_n100_));
  oai21  g59(.a(new_n69_), .b(x15), .c(new_n43_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n100_), .c(new_n65_), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z19));
  buf    g62(.a(x19), .O(z05));
endmodule


