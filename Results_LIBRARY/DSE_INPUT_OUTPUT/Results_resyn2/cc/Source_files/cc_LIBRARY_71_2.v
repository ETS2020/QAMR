// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  aoi21  g02(.a(new_n43_), .b(x19), .c(new_n42_), .O(z00));
  nor2   g03(.a(x19), .b(new_n42_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x15), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(x14), .c(new_n46_), .O(z01));
  inv1   g09(.a(x14), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n46_), .O(z02));
  nand3  g15(.a(new_n48_), .b(new_n51_), .c(x12), .O(new_n57_));
  nor3   g16(.a(new_n57_), .b(new_n45_), .c(new_n52_), .O(z03));
  nand2  g17(.a(new_n46_), .b(x18), .O(z04));
  nor2   g18(.a(new_n45_), .b(new_n52_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n45_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n46_), .O(z09));
  inv1   g25(.a(z09), .O(z10));
  nor2   g26(.a(new_n45_), .b(new_n51_), .O(z11));
  inv1   g27(.a(x00), .O(new_n69_));
  inv1   g28(.a(new_n53_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g30(.a(x12), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  aoi21  g32(.a(new_n53_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n46_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  oai21  g36(.a(new_n47_), .b(new_n77_), .c(x14), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n49_), .O(new_n79_));
  nor2   g38(.a(new_n45_), .b(new_n72_), .O(new_n80_));
  and2   g39(.a(new_n80_), .b(new_n79_), .O(z13));
  aoi21  g40(.a(new_n48_), .b(x02), .c(x15), .O(new_n82_));
  nand3  g41(.a(new_n49_), .b(new_n46_), .c(x12), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n82_), .O(z14));
  nand3  g43(.a(new_n53_), .b(x16), .c(new_n51_), .O(new_n85_));
  nand3  g44(.a(new_n48_), .b(new_n52_), .c(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n46_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand2  g49(.a(new_n70_), .b(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n53_), .b(new_n61_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(new_n80_), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  inv1   g53(.a(x05), .O(new_n95_));
  nand2  g54(.a(new_n70_), .b(new_n95_), .O(new_n96_));
  inv1   g55(.a(x18), .O(new_n97_));
  aoi21  g56(.a(new_n53_), .b(new_n97_), .c(new_n72_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n46_), .O(z17));
  inv1   g59(.a(x06), .O(new_n101_));
  nand2  g60(.a(new_n70_), .b(new_n101_), .O(new_n102_));
  inv1   g61(.a(x19), .O(new_n103_));
  aoi21  g62(.a(new_n53_), .b(new_n103_), .c(new_n72_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n46_), .O(z18));
  inv1   g65(.a(x07), .O(new_n107_));
  nand2  g66(.a(new_n70_), .b(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n53_), .b(new_n43_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n108_), .c(new_n80_), .O(new_n110_));
  inv1   g69(.a(new_n110_), .O(z19));
  buf    g70(.a(x19), .O(z05));
endmodule


