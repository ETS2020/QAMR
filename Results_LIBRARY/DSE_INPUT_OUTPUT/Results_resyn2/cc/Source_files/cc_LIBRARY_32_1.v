// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n95_, new_n96_;
  inv1   g00(.a(x15), .O(new_n42_));
  nor2   g01(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x12), .c(new_n42_), .O(z01));
  inv1   g08(.a(new_n43_), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  nand2  g15(.a(new_n50_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nor2   g17(.a(new_n43_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  nor2   g19(.a(new_n43_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n43_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n50_), .O(z09));
  nand3  g24(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nor2   g25(.a(new_n43_), .b(new_n46_), .O(z11));
  nand2  g26(.a(new_n47_), .b(new_n42_), .O(new_n68_));
  oai21  g27(.a(new_n51_), .b(x00), .c(x12), .O(new_n69_));
  nor2   g28(.a(x13), .b(new_n53_), .O(new_n70_));
  aoi22  g29(.a(new_n70_), .b(new_n68_), .c(new_n69_), .d(new_n42_), .O(z12));
  oai21  g30(.a(new_n47_), .b(new_n53_), .c(x15), .O(new_n72_));
  nand2  g31(.a(new_n47_), .b(x01), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(x14), .c(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n72_), .O(z13));
  nand3  g34(.a(x12), .b(x10), .c(x08), .O(new_n76_));
  aoi21  g35(.a(new_n42_), .b(x02), .c(new_n76_), .O(new_n77_));
  aoi21  g36(.a(new_n76_), .b(new_n42_), .c(new_n77_), .O(z14));
  oai21  g37(.a(new_n62_), .b(x14), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x15), .O(new_n80_));
  nand2  g39(.a(new_n42_), .b(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n47_), .O(new_n82_));
  oai21  g41(.a(new_n62_), .b(x14), .c(new_n51_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n82_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n80_), .O(z15));
  oai21  g44(.a(new_n51_), .b(x04), .c(x12), .O(new_n86_));
  nor2   g45(.a(x17), .b(new_n53_), .O(new_n87_));
  aoi22  g46(.a(new_n87_), .b(new_n68_), .c(new_n86_), .d(new_n42_), .O(z16));
  oai21  g47(.a(new_n51_), .b(x05), .c(x12), .O(new_n89_));
  nor2   g48(.a(x18), .b(new_n53_), .O(new_n90_));
  aoi22  g49(.a(new_n90_), .b(new_n68_), .c(new_n89_), .d(new_n42_), .O(z17));
  oai21  g50(.a(new_n51_), .b(x06), .c(x12), .O(new_n92_));
  nor2   g51(.a(x19), .b(new_n53_), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n68_), .c(new_n92_), .d(new_n42_), .O(z18));
  oai21  g53(.a(new_n51_), .b(x07), .c(x12), .O(new_n95_));
  nor2   g54(.a(x20), .b(new_n53_), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n68_), .c(new_n95_), .d(new_n42_), .O(z19));
  aoi21  g56(.a(new_n48_), .b(x12), .c(new_n42_), .O(z03));
  buf    g57(.a(x15), .O(z06));
endmodule


