// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x02), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nor3   g05(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n45_), .O(z01));
  and2   g11(.a(x10), .b(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n44_), .O(new_n54_));
  nand3  g13(.a(new_n45_), .b(x12), .c(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n48_), .c(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nor2   g16(.a(new_n51_), .b(new_n57_), .O(z03));
  nor2   g17(.a(new_n46_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n46_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x02), .O(new_n62_));
  nand2  g21(.a(new_n44_), .b(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n46_), .b(new_n64_), .O(z07));
  and2   g24(.a(new_n45_), .b(x16), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  inv1   g27(.a(z09), .O(z10));
  nand2  g28(.a(new_n45_), .b(new_n48_), .O(z11));
  inv1   g29(.a(x13), .O(new_n71_));
  nand2  g30(.a(x10), .b(x08), .O(new_n72_));
  nand2  g31(.a(x12), .b(x00), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n72_), .c(new_n62_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n44_), .O(new_n75_));
  nand2  g34(.a(new_n54_), .b(x12), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n71_), .c(new_n75_), .O(z12));
  nand3  g36(.a(x10), .b(x08), .c(x01), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(x14), .c(new_n50_), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n57_), .c(new_n45_), .O(z13));
  nor3   g39(.a(new_n53_), .b(new_n44_), .c(new_n57_), .O(z14));
  nand4  g40(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n62_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(new_n84_));
  nand2  g43(.a(x16), .b(new_n48_), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n76_), .c(new_n84_), .O(z15));
  nand4  g45(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n44_), .O(new_n89_));
  oai21  g48(.a(new_n76_), .b(new_n64_), .c(new_n89_), .O(z16));
  nand2  g49(.a(x18), .b(x15), .O(new_n91_));
  nand2  g50(.a(new_n44_), .b(x05), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n53_), .O(new_n93_));
  inv1   g52(.a(x18), .O(new_n94_));
  aoi21  g53(.a(new_n72_), .b(new_n94_), .c(x02), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n91_), .c(new_n57_), .O(z17));
  nand2  g56(.a(x19), .b(x15), .O(new_n98_));
  nand2  g57(.a(new_n44_), .b(x06), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n53_), .O(new_n100_));
  aoi21  g59(.a(new_n72_), .b(new_n60_), .c(x02), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n98_), .c(new_n57_), .O(z18));
  nand4  g62(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n62_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n44_), .O(new_n106_));
  oai21  g65(.a(new_n76_), .b(new_n43_), .c(new_n106_), .O(z19));
endmodule


