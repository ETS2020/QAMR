// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x12), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x12), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor4   g08(.a(new_n49_), .b(new_n44_), .c(x14), .d(new_n48_), .O(z01));
  inv1   g09(.a(x14), .O(new_n51_));
  oai21  g10(.a(new_n49_), .b(x15), .c(new_n51_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(z02));
  nand2  g13(.a(new_n46_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nor2   g15(.a(new_n45_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nor2   g17(.a(new_n45_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n46_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n46_), .O(z09));
  nand3  g22(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nor2   g23(.a(new_n45_), .b(new_n51_), .O(z11));
  inv1   g24(.a(x00), .O(new_n66_));
  nand4  g25(.a(new_n44_), .b(x10), .c(x08), .d(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  oai21  g27(.a(new_n49_), .b(x15), .c(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n67_), .c(x12), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z12));
  and2   g30(.a(x10), .b(x08), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n48_), .c(x15), .O(new_n73_));
  nand2  g32(.a(new_n72_), .b(x01), .O(new_n74_));
  nor2   g33(.a(new_n51_), .b(new_n48_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n73_), .O(z13));
  nand2  g36(.a(new_n72_), .b(x02), .O(new_n78_));
  oai21  g37(.a(new_n49_), .b(new_n44_), .c(x12), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n44_), .c(new_n79_), .O(z14));
  nand2  g39(.a(new_n72_), .b(new_n44_), .O(new_n81_));
  nand2  g40(.a(x16), .b(new_n51_), .O(new_n82_));
  inv1   g41(.a(x03), .O(new_n83_));
  nand4  g42(.a(new_n44_), .b(x10), .c(x08), .d(new_n83_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  aoi21  g44(.a(new_n82_), .b(new_n81_), .c(new_n85_), .O(z15));
  inv1   g45(.a(x04), .O(new_n87_));
  nand4  g46(.a(new_n44_), .b(x10), .c(x08), .d(new_n87_), .O(new_n88_));
  oai21  g47(.a(new_n49_), .b(x15), .c(new_n58_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n88_), .c(x12), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z16));
  oai21  g50(.a(new_n49_), .b(x05), .c(x12), .O(new_n92_));
  nor2   g51(.a(x18), .b(new_n48_), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n81_), .c(new_n92_), .d(new_n44_), .O(z17));
  oai21  g53(.a(new_n49_), .b(x06), .c(x12), .O(new_n95_));
  nor2   g54(.a(x19), .b(new_n48_), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n81_), .c(new_n95_), .d(new_n44_), .O(z18));
  inv1   g56(.a(x07), .O(new_n98_));
  nand4  g57(.a(new_n44_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  oai21  g58(.a(new_n49_), .b(x15), .c(new_n43_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z19));
  nor4   g61(.a(new_n49_), .b(new_n44_), .c(x14), .d(new_n48_), .O(z03));
  buf    g62(.a(x15), .O(z06));
endmodule


