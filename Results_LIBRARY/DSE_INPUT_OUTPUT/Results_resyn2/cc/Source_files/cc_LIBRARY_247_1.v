// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n55_, new_n58_, new_n60_, new_n62_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_;
  inv1   g00(.a(x15), .O(new_n42_));
  nor2   g01(.a(new_n42_), .b(x12), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x12), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor4   g06(.a(new_n47_), .b(new_n42_), .c(x14), .d(new_n46_), .O(z01));
  inv1   g07(.a(new_n43_), .O(new_n49_));
  nor2   g08(.a(new_n47_), .b(x15), .O(new_n50_));
  nor2   g09(.a(new_n46_), .b(x11), .O(new_n51_));
  oai21  g10(.a(new_n50_), .b(x14), .c(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n49_), .O(z02));
  nand2  g12(.a(new_n49_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nand2  g14(.a(new_n49_), .b(new_n55_), .O(z05));
  nor2   g15(.a(new_n42_), .b(new_n46_), .O(z06));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n49_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n49_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(new_n43_), .O(z09));
  inv1   g22(.a(z09), .O(z10));
  inv1   g23(.a(x14), .O(new_n65_));
  nand2  g24(.a(new_n49_), .b(new_n65_), .O(z11));
  inv1   g25(.a(x00), .O(new_n67_));
  and2   g26(.a(x10), .b(x08), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(new_n42_), .c(new_n67_), .O(new_n69_));
  inv1   g28(.a(x13), .O(new_n70_));
  oai21  g29(.a(new_n47_), .b(x15), .c(new_n70_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n69_), .c(x12), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  oai21  g32(.a(new_n68_), .b(new_n46_), .c(x15), .O(new_n74_));
  nand2  g33(.a(new_n68_), .b(x01), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x14), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(z13));
  nand3  g36(.a(x12), .b(x10), .c(x08), .O(new_n78_));
  aoi21  g37(.a(new_n42_), .b(x02), .c(new_n78_), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n42_), .c(new_n79_), .O(z14));
  oai21  g39(.a(new_n60_), .b(x14), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x15), .O(new_n82_));
  nand2  g41(.a(new_n42_), .b(x03), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n68_), .O(new_n84_));
  oai21  g43(.a(new_n60_), .b(x14), .c(new_n47_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n84_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n82_), .O(z15));
  inv1   g46(.a(x04), .O(new_n88_));
  nand3  g47(.a(new_n68_), .b(new_n42_), .c(new_n88_), .O(new_n89_));
  oai21  g48(.a(new_n47_), .b(x15), .c(new_n58_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(x12), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z16));
  nand2  g51(.a(new_n68_), .b(new_n42_), .O(new_n93_));
  oai21  g52(.a(new_n47_), .b(x05), .c(x12), .O(new_n94_));
  nor2   g53(.a(x18), .b(new_n46_), .O(new_n95_));
  aoi22  g54(.a(new_n95_), .b(new_n93_), .c(new_n94_), .d(new_n42_), .O(z17));
  inv1   g55(.a(x06), .O(new_n97_));
  nand3  g56(.a(new_n68_), .b(new_n42_), .c(new_n97_), .O(new_n98_));
  oai21  g57(.a(new_n47_), .b(x15), .c(new_n55_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(x12), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z18));
  oai21  g60(.a(new_n47_), .b(x07), .c(x12), .O(new_n102_));
  nor2   g61(.a(x20), .b(new_n46_), .O(new_n103_));
  aoi22  g62(.a(new_n103_), .b(new_n93_), .c(new_n102_), .d(new_n42_), .O(z19));
  nor4   g63(.a(new_n47_), .b(new_n42_), .c(x14), .d(new_n46_), .O(z03));
endmodule


