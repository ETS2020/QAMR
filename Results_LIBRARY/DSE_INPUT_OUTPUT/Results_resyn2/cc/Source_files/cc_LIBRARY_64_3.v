// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n65_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x09), .b(x04), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n43_), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n45_), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n43_), .O(z02));
  aoi21  g15(.a(x09), .b(x04), .c(new_n53_), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(new_n48_), .O(z03));
  inv1   g18(.a(new_n43_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z05));
  nand2  g22(.a(new_n43_), .b(new_n50_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nor2   g24(.a(new_n60_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n43_), .b(new_n67_), .O(z08));
  inv1   g27(.a(x04), .O(new_n69_));
  inv1   g28(.a(x08), .O(new_n70_));
  inv1   g29(.a(x09), .O(new_n71_));
  aoi21  g30(.a(new_n70_), .b(new_n69_), .c(new_n71_), .O(z09));
  nand3  g31(.a(x09), .b(x08), .c(new_n69_), .O(z10));
  nor2   g32(.a(new_n60_), .b(new_n45_), .O(z11));
  inv1   g33(.a(x00), .O(new_n75_));
  and2   g34(.a(x10), .b(x08), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n50_), .c(new_n75_), .O(new_n77_));
  inv1   g36(.a(x13), .O(new_n78_));
  nand2  g37(.a(new_n51_), .b(new_n78_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n77_), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n43_), .O(z12));
  nand3  g40(.a(x10), .b(x08), .c(x01), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x14), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n46_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n43_), .O(z13));
  aoi21  g45(.a(new_n76_), .b(x02), .c(x15), .O(new_n87_));
  nand2  g46(.a(new_n57_), .b(new_n46_), .O(new_n88_));
  nor2   g47(.a(new_n88_), .b(new_n87_), .O(z14));
  inv1   g48(.a(x03), .O(new_n90_));
  nand3  g49(.a(new_n76_), .b(new_n50_), .c(new_n90_), .O(new_n91_));
  nand2  g50(.a(x16), .b(new_n45_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n51_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n91_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n43_), .O(z15));
  nand3  g54(.a(new_n76_), .b(new_n50_), .c(new_n69_), .O(new_n96_));
  nand2  g55(.a(new_n51_), .b(new_n65_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(new_n57_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z16));
  inv1   g58(.a(x05), .O(new_n100_));
  nand3  g59(.a(new_n76_), .b(new_n50_), .c(new_n100_), .O(new_n101_));
  inv1   g60(.a(x18), .O(new_n102_));
  nand2  g61(.a(new_n51_), .b(new_n102_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n101_), .c(new_n57_), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z17));
  inv1   g64(.a(x06), .O(new_n106_));
  nand3  g65(.a(new_n76_), .b(new_n50_), .c(new_n106_), .O(new_n107_));
  nand2  g66(.a(new_n51_), .b(new_n62_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(new_n57_), .O(new_n109_));
  inv1   g68(.a(new_n109_), .O(z18));
  inv1   g69(.a(x07), .O(new_n111_));
  nand3  g70(.a(new_n76_), .b(new_n50_), .c(new_n111_), .O(new_n112_));
  inv1   g71(.a(x20), .O(new_n113_));
  nand2  g72(.a(new_n51_), .b(new_n113_), .O(new_n114_));
  nand3  g73(.a(new_n114_), .b(new_n112_), .c(new_n57_), .O(new_n115_));
  inv1   g74(.a(new_n115_), .O(z19));
endmodule


