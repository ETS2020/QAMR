// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n59_, new_n61_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x15), .O(new_n42_));
  nand2  g01(.a(x17), .b(new_n42_), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n47_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n43_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n43_), .O(z02));
  oai21  g15(.a(new_n49_), .b(new_n53_), .c(new_n43_), .O(z03));
  nand2  g16(.a(new_n43_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n44_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x17), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n42_), .O(z06));
  and2   g21(.a(new_n43_), .b(x16), .O(z08));
  nand3  g22(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(z10), .O(z09));
  nor2   g24(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g25(.a(x13), .O(new_n67_));
  nand2  g26(.a(x12), .b(x00), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n51_), .c(new_n61_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n42_), .O(new_n70_));
  oai21  g29(.a(new_n51_), .b(x15), .c(x12), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n67_), .c(new_n70_), .O(z12));
  oai21  g31(.a(new_n48_), .b(x14), .c(x15), .O(new_n73_));
  nand2  g32(.a(new_n48_), .b(x01), .O(new_n74_));
  nor2   g33(.a(x17), .b(new_n47_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n73_), .c(new_n53_), .O(z13));
  nand2  g36(.a(new_n51_), .b(x15), .O(new_n78_));
  nand3  g37(.a(new_n52_), .b(new_n61_), .c(x02), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n53_), .O(z14));
  nand2  g39(.a(x12), .b(x03), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n51_), .c(new_n61_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n42_), .O(new_n83_));
  nand2  g42(.a(x16), .b(new_n47_), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n71_), .c(new_n83_), .O(z15));
  nand2  g44(.a(new_n52_), .b(x04), .O(new_n86_));
  nand2  g45(.a(new_n43_), .b(x12), .O(new_n87_));
  aoi21  g46(.a(new_n86_), .b(new_n61_), .c(new_n87_), .O(z16));
  inv1   g47(.a(x18), .O(new_n89_));
  nand2  g48(.a(x12), .b(x05), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n51_), .c(new_n61_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n42_), .O(new_n92_));
  oai21  g51(.a(new_n71_), .b(new_n89_), .c(new_n92_), .O(z17));
  nand2  g52(.a(x19), .b(x15), .O(new_n94_));
  nand2  g53(.a(new_n42_), .b(x06), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n48_), .O(new_n96_));
  aoi21  g55(.a(new_n51_), .b(new_n59_), .c(x17), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n94_), .c(new_n53_), .O(z18));
  nand2  g58(.a(x20), .b(x15), .O(new_n100_));
  nand2  g59(.a(new_n42_), .b(x07), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  inv1   g61(.a(x20), .O(new_n103_));
  aoi21  g62(.a(new_n51_), .b(new_n103_), .c(x17), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n100_), .c(new_n53_), .O(z19));
  buf    g65(.a(x17), .O(z07));
endmodule


