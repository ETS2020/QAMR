// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x12), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g08(.a(x15), .b(x12), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n49_), .O(z01));
  nand2  g10(.a(new_n48_), .b(new_n43_), .O(new_n52_));
  inv1   g11(.a(x11), .O(new_n53_));
  nand2  g12(.a(x12), .b(new_n53_), .O(new_n54_));
  aoi21  g13(.a(new_n52_), .b(new_n47_), .c(new_n54_), .O(z02));
  aoi21  g14(.a(new_n49_), .b(x12), .c(new_n43_), .O(z03));
  nor2   g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nor2   g17(.a(new_n44_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n44_), .O(z09));
  inv1   g24(.a(z09), .O(z10));
  nor2   g25(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g26(.a(x00), .O(new_n68_));
  nand3  g27(.a(new_n48_), .b(new_n43_), .c(new_n68_), .O(new_n69_));
  inv1   g28(.a(x13), .O(new_n70_));
  nand2  g29(.a(x10), .b(x08), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(x15), .c(new_n70_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n69_), .c(x12), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  inv1   g33(.a(x12), .O(new_n75_));
  oai21  g34(.a(new_n48_), .b(new_n75_), .c(x15), .O(new_n76_));
  nand2  g35(.a(new_n48_), .b(x01), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x14), .c(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n76_), .O(z13));
  nand3  g38(.a(x12), .b(x10), .c(x08), .O(new_n80_));
  aoi21  g39(.a(new_n43_), .b(x02), .c(new_n80_), .O(new_n81_));
  aoi21  g40(.a(new_n80_), .b(new_n43_), .c(new_n81_), .O(z14));
  oai21  g41(.a(new_n62_), .b(x14), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x15), .O(new_n84_));
  nand2  g43(.a(new_n43_), .b(x03), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n48_), .O(new_n86_));
  oai21  g45(.a(new_n62_), .b(x14), .c(new_n71_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n86_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n84_), .O(z15));
  oai21  g48(.a(new_n71_), .b(x04), .c(x12), .O(new_n90_));
  nor2   g49(.a(x17), .b(new_n75_), .O(new_n91_));
  aoi22  g50(.a(new_n91_), .b(new_n52_), .c(new_n90_), .d(new_n43_), .O(z16));
  inv1   g51(.a(x05), .O(new_n93_));
  nand3  g52(.a(new_n48_), .b(new_n43_), .c(new_n93_), .O(new_n94_));
  inv1   g53(.a(x18), .O(new_n95_));
  oai21  g54(.a(new_n71_), .b(x15), .c(new_n95_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n94_), .c(x12), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z17));
  inv1   g57(.a(x06), .O(new_n99_));
  nand3  g58(.a(new_n48_), .b(new_n43_), .c(new_n99_), .O(new_n100_));
  oai21  g59(.a(new_n71_), .b(x15), .c(new_n58_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n100_), .c(x12), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z18));
  oai21  g62(.a(new_n71_), .b(x07), .c(x12), .O(new_n104_));
  nor2   g63(.a(x20), .b(new_n75_), .O(new_n105_));
  aoi22  g64(.a(new_n105_), .b(new_n52_), .c(new_n104_), .d(new_n43_), .O(z19));
  buf    g65(.a(x15), .O(z06));
endmodule


