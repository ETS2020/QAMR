// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n57_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x15), .b(x08), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(new_n44_), .O(z01));
  nand2  g05(.a(new_n44_), .b(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g10(.a(x12), .b(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n51_), .b(new_n47_), .c(new_n52_), .O(z02));
  nor2   g12(.a(z01), .b(x18), .O(z04));
  and2   g13(.a(new_n44_), .b(x19), .O(z05));
  nor2   g14(.a(new_n48_), .b(x08), .O(z06));
  inv1   g15(.a(x17), .O(new_n57_));
  nand2  g16(.a(new_n44_), .b(new_n57_), .O(z07));
  and2   g17(.a(new_n44_), .b(x16), .O(z08));
  nand3  g18(.a(new_n48_), .b(x09), .c(x08), .O(z10));
  inv1   g19(.a(z10), .O(z09));
  inv1   g20(.a(new_n47_), .O(z11));
  inv1   g21(.a(x00), .O(new_n63_));
  nand2  g22(.a(x12), .b(x10), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n63_), .c(new_n48_), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(x08), .O(new_n66_));
  nand3  g25(.a(new_n49_), .b(x13), .c(x12), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n66_), .O(z12));
  nand2  g27(.a(new_n50_), .b(x01), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(x14), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n44_), .O(z13));
  oai21  g30(.a(x12), .b(x08), .c(x15), .O(new_n72_));
  nand2  g31(.a(x12), .b(x02), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n49_), .c(new_n72_), .O(z14));
  inv1   g33(.a(x12), .O(new_n75_));
  nand3  g34(.a(new_n50_), .b(new_n48_), .c(x03), .O(new_n76_));
  inv1   g35(.a(x14), .O(new_n77_));
  oai21  g36(.a(x15), .b(x10), .c(x08), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x16), .c(new_n77_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n75_), .O(z15));
  nand3  g39(.a(x10), .b(x08), .c(x04), .O(new_n81_));
  oai21  g40(.a(new_n57_), .b(x10), .c(new_n81_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  inv1   g42(.a(x08), .O(new_n84_));
  nand2  g43(.a(x17), .b(new_n84_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n83_), .c(new_n75_), .O(z16));
  inv1   g45(.a(x05), .O(new_n87_));
  oai21  g46(.a(new_n64_), .b(new_n87_), .c(new_n48_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x08), .O(new_n89_));
  nand3  g48(.a(new_n49_), .b(x18), .c(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(z17));
  inv1   g50(.a(x06), .O(new_n92_));
  oai21  g51(.a(new_n64_), .b(new_n92_), .c(new_n48_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x08), .O(new_n94_));
  nand3  g53(.a(new_n49_), .b(x19), .c(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(z18));
  nand3  g55(.a(x10), .b(x08), .c(x07), .O(new_n97_));
  oai21  g56(.a(new_n43_), .b(x10), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g58(.a(x20), .b(new_n84_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n75_), .O(z19));
  inv1   g60(.a(new_n44_), .O(z03));
endmodule


