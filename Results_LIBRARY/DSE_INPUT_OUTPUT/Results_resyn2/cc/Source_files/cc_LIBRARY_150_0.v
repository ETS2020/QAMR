// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nor2   g03(.a(x20), .b(x15), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(x14), .c(new_n46_), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x15), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  nand3  g11(.a(new_n46_), .b(x12), .c(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n52_), .b(new_n49_), .c(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor3   g14(.a(new_n47_), .b(x14), .c(new_n55_), .O(z03));
  nand2  g15(.a(new_n46_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n46_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x15), .O(new_n60_));
  nand2  g19(.a(x20), .b(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n46_), .b(new_n64_), .O(z08));
  nand3  g24(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  inv1   g25(.a(z10), .O(z09));
  nand2  g26(.a(new_n46_), .b(new_n49_), .O(z11));
  nand2  g27(.a(new_n50_), .b(x20), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n60_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x13), .O(new_n71_));
  inv1   g30(.a(new_n50_), .O(new_n72_));
  nand4  g31(.a(new_n72_), .b(x20), .c(new_n60_), .d(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n55_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n50_), .b(new_n75_), .c(x20), .O(new_n76_));
  nand2  g35(.a(new_n47_), .b(new_n49_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  aoi21  g37(.a(new_n76_), .b(new_n60_), .c(new_n78_), .O(z13));
  aoi21  g38(.a(new_n72_), .b(x02), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n47_), .b(x12), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n80_), .c(new_n46_), .O(z14));
  nor2   g41(.a(new_n64_), .b(x14), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n70_), .O(new_n84_));
  nand4  g43(.a(new_n72_), .b(x20), .c(new_n60_), .d(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n55_), .O(z15));
  nand2  g45(.a(x12), .b(x04), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n50_), .c(x20), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n60_), .O(new_n89_));
  nand3  g48(.a(new_n52_), .b(x17), .c(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(z16));
  nand2  g50(.a(new_n70_), .b(x18), .O(new_n92_));
  nand4  g51(.a(new_n72_), .b(x20), .c(new_n60_), .d(x05), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n55_), .O(z17));
  nand2  g53(.a(x12), .b(x06), .O(new_n95_));
  oai21  g54(.a(new_n95_), .b(new_n50_), .c(x20), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n60_), .O(new_n97_));
  nand3  g56(.a(new_n52_), .b(x19), .c(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(z18));
  inv1   g58(.a(x07), .O(new_n100_));
  nand2  g59(.a(x20), .b(x12), .O(new_n101_));
  aoi21  g60(.a(new_n51_), .b(new_n100_), .c(new_n101_), .O(z19));
endmodule


