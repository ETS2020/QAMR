// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x08), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x18), .O(new_n45_));
  nand4  g04(.a(new_n45_), .b(x15), .c(x10), .d(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x10), .O(new_n49_));
  nor2   g08(.a(x15), .b(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x08), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand3  g11(.a(new_n43_), .b(x12), .c(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n48_), .c(new_n53_), .O(z02));
  nand2  g13(.a(z01), .b(x12), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z03));
  inv1   g15(.a(x08), .O(new_n57_));
  nand2  g16(.a(x18), .b(new_n57_), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n43_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n43_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n43_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x09), .O(new_n67_));
  aoi21  g26(.a(new_n45_), .b(new_n67_), .c(new_n57_), .O(z09));
  nand3  g27(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n43_), .b(new_n48_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  aoi21  g30(.a(new_n61_), .b(x10), .c(x18), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n57_), .c(x13), .O(new_n73_));
  nand4  g32(.a(new_n50_), .b(new_n45_), .c(x08), .d(x00), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n73_), .c(new_n71_), .O(z12));
  aoi21  g34(.a(x10), .b(x01), .c(x18), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n57_), .c(x14), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n46_), .c(new_n71_), .O(z13));
  oai21  g37(.a(x18), .b(x10), .c(x08), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x15), .O(new_n80_));
  nand4  g39(.a(new_n50_), .b(new_n45_), .c(x08), .d(x02), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n71_), .O(z14));
  nor2   g41(.a(new_n65_), .b(x14), .O(new_n83_));
  oai21  g42(.a(new_n72_), .b(new_n57_), .c(new_n83_), .O(new_n84_));
  nand4  g43(.a(new_n50_), .b(new_n45_), .c(x08), .d(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n71_), .O(z15));
  oai21  g45(.a(new_n72_), .b(new_n57_), .c(x17), .O(new_n87_));
  nand4  g46(.a(new_n50_), .b(new_n45_), .c(x08), .d(x04), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n71_), .O(z16));
  nand4  g48(.a(new_n50_), .b(new_n45_), .c(x08), .d(x05), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(z04), .c(new_n71_), .O(z17));
  nand4  g50(.a(new_n61_), .b(x12), .c(x10), .d(x06), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n45_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x08), .O(new_n94_));
  nand3  g53(.a(new_n51_), .b(x19), .c(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(z18));
  nand4  g55(.a(new_n61_), .b(x12), .c(x10), .d(x07), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n45_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x08), .O(new_n99_));
  nand3  g58(.a(new_n51_), .b(x20), .c(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(z19));
endmodule


