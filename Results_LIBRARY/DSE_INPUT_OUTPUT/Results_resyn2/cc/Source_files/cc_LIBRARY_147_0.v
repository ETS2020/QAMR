// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x18), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(new_n46_));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(x14), .c(new_n46_), .O(z01));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x15), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x14), .c(x12), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(x18), .c(x11), .O(z02));
  inv1   g11(.a(x12), .O(new_n53_));
  nand2  g12(.a(new_n46_), .b(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(z01), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z03));
  nor2   g15(.a(x18), .b(new_n42_), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n46_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x15), .O(new_n60_));
  nor2   g19(.a(x18), .b(x11), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n46_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n46_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n61_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n61_), .O(z10));
  inv1   g28(.a(x14), .O(new_n70_));
  nor2   g29(.a(new_n61_), .b(new_n70_), .O(z11));
  inv1   g30(.a(x00), .O(new_n72_));
  nand2  g31(.a(new_n50_), .b(new_n72_), .O(new_n73_));
  nor2   g32(.a(new_n61_), .b(new_n53_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  nand3  g34(.a(new_n60_), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n74_), .c(new_n73_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z12));
  inv1   g38(.a(new_n74_), .O(new_n80_));
  inv1   g39(.a(x01), .O(new_n81_));
  oai21  g40(.a(new_n49_), .b(new_n81_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n47_), .c(new_n80_), .O(z13));
  nand3  g42(.a(x10), .b(x08), .c(x02), .O(new_n84_));
  nand3  g43(.a(new_n47_), .b(new_n46_), .c(x12), .O(new_n85_));
  aoi21  g44(.a(new_n84_), .b(new_n60_), .c(new_n85_), .O(z14));
  inv1   g45(.a(new_n54_), .O(new_n87_));
  nand3  g46(.a(new_n76_), .b(x16), .c(new_n70_), .O(new_n88_));
  aoi21  g47(.a(new_n50_), .b(x03), .c(new_n61_), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  nand2  g50(.a(new_n50_), .b(new_n91_), .O(new_n92_));
  nand2  g51(.a(new_n76_), .b(new_n63_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n46_), .O(z16));
  inv1   g54(.a(x05), .O(new_n96_));
  nand2  g55(.a(new_n50_), .b(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n76_), .b(new_n45_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(new_n97_), .c(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n46_), .O(z17));
  inv1   g59(.a(x06), .O(new_n101_));
  nand2  g60(.a(new_n50_), .b(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n76_), .b(new_n58_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n102_), .c(new_n74_), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z18));
  inv1   g64(.a(x07), .O(new_n106_));
  nand2  g65(.a(new_n50_), .b(new_n106_), .O(new_n107_));
  nand2  g66(.a(new_n76_), .b(new_n43_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n46_), .O(z19));
endmodule


