// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x08), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x10), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x08), .c(new_n44_), .O(z01));
  nand2  g09(.a(new_n46_), .b(x14), .O(new_n51_));
  inv1   g10(.a(x10), .O(new_n52_));
  nor2   g11(.a(x15), .b(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x08), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(z02));
  nand3  g15(.a(new_n48_), .b(x12), .c(x10), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x08), .c(new_n44_), .O(z03));
  nand2  g17(.a(new_n46_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n46_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(z10));
  inv1   g25(.a(z10), .O(z09));
  inv1   g26(.a(new_n51_), .O(z11));
  inv1   g27(.a(x12), .O(new_n69_));
  inv1   g28(.a(x00), .O(new_n70_));
  inv1   g29(.a(x13), .O(new_n71_));
  nand2  g30(.a(x10), .b(x08), .O(new_n72_));
  oai22  g31(.a(new_n72_), .b(new_n70_), .c(new_n71_), .d(x08), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n44_), .O(new_n74_));
  nand2  g33(.a(new_n44_), .b(x10), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x13), .c(x08), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n74_), .c(new_n69_), .O(z12));
  inv1   g36(.a(x01), .O(new_n78_));
  oai22  g37(.a(new_n75_), .b(new_n78_), .c(x14), .d(x10), .O(new_n79_));
  oai22  g38(.a(new_n45_), .b(x12), .c(x15), .d(x14), .O(new_n80_));
  aoi21  g39(.a(new_n79_), .b(x08), .c(new_n80_), .O(z13));
  nand2  g40(.a(x15), .b(new_n52_), .O(new_n82_));
  nand2  g41(.a(new_n53_), .b(x02), .O(new_n83_));
  nand2  g42(.a(x12), .b(x08), .O(new_n84_));
  aoi21  g43(.a(new_n83_), .b(new_n82_), .c(new_n84_), .O(z14));
  inv1   g44(.a(x08), .O(new_n86_));
  nand2  g45(.a(x16), .b(new_n48_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n69_), .c(new_n44_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g48(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n90_));
  oai21  g49(.a(new_n87_), .b(new_n53_), .c(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n89_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  oai22  g53(.a(new_n72_), .b(new_n94_), .c(new_n62_), .d(x08), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n44_), .O(new_n96_));
  nand3  g55(.a(new_n75_), .b(x17), .c(x08), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n69_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  oai22  g59(.a(new_n72_), .b(new_n99_), .c(new_n100_), .d(x08), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n44_), .O(new_n102_));
  nand3  g61(.a(new_n75_), .b(x18), .c(x08), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n69_), .O(z17));
  oai21  g63(.a(new_n60_), .b(new_n69_), .c(new_n44_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  nand4  g65(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n107_));
  oai21  g66(.a(new_n53_), .b(new_n60_), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n106_), .O(z18));
  inv1   g69(.a(x07), .O(new_n111_));
  oai22  g70(.a(new_n72_), .b(new_n111_), .c(new_n43_), .d(x08), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n44_), .O(new_n113_));
  nand3  g72(.a(new_n75_), .b(x20), .c(x08), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n69_), .O(z19));
  buf    g74(.a(x15), .O(z06));
endmodule


