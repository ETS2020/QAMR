// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x18), .b(x05), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(x05), .O(new_n46_));
  inv1   g05(.a(x18), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(x15), .b(x10), .c(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n48_), .O(z01));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x15), .c(new_n49_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n48_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  oai21  g17(.a(new_n52_), .b(new_n58_), .c(new_n48_), .O(z03));
  nor2   g18(.a(x18), .b(new_n46_), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n48_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x15), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n48_), .b(new_n67_), .O(z08));
  nand2  g27(.a(x09), .b(x08), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n44_), .O(z09));
  aoi21  g29(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nor2   g30(.a(new_n44_), .b(new_n49_), .O(z11));
  inv1   g31(.a(x00), .O(new_n73_));
  nand4  g32(.a(new_n63_), .b(x10), .c(x08), .d(new_n73_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  oai21  g34(.a(new_n54_), .b(x15), .c(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n74_), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n48_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n54_), .b(new_n79_), .c(x14), .O(new_n80_));
  nand2  g39(.a(new_n48_), .b(x12), .O(new_n81_));
  aoi21  g40(.a(new_n80_), .b(new_n50_), .c(new_n81_), .O(z13));
  nand3  g41(.a(x10), .b(x08), .c(x02), .O(new_n83_));
  nand3  g42(.a(new_n50_), .b(new_n48_), .c(x12), .O(new_n84_));
  aoi21  g43(.a(new_n83_), .b(new_n63_), .c(new_n84_), .O(z14));
  nand3  g44(.a(new_n63_), .b(x10), .c(x08), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x16), .c(new_n49_), .O(new_n87_));
  nand4  g46(.a(new_n63_), .b(x10), .c(x08), .d(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n81_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand4  g49(.a(new_n63_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  oai21  g50(.a(new_n54_), .b(x15), .c(new_n65_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n91_), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n48_), .O(z16));
  nand4  g53(.a(new_n63_), .b(x10), .c(x08), .d(new_n46_), .O(new_n95_));
  oai21  g54(.a(new_n54_), .b(x15), .c(new_n47_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(x12), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z17));
  inv1   g57(.a(x06), .O(new_n99_));
  nand4  g58(.a(new_n63_), .b(x10), .c(x08), .d(new_n99_), .O(new_n100_));
  oai21  g59(.a(new_n54_), .b(x15), .c(new_n61_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n100_), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n48_), .O(z18));
  aoi21  g62(.a(new_n47_), .b(new_n46_), .c(new_n58_), .O(new_n104_));
  inv1   g63(.a(x07), .O(new_n105_));
  nand4  g64(.a(new_n63_), .b(x10), .c(x08), .d(new_n105_), .O(new_n106_));
  oai21  g65(.a(new_n54_), .b(x15), .c(new_n43_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z19));
endmodule


