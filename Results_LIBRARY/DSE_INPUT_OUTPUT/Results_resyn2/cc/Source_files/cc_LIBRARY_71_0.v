// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n59_, new_n61_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x19), .b(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x15), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(x14), .c(new_n44_), .O(z01));
  nor2   g08(.a(new_n46_), .b(x15), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x14), .c(x12), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(x19), .c(x11), .O(z02));
  inv1   g11(.a(x15), .O(new_n53_));
  inv1   g12(.a(x14), .O(new_n54_));
  nand3  g13(.a(new_n47_), .b(new_n54_), .c(x12), .O(new_n55_));
  nor3   g14(.a(new_n55_), .b(new_n43_), .c(new_n53_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  nor2   g16(.a(new_n43_), .b(new_n53_), .O(z06));
  inv1   g17(.a(x17), .O(new_n59_));
  nor2   g18(.a(new_n43_), .b(new_n59_), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nor2   g20(.a(new_n43_), .b(new_n61_), .O(z08));
  aoi21  g21(.a(x09), .b(x08), .c(new_n43_), .O(z10));
  inv1   g22(.a(z10), .O(z09));
  nor2   g23(.a(new_n43_), .b(new_n54_), .O(z11));
  inv1   g24(.a(x00), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  nand3  g27(.a(new_n53_), .b(x10), .c(x08), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n67_), .c(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n44_), .O(z12));
  inv1   g31(.a(x01), .O(new_n73_));
  oai21  g32(.a(new_n46_), .b(new_n73_), .c(x14), .O(new_n74_));
  inv1   g33(.a(x12), .O(new_n75_));
  nor2   g34(.a(new_n43_), .b(new_n75_), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(new_n77_));
  aoi21  g36(.a(new_n74_), .b(new_n48_), .c(new_n77_), .O(z13));
  aoi21  g37(.a(new_n47_), .b(x02), .c(x15), .O(new_n79_));
  nand3  g38(.a(new_n48_), .b(new_n44_), .c(x12), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n79_), .O(z14));
  nand3  g40(.a(new_n69_), .b(x16), .c(new_n54_), .O(new_n82_));
  nand3  g41(.a(new_n47_), .b(new_n53_), .c(x03), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n44_), .O(z15));
  inv1   g45(.a(x04), .O(new_n87_));
  nand2  g46(.a(new_n50_), .b(new_n87_), .O(new_n88_));
  nand2  g47(.a(new_n69_), .b(new_n59_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n88_), .c(new_n76_), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z16));
  inv1   g50(.a(x05), .O(new_n92_));
  nand2  g51(.a(new_n50_), .b(new_n92_), .O(new_n93_));
  inv1   g52(.a(x18), .O(new_n94_));
  nand2  g53(.a(new_n69_), .b(new_n94_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n93_), .c(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n44_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand2  g57(.a(new_n50_), .b(new_n98_), .O(new_n99_));
  inv1   g58(.a(x19), .O(new_n100_));
  nand2  g59(.a(new_n69_), .b(new_n100_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n99_), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n44_), .O(z18));
  inv1   g62(.a(x07), .O(new_n104_));
  nand2  g63(.a(new_n50_), .b(new_n104_), .O(new_n105_));
  inv1   g64(.a(x20), .O(new_n106_));
  nand2  g65(.a(new_n69_), .b(new_n106_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(new_n105_), .c(new_n76_), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z19));
  buf    g68(.a(x19), .O(z05));
endmodule


