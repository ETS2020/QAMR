// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n56_, new_n58_, new_n60_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(x12), .O(z06));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(x14), .O(new_n48_));
  and2   g07(.a(new_n48_), .b(z06), .O(z01));
  aoi21  g08(.a(x10), .b(x08), .c(x14), .O(new_n50_));
  nand2  g09(.a(new_n45_), .b(x12), .O(new_n51_));
  nor3   g10(.a(new_n51_), .b(new_n50_), .c(x11), .O(z02));
  inv1   g11(.a(new_n43_), .O(z03));
  nand2  g12(.a(new_n43_), .b(x18), .O(z04));
  and2   g13(.a(new_n43_), .b(x19), .O(z05));
  inv1   g14(.a(x17), .O(new_n56_));
  nand2  g15(.a(new_n43_), .b(new_n56_), .O(z07));
  inv1   g16(.a(x16), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z08));
  nand2  g18(.a(x09), .b(x08), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n43_), .O(z09));
  nand3  g20(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g21(.a(x14), .O(new_n63_));
  nand2  g22(.a(new_n43_), .b(new_n63_), .O(z11));
  inv1   g23(.a(x12), .O(new_n65_));
  nor2   g24(.a(x15), .b(new_n65_), .O(new_n66_));
  inv1   g25(.a(x00), .O(new_n67_));
  nand3  g26(.a(x10), .b(x08), .c(new_n67_), .O(new_n68_));
  inv1   g27(.a(x13), .O(new_n69_));
  nand2  g28(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n68_), .c(new_n66_), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z12));
  inv1   g31(.a(x01), .O(new_n73_));
  oai21  g32(.a(new_n47_), .b(new_n73_), .c(x14), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n45_), .c(new_n65_), .O(z13));
  inv1   g34(.a(x02), .O(new_n76_));
  nor3   g35(.a(new_n51_), .b(new_n47_), .c(new_n76_), .O(z14));
  nand2  g36(.a(new_n50_), .b(x16), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x03), .O(new_n79_));
  and2   g38(.a(new_n79_), .b(new_n45_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n78_), .c(new_n65_), .O(z15));
  inv1   g40(.a(x04), .O(new_n82_));
  nand3  g41(.a(x10), .b(x08), .c(new_n82_), .O(new_n83_));
  nand2  g42(.a(new_n47_), .b(new_n56_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(new_n83_), .c(new_n66_), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(z16));
  nand3  g45(.a(x10), .b(x08), .c(x05), .O(new_n87_));
  aoi21  g46(.a(new_n47_), .b(x18), .c(x15), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n65_), .O(z17));
  nand3  g48(.a(x10), .b(x08), .c(x06), .O(new_n90_));
  aoi21  g49(.a(new_n47_), .b(x19), .c(x15), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n65_), .O(z18));
  inv1   g51(.a(x07), .O(new_n93_));
  nand3  g52(.a(x10), .b(x08), .c(new_n93_), .O(new_n94_));
  inv1   g53(.a(x20), .O(new_n95_));
  nand2  g54(.a(new_n47_), .b(new_n95_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n94_), .c(new_n66_), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z19));
endmodule


