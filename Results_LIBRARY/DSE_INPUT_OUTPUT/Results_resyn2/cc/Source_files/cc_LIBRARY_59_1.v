// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(x06), .c(new_n43_), .O(z01));
  nor2   g10(.a(new_n48_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n45_), .O(z02));
  nand2  g15(.a(x15), .b(x06), .O(new_n57_));
  nor3   g16(.a(new_n57_), .b(new_n50_), .c(new_n53_), .O(z03));
  nor2   g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n44_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nor2   g27(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g28(.a(new_n57_), .O(new_n70_));
  aoi21  g29(.a(x10), .b(x08), .c(x15), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n70_), .c(x13), .O(new_n72_));
  nand2  g31(.a(new_n52_), .b(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n72_), .c(new_n53_), .O(z12));
  nand2  g33(.a(new_n52_), .b(x01), .O(new_n75_));
  oai21  g34(.a(new_n48_), .b(new_n43_), .c(new_n47_), .O(new_n76_));
  inv1   g35(.a(x06), .O(new_n77_));
  aoi21  g36(.a(x15), .b(new_n77_), .c(new_n53_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z13));
  nand2  g39(.a(new_n49_), .b(x02), .O(new_n81_));
  oai21  g40(.a(new_n48_), .b(new_n43_), .c(x12), .O(new_n82_));
  aoi21  g41(.a(new_n81_), .b(new_n57_), .c(new_n82_), .O(z14));
  nor2   g42(.a(new_n64_), .b(x14), .O(new_n84_));
  oai21  g43(.a(new_n71_), .b(new_n70_), .c(new_n84_), .O(new_n85_));
  nand2  g44(.a(new_n52_), .b(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n53_), .O(z15));
  nand2  g46(.a(new_n70_), .b(x17), .O(new_n88_));
  inv1   g47(.a(x04), .O(new_n89_));
  nand3  g48(.a(x10), .b(x08), .c(new_n89_), .O(new_n90_));
  nand2  g49(.a(new_n48_), .b(new_n62_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(new_n43_), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n88_), .c(new_n53_), .O(z16));
  nand2  g52(.a(new_n70_), .b(x18), .O(new_n94_));
  inv1   g53(.a(x05), .O(new_n95_));
  nand3  g54(.a(x10), .b(x08), .c(new_n95_), .O(new_n96_));
  inv1   g55(.a(x18), .O(new_n97_));
  nand2  g56(.a(new_n48_), .b(new_n97_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(new_n96_), .c(new_n43_), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n94_), .c(new_n53_), .O(z17));
  nor2   g59(.a(new_n71_), .b(x06), .O(new_n101_));
  oai21  g60(.a(new_n52_), .b(x19), .c(x12), .O(new_n102_));
  nor2   g61(.a(new_n102_), .b(new_n101_), .O(z18));
  nand2  g62(.a(new_n70_), .b(x20), .O(new_n104_));
  inv1   g63(.a(x07), .O(new_n105_));
  nand3  g64(.a(x10), .b(x08), .c(new_n105_), .O(new_n106_));
  inv1   g65(.a(x20), .O(new_n107_));
  nand2  g66(.a(new_n48_), .b(new_n107_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n106_), .c(new_n43_), .O(new_n109_));
  aoi21  g68(.a(new_n109_), .b(new_n104_), .c(new_n53_), .O(z19));
  buf    g69(.a(x15), .O(z06));
endmodule


