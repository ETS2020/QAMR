// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x12), .O(new_n43_));
  nand2  g02(.a(x15), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor4   g06(.a(new_n47_), .b(new_n46_), .c(x14), .d(new_n43_), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  and2   g08(.a(x10), .b(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand2  g11(.a(x12), .b(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n49_), .c(new_n53_), .O(z02));
  nand2  g13(.a(new_n44_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  aoi21  g15(.a(x15), .b(new_n43_), .c(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n44_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n44_), .O(z09));
  nand3  g22(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g23(.a(new_n44_), .b(new_n49_), .O(z11));
  inv1   g24(.a(x00), .O(new_n66_));
  nand3  g25(.a(new_n50_), .b(new_n46_), .c(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  oai21  g27(.a(new_n47_), .b(x15), .c(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n67_), .c(x12), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z12));
  oai21  g30(.a(new_n50_), .b(new_n43_), .c(x15), .O(new_n72_));
  nand2  g31(.a(new_n50_), .b(x01), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(x14), .c(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n72_), .O(z13));
  nand2  g34(.a(new_n50_), .b(x02), .O(new_n76_));
  oai21  g35(.a(new_n47_), .b(new_n46_), .c(x12), .O(new_n77_));
  aoi21  g36(.a(new_n76_), .b(new_n46_), .c(new_n77_), .O(z14));
  oai21  g37(.a(new_n60_), .b(x14), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x15), .O(new_n80_));
  nand2  g39(.a(new_n46_), .b(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n50_), .O(new_n82_));
  oai21  g41(.a(new_n60_), .b(x14), .c(new_n47_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n82_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n80_), .O(z15));
  oai21  g44(.a(new_n47_), .b(x04), .c(x12), .O(new_n86_));
  nor2   g45(.a(x17), .b(new_n43_), .O(new_n87_));
  aoi22  g46(.a(new_n87_), .b(new_n51_), .c(new_n86_), .d(new_n46_), .O(z16));
  inv1   g47(.a(x05), .O(new_n89_));
  nand3  g48(.a(new_n50_), .b(new_n46_), .c(new_n89_), .O(new_n90_));
  inv1   g49(.a(x18), .O(new_n91_));
  oai21  g50(.a(new_n47_), .b(x15), .c(new_n91_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(new_n90_), .c(x12), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z17));
  inv1   g53(.a(x06), .O(new_n95_));
  nand3  g54(.a(new_n50_), .b(new_n46_), .c(new_n95_), .O(new_n96_));
  oai21  g55(.a(new_n47_), .b(x15), .c(new_n56_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(x12), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z18));
  oai21  g58(.a(new_n47_), .b(x07), .c(x12), .O(new_n100_));
  nor2   g59(.a(x20), .b(new_n43_), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n51_), .c(new_n100_), .d(new_n46_), .O(z19));
  nor4   g61(.a(new_n47_), .b(new_n46_), .c(x14), .d(new_n43_), .O(z03));
  buf    g62(.a(x15), .O(z06));
endmodule


