// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_;
  nor2   g00(.a(x20), .b(x15), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n46_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x15), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  aoi21  g14(.a(new_n52_), .b(new_n46_), .c(new_n55_), .O(z02));
  nor2   g15(.a(new_n48_), .b(new_n53_), .O(z03));
  nand2  g16(.a(new_n43_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n43_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n42_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n42_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n42_), .O(z09));
  aoi21  g25(.a(x09), .b(x08), .c(new_n42_), .O(z10));
  nand2  g26(.a(new_n43_), .b(new_n46_), .O(z11));
  inv1   g27(.a(x15), .O(new_n69_));
  nand2  g28(.a(x12), .b(x00), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n50_), .c(x20), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g31(.a(new_n52_), .b(x13), .c(x12), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(z12));
  oai21  g33(.a(new_n47_), .b(x14), .c(x15), .O(new_n75_));
  nand2  g34(.a(new_n47_), .b(x01), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x20), .c(x14), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n53_), .O(z13));
  nand2  g37(.a(new_n50_), .b(x15), .O(new_n79_));
  nand3  g38(.a(new_n51_), .b(x20), .c(x02), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n53_), .O(z14));
  nand2  g40(.a(new_n50_), .b(x20), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  nor2   g42(.a(new_n63_), .b(x14), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g44(.a(new_n51_), .b(x20), .c(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n53_), .O(z15));
  nand2  g46(.a(new_n83_), .b(x17), .O(new_n88_));
  nand3  g47(.a(new_n51_), .b(x20), .c(x04), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n53_), .O(z16));
  nand2  g49(.a(x12), .b(x05), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n50_), .c(x20), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n69_), .O(new_n93_));
  nand3  g52(.a(new_n52_), .b(x18), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(z17));
  nand2  g54(.a(new_n83_), .b(x19), .O(new_n96_));
  nand3  g55(.a(new_n51_), .b(x20), .c(x06), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n53_), .O(z18));
  inv1   g57(.a(x07), .O(new_n99_));
  nand2  g58(.a(x20), .b(x12), .O(new_n100_));
  aoi21  g59(.a(new_n51_), .b(new_n99_), .c(new_n100_), .O(z19));
  buf    g60(.a(x15), .O(z06));
endmodule


