// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_,
    new_n62_, new_n64_, new_n65_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x18), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n43_), .O(z02));
  nand2  g15(.a(new_n47_), .b(x12), .O(new_n57_));
  nor2   g16(.a(x18), .b(new_n45_), .O(z06));
  nand2  g17(.a(z06), .b(new_n48_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n57_), .O(z03));
  nand2  g19(.a(x18), .b(new_n45_), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  inv1   g23(.a(new_n43_), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n64_), .O(z07));
  and2   g25(.a(new_n43_), .b(x16), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n65_), .O(z09));
  and2   g28(.a(new_n68_), .b(new_n43_), .O(z10));
  aoi21  g29(.a(x18), .b(x15), .c(new_n47_), .O(z11));
  nand2  g30(.a(new_n52_), .b(x00), .O(new_n72_));
  nand3  g31(.a(new_n45_), .b(x10), .c(x08), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n43_), .c(x13), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n72_), .c(new_n53_), .O(z12));
  inv1   g34(.a(x01), .O(new_n76_));
  nand2  g35(.a(new_n45_), .b(new_n76_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n48_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(z11), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n59_), .c(new_n53_), .O(z13));
  nand2  g39(.a(new_n51_), .b(x15), .O(new_n81_));
  nand4  g40(.a(new_n45_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n43_), .O(z14));
  inv1   g44(.a(x16), .O(new_n86_));
  oai21  g45(.a(new_n57_), .b(new_n86_), .c(new_n46_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand4  g47(.a(new_n45_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand3  g48(.a(new_n51_), .b(x16), .c(new_n47_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n88_), .O(z15));
  oai21  g52(.a(new_n64_), .b(new_n53_), .c(new_n46_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n51_), .b(x17), .O(new_n96_));
  nand4  g55(.a(new_n45_), .b(x10), .c(x08), .d(x04), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n95_), .O(z16));
  nand2  g59(.a(new_n51_), .b(new_n46_), .O(new_n101_));
  inv1   g60(.a(x05), .O(new_n102_));
  nand2  g61(.a(new_n48_), .b(new_n102_), .O(new_n103_));
  nand4  g62(.a(new_n103_), .b(new_n101_), .c(new_n45_), .d(x12), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z17));
  nand2  g64(.a(z06), .b(x19), .O(new_n106_));
  inv1   g65(.a(x06), .O(new_n107_));
  nand3  g66(.a(x10), .b(x08), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n51_), .b(new_n62_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n108_), .c(new_n45_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n106_), .c(new_n53_), .O(z18));
  nand2  g70(.a(z06), .b(x20), .O(new_n112_));
  inv1   g71(.a(x07), .O(new_n113_));
  nand3  g72(.a(x10), .b(x08), .c(new_n113_), .O(new_n114_));
  inv1   g73(.a(x20), .O(new_n115_));
  nand2  g74(.a(new_n51_), .b(new_n115_), .O(new_n116_));
  nand3  g75(.a(new_n116_), .b(new_n114_), .c(new_n45_), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n112_), .c(new_n53_), .O(z19));
endmodule


