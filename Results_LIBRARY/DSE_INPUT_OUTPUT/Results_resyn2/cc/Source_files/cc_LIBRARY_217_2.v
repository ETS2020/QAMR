// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  nand2  g01(.a(x10), .b(x08), .O(new_n43_));
  inv1   g02(.a(x14), .O(new_n44_));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x20), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n43_), .O(z01));
  nand2  g08(.a(new_n46_), .b(x15), .O(new_n50_));
  nor2   g09(.a(new_n43_), .b(x15), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  oai21  g12(.a(new_n51_), .b(x14), .c(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n50_), .O(z02));
  nand3  g14(.a(x12), .b(x10), .c(x08), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x20), .c(new_n45_), .O(z03));
  nor2   g18(.a(x20), .b(new_n45_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n50_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n60_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n60_), .O(z09));
  and2   g28(.a(new_n68_), .b(new_n50_), .O(z10));
  nor2   g29(.a(new_n60_), .b(new_n44_), .O(z11));
  nand2  g30(.a(x13), .b(x12), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x20), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x15), .O(new_n74_));
  nand2  g33(.a(new_n43_), .b(x13), .O(new_n75_));
  nand4  g34(.a(new_n45_), .b(x10), .c(x08), .d(x00), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n74_), .O(z12));
  oai21  g38(.a(new_n57_), .b(new_n46_), .c(x15), .O(new_n80_));
  inv1   g39(.a(new_n43_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x01), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x14), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n80_), .O(z13));
  nor2   g43(.a(new_n60_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n51_), .b(x02), .O(new_n86_));
  oai21  g45(.a(new_n43_), .b(new_n46_), .c(x15), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(z14));
  nand2  g47(.a(new_n51_), .b(x03), .O(new_n89_));
  nand3  g48(.a(new_n45_), .b(x10), .c(x08), .O(new_n90_));
  nand4  g49(.a(new_n90_), .b(new_n50_), .c(x16), .d(new_n44_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n89_), .c(new_n52_), .O(z15));
  nand2  g51(.a(new_n47_), .b(x17), .O(new_n93_));
  inv1   g52(.a(x04), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n43_), .b(new_n64_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n45_), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n93_), .c(new_n52_), .O(z16));
  nand2  g57(.a(new_n47_), .b(x18), .O(new_n99_));
  inv1   g58(.a(x05), .O(new_n100_));
  nand3  g59(.a(x10), .b(x08), .c(new_n100_), .O(new_n101_));
  inv1   g60(.a(x18), .O(new_n102_));
  nand2  g61(.a(new_n43_), .b(new_n102_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n101_), .c(new_n45_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n99_), .c(new_n52_), .O(z17));
  oai21  g64(.a(new_n62_), .b(new_n52_), .c(x20), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x15), .O(new_n107_));
  nand2  g66(.a(new_n43_), .b(x19), .O(new_n108_));
  nand4  g67(.a(new_n45_), .b(x10), .c(x08), .d(x06), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n107_), .O(z18));
  nand2  g71(.a(new_n81_), .b(x07), .O(new_n113_));
  aoi21  g72(.a(new_n43_), .b(x20), .c(x15), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n85_), .O(z19));
  buf    g74(.a(x15), .O(z06));
endmodule


