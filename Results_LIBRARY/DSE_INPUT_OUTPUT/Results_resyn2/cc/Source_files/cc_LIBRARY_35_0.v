// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n51_,
    new_n54_, new_n57_, new_n59_, new_n61_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x15), .b(x14), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  aoi21  g05(.a(new_n46_), .b(x15), .c(x14), .O(z01));
  nand2  g06(.a(x14), .b(x12), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(x11), .c(new_n44_), .O(z02));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(x12), .b(x10), .c(x08), .O(new_n51_));
  nor3   g10(.a(new_n51_), .b(new_n50_), .c(x14), .O(z03));
  nand2  g11(.a(new_n44_), .b(x18), .O(z04));
  inv1   g12(.a(x19), .O(new_n54_));
  nand2  g13(.a(new_n44_), .b(new_n54_), .O(z05));
  nand2  g14(.a(new_n50_), .b(x14), .O(z06));
  inv1   g15(.a(x17), .O(new_n57_));
  nand2  g16(.a(new_n44_), .b(new_n57_), .O(z07));
  inv1   g17(.a(x16), .O(new_n59_));
  nor2   g18(.a(new_n43_), .b(new_n59_), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n43_), .O(z09));
  aoi21  g21(.a(x09), .b(x08), .c(new_n43_), .O(z10));
  inv1   g22(.a(x00), .O(new_n64_));
  oai21  g23(.a(new_n51_), .b(new_n64_), .c(x14), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n50_), .O(new_n66_));
  and2   g25(.a(x10), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n50_), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(x13), .c(x12), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n66_), .O(z12));
  inv1   g29(.a(x14), .O(new_n71_));
  nand2  g30(.a(new_n51_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g32(.a(new_n67_), .b(new_n50_), .c(x01), .O(new_n74_));
  inv1   g33(.a(new_n74_), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n48_), .c(new_n73_), .O(z13));
  inv1   g35(.a(x12), .O(new_n77_));
  nand2  g36(.a(new_n46_), .b(x15), .O(new_n78_));
  nand4  g37(.a(new_n67_), .b(new_n50_), .c(x14), .d(x02), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(z14));
  nand3  g39(.a(x16), .b(x15), .c(new_n71_), .O(new_n81_));
  nand4  g40(.a(new_n67_), .b(new_n50_), .c(x14), .d(x03), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n77_), .O(z15));
  nand2  g42(.a(x17), .b(x15), .O(new_n84_));
  nand2  g43(.a(new_n50_), .b(x04), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  aoi21  g45(.a(new_n46_), .b(new_n57_), .c(new_n71_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n84_), .c(new_n77_), .O(z16));
  inv1   g48(.a(x05), .O(new_n90_));
  oai21  g49(.a(new_n51_), .b(new_n90_), .c(x14), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n50_), .O(new_n92_));
  nand3  g51(.a(new_n68_), .b(x18), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z17));
  inv1   g53(.a(x06), .O(new_n95_));
  oai21  g54(.a(new_n51_), .b(new_n95_), .c(x14), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n50_), .O(new_n97_));
  nand3  g56(.a(new_n68_), .b(x19), .c(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(z18));
  nand2  g58(.a(x20), .b(x15), .O(new_n100_));
  nand2  g59(.a(new_n50_), .b(x07), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n67_), .O(new_n102_));
  inv1   g61(.a(x20), .O(new_n103_));
  aoi21  g62(.a(new_n46_), .b(new_n103_), .c(new_n71_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n100_), .c(new_n77_), .O(z19));
  buf    g65(.a(x14), .O(z11));
endmodule


