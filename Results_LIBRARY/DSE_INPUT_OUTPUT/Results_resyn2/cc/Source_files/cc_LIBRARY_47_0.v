// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n61_, new_n63_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x03), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  and2   g08(.a(x10), .b(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g10(.a(new_n44_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n48_), .c(new_n53_), .O(z02));
  nand2  g13(.a(z01), .b(x12), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n52_), .O(z03));
  nand2  g15(.a(new_n52_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n52_), .b(new_n58_), .O(z05));
  nand2  g18(.a(new_n49_), .b(x03), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z08));
  aoi21  g23(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g24(.a(z10), .O(z09));
  nand2  g25(.a(new_n52_), .b(new_n48_), .O(z11));
  inv1   g26(.a(x12), .O(new_n68_));
  nand2  g27(.a(x15), .b(x13), .O(new_n69_));
  nand2  g28(.a(new_n49_), .b(x00), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n50_), .O(new_n71_));
  inv1   g30(.a(x03), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n73_), .c(new_n72_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n69_), .c(new_n68_), .O(z12));
  nand3  g36(.a(x10), .b(x08), .c(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x14), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n52_), .O(z13));
  nand3  g41(.a(new_n49_), .b(x03), .c(x02), .O(new_n83_));
  oai21  g42(.a(new_n50_), .b(x15), .c(x12), .O(new_n84_));
  aoi21  g43(.a(new_n83_), .b(new_n50_), .c(new_n84_), .O(z14));
  nand2  g44(.a(x16), .b(new_n48_), .O(new_n86_));
  nand2  g45(.a(new_n52_), .b(x12), .O(new_n87_));
  aoi21  g46(.a(new_n86_), .b(new_n51_), .c(new_n87_), .O(z15));
  nand4  g47(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n49_), .O(new_n91_));
  nand2  g50(.a(new_n51_), .b(x12), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n61_), .c(new_n91_), .O(z16));
  inv1   g52(.a(x18), .O(new_n94_));
  nand4  g53(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x03), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n49_), .O(new_n97_));
  oai21  g56(.a(new_n92_), .b(new_n94_), .c(new_n97_), .O(z17));
  nand4  g57(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x03), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n49_), .O(new_n101_));
  oai21  g60(.a(new_n92_), .b(new_n58_), .c(new_n101_), .O(z18));
  nand2  g61(.a(x20), .b(x15), .O(new_n103_));
  nand2  g62(.a(new_n49_), .b(x07), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n50_), .O(new_n105_));
  aoi21  g64(.a(new_n74_), .b(new_n43_), .c(new_n72_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n103_), .c(new_n68_), .O(z19));
endmodule


