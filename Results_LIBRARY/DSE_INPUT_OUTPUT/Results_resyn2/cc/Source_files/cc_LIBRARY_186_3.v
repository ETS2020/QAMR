// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x17), .b(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand2  g03(.a(x10), .b(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  nor2   g06(.a(x17), .b(new_n47_), .O(z06));
  nand2  g07(.a(z06), .b(new_n46_), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n45_), .O(z01));
  nor2   g09(.a(new_n45_), .b(x15), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  oai21  g12(.a(new_n51_), .b(x14), .c(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n43_), .O(z02));
  inv1   g14(.a(x17), .O(new_n56_));
  nand4  g15(.a(new_n46_), .b(x12), .c(x10), .d(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n56_), .c(new_n47_), .O(z03));
  inv1   g17(.a(new_n43_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n59_), .b(new_n61_), .O(z05));
  and2   g21(.a(new_n43_), .b(x16), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n43_), .O(z09));
  inv1   g24(.a(z09), .O(z10));
  nor2   g25(.a(new_n59_), .b(new_n46_), .O(z11));
  nand2  g26(.a(new_n51_), .b(x00), .O(new_n68_));
  nand3  g27(.a(new_n47_), .b(x10), .c(x08), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n43_), .c(x13), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n68_), .c(new_n52_), .O(z12));
  nand2  g30(.a(new_n45_), .b(new_n46_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(z06), .O(new_n73_));
  nand3  g32(.a(x10), .b(x08), .c(x01), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n47_), .c(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n52_), .O(z13));
  nand2  g35(.a(new_n45_), .b(x15), .O(new_n77_));
  nand4  g36(.a(new_n47_), .b(x10), .c(x08), .d(x02), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n43_), .O(z14));
  nand3  g40(.a(x16), .b(new_n46_), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n56_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x15), .O(new_n84_));
  nand3  g43(.a(new_n45_), .b(x16), .c(new_n46_), .O(new_n85_));
  nand4  g44(.a(new_n47_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n84_), .O(z15));
  nand2  g48(.a(new_n45_), .b(new_n56_), .O(new_n90_));
  inv1   g49(.a(x04), .O(new_n91_));
  nand3  g50(.a(x10), .b(x08), .c(new_n91_), .O(new_n92_));
  nand4  g51(.a(new_n92_), .b(new_n90_), .c(new_n47_), .d(x12), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z16));
  nand2  g53(.a(z06), .b(x18), .O(new_n95_));
  inv1   g54(.a(x05), .O(new_n96_));
  nand3  g55(.a(x10), .b(x08), .c(new_n96_), .O(new_n97_));
  inv1   g56(.a(x18), .O(new_n98_));
  nand2  g57(.a(new_n45_), .b(new_n98_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n97_), .c(new_n47_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n95_), .c(new_n52_), .O(z17));
  oai21  g60(.a(new_n61_), .b(new_n52_), .c(new_n56_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x15), .O(new_n103_));
  nand2  g62(.a(new_n45_), .b(x19), .O(new_n104_));
  nand4  g63(.a(new_n47_), .b(x10), .c(x08), .d(x06), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n103_), .O(z18));
  nand2  g67(.a(z06), .b(x20), .O(new_n109_));
  inv1   g68(.a(x07), .O(new_n110_));
  nand3  g69(.a(x10), .b(x08), .c(new_n110_), .O(new_n111_));
  inv1   g70(.a(x20), .O(new_n112_));
  nand2  g71(.a(new_n45_), .b(new_n112_), .O(new_n113_));
  nand3  g72(.a(new_n113_), .b(new_n111_), .c(new_n47_), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n109_), .c(new_n52_), .O(z19));
  buf    g74(.a(x17), .O(z07));
endmodule


