// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n57_, new_n59_, new_n61_, new_n64_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_;
  nor2   g00(.a(x17), .b(x15), .O(new_n42_));
  nand2  g01(.a(x20), .b(x11), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand3  g03(.a(x15), .b(x10), .c(x08), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(x14), .O(z01));
  inv1   g05(.a(new_n42_), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g08(.a(x12), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x11), .O(new_n51_));
  oai21  g10(.a(new_n49_), .b(x14), .c(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(z02));
  nand2  g12(.a(z01), .b(x12), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n47_), .O(z03));
  nor2   g14(.a(new_n42_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nand2  g16(.a(new_n47_), .b(new_n57_), .O(z05));
  inv1   g17(.a(x16), .O(new_n59_));
  nor2   g18(.a(new_n42_), .b(new_n59_), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n47_), .O(z09));
  nand3  g21(.a(new_n47_), .b(x09), .c(x08), .O(z10));
  inv1   g22(.a(x14), .O(new_n64_));
  nor2   g23(.a(new_n42_), .b(new_n64_), .O(z11));
  inv1   g24(.a(x15), .O(new_n66_));
  nand2  g25(.a(new_n48_), .b(x17), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(x13), .O(new_n69_));
  nand3  g28(.a(new_n49_), .b(x17), .c(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n50_), .O(z12));
  inv1   g30(.a(x01), .O(new_n72_));
  oai21  g31(.a(new_n48_), .b(new_n72_), .c(x14), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n47_), .O(z13));
  inv1   g35(.a(new_n48_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x17), .c(x02), .O(new_n78_));
  nand2  g37(.a(new_n45_), .b(x12), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n66_), .c(new_n79_), .O(z14));
  nor2   g39(.a(new_n59_), .b(x14), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n68_), .O(new_n82_));
  nand3  g41(.a(new_n49_), .b(x17), .c(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n50_), .O(z15));
  nand2  g43(.a(new_n77_), .b(new_n66_), .O(new_n85_));
  inv1   g44(.a(x17), .O(new_n86_));
  nor2   g45(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  oai21  g46(.a(new_n85_), .b(x04), .c(new_n87_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n47_), .O(z16));
  nand2  g48(.a(x18), .b(x15), .O(new_n90_));
  nand2  g49(.a(new_n66_), .b(x05), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n77_), .O(new_n92_));
  inv1   g51(.a(x18), .O(new_n93_));
  aoi21  g52(.a(new_n48_), .b(new_n93_), .c(new_n86_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n90_), .c(new_n50_), .O(z17));
  nand2  g55(.a(x19), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n66_), .b(x06), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n77_), .O(new_n99_));
  aoi21  g58(.a(new_n48_), .b(new_n57_), .c(new_n86_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n97_), .c(new_n50_), .O(z18));
  nand2  g61(.a(x12), .b(x07), .O(new_n103_));
  oai21  g62(.a(new_n103_), .b(new_n48_), .c(x17), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n66_), .O(new_n105_));
  nand3  g64(.a(new_n85_), .b(x20), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(z19));
  buf    g66(.a(x15), .O(z06));
  buf    g67(.a(x17), .O(z07));
endmodule


