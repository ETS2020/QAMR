// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n59_, new_n61_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x10), .O(new_n43_));
  nand2  g02(.a(x15), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x10), .c(new_n46_), .O(z01));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  oai21  g09(.a(new_n50_), .b(x15), .c(new_n47_), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n44_), .O(z02));
  nor4   g14(.a(new_n50_), .b(new_n46_), .c(x14), .d(new_n52_), .O(z03));
  inv1   g15(.a(new_n44_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n44_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n57_), .b(new_n61_), .O(z07));
  and2   g21(.a(new_n44_), .b(x16), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n44_), .O(z09));
  nand3  g24(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g25(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g26(.a(x00), .O(new_n68_));
  inv1   g27(.a(x13), .O(new_n69_));
  oai22  g28(.a(new_n50_), .b(new_n68_), .c(new_n69_), .d(x10), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g30(.a(new_n46_), .b(x08), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x13), .c(x10), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n52_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n50_), .b(new_n75_), .c(x14), .O(new_n76_));
  inv1   g35(.a(x08), .O(new_n77_));
  nand2  g36(.a(new_n47_), .b(new_n77_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n44_), .c(x12), .O(new_n79_));
  aoi21  g38(.a(new_n76_), .b(new_n46_), .c(new_n79_), .O(z13));
  nand4  g39(.a(new_n46_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  oai21  g40(.a(new_n46_), .b(x08), .c(new_n81_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(z14));
  nand3  g43(.a(x16), .b(new_n47_), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n46_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n43_), .O(new_n87_));
  nor2   g46(.a(x15), .b(new_n77_), .O(new_n88_));
  nand4  g47(.a(new_n46_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n47_), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n87_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  oai22  g53(.a(new_n50_), .b(new_n94_), .c(new_n61_), .d(x10), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n46_), .O(new_n96_));
  nand3  g55(.a(new_n72_), .b(x17), .c(x10), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n52_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  oai22  g59(.a(new_n50_), .b(new_n99_), .c(new_n100_), .d(x10), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n46_), .O(new_n102_));
  nand3  g61(.a(new_n72_), .b(x18), .c(x10), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n52_), .O(z17));
  oai21  g63(.a(new_n59_), .b(new_n52_), .c(new_n46_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n43_), .O(new_n106_));
  nand4  g65(.a(new_n46_), .b(x10), .c(x08), .d(x06), .O(new_n107_));
  oai21  g66(.a(new_n88_), .b(new_n59_), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n106_), .O(z18));
  inv1   g69(.a(x20), .O(new_n111_));
  oai21  g70(.a(new_n111_), .b(new_n52_), .c(new_n46_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n43_), .O(new_n113_));
  nand4  g72(.a(new_n46_), .b(x10), .c(x08), .d(x07), .O(new_n114_));
  oai21  g73(.a(new_n88_), .b(new_n111_), .c(new_n114_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x12), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n113_), .O(z19));
  buf    g76(.a(x15), .O(z06));
endmodule


