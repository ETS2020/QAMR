// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_;
  inv1   g00(.a(x15), .O(new_n42_));
  nor2   g01(.a(x19), .b(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x19), .c(new_n42_), .O(z01));
  inv1   g09(.a(new_n43_), .O(new_n51_));
  oai21  g10(.a(new_n47_), .b(x15), .c(new_n46_), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n51_), .O(z02));
  nand3  g15(.a(new_n48_), .b(new_n46_), .c(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x19), .c(new_n42_), .O(z03));
  nand2  g17(.a(new_n51_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n42_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n51_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n51_), .O(z09));
  nand3  g26(.a(new_n51_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n51_), .b(new_n46_), .O(z11));
  nand2  g28(.a(x13), .b(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x19), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n47_), .b(x13), .O(new_n73_));
  nand4  g32(.a(new_n42_), .b(x10), .c(x08), .d(x00), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n72_), .O(z12));
  nand2  g36(.a(new_n47_), .b(new_n46_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x19), .c(x15), .O(new_n79_));
  nand3  g38(.a(x10), .b(x08), .c(x01), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n42_), .c(x14), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n79_), .c(new_n53_), .O(z13));
  nand2  g41(.a(new_n47_), .b(x15), .O(new_n83_));
  nand4  g42(.a(new_n42_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n51_), .O(z14));
  nand3  g46(.a(x16), .b(new_n46_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x19), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand3  g49(.a(new_n47_), .b(x16), .c(new_n46_), .O(new_n91_));
  nand4  g50(.a(new_n42_), .b(x10), .c(x08), .d(x03), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  nand3  g54(.a(x19), .b(x17), .c(x15), .O(new_n96_));
  inv1   g55(.a(x04), .O(new_n97_));
  nand3  g56(.a(x10), .b(x08), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n47_), .b(new_n62_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(new_n42_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n53_), .O(z16));
  nand2  g60(.a(x18), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x19), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x15), .O(new_n104_));
  nand2  g63(.a(new_n47_), .b(x18), .O(new_n105_));
  nand4  g64(.a(new_n42_), .b(x10), .c(x08), .d(x05), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n104_), .O(z17));
  inv1   g68(.a(x06), .O(new_n110_));
  nand3  g69(.a(new_n48_), .b(new_n42_), .c(new_n110_), .O(new_n111_));
  oai21  g70(.a(new_n47_), .b(x15), .c(new_n60_), .O(new_n112_));
  nand3  g71(.a(new_n112_), .b(new_n111_), .c(x12), .O(new_n113_));
  inv1   g72(.a(new_n113_), .O(z18));
  nand2  g73(.a(x20), .b(x12), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x19), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(x15), .O(new_n117_));
  nand2  g76(.a(new_n47_), .b(x20), .O(new_n118_));
  nand4  g77(.a(new_n42_), .b(x10), .c(x08), .d(x07), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g79(.a(new_n120_), .b(x12), .O(new_n121_));
  nand2  g80(.a(new_n121_), .b(new_n117_), .O(z19));
  buf    g81(.a(x15), .O(z06));
endmodule


