// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n47_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n45_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x15), .c(new_n47_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n45_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor2   g14(.a(new_n49_), .b(new_n55_), .O(z03));
  nor2   g15(.a(new_n44_), .b(x18), .O(z04));
  nand2  g16(.a(new_n45_), .b(x19), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(z05));
  inv1   g18(.a(x15), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x06), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n44_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nor2   g27(.a(new_n44_), .b(new_n47_), .O(z11));
  nand2  g28(.a(x15), .b(x13), .O(new_n70_));
  nand2  g29(.a(new_n60_), .b(x00), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n48_), .O(new_n72_));
  inv1   g31(.a(x06), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  aoi21  g33(.a(new_n51_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n70_), .c(new_n55_), .O(z12));
  oai21  g36(.a(new_n48_), .b(x14), .c(x15), .O(new_n78_));
  nand2  g37(.a(new_n48_), .b(x01), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x14), .c(x06), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n78_), .c(new_n55_), .O(z13));
  nand2  g40(.a(new_n51_), .b(x15), .O(new_n82_));
  nand4  g41(.a(new_n48_), .b(new_n60_), .c(x06), .d(x02), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n55_), .O(z14));
  nand3  g43(.a(x16), .b(x15), .c(new_n47_), .O(new_n85_));
  nand2  g44(.a(new_n60_), .b(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n48_), .O(new_n87_));
  oai21  g46(.a(new_n64_), .b(x14), .c(new_n51_), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(new_n87_), .c(x06), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n85_), .c(new_n55_), .O(z15));
  nand2  g49(.a(x17), .b(x15), .O(new_n91_));
  nand2  g50(.a(new_n60_), .b(x04), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  aoi21  g52(.a(new_n51_), .b(new_n62_), .c(new_n73_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n91_), .c(new_n55_), .O(z16));
  nand2  g55(.a(x18), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n60_), .b(x05), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  inv1   g58(.a(x18), .O(new_n100_));
  aoi21  g59(.a(new_n51_), .b(new_n100_), .c(new_n73_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n97_), .c(new_n55_), .O(z17));
  nand3  g62(.a(new_n48_), .b(new_n60_), .c(x06), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n58_), .c(new_n55_), .O(z18));
  nand2  g64(.a(x20), .b(x15), .O(new_n106_));
  nand2  g65(.a(new_n60_), .b(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  aoi21  g67(.a(new_n51_), .b(new_n43_), .c(new_n73_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n106_), .c(new_n55_), .O(z19));
endmodule


