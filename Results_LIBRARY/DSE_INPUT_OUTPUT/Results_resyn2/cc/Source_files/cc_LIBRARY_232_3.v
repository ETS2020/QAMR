// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_;
  inv1   g00(.a(x12), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z03));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(z03), .O(z00));
  nor2   g05(.a(new_n43_), .b(x12), .O(z06));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x14), .O(new_n49_));
  and2   g08(.a(new_n49_), .b(z06), .O(z01));
  aoi21  g09(.a(x10), .b(x08), .c(x14), .O(new_n51_));
  nand2  g10(.a(new_n43_), .b(x12), .O(new_n52_));
  nor3   g11(.a(new_n52_), .b(new_n51_), .c(x11), .O(z02));
  inv1   g12(.a(z03), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nor2   g15(.a(z03), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nor2   g17(.a(z03), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n54_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(z03), .O(z09));
  nand3  g22(.a(new_n54_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(x14), .O(new_n65_));
  nand2  g24(.a(new_n54_), .b(new_n65_), .O(z11));
  inv1   g25(.a(new_n48_), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(x00), .O(new_n68_));
  aoi21  g27(.a(new_n48_), .b(x13), .c(x15), .O(new_n69_));
  aoi21  g28(.a(new_n69_), .b(new_n68_), .c(new_n42_), .O(z12));
  nand3  g29(.a(new_n43_), .b(x14), .c(x12), .O(new_n71_));
  aoi21  g30(.a(new_n67_), .b(x01), .c(new_n71_), .O(z13));
  nand2  g31(.a(new_n67_), .b(x02), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n52_), .O(z14));
  nand2  g33(.a(new_n51_), .b(x16), .O(new_n75_));
  nand3  g34(.a(x10), .b(x08), .c(x03), .O(new_n76_));
  and2   g35(.a(new_n76_), .b(new_n43_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n42_), .O(z15));
  nor2   g37(.a(x15), .b(new_n42_), .O(new_n79_));
  inv1   g38(.a(x04), .O(new_n80_));
  nand3  g39(.a(x10), .b(x08), .c(new_n80_), .O(new_n81_));
  nand2  g40(.a(new_n48_), .b(new_n58_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z16));
  nand2  g43(.a(new_n67_), .b(x05), .O(new_n85_));
  aoi21  g44(.a(new_n48_), .b(x18), .c(x15), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n42_), .O(z17));
  inv1   g46(.a(x06), .O(new_n88_));
  nand3  g47(.a(x10), .b(x08), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n48_), .b(new_n56_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n79_), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z18));
  inv1   g51(.a(x07), .O(new_n93_));
  nand3  g52(.a(x10), .b(x08), .c(new_n93_), .O(new_n94_));
  inv1   g53(.a(x20), .O(new_n95_));
  nand2  g54(.a(new_n48_), .b(new_n95_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n94_), .c(new_n79_), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z19));
endmodule


