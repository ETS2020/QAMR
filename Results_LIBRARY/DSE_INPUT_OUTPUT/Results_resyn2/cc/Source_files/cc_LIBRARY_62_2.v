// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x08), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand4  g04(.a(x19), .b(x15), .c(x10), .d(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x10), .O(new_n49_));
  nor2   g08(.a(x15), .b(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x08), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand3  g11(.a(new_n44_), .b(x12), .c(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n48_), .c(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor3   g14(.a(new_n46_), .b(x14), .c(new_n55_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x15), .O(new_n58_));
  aoi21  g17(.a(new_n43_), .b(x08), .c(new_n58_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n44_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n44_), .b(new_n62_), .O(z08));
  nand3  g22(.a(x19), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(z10), .O(z09));
  nand2  g24(.a(new_n44_), .b(new_n48_), .O(z11));
  inv1   g25(.a(x08), .O(new_n67_));
  aoi21  g26(.a(new_n58_), .b(x10), .c(new_n43_), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n67_), .c(x13), .O(new_n69_));
  nand4  g28(.a(new_n50_), .b(x19), .c(x08), .d(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n55_), .O(z12));
  aoi21  g30(.a(x10), .b(x01), .c(new_n43_), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n67_), .c(x14), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n46_), .c(new_n55_), .O(z13));
  nand4  g33(.a(new_n58_), .b(x12), .c(x10), .d(x02), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x19), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g36(.a(x10), .b(x08), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x15), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n77_), .O(z14));
  nand4  g39(.a(new_n58_), .b(x12), .c(x10), .d(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x19), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x08), .O(new_n83_));
  nand4  g42(.a(new_n51_), .b(x16), .c(new_n48_), .d(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(z15));
  nand4  g44(.a(new_n58_), .b(x12), .c(x10), .d(x04), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x19), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x08), .O(new_n88_));
  nand3  g47(.a(new_n51_), .b(x17), .c(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(z16));
  nand4  g49(.a(new_n58_), .b(x12), .c(x10), .d(x05), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x19), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x08), .O(new_n93_));
  nand3  g52(.a(new_n51_), .b(x18), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(z17));
  inv1   g54(.a(x06), .O(new_n96_));
  nand3  g55(.a(new_n50_), .b(x08), .c(new_n96_), .O(new_n97_));
  nor2   g56(.a(new_n43_), .b(new_n55_), .O(new_n98_));
  and2   g57(.a(new_n98_), .b(new_n97_), .O(z18));
  nand4  g58(.a(new_n58_), .b(x12), .c(x10), .d(x07), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x19), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x08), .O(new_n102_));
  nand3  g61(.a(new_n51_), .b(x20), .c(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(z19));
  buf    g63(.a(x19), .O(z05));
endmodule


