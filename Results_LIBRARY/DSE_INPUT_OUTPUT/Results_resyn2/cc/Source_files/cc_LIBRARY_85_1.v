// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n57_, new_n60_, new_n61_, new_n63_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  aoi21  g02(.a(x15), .b(new_n42_), .c(new_n43_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  and2   g04(.a(x10), .b(x08), .O(new_n46_));
  nand3  g05(.a(new_n46_), .b(x15), .c(new_n45_), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(z01));
  inv1   g07(.a(x15), .O(new_n49_));
  nand2  g08(.a(new_n46_), .b(new_n49_), .O(new_n50_));
  nand2  g09(.a(x20), .b(new_n49_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x12), .c(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n50_), .b(new_n45_), .c(new_n52_), .O(z02));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n47_), .b(new_n54_), .O(z03));
  nand2  g14(.a(new_n51_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nand2  g16(.a(new_n51_), .b(new_n57_), .O(z05));
  and2   g17(.a(new_n51_), .b(x17), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  inv1   g19(.a(new_n51_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n61_), .O(z09));
  and2   g23(.a(new_n63_), .b(new_n51_), .O(z10));
  nand2  g24(.a(new_n51_), .b(new_n45_), .O(z11));
  nand4  g25(.a(x12), .b(x10), .c(x08), .d(x00), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n43_), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  nand3  g28(.a(new_n50_), .b(x13), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n69_), .O(z12));
  oai21  g30(.a(new_n46_), .b(x14), .c(x15), .O(new_n72_));
  nand2  g31(.a(new_n46_), .b(x01), .O(new_n73_));
  nor2   g32(.a(x20), .b(new_n45_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n72_), .c(new_n54_), .O(z13));
  inv1   g35(.a(new_n46_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x15), .O(new_n78_));
  nand3  g37(.a(new_n43_), .b(new_n49_), .c(x10), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(new_n80_));
  and2   g39(.a(x08), .b(x02), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n78_), .c(new_n54_), .O(z14));
  aoi21  g42(.a(x10), .b(x08), .c(x20), .O(new_n84_));
  nor2   g43(.a(new_n60_), .b(x14), .O(new_n85_));
  oai21  g44(.a(new_n84_), .b(x15), .c(new_n85_), .O(new_n86_));
  and2   g45(.a(x08), .b(x03), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n80_), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n86_), .c(new_n54_), .O(z15));
  oai21  g48(.a(new_n84_), .b(x15), .c(x17), .O(new_n90_));
  and2   g49(.a(x08), .b(x04), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n80_), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n90_), .c(new_n54_), .O(z16));
  nand4  g52(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n43_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n49_), .O(new_n96_));
  nand3  g55(.a(new_n50_), .b(x18), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(z17));
  oai21  g57(.a(new_n84_), .b(x15), .c(x19), .O(new_n99_));
  and2   g58(.a(x08), .b(x06), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n80_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n99_), .c(new_n54_), .O(z18));
  nand2  g61(.a(x20), .b(x15), .O(new_n103_));
  and2   g62(.a(x08), .b(x07), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n80_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n103_), .c(new_n54_), .O(z19));
  buf    g65(.a(x15), .O(z06));
endmodule


