// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n59_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x05), .O(new_n43_));
  inv1   g02(.a(x18), .O(z04));
  nand2  g03(.a(z04), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand2  g05(.a(new_n45_), .b(x15), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  and2   g07(.a(x10), .b(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n47_), .O(z01));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x15), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  oai21  g14(.a(new_n53_), .b(x14), .c(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n45_), .O(z02));
  nor3   g16(.a(new_n50_), .b(new_n47_), .c(new_n54_), .O(z03));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(x18), .b(x05), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n59_), .O(z05));
  inv1   g20(.a(new_n47_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n60_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n60_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n60_), .O(z10));
  nor2   g28(.a(new_n60_), .b(new_n48_), .O(z11));
  inv1   g29(.a(x00), .O(new_n71_));
  nand2  g30(.a(new_n53_), .b(new_n71_), .O(new_n72_));
  nor2   g31(.a(new_n60_), .b(new_n54_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  inv1   g33(.a(x15), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n73_), .c(new_n72_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z12));
  inv1   g38(.a(new_n73_), .O(new_n80_));
  inv1   g39(.a(x01), .O(new_n81_));
  oai21  g40(.a(new_n52_), .b(new_n81_), .c(x14), .O(new_n82_));
  nand2  g41(.a(new_n49_), .b(x15), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n80_), .O(z13));
  aoi21  g43(.a(new_n49_), .b(x02), .c(x15), .O(new_n85_));
  nand2  g44(.a(new_n83_), .b(x12), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n85_), .c(new_n45_), .O(z14));
  inv1   g46(.a(x03), .O(new_n88_));
  nand2  g47(.a(new_n53_), .b(new_n88_), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n48_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n76_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n89_), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n45_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand2  g53(.a(new_n53_), .b(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n76_), .b(new_n63_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n73_), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z16));
  nand4  g57(.a(new_n75_), .b(x10), .c(x08), .d(new_n43_), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(x12), .c(z04), .O(new_n100_));
  oai21  g59(.a(new_n52_), .b(x15), .c(z04), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x12), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(x05), .c(new_n100_), .O(z17));
  inv1   g62(.a(x06), .O(new_n104_));
  nand2  g63(.a(new_n53_), .b(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n76_), .b(new_n59_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n105_), .c(new_n73_), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z18));
  inv1   g67(.a(x07), .O(new_n109_));
  nand2  g68(.a(new_n53_), .b(new_n109_), .O(new_n110_));
  inv1   g69(.a(x20), .O(new_n111_));
  nand2  g70(.a(new_n76_), .b(new_n111_), .O(new_n112_));
  nand3  g71(.a(new_n112_), .b(new_n110_), .c(x12), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n45_), .O(z19));
endmodule


