// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor4   g06(.a(new_n47_), .b(new_n43_), .c(x14), .d(x00), .O(z01));
  inv1   g07(.a(new_n44_), .O(new_n49_));
  nor2   g08(.a(new_n47_), .b(x15), .O(new_n50_));
  inv1   g09(.a(x12), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x11), .O(new_n52_));
  oai21  g11(.a(new_n50_), .b(x14), .c(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n49_), .O(z02));
  inv1   g13(.a(x14), .O(new_n55_));
  nand3  g14(.a(x12), .b(x10), .c(x08), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n42_), .c(new_n43_), .O(z03));
  nor2   g18(.a(new_n44_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  nor2   g21(.a(new_n43_), .b(x00), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n49_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n49_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n49_), .O(z09));
  nand3  g28(.a(new_n49_), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n49_), .b(new_n55_), .O(z11));
  oai21  g30(.a(new_n57_), .b(x15), .c(x00), .O(new_n72_));
  nand2  g31(.a(x13), .b(x12), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n50_), .c(new_n72_), .O(z12));
  oai21  g33(.a(new_n57_), .b(x00), .c(x15), .O(new_n75_));
  nand3  g34(.a(x10), .b(x08), .c(x01), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x14), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(z13));
  nand2  g37(.a(new_n47_), .b(x15), .O(new_n79_));
  nand4  g38(.a(new_n43_), .b(x10), .c(x08), .d(x02), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n49_), .O(z14));
  nand3  g42(.a(x16), .b(new_n55_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n42_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x15), .O(new_n86_));
  nand3  g45(.a(new_n47_), .b(x16), .c(new_n55_), .O(new_n87_));
  nand4  g46(.a(new_n43_), .b(x10), .c(x08), .d(x03), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n86_), .O(z15));
  oai21  g50(.a(new_n64_), .b(new_n51_), .c(new_n42_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x15), .O(new_n93_));
  nand2  g52(.a(new_n47_), .b(x17), .O(new_n94_));
  nand4  g53(.a(new_n43_), .b(x10), .c(x08), .d(x04), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n93_), .O(z16));
  nand2  g57(.a(z06), .b(x18), .O(new_n99_));
  inv1   g58(.a(x05), .O(new_n100_));
  nand3  g59(.a(x10), .b(x08), .c(new_n100_), .O(new_n101_));
  inv1   g60(.a(x18), .O(new_n102_));
  nand2  g61(.a(new_n47_), .b(new_n102_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n101_), .c(new_n43_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n99_), .c(new_n51_), .O(z17));
  nand2  g64(.a(z06), .b(x19), .O(new_n106_));
  inv1   g65(.a(x06), .O(new_n107_));
  nand3  g66(.a(x10), .b(x08), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n47_), .b(new_n61_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n108_), .c(new_n43_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n106_), .c(new_n51_), .O(z18));
  nand2  g70(.a(z06), .b(x20), .O(new_n112_));
  inv1   g71(.a(x07), .O(new_n113_));
  nand3  g72(.a(x10), .b(x08), .c(new_n113_), .O(new_n114_));
  inv1   g73(.a(x20), .O(new_n115_));
  nand2  g74(.a(new_n47_), .b(new_n115_), .O(new_n116_));
  nand3  g75(.a(new_n116_), .b(new_n114_), .c(new_n43_), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n112_), .c(new_n51_), .O(z19));
endmodule


