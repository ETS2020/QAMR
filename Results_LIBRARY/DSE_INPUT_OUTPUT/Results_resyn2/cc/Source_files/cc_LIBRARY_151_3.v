// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x15), .b(x09), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x09), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(new_n47_), .c(new_n48_), .O(z01));
  nand2  g12(.a(new_n44_), .b(x14), .O(new_n54_));
  nand3  g13(.a(new_n48_), .b(x10), .c(x08), .O(new_n55_));
  nand2  g14(.a(x12), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(z02));
  nand3  g16(.a(new_n51_), .b(new_n49_), .c(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(new_n47_), .c(new_n48_), .O(z03));
  nor2   g18(.a(new_n45_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n44_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z08));
  nand3  g25(.a(new_n48_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  inv1   g27(.a(new_n54_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  nand3  g29(.a(new_n55_), .b(new_n44_), .c(x13), .O(new_n71_));
  nor2   g30(.a(new_n50_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(z12));
  oai21  g33(.a(new_n50_), .b(new_n48_), .c(new_n49_), .O(new_n75_));
  nand4  g34(.a(new_n48_), .b(x10), .c(x08), .d(x01), .O(new_n76_));
  nand4  g35(.a(new_n76_), .b(new_n75_), .c(new_n44_), .d(x12), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z13));
  nor2   g37(.a(new_n48_), .b(x09), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(new_n80_));
  nand2  g39(.a(new_n51_), .b(x02), .O(new_n81_));
  oai21  g40(.a(new_n50_), .b(new_n48_), .c(x12), .O(new_n82_));
  aoi21  g41(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z14));
  nand4  g42(.a(new_n55_), .b(new_n44_), .c(x16), .d(new_n49_), .O(new_n84_));
  nand2  g43(.a(new_n72_), .b(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n70_), .O(z15));
  nand2  g45(.a(new_n79_), .b(x17), .O(new_n87_));
  inv1   g46(.a(x04), .O(new_n88_));
  nand3  g47(.a(x10), .b(x08), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n50_), .b(new_n63_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n48_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n87_), .c(new_n70_), .O(z16));
  nand2  g51(.a(new_n79_), .b(x18), .O(new_n93_));
  inv1   g52(.a(x05), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(new_n94_), .O(new_n95_));
  inv1   g54(.a(x18), .O(new_n96_));
  nand2  g55(.a(new_n50_), .b(new_n96_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n95_), .c(new_n48_), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n93_), .c(new_n70_), .O(z17));
  nand2  g58(.a(new_n79_), .b(x19), .O(new_n100_));
  inv1   g59(.a(x06), .O(new_n101_));
  nand3  g60(.a(x10), .b(x08), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n50_), .b(new_n61_), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n102_), .c(new_n48_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n100_), .c(new_n70_), .O(z18));
  nand2  g64(.a(new_n79_), .b(x20), .O(new_n106_));
  inv1   g65(.a(x07), .O(new_n107_));
  nand3  g66(.a(x10), .b(x08), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n50_), .b(new_n43_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n108_), .c(new_n48_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n106_), .c(new_n70_), .O(z19));
  buf    g70(.a(x15), .O(z06));
endmodule


