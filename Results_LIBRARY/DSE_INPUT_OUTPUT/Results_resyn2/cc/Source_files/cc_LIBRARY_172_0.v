// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_;
  nor2   g00(.a(x19), .b(x15), .O(new_n42_));
  nand2  g01(.a(x20), .b(x11), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(new_n42_), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n45_), .O(z01));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n45_), .O(z02));
  oai21  g15(.a(new_n49_), .b(new_n53_), .c(new_n45_), .O(z03));
  nand2  g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x15), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  nand2  g20(.a(x19), .b(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n42_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n45_), .O(z09));
  nand3  g27(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n45_), .b(new_n46_), .O(z11));
  inv1   g29(.a(x13), .O(new_n71_));
  nand2  g30(.a(x12), .b(x00), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n51_), .c(x19), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  oai21  g33(.a(new_n51_), .b(x15), .c(x12), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n71_), .c(new_n74_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  oai21  g36(.a(new_n51_), .b(new_n77_), .c(x19), .O(new_n78_));
  oai21  g37(.a(new_n48_), .b(x14), .c(x12), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n61_), .c(new_n79_), .O(z13));
  inv1   g39(.a(new_n51_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(x02), .c(x15), .O(new_n82_));
  nand2  g41(.a(new_n47_), .b(x12), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n82_), .c(new_n45_), .O(z14));
  nand2  g43(.a(x12), .b(x03), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n51_), .c(x19), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  nand2  g46(.a(x16), .b(new_n46_), .O(new_n88_));
  oai21  g47(.a(new_n88_), .b(new_n75_), .c(new_n87_), .O(z15));
  nand2  g48(.a(new_n51_), .b(x19), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n61_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x17), .O(new_n92_));
  nand4  g51(.a(new_n81_), .b(x19), .c(new_n61_), .d(x04), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n53_), .O(z16));
  inv1   g53(.a(x18), .O(new_n95_));
  nand2  g54(.a(x12), .b(x05), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n51_), .c(x19), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n61_), .O(new_n98_));
  oai21  g57(.a(new_n75_), .b(new_n95_), .c(new_n98_), .O(z17));
  inv1   g58(.a(x06), .O(new_n100_));
  nand2  g59(.a(x19), .b(x12), .O(new_n101_));
  aoi21  g60(.a(new_n52_), .b(new_n100_), .c(new_n101_), .O(z18));
  inv1   g61(.a(x20), .O(new_n103_));
  nand2  g62(.a(x12), .b(x07), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n51_), .c(x19), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n61_), .O(new_n106_));
  oai21  g65(.a(new_n75_), .b(new_n103_), .c(new_n106_), .O(z19));
endmodule


