// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n63_,
    new_n65_, new_n67_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x03), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  nor2   g07(.a(new_n44_), .b(new_n48_), .O(z11));
  inv1   g08(.a(z11), .O(new_n50_));
  inv1   g09(.a(x15), .O(new_n51_));
  and2   g10(.a(x10), .b(x08), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(new_n51_), .c(x03), .O(new_n53_));
  nand2  g12(.a(x12), .b(new_n42_), .O(new_n54_));
  aoi21  g13(.a(new_n53_), .b(new_n50_), .c(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nor3   g15(.a(new_n46_), .b(x14), .c(new_n56_), .O(z03));
  inv1   g16(.a(new_n44_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n58_), .b(new_n60_), .O(z05));
  nand2  g20(.a(new_n51_), .b(x03), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n44_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g28(.a(x13), .O(new_n70_));
  nand2  g29(.a(x10), .b(x08), .O(new_n71_));
  nand2  g30(.a(x12), .b(x00), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(x03), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n51_), .O(new_n74_));
  nand2  g33(.a(new_n52_), .b(new_n51_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n70_), .c(new_n74_), .O(z12));
  nand3  g36(.a(x10), .b(x08), .c(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x14), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n58_), .O(z13));
  nand2  g41(.a(new_n71_), .b(x15), .O(new_n83_));
  nand4  g42(.a(new_n52_), .b(new_n51_), .c(x03), .d(x02), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n56_), .O(z14));
  nand2  g44(.a(x16), .b(new_n48_), .O(new_n86_));
  nand2  g45(.a(new_n58_), .b(x12), .O(new_n87_));
  aoi21  g46(.a(new_n86_), .b(new_n75_), .c(new_n87_), .O(z15));
  nand2  g47(.a(x17), .b(x15), .O(new_n89_));
  nand2  g48(.a(new_n51_), .b(x04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n52_), .O(new_n91_));
  inv1   g50(.a(x03), .O(new_n92_));
  aoi21  g51(.a(new_n71_), .b(new_n63_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n89_), .c(new_n56_), .O(z16));
  nand2  g54(.a(x18), .b(x15), .O(new_n96_));
  nand2  g55(.a(new_n51_), .b(x05), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n52_), .O(new_n98_));
  inv1   g57(.a(x18), .O(new_n99_));
  aoi21  g58(.a(new_n71_), .b(new_n99_), .c(new_n92_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n96_), .c(new_n56_), .O(z17));
  nand4  g61(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x03), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n51_), .O(new_n105_));
  oai21  g64(.a(new_n76_), .b(new_n60_), .c(new_n105_), .O(z18));
  nand4  g65(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x03), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n51_), .O(new_n109_));
  oai21  g68(.a(new_n76_), .b(new_n43_), .c(new_n109_), .O(z19));
endmodule


