// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x01), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x15), .O(new_n48_));
  oai21  g07(.a(new_n48_), .b(x14), .c(new_n46_), .O(z01));
  inv1   g08(.a(x14), .O(new_n50_));
  nor2   g09(.a(new_n44_), .b(new_n50_), .O(z11));
  inv1   g10(.a(z11), .O(new_n52_));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x15), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x01), .O(new_n55_));
  nand2  g14(.a(x12), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n55_), .b(new_n52_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nor3   g17(.a(new_n48_), .b(x14), .c(new_n58_), .O(z03));
  nand2  g18(.a(new_n46_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n46_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n46_), .b(new_n65_), .O(z08));
  aoi21  g25(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  inv1   g27(.a(x13), .O(new_n69_));
  inv1   g28(.a(x15), .O(new_n70_));
  nand2  g29(.a(x12), .b(x00), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n53_), .c(x01), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  oai21  g32(.a(new_n53_), .b(x15), .c(x12), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n69_), .c(new_n73_), .O(z12));
  nor2   g34(.a(new_n47_), .b(x14), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n74_), .c(new_n46_), .O(z13));
  nand2  g36(.a(new_n53_), .b(x15), .O(new_n78_));
  nand3  g37(.a(new_n54_), .b(x02), .c(x01), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n58_), .O(z14));
  nand4  g39(.a(x12), .b(x10), .c(x08), .d(x03), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x01), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n70_), .O(new_n83_));
  nand3  g42(.a(x16), .b(new_n50_), .c(x12), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n54_), .c(new_n83_), .O(z15));
  nand4  g44(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x01), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n70_), .O(new_n88_));
  oai21  g47(.a(new_n74_), .b(new_n63_), .c(new_n88_), .O(z16));
  inv1   g48(.a(x18), .O(new_n90_));
  nand4  g49(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x01), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n70_), .O(new_n93_));
  oai21  g52(.a(new_n74_), .b(new_n90_), .c(new_n93_), .O(z17));
  nand2  g53(.a(x19), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n70_), .b(x06), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n47_), .O(new_n97_));
  inv1   g56(.a(x01), .O(new_n98_));
  aoi21  g57(.a(new_n53_), .b(new_n61_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n95_), .c(new_n58_), .O(z18));
  nand4  g60(.a(x12), .b(x10), .c(x08), .d(x07), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x01), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n70_), .O(new_n104_));
  oai21  g63(.a(new_n74_), .b(new_n43_), .c(new_n104_), .O(z19));
  buf    g64(.a(x15), .O(z06));
endmodule


