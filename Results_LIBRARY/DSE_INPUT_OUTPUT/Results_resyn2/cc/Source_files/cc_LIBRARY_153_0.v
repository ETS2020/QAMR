// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_;
  nor2   g00(.a(x15), .b(x08), .O(new_n42_));
  nand2  g01(.a(x20), .b(x11), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(new_n42_), .O(new_n45_));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  oai21  g05(.a(new_n46_), .b(x14), .c(new_n45_), .O(z01));
  inv1   g06(.a(x15), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x14), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand2  g11(.a(x12), .b(new_n52_), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n45_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor3   g14(.a(new_n46_), .b(x14), .c(new_n55_), .O(z03));
  nor2   g15(.a(new_n42_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z08));
  and2   g22(.a(x09), .b(x08), .O(z09));
  nor2   g23(.a(z09), .b(new_n42_), .O(z10));
  inv1   g24(.a(x14), .O(new_n66_));
  nor2   g25(.a(new_n42_), .b(new_n66_), .O(z11));
  inv1   g26(.a(x00), .O(new_n68_));
  nand2  g27(.a(new_n50_), .b(new_n68_), .O(new_n69_));
  inv1   g28(.a(x13), .O(new_n70_));
  aoi21  g29(.a(new_n49_), .b(new_n70_), .c(new_n55_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n45_), .O(z12));
  inv1   g32(.a(x08), .O(new_n74_));
  aoi21  g33(.a(x10), .b(x01), .c(new_n74_), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(x15), .c(x14), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n46_), .c(new_n55_), .O(z13));
  nand3  g36(.a(x10), .b(x08), .c(x02), .O(new_n78_));
  nand2  g37(.a(new_n46_), .b(x12), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n48_), .c(new_n79_), .O(z14));
  nand3  g39(.a(new_n48_), .b(x10), .c(x03), .O(new_n81_));
  nand3  g40(.a(new_n49_), .b(x16), .c(new_n66_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n45_), .O(z15));
  nand3  g44(.a(new_n48_), .b(x10), .c(x04), .O(new_n86_));
  inv1   g45(.a(x10), .O(new_n87_));
  nand2  g46(.a(x17), .b(new_n87_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x08), .O(new_n90_));
  nand2  g49(.a(x17), .b(x15), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n55_), .O(z16));
  nand3  g51(.a(new_n48_), .b(x10), .c(x05), .O(new_n93_));
  nand2  g52(.a(x18), .b(new_n87_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x08), .O(new_n96_));
  nand2  g55(.a(x18), .b(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n55_), .O(z17));
  nand3  g57(.a(new_n48_), .b(x10), .c(x06), .O(new_n99_));
  nand2  g58(.a(x19), .b(new_n87_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x08), .O(new_n102_));
  nand2  g61(.a(x19), .b(x15), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n55_), .O(z18));
  nand3  g63(.a(new_n48_), .b(x10), .c(x07), .O(new_n105_));
  nand2  g64(.a(x20), .b(new_n87_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x08), .O(new_n108_));
  nand2  g67(.a(x20), .b(x15), .O(new_n109_));
  aoi21  g68(.a(new_n109_), .b(new_n108_), .c(new_n55_), .O(z19));
  buf    g69(.a(x15), .O(z06));
endmodule


