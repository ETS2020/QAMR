// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x08), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand4  g06(.a(x15), .b(new_n47_), .c(x10), .d(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n50_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor2   g14(.a(new_n48_), .b(new_n55_), .O(z03));
  nor2   g15(.a(new_n45_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nor2   g17(.a(new_n45_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n50_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(z10));
  inv1   g23(.a(z10), .O(z09));
  nor2   g24(.a(new_n45_), .b(new_n47_), .O(z11));
  inv1   g25(.a(x08), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n55_), .c(new_n44_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  inv1   g29(.a(x10), .O(new_n71_));
  nor2   g30(.a(x15), .b(new_n71_), .O(new_n72_));
  nand4  g31(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  oai21  g32(.a(new_n72_), .b(new_n68_), .c(new_n73_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n70_), .O(z12));
  aoi21  g35(.a(x14), .b(x12), .c(x15), .O(new_n77_));
  nand2  g36(.a(x10), .b(x01), .O(new_n78_));
  aoi22  g37(.a(new_n78_), .b(x14), .c(x15), .d(x10), .O(new_n79_));
  oai22  g38(.a(new_n79_), .b(new_n55_), .c(new_n77_), .d(x08), .O(z13));
  inv1   g39(.a(x02), .O(new_n81_));
  nand2  g40(.a(x15), .b(new_n71_), .O(new_n82_));
  oai21  g41(.a(new_n51_), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n50_), .O(z14));
  nand2  g44(.a(x16), .b(new_n47_), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n55_), .c(new_n44_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n67_), .O(new_n88_));
  nand4  g47(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  oai21  g48(.a(new_n86_), .b(new_n72_), .c(new_n89_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n88_), .O(z15));
  oai21  g51(.a(new_n60_), .b(new_n55_), .c(new_n44_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n67_), .O(new_n94_));
  nand4  g53(.a(new_n44_), .b(x10), .c(x08), .d(x04), .O(new_n95_));
  oai21  g54(.a(new_n72_), .b(new_n60_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n94_), .O(z16));
  inv1   g57(.a(x18), .O(new_n99_));
  oai21  g58(.a(new_n99_), .b(new_n55_), .c(new_n44_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n67_), .O(new_n101_));
  nand4  g60(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n102_));
  oai21  g61(.a(new_n72_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n101_), .O(z17));
  nand2  g64(.a(x10), .b(x06), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x08), .O(new_n107_));
  nand2  g66(.a(new_n58_), .b(new_n67_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(new_n44_), .O(new_n109_));
  nand2  g68(.a(new_n44_), .b(x10), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(x19), .c(x08), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n109_), .c(new_n55_), .O(z18));
  oai21  g71(.a(new_n43_), .b(new_n55_), .c(new_n44_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n67_), .O(new_n114_));
  nand4  g73(.a(new_n44_), .b(x10), .c(x08), .d(x07), .O(new_n115_));
  oai21  g74(.a(new_n72_), .b(new_n43_), .c(new_n115_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(x12), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n114_), .O(z19));
  buf    g77(.a(x15), .O(z06));
endmodule


