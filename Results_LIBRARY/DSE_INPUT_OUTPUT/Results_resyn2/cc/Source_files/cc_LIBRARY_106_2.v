// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n63_, new_n66_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_;
  inv1   g00(.a(x17), .O(new_n42_));
  nand2  g01(.a(new_n42_), .b(x15), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x15), .O(new_n49_));
  nor2   g08(.a(new_n42_), .b(new_n49_), .O(z06));
  nand2  g09(.a(z06), .b(new_n48_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n47_), .O(z01));
  nor2   g11(.a(new_n47_), .b(x15), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  oai21  g14(.a(new_n53_), .b(x14), .c(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n43_), .O(z02));
  nand3  g16(.a(x12), .b(x10), .c(x08), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n48_), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(x17), .c(new_n49_), .O(z03));
  nor2   g20(.a(new_n44_), .b(x18), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nand2  g22(.a(new_n43_), .b(new_n63_), .O(z05));
  and2   g23(.a(new_n43_), .b(x16), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(z09));
  nand3  g26(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nor2   g27(.a(new_n44_), .b(new_n48_), .O(z11));
  nand3  g28(.a(new_n49_), .b(x10), .c(x08), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n43_), .c(x13), .O(new_n71_));
  nand2  g30(.a(new_n53_), .b(x00), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n71_), .c(new_n54_), .O(z12));
  oai21  g32(.a(new_n59_), .b(new_n42_), .c(x15), .O(new_n74_));
  nand3  g33(.a(x10), .b(x08), .c(x01), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x14), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(z13));
  nand2  g36(.a(new_n53_), .b(x02), .O(new_n78_));
  nand3  g37(.a(new_n47_), .b(x17), .c(x15), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n54_), .O(z14));
  nand2  g39(.a(new_n53_), .b(x03), .O(new_n81_));
  nand4  g40(.a(new_n70_), .b(new_n43_), .c(x16), .d(new_n48_), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n54_), .O(z15));
  nand2  g42(.a(new_n47_), .b(x17), .O(new_n84_));
  nand3  g43(.a(x10), .b(x08), .c(x04), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n84_), .c(new_n49_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n43_), .O(z16));
  nand2  g47(.a(z06), .b(x18), .O(new_n89_));
  inv1   g48(.a(x05), .O(new_n90_));
  nand3  g49(.a(x10), .b(x08), .c(new_n90_), .O(new_n91_));
  inv1   g50(.a(x18), .O(new_n92_));
  nand2  g51(.a(new_n47_), .b(new_n92_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n91_), .c(new_n49_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n89_), .c(new_n54_), .O(z17));
  nand2  g54(.a(z06), .b(x19), .O(new_n96_));
  inv1   g55(.a(x06), .O(new_n97_));
  nand3  g56(.a(x10), .b(x08), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n47_), .b(new_n63_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(new_n49_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n54_), .O(z18));
  nand2  g60(.a(x20), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x17), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x15), .O(new_n104_));
  nand2  g63(.a(new_n47_), .b(x20), .O(new_n105_));
  nand4  g64(.a(new_n49_), .b(x10), .c(x08), .d(x07), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n104_), .O(z19));
  buf    g68(.a(x17), .O(z07));
endmodule


