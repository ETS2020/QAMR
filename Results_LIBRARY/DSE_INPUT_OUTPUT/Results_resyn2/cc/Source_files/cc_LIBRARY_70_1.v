// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n60_, new_n62_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(x18), .b(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(x14), .c(new_n45_), .O(z01));
  nand2  g07(.a(new_n45_), .b(x14), .O(new_n49_));
  inv1   g08(.a(x18), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g12(.a(x12), .b(new_n42_), .O(new_n54_));
  aoi21  g13(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nor3   g15(.a(new_n47_), .b(x14), .c(new_n56_), .O(z03));
  nand2  g16(.a(x18), .b(x15), .O(z04));
  and2   g17(.a(new_n45_), .b(x19), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n45_), .O(z09));
  inv1   g24(.a(z09), .O(z10));
  inv1   g25(.a(new_n49_), .O(z11));
  aoi21  g26(.a(x10), .b(x08), .c(x18), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(x15), .c(x13), .O(new_n69_));
  nand3  g28(.a(new_n52_), .b(new_n50_), .c(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n56_), .O(z12));
  inv1   g30(.a(x01), .O(new_n72_));
  oai21  g31(.a(new_n51_), .b(new_n72_), .c(new_n50_), .O(new_n73_));
  inv1   g32(.a(new_n47_), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(x14), .c(x12), .O(new_n75_));
  aoi21  g34(.a(new_n73_), .b(new_n44_), .c(new_n75_), .O(z13));
  and2   g35(.a(x10), .b(x08), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(x02), .c(x15), .O(new_n78_));
  nand2  g37(.a(new_n47_), .b(x12), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n78_), .c(new_n45_), .O(z14));
  nor2   g39(.a(new_n62_), .b(x14), .O(new_n81_));
  oai21  g40(.a(new_n68_), .b(x15), .c(new_n81_), .O(new_n82_));
  nand3  g41(.a(new_n52_), .b(new_n50_), .c(x03), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n56_), .O(z15));
  oai21  g43(.a(new_n68_), .b(x15), .c(x17), .O(new_n85_));
  nand3  g44(.a(new_n52_), .b(new_n50_), .c(x04), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n56_), .O(z16));
  nand4  g46(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n50_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n45_), .O(z17));
  nand2  g50(.a(x12), .b(x06), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n51_), .c(new_n50_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n44_), .O(new_n94_));
  nand2  g53(.a(x19), .b(x12), .O(new_n95_));
  oai21  g54(.a(new_n95_), .b(new_n52_), .c(new_n94_), .O(z18));
  nand2  g55(.a(x20), .b(x15), .O(new_n97_));
  nand2  g56(.a(new_n44_), .b(x07), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n77_), .O(new_n99_));
  aoi21  g58(.a(new_n51_), .b(new_n43_), .c(x18), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n97_), .c(new_n56_), .O(z19));
  buf    g61(.a(x15), .O(z06));
endmodule


