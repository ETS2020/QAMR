// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  inv1   g03(.a(x18), .O(z04));
  nand2  g04(.a(z04), .b(new_n44_), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(x14), .c(new_n46_), .O(z01));
  nand2  g09(.a(new_n46_), .b(x14), .O(new_n51_));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x15), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x18), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n51_), .c(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nand2  g16(.a(new_n46_), .b(new_n57_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(z01), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n46_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n46_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n47_), .b(new_n65_), .O(z08));
  nand3  g25(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  inv1   g27(.a(new_n51_), .O(z11));
  nand2  g28(.a(new_n52_), .b(x18), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n44_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x13), .O(new_n72_));
  nand3  g31(.a(new_n53_), .b(x18), .c(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n72_), .c(new_n57_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n52_), .b(new_n75_), .c(x18), .O(new_n76_));
  inv1   g35(.a(new_n49_), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(x14), .c(x12), .O(new_n78_));
  aoi21  g37(.a(new_n76_), .b(new_n44_), .c(new_n78_), .O(z13));
  inv1   g38(.a(new_n52_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(x02), .c(x15), .O(new_n81_));
  nand2  g40(.a(new_n49_), .b(x12), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n81_), .c(new_n46_), .O(z14));
  nor2   g42(.a(new_n65_), .b(x14), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n71_), .O(new_n85_));
  nand3  g44(.a(new_n53_), .b(x18), .c(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n57_), .O(z15));
  nand2  g46(.a(x12), .b(x04), .O(new_n88_));
  oai21  g47(.a(new_n88_), .b(new_n52_), .c(x18), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n44_), .O(new_n90_));
  nand2  g49(.a(x17), .b(x12), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n53_), .c(new_n90_), .O(z16));
  inv1   g51(.a(x05), .O(new_n93_));
  nand2  g52(.a(x18), .b(x12), .O(new_n94_));
  aoi21  g53(.a(new_n53_), .b(new_n93_), .c(new_n94_), .O(z17));
  nand2  g54(.a(x19), .b(x15), .O(new_n96_));
  nand2  g55(.a(new_n44_), .b(x06), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n80_), .O(new_n98_));
  aoi21  g57(.a(new_n52_), .b(new_n61_), .c(z04), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n57_), .O(z18));
  nand2  g60(.a(x20), .b(x15), .O(new_n102_));
  nand2  g61(.a(new_n44_), .b(x07), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n80_), .O(new_n104_));
  aoi21  g63(.a(new_n52_), .b(new_n43_), .c(z04), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n102_), .c(new_n57_), .O(z19));
  buf    g66(.a(x15), .O(z06));
endmodule


