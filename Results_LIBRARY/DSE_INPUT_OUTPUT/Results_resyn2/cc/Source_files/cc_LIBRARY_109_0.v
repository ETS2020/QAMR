// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n63_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n102_,
    new_n103_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  inv1   g01(.a(x15), .O(new_n43_));
  inv1   g02(.a(x20), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(x14), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(z11));
  nor2   g06(.a(x20), .b(x15), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(x15), .c(new_n48_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(z11), .O(z01));
  nor2   g11(.a(new_n49_), .b(x15), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  oai21  g14(.a(new_n53_), .b(x14), .c(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n45_), .O(z02));
  oai21  g16(.a(new_n49_), .b(new_n43_), .c(new_n45_), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n54_), .O(new_n59_));
  nand3  g18(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z03));
  nand2  g20(.a(new_n45_), .b(x18), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nor2   g22(.a(new_n48_), .b(new_n63_), .O(z05));
  nor2   g23(.a(new_n44_), .b(x15), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z06));
  inv1   g25(.a(x17), .O(new_n67_));
  nand2  g26(.a(new_n45_), .b(new_n67_), .O(z07));
  inv1   g27(.a(x16), .O(new_n69_));
  nand2  g28(.a(new_n45_), .b(new_n69_), .O(z08));
  nand3  g29(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g30(.a(z10), .O(z09));
  nand2  g31(.a(x12), .b(x00), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n49_), .c(x20), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n43_), .O(new_n75_));
  inv1   g34(.a(new_n53_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x13), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n49_), .b(new_n79_), .c(x14), .O(new_n80_));
  aoi22  g39(.a(new_n80_), .b(new_n51_), .c(new_n45_), .d(new_n54_), .O(z13));
  nand2  g40(.a(new_n65_), .b(x02), .O(new_n82_));
  oai21  g41(.a(new_n50_), .b(x15), .c(x12), .O(new_n83_));
  aoi21  g42(.a(new_n82_), .b(new_n50_), .c(new_n83_), .O(z14));
  nand2  g43(.a(new_n49_), .b(x20), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n43_), .O(new_n86_));
  nor2   g45(.a(new_n69_), .b(x14), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g47(.a(new_n65_), .b(new_n50_), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n54_), .O(z15));
  nand2  g49(.a(new_n86_), .b(x17), .O(new_n91_));
  nand3  g50(.a(new_n65_), .b(new_n50_), .c(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n54_), .O(z16));
  nand2  g52(.a(x12), .b(x05), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n49_), .c(x20), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n43_), .O(new_n96_));
  nand3  g55(.a(new_n76_), .b(x18), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(z17));
  nand2  g57(.a(new_n86_), .b(x19), .O(new_n99_));
  nand3  g58(.a(new_n65_), .b(new_n50_), .c(x06), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n54_), .O(z18));
  inv1   g60(.a(x07), .O(new_n102_));
  nand2  g61(.a(x20), .b(x12), .O(new_n103_));
  aoi21  g62(.a(new_n53_), .b(new_n102_), .c(new_n103_), .O(z19));
endmodule


