// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n57_, new_n59_, new_n61_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_;
  nor2   g00(.a(x15), .b(x12), .O(new_n42_));
  nand2  g01(.a(x20), .b(x11), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand3  g03(.a(x15), .b(x10), .c(x08), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(x14), .O(z01));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x10), .c(x08), .O(new_n49_));
  inv1   g08(.a(x11), .O(new_n50_));
  nand2  g09(.a(x12), .b(new_n50_), .O(new_n51_));
  aoi21  g10(.a(new_n49_), .b(new_n47_), .c(new_n51_), .O(z02));
  inv1   g11(.a(new_n42_), .O(new_n53_));
  nand2  g12(.a(z01), .b(x12), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n53_), .O(z03));
  nand2  g14(.a(new_n53_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nor2   g16(.a(new_n42_), .b(new_n57_), .O(z05));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n53_), .b(new_n59_), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nand2  g20(.a(new_n53_), .b(new_n61_), .O(z08));
  aoi21  g21(.a(x09), .b(x08), .c(new_n42_), .O(z10));
  inv1   g22(.a(z10), .O(z09));
  nor2   g23(.a(new_n42_), .b(new_n47_), .O(z11));
  inv1   g24(.a(x12), .O(new_n66_));
  and2   g25(.a(x10), .b(x08), .O(new_n67_));
  aoi21  g26(.a(new_n67_), .b(x00), .c(new_n66_), .O(new_n68_));
  nand3  g27(.a(new_n49_), .b(x13), .c(x12), .O(new_n69_));
  oai21  g28(.a(new_n68_), .b(x15), .c(new_n69_), .O(z12));
  nand3  g29(.a(x10), .b(x08), .c(x01), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x14), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n45_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n53_), .O(z13));
  inv1   g34(.a(new_n45_), .O(new_n76_));
  nand2  g35(.a(new_n67_), .b(x02), .O(new_n77_));
  xor2a  g36(.a(x15), .b(x12), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(z14));
  nand3  g38(.a(new_n49_), .b(x16), .c(new_n47_), .O(new_n80_));
  nand3  g39(.a(new_n67_), .b(new_n48_), .c(x03), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n66_), .O(z15));
  aoi21  g41(.a(new_n67_), .b(x04), .c(new_n66_), .O(new_n83_));
  nand3  g42(.a(new_n49_), .b(x17), .c(x12), .O(new_n84_));
  oai21  g43(.a(new_n83_), .b(x15), .c(new_n84_), .O(z16));
  inv1   g44(.a(x05), .O(new_n86_));
  nand3  g45(.a(new_n67_), .b(new_n48_), .c(new_n86_), .O(new_n87_));
  inv1   g46(.a(x18), .O(new_n88_));
  nand2  g47(.a(new_n49_), .b(new_n88_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n87_), .c(x12), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z17));
  inv1   g50(.a(x06), .O(new_n92_));
  nand3  g51(.a(new_n67_), .b(new_n48_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n49_), .b(new_n57_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z18));
  aoi21  g55(.a(new_n67_), .b(x07), .c(new_n66_), .O(new_n97_));
  nand3  g56(.a(new_n49_), .b(x20), .c(x12), .O(new_n98_));
  oai21  g57(.a(new_n97_), .b(x15), .c(new_n98_), .O(z19));
  buf    g58(.a(x15), .O(z06));
endmodule


