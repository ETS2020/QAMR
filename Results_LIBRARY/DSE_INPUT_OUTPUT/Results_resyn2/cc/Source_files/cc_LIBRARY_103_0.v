// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n60_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x19), .b(x06), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n45_), .b(new_n47_), .O(z11));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(z11), .O(z01));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n45_), .O(z02));
  nand2  g14(.a(new_n47_), .b(x12), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n49_), .c(new_n45_), .O(z03));
  nor2   g16(.a(new_n44_), .b(x18), .O(z04));
  nand2  g17(.a(new_n45_), .b(new_n51_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z08));
  nand3  g22(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(z10), .O(z09));
  inv1   g24(.a(x00), .O(new_n66_));
  nand4  g25(.a(new_n51_), .b(x10), .c(x08), .d(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  nand2  g27(.a(new_n52_), .b(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n67_), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n45_), .O(z12));
  nand3  g30(.a(x10), .b(x08), .c(x01), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x14), .O(new_n73_));
  nand2  g32(.a(new_n45_), .b(x12), .O(new_n74_));
  aoi21  g33(.a(new_n73_), .b(new_n49_), .c(new_n74_), .O(z13));
  nand3  g34(.a(x10), .b(x08), .c(x02), .O(new_n76_));
  nand3  g35(.a(new_n49_), .b(new_n45_), .c(x12), .O(new_n77_));
  aoi21  g36(.a(new_n76_), .b(new_n51_), .c(new_n77_), .O(z14));
  inv1   g37(.a(x03), .O(new_n79_));
  nand4  g38(.a(new_n51_), .b(x10), .c(x08), .d(new_n79_), .O(new_n80_));
  nand2  g39(.a(x16), .b(new_n47_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n52_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n80_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n45_), .O(z15));
  inv1   g43(.a(x04), .O(new_n85_));
  nand4  g44(.a(new_n51_), .b(x10), .c(x08), .d(new_n85_), .O(new_n86_));
  nand2  g45(.a(new_n52_), .b(new_n60_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n86_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n45_), .O(z16));
  inv1   g48(.a(x05), .O(new_n90_));
  nand4  g49(.a(new_n51_), .b(x10), .c(x08), .d(new_n90_), .O(new_n91_));
  inv1   g50(.a(x18), .O(new_n92_));
  nand2  g51(.a(new_n52_), .b(new_n92_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n91_), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n45_), .O(z17));
  inv1   g54(.a(x06), .O(new_n96_));
  nand4  g55(.a(new_n51_), .b(x10), .c(x08), .d(new_n96_), .O(new_n97_));
  inv1   g56(.a(x19), .O(new_n98_));
  nand2  g57(.a(new_n52_), .b(new_n98_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n97_), .c(x12), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z18));
  inv1   g60(.a(x07), .O(new_n102_));
  nand4  g61(.a(new_n51_), .b(x10), .c(x08), .d(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n52_), .b(new_n43_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(new_n103_), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n45_), .O(z19));
  buf    g65(.a(x19), .O(z05));
endmodule


