// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n59_, new_n61_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_;
  nand2  g00(.a(x15), .b(x12), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(x12), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(x15), .b(new_n48_), .c(new_n47_), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n46_), .O(z01));
  aoi21  g09(.a(x10), .b(x08), .c(x14), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  nor3   g12(.a(new_n53_), .b(new_n51_), .c(x11), .O(z02));
  nor2   g13(.a(new_n43_), .b(x18), .O(z04));
  and2   g14(.a(new_n42_), .b(x19), .O(z05));
  and2   g15(.a(new_n42_), .b(x17), .O(z07));
  inv1   g16(.a(x16), .O(new_n59_));
  nand2  g17(.a(new_n42_), .b(new_n59_), .O(z08));
  nand2  g18(.a(x09), .b(x08), .O(new_n61_));
  nor2   g19(.a(new_n61_), .b(new_n43_), .O(z09));
  and2   g20(.a(new_n61_), .b(new_n42_), .O(z10));
  nand2  g21(.a(new_n42_), .b(new_n48_), .O(z11));
  nor2   g22(.a(x15), .b(new_n47_), .O(new_n65_));
  inv1   g23(.a(x00), .O(new_n66_));
  nand3  g24(.a(x10), .b(x08), .c(new_n66_), .O(new_n67_));
  inv1   g25(.a(x13), .O(new_n68_));
  nand2  g26(.a(new_n46_), .b(new_n68_), .O(new_n69_));
  nand3  g27(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(new_n70_));
  inv1   g28(.a(new_n70_), .O(z12));
  inv1   g29(.a(new_n46_), .O(new_n72_));
  nand3  g30(.a(new_n52_), .b(x14), .c(x12), .O(new_n73_));
  aoi21  g31(.a(new_n72_), .b(x01), .c(new_n73_), .O(z13));
  nand3  g32(.a(x10), .b(x08), .c(x02), .O(new_n75_));
  aoi21  g33(.a(new_n75_), .b(new_n52_), .c(new_n47_), .O(z14));
  nand3  g34(.a(x10), .b(x08), .c(x03), .O(new_n77_));
  nand2  g35(.a(new_n51_), .b(x16), .O(new_n78_));
  aoi21  g36(.a(new_n78_), .b(new_n77_), .c(new_n53_), .O(z15));
  nand3  g37(.a(x10), .b(x08), .c(x04), .O(new_n80_));
  aoi21  g38(.a(new_n46_), .b(x17), .c(x15), .O(new_n81_));
  aoi21  g39(.a(new_n81_), .b(new_n80_), .c(new_n47_), .O(z16));
  nand3  g40(.a(x10), .b(x08), .c(x05), .O(new_n83_));
  aoi21  g41(.a(new_n46_), .b(x18), .c(x15), .O(new_n84_));
  aoi21  g42(.a(new_n84_), .b(new_n83_), .c(new_n47_), .O(z17));
  nand3  g43(.a(x10), .b(x08), .c(x06), .O(new_n86_));
  aoi21  g44(.a(new_n46_), .b(x19), .c(x15), .O(new_n87_));
  aoi21  g45(.a(new_n87_), .b(new_n86_), .c(new_n47_), .O(z18));
  inv1   g46(.a(x07), .O(new_n89_));
  nand3  g47(.a(x10), .b(x08), .c(new_n89_), .O(new_n90_));
  inv1   g48(.a(x20), .O(new_n91_));
  nand2  g49(.a(new_n46_), .b(new_n91_), .O(new_n92_));
  nand3  g50(.a(new_n92_), .b(new_n90_), .c(new_n65_), .O(new_n93_));
  inv1   g51(.a(new_n93_), .O(z19));
  zero   g52(.O(z03));
  buf    g53(.a(x15), .O(z06));
endmodule


