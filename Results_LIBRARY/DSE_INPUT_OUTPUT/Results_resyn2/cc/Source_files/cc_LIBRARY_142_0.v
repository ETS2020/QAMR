// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n59_, new_n62_,
    new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x08), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(x14), .c(new_n45_), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  nand2  g08(.a(new_n44_), .b(x10), .O(new_n50_));
  inv1   g09(.a(x11), .O(new_n51_));
  nand3  g10(.a(new_n45_), .b(x12), .c(new_n51_), .O(new_n52_));
  aoi21  g11(.a(new_n50_), .b(new_n49_), .c(new_n52_), .O(z02));
  inv1   g12(.a(x12), .O(new_n54_));
  nand2  g13(.a(new_n45_), .b(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(z01), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z03));
  nand2  g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n45_), .b(new_n59_), .O(z05));
  nand2  g19(.a(new_n44_), .b(x08), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(x15), .b(x08), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n62_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n63_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  nor2   g27(.a(new_n63_), .b(new_n49_), .O(z11));
  inv1   g28(.a(x10), .O(new_n70_));
  nand2  g29(.a(x13), .b(new_n70_), .O(new_n71_));
  nand3  g30(.a(new_n44_), .b(x10), .c(x00), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x08), .O(new_n74_));
  nand2  g33(.a(x15), .b(x13), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n74_), .c(new_n54_), .O(z12));
  aoi21  g35(.a(x14), .b(x12), .c(new_n44_), .O(new_n77_));
  inv1   g36(.a(new_n47_), .O(new_n78_));
  nand3  g37(.a(new_n44_), .b(x10), .c(x01), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(x14), .c(new_n78_), .O(new_n80_));
  oai22  g39(.a(new_n80_), .b(new_n54_), .c(new_n77_), .d(x08), .O(z13));
  nand3  g40(.a(x10), .b(x08), .c(x02), .O(new_n82_));
  nand2  g41(.a(new_n47_), .b(x12), .O(new_n83_));
  aoi21  g42(.a(new_n82_), .b(new_n44_), .c(new_n83_), .O(z14));
  inv1   g43(.a(new_n55_), .O(new_n85_));
  nand3  g44(.a(new_n50_), .b(x16), .c(new_n49_), .O(new_n86_));
  nor2   g45(.a(x15), .b(new_n70_), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(x03), .c(new_n63_), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(z15));
  nand3  g48(.a(new_n44_), .b(x10), .c(x04), .O(new_n90_));
  nand2  g49(.a(x17), .b(new_n70_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x08), .O(new_n93_));
  nand2  g52(.a(x17), .b(x15), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n54_), .O(z16));
  nand3  g54(.a(new_n44_), .b(x10), .c(x05), .O(new_n96_));
  nand2  g55(.a(x18), .b(new_n70_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x08), .O(new_n99_));
  nand2  g58(.a(x18), .b(x15), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n54_), .O(z17));
  nand3  g60(.a(x12), .b(x10), .c(x06), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x08), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n44_), .O(new_n104_));
  nand3  g63(.a(new_n50_), .b(x19), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z18));
  nand3  g65(.a(new_n44_), .b(x10), .c(x07), .O(new_n107_));
  nand2  g66(.a(x20), .b(new_n70_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x08), .O(new_n110_));
  nand2  g69(.a(x20), .b(x15), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n54_), .O(z19));
endmodule


