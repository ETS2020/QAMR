// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n62_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x18), .O(z04));
  nand2  g03(.a(z04), .b(x15), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nor3   g05(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g06(.a(x15), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  and2   g08(.a(x10), .b(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(x18), .c(new_n48_), .O(z01));
  nand2  g11(.a(new_n45_), .b(x14), .O(new_n53_));
  nand3  g12(.a(new_n48_), .b(x10), .c(x08), .O(new_n54_));
  nand2  g13(.a(x12), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n54_), .b(new_n53_), .c(new_n55_), .O(z02));
  nand3  g15(.a(new_n50_), .b(new_n49_), .c(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x18), .c(new_n48_), .O(z03));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n45_), .b(new_n59_), .O(z05));
  nor2   g19(.a(z04), .b(new_n48_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z07));
  and2   g22(.a(new_n45_), .b(x16), .O(z08));
  nand3  g23(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g24(.a(z10), .O(z09));
  inv1   g25(.a(new_n53_), .O(z11));
  inv1   g26(.a(x12), .O(new_n68_));
  nand3  g27(.a(new_n54_), .b(new_n45_), .c(x13), .O(new_n69_));
  nand3  g28(.a(new_n50_), .b(new_n48_), .c(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z12));
  nand2  g30(.a(x10), .b(x08), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n48_), .c(new_n49_), .O(new_n73_));
  nand3  g32(.a(new_n50_), .b(new_n48_), .c(x01), .O(new_n74_));
  nand4  g33(.a(new_n74_), .b(new_n73_), .c(new_n45_), .d(x12), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z13));
  nand2  g35(.a(new_n50_), .b(x15), .O(new_n77_));
  inv1   g36(.a(x02), .O(new_n78_));
  oai21  g37(.a(new_n72_), .b(new_n78_), .c(new_n48_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n77_), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n45_), .O(z14));
  nand3  g40(.a(new_n45_), .b(x16), .c(new_n49_), .O(new_n82_));
  oai21  g41(.a(new_n54_), .b(x03), .c(x12), .O(new_n83_));
  aoi21  g42(.a(new_n82_), .b(new_n54_), .c(new_n83_), .O(z15));
  oai21  g43(.a(new_n62_), .b(new_n68_), .c(x18), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x15), .O(new_n86_));
  nand2  g45(.a(new_n72_), .b(x17), .O(new_n87_));
  nand4  g46(.a(new_n48_), .b(x10), .c(x08), .d(x04), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n86_), .O(z16));
  inv1   g50(.a(x05), .O(new_n92_));
  nand3  g51(.a(new_n50_), .b(new_n48_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n54_), .b(z04), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z17));
  nand2  g55(.a(z06), .b(x19), .O(new_n97_));
  inv1   g56(.a(x06), .O(new_n98_));
  nand3  g57(.a(x10), .b(x08), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n72_), .b(new_n59_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n48_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n97_), .c(new_n68_), .O(z18));
  nand2  g61(.a(z06), .b(x20), .O(new_n103_));
  inv1   g62(.a(x07), .O(new_n104_));
  nand3  g63(.a(x10), .b(x08), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n72_), .b(new_n43_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n105_), .c(new_n48_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n103_), .c(new_n68_), .O(z19));
endmodule


