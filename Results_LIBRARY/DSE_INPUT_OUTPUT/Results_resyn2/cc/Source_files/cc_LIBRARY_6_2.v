// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x18), .O(z04));
  nand2  g02(.a(z04), .b(x08), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand3  g04(.a(x18), .b(x10), .c(x08), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x15), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x14), .O(z01));
  inv1   g08(.a(x14), .O(new_n50_));
  inv1   g09(.a(x10), .O(new_n51_));
  nor2   g10(.a(x15), .b(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x08), .O(new_n53_));
  inv1   g12(.a(x11), .O(new_n54_));
  nand3  g13(.a(new_n44_), .b(x12), .c(new_n54_), .O(new_n55_));
  aoi21  g14(.a(new_n53_), .b(new_n50_), .c(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nor3   g16(.a(new_n48_), .b(x14), .c(new_n57_), .O(z03));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n44_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  nand2  g20(.a(new_n44_), .b(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  inv1   g26(.a(x09), .O(new_n68_));
  aoi21  g27(.a(x18), .b(new_n68_), .c(new_n67_), .O(z09));
  nand3  g28(.a(x18), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n44_), .b(new_n50_), .O(z11));
  aoi21  g30(.a(new_n61_), .b(x10), .c(z04), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n67_), .c(x13), .O(new_n73_));
  nor2   g32(.a(new_n46_), .b(x15), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x00), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n57_), .O(z12));
  aoi21  g35(.a(x10), .b(x01), .c(z04), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n67_), .c(x14), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n48_), .c(new_n57_), .O(z13));
  oai21  g38(.a(z04), .b(x10), .c(x08), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x15), .O(new_n81_));
  nand2  g40(.a(new_n74_), .b(x02), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n57_), .O(z14));
  nor2   g42(.a(new_n65_), .b(x14), .O(new_n84_));
  oai21  g43(.a(new_n72_), .b(new_n67_), .c(new_n84_), .O(new_n85_));
  nand2  g44(.a(new_n74_), .b(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n57_), .O(z15));
  oai21  g46(.a(new_n72_), .b(new_n67_), .c(x17), .O(new_n88_));
  nand2  g47(.a(new_n74_), .b(x04), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n57_), .O(z16));
  inv1   g49(.a(x05), .O(new_n91_));
  nand3  g50(.a(new_n52_), .b(x08), .c(new_n91_), .O(new_n92_));
  nor2   g51(.a(z04), .b(new_n57_), .O(new_n93_));
  and2   g52(.a(new_n93_), .b(new_n92_), .O(z17));
  nand4  g53(.a(new_n61_), .b(x12), .c(x10), .d(x06), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x18), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x08), .O(new_n97_));
  nand3  g56(.a(new_n53_), .b(x19), .c(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(z18));
  nand4  g58(.a(new_n61_), .b(x12), .c(x10), .d(x07), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x18), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x08), .O(new_n102_));
  nand3  g61(.a(new_n53_), .b(x20), .c(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(z19));
endmodule


