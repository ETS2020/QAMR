// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_;
  inv1   g00(.a(x15), .O(new_n42_));
  nor2   g01(.a(x16), .b(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(x16), .c(new_n42_), .O(z01));
  inv1   g09(.a(new_n43_), .O(new_n51_));
  nor2   g10(.a(new_n47_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n51_), .O(z02));
  nand3  g15(.a(new_n48_), .b(new_n46_), .c(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x16), .c(new_n42_), .O(z03));
  nor2   g17(.a(new_n43_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n43_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n51_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n42_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n51_), .O(z09));
  nand3  g26(.a(new_n51_), .b(x09), .c(x08), .O(z10));
  nor2   g27(.a(new_n43_), .b(new_n46_), .O(z11));
  nand2  g28(.a(new_n52_), .b(x00), .O(new_n70_));
  inv1   g29(.a(new_n52_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n51_), .c(x13), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n70_), .c(new_n53_), .O(z12));
  oai21  g32(.a(new_n47_), .b(new_n42_), .c(new_n46_), .O(new_n74_));
  nand2  g33(.a(new_n52_), .b(x01), .O(new_n75_));
  nand4  g34(.a(new_n75_), .b(new_n74_), .c(new_n51_), .d(x12), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z13));
  nand2  g36(.a(new_n52_), .b(x02), .O(new_n78_));
  nand3  g37(.a(new_n47_), .b(x16), .c(x15), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n53_), .O(z14));
  nand2  g39(.a(new_n47_), .b(x16), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n42_), .c(x14), .O(new_n82_));
  nand4  g41(.a(new_n42_), .b(x10), .c(x08), .d(x03), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n82_), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n51_), .O(z15));
  oai21  g45(.a(new_n62_), .b(new_n53_), .c(x16), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  aoi22  g47(.a(new_n52_), .b(x04), .c(new_n47_), .d(x17), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n53_), .c(new_n88_), .O(z16));
  nand2  g49(.a(x18), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x16), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x15), .O(new_n93_));
  aoi22  g52(.a(new_n52_), .b(x05), .c(new_n47_), .d(x18), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n53_), .c(new_n93_), .O(z17));
  aoi21  g54(.a(new_n47_), .b(new_n60_), .c(x15), .O(new_n96_));
  oai21  g55(.a(new_n47_), .b(x06), .c(new_n96_), .O(new_n97_));
  nand3  g56(.a(x19), .b(x16), .c(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n53_), .O(z18));
  nand2  g58(.a(x20), .b(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x16), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x15), .O(new_n102_));
  aoi22  g61(.a(new_n52_), .b(x07), .c(new_n47_), .d(x20), .O(new_n103_));
  oai21  g62(.a(new_n103_), .b(new_n53_), .c(new_n102_), .O(z19));
  buf    g63(.a(x15), .O(z06));
endmodule


