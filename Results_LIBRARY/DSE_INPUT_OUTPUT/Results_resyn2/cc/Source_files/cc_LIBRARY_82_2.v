// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x15), .O(new_n42_));
  nor2   g01(.a(x18), .b(new_n42_), .O(new_n43_));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x18), .c(new_n42_), .O(z01));
  inv1   g08(.a(new_n43_), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  nand2  g15(.a(new_n46_), .b(x12), .O(new_n57_));
  inv1   g16(.a(x18), .O(z04));
  nor2   g17(.a(z04), .b(new_n42_), .O(z06));
  nand2  g18(.a(z06), .b(new_n47_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n57_), .O(z03));
  inv1   g20(.a(x19), .O(new_n62_));
  nor2   g21(.a(new_n43_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n50_), .O(z09));
  nand3  g28(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n50_), .b(new_n46_), .O(z11));
  nand2  g30(.a(x13), .b(x12), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x18), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x15), .O(new_n74_));
  nand2  g33(.a(new_n51_), .b(x13), .O(new_n75_));
  nand4  g34(.a(new_n42_), .b(x10), .c(x08), .d(x00), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n74_), .O(z12));
  inv1   g38(.a(x01), .O(new_n80_));
  nand2  g39(.a(new_n42_), .b(new_n80_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n47_), .O(new_n82_));
  aoi21  g41(.a(z04), .b(x15), .c(new_n46_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n60_), .c(new_n53_), .O(z13));
  nand2  g44(.a(new_n52_), .b(x02), .O(new_n86_));
  nand3  g45(.a(new_n51_), .b(x18), .c(x15), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(new_n53_), .O(z14));
  oai21  g47(.a(new_n57_), .b(new_n66_), .c(x18), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand4  g49(.a(new_n42_), .b(x10), .c(x08), .d(x03), .O(new_n91_));
  nand3  g50(.a(new_n51_), .b(x16), .c(new_n46_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  nand2  g54(.a(z06), .b(x17), .O(new_n96_));
  inv1   g55(.a(x04), .O(new_n97_));
  nand3  g56(.a(x10), .b(x08), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n51_), .b(new_n64_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(new_n42_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n53_), .O(z16));
  nand2  g60(.a(new_n51_), .b(x18), .O(new_n102_));
  nand3  g61(.a(x10), .b(x08), .c(x05), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(new_n102_), .c(new_n42_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n50_), .O(z17));
  oai21  g65(.a(new_n62_), .b(new_n53_), .c(x18), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x15), .O(new_n108_));
  nand2  g67(.a(new_n51_), .b(x19), .O(new_n109_));
  nand4  g68(.a(new_n42_), .b(x10), .c(x08), .d(x06), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n108_), .O(z18));
  nand2  g72(.a(z06), .b(x20), .O(new_n114_));
  inv1   g73(.a(x07), .O(new_n115_));
  nand3  g74(.a(x10), .b(x08), .c(new_n115_), .O(new_n116_));
  inv1   g75(.a(x20), .O(new_n117_));
  nand2  g76(.a(new_n51_), .b(new_n117_), .O(new_n118_));
  nand3  g77(.a(new_n118_), .b(new_n116_), .c(new_n42_), .O(new_n119_));
  aoi21  g78(.a(new_n119_), .b(new_n114_), .c(new_n53_), .O(z19));
endmodule


