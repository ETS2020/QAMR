// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n59_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x07), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n44_), .O(z00));
  nand2  g05(.a(new_n44_), .b(x15), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  and2   g07(.a(x10), .b(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n47_), .O(z01));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x15), .O(new_n53_));
  inv1   g12(.a(x12), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x11), .O(new_n55_));
  oai21  g14(.a(new_n53_), .b(x14), .c(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n44_), .O(z02));
  nor3   g16(.a(new_n50_), .b(new_n47_), .c(new_n54_), .O(z03));
  nor2   g17(.a(x20), .b(x07), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n59_), .b(new_n61_), .O(z05));
  inv1   g21(.a(new_n47_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n44_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n44_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n59_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n59_), .O(z10));
  nor2   g29(.a(new_n59_), .b(new_n48_), .O(z11));
  inv1   g30(.a(x00), .O(new_n72_));
  nand2  g31(.a(new_n53_), .b(new_n72_), .O(new_n73_));
  nor2   g32(.a(new_n59_), .b(new_n54_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  inv1   g34(.a(x15), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n74_), .c(new_n73_), .O(new_n79_));
  inv1   g38(.a(new_n79_), .O(z12));
  inv1   g39(.a(new_n74_), .O(new_n81_));
  inv1   g40(.a(x01), .O(new_n82_));
  oai21  g41(.a(new_n52_), .b(new_n82_), .c(x14), .O(new_n83_));
  nand2  g42(.a(new_n49_), .b(x15), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(z13));
  aoi21  g44(.a(new_n49_), .b(x02), .c(x15), .O(new_n86_));
  nand2  g45(.a(new_n84_), .b(x12), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n86_), .c(new_n44_), .O(z14));
  nand3  g47(.a(new_n77_), .b(x16), .c(new_n48_), .O(new_n89_));
  nand2  g48(.a(new_n53_), .b(x03), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n81_), .O(z15));
  inv1   g50(.a(x04), .O(new_n92_));
  nand2  g51(.a(new_n53_), .b(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n77_), .b(new_n64_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n44_), .O(z16));
  inv1   g55(.a(x05), .O(new_n97_));
  nand2  g56(.a(new_n53_), .b(new_n97_), .O(new_n98_));
  inv1   g57(.a(x18), .O(new_n99_));
  nand2  g58(.a(new_n77_), .b(new_n99_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n98_), .c(new_n74_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand2  g62(.a(new_n53_), .b(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n77_), .b(new_n61_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(new_n74_), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z18));
  nand4  g66(.a(new_n76_), .b(x10), .c(x08), .d(new_n42_), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(x12), .c(new_n43_), .O(new_n109_));
  oai21  g68(.a(new_n52_), .b(x15), .c(new_n43_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(x07), .c(new_n109_), .O(z19));
endmodule


