// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n59_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x05), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand2  g03(.a(new_n43_), .b(x15), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand3  g05(.a(new_n46_), .b(x10), .c(x08), .O(new_n47_));
  nor2   g06(.a(new_n47_), .b(new_n45_), .O(z01));
  inv1   g07(.a(x15), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x10), .c(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n43_), .O(z02));
  nor3   g14(.a(new_n47_), .b(new_n45_), .c(new_n52_), .O(z03));
  inv1   g15(.a(x05), .O(new_n57_));
  nand2  g16(.a(x18), .b(new_n57_), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  inv1   g18(.a(new_n43_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n59_), .O(z05));
  inv1   g20(.a(new_n45_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n60_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n43_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n60_), .O(z09));
  and2   g27(.a(new_n67_), .b(new_n43_), .O(z10));
  nor2   g28(.a(new_n60_), .b(new_n46_), .O(z11));
  inv1   g29(.a(x00), .O(new_n71_));
  nand4  g30(.a(new_n49_), .b(x10), .c(x08), .d(new_n71_), .O(new_n72_));
  nand2  g31(.a(new_n43_), .b(x12), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  nand2  g34(.a(new_n50_), .b(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n74_), .c(new_n72_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  nand2  g38(.a(x10), .b(x08), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(x14), .O(new_n81_));
  nand3  g40(.a(x15), .b(x10), .c(x08), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n73_), .O(z13));
  inv1   g42(.a(x02), .O(new_n84_));
  oai21  g43(.a(new_n80_), .b(new_n84_), .c(new_n49_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n82_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n43_), .O(z14));
  inv1   g46(.a(x03), .O(new_n88_));
  nand4  g47(.a(new_n49_), .b(x10), .c(x08), .d(new_n88_), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n46_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n89_), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n43_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand4  g53(.a(new_n49_), .b(x10), .c(x08), .d(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n50_), .b(new_n63_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(new_n95_), .c(new_n74_), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z16));
  inv1   g57(.a(x18), .O(new_n99_));
  nand4  g58(.a(new_n49_), .b(x12), .c(x10), .d(x08), .O(new_n100_));
  nand2  g59(.a(new_n50_), .b(x12), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n57_), .c(new_n100_), .d(new_n99_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand4  g62(.a(new_n49_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n50_), .b(new_n59_), .O(new_n105_));
  nand3  g64(.a(new_n105_), .b(new_n104_), .c(new_n74_), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z18));
  inv1   g66(.a(x07), .O(new_n108_));
  nand4  g67(.a(new_n49_), .b(x10), .c(x08), .d(new_n108_), .O(new_n109_));
  inv1   g68(.a(x20), .O(new_n110_));
  nand2  g69(.a(new_n50_), .b(new_n110_), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(new_n109_), .c(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n43_), .O(z19));
endmodule


