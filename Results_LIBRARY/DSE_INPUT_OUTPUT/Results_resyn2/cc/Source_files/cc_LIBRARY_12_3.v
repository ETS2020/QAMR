// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n59_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x18), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n46_), .O(z01));
  nand2  g08(.a(x10), .b(x08), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x15), .O(new_n51_));
  inv1   g10(.a(x12), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x11), .O(new_n53_));
  oai21  g12(.a(new_n51_), .b(x14), .c(new_n53_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n43_), .O(z02));
  nor3   g14(.a(new_n48_), .b(new_n46_), .c(new_n52_), .O(z03));
  inv1   g15(.a(x15), .O(new_n57_));
  nand2  g16(.a(x18), .b(new_n57_), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n43_), .b(new_n59_), .O(z05));
  nor2   g19(.a(x18), .b(new_n57_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(z09));
  inv1   g26(.a(z09), .O(z10));
  nand2  g27(.a(new_n43_), .b(new_n47_), .O(z11));
  nand3  g28(.a(new_n57_), .b(x10), .c(x08), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n43_), .c(x13), .O(new_n71_));
  nand2  g30(.a(new_n51_), .b(x00), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n71_), .c(new_n52_), .O(z12));
  oai21  g32(.a(new_n50_), .b(new_n52_), .c(new_n45_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x15), .O(new_n75_));
  nand3  g34(.a(x10), .b(x08), .c(x01), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x14), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(z13));
  nand2  g37(.a(new_n50_), .b(x15), .O(new_n79_));
  nand4  g38(.a(new_n57_), .b(x10), .c(x08), .d(x02), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n43_), .O(z14));
  nand2  g42(.a(new_n51_), .b(x03), .O(new_n84_));
  nand4  g43(.a(new_n70_), .b(new_n43_), .c(x16), .d(new_n47_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n52_), .O(z15));
  nand2  g45(.a(z06), .b(x17), .O(new_n87_));
  inv1   g46(.a(x04), .O(new_n88_));
  nand3  g47(.a(x10), .b(x08), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n50_), .b(new_n62_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(new_n57_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n87_), .c(new_n52_), .O(z16));
  nand2  g51(.a(new_n50_), .b(new_n45_), .O(new_n93_));
  inv1   g52(.a(x05), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(new_n94_), .O(new_n95_));
  nand4  g54(.a(new_n95_), .b(new_n93_), .c(new_n57_), .d(x12), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z17));
  oai21  g56(.a(new_n59_), .b(new_n52_), .c(new_n45_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x15), .O(new_n99_));
  nand2  g58(.a(new_n50_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n57_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n99_), .O(z18));
  nand2  g63(.a(z06), .b(x20), .O(new_n105_));
  inv1   g64(.a(x07), .O(new_n106_));
  nand3  g65(.a(x10), .b(x08), .c(new_n106_), .O(new_n107_));
  inv1   g66(.a(x20), .O(new_n108_));
  nand2  g67(.a(new_n50_), .b(new_n108_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(new_n107_), .c(new_n57_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n105_), .c(new_n52_), .O(z19));
endmodule


