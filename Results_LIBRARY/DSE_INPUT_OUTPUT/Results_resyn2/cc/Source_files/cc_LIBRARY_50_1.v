// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x04), .O(new_n44_));
  nand2  g03(.a(x15), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nor3   g05(.a(new_n46_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g06(.a(x15), .O(new_n48_));
  inv1   g07(.a(x14), .O(new_n49_));
  and2   g08(.a(x10), .b(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(x04), .c(new_n48_), .O(z01));
  aoi21  g11(.a(x15), .b(new_n44_), .c(new_n49_), .O(z11));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x15), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(z11), .O(new_n56_));
  nand2  g15(.a(x12), .b(new_n42_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n56_), .O(z02));
  nand2  g17(.a(new_n49_), .b(x12), .O(new_n59_));
  nor2   g18(.a(new_n48_), .b(new_n44_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(new_n50_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n59_), .O(z03));
  nand2  g21(.a(new_n45_), .b(x18), .O(z04));
  inv1   g22(.a(x19), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z05));
  inv1   g24(.a(x17), .O(new_n66_));
  nor2   g25(.a(new_n46_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nand2  g27(.a(new_n45_), .b(new_n68_), .O(z08));
  nand3  g28(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g29(.a(z10), .O(z09));
  inv1   g30(.a(x12), .O(new_n72_));
  nand2  g31(.a(new_n55_), .b(x00), .O(new_n73_));
  nand3  g32(.a(new_n48_), .b(x10), .c(x08), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n45_), .c(x13), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  nand2  g36(.a(new_n48_), .b(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n50_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(z11), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n61_), .c(new_n72_), .O(z13));
  nand2  g40(.a(new_n54_), .b(x15), .O(new_n82_));
  nand4  g41(.a(new_n48_), .b(x10), .c(x08), .d(x02), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n45_), .O(z14));
  oai21  g45(.a(new_n59_), .b(new_n68_), .c(x04), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand4  g47(.a(new_n48_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand3  g48(.a(new_n54_), .b(x16), .c(new_n49_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n88_), .O(z15));
  nand2  g52(.a(new_n55_), .b(new_n44_), .O(new_n94_));
  nand2  g53(.a(new_n74_), .b(new_n66_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n45_), .O(z16));
  nand2  g56(.a(new_n60_), .b(x18), .O(new_n98_));
  inv1   g57(.a(x05), .O(new_n99_));
  nand3  g58(.a(x10), .b(x08), .c(new_n99_), .O(new_n100_));
  inv1   g59(.a(x18), .O(new_n101_));
  nand2  g60(.a(new_n54_), .b(new_n101_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(new_n100_), .c(new_n48_), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n98_), .c(new_n72_), .O(z17));
  nand2  g63(.a(new_n60_), .b(x19), .O(new_n105_));
  inv1   g64(.a(x06), .O(new_n106_));
  nand3  g65(.a(x10), .b(x08), .c(new_n106_), .O(new_n107_));
  nand2  g66(.a(new_n54_), .b(new_n64_), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(new_n107_), .c(new_n48_), .O(new_n109_));
  aoi21  g68(.a(new_n109_), .b(new_n105_), .c(new_n72_), .O(z18));
  oai21  g69(.a(new_n43_), .b(new_n72_), .c(x04), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x15), .O(new_n112_));
  nand2  g71(.a(new_n54_), .b(x20), .O(new_n113_));
  nand4  g72(.a(new_n48_), .b(x10), .c(x08), .d(x07), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x12), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n112_), .O(z19));
  buf    g76(.a(x15), .O(z06));
endmodule


