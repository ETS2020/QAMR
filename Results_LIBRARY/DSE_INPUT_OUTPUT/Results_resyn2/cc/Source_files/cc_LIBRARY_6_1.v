// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n61_, new_n63_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x08), .O(new_n44_));
  nand2  g03(.a(x18), .b(new_n44_), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand4  g06(.a(x15), .b(new_n47_), .c(x10), .d(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  nand2  g08(.a(new_n45_), .b(x14), .O(new_n50_));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(x12), .b(new_n42_), .O(new_n53_));
  aoi21  g12(.a(new_n52_), .b(new_n50_), .c(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor2   g14(.a(new_n48_), .b(new_n55_), .O(z03));
  nand2  g15(.a(x18), .b(x08), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n58_), .O(z05));
  nand2  g18(.a(new_n45_), .b(new_n51_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nand2  g20(.a(new_n45_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(z10));
  nand2  g24(.a(z10), .b(new_n45_), .O(z09));
  nand2  g25(.a(new_n45_), .b(new_n47_), .O(z11));
  nor2   g26(.a(x18), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(x13), .O(new_n69_));
  inv1   g28(.a(x00), .O(new_n70_));
  nand3  g29(.a(new_n51_), .b(x10), .c(new_n70_), .O(new_n71_));
  inv1   g30(.a(x13), .O(new_n72_));
  nand2  g31(.a(new_n51_), .b(x10), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n71_), .c(x08), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n69_), .c(new_n55_), .O(z12));
  nand2  g35(.a(x15), .b(x10), .O(new_n77_));
  nand2  g36(.a(x10), .b(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x14), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x08), .O(new_n81_));
  nand2  g40(.a(new_n68_), .b(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n55_), .O(z13));
  nand2  g42(.a(new_n68_), .b(x15), .O(new_n84_));
  nand2  g43(.a(x10), .b(x02), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n51_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n77_), .c(x08), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n84_), .c(new_n55_), .O(z14));
  nand3  g47(.a(new_n68_), .b(x16), .c(new_n47_), .O(new_n89_));
  nand2  g48(.a(x16), .b(new_n47_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n73_), .O(new_n91_));
  inv1   g50(.a(x03), .O(new_n92_));
  nand3  g51(.a(new_n51_), .b(x10), .c(new_n92_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n91_), .c(x08), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n89_), .c(new_n55_), .O(z15));
  nand2  g54(.a(new_n68_), .b(x17), .O(new_n96_));
  inv1   g55(.a(x04), .O(new_n97_));
  nand3  g56(.a(new_n51_), .b(x10), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n73_), .b(new_n61_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(x08), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n96_), .c(new_n55_), .O(z16));
  inv1   g60(.a(x18), .O(new_n102_));
  nor2   g61(.a(new_n55_), .b(new_n44_), .O(new_n103_));
  oai21  g62(.a(new_n73_), .b(x05), .c(new_n103_), .O(new_n104_));
  aoi21  g63(.a(new_n73_), .b(new_n102_), .c(new_n104_), .O(z17));
  oai21  g64(.a(new_n58_), .b(new_n55_), .c(new_n102_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n44_), .O(new_n107_));
  inv1   g66(.a(x10), .O(new_n108_));
  oai21  g67(.a(x15), .b(new_n108_), .c(x19), .O(new_n109_));
  nand4  g68(.a(new_n51_), .b(x10), .c(x08), .d(x06), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n107_), .O(z18));
  oai21  g72(.a(new_n43_), .b(new_n55_), .c(new_n102_), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n44_), .O(new_n115_));
  oai21  g74(.a(x15), .b(new_n108_), .c(x20), .O(new_n116_));
  nand4  g75(.a(new_n51_), .b(x10), .c(x08), .d(x07), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(x12), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n115_), .O(z19));
endmodule


