// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x05), .O(new_n44_));
  inv1   g03(.a(x18), .O(z04));
  nand2  g04(.a(z04), .b(new_n44_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(new_n46_), .b(new_n48_), .O(z11));
  nand3  g08(.a(x15), .b(x10), .c(x08), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(z11), .O(z01));
  nand2  g10(.a(x10), .b(x08), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x15), .c(new_n48_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(z02));
  inv1   g14(.a(new_n50_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n46_), .c(new_n48_), .d(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z03));
  inv1   g17(.a(x19), .O(new_n59_));
  inv1   g18(.a(new_n46_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n59_), .O(z05));
  inv1   g20(.a(x15), .O(new_n62_));
  nand2  g21(.a(new_n46_), .b(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n60_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n46_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n60_), .O(z09));
  and2   g28(.a(new_n68_), .b(new_n46_), .O(z10));
  inv1   g29(.a(x00), .O(new_n71_));
  nand4  g30(.a(new_n62_), .b(x10), .c(x08), .d(new_n71_), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  oai21  g32(.a(new_n52_), .b(x15), .c(new_n73_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n72_), .c(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n46_), .O(z12));
  inv1   g35(.a(x01), .O(new_n77_));
  oai21  g36(.a(new_n52_), .b(new_n77_), .c(x14), .O(new_n78_));
  nand2  g37(.a(new_n46_), .b(x12), .O(new_n79_));
  aoi21  g38(.a(new_n78_), .b(new_n50_), .c(new_n79_), .O(z13));
  nand3  g39(.a(x10), .b(x08), .c(x02), .O(new_n81_));
  nand3  g40(.a(new_n50_), .b(new_n46_), .c(x12), .O(new_n82_));
  aoi21  g41(.a(new_n81_), .b(new_n62_), .c(new_n82_), .O(z14));
  nand3  g42(.a(new_n62_), .b(x10), .c(x08), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x16), .c(new_n48_), .O(new_n85_));
  nand4  g44(.a(new_n62_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n79_), .O(z15));
  inv1   g46(.a(x04), .O(new_n88_));
  nand4  g47(.a(new_n62_), .b(x10), .c(x08), .d(new_n88_), .O(new_n89_));
  oai21  g48(.a(new_n52_), .b(x15), .c(new_n64_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n46_), .O(z16));
  nand4  g51(.a(new_n62_), .b(x10), .c(x08), .d(new_n44_), .O(new_n93_));
  oai21  g52(.a(new_n52_), .b(x15), .c(z04), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z17));
  inv1   g55(.a(x06), .O(new_n97_));
  nand4  g56(.a(new_n62_), .b(x10), .c(x08), .d(new_n97_), .O(new_n98_));
  oai21  g57(.a(new_n52_), .b(x15), .c(new_n59_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n46_), .O(z18));
  inv1   g60(.a(x12), .O(new_n102_));
  aoi21  g61(.a(z04), .b(new_n44_), .c(new_n102_), .O(new_n103_));
  inv1   g62(.a(x07), .O(new_n104_));
  nand4  g63(.a(new_n62_), .b(x10), .c(x08), .d(new_n104_), .O(new_n105_));
  oai21  g64(.a(new_n52_), .b(x15), .c(new_n43_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  inv1   g66(.a(new_n107_), .O(z19));
endmodule


