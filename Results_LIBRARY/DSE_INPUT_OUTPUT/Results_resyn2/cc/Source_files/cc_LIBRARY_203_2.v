// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x04), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  and2   g05(.a(x10), .b(x08), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x15), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x14), .O(z01));
  inv1   g08(.a(x14), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x15), .c(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n45_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor3   g14(.a(new_n48_), .b(x14), .c(new_n55_), .O(z03));
  nand2  g15(.a(new_n45_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  aoi21  g19(.a(new_n44_), .b(x04), .c(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n45_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n45_), .O(z09));
  nand3  g24(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g25(.a(new_n45_), .b(new_n50_), .O(z11));
  inv1   g26(.a(x13), .O(new_n68_));
  inv1   g27(.a(x04), .O(new_n69_));
  nand2  g28(.a(x12), .b(x00), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n51_), .c(new_n69_), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n44_), .O(new_n72_));
  oai21  g31(.a(new_n51_), .b(x15), .c(x12), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n68_), .c(new_n72_), .O(z12));
  oai21  g33(.a(new_n47_), .b(x14), .c(x15), .O(new_n75_));
  nand2  g34(.a(new_n47_), .b(x01), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x14), .c(new_n69_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n75_), .c(new_n55_), .O(z13));
  aoi21  g37(.a(new_n47_), .b(x02), .c(x15), .O(new_n79_));
  nand2  g38(.a(new_n48_), .b(x12), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(new_n45_), .O(z14));
  nand3  g40(.a(x16), .b(x15), .c(new_n50_), .O(new_n82_));
  nand2  g41(.a(new_n44_), .b(x03), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n47_), .O(new_n84_));
  oai21  g43(.a(new_n62_), .b(x14), .c(new_n51_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n84_), .c(new_n69_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n82_), .c(new_n55_), .O(z15));
  oai21  g46(.a(new_n73_), .b(new_n60_), .c(new_n45_), .O(z16));
  inv1   g47(.a(x18), .O(new_n89_));
  nand4  g48(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n69_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n44_), .O(new_n92_));
  oai21  g51(.a(new_n73_), .b(new_n89_), .c(new_n92_), .O(z17));
  nand4  g52(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n69_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n44_), .O(new_n96_));
  oai21  g55(.a(new_n73_), .b(new_n58_), .c(new_n96_), .O(z18));
  nand2  g56(.a(x20), .b(x15), .O(new_n98_));
  nand2  g57(.a(new_n44_), .b(x07), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  aoi21  g59(.a(new_n51_), .b(new_n43_), .c(x04), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n98_), .c(new_n55_), .O(z19));
  buf    g62(.a(x15), .O(z06));
endmodule


