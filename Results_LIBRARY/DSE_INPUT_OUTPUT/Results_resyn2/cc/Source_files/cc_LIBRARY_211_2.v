// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n59_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x14), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(new_n43_), .b(x10), .c(x08), .O(new_n49_));
  inv1   g08(.a(x11), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x12), .c(new_n50_), .O(new_n51_));
  aoi21  g10(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(z02));
  nand2  g11(.a(new_n48_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n46_), .c(new_n44_), .O(z03));
  inv1   g13(.a(new_n44_), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x18), .O(z04));
  and2   g15(.a(new_n44_), .b(x19), .O(z05));
  and2   g16(.a(new_n44_), .b(x17), .O(z07));
  inv1   g17(.a(x16), .O(new_n59_));
  nand2  g18(.a(new_n44_), .b(new_n59_), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n44_), .O(z09));
  inv1   g21(.a(z09), .O(z10));
  nand2  g22(.a(x10), .b(x08), .O(new_n64_));
  nand2  g23(.a(x12), .b(x00), .O(new_n65_));
  oai21  g24(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(new_n67_));
  nand3  g26(.a(new_n49_), .b(x13), .c(x12), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n67_), .O(z12));
  inv1   g28(.a(x12), .O(new_n70_));
  aoi22  g29(.a(new_n46_), .b(new_n48_), .c(x15), .d(new_n70_), .O(z13));
  nand3  g30(.a(x10), .b(x08), .c(x02), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n43_), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n46_), .c(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n44_), .O(z14));
  nor2   g34(.a(x14), .b(new_n70_), .O(new_n76_));
  inv1   g35(.a(x03), .O(new_n77_));
  nand4  g36(.a(new_n43_), .b(x10), .c(x08), .d(new_n77_), .O(new_n78_));
  oai21  g37(.a(new_n64_), .b(x15), .c(new_n59_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z15));
  nand2  g40(.a(x12), .b(x04), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n64_), .c(new_n48_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n43_), .O(new_n84_));
  nand3  g43(.a(new_n49_), .b(x17), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n84_), .O(z16));
  nand2  g45(.a(x12), .b(x05), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n64_), .c(new_n48_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n43_), .O(new_n89_));
  nand3  g48(.a(new_n49_), .b(x18), .c(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(z17));
  nand2  g50(.a(x12), .b(x06), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n64_), .c(new_n48_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n43_), .O(new_n94_));
  nand3  g53(.a(new_n49_), .b(x19), .c(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(z18));
  nand2  g55(.a(x12), .b(x07), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n64_), .c(new_n48_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n43_), .O(new_n99_));
  nand3  g58(.a(new_n49_), .b(x20), .c(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(z19));
  buf    g60(.a(x15), .O(z06));
  buf    g61(.a(x14), .O(z11));
endmodule


