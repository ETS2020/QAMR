// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n66_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_;
  inv1   g00(.a(x18), .O(z04));
  nand2  g01(.a(z04), .b(x12), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  nand3  g05(.a(x15), .b(x10), .c(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(new_n43_), .b(new_n48_), .O(z11));
  nor2   g08(.a(z11), .b(new_n47_), .O(z01));
  inv1   g09(.a(x12), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x15), .c(new_n48_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x18), .c(new_n51_), .O(z02));
  inv1   g15(.a(new_n47_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n48_), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x18), .c(new_n51_), .O(z03));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n43_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x15), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z06));
  and2   g22(.a(new_n43_), .b(x17), .O(z07));
  and2   g23(.a(new_n43_), .b(x16), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(z09));
  nand3  g26(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g27(.a(x00), .O(new_n69_));
  inv1   g28(.a(new_n53_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n62_), .c(new_n69_), .O(new_n71_));
  nor2   g30(.a(z04), .b(new_n51_), .O(new_n72_));
  inv1   g31(.a(x13), .O(new_n73_));
  nand3  g32(.a(new_n62_), .b(x10), .c(x08), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(new_n72_), .c(new_n71_), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z12));
  inv1   g36(.a(x01), .O(new_n78_));
  oai21  g37(.a(new_n53_), .b(new_n78_), .c(x14), .O(new_n79_));
  nor2   g38(.a(new_n57_), .b(z04), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n51_), .O(z13));
  aoi21  g40(.a(new_n70_), .b(x02), .c(x15), .O(new_n82_));
  nand2  g41(.a(new_n72_), .b(new_n47_), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n82_), .O(z14));
  inv1   g43(.a(new_n72_), .O(new_n85_));
  nand3  g44(.a(new_n74_), .b(x16), .c(new_n48_), .O(new_n86_));
  nor2   g45(.a(new_n53_), .b(x15), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n86_), .c(new_n85_), .O(z15));
  nand2  g48(.a(new_n87_), .b(x04), .O(new_n90_));
  aoi21  g49(.a(new_n74_), .b(x17), .c(z04), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n51_), .O(z16));
  inv1   g51(.a(x05), .O(new_n93_));
  aoi21  g52(.a(new_n87_), .b(new_n93_), .c(new_n85_), .O(z17));
  inv1   g53(.a(x06), .O(new_n95_));
  nand3  g54(.a(new_n70_), .b(new_n62_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n74_), .b(new_n60_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(new_n72_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z18));
  nand2  g58(.a(new_n87_), .b(x07), .O(new_n100_));
  aoi21  g59(.a(new_n74_), .b(x20), .c(z04), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n51_), .O(z19));
endmodule


