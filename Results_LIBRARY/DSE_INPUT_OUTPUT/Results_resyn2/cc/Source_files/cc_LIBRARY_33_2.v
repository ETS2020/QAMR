// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x18), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nor2   g07(.a(new_n43_), .b(new_n48_), .O(z06));
  nand2  g08(.a(z06), .b(new_n47_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n46_), .O(z01));
  nor2   g10(.a(new_n46_), .b(x15), .O(new_n52_));
  inv1   g11(.a(x12), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x11), .O(new_n54_));
  oai21  g13(.a(new_n52_), .b(x14), .c(new_n54_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n44_), .O(z02));
  and2   g15(.a(x10), .b(x08), .O(new_n57_));
  nand4  g16(.a(z06), .b(new_n57_), .c(new_n47_), .d(x12), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(z03));
  nor2   g18(.a(x18), .b(x15), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(x18), .b(new_n48_), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(new_n61_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n44_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n44_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n62_), .O(z09));
  and2   g28(.a(new_n68_), .b(new_n44_), .O(z10));
  nand2  g29(.a(new_n44_), .b(new_n47_), .O(z11));
  nand3  g30(.a(new_n48_), .b(x10), .c(x08), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n44_), .c(x13), .O(new_n73_));
  nand2  g32(.a(new_n52_), .b(x00), .O(new_n74_));
  aoi21  g33(.a(new_n74_), .b(new_n73_), .c(new_n53_), .O(z12));
  oai21  g34(.a(new_n46_), .b(new_n48_), .c(new_n47_), .O(new_n76_));
  nand3  g35(.a(new_n57_), .b(new_n48_), .c(x01), .O(new_n77_));
  nand4  g36(.a(new_n77_), .b(new_n76_), .c(new_n44_), .d(x12), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  nand2  g38(.a(new_n57_), .b(x15), .O(new_n80_));
  inv1   g39(.a(x02), .O(new_n81_));
  oai21  g40(.a(new_n46_), .b(new_n81_), .c(new_n48_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n80_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(z14));
  nand2  g43(.a(new_n52_), .b(x03), .O(new_n85_));
  nand4  g44(.a(new_n72_), .b(new_n44_), .c(x16), .d(new_n47_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n53_), .O(z15));
  oai21  g46(.a(new_n64_), .b(new_n53_), .c(x18), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x15), .O(new_n89_));
  nand2  g48(.a(new_n46_), .b(x17), .O(new_n90_));
  nand4  g49(.a(new_n48_), .b(x10), .c(x08), .d(x04), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n89_), .O(z16));
  nor2   g53(.a(new_n62_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n46_), .b(x18), .O(new_n96_));
  aoi21  g55(.a(new_n57_), .b(x05), .c(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n95_), .O(z17));
  nand2  g57(.a(z06), .b(x19), .O(new_n99_));
  inv1   g58(.a(x06), .O(new_n100_));
  nand3  g59(.a(x10), .b(x08), .c(new_n100_), .O(new_n101_));
  nand2  g60(.a(new_n46_), .b(new_n61_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(new_n101_), .c(new_n48_), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n99_), .c(new_n53_), .O(z18));
  nand2  g63(.a(x20), .b(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x18), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x15), .O(new_n107_));
  nand2  g66(.a(new_n46_), .b(x20), .O(new_n108_));
  nand4  g67(.a(new_n48_), .b(x10), .c(x08), .d(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n107_), .O(z19));
endmodule


