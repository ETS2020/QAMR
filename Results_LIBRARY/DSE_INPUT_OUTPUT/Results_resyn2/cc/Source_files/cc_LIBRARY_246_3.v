// Benchmark "FAU" written by ABC on Sat Aug  8 23:08:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x12), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z03));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(z03), .O(z00));
  nor2   g05(.a(new_n43_), .b(x12), .O(z06));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x14), .O(new_n49_));
  and2   g08(.a(new_n49_), .b(z06), .O(z01));
  aoi21  g09(.a(x10), .b(x08), .c(x14), .O(new_n51_));
  nand2  g10(.a(new_n43_), .b(x12), .O(new_n52_));
  nor3   g11(.a(new_n52_), .b(new_n51_), .c(x11), .O(z02));
  inv1   g12(.a(z03), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nand2  g15(.a(new_n54_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n54_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n54_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n54_), .O(z09));
  nand3  g22(.a(new_n54_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(x14), .O(new_n65_));
  nor2   g24(.a(z03), .b(new_n65_), .O(z11));
  nor2   g25(.a(x15), .b(new_n42_), .O(new_n67_));
  inv1   g26(.a(x00), .O(new_n68_));
  nand3  g27(.a(x10), .b(x08), .c(new_n68_), .O(new_n69_));
  inv1   g28(.a(x13), .O(new_n70_));
  nand2  g29(.a(new_n48_), .b(new_n70_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n69_), .c(new_n67_), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  inv1   g32(.a(x01), .O(new_n74_));
  oai21  g33(.a(new_n48_), .b(new_n74_), .c(x14), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(new_n43_), .c(new_n42_), .O(z13));
  nand3  g35(.a(x10), .b(x08), .c(x02), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n43_), .c(new_n42_), .O(z14));
  nand2  g37(.a(new_n51_), .b(x16), .O(new_n79_));
  nand3  g38(.a(x10), .b(x08), .c(x03), .O(new_n80_));
  and2   g39(.a(new_n80_), .b(new_n43_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n79_), .c(new_n42_), .O(z15));
  nand3  g41(.a(x10), .b(x08), .c(x04), .O(new_n83_));
  aoi21  g42(.a(new_n48_), .b(x17), .c(x15), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n42_), .O(z16));
  inv1   g44(.a(x05), .O(new_n86_));
  nand3  g45(.a(x10), .b(x08), .c(new_n86_), .O(new_n87_));
  inv1   g46(.a(x18), .O(new_n88_));
  nand2  g47(.a(new_n48_), .b(new_n88_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n87_), .c(new_n67_), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z17));
  inv1   g50(.a(x06), .O(new_n92_));
  nand3  g51(.a(x10), .b(x08), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n48_), .b(new_n56_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n93_), .c(new_n67_), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z18));
  inv1   g55(.a(x07), .O(new_n97_));
  nand3  g56(.a(x10), .b(x08), .c(new_n97_), .O(new_n98_));
  inv1   g57(.a(x20), .O(new_n99_));
  nand2  g58(.a(new_n48_), .b(new_n99_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n98_), .c(new_n67_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z19));
endmodule


