// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_;
  nand3  g00(.a(x20), .b(x15), .c(x11), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(z00));
  nand3  g02(.a(x15), .b(x10), .c(x08), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x14), .O(z01));
  inv1   g04(.a(x20), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x15), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(x15), .O(new_n50_));
  inv1   g09(.a(x12), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(x11), .O(new_n52_));
  oai21  g11(.a(new_n50_), .b(x14), .c(new_n52_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n48_), .O(z02));
  nand2  g13(.a(z01), .b(x12), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n48_), .O(z03));
  nor2   g15(.a(new_n47_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n48_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x15), .O(new_n60_));
  nand2  g19(.a(new_n46_), .b(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n47_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n48_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nor2   g25(.a(new_n66_), .b(new_n47_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n47_), .O(z10));
  inv1   g27(.a(x14), .O(new_n69_));
  nor2   g28(.a(new_n47_), .b(new_n69_), .O(z11));
  nand2  g29(.a(x12), .b(x00), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n49_), .c(new_n46_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  inv1   g32(.a(new_n50_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x13), .c(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n73_), .O(z12));
  nand3  g35(.a(x10), .b(x08), .c(x01), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x14), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n44_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n48_), .O(z13));
  inv1   g40(.a(new_n49_), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(x02), .c(x15), .O(new_n83_));
  nand2  g42(.a(new_n44_), .b(x12), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n83_), .c(new_n48_), .O(z14));
  aoi21  g44(.a(x10), .b(x08), .c(x20), .O(new_n86_));
  nor2   g45(.a(new_n64_), .b(x14), .O(new_n87_));
  oai21  g46(.a(new_n86_), .b(x15), .c(new_n87_), .O(new_n88_));
  nand4  g47(.a(new_n82_), .b(new_n46_), .c(new_n60_), .d(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n51_), .O(z15));
  oai21  g49(.a(new_n86_), .b(x15), .c(x17), .O(new_n91_));
  nand4  g50(.a(new_n82_), .b(new_n46_), .c(new_n60_), .d(x04), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n51_), .O(z16));
  oai21  g52(.a(new_n86_), .b(x15), .c(x18), .O(new_n94_));
  nand4  g53(.a(new_n82_), .b(new_n46_), .c(new_n60_), .d(x05), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n51_), .O(z17));
  nand2  g55(.a(x12), .b(x06), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n49_), .c(new_n46_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n60_), .O(new_n99_));
  nand3  g58(.a(new_n74_), .b(x19), .c(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(z18));
  nand4  g60(.a(new_n60_), .b(x10), .c(x08), .d(x07), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n46_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n48_), .O(z19));
endmodule


