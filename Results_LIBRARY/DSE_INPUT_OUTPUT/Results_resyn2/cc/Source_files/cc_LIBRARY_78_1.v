// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n56_, new_n58_, new_n60_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(x18), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand3  g04(.a(x15), .b(x10), .c(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(new_n43_), .b(x10), .c(x08), .O(new_n49_));
  inv1   g08(.a(x11), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x12), .c(new_n50_), .O(new_n51_));
  aoi21  g10(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(z02));
  inv1   g11(.a(x12), .O(new_n53_));
  nor3   g12(.a(new_n46_), .b(x14), .c(new_n53_), .O(z03));
  inv1   g13(.a(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nand2  g15(.a(new_n44_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  aoi21  g19(.a(x18), .b(new_n43_), .c(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n44_), .O(z09));
  nand3  g22(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nand2  g23(.a(new_n44_), .b(new_n48_), .O(z11));
  nand2  g24(.a(x10), .b(x08), .O(new_n66_));
  nand2  g25(.a(x12), .b(x00), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n66_), .c(z04), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n43_), .O(new_n69_));
  nand3  g28(.a(new_n49_), .b(x13), .c(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n69_), .O(z12));
  nand3  g30(.a(x10), .b(x08), .c(x01), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x14), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n46_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n44_), .O(z13));
  nand2  g35(.a(new_n66_), .b(x15), .O(new_n77_));
  nand3  g36(.a(z04), .b(new_n43_), .c(x10), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(new_n79_));
  and2   g38(.a(x08), .b(x02), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n77_), .c(new_n53_), .O(z14));
  aoi21  g41(.a(x10), .b(x08), .c(x18), .O(new_n83_));
  nor2   g42(.a(new_n60_), .b(x14), .O(new_n84_));
  oai21  g43(.a(new_n83_), .b(x15), .c(new_n84_), .O(new_n85_));
  and2   g44(.a(x08), .b(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n79_), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n85_), .c(new_n53_), .O(z15));
  nand2  g47(.a(x12), .b(x04), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n66_), .c(z04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n43_), .O(new_n91_));
  nand3  g50(.a(new_n49_), .b(x17), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(z16));
  nand2  g52(.a(x18), .b(x15), .O(new_n94_));
  and2   g53(.a(x08), .b(x05), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n94_), .c(new_n53_), .O(z17));
  nand2  g56(.a(x12), .b(x06), .O(new_n98_));
  oai21  g57(.a(new_n98_), .b(new_n66_), .c(z04), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand3  g59(.a(new_n49_), .b(x19), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(z18));
  nand2  g61(.a(x12), .b(x07), .O(new_n103_));
  oai21  g62(.a(new_n103_), .b(new_n66_), .c(z04), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n43_), .O(new_n105_));
  nand3  g64(.a(new_n49_), .b(x20), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(z19));
  buf    g66(.a(x15), .O(z06));
endmodule


