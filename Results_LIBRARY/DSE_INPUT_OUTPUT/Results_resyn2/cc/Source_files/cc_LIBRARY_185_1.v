// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x12), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  nand3  g05(.a(x12), .b(x10), .c(x08), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n44_), .c(x14), .O(z01));
  inv1   g07(.a(x14), .O(new_n49_));
  and2   g08(.a(x10), .b(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n44_), .O(new_n51_));
  nand2  g10(.a(x12), .b(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n51_), .b(new_n49_), .c(new_n52_), .O(z02));
  nor2   g12(.a(new_n45_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nor2   g14(.a(new_n45_), .b(new_n55_), .O(z05));
  inv1   g15(.a(x17), .O(new_n57_));
  nor2   g16(.a(new_n45_), .b(new_n57_), .O(z07));
  inv1   g17(.a(x16), .O(new_n59_));
  inv1   g18(.a(x12), .O(new_n60_));
  nand2  g19(.a(x15), .b(new_n60_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n59_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n45_), .O(z09));
  and2   g23(.a(new_n63_), .b(new_n61_), .O(z10));
  nand2  g24(.a(new_n61_), .b(new_n49_), .O(z11));
  inv1   g25(.a(x00), .O(new_n67_));
  nand4  g26(.a(new_n44_), .b(x10), .c(x08), .d(new_n67_), .O(new_n68_));
  inv1   g27(.a(x13), .O(new_n69_));
  nand2  g28(.a(x10), .b(x08), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(x15), .c(new_n69_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n68_), .c(x12), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  nand2  g32(.a(new_n50_), .b(x15), .O(new_n74_));
  inv1   g33(.a(x01), .O(new_n75_));
  oai21  g34(.a(new_n70_), .b(new_n75_), .c(x14), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n74_), .c(new_n60_), .O(z13));
  nand2  g36(.a(new_n47_), .b(x15), .O(new_n78_));
  nand2  g37(.a(new_n44_), .b(x02), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n47_), .c(new_n78_), .O(z14));
  nand2  g39(.a(x16), .b(new_n49_), .O(new_n81_));
  inv1   g40(.a(x03), .O(new_n82_));
  nand4  g41(.a(new_n44_), .b(x10), .c(x08), .d(new_n82_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x12), .O(new_n84_));
  aoi21  g43(.a(new_n81_), .b(new_n51_), .c(new_n84_), .O(z15));
  oai21  g44(.a(new_n70_), .b(x04), .c(x12), .O(new_n86_));
  nor2   g45(.a(x17), .b(new_n60_), .O(new_n87_));
  aoi22  g46(.a(new_n87_), .b(new_n51_), .c(new_n86_), .d(new_n44_), .O(z16));
  oai21  g47(.a(new_n70_), .b(x05), .c(x12), .O(new_n89_));
  nor2   g48(.a(x18), .b(new_n60_), .O(new_n90_));
  aoi22  g49(.a(new_n90_), .b(new_n51_), .c(new_n89_), .d(new_n44_), .O(z17));
  oai21  g50(.a(new_n70_), .b(x06), .c(x12), .O(new_n92_));
  nor2   g51(.a(x19), .b(new_n60_), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n51_), .c(new_n92_), .d(new_n44_), .O(z18));
  inv1   g53(.a(x07), .O(new_n95_));
  nand4  g54(.a(new_n44_), .b(x10), .c(x08), .d(new_n95_), .O(new_n96_));
  oai21  g55(.a(new_n70_), .b(x15), .c(new_n43_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n96_), .c(x12), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z19));
  nor3   g58(.a(new_n47_), .b(new_n44_), .c(x14), .O(z03));
  buf    g59(.a(x15), .O(z06));
endmodule


