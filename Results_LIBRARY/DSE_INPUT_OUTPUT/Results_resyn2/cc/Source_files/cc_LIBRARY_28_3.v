// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  inv1   g03(.a(x19), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n43_), .b(new_n42_), .c(new_n47_), .O(z00));
  inv1   g07(.a(x14), .O(new_n49_));
  nand3  g08(.a(x15), .b(x10), .c(x08), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n46_), .O(z01));
  nand2  g12(.a(x10), .b(x08), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x15), .c(new_n49_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n42_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(new_n45_), .c(new_n44_), .O(z02));
  nand2  g16(.a(new_n45_), .b(x12), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(new_n52_), .O(z03));
  nand2  g18(.a(new_n47_), .b(x18), .O(z04));
  nor2   g19(.a(new_n45_), .b(x12), .O(z05));
  inv1   g20(.a(x15), .O(new_n62_));
  nand2  g21(.a(new_n47_), .b(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n47_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n47_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n47_), .O(z09));
  nand3  g28(.a(new_n47_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n46_), .b(new_n49_), .O(z11));
  inv1   g30(.a(new_n58_), .O(new_n72_));
  inv1   g31(.a(x00), .O(new_n73_));
  nand4  g32(.a(new_n62_), .b(x10), .c(x08), .d(new_n73_), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  nand3  g34(.a(new_n62_), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z12));
  inv1   g38(.a(x01), .O(new_n80_));
  oai21  g39(.a(new_n54_), .b(new_n80_), .c(x14), .O(new_n81_));
  nor2   g40(.a(new_n51_), .b(x19), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n44_), .O(z13));
  nor2   g42(.a(new_n54_), .b(x15), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x02), .O(new_n85_));
  aoi21  g44(.a(new_n54_), .b(x15), .c(x19), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n44_), .O(z14));
  nand3  g46(.a(new_n76_), .b(x16), .c(new_n49_), .O(new_n88_));
  nand2  g47(.a(new_n84_), .b(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n58_), .O(z15));
  nand2  g49(.a(new_n84_), .b(x04), .O(new_n91_));
  aoi21  g50(.a(new_n76_), .b(x17), .c(x19), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n44_), .O(z16));
  nand2  g52(.a(new_n84_), .b(x05), .O(new_n94_));
  aoi21  g53(.a(new_n76_), .b(x18), .c(x19), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n44_), .O(z17));
  inv1   g55(.a(x06), .O(new_n97_));
  nor3   g56(.a(new_n58_), .b(new_n76_), .c(new_n97_), .O(z18));
  inv1   g57(.a(x07), .O(new_n99_));
  nand4  g58(.a(new_n62_), .b(x10), .c(x08), .d(new_n99_), .O(new_n100_));
  nand2  g59(.a(new_n76_), .b(new_n43_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n100_), .c(new_n72_), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z19));
endmodule


