// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x12), .O(new_n44_));
  nor2   g03(.a(x19), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(x15), .b(x10), .c(x08), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n45_), .O(z01));
  nand2  g11(.a(x10), .b(x08), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x15), .c(new_n48_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n42_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x19), .c(new_n44_), .O(z02));
  inv1   g15(.a(x19), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n44_), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n51_), .O(z03));
  nand2  g19(.a(new_n46_), .b(x18), .O(z04));
  inv1   g20(.a(x15), .O(new_n62_));
  nand2  g21(.a(new_n46_), .b(new_n62_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n46_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n46_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n46_), .O(z09));
  nand3  g28(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n45_), .b(new_n48_), .O(z11));
  inv1   g30(.a(x00), .O(new_n72_));
  nand4  g31(.a(new_n62_), .b(x10), .c(x08), .d(new_n72_), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  nand3  g33(.a(new_n62_), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n73_), .c(new_n58_), .O(new_n77_));
  inv1   g36(.a(new_n77_), .O(z12));
  inv1   g37(.a(x01), .O(new_n79_));
  oai21  g38(.a(new_n53_), .b(new_n79_), .c(x14), .O(new_n80_));
  nor2   g39(.a(new_n50_), .b(new_n57_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n44_), .O(z13));
  nor2   g41(.a(new_n53_), .b(x15), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x02), .O(new_n84_));
  aoi21  g43(.a(new_n53_), .b(x15), .c(new_n57_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n44_), .O(z14));
  nand3  g45(.a(new_n75_), .b(x16), .c(new_n48_), .O(new_n87_));
  nand2  g46(.a(new_n83_), .b(x03), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n87_), .c(new_n59_), .O(z15));
  nand2  g48(.a(new_n83_), .b(x04), .O(new_n90_));
  aoi21  g49(.a(new_n75_), .b(x17), .c(new_n57_), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(new_n44_), .O(z16));
  nand2  g51(.a(new_n83_), .b(x05), .O(new_n93_));
  aoi21  g52(.a(new_n75_), .b(x18), .c(new_n57_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n44_), .O(z17));
  inv1   g54(.a(x06), .O(new_n96_));
  aoi21  g55(.a(new_n83_), .b(new_n96_), .c(new_n59_), .O(z18));
  inv1   g56(.a(x07), .O(new_n98_));
  nand4  g57(.a(new_n62_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n75_), .b(new_n43_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(new_n58_), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z19));
  buf    g61(.a(x19), .O(z05));
endmodule


