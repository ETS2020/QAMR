// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_;
  nor2   g00(.a(x18), .b(x14), .O(new_n42_));
  nand2  g01(.a(x20), .b(x11), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand4  g03(.a(x18), .b(x15), .c(x10), .d(x08), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(x14), .O(z01));
  inv1   g05(.a(new_n42_), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(x15), .O(new_n49_));
  inv1   g08(.a(x12), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x11), .O(new_n51_));
  oai21  g10(.a(new_n49_), .b(x14), .c(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n47_), .O(z02));
  nor3   g12(.a(new_n45_), .b(x14), .c(new_n50_), .O(z03));
  inv1   g13(.a(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nor2   g15(.a(new_n42_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x15), .O(new_n58_));
  nand2  g17(.a(new_n47_), .b(new_n58_), .O(z06));
  inv1   g18(.a(x17), .O(new_n60_));
  nand2  g19(.a(new_n47_), .b(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n47_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(x08), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n47_), .O(z09));
  nand3  g24(.a(new_n47_), .b(x09), .c(x08), .O(z10));
  inv1   g25(.a(x13), .O(new_n67_));
  nand3  g26(.a(new_n58_), .b(x10), .c(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  inv1   g28(.a(x00), .O(new_n70_));
  nand4  g29(.a(new_n58_), .b(x10), .c(x08), .d(new_n70_), .O(new_n71_));
  nor2   g30(.a(new_n42_), .b(new_n50_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(new_n71_), .c(new_n69_), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  nand4  g33(.a(new_n58_), .b(x10), .c(x08), .d(x01), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x14), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n45_), .c(new_n50_), .O(z13));
  nand3  g36(.a(x15), .b(x10), .c(x08), .O(new_n78_));
  inv1   g37(.a(x02), .O(new_n79_));
  oai21  g38(.a(new_n48_), .b(new_n79_), .c(new_n58_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n78_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n47_), .O(z14));
  inv1   g41(.a(x03), .O(new_n83_));
  nand4  g42(.a(new_n58_), .b(x10), .c(x08), .d(new_n83_), .O(new_n84_));
  oai22  g43(.a(new_n48_), .b(x15), .c(new_n62_), .d(x14), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n84_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n47_), .O(z15));
  inv1   g46(.a(x04), .O(new_n88_));
  nand4  g47(.a(new_n58_), .b(x10), .c(x08), .d(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n68_), .b(new_n60_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n89_), .c(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n47_), .O(z16));
  nand2  g51(.a(new_n49_), .b(x14), .O(new_n93_));
  inv1   g52(.a(x05), .O(new_n94_));
  nand4  g53(.a(new_n58_), .b(x10), .c(x08), .d(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x12), .O(new_n96_));
  aoi21  g55(.a(new_n93_), .b(z04), .c(new_n96_), .O(z17));
  inv1   g56(.a(x06), .O(new_n98_));
  nand4  g57(.a(new_n58_), .b(x10), .c(x08), .d(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n68_), .b(new_n56_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n47_), .O(z18));
  inv1   g61(.a(x07), .O(new_n103_));
  nand4  g62(.a(new_n58_), .b(x10), .c(x08), .d(new_n103_), .O(new_n104_));
  inv1   g63(.a(x20), .O(new_n105_));
  nand2  g64(.a(new_n68_), .b(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(new_n104_), .c(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n47_), .O(z19));
  buf    g67(.a(x14), .O(z11));
endmodule


