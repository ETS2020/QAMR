// Benchmark "FAU" written by ABC on Sat Aug  8 23:07:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n54_, new_n55_, new_n56_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_;
  inv1   g00(.a(x12), .O(new_n42_));
  nand2  g01(.a(x15), .b(new_n42_), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(x20), .b(x11), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  and2   g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n42_), .O(z06));
  inv1   g10(.a(z06), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n49_), .O(z01));
  nand2  g12(.a(new_n48_), .b(new_n50_), .O(new_n54_));
  inv1   g13(.a(x11), .O(new_n55_));
  nand2  g14(.a(x12), .b(new_n55_), .O(new_n56_));
  aoi21  g15(.a(new_n54_), .b(new_n47_), .c(new_n56_), .O(z02));
  aoi21  g16(.a(new_n49_), .b(x12), .c(new_n50_), .O(z03));
  nand2  g17(.a(new_n43_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n44_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(z09));
  inv1   g26(.a(z09), .O(z10));
  nor2   g27(.a(new_n44_), .b(new_n47_), .O(z11));
  nand2  g28(.a(x10), .b(x08), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(x00), .c(x12), .O(new_n71_));
  nor2   g30(.a(x13), .b(new_n42_), .O(new_n72_));
  aoi22  g31(.a(new_n72_), .b(new_n54_), .c(new_n71_), .d(new_n50_), .O(z12));
  oai21  g32(.a(new_n48_), .b(new_n42_), .c(x15), .O(new_n74_));
  nand2  g33(.a(new_n48_), .b(x01), .O(new_n75_));
  nor2   g34(.a(new_n47_), .b(new_n42_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n74_), .O(z13));
  nand2  g37(.a(new_n48_), .b(x12), .O(new_n79_));
  oai21  g38(.a(new_n70_), .b(new_n42_), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n50_), .b(x02), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n79_), .c(new_n80_), .O(z14));
  nand2  g41(.a(x16), .b(new_n47_), .O(new_n83_));
  inv1   g42(.a(x03), .O(new_n84_));
  nand4  g43(.a(new_n50_), .b(x10), .c(x08), .d(new_n84_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  aoi21  g45(.a(new_n83_), .b(new_n54_), .c(new_n86_), .O(z15));
  oai21  g46(.a(new_n70_), .b(x04), .c(x12), .O(new_n88_));
  nor2   g47(.a(x17), .b(new_n42_), .O(new_n89_));
  aoi22  g48(.a(new_n89_), .b(new_n54_), .c(new_n88_), .d(new_n50_), .O(z16));
  inv1   g49(.a(x05), .O(new_n91_));
  nand4  g50(.a(new_n50_), .b(x10), .c(x08), .d(new_n91_), .O(new_n92_));
  inv1   g51(.a(x18), .O(new_n93_));
  oai21  g52(.a(new_n70_), .b(x15), .c(new_n93_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n92_), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z17));
  inv1   g55(.a(x06), .O(new_n97_));
  nand4  g56(.a(new_n50_), .b(x10), .c(x08), .d(new_n97_), .O(new_n98_));
  oai21  g57(.a(new_n70_), .b(x15), .c(new_n60_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(new_n98_), .c(x12), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z18));
  oai21  g60(.a(new_n70_), .b(x07), .c(x12), .O(new_n102_));
  nor2   g61(.a(x20), .b(new_n42_), .O(new_n103_));
  aoi22  g62(.a(new_n103_), .b(new_n54_), .c(new_n102_), .d(new_n50_), .O(z19));
endmodule


