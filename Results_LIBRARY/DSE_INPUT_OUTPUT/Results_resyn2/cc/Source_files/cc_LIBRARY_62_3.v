// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x19), .b(x08), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x19), .O(new_n45_));
  nand4  g04(.a(new_n45_), .b(x15), .c(x10), .d(x08), .O(new_n46_));
  nor2   g05(.a(new_n46_), .b(x14), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x10), .O(new_n49_));
  nor2   g08(.a(x15), .b(new_n49_), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(new_n45_), .c(x08), .O(new_n51_));
  inv1   g10(.a(x11), .O(new_n52_));
  nand3  g11(.a(new_n43_), .b(x12), .c(new_n52_), .O(new_n53_));
  aoi21  g12(.a(new_n51_), .b(new_n48_), .c(new_n53_), .O(z02));
  nand2  g13(.a(z01), .b(x12), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z03));
  nand2  g15(.a(new_n43_), .b(x18), .O(z04));
  inv1   g16(.a(x08), .O(new_n58_));
  nand2  g17(.a(x19), .b(new_n58_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z05));
  inv1   g19(.a(x15), .O(new_n61_));
  aoi21  g20(.a(x19), .b(x08), .c(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n43_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n43_), .b(new_n65_), .O(z08));
  nand3  g25(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(z10), .O(z09));
  nand2  g27(.a(new_n43_), .b(new_n48_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  aoi21  g29(.a(new_n61_), .b(x10), .c(x19), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n58_), .c(x13), .O(new_n72_));
  nand4  g31(.a(new_n50_), .b(new_n45_), .c(x08), .d(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(z12));
  aoi21  g33(.a(x10), .b(x01), .c(x19), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n58_), .c(x14), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n46_), .c(new_n70_), .O(z13));
  nand4  g36(.a(new_n61_), .b(x12), .c(x10), .d(x02), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n45_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x08), .O(new_n80_));
  nand2  g39(.a(x10), .b(x08), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x15), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n80_), .O(z14));
  nand4  g42(.a(new_n61_), .b(x12), .c(x10), .d(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n45_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x08), .O(new_n86_));
  nand2  g45(.a(new_n50_), .b(x08), .O(new_n87_));
  nand4  g46(.a(new_n87_), .b(x16), .c(new_n48_), .d(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n86_), .O(z15));
  nand4  g48(.a(new_n61_), .b(x12), .c(x10), .d(x04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n45_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x08), .O(new_n92_));
  nand3  g51(.a(new_n87_), .b(x17), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z16));
  nand4  g53(.a(new_n61_), .b(x12), .c(x10), .d(x05), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n45_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x08), .O(new_n97_));
  nand3  g56(.a(new_n87_), .b(x18), .c(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n97_), .O(z17));
  nand4  g58(.a(new_n50_), .b(new_n45_), .c(x08), .d(x06), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n59_), .c(new_n70_), .O(z18));
  nand4  g60(.a(new_n61_), .b(x12), .c(x10), .d(x07), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n45_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x08), .O(new_n104_));
  nand3  g63(.a(new_n87_), .b(x20), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z19));
endmodule


