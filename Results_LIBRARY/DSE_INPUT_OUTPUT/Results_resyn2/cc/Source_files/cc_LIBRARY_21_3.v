// Benchmark "FAU" written by ABC on Sat Aug  8 23:06:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n57_, new_n59_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_;
  nand2  g00(.a(x15), .b(x12), .O(new_n42_));
  inv1   g01(.a(new_n42_), .O(z03));
  nand2  g02(.a(x20), .b(x11), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(z03), .O(z00));
  inv1   g04(.a(x12), .O(new_n46_));
  inv1   g05(.a(x15), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x10), .b(x08), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g10(.a(new_n51_), .b(new_n46_), .c(new_n47_), .O(z01));
  aoi21  g11(.a(x10), .b(x08), .c(x14), .O(new_n53_));
  nand2  g12(.a(new_n47_), .b(x12), .O(new_n54_));
  nor3   g13(.a(new_n54_), .b(new_n53_), .c(x11), .O(z02));
  nand2  g14(.a(new_n42_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  nor2   g16(.a(z03), .b(new_n57_), .O(z05));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n42_), .b(new_n59_), .O(z07));
  and2   g19(.a(new_n42_), .b(x16), .O(z08));
  nand3  g20(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  inv1   g21(.a(z10), .O(z09));
  nor2   g22(.a(z03), .b(new_n48_), .O(z11));
  nor2   g23(.a(x15), .b(new_n46_), .O(new_n65_));
  inv1   g24(.a(x00), .O(new_n66_));
  nand3  g25(.a(x10), .b(x08), .c(new_n66_), .O(new_n67_));
  inv1   g26(.a(x13), .O(new_n68_));
  nand2  g27(.a(new_n49_), .b(new_n68_), .O(new_n69_));
  nand3  g28(.a(new_n69_), .b(new_n67_), .c(new_n65_), .O(new_n70_));
  inv1   g29(.a(new_n70_), .O(z12));
  nand3  g30(.a(new_n47_), .b(x14), .c(x12), .O(new_n72_));
  aoi21  g31(.a(new_n50_), .b(x01), .c(new_n72_), .O(z13));
  nand2  g32(.a(new_n50_), .b(x02), .O(new_n74_));
  nor2   g33(.a(new_n74_), .b(new_n54_), .O(z14));
  nand2  g34(.a(new_n53_), .b(x16), .O(new_n76_));
  nand3  g35(.a(x10), .b(x08), .c(x03), .O(new_n77_));
  and2   g36(.a(new_n77_), .b(new_n47_), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n76_), .c(new_n46_), .O(z15));
  nand2  g38(.a(new_n50_), .b(x04), .O(new_n80_));
  aoi21  g39(.a(new_n49_), .b(x17), .c(x15), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n46_), .O(z16));
  inv1   g41(.a(x05), .O(new_n83_));
  nand3  g42(.a(x10), .b(x08), .c(new_n83_), .O(new_n84_));
  inv1   g43(.a(x18), .O(new_n85_));
  nand2  g44(.a(new_n49_), .b(new_n85_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n84_), .c(new_n65_), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z17));
  inv1   g47(.a(x06), .O(new_n89_));
  nand3  g48(.a(x10), .b(x08), .c(new_n89_), .O(new_n90_));
  nand2  g49(.a(new_n49_), .b(new_n57_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n90_), .c(new_n65_), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z18));
  inv1   g52(.a(x07), .O(new_n94_));
  nand3  g53(.a(x10), .b(x08), .c(new_n94_), .O(new_n95_));
  inv1   g54(.a(x20), .O(new_n96_));
  nand2  g55(.a(new_n49_), .b(new_n96_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n95_), .c(new_n65_), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z19));
  buf    g58(.a(x15), .O(z06));
endmodule


