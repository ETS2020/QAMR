// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x35), .O(new_n53_));
  inv1   g01(.a(x19), .O(new_n54_));
  nor2   g02(.a(new_n54_), .b(x17), .O(new_n55_));
  nand2  g03(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g04(.a(x21), .b(x18), .O(new_n57_));
  oai21  g05(.a(x18), .b(x03), .c(new_n57_), .O(new_n58_));
  oai22  g06(.a(new_n58_), .b(new_n56_), .c(x20), .d(x19), .O(z00));
  nand2  g07(.a(x22), .b(x18), .O(new_n60_));
  oai21  g08(.a(x18), .b(x02), .c(new_n60_), .O(new_n61_));
  oai22  g09(.a(new_n61_), .b(new_n56_), .c(x21), .d(x19), .O(z01));
  nand2  g10(.a(x23), .b(x18), .O(new_n63_));
  inv1   g11(.a(x01), .O(new_n64_));
  inv1   g12(.a(x18), .O(new_n65_));
  nand2  g13(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g14(.a(new_n66_), .b(new_n63_), .c(new_n55_), .O(new_n67_));
  inv1   g15(.a(x22), .O(new_n68_));
  nand2  g16(.a(new_n68_), .b(new_n54_), .O(new_n69_));
  nand2  g17(.a(x35), .b(x19), .O(new_n70_));
  nand3  g18(.a(new_n70_), .b(new_n69_), .c(new_n67_), .O(z02));
  inv1   g19(.a(x00), .O(new_n72_));
  nand2  g20(.a(new_n65_), .b(new_n72_), .O(new_n73_));
  oai21  g21(.a(new_n65_), .b(x16), .c(new_n73_), .O(new_n74_));
  oai22  g22(.a(new_n74_), .b(new_n56_), .c(x23), .d(x19), .O(z03));
  nand2  g23(.a(x25), .b(x18), .O(new_n76_));
  oai21  g24(.a(x18), .b(x07), .c(new_n76_), .O(new_n77_));
  oai22  g25(.a(new_n77_), .b(new_n56_), .c(x24), .d(x19), .O(z04));
  nand2  g26(.a(x26), .b(x18), .O(new_n79_));
  inv1   g27(.a(x06), .O(new_n80_));
  nand2  g28(.a(new_n65_), .b(new_n80_), .O(new_n81_));
  nand3  g29(.a(new_n81_), .b(new_n79_), .c(new_n55_), .O(new_n82_));
  inv1   g30(.a(x25), .O(new_n83_));
  nand2  g31(.a(new_n83_), .b(new_n54_), .O(new_n84_));
  nand3  g32(.a(new_n84_), .b(new_n82_), .c(new_n70_), .O(z05));
  nand2  g33(.a(x27), .b(x18), .O(new_n86_));
  inv1   g34(.a(x05), .O(new_n87_));
  nand2  g35(.a(new_n65_), .b(new_n87_), .O(new_n88_));
  nand3  g36(.a(new_n88_), .b(new_n86_), .c(new_n55_), .O(new_n89_));
  inv1   g37(.a(x26), .O(new_n90_));
  nand2  g38(.a(new_n90_), .b(new_n54_), .O(new_n91_));
  nand3  g39(.a(new_n91_), .b(new_n89_), .c(new_n70_), .O(z06));
  nand2  g40(.a(x20), .b(x18), .O(new_n93_));
  oai21  g41(.a(x18), .b(x04), .c(new_n93_), .O(new_n94_));
  oai22  g42(.a(new_n94_), .b(new_n56_), .c(x27), .d(x19), .O(z07));
  nand2  g43(.a(x29), .b(x18), .O(new_n96_));
  oai21  g44(.a(x18), .b(x11), .c(new_n96_), .O(new_n97_));
  oai22  g45(.a(new_n97_), .b(new_n56_), .c(x28), .d(x19), .O(z08));
  nand2  g46(.a(x30), .b(x18), .O(new_n99_));
  inv1   g47(.a(x10), .O(new_n100_));
  nand2  g48(.a(new_n65_), .b(new_n100_), .O(new_n101_));
  nand3  g49(.a(new_n101_), .b(new_n99_), .c(new_n55_), .O(new_n102_));
  inv1   g50(.a(x29), .O(new_n103_));
  nand2  g51(.a(new_n103_), .b(new_n54_), .O(new_n104_));
  nand3  g52(.a(new_n104_), .b(new_n102_), .c(new_n70_), .O(z09));
  nand2  g53(.a(x31), .b(x18), .O(new_n106_));
  oai21  g54(.a(x18), .b(x09), .c(new_n106_), .O(new_n107_));
  oai22  g55(.a(new_n107_), .b(new_n56_), .c(x30), .d(x19), .O(z10));
  nand2  g56(.a(x24), .b(x18), .O(new_n109_));
  oai21  g57(.a(x18), .b(x08), .c(new_n109_), .O(new_n110_));
  oai22  g58(.a(new_n110_), .b(new_n56_), .c(x31), .d(x19), .O(z11));
  nand2  g59(.a(x33), .b(x18), .O(new_n112_));
  inv1   g60(.a(x15), .O(new_n113_));
  nand2  g61(.a(new_n65_), .b(new_n113_), .O(new_n114_));
  nand3  g62(.a(new_n114_), .b(new_n112_), .c(new_n55_), .O(new_n115_));
  inv1   g63(.a(x32), .O(new_n116_));
  nand2  g64(.a(new_n116_), .b(new_n54_), .O(new_n117_));
  nand3  g65(.a(new_n117_), .b(new_n115_), .c(new_n70_), .O(z12));
  nand2  g66(.a(x34), .b(x18), .O(new_n119_));
  oai21  g67(.a(x18), .b(x14), .c(new_n119_), .O(new_n120_));
  oai22  g68(.a(new_n120_), .b(new_n56_), .c(x33), .d(x19), .O(z13));
  nor2   g69(.a(x18), .b(x13), .O(new_n122_));
  oai22  g70(.a(new_n122_), .b(new_n56_), .c(x34), .d(x19), .O(z14));
  nand2  g71(.a(x28), .b(x18), .O(new_n124_));
  inv1   g72(.a(x12), .O(new_n125_));
  aoi21  g73(.a(new_n65_), .b(new_n125_), .c(x17), .O(new_n126_));
  aoi21  g74(.a(new_n126_), .b(new_n124_), .c(new_n54_), .O(new_n127_));
  oai21  g75(.a(new_n127_), .b(x35), .c(new_n70_), .O(z15));
endmodule


