// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_;
  nand2  g00(.a(x21), .b(x18), .O(new_n53_));
  inv1   g01(.a(x19), .O(new_n54_));
  nor2   g02(.a(new_n54_), .b(x17), .O(new_n55_));
  inv1   g03(.a(x03), .O(new_n56_));
  inv1   g04(.a(x18), .O(new_n57_));
  nand2  g05(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g06(.a(new_n58_), .b(new_n55_), .c(new_n53_), .O(new_n59_));
  nand2  g07(.a(new_n54_), .b(x18), .O(new_n60_));
  oai21  g08(.a(new_n60_), .b(x20), .c(new_n59_), .O(z00));
  nand2  g09(.a(x22), .b(x18), .O(new_n62_));
  inv1   g10(.a(x02), .O(new_n63_));
  nand2  g11(.a(new_n57_), .b(new_n63_), .O(new_n64_));
  nand3  g12(.a(new_n64_), .b(new_n62_), .c(new_n55_), .O(new_n65_));
  oai21  g13(.a(new_n60_), .b(x21), .c(new_n65_), .O(z01));
  inv1   g14(.a(x22), .O(new_n67_));
  nor2   g15(.a(x19), .b(new_n57_), .O(new_n68_));
  nand2  g16(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nor2   g17(.a(x18), .b(x01), .O(new_n70_));
  nand2  g18(.a(x23), .b(x18), .O(new_n71_));
  nand2  g19(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  oai21  g20(.a(new_n72_), .b(new_n70_), .c(new_n69_), .O(z02));
  inv1   g21(.a(x17), .O(new_n74_));
  inv1   g22(.a(x00), .O(new_n75_));
  inv1   g23(.a(x16), .O(new_n76_));
  nand2  g24(.a(x19), .b(x18), .O(new_n77_));
  oai22  g25(.a(new_n77_), .b(new_n76_), .c(x18), .d(new_n75_), .O(new_n78_));
  nand2  g26(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand2  g27(.a(new_n71_), .b(new_n54_), .O(new_n80_));
  nand2  g28(.a(new_n80_), .b(new_n79_), .O(z03));
  nand2  g29(.a(x25), .b(x18), .O(new_n82_));
  inv1   g30(.a(x07), .O(new_n83_));
  nand2  g31(.a(new_n57_), .b(new_n83_), .O(new_n84_));
  nand3  g32(.a(new_n84_), .b(new_n82_), .c(new_n55_), .O(new_n85_));
  oai21  g33(.a(new_n60_), .b(x24), .c(new_n85_), .O(z04));
  nand2  g34(.a(x26), .b(x18), .O(new_n87_));
  inv1   g35(.a(x06), .O(new_n88_));
  nand2  g36(.a(new_n57_), .b(new_n88_), .O(new_n89_));
  nand3  g37(.a(new_n89_), .b(new_n87_), .c(new_n55_), .O(new_n90_));
  oai21  g38(.a(new_n60_), .b(x25), .c(new_n90_), .O(z05));
  nand2  g39(.a(x27), .b(x18), .O(new_n92_));
  inv1   g40(.a(x05), .O(new_n93_));
  nand2  g41(.a(new_n57_), .b(new_n93_), .O(new_n94_));
  nand3  g42(.a(new_n94_), .b(new_n92_), .c(new_n55_), .O(new_n95_));
  oai21  g43(.a(new_n60_), .b(x26), .c(new_n95_), .O(z06));
  nand2  g44(.a(x20), .b(x18), .O(new_n97_));
  inv1   g45(.a(x04), .O(new_n98_));
  nand2  g46(.a(new_n57_), .b(new_n98_), .O(new_n99_));
  nand3  g47(.a(new_n99_), .b(new_n97_), .c(new_n55_), .O(new_n100_));
  oai21  g48(.a(new_n60_), .b(x27), .c(new_n100_), .O(z07));
  inv1   g49(.a(x11), .O(new_n102_));
  oai22  g50(.a(new_n77_), .b(x29), .c(x18), .d(new_n102_), .O(new_n103_));
  nand2  g51(.a(new_n103_), .b(new_n74_), .O(new_n104_));
  nand2  g52(.a(x28), .b(x18), .O(new_n105_));
  nand2  g53(.a(new_n105_), .b(new_n54_), .O(new_n106_));
  nand2  g54(.a(new_n106_), .b(new_n104_), .O(z08));
  inv1   g55(.a(x29), .O(new_n108_));
  nand2  g56(.a(new_n68_), .b(new_n108_), .O(new_n109_));
  nor2   g57(.a(x18), .b(x10), .O(new_n110_));
  nand2  g58(.a(x30), .b(x18), .O(new_n111_));
  nand2  g59(.a(new_n111_), .b(new_n55_), .O(new_n112_));
  oai21  g60(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(z09));
  nand2  g61(.a(new_n111_), .b(new_n54_), .O(new_n114_));
  nand2  g62(.a(new_n60_), .b(new_n74_), .O(new_n115_));
  nand2  g63(.a(x31), .b(x18), .O(new_n116_));
  oai21  g64(.a(x18), .b(x09), .c(new_n116_), .O(new_n117_));
  oai21  g65(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z10));
  inv1   g66(.a(x08), .O(new_n119_));
  oai22  g67(.a(new_n77_), .b(x24), .c(x18), .d(new_n119_), .O(new_n120_));
  nand2  g68(.a(new_n120_), .b(new_n74_), .O(new_n121_));
  nand2  g69(.a(new_n116_), .b(new_n54_), .O(new_n122_));
  nand2  g70(.a(new_n122_), .b(new_n121_), .O(z11));
  nand2  g71(.a(x33), .b(x18), .O(new_n124_));
  inv1   g72(.a(x15), .O(new_n125_));
  nand2  g73(.a(new_n57_), .b(new_n125_), .O(new_n126_));
  nand3  g74(.a(new_n126_), .b(new_n124_), .c(new_n55_), .O(new_n127_));
  oai21  g75(.a(new_n60_), .b(x32), .c(new_n127_), .O(z12));
  inv1   g76(.a(x33), .O(new_n129_));
  nand2  g77(.a(new_n68_), .b(new_n129_), .O(new_n130_));
  nor2   g78(.a(x18), .b(x14), .O(new_n131_));
  nand2  g79(.a(x34), .b(x18), .O(new_n132_));
  nand2  g80(.a(new_n132_), .b(new_n55_), .O(new_n133_));
  oai21  g81(.a(new_n133_), .b(new_n131_), .c(new_n130_), .O(z13));
  nand2  g82(.a(new_n132_), .b(new_n54_), .O(new_n135_));
  nand2  g83(.a(x35), .b(x18), .O(new_n136_));
  oai21  g84(.a(x18), .b(x13), .c(new_n136_), .O(new_n137_));
  oai21  g85(.a(new_n137_), .b(new_n115_), .c(new_n135_), .O(z14));
  nand2  g86(.a(new_n136_), .b(new_n54_), .O(new_n139_));
  oai21  g87(.a(x18), .b(x12), .c(new_n105_), .O(new_n140_));
  oai21  g88(.a(new_n140_), .b(new_n115_), .c(new_n139_), .O(z15));
endmodule


