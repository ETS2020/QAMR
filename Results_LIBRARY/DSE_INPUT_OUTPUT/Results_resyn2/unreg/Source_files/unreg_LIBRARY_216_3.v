// Benchmark "FAU" written by ABC on Tue Aug 11 20:01:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n134_, new_n135_;
  nand2  g00(.a(x21), .b(x18), .O(new_n53_));
  inv1   g01(.a(x03), .O(new_n54_));
  inv1   g02(.a(x18), .O(new_n55_));
  nand2  g03(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  inv1   g04(.a(x19), .O(new_n57_));
  nor2   g05(.a(new_n57_), .b(x17), .O(new_n58_));
  nand3  g06(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(new_n59_));
  inv1   g07(.a(x20), .O(new_n60_));
  nand2  g08(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  inv1   g09(.a(x29), .O(new_n62_));
  nor2   g10(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  inv1   g11(.a(new_n63_), .O(new_n64_));
  nand3  g12(.a(new_n64_), .b(new_n61_), .c(new_n59_), .O(z00));
  nand2  g13(.a(x22), .b(x18), .O(new_n66_));
  inv1   g14(.a(x02), .O(new_n67_));
  nand2  g15(.a(new_n55_), .b(new_n67_), .O(new_n68_));
  nand3  g16(.a(new_n68_), .b(new_n66_), .c(new_n58_), .O(new_n69_));
  inv1   g17(.a(x21), .O(new_n70_));
  nand2  g18(.a(new_n70_), .b(new_n57_), .O(new_n71_));
  nand3  g19(.a(new_n71_), .b(new_n69_), .c(new_n64_), .O(z01));
  nand2  g20(.a(new_n58_), .b(new_n62_), .O(new_n73_));
  nand2  g21(.a(x23), .b(x18), .O(new_n74_));
  oai21  g22(.a(x18), .b(x01), .c(new_n74_), .O(new_n75_));
  oai22  g23(.a(new_n75_), .b(new_n73_), .c(x22), .d(x19), .O(z02));
  nor2   g24(.a(new_n55_), .b(x16), .O(new_n77_));
  oai21  g25(.a(x18), .b(x00), .c(new_n58_), .O(new_n78_));
  nor2   g26(.a(x23), .b(x19), .O(new_n79_));
  nor2   g27(.a(new_n79_), .b(new_n63_), .O(new_n80_));
  oai21  g28(.a(new_n78_), .b(new_n77_), .c(new_n80_), .O(z03));
  nand2  g29(.a(x25), .b(x18), .O(new_n82_));
  inv1   g30(.a(x07), .O(new_n83_));
  nand2  g31(.a(new_n55_), .b(new_n83_), .O(new_n84_));
  nand3  g32(.a(new_n84_), .b(new_n82_), .c(new_n58_), .O(new_n85_));
  inv1   g33(.a(x24), .O(new_n86_));
  nand2  g34(.a(new_n86_), .b(new_n57_), .O(new_n87_));
  nand3  g35(.a(new_n87_), .b(new_n85_), .c(new_n64_), .O(z04));
  nand2  g36(.a(x26), .b(x18), .O(new_n89_));
  oai21  g37(.a(x18), .b(x06), .c(new_n89_), .O(new_n90_));
  oai22  g38(.a(new_n90_), .b(new_n73_), .c(x25), .d(x19), .O(z05));
  nand2  g39(.a(x27), .b(x18), .O(new_n92_));
  inv1   g40(.a(x05), .O(new_n93_));
  nand2  g41(.a(new_n55_), .b(new_n93_), .O(new_n94_));
  nand3  g42(.a(new_n94_), .b(new_n92_), .c(new_n58_), .O(new_n95_));
  inv1   g43(.a(x26), .O(new_n96_));
  nand2  g44(.a(new_n96_), .b(new_n57_), .O(new_n97_));
  nand3  g45(.a(new_n97_), .b(new_n95_), .c(new_n64_), .O(z06));
  nand2  g46(.a(x20), .b(x18), .O(new_n99_));
  inv1   g47(.a(x04), .O(new_n100_));
  nand2  g48(.a(new_n55_), .b(new_n100_), .O(new_n101_));
  nand3  g49(.a(new_n101_), .b(new_n99_), .c(new_n58_), .O(new_n102_));
  inv1   g50(.a(x27), .O(new_n103_));
  nand2  g51(.a(new_n103_), .b(new_n57_), .O(new_n104_));
  nand3  g52(.a(new_n104_), .b(new_n102_), .c(new_n64_), .O(z07));
  inv1   g53(.a(x17), .O(new_n106_));
  oai21  g54(.a(x18), .b(x11), .c(new_n106_), .O(new_n107_));
  nor2   g55(.a(x29), .b(new_n57_), .O(new_n108_));
  aoi22  g56(.a(new_n108_), .b(new_n107_), .c(x28), .d(new_n57_), .O(z08));
  nand2  g57(.a(x30), .b(x18), .O(new_n110_));
  inv1   g58(.a(x10), .O(new_n111_));
  aoi21  g59(.a(new_n55_), .b(new_n111_), .c(x17), .O(new_n112_));
  aoi21  g60(.a(new_n112_), .b(new_n110_), .c(new_n57_), .O(new_n113_));
  oai21  g61(.a(new_n113_), .b(x29), .c(new_n64_), .O(z09));
  nand2  g62(.a(x31), .b(x18), .O(new_n115_));
  oai21  g63(.a(x18), .b(x09), .c(new_n115_), .O(new_n116_));
  oai22  g64(.a(new_n116_), .b(new_n73_), .c(x30), .d(x19), .O(z10));
  nand2  g65(.a(x24), .b(x18), .O(new_n118_));
  oai21  g66(.a(x18), .b(x08), .c(new_n118_), .O(new_n119_));
  oai22  g67(.a(new_n119_), .b(new_n73_), .c(x31), .d(x19), .O(z11));
  nand2  g68(.a(x33), .b(x18), .O(new_n121_));
  oai21  g69(.a(x18), .b(x15), .c(new_n121_), .O(new_n122_));
  oai22  g70(.a(new_n122_), .b(new_n73_), .c(x32), .d(x19), .O(z12));
  nand2  g71(.a(x34), .b(x18), .O(new_n124_));
  inv1   g72(.a(x14), .O(new_n125_));
  nand2  g73(.a(new_n55_), .b(new_n125_), .O(new_n126_));
  nand3  g74(.a(new_n126_), .b(new_n124_), .c(new_n58_), .O(new_n127_));
  inv1   g75(.a(x33), .O(new_n128_));
  nand2  g76(.a(new_n128_), .b(new_n57_), .O(new_n129_));
  nand3  g77(.a(new_n129_), .b(new_n127_), .c(new_n64_), .O(z13));
  nand2  g78(.a(x35), .b(x18), .O(new_n131_));
  oai21  g79(.a(x18), .b(x13), .c(new_n131_), .O(new_n132_));
  oai22  g80(.a(new_n132_), .b(new_n73_), .c(x34), .d(x19), .O(z14));
  nand2  g81(.a(x28), .b(x18), .O(new_n134_));
  oai21  g82(.a(x18), .b(x12), .c(new_n134_), .O(new_n135_));
  oai22  g83(.a(new_n135_), .b(new_n73_), .c(x35), .d(x19), .O(z15));
endmodule


