// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n140_, new_n141_;
  inv1   g00(.a(x17), .O(new_n53_));
  inv1   g01(.a(x03), .O(new_n54_));
  nand2  g02(.a(x19), .b(x18), .O(new_n55_));
  oai22  g03(.a(new_n55_), .b(x21), .c(x18), .d(new_n54_), .O(new_n56_));
  nand2  g04(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g05(.a(x19), .O(new_n58_));
  nand2  g06(.a(x20), .b(x18), .O(new_n59_));
  nand2  g07(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g08(.a(new_n60_), .b(new_n57_), .O(z00));
  nand2  g09(.a(new_n58_), .b(x18), .O(new_n62_));
  nand2  g10(.a(x22), .b(x18), .O(new_n63_));
  nor2   g11(.a(new_n58_), .b(x17), .O(new_n64_));
  inv1   g12(.a(x02), .O(new_n65_));
  inv1   g13(.a(x18), .O(new_n66_));
  nand2  g14(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g15(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(new_n68_));
  oai21  g16(.a(new_n62_), .b(x21), .c(new_n68_), .O(z01));
  nand2  g17(.a(x23), .b(x18), .O(new_n70_));
  inv1   g18(.a(x01), .O(new_n71_));
  nand2  g19(.a(new_n66_), .b(new_n71_), .O(new_n72_));
  nand3  g20(.a(new_n72_), .b(new_n70_), .c(new_n64_), .O(new_n73_));
  oai21  g21(.a(new_n62_), .b(x22), .c(new_n73_), .O(z02));
  inv1   g22(.a(x16), .O(new_n75_));
  nand2  g23(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g24(.a(x00), .O(new_n77_));
  nand2  g25(.a(new_n66_), .b(new_n77_), .O(new_n78_));
  nand3  g26(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(new_n79_));
  oai21  g27(.a(new_n62_), .b(x23), .c(new_n79_), .O(z03));
  nand2  g28(.a(x25), .b(x18), .O(new_n81_));
  inv1   g29(.a(x07), .O(new_n82_));
  nand2  g30(.a(new_n66_), .b(new_n82_), .O(new_n83_));
  nand3  g31(.a(new_n83_), .b(new_n81_), .c(new_n64_), .O(new_n84_));
  oai21  g32(.a(new_n62_), .b(x24), .c(new_n84_), .O(z04));
  nand2  g33(.a(x26), .b(x18), .O(new_n86_));
  inv1   g34(.a(x06), .O(new_n87_));
  nand2  g35(.a(new_n66_), .b(new_n87_), .O(new_n88_));
  nand3  g36(.a(new_n88_), .b(new_n86_), .c(new_n64_), .O(new_n89_));
  oai21  g37(.a(new_n62_), .b(x25), .c(new_n89_), .O(z05));
  nand2  g38(.a(x27), .b(x18), .O(new_n91_));
  inv1   g39(.a(x05), .O(new_n92_));
  nand2  g40(.a(new_n66_), .b(new_n92_), .O(new_n93_));
  nand3  g41(.a(new_n93_), .b(new_n91_), .c(new_n64_), .O(new_n94_));
  oai21  g42(.a(new_n62_), .b(x26), .c(new_n94_), .O(z06));
  inv1   g43(.a(x27), .O(new_n96_));
  nor2   g44(.a(x19), .b(new_n66_), .O(new_n97_));
  nand2  g45(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nor2   g46(.a(x18), .b(x04), .O(new_n99_));
  nand2  g47(.a(new_n64_), .b(new_n59_), .O(new_n100_));
  oai21  g48(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z07));
  nand2  g49(.a(x28), .b(x18), .O(new_n102_));
  nand2  g50(.a(new_n102_), .b(new_n58_), .O(new_n103_));
  nand2  g51(.a(new_n62_), .b(new_n53_), .O(new_n104_));
  nand2  g52(.a(x29), .b(x18), .O(new_n105_));
  oai21  g53(.a(x18), .b(x11), .c(new_n105_), .O(new_n106_));
  oai21  g54(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(z08));
  inv1   g55(.a(x10), .O(new_n108_));
  oai22  g56(.a(new_n55_), .b(x30), .c(x18), .d(new_n108_), .O(new_n109_));
  nand2  g57(.a(new_n109_), .b(new_n53_), .O(new_n110_));
  nand2  g58(.a(new_n105_), .b(new_n58_), .O(new_n111_));
  nand2  g59(.a(new_n111_), .b(new_n110_), .O(z09));
  inv1   g60(.a(x30), .O(new_n113_));
  nand2  g61(.a(new_n97_), .b(new_n113_), .O(new_n114_));
  nor2   g62(.a(x18), .b(x09), .O(new_n115_));
  nand2  g63(.a(x31), .b(x18), .O(new_n116_));
  nand2  g64(.a(new_n116_), .b(new_n64_), .O(new_n117_));
  oai21  g65(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z10));
  inv1   g66(.a(x08), .O(new_n119_));
  oai22  g67(.a(new_n55_), .b(x24), .c(x18), .d(new_n119_), .O(new_n120_));
  nand2  g68(.a(new_n120_), .b(new_n53_), .O(new_n121_));
  nand2  g69(.a(new_n116_), .b(new_n58_), .O(new_n122_));
  nand2  g70(.a(new_n122_), .b(new_n121_), .O(z11));
  nand2  g71(.a(x33), .b(x18), .O(new_n124_));
  inv1   g72(.a(x15), .O(new_n125_));
  nand2  g73(.a(new_n66_), .b(new_n125_), .O(new_n126_));
  nand3  g74(.a(new_n126_), .b(new_n124_), .c(new_n64_), .O(new_n127_));
  oai21  g75(.a(new_n62_), .b(x32), .c(new_n127_), .O(z12));
  nand2  g76(.a(x34), .b(x18), .O(new_n129_));
  inv1   g77(.a(x14), .O(new_n130_));
  nand2  g78(.a(new_n66_), .b(new_n130_), .O(new_n131_));
  nand3  g79(.a(new_n131_), .b(new_n129_), .c(new_n64_), .O(new_n132_));
  oai21  g80(.a(new_n62_), .b(x33), .c(new_n132_), .O(z13));
  inv1   g81(.a(x34), .O(new_n134_));
  nand2  g82(.a(new_n97_), .b(new_n134_), .O(new_n135_));
  nor2   g83(.a(x18), .b(x13), .O(new_n136_));
  nand2  g84(.a(x35), .b(x18), .O(new_n137_));
  nand2  g85(.a(new_n137_), .b(new_n64_), .O(new_n138_));
  oai21  g86(.a(new_n138_), .b(new_n136_), .c(new_n135_), .O(z14));
  nand2  g87(.a(new_n137_), .b(new_n58_), .O(new_n140_));
  oai21  g88(.a(x18), .b(x12), .c(new_n102_), .O(new_n141_));
  oai21  g89(.a(new_n141_), .b(new_n104_), .c(new_n140_), .O(z15));
endmodule


