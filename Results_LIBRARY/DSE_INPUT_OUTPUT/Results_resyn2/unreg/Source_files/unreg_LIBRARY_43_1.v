// Benchmark "FAU" written by ABC on Tue Aug 11 19:59:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n153_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x28), .O(new_n54_));
  and2   g002(.a(x21), .b(x18), .O(new_n55_));
  oai21  g003(.a(x18), .b(x03), .c(x19), .O(new_n56_));
  oai21  g004(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g005(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  inv1   g006(.a(x19), .O(new_n59_));
  inv1   g007(.a(x20), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g009(.a(new_n61_), .b(new_n58_), .O(z00));
  nand3  g010(.a(new_n54_), .b(x19), .c(new_n53_), .O(new_n63_));
  nand2  g011(.a(x22), .b(x18), .O(new_n64_));
  oai21  g012(.a(x18), .b(x02), .c(new_n64_), .O(new_n65_));
  nand2  g013(.a(x28), .b(new_n53_), .O(new_n66_));
  nor2   g014(.a(x21), .b(x19), .O(new_n67_));
  nand2  g015(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g016(.a(new_n65_), .b(new_n63_), .c(new_n68_), .O(z01));
  and2   g017(.a(x23), .b(x18), .O(new_n70_));
  oai21  g018(.a(x18), .b(x01), .c(x19), .O(new_n71_));
  oai21  g019(.a(new_n71_), .b(new_n70_), .c(new_n54_), .O(new_n72_));
  nand2  g020(.a(new_n72_), .b(new_n53_), .O(new_n73_));
  inv1   g021(.a(x22), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(new_n59_), .O(new_n75_));
  nand2  g023(.a(new_n75_), .b(new_n73_), .O(z02));
  inv1   g024(.a(x16), .O(new_n77_));
  nand2  g025(.a(x18), .b(new_n77_), .O(new_n78_));
  inv1   g026(.a(x00), .O(new_n79_));
  inv1   g027(.a(x18), .O(new_n80_));
  aoi21  g028(.a(new_n80_), .b(new_n79_), .c(new_n59_), .O(new_n81_));
  aoi21  g029(.a(new_n81_), .b(new_n78_), .c(x28), .O(new_n82_));
  inv1   g030(.a(x23), .O(new_n83_));
  nand2  g031(.a(new_n83_), .b(new_n59_), .O(new_n84_));
  oai21  g032(.a(new_n82_), .b(x17), .c(new_n84_), .O(z03));
  and2   g033(.a(x25), .b(x18), .O(new_n86_));
  oai21  g034(.a(x18), .b(x07), .c(x19), .O(new_n87_));
  oai21  g035(.a(new_n87_), .b(new_n86_), .c(new_n54_), .O(new_n88_));
  nand2  g036(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  inv1   g037(.a(x24), .O(new_n90_));
  nand2  g038(.a(new_n90_), .b(new_n59_), .O(new_n91_));
  nand2  g039(.a(new_n91_), .b(new_n89_), .O(z04));
  and2   g040(.a(x26), .b(x18), .O(new_n93_));
  oai21  g041(.a(x18), .b(x06), .c(x19), .O(new_n94_));
  oai21  g042(.a(new_n94_), .b(new_n93_), .c(new_n54_), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  inv1   g044(.a(x25), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(new_n59_), .O(new_n98_));
  nand2  g046(.a(new_n98_), .b(new_n96_), .O(z05));
  nand2  g047(.a(x27), .b(x18), .O(new_n100_));
  oai21  g048(.a(x18), .b(x05), .c(new_n100_), .O(new_n101_));
  nor2   g049(.a(x26), .b(x19), .O(new_n102_));
  nand2  g050(.a(new_n102_), .b(new_n66_), .O(new_n103_));
  oai21  g051(.a(new_n101_), .b(new_n63_), .c(new_n103_), .O(z06));
  nand2  g052(.a(x20), .b(x18), .O(new_n105_));
  oai21  g053(.a(x18), .b(x04), .c(new_n105_), .O(new_n106_));
  nor2   g054(.a(x27), .b(x19), .O(new_n107_));
  nand2  g055(.a(new_n107_), .b(new_n66_), .O(new_n108_));
  oai21  g056(.a(new_n106_), .b(new_n63_), .c(new_n108_), .O(z07));
  nand2  g057(.a(new_n80_), .b(x11), .O(new_n110_));
  inv1   g058(.a(x29), .O(new_n111_));
  nand2  g059(.a(new_n111_), .b(x18), .O(new_n112_));
  nand3  g060(.a(new_n112_), .b(new_n110_), .c(new_n54_), .O(new_n113_));
  nand2  g061(.a(new_n113_), .b(new_n53_), .O(new_n114_));
  oai21  g062(.a(x28), .b(x19), .c(new_n114_), .O(z08));
  and2   g063(.a(x30), .b(x18), .O(new_n116_));
  oai21  g064(.a(x18), .b(x10), .c(x19), .O(new_n117_));
  oai21  g065(.a(new_n117_), .b(new_n116_), .c(new_n54_), .O(new_n118_));
  nand2  g066(.a(new_n118_), .b(new_n53_), .O(new_n119_));
  nand2  g067(.a(new_n111_), .b(new_n59_), .O(new_n120_));
  nand2  g068(.a(new_n120_), .b(new_n119_), .O(z09));
  and2   g069(.a(x31), .b(x18), .O(new_n122_));
  oai21  g070(.a(x18), .b(x09), .c(x19), .O(new_n123_));
  oai21  g071(.a(new_n123_), .b(new_n122_), .c(new_n54_), .O(new_n124_));
  nand2  g072(.a(new_n124_), .b(new_n53_), .O(new_n125_));
  inv1   g073(.a(x30), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n59_), .O(new_n127_));
  nand2  g075(.a(new_n127_), .b(new_n125_), .O(z10));
  nand2  g076(.a(x24), .b(x18), .O(new_n129_));
  oai21  g077(.a(x18), .b(x08), .c(new_n129_), .O(new_n130_));
  nor2   g078(.a(x31), .b(x19), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(new_n66_), .O(new_n132_));
  oai21  g080(.a(new_n130_), .b(new_n63_), .c(new_n132_), .O(z11));
  nand2  g081(.a(x33), .b(x18), .O(new_n134_));
  oai21  g082(.a(x18), .b(x15), .c(new_n134_), .O(new_n135_));
  nor2   g083(.a(x32), .b(x19), .O(new_n136_));
  nand2  g084(.a(new_n136_), .b(new_n66_), .O(new_n137_));
  oai21  g085(.a(new_n135_), .b(new_n63_), .c(new_n137_), .O(z12));
  and2   g086(.a(x34), .b(x18), .O(new_n139_));
  oai21  g087(.a(x18), .b(x14), .c(x19), .O(new_n140_));
  oai21  g088(.a(new_n140_), .b(new_n139_), .c(new_n54_), .O(new_n141_));
  nand2  g089(.a(new_n141_), .b(new_n53_), .O(new_n142_));
  inv1   g090(.a(x33), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n59_), .O(new_n144_));
  nand2  g092(.a(new_n144_), .b(new_n142_), .O(z13));
  nand2  g093(.a(x35), .b(x18), .O(new_n146_));
  oai21  g094(.a(x18), .b(x13), .c(new_n146_), .O(new_n147_));
  nor2   g095(.a(x34), .b(x19), .O(new_n148_));
  nand2  g096(.a(new_n148_), .b(new_n66_), .O(new_n149_));
  oai21  g097(.a(new_n147_), .b(new_n63_), .c(new_n149_), .O(z14));
  oai21  g098(.a(x18), .b(x12), .c(x19), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n54_), .O(new_n152_));
  nand2  g100(.a(new_n152_), .b(new_n53_), .O(new_n153_));
  oai21  g101(.a(x35), .b(x19), .c(new_n153_), .O(z15));
endmodule


