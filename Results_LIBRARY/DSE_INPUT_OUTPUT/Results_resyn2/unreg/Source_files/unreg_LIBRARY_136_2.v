// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_;
  inv1   g000(.a(x18), .O(new_n53_));
  inv1   g001(.a(x19), .O(new_n54_));
  nand2  g002(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g003(.a(x21), .b(x18), .O(new_n56_));
  nor2   g004(.a(new_n54_), .b(x17), .O(new_n57_));
  inv1   g005(.a(x03), .O(new_n58_));
  nand2  g006(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  nand3  g007(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  oai21  g008(.a(new_n55_), .b(x20), .c(new_n60_), .O(z00));
  inv1   g009(.a(x17), .O(new_n62_));
  nand3  g010(.a(x19), .b(new_n53_), .c(x02), .O(new_n63_));
  inv1   g011(.a(x22), .O(new_n64_));
  nand2  g012(.a(new_n64_), .b(x18), .O(new_n65_));
  nand2  g013(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g014(.a(new_n66_), .b(new_n62_), .O(new_n67_));
  inv1   g015(.a(x21), .O(new_n68_));
  oai21  g016(.a(new_n68_), .b(x18), .c(new_n54_), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n67_), .O(z01));
  nand3  g018(.a(x19), .b(new_n53_), .c(x01), .O(new_n71_));
  inv1   g019(.a(x23), .O(new_n72_));
  nand2  g020(.a(new_n72_), .b(x18), .O(new_n73_));
  nand2  g021(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand2  g022(.a(new_n74_), .b(new_n62_), .O(new_n75_));
  oai21  g023(.a(new_n64_), .b(x18), .c(new_n54_), .O(new_n76_));
  nand2  g024(.a(new_n76_), .b(new_n75_), .O(z02));
  inv1   g025(.a(x16), .O(new_n78_));
  nand2  g026(.a(x18), .b(new_n78_), .O(new_n79_));
  inv1   g027(.a(x00), .O(new_n80_));
  nand2  g028(.a(new_n53_), .b(new_n80_), .O(new_n81_));
  nand3  g029(.a(new_n81_), .b(new_n79_), .c(new_n57_), .O(new_n82_));
  oai21  g030(.a(new_n55_), .b(x23), .c(new_n82_), .O(z03));
  nand3  g031(.a(x19), .b(new_n53_), .c(x07), .O(new_n84_));
  inv1   g032(.a(x25), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(x18), .O(new_n86_));
  nand2  g034(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand2  g035(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  inv1   g036(.a(x24), .O(new_n89_));
  oai21  g037(.a(new_n89_), .b(x18), .c(new_n54_), .O(new_n90_));
  nand2  g038(.a(new_n90_), .b(new_n88_), .O(z04));
  nand3  g039(.a(x19), .b(new_n53_), .c(x06), .O(new_n92_));
  inv1   g040(.a(x26), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(x18), .O(new_n94_));
  nand2  g042(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n62_), .O(new_n96_));
  oai21  g044(.a(new_n85_), .b(x18), .c(new_n54_), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(new_n96_), .O(z05));
  nand2  g046(.a(x27), .b(x18), .O(new_n99_));
  inv1   g047(.a(x05), .O(new_n100_));
  nand2  g048(.a(new_n53_), .b(new_n100_), .O(new_n101_));
  nand3  g049(.a(new_n101_), .b(new_n99_), .c(new_n57_), .O(new_n102_));
  oai21  g050(.a(new_n55_), .b(x26), .c(new_n102_), .O(z06));
  nand2  g051(.a(x20), .b(x18), .O(new_n104_));
  inv1   g052(.a(x04), .O(new_n105_));
  nand2  g053(.a(new_n53_), .b(new_n105_), .O(new_n106_));
  nand3  g054(.a(new_n106_), .b(new_n104_), .c(new_n57_), .O(new_n107_));
  oai21  g055(.a(new_n55_), .b(x27), .c(new_n107_), .O(z07));
  nand2  g056(.a(x29), .b(x18), .O(new_n109_));
  inv1   g057(.a(x11), .O(new_n110_));
  nand2  g058(.a(new_n53_), .b(new_n110_), .O(new_n111_));
  nand3  g059(.a(new_n111_), .b(new_n109_), .c(new_n57_), .O(new_n112_));
  oai21  g060(.a(new_n55_), .b(x28), .c(new_n112_), .O(z08));
  nand2  g061(.a(x30), .b(x18), .O(new_n114_));
  inv1   g062(.a(x10), .O(new_n115_));
  nand2  g063(.a(new_n53_), .b(new_n115_), .O(new_n116_));
  nand3  g064(.a(new_n116_), .b(new_n114_), .c(new_n57_), .O(new_n117_));
  oai21  g065(.a(new_n55_), .b(x29), .c(new_n117_), .O(z09));
  nand2  g066(.a(x31), .b(x18), .O(new_n119_));
  inv1   g067(.a(x09), .O(new_n120_));
  nand2  g068(.a(new_n53_), .b(new_n120_), .O(new_n121_));
  nand3  g069(.a(new_n121_), .b(new_n119_), .c(new_n57_), .O(new_n122_));
  oai21  g070(.a(new_n55_), .b(x30), .c(new_n122_), .O(z10));
  nand3  g071(.a(x19), .b(new_n53_), .c(x08), .O(new_n124_));
  nand2  g072(.a(new_n89_), .b(x18), .O(new_n125_));
  nand2  g073(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n62_), .O(new_n127_));
  inv1   g075(.a(x31), .O(new_n128_));
  oai21  g076(.a(new_n128_), .b(x18), .c(new_n54_), .O(new_n129_));
  nand2  g077(.a(new_n129_), .b(new_n127_), .O(z11));
  nand3  g078(.a(x19), .b(new_n53_), .c(x15), .O(new_n131_));
  inv1   g079(.a(x33), .O(new_n132_));
  nand2  g080(.a(new_n132_), .b(x18), .O(new_n133_));
  nand2  g081(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n62_), .O(new_n135_));
  inv1   g083(.a(x32), .O(new_n136_));
  oai21  g084(.a(new_n136_), .b(x18), .c(new_n54_), .O(new_n137_));
  nand2  g085(.a(new_n137_), .b(new_n135_), .O(z12));
  nand2  g086(.a(x34), .b(x18), .O(new_n139_));
  inv1   g087(.a(x14), .O(new_n140_));
  nand2  g088(.a(new_n53_), .b(new_n140_), .O(new_n141_));
  nand3  g089(.a(new_n141_), .b(new_n139_), .c(new_n57_), .O(new_n142_));
  oai21  g090(.a(new_n55_), .b(x33), .c(new_n142_), .O(z13));
  nand2  g091(.a(x35), .b(x18), .O(new_n144_));
  inv1   g092(.a(x13), .O(new_n145_));
  nand2  g093(.a(new_n53_), .b(new_n145_), .O(new_n146_));
  nand3  g094(.a(new_n146_), .b(new_n144_), .c(new_n57_), .O(new_n147_));
  oai21  g095(.a(new_n55_), .b(x34), .c(new_n147_), .O(z14));
  nand3  g096(.a(x19), .b(new_n53_), .c(x12), .O(new_n149_));
  inv1   g097(.a(x28), .O(new_n150_));
  nand2  g098(.a(new_n150_), .b(x18), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand2  g100(.a(new_n152_), .b(new_n62_), .O(new_n153_));
  inv1   g101(.a(x35), .O(new_n154_));
  oai21  g102(.a(new_n154_), .b(x18), .c(new_n54_), .O(new_n155_));
  nand2  g103(.a(new_n155_), .b(new_n153_), .O(z15));
endmodule


