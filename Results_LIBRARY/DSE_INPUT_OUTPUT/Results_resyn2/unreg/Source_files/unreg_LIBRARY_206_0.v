// Benchmark "FAU" written by ABC on Tue Aug 11 20:01:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_;
  inv1   g00(.a(x19), .O(new_n53_));
  nand2  g01(.a(x20), .b(x18), .O(new_n54_));
  nand2  g02(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g03(.a(x17), .O(new_n56_));
  nand2  g04(.a(new_n53_), .b(x18), .O(new_n57_));
  nand2  g05(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g06(.a(x21), .b(x18), .O(new_n59_));
  oai21  g07(.a(x18), .b(x03), .c(new_n59_), .O(new_n60_));
  oai21  g08(.a(new_n60_), .b(new_n58_), .c(new_n55_), .O(z00));
  nand2  g09(.a(new_n59_), .b(new_n53_), .O(new_n62_));
  nand2  g10(.a(x22), .b(x18), .O(new_n63_));
  oai21  g11(.a(x18), .b(x02), .c(new_n63_), .O(new_n64_));
  oai21  g12(.a(new_n64_), .b(new_n58_), .c(new_n62_), .O(z01));
  inv1   g13(.a(x01), .O(new_n66_));
  nand2  g14(.a(x19), .b(x18), .O(new_n67_));
  oai22  g15(.a(new_n67_), .b(x23), .c(x18), .d(new_n66_), .O(new_n68_));
  nand2  g16(.a(new_n68_), .b(new_n56_), .O(new_n69_));
  nand2  g17(.a(new_n63_), .b(new_n53_), .O(new_n70_));
  nand2  g18(.a(new_n70_), .b(new_n69_), .O(z02));
  inv1   g19(.a(x16), .O(new_n72_));
  nand2  g20(.a(x18), .b(new_n72_), .O(new_n73_));
  nor2   g21(.a(new_n53_), .b(x17), .O(new_n74_));
  inv1   g22(.a(x00), .O(new_n75_));
  inv1   g23(.a(x18), .O(new_n76_));
  nand2  g24(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g25(.a(new_n77_), .b(new_n74_), .c(new_n73_), .O(new_n78_));
  oai21  g26(.a(new_n57_), .b(x23), .c(new_n78_), .O(z03));
  nand2  g27(.a(x25), .b(x18), .O(new_n80_));
  inv1   g28(.a(x07), .O(new_n81_));
  nand2  g29(.a(new_n76_), .b(new_n81_), .O(new_n82_));
  nand3  g30(.a(new_n82_), .b(new_n80_), .c(new_n74_), .O(new_n83_));
  oai21  g31(.a(new_n57_), .b(x24), .c(new_n83_), .O(z04));
  inv1   g32(.a(x25), .O(new_n85_));
  nor2   g33(.a(x19), .b(new_n76_), .O(new_n86_));
  nand2  g34(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nor2   g35(.a(x18), .b(x06), .O(new_n88_));
  nand2  g36(.a(x26), .b(x18), .O(new_n89_));
  nand2  g37(.a(new_n89_), .b(new_n74_), .O(new_n90_));
  oai21  g38(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(z05));
  inv1   g39(.a(x05), .O(new_n92_));
  oai22  g40(.a(new_n67_), .b(x27), .c(x18), .d(new_n92_), .O(new_n93_));
  nand2  g41(.a(new_n93_), .b(new_n56_), .O(new_n94_));
  nand2  g42(.a(new_n89_), .b(new_n53_), .O(new_n95_));
  nand2  g43(.a(new_n95_), .b(new_n94_), .O(z06));
  inv1   g44(.a(x27), .O(new_n97_));
  nand2  g45(.a(new_n86_), .b(new_n97_), .O(new_n98_));
  nor2   g46(.a(x18), .b(x04), .O(new_n99_));
  nand2  g47(.a(new_n74_), .b(new_n54_), .O(new_n100_));
  oai21  g48(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(z07));
  inv1   g49(.a(x11), .O(new_n102_));
  oai22  g50(.a(new_n67_), .b(x29), .c(x18), .d(new_n102_), .O(new_n103_));
  nand2  g51(.a(new_n103_), .b(new_n56_), .O(new_n104_));
  nand2  g52(.a(x28), .b(x18), .O(new_n105_));
  nand2  g53(.a(new_n105_), .b(new_n53_), .O(new_n106_));
  nand2  g54(.a(new_n106_), .b(new_n104_), .O(z08));
  inv1   g55(.a(x29), .O(new_n108_));
  nand2  g56(.a(new_n86_), .b(new_n108_), .O(new_n109_));
  nor2   g57(.a(x18), .b(x10), .O(new_n110_));
  nand2  g58(.a(x30), .b(x18), .O(new_n111_));
  nand2  g59(.a(new_n111_), .b(new_n74_), .O(new_n112_));
  oai21  g60(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(z09));
  inv1   g61(.a(x09), .O(new_n114_));
  oai22  g62(.a(new_n67_), .b(x31), .c(x18), .d(new_n114_), .O(new_n115_));
  nand2  g63(.a(new_n115_), .b(new_n56_), .O(new_n116_));
  nand2  g64(.a(new_n111_), .b(new_n53_), .O(new_n117_));
  nand2  g65(.a(new_n117_), .b(new_n116_), .O(z10));
  nand2  g66(.a(x24), .b(x18), .O(new_n119_));
  inv1   g67(.a(x08), .O(new_n120_));
  nand2  g68(.a(new_n76_), .b(new_n120_), .O(new_n121_));
  nand3  g69(.a(new_n121_), .b(new_n119_), .c(new_n74_), .O(new_n122_));
  oai21  g70(.a(new_n57_), .b(x31), .c(new_n122_), .O(z11));
  inv1   g71(.a(x15), .O(new_n124_));
  oai22  g72(.a(new_n67_), .b(x33), .c(x18), .d(new_n124_), .O(new_n125_));
  nand2  g73(.a(new_n125_), .b(new_n56_), .O(new_n126_));
  nand2  g74(.a(x32), .b(x18), .O(new_n127_));
  nand2  g75(.a(new_n127_), .b(new_n53_), .O(new_n128_));
  nand2  g76(.a(new_n128_), .b(new_n126_), .O(z12));
  nand2  g77(.a(x34), .b(x18), .O(new_n130_));
  inv1   g78(.a(x14), .O(new_n131_));
  nand2  g79(.a(new_n76_), .b(new_n131_), .O(new_n132_));
  nand3  g80(.a(new_n132_), .b(new_n130_), .c(new_n74_), .O(new_n133_));
  oai21  g81(.a(new_n57_), .b(x33), .c(new_n133_), .O(z13));
  inv1   g82(.a(x34), .O(new_n135_));
  nand2  g83(.a(new_n86_), .b(new_n135_), .O(new_n136_));
  nor2   g84(.a(x18), .b(x13), .O(new_n137_));
  nand2  g85(.a(x35), .b(x18), .O(new_n138_));
  nand2  g86(.a(new_n138_), .b(new_n74_), .O(new_n139_));
  oai21  g87(.a(new_n139_), .b(new_n137_), .c(new_n136_), .O(z14));
  nand2  g88(.a(new_n138_), .b(new_n53_), .O(new_n141_));
  oai21  g89(.a(x18), .b(x12), .c(new_n105_), .O(new_n142_));
  oai21  g90(.a(new_n142_), .b(new_n58_), .c(new_n141_), .O(z15));
endmodule


