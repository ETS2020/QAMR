// Benchmark "FAU" written by ABC on Tue Aug 11 20:01:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n134_, new_n135_,
    new_n136_, new_n137_, new_n138_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_, new_n160_, new_n162_, new_n163_,
    new_n164_;
  nand2  g000(.a(x21), .b(x18), .O(new_n53_));
  inv1   g001(.a(x19), .O(new_n54_));
  nor2   g002(.a(new_n54_), .b(x17), .O(new_n55_));
  inv1   g003(.a(x03), .O(new_n56_));
  inv1   g004(.a(x18), .O(new_n57_));
  nand2  g005(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand3  g006(.a(new_n58_), .b(new_n55_), .c(new_n53_), .O(new_n59_));
  nor2   g007(.a(x20), .b(x19), .O(new_n60_));
  nor2   g008(.a(x28), .b(x10), .O(new_n61_));
  nor2   g009(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n59_), .O(z00));
  nand2  g011(.a(x22), .b(x18), .O(new_n64_));
  inv1   g012(.a(x02), .O(new_n65_));
  nand2  g013(.a(new_n57_), .b(new_n65_), .O(new_n66_));
  nand3  g014(.a(new_n66_), .b(new_n64_), .c(new_n55_), .O(new_n67_));
  nor2   g015(.a(x21), .b(x19), .O(new_n68_));
  nor2   g016(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n67_), .O(z01));
  nand2  g018(.a(x23), .b(x18), .O(new_n71_));
  inv1   g019(.a(x01), .O(new_n72_));
  nand2  g020(.a(new_n57_), .b(new_n72_), .O(new_n73_));
  nand3  g021(.a(new_n73_), .b(new_n71_), .c(new_n55_), .O(new_n74_));
  nor2   g022(.a(x22), .b(x19), .O(new_n75_));
  nor2   g023(.a(new_n75_), .b(new_n61_), .O(new_n76_));
  nand2  g024(.a(new_n76_), .b(new_n74_), .O(z02));
  nor2   g025(.a(new_n57_), .b(x16), .O(new_n78_));
  inv1   g026(.a(x00), .O(new_n79_));
  nand2  g027(.a(new_n57_), .b(new_n79_), .O(new_n80_));
  nand2  g028(.a(new_n80_), .b(new_n55_), .O(new_n81_));
  nor2   g029(.a(x23), .b(x19), .O(new_n82_));
  nor2   g030(.a(new_n82_), .b(new_n61_), .O(new_n83_));
  oai21  g031(.a(new_n81_), .b(new_n78_), .c(new_n83_), .O(z03));
  inv1   g032(.a(x24), .O(new_n85_));
  nand2  g033(.a(new_n85_), .b(new_n54_), .O(new_n86_));
  nand2  g034(.a(x25), .b(x18), .O(new_n87_));
  inv1   g035(.a(x07), .O(new_n88_));
  nand2  g036(.a(new_n57_), .b(new_n88_), .O(new_n89_));
  nand3  g037(.a(new_n89_), .b(new_n87_), .c(new_n55_), .O(new_n90_));
  aoi21  g038(.a(new_n90_), .b(new_n86_), .c(new_n61_), .O(z04));
  inv1   g039(.a(x25), .O(new_n92_));
  nand2  g040(.a(new_n92_), .b(new_n54_), .O(new_n93_));
  nand2  g041(.a(x26), .b(x18), .O(new_n94_));
  inv1   g042(.a(x06), .O(new_n95_));
  nand2  g043(.a(new_n57_), .b(new_n95_), .O(new_n96_));
  nand3  g044(.a(new_n96_), .b(new_n94_), .c(new_n55_), .O(new_n97_));
  aoi21  g045(.a(new_n97_), .b(new_n93_), .c(new_n61_), .O(z05));
  nand2  g046(.a(x27), .b(x18), .O(new_n99_));
  inv1   g047(.a(x05), .O(new_n100_));
  nand2  g048(.a(new_n57_), .b(new_n100_), .O(new_n101_));
  nand3  g049(.a(new_n101_), .b(new_n99_), .c(new_n55_), .O(new_n102_));
  nor2   g050(.a(x26), .b(x19), .O(new_n103_));
  nor2   g051(.a(new_n103_), .b(new_n61_), .O(new_n104_));
  nand2  g052(.a(new_n104_), .b(new_n102_), .O(z06));
  inv1   g053(.a(x27), .O(new_n106_));
  nand2  g054(.a(new_n106_), .b(new_n54_), .O(new_n107_));
  nand2  g055(.a(x20), .b(x18), .O(new_n108_));
  inv1   g056(.a(x04), .O(new_n109_));
  nand2  g057(.a(new_n57_), .b(new_n109_), .O(new_n110_));
  nand3  g058(.a(new_n110_), .b(new_n108_), .c(new_n55_), .O(new_n111_));
  aoi21  g059(.a(new_n111_), .b(new_n107_), .c(new_n61_), .O(z07));
  aoi21  g060(.a(x29), .b(x18), .c(x17), .O(new_n113_));
  oai21  g061(.a(x18), .b(x11), .c(new_n113_), .O(new_n114_));
  inv1   g062(.a(x28), .O(new_n115_));
  inv1   g063(.a(x10), .O(new_n116_));
  nand2  g064(.a(new_n115_), .b(new_n116_), .O(new_n117_));
  oai21  g065(.a(new_n115_), .b(x19), .c(new_n117_), .O(new_n118_));
  aoi21  g066(.a(new_n114_), .b(x19), .c(new_n118_), .O(z08));
  aoi21  g067(.a(x28), .b(x18), .c(x10), .O(new_n120_));
  nand2  g068(.a(x30), .b(x18), .O(new_n121_));
  nand2  g069(.a(new_n121_), .b(new_n55_), .O(new_n122_));
  inv1   g070(.a(x29), .O(new_n123_));
  nand3  g071(.a(new_n117_), .b(new_n123_), .c(new_n54_), .O(new_n124_));
  oai21  g072(.a(new_n122_), .b(new_n120_), .c(new_n124_), .O(z09));
  inv1   g073(.a(x30), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n54_), .O(new_n127_));
  nand2  g075(.a(x31), .b(x18), .O(new_n128_));
  inv1   g076(.a(x09), .O(new_n129_));
  nand2  g077(.a(new_n57_), .b(new_n129_), .O(new_n130_));
  nand3  g078(.a(new_n130_), .b(new_n128_), .c(new_n55_), .O(new_n131_));
  aoi21  g079(.a(new_n131_), .b(new_n127_), .c(new_n61_), .O(z10));
  nand2  g080(.a(x24), .b(x18), .O(new_n133_));
  inv1   g081(.a(x08), .O(new_n134_));
  nand2  g082(.a(new_n57_), .b(new_n134_), .O(new_n135_));
  nand3  g083(.a(new_n135_), .b(new_n133_), .c(new_n55_), .O(new_n136_));
  nor2   g084(.a(x31), .b(x19), .O(new_n137_));
  nor2   g085(.a(new_n137_), .b(new_n61_), .O(new_n138_));
  nand2  g086(.a(new_n138_), .b(new_n136_), .O(z11));
  nand2  g087(.a(x33), .b(x18), .O(new_n140_));
  inv1   g088(.a(x15), .O(new_n141_));
  nand2  g089(.a(new_n57_), .b(new_n141_), .O(new_n142_));
  nand3  g090(.a(new_n142_), .b(new_n140_), .c(new_n55_), .O(new_n143_));
  nor2   g091(.a(x32), .b(x19), .O(new_n144_));
  nor2   g092(.a(new_n144_), .b(new_n61_), .O(new_n145_));
  nand2  g093(.a(new_n145_), .b(new_n143_), .O(z12));
  inv1   g094(.a(x33), .O(new_n147_));
  nand2  g095(.a(new_n147_), .b(new_n54_), .O(new_n148_));
  nand2  g096(.a(x34), .b(x18), .O(new_n149_));
  inv1   g097(.a(x14), .O(new_n150_));
  nand2  g098(.a(new_n57_), .b(new_n150_), .O(new_n151_));
  nand3  g099(.a(new_n151_), .b(new_n149_), .c(new_n55_), .O(new_n152_));
  aoi21  g100(.a(new_n152_), .b(new_n148_), .c(new_n61_), .O(z13));
  inv1   g101(.a(x17), .O(new_n154_));
  inv1   g102(.a(x35), .O(new_n155_));
  nand4  g103(.a(new_n155_), .b(x19), .c(x18), .d(new_n154_), .O(new_n156_));
  inv1   g104(.a(x34), .O(new_n157_));
  nand2  g105(.a(new_n157_), .b(new_n54_), .O(new_n158_));
  nand4  g106(.a(x19), .b(new_n57_), .c(new_n154_), .d(x13), .O(new_n159_));
  nand3  g107(.a(new_n159_), .b(new_n158_), .c(new_n156_), .O(new_n160_));
  and2   g108(.a(new_n160_), .b(new_n117_), .O(z14));
  aoi21  g109(.a(x28), .b(x18), .c(x17), .O(new_n162_));
  oai21  g110(.a(x18), .b(x12), .c(new_n162_), .O(new_n163_));
  oai21  g111(.a(new_n155_), .b(x19), .c(new_n117_), .O(new_n164_));
  aoi21  g112(.a(new_n163_), .b(x19), .c(new_n164_), .O(z15));
endmodule


