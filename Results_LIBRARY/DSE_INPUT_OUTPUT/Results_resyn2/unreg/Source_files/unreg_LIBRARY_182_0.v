// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x17), .O(new_n53_));
  inv1   g01(.a(x03), .O(new_n54_));
  nand2  g02(.a(x19), .b(x18), .O(new_n55_));
  oai22  g03(.a(new_n55_), .b(x21), .c(x18), .d(new_n54_), .O(new_n56_));
  nand2  g04(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g05(.a(x19), .O(new_n58_));
  nand2  g06(.a(x20), .b(x18), .O(new_n59_));
  nand2  g07(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g08(.a(new_n60_), .b(new_n57_), .O(z00));
  nand2  g09(.a(new_n58_), .b(x18), .O(new_n62_));
  nand2  g10(.a(x22), .b(x18), .O(new_n63_));
  nor2   g11(.a(new_n58_), .b(x17), .O(new_n64_));
  inv1   g12(.a(x02), .O(new_n65_));
  inv1   g13(.a(x18), .O(new_n66_));
  nand2  g14(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g15(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(new_n68_));
  oai21  g16(.a(new_n62_), .b(x21), .c(new_n68_), .O(z01));
  inv1   g17(.a(x22), .O(new_n70_));
  nor2   g18(.a(x19), .b(new_n66_), .O(new_n71_));
  nand2  g19(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nor2   g20(.a(x18), .b(x01), .O(new_n73_));
  nand2  g21(.a(x23), .b(x18), .O(new_n74_));
  nand2  g22(.a(new_n74_), .b(new_n64_), .O(new_n75_));
  oai21  g23(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(z02));
  inv1   g24(.a(x00), .O(new_n77_));
  inv1   g25(.a(x16), .O(new_n78_));
  oai22  g26(.a(new_n55_), .b(new_n78_), .c(x18), .d(new_n77_), .O(new_n79_));
  nand2  g27(.a(new_n79_), .b(new_n53_), .O(new_n80_));
  nand2  g28(.a(new_n74_), .b(new_n58_), .O(new_n81_));
  nand2  g29(.a(new_n81_), .b(new_n80_), .O(z03));
  nand2  g30(.a(x24), .b(x18), .O(new_n83_));
  nand2  g31(.a(new_n83_), .b(new_n58_), .O(new_n84_));
  nand2  g32(.a(new_n62_), .b(new_n53_), .O(new_n85_));
  nand2  g33(.a(x25), .b(x18), .O(new_n86_));
  oai21  g34(.a(x18), .b(x07), .c(new_n86_), .O(new_n87_));
  oai21  g35(.a(new_n87_), .b(new_n85_), .c(new_n84_), .O(z04));
  nand2  g36(.a(new_n86_), .b(new_n58_), .O(new_n89_));
  nand2  g37(.a(x26), .b(x18), .O(new_n90_));
  oai21  g38(.a(x18), .b(x06), .c(new_n90_), .O(new_n91_));
  oai21  g39(.a(new_n91_), .b(new_n85_), .c(new_n89_), .O(z05));
  nand2  g40(.a(new_n90_), .b(new_n58_), .O(new_n93_));
  nand2  g41(.a(x27), .b(x18), .O(new_n94_));
  oai21  g42(.a(x18), .b(x05), .c(new_n94_), .O(new_n95_));
  oai21  g43(.a(new_n95_), .b(new_n85_), .c(new_n93_), .O(z06));
  nand2  g44(.a(new_n94_), .b(new_n58_), .O(new_n97_));
  oai21  g45(.a(x18), .b(x04), .c(new_n59_), .O(new_n98_));
  oai21  g46(.a(new_n98_), .b(new_n85_), .c(new_n97_), .O(z07));
  nand2  g47(.a(x28), .b(x18), .O(new_n100_));
  nand2  g48(.a(new_n100_), .b(new_n58_), .O(new_n101_));
  nand2  g49(.a(x29), .b(x18), .O(new_n102_));
  oai21  g50(.a(x18), .b(x11), .c(new_n102_), .O(new_n103_));
  oai21  g51(.a(new_n103_), .b(new_n85_), .c(new_n101_), .O(z08));
  nand2  g52(.a(new_n102_), .b(new_n58_), .O(new_n105_));
  nand2  g53(.a(x30), .b(x18), .O(new_n106_));
  oai21  g54(.a(x18), .b(x10), .c(new_n106_), .O(new_n107_));
  oai21  g55(.a(new_n107_), .b(new_n85_), .c(new_n105_), .O(z09));
  nand2  g56(.a(new_n106_), .b(new_n58_), .O(new_n109_));
  nand2  g57(.a(x31), .b(x18), .O(new_n110_));
  oai21  g58(.a(x18), .b(x09), .c(new_n110_), .O(new_n111_));
  oai21  g59(.a(new_n111_), .b(new_n85_), .c(new_n109_), .O(z10));
  nand2  g60(.a(new_n110_), .b(new_n58_), .O(new_n113_));
  oai21  g61(.a(x18), .b(x08), .c(new_n83_), .O(new_n114_));
  oai21  g62(.a(new_n114_), .b(new_n85_), .c(new_n113_), .O(z11));
  nand2  g63(.a(x32), .b(x18), .O(new_n116_));
  nand2  g64(.a(new_n116_), .b(new_n58_), .O(new_n117_));
  nand2  g65(.a(x33), .b(x18), .O(new_n118_));
  oai21  g66(.a(x18), .b(x15), .c(new_n118_), .O(new_n119_));
  oai21  g67(.a(new_n119_), .b(new_n85_), .c(new_n117_), .O(z12));
  nand2  g68(.a(new_n118_), .b(new_n58_), .O(new_n121_));
  nand2  g69(.a(x34), .b(x18), .O(new_n122_));
  oai21  g70(.a(x18), .b(x14), .c(new_n122_), .O(new_n123_));
  oai21  g71(.a(new_n123_), .b(new_n85_), .c(new_n121_), .O(z13));
  inv1   g72(.a(x13), .O(new_n125_));
  oai22  g73(.a(new_n55_), .b(x35), .c(x18), .d(new_n125_), .O(new_n126_));
  nand2  g74(.a(new_n126_), .b(new_n53_), .O(new_n127_));
  nand2  g75(.a(new_n122_), .b(new_n58_), .O(new_n128_));
  nand2  g76(.a(new_n128_), .b(new_n127_), .O(z14));
  inv1   g77(.a(x35), .O(new_n130_));
  nand2  g78(.a(new_n71_), .b(new_n130_), .O(new_n131_));
  nor2   g79(.a(x18), .b(x12), .O(new_n132_));
  nand2  g80(.a(new_n100_), .b(new_n64_), .O(new_n133_));
  oai21  g81(.a(new_n133_), .b(new_n132_), .c(new_n131_), .O(z15));
endmodule


