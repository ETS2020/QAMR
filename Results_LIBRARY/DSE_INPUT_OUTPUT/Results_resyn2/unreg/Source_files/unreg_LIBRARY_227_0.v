// Benchmark "FAU" written by ABC on Tue Aug 11 20:01:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_;
  nor2   g000(.a(x28), .b(x24), .O(new_n53_));
  inv1   g001(.a(x19), .O(new_n54_));
  inv1   g002(.a(x20), .O(new_n55_));
  nand2  g003(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g004(.a(x21), .b(x18), .O(new_n57_));
  nor2   g005(.a(new_n54_), .b(x17), .O(new_n58_));
  inv1   g006(.a(x03), .O(new_n59_));
  inv1   g007(.a(x18), .O(new_n60_));
  nand2  g008(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g009(.a(new_n61_), .b(new_n58_), .c(new_n57_), .O(new_n62_));
  aoi21  g010(.a(new_n62_), .b(new_n56_), .c(new_n53_), .O(z00));
  nand2  g011(.a(x22), .b(x18), .O(new_n64_));
  inv1   g012(.a(x02), .O(new_n65_));
  nand2  g013(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  nand3  g014(.a(new_n66_), .b(new_n64_), .c(new_n58_), .O(new_n67_));
  nor2   g015(.a(x21), .b(x19), .O(new_n68_));
  nor2   g016(.a(new_n68_), .b(new_n53_), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n67_), .O(z01));
  nand2  g018(.a(x23), .b(x18), .O(new_n71_));
  inv1   g019(.a(x01), .O(new_n72_));
  nand2  g020(.a(new_n60_), .b(new_n72_), .O(new_n73_));
  nand3  g021(.a(new_n73_), .b(new_n71_), .c(new_n58_), .O(new_n74_));
  nor2   g022(.a(x22), .b(x19), .O(new_n75_));
  nor2   g023(.a(new_n75_), .b(new_n53_), .O(new_n76_));
  nand2  g024(.a(new_n76_), .b(new_n74_), .O(z02));
  inv1   g025(.a(x23), .O(new_n78_));
  nand2  g026(.a(new_n78_), .b(new_n54_), .O(new_n79_));
  inv1   g027(.a(x16), .O(new_n80_));
  nand2  g028(.a(x18), .b(new_n80_), .O(new_n81_));
  inv1   g029(.a(x00), .O(new_n82_));
  nand2  g030(.a(new_n60_), .b(new_n82_), .O(new_n83_));
  nand3  g031(.a(new_n83_), .b(new_n81_), .c(new_n58_), .O(new_n84_));
  aoi21  g032(.a(new_n84_), .b(new_n79_), .c(new_n53_), .O(z03));
  inv1   g033(.a(new_n53_), .O(new_n86_));
  nand2  g034(.a(new_n58_), .b(new_n86_), .O(new_n87_));
  nand2  g035(.a(x25), .b(x18), .O(new_n88_));
  oai21  g036(.a(x18), .b(x07), .c(new_n88_), .O(new_n89_));
  inv1   g037(.a(x24), .O(new_n90_));
  nand3  g038(.a(x28), .b(new_n90_), .c(new_n54_), .O(new_n91_));
  oai21  g039(.a(new_n89_), .b(new_n87_), .c(new_n91_), .O(z04));
  nand2  g040(.a(x26), .b(x18), .O(new_n93_));
  oai21  g041(.a(x18), .b(x06), .c(new_n93_), .O(new_n94_));
  nor2   g042(.a(x25), .b(x19), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n86_), .O(new_n96_));
  oai21  g044(.a(new_n94_), .b(new_n87_), .c(new_n96_), .O(z05));
  nand2  g045(.a(x27), .b(x18), .O(new_n98_));
  inv1   g046(.a(x05), .O(new_n99_));
  nand2  g047(.a(new_n60_), .b(new_n99_), .O(new_n100_));
  nand3  g048(.a(new_n100_), .b(new_n98_), .c(new_n58_), .O(new_n101_));
  nor2   g049(.a(x26), .b(x19), .O(new_n102_));
  nor2   g050(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  nand2  g051(.a(new_n103_), .b(new_n101_), .O(z06));
  inv1   g052(.a(x27), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n54_), .O(new_n106_));
  nand2  g054(.a(x20), .b(x18), .O(new_n107_));
  inv1   g055(.a(x04), .O(new_n108_));
  nand2  g056(.a(new_n60_), .b(new_n108_), .O(new_n109_));
  nand3  g057(.a(new_n109_), .b(new_n107_), .c(new_n58_), .O(new_n110_));
  aoi21  g058(.a(new_n110_), .b(new_n106_), .c(new_n53_), .O(z07));
  nand2  g059(.a(x29), .b(x18), .O(new_n112_));
  oai21  g060(.a(x18), .b(x11), .c(new_n112_), .O(new_n113_));
  inv1   g061(.a(x28), .O(new_n114_));
  nand3  g062(.a(new_n114_), .b(x24), .c(new_n54_), .O(new_n115_));
  oai21  g063(.a(new_n113_), .b(new_n87_), .c(new_n115_), .O(z08));
  nand2  g064(.a(x30), .b(x18), .O(new_n117_));
  oai21  g065(.a(x18), .b(x10), .c(new_n117_), .O(new_n118_));
  nor2   g066(.a(x29), .b(x19), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n86_), .O(new_n120_));
  oai21  g068(.a(new_n118_), .b(new_n87_), .c(new_n120_), .O(z09));
  nand2  g069(.a(x31), .b(x18), .O(new_n122_));
  inv1   g070(.a(x09), .O(new_n123_));
  nand2  g071(.a(new_n60_), .b(new_n123_), .O(new_n124_));
  nand3  g072(.a(new_n124_), .b(new_n122_), .c(new_n58_), .O(new_n125_));
  nor2   g073(.a(x30), .b(x19), .O(new_n126_));
  nor2   g074(.a(new_n126_), .b(new_n53_), .O(new_n127_));
  nand2  g075(.a(new_n127_), .b(new_n125_), .O(z10));
  inv1   g076(.a(x17), .O(new_n129_));
  nand2  g077(.a(x24), .b(x18), .O(new_n130_));
  inv1   g078(.a(x08), .O(new_n131_));
  nand2  g079(.a(new_n60_), .b(new_n131_), .O(new_n132_));
  nand3  g080(.a(new_n132_), .b(new_n130_), .c(new_n129_), .O(new_n133_));
  nand2  g081(.a(x31), .b(new_n54_), .O(new_n134_));
  nand2  g082(.a(new_n134_), .b(new_n86_), .O(new_n135_));
  aoi21  g083(.a(new_n133_), .b(x19), .c(new_n135_), .O(z11));
  inv1   g084(.a(x32), .O(new_n137_));
  nand2  g085(.a(new_n137_), .b(new_n54_), .O(new_n138_));
  nand2  g086(.a(x33), .b(x18), .O(new_n139_));
  inv1   g087(.a(x15), .O(new_n140_));
  nand2  g088(.a(new_n60_), .b(new_n140_), .O(new_n141_));
  nand3  g089(.a(new_n141_), .b(new_n139_), .c(new_n58_), .O(new_n142_));
  aoi21  g090(.a(new_n142_), .b(new_n138_), .c(new_n53_), .O(z12));
  inv1   g091(.a(x33), .O(new_n144_));
  nand2  g092(.a(new_n144_), .b(new_n54_), .O(new_n145_));
  nand2  g093(.a(x34), .b(x18), .O(new_n146_));
  inv1   g094(.a(x14), .O(new_n147_));
  nand2  g095(.a(new_n60_), .b(new_n147_), .O(new_n148_));
  nand3  g096(.a(new_n148_), .b(new_n146_), .c(new_n58_), .O(new_n149_));
  aoi21  g097(.a(new_n149_), .b(new_n145_), .c(new_n53_), .O(z13));
  inv1   g098(.a(x35), .O(new_n151_));
  nand4  g099(.a(new_n151_), .b(x19), .c(x18), .d(new_n129_), .O(new_n152_));
  inv1   g100(.a(x34), .O(new_n153_));
  nand2  g101(.a(new_n153_), .b(new_n54_), .O(new_n154_));
  nand4  g102(.a(x19), .b(new_n60_), .c(new_n129_), .d(x13), .O(new_n155_));
  nand3  g103(.a(new_n155_), .b(new_n154_), .c(new_n152_), .O(new_n156_));
  and2   g104(.a(new_n156_), .b(new_n86_), .O(z14));
  nand4  g105(.a(new_n114_), .b(x19), .c(x18), .d(new_n129_), .O(new_n158_));
  nand2  g106(.a(new_n151_), .b(new_n54_), .O(new_n159_));
  nand4  g107(.a(x19), .b(new_n60_), .c(new_n129_), .d(x12), .O(new_n160_));
  nand3  g108(.a(new_n160_), .b(new_n159_), .c(new_n158_), .O(new_n161_));
  and2   g109(.a(new_n161_), .b(new_n86_), .O(z15));
endmodule


