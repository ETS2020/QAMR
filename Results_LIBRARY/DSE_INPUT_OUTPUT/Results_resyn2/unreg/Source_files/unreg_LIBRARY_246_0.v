// Benchmark "FAU" written by ABC on Tue Aug 11 20:01:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_;
  inv1   g00(.a(x17), .O(new_n53_));
  inv1   g01(.a(x03), .O(new_n54_));
  nand2  g02(.a(x19), .b(x18), .O(new_n55_));
  oai22  g03(.a(new_n55_), .b(x21), .c(x18), .d(new_n54_), .O(new_n56_));
  nand2  g04(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g05(.a(x19), .O(new_n58_));
  nand2  g06(.a(x20), .b(x18), .O(new_n59_));
  nand2  g07(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g08(.a(new_n60_), .b(new_n57_), .O(z00));
  nand2  g09(.a(new_n58_), .b(x18), .O(new_n62_));
  nand2  g10(.a(x22), .b(x18), .O(new_n63_));
  nor2   g11(.a(new_n58_), .b(x17), .O(new_n64_));
  inv1   g12(.a(x02), .O(new_n65_));
  inv1   g13(.a(x18), .O(new_n66_));
  nand2  g14(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g15(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(new_n68_));
  oai21  g16(.a(new_n62_), .b(x21), .c(new_n68_), .O(z01));
  inv1   g17(.a(x01), .O(new_n70_));
  oai22  g18(.a(new_n55_), .b(x23), .c(x18), .d(new_n70_), .O(new_n71_));
  nand2  g19(.a(new_n71_), .b(new_n53_), .O(new_n72_));
  nand2  g20(.a(new_n63_), .b(new_n58_), .O(new_n73_));
  nand2  g21(.a(new_n73_), .b(new_n72_), .O(z02));
  inv1   g22(.a(x16), .O(new_n75_));
  nand2  g23(.a(x18), .b(new_n75_), .O(new_n76_));
  inv1   g24(.a(x00), .O(new_n77_));
  nand2  g25(.a(new_n66_), .b(new_n77_), .O(new_n78_));
  nand3  g26(.a(new_n78_), .b(new_n76_), .c(new_n64_), .O(new_n79_));
  oai21  g27(.a(new_n62_), .b(x23), .c(new_n79_), .O(z03));
  inv1   g28(.a(x24), .O(new_n81_));
  nor2   g29(.a(x19), .b(new_n66_), .O(new_n82_));
  nand2  g30(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g31(.a(x18), .b(x07), .O(new_n84_));
  nand2  g32(.a(x25), .b(x18), .O(new_n85_));
  nand2  g33(.a(new_n85_), .b(new_n64_), .O(new_n86_));
  oai21  g34(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(z04));
  inv1   g35(.a(x06), .O(new_n88_));
  oai22  g36(.a(new_n55_), .b(x26), .c(x18), .d(new_n88_), .O(new_n89_));
  nand2  g37(.a(new_n89_), .b(new_n53_), .O(new_n90_));
  nand2  g38(.a(new_n85_), .b(new_n58_), .O(new_n91_));
  nand2  g39(.a(new_n91_), .b(new_n90_), .O(z05));
  inv1   g40(.a(x26), .O(new_n93_));
  nand2  g41(.a(new_n82_), .b(new_n93_), .O(new_n94_));
  nor2   g42(.a(x18), .b(x05), .O(new_n95_));
  nand2  g43(.a(x27), .b(x18), .O(new_n96_));
  nand2  g44(.a(new_n96_), .b(new_n64_), .O(new_n97_));
  oai21  g45(.a(new_n97_), .b(new_n95_), .c(new_n94_), .O(z06));
  nand2  g46(.a(new_n96_), .b(new_n58_), .O(new_n99_));
  nor2   g47(.a(x18), .b(x04), .O(new_n100_));
  nand2  g48(.a(new_n64_), .b(new_n59_), .O(new_n101_));
  oai21  g49(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(z07));
  nand2  g50(.a(x29), .b(x18), .O(new_n103_));
  inv1   g51(.a(x11), .O(new_n104_));
  nand2  g52(.a(new_n66_), .b(new_n104_), .O(new_n105_));
  nand3  g53(.a(new_n105_), .b(new_n103_), .c(new_n64_), .O(new_n106_));
  oai21  g54(.a(new_n62_), .b(x28), .c(new_n106_), .O(z08));
  inv1   g55(.a(x29), .O(new_n108_));
  nand2  g56(.a(new_n82_), .b(new_n108_), .O(new_n109_));
  nor2   g57(.a(x18), .b(x10), .O(new_n110_));
  nand2  g58(.a(x30), .b(x18), .O(new_n111_));
  nand2  g59(.a(new_n111_), .b(new_n64_), .O(new_n112_));
  oai21  g60(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(z09));
  nand2  g61(.a(new_n111_), .b(new_n58_), .O(new_n114_));
  nand2  g62(.a(new_n62_), .b(new_n53_), .O(new_n115_));
  nand2  g63(.a(x31), .b(x18), .O(new_n116_));
  oai21  g64(.a(x18), .b(x09), .c(new_n116_), .O(new_n117_));
  oai21  g65(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z10));
  inv1   g66(.a(x08), .O(new_n119_));
  oai22  g67(.a(new_n55_), .b(x24), .c(x18), .d(new_n119_), .O(new_n120_));
  nand2  g68(.a(new_n120_), .b(new_n53_), .O(new_n121_));
  nand2  g69(.a(new_n116_), .b(new_n58_), .O(new_n122_));
  nand2  g70(.a(new_n122_), .b(new_n121_), .O(z11));
  nand2  g71(.a(x32), .b(x18), .O(new_n124_));
  nand2  g72(.a(new_n124_), .b(new_n58_), .O(new_n125_));
  nand2  g73(.a(x33), .b(x18), .O(new_n126_));
  oai21  g74(.a(x18), .b(x15), .c(new_n126_), .O(new_n127_));
  oai21  g75(.a(new_n127_), .b(new_n115_), .c(new_n125_), .O(z12));
  nand2  g76(.a(new_n126_), .b(new_n58_), .O(new_n129_));
  nand2  g77(.a(x34), .b(x18), .O(new_n130_));
  oai21  g78(.a(x18), .b(x14), .c(new_n130_), .O(new_n131_));
  oai21  g79(.a(new_n131_), .b(new_n115_), .c(new_n129_), .O(z13));
  inv1   g80(.a(x13), .O(new_n133_));
  oai22  g81(.a(new_n55_), .b(x35), .c(x18), .d(new_n133_), .O(new_n134_));
  nand2  g82(.a(new_n134_), .b(new_n53_), .O(new_n135_));
  nand2  g83(.a(new_n130_), .b(new_n58_), .O(new_n136_));
  nand2  g84(.a(new_n136_), .b(new_n135_), .O(z14));
  nand2  g85(.a(x28), .b(x18), .O(new_n138_));
  inv1   g86(.a(x12), .O(new_n139_));
  nand2  g87(.a(new_n66_), .b(new_n139_), .O(new_n140_));
  nand3  g88(.a(new_n140_), .b(new_n138_), .c(new_n64_), .O(new_n141_));
  oai21  g89(.a(new_n62_), .b(x35), .c(new_n141_), .O(z15));
endmodule


