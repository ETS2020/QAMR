// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x17), .O(new_n53_));
  nand3  g01(.a(x34), .b(x19), .c(new_n53_), .O(new_n54_));
  nand2  g02(.a(x21), .b(x18), .O(new_n55_));
  oai21  g03(.a(x18), .b(x03), .c(new_n55_), .O(new_n56_));
  oai22  g04(.a(new_n56_), .b(new_n54_), .c(x20), .d(x19), .O(z00));
  nand2  g05(.a(x22), .b(x18), .O(new_n58_));
  oai21  g06(.a(x18), .b(x02), .c(new_n58_), .O(new_n59_));
  oai22  g07(.a(new_n59_), .b(new_n54_), .c(x21), .d(x19), .O(z01));
  inv1   g08(.a(x19), .O(new_n61_));
  inv1   g09(.a(x34), .O(new_n62_));
  nand2  g10(.a(x23), .b(x18), .O(new_n63_));
  inv1   g11(.a(x01), .O(new_n64_));
  inv1   g12(.a(x18), .O(new_n65_));
  aoi21  g13(.a(new_n65_), .b(new_n64_), .c(x17), .O(new_n66_));
  aoi21  g14(.a(new_n66_), .b(new_n63_), .c(new_n62_), .O(new_n67_));
  inv1   g15(.a(x22), .O(new_n68_));
  nand2  g16(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  oai21  g17(.a(new_n67_), .b(new_n61_), .c(new_n69_), .O(z02));
  inv1   g18(.a(x16), .O(new_n71_));
  nand2  g19(.a(x18), .b(new_n71_), .O(new_n72_));
  inv1   g20(.a(x00), .O(new_n73_));
  aoi21  g21(.a(new_n65_), .b(new_n73_), .c(x17), .O(new_n74_));
  aoi21  g22(.a(new_n74_), .b(new_n72_), .c(new_n62_), .O(new_n75_));
  inv1   g23(.a(x23), .O(new_n76_));
  nand2  g24(.a(new_n76_), .b(new_n61_), .O(new_n77_));
  oai21  g25(.a(new_n75_), .b(new_n61_), .c(new_n77_), .O(z03));
  nand2  g26(.a(x25), .b(x18), .O(new_n79_));
  inv1   g27(.a(x07), .O(new_n80_));
  aoi21  g28(.a(new_n65_), .b(new_n80_), .c(x17), .O(new_n81_));
  aoi21  g29(.a(new_n81_), .b(new_n79_), .c(new_n62_), .O(new_n82_));
  inv1   g30(.a(x24), .O(new_n83_));
  nand2  g31(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  oai21  g32(.a(new_n82_), .b(new_n61_), .c(new_n84_), .O(z04));
  nand2  g33(.a(x26), .b(x18), .O(new_n86_));
  oai21  g34(.a(x18), .b(x06), .c(new_n86_), .O(new_n87_));
  oai22  g35(.a(new_n87_), .b(new_n54_), .c(x25), .d(x19), .O(z05));
  nand2  g36(.a(x27), .b(x18), .O(new_n89_));
  inv1   g37(.a(x05), .O(new_n90_));
  aoi21  g38(.a(new_n65_), .b(new_n90_), .c(x17), .O(new_n91_));
  aoi21  g39(.a(new_n91_), .b(new_n89_), .c(new_n62_), .O(new_n92_));
  inv1   g40(.a(x26), .O(new_n93_));
  nand2  g41(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  oai21  g42(.a(new_n92_), .b(new_n61_), .c(new_n94_), .O(z06));
  nand2  g43(.a(x20), .b(x18), .O(new_n96_));
  inv1   g44(.a(x04), .O(new_n97_));
  aoi21  g45(.a(new_n65_), .b(new_n97_), .c(x17), .O(new_n98_));
  aoi21  g46(.a(new_n98_), .b(new_n96_), .c(new_n62_), .O(new_n99_));
  inv1   g47(.a(x27), .O(new_n100_));
  nand2  g48(.a(new_n100_), .b(new_n61_), .O(new_n101_));
  oai21  g49(.a(new_n99_), .b(new_n61_), .c(new_n101_), .O(z07));
  nand2  g50(.a(x29), .b(x18), .O(new_n103_));
  inv1   g51(.a(x11), .O(new_n104_));
  aoi21  g52(.a(new_n65_), .b(new_n104_), .c(x17), .O(new_n105_));
  aoi21  g53(.a(new_n105_), .b(new_n103_), .c(new_n62_), .O(new_n106_));
  inv1   g54(.a(x28), .O(new_n107_));
  nand2  g55(.a(new_n107_), .b(new_n61_), .O(new_n108_));
  oai21  g56(.a(new_n106_), .b(new_n61_), .c(new_n108_), .O(z08));
  nand2  g57(.a(x30), .b(x18), .O(new_n110_));
  inv1   g58(.a(x10), .O(new_n111_));
  aoi21  g59(.a(new_n65_), .b(new_n111_), .c(x17), .O(new_n112_));
  aoi21  g60(.a(new_n112_), .b(new_n110_), .c(new_n62_), .O(new_n113_));
  inv1   g61(.a(x29), .O(new_n114_));
  nand2  g62(.a(new_n114_), .b(new_n61_), .O(new_n115_));
  oai21  g63(.a(new_n113_), .b(new_n61_), .c(new_n115_), .O(z09));
  nand2  g64(.a(x31), .b(x18), .O(new_n117_));
  oai21  g65(.a(x18), .b(x09), .c(new_n117_), .O(new_n118_));
  oai22  g66(.a(new_n118_), .b(new_n54_), .c(x30), .d(x19), .O(z10));
  nand2  g67(.a(x24), .b(x18), .O(new_n120_));
  oai21  g68(.a(x18), .b(x08), .c(new_n120_), .O(new_n121_));
  oai22  g69(.a(new_n121_), .b(new_n54_), .c(x31), .d(x19), .O(z11));
  nand2  g70(.a(x33), .b(x18), .O(new_n123_));
  oai21  g71(.a(x18), .b(x15), .c(new_n123_), .O(new_n124_));
  oai22  g72(.a(new_n124_), .b(new_n54_), .c(x32), .d(x19), .O(z12));
  nand3  g73(.a(new_n65_), .b(new_n53_), .c(x14), .O(new_n126_));
  nand2  g74(.a(new_n126_), .b(x34), .O(new_n127_));
  nand2  g75(.a(new_n127_), .b(x19), .O(new_n128_));
  oai21  g76(.a(x33), .b(x19), .c(new_n128_), .O(z13));
  nand2  g77(.a(x35), .b(x18), .O(new_n130_));
  oai21  g78(.a(x18), .b(x13), .c(new_n130_), .O(new_n131_));
  oai22  g79(.a(new_n131_), .b(new_n54_), .c(x34), .d(x19), .O(z14));
  nand2  g80(.a(x28), .b(x18), .O(new_n133_));
  inv1   g81(.a(x12), .O(new_n134_));
  aoi21  g82(.a(new_n65_), .b(new_n134_), .c(x17), .O(new_n135_));
  aoi21  g83(.a(new_n135_), .b(new_n133_), .c(new_n62_), .O(new_n136_));
  inv1   g84(.a(x35), .O(new_n137_));
  nand2  g85(.a(new_n137_), .b(new_n61_), .O(new_n138_));
  oai21  g86(.a(new_n136_), .b(new_n61_), .c(new_n138_), .O(z15));
endmodule


