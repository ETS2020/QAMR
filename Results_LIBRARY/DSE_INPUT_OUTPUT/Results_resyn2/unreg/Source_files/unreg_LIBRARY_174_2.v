// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand3  g002(.a(x19), .b(new_n54_), .c(x03), .O(new_n55_));
  inv1   g003(.a(x21), .O(new_n56_));
  nand2  g004(.a(new_n56_), .b(x18), .O(new_n57_));
  nand2  g005(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g006(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  inv1   g007(.a(x19), .O(new_n60_));
  inv1   g008(.a(x20), .O(new_n61_));
  oai21  g009(.a(new_n61_), .b(x18), .c(new_n60_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n59_), .O(z00));
  nand3  g011(.a(x19), .b(new_n54_), .c(x02), .O(new_n64_));
  inv1   g012(.a(x22), .O(new_n65_));
  nand2  g013(.a(new_n65_), .b(x18), .O(new_n66_));
  nand2  g014(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g015(.a(new_n67_), .b(new_n53_), .O(new_n68_));
  oai21  g016(.a(new_n56_), .b(x18), .c(new_n60_), .O(new_n69_));
  nand2  g017(.a(new_n69_), .b(new_n68_), .O(z01));
  nand2  g018(.a(new_n60_), .b(new_n54_), .O(new_n71_));
  nand2  g019(.a(x23), .b(x18), .O(new_n72_));
  nor2   g020(.a(new_n60_), .b(x17), .O(new_n73_));
  inv1   g021(.a(x01), .O(new_n74_));
  nand2  g022(.a(new_n54_), .b(new_n74_), .O(new_n75_));
  nand3  g023(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  oai21  g024(.a(new_n71_), .b(x22), .c(new_n76_), .O(z02));
  nand2  g025(.a(x18), .b(x16), .O(new_n78_));
  nand3  g026(.a(x19), .b(new_n54_), .c(x00), .O(new_n79_));
  nand2  g027(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g028(.a(new_n80_), .b(new_n53_), .O(new_n81_));
  inv1   g029(.a(x23), .O(new_n82_));
  oai21  g030(.a(new_n82_), .b(x18), .c(new_n60_), .O(new_n83_));
  nand2  g031(.a(new_n83_), .b(new_n81_), .O(z03));
  nand2  g032(.a(x25), .b(x18), .O(new_n85_));
  inv1   g033(.a(x07), .O(new_n86_));
  nand2  g034(.a(new_n54_), .b(new_n86_), .O(new_n87_));
  nand3  g035(.a(new_n87_), .b(new_n85_), .c(new_n73_), .O(new_n88_));
  oai21  g036(.a(new_n71_), .b(x24), .c(new_n88_), .O(z04));
  nand2  g037(.a(x26), .b(x18), .O(new_n90_));
  inv1   g038(.a(x06), .O(new_n91_));
  nand2  g039(.a(new_n54_), .b(new_n91_), .O(new_n92_));
  nand3  g040(.a(new_n92_), .b(new_n90_), .c(new_n73_), .O(new_n93_));
  oai21  g041(.a(new_n71_), .b(x25), .c(new_n93_), .O(z05));
  nand3  g042(.a(x19), .b(new_n54_), .c(x05), .O(new_n95_));
  inv1   g043(.a(x27), .O(new_n96_));
  nand2  g044(.a(new_n96_), .b(x18), .O(new_n97_));
  nand2  g045(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand2  g046(.a(new_n98_), .b(new_n53_), .O(new_n99_));
  inv1   g047(.a(x26), .O(new_n100_));
  oai21  g048(.a(new_n100_), .b(x18), .c(new_n60_), .O(new_n101_));
  nand2  g049(.a(new_n101_), .b(new_n99_), .O(z06));
  nand3  g050(.a(x19), .b(new_n54_), .c(x04), .O(new_n103_));
  nand2  g051(.a(new_n61_), .b(x18), .O(new_n104_));
  nand2  g052(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g053(.a(new_n105_), .b(new_n53_), .O(new_n106_));
  oai21  g054(.a(new_n96_), .b(x18), .c(new_n60_), .O(new_n107_));
  nand2  g055(.a(new_n107_), .b(new_n106_), .O(z07));
  nand3  g056(.a(x19), .b(new_n54_), .c(x11), .O(new_n109_));
  inv1   g057(.a(x29), .O(new_n110_));
  nand2  g058(.a(new_n110_), .b(x18), .O(new_n111_));
  nand2  g059(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g060(.a(new_n112_), .b(new_n53_), .O(new_n113_));
  inv1   g061(.a(x28), .O(new_n114_));
  oai21  g062(.a(new_n114_), .b(x18), .c(new_n60_), .O(new_n115_));
  nand2  g063(.a(new_n115_), .b(new_n113_), .O(z08));
  nand3  g064(.a(x19), .b(new_n54_), .c(x10), .O(new_n117_));
  inv1   g065(.a(x30), .O(new_n118_));
  nand2  g066(.a(new_n118_), .b(x18), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g068(.a(new_n120_), .b(new_n53_), .O(new_n121_));
  oai21  g069(.a(new_n110_), .b(x18), .c(new_n60_), .O(new_n122_));
  nand2  g070(.a(new_n122_), .b(new_n121_), .O(z09));
  nand2  g071(.a(x31), .b(x18), .O(new_n124_));
  inv1   g072(.a(x09), .O(new_n125_));
  nand2  g073(.a(new_n54_), .b(new_n125_), .O(new_n126_));
  nand3  g074(.a(new_n126_), .b(new_n124_), .c(new_n73_), .O(new_n127_));
  oai21  g075(.a(new_n71_), .b(x30), .c(new_n127_), .O(z10));
  nand3  g076(.a(x19), .b(new_n54_), .c(x08), .O(new_n129_));
  inv1   g077(.a(x24), .O(new_n130_));
  nand2  g078(.a(new_n130_), .b(x18), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand2  g080(.a(new_n132_), .b(new_n53_), .O(new_n133_));
  inv1   g081(.a(x31), .O(new_n134_));
  oai21  g082(.a(new_n134_), .b(x18), .c(new_n60_), .O(new_n135_));
  nand2  g083(.a(new_n135_), .b(new_n133_), .O(z11));
  nand3  g084(.a(x19), .b(new_n54_), .c(x15), .O(new_n137_));
  inv1   g085(.a(x33), .O(new_n138_));
  nand2  g086(.a(new_n138_), .b(x18), .O(new_n139_));
  nand2  g087(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  nand2  g088(.a(new_n140_), .b(new_n53_), .O(new_n141_));
  inv1   g089(.a(x32), .O(new_n142_));
  oai21  g090(.a(new_n142_), .b(x18), .c(new_n60_), .O(new_n143_));
  nand2  g091(.a(new_n143_), .b(new_n141_), .O(z12));
  nand2  g092(.a(x34), .b(x18), .O(new_n145_));
  inv1   g093(.a(x14), .O(new_n146_));
  nand2  g094(.a(new_n54_), .b(new_n146_), .O(new_n147_));
  nand3  g095(.a(new_n147_), .b(new_n145_), .c(new_n73_), .O(new_n148_));
  oai21  g096(.a(new_n71_), .b(x33), .c(new_n148_), .O(z13));
  nand3  g097(.a(x19), .b(new_n54_), .c(x13), .O(new_n150_));
  inv1   g098(.a(x35), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(x18), .O(new_n152_));
  nand2  g100(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g101(.a(new_n153_), .b(new_n53_), .O(new_n154_));
  inv1   g102(.a(x34), .O(new_n155_));
  oai21  g103(.a(new_n155_), .b(x18), .c(new_n60_), .O(new_n156_));
  nand2  g104(.a(new_n156_), .b(new_n154_), .O(z14));
  nand3  g105(.a(x19), .b(new_n54_), .c(x12), .O(new_n158_));
  nand2  g106(.a(new_n114_), .b(x18), .O(new_n159_));
  nand2  g107(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g108(.a(new_n160_), .b(new_n53_), .O(new_n161_));
  oai21  g109(.a(new_n151_), .b(x18), .c(new_n60_), .O(new_n162_));
  nand2  g110(.a(new_n162_), .b(new_n161_), .O(z15));
endmodule


