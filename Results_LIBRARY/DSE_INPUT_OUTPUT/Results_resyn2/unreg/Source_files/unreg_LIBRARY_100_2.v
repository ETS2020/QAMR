// Benchmark "FAU" written by ABC on Tue Aug 11 20:00:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_;
  inv1   g000(.a(x17), .O(new_n53_));
  inv1   g001(.a(x18), .O(new_n54_));
  nand3  g002(.a(x19), .b(new_n54_), .c(x03), .O(new_n55_));
  inv1   g003(.a(x21), .O(new_n56_));
  nand2  g004(.a(new_n56_), .b(x18), .O(new_n57_));
  nand2  g005(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g006(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  inv1   g007(.a(x19), .O(new_n60_));
  inv1   g008(.a(x20), .O(new_n61_));
  oai21  g009(.a(new_n61_), .b(x18), .c(new_n60_), .O(new_n62_));
  nand2  g010(.a(new_n62_), .b(new_n59_), .O(z00));
  nand2  g011(.a(new_n60_), .b(new_n54_), .O(new_n64_));
  nand2  g012(.a(x22), .b(x18), .O(new_n65_));
  nor2   g013(.a(new_n60_), .b(x17), .O(new_n66_));
  inv1   g014(.a(x02), .O(new_n67_));
  nand2  g015(.a(new_n54_), .b(new_n67_), .O(new_n68_));
  nand3  g016(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(new_n69_));
  oai21  g017(.a(new_n64_), .b(x21), .c(new_n69_), .O(z01));
  nand2  g018(.a(x23), .b(x18), .O(new_n71_));
  inv1   g019(.a(x01), .O(new_n72_));
  nand2  g020(.a(new_n54_), .b(new_n72_), .O(new_n73_));
  nand3  g021(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(new_n74_));
  oai21  g022(.a(new_n64_), .b(x22), .c(new_n74_), .O(z02));
  inv1   g023(.a(x16), .O(new_n76_));
  nand2  g024(.a(x18), .b(new_n76_), .O(new_n77_));
  inv1   g025(.a(x00), .O(new_n78_));
  nand2  g026(.a(new_n54_), .b(new_n78_), .O(new_n79_));
  nand3  g027(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  oai21  g028(.a(new_n64_), .b(x23), .c(new_n80_), .O(z03));
  nand2  g029(.a(x25), .b(x18), .O(new_n82_));
  inv1   g030(.a(x07), .O(new_n83_));
  nand2  g031(.a(new_n54_), .b(new_n83_), .O(new_n84_));
  nand3  g032(.a(new_n84_), .b(new_n82_), .c(new_n66_), .O(new_n85_));
  oai21  g033(.a(new_n64_), .b(x24), .c(new_n85_), .O(z04));
  nand2  g034(.a(x26), .b(x18), .O(new_n87_));
  inv1   g035(.a(x06), .O(new_n88_));
  nand2  g036(.a(new_n54_), .b(new_n88_), .O(new_n89_));
  nand3  g037(.a(new_n89_), .b(new_n87_), .c(new_n66_), .O(new_n90_));
  oai21  g038(.a(new_n64_), .b(x25), .c(new_n90_), .O(z05));
  nand3  g039(.a(x19), .b(new_n54_), .c(x05), .O(new_n92_));
  inv1   g040(.a(x27), .O(new_n93_));
  nand2  g041(.a(new_n93_), .b(x18), .O(new_n94_));
  nand2  g042(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand2  g043(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  inv1   g044(.a(x26), .O(new_n97_));
  oai21  g045(.a(new_n97_), .b(x18), .c(new_n60_), .O(new_n98_));
  nand2  g046(.a(new_n98_), .b(new_n96_), .O(z06));
  nand3  g047(.a(x19), .b(new_n54_), .c(x04), .O(new_n100_));
  nand2  g048(.a(new_n61_), .b(x18), .O(new_n101_));
  nand2  g049(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g050(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  oai21  g051(.a(new_n93_), .b(x18), .c(new_n60_), .O(new_n104_));
  nand2  g052(.a(new_n104_), .b(new_n103_), .O(z07));
  nand3  g053(.a(x19), .b(new_n54_), .c(x11), .O(new_n106_));
  inv1   g054(.a(x29), .O(new_n107_));
  nand2  g055(.a(new_n107_), .b(x18), .O(new_n108_));
  nand2  g056(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g057(.a(new_n109_), .b(new_n53_), .O(new_n110_));
  inv1   g058(.a(x28), .O(new_n111_));
  oai21  g059(.a(new_n111_), .b(x18), .c(new_n60_), .O(new_n112_));
  nand2  g060(.a(new_n112_), .b(new_n110_), .O(z08));
  nand3  g061(.a(x19), .b(new_n54_), .c(x10), .O(new_n114_));
  inv1   g062(.a(x30), .O(new_n115_));
  nand2  g063(.a(new_n115_), .b(x18), .O(new_n116_));
  nand2  g064(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g065(.a(new_n117_), .b(new_n53_), .O(new_n118_));
  oai21  g066(.a(new_n107_), .b(x18), .c(new_n60_), .O(new_n119_));
  nand2  g067(.a(new_n119_), .b(new_n118_), .O(z09));
  nand3  g068(.a(x19), .b(new_n54_), .c(x09), .O(new_n121_));
  inv1   g069(.a(x31), .O(new_n122_));
  nand2  g070(.a(new_n122_), .b(x18), .O(new_n123_));
  nand2  g071(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g072(.a(new_n124_), .b(new_n53_), .O(new_n125_));
  oai21  g073(.a(new_n115_), .b(x18), .c(new_n60_), .O(new_n126_));
  nand2  g074(.a(new_n126_), .b(new_n125_), .O(z10));
  nand3  g075(.a(x19), .b(new_n54_), .c(x08), .O(new_n128_));
  inv1   g076(.a(x24), .O(new_n129_));
  nand2  g077(.a(new_n129_), .b(x18), .O(new_n130_));
  nand2  g078(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand2  g079(.a(new_n131_), .b(new_n53_), .O(new_n132_));
  oai21  g080(.a(new_n122_), .b(x18), .c(new_n60_), .O(new_n133_));
  nand2  g081(.a(new_n133_), .b(new_n132_), .O(z11));
  nand3  g082(.a(x19), .b(new_n54_), .c(x15), .O(new_n135_));
  inv1   g083(.a(x33), .O(new_n136_));
  nand2  g084(.a(new_n136_), .b(x18), .O(new_n137_));
  nand2  g085(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g086(.a(new_n138_), .b(new_n53_), .O(new_n139_));
  inv1   g087(.a(x32), .O(new_n140_));
  oai21  g088(.a(new_n140_), .b(x18), .c(new_n60_), .O(new_n141_));
  nand2  g089(.a(new_n141_), .b(new_n139_), .O(z12));
  nand2  g090(.a(x34), .b(x18), .O(new_n143_));
  inv1   g091(.a(x14), .O(new_n144_));
  nand2  g092(.a(new_n54_), .b(new_n144_), .O(new_n145_));
  nand3  g093(.a(new_n145_), .b(new_n143_), .c(new_n66_), .O(new_n146_));
  oai21  g094(.a(new_n64_), .b(x33), .c(new_n146_), .O(z13));
  nand3  g095(.a(x19), .b(new_n54_), .c(x13), .O(new_n148_));
  inv1   g096(.a(x35), .O(new_n149_));
  nand2  g097(.a(new_n149_), .b(x18), .O(new_n150_));
  nand2  g098(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g099(.a(new_n151_), .b(new_n53_), .O(new_n152_));
  inv1   g100(.a(x34), .O(new_n153_));
  oai21  g101(.a(new_n153_), .b(x18), .c(new_n60_), .O(new_n154_));
  nand2  g102(.a(new_n154_), .b(new_n152_), .O(z14));
  nand2  g103(.a(x28), .b(x18), .O(new_n156_));
  inv1   g104(.a(x12), .O(new_n157_));
  nand2  g105(.a(new_n54_), .b(new_n157_), .O(new_n158_));
  nand3  g106(.a(new_n158_), .b(new_n156_), .c(new_n66_), .O(new_n159_));
  oai21  g107(.a(new_n64_), .b(x35), .c(new_n159_), .O(z15));
endmodule


