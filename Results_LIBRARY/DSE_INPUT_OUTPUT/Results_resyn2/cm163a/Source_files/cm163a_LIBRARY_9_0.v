// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_;
  nor2   g00(.a(x10), .b(x03), .O(new_n22_));
  inv1   g01(.a(x02), .O(new_n23_));
  inv1   g02(.a(x03), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand3  g05(.a(x09), .b(x03), .c(x02), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(new_n26_), .c(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(new_n28_), .c(new_n22_), .O(z0));
  nor2   g12(.a(x09), .b(new_n23_), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(x11), .c(x03), .O(new_n35_));
  inv1   g14(.a(x11), .O(new_n36_));
  nand3  g15(.a(new_n25_), .b(x03), .c(x02), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(new_n35_), .c(x04), .O(new_n39_));
  inv1   g18(.a(x01), .O(new_n40_));
  aoi21  g19(.a(new_n30_), .b(new_n40_), .c(new_n31_), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n39_), .c(new_n22_), .O(z1));
  inv1   g21(.a(x12), .O(new_n43_));
  aoi21  g22(.a(new_n34_), .b(new_n36_), .c(new_n43_), .O(new_n44_));
  nand2  g23(.a(new_n43_), .b(new_n36_), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(new_n37_), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n44_), .c(x04), .O(new_n47_));
  oai21  g26(.a(new_n43_), .b(new_n30_), .c(x10), .O(new_n48_));
  oai21  g27(.a(x06), .b(x04), .c(x05), .O(new_n49_));
  aoi21  g28(.a(new_n48_), .b(new_n24_), .c(new_n49_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n47_), .O(z2));
  oai22  g30(.a(new_n45_), .b(new_n37_), .c(x13), .d(new_n31_), .O(new_n52_));
  oai21  g31(.a(x05), .b(new_n24_), .c(x13), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n46_), .c(new_n30_), .O(new_n54_));
  inv1   g33(.a(new_n22_), .O(new_n55_));
  nand3  g34(.a(x07), .b(x05), .c(new_n30_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  aoi21  g36(.a(new_n54_), .b(new_n52_), .c(new_n57_), .O(z3));
  nand2  g37(.a(x08), .b(x03), .O(new_n59_));
  nand3  g38(.a(x15), .b(x14), .c(x10), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(new_n55_), .O(z4));
endmodule


