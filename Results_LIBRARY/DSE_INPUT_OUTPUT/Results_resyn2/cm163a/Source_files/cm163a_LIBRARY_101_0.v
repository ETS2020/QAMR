// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x09), .O(new_n22_));
  nor2   g01(.a(new_n22_), .b(x03), .O(new_n23_));
  nand3  g02(.a(new_n22_), .b(x03), .c(x02), .O(new_n24_));
  inv1   g03(.a(new_n24_), .O(new_n25_));
  oai21  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  inv1   g05(.a(x02), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  oai21  g07(.a(new_n22_), .b(new_n28_), .c(x10), .O(new_n29_));
  oai21  g08(.a(x04), .b(x00), .c(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n27_), .c(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n26_), .O(z0));
  aoi21  g11(.a(x11), .b(new_n27_), .c(x05), .O(new_n33_));
  nor2   g12(.a(x10), .b(x02), .O(new_n34_));
  aoi21  g13(.a(x05), .b(x01), .c(new_n34_), .O(new_n35_));
  oai21  g14(.a(new_n33_), .b(new_n28_), .c(new_n35_), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand2  g16(.a(new_n24_), .b(new_n37_), .O(new_n38_));
  nand2  g17(.a(new_n25_), .b(x11), .O(new_n39_));
  nor2   g18(.a(new_n34_), .b(new_n28_), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n39_), .c(new_n38_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n36_), .O(z1));
  nand3  g21(.a(new_n37_), .b(new_n22_), .c(x03), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x12), .O(new_n44_));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n22_), .c(x03), .d(x02), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x04), .O(new_n48_));
  nand2  g27(.a(x12), .b(x04), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x10), .O(new_n50_));
  oai21  g29(.a(x06), .b(x04), .c(x05), .O(new_n51_));
  aoi21  g30(.a(new_n50_), .b(new_n27_), .c(new_n51_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n48_), .O(z2));
  inv1   g32(.a(x13), .O(new_n54_));
  nand2  g33(.a(new_n46_), .b(new_n54_), .O(new_n55_));
  nand4  g34(.a(new_n45_), .b(x13), .c(new_n22_), .d(x03), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n55_), .c(x04), .O(new_n57_));
  oai21  g36(.a(new_n54_), .b(new_n28_), .c(x10), .O(new_n58_));
  oai21  g37(.a(x07), .b(x04), .c(x05), .O(new_n59_));
  aoi21  g38(.a(new_n58_), .b(new_n27_), .c(new_n59_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n57_), .O(z3));
  nand2  g40(.a(x08), .b(x03), .O(new_n62_));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  oai22  g42(.a(new_n63_), .b(new_n62_), .c(x10), .d(x02), .O(z4));
endmodule


