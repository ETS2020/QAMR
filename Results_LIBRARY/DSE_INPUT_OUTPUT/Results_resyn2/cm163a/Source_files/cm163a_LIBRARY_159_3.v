// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_;
  inv1   g00(.a(x09), .O(new_n22_));
  aoi21  g01(.a(x03), .b(x02), .c(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(x09), .O(new_n25_));
  oai21  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  nor2   g05(.a(x04), .b(x00), .O(new_n27_));
  inv1   g06(.a(x12), .O(new_n28_));
  inv1   g07(.a(x14), .O(new_n29_));
  oai21  g08(.a(new_n29_), .b(new_n28_), .c(x05), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n26_), .O(z0));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n22_), .c(x03), .d(x02), .O(new_n34_));
  oai21  g13(.a(new_n25_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  nor2   g15(.a(x04), .b(x01), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n36_), .O(z1));
  nor2   g18(.a(new_n29_), .b(new_n28_), .O(new_n40_));
  xor2a  g19(.a(new_n34_), .b(new_n28_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x04), .O(new_n42_));
  inv1   g21(.a(x04), .O(new_n43_));
  inv1   g22(.a(x05), .O(new_n44_));
  inv1   g23(.a(x06), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n43_), .c(new_n44_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n42_), .c(new_n40_), .O(z2));
  inv1   g26(.a(new_n34_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(x13), .c(new_n28_), .O(new_n49_));
  inv1   g28(.a(x13), .O(new_n50_));
  oai21  g29(.a(new_n34_), .b(x12), .c(new_n50_), .O(new_n51_));
  nor2   g30(.a(new_n40_), .b(new_n43_), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  aoi21  g32(.a(x13), .b(x12), .c(x05), .O(new_n54_));
  aoi21  g33(.a(x07), .b(x05), .c(new_n40_), .O(new_n55_));
  oai21  g34(.a(new_n54_), .b(new_n43_), .c(new_n55_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n53_), .O(z3));
  nand4  g36(.a(x15), .b(x10), .c(x08), .d(x03), .O(new_n58_));
  aoi21  g37(.a(new_n58_), .b(new_n28_), .c(new_n29_), .O(z4));
endmodule


