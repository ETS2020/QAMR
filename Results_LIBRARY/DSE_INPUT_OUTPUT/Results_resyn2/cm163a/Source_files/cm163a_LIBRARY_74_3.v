// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nor2   g04(.a(new_n23_), .b(x09), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n25_), .c(x04), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  nand2  g07(.a(x12), .b(x08), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n27_), .O(z0));
  inv1   g11(.a(x04), .O(new_n33_));
  nand3  g12(.a(new_n22_), .b(x03), .c(x02), .O(new_n34_));
  nor2   g13(.a(x11), .b(x09), .O(new_n35_));
  aoi22  g14(.a(new_n35_), .b(new_n24_), .c(new_n34_), .d(x11), .O(new_n36_));
  nor2   g15(.a(x04), .b(x01), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n30_), .O(new_n38_));
  oai21  g17(.a(new_n36_), .b(new_n33_), .c(new_n38_), .O(z1));
  oai21  g18(.a(x06), .b(x04), .c(x05), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n29_), .O(new_n41_));
  inv1   g20(.a(x11), .O(new_n42_));
  nand4  g21(.a(new_n42_), .b(new_n22_), .c(x03), .d(x02), .O(new_n43_));
  inv1   g22(.a(x12), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(x08), .O(new_n45_));
  nor2   g24(.a(x12), .b(x11), .O(new_n46_));
  aoi22  g25(.a(new_n46_), .b(new_n26_), .c(new_n45_), .d(new_n43_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n33_), .c(new_n41_), .O(z2));
  nor3   g27(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  aoi22  g28(.a(new_n49_), .b(new_n26_), .c(new_n43_), .d(x13), .O(new_n50_));
  inv1   g29(.a(x08), .O(new_n51_));
  nand2  g30(.a(x13), .b(x04), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g32(.a(x07), .b(x04), .c(x05), .O(new_n54_));
  aoi21  g33(.a(new_n53_), .b(x12), .c(new_n54_), .O(new_n55_));
  oai21  g34(.a(new_n50_), .b(new_n33_), .c(new_n55_), .O(z3));
  nand4  g35(.a(x15), .b(x14), .c(x10), .d(x03), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n44_), .c(new_n51_), .O(z4));
endmodule


