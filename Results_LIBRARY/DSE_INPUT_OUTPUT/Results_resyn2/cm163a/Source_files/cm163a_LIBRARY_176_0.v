// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x09), .O(new_n22_));
  inv1   g01(.a(x05), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  aoi21  g03(.a(new_n24_), .b(x04), .c(new_n23_), .O(new_n25_));
  inv1   g04(.a(x04), .O(new_n26_));
  nor2   g05(.a(new_n24_), .b(x09), .O(new_n27_));
  aoi21  g06(.a(new_n26_), .b(x00), .c(new_n23_), .O(new_n28_));
  oai21  g07(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  oai21  g08(.a(new_n25_), .b(new_n22_), .c(new_n29_), .O(z0));
  nand2  g09(.a(new_n26_), .b(x01), .O(new_n31_));
  inv1   g10(.a(x11), .O(new_n32_));
  nand4  g11(.a(new_n32_), .b(new_n22_), .c(x03), .d(x02), .O(new_n33_));
  nand3  g12(.a(new_n22_), .b(x03), .c(x02), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x11), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n33_), .c(x04), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n31_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x05), .O(z1));
  nor2   g17(.a(x12), .b(x11), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n27_), .c(new_n33_), .d(x12), .O(new_n40_));
  inv1   g19(.a(x06), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n26_), .c(new_n23_), .O(new_n42_));
  oai21  g21(.a(new_n40_), .b(new_n26_), .c(new_n42_), .O(z2));
  and2   g22(.a(x03), .b(x02), .O(new_n44_));
  and2   g23(.a(x13), .b(x04), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n39_), .c(new_n44_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(x05), .c(x09), .O(new_n47_));
  inv1   g26(.a(x13), .O(new_n48_));
  inv1   g27(.a(x12), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n32_), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n34_), .c(new_n48_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x04), .O(new_n52_));
  inv1   g31(.a(x07), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n26_), .c(new_n23_), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n52_), .c(new_n47_), .O(z3));
  nand3  g34(.a(x15), .b(x14), .c(x10), .O(new_n56_));
  nor2   g35(.a(x09), .b(x05), .O(new_n57_));
  nand2  g36(.a(x08), .b(x03), .O(new_n58_));
  nor3   g37(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(z4));
endmodule


