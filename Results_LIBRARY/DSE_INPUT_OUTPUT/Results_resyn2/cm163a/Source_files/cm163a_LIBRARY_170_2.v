// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_;
  inv1   g00(.a(x02), .O(new_n22_));
  nand3  g01(.a(x09), .b(x04), .c(x03), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x05), .c(new_n22_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(new_n28_), .c(new_n24_), .O(z0));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n25_), .b(x03), .c(x02), .O(new_n35_));
  inv1   g14(.a(new_n35_), .O(new_n36_));
  nor2   g15(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g16(.a(new_n36_), .b(new_n34_), .O(new_n38_));
  inv1   g17(.a(new_n38_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n37_), .c(x04), .O(new_n40_));
  inv1   g19(.a(x01), .O(new_n41_));
  aoi21  g20(.a(new_n30_), .b(new_n41_), .c(new_n31_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n40_), .O(z1));
  nor2   g22(.a(x12), .b(x11), .O(new_n44_));
  aoi22  g23(.a(new_n44_), .b(new_n36_), .c(new_n38_), .d(x12), .O(new_n45_));
  inv1   g24(.a(x06), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n30_), .c(new_n31_), .O(new_n47_));
  oai21  g26(.a(new_n45_), .b(new_n30_), .c(new_n47_), .O(z2));
  inv1   g27(.a(x13), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n30_), .c(x05), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  nor2   g30(.a(x07), .b(x04), .O(new_n52_));
  inv1   g31(.a(new_n44_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n35_), .c(new_n49_), .O(new_n54_));
  nand3  g33(.a(x13), .b(new_n25_), .c(x03), .O(new_n55_));
  inv1   g34(.a(new_n55_), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n44_), .c(new_n30_), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n54_), .c(new_n52_), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n31_), .c(new_n51_), .O(z3));
  nand2  g38(.a(x08), .b(x03), .O(new_n60_));
  aoi21  g39(.a(new_n31_), .b(x02), .c(new_n60_), .O(new_n61_));
  nand4  g40(.a(new_n61_), .b(x15), .c(x14), .d(x10), .O(new_n62_));
  inv1   g41(.a(new_n62_), .O(z4));
endmodule


