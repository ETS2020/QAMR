// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x05), .b(new_n22_), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  aoi21  g03(.a(x03), .b(x02), .c(new_n24_), .O(new_n25_));
  nand3  g04(.a(new_n24_), .b(x03), .c(x02), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x05), .O(new_n27_));
  oai21  g06(.a(new_n27_), .b(new_n25_), .c(x04), .O(new_n28_));
  oai21  g07(.a(new_n23_), .b(x00), .c(new_n28_), .O(z0));
  inv1   g08(.a(x11), .O(new_n30_));
  nand4  g09(.a(new_n30_), .b(new_n24_), .c(x03), .d(x02), .O(new_n31_));
  inv1   g10(.a(new_n31_), .O(new_n32_));
  aoi21  g11(.a(new_n26_), .b(x11), .c(new_n32_), .O(new_n33_));
  inv1   g12(.a(x01), .O(new_n34_));
  inv1   g13(.a(x05), .O(new_n35_));
  aoi21  g14(.a(new_n22_), .b(new_n34_), .c(new_n35_), .O(new_n36_));
  oai21  g15(.a(new_n33_), .b(new_n22_), .c(new_n36_), .O(z1));
  or2    g16(.a(new_n23_), .b(x06), .O(new_n38_));
  nand2  g17(.a(new_n31_), .b(x12), .O(new_n39_));
  inv1   g18(.a(x12), .O(new_n40_));
  nor2   g19(.a(x11), .b(x09), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n40_), .c(x03), .d(x02), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n39_), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x04), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n38_), .O(z2));
  inv1   g24(.a(x13), .O(new_n46_));
  nor2   g25(.a(new_n42_), .b(new_n46_), .O(new_n47_));
  nand2  g26(.a(new_n42_), .b(new_n46_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(x04), .O(new_n49_));
  inv1   g28(.a(x07), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n22_), .c(new_n35_), .O(new_n51_));
  oai21  g30(.a(new_n49_), .b(new_n47_), .c(new_n51_), .O(z3));
  nand3  g31(.a(x15), .b(x14), .c(x10), .O(new_n53_));
  nor2   g32(.a(x05), .b(x04), .O(new_n54_));
  nand2  g33(.a(x08), .b(x03), .O(new_n55_));
  nor3   g34(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(z4));
endmodule


