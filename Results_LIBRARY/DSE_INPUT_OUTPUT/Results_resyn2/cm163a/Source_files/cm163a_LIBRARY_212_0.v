// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_;
  nand2  g00(.a(x04), .b(x03), .O(new_n22_));
  oai21  g01(.a(new_n22_), .b(x09), .c(x05), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x02), .O(new_n24_));
  and2   g03(.a(x03), .b(x02), .O(new_n25_));
  nand2  g04(.a(x09), .b(x04), .O(new_n26_));
  oai22  g05(.a(new_n26_), .b(new_n25_), .c(x04), .d(x00), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x05), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n24_), .O(z0));
  inv1   g08(.a(x02), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  inv1   g10(.a(x03), .O(new_n32_));
  inv1   g11(.a(x04), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nor2   g13(.a(x11), .b(x09), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n34_), .c(new_n31_), .O(new_n36_));
  nor2   g15(.a(x04), .b(x01), .O(new_n37_));
  inv1   g16(.a(x09), .O(new_n38_));
  nand2  g17(.a(new_n25_), .b(new_n38_), .O(new_n39_));
  and2   g18(.a(x11), .b(x04), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n39_), .c(new_n37_), .O(new_n41_));
  oai22  g20(.a(new_n41_), .b(new_n31_), .c(new_n36_), .d(new_n30_), .O(z1));
  inv1   g21(.a(x12), .O(new_n43_));
  nand4  g22(.a(new_n35_), .b(new_n43_), .c(x04), .d(x03), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x05), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x02), .O(new_n46_));
  aoi21  g25(.a(new_n35_), .b(new_n25_), .c(new_n43_), .O(new_n47_));
  aoi21  g26(.a(x06), .b(new_n33_), .c(new_n31_), .O(new_n48_));
  oai21  g27(.a(new_n47_), .b(new_n33_), .c(new_n48_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n46_), .O(z2));
  oai21  g29(.a(new_n44_), .b(x13), .c(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x02), .O(new_n52_));
  nand4  g31(.a(new_n35_), .b(new_n43_), .c(x03), .d(x02), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(x13), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x04), .O(new_n55_));
  aoi21  g34(.a(x07), .b(new_n33_), .c(new_n31_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n52_), .O(z3));
  nand3  g37(.a(x15), .b(x14), .c(x10), .O(new_n59_));
  nor2   g38(.a(x05), .b(x02), .O(new_n60_));
  nand2  g39(.a(x08), .b(x03), .O(new_n61_));
  nor3   g40(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z4));
endmodule


