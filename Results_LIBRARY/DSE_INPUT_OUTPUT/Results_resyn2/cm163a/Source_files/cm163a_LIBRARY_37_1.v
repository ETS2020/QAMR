// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x09), .O(new_n22_));
  aoi21  g01(.a(x03), .b(x02), .c(new_n22_), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(x09), .O(new_n25_));
  oai21  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  nor2   g05(.a(x04), .b(x00), .O(new_n27_));
  nand2  g06(.a(x13), .b(new_n22_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n26_), .O(z0));
  inv1   g10(.a(new_n28_), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n22_), .c(x03), .d(x02), .O(new_n34_));
  oai21  g13(.a(new_n24_), .b(x09), .c(x11), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x04), .O(new_n37_));
  inv1   g16(.a(x01), .O(new_n38_));
  inv1   g17(.a(x04), .O(new_n39_));
  inv1   g18(.a(x05), .O(new_n40_));
  aoi21  g19(.a(new_n39_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n37_), .c(new_n32_), .O(z1));
  inv1   g21(.a(x12), .O(new_n43_));
  nor2   g22(.a(new_n43_), .b(new_n39_), .O(new_n44_));
  oai21  g23(.a(new_n34_), .b(x13), .c(new_n44_), .O(new_n45_));
  inv1   g24(.a(x13), .O(new_n46_));
  nor2   g25(.a(x12), .b(x11), .O(new_n47_));
  inv1   g26(.a(new_n47_), .O(new_n48_));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  oai21  g28(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n22_), .O(new_n51_));
  inv1   g30(.a(x06), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n39_), .c(new_n40_), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n51_), .c(new_n45_), .O(z2));
  aoi21  g33(.a(new_n47_), .b(new_n25_), .c(x13), .O(new_n55_));
  nor2   g34(.a(x07), .b(x04), .O(new_n56_));
  nor2   g35(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  oai21  g36(.a(new_n55_), .b(new_n39_), .c(new_n57_), .O(z3));
  nand2  g37(.a(x08), .b(x03), .O(new_n59_));
  nand3  g38(.a(x15), .b(x14), .c(x10), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(new_n28_), .O(z4));
endmodule


