// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(x15), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(x03), .c(x02), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(new_n24_), .O(new_n27_));
  aoi21  g06(.a(new_n23_), .b(x09), .c(new_n27_), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n22_), .b(new_n29_), .c(new_n30_), .O(new_n31_));
  oai21  g10(.a(new_n28_), .b(new_n22_), .c(new_n31_), .O(z0));
  nand2  g11(.a(x05), .b(x01), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  oai21  g13(.a(new_n23_), .b(x09), .c(x11), .O(new_n35_));
  inv1   g14(.a(x11), .O(new_n36_));
  nand4  g15(.a(new_n36_), .b(new_n25_), .c(x03), .d(x02), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n35_), .c(new_n22_), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n30_), .c(new_n24_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n34_), .O(z1));
  nand2  g19(.a(new_n37_), .b(x12), .O(new_n41_));
  nor2   g20(.a(new_n23_), .b(x09), .O(new_n42_));
  nor2   g21(.a(x12), .b(x11), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand3  g23(.a(new_n44_), .b(new_n41_), .c(new_n24_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x04), .O(new_n46_));
  inv1   g25(.a(x06), .O(new_n47_));
  aoi21  g26(.a(new_n47_), .b(new_n22_), .c(new_n30_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(z2));
  inv1   g28(.a(x13), .O(new_n50_));
  nand3  g29(.a(new_n43_), .b(new_n42_), .c(new_n50_), .O(new_n51_));
  inv1   g30(.a(x12), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n36_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n26_), .c(x13), .O(new_n54_));
  nand3  g33(.a(new_n54_), .b(new_n51_), .c(new_n24_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x04), .O(new_n56_));
  inv1   g35(.a(x07), .O(new_n57_));
  aoi21  g36(.a(new_n57_), .b(new_n22_), .c(new_n30_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n56_), .O(z3));
  nand2  g38(.a(new_n22_), .b(x03), .O(new_n60_));
  nand4  g39(.a(x15), .b(x14), .c(x10), .d(x08), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n60_), .O(z4));
endmodule


