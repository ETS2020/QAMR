// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x05), .O(new_n25_));
  nor2   g04(.a(x04), .b(x00), .O(new_n26_));
  nor2   g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  oai21  g06(.a(new_n23_), .b(new_n22_), .c(new_n27_), .O(z0));
  inv1   g07(.a(x11), .O(new_n29_));
  nand3  g08(.a(new_n29_), .b(x03), .c(x02), .O(new_n30_));
  nand2  g09(.a(new_n23_), .b(x11), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n30_), .c(x04), .O(new_n32_));
  aoi21  g11(.a(new_n22_), .b(x01), .c(x09), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x05), .O(z1));
  xor2a  g14(.a(new_n30_), .b(x12), .O(new_n36_));
  nor2   g15(.a(x06), .b(x04), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n25_), .O(new_n38_));
  oai21  g17(.a(new_n36_), .b(new_n22_), .c(new_n38_), .O(z2));
  inv1   g18(.a(x12), .O(new_n40_));
  inv1   g19(.a(new_n23_), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(x13), .c(new_n40_), .d(new_n29_), .O(new_n42_));
  inv1   g21(.a(x13), .O(new_n43_));
  nand4  g22(.a(new_n40_), .b(new_n29_), .c(x03), .d(x02), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n42_), .c(x04), .O(new_n46_));
  nor2   g25(.a(x07), .b(x04), .O(new_n47_));
  nor2   g26(.a(new_n47_), .b(new_n25_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n46_), .O(z3));
  nand2  g28(.a(x09), .b(x05), .O(new_n50_));
  nand2  g29(.a(x08), .b(x03), .O(new_n51_));
  nand3  g30(.a(x15), .b(x14), .c(x10), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z4));
endmodule


