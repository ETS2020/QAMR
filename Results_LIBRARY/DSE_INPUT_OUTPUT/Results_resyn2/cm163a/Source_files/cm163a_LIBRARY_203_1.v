// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_;
  nand2  g00(.a(x03), .b(x02), .O(new_n22_));
  nor2   g01(.a(new_n22_), .b(x09), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(x03), .O(new_n25_));
  oai21  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  inv1   g05(.a(x02), .O(new_n27_));
  nand2  g06(.a(x09), .b(new_n27_), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(new_n28_), .c(new_n26_), .d(x05), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n28_), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  inv1   g14(.a(x03), .O(new_n36_));
  nor2   g15(.a(x09), .b(new_n36_), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n35_), .c(x02), .O(new_n38_));
  nand3  g17(.a(new_n24_), .b(x03), .c(x02), .O(new_n39_));
  nand3  g18(.a(new_n28_), .b(new_n39_), .c(x11), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x04), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n34_), .O(z1));
  nand3  g22(.a(new_n35_), .b(new_n24_), .c(x03), .O(new_n44_));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  aoi22  g24(.a(new_n45_), .b(new_n23_), .c(new_n44_), .d(x12), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n30_), .c(new_n24_), .O(new_n48_));
  oai21  g27(.a(x06), .b(x04), .c(x05), .O(new_n49_));
  aoi21  g28(.a(new_n48_), .b(new_n27_), .c(new_n49_), .O(new_n50_));
  oai21  g29(.a(new_n46_), .b(new_n30_), .c(new_n50_), .O(z2));
  inv1   g30(.a(x13), .O(new_n52_));
  nand4  g31(.a(new_n47_), .b(new_n35_), .c(x03), .d(x02), .O(new_n53_));
  nand2  g32(.a(x13), .b(x02), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n24_), .O(new_n55_));
  aoi21  g34(.a(new_n53_), .b(new_n52_), .c(new_n55_), .O(new_n56_));
  aoi21  g35(.a(new_n45_), .b(new_n37_), .c(new_n54_), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n56_), .c(x04), .O(new_n58_));
  aoi21  g37(.a(x13), .b(new_n27_), .c(x05), .O(new_n59_));
  aoi22  g38(.a(x09), .b(new_n27_), .c(x07), .d(x05), .O(new_n60_));
  oai21  g39(.a(new_n59_), .b(new_n30_), .c(new_n60_), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n58_), .O(z3));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  inv1   g42(.a(new_n63_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(new_n28_), .c(x08), .d(x03), .O(new_n65_));
  inv1   g44(.a(new_n65_), .O(z4));
endmodule


