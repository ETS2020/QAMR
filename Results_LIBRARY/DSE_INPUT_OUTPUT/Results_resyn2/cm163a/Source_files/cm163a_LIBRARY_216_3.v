// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_;
  and2   g00(.a(x15), .b(x12), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xnor2a g02(.a(new_n23_), .b(x09), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x04), .O(new_n25_));
  inv1   g04(.a(x00), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  inv1   g06(.a(x05), .O(new_n28_));
  aoi21  g07(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  aoi21  g08(.a(new_n29_), .b(new_n25_), .c(new_n22_), .O(z0));
  oai21  g09(.a(new_n23_), .b(x09), .c(x11), .O(new_n31_));
  nor2   g10(.a(x11), .b(x09), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(x03), .c(x02), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n31_), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x04), .O(new_n35_));
  inv1   g14(.a(x01), .O(new_n36_));
  aoi21  g15(.a(new_n27_), .b(new_n36_), .c(new_n28_), .O(new_n37_));
  aoi21  g16(.a(new_n37_), .b(new_n35_), .c(new_n22_), .O(z1));
  xor2a  g17(.a(new_n33_), .b(x12), .O(new_n39_));
  nor2   g18(.a(x06), .b(x04), .O(new_n40_));
  nor3   g19(.a(new_n40_), .b(new_n22_), .c(new_n28_), .O(new_n41_));
  oai21  g20(.a(new_n39_), .b(new_n27_), .c(new_n41_), .O(z2));
  inv1   g21(.a(x12), .O(new_n43_));
  inv1   g22(.a(new_n23_), .O(new_n44_));
  nand4  g23(.a(new_n32_), .b(new_n44_), .c(x13), .d(new_n43_), .O(new_n45_));
  inv1   g24(.a(x13), .O(new_n46_));
  oai21  g25(.a(new_n33_), .b(x12), .c(new_n46_), .O(new_n47_));
  nor2   g26(.a(new_n22_), .b(new_n27_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  aoi21  g28(.a(x13), .b(x12), .c(x05), .O(new_n50_));
  aoi21  g29(.a(x07), .b(x05), .c(new_n22_), .O(new_n51_));
  oai21  g30(.a(new_n50_), .b(new_n27_), .c(new_n51_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n49_), .O(z3));
  nand2  g32(.a(x08), .b(x03), .O(new_n54_));
  nand4  g33(.a(x15), .b(x14), .c(new_n43_), .d(x10), .O(new_n55_));
  nor2   g34(.a(new_n55_), .b(new_n54_), .O(z4));
endmodule


