// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_;
  nand2  g00(.a(x03), .b(x02), .O(new_n22_));
  xor2a  g01(.a(new_n22_), .b(x09), .O(new_n23_));
  nand2  g02(.a(x04), .b(x00), .O(new_n24_));
  oai21  g03(.a(new_n24_), .b(new_n23_), .c(x05), .O(z0));
  inv1   g04(.a(x04), .O(new_n26_));
  inv1   g05(.a(x09), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(x03), .c(x02), .O(new_n28_));
  xor2a  g07(.a(new_n28_), .b(x11), .O(new_n29_));
  inv1   g08(.a(x01), .O(new_n30_));
  nand2  g09(.a(x05), .b(x00), .O(new_n31_));
  aoi21  g10(.a(new_n26_), .b(new_n30_), .c(new_n31_), .O(new_n32_));
  oai21  g11(.a(new_n29_), .b(new_n26_), .c(new_n32_), .O(z1));
  oai21  g12(.a(new_n28_), .b(x11), .c(x12), .O(new_n34_));
  nor2   g13(.a(new_n22_), .b(x09), .O(new_n35_));
  nor2   g14(.a(x12), .b(x11), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n34_), .c(x04), .O(new_n38_));
  inv1   g17(.a(x00), .O(new_n39_));
  aoi21  g18(.a(x06), .b(new_n26_), .c(new_n39_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x05), .O(z2));
  inv1   g21(.a(x13), .O(new_n43_));
  nand3  g22(.a(new_n36_), .b(new_n35_), .c(new_n43_), .O(new_n44_));
  inv1   g23(.a(x11), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n28_), .c(x13), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n44_), .c(x04), .O(new_n49_));
  aoi21  g28(.a(x07), .b(new_n26_), .c(new_n39_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x05), .O(z3));
  nand2  g31(.a(x08), .b(x03), .O(new_n53_));
  aoi21  g32(.a(x05), .b(new_n39_), .c(new_n53_), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(x15), .c(x14), .d(x10), .O(new_n55_));
  inv1   g34(.a(new_n55_), .O(z4));
endmodule


