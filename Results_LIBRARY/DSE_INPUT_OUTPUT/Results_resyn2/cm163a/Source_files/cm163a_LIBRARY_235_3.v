// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_;
  inv1   g00(.a(x13), .O(new_n22_));
  nand2  g01(.a(x04), .b(x02), .O(new_n23_));
  oai21  g02(.a(new_n23_), .b(x09), .c(new_n22_), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x03), .O(new_n25_));
  nand3  g04(.a(new_n22_), .b(x03), .c(x02), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  inv1   g06(.a(x09), .O(new_n28_));
  nor2   g07(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  oai21  g08(.a(x04), .b(x00), .c(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n26_), .c(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n25_), .O(z0));
  inv1   g11(.a(x11), .O(new_n33_));
  nor2   g12(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  oai21  g13(.a(new_n26_), .b(x09), .c(new_n34_), .O(new_n35_));
  nand2  g14(.a(new_n33_), .b(new_n28_), .O(new_n36_));
  oai21  g15(.a(new_n36_), .b(new_n23_), .c(new_n22_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x03), .O(new_n38_));
  inv1   g17(.a(x01), .O(new_n39_));
  inv1   g18(.a(x05), .O(new_n40_));
  aoi21  g19(.a(new_n27_), .b(new_n39_), .c(new_n40_), .O(new_n41_));
  nand3  g20(.a(new_n41_), .b(new_n38_), .c(new_n35_), .O(z1));
  nand2  g21(.a(x13), .b(x03), .O(new_n43_));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand3  g25(.a(new_n33_), .b(new_n28_), .c(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n22_), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(x03), .c(new_n46_), .O(new_n49_));
  nor3   g28(.a(new_n36_), .b(new_n26_), .c(x12), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n45_), .O(z2));
  nor2   g31(.a(new_n22_), .b(x03), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  aoi21  g33(.a(new_n22_), .b(x04), .c(x07), .O(new_n55_));
  oai21  g34(.a(new_n55_), .b(new_n40_), .c(new_n43_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n54_), .O(z3));
  nand2  g36(.a(x08), .b(x03), .O(new_n58_));
  nand4  g37(.a(x15), .b(x14), .c(new_n22_), .d(x10), .O(new_n59_));
  nor2   g38(.a(new_n59_), .b(new_n58_), .O(z4));
endmodule


