// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x15), .O(new_n25_));
  nor2   g04(.a(new_n25_), .b(x09), .O(new_n26_));
  oai21  g05(.a(x04), .b(x00), .c(x05), .O(new_n27_));
  nor2   g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n28_), .O(z0));
  nor2   g08(.a(new_n23_), .b(x09), .O(new_n30_));
  xor2a  g09(.a(new_n30_), .b(x11), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x04), .O(new_n32_));
  inv1   g11(.a(x01), .O(new_n33_));
  inv1   g12(.a(x05), .O(new_n34_));
  aoi21  g13(.a(new_n22_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  aoi21  g14(.a(new_n35_), .b(new_n32_), .c(new_n26_), .O(z1));
  inv1   g15(.a(x11), .O(new_n37_));
  nand3  g16(.a(new_n30_), .b(new_n25_), .c(new_n37_), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(x12), .c(x04), .O(new_n39_));
  inv1   g18(.a(x09), .O(new_n40_));
  inv1   g19(.a(x12), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n37_), .c(x03), .d(x02), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(new_n22_), .c(new_n25_), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n40_), .O(new_n44_));
  inv1   g23(.a(x06), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n22_), .c(new_n34_), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(new_n44_), .c(new_n39_), .O(z2));
  oai21  g26(.a(x07), .b(x04), .c(x05), .O(new_n48_));
  oai21  g27(.a(new_n25_), .b(x09), .c(new_n48_), .O(new_n49_));
  inv1   g28(.a(x13), .O(new_n50_));
  nand2  g29(.a(new_n42_), .b(new_n25_), .O(new_n51_));
  aoi21  g30(.a(new_n51_), .b(new_n40_), .c(new_n50_), .O(new_n52_));
  nand2  g31(.a(new_n50_), .b(new_n41_), .O(new_n53_));
  nor2   g32(.a(new_n53_), .b(new_n38_), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n52_), .c(x04), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n49_), .O(z3));
  nand2  g35(.a(x08), .b(x03), .O(new_n57_));
  nand4  g36(.a(x15), .b(x14), .c(x10), .d(x09), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n57_), .O(z4));
endmodule


