// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_;
  inv1   g00(.a(x08), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  nand2  g02(.a(x05), .b(x00), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  inv1   g04(.a(x05), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x09), .O(new_n28_));
  inv1   g07(.a(x09), .O(new_n29_));
  nand3  g08(.a(new_n29_), .b(x03), .c(x02), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(x04), .c(new_n26_), .O(new_n32_));
  oai21  g11(.a(new_n32_), .b(new_n22_), .c(new_n25_), .O(z0));
  nand2  g12(.a(x05), .b(x01), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n23_), .O(new_n35_));
  nand2  g14(.a(new_n30_), .b(x11), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand4  g16(.a(new_n37_), .b(new_n29_), .c(x03), .d(x02), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n36_), .c(new_n23_), .O(new_n39_));
  oai21  g18(.a(new_n39_), .b(new_n26_), .c(x08), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n35_), .O(z1));
  nand2  g20(.a(new_n38_), .b(x12), .O(new_n42_));
  nor2   g21(.a(new_n27_), .b(x09), .O(new_n43_));
  nor2   g22(.a(x12), .b(x11), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n42_), .c(x08), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x04), .O(new_n47_));
  inv1   g26(.a(x06), .O(new_n48_));
  aoi21  g27(.a(new_n48_), .b(new_n23_), .c(new_n26_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n47_), .O(z2));
  nor2   g29(.a(x08), .b(new_n23_), .O(new_n51_));
  inv1   g30(.a(x12), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n37_), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n30_), .c(x13), .O(new_n54_));
  inv1   g33(.a(x13), .O(new_n55_));
  nand3  g34(.a(new_n44_), .b(new_n43_), .c(new_n55_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(x04), .O(new_n58_));
  inv1   g37(.a(x07), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(new_n23_), .c(new_n26_), .O(new_n60_));
  aoi21  g39(.a(new_n60_), .b(new_n58_), .c(new_n51_), .O(z3));
  nand2  g40(.a(x08), .b(x03), .O(new_n62_));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n62_), .O(z4));
endmodule


