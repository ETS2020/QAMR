// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x02), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  aoi21  g03(.a(new_n24_), .b(x03), .c(new_n23_), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  nor2   g05(.a(new_n24_), .b(new_n23_), .O(new_n27_));
  oai21  g06(.a(x04), .b(x00), .c(x05), .O(new_n28_));
  aoi21  g07(.a(new_n27_), .b(new_n26_), .c(new_n28_), .O(new_n29_));
  oai21  g08(.a(new_n25_), .b(new_n22_), .c(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n24_), .c(x03), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(x04), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(x02), .O(new_n34_));
  nand3  g13(.a(new_n24_), .b(x03), .c(x02), .O(new_n35_));
  nor2   g14(.a(new_n31_), .b(new_n23_), .O(new_n36_));
  oai21  g15(.a(x04), .b(x01), .c(x05), .O(new_n37_));
  aoi21  g16(.a(new_n36_), .b(new_n35_), .c(new_n37_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n34_), .O(z1));
  inv1   g18(.a(x12), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  oai21  g20(.a(new_n32_), .b(new_n22_), .c(new_n41_), .O(new_n42_));
  nor2   g21(.a(x12), .b(x11), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n24_), .c(x03), .d(x02), .O(new_n44_));
  inv1   g23(.a(x06), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(x02), .c(new_n23_), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(new_n44_), .c(new_n42_), .d(x05), .O(z2));
  nand2  g26(.a(new_n40_), .b(new_n31_), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n24_), .c(x03), .O(new_n50_));
  oai21  g29(.a(new_n50_), .b(new_n48_), .c(x04), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x02), .O(new_n52_));
  nor2   g31(.a(new_n49_), .b(new_n23_), .O(new_n53_));
  oai21  g32(.a(x07), .b(x04), .c(x05), .O(new_n54_));
  aoi21  g33(.a(new_n53_), .b(new_n44_), .c(new_n54_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n52_), .O(z3));
  nand2  g35(.a(x08), .b(x03), .O(new_n57_));
  aoi21  g36(.a(new_n23_), .b(x02), .c(new_n57_), .O(new_n58_));
  nand4  g37(.a(new_n58_), .b(x15), .c(x14), .d(x10), .O(new_n59_));
  inv1   g38(.a(new_n59_), .O(z4));
endmodule


