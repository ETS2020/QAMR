// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_;
  inv1   g00(.a(x03), .O(new_n22_));
  inv1   g01(.a(x13), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  aoi21  g04(.a(x03), .b(x02), .c(new_n25_), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nor2   g06(.a(new_n27_), .b(x09), .O(new_n28_));
  oai21  g07(.a(new_n28_), .b(new_n26_), .c(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  inv1   g10(.a(x05), .O(new_n32_));
  aoi21  g11(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  aoi21  g12(.a(new_n33_), .b(new_n29_), .c(new_n24_), .O(z0));
  nor2   g13(.a(x11), .b(x09), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(x04), .c(x02), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n23_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x03), .O(new_n38_));
  nor2   g17(.a(x13), .b(x09), .O(new_n39_));
  nand3  g18(.a(new_n39_), .b(x03), .c(x02), .O(new_n40_));
  inv1   g19(.a(x11), .O(new_n41_));
  nor2   g20(.a(new_n41_), .b(new_n31_), .O(new_n42_));
  oai21  g21(.a(x04), .b(x01), .c(x05), .O(new_n43_));
  aoi21  g22(.a(new_n42_), .b(new_n40_), .c(new_n43_), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n38_), .O(z1));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n35_), .b(new_n46_), .c(x04), .d(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n23_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(x03), .O(new_n49_));
  nand4  g28(.a(new_n39_), .b(new_n41_), .c(x03), .d(x02), .O(new_n50_));
  nor2   g29(.a(new_n46_), .b(new_n31_), .O(new_n51_));
  oai21  g30(.a(x06), .b(x04), .c(x05), .O(new_n52_));
  aoi21  g31(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n49_), .O(z2));
  nor2   g33(.a(new_n23_), .b(x03), .O(new_n55_));
  nor3   g34(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  aoi21  g35(.a(new_n56_), .b(new_n28_), .c(new_n55_), .O(new_n57_));
  oai21  g36(.a(x07), .b(x04), .c(x05), .O(new_n58_));
  oai21  g37(.a(new_n23_), .b(new_n22_), .c(new_n58_), .O(new_n59_));
  oai21  g38(.a(new_n57_), .b(new_n31_), .c(new_n59_), .O(z3));
  nand4  g39(.a(x15), .b(x14), .c(x10), .d(x08), .O(new_n61_));
  aoi21  g40(.a(new_n61_), .b(new_n23_), .c(new_n22_), .O(z4));
endmodule


