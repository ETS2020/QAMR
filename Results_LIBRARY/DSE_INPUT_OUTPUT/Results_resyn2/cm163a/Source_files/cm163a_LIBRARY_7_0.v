// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x00), .O(new_n25_));
  inv1   g04(.a(x05), .O(new_n26_));
  aoi21  g05(.a(new_n22_), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g06(.a(new_n24_), .b(new_n22_), .c(new_n27_), .O(z0));
  nor2   g07(.a(x12), .b(x05), .O(new_n29_));
  nand2  g08(.a(new_n22_), .b(x01), .O(new_n30_));
  oai21  g09(.a(new_n23_), .b(x09), .c(x11), .O(new_n31_));
  inv1   g10(.a(x09), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n32_), .c(x03), .d(x02), .O(new_n34_));
  nand3  g13(.a(new_n34_), .b(new_n31_), .c(x04), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  aoi21  g15(.a(new_n36_), .b(x05), .c(new_n29_), .O(z1));
  nand3  g16(.a(new_n32_), .b(x03), .c(x02), .O(new_n38_));
  nor2   g17(.a(x12), .b(x11), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(x04), .O(new_n40_));
  oai22  g19(.a(new_n40_), .b(new_n38_), .c(x06), .d(x04), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x05), .O(new_n42_));
  and2   g21(.a(x03), .b(x02), .O(new_n43_));
  nor2   g22(.a(x11), .b(x09), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n43_), .c(new_n22_), .O(new_n45_));
  oai21  g24(.a(new_n45_), .b(new_n26_), .c(x12), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n42_), .O(z2));
  inv1   g26(.a(x13), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n22_), .c(x05), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(x12), .O(new_n50_));
  nand4  g29(.a(new_n39_), .b(new_n43_), .c(new_n48_), .d(new_n32_), .O(new_n51_));
  nand2  g30(.a(new_n34_), .b(x13), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n51_), .c(x04), .O(new_n53_));
  aoi21  g32(.a(x07), .b(new_n22_), .c(new_n26_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n50_), .O(z3));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  nand2  g36(.a(x08), .b(x03), .O(new_n58_));
  nor3   g37(.a(new_n58_), .b(new_n57_), .c(new_n29_), .O(z4));
endmodule


