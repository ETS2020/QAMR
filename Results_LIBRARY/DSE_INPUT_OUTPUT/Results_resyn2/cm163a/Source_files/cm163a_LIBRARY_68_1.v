// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x11), .O(new_n22_));
  nor2   g01(.a(new_n22_), .b(x05), .O(new_n23_));
  inv1   g02(.a(x04), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x00), .O(new_n25_));
  inv1   g04(.a(x09), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  inv1   g06(.a(new_n27_), .O(new_n28_));
  nor2   g07(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  oai21  g08(.a(new_n27_), .b(x09), .c(x04), .O(new_n30_));
  oai21  g09(.a(new_n30_), .b(new_n29_), .c(new_n25_), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(x05), .c(new_n23_), .O(z0));
  nand3  g11(.a(new_n28_), .b(new_n26_), .c(x04), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(x05), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  oai22  g14(.a(new_n30_), .b(new_n22_), .c(x04), .d(x01), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x05), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n35_), .O(z1));
  oai21  g17(.a(new_n33_), .b(x12), .c(x05), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n22_), .O(new_n40_));
  inv1   g19(.a(x12), .O(new_n41_));
  nor2   g20(.a(x11), .b(x09), .O(new_n42_));
  aoi21  g21(.a(new_n42_), .b(new_n28_), .c(new_n41_), .O(new_n43_));
  inv1   g22(.a(x05), .O(new_n44_));
  aoi21  g23(.a(x06), .b(new_n24_), .c(new_n44_), .O(new_n45_));
  oai21  g24(.a(new_n43_), .b(new_n24_), .c(new_n45_), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n40_), .O(z2));
  inv1   g26(.a(x13), .O(new_n48_));
  nor2   g27(.a(x09), .b(new_n24_), .O(new_n49_));
  nand4  g28(.a(new_n49_), .b(new_n28_), .c(new_n48_), .d(new_n41_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n22_), .O(new_n52_));
  nand4  g31(.a(new_n42_), .b(new_n41_), .c(x03), .d(x02), .O(new_n53_));
  nor2   g32(.a(new_n48_), .b(new_n24_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g34(.a(x07), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(new_n24_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(x05), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n52_), .O(z3));
  nand2  g39(.a(x08), .b(x03), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n23_), .O(new_n62_));
  nand4  g41(.a(new_n62_), .b(x15), .c(x14), .d(x10), .O(new_n63_));
  inv1   g42(.a(new_n63_), .O(z4));
endmodule


