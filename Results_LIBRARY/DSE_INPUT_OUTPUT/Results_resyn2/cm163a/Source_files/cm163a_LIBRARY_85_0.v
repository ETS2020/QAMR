// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x00), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  nand2  g02(.a(x09), .b(x04), .O(new_n24_));
  oai22  g03(.a(new_n24_), .b(new_n23_), .c(x04), .d(new_n22_), .O(new_n25_));
  nand2  g04(.a(new_n23_), .b(x04), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(x05), .c(x09), .O(new_n27_));
  aoi21  g06(.a(new_n25_), .b(x05), .c(new_n27_), .O(z0));
  nand2  g07(.a(x11), .b(x04), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x09), .O(new_n31_));
  nand2  g10(.a(new_n23_), .b(x11), .O(new_n32_));
  and2   g11(.a(x03), .b(x02), .O(new_n33_));
  nor2   g12(.a(x11), .b(x09), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n32_), .c(x04), .O(new_n36_));
  inv1   g15(.a(x04), .O(new_n37_));
  inv1   g16(.a(x05), .O(new_n38_));
  aoi21  g17(.a(new_n37_), .b(x01), .c(new_n38_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n36_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n31_), .O(z1));
  xor2a  g20(.a(new_n35_), .b(x12), .O(new_n42_));
  inv1   g21(.a(x06), .O(new_n43_));
  aoi21  g22(.a(new_n43_), .b(new_n37_), .c(new_n38_), .O(new_n44_));
  oai21  g23(.a(new_n42_), .b(new_n37_), .c(new_n44_), .O(z2));
  inv1   g24(.a(x13), .O(new_n46_));
  oai21  g25(.a(new_n46_), .b(new_n37_), .c(x05), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x09), .O(new_n48_));
  nor2   g27(.a(x12), .b(x11), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n33_), .c(new_n46_), .O(new_n50_));
  nor2   g29(.a(x13), .b(x12), .O(new_n51_));
  nand4  g30(.a(new_n51_), .b(new_n34_), .c(x03), .d(x02), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x04), .O(new_n53_));
  aoi21  g32(.a(x07), .b(new_n37_), .c(new_n38_), .O(new_n54_));
  oai21  g33(.a(new_n53_), .b(new_n50_), .c(new_n54_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n48_), .O(z3));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  nor2   g36(.a(x09), .b(x05), .O(new_n58_));
  nand2  g37(.a(x08), .b(x03), .O(new_n59_));
  nor3   g38(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z4));
endmodule


