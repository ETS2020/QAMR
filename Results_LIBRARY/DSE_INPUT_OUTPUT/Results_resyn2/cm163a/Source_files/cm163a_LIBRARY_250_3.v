// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x13), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  and2   g04(.a(x03), .b(x02), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(new_n24_), .c(new_n22_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x04), .O(new_n29_));
  inv1   g08(.a(x00), .O(new_n30_));
  inv1   g09(.a(x04), .O(new_n31_));
  inv1   g10(.a(x05), .O(new_n32_));
  aoi21  g11(.a(new_n31_), .b(new_n30_), .c(new_n32_), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n29_), .O(z0));
  nand2  g13(.a(x13), .b(x04), .O(new_n35_));
  inv1   g14(.a(new_n35_), .O(new_n36_));
  nand2  g15(.a(new_n31_), .b(x01), .O(new_n37_));
  inv1   g16(.a(x11), .O(new_n38_));
  nand4  g17(.a(new_n38_), .b(new_n25_), .c(x03), .d(x02), .O(new_n39_));
  oai21  g18(.a(new_n23_), .b(x09), .c(x11), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n39_), .c(x04), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n37_), .O(new_n42_));
  aoi21  g21(.a(new_n42_), .b(x05), .c(new_n36_), .O(z1));
  nand2  g22(.a(new_n39_), .b(x12), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nand4  g24(.a(new_n26_), .b(new_n45_), .c(new_n38_), .d(new_n25_), .O(new_n46_));
  nand3  g25(.a(new_n46_), .b(new_n44_), .c(new_n22_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x04), .O(new_n48_));
  inv1   g27(.a(x06), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n31_), .c(new_n32_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n48_), .O(z2));
  inv1   g30(.a(new_n39_), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n45_), .c(x13), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n31_), .c(new_n32_), .O(new_n55_));
  oai21  g34(.a(new_n53_), .b(new_n31_), .c(new_n55_), .O(z3));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  nand3  g36(.a(new_n35_), .b(x08), .c(x03), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n57_), .O(z4));
endmodule


