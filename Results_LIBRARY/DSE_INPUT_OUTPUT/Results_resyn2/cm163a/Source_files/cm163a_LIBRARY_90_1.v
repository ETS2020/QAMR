// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x11), .O(new_n22_));
  nor2   g01(.a(new_n22_), .b(x09), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(x05), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x09), .O(new_n28_));
  inv1   g07(.a(x09), .O(new_n29_));
  nand4  g08(.a(new_n22_), .b(new_n29_), .c(x03), .d(x02), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x04), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n26_), .O(z0));
  oai21  g12(.a(new_n27_), .b(x09), .c(new_n22_), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x04), .O(new_n35_));
  inv1   g14(.a(x01), .O(new_n36_));
  inv1   g15(.a(x04), .O(new_n37_));
  inv1   g16(.a(x05), .O(new_n38_));
  aoi21  g17(.a(new_n37_), .b(new_n36_), .c(new_n38_), .O(new_n39_));
  aoi21  g18(.a(new_n39_), .b(new_n35_), .c(new_n23_), .O(z1));
  nand4  g19(.a(x12), .b(new_n29_), .c(x03), .d(x02), .O(new_n41_));
  inv1   g20(.a(x12), .O(new_n42_));
  oai21  g21(.a(new_n27_), .b(x09), .c(new_n42_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n41_), .c(x04), .O(new_n44_));
  inv1   g23(.a(x06), .O(new_n45_));
  aoi21  g24(.a(new_n45_), .b(new_n37_), .c(new_n38_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n23_), .O(z2));
  inv1   g26(.a(x13), .O(new_n48_));
  nor2   g27(.a(new_n48_), .b(new_n37_), .O(new_n49_));
  oai21  g28(.a(new_n30_), .b(x12), .c(new_n49_), .O(new_n50_));
  nand3  g29(.a(new_n48_), .b(new_n42_), .c(x04), .O(new_n51_));
  oai21  g30(.a(new_n51_), .b(new_n27_), .c(new_n22_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n29_), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n37_), .c(new_n38_), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(new_n53_), .c(new_n50_), .O(z3));
  nand2  g35(.a(x08), .b(x03), .O(new_n57_));
  nand3  g36(.a(x15), .b(x14), .c(x10), .O(new_n58_));
  oai21  g37(.a(new_n58_), .b(new_n57_), .c(new_n24_), .O(z4));
endmodule


