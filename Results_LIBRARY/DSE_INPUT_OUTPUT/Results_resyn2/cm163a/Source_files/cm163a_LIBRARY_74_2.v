// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g03(.a(new_n22_), .b(x03), .c(x02), .O(new_n25_));
  inv1   g04(.a(new_n25_), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n24_), .c(x04), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  inv1   g07(.a(x12), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x08), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x05), .O(new_n31_));
  nor2   g10(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n27_), .O(z0));
  inv1   g12(.a(x04), .O(new_n34_));
  xor2a  g13(.a(new_n25_), .b(x11), .O(new_n35_));
  nor2   g14(.a(x04), .b(x01), .O(new_n36_));
  nor2   g15(.a(new_n36_), .b(new_n31_), .O(new_n37_));
  oai21  g16(.a(new_n35_), .b(new_n34_), .c(new_n37_), .O(z1));
  oai21  g17(.a(x06), .b(x04), .c(x05), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n30_), .O(new_n40_));
  inv1   g19(.a(x11), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(new_n22_), .c(x03), .d(x02), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x12), .O(new_n43_));
  inv1   g22(.a(x08), .O(new_n44_));
  nor2   g23(.a(x12), .b(x11), .O(new_n45_));
  nand4  g24(.a(new_n45_), .b(new_n23_), .c(new_n22_), .d(new_n44_), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x04), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n40_), .O(z2));
  nand3  g28(.a(new_n45_), .b(new_n26_), .c(x13), .O(new_n50_));
  inv1   g29(.a(x13), .O(new_n51_));
  inv1   g30(.a(new_n45_), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n25_), .c(new_n51_), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n50_), .c(x04), .O(new_n54_));
  oai21  g33(.a(new_n51_), .b(new_n34_), .c(x12), .O(new_n55_));
  oai21  g34(.a(x07), .b(x04), .c(x05), .O(new_n56_));
  aoi21  g35(.a(new_n55_), .b(x08), .c(new_n56_), .O(new_n57_));
  nand2  g36(.a(new_n57_), .b(new_n54_), .O(z3));
  nand4  g37(.a(x15), .b(x14), .c(x10), .d(x03), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(x12), .c(new_n44_), .O(z4));
endmodule


