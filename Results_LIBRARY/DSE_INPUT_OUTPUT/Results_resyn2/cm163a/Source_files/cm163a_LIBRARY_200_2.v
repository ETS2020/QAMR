// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x11), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x07), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  xor2a  g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(new_n28_), .c(new_n24_), .O(z0));
  inv1   g12(.a(x07), .O(new_n34_));
  nand4  g13(.a(new_n25_), .b(x04), .c(x03), .d(x02), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  and2   g16(.a(x03), .b(x02), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n25_), .O(new_n39_));
  nor2   g18(.a(new_n22_), .b(new_n30_), .O(new_n40_));
  oai21  g19(.a(x04), .b(x01), .c(x05), .O(new_n41_));
  aoi21  g20(.a(new_n40_), .b(new_n39_), .c(new_n41_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n37_), .O(z1));
  oai21  g22(.a(x06), .b(x04), .c(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n23_), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n22_), .b(new_n25_), .c(x03), .d(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand4  g27(.a(new_n38_), .b(x12), .c(new_n22_), .d(new_n25_), .O(new_n49_));
  aoi21  g28(.a(new_n22_), .b(x07), .c(new_n30_), .O(new_n50_));
  nand3  g29(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n45_), .O(z2));
  nand2  g31(.a(x13), .b(x04), .O(new_n53_));
  inv1   g32(.a(new_n53_), .O(new_n54_));
  oai21  g33(.a(new_n47_), .b(x12), .c(new_n54_), .O(new_n55_));
  inv1   g34(.a(x13), .O(new_n56_));
  inv1   g35(.a(new_n47_), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n56_), .c(new_n46_), .O(new_n58_));
  nand2  g37(.a(new_n34_), .b(x04), .O(new_n59_));
  nand3  g38(.a(new_n53_), .b(x11), .c(x07), .O(new_n60_));
  aoi21  g39(.a(new_n60_), .b(new_n59_), .c(new_n31_), .O(new_n61_));
  nand3  g40(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z3));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  inv1   g42(.a(new_n63_), .O(new_n64_));
  nand4  g43(.a(new_n64_), .b(new_n23_), .c(x08), .d(x03), .O(new_n65_));
  inv1   g44(.a(new_n65_), .O(z4));
endmodule


