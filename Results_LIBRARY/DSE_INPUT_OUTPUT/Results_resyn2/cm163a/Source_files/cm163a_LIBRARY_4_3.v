// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_;
  nand2  g00(.a(x08), .b(x03), .O(new_n22_));
  inv1   g01(.a(new_n22_), .O(z4));
  and2   g02(.a(x03), .b(x02), .O(new_n24_));
  xor2a  g03(.a(new_n24_), .b(x09), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(x04), .O(new_n26_));
  inv1   g05(.a(x00), .O(new_n27_));
  inv1   g06(.a(x04), .O(new_n28_));
  inv1   g07(.a(x05), .O(new_n29_));
  aoi21  g08(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  aoi21  g09(.a(new_n30_), .b(new_n26_), .c(z4), .O(z0));
  oai21  g10(.a(x04), .b(x01), .c(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  inv1   g12(.a(x02), .O(new_n34_));
  oai21  g13(.a(x09), .b(new_n34_), .c(x11), .O(new_n35_));
  inv1   g14(.a(x09), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand3  g16(.a(new_n24_), .b(new_n37_), .c(new_n36_), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n35_), .c(x08), .O(new_n39_));
  nor2   g18(.a(new_n37_), .b(x03), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n39_), .c(x04), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n33_), .O(z1));
  oai21  g21(.a(x06), .b(x04), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n22_), .O(new_n44_));
  inv1   g23(.a(x12), .O(new_n45_));
  nor2   g24(.a(new_n45_), .b(x03), .O(new_n46_));
  nand2  g25(.a(new_n38_), .b(new_n45_), .O(new_n47_));
  nor2   g26(.a(x09), .b(new_n34_), .O(new_n48_));
  nor2   g27(.a(new_n45_), .b(x11), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n48_), .c(x08), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n47_), .c(new_n46_), .O(new_n51_));
  oai21  g30(.a(new_n51_), .b(new_n28_), .c(new_n44_), .O(z2));
  oai21  g31(.a(x07), .b(x04), .c(x05), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n22_), .O(new_n54_));
  nand4  g33(.a(new_n45_), .b(new_n37_), .c(new_n36_), .d(x02), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(x13), .O(new_n56_));
  inv1   g35(.a(x13), .O(new_n57_));
  nor2   g36(.a(x12), .b(x11), .O(new_n58_));
  nand4  g37(.a(new_n58_), .b(new_n24_), .c(new_n57_), .d(new_n36_), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(new_n56_), .c(x08), .O(new_n60_));
  nor2   g39(.a(new_n57_), .b(x03), .O(new_n61_));
  oai21  g40(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand2  g41(.a(new_n62_), .b(new_n54_), .O(z3));
endmodule


