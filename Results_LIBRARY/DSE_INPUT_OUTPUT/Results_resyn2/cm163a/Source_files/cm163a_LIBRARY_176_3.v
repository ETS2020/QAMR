// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x04), .O(new_n22_));
  aoi21  g01(.a(x03), .b(x02), .c(new_n22_), .O(new_n23_));
  inv1   g02(.a(x00), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  oai21  g04(.a(x04), .b(new_n24_), .c(new_n25_), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n23_), .c(x05), .O(z0));
  nand2  g06(.a(x03), .b(x02), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x11), .O(new_n29_));
  nor2   g08(.a(new_n28_), .b(x11), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n29_), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(x04), .O(new_n33_));
  nor2   g12(.a(x04), .b(x01), .O(new_n34_));
  nand2  g13(.a(new_n25_), .b(x05), .O(new_n35_));
  nor2   g14(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n33_), .O(z1));
  inv1   g16(.a(x12), .O(new_n38_));
  inv1   g17(.a(x11), .O(new_n39_));
  nand4  g18(.a(new_n38_), .b(new_n39_), .c(x03), .d(x02), .O(new_n40_));
  oai21  g19(.a(new_n30_), .b(new_n38_), .c(new_n40_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x04), .O(new_n42_));
  nor2   g21(.a(x06), .b(x04), .O(new_n43_));
  nor2   g22(.a(new_n43_), .b(new_n35_), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n42_), .O(z2));
  inv1   g24(.a(x13), .O(new_n46_));
  nor2   g25(.a(x12), .b(x11), .O(new_n47_));
  nand4  g26(.a(new_n47_), .b(new_n46_), .c(x03), .d(x02), .O(new_n48_));
  nand2  g27(.a(new_n40_), .b(x13), .O(new_n49_));
  nand3  g28(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  aoi21  g29(.a(x07), .b(new_n22_), .c(x09), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x05), .O(z3));
  nand3  g32(.a(x15), .b(x14), .c(x10), .O(new_n54_));
  nand2  g33(.a(x09), .b(x05), .O(new_n55_));
  nand3  g34(.a(new_n55_), .b(x08), .c(x03), .O(new_n56_));
  nor2   g35(.a(new_n56_), .b(new_n54_), .O(z4));
endmodule


