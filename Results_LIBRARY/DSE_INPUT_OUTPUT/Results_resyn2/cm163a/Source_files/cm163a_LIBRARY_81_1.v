// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nor2   g04(.a(new_n23_), .b(x09), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n25_), .c(x04), .O(new_n27_));
  nor2   g06(.a(x04), .b(x00), .O(new_n28_));
  nand2  g07(.a(x12), .b(new_n22_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(new_n30_));
  nor2   g09(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n27_), .O(z0));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n22_), .c(x03), .d(x02), .O(new_n34_));
  oai21  g13(.a(new_n26_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  inv1   g16(.a(x04), .O(new_n38_));
  inv1   g17(.a(x05), .O(new_n39_));
  aoi21  g18(.a(new_n38_), .b(new_n37_), .c(new_n39_), .O(new_n40_));
  aoi22  g19(.a(new_n40_), .b(new_n36_), .c(x12), .d(new_n22_), .O(z1));
  inv1   g20(.a(new_n34_), .O(new_n42_));
  oai21  g21(.a(new_n42_), .b(x12), .c(x04), .O(new_n43_));
  nor2   g22(.a(x06), .b(x04), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(new_n30_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n43_), .O(z2));
  oai21  g25(.a(x07), .b(x04), .c(x05), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n22_), .O(new_n50_));
  nand2  g29(.a(new_n34_), .b(new_n49_), .O(new_n51_));
  nor2   g30(.a(new_n49_), .b(x11), .O(new_n52_));
  aoi21  g31(.a(new_n52_), .b(new_n24_), .c(x12), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  oai21  g33(.a(new_n54_), .b(new_n38_), .c(new_n48_), .O(z3));
  nand3  g34(.a(x15), .b(x14), .c(x10), .O(new_n56_));
  inv1   g35(.a(new_n56_), .O(new_n57_));
  nand4  g36(.a(new_n57_), .b(new_n29_), .c(x08), .d(x03), .O(new_n58_));
  inv1   g37(.a(new_n58_), .O(z4));
endmodule


