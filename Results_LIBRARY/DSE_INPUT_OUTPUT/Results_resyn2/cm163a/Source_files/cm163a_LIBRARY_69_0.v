// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x08), .O(new_n22_));
  inv1   g01(.a(x11), .O(new_n23_));
  inv1   g02(.a(x09), .O(new_n24_));
  nand4  g03(.a(new_n24_), .b(x04), .c(x03), .d(x02), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nor2   g06(.a(new_n24_), .b(new_n27_), .O(new_n28_));
  oai21  g07(.a(x04), .b(x00), .c(x05), .O(new_n29_));
  aoi21  g08(.a(new_n28_), .b(new_n26_), .c(new_n29_), .O(new_n30_));
  aoi22  g09(.a(new_n30_), .b(new_n25_), .c(new_n23_), .d(new_n22_), .O(z0));
  oai21  g10(.a(x04), .b(x01), .c(x05), .O(new_n32_));
  oai21  g11(.a(x11), .b(x08), .c(new_n32_), .O(new_n33_));
  nand3  g12(.a(new_n24_), .b(x03), .c(x02), .O(new_n34_));
  and2   g13(.a(new_n34_), .b(x11), .O(new_n35_));
  nand4  g14(.a(new_n23_), .b(new_n24_), .c(x03), .d(x02), .O(new_n36_));
  nor2   g15(.a(new_n36_), .b(new_n22_), .O(new_n37_));
  oai21  g16(.a(new_n37_), .b(new_n35_), .c(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n33_), .O(z1));
  inv1   g18(.a(x12), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  nor2   g20(.a(new_n41_), .b(new_n34_), .O(new_n42_));
  aoi21  g21(.a(new_n36_), .b(x12), .c(new_n42_), .O(new_n43_));
  oai21  g22(.a(new_n40_), .b(new_n27_), .c(x11), .O(new_n44_));
  oai21  g23(.a(x06), .b(x04), .c(x05), .O(new_n45_));
  aoi21  g24(.a(new_n44_), .b(new_n22_), .c(new_n45_), .O(new_n46_));
  oai21  g25(.a(new_n43_), .b(new_n27_), .c(new_n46_), .O(z2));
  nand3  g26(.a(x13), .b(new_n40_), .c(x05), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n25_), .c(x08), .O(new_n49_));
  inv1   g28(.a(x13), .O(new_n50_));
  oai21  g29(.a(new_n41_), .b(new_n34_), .c(new_n50_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x04), .O(new_n52_));
  inv1   g31(.a(x05), .O(new_n53_));
  inv1   g32(.a(x07), .O(new_n54_));
  aoi21  g33(.a(new_n54_), .b(new_n27_), .c(new_n53_), .O(new_n55_));
  aoi22  g34(.a(new_n55_), .b(new_n52_), .c(new_n49_), .d(new_n23_), .O(z3));
  nand2  g35(.a(x08), .b(x03), .O(new_n57_));
  nand3  g36(.a(x15), .b(x14), .c(x10), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n57_), .O(z4));
endmodule


