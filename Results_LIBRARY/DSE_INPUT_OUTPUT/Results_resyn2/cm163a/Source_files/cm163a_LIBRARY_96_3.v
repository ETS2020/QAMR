// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  oai21  g03(.a(x04), .b(x00), .c(x05), .O(new_n25_));
  aoi21  g04(.a(x09), .b(x06), .c(new_n25_), .O(new_n26_));
  oai21  g05(.a(new_n24_), .b(new_n22_), .c(new_n26_), .O(z0));
  inv1   g06(.a(x11), .O(new_n28_));
  oai21  g07(.a(new_n23_), .b(x09), .c(new_n28_), .O(new_n29_));
  inv1   g08(.a(new_n23_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x11), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(new_n29_), .c(x04), .O(new_n32_));
  inv1   g11(.a(x06), .O(new_n33_));
  oai21  g12(.a(new_n28_), .b(new_n22_), .c(new_n33_), .O(new_n34_));
  oai21  g13(.a(x04), .b(x01), .c(x05), .O(new_n35_));
  aoi21  g14(.a(new_n34_), .b(x09), .c(new_n35_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n32_), .O(z1));
  oai21  g16(.a(new_n23_), .b(x11), .c(x12), .O(new_n38_));
  nor2   g17(.a(x12), .b(x11), .O(new_n39_));
  nand3  g18(.a(new_n39_), .b(x03), .c(x02), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nor2   g20(.a(x09), .b(new_n22_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  inv1   g22(.a(x05), .O(new_n44_));
  nand2  g23(.a(x09), .b(x06), .O(new_n45_));
  nand2  g24(.a(x12), .b(x09), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(x04), .c(x06), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(new_n44_), .c(new_n45_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n43_), .O(z2));
  inv1   g28(.a(x09), .O(new_n50_));
  inv1   g29(.a(x13), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g31(.a(new_n52_), .b(new_n39_), .c(new_n30_), .O(new_n53_));
  nand2  g32(.a(new_n40_), .b(new_n51_), .O(new_n54_));
  nand3  g33(.a(new_n54_), .b(new_n53_), .c(x04), .O(new_n55_));
  oai21  g34(.a(new_n51_), .b(new_n22_), .c(new_n33_), .O(new_n56_));
  oai21  g35(.a(x07), .b(x04), .c(x05), .O(new_n57_));
  aoi21  g36(.a(new_n56_), .b(x09), .c(new_n57_), .O(new_n58_));
  nand2  g37(.a(new_n58_), .b(new_n55_), .O(z3));
  nand3  g38(.a(x15), .b(x14), .c(x10), .O(new_n60_));
  nand3  g39(.a(new_n45_), .b(x08), .c(x03), .O(new_n61_));
  nor2   g40(.a(new_n61_), .b(new_n60_), .O(z4));
endmodule


