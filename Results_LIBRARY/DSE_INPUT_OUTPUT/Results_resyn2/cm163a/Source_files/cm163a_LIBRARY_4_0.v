// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_;
  nor2   g00(.a(x08), .b(x03), .O(new_n22_));
  inv1   g01(.a(x02), .O(new_n23_));
  inv1   g02(.a(x03), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  oai21  g04(.a(new_n24_), .b(new_n23_), .c(new_n25_), .O(new_n26_));
  nand3  g05(.a(x09), .b(x03), .c(x02), .O(new_n27_));
  nand3  g06(.a(new_n27_), .b(new_n26_), .c(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x04), .O(new_n30_));
  inv1   g09(.a(x05), .O(new_n31_));
  aoi21  g10(.a(new_n30_), .b(new_n29_), .c(new_n31_), .O(new_n32_));
  aoi21  g11(.a(new_n32_), .b(new_n28_), .c(new_n22_), .O(z0));
  inv1   g12(.a(x11), .O(new_n34_));
  nand3  g13(.a(new_n25_), .b(x03), .c(x02), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g15(.a(x11), .b(new_n25_), .c(x03), .d(x02), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n36_), .c(x04), .O(new_n38_));
  inv1   g17(.a(x01), .O(new_n39_));
  aoi21  g18(.a(new_n30_), .b(new_n39_), .c(new_n31_), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n38_), .c(new_n22_), .O(z1));
  nor2   g20(.a(x09), .b(new_n23_), .O(new_n42_));
  nor2   g21(.a(x11), .b(new_n24_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n42_), .c(x12), .O(new_n44_));
  nor2   g23(.a(new_n22_), .b(new_n30_), .O(new_n45_));
  inv1   g24(.a(x12), .O(new_n46_));
  nand4  g25(.a(new_n34_), .b(new_n25_), .c(x03), .d(x02), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n45_), .c(new_n44_), .O(new_n49_));
  aoi21  g28(.a(x12), .b(new_n24_), .c(x05), .O(new_n50_));
  aoi21  g29(.a(x06), .b(x05), .c(new_n22_), .O(new_n51_));
  oai21  g30(.a(new_n50_), .b(new_n30_), .c(new_n51_), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(new_n49_), .O(z2));
  inv1   g32(.a(x13), .O(new_n54_));
  oai21  g33(.a(new_n47_), .b(x12), .c(new_n54_), .O(new_n55_));
  nand4  g34(.a(new_n43_), .b(new_n42_), .c(x13), .d(new_n46_), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n55_), .c(new_n45_), .O(new_n57_));
  aoi21  g36(.a(x13), .b(new_n24_), .c(x05), .O(new_n58_));
  aoi21  g37(.a(x07), .b(x05), .c(new_n22_), .O(new_n59_));
  oai21  g38(.a(new_n58_), .b(new_n30_), .c(new_n59_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n57_), .O(z3));
  nand2  g40(.a(x08), .b(x03), .O(new_n62_));
  nand3  g41(.a(x15), .b(x14), .c(x10), .O(new_n63_));
  nor2   g42(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  or2    g43(.a(new_n64_), .b(new_n22_), .O(z4));
endmodule


