// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x07), .b(new_n22_), .O(new_n23_));
  inv1   g02(.a(new_n23_), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  nand2  g04(.a(x03), .b(x02), .O(new_n26_));
  xor2a  g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  inv1   g07(.a(x00), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n22_), .b(new_n29_), .c(new_n30_), .O(new_n31_));
  aoi21  g10(.a(new_n31_), .b(new_n28_), .c(new_n24_), .O(z0));
  oai21  g11(.a(new_n26_), .b(x09), .c(x11), .O(new_n33_));
  inv1   g12(.a(x11), .O(new_n34_));
  nand4  g13(.a(new_n34_), .b(new_n25_), .c(x03), .d(x02), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(x04), .O(new_n37_));
  inv1   g16(.a(x07), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x01), .O(new_n39_));
  aoi21  g18(.a(new_n39_), .b(new_n22_), .c(new_n30_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(new_n37_), .O(z1));
  xor2a  g20(.a(new_n35_), .b(x12), .O(new_n42_));
  oai21  g21(.a(x06), .b(x04), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  oai21  g23(.a(new_n42_), .b(new_n22_), .c(new_n44_), .O(z2));
  inv1   g24(.a(x12), .O(new_n46_));
  inv1   g25(.a(x13), .O(new_n47_));
  inv1   g26(.a(new_n35_), .O(new_n48_));
  nand3  g27(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  oai21  g28(.a(new_n35_), .b(x12), .c(x13), .O(new_n50_));
  nand4  g29(.a(new_n50_), .b(new_n49_), .c(x05), .d(x04), .O(z3));
  nand2  g30(.a(x08), .b(x03), .O(new_n52_));
  nand3  g31(.a(x15), .b(x14), .c(x10), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n52_), .c(new_n23_), .O(z4));
endmodule


