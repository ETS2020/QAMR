// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x03), .c(x02), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  aoi21  g06(.a(new_n27_), .b(x00), .c(x01), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(new_n26_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(z0));
  nand2  g09(.a(new_n23_), .b(x11), .O(new_n31_));
  inv1   g10(.a(x11), .O(new_n32_));
  nand4  g11(.a(new_n32_), .b(new_n22_), .c(x03), .d(x02), .O(new_n33_));
  inv1   g12(.a(x05), .O(new_n34_));
  nor2   g13(.a(new_n34_), .b(x01), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(new_n33_), .c(new_n31_), .d(x04), .O(z1));
  nand2  g15(.a(new_n33_), .b(x12), .O(new_n37_));
  inv1   g16(.a(x12), .O(new_n38_));
  nor2   g17(.a(x11), .b(x09), .O(new_n39_));
  nand4  g18(.a(new_n39_), .b(new_n38_), .c(x03), .d(x02), .O(new_n40_));
  nand3  g19(.a(new_n40_), .b(new_n37_), .c(x04), .O(new_n41_));
  aoi21  g20(.a(x06), .b(new_n27_), .c(x01), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x05), .O(z2));
  inv1   g23(.a(x13), .O(new_n45_));
  nor2   g24(.a(new_n40_), .b(new_n45_), .O(new_n46_));
  nand2  g25(.a(new_n40_), .b(new_n45_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(x04), .O(new_n48_));
  nor2   g27(.a(x07), .b(x04), .O(new_n49_));
  nor3   g28(.a(new_n49_), .b(new_n34_), .c(x01), .O(new_n50_));
  oai21  g29(.a(new_n48_), .b(new_n46_), .c(new_n50_), .O(z3));
  nand3  g30(.a(x15), .b(x14), .c(x10), .O(new_n52_));
  nand2  g31(.a(x05), .b(x01), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(x08), .c(x03), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(new_n52_), .O(z4));
endmodule


