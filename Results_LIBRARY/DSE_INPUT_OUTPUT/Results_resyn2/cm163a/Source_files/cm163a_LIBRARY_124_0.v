// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x03), .c(x02), .O(new_n23_));
  oai21  g02(.a(new_n22_), .b(x03), .c(new_n23_), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x04), .O(new_n25_));
  inv1   g04(.a(x05), .O(new_n26_));
  oai21  g05(.a(new_n22_), .b(x02), .c(x04), .O(new_n27_));
  nand2  g06(.a(x02), .b(x00), .O(new_n28_));
  aoi21  g07(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n25_), .O(z0));
  inv1   g09(.a(x11), .O(new_n31_));
  nand2  g10(.a(new_n23_), .b(new_n31_), .O(new_n32_));
  inv1   g11(.a(x03), .O(new_n33_));
  nor2   g12(.a(x09), .b(new_n33_), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(x11), .O(new_n35_));
  nand3  g14(.a(new_n35_), .b(new_n32_), .c(x04), .O(new_n36_));
  oai21  g15(.a(new_n31_), .b(x02), .c(x04), .O(new_n37_));
  nand2  g16(.a(x02), .b(x01), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n37_), .c(new_n26_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n36_), .O(z1));
  nand3  g19(.a(new_n31_), .b(new_n22_), .c(x03), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(x12), .O(new_n42_));
  nor2   g21(.a(x12), .b(x11), .O(new_n43_));
  nand4  g22(.a(new_n43_), .b(new_n22_), .c(x03), .d(x02), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(x04), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  oai21  g26(.a(new_n47_), .b(x02), .c(x04), .O(new_n48_));
  nand2  g27(.a(x06), .b(x02), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n48_), .c(new_n26_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n46_), .O(z2));
  inv1   g30(.a(x13), .O(new_n52_));
  nand2  g31(.a(new_n52_), .b(x02), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(new_n43_), .c(new_n34_), .O(new_n54_));
  nand3  g33(.a(new_n43_), .b(new_n22_), .c(x03), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand3  g35(.a(new_n56_), .b(new_n54_), .c(x04), .O(new_n57_));
  oai21  g36(.a(new_n52_), .b(x02), .c(x04), .O(new_n58_));
  nand2  g37(.a(x07), .b(x02), .O(new_n59_));
  aoi21  g38(.a(new_n59_), .b(new_n58_), .c(new_n26_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n57_), .O(z3));
  nand3  g40(.a(x15), .b(x14), .c(x10), .O(new_n62_));
  nor2   g41(.a(x04), .b(x02), .O(new_n63_));
  nand2  g42(.a(x08), .b(x03), .O(new_n64_));
  nor3   g43(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z4));
endmodule


