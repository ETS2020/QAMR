// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  nor2   g03(.a(x04), .b(x00), .O(new_n25_));
  nand2  g04(.a(x13), .b(x09), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x05), .O(new_n27_));
  nor2   g06(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n28_), .O(z0));
  inv1   g08(.a(x09), .O(new_n30_));
  nand4  g09(.a(x11), .b(new_n30_), .c(x03), .d(x02), .O(new_n31_));
  inv1   g10(.a(x11), .O(new_n32_));
  oai21  g11(.a(new_n23_), .b(x09), .c(new_n32_), .O(new_n33_));
  nand3  g12(.a(new_n33_), .b(new_n31_), .c(x04), .O(new_n34_));
  inv1   g13(.a(x01), .O(new_n35_));
  inv1   g14(.a(x05), .O(new_n36_));
  aoi21  g15(.a(new_n22_), .b(new_n35_), .c(new_n36_), .O(new_n37_));
  aoi22  g16(.a(new_n37_), .b(new_n34_), .c(x13), .d(x09), .O(z1));
  oai21  g17(.a(new_n23_), .b(x11), .c(x12), .O(new_n39_));
  nor2   g18(.a(x12), .b(x11), .O(new_n40_));
  nand4  g19(.a(new_n40_), .b(new_n30_), .c(x03), .d(x02), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g21(.a(new_n42_), .b(x04), .O(new_n43_));
  inv1   g22(.a(x13), .O(new_n44_));
  nand2  g23(.a(x12), .b(x04), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  oai21  g25(.a(x06), .b(x04), .c(x05), .O(new_n47_));
  aoi21  g26(.a(new_n46_), .b(x09), .c(new_n47_), .O(new_n48_));
  nand2  g27(.a(new_n48_), .b(new_n43_), .O(z2));
  nor2   g28(.a(new_n23_), .b(x09), .O(new_n50_));
  aoi21  g29(.a(new_n40_), .b(new_n50_), .c(x13), .O(new_n51_));
  inv1   g30(.a(new_n40_), .O(new_n52_));
  nand3  g31(.a(x13), .b(x03), .c(x02), .O(new_n53_));
  oai21  g32(.a(new_n53_), .b(new_n52_), .c(x04), .O(new_n54_));
  nor2   g33(.a(x07), .b(x04), .O(new_n55_));
  nor2   g34(.a(new_n55_), .b(new_n27_), .O(new_n56_));
  oai21  g35(.a(new_n54_), .b(new_n51_), .c(new_n56_), .O(z3));
  nand2  g36(.a(x08), .b(x03), .O(new_n58_));
  nand3  g37(.a(x15), .b(x14), .c(x10), .O(new_n59_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(new_n26_), .O(z4));
endmodule


