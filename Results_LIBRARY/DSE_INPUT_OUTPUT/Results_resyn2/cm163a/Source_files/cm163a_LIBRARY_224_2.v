// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand3  g01(.a(new_n22_), .b(x03), .c(x02), .O(new_n23_));
  nand2  g02(.a(x03), .b(x02), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(x09), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  inv1   g06(.a(x13), .O(new_n28_));
  aoi21  g07(.a(new_n27_), .b(x00), .c(new_n28_), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(x05), .O(z0));
  xor2a  g10(.a(new_n23_), .b(x11), .O(new_n32_));
  inv1   g11(.a(x01), .O(new_n33_));
  nand2  g12(.a(x13), .b(x05), .O(new_n34_));
  aoi21  g13(.a(new_n27_), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  oai21  g14(.a(new_n32_), .b(new_n27_), .c(new_n35_), .O(z1));
  inv1   g15(.a(x11), .O(new_n37_));
  nand4  g16(.a(new_n37_), .b(new_n22_), .c(x03), .d(x02), .O(new_n38_));
  xor2a  g17(.a(new_n38_), .b(x12), .O(new_n39_));
  inv1   g18(.a(x06), .O(new_n40_));
  aoi21  g19(.a(new_n40_), .b(new_n27_), .c(new_n34_), .O(new_n41_));
  oai21  g20(.a(new_n39_), .b(new_n27_), .c(new_n41_), .O(z2));
  oai21  g21(.a(new_n38_), .b(x12), .c(x04), .O(new_n43_));
  inv1   g22(.a(x07), .O(new_n44_));
  aoi21  g23(.a(new_n44_), .b(new_n27_), .c(new_n34_), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n43_), .O(z3));
  nand2  g25(.a(x08), .b(x03), .O(new_n47_));
  aoi21  g26(.a(new_n28_), .b(x05), .c(new_n47_), .O(new_n48_));
  nand4  g27(.a(new_n48_), .b(x15), .c(x14), .d(x10), .O(new_n49_));
  inv1   g28(.a(new_n49_), .O(z4));
endmodule


