// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_;
  inv1   g00(.a(x09), .O(new_n22_));
  nand2  g01(.a(new_n22_), .b(x03), .O(new_n23_));
  inv1   g02(.a(x03), .O(new_n24_));
  nand2  g03(.a(x09), .b(new_n24_), .O(new_n25_));
  nand3  g04(.a(new_n25_), .b(new_n23_), .c(x04), .O(new_n26_));
  inv1   g05(.a(x04), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x00), .O(new_n28_));
  nand3  g07(.a(new_n28_), .b(new_n26_), .c(x02), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(x05), .O(z0));
  xor2a  g09(.a(new_n23_), .b(x11), .O(new_n31_));
  inv1   g10(.a(x01), .O(new_n32_));
  nand2  g11(.a(x05), .b(x02), .O(new_n33_));
  aoi21  g12(.a(new_n27_), .b(new_n32_), .c(new_n33_), .O(new_n34_));
  oai21  g13(.a(new_n31_), .b(new_n27_), .c(new_n34_), .O(z1));
  nor2   g14(.a(x09), .b(new_n24_), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand3  g16(.a(new_n37_), .b(new_n22_), .c(x03), .O(new_n38_));
  nor2   g17(.a(x12), .b(x11), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n36_), .c(new_n38_), .d(x12), .O(new_n40_));
  inv1   g19(.a(x06), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n27_), .c(new_n33_), .O(new_n42_));
  oai21  g21(.a(new_n40_), .b(new_n27_), .c(new_n42_), .O(z2));
  nand3  g22(.a(new_n39_), .b(new_n36_), .c(x13), .O(new_n44_));
  inv1   g23(.a(x13), .O(new_n45_));
  nand3  g24(.a(new_n39_), .b(new_n22_), .c(x03), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n44_), .c(x04), .O(new_n48_));
  inv1   g27(.a(x07), .O(new_n49_));
  aoi21  g28(.a(new_n49_), .b(new_n27_), .c(new_n33_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n48_), .O(z3));
  inv1   g30(.a(x05), .O(new_n52_));
  nand2  g31(.a(x08), .b(x03), .O(new_n53_));
  nand3  g32(.a(x15), .b(x14), .c(x10), .O(new_n54_));
  oai22  g33(.a(new_n54_), .b(new_n53_), .c(new_n52_), .d(x02), .O(z4));
endmodule


