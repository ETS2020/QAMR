// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  nor2   g03(.a(x04), .b(x00), .O(new_n25_));
  inv1   g04(.a(x12), .O(new_n26_));
  inv1   g05(.a(x14), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g07(.a(new_n28_), .b(x05), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  oai21  g09(.a(new_n24_), .b(new_n22_), .c(new_n30_), .O(z0));
  inv1   g10(.a(x09), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(x03), .c(x02), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(x11), .O(new_n34_));
  inv1   g13(.a(new_n23_), .O(new_n35_));
  nor2   g14(.a(x11), .b(x09), .O(new_n36_));
  nand2  g15(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x04), .O(new_n39_));
  nor2   g18(.a(x04), .b(x01), .O(new_n40_));
  nor2   g19(.a(new_n40_), .b(new_n29_), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n39_), .O(z1));
  oai21  g21(.a(x06), .b(x04), .c(x05), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n28_), .O(new_n44_));
  aoi21  g23(.a(new_n36_), .b(new_n35_), .c(new_n26_), .O(new_n45_));
  inv1   g24(.a(x11), .O(new_n46_));
  nand3  g25(.a(x14), .b(new_n26_), .c(new_n46_), .O(new_n47_));
  nor2   g26(.a(new_n47_), .b(new_n33_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n45_), .c(x04), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n44_), .O(z2));
  oai21  g29(.a(x07), .b(x04), .c(x05), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n28_), .O(new_n52_));
  nand2  g31(.a(x13), .b(new_n26_), .O(new_n53_));
  aoi21  g32(.a(new_n37_), .b(x14), .c(new_n53_), .O(new_n54_));
  inv1   g33(.a(x13), .O(new_n55_));
  oai21  g34(.a(new_n47_), .b(new_n33_), .c(new_n55_), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(x04), .O(new_n57_));
  oai21  g36(.a(new_n57_), .b(new_n54_), .c(new_n52_), .O(z3));
  nand2  g37(.a(x08), .b(x03), .O(new_n59_));
  nand3  g38(.a(x15), .b(x14), .c(x10), .O(new_n60_));
  oai21  g39(.a(new_n60_), .b(new_n59_), .c(new_n28_), .O(z4));
endmodule


