// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  oai21  g03(.a(new_n24_), .b(new_n22_), .c(x05), .O(z0));
  inv1   g04(.a(x09), .O(new_n26_));
  nand3  g05(.a(new_n26_), .b(x03), .c(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x11), .O(new_n28_));
  and2   g07(.a(x05), .b(x04), .O(new_n29_));
  inv1   g08(.a(x11), .O(new_n30_));
  nor2   g09(.a(new_n23_), .b(x09), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  nand3  g11(.a(new_n32_), .b(new_n29_), .c(new_n28_), .O(z1));
  nand2  g12(.a(new_n32_), .b(x12), .O(new_n34_));
  nor2   g13(.a(x12), .b(x11), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n31_), .O(new_n36_));
  nand3  g15(.a(new_n36_), .b(new_n34_), .c(new_n29_), .O(z2));
  inv1   g16(.a(x13), .O(new_n38_));
  inv1   g17(.a(x12), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n30_), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n27_), .c(new_n38_), .O(new_n41_));
  nand3  g20(.a(new_n35_), .b(new_n31_), .c(x13), .O(new_n42_));
  nand3  g21(.a(new_n42_), .b(new_n41_), .c(x04), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x05), .O(z3));
  nand2  g23(.a(x08), .b(x03), .O(new_n45_));
  aoi21  g24(.a(x05), .b(new_n22_), .c(new_n45_), .O(new_n46_));
  nand4  g25(.a(new_n46_), .b(x15), .c(x14), .d(x10), .O(new_n47_));
  inv1   g26(.a(new_n47_), .O(z4));
endmodule


