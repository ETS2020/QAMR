// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_;
  nand2  g00(.a(x03), .b(x02), .O(new_n22_));
  and2   g01(.a(new_n22_), .b(x04), .O(new_n23_));
  inv1   g02(.a(x00), .O(new_n24_));
  inv1   g03(.a(x09), .O(new_n25_));
  oai21  g04(.a(x04), .b(new_n24_), .c(new_n25_), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n23_), .c(x05), .O(z0));
  nand2  g06(.a(new_n22_), .b(x11), .O(new_n28_));
  inv1   g07(.a(x11), .O(new_n29_));
  nand4  g08(.a(new_n29_), .b(new_n25_), .c(x03), .d(x02), .O(new_n30_));
  nand2  g09(.a(new_n30_), .b(new_n28_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(x04), .O(new_n32_));
  nor2   g11(.a(x04), .b(x01), .O(new_n33_));
  nand2  g12(.a(new_n25_), .b(x05), .O(new_n34_));
  nor2   g13(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(new_n32_), .O(z1));
  oai21  g15(.a(new_n22_), .b(x11), .c(x12), .O(new_n37_));
  nor2   g16(.a(x12), .b(x11), .O(new_n38_));
  nand3  g17(.a(new_n38_), .b(x03), .c(x02), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g19(.a(new_n40_), .b(x04), .O(new_n41_));
  nor2   g20(.a(x06), .b(x04), .O(new_n42_));
  nor2   g21(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(new_n41_), .O(z2));
  nand4  g23(.a(new_n38_), .b(x13), .c(x03), .d(x02), .O(new_n45_));
  inv1   g24(.a(x13), .O(new_n46_));
  nand2  g25(.a(new_n39_), .b(new_n46_), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n45_), .c(x04), .O(new_n48_));
  nor2   g27(.a(x07), .b(x04), .O(new_n49_));
  nor2   g28(.a(new_n49_), .b(new_n34_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n48_), .O(z3));
  nand3  g30(.a(x15), .b(x14), .c(x10), .O(new_n52_));
  nand2  g31(.a(x09), .b(x05), .O(new_n53_));
  nand3  g32(.a(new_n53_), .b(x08), .c(x03), .O(new_n54_));
  nor2   g33(.a(new_n54_), .b(new_n52_), .O(z4));
endmodule


