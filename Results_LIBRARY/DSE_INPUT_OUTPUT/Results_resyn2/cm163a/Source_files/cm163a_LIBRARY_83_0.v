// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_;
  inv1   g00(.a(x05), .O(new_n22_));
  inv1   g01(.a(x04), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(x00), .O(new_n24_));
  nand4  g03(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n25_));
  aoi21  g04(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  nand2  g06(.a(new_n27_), .b(x04), .O(new_n28_));
  aoi21  g07(.a(new_n28_), .b(x05), .c(x09), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n26_), .O(z0));
  inv1   g09(.a(x11), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n23_), .c(x05), .O(new_n32_));
  nand2  g11(.a(new_n32_), .b(x09), .O(new_n33_));
  nand2  g12(.a(new_n27_), .b(x11), .O(new_n34_));
  inv1   g13(.a(x09), .O(new_n35_));
  nand4  g14(.a(new_n31_), .b(new_n35_), .c(x03), .d(x02), .O(new_n36_));
  nand3  g15(.a(new_n36_), .b(new_n34_), .c(x04), .O(new_n37_));
  aoi21  g16(.a(new_n23_), .b(x01), .c(new_n22_), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n33_), .O(z1));
  xor2a  g19(.a(new_n36_), .b(x12), .O(new_n41_));
  inv1   g20(.a(x06), .O(new_n42_));
  aoi21  g21(.a(new_n42_), .b(new_n23_), .c(new_n22_), .O(new_n43_));
  oai21  g22(.a(new_n41_), .b(new_n23_), .c(new_n43_), .O(z2));
  inv1   g23(.a(x12), .O(new_n45_));
  inv1   g24(.a(new_n27_), .O(new_n46_));
  nor2   g25(.a(x11), .b(x09), .O(new_n47_));
  nand4  g26(.a(new_n47_), .b(new_n46_), .c(x13), .d(new_n45_), .O(new_n48_));
  inv1   g27(.a(x13), .O(new_n49_));
  nand4  g28(.a(new_n47_), .b(new_n45_), .c(x03), .d(x02), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n48_), .c(x04), .O(new_n52_));
  inv1   g31(.a(x07), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n23_), .c(new_n22_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(new_n52_), .O(z3));
  nand2  g34(.a(x08), .b(x03), .O(new_n56_));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  oai22  g36(.a(new_n57_), .b(new_n56_), .c(x09), .d(x05), .O(z4));
endmodule


