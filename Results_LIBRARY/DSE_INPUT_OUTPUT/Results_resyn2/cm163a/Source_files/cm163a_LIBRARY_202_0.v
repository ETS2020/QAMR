// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_;
  inv1   g00(.a(x04), .O(new_n22_));
  inv1   g01(.a(x00), .O(new_n23_));
  inv1   g02(.a(x05), .O(new_n24_));
  nor2   g03(.a(new_n24_), .b(x04), .O(new_n25_));
  nand2  g04(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g05(.a(x03), .b(x02), .O(new_n27_));
  oai21  g06(.a(new_n27_), .b(x09), .c(x05), .O(new_n28_));
  aoi21  g07(.a(new_n27_), .b(x09), .c(new_n28_), .O(new_n29_));
  oai21  g08(.a(new_n29_), .b(new_n22_), .c(new_n26_), .O(z0));
  inv1   g09(.a(x01), .O(new_n31_));
  nand2  g10(.a(new_n25_), .b(new_n31_), .O(new_n32_));
  oai21  g11(.a(new_n27_), .b(x09), .c(x11), .O(new_n33_));
  inv1   g12(.a(x09), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand4  g14(.a(new_n35_), .b(new_n34_), .c(x03), .d(x02), .O(new_n36_));
  nand3  g15(.a(new_n36_), .b(new_n33_), .c(x05), .O(new_n37_));
  nand2  g16(.a(new_n37_), .b(x04), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(new_n32_), .O(z1));
  inv1   g18(.a(x06), .O(new_n40_));
  nand2  g19(.a(new_n25_), .b(new_n40_), .O(new_n41_));
  nand2  g20(.a(new_n36_), .b(x12), .O(new_n42_));
  inv1   g21(.a(x12), .O(new_n43_));
  nor2   g22(.a(x11), .b(x09), .O(new_n44_));
  nand4  g23(.a(new_n44_), .b(new_n43_), .c(x03), .d(x02), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n42_), .c(x05), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x04), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n41_), .O(z2));
  inv1   g27(.a(x13), .O(new_n49_));
  nor2   g28(.a(new_n45_), .b(new_n49_), .O(new_n50_));
  nand2  g29(.a(new_n45_), .b(new_n49_), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x04), .O(new_n52_));
  inv1   g31(.a(x07), .O(new_n53_));
  aoi21  g32(.a(new_n53_), .b(new_n22_), .c(new_n24_), .O(new_n54_));
  oai21  g33(.a(new_n52_), .b(new_n50_), .c(new_n54_), .O(z3));
  nand2  g34(.a(x08), .b(x03), .O(new_n56_));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  oai22  g36(.a(new_n57_), .b(new_n56_), .c(x05), .d(x04), .O(z4));
endmodule


