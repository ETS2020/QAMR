// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x00), .O(new_n25_));
  inv1   g04(.a(x05), .O(new_n26_));
  aoi21  g05(.a(new_n22_), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g06(.a(new_n24_), .b(new_n22_), .c(new_n27_), .O(z0));
  nand2  g07(.a(x05), .b(x01), .O(new_n29_));
  nand2  g08(.a(new_n29_), .b(new_n22_), .O(new_n30_));
  inv1   g09(.a(x09), .O(new_n31_));
  nand3  g10(.a(new_n31_), .b(x03), .c(x02), .O(new_n32_));
  xor2a  g11(.a(new_n32_), .b(x11), .O(new_n33_));
  nand2  g12(.a(x05), .b(x04), .O(new_n34_));
  oai21  g13(.a(new_n34_), .b(new_n33_), .c(new_n30_), .O(z1));
  nor2   g14(.a(new_n23_), .b(x09), .O(new_n36_));
  inv1   g15(.a(x11), .O(new_n37_));
  nand4  g16(.a(new_n37_), .b(new_n31_), .c(x03), .d(x02), .O(new_n38_));
  nor2   g17(.a(x12), .b(x11), .O(new_n39_));
  aoi22  g18(.a(new_n39_), .b(new_n36_), .c(new_n38_), .d(x12), .O(new_n40_));
  inv1   g19(.a(x06), .O(new_n41_));
  aoi21  g20(.a(new_n41_), .b(new_n22_), .c(new_n26_), .O(new_n42_));
  oai21  g21(.a(new_n40_), .b(new_n22_), .c(new_n42_), .O(z2));
  nand2  g22(.a(x07), .b(x05), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(new_n22_), .O(new_n45_));
  inv1   g24(.a(x13), .O(new_n46_));
  inv1   g25(.a(x12), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n37_), .O(new_n48_));
  oai21  g27(.a(new_n48_), .b(new_n32_), .c(new_n46_), .O(new_n49_));
  nand3  g28(.a(new_n39_), .b(new_n36_), .c(x13), .O(new_n50_));
  nand4  g29(.a(new_n50_), .b(new_n49_), .c(x05), .d(x04), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(new_n45_), .O(z3));
  nand2  g31(.a(x08), .b(x03), .O(new_n53_));
  aoi21  g32(.a(new_n26_), .b(x04), .c(new_n53_), .O(new_n54_));
  nand4  g33(.a(new_n54_), .b(x15), .c(x14), .d(x10), .O(new_n55_));
  inv1   g34(.a(new_n55_), .O(z4));
endmodule


