// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_;
  inv1   g00(.a(x04), .O(new_n22_));
  inv1   g01(.a(x09), .O(new_n23_));
  inv1   g02(.a(x14), .O(new_n24_));
  nand2  g03(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  oai21  g04(.a(x04), .b(x00), .c(x05), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g06(.a(x03), .b(x02), .O(new_n28_));
  nand4  g07(.a(x14), .b(new_n23_), .c(x03), .d(x02), .O(new_n29_));
  inv1   g08(.a(new_n29_), .O(new_n30_));
  aoi21  g09(.a(new_n28_), .b(x09), .c(new_n30_), .O(new_n31_));
  oai21  g10(.a(new_n31_), .b(new_n22_), .c(new_n27_), .O(z0));
  oai21  g11(.a(x04), .b(x01), .c(x05), .O(new_n33_));
  nand2  g12(.a(new_n33_), .b(new_n25_), .O(new_n34_));
  inv1   g13(.a(x11), .O(new_n35_));
  nand2  g14(.a(new_n28_), .b(x14), .O(new_n36_));
  aoi21  g15(.a(new_n36_), .b(new_n23_), .c(new_n35_), .O(new_n37_));
  nor2   g16(.a(new_n29_), .b(x11), .O(new_n38_));
  oai21  g17(.a(new_n38_), .b(new_n37_), .c(x04), .O(new_n39_));
  nand2  g18(.a(new_n39_), .b(new_n34_), .O(z1));
  inv1   g19(.a(x12), .O(new_n41_));
  inv1   g20(.a(new_n28_), .O(new_n42_));
  nor2   g21(.a(x11), .b(new_n22_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(new_n44_));
  nand2  g23(.a(new_n44_), .b(x14), .O(new_n45_));
  nand2  g24(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nor2   g25(.a(new_n41_), .b(new_n22_), .O(new_n47_));
  oai21  g26(.a(new_n29_), .b(x11), .c(new_n47_), .O(new_n48_));
  inv1   g27(.a(x05), .O(new_n49_));
  inv1   g28(.a(x06), .O(new_n50_));
  aoi21  g29(.a(new_n50_), .b(new_n22_), .c(new_n49_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n48_), .c(new_n46_), .O(z2));
  inv1   g31(.a(x13), .O(new_n53_));
  nand4  g32(.a(new_n43_), .b(new_n42_), .c(new_n53_), .d(new_n41_), .O(new_n54_));
  nand2  g33(.a(new_n54_), .b(x14), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n23_), .O(new_n56_));
  nor2   g35(.a(new_n24_), .b(x09), .O(new_n57_));
  nand4  g36(.a(new_n57_), .b(new_n42_), .c(new_n41_), .d(new_n35_), .O(new_n58_));
  nor2   g37(.a(new_n53_), .b(new_n22_), .O(new_n59_));
  oai21  g38(.a(x07), .b(x04), .c(x05), .O(new_n60_));
  aoi21  g39(.a(new_n59_), .b(new_n58_), .c(new_n60_), .O(new_n61_));
  nand2  g40(.a(new_n61_), .b(new_n56_), .O(z3));
  nand2  g41(.a(x08), .b(x03), .O(new_n63_));
  nand3  g42(.a(x15), .b(x14), .c(x10), .O(new_n64_));
  nor2   g43(.a(new_n64_), .b(new_n63_), .O(z4));
endmodule


