// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  inv1   g03(.a(x06), .O(new_n25_));
  oai21  g04(.a(x04), .b(x00), .c(x05), .O(new_n26_));
  aoi21  g05(.a(x09), .b(new_n25_), .c(new_n26_), .O(new_n27_));
  oai21  g06(.a(new_n24_), .b(new_n22_), .c(new_n27_), .O(z0));
  nor2   g07(.a(new_n23_), .b(x09), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(x11), .O(new_n30_));
  inv1   g09(.a(x11), .O(new_n31_));
  oai21  g10(.a(new_n23_), .b(new_n31_), .c(x04), .O(new_n32_));
  oai21  g11(.a(new_n31_), .b(new_n22_), .c(x06), .O(new_n33_));
  oai21  g12(.a(x04), .b(x01), .c(x05), .O(new_n34_));
  aoi21  g13(.a(new_n33_), .b(x09), .c(new_n34_), .O(new_n35_));
  oai21  g14(.a(new_n32_), .b(new_n30_), .c(new_n35_), .O(z1));
  inv1   g15(.a(x09), .O(new_n37_));
  inv1   g16(.a(x12), .O(new_n38_));
  nand2  g17(.a(new_n38_), .b(x05), .O(new_n39_));
  aoi21  g18(.a(new_n39_), .b(x06), .c(new_n37_), .O(new_n40_));
  and2   g19(.a(x03), .b(x02), .O(new_n41_));
  nand4  g20(.a(new_n41_), .b(x12), .c(new_n31_), .d(new_n37_), .O(new_n42_));
  oai21  g21(.a(new_n23_), .b(x11), .c(new_n38_), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n42_), .c(x04), .O(new_n44_));
  inv1   g23(.a(x05), .O(new_n45_));
  aoi21  g24(.a(new_n25_), .b(new_n22_), .c(new_n45_), .O(new_n46_));
  aoi21  g25(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(z2));
  nand4  g26(.a(new_n38_), .b(new_n31_), .c(x03), .d(x02), .O(new_n48_));
  nor3   g27(.a(x13), .b(x12), .c(x11), .O(new_n49_));
  aoi22  g28(.a(new_n49_), .b(new_n29_), .c(new_n48_), .d(x13), .O(new_n50_));
  nand2  g29(.a(x13), .b(x04), .O(new_n51_));
  nand2  g30(.a(new_n51_), .b(x06), .O(new_n52_));
  oai21  g31(.a(x07), .b(x04), .c(x05), .O(new_n53_));
  aoi21  g32(.a(new_n52_), .b(x09), .c(new_n53_), .O(new_n54_));
  oai21  g33(.a(new_n50_), .b(new_n22_), .c(new_n54_), .O(z3));
  nand2  g34(.a(x08), .b(x03), .O(new_n56_));
  aoi21  g35(.a(x09), .b(new_n25_), .c(new_n56_), .O(new_n57_));
  nand4  g36(.a(new_n57_), .b(x15), .c(x14), .d(x10), .O(new_n58_));
  inv1   g37(.a(new_n58_), .O(z4));
endmodule


