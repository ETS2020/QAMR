// Benchmark "FAU" written by ABC on Sat Aug  8 20:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_;
  inv1   g00(.a(x04), .O(new_n22_));
  nand2  g01(.a(x03), .b(x02), .O(new_n23_));
  xor2a  g02(.a(new_n23_), .b(x09), .O(new_n24_));
  nor2   g03(.a(x04), .b(x00), .O(new_n25_));
  nand2  g04(.a(x12), .b(x09), .O(new_n26_));
  nand2  g05(.a(new_n26_), .b(x05), .O(new_n27_));
  nor2   g06(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  oai21  g07(.a(new_n24_), .b(new_n22_), .c(new_n28_), .O(z0));
  inv1   g08(.a(new_n26_), .O(new_n30_));
  oai21  g09(.a(new_n23_), .b(x09), .c(x11), .O(new_n31_));
  inv1   g10(.a(x09), .O(new_n32_));
  inv1   g11(.a(x11), .O(new_n33_));
  nand4  g12(.a(new_n33_), .b(new_n32_), .c(x03), .d(x02), .O(new_n34_));
  nand2  g13(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g14(.a(new_n35_), .b(x04), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  inv1   g16(.a(x05), .O(new_n38_));
  aoi21  g17(.a(new_n22_), .b(new_n37_), .c(new_n38_), .O(new_n39_));
  aoi21  g18(.a(new_n39_), .b(new_n36_), .c(new_n30_), .O(z1));
  inv1   g19(.a(x12), .O(new_n41_));
  nand2  g20(.a(new_n34_), .b(new_n41_), .O(new_n42_));
  nor2   g21(.a(new_n23_), .b(x11), .O(new_n43_));
  nand2  g22(.a(new_n43_), .b(x12), .O(new_n44_));
  nand3  g23(.a(new_n44_), .b(new_n42_), .c(x04), .O(new_n45_));
  nor2   g24(.a(x06), .b(x04), .O(new_n46_));
  nor2   g25(.a(new_n46_), .b(new_n27_), .O(new_n47_));
  nand2  g26(.a(new_n47_), .b(new_n45_), .O(z2));
  nand4  g27(.a(new_n43_), .b(x13), .c(new_n41_), .d(new_n32_), .O(new_n49_));
  nand2  g28(.a(new_n26_), .b(x13), .O(new_n50_));
  oai21  g29(.a(new_n34_), .b(x12), .c(new_n50_), .O(new_n51_));
  nand3  g30(.a(new_n51_), .b(new_n49_), .c(x04), .O(new_n52_));
  aoi21  g31(.a(x13), .b(x09), .c(x05), .O(new_n53_));
  aoi21  g32(.a(x07), .b(x05), .c(new_n30_), .O(new_n54_));
  oai21  g33(.a(new_n53_), .b(new_n22_), .c(new_n54_), .O(new_n55_));
  nand2  g34(.a(new_n55_), .b(new_n52_), .O(z3));
  nand3  g35(.a(x15), .b(x14), .c(x10), .O(new_n57_));
  nand3  g36(.a(new_n26_), .b(x08), .c(x03), .O(new_n58_));
  nor2   g37(.a(new_n58_), .b(new_n57_), .O(z4));
endmodule


