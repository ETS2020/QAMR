// Benchmark "FAU" written by ABC on Sat Aug  8 20:54:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z0, z1, z2, z3, z4  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z0, z1, z2, z3, z4;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n28_,
    new_n29_, new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_;
  inv1   g00(.a(x09), .O(new_n22_));
  and2   g01(.a(x03), .b(x02), .O(new_n23_));
  nor2   g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g03(.a(x03), .b(x02), .O(new_n25_));
  nor2   g04(.a(new_n25_), .b(x09), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n24_), .c(x04), .O(new_n27_));
  inv1   g06(.a(x00), .O(new_n28_));
  inv1   g07(.a(x04), .O(new_n29_));
  inv1   g08(.a(x05), .O(new_n30_));
  aoi21  g09(.a(new_n29_), .b(new_n28_), .c(new_n30_), .O(new_n31_));
  nand2  g10(.a(new_n31_), .b(new_n27_), .O(z0));
  inv1   g11(.a(x03), .O(new_n33_));
  nand4  g12(.a(x11), .b(new_n22_), .c(x04), .d(x02), .O(new_n34_));
  aoi21  g13(.a(new_n34_), .b(x05), .c(new_n33_), .O(new_n35_));
  oai21  g14(.a(new_n26_), .b(x11), .c(x04), .O(new_n36_));
  inv1   g15(.a(x01), .O(new_n37_));
  aoi21  g16(.a(new_n29_), .b(new_n37_), .c(new_n30_), .O(new_n38_));
  aoi21  g17(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z1));
  inv1   g18(.a(x12), .O(new_n40_));
  oai21  g19(.a(new_n40_), .b(new_n29_), .c(x05), .O(new_n41_));
  nand2  g20(.a(new_n41_), .b(new_n33_), .O(new_n42_));
  nor2   g21(.a(x12), .b(x11), .O(new_n43_));
  nand3  g22(.a(new_n43_), .b(new_n23_), .c(new_n22_), .O(new_n44_));
  inv1   g23(.a(x11), .O(new_n45_));
  nand3  g24(.a(new_n45_), .b(new_n22_), .c(x02), .O(new_n46_));
  nand2  g25(.a(new_n46_), .b(x12), .O(new_n47_));
  nand3  g26(.a(new_n47_), .b(new_n44_), .c(x04), .O(new_n48_));
  aoi21  g27(.a(x06), .b(new_n29_), .c(new_n30_), .O(new_n49_));
  nand2  g28(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g29(.a(new_n50_), .b(new_n42_), .O(z2));
  inv1   g30(.a(x13), .O(new_n52_));
  oai21  g31(.a(new_n52_), .b(new_n29_), .c(x05), .O(new_n53_));
  nand2  g32(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nand4  g33(.a(new_n43_), .b(new_n23_), .c(new_n52_), .d(new_n22_), .O(new_n55_));
  nand4  g34(.a(new_n40_), .b(new_n45_), .c(new_n22_), .d(x02), .O(new_n56_));
  nand2  g35(.a(new_n56_), .b(x13), .O(new_n57_));
  nand3  g36(.a(new_n57_), .b(new_n55_), .c(x04), .O(new_n58_));
  aoi21  g37(.a(x07), .b(new_n29_), .c(new_n30_), .O(new_n59_));
  nand2  g38(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g39(.a(new_n60_), .b(new_n54_), .O(z3));
  nand4  g40(.a(x15), .b(x14), .c(x10), .d(x08), .O(new_n62_));
  nor3   g41(.a(new_n62_), .b(new_n30_), .c(new_n33_), .O(z4));
endmodule


