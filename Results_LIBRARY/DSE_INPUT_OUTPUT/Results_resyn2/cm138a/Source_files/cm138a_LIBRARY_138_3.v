// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n29_, new_n30_, new_n31_, new_n34_,
    new_n35_, new_n37_;
  nor2   g00(.a(x5), .b(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x4), .b(new_n16_), .O(new_n17_));
  nor2   g03(.a(x2), .b(x1), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x5), .O(new_n20_));
  nand4  g06(.a(new_n18_), .b(new_n17_), .c(new_n20_), .d(x0), .O(z1));
  xnor2a g07(.a(x5), .b(x0), .O(new_n22_));
  inv1   g08(.a(x2), .O(new_n23_));
  inv1   g09(.a(x4), .O(new_n24_));
  nand4  g10(.a(new_n24_), .b(x3), .c(new_n23_), .d(x1), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n20_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n22_), .O(z2));
  nand2  g13(.a(new_n26_), .b(x0), .O(z3));
  inv1   g14(.a(x1), .O(new_n29_));
  nand4  g15(.a(new_n24_), .b(x3), .c(x2), .d(new_n29_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n20_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n22_), .O(z4));
  nand2  g18(.a(new_n31_), .b(x0), .O(z5));
  nand4  g19(.a(new_n24_), .b(x3), .c(x2), .d(x1), .O(new_n34_));
  inv1   g20(.a(new_n34_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n15_), .O(z6));
  nand2  g22(.a(new_n34_), .b(new_n20_), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(x0), .O(z7));
endmodule


