// Benchmark "FAU" written by ABC on Sat Aug  8 22:45:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n33_, new_n34_, new_n37_, new_n38_;
  xor2a  g00(.a(x3), .b(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nor2   g03(.a(x5), .b(x4), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(x3), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n15_), .O(z0));
  nand2  g07(.a(new_n20_), .b(x0), .O(z1));
  inv1   g08(.a(x0), .O(new_n23_));
  nor2   g09(.a(x3), .b(new_n23_), .O(new_n24_));
  inv1   g10(.a(x3), .O(new_n25_));
  nor2   g11(.a(new_n25_), .b(x0), .O(new_n26_));
  inv1   g12(.a(x4), .O(new_n27_));
  inv1   g13(.a(x5), .O(new_n28_));
  nand4  g14(.a(new_n28_), .b(new_n27_), .c(new_n17_), .d(x1), .O(new_n29_));
  inv1   g15(.a(new_n29_), .O(new_n30_));
  aoi21  g16(.a(new_n30_), .b(new_n26_), .c(new_n24_), .O(z2));
  nand3  g17(.a(new_n30_), .b(x3), .c(x0), .O(z3));
  nand3  g18(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(x3), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n15_), .O(z4));
  nand2  g21(.a(new_n34_), .b(x0), .O(z5));
  nand3  g22(.a(new_n28_), .b(new_n27_), .c(x3), .O(new_n37_));
  inv1   g23(.a(new_n37_), .O(new_n38_));
  nand4  g24(.a(new_n38_), .b(x2), .c(x1), .d(new_n23_), .O(z6));
  nand4  g25(.a(new_n38_), .b(x2), .c(x1), .d(x0), .O(z7));
endmodule


