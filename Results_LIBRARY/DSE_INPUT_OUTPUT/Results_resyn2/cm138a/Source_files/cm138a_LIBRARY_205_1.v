// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:50 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n30_, new_n32_,
    new_n33_;
  nor2   g00(.a(x1), .b(x0), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  inv1   g02(.a(x4), .O(new_n17_));
  inv1   g03(.a(x5), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  inv1   g05(.a(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n15_), .c(x3), .O(z0));
  inv1   g07(.a(x1), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(x0), .O(new_n23_));
  oai21  g09(.a(new_n23_), .b(new_n19_), .c(x3), .O(new_n24_));
  inv1   g10(.a(x3), .O(new_n25_));
  nand2  g11(.a(new_n17_), .b(new_n25_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n24_), .O(z1));
  inv1   g13(.a(x0), .O(new_n28_));
  nand4  g14(.a(new_n20_), .b(x3), .c(x1), .d(new_n28_), .O(z2));
  nor2   g15(.a(new_n22_), .b(new_n28_), .O(new_n30_));
  nand3  g16(.a(new_n30_), .b(new_n20_), .c(x3), .O(z3));
  nand4  g17(.a(new_n18_), .b(new_n17_), .c(x3), .d(x2), .O(new_n32_));
  inv1   g18(.a(new_n32_), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n15_), .O(z4));
  nand3  g20(.a(new_n33_), .b(new_n22_), .c(x0), .O(z5));
  nand3  g21(.a(new_n33_), .b(x1), .c(new_n28_), .O(z6));
  nand2  g22(.a(new_n33_), .b(new_n30_), .O(z7));
endmodule


