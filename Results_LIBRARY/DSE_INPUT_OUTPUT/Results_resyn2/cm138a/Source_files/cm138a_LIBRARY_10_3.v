// Benchmark "FAU" written by ABC on Sat Aug  8 22:45:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n39_;
  inv1   g00(.a(x5), .O(new_n15_));
  nand2  g01(.a(new_n15_), .b(x2), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x0), .O(new_n18_));
  inv1   g04(.a(x1), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g06(.a(x4), .O(new_n21_));
  nand3  g07(.a(new_n15_), .b(new_n21_), .c(x3), .O(new_n22_));
  oai21  g08(.a(new_n22_), .b(new_n20_), .c(new_n17_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n16_), .O(z0));
  nand2  g10(.a(new_n19_), .b(x0), .O(new_n25_));
  oai21  g11(.a(new_n25_), .b(new_n22_), .c(new_n17_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n16_), .O(z1));
  inv1   g13(.a(new_n22_), .O(new_n28_));
  nand4  g14(.a(new_n28_), .b(new_n17_), .c(x1), .d(new_n18_), .O(z2));
  nand2  g15(.a(x1), .b(x0), .O(new_n30_));
  oai21  g16(.a(new_n30_), .b(new_n22_), .c(new_n17_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n16_), .O(z3));
  nand2  g18(.a(new_n21_), .b(x3), .O(new_n33_));
  oai21  g19(.a(new_n33_), .b(new_n20_), .c(new_n15_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x2), .O(z4));
  oai21  g21(.a(new_n25_), .b(new_n33_), .c(new_n15_), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(x2), .O(z5));
  nand4  g23(.a(new_n28_), .b(x2), .c(x1), .d(new_n18_), .O(z6));
  oai21  g24(.a(new_n30_), .b(new_n33_), .c(new_n15_), .O(new_n39_));
  nand2  g25(.a(new_n39_), .b(x2), .O(z7));
endmodule


