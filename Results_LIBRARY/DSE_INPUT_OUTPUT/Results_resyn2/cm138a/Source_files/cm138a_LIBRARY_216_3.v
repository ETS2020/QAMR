// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n27_, new_n29_;
  nor2   g00(.a(x2), .b(x1), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x4), .b(new_n16_), .O(new_n17_));
  nor2   g03(.a(x5), .b(x0), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x4), .O(new_n20_));
  inv1   g06(.a(x5), .O(new_n21_));
  nand4  g07(.a(new_n21_), .b(new_n20_), .c(x3), .d(x0), .O(new_n22_));
  inv1   g08(.a(new_n22_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n15_), .O(z1));
  inv1   g10(.a(x2), .O(new_n25_));
  nand4  g11(.a(new_n18_), .b(new_n17_), .c(new_n25_), .d(x1), .O(z2));
  nand2  g12(.a(new_n22_), .b(new_n25_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(x1), .O(z3));
  nor2   g14(.a(new_n25_), .b(x1), .O(new_n29_));
  nand3  g15(.a(new_n29_), .b(new_n18_), .c(new_n17_), .O(z4));
  nand2  g16(.a(new_n29_), .b(new_n23_), .O(z5));
  nand2  g17(.a(x2), .b(x1), .O(z7));
  one    g18(.O(z6));
endmodule


