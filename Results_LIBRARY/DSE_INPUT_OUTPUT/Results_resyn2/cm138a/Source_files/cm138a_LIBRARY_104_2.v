// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n28_;
  inv1   g00(.a(x2), .O(new_n15_));
  nor3   g01(.a(x4), .b(x1), .c(x0), .O(new_n16_));
  inv1   g02(.a(x3), .O(new_n17_));
  nor2   g03(.a(x5), .b(new_n17_), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n16_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x4), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(x0), .O(z1));
  inv1   g07(.a(x0), .O(new_n22_));
  inv1   g08(.a(x5), .O(new_n23_));
  nand4  g09(.a(new_n23_), .b(x3), .c(new_n15_), .d(x1), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n20_), .O(z2));
  nand3  g12(.a(new_n18_), .b(new_n16_), .c(x2), .O(z4));
  nor2   g13(.a(x4), .b(x0), .O(new_n28_));
  nand4  g14(.a(new_n18_), .b(new_n28_), .c(x2), .d(x1), .O(z6));
  buf    g15(.a(x0), .O(z3));
  nand2  g16(.a(new_n20_), .b(x0), .O(z5));
  nand2  g17(.a(new_n20_), .b(x0), .O(z7));
endmodule


