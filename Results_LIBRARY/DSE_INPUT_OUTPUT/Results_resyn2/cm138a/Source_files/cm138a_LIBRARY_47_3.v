// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n31_,
    new_n34_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nor2   g03(.a(x5), .b(x4), .O(new_n18_));
  nand4  g04(.a(new_n18_), .b(x3), .c(new_n17_), .d(new_n16_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n15_), .O(new_n20_));
  nand2  g06(.a(new_n17_), .b(x1), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n20_), .O(z0));
  inv1   g08(.a(x4), .O(new_n23_));
  inv1   g09(.a(x5), .O(new_n24_));
  nand3  g10(.a(new_n24_), .b(new_n23_), .c(x3), .O(new_n25_));
  nand2  g11(.a(new_n17_), .b(x0), .O(new_n26_));
  oai21  g12(.a(new_n26_), .b(new_n25_), .c(new_n15_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(new_n21_), .O(z1));
  inv1   g14(.a(new_n25_), .O(new_n29_));
  nand4  g15(.a(new_n29_), .b(new_n17_), .c(x1), .d(new_n16_), .O(z2));
  nor2   g16(.a(new_n15_), .b(new_n16_), .O(new_n31_));
  nand4  g17(.a(new_n31_), .b(new_n18_), .c(x3), .d(new_n17_), .O(z3));
  nand4  g18(.a(new_n29_), .b(x2), .c(new_n15_), .d(new_n16_), .O(z4));
  oai21  g19(.a(new_n25_), .b(new_n16_), .c(new_n15_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x2), .O(z5));
  one    g21(.O(z6));
  one    g22(.O(z7));
endmodule


