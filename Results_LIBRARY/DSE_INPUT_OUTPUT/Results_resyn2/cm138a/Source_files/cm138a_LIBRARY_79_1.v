// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n34_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x4), .O(new_n16_));
  inv1   g02(.a(x5), .O(new_n17_));
  nand4  g03(.a(new_n17_), .b(new_n16_), .c(x3), .d(new_n15_), .O(new_n18_));
  inv1   g04(.a(new_n18_), .O(new_n19_));
  nor2   g05(.a(x2), .b(x1), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n19_), .O(z0));
  nand4  g07(.a(new_n17_), .b(new_n16_), .c(x3), .d(x0), .O(new_n22_));
  inv1   g08(.a(new_n22_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n20_), .O(z1));
  nand2  g10(.a(new_n18_), .b(x1), .O(new_n25_));
  inv1   g11(.a(x1), .O(new_n26_));
  nand2  g12(.a(x2), .b(new_n26_), .O(z4));
  inv1   g13(.a(x2), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(x1), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(z4), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n25_), .O(z2));
  nand3  g17(.a(new_n23_), .b(new_n28_), .c(x1), .O(z3));
  nand2  g18(.a(new_n25_), .b(x2), .O(z6));
  nand2  g19(.a(new_n22_), .b(x1), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x2), .O(z7));
  nand2  g21(.a(x2), .b(new_n26_), .O(z5));
endmodule


