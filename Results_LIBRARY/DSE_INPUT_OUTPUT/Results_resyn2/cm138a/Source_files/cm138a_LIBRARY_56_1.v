// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n40_, new_n42_;
  nor2   g00(.a(x1), .b(x0), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  inv1   g02(.a(x4), .O(new_n17_));
  inv1   g03(.a(x5), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .O(new_n19_));
  inv1   g05(.a(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n15_), .c(x3), .O(z0));
  inv1   g07(.a(x0), .O(new_n22_));
  nor2   g08(.a(x1), .b(new_n22_), .O(new_n23_));
  nand3  g09(.a(new_n23_), .b(new_n20_), .c(x3), .O(z1));
  inv1   g10(.a(x1), .O(new_n25_));
  nor2   g11(.a(new_n25_), .b(x0), .O(new_n26_));
  nand3  g12(.a(new_n26_), .b(new_n20_), .c(x3), .O(z2));
  nand2  g13(.a(x1), .b(x0), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(new_n19_), .c(x3), .O(new_n29_));
  inv1   g15(.a(x3), .O(new_n30_));
  nand2  g16(.a(new_n18_), .b(new_n30_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n29_), .O(z3));
  nor2   g18(.a(x5), .b(x4), .O(new_n33_));
  nand3  g19(.a(new_n33_), .b(new_n15_), .c(x2), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x3), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n31_), .O(z4));
  nand4  g22(.a(new_n18_), .b(new_n17_), .c(x3), .d(x2), .O(new_n37_));
  inv1   g23(.a(new_n37_), .O(new_n38_));
  nand2  g24(.a(new_n38_), .b(new_n23_), .O(z5));
  nor2   g25(.a(new_n18_), .b(x3), .O(new_n40_));
  aoi21  g26(.a(new_n38_), .b(new_n26_), .c(new_n40_), .O(z6));
  inv1   g27(.a(new_n28_), .O(new_n42_));
  aoi21  g28(.a(new_n38_), .b(new_n42_), .c(new_n40_), .O(z7));
endmodule


