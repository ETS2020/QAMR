// Benchmark "FAU" written by ABC on Sat Aug  8 22:45:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n31_;
  nor2   g00(.a(x1), .b(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nor2   g02(.a(x4), .b(new_n16_), .O(new_n17_));
  nor2   g03(.a(x5), .b(x2), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x0), .O(new_n20_));
  nor2   g06(.a(x1), .b(new_n20_), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n18_), .c(new_n17_), .O(z1));
  nand4  g08(.a(new_n18_), .b(new_n17_), .c(x1), .d(new_n20_), .O(z2));
  inv1   g09(.a(x4), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand4  g11(.a(new_n26_), .b(new_n25_), .c(x3), .d(x2), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n15_), .O(z4));
  nand2  g14(.a(new_n28_), .b(new_n21_), .O(z5));
  nand2  g15(.a(new_n27_), .b(new_n20_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(z6));
  nand2  g17(.a(x1), .b(x0), .O(z7));
  one    g18(.O(z3));
endmodule


