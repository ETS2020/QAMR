// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n36_, new_n37_;
  nor2   g00(.a(x2), .b(x1), .O(new_n15_));
  inv1   g01(.a(new_n15_), .O(z0));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x5), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nor2   g05(.a(x4), .b(new_n20_), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(x0), .O(new_n23_));
  nand4  g08(.a(new_n23_), .b(new_n21_), .c(new_n19_), .d(new_n18_), .O(z2));
  inv1   g09(.a(x4), .O(new_n25_));
  nand4  g10(.a(new_n19_), .b(new_n25_), .c(x3), .d(x0), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(z3));
  nand4  g13(.a(new_n19_), .b(new_n25_), .c(x3), .d(x2), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nor2   g15(.a(x1), .b(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z4));
  nand2  g17(.a(new_n26_), .b(x2), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n22_), .O(z5));
  aoi21  g19(.a(new_n30_), .b(new_n23_), .c(new_n15_), .O(z6));
  nand2  g20(.a(x2), .b(x1), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(z0), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n27_), .O(z7));
  one    g23(.O(z1));
endmodule


