// Benchmark "FAU" written by ABC on Sat Aug  8 22:46:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n38_, new_n39_, new_n40_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nor2   g03(.a(x5), .b(x4), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(x3), .c(new_n17_), .O(new_n19_));
  inv1   g05(.a(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n16_), .c(new_n15_), .O(z0));
  nand3  g07(.a(new_n20_), .b(new_n16_), .c(x0), .O(z1));
  nor2   g08(.a(x3), .b(new_n15_), .O(new_n23_));
  inv1   g09(.a(x3), .O(new_n24_));
  nor2   g10(.a(new_n24_), .b(x0), .O(new_n25_));
  inv1   g11(.a(x4), .O(new_n26_));
  inv1   g12(.a(x5), .O(new_n27_));
  nand4  g13(.a(new_n27_), .b(new_n26_), .c(new_n17_), .d(x1), .O(new_n28_));
  inv1   g14(.a(new_n28_), .O(new_n29_));
  aoi21  g15(.a(new_n29_), .b(new_n25_), .c(new_n23_), .O(z2));
  nor2   g16(.a(x2), .b(new_n16_), .O(new_n31_));
  nand4  g17(.a(new_n31_), .b(new_n18_), .c(x3), .d(x0), .O(z3));
  nand3  g18(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n33_));
  inv1   g19(.a(new_n33_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n25_), .O(z4));
  nand2  g21(.a(new_n33_), .b(x3), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(x0), .O(z5));
  nand3  g23(.a(new_n18_), .b(x2), .c(x1), .O(new_n38_));
  nand2  g24(.a(new_n38_), .b(x3), .O(new_n39_));
  xor2a  g25(.a(x3), .b(x0), .O(new_n40_));
  nand2  g26(.a(new_n40_), .b(new_n39_), .O(z6));
  nand2  g27(.a(new_n39_), .b(x0), .O(z7));
endmodule


