// Benchmark "FAU" written by ABC on Sun Aug  9 00:28:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_;
  inv1   g000(.a(x9), .O(new_n17_));
  inv1   g001(.a(x6), .O(new_n18_));
  nor2   g002(.a(new_n18_), .b(x4), .O(new_n19_));
  inv1   g003(.a(x2), .O(new_n20_));
  inv1   g004(.a(x5), .O(new_n21_));
  nor2   g005(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g006(.a(new_n19_), .b(x7), .c(new_n22_), .O(new_n23_));
  nand2  g007(.a(x7), .b(x6), .O(new_n24_));
  inv1   g008(.a(x7), .O(new_n25_));
  nand2  g009(.a(x6), .b(x4), .O(new_n26_));
  oai21  g010(.a(new_n25_), .b(x5), .c(new_n26_), .O(new_n27_));
  nand2  g011(.a(new_n27_), .b(new_n20_), .O(new_n28_));
  nand3  g012(.a(new_n28_), .b(new_n24_), .c(new_n23_), .O(new_n29_));
  nand2  g013(.a(new_n29_), .b(x0), .O(new_n30_));
  oai21  g014(.a(new_n19_), .b(new_n21_), .c(new_n25_), .O(new_n31_));
  oai22  g015(.a(new_n31_), .b(x0), .c(new_n18_), .d(x5), .O(new_n32_));
  nand2  g016(.a(new_n32_), .b(new_n20_), .O(new_n33_));
  inv1   g017(.a(x0), .O(new_n34_));
  nor2   g018(.a(x4), .b(new_n20_), .O(new_n35_));
  inv1   g019(.a(x4), .O(new_n36_));
  aoi21  g020(.a(x7), .b(new_n21_), .c(new_n36_), .O(new_n37_));
  nand2  g021(.a(new_n18_), .b(x2), .O(new_n38_));
  oai22  g022(.a(new_n38_), .b(new_n37_), .c(x5), .d(x4), .O(new_n39_));
  aoi22  g023(.a(new_n39_), .b(new_n34_), .c(new_n35_), .d(x7), .O(new_n40_));
  nand3  g024(.a(new_n40_), .b(new_n33_), .c(new_n30_), .O(new_n41_));
  nand2  g025(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  inv1   g026(.a(x8), .O(new_n43_));
  nand3  g027(.a(x9), .b(new_n18_), .c(x4), .O(new_n44_));
  oai21  g028(.a(new_n18_), .b(x4), .c(x7), .O(new_n45_));
  nand3  g029(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  aoi21  g030(.a(x9), .b(new_n25_), .c(new_n19_), .O(new_n47_));
  nand2  g031(.a(new_n47_), .b(x8), .O(new_n48_));
  nand3  g032(.a(new_n48_), .b(new_n46_), .c(x0), .O(new_n49_));
  nand4  g033(.a(x9), .b(new_n43_), .c(new_n36_), .d(new_n34_), .O(new_n50_));
  aoi21  g034(.a(new_n50_), .b(new_n49_), .c(new_n20_), .O(new_n51_));
  nand2  g035(.a(new_n25_), .b(x4), .O(new_n52_));
  nand2  g036(.a(new_n43_), .b(x0), .O(new_n53_));
  aoi21  g037(.a(new_n52_), .b(new_n18_), .c(new_n53_), .O(new_n54_));
  aoi21  g038(.a(x6), .b(x0), .c(new_n43_), .O(new_n55_));
  oai21  g039(.a(new_n55_), .b(new_n54_), .c(new_n20_), .O(new_n56_));
  oai21  g040(.a(x7), .b(x6), .c(new_n43_), .O(new_n57_));
  nand3  g041(.a(new_n57_), .b(x4), .c(new_n34_), .O(new_n58_));
  aoi21  g042(.a(new_n58_), .b(new_n56_), .c(new_n17_), .O(new_n59_));
  oai21  g043(.a(new_n59_), .b(new_n51_), .c(x5), .O(new_n60_));
  nand2  g044(.a(new_n60_), .b(new_n42_), .O(z0));
  nand2  g045(.a(new_n43_), .b(new_n20_), .O(new_n62_));
  nand2  g046(.a(x8), .b(x2), .O(new_n63_));
  nand2  g047(.a(new_n63_), .b(x0), .O(new_n64_));
  nor2   g048(.a(new_n18_), .b(x3), .O(new_n65_));
  inv1   g049(.a(new_n65_), .O(new_n66_));
  aoi21  g050(.a(new_n64_), .b(new_n62_), .c(new_n66_), .O(new_n67_));
  nor2   g051(.a(new_n43_), .b(x6), .O(new_n68_));
  oai21  g052(.a(x4), .b(new_n20_), .c(new_n43_), .O(new_n69_));
  inv1   g053(.a(x3), .O(new_n70_));
  aoi21  g054(.a(new_n63_), .b(x0), .c(new_n70_), .O(new_n71_));
  aoi21  g055(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(new_n72_));
  nor2   g056(.a(new_n72_), .b(x7), .O(new_n73_));
  oai21  g057(.a(new_n73_), .b(new_n67_), .c(x1), .O(new_n74_));
  inv1   g058(.a(new_n69_), .O(new_n75_));
  oai21  g059(.a(new_n20_), .b(new_n34_), .c(new_n70_), .O(new_n76_));
  oai21  g060(.a(new_n76_), .b(new_n75_), .c(new_n52_), .O(new_n77_));
  nand2  g061(.a(new_n77_), .b(new_n18_), .O(new_n78_));
  aoi21  g062(.a(new_n78_), .b(new_n74_), .c(new_n17_), .O(new_n79_));
  nand3  g063(.a(new_n17_), .b(new_n36_), .c(x3), .O(new_n80_));
  nor2   g064(.a(x8), .b(x3), .O(new_n81_));
  nand2  g065(.a(new_n81_), .b(x4), .O(new_n82_));
  nand2  g066(.a(x6), .b(x1), .O(new_n83_));
  aoi21  g067(.a(new_n82_), .b(new_n80_), .c(new_n83_), .O(new_n84_));
  oai21  g068(.a(new_n84_), .b(new_n79_), .c(x5), .O(new_n85_));
  inv1   g069(.a(x1), .O(new_n86_));
  nor2   g070(.a(x5), .b(x3), .O(new_n87_));
  oai21  g071(.a(new_n87_), .b(x6), .c(new_n17_), .O(new_n88_));
  nor2   g072(.a(x6), .b(new_n70_), .O(new_n89_));
  nand2  g073(.a(new_n89_), .b(x2), .O(new_n90_));
  aoi21  g074(.a(new_n90_), .b(new_n26_), .c(new_n34_), .O(new_n91_));
  nand3  g075(.a(new_n64_), .b(new_n62_), .c(new_n19_), .O(new_n92_));
  oai21  g076(.a(x6), .b(x2), .c(new_n36_), .O(new_n93_));
  aoi21  g077(.a(new_n93_), .b(new_n43_), .c(new_n17_), .O(new_n94_));
  aoi21  g078(.a(new_n94_), .b(new_n92_), .c(new_n70_), .O(new_n95_));
  oai21  g079(.a(new_n95_), .b(new_n91_), .c(x5), .O(new_n96_));
  aoi21  g080(.a(new_n96_), .b(new_n88_), .c(new_n86_), .O(new_n97_));
  nor2   g081(.a(new_n20_), .b(new_n34_), .O(new_n98_));
  nand3  g082(.a(new_n98_), .b(new_n18_), .c(x5), .O(new_n99_));
  nand2  g083(.a(new_n99_), .b(x9), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(new_n36_), .O(new_n101_));
  nand4  g085(.a(new_n17_), .b(new_n18_), .c(new_n21_), .d(new_n86_), .O(new_n102_));
  aoi21  g086(.a(new_n102_), .b(new_n101_), .c(new_n70_), .O(new_n103_));
  oai21  g087(.a(new_n103_), .b(new_n97_), .c(x7), .O(new_n104_));
  inv1   g088(.a(new_n55_), .O(new_n105_));
  nand2  g089(.a(new_n53_), .b(x2), .O(new_n106_));
  nand3  g090(.a(new_n106_), .b(new_n105_), .c(x3), .O(new_n107_));
  oai21  g091(.a(new_n43_), .b(new_n18_), .c(x0), .O(new_n108_));
  nand3  g092(.a(new_n108_), .b(new_n70_), .c(x2), .O(new_n109_));
  aoi21  g093(.a(new_n109_), .b(new_n107_), .c(x4), .O(new_n110_));
  oai21  g094(.a(x7), .b(new_n20_), .c(x0), .O(new_n111_));
  oai21  g095(.a(x4), .b(new_n70_), .c(new_n111_), .O(new_n112_));
  nor2   g096(.a(x6), .b(x2), .O(new_n113_));
  nand2  g097(.a(new_n113_), .b(x4), .O(new_n114_));
  aoi21  g098(.a(new_n114_), .b(new_n112_), .c(new_n43_), .O(new_n115_));
  nor2   g099(.a(new_n17_), .b(new_n21_), .O(new_n116_));
  oai21  g100(.a(new_n115_), .b(new_n110_), .c(new_n116_), .O(new_n117_));
  oai21  g101(.a(new_n89_), .b(new_n21_), .c(new_n36_), .O(new_n118_));
  oai21  g102(.a(new_n31_), .b(x3), .c(new_n118_), .O(new_n119_));
  nand2  g103(.a(new_n119_), .b(new_n17_), .O(new_n120_));
  nand2  g104(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  aoi21  g105(.a(x4), .b(x1), .c(new_n21_), .O(new_n122_));
  nor3   g106(.a(new_n122_), .b(new_n66_), .c(x9), .O(new_n123_));
  aoi21  g107(.a(new_n121_), .b(new_n86_), .c(new_n123_), .O(new_n124_));
  nand3  g108(.a(new_n124_), .b(new_n104_), .c(new_n85_), .O(z1));
  nand2  g109(.a(x9), .b(new_n21_), .O(new_n126_));
  nand2  g110(.a(new_n70_), .b(x1), .O(new_n127_));
  nand2  g111(.a(x3), .b(new_n86_), .O(new_n128_));
  and2   g112(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  and2   g113(.a(new_n129_), .b(new_n126_), .O(z2));
  oai21  g114(.a(new_n70_), .b(new_n86_), .c(new_n126_), .O(z3));
  inv1   g115(.a(new_n116_), .O(new_n132_));
  nand3  g116(.a(x7), .b(x3), .c(new_n20_), .O(new_n133_));
  oai21  g117(.a(x8), .b(x6), .c(new_n133_), .O(new_n134_));
  nand2  g118(.a(new_n134_), .b(x1), .O(new_n135_));
  oai21  g119(.a(new_n25_), .b(x2), .c(x6), .O(new_n136_));
  oai21  g120(.a(new_n25_), .b(new_n20_), .c(new_n68_), .O(new_n137_));
  nand3  g121(.a(new_n137_), .b(new_n136_), .c(new_n70_), .O(new_n138_));
  aoi21  g122(.a(new_n138_), .b(new_n135_), .c(new_n34_), .O(new_n139_));
  nand2  g123(.a(new_n20_), .b(x0), .O(new_n140_));
  nand2  g124(.a(x2), .b(new_n34_), .O(new_n141_));
  nand2  g125(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nor2   g126(.a(x7), .b(new_n18_), .O(new_n143_));
  nand2  g127(.a(new_n66_), .b(new_n86_), .O(new_n144_));
  aoi21  g128(.a(new_n70_), .b(x1), .c(x8), .O(new_n145_));
  aoi22  g129(.a(new_n145_), .b(new_n144_), .c(new_n143_), .d(new_n129_), .O(new_n146_));
  nor2   g130(.a(new_n25_), .b(new_n86_), .O(new_n147_));
  oai21  g131(.a(new_n89_), .b(new_n65_), .c(new_n147_), .O(new_n148_));
  oai21  g132(.a(new_n146_), .b(new_n142_), .c(new_n148_), .O(new_n149_));
  oai21  g133(.a(new_n149_), .b(new_n139_), .c(new_n36_), .O(new_n150_));
  nor2   g134(.a(new_n70_), .b(new_n86_), .O(new_n151_));
  oai21  g135(.a(new_n151_), .b(new_n98_), .c(x7), .O(new_n152_));
  nand2  g136(.a(new_n140_), .b(new_n127_), .O(new_n153_));
  aoi21  g137(.a(new_n153_), .b(x6), .c(new_n36_), .O(new_n154_));
  aoi22  g138(.a(new_n154_), .b(new_n152_), .c(new_n113_), .d(new_n81_), .O(new_n155_));
  aoi21  g139(.a(new_n155_), .b(new_n150_), .c(new_n132_), .O(z4));
  inv1   g140(.a(new_n129_), .O(new_n157_));
  oai21  g141(.a(new_n142_), .b(new_n157_), .c(new_n126_), .O(z5));
endmodule


