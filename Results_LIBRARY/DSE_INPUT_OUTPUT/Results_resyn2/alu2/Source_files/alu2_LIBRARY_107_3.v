// Benchmark "FAU" written by ABC on Sun Aug  9 00:28:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n163_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n181_, new_n182_,
    new_n183_, new_n184_, new_n185_, new_n187_;
  inv1   g000(.a(x9), .O(new_n17_));
  nor2   g001(.a(x8), .b(x5), .O(new_n18_));
  oai21  g002(.a(new_n18_), .b(new_n17_), .c(x4), .O(new_n19_));
  inv1   g003(.a(x7), .O(new_n20_));
  nand2  g004(.a(new_n18_), .b(new_n20_), .O(new_n21_));
  nand2  g005(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  nand2  g006(.a(new_n22_), .b(x6), .O(new_n23_));
  inv1   g007(.a(x5), .O(new_n24_));
  inv1   g008(.a(x4), .O(new_n25_));
  nand2  g009(.a(x8), .b(new_n25_), .O(new_n26_));
  aoi21  g010(.a(new_n26_), .b(x9), .c(new_n20_), .O(new_n27_));
  nand2  g011(.a(x9), .b(x8), .O(new_n28_));
  nor2   g012(.a(new_n28_), .b(x6), .O(new_n29_));
  oai21  g013(.a(new_n29_), .b(new_n27_), .c(new_n24_), .O(new_n30_));
  nand3  g014(.a(new_n30_), .b(new_n23_), .c(x0), .O(new_n31_));
  nand2  g015(.a(x8), .b(x6), .O(new_n32_));
  aoi21  g016(.a(new_n32_), .b(x9), .c(x5), .O(new_n33_));
  inv1   g017(.a(x6), .O(new_n34_));
  nor2   g018(.a(new_n34_), .b(x4), .O(new_n35_));
  nand2  g019(.a(new_n35_), .b(new_n17_), .O(new_n36_));
  inv1   g020(.a(new_n36_), .O(new_n37_));
  oai21  g021(.a(new_n37_), .b(new_n33_), .c(new_n20_), .O(new_n38_));
  nand2  g022(.a(new_n20_), .b(x6), .O(new_n39_));
  nand2  g023(.a(new_n18_), .b(x9), .O(new_n40_));
  inv1   g024(.a(new_n40_), .O(new_n41_));
  aoi21  g025(.a(new_n41_), .b(new_n39_), .c(x0), .O(new_n42_));
  nand2  g026(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  inv1   g027(.a(x2), .O(new_n44_));
  nand2  g028(.a(new_n17_), .b(x6), .O(new_n45_));
  oai21  g029(.a(new_n45_), .b(x5), .c(new_n44_), .O(new_n46_));
  aoi21  g030(.a(new_n43_), .b(new_n31_), .c(new_n46_), .O(new_n47_));
  inv1   g031(.a(x8), .O(new_n48_));
  nand2  g032(.a(x6), .b(new_n25_), .O(new_n49_));
  nand2  g033(.a(new_n49_), .b(x7), .O(new_n50_));
  nand2  g034(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g035(.a(new_n49_), .b(x8), .c(x7), .O(new_n52_));
  nand4  g036(.a(new_n52_), .b(new_n51_), .c(x9), .d(new_n24_), .O(new_n53_));
  oai21  g037(.a(new_n35_), .b(x7), .c(x5), .O(new_n54_));
  or2    g038(.a(new_n54_), .b(x9), .O(new_n55_));
  nand3  g039(.a(new_n55_), .b(new_n53_), .c(x0), .O(new_n56_));
  nor2   g040(.a(new_n34_), .b(new_n25_), .O(new_n57_));
  nand4  g041(.a(new_n57_), .b(x9), .c(new_n48_), .d(new_n20_), .O(new_n58_));
  nor2   g042(.a(x9), .b(new_n20_), .O(new_n59_));
  nand2  g043(.a(new_n59_), .b(new_n34_), .O(new_n60_));
  nand2  g044(.a(new_n28_), .b(x4), .O(new_n61_));
  nand3  g045(.a(new_n61_), .b(new_n39_), .c(new_n26_), .O(new_n62_));
  nand3  g046(.a(new_n62_), .b(new_n60_), .c(new_n58_), .O(new_n63_));
  nand2  g047(.a(new_n63_), .b(new_n24_), .O(new_n64_));
  nor2   g048(.a(x6), .b(x4), .O(new_n65_));
  aoi21  g049(.a(new_n65_), .b(new_n17_), .c(x0), .O(new_n66_));
  nand2  g050(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand2  g051(.a(new_n59_), .b(new_n25_), .O(new_n68_));
  nand2  g052(.a(new_n68_), .b(x2), .O(new_n69_));
  aoi21  g053(.a(new_n67_), .b(new_n56_), .c(new_n69_), .O(new_n70_));
  inv1   g054(.a(x0), .O(new_n71_));
  nor2   g055(.a(new_n34_), .b(new_n71_), .O(new_n72_));
  nand3  g056(.a(x8), .b(new_n20_), .c(x6), .O(new_n73_));
  nand2  g057(.a(new_n73_), .b(x9), .O(new_n74_));
  nor3   g058(.a(x5), .b(x4), .c(x0), .O(new_n75_));
  aoi22  g059(.a(new_n75_), .b(new_n74_), .c(new_n72_), .d(new_n59_), .O(new_n76_));
  oai21  g060(.a(new_n70_), .b(new_n47_), .c(new_n76_), .O(z0));
  nand2  g061(.a(x4), .b(x2), .O(new_n78_));
  aoi21  g062(.a(new_n78_), .b(new_n71_), .c(x8), .O(new_n79_));
  nand2  g063(.a(x2), .b(x0), .O(new_n80_));
  inv1   g064(.a(new_n80_), .O(new_n81_));
  nor3   g065(.a(new_n81_), .b(new_n79_), .c(x6), .O(new_n82_));
  nand3  g066(.a(x7), .b(new_n25_), .c(new_n71_), .O(new_n83_));
  nor2   g067(.a(x2), .b(x0), .O(new_n84_));
  oai21  g068(.a(x8), .b(x7), .c(new_n84_), .O(new_n85_));
  nand2  g069(.a(new_n20_), .b(new_n44_), .O(new_n86_));
  nand2  g070(.a(new_n86_), .b(x8), .O(new_n87_));
  nand3  g071(.a(new_n87_), .b(new_n85_), .c(x6), .O(new_n88_));
  nand2  g072(.a(new_n88_), .b(new_n83_), .O(new_n89_));
  oai21  g073(.a(new_n89_), .b(new_n82_), .c(x9), .O(new_n90_));
  nand3  g074(.a(new_n20_), .b(x2), .c(new_n71_), .O(new_n91_));
  nand2  g075(.a(new_n91_), .b(x9), .O(new_n92_));
  aoi22  g076(.a(new_n92_), .b(new_n57_), .c(new_n59_), .d(new_n24_), .O(new_n93_));
  aoi21  g077(.a(new_n93_), .b(new_n90_), .c(x3), .O(new_n94_));
  nor2   g078(.a(new_n80_), .b(x6), .O(new_n95_));
  oai21  g079(.a(new_n95_), .b(new_n79_), .c(x7), .O(new_n96_));
  nand3  g080(.a(new_n81_), .b(x8), .c(new_n20_), .O(new_n97_));
  aoi21  g081(.a(new_n97_), .b(new_n96_), .c(new_n17_), .O(new_n98_));
  nand2  g082(.a(new_n28_), .b(new_n20_), .O(new_n99_));
  nand4  g083(.a(new_n99_), .b(new_n78_), .c(new_n50_), .d(new_n71_), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(new_n54_), .O(new_n101_));
  oai21  g085(.a(new_n101_), .b(new_n98_), .c(x3), .O(new_n102_));
  aoi21  g086(.a(x4), .b(x0), .c(new_n17_), .O(new_n103_));
  nor3   g087(.a(new_n103_), .b(new_n20_), .c(new_n34_), .O(new_n104_));
  nand2  g088(.a(new_n20_), .b(new_n34_), .O(new_n105_));
  nor3   g089(.a(new_n105_), .b(new_n79_), .c(new_n17_), .O(new_n106_));
  nor2   g090(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g091(.a(new_n107_), .b(new_n102_), .O(new_n108_));
  oai21  g092(.a(new_n108_), .b(new_n94_), .c(x1), .O(new_n109_));
  inv1   g093(.a(x1), .O(new_n110_));
  nor2   g094(.a(x7), .b(new_n34_), .O(new_n111_));
  nand2  g095(.a(new_n111_), .b(x3), .O(new_n112_));
  nor2   g096(.a(x5), .b(x3), .O(new_n113_));
  nand3  g097(.a(new_n113_), .b(new_n39_), .c(x2), .O(new_n114_));
  aoi21  g098(.a(new_n114_), .b(new_n112_), .c(x8), .O(new_n115_));
  nand2  g099(.a(new_n20_), .b(x0), .O(new_n116_));
  nand3  g100(.a(x8), .b(x3), .c(new_n44_), .O(new_n117_));
  aoi21  g101(.a(new_n116_), .b(x6), .c(new_n117_), .O(new_n118_));
  oai21  g102(.a(new_n118_), .b(new_n115_), .c(x4), .O(new_n119_));
  aoi21  g103(.a(new_n87_), .b(x6), .c(new_n71_), .O(new_n120_));
  oai21  g104(.a(new_n20_), .b(new_n44_), .c(x8), .O(new_n121_));
  or2    g105(.a(new_n121_), .b(x6), .O(new_n122_));
  inv1   g106(.a(x3), .O(new_n123_));
  nand2  g107(.a(new_n24_), .b(new_n123_), .O(new_n124_));
  oai21  g108(.a(new_n124_), .b(x6), .c(x4), .O(new_n125_));
  nand3  g109(.a(new_n125_), .b(new_n122_), .c(new_n120_), .O(new_n126_));
  aoi21  g110(.a(new_n126_), .b(new_n119_), .c(new_n17_), .O(new_n127_));
  inv1   g111(.a(new_n73_), .O(new_n128_));
  nand2  g112(.a(new_n113_), .b(new_n128_), .O(new_n129_));
  nand3  g113(.a(x9), .b(x4), .c(x3), .O(new_n130_));
  inv1   g114(.a(new_n130_), .O(new_n131_));
  nand2  g115(.a(new_n131_), .b(new_n48_), .O(new_n132_));
  aoi21  g116(.a(new_n132_), .b(new_n129_), .c(x2), .O(new_n133_));
  oai21  g117(.a(new_n86_), .b(new_n34_), .c(new_n131_), .O(new_n134_));
  nand2  g118(.a(new_n124_), .b(new_n17_), .O(new_n135_));
  nand3  g119(.a(new_n135_), .b(new_n111_), .c(new_n25_), .O(new_n136_));
  aoi21  g120(.a(new_n136_), .b(new_n134_), .c(new_n48_), .O(new_n137_));
  oai21  g121(.a(new_n137_), .b(new_n133_), .c(new_n71_), .O(new_n138_));
  nand3  g122(.a(new_n34_), .b(new_n25_), .c(x3), .O(new_n139_));
  nand2  g123(.a(new_n139_), .b(x5), .O(new_n140_));
  nand3  g124(.a(x7), .b(new_n34_), .c(x3), .O(new_n141_));
  inv1   g125(.a(new_n141_), .O(new_n142_));
  oai21  g126(.a(new_n142_), .b(new_n25_), .c(new_n140_), .O(new_n143_));
  nor2   g127(.a(x7), .b(x3), .O(new_n144_));
  oai21  g128(.a(new_n35_), .b(new_n24_), .c(new_n144_), .O(new_n145_));
  nand2  g129(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g130(.a(x7), .b(x4), .O(new_n147_));
  nand2  g131(.a(new_n113_), .b(new_n72_), .O(new_n148_));
  aoi21  g132(.a(new_n147_), .b(new_n87_), .c(new_n148_), .O(new_n149_));
  aoi21  g133(.a(new_n146_), .b(new_n17_), .c(new_n149_), .O(new_n150_));
  nand2  g134(.a(new_n150_), .b(new_n138_), .O(new_n151_));
  oai21  g135(.a(new_n151_), .b(new_n127_), .c(new_n110_), .O(new_n152_));
  nand3  g136(.a(x9), .b(x8), .c(x4), .O(new_n153_));
  oai21  g137(.a(new_n153_), .b(new_n105_), .c(new_n68_), .O(new_n154_));
  nor2   g138(.a(new_n17_), .b(new_n24_), .O(new_n155_));
  inv1   g139(.a(new_n155_), .O(new_n156_));
  oai21  g140(.a(new_n124_), .b(new_n45_), .c(new_n156_), .O(new_n157_));
  aoi21  g141(.a(new_n154_), .b(x3), .c(new_n157_), .O(new_n158_));
  nand3  g142(.a(new_n158_), .b(new_n152_), .c(new_n109_), .O(z1));
  nand2  g143(.a(x3), .b(x1), .O(new_n160_));
  inv1   g144(.a(new_n160_), .O(new_n161_));
  nor2   g145(.a(x3), .b(x1), .O(new_n162_));
  nor2   g146(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nor2   g147(.a(new_n163_), .b(new_n155_), .O(z2));
  nor2   g148(.a(new_n160_), .b(new_n155_), .O(z3));
  oai21  g149(.a(new_n25_), .b(x3), .c(x0), .O(new_n166_));
  aoi21  g150(.a(new_n166_), .b(x7), .c(x8), .O(new_n167_));
  nand2  g151(.a(new_n78_), .b(new_n71_), .O(new_n168_));
  nand2  g152(.a(x7), .b(new_n110_), .O(new_n169_));
  nand3  g153(.a(new_n169_), .b(new_n80_), .c(new_n168_), .O(new_n170_));
  nand2  g154(.a(x7), .b(new_n71_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(x3), .O(new_n172_));
  nand3  g156(.a(new_n172_), .b(x4), .c(x1), .O(new_n173_));
  oai21  g157(.a(new_n25_), .b(new_n123_), .c(new_n110_), .O(new_n174_));
  nand3  g158(.a(new_n174_), .b(new_n160_), .c(new_n171_), .O(new_n175_));
  nand3  g159(.a(new_n175_), .b(new_n173_), .c(new_n170_), .O(new_n176_));
  oai21  g160(.a(new_n176_), .b(new_n167_), .c(x6), .O(new_n177_));
  nand2  g161(.a(new_n174_), .b(new_n121_), .O(new_n178_));
  nand2  g162(.a(new_n161_), .b(x7), .O(new_n179_));
  aoi21  g163(.a(new_n179_), .b(new_n178_), .c(new_n71_), .O(new_n180_));
  nor2   g164(.a(new_n78_), .b(x8), .O(new_n181_));
  oai21  g165(.a(new_n181_), .b(new_n142_), .c(x1), .O(new_n182_));
  aoi21  g166(.a(new_n181_), .b(x3), .c(x5), .O(new_n183_));
  nand2  g167(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  nor2   g168(.a(new_n184_), .b(new_n180_), .O(new_n185_));
  aoi21  g169(.a(new_n185_), .b(new_n177_), .c(new_n17_), .O(z4));
  nor2   g170(.a(new_n84_), .b(new_n81_), .O(new_n187_));
  oai21  g171(.a(new_n187_), .b(new_n163_), .c(new_n156_), .O(z5));
endmodule


