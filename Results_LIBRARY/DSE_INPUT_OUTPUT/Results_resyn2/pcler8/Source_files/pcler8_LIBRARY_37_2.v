// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x07), .O(new_n45_));
  nor2   g01(.a(x16), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x25), .O(new_n51_));
  inv1   g07(.a(x26), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g09(.a(x08), .O(new_n54_));
  inv1   g10(.a(x10), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand4  g13(.a(new_n57_), .b(new_n53_), .c(new_n50_), .d(x24), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n47_), .O(z00));
  inv1   g15(.a(x00), .O(new_n60_));
  oai21  g16(.a(new_n54_), .b(new_n60_), .c(new_n47_), .O(z01));
  inv1   g17(.a(x01), .O(new_n62_));
  nand2  g18(.a(new_n47_), .b(x08), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n62_), .O(z02));
  nand2  g20(.a(x08), .b(x02), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n47_), .O(z03));
  inv1   g22(.a(x03), .O(new_n67_));
  nor2   g23(.a(new_n63_), .b(new_n67_), .O(z04));
  inv1   g24(.a(x04), .O(new_n69_));
  nor2   g25(.a(new_n63_), .b(new_n69_), .O(z05));
  aoi21  g26(.a(x08), .b(x05), .c(new_n46_), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z06));
  nand3  g28(.a(new_n47_), .b(x08), .c(x06), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z07));
  nand3  g30(.a(x16), .b(x08), .c(x07), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(z08));
  nor2   g32(.a(new_n56_), .b(new_n46_), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  and2   g34(.a(x22), .b(x21), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n78_), .c(x19), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n77_), .O(new_n82_));
  oai21  g38(.a(new_n63_), .b(new_n60_), .c(new_n82_), .O(z09));
  nand2  g39(.a(x08), .b(x01), .O(new_n84_));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  inv1   g41(.a(new_n78_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n79_), .c(x12), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nor2   g44(.a(x20), .b(x19), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n56_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n84_), .c(new_n46_), .O(z10));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(x22), .b(x13), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n78_), .c(new_n94_), .O(new_n96_));
  or2    g52(.a(new_n85_), .b(x21), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n96_), .c(new_n57_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n65_), .c(new_n46_), .O(z11));
  aoi21  g55(.a(new_n86_), .b(x14), .c(new_n49_), .O(new_n100_));
  oai21  g56(.a(new_n94_), .b(x22), .c(new_n77_), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(new_n100_), .c(new_n63_), .d(new_n67_), .O(z12));
  nand3  g58(.a(new_n53_), .b(x24), .c(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n50_), .O(new_n104_));
  aoi21  g60(.a(new_n49_), .b(new_n48_), .c(new_n56_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g62(.a(x08), .b(x04), .c(new_n46_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(z13));
  nand3  g64(.a(new_n53_), .b(x24), .c(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n49_), .b(new_n48_), .c(new_n111_), .O(new_n112_));
  nand4  g68(.a(new_n79_), .b(new_n85_), .c(x24), .d(x23), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n56_), .c(new_n71_), .O(z14));
  inv1   g71(.a(new_n77_), .O(new_n116_));
  nand2  g72(.a(new_n53_), .b(x17), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n50_), .b(x25), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n113_), .b(new_n51_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n116_), .c(new_n73_), .O(z15));
  nand2  g78(.a(new_n119_), .b(new_n52_), .O(new_n123_));
  nor2   g79(.a(new_n49_), .b(x18), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n86_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n77_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n75_), .O(z16));
endmodule


