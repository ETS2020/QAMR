// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x00), .O(new_n45_));
  nand2  g01(.a(x17), .b(new_n45_), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nand2  g03(.a(x24), .b(x23), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g05(.a(x26), .b(x25), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n46_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z03));
  inv1   g18(.a(x17), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x00), .O(new_n64_));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n64_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand3  g24(.a(new_n46_), .b(x08), .c(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand3  g26(.a(new_n46_), .b(x08), .c(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n46_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  nand2  g31(.a(x22), .b(x21), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(new_n80_));
  oai21  g36(.a(x17), .b(x08), .c(new_n45_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n52_), .c(x09), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n80_), .c(new_n57_), .O(z09));
  inv1   g39(.a(z02), .O(new_n84_));
  nand2  g40(.a(x20), .b(x19), .O(new_n85_));
  aoi21  g41(.a(new_n78_), .b(x12), .c(new_n85_), .O(new_n86_));
  inv1   g42(.a(new_n53_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(z10));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  nand2  g47(.a(x22), .b(x13), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n77_), .c(new_n91_), .O(new_n93_));
  inv1   g49(.a(x21), .O(new_n94_));
  aoi21  g50(.a(new_n85_), .b(new_n94_), .c(new_n53_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n61_), .c(new_n64_), .O(z11));
  inv1   g53(.a(new_n77_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(x14), .c(new_n47_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  nand2  g56(.a(new_n90_), .b(new_n100_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n87_), .c(new_n46_), .O(new_n102_));
  oai22  g58(.a(new_n102_), .b(new_n99_), .c(new_n65_), .d(new_n64_), .O(z12));
  inv1   g59(.a(x23), .O(new_n104_));
  nor2   g60(.a(new_n47_), .b(new_n104_), .O(new_n105_));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g63(.a(new_n47_), .b(new_n104_), .c(new_n53_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n67_), .c(new_n46_), .O(z13));
  nor2   g66(.a(new_n53_), .b(new_n64_), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n105_), .c(x24), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n47_), .b(new_n104_), .c(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n113_), .c(new_n111_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n69_), .O(z14));
  inv1   g73(.a(x25), .O(new_n118_));
  oai21  g74(.a(new_n48_), .b(new_n47_), .c(new_n118_), .O(new_n119_));
  nand2  g75(.a(new_n49_), .b(x25), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n111_), .O(new_n121_));
  nand3  g77(.a(new_n54_), .b(x17), .c(x00), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n71_), .O(z15));
  inv1   g79(.a(z08), .O(new_n124_));
  aoi21  g80(.a(new_n49_), .b(x25), .c(x26), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  inv1   g82(.a(new_n47_), .O(new_n127_));
  nand3  g83(.a(new_n98_), .b(new_n127_), .c(new_n126_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n87_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n125_), .c(new_n124_), .O(z16));
endmodule


