// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  nand2  g00(.a(x05), .b(x03), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n50_), .c(new_n48_), .d(x24), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z00));
  nand3  g12(.a(new_n45_), .b(x08), .c(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z02));
  inv1   g16(.a(new_n45_), .O(new_n61_));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  inv1   g20(.a(x05), .O(new_n65_));
  aoi21  g21(.a(new_n51_), .b(new_n65_), .c(new_n64_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z05));
  nand3  g24(.a(x08), .b(x05), .c(new_n64_), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n45_), .O(z08));
  nand2  g30(.a(new_n54_), .b(new_n45_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nand2  g33(.a(x24), .b(x23), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n50_), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  and2   g37(.a(x20), .b(x11), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n81_), .c(new_n76_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n75_), .c(new_n57_), .O(z09));
  inv1   g40(.a(x20), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n76_), .O(new_n86_));
  inv1   g42(.a(new_n77_), .O(new_n87_));
  nand4  g43(.a(new_n79_), .b(new_n50_), .c(new_n87_), .d(x12), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  aoi21  g45(.a(new_n85_), .b(new_n76_), .c(new_n53_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n59_), .c(new_n61_), .O(z10));
  inv1   g48(.a(new_n80_), .O(new_n93_));
  nand2  g49(.a(new_n86_), .b(x21), .O(new_n94_));
  and2   g50(.a(x22), .b(x13), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(new_n94_), .O(new_n96_));
  oai21  g52(.a(new_n86_), .b(x21), .c(new_n54_), .O(new_n97_));
  and2   g53(.a(new_n62_), .b(new_n45_), .O(new_n98_));
  oai21  g54(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(z11));
  nand3  g55(.a(new_n86_), .b(x22), .c(x21), .O(new_n100_));
  aoi21  g56(.a(new_n93_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n54_), .c(new_n45_), .O(new_n104_));
  nand3  g60(.a(x08), .b(new_n65_), .c(x03), .O(new_n105_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n105_), .O(z12));
  nand3  g62(.a(new_n50_), .b(x24), .c(x15), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  aoi21  g64(.a(new_n47_), .b(new_n46_), .c(new_n53_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n67_), .c(new_n45_), .O(z13));
  nand3  g67(.a(new_n50_), .b(x24), .c(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n47_), .b(new_n46_), .c(new_n114_), .O(new_n115_));
  inv1   g71(.a(new_n47_), .O(new_n116_));
  nand2  g72(.a(new_n79_), .b(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n75_), .c(new_n69_), .O(z14));
  nand2  g75(.a(new_n50_), .b(x17), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(new_n121_));
  nor2   g77(.a(new_n78_), .b(new_n47_), .O(new_n122_));
  xor2a  g78(.a(new_n122_), .b(x25), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n121_), .c(new_n54_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n71_), .c(new_n61_), .O(z15));
  inv1   g81(.a(x25), .O(new_n126_));
  nor2   g82(.a(new_n117_), .b(new_n126_), .O(new_n127_));
  nor2   g83(.a(new_n49_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n122_), .c(new_n53_), .O(new_n129_));
  oai21  g85(.a(new_n127_), .b(x26), .c(new_n129_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n73_), .c(new_n61_), .O(z16));
endmodule


