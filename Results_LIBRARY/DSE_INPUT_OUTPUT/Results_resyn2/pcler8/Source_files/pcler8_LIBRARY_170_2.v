// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x09), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nor2   g06(.a(x10), .b(x08), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x17), .c(new_n45_), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  inv1   g10(.a(x17), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x09), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nor2   g15(.a(new_n57_), .b(new_n59_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  nor2   g17(.a(new_n57_), .b(new_n61_), .O(z03));
  inv1   g18(.a(new_n57_), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  nor2   g22(.a(new_n57_), .b(new_n66_), .O(z05));
  nand2  g23(.a(new_n63_), .b(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(new_n63_), .b(x06), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  inv1   g28(.a(x08), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(new_n56_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  nand3  g36(.a(new_n51_), .b(x17), .c(x09), .O(new_n81_));
  oai22  g37(.a(new_n81_), .b(new_n80_), .c(new_n57_), .d(new_n54_), .O(z09));
  nand2  g38(.a(x08), .b(x01), .O(new_n83_));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n76_), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(x10), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  nor2   g45(.a(x20), .b(x19), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n87_), .c(new_n55_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n45_), .c(new_n83_), .O(z10));
  nand2  g49(.a(x08), .b(x02), .O(new_n94_));
  nand3  g50(.a(x21), .b(x20), .c(x19), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nand3  g52(.a(x23), .b(x22), .c(x13), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n49_), .c(new_n96_), .O(new_n98_));
  inv1   g54(.a(x21), .O(new_n99_));
  aoi21  g55(.a(new_n84_), .b(new_n99_), .c(new_n89_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n98_), .c(new_n55_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n45_), .c(new_n94_), .O(z11));
  aoi21  g58(.a(new_n77_), .b(x14), .c(new_n47_), .O(new_n103_));
  inv1   g59(.a(new_n81_), .O(new_n104_));
  oai21  g60(.a(new_n96_), .b(x22), .c(new_n104_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n103_), .c(new_n64_), .O(z12));
  nand2  g62(.a(new_n50_), .b(x15), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  aoi21  g64(.a(new_n47_), .b(new_n46_), .c(new_n81_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  oai21  g66(.a(new_n57_), .b(new_n66_), .c(new_n110_), .O(z13));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n47_), .b(new_n46_), .c(new_n114_), .O(new_n115_));
  inv1   g71(.a(new_n47_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(x24), .c(x23), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n81_), .c(new_n68_), .O(z14));
  inv1   g75(.a(x26), .O(new_n120_));
  nand2  g76(.a(x24), .b(x23), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(x25), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  nand2  g80(.a(new_n117_), .b(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n104_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n70_), .O(z15));
  aoi21  g83(.a(new_n122_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n77_), .b(new_n116_), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n104_), .O(new_n131_));
  oai22  g87(.a(new_n131_), .b(new_n128_), .c(new_n57_), .d(new_n72_), .O(z16));
endmodule


