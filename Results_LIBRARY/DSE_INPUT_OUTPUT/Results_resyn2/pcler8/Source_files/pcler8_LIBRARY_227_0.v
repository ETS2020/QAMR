// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  nor2   g00(.a(x17), .b(x05), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g05(.a(x26), .b(x25), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand4  g11(.a(new_n55_), .b(new_n51_), .c(new_n49_), .d(x24), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  aoi21  g15(.a(x08), .b(x01), .c(new_n45_), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand3  g17(.a(new_n46_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand3  g19(.a(new_n46_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n46_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n46_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  and2   g29(.a(x22), .b(x21), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand4  g32(.a(new_n76_), .b(new_n74_), .c(x20), .d(x11), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n55_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n58_), .c(new_n45_), .O(z09));
  and2   g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  and2   g38(.a(new_n76_), .b(new_n74_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(x12), .c(new_n82_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n55_), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n84_), .c(new_n60_), .O(z10));
  nand2  g42(.a(new_n81_), .b(x21), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n76_), .c(new_n87_), .O(new_n89_));
  nor2   g45(.a(new_n54_), .b(new_n45_), .O(new_n90_));
  oai21  g46(.a(new_n81_), .b(x21), .c(new_n90_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n89_), .c(new_n62_), .O(z11));
  aoi21  g48(.a(new_n76_), .b(x14), .c(new_n48_), .O(new_n93_));
  inv1   g49(.a(new_n87_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x22), .c(new_n90_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n64_), .O(z12));
  nand3  g52(.a(new_n51_), .b(x24), .c(x15), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n49_), .O(new_n98_));
  aoi21  g54(.a(new_n48_), .b(new_n47_), .c(new_n54_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n66_), .c(new_n46_), .O(z13));
  nand2  g57(.a(new_n51_), .b(x16), .O(new_n102_));
  nand2  g58(.a(x24), .b(x23), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  or2    g61(.a(new_n49_), .b(x24), .O(new_n106_));
  inv1   g62(.a(x17), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(x08), .O(new_n108_));
  nor2   g64(.a(new_n108_), .b(x05), .O(new_n109_));
  nand2  g65(.a(new_n53_), .b(x09), .O(new_n110_));
  nor2   g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n106_), .c(new_n105_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n68_), .O(z14));
  and2   g69(.a(x24), .b(x23), .O(new_n114_));
  nand4  g70(.a(new_n114_), .b(new_n74_), .c(new_n81_), .d(x25), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  oai21  g72(.a(new_n103_), .b(new_n48_), .c(new_n116_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n55_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n70_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n46_), .O(new_n120_));
  nand4  g76(.a(new_n108_), .b(new_n51_), .c(new_n53_), .d(x09), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand2  g79(.a(new_n115_), .b(new_n123_), .O(new_n124_));
  nor2   g80(.a(new_n50_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n104_), .c(new_n54_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n72_), .c(new_n45_), .O(z16));
endmodule


