// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x11), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  nand2  g06(.a(x09), .b(new_n50_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(new_n49_), .c(new_n48_), .d(x24), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x10), .O(new_n56_));
  nand2  g12(.a(x11), .b(new_n56_), .O(new_n57_));
  oai21  g13(.a(new_n50_), .b(new_n55_), .c(new_n57_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nand2  g15(.a(new_n57_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n60_), .b(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  nor2   g22(.a(new_n60_), .b(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n50_), .b(new_n70_), .c(new_n57_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  nor2   g28(.a(new_n60_), .b(new_n72_), .O(z08));
  nand3  g29(.a(new_n52_), .b(new_n45_), .c(new_n56_), .O(new_n74_));
  oai22  g30(.a(new_n74_), .b(x19), .c(new_n60_), .d(new_n55_), .O(z09));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand4  g33(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n78_));
  nand2  g34(.a(new_n49_), .b(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n51_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n80_), .c(x11), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(x10), .c(new_n50_), .d(new_n59_), .O(z10));
  nand2  g40(.a(new_n49_), .b(x24), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  nand3  g43(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n85_), .c(new_n87_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  aoi21  g46(.a(new_n76_), .b(new_n90_), .c(new_n51_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(x11), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x10), .c(new_n62_), .O(z11));
  and2   g49(.a(x24), .b(x23), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n49_), .c(x14), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n87_), .c(x22), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  aoi21  g53(.a(new_n86_), .b(new_n97_), .c(new_n51_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n96_), .c(x11), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(x10), .c(new_n50_), .d(new_n64_), .O(z12));
  inv1   g56(.a(x15), .O(new_n101_));
  oai21  g57(.a(new_n85_), .b(new_n101_), .c(new_n48_), .O(new_n102_));
  aoi21  g58(.a(new_n47_), .b(new_n46_), .c(new_n51_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n102_), .c(x11), .O(new_n104_));
  oai22  g60(.a(new_n104_), .b(x10), .c(new_n50_), .d(new_n66_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n47_), .b(new_n46_), .c(new_n108_), .O(new_n109_));
  and2   g65(.a(x22), .b(x21), .O(new_n110_));
  nand3  g66(.a(new_n94_), .b(new_n110_), .c(new_n77_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n51_), .c(new_n45_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n56_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n68_), .O(z14));
  nor2   g71(.a(new_n78_), .b(new_n76_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(x25), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n111_), .b(new_n118_), .O(new_n119_));
  aoi22  g75(.a(new_n119_), .b(new_n117_), .c(new_n49_), .d(x17), .O(new_n120_));
  oai22  g76(.a(new_n120_), .b(new_n74_), .c(new_n60_), .d(new_n70_), .O(z15));
  nand2  g77(.a(x08), .b(x07), .O(new_n122_));
  aoi21  g78(.a(new_n116_), .b(x25), .c(x26), .O(new_n123_));
  nor2   g79(.a(new_n97_), .b(x18), .O(new_n124_));
  nand4  g80(.a(new_n124_), .b(new_n87_), .c(new_n94_), .d(new_n49_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n52_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n123_), .c(new_n45_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n56_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n122_), .O(z16));
endmodule


