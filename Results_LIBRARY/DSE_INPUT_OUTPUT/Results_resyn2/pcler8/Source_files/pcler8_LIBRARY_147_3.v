// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  nand3  g00(.a(x21), .b(x20), .c(x19), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x23), .c(x22), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand4  g05(.a(x26), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n50_));
  nand2  g06(.a(x25), .b(x24), .O(new_n51_));
  nor3   g07(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(z00));
  inv1   g08(.a(x21), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(new_n54_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  oai21  g17(.a(new_n48_), .b(new_n61_), .c(new_n58_), .O(z03));
  nand2  g18(.a(new_n55_), .b(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(new_n55_), .b(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n58_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n58_), .O(z07));
  nand2  g26(.a(new_n55_), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  inv1   g28(.a(x09), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(x08), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai22  g32(.a(new_n76_), .b(new_n75_), .c(x19), .d(x10), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  aoi21  g34(.a(x08), .b(x00), .c(new_n54_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n78_), .O(z09));
  nand2  g36(.a(new_n74_), .b(new_n49_), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  and2   g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n75_), .c(new_n83_), .O(new_n85_));
  or2    g41(.a(x20), .b(x19), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n82_), .O(new_n87_));
  nand2  g43(.a(new_n55_), .b(x01), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n87_), .O(z10));
  nand3  g45(.a(x26), .b(x25), .c(x24), .O(new_n90_));
  nand3  g46(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  nor2   g47(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nor2   g48(.a(new_n92_), .b(new_n45_), .O(new_n93_));
  nand2  g49(.a(x20), .b(x19), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n53_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n74_), .c(new_n49_), .O(new_n96_));
  nand2  g52(.a(new_n55_), .b(x02), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n93_), .c(new_n97_), .O(z11));
  inv1   g54(.a(new_n75_), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  aoi21  g56(.a(new_n99_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n45_), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n74_), .c(new_n49_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n101_), .c(new_n63_), .O(z12));
  inv1   g61(.a(x23), .O(new_n106_));
  nand2  g62(.a(new_n100_), .b(new_n106_), .O(new_n107_));
  aoi22  g63(.a(new_n107_), .b(new_n47_), .c(new_n99_), .d(x15), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n81_), .c(new_n65_), .O(z13));
  inv1   g65(.a(x24), .O(new_n110_));
  nand2  g66(.a(new_n47_), .b(new_n110_), .O(new_n111_));
  nand3  g67(.a(x24), .b(x23), .c(x22), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand3  g69(.a(x26), .b(x25), .c(x16), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n46_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n111_), .c(new_n82_), .O(new_n116_));
  nand2  g72(.a(new_n55_), .b(x05), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(z14));
  nand3  g74(.a(new_n83_), .b(x09), .c(new_n48_), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  nand2  g76(.a(new_n113_), .b(new_n120_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n119_), .c(new_n49_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(x21), .O(new_n123_));
  nand2  g79(.a(x26), .b(x17), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n113_), .c(new_n46_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n82_), .c(x25), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n123_), .c(new_n69_), .O(z15));
  inv1   g83(.a(x26), .O(new_n128_));
  nand4  g84(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(new_n74_), .c(new_n83_), .d(new_n128_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n49_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(x21), .O(new_n133_));
  inv1   g89(.a(new_n50_), .O(new_n134_));
  inv1   g90(.a(x18), .O(new_n135_));
  nand3  g91(.a(new_n130_), .b(new_n46_), .c(new_n135_), .O(new_n136_));
  aoi22  g92(.a(new_n136_), .b(new_n134_), .c(x08), .d(x07), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n133_), .O(z16));
endmodule


