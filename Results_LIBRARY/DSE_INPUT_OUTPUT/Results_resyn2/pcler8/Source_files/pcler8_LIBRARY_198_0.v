// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  nor2   g00(.a(x25), .b(x17), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand2  g02(.a(x20), .b(x19), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x21), .O(new_n52_));
  nand3  g08(.a(x24), .b(x23), .c(x22), .O(new_n53_));
  nand2  g09(.a(x26), .b(x25), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  oai21  g12(.a(new_n56_), .b(new_n47_), .c(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n45_), .O(z02));
  nand3  g17(.a(new_n46_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand3  g19(.a(new_n46_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n45_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n45_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  oai21  g29(.a(new_n50_), .b(x19), .c(new_n58_), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  nand2  g31(.a(x20), .b(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n56_), .c(new_n75_), .O(z09));
  aoi21  g33(.a(new_n55_), .b(x12), .c(new_n47_), .O(new_n78_));
  oai21  g34(.a(x20), .b(x19), .c(new_n51_), .O(new_n79_));
  aoi21  g35(.a(x08), .b(x01), .c(new_n45_), .O(new_n80_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n80_), .O(z10));
  nand3  g37(.a(new_n55_), .b(new_n51_), .c(x13), .O(new_n82_));
  nor2   g38(.a(new_n50_), .b(new_n45_), .O(new_n83_));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  nand2  g40(.a(new_n47_), .b(new_n52_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n82_), .c(new_n62_), .O(z11));
  inv1   g43(.a(new_n84_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(x22), .O(new_n89_));
  inv1   g45(.a(x22), .O(new_n90_));
  nand2  g46(.a(new_n84_), .b(new_n90_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n89_), .c(new_n83_), .O(new_n92_));
  inv1   g48(.a(new_n53_), .O(new_n93_));
  inv1   g49(.a(new_n54_), .O(new_n94_));
  nand4  g50(.a(new_n94_), .b(new_n93_), .c(new_n51_), .d(x14), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n92_), .c(new_n64_), .O(z12));
  inv1   g52(.a(x23), .O(new_n97_));
  nand2  g53(.a(new_n89_), .b(new_n97_), .O(new_n98_));
  inv1   g54(.a(new_n89_), .O(new_n99_));
  nand3  g55(.a(new_n94_), .b(x24), .c(x15), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n99_), .c(x23), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n98_), .c(new_n51_), .O(new_n102_));
  aoi21  g58(.a(x08), .b(x04), .c(new_n45_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n102_), .O(z13));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n89_), .b(new_n97_), .c(new_n105_), .O(new_n106_));
  nor2   g62(.a(new_n105_), .b(new_n97_), .O(new_n107_));
  nand2  g63(.a(new_n94_), .b(x16), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n99_), .c(new_n107_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n106_), .c(new_n51_), .O(new_n110_));
  aoi21  g66(.a(x08), .b(x05), .c(new_n45_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n110_), .O(z14));
  inv1   g68(.a(x25), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x24), .c(x23), .d(x22), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n84_), .c(new_n54_), .O(new_n115_));
  oai21  g71(.a(new_n84_), .b(new_n53_), .c(x25), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n115_), .b(x17), .c(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n46_), .b(x08), .c(x06), .O(new_n119_));
  oai21  g75(.a(new_n118_), .b(new_n50_), .c(new_n119_), .O(z15));
  inv1   g76(.a(x18), .O(new_n121_));
  nand3  g77(.a(new_n88_), .b(new_n93_), .c(new_n121_), .O(new_n122_));
  nand4  g78(.a(x26), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n123_));
  inv1   g79(.a(new_n123_), .O(new_n124_));
  aoi22  g80(.a(new_n124_), .b(new_n122_), .c(x08), .d(x07), .O(new_n125_));
  inv1   g81(.a(x26), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n113_), .O(new_n127_));
  nand4  g83(.a(new_n127_), .b(new_n116_), .c(new_n83_), .d(new_n54_), .O(new_n128_));
  oai21  g84(.a(new_n125_), .b(new_n45_), .c(new_n128_), .O(z16));
endmodule


