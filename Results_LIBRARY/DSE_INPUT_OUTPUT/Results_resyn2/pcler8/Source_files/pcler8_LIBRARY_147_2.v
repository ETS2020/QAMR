// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n51_), .c(new_n48_), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  nor2   g10(.a(x21), .b(new_n50_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  oai21  g15(.a(new_n49_), .b(new_n59_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  oai21  g17(.a(new_n49_), .b(new_n61_), .c(new_n56_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n57_), .b(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nor2   g21(.a(new_n57_), .b(new_n65_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  oai21  g23(.a(new_n49_), .b(new_n67_), .c(new_n56_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n56_), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  nor2   g27(.a(new_n57_), .b(new_n71_), .O(z08));
  inv1   g28(.a(new_n51_), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  aoi21  g33(.a(x08), .b(x00), .c(new_n55_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n77_), .O(z09));
  inv1   g35(.a(x19), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n74_), .c(new_n82_), .O(new_n84_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n51_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n57_), .b(new_n59_), .c(new_n86_), .O(z10));
  inv1   g43(.a(new_n52_), .O(new_n88_));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n46_), .O(new_n91_));
  oai21  g47(.a(new_n82_), .b(x21), .c(new_n73_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n91_), .c(new_n57_), .d(new_n61_), .O(z11));
  inv1   g49(.a(new_n74_), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(x14), .c(new_n95_), .O(new_n96_));
  inv1   g52(.a(new_n46_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x22), .c(new_n73_), .O(new_n98_));
  oai22  g54(.a(new_n98_), .b(new_n96_), .c(new_n57_), .d(new_n63_), .O(z12));
  nand2  g55(.a(new_n88_), .b(x15), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nor2   g57(.a(new_n95_), .b(new_n101_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g59(.a(new_n95_), .b(new_n101_), .c(new_n51_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  oai21  g61(.a(new_n57_), .b(new_n65_), .c(new_n105_), .O(z13));
  nor2   g62(.a(new_n47_), .b(x24), .O(new_n107_));
  nand3  g63(.a(x26), .b(x25), .c(x16), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n47_), .c(x24), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n73_), .O(new_n110_));
  oai22  g66(.a(new_n110_), .b(new_n107_), .c(new_n57_), .d(new_n67_), .O(z14));
  nand4  g67(.a(x23), .b(x22), .c(x20), .d(x19), .O(new_n112_));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(x24), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n112_), .c(x25), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand3  g72(.a(new_n47_), .b(new_n116_), .c(x24), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n73_), .O(new_n119_));
  inv1   g75(.a(x21), .O(new_n120_));
  nand2  g76(.a(x09), .b(new_n49_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n116_), .c(new_n50_), .O(new_n122_));
  aoi22  g78(.a(new_n122_), .b(new_n120_), .c(x08), .d(x06), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n119_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  inv1   g81(.a(new_n112_), .O(new_n126_));
  nand2  g82(.a(x25), .b(x24), .O(new_n127_));
  nor2   g83(.a(new_n127_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n126_), .c(new_n125_), .O(new_n129_));
  nand4  g85(.a(new_n125_), .b(x25), .c(x24), .d(x23), .O(new_n130_));
  nor2   g86(.a(new_n130_), .b(new_n95_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n129_), .c(new_n73_), .O(new_n132_));
  oai21  g88(.a(new_n121_), .b(new_n125_), .c(new_n50_), .O(new_n133_));
  aoi22  g89(.a(new_n133_), .b(new_n120_), .c(x08), .d(x07), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n132_), .O(z16));
endmodule


