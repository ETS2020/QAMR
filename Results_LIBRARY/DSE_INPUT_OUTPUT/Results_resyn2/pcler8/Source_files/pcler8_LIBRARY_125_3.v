// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand2  g02(.a(x17), .b(x15), .O(new_n47_));
  nand4  g03(.a(new_n47_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n48_));
  nand4  g04(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n49_));
  nand3  g05(.a(x22), .b(x21), .c(x20), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n48_), .O(z00));
  nand3  g09(.a(new_n47_), .b(x08), .c(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z02));
  inv1   g13(.a(new_n47_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x02), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z04));
  nand3  g18(.a(new_n47_), .b(x08), .c(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n58_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n58_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n47_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  aoi21  g27(.a(new_n51_), .b(x11), .c(new_n71_), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n48_), .c(new_n54_), .O(z09));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  nand3  g30(.a(x22), .b(x21), .c(x12), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n49_), .c(new_n74_), .O(new_n76_));
  nand3  g32(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n77_));
  nor2   g33(.a(x20), .b(x19), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n56_), .c(new_n58_), .O(z10));
  inv1   g37(.a(new_n49_), .O(new_n82_));
  nand3  g38(.a(x21), .b(x20), .c(x19), .O(new_n83_));
  and2   g39(.a(x22), .b(x13), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n82_), .c(new_n83_), .O(new_n85_));
  inv1   g41(.a(new_n77_), .O(new_n86_));
  oai21  g42(.a(new_n74_), .b(x21), .c(new_n86_), .O(new_n87_));
  and2   g43(.a(new_n59_), .b(new_n47_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n85_), .c(new_n88_), .O(z11));
  inv1   g45(.a(z04), .O(new_n90_));
  inv1   g46(.a(new_n83_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x22), .O(new_n92_));
  aoi21  g48(.a(new_n82_), .b(x14), .c(new_n92_), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(x22), .c(new_n86_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n90_), .O(z12));
  and2   g51(.a(x22), .b(x21), .O(new_n96_));
  nand3  g52(.a(new_n74_), .b(new_n96_), .c(x23), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  nand4  g54(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n98_), .c(new_n48_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  inv1   g57(.a(x15), .O(new_n102_));
  nor2   g58(.a(x17), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n82_), .c(new_n86_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n101_), .c(new_n63_), .O(z13));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n99_), .b(new_n98_), .c(new_n106_), .O(new_n107_));
  and2   g63(.a(x24), .b(x23), .O(new_n108_));
  nand3  g64(.a(new_n74_), .b(new_n96_), .c(new_n108_), .O(new_n109_));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  aoi21  g67(.a(new_n109_), .b(new_n107_), .c(new_n111_), .O(new_n112_));
  and2   g68(.a(new_n65_), .b(new_n47_), .O(new_n113_));
  oai21  g69(.a(new_n112_), .b(new_n77_), .c(new_n113_), .O(z14));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(new_n109_), .b(new_n115_), .O(new_n116_));
  nand4  g72(.a(new_n74_), .b(new_n96_), .c(new_n108_), .d(x25), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n86_), .O(new_n118_));
  nand2  g74(.a(x26), .b(x25), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n77_), .c(new_n102_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n118_), .c(new_n67_), .O(z15));
  inv1   g78(.a(z08), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand3  g80(.a(new_n51_), .b(x19), .c(new_n124_), .O(new_n125_));
  inv1   g81(.a(x26), .O(new_n126_));
  nand2  g82(.a(new_n117_), .b(new_n126_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n125_), .c(new_n86_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n123_), .O(z16));
endmodule


