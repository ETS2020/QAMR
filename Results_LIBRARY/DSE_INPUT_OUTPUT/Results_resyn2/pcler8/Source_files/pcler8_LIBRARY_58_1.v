// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_;
  inv1   g00(.a(x10), .O(new_n45_));
  nand2  g01(.a(x26), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(z00));
  inv1   g03(.a(x00), .O(new_n48_));
  nand2  g04(.a(new_n46_), .b(x08), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(z01));
  nand2  g06(.a(x08), .b(x01), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n46_), .O(z02));
  inv1   g08(.a(x02), .O(new_n53_));
  nor2   g09(.a(new_n49_), .b(new_n53_), .O(z03));
  inv1   g10(.a(x03), .O(new_n55_));
  nor2   g11(.a(new_n49_), .b(new_n55_), .O(z04));
  nand2  g12(.a(x08), .b(x04), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z05));
  inv1   g14(.a(x05), .O(new_n59_));
  inv1   g15(.a(x08), .O(new_n60_));
  oai21  g16(.a(new_n60_), .b(new_n59_), .c(new_n46_), .O(z06));
  inv1   g17(.a(x06), .O(new_n62_));
  nor2   g18(.a(new_n49_), .b(new_n62_), .O(z07));
  inv1   g19(.a(x07), .O(new_n64_));
  nor2   g20(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g21(.a(x26), .O(new_n66_));
  inv1   g22(.a(x09), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x08), .O(new_n68_));
  nand3  g24(.a(new_n68_), .b(new_n66_), .c(new_n45_), .O(new_n69_));
  oai22  g25(.a(new_n69_), .b(x19), .c(new_n49_), .d(new_n48_), .O(z09));
  nor2   g26(.a(x20), .b(x19), .O(new_n71_));
  nand2  g27(.a(x20), .b(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n45_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n51_), .O(z10));
  aoi21  g32(.a(x20), .b(x19), .c(x21), .O(new_n77_));
  nand3  g33(.a(x21), .b(x20), .c(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n68_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n45_), .O(new_n81_));
  oai21  g37(.a(new_n60_), .b(new_n53_), .c(new_n81_), .O(z11));
  nand4  g38(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  nand2  g40(.a(new_n78_), .b(new_n84_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n83_), .c(new_n68_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n66_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n45_), .O(new_n88_));
  oai21  g44(.a(new_n60_), .b(new_n55_), .c(new_n88_), .O(z12));
  inv1   g45(.a(x23), .O(new_n90_));
  nand2  g46(.a(new_n83_), .b(new_n90_), .O(new_n91_));
  nand2  g47(.a(x09), .b(new_n60_), .O(new_n92_));
  nor2   g48(.a(new_n83_), .b(new_n90_), .O(new_n93_));
  nor2   g49(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n91_), .c(x26), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x10), .c(new_n57_), .O(z13));
  inv1   g52(.a(new_n69_), .O(new_n97_));
  nand2  g53(.a(new_n93_), .b(x24), .O(new_n98_));
  inv1   g54(.a(x24), .O(new_n99_));
  oai21  g55(.a(new_n83_), .b(new_n90_), .c(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(new_n101_));
  oai21  g57(.a(new_n49_), .b(new_n59_), .c(new_n101_), .O(z14));
  nand2  g58(.a(x08), .b(x06), .O(new_n103_));
  inv1   g59(.a(x25), .O(new_n104_));
  nand2  g60(.a(new_n83_), .b(new_n104_), .O(new_n105_));
  inv1   g61(.a(new_n83_), .O(new_n106_));
  nand2  g62(.a(x24), .b(x23), .O(new_n107_));
  xor2a  g63(.a(new_n107_), .b(x25), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n92_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n105_), .c(x26), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(x10), .c(new_n103_), .O(z15));
  inv1   g67(.a(new_n107_), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n106_), .c(new_n68_), .d(x25), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n66_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n45_), .O(new_n115_));
  oai21  g71(.a(new_n60_), .b(new_n64_), .c(new_n115_), .O(z16));
endmodule


