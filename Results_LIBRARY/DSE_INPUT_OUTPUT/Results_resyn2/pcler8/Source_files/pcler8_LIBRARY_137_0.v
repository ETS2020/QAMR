// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  and2   g03(.a(x24), .b(x23), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(new_n47_), .c(x25), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(x10), .b(new_n50_), .O(new_n51_));
  inv1   g07(.a(x05), .O(new_n52_));
  nor2   g08(.a(x08), .b(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  inv1   g12(.a(x10), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n56_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n59_), .b(new_n61_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  inv1   g19(.a(x08), .O(new_n64_));
  oai21  g20(.a(new_n64_), .b(new_n63_), .c(new_n58_), .O(z03));
  inv1   g21(.a(x03), .O(new_n66_));
  oai21  g22(.a(new_n64_), .b(new_n66_), .c(new_n58_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n58_), .O(z05));
  nor2   g25(.a(new_n64_), .b(new_n52_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n58_), .O(z07));
  inv1   g28(.a(x07), .O(new_n73_));
  nor2   g29(.a(new_n59_), .b(new_n73_), .O(z08));
  inv1   g30(.a(new_n54_), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  oai21  g36(.a(new_n59_), .b(new_n56_), .c(new_n80_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  inv1   g39(.a(new_n76_), .O(new_n84_));
  nand2  g40(.a(new_n77_), .b(x12), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n75_), .O(new_n88_));
  oai22  g44(.a(new_n88_), .b(new_n87_), .c(new_n59_), .d(new_n61_), .O(z10));
  nand2  g45(.a(new_n82_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n84_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n82_), .b(x21), .c(new_n75_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n92_), .c(new_n59_), .d(new_n63_), .O(z11));
  aoi21  g50(.a(new_n84_), .b(x14), .c(new_n46_), .O(new_n95_));
  inv1   g51(.a(new_n90_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x22), .c(new_n75_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n95_), .c(new_n59_), .d(new_n66_), .O(z12));
  nand3  g54(.a(x26), .b(x25), .c(x24), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(x15), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n47_), .c(x23), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  nand2  g59(.a(x09), .b(new_n64_), .O(new_n104_));
  aoi21  g60(.a(new_n46_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n102_), .c(new_n52_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(x10), .c(new_n68_), .O(z13));
  nand2  g63(.a(new_n100_), .b(x16), .O(new_n108_));
  nand3  g64(.a(new_n48_), .b(new_n77_), .c(new_n82_), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n46_), .b(new_n103_), .c(new_n110_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n51_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n64_), .c(new_n52_), .O(z14));
  nand2  g71(.a(x26), .b(x17), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n48_), .c(new_n47_), .d(x25), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  aoi21  g74(.a(new_n109_), .b(new_n118_), .c(new_n104_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n117_), .c(new_n52_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(x10), .c(new_n71_), .O(z15));
  nand2  g77(.a(x08), .b(x07), .O(new_n122_));
  nand2  g78(.a(new_n49_), .b(new_n45_), .O(new_n123_));
  nor2   g79(.a(new_n46_), .b(x18), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n84_), .c(new_n104_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n123_), .c(new_n52_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(x10), .c(new_n122_), .O(z16));
endmodule


