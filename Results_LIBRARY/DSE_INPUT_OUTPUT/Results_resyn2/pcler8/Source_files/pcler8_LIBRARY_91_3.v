// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x24), .O(new_n51_));
  inv1   g07(.a(x25), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n50_), .c(x26), .d(new_n47_), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(new_n45_), .c(new_n46_), .O(z00));
  nand3  g11(.a(new_n46_), .b(x08), .c(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  aoi21  g14(.a(new_n46_), .b(new_n58_), .c(new_n45_), .O(z02));
  nand3  g15(.a(new_n46_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  aoi21  g18(.a(new_n46_), .b(new_n62_), .c(new_n45_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(x09), .O(z05));
  oai21  g21(.a(x09), .b(x05), .c(x08), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand3  g23(.a(new_n46_), .b(x08), .c(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  oai21  g25(.a(x09), .b(x07), .c(x08), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nand3  g27(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n56_), .O(z09));
  nand2  g34(.a(new_n46_), .b(x08), .O(new_n79_));
  inv1   g35(.a(x19), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n74_), .c(new_n82_), .O(new_n84_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n72_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n79_), .b(new_n58_), .c(new_n86_), .O(z10));
  inv1   g43(.a(new_n74_), .O(new_n88_));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n82_), .b(x21), .c(new_n73_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n60_), .O(z11));
  aoi21  g49(.a(new_n88_), .b(x14), .c(new_n49_), .O(new_n94_));
  inv1   g50(.a(new_n89_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n73_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n94_), .c(new_n79_), .d(new_n62_), .O(z12));
  oai21  g53(.a(x09), .b(x04), .c(x08), .O(new_n98_));
  nand2  g54(.a(new_n47_), .b(x09), .O(new_n99_));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(x23), .c(new_n49_), .O(new_n101_));
  nand2  g57(.a(new_n89_), .b(new_n45_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(x23), .c(new_n101_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n99_), .c(new_n98_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(new_n49_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(x24), .c(x23), .O(new_n109_));
  oai21  g65(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n99_), .c(new_n66_), .O(z14));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n50_), .c(x25), .d(x24), .O(new_n114_));
  nand2  g70(.a(new_n109_), .b(new_n52_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n73_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n68_), .O(z15));
  inv1   g73(.a(x26), .O(new_n118_));
  nand4  g74(.a(new_n108_), .b(x25), .c(x24), .d(x23), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nor2   g76(.a(new_n49_), .b(x18), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n88_), .c(new_n99_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n70_), .O(z16));
endmodule


