// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x10), .O(new_n45_));
  nor2   g01(.a(x21), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand4  g03(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  nand3  g06(.a(new_n45_), .b(x09), .c(new_n50_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n47_), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  nand2  g13(.a(new_n47_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n47_), .O(z02));
  inv1   g17(.a(x02), .O(new_n62_));
  nor2   g18(.a(new_n58_), .b(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z05));
  inv1   g23(.a(x05), .O(new_n68_));
  nor2   g24(.a(new_n58_), .b(new_n68_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  nor2   g26(.a(new_n58_), .b(new_n70_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  nor2   g28(.a(new_n58_), .b(new_n72_), .O(z08));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n48_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n52_), .O(new_n76_));
  aoi21  g32(.a(x08), .b(x00), .c(new_n46_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n76_), .O(z09));
  inv1   g34(.a(x19), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n48_), .c(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n51_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n60_), .c(new_n47_), .O(z10));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  and2   g43(.a(x23), .b(x22), .O(new_n88_));
  nand4  g44(.a(x26), .b(x25), .c(x24), .d(x13), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n87_), .O(new_n91_));
  oai21  g47(.a(new_n81_), .b(x21), .c(new_n52_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n91_), .c(new_n58_), .d(new_n62_), .O(z11));
  aoi21  g49(.a(new_n49_), .b(x14), .c(new_n53_), .O(new_n94_));
  inv1   g50(.a(new_n87_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n52_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n94_), .c(new_n58_), .d(new_n64_), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n48_), .b(new_n98_), .O(new_n99_));
  inv1   g55(.a(x23), .O(new_n100_));
  nand2  g56(.a(new_n53_), .b(new_n100_), .O(new_n101_));
  nand4  g57(.a(x23), .b(x22), .c(x20), .d(x19), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(x21), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n101_), .c(new_n99_), .O(new_n105_));
  nand3  g61(.a(new_n47_), .b(x08), .c(x04), .O(new_n106_));
  oai21  g62(.a(new_n105_), .b(new_n51_), .c(new_n106_), .O(z13));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n104_), .b(new_n108_), .O(new_n109_));
  nand3  g65(.a(x26), .b(x25), .c(x16), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n103_), .c(x24), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n109_), .c(new_n52_), .O(new_n112_));
  nand2  g68(.a(x08), .b(x05), .O(new_n113_));
  inv1   g69(.a(x21), .O(new_n114_));
  nand2  g70(.a(x09), .b(new_n50_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n108_), .c(new_n45_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n113_), .c(new_n112_), .O(z14));
  nand2  g74(.a(x26), .b(x17), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(x24), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n102_), .c(x25), .O(new_n121_));
  nor2   g77(.a(x25), .b(new_n108_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n103_), .c(x21), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n52_), .O(new_n125_));
  inv1   g81(.a(x25), .O(new_n126_));
  oai21  g82(.a(new_n115_), .b(new_n126_), .c(new_n45_), .O(new_n127_));
  aoi22  g83(.a(new_n127_), .b(new_n114_), .c(x08), .d(x06), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n125_), .O(z15));
  inv1   g85(.a(x18), .O(new_n130_));
  nand3  g86(.a(x25), .b(x24), .c(new_n130_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n102_), .c(x26), .O(new_n132_));
  nor2   g88(.a(new_n108_), .b(new_n100_), .O(new_n133_));
  nor2   g89(.a(x26), .b(new_n126_), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n54_), .c(new_n133_), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n52_), .O(new_n137_));
  inv1   g93(.a(x26), .O(new_n138_));
  oai21  g94(.a(new_n115_), .b(new_n138_), .c(new_n45_), .O(new_n139_));
  aoi22  g95(.a(new_n139_), .b(new_n114_), .c(x08), .d(x07), .O(new_n140_));
  nand2  g96(.a(new_n140_), .b(new_n137_), .O(z16));
endmodule


