// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(x24), .b(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x25), .O(new_n49_));
  inv1   g05(.a(x07), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  nand2  g12(.a(new_n51_), .b(x07), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n56_), .O(z01));
  inv1   g15(.a(new_n58_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n57_), .O(z03));
  inv1   g20(.a(x03), .O(new_n65_));
  nor2   g21(.a(new_n58_), .b(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n57_), .O(z05));
  nand2  g24(.a(new_n60_), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(new_n60_), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand3  g28(.a(x10), .b(x08), .c(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n53_), .c(x07), .O(new_n78_));
  oai22  g34(.a(new_n78_), .b(x10), .c(new_n58_), .d(new_n56_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n75_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  inv1   g40(.a(new_n54_), .O(new_n85_));
  or2    g41(.a(x20), .b(x19), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n84_), .c(new_n61_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  nand3  g45(.a(x26), .b(x25), .c(x24), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n89_), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  nand2  g51(.a(new_n80_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n85_), .O(new_n97_));
  nand2  g53(.a(new_n60_), .b(x02), .O(new_n98_));
  oai21  g54(.a(new_n97_), .b(new_n94_), .c(new_n98_), .O(z11));
  nand2  g55(.a(x08), .b(x03), .O(new_n100_));
  inv1   g56(.a(x14), .O(new_n101_));
  inv1   g57(.a(new_n47_), .O(new_n102_));
  oai21  g58(.a(new_n75_), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  inv1   g59(.a(x22), .O(new_n104_));
  inv1   g60(.a(x08), .O(new_n105_));
  nand2  g61(.a(x09), .b(new_n105_), .O(new_n106_));
  aoi21  g62(.a(new_n89_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n103_), .c(x07), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(x10), .c(new_n100_), .O(z12));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n102_), .c(x23), .O(new_n111_));
  inv1   g67(.a(x23), .O(new_n112_));
  aoi21  g68(.a(new_n47_), .b(new_n112_), .c(new_n106_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(x07), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(x10), .c(new_n67_), .O(z13));
  nand3  g71(.a(new_n102_), .b(x24), .c(x23), .O(new_n116_));
  inv1   g72(.a(x24), .O(new_n117_));
  oai21  g73(.a(new_n47_), .b(new_n112_), .c(new_n117_), .O(new_n118_));
  aoi22  g74(.a(new_n118_), .b(new_n116_), .c(new_n91_), .d(x16), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n54_), .c(new_n69_), .O(z14));
  nand3  g76(.a(x26), .b(x25), .c(x17), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  oai21  g79(.a(new_n47_), .b(new_n46_), .c(new_n123_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n49_), .c(new_n122_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n54_), .c(new_n71_), .O(z15));
  aoi21  g82(.a(new_n48_), .b(x25), .c(x26), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand3  g84(.a(new_n81_), .b(new_n102_), .c(new_n128_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n85_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n127_), .c(new_n73_), .O(z16));
endmodule


