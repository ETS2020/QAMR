// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x11), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(x24), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g09(.a(x10), .O(new_n54_));
  nand2  g10(.a(x11), .b(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  inv1   g16(.a(x08), .O(new_n61_));
  aoi21  g17(.a(x11), .b(new_n54_), .c(new_n61_), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n60_), .O(z03));
  nand2  g20(.a(new_n62_), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  and2   g22(.a(new_n62_), .b(x04), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n55_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  nor2   g26(.a(new_n63_), .b(new_n70_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n55_), .O(z08));
  inv1   g29(.a(x19), .O(new_n74_));
  aoi21  g30(.a(new_n51_), .b(new_n74_), .c(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(x10), .c(new_n56_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand4  g33(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n78_));
  nand2  g34(.a(new_n49_), .b(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  inv1   g36(.a(new_n51_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n80_), .c(x11), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(x10), .c(new_n58_), .O(z10));
  nand2  g41(.a(new_n77_), .b(x21), .O(new_n86_));
  nand3  g42(.a(new_n49_), .b(x24), .c(x23), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n88_), .c(new_n86_), .O(new_n90_));
  nand3  g46(.a(new_n51_), .b(new_n45_), .c(new_n54_), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n77_), .b(x21), .c(new_n92_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n90_), .c(new_n63_), .d(new_n60_), .O(z11));
  inv1   g50(.a(new_n86_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(x22), .O(new_n96_));
  aoi21  g52(.a(new_n88_), .b(x14), .c(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n95_), .b(x22), .c(new_n92_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n97_), .c(new_n65_), .O(z12));
  nand2  g55(.a(x08), .b(x04), .O(new_n100_));
  nand3  g56(.a(new_n49_), .b(x24), .c(x15), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  aoi21  g58(.a(new_n47_), .b(new_n46_), .c(new_n81_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n102_), .c(x11), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(x10), .c(new_n100_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n47_), .b(new_n46_), .c(new_n108_), .O(new_n109_));
  and2   g65(.a(x22), .b(x21), .O(new_n110_));
  and2   g66(.a(x24), .b(x23), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n110_), .c(new_n77_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n109_), .c(new_n107_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n81_), .c(new_n45_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n54_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n68_), .O(z14));
  nand2  g72(.a(x20), .b(x19), .O(new_n117_));
  nor2   g73(.a(new_n78_), .b(new_n117_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(x25), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  nand2  g76(.a(new_n112_), .b(new_n120_), .O(new_n121_));
  aoi22  g77(.a(new_n121_), .b(new_n119_), .c(new_n49_), .d(x17), .O(new_n122_));
  oai22  g78(.a(new_n122_), .b(new_n91_), .c(new_n63_), .d(new_n70_), .O(z15));
  aoi21  g79(.a(new_n118_), .b(x25), .c(x26), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nand4  g81(.a(new_n111_), .b(new_n49_), .c(x22), .d(new_n125_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n86_), .c(new_n92_), .O(new_n127_));
  nand2  g83(.a(new_n62_), .b(x07), .O(new_n128_));
  oai21  g84(.a(new_n127_), .b(new_n124_), .c(new_n128_), .O(z16));
endmodule


