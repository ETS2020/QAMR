// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(x16), .O(new_n52_));
  nor2   g08(.a(x17), .b(new_n52_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g10(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n48_), .O(z00));
  inv1   g14(.a(new_n53_), .O(new_n59_));
  nand3  g15(.a(new_n59_), .b(x08), .c(x00), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z01));
  nand2  g17(.a(x08), .b(x01), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n53_), .O(z02));
  nand2  g19(.a(x08), .b(x02), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n59_), .O(z03));
  nand3  g21(.a(new_n59_), .b(x08), .c(x03), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z04));
  nand3  g23(.a(new_n59_), .b(x08), .c(x04), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z05));
  nand2  g25(.a(x08), .b(x05), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n53_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n59_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n59_), .O(z08));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n54_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n60_), .O(z09));
  inv1   g37(.a(x19), .O(new_n82_));
  inv1   g38(.a(x20), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g40(.a(new_n76_), .O(new_n85_));
  nand3  g41(.a(new_n77_), .b(new_n85_), .c(x12), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  aoi21  g43(.a(new_n83_), .b(new_n82_), .c(new_n51_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(new_n62_), .c(new_n59_), .O(z10));
  inv1   g46(.a(z03), .O(new_n91_));
  and2   g47(.a(x23), .b(x22), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(x13), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n56_), .c(new_n46_), .O(new_n95_));
  inv1   g51(.a(new_n51_), .O(new_n96_));
  oai21  g52(.a(new_n84_), .b(x21), .c(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n95_), .c(new_n91_), .O(z11));
  nand4  g54(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  aoi21  g55(.a(new_n85_), .b(x14), .c(new_n99_), .O(new_n100_));
  inv1   g56(.a(new_n46_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(x22), .c(new_n54_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n66_), .O(z12));
  nand2  g59(.a(new_n56_), .b(x15), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nor2   g61(.a(new_n99_), .b(new_n105_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g63(.a(new_n99_), .b(new_n105_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n107_), .c(new_n54_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n68_), .O(z13));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n46_), .b(new_n45_), .c(new_n111_), .O(new_n112_));
  nand2  g68(.a(new_n47_), .b(x24), .O(new_n113_));
  nand4  g69(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n113_), .b(new_n112_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(x08), .b(x05), .c(new_n53_), .O(new_n117_));
  oai21  g73(.a(new_n116_), .b(new_n51_), .c(new_n117_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n47_), .c(x24), .O(new_n121_));
  nand2  g77(.a(new_n113_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n96_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n72_), .c(new_n59_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  nand4  g81(.a(new_n101_), .b(new_n92_), .c(x25), .d(x24), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nor2   g83(.a(new_n99_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n85_), .c(new_n51_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n74_), .c(new_n53_), .O(z16));
endmodule


