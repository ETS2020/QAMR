// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x03), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n52_), .c(new_n50_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n47_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z02));
  nand3  g18(.a(new_n47_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  inv1   g20(.a(x17), .O(new_n65_));
  oai21  g21(.a(new_n65_), .b(x08), .c(x03), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z04));
  nand3  g23(.a(new_n47_), .b(x08), .c(x04), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z05));
  nand2  g25(.a(x08), .b(x05), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n46_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n46_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n47_), .O(z08));
  inv1   g31(.a(z01), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n77_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n55_), .c(new_n76_), .O(z09));
  inv1   g39(.a(z02), .O(new_n84_));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n80_), .b(x12), .c(new_n86_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n56_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n84_), .O(z10));
  inv1   g45(.a(new_n79_), .O(new_n90_));
  nand2  g46(.a(new_n85_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  nor2   g49(.a(new_n55_), .b(new_n46_), .O(new_n94_));
  oai21  g50(.a(new_n85_), .b(x21), .c(new_n94_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n63_), .O(z11));
  aoi21  g52(.a(new_n90_), .b(x14), .c(new_n49_), .O(new_n97_));
  inv1   g53(.a(new_n91_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n56_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n66_), .O(z12));
  inv1   g56(.a(new_n94_), .O(new_n101_));
  inv1   g57(.a(new_n49_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x23), .O(new_n103_));
  nand2  g59(.a(new_n49_), .b(new_n48_), .O(new_n104_));
  aoi22  g60(.a(new_n104_), .b(new_n103_), .c(new_n90_), .d(x15), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n101_), .c(new_n68_), .O(z13));
  inv1   g62(.a(x24), .O(new_n107_));
  nand2  g63(.a(new_n103_), .b(new_n107_), .O(new_n108_));
  inv1   g64(.a(x25), .O(new_n109_));
  inv1   g65(.a(x26), .O(new_n110_));
  nor2   g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n50_), .c(x24), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n108_), .c(new_n56_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n70_), .c(new_n47_), .O(z14));
  nor2   g71(.a(new_n107_), .b(new_n48_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n102_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n109_), .O(new_n118_));
  nand3  g74(.a(new_n116_), .b(new_n102_), .c(x25), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n94_), .O(new_n120_));
  nand2  g76(.a(new_n111_), .b(x17), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n56_), .c(z07), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n120_), .O(z15));
  nand2  g80(.a(new_n119_), .b(new_n110_), .O(new_n125_));
  nor2   g81(.a(new_n51_), .b(x18), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n50_), .c(new_n55_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n74_), .c(new_n46_), .O(z16));
endmodule


