// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x00), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor3   g07(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(z00));
  xor2a  g08(.a(x08), .b(x00), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z01));
  nand2  g10(.a(new_n45_), .b(x00), .O(new_n55_));
  oai21  g11(.a(new_n45_), .b(x01), .c(new_n55_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  oai21  g13(.a(new_n45_), .b(x02), .c(new_n55_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  inv1   g15(.a(x03), .O(new_n60_));
  nor2   g16(.a(new_n45_), .b(new_n60_), .O(z04));
  nand2  g17(.a(x08), .b(x04), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  oai21  g19(.a(new_n45_), .b(x05), .c(new_n55_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  nand2  g21(.a(x08), .b(x06), .O(new_n66_));
  oai21  g22(.a(x08), .b(x00), .c(new_n66_), .O(z07));
  oai21  g23(.a(new_n45_), .b(x07), .c(new_n55_), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z08));
  and2   g25(.a(x22), .b(x21), .O(new_n70_));
  nand3  g26(.a(new_n70_), .b(x20), .c(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n50_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n48_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n53_), .O(z09));
  inv1   g30(.a(x19), .O(new_n75_));
  inv1   g31(.a(x20), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g33(.a(new_n50_), .O(new_n78_));
  nand3  g34(.a(new_n70_), .b(new_n78_), .c(x12), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  aoi21  g36(.a(new_n76_), .b(new_n75_), .c(new_n47_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n56_), .O(z10));
  nand3  g39(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x23), .b(x22), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(x13), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n85_), .c(new_n86_), .O(new_n90_));
  oai21  g46(.a(new_n77_), .b(x21), .c(new_n48_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n90_), .c(new_n58_), .O(z11));
  aoi21  g48(.a(new_n78_), .b(x14), .c(new_n51_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n48_), .c(x00), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n93_), .c(new_n45_), .d(new_n60_), .O(z12));
  nand2  g53(.a(new_n85_), .b(x15), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  nor2   g55(.a(new_n51_), .b(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g57(.a(new_n51_), .b(new_n99_), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n101_), .c(new_n48_), .d(x00), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n62_), .O(z13));
  nand2  g60(.a(new_n85_), .b(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  inv1   g62(.a(new_n86_), .O(new_n107_));
  nand3  g63(.a(new_n87_), .b(new_n107_), .c(x24), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  nand2  g65(.a(new_n87_), .b(new_n107_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n108_), .c(new_n106_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n47_), .c(new_n64_), .O(z14));
  nand3  g69(.a(x26), .b(x25), .c(x17), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(new_n115_));
  nand4  g71(.a(new_n87_), .b(new_n107_), .c(x25), .d(x24), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n108_), .b(new_n117_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n49_), .c(new_n66_), .O(z15));
  inv1   g76(.a(new_n116_), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nor2   g78(.a(new_n51_), .b(new_n50_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n122_), .c(new_n47_), .O(new_n124_));
  oai21  g80(.a(new_n121_), .b(x26), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n68_), .O(z16));
endmodule


