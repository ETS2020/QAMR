// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  nand2  g00(.a(x17), .b(x04), .O(new_n45_));
  inv1   g01(.a(x25), .O(new_n46_));
  inv1   g02(.a(x26), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n47_), .c(new_n46_), .O(new_n51_));
  inv1   g07(.a(x23), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n51_), .c(x24), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z00));
  nand3  g12(.a(new_n45_), .b(x08), .c(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  and2   g17(.a(new_n61_), .b(new_n45_), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n45_), .O(z04));
  inv1   g21(.a(x17), .O(new_n66_));
  nand3  g22(.a(new_n66_), .b(x08), .c(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n45_), .O(z06));
  inv1   g26(.a(new_n45_), .O(new_n71_));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n71_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n45_), .O(z08));
  nand4  g31(.a(new_n45_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nand4  g34(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n57_), .O(z09));
  and2   g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n78_), .c(new_n83_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n59_), .c(new_n71_), .O(z10));
  inv1   g45(.a(new_n78_), .O(new_n90_));
  nand2  g46(.a(new_n83_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  inv1   g49(.a(new_n50_), .O(new_n94_));
  oai21  g50(.a(new_n83_), .b(x21), .c(new_n94_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n62_), .O(z11));
  nand3  g52(.a(new_n45_), .b(x08), .c(x03), .O(new_n97_));
  aoi21  g53(.a(new_n90_), .b(x14), .c(new_n53_), .O(new_n98_));
  inv1   g54(.a(new_n91_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(x22), .c(new_n77_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n97_), .O(z12));
  inv1   g57(.a(x15), .O(new_n102_));
  nor2   g58(.a(new_n78_), .b(new_n102_), .O(new_n103_));
  xor2a  g59(.a(new_n53_), .b(new_n52_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n103_), .c(new_n77_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n67_), .O(z13));
  nand3  g62(.a(x26), .b(x25), .c(x16), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n54_), .c(x24), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n53_), .b(new_n52_), .c(new_n109_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n108_), .c(new_n94_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n69_), .c(new_n71_), .O(z14));
  nand3  g68(.a(new_n54_), .b(x25), .c(x24), .O(new_n113_));
  nand2  g69(.a(new_n54_), .b(x24), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n46_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n113_), .c(new_n77_), .O(new_n116_));
  nor2   g72(.a(new_n66_), .b(x04), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n51_), .c(z07), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n116_), .O(z15));
  inv1   g75(.a(new_n53_), .O(new_n120_));
  nor2   g76(.a(new_n46_), .b(new_n109_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n120_), .c(x23), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n47_), .O(new_n123_));
  nand3  g79(.a(x26), .b(x25), .c(x24), .O(new_n124_));
  nor2   g80(.a(new_n124_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n54_), .c(new_n50_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n74_), .c(new_n71_), .O(z16));
endmodule


