// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n67_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nand3  g01(.a(x22), .b(x21), .c(x20), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nor2   g07(.a(x06), .b(x05), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n47_), .d(x19), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z00));
  aoi21  g11(.a(x08), .b(x00), .c(new_n52_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n52_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n53_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n52_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n53_), .O(z05));
  and2   g21(.a(x08), .b(x05), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n53_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  aoi21  g27(.a(new_n47_), .b(x11), .c(new_n71_), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n50_), .c(new_n56_), .O(z09));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  nand3  g30(.a(x22), .b(x21), .c(x12), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n45_), .c(new_n74_), .O(new_n76_));
  nor2   g32(.a(x20), .b(x19), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n50_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n58_), .c(new_n53_), .O(z10));
  nand3  g36(.a(x21), .b(x20), .c(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand3  g38(.a(x26), .b(x25), .c(x24), .O(new_n83_));
  nand3  g39(.a(x23), .b(x22), .c(x13), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n83_), .c(new_n82_), .O(new_n85_));
  inv1   g41(.a(x21), .O(new_n86_));
  nand2  g42(.a(x20), .b(x19), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n86_), .c(new_n50_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n60_), .c(new_n52_), .O(z11));
  inv1   g46(.a(new_n45_), .O(new_n91_));
  nand4  g47(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n92_));
  aoi21  g48(.a(new_n91_), .b(x14), .c(new_n92_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n81_), .b(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n53_), .c(new_n51_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n93_), .c(new_n62_), .d(new_n52_), .O(z12));
  inv1   g53(.a(new_n92_), .O(new_n98_));
  inv1   g54(.a(new_n83_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(x15), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n98_), .c(x23), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  aoi21  g58(.a(new_n92_), .b(new_n102_), .c(new_n50_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n64_), .c(new_n53_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  and2   g63(.a(x24), .b(x23), .O(new_n108_));
  and2   g64(.a(x22), .b(x21), .O(new_n109_));
  nand3  g65(.a(new_n74_), .b(new_n109_), .c(new_n108_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n92_), .b(new_n102_), .c(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n107_), .O(new_n113_));
  aoi21  g69(.a(new_n48_), .b(x05), .c(x06), .O(new_n114_));
  nor2   g70(.a(new_n114_), .b(z06), .O(new_n115_));
  oai21  g71(.a(new_n113_), .b(new_n50_), .c(new_n115_), .O(z14));
  inv1   g72(.a(x25), .O(new_n117_));
  aoi21  g73(.a(x26), .b(x17), .c(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n98_), .c(new_n108_), .O(new_n119_));
  nand2  g75(.a(new_n110_), .b(new_n117_), .O(new_n120_));
  nand2  g76(.a(new_n49_), .b(x09), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(new_n114_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n119_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n67_), .O(z15));
  inv1   g80(.a(new_n46_), .O(new_n125_));
  nor2   g81(.a(new_n71_), .b(x18), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n125_), .c(new_n91_), .O(new_n127_));
  inv1   g83(.a(x26), .O(new_n128_));
  nand3  g84(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  oai21  g85(.a(new_n92_), .b(new_n129_), .c(new_n128_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n127_), .c(new_n51_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n69_), .c(new_n52_), .O(z16));
endmodule


