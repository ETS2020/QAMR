// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand2  g06(.a(x15), .b(x10), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  nand2  g11(.a(new_n53_), .b(x01), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  nand2  g13(.a(new_n53_), .b(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  nand2  g15(.a(new_n53_), .b(x03), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  inv1   g17(.a(x04), .O(new_n62_));
  oai21  g18(.a(new_n46_), .b(new_n62_), .c(new_n51_), .O(z05));
  nand2  g19(.a(x08), .b(x05), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n51_), .O(z06));
  nand2  g21(.a(x08), .b(x06), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n51_), .O(z07));
  nand2  g23(.a(x08), .b(x07), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n51_), .O(z08));
  inv1   g25(.a(new_n48_), .O(new_n70_));
  nand4  g26(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n45_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  aoi21  g29(.a(x08), .b(x00), .c(new_n52_), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n73_), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  inv1   g32(.a(new_n45_), .O(new_n77_));
  and2   g33(.a(x22), .b(x21), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n77_), .c(x12), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n48_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n56_), .O(z10));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n77_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n76_), .b(x21), .c(new_n70_), .O(new_n88_));
  aoi21  g44(.a(x08), .b(x02), .c(new_n52_), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z11));
  aoi21  g46(.a(new_n77_), .b(x14), .c(new_n49_), .O(new_n91_));
  inv1   g47(.a(new_n85_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n70_), .O(new_n93_));
  aoi21  g49(.a(x08), .b(x03), .c(new_n52_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n91_), .c(new_n94_), .O(z12));
  nand3  g51(.a(x26), .b(x25), .c(x24), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  inv1   g53(.a(x09), .O(new_n98_));
  nor2   g54(.a(new_n98_), .b(x08), .O(new_n99_));
  inv1   g55(.a(new_n49_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n99_), .c(new_n97_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x15), .O(new_n103_));
  nor2   g59(.a(new_n46_), .b(new_n62_), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nor2   g61(.a(new_n49_), .b(new_n105_), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n49_), .b(new_n105_), .c(new_n48_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(new_n104_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n103_), .O(z13));
  inv1   g66(.a(z06), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand4  g69(.a(new_n78_), .b(new_n76_), .c(x24), .d(x23), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n49_), .b(new_n105_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n48_), .c(new_n111_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  nor2   g75(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n106_), .c(new_n120_), .O(new_n122_));
  nand2  g78(.a(new_n114_), .b(new_n119_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n70_), .O(new_n124_));
  nand2  g80(.a(new_n53_), .b(x06), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(z15));
  inv1   g82(.a(z08), .O(new_n127_));
  aoi21  g83(.a(new_n106_), .b(new_n120_), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n100_), .b(new_n77_), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n70_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n128_), .c(new_n127_), .O(z16));
endmodule


