// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  and2   g02(.a(x25), .b(x24), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x26), .c(x23), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand4  g09(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(new_n56_));
  oai21  g12(.a(new_n46_), .b(new_n45_), .c(new_n56_), .O(z00));
  inv1   g13(.a(x00), .O(new_n58_));
  aoi21  g14(.a(new_n46_), .b(new_n58_), .c(new_n45_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  aoi21  g16(.a(new_n46_), .b(new_n60_), .c(new_n45_), .O(z02));
  oai21  g17(.a(x10), .b(x02), .c(x08), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  aoi21  g20(.a(new_n46_), .b(new_n64_), .c(new_n45_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  aoi21  g22(.a(new_n46_), .b(new_n66_), .c(new_n45_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(x10), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(x10), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  aoi21  g28(.a(new_n46_), .b(new_n72_), .c(new_n45_), .O(z08));
  and2   g29(.a(x22), .b(x21), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n48_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n51_), .O(new_n77_));
  nand2  g33(.a(x08), .b(x00), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n77_), .c(x10), .O(z09));
  nand2  g35(.a(x08), .b(x01), .O(new_n80_));
  inv1   g36(.a(x19), .O(new_n81_));
  inv1   g37(.a(x20), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g39(.a(new_n74_), .b(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n48_), .c(new_n83_), .O(new_n85_));
  nand2  g41(.a(new_n82_), .b(new_n81_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n51_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n80_), .c(x10), .O(z10));
  nand2  g44(.a(new_n83_), .b(x21), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n49_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n83_), .b(x21), .c(new_n53_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n62_), .O(z11));
  nand2  g49(.a(x08), .b(x03), .O(new_n94_));
  inv1   g50(.a(x14), .O(new_n95_));
  oai21  g51(.a(new_n48_), .b(new_n95_), .c(new_n55_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n89_), .b(new_n97_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n96_), .c(new_n51_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n94_), .c(x10), .O(z12));
  nand2  g56(.a(x08), .b(x04), .O(new_n101_));
  inv1   g57(.a(x15), .O(new_n102_));
  nor2   g58(.a(new_n48_), .b(new_n102_), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  xor2a  g60(.a(new_n54_), .b(new_n104_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n103_), .c(new_n51_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n101_), .c(x10), .O(z13));
  nand3  g63(.a(new_n47_), .b(x26), .c(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nor2   g65(.a(new_n54_), .b(new_n104_), .O(new_n110_));
  xor2a  g66(.a(new_n110_), .b(x24), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n109_), .c(new_n51_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n68_), .c(x10), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n110_), .b(x24), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g72(.a(x26), .b(x17), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n110_), .c(new_n47_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(new_n51_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n70_), .c(x10), .O(z15));
  nand2  g76(.a(x08), .b(x07), .O(new_n121_));
  inv1   g77(.a(x26), .O(new_n122_));
  nand2  g78(.a(new_n110_), .b(new_n47_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nor2   g80(.a(new_n54_), .b(x18), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n49_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n51_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n121_), .c(x10), .O(z16));
endmodule


