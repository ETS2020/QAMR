// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_,
    new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(z00));
  inv1   g06(.a(x00), .O(new_n51_));
  aoi21  g07(.a(x10), .b(new_n51_), .c(new_n46_), .O(z01));
  nand3  g08(.a(x10), .b(x08), .c(x01), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z02));
  nand3  g10(.a(x10), .b(x08), .c(x02), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z03));
  inv1   g12(.a(x03), .O(new_n57_));
  aoi21  g13(.a(x10), .b(new_n57_), .c(new_n46_), .O(z04));
  nand3  g14(.a(x10), .b(x08), .c(x04), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z05));
  inv1   g16(.a(x05), .O(new_n61_));
  aoi21  g17(.a(x10), .b(new_n61_), .c(new_n46_), .O(z06));
  oai21  g18(.a(new_n47_), .b(x06), .c(x08), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z07));
  oai21  g20(.a(new_n47_), .b(x07), .c(x08), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z08));
  inv1   g22(.a(new_n48_), .O(new_n67_));
  nand4  g23(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n68_));
  oai21  g24(.a(new_n68_), .b(new_n45_), .c(x19), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g26(.a(x10), .b(x08), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n51_), .c(new_n70_), .O(z09));
  oai21  g28(.a(new_n47_), .b(x01), .c(x08), .O(new_n73_));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(x22), .b(x21), .c(x12), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n45_), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  inv1   g34(.a(x09), .O(new_n79_));
  nor2   g35(.a(x10), .b(new_n79_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n80_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n78_), .c(new_n73_), .O(z10));
  inv1   g38(.a(new_n45_), .O(new_n83_));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n74_), .b(x21), .c(new_n67_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n55_), .O(z11));
  aoi21  g44(.a(new_n83_), .b(x14), .c(new_n49_), .O(new_n89_));
  inv1   g45(.a(new_n84_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(x22), .c(new_n67_), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n89_), .c(new_n71_), .d(new_n57_), .O(z12));
  oai21  g48(.a(new_n47_), .b(x04), .c(x08), .O(new_n93_));
  nand4  g49(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(x23), .c(new_n49_), .O(new_n95_));
  inv1   g51(.a(x23), .O(new_n96_));
  nand2  g52(.a(new_n84_), .b(new_n46_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(x22), .c(new_n96_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n95_), .c(new_n80_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n93_), .O(z13));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  and2   g58(.a(x22), .b(x21), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(new_n74_), .c(x24), .d(x23), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n49_), .b(new_n96_), .c(new_n105_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n104_), .c(new_n102_), .O(new_n107_));
  nand3  g63(.a(x10), .b(x08), .c(x05), .O(new_n108_));
  oai21  g64(.a(new_n107_), .b(new_n48_), .c(new_n108_), .O(z14));
  nor2   g65(.a(new_n49_), .b(new_n96_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(x24), .c(x25), .O(new_n111_));
  and2   g67(.a(x25), .b(x24), .O(new_n112_));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n110_), .c(new_n112_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n80_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n111_), .c(new_n63_), .O(z15));
  aoi21  g72(.a(new_n110_), .b(new_n112_), .c(x26), .O(new_n117_));
  nor2   g73(.a(new_n49_), .b(x18), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n83_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n80_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n117_), .c(new_n65_), .O(z16));
endmodule


