// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x05), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n52_), .c(new_n50_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n47_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z02));
  nand3  g18(.a(new_n47_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n47_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n46_), .O(z05));
  and2   g24(.a(x08), .b(x05), .O(z06));
  nand3  g25(.a(new_n47_), .b(x08), .c(x06), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n56_), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n59_), .c(new_n47_), .O(z09));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n74_), .c(new_n79_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n55_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n61_), .c(new_n46_), .O(z10));
  inv1   g41(.a(new_n74_), .O(new_n86_));
  nand2  g42(.a(new_n79_), .b(x21), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  nor2   g45(.a(new_n55_), .b(new_n46_), .O(new_n90_));
  oai21  g46(.a(new_n79_), .b(x21), .c(new_n90_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n89_), .c(new_n63_), .O(z11));
  inv1   g48(.a(z04), .O(new_n93_));
  aoi21  g49(.a(new_n86_), .b(x14), .c(new_n49_), .O(new_n94_));
  inv1   g50(.a(new_n87_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n56_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n93_), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n74_), .b(new_n98_), .O(new_n99_));
  xor2a  g55(.a(new_n49_), .b(new_n48_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n99_), .c(new_n56_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n67_), .c(new_n47_), .O(z13));
  inv1   g58(.a(x24), .O(new_n103_));
  oai21  g59(.a(new_n49_), .b(new_n48_), .c(new_n103_), .O(new_n104_));
  inv1   g60(.a(new_n49_), .O(new_n105_));
  nor2   g61(.a(new_n103_), .b(new_n48_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi22  g63(.a(new_n107_), .b(new_n104_), .c(new_n52_), .d(x16), .O(new_n108_));
  nor2   g64(.a(z06), .b(new_n46_), .O(new_n109_));
  oai21  g65(.a(new_n108_), .b(new_n55_), .c(new_n109_), .O(z14));
  inv1   g66(.a(x26), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n45_), .c(x25), .O(new_n112_));
  nor2   g68(.a(new_n112_), .b(new_n107_), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n107_), .b(new_n114_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n90_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n113_), .c(new_n70_), .O(z15));
  nand3  g73(.a(new_n106_), .b(new_n105_), .c(x25), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n111_), .O(new_n119_));
  nor2   g75(.a(new_n51_), .b(x18), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n50_), .c(new_n55_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


