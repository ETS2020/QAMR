// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nor2   g00(.a(x17), .b(x04), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  inv1   g05(.a(x19), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  and2   g08(.a(x22), .b(x21), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n52_), .c(x20), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  and2   g11(.a(new_n55_), .b(new_n49_), .O(z00));
  inv1   g12(.a(new_n45_), .O(new_n57_));
  nand3  g13(.a(new_n57_), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n45_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n45_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n57_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n45_), .O(z07));
  nand3  g27(.a(new_n57_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(x11), .O(new_n74_));
  nor2   g30(.a(new_n54_), .b(new_n74_), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n50_), .c(new_n49_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n58_), .O(z09));
  inv1   g33(.a(x20), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n50_), .O(new_n79_));
  nand3  g35(.a(new_n53_), .b(new_n52_), .c(x12), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  aoi21  g37(.a(new_n78_), .b(new_n50_), .c(new_n48_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n60_), .c(new_n57_), .O(z10));
  nand2  g40(.a(new_n79_), .b(x21), .O(new_n85_));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  inv1   g45(.a(new_n48_), .O(new_n90_));
  oai21  g46(.a(new_n79_), .b(x21), .c(new_n90_), .O(new_n91_));
  aoi21  g47(.a(x08), .b(x02), .c(new_n45_), .O(new_n92_));
  oai21  g48(.a(new_n91_), .b(new_n89_), .c(new_n92_), .O(z11));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n94_));
  aoi21  g50(.a(new_n52_), .b(x14), .c(new_n94_), .O(new_n95_));
  inv1   g51(.a(new_n85_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x22), .c(new_n90_), .O(new_n97_));
  aoi21  g53(.a(x08), .b(x03), .c(new_n45_), .O(new_n98_));
  oai21  g54(.a(new_n97_), .b(new_n95_), .c(new_n98_), .O(z12));
  aoi21  g55(.a(x17), .b(new_n46_), .c(x04), .O(new_n100_));
  inv1   g56(.a(new_n86_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(x15), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  nor2   g59(.a(new_n94_), .b(new_n103_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  nand2  g61(.a(new_n94_), .b(new_n103_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n105_), .c(new_n47_), .d(x09), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n46_), .c(new_n100_), .O(z13));
  nand2  g64(.a(new_n101_), .b(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  xor2a  g66(.a(new_n104_), .b(x24), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n110_), .c(new_n90_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n68_), .c(new_n45_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n104_), .b(x24), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g72(.a(x26), .b(x17), .c(new_n114_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n104_), .c(x24), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(new_n90_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n70_), .c(new_n57_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  oai21  g77(.a(new_n115_), .b(new_n114_), .c(new_n121_), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand2  g79(.a(new_n55_), .b(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n49_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n72_), .O(z16));
endmodule


