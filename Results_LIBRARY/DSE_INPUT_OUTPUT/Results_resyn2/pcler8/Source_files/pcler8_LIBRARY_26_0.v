// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  and2   g00(.a(x20), .b(x19), .O(new_n45_));
  and2   g01(.a(x22), .b(x21), .O(new_n46_));
  nand4  g02(.a(new_n46_), .b(new_n45_), .c(x24), .d(x23), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  inv1   g04(.a(x26), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(x10), .b(new_n52_), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n51_), .c(x05), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n47_), .O(z00));
  inv1   g13(.a(x05), .O(new_n58_));
  inv1   g14(.a(x10), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g16(.a(x08), .b(x00), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n60_), .O(z01));
  inv1   g18(.a(x01), .O(new_n63_));
  oai21  g19(.a(new_n51_), .b(new_n63_), .c(new_n60_), .O(z02));
  inv1   g20(.a(x02), .O(new_n65_));
  nand2  g21(.a(new_n60_), .b(x08), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z03));
  nand2  g23(.a(x08), .b(x03), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n60_), .O(z04));
  inv1   g25(.a(x04), .O(new_n70_));
  nor2   g26(.a(new_n66_), .b(new_n70_), .O(z05));
  oai21  g27(.a(new_n51_), .b(new_n58_), .c(new_n60_), .O(z06));
  inv1   g28(.a(x06), .O(new_n73_));
  nor2   g29(.a(new_n66_), .b(new_n73_), .O(z07));
  inv1   g30(.a(x07), .O(new_n75_));
  nor2   g31(.a(new_n66_), .b(new_n75_), .O(z08));
  nor2   g32(.a(new_n52_), .b(x08), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nand3  g34(.a(new_n46_), .b(x20), .c(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n77_), .c(new_n58_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(x10), .c(new_n61_), .O(z09));
  inv1   g38(.a(new_n45_), .O(new_n83_));
  inv1   g39(.a(new_n78_), .O(new_n84_));
  nand2  g40(.a(new_n46_), .b(x12), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n55_), .O(new_n88_));
  oai22  g44(.a(new_n88_), .b(new_n87_), .c(new_n66_), .d(new_n63_), .O(z10));
  nand2  g45(.a(new_n45_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n84_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n45_), .b(x21), .c(new_n55_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n92_), .c(new_n66_), .d(new_n65_), .O(z11));
  inv1   g50(.a(x14), .O(new_n95_));
  nand4  g51(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n78_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  inv1   g55(.a(new_n77_), .O(new_n100_));
  aoi21  g56(.a(new_n90_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n98_), .c(new_n58_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x10), .c(new_n68_), .O(z12));
  inv1   g59(.a(x23), .O(new_n104_));
  nor2   g60(.a(new_n96_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n50_), .b(x24), .c(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g63(.a(new_n96_), .b(new_n104_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n107_), .c(new_n55_), .O(new_n109_));
  oai21  g65(.a(new_n66_), .b(new_n70_), .c(new_n109_), .O(z13));
  nand3  g66(.a(new_n50_), .b(x24), .c(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  xor2a  g68(.a(new_n105_), .b(x24), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n112_), .c(new_n53_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n51_), .c(new_n58_), .O(z14));
  nand2  g71(.a(x08), .b(x06), .O(new_n116_));
  nand2  g72(.a(x26), .b(x17), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(new_n105_), .c(x25), .d(x24), .O(new_n118_));
  aoi21  g74(.a(new_n47_), .b(new_n48_), .c(new_n100_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n118_), .c(new_n58_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(x10), .c(new_n116_), .O(z15));
  nand2  g77(.a(x08), .b(x07), .O(new_n122_));
  oai21  g78(.a(new_n47_), .b(new_n48_), .c(new_n49_), .O(new_n123_));
  nor2   g79(.a(new_n96_), .b(x18), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n84_), .c(new_n100_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n123_), .c(new_n58_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(x10), .c(new_n122_), .O(z16));
endmodule


