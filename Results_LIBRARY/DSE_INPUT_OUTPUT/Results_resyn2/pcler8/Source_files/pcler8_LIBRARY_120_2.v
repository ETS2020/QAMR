// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x19), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x10), .O(new_n46_));
  nand2  g02(.a(x26), .b(x25), .O(new_n47_));
  nand3  g03(.a(x24), .b(x23), .c(x22), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand3  g09(.a(x21), .b(x20), .c(x19), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  aoi21  g15(.a(new_n45_), .b(x10), .c(new_n50_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z03));
  nand2  g20(.a(new_n60_), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(z05));
  and2   g24(.a(new_n60_), .b(x05), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  nand2  g29(.a(x09), .b(new_n50_), .O(new_n74_));
  inv1   g30(.a(z01), .O(new_n75_));
  nand4  g31(.a(x21), .b(x20), .c(x11), .d(new_n51_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n49_), .c(new_n45_), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n74_), .c(new_n75_), .O(z09));
  inv1   g35(.a(x20), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n45_), .O(new_n81_));
  nand4  g37(.a(x26), .b(x25), .c(x21), .d(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n48_), .c(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n80_), .b(new_n45_), .c(new_n52_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n61_), .O(z10));
  aoi21  g42(.a(new_n49_), .b(x13), .c(new_n54_), .O(new_n87_));
  oai21  g43(.a(new_n81_), .b(x21), .c(new_n53_), .O(new_n88_));
  nand2  g44(.a(new_n60_), .b(x02), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z11));
  nand2  g46(.a(new_n55_), .b(x22), .O(new_n91_));
  nand3  g47(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  and2   g49(.a(x23), .b(x14), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n93_), .c(new_n91_), .O(new_n95_));
  oai21  g51(.a(new_n55_), .b(x22), .c(new_n53_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n65_), .O(z12));
  and2   g53(.a(x23), .b(x22), .O(new_n98_));
  nand2  g54(.a(new_n93_), .b(x15), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n55_), .c(new_n98_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nand2  g57(.a(new_n91_), .b(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n100_), .c(new_n53_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n67_), .c(new_n46_), .O(z13));
  inv1   g60(.a(x24), .O(new_n105_));
  nand2  g61(.a(new_n55_), .b(new_n98_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  inv1   g63(.a(new_n48_), .O(new_n108_));
  nand3  g64(.a(x26), .b(x25), .c(x16), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n108_), .c(x21), .d(x20), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n107_), .c(new_n53_), .O(new_n111_));
  nand2  g67(.a(x08), .b(x05), .O(new_n112_));
  oai21  g68(.a(new_n74_), .b(new_n105_), .c(new_n51_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n45_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(z14));
  nand3  g71(.a(x26), .b(x25), .c(x17), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(new_n117_));
  nand3  g73(.a(new_n55_), .b(new_n108_), .c(x25), .O(new_n118_));
  inv1   g74(.a(x25), .O(new_n119_));
  oai21  g75(.a(new_n54_), .b(new_n48_), .c(new_n119_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n118_), .c(new_n117_), .O(new_n121_));
  nand2  g77(.a(new_n60_), .b(x06), .O(new_n122_));
  oai21  g78(.a(new_n121_), .b(new_n52_), .c(new_n122_), .O(z15));
  nor2   g79(.a(new_n54_), .b(new_n48_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(x25), .c(x26), .O(new_n125_));
  nor2   g81(.a(new_n47_), .b(x18), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n53_), .O(new_n128_));
  nand2  g84(.a(new_n60_), .b(x07), .O(new_n129_));
  oai21  g85(.a(new_n128_), .b(new_n125_), .c(new_n129_), .O(z16));
endmodule


