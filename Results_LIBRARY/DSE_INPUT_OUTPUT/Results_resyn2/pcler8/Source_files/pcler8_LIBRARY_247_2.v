// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  nand3  g00(.a(x21), .b(x20), .c(x19), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x23), .c(x22), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g08(.a(x17), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(x16), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n47_), .O(z00));
  nand3  g12(.a(new_n54_), .b(x08), .c(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n54_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n54_), .O(z03));
  inv1   g18(.a(new_n54_), .O(new_n63_));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n63_), .O(z04));
  nand3  g21(.a(new_n54_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n63_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n54_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n54_), .O(z08));
  inv1   g29(.a(new_n50_), .O(new_n74_));
  nand2  g30(.a(new_n54_), .b(new_n74_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x23), .b(x22), .O(new_n77_));
  nor2   g33(.a(new_n51_), .b(new_n77_), .O(new_n78_));
  nand3  g34(.a(x21), .b(x20), .c(x11), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n75_), .c(new_n57_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  nand2  g39(.a(x21), .b(x12), .O(new_n84_));
  nor3   g40(.a(new_n84_), .b(new_n51_), .c(new_n77_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  oai21  g43(.a(new_n85_), .b(new_n83_), .c(new_n87_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n59_), .c(new_n63_), .O(z10));
  inv1   g45(.a(x13), .O(new_n90_));
  nor3   g46(.a(new_n51_), .b(new_n77_), .c(new_n90_), .O(new_n91_));
  aoi21  g47(.a(x20), .b(x19), .c(x21), .O(new_n92_));
  nor2   g48(.a(new_n92_), .b(new_n50_), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(new_n45_), .c(new_n93_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n61_), .c(new_n63_), .O(z11));
  nand2  g51(.a(x24), .b(x23), .O(new_n96_));
  nand3  g52(.a(x26), .b(x25), .c(x14), .O(new_n97_));
  nor2   g53(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g54(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  nor2   g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g56(.a(new_n46_), .b(x22), .c(new_n74_), .O(new_n101_));
  and2   g57(.a(new_n64_), .b(new_n54_), .O(new_n102_));
  oai21  g58(.a(new_n101_), .b(new_n100_), .c(new_n102_), .O(z12));
  inv1   g59(.a(new_n99_), .O(new_n104_));
  inv1   g60(.a(new_n51_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(x15), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n104_), .c(x23), .O(new_n107_));
  inv1   g63(.a(x23), .O(new_n108_));
  nand2  g64(.a(new_n99_), .b(new_n108_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n107_), .c(new_n54_), .d(new_n74_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n66_), .O(z13));
  nand3  g67(.a(new_n104_), .b(x24), .c(x23), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand2  g69(.a(new_n47_), .b(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n74_), .O(new_n115_));
  oai21  g71(.a(new_n52_), .b(new_n53_), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n68_), .O(z14));
  nor2   g73(.a(new_n99_), .b(new_n96_), .O(new_n118_));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g77(.a(new_n112_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n74_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n70_), .c(new_n54_), .O(z15));
  inv1   g80(.a(z08), .O(new_n125_));
  aoi21  g81(.a(new_n118_), .b(x25), .c(x26), .O(new_n126_));
  nor2   g82(.a(new_n45_), .b(x18), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n78_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n74_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n126_), .c(new_n125_), .O(z16));
endmodule


