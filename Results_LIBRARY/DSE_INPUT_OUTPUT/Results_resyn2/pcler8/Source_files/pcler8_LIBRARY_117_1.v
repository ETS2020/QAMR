// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_,
    new_n54_, new_n56_, new_n58_, new_n62_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  and2   g07(.a(x08), .b(x00), .O(z01));
  inv1   g08(.a(x01), .O(new_n53_));
  inv1   g09(.a(x08), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(z02));
  inv1   g11(.a(x02), .O(new_n56_));
  nor2   g12(.a(new_n54_), .b(new_n56_), .O(z03));
  nand2  g13(.a(x08), .b(x03), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z04));
  and2   g15(.a(x08), .b(x04), .O(z05));
  and2   g16(.a(x08), .b(x05), .O(z06));
  nand2  g17(.a(x10), .b(new_n54_), .O(new_n62_));
  nand2  g18(.a(x08), .b(x06), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n62_), .O(z07));
  nand2  g20(.a(x08), .b(x07), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n62_), .O(z08));
  nand2  g22(.a(x09), .b(new_n54_), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(new_n68_));
  nand4  g24(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n69_));
  nand4  g25(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n69_), .c(x19), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  inv1   g28(.a(new_n62_), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(z01), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n72_), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n69_), .c(new_n76_), .O(new_n78_));
  nand3  g34(.a(new_n45_), .b(x09), .c(new_n54_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  oai21  g38(.a(new_n54_), .b(new_n53_), .c(new_n82_), .O(z10));
  inv1   g39(.a(new_n69_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(new_n79_), .O(new_n88_));
  oai21  g44(.a(new_n76_), .b(x21), .c(new_n88_), .O(new_n89_));
  oai22  g45(.a(new_n89_), .b(new_n87_), .c(new_n54_), .d(new_n56_), .O(z11));
  inv1   g46(.a(new_n85_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x22), .O(new_n92_));
  aoi21  g48(.a(new_n84_), .b(x14), .c(new_n92_), .O(new_n93_));
  oai21  g49(.a(new_n91_), .b(x22), .c(new_n88_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n58_), .O(z12));
  nand3  g51(.a(new_n49_), .b(x24), .c(x15), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  aoi21  g53(.a(new_n47_), .b(new_n46_), .c(new_n67_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g55(.a(new_n73_), .b(z05), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(z13));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n47_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  and2   g61(.a(x22), .b(x21), .O(new_n106_));
  and2   g62(.a(x24), .b(x23), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n106_), .c(new_n76_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n105_), .c(new_n103_), .O(new_n109_));
  nor2   g65(.a(new_n73_), .b(z06), .O(new_n110_));
  oai21  g66(.a(new_n109_), .b(new_n67_), .c(new_n110_), .O(z14));
  nand3  g67(.a(x26), .b(x25), .c(x17), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand4  g69(.a(new_n107_), .b(new_n106_), .c(new_n76_), .d(x25), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(x24), .b(x23), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n47_), .c(new_n115_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n114_), .c(new_n113_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n79_), .c(new_n63_), .O(z15));
  inv1   g75(.a(z08), .O(new_n120_));
  nor2   g76(.a(new_n116_), .b(new_n47_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(x25), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand2  g79(.a(new_n49_), .b(new_n123_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n108_), .c(new_n68_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n122_), .c(new_n120_), .O(z16));
endmodule


