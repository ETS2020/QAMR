// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x07), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(x24), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  inv1   g10(.a(x08), .O(new_n55_));
  inv1   g11(.a(x10), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x07), .O(new_n57_));
  oai21  g13(.a(new_n55_), .b(new_n54_), .c(new_n57_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nand2  g15(.a(new_n57_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  inv1   g17(.a(x02), .O(new_n62_));
  nor2   g18(.a(new_n60_), .b(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n60_), .b(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n57_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z06));
  inv1   g25(.a(new_n60_), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  aoi21  g28(.a(x10), .b(new_n55_), .c(new_n45_), .O(z08));
  nand2  g29(.a(new_n51_), .b(new_n56_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n45_), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  and2   g35(.a(x22), .b(x21), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(x20), .c(x11), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n79_), .c(new_n77_), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n76_), .c(new_n60_), .d(new_n54_), .O(z09));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  nand3  g41(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n78_), .c(new_n85_), .O(new_n87_));
  nand2  g43(.a(x09), .b(new_n55_), .O(new_n88_));
  nor2   g44(.a(x20), .b(x19), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n87_), .c(x07), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(x10), .c(new_n55_), .d(new_n59_), .O(z10));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  and2   g49(.a(x22), .b(x13), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n79_), .c(new_n93_), .O(new_n95_));
  or2    g51(.a(new_n85_), .b(x21), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n75_), .c(new_n45_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n95_), .c(new_n60_), .d(new_n62_), .O(z11));
  inv1   g54(.a(x14), .O(new_n99_));
  inv1   g55(.a(new_n47_), .O(new_n100_));
  oai21  g56(.a(new_n78_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  aoi21  g58(.a(new_n93_), .b(new_n102_), .c(new_n88_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n101_), .c(x07), .O(new_n104_));
  oai22  g60(.a(new_n104_), .b(x10), .c(new_n55_), .d(new_n64_), .O(z12));
  nand3  g61(.a(new_n49_), .b(x24), .c(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  aoi21  g63(.a(new_n47_), .b(new_n46_), .c(new_n88_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(x07), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x10), .c(new_n66_), .O(z13));
  nand3  g66(.a(new_n49_), .b(x24), .c(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n47_), .b(new_n46_), .c(new_n113_), .O(new_n114_));
  nand4  g70(.a(new_n80_), .b(new_n85_), .c(x24), .d(x23), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  nand2  g72(.a(new_n70_), .b(x05), .O(new_n117_));
  oai21  g73(.a(new_n116_), .b(new_n76_), .c(new_n117_), .O(z14));
  nand2  g74(.a(new_n49_), .b(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n48_), .b(x25), .c(x24), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  nand2  g78(.a(new_n115_), .b(new_n122_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n76_), .c(new_n71_), .O(z15));
  inv1   g81(.a(z08), .O(new_n126_));
  inv1   g82(.a(new_n121_), .O(new_n127_));
  nor2   g83(.a(new_n47_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n79_), .c(new_n74_), .O(new_n129_));
  oai21  g85(.a(new_n127_), .b(x26), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n126_), .O(z16));
endmodule


