// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x22), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  inv1   g08(.a(x09), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(x08), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nor3   g11(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  nand2  g13(.a(new_n45_), .b(x10), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n57_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  nor2   g19(.a(new_n59_), .b(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n58_), .O(z04));
  inv1   g22(.a(x04), .O(new_n67_));
  nor2   g23(.a(new_n59_), .b(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n58_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  nor2   g27(.a(new_n59_), .b(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n58_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(new_n80_));
  oai22  g36(.a(new_n80_), .b(new_n55_), .c(new_n59_), .d(new_n57_), .O(z09));
  inv1   g37(.a(z02), .O(new_n82_));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  aoi21  g39(.a(new_n78_), .b(x12), .c(new_n83_), .O(new_n84_));
  inv1   g40(.a(x20), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n75_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n54_), .c(new_n52_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n84_), .c(new_n82_), .O(z10));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n51_), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(new_n48_), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  nand2  g48(.a(new_n83_), .b(new_n92_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n54_), .c(new_n52_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(new_n91_), .c(new_n59_), .d(new_n63_), .O(z11));
  inv1   g51(.a(x14), .O(new_n96_));
  oai21  g52(.a(new_n76_), .b(new_n96_), .c(new_n49_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n54_), .c(x22), .d(new_n52_), .O(new_n98_));
  inv1   g54(.a(x08), .O(new_n99_));
  nand2  g55(.a(x09), .b(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n48_), .c(new_n52_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n45_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n98_), .c(new_n65_), .O(z12));
  nor2   g59(.a(new_n46_), .b(x22), .O(new_n104_));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n49_), .c(x23), .O(new_n106_));
  nand2  g62(.a(x22), .b(new_n52_), .O(new_n107_));
  aoi21  g63(.a(new_n48_), .b(new_n46_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  aoi22  g65(.a(new_n45_), .b(x10), .c(x08), .d(x04), .O(new_n110_));
  oai21  g66(.a(new_n109_), .b(new_n100_), .c(new_n110_), .O(z13));
  inv1   g67(.a(new_n55_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand2  g69(.a(new_n50_), .b(new_n113_), .O(new_n114_));
  nand3  g70(.a(x26), .b(x25), .c(x16), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n49_), .c(new_n47_), .d(x24), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n114_), .c(new_n112_), .O(new_n117_));
  inv1   g73(.a(new_n59_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(x05), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n117_), .O(z14));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(new_n49_), .c(x24), .d(x23), .O(new_n122_));
  nand4  g78(.a(x24), .b(x21), .c(x20), .d(x19), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  aoi21  g82(.a(new_n122_), .b(x25), .c(new_n126_), .O(new_n127_));
  nor2   g83(.a(new_n99_), .b(new_n71_), .O(new_n128_));
  oai21  g84(.a(new_n100_), .b(new_n124_), .c(new_n52_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n45_), .c(new_n128_), .O(new_n130_));
  oai21  g86(.a(new_n127_), .b(new_n55_), .c(new_n130_), .O(z15));
  inv1   g87(.a(x26), .O(new_n132_));
  nand3  g88(.a(x25), .b(x23), .c(x22), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n123_), .c(new_n132_), .O(new_n134_));
  nor2   g90(.a(new_n83_), .b(x18), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n78_), .O(new_n136_));
  nand3  g92(.a(new_n136_), .b(new_n134_), .c(new_n112_), .O(new_n137_));
  nand2  g93(.a(new_n118_), .b(x07), .O(new_n138_));
  nand2  g94(.a(new_n138_), .b(new_n137_), .O(z16));
endmodule


