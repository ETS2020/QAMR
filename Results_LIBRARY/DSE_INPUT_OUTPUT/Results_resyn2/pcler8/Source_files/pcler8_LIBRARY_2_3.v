// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x08), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  aoi21  g09(.a(new_n45_), .b(new_n52_), .c(new_n53_), .O(z01));
  oai21  g10(.a(x10), .b(x01), .c(x08), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z02));
  oai21  g12(.a(x10), .b(x02), .c(x08), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z03));
  inv1   g14(.a(x03), .O(new_n59_));
  aoi21  g15(.a(new_n45_), .b(new_n59_), .c(new_n53_), .O(z04));
  nand2  g16(.a(x08), .b(x04), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(x10), .O(z05));
  nand2  g18(.a(x08), .b(x05), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x10), .O(z06));
  nand2  g20(.a(x08), .b(x06), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(x10), .O(z07));
  nand2  g22(.a(x08), .b(x07), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x10), .O(z08));
  inv1   g24(.a(x19), .O(new_n69_));
  nand2  g25(.a(x22), .b(x21), .O(new_n70_));
  nand2  g26(.a(x20), .b(x11), .O(new_n71_));
  nor3   g27(.a(new_n71_), .b(new_n50_), .c(new_n70_), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n69_), .c(new_n47_), .O(new_n73_));
  nand2  g29(.a(x08), .b(x00), .O(new_n74_));
  aoi21  g30(.a(new_n74_), .b(new_n73_), .c(x10), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nor2   g33(.a(new_n50_), .b(new_n70_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(x12), .c(new_n77_), .O(new_n79_));
  inv1   g35(.a(new_n48_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n80_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n79_), .c(new_n55_), .O(z10));
  inv1   g38(.a(new_n50_), .O(new_n83_));
  nand2  g39(.a(new_n76_), .b(x21), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n76_), .b(x21), .c(new_n80_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n57_), .O(z11));
  nand2  g44(.a(x08), .b(x03), .O(new_n89_));
  inv1   g45(.a(x14), .O(new_n90_));
  inv1   g46(.a(new_n49_), .O(new_n91_));
  oai21  g47(.a(new_n50_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  inv1   g48(.a(x22), .O(new_n93_));
  nand2  g49(.a(new_n84_), .b(new_n93_), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n92_), .c(new_n47_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n89_), .c(x10), .O(z12));
  inv1   g52(.a(x15), .O(new_n97_));
  nor2   g53(.a(new_n50_), .b(new_n97_), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n49_), .b(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n47_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n61_), .c(x10), .O(z13));
  oai21  g58(.a(x10), .b(x05), .c(x08), .O(new_n103_));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  and2   g61(.a(x22), .b(x21), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n76_), .c(x24), .d(x23), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n49_), .b(new_n99_), .c(new_n108_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n107_), .c(new_n105_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n48_), .c(new_n103_), .O(z14));
  oai21  g67(.a(x10), .b(x06), .c(x08), .O(new_n112_));
  nand3  g68(.a(x26), .b(x25), .c(x17), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  nor2   g70(.a(new_n49_), .b(new_n99_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(x25), .c(x24), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n107_), .b(new_n117_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n48_), .c(new_n112_), .O(z15));
  oai21  g76(.a(x10), .b(x07), .c(x08), .O(new_n121_));
  inv1   g77(.a(new_n116_), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nor2   g79(.a(new_n50_), .b(new_n49_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n123_), .c(new_n48_), .O(new_n125_));
  oai21  g81(.a(new_n122_), .b(x26), .c(new_n125_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n121_), .O(z16));
endmodule


