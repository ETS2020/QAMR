// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x17), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x00), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(x23), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand3  g13(.a(x17), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand3  g15(.a(new_n46_), .b(x08), .c(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  and2   g18(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand3  g20(.a(new_n46_), .b(x08), .c(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  and2   g25(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n46_), .O(z08));
  oai21  g31(.a(new_n45_), .b(x08), .c(x00), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  inv1   g34(.a(new_n51_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(x23), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  and2   g37(.a(x20), .b(x11), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n81_), .c(new_n77_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n49_), .c(new_n76_), .O(z09));
  inv1   g40(.a(x20), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n77_), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n81_), .b(x12), .c(new_n87_), .O(new_n88_));
  nand2  g44(.a(new_n85_), .b(new_n77_), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(new_n50_), .c(new_n46_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n88_), .c(new_n60_), .O(z10));
  inv1   g47(.a(new_n80_), .O(new_n92_));
  nand2  g48(.a(new_n86_), .b(x21), .O(new_n93_));
  and2   g49(.a(x22), .b(x13), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n93_), .O(new_n95_));
  oai21  g51(.a(new_n86_), .b(x21), .c(new_n50_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n63_), .O(z11));
  aoi21  g53(.a(new_n92_), .b(x14), .c(new_n52_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n93_), .b(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n50_), .c(new_n46_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z12));
  inv1   g58(.a(x15), .O(new_n103_));
  nor2   g59(.a(new_n80_), .b(new_n103_), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  xor2a  g61(.a(new_n52_), .b(new_n105_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n104_), .c(new_n50_), .O(new_n107_));
  aoi22  g63(.a(new_n107_), .b(new_n67_), .c(new_n45_), .d(x00), .O(z13));
  nor2   g64(.a(new_n52_), .b(new_n105_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(x24), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  nand2  g67(.a(new_n54_), .b(new_n111_), .O(new_n112_));
  aoi22  g68(.a(new_n112_), .b(new_n110_), .c(new_n79_), .d(x16), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n49_), .c(new_n70_), .O(z14));
  aoi21  g70(.a(new_n109_), .b(x24), .c(x25), .O(new_n115_));
  nand4  g71(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n50_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n115_), .c(new_n72_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n46_), .O(new_n119_));
  nand4  g75(.a(new_n50_), .b(x26), .c(x25), .d(x17), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n119_), .O(z15));
  inv1   g77(.a(x26), .O(new_n122_));
  nand2  g78(.a(new_n116_), .b(new_n122_), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand2  g80(.a(new_n55_), .b(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n50_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n74_), .c(new_n46_), .O(z16));
endmodule


