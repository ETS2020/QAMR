// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand4  g00(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  nand2  g05(.a(x09), .b(new_n49_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n48_), .c(new_n46_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x23), .c(x10), .O(z00));
  inv1   g09(.a(x10), .O(new_n54_));
  inv1   g10(.a(x23), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  oai21  g15(.a(new_n49_), .b(new_n59_), .c(new_n56_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  nand2  g17(.a(new_n56_), .b(x08), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n56_), .O(z05));
  inv1   g23(.a(x05), .O(new_n68_));
  nor2   g24(.a(new_n62_), .b(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n56_), .O(z08));
  nand4  g29(.a(x24), .b(x22), .c(x21), .d(x20), .O(new_n74_));
  nand3  g30(.a(x26), .b(x25), .c(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n51_), .c(new_n55_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(x10), .c(new_n57_), .O(z09));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  nand2  g35(.a(new_n48_), .b(x22), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  and2   g37(.a(x21), .b(x12), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(new_n83_));
  nand3  g39(.a(new_n51_), .b(x23), .c(new_n54_), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  oai21  g41(.a(x20), .b(x19), .c(new_n85_), .O(new_n86_));
  oai22  g42(.a(new_n86_), .b(new_n83_), .c(new_n62_), .d(new_n59_), .O(z10));
  and2   g43(.a(x21), .b(x20), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(x19), .O(new_n89_));
  aoi21  g45(.a(new_n81_), .b(x13), .c(new_n89_), .O(new_n90_));
  inv1   g46(.a(new_n79_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x21), .c(new_n85_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n90_), .c(new_n62_), .d(new_n61_), .O(z11));
  inv1   g49(.a(x14), .O(new_n94_));
  oai21  g50(.a(new_n47_), .b(new_n94_), .c(new_n46_), .O(new_n95_));
  inv1   g51(.a(x22), .O(new_n96_));
  aoi21  g52(.a(new_n89_), .b(new_n96_), .c(new_n50_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n95_), .c(new_n55_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x10), .c(new_n64_), .O(z12));
  inv1   g55(.a(x15), .O(new_n100_));
  oai21  g56(.a(new_n47_), .b(new_n100_), .c(new_n46_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n51_), .c(new_n55_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x10), .c(new_n66_), .O(z13));
  nor2   g59(.a(new_n45_), .b(x24), .O(new_n104_));
  nand2  g60(.a(new_n48_), .b(x16), .O(new_n105_));
  nand2  g61(.a(new_n45_), .b(x24), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  oai22  g64(.a(new_n108_), .b(new_n84_), .c(new_n62_), .d(new_n68_), .O(z14));
  nand3  g65(.a(x26), .b(x25), .c(x17), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  and2   g67(.a(x24), .b(x22), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n88_), .c(x25), .d(x19), .O(new_n113_));
  inv1   g69(.a(x19), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  oai21  g71(.a(new_n74_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n113_), .c(new_n111_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n50_), .c(x23), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n54_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n70_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n113_), .b(new_n121_), .O(new_n122_));
  nor2   g78(.a(new_n47_), .b(x18), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n46_), .c(new_n50_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n122_), .c(new_n55_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(x10), .c(new_n72_), .O(z16));
endmodule


