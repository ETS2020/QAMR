// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x10), .O(new_n46_));
  nand4  g02(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n52_), .c(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  oai21  g15(.a(new_n49_), .b(new_n59_), .c(new_n46_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  nand2  g17(.a(new_n46_), .b(x08), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n46_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n46_), .O(z05));
  nand3  g23(.a(new_n46_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n49_), .b(new_n70_), .c(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n47_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n52_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n57_), .c(new_n46_), .O(z09));
  and2   g33(.a(x20), .b(x19), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n47_), .c(new_n78_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n51_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g39(.a(new_n62_), .b(new_n59_), .c(new_n83_), .O(z10));
  nand2  g40(.a(new_n78_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n48_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n78_), .b(x21), .c(new_n52_), .O(new_n88_));
  oai22  g44(.a(new_n88_), .b(new_n87_), .c(new_n62_), .d(new_n61_), .O(z11));
  inv1   g45(.a(z04), .O(new_n90_));
  aoi21  g46(.a(new_n48_), .b(x14), .c(new_n53_), .O(new_n91_));
  inv1   g47(.a(x22), .O(new_n92_));
  nand2  g48(.a(new_n85_), .b(new_n92_), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(new_n52_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n91_), .c(new_n90_), .O(z12));
  nand2  g51(.a(x09), .b(new_n49_), .O(new_n96_));
  inv1   g52(.a(z05), .O(new_n97_));
  nand3  g53(.a(x26), .b(x25), .c(x24), .O(new_n98_));
  nand2  g54(.a(x15), .b(new_n50_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n98_), .c(x23), .O(new_n100_));
  nand3  g56(.a(new_n53_), .b(x23), .c(new_n50_), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n100_), .b(new_n54_), .c(new_n102_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n96_), .c(new_n97_), .O(z13));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n54_), .b(x24), .c(x23), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n53_), .b(new_n45_), .c(new_n108_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n51_), .c(new_n68_), .O(z14));
  nor2   g67(.a(new_n53_), .b(new_n45_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(x24), .c(x25), .O(new_n113_));
  nand2  g69(.a(x26), .b(x17), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(x25), .c(x24), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n53_), .c(new_n52_), .O(new_n116_));
  nor2   g72(.a(new_n49_), .b(new_n70_), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  oai21  g74(.a(new_n96_), .b(new_n118_), .c(new_n50_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n45_), .c(new_n117_), .O(new_n120_));
  oai21  g76(.a(new_n116_), .b(new_n113_), .c(new_n120_), .O(z15));
  inv1   g77(.a(x26), .O(new_n122_));
  nand4  g78(.a(new_n54_), .b(x25), .c(x24), .d(x23), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nor2   g80(.a(new_n98_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n54_), .c(new_n51_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  oai21  g83(.a(new_n96_), .b(new_n122_), .c(new_n50_), .O(new_n128_));
  aoi22  g84(.a(new_n128_), .b(new_n45_), .c(x08), .d(x07), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n127_), .O(z16));
endmodule


