// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n56_, new_n57_, new_n58_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g04(.a(x24), .O(new_n49_));
  inv1   g05(.a(x25), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n48_), .c(x26), .d(x09), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(x08), .O(z00));
  and2   g09(.a(x08), .b(x00), .O(z01));
  and2   g10(.a(x08), .b(x01), .O(z02));
  inv1   g11(.a(x08), .O(new_n56_));
  nand2  g12(.a(x10), .b(new_n56_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x02), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(z03));
  and2   g15(.a(x08), .b(x03), .O(z04));
  and2   g16(.a(x08), .b(x04), .O(z05));
  and2   g17(.a(x08), .b(x05), .O(z06));
  and2   g18(.a(x08), .b(x06), .O(z07));
  nand2  g19(.a(x08), .b(x07), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n57_), .O(z08));
  nand2  g21(.a(x09), .b(new_n56_), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(new_n67_));
  nand4  g23(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n68_));
  nand4  g24(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n69_));
  oai21  g25(.a(new_n69_), .b(new_n68_), .c(x19), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n67_), .O(new_n71_));
  inv1   g27(.a(new_n57_), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(z01), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n71_), .O(z09));
  nand2  g30(.a(x20), .b(x19), .O(new_n75_));
  inv1   g31(.a(new_n68_), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n67_), .O(new_n80_));
  nor2   g36(.a(new_n72_), .b(z02), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n81_), .O(z10));
  inv1   g38(.a(z03), .O(new_n83_));
  inv1   g39(.a(new_n75_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n76_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n84_), .b(x21), .c(new_n67_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z11));
  aoi21  g45(.a(new_n76_), .b(x14), .c(new_n47_), .O(new_n90_));
  inv1   g46(.a(x22), .O(new_n91_));
  nand2  g47(.a(new_n85_), .b(new_n91_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n67_), .O(new_n93_));
  nor2   g49(.a(z04), .b(new_n72_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n90_), .c(new_n94_), .O(z12));
  inv1   g51(.a(x15), .O(new_n96_));
  nor2   g52(.a(new_n68_), .b(new_n96_), .O(new_n97_));
  xor2a  g53(.a(new_n47_), .b(new_n46_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n97_), .c(new_n67_), .O(new_n99_));
  nor2   g55(.a(z05), .b(new_n72_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(z13));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  inv1   g59(.a(new_n47_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(x24), .c(x23), .O(new_n105_));
  oai21  g61(.a(new_n47_), .b(new_n46_), .c(new_n49_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(new_n107_));
  nor2   g63(.a(z06), .b(new_n72_), .O(new_n108_));
  oai21  g64(.a(new_n107_), .b(new_n66_), .c(new_n108_), .O(z14));
  nand2  g65(.a(x26), .b(x17), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(new_n48_), .c(x25), .d(x24), .O(new_n111_));
  nand2  g67(.a(new_n105_), .b(new_n50_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n111_), .c(new_n67_), .O(new_n113_));
  nor2   g69(.a(z07), .b(new_n72_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n113_), .O(z15));
  inv1   g71(.a(x26), .O(new_n116_));
  nand4  g72(.a(new_n104_), .b(x25), .c(x24), .d(x23), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nor2   g74(.a(new_n47_), .b(x18), .O(new_n119_));
  nand3  g75(.a(new_n45_), .b(x09), .c(new_n56_), .O(new_n120_));
  aoi21  g76(.a(new_n119_), .b(new_n76_), .c(new_n120_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n64_), .O(z16));
endmodule


