// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  nand2  g00(.a(x17), .b(x05), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n50_), .c(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n45_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand3  g16(.a(new_n45_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  and2   g19(.a(new_n63_), .b(new_n45_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  inv1   g21(.a(new_n45_), .O(new_n66_));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n66_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(x17), .O(z06));
  nand3  g26(.a(new_n45_), .b(x08), .c(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n45_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n57_), .c(new_n45_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n75_), .c(new_n80_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n53_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n59_), .c(new_n66_), .O(z10));
  inv1   g42(.a(new_n75_), .O(new_n87_));
  nand2  g43(.a(new_n80_), .b(x21), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n87_), .c(new_n88_), .O(new_n90_));
  or2    g46(.a(new_n80_), .b(x21), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n54_), .c(new_n45_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n90_), .c(new_n61_), .O(z11));
  aoi21  g49(.a(new_n87_), .b(x14), .c(new_n47_), .O(new_n94_));
  inv1   g50(.a(new_n88_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n54_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n64_), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n75_), .b(new_n98_), .O(new_n99_));
  xor2a  g55(.a(new_n47_), .b(new_n46_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n99_), .c(new_n54_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n67_), .c(new_n45_), .O(z13));
  oai21  g58(.a(x17), .b(x08), .c(x05), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n47_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  inv1   g61(.a(new_n47_), .O(new_n106_));
  nor2   g62(.a(new_n104_), .b(new_n46_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi22  g64(.a(new_n108_), .b(new_n105_), .c(new_n50_), .d(x16), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n53_), .c(new_n103_), .O(z14));
  inv1   g66(.a(x25), .O(new_n111_));
  aoi21  g67(.a(x26), .b(x17), .c(new_n111_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n107_), .c(new_n106_), .O(new_n113_));
  nand2  g69(.a(new_n108_), .b(new_n111_), .O(new_n114_));
  nand4  g70(.a(new_n114_), .b(new_n113_), .c(new_n54_), .d(new_n45_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n71_), .O(z15));
  inv1   g72(.a(x26), .O(new_n117_));
  nand3  g73(.a(new_n107_), .b(new_n106_), .c(x25), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nor2   g75(.a(new_n49_), .b(x18), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n48_), .c(new_n53_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n73_), .c(new_n66_), .O(z16));
endmodule


