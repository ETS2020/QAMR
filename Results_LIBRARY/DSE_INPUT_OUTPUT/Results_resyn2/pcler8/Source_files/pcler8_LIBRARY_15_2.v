// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x03), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g04(.a(x25), .O(new_n49_));
  inv1   g05(.a(x26), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n48_), .d(x24), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  inv1   g12(.a(x08), .O(new_n57_));
  nor2   g13(.a(x10), .b(new_n45_), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n56_), .O(z01));
  inv1   g17(.a(x01), .O(new_n62_));
  nor2   g18(.a(new_n60_), .b(new_n62_), .O(z02));
  nand2  g19(.a(x08), .b(x02), .O(new_n64_));
  oai21  g20(.a(x10), .b(new_n45_), .c(new_n64_), .O(z03));
  nand3  g21(.a(x10), .b(x08), .c(x03), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n58_), .O(z05));
  nand2  g25(.a(new_n59_), .b(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(new_n59_), .b(x06), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z07));
  nand2  g29(.a(new_n59_), .b(x07), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(z08));
  inv1   g31(.a(x10), .O(new_n76_));
  nand3  g32(.a(new_n53_), .b(new_n76_), .c(new_n45_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  and2   g35(.a(x22), .b(x21), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(x20), .c(x11), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n79_), .c(x19), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  oai21  g39(.a(new_n60_), .b(new_n56_), .c(new_n83_), .O(z09));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  nand3  g41(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n79_), .c(new_n85_), .O(new_n87_));
  nand2  g43(.a(x09), .b(new_n57_), .O(new_n88_));
  nor2   g44(.a(x20), .b(x19), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n87_), .c(x03), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(x10), .c(new_n57_), .d(new_n62_), .O(z10));
  inv1   g48(.a(new_n79_), .O(new_n93_));
  nand2  g49(.a(new_n85_), .b(x21), .O(new_n94_));
  and2   g50(.a(x22), .b(x13), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(new_n94_), .O(new_n96_));
  oai21  g52(.a(new_n85_), .b(x21), .c(new_n78_), .O(new_n97_));
  nand2  g53(.a(new_n59_), .b(x02), .O(new_n98_));
  oai21  g54(.a(new_n97_), .b(new_n96_), .c(new_n98_), .O(z11));
  oai21  g55(.a(new_n76_), .b(x08), .c(x03), .O(new_n100_));
  aoi21  g56(.a(new_n93_), .b(x14), .c(new_n47_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n53_), .c(new_n76_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n101_), .c(new_n100_), .O(z12));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n48_), .O(new_n107_));
  aoi21  g63(.a(new_n47_), .b(new_n46_), .c(new_n88_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(x03), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x10), .c(new_n68_), .O(z13));
  nand3  g66(.a(new_n51_), .b(x24), .c(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n47_), .b(new_n46_), .c(new_n113_), .O(new_n114_));
  nand4  g70(.a(new_n80_), .b(new_n85_), .c(x24), .d(x23), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n77_), .c(new_n70_), .O(z14));
  nand2  g73(.a(new_n51_), .b(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n48_), .b(x25), .c(x24), .O(new_n120_));
  nand2  g76(.a(new_n115_), .b(new_n49_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n77_), .c(new_n72_), .O(z15));
  nand2  g79(.a(new_n120_), .b(new_n50_), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nand4  g81(.a(new_n51_), .b(new_n48_), .c(x24), .d(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n78_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n74_), .O(z16));
endmodule


