// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor4   g06(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(z00));
  nand2  g07(.a(x10), .b(new_n48_), .O(new_n52_));
  nand2  g08(.a(x08), .b(x00), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n52_), .O(z01));
  and2   g10(.a(x08), .b(x01), .O(z02));
  nand2  g11(.a(x08), .b(x02), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n52_), .O(z03));
  nand2  g13(.a(x08), .b(x03), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n52_), .O(z04));
  nand2  g15(.a(x08), .b(x04), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n52_), .O(z05));
  nand2  g17(.a(x08), .b(x05), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n52_), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z07));
  nand2  g21(.a(x08), .b(x07), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n52_), .O(z08));
  nand2  g23(.a(x09), .b(new_n48_), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(new_n69_));
  nand4  g25(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n70_));
  nand4  g26(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n70_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(new_n53_), .c(new_n52_), .O(z09));
  and2   g30(.a(x20), .b(x19), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n69_), .O(new_n80_));
  aoi21  g36(.a(x10), .b(new_n48_), .c(z02), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n81_), .O(z10));
  inv1   g38(.a(z03), .O(new_n83_));
  inv1   g39(.a(new_n70_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n75_), .b(x21), .c(new_n69_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z11));
  inv1   g45(.a(new_n85_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x22), .O(new_n91_));
  aoi21  g47(.a(new_n84_), .b(x14), .c(new_n91_), .O(new_n92_));
  inv1   g48(.a(new_n50_), .O(new_n93_));
  oai21  g49(.a(new_n90_), .b(x22), .c(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n92_), .c(new_n58_), .O(z12));
  nor2   g51(.a(new_n46_), .b(new_n45_), .O(new_n96_));
  inv1   g52(.a(new_n47_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(x15), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  aoi21  g55(.a(new_n46_), .b(new_n45_), .c(new_n68_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n60_), .c(new_n52_), .O(z13));
  inv1   g58(.a(z06), .O(new_n103_));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n46_), .b(new_n45_), .c(new_n106_), .O(new_n107_));
  and2   g63(.a(x22), .b(x21), .O(new_n108_));
  and2   g64(.a(x24), .b(x23), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n108_), .c(new_n75_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n107_), .c(new_n105_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n68_), .c(new_n103_), .O(z14));
  nand2  g68(.a(x24), .b(x23), .O(new_n113_));
  nor2   g69(.a(new_n113_), .b(new_n46_), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  aoi21  g71(.a(x26), .b(x17), .c(new_n115_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand2  g73(.a(new_n110_), .b(new_n115_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n117_), .c(new_n93_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n64_), .O(z15));
  inv1   g76(.a(z08), .O(new_n121_));
  aoi21  g77(.a(new_n114_), .b(x25), .c(x26), .O(new_n122_));
  nand3  g78(.a(x22), .b(x21), .c(x20), .O(new_n123_));
  inv1   g79(.a(new_n123_), .O(new_n124_));
  inv1   g80(.a(x19), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(x18), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n84_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n69_), .O(new_n128_));
  oai21  g84(.a(new_n128_), .b(new_n122_), .c(new_n121_), .O(z16));
endmodule


