// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  nand2  g00(.a(x17), .b(x01), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x26), .c(x25), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x24), .c(x23), .O(new_n53_));
  oai21  g09(.a(new_n53_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g10(.a(new_n45_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  inv1   g14(.a(x17), .O(new_n59_));
  aoi21  g15(.a(new_n59_), .b(new_n46_), .c(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n45_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n55_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n55_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z06));
  and2   g24(.a(x08), .b(x06), .O(new_n69_));
  or2    g25(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n55_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n49_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n56_), .c(new_n45_), .O(z09));
  nand2  g33(.a(x20), .b(x19), .O(new_n78_));
  inv1   g34(.a(new_n73_), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  or2    g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n49_), .c(new_n45_), .O(new_n84_));
  nand3  g40(.a(new_n59_), .b(x08), .c(x01), .O(new_n85_));
  oai21  g41(.a(new_n84_), .b(new_n82_), .c(new_n85_), .O(z10));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n79_), .c(new_n87_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n78_), .b(new_n90_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n49_), .c(new_n45_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n89_), .c(new_n61_), .d(new_n55_), .O(z11));
  aoi21  g49(.a(new_n79_), .b(x14), .c(new_n51_), .O(new_n94_));
  inv1   g50(.a(new_n87_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n49_), .O(new_n96_));
  and2   g52(.a(new_n63_), .b(new_n45_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n94_), .c(new_n97_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n73_), .b(new_n99_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  xor2a  g57(.a(new_n51_), .b(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n49_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n65_), .c(new_n45_), .O(z13));
  nand3  g60(.a(x26), .b(x25), .c(x16), .O(new_n105_));
  nand4  g61(.a(new_n105_), .b(new_n52_), .c(x24), .d(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n51_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n106_), .c(new_n49_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n67_), .c(new_n55_), .O(z14));
  inv1   g66(.a(x25), .O(new_n111_));
  nand2  g67(.a(new_n53_), .b(new_n111_), .O(new_n112_));
  nor2   g68(.a(new_n51_), .b(new_n101_), .O(new_n113_));
  nor2   g69(.a(new_n111_), .b(new_n107_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n113_), .c(new_n48_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n112_), .O(new_n116_));
  nand2  g72(.a(x26), .b(x25), .O(new_n117_));
  oai21  g73(.a(new_n48_), .b(new_n117_), .c(new_n58_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(x17), .c(new_n69_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n116_), .O(z15));
  aoi21  g76(.a(new_n114_), .b(new_n113_), .c(x26), .O(new_n121_));
  inv1   g77(.a(new_n78_), .O(new_n122_));
  and2   g78(.a(x22), .b(x21), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(x23), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nand4  g81(.a(x26), .b(x25), .c(x24), .d(new_n125_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n124_), .c(new_n49_), .O(new_n127_));
  and2   g83(.a(new_n71_), .b(new_n45_), .O(new_n128_));
  oai21  g84(.a(new_n127_), .b(new_n121_), .c(new_n128_), .O(z16));
endmodule


