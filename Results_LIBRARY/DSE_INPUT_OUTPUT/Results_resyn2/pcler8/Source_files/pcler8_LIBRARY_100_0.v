// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nor2   g04(.a(x17), .b(x14), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  nand2  g08(.a(x23), .b(x22), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g10(.a(x21), .b(x20), .c(x19), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n51_), .O(z00));
  aoi21  g14(.a(x08), .b(x00), .c(new_n49_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n50_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n49_), .O(z03));
  aoi21  g20(.a(x08), .b(x03), .c(new_n49_), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n50_), .O(z05));
  nand3  g24(.a(new_n50_), .b(x08), .c(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n50_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n50_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  inv1   g31(.a(x22), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nand3  g33(.a(x21), .b(x20), .c(x11), .O(new_n78_));
  nor3   g34(.a(new_n78_), .b(new_n77_), .c(new_n76_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n75_), .c(new_n48_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n59_), .O(z09));
  nand2  g37(.a(x20), .b(x19), .O(new_n82_));
  nand2  g38(.a(x21), .b(x12), .O(new_n83_));
  nor3   g39(.a(new_n83_), .b(new_n77_), .c(new_n76_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  oai21  g42(.a(new_n84_), .b(new_n82_), .c(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n61_), .c(new_n49_), .O(z10));
  aoi21  g44(.a(new_n54_), .b(x13), .c(new_n55_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n82_), .b(new_n90_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n50_), .c(new_n48_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n89_), .c(new_n63_), .d(new_n49_), .O(z11));
  inv1   g49(.a(new_n77_), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(x14), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n56_), .b(x22), .c(new_n48_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n65_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  inv1   g55(.a(x23), .O(new_n100_));
  nor2   g56(.a(new_n95_), .b(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n52_), .b(new_n99_), .c(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n95_), .b(new_n100_), .c(new_n47_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n67_), .c(new_n50_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n55_), .b(new_n53_), .c(new_n108_), .O(new_n109_));
  nor2   g65(.a(new_n55_), .b(new_n53_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(x24), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n51_), .c(new_n69_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  aoi21  g70(.a(x26), .b(x17), .c(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n110_), .c(x24), .O(new_n116_));
  nand2  g72(.a(new_n111_), .b(new_n114_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n48_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n71_), .c(new_n50_), .O(z15));
  inv1   g75(.a(x18), .O(new_n120_));
  nand3  g76(.a(new_n56_), .b(new_n54_), .c(new_n120_), .O(new_n121_));
  inv1   g77(.a(x26), .O(new_n122_));
  nand3  g78(.a(x25), .b(x24), .c(x23), .O(new_n123_));
  oai21  g79(.a(new_n95_), .b(new_n123_), .c(new_n122_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n121_), .c(new_n48_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n73_), .c(new_n50_), .O(z16));
endmodule


