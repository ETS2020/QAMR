// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x05), .O(new_n45_));
  nand2  g01(.a(x17), .b(new_n45_), .O(new_n46_));
  nand2  g02(.a(x26), .b(x25), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g07(.a(x24), .b(x23), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  inv1   g12(.a(x17), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(x05), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z02));
  nand3  g18(.a(new_n46_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n46_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n46_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n58_), .O(z07));
  nand3  g28(.a(new_n46_), .b(x08), .c(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nor2   g30(.a(new_n50_), .b(new_n58_), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  nand4  g32(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  oai21  g35(.a(new_n59_), .b(new_n58_), .c(new_n79_), .O(z09));
  inv1   g36(.a(z02), .O(new_n81_));
  nand2  g37(.a(x20), .b(x19), .O(new_n82_));
  inv1   g38(.a(new_n76_), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  inv1   g42(.a(new_n50_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n86_), .c(new_n81_), .O(z10));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n83_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  nand2  g49(.a(new_n82_), .b(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n75_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n92_), .c(new_n63_), .O(z11));
  inv1   g52(.a(x14), .O(new_n97_));
  inv1   g53(.a(new_n53_), .O(new_n98_));
  oai21  g54(.a(new_n76_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  aoi21  g56(.a(new_n90_), .b(new_n100_), .c(new_n50_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n65_), .c(new_n58_), .O(z12));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n98_), .c(x23), .O(new_n105_));
  inv1   g61(.a(x23), .O(new_n106_));
  nand2  g62(.a(new_n53_), .b(new_n106_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n105_), .c(new_n87_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n67_), .c(new_n46_), .O(z13));
  inv1   g65(.a(new_n52_), .O(new_n110_));
  nand2  g66(.a(new_n98_), .b(new_n110_), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n53_), .b(new_n106_), .c(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n111_), .c(new_n113_), .O(new_n116_));
  oai21  g72(.a(x17), .b(x08), .c(new_n45_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n49_), .c(x09), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n116_), .c(new_n69_), .O(z14));
  nand2  g75(.a(new_n54_), .b(x25), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  nand2  g77(.a(new_n111_), .b(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n87_), .O(new_n123_));
  oai21  g79(.a(new_n51_), .b(new_n45_), .c(x17), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n123_), .c(new_n71_), .O(z15));
  aoi21  g81(.a(new_n54_), .b(x25), .c(x26), .O(new_n126_));
  inv1   g82(.a(x18), .O(new_n127_));
  nand3  g83(.a(new_n83_), .b(new_n98_), .c(new_n127_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n75_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n126_), .c(new_n73_), .O(z16));
endmodule


