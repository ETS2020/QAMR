// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand2  g04(.a(x17), .b(x16), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n48_), .O(z00));
  nand3  g12(.a(new_n49_), .b(x08), .c(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  inv1   g14(.a(new_n49_), .O(new_n59_));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  and2   g18(.a(new_n62_), .b(new_n49_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand3  g20(.a(new_n49_), .b(x08), .c(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand3  g22(.a(new_n49_), .b(x08), .c(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n59_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n49_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n49_), .O(z08));
  nand4  g30(.a(new_n49_), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x25), .b(x24), .O(new_n77_));
  nand3  g33(.a(x26), .b(x23), .c(x22), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g35(.a(x21), .b(x20), .c(x11), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n79_), .c(new_n76_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n75_), .c(new_n57_), .O(z09));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  and2   g40(.a(x21), .b(x12), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n79_), .c(new_n84_), .O(new_n86_));
  inv1   g42(.a(new_n52_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n87_), .O(new_n88_));
  and2   g44(.a(new_n60_), .b(new_n49_), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n86_), .c(new_n89_), .O(z10));
  inv1   g46(.a(new_n53_), .O(new_n91_));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n46_), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  nand2  g51(.a(new_n84_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n87_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n94_), .c(new_n63_), .O(z11));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n46_), .b(new_n99_), .O(new_n100_));
  nand4  g56(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n101_));
  aoi22  g57(.a(new_n101_), .b(new_n100_), .c(new_n79_), .d(x14), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n75_), .c(new_n65_), .O(z12));
  nand2  g59(.a(new_n91_), .b(x15), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nor2   g61(.a(new_n101_), .b(new_n105_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g63(.a(new_n101_), .b(new_n105_), .c(new_n75_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n67_), .O(z13));
  inv1   g66(.a(x24), .O(new_n111_));
  nand2  g67(.a(new_n48_), .b(new_n111_), .O(new_n112_));
  nand2  g68(.a(new_n47_), .b(x24), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n112_), .c(new_n87_), .O(new_n114_));
  oai21  g70(.a(new_n54_), .b(x17), .c(x16), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n69_), .O(z14));
  nand3  g72(.a(new_n47_), .b(x25), .c(x24), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n113_), .b(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n117_), .c(new_n87_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x25), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(x16), .c(x17), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n120_), .c(new_n71_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  inv1   g81(.a(new_n45_), .O(new_n126_));
  inv1   g82(.a(new_n46_), .O(new_n127_));
  inv1   g83(.a(new_n77_), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n127_), .c(new_n126_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand3  g86(.a(x21), .b(x20), .c(x19), .O(new_n131_));
  nor2   g87(.a(new_n131_), .b(x18), .O(new_n132_));
  aoi21  g88(.a(new_n132_), .b(new_n79_), .c(new_n52_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n73_), .c(new_n59_), .O(z16));
endmodule


