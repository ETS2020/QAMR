// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand4  g04(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  oai21  g08(.a(x09), .b(new_n45_), .c(new_n52_), .O(z00));
  nand3  g09(.a(x09), .b(x08), .c(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  inv1   g11(.a(x01), .O(new_n56_));
  aoi21  g12(.a(x09), .b(new_n56_), .c(new_n45_), .O(z02));
  nand3  g13(.a(x09), .b(x08), .c(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  inv1   g15(.a(x03), .O(new_n60_));
  aoi21  g16(.a(x09), .b(new_n60_), .c(new_n45_), .O(z04));
  nand3  g17(.a(x09), .b(x08), .c(x04), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  inv1   g19(.a(x09), .O(new_n64_));
  oai21  g20(.a(new_n64_), .b(x05), .c(x08), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  nand3  g22(.a(x09), .b(x08), .c(x06), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z07));
  oai21  g24(.a(new_n64_), .b(x07), .c(x08), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  nand2  g26(.a(x08), .b(x00), .O(new_n71_));
  nand2  g27(.a(new_n46_), .b(new_n45_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n49_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n71_), .c(new_n64_), .O(z09));
  nand2  g33(.a(x08), .b(x01), .O(new_n78_));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n49_), .c(new_n79_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n72_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n78_), .c(new_n64_), .O(z10));
  nand2  g41(.a(x08), .b(x02), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  nand2  g43(.a(x22), .b(x13), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n49_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(x20), .b(x19), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n90_), .c(new_n72_), .O(new_n92_));
  oai21  g48(.a(new_n89_), .b(new_n87_), .c(new_n92_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n86_), .c(new_n64_), .O(z11));
  nand2  g50(.a(x08), .b(x03), .O(new_n95_));
  inv1   g51(.a(x14), .O(new_n96_));
  nor2   g52(.a(new_n49_), .b(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  aoi21  g54(.a(new_n87_), .b(new_n98_), .c(new_n72_), .O(new_n99_));
  oai21  g55(.a(new_n97_), .b(new_n50_), .c(new_n99_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n95_), .c(new_n64_), .O(z12));
  oai21  g57(.a(new_n64_), .b(x04), .c(x08), .O(new_n102_));
  inv1   g58(.a(x15), .O(new_n103_));
  nor2   g59(.a(new_n49_), .b(new_n103_), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  xor2a  g61(.a(new_n50_), .b(new_n105_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n104_), .c(new_n48_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n102_), .O(z13));
  nand3  g64(.a(x26), .b(x25), .c(x24), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  and2   g66(.a(x22), .b(x21), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(new_n79_), .c(x24), .d(x23), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n50_), .b(new_n105_), .c(new_n113_), .O(new_n114_));
  aoi22  g70(.a(new_n114_), .b(new_n112_), .c(new_n110_), .d(x16), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n47_), .c(new_n65_), .O(z14));
  nand2  g72(.a(x08), .b(x06), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n112_), .b(new_n118_), .O(new_n119_));
  nor2   g75(.a(new_n118_), .b(new_n113_), .O(new_n120_));
  nor2   g76(.a(new_n50_), .b(new_n105_), .O(new_n121_));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n120_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n119_), .c(new_n73_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n117_), .c(new_n64_), .O(z15));
  inv1   g81(.a(x18), .O(new_n126_));
  nand2  g82(.a(new_n51_), .b(new_n126_), .O(new_n127_));
  inv1   g83(.a(x26), .O(new_n128_));
  nand2  g84(.a(new_n121_), .b(new_n120_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n127_), .c(new_n48_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n69_), .O(z16));
endmodule


