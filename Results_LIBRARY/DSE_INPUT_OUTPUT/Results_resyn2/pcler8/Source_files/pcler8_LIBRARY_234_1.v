// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n61_, new_n63_, new_n65_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  nand2  g07(.a(x08), .b(x00), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(z01));
  inv1   g09(.a(x08), .O(new_n54_));
  nand2  g10(.a(x10), .b(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z02));
  inv1   g13(.a(x02), .O(new_n58_));
  nor2   g14(.a(new_n54_), .b(new_n58_), .O(z03));
  and2   g15(.a(x08), .b(x03), .O(z04));
  nand2  g16(.a(x08), .b(x04), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n55_), .O(z05));
  nand2  g18(.a(x08), .b(x05), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n55_), .O(z06));
  nand2  g20(.a(x08), .b(x06), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n55_), .O(z07));
  and2   g22(.a(x08), .b(x07), .O(z08));
  inv1   g23(.a(x09), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(x08), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n45_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n52_), .O(z09));
  inv1   g32(.a(z02), .O(new_n77_));
  nand2  g33(.a(x20), .b(x19), .O(new_n78_));
  inv1   g34(.a(new_n72_), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  oai21  g38(.a(x20), .b(x19), .c(new_n69_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(new_n77_), .O(z10));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n79_), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  nand2  g44(.a(new_n78_), .b(new_n88_), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(new_n69_), .c(new_n45_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(new_n87_), .c(new_n54_), .d(new_n58_), .O(z11));
  inv1   g47(.a(new_n85_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(x22), .O(new_n93_));
  aoi21  g49(.a(new_n79_), .b(x14), .c(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n92_), .b(x22), .c(new_n69_), .O(new_n95_));
  inv1   g51(.a(new_n55_), .O(new_n96_));
  nor2   g52(.a(z04), .b(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(z12));
  nand3  g54(.a(new_n49_), .b(x24), .c(x15), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n48_), .O(new_n100_));
  nand2  g56(.a(new_n47_), .b(new_n46_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n100_), .c(new_n71_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n61_), .O(z13));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n47_), .b(new_n46_), .c(new_n106_), .O(new_n107_));
  inv1   g63(.a(new_n47_), .O(new_n108_));
  and2   g64(.a(x24), .b(x23), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n107_), .c(new_n105_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n70_), .c(new_n63_), .O(z14));
  inv1   g68(.a(z07), .O(new_n113_));
  inv1   g69(.a(new_n69_), .O(new_n114_));
  nand3  g70(.a(x26), .b(x25), .c(x17), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  and2   g72(.a(x20), .b(x19), .O(new_n117_));
  and2   g73(.a(x22), .b(x21), .O(new_n118_));
  nand4  g74(.a(new_n109_), .b(new_n118_), .c(new_n117_), .d(x25), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  nand2  g76(.a(x24), .b(x23), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n47_), .c(new_n120_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n119_), .c(new_n116_), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n114_), .c(new_n113_), .O(z15));
  nor2   g80(.a(new_n121_), .b(new_n47_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(x25), .c(x26), .O(new_n126_));
  inv1   g82(.a(x18), .O(new_n127_));
  nand3  g83(.a(new_n79_), .b(new_n108_), .c(new_n127_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n69_), .O(new_n129_));
  nor2   g85(.a(z08), .b(new_n96_), .O(new_n130_));
  oai21  g86(.a(new_n129_), .b(new_n126_), .c(new_n130_), .O(z16));
endmodule


