// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x06), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x05), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n45_), .O(z00));
  inv1   g11(.a(x05), .O(new_n56_));
  nand2  g12(.a(x06), .b(new_n56_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n47_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n47_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n57_), .O(z05));
  nor2   g23(.a(new_n48_), .b(new_n56_), .O(z06));
  nand2  g24(.a(z06), .b(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n57_), .O(z08));
  inv1   g28(.a(new_n50_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n45_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n58_), .c(new_n57_), .O(z09));
  and2   g33(.a(x20), .b(x19), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n45_), .c(new_n78_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n50_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n60_), .c(new_n57_), .O(z10));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  nand3  g42(.a(x26), .b(x25), .c(x24), .O(new_n87_));
  nand3  g43(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n86_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(x20), .b(x19), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n90_), .c(new_n50_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n62_), .c(new_n47_), .O(z11));
  inv1   g50(.a(new_n45_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(x14), .c(new_n52_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n85_), .b(new_n97_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n73_), .c(new_n57_), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(new_n96_), .c(new_n64_), .d(new_n47_), .O(z12));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n53_), .c(x23), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  nand2  g59(.a(new_n52_), .b(new_n103_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n102_), .c(new_n73_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n66_), .c(new_n57_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  nand2  g65(.a(x23), .b(x22), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n85_), .c(new_n109_), .O(new_n111_));
  and2   g67(.a(x24), .b(x23), .O(new_n112_));
  and2   g68(.a(x22), .b(x21), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n78_), .c(new_n112_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n111_), .c(new_n108_), .O(new_n115_));
  nor2   g71(.a(z06), .b(new_n47_), .O(new_n116_));
  oai21  g72(.a(new_n115_), .b(new_n50_), .c(new_n116_), .O(z14));
  inv1   g73(.a(x25), .O(new_n118_));
  aoi21  g74(.a(x26), .b(x17), .c(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n53_), .c(new_n112_), .O(new_n120_));
  nand2  g76(.a(new_n114_), .b(new_n118_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n120_), .c(new_n51_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n69_), .O(z15));
  nor2   g79(.a(new_n110_), .b(new_n85_), .O(new_n124_));
  nor2   g80(.a(new_n87_), .b(x18), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  inv1   g82(.a(x26), .O(new_n127_));
  nand3  g83(.a(x25), .b(x24), .c(x23), .O(new_n128_));
  oai21  g84(.a(new_n52_), .b(new_n128_), .c(new_n127_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n126_), .c(new_n73_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n71_), .c(new_n47_), .O(z16));
endmodule


