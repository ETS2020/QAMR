// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x17), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(x06), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n50_), .O(z01));
  inv1   g12(.a(new_n50_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z02));
  nand3  g15(.a(new_n50_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n50_), .O(z04));
  nand3  g19(.a(new_n50_), .b(x08), .c(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand3  g21(.a(new_n50_), .b(x08), .c(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand3  g23(.a(x17), .b(x08), .c(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand3  g25(.a(new_n50_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  and2   g27(.a(x22), .b(x21), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n52_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n48_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n55_), .c(new_n50_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  inv1   g33(.a(new_n52_), .O(new_n78_));
  nand3  g34(.a(new_n72_), .b(new_n78_), .c(x12), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n47_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n58_), .c(new_n50_), .O(z10));
  nand2  g40(.a(new_n77_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n78_), .c(new_n85_), .O(new_n87_));
  nor2   g43(.a(new_n57_), .b(new_n47_), .O(new_n88_));
  oai21  g44(.a(new_n77_), .b(x21), .c(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n87_), .c(new_n60_), .O(z11));
  inv1   g46(.a(x14), .O(new_n91_));
  inv1   g47(.a(new_n53_), .O(new_n92_));
  oai21  g48(.a(new_n52_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n85_), .b(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n93_), .c(new_n48_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n62_), .c(new_n57_), .O(z12));
  nand2  g53(.a(new_n92_), .b(x23), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  nand2  g55(.a(new_n53_), .b(new_n99_), .O(new_n100_));
  aoi22  g56(.a(new_n100_), .b(new_n98_), .c(new_n78_), .d(x15), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n51_), .c(new_n64_), .O(z13));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nand4  g60(.a(new_n72_), .b(new_n77_), .c(x24), .d(x23), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  nand2  g62(.a(new_n98_), .b(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n51_), .c(new_n66_), .O(z14));
  oai21  g65(.a(new_n49_), .b(x08), .c(x06), .O(new_n110_));
  nand3  g66(.a(x26), .b(x25), .c(x17), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  nand4  g68(.a(new_n92_), .b(x25), .c(x24), .d(x23), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n105_), .b(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n47_), .c(new_n110_), .O(z15));
  inv1   g73(.a(x26), .O(new_n118_));
  nand2  g74(.a(new_n113_), .b(new_n118_), .O(new_n119_));
  inv1   g75(.a(x18), .O(new_n120_));
  nand3  g76(.a(new_n92_), .b(new_n78_), .c(new_n120_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(new_n88_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n70_), .O(z16));
endmodule


