// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand4  g02(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n52_), .c(new_n48_), .O(new_n55_));
  oai21  g11(.a(new_n46_), .b(new_n45_), .c(new_n55_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(x10), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(x10), .O(z02));
  oai21  g16(.a(x10), .b(x02), .c(x08), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x10), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(x10), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x10), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(x10), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  aoi21  g27(.a(new_n46_), .b(new_n71_), .c(new_n45_), .O(z08));
  oai21  g28(.a(x10), .b(x00), .c(x08), .O(new_n73_));
  inv1   g29(.a(x19), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n48_), .c(new_n74_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n51_), .c(new_n73_), .O(z09));
  oai21  g34(.a(x10), .b(x01), .c(x08), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n47_), .c(new_n81_), .O(new_n83_));
  nand2  g39(.a(new_n80_), .b(new_n74_), .O(new_n84_));
  nand3  g40(.a(new_n84_), .b(new_n83_), .c(new_n52_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n79_), .O(z10));
  nand2  g42(.a(new_n81_), .b(x21), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n48_), .c(new_n87_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  oai21  g46(.a(new_n80_), .b(new_n74_), .c(new_n90_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n52_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n89_), .c(new_n61_), .O(z11));
  oai21  g49(.a(x10), .b(x03), .c(x08), .O(new_n94_));
  aoi21  g50(.a(new_n48_), .b(x14), .c(new_n53_), .O(new_n95_));
  inv1   g51(.a(x22), .O(new_n96_));
  nand2  g52(.a(new_n87_), .b(new_n96_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n52_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n95_), .c(new_n94_), .O(z12));
  oai21  g55(.a(x10), .b(x04), .c(x08), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  xor2a  g57(.a(new_n53_), .b(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n48_), .b(x15), .c(new_n102_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n51_), .c(new_n100_), .O(z13));
  oai21  g60(.a(x10), .b(x05), .c(x08), .O(new_n105_));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  nor2   g63(.a(new_n53_), .b(new_n101_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(x24), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n53_), .b(new_n101_), .c(new_n110_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n51_), .c(new_n105_), .O(z14));
  oai21  g69(.a(x10), .b(x06), .c(x08), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  aoi21  g71(.a(x26), .b(x17), .c(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n108_), .c(x24), .O(new_n117_));
  nand2  g73(.a(new_n109_), .b(new_n115_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n117_), .c(new_n52_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n114_), .O(z15));
  nand2  g76(.a(x08), .b(x07), .O(new_n121_));
  inv1   g77(.a(x26), .O(new_n122_));
  nor2   g78(.a(new_n115_), .b(new_n110_), .O(new_n123_));
  nand3  g79(.a(new_n54_), .b(new_n123_), .c(x23), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g81(.a(x09), .b(new_n45_), .O(new_n126_));
  nand3  g82(.a(x26), .b(x25), .c(x24), .O(new_n127_));
  nor2   g83(.a(new_n127_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n108_), .c(new_n126_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n121_), .c(x10), .O(z16));
endmodule


