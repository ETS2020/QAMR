// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n63_, new_n65_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_;
  nor2   g00(.a(x09), .b(x05), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  oai21  g10(.a(new_n54_), .b(new_n49_), .c(new_n46_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  nor2   g12(.a(new_n45_), .b(new_n47_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n56_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  oai21  g16(.a(new_n47_), .b(new_n60_), .c(new_n46_), .O(z02));
  and2   g17(.a(new_n57_), .b(x02), .O(z03));
  aoi21  g18(.a(x08), .b(x03), .c(new_n45_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  oai21  g21(.a(new_n47_), .b(new_n65_), .c(new_n46_), .O(z05));
  and2   g22(.a(x08), .b(x05), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n46_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z08));
  inv1   g27(.a(new_n49_), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n50_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n75_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n50_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n49_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(new_n58_), .b(new_n60_), .c(new_n82_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n51_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n77_), .b(x21), .c(new_n72_), .O(new_n87_));
  aoi21  g43(.a(x08), .b(x02), .c(new_n45_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(z11));
  aoi21  g45(.a(new_n51_), .b(x14), .c(new_n52_), .O(new_n90_));
  inv1   g46(.a(new_n84_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x22), .c(new_n72_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n90_), .c(new_n63_), .O(z12));
  inv1   g49(.a(x15), .O(new_n94_));
  nor2   g50(.a(new_n50_), .b(new_n94_), .O(new_n95_));
  inv1   g51(.a(x23), .O(new_n96_));
  xor2a  g52(.a(new_n52_), .b(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n95_), .c(new_n72_), .O(new_n98_));
  oai21  g54(.a(new_n58_), .b(new_n65_), .c(new_n98_), .O(z13));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  and2   g57(.a(x22), .b(x21), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n77_), .c(x24), .d(x23), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n52_), .b(new_n96_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n103_), .c(new_n101_), .O(new_n106_));
  nor2   g62(.a(z06), .b(new_n45_), .O(new_n107_));
  oai21  g63(.a(new_n106_), .b(new_n49_), .c(new_n107_), .O(z14));
  inv1   g64(.a(x25), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n104_), .O(new_n110_));
  nor2   g66(.a(new_n52_), .b(new_n96_), .O(new_n111_));
  nand2  g67(.a(x26), .b(x17), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n111_), .c(new_n110_), .O(new_n113_));
  nand2  g69(.a(new_n103_), .b(new_n109_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n72_), .O(new_n115_));
  nand2  g71(.a(new_n57_), .b(x06), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n115_), .O(z15));
  aoi21  g73(.a(new_n111_), .b(new_n110_), .c(x26), .O(new_n118_));
  oai21  g74(.a(new_n54_), .b(x18), .c(new_n72_), .O(new_n119_));
  nand2  g75(.a(new_n57_), .b(x07), .O(new_n120_));
  oai21  g76(.a(new_n119_), .b(new_n118_), .c(new_n120_), .O(z16));
endmodule


