// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  oai21  g07(.a(x10), .b(x00), .c(x08), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(z01));
  nand2  g09(.a(x08), .b(x01), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(x10), .O(z02));
  nand2  g11(.a(x08), .b(x02), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(x10), .O(z03));
  oai21  g13(.a(x10), .b(x03), .c(x08), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z04));
  nand2  g15(.a(x08), .b(x04), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(x10), .O(z05));
  oai21  g17(.a(x10), .b(x05), .c(x08), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(x10), .O(z07));
  nand2  g21(.a(x08), .b(x07), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(x10), .O(z08));
  inv1   g23(.a(x19), .O(new_n68_));
  inv1   g24(.a(new_n45_), .O(new_n69_));
  nand4  g25(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(new_n71_));
  aoi21  g27(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n49_), .c(new_n52_), .O(z09));
  oai21  g29(.a(x10), .b(x01), .c(x08), .O(new_n74_));
  inv1   g30(.a(new_n49_), .O(new_n75_));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n45_), .c(new_n77_), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n68_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n79_), .c(new_n75_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n74_), .O(z10));
  nand2  g39(.a(new_n77_), .b(x21), .O(new_n84_));
  nand2  g40(.a(x22), .b(x13), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n45_), .O(new_n86_));
  inv1   g42(.a(x21), .O(new_n87_));
  inv1   g43(.a(x08), .O(new_n88_));
  nand2  g44(.a(x09), .b(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n76_), .b(new_n87_), .c(new_n89_), .O(new_n90_));
  oai21  g46(.a(new_n86_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n56_), .c(x10), .O(z11));
  aoi21  g48(.a(new_n69_), .b(x14), .c(new_n50_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n84_), .b(new_n94_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n75_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n93_), .c(new_n58_), .O(z12));
  oai21  g53(.a(x10), .b(x04), .c(x08), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n50_), .b(new_n99_), .O(new_n100_));
  aoi21  g56(.a(new_n69_), .b(x15), .c(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n49_), .c(new_n98_), .O(z13));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nor2   g60(.a(new_n50_), .b(new_n99_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(x24), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n50_), .b(new_n99_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n49_), .c(new_n62_), .O(z14));
  oai21  g66(.a(x10), .b(x06), .c(x08), .O(new_n111_));
  inv1   g67(.a(x25), .O(new_n112_));
  aoi21  g68(.a(x26), .b(x17), .c(new_n112_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n105_), .c(x24), .O(new_n114_));
  nand2  g70(.a(new_n106_), .b(new_n112_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n75_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n111_), .O(z15));
  inv1   g73(.a(x26), .O(new_n118_));
  nor2   g74(.a(new_n112_), .b(new_n107_), .O(new_n119_));
  inv1   g75(.a(new_n50_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(x23), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand3  g78(.a(x26), .b(x25), .c(x24), .O(new_n123_));
  nor2   g79(.a(new_n123_), .b(x18), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n105_), .c(new_n89_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n66_), .c(x10), .O(z16));
endmodule


