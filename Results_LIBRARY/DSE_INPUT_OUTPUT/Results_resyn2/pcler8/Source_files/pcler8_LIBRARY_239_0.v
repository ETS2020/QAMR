// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nor2   g00(.a(x11), .b(x01), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  and2   g05(.a(x26), .b(x25), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n50_), .c(new_n49_), .d(x24), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  aoi21  g12(.a(x08), .b(x00), .c(new_n45_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n46_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n46_), .O(z05));
  nand3  g22(.a(new_n46_), .b(x08), .c(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  aoi21  g24(.a(x08), .b(x06), .c(new_n45_), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n45_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  nand2  g29(.a(x22), .b(x21), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g32(.a(x11), .O(new_n77_));
  inv1   g33(.a(x20), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n53_), .c(new_n57_), .O(z09));
  nand2  g37(.a(x20), .b(x19), .O(new_n82_));
  aoi21  g38(.a(new_n76_), .b(x12), .c(new_n82_), .O(new_n83_));
  inv1   g39(.a(x01), .O(new_n84_));
  oai21  g40(.a(new_n77_), .b(x08), .c(new_n84_), .O(new_n85_));
  nand2  g41(.a(new_n78_), .b(new_n73_), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n85_), .c(new_n52_), .d(x09), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n83_), .c(new_n59_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand2  g46(.a(x22), .b(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n75_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  aoi21  g49(.a(new_n82_), .b(new_n93_), .c(new_n53_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n61_), .c(new_n45_), .O(z11));
  inv1   g52(.a(new_n75_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(x14), .c(new_n48_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n89_), .b(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n54_), .c(new_n46_), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(new_n98_), .c(new_n63_), .d(new_n45_), .O(z12));
  nand3  g58(.a(new_n50_), .b(x24), .c(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n49_), .O(new_n104_));
  aoi21  g60(.a(new_n48_), .b(new_n47_), .c(new_n53_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n65_), .c(new_n46_), .O(z13));
  nand2  g63(.a(new_n54_), .b(new_n46_), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n48_), .b(new_n47_), .c(new_n111_), .O(new_n112_));
  and2   g68(.a(x20), .b(x19), .O(new_n113_));
  and2   g69(.a(x22), .b(x21), .O(new_n114_));
  and2   g70(.a(x24), .b(x23), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n112_), .c(new_n110_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n108_), .c(new_n67_), .O(z14));
  nand3  g74(.a(x26), .b(x25), .c(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  nand4  g76(.a(new_n115_), .b(new_n114_), .c(new_n113_), .d(x25), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  nand2  g78(.a(x24), .b(x23), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n48_), .c(new_n122_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n121_), .c(new_n120_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n53_), .c(new_n69_), .O(z15));
  nor2   g82(.a(new_n123_), .b(new_n48_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand2  g85(.a(new_n50_), .b(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n116_), .c(new_n54_), .O(new_n131_));
  aoi21  g87(.a(x08), .b(x07), .c(new_n45_), .O(new_n132_));
  oai21  g88(.a(new_n131_), .b(new_n128_), .c(new_n132_), .O(z16));
endmodule


