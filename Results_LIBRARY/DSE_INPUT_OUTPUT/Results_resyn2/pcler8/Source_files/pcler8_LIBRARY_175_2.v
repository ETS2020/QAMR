// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  nand3  g00(.a(x24), .b(x23), .c(x22), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x25), .O(new_n47_));
  inv1   g03(.a(x26), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x20), .O(new_n50_));
  inv1   g06(.a(x21), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n49_), .c(new_n46_), .O(new_n53_));
  inv1   g09(.a(x08), .O(new_n54_));
  inv1   g10(.a(x10), .O(new_n55_));
  nand4  g11(.a(x19), .b(new_n55_), .c(x09), .d(new_n54_), .O(new_n56_));
  inv1   g12(.a(x13), .O(new_n57_));
  nor2   g13(.a(x17), .b(new_n57_), .O(new_n58_));
  nor3   g14(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z00));
  nand2  g15(.a(x08), .b(x00), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n58_), .O(z01));
  inv1   g17(.a(new_n58_), .O(new_n62_));
  nand2  g18(.a(x08), .b(x01), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n62_), .O(z02));
  and2   g20(.a(x08), .b(x02), .O(new_n65_));
  and2   g21(.a(new_n65_), .b(new_n62_), .O(z03));
  nand3  g22(.a(new_n62_), .b(x08), .c(x03), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z04));
  nand2  g24(.a(x08), .b(x04), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n62_), .O(z05));
  nand2  g26(.a(x08), .b(x05), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n58_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n58_), .O(z07));
  nand2  g30(.a(x08), .b(x07), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n58_), .O(z08));
  nand3  g32(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  nand4  g35(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n79_), .c(x19), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  nand3  g38(.a(new_n82_), .b(new_n60_), .c(new_n62_), .O(z09));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  nand2  g40(.a(x23), .b(x22), .O(new_n85_));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand2  g42(.a(x21), .b(x12), .O(new_n87_));
  nor3   g43(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  inv1   g44(.a(x19), .O(new_n89_));
  aoi21  g45(.a(new_n50_), .b(new_n89_), .c(new_n77_), .O(new_n90_));
  oai21  g46(.a(new_n88_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n63_), .c(new_n58_), .O(z10));
  inv1   g48(.a(x17), .O(new_n93_));
  nor2   g49(.a(new_n86_), .b(new_n85_), .O(new_n94_));
  nor2   g50(.a(new_n56_), .b(new_n50_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  nand2  g52(.a(new_n84_), .b(new_n51_), .O(new_n97_));
  nand3  g53(.a(x21), .b(x20), .c(x19), .O(new_n98_));
  inv1   g54(.a(new_n98_), .O(new_n99_));
  nor2   g55(.a(new_n99_), .b(new_n77_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n97_), .c(new_n65_), .O(new_n101_));
  oai21  g57(.a(new_n96_), .b(new_n57_), .c(new_n101_), .O(z11));
  inv1   g58(.a(new_n79_), .O(new_n103_));
  nand4  g59(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n104_));
  aoi21  g60(.a(new_n103_), .b(x14), .c(new_n104_), .O(new_n105_));
  nor2   g61(.a(new_n58_), .b(new_n77_), .O(new_n106_));
  oai21  g62(.a(new_n99_), .b(x22), .c(new_n106_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n105_), .c(new_n67_), .O(z12));
  inv1   g64(.a(x23), .O(new_n109_));
  nand2  g65(.a(new_n104_), .b(new_n109_), .O(new_n110_));
  inv1   g66(.a(x15), .O(new_n111_));
  nor2   g67(.a(new_n98_), .b(new_n85_), .O(new_n112_));
  oai21  g68(.a(new_n86_), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n110_), .c(new_n78_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n69_), .c(new_n62_), .O(z13));
  inv1   g71(.a(x24), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(new_n109_), .O(new_n117_));
  inv1   g73(.a(new_n104_), .O(new_n118_));
  nand2  g74(.a(new_n49_), .b(x16), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  oai21  g76(.a(new_n98_), .b(new_n85_), .c(new_n116_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n120_), .c(new_n78_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n71_), .c(new_n62_), .O(z14));
  nand2  g79(.a(new_n118_), .b(new_n117_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n47_), .O(new_n125_));
  nand3  g81(.a(new_n118_), .b(new_n117_), .c(x25), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n125_), .c(new_n106_), .O(new_n127_));
  nand2  g83(.a(new_n49_), .b(x17), .O(new_n128_));
  inv1   g84(.a(new_n128_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n78_), .c(z07), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n127_), .O(z15));
  nand2  g87(.a(new_n126_), .b(new_n48_), .O(new_n132_));
  nor2   g88(.a(new_n89_), .b(x18), .O(new_n133_));
  nand4  g89(.a(new_n133_), .b(new_n52_), .c(new_n49_), .d(new_n46_), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n132_), .c(new_n78_), .O(new_n135_));
  nand3  g91(.a(new_n135_), .b(new_n75_), .c(new_n62_), .O(z16));
endmodule


