// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x07), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  nand2  g06(.a(x09), .b(new_n50_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x10), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x07), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n55_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n57_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n57_), .O(z07));
  aoi21  g27(.a(x10), .b(new_n50_), .c(new_n45_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand4  g31(.a(new_n56_), .b(x09), .c(new_n50_), .d(new_n45_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g34(.a(new_n58_), .b(new_n55_), .c(new_n78_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n73_), .c(new_n81_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n51_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(x07), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(x10), .c(new_n60_), .O(z10));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  nand3  g45(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n49_), .c(new_n89_), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  aoi21  g48(.a(new_n80_), .b(new_n92_), .c(new_n51_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(x07), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x10), .c(new_n62_), .O(z11));
  inv1   g51(.a(new_n73_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(x14), .c(new_n47_), .O(new_n97_));
  oai21  g53(.a(new_n89_), .b(x22), .c(new_n77_), .O(new_n98_));
  oai22  g54(.a(new_n98_), .b(new_n97_), .c(new_n58_), .d(new_n64_), .O(z12));
  inv1   g55(.a(x15), .O(new_n100_));
  oai21  g56(.a(new_n49_), .b(new_n100_), .c(new_n48_), .O(new_n101_));
  aoi21  g57(.a(new_n47_), .b(new_n46_), .c(new_n76_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  inv1   g59(.a(new_n58_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(x04), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n103_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n47_), .b(new_n46_), .c(new_n109_), .O(new_n110_));
  inv1   g66(.a(new_n47_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x24), .c(x23), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  nand2  g69(.a(new_n104_), .b(x05), .O(new_n114_));
  oai21  g70(.a(new_n113_), .b(new_n76_), .c(new_n114_), .O(z14));
  nand2  g71(.a(x24), .b(x23), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(new_n47_), .O(new_n117_));
  nand2  g73(.a(x26), .b(x17), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n117_), .c(x25), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  nand2  g76(.a(new_n112_), .b(new_n120_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(new_n77_), .O(new_n122_));
  nand3  g78(.a(new_n57_), .b(x08), .c(x06), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n122_), .O(z15));
  aoi21  g80(.a(new_n117_), .b(x25), .c(x26), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand3  g82(.a(new_n96_), .b(new_n111_), .c(new_n126_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n77_), .O(new_n128_));
  nand3  g84(.a(x10), .b(x08), .c(x07), .O(new_n129_));
  oai21  g85(.a(new_n128_), .b(new_n125_), .c(new_n129_), .O(z16));
endmodule


