// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x05), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  inv1   g09(.a(x23), .O(new_n54_));
  nand4  g10(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n53_), .c(new_n51_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n47_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z02));
  nand3  g18(.a(new_n47_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n47_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand3  g24(.a(x17), .b(x08), .c(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n46_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n47_), .O(z08));
  inv1   g30(.a(z01), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g35(.a(x20), .b(x11), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n50_), .c(new_n75_), .O(z09));
  inv1   g38(.a(x20), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n76_), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  inv1   g41(.a(x12), .O(new_n86_));
  nor3   g42(.a(new_n78_), .b(new_n77_), .c(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n83_), .b(new_n76_), .c(new_n50_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n85_), .c(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n61_), .c(new_n46_), .O(z10));
  inv1   g46(.a(new_n78_), .O(new_n91_));
  nand2  g47(.a(new_n84_), .b(x21), .O(new_n92_));
  and2   g48(.a(x22), .b(x13), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  nor2   g50(.a(new_n50_), .b(new_n46_), .O(new_n95_));
  oai21  g51(.a(new_n84_), .b(x21), .c(new_n95_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n63_), .O(z11));
  inv1   g53(.a(z04), .O(new_n98_));
  aoi21  g54(.a(new_n91_), .b(x14), .c(new_n55_), .O(new_n99_));
  inv1   g55(.a(new_n92_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(x22), .c(new_n51_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(z12));
  inv1   g58(.a(x15), .O(new_n103_));
  nor2   g59(.a(new_n78_), .b(new_n103_), .O(new_n104_));
  xor2a  g60(.a(new_n55_), .b(new_n54_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n104_), .c(new_n51_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n67_), .c(new_n47_), .O(z13));
  inv1   g63(.a(x17), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(x08), .c(x05), .O(new_n109_));
  inv1   g65(.a(x25), .O(new_n110_));
  inv1   g66(.a(x26), .O(new_n111_));
  nor2   g67(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(x16), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n56_), .c(x24), .O(new_n114_));
  or2    g70(.a(new_n56_), .b(x24), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n51_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n109_), .O(z14));
  inv1   g73(.a(new_n55_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(x25), .c(x24), .d(x23), .O(new_n119_));
  nand2  g75(.a(new_n56_), .b(x24), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n110_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(new_n95_), .O(new_n122_));
  nand2  g78(.a(new_n112_), .b(x17), .O(new_n123_));
  inv1   g79(.a(new_n123_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n51_), .c(z07), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n122_), .O(z15));
  nand2  g82(.a(new_n119_), .b(new_n111_), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand3  g84(.a(new_n56_), .b(new_n53_), .c(new_n128_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n127_), .c(new_n51_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n73_), .c(new_n46_), .O(z16));
endmodule


