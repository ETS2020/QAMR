// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(x24), .b(x23), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(new_n47_), .c(x25), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x14), .c(new_n51_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g11(.a(x14), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n51_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  nand2  g16(.a(new_n57_), .b(x08), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n57_), .O(z03));
  inv1   g20(.a(x03), .O(new_n65_));
  nor2   g21(.a(new_n61_), .b(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n57_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n57_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n57_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n57_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n53_), .c(new_n56_), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(x10), .c(new_n58_), .O(z09));
  nand2  g35(.a(x08), .b(x01), .O(new_n80_));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n75_), .c(new_n82_), .O(new_n84_));
  inv1   g40(.a(x08), .O(new_n85_));
  nand2  g41(.a(x09), .b(new_n85_), .O(new_n86_));
  nor2   g42(.a(x20), .b(x19), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n84_), .c(new_n56_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(x10), .c(new_n80_), .O(z10));
  nand3  g46(.a(x21), .b(x20), .c(x19), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  nand2  g48(.a(x23), .b(x22), .O(new_n93_));
  nand4  g49(.a(x26), .b(x25), .c(x24), .d(x13), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n92_), .O(new_n95_));
  inv1   g51(.a(x21), .O(new_n96_));
  aoi21  g52(.a(new_n81_), .b(new_n96_), .c(new_n86_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n95_), .c(new_n56_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x10), .c(new_n63_), .O(z11));
  inv1   g55(.a(new_n75_), .O(new_n100_));
  nor2   g56(.a(new_n100_), .b(new_n46_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n91_), .b(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(new_n53_), .c(x14), .d(new_n51_), .O(new_n104_));
  oai22  g60(.a(new_n104_), .b(new_n101_), .c(new_n61_), .d(new_n65_), .O(z12));
  nor2   g61(.a(new_n93_), .b(new_n91_), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  inv1   g64(.a(x23), .O(new_n109_));
  aoi21  g65(.a(new_n46_), .b(new_n109_), .c(new_n86_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n56_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(x10), .c(new_n67_), .O(z13));
  inv1   g68(.a(new_n54_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n93_), .b(new_n91_), .c(new_n114_), .O(new_n115_));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n106_), .c(x24), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  inv1   g74(.a(new_n61_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(x05), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n118_), .O(z14));
  inv1   g77(.a(x25), .O(new_n122_));
  oai21  g78(.a(new_n48_), .b(new_n46_), .c(new_n122_), .O(new_n123_));
  aoi21  g79(.a(x26), .b(x17), .c(new_n122_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n49_), .c(new_n47_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n113_), .O(new_n126_));
  nand2  g82(.a(new_n119_), .b(x06), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n126_), .O(z15));
  nand2  g84(.a(new_n50_), .b(new_n45_), .O(new_n129_));
  inv1   g85(.a(x18), .O(new_n130_));
  nand3  g86(.a(new_n100_), .b(new_n47_), .c(new_n130_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n129_), .c(new_n113_), .O(new_n132_));
  nand2  g88(.a(new_n119_), .b(x07), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n132_), .O(z16));
endmodule


