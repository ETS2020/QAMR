// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nor2   g04(.a(x17), .b(x15), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nand3  g08(.a(x22), .b(x21), .c(x20), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n51_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n49_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n50_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n49_), .O(z03));
  aoi21  g18(.a(x08), .b(x03), .c(new_n49_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand3  g20(.a(new_n50_), .b(x08), .c(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n49_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n49_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n50_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  aoi21  g29(.a(new_n54_), .b(x11), .c(new_n73_), .O(new_n74_));
  oai22  g30(.a(new_n74_), .b(new_n51_), .c(new_n57_), .d(new_n49_), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n52_), .c(new_n76_), .O(new_n78_));
  nor2   g34(.a(x20), .b(x19), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n47_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n59_), .c(new_n49_), .O(z10));
  inv1   g38(.a(new_n52_), .O(new_n83_));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n76_), .b(x21), .c(new_n48_), .O(new_n87_));
  aoi21  g43(.a(x08), .b(x02), .c(new_n49_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(z11));
  inv1   g45(.a(new_n84_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x22), .O(new_n91_));
  aoi21  g47(.a(new_n83_), .b(x14), .c(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n90_), .b(x22), .c(new_n48_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n92_), .c(new_n63_), .O(z12));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(x23), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  nand2  g54(.a(new_n95_), .b(new_n98_), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n97_), .c(new_n50_), .d(new_n48_), .O(new_n100_));
  nand3  g56(.a(new_n83_), .b(new_n48_), .c(x15), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n100_), .c(new_n65_), .O(z13));
  inv1   g58(.a(x24), .O(new_n103_));
  oai21  g59(.a(new_n95_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  and2   g60(.a(x24), .b(x23), .O(new_n105_));
  nand2  g61(.a(new_n96_), .b(new_n105_), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n106_), .b(new_n104_), .c(new_n108_), .O(new_n109_));
  aoi21  g65(.a(x08), .b(x05), .c(new_n49_), .O(new_n110_));
  oai21  g66(.a(new_n109_), .b(new_n47_), .c(new_n110_), .O(z14));
  nand3  g67(.a(x26), .b(x25), .c(x17), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  and2   g69(.a(x22), .b(x21), .O(new_n114_));
  nand4  g70(.a(new_n76_), .b(new_n114_), .c(new_n105_), .d(x25), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand2  g72(.a(x24), .b(x23), .O(new_n117_));
  oai21  g73(.a(new_n95_), .b(new_n117_), .c(new_n116_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n115_), .c(new_n113_), .O(new_n119_));
  aoi21  g75(.a(x08), .b(x06), .c(new_n49_), .O(new_n120_));
  oai21  g76(.a(new_n119_), .b(new_n47_), .c(new_n120_), .O(z15));
  inv1   g77(.a(x18), .O(new_n122_));
  nand3  g78(.a(new_n54_), .b(x19), .c(new_n122_), .O(new_n123_));
  inv1   g79(.a(x26), .O(new_n124_));
  nand2  g80(.a(new_n115_), .b(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n48_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n71_), .c(new_n50_), .O(z16));
endmodule


