// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_;
  nand2  g00(.a(x11), .b(x10), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n45_), .O(z00));
  nand2  g10(.a(new_n45_), .b(x08), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  oai21  g17(.a(new_n46_), .b(new_n61_), .c(new_n45_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  oai21  g19(.a(new_n46_), .b(new_n63_), .c(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n45_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n45_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(z08));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n50_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n57_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  inv1   g33(.a(new_n50_), .O(new_n78_));
  and2   g34(.a(x22), .b(x21), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n78_), .c(x12), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n48_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n59_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n78_), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(new_n77_), .b(x21), .c(new_n49_), .O(new_n89_));
  oai22  g45(.a(new_n89_), .b(new_n88_), .c(new_n55_), .d(new_n61_), .O(z11));
  aoi21  g46(.a(new_n78_), .b(x14), .c(new_n51_), .O(new_n91_));
  inv1   g47(.a(new_n86_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n49_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n91_), .c(new_n55_), .d(new_n63_), .O(z12));
  inv1   g50(.a(x15), .O(new_n95_));
  nor2   g51(.a(new_n50_), .b(new_n95_), .O(new_n96_));
  inv1   g52(.a(x23), .O(new_n97_));
  xor2a  g53(.a(new_n51_), .b(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n49_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n65_), .c(new_n45_), .O(z13));
  inv1   g56(.a(z06), .O(new_n101_));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n79_), .b(new_n77_), .c(x24), .d(x23), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n51_), .b(new_n97_), .c(new_n105_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n48_), .c(new_n101_), .O(z14));
  inv1   g64(.a(z07), .O(new_n109_));
  nor2   g65(.a(new_n51_), .b(new_n97_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(x24), .c(x25), .O(new_n111_));
  and2   g67(.a(x26), .b(x17), .O(new_n112_));
  and2   g68(.a(x25), .b(x24), .O(new_n113_));
  nand4  g69(.a(new_n79_), .b(new_n77_), .c(new_n113_), .d(x23), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n112_), .c(new_n49_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n111_), .c(new_n109_), .O(z15));
  inv1   g72(.a(x18), .O(new_n117_));
  nand2  g73(.a(new_n52_), .b(new_n117_), .O(new_n118_));
  inv1   g74(.a(x26), .O(new_n119_));
  nand2  g75(.a(new_n114_), .b(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n118_), .c(new_n49_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n71_), .c(new_n45_), .O(z16));
endmodule


