// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  oai21  g10(.a(new_n46_), .b(new_n45_), .c(new_n54_), .O(z00));
  oai21  g11(.a(x10), .b(x00), .c(x08), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  aoi21  g14(.a(new_n46_), .b(new_n58_), .c(new_n45_), .O(z02));
  oai21  g15(.a(x10), .b(x02), .c(x08), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(x10), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  aoi21  g20(.a(new_n46_), .b(new_n64_), .c(new_n45_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(x10), .O(z06));
  oai21  g23(.a(x10), .b(x06), .c(x08), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(x10), .O(z08));
  inv1   g27(.a(x19), .O(new_n72_));
  inv1   g28(.a(new_n51_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n49_), .c(new_n56_), .O(z09));
  nand2  g33(.a(x08), .b(x01), .O(new_n78_));
  inv1   g34(.a(x20), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n72_), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n51_), .c(new_n80_), .O(new_n82_));
  nand2  g38(.a(new_n79_), .b(new_n72_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n82_), .c(new_n48_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n78_), .c(x10), .O(z10));
  nand2  g41(.a(new_n80_), .b(x21), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n73_), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(new_n80_), .b(x21), .c(new_n50_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n88_), .c(new_n60_), .O(z11));
  inv1   g46(.a(x14), .O(new_n91_));
  inv1   g47(.a(new_n52_), .O(new_n92_));
  oai21  g48(.a(new_n51_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n93_), .c(new_n48_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n62_), .c(x10), .O(z12));
  nand2  g53(.a(x08), .b(x04), .O(new_n98_));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n51_), .b(new_n99_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  xor2a  g57(.a(new_n52_), .b(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n48_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n98_), .c(x10), .O(z13));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nor2   g62(.a(new_n52_), .b(new_n101_), .O(new_n107_));
  xor2a  g63(.a(new_n107_), .b(x24), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n106_), .c(new_n48_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n66_), .c(x10), .O(z14));
  nand3  g66(.a(x26), .b(x25), .c(x17), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  nand3  g68(.a(new_n107_), .b(x25), .c(x24), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand3  g70(.a(new_n92_), .b(x24), .c(x23), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n113_), .c(new_n112_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n49_), .c(new_n68_), .O(z15));
  oai21  g74(.a(x10), .b(x07), .c(x08), .O(new_n119_));
  inv1   g75(.a(new_n113_), .O(new_n120_));
  inv1   g76(.a(x18), .O(new_n121_));
  aoi21  g77(.a(new_n53_), .b(new_n121_), .c(new_n49_), .O(new_n122_));
  oai21  g78(.a(new_n120_), .b(x26), .c(new_n122_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n119_), .O(z16));
endmodule


