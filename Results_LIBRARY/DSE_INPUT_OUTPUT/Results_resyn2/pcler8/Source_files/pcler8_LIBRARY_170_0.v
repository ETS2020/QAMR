// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_;
  nor2   g00(.a(x17), .b(x09), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  oai21  g10(.a(new_n54_), .b(new_n49_), .c(new_n46_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  nor2   g12(.a(new_n45_), .b(new_n47_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n56_), .O(z01));
  and2   g15(.a(new_n57_), .b(x01), .O(z02));
  and2   g16(.a(new_n57_), .b(x02), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  nor2   g18(.a(new_n58_), .b(new_n62_), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z05));
  nand2  g21(.a(new_n57_), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand2  g23(.a(new_n57_), .b(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z08));
  inv1   g27(.a(new_n49_), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n50_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  oai21  g31(.a(new_n58_), .b(new_n56_), .c(new_n75_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  and2   g33(.a(x22), .b(x21), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n51_), .c(x12), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n49_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  aoi21  g39(.a(x08), .b(x01), .c(new_n45_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n51_), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(new_n77_), .b(x21), .c(new_n72_), .O(new_n89_));
  aoi21  g45(.a(x08), .b(x02), .c(new_n45_), .O(new_n90_));
  oai21  g46(.a(new_n89_), .b(new_n88_), .c(new_n90_), .O(z11));
  aoi21  g47(.a(new_n51_), .b(x14), .c(new_n52_), .O(new_n92_));
  inv1   g48(.a(new_n86_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(x22), .c(new_n72_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(new_n92_), .c(new_n58_), .d(new_n62_), .O(z12));
  inv1   g51(.a(x15), .O(new_n96_));
  nor2   g52(.a(new_n50_), .b(new_n96_), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  xor2a  g54(.a(new_n52_), .b(new_n98_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n72_), .O(new_n100_));
  oai21  g56(.a(new_n58_), .b(new_n64_), .c(new_n100_), .O(z13));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n78_), .b(new_n77_), .c(x24), .d(x23), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n52_), .b(new_n98_), .c(new_n105_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n104_), .c(new_n103_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n49_), .c(new_n66_), .O(z14));
  nor2   g64(.a(new_n52_), .b(new_n98_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(x24), .c(x25), .O(new_n110_));
  and2   g66(.a(x25), .b(x24), .O(new_n111_));
  nand2  g67(.a(x26), .b(x17), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n109_), .c(new_n111_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n72_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n110_), .c(new_n68_), .O(z15));
  aoi21  g71(.a(new_n109_), .b(new_n111_), .c(x26), .O(new_n116_));
  oai21  g72(.a(new_n54_), .b(x18), .c(new_n72_), .O(new_n117_));
  nand2  g73(.a(new_n57_), .b(x07), .O(new_n118_));
  oai21  g74(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(z16));
endmodule


