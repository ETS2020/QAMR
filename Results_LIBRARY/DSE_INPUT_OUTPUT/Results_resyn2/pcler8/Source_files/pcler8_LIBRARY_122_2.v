// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  and2   g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(z00));
  inv1   g10(.a(x22), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x10), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  nand2  g17(.a(new_n56_), .b(x08), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n62_), .b(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n56_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n56_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  nor2   g26(.a(new_n62_), .b(new_n70_), .O(z07));
  nand3  g27(.a(new_n56_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(new_n52_), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n57_), .c(new_n56_), .O(z09));
  inv1   g35(.a(x19), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n75_), .c(new_n82_), .O(new_n84_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n84_), .c(new_n74_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n59_), .c(new_n56_), .O(z10));
  nand3  g43(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n53_), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n46_), .O(new_n90_));
  inv1   g46(.a(x21), .O(new_n91_));
  oai21  g47(.a(new_n81_), .b(new_n80_), .c(new_n91_), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(new_n51_), .c(new_n49_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n90_), .c(new_n62_), .d(new_n61_), .O(z11));
  inv1   g50(.a(new_n75_), .O(new_n95_));
  nand2  g51(.a(new_n47_), .b(x22), .O(new_n96_));
  aoi21  g52(.a(new_n95_), .b(x14), .c(new_n96_), .O(new_n97_));
  nand2  g53(.a(new_n46_), .b(new_n55_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n51_), .c(new_n49_), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(new_n97_), .c(new_n62_), .d(new_n64_), .O(z12));
  inv1   g56(.a(x08), .O(new_n101_));
  nand2  g57(.a(x09), .b(new_n101_), .O(new_n102_));
  inv1   g58(.a(z05), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nor2   g60(.a(new_n104_), .b(x22), .O(new_n105_));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n106_));
  nand4  g62(.a(x23), .b(x21), .c(x20), .d(x19), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g65(.a(x22), .b(new_n49_), .O(new_n110_));
  aoi21  g66(.a(new_n46_), .b(new_n104_), .c(new_n110_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n105_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n102_), .c(new_n103_), .O(z13));
  inv1   g69(.a(x24), .O(new_n114_));
  nand2  g70(.a(new_n48_), .b(new_n114_), .O(new_n115_));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n108_), .c(x24), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n74_), .O(new_n118_));
  oai21  g74(.a(new_n102_), .b(new_n114_), .c(new_n49_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n55_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n118_), .c(new_n68_), .O(z14));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(x24), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n107_), .c(x25), .O(new_n124_));
  inv1   g80(.a(x25), .O(new_n125_));
  nand4  g81(.a(new_n47_), .b(new_n45_), .c(new_n125_), .d(x24), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n74_), .O(new_n128_));
  oai21  g84(.a(new_n102_), .b(new_n125_), .c(new_n49_), .O(new_n129_));
  aoi22  g85(.a(new_n129_), .b(new_n55_), .c(x08), .d(x06), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n128_), .O(z15));
  inv1   g87(.a(x18), .O(new_n132_));
  nand4  g88(.a(new_n95_), .b(new_n47_), .c(x22), .d(new_n132_), .O(new_n133_));
  inv1   g89(.a(x26), .O(new_n134_));
  nand2  g90(.a(x25), .b(x24), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(new_n48_), .c(new_n134_), .O(new_n136_));
  nand3  g92(.a(new_n136_), .b(new_n133_), .c(new_n74_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n72_), .O(z16));
endmodule


