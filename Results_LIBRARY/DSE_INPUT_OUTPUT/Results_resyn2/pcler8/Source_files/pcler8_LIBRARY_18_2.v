// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n45_));
  nand3  g01(.a(x26), .b(x25), .c(x24), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(new_n47_), .c(x09), .O(new_n51_));
  aoi21  g07(.a(new_n51_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g08(.a(x10), .O(new_n53_));
  oai21  g09(.a(new_n53_), .b(x00), .c(x08), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  oai21  g11(.a(new_n53_), .b(x01), .c(x08), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  nand3  g13(.a(x10), .b(x08), .c(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  nand3  g15(.a(x10), .b(x08), .c(x03), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  oai21  g17(.a(new_n53_), .b(x04), .c(x08), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  nand3  g19(.a(x10), .b(x08), .c(x05), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  nand3  g21(.a(x10), .b(x08), .c(x06), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z07));
  inv1   g23(.a(x07), .O(new_n68_));
  aoi21  g24(.a(x10), .b(new_n68_), .c(new_n45_), .O(z08));
  inv1   g25(.a(x09), .O(new_n70_));
  nor2   g26(.a(x10), .b(new_n70_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n54_), .O(z09));
  nand2  g32(.a(x20), .b(x19), .O(new_n77_));
  inv1   g33(.a(new_n72_), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n78_), .c(new_n77_), .O(new_n81_));
  oai21  g37(.a(x20), .b(x19), .c(new_n71_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n81_), .c(new_n56_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n78_), .c(new_n84_), .O(new_n86_));
  nand3  g42(.a(new_n53_), .b(x09), .c(new_n45_), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(new_n77_), .b(new_n89_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n86_), .c(new_n58_), .O(z11));
  aoi21  g48(.a(new_n78_), .b(x14), .c(new_n49_), .O(new_n93_));
  inv1   g49(.a(new_n84_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x22), .c(new_n88_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n60_), .O(z12));
  nand4  g52(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(x23), .c(new_n49_), .O(new_n98_));
  nand2  g54(.a(new_n84_), .b(new_n45_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(x22), .c(new_n48_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n71_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n62_), .O(z13));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  inv1   g60(.a(new_n49_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(x24), .c(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n49_), .b(new_n48_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n87_), .c(new_n64_), .O(z14));
  oai21  g66(.a(new_n53_), .b(x06), .c(x08), .O(new_n111_));
  nand2  g67(.a(x26), .b(x17), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n50_), .c(x25), .d(x24), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n106_), .b(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n113_), .c(new_n71_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n111_), .O(z15));
  inv1   g73(.a(x26), .O(new_n118_));
  nand4  g74(.a(new_n105_), .b(x25), .c(x24), .d(x23), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  inv1   g76(.a(x18), .O(new_n121_));
  nand3  g77(.a(new_n50_), .b(new_n47_), .c(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n88_), .O(new_n123_));
  nand3  g79(.a(x10), .b(x08), .c(x07), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n123_), .O(z16));
endmodule


