// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x08), .O(new_n45_));
  nand3  g01(.a(x26), .b(x25), .c(x24), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(new_n47_), .c(x09), .O(new_n51_));
  aoi21  g07(.a(new_n51_), .b(new_n45_), .c(x10), .O(z00));
  nand3  g08(.a(x10), .b(x08), .c(x00), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z01));
  inv1   g10(.a(x01), .O(new_n55_));
  aoi21  g11(.a(x10), .b(new_n55_), .c(new_n45_), .O(z02));
  inv1   g12(.a(x02), .O(new_n57_));
  aoi21  g13(.a(x10), .b(new_n57_), .c(new_n45_), .O(z03));
  nand3  g14(.a(x10), .b(x08), .c(x03), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z04));
  nand3  g16(.a(x10), .b(x08), .c(x04), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z05));
  inv1   g18(.a(x05), .O(new_n63_));
  aoi21  g19(.a(x10), .b(new_n63_), .c(new_n45_), .O(z06));
  inv1   g20(.a(x06), .O(new_n65_));
  aoi21  g21(.a(x10), .b(new_n65_), .c(new_n45_), .O(z07));
  inv1   g22(.a(x07), .O(new_n67_));
  aoi21  g23(.a(x10), .b(new_n67_), .c(new_n45_), .O(z08));
  inv1   g24(.a(x10), .O(new_n69_));
  nand3  g25(.a(new_n69_), .b(x09), .c(new_n45_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n53_), .O(z09));
  nor2   g32(.a(new_n69_), .b(new_n45_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  inv1   g34(.a(x19), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n72_), .c(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n80_), .b(new_n79_), .c(new_n70_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  oai21  g41(.a(new_n78_), .b(new_n55_), .c(new_n85_), .O(z10));
  inv1   g42(.a(new_n72_), .O(new_n87_));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n87_), .c(new_n88_), .O(new_n90_));
  oai21  g46(.a(new_n81_), .b(x21), .c(new_n71_), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n90_), .c(new_n78_), .d(new_n57_), .O(z11));
  aoi21  g48(.a(new_n87_), .b(x14), .c(new_n49_), .O(new_n93_));
  inv1   g49(.a(new_n88_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x22), .c(new_n71_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n59_), .O(z12));
  oai21  g52(.a(new_n69_), .b(x04), .c(x08), .O(new_n97_));
  nand4  g53(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(x23), .c(new_n49_), .O(new_n99_));
  nand2  g55(.a(new_n88_), .b(new_n45_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(x22), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(x23), .c(new_n99_), .O(new_n102_));
  nand2  g58(.a(new_n69_), .b(x09), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n102_), .c(new_n97_), .O(z13));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  inv1   g62(.a(new_n49_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(x24), .c(x23), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n49_), .b(new_n48_), .c(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  nand2  g67(.a(new_n77_), .b(x05), .O(new_n112_));
  oai21  g68(.a(new_n111_), .b(new_n70_), .c(new_n112_), .O(z14));
  nand2  g69(.a(new_n77_), .b(x06), .O(new_n114_));
  nand2  g70(.a(x26), .b(x17), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n50_), .c(x25), .d(x24), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n108_), .b(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(new_n71_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n114_), .O(z15));
  nand2  g76(.a(new_n77_), .b(x07), .O(new_n121_));
  inv1   g77(.a(x26), .O(new_n122_));
  nand4  g78(.a(new_n107_), .b(x25), .c(x24), .d(x23), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nand3  g81(.a(new_n50_), .b(new_n47_), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n71_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n121_), .O(z16));
endmodule


