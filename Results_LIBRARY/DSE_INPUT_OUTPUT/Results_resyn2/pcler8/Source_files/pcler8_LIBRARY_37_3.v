// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x16), .b(x07), .O(new_n45_));
  nand3  g01(.a(x26), .b(x25), .c(x24), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g06(.a(x23), .b(x22), .O(new_n51_));
  nand3  g07(.a(x21), .b(x20), .c(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n45_), .O(z01));
  inv1   g13(.a(new_n45_), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n45_), .O(z03));
  nand2  g19(.a(new_n59_), .b(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(new_n59_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n45_), .O(z06));
  nand2  g25(.a(new_n59_), .b(x06), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  inv1   g27(.a(x16), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x08), .c(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nand4  g30(.a(new_n45_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  nand2  g36(.a(new_n59_), .b(x00), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(z09));
  nand2  g38(.a(x08), .b(x01), .O(new_n83_));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n77_), .c(new_n85_), .O(new_n87_));
  nor2   g43(.a(x20), .b(x19), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n49_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n83_), .c(new_n58_), .O(z10));
  inv1   g47(.a(new_n52_), .O(new_n92_));
  nand3  g48(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n46_), .c(new_n92_), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  aoi21  g51(.a(new_n84_), .b(new_n95_), .c(new_n49_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n62_), .c(new_n58_), .O(z11));
  inv1   g54(.a(new_n77_), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  aoi21  g56(.a(new_n99_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n52_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n76_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n101_), .c(new_n64_), .O(z12));
  inv1   g61(.a(x15), .O(new_n106_));
  oai21  g62(.a(new_n46_), .b(new_n106_), .c(new_n53_), .O(new_n107_));
  inv1   g63(.a(x23), .O(new_n108_));
  aoi21  g64(.a(new_n100_), .b(new_n108_), .c(new_n49_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  aoi21  g66(.a(x08), .b(x04), .c(new_n58_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n110_), .O(z13));
  inv1   g68(.a(new_n49_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n52_), .b(new_n51_), .c(new_n114_), .O(new_n115_));
  inv1   g71(.a(new_n51_), .O(new_n116_));
  nand3  g72(.a(new_n92_), .b(new_n116_), .c(x24), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  oai21  g74(.a(new_n50_), .b(x07), .c(x16), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n68_), .O(z14));
  inv1   g76(.a(x25), .O(new_n121_));
  aoi21  g77(.a(x26), .b(x17), .c(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n53_), .c(x24), .O(new_n123_));
  nand2  g79(.a(new_n117_), .b(new_n121_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n123_), .c(new_n76_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n70_), .O(z15));
  nor3   g82(.a(new_n100_), .b(new_n77_), .c(x18), .O(new_n127_));
  inv1   g83(.a(x26), .O(new_n128_));
  nand3  g84(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n100_), .c(new_n128_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n76_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n127_), .c(new_n73_), .O(z16));
endmodule


