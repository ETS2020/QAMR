// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nand2  g00(.a(x13), .b(x10), .O(new_n45_));
  nand4  g01(.a(x23), .b(x22), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x09), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n49_), .c(new_n47_), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand2  g09(.a(x21), .b(new_n53_), .O(new_n54_));
  oai21  g10(.a(new_n54_), .b(new_n52_), .c(new_n45_), .O(z00));
  inv1   g11(.a(x08), .O(new_n56_));
  inv1   g12(.a(new_n45_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n45_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n45_), .O(z03));
  nand2  g20(.a(new_n58_), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z05));
  nand2  g24(.a(new_n58_), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(new_n58_), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(new_n58_), .b(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nand3  g30(.a(new_n53_), .b(x09), .c(new_n56_), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  aoi21  g36(.a(x08), .b(x00), .c(new_n57_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(z09));
  and2   g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n77_), .c(new_n83_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n75_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n61_), .c(new_n45_), .O(z10));
  nand2  g45(.a(new_n52_), .b(new_n53_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x13), .O(new_n91_));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  nor2   g49(.a(new_n93_), .b(new_n75_), .O(new_n94_));
  oai21  g50(.a(new_n83_), .b(x21), .c(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n91_), .c(new_n63_), .O(z11));
  inv1   g52(.a(new_n77_), .O(new_n97_));
  nand4  g53(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  aoi21  g54(.a(new_n97_), .b(x14), .c(new_n98_), .O(new_n99_));
  oai21  g55(.a(new_n93_), .b(x22), .c(new_n76_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n99_), .c(new_n65_), .O(z12));
  inv1   g57(.a(new_n98_), .O(new_n102_));
  nand2  g58(.a(new_n51_), .b(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n102_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  aoi21  g61(.a(new_n98_), .b(new_n105_), .c(new_n75_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g63(.a(new_n58_), .b(x04), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(z13));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  and2   g67(.a(x23), .b(x22), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n83_), .c(x24), .d(x21), .O(new_n113_));
  inv1   g69(.a(x21), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n46_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n113_), .c(new_n111_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n75_), .c(new_n69_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  nor2   g75(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(new_n120_), .c(new_n47_), .d(x21), .O(new_n122_));
  nand2  g78(.a(new_n113_), .b(new_n119_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n76_), .O(new_n124_));
  aoi21  g80(.a(x08), .b(x06), .c(new_n57_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(z15));
  inv1   g82(.a(x18), .O(new_n127_));
  nand3  g83(.a(new_n102_), .b(new_n97_), .c(new_n127_), .O(new_n128_));
  inv1   g84(.a(x26), .O(new_n129_));
  nand3  g85(.a(new_n120_), .b(new_n47_), .c(x21), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n128_), .c(new_n76_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n73_), .O(z16));
endmodule


