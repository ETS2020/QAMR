// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n60_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  inv1   g07(.a(x08), .O(new_n52_));
  nand2  g08(.a(x10), .b(new_n52_), .O(new_n53_));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  and2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  nor2   g13(.a(new_n52_), .b(new_n57_), .O(z02));
  and2   g14(.a(x08), .b(x02), .O(z03));
  nand2  g15(.a(x08), .b(x03), .O(new_n60_));
  and2   g16(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z04));
  nand2  g18(.a(x08), .b(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n53_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  inv1   g26(.a(x09), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(x08), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  and2   g29(.a(x22), .b(x21), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n73_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n55_), .O(z09));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  inv1   g35(.a(new_n73_), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(new_n74_), .c(x12), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g38(.a(new_n72_), .b(new_n45_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  oai21  g42(.a(new_n52_), .b(new_n57_), .c(new_n86_), .O(z10));
  nand2  g43(.a(new_n79_), .b(x21), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n80_), .c(new_n88_), .O(new_n90_));
  oai21  g46(.a(new_n79_), .b(x21), .c(new_n72_), .O(new_n91_));
  aoi21  g47(.a(x10), .b(new_n52_), .c(z03), .O(new_n92_));
  oai21  g48(.a(new_n91_), .b(new_n90_), .c(new_n92_), .O(z11));
  aoi21  g49(.a(new_n80_), .b(x14), .c(new_n47_), .O(new_n94_));
  inv1   g50(.a(new_n88_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n72_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n61_), .O(z12));
  nand3  g53(.a(new_n49_), .b(x24), .c(x15), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  aoi21  g55(.a(new_n47_), .b(new_n46_), .c(new_n83_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n63_), .O(z13));
  nand3  g58(.a(new_n49_), .b(x24), .c(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n47_), .b(new_n46_), .c(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n74_), .b(new_n79_), .c(x24), .d(x23), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n106_), .c(new_n104_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n83_), .c(new_n65_), .O(z14));
  nand2  g65(.a(new_n49_), .b(x17), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n48_), .b(x25), .c(x24), .O(new_n112_));
  inv1   g68(.a(x25), .O(new_n113_));
  nand2  g69(.a(new_n107_), .b(new_n113_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n83_), .c(new_n67_), .O(z15));
  inv1   g72(.a(new_n112_), .O(new_n117_));
  nor2   g73(.a(new_n47_), .b(x18), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n80_), .c(new_n83_), .O(new_n119_));
  oai21  g75(.a(new_n117_), .b(x26), .c(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n69_), .O(z16));
endmodule


