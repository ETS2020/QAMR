// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_;
  nand4  g00(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n45_));
  nand2  g01(.a(x24), .b(x23), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(x25), .O(new_n48_));
  inv1   g04(.a(x00), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x09), .c(new_n50_), .d(new_n49_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(x26), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n48_), .O(z00));
  nor2   g11(.a(x08), .b(x00), .O(new_n56_));
  nor2   g12(.a(new_n50_), .b(x01), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(z02));
  nor2   g14(.a(x08), .b(new_n49_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(new_n60_));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n60_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n60_), .O(z04));
  and2   g20(.a(x08), .b(x04), .O(z05));
  and2   g21(.a(x08), .b(x05), .O(z06));
  and2   g22(.a(x08), .b(x06), .O(z07));
  nand2  g23(.a(x08), .b(x07), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n60_), .O(z08));
  nand3  g25(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n49_), .c(new_n59_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n72_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n70_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(new_n57_), .b(new_n56_), .c(new_n82_), .O(z10));
  nand3  g39(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n85_), .c(new_n86_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(x20), .b(x19), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n53_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n89_), .c(new_n61_), .O(z11));
  inv1   g50(.a(new_n72_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(x14), .c(new_n45_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n86_), .b(new_n97_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n53_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n96_), .c(new_n63_), .O(z12));
  inv1   g56(.a(new_n45_), .O(new_n101_));
  nand2  g57(.a(new_n85_), .b(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x23), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  aoi21  g60(.a(new_n45_), .b(new_n104_), .c(new_n70_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nor2   g62(.a(z05), .b(new_n59_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(z13));
  and2   g64(.a(x22), .b(x21), .O(new_n109_));
  and2   g65(.a(x24), .b(x23), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n77_), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n45_), .b(new_n104_), .c(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n111_), .c(new_n113_), .O(new_n116_));
  nor2   g72(.a(z06), .b(new_n59_), .O(new_n117_));
  oai21  g73(.a(new_n116_), .b(new_n70_), .c(new_n117_), .O(z14));
  nand2  g74(.a(x26), .b(x17), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n47_), .c(x25), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  nand2  g77(.a(new_n111_), .b(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n71_), .O(new_n123_));
  nor2   g79(.a(z07), .b(new_n59_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n123_), .O(z15));
  aoi21  g81(.a(new_n47_), .b(x25), .c(x26), .O(new_n126_));
  inv1   g82(.a(x18), .O(new_n127_));
  nand3  g83(.a(new_n95_), .b(new_n101_), .c(new_n127_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n53_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n126_), .c(new_n68_), .O(z16));
  buf    g86(.a(x00), .O(z01));
endmodule


