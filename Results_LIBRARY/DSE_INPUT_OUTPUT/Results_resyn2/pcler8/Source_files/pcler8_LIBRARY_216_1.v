// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(x26), .b(x25), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  nand2  g09(.a(x24), .b(x23), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n46_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  nor2   g16(.a(new_n49_), .b(new_n60_), .O(z02));
  nand3  g17(.a(new_n47_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n47_), .O(z04));
  nand3  g21(.a(new_n47_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n46_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand3  g27(.a(new_n47_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(new_n51_), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g35(.a(x20), .b(x11), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(new_n81_));
  oai22  g37(.a(new_n81_), .b(new_n75_), .c(new_n58_), .d(new_n46_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  aoi21  g39(.a(new_n79_), .b(x12), .c(new_n83_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n74_), .O(new_n85_));
  nor2   g41(.a(z02), .b(new_n46_), .O(new_n86_));
  oai21  g42(.a(new_n85_), .b(new_n84_), .c(new_n86_), .O(z10));
  inv1   g43(.a(new_n78_), .O(new_n88_));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  nor2   g47(.a(new_n51_), .b(new_n46_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  nand2  g49(.a(new_n83_), .b(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n91_), .c(new_n62_), .O(z11));
  inv1   g52(.a(x14), .O(new_n97_));
  inv1   g53(.a(new_n53_), .O(new_n98_));
  oai21  g54(.a(new_n78_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  aoi21  g56(.a(new_n89_), .b(new_n100_), .c(new_n51_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n64_), .c(new_n46_), .O(z12));
  inv1   g59(.a(x15), .O(new_n104_));
  nor2   g60(.a(new_n78_), .b(new_n104_), .O(new_n105_));
  inv1   g61(.a(x23), .O(new_n106_));
  xor2a  g62(.a(new_n53_), .b(new_n106_), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n105_), .c(new_n92_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n66_), .O(z13));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n53_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  inv1   g67(.a(x16), .O(new_n112_));
  oai21  g68(.a(new_n48_), .b(new_n112_), .c(new_n55_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n111_), .c(new_n74_), .O(new_n114_));
  aoi21  g70(.a(x08), .b(x05), .c(new_n46_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(z14));
  inv1   g72(.a(x25), .O(new_n117_));
  oai21  g73(.a(new_n54_), .b(new_n53_), .c(new_n117_), .O(new_n118_));
  nand2  g74(.a(new_n55_), .b(x25), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n74_), .O(new_n120_));
  oai21  g76(.a(new_n52_), .b(new_n60_), .c(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n120_), .c(new_n70_), .O(z15));
  aoi21  g78(.a(new_n55_), .b(x25), .c(x26), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand3  g80(.a(new_n88_), .b(new_n98_), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n92_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n123_), .c(new_n72_), .O(z16));
endmodule


