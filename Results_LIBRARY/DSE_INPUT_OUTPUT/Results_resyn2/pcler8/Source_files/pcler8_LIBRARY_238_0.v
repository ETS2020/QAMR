// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n45_), .O(new_n49_));
  inv1   g05(.a(x25), .O(new_n50_));
  inv1   g06(.a(x26), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(x24), .O(new_n53_));
  nand3  g09(.a(x21), .b(x20), .c(x19), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  and2   g11(.a(new_n55_), .b(new_n49_), .O(z00));
  nor2   g12(.a(x26), .b(x25), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n57_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n57_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n58_), .O(z04));
  nand3  g22(.a(new_n58_), .b(x08), .c(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand3  g24(.a(new_n58_), .b(x08), .c(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n57_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n58_), .O(z08));
  inv1   g30(.a(new_n48_), .O(new_n75_));
  nand3  g31(.a(new_n52_), .b(x24), .c(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(new_n59_), .c(new_n58_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  nand2  g38(.a(new_n77_), .b(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n76_), .c(new_n82_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n48_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n61_), .c(new_n58_), .O(z10));
  inv1   g44(.a(new_n53_), .O(new_n89_));
  nand3  g45(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n54_), .O(new_n92_));
  oai21  g48(.a(new_n82_), .b(x21), .c(new_n75_), .O(new_n93_));
  aoi21  g49(.a(x08), .b(x02), .c(new_n57_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z11));
  inv1   g51(.a(new_n54_), .O(new_n96_));
  nor2   g52(.a(new_n96_), .b(x22), .O(new_n97_));
  nand4  g53(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n75_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n65_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n58_), .O(new_n101_));
  nand3  g57(.a(new_n89_), .b(new_n49_), .c(x14), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n101_), .O(z12));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n98_), .b(new_n104_), .O(new_n105_));
  nor2   g61(.a(new_n57_), .b(new_n48_), .O(new_n106_));
  nor2   g62(.a(new_n98_), .b(new_n104_), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand4  g65(.a(new_n55_), .b(new_n75_), .c(x22), .d(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n67_), .O(z13));
  nand4  g67(.a(new_n77_), .b(new_n82_), .c(x24), .d(x23), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand2  g69(.a(new_n108_), .b(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n106_), .O(new_n115_));
  nand3  g71(.a(new_n89_), .b(new_n75_), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n69_), .O(z14));
  nand2  g73(.a(new_n52_), .b(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n107_), .b(x25), .c(x24), .O(new_n120_));
  nand2  g76(.a(new_n112_), .b(new_n50_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  aoi21  g78(.a(x08), .b(x06), .c(new_n57_), .O(new_n123_));
  oai21  g79(.a(new_n122_), .b(new_n48_), .c(new_n123_), .O(z15));
  nand3  g80(.a(new_n58_), .b(x08), .c(x07), .O(new_n125_));
  nand2  g81(.a(new_n120_), .b(new_n51_), .O(new_n126_));
  inv1   g82(.a(x18), .O(new_n127_));
  nand4  g83(.a(new_n107_), .b(new_n52_), .c(x24), .d(new_n127_), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n126_), .c(new_n75_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n125_), .O(z16));
endmodule


