// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x17), .b(x11), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g04(.a(x25), .O(new_n49_));
  inv1   g05(.a(x26), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand4  g11(.a(new_n55_), .b(new_n51_), .c(new_n48_), .d(x24), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n45_), .O(z00));
  inv1   g13(.a(new_n45_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n58_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  and2   g19(.a(new_n63_), .b(new_n45_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n58_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n45_), .O(z05));
  nand3  g25(.a(new_n45_), .b(x08), .c(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n45_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n45_), .O(z08));
  inv1   g31(.a(x19), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  and2   g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n76_), .O(new_n82_));
  and2   g38(.a(new_n59_), .b(new_n45_), .O(new_n83_));
  oai21  g39(.a(new_n82_), .b(new_n54_), .c(new_n83_), .O(z09));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n80_), .b(x12), .c(new_n86_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n55_), .O(new_n88_));
  and2   g44(.a(new_n61_), .b(new_n45_), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z10));
  nand2  g46(.a(new_n85_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n79_), .c(new_n91_), .O(new_n93_));
  oai21  g49(.a(new_n85_), .b(x21), .c(new_n55_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(z11));
  nand3  g51(.a(new_n85_), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g52(.a(new_n79_), .b(x14), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(new_n91_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n55_), .O(new_n99_));
  and2   g55(.a(new_n66_), .b(new_n45_), .O(new_n100_));
  oai21  g56(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(z12));
  nand3  g57(.a(new_n51_), .b(x24), .c(x15), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n48_), .O(new_n103_));
  aoi21  g59(.a(new_n47_), .b(new_n46_), .c(new_n54_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n68_), .c(new_n45_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n47_), .b(new_n46_), .c(new_n109_), .O(new_n110_));
  inv1   g66(.a(new_n47_), .O(new_n111_));
  and2   g67(.a(x24), .b(x23), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n110_), .c(new_n108_), .O(new_n114_));
  nand2  g70(.a(new_n55_), .b(new_n45_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n114_), .c(new_n70_), .O(z14));
  nand2  g72(.a(x24), .b(x23), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n47_), .c(new_n49_), .O(new_n118_));
  nand4  g74(.a(new_n112_), .b(new_n77_), .c(new_n85_), .d(x25), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n55_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n45_), .O(new_n122_));
  inv1   g78(.a(x09), .O(new_n123_));
  nor2   g79(.a(new_n123_), .b(x08), .O(new_n124_));
  nor2   g80(.a(x11), .b(x10), .O(new_n125_));
  nand4  g81(.a(new_n125_), .b(new_n124_), .c(new_n51_), .d(x17), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n122_), .O(z15));
  nor2   g83(.a(new_n50_), .b(x18), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n112_), .c(new_n111_), .d(x25), .O(new_n129_));
  nand2  g85(.a(new_n119_), .b(new_n50_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n55_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n74_), .c(new_n58_), .O(z16));
endmodule


