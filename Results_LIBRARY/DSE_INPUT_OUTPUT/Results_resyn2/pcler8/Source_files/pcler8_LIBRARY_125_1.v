// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x15), .O(new_n49_));
  nand2  g05(.a(x17), .b(new_n49_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nand3  g08(.a(x22), .b(x21), .c(x20), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n51_), .O(z00));
  inv1   g12(.a(new_n50_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n50_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n57_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n50_), .O(z04));
  nand3  g21(.a(new_n50_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n57_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n57_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n50_), .O(z08));
  inv1   g29(.a(x19), .O(new_n74_));
  aoi21  g30(.a(new_n54_), .b(x11), .c(new_n74_), .O(new_n75_));
  oai22  g31(.a(new_n75_), .b(new_n51_), .c(new_n58_), .d(new_n57_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n52_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n47_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n60_), .c(new_n57_), .O(z10));
  inv1   g39(.a(new_n52_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n77_), .b(x21), .c(new_n48_), .O(new_n88_));
  and2   g44(.a(new_n62_), .b(new_n50_), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z11));
  inv1   g46(.a(z04), .O(new_n91_));
  inv1   g47(.a(new_n85_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(x22), .O(new_n93_));
  aoi21  g49(.a(new_n84_), .b(x14), .c(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n92_), .b(x22), .c(new_n48_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n94_), .c(new_n91_), .O(z12));
  and2   g52(.a(x22), .b(x21), .O(new_n97_));
  nand3  g53(.a(new_n77_), .b(new_n97_), .c(x23), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n98_), .c(new_n50_), .d(new_n48_), .O(new_n102_));
  nand3  g58(.a(new_n84_), .b(new_n48_), .c(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n102_), .c(new_n66_), .O(z13));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n100_), .b(new_n99_), .c(new_n105_), .O(new_n106_));
  and2   g62(.a(x24), .b(x23), .O(new_n107_));
  nand3  g63(.a(new_n77_), .b(new_n97_), .c(new_n107_), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n108_), .b(new_n106_), .c(new_n110_), .O(new_n111_));
  and2   g67(.a(new_n68_), .b(new_n50_), .O(new_n112_));
  oai21  g68(.a(new_n111_), .b(new_n47_), .c(new_n112_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n108_), .b(new_n114_), .O(new_n115_));
  nand4  g71(.a(new_n77_), .b(new_n97_), .c(new_n107_), .d(x25), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n48_), .O(new_n117_));
  nand2  g73(.a(x26), .b(x25), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n47_), .c(x15), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(x17), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n117_), .c(new_n70_), .O(z15));
  inv1   g77(.a(z08), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand3  g79(.a(new_n54_), .b(x19), .c(new_n123_), .O(new_n124_));
  inv1   g80(.a(x26), .O(new_n125_));
  nand2  g81(.a(new_n116_), .b(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n48_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n122_), .O(z16));
endmodule


