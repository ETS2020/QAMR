// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  nand2  g00(.a(x17), .b(x04), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x26), .c(x25), .O(new_n50_));
  and2   g06(.a(x20), .b(x19), .O(new_n51_));
  and2   g07(.a(x22), .b(x21), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(new_n51_), .c(x24), .d(x23), .O(new_n53_));
  oai21  g09(.a(new_n53_), .b(new_n50_), .c(new_n45_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z01));
  nand2  g12(.a(x08), .b(x01), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n45_), .O(z02));
  nand2  g14(.a(x08), .b(x02), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n45_), .O(z04));
  oai21  g18(.a(x17), .b(x08), .c(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand3  g20(.a(new_n45_), .b(x08), .c(x05), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  and2   g22(.a(x08), .b(x06), .O(new_n67_));
  and2   g23(.a(new_n67_), .b(new_n45_), .O(z07));
  inv1   g24(.a(new_n45_), .O(new_n69_));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n69_), .O(z08));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n49_), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n55_), .c(new_n69_), .O(z09));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n72_), .c(new_n51_), .O(new_n78_));
  nor2   g34(.a(x20), .b(x19), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n48_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n57_), .c(new_n69_), .O(z10));
  nand3  g38(.a(x21), .b(x20), .c(x19), .O(new_n83_));
  inv1   g39(.a(new_n83_), .O(new_n84_));
  nand2  g40(.a(x22), .b(x13), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n72_), .c(new_n84_), .O(new_n86_));
  inv1   g42(.a(x21), .O(new_n87_));
  nand2  g43(.a(x20), .b(x19), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n87_), .c(new_n48_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n59_), .c(new_n69_), .O(z11));
  inv1   g47(.a(z04), .O(new_n92_));
  nand4  g48(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n93_));
  inv1   g49(.a(new_n72_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(x14), .c(new_n93_), .O(new_n95_));
  oai21  g51(.a(new_n84_), .b(x22), .c(new_n49_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n92_), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n72_), .b(new_n98_), .O(new_n99_));
  inv1   g55(.a(x23), .O(new_n100_));
  xor2a  g56(.a(new_n93_), .b(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n99_), .c(new_n49_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n63_), .O(z13));
  nor2   g59(.a(new_n93_), .b(new_n100_), .O(new_n104_));
  nand3  g60(.a(x26), .b(x25), .c(x16), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n104_), .c(x24), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  nand3  g63(.a(new_n52_), .b(new_n51_), .c(x23), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n106_), .c(new_n49_), .d(new_n45_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n65_), .O(z14));
  inv1   g67(.a(x25), .O(new_n112_));
  nand2  g68(.a(new_n53_), .b(new_n112_), .O(new_n113_));
  nor2   g69(.a(new_n112_), .b(new_n107_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n104_), .c(new_n48_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  inv1   g72(.a(x04), .O(new_n117_));
  nand2  g73(.a(x26), .b(x25), .O(new_n118_));
  oai21  g74(.a(new_n48_), .b(new_n118_), .c(new_n117_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(x17), .c(new_n67_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n116_), .O(z15));
  aoi21  g77(.a(new_n114_), .b(new_n104_), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand4  g79(.a(x26), .b(x25), .c(x24), .d(new_n123_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n108_), .c(new_n49_), .O(new_n125_));
  and2   g81(.a(new_n70_), .b(new_n45_), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n122_), .c(new_n126_), .O(z16));
endmodule


