// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  nand2  g03(.a(x24), .b(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g06(.a(x26), .b(x25), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n47_), .O(z00));
  nand2  g10(.a(x14), .b(x10), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n55_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  oai21  g20(.a(new_n45_), .b(new_n64_), .c(new_n55_), .O(z04));
  nand2  g21(.a(new_n57_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n55_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n55_), .O(z07));
  nand2  g27(.a(new_n57_), .b(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(new_n47_), .O(new_n74_));
  nand3  g30(.a(x24), .b(x23), .c(x22), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  and2   g32(.a(x20), .b(x11), .O(new_n77_));
  nand4  g33(.a(new_n77_), .b(new_n76_), .c(new_n52_), .d(x21), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n58_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  nand4  g38(.a(new_n76_), .b(new_n52_), .c(x21), .d(x12), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n47_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  aoi21  g43(.a(x08), .b(x01), .c(new_n56_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n87_), .O(z10));
  nor2   g45(.a(new_n75_), .b(new_n51_), .O(new_n90_));
  nand2  g46(.a(new_n82_), .b(x21), .O(new_n91_));
  aoi21  g47(.a(new_n90_), .b(x13), .c(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n82_), .b(x21), .c(new_n74_), .O(new_n93_));
  nand2  g49(.a(new_n57_), .b(x02), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z11));
  nand4  g51(.a(new_n76_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n46_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(x14), .O(new_n98_));
  nor2   g54(.a(new_n45_), .b(new_n64_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  nand2  g56(.a(new_n91_), .b(new_n100_), .O(new_n101_));
  and2   g57(.a(x22), .b(x21), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n82_), .c(new_n47_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n101_), .c(new_n99_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n98_), .O(z12));
  inv1   g61(.a(x23), .O(new_n106_));
  nor2   g62(.a(new_n49_), .b(new_n106_), .O(new_n107_));
  nand3  g63(.a(new_n52_), .b(x24), .c(x15), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi21  g65(.a(new_n49_), .b(new_n106_), .c(new_n47_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n66_), .O(z13));
  nand2  g68(.a(new_n52_), .b(x16), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n107_), .c(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n49_), .b(new_n106_), .c(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n114_), .c(new_n74_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n68_), .c(new_n55_), .O(z14));
  nand3  g74(.a(x26), .b(x25), .c(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  and2   g76(.a(x24), .b(x23), .O(new_n121_));
  nand4  g77(.a(new_n102_), .b(new_n82_), .c(new_n121_), .d(x25), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  oai21  g79(.a(new_n49_), .b(new_n48_), .c(new_n123_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  nand2  g81(.a(new_n57_), .b(x06), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n47_), .c(new_n126_), .O(z15));
  aoi21  g83(.a(new_n50_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n52_), .b(new_n50_), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n74_), .O(new_n131_));
  aoi21  g87(.a(x08), .b(x07), .c(new_n56_), .O(new_n132_));
  oai21  g88(.a(new_n131_), .b(new_n128_), .c(new_n132_), .O(z16));
endmodule


