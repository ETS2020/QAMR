// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(x24), .b(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x25), .O(new_n49_));
  inv1   g05(.a(x00), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n51_), .c(new_n50_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z00));
  oai21  g11(.a(new_n51_), .b(x08), .c(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  nand2  g14(.a(new_n51_), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n58_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n59_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n60_), .b(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  inv1   g22(.a(x08), .O(new_n67_));
  oai21  g23(.a(new_n67_), .b(new_n66_), .c(new_n59_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n59_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n59_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n59_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n53_), .c(new_n51_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n56_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n75_), .c(new_n81_), .O(new_n83_));
  nand2  g39(.a(x09), .b(new_n67_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n83_), .c(x00), .O(new_n87_));
  oai22  g43(.a(new_n87_), .b(x10), .c(new_n67_), .d(new_n58_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand3  g46(.a(x26), .b(x25), .c(x24), .O(new_n91_));
  nand3  g47(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  inv1   g49(.a(x21), .O(new_n94_));
  aoi21  g50(.a(new_n80_), .b(new_n94_), .c(new_n84_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(x00), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x10), .c(new_n62_), .O(z11));
  nand2  g53(.a(x08), .b(x03), .O(new_n98_));
  inv1   g54(.a(x14), .O(new_n99_));
  inv1   g55(.a(new_n47_), .O(new_n100_));
  oai21  g56(.a(new_n75_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  aoi21  g58(.a(new_n89_), .b(new_n102_), .c(new_n84_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n101_), .c(x00), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(x10), .c(new_n98_), .O(z12));
  inv1   g61(.a(new_n54_), .O(new_n106_));
  inv1   g62(.a(new_n91_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(x15), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n100_), .c(x23), .O(new_n109_));
  inv1   g65(.a(x23), .O(new_n110_));
  nand2  g66(.a(new_n47_), .b(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n109_), .c(new_n106_), .O(new_n112_));
  oai21  g68(.a(new_n60_), .b(new_n66_), .c(new_n112_), .O(z13));
  nand3  g69(.a(new_n100_), .b(x24), .c(x23), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n47_), .b(new_n110_), .c(new_n115_), .O(new_n116_));
  aoi22  g72(.a(new_n116_), .b(new_n114_), .c(new_n107_), .d(x16), .O(new_n117_));
  inv1   g73(.a(new_n60_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(x05), .O(new_n119_));
  oai21  g75(.a(new_n117_), .b(new_n54_), .c(new_n119_), .O(z14));
  nand3  g76(.a(x26), .b(x25), .c(x17), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  oai21  g79(.a(new_n47_), .b(new_n46_), .c(new_n123_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n49_), .c(new_n122_), .O(new_n125_));
  nand2  g81(.a(new_n118_), .b(x06), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n54_), .c(new_n126_), .O(z15));
  aoi21  g83(.a(new_n48_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  inv1   g85(.a(new_n75_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n100_), .c(new_n129_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n106_), .O(new_n132_));
  nand2  g88(.a(new_n118_), .b(x07), .O(new_n133_));
  oai21  g89(.a(new_n132_), .b(new_n128_), .c(new_n133_), .O(z16));
endmodule


