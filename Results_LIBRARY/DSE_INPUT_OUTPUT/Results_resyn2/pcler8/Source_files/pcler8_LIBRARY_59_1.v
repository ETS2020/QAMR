// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n56_, new_n57_, new_n60_, new_n62_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(z01));
  and2   g10(.a(x08), .b(x01), .O(z02));
  nor2   g11(.a(new_n45_), .b(x08), .O(new_n56_));
  aoi21  g12(.a(x08), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z03));
  and2   g14(.a(x08), .b(x03), .O(z04));
  inv1   g15(.a(x04), .O(new_n60_));
  nor2   g16(.a(new_n53_), .b(new_n60_), .O(z05));
  nand2  g17(.a(x08), .b(x05), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z06));
  and2   g19(.a(x08), .b(x06), .O(z07));
  inv1   g20(.a(x07), .O(new_n65_));
  nor2   g21(.a(new_n53_), .b(new_n65_), .O(z08));
  nand3  g22(.a(new_n45_), .b(x09), .c(new_n53_), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(new_n68_));
  nand4  g24(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n69_));
  nand4  g25(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n69_), .c(x19), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  oai21  g28(.a(new_n53_), .b(new_n52_), .c(new_n72_), .O(z09));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(x22), .b(x21), .c(x12), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g34(.a(x09), .b(new_n53_), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n80_), .O(new_n81_));
  nor2   g37(.a(new_n56_), .b(z02), .O(new_n82_));
  oai21  g38(.a(new_n81_), .b(new_n78_), .c(new_n82_), .O(z10));
  inv1   g39(.a(new_n69_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n74_), .b(x21), .c(new_n80_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n57_), .O(z11));
  inv1   g45(.a(new_n85_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x22), .O(new_n91_));
  aoi21  g47(.a(new_n84_), .b(x14), .c(new_n91_), .O(new_n92_));
  oai21  g48(.a(new_n90_), .b(x22), .c(new_n80_), .O(new_n93_));
  nor2   g49(.a(z04), .b(new_n56_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z12));
  nand4  g51(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n48_), .O(new_n97_));
  aoi21  g53(.a(new_n47_), .b(new_n46_), .c(new_n67_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  oai21  g55(.a(new_n53_), .b(new_n60_), .c(new_n99_), .O(z13));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  inv1   g58(.a(x24), .O(new_n103_));
  oai21  g59(.a(new_n47_), .b(new_n46_), .c(new_n103_), .O(new_n104_));
  inv1   g60(.a(new_n47_), .O(new_n105_));
  and2   g61(.a(x24), .b(x23), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n104_), .c(new_n102_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n67_), .c(new_n62_), .O(z14));
  nand3  g65(.a(x26), .b(x25), .c(x17), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  and2   g67(.a(x22), .b(x21), .O(new_n112_));
  nand4  g68(.a(new_n106_), .b(new_n112_), .c(new_n74_), .d(x25), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(x24), .b(x23), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n47_), .c(new_n114_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n113_), .c(new_n111_), .O(new_n117_));
  nor2   g73(.a(z07), .b(new_n56_), .O(new_n118_));
  oai21  g74(.a(new_n117_), .b(new_n79_), .c(new_n118_), .O(z15));
  nor2   g75(.a(new_n115_), .b(new_n47_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(x25), .c(x26), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand3  g78(.a(new_n84_), .b(new_n105_), .c(new_n122_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n68_), .O(new_n124_));
  oai22  g80(.a(new_n124_), .b(new_n121_), .c(new_n53_), .d(new_n65_), .O(z16));
endmodule


