// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x11), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(x20), .b(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nand2  g08(.a(x23), .b(x22), .O(new_n53_));
  nand4  g09(.a(x26), .b(x25), .c(x24), .d(x21), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z00));
  nand3  g13(.a(new_n47_), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n47_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n47_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n47_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n47_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  nand4  g29(.a(new_n55_), .b(x20), .c(x17), .d(x11), .O(new_n74_));
  inv1   g30(.a(x19), .O(new_n75_));
  inv1   g31(.a(new_n52_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n47_), .c(new_n75_), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n74_), .c(new_n58_), .O(z09));
  inv1   g34(.a(z02), .O(new_n79_));
  nor2   g35(.a(new_n54_), .b(new_n53_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(x12), .c(new_n48_), .O(new_n81_));
  oai21  g37(.a(x20), .b(x19), .c(new_n76_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n81_), .c(new_n79_), .O(z10));
  inv1   g39(.a(z03), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  and2   g43(.a(x23), .b(x22), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(x13), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n87_), .c(new_n85_), .O(new_n91_));
  oai21  g47(.a(new_n49_), .b(x21), .c(new_n76_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n84_), .O(z11));
  inv1   g49(.a(z04), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  and2   g51(.a(x23), .b(x14), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n87_), .c(new_n95_), .O(new_n97_));
  inv1   g53(.a(new_n85_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n76_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n94_), .O(z12));
  nand2  g56(.a(new_n87_), .b(x15), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nor2   g58(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g60(.a(new_n95_), .b(new_n102_), .c(new_n52_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n66_), .c(new_n46_), .O(z13));
  inv1   g63(.a(z06), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n85_), .b(new_n53_), .c(new_n111_), .O(new_n112_));
  nor2   g68(.a(new_n85_), .b(new_n53_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(x24), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n52_), .c(new_n108_), .O(z14));
  aoi21  g72(.a(new_n113_), .b(x24), .c(x25), .O(new_n117_));
  nand4  g73(.a(new_n98_), .b(new_n88_), .c(x25), .d(x24), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n76_), .c(new_n47_), .O(new_n119_));
  nand3  g75(.a(x26), .b(x25), .c(x17), .O(new_n120_));
  nor2   g76(.a(new_n120_), .b(new_n52_), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(z07), .O(new_n122_));
  oai21  g78(.a(new_n119_), .b(new_n117_), .c(new_n122_), .O(z15));
  inv1   g79(.a(x26), .O(new_n124_));
  nand2  g80(.a(new_n118_), .b(new_n124_), .O(new_n125_));
  nor2   g81(.a(new_n86_), .b(x18), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n113_), .c(new_n52_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


