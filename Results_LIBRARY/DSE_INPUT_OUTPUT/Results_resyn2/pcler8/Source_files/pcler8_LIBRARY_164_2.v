// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x07), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x05), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  and2   g05(.a(x26), .b(x25), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n50_), .c(new_n49_), .d(x24), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand3  g12(.a(new_n46_), .b(x08), .c(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand3  g14(.a(new_n46_), .b(x08), .c(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand3  g16(.a(new_n46_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z04));
  inv1   g20(.a(new_n46_), .O(new_n65_));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z05));
  nand3  g23(.a(x08), .b(x07), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n65_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  nor2   g29(.a(new_n53_), .b(new_n65_), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  and2   g31(.a(x22), .b(x21), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n75_), .c(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n57_), .O(z09));
  and2   g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  inv1   g38(.a(new_n75_), .O(new_n83_));
  nand2  g39(.a(new_n76_), .b(x12), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n82_), .O(new_n86_));
  or2    g42(.a(x20), .b(x19), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n54_), .c(new_n46_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n86_), .c(new_n59_), .O(z10));
  nand2  g45(.a(new_n81_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n83_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n81_), .b(x21), .c(new_n74_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n92_), .c(new_n61_), .O(z11));
  inv1   g50(.a(x14), .O(new_n95_));
  inv1   g51(.a(new_n48_), .O(new_n96_));
  oai21  g52(.a(new_n75_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(new_n90_), .b(new_n98_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n97_), .c(new_n54_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n63_), .c(new_n65_), .O(z12));
  nand3  g57(.a(new_n50_), .b(x24), .c(x15), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n49_), .O(new_n103_));
  aoi21  g59(.a(new_n48_), .b(new_n47_), .c(new_n53_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n66_), .c(new_n46_), .O(z13));
  oai21  g62(.a(x08), .b(new_n45_), .c(x05), .O(new_n107_));
  nand3  g63(.a(new_n50_), .b(x24), .c(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n48_), .b(new_n47_), .c(new_n110_), .O(new_n111_));
  nand4  g67(.a(new_n76_), .b(new_n81_), .c(x24), .d(x23), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n111_), .c(new_n109_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n53_), .c(new_n107_), .O(z14));
  nand2  g70(.a(new_n50_), .b(x17), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n49_), .b(x25), .c(x24), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n112_), .b(new_n118_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(new_n120_));
  and2   g76(.a(new_n70_), .b(new_n46_), .O(new_n121_));
  oai21  g77(.a(new_n120_), .b(new_n53_), .c(new_n121_), .O(z15));
  inv1   g78(.a(new_n117_), .O(new_n123_));
  nor2   g79(.a(new_n48_), .b(x18), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n83_), .c(new_n53_), .O(new_n125_));
  oai21  g81(.a(new_n123_), .b(x26), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


