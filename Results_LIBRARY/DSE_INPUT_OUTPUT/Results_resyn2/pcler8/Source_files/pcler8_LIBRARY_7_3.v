// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x09), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x25), .O(new_n51_));
  inv1   g07(.a(x26), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n50_), .c(x24), .d(new_n47_), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand2  g11(.a(x10), .b(x09), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  aoi21  g14(.a(x10), .b(x09), .c(new_n47_), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(new_n59_), .b(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(new_n59_), .b(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(new_n59_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n56_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g27(.a(new_n59_), .b(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  nand3  g29(.a(new_n46_), .b(x09), .c(new_n47_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n53_), .b(x24), .c(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  nand2  g36(.a(new_n59_), .b(x00), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(z09));
  and2   g38(.a(x20), .b(x19), .O(new_n83_));
  nand2  g39(.a(new_n77_), .b(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n76_), .c(new_n83_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n74_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n60_), .O(z10));
  inv1   g45(.a(new_n76_), .O(new_n90_));
  nand2  g46(.a(new_n83_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  oai21  g49(.a(new_n83_), .b(x21), .c(new_n75_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n62_), .O(z11));
  aoi21  g51(.a(new_n90_), .b(x14), .c(new_n49_), .O(new_n96_));
  inv1   g52(.a(new_n91_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x22), .c(new_n75_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n64_), .O(z12));
  nand2  g55(.a(x08), .b(x04), .O(new_n100_));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n50_), .O(new_n102_));
  aoi21  g58(.a(new_n49_), .b(new_n48_), .c(x08), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n102_), .c(x10), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n45_), .c(new_n100_), .O(z13));
  nand3  g61(.a(new_n53_), .b(x24), .c(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n49_), .b(new_n48_), .c(new_n108_), .O(new_n109_));
  nand4  g65(.a(new_n77_), .b(new_n83_), .c(x24), .d(x23), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  nand2  g67(.a(new_n59_), .b(x05), .O(new_n112_));
  oai21  g68(.a(new_n111_), .b(new_n74_), .c(new_n112_), .O(z14));
  nand2  g69(.a(new_n53_), .b(x17), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n50_), .b(x25), .c(x24), .O(new_n116_));
  nand2  g72(.a(new_n110_), .b(new_n51_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  nand2  g74(.a(new_n59_), .b(x06), .O(new_n119_));
  oai21  g75(.a(new_n118_), .b(new_n74_), .c(new_n119_), .O(z15));
  nand2  g76(.a(new_n116_), .b(new_n52_), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand4  g78(.a(new_n53_), .b(new_n50_), .c(x24), .d(new_n122_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n121_), .c(new_n75_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n72_), .O(z16));
endmodule


