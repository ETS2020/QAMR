// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  nand2  g01(.a(x23), .b(x22), .O(new_n46_));
  nand2  g02(.a(x21), .b(x20), .O(new_n47_));
  nor3   g03(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nor2   g08(.a(x17), .b(x04), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n52_), .c(new_n48_), .d(x19), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n54_), .O(z01));
  nand3  g14(.a(new_n54_), .b(x08), .c(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n54_), .O(z03));
  nand3  g18(.a(new_n54_), .b(x08), .c(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  and2   g20(.a(x08), .b(x04), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n54_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n54_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n54_), .O(z08));
  nor2   g27(.a(new_n46_), .b(new_n45_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand3  g29(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n52_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n57_), .c(new_n53_), .O(z09));
  nand2  g33(.a(x20), .b(x19), .O(new_n78_));
  and2   g34(.a(x21), .b(x12), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n72_), .c(new_n78_), .O(new_n80_));
  inv1   g36(.a(x19), .O(new_n81_));
  inv1   g37(.a(x20), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n81_), .c(new_n53_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n52_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n80_), .c(new_n59_), .O(z10));
  inv1   g41(.a(x13), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n73_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  aoi21  g46(.a(new_n78_), .b(new_n90_), .c(new_n51_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n61_), .c(new_n53_), .O(z11));
  inv1   g49(.a(new_n45_), .O(new_n94_));
  nand2  g50(.a(new_n88_), .b(x22), .O(new_n95_));
  and2   g51(.a(x23), .b(x14), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n94_), .c(new_n95_), .O(new_n97_));
  nor2   g53(.a(new_n53_), .b(new_n51_), .O(new_n98_));
  oai21  g54(.a(new_n88_), .b(x22), .c(new_n98_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n63_), .O(z12));
  nand2  g56(.a(new_n94_), .b(x15), .O(new_n101_));
  nor2   g57(.a(new_n87_), .b(new_n46_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n95_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n103_), .c(new_n52_), .O(new_n106_));
  nor2   g62(.a(z05), .b(new_n53_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(z13));
  nand2  g64(.a(new_n94_), .b(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  xor2a  g66(.a(new_n102_), .b(x24), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n110_), .c(new_n52_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n66_), .c(new_n53_), .O(z14));
  aoi21  g69(.a(new_n102_), .b(x24), .c(x25), .O(new_n114_));
  inv1   g70(.a(new_n46_), .O(new_n115_));
  nand4  g71(.a(new_n88_), .b(new_n115_), .c(x25), .d(x24), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n114_), .c(new_n68_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n54_), .O(new_n119_));
  nand4  g75(.a(new_n52_), .b(x26), .c(x25), .d(x17), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n119_), .O(z15));
  inv1   g77(.a(new_n116_), .O(new_n122_));
  nor2   g78(.a(new_n81_), .b(x18), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n48_), .c(new_n51_), .O(new_n124_));
  oai21  g80(.a(new_n122_), .b(x26), .c(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n70_), .c(new_n54_), .O(z16));
endmodule


