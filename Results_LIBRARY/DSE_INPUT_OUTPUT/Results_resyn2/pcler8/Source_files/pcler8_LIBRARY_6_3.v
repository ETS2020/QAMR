// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n57_, new_n59_, new_n61_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g07(.a(x08), .b(x00), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x10), .O(z01));
  oai21  g09(.a(x10), .b(x01), .c(x08), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z02));
  inv1   g11(.a(x02), .O(new_n56_));
  inv1   g12(.a(x08), .O(new_n57_));
  aoi21  g13(.a(new_n46_), .b(new_n56_), .c(new_n57_), .O(z03));
  nand2  g14(.a(x08), .b(x03), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(x10), .O(z04));
  inv1   g16(.a(x04), .O(new_n61_));
  aoi21  g17(.a(new_n46_), .b(new_n61_), .c(new_n57_), .O(z05));
  nand2  g18(.a(x08), .b(x05), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x10), .O(z06));
  nand2  g20(.a(x08), .b(x06), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(x10), .O(z07));
  nand2  g22(.a(x08), .b(x07), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x10), .O(z08));
  oai21  g24(.a(x10), .b(x00), .c(x08), .O(new_n69_));
  inv1   g25(.a(x19), .O(new_n70_));
  inv1   g26(.a(new_n45_), .O(new_n71_));
  nand4  g27(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  aoi21  g29(.a(new_n73_), .b(new_n71_), .c(new_n70_), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n49_), .c(new_n69_), .O(z09));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n71_), .c(new_n76_), .O(new_n79_));
  inv1   g35(.a(new_n49_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n80_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n79_), .c(new_n54_), .O(z10));
  nand2  g38(.a(x08), .b(x02), .O(new_n83_));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  nand2  g40(.a(x22), .b(x13), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n45_), .O(new_n86_));
  inv1   g42(.a(x21), .O(new_n87_));
  nand2  g43(.a(x09), .b(new_n57_), .O(new_n88_));
  aoi21  g44(.a(new_n76_), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n86_), .b(new_n84_), .c(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n83_), .c(x10), .O(z11));
  inv1   g47(.a(x14), .O(new_n92_));
  inv1   g48(.a(new_n50_), .O(new_n93_));
  oai21  g49(.a(new_n45_), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  aoi21  g51(.a(new_n84_), .b(new_n95_), .c(new_n88_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n59_), .c(x10), .O(z12));
  nand2  g54(.a(x08), .b(x04), .O(new_n99_));
  inv1   g55(.a(x15), .O(new_n100_));
  nor2   g56(.a(new_n45_), .b(new_n100_), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  xor2a  g58(.a(new_n50_), .b(new_n102_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n101_), .c(new_n48_), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n99_), .c(x10), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  nor2   g63(.a(new_n50_), .b(new_n102_), .O(new_n108_));
  xor2a  g64(.a(new_n108_), .b(x24), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n107_), .c(new_n48_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n63_), .c(x10), .O(z14));
  oai21  g67(.a(x10), .b(x06), .c(x08), .O(new_n112_));
  nand3  g68(.a(x26), .b(x25), .c(x17), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n108_), .b(x25), .c(x24), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand3  g72(.a(new_n93_), .b(x24), .c(x23), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n115_), .c(new_n114_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n49_), .c(new_n112_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n115_), .b(new_n121_), .O(new_n122_));
  nor2   g78(.a(new_n50_), .b(x18), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n71_), .c(new_n88_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n67_), .c(x10), .O(z16));
endmodule


