// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x05), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nand2  g04(.a(x24), .b(x23), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g06(.a(x26), .b(x25), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n47_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n47_), .O(z02));
  nand3  g17(.a(new_n47_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand3  g19(.a(new_n47_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n47_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n47_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  inv1   g29(.a(new_n54_), .O(new_n74_));
  nand2  g30(.a(x22), .b(x21), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n78_), .c(new_n76_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(x19), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n58_), .c(new_n46_), .O(z09));
  inv1   g39(.a(z02), .O(new_n84_));
  and2   g40(.a(x20), .b(x19), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  nor2   g42(.a(new_n77_), .b(new_n75_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(x12), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(x20), .b(x19), .c(new_n74_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n88_), .c(new_n84_), .O(z10));
  nand2  g46(.a(new_n85_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n78_), .c(new_n91_), .O(new_n93_));
  nor2   g49(.a(new_n54_), .b(new_n46_), .O(new_n94_));
  oai21  g50(.a(new_n85_), .b(x21), .c(new_n94_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n62_), .O(z11));
  aoi21  g52(.a(new_n78_), .b(x14), .c(new_n48_), .O(new_n97_));
  inv1   g53(.a(new_n91_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n94_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n64_), .O(z12));
  inv1   g56(.a(new_n48_), .O(new_n101_));
  inv1   g57(.a(new_n51_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(x24), .c(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n101_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  aoi21  g61(.a(new_n48_), .b(new_n105_), .c(new_n54_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n66_), .c(new_n47_), .O(z13));
  aoi21  g64(.a(new_n45_), .b(new_n52_), .c(x05), .O(new_n109_));
  nand2  g65(.a(new_n102_), .b(x16), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n50_), .O(new_n111_));
  inv1   g67(.a(x24), .O(new_n112_));
  oai21  g68(.a(new_n48_), .b(new_n105_), .c(new_n112_), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n111_), .c(new_n53_), .d(x09), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n52_), .c(new_n109_), .O(z14));
  inv1   g71(.a(new_n49_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n101_), .c(x25), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  oai21  g74(.a(new_n49_), .b(new_n48_), .c(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n117_), .c(new_n74_), .O(new_n120_));
  aoi22  g76(.a(new_n55_), .b(x17), .c(x08), .d(x06), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n46_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand2  g79(.a(new_n117_), .b(new_n123_), .O(new_n124_));
  nor2   g80(.a(new_n51_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n50_), .c(new_n54_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


