// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x17), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  and2   g06(.a(x20), .b(x19), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  and2   g09(.a(x22), .b(x21), .O(new_n54_));
  and2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  and2   g14(.a(new_n58_), .b(new_n46_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand3  g16(.a(x17), .b(x08), .c(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  and2   g19(.a(new_n63_), .b(new_n46_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  inv1   g21(.a(new_n46_), .O(new_n66_));
  nand2  g22(.a(x08), .b(x03), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n66_), .O(z04));
  nand2  g24(.a(x08), .b(x04), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n66_), .O(z05));
  nand3  g26(.a(new_n46_), .b(x08), .c(x05), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n66_), .O(z07));
  nand2  g30(.a(x08), .b(x07), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n66_), .O(z08));
  inv1   g32(.a(x19), .O(new_n77_));
  and2   g33(.a(x20), .b(x11), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n55_), .c(new_n77_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n49_), .c(new_n59_), .O(z09));
  oai21  g36(.a(new_n45_), .b(x08), .c(x01), .O(new_n81_));
  inv1   g37(.a(new_n51_), .O(new_n82_));
  aoi21  g38(.a(new_n55_), .b(x12), .c(new_n82_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n50_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n83_), .c(new_n81_), .O(z10));
  nand2  g41(.a(new_n51_), .b(x21), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n53_), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(new_n51_), .b(x21), .c(new_n50_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n88_), .c(new_n64_), .O(z11));
  nand3  g46(.a(new_n51_), .b(x22), .c(x21), .O(new_n91_));
  aoi21  g47(.a(new_n53_), .b(x14), .c(new_n91_), .O(new_n92_));
  inv1   g48(.a(new_n86_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(x22), .c(new_n50_), .O(new_n94_));
  and2   g50(.a(new_n67_), .b(new_n46_), .O(new_n95_));
  oai21  g51(.a(new_n94_), .b(new_n92_), .c(new_n95_), .O(z12));
  inv1   g52(.a(x15), .O(new_n97_));
  nor2   g53(.a(new_n52_), .b(new_n97_), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  xor2a  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n98_), .c(new_n50_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n69_), .c(new_n46_), .O(z13));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  nand4  g61(.a(new_n54_), .b(new_n51_), .c(x24), .d(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n100_), .b(new_n99_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  nand2  g65(.a(new_n50_), .b(new_n46_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n109_), .c(new_n71_), .O(z14));
  nand3  g67(.a(x26), .b(x25), .c(x17), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(new_n100_), .O(new_n114_));
  nand4  g70(.a(new_n114_), .b(x25), .c(x24), .d(x23), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand2  g72(.a(new_n106_), .b(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  and2   g74(.a(new_n73_), .b(new_n46_), .O(new_n119_));
  oai21  g75(.a(new_n118_), .b(new_n49_), .c(new_n119_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n115_), .b(new_n121_), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand3  g79(.a(new_n55_), .b(new_n51_), .c(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n50_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n75_), .c(new_n46_), .O(z16));
endmodule


