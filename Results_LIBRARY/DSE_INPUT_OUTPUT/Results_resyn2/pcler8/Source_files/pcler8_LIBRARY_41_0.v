// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n68_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  nor2   g00(.a(x07), .b(x05), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nand3  g08(.a(x22), .b(x21), .c(x20), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n51_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n45_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n45_), .O(z03));
  aoi21  g18(.a(x08), .b(x03), .c(new_n45_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n46_), .O(z05));
  and2   g22(.a(x08), .b(x05), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n45_), .O(z07));
  and2   g25(.a(x08), .b(x07), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  aoi21  g27(.a(new_n54_), .b(x11), .c(new_n71_), .O(new_n72_));
  oai22  g28(.a(new_n72_), .b(new_n51_), .c(new_n57_), .d(new_n45_), .O(z09));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  nand3  g30(.a(x22), .b(x21), .c(x12), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n52_), .c(new_n74_), .O(new_n76_));
  nor2   g32(.a(x20), .b(x19), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n49_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n59_), .c(new_n46_), .O(z10));
  nand3  g36(.a(x21), .b(x20), .c(x19), .O(new_n81_));
  nand3  g37(.a(x26), .b(x25), .c(x24), .O(new_n82_));
  nand3  g38(.a(x23), .b(x22), .c(x13), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  oai21  g41(.a(new_n74_), .b(x21), .c(new_n50_), .O(new_n86_));
  aoi21  g42(.a(x08), .b(x02), .c(new_n45_), .O(new_n87_));
  oai21  g43(.a(new_n86_), .b(new_n85_), .c(new_n87_), .O(z11));
  inv1   g44(.a(new_n52_), .O(new_n89_));
  nand4  g45(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n90_));
  aoi21  g46(.a(new_n89_), .b(x14), .c(new_n90_), .O(new_n91_));
  inv1   g47(.a(new_n81_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n50_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(new_n63_), .O(z12));
  inv1   g50(.a(new_n90_), .O(new_n95_));
  nand4  g51(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n95_), .c(x23), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  nand2  g54(.a(new_n90_), .b(new_n98_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n97_), .c(new_n50_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n65_), .c(new_n45_), .O(z13));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  and2   g59(.a(x24), .b(x23), .O(new_n104_));
  nand2  g60(.a(new_n95_), .b(new_n104_), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n90_), .b(new_n98_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nor2   g64(.a(z06), .b(new_n45_), .O(new_n109_));
  oai21  g65(.a(new_n108_), .b(new_n49_), .c(new_n109_), .O(z14));
  nand3  g66(.a(x26), .b(x25), .c(x17), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  and2   g68(.a(x22), .b(x21), .O(new_n113_));
  nand4  g69(.a(new_n74_), .b(new_n113_), .c(new_n104_), .d(x25), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(x24), .b(x23), .O(new_n116_));
  oai21  g72(.a(new_n90_), .b(new_n116_), .c(new_n115_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n114_), .c(new_n112_), .O(new_n118_));
  aoi21  g74(.a(x08), .b(x06), .c(new_n45_), .O(new_n119_));
  oai21  g75(.a(new_n118_), .b(new_n49_), .c(new_n119_), .O(z15));
  nor2   g76(.a(new_n90_), .b(new_n116_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(x25), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand3  g79(.a(new_n54_), .b(x19), .c(new_n123_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n50_), .O(new_n125_));
  nor2   g81(.a(z08), .b(new_n45_), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n122_), .c(new_n126_), .O(z16));
endmodule


