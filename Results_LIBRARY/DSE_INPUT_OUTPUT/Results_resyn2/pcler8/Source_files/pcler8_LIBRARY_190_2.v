// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  inv1   g00(.a(x07), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  nand2  g03(.a(x09), .b(new_n47_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(new_n46_), .c(new_n45_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  nand2  g13(.a(new_n46_), .b(x07), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n57_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n59_), .b(new_n61_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  nor2   g19(.a(new_n59_), .b(new_n63_), .O(z03));
  inv1   g20(.a(x03), .O(new_n65_));
  nor2   g21(.a(new_n59_), .b(new_n65_), .O(z04));
  inv1   g22(.a(x04), .O(new_n67_));
  oai21  g23(.a(new_n47_), .b(new_n67_), .c(new_n58_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n58_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  nor2   g27(.a(new_n59_), .b(new_n71_), .O(z07));
  nand3  g28(.a(x10), .b(x08), .c(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  and2   g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n52_), .c(x19), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n49_), .c(x07), .O(new_n78_));
  oai22  g34(.a(new_n78_), .b(x10), .c(new_n59_), .d(new_n57_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n52_), .O(new_n81_));
  nand3  g37(.a(new_n75_), .b(new_n81_), .c(x12), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n48_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(x07), .O(new_n86_));
  oai22  g42(.a(new_n86_), .b(x10), .c(new_n47_), .d(new_n61_), .O(z10));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  nand2  g45(.a(x22), .b(x13), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n52_), .c(new_n89_), .O(new_n91_));
  nor2   g47(.a(new_n80_), .b(x21), .O(new_n92_));
  nor2   g48(.a(new_n92_), .b(new_n48_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(x07), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(x10), .c(new_n47_), .d(new_n63_), .O(z11));
  aoi21  g51(.a(new_n81_), .b(x14), .c(new_n53_), .O(new_n96_));
  oai21  g52(.a(new_n89_), .b(x22), .c(new_n51_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n96_), .c(new_n59_), .d(new_n65_), .O(z12));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n53_), .b(new_n99_), .O(new_n100_));
  aoi21  g56(.a(new_n81_), .b(x15), .c(new_n100_), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(new_n50_), .c(new_n59_), .d(new_n67_), .O(z13));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nand4  g60(.a(new_n75_), .b(new_n80_), .c(x24), .d(x23), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n53_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n104_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n48_), .c(new_n45_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n46_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n69_), .O(z14));
  and2   g67(.a(x26), .b(x25), .O(new_n112_));
  nor2   g68(.a(new_n53_), .b(new_n99_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(x25), .c(x24), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(new_n105_), .b(new_n115_), .O(new_n116_));
  aoi22  g72(.a(new_n116_), .b(new_n114_), .c(new_n112_), .d(x17), .O(new_n117_));
  oai22  g73(.a(new_n117_), .b(new_n50_), .c(new_n59_), .d(new_n71_), .O(z15));
  inv1   g74(.a(new_n114_), .O(new_n119_));
  inv1   g75(.a(x18), .O(new_n120_));
  aoi21  g76(.a(new_n54_), .b(new_n120_), .c(new_n50_), .O(new_n121_));
  oai21  g77(.a(new_n119_), .b(x26), .c(new_n121_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n73_), .O(z16));
endmodule


