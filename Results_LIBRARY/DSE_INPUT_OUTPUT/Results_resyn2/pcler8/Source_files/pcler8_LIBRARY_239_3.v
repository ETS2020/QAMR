// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g00(.a(x11), .b(x01), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n50_), .c(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n45_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  inv1   g15(.a(x11), .O(new_n60_));
  aoi21  g16(.a(new_n60_), .b(new_n51_), .c(new_n59_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n45_), .O(z03));
  nand3  g19(.a(new_n45_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n45_), .O(z05));
  nand3  g23(.a(new_n45_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n45_), .O(z07));
  inv1   g27(.a(new_n45_), .O(new_n72_));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n72_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n54_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n57_), .c(new_n45_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n75_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  or2    g40(.a(x20), .b(x19), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n54_), .c(new_n45_), .O(new_n86_));
  nand3  g42(.a(new_n60_), .b(x08), .c(x01), .O(new_n87_));
  oai21  g43(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n81_), .c(new_n89_), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  nand2  g48(.a(new_n80_), .b(new_n92_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n54_), .c(new_n45_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(new_n91_), .c(new_n62_), .d(new_n72_), .O(z11));
  inv1   g51(.a(new_n89_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(x22), .O(new_n97_));
  aoi21  g53(.a(new_n81_), .b(x14), .c(new_n97_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n89_), .b(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n54_), .c(new_n45_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n98_), .c(new_n64_), .O(z12));
  nand2  g58(.a(new_n50_), .b(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  aoi21  g60(.a(new_n47_), .b(new_n46_), .c(new_n53_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n66_), .c(new_n45_), .O(z13));
  nand2  g63(.a(new_n54_), .b(new_n45_), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n47_), .b(new_n46_), .c(new_n111_), .O(new_n112_));
  inv1   g68(.a(new_n47_), .O(new_n113_));
  and2   g69(.a(x24), .b(x23), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n112_), .c(new_n110_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n108_), .c(new_n68_), .O(z14));
  inv1   g73(.a(z07), .O(new_n118_));
  nand3  g74(.a(x26), .b(x25), .c(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  and2   g76(.a(x20), .b(x19), .O(new_n121_));
  and2   g77(.a(x22), .b(x21), .O(new_n122_));
  nand4  g78(.a(new_n114_), .b(new_n122_), .c(new_n121_), .d(x25), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  nand2  g80(.a(x24), .b(x23), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n47_), .c(new_n124_), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n123_), .c(new_n120_), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n53_), .c(new_n118_), .O(z15));
  nor2   g84(.a(new_n125_), .b(new_n47_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(x25), .c(x26), .O(new_n130_));
  inv1   g86(.a(x18), .O(new_n131_));
  nand3  g87(.a(new_n81_), .b(new_n113_), .c(new_n131_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n54_), .O(new_n133_));
  and2   g89(.a(new_n73_), .b(new_n45_), .O(new_n134_));
  oai21  g90(.a(new_n133_), .b(new_n130_), .c(new_n134_), .O(z16));
endmodule


