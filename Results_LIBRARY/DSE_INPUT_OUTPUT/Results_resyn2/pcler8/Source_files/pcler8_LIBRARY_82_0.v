// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand3  g03(.a(x26), .b(x25), .c(x24), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  nand2  g05(.a(x09), .b(new_n49_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x07), .c(x10), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  inv1   g10(.a(x07), .O(new_n55_));
  inv1   g11(.a(x10), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n54_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z03));
  inv1   g19(.a(new_n58_), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n57_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n57_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n57_), .O(z07));
  oai21  g28(.a(new_n49_), .b(new_n55_), .c(new_n57_), .O(z08));
  inv1   g29(.a(x19), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand4  g32(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  inv1   g35(.a(x09), .O(new_n80_));
  nor2   g36(.a(x10), .b(new_n80_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n49_), .c(x07), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(new_n79_), .c(new_n58_), .d(new_n54_), .O(z09));
  and2   g39(.a(x20), .b(x19), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n75_), .c(new_n84_), .O(new_n86_));
  nor2   g42(.a(x20), .b(x19), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n50_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n55_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(x10), .c(new_n60_), .O(z10));
  nand3  g46(.a(x21), .b(x20), .c(x19), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  nand3  g48(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n48_), .c(new_n92_), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  nand2  g51(.a(x20), .b(x19), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n95_), .c(new_n50_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n94_), .c(new_n55_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x10), .c(new_n62_), .O(z11));
  aoi21  g55(.a(new_n76_), .b(x14), .c(new_n46_), .O(new_n100_));
  inv1   g56(.a(new_n82_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n91_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n100_), .c(new_n65_), .O(z12));
  inv1   g61(.a(x15), .O(new_n106_));
  oai21  g62(.a(new_n48_), .b(new_n106_), .c(new_n47_), .O(new_n107_));
  nand2  g63(.a(new_n46_), .b(new_n45_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n107_), .c(new_n101_), .O(new_n109_));
  nand2  g65(.a(new_n64_), .b(x04), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n109_), .O(z13));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand2  g69(.a(new_n47_), .b(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n46_), .b(new_n45_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n114_), .c(new_n113_), .O(new_n117_));
  nand2  g73(.a(new_n64_), .b(x05), .O(new_n118_));
  oai21  g74(.a(new_n117_), .b(new_n82_), .c(new_n118_), .O(z14));
  inv1   g75(.a(x25), .O(new_n120_));
  aoi21  g76(.a(x26), .b(x17), .c(new_n120_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n47_), .c(x24), .O(new_n122_));
  nand2  g78(.a(new_n114_), .b(new_n120_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n101_), .O(new_n124_));
  nand2  g80(.a(new_n64_), .b(x06), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(z15));
  inv1   g82(.a(x26), .O(new_n127_));
  and2   g83(.a(x22), .b(x21), .O(new_n128_));
  and2   g84(.a(x25), .b(x24), .O(new_n129_));
  nand4  g85(.a(new_n129_), .b(new_n128_), .c(new_n84_), .d(x23), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nor2   g87(.a(new_n46_), .b(x18), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n76_), .O(new_n133_));
  nand3  g89(.a(new_n133_), .b(new_n131_), .c(new_n81_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n49_), .c(new_n55_), .O(z16));
endmodule


