// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_, new_n63_,
    new_n66_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(z01));
  nor2   g10(.a(new_n46_), .b(x08), .O(new_n55_));
  aoi21  g11(.a(x08), .b(x01), .c(new_n55_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  nand2  g13(.a(x08), .b(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  aoi21  g15(.a(x08), .b(x03), .c(new_n55_), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  inv1   g17(.a(new_n55_), .O(new_n62_));
  nand2  g18(.a(x08), .b(x04), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n62_), .O(z05));
  and2   g20(.a(x08), .b(x05), .O(z06));
  nand2  g21(.a(x08), .b(x06), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n62_), .O(z07));
  and2   g23(.a(x08), .b(x07), .O(z08));
  inv1   g24(.a(new_n49_), .O(new_n69_));
  and2   g25(.a(x22), .b(x21), .O(new_n70_));
  nand3  g26(.a(new_n70_), .b(x20), .c(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n45_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  oai21  g29(.a(new_n53_), .b(new_n52_), .c(new_n73_), .O(z09));
  inv1   g30(.a(new_n45_), .O(new_n75_));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n70_), .b(x12), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n75_), .c(new_n77_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n48_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n80_), .c(new_n56_), .O(z10));
  nand2  g38(.a(new_n76_), .b(x21), .O(new_n83_));
  and2   g39(.a(x22), .b(x13), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n75_), .c(new_n83_), .O(new_n85_));
  oai21  g41(.a(new_n76_), .b(x21), .c(new_n69_), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n85_), .c(new_n58_), .O(z11));
  aoi21  g43(.a(new_n75_), .b(x14), .c(new_n50_), .O(new_n88_));
  inv1   g44(.a(new_n83_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(x22), .c(new_n48_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n88_), .c(new_n60_), .O(z12));
  inv1   g47(.a(x23), .O(new_n92_));
  xor2a  g48(.a(new_n50_), .b(new_n92_), .O(new_n93_));
  aoi21  g49(.a(new_n75_), .b(x15), .c(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n49_), .c(new_n63_), .O(z13));
  inv1   g51(.a(new_n48_), .O(new_n96_));
  nand4  g52(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(new_n98_));
  nand4  g54(.a(new_n70_), .b(new_n76_), .c(x24), .d(x23), .O(new_n99_));
  inv1   g55(.a(x24), .O(new_n100_));
  oai21  g56(.a(new_n50_), .b(new_n92_), .c(new_n100_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n99_), .c(new_n98_), .O(new_n102_));
  nor2   g58(.a(z06), .b(new_n55_), .O(new_n103_));
  oai21  g59(.a(new_n102_), .b(new_n96_), .c(new_n103_), .O(z14));
  nand3  g60(.a(x26), .b(x25), .c(x17), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nor2   g62(.a(new_n50_), .b(new_n92_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(x25), .c(x24), .O(new_n108_));
  inv1   g64(.a(x25), .O(new_n109_));
  nand2  g65(.a(new_n99_), .b(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n49_), .c(new_n66_), .O(z15));
  inv1   g68(.a(new_n108_), .O(new_n113_));
  nor2   g69(.a(new_n50_), .b(x18), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n75_), .c(new_n96_), .O(new_n115_));
  oai21  g71(.a(new_n113_), .b(x26), .c(new_n115_), .O(new_n116_));
  nor2   g72(.a(z08), .b(new_n55_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(z16));
endmodule


