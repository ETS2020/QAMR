// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x09), .O(new_n45_));
  inv1   g01(.a(x17), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nor2   g07(.a(x10), .b(x08), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(new_n46_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x08), .O(new_n55_));
  aoi21  g11(.a(x17), .b(x09), .c(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(new_n56_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(new_n56_), .b(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(new_n56_), .b(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  nand2  g24(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  oai21  g27(.a(new_n46_), .b(new_n45_), .c(new_n71_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand3  g31(.a(new_n52_), .b(new_n46_), .c(x09), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n57_), .O(z09));
  nand2  g35(.a(x08), .b(x01), .O(new_n80_));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n73_), .c(new_n82_), .O(new_n84_));
  inv1   g40(.a(x10), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n55_), .O(new_n86_));
  nor2   g42(.a(x20), .b(x19), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n84_), .c(x17), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n45_), .c(new_n80_), .O(z10));
  nand2  g46(.a(x08), .b(x02), .O(new_n91_));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  nand3  g49(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n50_), .c(new_n93_), .O(new_n95_));
  inv1   g51(.a(x21), .O(new_n96_));
  aoi21  g52(.a(new_n81_), .b(new_n96_), .c(new_n86_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n95_), .c(x17), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n45_), .c(new_n91_), .O(z11));
  inv1   g55(.a(new_n73_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(x14), .c(new_n48_), .O(new_n101_));
  oai21  g57(.a(new_n93_), .b(x22), .c(new_n77_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n101_), .c(new_n63_), .O(z12));
  nand2  g59(.a(new_n51_), .b(x15), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n49_), .O(new_n105_));
  aoi21  g61(.a(new_n48_), .b(new_n47_), .c(new_n76_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n65_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n48_), .b(new_n47_), .c(new_n111_), .O(new_n112_));
  inv1   g68(.a(new_n48_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(x24), .c(x23), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n76_), .c(new_n67_), .O(z14));
  nand2  g72(.a(x24), .b(x23), .O(new_n117_));
  nor2   g73(.a(new_n117_), .b(new_n48_), .O(new_n118_));
  nor2   g74(.a(new_n118_), .b(x25), .O(new_n119_));
  nand2  g75(.a(new_n118_), .b(x25), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n77_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n119_), .c(new_n69_), .O(z15));
  aoi21  g78(.a(new_n118_), .b(x25), .c(x26), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand3  g80(.a(new_n100_), .b(new_n113_), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n77_), .O(new_n126_));
  nand2  g82(.a(new_n56_), .b(x07), .O(new_n127_));
  oai21  g83(.a(new_n126_), .b(new_n123_), .c(new_n127_), .O(z16));
endmodule


