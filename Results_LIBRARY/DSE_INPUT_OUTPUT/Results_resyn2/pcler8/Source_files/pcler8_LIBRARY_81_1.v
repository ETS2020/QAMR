// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nand3  g02(.a(x25), .b(x24), .c(x23), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x14), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n49_), .c(new_n48_), .O(new_n52_));
  nor4   g08(.a(new_n52_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  nand2  g10(.a(x14), .b(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n54_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g15(.a(new_n56_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n56_), .b(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nor2   g21(.a(new_n56_), .b(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  nor2   g27(.a(new_n56_), .b(new_n71_), .O(z08));
  inv1   g28(.a(new_n52_), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n73_), .O(new_n77_));
  oai21  g33(.a(new_n56_), .b(new_n54_), .c(new_n77_), .O(z09));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  inv1   g35(.a(new_n74_), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n73_), .O(new_n84_));
  nand2  g40(.a(new_n60_), .b(x01), .O(new_n85_));
  oai21  g41(.a(new_n84_), .b(new_n83_), .c(new_n85_), .O(z10));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n80_), .c(new_n87_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n79_), .b(new_n90_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n89_), .c(new_n61_), .O(z11));
  nand2  g49(.a(x08), .b(x03), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  nand2  g51(.a(new_n87_), .b(new_n95_), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n51_), .c(new_n46_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n49_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n94_), .O(z12));
  inv1   g56(.a(new_n46_), .O(new_n101_));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x23), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n46_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n103_), .c(new_n73_), .O(new_n106_));
  oai21  g62(.a(new_n56_), .b(new_n65_), .c(new_n106_), .O(z13));
  nand2  g63(.a(x24), .b(x23), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand3  g65(.a(x26), .b(x25), .c(x16), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n101_), .O(new_n111_));
  inv1   g67(.a(x24), .O(new_n112_));
  oai21  g68(.a(new_n46_), .b(new_n104_), .c(new_n112_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n111_), .c(new_n51_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n49_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n48_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n67_), .O(z14));
  inv1   g73(.a(new_n47_), .O(new_n118_));
  nand2  g74(.a(x26), .b(x17), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n101_), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  oai21  g77(.a(new_n108_), .b(new_n46_), .c(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n51_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n49_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n48_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n69_), .O(z15));
  nand2  g82(.a(x08), .b(x07), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand3  g84(.a(new_n80_), .b(new_n101_), .c(new_n128_), .O(new_n129_));
  oai21  g85(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n51_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n49_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n48_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n127_), .O(z16));
endmodule


