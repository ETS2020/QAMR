// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  inv1   g00(.a(x18), .O(new_n45_));
  inv1   g01(.a(x24), .O(new_n46_));
  nand2  g02(.a(x26), .b(x25), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x23), .O(new_n53_));
  nand4  g09(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n52_), .c(new_n48_), .O(new_n56_));
  aoi21  g12(.a(new_n56_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g13(.a(new_n46_), .b(new_n45_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(new_n59_));
  nand2  g15(.a(x08), .b(x00), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n59_), .O(z01));
  nand2  g17(.a(x08), .b(x01), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n59_), .O(z02));
  nor2   g19(.a(new_n58_), .b(new_n49_), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(x02), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z03));
  nand2  g22(.a(x08), .b(x03), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n59_), .O(z04));
  nand2  g24(.a(new_n64_), .b(x04), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z05));
  nand2  g26(.a(x08), .b(x05), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n59_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n59_), .O(z07));
  nand2  g30(.a(new_n64_), .b(x07), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(z08));
  nand3  g32(.a(new_n48_), .b(x24), .c(x23), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(new_n60_), .c(new_n59_), .O(z09));
  inv1   g37(.a(x19), .O(new_n82_));
  inv1   g38(.a(x20), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n77_), .c(new_n84_), .O(new_n86_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n51_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n62_), .c(new_n59_), .O(z10));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  nand2  g46(.a(new_n48_), .b(x24), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  nand3  g48(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n90_), .O(new_n95_));
  oai21  g51(.a(new_n84_), .b(x21), .c(new_n52_), .O(new_n96_));
  aoi21  g52(.a(x08), .b(x02), .c(new_n58_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(z11));
  inv1   g54(.a(new_n90_), .O(new_n99_));
  nor2   g55(.a(new_n99_), .b(x22), .O(new_n100_));
  nand2  g56(.a(new_n54_), .b(new_n52_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n100_), .c(new_n67_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n59_), .O(new_n103_));
  nand4  g59(.a(x23), .b(x22), .c(new_n45_), .d(x14), .O(new_n104_));
  nor2   g60(.a(new_n104_), .b(new_n51_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n92_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n103_), .O(z12));
  inv1   g63(.a(new_n55_), .O(new_n108_));
  nand2  g64(.a(new_n54_), .b(new_n53_), .O(new_n109_));
  nand2  g65(.a(new_n59_), .b(new_n52_), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  inv1   g68(.a(new_n54_), .O(new_n113_));
  inv1   g69(.a(x15), .O(new_n114_));
  nor2   g70(.a(x18), .b(new_n114_), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n92_), .c(new_n113_), .d(new_n52_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n112_), .c(new_n69_), .O(z13));
  nand2  g73(.a(new_n48_), .b(x16), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n55_), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n108_), .b(new_n46_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n111_), .O(new_n121_));
  nand2  g77(.a(new_n64_), .b(x05), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n121_), .O(z14));
  inv1   g79(.a(x25), .O(new_n124_));
  nand3  g80(.a(new_n55_), .b(new_n52_), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n45_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(x24), .O(new_n127_));
  nand2  g83(.a(x26), .b(x17), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n55_), .c(x24), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n52_), .c(x25), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n127_), .c(new_n73_), .O(z15));
  inv1   g87(.a(x26), .O(new_n132_));
  nand3  g88(.a(new_n55_), .b(x25), .c(x24), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nor3   g90(.a(new_n54_), .b(new_n47_), .c(new_n53_), .O(new_n135_));
  oai21  g91(.a(new_n135_), .b(x18), .c(x24), .O(new_n136_));
  nand3  g92(.a(new_n136_), .b(new_n134_), .c(new_n52_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n75_), .O(z16));
endmodule


