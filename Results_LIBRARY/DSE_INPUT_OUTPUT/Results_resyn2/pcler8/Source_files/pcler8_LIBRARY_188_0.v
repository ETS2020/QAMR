// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  nand2  g03(.a(x09), .b(new_n47_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n49_), .c(new_n46_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x18), .c(x10), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  inv1   g10(.a(x10), .O(new_n55_));
  inv1   g11(.a(x18), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n54_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  oai21  g16(.a(new_n47_), .b(new_n60_), .c(new_n57_), .O(z02));
  inv1   g17(.a(x02), .O(new_n62_));
  nor2   g18(.a(new_n58_), .b(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  oai21  g22(.a(new_n47_), .b(new_n66_), .c(new_n57_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n57_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n47_), .b(new_n70_), .c(new_n57_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  nor2   g28(.a(new_n58_), .b(new_n72_), .O(z08));
  and2   g29(.a(x22), .b(x21), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n45_), .c(x19), .O(new_n76_));
  nand3  g32(.a(new_n49_), .b(x18), .c(new_n55_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g35(.a(new_n58_), .b(new_n54_), .c(new_n79_), .O(z09));
  and2   g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand2  g38(.a(new_n74_), .b(x12), .O(new_n83_));
  inv1   g39(.a(new_n83_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n46_), .c(new_n82_), .O(new_n85_));
  oai21  g41(.a(x20), .b(x19), .c(new_n78_), .O(new_n86_));
  oai22  g42(.a(new_n86_), .b(new_n85_), .c(new_n58_), .d(new_n60_), .O(z10));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  nand2  g45(.a(x22), .b(x13), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n45_), .c(new_n89_), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  aoi21  g48(.a(new_n82_), .b(new_n92_), .c(new_n48_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n56_), .O(new_n94_));
  oai22  g50(.a(new_n94_), .b(x10), .c(new_n47_), .d(new_n62_), .O(z11));
  aoi21  g51(.a(new_n46_), .b(x14), .c(new_n50_), .O(new_n96_));
  oai21  g52(.a(new_n89_), .b(x22), .c(new_n78_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n96_), .c(new_n58_), .d(new_n64_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n45_), .b(new_n99_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nor2   g57(.a(new_n50_), .b(new_n101_), .O(new_n102_));
  nor2   g58(.a(new_n51_), .b(x23), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n100_), .c(new_n78_), .O(new_n105_));
  oai21  g61(.a(new_n58_), .b(new_n66_), .c(new_n105_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nand4  g64(.a(new_n74_), .b(new_n81_), .c(x24), .d(x23), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n50_), .b(new_n101_), .c(new_n110_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n48_), .c(x18), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n55_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n68_), .O(z14));
  inv1   g71(.a(x25), .O(new_n116_));
  inv1   g72(.a(x26), .O(new_n117_));
  nor2   g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g74(.a(new_n102_), .b(x25), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n109_), .b(new_n116_), .O(new_n120_));
  aoi22  g76(.a(new_n120_), .b(new_n119_), .c(new_n118_), .d(x17), .O(new_n121_));
  oai22  g77(.a(new_n121_), .b(new_n77_), .c(new_n58_), .d(new_n70_), .O(z15));
  nand2  g78(.a(x08), .b(x07), .O(new_n123_));
  aoi21  g79(.a(new_n119_), .b(new_n117_), .c(new_n48_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n56_), .c(new_n55_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n123_), .O(z16));
endmodule


