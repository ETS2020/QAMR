// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x16), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x25), .O(new_n51_));
  inv1   g07(.a(x26), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  inv1   g09(.a(x08), .O(new_n54_));
  inv1   g10(.a(x10), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand4  g13(.a(new_n57_), .b(new_n53_), .c(new_n50_), .d(x24), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n47_), .O(z00));
  nand3  g15(.a(new_n47_), .b(x08), .c(x00), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z01));
  nand2  g17(.a(x08), .b(x01), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n46_), .O(z02));
  nand3  g19(.a(new_n47_), .b(x08), .c(x02), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n47_), .O(z05));
  nand2  g25(.a(x08), .b(x05), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n47_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n47_), .O(z08));
  nor2   g31(.a(new_n56_), .b(new_n46_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  inv1   g33(.a(x19), .O(new_n78_));
  and2   g34(.a(x22), .b(x21), .O(new_n79_));
  nand4  g35(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  and2   g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  and2   g38(.a(x20), .b(x11), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n77_), .c(new_n60_), .O(z09));
  and2   g41(.a(x20), .b(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n82_), .b(x12), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(x20), .b(x19), .c(new_n57_), .O(new_n89_));
  aoi21  g45(.a(x08), .b(x01), .c(new_n46_), .O(new_n90_));
  oai21  g46(.a(new_n89_), .b(new_n88_), .c(new_n90_), .O(z10));
  nand2  g47(.a(new_n86_), .b(x21), .O(new_n92_));
  and2   g48(.a(x22), .b(x13), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n81_), .c(new_n92_), .O(new_n94_));
  oai21  g50(.a(new_n86_), .b(x21), .c(new_n76_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n94_), .c(new_n64_), .O(z11));
  inv1   g52(.a(z04), .O(new_n97_));
  nand3  g53(.a(new_n86_), .b(x22), .c(x21), .O(new_n98_));
  aoi21  g54(.a(new_n81_), .b(x14), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(new_n92_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(x22), .c(new_n57_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(z12));
  inv1   g58(.a(x15), .O(new_n103_));
  nor2   g59(.a(new_n80_), .b(new_n103_), .O(new_n104_));
  xor2a  g60(.a(new_n49_), .b(new_n48_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n104_), .c(new_n57_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n68_), .c(new_n46_), .O(z13));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n50_), .b(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n53_), .b(x24), .c(x16), .O(new_n110_));
  nand3  g66(.a(new_n79_), .b(new_n86_), .c(x23), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(x24), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n57_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n70_), .c(new_n47_), .O(z14));
  and2   g71(.a(x24), .b(x23), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n79_), .c(new_n86_), .d(x25), .O(new_n117_));
  nand2  g73(.a(x24), .b(x23), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n49_), .c(new_n51_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n117_), .c(new_n57_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n72_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  inv1   g78(.a(x09), .O(new_n123_));
  nor2   g79(.a(new_n123_), .b(x08), .O(new_n124_));
  inv1   g80(.a(x16), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(x10), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n124_), .c(new_n53_), .d(x17), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n122_), .O(z15));
  inv1   g84(.a(z08), .O(new_n129_));
  and2   g85(.a(new_n117_), .b(new_n52_), .O(new_n130_));
  inv1   g86(.a(x18), .O(new_n131_));
  nand3  g87(.a(new_n53_), .b(x24), .c(new_n131_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n111_), .c(new_n57_), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n130_), .c(new_n129_), .O(z16));
endmodule


