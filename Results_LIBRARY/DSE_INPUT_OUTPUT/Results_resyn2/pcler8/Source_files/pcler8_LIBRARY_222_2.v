// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(x04), .O(new_n47_));
  nor2   g03(.a(x17), .b(new_n47_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  nor4   g07(.a(new_n51_), .b(new_n48_), .c(new_n46_), .d(new_n45_), .O(z00));
  nand2  g08(.a(x08), .b(x00), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n48_), .O(z01));
  inv1   g10(.a(x17), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x04), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(x08), .c(x01), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z02));
  nand2  g14(.a(x08), .b(x02), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n48_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n56_), .O(z04));
  nand3  g18(.a(x17), .b(x08), .c(x04), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n48_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n56_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n56_), .O(z08));
  inv1   g26(.a(new_n51_), .O(new_n71_));
  nand4  g27(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n45_), .c(x19), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(new_n53_), .c(new_n56_), .O(z09));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x26), .b(x25), .c(x24), .O(new_n77_));
  nand2  g33(.a(x23), .b(x22), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g35(.a(x21), .b(x12), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(new_n81_));
  nor2   g37(.a(new_n51_), .b(new_n48_), .O(new_n82_));
  or2    g38(.a(x20), .b(x19), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n57_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  aoi21  g42(.a(new_n79_), .b(x13), .c(new_n86_), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  nand2  g44(.a(new_n76_), .b(new_n88_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n71_), .O(new_n90_));
  and2   g46(.a(new_n59_), .b(new_n56_), .O(new_n91_));
  oai21  g47(.a(new_n90_), .b(new_n87_), .c(new_n91_), .O(z11));
  inv1   g48(.a(z04), .O(new_n93_));
  inv1   g49(.a(new_n45_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(x14), .c(new_n46_), .O(new_n95_));
  inv1   g51(.a(new_n86_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x22), .c(new_n71_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n95_), .c(new_n93_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n78_), .b(new_n86_), .O(new_n100_));
  oai21  g56(.a(new_n77_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nand2  g58(.a(new_n46_), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n101_), .c(new_n82_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n63_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n78_), .b(new_n86_), .c(new_n108_), .O(new_n109_));
  nand2  g65(.a(x24), .b(x23), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n96_), .c(x22), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n109_), .c(new_n107_), .O(new_n113_));
  and2   g69(.a(new_n65_), .b(new_n56_), .O(new_n114_));
  oai21  g70(.a(new_n113_), .b(new_n51_), .c(new_n114_), .O(z14));
  nor2   g71(.a(new_n110_), .b(new_n46_), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  aoi21  g73(.a(x26), .b(x17), .c(new_n117_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g75(.a(new_n112_), .b(new_n117_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n71_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n67_), .c(new_n56_), .O(z15));
  inv1   g78(.a(z08), .O(new_n123_));
  aoi21  g79(.a(new_n116_), .b(x25), .c(x26), .O(new_n124_));
  nor2   g80(.a(new_n86_), .b(x18), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n79_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n71_), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n124_), .c(new_n123_), .O(z16));
endmodule


