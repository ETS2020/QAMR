// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand3  g04(.a(x21), .b(x20), .c(x19), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(new_n51_));
  aoi21  g07(.a(new_n51_), .b(x22), .c(x10), .O(z00));
  inv1   g08(.a(x00), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  inv1   g10(.a(x22), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n53_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n56_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n56_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n57_), .b(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n56_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  nor2   g23(.a(new_n57_), .b(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n56_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n56_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  nand3  g29(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n46_), .c(new_n73_), .O(new_n76_));
  nand3  g32(.a(new_n48_), .b(x22), .c(new_n54_), .O(new_n77_));
  oai22  g33(.a(new_n77_), .b(new_n76_), .c(new_n57_), .d(new_n53_), .O(z09));
  inv1   g34(.a(x20), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand2  g36(.a(x21), .b(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n45_), .c(new_n80_), .O(new_n82_));
  inv1   g38(.a(x08), .O(new_n83_));
  nand2  g39(.a(x09), .b(new_n83_), .O(new_n84_));
  aoi21  g40(.a(new_n79_), .b(new_n73_), .c(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n82_), .c(new_n55_), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(x10), .c(new_n59_), .O(z10));
  aoi21  g43(.a(new_n46_), .b(x13), .c(new_n49_), .O(new_n88_));
  inv1   g44(.a(new_n77_), .O(new_n89_));
  oai21  g45(.a(new_n80_), .b(x21), .c(new_n89_), .O(new_n90_));
  inv1   g46(.a(new_n57_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x02), .O(new_n92_));
  oai21  g48(.a(new_n90_), .b(new_n88_), .c(new_n92_), .O(z11));
  inv1   g49(.a(x14), .O(new_n94_));
  oai21  g50(.a(new_n45_), .b(new_n94_), .c(new_n50_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n48_), .c(new_n55_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(x10), .c(new_n83_), .d(new_n63_), .O(z12));
  nand4  g53(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(x23), .O(new_n99_));
  nand3  g55(.a(x21), .b(x20), .c(x19), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand2  g58(.a(new_n49_), .b(x23), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n102_), .c(new_n84_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n55_), .c(new_n54_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n65_), .O(z13));
  nand2  g62(.a(x08), .b(x05), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  nand4  g64(.a(x23), .b(x21), .c(x20), .d(x19), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x24), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(x16), .O(new_n114_));
  nand2  g70(.a(new_n109_), .b(x24), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n111_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n48_), .c(new_n55_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(x10), .c(new_n107_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  inv1   g75(.a(x26), .O(new_n120_));
  nor2   g76(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g77(.a(new_n50_), .b(x25), .c(x24), .d(x23), .O(new_n122_));
  nand2  g78(.a(x24), .b(x23), .O(new_n123_));
  oai21  g79(.a(new_n49_), .b(new_n123_), .c(new_n119_), .O(new_n124_));
  aoi22  g80(.a(new_n124_), .b(new_n122_), .c(new_n121_), .d(x17), .O(new_n125_));
  nand2  g81(.a(new_n91_), .b(x06), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n77_), .c(new_n126_), .O(z15));
  nand2  g83(.a(new_n122_), .b(new_n120_), .O(new_n128_));
  nor2   g84(.a(new_n112_), .b(x18), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n110_), .c(new_n84_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n128_), .c(new_n55_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(x10), .c(new_n71_), .O(z16));
endmodule


