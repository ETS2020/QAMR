// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g00(.a(x17), .b(x15), .O(new_n45_));
  nand2  g01(.a(x26), .b(x25), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand2  g06(.a(x24), .b(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  and2   g12(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  inv1   g14(.a(new_n45_), .O(new_n59_));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  and2   g18(.a(new_n62_), .b(new_n45_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  and2   g21(.a(new_n65_), .b(new_n45_), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n59_), .O(z05));
  nand3  g25(.a(new_n45_), .b(x08), .c(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n45_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nor2   g30(.a(new_n74_), .b(new_n59_), .O(z08));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g35(.a(x20), .b(x11), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n49_), .c(new_n57_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  aoi21  g39(.a(new_n79_), .b(x12), .c(new_n83_), .O(new_n84_));
  inv1   g40(.a(new_n49_), .O(new_n85_));
  oai21  g41(.a(x20), .b(x19), .c(new_n85_), .O(new_n86_));
  and2   g42(.a(new_n60_), .b(new_n45_), .O(new_n87_));
  oai21  g43(.a(new_n86_), .b(new_n84_), .c(new_n87_), .O(z10));
  inv1   g44(.a(new_n78_), .O(new_n89_));
  inv1   g45(.a(new_n83_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x21), .O(new_n91_));
  and2   g47(.a(x22), .b(x13), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n89_), .c(new_n91_), .O(new_n93_));
  oai21  g49(.a(new_n90_), .b(x21), .c(new_n85_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n93_), .c(new_n63_), .O(z11));
  nand3  g51(.a(new_n90_), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g52(.a(new_n89_), .b(x14), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(new_n91_), .b(new_n98_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n85_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n97_), .c(new_n66_), .O(z12));
  inv1   g57(.a(x17), .O(new_n102_));
  inv1   g58(.a(new_n46_), .O(new_n103_));
  inv1   g59(.a(x09), .O(new_n104_));
  nor2   g60(.a(new_n104_), .b(x08), .O(new_n105_));
  inv1   g61(.a(new_n51_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(new_n48_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n102_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(x15), .O(new_n109_));
  inv1   g65(.a(new_n52_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(x23), .O(new_n111_));
  inv1   g67(.a(x23), .O(new_n112_));
  aoi21  g68(.a(new_n52_), .b(new_n112_), .c(new_n49_), .O(new_n113_));
  aoi22  g69(.a(new_n113_), .b(new_n111_), .c(x08), .d(x04), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n109_), .O(z13));
  nand2  g71(.a(new_n110_), .b(new_n106_), .O(new_n116_));
  inv1   g72(.a(x24), .O(new_n117_));
  oai21  g73(.a(new_n52_), .b(new_n112_), .c(new_n117_), .O(new_n118_));
  nand4  g74(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  aoi21  g76(.a(new_n118_), .b(new_n116_), .c(new_n120_), .O(new_n121_));
  nand2  g77(.a(new_n85_), .b(new_n45_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n121_), .c(new_n70_), .O(z14));
  inv1   g79(.a(x25), .O(new_n124_));
  nand2  g80(.a(new_n116_), .b(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n110_), .b(new_n106_), .c(x25), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n125_), .c(new_n85_), .O(new_n127_));
  oai21  g83(.a(new_n50_), .b(x15), .c(x17), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n127_), .c(new_n72_), .O(z15));
  inv1   g85(.a(x26), .O(new_n130_));
  nor2   g86(.a(new_n130_), .b(x18), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n53_), .c(x25), .O(new_n132_));
  nand2  g88(.a(new_n126_), .b(new_n130_), .O(new_n133_));
  nand3  g89(.a(new_n133_), .b(new_n132_), .c(new_n85_), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n74_), .c(new_n45_), .O(z16));
endmodule


