// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n87_, new_n89_, new_n90_, new_n92_;
  inv1   g00(.a(x00), .O(new_n46_));
  inv1   g01(.a(x10), .O(new_n47_));
  nand2  g02(.a(x22), .b(new_n47_), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z01));
  nand2  g05(.a(x08), .b(x01), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n48_), .O(z02));
  inv1   g07(.a(x02), .O(new_n53_));
  nor2   g08(.a(new_n49_), .b(new_n53_), .O(z03));
  nand2  g09(.a(x08), .b(x03), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n48_), .O(z04));
  inv1   g11(.a(x04), .O(new_n57_));
  nor2   g12(.a(new_n49_), .b(new_n57_), .O(z05));
  inv1   g13(.a(x05), .O(new_n59_));
  inv1   g14(.a(x08), .O(new_n60_));
  oai21  g15(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(z06));
  inv1   g16(.a(x06), .O(new_n62_));
  nor2   g17(.a(new_n49_), .b(new_n62_), .O(z07));
  inv1   g18(.a(x07), .O(new_n64_));
  oai21  g19(.a(new_n60_), .b(new_n64_), .c(new_n48_), .O(z08));
  inv1   g20(.a(x22), .O(new_n66_));
  inv1   g21(.a(x09), .O(new_n67_));
  nor2   g22(.a(new_n67_), .b(x08), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(new_n69_));
  oai22  g24(.a(new_n69_), .b(x19), .c(new_n49_), .d(new_n46_), .O(z09));
  nor2   g25(.a(x20), .b(x19), .O(new_n71_));
  nand2  g26(.a(x20), .b(x19), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(x09), .c(new_n60_), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n71_), .c(new_n66_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n51_), .O(z10));
  aoi21  g31(.a(x20), .b(x19), .c(x21), .O(new_n77_));
  nand3  g32(.a(x21), .b(x20), .c(x19), .O(new_n78_));
  nand4  g33(.a(new_n78_), .b(new_n68_), .c(new_n66_), .d(new_n47_), .O(new_n79_));
  oai22  g34(.a(new_n79_), .b(new_n77_), .c(new_n49_), .d(new_n53_), .O(z11));
  inv1   g35(.a(new_n78_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n68_), .c(x22), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x10), .c(new_n55_), .O(z12));
  nand2  g38(.a(x08), .b(x04), .O(new_n84_));
  aoi21  g39(.a(new_n68_), .b(x23), .c(x22), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x10), .c(new_n84_), .O(z13));
  inv1   g41(.a(x24), .O(new_n87_));
  oai22  g42(.a(new_n69_), .b(new_n87_), .c(new_n49_), .d(new_n59_), .O(z14));
  nand2  g43(.a(x08), .b(x06), .O(new_n89_));
  aoi21  g44(.a(new_n68_), .b(x25), .c(x22), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x10), .c(new_n89_), .O(z15));
  inv1   g46(.a(x26), .O(new_n92_));
  oai22  g47(.a(new_n69_), .b(new_n92_), .c(new_n49_), .d(new_n64_), .O(z16));
  zero   g48(.O(z00));
endmodule


