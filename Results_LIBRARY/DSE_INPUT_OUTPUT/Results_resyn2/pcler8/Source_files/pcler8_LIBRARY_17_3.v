// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand2  g03(.a(x06), .b(x05), .O(new_n48_));
  nand4  g04(.a(new_n48_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g07(.a(x08), .b(x00), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n48_), .O(z01));
  inv1   g09(.a(new_n48_), .O(new_n54_));
  nand2  g10(.a(x08), .b(x01), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(z02));
  nand2  g12(.a(x08), .b(x02), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n48_), .O(z03));
  nand3  g14(.a(new_n48_), .b(x08), .c(x03), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z04));
  nand2  g16(.a(x08), .b(x04), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n48_), .O(z05));
  inv1   g18(.a(x06), .O(new_n63_));
  nand3  g19(.a(x08), .b(new_n63_), .c(x05), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  inv1   g21(.a(x05), .O(new_n66_));
  nand3  g22(.a(x08), .b(x06), .c(new_n66_), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n48_), .O(z08));
  nand3  g26(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n45_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n72_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n52_), .c(new_n48_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n45_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g38(.a(new_n82_), .b(new_n55_), .c(new_n48_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(x20), .b(x19), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n89_), .c(new_n71_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n57_), .c(new_n54_), .O(z11));
  inv1   g49(.a(new_n45_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(x14), .c(new_n50_), .O(new_n95_));
  inv1   g51(.a(new_n49_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n84_), .b(new_n97_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n95_), .c(new_n59_), .O(z12));
  inv1   g56(.a(new_n50_), .O(new_n101_));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x23), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n50_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n103_), .c(new_n72_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n61_), .c(new_n48_), .O(z13));
  oai21  g63(.a(x08), .b(x06), .c(x05), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  nand2  g67(.a(x23), .b(x22), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n84_), .c(new_n111_), .O(new_n113_));
  and2   g69(.a(x24), .b(x23), .O(new_n114_));
  and2   g70(.a(x22), .b(x21), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n77_), .c(new_n114_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n113_), .c(new_n110_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n71_), .c(new_n108_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n101_), .c(new_n114_), .O(new_n121_));
  nand2  g77(.a(new_n116_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n96_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n67_), .O(z15));
  nor2   g80(.a(new_n112_), .b(new_n84_), .O(new_n125_));
  nor2   g81(.a(new_n86_), .b(x18), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  inv1   g83(.a(x26), .O(new_n128_));
  nand3  g84(.a(x25), .b(x24), .c(x23), .O(new_n129_));
  oai21  g85(.a(new_n50_), .b(new_n129_), .c(new_n128_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n127_), .c(new_n72_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n69_), .c(new_n54_), .O(z16));
endmodule


