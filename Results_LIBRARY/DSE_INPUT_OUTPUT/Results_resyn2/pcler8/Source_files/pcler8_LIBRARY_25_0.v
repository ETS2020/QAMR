// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  and2   g00(.a(x20), .b(x19), .O(new_n45_));
  and2   g01(.a(x22), .b(x21), .O(new_n46_));
  nand4  g02(.a(new_n46_), .b(new_n45_), .c(x24), .d(x23), .O(new_n47_));
  and2   g03(.a(x26), .b(x25), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(x11), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n47_), .O(z00));
  nor2   g12(.a(x11), .b(x08), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n58_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n58_), .O(z04));
  and2   g22(.a(x08), .b(x04), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n58_), .O(z07));
  and2   g27(.a(x08), .b(x07), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  inv1   g29(.a(x23), .O(new_n74_));
  nand2  g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(x26), .b(x25), .c(x24), .O(new_n76_));
  nor3   g32(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(x20), .c(new_n73_), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n53_), .c(new_n59_), .O(z09));
  inv1   g35(.a(new_n45_), .O(new_n80_));
  aoi21  g36(.a(new_n77_), .b(x12), .c(new_n80_), .O(new_n81_));
  inv1   g37(.a(x20), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n54_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n61_), .O(z10));
  nand2  g41(.a(new_n45_), .b(x21), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n76_), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n80_), .b(new_n90_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n54_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n89_), .c(new_n63_), .O(z11));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n94_));
  inv1   g50(.a(new_n76_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(x23), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(x14), .c(new_n94_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n86_), .b(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n54_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n98_), .c(new_n65_), .O(z12));
  nor2   g58(.a(new_n94_), .b(new_n74_), .O(new_n103_));
  nand2  g59(.a(new_n95_), .b(x15), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g61(.a(new_n94_), .b(new_n74_), .c(new_n51_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nor2   g63(.a(z05), .b(new_n57_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(z13));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n94_), .b(new_n74_), .c(new_n110_), .O(new_n111_));
  aoi22  g67(.a(new_n111_), .b(new_n47_), .c(new_n95_), .d(x16), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n53_), .c(new_n68_), .O(z14));
  nand2  g69(.a(new_n48_), .b(x17), .O(new_n114_));
  inv1   g70(.a(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n103_), .b(x25), .c(x24), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n47_), .b(new_n117_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n116_), .c(new_n115_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n53_), .c(new_n70_), .O(z15));
  inv1   g76(.a(new_n116_), .O(new_n121_));
  nor2   g77(.a(new_n76_), .b(x18), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n103_), .c(new_n51_), .O(new_n123_));
  oai21  g79(.a(new_n121_), .b(x26), .c(new_n123_), .O(new_n124_));
  nor2   g80(.a(z08), .b(new_n57_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(z16));
endmodule


