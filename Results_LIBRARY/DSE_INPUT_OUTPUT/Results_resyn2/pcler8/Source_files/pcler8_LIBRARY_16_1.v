// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x13), .O(new_n45_));
  nand2  g01(.a(x17), .b(new_n45_), .O(new_n46_));
  nand3  g02(.a(x21), .b(x20), .c(x19), .O(new_n47_));
  nand3  g03(.a(x26), .b(x25), .c(x24), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  inv1   g07(.a(x22), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x10), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n49_), .d(x23), .O(new_n54_));
  oai21  g10(.a(new_n54_), .b(new_n47_), .c(new_n46_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z01));
  inv1   g13(.a(new_n46_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z03));
  nand3  g18(.a(new_n46_), .b(x08), .c(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n58_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n58_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n58_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  inv1   g31(.a(x08), .O(new_n76_));
  inv1   g32(.a(x10), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x09), .c(new_n76_), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n56_), .c(new_n58_), .O(z09));
  inv1   g37(.a(x19), .O(new_n82_));
  inv1   g38(.a(x20), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n73_), .c(new_n84_), .O(new_n86_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n59_), .c(new_n46_), .O(z10));
  aoi21  g45(.a(x20), .b(x19), .c(x21), .O(new_n90_));
  nand4  g46(.a(new_n47_), .b(new_n77_), .c(x09), .d(new_n76_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n90_), .c(new_n61_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n46_), .O(new_n93_));
  nand2  g49(.a(x21), .b(x13), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n54_), .c(new_n93_), .O(z11));
  inv1   g51(.a(new_n73_), .O(new_n96_));
  nand4  g52(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n97_));
  aoi21  g53(.a(new_n96_), .b(x14), .c(new_n97_), .O(new_n98_));
  nand2  g54(.a(new_n47_), .b(new_n52_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n79_), .c(new_n46_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n63_), .O(z12));
  inv1   g57(.a(new_n97_), .O(new_n102_));
  nand2  g58(.a(new_n49_), .b(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n102_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  aoi21  g61(.a(new_n97_), .b(new_n105_), .c(new_n78_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n65_), .c(new_n46_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  nand3  g66(.a(new_n102_), .b(x24), .c(x23), .O(new_n111_));
  inv1   g67(.a(x24), .O(new_n112_));
  oai21  g68(.a(new_n97_), .b(new_n105_), .c(new_n112_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  and2   g70(.a(new_n67_), .b(new_n46_), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n78_), .c(new_n115_), .O(z14));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n111_), .b(new_n117_), .O(new_n118_));
  nand4  g74(.a(new_n102_), .b(x25), .c(x24), .d(x23), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n79_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x25), .O(new_n121_));
  oai21  g77(.a(new_n78_), .b(new_n121_), .c(x13), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(x17), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n120_), .c(new_n69_), .O(z15));
  inv1   g80(.a(z08), .O(new_n125_));
  inv1   g81(.a(x26), .O(new_n126_));
  nand2  g82(.a(new_n119_), .b(new_n126_), .O(new_n127_));
  nor2   g83(.a(new_n97_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n96_), .c(new_n78_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n125_), .O(z16));
endmodule


