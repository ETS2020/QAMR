// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nor2   g00(.a(x17), .b(x11), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  aoi21  g13(.a(x08), .b(x00), .c(new_n45_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n45_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n45_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n46_), .O(z04));
  aoi21  g21(.a(x08), .b(x04), .c(new_n45_), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  aoi21  g23(.a(x08), .b(x05), .c(new_n45_), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  aoi21  g25(.a(x08), .b(x06), .c(new_n45_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  and2   g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n74_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n58_), .O(z09));
  inv1   g35(.a(x19), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  inv1   g38(.a(new_n74_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n75_), .c(x12), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  aoi21  g41(.a(new_n81_), .b(new_n80_), .c(new_n54_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n60_), .c(new_n46_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n83_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n82_), .b(x21), .c(new_n55_), .O(new_n92_));
  aoi21  g48(.a(x08), .b(x02), .c(new_n45_), .O(new_n93_));
  oai21  g49(.a(new_n92_), .b(new_n91_), .c(new_n93_), .O(z11));
  inv1   g50(.a(x14), .O(new_n95_));
  inv1   g51(.a(new_n48_), .O(new_n96_));
  oai21  g52(.a(new_n74_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  aoi21  g54(.a(new_n89_), .b(new_n98_), .c(new_n54_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n64_), .c(new_n45_), .O(z12));
  nand2  g57(.a(new_n51_), .b(x15), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n49_), .O(new_n103_));
  aoi21  g59(.a(new_n48_), .b(new_n47_), .c(new_n54_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n66_), .O(z13));
  nand2  g62(.a(new_n51_), .b(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n48_), .b(new_n47_), .c(new_n109_), .O(new_n110_));
  nand2  g66(.a(new_n49_), .b(x24), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n54_), .c(new_n68_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  aoi21  g70(.a(x26), .b(x17), .c(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n49_), .c(x24), .O(new_n116_));
  nand2  g72(.a(new_n111_), .b(new_n114_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n55_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n70_), .O(z15));
  inv1   g75(.a(x26), .O(new_n120_));
  nand4  g76(.a(new_n96_), .b(x25), .c(x24), .d(x23), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nor2   g78(.a(new_n48_), .b(x18), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n83_), .c(new_n54_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n72_), .c(new_n45_), .O(z16));
endmodule


