// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x03), .O(new_n45_));
  nor2   g01(.a(x05), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g06(.a(x26), .b(x25), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand4  g12(.a(new_n56_), .b(new_n52_), .c(new_n50_), .d(x24), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand3  g14(.a(new_n47_), .b(x08), .c(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n46_), .O(z03));
  aoi21  g20(.a(new_n53_), .b(x05), .c(new_n45_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n47_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  nand2  g29(.a(new_n56_), .b(new_n47_), .O(new_n74_));
  inv1   g30(.a(x19), .O(new_n75_));
  nand2  g31(.a(x22), .b(x21), .O(new_n76_));
  nand2  g32(.a(x24), .b(x23), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n52_), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n75_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n74_), .c(new_n59_), .O(z09));
  inv1   g39(.a(x20), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n75_), .O(new_n85_));
  inv1   g41(.a(new_n76_), .O(new_n86_));
  nand4  g42(.a(new_n78_), .b(new_n52_), .c(new_n86_), .d(x12), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  aoi21  g44(.a(new_n84_), .b(new_n75_), .c(new_n55_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n61_), .c(new_n46_), .O(z10));
  inv1   g47(.a(new_n79_), .O(new_n92_));
  nand2  g48(.a(new_n85_), .b(x21), .O(new_n93_));
  and2   g49(.a(x22), .b(x13), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n93_), .O(new_n95_));
  oai21  g51(.a(new_n85_), .b(x21), .c(new_n56_), .O(new_n96_));
  aoi21  g52(.a(x08), .b(x02), .c(new_n46_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(z11));
  aoi21  g54(.a(new_n92_), .b(x14), .c(new_n49_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  nand2  g56(.a(new_n93_), .b(new_n100_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n56_), .c(new_n47_), .O(new_n102_));
  oai22  g58(.a(new_n102_), .b(new_n99_), .c(new_n68_), .d(new_n45_), .O(z12));
  nand3  g59(.a(new_n52_), .b(x24), .c(x15), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n50_), .O(new_n105_));
  aoi21  g61(.a(new_n49_), .b(new_n48_), .c(new_n55_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n66_), .c(new_n47_), .O(z13));
  nand3  g64(.a(new_n52_), .b(x24), .c(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n49_), .b(new_n48_), .c(new_n111_), .O(new_n112_));
  inv1   g68(.a(new_n49_), .O(new_n113_));
  nand2  g69(.a(new_n78_), .b(new_n113_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  inv1   g71(.a(x05), .O(new_n116_));
  oai21  g72(.a(x08), .b(x03), .c(new_n116_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n54_), .c(x09), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n115_), .c(new_n68_), .O(z14));
  nand2  g75(.a(new_n52_), .b(x17), .O(new_n120_));
  inv1   g76(.a(new_n120_), .O(new_n121_));
  nor2   g77(.a(new_n77_), .b(new_n49_), .O(new_n122_));
  xor2a  g78(.a(new_n122_), .b(x25), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n121_), .c(new_n56_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n70_), .c(new_n46_), .O(z15));
  inv1   g81(.a(x25), .O(new_n126_));
  nor2   g82(.a(new_n114_), .b(new_n126_), .O(new_n127_));
  nor2   g83(.a(new_n51_), .b(x18), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n122_), .c(new_n55_), .O(new_n129_));
  oai21  g85(.a(new_n127_), .b(x26), .c(new_n129_), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


