// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x17), .b(x01), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand2  g03(.a(x24), .b(x23), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand2  g05(.a(x26), .b(x25), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n49_), .c(new_n47_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  and2   g13(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  inv1   g15(.a(x17), .O(new_n60_));
  nand3  g16(.a(new_n60_), .b(x08), .c(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  nand3  g18(.a(new_n45_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  inv1   g20(.a(new_n45_), .O(new_n65_));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n65_), .O(z05));
  nand3  g25(.a(new_n45_), .b(x08), .c(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  and2   g27(.a(x08), .b(x06), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n45_), .O(z07));
  nand2  g30(.a(x08), .b(x07), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n65_), .O(z08));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n77_), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n53_), .c(new_n58_), .O(z09));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  aoi21  g40(.a(new_n80_), .b(x12), .c(new_n84_), .O(new_n85_));
  inv1   g41(.a(new_n53_), .O(new_n86_));
  inv1   g42(.a(x20), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n77_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n86_), .c(new_n45_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n85_), .c(new_n61_), .O(z10));
  inv1   g46(.a(new_n79_), .O(new_n91_));
  inv1   g47(.a(new_n84_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(x21), .O(new_n93_));
  and2   g49(.a(x22), .b(x13), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n91_), .c(new_n93_), .O(new_n95_));
  inv1   g51(.a(x21), .O(new_n96_));
  nand2  g52(.a(new_n84_), .b(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n86_), .c(new_n45_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n95_), .c(new_n63_), .O(z11));
  nand3  g55(.a(new_n92_), .b(x22), .c(x21), .O(new_n100_));
  aoi21  g56(.a(new_n91_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n93_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  and2   g60(.a(new_n66_), .b(new_n45_), .O(new_n105_));
  oai21  g61(.a(new_n104_), .b(new_n101_), .c(new_n105_), .O(z12));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n47_), .c(x23), .O(new_n108_));
  inv1   g64(.a(x23), .O(new_n109_));
  nand2  g65(.a(new_n46_), .b(new_n109_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n108_), .c(new_n86_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n68_), .c(new_n45_), .O(z13));
  nand2  g68(.a(new_n86_), .b(new_n45_), .O(new_n113_));
  nand2  g69(.a(new_n49_), .b(new_n47_), .O(new_n114_));
  nand4  g70(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  inv1   g72(.a(x24), .O(new_n117_));
  oai21  g73(.a(new_n46_), .b(new_n109_), .c(new_n117_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n114_), .c(new_n116_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n113_), .c(new_n70_), .O(z14));
  inv1   g76(.a(x25), .O(new_n121_));
  oai21  g77(.a(new_n48_), .b(new_n46_), .c(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n49_), .b(new_n47_), .c(x25), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n86_), .O(new_n124_));
  aoi21  g80(.a(new_n54_), .b(x17), .c(new_n72_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n124_), .c(new_n65_), .O(z15));
  inv1   g82(.a(x26), .O(new_n127_));
  nor2   g83(.a(new_n127_), .b(x18), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n49_), .c(new_n47_), .d(x25), .O(new_n129_));
  nand2  g85(.a(new_n123_), .b(new_n127_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n86_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n75_), .c(new_n45_), .O(z16));
endmodule


