// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(x24), .b(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x25), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x11), .c(new_n50_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x11), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n55_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n57_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n57_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  nor2   g22(.a(new_n58_), .b(new_n66_), .O(z05));
  inv1   g23(.a(new_n58_), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n57_), .O(z07));
  nand2  g28(.a(new_n68_), .b(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  inv1   g30(.a(x08), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n52_), .c(new_n56_), .O(new_n79_));
  oai22  g35(.a(new_n79_), .b(x10), .c(new_n75_), .d(new_n55_), .O(z09));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n76_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  inv1   g39(.a(new_n83_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  inv1   g41(.a(new_n53_), .O(new_n86_));
  or2    g42(.a(x20), .b(x19), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand2  g44(.a(new_n68_), .b(x01), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n85_), .c(new_n89_), .O(z10));
  nand3  g46(.a(x21), .b(x20), .c(x19), .O(new_n91_));
  nand3  g47(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  nand3  g49(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  inv1   g50(.a(new_n94_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(new_n91_), .O(new_n96_));
  inv1   g52(.a(x21), .O(new_n97_));
  nand2  g53(.a(new_n81_), .b(new_n97_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  nand2  g55(.a(new_n68_), .b(x02), .O(new_n100_));
  oai21  g56(.a(new_n99_), .b(new_n96_), .c(new_n100_), .O(z11));
  inv1   g57(.a(x14), .O(new_n102_));
  inv1   g58(.a(new_n47_), .O(new_n103_));
  oai21  g59(.a(new_n76_), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  inv1   g60(.a(x22), .O(new_n105_));
  nand2  g61(.a(x09), .b(new_n75_), .O(new_n106_));
  aoi21  g62(.a(new_n91_), .b(new_n105_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n104_), .c(new_n56_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(x10), .c(new_n64_), .O(z12));
  nand2  g65(.a(new_n93_), .b(x15), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n103_), .c(x23), .O(new_n111_));
  inv1   g67(.a(x23), .O(new_n112_));
  nand2  g68(.a(new_n47_), .b(new_n112_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n111_), .c(new_n86_), .O(new_n114_));
  oai21  g70(.a(new_n58_), .b(new_n66_), .c(new_n114_), .O(z13));
  nand3  g71(.a(new_n103_), .b(x24), .c(x23), .O(new_n116_));
  inv1   g72(.a(x24), .O(new_n117_));
  oai21  g73(.a(new_n47_), .b(new_n112_), .c(new_n117_), .O(new_n118_));
  aoi22  g74(.a(new_n118_), .b(new_n116_), .c(new_n93_), .d(x16), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n53_), .c(new_n69_), .O(z14));
  nand3  g76(.a(x26), .b(x25), .c(x17), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  oai21  g79(.a(new_n47_), .b(new_n46_), .c(new_n123_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n49_), .c(new_n122_), .O(new_n125_));
  nand2  g81(.a(new_n68_), .b(x06), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n53_), .c(new_n126_), .O(z15));
  aoi21  g83(.a(new_n48_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n82_), .b(new_n103_), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n86_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n128_), .c(new_n73_), .O(z16));
endmodule


