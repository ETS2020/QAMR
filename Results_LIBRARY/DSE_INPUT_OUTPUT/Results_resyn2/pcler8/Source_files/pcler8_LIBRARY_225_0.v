// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  and2   g00(.a(x20), .b(x19), .O(new_n45_));
  and2   g01(.a(x22), .b(x21), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(new_n45_), .c(x23), .O(new_n47_));
  and2   g03(.a(x26), .b(x25), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g09(.a(x00), .O(new_n54_));
  nor2   g10(.a(x10), .b(x09), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nor2   g15(.a(new_n57_), .b(new_n59_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  inv1   g17(.a(new_n55_), .O(new_n62_));
  oai21  g18(.a(new_n50_), .b(new_n61_), .c(new_n62_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n62_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n62_), .O(z05));
  inv1   g23(.a(x05), .O(new_n68_));
  oai21  g24(.a(new_n50_), .b(new_n68_), .c(new_n62_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n62_), .O(z07));
  nand2  g27(.a(new_n56_), .b(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(new_n52_), .O(new_n74_));
  and2   g30(.a(x24), .b(x23), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n76_), .c(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  oai21  g37(.a(new_n57_), .b(new_n54_), .c(new_n81_), .O(z09));
  nand2  g38(.a(new_n46_), .b(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n76_), .c(new_n45_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n52_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  oai21  g43(.a(new_n57_), .b(new_n59_), .c(new_n87_), .O(z10));
  inv1   g44(.a(new_n76_), .O(new_n89_));
  nand2  g45(.a(new_n45_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n45_), .b(x21), .c(new_n74_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n92_), .c(new_n57_), .d(new_n61_), .O(z11));
  inv1   g50(.a(x09), .O(new_n95_));
  inv1   g51(.a(x14), .O(new_n96_));
  nand4  g52(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n76_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  aoi21  g56(.a(new_n90_), .b(new_n100_), .c(x08), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n99_), .c(new_n95_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x10), .c(new_n64_), .O(z12));
  inv1   g59(.a(x15), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nand4  g61(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n106_));
  nor2   g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  oai21  g63(.a(new_n49_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n106_), .b(new_n105_), .c(x08), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n108_), .c(new_n95_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(x10), .c(new_n66_), .O(z13));
  inv1   g67(.a(new_n49_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand2  g69(.a(x24), .b(x23), .O(new_n114_));
  nor2   g70(.a(new_n114_), .b(new_n106_), .O(new_n115_));
  aoi21  g71(.a(new_n47_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n112_), .b(x16), .c(new_n116_), .O(new_n117_));
  oai22  g73(.a(new_n117_), .b(new_n52_), .c(new_n57_), .d(new_n68_), .O(z14));
  nand3  g74(.a(x26), .b(x25), .c(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  nand4  g76(.a(new_n75_), .b(new_n46_), .c(new_n45_), .d(x25), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  oai21  g78(.a(new_n114_), .b(new_n106_), .c(new_n122_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(x08), .c(x09), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n51_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n70_), .O(z15));
  aoi21  g83(.a(new_n115_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n78_), .b(x19), .c(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n76_), .c(new_n74_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n128_), .c(new_n72_), .O(z16));
endmodule


