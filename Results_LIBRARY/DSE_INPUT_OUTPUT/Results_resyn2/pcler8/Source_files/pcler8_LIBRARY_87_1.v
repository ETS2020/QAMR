// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x16), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  inv1   g09(.a(x23), .O(new_n54_));
  nand4  g10(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n53_), .c(new_n51_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n46_), .O(z01));
  nand3  g16(.a(new_n47_), .b(x08), .c(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n47_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n46_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n46_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n46_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n47_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  nand2  g31(.a(x22), .b(x21), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(new_n80_));
  aoi21  g36(.a(x08), .b(x00), .c(new_n46_), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n50_), .c(new_n81_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  aoi21  g39(.a(new_n78_), .b(x12), .c(new_n83_), .O(new_n84_));
  inv1   g40(.a(x20), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n75_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n51_), .c(new_n47_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n84_), .c(new_n61_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand2  g46(.a(x22), .b(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n77_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  aoi21  g49(.a(new_n83_), .b(new_n93_), .c(new_n50_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n63_), .c(new_n46_), .O(z11));
  inv1   g52(.a(new_n77_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(x14), .c(new_n55_), .O(new_n98_));
  oai21  g54(.a(new_n90_), .b(x22), .c(new_n51_), .O(new_n99_));
  aoi21  g55(.a(x08), .b(x03), .c(new_n46_), .O(new_n100_));
  oai21  g56(.a(new_n99_), .b(new_n98_), .c(new_n100_), .O(z12));
  inv1   g57(.a(x15), .O(new_n102_));
  nor2   g58(.a(new_n77_), .b(new_n102_), .O(new_n103_));
  xor2a  g59(.a(new_n55_), .b(new_n54_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n103_), .c(new_n51_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n67_), .c(new_n47_), .O(z13));
  nand3  g62(.a(x26), .b(x25), .c(x16), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n56_), .c(x24), .O(new_n108_));
  or2    g64(.a(new_n56_), .b(x24), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n108_), .c(new_n51_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n69_), .c(new_n47_), .O(z14));
  aoi21  g67(.a(new_n56_), .b(x24), .c(x25), .O(new_n112_));
  inv1   g68(.a(new_n55_), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x25), .c(x24), .d(x23), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n51_), .c(new_n47_), .O(new_n115_));
  nand4  g71(.a(x17), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(new_n107_), .O(new_n117_));
  nor2   g73(.a(new_n117_), .b(z07), .O(new_n118_));
  oai21  g74(.a(new_n115_), .b(new_n112_), .c(new_n118_), .O(z15));
  inv1   g75(.a(x26), .O(new_n120_));
  nand2  g76(.a(new_n114_), .b(new_n120_), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand3  g78(.a(new_n56_), .b(new_n53_), .c(new_n122_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n121_), .c(new_n51_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n73_), .c(new_n46_), .O(z16));
endmodule


