// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x09), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x25), .O(new_n51_));
  inv1   g07(.a(x26), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n50_), .c(x24), .d(new_n47_), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand2  g11(.a(x10), .b(x09), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  oai21  g17(.a(new_n47_), .b(new_n61_), .c(new_n56_), .O(z02));
  nand2  g18(.a(new_n58_), .b(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n56_), .O(z04));
  inv1   g22(.a(x04), .O(new_n67_));
  oai21  g23(.a(new_n47_), .b(new_n67_), .c(new_n56_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n56_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  oai21  g27(.a(new_n47_), .b(new_n71_), .c(new_n56_), .O(z07));
  nand2  g28(.a(new_n58_), .b(x07), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z08));
  nand2  g30(.a(x08), .b(x00), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n47_), .c(x10), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n45_), .c(new_n75_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  inv1   g39(.a(new_n76_), .O(new_n84_));
  nand2  g40(.a(new_n77_), .b(x12), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand3  g43(.a(new_n46_), .b(x09), .c(new_n47_), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  oai21  g45(.a(x20), .b(x19), .c(new_n89_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(new_n87_), .c(new_n57_), .d(new_n61_), .O(z10));
  nand2  g47(.a(x08), .b(x02), .O(new_n92_));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(x22), .b(x13), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n76_), .c(new_n94_), .O(new_n96_));
  inv1   g52(.a(x21), .O(new_n97_));
  aoi21  g53(.a(new_n83_), .b(new_n97_), .c(x08), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n96_), .c(x10), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n45_), .c(new_n92_), .O(z11));
  nand2  g56(.a(new_n84_), .b(x14), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n94_), .c(x22), .O(new_n102_));
  inv1   g58(.a(x22), .O(new_n103_));
  aoi21  g59(.a(new_n93_), .b(new_n103_), .c(x08), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n102_), .c(x10), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n45_), .c(new_n65_), .O(z12));
  nand3  g62(.a(new_n53_), .b(x24), .c(x15), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n50_), .O(new_n108_));
  aoi21  g64(.a(new_n49_), .b(new_n48_), .c(new_n88_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  oai21  g66(.a(new_n57_), .b(new_n67_), .c(new_n110_), .O(z13));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n49_), .b(new_n48_), .c(new_n114_), .O(new_n115_));
  nand4  g71(.a(new_n77_), .b(new_n82_), .c(x24), .d(x23), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n115_), .c(new_n113_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(x08), .c(new_n46_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(x09), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n69_), .O(z14));
  nand3  g76(.a(new_n50_), .b(x25), .c(x24), .O(new_n121_));
  nand2  g77(.a(new_n116_), .b(new_n51_), .O(new_n122_));
  aoi22  g78(.a(new_n122_), .b(new_n121_), .c(new_n53_), .d(x17), .O(new_n123_));
  oai22  g79(.a(new_n123_), .b(new_n88_), .c(new_n57_), .d(new_n71_), .O(z15));
  nand2  g80(.a(x08), .b(x07), .O(new_n125_));
  nand2  g81(.a(new_n121_), .b(new_n52_), .O(new_n126_));
  nor2   g82(.a(new_n49_), .b(x18), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n84_), .c(x08), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n126_), .c(x10), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n45_), .c(new_n125_), .O(z16));
endmodule


