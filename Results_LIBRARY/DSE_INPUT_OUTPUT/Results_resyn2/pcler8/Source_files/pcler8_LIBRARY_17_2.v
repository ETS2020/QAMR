// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nand3  g01(.a(x22), .b(x21), .c(x20), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x06), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(x05), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n47_), .d(x19), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n53_), .O(z01));
  inv1   g13(.a(new_n53_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n53_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n58_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n53_), .O(z05));
  nand3  g22(.a(x08), .b(x06), .c(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n53_), .O(z08));
  inv1   g28(.a(z01), .O(new_n73_));
  inv1   g29(.a(x19), .O(new_n74_));
  aoi21  g30(.a(new_n47_), .b(x11), .c(new_n74_), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n50_), .c(new_n73_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n45_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand3  g38(.a(new_n82_), .b(new_n59_), .c(new_n53_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(x20), .b(x19), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n89_), .c(new_n50_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n61_), .c(new_n58_), .O(z11));
  inv1   g49(.a(new_n45_), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  aoi21  g51(.a(new_n94_), .b(x14), .c(new_n95_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n84_), .b(new_n97_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n53_), .c(new_n51_), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(new_n96_), .c(new_n63_), .d(new_n58_), .O(z12));
  inv1   g56(.a(new_n95_), .O(new_n101_));
  inv1   g57(.a(new_n86_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n101_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  aoi21  g61(.a(new_n95_), .b(new_n105_), .c(new_n50_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n65_), .c(new_n53_), .O(z13));
  oai21  g64(.a(x08), .b(new_n52_), .c(x05), .O(new_n109_));
  nand4  g65(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  and2   g67(.a(x24), .b(x23), .O(new_n112_));
  and2   g68(.a(x22), .b(x21), .O(new_n113_));
  nand3  g69(.a(new_n77_), .b(new_n113_), .c(new_n112_), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n95_), .b(new_n105_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n114_), .c(new_n111_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n50_), .c(new_n109_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n101_), .c(new_n112_), .O(new_n121_));
  nand2  g77(.a(new_n114_), .b(new_n119_), .O(new_n122_));
  inv1   g78(.a(x05), .O(new_n123_));
  nand2  g79(.a(new_n48_), .b(new_n123_), .O(new_n124_));
  nand2  g80(.a(new_n49_), .b(x09), .O(new_n125_));
  aoi21  g81(.a(new_n124_), .b(new_n52_), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n122_), .c(new_n121_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n69_), .O(z15));
  inv1   g84(.a(new_n46_), .O(new_n129_));
  nor2   g85(.a(new_n74_), .b(x18), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n94_), .O(new_n131_));
  inv1   g87(.a(x26), .O(new_n132_));
  nand3  g88(.a(x25), .b(x24), .c(x23), .O(new_n133_));
  oai21  g89(.a(new_n95_), .b(new_n133_), .c(new_n132_), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n131_), .c(new_n51_), .O(new_n135_));
  aoi21  g91(.a(new_n135_), .b(new_n71_), .c(new_n58_), .O(z16));
endmodule


