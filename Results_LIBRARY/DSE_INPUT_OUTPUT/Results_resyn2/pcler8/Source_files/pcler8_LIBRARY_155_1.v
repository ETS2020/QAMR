// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_,
    new_n54_, new_n57_, new_n59_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  and2   g07(.a(x08), .b(x00), .O(z01));
  nor2   g08(.a(new_n45_), .b(x08), .O(new_n53_));
  aoi21  g09(.a(x08), .b(x01), .c(new_n53_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z02));
  and2   g11(.a(x08), .b(x02), .O(z03));
  aoi21  g12(.a(x08), .b(x03), .c(new_n53_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z04));
  inv1   g14(.a(x04), .O(new_n59_));
  inv1   g15(.a(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z05));
  aoi21  g17(.a(x08), .b(x05), .c(new_n53_), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z07));
  inv1   g21(.a(new_n53_), .O(new_n66_));
  nand2  g22(.a(x08), .b(x07), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n66_), .O(z08));
  inv1   g24(.a(x09), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(x08), .O(new_n70_));
  nand4  g26(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n71_));
  and2   g27(.a(x22), .b(x21), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n71_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nor2   g31(.a(new_n53_), .b(z01), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n75_), .O(z09));
  and2   g33(.a(x20), .b(x19), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  inv1   g35(.a(new_n71_), .O(new_n80_));
  nand2  g36(.a(new_n72_), .b(x12), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n70_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n83_), .c(new_n54_), .O(z10));
  nand2  g41(.a(new_n78_), .b(x21), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n80_), .c(new_n86_), .O(new_n88_));
  oai21  g44(.a(new_n78_), .b(x21), .c(new_n70_), .O(new_n89_));
  nor2   g45(.a(z03), .b(new_n53_), .O(new_n90_));
  oai21  g46(.a(new_n89_), .b(new_n88_), .c(new_n90_), .O(z11));
  aoi21  g47(.a(new_n80_), .b(x14), .c(new_n47_), .O(new_n92_));
  inv1   g48(.a(new_n86_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(x22), .c(new_n70_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n92_), .c(new_n57_), .O(z12));
  nand2  g51(.a(new_n70_), .b(new_n45_), .O(new_n96_));
  xor2a  g52(.a(new_n47_), .b(new_n46_), .O(new_n97_));
  aoi21  g53(.a(new_n80_), .b(x15), .c(new_n97_), .O(new_n98_));
  oai22  g54(.a(new_n98_), .b(new_n96_), .c(new_n60_), .d(new_n59_), .O(z13));
  inv1   g55(.a(new_n70_), .O(new_n100_));
  nand4  g56(.a(new_n72_), .b(new_n78_), .c(x24), .d(x23), .O(new_n101_));
  nand3  g57(.a(new_n49_), .b(x24), .c(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n47_), .b(new_n46_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n101_), .c(new_n103_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n100_), .c(new_n62_), .O(z14));
  nand2  g63(.a(new_n49_), .b(x17), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n48_), .b(x25), .c(x24), .O(new_n110_));
  inv1   g66(.a(x25), .O(new_n111_));
  nand2  g67(.a(new_n101_), .b(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n96_), .c(new_n64_), .O(z15));
  inv1   g70(.a(new_n110_), .O(new_n115_));
  nor2   g71(.a(new_n47_), .b(x18), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n80_), .c(new_n96_), .O(new_n117_));
  oai21  g73(.a(new_n115_), .b(x26), .c(new_n117_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n67_), .O(z16));
endmodule


