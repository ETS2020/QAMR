// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x08), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nand4  g05(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(z00));
  nand2  g07(.a(x08), .b(x00), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x10), .O(z01));
  nand2  g09(.a(x08), .b(x01), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(x10), .O(z02));
  nand2  g11(.a(x08), .b(x02), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(x10), .O(z03));
  nand2  g13(.a(x08), .b(x03), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(x10), .O(z04));
  nand2  g15(.a(x08), .b(x04), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(x10), .O(z05));
  oai21  g17(.a(x10), .b(x05), .c(x08), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(x10), .O(z07));
  nand2  g21(.a(x08), .b(x07), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(x10), .O(z08));
  oai21  g23(.a(x10), .b(x00), .c(x08), .O(new_n68_));
  inv1   g24(.a(x19), .O(new_n69_));
  inv1   g25(.a(x21), .O(new_n70_));
  inv1   g26(.a(x22), .O(new_n71_));
  nor3   g27(.a(new_n50_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  and2   g28(.a(x20), .b(x11), .O(new_n73_));
  aoi21  g29(.a(new_n73_), .b(new_n72_), .c(new_n69_), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n48_), .c(new_n68_), .O(z09));
  oai21  g31(.a(x10), .b(x01), .c(x08), .O(new_n76_));
  nand2  g32(.a(x20), .b(x19), .O(new_n77_));
  aoi21  g33(.a(new_n72_), .b(x12), .c(new_n77_), .O(new_n78_));
  inv1   g34(.a(new_n48_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n79_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(z10));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(x21), .O(new_n83_));
  nand2  g39(.a(x22), .b(x13), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n50_), .O(new_n85_));
  inv1   g41(.a(x08), .O(new_n86_));
  nand2  g42(.a(x09), .b(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n77_), .b(new_n70_), .c(new_n87_), .O(new_n88_));
  oai21  g44(.a(new_n85_), .b(new_n83_), .c(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n56_), .c(x10), .O(z11));
  inv1   g46(.a(x14), .O(new_n91_));
  inv1   g47(.a(new_n49_), .O(new_n92_));
  oai21  g48(.a(new_n50_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  nand2  g49(.a(new_n83_), .b(new_n71_), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n93_), .c(new_n47_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n58_), .c(x10), .O(z12));
  inv1   g52(.a(x15), .O(new_n97_));
  nor2   g53(.a(new_n50_), .b(new_n97_), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n49_), .b(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n47_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n60_), .c(x10), .O(z13));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  and2   g60(.a(x22), .b(x21), .O(new_n105_));
  nand4  g61(.a(new_n105_), .b(new_n82_), .c(x24), .d(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n49_), .b(new_n99_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n48_), .c(new_n62_), .O(z14));
  oai21  g66(.a(x10), .b(x06), .c(x08), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x17), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nor2   g69(.a(new_n49_), .b(new_n99_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(x25), .c(x24), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand2  g72(.a(new_n106_), .b(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  oai21  g74(.a(new_n118_), .b(new_n48_), .c(new_n111_), .O(z15));
  oai21  g75(.a(x10), .b(x07), .c(x08), .O(new_n120_));
  inv1   g76(.a(new_n115_), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nor2   g78(.a(new_n50_), .b(new_n49_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n122_), .c(new_n48_), .O(new_n124_));
  oai21  g80(.a(new_n121_), .b(x26), .c(new_n124_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n120_), .O(z16));
endmodule


