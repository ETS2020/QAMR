// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  and2   g02(.a(x22), .b(x21), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  and2   g08(.a(x20), .b(x19), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n52_), .c(x23), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n48_), .O(z00));
  inv1   g11(.a(x21), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nor2   g16(.a(new_n57_), .b(new_n49_), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z02));
  nand2  g19(.a(new_n61_), .b(x02), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  inv1   g21(.a(x03), .O(new_n66_));
  oai21  g22(.a(new_n49_), .b(new_n66_), .c(new_n58_), .O(z04));
  nand2  g23(.a(new_n61_), .b(x04), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z05));
  nand2  g25(.a(new_n61_), .b(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n58_), .O(z07));
  inv1   g29(.a(x07), .O(new_n74_));
  oai21  g30(.a(new_n49_), .b(new_n74_), .c(new_n58_), .O(z08));
  and2   g31(.a(x20), .b(x11), .O(new_n76_));
  nand4  g32(.a(new_n76_), .b(new_n47_), .c(new_n46_), .d(x23), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n52_), .O(new_n79_));
  nand2  g35(.a(new_n61_), .b(x00), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n79_), .O(z09));
  nand4  g37(.a(new_n47_), .b(new_n46_), .c(x23), .d(x12), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n51_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n62_), .O(z10));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  nand4  g44(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n90_), .c(new_n88_), .O(new_n92_));
  nand2  g48(.a(x20), .b(x19), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(new_n56_), .O(new_n94_));
  nand2  g50(.a(new_n53_), .b(x10), .O(new_n95_));
  nand4  g51(.a(new_n95_), .b(new_n94_), .c(x09), .d(new_n49_), .O(new_n96_));
  aoi21  g52(.a(x08), .b(x02), .c(new_n57_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n92_), .c(new_n97_), .O(z11));
  nand2  g54(.a(new_n58_), .b(x08), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  aoi21  g56(.a(new_n90_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(new_n88_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x22), .c(new_n52_), .O(new_n103_));
  oai22  g59(.a(new_n103_), .b(new_n101_), .c(new_n99_), .d(new_n66_), .O(z12));
  inv1   g60(.a(x15), .O(new_n105_));
  nor2   g61(.a(new_n89_), .b(new_n105_), .O(new_n106_));
  inv1   g62(.a(x23), .O(new_n107_));
  xor2a  g63(.a(new_n100_), .b(new_n107_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n106_), .c(new_n52_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n68_), .O(z13));
  nand4  g66(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  nand4  g68(.a(new_n53_), .b(new_n47_), .c(x24), .d(x23), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n100_), .b(new_n107_), .c(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n51_), .c(new_n70_), .O(z14));
  inv1   g73(.a(x25), .O(new_n118_));
  nor2   g74(.a(new_n118_), .b(new_n114_), .O(new_n119_));
  nor2   g75(.a(new_n100_), .b(new_n107_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  nand2  g78(.a(new_n113_), .b(new_n118_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n52_), .O(new_n124_));
  nand2  g80(.a(new_n61_), .b(x06), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(z15));
  aoi21  g82(.a(new_n120_), .b(new_n119_), .c(x26), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  inv1   g84(.a(new_n100_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n90_), .c(new_n128_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n52_), .O(new_n131_));
  oai22  g87(.a(new_n131_), .b(new_n127_), .c(new_n99_), .d(new_n74_), .O(z16));
endmodule


