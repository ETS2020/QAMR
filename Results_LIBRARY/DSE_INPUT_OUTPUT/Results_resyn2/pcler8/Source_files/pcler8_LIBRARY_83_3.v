// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand2  g00(.a(x17), .b(x15), .O(new_n45_));
  nand4  g01(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  oai21  g09(.a(new_n53_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand3  g10(.a(new_n45_), .b(x08), .c(x00), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  nand2  g12(.a(x08), .b(x01), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n45_), .O(z02));
  nand2  g14(.a(x08), .b(x02), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z03));
  nand3  g16(.a(new_n45_), .b(x08), .c(x03), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z04));
  nand2  g18(.a(x08), .b(x04), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n45_), .O(z05));
  inv1   g20(.a(new_n45_), .O(new_n65_));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n45_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n45_), .O(z08));
  and2   g27(.a(x22), .b(x21), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n46_), .c(x19), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(new_n50_), .c(new_n45_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n55_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand2  g33(.a(new_n72_), .b(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n46_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n49_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n57_), .c(new_n65_), .O(z10));
  nand2  g39(.a(new_n77_), .b(x21), .O(new_n84_));
  nand4  g40(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n85_));
  nand2  g41(.a(x23), .b(x13), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  nand2  g44(.a(x20), .b(x19), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n88_), .c(new_n49_), .O(new_n90_));
  oai21  g46(.a(new_n87_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n59_), .c(new_n65_), .O(z11));
  nand3  g48(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  nand2  g49(.a(x23), .b(x14), .O(new_n94_));
  nor2   g50(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nor2   g51(.a(new_n95_), .b(new_n51_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n84_), .b(new_n97_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n50_), .c(new_n45_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n96_), .c(new_n61_), .O(z12));
  inv1   g56(.a(x23), .O(new_n101_));
  nand2  g57(.a(new_n51_), .b(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n72_), .b(new_n77_), .c(x23), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n102_), .c(new_n50_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n63_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n45_), .O(new_n106_));
  inv1   g62(.a(new_n85_), .O(new_n107_));
  nand2  g63(.a(new_n94_), .b(new_n84_), .O(new_n108_));
  inv1   g64(.a(x15), .O(new_n109_));
  nor2   g65(.a(x17), .b(new_n109_), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(new_n108_), .c(new_n107_), .d(new_n50_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n106_), .O(z13));
  nand2  g68(.a(x25), .b(x24), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  and2   g70(.a(x26), .b(x16), .O(new_n115_));
  nand3  g71(.a(new_n52_), .b(x24), .c(x23), .O(new_n116_));
  inv1   g72(.a(x24), .O(new_n117_));
  oai21  g73(.a(new_n51_), .b(new_n101_), .c(new_n117_), .O(new_n118_));
  aoi22  g74(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n114_), .O(new_n119_));
  and2   g75(.a(new_n66_), .b(new_n45_), .O(new_n120_));
  oai21  g76(.a(new_n119_), .b(new_n49_), .c(new_n120_), .O(z14));
  inv1   g77(.a(x25), .O(new_n122_));
  nand2  g78(.a(new_n116_), .b(new_n122_), .O(new_n123_));
  nor2   g79(.a(new_n51_), .b(new_n101_), .O(new_n124_));
  aoi21  g80(.a(x26), .b(x17), .c(new_n113_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n124_), .c(new_n49_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(new_n68_), .c(new_n65_), .O(z15));
  inv1   g84(.a(x26), .O(new_n129_));
  nand3  g85(.a(new_n52_), .b(new_n114_), .c(x23), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nor2   g87(.a(new_n93_), .b(x18), .O(new_n132_));
  aoi21  g88(.a(new_n132_), .b(new_n124_), .c(new_n49_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n70_), .c(new_n65_), .O(z16));
endmodule


