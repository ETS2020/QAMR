// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand2  g00(.a(x17), .b(x10), .O(new_n45_));
  nand4  g01(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(new_n51_), .c(new_n47_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  inv1   g11(.a(new_n45_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(z01));
  nand3  g14(.a(new_n45_), .b(x08), .c(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n56_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n56_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n56_), .O(z06));
  and2   g24(.a(x08), .b(x06), .O(new_n69_));
  and2   g25(.a(new_n69_), .b(new_n45_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(z08));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n46_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n51_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n57_), .c(new_n45_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n46_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n50_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n59_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n47_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n77_), .b(x21), .c(new_n51_), .O(new_n87_));
  and2   g43(.a(new_n61_), .b(new_n45_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(z11));
  inv1   g45(.a(z04), .O(new_n90_));
  aoi21  g46(.a(new_n47_), .b(x14), .c(new_n52_), .O(new_n91_));
  inv1   g47(.a(new_n84_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n51_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(new_n90_), .O(z12));
  inv1   g50(.a(x15), .O(new_n95_));
  nor2   g51(.a(new_n46_), .b(new_n95_), .O(new_n96_));
  inv1   g52(.a(x23), .O(new_n97_));
  xor2a  g53(.a(new_n52_), .b(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n51_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n65_), .c(new_n45_), .O(z13));
  nor2   g56(.a(new_n52_), .b(new_n97_), .O(new_n101_));
  nand3  g57(.a(x26), .b(x25), .c(x16), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x24), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  and2   g60(.a(x22), .b(x21), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n77_), .c(x23), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n103_), .c(new_n51_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n67_), .c(new_n45_), .O(z14));
  inv1   g65(.a(x25), .O(new_n110_));
  nand3  g66(.a(new_n53_), .b(x24), .c(x23), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nor2   g68(.a(new_n110_), .b(new_n104_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n101_), .c(new_n50_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g71(.a(x26), .b(x25), .O(new_n116_));
  nand2  g72(.a(x09), .b(new_n48_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n116_), .c(new_n49_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(x17), .c(new_n69_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n115_), .O(z15));
  inv1   g76(.a(z08), .O(new_n121_));
  aoi21  g77(.a(new_n113_), .b(new_n101_), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand4  g79(.a(x26), .b(x25), .c(x24), .d(new_n123_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n106_), .c(new_n51_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n122_), .c(new_n121_), .O(z16));
endmodule


