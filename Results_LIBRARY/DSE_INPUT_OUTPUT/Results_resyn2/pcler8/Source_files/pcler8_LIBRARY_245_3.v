// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x18), .b(x13), .O(new_n45_));
  nand2  g01(.a(x20), .b(x19), .O(new_n46_));
  nand3  g02(.a(x23), .b(x22), .c(x21), .O(new_n47_));
  nand2  g03(.a(x25), .b(x24), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand4  g07(.a(x26), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  oai21  g10(.a(new_n54_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n45_), .b(x08), .c(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  nand3  g13(.a(new_n45_), .b(x08), .c(x01), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n45_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n45_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n45_), .O(z05));
  nand3  g21(.a(new_n45_), .b(x08), .c(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n45_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n45_), .O(z08));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n45_), .O(new_n74_));
  inv1   g30(.a(x19), .O(new_n75_));
  nand3  g31(.a(x26), .b(x25), .c(x24), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(x20), .b(x11), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n77_), .c(new_n75_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n74_), .c(new_n56_), .O(z09));
  inv1   g37(.a(new_n47_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(x12), .O(new_n83_));
  inv1   g39(.a(new_n83_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n77_), .c(new_n46_), .O(new_n85_));
  inv1   g41(.a(x20), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n75_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n73_), .c(new_n45_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n85_), .c(new_n58_), .O(z10));
  inv1   g45(.a(x13), .O(new_n90_));
  aoi21  g46(.a(new_n53_), .b(new_n49_), .c(x18), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  nand2  g48(.a(new_n46_), .b(new_n92_), .O(new_n93_));
  nand3  g49(.a(x21), .b(x20), .c(x19), .O(new_n94_));
  inv1   g50(.a(new_n94_), .O(new_n95_));
  nor2   g51(.a(new_n95_), .b(new_n72_), .O(new_n96_));
  aoi22  g52(.a(new_n96_), .b(new_n93_), .c(x08), .d(x02), .O(new_n97_));
  oai21  g53(.a(new_n91_), .b(new_n90_), .c(new_n97_), .O(z11));
  inv1   g54(.a(new_n45_), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  nand2  g56(.a(x23), .b(x14), .O(new_n101_));
  nor2   g57(.a(new_n101_), .b(new_n76_), .O(new_n102_));
  inv1   g58(.a(x22), .O(new_n103_));
  aoi21  g59(.a(new_n94_), .b(new_n103_), .c(new_n72_), .O(new_n104_));
  oai21  g60(.a(new_n102_), .b(new_n100_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n62_), .c(new_n99_), .O(z12));
  nand2  g62(.a(new_n77_), .b(x15), .O(new_n107_));
  inv1   g63(.a(x23), .O(new_n108_));
  nor2   g64(.a(new_n100_), .b(new_n108_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  aoi21  g66(.a(new_n100_), .b(new_n108_), .c(new_n72_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n64_), .c(new_n45_), .O(z13));
  nand2  g69(.a(new_n109_), .b(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n100_), .b(new_n108_), .c(new_n115_), .O(new_n116_));
  aoi22  g72(.a(new_n116_), .b(new_n114_), .c(new_n77_), .d(x16), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n74_), .c(new_n66_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n109_), .c(x24), .O(new_n121_));
  nand2  g77(.a(new_n114_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n73_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n68_), .c(new_n45_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  nor2   g81(.a(new_n48_), .b(x18), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n109_), .c(new_n125_), .O(new_n127_));
  nor2   g83(.a(new_n46_), .b(x26), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n49_), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n127_), .c(new_n73_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n70_), .c(new_n99_), .O(z16));
endmodule


