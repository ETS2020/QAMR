// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n54_, new_n56_, new_n58_, new_n60_, new_n61_, new_n63_, new_n65_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand2  g07(.a(x08), .b(x00), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x10), .O(z01));
  oai21  g09(.a(x10), .b(x01), .c(x08), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z02));
  nand2  g11(.a(x08), .b(x02), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(x10), .O(z03));
  oai21  g13(.a(x10), .b(x03), .c(x08), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z04));
  inv1   g15(.a(x04), .O(new_n60_));
  inv1   g16(.a(x08), .O(new_n61_));
  aoi21  g17(.a(new_n46_), .b(new_n60_), .c(new_n61_), .O(z05));
  nand2  g18(.a(x08), .b(x05), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x10), .O(z06));
  inv1   g20(.a(x06), .O(new_n65_));
  aoi21  g21(.a(new_n46_), .b(new_n65_), .c(new_n61_), .O(z07));
  nand2  g22(.a(x08), .b(x07), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x10), .O(z08));
  nand4  g24(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n69_));
  oai21  g25(.a(new_n69_), .b(new_n45_), .c(x19), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n48_), .O(new_n71_));
  aoi21  g27(.a(new_n71_), .b(new_n52_), .c(x10), .O(z09));
  inv1   g28(.a(new_n49_), .O(new_n73_));
  and2   g29(.a(x20), .b(x19), .O(new_n74_));
  inv1   g30(.a(new_n45_), .O(new_n75_));
  and2   g31(.a(x22), .b(x21), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n75_), .c(x12), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n74_), .O(new_n78_));
  or2    g34(.a(x20), .b(x19), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(new_n78_), .c(new_n73_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n54_), .O(z10));
  nand3  g37(.a(x21), .b(x20), .c(x19), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  nand2  g39(.a(x22), .b(x13), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n45_), .c(new_n83_), .O(new_n85_));
  or2    g41(.a(new_n74_), .b(x21), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n48_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n56_), .c(x10), .O(z11));
  aoi21  g44(.a(new_n75_), .b(x14), .c(new_n50_), .O(new_n89_));
  inv1   g45(.a(x22), .O(new_n90_));
  nand2  g46(.a(new_n82_), .b(new_n90_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n73_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n89_), .c(new_n58_), .O(z12));
  nand2  g49(.a(x08), .b(x04), .O(new_n94_));
  inv1   g50(.a(x15), .O(new_n95_));
  nor2   g51(.a(new_n45_), .b(new_n95_), .O(new_n96_));
  inv1   g52(.a(x23), .O(new_n97_));
  xor2a  g53(.a(new_n50_), .b(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n48_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n94_), .c(x10), .O(z13));
  oai21  g56(.a(x10), .b(x05), .c(x08), .O(new_n101_));
  nand3  g57(.a(x26), .b(x25), .c(x24), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nand4  g59(.a(new_n76_), .b(new_n74_), .c(x24), .d(x23), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n50_), .b(new_n97_), .c(new_n105_), .O(new_n106_));
  aoi22  g62(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(x16), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(new_n49_), .c(new_n101_), .O(z14));
  nand2  g64(.a(x08), .b(x06), .O(new_n109_));
  inv1   g65(.a(x25), .O(new_n110_));
  nor2   g66(.a(new_n110_), .b(new_n105_), .O(new_n111_));
  nor2   g67(.a(new_n50_), .b(new_n97_), .O(new_n112_));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  nand2  g70(.a(new_n104_), .b(new_n110_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n48_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n109_), .c(x10), .O(z15));
  oai21  g73(.a(x10), .b(x07), .c(x08), .O(new_n118_));
  aoi21  g74(.a(new_n112_), .b(new_n111_), .c(x26), .O(new_n119_));
  nor2   g75(.a(new_n50_), .b(x18), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n75_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n73_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n119_), .c(new_n118_), .O(z16));
endmodule


