// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(x24), .O(new_n48_));
  inv1   g04(.a(x25), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand4  g07(.a(x26), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n49_), .c(new_n48_), .O(new_n53_));
  and2   g09(.a(new_n53_), .b(new_n47_), .O(z00));
  nand3  g10(.a(x26), .b(x08), .c(x00), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z01));
  inv1   g12(.a(x26), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x00), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n58_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  oai21  g21(.a(new_n50_), .b(new_n65_), .c(new_n58_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n58_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n58_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n58_), .O(z08));
  oai21  g28(.a(new_n57_), .b(x08), .c(x00), .O(new_n73_));
  nand3  g29(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(x26), .b(x25), .c(x24), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x23), .O(new_n78_));
  nand4  g34(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n75_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n73_), .O(z09));
  xnor2a g38(.a(x20), .b(x19), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n74_), .c(new_n59_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n58_), .O(new_n85_));
  nand3  g41(.a(x23), .b(x19), .c(x12), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  nand4  g43(.a(new_n87_), .b(new_n53_), .c(x22), .d(x21), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n85_), .O(z10));
  inv1   g45(.a(z03), .O(new_n90_));
  inv1   g46(.a(new_n45_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x13), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n77_), .c(new_n46_), .O(new_n94_));
  and2   g50(.a(x20), .b(x19), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x21), .c(new_n75_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n90_), .O(z11));
  inv1   g53(.a(z04), .O(new_n98_));
  inv1   g54(.a(new_n78_), .O(new_n99_));
  nand4  g55(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n100_));
  aoi21  g56(.a(new_n99_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(new_n46_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x22), .c(new_n75_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n101_), .c(new_n98_), .O(z12));
  inv1   g60(.a(new_n58_), .O(new_n105_));
  nor2   g61(.a(new_n50_), .b(new_n65_), .O(new_n106_));
  inv1   g62(.a(x23), .O(new_n107_));
  nand2  g63(.a(new_n100_), .b(new_n107_), .O(new_n108_));
  nor2   g64(.a(new_n74_), .b(new_n47_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n108_), .c(new_n106_), .O(new_n110_));
  inv1   g66(.a(new_n52_), .O(new_n111_));
  nand3  g67(.a(x25), .b(x24), .c(x23), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n111_), .c(x15), .O(new_n114_));
  oai21  g70(.a(new_n110_), .b(new_n105_), .c(new_n114_), .O(z13));
  oai21  g71(.a(new_n46_), .b(new_n45_), .c(new_n48_), .O(new_n116_));
  nand3  g72(.a(new_n102_), .b(new_n91_), .c(x24), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n75_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n67_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n58_), .O(new_n120_));
  nand2  g76(.a(new_n53_), .b(x16), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(z14));
  aoi21  g78(.a(x26), .b(x17), .c(new_n49_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n47_), .c(x24), .O(new_n124_));
  nand2  g80(.a(new_n117_), .b(new_n49_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n124_), .c(new_n75_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n69_), .c(new_n58_), .O(z15));
  nor2   g83(.a(new_n100_), .b(new_n74_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n113_), .c(x00), .O(new_n129_));
  inv1   g85(.a(x18), .O(new_n130_));
  nor2   g86(.a(new_n49_), .b(new_n48_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n47_), .c(new_n130_), .O(new_n132_));
  aoi22  g88(.a(new_n132_), .b(new_n111_), .c(x08), .d(x07), .O(new_n133_));
  oai21  g89(.a(new_n129_), .b(x26), .c(new_n133_), .O(z16));
endmodule


