// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  nand3  g00(.a(x21), .b(x20), .c(x19), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x23), .c(x22), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  nor3   g07(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(z00));
  inv1   g08(.a(x00), .O(new_n53_));
  inv1   g09(.a(x20), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x10), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n53_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  nor2   g14(.a(new_n56_), .b(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n55_), .O(z03));
  inv1   g17(.a(x03), .O(new_n62_));
  nor2   g18(.a(new_n56_), .b(new_n62_), .O(z04));
  inv1   g19(.a(x04), .O(new_n64_));
  nor2   g20(.a(new_n56_), .b(new_n64_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n55_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n55_), .O(z07));
  inv1   g25(.a(x07), .O(new_n70_));
  nor2   g26(.a(new_n56_), .b(new_n70_), .O(z08));
  inv1   g27(.a(new_n50_), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  oai21  g32(.a(new_n56_), .b(new_n53_), .c(new_n76_), .O(z09));
  nand2  g33(.a(x20), .b(x19), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  nand4  g35(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n80_));
  nand4  g36(.a(x26), .b(x25), .c(x12), .d(new_n49_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n80_), .c(new_n79_), .O(new_n82_));
  inv1   g38(.a(x19), .O(new_n83_));
  nand2  g39(.a(x09), .b(new_n48_), .O(new_n84_));
  nand2  g40(.a(x20), .b(new_n49_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(new_n84_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  inv1   g43(.a(new_n55_), .O(new_n88_));
  aoi21  g44(.a(x08), .b(x01), .c(new_n88_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n87_), .O(z10));
  inv1   g46(.a(z03), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  nor2   g48(.a(new_n92_), .b(x20), .O(new_n93_));
  and2   g49(.a(x21), .b(x19), .O(new_n94_));
  nand3  g50(.a(x23), .b(x22), .c(x13), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n51_), .c(new_n94_), .O(new_n96_));
  aoi21  g52(.a(new_n92_), .b(new_n83_), .c(new_n85_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n84_), .c(new_n91_), .O(z11));
  inv1   g55(.a(new_n73_), .O(new_n100_));
  nand4  g56(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n101_));
  aoi21  g57(.a(new_n100_), .b(x14), .c(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n46_), .b(x22), .c(new_n72_), .O(new_n103_));
  oai22  g59(.a(new_n103_), .b(new_n102_), .c(new_n56_), .d(new_n62_), .O(z12));
  inv1   g60(.a(new_n51_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(x15), .O(new_n106_));
  inv1   g62(.a(x23), .O(new_n107_));
  nor2   g63(.a(new_n101_), .b(new_n107_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  aoi21  g65(.a(new_n101_), .b(new_n107_), .c(new_n50_), .O(new_n110_));
  nand2  g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  aoi21  g67(.a(x08), .b(x04), .c(new_n88_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n111_), .O(z13));
  inv1   g69(.a(x24), .O(new_n114_));
  nand2  g70(.a(new_n47_), .b(new_n114_), .O(new_n115_));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand4  g72(.a(x23), .b(x22), .c(x21), .d(x19), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(x24), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n115_), .c(new_n72_), .O(new_n120_));
  oai21  g76(.a(new_n84_), .b(new_n114_), .c(new_n49_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n54_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n66_), .O(z14));
  nand3  g79(.a(x26), .b(x25), .c(x17), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(new_n125_));
  inv1   g81(.a(new_n80_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n79_), .c(x25), .O(new_n127_));
  inv1   g83(.a(x25), .O(new_n128_));
  oai21  g84(.a(new_n80_), .b(new_n78_), .c(new_n128_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n127_), .c(new_n125_), .O(new_n130_));
  nand3  g86(.a(new_n55_), .b(x08), .c(x06), .O(new_n131_));
  oai21  g87(.a(new_n130_), .b(new_n50_), .c(new_n131_), .O(z15));
  nor2   g88(.a(new_n80_), .b(new_n78_), .O(new_n133_));
  aoi21  g89(.a(new_n133_), .b(x25), .c(x26), .O(new_n134_));
  inv1   g90(.a(x18), .O(new_n135_));
  nand3  g91(.a(new_n118_), .b(new_n105_), .c(new_n135_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n72_), .O(new_n137_));
  nor2   g93(.a(new_n48_), .b(new_n70_), .O(new_n138_));
  inv1   g94(.a(x26), .O(new_n139_));
  oai21  g95(.a(new_n84_), .b(new_n139_), .c(new_n49_), .O(new_n140_));
  aoi21  g96(.a(new_n140_), .b(new_n54_), .c(new_n138_), .O(new_n141_));
  oai21  g97(.a(new_n137_), .b(new_n134_), .c(new_n141_), .O(z16));
endmodule


