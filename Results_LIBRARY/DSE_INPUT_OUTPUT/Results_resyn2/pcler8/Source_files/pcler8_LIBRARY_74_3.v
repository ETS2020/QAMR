// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand4  g04(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  oai21  g08(.a(new_n46_), .b(new_n45_), .c(new_n52_), .O(z00));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(x10), .O(z01));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(x10), .O(z02));
  oai21  g13(.a(x10), .b(x02), .c(x08), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  inv1   g15(.a(x03), .O(new_n60_));
  aoi21  g16(.a(new_n46_), .b(new_n60_), .c(new_n45_), .O(z04));
  oai21  g17(.a(x10), .b(x04), .c(x08), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z05));
  oai21  g19(.a(x10), .b(x05), .c(x08), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z06));
  oai21  g21(.a(x10), .b(x06), .c(x08), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z07));
  oai21  g23(.a(x10), .b(x07), .c(x08), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z08));
  nand4  g25(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n49_), .c(x19), .O(new_n71_));
  nand3  g27(.a(new_n71_), .b(x09), .c(new_n45_), .O(new_n72_));
  aoi21  g28(.a(new_n72_), .b(new_n54_), .c(x10), .O(z09));
  oai21  g29(.a(x10), .b(x01), .c(x08), .O(new_n74_));
  and2   g30(.a(x20), .b(x19), .O(new_n75_));
  inv1   g31(.a(new_n49_), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n76_), .c(x12), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n47_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n74_), .O(z10));
  nand3  g39(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x13), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n76_), .c(new_n84_), .O(new_n86_));
  oai21  g42(.a(new_n75_), .b(x21), .c(new_n48_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n58_), .O(z11));
  nand2  g44(.a(x08), .b(x03), .O(new_n89_));
  inv1   g45(.a(x14), .O(new_n90_));
  inv1   g46(.a(new_n50_), .O(new_n91_));
  oai21  g47(.a(new_n49_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  inv1   g48(.a(x22), .O(new_n93_));
  nand2  g49(.a(x09), .b(new_n45_), .O(new_n94_));
  aoi21  g50(.a(new_n84_), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n89_), .c(x10), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n49_), .b(new_n98_), .O(new_n99_));
  inv1   g55(.a(x23), .O(new_n100_));
  xor2a  g56(.a(new_n50_), .b(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n99_), .c(new_n48_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n62_), .O(z13));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n104_));
  inv1   g60(.a(new_n104_), .O(new_n105_));
  nand4  g61(.a(new_n77_), .b(new_n75_), .c(x24), .d(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n50_), .b(new_n100_), .c(new_n107_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n105_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n47_), .c(new_n64_), .O(z14));
  nor2   g66(.a(new_n50_), .b(new_n100_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(x24), .c(x25), .O(new_n112_));
  and2   g68(.a(x25), .b(x24), .O(new_n113_));
  nand2  g69(.a(x26), .b(x17), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n111_), .c(new_n113_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n48_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n112_), .c(new_n66_), .O(z15));
  inv1   g73(.a(x18), .O(new_n118_));
  nand2  g74(.a(new_n51_), .b(new_n118_), .O(new_n119_));
  inv1   g75(.a(x26), .O(new_n120_));
  nand3  g76(.a(new_n91_), .b(new_n113_), .c(x23), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n119_), .c(new_n48_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n68_), .O(z16));
endmodule


