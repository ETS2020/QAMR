// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  and2   g00(.a(x20), .b(x19), .O(new_n45_));
  and2   g01(.a(x22), .b(x21), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(new_n45_), .c(x23), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand2  g07(.a(x17), .b(x01), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n47_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n52_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nor3   g15(.a(x17), .b(new_n48_), .c(new_n59_), .O(z02));
  nand3  g16(.a(new_n52_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n52_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n52_), .O(z05));
  nand3  g22(.a(new_n52_), .b(x08), .c(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  and2   g24(.a(x08), .b(x06), .O(new_n69_));
  and2   g25(.a(new_n69_), .b(new_n52_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  aoi21  g27(.a(x17), .b(x01), .c(new_n71_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n51_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n57_), .c(new_n52_), .O(z09));
  oai21  g33(.a(x17), .b(x08), .c(x01), .O(new_n78_));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  inv1   g35(.a(new_n73_), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n80_), .c(new_n79_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n51_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n83_), .c(new_n78_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  nand3  g42(.a(x23), .b(x22), .c(x13), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n54_), .c(new_n86_), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n79_), .b(new_n90_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n52_), .c(new_n51_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n89_), .c(new_n61_), .O(z11));
  inv1   g49(.a(z04), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  aoi21  g51(.a(new_n80_), .b(x14), .c(new_n95_), .O(new_n96_));
  inv1   g52(.a(new_n86_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x22), .c(new_n51_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(z12));
  inv1   g55(.a(x15), .O(new_n100_));
  nor2   g56(.a(new_n73_), .b(new_n100_), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  xor2a  g58(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n101_), .c(new_n51_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n65_), .c(new_n52_), .O(z13));
  nor2   g61(.a(new_n95_), .b(new_n102_), .O(new_n106_));
  nand3  g62(.a(x26), .b(x25), .c(x16), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n106_), .c(x24), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  nand2  g65(.a(new_n47_), .b(new_n109_), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(new_n108_), .c(new_n52_), .d(new_n51_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n67_), .O(z14));
  inv1   g68(.a(x25), .O(new_n113_));
  nand4  g69(.a(new_n46_), .b(new_n45_), .c(x24), .d(x23), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nor2   g71(.a(new_n113_), .b(new_n109_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n106_), .c(new_n50_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g74(.a(x26), .b(x25), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n50_), .c(new_n59_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(x17), .c(new_n69_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n118_), .O(z15));
  aoi21  g78(.a(new_n116_), .b(new_n106_), .c(x26), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand2  g80(.a(new_n54_), .b(new_n124_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n47_), .c(new_n51_), .O(new_n126_));
  and2   g82(.a(new_n71_), .b(new_n52_), .O(new_n127_));
  oai21  g83(.a(new_n126_), .b(new_n123_), .c(new_n127_), .O(z16));
endmodule


