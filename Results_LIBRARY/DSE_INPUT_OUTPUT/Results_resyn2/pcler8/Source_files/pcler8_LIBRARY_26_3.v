// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g07(.a(x05), .O(new_n52_));
  nor2   g08(.a(new_n46_), .b(new_n52_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  inv1   g13(.a(x08), .O(new_n58_));
  oai21  g14(.a(new_n58_), .b(new_n57_), .c(new_n54_), .O(z02));
  nand2  g15(.a(new_n54_), .b(x08), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  aoi21  g19(.a(x08), .b(x03), .c(new_n53_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(new_n61_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  aoi21  g23(.a(new_n46_), .b(new_n58_), .c(new_n52_), .O(z06));
  nand2  g24(.a(new_n61_), .b(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(new_n61_), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  inv1   g28(.a(new_n49_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n45_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n55_), .c(new_n54_), .O(z09));
  inv1   g33(.a(x19), .O(new_n78_));
  inv1   g34(.a(x20), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n45_), .c(new_n80_), .O(new_n82_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n82_), .c(new_n73_), .O(new_n84_));
  oai21  g40(.a(new_n60_), .b(new_n57_), .c(new_n84_), .O(z10));
  inv1   g41(.a(new_n45_), .O(new_n86_));
  nand2  g42(.a(new_n80_), .b(x21), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  oai21  g45(.a(new_n80_), .b(x21), .c(new_n73_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n89_), .c(new_n62_), .O(z11));
  aoi21  g47(.a(new_n86_), .b(x14), .c(new_n50_), .O(new_n92_));
  inv1   g48(.a(new_n87_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(x22), .c(new_n73_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n92_), .c(new_n64_), .O(z12));
  inv1   g51(.a(x23), .O(new_n96_));
  xor2a  g52(.a(new_n50_), .b(new_n96_), .O(new_n97_));
  aoi21  g53(.a(new_n86_), .b(x15), .c(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n49_), .c(new_n66_), .O(z13));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nor2   g57(.a(new_n50_), .b(new_n96_), .O(new_n102_));
  xor2a  g58(.a(new_n102_), .b(x24), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n101_), .c(new_n48_), .O(new_n104_));
  nand2  g60(.a(x08), .b(x05), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n104_), .c(x10), .O(z14));
  nand3  g62(.a(x26), .b(x25), .c(x17), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n102_), .b(x25), .c(x24), .O(new_n109_));
  inv1   g65(.a(x25), .O(new_n110_));
  inv1   g66(.a(new_n50_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x24), .c(x23), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n109_), .c(new_n108_), .O(new_n114_));
  aoi21  g70(.a(x08), .b(x06), .c(new_n53_), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n49_), .c(new_n115_), .O(z15));
  inv1   g72(.a(new_n109_), .O(new_n117_));
  nor2   g73(.a(new_n50_), .b(x18), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n86_), .c(new_n49_), .O(new_n119_));
  oai21  g75(.a(new_n117_), .b(x26), .c(new_n119_), .O(new_n120_));
  aoi21  g76(.a(x08), .b(x07), .c(new_n53_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(z16));
endmodule


