// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_;
  inv1   g00(.a(x17), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x09), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  oai21  g11(.a(new_n55_), .b(new_n50_), .c(new_n47_), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  nand2  g13(.a(new_n47_), .b(x08), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n57_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  nor2   g16(.a(new_n58_), .b(new_n60_), .O(z02));
  inv1   g17(.a(x02), .O(new_n62_));
  nor2   g18(.a(new_n58_), .b(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nor2   g20(.a(new_n58_), .b(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  nor2   g22(.a(new_n58_), .b(new_n66_), .O(z05));
  nor2   g23(.a(new_n46_), .b(new_n48_), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(new_n68_), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n47_), .O(z08));
  inv1   g30(.a(new_n50_), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n51_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai21  g34(.a(new_n58_), .b(new_n57_), .c(new_n78_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  and2   g36(.a(x22), .b(x21), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n52_), .c(x12), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n50_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  aoi21  g42(.a(x08), .b(x01), .c(new_n46_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n86_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n52_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n80_), .b(x21), .c(new_n75_), .O(new_n92_));
  aoi21  g48(.a(x08), .b(x02), .c(new_n46_), .O(new_n93_));
  oai21  g49(.a(new_n92_), .b(new_n91_), .c(new_n93_), .O(z11));
  aoi21  g50(.a(new_n52_), .b(x14), .c(new_n53_), .O(new_n95_));
  inv1   g51(.a(new_n89_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(x22), .c(new_n75_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n95_), .c(new_n58_), .d(new_n64_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n51_), .b(new_n99_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  xor2a  g57(.a(new_n53_), .b(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n75_), .O(new_n103_));
  oai21  g59(.a(new_n58_), .b(new_n66_), .c(new_n103_), .O(z13));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n81_), .b(new_n80_), .c(x24), .d(x23), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n53_), .b(new_n101_), .c(new_n108_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n50_), .c(new_n69_), .O(z14));
  nor2   g67(.a(new_n53_), .b(new_n101_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(x24), .c(x25), .O(new_n113_));
  and2   g69(.a(x25), .b(x24), .O(new_n114_));
  nand2  g70(.a(x26), .b(x17), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n112_), .c(new_n114_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n75_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n113_), .c(new_n71_), .O(z15));
  aoi21  g74(.a(new_n112_), .b(new_n114_), .c(x26), .O(new_n119_));
  oai21  g75(.a(new_n55_), .b(x18), .c(new_n75_), .O(new_n120_));
  nand2  g76(.a(new_n68_), .b(x07), .O(new_n121_));
  oai21  g77(.a(new_n120_), .b(new_n119_), .c(new_n121_), .O(z16));
endmodule


