// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n60_, new_n62_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n49_), .c(new_n48_), .O(z00));
  inv1   g10(.a(x08), .O(new_n55_));
  nand2  g11(.a(x10), .b(new_n55_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  and2   g14(.a(x08), .b(x01), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  nor2   g16(.a(new_n55_), .b(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n56_), .O(z04));
  and2   g19(.a(x08), .b(x04), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n56_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n56_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n56_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n73_), .c(new_n71_), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n53_), .c(new_n57_), .O(z09));
  nand2  g33(.a(x20), .b(x19), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n73_), .c(new_n78_), .O(new_n81_));
  oai21  g37(.a(x20), .b(x19), .c(new_n52_), .O(new_n82_));
  inv1   g38(.a(new_n56_), .O(new_n83_));
  nor2   g39(.a(z02), .b(new_n83_), .O(new_n84_));
  oai21  g40(.a(new_n82_), .b(new_n81_), .c(new_n84_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n73_), .c(new_n86_), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(new_n78_), .b(new_n89_), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(new_n52_), .c(new_n50_), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n88_), .c(new_n55_), .d(new_n60_), .O(z11));
  inv1   g48(.a(new_n86_), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(x22), .O(new_n94_));
  aoi21  g50(.a(new_n73_), .b(x14), .c(new_n94_), .O(new_n95_));
  inv1   g51(.a(x22), .O(new_n96_));
  nand2  g52(.a(new_n86_), .b(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n52_), .c(new_n50_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n95_), .c(new_n62_), .O(z12));
  inv1   g55(.a(new_n49_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(x15), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n47_), .O(new_n102_));
  nand2  g58(.a(new_n46_), .b(new_n45_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n102_), .c(new_n52_), .O(new_n104_));
  nor2   g60(.a(z05), .b(new_n83_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n46_), .b(new_n45_), .c(new_n109_), .O(new_n110_));
  and2   g66(.a(x20), .b(x19), .O(new_n111_));
  and2   g67(.a(x22), .b(x21), .O(new_n112_));
  and2   g68(.a(x24), .b(x23), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n112_), .c(new_n111_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n110_), .c(new_n108_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n53_), .c(new_n65_), .O(z14));
  nand2  g72(.a(x24), .b(x23), .O(new_n117_));
  nor2   g73(.a(new_n117_), .b(new_n46_), .O(new_n118_));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand2  g77(.a(new_n114_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n52_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n67_), .c(new_n56_), .O(z15));
  inv1   g80(.a(z08), .O(new_n125_));
  aoi21  g81(.a(new_n118_), .b(x25), .c(x26), .O(new_n126_));
  nand3  g82(.a(x22), .b(x21), .c(x20), .O(new_n127_));
  inv1   g83(.a(new_n127_), .O(new_n128_));
  nor2   g84(.a(new_n71_), .b(x18), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n128_), .c(new_n73_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n52_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n126_), .c(new_n125_), .O(z16));
endmodule


