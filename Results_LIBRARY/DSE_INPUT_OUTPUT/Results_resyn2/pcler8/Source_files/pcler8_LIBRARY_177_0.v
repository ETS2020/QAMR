// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(x24), .b(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x25), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x11), .c(new_n50_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  inv1   g10(.a(x11), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n56_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n56_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n56_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nand2  g21(.a(new_n56_), .b(x08), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z05));
  inv1   g23(.a(new_n66_), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(new_n68_), .b(x06), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n56_), .O(z08));
  inv1   g30(.a(x00), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x20), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  oai22  g37(.a(new_n81_), .b(new_n53_), .c(new_n66_), .d(new_n75_), .O(z09));
  inv1   g38(.a(x20), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n76_), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n77_), .c(new_n84_), .O(new_n86_));
  inv1   g42(.a(x08), .O(new_n87_));
  nand2  g43(.a(x09), .b(new_n87_), .O(new_n88_));
  aoi21  g44(.a(new_n83_), .b(new_n76_), .c(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n86_), .c(new_n55_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(x10), .c(new_n59_), .O(z10));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  nand3  g48(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand3  g50(.a(x23), .b(x22), .c(x13), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n94_), .c(new_n92_), .O(new_n97_));
  inv1   g53(.a(new_n53_), .O(new_n98_));
  inv1   g54(.a(x21), .O(new_n99_));
  oai21  g55(.a(new_n83_), .b(new_n76_), .c(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g57(.a(new_n68_), .b(x02), .O(new_n102_));
  oai21  g58(.a(new_n101_), .b(new_n97_), .c(new_n102_), .O(z11));
  inv1   g59(.a(x14), .O(new_n104_));
  inv1   g60(.a(new_n47_), .O(new_n105_));
  oai21  g61(.a(new_n77_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  inv1   g62(.a(x22), .O(new_n107_));
  aoi21  g63(.a(new_n92_), .b(new_n107_), .c(new_n88_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n106_), .c(new_n55_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x10), .c(new_n63_), .O(z12));
  nand2  g66(.a(new_n94_), .b(x15), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n105_), .c(x23), .O(new_n112_));
  inv1   g68(.a(x23), .O(new_n113_));
  nand2  g69(.a(new_n47_), .b(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n98_), .O(new_n115_));
  oai21  g71(.a(new_n66_), .b(new_n65_), .c(new_n115_), .O(z13));
  nand3  g72(.a(new_n105_), .b(x24), .c(x23), .O(new_n117_));
  inv1   g73(.a(x24), .O(new_n118_));
  oai21  g74(.a(new_n47_), .b(new_n113_), .c(new_n118_), .O(new_n119_));
  aoi22  g75(.a(new_n119_), .b(new_n117_), .c(new_n94_), .d(x16), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n53_), .c(new_n69_), .O(z14));
  nand3  g77(.a(x26), .b(x25), .c(x17), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  oai21  g80(.a(new_n47_), .b(new_n46_), .c(new_n124_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n49_), .c(new_n123_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n53_), .c(new_n71_), .O(z15));
  aoi21  g83(.a(new_n48_), .b(x25), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n78_), .b(new_n105_), .c(new_n129_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n98_), .O(new_n131_));
  nand2  g87(.a(new_n68_), .b(x07), .O(new_n132_));
  oai21  g88(.a(new_n131_), .b(new_n128_), .c(new_n132_), .O(z16));
endmodule


