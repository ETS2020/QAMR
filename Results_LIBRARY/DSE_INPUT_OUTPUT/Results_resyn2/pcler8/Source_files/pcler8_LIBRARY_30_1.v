// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  nor2   g07(.a(new_n45_), .b(x08), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n53_), .O(z01));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n53_), .O(z02));
  aoi21  g13(.a(x08), .b(x02), .c(new_n52_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  and2   g15(.a(x08), .b(x03), .O(z04));
  and2   g16(.a(x08), .b(x04), .O(z05));
  and2   g17(.a(x08), .b(x05), .O(z06));
  nand2  g18(.a(x08), .b(x06), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z07));
  aoi21  g20(.a(x08), .b(x07), .c(new_n52_), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z08));
  inv1   g22(.a(x09), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(x08), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n45_), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(new_n70_));
  nand4  g26(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n71_));
  and2   g27(.a(x22), .b(x21), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x20), .c(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n71_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n54_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  inv1   g34(.a(new_n71_), .O(new_n79_));
  nand2  g35(.a(new_n72_), .b(x12), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  oai21  g38(.a(x20), .b(x19), .c(new_n70_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(new_n56_), .O(z10));
  nand2  g40(.a(new_n77_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n79_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n77_), .b(x21), .c(new_n68_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n58_), .O(z11));
  aoi21  g45(.a(new_n79_), .b(x14), .c(new_n47_), .O(new_n90_));
  inv1   g46(.a(new_n85_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x22), .c(new_n68_), .O(new_n92_));
  nor2   g48(.a(z04), .b(new_n52_), .O(new_n93_));
  oai21  g49(.a(new_n92_), .b(new_n90_), .c(new_n93_), .O(z12));
  inv1   g50(.a(x15), .O(new_n95_));
  nor2   g51(.a(new_n71_), .b(new_n95_), .O(new_n96_));
  xor2a  g52(.a(new_n47_), .b(new_n46_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n68_), .O(new_n98_));
  nor2   g54(.a(z05), .b(new_n52_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(z13));
  inv1   g56(.a(new_n68_), .O(new_n101_));
  nand4  g57(.a(new_n72_), .b(new_n77_), .c(x24), .d(x23), .O(new_n102_));
  nand3  g58(.a(new_n49_), .b(x24), .c(x16), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  inv1   g60(.a(x24), .O(new_n105_));
  oai21  g61(.a(new_n47_), .b(new_n46_), .c(new_n105_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n102_), .c(new_n104_), .O(new_n107_));
  nor2   g63(.a(z06), .b(new_n52_), .O(new_n108_));
  oai21  g64(.a(new_n107_), .b(new_n101_), .c(new_n108_), .O(z14));
  nand2  g65(.a(new_n49_), .b(x17), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n48_), .b(x25), .c(x24), .O(new_n112_));
  inv1   g68(.a(x25), .O(new_n113_));
  nand2  g69(.a(new_n102_), .b(new_n113_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n111_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n69_), .c(new_n63_), .O(z15));
  inv1   g72(.a(new_n112_), .O(new_n117_));
  nor2   g73(.a(new_n47_), .b(x18), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n79_), .c(new_n101_), .O(new_n119_));
  oai21  g75(.a(new_n117_), .b(x26), .c(new_n119_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n65_), .O(z16));
endmodule


