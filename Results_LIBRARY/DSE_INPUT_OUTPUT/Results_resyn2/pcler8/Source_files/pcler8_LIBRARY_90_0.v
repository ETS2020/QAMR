// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_;
  nor2   g00(.a(x17), .b(x01), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  and2   g06(.a(x20), .b(x19), .O(new_n51_));
  nand4  g07(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  and2   g09(.a(x22), .b(x21), .O(new_n54_));
  and2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  aoi21  g13(.a(x08), .b(x00), .c(new_n45_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  and2   g15(.a(x08), .b(x01), .O(z02));
  aoi21  g16(.a(x08), .b(x02), .c(new_n45_), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n45_), .O(z05));
  nand3  g22(.a(new_n46_), .b(x08), .c(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n45_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n45_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  and2   g29(.a(x20), .b(x11), .O(new_n74_));
  aoi21  g30(.a(new_n74_), .b(new_n55_), .c(new_n73_), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n49_), .c(new_n58_), .O(z09));
  inv1   g32(.a(new_n51_), .O(new_n77_));
  aoi21  g33(.a(new_n55_), .b(x12), .c(new_n77_), .O(new_n78_));
  oai21  g34(.a(x20), .b(x19), .c(new_n50_), .O(new_n79_));
  nor2   g35(.a(z02), .b(new_n45_), .O(new_n80_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(new_n80_), .O(z10));
  nand2  g37(.a(new_n51_), .b(x21), .O(new_n82_));
  and2   g38(.a(x22), .b(x13), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n53_), .c(new_n82_), .O(new_n84_));
  oai21  g40(.a(new_n51_), .b(x21), .c(new_n50_), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n84_), .c(new_n61_), .O(z11));
  nand3  g42(.a(new_n51_), .b(x22), .c(x21), .O(new_n87_));
  aoi21  g43(.a(new_n53_), .b(x14), .c(new_n87_), .O(new_n88_));
  inv1   g44(.a(new_n82_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(x22), .c(new_n50_), .O(new_n90_));
  aoi21  g46(.a(x08), .b(x03), .c(new_n45_), .O(new_n91_));
  oai21  g47(.a(new_n90_), .b(new_n88_), .c(new_n91_), .O(z12));
  inv1   g48(.a(x15), .O(new_n93_));
  nor2   g49(.a(new_n52_), .b(new_n93_), .O(new_n94_));
  inv1   g50(.a(x23), .O(new_n95_));
  nand4  g51(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n96_));
  xor2a  g52(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n94_), .c(new_n50_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n65_), .c(new_n46_), .O(z13));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nand4  g57(.a(new_n54_), .b(new_n51_), .c(x24), .d(x23), .O(new_n102_));
  inv1   g58(.a(x24), .O(new_n103_));
  oai21  g59(.a(new_n96_), .b(new_n95_), .c(new_n103_), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n102_), .c(new_n101_), .O(new_n105_));
  nand2  g61(.a(new_n50_), .b(new_n46_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n105_), .c(new_n67_), .O(z14));
  nand3  g63(.a(x26), .b(x25), .c(x17), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  inv1   g65(.a(new_n96_), .O(new_n110_));
  nand4  g66(.a(new_n110_), .b(x25), .c(x24), .d(x23), .O(new_n111_));
  inv1   g67(.a(x25), .O(new_n112_));
  nand2  g68(.a(new_n102_), .b(new_n112_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  aoi21  g70(.a(x08), .b(x06), .c(new_n45_), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n49_), .c(new_n115_), .O(z15));
  inv1   g72(.a(x26), .O(new_n117_));
  nand2  g73(.a(new_n111_), .b(new_n117_), .O(new_n118_));
  inv1   g74(.a(x18), .O(new_n119_));
  nand3  g75(.a(new_n55_), .b(new_n51_), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n118_), .c(new_n50_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n71_), .c(new_n46_), .O(z16));
endmodule


