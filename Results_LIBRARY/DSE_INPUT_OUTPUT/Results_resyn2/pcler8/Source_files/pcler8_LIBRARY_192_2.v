// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_;
  nand2  g00(.a(x26), .b(x25), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g04(.a(x23), .O(new_n49_));
  nand4  g05(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(x24), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand3  g09(.a(x17), .b(x08), .c(x00), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z01));
  nand3  g11(.a(x17), .b(x08), .c(x01), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z02));
  nand3  g13(.a(x17), .b(x08), .c(x02), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z03));
  nand3  g15(.a(x17), .b(x08), .c(x03), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z04));
  inv1   g17(.a(x17), .O(new_n62_));
  oai21  g18(.a(new_n62_), .b(x04), .c(x08), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z05));
  oai21  g20(.a(new_n62_), .b(x05), .c(x08), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z06));
  inv1   g22(.a(x06), .O(new_n67_));
  aoi21  g23(.a(x17), .b(new_n67_), .c(new_n46_), .O(z07));
  nand3  g24(.a(x17), .b(x08), .c(x07), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  inv1   g26(.a(new_n48_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n54_), .O(z09));
  oai21  g32(.a(new_n62_), .b(x01), .c(x08), .O(new_n77_));
  and2   g33(.a(x20), .b(x19), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n72_), .c(new_n78_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n48_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n77_), .O(z10));
  oai21  g40(.a(new_n62_), .b(x02), .c(x08), .O(new_n85_));
  inv1   g41(.a(new_n72_), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  oai21  g45(.a(new_n78_), .b(x21), .c(new_n71_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n89_), .c(new_n85_), .O(z11));
  oai21  g47(.a(new_n62_), .b(x03), .c(x08), .O(new_n92_));
  aoi21  g48(.a(new_n86_), .b(x14), .c(new_n50_), .O(new_n93_));
  inv1   g49(.a(new_n87_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x22), .c(new_n71_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(z12));
  inv1   g52(.a(x15), .O(new_n97_));
  nor2   g53(.a(new_n72_), .b(new_n97_), .O(new_n98_));
  xor2a  g54(.a(new_n50_), .b(new_n49_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n98_), .c(new_n71_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n63_), .O(z13));
  nand3  g57(.a(x26), .b(x25), .c(x16), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n51_), .c(x24), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  and2   g60(.a(x22), .b(x21), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n78_), .c(x23), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n103_), .c(new_n71_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n65_), .O(z14));
  nand2  g65(.a(x08), .b(x06), .O(new_n110_));
  oai21  g66(.a(new_n48_), .b(new_n45_), .c(new_n110_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(x17), .O(new_n112_));
  aoi21  g68(.a(new_n51_), .b(x24), .c(x25), .O(new_n113_));
  nand2  g69(.a(x25), .b(x24), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n106_), .c(new_n71_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n113_), .c(new_n112_), .O(z15));
  inv1   g72(.a(new_n114_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n51_), .c(x26), .O(new_n118_));
  inv1   g74(.a(x18), .O(new_n119_));
  nand4  g75(.a(x26), .b(x25), .c(x24), .d(new_n119_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n106_), .c(new_n71_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n118_), .c(new_n69_), .O(z16));
endmodule


