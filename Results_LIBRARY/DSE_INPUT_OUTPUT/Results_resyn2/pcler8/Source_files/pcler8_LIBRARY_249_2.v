// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x09), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x08), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(new_n46_), .c(x23), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(x10), .O(z00));
  inv1   g07(.a(x24), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(x10), .O(new_n53_));
  nand2  g09(.a(x08), .b(x00), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n53_), .O(z01));
  nand2  g11(.a(x08), .b(x01), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n53_), .O(z02));
  inv1   g13(.a(x02), .O(new_n58_));
  nand2  g14(.a(new_n53_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n53_), .O(z04));
  inv1   g18(.a(x04), .O(new_n63_));
  nor2   g19(.a(new_n59_), .b(new_n63_), .O(z05));
  inv1   g20(.a(x05), .O(new_n65_));
  nor2   g21(.a(new_n59_), .b(new_n65_), .O(z06));
  nand2  g22(.a(x08), .b(x06), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n53_), .O(z07));
  inv1   g24(.a(x07), .O(new_n69_));
  nor2   g25(.a(new_n59_), .b(new_n69_), .O(z08));
  inv1   g26(.a(x08), .O(new_n71_));
  inv1   g27(.a(x10), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x09), .c(new_n71_), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x20), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n76_), .c(x11), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n54_), .c(new_n53_), .O(z09));
  and2   g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n75_), .c(new_n83_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n73_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n56_), .c(new_n53_), .O(z10));
  nand2  g45(.a(new_n83_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n76_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n83_), .b(x21), .c(new_n74_), .O(new_n93_));
  aoi22  g49(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z11));
  inv1   g51(.a(z04), .O(new_n96_));
  nand3  g52(.a(new_n83_), .b(x22), .c(x21), .O(new_n97_));
  aoi21  g53(.a(new_n76_), .b(x14), .c(new_n97_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n90_), .b(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n74_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n98_), .c(new_n96_), .O(z12));
  inv1   g58(.a(x23), .O(new_n103_));
  nor2   g59(.a(new_n47_), .b(new_n103_), .O(new_n104_));
  inv1   g60(.a(new_n50_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  aoi21  g63(.a(new_n47_), .b(new_n103_), .c(new_n73_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g65(.a(new_n59_), .b(new_n63_), .c(new_n109_), .O(z13));
  nand2  g66(.a(new_n49_), .b(new_n72_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n52_), .O(new_n112_));
  nor2   g68(.a(new_n71_), .b(new_n65_), .O(new_n113_));
  nand3  g69(.a(x26), .b(x25), .c(x16), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n104_), .O(new_n115_));
  nand4  g71(.a(x24), .b(new_n72_), .c(x09), .d(new_n71_), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n113_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n112_), .O(z14));
  nand2  g75(.a(x26), .b(x17), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n48_), .c(x23), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(x24), .c(x23), .O(new_n123_));
  nor2   g79(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  aoi21  g80(.a(new_n121_), .b(x25), .c(new_n124_), .O(new_n125_));
  nand3  g81(.a(x25), .b(x09), .c(new_n71_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n72_), .O(new_n127_));
  aoi22  g83(.a(new_n127_), .b(new_n52_), .c(x08), .d(x06), .O(new_n128_));
  oai21  g84(.a(new_n125_), .b(new_n73_), .c(new_n128_), .O(z15));
  inv1   g85(.a(x18), .O(new_n130_));
  nand4  g86(.a(new_n78_), .b(new_n76_), .c(x19), .d(new_n130_), .O(new_n131_));
  inv1   g87(.a(x26), .O(new_n132_));
  nand3  g88(.a(x25), .b(x24), .c(x23), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n47_), .c(new_n132_), .O(new_n134_));
  nand3  g90(.a(new_n134_), .b(new_n131_), .c(new_n74_), .O(new_n135_));
  oai21  g91(.a(new_n59_), .b(new_n69_), .c(new_n135_), .O(z16));
endmodule


