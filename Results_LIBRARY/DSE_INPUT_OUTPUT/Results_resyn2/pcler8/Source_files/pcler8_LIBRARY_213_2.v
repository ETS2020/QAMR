// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x03), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(new_n45_), .c(x10), .O(z00));
  nor2   g10(.a(x10), .b(new_n45_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  nand2  g15(.a(new_n56_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  inv1   g17(.a(x02), .O(new_n62_));
  nor2   g18(.a(new_n60_), .b(new_n62_), .O(z03));
  nand3  g19(.a(x10), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n55_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n56_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  inv1   g26(.a(x08), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n70_), .c(new_n56_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n56_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n52_), .c(x03), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(x10), .c(new_n57_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n75_), .c(new_n81_), .O(new_n83_));
  nand2  g39(.a(x09), .b(new_n71_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n83_), .c(x03), .O(new_n87_));
  oai22  g43(.a(new_n87_), .b(x10), .c(new_n71_), .d(new_n59_), .O(z10));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand2  g46(.a(x22), .b(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n75_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  aoi21  g49(.a(new_n80_), .b(new_n93_), .c(new_n84_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(x03), .O(new_n95_));
  oai22  g51(.a(new_n95_), .b(x10), .c(new_n71_), .d(new_n62_), .O(z11));
  inv1   g52(.a(x10), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x08), .c(x03), .O(new_n98_));
  inv1   g54(.a(new_n75_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(x14), .c(new_n47_), .O(new_n100_));
  inv1   g56(.a(x22), .O(new_n101_));
  nand2  g57(.a(new_n89_), .b(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n52_), .c(new_n97_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n100_), .c(new_n98_), .O(z12));
  nand2  g60(.a(new_n50_), .b(x15), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n48_), .O(new_n106_));
  aoi21  g62(.a(new_n47_), .b(new_n46_), .c(new_n84_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n106_), .c(x03), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(x10), .c(new_n66_), .O(z13));
  nand3  g65(.a(new_n52_), .b(new_n97_), .c(new_n45_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n47_), .b(new_n46_), .c(new_n111_), .O(new_n112_));
  nand2  g68(.a(new_n48_), .b(x24), .O(new_n113_));
  aoi22  g69(.a(new_n113_), .b(new_n112_), .c(new_n50_), .d(x16), .O(new_n114_));
  nand3  g70(.a(new_n56_), .b(x08), .c(x05), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n110_), .c(new_n115_), .O(z14));
  inv1   g72(.a(new_n110_), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n113_), .b(new_n118_), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n118_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n48_), .c(x24), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(new_n117_), .O(new_n122_));
  oai21  g78(.a(new_n60_), .b(new_n70_), .c(new_n122_), .O(z15));
  inv1   g79(.a(new_n47_), .O(new_n124_));
  nand3  g80(.a(x25), .b(x24), .c(x23), .O(new_n125_));
  inv1   g81(.a(new_n125_), .O(new_n126_));
  inv1   g82(.a(x26), .O(new_n127_));
  nor2   g83(.a(new_n127_), .b(x18), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n126_), .c(new_n124_), .O(new_n129_));
  oai21  g85(.a(new_n125_), .b(new_n47_), .c(new_n127_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n52_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n45_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n97_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n73_), .O(z16));
endmodule


