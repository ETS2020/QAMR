// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x17), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x06), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  nand4  g08(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nand4  g09(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n51_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n50_), .O(z02));
  nand3  g15(.a(new_n51_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n51_), .O(z04));
  nand3  g19(.a(new_n51_), .b(x08), .c(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  nand3  g21(.a(new_n51_), .b(x08), .c(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  and2   g23(.a(x08), .b(x06), .O(z07));
  nand3  g24(.a(new_n51_), .b(x08), .c(x07), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  nand4  g26(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n53_), .c(x19), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n48_), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(new_n56_), .c(new_n51_), .O(z09));
  inv1   g30(.a(new_n53_), .O(new_n75_));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n75_), .c(new_n76_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n48_), .O(new_n80_));
  aoi21  g36(.a(x08), .b(x01), .c(new_n50_), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n81_), .O(z10));
  nand3  g38(.a(x21), .b(x20), .c(x19), .O(new_n83_));
  and2   g39(.a(x22), .b(x13), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n75_), .c(new_n83_), .O(new_n85_));
  nor2   g41(.a(new_n50_), .b(new_n47_), .O(new_n86_));
  inv1   g42(.a(x21), .O(new_n87_));
  nand2  g43(.a(new_n76_), .b(new_n87_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n85_), .c(new_n60_), .O(z11));
  inv1   g46(.a(x14), .O(new_n91_));
  inv1   g47(.a(new_n54_), .O(new_n92_));
  oai21  g48(.a(new_n53_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  aoi21  g50(.a(new_n83_), .b(new_n94_), .c(new_n47_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n62_), .c(new_n50_), .O(z12));
  inv1   g53(.a(x15), .O(new_n98_));
  nor2   g54(.a(new_n53_), .b(new_n98_), .O(new_n99_));
  nand2  g55(.a(new_n92_), .b(x23), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nand2  g57(.a(new_n54_), .b(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n100_), .c(new_n99_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n52_), .c(new_n64_), .O(z13));
  nand4  g60(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n92_), .b(x24), .c(x23), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n54_), .b(new_n101_), .c(new_n108_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n107_), .c(new_n106_), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n52_), .c(new_n66_), .O(z14));
  inv1   g67(.a(x25), .O(new_n112_));
  nand2  g68(.a(new_n107_), .b(new_n112_), .O(new_n113_));
  nand4  g69(.a(new_n92_), .b(x25), .c(x24), .d(x23), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n48_), .O(new_n115_));
  nand4  g71(.a(x26), .b(x25), .c(new_n46_), .d(x09), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(x06), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(x17), .c(z07), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n115_), .O(z15));
  inv1   g75(.a(x26), .O(new_n120_));
  nand2  g76(.a(new_n114_), .b(new_n120_), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand3  g78(.a(new_n92_), .b(new_n75_), .c(new_n122_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n121_), .c(new_n86_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n69_), .O(z16));
endmodule


