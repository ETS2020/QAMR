// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor3   g05(.a(new_n49_), .b(new_n48_), .c(new_n45_), .O(z00));
  inv1   g06(.a(x00), .O(new_n51_));
  nor2   g07(.a(x22), .b(new_n47_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  oai21  g09(.a(new_n46_), .b(new_n51_), .c(new_n53_), .O(z01));
  inv1   g10(.a(x01), .O(new_n55_));
  nand2  g11(.a(new_n53_), .b(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z02));
  inv1   g13(.a(x02), .O(new_n58_));
  nor2   g14(.a(new_n56_), .b(new_n58_), .O(z03));
  nand2  g15(.a(x08), .b(x03), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n53_), .O(z04));
  nand2  g17(.a(x08), .b(x04), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n53_), .O(z05));
  inv1   g19(.a(x05), .O(new_n64_));
  nor2   g20(.a(new_n56_), .b(new_n64_), .O(z06));
  inv1   g21(.a(x06), .O(new_n66_));
  nor2   g22(.a(new_n46_), .b(new_n66_), .O(new_n67_));
  or2    g23(.a(new_n67_), .b(new_n52_), .O(z07));
  inv1   g24(.a(x07), .O(new_n69_));
  oai21  g25(.a(new_n46_), .b(new_n69_), .c(new_n53_), .O(z08));
  inv1   g26(.a(new_n48_), .O(new_n71_));
  nand4  g27(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n45_), .c(x19), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n71_), .O(new_n74_));
  oai21  g30(.a(new_n56_), .b(new_n51_), .c(new_n74_), .O(z09));
  inv1   g31(.a(x19), .O(new_n76_));
  inv1   g32(.a(x20), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n45_), .c(new_n78_), .O(new_n80_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n48_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  aoi21  g38(.a(x08), .b(x01), .c(new_n52_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n82_), .O(z10));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  inv1   g41(.a(x24), .O(new_n86_));
  inv1   g42(.a(x25), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand4  g44(.a(x26), .b(x23), .c(x22), .d(x13), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n85_), .O(new_n91_));
  oai21  g47(.a(new_n78_), .b(x21), .c(new_n71_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n91_), .c(new_n56_), .d(new_n58_), .O(z11));
  inv1   g49(.a(x14), .O(new_n94_));
  inv1   g50(.a(new_n85_), .O(new_n95_));
  oai21  g51(.a(new_n45_), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n71_), .c(x22), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(x09), .b(new_n46_), .O(new_n99_));
  oai21  g55(.a(new_n85_), .b(new_n99_), .c(new_n47_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n97_), .c(new_n60_), .O(z12));
  inv1   g58(.a(new_n45_), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n49_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n95_), .b(x23), .c(x22), .O(new_n106_));
  aoi22  g62(.a(new_n106_), .b(new_n105_), .c(new_n103_), .d(x15), .O(new_n107_));
  nand3  g63(.a(new_n53_), .b(x08), .c(x04), .O(new_n108_));
  oai21  g64(.a(new_n107_), .b(new_n48_), .c(new_n108_), .O(z13));
  nand2  g65(.a(new_n106_), .b(new_n86_), .O(new_n110_));
  and2   g66(.a(x24), .b(x23), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n95_), .c(new_n111_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n110_), .c(new_n71_), .O(new_n114_));
  nand2  g70(.a(x08), .b(x05), .O(new_n115_));
  oai21  g71(.a(new_n99_), .b(new_n86_), .c(new_n47_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n98_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(z14));
  nand2  g74(.a(x23), .b(x22), .O(new_n119_));
  nor2   g75(.a(new_n119_), .b(new_n85_), .O(new_n120_));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n95_), .c(new_n111_), .O(new_n122_));
  nor2   g78(.a(x25), .b(new_n86_), .O(new_n123_));
  aoi22  g79(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(x25), .O(new_n124_));
  oai21  g80(.a(new_n99_), .b(new_n87_), .c(new_n47_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n98_), .c(new_n67_), .O(new_n126_));
  oai21  g82(.a(new_n124_), .b(new_n48_), .c(new_n126_), .O(z15));
  aoi21  g83(.a(new_n120_), .b(new_n88_), .c(x26), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  inv1   g85(.a(new_n49_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n103_), .c(new_n129_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n71_), .O(new_n132_));
  oai22  g88(.a(new_n132_), .b(new_n128_), .c(new_n56_), .d(new_n69_), .O(z16));
endmodule


