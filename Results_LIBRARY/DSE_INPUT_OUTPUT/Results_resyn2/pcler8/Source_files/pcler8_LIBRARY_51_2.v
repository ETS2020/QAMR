// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand4  g02(.a(x26), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand3  g05(.a(x24), .b(x23), .c(x22), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(new_n49_), .c(x25), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n47_), .O(z00));
  inv1   g09(.a(x26), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x16), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n55_), .O(z02));
  nand3  g16(.a(new_n55_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n55_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n56_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n56_), .O(z06));
  nand3  g24(.a(new_n55_), .b(x08), .c(x06), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n55_), .O(z08));
  nand3  g28(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(new_n74_));
  nand3  g30(.a(x26), .b(x25), .c(x24), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(x23), .O(new_n77_));
  and2   g33(.a(x22), .b(x21), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x20), .c(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n77_), .c(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n57_), .c(new_n55_), .O(z09));
  inv1   g38(.a(x19), .O(new_n83_));
  inv1   g39(.a(x20), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g41(.a(new_n78_), .b(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n77_), .c(new_n85_), .O(new_n87_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n73_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(new_n59_), .c(new_n55_), .O(z10));
  nand2  g46(.a(x23), .b(x22), .O(new_n91_));
  nor2   g47(.a(new_n75_), .b(new_n91_), .O(new_n92_));
  nand4  g48(.a(new_n92_), .b(new_n74_), .c(x21), .d(x13), .O(new_n93_));
  or2    g49(.a(new_n85_), .b(x21), .O(new_n94_));
  nand4  g50(.a(new_n94_), .b(new_n55_), .c(new_n48_), .d(new_n74_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n93_), .c(new_n61_), .O(z11));
  nor2   g52(.a(new_n49_), .b(x22), .O(new_n97_));
  nand4  g53(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n63_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n55_), .O(new_n101_));
  nand3  g57(.a(new_n92_), .b(new_n74_), .c(x14), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n101_), .O(z12));
  nor2   g59(.a(new_n91_), .b(new_n48_), .O(new_n104_));
  nand2  g60(.a(new_n76_), .b(x15), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  inv1   g62(.a(x23), .O(new_n107_));
  aoi21  g63(.a(new_n98_), .b(new_n107_), .c(new_n73_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n65_), .c(new_n55_), .O(z13));
  aoi21  g66(.a(new_n104_), .b(x24), .c(new_n73_), .O(new_n111_));
  oai21  g67(.a(new_n104_), .b(x24), .c(new_n111_), .O(new_n112_));
  nand2  g68(.a(x25), .b(x24), .O(new_n113_));
  nor2   g69(.a(new_n113_), .b(new_n73_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n54_), .c(x16), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n112_), .c(new_n67_), .O(z14));
  nand3  g72(.a(x26), .b(x25), .c(x17), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  oai21  g74(.a(new_n50_), .b(new_n48_), .c(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n55_), .c(new_n52_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n117_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n74_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n69_), .O(z15));
  aoi21  g79(.a(new_n114_), .b(new_n104_), .c(x16), .O(new_n124_));
  inv1   g80(.a(new_n47_), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand4  g82(.a(new_n51_), .b(new_n49_), .c(x25), .d(new_n126_), .O(new_n127_));
  aoi22  g83(.a(new_n127_), .b(new_n125_), .c(x08), .d(x07), .O(new_n128_));
  oai21  g84(.a(new_n124_), .b(x26), .c(new_n128_), .O(z16));
endmodule


