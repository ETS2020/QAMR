// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x16), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g04(.a(x25), .O(new_n49_));
  inv1   g05(.a(x26), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  inv1   g07(.a(x09), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(x08), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n51_), .c(new_n48_), .d(x24), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  inv1   g12(.a(x10), .O(new_n57_));
  nand2  g13(.a(x16), .b(new_n57_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n56_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z02));
  inv1   g18(.a(new_n59_), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  inv1   g21(.a(x03), .O(new_n66_));
  nor2   g22(.a(new_n59_), .b(new_n66_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n58_), .O(z05));
  inv1   g25(.a(x05), .O(new_n70_));
  inv1   g26(.a(x08), .O(new_n71_));
  oai21  g27(.a(new_n71_), .b(new_n70_), .c(new_n58_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n58_), .O(z07));
  nand2  g30(.a(new_n63_), .b(x07), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(z08));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n53_), .c(x16), .O(new_n80_));
  oai22  g36(.a(new_n80_), .b(x10), .c(new_n71_), .d(new_n56_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  inv1   g39(.a(new_n77_), .O(new_n84_));
  and2   g40(.a(x22), .b(x21), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x12), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n84_), .c(new_n83_), .O(new_n88_));
  nand3  g44(.a(new_n53_), .b(new_n45_), .c(new_n57_), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  oai21  g46(.a(x20), .b(x19), .c(new_n90_), .O(new_n91_));
  nand2  g47(.a(new_n63_), .b(x01), .O(new_n92_));
  oai21  g48(.a(new_n91_), .b(new_n88_), .c(new_n92_), .O(z10));
  nand3  g49(.a(x21), .b(x20), .c(x19), .O(new_n94_));
  and2   g50(.a(x22), .b(x13), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n84_), .c(new_n94_), .O(new_n96_));
  oai21  g52(.a(new_n82_), .b(x21), .c(new_n90_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n96_), .c(new_n64_), .O(z11));
  inv1   g54(.a(x14), .O(new_n99_));
  inv1   g55(.a(new_n47_), .O(new_n100_));
  oai21  g56(.a(new_n77_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(x09), .b(new_n71_), .O(new_n103_));
  aoi21  g59(.a(new_n94_), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  oai22  g61(.a(new_n105_), .b(x10), .c(new_n71_), .d(new_n66_), .O(z12));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  aoi21  g64(.a(new_n47_), .b(new_n46_), .c(new_n103_), .O(new_n109_));
  aoi21  g65(.a(new_n109_), .b(new_n108_), .c(x16), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(x10), .c(new_n68_), .O(z13));
  nand4  g67(.a(new_n85_), .b(new_n82_), .c(x24), .d(x23), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n47_), .b(new_n46_), .c(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n90_), .O(new_n115_));
  oai21  g71(.a(new_n59_), .b(new_n70_), .c(new_n115_), .O(z14));
  nand2  g72(.a(new_n51_), .b(x17), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n48_), .b(x25), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n112_), .b(new_n49_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  nand2  g77(.a(new_n63_), .b(x06), .O(new_n122_));
  oai21  g78(.a(new_n121_), .b(new_n89_), .c(new_n122_), .O(z15));
  nand2  g79(.a(new_n119_), .b(new_n50_), .O(new_n124_));
  nor2   g80(.a(new_n47_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n84_), .c(new_n89_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n75_), .O(z16));
endmodule


