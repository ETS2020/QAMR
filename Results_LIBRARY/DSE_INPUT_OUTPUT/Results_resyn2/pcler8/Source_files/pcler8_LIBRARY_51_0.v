// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  nand2  g00(.a(x23), .b(x22), .O(new_n45_));
  nand3  g01(.a(x21), .b(x20), .c(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n49_), .c(new_n48_), .O(z00));
  nor2   g09(.a(x26), .b(x16), .O(new_n54_));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(z01));
  inv1   g12(.a(new_n54_), .O(new_n57_));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(z02));
  nand3  g15(.a(new_n57_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n57_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n54_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n54_), .O(z06));
  nand3  g23(.a(new_n57_), .b(x08), .c(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  aoi21  g25(.a(x08), .b(x07), .c(new_n54_), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  inv1   g27(.a(new_n52_), .O(new_n72_));
  inv1   g28(.a(new_n49_), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(x23), .O(new_n74_));
  and2   g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n74_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n72_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n55_), .c(new_n57_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  nand2  g36(.a(new_n75_), .b(x12), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n52_), .O(new_n84_));
  oai21  g40(.a(new_n82_), .b(new_n80_), .c(new_n84_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n58_), .c(new_n57_), .O(z10));
  nor3   g42(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(x21), .c(x13), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(new_n80_), .b(new_n89_), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(new_n57_), .c(new_n72_), .d(new_n46_), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(new_n88_), .c(new_n60_), .O(z11));
  inv1   g48(.a(new_n46_), .O(new_n93_));
  nor2   g49(.a(new_n93_), .b(x22), .O(new_n94_));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  nand4  g51(.a(new_n95_), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(new_n62_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n57_), .O(new_n98_));
  nand2  g54(.a(new_n87_), .b(x14), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(z12));
  nand2  g56(.a(new_n73_), .b(x15), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nor2   g58(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g60(.a(new_n95_), .b(new_n102_), .c(new_n52_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n64_), .c(new_n57_), .O(z13));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n48_), .b(new_n108_), .O(new_n109_));
  nand3  g65(.a(x26), .b(x25), .c(x16), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n47_), .c(x24), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n109_), .c(new_n72_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n66_), .c(new_n57_), .O(z14));
  nand3  g69(.a(x26), .b(x25), .c(x17), .O(new_n114_));
  inv1   g70(.a(new_n45_), .O(new_n115_));
  and2   g71(.a(x25), .b(x24), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n93_), .c(new_n115_), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand3  g74(.a(x24), .b(x23), .c(x22), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n46_), .c(new_n118_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n117_), .c(new_n57_), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n114_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n72_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n68_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(x16), .O(new_n126_));
  inv1   g82(.a(x18), .O(new_n127_));
  nand3  g83(.a(new_n116_), .b(new_n47_), .c(new_n127_), .O(new_n128_));
  nand4  g84(.a(new_n125_), .b(x25), .c(x24), .d(x23), .O(new_n129_));
  nor2   g85(.a(new_n129_), .b(new_n95_), .O(new_n130_));
  aoi21  g86(.a(new_n128_), .b(new_n126_), .c(new_n130_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n52_), .c(new_n70_), .O(z16));
endmodule


