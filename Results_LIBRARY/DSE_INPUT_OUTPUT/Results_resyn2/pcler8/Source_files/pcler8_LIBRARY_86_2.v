// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x10), .O(new_n46_));
  nand2  g02(.a(x23), .b(x22), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  inv1   g15(.a(new_n46_), .O(new_n60_));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n60_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n46_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n46_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n60_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n58_), .c(new_n46_), .O(z09));
  inv1   g35(.a(x19), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n75_), .c(new_n82_), .O(new_n84_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n54_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n61_), .c(new_n46_), .O(z10));
  inv1   g43(.a(z03), .O(new_n88_));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n51_), .c(new_n48_), .O(new_n91_));
  oai21  g47(.a(new_n82_), .b(x21), .c(new_n55_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n88_), .O(z11));
  oai21  g49(.a(new_n45_), .b(new_n53_), .c(new_n48_), .O(new_n94_));
  inv1   g50(.a(new_n75_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(x14), .c(new_n53_), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(new_n94_), .c(x22), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(x09), .b(new_n52_), .O(new_n99_));
  inv1   g55(.a(new_n48_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n53_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n98_), .c(new_n99_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n97_), .O(new_n103_));
  and2   g59(.a(new_n65_), .b(new_n46_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(z12));
  nand2  g61(.a(new_n51_), .b(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n49_), .O(new_n107_));
  nand4  g63(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n45_), .c(new_n54_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  oai21  g66(.a(new_n67_), .b(new_n60_), .c(new_n110_), .O(z13));
  inv1   g67(.a(x24), .O(new_n112_));
  oai21  g68(.a(new_n48_), .b(new_n47_), .c(new_n112_), .O(new_n113_));
  inv1   g69(.a(new_n108_), .O(new_n114_));
  nand3  g70(.a(x26), .b(x25), .c(x16), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(x24), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n113_), .c(new_n55_), .O(new_n117_));
  oai21  g73(.a(new_n99_), .b(new_n112_), .c(new_n53_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n45_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n117_), .c(new_n69_), .O(z14));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n114_), .c(x24), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(x24), .c(x23), .O(new_n124_));
  nor2   g80(.a(new_n124_), .b(new_n108_), .O(new_n125_));
  aoi21  g81(.a(new_n122_), .b(x25), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(x25), .b(x09), .c(new_n52_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n53_), .O(new_n128_));
  aoi22  g84(.a(new_n128_), .b(new_n45_), .c(x08), .d(x06), .O(new_n129_));
  oai21  g85(.a(new_n126_), .b(new_n54_), .c(new_n129_), .O(z15));
  inv1   g86(.a(x18), .O(new_n131_));
  nand3  g87(.a(x25), .b(x24), .c(new_n131_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n108_), .c(x26), .O(new_n133_));
  nor2   g89(.a(new_n112_), .b(new_n45_), .O(new_n134_));
  nor2   g90(.a(x26), .b(new_n123_), .O(new_n135_));
  nand3  g91(.a(new_n135_), .b(new_n114_), .c(new_n134_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n55_), .O(new_n138_));
  inv1   g94(.a(x26), .O(new_n139_));
  oai21  g95(.a(new_n99_), .b(new_n139_), .c(new_n53_), .O(new_n140_));
  aoi22  g96(.a(new_n140_), .b(new_n45_), .c(x08), .d(x07), .O(new_n141_));
  nand2  g97(.a(new_n141_), .b(new_n138_), .O(z16));
endmodule


