// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  nand2  g00(.a(x17), .b(x16), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand3  g03(.a(x26), .b(x25), .c(x24), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n49_), .c(new_n47_), .d(x23), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n45_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  aoi21  g14(.a(x17), .b(x16), .c(new_n58_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n45_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n45_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n45_), .O(z05));
  nand3  g21(.a(new_n45_), .b(x08), .c(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand3  g23(.a(new_n45_), .b(x08), .c(x06), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z07));
  nand3  g25(.a(new_n45_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  nand4  g28(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n73_));
  oai21  g29(.a(new_n73_), .b(new_n72_), .c(x19), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n53_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n56_), .c(new_n45_), .O(z09));
  inv1   g32(.a(x19), .O(new_n77_));
  inv1   g33(.a(x20), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n72_), .c(new_n79_), .O(new_n81_));
  aoi21  g37(.a(new_n78_), .b(new_n77_), .c(new_n52_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n58_), .c(new_n45_), .O(z10));
  inv1   g40(.a(z03), .O(new_n85_));
  inv1   g41(.a(new_n72_), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(new_n87_), .O(new_n89_));
  oai21  g45(.a(new_n79_), .b(x21), .c(new_n53_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n89_), .c(new_n85_), .O(z11));
  inv1   g47(.a(z04), .O(new_n92_));
  aoi21  g48(.a(new_n86_), .b(x14), .c(new_n46_), .O(new_n93_));
  inv1   g49(.a(new_n87_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(x22), .c(new_n53_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(z12));
  inv1   g52(.a(x15), .O(new_n97_));
  nor2   g53(.a(new_n72_), .b(new_n97_), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n46_), .b(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n53_), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(new_n64_), .c(new_n45_), .O(z13));
  and2   g58(.a(x26), .b(x25), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(x16), .O(new_n104_));
  nand2  g60(.a(x24), .b(x23), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n46_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n46_), .b(new_n99_), .c(new_n108_), .O(new_n109_));
  nand4  g65(.a(new_n45_), .b(new_n51_), .c(x09), .d(new_n50_), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n109_), .c(new_n107_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n66_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  oai21  g70(.a(new_n105_), .b(new_n46_), .c(new_n114_), .O(new_n115_));
  nand2  g71(.a(new_n106_), .b(x25), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n111_), .O(new_n117_));
  inv1   g73(.a(x09), .O(new_n118_));
  nor2   g74(.a(new_n118_), .b(x08), .O(new_n119_));
  nor2   g75(.a(x16), .b(x10), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(new_n119_), .c(new_n103_), .d(x17), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n117_), .c(new_n68_), .O(z15));
  aoi21  g78(.a(new_n106_), .b(x25), .c(x26), .O(new_n123_));
  nor2   g79(.a(new_n48_), .b(x18), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n47_), .c(x23), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n111_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n123_), .c(new_n70_), .O(z16));
endmodule


