// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x05), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n52_), .c(new_n50_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand3  g14(.a(new_n47_), .b(x08), .c(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n47_), .O(z02));
  nand3  g18(.a(new_n47_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n47_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n47_), .O(z05));
  aoi21  g24(.a(x17), .b(new_n53_), .c(new_n45_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand3  g27(.a(new_n47_), .b(x08), .c(x07), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  nor2   g29(.a(new_n55_), .b(new_n46_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand4  g34(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n75_), .c(new_n59_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  nor2   g41(.a(x20), .b(x19), .O(new_n86_));
  nor2   g42(.a(new_n86_), .b(new_n55_), .O(new_n87_));
  oai21  g43(.a(new_n85_), .b(new_n83_), .c(new_n87_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n61_), .c(new_n47_), .O(z10));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n78_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  nand2  g49(.a(new_n83_), .b(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n74_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n92_), .c(new_n63_), .O(z11));
  inv1   g52(.a(x14), .O(new_n97_));
  inv1   g53(.a(new_n49_), .O(new_n98_));
  oai21  g54(.a(new_n77_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  aoi21  g56(.a(new_n90_), .b(new_n100_), .c(new_n55_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n65_), .c(new_n46_), .O(z12));
  nand2  g59(.a(new_n52_), .b(x15), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n50_), .O(new_n105_));
  aoi21  g61(.a(new_n49_), .b(new_n48_), .c(new_n55_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n67_), .c(new_n47_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n49_), .b(new_n48_), .c(new_n111_), .O(new_n112_));
  nand3  g68(.a(new_n98_), .b(x24), .c(x23), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n114_));
  nand3  g70(.a(x17), .b(x08), .c(x05), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n75_), .c(new_n115_), .O(z14));
  inv1   g72(.a(x25), .O(new_n117_));
  nand2  g73(.a(new_n113_), .b(new_n117_), .O(new_n118_));
  aoi21  g74(.a(x26), .b(x17), .c(new_n117_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n50_), .c(x24), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n118_), .c(new_n56_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n70_), .c(new_n47_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand4  g79(.a(new_n98_), .b(x25), .c(x24), .d(x23), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand3  g82(.a(new_n78_), .b(new_n98_), .c(new_n126_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n125_), .c(new_n74_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n72_), .O(z16));
endmodule


