// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nand2  g00(.a(x16), .b(x12), .O(new_n45_));
  nand3  g01(.a(x26), .b(x25), .c(x24), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  inv1   g06(.a(x22), .O(new_n51_));
  inv1   g07(.a(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x21), .c(x20), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n50_), .c(x19), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n45_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n45_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n45_), .O(z02));
  inv1   g17(.a(new_n45_), .O(new_n62_));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n62_), .O(z03));
  nand3  g20(.a(new_n45_), .b(x08), .c(x03), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand3  g22(.a(new_n45_), .b(x08), .c(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n45_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n45_), .O(z08));
  inv1   g30(.a(new_n49_), .O(new_n75_));
  nand3  g31(.a(x23), .b(x22), .c(x21), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x24), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x20), .d(x11), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n75_), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n58_), .c(new_n62_), .O(z09));
  inv1   g38(.a(x12), .O(new_n83_));
  aoi21  g39(.a(new_n55_), .b(new_n50_), .c(x16), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nand2  g41(.a(x20), .b(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  nor2   g43(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  aoi22  g44(.a(new_n88_), .b(new_n75_), .c(x08), .d(x01), .O(new_n89_));
  oai21  g45(.a(new_n84_), .b(new_n83_), .c(new_n89_), .O(z10));
  inv1   g46(.a(new_n46_), .O(new_n91_));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  nand2  g48(.a(new_n53_), .b(x13), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n91_), .c(new_n92_), .O(new_n95_));
  oai21  g51(.a(new_n87_), .b(x21), .c(new_n75_), .O(new_n96_));
  and2   g52(.a(new_n63_), .b(new_n45_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(z11));
  inv1   g54(.a(new_n92_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(x22), .O(new_n100_));
  and2   g56(.a(x23), .b(x14), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n91_), .c(new_n100_), .O(new_n102_));
  nand2  g58(.a(new_n75_), .b(new_n45_), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nand2  g60(.a(new_n92_), .b(new_n51_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n102_), .c(new_n65_), .O(z12));
  nand2  g63(.a(new_n91_), .b(x15), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(new_n99_), .c(x23), .d(x22), .O(new_n109_));
  nand2  g65(.a(new_n100_), .b(new_n52_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n104_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n67_), .O(z13));
  nand3  g68(.a(new_n87_), .b(new_n77_), .c(x24), .O(new_n113_));
  nor2   g69(.a(new_n86_), .b(new_n76_), .O(new_n114_));
  or2    g70(.a(new_n114_), .b(x24), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n113_), .c(new_n75_), .O(new_n116_));
  oai21  g72(.a(new_n50_), .b(x12), .c(x16), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n69_), .O(z14));
  inv1   g74(.a(x25), .O(new_n119_));
  aoi21  g75(.a(x26), .b(x17), .c(new_n119_), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(new_n87_), .c(new_n77_), .d(x24), .O(new_n121_));
  nand2  g77(.a(new_n113_), .b(new_n119_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n121_), .c(new_n75_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n71_), .c(new_n62_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand3  g82(.a(x25), .b(x24), .c(new_n126_), .O(new_n127_));
  inv1   g83(.a(new_n127_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n114_), .c(new_n125_), .O(new_n129_));
  nand2  g85(.a(new_n125_), .b(x25), .O(new_n130_));
  nor2   g86(.a(new_n130_), .b(new_n113_), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n129_), .c(new_n75_), .O(new_n132_));
  aoi21  g88(.a(new_n132_), .b(new_n73_), .c(new_n62_), .O(z16));
endmodule


