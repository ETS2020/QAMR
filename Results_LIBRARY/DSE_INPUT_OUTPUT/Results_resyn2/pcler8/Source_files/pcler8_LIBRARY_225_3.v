// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  and2   g00(.a(x20), .b(x19), .O(new_n45_));
  and2   g01(.a(x22), .b(x21), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(new_n45_), .c(x23), .O(new_n47_));
  and2   g03(.a(x26), .b(x25), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  nand2  g09(.a(x10), .b(x09), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x08), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  oai21  g17(.a(new_n50_), .b(new_n61_), .c(new_n54_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n54_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n54_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  oai21  g23(.a(new_n50_), .b(new_n67_), .c(new_n54_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n54_), .O(z07));
  nand2  g26(.a(new_n56_), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  inv1   g28(.a(new_n52_), .O(new_n73_));
  and2   g29(.a(x24), .b(x23), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n48_), .O(new_n75_));
  nand3  g31(.a(x22), .b(x21), .c(x20), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n75_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n73_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n57_), .O(z09));
  nand2  g37(.a(new_n46_), .b(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n75_), .c(new_n45_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n52_), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n59_), .O(z10));
  inv1   g43(.a(new_n75_), .O(new_n88_));
  nand2  g44(.a(new_n45_), .b(x21), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n45_), .b(x21), .c(new_n73_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(new_n91_), .c(new_n55_), .d(new_n61_), .O(z11));
  inv1   g49(.a(x09), .O(new_n94_));
  inv1   g50(.a(x14), .O(new_n95_));
  nand4  g51(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n75_), .b(new_n95_), .c(new_n97_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  aoi21  g55(.a(new_n89_), .b(new_n99_), .c(x08), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n98_), .c(x10), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n94_), .c(new_n63_), .O(z12));
  inv1   g58(.a(x15), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand4  g60(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  oai21  g62(.a(new_n49_), .b(new_n103_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n105_), .b(new_n104_), .c(x08), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(x10), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n94_), .c(new_n65_), .O(z13));
  inv1   g66(.a(new_n49_), .O(new_n111_));
  inv1   g67(.a(x24), .O(new_n112_));
  nand2  g68(.a(x24), .b(x23), .O(new_n113_));
  nor2   g69(.a(new_n113_), .b(new_n105_), .O(new_n114_));
  aoi21  g70(.a(new_n47_), .b(new_n112_), .c(new_n114_), .O(new_n115_));
  aoi21  g71(.a(new_n111_), .b(x16), .c(new_n115_), .O(new_n116_));
  oai22  g72(.a(new_n116_), .b(new_n52_), .c(new_n55_), .d(new_n67_), .O(z14));
  nand3  g73(.a(x26), .b(x25), .c(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  nand4  g75(.a(new_n74_), .b(new_n46_), .c(new_n45_), .d(x25), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  oai21  g77(.a(new_n113_), .b(new_n105_), .c(new_n121_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n120_), .c(new_n119_), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(x08), .c(new_n51_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(x09), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n69_), .O(z15));
  aoi21  g82(.a(new_n114_), .b(x25), .c(x26), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand3  g84(.a(new_n77_), .b(x19), .c(new_n128_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n75_), .c(new_n73_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n127_), .c(new_n71_), .O(z16));
endmodule


