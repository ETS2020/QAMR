// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n62_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x17), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x25), .b(x24), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(x10), .b(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(x26), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(x08), .O(z00));
  nor2   g09(.a(new_n45_), .b(x08), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  inv1   g14(.a(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z02));
  and2   g16(.a(x08), .b(x02), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n55_), .O(z04));
  and2   g19(.a(x08), .b(x04), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n55_), .O(z06));
  nor2   g22(.a(new_n59_), .b(x06), .O(new_n67_));
  aoi21  g23(.a(new_n45_), .b(new_n59_), .c(new_n67_), .O(z07));
  nand2  g24(.a(x08), .b(x07), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z08));
  inv1   g26(.a(x19), .O(new_n71_));
  nand2  g27(.a(x22), .b(x21), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  and2   g30(.a(x20), .b(x11), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n74_), .c(new_n71_), .O(new_n76_));
  nand3  g32(.a(new_n51_), .b(new_n45_), .c(new_n59_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(new_n56_), .O(z09));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  aoi21  g35(.a(new_n74_), .b(x12), .c(new_n79_), .O(new_n80_));
  inv1   g36(.a(x20), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n71_), .O(new_n82_));
  nand4  g38(.a(new_n82_), .b(new_n51_), .c(new_n45_), .d(new_n59_), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n80_), .c(new_n59_), .d(new_n58_), .O(z10));
  inv1   g40(.a(new_n73_), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x22), .b(x13), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n85_), .c(new_n86_), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(new_n79_), .b(new_n89_), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(new_n51_), .c(new_n59_), .O(new_n91_));
  nor2   g47(.a(z03), .b(new_n54_), .O(new_n92_));
  oai21  g48(.a(new_n91_), .b(new_n88_), .c(new_n92_), .O(z11));
  inv1   g49(.a(new_n86_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(x22), .O(new_n95_));
  aoi21  g51(.a(new_n85_), .b(x14), .c(new_n95_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n86_), .b(new_n97_), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n51_), .c(new_n45_), .d(new_n59_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n96_), .c(new_n62_), .O(z12));
  nand2  g56(.a(new_n51_), .b(new_n59_), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n49_), .b(x26), .c(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n48_), .O(new_n104_));
  nand2  g60(.a(new_n47_), .b(new_n46_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(new_n106_));
  nor2   g62(.a(z05), .b(new_n54_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n47_), .b(new_n46_), .c(new_n111_), .O(new_n112_));
  and2   g68(.a(x20), .b(x19), .O(new_n113_));
  and2   g69(.a(x22), .b(x21), .O(new_n114_));
  and2   g70(.a(x24), .b(x23), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n112_), .c(new_n110_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n77_), .c(new_n65_), .O(z14));
  inv1   g74(.a(z07), .O(new_n119_));
  nand4  g75(.a(new_n49_), .b(new_n114_), .c(new_n113_), .d(x23), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  nand2  g77(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n102_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n119_), .O(z15));
  inv1   g80(.a(new_n77_), .O(new_n125_));
  inv1   g81(.a(x26), .O(new_n126_));
  nor2   g82(.a(new_n126_), .b(x18), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n49_), .c(new_n48_), .O(new_n128_));
  nand2  g84(.a(new_n120_), .b(new_n126_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n128_), .c(new_n125_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n69_), .O(z16));
endmodule


