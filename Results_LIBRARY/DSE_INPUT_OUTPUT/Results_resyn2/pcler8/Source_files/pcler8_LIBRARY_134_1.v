// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  nand2  g01(.a(x23), .b(x22), .O(new_n46_));
  nand2  g02(.a(x21), .b(x20), .O(new_n47_));
  nor3   g03(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  inv1   g08(.a(x04), .O(new_n53_));
  nand2  g09(.a(x17), .b(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n52_), .c(new_n48_), .d(x19), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n54_), .O(z01));
  nand3  g14(.a(new_n54_), .b(x08), .c(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n54_), .O(z03));
  nand3  g18(.a(new_n54_), .b(x08), .c(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nor2   g20(.a(new_n49_), .b(new_n53_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n54_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n54_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n54_), .O(z08));
  inv1   g27(.a(new_n54_), .O(new_n72_));
  inv1   g28(.a(x19), .O(new_n73_));
  nand3  g29(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  nor3   g30(.a(new_n74_), .b(new_n46_), .c(new_n45_), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n73_), .c(new_n52_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n57_), .c(new_n72_), .O(z09));
  nor2   g33(.a(new_n46_), .b(new_n45_), .O(new_n78_));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  and2   g35(.a(x21), .b(x12), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n78_), .c(new_n79_), .O(new_n81_));
  inv1   g37(.a(x20), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n73_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n54_), .c(new_n52_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n81_), .c(new_n59_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(new_n78_), .b(x13), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  aoi21  g44(.a(new_n79_), .b(new_n88_), .c(new_n51_), .O(new_n89_));
  oai21  g45(.a(new_n87_), .b(new_n86_), .c(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n61_), .c(new_n72_), .O(z11));
  inv1   g47(.a(new_n45_), .O(new_n92_));
  inv1   g48(.a(new_n86_), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(x22), .O(new_n94_));
  and2   g50(.a(x23), .b(x14), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n92_), .c(new_n94_), .O(new_n96_));
  nor2   g52(.a(new_n72_), .b(new_n51_), .O(new_n97_));
  oai21  g53(.a(new_n93_), .b(x22), .c(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n63_), .O(z12));
  nand2  g55(.a(new_n92_), .b(x15), .O(new_n100_));
  nor2   g56(.a(new_n86_), .b(new_n46_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  nand2  g59(.a(new_n94_), .b(new_n103_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n102_), .c(new_n52_), .O(new_n105_));
  nor2   g61(.a(z05), .b(new_n72_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(z13));
  nand2  g63(.a(new_n92_), .b(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  xor2a  g65(.a(new_n101_), .b(x24), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n109_), .c(new_n52_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n66_), .c(new_n72_), .O(z14));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n101_), .c(x25), .d(x24), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand2  g71(.a(new_n101_), .b(x24), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(new_n114_), .c(new_n50_), .d(x09), .O(new_n118_));
  oai21  g74(.a(new_n49_), .b(x06), .c(new_n54_), .O(new_n119_));
  aoi21  g75(.a(new_n118_), .b(new_n49_), .c(new_n119_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  oai21  g77(.a(new_n116_), .b(new_n115_), .c(new_n121_), .O(new_n122_));
  nor2   g78(.a(new_n73_), .b(x18), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n48_), .c(new_n51_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n70_), .c(new_n54_), .O(z16));
endmodule


