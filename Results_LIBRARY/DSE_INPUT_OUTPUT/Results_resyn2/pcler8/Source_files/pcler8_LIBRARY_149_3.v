// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_, new_n64_,
    new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  and2   g00(.a(x25), .b(x24), .O(new_n45_));
  nand3  g01(.a(new_n45_), .b(x26), .c(x23), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  inv1   g03(.a(x09), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor3   g07(.a(new_n51_), .b(new_n50_), .c(new_n46_), .O(z00));
  oai21  g08(.a(x10), .b(x00), .c(x08), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z01));
  inv1   g10(.a(x01), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  aoi21  g12(.a(new_n47_), .b(new_n55_), .c(new_n56_), .O(z02));
  inv1   g13(.a(x02), .O(new_n58_));
  aoi21  g14(.a(new_n47_), .b(new_n58_), .c(new_n56_), .O(z03));
  inv1   g15(.a(x03), .O(new_n60_));
  aoi21  g16(.a(new_n47_), .b(new_n60_), .c(new_n56_), .O(z04));
  nand2  g17(.a(x08), .b(x04), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(x10), .O(z05));
  nand2  g19(.a(x08), .b(x05), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(x10), .O(z06));
  inv1   g21(.a(x06), .O(new_n66_));
  aoi21  g22(.a(new_n47_), .b(new_n66_), .c(new_n56_), .O(z07));
  oai21  g23(.a(x10), .b(x07), .c(x08), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z08));
  inv1   g25(.a(new_n50_), .O(new_n70_));
  inv1   g26(.a(x21), .O(new_n71_));
  inv1   g27(.a(x22), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n46_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n70_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n53_), .O(z09));
  nand2  g33(.a(x08), .b(x01), .O(new_n78_));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  nand2  g35(.a(new_n73_), .b(x12), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n46_), .O(new_n81_));
  nand2  g37(.a(x09), .b(new_n56_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  oai21  g40(.a(new_n81_), .b(new_n79_), .c(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n78_), .c(x10), .O(z10));
  nand2  g42(.a(x08), .b(x02), .O(new_n87_));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  nand2  g44(.a(x22), .b(x13), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n46_), .O(new_n90_));
  aoi21  g46(.a(new_n79_), .b(new_n71_), .c(new_n82_), .O(new_n91_));
  oai21  g47(.a(new_n90_), .b(new_n88_), .c(new_n91_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n87_), .c(x10), .O(z11));
  nand2  g49(.a(x08), .b(x03), .O(new_n94_));
  inv1   g50(.a(x14), .O(new_n95_));
  inv1   g51(.a(new_n51_), .O(new_n96_));
  oai21  g52(.a(new_n46_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  aoi21  g53(.a(new_n88_), .b(new_n72_), .c(new_n82_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n94_), .c(x10), .O(z12));
  inv1   g56(.a(x15), .O(new_n101_));
  nor2   g57(.a(new_n46_), .b(new_n101_), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  xor2a  g59(.a(new_n51_), .b(new_n103_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n102_), .c(new_n49_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n62_), .c(x10), .O(z13));
  nand3  g62(.a(new_n45_), .b(x26), .c(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nor2   g64(.a(new_n51_), .b(new_n103_), .O(new_n109_));
  xor2a  g65(.a(new_n109_), .b(x24), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n108_), .c(new_n49_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n64_), .c(x10), .O(z14));
  nand2  g68(.a(x08), .b(x06), .O(new_n113_));
  nand2  g69(.a(x26), .b(x17), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n109_), .c(new_n45_), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  nand2  g72(.a(new_n109_), .b(x24), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n115_), .c(new_n49_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n113_), .c(x10), .O(z15));
  aoi21  g76(.a(new_n109_), .b(new_n45_), .c(x26), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand2  g78(.a(new_n96_), .b(new_n122_), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n46_), .c(new_n70_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n121_), .c(new_n68_), .O(z16));
endmodule


