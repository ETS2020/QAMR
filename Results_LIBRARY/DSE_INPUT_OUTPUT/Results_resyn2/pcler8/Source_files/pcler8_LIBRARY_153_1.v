// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x11), .O(new_n45_));
  nand2  g01(.a(x16), .b(new_n45_), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  nand3  g03(.a(x23), .b(x22), .c(x21), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g05(.a(x20), .b(x19), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  inv1   g15(.a(x16), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(x11), .O(new_n61_));
  nand2  g17(.a(x08), .b(x01), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n61_), .O(z02));
  nand2  g19(.a(x08), .b(x02), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n61_), .O(z03));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n61_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n61_), .O(z05));
  nand3  g25(.a(new_n46_), .b(x08), .c(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z07));
  nand3  g29(.a(new_n46_), .b(x08), .c(x07), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(z08));
  oai21  g31(.a(new_n54_), .b(x19), .c(new_n58_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n46_), .O(new_n77_));
  nand4  g33(.a(new_n55_), .b(new_n49_), .c(x20), .d(x11), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n77_), .O(z09));
  aoi21  g35(.a(new_n49_), .b(x12), .c(new_n50_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n55_), .O(new_n81_));
  and2   g37(.a(new_n62_), .b(new_n46_), .O(new_n82_));
  oai21  g38(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z10));
  inv1   g39(.a(new_n47_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  nand3  g41(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n84_), .c(new_n85_), .O(new_n88_));
  oai21  g44(.a(new_n51_), .b(x21), .c(new_n55_), .O(new_n89_));
  and2   g45(.a(new_n64_), .b(new_n46_), .O(new_n90_));
  oai21  g46(.a(new_n89_), .b(new_n88_), .c(new_n90_), .O(z11));
  nand4  g47(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n92_));
  inv1   g48(.a(x23), .O(new_n93_));
  inv1   g49(.a(x24), .O(new_n94_));
  nor2   g50(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g51(.a(x26), .b(x25), .c(x14), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n95_), .c(new_n92_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  nand2  g55(.a(new_n85_), .b(new_n99_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n55_), .c(new_n46_), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(new_n98_), .c(new_n66_), .d(new_n61_), .O(z12));
  nand2  g58(.a(new_n84_), .b(x15), .O(new_n103_));
  nor2   g59(.a(new_n92_), .b(new_n93_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g61(.a(new_n92_), .b(new_n93_), .c(new_n54_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n68_), .c(new_n46_), .O(z13));
  nor2   g64(.a(new_n54_), .b(new_n61_), .O(new_n109_));
  nand2  g65(.a(x23), .b(x22), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  inv1   g67(.a(new_n85_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n111_), .c(x24), .O(new_n113_));
  oai21  g69(.a(new_n85_), .b(new_n110_), .c(new_n94_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n113_), .c(new_n109_), .O(new_n115_));
  nand4  g71(.a(new_n55_), .b(new_n84_), .c(x16), .d(x11), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n115_), .c(new_n70_), .O(z14));
  inv1   g73(.a(z07), .O(new_n118_));
  nor2   g74(.a(new_n85_), .b(new_n110_), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  aoi21  g76(.a(x26), .b(x17), .c(new_n120_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n119_), .c(x24), .O(new_n122_));
  nand2  g78(.a(new_n113_), .b(new_n120_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(new_n55_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n118_), .O(z15));
  inv1   g81(.a(x26), .O(new_n126_));
  nand3  g82(.a(x25), .b(x24), .c(x23), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n92_), .c(new_n126_), .O(new_n128_));
  nor2   g84(.a(new_n47_), .b(x18), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n119_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n128_), .c(new_n109_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n74_), .O(z16));
endmodule


