// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:23 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand4  g02(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(new_n51_));
  aoi21  g07(.a(new_n51_), .b(x19), .c(x10), .O(z00));
  inv1   g08(.a(x10), .O(new_n53_));
  inv1   g09(.a(x19), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  nand2  g14(.a(new_n55_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n55_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n59_), .b(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n55_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  nor2   g23(.a(new_n59_), .b(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  inv1   g27(.a(x08), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n71_), .c(new_n55_), .O(z08));
  nand4  g29(.a(new_n50_), .b(new_n48_), .c(new_n46_), .d(x11), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n53_), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n56_), .O(z09));
  inv1   g33(.a(x20), .O(new_n78_));
  nand2  g34(.a(x23), .b(x22), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n45_), .O(new_n80_));
  and2   g36(.a(x21), .b(x12), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  nand4  g38(.a(x19), .b(new_n53_), .c(x09), .d(new_n72_), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n82_), .c(new_n59_), .d(new_n58_), .O(z10));
  nand2  g40(.a(x21), .b(x20), .O(new_n85_));
  aoi21  g41(.a(new_n80_), .b(x13), .c(new_n85_), .O(new_n86_));
  inv1   g42(.a(new_n83_), .O(new_n87_));
  oai21  g43(.a(x21), .b(x20), .c(new_n87_), .O(new_n88_));
  inv1   g44(.a(new_n59_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(x02), .O(new_n90_));
  oai21  g46(.a(new_n88_), .b(new_n86_), .c(new_n90_), .O(z11));
  nand3  g47(.a(x22), .b(x21), .c(x20), .O(new_n92_));
  and2   g48(.a(x23), .b(x14), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n46_), .c(new_n92_), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  nand2  g51(.a(new_n85_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n87_), .O(new_n97_));
  oai22  g53(.a(new_n97_), .b(new_n94_), .c(new_n59_), .d(new_n63_), .O(z12));
  nand4  g54(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n48_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nand2  g57(.a(x09), .b(new_n72_), .O(new_n102_));
  aoi21  g58(.a(new_n92_), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n100_), .c(new_n54_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(x10), .c(new_n65_), .O(z13));
  nand2  g61(.a(x08), .b(x05), .O(new_n106_));
  nand3  g62(.a(x26), .b(x25), .c(x16), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n48_), .c(x24), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  aoi21  g65(.a(new_n47_), .b(new_n109_), .c(new_n102_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n54_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(x10), .c(new_n106_), .O(z14));
  nand3  g68(.a(x26), .b(x25), .c(x17), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n48_), .b(x25), .c(x24), .O(new_n115_));
  inv1   g71(.a(x25), .O(new_n116_));
  oai21  g72(.a(new_n47_), .b(new_n109_), .c(new_n116_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n115_), .c(new_n114_), .O(new_n118_));
  nand2  g74(.a(new_n89_), .b(x06), .O(new_n119_));
  oai21  g75(.a(new_n118_), .b(new_n83_), .c(new_n119_), .O(z15));
  nor2   g76(.a(new_n47_), .b(new_n109_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(x25), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand3  g79(.a(new_n48_), .b(new_n46_), .c(new_n123_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n87_), .O(new_n125_));
  oai22  g81(.a(new_n125_), .b(new_n122_), .c(new_n59_), .d(new_n71_), .O(z16));
endmodule


