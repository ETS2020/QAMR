// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nor2   g00(.a(x26), .b(x17), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  nand3  g03(.a(x23), .b(x22), .c(x21), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g05(.a(x20), .b(x19), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n49_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n45_), .O(z01));
  nand3  g14(.a(new_n46_), .b(x08), .c(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand3  g16(.a(new_n46_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  aoi21  g18(.a(x08), .b(x03), .c(new_n45_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n45_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  oai21  g23(.a(new_n51_), .b(new_n67_), .c(new_n46_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n46_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  and2   g29(.a(x20), .b(x11), .O(new_n74_));
  aoi21  g30(.a(new_n74_), .b(new_n49_), .c(new_n73_), .O(new_n75_));
  aoi21  g31(.a(x08), .b(x00), .c(new_n45_), .O(new_n76_));
  oai21  g32(.a(new_n75_), .b(new_n53_), .c(new_n76_), .O(z09));
  inv1   g33(.a(new_n53_), .O(new_n78_));
  nand3  g34(.a(new_n49_), .b(x19), .c(x12), .O(new_n79_));
  inv1   g35(.a(x20), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n73_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n50_), .c(new_n46_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n59_), .O(z10));
  inv1   g41(.a(new_n47_), .O(new_n86_));
  nand2  g42(.a(x23), .b(x22), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  nand4  g44(.a(new_n54_), .b(new_n88_), .c(new_n86_), .d(x13), .O(new_n89_));
  inv1   g45(.a(x21), .O(new_n90_));
  nand2  g46(.a(new_n50_), .b(new_n90_), .O(new_n91_));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  nand4  g48(.a(new_n92_), .b(new_n91_), .c(new_n78_), .d(new_n46_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n89_), .c(new_n61_), .O(z11));
  nand4  g50(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n95_));
  and2   g51(.a(x23), .b(x14), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n86_), .c(new_n95_), .O(new_n97_));
  inv1   g53(.a(new_n92_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n78_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n63_), .O(z12));
  nand2  g56(.a(new_n86_), .b(x15), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nor2   g58(.a(new_n95_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g60(.a(new_n95_), .b(new_n102_), .c(new_n53_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n65_), .c(new_n46_), .O(z13));
  nor2   g63(.a(new_n51_), .b(new_n67_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n92_), .b(new_n87_), .c(new_n109_), .O(new_n110_));
  nand3  g66(.a(x24), .b(x23), .c(x22), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n98_), .c(new_n53_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n110_), .c(new_n108_), .O(new_n114_));
  nand3  g70(.a(new_n78_), .b(new_n86_), .c(x16), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n45_), .c(new_n115_), .O(z14));
  nand2  g72(.a(x26), .b(x17), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(new_n112_), .c(new_n98_), .d(x25), .O(new_n118_));
  inv1   g74(.a(x25), .O(new_n119_));
  oai21  g75(.a(new_n111_), .b(new_n92_), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n118_), .c(new_n78_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n69_), .c(new_n46_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nor2   g79(.a(new_n92_), .b(new_n87_), .O(new_n124_));
  nand2  g80(.a(x25), .b(x24), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(x18), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(new_n127_));
  and2   g83(.a(x25), .b(x17), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n112_), .c(new_n98_), .d(new_n123_), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n127_), .c(new_n78_), .O(new_n131_));
  nand3  g87(.a(new_n46_), .b(x08), .c(x07), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n131_), .O(z16));
endmodule


