// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  nand4  g00(.a(x23), .b(x22), .c(x21), .d(x20), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand3  g06(.a(x26), .b(x25), .c(x24), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n50_), .c(new_n46_), .O(new_n53_));
  inv1   g09(.a(x04), .O(new_n54_));
  nand2  g10(.a(x11), .b(new_n54_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x19), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n53_), .O(z00));
  nand3  g13(.a(new_n55_), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n55_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n55_), .O(z03));
  inv1   g19(.a(new_n55_), .O(new_n64_));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n64_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n55_), .O(z05));
  nand2  g24(.a(x08), .b(x05), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n64_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n64_), .O(z08));
  nor2   g30(.a(new_n51_), .b(new_n49_), .O(new_n75_));
  nand4  g31(.a(new_n75_), .b(new_n46_), .c(x11), .d(x04), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  nand3  g33(.a(new_n55_), .b(new_n50_), .c(new_n77_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n76_), .c(new_n58_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  nand4  g36(.a(x23), .b(x22), .c(x21), .d(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n51_), .c(new_n80_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n49_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n60_), .c(new_n64_), .O(z10));
  inv1   g42(.a(z03), .O(new_n87_));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  nand2  g44(.a(x23), .b(x22), .O(new_n89_));
  nor2   g45(.a(new_n51_), .b(new_n89_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(x13), .c(new_n88_), .O(new_n91_));
  oai21  g47(.a(new_n80_), .b(x21), .c(new_n50_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(z11));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n94_));
  and2   g50(.a(x23), .b(x14), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n52_), .c(new_n94_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand2  g53(.a(new_n88_), .b(new_n97_), .O(new_n98_));
  nand3  g54(.a(new_n98_), .b(new_n55_), .c(new_n50_), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(new_n96_), .c(new_n65_), .d(new_n64_), .O(z12));
  nand2  g56(.a(new_n52_), .b(x15), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nor2   g58(.a(new_n94_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  aoi21  g60(.a(new_n94_), .b(new_n102_), .c(new_n49_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n67_), .c(new_n55_), .O(z13));
  inv1   g63(.a(z06), .O(new_n108_));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(new_n88_), .O(new_n111_));
  nand3  g67(.a(x24), .b(x23), .c(x22), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n88_), .b(new_n89_), .c(new_n115_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n114_), .c(new_n110_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n49_), .c(new_n108_), .O(z14));
  nand3  g74(.a(x26), .b(x25), .c(x17), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n113_), .b(new_n111_), .c(x25), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  oai21  g78(.a(new_n112_), .b(new_n88_), .c(new_n122_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n120_), .O(new_n124_));
  and2   g80(.a(new_n71_), .b(new_n55_), .O(new_n125_));
  oai21  g81(.a(new_n124_), .b(new_n49_), .c(new_n125_), .O(z15));
  nor2   g82(.a(new_n112_), .b(new_n88_), .O(new_n127_));
  aoi21  g83(.a(new_n127_), .b(x25), .c(x26), .O(new_n128_));
  nor2   g84(.a(new_n88_), .b(x18), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n90_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n50_), .O(new_n131_));
  and2   g87(.a(new_n73_), .b(new_n55_), .O(new_n132_));
  oai21  g88(.a(new_n131_), .b(new_n128_), .c(new_n132_), .O(z16));
endmodule


