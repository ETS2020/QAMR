// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g03(.a(new_n47_), .b(x24), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  inv1   g05(.a(x17), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  and2   g10(.a(x26), .b(x25), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n48_), .O(z00));
  nand2  g13(.a(x17), .b(new_n49_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x00), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z02));
  inv1   g18(.a(x02), .O(new_n63_));
  inv1   g19(.a(x08), .O(new_n64_));
  aoi21  g20(.a(x17), .b(new_n49_), .c(new_n64_), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g23(.a(x03), .O(new_n68_));
  nor2   g24(.a(new_n66_), .b(new_n68_), .O(z04));
  inv1   g25(.a(x04), .O(new_n70_));
  oai21  g26(.a(new_n64_), .b(new_n70_), .c(new_n58_), .O(z05));
  nand2  g27(.a(x08), .b(x05), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n58_), .O(z06));
  and2   g29(.a(new_n65_), .b(x06), .O(z07));
  nand2  g30(.a(x08), .b(x07), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n58_), .O(z08));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  nand3  g34(.a(new_n55_), .b(x24), .c(x23), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n77_), .O(new_n82_));
  nand2  g38(.a(new_n65_), .b(x00), .O(new_n83_));
  oai21  g39(.a(new_n82_), .b(new_n53_), .c(new_n83_), .O(z09));
  nand2  g40(.a(x20), .b(x19), .O(new_n85_));
  aoi21  g41(.a(new_n80_), .b(x12), .c(new_n85_), .O(new_n86_));
  oai21  g42(.a(x20), .b(x19), .c(new_n54_), .O(new_n87_));
  nand2  g43(.a(new_n65_), .b(x01), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(z10));
  inv1   g45(.a(new_n79_), .O(new_n90_));
  inv1   g46(.a(new_n85_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x21), .O(new_n92_));
  and2   g48(.a(x22), .b(x13), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n90_), .c(new_n92_), .O(new_n94_));
  oai21  g50(.a(new_n91_), .b(x21), .c(new_n54_), .O(new_n95_));
  oai22  g51(.a(new_n95_), .b(new_n94_), .c(new_n66_), .d(new_n63_), .O(z11));
  aoi21  g52(.a(new_n90_), .b(x14), .c(new_n46_), .O(new_n97_));
  inv1   g53(.a(new_n92_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n54_), .O(new_n99_));
  oai22  g55(.a(new_n99_), .b(new_n97_), .c(new_n66_), .d(new_n68_), .O(z12));
  inv1   g56(.a(x15), .O(new_n101_));
  nor2   g57(.a(new_n79_), .b(new_n101_), .O(new_n102_));
  inv1   g58(.a(new_n46_), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(x23), .O(new_n104_));
  nor2   g60(.a(new_n104_), .b(new_n47_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n102_), .c(new_n54_), .O(new_n106_));
  oai21  g62(.a(new_n66_), .b(new_n70_), .c(new_n106_), .O(z13));
  nand2  g63(.a(new_n55_), .b(x16), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n47_), .c(x24), .O(new_n109_));
  or2    g65(.a(new_n47_), .b(x24), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n54_), .O(new_n111_));
  nand2  g67(.a(new_n65_), .b(x05), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n111_), .O(z14));
  nand2  g69(.a(x08), .b(x06), .O(new_n114_));
  aoi21  g70(.a(new_n47_), .b(x24), .c(x25), .O(new_n115_));
  nand4  g71(.a(new_n103_), .b(x25), .c(x24), .d(x23), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n52_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n115_), .c(new_n50_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n49_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n114_), .O(z15));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand4  g79(.a(new_n55_), .b(new_n47_), .c(x24), .d(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n54_), .O(new_n125_));
  nand2  g81(.a(new_n65_), .b(x07), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n125_), .O(z16));
endmodule


