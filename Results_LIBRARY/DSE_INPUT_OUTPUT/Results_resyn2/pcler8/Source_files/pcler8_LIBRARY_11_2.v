// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_;
  inv1   g00(.a(x02), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  inv1   g09(.a(x23), .O(new_n54_));
  nand4  g10(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n53_), .c(new_n51_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n47_), .O(z00));
  nand3  g14(.a(new_n47_), .b(x08), .c(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand3  g16(.a(new_n47_), .b(x08), .c(x01), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  aoi21  g18(.a(x17), .b(new_n48_), .c(new_n45_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n47_), .O(z04));
  nand3  g21(.a(new_n47_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n46_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n46_), .O(z08));
  nor2   g29(.a(new_n50_), .b(new_n46_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  inv1   g31(.a(x19), .O(new_n76_));
  nand2  g32(.a(x22), .b(x21), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g35(.a(x20), .b(x11), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n79_), .c(new_n76_), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n75_), .c(new_n59_), .O(z09));
  nand2  g38(.a(x20), .b(x19), .O(new_n83_));
  aoi21  g39(.a(new_n79_), .b(x12), .c(new_n83_), .O(new_n84_));
  inv1   g40(.a(x20), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n76_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n74_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n84_), .c(new_n61_), .O(z10));
  inv1   g44(.a(new_n78_), .O(new_n89_));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  nand2  g49(.a(new_n83_), .b(new_n93_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n74_), .O(new_n95_));
  nand3  g51(.a(x17), .b(x08), .c(x02), .O(new_n96_));
  oai21  g52(.a(new_n95_), .b(new_n92_), .c(new_n96_), .O(z11));
  inv1   g53(.a(x14), .O(new_n98_));
  inv1   g54(.a(new_n55_), .O(new_n99_));
  oai21  g55(.a(new_n78_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  inv1   g56(.a(x22), .O(new_n101_));
  aoi21  g57(.a(new_n90_), .b(new_n101_), .c(new_n50_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n64_), .c(new_n46_), .O(z12));
  nand2  g60(.a(new_n99_), .b(x23), .O(new_n105_));
  inv1   g61(.a(x15), .O(new_n106_));
  nor2   g62(.a(new_n78_), .b(new_n106_), .O(new_n107_));
  nand2  g63(.a(new_n55_), .b(new_n54_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n105_), .c(new_n107_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(new_n75_), .c(new_n66_), .O(z13));
  inv1   g66(.a(x25), .O(new_n111_));
  inv1   g67(.a(x26), .O(new_n112_));
  nor2   g68(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n56_), .c(x24), .O(new_n115_));
  inv1   g71(.a(x24), .O(new_n116_));
  nand2  g72(.a(new_n105_), .b(new_n116_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n115_), .c(new_n51_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n68_), .c(new_n47_), .O(z14));
  nand4  g75(.a(new_n99_), .b(x25), .c(x24), .d(x23), .O(new_n120_));
  nand3  g76(.a(new_n99_), .b(x24), .c(x23), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n111_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n74_), .O(new_n123_));
  nand2  g79(.a(new_n113_), .b(x17), .O(new_n124_));
  inv1   g80(.a(new_n124_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n51_), .c(z07), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n123_), .O(z15));
  nand2  g83(.a(new_n120_), .b(new_n112_), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n56_), .b(new_n53_), .c(new_n129_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n128_), .c(new_n51_), .O(new_n131_));
  aoi21  g87(.a(x08), .b(x07), .c(new_n46_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n131_), .O(z16));
endmodule


