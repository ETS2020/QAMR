// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x15), .O(new_n45_));
  nor2   g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand4  g03(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  nand4  g09(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z00));
  nand3  g13(.a(new_n47_), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n47_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n47_), .O(z03));
  nand3  g19(.a(new_n47_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n47_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(new_n46_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n47_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  nor2   g29(.a(new_n52_), .b(new_n46_), .O(new_n74_));
  nand4  g30(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  oai21  g31(.a(new_n75_), .b(new_n48_), .c(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n58_), .O(z09));
  nand2  g34(.a(x20), .b(x19), .O(new_n79_));
  nand3  g35(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n48_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n52_), .O(new_n83_));
  oai21  g39(.a(new_n81_), .b(new_n79_), .c(new_n83_), .O(new_n84_));
  aoi21  g40(.a(new_n84_), .b(new_n60_), .c(new_n46_), .O(z10));
  nand3  g41(.a(x26), .b(x25), .c(x24), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n86_), .c(new_n88_), .O(new_n90_));
  inv1   g46(.a(x21), .O(new_n91_));
  aoi21  g47(.a(new_n79_), .b(new_n91_), .c(new_n52_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n62_), .c(new_n46_), .O(z11));
  aoi21  g50(.a(new_n49_), .b(x14), .c(new_n54_), .O(new_n95_));
  oai21  g51(.a(new_n88_), .b(x22), .c(new_n74_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n64_), .O(z12));
  nor2   g53(.a(new_n48_), .b(new_n45_), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n54_), .b(new_n99_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n53_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n66_), .c(new_n46_), .O(z13));
  inv1   g58(.a(x24), .O(new_n103_));
  oai21  g59(.a(new_n54_), .b(new_n99_), .c(new_n103_), .O(new_n104_));
  nor2   g60(.a(new_n54_), .b(new_n99_), .O(new_n105_));
  inv1   g61(.a(x25), .O(new_n106_));
  inv1   g62(.a(x26), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n105_), .c(x24), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n104_), .c(new_n53_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n68_), .c(new_n47_), .O(z14));
  nand3  g68(.a(x24), .b(x23), .c(x22), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n87_), .c(new_n106_), .O(new_n114_));
  inv1   g70(.a(new_n113_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n88_), .c(x25), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n114_), .c(new_n53_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n70_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n47_), .O(new_n119_));
  nand3  g75(.a(new_n53_), .b(new_n108_), .c(x17), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n119_), .O(z15));
  nand2  g77(.a(new_n116_), .b(new_n107_), .O(new_n122_));
  nor2   g78(.a(new_n86_), .b(x18), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n105_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n53_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


