// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand3  g02(.a(new_n46_), .b(x09), .c(new_n45_), .O(new_n47_));
  nor2   g03(.a(x17), .b(x11), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nor3   g10(.a(new_n54_), .b(new_n48_), .c(new_n47_), .O(z00));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n48_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n48_), .O(z02));
  inv1   g15(.a(new_n48_), .O(new_n60_));
  nand3  g16(.a(new_n60_), .b(x08), .c(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n60_), .O(z04));
  nand3  g20(.a(new_n60_), .b(x08), .c(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n60_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n48_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n60_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  nand2  g29(.a(x22), .b(x21), .O(new_n74_));
  nand2  g30(.a(new_n50_), .b(x23), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  and2   g32(.a(x20), .b(x11), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n76_), .c(new_n73_), .O(new_n78_));
  aoi21  g34(.a(x08), .b(x00), .c(new_n48_), .O(new_n79_));
  oai21  g35(.a(new_n78_), .b(new_n47_), .c(new_n79_), .O(z09));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  aoi21  g37(.a(new_n76_), .b(x12), .c(new_n81_), .O(new_n82_));
  inv1   g38(.a(new_n47_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n83_), .O(new_n84_));
  aoi21  g40(.a(x08), .b(x01), .c(new_n48_), .O(new_n85_));
  oai21  g41(.a(new_n84_), .b(new_n82_), .c(new_n85_), .O(z10));
  inv1   g42(.a(new_n75_), .O(new_n87_));
  inv1   g43(.a(new_n81_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(x21), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n87_), .c(new_n89_), .O(new_n91_));
  nor2   g47(.a(new_n48_), .b(new_n47_), .O(new_n92_));
  oai21  g48(.a(new_n88_), .b(x21), .c(new_n92_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(new_n61_), .O(z11));
  inv1   g50(.a(x14), .O(new_n95_));
  inv1   g51(.a(new_n52_), .O(new_n96_));
  oai21  g52(.a(new_n75_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  aoi21  g54(.a(new_n89_), .b(new_n98_), .c(new_n47_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n97_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n63_), .c(new_n48_), .O(z12));
  inv1   g57(.a(x15), .O(new_n102_));
  nor2   g58(.a(new_n75_), .b(new_n102_), .O(new_n103_));
  nor2   g59(.a(new_n96_), .b(x23), .O(new_n104_));
  nor2   g60(.a(new_n104_), .b(new_n53_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n103_), .c(new_n92_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n65_), .O(z13));
  nand2  g63(.a(new_n50_), .b(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  xor2a  g65(.a(new_n53_), .b(x24), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n109_), .c(new_n83_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n67_), .c(new_n48_), .O(z14));
  nand3  g68(.a(new_n53_), .b(x25), .c(x24), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n53_), .b(x24), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n113_), .c(new_n92_), .O(new_n117_));
  nand3  g73(.a(x26), .b(x25), .c(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n83_), .c(z07), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n117_), .O(z15));
  inv1   g77(.a(x26), .O(new_n122_));
  nand2  g78(.a(new_n113_), .b(new_n122_), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand3  g80(.a(new_n53_), .b(new_n50_), .c(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n83_), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n71_), .c(new_n48_), .O(z16));
endmodule


