// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_;
  nand4  g00(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x17), .O(new_n51_));
  nand2  g07(.a(new_n51_), .b(x01), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n46_), .c(new_n45_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n52_), .O(z01));
  nand3  g12(.a(x17), .b(x08), .c(x01), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z02));
  nand3  g14(.a(new_n52_), .b(x08), .c(x02), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  and2   g17(.a(new_n61_), .b(new_n52_), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n52_), .O(z05));
  nand3  g21(.a(new_n52_), .b(x08), .c(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  inv1   g23(.a(new_n52_), .O(new_n68_));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n68_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(new_n68_), .O(z08));
  and2   g28(.a(x22), .b(x21), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n45_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n50_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(new_n55_), .c(new_n52_), .O(z09));
  oai21  g33(.a(new_n51_), .b(x08), .c(x01), .O(new_n78_));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  inv1   g35(.a(new_n45_), .O(new_n80_));
  nand3  g36(.a(new_n73_), .b(new_n80_), .c(x12), .O(new_n81_));
  and2   g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(x20), .b(x19), .c(new_n50_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(z10));
  nand2  g40(.a(new_n79_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n80_), .c(new_n85_), .O(new_n87_));
  nor2   g43(.a(new_n68_), .b(new_n49_), .O(new_n88_));
  oai21  g44(.a(new_n79_), .b(x21), .c(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n87_), .c(new_n59_), .O(z11));
  aoi21  g46(.a(new_n80_), .b(x14), .c(new_n46_), .O(new_n91_));
  inv1   g47(.a(new_n85_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n50_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(new_n62_), .O(z12));
  inv1   g50(.a(x15), .O(new_n95_));
  nor2   g51(.a(new_n45_), .b(new_n95_), .O(new_n96_));
  inv1   g52(.a(x23), .O(new_n97_));
  xor2a  g53(.a(new_n46_), .b(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n50_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n64_), .c(new_n52_), .O(z13));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n101_));
  inv1   g57(.a(new_n101_), .O(new_n102_));
  nand4  g58(.a(new_n73_), .b(new_n79_), .c(x24), .d(x23), .O(new_n103_));
  inv1   g59(.a(x24), .O(new_n104_));
  oai21  g60(.a(new_n46_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n103_), .c(new_n102_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n53_), .c(new_n66_), .O(z14));
  nand3  g63(.a(x26), .b(x25), .c(x17), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nor2   g65(.a(new_n46_), .b(new_n97_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(x25), .c(x24), .O(new_n111_));
  inv1   g67(.a(x25), .O(new_n112_));
  nand2  g68(.a(new_n103_), .b(new_n112_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  and2   g70(.a(new_n69_), .b(new_n52_), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n49_), .c(new_n115_), .O(z15));
  inv1   g72(.a(new_n111_), .O(new_n117_));
  nor2   g73(.a(new_n46_), .b(x18), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n80_), .c(new_n49_), .O(new_n119_));
  oai21  g75(.a(new_n117_), .b(x26), .c(new_n119_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n71_), .c(new_n52_), .O(z16));
endmodule


