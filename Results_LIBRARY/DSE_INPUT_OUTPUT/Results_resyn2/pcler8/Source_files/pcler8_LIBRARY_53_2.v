// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x17), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x15), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(x23), .O(new_n49_));
  inv1   g05(.a(x25), .O(new_n50_));
  inv1   g06(.a(x26), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n52_), .c(x24), .O(new_n57_));
  oai21  g13(.a(new_n57_), .b(new_n49_), .c(new_n46_), .O(z00));
  nand3  g14(.a(new_n46_), .b(x08), .c(x00), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z03));
  inv1   g20(.a(new_n46_), .O(new_n65_));
  nand2  g21(.a(x08), .b(x03), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z04));
  nand2  g23(.a(x08), .b(x04), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n46_), .O(z05));
  nand2  g25(.a(x08), .b(x05), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nor2   g28(.a(new_n72_), .b(new_n65_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n46_), .O(z08));
  nand2  g31(.a(new_n56_), .b(new_n46_), .O(new_n76_));
  inv1   g32(.a(x19), .O(new_n77_));
  and2   g33(.a(x22), .b(x21), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  inv1   g35(.a(new_n79_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  and2   g38(.a(x20), .b(x11), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n82_), .c(new_n77_), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n76_), .c(new_n59_), .O(z09));
  inv1   g41(.a(x12), .O(new_n86_));
  and2   g42(.a(x20), .b(x19), .O(new_n87_));
  oai21  g43(.a(new_n81_), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nor2   g44(.a(x20), .b(x19), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n55_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n61_), .c(new_n65_), .O(z10));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(x22), .b(x13), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n79_), .c(new_n94_), .O(new_n96_));
  or2    g52(.a(new_n87_), .b(x21), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n96_), .c(new_n56_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n63_), .c(new_n65_), .O(z11));
  nand2  g55(.a(new_n94_), .b(x22), .O(new_n100_));
  aoi21  g56(.a(new_n80_), .b(x14), .c(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n94_), .b(x22), .c(new_n56_), .O(new_n102_));
  and2   g58(.a(new_n66_), .b(new_n46_), .O(new_n103_));
  oai21  g59(.a(new_n102_), .b(new_n101_), .c(new_n103_), .O(z12));
  inv1   g60(.a(x23), .O(new_n105_));
  nand2  g61(.a(new_n47_), .b(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n56_), .c(new_n49_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n68_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n46_), .O(new_n109_));
  nand2  g65(.a(new_n56_), .b(x17), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n80_), .c(x15), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n109_), .O(z13));
  nand3  g69(.a(new_n52_), .b(x24), .c(x16), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n47_), .b(new_n105_), .c(new_n115_), .O(new_n116_));
  and2   g72(.a(x24), .b(x23), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n78_), .c(new_n87_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n114_), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n56_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n70_), .c(new_n65_), .O(z14));
  nand3  g78(.a(new_n117_), .b(new_n48_), .c(x25), .O(new_n123_));
  aoi21  g79(.a(new_n118_), .b(new_n50_), .c(new_n76_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi21  g81(.a(new_n111_), .b(new_n52_), .c(z07), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n125_), .O(z15));
  nor2   g83(.a(new_n51_), .b(x18), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n117_), .c(new_n48_), .d(x25), .O(new_n129_));
  nand2  g85(.a(new_n123_), .b(new_n51_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n129_), .c(new_n56_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n74_), .c(new_n65_), .O(z16));
endmodule


