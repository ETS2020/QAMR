// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nand3  g02(.a(x25), .b(x24), .c(x23), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  inv1   g04(.a(x09), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x08), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x14), .c(new_n48_), .O(new_n51_));
  nor4   g07(.a(new_n51_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(z00));
  inv1   g08(.a(x00), .O(new_n53_));
  inv1   g09(.a(x14), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x08), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n53_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n55_), .O(z02));
  inv1   g15(.a(new_n56_), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n56_), .b(new_n63_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  nor2   g21(.a(new_n56_), .b(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  nor2   g27(.a(new_n56_), .b(new_n71_), .O(z08));
  inv1   g28(.a(new_n51_), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  and2   g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n74_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  oai21  g34(.a(new_n56_), .b(new_n53_), .c(new_n78_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n74_), .O(new_n81_));
  nand2  g37(.a(new_n75_), .b(x12), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n73_), .O(new_n85_));
  nand2  g41(.a(new_n60_), .b(x01), .O(new_n86_));
  oai21  g42(.a(new_n85_), .b(new_n84_), .c(new_n86_), .O(z10));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n81_), .c(new_n88_), .O(new_n90_));
  inv1   g46(.a(x21), .O(new_n91_));
  nand2  g47(.a(new_n80_), .b(new_n91_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n73_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n90_), .c(new_n61_), .O(z11));
  inv1   g50(.a(x08), .O(new_n95_));
  inv1   g51(.a(new_n46_), .O(new_n96_));
  nand2  g52(.a(new_n74_), .b(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(x09), .b(new_n95_), .O(new_n99_));
  aoi21  g55(.a(new_n88_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  aoi21  g56(.a(new_n100_), .b(new_n97_), .c(new_n54_), .O(new_n101_));
  oai22  g57(.a(new_n101_), .b(x10), .c(new_n95_), .d(new_n63_), .O(z12));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n96_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nand2  g61(.a(new_n46_), .b(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n104_), .c(new_n73_), .O(new_n107_));
  oai21  g63(.a(new_n56_), .b(new_n65_), .c(new_n107_), .O(z13));
  nand2  g64(.a(x24), .b(x23), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  nand3  g66(.a(x26), .b(x25), .c(x16), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n110_), .c(new_n96_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n46_), .b(new_n105_), .c(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n114_), .b(new_n112_), .c(new_n50_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(x14), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n48_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n67_), .O(z14));
  inv1   g74(.a(new_n47_), .O(new_n119_));
  nand2  g75(.a(x26), .b(x17), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n96_), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  oai21  g78(.a(new_n109_), .b(new_n46_), .c(new_n122_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n121_), .c(new_n50_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(x14), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n48_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n69_), .O(z15));
  nand2  g83(.a(x08), .b(x07), .O(new_n128_));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(new_n81_), .b(new_n96_), .c(new_n129_), .O(new_n130_));
  oai21  g86(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n130_), .c(new_n50_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(x14), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n48_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n128_), .O(z16));
endmodule


