// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x05), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  and2   g05(.a(x26), .b(x25), .O(new_n50_));
  nor2   g06(.a(x10), .b(x08), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x24), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(new_n46_), .O(z00));
  inv1   g09(.a(x08), .O(new_n54_));
  aoi21  g10(.a(x09), .b(x05), .c(new_n54_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  nand2  g13(.a(x09), .b(x05), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(new_n55_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n58_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n58_), .O(z05));
  nor3   g22(.a(x09), .b(new_n54_), .c(new_n45_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n58_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n58_), .O(z08));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n45_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand4  g29(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n74_));
  and2   g30(.a(x22), .b(x21), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n74_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n56_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n74_), .O(new_n81_));
  nand2  g37(.a(new_n75_), .b(x12), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n81_), .c(new_n80_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n73_), .O(new_n85_));
  nand2  g41(.a(new_n55_), .b(x01), .O(new_n86_));
  oai21  g42(.a(new_n85_), .b(new_n84_), .c(new_n86_), .O(z10));
  nand2  g43(.a(x08), .b(x02), .O(new_n88_));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  inv1   g45(.a(new_n89_), .O(new_n90_));
  nand2  g46(.a(x22), .b(x13), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n74_), .c(new_n90_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  inv1   g49(.a(new_n51_), .O(new_n94_));
  aoi21  g50(.a(new_n80_), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n92_), .c(x05), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n46_), .c(new_n88_), .O(z11));
  inv1   g53(.a(x14), .O(new_n98_));
  inv1   g54(.a(new_n48_), .O(new_n99_));
  oai21  g55(.a(new_n74_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  inv1   g56(.a(x22), .O(new_n101_));
  aoi21  g57(.a(new_n89_), .b(new_n101_), .c(new_n94_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n100_), .c(x05), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n46_), .c(new_n63_), .O(z12));
  nand3  g60(.a(new_n50_), .b(x24), .c(x15), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n49_), .O(new_n106_));
  aoi21  g62(.a(new_n48_), .b(new_n47_), .c(new_n72_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g64(.a(new_n55_), .b(x04), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n108_), .O(z13));
  nand2  g66(.a(new_n51_), .b(x09), .O(new_n111_));
  oai21  g67(.a(x09), .b(x08), .c(x05), .O(new_n112_));
  nand3  g68(.a(new_n50_), .b(x24), .c(x16), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  oai21  g71(.a(new_n48_), .b(new_n47_), .c(new_n115_), .O(new_n116_));
  and2   g72(.a(x20), .b(x19), .O(new_n117_));
  nand4  g73(.a(new_n75_), .b(new_n117_), .c(x24), .d(x23), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n116_), .c(new_n114_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n111_), .c(new_n112_), .O(z14));
  nand2  g76(.a(new_n50_), .b(x17), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n49_), .b(x25), .c(x24), .O(new_n123_));
  inv1   g79(.a(x25), .O(new_n124_));
  nand2  g80(.a(new_n118_), .b(new_n124_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n123_), .c(new_n122_), .O(new_n126_));
  nand2  g82(.a(new_n55_), .b(x06), .O(new_n127_));
  oai21  g83(.a(new_n126_), .b(new_n72_), .c(new_n127_), .O(z15));
  inv1   g84(.a(new_n123_), .O(new_n129_));
  nor2   g85(.a(new_n48_), .b(x18), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n81_), .c(new_n72_), .O(new_n131_));
  oai21  g87(.a(new_n129_), .b(x26), .c(new_n131_), .O(new_n132_));
  nand2  g88(.a(new_n55_), .b(x07), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n132_), .O(z16));
endmodule


