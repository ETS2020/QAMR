// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_;
  nand2  g00(.a(x17), .b(x01), .O(new_n45_));
  nand2  g01(.a(x26), .b(x25), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nand2  g07(.a(x24), .b(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  nand3  g11(.a(new_n45_), .b(x08), .c(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(x17), .O(z02));
  nand3  g15(.a(new_n45_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n45_), .O(z04));
  nand3  g19(.a(new_n45_), .b(x08), .c(x04), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z05));
  inv1   g21(.a(new_n45_), .O(new_n66_));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n66_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n66_), .O(z07));
  nand3  g26(.a(new_n45_), .b(x08), .c(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  nand4  g28(.a(new_n45_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n73_));
  inv1   g29(.a(x19), .O(new_n74_));
  nand2  g30(.a(x22), .b(x21), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  nor2   g32(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  and2   g33(.a(x20), .b(x11), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n77_), .c(new_n74_), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n73_), .c(new_n56_), .O(z09));
  oai21  g36(.a(x17), .b(x08), .c(x01), .O(new_n81_));
  nand2  g37(.a(x20), .b(x19), .O(new_n82_));
  aoi21  g38(.a(new_n77_), .b(x12), .c(new_n82_), .O(new_n83_));
  inv1   g39(.a(new_n49_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n84_), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n83_), .c(new_n81_), .O(z10));
  inv1   g42(.a(new_n76_), .O(new_n87_));
  nand3  g43(.a(x21), .b(x20), .c(x19), .O(new_n88_));
  and2   g44(.a(x22), .b(x13), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n87_), .c(new_n88_), .O(new_n90_));
  inv1   g46(.a(new_n73_), .O(new_n91_));
  inv1   g47(.a(x21), .O(new_n92_));
  nand2  g48(.a(new_n82_), .b(new_n92_), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n90_), .c(new_n60_), .O(z11));
  inv1   g51(.a(x14), .O(new_n96_));
  inv1   g52(.a(new_n51_), .O(new_n97_));
  oai21  g53(.a(new_n76_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  inv1   g54(.a(x22), .O(new_n99_));
  aoi21  g55(.a(new_n88_), .b(new_n99_), .c(new_n49_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n62_), .c(new_n66_), .O(z12));
  inv1   g58(.a(x15), .O(new_n103_));
  nor2   g59(.a(new_n76_), .b(new_n103_), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  xor2a  g61(.a(new_n51_), .b(new_n105_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n104_), .c(new_n91_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n64_), .O(z13));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n51_), .b(new_n105_), .c(new_n109_), .O(new_n110_));
  inv1   g66(.a(x16), .O(new_n111_));
  oai21  g67(.a(new_n46_), .b(new_n111_), .c(new_n53_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n110_), .c(new_n84_), .O(new_n113_));
  and2   g69(.a(new_n67_), .b(new_n45_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n113_), .O(z14));
  inv1   g71(.a(x25), .O(new_n116_));
  oai21  g72(.a(new_n52_), .b(new_n51_), .c(new_n116_), .O(new_n117_));
  nand2  g73(.a(new_n53_), .b(x25), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n117_), .c(new_n84_), .O(new_n119_));
  oai21  g75(.a(new_n50_), .b(x01), .c(x17), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n69_), .O(z15));
  aoi21  g77(.a(new_n53_), .b(x25), .c(x26), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nand3  g79(.a(new_n87_), .b(new_n97_), .c(new_n123_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n91_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n122_), .c(new_n71_), .O(z16));
endmodule


