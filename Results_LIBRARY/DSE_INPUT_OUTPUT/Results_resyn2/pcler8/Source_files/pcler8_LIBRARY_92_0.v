// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  nor2   g00(.a(x17), .b(x01), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x25), .O(new_n50_));
  inv1   g06(.a(x26), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand4  g12(.a(new_n56_), .b(new_n52_), .c(new_n49_), .d(x24), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z00));
  aoi21  g14(.a(x08), .b(x00), .c(new_n45_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n53_), .b(new_n61_), .O(z02));
  nand3  g18(.a(new_n46_), .b(x08), .c(x02), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n45_), .O(z04));
  nand2  g22(.a(x08), .b(x04), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n45_), .O(z05));
  nand3  g24(.a(new_n46_), .b(x08), .c(x05), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z06));
  nand2  g26(.a(x08), .b(x06), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n45_), .O(z08));
  inv1   g30(.a(x19), .O(new_n75_));
  nand2  g31(.a(x22), .b(x21), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n55_), .c(new_n59_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  inv1   g38(.a(new_n82_), .O(new_n83_));
  aoi21  g39(.a(new_n78_), .b(x12), .c(new_n83_), .O(new_n84_));
  inv1   g40(.a(x17), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(x08), .c(new_n61_), .O(new_n86_));
  inv1   g42(.a(x20), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n75_), .O(new_n88_));
  inv1   g44(.a(x09), .O(new_n89_));
  nor2   g45(.a(x10), .b(new_n89_), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n84_), .c(new_n53_), .d(new_n61_), .O(z10));
  inv1   g48(.a(new_n77_), .O(new_n93_));
  nand2  g49(.a(new_n82_), .b(x21), .O(new_n94_));
  and2   g50(.a(x22), .b(x13), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n93_), .c(new_n94_), .O(new_n96_));
  nor2   g52(.a(new_n55_), .b(new_n45_), .O(new_n97_));
  oai21  g53(.a(new_n82_), .b(x21), .c(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(new_n63_), .O(z11));
  nand3  g55(.a(new_n82_), .b(x22), .c(x21), .O(new_n100_));
  aoi21  g56(.a(new_n93_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(new_n94_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(x22), .c(new_n56_), .O(new_n103_));
  aoi21  g59(.a(x08), .b(x03), .c(new_n45_), .O(new_n104_));
  oai21  g60(.a(new_n103_), .b(new_n101_), .c(new_n104_), .O(z12));
  nand3  g61(.a(new_n52_), .b(x24), .c(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n49_), .O(new_n107_));
  aoi21  g63(.a(new_n48_), .b(new_n47_), .c(new_n55_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n67_), .c(new_n46_), .O(z13));
  inv1   g66(.a(new_n97_), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n48_), .b(new_n47_), .c(new_n114_), .O(new_n115_));
  inv1   g71(.a(new_n48_), .O(new_n116_));
  and2   g72(.a(x24), .b(x23), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n115_), .c(new_n113_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n111_), .c(new_n69_), .O(z14));
  nand2  g76(.a(x24), .b(x23), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n48_), .c(new_n50_), .O(new_n122_));
  and2   g78(.a(x22), .b(x21), .O(new_n123_));
  nand4  g79(.a(new_n117_), .b(new_n123_), .c(new_n82_), .d(x25), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n56_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n71_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n46_), .O(new_n127_));
  nand4  g83(.a(new_n90_), .b(new_n52_), .c(x17), .d(new_n53_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n127_), .O(z15));
  nor2   g85(.a(new_n51_), .b(x18), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(new_n117_), .c(new_n116_), .d(x25), .O(new_n131_));
  nand2  g87(.a(new_n124_), .b(new_n51_), .O(new_n132_));
  nand3  g88(.a(new_n132_), .b(new_n131_), .c(new_n56_), .O(new_n133_));
  nand3  g89(.a(new_n133_), .b(new_n73_), .c(new_n46_), .O(z16));
endmodule


