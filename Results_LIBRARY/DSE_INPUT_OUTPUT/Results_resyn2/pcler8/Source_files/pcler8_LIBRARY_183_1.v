// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x01), .O(new_n45_));
  nand2  g01(.a(x17), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nand3  g09(.a(x22), .b(x21), .c(x20), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x19), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n52_), .O(z00));
  nand3  g13(.a(new_n46_), .b(x08), .c(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nor2   g15(.a(new_n48_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n47_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z04));
  nand3  g20(.a(new_n46_), .b(x08), .c(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand3  g22(.a(new_n46_), .b(x08), .c(x05), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n47_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  aoi21  g29(.a(new_n55_), .b(x11), .c(new_n73_), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n52_), .c(new_n58_), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n53_), .c(new_n76_), .O(new_n78_));
  nor2   g34(.a(x20), .b(x19), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n50_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  nor2   g37(.a(z02), .b(new_n47_), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(new_n81_), .O(z10));
  inv1   g39(.a(new_n53_), .O(new_n84_));
  nand2  g40(.a(new_n76_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  inv1   g43(.a(new_n50_), .O(new_n88_));
  oai21  g44(.a(new_n76_), .b(x21), .c(new_n88_), .O(new_n89_));
  and2   g45(.a(new_n61_), .b(new_n46_), .O(new_n90_));
  oai21  g46(.a(new_n89_), .b(new_n87_), .c(new_n90_), .O(z11));
  inv1   g47(.a(z04), .O(new_n92_));
  nand4  g48(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n93_));
  aoi21  g49(.a(new_n84_), .b(x14), .c(new_n93_), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  nand2  g51(.a(new_n85_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n94_), .c(new_n92_), .O(z12));
  inv1   g54(.a(new_n93_), .O(new_n99_));
  nand4  g55(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n99_), .c(x23), .O(new_n101_));
  inv1   g57(.a(x23), .O(new_n102_));
  nand2  g58(.a(new_n93_), .b(new_n102_), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n101_), .c(new_n51_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n65_), .O(z13));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n93_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nand3  g65(.a(new_n99_), .b(x24), .c(x23), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n109_), .c(new_n107_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n52_), .c(new_n67_), .O(z14));
  nand4  g68(.a(new_n99_), .b(x25), .c(x24), .d(x23), .O(new_n113_));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n110_), .b(new_n114_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n113_), .c(new_n51_), .O(new_n116_));
  nand3  g72(.a(x17), .b(new_n49_), .c(x09), .O(new_n117_));
  nand4  g73(.a(x26), .b(x25), .c(new_n48_), .d(x01), .O(new_n118_));
  nor2   g74(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nor2   g75(.a(new_n119_), .b(z07), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(new_n116_), .O(z15));
  inv1   g77(.a(z08), .O(new_n122_));
  inv1   g78(.a(x26), .O(new_n123_));
  nand2  g79(.a(new_n113_), .b(new_n123_), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nand3  g81(.a(new_n55_), .b(x19), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n124_), .c(new_n88_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n122_), .O(z16));
endmodule


