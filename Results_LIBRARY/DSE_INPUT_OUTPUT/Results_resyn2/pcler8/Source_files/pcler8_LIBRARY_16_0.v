// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  nor2   g00(.a(x17), .b(x13), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand3  g02(.a(x21), .b(x20), .c(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand3  g04(.a(x26), .b(x25), .c(x24), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  nand2  g06(.a(x09), .b(new_n50_), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(x23), .b(x22), .c(new_n52_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n46_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n45_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(new_n45_), .O(z07));
  aoi21  g26(.a(x08), .b(x07), .c(new_n45_), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand3  g31(.a(new_n52_), .b(x09), .c(new_n50_), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n57_), .c(new_n45_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  nand3  g36(.a(x22), .b(x21), .c(x12), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(new_n73_), .c(new_n80_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n76_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand3  g41(.a(new_n85_), .b(new_n59_), .c(new_n46_), .O(z10));
  aoi21  g42(.a(x20), .b(x19), .c(x21), .O(new_n87_));
  nand4  g43(.a(new_n47_), .b(new_n52_), .c(x09), .d(new_n50_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n61_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n46_), .O(new_n90_));
  nand3  g46(.a(new_n54_), .b(x21), .c(x13), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n90_), .O(z11));
  inv1   g48(.a(new_n73_), .O(new_n93_));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n94_));
  aoi21  g50(.a(new_n93_), .b(x14), .c(new_n94_), .O(new_n95_));
  inv1   g51(.a(x22), .O(new_n96_));
  nand2  g52(.a(new_n47_), .b(new_n96_), .O(new_n97_));
  nand3  g53(.a(new_n97_), .b(new_n77_), .c(new_n46_), .O(new_n98_));
  oai22  g54(.a(new_n98_), .b(new_n95_), .c(new_n63_), .d(new_n45_), .O(z12));
  inv1   g55(.a(x15), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nor2   g57(.a(new_n94_), .b(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n49_), .b(new_n100_), .c(new_n102_), .O(new_n103_));
  aoi21  g59(.a(new_n94_), .b(new_n101_), .c(new_n76_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n65_), .c(new_n46_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  and2   g64(.a(x22), .b(x21), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n80_), .c(x24), .d(x23), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n94_), .b(new_n101_), .c(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  aoi21  g69(.a(x08), .b(x05), .c(new_n45_), .O(new_n114_));
  oai21  g70(.a(new_n113_), .b(new_n76_), .c(new_n114_), .O(z14));
  inv1   g71(.a(x25), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(new_n111_), .O(new_n117_));
  nand2  g73(.a(x26), .b(x17), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n102_), .c(new_n117_), .O(new_n119_));
  nand2  g75(.a(new_n110_), .b(new_n116_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n77_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n69_), .c(new_n46_), .O(z15));
  aoi21  g78(.a(new_n102_), .b(new_n117_), .c(x26), .O(new_n123_));
  nor2   g79(.a(new_n94_), .b(x18), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n93_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n77_), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n123_), .c(new_n71_), .O(z16));
endmodule


