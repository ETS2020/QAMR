// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n49_, new_n51_, new_n53_, new_n55_, new_n57_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_;
  nand2  g00(.a(x26), .b(x25), .O(new_n46_));
  nand2  g01(.a(x08), .b(x00), .O(new_n47_));
  nand2  g02(.a(new_n47_), .b(new_n46_), .O(z01));
  nand3  g03(.a(new_n46_), .b(x08), .c(x01), .O(new_n49_));
  inv1   g04(.a(new_n49_), .O(z02));
  nand2  g05(.a(x08), .b(x02), .O(new_n51_));
  nand2  g06(.a(new_n51_), .b(new_n46_), .O(z03));
  nand3  g07(.a(new_n46_), .b(x08), .c(x03), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(z04));
  nand3  g09(.a(new_n46_), .b(x08), .c(x04), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(z05));
  nand2  g11(.a(x08), .b(x05), .O(new_n57_));
  nand2  g12(.a(new_n57_), .b(new_n46_), .O(z06));
  nand2  g13(.a(x08), .b(x06), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n46_), .O(z07));
  nand2  g15(.a(x08), .b(x07), .O(new_n61_));
  and2   g16(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(z08));
  inv1   g18(.a(new_n46_), .O(new_n64_));
  inv1   g19(.a(x19), .O(new_n65_));
  inv1   g20(.a(x08), .O(new_n66_));
  inv1   g21(.a(x10), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(x09), .c(new_n66_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n65_), .O(new_n70_));
  aoi21  g25(.a(new_n70_), .b(new_n47_), .c(new_n64_), .O(z09));
  nand4  g26(.a(new_n46_), .b(new_n67_), .c(x09), .d(new_n66_), .O(new_n72_));
  xnor2a g27(.a(x20), .b(x19), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(new_n72_), .c(new_n49_), .O(z10));
  inv1   g29(.a(x21), .O(new_n75_));
  nand2  g30(.a(x20), .b(x19), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g32(.a(x21), .b(x20), .c(x19), .O(new_n78_));
  nand3  g33(.a(new_n78_), .b(new_n77_), .c(new_n69_), .O(new_n79_));
  aoi21  g34(.a(new_n79_), .b(new_n51_), .c(new_n64_), .O(z11));
  nand4  g35(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n81_));
  inv1   g36(.a(x22), .O(new_n82_));
  aoi21  g37(.a(new_n78_), .b(new_n82_), .c(new_n72_), .O(new_n83_));
  nand2  g38(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand2  g39(.a(new_n84_), .b(new_n53_), .O(z12));
  inv1   g40(.a(new_n81_), .O(new_n86_));
  aoi21  g41(.a(new_n86_), .b(x23), .c(new_n72_), .O(new_n87_));
  oai21  g42(.a(new_n86_), .b(x23), .c(new_n87_), .O(new_n88_));
  nand2  g43(.a(new_n88_), .b(new_n55_), .O(z13));
  inv1   g44(.a(x23), .O(new_n90_));
  inv1   g45(.a(x24), .O(new_n91_));
  oai21  g46(.a(new_n81_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand3  g47(.a(new_n86_), .b(x24), .c(x23), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(new_n92_), .c(new_n69_), .O(new_n94_));
  aoi21  g49(.a(new_n94_), .b(new_n57_), .c(new_n64_), .O(z14));
  nand4  g50(.a(new_n86_), .b(x25), .c(x24), .d(x23), .O(new_n96_));
  inv1   g51(.a(x25), .O(new_n97_));
  nand2  g52(.a(new_n93_), .b(new_n97_), .O(new_n98_));
  nand3  g53(.a(new_n98_), .b(new_n96_), .c(new_n69_), .O(new_n99_));
  nand3  g54(.a(new_n99_), .b(new_n59_), .c(new_n46_), .O(z15));
  inv1   g55(.a(x26), .O(new_n101_));
  nand2  g56(.a(new_n96_), .b(new_n101_), .O(new_n102_));
  nand2  g57(.a(new_n102_), .b(new_n69_), .O(new_n103_));
  nand2  g58(.a(new_n103_), .b(new_n62_), .O(z16));
  zero   g59(.O(z00));
endmodule


