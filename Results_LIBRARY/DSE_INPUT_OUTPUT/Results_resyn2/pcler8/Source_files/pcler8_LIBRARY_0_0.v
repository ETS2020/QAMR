// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  nor2   g00(.a(x16), .b(x11), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand2  g02(.a(x20), .b(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand3  g04(.a(x23), .b(x22), .c(x21), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nor3   g09(.a(new_n53_), .b(new_n50_), .c(new_n49_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n48_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n45_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n45_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n45_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n46_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n46_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n46_), .O(z08));
  nand3  g28(.a(new_n54_), .b(x20), .c(x11), .O(new_n73_));
  inv1   g29(.a(new_n53_), .O(new_n74_));
  nor2   g30(.a(new_n45_), .b(x19), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n74_), .c(z01), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(new_n73_), .O(z09));
  nor2   g33(.a(new_n50_), .b(new_n49_), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(x12), .c(new_n47_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n74_), .O(new_n80_));
  aoi21  g36(.a(x08), .b(x01), .c(new_n45_), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n81_), .O(z10));
  nand2  g38(.a(new_n48_), .b(x21), .O(new_n83_));
  nand3  g39(.a(x23), .b(x22), .c(x13), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n50_), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  oai21  g42(.a(new_n48_), .b(x21), .c(new_n74_), .O(new_n87_));
  aoi21  g43(.a(x08), .b(x02), .c(new_n45_), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(z11));
  inv1   g45(.a(new_n50_), .O(new_n90_));
  nand4  g46(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n91_));
  and2   g47(.a(x23), .b(x14), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n90_), .c(new_n91_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n83_), .b(new_n94_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n74_), .O(new_n96_));
  aoi21  g52(.a(x08), .b(x03), .c(new_n45_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n93_), .c(new_n97_), .O(z12));
  nand2  g54(.a(new_n90_), .b(x15), .O(new_n99_));
  inv1   g55(.a(x23), .O(new_n100_));
  nor2   g56(.a(new_n91_), .b(new_n100_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g58(.a(new_n91_), .b(new_n100_), .c(new_n53_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  aoi21  g60(.a(new_n104_), .b(new_n65_), .c(new_n45_), .O(z13));
  nor2   g61(.a(new_n49_), .b(new_n47_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(x24), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  oai21  g64(.a(new_n49_), .b(new_n47_), .c(new_n108_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n107_), .c(new_n74_), .d(new_n46_), .O(new_n110_));
  nand2  g66(.a(new_n90_), .b(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n74_), .c(z06), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n110_), .O(z14));
  inv1   g70(.a(x25), .O(new_n115_));
  aoi21  g71(.a(x26), .b(x17), .c(new_n115_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n106_), .c(x24), .O(new_n117_));
  nand2  g73(.a(new_n107_), .b(new_n115_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n117_), .c(new_n74_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n69_), .c(new_n46_), .O(z15));
  and2   g76(.a(x25), .b(x24), .O(new_n121_));
  inv1   g77(.a(x26), .O(new_n122_));
  nor2   g78(.a(new_n122_), .b(x18), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n106_), .c(new_n121_), .O(new_n124_));
  inv1   g80(.a(new_n49_), .O(new_n125_));
  nand3  g81(.a(new_n121_), .b(new_n125_), .c(new_n48_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n122_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n124_), .c(new_n74_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n71_), .c(new_n45_), .O(z16));
endmodule


