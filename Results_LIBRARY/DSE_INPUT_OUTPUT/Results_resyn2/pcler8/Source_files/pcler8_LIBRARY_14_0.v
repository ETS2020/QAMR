// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  nand3  g00(.a(x21), .b(x20), .c(x19), .O(new_n45_));
  nand3  g01(.a(x26), .b(x25), .c(x24), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n47_), .c(x23), .d(x22), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g09(.a(x25), .b(x22), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nor2   g14(.a(new_n58_), .b(new_n54_), .O(z02));
  nand3  g15(.a(new_n55_), .b(x08), .c(x02), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z03));
  nand2  g17(.a(x08), .b(x03), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n54_), .O(z04));
  nand2  g19(.a(x08), .b(x04), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n54_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n55_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n55_), .O(z07));
  nand2  g25(.a(x08), .b(x07), .O(new_n70_));
  nor2   g26(.a(new_n70_), .b(new_n54_), .O(z08));
  oai21  g27(.a(new_n50_), .b(x19), .c(new_n56_), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n55_), .O(new_n73_));
  nand3  g29(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n52_), .c(new_n73_), .O(z09));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand4  g32(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n77_));
  nand3  g33(.a(x26), .b(x25), .c(x12), .O(new_n78_));
  nor2   g34(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  oai21  g36(.a(x20), .b(x19), .c(new_n51_), .O(new_n81_));
  aoi21  g37(.a(x08), .b(x01), .c(new_n54_), .O(new_n82_));
  oai21  g38(.a(new_n81_), .b(new_n80_), .c(new_n82_), .O(z10));
  inv1   g39(.a(x21), .O(new_n84_));
  inv1   g40(.a(x22), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n47_), .c(x23), .d(x13), .O(new_n87_));
  nand2  g43(.a(new_n76_), .b(new_n84_), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(new_n55_), .c(new_n45_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n51_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n60_), .O(z11));
  nand2  g48(.a(x24), .b(x23), .O(new_n93_));
  nor2   g49(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand3  g50(.a(x26), .b(x25), .c(x14), .O(new_n95_));
  inv1   g51(.a(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n45_), .b(new_n85_), .O(new_n97_));
  nand4  g53(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  aoi22  g54(.a(new_n98_), .b(new_n97_), .c(new_n96_), .d(new_n94_), .O(new_n99_));
  aoi21  g55(.a(x08), .b(x03), .c(new_n54_), .O(new_n100_));
  oai21  g56(.a(new_n99_), .b(new_n50_), .c(new_n100_), .O(z12));
  nand2  g57(.a(new_n47_), .b(x15), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  nor2   g59(.a(new_n98_), .b(new_n103_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  aoi21  g61(.a(new_n98_), .b(new_n103_), .c(new_n50_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n64_), .c(new_n55_), .O(z13));
  inv1   g64(.a(x24), .O(new_n109_));
  nand3  g65(.a(x26), .b(x25), .c(x16), .O(new_n110_));
  nand4  g66(.a(x23), .b(x21), .c(x20), .d(x19), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n109_), .O(new_n113_));
  nand3  g69(.a(new_n109_), .b(x23), .c(x22), .O(new_n114_));
  nor2   g70(.a(new_n114_), .b(new_n45_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n113_), .c(new_n51_), .O(new_n116_));
  oai21  g72(.a(new_n50_), .b(new_n109_), .c(x25), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n85_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n116_), .c(new_n66_), .O(z14));
  oai21  g75(.a(new_n93_), .b(new_n45_), .c(x25), .O(new_n120_));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(x25), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n94_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n120_), .c(x22), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n51_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n68_), .c(new_n55_), .O(z15));
  inv1   g82(.a(x26), .O(new_n127_));
  inv1   g83(.a(x25), .O(new_n128_));
  nor2   g84(.a(new_n128_), .b(x18), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n94_), .c(new_n127_), .O(new_n130_));
  nand4  g86(.a(new_n127_), .b(x25), .c(x20), .d(x19), .O(new_n131_));
  nor2   g87(.a(new_n131_), .b(new_n77_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n130_), .c(new_n51_), .O(new_n133_));
  oai21  g89(.a(new_n50_), .b(new_n127_), .c(x25), .O(new_n134_));
  aoi22  g90(.a(new_n134_), .b(new_n85_), .c(x08), .d(x07), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n133_), .O(z16));
endmodule


