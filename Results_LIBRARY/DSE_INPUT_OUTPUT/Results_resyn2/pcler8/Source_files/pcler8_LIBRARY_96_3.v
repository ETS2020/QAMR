// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  inv1   g02(.a(x25), .O(new_n47_));
  inv1   g03(.a(x26), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(x24), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nor4   g09(.a(new_n53_), .b(new_n50_), .c(new_n46_), .d(new_n45_), .O(z00));
  nand2  g10(.a(x10), .b(x09), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x08), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(z01));
  nand2  g15(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(new_n57_), .b(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(new_n57_), .b(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(new_n57_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n55_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n51_), .b(new_n70_), .c(new_n55_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  oai21  g28(.a(new_n51_), .b(new_n72_), .c(new_n55_), .O(z08));
  inv1   g29(.a(new_n53_), .O(new_n74_));
  nand3  g30(.a(new_n49_), .b(x24), .c(x23), .O(new_n75_));
  nand3  g31(.a(x22), .b(x21), .c(x20), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n75_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n74_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n58_), .O(z09));
  inv1   g37(.a(x09), .O(new_n82_));
  nand2  g38(.a(x08), .b(x01), .O(new_n83_));
  and2   g39(.a(x20), .b(x19), .O(new_n84_));
  and2   g40(.a(x22), .b(x21), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x12), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n75_), .c(new_n84_), .O(new_n87_));
  nor2   g43(.a(x20), .b(x19), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(x08), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n87_), .c(x10), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n82_), .c(new_n83_), .O(z10));
  nand2  g47(.a(x08), .b(x02), .O(new_n92_));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  inv1   g49(.a(new_n93_), .O(new_n94_));
  nand2  g50(.a(x22), .b(x13), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n75_), .c(new_n94_), .O(new_n96_));
  nor2   g52(.a(new_n84_), .b(x21), .O(new_n97_));
  nor2   g53(.a(new_n97_), .b(x08), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n96_), .c(x10), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n82_), .c(new_n92_), .O(z11));
  nand2  g56(.a(x08), .b(x03), .O(new_n101_));
  inv1   g57(.a(x14), .O(new_n102_));
  nand4  g58(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  oai21  g60(.a(new_n75_), .b(new_n102_), .c(new_n104_), .O(new_n105_));
  inv1   g61(.a(x22), .O(new_n106_));
  aoi21  g62(.a(new_n93_), .b(new_n106_), .c(x08), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(x10), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(new_n82_), .c(new_n101_), .O(z12));
  nand2  g65(.a(x08), .b(x04), .O(new_n110_));
  inv1   g66(.a(x15), .O(new_n111_));
  nor2   g67(.a(new_n46_), .b(new_n45_), .O(new_n112_));
  oai21  g68(.a(new_n50_), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  aoi21  g69(.a(new_n46_), .b(new_n45_), .c(x08), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n113_), .c(x10), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n82_), .c(new_n110_), .O(z13));
  nand4  g72(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  inv1   g74(.a(x24), .O(new_n119_));
  oai21  g75(.a(new_n46_), .b(new_n45_), .c(new_n119_), .O(new_n120_));
  nand4  g76(.a(new_n85_), .b(new_n84_), .c(x24), .d(x23), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n118_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(x08), .c(new_n52_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(x09), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n68_), .O(z14));
  nand3  g81(.a(new_n112_), .b(x25), .c(x24), .O(new_n126_));
  nand2  g82(.a(new_n121_), .b(new_n47_), .O(new_n127_));
  aoi22  g83(.a(new_n127_), .b(new_n126_), .c(new_n49_), .d(x17), .O(new_n128_));
  oai22  g84(.a(new_n128_), .b(new_n53_), .c(new_n56_), .d(new_n70_), .O(z15));
  nand2  g85(.a(new_n126_), .b(new_n48_), .O(new_n130_));
  inv1   g86(.a(x18), .O(new_n131_));
  inv1   g87(.a(new_n75_), .O(new_n132_));
  nand4  g88(.a(new_n77_), .b(new_n132_), .c(x19), .d(new_n131_), .O(new_n133_));
  nand3  g89(.a(new_n133_), .b(new_n130_), .c(new_n74_), .O(new_n134_));
  oai21  g90(.a(new_n56_), .b(new_n72_), .c(new_n134_), .O(z16));
endmodule


