// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_;
  inv1   g00(.a(x04), .O(new_n45_));
  nand2  g01(.a(x17), .b(new_n45_), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand2  g04(.a(x24), .b(x23), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand2  g06(.a(x26), .b(x25), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n50_), .c(new_n48_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  inv1   g15(.a(new_n46_), .O(new_n60_));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n60_), .O(z03));
  nand2  g20(.a(x08), .b(x03), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n60_), .O(z04));
  nor2   g22(.a(new_n52_), .b(new_n45_), .O(z05));
  nand3  g23(.a(new_n46_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n52_), .b(new_n70_), .c(new_n46_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z08));
  inv1   g29(.a(z01), .O(new_n74_));
  inv1   g30(.a(x19), .O(new_n75_));
  nand2  g31(.a(x22), .b(x21), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  and2   g34(.a(x20), .b(x11), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n78_), .c(new_n75_), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n54_), .c(new_n74_), .O(z09));
  nand2  g37(.a(x20), .b(x19), .O(new_n82_));
  aoi21  g38(.a(new_n78_), .b(x12), .c(new_n82_), .O(new_n83_));
  inv1   g39(.a(new_n54_), .O(new_n84_));
  oai21  g40(.a(x20), .b(x19), .c(new_n84_), .O(new_n85_));
  and2   g41(.a(new_n61_), .b(new_n46_), .O(new_n86_));
  oai21  g42(.a(new_n85_), .b(new_n83_), .c(new_n86_), .O(z10));
  inv1   g43(.a(new_n77_), .O(new_n88_));
  inv1   g44(.a(new_n82_), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(x21), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n88_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n89_), .b(x21), .c(new_n84_), .O(new_n93_));
  and2   g49(.a(new_n63_), .b(new_n46_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z11));
  nand3  g51(.a(new_n89_), .b(x22), .c(x21), .O(new_n96_));
  aoi21  g52(.a(new_n88_), .b(x14), .c(new_n96_), .O(new_n97_));
  inv1   g53(.a(x22), .O(new_n98_));
  nand2  g54(.a(new_n90_), .b(new_n98_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n84_), .O(new_n100_));
  and2   g56(.a(new_n65_), .b(new_n46_), .O(new_n101_));
  oai21  g57(.a(new_n100_), .b(new_n97_), .c(new_n101_), .O(z12));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n48_), .c(x23), .O(new_n104_));
  inv1   g60(.a(x23), .O(new_n105_));
  nand2  g61(.a(new_n47_), .b(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(new_n104_), .c(new_n84_), .O(new_n107_));
  nor2   g63(.a(z05), .b(new_n60_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(z13));
  nand2  g65(.a(new_n50_), .b(new_n48_), .O(new_n110_));
  nand4  g66(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n47_), .b(new_n105_), .c(new_n113_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n110_), .c(new_n112_), .O(new_n115_));
  nand2  g71(.a(new_n84_), .b(new_n46_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n115_), .c(new_n68_), .O(z14));
  inv1   g73(.a(x25), .O(new_n118_));
  oai21  g74(.a(new_n49_), .b(new_n47_), .c(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n50_), .b(new_n48_), .c(x25), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n119_), .c(new_n84_), .O(new_n121_));
  nor2   g77(.a(new_n52_), .b(new_n70_), .O(new_n122_));
  aoi21  g78(.a(new_n55_), .b(x17), .c(new_n122_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n60_), .O(z15));
  inv1   g80(.a(x26), .O(new_n125_));
  nor2   g81(.a(new_n125_), .b(x18), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n50_), .c(new_n48_), .d(x25), .O(new_n127_));
  nand2  g83(.a(new_n120_), .b(new_n125_), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n127_), .c(new_n84_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


