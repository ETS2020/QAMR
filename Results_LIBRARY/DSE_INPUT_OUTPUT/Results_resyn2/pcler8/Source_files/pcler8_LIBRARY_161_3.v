// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_;
  nand2  g00(.a(x11), .b(x10), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  nand4  g04(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand4  g06(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand2  g08(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  oai21  g09(.a(new_n53_), .b(new_n48_), .c(new_n45_), .O(z00));
  nand2  g10(.a(x08), .b(x00), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n45_), .O(z01));
  inv1   g12(.a(new_n45_), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n46_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n45_), .O(z03));
  nand2  g18(.a(new_n58_), .b(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(new_n58_), .b(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n45_), .O(z07));
  nand2  g26(.a(new_n58_), .b(x07), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z08));
  inv1   g28(.a(new_n48_), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n49_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g32(.a(new_n58_), .b(x00), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n76_), .O(z09));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  and2   g35(.a(x22), .b(x21), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(new_n50_), .c(x12), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nor2   g38(.a(x20), .b(x19), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n48_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  aoi21  g41(.a(x08), .b(x01), .c(new_n57_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n85_), .O(z10));
  inv1   g43(.a(z03), .O(new_n88_));
  nand3  g44(.a(x21), .b(x20), .c(x19), .O(new_n89_));
  and2   g45(.a(x22), .b(x13), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n50_), .c(new_n89_), .O(new_n91_));
  oai21  g47(.a(new_n79_), .b(x21), .c(new_n73_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n91_), .c(new_n88_), .O(z11));
  aoi21  g49(.a(new_n50_), .b(x14), .c(new_n51_), .O(new_n94_));
  inv1   g50(.a(new_n89_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(x22), .c(new_n73_), .O(new_n96_));
  aoi21  g52(.a(x08), .b(x03), .c(new_n57_), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n94_), .c(new_n97_), .O(z12));
  inv1   g54(.a(x15), .O(new_n99_));
  nor2   g55(.a(new_n49_), .b(new_n99_), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  xor2a  g57(.a(new_n51_), .b(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n73_), .O(new_n103_));
  aoi21  g59(.a(x08), .b(x04), .c(new_n57_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(z13));
  inv1   g61(.a(z06), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  nand4  g64(.a(new_n80_), .b(new_n79_), .c(x24), .d(x23), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n51_), .b(new_n101_), .c(new_n110_), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n109_), .c(new_n108_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n48_), .c(new_n106_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  nor2   g70(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nor2   g71(.a(new_n51_), .b(new_n101_), .O(new_n116_));
  nand2  g72(.a(x26), .b(x17), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n116_), .c(new_n115_), .O(new_n118_));
  nand2  g74(.a(new_n109_), .b(new_n114_), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n118_), .c(new_n73_), .O(new_n120_));
  nand2  g76(.a(new_n58_), .b(x06), .O(new_n121_));
  nand2  g77(.a(new_n121_), .b(new_n120_), .O(z15));
  aoi21  g78(.a(new_n116_), .b(new_n115_), .c(x26), .O(new_n123_));
  oai21  g79(.a(new_n53_), .b(x18), .c(new_n73_), .O(new_n124_));
  aoi21  g80(.a(x08), .b(x07), .c(new_n57_), .O(new_n125_));
  oai21  g81(.a(new_n124_), .b(new_n123_), .c(new_n125_), .O(z16));
endmodule


