// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x16), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x12), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand3  g03(.a(x26), .b(x25), .c(x24), .O(new_n48_));
  inv1   g04(.a(x08), .O(new_n49_));
  inv1   g05(.a(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n49_), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n48_), .O(new_n52_));
  inv1   g08(.a(x21), .O(new_n53_));
  nand4  g09(.a(x23), .b(x22), .c(x20), .d(x19), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n47_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n47_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n47_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nor2   g18(.a(new_n62_), .b(new_n46_), .O(z03));
  nand3  g19(.a(new_n47_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand3  g21(.a(new_n47_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(x08), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n47_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n47_), .O(z07));
  nand2  g27(.a(x08), .b(x07), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n47_), .O(z08));
  inv1   g29(.a(new_n51_), .O(new_n74_));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  and2   g31(.a(x22), .b(x21), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n75_), .c(x19), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n58_), .c(new_n46_), .O(z09));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  nand2  g37(.a(new_n76_), .b(x12), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n75_), .O(new_n83_));
  nor2   g39(.a(x20), .b(x19), .O(new_n84_));
  nor2   g40(.a(new_n84_), .b(new_n51_), .O(new_n85_));
  oai21  g41(.a(new_n83_), .b(new_n81_), .c(new_n85_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n60_), .c(new_n47_), .O(z10));
  inv1   g43(.a(x13), .O(new_n88_));
  nor2   g44(.a(new_n54_), .b(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n52_), .c(x16), .O(new_n90_));
  nand3  g46(.a(x21), .b(x20), .c(x19), .O(new_n91_));
  inv1   g47(.a(new_n91_), .O(new_n92_));
  nand3  g48(.a(x22), .b(x13), .c(x12), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n75_), .c(new_n92_), .O(new_n94_));
  aoi21  g50(.a(new_n81_), .b(new_n53_), .c(new_n51_), .O(new_n95_));
  aoi22  g51(.a(new_n95_), .b(new_n94_), .c(x08), .d(x02), .O(new_n96_));
  oai21  g52(.a(new_n90_), .b(x12), .c(new_n96_), .O(z11));
  inv1   g53(.a(new_n75_), .O(new_n98_));
  nand4  g54(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  aoi21  g55(.a(new_n98_), .b(x14), .c(new_n99_), .O(new_n100_));
  nor2   g56(.a(new_n51_), .b(new_n46_), .O(new_n101_));
  oai21  g57(.a(new_n92_), .b(x22), .c(new_n101_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(new_n64_), .O(z12));
  inv1   g59(.a(x15), .O(new_n104_));
  oai21  g60(.a(new_n48_), .b(new_n104_), .c(new_n55_), .O(new_n105_));
  inv1   g61(.a(x23), .O(new_n106_));
  nand2  g62(.a(new_n99_), .b(new_n106_), .O(new_n107_));
  nand3  g63(.a(new_n107_), .b(new_n105_), .c(new_n101_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n66_), .O(z13));
  aoi21  g65(.a(new_n55_), .b(x24), .c(new_n51_), .O(new_n110_));
  oai21  g66(.a(new_n55_), .b(x24), .c(new_n110_), .O(new_n111_));
  inv1   g67(.a(x12), .O(new_n112_));
  oai21  g68(.a(new_n52_), .b(new_n112_), .c(x16), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n111_), .c(new_n68_), .O(z14));
  nand2  g70(.a(x26), .b(x17), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n55_), .c(x25), .d(x24), .O(new_n116_));
  inv1   g72(.a(x25), .O(new_n117_));
  inv1   g73(.a(new_n54_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(x24), .c(x21), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n116_), .c(new_n74_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n70_), .c(new_n46_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand4  g79(.a(new_n118_), .b(x25), .c(x24), .d(x21), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  nor2   g81(.a(new_n99_), .b(x18), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(new_n98_), .c(new_n51_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n72_), .c(new_n46_), .O(z16));
endmodule


