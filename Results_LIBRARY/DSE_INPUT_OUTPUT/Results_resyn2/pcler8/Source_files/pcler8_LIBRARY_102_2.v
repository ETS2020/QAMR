// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_;
  inv1   g00(.a(x00), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand3  g03(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand2  g05(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(z00));
  nand2  g09(.a(new_n46_), .b(x00), .O(new_n54_));
  nand2  g10(.a(x08), .b(x01), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(z02));
  nand2  g12(.a(x08), .b(x02), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n54_), .O(z03));
  inv1   g14(.a(x03), .O(new_n59_));
  nor2   g15(.a(new_n46_), .b(new_n59_), .O(z04));
  nand2  g16(.a(x08), .b(x04), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z05));
  nand2  g18(.a(x08), .b(x05), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n54_), .O(z06));
  nand2  g20(.a(x08), .b(x06), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n54_), .O(z07));
  nand2  g22(.a(x08), .b(x07), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n54_), .O(z08));
  and2   g24(.a(x22), .b(x21), .O(new_n69_));
  nand3  g25(.a(new_n69_), .b(x20), .c(x11), .O(new_n70_));
  oai21  g26(.a(new_n70_), .b(new_n51_), .c(x19), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n49_), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n45_), .O(z09));
  inv1   g29(.a(x19), .O(new_n74_));
  inv1   g30(.a(x20), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g32(.a(new_n51_), .O(new_n77_));
  nand3  g33(.a(new_n69_), .b(new_n77_), .c(x12), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  aoi21  g35(.a(new_n75_), .b(new_n74_), .c(new_n48_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(new_n55_), .c(new_n54_), .O(z10));
  inv1   g38(.a(z03), .O(new_n83_));
  nand3  g39(.a(x26), .b(x25), .c(x24), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  and2   g42(.a(x23), .b(x22), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(x13), .O(new_n88_));
  inv1   g44(.a(new_n88_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n85_), .c(new_n86_), .O(new_n90_));
  oai21  g46(.a(new_n76_), .b(x21), .c(new_n49_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n90_), .c(new_n83_), .O(z11));
  aoi21  g48(.a(new_n77_), .b(x14), .c(new_n52_), .O(new_n93_));
  inv1   g49(.a(x22), .O(new_n94_));
  nand2  g50(.a(new_n86_), .b(new_n94_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n49_), .c(new_n45_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n93_), .c(new_n46_), .d(new_n59_), .O(z12));
  nand2  g53(.a(new_n85_), .b(x15), .O(new_n98_));
  inv1   g54(.a(x23), .O(new_n99_));
  nor2   g55(.a(new_n52_), .b(new_n99_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g57(.a(new_n52_), .b(new_n99_), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n101_), .c(new_n49_), .d(new_n45_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n61_), .O(z13));
  inv1   g60(.a(z06), .O(new_n105_));
  nand2  g61(.a(new_n85_), .b(x16), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  inv1   g63(.a(new_n86_), .O(new_n108_));
  nand3  g64(.a(new_n87_), .b(new_n108_), .c(x24), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  nand2  g66(.a(new_n87_), .b(new_n108_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n109_), .c(new_n107_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n48_), .c(new_n105_), .O(z14));
  nand3  g70(.a(x26), .b(x25), .c(x17), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  nand4  g72(.a(new_n87_), .b(new_n108_), .c(x25), .d(x24), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n109_), .b(new_n118_), .O(new_n119_));
  aoi21  g75(.a(new_n119_), .b(new_n117_), .c(new_n116_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n50_), .c(new_n65_), .O(z15));
  inv1   g77(.a(new_n117_), .O(new_n122_));
  inv1   g78(.a(x18), .O(new_n123_));
  nor2   g79(.a(new_n52_), .b(new_n51_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n123_), .c(new_n48_), .O(new_n125_));
  oai21  g81(.a(new_n122_), .b(x26), .c(new_n125_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n67_), .c(new_n54_), .O(z16));
  buf    g83(.a(x00), .O(z01));
endmodule


