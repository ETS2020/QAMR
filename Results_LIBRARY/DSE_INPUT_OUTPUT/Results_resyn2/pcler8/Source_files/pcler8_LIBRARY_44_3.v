// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  nand2  g00(.a(x10), .b(x06), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g04(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n45_), .O(z00));
  and2   g11(.a(new_n45_), .b(x08), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n45_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n45_), .O(z03));
  nand2  g18(.a(new_n56_), .b(x03), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z04));
  nand2  g20(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(x10), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n45_), .O(z08));
  and2   g28(.a(x22), .b(x21), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n51_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n50_), .O(new_n76_));
  nand2  g32(.a(x08), .b(x00), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(new_n76_), .c(new_n45_), .O(z09));
  and2   g34(.a(x20), .b(x19), .O(new_n79_));
  inv1   g35(.a(new_n51_), .O(new_n80_));
  nand3  g36(.a(new_n73_), .b(new_n80_), .c(x12), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  or2    g38(.a(x20), .b(x19), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n82_), .c(new_n50_), .O(new_n84_));
  nand2  g40(.a(new_n56_), .b(x01), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(new_n84_), .O(z10));
  nand2  g42(.a(new_n79_), .b(x21), .O(new_n87_));
  and2   g43(.a(x22), .b(x13), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n80_), .c(new_n87_), .O(new_n89_));
  or2    g45(.a(new_n79_), .b(x21), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(new_n50_), .O(new_n91_));
  nand2  g47(.a(new_n56_), .b(x02), .O(new_n92_));
  oai21  g48(.a(new_n91_), .b(new_n89_), .c(new_n92_), .O(z11));
  aoi21  g49(.a(new_n80_), .b(x14), .c(new_n52_), .O(new_n94_));
  inv1   g50(.a(x22), .O(new_n95_));
  nand2  g51(.a(new_n87_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n50_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n94_), .c(new_n63_), .O(z12));
  inv1   g54(.a(x23), .O(new_n99_));
  xor2a  g55(.a(new_n52_), .b(new_n99_), .O(new_n100_));
  aoi21  g56(.a(new_n80_), .b(x15), .c(new_n100_), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n49_), .c(new_n65_), .O(z13));
  nand3  g58(.a(x26), .b(x25), .c(x24), .O(new_n103_));
  inv1   g59(.a(new_n103_), .O(new_n104_));
  nand4  g60(.a(new_n73_), .b(new_n79_), .c(x24), .d(x23), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n52_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  aoi22  g63(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(x16), .O(new_n108_));
  nand2  g64(.a(new_n56_), .b(x05), .O(new_n109_));
  oai21  g65(.a(new_n108_), .b(new_n49_), .c(new_n109_), .O(z14));
  inv1   g66(.a(x25), .O(new_n111_));
  nand2  g67(.a(new_n105_), .b(new_n111_), .O(new_n112_));
  nor2   g68(.a(new_n111_), .b(new_n106_), .O(new_n113_));
  nor2   g69(.a(new_n52_), .b(new_n99_), .O(new_n114_));
  nand2  g70(.a(x26), .b(x17), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n114_), .c(new_n113_), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(new_n112_), .c(new_n48_), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(new_n69_), .c(x10), .O(z15));
  inv1   g74(.a(x18), .O(new_n119_));
  nand2  g75(.a(new_n53_), .b(new_n119_), .O(new_n120_));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n114_), .b(new_n113_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n120_), .c(new_n50_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n71_), .c(new_n45_), .O(z16));
endmodule


