// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x08), .O(new_n45_));
  nand2  g01(.a(x09), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  nand3  g03(.a(x25), .b(x24), .c(x23), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g06(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  aoi21  g07(.a(new_n51_), .b(x26), .c(x10), .O(z00));
  inv1   g08(.a(x00), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  inv1   g10(.a(x26), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n53_), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n56_), .O(z02));
  inv1   g16(.a(x02), .O(new_n61_));
  nor2   g17(.a(new_n57_), .b(new_n61_), .O(z03));
  inv1   g18(.a(x03), .O(new_n63_));
  nor2   g19(.a(new_n57_), .b(new_n63_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n56_), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  oai21  g23(.a(new_n45_), .b(new_n67_), .c(new_n56_), .O(z06));
  inv1   g24(.a(x06), .O(new_n69_));
  nor2   g25(.a(new_n57_), .b(new_n69_), .O(z07));
  inv1   g26(.a(x07), .O(new_n71_));
  nor2   g27(.a(new_n57_), .b(new_n71_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  nand4  g29(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(x21), .b(x20), .c(x11), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n75_), .c(new_n73_), .O(new_n78_));
  nand3  g34(.a(new_n47_), .b(x26), .c(new_n54_), .O(new_n79_));
  oai22  g35(.a(new_n79_), .b(new_n78_), .c(new_n57_), .d(new_n53_), .O(z09));
  nand2  g36(.a(x20), .b(x19), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  nand2  g38(.a(x21), .b(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n74_), .c(new_n82_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n46_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n55_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(x10), .c(new_n59_), .O(z10));
  inv1   g44(.a(x13), .O(new_n89_));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  inv1   g46(.a(new_n90_), .O(new_n91_));
  oai21  g47(.a(new_n74_), .b(new_n89_), .c(new_n91_), .O(new_n92_));
  inv1   g48(.a(x21), .O(new_n93_));
  aoi21  g49(.a(new_n81_), .b(new_n93_), .c(new_n46_), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n55_), .O(new_n95_));
  oai22  g51(.a(new_n95_), .b(x10), .c(new_n45_), .d(new_n61_), .O(z11));
  nand2  g52(.a(x08), .b(x03), .O(new_n97_));
  nand2  g53(.a(new_n75_), .b(x14), .O(new_n98_));
  xor2a  g54(.a(new_n90_), .b(x22), .O(new_n99_));
  aoi21  g55(.a(new_n99_), .b(new_n98_), .c(new_n46_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n55_), .c(new_n54_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n97_), .O(z12));
  inv1   g58(.a(new_n48_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(x15), .O(new_n104_));
  xor2a  g60(.a(new_n49_), .b(x23), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n104_), .c(new_n46_), .O(new_n106_));
  oai21  g62(.a(new_n106_), .b(new_n55_), .c(new_n54_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n65_), .O(z13));
  inv1   g64(.a(x24), .O(new_n109_));
  inv1   g65(.a(x25), .O(new_n110_));
  nor2   g66(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  inv1   g67(.a(new_n49_), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(x24), .c(x23), .O(new_n113_));
  inv1   g69(.a(x23), .O(new_n114_));
  oai21  g70(.a(new_n49_), .b(new_n114_), .c(new_n109_), .O(new_n115_));
  aoi22  g71(.a(new_n115_), .b(new_n113_), .c(new_n111_), .d(x16), .O(new_n116_));
  oai22  g72(.a(new_n116_), .b(new_n79_), .c(new_n57_), .d(new_n67_), .O(z14));
  nand2  g73(.a(x08), .b(x06), .O(new_n118_));
  nand2  g74(.a(new_n113_), .b(new_n110_), .O(new_n119_));
  inv1   g75(.a(x17), .O(new_n120_));
  aoi21  g76(.a(new_n50_), .b(new_n120_), .c(new_n46_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n119_), .c(new_n55_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(x10), .c(new_n118_), .O(z15));
  inv1   g79(.a(x18), .O(new_n124_));
  nand2  g80(.a(new_n50_), .b(new_n124_), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n47_), .c(new_n55_), .O(new_n126_));
  oai22  g82(.a(new_n126_), .b(x10), .c(new_n45_), .d(new_n71_), .O(z16));
endmodule


