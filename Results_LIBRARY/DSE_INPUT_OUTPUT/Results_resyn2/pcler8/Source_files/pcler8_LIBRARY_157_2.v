// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_;
  inv1   g00(.a(x19), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x10), .O(new_n46_));
  inv1   g02(.a(x22), .O(new_n47_));
  nand4  g03(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  inv1   g06(.a(x10), .O(new_n51_));
  nand3  g07(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  inv1   g08(.a(new_n52_), .O(new_n53_));
  inv1   g09(.a(x20), .O(new_n54_));
  inv1   g10(.a(x21), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g12(.a(new_n56_), .b(new_n53_), .c(new_n49_), .d(x19), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z00));
  inv1   g14(.a(x00), .O(new_n59_));
  nand2  g15(.a(new_n46_), .b(x08), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z01));
  inv1   g17(.a(x01), .O(new_n62_));
  nor2   g18(.a(new_n60_), .b(new_n62_), .O(z02));
  nand3  g19(.a(new_n46_), .b(x08), .c(x02), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  inv1   g21(.a(x03), .O(new_n66_));
  nor2   g22(.a(new_n60_), .b(new_n66_), .O(z04));
  inv1   g23(.a(x04), .O(new_n68_));
  oai21  g24(.a(new_n50_), .b(new_n68_), .c(new_n46_), .O(z05));
  inv1   g25(.a(x05), .O(new_n70_));
  nor2   g26(.a(new_n60_), .b(new_n70_), .O(z06));
  nand2  g27(.a(x08), .b(x06), .O(new_n72_));
  nand2  g28(.a(new_n72_), .b(new_n46_), .O(z07));
  nand2  g29(.a(x08), .b(x07), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n46_), .O(z08));
  nand3  g31(.a(x21), .b(x20), .c(x11), .O(new_n76_));
  inv1   g32(.a(new_n76_), .O(new_n77_));
  aoi21  g33(.a(new_n77_), .b(new_n49_), .c(new_n45_), .O(new_n78_));
  oai22  g34(.a(new_n78_), .b(new_n52_), .c(new_n60_), .d(new_n59_), .O(z09));
  nand2  g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n48_), .c(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n54_), .b(new_n45_), .c(new_n52_), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g41(.a(x08), .b(x01), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(new_n46_), .O(z10));
  nand2  g43(.a(new_n56_), .b(x19), .O(new_n88_));
  aoi21  g44(.a(new_n49_), .b(x13), .c(new_n88_), .O(new_n89_));
  oai21  g45(.a(new_n81_), .b(x21), .c(new_n53_), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n89_), .c(new_n64_), .O(z11));
  inv1   g47(.a(new_n48_), .O(new_n92_));
  nand4  g48(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n93_));
  aoi21  g49(.a(new_n92_), .b(x14), .c(new_n93_), .O(new_n94_));
  oai21  g50(.a(new_n80_), .b(new_n55_), .c(new_n47_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n53_), .O(new_n96_));
  oai22  g52(.a(new_n96_), .b(new_n94_), .c(new_n60_), .d(new_n66_), .O(z12));
  inv1   g53(.a(new_n93_), .O(new_n98_));
  nand4  g54(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n98_), .c(x23), .O(new_n100_));
  inv1   g56(.a(x23), .O(new_n101_));
  nand2  g57(.a(new_n93_), .b(new_n101_), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n100_), .c(new_n53_), .O(new_n103_));
  oai21  g59(.a(new_n60_), .b(new_n68_), .c(new_n103_), .O(z13));
  nand3  g60(.a(x26), .b(x25), .c(x16), .O(new_n105_));
  nand4  g61(.a(new_n105_), .b(new_n98_), .c(x24), .d(x23), .O(new_n106_));
  inv1   g62(.a(x24), .O(new_n107_));
  oai21  g63(.a(new_n93_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n106_), .c(new_n53_), .O(new_n109_));
  oai21  g65(.a(new_n60_), .b(new_n70_), .c(new_n109_), .O(z14));
  nand3  g66(.a(x24), .b(x23), .c(x20), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  nand2  g68(.a(x26), .b(x17), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n112_), .c(x22), .d(x21), .O(new_n114_));
  inv1   g70(.a(x25), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(x24), .c(x23), .O(new_n116_));
  nor2   g72(.a(new_n116_), .b(new_n93_), .O(new_n117_));
  aoi21  g73(.a(new_n114_), .b(x25), .c(new_n117_), .O(new_n118_));
  nand3  g74(.a(x25), .b(x09), .c(new_n50_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n51_), .O(new_n120_));
  aoi22  g76(.a(new_n120_), .b(new_n45_), .c(x08), .d(x06), .O(new_n121_));
  oai21  g77(.a(new_n118_), .b(new_n52_), .c(new_n121_), .O(z15));
  inv1   g78(.a(x26), .O(new_n123_));
  nand3  g79(.a(x25), .b(x24), .c(x23), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n93_), .c(new_n123_), .O(new_n125_));
  inv1   g81(.a(x18), .O(new_n126_));
  nand3  g82(.a(new_n56_), .b(new_n49_), .c(new_n126_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n125_), .c(new_n53_), .O(new_n128_));
  nand3  g84(.a(x26), .b(x09), .c(new_n50_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n51_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n45_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n128_), .c(new_n74_), .O(z16));
endmodule


