// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_;
  inv1   g00(.a(x17), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x02), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g05(.a(x25), .O(new_n50_));
  inv1   g06(.a(x26), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  inv1   g08(.a(x08), .O(new_n53_));
  inv1   g09(.a(x10), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand4  g12(.a(new_n56_), .b(new_n52_), .c(new_n49_), .d(x24), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z00));
  inv1   g14(.a(x02), .O(new_n59_));
  nor2   g15(.a(x17), .b(new_n59_), .O(new_n60_));
  nand2  g16(.a(x08), .b(x00), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z01));
  nand2  g18(.a(x08), .b(x01), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n46_), .O(z02));
  oai21  g20(.a(new_n45_), .b(x08), .c(x02), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z03));
  nand2  g22(.a(x08), .b(x03), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n60_), .O(z04));
  nand2  g24(.a(x08), .b(x04), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n46_), .O(z05));
  nand3  g26(.a(new_n46_), .b(x08), .c(x05), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n60_), .O(z07));
  nand2  g30(.a(x08), .b(x07), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n46_), .O(z08));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  nand4  g33(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n56_), .O(new_n80_));
  nand3  g36(.a(new_n80_), .b(new_n61_), .c(new_n46_), .O(z09));
  and2   g37(.a(x20), .b(x19), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n77_), .c(new_n82_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n55_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n63_), .c(new_n46_), .O(z10));
  inv1   g44(.a(new_n77_), .O(new_n89_));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n82_), .b(x21), .c(new_n56_), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n92_), .c(new_n65_), .O(z11));
  inv1   g50(.a(new_n90_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(x22), .O(new_n96_));
  aoi21  g52(.a(new_n89_), .b(x14), .c(new_n96_), .O(new_n97_));
  oai21  g53(.a(new_n95_), .b(x22), .c(new_n56_), .O(new_n98_));
  and2   g54(.a(new_n67_), .b(new_n46_), .O(new_n99_));
  oai21  g55(.a(new_n98_), .b(new_n97_), .c(new_n99_), .O(z12));
  inv1   g56(.a(new_n48_), .O(new_n101_));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n101_), .c(x23), .O(new_n103_));
  nand2  g59(.a(new_n48_), .b(new_n47_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n103_), .c(new_n56_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n69_), .c(new_n60_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n48_), .b(new_n47_), .c(new_n109_), .O(new_n110_));
  and2   g66(.a(x22), .b(x21), .O(new_n111_));
  and2   g67(.a(x24), .b(x23), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n111_), .c(new_n82_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n110_), .c(new_n108_), .O(new_n114_));
  nand2  g70(.a(new_n56_), .b(new_n46_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n114_), .c(new_n71_), .O(z14));
  nand4  g72(.a(new_n112_), .b(new_n111_), .c(new_n82_), .d(x25), .O(new_n117_));
  nand2  g73(.a(new_n113_), .b(new_n50_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n117_), .c(new_n56_), .d(new_n46_), .O(new_n119_));
  nand3  g75(.a(x26), .b(x25), .c(x17), .O(new_n120_));
  nor2   g76(.a(new_n120_), .b(new_n55_), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(z07), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n119_), .O(z15));
  inv1   g79(.a(z08), .O(new_n124_));
  nor2   g80(.a(new_n51_), .b(x18), .O(new_n125_));
  nand4  g81(.a(new_n125_), .b(new_n112_), .c(new_n101_), .d(x25), .O(new_n126_));
  nand2  g82(.a(new_n117_), .b(new_n51_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n126_), .c(new_n56_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n124_), .O(z16));
endmodule


