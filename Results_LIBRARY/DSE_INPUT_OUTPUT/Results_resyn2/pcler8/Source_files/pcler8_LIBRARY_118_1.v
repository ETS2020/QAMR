// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n63_,
    new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x09), .O(new_n45_));
  nand2  g01(.a(x10), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  inv1   g03(.a(x10), .O(new_n48_));
  nand3  g04(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  nand4  g06(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n51_));
  nand4  g07(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n46_), .O(z00));
  nor2   g11(.a(new_n48_), .b(x09), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  and2   g13(.a(new_n57_), .b(x00), .O(z01));
  inv1   g14(.a(x01), .O(new_n59_));
  oai21  g15(.a(new_n47_), .b(new_n59_), .c(new_n46_), .O(z02));
  and2   g16(.a(new_n57_), .b(x02), .O(z03));
  and2   g17(.a(new_n57_), .b(x03), .O(z04));
  inv1   g18(.a(x04), .O(new_n63_));
  inv1   g19(.a(new_n57_), .O(new_n64_));
  nor2   g20(.a(new_n64_), .b(new_n63_), .O(z05));
  nand2  g21(.a(x08), .b(x05), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n46_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n46_), .O(z07));
  nand2  g25(.a(new_n57_), .b(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nand4  g27(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n51_), .c(x19), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n50_), .O(new_n74_));
  aoi21  g30(.a(x08), .b(x00), .c(new_n56_), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n74_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  nand3  g33(.a(x22), .b(x21), .c(x12), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n51_), .c(new_n77_), .O(new_n79_));
  nor2   g35(.a(x20), .b(x19), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(new_n49_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(new_n64_), .b(new_n59_), .c(new_n82_), .O(z10));
  inv1   g39(.a(new_n51_), .O(new_n84_));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n77_), .b(x21), .c(new_n50_), .O(new_n88_));
  aoi21  g44(.a(x08), .b(x02), .c(new_n56_), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z11));
  aoi21  g46(.a(new_n84_), .b(x14), .c(new_n52_), .O(new_n91_));
  inv1   g47(.a(new_n85_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(x22), .c(new_n50_), .O(new_n93_));
  aoi21  g49(.a(x08), .b(x03), .c(new_n56_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n91_), .c(new_n94_), .O(z12));
  inv1   g51(.a(x15), .O(new_n96_));
  nor2   g52(.a(new_n51_), .b(new_n96_), .O(new_n97_));
  inv1   g53(.a(x23), .O(new_n98_));
  xor2a  g54(.a(new_n52_), .b(new_n98_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n97_), .c(new_n50_), .O(new_n100_));
  oai21  g56(.a(new_n64_), .b(new_n63_), .c(new_n100_), .O(z13));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  and2   g59(.a(x22), .b(x21), .O(new_n104_));
  nand4  g60(.a(new_n104_), .b(new_n77_), .c(x24), .d(x23), .O(new_n105_));
  inv1   g61(.a(x24), .O(new_n106_));
  oai21  g62(.a(new_n52_), .b(new_n98_), .c(new_n106_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n105_), .c(new_n103_), .O(new_n108_));
  nand2  g64(.a(new_n57_), .b(x05), .O(new_n109_));
  oai21  g65(.a(new_n108_), .b(new_n49_), .c(new_n109_), .O(z14));
  inv1   g66(.a(z07), .O(new_n111_));
  nor2   g67(.a(new_n52_), .b(new_n98_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(x24), .c(x25), .O(new_n113_));
  and2   g69(.a(x26), .b(x17), .O(new_n114_));
  and2   g70(.a(x25), .b(x24), .O(new_n115_));
  nand4  g71(.a(new_n104_), .b(new_n77_), .c(new_n115_), .d(x23), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n114_), .c(new_n50_), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n113_), .c(new_n111_), .O(z15));
  inv1   g74(.a(x18), .O(new_n119_));
  nand2  g75(.a(new_n53_), .b(new_n119_), .O(new_n120_));
  inv1   g76(.a(x26), .O(new_n121_));
  nand2  g77(.a(new_n116_), .b(new_n121_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n120_), .c(new_n50_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n70_), .O(z16));
endmodule


