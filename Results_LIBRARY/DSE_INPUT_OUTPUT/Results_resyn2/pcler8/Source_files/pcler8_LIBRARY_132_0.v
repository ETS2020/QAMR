// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor4   g06(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  inv1   g08(.a(x09), .O(new_n53_));
  nand2  g09(.a(new_n49_), .b(new_n53_), .O(new_n54_));
  oai21  g10(.a(new_n48_), .b(new_n52_), .c(new_n54_), .O(z01));
  inv1   g11(.a(x01), .O(new_n56_));
  nand2  g12(.a(new_n54_), .b(x08), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(new_n56_), .O(z02));
  inv1   g14(.a(x02), .O(new_n59_));
  nor2   g15(.a(new_n57_), .b(new_n59_), .O(z03));
  inv1   g16(.a(x03), .O(new_n61_));
  nor2   g17(.a(new_n57_), .b(new_n61_), .O(z04));
  inv1   g18(.a(x04), .O(new_n63_));
  oai21  g19(.a(new_n48_), .b(new_n63_), .c(new_n54_), .O(z05));
  nand2  g20(.a(x08), .b(x05), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n54_), .O(z06));
  inv1   g22(.a(x06), .O(new_n67_));
  nor2   g23(.a(new_n57_), .b(new_n67_), .O(z07));
  inv1   g24(.a(x07), .O(new_n69_));
  nor2   g25(.a(new_n57_), .b(new_n69_), .O(z08));
  inv1   g26(.a(new_n50_), .O(new_n71_));
  nand4  g27(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(new_n73_));
  nand3  g29(.a(x22), .b(x21), .c(x20), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n73_), .c(x11), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n71_), .O(new_n78_));
  oai21  g34(.a(new_n57_), .b(new_n52_), .c(new_n78_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  and2   g36(.a(x22), .b(x21), .O(new_n81_));
  nand3  g37(.a(new_n73_), .b(new_n81_), .c(x12), .O(new_n82_));
  and2   g38(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g39(.a(x20), .b(x19), .c(new_n71_), .O(new_n84_));
  oai22  g40(.a(new_n84_), .b(new_n83_), .c(new_n57_), .d(new_n56_), .O(z10));
  nand3  g41(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  inv1   g42(.a(new_n86_), .O(new_n87_));
  nand2  g43(.a(x22), .b(x13), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n72_), .c(new_n87_), .O(new_n89_));
  nor2   g45(.a(new_n80_), .b(x21), .O(new_n90_));
  nor2   g46(.a(new_n90_), .b(x08), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n53_), .O(new_n92_));
  oai22  g48(.a(new_n92_), .b(x10), .c(new_n48_), .d(new_n59_), .O(z11));
  nand2  g49(.a(new_n73_), .b(x14), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n87_), .c(x22), .O(new_n95_));
  inv1   g51(.a(x22), .O(new_n96_));
  aoi21  g52(.a(new_n86_), .b(new_n96_), .c(x08), .O(new_n97_));
  aoi21  g53(.a(new_n97_), .b(new_n95_), .c(new_n53_), .O(new_n98_));
  oai22  g54(.a(new_n98_), .b(x10), .c(new_n48_), .d(new_n61_), .O(z12));
  inv1   g55(.a(new_n46_), .O(new_n100_));
  inv1   g56(.a(new_n47_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n100_), .c(x23), .O(new_n103_));
  aoi21  g59(.a(new_n46_), .b(new_n45_), .c(new_n50_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  oai21  g61(.a(new_n57_), .b(new_n63_), .c(new_n105_), .O(z13));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n46_), .b(new_n45_), .c(new_n109_), .O(new_n110_));
  and2   g66(.a(x24), .b(x23), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n100_), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n110_), .c(new_n108_), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(x08), .c(x09), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n49_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n65_), .O(z14));
  nand2  g72(.a(x08), .b(x06), .O(new_n117_));
  nand3  g73(.a(x26), .b(x25), .c(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  nand4  g75(.a(new_n111_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n120_));
  inv1   g76(.a(x25), .O(new_n121_));
  nand2  g77(.a(x24), .b(x23), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n46_), .c(new_n121_), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n120_), .c(new_n119_), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(x08), .c(x09), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n49_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n117_), .O(z15));
  nand2  g83(.a(x08), .b(x07), .O(new_n128_));
  nor2   g84(.a(new_n122_), .b(new_n46_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(x25), .c(x26), .O(new_n130_));
  inv1   g86(.a(x18), .O(new_n131_));
  nand4  g87(.a(new_n75_), .b(new_n73_), .c(x19), .d(new_n131_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n48_), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n130_), .c(x09), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n49_), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n128_), .O(z16));
endmodule


