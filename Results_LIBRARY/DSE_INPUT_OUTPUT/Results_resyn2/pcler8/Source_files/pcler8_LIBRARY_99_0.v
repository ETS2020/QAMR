// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_;
  nand3  g00(.a(x26), .b(x25), .c(x24), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand2  g02(.a(new_n46_), .b(x23), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand4  g04(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n49_));
  inv1   g05(.a(new_n49_), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  inv1   g08(.a(x17), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(x10), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n50_), .c(new_n48_), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z00));
  inv1   g14(.a(x00), .O(new_n59_));
  inv1   g15(.a(x08), .O(new_n60_));
  inv1   g16(.a(x10), .O(new_n61_));
  nand2  g17(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  oai21  g18(.a(new_n60_), .b(new_n59_), .c(new_n62_), .O(z01));
  nand2  g19(.a(x08), .b(x01), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n62_), .O(z02));
  nand2  g21(.a(new_n62_), .b(x08), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(x02), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z03));
  nand2  g25(.a(new_n67_), .b(x03), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z04));
  inv1   g27(.a(x04), .O(new_n72_));
  oai21  g28(.a(new_n60_), .b(new_n72_), .c(new_n62_), .O(z05));
  nand2  g29(.a(x08), .b(x05), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n62_), .O(z06));
  inv1   g31(.a(x06), .O(new_n76_));
  nor2   g32(.a(new_n66_), .b(new_n76_), .O(z07));
  nand2  g33(.a(x08), .b(x07), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n62_), .O(z08));
  nand4  g35(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n47_), .c(x19), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n56_), .O(new_n82_));
  oai21  g38(.a(new_n66_), .b(new_n59_), .c(new_n82_), .O(z09));
  nand2  g39(.a(x20), .b(x19), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n48_), .c(new_n84_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n56_), .O(new_n88_));
  nand2  g44(.a(new_n67_), .b(x01), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n87_), .c(new_n89_), .O(z10));
  inv1   g46(.a(new_n84_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(x21), .O(new_n92_));
  nand3  g48(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  nor2   g49(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nor2   g50(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  oai21  g51(.a(new_n91_), .b(x21), .c(new_n56_), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n95_), .c(new_n68_), .O(z11));
  aoi21  g53(.a(new_n48_), .b(x14), .c(new_n49_), .O(new_n98_));
  inv1   g54(.a(new_n92_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(x22), .c(new_n56_), .O(new_n100_));
  oai21  g56(.a(new_n100_), .b(new_n98_), .c(new_n70_), .O(z12));
  nand2  g57(.a(new_n46_), .b(x15), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(new_n50_), .c(x23), .O(new_n103_));
  inv1   g59(.a(x23), .O(new_n104_));
  nand2  g60(.a(new_n49_), .b(new_n104_), .O(new_n105_));
  nand3  g61(.a(new_n105_), .b(new_n103_), .c(new_n56_), .O(new_n106_));
  oai21  g62(.a(new_n66_), .b(new_n72_), .c(new_n106_), .O(z13));
  nand2  g63(.a(new_n46_), .b(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  nand2  g65(.a(x24), .b(x23), .O(new_n110_));
  nor2   g66(.a(new_n110_), .b(new_n49_), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand2  g69(.a(new_n50_), .b(x23), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n112_), .c(new_n109_), .O(new_n116_));
  nand2  g72(.a(new_n67_), .b(x05), .O(new_n117_));
  oai21  g73(.a(new_n116_), .b(new_n55_), .c(new_n117_), .O(z14));
  nand2  g74(.a(x08), .b(x06), .O(new_n119_));
  nand2  g75(.a(x25), .b(x24), .O(new_n120_));
  nor2   g76(.a(new_n120_), .b(x26), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n50_), .c(x23), .O(new_n122_));
  inv1   g78(.a(x25), .O(new_n123_));
  oai21  g79(.a(new_n110_), .b(new_n49_), .c(new_n123_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n122_), .c(new_n52_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(x17), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n61_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n119_), .O(z15));
  inv1   g84(.a(new_n122_), .O(new_n129_));
  inv1   g85(.a(x26), .O(new_n130_));
  nor2   g86(.a(new_n123_), .b(x18), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n111_), .c(new_n130_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n129_), .c(new_n56_), .O(new_n133_));
  nand2  g89(.a(new_n67_), .b(x07), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n133_), .O(z16));
endmodule


