// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(new_n48_), .c(x24), .d(x09), .O(new_n50_));
  aoi21  g06(.a(new_n50_), .b(new_n45_), .c(x08), .O(z00));
  and2   g07(.a(x08), .b(x00), .O(z01));
  nor2   g08(.a(new_n45_), .b(x08), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand2  g10(.a(x08), .b(x01), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n54_), .O(z02));
  nand2  g12(.a(x08), .b(x02), .O(new_n57_));
  inv1   g13(.a(new_n57_), .O(z03));
  nand2  g14(.a(x08), .b(x03), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z04));
  nand2  g16(.a(x08), .b(x04), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n54_), .O(z05));
  and2   g18(.a(x08), .b(x05), .O(z06));
  nand2  g19(.a(x08), .b(x06), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n54_), .O(z07));
  aoi21  g21(.a(x08), .b(x07), .c(new_n53_), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z08));
  inv1   g23(.a(x09), .O(new_n68_));
  nor2   g24(.a(new_n68_), .b(x08), .O(new_n69_));
  nand4  g25(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n70_));
  and2   g26(.a(x22), .b(x21), .O(new_n71_));
  nand3  g27(.a(new_n71_), .b(x20), .c(x11), .O(new_n72_));
  oai21  g28(.a(new_n72_), .b(new_n70_), .c(x19), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nor2   g30(.a(new_n53_), .b(z01), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n74_), .O(z09));
  and2   g32(.a(x20), .b(x19), .O(new_n77_));
  inv1   g33(.a(new_n70_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n71_), .c(x12), .O(new_n79_));
  and2   g35(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand2  g36(.a(new_n69_), .b(new_n45_), .O(new_n81_));
  inv1   g37(.a(new_n81_), .O(new_n82_));
  oai21  g38(.a(x20), .b(x19), .c(new_n82_), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n80_), .c(new_n55_), .O(z10));
  nand2  g40(.a(new_n77_), .b(x21), .O(new_n85_));
  and2   g41(.a(x22), .b(x13), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n78_), .c(new_n85_), .O(new_n87_));
  oai21  g43(.a(new_n77_), .b(x21), .c(new_n82_), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n87_), .c(new_n57_), .O(z11));
  aoi21  g45(.a(new_n78_), .b(x14), .c(new_n47_), .O(new_n90_));
  inv1   g46(.a(new_n85_), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x22), .c(new_n82_), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n90_), .c(new_n59_), .O(z12));
  xor2a  g49(.a(new_n47_), .b(new_n46_), .O(new_n94_));
  aoi21  g50(.a(new_n78_), .b(x15), .c(new_n94_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n81_), .c(new_n61_), .O(z13));
  inv1   g52(.a(new_n69_), .O(new_n97_));
  nand4  g53(.a(new_n71_), .b(new_n77_), .c(x24), .d(x23), .O(new_n98_));
  nand3  g54(.a(new_n49_), .b(x24), .c(x16), .O(new_n99_));
  inv1   g55(.a(new_n99_), .O(new_n100_));
  inv1   g56(.a(x24), .O(new_n101_));
  oai21  g57(.a(new_n47_), .b(new_n46_), .c(new_n101_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n98_), .c(new_n100_), .O(new_n103_));
  nor2   g59(.a(z06), .b(new_n53_), .O(new_n104_));
  oai21  g60(.a(new_n103_), .b(new_n97_), .c(new_n104_), .O(z14));
  nand2  g61(.a(new_n49_), .b(x17), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  nand3  g63(.a(new_n48_), .b(x25), .c(x24), .O(new_n108_));
  inv1   g64(.a(x25), .O(new_n109_));
  nand2  g65(.a(new_n98_), .b(new_n109_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n108_), .c(new_n107_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n81_), .c(new_n64_), .O(z15));
  inv1   g68(.a(new_n108_), .O(new_n113_));
  nor2   g69(.a(new_n47_), .b(x18), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n78_), .c(new_n97_), .O(new_n115_));
  oai21  g71(.a(new_n113_), .b(x26), .c(new_n115_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n66_), .O(z16));
endmodule


