// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  inv1   g00(.a(x11), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x01), .O(new_n46_));
  inv1   g02(.a(x23), .O(new_n47_));
  nand4  g03(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n48_));
  nor2   g04(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  inv1   g08(.a(x10), .O(new_n53_));
  nand3  g09(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand3  g11(.a(new_n55_), .b(new_n51_), .c(new_n49_), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n46_), .O(z00));
  nand2  g13(.a(x08), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n46_), .O(z01));
  inv1   g15(.a(x01), .O(new_n60_));
  aoi21  g16(.a(x11), .b(new_n52_), .c(new_n60_), .O(z02));
  nand2  g17(.a(x08), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n46_), .O(z03));
  nand3  g19(.a(new_n46_), .b(x08), .c(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  nand2  g21(.a(x08), .b(x04), .O(new_n66_));
  nand2  g22(.a(new_n66_), .b(new_n46_), .O(z05));
  nand3  g23(.a(new_n46_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z07));
  nor2   g27(.a(x11), .b(new_n60_), .O(new_n72_));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  nor2   g29(.a(new_n73_), .b(new_n72_), .O(z08));
  nand4  g30(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n55_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n58_), .c(new_n46_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n75_), .O(new_n83_));
  nor2   g39(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  or2    g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n54_), .b(new_n72_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g43(.a(x11), .b(x08), .c(x01), .O(new_n88_));
  oai21  g44(.a(new_n87_), .b(new_n84_), .c(new_n88_), .O(z10));
  nand3  g45(.a(new_n46_), .b(x08), .c(x02), .O(new_n90_));
  inv1   g46(.a(new_n75_), .O(new_n91_));
  nand3  g47(.a(x21), .b(x20), .c(x19), .O(new_n92_));
  and2   g48(.a(x22), .b(x13), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  nand2  g51(.a(new_n81_), .b(new_n95_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n86_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n94_), .c(new_n90_), .O(z11));
  inv1   g54(.a(new_n92_), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(x22), .O(new_n100_));
  aoi21  g56(.a(new_n91_), .b(x14), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  nand2  g58(.a(new_n92_), .b(new_n102_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n101_), .c(new_n64_), .O(z12));
  nand2  g61(.a(new_n51_), .b(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n49_), .O(new_n107_));
  aoi21  g63(.a(new_n48_), .b(new_n47_), .c(new_n54_), .O(new_n108_));
  nand2  g64(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n66_), .c(new_n46_), .O(z13));
  inv1   g66(.a(new_n86_), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n112_));
  inv1   g68(.a(new_n112_), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  oai21  g70(.a(new_n48_), .b(new_n47_), .c(new_n114_), .O(new_n115_));
  inv1   g71(.a(new_n48_), .O(new_n116_));
  and2   g72(.a(x24), .b(x23), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g74(.a(new_n118_), .b(new_n115_), .c(new_n113_), .O(new_n119_));
  oai21  g75(.a(new_n119_), .b(new_n111_), .c(new_n68_), .O(z14));
  inv1   g76(.a(z07), .O(new_n121_));
  nand3  g77(.a(x26), .b(x25), .c(x17), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(new_n123_));
  and2   g79(.a(x22), .b(x21), .O(new_n124_));
  nand4  g80(.a(new_n117_), .b(new_n124_), .c(new_n80_), .d(x25), .O(new_n125_));
  inv1   g81(.a(x25), .O(new_n126_));
  nand2  g82(.a(x24), .b(x23), .O(new_n127_));
  oai21  g83(.a(new_n127_), .b(new_n48_), .c(new_n126_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n125_), .c(new_n123_), .O(new_n129_));
  oai21  g85(.a(new_n129_), .b(new_n54_), .c(new_n121_), .O(z15));
  nor2   g86(.a(new_n127_), .b(new_n48_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(x25), .c(x26), .O(new_n132_));
  inv1   g88(.a(x18), .O(new_n133_));
  nand3  g89(.a(new_n91_), .b(new_n116_), .c(new_n133_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n55_), .O(new_n135_));
  and2   g91(.a(new_n73_), .b(new_n46_), .O(new_n136_));
  oai21  g92(.a(new_n135_), .b(new_n132_), .c(new_n136_), .O(z16));
endmodule


