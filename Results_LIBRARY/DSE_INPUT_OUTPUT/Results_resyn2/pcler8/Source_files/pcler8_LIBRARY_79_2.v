// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x06), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(x08), .O(new_n50_));
  nand2  g06(.a(x09), .b(new_n50_), .O(new_n51_));
  inv1   g07(.a(new_n51_), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(new_n49_), .c(new_n48_), .d(x24), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(new_n45_), .c(x10), .O(z00));
  nor2   g10(.a(x10), .b(new_n45_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nor2   g12(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g13(.a(new_n55_), .O(new_n58_));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nand2  g15(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g16(.a(x08), .b(x02), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n58_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(new_n55_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n58_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n58_), .O(z06));
  aoi21  g24(.a(x10), .b(new_n50_), .c(new_n45_), .O(z07));
  nand3  g25(.a(new_n58_), .b(x08), .c(x07), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z08));
  nand3  g27(.a(new_n49_), .b(x24), .c(x23), .O(new_n72_));
  and2   g28(.a(x22), .b(x21), .O(new_n73_));
  nand3  g29(.a(new_n73_), .b(x20), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n72_), .c(x19), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(new_n52_), .c(x06), .O(new_n76_));
  oai21  g32(.a(new_n76_), .b(x10), .c(new_n56_), .O(z09));
  and2   g33(.a(x20), .b(x19), .O(new_n78_));
  nand2  g34(.a(new_n73_), .b(x12), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n72_), .c(new_n78_), .O(new_n80_));
  nor2   g36(.a(x20), .b(x19), .O(new_n81_));
  nor2   g37(.a(new_n81_), .b(new_n51_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(new_n80_), .c(x06), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(x10), .c(new_n59_), .O(z10));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n85_));
  inv1   g41(.a(new_n85_), .O(new_n86_));
  nand2  g42(.a(x22), .b(x13), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n72_), .c(new_n86_), .O(new_n88_));
  nor2   g44(.a(new_n78_), .b(x21), .O(new_n89_));
  nor2   g45(.a(new_n89_), .b(new_n51_), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(x06), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x10), .c(new_n61_), .O(z11));
  inv1   g48(.a(x14), .O(new_n93_));
  nand4  g49(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n94_));
  inv1   g50(.a(new_n94_), .O(new_n95_));
  oai21  g51(.a(new_n72_), .b(new_n93_), .c(new_n95_), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  aoi21  g53(.a(new_n85_), .b(new_n97_), .c(new_n51_), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n96_), .c(x06), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(x10), .c(new_n63_), .O(z12));
  nand3  g56(.a(new_n49_), .b(x24), .c(x15), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n48_), .O(new_n102_));
  aoi21  g58(.a(new_n47_), .b(new_n46_), .c(new_n51_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n102_), .c(x06), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(x10), .c(new_n65_), .O(z13));
  inv1   g61(.a(x10), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n107_));
  inv1   g63(.a(new_n107_), .O(new_n108_));
  inv1   g64(.a(x24), .O(new_n109_));
  oai21  g65(.a(new_n47_), .b(new_n46_), .c(new_n109_), .O(new_n110_));
  nand4  g66(.a(new_n73_), .b(new_n78_), .c(x24), .d(x23), .O(new_n111_));
  aoi21  g67(.a(new_n111_), .b(new_n110_), .c(new_n108_), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n51_), .c(new_n45_), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n106_), .O(new_n114_));
  nand2  g70(.a(new_n114_), .b(new_n67_), .O(z14));
  nand3  g71(.a(new_n52_), .b(new_n106_), .c(new_n45_), .O(new_n116_));
  nand3  g72(.a(new_n48_), .b(x25), .c(x24), .O(new_n117_));
  inv1   g73(.a(x25), .O(new_n118_));
  nand2  g74(.a(new_n111_), .b(new_n118_), .O(new_n119_));
  aoi22  g75(.a(new_n119_), .b(new_n117_), .c(new_n49_), .d(x17), .O(new_n120_));
  nand3  g76(.a(x10), .b(x08), .c(x06), .O(new_n121_));
  oai21  g77(.a(new_n120_), .b(new_n116_), .c(new_n121_), .O(z15));
  inv1   g78(.a(new_n117_), .O(new_n123_));
  inv1   g79(.a(new_n72_), .O(new_n124_));
  nor2   g80(.a(new_n47_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n124_), .c(new_n116_), .O(new_n126_));
  oai21  g82(.a(new_n123_), .b(x26), .c(new_n126_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n70_), .O(z16));
endmodule


