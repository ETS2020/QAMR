// Benchmark "FAU" written by ABC on Tue Aug 11 20:08:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  inv1   g04(.a(x26), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(new_n50_), .c(new_n47_), .d(x24), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(x03), .c(x10), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x03), .O(new_n56_));
  inv1   g12(.a(x10), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x08), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(new_n55_), .O(z01));
  inv1   g16(.a(x01), .O(new_n61_));
  nor2   g17(.a(new_n59_), .b(new_n61_), .O(z02));
  nand2  g18(.a(x08), .b(x02), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n58_), .O(z03));
  inv1   g20(.a(x08), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n56_), .O(z04));
  inv1   g22(.a(x04), .O(new_n67_));
  nor2   g23(.a(new_n59_), .b(new_n67_), .O(z05));
  inv1   g24(.a(new_n59_), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(x05), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z06));
  nand2  g27(.a(new_n69_), .b(x06), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z07));
  nand2  g29(.a(new_n69_), .b(x07), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(z08));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  and2   g32(.a(x22), .b(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n76_), .c(x19), .O(new_n79_));
  nand3  g35(.a(new_n52_), .b(new_n57_), .c(x03), .O(new_n80_));
  inv1   g36(.a(new_n80_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g38(.a(new_n59_), .b(new_n55_), .c(new_n82_), .O(z09));
  and2   g39(.a(x20), .b(x19), .O(new_n84_));
  nand3  g40(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n76_), .c(new_n84_), .O(new_n86_));
  nand2  g42(.a(x09), .b(new_n65_), .O(new_n87_));
  nor2   g43(.a(x20), .b(x19), .O(new_n88_));
  nor2   g44(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  aoi21  g45(.a(new_n89_), .b(new_n86_), .c(new_n56_), .O(new_n90_));
  oai22  g46(.a(new_n90_), .b(x10), .c(new_n65_), .d(new_n61_), .O(z10));
  inv1   g47(.a(new_n76_), .O(new_n92_));
  nand3  g48(.a(x21), .b(x20), .c(x19), .O(new_n93_));
  and2   g49(.a(x22), .b(x13), .O(new_n94_));
  aoi21  g50(.a(new_n94_), .b(new_n92_), .c(new_n93_), .O(new_n95_));
  oai21  g51(.a(new_n84_), .b(x21), .c(new_n81_), .O(new_n96_));
  nand2  g52(.a(new_n69_), .b(x02), .O(new_n97_));
  oai21  g53(.a(new_n96_), .b(new_n95_), .c(new_n97_), .O(z11));
  inv1   g54(.a(x14), .O(new_n99_));
  inv1   g55(.a(new_n46_), .O(new_n100_));
  oai21  g56(.a(new_n76_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  inv1   g57(.a(x22), .O(new_n102_));
  aoi21  g58(.a(new_n93_), .b(new_n102_), .c(new_n51_), .O(new_n103_));
  aoi21  g59(.a(new_n103_), .b(new_n101_), .c(new_n56_), .O(new_n104_));
  oai22  g60(.a(new_n104_), .b(x10), .c(new_n65_), .d(new_n56_), .O(z12));
  nand4  g61(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  aoi21  g63(.a(new_n46_), .b(new_n45_), .c(new_n87_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n107_), .c(new_n56_), .O(new_n109_));
  oai22  g65(.a(new_n109_), .b(x10), .c(new_n65_), .d(new_n67_), .O(z13));
  nand3  g66(.a(new_n50_), .b(x24), .c(x16), .O(new_n111_));
  inv1   g67(.a(new_n111_), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  oai21  g69(.a(new_n46_), .b(new_n45_), .c(new_n113_), .O(new_n114_));
  nand4  g70(.a(new_n77_), .b(new_n84_), .c(x24), .d(x23), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n114_), .c(new_n112_), .O(new_n116_));
  oai21  g72(.a(new_n116_), .b(new_n80_), .c(new_n70_), .O(z14));
  nand2  g73(.a(new_n50_), .b(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  nand3  g75(.a(new_n47_), .b(x25), .c(x24), .O(new_n120_));
  nand2  g76(.a(new_n115_), .b(new_n48_), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n119_), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n80_), .c(new_n72_), .O(z15));
  nand2  g79(.a(new_n120_), .b(new_n49_), .O(new_n124_));
  nor2   g80(.a(new_n46_), .b(x18), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n92_), .c(new_n80_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n74_), .O(z16));
endmodule


