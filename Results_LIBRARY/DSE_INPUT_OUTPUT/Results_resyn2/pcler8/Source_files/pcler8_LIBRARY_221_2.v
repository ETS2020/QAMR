// Benchmark "FAU" written by ABC on Tue Aug 11 20:10:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nand3  g02(.a(x26), .b(x25), .c(x24), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor4   g06(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(z00));
  inv1   g07(.a(x00), .O(new_n52_));
  inv1   g08(.a(x16), .O(new_n53_));
  nand2  g09(.a(new_n53_), .b(x10), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(x08), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n52_), .O(z01));
  inv1   g12(.a(x01), .O(new_n57_));
  nor2   g13(.a(new_n55_), .b(new_n57_), .O(z02));
  inv1   g14(.a(x02), .O(new_n59_));
  nor2   g15(.a(new_n55_), .b(new_n59_), .O(z03));
  nand2  g16(.a(x08), .b(x03), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n54_), .O(z04));
  inv1   g18(.a(x04), .O(new_n63_));
  nor2   g19(.a(new_n55_), .b(new_n63_), .O(z05));
  inv1   g20(.a(new_n55_), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(x05), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z06));
  nand2  g23(.a(x08), .b(x06), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n54_), .O(z07));
  inv1   g25(.a(x07), .O(new_n70_));
  nor2   g26(.a(new_n55_), .b(new_n70_), .O(z08));
  inv1   g27(.a(new_n50_), .O(new_n72_));
  nand4  g28(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n73_));
  nand4  g29(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(new_n73_), .c(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(new_n72_), .O(new_n76_));
  nor2   g32(.a(x16), .b(new_n49_), .O(new_n77_));
  aoi21  g33(.a(x08), .b(x00), .c(new_n77_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(new_n76_), .O(z09));
  and2   g35(.a(x20), .b(x19), .O(new_n80_));
  and2   g36(.a(x22), .b(x21), .O(new_n81_));
  inv1   g37(.a(new_n73_), .O(new_n82_));
  nand3  g38(.a(new_n82_), .b(new_n81_), .c(x12), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nor2   g40(.a(x20), .b(x19), .O(new_n85_));
  nor2   g41(.a(new_n85_), .b(new_n50_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n84_), .O(new_n87_));
  aoi21  g43(.a(x08), .b(x01), .c(new_n77_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n87_), .O(z10));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n82_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n80_), .b(x21), .c(new_n72_), .O(new_n93_));
  oai22  g49(.a(new_n93_), .b(new_n92_), .c(new_n55_), .d(new_n59_), .O(z11));
  inv1   g50(.a(z04), .O(new_n95_));
  inv1   g51(.a(new_n90_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(x22), .O(new_n97_));
  aoi21  g53(.a(new_n82_), .b(x14), .c(new_n97_), .O(new_n98_));
  oai21  g54(.a(new_n96_), .b(x22), .c(new_n72_), .O(new_n99_));
  oai21  g55(.a(new_n99_), .b(new_n98_), .c(new_n95_), .O(z12));
  inv1   g56(.a(new_n46_), .O(new_n101_));
  inv1   g57(.a(new_n47_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(new_n101_), .c(x23), .O(new_n104_));
  aoi21  g60(.a(new_n46_), .b(new_n45_), .c(new_n50_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g62(.a(x08), .b(x04), .c(new_n77_), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n46_), .b(new_n45_), .c(new_n111_), .O(new_n112_));
  and2   g68(.a(x24), .b(x23), .O(new_n113_));
  nand2  g69(.a(new_n113_), .b(new_n101_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n110_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(new_n50_), .c(new_n66_), .O(z14));
  nand3  g72(.a(x26), .b(x25), .c(x17), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand4  g74(.a(new_n113_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n119_));
  inv1   g75(.a(x25), .O(new_n120_));
  nand2  g76(.a(x24), .b(x23), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n46_), .c(new_n120_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n119_), .c(new_n118_), .O(new_n123_));
  nand2  g79(.a(new_n65_), .b(x06), .O(new_n124_));
  oai21  g80(.a(new_n123_), .b(new_n50_), .c(new_n124_), .O(z15));
  nor2   g81(.a(new_n121_), .b(new_n46_), .O(new_n126_));
  aoi21  g82(.a(new_n126_), .b(x25), .c(x26), .O(new_n127_));
  inv1   g83(.a(x18), .O(new_n128_));
  nand3  g84(.a(x22), .b(x21), .c(x20), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(new_n130_));
  nand4  g86(.a(new_n130_), .b(new_n82_), .c(x19), .d(new_n128_), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n72_), .O(new_n132_));
  aoi21  g88(.a(x08), .b(x07), .c(new_n77_), .O(new_n133_));
  oai21  g89(.a(new_n132_), .b(new_n127_), .c(new_n133_), .O(z16));
endmodule


