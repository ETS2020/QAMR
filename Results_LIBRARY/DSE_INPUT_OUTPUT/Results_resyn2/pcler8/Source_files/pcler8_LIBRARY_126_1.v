// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_;
  inv1   g00(.a(x11), .O(new_n45_));
  inv1   g01(.a(x23), .O(new_n46_));
  nand4  g02(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n47_));
  nor2   g03(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  and2   g04(.a(x26), .b(x25), .O(new_n49_));
  inv1   g05(.a(x09), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(x08), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(x24), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g09(.a(x10), .O(new_n54_));
  nand2  g10(.a(x11), .b(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nand2  g13(.a(x08), .b(x01), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n55_), .O(z02));
  nand2  g15(.a(x08), .b(x02), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n55_), .O(z03));
  nand3  g17(.a(new_n55_), .b(x08), .c(x03), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z04));
  inv1   g19(.a(new_n55_), .O(new_n64_));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n64_), .O(z05));
  nand2  g22(.a(x08), .b(x05), .O(new_n67_));
  nor2   g23(.a(new_n67_), .b(new_n64_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n55_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  aoi21  g29(.a(new_n51_), .b(new_n73_), .c(x11), .O(new_n74_));
  oai21  g30(.a(new_n74_), .b(x10), .c(new_n56_), .O(z09));
  and2   g31(.a(x20), .b(x19), .O(new_n76_));
  nand4  g32(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(new_n49_), .c(x12), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n76_), .O(new_n80_));
  inv1   g36(.a(new_n51_), .O(new_n81_));
  nor2   g37(.a(x20), .b(x19), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g39(.a(new_n83_), .b(new_n80_), .c(x11), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(x10), .c(new_n58_), .O(z10));
  nand2  g41(.a(new_n49_), .b(x24), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  inv1   g43(.a(new_n87_), .O(new_n88_));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n86_), .c(new_n88_), .O(new_n90_));
  nor2   g46(.a(new_n76_), .b(x21), .O(new_n91_));
  nor2   g47(.a(new_n91_), .b(new_n81_), .O(new_n92_));
  aoi21  g48(.a(new_n92_), .b(new_n90_), .c(x11), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(x10), .c(new_n60_), .O(z11));
  and2   g50(.a(x24), .b(x23), .O(new_n95_));
  nand2  g51(.a(new_n49_), .b(x14), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(new_n97_));
  nand2  g53(.a(new_n88_), .b(x22), .O(new_n98_));
  aoi21  g54(.a(new_n97_), .b(new_n95_), .c(new_n98_), .O(new_n99_));
  inv1   g55(.a(x22), .O(new_n100_));
  nand2  g56(.a(new_n87_), .b(new_n100_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n51_), .c(new_n45_), .d(new_n54_), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n99_), .c(new_n62_), .O(z12));
  inv1   g59(.a(x15), .O(new_n104_));
  oai21  g60(.a(new_n86_), .b(new_n104_), .c(new_n48_), .O(new_n105_));
  aoi21  g61(.a(new_n47_), .b(new_n46_), .c(new_n81_), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n105_), .c(x11), .O(new_n107_));
  oai21  g63(.a(new_n107_), .b(x10), .c(new_n65_), .O(z13));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n109_));
  inv1   g65(.a(new_n109_), .O(new_n110_));
  inv1   g66(.a(x24), .O(new_n111_));
  oai21  g67(.a(new_n47_), .b(new_n46_), .c(new_n111_), .O(new_n112_));
  nand2  g68(.a(new_n78_), .b(new_n76_), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n112_), .c(new_n110_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n81_), .c(new_n45_), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n54_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n67_), .O(z14));
  nand3  g73(.a(x26), .b(x25), .c(x17), .O(new_n118_));
  inv1   g74(.a(new_n118_), .O(new_n119_));
  and2   g75(.a(x22), .b(x21), .O(new_n120_));
  nand4  g76(.a(new_n95_), .b(new_n120_), .c(new_n76_), .d(x25), .O(new_n121_));
  inv1   g77(.a(x25), .O(new_n122_));
  nand2  g78(.a(x20), .b(x19), .O(new_n123_));
  oai21  g79(.a(new_n77_), .b(new_n123_), .c(new_n122_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n121_), .c(new_n119_), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(new_n81_), .c(new_n45_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n54_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n69_), .O(z15));
  nor2   g84(.a(new_n77_), .b(new_n123_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(x25), .c(x26), .O(new_n130_));
  nor2   g86(.a(new_n100_), .b(x18), .O(new_n131_));
  nand4  g87(.a(new_n131_), .b(new_n88_), .c(new_n95_), .d(new_n49_), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n51_), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n130_), .c(new_n45_), .O(new_n134_));
  nand2  g90(.a(new_n134_), .b(new_n54_), .O(new_n135_));
  nand2  g91(.a(new_n135_), .b(new_n71_), .O(z16));
endmodule


