// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  nand4  g00(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n45_));
  inv1   g01(.a(new_n45_), .O(new_n46_));
  nand2  g02(.a(new_n46_), .b(x23), .O(new_n47_));
  inv1   g03(.a(x24), .O(new_n48_));
  inv1   g04(.a(x25), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x08), .O(new_n51_));
  inv1   g07(.a(x10), .O(new_n52_));
  nand3  g08(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n50_), .c(x26), .O(new_n55_));
  nor2   g11(.a(new_n55_), .b(new_n47_), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  nor2   g13(.a(x21), .b(new_n52_), .O(new_n58_));
  inv1   g14(.a(new_n58_), .O(new_n59_));
  oai21  g15(.a(new_n51_), .b(new_n57_), .c(new_n59_), .O(z01));
  nor2   g16(.a(new_n58_), .b(new_n51_), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z02));
  inv1   g19(.a(x02), .O(new_n64_));
  nand2  g20(.a(new_n59_), .b(x08), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n64_), .O(z03));
  nand2  g22(.a(x08), .b(x03), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n59_), .O(z04));
  nand2  g24(.a(new_n61_), .b(x04), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z05));
  nand2  g26(.a(new_n61_), .b(x05), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z06));
  nand2  g28(.a(x08), .b(x06), .O(new_n73_));
  nand2  g29(.a(new_n73_), .b(new_n59_), .O(z07));
  inv1   g30(.a(x07), .O(new_n75_));
  oai21  g31(.a(new_n51_), .b(new_n75_), .c(new_n59_), .O(z08));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  and2   g36(.a(x20), .b(x11), .O(new_n81_));
  aoi21  g37(.a(new_n81_), .b(new_n80_), .c(new_n77_), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(new_n53_), .c(new_n65_), .d(new_n57_), .O(z09));
  and2   g39(.a(x20), .b(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n80_), .b(x12), .c(new_n85_), .O(new_n86_));
  oai21  g42(.a(x20), .b(x19), .c(new_n54_), .O(new_n87_));
  oai21  g43(.a(new_n87_), .b(new_n86_), .c(new_n62_), .O(z10));
  inv1   g44(.a(new_n79_), .O(new_n89_));
  nand3  g45(.a(x21), .b(x20), .c(x19), .O(new_n90_));
  and2   g46(.a(x22), .b(x13), .O(new_n91_));
  aoi21  g47(.a(new_n91_), .b(new_n89_), .c(new_n90_), .O(new_n92_));
  oai21  g48(.a(new_n84_), .b(x21), .c(new_n54_), .O(new_n93_));
  aoi21  g49(.a(x08), .b(x02), .c(new_n58_), .O(new_n94_));
  oai21  g50(.a(new_n93_), .b(new_n92_), .c(new_n94_), .O(z11));
  aoi21  g51(.a(new_n89_), .b(x14), .c(new_n45_), .O(new_n96_));
  inv1   g52(.a(new_n90_), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x22), .c(new_n54_), .O(new_n98_));
  nand2  g54(.a(new_n61_), .b(x03), .O(new_n99_));
  oai21  g55(.a(new_n98_), .b(new_n96_), .c(new_n99_), .O(z12));
  inv1   g56(.a(x15), .O(new_n101_));
  nor2   g57(.a(new_n79_), .b(new_n101_), .O(new_n102_));
  inv1   g58(.a(x23), .O(new_n103_));
  xor2a  g59(.a(new_n45_), .b(new_n103_), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n102_), .c(new_n54_), .O(new_n105_));
  nand2  g61(.a(new_n105_), .b(new_n69_), .O(z13));
  nor2   g62(.a(new_n45_), .b(new_n103_), .O(new_n107_));
  nand3  g63(.a(x26), .b(x25), .c(x16), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(new_n107_), .c(x24), .O(new_n109_));
  nand2  g65(.a(new_n47_), .b(new_n48_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n109_), .c(new_n54_), .O(new_n111_));
  nand2  g67(.a(new_n111_), .b(new_n71_), .O(z14));
  aoi21  g68(.a(x26), .b(x17), .c(new_n49_), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(new_n107_), .c(x24), .O(new_n114_));
  and2   g70(.a(x22), .b(x21), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n84_), .c(x24), .d(x23), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n49_), .O(new_n117_));
  nand3  g73(.a(new_n117_), .b(new_n114_), .c(new_n54_), .O(new_n118_));
  nand2  g74(.a(new_n61_), .b(x06), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n118_), .O(z15));
  aoi21  g76(.a(new_n50_), .b(new_n107_), .c(x26), .O(new_n121_));
  inv1   g77(.a(x18), .O(new_n122_));
  nand3  g78(.a(new_n89_), .b(new_n46_), .c(new_n122_), .O(new_n123_));
  nand2  g79(.a(new_n123_), .b(new_n54_), .O(new_n124_));
  oai22  g80(.a(new_n124_), .b(new_n121_), .c(new_n65_), .d(new_n75_), .O(z16));
endmodule


