// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_;
  inv1   g00(.a(x26), .O(new_n45_));
  nand2  g01(.a(new_n45_), .b(x14), .O(new_n46_));
  nand2  g02(.a(x20), .b(x19), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand3  g05(.a(new_n49_), .b(x09), .c(new_n48_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand3  g07(.a(x23), .b(x22), .c(x21), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(new_n46_), .O(z01));
  inv1   g14(.a(new_n46_), .O(new_n59_));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n59_), .O(z02));
  nand3  g17(.a(new_n46_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(x08), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n46_), .O(z04));
  nand3  g21(.a(new_n46_), .b(x08), .c(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand3  g23(.a(new_n46_), .b(x08), .c(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(x08), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n46_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  oai21  g28(.a(new_n48_), .b(new_n72_), .c(new_n46_), .O(z08));
  oai21  g29(.a(new_n50_), .b(x19), .c(new_n57_), .O(new_n74_));
  nand2  g30(.a(new_n74_), .b(new_n46_), .O(new_n75_));
  inv1   g31(.a(new_n50_), .O(new_n76_));
  nand4  g32(.a(new_n54_), .b(new_n76_), .c(x20), .d(x11), .O(new_n77_));
  nand2  g33(.a(new_n77_), .b(new_n75_), .O(z09));
  aoi21  g34(.a(new_n54_), .b(x12), .c(new_n47_), .O(new_n79_));
  oai21  g35(.a(x20), .b(x19), .c(new_n76_), .O(new_n80_));
  and2   g36(.a(new_n60_), .b(new_n46_), .O(new_n81_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n81_), .O(z10));
  inv1   g38(.a(x22), .O(new_n83_));
  inv1   g39(.a(x23), .O(new_n84_));
  nor3   g40(.a(new_n53_), .b(new_n84_), .c(new_n83_), .O(new_n85_));
  nand4  g41(.a(new_n85_), .b(new_n76_), .c(x21), .d(x13), .O(new_n86_));
  nand3  g42(.a(x21), .b(x20), .c(x19), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  nand2  g44(.a(new_n47_), .b(new_n88_), .O(new_n89_));
  nand4  g45(.a(new_n89_), .b(new_n87_), .c(new_n76_), .d(new_n46_), .O(new_n90_));
  nand3  g46(.a(new_n90_), .b(new_n86_), .c(new_n62_), .O(z11));
  and2   g47(.a(new_n87_), .b(new_n83_), .O(new_n92_));
  nand4  g48(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n92_), .c(new_n64_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n46_), .O(new_n96_));
  nand3  g52(.a(new_n85_), .b(new_n76_), .c(x14), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n96_), .O(z12));
  inv1   g54(.a(new_n47_), .O(new_n99_));
  inv1   g55(.a(new_n52_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi22  g57(.a(new_n93_), .b(new_n84_), .c(new_n45_), .d(x14), .O(new_n102_));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(new_n93_), .O(new_n104_));
  aoi21  g60(.a(new_n102_), .b(new_n101_), .c(new_n104_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n50_), .c(new_n66_), .O(z13));
  nand3  g62(.a(new_n100_), .b(new_n99_), .c(x24), .O(new_n107_));
  inv1   g63(.a(x24), .O(new_n108_));
  nand2  g64(.a(new_n101_), .b(new_n108_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n107_), .c(new_n76_), .d(new_n46_), .O(new_n110_));
  inv1   g66(.a(new_n53_), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(new_n76_), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(new_n110_), .c(new_n68_), .O(z14));
  inv1   g69(.a(x25), .O(new_n114_));
  nand2  g70(.a(new_n107_), .b(new_n114_), .O(new_n115_));
  nor2   g71(.a(new_n114_), .b(new_n108_), .O(new_n116_));
  inv1   g72(.a(new_n93_), .O(new_n117_));
  nand2  g73(.a(x26), .b(x17), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n117_), .c(new_n116_), .d(x23), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(new_n115_), .c(new_n76_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n70_), .c(new_n59_), .O(z15));
  nor3   g77(.a(new_n52_), .b(new_n114_), .c(new_n108_), .O(new_n122_));
  aoi21  g78(.a(new_n122_), .b(new_n51_), .c(x14), .O(new_n123_));
  nor2   g79(.a(new_n48_), .b(new_n72_), .O(new_n124_));
  inv1   g80(.a(x18), .O(new_n125_));
  nor2   g81(.a(new_n52_), .b(new_n47_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n116_), .c(new_n125_), .O(new_n127_));
  nand4  g83(.a(x26), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n128_));
  inv1   g84(.a(new_n128_), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n127_), .c(new_n124_), .O(new_n130_));
  oai21  g86(.a(new_n123_), .b(x26), .c(new_n130_), .O(z16));
endmodule


