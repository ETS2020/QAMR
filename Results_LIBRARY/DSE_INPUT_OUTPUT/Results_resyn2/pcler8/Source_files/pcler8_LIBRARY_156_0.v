// Benchmark "FAU" written by ABC on Tue Aug 11 20:09:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n123_, new_n124_, new_n125_, new_n126_;
  inv1   g00(.a(x23), .O(new_n45_));
  nand4  g01(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g03(.a(x25), .O(new_n48_));
  inv1   g04(.a(x26), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  inv1   g06(.a(x09), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(x08), .O(new_n52_));
  nand4  g08(.a(new_n52_), .b(new_n50_), .c(new_n47_), .d(x24), .O(new_n53_));
  aoi21  g09(.a(new_n53_), .b(x11), .c(x10), .O(z00));
  inv1   g10(.a(x00), .O(new_n55_));
  inv1   g11(.a(x08), .O(new_n56_));
  inv1   g12(.a(x10), .O(new_n57_));
  inv1   g13(.a(x11), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  oai21  g15(.a(new_n56_), .b(new_n55_), .c(new_n59_), .O(z01));
  nand2  g16(.a(x08), .b(x01), .O(new_n61_));
  nand2  g17(.a(new_n61_), .b(new_n59_), .O(z02));
  nand2  g18(.a(new_n59_), .b(x08), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(x02), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z03));
  nand2  g22(.a(x08), .b(x03), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n59_), .O(z04));
  inv1   g24(.a(x04), .O(new_n69_));
  nor2   g25(.a(new_n63_), .b(new_n69_), .O(z05));
  nand2  g26(.a(x08), .b(x05), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n59_), .O(z06));
  nand2  g28(.a(new_n64_), .b(x06), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z07));
  nand2  g30(.a(new_n64_), .b(x07), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(z08));
  inv1   g32(.a(x19), .O(new_n77_));
  nand2  g33(.a(x22), .b(x21), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(x20), .c(new_n77_), .O(new_n81_));
  nand3  g37(.a(new_n52_), .b(x11), .c(new_n57_), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(new_n81_), .c(new_n63_), .d(new_n55_), .O(z09));
  and2   g39(.a(x20), .b(x19), .O(new_n84_));
  inv1   g40(.a(new_n84_), .O(new_n85_));
  aoi21  g41(.a(new_n80_), .b(x12), .c(new_n85_), .O(new_n86_));
  inv1   g42(.a(new_n82_), .O(new_n87_));
  oai21  g43(.a(x20), .b(x19), .c(new_n87_), .O(new_n88_));
  nand2  g44(.a(new_n64_), .b(x01), .O(new_n89_));
  oai21  g45(.a(new_n88_), .b(new_n86_), .c(new_n89_), .O(z10));
  inv1   g46(.a(new_n79_), .O(new_n91_));
  nand2  g47(.a(new_n84_), .b(x21), .O(new_n92_));
  and2   g48(.a(x22), .b(x13), .O(new_n93_));
  aoi21  g49(.a(new_n93_), .b(new_n91_), .c(new_n92_), .O(new_n94_));
  oai21  g50(.a(new_n84_), .b(x21), .c(new_n87_), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n94_), .c(new_n65_), .O(z11));
  aoi21  g52(.a(new_n91_), .b(x14), .c(new_n46_), .O(new_n97_));
  inv1   g53(.a(new_n92_), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(x22), .c(new_n87_), .O(new_n99_));
  nand2  g55(.a(new_n64_), .b(x03), .O(new_n100_));
  oai21  g56(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(z12));
  nand4  g57(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n47_), .O(new_n103_));
  nand2  g59(.a(x09), .b(new_n56_), .O(new_n104_));
  aoi21  g60(.a(new_n46_), .b(new_n45_), .c(new_n104_), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n103_), .c(new_n58_), .O(new_n106_));
  oai22  g62(.a(new_n106_), .b(x10), .c(new_n56_), .d(new_n69_), .O(z13));
  nand3  g63(.a(new_n50_), .b(x24), .c(x16), .O(new_n108_));
  inv1   g64(.a(new_n108_), .O(new_n109_));
  inv1   g65(.a(x24), .O(new_n110_));
  oai21  g66(.a(new_n46_), .b(new_n45_), .c(new_n110_), .O(new_n111_));
  and2   g67(.a(x22), .b(x21), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n84_), .c(x24), .d(x23), .O(new_n113_));
  aoi21  g69(.a(new_n113_), .b(new_n111_), .c(new_n109_), .O(new_n114_));
  nand2  g70(.a(new_n64_), .b(x05), .O(new_n115_));
  oai21  g71(.a(new_n114_), .b(new_n82_), .c(new_n115_), .O(z14));
  nand2  g72(.a(new_n50_), .b(x17), .O(new_n117_));
  inv1   g73(.a(new_n117_), .O(new_n118_));
  nand3  g74(.a(new_n47_), .b(x25), .c(x24), .O(new_n119_));
  nand2  g75(.a(new_n113_), .b(new_n48_), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n119_), .c(new_n118_), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n82_), .c(new_n73_), .O(z15));
  nand2  g78(.a(new_n119_), .b(new_n49_), .O(new_n123_));
  inv1   g79(.a(x18), .O(new_n124_));
  nand4  g80(.a(new_n50_), .b(new_n47_), .c(x24), .d(new_n124_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n123_), .c(new_n87_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n75_), .O(z16));
endmodule


