// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  nor2   g04(.a(x04), .b(new_n28_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(x02), .c(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n25_), .O(new_n32_));
  inv1   g08(.a(x04), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n33_), .c(x02), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  nand2  g11(.a(x03), .b(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(x01), .c(new_n27_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n32_), .O(z0));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n25_), .c(x04), .d(x03), .O(new_n41_));
  oai22  g17(.a(new_n41_), .b(new_n35_), .c(new_n36_), .d(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n27_), .O(new_n43_));
  nand2  g19(.a(x10), .b(new_n25_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(z1));
  oai21  g21(.a(x05), .b(new_n33_), .c(x01), .O(new_n46_));
  nand2  g22(.a(x05), .b(x01), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x04), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n29_), .c(new_n25_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(new_n51_));
  nand2  g27(.a(x07), .b(new_n28_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  nand3  g29(.a(new_n26_), .b(x09), .c(new_n53_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n52_), .c(x01), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(x02), .c(new_n55_), .O(new_n56_));
  nor2   g32(.a(x08), .b(x07), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n26_), .c(x09), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n26_), .O(new_n59_));
  nand2  g35(.a(new_n57_), .b(x02), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  aoi21  g37(.a(new_n59_), .b(new_n25_), .c(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n56_), .b(x00), .c(new_n62_), .O(z2));
  nand2  g39(.a(x11), .b(x07), .O(new_n64_));
  aoi21  g40(.a(x12), .b(new_n27_), .c(x10), .O(new_n65_));
  nor2   g41(.a(x12), .b(new_n25_), .O(new_n66_));
  oai21  g42(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nand3  g43(.a(new_n64_), .b(x01), .c(x00), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n67_), .O(z3));
  inv1   g45(.a(x09), .O(new_n70_));
  oai21  g46(.a(x10), .b(new_n70_), .c(new_n25_), .O(new_n71_));
  nand2  g47(.a(x08), .b(x00), .O(new_n72_));
  nand2  g48(.a(x01), .b(x00), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(x10), .c(x09), .O(new_n74_));
  aoi21  g50(.a(new_n72_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  inv1   g51(.a(x11), .O(new_n76_));
  nand3  g52(.a(new_n44_), .b(new_n76_), .c(x07), .O(new_n77_));
  oai21  g53(.a(new_n75_), .b(x07), .c(new_n77_), .O(z4));
  inv1   g54(.a(x12), .O(new_n79_));
  inv1   g55(.a(x13), .O(new_n80_));
  aoi21  g56(.a(new_n53_), .b(x00), .c(new_n80_), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n79_), .c(new_n26_), .d(new_n53_), .O(z5));
  oai21  g58(.a(x09), .b(new_n28_), .c(new_n25_), .O(new_n83_));
  nor2   g59(.a(x02), .b(x00), .O(new_n84_));
  nor2   g60(.a(x08), .b(new_n27_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  oai21  g62(.a(new_n70_), .b(x03), .c(new_n35_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n72_), .c(new_n25_), .O(new_n88_));
  inv1   g64(.a(x14), .O(new_n89_));
  nand3  g65(.a(new_n89_), .b(x01), .c(new_n27_), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(new_n91_));
  nand3  g67(.a(new_n91_), .b(new_n26_), .c(new_n53_), .O(new_n92_));
  inv1   g68(.a(new_n92_), .O(z6));
  oai21  g69(.a(new_n57_), .b(x10), .c(x01), .O(new_n94_));
  nand3  g70(.a(x09), .b(x03), .c(new_n35_), .O(new_n95_));
  aoi21  g71(.a(new_n95_), .b(new_n60_), .c(x01), .O(new_n96_));
  oai21  g72(.a(x08), .b(x03), .c(new_n53_), .O(new_n97_));
  oai21  g73(.a(new_n97_), .b(new_n96_), .c(new_n26_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n94_), .O(z7));
  oai22  g75(.a(x10), .b(x09), .c(x07), .d(new_n25_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n27_), .O(new_n101_));
  nand3  g77(.a(x09), .b(new_n53_), .c(x00), .O(new_n102_));
  nand2  g78(.a(new_n102_), .b(new_n26_), .O(new_n103_));
  nand2  g79(.a(new_n103_), .b(x01), .O(new_n104_));
  nand3  g80(.a(new_n79_), .b(x03), .c(new_n35_), .O(new_n105_));
  nand2  g81(.a(new_n105_), .b(x09), .O(new_n106_));
  nand2  g82(.a(new_n106_), .b(new_n53_), .O(new_n107_));
  nand2  g83(.a(new_n107_), .b(new_n26_), .O(new_n108_));
  nand3  g84(.a(new_n108_), .b(new_n104_), .c(new_n101_), .O(z8));
endmodule


