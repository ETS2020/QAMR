// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  inv1   g11(.a(x08), .O(new_n36_));
  inv1   g12(.a(x10), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n35_), .O(z0));
  oai21  g15(.a(x04), .b(x03), .c(new_n29_), .O(new_n40_));
  nand2  g16(.a(x06), .b(x05), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x04), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n40_), .c(new_n27_), .O(new_n43_));
  nand3  g19(.a(x06), .b(x05), .c(x02), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(x03), .c(x01), .O(new_n45_));
  inv1   g21(.a(new_n45_), .O(new_n46_));
  oai21  g22(.a(new_n46_), .b(new_n43_), .c(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n38_), .O(z1));
  oai21  g24(.a(x03), .b(x01), .c(new_n27_), .O(new_n49_));
  oai21  g25(.a(x08), .b(x07), .c(x00), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  nand2  g27(.a(x07), .b(new_n26_), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  nand3  g29(.a(new_n37_), .b(x09), .c(new_n53_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n52_), .c(x01), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(x02), .c(x08), .O(new_n56_));
  oai21  g32(.a(new_n56_), .b(x00), .c(new_n51_), .O(z2));
  inv1   g33(.a(x11), .O(new_n58_));
  oai22  g34(.a(new_n58_), .b(new_n53_), .c(x10), .d(x08), .O(new_n59_));
  inv1   g35(.a(x12), .O(new_n60_));
  nand2  g36(.a(x11), .b(x07), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n38_), .c(new_n60_), .O(new_n62_));
  oai21  g38(.a(new_n59_), .b(new_n25_), .c(new_n62_), .O(z3));
  aoi21  g39(.a(new_n53_), .b(x00), .c(new_n37_), .O(new_n64_));
  aoi21  g40(.a(x09), .b(x00), .c(x07), .O(new_n65_));
  aoi21  g41(.a(new_n58_), .b(x07), .c(new_n65_), .O(new_n66_));
  oai21  g42(.a(new_n64_), .b(x08), .c(new_n66_), .O(z4));
  oai21  g43(.a(x07), .b(new_n25_), .c(x13), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x08), .O(new_n69_));
  nor2   g45(.a(x10), .b(x07), .O(new_n70_));
  nand4  g46(.a(new_n70_), .b(new_n69_), .c(new_n38_), .d(new_n60_), .O(z5));
  nand2  g47(.a(x14), .b(x02), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(x01), .O(new_n73_));
  inv1   g49(.a(x09), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(x03), .c(new_n27_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n29_), .O(new_n76_));
  nand3  g52(.a(new_n74_), .b(x03), .c(new_n27_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n73_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n53_), .c(new_n25_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(x08), .c(x10), .O(z6));
  nand4  g56(.a(x09), .b(x03), .c(new_n27_), .d(new_n29_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n70_), .c(new_n38_), .O(z7));
  nand4  g58(.a(new_n60_), .b(x03), .c(new_n27_), .d(new_n29_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(x09), .O(new_n84_));
  aoi21  g60(.a(new_n74_), .b(new_n25_), .c(x07), .O(new_n85_));
  nand4  g61(.a(new_n85_), .b(new_n84_), .c(new_n37_), .d(x08), .O(z8));
endmodule


