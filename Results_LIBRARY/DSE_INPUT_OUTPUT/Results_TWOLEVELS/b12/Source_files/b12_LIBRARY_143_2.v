// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  nor2   g03(.a(x02), .b(new_n27_), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n27_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n26_), .O(new_n32_));
  inv1   g08(.a(x02), .O(new_n33_));
  nor2   g09(.a(x05), .b(new_n30_), .O(new_n34_));
  nor3   g10(.a(new_n34_), .b(new_n33_), .c(new_n27_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n32_), .c(new_n25_), .O(new_n36_));
  inv1   g12(.a(x10), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(x00), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n36_), .O(z0));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n27_), .c(x04), .d(x03), .O(new_n41_));
  nor2   g17(.a(new_n26_), .b(x02), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x01), .O(new_n43_));
  oai21  g19(.a(new_n41_), .b(new_n33_), .c(new_n43_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n38_), .O(z1));
  oai21  g22(.a(x05), .b(new_n30_), .c(x01), .O(new_n47_));
  nand2  g23(.a(x05), .b(x01), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(x04), .O(new_n49_));
  nor2   g25(.a(x04), .b(new_n26_), .O(new_n50_));
  nor2   g26(.a(x10), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(new_n50_), .c(new_n27_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(new_n53_));
  inv1   g29(.a(x07), .O(new_n54_));
  nand2  g30(.a(x03), .b(x02), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(new_n37_), .c(x09), .d(new_n54_), .O(new_n56_));
  oai21  g32(.a(x10), .b(x07), .c(new_n26_), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n56_), .c(x01), .O(new_n58_));
  aoi21  g34(.a(new_n53_), .b(x02), .c(new_n58_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  oai21  g36(.a(x03), .b(x01), .c(new_n33_), .O(new_n61_));
  nand4  g37(.a(new_n61_), .b(x10), .c(new_n60_), .d(new_n54_), .O(new_n62_));
  oai21  g38(.a(new_n59_), .b(x00), .c(new_n62_), .O(z2));
  nand2  g39(.a(x11), .b(x07), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(x10), .c(x00), .O(new_n65_));
  inv1   g41(.a(x12), .O(new_n66_));
  nand2  g42(.a(x11), .b(x07), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(new_n25_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n65_), .O(z3));
  aoi21  g45(.a(x10), .b(x07), .c(new_n25_), .O(new_n70_));
  nand2  g46(.a(x09), .b(x08), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(x10), .c(new_n25_), .O(new_n72_));
  oai22  g48(.a(new_n72_), .b(x07), .c(new_n70_), .d(x11), .O(z4));
  nand3  g49(.a(x13), .b(new_n66_), .c(new_n54_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n25_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n37_), .O(z5));
  inv1   g52(.a(x09), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(x03), .c(new_n33_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n27_), .O(new_n79_));
  oai21  g55(.a(x09), .b(new_n26_), .c(new_n27_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n33_), .O(new_n81_));
  inv1   g57(.a(x14), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(x02), .O(new_n83_));
  nand3  g59(.a(new_n83_), .b(new_n81_), .c(new_n79_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n37_), .c(new_n54_), .d(new_n25_), .O(new_n85_));
  inv1   g61(.a(new_n85_), .O(z6));
  aoi21  g62(.a(new_n42_), .b(new_n27_), .c(x08), .O(new_n87_));
  nand2  g63(.a(new_n42_), .b(new_n27_), .O(new_n88_));
  nand2  g64(.a(new_n37_), .b(x09), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(new_n88_), .c(new_n54_), .O(new_n90_));
  oai21  g66(.a(new_n90_), .b(new_n87_), .c(new_n25_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n37_), .O(z7));
  aoi21  g68(.a(new_n78_), .b(new_n27_), .c(new_n28_), .O(new_n93_));
  oai21  g69(.a(new_n93_), .b(x10), .c(new_n66_), .O(new_n94_));
  oai21  g70(.a(new_n34_), .b(x01), .c(x02), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(x09), .c(new_n54_), .O(new_n96_));
  aoi21  g72(.a(new_n94_), .b(new_n54_), .c(new_n96_), .O(new_n97_));
  oai21  g73(.a(new_n97_), .b(x00), .c(new_n37_), .O(z8));
endmodule


