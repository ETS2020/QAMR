// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand3  g04(.a(x10), .b(new_n28_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand2  g12(.a(x06), .b(x05), .O(new_n37_));
  oai22  g13(.a(new_n37_), .b(new_n25_), .c(x04), .d(x03), .O(new_n38_));
  nand4  g14(.a(x10), .b(x03), .c(new_n28_), .d(x01), .O(new_n39_));
  oai21  g15(.a(new_n38_), .b(new_n28_), .c(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  nor2   g17(.a(x10), .b(x02), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n41_), .O(z1));
  oai21  g20(.a(x03), .b(x01), .c(new_n28_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n36_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  oai21  g24(.a(x03), .b(x01), .c(new_n28_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n46_), .c(new_n43_), .O(z2));
  inv1   g27(.a(x11), .O(new_n52_));
  nor2   g28(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  inv1   g29(.a(x12), .O(new_n54_));
  aoi22  g30(.a(new_n43_), .b(x00), .c(new_n54_), .d(x10), .O(new_n55_));
  oai21  g31(.a(x07), .b(x00), .c(x11), .O(new_n56_));
  nand3  g32(.a(new_n56_), .b(new_n54_), .c(x02), .O(new_n57_));
  oai21  g33(.a(new_n55_), .b(new_n53_), .c(new_n57_), .O(z3));
  nand3  g34(.a(x09), .b(x08), .c(x00), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n47_), .O(new_n60_));
  nand2  g36(.a(new_n52_), .b(x07), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n60_), .c(new_n43_), .O(z4));
  inv1   g38(.a(x10), .O(new_n63_));
  nand4  g39(.a(x13), .b(new_n54_), .c(new_n47_), .d(new_n36_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(x02), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n63_), .O(z5));
  aoi21  g42(.a(x14), .b(x01), .c(x00), .O(new_n67_));
  nor2   g43(.a(x08), .b(new_n36_), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n67_), .c(new_n47_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(x02), .c(x10), .O(z6));
  nand4  g46(.a(new_n63_), .b(x08), .c(new_n47_), .d(x02), .O(z7));
  inv1   g47(.a(x09), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n47_), .c(x00), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x02), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n63_), .O(z8));
endmodule


