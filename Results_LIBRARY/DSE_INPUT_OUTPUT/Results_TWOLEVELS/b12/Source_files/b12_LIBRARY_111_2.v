// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_,
    new_n89_;
  inv1   g00(.a(x10), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x07), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(new_n30_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n31_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n30_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n33_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  nand2  g14(.a(new_n32_), .b(new_n29_), .O(new_n39_));
  aoi21  g15(.a(x06), .b(x05), .c(new_n28_), .O(new_n40_));
  aoi21  g16(.a(new_n39_), .b(new_n26_), .c(new_n40_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(x04), .c(x02), .O(new_n43_));
  oai21  g19(.a(new_n41_), .b(new_n27_), .c(new_n43_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n26_), .O(z1));
  inv1   g22(.a(x07), .O(new_n47_));
  nand2  g23(.a(x10), .b(new_n27_), .O(new_n48_));
  nand3  g24(.a(new_n25_), .b(x09), .c(new_n47_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n48_), .c(x00), .O(new_n50_));
  nand2  g26(.a(new_n25_), .b(x09), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n48_), .c(x08), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n47_), .c(new_n50_), .O(new_n53_));
  inv1   g29(.a(x08), .O(new_n54_));
  aoi22  g30(.a(new_n26_), .b(new_n38_), .c(new_n54_), .d(new_n47_), .O(new_n55_));
  oai22  g31(.a(new_n55_), .b(new_n28_), .c(new_n53_), .d(x01), .O(z2));
  inv1   g32(.a(x11), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(x10), .O(new_n58_));
  aoi22  g34(.a(new_n58_), .b(x07), .c(x12), .d(new_n38_), .O(z3));
  nand3  g35(.a(x09), .b(x08), .c(x00), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n47_), .O(new_n61_));
  oai21  g37(.a(new_n57_), .b(new_n25_), .c(x07), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(z4));
  inv1   g39(.a(x12), .O(new_n64_));
  nand3  g40(.a(x13), .b(new_n64_), .c(new_n38_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n25_), .O(z5));
  oai21  g43(.a(x09), .b(new_n27_), .c(new_n30_), .O(new_n68_));
  nor2   g44(.a(x02), .b(x00), .O(new_n69_));
  nor2   g45(.a(x08), .b(new_n38_), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(new_n71_));
  inv1   g47(.a(x09), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(x03), .c(new_n28_), .O(new_n73_));
  nand2  g49(.a(x08), .b(x00), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n30_), .O(new_n75_));
  inv1   g51(.a(x14), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(x01), .c(new_n38_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n71_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n25_), .c(new_n47_), .O(new_n79_));
  inv1   g55(.a(new_n79_), .O(z6));
  nand3  g56(.a(x09), .b(x03), .c(new_n28_), .O(new_n81_));
  nand3  g57(.a(new_n25_), .b(new_n54_), .c(x02), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  aoi21  g59(.a(x03), .b(new_n30_), .c(x08), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(new_n83_), .c(new_n47_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n25_), .O(z7));
  nand4  g62(.a(new_n64_), .b(x03), .c(new_n28_), .d(new_n30_), .O(new_n87_));
  nor2   g63(.a(x09), .b(x00), .O(new_n88_));
  aoi21  g64(.a(new_n87_), .b(x09), .c(new_n88_), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(x07), .c(new_n25_), .O(z8));
endmodule


