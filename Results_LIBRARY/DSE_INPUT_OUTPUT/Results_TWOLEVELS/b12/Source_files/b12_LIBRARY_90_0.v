// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n88_,
    new_n89_;
  nor2   g00(.a(x10), .b(x09), .O(new_n25_));
  inv1   g01(.a(new_n25_), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand3  g08(.a(new_n26_), .b(x03), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x03), .c(x02), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n34_), .c(new_n27_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n26_), .O(z0));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(new_n35_), .b(new_n40_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(x02), .O(new_n43_));
  nor2   g19(.a(new_n40_), .b(x02), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(x01), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n26_), .c(new_n27_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(z1));
  nand2  g24(.a(new_n30_), .b(x01), .O(new_n49_));
  aoi22  g25(.a(new_n29_), .b(x04), .c(x03), .d(new_n28_), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n25_), .c(new_n49_), .O(new_n51_));
  inv1   g27(.a(x07), .O(new_n52_));
  inv1   g28(.a(x10), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(x03), .c(x01), .O(new_n55_));
  aoi21  g31(.a(new_n51_), .b(x02), .c(new_n55_), .O(new_n56_));
  oai21  g32(.a(new_n53_), .b(new_n40_), .c(new_n28_), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n32_), .c(x08), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n52_), .c(new_n25_), .O(new_n59_));
  oai21  g35(.a(new_n56_), .b(x00), .c(new_n59_), .O(z2));
  inv1   g36(.a(x11), .O(new_n61_));
  inv1   g37(.a(x12), .O(new_n62_));
  oai22  g38(.a(new_n62_), .b(x00), .c(new_n61_), .d(new_n52_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n26_), .O(z3));
  inv1   g40(.a(x09), .O(new_n65_));
  oai21  g41(.a(new_n53_), .b(new_n52_), .c(new_n65_), .O(new_n66_));
  oai21  g42(.a(new_n65_), .b(x08), .c(x00), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n52_), .O(new_n68_));
  nand2  g44(.a(new_n61_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z4));
  oai21  g46(.a(x12), .b(x07), .c(x09), .O(new_n71_));
  aoi21  g47(.a(new_n52_), .b(x00), .c(x10), .O(new_n72_));
  nand4  g48(.a(new_n72_), .b(new_n71_), .c(new_n26_), .d(x13), .O(z5));
  inv1   g49(.a(x08), .O(new_n74_));
  aoi21  g50(.a(new_n32_), .b(new_n28_), .c(new_n27_), .O(new_n75_));
  nor2   g51(.a(x03), .b(x01), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(new_n77_));
  aoi21  g53(.a(x14), .b(x02), .c(new_n28_), .O(new_n78_));
  aoi21  g54(.a(x03), .b(new_n32_), .c(x01), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n78_), .c(new_n27_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n53_), .c(x09), .d(new_n52_), .O(new_n82_));
  inv1   g58(.a(new_n82_), .O(z6));
  nand2  g59(.a(new_n44_), .b(new_n28_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(x08), .c(new_n52_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(x09), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n53_), .O(z7));
  nand2  g63(.a(x03), .b(new_n28_), .O(new_n88_));
  nor4   g64(.a(new_n88_), .b(x12), .c(x07), .d(x02), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(new_n65_), .c(new_n53_), .O(z8));
endmodule


