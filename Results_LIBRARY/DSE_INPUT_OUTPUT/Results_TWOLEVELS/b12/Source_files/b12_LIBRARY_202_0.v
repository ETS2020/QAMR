// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  inv1   g11(.a(x11), .O(new_n36_));
  inv1   g12(.a(x13), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n35_), .O(z0));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n29_), .c(x04), .d(x03), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n27_), .c(x01), .O(new_n42_));
  oai21  g18(.a(new_n41_), .b(new_n27_), .c(new_n42_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n38_), .O(z1));
  oai21  g21(.a(x05), .b(new_n30_), .c(x01), .O(new_n46_));
  nand2  g22(.a(x05), .b(x01), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(x04), .O(new_n48_));
  nand2  g24(.a(new_n30_), .b(x03), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n38_), .b(new_n51_), .c(new_n50_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n48_), .c(new_n46_), .O(new_n55_));
  oai21  g31(.a(x10), .b(x07), .c(new_n26_), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(x10), .b(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n50_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n56_), .c(x01), .O(new_n60_));
  aoi21  g36(.a(new_n55_), .b(x02), .c(new_n60_), .O(new_n61_));
  inv1   g37(.a(new_n38_), .O(new_n62_));
  nor2   g38(.a(new_n51_), .b(x03), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n58_), .c(new_n29_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n27_), .c(x08), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n50_), .c(new_n62_), .O(new_n66_));
  oai21  g42(.a(new_n61_), .b(x00), .c(new_n66_), .O(z2));
  nand2  g43(.a(x13), .b(new_n36_), .O(new_n68_));
  oai21  g44(.a(new_n36_), .b(x07), .c(new_n68_), .O(new_n69_));
  inv1   g45(.a(new_n69_), .O(new_n70_));
  aoi21  g46(.a(x12), .b(new_n25_), .c(new_n70_), .O(z3));
  oai21  g47(.a(new_n37_), .b(x07), .c(new_n36_), .O(new_n72_));
  nand3  g48(.a(x09), .b(x08), .c(x00), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n50_), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n72_), .O(z4));
  nand2  g51(.a(new_n69_), .b(x00), .O(new_n76_));
  oai21  g52(.a(x13), .b(x11), .c(x07), .O(new_n77_));
  nand2  g53(.a(x13), .b(x12), .O(new_n78_));
  oai21  g54(.a(x13), .b(new_n36_), .c(new_n78_), .O(new_n79_));
  aoi22  g55(.a(new_n79_), .b(new_n50_), .c(x13), .d(x10), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n77_), .c(new_n76_), .O(z5));
  inv1   g57(.a(x08), .O(new_n82_));
  aoi21  g58(.a(x09), .b(new_n26_), .c(x02), .O(new_n83_));
  nand3  g59(.a(new_n57_), .b(x03), .c(new_n27_), .O(new_n84_));
  oai21  g60(.a(new_n83_), .b(x01), .c(new_n84_), .O(new_n85_));
  oai21  g61(.a(new_n82_), .b(new_n25_), .c(new_n85_), .O(new_n86_));
  aoi21  g62(.a(x14), .b(x02), .c(x00), .O(new_n87_));
  nor2   g63(.a(x08), .b(new_n25_), .O(new_n88_));
  oai21  g64(.a(new_n88_), .b(new_n87_), .c(x01), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand4  g66(.a(new_n90_), .b(new_n38_), .c(new_n51_), .d(new_n50_), .O(new_n91_));
  inv1   g67(.a(new_n91_), .O(z6));
  nand3  g68(.a(x09), .b(x03), .c(new_n27_), .O(new_n93_));
  nand4  g69(.a(new_n51_), .b(new_n82_), .c(new_n50_), .d(x02), .O(new_n94_));
  aoi21  g70(.a(new_n94_), .b(new_n93_), .c(x01), .O(new_n95_));
  nor2   g71(.a(new_n26_), .b(x01), .O(new_n96_));
  oai21  g72(.a(new_n96_), .b(x08), .c(new_n51_), .O(new_n97_));
  oai21  g73(.a(new_n97_), .b(new_n95_), .c(new_n38_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n77_), .O(z7));
  oai21  g75(.a(x12), .b(x02), .c(x09), .O(new_n100_));
  nand2  g76(.a(new_n57_), .b(new_n25_), .O(new_n101_));
  nand3  g77(.a(new_n101_), .b(new_n100_), .c(new_n51_), .O(new_n102_));
  nand2  g78(.a(new_n102_), .b(new_n38_), .O(new_n103_));
  inv1   g79(.a(new_n96_), .O(new_n104_));
  nand3  g80(.a(new_n104_), .b(new_n69_), .c(x09), .O(new_n105_));
  nand3  g81(.a(new_n105_), .b(new_n103_), .c(new_n77_), .O(z8));
endmodule


