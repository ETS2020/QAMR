// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand3  g04(.a(x07), .b(new_n28_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand2  g12(.a(x06), .b(x05), .O(new_n37_));
  oai22  g13(.a(new_n37_), .b(new_n25_), .c(x04), .d(x03), .O(new_n38_));
  nand4  g14(.a(x07), .b(x03), .c(new_n28_), .d(x01), .O(new_n39_));
  oai21  g15(.a(new_n38_), .b(new_n28_), .c(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  inv1   g17(.a(x07), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n28_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n41_), .O(z1));
  nor2   g20(.a(x04), .b(x03), .O(new_n45_));
  nand4  g21(.a(new_n45_), .b(new_n33_), .c(x07), .d(new_n25_), .O(new_n46_));
  inv1   g22(.a(x03), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n47_), .c(new_n25_), .O(new_n48_));
  inv1   g24(.a(new_n48_), .O(new_n49_));
  aoi21  g25(.a(new_n46_), .b(x02), .c(new_n49_), .O(new_n50_));
  nor2   g26(.a(x08), .b(x07), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x02), .O(new_n52_));
  oai21  g28(.a(new_n50_), .b(x00), .c(new_n52_), .O(z2));
  aoi21  g29(.a(x09), .b(new_n28_), .c(new_n36_), .O(new_n54_));
  oai21  g30(.a(x12), .b(x00), .c(x02), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n54_), .c(new_n42_), .O(new_n56_));
  inv1   g32(.a(x12), .O(new_n57_));
  nor2   g33(.a(new_n57_), .b(x00), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x11), .c(new_n56_), .O(z3));
  nand4  g35(.a(x09), .b(x08), .c(x02), .d(x00), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n42_), .O(new_n61_));
  oai21  g37(.a(x11), .b(new_n42_), .c(new_n61_), .O(z4));
  inv1   g38(.a(new_n54_), .O(new_n63_));
  aoi21  g39(.a(x12), .b(new_n36_), .c(new_n28_), .O(new_n64_));
  inv1   g40(.a(x13), .O(new_n65_));
  aoi21  g41(.a(x10), .b(x02), .c(new_n65_), .O(new_n66_));
  nand4  g42(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(new_n42_), .O(z5));
  inv1   g43(.a(x10), .O(new_n68_));
  nand2  g44(.a(x14), .b(x01), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n36_), .O(new_n70_));
  oai21  g46(.a(x08), .b(new_n36_), .c(new_n70_), .O(new_n71_));
  nand4  g47(.a(new_n71_), .b(new_n68_), .c(new_n42_), .d(x02), .O(new_n72_));
  inv1   g48(.a(new_n72_), .O(z6));
  oai21  g49(.a(new_n51_), .b(x10), .c(x02), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n42_), .O(z7));
  inv1   g51(.a(x09), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(x00), .c(x07), .O(new_n77_));
  oai21  g53(.a(new_n77_), .b(x10), .c(x02), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n42_), .O(z8));
endmodule


