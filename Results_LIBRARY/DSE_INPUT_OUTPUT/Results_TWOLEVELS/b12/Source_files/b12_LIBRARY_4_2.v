// Benchmark "FAU" written by ABC on Tue Aug 18 15:31:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand4  g06(.a(x09), .b(new_n30_), .c(x02), .d(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x04), .O(new_n34_));
  nand4  g10(.a(new_n34_), .b(x09), .c(x02), .d(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n32_), .c(new_n25_), .O(new_n37_));
  inv1   g13(.a(x09), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(z0));
  and2   g16(.a(x06), .b(x05), .O(new_n41_));
  aoi22  g17(.a(new_n41_), .b(x01), .c(new_n30_), .d(new_n26_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(x09), .c(x02), .O(new_n43_));
  nand3  g19(.a(x03), .b(new_n27_), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x00), .O(z1));
  nand3  g21(.a(new_n30_), .b(new_n26_), .c(new_n29_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x02), .O(new_n47_));
  nor2   g23(.a(x10), .b(x07), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n26_), .c(new_n29_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n47_), .c(new_n38_), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g29(.a(new_n53_), .b(new_n26_), .c(new_n27_), .d(new_n29_), .O(new_n54_));
  inv1   g30(.a(new_n54_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n50_), .c(new_n25_), .O(new_n56_));
  inv1   g32(.a(x08), .O(new_n57_));
  nand3  g33(.a(x10), .b(new_n26_), .c(new_n27_), .O(new_n58_));
  nand2  g34(.a(new_n52_), .b(x09), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n29_), .O(new_n61_));
  nand2  g37(.a(x09), .b(x02), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n57_), .c(new_n51_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n56_), .O(z2));
  inv1   g41(.a(x11), .O(new_n66_));
  inv1   g42(.a(x12), .O(new_n67_));
  oai22  g43(.a(new_n67_), .b(x00), .c(new_n66_), .d(new_n51_), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n39_), .O(z3));
  oai21  g45(.a(new_n51_), .b(x02), .c(new_n38_), .O(new_n70_));
  nand2  g46(.a(x08), .b(x00), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n51_), .O(new_n72_));
  nand2  g48(.a(new_n66_), .b(x07), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(z4));
  nand3  g50(.a(x13), .b(new_n67_), .c(new_n25_), .O(new_n75_));
  inv1   g51(.a(new_n75_), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n39_), .c(new_n52_), .d(new_n51_), .O(z5));
  nand3  g53(.a(x09), .b(new_n26_), .c(new_n29_), .O(new_n78_));
  nand2  g54(.a(new_n38_), .b(x03), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n78_), .c(new_n28_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  aoi21  g57(.a(x14), .b(x01), .c(x00), .O(new_n82_));
  nor2   g58(.a(x08), .b(new_n25_), .O(new_n83_));
  oai21  g59(.a(new_n83_), .b(new_n82_), .c(x02), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n52_), .c(new_n51_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n39_), .O(z6));
  oai21  g63(.a(x08), .b(x07), .c(x09), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(x02), .O(new_n89_));
  nand3  g65(.a(x09), .b(x03), .c(new_n29_), .O(new_n90_));
  nand4  g66(.a(new_n52_), .b(new_n57_), .c(new_n51_), .d(x01), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n27_), .O(new_n93_));
  aoi21  g69(.a(new_n57_), .b(new_n26_), .c(new_n53_), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n93_), .c(new_n89_), .O(z7));
  nand2  g71(.a(new_n38_), .b(new_n25_), .O(new_n96_));
  nand3  g72(.a(new_n96_), .b(new_n52_), .c(new_n51_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(new_n27_), .O(new_n98_));
  nand4  g74(.a(new_n67_), .b(x03), .c(new_n27_), .d(new_n29_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(x09), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n98_), .O(z8));
endmodule


