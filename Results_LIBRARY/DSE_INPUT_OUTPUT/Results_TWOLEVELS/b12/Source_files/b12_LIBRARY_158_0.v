// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_;
  nor2   g00(.a(x10), .b(x09), .O(new_n25_));
  inv1   g01(.a(new_n25_), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(new_n30_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n31_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n30_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n33_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  nand2  g14(.a(x06), .b(x05), .O(new_n39_));
  oai22  g15(.a(new_n39_), .b(new_n30_), .c(x04), .d(x03), .O(new_n40_));
  nor2   g16(.a(new_n27_), .b(x02), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x01), .O(new_n42_));
  oai21  g18(.a(new_n40_), .b(new_n28_), .c(new_n42_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n26_), .c(new_n38_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  aoi21  g21(.a(new_n27_), .b(new_n30_), .c(x02), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(x09), .O(new_n50_));
  oai22  g26(.a(new_n50_), .b(new_n48_), .c(new_n46_), .d(new_n25_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n38_), .O(new_n52_));
  inv1   g28(.a(x08), .O(new_n53_));
  nand2  g29(.a(x10), .b(new_n27_), .O(new_n54_));
  aoi21  g30(.a(new_n54_), .b(new_n50_), .c(x01), .O(new_n55_));
  aoi21  g31(.a(x09), .b(x00), .c(x10), .O(new_n56_));
  nor2   g32(.a(new_n56_), .b(new_n28_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n53_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(x07), .c(new_n52_), .O(z2));
  inv1   g35(.a(x09), .O(new_n60_));
  oai21  g36(.a(x07), .b(new_n38_), .c(x10), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  oai21  g38(.a(new_n60_), .b(x07), .c(x11), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(x00), .O(new_n64_));
  inv1   g40(.a(x11), .O(new_n65_));
  inv1   g41(.a(x12), .O(new_n66_));
  oai21  g42(.a(new_n65_), .b(new_n47_), .c(new_n66_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n64_), .c(new_n62_), .O(z3));
  oai21  g44(.a(new_n60_), .b(x08), .c(x00), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n47_), .O(new_n70_));
  nand2  g46(.a(new_n65_), .b(x07), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n62_), .O(z4));
  nand4  g48(.a(x13), .b(new_n66_), .c(new_n47_), .d(new_n38_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(x09), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n49_), .O(z5));
  nor2   g51(.a(x02), .b(x01), .O(new_n76_));
  oai22  g52(.a(new_n76_), .b(new_n38_), .c(x03), .d(x01), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n53_), .O(new_n78_));
  aoi21  g54(.a(x03), .b(new_n28_), .c(x01), .O(new_n79_));
  inv1   g55(.a(x14), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x02), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n29_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n79_), .c(new_n38_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n78_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n49_), .c(x09), .d(new_n47_), .O(new_n85_));
  inv1   g61(.a(new_n85_), .O(z6));
  nand2  g62(.a(new_n41_), .b(new_n30_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(x08), .c(new_n47_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(x09), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n49_), .O(z7));
  nand4  g66(.a(new_n76_), .b(new_n66_), .c(new_n47_), .d(x03), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(x09), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n49_), .O(z8));
endmodule


