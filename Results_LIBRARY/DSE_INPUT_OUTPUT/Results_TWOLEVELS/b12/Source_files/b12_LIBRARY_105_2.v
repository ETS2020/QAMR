// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand4  g06(.a(x10), .b(new_n30_), .c(x02), .d(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x04), .O(new_n34_));
  nand4  g10(.a(new_n34_), .b(x10), .c(x02), .d(x01), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n32_), .c(new_n25_), .O(new_n37_));
  inv1   g13(.a(x10), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(x02), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n37_), .O(z0));
  and2   g16(.a(x06), .b(x05), .O(new_n41_));
  aoi22  g17(.a(new_n41_), .b(x01), .c(new_n30_), .d(new_n26_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(x10), .c(x02), .O(new_n43_));
  nand3  g19(.a(x03), .b(new_n27_), .c(x01), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n43_), .c(x00), .O(z1));
  oai21  g21(.a(x03), .b(x01), .c(new_n27_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x10), .O(new_n47_));
  nand2  g23(.a(x07), .b(new_n26_), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  nand3  g25(.a(new_n38_), .b(x09), .c(new_n49_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n27_), .c(new_n29_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n25_), .O(new_n54_));
  inv1   g30(.a(x08), .O(new_n55_));
  nand2  g31(.a(x10), .b(new_n26_), .O(new_n56_));
  nand3  g32(.a(new_n38_), .b(x09), .c(new_n27_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n29_), .O(new_n59_));
  nand2  g35(.a(x10), .b(x02), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n55_), .c(new_n49_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n54_), .O(z2));
  nand2  g39(.a(x11), .b(x07), .O(new_n64_));
  nand2  g40(.a(x12), .b(new_n25_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n39_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(z3));
  inv1   g43(.a(x09), .O(new_n68_));
  nand2  g44(.a(x08), .b(x00), .O(new_n69_));
  nor2   g45(.a(x09), .b(x01), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(x02), .c(new_n38_), .O(new_n71_));
  aoi22  g47(.a(new_n71_), .b(new_n69_), .c(new_n39_), .d(new_n68_), .O(new_n72_));
  inv1   g48(.a(x11), .O(new_n73_));
  nand3  g49(.a(new_n39_), .b(new_n73_), .c(x07), .O(new_n74_));
  oai21  g50(.a(new_n72_), .b(x07), .c(new_n74_), .O(z4));
  inv1   g51(.a(x12), .O(new_n76_));
  nand4  g52(.a(x13), .b(new_n76_), .c(new_n49_), .d(new_n25_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n27_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n38_), .O(z5));
  nand2  g55(.a(x09), .b(new_n26_), .O(new_n80_));
  nand2  g56(.a(new_n68_), .b(x03), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(new_n28_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n69_), .c(new_n49_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n27_), .c(x10), .O(z6));
  nand3  g60(.a(new_n38_), .b(new_n49_), .c(x01), .O(new_n85_));
  aoi21  g61(.a(new_n85_), .b(x03), .c(x08), .O(new_n86_));
  nand2  g62(.a(x09), .b(x03), .O(new_n87_));
  oai21  g63(.a(new_n87_), .b(x01), .c(new_n49_), .O(new_n88_));
  oai21  g64(.a(new_n88_), .b(new_n86_), .c(new_n27_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n38_), .O(z7));
  nand3  g66(.a(new_n76_), .b(x03), .c(new_n29_), .O(new_n91_));
  oai21  g67(.a(x09), .b(x00), .c(new_n49_), .O(new_n92_));
  aoi21  g68(.a(new_n91_), .b(x09), .c(new_n92_), .O(new_n93_));
  oai21  g69(.a(new_n93_), .b(x02), .c(new_n38_), .O(z8));
endmodule


