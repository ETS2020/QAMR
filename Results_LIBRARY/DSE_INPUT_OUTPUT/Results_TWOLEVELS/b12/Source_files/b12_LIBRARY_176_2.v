// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:28 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n82_, new_n83_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  nor2   g03(.a(x02), .b(new_n27_), .O(new_n28_));
  inv1   g04(.a(new_n28_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n27_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  nor2   g11(.a(x07), .b(new_n26_), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n35_), .O(z0));
  inv1   g14(.a(x02), .O(new_n39_));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n27_), .c(x04), .d(x03), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n39_), .c(x01), .O(new_n42_));
  oai21  g18(.a(new_n41_), .b(new_n39_), .c(new_n42_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n37_), .O(z1));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n26_), .c(new_n27_), .O(new_n50_));
  oai21  g26(.a(new_n36_), .b(new_n39_), .c(new_n50_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n25_), .O(new_n52_));
  inv1   g28(.a(x08), .O(new_n53_));
  nor2   g29(.a(x10), .b(x09), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(x01), .c(new_n39_), .O(new_n55_));
  nand4  g31(.a(new_n55_), .b(new_n53_), .c(new_n46_), .d(new_n26_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n52_), .O(z2));
  inv1   g33(.a(x12), .O(new_n58_));
  nor2   g34(.a(new_n58_), .b(x00), .O(new_n59_));
  nand3  g35(.a(x12), .b(new_n26_), .c(new_n25_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n46_), .O(new_n61_));
  oai21  g37(.a(new_n59_), .b(x11), .c(new_n61_), .O(z3));
  nand3  g38(.a(x09), .b(x08), .c(x00), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n46_), .c(new_n26_), .O(new_n64_));
  oai21  g40(.a(x11), .b(new_n46_), .c(new_n64_), .O(z4));
  nand2  g41(.a(new_n58_), .b(new_n25_), .O(new_n66_));
  aoi21  g42(.a(new_n66_), .b(new_n46_), .c(x10), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(x13), .c(new_n46_), .d(new_n26_), .O(z5));
  oai21  g44(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand3  g45(.a(x09), .b(new_n26_), .c(new_n27_), .O(new_n70_));
  aoi21  g46(.a(new_n70_), .b(new_n69_), .c(x08), .O(new_n71_));
  oai21  g47(.a(new_n47_), .b(x03), .c(new_n39_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n27_), .O(new_n73_));
  nor2   g49(.a(x14), .b(new_n39_), .O(new_n74_));
  nor2   g50(.a(new_n74_), .b(new_n28_), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(new_n73_), .c(x00), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n71_), .c(new_n48_), .O(new_n77_));
  aoi21  g53(.a(new_n77_), .b(new_n26_), .c(x07), .O(z6));
  nor2   g54(.a(x08), .b(x07), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(x10), .c(new_n26_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n46_), .O(z7));
  aoi21  g57(.a(new_n47_), .b(x00), .c(x07), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(x10), .c(new_n26_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n46_), .O(z8));
endmodule


