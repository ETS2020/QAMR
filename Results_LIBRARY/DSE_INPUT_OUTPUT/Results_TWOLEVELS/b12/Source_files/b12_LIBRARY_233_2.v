// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_;
  inv1   g00(.a(x11), .O(new_n25_));
  nor2   g01(.a(x12), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x01), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  inv1   g07(.a(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n31_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(new_n28_), .O(new_n34_));
  oai21  g10(.a(x05), .b(new_n32_), .c(x02), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(new_n31_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n34_), .c(new_n27_), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(x00), .O(z0));
  inv1   g14(.a(x00), .O(new_n39_));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n31_), .c(x04), .d(x03), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n29_), .c(x01), .O(new_n42_));
  oai21  g18(.a(new_n41_), .b(new_n29_), .c(new_n42_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n27_), .c(new_n39_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x08), .O(new_n47_));
  nand3  g23(.a(new_n32_), .b(new_n28_), .c(new_n31_), .O(new_n48_));
  aoi22  g24(.a(new_n48_), .b(new_n39_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nor2   g25(.a(x10), .b(x07), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n31_), .c(new_n39_), .O(new_n51_));
  oai21  g27(.a(new_n49_), .b(new_n26_), .c(new_n51_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(x02), .O(new_n53_));
  inv1   g29(.a(x10), .O(new_n54_));
  nor2   g30(.a(new_n54_), .b(x03), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(x10), .b(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(new_n58_));
  nand2  g34(.a(new_n57_), .b(new_n39_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(x07), .O(new_n60_));
  nor3   g36(.a(new_n50_), .b(x03), .c(x00), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(new_n60_), .c(new_n31_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n53_), .c(new_n27_), .O(z2));
  oai21  g39(.a(new_n25_), .b(new_n46_), .c(x00), .O(new_n64_));
  inv1   g40(.a(x12), .O(new_n65_));
  oai21  g41(.a(x11), .b(new_n39_), .c(new_n65_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n64_), .O(z3));
  nor2   g43(.a(new_n65_), .b(x07), .O(new_n68_));
  nor2   g44(.a(x12), .b(x11), .O(new_n69_));
  oai21  g45(.a(new_n69_), .b(new_n68_), .c(new_n39_), .O(new_n70_));
  oai22  g46(.a(new_n68_), .b(new_n25_), .c(new_n56_), .d(new_n47_), .O(new_n71_));
  nand2  g47(.a(new_n25_), .b(x07), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z4));
  nand2  g49(.a(new_n27_), .b(x07), .O(new_n74_));
  oai21  g50(.a(x09), .b(new_n46_), .c(x12), .O(new_n75_));
  nand3  g51(.a(x13), .b(new_n54_), .c(new_n39_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n25_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n75_), .c(new_n74_), .O(z5));
  nand2  g54(.a(x08), .b(x00), .O(new_n79_));
  aoi21  g55(.a(x09), .b(new_n28_), .c(x02), .O(new_n80_));
  nand3  g56(.a(new_n56_), .b(x03), .c(new_n29_), .O(new_n81_));
  oai21  g57(.a(new_n80_), .b(x01), .c(new_n81_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n79_), .O(new_n83_));
  aoi21  g59(.a(x14), .b(x02), .c(x00), .O(new_n84_));
  nor2   g60(.a(x08), .b(new_n39_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(x01), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n54_), .c(new_n46_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n27_), .O(z6));
  nand2  g65(.a(new_n46_), .b(x02), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(x03), .c(new_n31_), .O(new_n91_));
  nand4  g67(.a(x09), .b(x03), .c(new_n29_), .d(new_n31_), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n54_), .c(new_n46_), .O(new_n93_));
  aoi21  g69(.a(new_n91_), .b(new_n47_), .c(new_n93_), .O(new_n94_));
  nor2   g70(.a(new_n94_), .b(new_n26_), .O(z7));
  nor2   g71(.a(x11), .b(x09), .O(new_n96_));
  oai21  g72(.a(new_n96_), .b(new_n68_), .c(new_n39_), .O(new_n97_));
  inv1   g73(.a(new_n50_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n27_), .O(new_n99_));
  nand3  g75(.a(x03), .b(new_n29_), .c(new_n31_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n25_), .O(new_n101_));
  nand2  g77(.a(new_n101_), .b(new_n65_), .O(new_n102_));
  nand2  g78(.a(new_n102_), .b(x09), .O(new_n103_));
  nand3  g79(.a(new_n103_), .b(new_n99_), .c(new_n97_), .O(z8));
endmodule


