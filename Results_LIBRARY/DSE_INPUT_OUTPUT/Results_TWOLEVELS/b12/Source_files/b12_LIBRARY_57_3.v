// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  nand2  g11(.a(x10), .b(x03), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(z0));
  nand2  g13(.a(x06), .b(x05), .O(new_n38_));
  oai22  g14(.a(new_n38_), .b(new_n29_), .c(x04), .d(x03), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n27_), .c(x01), .O(new_n40_));
  oai21  g16(.a(new_n39_), .b(new_n27_), .c(new_n40_), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n25_), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n36_), .O(z1));
  oai21  g19(.a(x05), .b(new_n30_), .c(x01), .O(new_n44_));
  nand2  g20(.a(x05), .b(x01), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(x04), .O(new_n46_));
  oai22  g22(.a(x10), .b(x07), .c(x04), .d(new_n26_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(new_n49_));
  nand2  g25(.a(x07), .b(new_n26_), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  aoi21  g27(.a(x09), .b(new_n51_), .c(x10), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n50_), .c(x01), .O(new_n53_));
  aoi21  g29(.a(new_n49_), .b(x02), .c(new_n53_), .O(new_n54_));
  inv1   g30(.a(new_n36_), .O(new_n55_));
  oai21  g31(.a(x10), .b(x09), .c(new_n29_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n27_), .c(x08), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n51_), .c(new_n55_), .O(new_n58_));
  oai21  g34(.a(new_n54_), .b(x00), .c(new_n58_), .O(z2));
  inv1   g35(.a(x12), .O(new_n60_));
  nor2   g36(.a(new_n60_), .b(x00), .O(new_n61_));
  nand3  g37(.a(x10), .b(x09), .c(x03), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x00), .O(new_n63_));
  oai21  g39(.a(x12), .b(x00), .c(new_n63_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n51_), .c(new_n55_), .O(new_n65_));
  oai21  g41(.a(new_n61_), .b(x11), .c(new_n65_), .O(z3));
  inv1   g42(.a(x08), .O(new_n67_));
  nand2  g43(.a(x09), .b(new_n29_), .O(new_n68_));
  inv1   g44(.a(x10), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(x01), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n68_), .c(x03), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n67_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(x09), .c(x00), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n51_), .O(new_n74_));
  inv1   g50(.a(x11), .O(new_n75_));
  aoi21  g51(.a(new_n75_), .b(x07), .c(new_n55_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n74_), .O(z4));
  aoi21  g53(.a(new_n69_), .b(x00), .c(new_n61_), .O(new_n78_));
  nand4  g54(.a(new_n78_), .b(x13), .c(new_n69_), .d(new_n51_), .O(z5));
  oai21  g55(.a(x09), .b(new_n26_), .c(new_n29_), .O(new_n80_));
  nor2   g56(.a(x02), .b(x00), .O(new_n81_));
  nor2   g57(.a(x08), .b(new_n25_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  inv1   g59(.a(x09), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(x03), .c(new_n27_), .O(new_n85_));
  nand2  g61(.a(x08), .b(x00), .O(new_n86_));
  nand3  g62(.a(new_n86_), .b(new_n85_), .c(new_n29_), .O(new_n87_));
  inv1   g63(.a(x14), .O(new_n88_));
  nand3  g64(.a(new_n88_), .b(x01), .c(new_n25_), .O(new_n89_));
  nand3  g65(.a(new_n89_), .b(new_n87_), .c(new_n83_), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(new_n69_), .c(new_n51_), .O(new_n91_));
  inv1   g67(.a(new_n91_), .O(z6));
  nor2   g68(.a(x08), .b(x07), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(x02), .O(new_n94_));
  nand3  g70(.a(x09), .b(x03), .c(new_n27_), .O(new_n95_));
  aoi21  g71(.a(new_n95_), .b(new_n94_), .c(x01), .O(new_n96_));
  oai21  g72(.a(x08), .b(new_n29_), .c(new_n51_), .O(new_n97_));
  oai21  g73(.a(new_n97_), .b(new_n96_), .c(new_n69_), .O(new_n98_));
  oai21  g74(.a(new_n93_), .b(x10), .c(new_n26_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n98_), .O(z7));
  nand2  g76(.a(new_n26_), .b(new_n29_), .O(new_n101_));
  nand3  g77(.a(new_n69_), .b(x09), .c(new_n51_), .O(new_n102_));
  oai21  g78(.a(new_n102_), .b(new_n101_), .c(x09), .O(new_n103_));
  nand2  g79(.a(new_n103_), .b(new_n25_), .O(new_n104_));
  nand3  g80(.a(x09), .b(new_n51_), .c(x00), .O(new_n105_));
  nand2  g81(.a(new_n105_), .b(new_n69_), .O(new_n106_));
  nand2  g82(.a(new_n106_), .b(new_n26_), .O(new_n107_));
  nand3  g83(.a(new_n60_), .b(new_n27_), .c(new_n29_), .O(new_n108_));
  oai21  g84(.a(x10), .b(new_n51_), .c(new_n36_), .O(new_n109_));
  aoi21  g85(.a(new_n108_), .b(x09), .c(new_n109_), .O(new_n110_));
  nand3  g86(.a(new_n110_), .b(new_n107_), .c(new_n104_), .O(z8));
endmodule


