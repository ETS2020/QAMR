// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x10), .O(new_n25_));
  inv1   g01(.a(x14), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x00), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  inv1   g08(.a(x02), .O(new_n33_));
  nand3  g09(.a(new_n27_), .b(x03), .c(new_n33_), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n32_), .c(new_n29_), .O(new_n35_));
  inv1   g11(.a(x04), .O(new_n36_));
  nand3  g12(.a(new_n36_), .b(x03), .c(x02), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n35_), .c(new_n28_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n27_), .O(z0));
  inv1   g16(.a(x03), .O(new_n41_));
  nand2  g17(.a(new_n36_), .b(new_n41_), .O(new_n42_));
  nand3  g18(.a(x06), .b(x05), .c(x01), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n42_), .c(x02), .O(new_n44_));
  nand3  g20(.a(x03), .b(new_n33_), .c(x01), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(new_n27_), .c(new_n28_), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(z1));
  nand2  g24(.a(new_n31_), .b(x01), .O(new_n49_));
  oai21  g25(.a(new_n41_), .b(x01), .c(new_n31_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n27_), .O(new_n51_));
  inv1   g27(.a(x07), .O(new_n52_));
  nand4  g28(.a(x14), .b(new_n25_), .c(new_n52_), .d(new_n29_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  oai21  g30(.a(x10), .b(x07), .c(new_n41_), .O(new_n55_));
  nand3  g31(.a(new_n25_), .b(x09), .c(new_n52_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n55_), .c(x01), .O(new_n57_));
  aoi21  g33(.a(new_n54_), .b(x02), .c(new_n57_), .O(new_n58_));
  inv1   g34(.a(new_n27_), .O(new_n59_));
  nand2  g35(.a(x10), .b(new_n41_), .O(new_n60_));
  nand3  g36(.a(x14), .b(new_n25_), .c(x09), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n29_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n33_), .c(x08), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n52_), .c(new_n59_), .O(new_n65_));
  oai21  g41(.a(new_n58_), .b(x00), .c(new_n65_), .O(z2));
  aoi21  g42(.a(x11), .b(x07), .c(new_n28_), .O(new_n67_));
  nor2   g43(.a(x12), .b(x11), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n67_), .c(new_n27_), .O(new_n69_));
  inv1   g45(.a(x12), .O(new_n70_));
  oai21  g46(.a(new_n26_), .b(x00), .c(new_n25_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(new_n70_), .c(new_n52_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n69_), .O(z3));
  nor2   g49(.a(x09), .b(x07), .O(new_n74_));
  nor2   g50(.a(x11), .b(new_n52_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n74_), .c(new_n27_), .O(new_n76_));
  inv1   g52(.a(x08), .O(new_n77_));
  nor2   g53(.a(new_n77_), .b(new_n28_), .O(new_n78_));
  nor2   g54(.a(new_n78_), .b(new_n25_), .O(new_n79_));
  inv1   g55(.a(x09), .O(new_n80_));
  nand2  g56(.a(x01), .b(x00), .O(new_n81_));
  oai21  g57(.a(new_n80_), .b(x01), .c(new_n81_), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n25_), .c(new_n77_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(x00), .c(new_n26_), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(new_n79_), .c(new_n52_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n76_), .O(z4));
  nand4  g62(.a(x13), .b(new_n70_), .c(new_n52_), .d(new_n28_), .O(new_n87_));
  nand2  g63(.a(new_n87_), .b(x14), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n25_), .O(z5));
  nor2   g65(.a(x02), .b(x00), .O(new_n90_));
  nor2   g66(.a(x08), .b(new_n28_), .O(new_n91_));
  oai21  g67(.a(new_n91_), .b(new_n90_), .c(x01), .O(new_n92_));
  oai21  g68(.a(new_n80_), .b(x03), .c(new_n33_), .O(new_n93_));
  nor2   g69(.a(x09), .b(new_n41_), .O(new_n94_));
  aoi22  g70(.a(new_n94_), .b(new_n33_), .c(new_n93_), .d(new_n29_), .O(new_n95_));
  oai21  g71(.a(new_n95_), .b(new_n78_), .c(new_n92_), .O(new_n96_));
  nand4  g72(.a(new_n96_), .b(x14), .c(new_n25_), .d(new_n52_), .O(new_n97_));
  inv1   g73(.a(new_n97_), .O(z6));
  nand2  g74(.a(new_n52_), .b(x02), .O(new_n99_));
  nand3  g75(.a(new_n99_), .b(x03), .c(new_n29_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n77_), .O(new_n101_));
  nor2   g77(.a(x02), .b(x01), .O(new_n102_));
  nor2   g78(.a(new_n80_), .b(new_n41_), .O(new_n103_));
  aoi21  g79(.a(new_n103_), .b(new_n102_), .c(x10), .O(new_n104_));
  nand2  g80(.a(x14), .b(x07), .O(new_n105_));
  nand4  g81(.a(new_n105_), .b(new_n104_), .c(new_n101_), .d(new_n27_), .O(z7));
  nand3  g82(.a(new_n102_), .b(new_n70_), .c(x03), .O(new_n107_));
  oai21  g83(.a(x09), .b(x00), .c(new_n52_), .O(new_n108_));
  aoi21  g84(.a(new_n107_), .b(x09), .c(new_n108_), .O(new_n109_));
  oai21  g85(.a(new_n109_), .b(new_n26_), .c(new_n25_), .O(z8));
endmodule


