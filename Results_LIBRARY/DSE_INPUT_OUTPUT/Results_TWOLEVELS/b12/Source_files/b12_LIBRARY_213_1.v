// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_;
  inv1   g00(.a(x07), .O(new_n25_));
  nand2  g01(.a(x10), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(new_n30_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n33_), .c(new_n26_), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(x00), .O(z0));
  inv1   g15(.a(x00), .O(new_n40_));
  nand2  g16(.a(new_n32_), .b(new_n29_), .O(new_n41_));
  aoi21  g17(.a(x06), .b(x05), .c(new_n28_), .O(new_n42_));
  aoi21  g18(.a(new_n41_), .b(new_n26_), .c(new_n42_), .O(new_n43_));
  nand3  g19(.a(x06), .b(x05), .c(x01), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(x04), .c(x02), .O(new_n45_));
  oai21  g21(.a(new_n43_), .b(new_n27_), .c(new_n45_), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n40_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n26_), .O(z1));
  oai21  g24(.a(new_n34_), .b(new_n30_), .c(x04), .O(new_n49_));
  oai21  g25(.a(x04), .b(new_n27_), .c(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n30_), .O(new_n51_));
  nand3  g27(.a(new_n35_), .b(new_n26_), .c(x01), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n51_), .c(new_n49_), .O(new_n53_));
  nand2  g29(.a(x07), .b(new_n27_), .O(new_n54_));
  nand2  g30(.a(x09), .b(new_n25_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n54_), .c(x01), .O(new_n56_));
  aoi21  g32(.a(new_n53_), .b(x02), .c(new_n56_), .O(new_n57_));
  inv1   g33(.a(x10), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(x09), .c(new_n30_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n28_), .c(x08), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(x10), .c(new_n25_), .O(new_n61_));
  oai21  g37(.a(new_n57_), .b(x00), .c(new_n61_), .O(z2));
  inv1   g38(.a(x11), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n25_), .c(x00), .O(new_n64_));
  oai21  g40(.a(x12), .b(x00), .c(new_n58_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n25_), .O(new_n66_));
  inv1   g42(.a(x12), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n63_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n66_), .c(new_n64_), .O(z3));
  inv1   g45(.a(x08), .O(new_n70_));
  oai21  g46(.a(x09), .b(x01), .c(new_n70_), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(x09), .c(x00), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n58_), .c(new_n25_), .O(new_n73_));
  oai21  g49(.a(x11), .b(new_n25_), .c(new_n73_), .O(z4));
  nand3  g50(.a(x12), .b(new_n25_), .c(new_n40_), .O(new_n75_));
  and2   g51(.a(new_n75_), .b(x13), .O(new_n76_));
  nand4  g52(.a(new_n76_), .b(new_n58_), .c(new_n25_), .d(new_n40_), .O(z5));
  oai21  g53(.a(x09), .b(new_n27_), .c(new_n30_), .O(new_n78_));
  nor2   g54(.a(x02), .b(x00), .O(new_n79_));
  nor2   g55(.a(x08), .b(new_n40_), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nand2  g57(.a(x09), .b(new_n27_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(new_n28_), .c(x00), .O(new_n83_));
  nand2  g59(.a(new_n58_), .b(x02), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n82_), .c(x08), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n83_), .c(new_n30_), .O(new_n86_));
  nor2   g62(.a(x14), .b(new_n28_), .O(new_n87_));
  aoi21  g63(.a(new_n87_), .b(new_n40_), .c(x10), .O(new_n88_));
  nand3  g64(.a(new_n88_), .b(new_n86_), .c(new_n81_), .O(new_n89_));
  and2   g65(.a(new_n89_), .b(new_n25_), .O(z6));
  nand3  g66(.a(new_n70_), .b(new_n25_), .c(x02), .O(new_n91_));
  nand3  g67(.a(x09), .b(x03), .c(new_n28_), .O(new_n92_));
  aoi21  g68(.a(new_n92_), .b(new_n91_), .c(x01), .O(new_n93_));
  nand2  g69(.a(new_n25_), .b(x01), .O(new_n94_));
  aoi21  g70(.a(new_n94_), .b(x03), .c(x08), .O(new_n95_));
  oai21  g71(.a(new_n95_), .b(new_n93_), .c(new_n58_), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n25_), .O(z7));
  nand2  g73(.a(new_n25_), .b(new_n27_), .O(new_n98_));
  nand4  g74(.a(new_n98_), .b(new_n67_), .c(new_n28_), .d(new_n30_), .O(new_n99_));
  nor2   g75(.a(x09), .b(x00), .O(new_n100_));
  aoi21  g76(.a(new_n99_), .b(x09), .c(new_n100_), .O(new_n101_));
  oai21  g77(.a(new_n101_), .b(x10), .c(new_n25_), .O(z8));
endmodule


