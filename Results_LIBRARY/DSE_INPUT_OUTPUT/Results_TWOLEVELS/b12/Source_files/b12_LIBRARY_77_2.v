// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x11), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x10), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x04), .O(new_n29_));
  oai21  g05(.a(x05), .b(new_n29_), .c(x02), .O(new_n30_));
  inv1   g06(.a(x02), .O(new_n31_));
  nand3  g07(.a(new_n26_), .b(x03), .c(new_n31_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  nand3  g09(.a(new_n29_), .b(x03), .c(x02), .O(new_n34_));
  inv1   g10(.a(new_n34_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n33_), .c(new_n27_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n26_), .O(z0));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n29_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n31_), .c(x01), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n26_), .c(new_n27_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  inv1   g21(.a(x08), .O(new_n46_));
  inv1   g22(.a(x09), .O(new_n47_));
  nand2  g23(.a(x11), .b(x10), .O(new_n48_));
  oai22  g24(.a(new_n48_), .b(x03), .c(x10), .d(new_n47_), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(x09), .c(new_n27_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  aoi21  g28(.a(new_n49_), .b(new_n46_), .c(new_n52_), .O(new_n53_));
  inv1   g29(.a(x07), .O(new_n54_));
  oai21  g30(.a(x10), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n38_), .c(new_n27_), .O(new_n56_));
  oai21  g32(.a(new_n53_), .b(x07), .c(new_n56_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n28_), .O(new_n58_));
  oai21  g34(.a(x11), .b(new_n50_), .c(new_n27_), .O(new_n59_));
  nand3  g35(.a(new_n26_), .b(new_n46_), .c(new_n54_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(x02), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n58_), .O(z2));
  nand2  g39(.a(new_n25_), .b(new_n50_), .O(new_n64_));
  oai21  g40(.a(new_n25_), .b(x07), .c(new_n64_), .O(new_n65_));
  nand2  g41(.a(x12), .b(new_n27_), .O(new_n66_));
  and2   g42(.a(new_n66_), .b(new_n65_), .O(z3));
  nand2  g43(.a(new_n31_), .b(x01), .O(new_n68_));
  nand2  g44(.a(x09), .b(new_n28_), .O(new_n69_));
  aoi21  g45(.a(new_n69_), .b(new_n68_), .c(x00), .O(new_n70_));
  nand2  g46(.a(x01), .b(x00), .O(new_n71_));
  aoi21  g47(.a(new_n71_), .b(new_n69_), .c(x08), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n70_), .c(new_n50_), .O(new_n73_));
  nand3  g49(.a(x09), .b(x08), .c(x00), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(x11), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n54_), .O(new_n77_));
  nand2  g53(.a(x02), .b(new_n27_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(x09), .c(new_n54_), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n25_), .c(new_n50_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(new_n77_), .O(z4));
  nand2  g57(.a(new_n65_), .b(x00), .O(new_n82_));
  oai21  g58(.a(x11), .b(new_n50_), .c(x07), .O(new_n83_));
  nor2   g59(.a(x13), .b(x07), .O(new_n84_));
  oai21  g60(.a(new_n84_), .b(x12), .c(new_n50_), .O(new_n85_));
  nand4  g61(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n48_), .O(z5));
  nand2  g62(.a(x08), .b(x00), .O(new_n87_));
  aoi21  g63(.a(x09), .b(new_n38_), .c(x02), .O(new_n88_));
  nand3  g64(.a(new_n47_), .b(x03), .c(new_n31_), .O(new_n89_));
  oai21  g65(.a(new_n88_), .b(x01), .c(new_n89_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n87_), .O(new_n91_));
  aoi21  g67(.a(x14), .b(x02), .c(x00), .O(new_n92_));
  nor2   g68(.a(x08), .b(new_n27_), .O(new_n93_));
  oai21  g69(.a(new_n93_), .b(new_n92_), .c(x01), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(new_n50_), .c(new_n54_), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n26_), .O(z6));
  nand3  g73(.a(x09), .b(x03), .c(new_n31_), .O(new_n98_));
  nand4  g74(.a(new_n50_), .b(new_n46_), .c(new_n54_), .d(x02), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n28_), .O(new_n101_));
  nand2  g77(.a(x03), .b(new_n28_), .O(new_n102_));
  aoi21  g78(.a(new_n102_), .b(new_n46_), .c(x10), .O(new_n103_));
  nand3  g79(.a(new_n103_), .b(new_n101_), .c(new_n83_), .O(z7));
  oai21  g80(.a(new_n38_), .b(x02), .c(new_n54_), .O(new_n105_));
  nor2   g81(.a(x12), .b(x01), .O(new_n106_));
  aoi21  g82(.a(new_n106_), .b(new_n105_), .c(new_n47_), .O(new_n107_));
  nor2   g83(.a(x09), .b(x00), .O(new_n108_));
  oai21  g84(.a(new_n108_), .b(new_n107_), .c(new_n50_), .O(new_n109_));
  nand3  g85(.a(new_n109_), .b(new_n83_), .c(new_n48_), .O(z8));
endmodule


