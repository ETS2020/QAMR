// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x14), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x10), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x01), .O(new_n30_));
  inv1   g06(.a(x01), .O(new_n31_));
  inv1   g07(.a(x04), .O(new_n32_));
  nand3  g08(.a(new_n32_), .b(x02), .c(new_n31_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n30_), .c(new_n28_), .O(new_n34_));
  oai21  g10(.a(x05), .b(new_n32_), .c(x02), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(new_n31_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n34_), .c(new_n27_), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(x00), .O(z0));
  inv1   g14(.a(x00), .O(new_n39_));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  oai22  g16(.a(new_n40_), .b(new_n31_), .c(x04), .d(x03), .O(new_n41_));
  nand3  g17(.a(x03), .b(new_n29_), .c(x01), .O(new_n42_));
  oai21  g18(.a(new_n41_), .b(new_n29_), .c(new_n42_), .O(new_n43_));
  nand3  g19(.a(new_n43_), .b(new_n27_), .c(new_n39_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(z1));
  nor3   g21(.a(x04), .b(x03), .c(x01), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  nor2   g23(.a(x14), .b(x10), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  oai21  g25(.a(new_n46_), .b(new_n26_), .c(new_n49_), .O(new_n50_));
  aoi21  g26(.a(new_n25_), .b(x07), .c(x10), .O(new_n51_));
  nand3  g27(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n52_));
  oai21  g28(.a(new_n51_), .b(x03), .c(new_n52_), .O(new_n53_));
  aoi22  g29(.a(new_n53_), .b(new_n31_), .c(new_n50_), .d(x02), .O(new_n54_));
  inv1   g30(.a(x08), .O(new_n55_));
  aoi22  g31(.a(new_n48_), .b(x09), .c(x10), .d(new_n28_), .O(new_n56_));
  oai21  g32(.a(new_n25_), .b(x10), .c(x02), .O(new_n57_));
  oai21  g33(.a(new_n56_), .b(x01), .c(new_n57_), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n55_), .c(new_n47_), .O(new_n59_));
  oai21  g35(.a(new_n54_), .b(x00), .c(new_n59_), .O(z2));
  nand2  g36(.a(x11), .b(x07), .O(new_n61_));
  nand2  g37(.a(x12), .b(new_n39_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n61_), .c(new_n27_), .O(new_n63_));
  inv1   g39(.a(new_n63_), .O(z3));
  nand3  g40(.a(x09), .b(x08), .c(x00), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  inv1   g42(.a(x11), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(x07), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n66_), .c(new_n27_), .O(z4));
  inv1   g45(.a(x10), .O(new_n70_));
  inv1   g46(.a(x12), .O(new_n71_));
  nand4  g47(.a(x13), .b(new_n71_), .c(new_n47_), .d(new_n39_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n25_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n70_), .O(z5));
  nand2  g50(.a(x08), .b(x00), .O(new_n75_));
  nor2   g51(.a(x02), .b(x01), .O(new_n76_));
  nand2  g52(.a(x09), .b(new_n28_), .O(new_n77_));
  inv1   g53(.a(x09), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x03), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n75_), .c(new_n25_), .d(new_n70_), .O(new_n81_));
  nor2   g57(.a(new_n81_), .b(x07), .O(z6));
  inv1   g58(.a(new_n76_), .O(new_n83_));
  nand3  g59(.a(new_n83_), .b(new_n70_), .c(new_n47_), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(x03), .c(x08), .O(new_n85_));
  nand3  g61(.a(new_n76_), .b(x09), .c(x03), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n47_), .O(new_n87_));
  oai21  g63(.a(new_n87_), .b(new_n85_), .c(new_n25_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n70_), .O(z7));
  nand3  g65(.a(new_n76_), .b(new_n71_), .c(x03), .O(new_n90_));
  oai21  g66(.a(x09), .b(x00), .c(new_n47_), .O(new_n91_));
  aoi21  g67(.a(new_n90_), .b(x09), .c(new_n91_), .O(new_n92_));
  oai21  g68(.a(new_n92_), .b(x14), .c(new_n70_), .O(z8));
endmodule


