// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  oai21  g03(.a(x02), .b(new_n25_), .c(new_n27_), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x03), .O(new_n29_));
  inv1   g05(.a(x05), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(x01), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(x00), .O(z0));
  inv1   g09(.a(x00), .O(new_n34_));
  inv1   g10(.a(x02), .O(new_n35_));
  nand2  g11(.a(x06), .b(x05), .O(new_n36_));
  oai22  g12(.a(new_n36_), .b(new_n25_), .c(x04), .d(x03), .O(new_n37_));
  nand3  g13(.a(x03), .b(new_n35_), .c(x01), .O(new_n38_));
  oai21  g14(.a(new_n37_), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  nand2  g16(.a(x10), .b(x00), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(z1));
  nand2  g18(.a(new_n31_), .b(x01), .O(new_n43_));
  oai21  g19(.a(new_n30_), .b(new_n25_), .c(x04), .O(new_n44_));
  inv1   g20(.a(x03), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  oai21  g24(.a(x04), .b(new_n45_), .c(new_n48_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n25_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n44_), .c(new_n43_), .O(new_n51_));
  nand2  g27(.a(new_n48_), .b(new_n45_), .O(new_n52_));
  nand3  g28(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n52_), .c(x01), .O(new_n54_));
  aoi21  g30(.a(new_n51_), .b(x02), .c(new_n54_), .O(new_n55_));
  inv1   g31(.a(x08), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(x01), .c(new_n35_), .O(new_n58_));
  nand4  g34(.a(new_n58_), .b(new_n47_), .c(new_n56_), .d(new_n46_), .O(new_n59_));
  oai21  g35(.a(new_n55_), .b(x00), .c(new_n59_), .O(z2));
  nand2  g36(.a(new_n47_), .b(x00), .O(new_n61_));
  inv1   g37(.a(x12), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n34_), .O(new_n63_));
  aoi22  g39(.a(new_n63_), .b(new_n61_), .c(x11), .d(x07), .O(z3));
  nand3  g40(.a(new_n56_), .b(new_n46_), .c(x01), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(x00), .O(new_n67_));
  nand4  g43(.a(new_n47_), .b(x09), .c(new_n56_), .d(new_n25_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(x09), .c(x00), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  inv1   g46(.a(x11), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x07), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n70_), .c(new_n67_), .O(z4));
  inv1   g49(.a(x13), .O(new_n74_));
  aoi21  g50(.a(x12), .b(new_n46_), .c(new_n74_), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n47_), .c(new_n46_), .d(new_n34_), .O(z5));
  nand2  g52(.a(x14), .b(x02), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(x01), .O(new_n78_));
  oai21  g54(.a(new_n57_), .b(x03), .c(new_n35_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n25_), .O(new_n80_));
  nand3  g56(.a(new_n57_), .b(x03), .c(new_n35_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n80_), .c(new_n78_), .O(new_n82_));
  nand3  g58(.a(new_n56_), .b(x02), .c(new_n25_), .O(new_n83_));
  inv1   g59(.a(new_n83_), .O(new_n84_));
  aoi21  g60(.a(new_n82_), .b(new_n34_), .c(new_n84_), .O(new_n85_));
  nand2  g61(.a(x09), .b(new_n45_), .O(new_n86_));
  nand2  g62(.a(new_n57_), .b(x03), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n86_), .c(new_n25_), .O(new_n88_));
  nand3  g64(.a(new_n88_), .b(new_n56_), .c(x00), .O(new_n89_));
  oai21  g65(.a(new_n85_), .b(x10), .c(new_n89_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n41_), .O(z6));
  nand3  g68(.a(x09), .b(x03), .c(new_n35_), .O(new_n93_));
  nand4  g69(.a(new_n47_), .b(new_n56_), .c(new_n46_), .d(x02), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n25_), .O(new_n96_));
  nand2  g72(.a(x03), .b(new_n25_), .O(new_n97_));
  aoi21  g73(.a(new_n97_), .b(new_n56_), .c(new_n48_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n96_), .O(z7));
  nand4  g75(.a(new_n62_), .b(x03), .c(new_n35_), .d(new_n25_), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(x09), .O(new_n101_));
  aoi21  g77(.a(new_n57_), .b(new_n34_), .c(x10), .O(new_n102_));
  nand3  g78(.a(new_n102_), .b(new_n101_), .c(new_n46_), .O(z8));
endmodule


