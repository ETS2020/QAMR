// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_;
  nand2  g00(.a(x10), .b(x07), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  nor2   g11(.a(new_n35_), .b(x00), .O(z0));
  inv1   g12(.a(x00), .O(new_n37_));
  nand2  g13(.a(x06), .b(x05), .O(new_n38_));
  oai22  g14(.a(new_n38_), .b(new_n29_), .c(x04), .d(x03), .O(new_n39_));
  nand3  g15(.a(x03), .b(new_n27_), .c(x01), .O(new_n40_));
  oai21  g16(.a(new_n39_), .b(new_n27_), .c(new_n40_), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n25_), .c(new_n37_), .O(new_n42_));
  inv1   g18(.a(new_n42_), .O(z1));
  nand3  g19(.a(new_n30_), .b(new_n26_), .c(new_n29_), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  inv1   g21(.a(x07), .O(new_n46_));
  inv1   g22(.a(x10), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(new_n46_), .c(new_n29_), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n45_), .c(new_n27_), .O(new_n49_));
  nand2  g25(.a(new_n47_), .b(new_n46_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n26_), .O(new_n51_));
  nand3  g27(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n51_), .c(x01), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n49_), .c(new_n37_), .O(new_n54_));
  inv1   g30(.a(x08), .O(new_n55_));
  nand2  g31(.a(x10), .b(new_n26_), .O(new_n56_));
  aoi21  g32(.a(new_n56_), .b(new_n52_), .c(x01), .O(new_n57_));
  nor2   g33(.a(x07), .b(new_n27_), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n57_), .c(new_n55_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n54_), .c(new_n25_), .O(z2));
  inv1   g36(.a(x11), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  aoi22  g38(.a(new_n62_), .b(x07), .c(x12), .d(new_n37_), .O(z3));
  nand3  g39(.a(x09), .b(x08), .c(x00), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n46_), .O(new_n65_));
  oai21  g41(.a(new_n61_), .b(x10), .c(x07), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n65_), .O(z4));
  inv1   g43(.a(x13), .O(new_n68_));
  aoi21  g44(.a(x12), .b(new_n37_), .c(new_n68_), .O(new_n69_));
  nand4  g45(.a(new_n69_), .b(new_n47_), .c(new_n46_), .d(new_n37_), .O(z5));
  oai21  g46(.a(x09), .b(new_n26_), .c(new_n29_), .O(new_n71_));
  nor2   g47(.a(x02), .b(x00), .O(new_n72_));
  nor2   g48(.a(x08), .b(new_n37_), .O(new_n73_));
  oai21  g49(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  inv1   g50(.a(x09), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(x03), .c(new_n27_), .O(new_n76_));
  nand2  g52(.a(x08), .b(x00), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n29_), .O(new_n78_));
  inv1   g54(.a(x14), .O(new_n79_));
  nand3  g55(.a(new_n79_), .b(x01), .c(new_n37_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n74_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n47_), .c(new_n46_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n25_), .O(z6));
  nand3  g59(.a(x09), .b(x03), .c(new_n27_), .O(new_n84_));
  nand3  g60(.a(new_n58_), .b(new_n47_), .c(new_n55_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n29_), .O(new_n87_));
  nand2  g63(.a(x03), .b(new_n29_), .O(new_n88_));
  aoi21  g64(.a(new_n88_), .b(new_n55_), .c(new_n50_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n87_), .O(z7));
  inv1   g66(.a(x12), .O(new_n91_));
  nand4  g67(.a(new_n91_), .b(x03), .c(new_n27_), .d(new_n29_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(x09), .O(new_n93_));
  aoi21  g69(.a(new_n75_), .b(new_n37_), .c(x10), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n93_), .c(new_n46_), .O(z8));
endmodule


