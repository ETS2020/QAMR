// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_;
  inv1   g00(.a(x12), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x11), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(new_n30_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n31_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n30_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n33_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  nand2  g14(.a(x06), .b(x05), .O(new_n39_));
  oai22  g15(.a(new_n39_), .b(new_n30_), .c(x04), .d(x03), .O(new_n40_));
  nand3  g16(.a(x03), .b(new_n28_), .c(x01), .O(new_n41_));
  oai21  g17(.a(new_n40_), .b(new_n28_), .c(new_n41_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n26_), .c(new_n38_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  oai21  g20(.a(x05), .b(new_n31_), .c(x01), .O(new_n45_));
  nand2  g21(.a(x05), .b(x01), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x04), .O(new_n47_));
  nor2   g23(.a(x04), .b(new_n27_), .O(new_n48_));
  nor2   g24(.a(x10), .b(x07), .O(new_n49_));
  oai21  g25(.a(new_n49_), .b(new_n48_), .c(new_n30_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n47_), .c(new_n45_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n26_), .c(x02), .O(new_n52_));
  inv1   g28(.a(x07), .O(new_n53_));
  inv1   g29(.a(new_n49_), .O(new_n54_));
  inv1   g30(.a(x09), .O(new_n55_));
  nor2   g31(.a(x10), .b(new_n55_), .O(new_n56_));
  aoi22  g32(.a(new_n56_), .b(new_n53_), .c(new_n54_), .d(new_n27_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(x01), .c(new_n52_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n38_), .O(new_n59_));
  inv1   g35(.a(new_n26_), .O(new_n60_));
  inv1   g36(.a(x10), .O(new_n61_));
  nor2   g37(.a(new_n61_), .b(x03), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n56_), .c(new_n30_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n28_), .c(x08), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n53_), .c(new_n60_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n59_), .O(z2));
  inv1   g42(.a(x11), .O(new_n67_));
  aoi21  g43(.a(x12), .b(new_n53_), .c(new_n67_), .O(new_n68_));
  oai22  g44(.a(new_n68_), .b(new_n38_), .c(x12), .d(x11), .O(z3));
  nand3  g45(.a(x09), .b(x08), .c(x00), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n53_), .O(new_n71_));
  nand2  g47(.a(new_n67_), .b(x07), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n71_), .c(new_n26_), .O(z4));
  nor2   g49(.a(new_n54_), .b(x00), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(x13), .c(new_n25_), .d(new_n67_), .O(z5));
  nand2  g51(.a(x09), .b(new_n27_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(new_n28_), .c(x01), .O(new_n77_));
  nand2  g53(.a(new_n55_), .b(x03), .O(new_n78_));
  aoi21  g54(.a(new_n78_), .b(new_n30_), .c(x02), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n77_), .c(new_n26_), .O(new_n80_));
  inv1   g56(.a(x14), .O(new_n81_));
  oai22  g57(.a(new_n25_), .b(new_n28_), .c(x11), .d(new_n30_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  aoi21  g59(.a(new_n83_), .b(new_n80_), .c(x00), .O(new_n84_));
  aoi21  g60(.a(new_n78_), .b(new_n30_), .c(new_n38_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n77_), .c(new_n26_), .O(new_n86_));
  nor2   g62(.a(new_n86_), .b(x08), .O(new_n87_));
  oai21  g63(.a(new_n87_), .b(new_n84_), .c(new_n61_), .O(new_n88_));
  nor2   g64(.a(new_n88_), .b(x07), .O(z6));
  nand3  g65(.a(x09), .b(x03), .c(new_n28_), .O(new_n90_));
  inv1   g66(.a(x08), .O(new_n91_));
  nand4  g67(.a(new_n61_), .b(new_n91_), .c(new_n53_), .d(x02), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n30_), .O(new_n94_));
  aoi21  g70(.a(x03), .b(new_n30_), .c(x08), .O(new_n95_));
  nor2   g71(.a(new_n95_), .b(new_n54_), .O(new_n96_));
  aoi21  g72(.a(new_n96_), .b(new_n94_), .c(new_n60_), .O(z7));
  nand4  g73(.a(new_n25_), .b(x03), .c(new_n28_), .d(new_n30_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(x09), .O(new_n99_));
  nand2  g75(.a(new_n55_), .b(new_n38_), .O(new_n100_));
  nand4  g76(.a(new_n100_), .b(new_n99_), .c(new_n49_), .d(new_n26_), .O(z8));
endmodule


