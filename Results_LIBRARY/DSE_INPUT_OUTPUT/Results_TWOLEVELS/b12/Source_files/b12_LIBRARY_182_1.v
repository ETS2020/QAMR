// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_;
  inv1   g00(.a(x11), .O(new_n25_));
  nor2   g01(.a(new_n25_), .b(x10), .O(new_n26_));
  inv1   g02(.a(new_n26_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x02), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n33_), .c(new_n27_), .O(new_n39_));
  nor2   g15(.a(new_n39_), .b(x00), .O(z0));
  inv1   g16(.a(x00), .O(new_n41_));
  nand3  g17(.a(new_n27_), .b(new_n29_), .c(x01), .O(new_n42_));
  nand3  g18(.a(x06), .b(x05), .c(x01), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x02), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(new_n28_), .O(new_n45_));
  nand3  g21(.a(new_n43_), .b(x04), .c(x02), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n27_), .O(z1));
  oai21  g25(.a(x10), .b(x07), .c(new_n28_), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  inv1   g27(.a(x10), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n50_), .c(new_n29_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n34_), .O(new_n55_));
  oai21  g31(.a(new_n26_), .b(new_n34_), .c(new_n36_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(x02), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n41_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nor2   g36(.a(x11), .b(x10), .O(new_n61_));
  aoi22  g37(.a(new_n61_), .b(x09), .c(x10), .d(new_n28_), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(x01), .c(new_n29_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n60_), .c(new_n51_), .O(new_n64_));
  nand3  g40(.a(new_n64_), .b(new_n59_), .c(new_n27_), .O(z2));
  nand2  g41(.a(x10), .b(new_n51_), .O(new_n66_));
  aoi22  g42(.a(new_n66_), .b(x11), .c(x12), .d(new_n41_), .O(z3));
  inv1   g43(.a(x09), .O(new_n68_));
  nand2  g44(.a(new_n66_), .b(x11), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  oai21  g46(.a(new_n60_), .b(new_n41_), .c(x10), .O(new_n71_));
  nand2  g47(.a(x02), .b(x00), .O(new_n72_));
  nand2  g48(.a(x09), .b(new_n34_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n30_), .O(new_n74_));
  nand4  g50(.a(new_n74_), .b(new_n25_), .c(new_n52_), .d(new_n60_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n71_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n51_), .O(new_n77_));
  oai21  g53(.a(x07), .b(new_n41_), .c(new_n25_), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(new_n77_), .c(new_n70_), .O(z4));
  inv1   g55(.a(x12), .O(new_n80_));
  nand4  g56(.a(x13), .b(new_n80_), .c(new_n51_), .d(new_n41_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n25_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n52_), .O(z5));
  nand2  g59(.a(new_n68_), .b(x03), .O(new_n84_));
  nand2  g60(.a(new_n25_), .b(x02), .O(new_n85_));
  aoi21  g61(.a(new_n85_), .b(new_n84_), .c(new_n41_), .O(new_n86_));
  nand3  g62(.a(new_n25_), .b(new_n29_), .c(x01), .O(new_n87_));
  nand3  g63(.a(x09), .b(new_n28_), .c(new_n34_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(new_n86_), .c(new_n60_), .O(new_n90_));
  nand2  g66(.a(x14), .b(x02), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(x01), .O(new_n92_));
  oai21  g68(.a(new_n68_), .b(x03), .c(new_n29_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n34_), .O(new_n94_));
  nand3  g70(.a(new_n68_), .b(x03), .c(new_n29_), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(new_n94_), .c(new_n92_), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n41_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(new_n90_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n51_), .O(new_n99_));
  aoi21  g75(.a(new_n99_), .b(new_n25_), .c(x10), .O(z6));
  nand3  g76(.a(x09), .b(x03), .c(new_n34_), .O(new_n101_));
  nand2  g77(.a(new_n51_), .b(x01), .O(new_n102_));
  nand2  g78(.a(new_n61_), .b(new_n60_), .O(new_n103_));
  oai21  g79(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  nand2  g80(.a(new_n104_), .b(new_n29_), .O(new_n105_));
  oai21  g81(.a(x07), .b(new_n29_), .c(x03), .O(new_n106_));
  nand2  g82(.a(new_n106_), .b(new_n60_), .O(new_n107_));
  aoi21  g83(.a(new_n25_), .b(x07), .c(new_n26_), .O(new_n108_));
  nand4  g84(.a(new_n108_), .b(new_n107_), .c(new_n105_), .d(new_n52_), .O(z7));
  nand4  g85(.a(new_n80_), .b(x03), .c(new_n29_), .d(new_n34_), .O(new_n110_));
  nand2  g86(.a(new_n110_), .b(x09), .O(new_n111_));
  aoi21  g87(.a(new_n68_), .b(new_n41_), .c(x10), .O(new_n112_));
  nand3  g88(.a(new_n112_), .b(new_n111_), .c(new_n108_), .O(z8));
endmodule


