// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x12), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x09), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  oai21  g09(.a(x05), .b(new_n30_), .c(x02), .O(new_n34_));
  nor2   g10(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g11(.a(new_n35_), .b(new_n32_), .c(new_n26_), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(x00), .O(z0));
  inv1   g13(.a(x00), .O(new_n38_));
  nand3  g14(.a(new_n26_), .b(new_n28_), .c(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(x02), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n39_), .c(new_n27_), .O(new_n42_));
  nand3  g18(.a(new_n40_), .b(x04), .c(x02), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n42_), .c(new_n38_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n26_), .O(z1));
  inv1   g22(.a(x07), .O(new_n47_));
  nand3  g23(.a(new_n26_), .b(x10), .c(new_n27_), .O(new_n48_));
  inv1   g24(.a(x10), .O(new_n49_));
  nand3  g25(.a(x12), .b(new_n49_), .c(x09), .O(new_n50_));
  aoi21  g26(.a(new_n50_), .b(new_n48_), .c(x08), .O(new_n51_));
  nand4  g27(.a(x12), .b(new_n49_), .c(x09), .d(new_n38_), .O(new_n52_));
  inv1   g28(.a(new_n52_), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n51_), .c(new_n47_), .O(new_n54_));
  nor2   g30(.a(x10), .b(x07), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(new_n25_), .b(new_n56_), .O(new_n57_));
  inv1   g33(.a(new_n57_), .O(new_n58_));
  oai21  g34(.a(new_n55_), .b(x09), .c(new_n58_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n27_), .c(new_n38_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n33_), .O(new_n62_));
  oai21  g38(.a(x08), .b(x07), .c(x00), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n26_), .c(x02), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n62_), .O(z2));
  aoi21  g41(.a(new_n47_), .b(x00), .c(new_n25_), .O(new_n66_));
  inv1   g42(.a(x11), .O(new_n67_));
  nor2   g43(.a(x09), .b(x07), .O(new_n68_));
  oai22  g44(.a(new_n68_), .b(new_n67_), .c(new_n25_), .d(x00), .O(new_n69_));
  oai21  g45(.a(new_n66_), .b(new_n56_), .c(new_n69_), .O(z3));
  aoi21  g46(.a(new_n25_), .b(x09), .c(x00), .O(new_n71_));
  oai22  g47(.a(new_n58_), .b(x08), .c(x09), .d(new_n38_), .O(new_n72_));
  oai21  g48(.a(new_n72_), .b(new_n71_), .c(new_n47_), .O(new_n73_));
  nand2  g49(.a(new_n67_), .b(x07), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n26_), .O(z4));
  oai21  g51(.a(x09), .b(new_n38_), .c(new_n25_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n47_), .O(new_n77_));
  nand2  g53(.a(new_n26_), .b(x07), .O(new_n78_));
  nand2  g54(.a(x10), .b(new_n56_), .O(new_n79_));
  and2   g55(.a(new_n26_), .b(x13), .O(new_n80_));
  nand4  g56(.a(new_n80_), .b(new_n79_), .c(new_n78_), .d(new_n77_), .O(z5));
  oai21  g57(.a(new_n56_), .b(x03), .c(new_n28_), .O(new_n82_));
  inv1   g58(.a(x08), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(x00), .O(new_n84_));
  oai21  g60(.a(x01), .b(x00), .c(new_n84_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  oai21  g62(.a(x09), .b(new_n27_), .c(new_n33_), .O(new_n87_));
  oai21  g63(.a(x02), .b(x00), .c(new_n84_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g65(.a(x14), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(x01), .c(new_n38_), .O(new_n91_));
  nand3  g67(.a(new_n91_), .b(new_n89_), .c(new_n86_), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n49_), .c(new_n47_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n26_), .O(z6));
  inv1   g70(.a(new_n55_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n26_), .O(new_n96_));
  nand2  g72(.a(new_n56_), .b(x02), .O(new_n97_));
  aoi21  g73(.a(new_n97_), .b(new_n58_), .c(x07), .O(new_n98_));
  aoi21  g74(.a(x03), .b(new_n33_), .c(x09), .O(new_n99_));
  oai21  g75(.a(new_n99_), .b(new_n98_), .c(new_n83_), .O(new_n100_));
  nand4  g76(.a(new_n57_), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n101_));
  nand3  g77(.a(new_n101_), .b(new_n100_), .c(new_n96_), .O(z7));
  nand2  g78(.a(new_n57_), .b(x00), .O(new_n103_));
  inv1   g79(.a(new_n103_), .O(new_n104_));
  oai21  g80(.a(new_n104_), .b(new_n71_), .c(new_n47_), .O(new_n105_));
  nand3  g81(.a(new_n105_), .b(new_n79_), .c(new_n78_), .O(z8));
endmodule


