// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x10), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x07), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(x02), .c(new_n30_), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n29_), .c(new_n27_), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n33_), .c(new_n26_), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(x00), .O(z0));
  inv1   g15(.a(x00), .O(new_n40_));
  nand2  g16(.a(x06), .b(x05), .O(new_n41_));
  oai22  g17(.a(new_n41_), .b(new_n30_), .c(x04), .d(x03), .O(new_n42_));
  nand3  g18(.a(x03), .b(new_n28_), .c(x01), .O(new_n43_));
  oai21  g19(.a(new_n42_), .b(new_n28_), .c(new_n43_), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n26_), .c(new_n40_), .O(new_n45_));
  inv1   g21(.a(new_n45_), .O(z1));
  aoi21  g22(.a(x03), .b(new_n28_), .c(x00), .O(new_n47_));
  nor3   g23(.a(x08), .b(x07), .c(x03), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  inv1   g25(.a(x09), .O(new_n50_));
  oai21  g26(.a(x07), .b(new_n28_), .c(new_n50_), .O(new_n51_));
  nor2   g27(.a(new_n50_), .b(x08), .O(new_n52_));
  aoi21  g28(.a(new_n51_), .b(new_n40_), .c(new_n52_), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(x10), .c(new_n49_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n30_), .O(new_n55_));
  inv1   g31(.a(new_n26_), .O(new_n56_));
  inv1   g32(.a(x07), .O(new_n57_));
  inv1   g33(.a(x08), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand2  g35(.a(new_n35_), .b(new_n30_), .O(new_n60_));
  nand3  g36(.a(new_n60_), .b(new_n26_), .c(new_n40_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  aoi21  g38(.a(new_n62_), .b(x02), .c(new_n56_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n55_), .O(z2));
  inv1   g40(.a(x11), .O(new_n65_));
  inv1   g41(.a(x12), .O(new_n66_));
  oai22  g42(.a(new_n66_), .b(x00), .c(new_n65_), .d(new_n57_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n26_), .O(z3));
  oai21  g44(.a(new_n65_), .b(new_n25_), .c(x07), .O(new_n69_));
  nand3  g45(.a(x09), .b(x08), .c(x00), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n57_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(z4));
  inv1   g48(.a(x13), .O(new_n73_));
  nor2   g49(.a(new_n73_), .b(x07), .O(new_n74_));
  nand2  g50(.a(new_n57_), .b(x00), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n66_), .d(new_n25_), .O(z5));
  oai21  g52(.a(x09), .b(new_n27_), .c(new_n30_), .O(new_n77_));
  nor2   g53(.a(x02), .b(x00), .O(new_n78_));
  nor2   g54(.a(x08), .b(new_n40_), .O(new_n79_));
  oai21  g55(.a(new_n79_), .b(new_n78_), .c(new_n77_), .O(new_n80_));
  oai21  g56(.a(new_n50_), .b(x03), .c(new_n28_), .O(new_n81_));
  nand2  g57(.a(x08), .b(x00), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n81_), .c(new_n30_), .O(new_n83_));
  inv1   g59(.a(x14), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(x01), .c(new_n40_), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n83_), .c(new_n80_), .O(new_n86_));
  nand3  g62(.a(new_n86_), .b(new_n25_), .c(new_n57_), .O(new_n87_));
  inv1   g63(.a(new_n87_), .O(z6));
  nand3  g64(.a(x09), .b(x03), .c(new_n28_), .O(new_n89_));
  nand4  g65(.a(new_n25_), .b(new_n58_), .c(new_n57_), .d(x02), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n30_), .O(new_n92_));
  nand2  g68(.a(x03), .b(new_n30_), .O(new_n93_));
  nand2  g69(.a(new_n25_), .b(new_n57_), .O(new_n94_));
  aoi21  g70(.a(new_n93_), .b(new_n58_), .c(new_n94_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n92_), .O(z7));
  nand4  g72(.a(new_n66_), .b(x03), .c(new_n28_), .d(new_n30_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(x09), .O(new_n98_));
  aoi21  g74(.a(new_n50_), .b(new_n40_), .c(new_n94_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n98_), .O(z8));
endmodule


