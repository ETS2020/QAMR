// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_;
  inv1   g00(.a(x08), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x03), .O(new_n28_));
  inv1   g04(.a(x02), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x01), .O(new_n30_));
  inv1   g06(.a(x04), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x02), .O(new_n32_));
  aoi21  g08(.a(new_n32_), .b(new_n30_), .c(new_n28_), .O(new_n33_));
  inv1   g09(.a(x01), .O(new_n34_));
  inv1   g10(.a(x05), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x04), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(x02), .O(new_n37_));
  nor2   g13(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  oai21  g14(.a(new_n38_), .b(new_n33_), .c(new_n27_), .O(new_n39_));
  nor2   g15(.a(new_n39_), .b(x00), .O(z0));
  inv1   g16(.a(x00), .O(new_n41_));
  nand3  g17(.a(new_n27_), .b(new_n29_), .c(x01), .O(new_n42_));
  nand3  g18(.a(x06), .b(x05), .c(x01), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x02), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(new_n28_), .O(new_n45_));
  nand3  g21(.a(new_n43_), .b(x04), .c(x02), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n45_), .c(new_n41_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n27_), .O(z1));
  oai21  g25(.a(x10), .b(x07), .c(new_n28_), .O(new_n50_));
  inv1   g26(.a(x07), .O(new_n51_));
  nand3  g27(.a(new_n26_), .b(x09), .c(new_n51_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n50_), .c(new_n29_), .O(new_n53_));
  nor2   g29(.a(x10), .b(x08), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n34_), .c(new_n36_), .O(new_n55_));
  aoi22  g31(.a(new_n55_), .b(x02), .c(new_n53_), .d(new_n34_), .O(new_n56_));
  nor2   g32(.a(x03), .b(x01), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(x02), .c(new_n51_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(x10), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n25_), .O(new_n60_));
  oai21  g36(.a(new_n56_), .b(x00), .c(new_n60_), .O(z2));
  inv1   g37(.a(x11), .O(new_n62_));
  inv1   g38(.a(x12), .O(new_n63_));
  oai22  g39(.a(new_n63_), .b(x00), .c(new_n62_), .d(new_n51_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n27_), .O(z3));
  oai21  g41(.a(new_n26_), .b(new_n51_), .c(new_n25_), .O(new_n66_));
  inv1   g42(.a(x09), .O(new_n67_));
  oai21  g43(.a(new_n67_), .b(new_n41_), .c(new_n51_), .O(new_n68_));
  nand2  g44(.a(new_n62_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z4));
  inv1   g46(.a(x13), .O(new_n71_));
  nor2   g47(.a(new_n71_), .b(x12), .O(new_n72_));
  nor2   g48(.a(x10), .b(new_n25_), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n72_), .c(new_n51_), .d(new_n41_), .O(z5));
  oai21  g50(.a(new_n67_), .b(x03), .c(new_n29_), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n34_), .O(new_n76_));
  oai21  g52(.a(x09), .b(new_n28_), .c(new_n34_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n29_), .O(new_n78_));
  inv1   g54(.a(x14), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(x02), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n78_), .c(new_n76_), .O(new_n81_));
  nand3  g57(.a(new_n81_), .b(new_n51_), .c(new_n41_), .O(new_n82_));
  aoi21  g58(.a(new_n82_), .b(x08), .c(x10), .O(z6));
  nand4  g59(.a(x09), .b(x03), .c(new_n29_), .d(new_n34_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n26_), .c(x08), .d(new_n51_), .O(z7));
  aoi21  g61(.a(x03), .b(new_n29_), .c(x07), .O(new_n86_));
  nand2  g62(.a(new_n63_), .b(new_n34_), .O(new_n87_));
  oai21  g63(.a(new_n87_), .b(new_n86_), .c(x09), .O(new_n88_));
  aoi21  g64(.a(new_n67_), .b(new_n41_), .c(x07), .O(new_n89_));
  nand3  g65(.a(new_n89_), .b(new_n88_), .c(new_n73_), .O(z8));
endmodule


