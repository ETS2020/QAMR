// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_;
  nor2   g00(.a(x10), .b(x06), .O(new_n25_));
  inv1   g01(.a(new_n25_), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nor2   g04(.a(x05), .b(new_n28_), .O(new_n29_));
  nand2  g05(.a(x03), .b(new_n27_), .O(new_n30_));
  oai21  g06(.a(new_n29_), .b(new_n27_), .c(new_n30_), .O(new_n31_));
  nand3  g07(.a(new_n31_), .b(new_n26_), .c(x01), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  nand2  g09(.a(x06), .b(new_n33_), .O(new_n34_));
  inv1   g10(.a(x06), .O(new_n35_));
  nand2  g11(.a(x10), .b(new_n35_), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand4  g13(.a(new_n37_), .b(new_n28_), .c(x03), .d(x02), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n32_), .c(x00), .O(z0));
  nor3   g15(.a(new_n25_), .b(x02), .c(new_n33_), .O(new_n40_));
  nand2  g16(.a(new_n37_), .b(new_n28_), .O(new_n41_));
  inv1   g17(.a(x05), .O(new_n42_));
  nand2  g18(.a(x06), .b(new_n42_), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n41_), .c(new_n27_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n40_), .c(x03), .O(new_n45_));
  oai21  g21(.a(new_n42_), .b(new_n33_), .c(x06), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(new_n36_), .O(new_n47_));
  nand3  g23(.a(new_n47_), .b(x04), .c(x02), .O(new_n48_));
  aoi21  g24(.a(new_n48_), .b(new_n45_), .c(x00), .O(z1));
  oai21  g25(.a(x04), .b(x03), .c(new_n37_), .O(new_n50_));
  oai22  g26(.a(new_n25_), .b(x04), .c(new_n35_), .d(new_n42_), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(x01), .O(new_n52_));
  nor3   g28(.a(x10), .b(x07), .c(x01), .O(new_n53_));
  oai21  g29(.a(new_n53_), .b(new_n29_), .c(x06), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(new_n55_));
  inv1   g31(.a(x03), .O(new_n56_));
  inv1   g32(.a(x07), .O(new_n57_));
  inv1   g33(.a(x10), .O(new_n58_));
  oai21  g34(.a(new_n57_), .b(new_n35_), .c(new_n58_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nand4  g36(.a(new_n58_), .b(x09), .c(new_n57_), .d(x06), .O(new_n61_));
  aoi21  g37(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  aoi21  g38(.a(new_n55_), .b(x02), .c(new_n62_), .O(new_n63_));
  inv1   g39(.a(x08), .O(new_n64_));
  nand3  g40(.a(new_n58_), .b(x09), .c(x06), .O(new_n65_));
  oai21  g41(.a(new_n58_), .b(x03), .c(new_n65_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  oai21  g43(.a(new_n25_), .b(new_n27_), .c(new_n67_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n64_), .c(new_n57_), .O(new_n69_));
  oai21  g45(.a(new_n63_), .b(x00), .c(new_n69_), .O(z2));
  inv1   g46(.a(x11), .O(new_n71_));
  inv1   g47(.a(x12), .O(new_n72_));
  oai22  g48(.a(new_n72_), .b(x00), .c(new_n71_), .d(new_n57_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n26_), .O(z3));
  nand3  g50(.a(x09), .b(x08), .c(x00), .O(new_n75_));
  nand2  g51(.a(new_n75_), .b(new_n57_), .O(new_n76_));
  nand2  g52(.a(new_n71_), .b(x07), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n26_), .O(z4));
  inv1   g54(.a(x00), .O(new_n79_));
  nand4  g55(.a(x13), .b(new_n72_), .c(new_n57_), .d(new_n79_), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x06), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n58_), .O(z5));
  inv1   g58(.a(x09), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(x03), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n27_), .c(new_n79_), .O(new_n85_));
  nand3  g61(.a(x09), .b(new_n56_), .c(new_n33_), .O(new_n86_));
  oai21  g62(.a(x02), .b(new_n33_), .c(new_n86_), .O(new_n87_));
  oai21  g63(.a(new_n87_), .b(new_n85_), .c(new_n64_), .O(new_n88_));
  oai21  g64(.a(new_n83_), .b(x03), .c(new_n27_), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n33_), .O(new_n90_));
  oai21  g66(.a(x09), .b(new_n56_), .c(new_n33_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n27_), .O(new_n92_));
  inv1   g68(.a(x14), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(x02), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n92_), .c(new_n90_), .O(new_n95_));
  nand2  g71(.a(new_n95_), .b(new_n79_), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  nand4  g73(.a(new_n97_), .b(new_n58_), .c(new_n57_), .d(x06), .O(new_n98_));
  inv1   g74(.a(new_n98_), .O(z6));
  nand3  g75(.a(x09), .b(x03), .c(new_n33_), .O(new_n100_));
  nand2  g76(.a(x06), .b(x01), .O(new_n101_));
  nand3  g77(.a(new_n58_), .b(new_n64_), .c(new_n57_), .O(new_n102_));
  oai21  g78(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  nand2  g79(.a(new_n103_), .b(new_n27_), .O(new_n104_));
  oai21  g80(.a(x08), .b(new_n27_), .c(new_n57_), .O(new_n105_));
  nand2  g81(.a(new_n105_), .b(x06), .O(new_n106_));
  nand2  g82(.a(new_n64_), .b(new_n56_), .O(new_n107_));
  nor2   g83(.a(x10), .b(new_n35_), .O(new_n108_));
  nand4  g84(.a(new_n108_), .b(new_n107_), .c(new_n106_), .d(new_n104_), .O(z7));
  nand4  g85(.a(new_n72_), .b(x03), .c(new_n27_), .d(new_n33_), .O(new_n110_));
  nand2  g86(.a(new_n110_), .b(x09), .O(new_n111_));
  aoi22  g87(.a(new_n83_), .b(new_n79_), .c(x07), .d(x06), .O(new_n112_));
  nand3  g88(.a(new_n112_), .b(new_n111_), .c(new_n108_), .O(z8));
endmodule


