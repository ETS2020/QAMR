// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_;
  nand2  g00(.a(x13), .b(x10), .O(new_n25_));
  inv1   g01(.a(x00), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  oai21  g04(.a(x05), .b(new_n28_), .c(x02), .O(new_n29_));
  inv1   g05(.a(x02), .O(new_n30_));
  nand3  g06(.a(new_n25_), .b(x03), .c(new_n30_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  nand3  g08(.a(new_n28_), .b(x03), .c(x02), .O(new_n33_));
  inv1   g09(.a(new_n33_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n26_), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n25_), .O(z0));
  inv1   g12(.a(x03), .O(new_n37_));
  nand2  g13(.a(new_n28_), .b(new_n37_), .O(new_n38_));
  nand3  g14(.a(x06), .b(x05), .c(x01), .O(new_n39_));
  nand3  g15(.a(new_n39_), .b(new_n38_), .c(x02), .O(new_n40_));
  nand3  g16(.a(x03), .b(new_n30_), .c(x01), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n25_), .c(new_n26_), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(z1));
  inv1   g20(.a(x07), .O(new_n45_));
  inv1   g21(.a(x10), .O(new_n46_));
  nand2  g22(.a(new_n46_), .b(x09), .O(new_n47_));
  inv1   g23(.a(x13), .O(new_n48_));
  nand3  g24(.a(new_n48_), .b(x10), .c(new_n37_), .O(new_n49_));
  aoi21  g25(.a(new_n49_), .b(new_n47_), .c(x08), .O(new_n50_));
  nand3  g26(.a(new_n46_), .b(x09), .c(new_n26_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n50_), .c(new_n45_), .O(new_n53_));
  nand2  g29(.a(new_n48_), .b(x10), .O(new_n54_));
  oai21  g30(.a(x10), .b(new_n45_), .c(new_n54_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n37_), .c(new_n26_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n27_), .O(new_n58_));
  oai21  g34(.a(x08), .b(x07), .c(x00), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n25_), .c(x02), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n58_), .O(z2));
  nand2  g37(.a(x11), .b(x07), .O(new_n62_));
  nand2  g38(.a(x12), .b(new_n26_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n25_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z3));
  nand3  g41(.a(x09), .b(x08), .c(x00), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n45_), .O(new_n67_));
  inv1   g43(.a(x11), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n67_), .c(new_n25_), .O(z4));
  oai22  g46(.a(x13), .b(x12), .c(x10), .d(new_n26_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n45_), .O(new_n72_));
  oai21  g48(.a(x12), .b(x07), .c(new_n46_), .O(new_n73_));
  nand3  g49(.a(new_n73_), .b(new_n72_), .c(new_n54_), .O(z5));
  nand2  g50(.a(x08), .b(x00), .O(new_n75_));
  aoi21  g51(.a(x09), .b(new_n37_), .c(x02), .O(new_n76_));
  inv1   g52(.a(x09), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(x03), .c(new_n30_), .O(new_n78_));
  oai21  g54(.a(new_n76_), .b(x01), .c(new_n78_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  aoi21  g56(.a(x14), .b(x02), .c(x00), .O(new_n81_));
  nor2   g57(.a(x08), .b(new_n26_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n80_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n46_), .c(new_n45_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n25_), .O(z6));
  inv1   g62(.a(x08), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(new_n45_), .c(x02), .O(new_n88_));
  nand3  g64(.a(x09), .b(x03), .c(new_n30_), .O(new_n89_));
  aoi21  g65(.a(new_n89_), .b(new_n88_), .c(x01), .O(new_n90_));
  oai21  g66(.a(new_n37_), .b(x01), .c(new_n87_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n45_), .O(new_n92_));
  oai21  g68(.a(new_n92_), .b(new_n90_), .c(new_n46_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n54_), .O(z7));
  inv1   g70(.a(x12), .O(new_n95_));
  nand4  g71(.a(new_n95_), .b(x03), .c(new_n30_), .d(new_n27_), .O(new_n96_));
  oai21  g72(.a(x09), .b(x00), .c(new_n45_), .O(new_n97_));
  aoi21  g73(.a(new_n96_), .b(x09), .c(new_n97_), .O(new_n98_));
  oai21  g74(.a(new_n98_), .b(x10), .c(new_n54_), .O(z8));
endmodule


