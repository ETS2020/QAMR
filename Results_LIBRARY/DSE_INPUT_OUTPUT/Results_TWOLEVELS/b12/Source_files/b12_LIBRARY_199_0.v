// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nand4  g04(.a(x10), .b(new_n28_), .c(x02), .d(new_n27_), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n26_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  oai21  g11(.a(x04), .b(x03), .c(x10), .O(new_n36_));
  inv1   g12(.a(x06), .O(new_n37_));
  aoi21  g13(.a(new_n32_), .b(x01), .c(new_n37_), .O(new_n38_));
  oai22  g14(.a(new_n38_), .b(new_n28_), .c(new_n36_), .d(x01), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x02), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(x03), .c(x01), .O(new_n41_));
  inv1   g17(.a(new_n41_), .O(new_n42_));
  aoi21  g18(.a(new_n39_), .b(x02), .c(new_n42_), .O(new_n43_));
  inv1   g19(.a(x10), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n27_), .O(new_n45_));
  oai21  g21(.a(new_n43_), .b(x00), .c(new_n45_), .O(z1));
  inv1   g22(.a(x00), .O(new_n47_));
  inv1   g23(.a(x03), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(x02), .c(new_n47_), .O(new_n49_));
  nor2   g25(.a(x08), .b(x07), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n49_), .c(x01), .O(new_n52_));
  nand2  g28(.a(new_n50_), .b(x02), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n52_), .c(x10), .O(new_n55_));
  nor2   g31(.a(x05), .b(x04), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(x04), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n47_), .O(new_n58_));
  oai21  g34(.a(x08), .b(x07), .c(new_n58_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(x02), .c(x01), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n55_), .O(z2));
  inv1   g37(.a(x07), .O(new_n62_));
  inv1   g38(.a(x11), .O(new_n63_));
  inv1   g39(.a(x12), .O(new_n64_));
  oai22  g40(.a(new_n64_), .b(x00), .c(new_n63_), .d(new_n62_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n45_), .O(z3));
  nand3  g42(.a(x09), .b(x08), .c(x00), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n62_), .O(new_n68_));
  nand2  g44(.a(new_n63_), .b(x07), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n45_), .O(z4));
  nand4  g46(.a(x13), .b(new_n64_), .c(new_n62_), .d(new_n47_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(x01), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n44_), .O(z5));
  aoi21  g49(.a(new_n26_), .b(x14), .c(x00), .O(new_n74_));
  nor2   g50(.a(x08), .b(new_n47_), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(new_n74_), .c(new_n62_), .O(new_n76_));
  aoi21  g52(.a(new_n76_), .b(x01), .c(x10), .O(z6));
  inv1   g53(.a(x08), .O(new_n78_));
  oai21  g54(.a(new_n78_), .b(x07), .c(x01), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n44_), .O(z7));
  nor3   g56(.a(x10), .b(x07), .c(x02), .O(new_n81_));
  aoi21  g57(.a(new_n57_), .b(x02), .c(new_n81_), .O(new_n82_));
  nor2   g58(.a(x09), .b(x07), .O(new_n83_));
  oai21  g59(.a(new_n82_), .b(x00), .c(new_n83_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(x01), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n44_), .O(z8));
endmodule


