// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand3  g04(.a(x10), .b(new_n28_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x05), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x04), .O(new_n33_));
  nand3  g09(.a(new_n33_), .b(x02), .c(x01), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g11(.a(x00), .O(new_n36_));
  nand2  g12(.a(x06), .b(x05), .O(new_n37_));
  oai22  g13(.a(new_n37_), .b(new_n25_), .c(x04), .d(x03), .O(new_n38_));
  nand4  g14(.a(x10), .b(x03), .c(new_n28_), .d(x01), .O(new_n39_));
  oai21  g15(.a(new_n38_), .b(new_n28_), .c(new_n39_), .O(new_n40_));
  nand2  g16(.a(new_n40_), .b(new_n36_), .O(new_n41_));
  inv1   g17(.a(x10), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n28_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n41_), .O(z1));
  oai21  g20(.a(x03), .b(x01), .c(new_n28_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n36_), .O(new_n46_));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x08), .O(new_n48_));
  oai21  g24(.a(x03), .b(x01), .c(new_n28_), .O(new_n49_));
  nand3  g25(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(new_n46_), .c(new_n43_), .O(z2));
  inv1   g27(.a(x11), .O(new_n52_));
  inv1   g28(.a(x12), .O(new_n53_));
  oai22  g29(.a(new_n53_), .b(x00), .c(new_n52_), .d(new_n47_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n43_), .O(z3));
  nand3  g31(.a(x09), .b(x08), .c(x00), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g33(.a(new_n52_), .b(x07), .O(new_n58_));
  nand3  g34(.a(new_n58_), .b(new_n57_), .c(new_n43_), .O(z4));
  nand3  g35(.a(x13), .b(new_n53_), .c(new_n42_), .O(new_n60_));
  inv1   g36(.a(new_n60_), .O(new_n61_));
  nand2  g37(.a(new_n47_), .b(x00), .O(new_n62_));
  nand2  g38(.a(x07), .b(x02), .O(new_n63_));
  nand4  g39(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n43_), .O(z5));
  nand2  g40(.a(x14), .b(x01), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n36_), .O(new_n66_));
  oai21  g42(.a(x08), .b(new_n36_), .c(new_n66_), .O(new_n67_));
  nand4  g43(.a(new_n67_), .b(new_n42_), .c(new_n47_), .d(x02), .O(new_n68_));
  inv1   g44(.a(new_n68_), .O(z6));
  oai21  g45(.a(new_n48_), .b(x07), .c(x02), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n42_), .O(z7));
  nand2  g47(.a(new_n42_), .b(new_n47_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n33_), .c(new_n25_), .O(new_n73_));
  inv1   g49(.a(x09), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  aoi21  g51(.a(new_n73_), .b(new_n36_), .c(new_n75_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n28_), .c(new_n42_), .O(z8));
endmodule


