// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  nand2  g11(.a(x14), .b(x10), .O(new_n36_));
  nand2  g12(.a(new_n36_), .b(new_n35_), .O(z0));
  oai21  g13(.a(x04), .b(x03), .c(new_n29_), .O(new_n38_));
  nand2  g14(.a(x06), .b(x05), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(x04), .O(new_n40_));
  aoi21  g16(.a(new_n40_), .b(new_n38_), .c(new_n27_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x02), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(x03), .c(x01), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n41_), .c(new_n25_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n36_), .O(z1));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(x09), .O(new_n49_));
  inv1   g25(.a(x14), .O(new_n50_));
  nand3  g26(.a(new_n50_), .b(x10), .c(new_n26_), .O(new_n51_));
  aoi21  g27(.a(new_n51_), .b(new_n49_), .c(x08), .O(new_n52_));
  nand3  g28(.a(new_n48_), .b(x09), .c(new_n25_), .O(new_n53_));
  inv1   g29(.a(new_n53_), .O(new_n54_));
  oai21  g30(.a(new_n54_), .b(new_n52_), .c(new_n47_), .O(new_n55_));
  nand2  g31(.a(new_n50_), .b(x10), .O(new_n56_));
  oai21  g32(.a(x10), .b(new_n47_), .c(new_n56_), .O(new_n57_));
  nand3  g33(.a(new_n57_), .b(new_n26_), .c(new_n25_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  nand2  g35(.a(new_n59_), .b(new_n29_), .O(new_n60_));
  oai21  g36(.a(x08), .b(x07), .c(x00), .O(new_n61_));
  nand3  g37(.a(new_n61_), .b(new_n36_), .c(x02), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n60_), .O(z2));
  inv1   g39(.a(x11), .O(new_n64_));
  inv1   g40(.a(x12), .O(new_n65_));
  oai22  g41(.a(new_n65_), .b(x00), .c(new_n64_), .d(new_n47_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n36_), .O(z3));
  nand3  g43(.a(x09), .b(x08), .c(x00), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n47_), .O(new_n69_));
  nand2  g45(.a(new_n64_), .b(x07), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n36_), .O(z4));
  inv1   g47(.a(x13), .O(new_n72_));
  nor2   g48(.a(new_n72_), .b(x00), .O(new_n73_));
  nand4  g49(.a(new_n73_), .b(new_n65_), .c(new_n48_), .d(new_n47_), .O(z5));
  nor2   g50(.a(x02), .b(x00), .O(new_n75_));
  nor2   g51(.a(x08), .b(new_n25_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  nand2  g53(.a(x08), .b(x00), .O(new_n78_));
  aoi21  g54(.a(x09), .b(new_n26_), .c(x02), .O(new_n79_));
  inv1   g55(.a(x09), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(x03), .c(new_n27_), .O(new_n81_));
  oai21  g57(.a(new_n79_), .b(x01), .c(new_n81_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n78_), .O(new_n83_));
  nand3  g59(.a(new_n50_), .b(x02), .c(new_n25_), .O(new_n84_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(new_n77_), .O(new_n85_));
  nand3  g61(.a(new_n85_), .b(new_n48_), .c(new_n47_), .O(new_n86_));
  inv1   g62(.a(new_n86_), .O(z6));
  nand3  g63(.a(x09), .b(x03), .c(new_n27_), .O(new_n88_));
  inv1   g64(.a(x08), .O(new_n89_));
  nand4  g65(.a(new_n48_), .b(new_n89_), .c(new_n47_), .d(x02), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n29_), .O(new_n92_));
  oai21  g68(.a(new_n26_), .b(x01), .c(new_n89_), .O(new_n93_));
  nand4  g69(.a(new_n93_), .b(new_n92_), .c(new_n48_), .d(new_n47_), .O(z7));
  nand3  g70(.a(x03), .b(new_n27_), .c(new_n29_), .O(new_n95_));
  aoi21  g71(.a(new_n95_), .b(new_n47_), .c(x12), .O(new_n96_));
  aoi21  g72(.a(new_n80_), .b(new_n25_), .c(x07), .O(new_n97_));
  oai21  g73(.a(new_n96_), .b(new_n80_), .c(new_n97_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n56_), .O(z8));
endmodule


