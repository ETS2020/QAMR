// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  nand2  g00(.a(x05), .b(x01), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x04), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x03), .O(new_n27_));
  inv1   g03(.a(x10), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(new_n28_), .c(x01), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(new_n27_), .O(new_n32_));
  nand2  g08(.a(new_n32_), .b(x02), .O(new_n33_));
  inv1   g09(.a(x02), .O(new_n34_));
  nand3  g10(.a(x03), .b(new_n34_), .c(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n33_), .c(x00), .O(z0));
  inv1   g12(.a(x00), .O(new_n37_));
  inv1   g13(.a(x03), .O(new_n38_));
  inv1   g14(.a(x04), .O(new_n39_));
  nand2  g15(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g16(.a(x06), .b(x05), .c(x01), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(x02), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  nand2  g20(.a(x10), .b(new_n38_), .O(new_n45_));
  nand2  g21(.a(new_n45_), .b(new_n44_), .O(z1));
  inv1   g22(.a(x01), .O(new_n47_));
  nand3  g23(.a(x07), .b(new_n47_), .c(new_n37_), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n28_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n38_), .O(new_n50_));
  nor2   g26(.a(x10), .b(x07), .O(new_n51_));
  oai21  g27(.a(new_n51_), .b(x03), .c(new_n47_), .O(new_n52_));
  aoi22  g28(.a(new_n28_), .b(x01), .c(new_n29_), .d(x04), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n27_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(x02), .O(new_n55_));
  inv1   g31(.a(x07), .O(new_n56_));
  nand4  g32(.a(new_n28_), .b(x09), .c(new_n56_), .d(new_n47_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n37_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  nand3  g36(.a(new_n28_), .b(x09), .c(new_n47_), .O(new_n61_));
  nand2  g37(.a(new_n61_), .b(new_n34_), .O(new_n62_));
  nand3  g38(.a(new_n62_), .b(new_n60_), .c(new_n56_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n59_), .c(new_n50_), .O(z2));
  inv1   g40(.a(x11), .O(new_n65_));
  inv1   g41(.a(x12), .O(new_n66_));
  oai22  g42(.a(new_n66_), .b(x00), .c(new_n65_), .d(new_n56_), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n45_), .O(z3));
  oai21  g44(.a(new_n28_), .b(x03), .c(new_n37_), .O(new_n69_));
  inv1   g45(.a(x09), .O(new_n70_));
  oai21  g46(.a(new_n70_), .b(new_n60_), .c(x03), .O(new_n71_));
  nand2  g47(.a(x01), .b(x00), .O(new_n72_));
  nand2  g48(.a(x09), .b(new_n47_), .O(new_n73_));
  aoi21  g49(.a(new_n73_), .b(new_n72_), .c(x08), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(new_n70_), .c(new_n28_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n71_), .c(new_n69_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n56_), .O(new_n77_));
  nand3  g53(.a(new_n45_), .b(new_n65_), .c(x07), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n77_), .O(z4));
  inv1   g55(.a(x13), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(x12), .O(new_n81_));
  nand4  g57(.a(new_n81_), .b(new_n28_), .c(new_n56_), .d(new_n37_), .O(z5));
  oai21  g58(.a(x09), .b(new_n38_), .c(new_n47_), .O(new_n83_));
  nor2   g59(.a(x02), .b(x00), .O(new_n84_));
  nor2   g60(.a(x08), .b(new_n37_), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n84_), .c(new_n83_), .O(new_n86_));
  oai21  g62(.a(new_n70_), .b(x03), .c(new_n34_), .O(new_n87_));
  nand2  g63(.a(x08), .b(x00), .O(new_n88_));
  nand3  g64(.a(new_n88_), .b(new_n87_), .c(new_n47_), .O(new_n89_));
  inv1   g65(.a(x14), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(x01), .c(new_n37_), .O(new_n91_));
  nand3  g67(.a(new_n91_), .b(new_n89_), .c(new_n86_), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n28_), .c(new_n56_), .O(new_n93_));
  inv1   g69(.a(new_n93_), .O(z6));
  nand3  g70(.a(x09), .b(x03), .c(new_n34_), .O(new_n95_));
  nand4  g71(.a(new_n28_), .b(new_n60_), .c(new_n56_), .d(x02), .O(new_n96_));
  nand2  g72(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(new_n47_), .O(new_n98_));
  oai21  g74(.a(x10), .b(new_n60_), .c(new_n38_), .O(new_n99_));
  oai21  g75(.a(x08), .b(new_n47_), .c(new_n56_), .O(new_n100_));
  nor2   g76(.a(new_n28_), .b(new_n38_), .O(new_n101_));
  aoi21  g77(.a(new_n100_), .b(new_n28_), .c(new_n101_), .O(new_n102_));
  nand3  g78(.a(new_n102_), .b(new_n99_), .c(new_n98_), .O(z7));
  nand2  g79(.a(x05), .b(x02), .O(new_n104_));
  oai21  g80(.a(new_n104_), .b(x00), .c(new_n70_), .O(new_n105_));
  nand2  g81(.a(new_n105_), .b(x01), .O(new_n106_));
  nor3   g82(.a(x07), .b(x01), .c(x00), .O(new_n107_));
  oai21  g83(.a(new_n107_), .b(x09), .c(x02), .O(new_n108_));
  oai21  g84(.a(x09), .b(x02), .c(x03), .O(new_n109_));
  nand3  g85(.a(new_n109_), .b(new_n56_), .c(new_n37_), .O(new_n110_));
  aoi21  g86(.a(x09), .b(new_n38_), .c(x07), .O(new_n111_));
  nand4  g87(.a(new_n111_), .b(new_n110_), .c(new_n108_), .d(new_n106_), .O(new_n112_));
  nand2  g88(.a(new_n112_), .b(new_n28_), .O(new_n113_));
  nand3  g89(.a(new_n29_), .b(x02), .c(new_n37_), .O(new_n114_));
  aoi21  g90(.a(x12), .b(x09), .c(x10), .O(new_n115_));
  nand2  g91(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g92(.a(new_n116_), .b(x03), .O(new_n117_));
  nand2  g93(.a(new_n117_), .b(new_n113_), .O(z8));
endmodule


