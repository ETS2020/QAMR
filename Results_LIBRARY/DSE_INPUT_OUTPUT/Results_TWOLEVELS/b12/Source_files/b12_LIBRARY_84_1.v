// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x07), .O(new_n25_));
  nand2  g01(.a(x10), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  nand3  g04(.a(new_n28_), .b(x02), .c(new_n27_), .O(new_n29_));
  oai21  g05(.a(x02), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(new_n26_), .c(x03), .O(new_n31_));
  nand2  g07(.a(x10), .b(new_n25_), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x04), .O(new_n34_));
  nand4  g10(.a(new_n34_), .b(new_n32_), .c(x02), .d(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n31_), .c(x00), .O(z0));
  inv1   g12(.a(x02), .O(new_n37_));
  oai21  g13(.a(x04), .b(x03), .c(new_n27_), .O(new_n38_));
  inv1   g14(.a(x06), .O(new_n39_));
  nand2  g15(.a(x06), .b(x05), .O(new_n40_));
  aoi22  g16(.a(new_n40_), .b(x03), .c(new_n39_), .d(x04), .O(new_n41_));
  aoi21  g17(.a(new_n41_), .b(new_n38_), .c(new_n37_), .O(new_n42_));
  nand3  g18(.a(x03), .b(new_n37_), .c(x01), .O(new_n43_));
  inv1   g19(.a(new_n43_), .O(new_n44_));
  oai21  g20(.a(new_n44_), .b(new_n42_), .c(new_n26_), .O(new_n45_));
  nand4  g21(.a(new_n32_), .b(new_n33_), .c(x04), .d(x02), .O(new_n46_));
  aoi21  g22(.a(new_n46_), .b(new_n45_), .c(x00), .O(z1));
  inv1   g23(.a(x00), .O(new_n48_));
  nand2  g24(.a(new_n34_), .b(new_n27_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n32_), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand2  g27(.a(new_n51_), .b(new_n25_), .O(new_n52_));
  oai21  g28(.a(x04), .b(x03), .c(x07), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g30(.a(new_n54_), .b(new_n27_), .O(new_n55_));
  aoi21  g31(.a(new_n55_), .b(new_n50_), .c(new_n37_), .O(new_n56_));
  inv1   g32(.a(x03), .O(new_n57_));
  nand2  g33(.a(x07), .b(new_n57_), .O(new_n58_));
  nand3  g34(.a(new_n51_), .b(x09), .c(new_n25_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  oai21  g36(.a(new_n60_), .b(new_n56_), .c(new_n48_), .O(new_n61_));
  inv1   g37(.a(x08), .O(new_n62_));
  inv1   g38(.a(x09), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(x01), .c(new_n37_), .O(new_n64_));
  nand4  g40(.a(new_n64_), .b(new_n51_), .c(new_n62_), .d(new_n25_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n61_), .O(z2));
  inv1   g42(.a(x12), .O(new_n67_));
  nand2  g43(.a(new_n52_), .b(x11), .O(new_n68_));
  oai21  g44(.a(new_n67_), .b(x00), .c(new_n68_), .O(new_n69_));
  nand2  g45(.a(new_n69_), .b(new_n26_), .O(z3));
  nand2  g46(.a(x08), .b(x00), .O(new_n71_));
  nand3  g47(.a(new_n71_), .b(x09), .c(new_n27_), .O(new_n72_));
  oai21  g48(.a(new_n34_), .b(new_n37_), .c(new_n27_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n48_), .O(new_n74_));
  nand3  g50(.a(new_n62_), .b(x01), .c(x00), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n74_), .c(new_n72_), .d(x09), .O(new_n76_));
  nand3  g52(.a(new_n76_), .b(new_n51_), .c(new_n25_), .O(new_n77_));
  inv1   g53(.a(x11), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(x07), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n77_), .O(z4));
  nor2   g56(.a(x07), .b(new_n48_), .O(new_n81_));
  nand2  g57(.a(x13), .b(new_n67_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n81_), .c(new_n51_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n25_), .O(z5));
  xnor2a g60(.a(x02), .b(x01), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(x00), .c(new_n75_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n51_), .O(new_n87_));
  nand2  g63(.a(new_n62_), .b(new_n27_), .O(new_n88_));
  inv1   g64(.a(x14), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n48_), .O(new_n90_));
  aoi21  g66(.a(new_n90_), .b(new_n88_), .c(new_n37_), .O(new_n91_));
  nand3  g67(.a(x09), .b(new_n57_), .c(new_n27_), .O(new_n92_));
  nand3  g68(.a(new_n63_), .b(x03), .c(new_n37_), .O(new_n93_));
  aoi22  g69(.a(new_n93_), .b(new_n92_), .c(x08), .d(x00), .O(new_n94_));
  nor3   g70(.a(new_n94_), .b(new_n91_), .c(x10), .O(new_n95_));
  aoi21  g71(.a(new_n95_), .b(new_n87_), .c(x07), .O(z6));
  nand2  g72(.a(new_n25_), .b(x02), .O(new_n97_));
  nand3  g73(.a(new_n97_), .b(x03), .c(new_n27_), .O(new_n98_));
  nand4  g74(.a(x09), .b(x03), .c(new_n37_), .d(new_n27_), .O(new_n99_));
  inv1   g75(.a(new_n99_), .O(new_n100_));
  aoi21  g76(.a(new_n98_), .b(new_n62_), .c(new_n100_), .O(new_n101_));
  oai21  g77(.a(new_n101_), .b(x10), .c(new_n25_), .O(z7));
  nand4  g78(.a(new_n67_), .b(x03), .c(new_n37_), .d(new_n27_), .O(new_n103_));
  nor2   g79(.a(x09), .b(x00), .O(new_n104_));
  aoi21  g80(.a(new_n103_), .b(x09), .c(new_n104_), .O(new_n105_));
  oai21  g81(.a(new_n105_), .b(x10), .c(new_n25_), .O(z8));
endmodule


