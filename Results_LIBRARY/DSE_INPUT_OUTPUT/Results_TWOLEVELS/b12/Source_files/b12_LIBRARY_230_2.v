// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n89_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x02), .O(new_n26_));
  nand2  g02(.a(x03), .b(new_n26_), .O(new_n27_));
  nand2  g03(.a(x05), .b(x02), .O(new_n28_));
  aoi21  g04(.a(new_n28_), .b(new_n27_), .c(new_n25_), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x03), .c(x02), .O(new_n31_));
  inv1   g07(.a(new_n31_), .O(new_n32_));
  oai21  g08(.a(new_n32_), .b(new_n29_), .c(x10), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  aoi21  g10(.a(new_n34_), .b(x04), .c(x03), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x02), .c(x01), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n33_), .c(x00), .O(z0));
  inv1   g13(.a(x03), .O(new_n38_));
  nand2  g14(.a(new_n30_), .b(new_n38_), .O(new_n39_));
  nand3  g15(.a(x06), .b(x05), .c(x01), .O(new_n40_));
  nand3  g16(.a(new_n40_), .b(new_n39_), .c(x02), .O(new_n41_));
  nand4  g17(.a(x10), .b(x03), .c(new_n26_), .d(x01), .O(new_n42_));
  and2   g18(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nor2   g19(.a(x10), .b(new_n38_), .O(new_n44_));
  inv1   g20(.a(new_n44_), .O(new_n45_));
  oai21  g21(.a(new_n43_), .b(x00), .c(new_n45_), .O(z1));
  inv1   g22(.a(x07), .O(new_n47_));
  inv1   g23(.a(x10), .O(new_n48_));
  nand2  g24(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n38_), .O(new_n50_));
  nand2  g26(.a(new_n50_), .b(new_n25_), .O(new_n51_));
  nor2   g27(.a(new_n48_), .b(new_n34_), .O(new_n52_));
  oai21  g28(.a(new_n52_), .b(new_n35_), .c(x01), .O(new_n53_));
  nor2   g29(.a(new_n48_), .b(x04), .O(new_n54_));
  aoi22  g30(.a(new_n54_), .b(x03), .c(new_n34_), .d(x04), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(new_n56_));
  inv1   g32(.a(x09), .O(new_n57_));
  nor2   g33(.a(x10), .b(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n47_), .O(new_n59_));
  aoi21  g35(.a(new_n59_), .b(new_n50_), .c(x01), .O(new_n60_));
  aoi21  g36(.a(new_n56_), .b(x02), .c(new_n60_), .O(new_n61_));
  nor2   g37(.a(new_n48_), .b(x03), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n58_), .c(new_n25_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n26_), .c(x08), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n47_), .c(new_n44_), .O(new_n65_));
  oai21  g41(.a(new_n61_), .b(x00), .c(new_n65_), .O(z2));
  inv1   g42(.a(x11), .O(new_n67_));
  inv1   g43(.a(x12), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x00), .c(new_n67_), .O(new_n69_));
  oai21  g45(.a(new_n68_), .b(x00), .c(new_n47_), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n45_), .O(z3));
  nand3  g47(.a(x09), .b(x08), .c(x00), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n47_), .O(new_n73_));
  nand2  g49(.a(new_n67_), .b(x07), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n45_), .O(z4));
  inv1   g51(.a(new_n49_), .O(new_n76_));
  oai21  g52(.a(x12), .b(x00), .c(new_n47_), .O(new_n77_));
  nand4  g53(.a(new_n77_), .b(new_n76_), .c(new_n45_), .d(x13), .O(z5));
  oai21  g54(.a(x02), .b(x01), .c(x00), .O(new_n79_));
  nand2  g55(.a(x09), .b(new_n25_), .O(new_n80_));
  aoi21  g56(.a(new_n80_), .b(new_n79_), .c(x08), .O(new_n81_));
  nand2  g57(.a(x14), .b(x02), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(x01), .O(new_n83_));
  oai21  g59(.a(x09), .b(x02), .c(new_n25_), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n83_), .c(x00), .O(new_n85_));
  oai21  g61(.a(new_n85_), .b(new_n81_), .c(new_n47_), .O(new_n86_));
  aoi21  g62(.a(new_n86_), .b(new_n38_), .c(x10), .O(z6));
  nand4  g63(.a(new_n48_), .b(x08), .c(new_n47_), .d(new_n38_), .O(z7));
  nor2   g64(.a(x10), .b(x03), .O(new_n89_));
  nand4  g65(.a(new_n89_), .b(new_n57_), .c(new_n47_), .d(x00), .O(z8));
endmodule


