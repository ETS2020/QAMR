// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_;
  inv1   g00(.a(x05), .O(new_n25_));
  oai21  g01(.a(x10), .b(x04), .c(new_n25_), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(x02), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(x10), .b(new_n25_), .O(new_n29_));
  nand3  g05(.a(new_n29_), .b(x03), .c(new_n28_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x01), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  nand2  g09(.a(x05), .b(new_n33_), .O(new_n34_));
  inv1   g10(.a(x10), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(new_n25_), .O(new_n36_));
  aoi21  g12(.a(new_n36_), .b(new_n34_), .c(x04), .O(new_n37_));
  nand3  g13(.a(new_n37_), .b(x03), .c(x02), .O(new_n38_));
  aoi21  g14(.a(new_n38_), .b(new_n32_), .c(x00), .O(z0));
  inv1   g15(.a(x00), .O(new_n40_));
  inv1   g16(.a(x03), .O(new_n41_));
  nand3  g17(.a(new_n29_), .b(new_n28_), .c(x01), .O(new_n42_));
  inv1   g18(.a(x06), .O(new_n43_));
  oai21  g19(.a(new_n37_), .b(new_n43_), .c(x02), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(new_n45_));
  nand3  g21(.a(new_n36_), .b(new_n34_), .c(x06), .O(new_n46_));
  nand3  g22(.a(new_n46_), .b(x04), .c(x02), .O(new_n47_));
  inv1   g23(.a(new_n47_), .O(new_n48_));
  oai21  g24(.a(new_n48_), .b(new_n45_), .c(new_n40_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n29_), .O(z1));
  nand2  g26(.a(new_n26_), .b(x01), .O(new_n51_));
  nand2  g27(.a(new_n36_), .b(new_n34_), .O(new_n52_));
  oai21  g28(.a(x04), .b(x03), .c(new_n52_), .O(new_n53_));
  inv1   g29(.a(x07), .O(new_n54_));
  nand3  g30(.a(new_n35_), .b(new_n54_), .c(new_n33_), .O(new_n55_));
  nand3  g31(.a(new_n55_), .b(new_n53_), .c(new_n51_), .O(new_n56_));
  nand2  g32(.a(x10), .b(x05), .O(new_n57_));
  oai21  g33(.a(x10), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n41_), .O(new_n59_));
  nand3  g35(.a(new_n35_), .b(x09), .c(new_n54_), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  aoi21  g37(.a(new_n56_), .b(x02), .c(new_n61_), .O(new_n62_));
  inv1   g38(.a(x08), .O(new_n63_));
  inv1   g39(.a(x09), .O(new_n64_));
  oai22  g40(.a(new_n57_), .b(x03), .c(x10), .d(new_n64_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n33_), .O(new_n66_));
  nand2  g42(.a(new_n29_), .b(x02), .O(new_n67_));
  nand2  g43(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g44(.a(new_n68_), .b(new_n63_), .c(new_n54_), .O(new_n69_));
  oai21  g45(.a(new_n62_), .b(x00), .c(new_n69_), .O(z2));
  inv1   g46(.a(x12), .O(new_n71_));
  nand2  g47(.a(x11), .b(x07), .O(new_n72_));
  oai21  g48(.a(new_n71_), .b(x00), .c(new_n72_), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n29_), .O(z3));
  aoi21  g50(.a(x09), .b(x00), .c(x07), .O(new_n75_));
  nor2   g51(.a(x11), .b(new_n54_), .O(new_n76_));
  oai21  g52(.a(new_n76_), .b(new_n75_), .c(new_n29_), .O(new_n77_));
  nand2  g53(.a(x01), .b(x00), .O(new_n78_));
  nand2  g54(.a(x09), .b(new_n33_), .O(new_n79_));
  aoi21  g55(.a(new_n79_), .b(new_n78_), .c(x10), .O(new_n80_));
  oai21  g56(.a(new_n80_), .b(x05), .c(new_n63_), .O(new_n81_));
  oai21  g57(.a(new_n81_), .b(x07), .c(new_n77_), .O(z4));
  nand4  g58(.a(x13), .b(new_n71_), .c(new_n54_), .d(new_n40_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n35_), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n57_), .O(z5));
  oai21  g61(.a(new_n64_), .b(x03), .c(new_n28_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n33_), .O(new_n87_));
  nand3  g63(.a(new_n64_), .b(x03), .c(new_n28_), .O(new_n88_));
  aoi22  g64(.a(new_n88_), .b(new_n87_), .c(x08), .d(x00), .O(new_n89_));
  nand2  g65(.a(x14), .b(x02), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n40_), .O(new_n91_));
  nand2  g67(.a(new_n63_), .b(x00), .O(new_n92_));
  aoi21  g68(.a(new_n92_), .b(new_n91_), .c(new_n33_), .O(new_n93_));
  oai21  g69(.a(new_n93_), .b(new_n89_), .c(new_n35_), .O(new_n94_));
  nor2   g70(.a(new_n94_), .b(x07), .O(z6));
  nand2  g71(.a(new_n54_), .b(x02), .O(new_n96_));
  nand3  g72(.a(new_n96_), .b(x03), .c(new_n33_), .O(new_n97_));
  nand4  g73(.a(x09), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n98_));
  nand2  g74(.a(new_n98_), .b(new_n54_), .O(new_n99_));
  aoi21  g75(.a(new_n97_), .b(new_n63_), .c(new_n99_), .O(new_n100_));
  oai21  g76(.a(new_n100_), .b(x10), .c(new_n57_), .O(z7));
  nand4  g77(.a(new_n71_), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n102_));
  oai21  g78(.a(x09), .b(x00), .c(new_n54_), .O(new_n103_));
  aoi21  g79(.a(new_n102_), .b(x09), .c(new_n103_), .O(new_n104_));
  oai21  g80(.a(new_n104_), .b(x10), .c(new_n57_), .O(z8));
endmodule


