// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  inv1   g00(.a(x11), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x10), .O(new_n26_));
  inv1   g02(.a(x03), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand2  g04(.a(new_n28_), .b(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n29_), .c(new_n27_), .O(new_n32_));
  inv1   g08(.a(x01), .O(new_n33_));
  inv1   g09(.a(x05), .O(new_n34_));
  nand2  g10(.a(new_n34_), .b(x04), .O(new_n35_));
  nand2  g11(.a(new_n35_), .b(x02), .O(new_n36_));
  nor2   g12(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n32_), .c(new_n26_), .O(new_n38_));
  nor2   g14(.a(new_n38_), .b(x00), .O(z0));
  inv1   g15(.a(x00), .O(new_n40_));
  nand3  g16(.a(new_n26_), .b(new_n28_), .c(x01), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand2  g18(.a(new_n42_), .b(x02), .O(new_n43_));
  aoi21  g19(.a(new_n43_), .b(new_n41_), .c(new_n27_), .O(new_n44_));
  nand3  g20(.a(new_n42_), .b(x04), .c(x02), .O(new_n45_));
  inv1   g21(.a(new_n45_), .O(new_n46_));
  oai21  g22(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n26_), .O(z1));
  oai21  g24(.a(x10), .b(x07), .c(new_n27_), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  inv1   g26(.a(x10), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(x09), .c(new_n50_), .O(new_n52_));
  nand3  g28(.a(new_n52_), .b(new_n49_), .c(new_n28_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n33_), .O(new_n54_));
  nor2   g30(.a(x11), .b(new_n51_), .O(new_n55_));
  oai21  g31(.a(new_n55_), .b(new_n33_), .c(new_n35_), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(x02), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n40_), .O(new_n59_));
  inv1   g35(.a(x08), .O(new_n60_));
  inv1   g36(.a(x09), .O(new_n61_));
  nor2   g37(.a(x10), .b(new_n61_), .O(new_n62_));
  nor2   g38(.a(new_n51_), .b(x03), .O(new_n63_));
  oai21  g39(.a(new_n63_), .b(new_n62_), .c(new_n33_), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n28_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n60_), .c(new_n50_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n59_), .c(new_n26_), .O(z2));
  nand2  g43(.a(new_n25_), .b(new_n51_), .O(new_n68_));
  oai21  g44(.a(new_n25_), .b(x07), .c(new_n68_), .O(new_n69_));
  nand2  g45(.a(x12), .b(new_n40_), .O(new_n70_));
  and2   g46(.a(new_n70_), .b(new_n69_), .O(z3));
  oai21  g47(.a(x11), .b(new_n51_), .c(new_n40_), .O(new_n72_));
  nand2  g48(.a(x01), .b(x00), .O(new_n73_));
  nand2  g49(.a(x09), .b(new_n33_), .O(new_n74_));
  aoi21  g50(.a(new_n74_), .b(new_n73_), .c(x10), .O(new_n75_));
  oai21  g51(.a(new_n75_), .b(x11), .c(new_n60_), .O(new_n76_));
  nand2  g52(.a(x11), .b(new_n61_), .O(new_n77_));
  nand3  g53(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(new_n78_));
  nand2  g54(.a(new_n78_), .b(new_n50_), .O(new_n79_));
  nand2  g55(.a(x09), .b(new_n50_), .O(new_n80_));
  nand3  g56(.a(new_n80_), .b(new_n25_), .c(new_n51_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n79_), .O(z4));
  nand2  g58(.a(new_n69_), .b(x00), .O(new_n83_));
  oai21  g59(.a(x11), .b(new_n51_), .c(x07), .O(new_n84_));
  nand2  g60(.a(x11), .b(x10), .O(new_n85_));
  oai22  g61(.a(new_n85_), .b(x00), .c(x13), .d(x10), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  nand2  g63(.a(x12), .b(new_n51_), .O(new_n88_));
  nand4  g64(.a(new_n88_), .b(new_n87_), .c(new_n84_), .d(new_n83_), .O(z5));
  nand2  g65(.a(x08), .b(x00), .O(new_n90_));
  aoi21  g66(.a(x09), .b(new_n27_), .c(x02), .O(new_n91_));
  nand3  g67(.a(new_n61_), .b(x03), .c(new_n28_), .O(new_n92_));
  oai21  g68(.a(new_n91_), .b(x01), .c(new_n92_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  aoi21  g70(.a(x14), .b(x02), .c(x00), .O(new_n95_));
  nor2   g71(.a(x08), .b(new_n40_), .O(new_n96_));
  oai21  g72(.a(new_n96_), .b(new_n95_), .c(x01), .O(new_n97_));
  nand2  g73(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nand3  g74(.a(new_n98_), .b(new_n51_), .c(new_n50_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n26_), .O(z6));
  nand2  g76(.a(new_n50_), .b(x02), .O(new_n101_));
  nand3  g77(.a(new_n101_), .b(x03), .c(new_n33_), .O(new_n102_));
  nand2  g78(.a(new_n102_), .b(new_n60_), .O(new_n103_));
  nand4  g79(.a(x09), .b(x03), .c(new_n28_), .d(new_n33_), .O(new_n104_));
  nand4  g80(.a(new_n104_), .b(new_n103_), .c(new_n84_), .d(new_n51_), .O(z7));
  nand2  g81(.a(x03), .b(new_n28_), .O(new_n106_));
  nand3  g82(.a(new_n106_), .b(new_n51_), .c(new_n50_), .O(new_n107_));
  nor2   g83(.a(x12), .b(x01), .O(new_n108_));
  nand2  g84(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g85(.a(new_n109_), .b(x09), .O(new_n110_));
  aoi21  g86(.a(new_n61_), .b(new_n40_), .c(x10), .O(new_n111_));
  nand3  g87(.a(new_n111_), .b(new_n110_), .c(new_n84_), .O(z8));
endmodule


