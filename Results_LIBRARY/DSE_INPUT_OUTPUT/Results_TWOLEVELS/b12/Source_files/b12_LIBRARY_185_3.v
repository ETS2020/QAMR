// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_;
  inv1   g00(.a(x02), .O(new_n25_));
  nand2  g01(.a(new_n25_), .b(x01), .O(new_n26_));
  inv1   g02(.a(x01), .O(new_n27_));
  inv1   g03(.a(x04), .O(new_n28_));
  inv1   g04(.a(x07), .O(new_n29_));
  nand4  g05(.a(new_n29_), .b(new_n28_), .c(x02), .d(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand2  g07(.a(new_n31_), .b(x03), .O(new_n32_));
  inv1   g08(.a(x05), .O(new_n33_));
  nand2  g09(.a(new_n33_), .b(x04), .O(new_n34_));
  nand4  g10(.a(new_n34_), .b(new_n29_), .c(x02), .d(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n32_), .c(x00), .O(z0));
  inv1   g12(.a(x00), .O(new_n37_));
  inv1   g13(.a(new_n34_), .O(new_n38_));
  inv1   g14(.a(x03), .O(new_n39_));
  aoi21  g15(.a(new_n28_), .b(new_n39_), .c(x01), .O(new_n40_));
  oai21  g16(.a(new_n40_), .b(new_n38_), .c(new_n29_), .O(new_n41_));
  inv1   g17(.a(x06), .O(new_n42_));
  nand2  g18(.a(x06), .b(x05), .O(new_n43_));
  aoi22  g19(.a(new_n43_), .b(x03), .c(new_n42_), .d(x04), .O(new_n44_));
  aoi21  g20(.a(new_n44_), .b(new_n41_), .c(new_n25_), .O(new_n45_));
  nand3  g21(.a(x03), .b(new_n25_), .c(x01), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(new_n47_));
  oai21  g23(.a(new_n47_), .b(new_n45_), .c(new_n37_), .O(new_n48_));
  nand2  g24(.a(x07), .b(x02), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n48_), .O(z1));
  nand2  g26(.a(new_n34_), .b(x01), .O(new_n51_));
  oai21  g27(.a(new_n33_), .b(new_n27_), .c(x04), .O(new_n52_));
  oai21  g28(.a(x04), .b(new_n39_), .c(x10), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n52_), .c(new_n51_), .O(new_n55_));
  nand2  g31(.a(x10), .b(new_n39_), .O(new_n56_));
  inv1   g32(.a(x10), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(x09), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n56_), .c(x01), .O(new_n59_));
  aoi21  g35(.a(new_n55_), .b(x02), .c(new_n59_), .O(new_n60_));
  inv1   g36(.a(x08), .O(new_n61_));
  oai21  g37(.a(new_n59_), .b(x02), .c(new_n61_), .O(new_n62_));
  oai21  g38(.a(new_n60_), .b(x00), .c(new_n62_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n29_), .O(new_n64_));
  nor2   g40(.a(x01), .b(x00), .O(new_n65_));
  nand4  g41(.a(new_n65_), .b(x07), .c(new_n39_), .d(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n64_), .O(z2));
  inv1   g43(.a(x11), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(new_n29_), .c(x00), .O(new_n69_));
  inv1   g45(.a(x12), .O(new_n70_));
  oai21  g46(.a(x07), .b(x00), .c(x11), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g48(.a(new_n72_), .b(new_n69_), .c(new_n49_), .O(z3));
  nand3  g49(.a(x09), .b(x08), .c(x00), .O(new_n74_));
  nand2  g50(.a(new_n74_), .b(new_n29_), .O(new_n75_));
  oai21  g51(.a(new_n68_), .b(x02), .c(x07), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n75_), .O(z4));
  nor3   g53(.a(x12), .b(x10), .c(x00), .O(new_n78_));
  nand3  g54(.a(new_n78_), .b(x13), .c(new_n29_), .O(z5));
  nor2   g55(.a(x09), .b(new_n39_), .O(new_n80_));
  nor2   g56(.a(new_n80_), .b(x02), .O(new_n81_));
  nor2   g57(.a(new_n81_), .b(new_n37_), .O(new_n82_));
  inv1   g58(.a(x09), .O(new_n83_));
  nor2   g59(.a(new_n83_), .b(x03), .O(new_n84_));
  nand2  g60(.a(new_n84_), .b(new_n27_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n26_), .O(new_n86_));
  oai21  g62(.a(new_n86_), .b(new_n82_), .c(new_n61_), .O(new_n87_));
  oai21  g63(.a(new_n84_), .b(x02), .c(new_n27_), .O(new_n88_));
  oai21  g64(.a(new_n80_), .b(x01), .c(new_n25_), .O(new_n89_));
  inv1   g65(.a(x14), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(x02), .O(new_n91_));
  nand3  g67(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(new_n92_));
  nand2  g68(.a(new_n92_), .b(new_n37_), .O(new_n93_));
  nand2  g69(.a(new_n93_), .b(new_n87_), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(new_n57_), .c(new_n29_), .O(new_n95_));
  inv1   g71(.a(new_n95_), .O(z6));
  nand3  g72(.a(x09), .b(x03), .c(new_n27_), .O(new_n97_));
  nand4  g73(.a(new_n57_), .b(new_n61_), .c(new_n29_), .d(x01), .O(new_n98_));
  nand3  g74(.a(new_n98_), .b(new_n97_), .c(new_n29_), .O(new_n99_));
  nand2  g75(.a(new_n99_), .b(new_n25_), .O(new_n100_));
  aoi21  g76(.a(x03), .b(new_n25_), .c(x08), .O(new_n101_));
  oai21  g77(.a(new_n101_), .b(x10), .c(new_n29_), .O(new_n102_));
  nand2  g78(.a(new_n102_), .b(new_n100_), .O(z7));
  nand4  g79(.a(new_n70_), .b(x03), .c(new_n25_), .d(new_n27_), .O(new_n104_));
  oai21  g80(.a(x09), .b(x00), .c(new_n57_), .O(new_n105_));
  aoi21  g81(.a(new_n104_), .b(x09), .c(new_n105_), .O(new_n106_));
  nand2  g82(.a(x07), .b(new_n25_), .O(new_n107_));
  oai21  g83(.a(new_n106_), .b(x07), .c(new_n107_), .O(z8));
endmodule


