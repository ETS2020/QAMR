// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x00), .O(new_n25_));
  inv1   g01(.a(x03), .O(new_n26_));
  inv1   g02(.a(x02), .O(new_n27_));
  nand2  g03(.a(new_n27_), .b(x01), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  nand3  g06(.a(new_n30_), .b(x02), .c(new_n29_), .O(new_n31_));
  aoi21  g07(.a(new_n31_), .b(new_n28_), .c(new_n26_), .O(new_n32_));
  oai21  g08(.a(x05), .b(new_n30_), .c(x02), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n29_), .O(new_n34_));
  oai21  g10(.a(new_n34_), .b(new_n32_), .c(new_n25_), .O(new_n35_));
  inv1   g11(.a(x08), .O(new_n36_));
  nor2   g12(.a(x10), .b(new_n36_), .O(new_n37_));
  inv1   g13(.a(new_n37_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n35_), .O(z0));
  oai21  g15(.a(x04), .b(x03), .c(new_n29_), .O(new_n40_));
  nand2  g16(.a(x06), .b(x05), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x04), .O(new_n42_));
  aoi21  g18(.a(new_n42_), .b(new_n40_), .c(new_n27_), .O(new_n43_));
  nand3  g19(.a(x06), .b(x05), .c(x02), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(x03), .c(x01), .O(new_n45_));
  inv1   g21(.a(new_n45_), .O(new_n46_));
  oai21  g22(.a(new_n46_), .b(new_n43_), .c(new_n25_), .O(new_n47_));
  nand2  g23(.a(new_n47_), .b(new_n38_), .O(z1));
  inv1   g24(.a(x07), .O(new_n49_));
  inv1   g25(.a(x10), .O(new_n50_));
  oai21  g26(.a(x08), .b(new_n49_), .c(new_n50_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n26_), .c(new_n29_), .O(new_n52_));
  oai21  g28(.a(new_n37_), .b(new_n27_), .c(new_n52_), .O(new_n53_));
  nand2  g29(.a(new_n53_), .b(new_n25_), .O(new_n54_));
  nor2   g30(.a(new_n50_), .b(x03), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(x10), .b(new_n56_), .O(new_n57_));
  oai21  g33(.a(new_n57_), .b(new_n55_), .c(new_n29_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n27_), .O(new_n59_));
  nand3  g35(.a(new_n59_), .b(new_n36_), .c(new_n49_), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n54_), .O(z2));
  nand2  g37(.a(x11), .b(x07), .O(new_n62_));
  nand2  g38(.a(x12), .b(new_n25_), .O(new_n63_));
  nand3  g39(.a(new_n63_), .b(new_n62_), .c(new_n38_), .O(new_n64_));
  inv1   g40(.a(new_n64_), .O(z3));
  oai21  g41(.a(x12), .b(x10), .c(new_n25_), .O(new_n66_));
  nand2  g42(.a(new_n63_), .b(new_n36_), .O(new_n67_));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(x09), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  nor2   g45(.a(x11), .b(new_n49_), .O(new_n70_));
  nor2   g46(.a(new_n70_), .b(new_n37_), .O(new_n71_));
  nand2  g47(.a(new_n71_), .b(new_n69_), .O(z4));
  nand2  g48(.a(new_n38_), .b(x00), .O(new_n73_));
  nor2   g49(.a(x13), .b(x08), .O(new_n74_));
  nor2   g50(.a(new_n74_), .b(new_n37_), .O(new_n75_));
  nand4  g51(.a(new_n75_), .b(new_n73_), .c(new_n66_), .d(new_n49_), .O(z5));
  nand2  g52(.a(x14), .b(x02), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(x01), .O(new_n78_));
  oai21  g54(.a(new_n56_), .b(x03), .c(new_n27_), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n29_), .O(new_n80_));
  nand2  g56(.a(x02), .b(x00), .O(new_n81_));
  nand3  g57(.a(new_n56_), .b(x03), .c(new_n27_), .O(new_n82_));
  nand4  g58(.a(new_n82_), .b(new_n81_), .c(new_n80_), .d(new_n78_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n49_), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n36_), .c(x10), .O(z6));
  nand2  g61(.a(new_n57_), .b(new_n29_), .O(new_n86_));
  nand2  g62(.a(new_n86_), .b(new_n27_), .O(new_n87_));
  oai21  g63(.a(x02), .b(new_n29_), .c(new_n50_), .O(new_n88_));
  aoi21  g64(.a(new_n87_), .b(new_n36_), .c(new_n88_), .O(new_n89_));
  nand4  g65(.a(new_n89_), .b(new_n38_), .c(new_n49_), .d(x03), .O(z7));
  oai21  g66(.a(new_n50_), .b(x07), .c(x09), .O(new_n91_));
  nand2  g67(.a(new_n91_), .b(new_n25_), .O(new_n92_));
  nand3  g68(.a(new_n57_), .b(new_n26_), .c(new_n29_), .O(new_n93_));
  oai21  g69(.a(new_n50_), .b(new_n25_), .c(new_n93_), .O(new_n94_));
  nand2  g70(.a(new_n94_), .b(new_n49_), .O(new_n95_));
  nand2  g71(.a(new_n38_), .b(new_n49_), .O(new_n96_));
  inv1   g72(.a(x12), .O(new_n97_));
  nand3  g73(.a(new_n97_), .b(new_n27_), .c(new_n29_), .O(new_n98_));
  aoi21  g74(.a(new_n98_), .b(x09), .c(new_n96_), .O(new_n99_));
  nand3  g75(.a(new_n99_), .b(new_n95_), .c(new_n92_), .O(z8));
endmodule


