// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n91_, new_n92_;
  inv1   g00(.a(x09), .O(new_n25_));
  inv1   g01(.a(x10), .O(new_n26_));
  nand2  g02(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  inv1   g03(.a(x00), .O(new_n28_));
  inv1   g04(.a(x01), .O(new_n29_));
  inv1   g05(.a(x04), .O(new_n30_));
  oai21  g06(.a(x05), .b(new_n30_), .c(x02), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand3  g08(.a(new_n27_), .b(x03), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n29_), .O(new_n34_));
  nand3  g10(.a(new_n30_), .b(x03), .c(x02), .O(new_n35_));
  inv1   g11(.a(new_n35_), .O(new_n36_));
  oai21  g12(.a(new_n36_), .b(new_n34_), .c(new_n28_), .O(new_n37_));
  nand2  g13(.a(new_n37_), .b(new_n27_), .O(z0));
  inv1   g14(.a(x03), .O(new_n39_));
  nand2  g15(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  nand3  g16(.a(x06), .b(x05), .c(x01), .O(new_n41_));
  nand3  g17(.a(new_n41_), .b(new_n40_), .c(x02), .O(new_n42_));
  nor2   g18(.a(new_n39_), .b(x02), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x01), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n27_), .c(new_n28_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(z1));
  aoi21  g23(.a(new_n26_), .b(new_n25_), .c(x00), .O(new_n48_));
  inv1   g24(.a(x07), .O(new_n49_));
  inv1   g25(.a(x08), .O(new_n50_));
  nand3  g26(.a(x10), .b(new_n50_), .c(new_n49_), .O(new_n51_));
  inv1   g27(.a(new_n51_), .O(new_n52_));
  oai21  g28(.a(x03), .b(x01), .c(new_n32_), .O(new_n53_));
  oai21  g29(.a(new_n52_), .b(new_n48_), .c(new_n53_), .O(new_n54_));
  oai21  g30(.a(new_n50_), .b(new_n28_), .c(new_n29_), .O(new_n55_));
  nand3  g31(.a(new_n50_), .b(x02), .c(x00), .O(new_n56_));
  nand2  g32(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g33(.a(new_n57_), .b(new_n26_), .c(x09), .d(new_n49_), .O(new_n58_));
  nand2  g34(.a(new_n58_), .b(new_n54_), .O(z2));
  oai21  g35(.a(x07), .b(new_n28_), .c(x10), .O(new_n60_));
  nand2  g36(.a(new_n60_), .b(new_n25_), .O(new_n61_));
  oai21  g37(.a(new_n25_), .b(x07), .c(x11), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(x00), .O(new_n63_));
  inv1   g39(.a(x11), .O(new_n64_));
  inv1   g40(.a(x12), .O(new_n65_));
  oai21  g41(.a(new_n64_), .b(new_n49_), .c(new_n65_), .O(new_n66_));
  nand3  g42(.a(new_n66_), .b(new_n63_), .c(new_n61_), .O(z3));
  oai21  g43(.a(new_n25_), .b(x08), .c(x00), .O(new_n68_));
  nand2  g44(.a(new_n68_), .b(new_n49_), .O(new_n69_));
  nand2  g45(.a(new_n64_), .b(x07), .O(new_n70_));
  nand3  g46(.a(new_n70_), .b(new_n69_), .c(new_n61_), .O(z4));
  nand4  g47(.a(x13), .b(new_n65_), .c(new_n49_), .d(new_n28_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(x09), .O(new_n73_));
  nand2  g49(.a(new_n73_), .b(new_n26_), .O(z5));
  nor2   g50(.a(x02), .b(x01), .O(new_n75_));
  oai22  g51(.a(new_n75_), .b(new_n28_), .c(x03), .d(x01), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n50_), .O(new_n77_));
  aoi21  g53(.a(x03), .b(new_n32_), .c(x01), .O(new_n78_));
  nand2  g54(.a(new_n32_), .b(x01), .O(new_n79_));
  inv1   g55(.a(x14), .O(new_n80_));
  nand2  g56(.a(new_n80_), .b(x02), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  oai21  g58(.a(new_n82_), .b(new_n78_), .c(new_n28_), .O(new_n83_));
  nand2  g59(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nand4  g60(.a(new_n84_), .b(new_n26_), .c(x09), .d(new_n49_), .O(new_n85_));
  inv1   g61(.a(new_n85_), .O(z6));
  nand2  g62(.a(new_n43_), .b(new_n29_), .O(new_n87_));
  nand3  g63(.a(new_n87_), .b(x08), .c(new_n49_), .O(new_n88_));
  nand2  g64(.a(new_n88_), .b(x09), .O(new_n89_));
  nand2  g65(.a(new_n89_), .b(new_n26_), .O(z7));
  oai21  g66(.a(x12), .b(x07), .c(x09), .O(new_n91_));
  nor2   g67(.a(x10), .b(new_n25_), .O(new_n92_));
  nand4  g68(.a(new_n92_), .b(new_n91_), .c(new_n75_), .d(x03), .O(z8));
endmodule


