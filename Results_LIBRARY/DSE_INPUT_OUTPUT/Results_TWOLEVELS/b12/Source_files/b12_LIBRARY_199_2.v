// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x01), .O(new_n25_));
  inv1   g01(.a(x04), .O(new_n26_));
  nand3  g02(.a(new_n26_), .b(x02), .c(new_n25_), .O(new_n27_));
  inv1   g03(.a(x02), .O(new_n28_));
  nand3  g04(.a(x10), .b(new_n28_), .c(x01), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x03), .O(new_n31_));
  inv1   g07(.a(x10), .O(new_n32_));
  nor2   g08(.a(x05), .b(new_n26_), .O(new_n33_));
  nor2   g09(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand3  g10(.a(new_n34_), .b(x02), .c(x01), .O(new_n35_));
  aoi21  g11(.a(new_n35_), .b(new_n31_), .c(x00), .O(z0));
  nor2   g12(.a(x02), .b(x00), .O(new_n37_));
  aoi21  g13(.a(new_n37_), .b(x03), .c(new_n32_), .O(new_n38_));
  inv1   g14(.a(x00), .O(new_n39_));
  oai21  g15(.a(x04), .b(x03), .c(new_n25_), .O(new_n40_));
  nand2  g16(.a(x06), .b(x05), .O(new_n41_));
  nand2  g17(.a(new_n41_), .b(x03), .O(new_n42_));
  oai21  g18(.a(new_n32_), .b(x05), .c(x06), .O(new_n43_));
  nand2  g19(.a(new_n43_), .b(x04), .O(new_n44_));
  nand3  g20(.a(new_n44_), .b(new_n42_), .c(new_n40_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(x02), .c(new_n39_), .O(new_n46_));
  oai21  g22(.a(new_n38_), .b(new_n25_), .c(new_n46_), .O(z1));
  oai21  g23(.a(new_n33_), .b(x01), .c(x10), .O(new_n48_));
  inv1   g24(.a(x03), .O(new_n49_));
  inv1   g25(.a(x07), .O(new_n50_));
  nand2  g26(.a(new_n32_), .b(new_n50_), .O(new_n51_));
  nand3  g27(.a(new_n51_), .b(new_n26_), .c(new_n49_), .O(new_n52_));
  nand2  g28(.a(new_n52_), .b(new_n25_), .O(new_n53_));
  aoi21  g29(.a(new_n53_), .b(new_n48_), .c(new_n28_), .O(new_n54_));
  nand2  g30(.a(new_n51_), .b(new_n49_), .O(new_n55_));
  inv1   g31(.a(x09), .O(new_n56_));
  nor2   g32(.a(x10), .b(new_n56_), .O(new_n57_));
  nand2  g33(.a(new_n57_), .b(new_n50_), .O(new_n58_));
  aoi21  g34(.a(new_n58_), .b(new_n55_), .c(x01), .O(new_n59_));
  oai21  g35(.a(new_n59_), .b(new_n54_), .c(new_n39_), .O(new_n60_));
  oai21  g36(.a(x10), .b(new_n25_), .c(x02), .O(new_n61_));
  nor2   g37(.a(new_n32_), .b(x03), .O(new_n62_));
  oai21  g38(.a(new_n62_), .b(new_n57_), .c(new_n25_), .O(new_n63_));
  aoi21  g39(.a(new_n63_), .b(new_n61_), .c(x08), .O(new_n64_));
  nand2  g40(.a(new_n64_), .b(new_n50_), .O(new_n65_));
  nand2  g41(.a(new_n65_), .b(new_n60_), .O(z2));
  inv1   g42(.a(x11), .O(new_n67_));
  inv1   g43(.a(x12), .O(new_n68_));
  oai22  g44(.a(new_n68_), .b(x00), .c(new_n67_), .d(new_n50_), .O(new_n69_));
  nand2  g45(.a(new_n32_), .b(x01), .O(new_n70_));
  nand2  g46(.a(new_n70_), .b(new_n69_), .O(z3));
  nand3  g47(.a(x09), .b(x08), .c(x00), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n50_), .O(new_n73_));
  nand2  g49(.a(new_n67_), .b(x07), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n73_), .c(new_n70_), .O(z4));
  nand4  g51(.a(x13), .b(new_n68_), .c(new_n50_), .d(new_n39_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n25_), .O(new_n77_));
  nand2  g53(.a(new_n77_), .b(new_n32_), .O(z5));
  nand2  g54(.a(x08), .b(x00), .O(new_n79_));
  nand2  g55(.a(x02), .b(new_n25_), .O(new_n80_));
  nand2  g56(.a(x09), .b(new_n49_), .O(new_n81_));
  nand2  g57(.a(new_n56_), .b(x03), .O(new_n82_));
  nand3  g58(.a(new_n82_), .b(new_n81_), .c(new_n80_), .O(new_n83_));
  nand3  g59(.a(new_n83_), .b(new_n79_), .c(new_n50_), .O(new_n84_));
  aoi21  g60(.a(new_n84_), .b(new_n25_), .c(x10), .O(z6));
  nand3  g61(.a(new_n32_), .b(new_n50_), .c(x02), .O(new_n86_));
  aoi21  g62(.a(new_n86_), .b(x03), .c(x08), .O(new_n87_));
  nand2  g63(.a(x09), .b(x03), .O(new_n88_));
  oai21  g64(.a(new_n88_), .b(x02), .c(new_n50_), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(new_n87_), .c(new_n25_), .O(new_n90_));
  nand2  g66(.a(new_n90_), .b(new_n32_), .O(z7));
  nand3  g67(.a(new_n68_), .b(x03), .c(new_n28_), .O(new_n92_));
  oai21  g68(.a(x09), .b(x00), .c(new_n50_), .O(new_n93_));
  aoi21  g69(.a(new_n92_), .b(x09), .c(new_n93_), .O(new_n94_));
  oai21  g70(.a(new_n94_), .b(x01), .c(new_n32_), .O(z8));
endmodule


