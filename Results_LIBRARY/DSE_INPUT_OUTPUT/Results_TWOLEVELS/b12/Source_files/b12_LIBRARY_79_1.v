// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_;
  inv1   g00(.a(x08), .O(new_n25_));
  nand2  g01(.a(x10), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand3  g08(.a(new_n26_), .b(x03), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x03), .c(x02), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n34_), .c(new_n27_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n26_), .O(z0));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(new_n35_), .b(new_n40_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(x02), .O(new_n43_));
  nand3  g19(.a(x03), .b(new_n32_), .c(x01), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n26_), .c(new_n27_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(z1));
  nand2  g23(.a(new_n30_), .b(x01), .O(new_n48_));
  nor2   g24(.a(x05), .b(new_n35_), .O(new_n49_));
  nor2   g25(.a(new_n40_), .b(x01), .O(new_n50_));
  oai21  g26(.a(new_n50_), .b(new_n49_), .c(new_n26_), .O(new_n51_));
  inv1   g27(.a(x07), .O(new_n52_));
  inv1   g28(.a(x10), .O(new_n53_));
  nand3  g29(.a(new_n53_), .b(new_n52_), .c(new_n28_), .O(new_n54_));
  nand3  g30(.a(new_n54_), .b(new_n51_), .c(new_n48_), .O(new_n55_));
  oai21  g31(.a(x10), .b(x07), .c(new_n40_), .O(new_n56_));
  nand3  g32(.a(new_n53_), .b(x09), .c(new_n52_), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n56_), .c(x01), .O(new_n58_));
  aoi21  g34(.a(new_n55_), .b(x02), .c(new_n58_), .O(new_n59_));
  nand3  g35(.a(new_n53_), .b(x09), .c(new_n28_), .O(new_n60_));
  aoi21  g36(.a(new_n60_), .b(new_n32_), .c(x07), .O(new_n61_));
  oai21  g37(.a(new_n61_), .b(x10), .c(new_n25_), .O(new_n62_));
  oai21  g38(.a(new_n59_), .b(x00), .c(new_n62_), .O(z2));
  nand2  g39(.a(x11), .b(x07), .O(new_n64_));
  nand2  g40(.a(x12), .b(new_n27_), .O(new_n65_));
  nand3  g41(.a(new_n65_), .b(new_n64_), .c(new_n26_), .O(new_n66_));
  inv1   g42(.a(new_n66_), .O(z3));
  nor2   g43(.a(x10), .b(x09), .O(new_n68_));
  oai21  g44(.a(new_n68_), .b(x08), .c(new_n27_), .O(new_n69_));
  inv1   g45(.a(x09), .O(new_n70_));
  nand2  g46(.a(x03), .b(x00), .O(new_n71_));
  oai21  g47(.a(new_n71_), .b(x10), .c(new_n25_), .O(new_n72_));
  nand2  g48(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand3  g49(.a(new_n70_), .b(x03), .c(new_n28_), .O(new_n74_));
  nand3  g50(.a(new_n74_), .b(new_n53_), .c(new_n25_), .O(new_n75_));
  nand3  g51(.a(new_n75_), .b(new_n73_), .c(new_n69_), .O(new_n76_));
  nand2  g52(.a(new_n76_), .b(new_n52_), .O(new_n77_));
  inv1   g53(.a(x11), .O(new_n78_));
  nand3  g54(.a(new_n26_), .b(new_n78_), .c(x07), .O(new_n79_));
  nand2  g55(.a(new_n79_), .b(new_n77_), .O(z4));
  inv1   g56(.a(x12), .O(new_n81_));
  nand4  g57(.a(x13), .b(new_n81_), .c(new_n52_), .d(new_n27_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  nor2   g59(.a(new_n53_), .b(new_n25_), .O(new_n84_));
  inv1   g60(.a(new_n84_), .O(new_n85_));
  nand2  g61(.a(new_n85_), .b(new_n83_), .O(z5));
  oai21  g62(.a(x09), .b(new_n40_), .c(new_n28_), .O(new_n87_));
  nor2   g63(.a(x02), .b(x00), .O(new_n88_));
  nor2   g64(.a(x08), .b(new_n27_), .O(new_n89_));
  oai21  g65(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  oai21  g66(.a(new_n70_), .b(x03), .c(new_n32_), .O(new_n91_));
  nand2  g67(.a(x08), .b(x00), .O(new_n92_));
  nand3  g68(.a(new_n92_), .b(new_n91_), .c(new_n28_), .O(new_n93_));
  inv1   g69(.a(x14), .O(new_n94_));
  nand3  g70(.a(new_n94_), .b(x01), .c(new_n27_), .O(new_n95_));
  nand3  g71(.a(new_n95_), .b(new_n93_), .c(new_n90_), .O(new_n96_));
  nand3  g72(.a(new_n96_), .b(new_n53_), .c(new_n52_), .O(new_n97_));
  inv1   g73(.a(new_n97_), .O(z6));
  nand3  g74(.a(x09), .b(x03), .c(new_n32_), .O(new_n99_));
  nand4  g75(.a(new_n53_), .b(new_n25_), .c(new_n52_), .d(x02), .O(new_n100_));
  nand2  g76(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g77(.a(new_n101_), .b(new_n28_), .O(new_n102_));
  oai21  g78(.a(new_n50_), .b(x07), .c(new_n53_), .O(new_n103_));
  nand2  g79(.a(new_n103_), .b(new_n25_), .O(new_n104_));
  aoi21  g80(.a(new_n53_), .b(x07), .c(new_n84_), .O(new_n105_));
  nand3  g81(.a(new_n105_), .b(new_n104_), .c(new_n102_), .O(z7));
  aoi21  g82(.a(new_n70_), .b(new_n27_), .c(x07), .O(new_n107_));
  nand4  g83(.a(new_n81_), .b(x03), .c(new_n32_), .d(new_n28_), .O(new_n108_));
  nand2  g84(.a(new_n108_), .b(x09), .O(new_n109_));
  nand3  g85(.a(new_n109_), .b(new_n107_), .c(new_n53_), .O(z8));
endmodule


