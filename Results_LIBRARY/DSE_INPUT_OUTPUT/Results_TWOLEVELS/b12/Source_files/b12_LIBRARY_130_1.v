// Benchmark "FAU" written by ABC on Tue Aug 18 15:32:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x10), .O(new_n25_));
  nand2  g01(.a(x14), .b(new_n25_), .O(new_n26_));
  inv1   g02(.a(x00), .O(new_n27_));
  inv1   g03(.a(x01), .O(new_n28_));
  inv1   g04(.a(x05), .O(new_n29_));
  nand2  g05(.a(new_n29_), .b(x04), .O(new_n30_));
  nand2  g06(.a(new_n30_), .b(x02), .O(new_n31_));
  inv1   g07(.a(x02), .O(new_n32_));
  nand3  g08(.a(new_n26_), .b(x03), .c(new_n32_), .O(new_n33_));
  aoi21  g09(.a(new_n33_), .b(new_n31_), .c(new_n28_), .O(new_n34_));
  inv1   g10(.a(x04), .O(new_n35_));
  nand3  g11(.a(new_n35_), .b(x03), .c(x02), .O(new_n36_));
  inv1   g12(.a(new_n36_), .O(new_n37_));
  oai21  g13(.a(new_n37_), .b(new_n34_), .c(new_n27_), .O(new_n38_));
  nand2  g14(.a(new_n38_), .b(new_n26_), .O(z0));
  inv1   g15(.a(x03), .O(new_n40_));
  nand2  g16(.a(new_n35_), .b(new_n40_), .O(new_n41_));
  nand3  g17(.a(x06), .b(x05), .c(x01), .O(new_n42_));
  nand3  g18(.a(new_n42_), .b(new_n41_), .c(x02), .O(new_n43_));
  nand3  g19(.a(x03), .b(new_n32_), .c(x01), .O(new_n44_));
  nand2  g20(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g21(.a(new_n45_), .b(new_n26_), .c(new_n27_), .O(new_n46_));
  inv1   g22(.a(new_n46_), .O(z1));
  nand2  g23(.a(new_n30_), .b(x01), .O(new_n48_));
  nand2  g24(.a(x03), .b(new_n28_), .O(new_n49_));
  nand2  g25(.a(new_n49_), .b(new_n30_), .O(new_n50_));
  nor3   g26(.a(x14), .b(x10), .c(x07), .O(new_n51_));
  aoi21  g27(.a(new_n50_), .b(new_n26_), .c(new_n51_), .O(new_n52_));
  aoi21  g28(.a(new_n52_), .b(new_n48_), .c(new_n32_), .O(new_n53_));
  oai21  g29(.a(x10), .b(x07), .c(new_n40_), .O(new_n54_));
  inv1   g30(.a(x07), .O(new_n55_));
  inv1   g31(.a(x14), .O(new_n56_));
  nand4  g32(.a(new_n56_), .b(new_n25_), .c(x09), .d(new_n55_), .O(new_n57_));
  aoi21  g33(.a(new_n57_), .b(new_n54_), .c(x01), .O(new_n58_));
  oai21  g34(.a(new_n58_), .b(new_n53_), .c(new_n27_), .O(new_n59_));
  inv1   g35(.a(new_n26_), .O(new_n60_));
  nand2  g36(.a(x10), .b(new_n40_), .O(new_n61_));
  nand3  g37(.a(new_n56_), .b(new_n25_), .c(x09), .O(new_n62_));
  nand2  g38(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g39(.a(new_n63_), .b(new_n28_), .O(new_n64_));
  aoi21  g40(.a(new_n64_), .b(new_n32_), .c(x08), .O(new_n65_));
  aoi21  g41(.a(new_n65_), .b(new_n55_), .c(new_n60_), .O(new_n66_));
  nand2  g42(.a(new_n66_), .b(new_n59_), .O(z2));
  nand2  g43(.a(x11), .b(x07), .O(new_n68_));
  nand2  g44(.a(x12), .b(new_n27_), .O(new_n69_));
  nand3  g45(.a(new_n69_), .b(new_n68_), .c(new_n26_), .O(new_n70_));
  inv1   g46(.a(new_n70_), .O(z3));
  inv1   g47(.a(x09), .O(new_n72_));
  nand2  g48(.a(x08), .b(x00), .O(new_n73_));
  nor2   g49(.a(x09), .b(x01), .O(new_n74_));
  oai21  g50(.a(new_n74_), .b(x14), .c(new_n25_), .O(new_n75_));
  aoi22  g51(.a(new_n75_), .b(new_n73_), .c(new_n26_), .d(new_n72_), .O(new_n76_));
  inv1   g52(.a(x11), .O(new_n77_));
  nand3  g53(.a(new_n26_), .b(new_n77_), .c(x07), .O(new_n78_));
  oai21  g54(.a(new_n76_), .b(x07), .c(new_n78_), .O(z4));
  inv1   g55(.a(x12), .O(new_n80_));
  nand4  g56(.a(x13), .b(new_n80_), .c(new_n55_), .d(new_n27_), .O(new_n81_));
  nand2  g57(.a(new_n81_), .b(new_n56_), .O(new_n82_));
  nand2  g58(.a(new_n82_), .b(new_n25_), .O(z5));
  nor2   g59(.a(x02), .b(x01), .O(new_n84_));
  nand2  g60(.a(x09), .b(new_n40_), .O(new_n85_));
  nand2  g61(.a(new_n72_), .b(x03), .O(new_n86_));
  nand3  g62(.a(new_n86_), .b(new_n85_), .c(new_n84_), .O(new_n87_));
  nand4  g63(.a(new_n87_), .b(new_n73_), .c(new_n56_), .d(new_n25_), .O(new_n88_));
  nor2   g64(.a(new_n88_), .b(x07), .O(z6));
  nor2   g65(.a(new_n84_), .b(x14), .O(new_n90_));
  nand3  g66(.a(new_n90_), .b(new_n25_), .c(new_n55_), .O(new_n91_));
  aoi21  g67(.a(new_n91_), .b(x03), .c(x08), .O(new_n92_));
  nand3  g68(.a(new_n84_), .b(x09), .c(x03), .O(new_n93_));
  nand4  g69(.a(new_n93_), .b(new_n56_), .c(new_n25_), .d(new_n55_), .O(new_n94_));
  or2    g70(.a(new_n94_), .b(new_n92_), .O(z7));
  nand3  g71(.a(new_n84_), .b(new_n80_), .c(x03), .O(new_n96_));
  oai21  g72(.a(x09), .b(x00), .c(new_n55_), .O(new_n97_));
  aoi21  g73(.a(new_n96_), .b(x09), .c(new_n97_), .O(new_n98_));
  oai21  g74(.a(new_n98_), .b(x14), .c(new_n25_), .O(z8));
endmodule


