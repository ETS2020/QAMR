// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x18), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x20), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x18), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(z01));
  inv1   g015(.a(x01), .O(new_n60_));
  inv1   g016(.a(new_n57_), .O(new_n61_));
  nor2   g017(.a(new_n61_), .b(new_n60_), .O(z02));
  nand2  g018(.a(new_n57_), .b(x02), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z03));
  nand2  g020(.a(new_n57_), .b(x03), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z04));
  nand2  g022(.a(new_n57_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  inv1   g024(.a(new_n56_), .O(new_n69_));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(z06));
  nand2  g027(.a(new_n57_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  nand2  g029(.a(x08), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n69_), .O(z08));
  inv1   g031(.a(x21), .O(new_n76_));
  nor2   g032(.a(new_n46_), .b(new_n76_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n58_), .c(new_n69_), .O(z09));
  inv1   g038(.a(x19), .O(new_n83_));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n79_), .c(x19), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(x20), .c(x18), .O(new_n86_));
  oai21  g042(.a(x20), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n88_));
  oai21  g044(.a(new_n61_), .b(new_n60_), .c(new_n88_), .O(z10));
  inv1   g045(.a(x18), .O(new_n90_));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  nand3  g047(.a(x26), .b(x25), .c(x24), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n91_), .c(x19), .O(new_n93_));
  nor2   g049(.a(x21), .b(new_n55_), .O(new_n94_));
  aoi22  g050(.a(new_n94_), .b(x19), .c(new_n93_), .d(x21), .O(new_n95_));
  nand2  g051(.a(x21), .b(new_n55_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n90_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n63_), .O(z11));
  nand3  g055(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n100_));
  nand3  g056(.a(new_n46_), .b(x21), .c(x19), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n100_), .c(x18), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x20), .O(new_n103_));
  nor2   g059(.a(new_n55_), .b(new_n83_), .O(new_n104_));
  inv1   g060(.a(new_n92_), .O(new_n105_));
  and2   g061(.a(x23), .b(x14), .O(new_n106_));
  aoi21  g062(.a(new_n106_), .b(new_n105_), .c(new_n76_), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n104_), .c(new_n46_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n103_), .c(new_n65_), .O(z12));
  inv1   g066(.a(x23), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(x22), .c(x21), .d(x19), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n100_), .c(x18), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(x20), .O(new_n114_));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n104_), .c(x22), .d(x21), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(new_n48_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x09), .c(new_n47_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n114_), .c(new_n67_), .O(z13));
  nand4  g076(.a(x19), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n121_));
  inv1   g077(.a(x24), .O(new_n122_));
  nand3  g078(.a(new_n77_), .b(new_n122_), .c(x23), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n121_), .c(x18), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x20), .O(new_n125_));
  nand3  g081(.a(x21), .b(x20), .c(x19), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nand3  g083(.a(x26), .b(x25), .c(x16), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n127_), .c(new_n122_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n125_), .c(new_n70_), .O(z14));
  inv1   g089(.a(x25), .O(new_n134_));
  nand4  g090(.a(new_n77_), .b(new_n134_), .c(x24), .d(x23), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n121_), .c(x18), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x20), .O(new_n137_));
  nor2   g093(.a(new_n111_), .b(new_n46_), .O(new_n138_));
  nand2  g094(.a(x26), .b(x17), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n138_), .c(x24), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n127_), .c(new_n134_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n137_), .c(new_n72_), .O(z15));
  nand3  g100(.a(new_n138_), .b(x25), .c(x24), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n126_), .c(new_n45_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n74_), .c(new_n69_), .O(z16));
endmodule


