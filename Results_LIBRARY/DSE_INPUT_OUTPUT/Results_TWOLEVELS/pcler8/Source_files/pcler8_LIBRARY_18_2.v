// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_;
  inv1   g00(.a(x11), .O(new_n45_));
  nor2   g01(.a(x15), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(x10), .O(new_n49_));
  nand2  g05(.a(x20), .b(x19), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n52_));
  inv1   g08(.a(x22), .O(new_n53_));
  inv1   g09(.a(x23), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g11(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(new_n57_));
  nand3  g13(.a(new_n57_), .b(new_n55_), .c(x21), .O(new_n58_));
  oai21  g14(.a(new_n58_), .b(new_n52_), .c(new_n47_), .O(z00));
  aoi21  g15(.a(x08), .b(x00), .c(new_n46_), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z01));
  aoi21  g17(.a(x08), .b(x01), .c(new_n46_), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z02));
  aoi21  g19(.a(x08), .b(x02), .c(new_n46_), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z03));
  aoi21  g21(.a(x08), .b(x03), .c(new_n46_), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z04));
  nand3  g23(.a(new_n47_), .b(x08), .c(x04), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z05));
  inv1   g25(.a(x05), .O(new_n70_));
  oai21  g26(.a(new_n48_), .b(new_n70_), .c(new_n47_), .O(z06));
  inv1   g27(.a(x06), .O(new_n72_));
  oai21  g28(.a(new_n48_), .b(new_n72_), .c(new_n47_), .O(z07));
  inv1   g29(.a(x07), .O(new_n74_));
  oai21  g30(.a(new_n48_), .b(new_n74_), .c(new_n47_), .O(z08));
  inv1   g31(.a(x21), .O(new_n76_));
  nor2   g32(.a(new_n53_), .b(new_n76_), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g34(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g35(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g36(.a(new_n80_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n60_), .O(z09));
  inv1   g38(.a(x19), .O(new_n83_));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n79_), .c(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x20), .O(new_n86_));
  oai21  g42(.a(x20), .b(new_n83_), .c(new_n86_), .O(new_n87_));
  nand4  g43(.a(new_n87_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n88_));
  nand2  g44(.a(new_n88_), .b(new_n62_), .O(z10));
  nand3  g45(.a(x23), .b(x22), .c(x13), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n56_), .c(x20), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(new_n83_), .c(x21), .O(new_n92_));
  nand3  g48(.a(new_n76_), .b(x20), .c(x19), .O(new_n93_));
  nand2  g49(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g50(.a(new_n94_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n95_));
  nand2  g51(.a(new_n95_), .b(new_n64_), .O(z11));
  nand2  g52(.a(x23), .b(x14), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n56_), .c(x21), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n50_), .c(x22), .O(new_n99_));
  nand3  g55(.a(new_n51_), .b(new_n53_), .c(x21), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(new_n66_), .O(z12));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(x22), .c(x21), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n50_), .c(x23), .O(new_n106_));
  nand4  g62(.a(new_n51_), .b(new_n54_), .c(x22), .d(x21), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(new_n68_), .c(new_n47_), .O(z13));
  nand3  g66(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(x23), .c(x22), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n111_), .c(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  inv1   g71(.a(new_n111_), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n115_), .c(x23), .d(x22), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n119_));
  nand2  g75(.a(x08), .b(x05), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n119_), .c(new_n46_), .O(z14));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand4  g78(.a(new_n122_), .b(x24), .c(x23), .d(x22), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n111_), .c(x25), .O(new_n124_));
  inv1   g80(.a(x25), .O(new_n125_));
  nand4  g81(.a(new_n116_), .b(new_n55_), .c(new_n125_), .d(x24), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand4  g83(.a(new_n127_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n128_));
  nand2  g84(.a(x08), .b(x06), .O(new_n129_));
  aoi21  g85(.a(new_n129_), .b(new_n128_), .c(new_n46_), .O(z15));
  inv1   g86(.a(x18), .O(new_n131_));
  nand4  g87(.a(x21), .b(x20), .c(x19), .d(new_n131_), .O(new_n132_));
  nand4  g88(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n133_));
  oai21  g89(.a(new_n133_), .b(new_n132_), .c(x26), .O(new_n134_));
  nor2   g90(.a(new_n115_), .b(new_n54_), .O(new_n135_));
  nor2   g91(.a(x26), .b(new_n125_), .O(new_n136_));
  nand4  g92(.a(new_n136_), .b(new_n135_), .c(new_n77_), .d(new_n51_), .O(new_n137_));
  nand2  g93(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand4  g94(.a(new_n138_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n139_));
  nand2  g95(.a(x08), .b(x07), .O(new_n140_));
  aoi21  g96(.a(new_n140_), .b(new_n139_), .c(new_n46_), .O(z16));
endmodule


