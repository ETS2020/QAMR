// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x13), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x12), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g013(.a(new_n57_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n46_), .O(z00));
  inv1   g015(.a(x00), .O(new_n60_));
  inv1   g016(.a(x08), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n60_), .c(new_n46_), .O(z01));
  inv1   g018(.a(x01), .O(new_n63_));
  oai21  g019(.a(new_n61_), .b(new_n63_), .c(new_n46_), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  oai21  g021(.a(new_n61_), .b(new_n65_), .c(new_n46_), .O(z03));
  inv1   g022(.a(x03), .O(new_n67_));
  inv1   g023(.a(new_n46_), .O(new_n68_));
  nor3   g024(.a(new_n68_), .b(new_n61_), .c(new_n67_), .O(z04));
  inv1   g025(.a(x04), .O(new_n70_));
  oai21  g026(.a(new_n61_), .b(new_n70_), .c(new_n46_), .O(z05));
  inv1   g027(.a(x05), .O(new_n72_));
  oai21  g028(.a(new_n61_), .b(new_n72_), .c(new_n46_), .O(z06));
  inv1   g029(.a(x06), .O(new_n74_));
  oai21  g030(.a(new_n61_), .b(new_n74_), .c(new_n46_), .O(z07));
  inv1   g031(.a(x07), .O(new_n76_));
  oai21  g032(.a(new_n61_), .b(new_n76_), .c(new_n46_), .O(z08));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n52_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  inv1   g036(.a(x24), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x26), .c(x25), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n80_), .c(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n85_));
  nand2  g041(.a(x08), .b(x00), .O(new_n86_));
  aoi21  g042(.a(new_n86_), .b(new_n85_), .c(new_n68_), .O(z09));
  xor2a  g043(.a(x20), .b(x19), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n89_));
  oai21  g045(.a(new_n61_), .b(new_n63_), .c(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  nor2   g047(.a(x10), .b(new_n48_), .O(new_n92_));
  nand3  g048(.a(x20), .b(x13), .c(x12), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n92_), .c(new_n57_), .d(new_n61_), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n91_), .O(z10));
  nand3  g052(.a(new_n78_), .b(x20), .c(x19), .O(new_n97_));
  oai21  g053(.a(new_n51_), .b(new_n78_), .c(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n99_));
  nand2  g055(.a(x08), .b(x02), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n46_), .O(new_n102_));
  inv1   g058(.a(new_n56_), .O(new_n103_));
  nor3   g059(.a(new_n78_), .b(new_n45_), .c(x10), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n103_), .c(new_n54_), .d(new_n49_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n102_), .O(z11));
  nand2  g062(.a(x23), .b(x14), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n56_), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n50_), .c(x22), .O(new_n109_));
  nand3  g065(.a(new_n51_), .b(new_n52_), .c(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n112_));
  nand2  g068(.a(x08), .b(x03), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n112_), .c(new_n68_), .O(z12));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x22), .c(x21), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n50_), .c(x23), .O(new_n117_));
  nand4  g073(.a(new_n51_), .b(new_n53_), .c(x22), .d(x21), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n120_));
  nand2  g076(.a(x08), .b(x04), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n120_), .c(new_n68_), .O(z13));
  nand3  g078(.a(x21), .b(x20), .c(x19), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n123_), .c(x24), .O(new_n126_));
  inv1   g082(.a(new_n123_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n81_), .c(x23), .d(x22), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n130_));
  nand2  g086(.a(x08), .b(x05), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n130_), .c(new_n68_), .O(z14));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(x24), .c(x23), .d(x22), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n123_), .c(x25), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand4  g092(.a(new_n127_), .b(new_n54_), .c(new_n136_), .d(x24), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n139_));
  nand2  g095(.a(x08), .b(x06), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n139_), .c(new_n68_), .O(z15));
  inv1   g097(.a(x18), .O(new_n142_));
  nand4  g098(.a(x21), .b(x20), .c(x19), .d(new_n142_), .O(new_n143_));
  nand4  g099(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n143_), .c(x26), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n136_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n82_), .c(new_n79_), .d(new_n51_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n149_));
  nand2  g105(.a(x08), .b(x07), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(new_n68_), .O(z16));
endmodule


