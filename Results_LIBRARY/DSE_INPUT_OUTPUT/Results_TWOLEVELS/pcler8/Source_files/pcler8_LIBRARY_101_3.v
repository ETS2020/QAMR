// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x13), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  nor2   g011(.a(new_n46_), .b(new_n49_), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(z01));
  nand2  g015(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  nand2  g017(.a(new_n57_), .b(x02), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z03));
  inv1   g019(.a(new_n56_), .O(new_n64_));
  nand2  g020(.a(new_n57_), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n64_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n64_), .O(z05));
  nand2  g024(.a(new_n57_), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n64_), .O(z06));
  nand2  g026(.a(new_n57_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  nand2  g028(.a(new_n57_), .b(x07), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(z08));
  nand3  g030(.a(x21), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x22), .c(new_n49_), .O(new_n78_));
  oai21  g034(.a(x22), .b(x19), .c(new_n78_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n58_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n76_), .c(x19), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x20), .O(new_n85_));
  oai21  g041(.a(x20), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n60_), .c(new_n64_), .O(z10));
  inv1   g044(.a(x21), .O(new_n89_));
  nand2  g045(.a(x20), .b(x19), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand3  g047(.a(new_n89_), .b(x20), .c(x19), .O(new_n92_));
  oai21  g048(.a(new_n91_), .b(new_n89_), .c(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(new_n62_), .c(new_n64_), .O(z11));
  nand2  g051(.a(x23), .b(x14), .O(new_n96_));
  nand3  g052(.a(x26), .b(x25), .c(x24), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n96_), .c(x21), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n90_), .c(x22), .O(new_n99_));
  nand3  g055(.a(new_n91_), .b(new_n46_), .c(x21), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(x13), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n65_), .O(z12));
  nand3  g059(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n104_));
  inv1   g060(.a(x23), .O(new_n105_));
  nand3  g061(.a(new_n91_), .b(new_n105_), .c(x21), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n104_), .c(new_n49_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x22), .O(new_n108_));
  nand4  g064(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n91_), .c(x22), .d(x21), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x23), .c(new_n48_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x09), .c(new_n47_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n108_), .c(new_n67_), .O(z13));
  nand3  g070(.a(x26), .b(x25), .c(x16), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n91_), .c(x23), .d(x21), .O(new_n116_));
  nand3  g072(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  inv1   g073(.a(x24), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x23), .c(x22), .O(new_n119_));
  nor2   g075(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  aoi21  g076(.a(new_n116_), .b(x24), .c(new_n120_), .O(new_n121_));
  nand2  g077(.a(x24), .b(new_n46_), .O(new_n122_));
  oai21  g078(.a(new_n121_), .b(x13), .c(new_n122_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n69_), .O(z14));
  inv1   g081(.a(x25), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x24), .c(x23), .O(new_n127_));
  nand3  g083(.a(x26), .b(x25), .c(x17), .O(new_n128_));
  oai21  g084(.a(new_n127_), .b(new_n117_), .c(new_n128_), .O(new_n129_));
  nand3  g085(.a(x24), .b(x23), .c(x21), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n91_), .c(new_n126_), .O(new_n132_));
  aoi21  g088(.a(new_n129_), .b(x22), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(x25), .b(new_n46_), .O(new_n134_));
  oai21  g090(.a(new_n133_), .b(x13), .c(new_n134_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n71_), .O(z15));
  nand3  g093(.a(x26), .b(x18), .c(new_n49_), .O(new_n138_));
  nand4  g094(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n117_), .c(new_n138_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x22), .O(new_n141_));
  nand4  g097(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n117_), .c(x26), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n73_), .c(new_n64_), .O(z16));
endmodule


