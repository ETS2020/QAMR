// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n63_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n103_;
  inv1   g00(.a(x10), .O(new_n46_));
  nand2  g01(.a(x23), .b(new_n46_), .O(new_n47_));
  nand2  g02(.a(x08), .b(x00), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(z01));
  nand2  g04(.a(x08), .b(x01), .O(new_n50_));
  nand2  g05(.a(new_n50_), .b(new_n47_), .O(z02));
  nand2  g06(.a(new_n47_), .b(x08), .O(new_n52_));
  inv1   g07(.a(new_n52_), .O(new_n53_));
  nand2  g08(.a(new_n53_), .b(x02), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(z03));
  nand2  g10(.a(new_n53_), .b(x03), .O(new_n56_));
  inv1   g11(.a(new_n56_), .O(z04));
  inv1   g12(.a(x04), .O(new_n58_));
  inv1   g13(.a(x08), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n58_), .c(new_n47_), .O(z05));
  inv1   g15(.a(x05), .O(new_n61_));
  nor2   g16(.a(new_n52_), .b(new_n61_), .O(z06));
  inv1   g17(.a(x06), .O(new_n63_));
  nor2   g18(.a(new_n52_), .b(new_n63_), .O(z07));
  inv1   g19(.a(x07), .O(new_n65_));
  oai21  g20(.a(new_n59_), .b(new_n65_), .c(new_n47_), .O(z08));
  inv1   g21(.a(x23), .O(new_n67_));
  inv1   g22(.a(x19), .O(new_n68_));
  nand3  g23(.a(new_n68_), .b(x09), .c(new_n59_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n46_), .O(new_n71_));
  nand2  g26(.a(new_n71_), .b(new_n48_), .O(z09));
  xor2a  g27(.a(x20), .b(x19), .O(new_n73_));
  nand3  g28(.a(new_n73_), .b(x09), .c(new_n59_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n67_), .O(new_n75_));
  nand2  g30(.a(new_n75_), .b(new_n46_), .O(new_n76_));
  nand2  g31(.a(new_n76_), .b(new_n50_), .O(z10));
  inv1   g32(.a(x09), .O(new_n78_));
  nand2  g33(.a(x20), .b(x19), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(x21), .O(new_n80_));
  inv1   g35(.a(x21), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x20), .c(x19), .O(new_n82_));
  aoi21  g37(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(new_n83_));
  aoi21  g38(.a(new_n83_), .b(new_n59_), .c(x23), .O(new_n84_));
  oai21  g39(.a(new_n84_), .b(x10), .c(new_n54_), .O(z11));
  nand3  g40(.a(x21), .b(x20), .c(x19), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(x22), .O(new_n87_));
  inv1   g42(.a(x22), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(x21), .c(x20), .d(x19), .O(new_n89_));
  aoi21  g44(.a(new_n89_), .b(new_n87_), .c(x23), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n46_), .c(x09), .d(new_n59_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n56_), .O(z12));
  nor2   g47(.a(new_n78_), .b(x08), .O(new_n93_));
  nand3  g48(.a(new_n93_), .b(x19), .c(new_n46_), .O(new_n94_));
  nand4  g49(.a(new_n67_), .b(x22), .c(x21), .d(x20), .O(new_n95_));
  oai22  g50(.a(new_n95_), .b(new_n94_), .c(new_n52_), .d(new_n58_), .O(z13));
  nand4  g51(.a(new_n93_), .b(x24), .c(new_n67_), .d(new_n46_), .O(new_n97_));
  oai21  g52(.a(new_n52_), .b(new_n61_), .c(new_n97_), .O(z14));
  nand2  g53(.a(x25), .b(x09), .O(new_n99_));
  oai21  g54(.a(new_n99_), .b(x08), .c(new_n67_), .O(new_n100_));
  nand2  g55(.a(new_n100_), .b(new_n46_), .O(new_n101_));
  oai21  g56(.a(new_n52_), .b(new_n63_), .c(new_n101_), .O(z15));
  nand4  g57(.a(new_n93_), .b(x26), .c(new_n67_), .d(new_n46_), .O(new_n103_));
  oai21  g58(.a(new_n52_), .b(new_n65_), .c(new_n103_), .O(z16));
  zero   g59(.O(z00));
endmodule


