// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x09), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x08), .O(new_n47_));
  inv1   g003(.a(x20), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g005(.a(x21), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(x25), .b(x24), .c(x23), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nand4  g010(.a(new_n54_), .b(new_n52_), .c(new_n49_), .d(new_n47_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(x19), .c(new_n45_), .O(z00));
  nor2   g012(.a(new_n45_), .b(x19), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand2  g014(.a(x08), .b(x00), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(z01));
  inv1   g016(.a(x08), .O(new_n61_));
  nor2   g017(.a(new_n57_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(x01), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n58_), .O(z02));
  nand2  g020(.a(new_n62_), .b(x02), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z03));
  nand2  g022(.a(new_n62_), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n58_), .O(z04));
  nand2  g024(.a(new_n62_), .b(x04), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n58_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n58_), .O(z06));
  nand2  g028(.a(new_n62_), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n58_), .O(z07));
  nand2  g030(.a(new_n62_), .b(x07), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n58_), .O(z08));
  inv1   g032(.a(x10), .O(new_n77_));
  nand3  g033(.a(new_n52_), .b(x20), .c(x11), .O(new_n78_));
  inv1   g034(.a(x25), .O(new_n79_));
  nor2   g035(.a(new_n45_), .b(new_n79_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x24), .c(x23), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n78_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n59_), .c(new_n58_), .O(z09));
  nand2  g040(.a(new_n52_), .b(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n81_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  inv1   g043(.a(x19), .O(new_n88_));
  nand3  g044(.a(new_n45_), .b(x20), .c(new_n88_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n63_), .O(z10));
  nand3  g048(.a(new_n47_), .b(x21), .c(new_n77_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nand3  g051(.a(x23), .b(x22), .c(x13), .O(new_n96_));
  nand3  g052(.a(x26), .b(x25), .c(x24), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(x21), .O(new_n99_));
  nand3  g055(.a(new_n50_), .b(x20), .c(x19), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n95_), .c(new_n65_), .O(z11));
  nand2  g059(.a(x23), .b(x14), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n97_), .c(x21), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n48_), .c(x22), .O(new_n106_));
  nand3  g062(.a(new_n51_), .b(x21), .c(x20), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x19), .O(new_n109_));
  nand3  g065(.a(new_n45_), .b(x22), .c(new_n88_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n67_), .O(z12));
  nor2   g069(.a(new_n50_), .b(new_n48_), .O(new_n114_));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n114_), .c(x22), .O(new_n116_));
  nor2   g072(.a(x23), .b(new_n51_), .O(new_n117_));
  aoi22  g073(.a(new_n117_), .b(new_n114_), .c(new_n116_), .d(x23), .O(new_n118_));
  nand3  g074(.a(new_n45_), .b(x23), .c(new_n88_), .O(new_n119_));
  oai21  g075(.a(new_n118_), .b(new_n88_), .c(new_n119_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n69_), .O(z13));
  nand3  g078(.a(new_n47_), .b(x24), .c(new_n77_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n45_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n88_), .O(new_n125_));
  inv1   g081(.a(new_n114_), .O(new_n126_));
  nand2  g082(.a(new_n80_), .b(x16), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x23), .c(x22), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n126_), .c(x24), .O(new_n129_));
  inv1   g085(.a(x23), .O(new_n130_));
  nor2   g086(.a(x24), .b(new_n130_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n114_), .c(x22), .d(x19), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n125_), .c(new_n71_), .O(z14));
  nand3  g091(.a(x22), .b(x21), .c(x20), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nand2  g093(.a(x26), .b(x17), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n137_), .c(x24), .d(x23), .O(new_n139_));
  nand3  g095(.a(new_n79_), .b(x24), .c(x23), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  aoi21  g097(.a(new_n139_), .b(x25), .c(new_n141_), .O(new_n142_));
  nor2   g098(.a(x26), .b(new_n79_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n88_), .O(new_n144_));
  oai21  g100(.a(new_n142_), .b(new_n88_), .c(new_n144_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n73_), .O(z15));
  inv1   g103(.a(x18), .O(new_n148_));
  nand2  g104(.a(new_n114_), .b(new_n148_), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x26), .O(new_n151_));
  nand4  g107(.a(new_n143_), .b(new_n137_), .c(x24), .d(x23), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n151_), .c(new_n88_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n77_), .c(x09), .d(new_n61_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n75_), .O(z16));
endmodule


