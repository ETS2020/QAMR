// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x00), .O(new_n54_));
  nor2   g010(.a(x24), .b(x10), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n54_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(new_n55_), .O(new_n60_));
  oai21  g016(.a(new_n46_), .b(new_n59_), .c(new_n60_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  nor2   g018(.a(new_n57_), .b(new_n62_), .O(z03));
  nand2  g019(.a(new_n56_), .b(x03), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z04));
  nand2  g021(.a(new_n56_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(new_n56_), .b(x05), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z06));
  inv1   g025(.a(x06), .O(new_n70_));
  nor2   g026(.a(new_n57_), .b(new_n70_), .O(z07));
  inv1   g027(.a(x07), .O(new_n72_));
  oai21  g028(.a(new_n46_), .b(new_n72_), .c(new_n60_), .O(z08));
  nand3  g029(.a(x21), .b(x20), .c(x11), .O(new_n74_));
  inv1   g030(.a(x22), .O(new_n75_));
  inv1   g031(.a(x23), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  inv1   g033(.a(x25), .O(new_n78_));
  nor2   g034(.a(new_n45_), .b(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n74_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(x24), .c(new_n47_), .d(x09), .O(new_n82_));
  oai22  g038(.a(new_n82_), .b(x08), .c(new_n57_), .d(new_n54_), .O(z09));
  inv1   g039(.a(x24), .O(new_n84_));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  nand3  g041(.a(x26), .b(x25), .c(x23), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(x19), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x20), .O(new_n88_));
  inv1   g044(.a(x20), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(x19), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n88_), .c(new_n84_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n92_));
  oai21  g048(.a(new_n57_), .b(new_n59_), .c(new_n92_), .O(z10));
  inv1   g049(.a(x09), .O(new_n94_));
  inv1   g050(.a(x19), .O(new_n95_));
  nand2  g051(.a(x22), .b(x13), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n86_), .c(x20), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n95_), .c(x21), .O(new_n98_));
  inv1   g054(.a(x21), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x20), .c(x19), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n98_), .c(new_n94_), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n46_), .c(new_n84_), .O(new_n102_));
  oai22  g058(.a(new_n102_), .b(x10), .c(new_n57_), .d(new_n62_), .O(z11));
  nor2   g059(.a(new_n89_), .b(new_n95_), .O(new_n104_));
  nand3  g060(.a(new_n79_), .b(x23), .c(x14), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n104_), .c(x21), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x22), .O(new_n107_));
  nand3  g063(.a(new_n104_), .b(new_n75_), .c(x21), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n107_), .c(new_n84_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n64_), .O(z12));
  inv1   g067(.a(new_n104_), .O(new_n112_));
  nand2  g068(.a(new_n79_), .b(x15), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x22), .c(x21), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x23), .O(new_n115_));
  nand4  g071(.a(new_n104_), .b(new_n76_), .c(x22), .d(x21), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n115_), .c(new_n84_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n66_), .O(z13));
  nand2  g075(.a(x23), .b(x22), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(x24), .c(new_n99_), .O(new_n121_));
  nand2  g077(.a(x24), .b(new_n89_), .O(new_n122_));
  oai21  g078(.a(new_n121_), .b(new_n89_), .c(new_n122_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x19), .O(new_n124_));
  aoi22  g080(.a(new_n79_), .b(x16), .c(x24), .d(new_n95_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n124_), .c(new_n94_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n46_), .c(new_n84_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(x10), .c(new_n68_), .O(z14));
  nand3  g084(.a(x21), .b(x20), .c(x19), .O(new_n129_));
  nand2  g085(.a(x26), .b(x17), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x23), .c(x22), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n129_), .c(x25), .O(new_n132_));
  inv1   g088(.a(new_n129_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n78_), .c(x23), .d(x22), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n132_), .c(new_n94_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n46_), .c(new_n84_), .O(new_n136_));
  oai22  g092(.a(new_n136_), .b(x10), .c(new_n57_), .d(new_n70_), .O(z15));
  inv1   g093(.a(x18), .O(new_n138_));
  nand2  g094(.a(new_n104_), .b(new_n138_), .O(new_n139_));
  nand4  g095(.a(x25), .b(x23), .c(x22), .d(x21), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n139_), .c(x26), .O(new_n141_));
  nand4  g097(.a(new_n133_), .b(new_n77_), .c(new_n45_), .d(x25), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n141_), .c(new_n84_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  oai21  g100(.a(new_n57_), .b(new_n72_), .c(new_n144_), .O(z16));
endmodule


