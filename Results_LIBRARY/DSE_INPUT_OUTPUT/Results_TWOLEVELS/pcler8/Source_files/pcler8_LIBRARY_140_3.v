// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n70_, new_n72_, new_n74_, new_n76_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x12), .O(new_n45_));
  inv1   g001(.a(x21), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x19), .O(new_n49_));
  inv1   g005(.a(x20), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  or2    g014(.a(new_n58_), .b(new_n52_), .O(new_n59_));
  aoi21  g015(.a(new_n59_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g016(.a(new_n46_), .b(new_n45_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nor2   g018(.a(new_n61_), .b(new_n47_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x00), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n62_), .O(z01));
  nand2  g021(.a(new_n63_), .b(x01), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z02));
  nand2  g023(.a(x08), .b(x02), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n62_), .O(z03));
  nand2  g025(.a(new_n63_), .b(x03), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z04));
  nand2  g027(.a(new_n63_), .b(x04), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z05));
  nand2  g029(.a(new_n63_), .b(x05), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z06));
  nand2  g031(.a(new_n63_), .b(x06), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(z07));
  nand2  g033(.a(new_n63_), .b(x07), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z08));
  nor2   g035(.a(new_n53_), .b(new_n50_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(x11), .O(new_n81_));
  nand4  g037(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n81_), .c(x19), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(x21), .c(new_n45_), .O(new_n84_));
  nand2  g040(.a(new_n46_), .b(new_n49_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n64_), .O(z09));
  xor2a  g044(.a(x20), .b(x19), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n66_), .c(new_n62_), .O(z10));
  nor2   g047(.a(x19), .b(x12), .O(new_n92_));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n56_), .c(x20), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n92_), .c(x21), .O(new_n95_));
  nand3  g051(.a(new_n46_), .b(x20), .c(x19), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n68_), .c(new_n62_), .O(z11));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n56_), .c(x20), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n49_), .c(x22), .O(new_n102_));
  nand3  g058(.a(new_n51_), .b(new_n53_), .c(x21), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand2  g061(.a(x22), .b(new_n46_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n70_), .O(z12));
  nand2  g065(.a(x23), .b(new_n53_), .O(new_n110_));
  nand4  g066(.a(new_n54_), .b(x22), .c(x20), .d(x19), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x20), .c(x19), .O(new_n114_));
  aoi22  g070(.a(new_n114_), .b(x23), .c(new_n112_), .d(x21), .O(new_n115_));
  nand2  g071(.a(x23), .b(new_n46_), .O(new_n116_));
  oai21  g072(.a(new_n115_), .b(x12), .c(new_n116_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n72_), .O(z13));
  inv1   g075(.a(x24), .O(new_n120_));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n51_), .c(x23), .d(x22), .O(new_n122_));
  nand3  g078(.a(x21), .b(x20), .c(x19), .O(new_n123_));
  nand3  g079(.a(new_n120_), .b(x23), .c(x22), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  aoi21  g081(.a(new_n122_), .b(x24), .c(new_n125_), .O(new_n126_));
  oai22  g082(.a(new_n126_), .b(x12), .c(new_n120_), .d(x21), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n74_), .O(z14));
  nand4  g085(.a(x19), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n130_));
  inv1   g086(.a(x25), .O(new_n131_));
  nand4  g087(.a(new_n80_), .b(new_n131_), .c(x24), .d(x23), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n130_), .c(new_n45_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x21), .O(new_n134_));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n55_), .c(x24), .O(new_n136_));
  nor2   g092(.a(new_n136_), .b(new_n123_), .O(new_n137_));
  nor2   g093(.a(new_n137_), .b(new_n131_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n134_), .c(new_n76_), .O(z15));
  inv1   g096(.a(x26), .O(new_n141_));
  nand4  g097(.a(new_n55_), .b(new_n141_), .c(x25), .d(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n52_), .c(new_n45_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x21), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand4  g101(.a(x21), .b(x20), .c(x19), .d(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n55_), .b(x25), .c(x24), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n141_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n144_), .c(new_n78_), .O(z16));
endmodule


