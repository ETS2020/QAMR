// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_;
  inv1   g00(.a(x08), .O(new_n45_));
  nand2  g01(.a(x21), .b(x20), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x22), .O(new_n48_));
  inv1   g04(.a(x23), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n47_), .c(x09), .d(new_n45_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x19), .c(x10), .O(z00));
  nor2   g09(.a(x19), .b(x10), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nor2   g13(.a(new_n54_), .b(new_n45_), .O(new_n58_));
  and2   g14(.a(new_n58_), .b(x01), .O(z02));
  inv1   g15(.a(x02), .O(new_n60_));
  inv1   g16(.a(new_n58_), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n60_), .O(z03));
  nand2  g18(.a(new_n58_), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n55_), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nand2  g21(.a(new_n65_), .b(new_n55_), .O(z05));
  nand2  g22(.a(new_n58_), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g24(.a(new_n58_), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g26(.a(new_n58_), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n55_), .O(z08));
  inv1   g28(.a(x19), .O(new_n73_));
  inv1   g29(.a(x09), .O(new_n74_));
  inv1   g30(.a(x11), .O(new_n75_));
  nor4   g31(.a(new_n46_), .b(new_n75_), .c(new_n74_), .d(x08), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(new_n51_), .c(new_n73_), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(x10), .c(new_n56_), .O(z09));
  nand3  g34(.a(x22), .b(x21), .c(x12), .O(new_n79_));
  nand4  g35(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n81_));
  nand3  g37(.a(new_n81_), .b(x09), .c(new_n45_), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(x19), .c(x10), .O(new_n83_));
  or2    g39(.a(new_n83_), .b(z02), .O(z10));
  inv1   g40(.a(x10), .O(new_n85_));
  nand3  g41(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n50_), .c(x20), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(x21), .O(new_n88_));
  inv1   g44(.a(x21), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(x20), .O(new_n90_));
  aoi21  g46(.a(new_n90_), .b(new_n88_), .c(new_n73_), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(new_n85_), .c(x09), .d(new_n45_), .O(new_n92_));
  oai21  g48(.a(new_n61_), .b(new_n60_), .c(new_n92_), .O(z11));
  inv1   g49(.a(x20), .O(new_n94_));
  nand2  g50(.a(x23), .b(x14), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n50_), .c(x21), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n94_), .c(x22), .O(new_n97_));
  nand3  g53(.a(new_n48_), .b(x21), .c(x20), .O(new_n98_));
  aoi21  g54(.a(new_n98_), .b(new_n97_), .c(new_n73_), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n85_), .c(x09), .d(new_n45_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n63_), .O(z12));
  oai21  g57(.a(x21), .b(new_n73_), .c(x20), .O(new_n102_));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(x22), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n102_), .c(x23), .O(new_n105_));
  nand3  g61(.a(new_n47_), .b(new_n49_), .c(x22), .O(new_n106_));
  aoi21  g62(.a(new_n106_), .b(new_n105_), .c(new_n74_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n45_), .c(new_n73_), .O(new_n108_));
  oai21  g64(.a(new_n108_), .b(x10), .c(new_n65_), .O(z13));
  nand3  g65(.a(x26), .b(x25), .c(x16), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(x23), .c(x22), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(new_n46_), .c(x24), .O(new_n112_));
  inv1   g68(.a(x24), .O(new_n113_));
  nand4  g69(.a(new_n47_), .b(new_n113_), .c(x23), .d(x22), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n112_), .c(new_n73_), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n85_), .c(x09), .d(new_n45_), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n67_), .O(z14));
  nand3  g73(.a(x22), .b(x21), .c(x20), .O(new_n118_));
  nand2  g74(.a(x26), .b(x17), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(x24), .c(x23), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n118_), .c(x25), .O(new_n121_));
  inv1   g77(.a(new_n118_), .O(new_n122_));
  nor2   g78(.a(x25), .b(new_n113_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n122_), .c(x23), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n121_), .c(new_n73_), .O(new_n125_));
  nand4  g81(.a(new_n125_), .b(new_n85_), .c(x09), .d(new_n45_), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n69_), .O(z15));
  inv1   g83(.a(x18), .O(new_n128_));
  nand2  g84(.a(new_n47_), .b(new_n128_), .O(new_n129_));
  nand4  g85(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n129_), .c(x26), .O(new_n131_));
  inv1   g87(.a(x26), .O(new_n132_));
  nor2   g88(.a(new_n113_), .b(new_n49_), .O(new_n133_));
  nand4  g89(.a(new_n122_), .b(new_n133_), .c(new_n132_), .d(x25), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n131_), .c(new_n73_), .O(new_n135_));
  nand4  g91(.a(new_n135_), .b(new_n85_), .c(x09), .d(new_n45_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n71_), .O(z16));
endmodule


