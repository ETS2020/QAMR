// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x11), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nand2  g004(.a(x20), .b(x19), .O(new_n49_));
  nor4   g005(.a(new_n49_), .b(x10), .c(new_n48_), .d(x08), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  inv1   g007(.a(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(x21), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n47_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  inv1   g015(.a(x08), .O(new_n60_));
  nor3   g016(.a(new_n46_), .b(new_n60_), .c(new_n59_), .O(z01));
  nor2   g017(.a(new_n46_), .b(new_n60_), .O(new_n62_));
  and2   g018(.a(new_n62_), .b(x01), .O(z02));
  and2   g019(.a(new_n62_), .b(x02), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n47_), .O(z04));
  nand2  g022(.a(new_n62_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n47_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n47_), .O(z07));
  nand2  g028(.a(new_n62_), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n47_), .O(z08));
  inv1   g030(.a(x10), .O(new_n75_));
  inv1   g031(.a(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n77_));
  oai21  g033(.a(new_n60_), .b(new_n59_), .c(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n47_), .O(new_n79_));
  nor2   g035(.a(x10), .b(new_n48_), .O(new_n80_));
  nand3  g036(.a(x20), .b(x18), .c(x11), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n80_), .c(new_n56_), .d(new_n60_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n79_), .O(z09));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  nand4  g041(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n86_));
  oai21  g042(.a(new_n86_), .b(new_n85_), .c(x19), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x20), .O(new_n88_));
  oai21  g044(.a(x20), .b(new_n76_), .c(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n90_));
  nand2  g046(.a(x08), .b(x01), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n90_), .c(new_n46_), .O(z10));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n55_), .c(x20), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n76_), .c(x21), .O(new_n95_));
  inv1   g051(.a(x21), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x20), .c(x19), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n99_));
  nand2  g055(.a(x08), .b(x02), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n99_), .c(new_n46_), .O(z11));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n55_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n49_), .c(x22), .O(new_n104_));
  inv1   g060(.a(new_n49_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n51_), .c(x21), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n65_), .c(new_n47_), .O(z12));
  nand4  g065(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n49_), .c(x23), .O(new_n112_));
  nand4  g068(.a(new_n105_), .b(new_n52_), .c(x22), .d(x21), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n67_), .c(new_n47_), .O(z13));
  nand3  g072(.a(x21), .b(x20), .c(x19), .O(new_n117_));
  nand3  g073(.a(x26), .b(x25), .c(x16), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x23), .c(x22), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(x24), .O(new_n120_));
  inv1   g076(.a(x24), .O(new_n121_));
  inv1   g077(.a(new_n117_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n121_), .c(x23), .d(x22), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n120_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n69_), .c(new_n47_), .O(z14));
  nand2  g082(.a(x26), .b(x17), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n53_), .c(x24), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n117_), .c(x25), .O(new_n129_));
  inv1   g085(.a(x25), .O(new_n130_));
  nand4  g086(.a(new_n122_), .b(new_n53_), .c(new_n130_), .d(x24), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n71_), .c(new_n47_), .O(z15));
  nand4  g090(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n135_));
  nor2   g091(.a(new_n135_), .b(new_n117_), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(x26), .c(x18), .O(new_n137_));
  oai21  g093(.a(new_n135_), .b(new_n117_), .c(x26), .O(new_n138_));
  nor2   g094(.a(new_n121_), .b(new_n52_), .O(new_n139_));
  nand4  g095(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nor2   g097(.a(x26), .b(new_n130_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n141_), .c(new_n139_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n138_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n45_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n137_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n75_), .c(x09), .d(new_n60_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n73_), .O(z16));
endmodule


