// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x16), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  inv1   g011(.a(x21), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n49_), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nor2   g014(.a(new_n57_), .b(new_n47_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x00), .O(new_n60_));
  nand2  g016(.a(new_n60_), .b(new_n58_), .O(z01));
  nand2  g017(.a(new_n59_), .b(x01), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z02));
  nand2  g019(.a(x08), .b(x02), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n58_), .O(z03));
  nand2  g021(.a(new_n59_), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n58_), .O(z04));
  nand2  g023(.a(new_n59_), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n58_), .O(z05));
  nand2  g025(.a(new_n59_), .b(x05), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n58_), .O(z07));
  nand2  g029(.a(new_n59_), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n58_), .O(z08));
  inv1   g031(.a(x20), .O(new_n76_));
  nor2   g032(.a(new_n46_), .b(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x21), .c(new_n49_), .O(new_n81_));
  inv1   g037(.a(x19), .O(new_n82_));
  nand2  g038(.a(new_n56_), .b(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(new_n81_), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n60_), .O(z09));
  inv1   g042(.a(x09), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(x08), .O(new_n88_));
  nand3  g044(.a(x20), .b(new_n82_), .c(new_n48_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  aoi22  g046(.a(new_n90_), .b(new_n88_), .c(x08), .d(x01), .O(new_n91_));
  nand3  g047(.a(x22), .b(x20), .c(x12), .O(new_n92_));
  oai22  g048(.a(new_n92_), .b(new_n79_), .c(x20), .d(new_n82_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(x21), .c(new_n49_), .O(new_n94_));
  nand3  g050(.a(new_n56_), .b(new_n76_), .c(x19), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n97_));
  oai21  g053(.a(new_n91_), .b(new_n57_), .c(new_n97_), .O(z10));
  aoi21  g054(.a(x20), .b(x19), .c(x16), .O(new_n99_));
  nand3  g055(.a(x23), .b(x22), .c(x13), .O(new_n100_));
  nand3  g056(.a(x26), .b(x25), .c(x24), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n99_), .c(x21), .O(new_n103_));
  nand3  g059(.a(new_n56_), .b(x20), .c(x19), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n64_), .c(new_n58_), .O(z11));
  nand3  g063(.a(new_n46_), .b(x20), .c(x19), .O(new_n108_));
  nand3  g064(.a(x23), .b(x22), .c(x14), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n101_), .c(new_n108_), .O(new_n110_));
  aoi21  g066(.a(x20), .b(x19), .c(new_n46_), .O(new_n111_));
  aoi21  g067(.a(new_n110_), .b(x21), .c(new_n111_), .O(new_n112_));
  nand2  g068(.a(x22), .b(new_n56_), .O(new_n113_));
  oai21  g069(.a(new_n112_), .b(x16), .c(new_n113_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n66_), .O(z12));
  nand2  g072(.a(x20), .b(x19), .O(new_n117_));
  inv1   g073(.a(x23), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x22), .O(new_n119_));
  nand2  g075(.a(x23), .b(x15), .O(new_n120_));
  oai22  g076(.a(new_n120_), .b(new_n101_), .c(new_n119_), .d(new_n117_), .O(new_n121_));
  nand3  g077(.a(x22), .b(x20), .c(x19), .O(new_n122_));
  and2   g078(.a(new_n122_), .b(x23), .O(new_n123_));
  aoi21  g079(.a(new_n121_), .b(x21), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(x23), .b(new_n56_), .O(new_n125_));
  oai21  g081(.a(new_n124_), .b(x16), .c(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n68_), .O(z13));
  nand3  g084(.a(new_n88_), .b(x19), .c(new_n48_), .O(new_n129_));
  inv1   g085(.a(x24), .O(new_n130_));
  nand3  g086(.a(new_n77_), .b(new_n130_), .c(x23), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n129_), .c(new_n49_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x21), .O(new_n133_));
  nor4   g089(.a(new_n117_), .b(new_n118_), .c(new_n46_), .d(new_n56_), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n133_), .c(new_n70_), .O(z14));
  inv1   g093(.a(x25), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x24), .c(x23), .O(new_n139_));
  nand3  g095(.a(x26), .b(x25), .c(x17), .O(new_n140_));
  oai21  g096(.a(new_n139_), .b(new_n122_), .c(new_n140_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x21), .O(new_n142_));
  nand3  g098(.a(x21), .b(x20), .c(x19), .O(new_n143_));
  nand3  g099(.a(x24), .b(x23), .c(x22), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n143_), .c(x25), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n142_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n72_), .c(new_n58_), .O(z15));
  nor2   g104(.a(new_n117_), .b(x18), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n149_), .c(new_n45_), .O(new_n152_));
  nand4  g108(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n153_));
  nand4  g109(.a(new_n45_), .b(x25), .c(x24), .d(x23), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n152_), .c(new_n49_), .O(new_n156_));
  nand2  g112(.a(x26), .b(new_n56_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n74_), .O(z16));
endmodule


