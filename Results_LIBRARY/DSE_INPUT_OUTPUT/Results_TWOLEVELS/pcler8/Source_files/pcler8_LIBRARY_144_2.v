// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x00), .O(new_n47_));
  inv1   g003(.a(x08), .O(new_n48_));
  inv1   g004(.a(x10), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  nand3  g011(.a(x10), .b(x08), .c(x00), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(z01));
  nor2   g013(.a(x10), .b(new_n47_), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n48_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  inv1   g017(.a(new_n58_), .O(new_n62_));
  nand2  g018(.a(new_n59_), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n62_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n62_), .O(z04));
  nand2  g022(.a(new_n59_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n62_), .O(z06));
  nand2  g026(.a(new_n59_), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n62_), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  inv1   g029(.a(new_n59_), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(z08));
  oai21  g031(.a(new_n49_), .b(x08), .c(x00), .O(new_n76_));
  inv1   g032(.a(x21), .O(new_n77_));
  nor2   g033(.a(new_n46_), .b(new_n77_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(x20), .c(x11), .O(new_n79_));
  inv1   g035(.a(x23), .O(new_n80_));
  inv1   g036(.a(x24), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x26), .c(x25), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n79_), .c(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n49_), .c(x09), .d(new_n48_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n76_), .O(z09));
  nand2  g042(.a(new_n78_), .b(x12), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n83_), .c(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x20), .O(new_n89_));
  inv1   g045(.a(x20), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x19), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n89_), .c(x10), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n60_), .O(z10));
  inv1   g050(.a(x19), .O(new_n95_));
  nand3  g051(.a(x23), .b(x22), .c(x13), .O(new_n96_));
  nand3  g052(.a(x26), .b(x25), .c(x24), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n95_), .c(x21), .O(new_n99_));
  nand3  g055(.a(new_n77_), .b(x20), .c(x19), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n99_), .c(x10), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n63_), .O(z11));
  nand2  g059(.a(x23), .b(x14), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n97_), .c(x20), .O(new_n105_));
  nand2  g061(.a(x21), .b(x19), .O(new_n106_));
  aoi21  g062(.a(new_n105_), .b(new_n47_), .c(new_n106_), .O(new_n107_));
  nor2   g063(.a(new_n90_), .b(new_n95_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n46_), .c(x21), .O(new_n109_));
  oai21  g065(.a(new_n107_), .b(new_n46_), .c(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x09), .c(new_n48_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n47_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n49_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n65_), .O(z12));
  inv1   g070(.a(x09), .O(new_n115_));
  nand4  g071(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(x20), .c(x00), .O(new_n117_));
  nand2  g073(.a(new_n78_), .b(x19), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n117_), .c(x23), .O(new_n119_));
  nand4  g075(.a(new_n108_), .b(new_n80_), .c(x22), .d(x21), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n115_), .O(new_n121_));
  aoi21  g077(.a(new_n121_), .b(new_n48_), .c(x00), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(x10), .c(new_n67_), .O(z13));
  nand3  g079(.a(x21), .b(x20), .c(x19), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand4  g081(.a(x26), .b(x25), .c(x16), .d(new_n47_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n125_), .c(x23), .d(x22), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x24), .O(new_n128_));
  nand4  g084(.a(new_n125_), .b(new_n81_), .c(x23), .d(x22), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n128_), .c(new_n115_), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n48_), .c(x00), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(x10), .c(new_n69_), .O(z14));
  nor2   g088(.a(new_n80_), .b(new_n46_), .O(new_n133_));
  nand2  g089(.a(x26), .b(x17), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n133_), .c(x24), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n124_), .c(x25), .O(new_n136_));
  inv1   g092(.a(x25), .O(new_n137_));
  nand4  g093(.a(new_n125_), .b(new_n133_), .c(new_n137_), .d(x24), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n136_), .c(x10), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n71_), .O(z15));
  inv1   g097(.a(x18), .O(new_n142_));
  nand4  g098(.a(x21), .b(x20), .c(x19), .d(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n133_), .b(x25), .c(x24), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n143_), .c(x26), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n137_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n108_), .c(new_n82_), .d(new_n78_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n145_), .c(x10), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n149_));
  oai21  g105(.a(new_n74_), .b(new_n73_), .c(new_n149_), .O(z16));
endmodule


