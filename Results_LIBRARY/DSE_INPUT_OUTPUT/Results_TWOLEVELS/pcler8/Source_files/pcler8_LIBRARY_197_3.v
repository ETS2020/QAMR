// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nand2  g009(.a(x24), .b(x10), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x08), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n54_), .O(z01));
  nand2  g014(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  nand2  g016(.a(new_n56_), .b(x02), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z03));
  nand2  g018(.a(new_n56_), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n54_), .O(z04));
  nand2  g020(.a(x08), .b(x04), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n54_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n54_), .O(z06));
  nand2  g024(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(x08), .b(x07), .O(new_n71_));
  and2   g027(.a(new_n71_), .b(new_n54_), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  nand4  g029(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  nand4  g030(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n57_), .O(z09));
  inv1   g034(.a(x19), .O(new_n79_));
  nand3  g035(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n75_), .c(x19), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x20), .O(new_n82_));
  oai21  g038(.a(x20), .b(new_n79_), .c(new_n82_), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n59_), .O(z10));
  nand3  g041(.a(x23), .b(x22), .c(x13), .O(new_n86_));
  nand3  g042(.a(x26), .b(x25), .c(x24), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n79_), .c(x21), .O(new_n89_));
  inv1   g045(.a(x21), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(x20), .c(x19), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n61_), .O(z11));
  nand2  g050(.a(x20), .b(x19), .O(new_n95_));
  nand2  g051(.a(x23), .b(x14), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n87_), .c(x21), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n95_), .c(x22), .O(new_n98_));
  inv1   g054(.a(x22), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x21), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n95_), .c(new_n98_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n63_), .O(z12));
  nand3  g059(.a(x15), .b(x09), .c(new_n46_), .O(new_n104_));
  nand3  g060(.a(x26), .b(x25), .c(x23), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n104_), .c(new_n47_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x24), .O(new_n107_));
  nand4  g063(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x23), .O(new_n109_));
  inv1   g065(.a(x23), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n95_), .c(new_n109_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n107_), .c(new_n65_), .O(z13));
  inv1   g070(.a(x24), .O(new_n115_));
  nand3  g071(.a(x21), .b(x20), .c(x19), .O(new_n116_));
  nand3  g072(.a(new_n115_), .b(x23), .c(x22), .O(new_n117_));
  oai22  g073(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(x20), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n47_), .O(new_n119_));
  nand2  g075(.a(x21), .b(x19), .O(new_n120_));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(x24), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n119_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x09), .c(new_n46_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n67_), .c(new_n54_), .O(z14));
  inv1   g082(.a(x25), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x23), .c(x22), .O(new_n128_));
  nand3  g084(.a(x26), .b(x25), .c(x17), .O(new_n129_));
  oai21  g085(.a(new_n128_), .b(new_n116_), .c(new_n129_), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x24), .O(new_n131_));
  nor2   g087(.a(new_n115_), .b(new_n110_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n116_), .c(x25), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n69_), .O(z15));
  nand3  g093(.a(x25), .b(x24), .c(x21), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n47_), .O(new_n139_));
  inv1   g095(.a(x18), .O(new_n140_));
  nand2  g096(.a(x19), .b(new_n140_), .O(new_n141_));
  nand3  g097(.a(x23), .b(x22), .c(x20), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x24), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n139_), .c(new_n45_), .O(new_n144_));
  nand3  g100(.a(new_n132_), .b(new_n45_), .c(x25), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n108_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(x09), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(x08), .c(new_n72_), .O(z16));
endmodule


