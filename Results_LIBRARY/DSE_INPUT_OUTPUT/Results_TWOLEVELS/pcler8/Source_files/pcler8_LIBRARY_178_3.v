// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nand2  g009(.a(x19), .b(x10), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x08), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(z01));
  nand2  g014(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  nand2  g016(.a(new_n56_), .b(x02), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z03));
  nand2  g018(.a(new_n56_), .b(x03), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z04));
  nand2  g020(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n54_), .O(z06));
  nand2  g024(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(new_n56_), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n54_), .O(z08));
  nand4  g028(.a(x21), .b(x20), .c(x19), .d(x11), .O(new_n73_));
  nand2  g029(.a(x23), .b(x22), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand3  g031(.a(x26), .b(x25), .c(x24), .O(new_n76_));
  inv1   g032(.a(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  oai22  g034(.a(new_n78_), .b(new_n73_), .c(x19), .d(x10), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x09), .c(new_n46_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n57_), .c(new_n54_), .O(z09));
  nand3  g037(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nand4  g038(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x19), .O(new_n85_));
  inv1   g041(.a(x19), .O(new_n86_));
  nand3  g042(.a(x20), .b(new_n86_), .c(new_n47_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(x09), .c(new_n46_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n59_), .c(new_n54_), .O(z10));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n76_), .c(x20), .O(new_n92_));
  inv1   g048(.a(x20), .O(new_n93_));
  nor2   g049(.a(x21), .b(new_n93_), .O(new_n94_));
  aoi21  g050(.a(new_n92_), .b(x21), .c(new_n94_), .O(new_n95_));
  nand3  g051(.a(x21), .b(new_n86_), .c(new_n47_), .O(new_n96_));
  oai21  g052(.a(new_n95_), .b(new_n86_), .c(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x09), .c(new_n46_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n61_), .c(new_n54_), .O(z11));
  inv1   g055(.a(x22), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x21), .c(x20), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  inv1   g058(.a(x14), .O(new_n103_));
  nor3   g059(.a(new_n76_), .b(new_n74_), .c(new_n103_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n102_), .c(x19), .O(new_n105_));
  nand3  g061(.a(x21), .b(x20), .c(x19), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x22), .c(new_n47_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x09), .c(new_n46_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n63_), .c(new_n54_), .O(z12));
  nor2   g066(.a(new_n93_), .b(new_n86_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x22), .c(x21), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n112_), .c(x23), .O(new_n115_));
  inv1   g071(.a(x23), .O(new_n116_));
  nand4  g072(.a(new_n111_), .b(new_n116_), .c(x22), .d(x21), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n65_), .O(z13));
  nand2  g076(.a(x21), .b(x20), .O(new_n121_));
  inv1   g077(.a(x24), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x23), .c(x22), .O(new_n123_));
  nand4  g079(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n124_));
  oai21  g080(.a(new_n123_), .b(new_n121_), .c(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x19), .O(new_n126_));
  nand3  g082(.a(new_n111_), .b(new_n75_), .c(x21), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(x24), .c(new_n47_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n126_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x09), .c(new_n46_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n67_), .c(new_n54_), .O(z14));
  nand3  g087(.a(x22), .b(x21), .c(x20), .O(new_n132_));
  inv1   g088(.a(x25), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x24), .c(x23), .O(new_n134_));
  nand4  g090(.a(x26), .b(x25), .c(x17), .d(new_n47_), .O(new_n135_));
  oai21  g091(.a(new_n134_), .b(new_n132_), .c(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x19), .O(new_n137_));
  nor2   g093(.a(new_n122_), .b(new_n116_), .O(new_n138_));
  inv1   g094(.a(new_n106_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n138_), .c(x22), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(x25), .c(new_n47_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n137_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x09), .c(new_n46_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n69_), .c(new_n54_), .O(z15));
  inv1   g100(.a(x18), .O(new_n145_));
  nand4  g101(.a(x21), .b(x20), .c(x19), .d(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n75_), .b(x25), .c(x24), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n146_), .c(x26), .O(new_n148_));
  and2   g104(.a(x22), .b(x21), .O(new_n149_));
  nor2   g105(.a(x26), .b(new_n133_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n111_), .c(new_n138_), .d(new_n149_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n148_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n71_), .O(z16));
endmodule


