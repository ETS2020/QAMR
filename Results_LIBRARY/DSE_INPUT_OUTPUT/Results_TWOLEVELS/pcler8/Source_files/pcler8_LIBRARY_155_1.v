// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x22), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand2  g03(.a(x20), .b(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x21), .O(new_n50_));
  inv1   g06(.a(x23), .O(new_n51_));
  nand3  g07(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  nor3   g08(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand4  g09(.a(new_n53_), .b(new_n49_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  aoi21  g10(.a(new_n54_), .b(x09), .c(new_n45_), .O(z00));
  inv1   g11(.a(x00), .O(new_n56_));
  inv1   g12(.a(x09), .O(new_n57_));
  nand2  g13(.a(x22), .b(new_n57_), .O(new_n58_));
  oai21  g14(.a(new_n46_), .b(new_n56_), .c(new_n58_), .O(z01));
  nand2  g15(.a(x08), .b(x01), .O(new_n60_));
  nand2  g16(.a(new_n60_), .b(new_n58_), .O(z02));
  nand3  g17(.a(new_n58_), .b(x08), .c(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  inv1   g19(.a(x03), .O(new_n64_));
  nand2  g20(.a(new_n58_), .b(x08), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(new_n64_), .O(z04));
  inv1   g22(.a(x04), .O(new_n67_));
  nor2   g23(.a(new_n65_), .b(new_n67_), .O(z05));
  inv1   g24(.a(x05), .O(new_n69_));
  oai21  g25(.a(new_n46_), .b(new_n69_), .c(new_n58_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  oai21  g27(.a(new_n46_), .b(new_n71_), .c(new_n58_), .O(z07));
  nand2  g28(.a(x08), .b(x07), .O(new_n73_));
  and2   g29(.a(new_n73_), .b(new_n58_), .O(new_n74_));
  inv1   g30(.a(new_n74_), .O(z08));
  nor2   g31(.a(new_n45_), .b(new_n50_), .O(new_n76_));
  nand3  g32(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  nand2  g33(.a(x24), .b(x23), .O(new_n78_));
  inv1   g34(.a(new_n78_), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(x26), .c(x25), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n77_), .c(x19), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n82_));
  oai21  g38(.a(new_n65_), .b(new_n56_), .c(new_n82_), .O(z09));
  xnor2a g39(.a(x20), .b(x19), .O(new_n84_));
  nand3  g40(.a(new_n76_), .b(x20), .c(x12), .O(new_n85_));
  oai22  g41(.a(new_n85_), .b(new_n80_), .c(new_n84_), .d(new_n57_), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n47_), .c(new_n46_), .O(new_n87_));
  nand3  g43(.a(new_n87_), .b(new_n60_), .c(new_n58_), .O(z10));
  nand4  g44(.a(x21), .b(x13), .c(new_n47_), .d(new_n46_), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n80_), .c(x09), .O(new_n90_));
  nand2  g46(.a(new_n90_), .b(x22), .O(new_n91_));
  nand3  g47(.a(new_n50_), .b(x20), .c(x19), .O(new_n92_));
  oai21  g48(.a(new_n49_), .b(new_n50_), .c(new_n92_), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n94_));
  nand3  g50(.a(new_n94_), .b(new_n91_), .c(new_n62_), .O(z11));
  nand2  g51(.a(x23), .b(x14), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n52_), .c(x21), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n48_), .c(x22), .O(new_n98_));
  nand3  g54(.a(new_n49_), .b(new_n45_), .c(x21), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g56(.a(new_n100_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n101_));
  oai21  g57(.a(new_n65_), .b(new_n64_), .c(new_n101_), .O(z12));
  nand4  g58(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n103_));
  nand3  g59(.a(new_n103_), .b(x22), .c(x21), .O(new_n104_));
  oai21  g60(.a(new_n104_), .b(new_n48_), .c(x23), .O(new_n105_));
  nand4  g61(.a(new_n49_), .b(new_n51_), .c(x22), .d(x21), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g63(.a(new_n107_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n108_));
  oai21  g64(.a(new_n65_), .b(new_n67_), .c(new_n108_), .O(z13));
  nand3  g65(.a(x21), .b(x20), .c(x19), .O(new_n110_));
  nand3  g66(.a(x26), .b(x25), .c(x16), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x23), .c(x22), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n110_), .c(x24), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  inv1   g70(.a(new_n110_), .O(new_n115_));
  nand4  g71(.a(new_n115_), .b(new_n114_), .c(x23), .d(x22), .O(new_n116_));
  nand2  g72(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n118_));
  oai21  g74(.a(new_n65_), .b(new_n69_), .c(new_n118_), .O(z14));
  nand2  g75(.a(x26), .b(x17), .O(new_n120_));
  nand4  g76(.a(new_n120_), .b(x24), .c(x23), .d(x22), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n110_), .c(x25), .O(new_n122_));
  nor2   g78(.a(x25), .b(new_n114_), .O(new_n123_));
  nand4  g79(.a(new_n123_), .b(new_n115_), .c(x23), .d(x22), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand4  g81(.a(new_n125_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n126_));
  oai21  g82(.a(new_n65_), .b(new_n71_), .c(new_n126_), .O(z15));
  inv1   g83(.a(x26), .O(new_n128_));
  oai21  g84(.a(new_n78_), .b(new_n45_), .c(x09), .O(new_n129_));
  inv1   g85(.a(x18), .O(new_n130_));
  nand2  g86(.a(x19), .b(new_n130_), .O(new_n131_));
  nand3  g87(.a(x25), .b(x21), .c(x20), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n131_), .c(x22), .O(new_n133_));
  aoi21  g89(.a(new_n133_), .b(new_n129_), .c(new_n128_), .O(new_n134_));
  nand2  g90(.a(new_n76_), .b(new_n49_), .O(new_n135_));
  nand3  g91(.a(new_n79_), .b(new_n128_), .c(x25), .O(new_n136_));
  nor2   g92(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  oai21  g93(.a(new_n137_), .b(new_n134_), .c(new_n47_), .O(new_n138_));
  oai21  g94(.a(new_n138_), .b(x08), .c(new_n74_), .O(z16));
endmodule


