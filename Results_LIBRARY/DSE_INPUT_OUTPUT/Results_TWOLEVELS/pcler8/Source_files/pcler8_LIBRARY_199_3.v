// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x13), .O(new_n45_));
  inv1   g001(.a(x18), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  inv1   g004(.a(x10), .O(new_n49_));
  inv1   g005(.a(x09), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(x08), .O(new_n51_));
  nand2  g007(.a(x20), .b(x19), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  nand3  g009(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(new_n54_));
  nand3  g010(.a(x23), .b(x22), .c(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  oai21  g014(.a(new_n58_), .b(new_n54_), .c(new_n48_), .O(z00));
  inv1   g015(.a(x08), .O(new_n60_));
  nor2   g016(.a(new_n47_), .b(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(x00), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z01));
  nand2  g019(.a(new_n61_), .b(x01), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z02));
  inv1   g021(.a(x02), .O(new_n66_));
  oai21  g022(.a(new_n60_), .b(new_n66_), .c(new_n48_), .O(z03));
  nand2  g023(.a(new_n61_), .b(x03), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z04));
  nand2  g025(.a(new_n61_), .b(x04), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z05));
  inv1   g027(.a(x05), .O(new_n72_));
  oai21  g028(.a(new_n60_), .b(new_n72_), .c(new_n48_), .O(z06));
  nand2  g029(.a(new_n61_), .b(x06), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z07));
  inv1   g031(.a(x07), .O(new_n76_));
  oai21  g032(.a(new_n60_), .b(new_n76_), .c(new_n48_), .O(z08));
  nand4  g033(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n81_));
  nand2  g037(.a(x08), .b(x00), .O(new_n82_));
  aoi21  g038(.a(new_n82_), .b(new_n81_), .c(new_n47_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n79_), .c(x19), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g042(.a(x20), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n90_));
  nand2  g046(.a(x08), .b(x01), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n90_), .c(new_n47_), .O(z10));
  nand2  g048(.a(new_n52_), .b(x21), .O(new_n93_));
  inv1   g049(.a(x21), .O(new_n94_));
  nand3  g050(.a(new_n94_), .b(x20), .c(x19), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n97_));
  oai21  g053(.a(new_n60_), .b(new_n66_), .c(new_n97_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n48_), .O(new_n99_));
  nor2   g055(.a(x18), .b(new_n45_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n57_), .c(new_n51_), .d(new_n49_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n99_), .O(z11));
  nand2  g058(.a(x23), .b(x14), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n56_), .c(x21), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n52_), .c(x22), .O(new_n105_));
  inv1   g061(.a(x22), .O(new_n106_));
  nand3  g062(.a(new_n53_), .b(new_n106_), .c(x21), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n68_), .c(new_n48_), .O(z12));
  nand4  g066(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n52_), .c(x23), .O(new_n113_));
  inv1   g069(.a(x23), .O(new_n114_));
  nand4  g070(.a(new_n53_), .b(new_n114_), .c(x22), .d(x21), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n117_));
  nand2  g073(.a(x08), .b(x04), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n47_), .O(z13));
  nand3  g075(.a(x21), .b(x20), .c(x19), .O(new_n120_));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(x24), .O(new_n123_));
  inv1   g079(.a(new_n120_), .O(new_n124_));
  nor2   g080(.a(x24), .b(new_n114_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n124_), .c(x22), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n128_));
  nand2  g084(.a(x08), .b(x05), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n128_), .c(new_n47_), .O(z14));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(x24), .c(x23), .d(x22), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n120_), .c(x25), .O(new_n133_));
  inv1   g089(.a(x25), .O(new_n134_));
  nor2   g090(.a(new_n114_), .b(new_n106_), .O(new_n135_));
  nand4  g091(.a(new_n124_), .b(new_n135_), .c(new_n134_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n74_), .c(new_n48_), .O(z15));
  inv1   g095(.a(x26), .O(new_n140_));
  inv1   g096(.a(new_n55_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n140_), .c(x25), .d(x24), .O(new_n142_));
  oai22  g098(.a(new_n142_), .b(new_n54_), .c(new_n60_), .d(new_n76_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n48_), .O(new_n144_));
  nand4  g100(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n120_), .c(new_n46_), .O(new_n146_));
  nand2  g102(.a(x18), .b(new_n45_), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n146_), .c(new_n140_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n49_), .c(x09), .d(new_n60_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n144_), .O(z16));
endmodule


