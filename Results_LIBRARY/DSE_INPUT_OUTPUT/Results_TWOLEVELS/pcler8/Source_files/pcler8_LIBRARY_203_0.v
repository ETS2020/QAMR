// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_, new_n158_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x09), .c(new_n47_), .d(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  nor2   g010(.a(x10), .b(x02), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(new_n56_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  inv1   g018(.a(new_n55_), .O(new_n63_));
  oai21  g019(.a(new_n47_), .b(new_n62_), .c(new_n63_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n63_), .O(z04));
  inv1   g022(.a(x04), .O(new_n67_));
  nor2   g023(.a(new_n60_), .b(new_n67_), .O(z05));
  inv1   g024(.a(x05), .O(new_n69_));
  nor2   g025(.a(new_n60_), .b(new_n69_), .O(z06));
  nand2  g026(.a(new_n56_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  nor2   g029(.a(new_n60_), .b(new_n73_), .O(z08));
  nand4  g030(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x09), .c(new_n47_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x02), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n48_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n57_), .O(z09));
  inv1   g037(.a(x09), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n76_), .c(x19), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x20), .O(new_n85_));
  inv1   g041(.a(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  aoi21  g043(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n47_), .c(new_n62_), .O(new_n89_));
  oai22  g045(.a(new_n89_), .b(x10), .c(new_n60_), .d(new_n59_), .O(z10));
  inv1   g046(.a(x19), .O(new_n91_));
  nand3  g047(.a(x23), .b(x22), .c(x13), .O(new_n92_));
  nand3  g048(.a(x26), .b(x25), .c(x24), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n91_), .c(x21), .O(new_n95_));
  inv1   g051(.a(x21), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x20), .c(x19), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n48_), .c(x09), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n47_), .c(new_n62_), .O(z11));
  nand2  g056(.a(x20), .b(x19), .O(new_n101_));
  nand3  g057(.a(x23), .b(x14), .c(x02), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n93_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(x22), .O(new_n104_));
  inv1   g060(.a(new_n101_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n46_), .c(x21), .O(new_n106_));
  aoi21  g062(.a(new_n106_), .b(new_n104_), .c(new_n82_), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n47_), .c(new_n62_), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(x10), .c(new_n65_), .O(z12));
  nand4  g065(.a(x15), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n110_));
  oai22  g066(.a(new_n110_), .b(new_n76_), .c(new_n47_), .d(new_n67_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x02), .O(new_n112_));
  inv1   g068(.a(x23), .O(new_n113_));
  nand4  g069(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n113_), .b(x22), .c(x21), .O(new_n116_));
  oai22  g072(.a(new_n116_), .b(new_n101_), .c(new_n115_), .d(new_n113_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x09), .c(new_n47_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x02), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n48_), .O(new_n120_));
  nand3  g076(.a(x10), .b(x08), .c(x04), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n120_), .c(new_n112_), .O(z13));
  nand3  g078(.a(x21), .b(x20), .c(x19), .O(new_n123_));
  nand3  g079(.a(x26), .b(x25), .c(x16), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(x23), .c(x22), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n123_), .c(x24), .O(new_n126_));
  inv1   g082(.a(x24), .O(new_n127_));
  inv1   g083(.a(new_n123_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n127_), .c(x23), .d(x22), .O(new_n129_));
  aoi21  g085(.a(new_n129_), .b(new_n126_), .c(x10), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(x09), .c(new_n47_), .d(x02), .O(new_n131_));
  oai21  g087(.a(new_n60_), .b(new_n69_), .c(new_n131_), .O(z14));
  nand2  g088(.a(x26), .b(x17), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(x23), .c(new_n62_), .O(new_n134_));
  nand3  g090(.a(x24), .b(x22), .c(x21), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n105_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n134_), .c(x25), .O(new_n138_));
  nor2   g094(.a(x25), .b(new_n127_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n128_), .c(x23), .d(x22), .O(new_n140_));
  aoi21  g096(.a(new_n140_), .b(new_n138_), .c(new_n82_), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n47_), .c(new_n62_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(x10), .c(new_n71_), .O(z15));
  nand2  g099(.a(x09), .b(new_n47_), .O(new_n144_));
  nand3  g100(.a(x26), .b(new_n113_), .c(new_n48_), .O(new_n145_));
  oai22  g101(.a(new_n145_), .b(new_n144_), .c(new_n47_), .d(new_n73_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x02), .O(new_n147_));
  inv1   g103(.a(x18), .O(new_n148_));
  nand3  g104(.a(x20), .b(x19), .c(new_n148_), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x26), .O(new_n151_));
  nor2   g107(.a(new_n127_), .b(new_n113_), .O(new_n152_));
  nand4  g108(.a(new_n115_), .b(new_n152_), .c(new_n45_), .d(x25), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x09), .c(new_n47_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x02), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n48_), .O(new_n157_));
  nand3  g113(.a(x10), .b(x08), .c(x07), .O(new_n158_));
  nand3  g114(.a(new_n158_), .b(new_n157_), .c(new_n147_), .O(z16));
endmodule


