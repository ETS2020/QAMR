// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_;
  nor2   g000(.a(x26), .b(x19), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  and2   g005(.a(x20), .b(x19), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand3  g012(.a(new_n56_), .b(new_n54_), .c(x21), .O(new_n57_));
  oai21  g013(.a(new_n57_), .b(new_n51_), .c(new_n46_), .O(z00));
  nand2  g014(.a(x08), .b(x00), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n46_), .O(z01));
  inv1   g016(.a(x08), .O(new_n61_));
  nor2   g017(.a(new_n45_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(x01), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n46_), .O(z02));
  nand2  g020(.a(new_n62_), .b(x02), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z03));
  nand2  g022(.a(new_n62_), .b(x03), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n46_), .O(z04));
  nand2  g024(.a(new_n62_), .b(x04), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n46_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n46_), .O(z06));
  nand2  g028(.a(new_n62_), .b(x06), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n46_), .O(z07));
  nand2  g030(.a(new_n62_), .b(x07), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n46_), .O(z08));
  nand4  g032(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n77_));
  nand4  g033(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n59_), .c(new_n46_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  nand3  g039(.a(x25), .b(x24), .c(x23), .O(new_n84_));
  oai21  g040(.a(new_n84_), .b(new_n83_), .c(x19), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(x26), .c(x20), .O(new_n86_));
  oai21  g042(.a(x20), .b(new_n82_), .c(new_n86_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n63_), .O(z10));
  nand3  g045(.a(new_n49_), .b(x21), .c(new_n47_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x26), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n82_), .O(new_n92_));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n55_), .c(x20), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x21), .O(new_n95_));
  inv1   g051(.a(x21), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x20), .c(x19), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n92_), .c(new_n65_), .O(z11));
  aoi21  g056(.a(x21), .b(x20), .c(new_n82_), .O(new_n101_));
  inv1   g057(.a(x26), .O(new_n102_));
  nand4  g058(.a(x25), .b(x24), .c(x23), .d(x14), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(x19), .c(new_n102_), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n101_), .c(x22), .O(new_n105_));
  nand3  g061(.a(new_n50_), .b(new_n52_), .c(x21), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n67_), .O(z12));
  nand3  g065(.a(x25), .b(x24), .c(x15), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(x19), .c(new_n102_), .O(new_n111_));
  nand3  g067(.a(x22), .b(x21), .c(x20), .O(new_n112_));
  and2   g068(.a(new_n112_), .b(x19), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x23), .O(new_n114_));
  nand4  g070(.a(new_n50_), .b(new_n53_), .c(x22), .d(x21), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n69_), .O(z13));
  nand3  g074(.a(new_n49_), .b(x24), .c(new_n47_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x26), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n82_), .O(new_n121_));
  nand2  g077(.a(x21), .b(x20), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  nand3  g081(.a(x21), .b(x20), .c(x19), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nor2   g083(.a(x24), .b(new_n53_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n127_), .c(x22), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n121_), .c(new_n71_), .O(z14));
  oai21  g088(.a(new_n82_), .b(x17), .c(x26), .O(new_n133_));
  nand3  g089(.a(x24), .b(x23), .c(x22), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n122_), .c(x19), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x25), .O(new_n137_));
  inv1   g093(.a(x25), .O(new_n138_));
  nand4  g094(.a(new_n127_), .b(new_n54_), .c(new_n138_), .d(x24), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n137_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n73_), .O(z15));
  nand2  g098(.a(x26), .b(new_n82_), .O(new_n143_));
  nand3  g099(.a(x22), .b(x21), .c(x19), .O(new_n144_));
  nand4  g100(.a(new_n102_), .b(x25), .c(x24), .d(x23), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(new_n143_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x20), .O(new_n147_));
  inv1   g103(.a(x18), .O(new_n148_));
  nand3  g104(.a(x21), .b(x20), .c(new_n148_), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x26), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n147_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n75_), .O(z16));
endmodule


