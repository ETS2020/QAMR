// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x10), .O(new_n45_));
  inv1   g01(.a(x19), .O(new_n46_));
  inv1   g02(.a(x08), .O(new_n47_));
  and2   g03(.a(x21), .b(x20), .O(new_n48_));
  nand2  g04(.a(x23), .b(x22), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(new_n45_), .c(new_n46_), .O(z00));
  nor2   g09(.a(new_n46_), .b(new_n45_), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  nor2   g13(.a(new_n54_), .b(new_n47_), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z02));
  nand2  g16(.a(new_n58_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(new_n58_), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n55_), .O(z04));
  aoi21  g20(.a(x08), .b(x04), .c(new_n54_), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z05));
  nand2  g22(.a(new_n58_), .b(x05), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g24(.a(new_n58_), .b(x06), .O(new_n69_));
  nand2  g25(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g26(.a(new_n58_), .b(x07), .O(new_n71_));
  nand2  g27(.a(new_n71_), .b(new_n55_), .O(z08));
  nand4  g28(.a(new_n51_), .b(new_n48_), .c(x19), .d(x11), .O(new_n73_));
  oai21  g29(.a(x19), .b(x10), .c(new_n73_), .O(new_n74_));
  nand3  g30(.a(new_n74_), .b(x09), .c(new_n47_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n56_), .c(new_n55_), .O(z09));
  nand3  g32(.a(x22), .b(x21), .c(x12), .O(new_n77_));
  nand4  g33(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g34(.a(new_n78_), .b(new_n77_), .c(x20), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(x19), .O(new_n80_));
  nand3  g36(.a(x20), .b(new_n46_), .c(new_n45_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand3  g38(.a(new_n82_), .b(x09), .c(new_n47_), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(new_n59_), .c(new_n55_), .O(z10));
  nand3  g40(.a(x23), .b(x22), .c(x13), .O(new_n85_));
  oai21  g41(.a(new_n85_), .b(new_n50_), .c(x20), .O(new_n86_));
  oai21  g42(.a(new_n86_), .b(new_n46_), .c(x21), .O(new_n87_));
  inv1   g43(.a(x21), .O(new_n88_));
  nand3  g44(.a(new_n88_), .b(x20), .c(x19), .O(new_n89_));
  nand2  g45(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n91_));
  nand2  g47(.a(new_n91_), .b(new_n61_), .O(z11));
  nand2  g48(.a(x20), .b(x19), .O(new_n93_));
  nand2  g49(.a(x23), .b(x14), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n50_), .c(x21), .O(new_n95_));
  oai21  g51(.a(new_n95_), .b(new_n93_), .c(x22), .O(new_n96_));
  inv1   g52(.a(x22), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(x21), .c(x20), .d(x19), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand4  g55(.a(new_n99_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(new_n63_), .O(z12));
  nor2   g57(.a(x23), .b(new_n97_), .O(new_n102_));
  nand2  g58(.a(x23), .b(x15), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(new_n50_), .O(new_n104_));
  aoi21  g60(.a(new_n102_), .b(new_n48_), .c(new_n104_), .O(new_n105_));
  nand4  g61(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n106_));
  nand3  g62(.a(new_n106_), .b(x23), .c(new_n45_), .O(new_n107_));
  oai21  g63(.a(new_n105_), .b(new_n46_), .c(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(x09), .c(new_n47_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n65_), .O(z13));
  nand3  g66(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(x23), .c(x22), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n111_), .c(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  inv1   g71(.a(new_n111_), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n115_), .c(x23), .d(x22), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand4  g74(.a(new_n118_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n67_), .O(z14));
  nand2  g76(.a(x26), .b(x17), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(x24), .c(x23), .d(x22), .O(new_n122_));
  oai21  g78(.a(new_n122_), .b(new_n111_), .c(x25), .O(new_n123_));
  nor2   g79(.a(x25), .b(new_n115_), .O(new_n124_));
  nand4  g80(.a(new_n124_), .b(new_n116_), .c(x23), .d(x22), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n69_), .O(z15));
  inv1   g84(.a(x18), .O(new_n129_));
  nand4  g85(.a(x21), .b(x20), .c(x19), .d(new_n129_), .O(new_n130_));
  nand4  g86(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n130_), .c(x26), .O(new_n132_));
  inv1   g88(.a(new_n106_), .O(new_n133_));
  inv1   g89(.a(x25), .O(new_n134_));
  nor2   g90(.a(x26), .b(new_n134_), .O(new_n135_));
  nand4  g91(.a(new_n135_), .b(new_n133_), .c(x24), .d(x23), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand4  g93(.a(new_n137_), .b(new_n45_), .c(x09), .d(new_n47_), .O(new_n138_));
  nand2  g94(.a(new_n138_), .b(new_n71_), .O(z16));
endmodule


