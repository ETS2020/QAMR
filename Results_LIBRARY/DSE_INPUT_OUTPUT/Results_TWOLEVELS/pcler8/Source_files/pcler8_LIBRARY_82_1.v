// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x10), .O(new_n45_));
  inv1   g001(.a(x20), .O(new_n46_));
  inv1   g002(.a(x14), .O(new_n47_));
  nand2  g003(.a(x16), .b(new_n47_), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x26), .c(x25), .d(x24), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x23), .c(x22), .d(x21), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x19), .c(new_n45_), .d(x09), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(x08), .O(z00));
  nand2  g010(.a(x08), .b(x00), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n48_), .O(z01));
  nand2  g012(.a(x08), .b(x01), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n48_), .O(z02));
  inv1   g014(.a(x08), .O(new_n59_));
  inv1   g015(.a(new_n48_), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(x02), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  oai21  g020(.a(new_n59_), .b(new_n64_), .c(new_n48_), .O(z04));
  nand2  g021(.a(new_n61_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n48_), .O(z06));
  nand2  g025(.a(new_n61_), .b(x06), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z07));
  and2   g027(.a(new_n61_), .b(x07), .O(z08));
  inv1   g028(.a(x21), .O(new_n73_));
  inv1   g029(.a(x22), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  inv1   g032(.a(x23), .O(new_n77_));
  inv1   g033(.a(x24), .O(new_n78_));
  nor2   g034(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  and2   g035(.a(x26), .b(x25), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n76_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n55_), .c(new_n48_), .O(z09));
  nand2  g040(.a(new_n75_), .b(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n81_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  nand2  g043(.a(new_n46_), .b(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n57_), .c(new_n48_), .O(z10));
  inv1   g047(.a(x19), .O(new_n92_));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  nand2  g049(.a(new_n80_), .b(x24), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n92_), .c(x21), .O(new_n96_));
  nand3  g052(.a(new_n73_), .b(x20), .c(x19), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n62_), .c(new_n48_), .O(z11));
  nand3  g056(.a(x21), .b(x20), .c(x19), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(new_n102_));
  nand2  g058(.a(x20), .b(x19), .O(new_n103_));
  nand2  g059(.a(new_n74_), .b(x21), .O(new_n104_));
  oai22  g060(.a(new_n104_), .b(new_n103_), .c(new_n102_), .d(new_n74_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n106_));
  oai21  g062(.a(new_n59_), .b(new_n64_), .c(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n48_), .O(new_n108_));
  nand3  g064(.a(new_n45_), .b(x09), .c(new_n59_), .O(new_n109_));
  nor3   g065(.a(new_n109_), .b(new_n103_), .c(new_n47_), .O(new_n110_));
  nor2   g066(.a(new_n77_), .b(new_n74_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x21), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n94_), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n108_), .O(z12));
  nand3  g071(.a(new_n80_), .b(x24), .c(x15), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x22), .c(x21), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n103_), .c(x23), .O(new_n118_));
  inv1   g074(.a(new_n103_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n77_), .c(x22), .d(x21), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n118_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n66_), .c(new_n48_), .O(z13));
  nand3  g079(.a(new_n80_), .b(x24), .c(x14), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n109_), .c(x14), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x16), .O(new_n126_));
  oai21  g082(.a(new_n112_), .b(new_n103_), .c(x24), .O(new_n127_));
  nand3  g083(.a(new_n78_), .b(x23), .c(x22), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n101_), .c(new_n127_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n126_), .c(new_n68_), .O(z14));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n111_), .c(x24), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n101_), .c(x25), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  nand4  g091(.a(new_n102_), .b(new_n111_), .c(new_n135_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n70_), .c(new_n48_), .O(z15));
  inv1   g095(.a(x18), .O(new_n140_));
  nand4  g096(.a(x21), .b(x20), .c(x19), .d(new_n140_), .O(new_n141_));
  nand3  g097(.a(new_n111_), .b(x25), .c(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(x26), .O(new_n143_));
  nor2   g099(.a(x26), .b(new_n135_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n119_), .c(new_n79_), .d(new_n75_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n45_), .c(x09), .d(new_n59_), .O(new_n147_));
  nand2  g103(.a(x08), .b(x07), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n147_), .c(new_n60_), .O(z16));
endmodule


