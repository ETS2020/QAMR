// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x12), .O(new_n45_));
  nand2  g001(.a(x13), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(x21), .O(new_n55_));
  nand3  g011(.a(x26), .b(x25), .c(x24), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g013(.a(new_n57_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(new_n46_), .O(z00));
  inv1   g015(.a(x00), .O(new_n60_));
  inv1   g016(.a(x08), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n60_), .c(new_n46_), .O(z01));
  inv1   g018(.a(x01), .O(new_n63_));
  oai21  g019(.a(new_n61_), .b(new_n63_), .c(new_n46_), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  oai21  g021(.a(new_n61_), .b(new_n65_), .c(new_n46_), .O(z03));
  inv1   g022(.a(x03), .O(new_n67_));
  inv1   g023(.a(new_n46_), .O(new_n68_));
  nor3   g024(.a(new_n68_), .b(new_n61_), .c(new_n67_), .O(z04));
  inv1   g025(.a(x04), .O(new_n70_));
  oai21  g026(.a(new_n61_), .b(new_n70_), .c(new_n46_), .O(z05));
  inv1   g027(.a(x05), .O(new_n72_));
  oai21  g028(.a(new_n61_), .b(new_n72_), .c(new_n46_), .O(z06));
  inv1   g029(.a(x06), .O(new_n74_));
  oai21  g030(.a(new_n61_), .b(new_n74_), .c(new_n46_), .O(z07));
  inv1   g031(.a(x07), .O(new_n76_));
  oai21  g032(.a(new_n61_), .b(new_n76_), .c(new_n46_), .O(z08));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n52_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  inv1   g036(.a(x24), .O(new_n81_));
  nor2   g037(.a(new_n81_), .b(new_n53_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x26), .c(x25), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n80_), .c(x19), .O(new_n84_));
  nand4  g040(.a(new_n84_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n85_));
  nand2  g041(.a(x08), .b(x00), .O(new_n86_));
  aoi21  g042(.a(new_n86_), .b(new_n85_), .c(new_n68_), .O(z09));
  xor2a  g043(.a(x20), .b(x19), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n89_));
  oai21  g045(.a(new_n61_), .b(new_n63_), .c(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  nand2  g047(.a(x20), .b(x12), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(x10), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n57_), .c(new_n49_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n91_), .O(z10));
  nand3  g051(.a(new_n78_), .b(x20), .c(x19), .O(new_n96_));
  oai21  g052(.a(new_n51_), .b(new_n78_), .c(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n98_));
  nand2  g054(.a(x08), .b(x02), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n46_), .O(new_n101_));
  nand2  g057(.a(x13), .b(x12), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(x10), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n57_), .c(new_n49_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n101_), .O(z11));
  nand2  g061(.a(x23), .b(x14), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n56_), .c(x21), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n50_), .c(x22), .O(new_n108_));
  nand3  g064(.a(new_n51_), .b(new_n52_), .c(x21), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n111_));
  nand2  g067(.a(x08), .b(x03), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n111_), .c(new_n68_), .O(z12));
  nand4  g069(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x22), .c(x21), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n50_), .c(x23), .O(new_n116_));
  nand4  g072(.a(new_n51_), .b(new_n53_), .c(x22), .d(x21), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n119_));
  nand2  g075(.a(x08), .b(x04), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n68_), .O(z13));
  nand3  g077(.a(x21), .b(x20), .c(x19), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  inv1   g081(.a(new_n122_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n81_), .c(x23), .d(x22), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n129_));
  nand2  g085(.a(x08), .b(x05), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n129_), .c(new_n68_), .O(z14));
  nand2  g087(.a(x26), .b(x17), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x24), .c(x23), .d(x22), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n122_), .c(x25), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  nand4  g091(.a(new_n126_), .b(new_n54_), .c(new_n135_), .d(x24), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n138_));
  nand2  g094(.a(x08), .b(x06), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n138_), .c(new_n68_), .O(z15));
  inv1   g096(.a(x18), .O(new_n141_));
  nand4  g097(.a(x21), .b(x20), .c(x19), .d(new_n141_), .O(new_n142_));
  nand4  g098(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(x26), .O(new_n144_));
  nor2   g100(.a(x26), .b(new_n135_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n82_), .c(new_n79_), .d(new_n51_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n148_));
  nand2  g104(.a(x08), .b(x07), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n148_), .c(new_n68_), .O(z16));
endmodule


