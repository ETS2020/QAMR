// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  nor2   g000(.a(x23), .b(x20), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  and2   g005(.a(x20), .b(x19), .O(new_n50_));
  nand3  g006(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(new_n51_));
  inv1   g007(.a(x22), .O(new_n52_));
  inv1   g008(.a(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand3  g012(.a(new_n56_), .b(new_n54_), .c(x21), .O(new_n57_));
  oai21  g013(.a(new_n57_), .b(new_n51_), .c(new_n46_), .O(z00));
  nand2  g014(.a(x08), .b(x00), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n46_), .O(z01));
  inv1   g016(.a(x08), .O(new_n61_));
  nor2   g017(.a(new_n45_), .b(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  inv1   g021(.a(new_n62_), .O(new_n66_));
  nor2   g022(.a(new_n66_), .b(new_n65_), .O(z03));
  nand2  g023(.a(new_n62_), .b(x03), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z04));
  nand2  g025(.a(new_n62_), .b(x04), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z05));
  nand2  g027(.a(x08), .b(x05), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n46_), .O(z06));
  nand2  g029(.a(new_n62_), .b(x06), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(z07));
  nand2  g031(.a(x08), .b(x07), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n46_), .O(z08));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n52_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  nand4  g036(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n81_));
  oai22  g037(.a(new_n81_), .b(new_n80_), .c(new_n45_), .d(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n59_), .c(new_n46_), .O(z09));
  nand3  g040(.a(x22), .b(x21), .c(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n81_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  inv1   g043(.a(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n63_), .c(new_n46_), .O(z10));
  xor2a  g048(.a(x21), .b(x19), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x20), .O(new_n94_));
  nand2  g050(.a(x22), .b(x13), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n55_), .c(x20), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(x23), .c(x21), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n99_));
  oai21  g055(.a(new_n66_), .b(new_n65_), .c(new_n99_), .O(z11));
  aoi21  g056(.a(x21), .b(x19), .c(new_n88_), .O(new_n101_));
  nand4  g057(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(x20), .c(new_n53_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n101_), .c(x22), .O(new_n104_));
  nand3  g060(.a(new_n50_), .b(new_n52_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n68_), .O(z12));
  nand2  g064(.a(x23), .b(new_n78_), .O(new_n109_));
  nand4  g065(.a(new_n53_), .b(x22), .c(x21), .d(x20), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x19), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(x22), .c(x20), .d(x19), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x23), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n112_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n70_), .O(z13));
  nand3  g074(.a(new_n49_), .b(x24), .c(new_n47_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n88_), .O(new_n121_));
  nand2  g077(.a(x21), .b(x19), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  inv1   g081(.a(x24), .O(new_n126_));
  nand3  g082(.a(x21), .b(x20), .c(x19), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n126_), .c(x23), .d(x22), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n121_), .c(new_n72_), .O(z14));
  nand3  g088(.a(new_n49_), .b(x25), .c(new_n47_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x23), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n88_), .O(new_n135_));
  nand3  g091(.a(x22), .b(x21), .c(x19), .O(new_n136_));
  nand2  g092(.a(x26), .b(x17), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x24), .c(x23), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x25), .O(new_n139_));
  inv1   g095(.a(x25), .O(new_n140_));
  nand4  g096(.a(new_n128_), .b(new_n54_), .c(new_n140_), .d(x24), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n135_), .c(new_n74_), .O(z15));
  nand3  g100(.a(new_n49_), .b(x26), .c(new_n47_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x23), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n88_), .O(new_n147_));
  inv1   g103(.a(x18), .O(new_n148_));
  nand3  g104(.a(x21), .b(x19), .c(new_n148_), .O(new_n149_));
  nand4  g105(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n149_), .c(x26), .O(new_n151_));
  nor2   g107(.a(new_n126_), .b(new_n53_), .O(new_n152_));
  nor2   g108(.a(x26), .b(new_n140_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n152_), .c(new_n79_), .d(new_n50_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n151_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n47_), .c(x09), .d(new_n61_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n147_), .c(new_n76_), .O(z16));
endmodule


