// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x19), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  nor2   g014(.a(new_n55_), .b(new_n46_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n56_), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g021(.a(x08), .b(x04), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n56_), .O(z05));
  nand2  g023(.a(new_n59_), .b(x05), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g027(.a(new_n59_), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n56_), .O(z08));
  nand2  g029(.a(x21), .b(x20), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(x19), .c(x11), .O(new_n76_));
  nand2  g032(.a(x23), .b(x22), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(new_n78_));
  nand3  g034(.a(x26), .b(x25), .c(x24), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n78_), .O(new_n81_));
  oai22  g037(.a(new_n81_), .b(new_n76_), .c(x19), .d(x10), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x09), .c(new_n46_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(new_n57_), .c(new_n56_), .O(z09));
  inv1   g040(.a(x20), .O(new_n85_));
  nand3  g041(.a(x22), .b(x21), .c(x12), .O(new_n86_));
  nand4  g042(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n86_), .c(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  oai21  g045(.a(new_n85_), .b(x19), .c(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n60_), .O(z10));
  nand3  g048(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n79_), .c(x20), .O(new_n94_));
  nor2   g050(.a(x21), .b(new_n85_), .O(new_n95_));
  aoi21  g051(.a(new_n94_), .b(x21), .c(new_n95_), .O(new_n96_));
  nand3  g052(.a(x21), .b(new_n54_), .c(new_n47_), .O(new_n97_));
  oai21  g053(.a(new_n96_), .b(new_n54_), .c(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(x09), .c(new_n46_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n62_), .c(new_n56_), .O(z11));
  inv1   g056(.a(x22), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(x21), .c(x20), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  inv1   g059(.a(x14), .O(new_n104_));
  nor3   g060(.a(new_n79_), .b(new_n77_), .c(new_n104_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n103_), .c(x19), .O(new_n106_));
  nand3  g062(.a(x21), .b(x20), .c(x19), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x22), .c(new_n47_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x09), .c(new_n46_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(new_n64_), .c(new_n56_), .O(z12));
  nor2   g067(.a(x23), .b(new_n101_), .O(new_n112_));
  and2   g068(.a(x23), .b(x15), .O(new_n113_));
  aoi22  g069(.a(new_n113_), .b(new_n80_), .c(new_n112_), .d(new_n75_), .O(new_n114_));
  nand4  g070(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x23), .c(new_n47_), .O(new_n116_));
  oai21  g072(.a(new_n114_), .b(new_n54_), .c(new_n116_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(x09), .c(new_n46_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n66_), .c(new_n56_), .O(z13));
  nand2  g075(.a(x26), .b(x25), .O(new_n120_));
  inv1   g076(.a(x24), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  nand2  g078(.a(x24), .b(x16), .O(new_n123_));
  oai22  g079(.a(new_n123_), .b(new_n120_), .c(new_n122_), .d(new_n74_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x19), .O(new_n125_));
  nand4  g081(.a(new_n78_), .b(x21), .c(x20), .d(x19), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x24), .c(new_n47_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x09), .c(new_n46_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n68_), .c(new_n56_), .O(z14));
  inv1   g086(.a(x17), .O(new_n131_));
  nand3  g087(.a(x22), .b(x21), .c(x20), .O(new_n132_));
  inv1   g088(.a(x25), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x24), .c(x23), .O(new_n134_));
  oai22  g090(.a(new_n134_), .b(new_n132_), .c(new_n120_), .d(new_n131_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x19), .O(new_n136_));
  and2   g092(.a(x24), .b(x23), .O(new_n137_));
  inv1   g093(.a(new_n107_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n137_), .c(x22), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x25), .c(new_n47_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x09), .c(new_n46_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n70_), .c(new_n56_), .O(z15));
  inv1   g099(.a(x18), .O(new_n144_));
  nand3  g100(.a(new_n75_), .b(x19), .c(new_n144_), .O(new_n145_));
  nand3  g101(.a(new_n78_), .b(x25), .c(x24), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n145_), .c(x26), .O(new_n147_));
  inv1   g103(.a(new_n115_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n137_), .c(new_n45_), .d(x25), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n72_), .O(z16));
endmodule


