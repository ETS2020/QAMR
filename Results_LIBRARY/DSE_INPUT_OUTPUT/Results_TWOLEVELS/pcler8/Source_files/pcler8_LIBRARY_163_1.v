// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_,
    new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x10), .O(new_n45_));
  nand2  g01(.a(x23), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(z00));
  inv1   g03(.a(x00), .O(new_n48_));
  nand2  g04(.a(new_n46_), .b(x08), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(z01));
  inv1   g06(.a(x01), .O(new_n51_));
  inv1   g07(.a(x08), .O(new_n52_));
  oai21  g08(.a(new_n52_), .b(new_n51_), .c(new_n46_), .O(z02));
  nand2  g09(.a(x08), .b(x02), .O(new_n54_));
  nand2  g10(.a(new_n54_), .b(new_n46_), .O(z03));
  nand3  g11(.a(new_n46_), .b(x08), .c(x03), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z04));
  inv1   g13(.a(x04), .O(new_n58_));
  oai21  g14(.a(new_n52_), .b(new_n58_), .c(new_n46_), .O(z05));
  inv1   g15(.a(x05), .O(new_n60_));
  oai21  g16(.a(new_n52_), .b(new_n60_), .c(new_n46_), .O(z06));
  inv1   g17(.a(x06), .O(new_n62_));
  nor2   g18(.a(new_n49_), .b(new_n62_), .O(z07));
  inv1   g19(.a(x07), .O(new_n64_));
  nor2   g20(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g21(.a(x23), .O(new_n66_));
  inv1   g22(.a(x19), .O(new_n67_));
  nand3  g23(.a(new_n67_), .b(x09), .c(new_n52_), .O(new_n68_));
  aoi21  g24(.a(new_n68_), .b(new_n66_), .c(x10), .O(new_n69_));
  or2    g25(.a(new_n69_), .b(z01), .O(z09));
  xor2a  g26(.a(x20), .b(x19), .O(new_n71_));
  nand4  g27(.a(new_n71_), .b(new_n66_), .c(new_n45_), .d(x09), .O(new_n72_));
  oai22  g28(.a(new_n72_), .b(x08), .c(new_n49_), .d(new_n51_), .O(z10));
  inv1   g29(.a(x09), .O(new_n74_));
  nand2  g30(.a(x20), .b(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(x21), .O(new_n76_));
  inv1   g32(.a(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x19), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n76_), .c(new_n74_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n52_), .c(x23), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(x10), .c(new_n54_), .O(z11));
  nand3  g37(.a(x21), .b(x20), .c(x19), .O(new_n82_));
  nand2  g38(.a(new_n82_), .b(x22), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  nand4  g40(.a(new_n84_), .b(x21), .c(x20), .d(x19), .O(new_n85_));
  aoi21  g41(.a(new_n85_), .b(new_n83_), .c(x23), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n45_), .c(x09), .d(new_n52_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n56_), .O(z12));
  nor2   g44(.a(new_n74_), .b(x08), .O(new_n89_));
  nand3  g45(.a(new_n89_), .b(x19), .c(new_n45_), .O(new_n90_));
  nand4  g46(.a(new_n66_), .b(x22), .c(x21), .d(x20), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n90_), .c(new_n49_), .d(new_n58_), .O(z13));
  nand4  g48(.a(new_n89_), .b(x24), .c(new_n66_), .d(new_n45_), .O(new_n93_));
  oai21  g49(.a(new_n49_), .b(new_n60_), .c(new_n93_), .O(z14));
  nand4  g50(.a(new_n89_), .b(x25), .c(new_n66_), .d(new_n45_), .O(new_n95_));
  oai21  g51(.a(new_n49_), .b(new_n62_), .c(new_n95_), .O(z15));
  nand2  g52(.a(x26), .b(x09), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(x08), .c(new_n66_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  oai21  g55(.a(new_n49_), .b(new_n64_), .c(new_n99_), .O(z16));
endmodule


