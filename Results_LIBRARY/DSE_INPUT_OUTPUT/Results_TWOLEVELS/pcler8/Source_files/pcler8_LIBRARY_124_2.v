// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g00(.a(x04), .O(new_n45_));
  inv1   g01(.a(x09), .O(new_n46_));
  nor2   g02(.a(new_n46_), .b(x08), .O(new_n47_));
  nand3  g03(.a(x21), .b(x20), .c(x19), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  inv1   g05(.a(x22), .O(new_n50_));
  inv1   g06(.a(x23), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand4  g10(.a(new_n54_), .b(new_n52_), .c(new_n49_), .d(new_n47_), .O(new_n55_));
  aoi21  g11(.a(new_n55_), .b(new_n45_), .c(x10), .O(z00));
  inv1   g12(.a(x00), .O(new_n57_));
  inv1   g13(.a(x08), .O(new_n58_));
  inv1   g14(.a(x10), .O(new_n59_));
  aoi21  g15(.a(new_n59_), .b(x04), .c(new_n58_), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(new_n61_));
  nor2   g17(.a(new_n61_), .b(new_n57_), .O(z01));
  inv1   g18(.a(x01), .O(new_n63_));
  nor2   g19(.a(new_n61_), .b(new_n63_), .O(z02));
  inv1   g20(.a(x02), .O(new_n65_));
  nor2   g21(.a(new_n61_), .b(new_n65_), .O(z03));
  inv1   g22(.a(x03), .O(new_n67_));
  nor2   g23(.a(new_n61_), .b(new_n67_), .O(z04));
  nand3  g24(.a(x10), .b(x08), .c(x04), .O(new_n69_));
  inv1   g25(.a(new_n69_), .O(z05));
  nand2  g26(.a(new_n60_), .b(x05), .O(new_n71_));
  inv1   g27(.a(new_n71_), .O(z06));
  nand2  g28(.a(new_n60_), .b(x06), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(z07));
  nand2  g30(.a(new_n60_), .b(x07), .O(new_n75_));
  inv1   g31(.a(new_n75_), .O(z08));
  inv1   g32(.a(x21), .O(new_n77_));
  nor2   g33(.a(new_n50_), .b(new_n77_), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x20), .c(x11), .O(new_n79_));
  nand4  g35(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(new_n79_), .c(x19), .O(new_n81_));
  nand4  g37(.a(new_n81_), .b(new_n59_), .c(x09), .d(new_n58_), .O(new_n82_));
  oai22  g38(.a(new_n82_), .b(x04), .c(new_n61_), .d(new_n57_), .O(z09));
  nand3  g39(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  oai21  g40(.a(new_n84_), .b(new_n80_), .c(x19), .O(new_n85_));
  nand2  g41(.a(new_n85_), .b(x20), .O(new_n86_));
  inv1   g42(.a(x20), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(x19), .O(new_n88_));
  aoi21  g44(.a(new_n88_), .b(new_n86_), .c(x10), .O(new_n89_));
  nand4  g45(.a(new_n89_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n90_));
  oai21  g46(.a(new_n61_), .b(new_n63_), .c(new_n90_), .O(z10));
  inv1   g47(.a(x19), .O(new_n92_));
  nand3  g48(.a(x23), .b(x22), .c(x13), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n53_), .c(x20), .O(new_n94_));
  oai21  g50(.a(new_n94_), .b(new_n92_), .c(x21), .O(new_n95_));
  nand3  g51(.a(new_n77_), .b(x20), .c(x19), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n95_), .c(x10), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n98_));
  oai21  g54(.a(new_n61_), .b(new_n65_), .c(new_n98_), .O(z11));
  nand2  g55(.a(x20), .b(x19), .O(new_n100_));
  nand2  g56(.a(x23), .b(x14), .O(new_n101_));
  oai21  g57(.a(new_n101_), .b(new_n53_), .c(x21), .O(new_n102_));
  oai21  g58(.a(new_n102_), .b(new_n100_), .c(x22), .O(new_n103_));
  inv1   g59(.a(new_n100_), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(new_n50_), .c(x21), .O(new_n105_));
  aoi21  g61(.a(new_n105_), .b(new_n103_), .c(x10), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n107_));
  oai21  g63(.a(new_n61_), .b(new_n67_), .c(new_n107_), .O(z12));
  nand4  g64(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n109_));
  nand3  g65(.a(new_n109_), .b(x22), .c(x21), .O(new_n110_));
  oai21  g66(.a(new_n110_), .b(new_n100_), .c(x23), .O(new_n111_));
  nand4  g67(.a(new_n104_), .b(new_n51_), .c(x22), .d(x21), .O(new_n112_));
  aoi21  g68(.a(new_n112_), .b(new_n111_), .c(x10), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n114_));
  nand3  g70(.a(x10), .b(x08), .c(x04), .O(new_n115_));
  nand2  g71(.a(new_n115_), .b(new_n114_), .O(z13));
  nand3  g72(.a(x22), .b(x21), .c(x20), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n45_), .O(new_n118_));
  nand3  g74(.a(x26), .b(x25), .c(x16), .O(new_n119_));
  nand4  g75(.a(new_n119_), .b(new_n118_), .c(x23), .d(x19), .O(new_n120_));
  nand2  g76(.a(new_n120_), .b(x24), .O(new_n121_));
  inv1   g77(.a(x24), .O(new_n122_));
  nand4  g78(.a(new_n49_), .b(new_n122_), .c(x23), .d(x22), .O(new_n123_));
  aoi21  g79(.a(new_n123_), .b(new_n121_), .c(new_n46_), .O(new_n124_));
  aoi21  g80(.a(new_n124_), .b(new_n58_), .c(x04), .O(new_n125_));
  oai21  g81(.a(new_n125_), .b(x10), .c(new_n71_), .O(z14));
  nand2  g82(.a(x26), .b(x17), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(new_n52_), .c(x24), .O(new_n128_));
  oai21  g84(.a(new_n128_), .b(new_n48_), .c(x25), .O(new_n129_));
  inv1   g85(.a(x25), .O(new_n130_));
  nand4  g86(.a(new_n52_), .b(new_n49_), .c(new_n130_), .d(x24), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n129_), .c(x10), .O(new_n132_));
  nand4  g88(.a(new_n132_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n73_), .O(z15));
  inv1   g90(.a(x18), .O(new_n135_));
  nand4  g91(.a(x21), .b(x20), .c(x19), .d(new_n135_), .O(new_n136_));
  nand3  g92(.a(new_n52_), .b(x25), .c(x24), .O(new_n137_));
  oai21  g93(.a(new_n137_), .b(new_n136_), .c(x26), .O(new_n138_));
  nor2   g94(.a(new_n122_), .b(new_n51_), .O(new_n139_));
  nor2   g95(.a(x26), .b(new_n130_), .O(new_n140_));
  nand4  g96(.a(new_n140_), .b(new_n104_), .c(new_n139_), .d(new_n78_), .O(new_n141_));
  aoi21  g97(.a(new_n141_), .b(new_n138_), .c(x10), .O(new_n142_));
  nand4  g98(.a(new_n142_), .b(x09), .c(new_n58_), .d(new_n45_), .O(new_n143_));
  nand2  g99(.a(new_n143_), .b(new_n75_), .O(z16));
endmodule


