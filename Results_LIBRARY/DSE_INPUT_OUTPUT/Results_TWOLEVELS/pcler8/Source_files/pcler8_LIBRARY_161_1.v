// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_,
    new_n56_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_,
    new_n87_, new_n89_, new_n91_, new_n92_, new_n93_;
  inv1   g00(.a(x22), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x10), .O(z00));
  inv1   g02(.a(x00), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  inv1   g04(.a(z00), .O(new_n49_));
  oai21  g05(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(z01));
  inv1   g06(.a(x01), .O(new_n51_));
  nand2  g07(.a(new_n49_), .b(x08), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n51_), .O(z02));
  nand3  g09(.a(new_n49_), .b(x08), .c(x02), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(z03));
  inv1   g11(.a(x03), .O(new_n56_));
  nor2   g12(.a(new_n52_), .b(new_n56_), .O(z04));
  inv1   g13(.a(x04), .O(new_n58_));
  nor2   g14(.a(new_n52_), .b(new_n58_), .O(z05));
  inv1   g15(.a(x05), .O(new_n60_));
  oai21  g16(.a(new_n48_), .b(new_n60_), .c(new_n49_), .O(z06));
  inv1   g17(.a(x06), .O(new_n62_));
  oai21  g18(.a(new_n48_), .b(new_n62_), .c(new_n49_), .O(z07));
  inv1   g19(.a(x07), .O(new_n64_));
  nor2   g20(.a(new_n52_), .b(new_n64_), .O(z08));
  inv1   g21(.a(x09), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(x08), .O(new_n67_));
  nor3   g23(.a(x22), .b(x19), .c(x10), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g25(.a(new_n52_), .b(new_n47_), .c(new_n69_), .O(z09));
  inv1   g26(.a(x10), .O(new_n71_));
  xor2a  g27(.a(x20), .b(x19), .O(new_n72_));
  nand4  g28(.a(new_n72_), .b(new_n45_), .c(new_n71_), .d(x09), .O(new_n73_));
  oai22  g29(.a(new_n73_), .b(x08), .c(new_n52_), .d(new_n51_), .O(z10));
  nand2  g30(.a(x20), .b(x19), .O(new_n75_));
  nand2  g31(.a(new_n75_), .b(x21), .O(new_n76_));
  inv1   g32(.a(x21), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x20), .c(x19), .O(new_n78_));
  aoi21  g34(.a(new_n78_), .b(new_n76_), .c(new_n66_), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n48_), .c(x22), .O(new_n80_));
  oai21  g36(.a(new_n80_), .b(x10), .c(new_n54_), .O(z11));
  nand3  g37(.a(new_n71_), .b(x09), .c(new_n48_), .O(new_n82_));
  nand4  g38(.a(new_n45_), .b(x21), .c(x20), .d(x19), .O(new_n83_));
  oai22  g39(.a(new_n83_), .b(new_n82_), .c(new_n52_), .d(new_n56_), .O(z12));
  nand4  g40(.a(new_n67_), .b(x23), .c(new_n45_), .d(new_n71_), .O(new_n85_));
  oai21  g41(.a(new_n52_), .b(new_n58_), .c(new_n85_), .O(z13));
  nand4  g42(.a(new_n67_), .b(x24), .c(new_n45_), .d(new_n71_), .O(new_n87_));
  oai21  g43(.a(new_n52_), .b(new_n60_), .c(new_n87_), .O(z14));
  nand4  g44(.a(new_n67_), .b(x25), .c(new_n45_), .d(new_n71_), .O(new_n89_));
  oai21  g45(.a(new_n52_), .b(new_n62_), .c(new_n89_), .O(z15));
  nand2  g46(.a(x26), .b(x09), .O(new_n91_));
  oai21  g47(.a(new_n91_), .b(x08), .c(new_n45_), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n71_), .O(new_n93_));
  oai21  g49(.a(new_n52_), .b(new_n64_), .c(new_n93_), .O(z16));
endmodule


