// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_;
  inv1   g00(.a(x08), .O(new_n45_));
  nand2  g01(.a(x20), .b(x19), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x21), .O(new_n48_));
  inv1   g04(.a(x22), .O(new_n49_));
  nand3  g05(.a(x26), .b(x25), .c(x24), .O(new_n50_));
  nor3   g06(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(new_n47_), .c(x09), .d(new_n45_), .O(new_n52_));
  aoi21  g08(.a(new_n52_), .b(x23), .c(x10), .O(z00));
  nor2   g09(.a(x23), .b(x10), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n45_), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(x00), .O(new_n56_));
  inv1   g12(.a(new_n56_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  inv1   g14(.a(new_n54_), .O(new_n59_));
  oai21  g15(.a(new_n45_), .b(new_n58_), .c(new_n59_), .O(z02));
  nand2  g16(.a(new_n55_), .b(x02), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(new_n55_), .b(x03), .O(new_n63_));
  nand2  g19(.a(new_n63_), .b(new_n59_), .O(z04));
  inv1   g20(.a(x04), .O(new_n65_));
  inv1   g21(.a(new_n55_), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(new_n65_), .O(z05));
  nand2  g23(.a(new_n55_), .b(x05), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n59_), .O(z06));
  inv1   g25(.a(x06), .O(new_n70_));
  oai21  g26(.a(new_n45_), .b(new_n70_), .c(new_n59_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  nor2   g28(.a(new_n66_), .b(new_n72_), .O(z08));
  inv1   g29(.a(x10), .O(new_n74_));
  inv1   g30(.a(x23), .O(new_n75_));
  nand3  g31(.a(x21), .b(x20), .c(x11), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n77_));
  oai22  g33(.a(new_n77_), .b(new_n76_), .c(new_n75_), .d(x19), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x09), .c(new_n45_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(x23), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n74_), .O(new_n81_));
  nand2  g37(.a(new_n81_), .b(new_n56_), .O(z09));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n50_), .c(x19), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x20), .O(new_n85_));
  inv1   g41(.a(x20), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(x19), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n85_), .c(new_n75_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n89_));
  oai21  g45(.a(new_n66_), .b(new_n58_), .c(new_n89_), .O(z10));
  inv1   g46(.a(x19), .O(new_n91_));
  nand2  g47(.a(x22), .b(x13), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n50_), .c(x20), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(x21), .O(new_n94_));
  nand3  g50(.a(new_n48_), .b(x20), .c(x19), .O(new_n95_));
  aoi21  g51(.a(new_n95_), .b(new_n94_), .c(new_n75_), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n97_));
  nand2  g53(.a(new_n97_), .b(new_n61_), .O(z11));
  nand4  g54(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n47_), .c(x21), .O(new_n100_));
  nand2  g56(.a(new_n100_), .b(x22), .O(new_n101_));
  nand3  g57(.a(new_n47_), .b(new_n49_), .c(x21), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n101_), .c(new_n75_), .O(new_n103_));
  nand4  g59(.a(new_n103_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(new_n63_), .O(z12));
  nor2   g61(.a(new_n49_), .b(new_n48_), .O(new_n106_));
  nand4  g62(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand4  g63(.a(new_n107_), .b(new_n106_), .c(x20), .d(x19), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(x23), .c(new_n74_), .d(x09), .O(new_n109_));
  oai22  g65(.a(new_n109_), .b(x08), .c(new_n66_), .d(new_n65_), .O(z13));
  nand3  g66(.a(x26), .b(x25), .c(x16), .O(new_n111_));
  nand3  g67(.a(new_n111_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g68(.a(new_n112_), .b(new_n46_), .c(x24), .O(new_n113_));
  inv1   g69(.a(x24), .O(new_n114_));
  nand4  g70(.a(new_n47_), .b(new_n114_), .c(x22), .d(x21), .O(new_n115_));
  aoi21  g71(.a(new_n115_), .b(new_n113_), .c(new_n75_), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n68_), .O(z14));
  nand3  g74(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nand2  g75(.a(x26), .b(x17), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(x24), .c(x22), .O(new_n121_));
  oai21  g77(.a(new_n121_), .b(new_n119_), .c(x25), .O(new_n122_));
  inv1   g78(.a(new_n119_), .O(new_n123_));
  nor2   g79(.a(x25), .b(new_n114_), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(new_n123_), .c(x22), .O(new_n125_));
  aoi21  g81(.a(new_n125_), .b(new_n122_), .c(new_n75_), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n127_));
  oai21  g83(.a(new_n66_), .b(new_n70_), .c(new_n127_), .O(z15));
  inv1   g84(.a(x18), .O(new_n129_));
  nand3  g85(.a(x20), .b(x19), .c(new_n129_), .O(new_n130_));
  nand4  g86(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n131_));
  oai21  g87(.a(new_n131_), .b(new_n130_), .c(x26), .O(new_n132_));
  inv1   g88(.a(x26), .O(new_n133_));
  nor2   g89(.a(new_n114_), .b(new_n49_), .O(new_n134_));
  nand4  g90(.a(new_n123_), .b(new_n134_), .c(new_n133_), .d(x25), .O(new_n135_));
  aoi21  g91(.a(new_n135_), .b(new_n132_), .c(new_n75_), .O(new_n136_));
  nand4  g92(.a(new_n136_), .b(new_n74_), .c(x09), .d(new_n45_), .O(new_n137_));
  oai21  g93(.a(new_n66_), .b(new_n72_), .c(new_n137_), .O(z16));
endmodule


