// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x12), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  nor2   g010(.a(new_n46_), .b(x12), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n47_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(z01));
  nand2  g014(.a(new_n56_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  inv1   g016(.a(x02), .O(new_n61_));
  nor3   g017(.a(new_n55_), .b(new_n47_), .c(new_n61_), .O(z03));
  inv1   g018(.a(new_n55_), .O(new_n63_));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n63_), .O(z04));
  nand2  g021(.a(new_n56_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(new_n56_), .b(x05), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z06));
  nand2  g025(.a(new_n56_), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n63_), .O(z07));
  nand2  g027(.a(new_n56_), .b(x07), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  nand4  g029(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n74_));
  nand4  g030(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n74_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n57_), .c(new_n63_), .O(z09));
  inv1   g034(.a(x20), .O(new_n79_));
  nand4  g035(.a(x22), .b(x21), .c(x20), .d(x12), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n75_), .c(x20), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x19), .O(new_n82_));
  oai21  g038(.a(new_n79_), .b(x19), .c(new_n82_), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(new_n59_), .c(new_n63_), .O(z10));
  nand3  g041(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n86_));
  inv1   g042(.a(x21), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(x20), .c(x19), .O(new_n88_));
  oai22  g044(.a(new_n88_), .b(new_n86_), .c(new_n47_), .d(new_n61_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n63_), .O(new_n90_));
  nand2  g046(.a(x20), .b(x19), .O(new_n91_));
  oai21  g047(.a(new_n46_), .b(x12), .c(new_n91_), .O(new_n92_));
  inv1   g048(.a(new_n75_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(x22), .c(x13), .d(x12), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(new_n92_), .c(new_n87_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n90_), .O(z11));
  nand3  g053(.a(new_n91_), .b(x21), .c(x12), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  nand3  g056(.a(x26), .b(x25), .c(x24), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n100_), .c(x21), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n99_), .c(x22), .O(new_n103_));
  inv1   g059(.a(new_n91_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n46_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n64_), .c(new_n63_), .O(z12));
  inv1   g064(.a(x23), .O(new_n109_));
  nand3  g065(.a(new_n104_), .b(new_n109_), .c(x21), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n86_), .c(x12), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x22), .O(new_n112_));
  nand4  g068(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n104_), .c(x22), .d(x21), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x23), .c(new_n48_), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x09), .c(new_n47_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n112_), .c(new_n66_), .O(z13));
  inv1   g074(.a(x09), .O(new_n119_));
  nor2   g075(.a(new_n119_), .b(x08), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x19), .c(new_n48_), .O(new_n121_));
  inv1   g077(.a(x24), .O(new_n122_));
  nor2   g078(.a(new_n87_), .b(new_n79_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n122_), .c(x23), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n121_), .c(x12), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(x22), .O(new_n126_));
  nand3  g082(.a(x21), .b(x20), .c(x19), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(x26), .b(x25), .c(x16), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x23), .c(x22), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n128_), .c(new_n122_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n126_), .c(new_n68_), .O(z14));
  nand2  g090(.a(x26), .b(x17), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x24), .c(x23), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n127_), .c(x25), .O(new_n137_));
  nor2   g093(.a(new_n109_), .b(new_n46_), .O(new_n138_));
  nor2   g094(.a(x25), .b(new_n122_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n138_), .c(new_n128_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n137_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x12), .O(new_n142_));
  nand2  g098(.a(x25), .b(new_n46_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n70_), .O(z15));
  nand3  g102(.a(new_n104_), .b(new_n120_), .c(new_n48_), .O(new_n147_));
  nor2   g103(.a(new_n109_), .b(new_n87_), .O(new_n148_));
  nand4  g104(.a(new_n148_), .b(new_n45_), .c(x25), .d(x24), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n147_), .c(x12), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x22), .O(new_n151_));
  inv1   g107(.a(x18), .O(new_n152_));
  nand3  g108(.a(new_n123_), .b(x19), .c(new_n152_), .O(new_n153_));
  nand3  g109(.a(new_n138_), .b(x25), .c(x24), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n45_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n151_), .c(new_n72_), .O(z16));
endmodule


