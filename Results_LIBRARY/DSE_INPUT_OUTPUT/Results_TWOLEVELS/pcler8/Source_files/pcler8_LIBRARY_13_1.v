// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n59_, new_n61_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n98_;
  inv1   g00(.a(x00), .O(new_n46_));
  inv1   g01(.a(x10), .O(new_n47_));
  nand2  g02(.a(x22), .b(new_n47_), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(x08), .O(new_n49_));
  nor2   g04(.a(new_n49_), .b(new_n46_), .O(z01));
  inv1   g05(.a(x01), .O(new_n51_));
  nor2   g06(.a(new_n49_), .b(new_n51_), .O(z02));
  nand3  g07(.a(new_n48_), .b(x08), .c(x02), .O(new_n53_));
  inv1   g08(.a(new_n53_), .O(z03));
  nand2  g09(.a(x08), .b(x03), .O(new_n55_));
  nand2  g10(.a(new_n55_), .b(new_n48_), .O(z04));
  inv1   g11(.a(x04), .O(new_n57_));
  nor2   g12(.a(new_n49_), .b(new_n57_), .O(z05));
  nand2  g13(.a(x08), .b(x05), .O(new_n59_));
  nand2  g14(.a(new_n59_), .b(new_n48_), .O(z06));
  nand2  g15(.a(x08), .b(x06), .O(new_n61_));
  nand2  g16(.a(new_n61_), .b(new_n48_), .O(z07));
  inv1   g17(.a(x07), .O(new_n63_));
  nor2   g18(.a(new_n49_), .b(new_n63_), .O(z08));
  inv1   g19(.a(x09), .O(new_n65_));
  nor2   g20(.a(new_n65_), .b(x08), .O(new_n66_));
  nor3   g21(.a(x22), .b(x19), .c(x10), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  oai21  g23(.a(new_n49_), .b(new_n46_), .c(new_n68_), .O(z09));
  inv1   g24(.a(x22), .O(new_n70_));
  inv1   g25(.a(x08), .O(new_n71_));
  xor2a  g26(.a(x20), .b(x19), .O(new_n72_));
  nand3  g27(.a(new_n72_), .b(x09), .c(new_n71_), .O(new_n73_));
  nand2  g28(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand2  g29(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  oai21  g30(.a(new_n49_), .b(new_n51_), .c(new_n75_), .O(z10));
  nand2  g31(.a(x20), .b(x19), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(x21), .O(new_n78_));
  inv1   g33(.a(x21), .O(new_n79_));
  nand3  g34(.a(new_n79_), .b(x20), .c(x19), .O(new_n80_));
  aoi21  g35(.a(new_n80_), .b(new_n78_), .c(new_n65_), .O(new_n81_));
  aoi21  g36(.a(new_n81_), .b(new_n71_), .c(x22), .O(new_n82_));
  oai21  g37(.a(new_n82_), .b(x10), .c(new_n53_), .O(z11));
  nand3  g38(.a(x21), .b(x20), .c(x19), .O(new_n84_));
  inv1   g39(.a(new_n84_), .O(new_n85_));
  aoi21  g40(.a(new_n85_), .b(new_n66_), .c(x22), .O(new_n86_));
  oai21  g41(.a(new_n86_), .b(x10), .c(new_n55_), .O(z12));
  nand4  g42(.a(new_n66_), .b(x23), .c(new_n70_), .d(new_n47_), .O(new_n88_));
  oai21  g43(.a(new_n49_), .b(new_n57_), .c(new_n88_), .O(z13));
  nand2  g44(.a(x24), .b(x09), .O(new_n90_));
  oai21  g45(.a(new_n90_), .b(x08), .c(new_n70_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n47_), .O(new_n92_));
  nand2  g47(.a(new_n92_), .b(new_n59_), .O(z14));
  nand2  g48(.a(x25), .b(x09), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(x08), .c(new_n70_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n47_), .O(new_n96_));
  nand2  g51(.a(new_n96_), .b(new_n61_), .O(z15));
  nand4  g52(.a(new_n66_), .b(x26), .c(new_n70_), .d(new_n47_), .O(new_n98_));
  oai21  g53(.a(new_n49_), .b(new_n63_), .c(new_n98_), .O(z16));
  zero   g54(.O(z00));
endmodule


