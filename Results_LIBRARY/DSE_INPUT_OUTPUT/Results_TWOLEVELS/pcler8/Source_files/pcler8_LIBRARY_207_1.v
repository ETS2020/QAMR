// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n53_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n97_;
  inv1   g00(.a(x10), .O(new_n45_));
  nand2  g01(.a(x23), .b(new_n45_), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(z00));
  inv1   g03(.a(x00), .O(new_n48_));
  nand2  g04(.a(new_n46_), .b(x08), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(new_n48_), .O(z01));
  inv1   g06(.a(x01), .O(new_n51_));
  nor2   g07(.a(new_n49_), .b(new_n51_), .O(z02));
  nand3  g08(.a(new_n46_), .b(x08), .c(x02), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(z03));
  nand2  g10(.a(x08), .b(x03), .O(new_n55_));
  nand2  g11(.a(new_n55_), .b(new_n46_), .O(z04));
  inv1   g12(.a(x04), .O(new_n57_));
  inv1   g13(.a(x08), .O(new_n58_));
  oai21  g14(.a(new_n58_), .b(new_n57_), .c(new_n46_), .O(z05));
  inv1   g15(.a(x05), .O(new_n60_));
  nor2   g16(.a(new_n49_), .b(new_n60_), .O(z06));
  inv1   g17(.a(x06), .O(new_n62_));
  nor2   g18(.a(new_n49_), .b(new_n62_), .O(z07));
  inv1   g19(.a(x07), .O(new_n64_));
  nor2   g20(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g21(.a(x09), .O(new_n66_));
  nor2   g22(.a(new_n66_), .b(x08), .O(new_n67_));
  nor3   g23(.a(x23), .b(x19), .c(x10), .O(new_n68_));
  nand2  g24(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  oai21  g25(.a(new_n49_), .b(new_n48_), .c(new_n69_), .O(z09));
  inv1   g26(.a(x23), .O(new_n71_));
  xor2a  g27(.a(x20), .b(x19), .O(new_n72_));
  nand3  g28(.a(new_n72_), .b(x09), .c(new_n58_), .O(new_n73_));
  aoi21  g29(.a(new_n73_), .b(new_n71_), .c(x10), .O(new_n74_));
  or2    g30(.a(new_n74_), .b(z02), .O(z10));
  nand2  g31(.a(x20), .b(x19), .O(new_n76_));
  nand2  g32(.a(new_n76_), .b(x21), .O(new_n77_));
  inv1   g33(.a(x21), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x20), .c(x19), .O(new_n79_));
  aoi21  g35(.a(new_n79_), .b(new_n77_), .c(new_n66_), .O(new_n80_));
  aoi21  g36(.a(new_n80_), .b(new_n58_), .c(x23), .O(new_n81_));
  oai21  g37(.a(new_n81_), .b(x10), .c(new_n53_), .O(z11));
  nand3  g38(.a(x21), .b(x20), .c(x19), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(x22), .O(new_n84_));
  inv1   g40(.a(x22), .O(new_n85_));
  nand4  g41(.a(new_n85_), .b(x21), .c(x20), .d(x19), .O(new_n86_));
  aoi21  g42(.a(new_n86_), .b(new_n84_), .c(new_n66_), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n58_), .c(x23), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(x10), .c(new_n55_), .O(z12));
  nand3  g45(.a(new_n67_), .b(x19), .c(new_n45_), .O(new_n90_));
  nand4  g46(.a(new_n71_), .b(x22), .c(x21), .d(x20), .O(new_n91_));
  oai22  g47(.a(new_n91_), .b(new_n90_), .c(new_n49_), .d(new_n57_), .O(z13));
  nand4  g48(.a(new_n67_), .b(x24), .c(new_n71_), .d(new_n45_), .O(new_n93_));
  oai21  g49(.a(new_n49_), .b(new_n60_), .c(new_n93_), .O(z14));
  nand4  g50(.a(new_n67_), .b(x25), .c(new_n71_), .d(new_n45_), .O(new_n95_));
  oai21  g51(.a(new_n49_), .b(new_n62_), .c(new_n95_), .O(z15));
  nand4  g52(.a(new_n67_), .b(x26), .c(new_n71_), .d(new_n45_), .O(new_n97_));
  oai21  g53(.a(new_n49_), .b(new_n64_), .c(new_n97_), .O(z16));
endmodule


