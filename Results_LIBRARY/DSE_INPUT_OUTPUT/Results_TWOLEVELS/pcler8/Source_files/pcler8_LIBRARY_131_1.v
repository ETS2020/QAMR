// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n46_, new_n47_, new_n48_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n57_, new_n58_, new_n60_, new_n62_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x10), .O(new_n46_));
  nand2  g01(.a(x21), .b(new_n46_), .O(new_n47_));
  nand2  g02(.a(x08), .b(x00), .O(new_n48_));
  nand2  g03(.a(new_n48_), .b(new_n47_), .O(z01));
  nand3  g04(.a(new_n47_), .b(x08), .c(x01), .O(new_n50_));
  inv1   g05(.a(new_n50_), .O(z02));
  inv1   g06(.a(x02), .O(new_n52_));
  inv1   g07(.a(x08), .O(new_n53_));
  oai21  g08(.a(new_n53_), .b(new_n52_), .c(new_n47_), .O(z03));
  nand3  g09(.a(new_n47_), .b(x08), .c(x03), .O(new_n55_));
  inv1   g10(.a(new_n55_), .O(z04));
  inv1   g11(.a(x04), .O(new_n57_));
  nand2  g12(.a(new_n47_), .b(x08), .O(new_n58_));
  nor2   g13(.a(new_n58_), .b(new_n57_), .O(z05));
  nand3  g14(.a(new_n47_), .b(x08), .c(x05), .O(new_n60_));
  inv1   g15(.a(new_n60_), .O(z06));
  nand3  g16(.a(new_n47_), .b(x08), .c(x06), .O(new_n62_));
  inv1   g17(.a(new_n62_), .O(z07));
  nand2  g18(.a(x08), .b(x07), .O(new_n64_));
  nand2  g19(.a(new_n64_), .b(new_n47_), .O(z08));
  inv1   g20(.a(x21), .O(new_n66_));
  inv1   g21(.a(x19), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(x09), .c(new_n53_), .O(new_n68_));
  nand2  g23(.a(new_n68_), .b(new_n66_), .O(new_n69_));
  nand2  g24(.a(new_n69_), .b(new_n46_), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n48_), .O(z09));
  xor2a  g26(.a(x20), .b(x19), .O(new_n72_));
  nand4  g27(.a(new_n72_), .b(new_n66_), .c(new_n46_), .d(x09), .O(new_n73_));
  oai21  g28(.a(new_n73_), .b(x08), .c(new_n50_), .O(z10));
  nand3  g29(.a(new_n46_), .b(x09), .c(new_n53_), .O(new_n75_));
  nand3  g30(.a(new_n66_), .b(x20), .c(x19), .O(new_n76_));
  oai22  g31(.a(new_n76_), .b(new_n75_), .c(new_n58_), .d(new_n52_), .O(z11));
  nand2  g32(.a(x22), .b(x09), .O(new_n78_));
  oai21  g33(.a(new_n78_), .b(x08), .c(new_n66_), .O(new_n79_));
  nand2  g34(.a(new_n79_), .b(new_n46_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n55_), .O(z12));
  nand2  g36(.a(x09), .b(new_n53_), .O(new_n82_));
  nand3  g37(.a(x23), .b(new_n66_), .c(new_n46_), .O(new_n83_));
  oai22  g38(.a(new_n83_), .b(new_n82_), .c(new_n58_), .d(new_n57_), .O(z13));
  nand2  g39(.a(x24), .b(x09), .O(new_n85_));
  oai21  g40(.a(new_n85_), .b(x08), .c(new_n66_), .O(new_n86_));
  nand2  g41(.a(new_n86_), .b(new_n46_), .O(new_n87_));
  nand2  g42(.a(new_n87_), .b(new_n60_), .O(z14));
  nand2  g43(.a(x25), .b(x09), .O(new_n89_));
  oai21  g44(.a(new_n89_), .b(x08), .c(new_n66_), .O(new_n90_));
  nand2  g45(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  nand2  g46(.a(new_n91_), .b(new_n62_), .O(z15));
  nand2  g47(.a(x26), .b(x09), .O(new_n93_));
  oai21  g48(.a(new_n93_), .b(x08), .c(new_n66_), .O(new_n94_));
  nand2  g49(.a(new_n94_), .b(new_n46_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(new_n64_), .O(z16));
  zero   g51(.O(z00));
endmodule


