// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x15), .O(new_n45_));
  nor2   g01(.a(new_n45_), .b(x11), .O(new_n46_));
  inv1   g02(.a(new_n46_), .O(new_n47_));
  inv1   g03(.a(x08), .O(new_n48_));
  nand2  g04(.a(x20), .b(x19), .O(new_n49_));
  nor2   g05(.a(new_n49_), .b(x10), .O(new_n50_));
  nand3  g06(.a(new_n50_), .b(x09), .c(new_n48_), .O(new_n51_));
  inv1   g07(.a(x22), .O(new_n52_));
  inv1   g08(.a(x23), .O(new_n53_));
  nor2   g09(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g10(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nand3  g12(.a(new_n56_), .b(new_n54_), .c(x21), .O(new_n57_));
  oai21  g13(.a(new_n57_), .b(new_n51_), .c(new_n47_), .O(z00));
  aoi21  g14(.a(x08), .b(x00), .c(new_n46_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(z01));
  aoi21  g16(.a(x08), .b(x01), .c(new_n46_), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z02));
  aoi21  g18(.a(x08), .b(x02), .c(new_n46_), .O(new_n63_));
  inv1   g19(.a(new_n63_), .O(z03));
  aoi21  g20(.a(x08), .b(x03), .c(new_n46_), .O(new_n65_));
  inv1   g21(.a(new_n65_), .O(z04));
  nand3  g22(.a(new_n47_), .b(x08), .c(x04), .O(new_n67_));
  inv1   g23(.a(new_n67_), .O(z05));
  inv1   g24(.a(x05), .O(new_n69_));
  oai21  g25(.a(new_n48_), .b(new_n69_), .c(new_n47_), .O(z06));
  inv1   g26(.a(x06), .O(new_n71_));
  oai21  g27(.a(new_n48_), .b(new_n71_), .c(new_n47_), .O(z07));
  inv1   g28(.a(x07), .O(new_n73_));
  oai21  g29(.a(new_n48_), .b(new_n73_), .c(new_n47_), .O(z08));
  inv1   g30(.a(x10), .O(new_n75_));
  nand4  g31(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n59_), .O(z09));
  inv1   g36(.a(x19), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n77_), .c(x19), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(x20), .O(new_n84_));
  oai21  g40(.a(x20), .b(new_n81_), .c(new_n84_), .O(new_n85_));
  nand4  g41(.a(new_n85_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n61_), .O(z10));
  nand3  g43(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g44(.a(new_n88_), .b(new_n55_), .c(x20), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n81_), .c(x21), .O(new_n90_));
  inv1   g46(.a(x21), .O(new_n91_));
  nand3  g47(.a(new_n91_), .b(x20), .c(x19), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n63_), .O(z11));
  nand2  g51(.a(x23), .b(x14), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n55_), .c(x21), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n49_), .c(x22), .O(new_n98_));
  nand4  g54(.a(new_n52_), .b(x21), .c(x20), .d(x19), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand4  g56(.a(new_n100_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n65_), .O(z12));
  nand3  g58(.a(new_n75_), .b(x09), .c(new_n48_), .O(new_n103_));
  oai21  g59(.a(new_n103_), .b(new_n77_), .c(x11), .O(new_n104_));
  nand2  g60(.a(new_n104_), .b(x15), .O(new_n105_));
  nand4  g61(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n106_));
  inv1   g62(.a(new_n106_), .O(new_n107_));
  nand3  g63(.a(new_n53_), .b(x22), .c(x21), .O(new_n108_));
  oai22  g64(.a(new_n108_), .b(new_n49_), .c(new_n107_), .d(new_n53_), .O(new_n109_));
  nand4  g65(.a(new_n109_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n110_));
  nand3  g66(.a(new_n110_), .b(new_n105_), .c(new_n67_), .O(z13));
  nand3  g67(.a(x21), .b(x20), .c(x19), .O(new_n112_));
  nand3  g68(.a(x26), .b(x25), .c(x16), .O(new_n113_));
  nand3  g69(.a(new_n113_), .b(x23), .c(x22), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n112_), .c(x24), .O(new_n115_));
  inv1   g71(.a(x24), .O(new_n116_));
  inv1   g72(.a(new_n112_), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(new_n116_), .c(x23), .d(x22), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand4  g75(.a(new_n119_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n120_));
  nand2  g76(.a(x08), .b(x05), .O(new_n121_));
  aoi21  g77(.a(new_n121_), .b(new_n120_), .c(new_n46_), .O(z14));
  nand2  g78(.a(x26), .b(x17), .O(new_n123_));
  nand4  g79(.a(new_n123_), .b(x24), .c(x23), .d(x22), .O(new_n124_));
  oai21  g80(.a(new_n124_), .b(new_n112_), .c(x25), .O(new_n125_));
  nor2   g81(.a(x25), .b(new_n116_), .O(new_n126_));
  nand3  g82(.a(new_n126_), .b(new_n117_), .c(new_n54_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n125_), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n129_));
  nand2  g85(.a(x08), .b(x06), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n129_), .c(new_n46_), .O(z15));
  inv1   g87(.a(x18), .O(new_n132_));
  nand4  g88(.a(x21), .b(x20), .c(x19), .d(new_n132_), .O(new_n133_));
  nand4  g89(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n134_));
  oai21  g90(.a(new_n134_), .b(new_n133_), .c(x26), .O(new_n135_));
  inv1   g91(.a(x26), .O(new_n136_));
  nor2   g92(.a(new_n116_), .b(new_n53_), .O(new_n137_));
  nand4  g93(.a(new_n107_), .b(new_n137_), .c(new_n136_), .d(x25), .O(new_n138_));
  nand2  g94(.a(new_n138_), .b(new_n135_), .O(new_n139_));
  nand4  g95(.a(new_n139_), .b(new_n75_), .c(x09), .d(new_n48_), .O(new_n140_));
  nand2  g96(.a(x08), .b(x07), .O(new_n141_));
  aoi21  g97(.a(new_n141_), .b(new_n140_), .c(new_n46_), .O(z16));
endmodule


