// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x25), .b(x10), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nor2   g011(.a(new_n54_), .b(new_n46_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n55_), .O(z01));
  nand2  g014(.a(new_n56_), .b(x01), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n55_), .O(z02));
  nand2  g016(.a(x08), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n55_), .O(z03));
  nand2  g018(.a(x08), .b(x03), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n55_), .O(z04));
  nand2  g020(.a(new_n56_), .b(x04), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n55_), .O(z05));
  nand2  g022(.a(x08), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n55_), .O(z06));
  nand2  g024(.a(x08), .b(x06), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n55_), .O(z07));
  nand2  g026(.a(new_n56_), .b(x07), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n55_), .O(z08));
  nand3  g028(.a(x21), .b(x20), .c(x11), .O(new_n73_));
  inv1   g029(.a(x22), .O(new_n74_));
  inv1   g030(.a(x23), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  inv1   g032(.a(x24), .O(new_n77_));
  nor2   g033(.a(new_n45_), .b(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n73_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(x25), .c(new_n47_), .d(x09), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(x08), .c(new_n57_), .O(z09));
  inv1   g038(.a(x25), .O(new_n83_));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand3  g040(.a(x26), .b(x24), .c(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  inv1   g043(.a(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  aoi21  g045(.a(new_n89_), .b(new_n87_), .c(new_n83_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n59_), .O(z10));
  inv1   g048(.a(x09), .O(new_n93_));
  inv1   g049(.a(x19), .O(new_n94_));
  nand2  g050(.a(x22), .b(x13), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n85_), .c(x20), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n94_), .c(x21), .O(new_n97_));
  inv1   g053(.a(x21), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(x20), .c(x19), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n97_), .c(new_n93_), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n46_), .c(new_n83_), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(x10), .c(new_n61_), .O(z11));
  nor2   g058(.a(new_n88_), .b(new_n94_), .O(new_n103_));
  nand3  g059(.a(new_n78_), .b(x23), .c(x14), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n103_), .c(x21), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(x22), .O(new_n106_));
  nand3  g062(.a(new_n103_), .b(new_n74_), .c(x21), .O(new_n107_));
  aoi21  g063(.a(new_n107_), .b(new_n106_), .c(new_n93_), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n46_), .c(new_n83_), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(x10), .c(new_n63_), .O(z12));
  inv1   g066(.a(new_n103_), .O(new_n111_));
  nand2  g067(.a(new_n78_), .b(x15), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x22), .c(x21), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(x23), .O(new_n114_));
  nand4  g070(.a(new_n103_), .b(new_n75_), .c(x22), .d(x21), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(new_n83_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n65_), .O(z13));
  nand3  g074(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nand2  g075(.a(x26), .b(x16), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x23), .c(x22), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x24), .O(new_n122_));
  inv1   g078(.a(new_n119_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n77_), .c(x23), .d(x22), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(new_n93_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n46_), .c(new_n83_), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(x10), .c(new_n67_), .O(z14));
  nand2  g083(.a(new_n77_), .b(x23), .O(new_n128_));
  nand2  g084(.a(x25), .b(new_n75_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n128_), .c(x22), .d(x21), .O(new_n130_));
  nor2   g086(.a(new_n83_), .b(x20), .O(new_n131_));
  aoi21  g087(.a(new_n130_), .b(x20), .c(new_n131_), .O(new_n132_));
  aoi22  g088(.a(x26), .b(x17), .c(x25), .d(new_n94_), .O(new_n133_));
  oai21  g089(.a(new_n132_), .b(new_n94_), .c(new_n133_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x09), .c(new_n46_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x25), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n69_), .O(z15));
  inv1   g094(.a(x18), .O(new_n139_));
  nand2  g095(.a(new_n103_), .b(new_n139_), .O(new_n140_));
  nand4  g096(.a(x24), .b(x23), .c(x22), .d(x21), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n140_), .c(x26), .O(new_n142_));
  nand4  g098(.a(new_n123_), .b(new_n76_), .c(new_n45_), .d(x24), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n142_), .c(new_n83_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n71_), .O(z16));
endmodule


