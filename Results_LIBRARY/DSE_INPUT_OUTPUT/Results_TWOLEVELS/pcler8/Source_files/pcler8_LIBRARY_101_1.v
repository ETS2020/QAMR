// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x13), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  nor2   g011(.a(new_n46_), .b(x13), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n55_), .O(z01));
  nand2  g015(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  nand2  g017(.a(new_n57_), .b(x02), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  inv1   g020(.a(new_n56_), .O(new_n65_));
  oai21  g021(.a(new_n47_), .b(new_n64_), .c(new_n65_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n65_), .O(z05));
  nand2  g024(.a(new_n57_), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n65_), .O(z06));
  nand2  g026(.a(new_n57_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  nand2  g028(.a(new_n57_), .b(x07), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(z08));
  nand3  g030(.a(x21), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x22), .c(x13), .O(new_n78_));
  oai21  g034(.a(x22), .b(x19), .c(new_n78_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n80_));
  oai21  g036(.a(new_n58_), .b(new_n55_), .c(new_n80_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n76_), .c(x19), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x20), .O(new_n85_));
  oai21  g041(.a(x20), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n60_), .c(new_n65_), .O(z10));
  inv1   g044(.a(x21), .O(new_n89_));
  nand3  g045(.a(x22), .b(x21), .c(x13), .O(new_n90_));
  oai21  g046(.a(new_n90_), .b(new_n76_), .c(x21), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(x20), .c(x19), .O(new_n92_));
  nand2  g048(.a(x20), .b(x19), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n89_), .c(new_n92_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n62_), .c(new_n65_), .O(z11));
  inv1   g053(.a(x13), .O(new_n98_));
  nand2  g054(.a(x23), .b(x14), .O(new_n99_));
  nand3  g055(.a(x26), .b(x25), .c(x24), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n99_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n93_), .c(x22), .O(new_n102_));
  nand3  g058(.a(new_n94_), .b(new_n46_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n98_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n105_));
  oai21  g061(.a(new_n58_), .b(new_n64_), .c(new_n105_), .O(z12));
  nand3  g062(.a(new_n48_), .b(x09), .c(new_n47_), .O(new_n107_));
  inv1   g063(.a(x23), .O(new_n108_));
  nand3  g064(.a(new_n94_), .b(new_n108_), .c(x21), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n107_), .c(x13), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x22), .O(new_n111_));
  nand4  g067(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n94_), .c(x22), .d(x21), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x23), .c(new_n48_), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x09), .c(new_n47_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(new_n111_), .c(new_n67_), .O(z13));
  nand3  g073(.a(x26), .b(x25), .c(x16), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n94_), .c(x23), .d(x21), .O(new_n119_));
  nand3  g075(.a(x21), .b(x20), .c(x19), .O(new_n120_));
  inv1   g076(.a(x24), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  aoi21  g079(.a(new_n119_), .b(x24), .c(new_n123_), .O(new_n124_));
  nand2  g080(.a(x24), .b(new_n46_), .O(new_n125_));
  oai21  g081(.a(new_n124_), .b(new_n98_), .c(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n69_), .O(z14));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x24), .c(x23), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n120_), .c(x25), .O(new_n131_));
  inv1   g087(.a(new_n120_), .O(new_n132_));
  nor2   g088(.a(new_n108_), .b(new_n46_), .O(new_n133_));
  nor2   g089(.a(x25), .b(new_n121_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x13), .O(new_n137_));
  nand2  g093(.a(x25), .b(new_n46_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n71_), .O(z15));
  nand4  g097(.a(new_n94_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n142_));
  nor2   g098(.a(new_n108_), .b(new_n89_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n45_), .c(x25), .d(x24), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n142_), .c(x13), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x22), .O(new_n146_));
  inv1   g102(.a(x18), .O(new_n147_));
  nand4  g103(.a(x21), .b(x20), .c(x19), .d(new_n147_), .O(new_n148_));
  nand3  g104(.a(new_n133_), .b(x25), .c(x24), .O(new_n149_));
  nor2   g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nor2   g106(.a(new_n150_), .b(new_n45_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n146_), .c(new_n73_), .O(z16));
endmodule


