// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(new_n48_), .b(x09), .c(new_n47_), .d(x02), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  nor2   g011(.a(x10), .b(x02), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nor2   g014(.a(new_n58_), .b(new_n55_), .O(z01));
  nand2  g015(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  inv1   g017(.a(x02), .O(new_n62_));
  inv1   g018(.a(new_n56_), .O(new_n63_));
  oai21  g019(.a(new_n47_), .b(new_n62_), .c(new_n63_), .O(z03));
  nand2  g020(.a(x08), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n63_), .O(z04));
  nand2  g022(.a(new_n57_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(x08), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n63_), .O(z06));
  nand2  g026(.a(x08), .b(x06), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n63_), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  nor2   g029(.a(new_n58_), .b(new_n73_), .O(z08));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n46_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  inv1   g033(.a(x23), .O(new_n78_));
  inv1   g034(.a(x24), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x26), .c(x25), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n77_), .c(x19), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n83_));
  oai22  g039(.a(new_n83_), .b(new_n62_), .c(new_n58_), .d(new_n55_), .O(z09));
  nand2  g040(.a(new_n76_), .b(x12), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n81_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  inv1   g043(.a(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  aoi21  g045(.a(new_n89_), .b(new_n87_), .c(x10), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(x09), .c(new_n47_), .d(x02), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n60_), .O(z10));
  inv1   g048(.a(x19), .O(new_n93_));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  nand3  g050(.a(x26), .b(x25), .c(x24), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n93_), .c(x21), .O(new_n97_));
  nand3  g053(.a(new_n75_), .b(x20), .c(x19), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n48_), .c(x09), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n47_), .c(new_n62_), .O(z11));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n95_), .c(x20), .O(new_n103_));
  nand2  g059(.a(x21), .b(x19), .O(new_n104_));
  aoi21  g060(.a(new_n103_), .b(x02), .c(new_n104_), .O(new_n105_));
  nor2   g061(.a(new_n88_), .b(new_n93_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n46_), .c(x21), .O(new_n107_));
  oai21  g063(.a(new_n105_), .b(new_n46_), .c(new_n107_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x09), .c(new_n47_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x02), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n48_), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n65_), .O(z12));
  inv1   g068(.a(x09), .O(new_n113_));
  nand4  g069(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(x20), .c(new_n62_), .O(new_n115_));
  nand2  g071(.a(new_n76_), .b(x19), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n115_), .c(x23), .O(new_n117_));
  nand4  g073(.a(new_n106_), .b(new_n78_), .c(x22), .d(x21), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n113_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n47_), .c(new_n62_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(x10), .c(new_n67_), .O(z13));
  nand3  g077(.a(x21), .b(x20), .c(x19), .O(new_n122_));
  nand4  g078(.a(x26), .b(x25), .c(x16), .d(x02), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(x24), .O(new_n125_));
  inv1   g081(.a(new_n122_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n79_), .c(x23), .d(x22), .O(new_n127_));
  aoi21  g083(.a(new_n127_), .b(new_n125_), .c(new_n113_), .O(new_n128_));
  aoi21  g084(.a(new_n128_), .b(new_n47_), .c(new_n62_), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(x10), .c(new_n69_), .O(z14));
  nor2   g086(.a(new_n78_), .b(new_n46_), .O(new_n131_));
  nand3  g087(.a(x26), .b(x17), .c(x02), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n131_), .c(x24), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n122_), .c(x25), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  nand4  g091(.a(new_n126_), .b(new_n131_), .c(new_n135_), .d(x24), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n134_), .c(new_n113_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n47_), .c(new_n62_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(x10), .c(new_n71_), .O(z15));
  nand2  g095(.a(x09), .b(new_n47_), .O(new_n140_));
  nand3  g096(.a(x26), .b(x18), .c(new_n48_), .O(new_n141_));
  oai22  g097(.a(new_n141_), .b(new_n140_), .c(new_n47_), .d(new_n73_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x02), .O(new_n143_));
  nand4  g099(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n122_), .c(x26), .O(new_n145_));
  nor2   g101(.a(x26), .b(new_n135_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n106_), .c(new_n80_), .d(new_n76_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x09), .c(new_n47_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x02), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n48_), .O(new_n151_));
  nand3  g107(.a(x10), .b(x08), .c(x07), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n151_), .c(new_n143_), .O(z16));
endmodule


