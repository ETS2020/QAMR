// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x14), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(x25), .c(x24), .d(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n45_), .O(z00));
  nor2   g011(.a(new_n49_), .b(x10), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  and2   g013(.a(new_n57_), .b(x00), .O(z01));
  nand2  g014(.a(new_n57_), .b(x01), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z02));
  inv1   g016(.a(new_n56_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z03));
  inv1   g019(.a(x03), .O(new_n64_));
  inv1   g020(.a(new_n57_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(z04));
  nand2  g022(.a(x08), .b(x04), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n61_), .O(z05));
  nand2  g024(.a(new_n57_), .b(x05), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n61_), .O(z06));
  inv1   g026(.a(x06), .O(new_n71_));
  oai21  g027(.a(new_n47_), .b(new_n71_), .c(new_n61_), .O(z07));
  inv1   g028(.a(x07), .O(new_n73_));
  nor2   g029(.a(new_n65_), .b(new_n73_), .O(z08));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n46_), .b(new_n75_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x20), .c(x11), .O(new_n77_));
  inv1   g033(.a(x23), .O(new_n78_));
  inv1   g034(.a(x24), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  inv1   g036(.a(x25), .O(new_n81_));
  nor2   g037(.a(new_n45_), .b(new_n81_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n77_), .c(x19), .O(new_n84_));
  nand3  g040(.a(new_n84_), .b(x09), .c(new_n47_), .O(new_n85_));
  aoi21  g041(.a(new_n85_), .b(new_n49_), .c(x10), .O(new_n86_));
  or2    g042(.a(new_n86_), .b(z01), .O(z09));
  nand2  g043(.a(new_n76_), .b(x12), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n83_), .c(x19), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(x20), .O(new_n90_));
  inv1   g046(.a(x20), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(x19), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n90_), .c(x14), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n59_), .O(z10));
  inv1   g051(.a(x09), .O(new_n96_));
  inv1   g052(.a(x19), .O(new_n97_));
  nand3  g053(.a(x23), .b(x22), .c(x13), .O(new_n98_));
  nand3  g054(.a(x26), .b(x25), .c(x24), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n97_), .c(x21), .O(new_n101_));
  nand3  g057(.a(new_n75_), .b(x20), .c(x19), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(new_n101_), .c(new_n96_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n47_), .c(x14), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(x10), .c(new_n62_), .O(z11));
  nand3  g061(.a(x21), .b(x20), .c(x19), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x22), .O(new_n107_));
  nor2   g063(.a(new_n91_), .b(new_n97_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n46_), .c(x21), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n107_), .c(new_n96_), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n47_), .c(x14), .O(new_n111_));
  oai22  g067(.a(new_n111_), .b(x10), .c(new_n65_), .d(new_n64_), .O(z12));
  inv1   g068(.a(new_n99_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x15), .c(new_n49_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n108_), .c(x22), .d(x21), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x23), .O(new_n116_));
  nand4  g072(.a(new_n108_), .b(new_n78_), .c(x22), .d(x21), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n116_), .c(new_n96_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n47_), .c(x14), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(x10), .c(new_n67_), .O(z13));
  nand2  g076(.a(new_n82_), .b(x16), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x23), .c(x22), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n106_), .c(x24), .O(new_n123_));
  inv1   g079(.a(new_n106_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n79_), .c(x23), .d(x22), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n123_), .c(x14), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n69_), .O(z14));
  nor2   g084(.a(new_n78_), .b(new_n46_), .O(new_n129_));
  nand2  g085(.a(x26), .b(x17), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n129_), .c(x24), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n106_), .c(x25), .O(new_n132_));
  nand4  g088(.a(new_n124_), .b(new_n129_), .c(new_n81_), .d(x24), .O(new_n133_));
  aoi21  g089(.a(new_n133_), .b(new_n132_), .c(x14), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n135_));
  oai21  g091(.a(new_n65_), .b(new_n71_), .c(new_n135_), .O(z15));
  inv1   g092(.a(x18), .O(new_n137_));
  nand4  g093(.a(x21), .b(x20), .c(x19), .d(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n129_), .b(x25), .c(x24), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x26), .O(new_n140_));
  nor2   g096(.a(x26), .b(new_n81_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n108_), .c(new_n80_), .d(new_n76_), .O(new_n142_));
  aoi21  g098(.a(new_n142_), .b(new_n140_), .c(x14), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n144_));
  oai21  g100(.a(new_n65_), .b(new_n73_), .c(new_n144_), .O(z16));
endmodule


