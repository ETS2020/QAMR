// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x20), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x10), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  nand2  g003(.a(x09), .b(new_n47_), .O(new_n48_));
  nand2  g004(.a(x20), .b(x19), .O(new_n49_));
  nor3   g005(.a(new_n49_), .b(new_n48_), .c(x10), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  inv1   g007(.a(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g009(.a(x26), .b(x25), .c(x24), .O(new_n54_));
  inv1   g010(.a(new_n54_), .O(new_n55_));
  nand4  g011(.a(new_n55_), .b(new_n53_), .c(new_n50_), .d(x21), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n46_), .O(z00));
  aoi21  g013(.a(new_n45_), .b(x10), .c(new_n47_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z01));
  nand2  g016(.a(new_n58_), .b(x01), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z02));
  nand2  g018(.a(new_n58_), .b(x02), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n46_), .O(z03));
  nand2  g020(.a(new_n58_), .b(x03), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n46_), .O(z04));
  nand2  g022(.a(new_n58_), .b(x04), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z05));
  nand2  g024(.a(new_n58_), .b(x05), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z06));
  nand2  g026(.a(new_n58_), .b(x06), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z07));
  nand2  g028(.a(new_n58_), .b(x07), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(new_n46_), .O(z08));
  inv1   g030(.a(x10), .O(new_n75_));
  inv1   g031(.a(x21), .O(new_n76_));
  nor2   g032(.a(new_n51_), .b(new_n76_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(x20), .c(x11), .O(new_n78_));
  nand4  g034(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n79_));
  oai21  g035(.a(new_n79_), .b(new_n78_), .c(x19), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n59_), .O(z09));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n79_), .c(x19), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x20), .O(new_n85_));
  nand2  g041(.a(new_n45_), .b(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n61_), .O(z10));
  inv1   g045(.a(x19), .O(new_n90_));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n54_), .c(x20), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n90_), .c(x21), .O(new_n93_));
  nand3  g049(.a(new_n76_), .b(x20), .c(x19), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n63_), .O(z11));
  nand2  g053(.a(x23), .b(x14), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n54_), .c(x21), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n49_), .c(x22), .O(new_n100_));
  inv1   g056(.a(new_n49_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n51_), .c(x21), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n65_), .O(z12));
  nand2  g061(.a(x23), .b(x09), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(x08), .c(new_n75_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n45_), .O(new_n108_));
  nand2  g064(.a(x21), .b(x19), .O(new_n109_));
  nand4  g065(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x22), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n109_), .c(x23), .O(new_n112_));
  nand4  g068(.a(new_n101_), .b(new_n52_), .c(x22), .d(x21), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n108_), .c(new_n67_), .O(z13));
  inv1   g072(.a(x24), .O(new_n117_));
  oai21  g073(.a(new_n48_), .b(new_n117_), .c(new_n75_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n45_), .O(new_n119_));
  nand3  g075(.a(x26), .b(x25), .c(x16), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x23), .c(x22), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n109_), .c(x24), .O(new_n122_));
  nand3  g078(.a(x21), .b(x20), .c(x19), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n117_), .c(x23), .d(x22), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n119_), .c(new_n69_), .O(z14));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(x24), .c(x23), .d(x22), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n123_), .c(x25), .O(new_n131_));
  inv1   g087(.a(x25), .O(new_n132_));
  nand4  g088(.a(new_n124_), .b(new_n53_), .c(new_n132_), .d(x24), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n71_), .O(z15));
  inv1   g092(.a(x18), .O(new_n137_));
  nand4  g093(.a(x21), .b(x20), .c(x19), .d(new_n137_), .O(new_n138_));
  nand4  g094(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x26), .O(new_n140_));
  nor2   g096(.a(new_n117_), .b(new_n52_), .O(new_n141_));
  nor2   g097(.a(x26), .b(new_n132_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n141_), .c(new_n77_), .d(new_n101_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n140_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n75_), .c(x09), .d(new_n47_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n73_), .O(z16));
endmodule


