// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_,
    new_n157_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x25), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n46_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(x00), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(z01));
  inv1   g014(.a(x01), .O(new_n59_));
  inv1   g015(.a(new_n55_), .O(new_n60_));
  oai21  g016(.a(new_n46_), .b(new_n59_), .c(new_n60_), .O(z02));
  nand2  g017(.a(x08), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n60_), .O(z03));
  and2   g019(.a(new_n56_), .b(x03), .O(z04));
  nand2  g020(.a(new_n56_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  nand2  g022(.a(new_n56_), .b(x05), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n60_), .O(z06));
  nand2  g024(.a(new_n56_), .b(x06), .O(new_n69_));
  inv1   g025(.a(new_n69_), .O(z07));
  nand2  g026(.a(new_n56_), .b(x07), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(z08));
  nand3  g028(.a(x21), .b(x20), .c(x11), .O(new_n73_));
  nor2   g029(.a(new_n45_), .b(new_n54_), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x24), .c(x22), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n73_), .c(x19), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n57_), .c(new_n60_), .O(z09));
  inv1   g034(.a(x09), .O(new_n79_));
  nor2   g035(.a(new_n79_), .b(x08), .O(new_n80_));
  inv1   g036(.a(x19), .O(new_n81_));
  nand3  g037(.a(x20), .b(new_n81_), .c(new_n47_), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  aoi22  g039(.a(new_n83_), .b(new_n80_), .c(x08), .d(x01), .O(new_n84_));
  inv1   g040(.a(x23), .O(new_n85_));
  aoi21  g041(.a(x25), .b(new_n85_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  nand4  g043(.a(x22), .b(x21), .c(x20), .d(x12), .O(new_n88_));
  nand3  g044(.a(new_n74_), .b(x24), .c(x23), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n88_), .c(new_n87_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n91_));
  oai21  g047(.a(new_n84_), .b(new_n55_), .c(new_n91_), .O(z10));
  nand3  g048(.a(new_n80_), .b(x13), .c(new_n47_), .O(new_n93_));
  nand4  g049(.a(x26), .b(x24), .c(x22), .d(x21), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n93_), .c(x23), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(x25), .O(new_n96_));
  inv1   g052(.a(x21), .O(new_n97_));
  nand2  g053(.a(x20), .b(x19), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n97_), .b(x20), .c(x19), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n96_), .c(new_n62_), .O(z11));
  inv1   g059(.a(x22), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(x21), .O(new_n105_));
  oai22  g061(.a(new_n105_), .b(new_n98_), .c(new_n104_), .d(x20), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n107_));
  nand2  g063(.a(x08), .b(x03), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n60_), .O(new_n110_));
  nand2  g066(.a(x21), .b(x19), .O(new_n111_));
  oai21  g067(.a(new_n54_), .b(x23), .c(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n74_), .b(x24), .c(x23), .d(x14), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n112_), .c(new_n104_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n110_), .O(z12));
  nand2  g072(.a(new_n85_), .b(x22), .O(new_n117_));
  oai22  g073(.a(new_n117_), .b(new_n111_), .c(new_n85_), .d(x19), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x20), .O(new_n119_));
  nand4  g075(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(x22), .c(x21), .d(x20), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x23), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n119_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n65_), .c(new_n60_), .O(z13));
  nand3  g081(.a(x26), .b(x25), .c(x16), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n99_), .c(x22), .d(x21), .O(new_n127_));
  inv1   g083(.a(x24), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x22), .c(x21), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n98_), .O(new_n130_));
  aoi21  g086(.a(new_n127_), .b(x24), .c(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n54_), .b(x24), .c(new_n85_), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n85_), .c(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n67_), .O(z14));
  nand2  g091(.a(x21), .b(x20), .O(new_n136_));
  nand3  g092(.a(new_n54_), .b(x24), .c(x22), .O(new_n137_));
  oai22  g093(.a(new_n137_), .b(new_n136_), .c(new_n54_), .d(x20), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(x19), .O(new_n139_));
  aoi21  g095(.a(x21), .b(x19), .c(new_n104_), .O(new_n140_));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x24), .c(x22), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x25), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n139_), .c(new_n85_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n69_), .O(z15));
  nor2   g102(.a(new_n98_), .b(x18), .O(new_n147_));
  nand3  g103(.a(x24), .b(x22), .c(x21), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n45_), .O(new_n150_));
  nand3  g106(.a(x21), .b(x20), .c(x19), .O(new_n151_));
  nand4  g107(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n152_));
  nor2   g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n150_), .c(x23), .O(new_n154_));
  nand2  g110(.a(x26), .b(new_n54_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n71_), .O(z16));
endmodule


