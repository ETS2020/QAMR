// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x18), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x11), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x09), .O(new_n49_));
  nor2   g005(.a(new_n49_), .b(x08), .O(new_n50_));
  nand2  g006(.a(x20), .b(x19), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  nand3  g008(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(new_n53_));
  nand3  g009(.a(x23), .b(x22), .c(x21), .O(new_n54_));
  nand3  g010(.a(x26), .b(x25), .c(x24), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(new_n57_));
  oai21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z00));
  inv1   g014(.a(x00), .O(new_n59_));
  nand2  g015(.a(new_n47_), .b(x08), .O(new_n60_));
  nor2   g016(.a(new_n60_), .b(new_n59_), .O(z01));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n60_), .b(new_n62_), .O(z02));
  inv1   g019(.a(x02), .O(new_n64_));
  nor2   g020(.a(new_n60_), .b(new_n64_), .O(z03));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n47_), .O(z04));
  nand3  g023(.a(new_n47_), .b(x08), .c(x04), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n47_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n47_), .O(z07));
  inv1   g029(.a(x07), .O(new_n74_));
  inv1   g030(.a(x08), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n74_), .c(new_n47_), .O(z08));
  inv1   g032(.a(x19), .O(new_n77_));
  nand3  g033(.a(new_n50_), .b(new_n77_), .c(new_n48_), .O(new_n78_));
  oai21  g034(.a(new_n75_), .b(new_n59_), .c(new_n78_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n47_), .O(new_n80_));
  and2   g036(.a(x20), .b(x11), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n56_), .c(new_n50_), .d(new_n48_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n80_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand4  g040(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  inv1   g043(.a(x20), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x19), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n91_));
  nand2  g047(.a(x08), .b(x01), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n91_), .c(new_n46_), .O(z10));
  nand3  g049(.a(x23), .b(x22), .c(x13), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n55_), .c(x20), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n77_), .c(x21), .O(new_n96_));
  inv1   g052(.a(x21), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x20), .c(x19), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n96_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n100_));
  nand2  g056(.a(x08), .b(x02), .O(new_n101_));
  aoi21  g057(.a(new_n101_), .b(new_n100_), .c(new_n46_), .O(z11));
  nand2  g058(.a(x23), .b(x14), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n55_), .c(x21), .O(new_n104_));
  oai21  g060(.a(new_n104_), .b(new_n51_), .c(x22), .O(new_n105_));
  inv1   g061(.a(x22), .O(new_n106_));
  nand3  g062(.a(new_n52_), .b(new_n106_), .c(x21), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n66_), .c(new_n47_), .O(z12));
  nand4  g066(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x22), .c(x21), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n51_), .c(x23), .O(new_n113_));
  inv1   g069(.a(x23), .O(new_n114_));
  nand4  g070(.a(new_n52_), .b(new_n114_), .c(x22), .d(x21), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n113_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n68_), .c(new_n47_), .O(z13));
  nand3  g074(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nand3  g075(.a(x26), .b(x25), .c(x16), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(x23), .c(x22), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n119_), .c(x24), .O(new_n122_));
  inv1   g078(.a(new_n119_), .O(new_n123_));
  nor2   g079(.a(x24), .b(new_n114_), .O(new_n124_));
  nand3  g080(.a(new_n124_), .b(new_n123_), .c(x22), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n70_), .c(new_n47_), .O(z14));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(x24), .c(x23), .d(x22), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n119_), .c(x25), .O(new_n131_));
  inv1   g087(.a(x25), .O(new_n132_));
  nor2   g088(.a(new_n114_), .b(new_n106_), .O(new_n133_));
  nand4  g089(.a(new_n123_), .b(new_n133_), .c(new_n132_), .d(x24), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n72_), .c(new_n47_), .O(z15));
  inv1   g093(.a(x26), .O(new_n138_));
  inv1   g094(.a(new_n54_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n138_), .c(x25), .d(x24), .O(new_n140_));
  oai22  g096(.a(new_n140_), .b(new_n53_), .c(new_n75_), .d(new_n74_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n47_), .O(new_n142_));
  nand4  g098(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n119_), .c(new_n45_), .O(new_n144_));
  nand2  g100(.a(x18), .b(x11), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n144_), .c(new_n138_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n48_), .c(x09), .d(new_n75_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n142_), .O(z16));
endmodule


