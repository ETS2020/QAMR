// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x25), .O(new_n45_));
  inv1   g001(.a(x10), .O(new_n46_));
  inv1   g002(.a(x09), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x08), .O(new_n48_));
  inv1   g004(.a(x20), .O(new_n49_));
  inv1   g005(.a(x22), .O(new_n50_));
  nand3  g006(.a(x26), .b(x24), .c(x23), .O(new_n51_));
  nor3   g007(.a(new_n51_), .b(new_n50_), .c(new_n49_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(new_n48_), .c(x19), .d(new_n46_), .O(new_n53_));
  aoi21  g009(.a(new_n53_), .b(x21), .c(new_n45_), .O(z00));
  inv1   g010(.a(x00), .O(new_n55_));
  inv1   g011(.a(x08), .O(new_n56_));
  nor2   g012(.a(new_n45_), .b(x21), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(new_n55_), .O(z01));
  nand2  g016(.a(new_n58_), .b(x01), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z02));
  inv1   g018(.a(x02), .O(new_n63_));
  nor2   g019(.a(new_n59_), .b(new_n63_), .O(z03));
  inv1   g020(.a(new_n57_), .O(new_n65_));
  nand2  g021(.a(x08), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n65_), .O(z04));
  nand2  g023(.a(new_n58_), .b(x04), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z05));
  nand2  g025(.a(new_n58_), .b(x05), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z06));
  nand2  g027(.a(x08), .b(x06), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n65_), .O(z07));
  nand2  g029(.a(new_n58_), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n65_), .O(z08));
  inv1   g031(.a(x19), .O(new_n76_));
  nand3  g032(.a(new_n48_), .b(new_n76_), .c(new_n46_), .O(new_n77_));
  oai21  g033(.a(new_n56_), .b(new_n55_), .c(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x10), .O(new_n81_));
  inv1   g037(.a(x23), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n50_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x21), .O(new_n84_));
  nand3  g040(.a(x26), .b(x25), .c(x24), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n81_), .c(new_n48_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n79_), .O(z09));
  nand3  g044(.a(x23), .b(x22), .c(x12), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n85_), .c(x19), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x20), .O(new_n91_));
  oai21  g047(.a(x20), .b(new_n76_), .c(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n61_), .c(new_n65_), .O(z10));
  nand3  g050(.a(x23), .b(x22), .c(x13), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n85_), .c(x20), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n76_), .c(x21), .O(new_n97_));
  inv1   g053(.a(x21), .O(new_n98_));
  nand2  g054(.a(x20), .b(x19), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n45_), .c(new_n98_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n97_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n103_));
  oai21  g059(.a(new_n59_), .b(new_n63_), .c(new_n103_), .O(z11));
  nand3  g060(.a(new_n48_), .b(x22), .c(new_n46_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n45_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n98_), .O(new_n107_));
  nand2  g063(.a(x23), .b(x14), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n85_), .c(x20), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n76_), .c(x22), .O(new_n110_));
  nand3  g066(.a(new_n100_), .b(new_n50_), .c(x21), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n107_), .c(new_n66_), .O(z12));
  nand3  g070(.a(new_n48_), .b(x23), .c(new_n46_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n45_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n98_), .O(new_n117_));
  nand4  g073(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(new_n100_), .c(x22), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x23), .O(new_n120_));
  nand4  g076(.a(new_n100_), .b(new_n82_), .c(x22), .d(x21), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(new_n117_), .c(new_n68_), .O(z13));
  nand3  g080(.a(new_n48_), .b(x24), .c(new_n46_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n45_), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(new_n98_), .O(new_n127_));
  nand3  g083(.a(x26), .b(x25), .c(x16), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(x23), .c(x22), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n99_), .c(x24), .O(new_n130_));
  nand3  g086(.a(x21), .b(x20), .c(x19), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nor2   g088(.a(x24), .b(new_n82_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n132_), .c(x22), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n127_), .c(new_n70_), .O(z14));
  nand3  g093(.a(x22), .b(x20), .c(x19), .O(new_n138_));
  nand2  g094(.a(x26), .b(x17), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(x24), .c(x23), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n138_), .c(x25), .O(new_n141_));
  nand4  g097(.a(new_n132_), .b(new_n83_), .c(new_n45_), .d(x24), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n72_), .c(new_n65_), .O(z15));
  inv1   g101(.a(x26), .O(new_n146_));
  nor2   g102(.a(new_n99_), .b(x18), .O(new_n147_));
  nand3  g103(.a(x24), .b(x23), .c(x22), .O(new_n148_));
  inv1   g104(.a(new_n148_), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n146_), .O(new_n150_));
  nand4  g106(.a(new_n146_), .b(x25), .c(x24), .d(x23), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n138_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x21), .O(new_n153_));
  nand2  g109(.a(x26), .b(new_n45_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n46_), .c(x09), .d(new_n56_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n74_), .O(z16));
endmodule


