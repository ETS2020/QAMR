// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_;
  inv1   g000(.a(x09), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x08), .O(new_n46_));
  nand3  g002(.a(x21), .b(x20), .c(x19), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  inv1   g004(.a(x22), .O(new_n49_));
  inv1   g005(.a(x23), .O(new_n50_));
  nor2   g006(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g007(.a(x26), .b(x25), .c(x24), .O(new_n52_));
  inv1   g008(.a(new_n52_), .O(new_n53_));
  nand4  g009(.a(new_n53_), .b(new_n51_), .c(new_n48_), .d(new_n46_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(x06), .c(x10), .O(z00));
  inv1   g011(.a(x08), .O(new_n56_));
  nor2   g012(.a(x10), .b(x06), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(x00), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(z01));
  inv1   g016(.a(new_n57_), .O(new_n61_));
  nand2  g017(.a(x08), .b(x01), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n61_), .O(z02));
  nand2  g019(.a(new_n58_), .b(x02), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n61_), .O(z03));
  nand2  g021(.a(new_n58_), .b(x03), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n61_), .O(z04));
  nand2  g023(.a(new_n58_), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n61_), .O(z05));
  nand2  g025(.a(x08), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n61_), .O(z06));
  inv1   g027(.a(x06), .O(new_n72_));
  oai21  g028(.a(new_n56_), .b(new_n72_), .c(new_n61_), .O(z07));
  inv1   g029(.a(x07), .O(new_n74_));
  inv1   g030(.a(new_n58_), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(z08));
  inv1   g032(.a(x10), .O(new_n77_));
  inv1   g033(.a(x21), .O(new_n78_));
  nor2   g034(.a(new_n49_), .b(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x20), .c(x11), .O(new_n80_));
  nand4  g036(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n81_));
  oai21  g037(.a(new_n81_), .b(new_n80_), .c(x19), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(x09), .c(new_n56_), .O(new_n83_));
  nand2  g039(.a(new_n83_), .b(x06), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n77_), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n59_), .O(z09));
  nand3  g042(.a(x22), .b(x21), .c(x12), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n81_), .c(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x20), .O(new_n89_));
  inv1   g045(.a(x20), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x19), .O(new_n91_));
  aoi21  g047(.a(new_n91_), .b(new_n89_), .c(new_n45_), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n56_), .c(new_n72_), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(x10), .c(new_n62_), .O(z10));
  inv1   g050(.a(x19), .O(new_n95_));
  nand3  g051(.a(x23), .b(x22), .c(x13), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n52_), .c(x20), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n95_), .c(x21), .O(new_n98_));
  nand3  g054(.a(new_n78_), .b(x20), .c(x19), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n98_), .c(x10), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(x09), .c(new_n56_), .d(x06), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n64_), .O(z11));
  nor2   g058(.a(new_n90_), .b(new_n95_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand2  g060(.a(x23), .b(x14), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n52_), .c(x21), .O(new_n106_));
  oai21  g062(.a(new_n106_), .b(new_n104_), .c(x22), .O(new_n107_));
  nand3  g063(.a(new_n103_), .b(new_n49_), .c(x21), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n107_), .c(x10), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(x09), .c(new_n56_), .d(x06), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n66_), .O(z12));
  nand4  g067(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(x22), .c(x21), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n104_), .c(x23), .O(new_n114_));
  nand4  g070(.a(new_n103_), .b(new_n50_), .c(x22), .d(x21), .O(new_n115_));
  aoi21  g071(.a(new_n115_), .b(new_n114_), .c(x10), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(x09), .c(new_n56_), .d(x06), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n68_), .O(z13));
  nand3  g074(.a(x22), .b(x21), .c(x20), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x06), .O(new_n120_));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n120_), .c(x23), .d(x19), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x24), .O(new_n123_));
  inv1   g079(.a(x24), .O(new_n124_));
  nand4  g080(.a(new_n48_), .b(new_n124_), .c(x23), .d(x22), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n123_), .c(new_n45_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n56_), .c(new_n72_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(x10), .c(new_n70_), .O(z14));
  nand2  g084(.a(x26), .b(x17), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n51_), .c(x24), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n47_), .c(x25), .O(new_n131_));
  inv1   g087(.a(x25), .O(new_n132_));
  nand4  g088(.a(new_n51_), .b(new_n48_), .c(new_n132_), .d(x24), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n77_), .c(x09), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n56_), .c(new_n72_), .O(z15));
  inv1   g092(.a(x18), .O(new_n137_));
  nand4  g093(.a(x21), .b(x20), .c(x19), .d(new_n137_), .O(new_n138_));
  nand4  g094(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x26), .O(new_n140_));
  nor2   g096(.a(new_n124_), .b(new_n50_), .O(new_n141_));
  nor2   g097(.a(x26), .b(new_n132_), .O(new_n142_));
  nand4  g098(.a(new_n142_), .b(new_n103_), .c(new_n141_), .d(new_n79_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n140_), .c(new_n45_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n56_), .c(new_n72_), .O(new_n145_));
  oai22  g101(.a(new_n145_), .b(x10), .c(new_n75_), .d(new_n74_), .O(z16));
endmodule


