// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_;
  inv1   g00(.a(x26), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand4  g03(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g09(.a(x22), .O(new_n54_));
  nor2   g10(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g11(.a(new_n55_), .O(new_n56_));
  nor2   g12(.a(new_n55_), .b(new_n46_), .O(new_n57_));
  nand2  g13(.a(new_n57_), .b(x00), .O(new_n58_));
  nand2  g14(.a(new_n58_), .b(new_n56_), .O(z01));
  nand2  g15(.a(new_n57_), .b(x01), .O(new_n60_));
  inv1   g16(.a(new_n60_), .O(z02));
  nand2  g17(.a(new_n57_), .b(x02), .O(new_n62_));
  nand2  g18(.a(new_n62_), .b(new_n56_), .O(z03));
  nand2  g19(.a(new_n57_), .b(x03), .O(new_n64_));
  nand2  g20(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g21(.a(new_n57_), .b(x04), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z05));
  nand2  g23(.a(new_n57_), .b(x05), .O(new_n68_));
  inv1   g24(.a(new_n68_), .O(z06));
  nand2  g25(.a(new_n57_), .b(x06), .O(new_n70_));
  nand2  g26(.a(new_n70_), .b(new_n56_), .O(z07));
  aoi21  g27(.a(x08), .b(x07), .c(new_n55_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z08));
  inv1   g29(.a(x21), .O(new_n74_));
  nor2   g30(.a(new_n54_), .b(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n58_), .O(z09));
  inv1   g36(.a(x19), .O(new_n81_));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  oai21  g38(.a(new_n82_), .b(new_n77_), .c(x19), .O(new_n83_));
  nand2  g39(.a(new_n83_), .b(x20), .O(new_n84_));
  oai21  g40(.a(x20), .b(new_n81_), .c(new_n84_), .O(new_n85_));
  nand4  g41(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(new_n60_), .O(z10));
  nand3  g43(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  nand3  g44(.a(x26), .b(x25), .c(x24), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n81_), .c(x21), .O(new_n91_));
  nand3  g47(.a(new_n74_), .b(x20), .c(x19), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n62_), .O(z11));
  nand2  g51(.a(x20), .b(x19), .O(new_n96_));
  nand2  g52(.a(x23), .b(x14), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n89_), .c(x21), .O(new_n98_));
  oai21  g54(.a(new_n98_), .b(new_n96_), .c(x22), .O(new_n99_));
  inv1   g55(.a(new_n96_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(new_n54_), .c(x21), .O(new_n101_));
  nand2  g57(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  nand4  g58(.a(new_n102_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n103_));
  nand2  g59(.a(new_n103_), .b(new_n64_), .O(z12));
  inv1   g60(.a(new_n89_), .O(new_n105_));
  nor2   g61(.a(x23), .b(new_n74_), .O(new_n106_));
  and2   g62(.a(x23), .b(x15), .O(new_n107_));
  aoi22  g63(.a(new_n107_), .b(new_n105_), .c(new_n106_), .d(new_n100_), .O(new_n108_));
  nand2  g64(.a(new_n100_), .b(new_n75_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(x23), .O(new_n110_));
  oai21  g66(.a(new_n108_), .b(new_n54_), .c(new_n110_), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n66_), .O(z13));
  inv1   g69(.a(x24), .O(new_n114_));
  aoi21  g70(.a(x23), .b(x22), .c(x10), .O(new_n115_));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(x21), .c(x20), .d(x19), .O(new_n117_));
  aoi21  g73(.a(new_n117_), .b(x22), .c(new_n115_), .O(new_n118_));
  nand3  g74(.a(x21), .b(x20), .c(x19), .O(new_n119_));
  nand3  g75(.a(new_n114_), .b(x23), .c(x22), .O(new_n120_));
  oai22  g76(.a(new_n120_), .b(new_n119_), .c(new_n118_), .d(new_n114_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(x09), .c(new_n46_), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(new_n68_), .c(new_n56_), .O(z14));
  inv1   g79(.a(x25), .O(new_n124_));
  nand3  g80(.a(new_n124_), .b(x24), .c(x23), .O(new_n125_));
  nand3  g81(.a(x26), .b(x25), .c(x17), .O(new_n126_));
  oai21  g82(.a(new_n125_), .b(new_n119_), .c(new_n126_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(x22), .O(new_n128_));
  and2   g84(.a(x24), .b(x23), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(x22), .O(new_n130_));
  oai21  g86(.a(new_n130_), .b(new_n119_), .c(x25), .O(new_n131_));
  nand2  g87(.a(new_n131_), .b(new_n128_), .O(new_n132_));
  nand4  g88(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n133_));
  nand2  g89(.a(new_n133_), .b(new_n70_), .O(z15));
  nand2  g90(.a(new_n119_), .b(x22), .O(new_n135_));
  inv1   g91(.a(x18), .O(new_n136_));
  nand2  g92(.a(x22), .b(new_n136_), .O(new_n137_));
  nand3  g93(.a(x25), .b(x24), .c(x23), .O(new_n138_));
  oai21  g94(.a(new_n138_), .b(new_n137_), .c(new_n47_), .O(new_n139_));
  aoi21  g95(.a(new_n139_), .b(new_n135_), .c(new_n45_), .O(new_n140_));
  nand3  g96(.a(new_n129_), .b(new_n45_), .c(x25), .O(new_n141_));
  nor2   g97(.a(new_n141_), .b(new_n109_), .O(new_n142_));
  oai21  g98(.a(new_n142_), .b(new_n140_), .c(x09), .O(new_n143_));
  oai21  g99(.a(new_n143_), .b(x08), .c(new_n72_), .O(z16));
endmodule


