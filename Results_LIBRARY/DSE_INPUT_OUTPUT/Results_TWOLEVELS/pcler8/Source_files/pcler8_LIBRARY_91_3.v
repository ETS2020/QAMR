// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_;
  inv1   g00(.a(x08), .O(new_n45_));
  inv1   g01(.a(x10), .O(new_n46_));
  nand2  g02(.a(x20), .b(x19), .O(new_n47_));
  inv1   g03(.a(new_n47_), .O(new_n48_));
  nand4  g04(.a(new_n48_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n49_));
  inv1   g05(.a(x22), .O(new_n50_));
  inv1   g06(.a(x23), .O(new_n51_));
  nor2   g07(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g08(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g09(.a(new_n53_), .O(new_n54_));
  nand3  g10(.a(new_n54_), .b(new_n52_), .c(x21), .O(new_n55_));
  oai22  g11(.a(new_n55_), .b(new_n49_), .c(new_n46_), .d(new_n45_), .O(z00));
  nand2  g12(.a(x08), .b(x00), .O(new_n57_));
  nor2   g13(.a(new_n57_), .b(x10), .O(z01));
  nand2  g14(.a(x08), .b(x01), .O(new_n59_));
  nor2   g15(.a(new_n59_), .b(x10), .O(z02));
  oai21  g16(.a(x10), .b(x02), .c(x08), .O(new_n61_));
  inv1   g17(.a(new_n61_), .O(z03));
  nand2  g18(.a(x08), .b(x03), .O(new_n63_));
  nor2   g19(.a(new_n63_), .b(x10), .O(z04));
  nand2  g20(.a(x08), .b(x04), .O(new_n65_));
  nor2   g21(.a(new_n65_), .b(x10), .O(z05));
  inv1   g22(.a(x05), .O(new_n67_));
  aoi21  g23(.a(new_n46_), .b(new_n67_), .c(new_n45_), .O(z06));
  nand2  g24(.a(x08), .b(x06), .O(new_n69_));
  nor2   g25(.a(new_n69_), .b(x10), .O(z07));
  nand2  g26(.a(x08), .b(x07), .O(new_n71_));
  nor2   g27(.a(new_n71_), .b(x10), .O(z08));
  oai21  g28(.a(x10), .b(x00), .c(x08), .O(new_n73_));
  inv1   g29(.a(x21), .O(new_n74_));
  nor2   g30(.a(new_n50_), .b(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(x20), .c(x11), .O(new_n76_));
  nand4  g32(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g33(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand4  g34(.a(new_n78_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n73_), .O(z09));
  oai21  g36(.a(x10), .b(x01), .c(x08), .O(new_n81_));
  inv1   g37(.a(x19), .O(new_n82_));
  nand3  g38(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n77_), .c(x19), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x20), .O(new_n85_));
  oai21  g41(.a(x20), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand4  g42(.a(new_n86_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n87_));
  nand2  g43(.a(new_n87_), .b(new_n81_), .O(z10));
  nand3  g44(.a(x23), .b(x22), .c(x13), .O(new_n89_));
  oai21  g45(.a(new_n89_), .b(new_n53_), .c(x20), .O(new_n90_));
  oai21  g46(.a(new_n90_), .b(new_n82_), .c(x21), .O(new_n91_));
  nand3  g47(.a(new_n74_), .b(x20), .c(x19), .O(new_n92_));
  nand2  g48(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g49(.a(new_n93_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n94_));
  nand2  g50(.a(new_n94_), .b(new_n61_), .O(z11));
  nand2  g51(.a(x23), .b(x14), .O(new_n96_));
  oai21  g52(.a(new_n96_), .b(new_n53_), .c(x21), .O(new_n97_));
  oai21  g53(.a(new_n97_), .b(new_n47_), .c(x22), .O(new_n98_));
  nand3  g54(.a(new_n48_), .b(new_n50_), .c(x21), .O(new_n99_));
  nand2  g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand3  g56(.a(new_n100_), .b(x09), .c(new_n45_), .O(new_n101_));
  aoi21  g57(.a(new_n101_), .b(new_n63_), .c(x10), .O(z12));
  oai21  g58(.a(x10), .b(x04), .c(x08), .O(new_n103_));
  nand4  g59(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(x22), .c(x21), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n47_), .c(x23), .O(new_n106_));
  nand4  g62(.a(new_n48_), .b(new_n51_), .c(x22), .d(x21), .O(new_n107_));
  nand2  g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand4  g64(.a(new_n108_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n109_));
  nand2  g65(.a(new_n109_), .b(new_n103_), .O(z13));
  nand3  g66(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  nand3  g67(.a(x26), .b(x25), .c(x16), .O(new_n112_));
  nand3  g68(.a(new_n112_), .b(x23), .c(x22), .O(new_n113_));
  oai21  g69(.a(new_n113_), .b(new_n111_), .c(x24), .O(new_n114_));
  inv1   g70(.a(x24), .O(new_n115_));
  inv1   g71(.a(new_n111_), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n115_), .c(x23), .d(x22), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n114_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(x09), .c(new_n45_), .O(new_n119_));
  nand2  g75(.a(x08), .b(x05), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n119_), .c(x10), .O(z14));
  nand2  g77(.a(x26), .b(x17), .O(new_n122_));
  nand4  g78(.a(new_n122_), .b(x24), .c(x23), .d(x22), .O(new_n123_));
  oai21  g79(.a(new_n123_), .b(new_n111_), .c(x25), .O(new_n124_));
  inv1   g80(.a(x25), .O(new_n125_));
  nand4  g81(.a(new_n116_), .b(new_n52_), .c(new_n125_), .d(x24), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n124_), .O(new_n127_));
  nand3  g83(.a(new_n127_), .b(x09), .c(new_n45_), .O(new_n128_));
  aoi21  g84(.a(new_n128_), .b(new_n69_), .c(x10), .O(z15));
  inv1   g85(.a(x18), .O(new_n130_));
  nand4  g86(.a(x21), .b(x20), .c(x19), .d(new_n130_), .O(new_n131_));
  nand4  g87(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(new_n131_), .c(x26), .O(new_n133_));
  nor2   g89(.a(new_n115_), .b(new_n51_), .O(new_n134_));
  nor2   g90(.a(x26), .b(new_n125_), .O(new_n135_));
  nand4  g91(.a(new_n135_), .b(new_n134_), .c(new_n75_), .d(new_n48_), .O(new_n136_));
  nand2  g92(.a(new_n136_), .b(new_n133_), .O(new_n137_));
  nand3  g93(.a(new_n137_), .b(x09), .c(new_n45_), .O(new_n138_));
  aoi21  g94(.a(new_n138_), .b(new_n71_), .c(x10), .O(z16));
endmodule


