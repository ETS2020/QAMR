// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n65_, new_n67_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_;
  inv1   g000(.a(x13), .O(new_n45_));
  nor2   g001(.a(x18), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  inv1   g004(.a(x09), .O(new_n49_));
  nor2   g005(.a(new_n49_), .b(x08), .O(new_n50_));
  nand2  g006(.a(x20), .b(x19), .O(new_n51_));
  inv1   g007(.a(new_n51_), .O(new_n52_));
  inv1   g008(.a(x22), .O(new_n53_));
  inv1   g009(.a(x23), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(x21), .O(new_n56_));
  nand3  g012(.a(x26), .b(x25), .c(x24), .O(new_n57_));
  nor2   g013(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand4  g014(.a(new_n58_), .b(new_n52_), .c(new_n50_), .d(new_n48_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n47_), .O(z00));
  inv1   g016(.a(x08), .O(new_n61_));
  nor2   g017(.a(new_n46_), .b(new_n61_), .O(new_n62_));
  and2   g018(.a(new_n62_), .b(x00), .O(z01));
  and2   g019(.a(new_n62_), .b(x01), .O(z02));
  inv1   g020(.a(x02), .O(new_n65_));
  oai21  g021(.a(new_n61_), .b(new_n65_), .c(new_n47_), .O(z03));
  nand2  g022(.a(new_n62_), .b(x03), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z04));
  and2   g024(.a(new_n62_), .b(x04), .O(z05));
  inv1   g025(.a(x05), .O(new_n70_));
  oai21  g026(.a(new_n61_), .b(new_n70_), .c(new_n47_), .O(z06));
  nand2  g027(.a(new_n62_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  nand2  g029(.a(new_n62_), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n47_), .O(z08));
  nand4  g031(.a(x22), .b(x21), .c(x20), .d(x11), .O(new_n76_));
  nand4  g032(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n76_), .c(x19), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n79_));
  nand2  g035(.a(x08), .b(x00), .O(new_n80_));
  aoi21  g036(.a(new_n80_), .b(new_n79_), .c(new_n46_), .O(z09));
  inv1   g037(.a(x19), .O(new_n82_));
  nand3  g038(.a(x22), .b(x21), .c(x12), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n77_), .c(x19), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x20), .O(new_n85_));
  oai21  g041(.a(x20), .b(new_n82_), .c(new_n85_), .O(new_n86_));
  nand4  g042(.a(new_n86_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n87_));
  nand2  g043(.a(x08), .b(x01), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n87_), .c(new_n46_), .O(z10));
  inv1   g045(.a(x21), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(x20), .c(x19), .O(new_n91_));
  oai21  g047(.a(new_n52_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n93_));
  oai21  g049(.a(new_n61_), .b(new_n65_), .c(new_n93_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n47_), .O(new_n95_));
  nand2  g051(.a(x18), .b(x13), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(x10), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(new_n58_), .c(new_n50_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n95_), .O(z11));
  nand2  g055(.a(x23), .b(x14), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n57_), .c(x21), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n51_), .c(x22), .O(new_n102_));
  nand3  g058(.a(new_n52_), .b(new_n53_), .c(x21), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n67_), .c(new_n47_), .O(z12));
  nand4  g062(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x22), .c(x21), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n51_), .c(x23), .O(new_n109_));
  nand4  g065(.a(new_n52_), .b(new_n54_), .c(x22), .d(x21), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n112_));
  nand2  g068(.a(x08), .b(x04), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n112_), .c(new_n46_), .O(z13));
  nand3  g070(.a(x21), .b(x20), .c(x19), .O(new_n115_));
  nand3  g071(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(x22), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n115_), .c(x24), .O(new_n118_));
  inv1   g074(.a(x24), .O(new_n119_));
  inv1   g075(.a(new_n115_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n119_), .c(x23), .d(x22), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n118_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n123_));
  nand2  g079(.a(x08), .b(x05), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n123_), .c(new_n46_), .O(z14));
  nand2  g081(.a(x26), .b(x17), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n55_), .c(x24), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n115_), .c(x25), .O(new_n128_));
  inv1   g084(.a(x25), .O(new_n129_));
  nand4  g085(.a(new_n120_), .b(new_n55_), .c(new_n129_), .d(x24), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n128_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n72_), .c(new_n47_), .O(z15));
  nand4  g089(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n115_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(x26), .c(x18), .O(new_n136_));
  oai21  g092(.a(new_n134_), .b(new_n115_), .c(x26), .O(new_n137_));
  nor2   g093(.a(new_n119_), .b(new_n54_), .O(new_n138_));
  nand4  g094(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nor2   g096(.a(x26), .b(new_n129_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n140_), .c(new_n138_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n137_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n45_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n136_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n48_), .c(x09), .d(new_n61_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n74_), .O(z16));
endmodule


