// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x22), .O(new_n46_));
  inv1   g002(.a(x08), .O(new_n47_));
  inv1   g003(.a(x10), .O(new_n48_));
  nand4  g004(.a(x11), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n49_));
  inv1   g005(.a(new_n49_), .O(new_n50_));
  nand4  g006(.a(new_n50_), .b(x21), .c(x20), .d(x19), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g008(.a(new_n52_), .b(x25), .c(x24), .d(x23), .O(new_n53_));
  nor2   g009(.a(new_n53_), .b(new_n45_), .O(z00));
  inv1   g010(.a(x23), .O(new_n55_));
  nor2   g011(.a(new_n55_), .b(x11), .O(new_n56_));
  nor2   g012(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g014(.a(new_n58_), .O(z01));
  inv1   g015(.a(new_n56_), .O(new_n60_));
  nand2  g016(.a(x08), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n60_), .O(z02));
  and2   g018(.a(new_n57_), .b(x02), .O(z03));
  nand2  g019(.a(x08), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n60_), .O(z04));
  aoi21  g021(.a(x08), .b(x04), .c(new_n56_), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(new_n57_), .b(x05), .O(new_n68_));
  inv1   g024(.a(new_n68_), .O(z06));
  nand2  g025(.a(new_n57_), .b(x06), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z07));
  nand2  g027(.a(new_n57_), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n60_), .O(z08));
  inv1   g029(.a(x20), .O(new_n74_));
  inv1   g030(.a(x21), .O(new_n75_));
  nor2   g031(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(x19), .c(x11), .O(new_n77_));
  nand3  g033(.a(x26), .b(x25), .c(x24), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x23), .c(x22), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n77_), .c(x19), .O(new_n81_));
  nand4  g037(.a(new_n81_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n58_), .c(new_n60_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  inv1   g040(.a(x25), .O(new_n85_));
  nor2   g041(.a(new_n45_), .b(new_n85_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(x24), .c(x23), .O(new_n87_));
  oai21  g043(.a(new_n87_), .b(new_n84_), .c(x19), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(x20), .O(new_n89_));
  nand2  g045(.a(new_n74_), .b(x19), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n61_), .c(new_n60_), .O(z10));
  nand2  g049(.a(x20), .b(x19), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(x21), .c(new_n48_), .d(x09), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(x08), .O(new_n96_));
  aoi21  g052(.a(x08), .b(x02), .c(new_n96_), .O(new_n97_));
  nand3  g053(.a(new_n75_), .b(x20), .c(x19), .O(new_n98_));
  nand3  g054(.a(x22), .b(x21), .c(x13), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n78_), .c(new_n98_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(x23), .c(x11), .O(new_n101_));
  inv1   g057(.a(new_n94_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n55_), .c(new_n75_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n105_));
  oai21  g061(.a(new_n97_), .b(new_n56_), .c(new_n105_), .O(z11));
  oai21  g062(.a(new_n55_), .b(x11), .c(new_n74_), .O(new_n107_));
  nand3  g063(.a(new_n79_), .b(x23), .c(x14), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n107_), .c(x21), .d(x19), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x22), .O(new_n110_));
  nand3  g066(.a(new_n102_), .b(new_n46_), .c(x21), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(new_n64_), .c(new_n60_), .O(z12));
  inv1   g070(.a(x11), .O(new_n115_));
  nand2  g071(.a(x22), .b(new_n74_), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n98_), .c(new_n115_), .O(new_n117_));
  nand3  g073(.a(new_n86_), .b(x24), .c(x15), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x22), .c(x19), .O(new_n119_));
  oai21  g075(.a(new_n119_), .b(new_n117_), .c(x23), .O(new_n120_));
  nand4  g076(.a(new_n102_), .b(new_n55_), .c(x22), .d(x21), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n66_), .O(z13));
  nand4  g080(.a(x19), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n125_));
  inv1   g081(.a(x24), .O(new_n126_));
  nand3  g082(.a(new_n76_), .b(new_n126_), .c(x22), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(x11), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x23), .O(new_n129_));
  nand3  g085(.a(x21), .b(x20), .c(x19), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand2  g087(.a(new_n86_), .b(x16), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(x23), .c(x22), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  aoi21  g090(.a(new_n134_), .b(new_n131_), .c(new_n126_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n129_), .c(new_n68_), .O(z14));
  nand4  g093(.a(new_n76_), .b(new_n85_), .c(x24), .d(x22), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n125_), .c(x11), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x23), .O(new_n140_));
  nand2  g096(.a(x26), .b(x17), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(x24), .c(x23), .d(x22), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n131_), .c(new_n85_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n140_), .c(new_n70_), .O(z15));
  nand2  g102(.a(x26), .b(x18), .O(new_n147_));
  nand4  g103(.a(new_n45_), .b(x25), .c(x24), .d(x22), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n130_), .c(new_n147_), .O(new_n149_));
  nand3  g105(.a(x25), .b(x24), .c(x22), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n131_), .c(new_n45_), .O(new_n152_));
  aoi21  g108(.a(new_n149_), .b(x23), .c(new_n152_), .O(new_n153_));
  nand2  g109(.a(x26), .b(new_n55_), .O(new_n154_));
  oai21  g110(.a(new_n153_), .b(new_n115_), .c(new_n154_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n48_), .c(x09), .d(new_n47_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n72_), .O(z16));
endmodule


