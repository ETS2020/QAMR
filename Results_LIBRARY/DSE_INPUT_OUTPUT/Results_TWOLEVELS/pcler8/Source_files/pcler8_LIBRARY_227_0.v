// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g009(.a(x20), .b(x18), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(x00), .O(new_n56_));
  inv1   g012(.a(new_n56_), .O(z01));
  inv1   g013(.a(x01), .O(new_n58_));
  inv1   g014(.a(new_n55_), .O(new_n59_));
  nor2   g015(.a(new_n59_), .b(new_n58_), .O(z02));
  nand2  g016(.a(new_n55_), .b(x02), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(z03));
  nand2  g018(.a(new_n55_), .b(x03), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(z04));
  nand2  g020(.a(new_n55_), .b(x04), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(z05));
  inv1   g022(.a(new_n54_), .O(new_n67_));
  nand2  g023(.a(x08), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n67_), .O(z06));
  nand2  g025(.a(new_n55_), .b(x06), .O(new_n70_));
  inv1   g026(.a(new_n70_), .O(z07));
  aoi21  g027(.a(x08), .b(x07), .c(new_n54_), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z08));
  and2   g029(.a(x22), .b(x21), .O(new_n74_));
  nand3  g030(.a(new_n74_), .b(x20), .c(x11), .O(new_n75_));
  nand4  g031(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n75_), .c(x19), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n56_), .c(new_n67_), .O(z09));
  nand3  g035(.a(x22), .b(x21), .c(x12), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n76_), .c(x19), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(x20), .O(new_n82_));
  inv1   g038(.a(x20), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(x19), .c(x18), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  oai21  g042(.a(new_n59_), .b(new_n58_), .c(new_n86_), .O(z10));
  nand3  g043(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  nand3  g044(.a(x26), .b(x25), .c(x24), .O(new_n89_));
  oai21  g045(.a(new_n89_), .b(new_n88_), .c(x19), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  nor2   g047(.a(x21), .b(new_n91_), .O(new_n92_));
  aoi21  g048(.a(new_n90_), .b(x21), .c(new_n92_), .O(new_n93_));
  nand3  g049(.a(x21), .b(new_n83_), .c(x18), .O(new_n94_));
  oai21  g050(.a(new_n93_), .b(new_n83_), .c(new_n94_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(new_n61_), .O(z11));
  nand2  g053(.a(x09), .b(new_n46_), .O(new_n98_));
  nand3  g054(.a(x22), .b(x18), .c(new_n47_), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n98_), .c(x18), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n83_), .O(new_n101_));
  nand2  g057(.a(x23), .b(x14), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n89_), .c(x21), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n91_), .c(x22), .O(new_n104_));
  inv1   g060(.a(x22), .O(new_n105_));
  nor2   g061(.a(new_n83_), .b(new_n91_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n105_), .c(x21), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n101_), .c(new_n63_), .O(z12));
  nand3  g066(.a(x23), .b(x18), .c(new_n47_), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n98_), .c(x18), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n83_), .O(new_n113_));
  nand2  g069(.a(x21), .b(x19), .O(new_n114_));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(x22), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n114_), .c(x23), .O(new_n117_));
  inv1   g073(.a(x23), .O(new_n118_));
  nand4  g074(.a(new_n106_), .b(new_n118_), .c(x22), .d(x21), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(new_n113_), .c(new_n65_), .O(z13));
  nand2  g078(.a(x24), .b(new_n47_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n98_), .c(x18), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n83_), .O(new_n125_));
  nand3  g081(.a(x26), .b(x25), .c(x16), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x23), .c(x22), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n114_), .c(x24), .O(new_n128_));
  nand3  g084(.a(x21), .b(x20), .c(x19), .O(new_n129_));
  inv1   g085(.a(x24), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x23), .c(x22), .O(new_n131_));
  oai21  g087(.a(new_n131_), .b(new_n129_), .c(new_n128_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(new_n125_), .c(new_n68_), .O(z14));
  nand3  g090(.a(x22), .b(x21), .c(x19), .O(new_n135_));
  inv1   g091(.a(x25), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x24), .c(x23), .O(new_n137_));
  nand3  g093(.a(x26), .b(x25), .c(x17), .O(new_n138_));
  oai21  g094(.a(new_n137_), .b(new_n135_), .c(new_n138_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x20), .O(new_n140_));
  nand3  g096(.a(x24), .b(x23), .c(x22), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n129_), .c(x25), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n70_), .c(new_n67_), .O(z15));
  inv1   g101(.a(x18), .O(new_n146_));
  nand3  g102(.a(x21), .b(x19), .c(new_n146_), .O(new_n147_));
  nand4  g103(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(x26), .O(new_n149_));
  nor2   g105(.a(new_n130_), .b(new_n118_), .O(new_n150_));
  nor2   g106(.a(x26), .b(new_n136_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n106_), .c(new_n150_), .d(new_n74_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(new_n72_), .O(z16));
endmodule


