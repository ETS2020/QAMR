// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_;
  nor2   g000(.a(x23), .b(x12), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  inv1   g003(.a(x09), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x08), .O(new_n49_));
  nand2  g005(.a(x20), .b(x19), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  inv1   g007(.a(x21), .O(new_n52_));
  nand2  g008(.a(x23), .b(x22), .O(new_n53_));
  nand3  g009(.a(x26), .b(x25), .c(x24), .O(new_n54_));
  nor3   g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand4  g011(.a(new_n55_), .b(new_n51_), .c(new_n49_), .d(new_n47_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n46_), .O(z00));
  inv1   g013(.a(x00), .O(new_n58_));
  inv1   g014(.a(x08), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(z01));
  nand2  g016(.a(x08), .b(x01), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(new_n46_), .O(z02));
  nor2   g018(.a(new_n45_), .b(new_n59_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(x02), .O(new_n64_));
  inv1   g020(.a(new_n64_), .O(z03));
  inv1   g021(.a(x03), .O(new_n66_));
  oai21  g022(.a(new_n59_), .b(new_n66_), .c(new_n46_), .O(z04));
  nand2  g023(.a(x08), .b(x04), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n46_), .O(z05));
  nand2  g025(.a(new_n63_), .b(x05), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n46_), .O(z06));
  nand2  g027(.a(new_n63_), .b(x06), .O(new_n72_));
  inv1   g028(.a(new_n72_), .O(z07));
  nand2  g029(.a(new_n63_), .b(x07), .O(new_n74_));
  nand2  g030(.a(new_n74_), .b(new_n46_), .O(z08));
  inv1   g031(.a(x19), .O(new_n76_));
  nand3  g032(.a(new_n49_), .b(new_n76_), .c(new_n47_), .O(new_n77_));
  oai21  g033(.a(new_n59_), .b(new_n58_), .c(new_n77_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n46_), .O(new_n79_));
  nand2  g035(.a(x20), .b(x11), .O(new_n80_));
  nor2   g036(.a(new_n80_), .b(x10), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(new_n55_), .c(new_n49_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n79_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand4  g040(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x19), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x20), .O(new_n87_));
  oai21  g043(.a(x20), .b(new_n76_), .c(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n61_), .c(new_n46_), .O(z10));
  nand3  g046(.a(x23), .b(x22), .c(x13), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n54_), .c(x20), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n76_), .c(x21), .O(new_n93_));
  nand3  g049(.a(new_n52_), .b(x20), .c(x19), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n64_), .c(new_n46_), .O(z11));
  inv1   g053(.a(x22), .O(new_n98_));
  nand3  g054(.a(x21), .b(x20), .c(x19), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand2  g056(.a(new_n98_), .b(x21), .O(new_n101_));
  oai22  g057(.a(new_n101_), .b(new_n50_), .c(new_n100_), .d(new_n98_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n103_));
  oai21  g059(.a(new_n59_), .b(new_n66_), .c(new_n103_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n46_), .O(new_n105_));
  nor2   g061(.a(new_n54_), .b(new_n53_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n49_), .c(x14), .d(new_n47_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(z12));
  inv1   g064(.a(x23), .O(new_n109_));
  nand3  g065(.a(new_n47_), .b(x09), .c(new_n59_), .O(new_n110_));
  nand3  g066(.a(new_n51_), .b(x22), .c(x21), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n110_), .c(x12), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n109_), .O(new_n113_));
  and2   g069(.a(x21), .b(x20), .O(new_n114_));
  nand4  g070(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n114_), .c(x22), .d(x19), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x23), .c(new_n47_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x09), .c(new_n59_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n113_), .c(new_n68_), .O(z13));
  nand3  g076(.a(x26), .b(x25), .c(x16), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n51_), .c(x22), .d(x21), .O(new_n122_));
  inv1   g078(.a(x24), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x22), .c(x21), .O(new_n124_));
  nor2   g080(.a(new_n124_), .b(new_n50_), .O(new_n125_));
  aoi21  g081(.a(new_n122_), .b(x24), .c(new_n125_), .O(new_n126_));
  nand3  g082(.a(x24), .b(new_n109_), .c(x12), .O(new_n127_));
  oai21  g083(.a(new_n126_), .b(new_n109_), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n70_), .O(z14));
  nand2  g086(.a(x26), .b(x17), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n100_), .c(x24), .d(x22), .O(new_n132_));
  inv1   g088(.a(x25), .O(new_n133_));
  nand3  g089(.a(new_n133_), .b(x24), .c(x22), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n99_), .O(new_n135_));
  aoi21  g091(.a(new_n132_), .b(x25), .c(new_n135_), .O(new_n136_));
  nand3  g092(.a(x25), .b(new_n109_), .c(x12), .O(new_n137_));
  oai21  g093(.a(new_n136_), .b(new_n109_), .c(new_n137_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n72_), .O(z15));
  inv1   g096(.a(x26), .O(new_n141_));
  nor2   g097(.a(new_n50_), .b(x18), .O(new_n142_));
  nand4  g098(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  aoi21  g100(.a(new_n144_), .b(new_n142_), .c(new_n141_), .O(new_n145_));
  nand4  g101(.a(new_n141_), .b(x25), .c(x24), .d(x22), .O(new_n146_));
  nor2   g102(.a(new_n146_), .b(new_n99_), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n145_), .c(x23), .O(new_n148_));
  nand3  g104(.a(x26), .b(new_n109_), .c(x12), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n47_), .c(x09), .d(new_n59_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n74_), .O(z16));
endmodule


