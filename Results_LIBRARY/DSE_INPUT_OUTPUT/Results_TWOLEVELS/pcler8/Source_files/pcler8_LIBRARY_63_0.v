// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_;
  inv1   g00(.a(x26), .O(new_n45_));
  inv1   g01(.a(x08), .O(new_n46_));
  inv1   g02(.a(x10), .O(new_n47_));
  nand4  g03(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g06(.a(new_n50_), .O(new_n51_));
  nand4  g07(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g08(.a(new_n52_), .b(new_n45_), .O(z00));
  nor2   g09(.a(x23), .b(x10), .O(new_n54_));
  inv1   g10(.a(new_n54_), .O(new_n55_));
  nand2  g11(.a(x08), .b(x00), .O(new_n56_));
  nand2  g12(.a(new_n56_), .b(new_n55_), .O(z01));
  inv1   g13(.a(x01), .O(new_n58_));
  nor2   g14(.a(new_n54_), .b(new_n46_), .O(new_n59_));
  inv1   g15(.a(new_n59_), .O(new_n60_));
  nor2   g16(.a(new_n60_), .b(new_n58_), .O(z02));
  nand2  g17(.a(new_n59_), .b(x02), .O(new_n62_));
  inv1   g18(.a(new_n62_), .O(z03));
  nand2  g19(.a(new_n59_), .b(x03), .O(new_n64_));
  inv1   g20(.a(new_n64_), .O(z04));
  inv1   g21(.a(x04), .O(new_n66_));
  oai21  g22(.a(new_n46_), .b(new_n66_), .c(new_n55_), .O(z05));
  inv1   g23(.a(x05), .O(new_n68_));
  nor2   g24(.a(new_n60_), .b(new_n68_), .O(z06));
  nand2  g25(.a(new_n59_), .b(x06), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(z07));
  inv1   g27(.a(x07), .O(new_n72_));
  oai21  g28(.a(new_n46_), .b(new_n72_), .c(new_n55_), .O(z08));
  inv1   g29(.a(x23), .O(new_n74_));
  nand3  g30(.a(x21), .b(x20), .c(x11), .O(new_n75_));
  nand4  g31(.a(x26), .b(x25), .c(x24), .d(x22), .O(new_n76_));
  oai22  g32(.a(new_n76_), .b(new_n75_), .c(new_n74_), .d(x19), .O(new_n77_));
  nand3  g33(.a(new_n77_), .b(x09), .c(new_n46_), .O(new_n78_));
  nand2  g34(.a(new_n78_), .b(x23), .O(new_n79_));
  nand2  g35(.a(new_n79_), .b(new_n47_), .O(new_n80_));
  nand2  g36(.a(new_n80_), .b(new_n56_), .O(z09));
  nand3  g37(.a(x22), .b(x21), .c(x12), .O(new_n82_));
  nand3  g38(.a(x26), .b(x25), .c(x24), .O(new_n83_));
  oai21  g39(.a(new_n83_), .b(new_n82_), .c(x19), .O(new_n84_));
  nand2  g40(.a(new_n84_), .b(x20), .O(new_n85_));
  inv1   g41(.a(x20), .O(new_n86_));
  nand2  g42(.a(new_n86_), .b(x19), .O(new_n87_));
  aoi21  g43(.a(new_n87_), .b(new_n85_), .c(new_n74_), .O(new_n88_));
  nand4  g44(.a(new_n88_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n89_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n89_), .O(z10));
  inv1   g46(.a(x19), .O(new_n91_));
  nand2  g47(.a(x22), .b(x13), .O(new_n92_));
  oai21  g48(.a(new_n92_), .b(new_n83_), .c(x20), .O(new_n93_));
  oai21  g49(.a(new_n93_), .b(new_n91_), .c(x21), .O(new_n94_));
  inv1   g50(.a(x21), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(x20), .c(x19), .O(new_n96_));
  aoi21  g52(.a(new_n96_), .b(new_n94_), .c(new_n74_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n62_), .O(z11));
  inv1   g55(.a(x09), .O(new_n100_));
  nand4  g56(.a(x26), .b(x25), .c(x24), .d(x14), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(x21), .c(x20), .d(x19), .O(new_n102_));
  nand2  g58(.a(new_n102_), .b(x22), .O(new_n103_));
  inv1   g59(.a(x22), .O(new_n104_));
  nand2  g60(.a(x20), .b(x19), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(x23), .c(new_n104_), .d(x21), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n103_), .c(new_n100_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n46_), .c(new_n74_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x10), .c(new_n64_), .O(z12));
  nor2   g66(.a(new_n104_), .b(new_n95_), .O(new_n111_));
  nand4  g67(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n112_));
  nand4  g68(.a(new_n112_), .b(new_n111_), .c(x20), .d(x19), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(x23), .c(new_n47_), .d(x09), .O(new_n114_));
  oai22  g70(.a(new_n114_), .b(x08), .c(new_n60_), .d(new_n66_), .O(z13));
  nand3  g71(.a(x26), .b(x25), .c(x16), .O(new_n116_));
  nand3  g72(.a(new_n116_), .b(x22), .c(x21), .O(new_n117_));
  oai21  g73(.a(new_n117_), .b(new_n105_), .c(x24), .O(new_n118_));
  inv1   g74(.a(x24), .O(new_n119_));
  nand4  g75(.a(new_n106_), .b(new_n119_), .c(x22), .d(x21), .O(new_n120_));
  aoi21  g76(.a(new_n120_), .b(new_n118_), .c(new_n74_), .O(new_n121_));
  nand4  g77(.a(new_n121_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n122_));
  oai21  g78(.a(new_n60_), .b(new_n68_), .c(new_n122_), .O(z14));
  nand3  g79(.a(x21), .b(x20), .c(x19), .O(new_n124_));
  nand2  g80(.a(x26), .b(x17), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(x24), .c(x22), .O(new_n126_));
  oai21  g82(.a(new_n126_), .b(new_n124_), .c(x25), .O(new_n127_));
  inv1   g83(.a(new_n124_), .O(new_n128_));
  nor2   g84(.a(x25), .b(new_n119_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n128_), .c(x22), .O(new_n130_));
  aoi21  g86(.a(new_n130_), .b(new_n127_), .c(new_n100_), .O(new_n131_));
  aoi21  g87(.a(new_n131_), .b(new_n46_), .c(new_n74_), .O(new_n132_));
  oai21  g88(.a(new_n132_), .b(x10), .c(new_n70_), .O(z15));
  inv1   g89(.a(x18), .O(new_n134_));
  nand3  g90(.a(x20), .b(x19), .c(new_n134_), .O(new_n135_));
  nand4  g91(.a(x25), .b(x24), .c(x22), .d(x21), .O(new_n136_));
  oai21  g92(.a(new_n136_), .b(new_n135_), .c(x26), .O(new_n137_));
  nor2   g93(.a(new_n119_), .b(new_n104_), .O(new_n138_));
  nand4  g94(.a(new_n128_), .b(new_n138_), .c(new_n45_), .d(x25), .O(new_n139_));
  aoi21  g95(.a(new_n139_), .b(new_n137_), .c(new_n74_), .O(new_n140_));
  nand4  g96(.a(new_n140_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n141_));
  oai21  g97(.a(new_n60_), .b(new_n72_), .c(new_n141_), .O(z16));
endmodule


