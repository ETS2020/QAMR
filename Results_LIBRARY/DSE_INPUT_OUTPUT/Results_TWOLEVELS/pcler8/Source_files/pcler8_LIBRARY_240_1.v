// Benchmark "FAU" written by ABC on Wed Aug 19 15:37:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_;
  inv1   g000(.a(x23), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  inv1   g005(.a(x21), .O(new_n50_));
  inv1   g006(.a(x22), .O(new_n51_));
  nor2   g007(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g008(.a(x26), .b(x25), .c(x24), .O(new_n53_));
  inv1   g009(.a(new_n53_), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n52_), .c(new_n49_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(x20), .c(new_n45_), .O(z00));
  nor2   g012(.a(new_n45_), .b(x20), .O(new_n57_));
  inv1   g013(.a(new_n57_), .O(new_n58_));
  nand2  g014(.a(x08), .b(x00), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(new_n58_), .O(z01));
  nor2   g016(.a(new_n57_), .b(new_n46_), .O(new_n61_));
  nand2  g017(.a(new_n61_), .b(x01), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(z02));
  inv1   g019(.a(x02), .O(new_n64_));
  inv1   g020(.a(new_n61_), .O(new_n65_));
  nor2   g021(.a(new_n65_), .b(new_n64_), .O(z03));
  nand2  g022(.a(new_n61_), .b(x03), .O(new_n67_));
  inv1   g023(.a(new_n67_), .O(z04));
  inv1   g024(.a(x04), .O(new_n69_));
  nor2   g025(.a(new_n65_), .b(new_n69_), .O(z05));
  nand2  g026(.a(x08), .b(x05), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n58_), .O(z06));
  nand2  g028(.a(new_n61_), .b(x06), .O(new_n73_));
  inv1   g029(.a(new_n73_), .O(z07));
  nand2  g030(.a(x08), .b(x07), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n58_), .O(z08));
  nand2  g032(.a(new_n52_), .b(x11), .O(new_n77_));
  nand4  g033(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n77_), .c(x19), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(new_n59_), .c(new_n58_), .O(z09));
  inv1   g037(.a(x20), .O(new_n82_));
  oai21  g038(.a(new_n49_), .b(x23), .c(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n52_), .b(x12), .O(new_n84_));
  oai22  g040(.a(new_n84_), .b(new_n78_), .c(new_n82_), .d(x19), .O(new_n85_));
  nand4  g041(.a(new_n85_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n83_), .c(new_n62_), .O(z10));
  nand3  g043(.a(x23), .b(x22), .c(x13), .O(new_n88_));
  oai21  g044(.a(new_n88_), .b(new_n53_), .c(x19), .O(new_n89_));
  inv1   g045(.a(x19), .O(new_n90_));
  nor2   g046(.a(x21), .b(new_n90_), .O(new_n91_));
  aoi21  g047(.a(new_n89_), .b(x21), .c(new_n91_), .O(new_n92_));
  nand3  g048(.a(new_n45_), .b(x21), .c(new_n82_), .O(new_n93_));
  oai21  g049(.a(new_n92_), .b(new_n82_), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n95_));
  oai21  g051(.a(new_n65_), .b(new_n64_), .c(new_n95_), .O(z11));
  oai21  g052(.a(new_n45_), .b(x20), .c(new_n50_), .O(new_n97_));
  nand2  g053(.a(x23), .b(x14), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n53_), .c(x19), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x20), .O(new_n100_));
  nand2  g056(.a(new_n45_), .b(new_n82_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n100_), .c(new_n97_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(x22), .O(new_n103_));
  nand4  g059(.a(new_n51_), .b(x21), .c(x20), .d(x19), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n67_), .O(z12));
  nand3  g063(.a(new_n45_), .b(x21), .c(x19), .O(new_n108_));
  oai21  g064(.a(new_n45_), .b(x21), .c(new_n108_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x22), .O(new_n110_));
  nand4  g066(.a(x26), .b(x25), .c(x24), .d(x15), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x22), .c(x19), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x23), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n110_), .c(new_n82_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n115_));
  oai21  g071(.a(new_n65_), .b(new_n69_), .c(new_n115_), .O(z13));
  nand3  g072(.a(new_n47_), .b(x09), .c(new_n46_), .O(new_n117_));
  inv1   g073(.a(x24), .O(new_n118_));
  nor2   g074(.a(new_n50_), .b(new_n90_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(x22), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n117_), .c(x20), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x23), .O(new_n122_));
  nand3  g078(.a(x26), .b(x25), .c(x16), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x23), .c(x22), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n119_), .c(new_n118_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n122_), .c(new_n71_), .O(z14));
  inv1   g084(.a(x25), .O(new_n129_));
  nand3  g085(.a(new_n52_), .b(new_n129_), .c(x24), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n48_), .c(x20), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x23), .O(new_n132_));
  nand2  g088(.a(new_n52_), .b(x19), .O(new_n133_));
  nand2  g089(.a(x26), .b(x17), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x24), .c(x23), .O(new_n135_));
  nor2   g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nor2   g092(.a(new_n136_), .b(new_n129_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n132_), .c(new_n73_), .O(z15));
  inv1   g095(.a(x26), .O(new_n140_));
  nand4  g096(.a(new_n52_), .b(new_n140_), .c(x25), .d(x24), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n48_), .c(x20), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x23), .O(new_n143_));
  nor3   g099(.a(new_n50_), .b(new_n90_), .c(x18), .O(new_n144_));
  nand4  g100(.a(x25), .b(x24), .c(x23), .d(x22), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n140_), .O(new_n147_));
  nand4  g103(.a(new_n147_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n143_), .c(new_n75_), .O(z16));
endmodule


