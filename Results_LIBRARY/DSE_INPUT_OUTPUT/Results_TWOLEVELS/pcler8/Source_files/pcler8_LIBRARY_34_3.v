// Benchmark "FAU" written by ABC on Wed Aug 19 15:36:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_;
  inv1   g000(.a(x26), .O(new_n45_));
  inv1   g001(.a(x08), .O(new_n46_));
  inv1   g002(.a(x10), .O(new_n47_));
  nand4  g003(.a(x19), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n48_));
  inv1   g004(.a(new_n48_), .O(new_n49_));
  nand4  g005(.a(new_n49_), .b(x22), .c(x21), .d(x20), .O(new_n50_));
  inv1   g006(.a(new_n50_), .O(new_n51_));
  nand4  g007(.a(new_n51_), .b(x25), .c(x24), .d(x23), .O(new_n52_));
  nor2   g008(.a(new_n52_), .b(new_n45_), .O(z00));
  inv1   g009(.a(x19), .O(new_n54_));
  nor2   g010(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  inv1   g011(.a(new_n55_), .O(new_n56_));
  nand2  g012(.a(x08), .b(x00), .O(new_n57_));
  nand2  g013(.a(new_n57_), .b(new_n56_), .O(z01));
  nor2   g014(.a(new_n55_), .b(new_n46_), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(z02));
  nand2  g017(.a(new_n59_), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n62_), .b(new_n56_), .O(z03));
  nand2  g019(.a(new_n59_), .b(x03), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n56_), .O(z04));
  nand2  g021(.a(new_n59_), .b(x04), .O(new_n66_));
  inv1   g022(.a(new_n66_), .O(z05));
  nand2  g023(.a(new_n59_), .b(x05), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n56_), .O(z06));
  nand2  g025(.a(x08), .b(x06), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n56_), .O(z07));
  nand2  g027(.a(x08), .b(x07), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n56_), .O(z08));
  and2   g029(.a(x19), .b(x11), .O(new_n74_));
  nand2  g030(.a(x21), .b(x20), .O(new_n75_));
  inv1   g031(.a(new_n75_), .O(new_n76_));
  and2   g032(.a(x23), .b(x22), .O(new_n77_));
  nand3  g033(.a(x26), .b(x25), .c(x24), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n77_), .c(new_n76_), .d(new_n74_), .O(new_n80_));
  oai21  g036(.a(x19), .b(x10), .c(new_n80_), .O(new_n81_));
  nand3  g037(.a(new_n81_), .b(x09), .c(new_n46_), .O(new_n82_));
  nand3  g038(.a(new_n82_), .b(new_n57_), .c(new_n56_), .O(z09));
  nand3  g039(.a(x22), .b(x21), .c(x12), .O(new_n84_));
  nand4  g040(.a(x26), .b(x25), .c(x24), .d(x23), .O(new_n85_));
  oai21  g041(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(x19), .O(new_n87_));
  nand3  g043(.a(x20), .b(new_n54_), .c(new_n47_), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(x09), .c(new_n46_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n60_), .c(new_n56_), .O(z10));
  inv1   g047(.a(x20), .O(new_n92_));
  nand3  g048(.a(x22), .b(x21), .c(x13), .O(new_n93_));
  oai22  g049(.a(new_n93_), .b(new_n85_), .c(x21), .d(new_n92_), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x19), .O(new_n95_));
  nand2  g051(.a(x20), .b(x19), .O(new_n96_));
  nand2  g052(.a(new_n96_), .b(x21), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n62_), .O(z11));
  inv1   g056(.a(x22), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(x21), .c(x20), .O(new_n102_));
  nand3  g058(.a(x23), .b(x22), .c(x14), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n78_), .c(new_n102_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(x19), .O(new_n105_));
  nand3  g061(.a(x21), .b(x20), .c(x19), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(x22), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n64_), .O(z12));
  nor2   g066(.a(x23), .b(new_n101_), .O(new_n111_));
  and2   g067(.a(x23), .b(x15), .O(new_n112_));
  aoi22  g068(.a(new_n112_), .b(new_n79_), .c(new_n111_), .d(new_n76_), .O(new_n113_));
  nand4  g069(.a(x22), .b(x21), .c(x20), .d(x19), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x23), .c(new_n47_), .O(new_n115_));
  oai21  g071(.a(new_n113_), .b(new_n54_), .c(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x09), .c(new_n46_), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n66_), .c(new_n56_), .O(z13));
  inv1   g074(.a(x24), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x23), .c(x22), .O(new_n120_));
  nand4  g076(.a(x26), .b(x25), .c(x24), .d(x16), .O(new_n121_));
  oai21  g077(.a(new_n120_), .b(new_n75_), .c(new_n121_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x19), .O(new_n123_));
  nand2  g079(.a(new_n77_), .b(x21), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n96_), .c(x24), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n68_), .O(z14));
  nand3  g084(.a(x22), .b(x21), .c(x20), .O(new_n129_));
  inv1   g085(.a(x25), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x24), .c(x23), .O(new_n131_));
  nand3  g087(.a(x26), .b(x25), .c(x17), .O(new_n132_));
  oai21  g088(.a(new_n131_), .b(new_n129_), .c(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x19), .O(new_n134_));
  inv1   g090(.a(new_n106_), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(x24), .c(x23), .d(x22), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x25), .c(new_n47_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n134_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x09), .c(new_n46_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n70_), .c(new_n56_), .O(z15));
  nand3  g096(.a(new_n76_), .b(x09), .c(new_n46_), .O(new_n141_));
  nand4  g097(.a(new_n77_), .b(new_n45_), .c(x25), .d(x24), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n141_), .c(new_n47_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x19), .O(new_n144_));
  inv1   g100(.a(x18), .O(new_n145_));
  nand3  g101(.a(new_n76_), .b(x19), .c(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n77_), .b(x25), .c(x24), .O(new_n147_));
  nor2   g103(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nor2   g104(.a(new_n148_), .b(new_n45_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n47_), .c(x09), .d(new_n46_), .O(new_n150_));
  nand3  g106(.a(new_n150_), .b(new_n144_), .c(new_n72_), .O(z16));
endmodule


