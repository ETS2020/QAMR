// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:18 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n21_, new_n23_, new_n28_,
    new_n29_, new_n30_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nand2  g03(.a(new_n16_), .b(new_n17_), .O(z3));
  nand2  g04(.a(x2), .b(new_n15_), .O(new_n19_));
  nand4  g05(.a(new_n19_), .b(z3), .c(new_n16_), .d(new_n15_), .O(z0));
  inv1   g06(.a(x1), .O(new_n21_));
  nand4  g07(.a(new_n16_), .b(x2), .c(new_n21_), .d(new_n15_), .O(z4));
  oai21  g08(.a(x1), .b(new_n15_), .c(x2), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n16_), .O(z5));
  nand4  g10(.a(new_n16_), .b(x2), .c(x1), .d(new_n15_), .O(z6));
  nand4  g11(.a(new_n16_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g12(.a(x3), .b(new_n17_), .c(new_n21_), .d(new_n15_), .O(z8));
  nand2  g13(.a(x3), .b(new_n15_), .O(new_n28_));
  nand2  g14(.a(x2), .b(new_n21_), .O(new_n29_));
  oai21  g15(.a(x3), .b(x2), .c(x1), .O(new_n30_));
  nand3  g16(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(z9));
  nand4  g17(.a(new_n19_), .b(z3), .c(new_n16_), .d(new_n15_), .O(z1));
  nand4  g18(.a(new_n19_), .b(z3), .c(new_n16_), .d(new_n15_), .O(z2));
endmodule


