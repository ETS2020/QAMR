// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:10 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n27_, new_n28_, new_n31_, new_n35_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x3), .O(new_n18_));
  nand4  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .d(new_n15_), .O(z0));
  oai21  g05(.a(x3), .b(x1), .c(x0), .O(new_n20_));
  oai21  g06(.a(x2), .b(new_n15_), .c(new_n18_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n20_), .O(z1));
  nor2   g08(.a(x1), .b(x0), .O(new_n23_));
  oai21  g09(.a(new_n23_), .b(x2), .c(new_n18_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n15_), .O(z2));
  nand4  g11(.a(new_n18_), .b(new_n17_), .c(x1), .d(x0), .O(z3));
  oai21  g12(.a(x3), .b(x2), .c(x0), .O(new_n27_));
  aoi21  g13(.a(x1), .b(new_n15_), .c(new_n17_), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(x3), .c(new_n27_), .O(z4));
  nand4  g15(.a(new_n18_), .b(x2), .c(new_n16_), .d(x0), .O(z5));
  oai21  g16(.a(new_n23_), .b(new_n17_), .c(new_n18_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n27_), .O(z6));
  nand4  g18(.a(new_n18_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand2  g19(.a(x3), .b(new_n15_), .O(z8));
  oai21  g20(.a(x2), .b(x1), .c(x0), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(x3), .O(z9));
endmodule


