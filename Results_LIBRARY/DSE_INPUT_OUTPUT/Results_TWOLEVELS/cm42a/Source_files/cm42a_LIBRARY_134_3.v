// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:50 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n26_, new_n28_, new_n29_, new_n31_, new_n32_, new_n37_;
  inv1   g00(.a(x1), .O(new_n15_));
  oai21  g01(.a(x2), .b(x0), .c(new_n15_), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  oai21  g03(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n16_), .O(z0));
  inv1   g05(.a(x3), .O(new_n20_));
  nand4  g06(.a(new_n20_), .b(new_n17_), .c(new_n15_), .d(x0), .O(z1));
  inv1   g07(.a(x0), .O(new_n22_));
  oai21  g08(.a(x2), .b(new_n22_), .c(new_n15_), .O(new_n23_));
  oai21  g09(.a(x3), .b(x0), .c(new_n17_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n23_), .O(z2));
  oai21  g11(.a(x3), .b(new_n22_), .c(new_n17_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n16_), .O(z3));
  oai21  g13(.a(x3), .b(x0), .c(new_n15_), .O(new_n28_));
  oai21  g14(.a(x1), .b(new_n22_), .c(new_n17_), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n28_), .O(z4));
  oai21  g16(.a(x3), .b(new_n22_), .c(new_n15_), .O(new_n31_));
  oai21  g17(.a(x1), .b(x0), .c(new_n17_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n31_), .O(z5));
  nand4  g19(.a(x3), .b(new_n17_), .c(new_n15_), .d(new_n22_), .O(z8));
  oai21  g20(.a(new_n20_), .b(x1), .c(new_n17_), .O(new_n37_));
  nand2  g21(.a(new_n37_), .b(new_n23_), .O(z9));
  one    g22(.O(z6));
  one    g23(.O(z7));
endmodule


