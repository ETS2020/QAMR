// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:17 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n29_, new_n31_, new_n32_, new_n33_;
  inv1   g00(.a(x0), .O(new_n17_));
  inv1   g01(.a(x2), .O(new_n18_));
  inv1   g02(.a(x3), .O(new_n19_));
  nand4  g03(.a(new_n19_), .b(new_n18_), .c(x1), .d(new_n17_), .O(z2));
  oai21  g04(.a(x3), .b(new_n17_), .c(x1), .O(new_n21_));
  inv1   g05(.a(x1), .O(new_n22_));
  oai21  g06(.a(new_n22_), .b(x0), .c(x2), .O(new_n23_));
  nand2  g07(.a(new_n23_), .b(new_n21_), .O(z3));
  oai21  g08(.a(new_n18_), .b(new_n17_), .c(x1), .O(new_n25_));
  oai21  g09(.a(x3), .b(x0), .c(x2), .O(new_n26_));
  nand2  g10(.a(new_n26_), .b(new_n25_), .O(z4));
  nand4  g11(.a(new_n19_), .b(x2), .c(new_n22_), .d(x0), .O(z5));
  oai21  g12(.a(x3), .b(new_n18_), .c(x1), .O(new_n29_));
  nand2  g13(.a(new_n29_), .b(new_n23_), .O(z6));
  nand3  g14(.a(new_n19_), .b(x2), .c(x0), .O(new_n31_));
  nand2  g15(.a(new_n31_), .b(x1), .O(new_n32_));
  nand2  g16(.a(x2), .b(new_n22_), .O(new_n33_));
  nand2  g17(.a(new_n33_), .b(new_n32_), .O(z7));
  one    g18(.O(z0));
  one    g19(.O(z1));
  one    g20(.O(z8));
  one    g21(.O(z9));
endmodule


