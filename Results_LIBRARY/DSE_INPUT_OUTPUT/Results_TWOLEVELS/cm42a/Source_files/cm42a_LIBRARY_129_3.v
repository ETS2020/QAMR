// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:49 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n23_,
    new_n25_, new_n27_, new_n29_, new_n30_, new_n34_, new_n35_, new_n37_,
    new_n38_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  oai21  g02(.a(x3), .b(x0), .c(new_n16_), .O(new_n17_));
  nand2  g03(.a(x2), .b(new_n15_), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x0), .O(new_n20_));
  oai21  g06(.a(x3), .b(new_n20_), .c(new_n16_), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n18_), .c(new_n15_), .O(z1));
  oai21  g08(.a(x2), .b(new_n20_), .c(new_n15_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n17_), .O(z2));
  oai21  g10(.a(x2), .b(x0), .c(new_n15_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n21_), .O(z3));
  inv1   g12(.a(x3), .O(new_n27_));
  nand4  g13(.a(new_n27_), .b(x2), .c(new_n15_), .d(new_n20_), .O(z4));
  oai21  g14(.a(x3), .b(new_n20_), .c(new_n15_), .O(new_n29_));
  oai21  g15(.a(x1), .b(x0), .c(new_n16_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n29_), .O(z5));
  nand2  g17(.a(x2), .b(x1), .O(z7));
  nand3  g18(.a(x3), .b(new_n15_), .c(new_n20_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n18_), .O(z8));
  nand3  g21(.a(x3), .b(new_n15_), .c(x0), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n18_), .O(z9));
  one    g24(.O(z6));
endmodule


