// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:10 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_;
  inv1   g00(.a(x0), .O(new_n15_));
  nor2   g01(.a(x3), .b(x1), .O(new_n16_));
  inv1   g02(.a(x1), .O(new_n17_));
  nand2  g03(.a(x2), .b(new_n17_), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n16_), .c(new_n15_), .O(z0));
  oai21  g05(.a(x2), .b(new_n15_), .c(x1), .O(new_n20_));
  oai21  g06(.a(x3), .b(new_n17_), .c(new_n15_), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(z1));
  inv1   g08(.a(x2), .O(new_n23_));
  inv1   g09(.a(x3), .O(new_n24_));
  nand4  g10(.a(new_n24_), .b(new_n23_), .c(x1), .d(new_n15_), .O(z2));
  nand3  g11(.a(new_n16_), .b(x2), .c(new_n15_), .O(z4));
  nand3  g12(.a(new_n16_), .b(x2), .c(x0), .O(z5));
  nand4  g13(.a(new_n24_), .b(x2), .c(x1), .d(new_n15_), .O(z6));
  oai21  g14(.a(x3), .b(x1), .c(new_n15_), .O(new_n30_));
  nand2  g15(.a(x3), .b(x2), .O(new_n31_));
  nor2   g16(.a(x3), .b(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(new_n17_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(z7));
  oai21  g19(.a(new_n24_), .b(x1), .c(new_n15_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n23_), .O(z8));
  one    g21(.O(z3));
  one    g22(.O(z9));
endmodule


