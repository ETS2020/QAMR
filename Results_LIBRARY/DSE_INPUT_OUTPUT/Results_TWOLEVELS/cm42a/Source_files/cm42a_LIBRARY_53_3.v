// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:30 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n21_, new_n22_, new_n25_,
    new_n28_, new_n29_, new_n31_;
  nor3   g00(.a(x2), .b(x1), .c(x0), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  nand2  g02(.a(x3), .b(new_n16_), .O(new_n17_));
  oai21  g03(.a(new_n15_), .b(x3), .c(new_n17_), .O(z0));
  nor2   g04(.a(x2), .b(x1), .O(new_n19_));
  oai21  g05(.a(new_n19_), .b(x3), .c(x0), .O(z1));
  inv1   g06(.a(x2), .O(new_n21_));
  inv1   g07(.a(x3), .O(new_n22_));
  nand4  g08(.a(new_n22_), .b(new_n21_), .c(x1), .d(new_n16_), .O(z2));
  nand4  g09(.a(new_n22_), .b(new_n21_), .c(x1), .d(x0), .O(z3));
  inv1   g10(.a(x1), .O(new_n25_));
  nand4  g11(.a(new_n22_), .b(x2), .c(new_n25_), .d(new_n16_), .O(z4));
  nand4  g12(.a(new_n22_), .b(x2), .c(new_n25_), .d(x0), .O(z5));
  nand2  g13(.a(x2), .b(x1), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(x0), .c(new_n22_), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n17_), .O(z6));
  nand2  g16(.a(new_n28_), .b(new_n22_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(x0), .O(z7));
  nand4  g18(.a(x3), .b(new_n21_), .c(new_n25_), .d(new_n16_), .O(z8));
  nand2  g19(.a(x3), .b(x0), .O(z9));
endmodule


