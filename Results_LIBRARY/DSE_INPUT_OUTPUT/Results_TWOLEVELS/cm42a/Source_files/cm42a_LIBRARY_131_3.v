// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:49 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n20_, new_n21_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n40_, new_n41_;
  nor2   g00(.a(x3), .b(x1), .O(new_n15_));
  nor2   g01(.a(x2), .b(x0), .O(new_n16_));
  oai22  g02(.a(new_n16_), .b(x3), .c(new_n15_), .d(x2), .O(z0));
  inv1   g03(.a(x2), .O(new_n18_));
  nand3  g04(.a(new_n15_), .b(new_n18_), .c(x0), .O(z1));
  inv1   g05(.a(x0), .O(new_n20_));
  inv1   g06(.a(x3), .O(new_n21_));
  nand4  g07(.a(new_n21_), .b(new_n18_), .c(x1), .d(new_n20_), .O(z2));
  nand4  g08(.a(new_n21_), .b(new_n18_), .c(x1), .d(x0), .O(z3));
  nand2  g09(.a(x3), .b(x2), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(x0), .O(new_n25_));
  nand2  g11(.a(new_n18_), .b(new_n20_), .O(new_n26_));
  nand2  g12(.a(new_n21_), .b(x1), .O(new_n27_));
  nand3  g13(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(z4));
  nand2  g14(.a(x3), .b(x2), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  nand2  g16(.a(new_n18_), .b(x0), .O(new_n31_));
  nand3  g17(.a(new_n31_), .b(new_n30_), .c(new_n27_), .O(z5));
  inv1   g18(.a(x1), .O(new_n33_));
  oai21  g19(.a(new_n33_), .b(x0), .c(new_n21_), .O(new_n34_));
  oai21  g20(.a(x3), .b(new_n20_), .c(new_n18_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n34_), .O(z6));
  inv1   g22(.a(new_n15_), .O(new_n37_));
  nand3  g23(.a(new_n31_), .b(new_n30_), .c(new_n37_), .O(z7));
  nand4  g24(.a(x3), .b(new_n18_), .c(new_n33_), .d(new_n20_), .O(z8));
  nand2  g25(.a(new_n21_), .b(x0), .O(new_n40_));
  nand2  g26(.a(new_n18_), .b(x1), .O(new_n41_));
  nand3  g27(.a(new_n41_), .b(new_n30_), .c(new_n40_), .O(z9));
endmodule


