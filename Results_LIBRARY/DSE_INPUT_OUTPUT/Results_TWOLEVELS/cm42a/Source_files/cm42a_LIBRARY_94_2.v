// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:40 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n28_, new_n32_;
  inv1   g00(.a(x0), .O(new_n15_));
  oai21  g01(.a(x3), .b(x2), .c(new_n15_), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  oai21  g03(.a(new_n17_), .b(x0), .c(x1), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n16_), .O(z0));
  inv1   g05(.a(x1), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(x0), .O(z1));
  inv1   g07(.a(x3), .O(new_n22_));
  nand3  g08(.a(new_n22_), .b(new_n17_), .c(x1), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n15_), .O(new_n24_));
  nand2  g10(.a(x1), .b(x0), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n24_), .O(z2));
  nand4  g12(.a(new_n22_), .b(new_n17_), .c(x1), .d(x0), .O(z3));
  aoi21  g13(.a(new_n17_), .b(new_n20_), .c(x3), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(x0), .c(new_n20_), .O(z4));
  nand4  g15(.a(new_n22_), .b(x2), .c(x1), .d(new_n15_), .O(z6));
  nand4  g16(.a(new_n22_), .b(x2), .c(x1), .d(x0), .O(z7));
  aoi21  g17(.a(new_n22_), .b(new_n20_), .c(x2), .O(new_n32_));
  oai21  g18(.a(new_n32_), .b(x0), .c(new_n18_), .O(z8));
  nand2  g19(.a(new_n20_), .b(x0), .O(z5));
  nand2  g20(.a(new_n20_), .b(x0), .O(z9));
endmodule


