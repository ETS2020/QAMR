// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:11 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n31_;
  inv1   g00(.a(x2), .O(new_n15_));
  oai21  g01(.a(x3), .b(x1), .c(new_n15_), .O(new_n16_));
  inv1   g02(.a(x3), .O(new_n17_));
  oai21  g03(.a(x2), .b(x0), .c(new_n17_), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n16_), .O(z0));
  nor2   g05(.a(x1), .b(x0), .O(new_n20_));
  oai21  g06(.a(new_n20_), .b(x2), .c(new_n17_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n16_), .O(z1));
  inv1   g08(.a(x1), .O(new_n23_));
  oai21  g09(.a(new_n23_), .b(x0), .c(new_n17_), .O(new_n24_));
  nand3  g10(.a(new_n24_), .b(new_n17_), .c(new_n15_), .O(z2));
  nand4  g11(.a(new_n17_), .b(new_n15_), .c(x1), .d(x0), .O(z3));
  oai21  g12(.a(new_n20_), .b(x3), .c(x2), .O(z4));
  nand4  g13(.a(new_n17_), .b(x2), .c(new_n23_), .d(x0), .O(z5));
  nand2  g14(.a(new_n24_), .b(x2), .O(z6));
  nand4  g15(.a(new_n17_), .b(x2), .c(x1), .d(x0), .O(z7));
  inv1   g16(.a(x0), .O(new_n31_));
  nand4  g17(.a(x3), .b(new_n15_), .c(new_n23_), .d(new_n31_), .O(z8));
  nand4  g18(.a(x3), .b(new_n15_), .c(new_n23_), .d(x0), .O(z9));
endmodule


