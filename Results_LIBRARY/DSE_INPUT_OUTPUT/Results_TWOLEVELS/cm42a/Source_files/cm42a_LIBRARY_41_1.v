// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:27 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n31_, new_n35_, new_n37_,
    new_n38_;
  inv1   g00(.a(x1), .O(new_n15_));
  oai21  g01(.a(x2), .b(new_n15_), .c(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x3), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n15_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n16_), .O(z0));
  nand3  g07(.a(new_n18_), .b(new_n15_), .c(x0), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n17_), .O(new_n23_));
  nand2  g09(.a(x2), .b(x0), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n23_), .O(z1));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n26_));
  oai21  g12(.a(x3), .b(new_n15_), .c(new_n17_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(new_n26_), .O(z2));
  nand4  g14(.a(new_n18_), .b(new_n17_), .c(x1), .d(x0), .O(z3));
  nand2  g15(.a(new_n19_), .b(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(z5));
  nand4  g17(.a(new_n18_), .b(x2), .c(x1), .d(x0), .O(z7));
  oai21  g18(.a(new_n18_), .b(x1), .c(new_n17_), .O(new_n35_));
  nand2  g19(.a(new_n35_), .b(new_n16_), .O(z8));
  nand3  g20(.a(x3), .b(new_n15_), .c(x0), .O(new_n37_));
  nand2  g21(.a(new_n37_), .b(new_n17_), .O(new_n38_));
  nand2  g22(.a(new_n38_), .b(new_n24_), .O(z9));
  one    g23(.O(z4));
  one    g24(.O(z6));
endmodule


