// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:17 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n25_, new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_;
  inv1   g00(.a(x1), .O(new_n15_));
  nor2   g01(.a(x3), .b(new_n15_), .O(new_n16_));
  nand3  g02(.a(new_n16_), .b(new_n15_), .c(x0), .O(z0));
  inv1   g03(.a(x2), .O(new_n18_));
  inv1   g04(.a(x3), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n15_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(x0), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n18_), .O(z1));
  inv1   g08(.a(x0), .O(new_n23_));
  nand2  g09(.a(new_n18_), .b(new_n23_), .O(z2));
  oai21  g10(.a(x3), .b(new_n15_), .c(x0), .O(new_n25_));
  oai21  g11(.a(x1), .b(new_n23_), .c(x2), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n25_), .O(z3));
  nand2  g13(.a(new_n20_), .b(x2), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n23_), .O(z4));
  nand2  g15(.a(new_n18_), .b(x0), .O(new_n30_));
  nand3  g16(.a(new_n19_), .b(new_n15_), .c(x0), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(x2), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n30_), .O(z5));
  oai21  g19(.a(new_n18_), .b(x1), .c(x0), .O(new_n34_));
  oai21  g20(.a(x3), .b(new_n15_), .c(x2), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n34_), .O(z6));
  nand3  g22(.a(new_n35_), .b(new_n30_), .c(x0), .O(z7));
  nand4  g23(.a(x3), .b(new_n18_), .c(new_n15_), .d(x0), .O(z9));
  nand3  g24(.a(new_n16_), .b(new_n15_), .c(x0), .O(z8));
endmodule


