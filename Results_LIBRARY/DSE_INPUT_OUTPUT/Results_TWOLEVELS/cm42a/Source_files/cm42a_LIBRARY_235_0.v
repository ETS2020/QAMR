// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:14 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n25_, new_n26_, new_n27_, new_n29_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  oai21  g03(.a(x3), .b(x0), .c(x1), .O(new_n18_));
  inv1   g04(.a(x0), .O(new_n19_));
  oai21  g05(.a(new_n15_), .b(new_n19_), .c(x2), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n18_), .O(z2));
  oai21  g07(.a(x3), .b(new_n19_), .c(x1), .O(new_n22_));
  oai21  g08(.a(new_n15_), .b(x0), .c(x2), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n22_), .O(z3));
  nand2  g10(.a(x1), .b(new_n19_), .O(new_n25_));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n26_));
  nand2  g12(.a(x3), .b(x2), .O(new_n27_));
  nand3  g13(.a(new_n27_), .b(new_n26_), .c(new_n25_), .O(z4));
  inv1   g14(.a(x3), .O(new_n29_));
  nand4  g15(.a(new_n29_), .b(x2), .c(new_n15_), .d(x0), .O(z5));
  nand3  g16(.a(new_n18_), .b(x2), .c(x1), .O(z6));
  nand4  g17(.a(new_n29_), .b(x2), .c(x1), .d(x0), .O(z7));
  one    g18(.O(z9));
  nand2  g19(.a(new_n16_), .b(new_n15_), .O(z1));
  nand2  g20(.a(new_n16_), .b(new_n15_), .O(z8));
endmodule


