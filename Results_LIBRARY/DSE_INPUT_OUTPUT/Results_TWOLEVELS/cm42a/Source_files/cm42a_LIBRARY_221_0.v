// Benchmark "FAU" written by ABC on Wed Aug 19 17:23:11 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n26_, new_n28_, new_n29_, new_n32_, new_n33_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  oai21  g03(.a(x2), .b(x1), .c(x0), .O(new_n18_));
  inv1   g04(.a(x0), .O(new_n19_));
  nand2  g05(.a(x2), .b(new_n19_), .O(new_n20_));
  nand2  g06(.a(x3), .b(x1), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(z2));
  oai21  g08(.a(x2), .b(x1), .c(new_n19_), .O(new_n23_));
  aoi22  g09(.a(x3), .b(x1), .c(x2), .d(x0), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n23_), .O(z3));
  inv1   g11(.a(x3), .O(new_n26_));
  nand4  g12(.a(new_n26_), .b(x2), .c(new_n15_), .d(new_n19_), .O(z4));
  oai21  g13(.a(new_n16_), .b(x0), .c(x1), .O(new_n28_));
  oai21  g14(.a(x3), .b(new_n19_), .c(x2), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n28_), .O(z5));
  nand4  g16(.a(new_n26_), .b(x2), .c(x1), .d(new_n19_), .O(z6));
  oai21  g17(.a(x3), .b(new_n16_), .c(x1), .O(new_n32_));
  oai21  g18(.a(new_n15_), .b(new_n19_), .c(x2), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n32_), .O(z7));
  nand2  g20(.a(new_n16_), .b(new_n15_), .O(z1));
  nand2  g21(.a(new_n16_), .b(new_n15_), .O(z8));
  nand2  g22(.a(new_n16_), .b(new_n15_), .O(z9));
endmodule


