// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:25 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_,
    new_n27_;
  oai21  g00(.a(x2), .b(x0), .c(x1), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  oai21  g02(.a(x3), .b(new_n16_), .c(x0), .O(new_n17_));
  nand2  g03(.a(x2), .b(new_n16_), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z0));
  inv1   g05(.a(x0), .O(new_n20_));
  inv1   g06(.a(x2), .O(new_n21_));
  nor2   g07(.a(x3), .b(x1), .O(new_n22_));
  oai21  g08(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(z1));
  nand2  g09(.a(new_n17_), .b(new_n21_), .O(z3));
  oai21  g10(.a(new_n22_), .b(new_n21_), .c(new_n20_), .O(z4));
  inv1   g11(.a(x3), .O(new_n27_));
  nand4  g12(.a(new_n27_), .b(x2), .c(new_n16_), .d(x0), .O(z5));
  nand4  g13(.a(new_n27_), .b(x2), .c(x1), .d(new_n20_), .O(z6));
  nand4  g14(.a(new_n27_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g15(.a(x3), .b(new_n21_), .c(new_n16_), .d(x0), .O(z9));
  one    g16(.O(z2));
  nand3  g17(.a(new_n18_), .b(new_n17_), .c(new_n15_), .O(z8));
endmodule


