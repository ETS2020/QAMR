// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:28 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n28_, new_n29_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  nor2   g02(.a(x3), .b(x1), .O(new_n17_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(new_n15_), .O(z0));
  oai21  g04(.a(x3), .b(x2), .c(x0), .O(new_n19_));
  oai21  g05(.a(x1), .b(new_n15_), .c(new_n16_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n19_), .O(z1));
  inv1   g07(.a(x1), .O(new_n22_));
  nor2   g08(.a(x3), .b(new_n22_), .O(new_n23_));
  nand3  g09(.a(new_n23_), .b(new_n16_), .c(new_n15_), .O(z2));
  nand3  g10(.a(new_n23_), .b(new_n16_), .c(x0), .O(z3));
  nand2  g11(.a(x2), .b(new_n15_), .O(z4));
  nand3  g12(.a(new_n17_), .b(x2), .c(x0), .O(z5));
  inv1   g13(.a(x3), .O(new_n28_));
  nor2   g14(.a(new_n28_), .b(x1), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n28_), .O(z6));
  nand3  g16(.a(new_n23_), .b(x2), .c(x0), .O(z7));
  nand4  g17(.a(x3), .b(new_n16_), .c(new_n22_), .d(new_n15_), .O(z8));
  nand3  g18(.a(new_n29_), .b(new_n16_), .c(x0), .O(z9));
endmodule


