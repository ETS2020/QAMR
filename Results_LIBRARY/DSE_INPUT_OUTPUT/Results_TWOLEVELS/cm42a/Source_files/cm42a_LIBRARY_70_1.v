// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:34 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n21_, new_n23_, new_n24_,
    new_n27_, new_n30_, new_n31_, new_n33_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x3), .O(new_n18_));
  nand4  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .d(new_n15_), .O(z0));
  nand4  g05(.a(new_n18_), .b(new_n17_), .c(new_n16_), .d(x0), .O(z1));
  oai21  g06(.a(x3), .b(new_n16_), .c(new_n17_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n15_), .O(z2));
  oai21  g08(.a(x3), .b(new_n15_), .c(new_n17_), .O(new_n23_));
  oai21  g09(.a(x2), .b(new_n16_), .c(x0), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n23_), .O(z3));
  oai21  g11(.a(x3), .b(x1), .c(x0), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x2), .O(z5));
  oai21  g13(.a(x1), .b(new_n15_), .c(new_n17_), .O(new_n30_));
  oai21  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n31_));
  nand2  g15(.a(new_n31_), .b(new_n30_), .O(z7));
  oai21  g16(.a(new_n18_), .b(x1), .c(new_n17_), .O(new_n33_));
  nand2  g17(.a(new_n33_), .b(new_n24_), .O(z8));
  nand4  g18(.a(x3), .b(new_n17_), .c(new_n16_), .d(x0), .O(z9));
  one    g19(.O(z4));
  one    g20(.O(z6));
endmodule


