// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:30 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n20_, new_n22_, new_n23_,
    new_n25_, new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x3), .O(new_n18_));
  nand4  g04(.a(new_n18_), .b(new_n17_), .c(new_n16_), .d(new_n15_), .O(z0));
  nor2   g05(.a(x2), .b(x1), .O(new_n20_));
  oai21  g06(.a(new_n20_), .b(x3), .c(x0), .O(z1));
  oai21  g07(.a(x3), .b(x2), .c(new_n15_), .O(new_n22_));
  oai21  g08(.a(new_n16_), .b(x0), .c(new_n18_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n22_), .O(z2));
  nand2  g10(.a(x2), .b(x0), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(x1), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n18_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(x0), .O(z3));
  nand4  g14(.a(new_n18_), .b(x2), .c(new_n16_), .d(new_n15_), .O(z4));
  nand2  g15(.a(x1), .b(x0), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(x2), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n18_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(x0), .O(z5));
  nand3  g19(.a(x2), .b(x1), .c(new_n15_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  nand2  g21(.a(x3), .b(new_n15_), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(new_n35_), .O(z6));
  oai21  g23(.a(x3), .b(x1), .c(new_n15_), .O(new_n38_));
  oai21  g24(.a(new_n17_), .b(new_n16_), .c(new_n18_), .O(new_n39_));
  nand2  g25(.a(new_n39_), .b(new_n38_), .O(z7));
  oai21  g26(.a(x2), .b(x1), .c(new_n15_), .O(new_n41_));
  nand2  g27(.a(new_n41_), .b(new_n23_), .O(z8));
  nand2  g28(.a(new_n23_), .b(x0), .O(z9));
endmodule


