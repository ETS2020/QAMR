// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:59 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n21_, new_n22_, new_n24_,
    new_n26_, new_n30_, new_n31_, new_n32_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x1), .O(new_n18_));
  inv1   g04(.a(x2), .O(new_n19_));
  nand4  g05(.a(new_n16_), .b(new_n19_), .c(new_n18_), .d(x0), .O(z1));
  nor2   g06(.a(x3), .b(x1), .O(new_n21_));
  oai21  g07(.a(new_n21_), .b(x2), .c(x0), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n16_), .O(z3));
  nor2   g09(.a(x3), .b(new_n15_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n15_), .O(z4));
  oai21  g11(.a(new_n19_), .b(x1), .c(x0), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n16_), .O(z5));
  nand4  g13(.a(new_n16_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g14(.a(x3), .b(new_n19_), .c(new_n18_), .d(new_n15_), .O(z8));
  nand3  g15(.a(x3), .b(new_n19_), .c(new_n18_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(x0), .O(new_n31_));
  nand2  g17(.a(x3), .b(new_n15_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n31_), .O(z9));
  nand2  g19(.a(new_n16_), .b(new_n15_), .O(z2));
  nand2  g20(.a(new_n24_), .b(new_n15_), .O(z6));
endmodule


