// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:36 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n25_, new_n27_, new_n28_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x3), .O(new_n16_));
  oai21  g02(.a(new_n16_), .b(new_n15_), .c(x0), .O(new_n17_));
  oai21  g03(.a(x3), .b(x2), .c(new_n15_), .O(new_n18_));
  inv1   g04(.a(x0), .O(new_n19_));
  nand3  g05(.a(new_n16_), .b(x1), .c(new_n19_), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n18_), .c(new_n17_), .O(z0));
  inv1   g07(.a(x2), .O(new_n22_));
  nand4  g08(.a(new_n16_), .b(new_n22_), .c(new_n15_), .d(x0), .O(z1));
  nand4  g09(.a(new_n16_), .b(new_n22_), .c(x1), .d(new_n19_), .O(z2));
  oai21  g10(.a(x2), .b(new_n19_), .c(new_n16_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(x1), .O(z3));
  oai21  g12(.a(x3), .b(x0), .c(new_n15_), .O(new_n27_));
  oai21  g13(.a(new_n22_), .b(x1), .c(new_n16_), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(new_n27_), .O(z4));
  nand4  g15(.a(new_n16_), .b(x2), .c(new_n15_), .d(x0), .O(z5));
  nand4  g16(.a(new_n16_), .b(x2), .c(x1), .d(new_n19_), .O(z6));
  nand4  g17(.a(new_n16_), .b(x2), .c(x1), .d(x0), .O(z7));
  nand4  g18(.a(x3), .b(new_n22_), .c(new_n15_), .d(new_n19_), .O(z8));
  nand4  g19(.a(x3), .b(new_n22_), .c(new_n15_), .d(x0), .O(z9));
endmodule


