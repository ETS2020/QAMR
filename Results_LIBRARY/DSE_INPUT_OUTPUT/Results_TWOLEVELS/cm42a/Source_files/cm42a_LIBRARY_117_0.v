// Benchmark "FAU" written by ABC on Wed Aug 19 17:22:45 2020

module FAU ( 
    x0, x1, x2, x3,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n26_, new_n27_,
    new_n28_, new_n31_, new_n33_;
  inv1   g00(.a(x0), .O(new_n17_));
  inv1   g01(.a(x2), .O(new_n18_));
  inv1   g02(.a(x3), .O(new_n19_));
  nand4  g03(.a(new_n19_), .b(new_n18_), .c(x1), .d(new_n17_), .O(z2));
  oai21  g04(.a(x3), .b(x1), .c(new_n17_), .O(new_n21_));
  aoi22  g05(.a(x3), .b(x0), .c(x2), .d(x1), .O(new_n22_));
  nand2  g06(.a(new_n22_), .b(new_n21_), .O(z3));
  oai21  g07(.a(x3), .b(x1), .c(x0), .O(new_n26_));
  nand2  g08(.a(x3), .b(new_n17_), .O(new_n27_));
  nand2  g09(.a(new_n18_), .b(x1), .O(new_n28_));
  nand3  g10(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(z6));
  nand4  g11(.a(new_n19_), .b(x2), .c(x1), .d(x0), .O(z7));
  inv1   g12(.a(x1), .O(new_n31_));
  nand4  g13(.a(x3), .b(new_n18_), .c(new_n31_), .d(new_n17_), .O(z8));
  aoi22  g14(.a(x3), .b(x2), .c(x1), .d(x0), .O(new_n33_));
  nand2  g15(.a(new_n33_), .b(new_n21_), .O(z9));
  one    g16(.O(z0));
  one    g17(.O(z1));
  one    g18(.O(z4));
  one    g19(.O(z5));
endmodule


