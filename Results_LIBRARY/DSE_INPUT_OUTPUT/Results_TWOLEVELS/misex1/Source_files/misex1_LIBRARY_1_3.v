// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x4), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(x4), .b(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nand4  g07(.a(x3), .b(x2), .c(x1), .d(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  inv1   g09(.a(x3), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n16_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x0), .O(new_n29_));
  oai21  g14(.a(x5), .b(x3), .c(x2), .O(new_n30_));
  nor2   g15(.a(new_n30_), .b(x1), .O(new_n31_));
  aoi21  g16(.a(new_n16_), .b(x1), .c(x6), .O(new_n32_));
  oai22  g17(.a(new_n32_), .b(x3), .c(new_n18_), .d(new_n17_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n26_), .c(new_n31_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x0), .c(new_n29_), .O(z1));
  nor2   g20(.a(x5), .b(new_n26_), .O(new_n36_));
  nor2   g21(.a(x6), .b(x2), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n36_), .c(new_n25_), .O(new_n38_));
  oai21  g23(.a(new_n16_), .b(x2), .c(new_n25_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  oai21  g25(.a(new_n38_), .b(x1), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n22_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n29_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n26_), .c(x1), .O(new_n45_));
  nor2   g30(.a(x5), .b(x3), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(x2), .c(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n22_), .O(new_n49_));
  nand4  g34(.a(new_n18_), .b(x2), .c(new_n17_), .d(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z3));
  oai21  g36(.a(x2), .b(new_n17_), .c(new_n22_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x4), .O(new_n53_));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n16_), .c(x0), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  aoi21  g41(.a(new_n38_), .b(new_n30_), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n17_), .O(new_n58_));
  oai21  g43(.a(x4), .b(x2), .c(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(x1), .O(new_n60_));
  nand3  g45(.a(x6), .b(new_n25_), .c(new_n26_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n22_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n58_), .c(new_n53_), .O(z4));
  nor2   g49(.a(new_n30_), .b(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n56_), .c(new_n17_), .O(new_n66_));
  inv1   g51(.a(new_n61_), .O(new_n67_));
  aoi21  g52(.a(new_n25_), .b(x2), .c(new_n17_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n22_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand3  g55(.a(new_n46_), .b(x2), .c(new_n22_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n17_), .O(new_n73_));
  oai21  g58(.a(new_n60_), .b(x0), .c(new_n73_), .O(z6));
endmodule


