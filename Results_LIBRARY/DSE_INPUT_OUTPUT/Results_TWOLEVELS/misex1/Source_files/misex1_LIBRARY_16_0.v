// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x5), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nand2  g07(.a(x2), .b(new_n19_), .O(new_n23_));
  nand3  g08(.a(x6), .b(new_n20_), .c(new_n16_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x0), .O(new_n25_));
  nand4  g10(.a(x3), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(x5), .O(new_n28_));
  nand3  g13(.a(new_n16_), .b(x1), .c(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(z1));
  inv1   g15(.a(x4), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x2), .c(new_n20_), .O(new_n32_));
  nor2   g17(.a(x2), .b(x1), .O(new_n33_));
  nor2   g18(.a(x6), .b(x3), .O(new_n34_));
  aoi22  g19(.a(new_n34_), .b(new_n33_), .c(new_n32_), .d(x1), .O(new_n35_));
  nand2  g20(.a(new_n16_), .b(x0), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n20_), .c(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n19_), .O(new_n38_));
  oai21  g23(.a(new_n35_), .b(x0), .c(new_n38_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n41_));
  nand2  g26(.a(x2), .b(x0), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(x3), .c(x5), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n19_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n41_), .O(z3));
  xor2a  g30(.a(x3), .b(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(x0), .O(new_n47_));
  nand2  g32(.a(x2), .b(new_n17_), .O(new_n48_));
  nand3  g33(.a(new_n34_), .b(new_n16_), .c(new_n17_), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(x5), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n19_), .O(new_n51_));
  oai21  g36(.a(x3), .b(new_n16_), .c(x1), .O(new_n52_));
  nand4  g37(.a(x6), .b(x5), .c(new_n20_), .d(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n51_), .O(z4));
  nand3  g41(.a(new_n48_), .b(new_n47_), .c(x5), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n19_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z5));
  nand4  g44(.a(new_n46_), .b(x5), .c(new_n19_), .d(x0), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(x1), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


