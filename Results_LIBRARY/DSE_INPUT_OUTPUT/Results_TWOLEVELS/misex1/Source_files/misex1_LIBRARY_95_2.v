// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(x2), .b(new_n16_), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x3), .c(x4), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n19_), .O(z0));
  inv1   g08(.a(x3), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n16_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n24_), .c(x4), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  oai21  g13(.a(x5), .b(x3), .c(x2), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x1), .O(new_n30_));
  inv1   g15(.a(x4), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(x1), .c(x6), .O(new_n32_));
  nand2  g17(.a(new_n31_), .b(new_n24_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  oai21  g19(.a(new_n32_), .b(x3), .c(new_n34_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n25_), .c(new_n30_), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(x0), .c(new_n28_), .O(z1));
  nor2   g22(.a(x5), .b(new_n25_), .O(new_n38_));
  nor2   g23(.a(x6), .b(x2), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n38_), .c(new_n24_), .O(new_n40_));
  oai21  g25(.a(new_n31_), .b(x2), .c(new_n24_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  oai21  g27(.a(new_n40_), .b(x1), .c(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n20_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n28_), .O(z2));
  nand2  g30(.a(new_n33_), .b(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n25_), .c(x1), .O(new_n47_));
  nor2   g32(.a(x5), .b(x3), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n19_), .O(z3));
  nand2  g37(.a(new_n24_), .b(new_n25_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n16_), .c(new_n20_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(x4), .c(x0), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  aoi21  g43(.a(new_n40_), .b(new_n29_), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n16_), .O(new_n60_));
  nand3  g45(.a(x6), .b(new_n24_), .c(new_n25_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n42_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n20_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n60_), .c(new_n55_), .O(z4));
  nor2   g49(.a(new_n29_), .b(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n58_), .c(new_n16_), .O(new_n66_));
  inv1   g51(.a(new_n61_), .O(new_n67_));
  aoi21  g52(.a(new_n24_), .b(x2), .c(new_n16_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n20_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand3  g55(.a(new_n48_), .b(x2), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n57_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n16_), .O(new_n73_));
  oai21  g58(.a(x4), .b(x2), .c(new_n24_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(x1), .c(new_n20_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n73_), .O(z6));
endmodule


