// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(x4), .b(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x3), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(new_n16_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n19_), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n19_), .c(x3), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n26_), .c(x4), .O(new_n30_));
  nor3   g15(.a(new_n20_), .b(x2), .c(new_n19_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n30_), .c(new_n17_), .O(new_n32_));
  inv1   g17(.a(x4), .O(new_n33_));
  nand2  g18(.a(new_n16_), .b(x0), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n23_), .c(new_n33_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n19_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z1));
  nand2  g22(.a(new_n24_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n27_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n33_), .c(new_n23_), .d(new_n19_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  aoi21  g27(.a(x4), .b(new_n16_), .c(x3), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(new_n19_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n42_), .c(new_n17_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n36_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(x1), .O(new_n48_));
  nor2   g33(.a(x5), .b(x4), .O(new_n49_));
  nand4  g34(.a(new_n49_), .b(new_n23_), .c(x2), .d(new_n19_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  nand4  g37(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(z3));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(x0), .O(new_n56_));
  aoi21  g41(.a(new_n40_), .b(new_n23_), .c(new_n25_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x0), .c(new_n56_), .O(new_n58_));
  nand3  g43(.a(new_n28_), .b(new_n16_), .c(new_n17_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  aoi21  g45(.a(new_n58_), .b(new_n19_), .c(new_n60_), .O(new_n61_));
  nand2  g46(.a(new_n44_), .b(new_n17_), .O(new_n62_));
  oai21  g47(.a(new_n61_), .b(x4), .c(new_n62_), .O(z4));
  nand2  g48(.a(new_n25_), .b(new_n17_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n56_), .c(x1), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n60_), .c(new_n33_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n62_), .O(z5));
  nand4  g52(.a(new_n24_), .b(new_n23_), .c(x2), .d(new_n17_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n56_), .c(new_n33_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n19_), .O(new_n70_));
  oai21  g55(.a(x4), .b(x2), .c(new_n23_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(x1), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n70_), .O(z6));
endmodule


