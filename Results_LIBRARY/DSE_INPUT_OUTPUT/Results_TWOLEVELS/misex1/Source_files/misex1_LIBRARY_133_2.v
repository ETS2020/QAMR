// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:23 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x6), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n17_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x6), .c(new_n16_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(x1), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand4  g12(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z1));
  inv1   g14(.a(x5), .O(new_n30_));
  nor2   g15(.a(x6), .b(x2), .O(new_n31_));
  aoi21  g16(.a(new_n30_), .b(x2), .c(new_n31_), .O(new_n32_));
  nor2   g17(.a(new_n32_), .b(x3), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n18_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(x6), .c(x1), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  aoi21  g22(.a(new_n33_), .b(new_n17_), .c(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x0), .c(new_n28_), .O(z2));
  nand2  g24(.a(new_n17_), .b(x0), .O(new_n40_));
  oai21  g25(.a(x4), .b(x3), .c(x7), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(x6), .c(new_n16_), .d(x1), .O(new_n42_));
  nor2   g27(.a(x5), .b(x3), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x2), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nand2  g31(.a(new_n18_), .b(x2), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n40_), .c(new_n46_), .O(z3));
  nand2  g33(.a(x3), .b(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n47_), .b(new_n49_), .c(new_n20_), .O(new_n50_));
  oai21  g35(.a(new_n32_), .b(x3), .c(new_n23_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n20_), .c(new_n50_), .O(new_n52_));
  nand2  g37(.a(x3), .b(x1), .O(new_n53_));
  oai21  g38(.a(x3), .b(x2), .c(new_n53_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(x6), .c(new_n20_), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  nor2   g41(.a(x6), .b(new_n17_), .O(new_n57_));
  nor2   g42(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  oai21  g43(.a(new_n52_), .b(x1), .c(new_n58_), .O(z4));
  nor2   g44(.a(new_n23_), .b(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n50_), .c(new_n17_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n55_), .O(z5));
  nand3  g47(.a(new_n43_), .b(x2), .c(new_n20_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n50_), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n18_), .O(new_n66_));
  nand4  g51(.a(new_n66_), .b(x6), .c(x1), .d(new_n20_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


