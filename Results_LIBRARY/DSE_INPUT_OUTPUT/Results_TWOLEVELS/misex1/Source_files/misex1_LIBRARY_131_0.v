// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(x6), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand4  g06(.a(x3), .b(x2), .c(x1), .d(new_n17_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  nand2  g09(.a(x5), .b(new_n18_), .O(new_n25_));
  nand3  g10(.a(x6), .b(x3), .c(x2), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n25_), .c(x1), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(x1), .c(x6), .O(new_n29_));
  oai21  g14(.a(x4), .b(x3), .c(x1), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(x3), .c(new_n30_), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n24_), .c(new_n27_), .O(new_n32_));
  nand2  g17(.a(new_n24_), .b(x0), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n18_), .c(x6), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  oai21  g20(.a(new_n32_), .b(x0), .c(new_n35_), .O(z1));
  nand2  g21(.a(x3), .b(x1), .O(new_n37_));
  nor2   g22(.a(x5), .b(x3), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n16_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n37_), .c(new_n24_), .O(new_n40_));
  nand2  g25(.a(new_n28_), .b(new_n18_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  nor2   g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n40_), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n35_), .O(z2));
  nand2  g30(.a(new_n41_), .b(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n24_), .c(x1), .O(new_n47_));
  nand3  g32(.a(new_n38_), .b(x2), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n21_), .O(z3));
  nand3  g36(.a(x3), .b(x2), .c(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n18_), .b(new_n24_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n52_), .c(x0), .O(new_n54_));
  nand3  g39(.a(x3), .b(new_n24_), .c(x0), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n19_), .c(x1), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n54_), .c(x6), .O(new_n57_));
  aoi21  g42(.a(new_n18_), .b(x2), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n57_), .O(z4));
  nand3  g45(.a(x6), .b(x3), .c(new_n24_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n19_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(x0), .O(new_n63_));
  nand2  g48(.a(new_n26_), .b(new_n25_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n17_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n63_), .c(x6), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n16_), .O(new_n67_));
  nand3  g52(.a(x6), .b(new_n18_), .c(new_n24_), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n58_), .c(new_n17_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z5));
  nand3  g56(.a(new_n38_), .b(x2), .c(new_n17_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n63_), .c(x6), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n16_), .O(new_n74_));
  oai21  g59(.a(x4), .b(x2), .c(new_n18_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(x1), .c(new_n17_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n74_), .O(z6));
endmodule


