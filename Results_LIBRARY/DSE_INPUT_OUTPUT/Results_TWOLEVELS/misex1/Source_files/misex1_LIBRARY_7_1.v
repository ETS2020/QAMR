// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x4), .O(new_n16_));
  nand2  g01(.a(x6), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x0), .O(new_n18_));
  nand3  g03(.a(x3), .b(x1), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n17_), .c(x2), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(z0));
  inv1   g10(.a(new_n17_), .O(new_n26_));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  oai21  g14(.a(new_n27_), .b(x1), .c(new_n29_), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n18_), .O(new_n31_));
  nor2   g16(.a(new_n21_), .b(x2), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n31_), .c(new_n30_), .d(new_n18_), .O(new_n33_));
  nand2  g18(.a(x6), .b(x4), .O(new_n34_));
  nor2   g19(.a(new_n34_), .b(x3), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n28_), .c(new_n18_), .O(new_n36_));
  oai21  g21(.a(new_n33_), .b(new_n26_), .c(new_n36_), .O(z1));
  nand3  g22(.a(x3), .b(new_n28_), .c(x0), .O(new_n38_));
  nor2   g23(.a(x5), .b(x3), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(x2), .c(new_n18_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n38_), .c(new_n26_), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n21_), .c(new_n28_), .d(new_n18_), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n41_), .c(new_n20_), .O(new_n45_));
  nand2  g30(.a(x4), .b(new_n28_), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n21_), .c(new_n20_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n18_), .c(new_n26_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n45_), .O(z2));
  nand3  g34(.a(new_n39_), .b(x2), .c(new_n20_), .O(new_n50_));
  inv1   g35(.a(x7), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n28_), .c(x1), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n50_), .c(x0), .O(new_n53_));
  nand3  g38(.a(new_n31_), .b(new_n21_), .c(x2), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n17_), .O(new_n56_));
  nor3   g41(.a(x6), .b(x4), .c(x3), .O(new_n57_));
  nand4  g42(.a(new_n57_), .b(new_n28_), .c(x1), .d(new_n18_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z3));
  xor2a  g44(.a(x3), .b(x2), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(x0), .O(new_n61_));
  nand2  g46(.a(x2), .b(new_n18_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n61_), .c(x1), .O(new_n63_));
  oai21  g48(.a(x3), .b(new_n28_), .c(x1), .O(new_n64_));
  nor2   g49(.a(new_n64_), .b(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  oai21  g51(.a(x6), .b(x1), .c(new_n34_), .O(new_n67_));
  nand4  g52(.a(new_n67_), .b(new_n21_), .c(new_n28_), .d(new_n18_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z4));
  inv1   g54(.a(x5), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n21_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(x2), .c(new_n18_), .O(new_n72_));
  aoi21  g57(.a(new_n72_), .b(new_n61_), .c(x1), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n65_), .c(new_n17_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n36_), .O(z5));
  nand4  g60(.a(new_n42_), .b(new_n16_), .c(new_n21_), .d(new_n28_), .O(new_n76_));
  aoi21  g61(.a(new_n76_), .b(new_n21_), .c(new_n20_), .O(new_n77_));
  nand4  g62(.a(new_n17_), .b(new_n70_), .c(new_n21_), .d(x2), .O(new_n78_));
  nor2   g63(.a(new_n78_), .b(x1), .O(new_n79_));
  oai21  g64(.a(new_n79_), .b(new_n77_), .c(new_n18_), .O(new_n80_));
  nand4  g65(.a(new_n60_), .b(new_n17_), .c(new_n20_), .d(x0), .O(new_n81_));
  and2   g66(.a(new_n81_), .b(new_n17_), .O(new_n82_));
  nand2  g67(.a(new_n82_), .b(new_n80_), .O(z6));
endmodule


