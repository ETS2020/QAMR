// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x1), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(new_n16_), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  nand3  g05(.a(x3), .b(new_n16_), .c(x0), .O(new_n21_));
  oai21  g06(.a(new_n16_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  oai21  g08(.a(x4), .b(x3), .c(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x1), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand2  g11(.a(x6), .b(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand2  g14(.a(new_n26_), .b(x2), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n29_), .c(new_n23_), .O(z1));
  inv1   g16(.a(z0), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n26_), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n20_), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n26_), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n36_), .c(new_n32_), .O(z2));
  oai21  g25(.a(x7), .b(x2), .c(new_n38_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(x1), .c(new_n17_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n30_), .O(z3));
  aoi21  g28(.a(x3), .b(new_n20_), .c(x2), .O(new_n44_));
  nor2   g29(.a(new_n26_), .b(new_n16_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n17_), .O(new_n46_));
  nand4  g31(.a(x3), .b(new_n16_), .c(new_n20_), .d(x0), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(z4));
  oai21  g33(.a(new_n26_), .b(new_n17_), .c(x2), .O(new_n49_));
  oai21  g34(.a(x4), .b(x3), .c(x1), .O(new_n50_));
  nand3  g35(.a(x3), .b(new_n20_), .c(x0), .O(new_n51_));
  oai21  g36(.a(new_n50_), .b(x0), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n16_), .O(new_n53_));
  oai21  g38(.a(x4), .b(new_n20_), .c(new_n33_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n26_), .c(new_n17_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n53_), .c(new_n49_), .O(z5));
  oai21  g41(.a(new_n26_), .b(x2), .c(new_n38_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(x1), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n47_), .c(new_n32_), .O(z6));
endmodule


