// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nor2   g07(.a(x4), .b(new_n18_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g10(.a(x4), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x3), .O(new_n27_));
  nand2  g12(.a(x5), .b(new_n18_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n27_), .c(new_n16_), .O(new_n29_));
  nand2  g14(.a(x4), .b(x3), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n18_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n31_), .c(x2), .O(new_n33_));
  aoi21  g18(.a(new_n29_), .b(new_n17_), .c(new_n33_), .O(new_n34_));
  nor2   g19(.a(x1), .b(new_n20_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n16_), .c(x4), .O(new_n36_));
  oai22  g21(.a(new_n36_), .b(new_n18_), .c(new_n34_), .d(x0), .O(z1));
  inv1   g22(.a(x5), .O(new_n38_));
  nor2   g23(.a(x6), .b(x2), .O(new_n39_));
  aoi21  g24(.a(new_n38_), .b(x2), .c(new_n39_), .O(new_n40_));
  nand3  g25(.a(x4), .b(new_n16_), .c(x1), .O(new_n41_));
  oai21  g26(.a(new_n40_), .b(x1), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n23_), .b(x1), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  aoi21  g29(.a(new_n42_), .b(new_n18_), .c(new_n44_), .O(new_n45_));
  nand3  g30(.a(new_n35_), .b(new_n23_), .c(new_n16_), .O(new_n46_));
  oai21  g31(.a(new_n45_), .b(x0), .c(new_n46_), .O(z2));
  nand2  g32(.a(x7), .b(x4), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n16_), .c(x1), .O(new_n49_));
  nand3  g34(.a(new_n38_), .b(x2), .c(new_n17_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(x0), .O(new_n51_));
  nand3  g36(.a(x2), .b(new_n17_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n18_), .O(new_n54_));
  nor3   g39(.a(x7), .b(x4), .c(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n22_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z3));
  nand3  g42(.a(new_n26_), .b(x3), .c(new_n16_), .O(new_n58_));
  nand2  g43(.a(new_n18_), .b(x2), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(new_n20_), .O(new_n60_));
  nand2  g45(.a(new_n30_), .b(x2), .O(new_n61_));
  inv1   g46(.a(x6), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n18_), .c(new_n16_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n61_), .c(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n60_), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(x3), .b(new_n16_), .c(new_n26_), .O(new_n66_));
  nand3  g51(.a(x4), .b(new_n18_), .c(new_n16_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(new_n66_), .c(new_n17_), .O(new_n68_));
  nand3  g53(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n68_), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n65_), .O(z4));
  aoi21  g57(.a(new_n29_), .b(new_n20_), .c(new_n60_), .O(new_n73_));
  oai21  g58(.a(new_n21_), .b(x2), .c(new_n18_), .O(new_n74_));
  oai21  g59(.a(new_n66_), .b(new_n17_), .c(new_n69_), .O(new_n75_));
  aoi22  g60(.a(new_n75_), .b(new_n20_), .c(new_n74_), .d(x4), .O(new_n76_));
  oai21  g61(.a(new_n73_), .b(x1), .c(new_n76_), .O(z5));
  nand4  g62(.a(new_n38_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n78_));
  inv1   g63(.a(new_n78_), .O(new_n79_));
  oai21  g64(.a(new_n79_), .b(new_n60_), .c(new_n17_), .O(new_n80_));
  nor2   g65(.a(new_n66_), .b(new_n17_), .O(new_n81_));
  aoi22  g66(.a(new_n81_), .b(new_n20_), .c(x4), .d(x3), .O(new_n82_));
  nand2  g67(.a(new_n82_), .b(new_n80_), .O(z6));
endmodule


