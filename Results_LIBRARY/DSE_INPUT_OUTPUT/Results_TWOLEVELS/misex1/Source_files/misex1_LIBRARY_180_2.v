// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:34 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  nand2  g00(.a(x3), .b(x2), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x4), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n18_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  inv1   g08(.a(x2), .O(new_n24_));
  nand3  g09(.a(x6), .b(new_n20_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g11(.a(x4), .b(new_n24_), .c(x1), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  aoi21  g13(.a(new_n26_), .b(new_n19_), .c(new_n28_), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n24_), .c(new_n19_), .d(x0), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(x0), .c(new_n30_), .O(z1));
  inv1   g16(.a(x0), .O(new_n32_));
  nand2  g17(.a(new_n20_), .b(x2), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(x4), .c(x1), .O(new_n34_));
  inv1   g19(.a(x5), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x2), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n24_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n20_), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n34_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n32_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n30_), .O(z2));
  nor2   g28(.a(x5), .b(x3), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(x2), .c(new_n19_), .O(new_n45_));
  inv1   g30(.a(x7), .O(new_n46_));
  nand4  g31(.a(new_n46_), .b(x4), .c(new_n24_), .d(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n32_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n21_), .O(z3));
  nand2  g35(.a(x3), .b(new_n24_), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n33_), .c(new_n32_), .O(new_n52_));
  aoi21  g37(.a(x3), .b(new_n24_), .c(x0), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n52_), .c(new_n19_), .O(new_n54_));
  or2    g39(.a(new_n34_), .b(x0), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(z4));
  aoi21  g41(.a(new_n25_), .b(new_n23_), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n52_), .c(new_n19_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z5));
  nand3  g44(.a(new_n44_), .b(x2), .c(new_n32_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n52_), .c(new_n19_), .O(new_n62_));
  oai21  g47(.a(new_n20_), .b(x0), .c(x4), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(x1), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


