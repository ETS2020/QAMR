// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nor3   g02(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n18_));
  nand3  g03(.a(new_n17_), .b(new_n16_), .c(x0), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n18_), .c(x2), .O(new_n21_));
  inv1   g06(.a(x2), .O(new_n22_));
  nand2  g07(.a(new_n17_), .b(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n16_), .O(new_n28_));
  nand3  g13(.a(x3), .b(new_n22_), .c(x1), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n25_), .O(new_n31_));
  oai21  g16(.a(x1), .b(new_n25_), .c(x3), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n22_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n31_), .O(z1));
  nor2   g19(.a(new_n17_), .b(new_n16_), .O(new_n35_));
  nor2   g20(.a(x5), .b(x3), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x2), .c(new_n16_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n25_), .O(new_n39_));
  nand4  g24(.a(x3), .b(new_n22_), .c(new_n16_), .d(x0), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(z2));
  inv1   g26(.a(x7), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(x3), .c(new_n22_), .d(x1), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n37_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n25_), .O(new_n45_));
  nand4  g30(.a(new_n17_), .b(x2), .c(new_n16_), .d(x0), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(z3));
  inv1   g32(.a(new_n18_), .O(new_n48_));
  xor2a  g33(.a(x3), .b(x2), .O(new_n49_));
  nor2   g34(.a(new_n22_), .b(x0), .O(new_n50_));
  aoi21  g35(.a(new_n49_), .b(x0), .c(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(x1), .c(new_n48_), .O(z4));
  nand2  g37(.a(new_n49_), .b(x0), .O(new_n53_));
  nand3  g38(.a(new_n27_), .b(x2), .c(new_n25_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n16_), .O(new_n56_));
  aoi21  g41(.a(new_n17_), .b(new_n22_), .c(new_n18_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n56_), .O(z5));
  nand3  g43(.a(new_n36_), .b(x2), .c(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n53_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n57_), .O(z6));
endmodule


