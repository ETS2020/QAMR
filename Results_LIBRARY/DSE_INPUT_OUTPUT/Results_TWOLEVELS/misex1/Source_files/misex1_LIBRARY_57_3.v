// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:04 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  nand2  g00(.a(x7), .b(x5), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand4  g03(.a(new_n16_), .b(new_n18_), .c(new_n17_), .d(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nor2   g05(.a(new_n18_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n16_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(x3), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n20_), .O(new_n29_));
  nand3  g14(.a(x3), .b(new_n17_), .c(x0), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n16_), .c(new_n26_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x3), .O(new_n34_));
  oai21  g19(.a(x7), .b(new_n33_), .c(new_n34_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(x2), .c(new_n17_), .d(new_n20_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z1));
  nand3  g22(.a(new_n16_), .b(x3), .c(x0), .O(new_n38_));
  nand3  g23(.a(new_n27_), .b(new_n18_), .c(new_n20_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x2), .O(new_n40_));
  nand4  g25(.a(new_n33_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n40_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(x4), .b(new_n26_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n18_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x1), .c(new_n20_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n43_), .c(new_n16_), .O(z2));
  oai21  g32(.a(x7), .b(new_n20_), .c(x5), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n17_), .O(new_n49_));
  nor2   g34(.a(x5), .b(x4), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n26_), .c(x1), .d(new_n20_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  inv1   g38(.a(x7), .O(new_n54_));
  nand4  g39(.a(new_n54_), .b(new_n26_), .c(x1), .d(new_n20_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(z3));
  nand2  g41(.a(new_n17_), .b(new_n20_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n26_), .c(new_n54_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(x5), .O(new_n59_));
  xor2a  g44(.a(x3), .b(x2), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n17_), .c(x0), .O(new_n61_));
  nand3  g46(.a(new_n28_), .b(new_n26_), .c(new_n20_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n33_), .b(x2), .O(new_n65_));
  nand3  g50(.a(new_n27_), .b(new_n18_), .c(new_n26_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n65_), .c(x1), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n21_), .c(new_n20_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n64_), .c(new_n59_), .O(z4));
  nand2  g54(.a(new_n65_), .b(new_n17_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x3), .c(new_n20_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n64_), .c(new_n59_), .O(z5));
  nor2   g57(.a(x4), .b(x2), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(x3), .c(x1), .O(new_n74_));
  nand4  g59(.a(new_n33_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n20_), .O(new_n77_));
  nand4  g62(.a(new_n60_), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n77_), .c(new_n16_), .O(z6));
endmodule


