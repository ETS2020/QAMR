// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x7), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x5), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand4  g04(.a(new_n17_), .b(new_n19_), .c(new_n18_), .d(x0), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nand3  g06(.a(x3), .b(x1), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n17_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand4  g11(.a(new_n17_), .b(x3), .c(new_n18_), .d(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n19_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n18_), .c(x0), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n28_), .c(new_n26_), .O(new_n31_));
  nand2  g16(.a(x7), .b(x5), .O(new_n32_));
  oai21  g17(.a(x5), .b(new_n19_), .c(new_n32_), .O(new_n33_));
  nand4  g18(.a(new_n33_), .b(x2), .c(new_n18_), .d(new_n21_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n31_), .c(new_n17_), .O(z1));
  nand3  g20(.a(new_n26_), .b(new_n18_), .c(x0), .O(new_n36_));
  oai21  g21(.a(new_n18_), .b(x0), .c(new_n36_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n17_), .c(x3), .O(new_n38_));
  inv1   g23(.a(x5), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x2), .O(new_n40_));
  inv1   g25(.a(x6), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n26_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n19_), .c(new_n18_), .O(new_n44_));
  nand3  g29(.a(x4), .b(new_n26_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n21_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n38_), .c(new_n17_), .O(z2));
  oai21  g33(.a(new_n16_), .b(new_n21_), .c(x5), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(x2), .c(new_n18_), .O(new_n50_));
  inv1   g35(.a(x4), .O(new_n51_));
  nor2   g36(.a(new_n18_), .b(x0), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(x7), .c(new_n51_), .d(new_n26_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n19_), .O(new_n55_));
  nand4  g40(.a(new_n52_), .b(new_n16_), .c(new_n39_), .d(new_n26_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(z3));
  xor2a  g42(.a(x3), .b(x2), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n18_), .c(x0), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  nand2  g45(.a(x3), .b(x1), .O(new_n61_));
  nand2  g46(.a(new_n19_), .b(new_n26_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n61_), .c(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n60_), .c(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n16_), .b(x5), .O(new_n65_));
  nand4  g50(.a(new_n65_), .b(x2), .c(new_n18_), .d(new_n21_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n64_), .O(z4));
  nand2  g52(.a(new_n18_), .b(new_n21_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n26_), .c(x7), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(x5), .O(new_n70_));
  nand3  g55(.a(new_n58_), .b(new_n17_), .c(x0), .O(new_n71_));
  nand4  g56(.a(new_n39_), .b(x3), .c(x2), .d(new_n21_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n18_), .O(new_n74_));
  aoi21  g59(.a(new_n19_), .b(x2), .c(new_n18_), .O(new_n75_));
  nand3  g60(.a(x6), .b(new_n19_), .c(new_n26_), .O(new_n76_));
  inv1   g61(.a(new_n76_), .O(new_n77_));
  oai21  g62(.a(new_n77_), .b(new_n75_), .c(new_n21_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n74_), .c(new_n70_), .O(z5));
  nand3  g64(.a(new_n51_), .b(new_n26_), .c(x1), .O(new_n80_));
  nand4  g65(.a(new_n39_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n81_));
  nand2  g66(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g67(.a(new_n82_), .b(new_n21_), .O(new_n83_));
  nand4  g68(.a(new_n83_), .b(new_n59_), .c(new_n22_), .d(new_n17_), .O(z6));
endmodule


