// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x4), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x5), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n18_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x2), .c(new_n17_), .O(new_n25_));
  inv1   g10(.a(x6), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(new_n17_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n25_), .c(x4), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  nand4  g15(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(z1));
  aoi21  g17(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n33_));
  nand3  g18(.a(new_n23_), .b(x2), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n26_), .b(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x3), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n33_), .c(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x0), .c(new_n31_), .O(z2));
  inv1   g23(.a(x4), .O(new_n39_));
  oai21  g24(.a(x5), .b(new_n39_), .c(new_n20_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n41_));
  inv1   g26(.a(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n16_), .c(x1), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x4), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n41_), .O(z3));
  nand2  g31(.a(x3), .b(new_n16_), .O(new_n47_));
  nand2  g32(.a(new_n18_), .b(x2), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n47_), .c(new_n20_), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  nor2   g35(.a(x5), .b(new_n18_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  nand4  g37(.a(new_n52_), .b(x4), .c(x2), .d(new_n20_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  inv1   g40(.a(new_n33_), .O(new_n56_));
  oai21  g41(.a(x3), .b(x2), .c(new_n56_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(x4), .c(new_n20_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z4));
  nand3  g44(.a(new_n24_), .b(x4), .c(x2), .O(new_n60_));
  nor2   g45(.a(new_n60_), .b(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n49_), .c(new_n17_), .O(new_n62_));
  nand3  g47(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n56_), .c(x4), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n20_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  nand3  g51(.a(new_n23_), .b(x4), .c(new_n18_), .O(new_n67_));
  nor3   g52(.a(new_n67_), .b(new_n16_), .c(x0), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n49_), .c(new_n17_), .O(new_n69_));
  aoi21  g54(.a(x3), .b(x1), .c(new_n39_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(x0), .c(new_n69_), .O(z6));
endmodule


