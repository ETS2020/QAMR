// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  inv1   g02(.a(x4), .O(new_n18_));
  inv1   g03(.a(x2), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(x1), .O(new_n20_));
  aoi21  g05(.a(new_n20_), .b(new_n17_), .c(new_n18_), .O(new_n21_));
  nand2  g06(.a(x1), .b(new_n16_), .O(new_n22_));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  oai22  g08(.a(new_n23_), .b(new_n22_), .c(new_n21_), .d(new_n16_), .O(z0));
  inv1   g09(.a(x1), .O(new_n25_));
  nand2  g10(.a(new_n19_), .b(new_n25_), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n17_), .c(x4), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x0), .O(new_n28_));
  oai21  g13(.a(x5), .b(x3), .c(x2), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x1), .O(new_n30_));
  aoi21  g15(.a(new_n18_), .b(x1), .c(x6), .O(new_n31_));
  nand2  g16(.a(new_n18_), .b(new_n17_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x1), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x3), .c(new_n33_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n19_), .c(new_n30_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x0), .c(new_n28_), .O(z1));
  nor2   g21(.a(x5), .b(new_n19_), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n37_), .c(new_n17_), .O(new_n39_));
  oai21  g24(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(x1), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n28_), .O(z2));
  nand2  g29(.a(new_n32_), .b(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n19_), .c(x1), .O(new_n46_));
  nor2   g31(.a(x5), .b(x3), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  nor2   g35(.a(new_n18_), .b(x3), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(x2), .c(new_n25_), .d(x0), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  nand2  g38(.a(new_n17_), .b(new_n19_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n25_), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n18_), .O(new_n56_));
  xor2a  g41(.a(x3), .b(x2), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(x4), .c(x0), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  aoi21  g44(.a(new_n39_), .b(new_n29_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n25_), .O(new_n61_));
  nand3  g46(.a(x6), .b(new_n17_), .c(new_n19_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n41_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n16_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n61_), .c(new_n56_), .O(z4));
  nor2   g50(.a(new_n29_), .b(x0), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n59_), .c(new_n25_), .O(new_n67_));
  inv1   g52(.a(new_n62_), .O(new_n68_));
  aoi21  g53(.a(new_n17_), .b(x2), .c(new_n25_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n68_), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z5));
  nand3  g56(.a(new_n47_), .b(x2), .c(new_n16_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n58_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n25_), .O(new_n74_));
  oai21  g59(.a(x4), .b(x2), .c(new_n17_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(x1), .c(new_n16_), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n74_), .O(z6));
endmodule


