// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:16 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(x2), .b(x0), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x3), .c(x6), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x3), .b(x2), .c(x1), .d(new_n20_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n19_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  oai22  g08(.a(new_n23_), .b(x1), .c(x3), .d(x2), .O(new_n24_));
  nor2   g09(.a(x2), .b(new_n16_), .O(new_n25_));
  aoi21  g10(.a(new_n24_), .b(x6), .c(new_n25_), .O(new_n26_));
  inv1   g11(.a(x3), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x0), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(x6), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n16_), .O(new_n31_));
  oai21  g16(.a(new_n26_), .b(x0), .c(new_n31_), .O(z1));
  nand3  g17(.a(x3), .b(new_n28_), .c(x0), .O(new_n33_));
  inv1   g18(.a(x5), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n27_), .c(x2), .d(new_n20_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x6), .c(new_n16_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n27_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(x1), .c(new_n20_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n37_), .O(z2));
  oai21  g26(.a(x4), .b(x3), .c(x7), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n28_), .c(x1), .O(new_n43_));
  nand2  g28(.a(x2), .b(new_n16_), .O(new_n44_));
  nand3  g29(.a(x6), .b(new_n34_), .c(new_n27_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n43_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n20_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n19_), .O(z3));
  inv1   g33(.a(x6), .O(new_n49_));
  xnor2a g34(.a(x3), .b(x2), .O(new_n50_));
  nand2  g35(.a(x2), .b(new_n20_), .O(new_n51_));
  oai21  g36(.a(new_n50_), .b(new_n20_), .c(new_n51_), .O(new_n52_));
  nor3   g37(.a(x3), .b(x2), .c(x0), .O(new_n53_));
  aoi21  g38(.a(new_n52_), .b(new_n16_), .c(new_n53_), .O(new_n54_));
  aoi21  g39(.a(new_n27_), .b(x2), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n20_), .O(new_n56_));
  oai21  g41(.a(new_n54_), .b(new_n49_), .c(new_n56_), .O(z4));
  xor2a  g42(.a(x3), .b(x2), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(x0), .O(new_n59_));
  nand2  g44(.a(new_n34_), .b(new_n27_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(x2), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n59_), .c(x6), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n16_), .O(new_n63_));
  nand3  g48(.a(x6), .b(new_n27_), .c(new_n28_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n55_), .c(new_n20_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  nand3  g52(.a(new_n59_), .b(new_n35_), .c(x6), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n16_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n27_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x1), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


