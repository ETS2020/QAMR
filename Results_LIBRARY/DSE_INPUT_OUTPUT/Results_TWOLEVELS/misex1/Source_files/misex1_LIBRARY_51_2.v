// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand4  g01(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(z0));
  inv1   g03(.a(x2), .O(new_n19_));
  nor2   g04(.a(x5), .b(x3), .O(new_n20_));
  nor3   g05(.a(new_n20_), .b(new_n19_), .c(x1), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x6), .O(new_n24_));
  oai21  g09(.a(x4), .b(new_n23_), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n22_), .O(new_n26_));
  inv1   g11(.a(x4), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n22_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n26_), .c(x2), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n21_), .c(new_n16_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n19_), .c(new_n23_), .d(x0), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(z1));
  oai21  g18(.a(x2), .b(x1), .c(x3), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x0), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x2), .c(new_n16_), .O(new_n37_));
  oai21  g22(.a(x6), .b(x2), .c(new_n37_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n22_), .c(new_n23_), .O(new_n39_));
  oai21  g24(.a(new_n27_), .b(x2), .c(new_n22_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x1), .c(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n39_), .c(new_n35_), .O(z2));
  nand2  g27(.a(new_n28_), .b(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n19_), .c(x1), .O(new_n44_));
  nand3  g29(.a(new_n20_), .b(x2), .c(new_n23_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(x0), .O(z3));
  aoi21  g31(.a(new_n22_), .b(x1), .c(new_n19_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n30_), .c(new_n16_), .O(new_n48_));
  nand4  g33(.a(new_n24_), .b(new_n22_), .c(new_n19_), .d(new_n23_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n48_), .c(new_n35_), .O(z4));
  aoi21  g35(.a(x5), .b(new_n23_), .c(x3), .O(new_n51_));
  nor2   g36(.a(new_n51_), .b(new_n19_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n30_), .c(new_n16_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n35_), .O(z5));
  nand3  g39(.a(x3), .b(new_n19_), .c(x0), .O(new_n55_));
  nand3  g40(.a(new_n20_), .b(x2), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n23_), .O(new_n58_));
  oai21  g43(.a(x4), .b(x2), .c(new_n22_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(x1), .c(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z6));
endmodule


