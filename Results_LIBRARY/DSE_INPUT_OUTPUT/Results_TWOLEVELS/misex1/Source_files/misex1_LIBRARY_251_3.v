// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  nand3  g03(.a(new_n16_), .b(new_n18_), .c(x0), .O(new_n19_));
  nor2   g04(.a(new_n18_), .b(x0), .O(new_n20_));
  nor2   g05(.a(x5), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(x2), .O(new_n24_));
  oai21  g09(.a(new_n17_), .b(new_n16_), .c(new_n24_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x1), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x1), .c(x6), .O(new_n30_));
  aoi21  g15(.a(new_n17_), .b(x3), .c(x4), .O(new_n31_));
  oai22  g16(.a(new_n31_), .b(new_n18_), .c(new_n30_), .d(x3), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n26_), .c(new_n28_), .O(new_n33_));
  nand2  g18(.a(new_n18_), .b(x0), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x2), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x3), .O(new_n36_));
  oai21  g21(.a(new_n33_), .b(x0), .c(new_n36_), .O(z1));
  inv1   g22(.a(x0), .O(new_n38_));
  nor2   g23(.a(x5), .b(new_n26_), .O(new_n39_));
  nor2   g24(.a(x6), .b(x2), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n39_), .c(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n17_), .b(x3), .O(new_n42_));
  oai21  g27(.a(new_n29_), .b(x2), .c(new_n42_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x1), .O(new_n44_));
  oai21  g29(.a(new_n41_), .b(x1), .c(new_n44_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n36_), .O(z2));
  nand2  g32(.a(x7), .b(x4), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n26_), .c(x1), .O(new_n49_));
  nand2  g34(.a(new_n39_), .b(new_n18_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(x0), .O(new_n51_));
  nand3  g36(.a(x2), .b(new_n18_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n16_), .O(new_n54_));
  nor3   g39(.a(x7), .b(x5), .c(x2), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z3));
  nand2  g42(.a(new_n16_), .b(x2), .O(new_n58_));
  nand2  g43(.a(new_n21_), .b(new_n26_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(new_n38_), .O(new_n60_));
  aoi21  g45(.a(new_n41_), .b(new_n27_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n60_), .c(new_n18_), .O(new_n62_));
  aoi21  g47(.a(new_n29_), .b(x3), .c(x2), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n21_), .c(x1), .O(new_n64_));
  nand3  g49(.a(x6), .b(new_n16_), .c(new_n26_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  aoi22  g51(.a(new_n66_), .b(new_n38_), .c(x5), .d(x3), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n62_), .O(z4));
  nor2   g53(.a(new_n27_), .b(x0), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n60_), .c(new_n18_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n67_), .O(z5));
  nand4  g56(.a(new_n17_), .b(new_n16_), .c(x2), .d(new_n38_), .O(new_n72_));
  inv1   g57(.a(new_n72_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n60_), .c(new_n18_), .O(new_n74_));
  nand2  g59(.a(new_n29_), .b(new_n16_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(x2), .c(new_n42_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(x1), .c(new_n38_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n74_), .O(z6));
endmodule


