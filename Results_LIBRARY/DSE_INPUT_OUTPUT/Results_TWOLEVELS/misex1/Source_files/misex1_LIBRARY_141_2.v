// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:25 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x7), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(x1), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  inv1   g10(.a(x4), .O(new_n26_));
  nand3  g11(.a(x7), .b(new_n26_), .c(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n18_), .O(new_n29_));
  nand2  g14(.a(new_n26_), .b(new_n18_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(x7), .c(x1), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n29_), .c(x2), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n24_), .c(new_n20_), .O(new_n33_));
  nor2   g18(.a(new_n18_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n17_), .c(x0), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  nor2   g21(.a(x7), .b(new_n17_), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n33_), .O(z1));
  nor2   g24(.a(x5), .b(new_n16_), .O(new_n40_));
  nor2   g25(.a(x6), .b(x2), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(new_n40_), .c(new_n18_), .O(new_n42_));
  oai21  g27(.a(new_n26_), .b(x2), .c(new_n18_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x7), .c(x1), .O(new_n44_));
  oai21  g29(.a(new_n42_), .b(x1), .c(new_n44_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n35_), .O(z2));
  nand2  g32(.a(new_n40_), .b(new_n17_), .O(new_n48_));
  nand4  g33(.a(x7), .b(new_n26_), .c(new_n16_), .d(x1), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand3  g36(.a(x2), .b(new_n17_), .c(x0), .O(new_n52_));
  aoi21  g37(.a(new_n52_), .b(new_n51_), .c(x3), .O(z3));
  inv1   g38(.a(new_n34_), .O(new_n54_));
  nand2  g39(.a(new_n18_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n54_), .c(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n42_), .b(new_n23_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n20_), .c(new_n56_), .O(new_n58_));
  nand2  g43(.a(new_n18_), .b(x2), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(x7), .c(x1), .O(new_n60_));
  nand3  g45(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n20_), .c(new_n37_), .O(new_n63_));
  oai21  g48(.a(new_n58_), .b(x1), .c(new_n63_), .O(z4));
  nor2   g49(.a(new_n23_), .b(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n56_), .c(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n63_), .O(z5));
  inv1   g52(.a(x5), .O(new_n68_));
  nand4  g53(.a(new_n68_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n56_), .c(new_n17_), .O(new_n71_));
  aoi21  g56(.a(new_n26_), .b(new_n16_), .c(x3), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(x0), .c(x7), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(x1), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n71_), .O(z6));
endmodule


