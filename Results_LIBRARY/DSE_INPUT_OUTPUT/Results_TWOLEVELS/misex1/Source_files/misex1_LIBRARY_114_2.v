// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nor2   g07(.a(x5), .b(new_n19_), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n26_), .c(x0), .O(new_n27_));
  oai21  g12(.a(new_n26_), .b(x0), .c(new_n27_), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(x5), .c(new_n18_), .O(new_n29_));
  inv1   g14(.a(x6), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(x3), .c(new_n18_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n26_), .c(new_n16_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n29_), .c(new_n24_), .O(z1));
  nand3  g18(.a(x4), .b(new_n19_), .c(new_n26_), .O(new_n34_));
  nand2  g19(.a(x5), .b(x3), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(new_n18_), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n30_), .b(new_n26_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n38_), .c(x3), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n18_), .c(new_n36_), .O(new_n41_));
  nor2   g26(.a(x1), .b(new_n16_), .O(new_n42_));
  nand3  g27(.a(x5), .b(x3), .c(new_n26_), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  oai21  g30(.a(new_n41_), .b(x0), .c(new_n45_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n26_), .c(x1), .O(new_n48_));
  nor2   g33(.a(x5), .b(x3), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(x2), .c(new_n18_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n16_), .O(new_n52_));
  nand2  g37(.a(new_n19_), .b(x2), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n42_), .c(new_n23_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n52_), .O(z3));
  aoi21  g41(.a(new_n53_), .b(new_n43_), .c(new_n16_), .O(new_n57_));
  oai21  g42(.a(x5), .b(new_n19_), .c(x2), .O(new_n58_));
  nand3  g43(.a(new_n30_), .b(new_n19_), .c(new_n26_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n57_), .c(new_n18_), .O(new_n61_));
  oai21  g46(.a(new_n18_), .b(x0), .c(x5), .O(new_n62_));
  nand2  g47(.a(new_n30_), .b(new_n18_), .O(new_n63_));
  nand4  g48(.a(new_n63_), .b(new_n19_), .c(new_n26_), .d(new_n16_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  aoi21  g50(.a(new_n62_), .b(x3), .c(new_n65_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n61_), .O(z4));
  nor3   g52(.a(new_n37_), .b(new_n26_), .c(x0), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n57_), .c(new_n18_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand3  g55(.a(new_n49_), .b(x2), .c(new_n16_), .O(new_n71_));
  inv1   g56(.a(new_n71_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n57_), .c(new_n18_), .O(new_n73_));
  inv1   g58(.a(x4), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(new_n19_), .c(new_n26_), .O(new_n75_));
  aoi21  g60(.a(new_n75_), .b(new_n35_), .c(new_n18_), .O(new_n76_));
  aoi21  g61(.a(new_n76_), .b(new_n16_), .c(new_n23_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n73_), .O(z6));
endmodule


