// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  inv1   g08(.a(x4), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n22_), .O(z0));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x1), .O(new_n28_));
  nand2  g13(.a(x4), .b(x1), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n19_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n29_), .c(x2), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n28_), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n18_), .b(x0), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n19_), .c(x4), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n23_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z1));
  nand2  g21(.a(x3), .b(x2), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n23_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n37_), .c(new_n18_), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(x2), .c(new_n18_), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(x4), .c(new_n23_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(x3), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n39_), .c(new_n16_), .O(new_n45_));
  nand3  g30(.a(x4), .b(x3), .c(new_n23_), .O(new_n46_));
  or2    g31(.a(new_n46_), .b(new_n33_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n45_), .O(z2));
  nor2   g33(.a(x5), .b(x3), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(x2), .c(new_n18_), .O(new_n50_));
  inv1   g35(.a(x7), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(x4), .c(new_n23_), .d(x1), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n16_), .O(new_n54_));
  nand2  g39(.a(new_n19_), .b(x2), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n33_), .c(new_n54_), .O(z3));
  aoi21  g41(.a(new_n55_), .b(new_n46_), .c(new_n16_), .O(new_n57_));
  nor2   g42(.a(new_n23_), .b(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n57_), .c(new_n18_), .O(new_n59_));
  nor2   g44(.a(x6), .b(x4), .O(new_n60_));
  nor3   g45(.a(new_n60_), .b(x3), .c(x2), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n39_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n59_), .c(new_n25_), .O(z4));
  nor2   g48(.a(new_n27_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n57_), .c(new_n18_), .O(new_n65_));
  nand3  g50(.a(x6), .b(new_n19_), .c(new_n23_), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n39_), .c(new_n16_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n65_), .c(new_n25_), .O(z5));
  nand3  g54(.a(new_n49_), .b(x2), .c(new_n16_), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n57_), .c(new_n18_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n25_), .c(new_n17_), .O(z6));
endmodule


