// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  oai21  g07(.a(x3), .b(x2), .c(new_n22_), .O(z0));
  nand3  g08(.a(x5), .b(new_n18_), .c(new_n16_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x2), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n19_), .O(new_n26_));
  xnor2a g11(.a(x2), .b(x1), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n18_), .c(x0), .O(new_n29_));
  oai21  g14(.a(new_n27_), .b(x0), .c(new_n29_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x3), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n26_), .O(z1));
  nand2  g17(.a(x3), .b(x1), .O(new_n33_));
  nor2   g18(.a(x5), .b(x3), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x2), .c(new_n18_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n18_), .b(x0), .c(new_n19_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n37_), .O(z2));
  inv1   g24(.a(x7), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n28_), .c(x1), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n18_), .b(x0), .c(new_n28_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x3), .c(new_n43_), .O(z3));
  xnor2a g30(.a(x3), .b(x2), .O(new_n46_));
  nor2   g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  aoi21  g32(.a(x5), .b(new_n19_), .c(x2), .O(new_n48_));
  nor2   g33(.a(new_n48_), .b(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n18_), .O(new_n50_));
  inv1   g35(.a(new_n17_), .O(new_n51_));
  aoi21  g36(.a(new_n19_), .b(new_n28_), .c(new_n51_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z4));
  nand2  g38(.a(x5), .b(new_n19_), .O(new_n54_));
  nand2  g39(.a(x3), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n54_), .c(x0), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n47_), .c(new_n18_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n52_), .O(z5));
  nand3  g43(.a(new_n34_), .b(x2), .c(new_n16_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n47_), .c(new_n18_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n17_), .O(z6));
endmodule


