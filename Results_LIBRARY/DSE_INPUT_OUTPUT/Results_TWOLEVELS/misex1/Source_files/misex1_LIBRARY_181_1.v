// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x4), .O(new_n16_));
  nand2  g01(.a(x5), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand4  g04(.a(new_n17_), .b(new_n19_), .c(new_n18_), .d(x0), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x1), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(x0), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n21_), .c(x2), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n17_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n18_), .c(x0), .O(new_n27_));
  inv1   g12(.a(x0), .O(new_n28_));
  nand3  g13(.a(x6), .b(new_n19_), .c(new_n28_), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n27_), .c(x5), .d(new_n16_), .O(new_n30_));
  nor2   g15(.a(new_n18_), .b(x0), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n30_), .c(new_n26_), .O(new_n32_));
  nand2  g17(.a(x5), .b(x4), .O(new_n33_));
  oai21  g18(.a(x5), .b(new_n19_), .c(new_n33_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x2), .c(new_n18_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n28_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n32_), .c(new_n17_), .O(z1));
  nand4  g23(.a(new_n17_), .b(x3), .c(new_n26_), .d(x0), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand2  g25(.a(x6), .b(new_n26_), .O(new_n41_));
  nand4  g26(.a(new_n41_), .b(new_n40_), .c(new_n19_), .d(new_n28_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n39_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  oai21  g29(.a(x6), .b(x3), .c(new_n18_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x4), .c(new_n26_), .O(new_n46_));
  nand2  g31(.a(x5), .b(new_n26_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x3), .c(x1), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n28_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n44_), .c(new_n17_), .O(z2));
  nand4  g36(.a(new_n19_), .b(x2), .c(new_n18_), .d(x0), .O(new_n52_));
  nor2   g37(.a(x7), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n31_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n17_), .O(new_n56_));
  nand3  g41(.a(new_n16_), .b(new_n26_), .c(x1), .O(new_n57_));
  oai21  g42(.a(new_n26_), .b(x1), .c(new_n57_), .O(new_n58_));
  nand4  g43(.a(new_n58_), .b(new_n40_), .c(new_n19_), .d(new_n28_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n56_), .O(z3));
  xor2a  g45(.a(x3), .b(x2), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n18_), .c(x0), .O(new_n62_));
  inv1   g47(.a(new_n62_), .O(new_n63_));
  nand3  g48(.a(x6), .b(new_n19_), .c(new_n26_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n22_), .c(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n17_), .O(new_n66_));
  inv1   g51(.a(new_n57_), .O(new_n67_));
  aoi21  g52(.a(x6), .b(new_n26_), .c(x1), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n40_), .O(new_n69_));
  inv1   g54(.a(x6), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x4), .c(new_n26_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n69_), .c(x3), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n36_), .c(new_n28_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n66_), .O(z4));
  oai21  g59(.a(x5), .b(x3), .c(new_n16_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(new_n26_), .c(x1), .O(new_n76_));
  nand2  g61(.a(new_n76_), .b(new_n35_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n28_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n66_), .O(z5));
  oai21  g64(.a(new_n63_), .b(new_n23_), .c(new_n17_), .O(new_n80_));
  nand2  g65(.a(new_n80_), .b(new_n59_), .O(z6));
endmodule


