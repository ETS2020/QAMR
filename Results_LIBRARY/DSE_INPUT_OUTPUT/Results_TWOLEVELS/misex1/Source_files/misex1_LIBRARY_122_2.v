// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x1), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(new_n16_), .O(z0));
  inv1   g04(.a(x1), .O(new_n20_));
  nand3  g05(.a(x3), .b(new_n16_), .c(x0), .O(new_n21_));
  oai21  g06(.a(new_n16_), .b(x0), .c(new_n21_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  aoi21  g09(.a(x4), .b(new_n24_), .c(new_n16_), .O(new_n25_));
  nand2  g10(.a(x6), .b(new_n24_), .O(new_n26_));
  oai21  g11(.a(new_n25_), .b(new_n20_), .c(new_n26_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n17_), .O(new_n28_));
  nand2  g13(.a(new_n24_), .b(x2), .O(new_n29_));
  nand3  g14(.a(new_n29_), .b(new_n28_), .c(new_n23_), .O(z1));
  inv1   g15(.a(z0), .O(new_n31_));
  nor2   g16(.a(x6), .b(x3), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n20_), .O(new_n35_));
  nand2  g20(.a(x4), .b(new_n24_), .O(new_n36_));
  oai21  g21(.a(new_n24_), .b(x2), .c(new_n36_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(x1), .c(new_n17_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n35_), .c(new_n31_), .O(z2));
  oai21  g24(.a(x4), .b(x3), .c(x7), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(z3));
  nand2  g27(.a(x4), .b(new_n24_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(x1), .c(new_n17_), .O(new_n44_));
  nand3  g29(.a(x3), .b(new_n20_), .c(x0), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n16_), .O(new_n47_));
  aoi21  g32(.a(new_n36_), .b(new_n16_), .c(new_n20_), .O(new_n48_));
  oai21  g33(.a(new_n32_), .b(x2), .c(new_n20_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n26_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n17_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n47_), .c(new_n29_), .O(z4));
  nand3  g37(.a(new_n28_), .b(new_n23_), .c(new_n31_), .O(z5));
  nand2  g38(.a(new_n47_), .b(new_n31_), .O(z6));
endmodule


