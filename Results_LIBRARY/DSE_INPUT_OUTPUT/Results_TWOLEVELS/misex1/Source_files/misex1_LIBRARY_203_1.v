// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x6), .b(x1), .O(new_n23_));
  nor3   g08(.a(new_n23_), .b(x3), .c(x2), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(new_n16_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n24_), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x0), .O(z1));
  nand2  g13(.a(x3), .b(x2), .O(new_n29_));
  nand3  g14(.a(x4), .b(new_n20_), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  oai21  g19(.a(x6), .b(x2), .c(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n20_), .c(new_n19_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n32_), .c(x0), .O(z2));
  oai21  g22(.a(x4), .b(x3), .c(x7), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n16_), .c(x1), .O(new_n39_));
  nand3  g24(.a(new_n25_), .b(x2), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  nand4  g27(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n43_));
  nand2  g28(.a(x3), .b(new_n16_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z3));
  aoi21  g30(.a(new_n20_), .b(x1), .c(new_n16_), .O(new_n46_));
  nor2   g31(.a(x6), .b(new_n19_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n19_), .c(x3), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n16_), .c(new_n46_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(x0), .c(new_n43_), .O(z4));
  aoi21  g35(.a(x5), .b(new_n19_), .c(x3), .O(new_n51_));
  nor2   g36(.a(new_n51_), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n24_), .c(new_n17_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n43_), .O(z5));
  inv1   g39(.a(new_n40_), .O(new_n55_));
  inv1   g40(.a(x4), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n20_), .c(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n29_), .c(new_n19_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n55_), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n43_), .O(z6));
endmodule


