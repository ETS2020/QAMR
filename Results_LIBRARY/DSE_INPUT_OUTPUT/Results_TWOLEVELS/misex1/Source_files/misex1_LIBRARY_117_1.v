// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n17_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n21_), .c(new_n16_), .O(z0));
  nand2  g10(.a(new_n22_), .b(x5), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  oai21  g13(.a(x4), .b(new_n19_), .c(new_n22_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(x1), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand2  g18(.a(x3), .b(new_n16_), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  nor2   g20(.a(new_n22_), .b(x3), .O(new_n36_));
  aoi21  g21(.a(new_n35_), .b(new_n23_), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n33_), .O(z1));
  nand2  g23(.a(x5), .b(x2), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n19_), .c(new_n18_), .O(new_n40_));
  nand3  g25(.a(x4), .b(new_n16_), .c(x1), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(x6), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n20_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n37_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n16_), .c(x1), .O(new_n46_));
  inv1   g31(.a(x5), .O(new_n47_));
  nand4  g32(.a(new_n47_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n46_), .c(x0), .O(new_n49_));
  nand3  g34(.a(new_n23_), .b(new_n19_), .c(x2), .O(new_n50_));
  inv1   g35(.a(new_n50_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n49_), .c(new_n22_), .O(new_n52_));
  nor2   g37(.a(x7), .b(new_n19_), .O(new_n53_));
  nand4  g38(.a(new_n53_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n52_), .O(z3));
  nand3  g40(.a(new_n22_), .b(new_n19_), .c(x2), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n34_), .c(new_n17_), .O(new_n57_));
  aoi21  g42(.a(x5), .b(x2), .c(new_n19_), .O(new_n58_));
  nand2  g43(.a(x3), .b(x2), .O(new_n59_));
  oai21  g44(.a(new_n58_), .b(x6), .c(new_n59_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n17_), .c(new_n57_), .O(new_n61_));
  oai21  g46(.a(new_n29_), .b(x2), .c(new_n19_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n17_), .O(new_n63_));
  oai21  g48(.a(new_n61_), .b(x1), .c(new_n63_), .O(z4));
  inv1   g49(.a(new_n36_), .O(new_n65_));
  nand2  g50(.a(new_n56_), .b(new_n34_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(x0), .O(new_n67_));
  nand2  g52(.a(new_n27_), .b(new_n17_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n18_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n63_), .c(new_n65_), .O(z5));
  nand3  g56(.a(new_n22_), .b(new_n47_), .c(new_n19_), .O(new_n72_));
  nor3   g57(.a(new_n72_), .b(new_n16_), .c(x0), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n57_), .c(new_n18_), .O(new_n74_));
  inv1   g59(.a(x4), .O(new_n75_));
  nand2  g60(.a(new_n22_), .b(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(x2), .c(new_n19_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n17_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n74_), .c(new_n65_), .O(z6));
endmodule


