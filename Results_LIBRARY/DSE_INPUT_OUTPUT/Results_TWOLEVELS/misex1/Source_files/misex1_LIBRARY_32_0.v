// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  oai21  g02(.a(x3), .b(new_n17_), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  nand4  g04(.a(x3), .b(x2), .c(x1), .d(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n19_), .O(z0));
  oai21  g06(.a(x5), .b(x3), .c(new_n16_), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nor2   g09(.a(x4), .b(new_n24_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x2), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n23_), .c(new_n17_), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n27_), .O(z1));
  nor2   g15(.a(x5), .b(x3), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n17_), .c(new_n28_), .O(new_n32_));
  inv1   g17(.a(x4), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nand3  g19(.a(x3), .b(x2), .c(x1), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  aoi21  g21(.a(new_n34_), .b(new_n28_), .c(new_n36_), .O(new_n37_));
  oai22  g22(.a(new_n37_), .b(x0), .c(new_n32_), .d(x1), .O(z2));
  nand2  g23(.a(new_n34_), .b(x7), .O(new_n39_));
  aoi22  g24(.a(new_n39_), .b(new_n28_), .c(new_n31_), .d(new_n16_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(x0), .c(new_n19_), .O(z3));
  nand3  g26(.a(new_n24_), .b(x2), .c(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n19_), .O(z4));
  nor2   g29(.a(x1), .b(new_n17_), .O(new_n45_));
  nor3   g30(.a(x4), .b(x2), .c(x0), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n45_), .c(new_n24_), .O(new_n47_));
  nand2  g32(.a(new_n34_), .b(new_n28_), .O(new_n48_));
  nand3  g33(.a(new_n35_), .b(new_n48_), .c(new_n22_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n47_), .c(new_n29_), .O(z5));
  nand2  g36(.a(new_n31_), .b(new_n16_), .O(new_n52_));
  oai21  g37(.a(new_n33_), .b(x3), .c(new_n28_), .O(new_n53_));
  nand3  g38(.a(new_n53_), .b(new_n52_), .c(new_n35_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n19_), .O(z6));
endmodule


