// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x4), .b(new_n20_), .c(x1), .O(new_n23_));
  nor2   g08(.a(x6), .b(x3), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x2), .O(new_n25_));
  nand2  g10(.a(x5), .b(x2), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x1), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n17_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n19_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(z1));
  nand2  g15(.a(x3), .b(x2), .O(new_n31_));
  nand2  g16(.a(x4), .b(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  inv1   g19(.a(x5), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n20_), .c(x2), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n19_), .O(new_n40_));
  nand2  g25(.a(x3), .b(new_n16_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n40_), .c(new_n34_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n29_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n16_), .c(x1), .O(new_n46_));
  nand4  g31(.a(new_n35_), .b(new_n20_), .c(x2), .d(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n17_), .O(new_n49_));
  nand4  g34(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z3));
  oai21  g36(.a(new_n16_), .b(x1), .c(x3), .O(new_n52_));
  inv1   g37(.a(x4), .O(new_n53_));
  nand4  g38(.a(new_n37_), .b(new_n53_), .c(x3), .d(x1), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n35_), .b(x3), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(x2), .c(new_n19_), .O(new_n57_));
  nand3  g42(.a(new_n57_), .b(new_n55_), .c(new_n52_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n17_), .O(new_n59_));
  oai21  g44(.a(new_n16_), .b(new_n17_), .c(new_n20_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n19_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n59_), .O(z4));
  nand3  g47(.a(x2), .b(new_n19_), .c(x0), .O(new_n63_));
  nand4  g48(.a(new_n53_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n20_), .O(new_n66_));
  aoi21  g51(.a(new_n32_), .b(new_n31_), .c(new_n19_), .O(new_n67_));
  oai22  g52(.a(new_n26_), .b(x1), .c(new_n24_), .d(x2), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n67_), .c(new_n17_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n66_), .c(new_n29_), .O(z5));
  nand3  g55(.a(new_n53_), .b(new_n20_), .c(new_n16_), .O(new_n71_));
  aoi21  g56(.a(new_n71_), .b(new_n31_), .c(new_n19_), .O(new_n72_));
  nand2  g57(.a(new_n47_), .b(new_n41_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n72_), .c(new_n17_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n61_), .O(z6));
endmodule


