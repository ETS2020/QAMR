// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(x7), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n17_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n21_), .c(new_n16_), .O(z0));
  nand2  g10(.a(new_n22_), .b(x5), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n16_), .O(new_n27_));
  nor2   g12(.a(new_n22_), .b(x3), .O(new_n28_));
  nand3  g13(.a(new_n22_), .b(x6), .c(new_n19_), .O(new_n29_));
  oai21  g14(.a(new_n28_), .b(new_n18_), .c(new_n29_), .O(new_n30_));
  aoi22  g15(.a(new_n30_), .b(new_n16_), .c(new_n27_), .d(new_n18_), .O(new_n31_));
  nor2   g16(.a(new_n19_), .b(x2), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n23_), .c(new_n28_), .O(new_n33_));
  oai21  g18(.a(new_n31_), .b(x0), .c(new_n33_), .O(z1));
  inv1   g19(.a(x5), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x2), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n16_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n19_), .c(new_n18_), .O(new_n40_));
  nand3  g25(.a(x4), .b(new_n16_), .c(x1), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(new_n40_), .c(x7), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n20_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n33_), .O(z2));
  inv1   g29(.a(new_n28_), .O(new_n45_));
  nand2  g30(.a(new_n16_), .b(x1), .O(new_n46_));
  nand4  g31(.a(new_n35_), .b(new_n19_), .c(x2), .d(new_n18_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n46_), .c(x0), .O(new_n48_));
  nand3  g33(.a(new_n23_), .b(new_n19_), .c(x2), .O(new_n49_));
  inv1   g34(.a(new_n49_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(new_n48_), .c(new_n22_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n45_), .O(z3));
  inv1   g37(.a(new_n32_), .O(new_n53_));
  nand3  g38(.a(new_n22_), .b(new_n19_), .c(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n55_));
  aoi21  g40(.a(new_n35_), .b(x3), .c(new_n16_), .O(new_n56_));
  nor3   g41(.a(x6), .b(x3), .c(x2), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n22_), .O(new_n58_));
  nand2  g43(.a(x3), .b(x2), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n55_), .c(new_n18_), .O(new_n61_));
  nor2   g46(.a(x7), .b(x2), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(x3), .c(x1), .O(new_n63_));
  nand4  g48(.a(new_n22_), .b(x6), .c(new_n19_), .d(new_n16_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n17_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n61_), .O(z4));
  nand2  g52(.a(new_n27_), .b(new_n17_), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n55_), .c(new_n18_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n66_), .c(new_n45_), .O(z5));
  nand3  g56(.a(new_n22_), .b(new_n35_), .c(new_n19_), .O(new_n72_));
  nor3   g57(.a(new_n72_), .b(new_n16_), .c(x0), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n55_), .c(new_n18_), .O(new_n74_));
  inv1   g59(.a(x4), .O(new_n75_));
  nand2  g60(.a(new_n22_), .b(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(x2), .c(new_n19_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n17_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n74_), .O(z6));
endmodule


