// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_;
  nand2  g00(.a(x3), .b(x1), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  oai21  g04(.a(new_n16_), .b(x0), .c(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x2), .O(new_n21_));
  nor2   g06(.a(x6), .b(x3), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  nand2  g09(.a(x3), .b(x2), .O(new_n25_));
  nand2  g10(.a(x5), .b(new_n18_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  aoi21  g12(.a(x3), .b(new_n17_), .c(x2), .O(new_n28_));
  aoi21  g13(.a(new_n27_), .b(new_n17_), .c(new_n28_), .O(new_n29_));
  inv1   g14(.a(x0), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n30_), .O(new_n31_));
  inv1   g16(.a(x2), .O(new_n32_));
  nand2  g17(.a(x3), .b(new_n32_), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n31_), .c(new_n22_), .O(new_n35_));
  oai21  g20(.a(new_n29_), .b(x0), .c(new_n35_), .O(z1));
  nor2   g21(.a(x5), .b(x3), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(x2), .c(new_n30_), .O(new_n38_));
  oai21  g23(.a(new_n33_), .b(new_n30_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  inv1   g25(.a(x4), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x2), .c(new_n18_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(x1), .c(new_n30_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n40_), .c(new_n23_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n32_), .c(x1), .O(new_n46_));
  nand3  g31(.a(new_n37_), .b(x2), .c(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  nand3  g34(.a(x2), .b(new_n17_), .c(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x6), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z3));
  nand2  g38(.a(new_n18_), .b(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n33_), .c(new_n30_), .O(new_n55_));
  aoi21  g40(.a(x5), .b(new_n18_), .c(x2), .O(new_n56_));
  nor2   g41(.a(new_n56_), .b(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n17_), .O(new_n58_));
  oai21  g43(.a(x3), .b(x2), .c(new_n16_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n30_), .c(new_n22_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n58_), .O(z4));
  aoi21  g46(.a(new_n26_), .b(new_n25_), .c(x0), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n55_), .c(new_n17_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z5));
  inv1   g49(.a(new_n38_), .O(new_n65_));
  oai21  g50(.a(new_n55_), .b(new_n65_), .c(new_n17_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x1), .c(new_n30_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n66_), .c(new_n23_), .O(z6));
endmodule


