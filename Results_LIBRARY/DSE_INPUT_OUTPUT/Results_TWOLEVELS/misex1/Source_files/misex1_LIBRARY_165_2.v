// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x5), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g07(.a(x2), .b(new_n17_), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n16_), .c(x1), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x0), .O(new_n25_));
  nand4  g10(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n26_));
  inv1   g11(.a(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(x5), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n17_), .O(new_n30_));
  nand4  g15(.a(new_n30_), .b(new_n18_), .c(new_n16_), .d(new_n20_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(z1));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n29_), .b(new_n18_), .c(new_n16_), .O(new_n35_));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x1), .O(new_n36_));
  nand2  g21(.a(x5), .b(x3), .O(new_n37_));
  nand2  g22(.a(x4), .b(new_n18_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n37_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(x1), .c(new_n36_), .O(new_n40_));
  nand2  g25(.a(new_n17_), .b(x0), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x2), .c(x5), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x3), .O(new_n43_));
  oai21  g28(.a(new_n40_), .b(x0), .c(new_n43_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n16_), .c(x1), .O(new_n46_));
  oai21  g31(.a(new_n34_), .b(x1), .c(new_n46_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n18_), .b(x2), .O(new_n49_));
  or2    g34(.a(new_n49_), .b(new_n41_), .O(new_n50_));
  nand2  g35(.a(new_n33_), .b(x3), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(z3));
  nand3  g37(.a(x5), .b(x3), .c(new_n16_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n49_), .c(new_n20_), .O(new_n54_));
  aoi21  g39(.a(new_n35_), .b(new_n16_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n18_), .b(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n37_), .c(new_n17_), .O(new_n58_));
  nand3  g43(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n20_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n56_), .c(new_n51_), .O(z4));
  nor3   g47(.a(new_n33_), .b(new_n16_), .c(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n54_), .c(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(z5));
  nand3  g50(.a(new_n33_), .b(x2), .c(new_n20_), .O(new_n66_));
  inv1   g51(.a(new_n66_), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n54_), .c(new_n17_), .O(new_n68_));
  inv1   g53(.a(x4), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n18_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(x2), .c(new_n37_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(x1), .c(new_n20_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n68_), .c(new_n51_), .O(z6));
endmodule


