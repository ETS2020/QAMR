// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  inv1   g06(.a(x6), .O(new_n22_));
  nor2   g07(.a(x1), .b(new_n17_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n21_), .c(new_n16_), .O(z0));
  nand2  g10(.a(new_n22_), .b(x5), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n18_), .O(new_n28_));
  oai21  g13(.a(x4), .b(new_n19_), .c(new_n22_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(new_n16_), .c(x1), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n28_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n17_), .O(new_n33_));
  nand3  g18(.a(new_n23_), .b(x3), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(z1));
  nand2  g20(.a(x5), .b(x2), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n18_), .O(new_n37_));
  nand3  g22(.a(x4), .b(new_n16_), .c(x1), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n37_), .c(x6), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n20_), .c(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n34_), .O(z2));
  inv1   g26(.a(x4), .O(new_n42_));
  nand3  g27(.a(new_n22_), .b(new_n42_), .c(new_n19_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(x7), .c(x2), .O(new_n44_));
  inv1   g29(.a(x5), .O(new_n45_));
  nand3  g30(.a(new_n22_), .b(new_n45_), .c(new_n19_), .O(new_n46_));
  nor3   g31(.a(new_n46_), .b(new_n16_), .c(x1), .O(new_n47_));
  aoi21  g32(.a(new_n44_), .b(x1), .c(new_n47_), .O(new_n48_));
  inv1   g33(.a(new_n23_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n16_), .c(new_n22_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n19_), .O(new_n51_));
  oai21  g36(.a(new_n48_), .b(x0), .c(new_n51_), .O(z3));
  nand2  g37(.a(x3), .b(new_n16_), .O(new_n53_));
  nand3  g38(.a(new_n22_), .b(new_n19_), .c(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n53_), .c(new_n17_), .O(new_n55_));
  aoi21  g40(.a(x5), .b(x2), .c(new_n19_), .O(new_n56_));
  nand2  g41(.a(x3), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n56_), .b(x6), .c(new_n57_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n17_), .c(new_n55_), .O(new_n59_));
  oai21  g44(.a(new_n29_), .b(x2), .c(new_n19_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(x1), .c(new_n17_), .O(new_n61_));
  oai21  g46(.a(new_n59_), .b(x1), .c(new_n61_), .O(z4));
  nand2  g47(.a(new_n54_), .b(new_n53_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(x0), .O(new_n64_));
  nand2  g49(.a(new_n27_), .b(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n18_), .O(new_n67_));
  nand2  g52(.a(x6), .b(new_n19_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(z5));
  nor3   g54(.a(new_n46_), .b(new_n16_), .c(x0), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n55_), .c(new_n18_), .O(new_n71_));
  oai21  g56(.a(new_n43_), .b(x2), .c(new_n19_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(x1), .c(new_n17_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n71_), .O(z6));
endmodule


