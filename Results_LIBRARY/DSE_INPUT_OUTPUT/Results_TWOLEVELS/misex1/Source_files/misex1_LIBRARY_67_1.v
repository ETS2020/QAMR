// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nor2   g07(.a(new_n19_), .b(x2), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  nor2   g10(.a(x6), .b(x1), .O(new_n26_));
  nor3   g11(.a(new_n26_), .b(x3), .c(x2), .O(new_n27_));
  inv1   g12(.a(x2), .O(new_n28_));
  nor2   g13(.a(x5), .b(x3), .O(new_n29_));
  nor3   g14(.a(new_n29_), .b(new_n28_), .c(x1), .O(new_n30_));
  oai21  g15(.a(new_n30_), .b(new_n27_), .c(new_n16_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n24_), .O(z1));
  nand2  g17(.a(x3), .b(x2), .O(new_n33_));
  nand3  g18(.a(x4), .b(new_n19_), .c(new_n28_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(x1), .O(new_n36_));
  inv1   g21(.a(x5), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  oai21  g23(.a(x6), .b(x2), .c(new_n38_), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n19_), .c(new_n18_), .O(new_n40_));
  aoi21  g25(.a(new_n40_), .b(new_n36_), .c(x0), .O(z2));
  nand2  g26(.a(x7), .b(x4), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n28_), .c(x1), .O(new_n43_));
  oai21  g28(.a(new_n38_), .b(x1), .c(new_n43_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  nand3  g30(.a(x2), .b(new_n18_), .c(x0), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n45_), .c(x3), .O(z3));
  aoi21  g32(.a(new_n19_), .b(x1), .c(new_n28_), .O(new_n48_));
  nor2   g33(.a(x6), .b(new_n18_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n18_), .c(x3), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n28_), .c(new_n48_), .O(new_n51_));
  nand4  g36(.a(new_n19_), .b(x2), .c(new_n18_), .d(x0), .O(new_n52_));
  oai21  g37(.a(new_n51_), .b(x0), .c(new_n52_), .O(z4));
  aoi21  g38(.a(x5), .b(new_n18_), .c(x3), .O(new_n54_));
  nor2   g39(.a(new_n54_), .b(new_n28_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n27_), .c(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n52_), .O(new_n57_));
  nor2   g42(.a(new_n57_), .b(new_n23_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z5));
  inv1   g44(.a(x4), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n19_), .c(new_n28_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n33_), .c(new_n18_), .O(new_n62_));
  nand3  g47(.a(new_n29_), .b(x2), .c(new_n18_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n62_), .c(new_n16_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n58_), .O(z6));
endmodule


