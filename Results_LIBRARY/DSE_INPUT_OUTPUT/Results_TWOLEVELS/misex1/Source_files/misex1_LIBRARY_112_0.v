// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n51_, new_n52_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  oai21  g02(.a(x1), .b(new_n17_), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  nand4  g04(.a(x3), .b(x2), .c(x1), .d(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n19_), .O(z0));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nand3  g08(.a(x3), .b(new_n23_), .c(x1), .O(new_n24_));
  oai21  g09(.a(new_n22_), .b(x1), .c(new_n24_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  oai21  g11(.a(x1), .b(new_n17_), .c(x3), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n23_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n26_), .O(z1));
  inv1   g14(.a(x1), .O(new_n30_));
  nor2   g15(.a(new_n16_), .b(new_n30_), .O(new_n31_));
  nor3   g16(.a(x5), .b(x3), .c(x1), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n31_), .c(new_n17_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n28_), .O(z2));
  inv1   g19(.a(x7), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n23_), .c(x1), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n32_), .c(new_n17_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n19_), .O(z3));
  aoi21  g24(.a(new_n23_), .b(new_n30_), .c(x0), .O(new_n40_));
  nand3  g25(.a(new_n23_), .b(new_n30_), .c(x0), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n40_), .c(x3), .O(new_n43_));
  nand3  g28(.a(new_n16_), .b(x2), .c(new_n30_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z4));
  aoi21  g30(.a(x3), .b(x2), .c(new_n17_), .O(new_n46_));
  nor2   g31(.a(new_n22_), .b(x0), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n46_), .c(new_n30_), .O(new_n48_));
  aoi22  g33(.a(new_n31_), .b(new_n17_), .c(new_n16_), .d(new_n23_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z5));
  nor3   g35(.a(x5), .b(x3), .c(x0), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n46_), .c(new_n30_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n49_), .O(z6));
endmodule


