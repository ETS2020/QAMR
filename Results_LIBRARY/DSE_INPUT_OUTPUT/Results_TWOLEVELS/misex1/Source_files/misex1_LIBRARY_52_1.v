// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(new_n18_), .b(new_n17_), .c(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand4  g06(.a(x3), .b(x2), .c(x1), .d(new_n17_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(x2), .b(new_n17_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(new_n24_), .c(new_n19_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n16_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  oai21  g13(.a(x4), .b(new_n16_), .c(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  inv1   g15(.a(x4), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n19_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x1), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n18_), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n27_), .O(z1));
  nor2   g21(.a(x5), .b(x3), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  aoi21  g23(.a(new_n37_), .b(x2), .c(new_n38_), .O(new_n39_));
  nor2   g24(.a(new_n39_), .b(x1), .O(new_n40_));
  aoi21  g25(.a(x4), .b(new_n18_), .c(x3), .O(new_n41_));
  nor2   g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n40_), .c(new_n17_), .O(new_n43_));
  nand2  g28(.a(x3), .b(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z2));
  nand2  g30(.a(new_n32_), .b(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n18_), .c(x1), .O(new_n47_));
  nand3  g32(.a(new_n37_), .b(x2), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n17_), .O(new_n50_));
  nor2   g35(.a(x3), .b(new_n18_), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n16_), .c(x0), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z3));
  oai21  g38(.a(x3), .b(new_n18_), .c(x1), .O(new_n54_));
  oai21  g39(.a(new_n38_), .b(new_n51_), .c(new_n16_), .O(new_n55_));
  nand3  g40(.a(x6), .b(new_n19_), .c(new_n18_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n21_), .O(z4));
  nand2  g44(.a(new_n24_), .b(new_n17_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(x2), .c(new_n16_), .O(new_n61_));
  nand3  g46(.a(new_n29_), .b(new_n18_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n19_), .O(new_n64_));
  nand2  g49(.a(new_n42_), .b(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n64_), .O(z5));
  nand2  g51(.a(x3), .b(x1), .O(new_n67_));
  nand2  g52(.a(new_n37_), .b(new_n16_), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n67_), .c(new_n18_), .O(new_n69_));
  oai21  g54(.a(new_n31_), .b(x3), .c(new_n18_), .O(new_n70_));
  nor2   g55(.a(new_n70_), .b(new_n16_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n69_), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n21_), .O(z6));
endmodule


