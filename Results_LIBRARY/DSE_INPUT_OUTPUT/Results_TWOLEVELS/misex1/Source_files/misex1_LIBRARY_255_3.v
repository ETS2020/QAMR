// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x7), .O(new_n20_));
  nor2   g05(.a(new_n17_), .b(x0), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n20_), .c(x3), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  nand2  g09(.a(new_n20_), .b(x1), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n18_), .c(new_n17_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n25_), .c(x2), .O(new_n27_));
  oai21  g12(.a(x5), .b(x3), .c(x2), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n24_), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n24_), .O(new_n31_));
  nand2  g16(.a(x3), .b(new_n16_), .O(new_n32_));
  inv1   g17(.a(new_n32_), .O(new_n33_));
  aoi22  g18(.a(new_n33_), .b(new_n31_), .c(x7), .d(x1), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n30_), .O(z1));
  nand3  g20(.a(new_n16_), .b(new_n17_), .c(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n21_), .c(x3), .O(new_n38_));
  inv1   g23(.a(x4), .O(new_n39_));
  nand2  g24(.a(new_n16_), .b(new_n24_), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n39_), .c(new_n20_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x1), .O(new_n42_));
  inv1   g27(.a(x5), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x2), .O(new_n44_));
  oai21  g29(.a(x6), .b(x2), .c(new_n44_), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n18_), .c(new_n17_), .d(new_n24_), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n42_), .c(new_n38_), .O(z2));
  nand3  g32(.a(new_n20_), .b(new_n16_), .c(x1), .O(new_n48_));
  nor2   g33(.a(x5), .b(x3), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(x2), .c(new_n17_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n24_), .O(new_n52_));
  nand3  g37(.a(new_n31_), .b(new_n18_), .c(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(z3));
  nand2  g39(.a(new_n18_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n32_), .c(new_n24_), .O(new_n56_));
  aoi21  g41(.a(x3), .b(new_n16_), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n17_), .O(new_n58_));
  nor2   g43(.a(x3), .b(new_n16_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(x0), .c(new_n20_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(x1), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n58_), .O(z4));
  nand3  g47(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n28_), .c(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n56_), .c(new_n17_), .O(new_n65_));
  nor2   g50(.a(new_n59_), .b(x7), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x1), .c(new_n24_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z5));
  nand3  g53(.a(new_n49_), .b(x2), .c(new_n24_), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n56_), .c(new_n17_), .O(new_n71_));
  nand3  g56(.a(new_n20_), .b(new_n39_), .c(new_n16_), .O(new_n72_));
  aoi21  g57(.a(new_n72_), .b(new_n18_), .c(x0), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(x7), .c(x1), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n71_), .O(z6));
endmodule


