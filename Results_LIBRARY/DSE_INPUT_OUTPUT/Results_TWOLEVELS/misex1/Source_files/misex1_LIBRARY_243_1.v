// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n45_, new_n47_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(x2), .O(new_n20_));
  inv1   g05(.a(new_n20_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n18_), .c(x0), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n17_), .O(z0));
  oai21  g08(.a(x5), .b(x3), .c(x2), .O(new_n24_));
  inv1   g09(.a(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n19_), .c(new_n25_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n24_), .c(x0), .O(new_n27_));
  inv1   g12(.a(new_n27_), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n25_), .O(new_n29_));
  inv1   g14(.a(new_n29_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x0), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(new_n28_), .c(x1), .O(z1));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  oai21  g19(.a(x6), .b(x2), .c(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n19_), .c(new_n16_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n31_), .c(x1), .O(z2));
  nor2   g22(.a(x5), .b(x3), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(x2), .c(x1), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(x0), .c(new_n22_), .O(z3));
  aoi21  g25(.a(new_n29_), .b(new_n20_), .c(new_n16_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n19_), .b(x2), .c(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n42_), .c(x1), .O(z4));
  oai21  g29(.a(new_n41_), .b(new_n27_), .c(new_n18_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n17_), .O(z5));
  nand3  g31(.a(new_n38_), .b(x2), .c(new_n16_), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n42_), .c(x1), .O(z6));
endmodule


