// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_;
  nand2  g00(.a(x3), .b(x2), .O(new_n16_));
  oai21  g01(.a(new_n16_), .b(x0), .c(x4), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x1), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x2), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n19_), .c(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n18_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n20_), .c(new_n26_), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(new_n25_), .c(x1), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  nor2   g14(.a(new_n29_), .b(x2), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(x1), .c(new_n28_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n26_), .c(new_n19_), .d(x0), .O(new_n32_));
  oai21  g17(.a(new_n31_), .b(x0), .c(new_n32_), .O(z1));
  inv1   g18(.a(x0), .O(new_n34_));
  nand3  g19(.a(new_n21_), .b(x4), .c(x1), .O(new_n35_));
  inv1   g20(.a(x5), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(x2), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n26_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n20_), .c(new_n19_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n35_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n32_), .O(z2));
  nor2   g29(.a(x5), .b(x3), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x2), .c(new_n19_), .O(new_n46_));
  inv1   g31(.a(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n26_), .c(x1), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n34_), .O(new_n50_));
  nand2  g35(.a(new_n29_), .b(x1), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n23_), .O(z3));
  nand2  g37(.a(x3), .b(new_n26_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n21_), .c(new_n34_), .O(new_n54_));
  aoi21  g39(.a(x3), .b(new_n26_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n19_), .O(new_n56_));
  oai21  g41(.a(new_n22_), .b(x0), .c(x4), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z4));
  aoi21  g44(.a(new_n27_), .b(new_n25_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n54_), .c(new_n19_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n58_), .O(z5));
  nand3  g47(.a(new_n45_), .b(x2), .c(new_n34_), .O(new_n63_));
  inv1   g48(.a(new_n63_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n54_), .c(new_n19_), .O(new_n65_));
  nand4  g50(.a(x4), .b(x3), .c(x1), .d(new_n34_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n65_), .O(z6));
endmodule


