// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x4), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  nor2   g02(.a(x4), .b(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(x2), .c(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x0), .O(new_n22_));
  nand4  g07(.a(x3), .b(x2), .c(x1), .d(new_n22_), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n21_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x1), .O(new_n27_));
  aoi21  g12(.a(new_n16_), .b(x1), .c(x6), .O(new_n28_));
  oai22  g13(.a(new_n28_), .b(x3), .c(new_n18_), .d(new_n17_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n25_), .c(new_n27_), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n22_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n16_), .c(x3), .d(new_n25_), .O(new_n32_));
  oai21  g17(.a(new_n30_), .b(x0), .c(new_n32_), .O(z1));
  inv1   g18(.a(x3), .O(new_n34_));
  nor2   g19(.a(x5), .b(new_n25_), .O(new_n35_));
  nor2   g20(.a(x6), .b(x2), .O(new_n36_));
  oai21  g21(.a(new_n36_), .b(new_n35_), .c(new_n34_), .O(new_n37_));
  oai21  g22(.a(new_n16_), .b(x2), .c(new_n34_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  oai21  g24(.a(new_n37_), .b(x1), .c(new_n39_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n22_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n32_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n25_), .c(x1), .O(new_n44_));
  nor2   g29(.a(x5), .b(x3), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x2), .c(new_n17_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n22_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n21_), .O(z3));
  oai21  g34(.a(x2), .b(new_n17_), .c(new_n22_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x4), .O(new_n51_));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n16_), .c(x0), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  aoi21  g39(.a(new_n37_), .b(new_n26_), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n54_), .c(new_n17_), .O(new_n56_));
  oai21  g41(.a(x4), .b(x2), .c(new_n34_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nand3  g43(.a(x6), .b(new_n34_), .c(new_n25_), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n22_), .O(new_n61_));
  nand3  g46(.a(new_n61_), .b(new_n56_), .c(new_n51_), .O(z4));
  nor2   g47(.a(new_n26_), .b(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n54_), .c(new_n17_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n61_), .c(new_n51_), .O(z5));
  nand3  g50(.a(new_n45_), .b(x2), .c(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n53_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n17_), .O(new_n68_));
  oai21  g53(.a(new_n58_), .b(x0), .c(new_n68_), .O(z6));
endmodule


