// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nor2   g05(.a(x5), .b(new_n18_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(x1), .c(new_n20_), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n19_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x3), .O(new_n25_));
  nand2  g10(.a(x5), .b(new_n18_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n17_), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  nand2  g14(.a(x5), .b(x3), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n18_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n31_), .c(x2), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n29_), .c(new_n20_), .O(new_n34_));
  nor2   g19(.a(x1), .b(new_n20_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n21_), .c(new_n16_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n34_), .O(z1));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  aoi21  g23(.a(new_n24_), .b(x2), .c(new_n38_), .O(new_n39_));
  nand3  g24(.a(x4), .b(new_n16_), .c(x1), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x1), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n21_), .b(x1), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  aoi21  g28(.a(new_n41_), .b(new_n18_), .c(new_n43_), .O(new_n44_));
  inv1   g29(.a(new_n35_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(x2), .c(new_n24_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(x3), .O(new_n47_));
  oai21  g32(.a(new_n44_), .b(x0), .c(new_n47_), .O(z2));
  inv1   g33(.a(x4), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n18_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(x7), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n16_), .c(x1), .O(new_n52_));
  nand4  g37(.a(new_n24_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n20_), .O(new_n55_));
  nor2   g40(.a(x3), .b(new_n16_), .O(new_n56_));
  nor2   g41(.a(new_n24_), .b(new_n18_), .O(new_n57_));
  aoi21  g42(.a(new_n56_), .b(new_n35_), .c(new_n57_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n55_), .O(z3));
  inv1   g44(.a(new_n57_), .O(new_n60_));
  nand2  g45(.a(new_n21_), .b(new_n16_), .O(new_n61_));
  inv1   g46(.a(new_n56_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n61_), .c(new_n20_), .O(new_n63_));
  nand2  g48(.a(new_n30_), .b(x2), .O(new_n64_));
  inv1   g49(.a(x6), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n18_), .c(new_n16_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n64_), .c(x0), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n63_), .c(new_n17_), .O(new_n68_));
  nand2  g53(.a(new_n18_), .b(new_n16_), .O(new_n69_));
  aoi21  g54(.a(new_n69_), .b(new_n25_), .c(new_n17_), .O(new_n70_));
  nand3  g55(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n71_));
  inv1   g56(.a(new_n71_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n70_), .c(new_n20_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(new_n68_), .c(new_n60_), .O(z4));
  nand3  g59(.a(new_n27_), .b(x2), .c(new_n20_), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n63_), .c(new_n17_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n73_), .O(z5));
  nand4  g63(.a(new_n24_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n79_));
  inv1   g64(.a(new_n79_), .O(new_n80_));
  oai21  g65(.a(new_n80_), .b(new_n63_), .c(new_n17_), .O(new_n81_));
  oai21  g66(.a(new_n50_), .b(x2), .c(new_n25_), .O(new_n82_));
  nand3  g67(.a(new_n82_), .b(x1), .c(new_n20_), .O(new_n83_));
  nand3  g68(.a(new_n83_), .b(new_n81_), .c(new_n60_), .O(z6));
endmodule


