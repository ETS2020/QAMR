// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  inv1   g08(.a(x5), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x2), .c(new_n18_), .O(new_n26_));
  nand3  g11(.a(x3), .b(new_n16_), .c(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  nand4  g14(.a(x3), .b(new_n16_), .c(new_n18_), .d(x0), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(z1));
  nor2   g16(.a(x5), .b(x3), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(x2), .c(new_n18_), .O(new_n33_));
  inv1   g18(.a(new_n33_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n20_), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n30_), .O(z2));
  inv1   g21(.a(x7), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(x3), .c(new_n16_), .d(x1), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n33_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand4  g25(.a(new_n19_), .b(x2), .c(new_n18_), .d(x0), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z3));
  xor2a  g27(.a(x3), .b(x2), .O(new_n43_));
  nor2   g28(.a(new_n16_), .b(x0), .O(new_n44_));
  aoi21  g29(.a(new_n43_), .b(x0), .c(new_n44_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(x1), .c(new_n21_), .O(z4));
  nand2  g31(.a(new_n43_), .b(x0), .O(new_n47_));
  nand3  g32(.a(new_n25_), .b(x2), .c(new_n17_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n18_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n21_), .O(z5));
  nand3  g36(.a(new_n32_), .b(x2), .c(new_n17_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  nand2  g39(.a(new_n19_), .b(new_n16_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(new_n21_), .O(z6));
endmodule


