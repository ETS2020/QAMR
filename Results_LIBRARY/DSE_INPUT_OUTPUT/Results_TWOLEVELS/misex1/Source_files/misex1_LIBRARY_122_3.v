// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  aoi21  g02(.a(new_n17_), .b(x0), .c(x3), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(new_n16_), .O(z0));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nor2   g06(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  inv1   g07(.a(x5), .O(new_n23_));
  nor3   g08(.a(new_n23_), .b(new_n16_), .c(x0), .O(new_n24_));
  oai21  g09(.a(new_n24_), .b(new_n22_), .c(new_n17_), .O(new_n25_));
  nand2  g10(.a(x1), .b(new_n20_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n16_), .c(new_n21_), .O(new_n27_));
  oai21  g12(.a(x4), .b(new_n21_), .c(x1), .O(new_n28_));
  nand2  g13(.a(x6), .b(new_n21_), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(x2), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n20_), .c(new_n27_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n25_), .O(z1));
  nand2  g17(.a(x4), .b(new_n16_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n21_), .c(new_n17_), .O(new_n34_));
  nand2  g19(.a(new_n23_), .b(x2), .O(new_n35_));
  inv1   g20(.a(x6), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n35_), .c(x3), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n17_), .c(new_n34_), .O(new_n39_));
  oai21  g24(.a(x1), .b(new_n20_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x3), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(x0), .c(new_n41_), .O(z2));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n16_), .c(x1), .O(new_n44_));
  nor2   g29(.a(x5), .b(x3), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x2), .c(new_n17_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand4  g33(.a(new_n21_), .b(x2), .c(new_n17_), .d(x0), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(z3));
  aoi21  g35(.a(new_n21_), .b(new_n16_), .c(new_n20_), .O(new_n51_));
  oai21  g36(.a(x5), .b(new_n21_), .c(x2), .O(new_n52_));
  nand3  g37(.a(new_n36_), .b(new_n21_), .c(new_n16_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n52_), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n51_), .c(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n31_), .O(z4));
  oai21  g41(.a(new_n51_), .b(new_n24_), .c(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n31_), .O(z5));
  nand3  g43(.a(new_n45_), .b(x2), .c(new_n20_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n51_), .c(new_n17_), .O(new_n61_));
  oai21  g46(.a(x4), .b(x2), .c(new_n21_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(x1), .c(new_n20_), .O(new_n63_));
  nand2  g48(.a(x3), .b(x2), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(z6));
endmodule


