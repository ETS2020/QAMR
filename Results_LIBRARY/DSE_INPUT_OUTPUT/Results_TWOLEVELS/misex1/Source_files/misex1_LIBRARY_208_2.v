// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:41 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x6), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  oai21  g07(.a(x5), .b(x3), .c(x2), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n17_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x6), .c(new_n16_), .O(new_n25_));
  oai21  g10(.a(new_n23_), .b(x1), .c(new_n25_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n20_), .O(new_n27_));
  nand2  g12(.a(new_n17_), .b(x0), .O(new_n28_));
  nand2  g13(.a(x3), .b(new_n16_), .O(new_n29_));
  or2    g14(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n27_), .O(z1));
  nor2   g16(.a(x5), .b(x3), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(x2), .c(new_n20_), .O(new_n33_));
  oai21  g18(.a(new_n29_), .b(new_n20_), .c(new_n33_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n17_), .O(new_n35_));
  aoi22  g20(.a(x6), .b(x3), .c(x4), .d(new_n16_), .O(new_n36_));
  inv1   g21(.a(x6), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n18_), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(new_n36_), .b(new_n17_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n20_), .O(new_n40_));
  nor2   g25(.a(x6), .b(new_n17_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n40_), .c(new_n35_), .O(z2));
  oai21  g28(.a(x4), .b(x3), .c(x7), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(x6), .c(new_n16_), .d(x1), .O(new_n45_));
  nand3  g30(.a(new_n32_), .b(x2), .c(new_n17_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n20_), .O(new_n48_));
  nand2  g33(.a(new_n18_), .b(x2), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n28_), .c(new_n48_), .O(z3));
  aoi21  g35(.a(new_n49_), .b(new_n29_), .c(new_n20_), .O(new_n51_));
  nor2   g36(.a(new_n16_), .b(x0), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n51_), .c(new_n17_), .O(new_n53_));
  nand2  g38(.a(x3), .b(x1), .O(new_n54_));
  oai22  g39(.a(new_n54_), .b(new_n37_), .c(x3), .d(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n20_), .c(new_n41_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n53_), .O(z4));
  nor2   g42(.a(new_n23_), .b(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n51_), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(new_n18_), .b(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n54_), .c(new_n37_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n20_), .c(new_n41_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z5));
  inv1   g48(.a(new_n33_), .O(new_n64_));
  oai21  g49(.a(new_n51_), .b(new_n64_), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(x4), .b(x2), .c(new_n18_), .O(new_n66_));
  nand4  g51(.a(new_n66_), .b(x6), .c(x1), .d(new_n20_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(z6));
endmodule


