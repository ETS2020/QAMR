// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x2), .c(x1), .O(new_n17_));
  nand2  g02(.a(new_n17_), .b(x4), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  nor2   g06(.a(x3), .b(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n20_), .c(x0), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n19_), .O(z0));
  inv1   g09(.a(x3), .O(new_n25_));
  inv1   g10(.a(x5), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand3  g12(.a(new_n27_), .b(x2), .c(new_n20_), .O(new_n28_));
  aoi21  g13(.a(x6), .b(new_n25_), .c(x1), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(x2), .c(new_n28_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(x4), .c(new_n16_), .O(new_n31_));
  nor2   g16(.a(new_n25_), .b(x2), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n20_), .c(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n31_), .O(z1));
  oai21  g19(.a(x3), .b(new_n21_), .c(x1), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  nand3  g21(.a(new_n26_), .b(x2), .c(new_n20_), .O(new_n37_));
  inv1   g22(.a(x6), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n21_), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n37_), .c(x3), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n36_), .c(x4), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x0), .c(new_n33_), .O(z2));
  inv1   g27(.a(x4), .O(new_n43_));
  oai21  g28(.a(x5), .b(new_n43_), .c(new_n16_), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n25_), .c(x2), .d(new_n20_), .O(new_n45_));
  inv1   g30(.a(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n21_), .c(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x4), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n45_), .O(z3));
  oai21  g35(.a(new_n32_), .b(new_n22_), .c(x0), .O(new_n51_));
  nor2   g36(.a(x5), .b(new_n25_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n25_), .O(new_n53_));
  nand4  g38(.a(new_n53_), .b(x4), .c(x2), .d(new_n16_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n51_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n20_), .O(new_n56_));
  nand2  g41(.a(new_n25_), .b(new_n21_), .O(new_n57_));
  nand2  g42(.a(x3), .b(x1), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(x4), .O(new_n59_));
  nand2  g44(.a(new_n59_), .b(new_n16_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z4));
  nand4  g46(.a(new_n27_), .b(x4), .c(x2), .d(new_n16_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n51_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n20_), .O(new_n64_));
  nand3  g49(.a(x6), .b(new_n25_), .c(new_n21_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n35_), .c(x4), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n16_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n64_), .O(z5));
  inv1   g53(.a(new_n51_), .O(new_n69_));
  nand3  g54(.a(new_n26_), .b(x4), .c(new_n25_), .O(new_n70_));
  nor3   g55(.a(new_n70_), .b(new_n21_), .c(x0), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n69_), .c(new_n20_), .O(new_n72_));
  nand4  g57(.a(x4), .b(x3), .c(x1), .d(new_n16_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n72_), .O(z6));
endmodule


