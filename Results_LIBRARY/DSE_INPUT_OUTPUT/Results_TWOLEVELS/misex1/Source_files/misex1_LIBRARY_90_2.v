// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x7), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(x3), .c(new_n17_), .O(new_n20_));
  nor2   g05(.a(x3), .b(new_n18_), .O(new_n21_));
  nand3  g06(.a(new_n21_), .b(new_n16_), .c(x0), .O(new_n22_));
  oai21  g07(.a(new_n20_), .b(new_n16_), .c(new_n22_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  inv1   g10(.a(x3), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n26_), .c(new_n18_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n16_), .O(new_n29_));
  nor2   g14(.a(x4), .b(new_n26_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(x7), .c(new_n18_), .d(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n29_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nor2   g19(.a(new_n26_), .b(x2), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n16_), .c(x0), .O(new_n36_));
  nand2  g21(.a(new_n17_), .b(x1), .O(new_n37_));
  and2   g22(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n34_), .O(z1));
  inv1   g24(.a(x5), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x2), .O(new_n41_));
  oai21  g26(.a(x6), .b(x2), .c(new_n41_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n26_), .c(new_n16_), .O(new_n43_));
  inv1   g28(.a(x4), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x2), .c(new_n26_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(x7), .c(x1), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n43_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n24_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n36_), .O(z2));
  nand3  g34(.a(new_n40_), .b(x2), .c(new_n16_), .O(new_n50_));
  nand4  g35(.a(x7), .b(new_n44_), .c(new_n18_), .d(x1), .O(new_n51_));
  aoi21  g36(.a(new_n51_), .b(new_n50_), .c(x0), .O(new_n52_));
  nand3  g37(.a(x2), .b(new_n16_), .c(x0), .O(new_n53_));
  inv1   g38(.a(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n52_), .c(new_n26_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n37_), .O(z3));
  nor2   g41(.a(new_n35_), .b(new_n21_), .O(new_n57_));
  nor2   g42(.a(new_n57_), .b(new_n24_), .O(new_n58_));
  aoi21  g43(.a(x3), .b(new_n18_), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n58_), .c(new_n16_), .O(new_n60_));
  nor2   g45(.a(x3), .b(new_n18_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(x0), .c(x7), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(x1), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n60_), .O(z4));
  aoi21  g49(.a(new_n27_), .b(new_n25_), .c(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n58_), .c(new_n16_), .O(new_n66_));
  nor2   g51(.a(new_n61_), .b(new_n17_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x1), .c(new_n24_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z5));
  nand3  g54(.a(new_n19_), .b(new_n40_), .c(new_n26_), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n58_), .c(new_n16_), .O(new_n72_));
  oai21  g57(.a(x4), .b(x2), .c(new_n26_), .O(new_n73_));
  nand4  g58(.a(new_n73_), .b(x7), .c(x1), .d(new_n24_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n72_), .O(z6));
endmodule


