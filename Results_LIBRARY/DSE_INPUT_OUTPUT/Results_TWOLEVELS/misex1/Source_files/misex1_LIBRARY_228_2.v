// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  nand2  g01(.a(x2), .b(new_n16_), .O(new_n17_));
  oai21  g02(.a(new_n17_), .b(x3), .c(x4), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x1), .b(new_n20_), .O(new_n21_));
  nand2  g06(.a(x3), .b(x2), .O(new_n22_));
  oai21  g07(.a(new_n22_), .b(new_n21_), .c(new_n19_), .O(z0));
  inv1   g08(.a(x2), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(x1), .O(new_n26_));
  inv1   g11(.a(x4), .O(new_n27_));
  aoi21  g12(.a(new_n27_), .b(x1), .c(x6), .O(new_n28_));
  inv1   g13(.a(x3), .O(new_n29_));
  nand2  g14(.a(new_n27_), .b(new_n29_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  oai21  g16(.a(new_n28_), .b(x3), .c(new_n31_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n24_), .c(new_n26_), .O(new_n33_));
  nor2   g18(.a(new_n27_), .b(new_n29_), .O(new_n34_));
  nand4  g19(.a(new_n34_), .b(new_n24_), .c(new_n16_), .d(x0), .O(new_n35_));
  oai21  g20(.a(new_n33_), .b(x0), .c(new_n35_), .O(z1));
  nor2   g21(.a(x5), .b(new_n24_), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n37_), .c(new_n29_), .O(new_n39_));
  oai21  g24(.a(new_n27_), .b(x2), .c(new_n29_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(x1), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n20_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n35_), .O(z2));
  nand2  g29(.a(new_n30_), .b(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n24_), .c(x1), .O(new_n46_));
  nor2   g31(.a(x5), .b(x3), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x2), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n20_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n19_), .O(z3));
  nand2  g36(.a(new_n29_), .b(new_n24_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(new_n16_), .c(new_n20_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n27_), .O(new_n54_));
  xor2a  g39(.a(x3), .b(x2), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(x4), .c(x0), .O(new_n56_));
  inv1   g41(.a(new_n56_), .O(new_n57_));
  aoi21  g42(.a(new_n39_), .b(new_n25_), .c(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n57_), .c(new_n16_), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n29_), .c(new_n24_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n41_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n20_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n59_), .c(new_n54_), .O(z4));
  nor2   g48(.a(new_n25_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n57_), .c(new_n16_), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n62_), .c(new_n54_), .O(z5));
  nand3  g51(.a(new_n47_), .b(x2), .c(new_n20_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n56_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n16_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n29_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x1), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


