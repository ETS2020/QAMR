// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:55 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x6), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  nor2   g03(.a(new_n18_), .b(x0), .O(new_n19_));
  aoi21  g04(.a(new_n19_), .b(x3), .c(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n16_), .b(x0), .O(new_n21_));
  inv1   g06(.a(x3), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x2), .O(new_n23_));
  oai22  g08(.a(new_n23_), .b(new_n21_), .c(new_n20_), .d(new_n16_), .O(z0));
  inv1   g09(.a(x0), .O(new_n25_));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  oai21  g11(.a(new_n22_), .b(x1), .c(x6), .O(new_n27_));
  oai22  g12(.a(new_n27_), .b(x2), .c(new_n26_), .d(x1), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n25_), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n18_), .O(new_n30_));
  or2    g15(.a(new_n30_), .b(new_n21_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n29_), .O(z1));
  inv1   g17(.a(x5), .O(new_n33_));
  nor2   g18(.a(x6), .b(x2), .O(new_n34_));
  aoi21  g19(.a(new_n33_), .b(x2), .c(new_n34_), .O(new_n35_));
  nor2   g20(.a(new_n35_), .b(x3), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n22_), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(x6), .c(x1), .O(new_n39_));
  inv1   g24(.a(new_n39_), .O(new_n40_));
  aoi21  g25(.a(new_n36_), .b(new_n16_), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n41_), .b(x0), .c(new_n31_), .O(z2));
  nand3  g27(.a(new_n37_), .b(new_n18_), .c(x1), .O(new_n43_));
  nand3  g28(.a(new_n33_), .b(x2), .c(new_n16_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(x0), .O(new_n45_));
  nand3  g30(.a(x2), .b(new_n16_), .c(x0), .O(new_n46_));
  inv1   g31(.a(new_n46_), .O(new_n47_));
  oai21  g32(.a(new_n47_), .b(new_n45_), .c(new_n22_), .O(new_n48_));
  nor3   g33(.a(x7), .b(x2), .c(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n17_), .c(x1), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(z3));
  aoi21  g36(.a(new_n30_), .b(new_n23_), .c(new_n25_), .O(new_n52_));
  oai21  g37(.a(new_n35_), .b(x3), .c(new_n26_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n25_), .c(new_n52_), .O(new_n54_));
  nand2  g39(.a(x3), .b(x1), .O(new_n55_));
  oai21  g40(.a(x3), .b(x2), .c(new_n55_), .O(new_n56_));
  nand3  g41(.a(new_n56_), .b(x6), .c(new_n25_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  aoi21  g43(.a(new_n17_), .b(x1), .c(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n54_), .b(x1), .c(new_n59_), .O(z4));
  nor2   g45(.a(new_n26_), .b(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n52_), .c(new_n16_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n57_), .O(z5));
  nand3  g48(.a(new_n19_), .b(new_n33_), .c(new_n22_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n52_), .c(new_n16_), .O(new_n66_));
  nand2  g51(.a(new_n37_), .b(new_n22_), .O(new_n67_));
  oai22  g52(.a(new_n67_), .b(x2), .c(new_n17_), .d(new_n22_), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n25_), .c(new_n17_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n16_), .c(new_n66_), .O(z6));
endmodule


