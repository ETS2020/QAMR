// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand2  g05(.a(x7), .b(x3), .O(new_n21_));
  inv1   g06(.a(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(x1), .c(new_n20_), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g09(.a(x5), .b(new_n18_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n21_), .c(new_n16_), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  oai21  g12(.a(x7), .b(new_n18_), .c(x1), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x3), .c(new_n28_), .O(new_n29_));
  aoi22  g14(.a(new_n29_), .b(new_n16_), .c(new_n26_), .d(new_n17_), .O(new_n30_));
  nor2   g15(.a(x1), .b(new_n20_), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(x7), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x3), .O(new_n34_));
  oai21  g19(.a(new_n30_), .b(x0), .c(new_n34_), .O(z1));
  inv1   g20(.a(x5), .O(new_n36_));
  nor2   g21(.a(x6), .b(x2), .O(new_n37_));
  aoi21  g22(.a(new_n36_), .b(x2), .c(new_n37_), .O(new_n38_));
  nand3  g23(.a(x4), .b(new_n16_), .c(x1), .O(new_n39_));
  oai21  g24(.a(new_n38_), .b(x1), .c(new_n39_), .O(new_n40_));
  nand2  g25(.a(new_n22_), .b(x1), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  aoi21  g27(.a(new_n40_), .b(new_n18_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(x0), .c(new_n34_), .O(z2));
  inv1   g29(.a(x4), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n18_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(x1), .O(new_n48_));
  nand4  g33(.a(new_n36_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nor2   g36(.a(x3), .b(new_n16_), .O(new_n52_));
  nor2   g37(.a(x7), .b(new_n18_), .O(new_n53_));
  aoi21  g38(.a(new_n52_), .b(new_n31_), .c(new_n53_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n51_), .O(z3));
  nor2   g40(.a(new_n21_), .b(x2), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n52_), .c(x0), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(x7), .b(new_n18_), .c(x2), .O(new_n59_));
  nand3  g44(.a(new_n27_), .b(new_n18_), .c(new_n16_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n59_), .c(x0), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n58_), .c(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n18_), .b(new_n16_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n21_), .c(new_n17_), .O(new_n64_));
  nand3  g49(.a(x6), .b(new_n18_), .c(new_n16_), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n64_), .c(new_n20_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n62_), .O(z4));
  inv1   g53(.a(new_n53_), .O(new_n69_));
  nand2  g54(.a(new_n26_), .b(new_n20_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n57_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n17_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n67_), .c(new_n69_), .O(z5));
  nand4  g58(.a(new_n36_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n57_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n17_), .O(new_n76_));
  oai21  g61(.a(new_n46_), .b(x2), .c(new_n21_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(x1), .c(new_n20_), .O(new_n78_));
  nand2  g63(.a(new_n78_), .b(new_n76_), .O(z6));
endmodule


