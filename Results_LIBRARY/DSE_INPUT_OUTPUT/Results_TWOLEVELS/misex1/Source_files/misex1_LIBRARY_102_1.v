// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n17_), .O(new_n22_));
  nor2   g07(.a(x4), .b(x3), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n21_), .c(new_n16_), .O(z0));
  inv1   g10(.a(x4), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(x3), .c(x1), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n26_), .c(new_n19_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand2  g14(.a(x5), .b(new_n26_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n19_), .c(new_n16_), .O(new_n31_));
  aoi22  g16(.a(new_n31_), .b(new_n18_), .c(new_n29_), .d(new_n16_), .O(new_n32_));
  nor2   g17(.a(new_n19_), .b(x2), .O(new_n33_));
  nor2   g18(.a(new_n26_), .b(x3), .O(new_n34_));
  aoi21  g19(.a(new_n33_), .b(new_n22_), .c(new_n34_), .O(new_n35_));
  oai21  g20(.a(new_n32_), .b(x0), .c(new_n35_), .O(z1));
  inv1   g21(.a(x5), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(new_n26_), .c(x2), .O(new_n38_));
  inv1   g23(.a(x6), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n19_), .O(new_n42_));
  inv1   g27(.a(new_n42_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n18_), .c(new_n20_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(x0), .c(new_n35_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n16_), .c(x1), .O(new_n47_));
  nand2  g32(.a(x2), .b(new_n18_), .O(new_n48_));
  nand3  g33(.a(new_n37_), .b(new_n26_), .c(new_n19_), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n17_), .O(new_n51_));
  aoi21  g36(.a(new_n22_), .b(x2), .c(x4), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(new_n51_), .O(z3));
  inv1   g38(.a(new_n33_), .O(new_n54_));
  nand2  g39(.a(new_n23_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n54_), .c(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n30_), .b(new_n19_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x2), .O(new_n58_));
  aoi21  g43(.a(new_n42_), .b(new_n58_), .c(x0), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n56_), .c(new_n18_), .O(new_n60_));
  oai21  g45(.a(x4), .b(x2), .c(new_n19_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(x1), .O(new_n62_));
  nand4  g47(.a(x6), .b(new_n26_), .c(new_n19_), .d(new_n16_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n17_), .c(new_n34_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n60_), .O(z4));
  nand2  g51(.a(new_n64_), .b(new_n17_), .O(new_n67_));
  aoi21  g52(.a(new_n31_), .b(new_n17_), .c(new_n56_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(x1), .c(new_n67_), .O(z5));
  nor3   g54(.a(new_n49_), .b(new_n16_), .c(x0), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n56_), .c(new_n18_), .O(new_n71_));
  oai21  g56(.a(new_n62_), .b(x0), .c(new_n71_), .O(z6));
endmodule


