// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  oai21  g05(.a(new_n20_), .b(new_n16_), .c(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x5), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  inv1   g10(.a(x2), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x0), .O(new_n27_));
  nand3  g12(.a(x3), .b(new_n26_), .c(x0), .O(new_n28_));
  inv1   g13(.a(new_n28_), .O(new_n29_));
  oai21  g14(.a(new_n29_), .b(new_n27_), .c(new_n18_), .O(new_n30_));
  inv1   g15(.a(x4), .O(new_n31_));
  aoi21  g16(.a(new_n31_), .b(x1), .c(x6), .O(new_n32_));
  aoi21  g17(.a(x5), .b(x4), .c(x3), .O(new_n33_));
  oai22  g18(.a(new_n33_), .b(new_n18_), .c(new_n32_), .d(x3), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n26_), .c(new_n16_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n30_), .c(new_n24_), .O(z1));
  inv1   g21(.a(x6), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x5), .O(new_n38_));
  oai22  g23(.a(new_n38_), .b(new_n20_), .c(new_n33_), .d(new_n18_), .O(new_n39_));
  nand3  g24(.a(x3), .b(new_n18_), .c(x0), .O(new_n40_));
  inv1   g25(.a(new_n40_), .O(new_n41_));
  aoi21  g26(.a(new_n39_), .b(new_n16_), .c(new_n41_), .O(new_n42_));
  nand4  g27(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n43_));
  oai21  g28(.a(new_n42_), .b(x2), .c(new_n43_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand4  g30(.a(new_n45_), .b(new_n26_), .c(x1), .d(new_n16_), .O(new_n46_));
  nand3  g31(.a(x2), .b(new_n18_), .c(x0), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x5), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n46_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  and2   g36(.a(new_n51_), .b(x0), .O(new_n52_));
  nand4  g37(.a(new_n37_), .b(x5), .c(new_n19_), .d(new_n26_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n26_), .c(x0), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(new_n52_), .c(new_n18_), .O(new_n55_));
  and2   g40(.a(x5), .b(x4), .O(new_n56_));
  nor2   g41(.a(x4), .b(x3), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n26_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n19_), .c(new_n18_), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n19_), .c(new_n26_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n16_), .O(new_n62_));
  nand3  g47(.a(new_n62_), .b(new_n55_), .c(new_n24_), .O(z4));
  oai21  g48(.a(new_n52_), .b(new_n27_), .c(new_n18_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n62_), .c(new_n24_), .O(z5));
  nand3  g50(.a(new_n51_), .b(new_n18_), .c(x0), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n19_), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x1), .c(new_n16_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n66_), .c(new_n24_), .O(z6));
endmodule


