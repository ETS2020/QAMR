// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand4  g05(.a(x4), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nand2  g07(.a(new_n16_), .b(new_n19_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(new_n20_), .c(x4), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(x0), .O(new_n25_));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  nor2   g11(.a(new_n26_), .b(x1), .O(new_n27_));
  inv1   g12(.a(x4), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(x1), .c(x6), .O(new_n29_));
  nand2  g14(.a(new_n28_), .b(new_n20_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(x1), .O(new_n31_));
  oai21  g16(.a(new_n29_), .b(x3), .c(new_n31_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n16_), .c(new_n27_), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x0), .c(new_n25_), .O(z1));
  inv1   g19(.a(x5), .O(new_n35_));
  nor2   g20(.a(x6), .b(x2), .O(new_n36_));
  aoi21  g21(.a(new_n35_), .b(x2), .c(new_n36_), .O(new_n37_));
  nor2   g22(.a(new_n37_), .b(x3), .O(new_n38_));
  aoi21  g23(.a(x4), .b(new_n16_), .c(x3), .O(new_n39_));
  nor2   g24(.a(new_n39_), .b(new_n19_), .O(new_n40_));
  aoi21  g25(.a(new_n38_), .b(new_n19_), .c(new_n40_), .O(new_n41_));
  nor2   g26(.a(new_n28_), .b(new_n20_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(new_n16_), .c(new_n19_), .d(x0), .O(new_n43_));
  oai21  g28(.a(new_n41_), .b(x0), .c(new_n43_), .O(z2));
  nor2   g29(.a(new_n16_), .b(x1), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n20_), .c(new_n28_), .O(new_n46_));
  nand2  g31(.a(new_n30_), .b(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(x1), .O(new_n48_));
  nor2   g33(.a(x5), .b(x3), .O(new_n49_));
  nand2  g34(.a(new_n45_), .b(new_n49_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  oai21  g37(.a(new_n46_), .b(new_n17_), .c(new_n52_), .O(z3));
  xor2a  g38(.a(x3), .b(x2), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(x4), .c(x0), .O(new_n55_));
  inv1   g40(.a(new_n55_), .O(new_n56_));
  oai21  g41(.a(new_n37_), .b(x3), .c(new_n26_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n17_), .c(new_n56_), .O(new_n58_));
  aoi21  g43(.a(new_n20_), .b(x2), .c(new_n19_), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n20_), .c(new_n16_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n59_), .c(new_n17_), .O(new_n62_));
  oai21  g47(.a(new_n58_), .b(x1), .c(new_n62_), .O(z4));
  nand2  g48(.a(new_n20_), .b(new_n16_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n19_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n28_), .O(new_n66_));
  nor2   g51(.a(new_n26_), .b(x0), .O(new_n67_));
  oai21  g52(.a(new_n67_), .b(new_n56_), .c(new_n19_), .O(new_n68_));
  oai21  g53(.a(new_n61_), .b(new_n40_), .c(new_n17_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n68_), .c(new_n66_), .O(z5));
  aoi21  g55(.a(new_n54_), .b(new_n19_), .c(new_n28_), .O(new_n71_));
  nand2  g56(.a(x3), .b(x1), .O(new_n72_));
  nand2  g57(.a(new_n49_), .b(new_n19_), .O(new_n73_));
  aoi21  g58(.a(new_n73_), .b(new_n72_), .c(new_n16_), .O(new_n74_));
  oai21  g59(.a(new_n28_), .b(x3), .c(new_n16_), .O(new_n75_));
  nor2   g60(.a(new_n75_), .b(new_n19_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n74_), .c(new_n17_), .O(new_n77_));
  oai21  g62(.a(new_n71_), .b(new_n17_), .c(new_n77_), .O(z6));
endmodule


