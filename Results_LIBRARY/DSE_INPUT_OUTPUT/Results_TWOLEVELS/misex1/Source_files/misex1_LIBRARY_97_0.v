// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(x7), .c(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x5), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand3  g11(.a(x6), .b(new_n19_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nor2   g13(.a(x2), .b(new_n23_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(x7), .c(new_n29_), .O(new_n30_));
  nand4  g15(.a(new_n20_), .b(x7), .c(x3), .d(new_n16_), .O(new_n31_));
  oai21  g16(.a(new_n30_), .b(x0), .c(new_n31_), .O(z1));
  nand2  g17(.a(new_n24_), .b(x2), .O(new_n33_));
  inv1   g18(.a(x6), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(x7), .c(new_n19_), .d(new_n23_), .O(new_n37_));
  aoi21  g22(.a(x4), .b(new_n16_), .c(x3), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n23_), .c(new_n37_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n17_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n31_), .O(z2));
  nand2  g26(.a(x5), .b(new_n17_), .O(new_n42_));
  nand4  g27(.a(new_n42_), .b(x7), .c(new_n19_), .d(x2), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(x7), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n23_), .O(new_n45_));
  inv1   g30(.a(x4), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n19_), .c(x1), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n16_), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n45_), .O(z3));
  xor2a  g35(.a(x3), .b(x2), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(x0), .O(new_n52_));
  aoi21  g37(.a(new_n35_), .b(new_n33_), .c(x3), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n25_), .c(new_n17_), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n52_), .c(x7), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n23_), .O(new_n56_));
  aoi21  g41(.a(new_n19_), .b(x2), .c(new_n23_), .O(new_n57_));
  nand4  g42(.a(x7), .b(x6), .c(new_n19_), .d(new_n16_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n17_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n56_), .O(z4));
  nand2  g46(.a(new_n25_), .b(new_n17_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n52_), .c(x1), .O(new_n63_));
  nand4  g48(.a(x6), .b(new_n19_), .c(new_n16_), .d(new_n17_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(x7), .O(new_n66_));
  nand2  g51(.a(new_n57_), .b(new_n17_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n66_), .O(z5));
  nand4  g53(.a(new_n24_), .b(new_n19_), .c(x2), .d(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n52_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x7), .c(new_n23_), .O(new_n71_));
  oai21  g56(.a(x4), .b(x2), .c(new_n19_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(x1), .c(new_n17_), .O(new_n73_));
  nand2  g58(.a(new_n73_), .b(new_n71_), .O(z6));
endmodule


