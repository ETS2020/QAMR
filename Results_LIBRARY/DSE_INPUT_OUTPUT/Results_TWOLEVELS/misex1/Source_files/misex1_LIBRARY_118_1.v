// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:19 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x5), .O(new_n17_));
  nand2  g02(.a(x2), .b(x0), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(x3), .c(new_n17_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n16_), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  inv1   g06(.a(x2), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x1), .c(new_n21_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n20_), .O(z0));
  nand2  g11(.a(new_n22_), .b(x1), .O(new_n27_));
  nand3  g12(.a(new_n17_), .b(x2), .c(new_n16_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n27_), .c(x0), .O(new_n29_));
  nand4  g14(.a(new_n17_), .b(new_n22_), .c(new_n16_), .d(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(new_n29_), .c(x3), .O(new_n32_));
  oai21  g17(.a(x4), .b(new_n23_), .c(x1), .O(new_n33_));
  nand3  g18(.a(x6), .b(new_n17_), .c(new_n23_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n22_), .c(new_n21_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n32_), .O(z1));
  nand2  g22(.a(x6), .b(new_n22_), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(new_n17_), .c(new_n23_), .d(new_n16_), .O(new_n39_));
  aoi21  g24(.a(x4), .b(new_n22_), .c(x3), .O(new_n40_));
  oai21  g25(.a(new_n40_), .b(new_n16_), .c(new_n39_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  nand2  g27(.a(new_n22_), .b(x0), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n23_), .c(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n22_), .c(x1), .O(new_n48_));
  nand4  g33(.a(new_n17_), .b(new_n23_), .c(x2), .d(new_n16_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n20_), .O(z3));
  xor2a  g37(.a(x3), .b(x2), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(x0), .O(new_n54_));
  nor2   g39(.a(x6), .b(x3), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(x2), .c(new_n21_), .O(new_n56_));
  aoi21  g41(.a(new_n56_), .b(new_n54_), .c(x1), .O(new_n57_));
  nand4  g42(.a(x6), .b(new_n23_), .c(new_n22_), .d(new_n21_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n57_), .c(new_n17_), .O(new_n60_));
  aoi21  g45(.a(new_n23_), .b(x2), .c(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n21_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z4));
  nand2  g48(.a(new_n24_), .b(new_n21_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(new_n54_), .c(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n16_), .O(new_n66_));
  nand4  g51(.a(x6), .b(new_n17_), .c(new_n23_), .d(new_n22_), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n61_), .c(new_n21_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n66_), .O(z5));
  nand3  g55(.a(new_n23_), .b(x2), .c(new_n21_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n54_), .O(new_n72_));
  nand3  g57(.a(new_n72_), .b(new_n17_), .c(new_n16_), .O(new_n73_));
  oai21  g58(.a(x4), .b(x2), .c(new_n23_), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(x1), .c(new_n21_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n73_), .O(z6));
endmodule


