// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x5), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x1), .O(new_n18_));
  nor2   g03(.a(x5), .b(x3), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x0), .O(new_n22_));
  inv1   g07(.a(x0), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  nor2   g09(.a(new_n24_), .b(new_n17_), .O(new_n25_));
  nand3  g10(.a(new_n25_), .b(x1), .c(new_n23_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n22_), .O(z0));
  oai21  g12(.a(x5), .b(x3), .c(x2), .O(new_n28_));
  nor2   g13(.a(new_n28_), .b(x1), .O(new_n29_));
  inv1   g14(.a(x4), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(x1), .c(x6), .O(new_n31_));
  oai21  g16(.a(x4), .b(x3), .c(x1), .O(new_n32_));
  oai21  g17(.a(new_n31_), .b(x3), .c(new_n32_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n17_), .c(new_n29_), .O(new_n34_));
  inv1   g19(.a(x1), .O(new_n35_));
  nor2   g20(.a(x5), .b(new_n24_), .O(new_n36_));
  nand4  g21(.a(new_n36_), .b(new_n17_), .c(new_n35_), .d(x0), .O(new_n37_));
  oai21  g22(.a(new_n34_), .b(x0), .c(new_n37_), .O(z1));
  nand2  g23(.a(new_n17_), .b(new_n35_), .O(new_n39_));
  oai21  g24(.a(new_n39_), .b(new_n24_), .c(new_n16_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x0), .O(new_n41_));
  nand2  g26(.a(new_n16_), .b(x2), .O(new_n42_));
  inv1   g27(.a(x6), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n17_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n24_), .c(new_n35_), .O(new_n46_));
  oai21  g31(.a(new_n30_), .b(x2), .c(new_n24_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x1), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n41_), .O(z2));
  oai21  g36(.a(x4), .b(x3), .c(x7), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n17_), .c(x1), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n20_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n23_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n22_), .O(z3));
  xnor2a g41(.a(x3), .b(x2), .O(new_n57_));
  nand3  g42(.a(new_n24_), .b(x2), .c(new_n23_), .O(new_n58_));
  oai21  g43(.a(new_n57_), .b(new_n23_), .c(new_n58_), .O(new_n59_));
  nand3  g44(.a(new_n43_), .b(new_n24_), .c(new_n17_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n28_), .c(x0), .O(new_n61_));
  aoi21  g46(.a(new_n59_), .b(new_n16_), .c(new_n61_), .O(new_n62_));
  aoi21  g47(.a(new_n24_), .b(x2), .c(new_n35_), .O(new_n63_));
  nand3  g48(.a(x6), .b(new_n24_), .c(new_n17_), .O(new_n64_));
  inv1   g49(.a(new_n64_), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n63_), .c(new_n23_), .O(new_n66_));
  oai21  g51(.a(new_n62_), .b(x1), .c(new_n66_), .O(z4));
  oai21  g52(.a(new_n18_), .b(x0), .c(x5), .O(new_n68_));
  inv1   g53(.a(new_n57_), .O(new_n69_));
  nand3  g54(.a(new_n69_), .b(new_n16_), .c(x0), .O(new_n70_));
  nand2  g55(.a(new_n25_), .b(new_n23_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n35_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(new_n68_), .c(new_n66_), .O(z5));
  aoi21  g59(.a(new_n69_), .b(new_n35_), .c(x5), .O(new_n75_));
  nand2  g60(.a(x3), .b(x1), .O(new_n76_));
  nand2  g61(.a(new_n19_), .b(new_n35_), .O(new_n77_));
  aoi21  g62(.a(new_n77_), .b(new_n76_), .c(new_n17_), .O(new_n78_));
  oai21  g63(.a(new_n30_), .b(x3), .c(new_n17_), .O(new_n79_));
  nor2   g64(.a(new_n79_), .b(new_n35_), .O(new_n80_));
  oai21  g65(.a(new_n80_), .b(new_n78_), .c(new_n23_), .O(new_n81_));
  oai21  g66(.a(new_n75_), .b(new_n23_), .c(new_n81_), .O(z6));
endmodule


