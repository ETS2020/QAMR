// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  inv1   g07(.a(x2), .O(new_n23_));
  nand2  g08(.a(x5), .b(new_n23_), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n22_), .O(z0));
  oai21  g10(.a(x5), .b(x3), .c(x2), .O(new_n26_));
  or2    g11(.a(new_n26_), .b(x1), .O(new_n27_));
  inv1   g12(.a(x5), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x1), .c(x6), .O(new_n30_));
  oai21  g15(.a(x4), .b(x3), .c(x1), .O(new_n31_));
  oai21  g16(.a(new_n30_), .b(x3), .c(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n28_), .c(new_n23_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n27_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n18_), .b(x0), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nand3  g22(.a(new_n28_), .b(x3), .c(new_n23_), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n35_), .O(z1));
  nand2  g26(.a(new_n29_), .b(new_n19_), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n23_), .c(x1), .O(new_n43_));
  nand2  g28(.a(x6), .b(new_n23_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n19_), .c(new_n18_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n43_), .c(x5), .O(new_n46_));
  nand3  g31(.a(x3), .b(x2), .c(x1), .O(new_n47_));
  inv1   g32(.a(new_n47_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n46_), .c(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n37_), .b(x3), .c(x5), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(x2), .c(new_n49_), .O(z2));
  nand2  g36(.a(new_n42_), .b(x7), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n23_), .c(x1), .O(new_n53_));
  nand2  g38(.a(new_n19_), .b(x2), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x1), .c(new_n53_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n28_), .c(new_n16_), .O(new_n56_));
  inv1   g41(.a(new_n54_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n37_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n56_), .O(z3));
  aoi21  g44(.a(new_n54_), .b(new_n38_), .c(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n44_), .b(new_n28_), .c(new_n19_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n26_), .c(x0), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n60_), .c(new_n18_), .O(new_n63_));
  oai21  g48(.a(new_n28_), .b(x2), .c(x3), .O(new_n64_));
  nand2  g49(.a(new_n29_), .b(x3), .O(new_n65_));
  nand3  g50(.a(new_n65_), .b(new_n28_), .c(new_n23_), .O(new_n66_));
  aoi21  g51(.a(new_n66_), .b(new_n64_), .c(new_n18_), .O(new_n67_));
  nand4  g52(.a(x6), .b(new_n28_), .c(new_n19_), .d(new_n23_), .O(new_n68_));
  inv1   g53(.a(new_n68_), .O(new_n69_));
  oai21  g54(.a(new_n69_), .b(new_n67_), .c(new_n16_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n63_), .O(z4));
  nor2   g56(.a(new_n26_), .b(x0), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n60_), .c(new_n18_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(new_n70_), .c(new_n24_), .O(z5));
  nand4  g59(.a(new_n28_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n75_));
  inv1   g60(.a(new_n75_), .O(new_n76_));
  oai21  g61(.a(new_n76_), .b(new_n60_), .c(new_n18_), .O(new_n77_));
  nand2  g62(.a(x4), .b(new_n19_), .O(new_n78_));
  nand3  g63(.a(new_n78_), .b(new_n28_), .c(new_n23_), .O(new_n79_));
  oai21  g64(.a(new_n19_), .b(new_n23_), .c(new_n79_), .O(new_n80_));
  nand3  g65(.a(new_n80_), .b(x1), .c(new_n16_), .O(new_n81_));
  nand2  g66(.a(new_n81_), .b(new_n77_), .O(z6));
endmodule


