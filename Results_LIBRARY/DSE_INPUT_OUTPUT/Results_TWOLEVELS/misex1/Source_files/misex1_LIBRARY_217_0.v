// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:43 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  oai21  g02(.a(x3), .b(new_n17_), .c(x2), .O(new_n18_));
  nand2  g03(.a(new_n18_), .b(new_n16_), .O(new_n19_));
  nand4  g04(.a(x3), .b(x2), .c(x1), .d(new_n17_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n19_), .O(z0));
  inv1   g06(.a(x3), .O(new_n22_));
  inv1   g07(.a(x5), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x2), .c(new_n16_), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x1), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n25_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n17_), .O(new_n29_));
  oai21  g14(.a(x2), .b(x1), .c(new_n29_), .O(z1));
  nand2  g15(.a(x3), .b(x1), .O(new_n31_));
  oai21  g16(.a(new_n24_), .b(x1), .c(new_n31_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x2), .O(new_n33_));
  inv1   g18(.a(x4), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n22_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n26_), .c(x1), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n33_), .c(x0), .O(z2));
  aoi21  g22(.a(x5), .b(new_n17_), .c(x3), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n26_), .c(new_n16_), .O(new_n39_));
  oai21  g24(.a(new_n35_), .b(new_n16_), .c(x7), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n26_), .c(new_n17_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n39_), .O(z3));
  nand3  g27(.a(new_n34_), .b(new_n26_), .c(x1), .O(new_n43_));
  nand3  g28(.a(new_n23_), .b(x2), .c(new_n16_), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(x3), .O(new_n45_));
  oai21  g30(.a(new_n23_), .b(x1), .c(new_n22_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(x2), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n36_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n45_), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n19_), .O(z4));
  nand3  g35(.a(x2), .b(new_n16_), .c(x0), .O(new_n51_));
  nand4  g36(.a(new_n34_), .b(new_n26_), .c(x1), .d(new_n17_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n22_), .O(new_n54_));
  nand2  g39(.a(new_n48_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n54_), .O(z5));
  nand3  g41(.a(new_n22_), .b(new_n16_), .c(x0), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  aoi21  g43(.a(new_n32_), .b(new_n17_), .c(new_n58_), .O(new_n59_));
  nand2  g44(.a(x4), .b(new_n22_), .O(new_n60_));
  nand4  g45(.a(new_n60_), .b(new_n26_), .c(x1), .d(new_n17_), .O(new_n61_));
  oai21  g46(.a(new_n59_), .b(new_n26_), .c(new_n61_), .O(z6));
endmodule


