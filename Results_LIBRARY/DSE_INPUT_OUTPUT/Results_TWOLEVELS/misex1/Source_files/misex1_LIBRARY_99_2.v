// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x1), .b(new_n17_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(new_n16_), .O(z0));
  nand2  g04(.a(new_n16_), .b(x1), .O(new_n20_));
  inv1   g05(.a(x1), .O(new_n21_));
  nand2  g06(.a(x2), .b(new_n21_), .O(new_n22_));
  inv1   g07(.a(x3), .O(new_n23_));
  nand2  g08(.a(x6), .b(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n22_), .c(new_n20_), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n17_), .O(new_n26_));
  nor2   g11(.a(x1), .b(new_n17_), .O(new_n27_));
  nor2   g12(.a(new_n23_), .b(x2), .O(new_n28_));
  nor2   g13(.a(x3), .b(new_n16_), .O(new_n29_));
  aoi21  g14(.a(new_n28_), .b(new_n27_), .c(new_n29_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n26_), .O(z1));
  inv1   g16(.a(x4), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(new_n23_), .O(new_n33_));
  nor3   g18(.a(x6), .b(x3), .c(x1), .O(new_n34_));
  aoi21  g19(.a(new_n33_), .b(x1), .c(new_n34_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x0), .c(new_n30_), .O(z2));
  inv1   g21(.a(new_n29_), .O(new_n37_));
  oai22  g22(.a(x7), .b(x2), .c(x4), .d(x3), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(x1), .c(new_n17_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n37_), .O(z3));
  oai21  g25(.a(x1), .b(x0), .c(x3), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x2), .O(new_n42_));
  nand3  g27(.a(x3), .b(new_n16_), .c(x0), .O(new_n43_));
  inv1   g28(.a(new_n43_), .O(new_n44_));
  nor3   g29(.a(x6), .b(x3), .c(x0), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n44_), .c(new_n21_), .O(new_n46_));
  inv1   g31(.a(new_n24_), .O(new_n47_));
  aoi21  g32(.a(new_n23_), .b(x2), .c(new_n21_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n47_), .c(new_n17_), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n46_), .c(new_n42_), .O(z4));
  nor2   g35(.a(new_n16_), .b(x0), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n44_), .c(new_n21_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n49_), .c(new_n37_), .O(z5));
  nand2  g38(.a(new_n28_), .b(new_n27_), .O(new_n54_));
  oai21  g39(.a(x4), .b(x2), .c(new_n23_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(x1), .c(new_n17_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z6));
endmodule


