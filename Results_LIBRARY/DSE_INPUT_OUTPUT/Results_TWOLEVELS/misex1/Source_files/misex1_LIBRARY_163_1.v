// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  nor2   g03(.a(x1), .b(new_n17_), .O(new_n19_));
  nor2   g04(.a(x4), .b(x3), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x1), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n23_), .O(new_n27_));
  inv1   g12(.a(x6), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n23_), .c(x3), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n16_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n27_), .c(x4), .O(new_n31_));
  nor3   g16(.a(new_n20_), .b(x2), .c(new_n23_), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n31_), .c(new_n17_), .O(new_n33_));
  nor2   g18(.a(x2), .b(new_n17_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(x3), .c(x4), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(x1), .c(new_n33_), .O(z1));
  inv1   g21(.a(x4), .O(new_n37_));
  nand2  g22(.a(new_n25_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n28_), .b(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand4  g25(.a(new_n40_), .b(new_n37_), .c(new_n24_), .d(new_n23_), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  aoi21  g27(.a(x4), .b(new_n16_), .c(x3), .O(new_n43_));
  nor2   g28(.a(new_n43_), .b(new_n23_), .O(new_n44_));
  oai21  g29(.a(new_n44_), .b(new_n42_), .c(new_n17_), .O(new_n45_));
  nand4  g30(.a(new_n19_), .b(new_n37_), .c(x3), .d(new_n16_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n45_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n16_), .c(x1), .O(new_n49_));
  nor2   g34(.a(x5), .b(x4), .O(new_n50_));
  nand4  g35(.a(new_n50_), .b(new_n24_), .c(x2), .d(new_n23_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  nand3  g38(.a(new_n20_), .b(new_n19_), .c(x2), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n53_), .O(z3));
  xor2a  g40(.a(x3), .b(x2), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(x0), .O(new_n57_));
  aoi21  g42(.a(new_n40_), .b(new_n24_), .c(new_n26_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(x0), .c(new_n57_), .O(new_n59_));
  nand3  g44(.a(new_n29_), .b(new_n16_), .c(new_n17_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  aoi21  g46(.a(new_n59_), .b(new_n23_), .c(new_n61_), .O(new_n62_));
  nand2  g47(.a(new_n44_), .b(new_n17_), .O(new_n63_));
  oai21  g48(.a(new_n62_), .b(x4), .c(new_n63_), .O(z4));
  nand2  g49(.a(new_n26_), .b(new_n17_), .O(new_n65_));
  aoi21  g50(.a(new_n65_), .b(new_n57_), .c(x1), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n61_), .c(new_n37_), .O(new_n67_));
  nand2  g52(.a(x4), .b(new_n23_), .O(new_n68_));
  nand3  g53(.a(new_n68_), .b(new_n67_), .c(new_n63_), .O(z5));
  nand4  g54(.a(new_n25_), .b(new_n24_), .c(x2), .d(new_n17_), .O(new_n70_));
  aoi21  g55(.a(new_n70_), .b(new_n57_), .c(x1), .O(new_n71_));
  nand4  g56(.a(new_n24_), .b(new_n16_), .c(x1), .d(new_n17_), .O(new_n72_));
  inv1   g57(.a(new_n72_), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n71_), .c(new_n37_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n18_), .O(z6));
endmodule


