// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:45 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(x4), .O(new_n19_));
  nor2   g04(.a(x3), .b(new_n17_), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n16_), .c(x0), .O(new_n21_));
  oai21  g06(.a(new_n19_), .b(x0), .c(new_n21_), .O(z0));
  inv1   g07(.a(x0), .O(new_n23_));
  inv1   g08(.a(x4), .O(new_n24_));
  inv1   g09(.a(x6), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x3), .c(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n17_), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  inv1   g13(.a(x5), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(new_n30_));
  nand3  g15(.a(new_n30_), .b(x2), .c(new_n16_), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n27_), .c(new_n24_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n23_), .O(new_n33_));
  nor2   g18(.a(new_n28_), .b(x2), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(new_n16_), .c(x0), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(z1));
  nand2  g21(.a(x3), .b(x1), .O(new_n37_));
  nand4  g22(.a(new_n29_), .b(new_n28_), .c(x2), .d(new_n16_), .O(new_n38_));
  nand4  g23(.a(new_n25_), .b(new_n28_), .c(new_n17_), .d(new_n16_), .O(new_n39_));
  nand4  g24(.a(new_n39_), .b(new_n38_), .c(new_n37_), .d(new_n24_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n23_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n35_), .O(z2));
  nand2  g27(.a(x7), .b(x3), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n17_), .c(x1), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n24_), .c(new_n23_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n21_), .O(z3));
  nand2  g32(.a(new_n28_), .b(x2), .O(new_n48_));
  nand2  g33(.a(x3), .b(new_n17_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(new_n48_), .c(new_n23_), .O(new_n50_));
  nor2   g35(.a(x5), .b(x4), .O(new_n51_));
  nor2   g36(.a(x6), .b(x2), .O(new_n52_));
  aoi21  g37(.a(new_n51_), .b(x2), .c(new_n52_), .O(new_n53_));
  nand3  g38(.a(new_n30_), .b(new_n24_), .c(x2), .O(new_n54_));
  oai21  g39(.a(new_n53_), .b(x3), .c(new_n54_), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n23_), .c(new_n50_), .O(new_n56_));
  nand2  g41(.a(new_n28_), .b(x2), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(x1), .O(new_n58_));
  nand3  g43(.a(x6), .b(new_n28_), .c(new_n17_), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n58_), .c(x4), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x4), .c(new_n23_), .O(new_n61_));
  oai21  g46(.a(new_n56_), .b(x1), .c(new_n61_), .O(z4));
  oai21  g47(.a(new_n34_), .b(new_n20_), .c(x0), .O(new_n63_));
  oai21  g48(.a(new_n54_), .b(x0), .c(new_n63_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n16_), .O(new_n65_));
  nand2  g50(.a(new_n60_), .b(new_n23_), .O(new_n66_));
  nand2  g51(.a(new_n66_), .b(new_n65_), .O(z5));
  nand4  g52(.a(new_n51_), .b(new_n28_), .c(x2), .d(new_n23_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n16_), .O(new_n70_));
  nand4  g55(.a(new_n57_), .b(new_n24_), .c(x1), .d(new_n23_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n70_), .O(z6));
endmodule


