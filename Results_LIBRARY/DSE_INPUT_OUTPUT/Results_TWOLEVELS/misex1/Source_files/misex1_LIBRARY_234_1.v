// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  oai21  g04(.a(new_n18_), .b(new_n17_), .c(new_n19_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand3  g06(.a(x3), .b(x2), .c(new_n17_), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(z0));
  oai21  g08(.a(x4), .b(new_n19_), .c(x1), .O(new_n24_));
  nor2   g09(.a(x6), .b(x3), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n24_), .c(x2), .O(new_n26_));
  inv1   g11(.a(x5), .O(new_n27_));
  nor3   g12(.a(new_n27_), .b(new_n18_), .c(x1), .O(new_n28_));
  oai21  g13(.a(new_n28_), .b(new_n26_), .c(new_n17_), .O(new_n29_));
  nand2  g14(.a(x3), .b(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(z1));
  inv1   g16(.a(x4), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(x2), .c(new_n19_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x1), .O(new_n34_));
  nand2  g19(.a(new_n27_), .b(x2), .O(new_n35_));
  oai21  g20(.a(x6), .b(x2), .c(new_n35_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n16_), .O(new_n37_));
  aoi21  g22(.a(new_n37_), .b(new_n34_), .c(x0), .O(z2));
  oai21  g23(.a(x4), .b(x3), .c(x7), .O(new_n39_));
  nand3  g24(.a(new_n39_), .b(new_n18_), .c(x1), .O(new_n40_));
  nand4  g25(.a(new_n27_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(z3));
  oai21  g29(.a(new_n16_), .b(new_n17_), .c(x3), .O(new_n45_));
  aoi21  g30(.a(new_n19_), .b(new_n16_), .c(x6), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n24_), .c(x2), .O(new_n47_));
  oai21  g32(.a(x5), .b(new_n19_), .c(x2), .O(new_n48_));
  nor2   g33(.a(new_n48_), .b(x1), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n47_), .c(new_n17_), .O(new_n50_));
  nand3  g35(.a(x2), .b(new_n16_), .c(x0), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n50_), .c(new_n45_), .O(z4));
  aoi21  g37(.a(new_n27_), .b(new_n17_), .c(new_n18_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(x3), .c(new_n16_), .O(new_n54_));
  aoi21  g39(.a(new_n19_), .b(x2), .c(new_n16_), .O(new_n55_));
  inv1   g40(.a(x6), .O(new_n56_));
  nor2   g41(.a(new_n56_), .b(x2), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n55_), .c(new_n17_), .O(new_n58_));
  nand2  g43(.a(new_n58_), .b(new_n54_), .O(z5));
  aoi21  g44(.a(new_n32_), .b(new_n18_), .c(x3), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n16_), .c(new_n41_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n17_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n21_), .O(z6));
endmodule


