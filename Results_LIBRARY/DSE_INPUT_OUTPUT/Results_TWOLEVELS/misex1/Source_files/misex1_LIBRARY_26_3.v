// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  nand2  g08(.a(x3), .b(new_n16_), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x1), .c(x0), .O(new_n26_));
  inv1   g11(.a(x4), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n19_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n19_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n29_), .c(x2), .O(new_n31_));
  oai21  g16(.a(x5), .b(x3), .c(x2), .O(new_n32_));
  nor2   g17(.a(new_n32_), .b(x1), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(new_n31_), .c(new_n17_), .O(new_n34_));
  nand4  g19(.a(new_n27_), .b(new_n19_), .c(new_n16_), .d(x1), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n34_), .c(new_n26_), .O(z1));
  nor2   g21(.a(x5), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n37_), .c(new_n19_), .O(new_n39_));
  oai21  g24(.a(new_n27_), .b(x2), .c(new_n19_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x1), .O(new_n41_));
  oai21  g26(.a(new_n39_), .b(x1), .c(new_n41_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n17_), .O(new_n43_));
  nand3  g28(.a(new_n25_), .b(new_n18_), .c(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(z2));
  nand2  g30(.a(new_n19_), .b(x2), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(x0), .O(new_n48_));
  nand2  g33(.a(new_n28_), .b(x7), .O(new_n49_));
  nand3  g34(.a(new_n49_), .b(new_n16_), .c(x1), .O(new_n50_));
  nor2   g35(.a(x5), .b(x3), .O(new_n51_));
  nand4  g36(.a(new_n51_), .b(x2), .c(new_n18_), .d(new_n17_), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(z3));
  aoi21  g38(.a(new_n46_), .b(new_n24_), .c(new_n17_), .O(new_n54_));
  inv1   g39(.a(x5), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(x2), .c(new_n38_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x3), .c(new_n32_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n17_), .c(new_n54_), .O(new_n58_));
  inv1   g43(.a(new_n30_), .O(new_n59_));
  nand3  g44(.a(new_n19_), .b(x2), .c(new_n17_), .O(new_n60_));
  nor2   g45(.a(x2), .b(x0), .O(new_n61_));
  aoi22  g46(.a(new_n61_), .b(new_n59_), .c(new_n60_), .d(x1), .O(new_n62_));
  oai21  g47(.a(new_n58_), .b(x1), .c(new_n62_), .O(z4));
  nor2   g48(.a(new_n32_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n54_), .c(new_n18_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  nand3  g51(.a(new_n46_), .b(new_n24_), .c(new_n18_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(x0), .O(new_n68_));
  nand3  g53(.a(new_n51_), .b(x2), .c(new_n18_), .O(new_n69_));
  inv1   g54(.a(new_n69_), .O(new_n70_));
  oai21  g55(.a(new_n70_), .b(new_n20_), .c(new_n17_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n68_), .c(new_n35_), .O(z6));
endmodule


