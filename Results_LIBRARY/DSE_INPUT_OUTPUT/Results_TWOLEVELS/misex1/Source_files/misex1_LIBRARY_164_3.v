// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:30 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  inv1   g05(.a(x7), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x3), .O(new_n22_));
  inv1   g07(.a(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(x1), .c(new_n20_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g10(.a(x5), .b(new_n18_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n22_), .c(new_n16_), .O(new_n27_));
  nand2  g12(.a(x7), .b(x3), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x1), .O(new_n29_));
  nand2  g14(.a(x6), .b(new_n18_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n29_), .c(x2), .O(new_n31_));
  aoi21  g16(.a(new_n27_), .b(new_n17_), .c(new_n31_), .O(new_n32_));
  nor2   g17(.a(x1), .b(new_n20_), .O(new_n33_));
  nand3  g18(.a(new_n21_), .b(x3), .c(new_n16_), .O(new_n34_));
  inv1   g19(.a(new_n34_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  oai21  g21(.a(new_n32_), .b(x0), .c(new_n36_), .O(z1));
  nand3  g22(.a(x4), .b(new_n18_), .c(new_n16_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n22_), .c(new_n17_), .O(new_n39_));
  inv1   g24(.a(x5), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(x2), .O(new_n41_));
  inv1   g26(.a(x6), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n16_), .O(new_n43_));
  aoi21  g28(.a(new_n43_), .b(new_n41_), .c(x3), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n17_), .c(new_n39_), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(x0), .c(new_n36_), .O(z2));
  oai21  g31(.a(x4), .b(x3), .c(x7), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(new_n16_), .c(x1), .O(new_n48_));
  nand4  g33(.a(new_n40_), .b(new_n18_), .c(x2), .d(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n33_), .b(new_n18_), .c(x2), .O(new_n52_));
  nand3  g37(.a(new_n52_), .b(new_n51_), .c(new_n28_), .O(z3));
  nand2  g38(.a(new_n18_), .b(x2), .O(new_n54_));
  aoi21  g39(.a(new_n54_), .b(new_n34_), .c(new_n20_), .O(new_n55_));
  oai21  g40(.a(new_n21_), .b(new_n18_), .c(x2), .O(new_n56_));
  nand3  g41(.a(new_n42_), .b(new_n18_), .c(new_n16_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n56_), .c(x0), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n55_), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(x1), .b(new_n20_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n16_), .c(new_n21_), .O(new_n61_));
  aoi22  g46(.a(new_n61_), .b(x3), .c(new_n31_), .d(new_n20_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n59_), .O(z4));
  nand2  g48(.a(new_n54_), .b(new_n34_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(x0), .O(new_n65_));
  nand2  g50(.a(new_n26_), .b(new_n22_), .O(new_n66_));
  nand3  g51(.a(new_n66_), .b(x2), .c(new_n20_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n62_), .O(z5));
  nand4  g55(.a(new_n40_), .b(new_n18_), .c(x2), .d(new_n20_), .O(new_n71_));
  inv1   g56(.a(new_n71_), .O(new_n72_));
  oai21  g57(.a(new_n72_), .b(new_n55_), .c(new_n17_), .O(new_n73_));
  inv1   g58(.a(x4), .O(new_n74_));
  nand3  g59(.a(new_n74_), .b(new_n18_), .c(new_n16_), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n22_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(x1), .c(new_n20_), .O(new_n77_));
  nand2  g62(.a(new_n77_), .b(new_n73_), .O(z6));
endmodule


