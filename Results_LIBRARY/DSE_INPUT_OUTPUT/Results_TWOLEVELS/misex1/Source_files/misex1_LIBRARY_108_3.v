// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(x6), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x1), .b(new_n21_), .O(new_n22_));
  nand3  g07(.a(new_n22_), .b(new_n20_), .c(x2), .O(new_n23_));
  oai21  g08(.a(new_n19_), .b(new_n16_), .c(new_n23_), .O(z0));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n20_), .c(new_n17_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  nor2   g14(.a(x4), .b(new_n20_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n20_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n29_), .c(new_n17_), .d(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  nand2  g19(.a(x3), .b(new_n17_), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  aoi22  g21(.a(new_n36_), .b(new_n22_), .c(x6), .d(x1), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n34_), .O(z1));
  nand2  g23(.a(new_n29_), .b(new_n17_), .O(new_n39_));
  oai21  g24(.a(x5), .b(new_n17_), .c(new_n39_), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n20_), .c(new_n16_), .O(new_n41_));
  inv1   g26(.a(x4), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(x2), .c(new_n20_), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n29_), .c(x1), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n41_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n21_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n37_), .O(z2));
  oai21  g32(.a(x4), .b(x3), .c(x7), .O(new_n48_));
  nand4  g33(.a(new_n48_), .b(new_n29_), .c(new_n17_), .d(x1), .O(new_n49_));
  nor2   g34(.a(x5), .b(x3), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(x2), .c(new_n16_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n23_), .O(z3));
  nand2  g39(.a(new_n20_), .b(x2), .O(new_n55_));
  aoi21  g40(.a(new_n35_), .b(new_n55_), .c(new_n21_), .O(new_n56_));
  aoi21  g41(.a(x3), .b(new_n17_), .c(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n56_), .c(new_n16_), .O(new_n58_));
  nor2   g43(.a(x3), .b(new_n17_), .O(new_n59_));
  nor2   g44(.a(new_n59_), .b(x6), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(x1), .c(new_n21_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n58_), .O(z4));
  aoi21  g47(.a(new_n26_), .b(new_n25_), .c(x0), .O(new_n63_));
  oai21  g48(.a(new_n63_), .b(new_n56_), .c(new_n16_), .O(new_n64_));
  aoi21  g49(.a(new_n60_), .b(new_n21_), .c(x6), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n16_), .c(new_n64_), .O(z5));
  nand2  g51(.a(new_n50_), .b(new_n18_), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n56_), .c(new_n16_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n20_), .O(new_n70_));
  nand4  g55(.a(new_n70_), .b(new_n29_), .c(x1), .d(new_n21_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


