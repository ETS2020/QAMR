// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand3  g01(.a(x3), .b(x1), .c(new_n16_), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nand3  g04(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  nand2  g07(.a(x6), .b(x3), .O(new_n23_));
  nand2  g08(.a(new_n23_), .b(new_n22_), .O(z0));
  inv1   g09(.a(x2), .O(new_n25_));
  nand2  g10(.a(x5), .b(new_n19_), .O(new_n26_));
  inv1   g11(.a(x6), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(x3), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(new_n29_));
  oai21  g14(.a(new_n27_), .b(new_n19_), .c(x1), .O(new_n30_));
  nand2  g15(.a(x6), .b(new_n19_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  aoi22  g17(.a(new_n32_), .b(new_n25_), .c(new_n29_), .d(new_n18_), .O(new_n33_));
  nand2  g18(.a(new_n18_), .b(x0), .O(new_n34_));
  nand3  g19(.a(new_n27_), .b(x3), .c(new_n25_), .O(new_n35_));
  oai22  g20(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(x0), .O(z1));
  inv1   g21(.a(x5), .O(new_n37_));
  nor2   g22(.a(x6), .b(x2), .O(new_n38_));
  aoi21  g23(.a(new_n37_), .b(x2), .c(new_n38_), .O(new_n39_));
  nand3  g24(.a(x4), .b(new_n25_), .c(x1), .O(new_n40_));
  oai21  g25(.a(new_n39_), .b(x1), .c(new_n40_), .O(new_n41_));
  oai21  g26(.a(new_n27_), .b(x2), .c(x3), .O(new_n42_));
  nor2   g27(.a(new_n42_), .b(new_n18_), .O(new_n43_));
  aoi21  g28(.a(new_n41_), .b(new_n19_), .c(new_n43_), .O(new_n44_));
  oai21  g29(.a(new_n34_), .b(x2), .c(new_n27_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(x3), .O(new_n46_));
  oai21  g31(.a(new_n44_), .b(x0), .c(new_n46_), .O(z2));
  nand2  g32(.a(x7), .b(x4), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n25_), .c(x1), .O(new_n49_));
  nand3  g34(.a(new_n37_), .b(x2), .c(new_n18_), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(x0), .O(new_n51_));
  nand3  g36(.a(x2), .b(new_n18_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n19_), .O(new_n54_));
  nor2   g39(.a(x7), .b(x6), .O(new_n55_));
  nand4  g40(.a(new_n55_), .b(new_n25_), .c(x1), .d(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z3));
  nand2  g42(.a(new_n19_), .b(x2), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n35_), .c(new_n16_), .O(new_n59_));
  xor2a  g44(.a(x3), .b(x2), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(x6), .c(new_n58_), .O(new_n61_));
  aoi21  g46(.a(new_n61_), .b(new_n16_), .c(new_n59_), .O(new_n62_));
  oai21  g47(.a(x2), .b(x0), .c(new_n19_), .O(new_n63_));
  nand2  g48(.a(new_n19_), .b(new_n25_), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n42_), .c(new_n18_), .O(new_n65_));
  aoi22  g50(.a(new_n65_), .b(new_n16_), .c(new_n63_), .d(x6), .O(new_n66_));
  oai21  g51(.a(new_n62_), .b(x1), .c(new_n66_), .O(z4));
  aoi21  g52(.a(new_n29_), .b(new_n16_), .c(new_n59_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(x1), .c(new_n66_), .O(z5));
  nand4  g54(.a(new_n37_), .b(new_n19_), .c(x2), .d(new_n16_), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n59_), .c(new_n18_), .O(new_n72_));
  nand2  g57(.a(x3), .b(x2), .O(new_n73_));
  nor2   g58(.a(x4), .b(x3), .O(new_n74_));
  aoi21  g59(.a(new_n27_), .b(x3), .c(new_n74_), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(x2), .c(new_n73_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(x1), .c(new_n16_), .O(new_n77_));
  nand3  g62(.a(new_n77_), .b(new_n72_), .c(new_n23_), .O(z6));
endmodule


