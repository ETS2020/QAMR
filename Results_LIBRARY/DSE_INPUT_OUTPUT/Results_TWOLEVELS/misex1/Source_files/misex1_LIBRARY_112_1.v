// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x2), .O(new_n18_));
  aoi21  g03(.a(x1), .b(new_n17_), .c(new_n18_), .O(new_n19_));
  inv1   g04(.a(x1), .O(new_n20_));
  nand4  g05(.a(new_n16_), .b(x2), .c(new_n20_), .d(x0), .O(new_n21_));
  oai21  g06(.a(new_n19_), .b(new_n16_), .c(new_n21_), .O(z0));
  inv1   g07(.a(x6), .O(new_n23_));
  aoi21  g08(.a(new_n23_), .b(new_n20_), .c(x2), .O(new_n24_));
  nand2  g09(.a(x5), .b(x2), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n16_), .c(x1), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n24_), .c(new_n17_), .O(new_n27_));
  nand2  g12(.a(x3), .b(new_n18_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(z1));
  nand2  g14(.a(x4), .b(new_n18_), .O(new_n30_));
  aoi21  g15(.a(new_n30_), .b(new_n16_), .c(new_n20_), .O(new_n31_));
  inv1   g16(.a(x5), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n16_), .c(x2), .O(new_n33_));
  nand2  g18(.a(new_n23_), .b(new_n18_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n33_), .c(x1), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n31_), .c(new_n17_), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n28_), .O(z2));
  nand2  g22(.a(x7), .b(x4), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n18_), .c(x1), .O(new_n39_));
  nand4  g24(.a(new_n32_), .b(new_n16_), .c(x2), .d(new_n20_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n17_), .O(new_n42_));
  and2   g27(.a(new_n28_), .b(new_n21_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n42_), .O(z3));
  aoi21  g29(.a(new_n16_), .b(x1), .c(new_n18_), .O(new_n45_));
  nor2   g30(.a(x3), .b(x2), .O(new_n46_));
  oai21  g31(.a(new_n46_), .b(new_n45_), .c(new_n17_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n21_), .O(z4));
  oai21  g33(.a(new_n32_), .b(x1), .c(new_n16_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(x2), .c(new_n24_), .O(new_n50_));
  oai21  g35(.a(new_n50_), .b(x0), .c(new_n43_), .O(z5));
  nor2   g36(.a(x4), .b(x2), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(x1), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n40_), .O(new_n54_));
  nand2  g39(.a(new_n54_), .b(new_n17_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n43_), .O(z6));
endmodule


