// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_;
  inv1   g00(.a(x3), .O(new_n16_));
  nand2  g01(.a(new_n16_), .b(x2), .O(new_n17_));
  inv1   g02(.a(new_n17_), .O(new_n18_));
  oai21  g03(.a(new_n18_), .b(x1), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x3), .b(x2), .c(x1), .d(new_n20_), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(new_n19_), .O(z0));
  inv1   g07(.a(x2), .O(new_n23_));
  nand2  g08(.a(x3), .b(new_n23_), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n25_), .b(x1), .c(x0), .O(new_n26_));
  oai21  g11(.a(x5), .b(x3), .c(x2), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x1), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x1), .c(x6), .O(new_n30_));
  nand2  g15(.a(new_n29_), .b(new_n16_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  oai21  g17(.a(new_n30_), .b(x3), .c(new_n32_), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n23_), .c(new_n28_), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(x0), .c(new_n26_), .O(z1));
  nor2   g20(.a(x5), .b(new_n23_), .O(new_n36_));
  nor2   g21(.a(x6), .b(x2), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n36_), .c(new_n16_), .O(new_n38_));
  oai21  g23(.a(new_n29_), .b(x2), .c(new_n16_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  oai21  g25(.a(new_n38_), .b(x1), .c(new_n40_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n20_), .O(new_n42_));
  inv1   g27(.a(x1), .O(new_n43_));
  nand3  g28(.a(new_n25_), .b(new_n43_), .c(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z2));
  nand2  g30(.a(new_n31_), .b(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n23_), .c(x1), .O(new_n47_));
  nor2   g32(.a(x5), .b(x3), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n43_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n20_), .O(new_n51_));
  nand3  g36(.a(new_n18_), .b(new_n43_), .c(x0), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n51_), .O(z3));
  aoi21  g38(.a(new_n24_), .b(new_n17_), .c(new_n20_), .O(new_n54_));
  inv1   g39(.a(x5), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(x2), .c(new_n37_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(x3), .c(new_n27_), .O(new_n57_));
  aoi21  g42(.a(new_n57_), .b(new_n20_), .c(new_n54_), .O(new_n58_));
  oai21  g43(.a(x3), .b(new_n23_), .c(x1), .O(new_n59_));
  nand3  g44(.a(x6), .b(new_n16_), .c(new_n23_), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n59_), .c(x0), .O(new_n61_));
  aoi21  g46(.a(x1), .b(x0), .c(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n58_), .b(x1), .c(new_n62_), .O(z4));
  nor2   g48(.a(new_n27_), .b(x0), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(new_n54_), .c(new_n43_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n62_), .O(z5));
  nand3  g51(.a(new_n48_), .b(x2), .c(new_n20_), .O(new_n67_));
  inv1   g52(.a(new_n67_), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n54_), .c(new_n43_), .O(new_n69_));
  oai21  g54(.a(x4), .b(x2), .c(new_n16_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(x1), .c(new_n20_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n69_), .O(z6));
endmodule


