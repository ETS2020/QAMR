// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:03 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x3), .O(new_n17_));
  nand4  g02(.a(new_n17_), .b(x2), .c(new_n16_), .d(x0), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(z0));
  nand2  g04(.a(new_n16_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x0), .O(new_n21_));
  nor2   g06(.a(x6), .b(x1), .O(new_n22_));
  nand2  g07(.a(x5), .b(x2), .O(new_n23_));
  oai22  g08(.a(new_n23_), .b(x1), .c(new_n22_), .d(x2), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(new_n17_), .c(new_n21_), .O(new_n25_));
  inv1   g10(.a(x2), .O(new_n26_));
  nand2  g11(.a(x3), .b(new_n26_), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n20_), .c(new_n25_), .O(z1));
  oai21  g13(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x3), .O(new_n30_));
  inv1   g15(.a(x5), .O(new_n31_));
  nand3  g16(.a(new_n31_), .b(new_n17_), .c(x2), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n26_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n32_), .c(x1), .O(new_n35_));
  nand4  g20(.a(x4), .b(new_n17_), .c(new_n26_), .d(x1), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(new_n35_), .c(new_n21_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n30_), .O(z2));
  nand2  g24(.a(x7), .b(x4), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n26_), .c(x1), .O(new_n41_));
  nand3  g26(.a(new_n31_), .b(x2), .c(new_n16_), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n21_), .O(new_n44_));
  nand3  g29(.a(x2), .b(new_n16_), .c(x0), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n44_), .c(x3), .O(z3));
  nand2  g31(.a(new_n17_), .b(x2), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n27_), .c(new_n21_), .O(new_n48_));
  aoi21  g33(.a(new_n47_), .b(new_n34_), .c(x0), .O(new_n49_));
  oai21  g34(.a(new_n49_), .b(new_n48_), .c(new_n16_), .O(new_n50_));
  oai21  g35(.a(new_n22_), .b(x2), .c(new_n17_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n21_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n50_), .O(z4));
  nand4  g38(.a(x5), .b(new_n17_), .c(x2), .d(new_n21_), .O(new_n54_));
  inv1   g39(.a(new_n54_), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n48_), .c(new_n16_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n52_), .O(z5));
  nand4  g42(.a(new_n31_), .b(new_n17_), .c(x2), .d(new_n21_), .O(new_n58_));
  inv1   g43(.a(new_n58_), .O(new_n59_));
  oai21  g44(.a(new_n59_), .b(new_n48_), .c(new_n16_), .O(new_n60_));
  nor2   g45(.a(x4), .b(x3), .O(new_n61_));
  nand4  g46(.a(new_n61_), .b(new_n26_), .c(x1), .d(new_n21_), .O(new_n62_));
  nand2  g47(.a(new_n62_), .b(new_n60_), .O(z6));
endmodule


