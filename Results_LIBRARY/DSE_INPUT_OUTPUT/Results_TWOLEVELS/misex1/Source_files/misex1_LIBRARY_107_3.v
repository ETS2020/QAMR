// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:17 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x1), .O(new_n16_));
  inv1   g01(.a(x2), .O(new_n17_));
  nor2   g02(.a(new_n17_), .b(x0), .O(new_n18_));
  aoi21  g03(.a(new_n18_), .b(x3), .c(x6), .O(new_n19_));
  nand2  g04(.a(new_n16_), .b(x0), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  nand2  g06(.a(new_n21_), .b(x2), .O(new_n22_));
  oai22  g07(.a(new_n22_), .b(new_n20_), .c(new_n19_), .d(new_n16_), .O(z0));
  inv1   g08(.a(x0), .O(new_n24_));
  oai21  g09(.a(x5), .b(x3), .c(x2), .O(new_n25_));
  nand3  g10(.a(x6), .b(new_n21_), .c(new_n17_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(new_n25_), .O(new_n27_));
  nand2  g12(.a(new_n27_), .b(new_n16_), .O(new_n28_));
  inv1   g13(.a(x6), .O(new_n29_));
  nor2   g14(.a(x4), .b(new_n21_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n21_), .O(new_n31_));
  nand4  g16(.a(new_n31_), .b(new_n29_), .c(new_n17_), .d(x1), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nand4  g19(.a(x3), .b(new_n17_), .c(new_n16_), .d(x0), .O(new_n35_));
  inv1   g20(.a(new_n35_), .O(new_n36_));
  aoi21  g21(.a(x6), .b(x1), .c(new_n36_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(new_n34_), .O(z1));
  inv1   g23(.a(x5), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x2), .O(new_n40_));
  oai21  g25(.a(x6), .b(x2), .c(new_n40_), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n21_), .c(new_n16_), .O(new_n42_));
  inv1   g27(.a(x4), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(x2), .c(new_n21_), .O(new_n44_));
  nand3  g29(.a(new_n44_), .b(new_n29_), .c(x1), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n42_), .O(new_n46_));
  nand2  g31(.a(new_n46_), .b(new_n24_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n35_), .O(z2));
  nand3  g33(.a(new_n39_), .b(x2), .c(new_n16_), .O(new_n49_));
  nand4  g34(.a(new_n29_), .b(new_n43_), .c(new_n17_), .d(x1), .O(new_n50_));
  aoi21  g35(.a(new_n50_), .b(new_n49_), .c(x0), .O(new_n51_));
  nand3  g36(.a(x2), .b(new_n16_), .c(x0), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  oai21  g38(.a(new_n53_), .b(new_n51_), .c(new_n21_), .O(new_n54_));
  nor3   g39(.a(x7), .b(x2), .c(x0), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(x6), .c(x1), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n54_), .O(z3));
  nand2  g42(.a(x3), .b(new_n17_), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n22_), .c(new_n24_), .O(new_n59_));
  aoi21  g44(.a(x3), .b(new_n17_), .c(x0), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n59_), .c(new_n16_), .O(new_n61_));
  nor2   g46(.a(x3), .b(new_n17_), .O(new_n62_));
  nor2   g47(.a(new_n62_), .b(x6), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(x1), .c(new_n24_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(z4));
  aoi21  g50(.a(new_n26_), .b(new_n25_), .c(x0), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n59_), .c(new_n16_), .O(new_n67_));
  aoi21  g52(.a(new_n63_), .b(new_n24_), .c(x6), .O(new_n68_));
  oai21  g53(.a(new_n68_), .b(new_n16_), .c(new_n67_), .O(z5));
  nand3  g54(.a(new_n18_), .b(new_n39_), .c(new_n21_), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n59_), .c(new_n16_), .O(new_n72_));
  aoi21  g57(.a(new_n43_), .b(new_n17_), .c(x3), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(x0), .c(new_n29_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(x1), .O(new_n75_));
  nand2  g60(.a(new_n75_), .b(new_n72_), .O(z6));
endmodule


