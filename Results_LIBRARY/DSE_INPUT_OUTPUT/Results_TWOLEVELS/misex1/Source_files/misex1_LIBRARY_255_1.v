// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  nor2   g04(.a(x7), .b(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  inv1   g07(.a(x7), .O(new_n23_));
  inv1   g08(.a(x3), .O(new_n24_));
  inv1   g09(.a(x5), .O(new_n25_));
  nand2  g10(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(x2), .c(new_n19_), .O(new_n27_));
  nand3  g12(.a(x6), .b(new_n24_), .c(new_n16_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nor2   g14(.a(x2), .b(new_n19_), .O(new_n30_));
  aoi21  g15(.a(new_n29_), .b(new_n23_), .c(new_n30_), .O(new_n31_));
  nand2  g16(.a(new_n16_), .b(x0), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n24_), .c(new_n23_), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n19_), .O(new_n34_));
  oai21  g19(.a(new_n31_), .b(x0), .c(new_n34_), .O(z1));
  inv1   g20(.a(x6), .O(new_n36_));
  nand2  g21(.a(new_n36_), .b(new_n16_), .O(new_n37_));
  nor2   g22(.a(x7), .b(x5), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x2), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n37_), .c(x3), .O(new_n40_));
  aoi21  g25(.a(x4), .b(new_n16_), .c(x3), .O(new_n41_));
  nor2   g26(.a(new_n41_), .b(new_n19_), .O(new_n42_));
  aoi21  g27(.a(new_n40_), .b(new_n19_), .c(new_n42_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(x0), .c(new_n34_), .O(z2));
  oai21  g29(.a(x4), .b(x3), .c(x7), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n16_), .c(x1), .O(new_n46_));
  nand4  g31(.a(new_n38_), .b(new_n24_), .c(x2), .d(new_n19_), .O(new_n47_));
  nand2  g32(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n17_), .O(new_n49_));
  nand4  g34(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n49_), .O(z3));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x0), .O(new_n53_));
  nand2  g38(.a(x2), .b(new_n17_), .O(new_n54_));
  nand4  g39(.a(new_n36_), .b(new_n24_), .c(new_n16_), .d(new_n17_), .O(new_n55_));
  nand4  g40(.a(new_n55_), .b(new_n54_), .c(new_n53_), .d(new_n23_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n19_), .O(new_n57_));
  aoi21  g42(.a(new_n24_), .b(x2), .c(new_n19_), .O(new_n58_));
  nand4  g43(.a(new_n23_), .b(x6), .c(new_n24_), .d(new_n16_), .O(new_n59_));
  inv1   g44(.a(new_n59_), .O(new_n60_));
  oai21  g45(.a(new_n60_), .b(new_n58_), .c(new_n17_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n57_), .O(z4));
  nand3  g47(.a(new_n26_), .b(x2), .c(new_n17_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n53_), .c(x1), .O(new_n64_));
  nand4  g49(.a(x6), .b(new_n24_), .c(new_n16_), .d(new_n17_), .O(new_n65_));
  inv1   g50(.a(new_n65_), .O(new_n66_));
  oai21  g51(.a(new_n66_), .b(new_n64_), .c(new_n23_), .O(new_n67_));
  nand2  g52(.a(new_n58_), .b(new_n17_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n67_), .O(z5));
  nand4  g54(.a(new_n25_), .b(new_n24_), .c(x2), .d(new_n17_), .O(new_n70_));
  nand3  g55(.a(new_n70_), .b(new_n53_), .c(new_n23_), .O(new_n71_));
  nand2  g56(.a(new_n71_), .b(new_n19_), .O(new_n72_));
  oai21  g57(.a(x4), .b(x2), .c(new_n24_), .O(new_n73_));
  nand3  g58(.a(new_n73_), .b(x1), .c(new_n17_), .O(new_n74_));
  nand2  g59(.a(new_n74_), .b(new_n72_), .O(z6));
endmodule


