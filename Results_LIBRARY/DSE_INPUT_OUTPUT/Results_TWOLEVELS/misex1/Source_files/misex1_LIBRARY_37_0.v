// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_;
  inv1   g00(.a(x0), .O(new_n16_));
  nand2  g01(.a(x1), .b(new_n16_), .O(new_n17_));
  nand2  g02(.a(x3), .b(x2), .O(new_n18_));
  oai22  g03(.a(new_n18_), .b(new_n17_), .c(x3), .d(x1), .O(z0));
  inv1   g04(.a(x3), .O(new_n20_));
  inv1   g05(.a(x2), .O(new_n21_));
  inv1   g06(.a(x4), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  oai21  g08(.a(new_n23_), .b(x0), .c(x1), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand2  g10(.a(new_n22_), .b(new_n20_), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n21_), .c(x1), .O(new_n27_));
  inv1   g12(.a(x1), .O(new_n28_));
  nand3  g13(.a(x3), .b(x2), .c(new_n28_), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n16_), .O(new_n31_));
  nand4  g16(.a(x3), .b(new_n21_), .c(new_n28_), .d(x0), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n31_), .c(new_n25_), .O(z1));
  oai21  g18(.a(new_n22_), .b(x2), .c(new_n20_), .O(new_n34_));
  nand3  g19(.a(new_n34_), .b(x1), .c(new_n16_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n32_), .O(z2));
  nand2  g21(.a(new_n26_), .b(x7), .O(new_n37_));
  nand4  g22(.a(new_n37_), .b(new_n21_), .c(x1), .d(new_n16_), .O(new_n38_));
  inv1   g23(.a(new_n38_), .O(z3));
  aoi21  g24(.a(new_n21_), .b(new_n28_), .c(x0), .O(new_n40_));
  nand3  g25(.a(new_n21_), .b(new_n28_), .c(x0), .O(new_n41_));
  inv1   g26(.a(new_n41_), .O(new_n42_));
  oai21  g27(.a(new_n42_), .b(new_n40_), .c(x3), .O(new_n43_));
  nand2  g28(.a(new_n22_), .b(x3), .O(new_n44_));
  nand4  g29(.a(new_n44_), .b(new_n21_), .c(x1), .d(new_n16_), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n43_), .O(z4));
  oai21  g31(.a(x4), .b(x2), .c(new_n20_), .O(new_n47_));
  nand3  g32(.a(new_n47_), .b(x1), .c(new_n16_), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(new_n32_), .O(z6));
  nand2  g34(.a(new_n45_), .b(new_n43_), .O(z5));
endmodule


