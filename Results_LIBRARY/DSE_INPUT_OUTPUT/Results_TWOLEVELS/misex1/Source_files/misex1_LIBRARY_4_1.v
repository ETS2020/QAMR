// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand3  g02(.a(x3), .b(x1), .c(new_n17_), .O(new_n18_));
  inv1   g03(.a(x1), .O(new_n19_));
  inv1   g04(.a(x3), .O(new_n20_));
  nand3  g05(.a(new_n20_), .b(new_n19_), .c(x0), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n18_), .c(new_n16_), .O(z0));
  nor2   g07(.a(x6), .b(x1), .O(new_n23_));
  nor3   g08(.a(new_n23_), .b(x3), .c(x2), .O(new_n24_));
  nor2   g09(.a(x5), .b(x3), .O(new_n25_));
  nor2   g10(.a(new_n25_), .b(new_n16_), .O(new_n26_));
  aoi21  g11(.a(new_n26_), .b(new_n19_), .c(new_n24_), .O(new_n27_));
  nor2   g12(.a(new_n27_), .b(x0), .O(z1));
  nand2  g13(.a(x3), .b(x2), .O(new_n29_));
  nand3  g14(.a(x4), .b(new_n20_), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x1), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(x2), .O(new_n34_));
  oai21  g19(.a(x6), .b(x2), .c(new_n34_), .O(new_n35_));
  nand3  g20(.a(new_n35_), .b(new_n20_), .c(new_n19_), .O(new_n36_));
  aoi21  g21(.a(new_n36_), .b(new_n32_), .c(x0), .O(z2));
  nand2  g22(.a(x7), .b(x4), .O(new_n38_));
  nand3  g23(.a(new_n38_), .b(new_n16_), .c(x1), .O(new_n39_));
  oai21  g24(.a(new_n34_), .b(x1), .c(new_n39_), .O(new_n40_));
  nand2  g25(.a(new_n40_), .b(new_n17_), .O(new_n41_));
  nand3  g26(.a(x2), .b(new_n19_), .c(x0), .O(new_n42_));
  aoi21  g27(.a(new_n42_), .b(new_n41_), .c(x3), .O(z3));
  aoi21  g28(.a(new_n20_), .b(x1), .c(new_n16_), .O(new_n44_));
  nor2   g29(.a(x6), .b(new_n19_), .O(new_n45_));
  aoi21  g30(.a(new_n45_), .b(new_n19_), .c(x3), .O(new_n46_));
  aoi21  g31(.a(new_n46_), .b(new_n16_), .c(new_n44_), .O(new_n47_));
  nand4  g32(.a(new_n20_), .b(x2), .c(new_n19_), .d(x0), .O(new_n48_));
  oai21  g33(.a(new_n47_), .b(x0), .c(new_n48_), .O(z4));
  aoi21  g34(.a(x5), .b(new_n19_), .c(x3), .O(new_n50_));
  nor2   g35(.a(new_n50_), .b(new_n16_), .O(new_n51_));
  oai21  g36(.a(new_n51_), .b(new_n24_), .c(new_n17_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n48_), .O(z5));
  inv1   g38(.a(x4), .O(new_n54_));
  nand3  g39(.a(new_n54_), .b(new_n20_), .c(new_n16_), .O(new_n55_));
  aoi21  g40(.a(new_n55_), .b(new_n29_), .c(new_n19_), .O(new_n56_));
  nand3  g41(.a(new_n25_), .b(x2), .c(new_n19_), .O(new_n57_));
  inv1   g42(.a(new_n57_), .O(new_n58_));
  oai21  g43(.a(new_n58_), .b(new_n56_), .c(new_n17_), .O(new_n59_));
  nand2  g44(.a(x3), .b(new_n16_), .O(new_n60_));
  nand3  g45(.a(new_n60_), .b(new_n59_), .c(new_n48_), .O(z6));
endmodule


