// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x3), .b(x1), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nand2  g04(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  inv1   g05(.a(x3), .O(new_n21_));
  inv1   g06(.a(x1), .O(new_n22_));
  nand2  g07(.a(new_n22_), .b(x0), .O(new_n23_));
  inv1   g08(.a(new_n23_), .O(new_n24_));
  nand3  g09(.a(new_n24_), .b(x7), .c(new_n21_), .O(new_n25_));
  aoi21  g10(.a(new_n25_), .b(new_n20_), .c(new_n16_), .O(z0));
  aoi21  g11(.a(x7), .b(x5), .c(x3), .O(new_n27_));
  nor3   g12(.a(new_n27_), .b(new_n16_), .c(x1), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x3), .c(new_n22_), .O(new_n30_));
  inv1   g15(.a(x6), .O(new_n31_));
  nor2   g16(.a(new_n31_), .b(x3), .O(new_n32_));
  oai21  g17(.a(new_n32_), .b(new_n30_), .c(x7), .O(new_n33_));
  aoi21  g18(.a(new_n33_), .b(new_n18_), .c(x2), .O(new_n34_));
  oai21  g19(.a(new_n34_), .b(new_n28_), .c(new_n17_), .O(new_n35_));
  nand2  g20(.a(x3), .b(new_n16_), .O(new_n36_));
  inv1   g21(.a(new_n36_), .O(new_n37_));
  nor2   g22(.a(x7), .b(x3), .O(new_n38_));
  aoi21  g23(.a(new_n37_), .b(new_n24_), .c(new_n38_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n35_), .O(z1));
  inv1   g25(.a(x7), .O(new_n41_));
  inv1   g26(.a(x5), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(x2), .O(new_n43_));
  nand2  g28(.a(new_n31_), .b(new_n16_), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n21_), .c(new_n22_), .O(new_n46_));
  nand3  g31(.a(x4), .b(new_n16_), .c(x1), .O(new_n47_));
  aoi21  g32(.a(new_n47_), .b(new_n46_), .c(new_n41_), .O(new_n48_));
  oai21  g33(.a(new_n48_), .b(new_n19_), .c(new_n17_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n39_), .O(z2));
  oai21  g35(.a(x4), .b(x3), .c(x7), .O(new_n51_));
  nand3  g36(.a(new_n51_), .b(new_n16_), .c(x1), .O(new_n52_));
  inv1   g37(.a(new_n52_), .O(new_n53_));
  nand3  g38(.a(x7), .b(new_n42_), .c(new_n21_), .O(new_n54_));
  nor3   g39(.a(new_n54_), .b(new_n16_), .c(x1), .O(new_n55_));
  oai21  g40(.a(new_n55_), .b(new_n53_), .c(new_n17_), .O(new_n56_));
  aoi21  g41(.a(new_n24_), .b(x2), .c(new_n41_), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(x3), .c(new_n56_), .O(z3));
  nand3  g43(.a(x7), .b(new_n21_), .c(x2), .O(new_n59_));
  aoi21  g44(.a(new_n59_), .b(new_n36_), .c(new_n17_), .O(new_n60_));
  aoi21  g45(.a(new_n42_), .b(x3), .c(new_n16_), .O(new_n61_));
  nor3   g46(.a(x6), .b(x3), .c(x2), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n61_), .c(x7), .O(new_n63_));
  nand2  g48(.a(x3), .b(x2), .O(new_n64_));
  aoi21  g49(.a(new_n64_), .b(new_n63_), .c(x0), .O(new_n65_));
  oai21  g50(.a(new_n65_), .b(new_n60_), .c(new_n22_), .O(new_n66_));
  nand2  g51(.a(new_n29_), .b(x3), .O(new_n67_));
  nand3  g52(.a(new_n67_), .b(x7), .c(new_n16_), .O(new_n68_));
  aoi21  g53(.a(new_n68_), .b(new_n21_), .c(new_n22_), .O(new_n69_));
  nand4  g54(.a(x7), .b(x6), .c(new_n21_), .d(new_n16_), .O(new_n70_));
  inv1   g55(.a(new_n70_), .O(new_n71_));
  oai21  g56(.a(new_n71_), .b(new_n69_), .c(new_n17_), .O(new_n72_));
  nand2  g57(.a(new_n72_), .b(new_n66_), .O(z4));
  inv1   g58(.a(new_n38_), .O(new_n74_));
  nor3   g59(.a(new_n27_), .b(new_n16_), .c(x0), .O(new_n75_));
  oai21  g60(.a(new_n75_), .b(new_n60_), .c(new_n22_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(new_n72_), .c(new_n74_), .O(z5));
  nor3   g62(.a(new_n54_), .b(new_n16_), .c(x0), .O(new_n78_));
  oai21  g63(.a(new_n78_), .b(new_n60_), .c(new_n22_), .O(new_n79_));
  nand2  g64(.a(x7), .b(new_n29_), .O(new_n80_));
  oai21  g65(.a(new_n80_), .b(x2), .c(new_n21_), .O(new_n81_));
  nand3  g66(.a(new_n81_), .b(x1), .c(new_n17_), .O(new_n82_));
  nand2  g67(.a(new_n82_), .b(new_n79_), .O(z6));
endmodule


