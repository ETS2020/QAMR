// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:38 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x0), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x5), .O(new_n18_));
  nand2  g03(.a(x3), .b(x2), .O(new_n19_));
  oai21  g04(.a(new_n19_), .b(new_n17_), .c(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  inv1   g06(.a(x2), .O(new_n22_));
  nor2   g07(.a(x3), .b(new_n22_), .O(new_n23_));
  nand3  g08(.a(new_n23_), .b(new_n17_), .c(x0), .O(new_n24_));
  nand2  g09(.a(new_n24_), .b(new_n21_), .O(z0));
  xnor2a g10(.a(x2), .b(x1), .O(new_n26_));
  nand3  g11(.a(new_n22_), .b(new_n17_), .c(x0), .O(new_n27_));
  oai21  g12(.a(new_n26_), .b(x0), .c(new_n27_), .O(new_n28_));
  nand2  g13(.a(new_n28_), .b(x3), .O(new_n29_));
  inv1   g14(.a(x3), .O(new_n30_));
  oai21  g15(.a(x4), .b(new_n30_), .c(x1), .O(new_n31_));
  nand2  g16(.a(x6), .b(new_n30_), .O(new_n32_));
  aoi21  g17(.a(new_n32_), .b(new_n31_), .c(x2), .O(new_n33_));
  oai21  g18(.a(new_n33_), .b(x5), .c(new_n16_), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n29_), .O(z1));
  nand2  g20(.a(x6), .b(new_n22_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n30_), .c(new_n17_), .O(new_n37_));
  inv1   g22(.a(x4), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(x2), .c(new_n30_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(x1), .O(new_n40_));
  nand3  g25(.a(new_n40_), .b(new_n37_), .c(new_n18_), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n16_), .O(new_n42_));
  nor2   g27(.a(new_n30_), .b(x2), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n17_), .c(x0), .O(new_n44_));
  nand2  g29(.a(new_n44_), .b(new_n42_), .O(z2));
  oai21  g30(.a(x4), .b(x3), .c(x7), .O(new_n46_));
  nand3  g31(.a(new_n46_), .b(new_n22_), .c(x1), .O(new_n47_));
  nand2  g32(.a(new_n23_), .b(new_n17_), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(new_n47_), .c(new_n18_), .O(new_n49_));
  nand2  g34(.a(new_n49_), .b(new_n16_), .O(new_n50_));
  nand2  g35(.a(new_n50_), .b(new_n24_), .O(z3));
  xor2a  g36(.a(x3), .b(x2), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(x0), .O(new_n53_));
  nor2   g38(.a(x6), .b(x3), .O(new_n54_));
  oai21  g39(.a(new_n54_), .b(x2), .c(new_n16_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n17_), .O(new_n57_));
  oai21  g42(.a(x3), .b(new_n22_), .c(x1), .O(new_n58_));
  nand3  g43(.a(x6), .b(new_n30_), .c(new_n22_), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n18_), .O(new_n60_));
  nand2  g45(.a(new_n60_), .b(new_n16_), .O(new_n61_));
  nand2  g46(.a(new_n61_), .b(new_n57_), .O(z4));
  oai21  g47(.a(new_n19_), .b(x0), .c(new_n53_), .O(new_n63_));
  nand2  g48(.a(new_n63_), .b(new_n17_), .O(new_n64_));
  nand2  g49(.a(new_n64_), .b(new_n61_), .O(z5));
  aoi21  g50(.a(new_n43_), .b(x0), .c(new_n23_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n30_), .O(new_n67_));
  aoi21  g52(.a(new_n67_), .b(x1), .c(x5), .O(new_n68_));
  oai22  g53(.a(new_n68_), .b(x0), .c(new_n66_), .d(x1), .O(z6));
endmodule


