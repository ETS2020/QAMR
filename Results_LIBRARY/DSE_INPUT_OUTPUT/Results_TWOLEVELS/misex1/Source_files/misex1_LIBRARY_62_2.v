// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  inv1   g02(.a(x1), .O(new_n18_));
  inv1   g03(.a(x3), .O(new_n19_));
  nor2   g04(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nand2  g05(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g06(.a(new_n19_), .b(new_n18_), .c(x0), .O(new_n22_));
  aoi21  g07(.a(new_n22_), .b(new_n21_), .c(new_n16_), .O(z0));
  nor2   g08(.a(x4), .b(x3), .O(new_n24_));
  nor3   g09(.a(new_n24_), .b(x2), .c(new_n18_), .O(new_n25_));
  nor2   g10(.a(x5), .b(x3), .O(new_n26_));
  nor3   g11(.a(new_n26_), .b(new_n16_), .c(x1), .O(new_n27_));
  oai21  g12(.a(new_n27_), .b(new_n25_), .c(new_n17_), .O(new_n28_));
  inv1   g13(.a(x4), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(x1), .c(x6), .O(new_n30_));
  nor2   g15(.a(new_n30_), .b(x3), .O(new_n31_));
  oai21  g16(.a(new_n31_), .b(x0), .c(new_n16_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(z1));
  inv1   g18(.a(x5), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(x2), .O(new_n35_));
  oai21  g20(.a(x6), .b(x2), .c(new_n35_), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(new_n19_), .c(new_n18_), .O(new_n37_));
  oai21  g22(.a(new_n29_), .b(x2), .c(new_n19_), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(x1), .O(new_n39_));
  aoi21  g24(.a(new_n39_), .b(new_n37_), .c(x0), .O(z2));
  oai21  g25(.a(x3), .b(x1), .c(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(x0), .O(new_n42_));
  oai21  g27(.a(x4), .b(x3), .c(x7), .O(new_n43_));
  nand3  g28(.a(new_n43_), .b(new_n16_), .c(x1), .O(new_n44_));
  nand4  g29(.a(new_n26_), .b(x2), .c(new_n18_), .d(new_n17_), .O(new_n45_));
  nand3  g30(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(z3));
  oai21  g31(.a(x2), .b(x1), .c(x3), .O(new_n47_));
  nand2  g32(.a(new_n34_), .b(x3), .O(new_n48_));
  nand3  g33(.a(new_n48_), .b(x2), .c(new_n18_), .O(new_n49_));
  nand2  g34(.a(new_n19_), .b(new_n16_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n17_), .O(new_n52_));
  nand4  g37(.a(new_n19_), .b(x2), .c(new_n18_), .d(x0), .O(new_n53_));
  nand2  g38(.a(new_n53_), .b(new_n52_), .O(z4));
  aoi21  g39(.a(x5), .b(new_n18_), .c(x3), .O(new_n55_));
  nor2   g40(.a(new_n55_), .b(new_n16_), .O(new_n56_));
  oai21  g41(.a(new_n56_), .b(new_n25_), .c(new_n17_), .O(new_n57_));
  nand2  g42(.a(new_n31_), .b(new_n16_), .O(new_n58_));
  nand3  g43(.a(new_n58_), .b(new_n57_), .c(new_n42_), .O(z5));
  nand3  g44(.a(new_n26_), .b(x2), .c(new_n18_), .O(new_n60_));
  inv1   g45(.a(new_n60_), .O(new_n61_));
  oai21  g46(.a(new_n61_), .b(new_n20_), .c(new_n17_), .O(new_n62_));
  nand3  g47(.a(new_n24_), .b(new_n16_), .c(x1), .O(new_n63_));
  nand3  g48(.a(new_n63_), .b(new_n62_), .c(new_n42_), .O(z6));
endmodule


