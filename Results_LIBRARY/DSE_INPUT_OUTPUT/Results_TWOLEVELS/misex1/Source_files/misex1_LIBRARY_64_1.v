// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nand2  g02(.a(x3), .b(x1), .O(new_n18_));
  inv1   g03(.a(new_n18_), .O(new_n19_));
  nor2   g04(.a(x1), .b(new_n17_), .O(new_n20_));
  nor2   g05(.a(x6), .b(x3), .O(new_n21_));
  aoi22  g06(.a(new_n21_), .b(new_n20_), .c(new_n19_), .d(new_n17_), .O(new_n22_));
  inv1   g07(.a(x6), .O(new_n23_));
  nor2   g08(.a(new_n23_), .b(x3), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  oai21  g10(.a(new_n22_), .b(new_n16_), .c(new_n25_), .O(z0));
  inv1   g11(.a(x1), .O(new_n27_));
  inv1   g12(.a(x3), .O(new_n28_));
  nand2  g13(.a(new_n23_), .b(x5), .O(new_n29_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(new_n16_), .O(new_n30_));
  nand2  g15(.a(new_n30_), .b(new_n27_), .O(new_n31_));
  oai21  g16(.a(x4), .b(new_n28_), .c(new_n23_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n28_), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n16_), .c(x1), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g20(.a(new_n35_), .b(new_n17_), .O(new_n36_));
  nand2  g21(.a(x3), .b(new_n16_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  aoi21  g23(.a(new_n38_), .b(new_n20_), .c(new_n24_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n36_), .O(z1));
  inv1   g25(.a(new_n20_), .O(new_n41_));
  nand2  g26(.a(x5), .b(x2), .O(new_n42_));
  nand3  g27(.a(new_n42_), .b(new_n28_), .c(new_n27_), .O(new_n43_));
  nand3  g28(.a(x4), .b(new_n16_), .c(x1), .O(new_n44_));
  aoi21  g29(.a(new_n44_), .b(new_n43_), .c(x6), .O(new_n45_));
  oai21  g30(.a(new_n45_), .b(new_n19_), .c(new_n17_), .O(new_n46_));
  oai21  g31(.a(new_n37_), .b(new_n41_), .c(new_n46_), .O(z2));
  inv1   g32(.a(x4), .O(new_n48_));
  nand3  g33(.a(new_n23_), .b(new_n48_), .c(new_n28_), .O(new_n49_));
  aoi21  g34(.a(new_n49_), .b(x7), .c(x2), .O(new_n50_));
  inv1   g35(.a(x5), .O(new_n51_));
  nand3  g36(.a(new_n23_), .b(new_n51_), .c(new_n28_), .O(new_n52_));
  nor3   g37(.a(new_n52_), .b(new_n16_), .c(x1), .O(new_n53_));
  aoi21  g38(.a(new_n50_), .b(x1), .c(new_n53_), .O(new_n54_));
  oai21  g39(.a(new_n41_), .b(new_n16_), .c(new_n23_), .O(new_n55_));
  nand2  g40(.a(new_n55_), .b(new_n28_), .O(new_n56_));
  oai21  g41(.a(new_n54_), .b(x0), .c(new_n56_), .O(z3));
  nand2  g42(.a(new_n21_), .b(x2), .O(new_n58_));
  aoi21  g43(.a(new_n58_), .b(new_n37_), .c(new_n17_), .O(new_n59_));
  aoi21  g44(.a(x5), .b(x2), .c(new_n28_), .O(new_n60_));
  nand2  g45(.a(x3), .b(x2), .O(new_n61_));
  oai21  g46(.a(new_n60_), .b(x6), .c(new_n61_), .O(new_n62_));
  aoi21  g47(.a(new_n62_), .b(new_n17_), .c(new_n59_), .O(new_n63_));
  oai21  g48(.a(new_n32_), .b(x2), .c(new_n28_), .O(new_n64_));
  nand3  g49(.a(new_n64_), .b(x1), .c(new_n17_), .O(new_n65_));
  oai21  g50(.a(new_n63_), .b(x1), .c(new_n65_), .O(z4));
  nand2  g51(.a(new_n58_), .b(new_n37_), .O(new_n67_));
  nand2  g52(.a(new_n67_), .b(x0), .O(new_n68_));
  nand2  g53(.a(new_n30_), .b(new_n17_), .O(new_n69_));
  nand2  g54(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g55(.a(new_n70_), .b(new_n27_), .O(new_n71_));
  nand3  g56(.a(new_n71_), .b(new_n65_), .c(new_n25_), .O(z5));
  nor3   g57(.a(new_n52_), .b(new_n16_), .c(x0), .O(new_n73_));
  oai21  g58(.a(new_n73_), .b(new_n59_), .c(new_n27_), .O(new_n74_));
  oai21  g59(.a(new_n49_), .b(x2), .c(new_n28_), .O(new_n75_));
  nand3  g60(.a(new_n75_), .b(x1), .c(new_n17_), .O(new_n76_));
  nand3  g61(.a(new_n76_), .b(new_n74_), .c(new_n25_), .O(z6));
endmodule


