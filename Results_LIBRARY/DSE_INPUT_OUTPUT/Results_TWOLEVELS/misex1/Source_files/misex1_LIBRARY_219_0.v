// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x2), .O(new_n16_));
  inv1   g01(.a(x1), .O(new_n17_));
  inv1   g02(.a(x3), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n17_), .c(x0), .O(new_n19_));
  inv1   g04(.a(x0), .O(new_n20_));
  nand4  g05(.a(x6), .b(x3), .c(x1), .d(new_n20_), .O(new_n21_));
  aoi21  g06(.a(new_n21_), .b(new_n19_), .c(new_n16_), .O(z0));
  nand2  g07(.a(x3), .b(x2), .O(new_n23_));
  nand2  g08(.a(x5), .b(new_n18_), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(new_n23_), .c(x1), .O(new_n25_));
  aoi21  g10(.a(x3), .b(new_n17_), .c(x2), .O(new_n26_));
  oai21  g11(.a(new_n26_), .b(new_n25_), .c(x6), .O(new_n27_));
  nand4  g12(.a(x3), .b(new_n16_), .c(new_n17_), .d(x0), .O(new_n28_));
  oai21  g13(.a(new_n27_), .b(x0), .c(new_n28_), .O(z1));
  nand3  g14(.a(x3), .b(new_n16_), .c(x0), .O(new_n30_));
  inv1   g15(.a(new_n30_), .O(new_n31_));
  nand2  g16(.a(x2), .b(new_n20_), .O(new_n32_));
  inv1   g17(.a(x5), .O(new_n33_));
  nand3  g18(.a(x6), .b(new_n33_), .c(new_n18_), .O(new_n34_));
  nor2   g19(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  oai21  g20(.a(new_n35_), .b(new_n31_), .c(new_n17_), .O(new_n36_));
  inv1   g21(.a(x4), .O(new_n37_));
  oai21  g22(.a(new_n37_), .b(x2), .c(new_n18_), .O(new_n38_));
  nand4  g23(.a(new_n38_), .b(x6), .c(x1), .d(new_n20_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n36_), .O(z2));
  nand3  g25(.a(x6), .b(new_n37_), .c(new_n18_), .O(new_n41_));
  aoi21  g26(.a(new_n41_), .b(x7), .c(x2), .O(new_n42_));
  nand2  g27(.a(x2), .b(new_n17_), .O(new_n43_));
  oai21  g28(.a(new_n43_), .b(new_n34_), .c(x6), .O(new_n44_));
  aoi21  g29(.a(new_n42_), .b(x1), .c(new_n44_), .O(new_n45_));
  nand4  g30(.a(new_n18_), .b(x2), .c(new_n17_), .d(x0), .O(new_n46_));
  oai21  g31(.a(new_n45_), .b(x0), .c(new_n46_), .O(z3));
  xor2a  g32(.a(x3), .b(x2), .O(new_n48_));
  nand2  g33(.a(new_n48_), .b(x0), .O(new_n49_));
  oai21  g34(.a(new_n33_), .b(x3), .c(new_n16_), .O(new_n50_));
  nand3  g35(.a(new_n50_), .b(x6), .c(new_n20_), .O(new_n51_));
  nand2  g36(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand2  g37(.a(new_n52_), .b(new_n17_), .O(new_n53_));
  nand2  g38(.a(x3), .b(x1), .O(new_n54_));
  nand2  g39(.a(new_n18_), .b(new_n16_), .O(new_n55_));
  nand3  g40(.a(new_n55_), .b(new_n54_), .c(x6), .O(new_n56_));
  nand2  g41(.a(new_n56_), .b(new_n20_), .O(new_n57_));
  nand2  g42(.a(new_n57_), .b(new_n53_), .O(z4));
  nand2  g43(.a(x3), .b(new_n16_), .O(new_n59_));
  nand2  g44(.a(new_n18_), .b(x2), .O(new_n60_));
  aoi21  g45(.a(new_n60_), .b(new_n59_), .c(new_n20_), .O(new_n61_));
  inv1   g46(.a(x6), .O(new_n62_));
  aoi21  g47(.a(new_n24_), .b(new_n23_), .c(new_n62_), .O(new_n63_));
  aoi21  g48(.a(new_n63_), .b(new_n20_), .c(new_n61_), .O(new_n64_));
  oai21  g49(.a(new_n64_), .b(x1), .c(new_n57_), .O(z5));
  oai21  g50(.a(new_n61_), .b(new_n35_), .c(new_n17_), .O(new_n66_));
  oai21  g51(.a(x4), .b(x2), .c(new_n18_), .O(new_n67_));
  nand4  g52(.a(new_n67_), .b(x6), .c(x1), .d(new_n20_), .O(new_n68_));
  nand2  g53(.a(new_n68_), .b(new_n66_), .O(z6));
endmodule


