// Benchmark "FAU" written by ABC on Wed Aug 19 17:17:22 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_;
  inv1   g00(.a(x3), .O(new_n16_));
  inv1   g01(.a(x0), .O(new_n17_));
  nor2   g02(.a(x1), .b(new_n17_), .O(new_n18_));
  nand3  g03(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n19_));
  inv1   g04(.a(new_n19_), .O(z0));
  inv1   g05(.a(x2), .O(new_n21_));
  oai21  g06(.a(x5), .b(x3), .c(x2), .O(new_n22_));
  nor2   g07(.a(new_n22_), .b(x1), .O(new_n23_));
  inv1   g08(.a(x4), .O(new_n24_));
  aoi21  g09(.a(new_n24_), .b(x1), .c(x6), .O(new_n25_));
  nand2  g10(.a(new_n24_), .b(new_n16_), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x1), .O(new_n27_));
  oai21  g12(.a(new_n25_), .b(x3), .c(new_n27_), .O(new_n28_));
  aoi21  g13(.a(new_n28_), .b(new_n21_), .c(new_n23_), .O(new_n29_));
  nand3  g14(.a(new_n18_), .b(x3), .c(new_n21_), .O(new_n30_));
  oai21  g15(.a(new_n29_), .b(x0), .c(new_n30_), .O(z1));
  inv1   g16(.a(x1), .O(new_n32_));
  inv1   g17(.a(x6), .O(new_n33_));
  nand3  g18(.a(new_n33_), .b(new_n16_), .c(new_n32_), .O(new_n34_));
  aoi21  g19(.a(new_n34_), .b(new_n27_), .c(x2), .O(new_n35_));
  nor2   g20(.a(x5), .b(x3), .O(new_n36_));
  nand3  g21(.a(new_n36_), .b(x2), .c(new_n32_), .O(new_n37_));
  inv1   g22(.a(new_n37_), .O(new_n38_));
  oai21  g23(.a(new_n38_), .b(new_n35_), .c(new_n17_), .O(new_n39_));
  nand2  g24(.a(new_n39_), .b(new_n30_), .O(z2));
  nand2  g25(.a(new_n26_), .b(x7), .O(new_n41_));
  nand3  g26(.a(new_n41_), .b(new_n21_), .c(x1), .O(new_n42_));
  nand2  g27(.a(new_n42_), .b(new_n37_), .O(new_n43_));
  nand2  g28(.a(new_n43_), .b(new_n17_), .O(new_n44_));
  nand3  g29(.a(new_n18_), .b(new_n16_), .c(x2), .O(new_n45_));
  nand2  g30(.a(new_n45_), .b(new_n44_), .O(z3));
  nand2  g31(.a(x3), .b(new_n21_), .O(new_n47_));
  nand2  g32(.a(new_n16_), .b(x2), .O(new_n48_));
  aoi21  g33(.a(new_n48_), .b(new_n47_), .c(new_n17_), .O(new_n49_));
  inv1   g34(.a(x5), .O(new_n50_));
  nor2   g35(.a(x6), .b(x2), .O(new_n51_));
  aoi21  g36(.a(new_n50_), .b(x2), .c(new_n51_), .O(new_n52_));
  oai21  g37(.a(new_n52_), .b(x3), .c(new_n22_), .O(new_n53_));
  aoi21  g38(.a(new_n53_), .b(new_n17_), .c(new_n49_), .O(new_n54_));
  nand3  g39(.a(new_n28_), .b(new_n21_), .c(new_n17_), .O(new_n55_));
  oai21  g40(.a(new_n54_), .b(x1), .c(new_n55_), .O(z4));
  nor2   g41(.a(new_n22_), .b(x0), .O(new_n57_));
  oai21  g42(.a(new_n57_), .b(new_n49_), .c(new_n32_), .O(new_n58_));
  nand2  g43(.a(x2), .b(x1), .O(new_n59_));
  nand3  g44(.a(new_n59_), .b(new_n58_), .c(new_n55_), .O(z5));
  nand3  g45(.a(new_n36_), .b(x2), .c(new_n17_), .O(new_n61_));
  inv1   g46(.a(new_n61_), .O(new_n62_));
  oai21  g47(.a(new_n62_), .b(new_n49_), .c(new_n32_), .O(new_n63_));
  nand2  g48(.a(x4), .b(new_n16_), .O(new_n64_));
  nand4  g49(.a(new_n64_), .b(new_n21_), .c(x1), .d(new_n17_), .O(new_n65_));
  nand2  g50(.a(new_n65_), .b(new_n63_), .O(z6));
endmodule


