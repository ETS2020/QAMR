// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x00), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x10), .O(new_n47_));
  nand2  g06(.a(x08), .b(x00), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n44_), .c(x14), .d(new_n47_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n44_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  nand4  g15(.a(new_n52_), .b(x12), .c(x10), .d(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x00), .c(new_n44_), .O(z03));
  nor2   g17(.a(new_n45_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n45_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x00), .O(new_n62_));
  nand2  g21(.a(x09), .b(new_n51_), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n51_), .c(x15), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n62_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nand2  g25(.a(new_n50_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nor2   g27(.a(new_n45_), .b(new_n68_), .O(z08));
  oai21  g28(.a(new_n44_), .b(x00), .c(x09), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(new_n51_), .O(z09));
  aoi22  g30(.a(x15), .b(new_n62_), .c(x09), .d(x08), .O(z10));
  nor2   g31(.a(new_n45_), .b(new_n52_), .O(z11));
  nand2  g32(.a(x13), .b(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x00), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x15), .O(new_n76_));
  nand2  g35(.a(x10), .b(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x13), .O(new_n78_));
  oai21  g37(.a(new_n53_), .b(new_n48_), .c(new_n78_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n76_), .O(z12));
  aoi21  g40(.a(x10), .b(x08), .c(x14), .O(new_n82_));
  nor2   g41(.a(new_n82_), .b(new_n44_), .O(new_n83_));
  nand3  g42(.a(x10), .b(x08), .c(x01), .O(new_n84_));
  and2   g43(.a(new_n84_), .b(x14), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n83_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n50_), .O(z13));
  inv1   g46(.a(x12), .O(new_n88_));
  nand3  g47(.a(new_n77_), .b(x15), .c(x00), .O(new_n89_));
  inv1   g48(.a(new_n53_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(x08), .c(x02), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n89_), .c(new_n88_), .O(z14));
  nand2  g51(.a(x15), .b(x00), .O(new_n93_));
  nand2  g52(.a(new_n77_), .b(new_n44_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(x16), .c(new_n52_), .O(new_n96_));
  nand3  g55(.a(new_n90_), .b(x08), .c(x03), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n88_), .O(z15));
  oai21  g57(.a(new_n66_), .b(new_n88_), .c(x00), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x15), .O(new_n100_));
  nand2  g59(.a(new_n77_), .b(x17), .O(new_n101_));
  nand2  g60(.a(x08), .b(x04), .O(new_n102_));
  oai21  g61(.a(new_n102_), .b(new_n53_), .c(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n100_), .O(z16));
  nand2  g64(.a(x18), .b(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x00), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x15), .O(new_n108_));
  nand2  g67(.a(new_n77_), .b(x18), .O(new_n109_));
  nand2  g68(.a(x08), .b(x05), .O(new_n110_));
  oai21  g69(.a(new_n110_), .b(new_n53_), .c(new_n109_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n108_), .O(z17));
  oai21  g72(.a(new_n60_), .b(new_n88_), .c(x00), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x15), .O(new_n115_));
  nand2  g74(.a(new_n77_), .b(x19), .O(new_n116_));
  nand2  g75(.a(x08), .b(x06), .O(new_n117_));
  oai21  g76(.a(new_n117_), .b(new_n53_), .c(new_n116_), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(x12), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n115_), .O(z18));
  nand2  g79(.a(new_n77_), .b(x20), .O(new_n121_));
  nand3  g80(.a(x10), .b(x08), .c(x07), .O(new_n122_));
  nand2  g81(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g82(.a(new_n123_), .b(new_n44_), .O(new_n124_));
  nand3  g83(.a(x20), .b(x15), .c(x00), .O(new_n125_));
  aoi21  g84(.a(new_n125_), .b(new_n124_), .c(new_n88_), .O(z19));
endmodule


