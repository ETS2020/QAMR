// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n67_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_;
  nand2  g00(.a(x15), .b(x03), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  nand3  g08(.a(new_n46_), .b(x10), .c(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x12), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x11), .c(new_n42_), .O(z02));
  nand4  g12(.a(x12), .b(x10), .c(x08), .d(new_n45_), .O(new_n54_));
  nor3   g13(.a(new_n54_), .b(new_n46_), .c(x14), .O(z03));
  inv1   g14(.a(new_n42_), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n42_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x08), .O(new_n60_));
  nand2  g19(.a(x09), .b(new_n60_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(new_n45_), .c(new_n46_), .O(z06));
  and2   g22(.a(new_n42_), .b(x17), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n42_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x15), .b(x03), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(x09), .c(x08), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z09));
  aoi22  g28(.a(x15), .b(x03), .c(x09), .d(x08), .O(z10));
  nor2   g29(.a(new_n56_), .b(new_n47_), .O(z11));
  aoi21  g30(.a(x13), .b(x12), .c(x03), .O(new_n72_));
  nand2  g31(.a(new_n46_), .b(x10), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x13), .O(new_n75_));
  nand2  g34(.a(x08), .b(x00), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n73_), .c(new_n75_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x12), .O(new_n78_));
  oai21  g37(.a(new_n72_), .b(new_n46_), .c(new_n78_), .O(z12));
  nand4  g38(.a(new_n47_), .b(x10), .c(x08), .d(new_n45_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n47_), .c(new_n46_), .O(new_n81_));
  and2   g40(.a(x10), .b(x08), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(x01), .c(new_n47_), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n81_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n42_), .O(z13));
  nand2  g44(.a(x08), .b(x02), .O(new_n86_));
  oai22  g45(.a(new_n86_), .b(new_n73_), .c(new_n82_), .d(new_n46_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n42_), .O(z14));
  nand3  g48(.a(x12), .b(x10), .c(x08), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x03), .O(new_n92_));
  nand4  g51(.a(new_n50_), .b(x16), .c(new_n47_), .d(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z15));
  inv1   g53(.a(x12), .O(new_n95_));
  inv1   g54(.a(x04), .O(new_n96_));
  nand2  g55(.a(new_n74_), .b(x17), .O(new_n97_));
  oai21  g56(.a(new_n74_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n46_), .O(new_n99_));
  nand3  g58(.a(x17), .b(x15), .c(new_n45_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n95_), .O(z16));
  aoi21  g60(.a(x18), .b(x12), .c(x03), .O(new_n102_));
  nand2  g61(.a(new_n74_), .b(x18), .O(new_n103_));
  nand2  g62(.a(x08), .b(x05), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n73_), .c(new_n103_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x12), .O(new_n106_));
  oai21  g65(.a(new_n102_), .b(new_n46_), .c(new_n106_), .O(z17));
  aoi21  g66(.a(x19), .b(x12), .c(x03), .O(new_n108_));
  nand2  g67(.a(x08), .b(x06), .O(new_n109_));
  oai22  g68(.a(new_n109_), .b(new_n73_), .c(new_n82_), .d(new_n58_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x12), .O(new_n111_));
  oai21  g70(.a(new_n108_), .b(new_n46_), .c(new_n111_), .O(z18));
  aoi21  g71(.a(x20), .b(x12), .c(x03), .O(new_n113_));
  nand2  g72(.a(new_n74_), .b(x20), .O(new_n114_));
  nand2  g73(.a(x08), .b(x07), .O(new_n115_));
  oai21  g74(.a(new_n115_), .b(new_n73_), .c(new_n114_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(x12), .O(new_n117_));
  oai21  g76(.a(new_n113_), .b(new_n46_), .c(new_n117_), .O(z19));
endmodule


