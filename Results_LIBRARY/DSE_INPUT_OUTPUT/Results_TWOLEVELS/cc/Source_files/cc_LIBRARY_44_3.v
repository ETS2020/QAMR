// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_;
  inv1   g00(.a(x07), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand3  g04(.a(new_n45_), .b(x20), .c(x11), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(z00));
  nand3  g06(.a(x10), .b(x08), .c(new_n42_), .O(new_n48_));
  nor3   g07(.a(new_n48_), .b(new_n43_), .c(x14), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n43_), .b(x10), .O(new_n53_));
  oai22  g12(.a(new_n53_), .b(new_n51_), .c(new_n44_), .d(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nor4   g16(.a(new_n48_), .b(new_n43_), .c(x14), .d(new_n57_), .O(z03));
  nand2  g17(.a(new_n45_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n44_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n45_), .O(z09));
  nand3  g26(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n45_), .b(new_n52_), .O(z11));
  aoi21  g28(.a(x13), .b(x12), .c(x07), .O(new_n70_));
  inv1   g29(.a(x10), .O(new_n71_));
  nor2   g30(.a(x15), .b(new_n71_), .O(new_n72_));
  nand2  g31(.a(x10), .b(x08), .O(new_n73_));
  and2   g32(.a(x08), .b(x00), .O(new_n74_));
  aoi22  g33(.a(new_n74_), .b(new_n72_), .c(new_n73_), .d(x13), .O(new_n75_));
  oai22  g34(.a(new_n75_), .b(new_n57_), .c(new_n70_), .d(new_n43_), .O(z12));
  nand4  g35(.a(new_n52_), .b(x10), .c(x08), .d(new_n42_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n52_), .c(new_n43_), .O(new_n78_));
  inv1   g37(.a(new_n73_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(x01), .c(new_n52_), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n45_), .O(z13));
  nand3  g41(.a(new_n73_), .b(x15), .c(new_n42_), .O(new_n83_));
  nand3  g42(.a(new_n72_), .b(x08), .c(x02), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n57_), .O(z14));
  nand2  g44(.a(x15), .b(new_n42_), .O(new_n86_));
  oai21  g45(.a(new_n79_), .b(x15), .c(new_n86_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x16), .c(new_n52_), .O(new_n88_));
  nand3  g47(.a(new_n72_), .b(x08), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n57_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  nand2  g50(.a(new_n73_), .b(x17), .O(new_n92_));
  oai21  g51(.a(new_n73_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n43_), .O(new_n94_));
  nand3  g53(.a(x17), .b(x15), .c(new_n42_), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n57_), .O(z16));
  inv1   g55(.a(x05), .O(new_n97_));
  nand2  g56(.a(new_n73_), .b(x18), .O(new_n98_));
  oai21  g57(.a(new_n73_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand3  g59(.a(x18), .b(x15), .c(new_n42_), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n57_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand2  g62(.a(new_n73_), .b(x19), .O(new_n104_));
  oai21  g63(.a(new_n73_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n43_), .O(new_n106_));
  nand3  g65(.a(x19), .b(x15), .c(new_n42_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n57_), .O(z18));
  nand2  g67(.a(new_n73_), .b(x20), .O(new_n109_));
  oai21  g68(.a(new_n73_), .b(new_n42_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n43_), .O(new_n111_));
  nand3  g70(.a(x20), .b(x15), .c(new_n42_), .O(new_n112_));
  aoi21  g71(.a(new_n112_), .b(new_n111_), .c(new_n57_), .O(z19));
  buf    g72(.a(x15), .O(z06));
endmodule


