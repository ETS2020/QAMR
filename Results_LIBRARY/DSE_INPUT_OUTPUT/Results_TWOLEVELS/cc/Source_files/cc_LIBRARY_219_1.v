// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x04), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x04), .c(new_n44_), .O(z01));
  nand3  g08(.a(new_n44_), .b(x10), .c(x08), .O(new_n50_));
  oai21  g09(.a(new_n45_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x12), .c(new_n42_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(z02));
  nand4  g12(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n54_));
  nor3   g13(.a(new_n54_), .b(new_n44_), .c(x14), .O(z03));
  inv1   g14(.a(new_n45_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n56_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x08), .O(new_n60_));
  nand2  g19(.a(x09), .b(new_n60_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(x04), .c(new_n44_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z08));
  oai21  g26(.a(new_n44_), .b(x04), .c(x09), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n60_), .O(z09));
  inv1   g28(.a(x04), .O(new_n70_));
  aoi22  g29(.a(x15), .b(new_n70_), .c(x09), .d(x08), .O(z10));
  nor2   g30(.a(new_n45_), .b(new_n47_), .O(z11));
  inv1   g31(.a(x12), .O(new_n73_));
  nand2  g32(.a(x15), .b(x04), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n44_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x13), .O(new_n78_));
  inv1   g37(.a(x10), .O(new_n79_));
  nor2   g38(.a(x15), .b(new_n79_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x08), .c(x00), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n78_), .c(new_n73_), .O(z12));
  nand2  g41(.a(x08), .b(x04), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n79_), .c(new_n47_), .O(new_n84_));
  nand3  g43(.a(x10), .b(x08), .c(x01), .O(new_n85_));
  aoi22  g44(.a(new_n85_), .b(x14), .c(new_n84_), .d(x15), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n73_), .c(new_n56_), .O(z13));
  nand3  g46(.a(new_n75_), .b(x15), .c(x04), .O(new_n88_));
  nand3  g47(.a(new_n80_), .b(x08), .c(x02), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n73_), .O(z14));
  nand3  g49(.a(new_n77_), .b(x16), .c(new_n47_), .O(new_n91_));
  nand3  g50(.a(new_n80_), .b(x08), .c(x03), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n91_), .c(new_n73_), .O(z15));
  nand2  g52(.a(new_n50_), .b(new_n64_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x04), .O(new_n95_));
  nand3  g54(.a(new_n75_), .b(x17), .c(new_n44_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n73_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  nand2  g57(.a(new_n75_), .b(x18), .O(new_n99_));
  oai21  g58(.a(new_n75_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n44_), .O(new_n101_));
  nand3  g60(.a(x18), .b(x15), .c(x04), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n73_), .O(z17));
  inv1   g62(.a(x06), .O(new_n104_));
  nand2  g63(.a(new_n75_), .b(x19), .O(new_n105_));
  oai21  g64(.a(new_n75_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n44_), .O(new_n107_));
  nand3  g66(.a(x19), .b(x15), .c(x04), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n73_), .O(z18));
  aoi21  g68(.a(x20), .b(x12), .c(new_n70_), .O(new_n110_));
  nand2  g69(.a(new_n75_), .b(x20), .O(new_n111_));
  nand4  g70(.a(new_n44_), .b(x10), .c(x08), .d(x07), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(x12), .O(new_n114_));
  oai21  g73(.a(new_n110_), .b(new_n44_), .c(new_n114_), .O(z19));
endmodule


