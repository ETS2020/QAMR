// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n60_, new_n61_,
    new_n62_, new_n65_, new_n67_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  nand2  g00(.a(x15), .b(x02), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x02), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  nand2  g09(.a(new_n42_), .b(x14), .O(new_n51_));
  nand3  g10(.a(new_n46_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n50_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  nand4  g14(.a(x12), .b(x10), .c(x08), .d(new_n45_), .O(new_n56_));
  nor3   g15(.a(new_n56_), .b(new_n46_), .c(x14), .O(z03));
  nand2  g16(.a(new_n42_), .b(x18), .O(z04));
  and2   g17(.a(new_n42_), .b(x19), .O(z05));
  inv1   g18(.a(x08), .O(new_n60_));
  nand2  g19(.a(x09), .b(new_n60_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g21(.a(new_n62_), .b(new_n45_), .c(new_n46_), .O(z06));
  and2   g22(.a(new_n42_), .b(x17), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n42_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x15), .b(x02), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(x09), .c(x08), .O(new_n68_));
  inv1   g27(.a(new_n68_), .O(z09));
  aoi22  g28(.a(x15), .b(x02), .c(x09), .d(x08), .O(z10));
  inv1   g29(.a(new_n51_), .O(z11));
  nand2  g30(.a(x13), .b(x12), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n45_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x15), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x13), .O(new_n76_));
  nand4  g35(.a(new_n46_), .b(x10), .c(x08), .d(x00), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n74_), .O(z12));
  inv1   g39(.a(x12), .O(new_n81_));
  inv1   g40(.a(x10), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n60_), .c(new_n47_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(x15), .c(new_n45_), .O(new_n84_));
  nand3  g43(.a(x10), .b(x08), .c(x01), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n46_), .c(x14), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n84_), .c(new_n81_), .O(z13));
  nand3  g46(.a(x12), .b(x10), .c(x08), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n46_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x02), .O(new_n90_));
  nand3  g49(.a(new_n75_), .b(x15), .c(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(z14));
  nand3  g51(.a(x16), .b(new_n47_), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand3  g54(.a(new_n75_), .b(x16), .c(new_n47_), .O(new_n96_));
  nand4  g55(.a(new_n46_), .b(x10), .c(x08), .d(x03), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n95_), .O(z15));
  nand2  g59(.a(new_n75_), .b(x17), .O(new_n101_));
  nand3  g60(.a(x10), .b(x08), .c(x04), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n46_), .O(new_n104_));
  nand3  g63(.a(x17), .b(x15), .c(new_n45_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n81_), .O(z16));
  nand2  g65(.a(new_n75_), .b(x18), .O(new_n107_));
  nand3  g66(.a(x10), .b(x08), .c(x05), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n46_), .O(new_n110_));
  nand3  g69(.a(x18), .b(x15), .c(new_n45_), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n81_), .O(z17));
  nand2  g71(.a(new_n75_), .b(x19), .O(new_n113_));
  nand3  g72(.a(x10), .b(x08), .c(x06), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n46_), .O(new_n116_));
  nand3  g75(.a(x19), .b(x15), .c(new_n45_), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n116_), .c(new_n81_), .O(z18));
  nand2  g77(.a(new_n75_), .b(x20), .O(new_n119_));
  nand3  g78(.a(x10), .b(x08), .c(x07), .O(new_n120_));
  nand2  g79(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand2  g80(.a(new_n121_), .b(new_n46_), .O(new_n122_));
  nand3  g81(.a(x20), .b(x15), .c(new_n45_), .O(new_n123_));
  aoi21  g82(.a(new_n123_), .b(new_n122_), .c(new_n81_), .O(z19));
endmodule


