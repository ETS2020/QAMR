// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n68_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x14), .O(new_n44_));
  nor2   g03(.a(x15), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(new_n45_), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(x15), .b(new_n44_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(z01));
  nand2  g09(.a(x15), .b(x14), .O(new_n51_));
  nor2   g10(.a(x15), .b(x14), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  inv1   g16(.a(x15), .O(new_n58_));
  nor4   g17(.a(new_n48_), .b(new_n58_), .c(x14), .d(new_n57_), .O(z03));
  nand2  g18(.a(new_n47_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n47_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n45_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  oai21  g26(.a(x15), .b(new_n44_), .c(x09), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n67_), .O(z09));
  aoi22  g28(.a(new_n58_), .b(x14), .c(x09), .d(x08), .O(z10));
  nand2  g29(.a(x09), .b(new_n67_), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n67_), .c(x15), .O(new_n72_));
  nor2   g31(.a(new_n72_), .b(new_n44_), .O(z11));
  aoi21  g32(.a(x10), .b(x08), .c(x14), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(x15), .c(x13), .O(new_n75_));
  nand4  g34(.a(new_n52_), .b(x10), .c(x08), .d(x00), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n75_), .c(new_n57_), .O(z12));
  oai21  g36(.a(new_n48_), .b(new_n58_), .c(new_n44_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n47_), .O(z13));
  nand2  g39(.a(new_n48_), .b(x15), .O(new_n81_));
  nand4  g40(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n81_), .c(new_n57_), .O(z14));
  nand3  g42(.a(new_n58_), .b(x10), .c(x08), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x16), .O(new_n85_));
  nand4  g44(.a(new_n58_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n44_), .c(x12), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z15));
  nand4  g48(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n44_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  nand3  g51(.a(new_n84_), .b(x17), .c(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(z16));
  nand2  g53(.a(new_n48_), .b(x18), .O(new_n95_));
  nand4  g54(.a(new_n58_), .b(x10), .c(x08), .d(x05), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n44_), .O(new_n98_));
  nand2  g57(.a(x18), .b(x15), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n98_), .c(new_n57_), .O(z17));
  nand4  g59(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n44_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n58_), .O(new_n103_));
  nand3  g62(.a(new_n84_), .b(x19), .c(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(z18));
  nand2  g64(.a(new_n48_), .b(x20), .O(new_n106_));
  nand4  g65(.a(new_n58_), .b(x10), .c(x08), .d(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n44_), .O(new_n109_));
  nand2  g68(.a(x20), .b(x15), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n57_), .O(z19));
  buf    g70(.a(x15), .O(z06));
endmodule


