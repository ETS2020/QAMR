// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n52_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n65_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x15), .b(new_n42_), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(x15), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  nor4   g06(.a(new_n47_), .b(new_n46_), .c(x14), .d(new_n42_), .O(z01));
  inv1   g07(.a(new_n47_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(x14), .c(x12), .O(new_n50_));
  aoi21  g09(.a(new_n50_), .b(new_n46_), .c(x11), .O(z02));
  nand3  g10(.a(new_n49_), .b(x12), .c(x11), .O(new_n52_));
  nor3   g11(.a(new_n52_), .b(new_n46_), .c(x14), .O(z03));
  nand2  g12(.a(new_n44_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nand2  g14(.a(new_n44_), .b(new_n55_), .O(z05));
  inv1   g15(.a(x08), .O(new_n57_));
  nand2  g16(.a(x09), .b(new_n57_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(x11), .c(new_n46_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  aoi21  g20(.a(x15), .b(new_n42_), .c(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n44_), .b(new_n63_), .O(z08));
  oai21  g23(.a(new_n46_), .b(x11), .c(x09), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n57_), .O(z09));
  aoi22  g25(.a(x15), .b(new_n42_), .c(x09), .d(x08), .O(z10));
  inv1   g26(.a(x14), .O(new_n68_));
  nand2  g27(.a(new_n44_), .b(new_n68_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  nor2   g29(.a(new_n46_), .b(new_n42_), .O(new_n71_));
  aoi21  g30(.a(x10), .b(x08), .c(x15), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(x13), .O(new_n73_));
  inv1   g32(.a(x10), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n74_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x08), .c(x00), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n73_), .c(new_n70_), .O(z12));
  oai21  g36(.a(new_n47_), .b(new_n42_), .c(new_n68_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  aoi22  g38(.a(new_n79_), .b(x14), .c(new_n78_), .d(x15), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n70_), .c(new_n44_), .O(z13));
  nand3  g40(.a(new_n47_), .b(x15), .c(x11), .O(new_n82_));
  nand3  g41(.a(new_n75_), .b(x08), .c(x02), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n70_), .O(z14));
  nand3  g43(.a(x16), .b(new_n68_), .c(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x11), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x15), .O(new_n87_));
  nand3  g46(.a(new_n47_), .b(x16), .c(new_n68_), .O(new_n88_));
  nand4  g47(.a(new_n46_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n87_), .O(z15));
  oai21  g51(.a(new_n61_), .b(new_n70_), .c(x11), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x15), .O(new_n94_));
  nand4  g53(.a(new_n46_), .b(x10), .c(x08), .d(x04), .O(new_n95_));
  oai21  g54(.a(new_n49_), .b(new_n61_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n94_), .O(z16));
  inv1   g57(.a(x18), .O(new_n99_));
  oai21  g58(.a(new_n99_), .b(new_n70_), .c(x11), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x15), .O(new_n101_));
  nand4  g60(.a(new_n46_), .b(x10), .c(x08), .d(x05), .O(new_n102_));
  oai21  g61(.a(new_n49_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n101_), .O(z17));
  oai21  g64(.a(new_n55_), .b(new_n70_), .c(x11), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x15), .O(new_n107_));
  nand4  g66(.a(new_n46_), .b(x10), .c(x08), .d(x06), .O(new_n108_));
  oai21  g67(.a(new_n49_), .b(new_n55_), .c(new_n108_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x12), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n107_), .O(z18));
  oai21  g70(.a(new_n43_), .b(new_n70_), .c(x11), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x15), .O(new_n113_));
  nand4  g72(.a(new_n46_), .b(x10), .c(x08), .d(x07), .O(new_n114_));
  oai21  g73(.a(new_n49_), .b(new_n43_), .c(new_n114_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x12), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n113_), .O(z19));
endmodule


