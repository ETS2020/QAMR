// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n63_, new_n66_, new_n68_, new_n70_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x17), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x17), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x08), .O(new_n49_));
  inv1   g08(.a(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x15), .c(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(new_n47_), .O(z01));
  nand2  g12(.a(new_n44_), .b(x10), .O(new_n54_));
  oai22  g13(.a(new_n54_), .b(new_n49_), .c(new_n45_), .d(new_n48_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  nand2  g16(.a(new_n51_), .b(x12), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x17), .c(x15), .d(new_n48_), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z03));
  nor2   g20(.a(new_n45_), .b(x18), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z05));
  nand2  g23(.a(new_n47_), .b(new_n44_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n45_), .b(new_n68_), .c(new_n49_), .O(z09));
  inv1   g28(.a(new_n45_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(x09), .c(x08), .O(z10));
  nor2   g30(.a(new_n45_), .b(new_n48_), .O(z11));
  inv1   g31(.a(x12), .O(new_n73_));
  inv1   g32(.a(x13), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n73_), .c(x17), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x15), .O(new_n76_));
  nand2  g35(.a(x08), .b(x00), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n54_), .c(new_n51_), .d(new_n74_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n76_), .O(z12));
  oai21  g39(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x17), .c(x15), .O(new_n82_));
  nand2  g41(.a(new_n51_), .b(x01), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n44_), .c(x14), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n82_), .c(new_n73_), .O(z13));
  nand2  g44(.a(x08), .b(x02), .O(new_n86_));
  oai22  g45(.a(new_n86_), .b(new_n54_), .c(new_n51_), .d(new_n44_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n70_), .O(z14));
  nand2  g48(.a(x17), .b(x15), .O(new_n90_));
  oai21  g49(.a(new_n51_), .b(x15), .c(new_n90_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(x16), .c(new_n48_), .O(new_n92_));
  nand4  g51(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n73_), .O(z15));
  inv1   g53(.a(x04), .O(new_n95_));
  oai21  g54(.a(new_n90_), .b(x14), .c(new_n95_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(x10), .c(x08), .O(new_n97_));
  nand2  g56(.a(x15), .b(x14), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(x10), .c(x08), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x17), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n70_), .O(z16));
  inv1   g62(.a(x18), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n73_), .c(x17), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x15), .O(new_n106_));
  nand2  g65(.a(x08), .b(x05), .O(new_n107_));
  oai22  g66(.a(new_n107_), .b(new_n54_), .c(new_n51_), .d(new_n104_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n106_), .O(z17));
  oai21  g69(.a(new_n63_), .b(new_n73_), .c(x17), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x15), .O(new_n112_));
  nand2  g71(.a(x08), .b(x06), .O(new_n113_));
  oai22  g72(.a(new_n113_), .b(new_n54_), .c(new_n51_), .d(new_n63_), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x12), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n112_), .O(z18));
  oai21  g75(.a(new_n43_), .b(new_n73_), .c(x17), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(x15), .O(new_n118_));
  nand2  g77(.a(x08), .b(x07), .O(new_n119_));
  oai22  g78(.a(new_n119_), .b(new_n54_), .c(new_n51_), .d(new_n43_), .O(new_n120_));
  nand2  g79(.a(new_n120_), .b(x12), .O(new_n121_));
  nand2  g80(.a(new_n121_), .b(new_n118_), .O(z19));
  buf    g81(.a(x15), .O(z06));
endmodule


