// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n62_,
    new_n63_, new_n66_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_;
  nand2  g00(.a(x16), .b(x15), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  nor4   g05(.a(new_n46_), .b(x16), .c(new_n45_), .d(x14), .O(z01));
  inv1   g06(.a(x11), .O(new_n48_));
  inv1   g07(.a(x08), .O(new_n49_));
  nand2  g08(.a(new_n42_), .b(x14), .O(new_n50_));
  nand2  g09(.a(new_n45_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n49_), .c(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n48_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(x16), .O(new_n55_));
  nor2   g14(.a(x16), .b(x14), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(x12), .c(x10), .d(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(new_n55_), .c(new_n45_), .O(z03));
  nand2  g17(.a(new_n42_), .b(x18), .O(z04));
  and2   g18(.a(new_n42_), .b(x19), .O(z05));
  nor2   g19(.a(x16), .b(new_n45_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  inv1   g21(.a(new_n42_), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n62_), .O(z07));
  nor2   g23(.a(new_n55_), .b(x15), .O(z08));
  nand3  g24(.a(new_n42_), .b(x09), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z09));
  aoi21  g26(.a(x09), .b(x08), .c(new_n63_), .O(z10));
  inv1   g27(.a(x14), .O(new_n69_));
  nand2  g28(.a(new_n42_), .b(new_n69_), .O(z11));
  nand2  g29(.a(x13), .b(x12), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n55_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x15), .O(new_n73_));
  nand2  g32(.a(new_n46_), .b(x13), .O(new_n74_));
  nand2  g33(.a(x08), .b(x00), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n51_), .c(new_n74_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n73_), .O(z12));
  inv1   g37(.a(x12), .O(new_n79_));
  inv1   g38(.a(x10), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n49_), .c(new_n69_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n55_), .c(x15), .O(new_n82_));
  nand3  g41(.a(x10), .b(x08), .c(x01), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n45_), .c(x14), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n82_), .c(new_n79_), .O(z13));
  nand3  g44(.a(new_n46_), .b(new_n55_), .c(x15), .O(new_n86_));
  nand4  g45(.a(new_n45_), .b(x10), .c(x08), .d(x02), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(new_n79_), .O(z14));
  nand3  g47(.a(new_n46_), .b(x16), .c(new_n69_), .O(new_n89_));
  nand3  g48(.a(x10), .b(x08), .c(x03), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(new_n45_), .c(x12), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z15));
  oai21  g52(.a(new_n62_), .b(new_n79_), .c(new_n55_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x15), .O(new_n95_));
  nand2  g54(.a(new_n46_), .b(x17), .O(new_n96_));
  nand2  g55(.a(x08), .b(x04), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n51_), .c(new_n96_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x12), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n95_), .O(z16));
  nand2  g59(.a(x18), .b(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n55_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x15), .O(new_n103_));
  nand2  g62(.a(new_n46_), .b(x18), .O(new_n104_));
  nand2  g63(.a(x08), .b(x05), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n51_), .c(new_n104_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x12), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n103_), .O(z17));
  nand2  g67(.a(new_n46_), .b(x19), .O(new_n109_));
  nand3  g68(.a(x10), .b(x08), .c(x06), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n45_), .O(new_n112_));
  nand3  g71(.a(x19), .b(new_n55_), .c(x15), .O(new_n113_));
  aoi21  g72(.a(new_n113_), .b(new_n112_), .c(new_n79_), .O(z18));
  nand2  g73(.a(new_n46_), .b(x20), .O(new_n115_));
  nand3  g74(.a(x10), .b(x08), .c(x07), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(new_n45_), .O(new_n118_));
  nand3  g77(.a(x20), .b(new_n55_), .c(x15), .O(new_n119_));
  aoi21  g78(.a(new_n119_), .b(new_n118_), .c(new_n79_), .O(z19));
endmodule


