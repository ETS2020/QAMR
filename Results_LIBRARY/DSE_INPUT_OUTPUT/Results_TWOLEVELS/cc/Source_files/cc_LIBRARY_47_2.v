// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n58_, new_n61_, new_n64_,
    new_n65_, new_n69_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n80_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x12), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x15), .b(new_n48_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n47_), .c(new_n45_), .O(z01));
  inv1   g09(.a(x12), .O(new_n51_));
  nand2  g10(.a(x14), .b(new_n42_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(x15), .c(new_n51_), .O(z02));
  nand3  g12(.a(new_n48_), .b(x10), .c(x08), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x15), .c(new_n51_), .O(z03));
  inv1   g14(.a(new_n45_), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  aoi21  g17(.a(new_n44_), .b(x12), .c(new_n58_), .O(z05));
  nand2  g18(.a(new_n44_), .b(new_n51_), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  aoi21  g20(.a(new_n44_), .b(x12), .c(new_n61_), .O(z07));
  and2   g21(.a(new_n45_), .b(x16), .O(z08));
  inv1   g22(.a(x08), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(x09), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n64_), .O(z09));
  nand3  g25(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g26(.a(new_n45_), .b(new_n48_), .O(z11));
  inv1   g27(.a(x13), .O(new_n69_));
  aoi21  g28(.a(x15), .b(new_n69_), .c(new_n51_), .O(z12));
  nand3  g29(.a(new_n54_), .b(x15), .c(new_n48_), .O(new_n71_));
  and2   g30(.a(new_n71_), .b(x12), .O(z13));
  nand3  g31(.a(new_n47_), .b(x15), .c(x12), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z14));
  nand2  g33(.a(x16), .b(new_n48_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(x15), .c(new_n51_), .O(z15));
  nor2   g35(.a(new_n44_), .b(new_n51_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x17), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z16));
  nand2  g38(.a(new_n77_), .b(x18), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z17));
  aoi21  g40(.a(new_n58_), .b(x15), .c(new_n51_), .O(z18));
  aoi21  g41(.a(new_n43_), .b(x15), .c(new_n51_), .O(z19));
endmodule


