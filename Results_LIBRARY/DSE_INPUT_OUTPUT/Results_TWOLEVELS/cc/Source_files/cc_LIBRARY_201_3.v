// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n60_,
    new_n64_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_;
  nand2  g00(.a(x15), .b(x13), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  nor4   g05(.a(new_n46_), .b(new_n45_), .c(x14), .d(x13), .O(z01));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(new_n45_), .b(x10), .c(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x12), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(x11), .c(new_n42_), .O(z02));
  inv1   g11(.a(x13), .O(new_n53_));
  inv1   g12(.a(new_n46_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x12), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(x15), .c(new_n48_), .d(new_n53_), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z03));
  nand2  g17(.a(new_n42_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n42_), .b(new_n60_), .O(z05));
  and2   g20(.a(new_n42_), .b(x17), .O(z07));
  and2   g21(.a(new_n42_), .b(x16), .O(z08));
  nand3  g22(.a(new_n42_), .b(x09), .c(x08), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z09));
  nand3  g24(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  aoi21  g25(.a(x15), .b(x13), .c(new_n48_), .O(z11));
  inv1   g26(.a(x00), .O(new_n68_));
  nand2  g27(.a(new_n46_), .b(x13), .O(new_n69_));
  oai21  g28(.a(new_n46_), .b(new_n68_), .c(new_n69_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(new_n45_), .c(x12), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z12));
  inv1   g31(.a(x12), .O(new_n73_));
  oai21  g32(.a(new_n46_), .b(x13), .c(new_n48_), .O(new_n74_));
  nand3  g33(.a(x10), .b(x08), .c(x01), .O(new_n75_));
  aoi22  g34(.a(new_n75_), .b(x14), .c(new_n74_), .d(x15), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n73_), .c(new_n42_), .O(z13));
  nand4  g36(.a(new_n45_), .b(x10), .c(x08), .d(x02), .O(new_n78_));
  oai21  g37(.a(new_n54_), .b(new_n45_), .c(new_n78_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n42_), .O(z14));
  nand3  g40(.a(x16), .b(new_n48_), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n53_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x15), .O(new_n84_));
  nand3  g43(.a(new_n46_), .b(x16), .c(new_n48_), .O(new_n85_));
  nand4  g44(.a(new_n45_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n84_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand2  g49(.a(new_n46_), .b(x17), .O(new_n91_));
  oai21  g50(.a(new_n46_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n45_), .O(new_n93_));
  nand3  g52(.a(x17), .b(x15), .c(new_n53_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n73_), .O(z16));
  inv1   g54(.a(x18), .O(new_n96_));
  oai21  g55(.a(new_n96_), .b(new_n73_), .c(new_n53_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x15), .O(new_n98_));
  nand4  g57(.a(new_n45_), .b(x10), .c(x08), .d(x05), .O(new_n99_));
  oai21  g58(.a(new_n54_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n98_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand2  g62(.a(new_n46_), .b(x19), .O(new_n104_));
  oai21  g63(.a(new_n46_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n45_), .O(new_n106_));
  nand3  g65(.a(x19), .b(x15), .c(new_n53_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n73_), .O(z18));
  inv1   g67(.a(x07), .O(new_n109_));
  nand2  g68(.a(new_n46_), .b(x20), .O(new_n110_));
  oai21  g69(.a(new_n46_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n45_), .O(new_n112_));
  nand3  g71(.a(x20), .b(x15), .c(new_n53_), .O(new_n113_));
  aoi21  g72(.a(new_n113_), .b(new_n112_), .c(new_n73_), .O(z19));
  buf    g73(.a(x15), .O(z06));
endmodule


