// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x13), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x13), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n44_), .c(x14), .d(new_n47_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n44_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  inv1   g15(.a(new_n48_), .O(new_n57_));
  nand3  g16(.a(new_n57_), .b(new_n52_), .c(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x13), .c(new_n44_), .O(z03));
  nor2   g18(.a(new_n45_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n45_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n50_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n50_), .O(z09));
  inv1   g27(.a(z09), .O(z10));
  nand2  g28(.a(new_n50_), .b(new_n52_), .O(z11));
  inv1   g29(.a(x00), .O(new_n71_));
  oai21  g30(.a(new_n44_), .b(x14), .c(new_n71_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x10), .c(x08), .O(new_n73_));
  aoi22  g32(.a(new_n48_), .b(x13), .c(x15), .d(x14), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n50_), .O(z12));
  aoi21  g36(.a(x10), .b(x08), .c(x14), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(new_n44_), .O(new_n79_));
  aoi21  g38(.a(new_n57_), .b(x01), .c(new_n52_), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n79_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n50_), .O(z13));
  inv1   g41(.a(x12), .O(new_n83_));
  inv1   g42(.a(x10), .O(new_n84_));
  nor2   g43(.a(x15), .b(new_n84_), .O(new_n85_));
  and2   g44(.a(x08), .b(x02), .O(new_n86_));
  aoi22  g45(.a(new_n86_), .b(new_n85_), .c(new_n48_), .d(x15), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n83_), .c(new_n50_), .O(z14));
  nand3  g47(.a(x16), .b(new_n52_), .c(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x13), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x15), .O(new_n91_));
  nand3  g50(.a(new_n48_), .b(x16), .c(new_n52_), .O(new_n92_));
  nand2  g51(.a(x08), .b(x03), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n53_), .c(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n91_), .O(z15));
  inv1   g55(.a(x04), .O(new_n97_));
  nand2  g56(.a(new_n48_), .b(x17), .O(new_n98_));
  oai21  g57(.a(new_n48_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n44_), .O(new_n100_));
  nand3  g59(.a(x17), .b(x15), .c(x13), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n83_), .O(z16));
  inv1   g61(.a(x05), .O(new_n103_));
  nand2  g62(.a(new_n48_), .b(x18), .O(new_n104_));
  oai21  g63(.a(new_n48_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n44_), .O(new_n106_));
  nand3  g65(.a(x18), .b(x15), .c(x13), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n83_), .O(z17));
  aoi21  g67(.a(x19), .b(x12), .c(new_n47_), .O(new_n109_));
  and2   g68(.a(x08), .b(x06), .O(new_n110_));
  aoi22  g69(.a(new_n110_), .b(new_n85_), .c(new_n48_), .d(x19), .O(new_n111_));
  oai22  g70(.a(new_n111_), .b(new_n83_), .c(new_n109_), .d(new_n44_), .O(z18));
  inv1   g71(.a(x07), .O(new_n113_));
  nand2  g72(.a(new_n48_), .b(x20), .O(new_n114_));
  oai21  g73(.a(new_n48_), .b(new_n113_), .c(new_n114_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n44_), .O(new_n116_));
  nand3  g75(.a(x20), .b(x15), .c(x13), .O(new_n117_));
  aoi21  g76(.a(new_n117_), .b(new_n116_), .c(new_n83_), .O(z19));
  buf    g77(.a(x15), .O(z06));
endmodule


