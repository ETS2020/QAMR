// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n56_, new_n58_, new_n60_, new_n62_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n79_, new_n81_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(x10), .O(new_n45_));
  oai21  g04(.a(new_n43_), .b(new_n42_), .c(new_n45_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand4  g06(.a(x15), .b(new_n47_), .c(x10), .d(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  nand4  g08(.a(new_n45_), .b(x14), .c(x12), .d(new_n42_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(z02));
  inv1   g10(.a(x10), .O(new_n52_));
  nand4  g11(.a(x15), .b(new_n47_), .c(x12), .d(x08), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(x15), .c(new_n52_), .O(z03));
  aoi21  g13(.a(new_n44_), .b(x10), .c(x18), .O(z04));
  inv1   g14(.a(x19), .O(new_n56_));
  nand2  g15(.a(new_n45_), .b(new_n56_), .O(z05));
  inv1   g16(.a(x17), .O(new_n58_));
  nand2  g17(.a(new_n45_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nand2  g19(.a(new_n45_), .b(new_n60_), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n45_), .O(z09));
  nand3  g22(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  oai21  g23(.a(x15), .b(new_n52_), .c(x14), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z11));
  inv1   g25(.a(x12), .O(new_n67_));
  oai21  g26(.a(x15), .b(new_n52_), .c(x13), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n67_), .O(z12));
  nand4  g28(.a(x15), .b(new_n47_), .c(x10), .d(x08), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n65_), .c(new_n67_), .O(z13));
  nand2  g30(.a(x10), .b(x08), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(x15), .c(x12), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n45_), .O(z14));
  nand3  g33(.a(x16), .b(new_n47_), .c(x12), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n45_), .O(z15));
  oai21  g35(.a(x15), .b(new_n52_), .c(x17), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n67_), .O(z16));
  oai21  g37(.a(x15), .b(new_n52_), .c(x18), .O(new_n79_));
  nor2   g38(.a(new_n79_), .b(new_n67_), .O(z17));
  oai21  g39(.a(x15), .b(new_n52_), .c(x19), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n67_), .O(z18));
  oai21  g41(.a(new_n43_), .b(new_n67_), .c(new_n45_), .O(z19));
  buf    g42(.a(x15), .O(z06));
endmodule


