// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n60_,
    new_n61_, new_n63_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x18), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x18), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n47_), .c(new_n44_), .d(x14), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x14), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x10), .O(new_n52_));
  oai22  g11(.a(new_n52_), .b(new_n50_), .c(new_n45_), .d(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(new_n48_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(x18), .c(new_n51_), .d(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x18), .c(new_n44_), .O(z03));
  nor2   g17(.a(x18), .b(x15), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  inv1   g19(.a(new_n45_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n60_), .O(z05));
  nor2   g21(.a(new_n47_), .b(new_n50_), .O(new_n63_));
  aoi21  g22(.a(new_n63_), .b(new_n50_), .c(new_n44_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nand2  g24(.a(new_n61_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n61_), .b(new_n67_), .O(z08));
  nand2  g27(.a(x09), .b(x08), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n61_), .O(z09));
  aoi22  g29(.a(new_n47_), .b(x15), .c(x09), .d(x08), .O(z10));
  nor2   g30(.a(new_n45_), .b(new_n51_), .O(z11));
  inv1   g31(.a(x12), .O(new_n73_));
  nand2  g32(.a(x18), .b(x15), .O(new_n74_));
  oai21  g33(.a(new_n56_), .b(x15), .c(new_n74_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x13), .O(new_n76_));
  inv1   g35(.a(x10), .O(new_n77_));
  nor2   g36(.a(x15), .b(new_n77_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x08), .c(x00), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n73_), .O(z12));
  oai21  g39(.a(new_n77_), .b(new_n50_), .c(new_n51_), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x18), .c(x15), .O(new_n82_));
  nand2  g41(.a(new_n56_), .b(x01), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(new_n44_), .c(x14), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n82_), .c(new_n73_), .O(z13));
  and2   g44(.a(x08), .b(x02), .O(new_n86_));
  aoi22  g45(.a(new_n86_), .b(new_n78_), .c(new_n48_), .d(x15), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n73_), .c(new_n61_), .O(z14));
  nand3  g47(.a(new_n75_), .b(x16), .c(new_n51_), .O(new_n89_));
  nand3  g48(.a(new_n78_), .b(x08), .c(x03), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n73_), .O(z15));
  inv1   g50(.a(x04), .O(new_n92_));
  nand2  g51(.a(new_n48_), .b(x17), .O(new_n93_));
  oai21  g52(.a(new_n48_), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n44_), .O(new_n95_));
  nand3  g54(.a(x18), .b(x17), .c(x15), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n73_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  oai21  g57(.a(new_n74_), .b(x14), .c(new_n98_), .O(new_n99_));
  nand3  g58(.a(new_n99_), .b(x10), .c(x08), .O(new_n100_));
  nand2  g59(.a(x15), .b(x14), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(x10), .c(x08), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x18), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n61_), .O(z17));
  aoi21  g65(.a(x19), .b(x12), .c(new_n47_), .O(new_n107_));
  and2   g66(.a(x08), .b(x06), .O(new_n108_));
  aoi22  g67(.a(new_n108_), .b(new_n78_), .c(new_n48_), .d(x19), .O(new_n109_));
  oai22  g68(.a(new_n109_), .b(new_n73_), .c(new_n107_), .d(new_n44_), .O(z18));
  inv1   g69(.a(x07), .O(new_n111_));
  nand2  g70(.a(new_n48_), .b(x20), .O(new_n112_));
  oai21  g71(.a(new_n48_), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n44_), .O(new_n114_));
  nand3  g73(.a(x20), .b(x18), .c(x15), .O(new_n115_));
  aoi21  g74(.a(new_n115_), .b(new_n114_), .c(new_n73_), .O(z19));
endmodule


