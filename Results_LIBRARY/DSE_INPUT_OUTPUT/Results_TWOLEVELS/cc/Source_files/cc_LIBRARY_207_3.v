// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n65_, new_n67_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_;
  nand2  g00(.a(x12), .b(x02), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x12), .b(x02), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(x15), .c(new_n46_), .d(x10), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g08(.a(x02), .O(new_n50_));
  inv1   g09(.a(x11), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(x12), .c(new_n51_), .d(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(x12), .O(new_n57_));
  nand2  g16(.a(x08), .b(new_n50_), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x15), .c(new_n46_), .d(x10), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(new_n50_), .c(new_n57_), .O(z03));
  nand2  g20(.a(new_n42_), .b(x18), .O(z04));
  and2   g21(.a(new_n42_), .b(x19), .O(z05));
  nand2  g22(.a(new_n42_), .b(new_n52_), .O(z06));
  inv1   g23(.a(x17), .O(new_n65_));
  nand2  g24(.a(new_n42_), .b(new_n65_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n42_), .b(new_n67_), .O(z08));
  nand2  g27(.a(x09), .b(x08), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n42_), .O(z09));
  inv1   g29(.a(z09), .O(z10));
  aoi21  g30(.a(x12), .b(x02), .c(new_n46_), .O(z11));
  nand2  g31(.a(new_n53_), .b(x13), .O(new_n73_));
  inv1   g32(.a(x10), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n74_), .O(new_n75_));
  and2   g34(.a(x08), .b(x00), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n75_), .c(x02), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n73_), .c(new_n57_), .O(z12));
  oai21  g37(.a(new_n58_), .b(new_n74_), .c(new_n46_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x15), .O(new_n80_));
  nand3  g39(.a(x10), .b(x08), .c(x01), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(x14), .c(x02), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n57_), .O(z13));
  oai21  g42(.a(new_n74_), .b(new_n45_), .c(x15), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n50_), .c(new_n57_), .O(z14));
  nand3  g44(.a(new_n53_), .b(x16), .c(new_n46_), .O(new_n86_));
  nand4  g45(.a(new_n52_), .b(x10), .c(x08), .d(x03), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(x12), .c(new_n50_), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z15));
  nand2  g49(.a(new_n53_), .b(x17), .O(new_n91_));
  nand4  g50(.a(new_n52_), .b(x10), .c(x08), .d(x04), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(x12), .c(new_n50_), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z16));
  nand2  g54(.a(new_n53_), .b(x18), .O(new_n96_));
  and2   g55(.a(x08), .b(x05), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n75_), .c(x02), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n96_), .c(new_n57_), .O(z17));
  nand2  g58(.a(new_n53_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n52_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand3  g61(.a(new_n102_), .b(x12), .c(new_n50_), .O(new_n103_));
  inv1   g62(.a(new_n103_), .O(z18));
  nand2  g63(.a(new_n53_), .b(x20), .O(new_n105_));
  nand4  g64(.a(new_n52_), .b(x10), .c(x08), .d(x07), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(x12), .c(new_n50_), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z19));
endmodule


