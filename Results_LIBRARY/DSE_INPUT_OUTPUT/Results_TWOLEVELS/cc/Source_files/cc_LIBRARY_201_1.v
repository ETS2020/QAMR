// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n67_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x13), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(new_n47_), .c(x13), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n44_), .O(z01));
  inv1   g10(.a(new_n45_), .O(new_n52_));
  nand3  g11(.a(new_n44_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n52_), .O(z02));
  nand3  g15(.a(new_n49_), .b(x13), .c(x12), .O(new_n57_));
  nor3   g16(.a(new_n57_), .b(new_n44_), .c(x14), .O(z03));
  nand2  g17(.a(new_n52_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n52_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z08));
  inv1   g24(.a(x08), .O(new_n66_));
  inv1   g25(.a(x09), .O(new_n67_));
  nor3   g26(.a(new_n45_), .b(new_n67_), .c(new_n66_), .O(z09));
  nand3  g27(.a(new_n52_), .b(x09), .c(x08), .O(z10));
  nor2   g28(.a(new_n45_), .b(new_n47_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  nand3  g30(.a(x15), .b(new_n47_), .c(x13), .O(new_n72_));
  nand2  g31(.a(new_n44_), .b(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(x15), .b(x14), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x13), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n75_), .c(new_n71_), .O(z12));
  aoi21  g38(.a(x10), .b(x08), .c(x14), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n44_), .O(new_n81_));
  nand3  g40(.a(x10), .b(x08), .c(x01), .O(new_n82_));
  and2   g41(.a(new_n82_), .b(x14), .O(new_n83_));
  aoi21  g42(.a(new_n81_), .b(x13), .c(new_n83_), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n71_), .c(new_n52_), .O(z13));
  nand4  g44(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n86_));
  oai21  g45(.a(new_n49_), .b(new_n44_), .c(new_n86_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n52_), .O(z14));
  nand3  g48(.a(x16), .b(new_n47_), .c(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x13), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x15), .O(new_n92_));
  nand3  g51(.a(new_n48_), .b(x16), .c(new_n47_), .O(new_n93_));
  nand4  g52(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n92_), .O(z15));
  inv1   g56(.a(x04), .O(new_n98_));
  nand2  g57(.a(new_n48_), .b(x17), .O(new_n99_));
  oai21  g58(.a(new_n48_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n44_), .O(new_n101_));
  nand3  g60(.a(x17), .b(x15), .c(x13), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n71_), .O(z16));
  inv1   g62(.a(x18), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n71_), .c(x13), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x15), .O(new_n106_));
  nand4  g65(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n107_));
  oai21  g66(.a(new_n49_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n106_), .O(z17));
  inv1   g69(.a(x06), .O(new_n111_));
  nand2  g70(.a(new_n48_), .b(x19), .O(new_n112_));
  oai21  g71(.a(new_n48_), .b(new_n111_), .c(new_n112_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n44_), .O(new_n114_));
  nand3  g73(.a(x19), .b(x15), .c(x13), .O(new_n115_));
  aoi21  g74(.a(new_n115_), .b(new_n114_), .c(new_n71_), .O(z18));
  inv1   g75(.a(x07), .O(new_n117_));
  nand2  g76(.a(new_n48_), .b(x20), .O(new_n118_));
  oai21  g77(.a(new_n48_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  nand3  g79(.a(x20), .b(x15), .c(x13), .O(new_n121_));
  aoi21  g80(.a(new_n121_), .b(new_n120_), .c(new_n71_), .O(z19));
  buf    g81(.a(x15), .O(z06));
endmodule


