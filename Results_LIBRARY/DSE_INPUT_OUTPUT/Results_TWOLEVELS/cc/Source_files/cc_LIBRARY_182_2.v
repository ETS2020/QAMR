// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n60_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n75_, new_n76_,
    new_n78_, new_n81_, new_n84_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x10), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand4  g05(.a(x15), .b(new_n46_), .c(x10), .d(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(z01));
  nand2  g07(.a(x14), .b(x12), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(x11), .c(new_n44_), .O(z02));
  nand3  g09(.a(new_n46_), .b(x12), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x15), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x10), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z03));
  inv1   g13(.a(x10), .O(new_n55_));
  nor2   g14(.a(x15), .b(new_n55_), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x09), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(x08), .O(new_n61_));
  aoi21  g20(.a(new_n61_), .b(x08), .c(new_n43_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n44_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x08), .O(new_n67_));
  nor3   g26(.a(new_n56_), .b(new_n60_), .c(new_n67_), .O(z09));
  aoi21  g27(.a(x09), .b(x08), .c(new_n56_), .O(z10));
  nor2   g28(.a(new_n56_), .b(new_n46_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  nand2  g30(.a(new_n44_), .b(x13), .O(new_n72_));
  nor2   g31(.a(new_n72_), .b(new_n71_), .O(z12));
  nand2  g32(.a(new_n53_), .b(new_n49_), .O(z13));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x15), .c(x12), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(z14));
  nand3  g36(.a(x16), .b(new_n46_), .c(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n44_), .O(z15));
  oai21  g38(.a(new_n63_), .b(new_n71_), .c(new_n44_), .O(z16));
  nand2  g39(.a(new_n44_), .b(x18), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n71_), .O(z17));
  oai21  g41(.a(new_n58_), .b(new_n71_), .c(new_n44_), .O(z18));
  nand2  g42(.a(x20), .b(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n44_), .O(z19));
endmodule


