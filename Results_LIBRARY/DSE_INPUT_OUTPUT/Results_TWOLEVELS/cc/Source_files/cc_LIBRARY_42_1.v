// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n67_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x13), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x13), .c(new_n44_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n50_), .O(z02));
  nand4  g14(.a(x13), .b(x12), .c(x10), .d(x08), .O(new_n56_));
  nor3   g15(.a(new_n56_), .b(new_n44_), .c(x14), .O(z03));
  nor2   g16(.a(new_n45_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n45_), .b(new_n59_), .O(z05));
  nand2  g19(.a(x15), .b(x13), .O(new_n61_));
  inv1   g20(.a(new_n61_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nand2  g24(.a(new_n50_), .b(new_n65_), .O(z08));
  nand2  g25(.a(x09), .b(x08), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n50_), .O(z09));
  inv1   g27(.a(x13), .O(new_n69_));
  aoi22  g28(.a(x15), .b(new_n69_), .c(x09), .d(x08), .O(z10));
  nand2  g29(.a(new_n50_), .b(new_n47_), .O(z11));
  inv1   g30(.a(x12), .O(new_n72_));
  nand3  g31(.a(x15), .b(new_n47_), .c(x13), .O(new_n73_));
  nand2  g32(.a(new_n44_), .b(x00), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x10), .c(x08), .O(new_n76_));
  nand2  g35(.a(x15), .b(x14), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(x10), .c(x08), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x13), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n72_), .O(z12));
  aoi21  g39(.a(x10), .b(x08), .c(x14), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(new_n44_), .O(new_n82_));
  nand3  g41(.a(x10), .b(x08), .c(x01), .O(new_n83_));
  and2   g42(.a(new_n83_), .b(x14), .O(new_n84_));
  aoi21  g43(.a(new_n82_), .b(x13), .c(new_n84_), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n72_), .c(new_n50_), .O(z13));
  nand2  g45(.a(x10), .b(x08), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand2  g47(.a(x08), .b(x02), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n52_), .c(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n50_), .O(z14));
  nand2  g51(.a(new_n87_), .b(new_n44_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(x16), .c(new_n47_), .O(new_n95_));
  nand4  g54(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n72_), .O(z15));
  aoi21  g56(.a(x17), .b(x12), .c(new_n69_), .O(new_n98_));
  nand2  g57(.a(new_n87_), .b(x17), .O(new_n99_));
  nand2  g58(.a(x08), .b(x04), .O(new_n100_));
  oai21  g59(.a(new_n100_), .b(new_n52_), .c(new_n99_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x12), .O(new_n102_));
  oai21  g61(.a(new_n98_), .b(new_n44_), .c(new_n102_), .O(z16));
  inv1   g62(.a(x05), .O(new_n104_));
  nand2  g63(.a(new_n87_), .b(x18), .O(new_n105_));
  oai21  g64(.a(new_n87_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n44_), .O(new_n107_));
  nand3  g66(.a(x18), .b(x15), .c(x13), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n72_), .O(z17));
  inv1   g68(.a(x06), .O(new_n110_));
  nand2  g69(.a(new_n87_), .b(x19), .O(new_n111_));
  oai21  g70(.a(new_n87_), .b(new_n110_), .c(new_n111_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n44_), .O(new_n113_));
  nand3  g72(.a(x19), .b(x15), .c(x13), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n72_), .O(z18));
  inv1   g74(.a(x07), .O(new_n116_));
  nand2  g75(.a(new_n87_), .b(x20), .O(new_n117_));
  oai21  g76(.a(new_n87_), .b(new_n116_), .c(new_n117_), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(new_n44_), .O(new_n119_));
  nand3  g78(.a(x20), .b(x15), .c(x13), .O(new_n120_));
  aoi21  g79(.a(new_n120_), .b(new_n119_), .c(new_n72_), .O(z19));
endmodule


