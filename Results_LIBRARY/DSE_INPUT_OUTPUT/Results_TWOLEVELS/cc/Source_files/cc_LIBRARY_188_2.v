// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n61_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x19), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x19), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n47_), .c(new_n44_), .d(x14), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n44_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  inv1   g15(.a(new_n48_), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(x19), .c(new_n52_), .d(x12), .O(new_n58_));
  aoi21  g17(.a(new_n58_), .b(x19), .c(new_n44_), .O(z03));
  nand2  g18(.a(new_n50_), .b(x18), .O(z04));
  nand2  g19(.a(x09), .b(new_n51_), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(new_n51_), .c(x19), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n50_), .O(z05));
  nand2  g22(.a(x19), .b(x15), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nand2  g27(.a(new_n50_), .b(new_n68_), .O(z08));
  inv1   g28(.a(x09), .O(new_n70_));
  nor3   g29(.a(new_n45_), .b(new_n70_), .c(new_n51_), .O(z09));
  aoi21  g30(.a(x09), .b(x08), .c(new_n45_), .O(z10));
  nand2  g31(.a(new_n50_), .b(new_n52_), .O(z11));
  nand2  g32(.a(x13), .b(x12), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(x19), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x15), .O(new_n76_));
  nand2  g35(.a(new_n48_), .b(x13), .O(new_n77_));
  nand2  g36(.a(x08), .b(x00), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n53_), .c(new_n77_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n76_), .O(z12));
  inv1   g40(.a(x12), .O(new_n82_));
  inv1   g41(.a(x10), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n51_), .c(new_n52_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x19), .c(x15), .O(new_n85_));
  nand2  g44(.a(new_n57_), .b(x01), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n44_), .c(x14), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n85_), .c(new_n82_), .O(z13));
  nand2  g47(.a(new_n48_), .b(x15), .O(new_n89_));
  nand2  g48(.a(x08), .b(x02), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n53_), .c(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n50_), .O(z14));
  nand2  g52(.a(new_n48_), .b(new_n44_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n64_), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(x16), .c(new_n52_), .O(new_n96_));
  nand4  g55(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n82_), .O(z15));
  nand2  g57(.a(new_n48_), .b(x17), .O(new_n99_));
  nand3  g58(.a(x10), .b(x08), .c(x04), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n44_), .O(new_n102_));
  nand3  g61(.a(x19), .b(x17), .c(x15), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n82_), .O(z16));
  nand2  g63(.a(new_n48_), .b(x18), .O(new_n105_));
  nand3  g64(.a(x10), .b(x08), .c(x05), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n44_), .O(new_n108_));
  nand3  g67(.a(x19), .b(x18), .c(x15), .O(new_n109_));
  aoi21  g68(.a(new_n109_), .b(new_n108_), .c(new_n82_), .O(z17));
  inv1   g69(.a(x06), .O(new_n111_));
  oai22  g70(.a(new_n64_), .b(x14), .c(x15), .d(new_n111_), .O(new_n112_));
  nand3  g71(.a(new_n112_), .b(x10), .c(x08), .O(new_n113_));
  nand2  g72(.a(x15), .b(x14), .O(new_n114_));
  nand3  g73(.a(new_n114_), .b(x10), .c(x08), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x19), .O(new_n116_));
  aoi21  g75(.a(new_n116_), .b(new_n113_), .c(new_n82_), .O(z18));
  oai21  g76(.a(new_n43_), .b(new_n82_), .c(x19), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(x15), .O(new_n119_));
  nand2  g78(.a(new_n48_), .b(x20), .O(new_n120_));
  nand2  g79(.a(x08), .b(x07), .O(new_n121_));
  oai21  g80(.a(new_n121_), .b(new_n53_), .c(new_n120_), .O(new_n122_));
  nand2  g81(.a(new_n122_), .b(x12), .O(new_n123_));
  nand2  g82(.a(new_n123_), .b(new_n119_), .O(z19));
endmodule


