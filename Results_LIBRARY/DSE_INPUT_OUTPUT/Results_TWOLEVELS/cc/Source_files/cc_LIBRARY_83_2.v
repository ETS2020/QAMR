// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n60_,
    new_n62_, new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x20), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n43_), .c(new_n44_), .d(x14), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x14), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n46_), .O(z02));
  inv1   g14(.a(new_n48_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(x20), .c(new_n51_), .d(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x20), .c(new_n44_), .O(z03));
  nor2   g17(.a(new_n45_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n45_), .b(new_n60_), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z08));
  inv1   g24(.a(x09), .O(new_n66_));
  nor3   g25(.a(new_n45_), .b(new_n66_), .c(new_n50_), .O(z09));
  nand3  g26(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nand2  g27(.a(new_n46_), .b(new_n51_), .O(z11));
  aoi21  g28(.a(x13), .b(x12), .c(new_n43_), .O(new_n70_));
  nand2  g29(.a(new_n48_), .b(x13), .O(new_n71_));
  nand2  g30(.a(x08), .b(x00), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n52_), .c(new_n71_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x12), .O(new_n74_));
  oai21  g33(.a(new_n70_), .b(new_n44_), .c(new_n74_), .O(z12));
  inv1   g34(.a(x12), .O(new_n76_));
  aoi21  g35(.a(x10), .b(x08), .c(x14), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n43_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  and2   g38(.a(new_n79_), .b(x14), .O(new_n80_));
  aoi21  g39(.a(new_n78_), .b(x15), .c(new_n80_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n76_), .c(new_n46_), .O(z13));
  nand3  g41(.a(new_n48_), .b(x20), .c(x15), .O(new_n83_));
  nand4  g42(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n76_), .O(z14));
  nand3  g44(.a(x16), .b(new_n51_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x20), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x15), .O(new_n88_));
  nand3  g47(.a(new_n48_), .b(x16), .c(new_n51_), .O(new_n89_));
  nand2  g48(.a(x08), .b(x03), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n52_), .c(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n88_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  nand2  g53(.a(new_n48_), .b(x17), .O(new_n95_));
  oai21  g54(.a(new_n48_), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n44_), .O(new_n97_));
  nand3  g56(.a(x20), .b(x17), .c(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n76_), .O(z16));
  inv1   g58(.a(x05), .O(new_n100_));
  nand2  g59(.a(new_n48_), .b(x18), .O(new_n101_));
  oai21  g60(.a(new_n48_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n44_), .O(new_n103_));
  nand3  g62(.a(x20), .b(x18), .c(x15), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n76_), .O(z17));
  inv1   g64(.a(x06), .O(new_n106_));
  nand2  g65(.a(new_n48_), .b(x19), .O(new_n107_));
  oai21  g66(.a(new_n48_), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n44_), .O(new_n109_));
  nand3  g68(.a(x20), .b(x19), .c(x15), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n76_), .O(z18));
  nand2  g70(.a(new_n44_), .b(x07), .O(new_n112_));
  nand3  g71(.a(x20), .b(x15), .c(new_n51_), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g73(.a(new_n114_), .b(x10), .c(x08), .O(new_n115_));
  nand2  g74(.a(x15), .b(x14), .O(new_n116_));
  nand3  g75(.a(new_n116_), .b(x10), .c(x08), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(x20), .O(new_n118_));
  aoi21  g77(.a(new_n118_), .b(new_n115_), .c(new_n76_), .O(z19));
  buf    g78(.a(x15), .O(z06));
endmodule


