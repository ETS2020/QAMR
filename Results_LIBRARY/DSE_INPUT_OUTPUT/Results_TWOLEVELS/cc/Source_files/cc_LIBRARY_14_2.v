// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n63_, new_n65_, new_n67_, new_n69_, new_n71_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x17), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x17), .c(new_n44_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n51_), .c(new_n47_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n50_), .O(z02));
  nand2  g14(.a(x10), .b(x08), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x12), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x17), .c(x15), .d(new_n47_), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z03));
  nand2  g20(.a(new_n50_), .b(x18), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z05));
  nand2  g23(.a(x17), .b(x15), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z06));
  inv1   g25(.a(x17), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n44_), .O(z07));
  inv1   g27(.a(x16), .O(new_n69_));
  nor2   g28(.a(new_n45_), .b(new_n69_), .O(z08));
  inv1   g29(.a(x09), .O(new_n71_));
  nor3   g30(.a(new_n45_), .b(new_n71_), .c(new_n51_), .O(z09));
  nand3  g31(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nand2  g32(.a(new_n50_), .b(new_n47_), .O(z11));
  inv1   g33(.a(x12), .O(new_n75_));
  oai21  g34(.a(new_n57_), .b(x15), .c(new_n65_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x13), .O(new_n77_));
  inv1   g36(.a(x10), .O(new_n78_));
  nor2   g37(.a(x15), .b(new_n78_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x08), .c(x00), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n77_), .c(new_n75_), .O(z12));
  oai21  g40(.a(new_n56_), .b(new_n67_), .c(new_n47_), .O(new_n82_));
  nand2  g41(.a(new_n57_), .b(x01), .O(new_n83_));
  aoi22  g42(.a(new_n83_), .b(x14), .c(new_n82_), .d(x15), .O(new_n84_));
  oai21  g43(.a(new_n84_), .b(new_n75_), .c(new_n50_), .O(z13));
  nand3  g44(.a(new_n56_), .b(x17), .c(x15), .O(new_n86_));
  nand3  g45(.a(new_n79_), .b(x08), .c(x02), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n86_), .c(new_n75_), .O(z14));
  nand3  g47(.a(x16), .b(new_n47_), .c(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x17), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x15), .O(new_n91_));
  nand3  g50(.a(new_n56_), .b(x16), .c(new_n47_), .O(new_n92_));
  nand2  g51(.a(x08), .b(x03), .O(new_n93_));
  oai21  g52(.a(new_n93_), .b(new_n52_), .c(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n91_), .O(z15));
  inv1   g55(.a(x04), .O(new_n97_));
  oai21  g56(.a(new_n65_), .b(x14), .c(new_n97_), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(x10), .c(x08), .O(new_n99_));
  aoi22  g58(.a(new_n56_), .b(x17), .c(x15), .d(x14), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n50_), .O(z16));
  inv1   g62(.a(x05), .O(new_n104_));
  nand2  g63(.a(new_n56_), .b(x18), .O(new_n105_));
  oai21  g64(.a(new_n56_), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n44_), .O(new_n107_));
  nand3  g66(.a(x18), .b(x17), .c(x15), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n75_), .O(z17));
  aoi21  g68(.a(x19), .b(x12), .c(new_n67_), .O(new_n110_));
  and2   g69(.a(x08), .b(x06), .O(new_n111_));
  aoi22  g70(.a(new_n111_), .b(new_n79_), .c(new_n56_), .d(x19), .O(new_n112_));
  oai22  g71(.a(new_n112_), .b(new_n75_), .c(new_n110_), .d(new_n44_), .O(z18));
  aoi21  g72(.a(x20), .b(x12), .c(new_n67_), .O(new_n114_));
  and2   g73(.a(x08), .b(x07), .O(new_n115_));
  aoi22  g74(.a(new_n115_), .b(new_n79_), .c(new_n56_), .d(x20), .O(new_n116_));
  oai22  g75(.a(new_n116_), .b(new_n75_), .c(new_n114_), .d(new_n44_), .O(z19));
endmodule


