// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nand2  g02(.a(x15), .b(x14), .O(new_n44_));
  oai21  g03(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n46_), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(z01));
  aoi21  g09(.a(new_n47_), .b(new_n46_), .c(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x12), .c(new_n42_), .O(new_n52_));
  inv1   g11(.a(new_n52_), .O(z02));
  inv1   g12(.a(x12), .O(new_n54_));
  inv1   g13(.a(x15), .O(new_n55_));
  nor4   g14(.a(new_n47_), .b(new_n55_), .c(x14), .d(new_n54_), .O(z03));
  nand2  g15(.a(new_n44_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n44_), .b(new_n58_), .O(z05));
  inv1   g18(.a(x17), .O(new_n60_));
  aoi21  g19(.a(x15), .b(x14), .c(new_n60_), .O(z07));
  inv1   g20(.a(x16), .O(new_n62_));
  nand2  g21(.a(new_n44_), .b(new_n62_), .O(z08));
  nand3  g22(.a(new_n44_), .b(x09), .c(x08), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z09));
  nand3  g24(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  aoi21  g25(.a(x10), .b(x08), .c(x15), .O(new_n67_));
  nor2   g26(.a(new_n55_), .b(x14), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n67_), .c(x13), .O(new_n69_));
  inv1   g28(.a(x10), .O(new_n70_));
  nor2   g29(.a(x15), .b(new_n70_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(x08), .c(x00), .O(new_n72_));
  aoi21  g31(.a(new_n72_), .b(new_n69_), .c(new_n54_), .O(z12));
  aoi21  g32(.a(new_n48_), .b(x12), .c(x14), .O(new_n74_));
  nand2  g33(.a(new_n48_), .b(x01), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x14), .c(x12), .O(new_n76_));
  oai21  g35(.a(new_n74_), .b(new_n55_), .c(new_n76_), .O(z13));
  nand3  g36(.a(new_n47_), .b(x15), .c(new_n46_), .O(new_n78_));
  nand3  g37(.a(new_n71_), .b(x08), .c(x02), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n78_), .c(new_n54_), .O(z14));
  nand4  g39(.a(new_n55_), .b(x10), .c(x08), .d(x03), .O(new_n81_));
  oai21  g40(.a(new_n51_), .b(new_n62_), .c(new_n81_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(z15));
  oai21  g43(.a(new_n60_), .b(new_n54_), .c(new_n46_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x15), .O(new_n86_));
  nand4  g45(.a(new_n55_), .b(x10), .c(x08), .d(x04), .O(new_n87_));
  oai21  g46(.a(new_n48_), .b(new_n60_), .c(new_n87_), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n86_), .O(z16));
  inv1   g49(.a(x18), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n54_), .c(new_n46_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x15), .O(new_n93_));
  nand4  g52(.a(new_n55_), .b(x10), .c(x08), .d(x05), .O(new_n94_));
  oai21  g53(.a(new_n48_), .b(new_n91_), .c(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n93_), .O(z17));
  oai21  g56(.a(new_n58_), .b(new_n54_), .c(new_n46_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x15), .O(new_n99_));
  nand4  g58(.a(new_n55_), .b(x10), .c(x08), .d(x06), .O(new_n100_));
  oai21  g59(.a(new_n48_), .b(new_n58_), .c(new_n100_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n99_), .O(z18));
  oai21  g62(.a(new_n43_), .b(new_n54_), .c(new_n46_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x15), .O(new_n105_));
  nand4  g64(.a(new_n55_), .b(x10), .c(x08), .d(x07), .O(new_n106_));
  oai21  g65(.a(new_n48_), .b(new_n43_), .c(new_n106_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n105_), .O(z19));
  buf    g68(.a(x15), .O(z06));
  buf    g69(.a(x14), .O(z11));
endmodule


