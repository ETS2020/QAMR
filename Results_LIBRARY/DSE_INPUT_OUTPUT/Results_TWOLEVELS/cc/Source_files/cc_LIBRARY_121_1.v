// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n118_, new_n119_, new_n120_,
    new_n121_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x10), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(new_n47_), .b(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x10), .c(new_n44_), .O(z01));
  inv1   g08(.a(x10), .O(new_n50_));
  aoi21  g09(.a(new_n44_), .b(x08), .c(x14), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x14), .O(new_n52_));
  oai21  g11(.a(new_n51_), .b(new_n50_), .c(new_n52_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(x12), .O(new_n56_));
  nand2  g15(.a(x10), .b(x08), .O(new_n57_));
  nor4   g16(.a(new_n57_), .b(new_n44_), .c(x14), .d(new_n56_), .O(z03));
  inv1   g17(.a(new_n45_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n59_), .b(new_n61_), .O(z05));
  nor2   g21(.a(new_n44_), .b(new_n50_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n59_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x08), .O(new_n68_));
  inv1   g27(.a(x09), .O(new_n69_));
  nor3   g28(.a(new_n45_), .b(new_n69_), .c(new_n68_), .O(z09));
  nand3  g29(.a(new_n59_), .b(x09), .c(x08), .O(z10));
  nor2   g30(.a(new_n45_), .b(new_n47_), .O(z11));
  inv1   g31(.a(x13), .O(new_n73_));
  oai21  g32(.a(new_n73_), .b(new_n56_), .c(new_n44_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n50_), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n68_), .O(new_n76_));
  nand4  g35(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n77_));
  oai21  g36(.a(new_n76_), .b(new_n73_), .c(new_n77_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n75_), .O(z12));
  oai21  g39(.a(new_n47_), .b(new_n56_), .c(new_n44_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n50_), .O(new_n82_));
  aoi21  g41(.a(x10), .b(x08), .c(x14), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(new_n44_), .O(new_n84_));
  aoi21  g43(.a(x08), .b(x01), .c(new_n47_), .O(new_n85_));
  oai21  g44(.a(new_n85_), .b(new_n84_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n82_), .O(z13));
  nand4  g46(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n88_));
  oai21  g47(.a(new_n44_), .b(x08), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n59_), .O(z14));
  nand3  g50(.a(x10), .b(x08), .c(x03), .O(new_n92_));
  nand3  g51(.a(x16), .b(new_n47_), .c(new_n50_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n44_), .O(new_n95_));
  nand2  g54(.a(new_n44_), .b(x08), .O(new_n96_));
  nand4  g55(.a(new_n96_), .b(x16), .c(new_n47_), .d(x10), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n95_), .c(new_n56_), .O(z15));
  oai21  g57(.a(new_n64_), .b(new_n56_), .c(new_n44_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n50_), .O(new_n100_));
  nand4  g59(.a(new_n44_), .b(x10), .c(x08), .d(x04), .O(new_n101_));
  oai21  g60(.a(new_n76_), .b(new_n64_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n100_), .O(z16));
  inv1   g63(.a(x18), .O(new_n105_));
  oai21  g64(.a(new_n105_), .b(new_n56_), .c(new_n44_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n50_), .O(new_n107_));
  nand4  g66(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n108_));
  oai21  g67(.a(new_n76_), .b(new_n105_), .c(new_n108_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x12), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n107_), .O(z17));
  oai21  g70(.a(new_n61_), .b(new_n56_), .c(new_n44_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n50_), .O(new_n113_));
  nand4  g72(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n114_));
  oai21  g73(.a(new_n76_), .b(new_n61_), .c(new_n114_), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(x12), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n113_), .O(z18));
  inv1   g76(.a(x07), .O(new_n118_));
  oai22  g77(.a(new_n57_), .b(new_n118_), .c(new_n43_), .d(x10), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  nand3  g79(.a(new_n96_), .b(x20), .c(x10), .O(new_n121_));
  aoi21  g80(.a(new_n121_), .b(new_n120_), .c(new_n56_), .O(z19));
endmodule


