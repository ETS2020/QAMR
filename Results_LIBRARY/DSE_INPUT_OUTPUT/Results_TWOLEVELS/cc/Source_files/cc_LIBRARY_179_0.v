// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x15), .b(x12), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  nand4  g05(.a(x15), .b(new_n46_), .c(x10), .d(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(z01));
  inv1   g07(.a(x11), .O(new_n49_));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nand2  g14(.a(x10), .b(x08), .O(new_n56_));
  nor4   g15(.a(new_n56_), .b(new_n50_), .c(x14), .d(new_n55_), .O(z03));
  nand2  g16(.a(new_n44_), .b(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nor2   g18(.a(new_n43_), .b(new_n59_), .O(z05));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(new_n43_), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n44_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x09), .b(x08), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(new_n44_), .O(z09));
  nand3  g25(.a(new_n44_), .b(x09), .c(x08), .O(z10));
  nor2   g26(.a(new_n43_), .b(new_n46_), .O(z11));
  inv1   g27(.a(x10), .O(new_n69_));
  oai21  g28(.a(x15), .b(new_n69_), .c(x12), .O(new_n70_));
  inv1   g29(.a(x08), .O(new_n71_));
  nand2  g30(.a(new_n50_), .b(new_n71_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x13), .O(new_n74_));
  inv1   g33(.a(x00), .O(new_n75_));
  oai21  g34(.a(new_n56_), .b(new_n75_), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n50_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n74_), .O(z12));
  oai21  g37(.a(new_n69_), .b(new_n71_), .c(new_n46_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x15), .O(new_n80_));
  inv1   g39(.a(x01), .O(new_n81_));
  oai21  g40(.a(new_n56_), .b(new_n81_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n55_), .O(z13));
  nand3  g42(.a(new_n56_), .b(x15), .c(x12), .O(new_n84_));
  inv1   g43(.a(x02), .O(new_n85_));
  oai21  g44(.a(new_n56_), .b(new_n85_), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n50_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(new_n84_), .O(z14));
  nand3  g47(.a(new_n73_), .b(x16), .c(new_n46_), .O(new_n89_));
  inv1   g48(.a(x03), .O(new_n90_));
  oai21  g49(.a(new_n56_), .b(new_n90_), .c(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n50_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n89_), .O(z15));
  inv1   g52(.a(x04), .O(new_n94_));
  oai21  g53(.a(new_n56_), .b(new_n94_), .c(x12), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n50_), .O(new_n96_));
  nand3  g55(.a(new_n51_), .b(x17), .c(x12), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n96_), .O(z16));
  nand2  g57(.a(new_n73_), .b(x18), .O(new_n99_));
  inv1   g58(.a(x05), .O(new_n100_));
  oai21  g59(.a(new_n56_), .b(new_n100_), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n50_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n99_), .O(z17));
  nand2  g62(.a(new_n51_), .b(x19), .O(new_n104_));
  nand4  g63(.a(new_n50_), .b(x10), .c(x08), .d(x06), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n55_), .O(z18));
  nand2  g65(.a(new_n73_), .b(x20), .O(new_n107_));
  inv1   g66(.a(x07), .O(new_n108_));
  oai21  g67(.a(new_n56_), .b(new_n108_), .c(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n50_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n107_), .O(z19));
  buf    g70(.a(x15), .O(z06));
endmodule


