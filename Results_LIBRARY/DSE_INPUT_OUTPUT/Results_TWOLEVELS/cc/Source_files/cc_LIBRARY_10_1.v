// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n60_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  aoi21  g02(.a(x17), .b(new_n42_), .c(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nor2   g05(.a(new_n43_), .b(x17), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(x15), .c(new_n46_), .d(x10), .O(new_n49_));
  nor2   g08(.a(new_n49_), .b(new_n45_), .O(z01));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n46_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(new_n48_), .O(z02));
  nand2  g14(.a(x10), .b(x08), .O(new_n56_));
  nand3  g15(.a(x15), .b(new_n46_), .c(x12), .O(new_n57_));
  oai21  g16(.a(new_n57_), .b(new_n56_), .c(new_n48_), .O(z03));
  nand2  g17(.a(new_n48_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nor2   g19(.a(new_n47_), .b(new_n60_), .O(z05));
  nor2   g20(.a(new_n47_), .b(new_n51_), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  inv1   g22(.a(x09), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(x08), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(x08), .c(new_n63_), .O(z07));
  inv1   g25(.a(x16), .O(new_n67_));
  nand2  g26(.a(new_n48_), .b(new_n67_), .O(z08));
  nor3   g27(.a(new_n47_), .b(new_n64_), .c(new_n45_), .O(z09));
  aoi21  g28(.a(x09), .b(x08), .c(new_n47_), .O(z10));
  nor2   g29(.a(new_n47_), .b(new_n46_), .O(z11));
  nand2  g30(.a(new_n52_), .b(x13), .O(new_n72_));
  nand4  g31(.a(new_n51_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(new_n48_), .c(x12), .O(new_n75_));
  inv1   g34(.a(new_n75_), .O(z12));
  aoi21  g35(.a(x10), .b(x08), .c(x14), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(new_n51_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  and2   g38(.a(new_n79_), .b(x14), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n78_), .c(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n48_), .O(z13));
  nand2  g41(.a(new_n56_), .b(x15), .O(new_n83_));
  nand4  g42(.a(new_n51_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n48_), .c(x12), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z14));
  nand3  g46(.a(new_n52_), .b(x16), .c(new_n46_), .O(new_n88_));
  nand4  g47(.a(new_n51_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n48_), .O(z15));
  inv1   g51(.a(x12), .O(new_n93_));
  nand4  g52(.a(new_n43_), .b(new_n51_), .c(x10), .d(x08), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n63_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x04), .O(new_n96_));
  nand2  g55(.a(new_n52_), .b(x17), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n93_), .O(z16));
  nand2  g57(.a(new_n52_), .b(x18), .O(new_n99_));
  nand4  g58(.a(new_n51_), .b(x10), .c(x08), .d(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(new_n48_), .c(x12), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z17));
  inv1   g62(.a(x10), .O(new_n104_));
  nor2   g63(.a(x15), .b(new_n104_), .O(new_n105_));
  and2   g64(.a(x08), .b(x06), .O(new_n106_));
  aoi22  g65(.a(new_n106_), .b(new_n105_), .c(new_n52_), .d(x19), .O(new_n107_));
  oai21  g66(.a(new_n107_), .b(new_n93_), .c(new_n48_), .O(z18));
  nand2  g67(.a(x20), .b(x17), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n94_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x07), .O(new_n111_));
  nand3  g70(.a(new_n52_), .b(x20), .c(x17), .O(new_n112_));
  aoi21  g71(.a(new_n112_), .b(new_n111_), .c(new_n93_), .O(z19));
endmodule


