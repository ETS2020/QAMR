// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x17), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x17), .c(new_n44_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  inv1   g09(.a(x10), .O(new_n51_));
  nor2   g10(.a(x15), .b(new_n51_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n42_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n50_), .O(z02));
  nand2  g15(.a(x10), .b(x08), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(x17), .c(new_n47_), .d(x12), .O(new_n59_));
  aoi21  g18(.a(new_n59_), .b(x17), .c(new_n44_), .O(z03));
  nand2  g19(.a(new_n50_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nor2   g21(.a(new_n45_), .b(new_n62_), .O(z05));
  nand2  g22(.a(x17), .b(x15), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n44_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nor2   g27(.a(new_n45_), .b(new_n68_), .O(z08));
  nand2  g28(.a(x09), .b(x08), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n50_), .O(z09));
  inv1   g30(.a(z09), .O(z10));
  nand2  g31(.a(new_n50_), .b(new_n47_), .O(z11));
  inv1   g32(.a(x12), .O(new_n74_));
  aoi21  g33(.a(x13), .b(x12), .c(new_n66_), .O(new_n75_));
  and2   g34(.a(x08), .b(x00), .O(new_n76_));
  aoi22  g35(.a(new_n76_), .b(new_n52_), .c(new_n57_), .d(x13), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n74_), .c(new_n75_), .d(new_n44_), .O(z12));
  nand2  g37(.a(new_n48_), .b(new_n47_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x17), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n58_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n44_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n74_), .O(z13));
  nand3  g42(.a(new_n57_), .b(x17), .c(x15), .O(new_n84_));
  nand3  g43(.a(new_n52_), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n74_), .O(z14));
  oai21  g45(.a(new_n58_), .b(x15), .c(new_n64_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x16), .c(new_n47_), .O(new_n88_));
  nand3  g47(.a(new_n52_), .b(x08), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n74_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  oai21  g50(.a(new_n64_), .b(x14), .c(new_n91_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x10), .c(x08), .O(new_n93_));
  nand2  g52(.a(x15), .b(x14), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(x10), .c(x08), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x17), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n50_), .O(z16));
  aoi21  g58(.a(x18), .b(x12), .c(new_n66_), .O(new_n100_));
  and2   g59(.a(x08), .b(x05), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n52_), .c(new_n57_), .d(x18), .O(new_n102_));
  oai22  g61(.a(new_n102_), .b(new_n74_), .c(new_n100_), .d(new_n44_), .O(z17));
  aoi21  g62(.a(x19), .b(x12), .c(new_n66_), .O(new_n104_));
  and2   g63(.a(x08), .b(x06), .O(new_n105_));
  aoi22  g64(.a(new_n105_), .b(new_n52_), .c(new_n57_), .d(x19), .O(new_n106_));
  oai22  g65(.a(new_n106_), .b(new_n74_), .c(new_n104_), .d(new_n44_), .O(z18));
  aoi21  g66(.a(x20), .b(x12), .c(new_n66_), .O(new_n108_));
  and2   g67(.a(x08), .b(x07), .O(new_n109_));
  aoi22  g68(.a(new_n109_), .b(new_n52_), .c(new_n57_), .d(x20), .O(new_n110_));
  oai22  g69(.a(new_n110_), .b(new_n74_), .c(new_n108_), .d(new_n44_), .O(z19));
endmodule


