// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n64_, new_n67_, new_n69_, new_n71_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x03), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(x14), .O(new_n46_));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x10), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x15), .c(new_n46_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(z01));
  inv1   g10(.a(x03), .O(new_n52_));
  inv1   g11(.a(x15), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x10), .c(x08), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n46_), .c(new_n52_), .O(new_n55_));
  nor2   g14(.a(new_n53_), .b(new_n46_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n55_), .c(x12), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x11), .O(z02));
  inv1   g17(.a(new_n44_), .O(new_n59_));
  inv1   g18(.a(new_n49_), .O(new_n60_));
  nand3  g19(.a(x15), .b(new_n46_), .c(x12), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(z03));
  nand2  g21(.a(new_n59_), .b(x18), .O(z04));
  inv1   g22(.a(x19), .O(new_n64_));
  nand2  g23(.a(new_n59_), .b(new_n64_), .O(z05));
  nand2  g24(.a(new_n53_), .b(x03), .O(z06));
  inv1   g25(.a(x17), .O(new_n67_));
  nand2  g26(.a(new_n59_), .b(new_n67_), .O(z07));
  inv1   g27(.a(x16), .O(new_n69_));
  nand2  g28(.a(new_n59_), .b(new_n69_), .O(z08));
  inv1   g29(.a(x09), .O(new_n71_));
  nor3   g30(.a(new_n44_), .b(new_n71_), .c(new_n47_), .O(z09));
  nand3  g31(.a(new_n59_), .b(x09), .c(x08), .O(z10));
  nand2  g32(.a(new_n59_), .b(new_n46_), .O(z11));
  inv1   g33(.a(x12), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n48_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x03), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  nand3  g38(.a(new_n54_), .b(x13), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(z12));
  oai21  g40(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x15), .O(new_n83_));
  nand2  g42(.a(new_n49_), .b(x01), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x14), .c(x03), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n83_), .c(new_n75_), .O(z13));
  nand2  g45(.a(new_n60_), .b(x15), .O(new_n87_));
  inv1   g46(.a(new_n54_), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(x03), .c(x02), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n87_), .c(new_n75_), .O(z14));
  nor2   g49(.a(new_n69_), .b(x14), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n88_), .c(x03), .O(new_n92_));
  nand3  g51(.a(x16), .b(x15), .c(new_n46_), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n75_), .O(z15));
  nand4  g53(.a(new_n53_), .b(x10), .c(x08), .d(x04), .O(new_n95_));
  oai21  g54(.a(new_n49_), .b(new_n67_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x03), .O(new_n97_));
  nand2  g56(.a(x17), .b(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n75_), .O(z16));
  nand3  g58(.a(new_n76_), .b(x08), .c(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x03), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n53_), .O(new_n102_));
  nand3  g61(.a(new_n54_), .b(x18), .c(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(z17));
  nand3  g63(.a(new_n76_), .b(x08), .c(x06), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(x03), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n53_), .O(new_n107_));
  nand3  g66(.a(new_n54_), .b(x19), .c(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(z18));
  nand3  g68(.a(new_n76_), .b(x08), .c(x07), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x03), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n53_), .O(new_n112_));
  nand3  g71(.a(new_n54_), .b(x20), .c(x12), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n112_), .O(z19));
endmodule


