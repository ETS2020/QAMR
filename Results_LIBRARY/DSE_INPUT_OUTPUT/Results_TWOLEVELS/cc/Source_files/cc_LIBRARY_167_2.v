// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n63_, new_n65_, new_n66_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x17), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x17), .c(new_n44_), .O(z01));
  oai21  g08(.a(x17), .b(new_n44_), .c(x14), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(z02));
  nand2  g13(.a(x10), .b(x08), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(x17), .c(new_n47_), .d(x12), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x17), .c(new_n44_), .O(z03));
  inv1   g17(.a(new_n45_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n59_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x16), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z08));
  inv1   g23(.a(x08), .O(new_n65_));
  inv1   g24(.a(x09), .O(new_n66_));
  nor3   g25(.a(new_n45_), .b(new_n66_), .c(new_n65_), .O(z09));
  nand3  g26(.a(new_n59_), .b(x09), .c(x08), .O(z10));
  inv1   g27(.a(new_n50_), .O(z11));
  nand2  g28(.a(x13), .b(x12), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x17), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n55_), .b(x13), .O(new_n73_));
  nand4  g32(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n72_), .O(z12));
  inv1   g36(.a(x12), .O(new_n78_));
  nand2  g37(.a(new_n48_), .b(new_n47_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x17), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n56_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n44_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n78_), .O(z13));
  nand3  g42(.a(new_n55_), .b(x17), .c(x15), .O(new_n84_));
  inv1   g43(.a(x10), .O(new_n85_));
  nor2   g44(.a(x15), .b(new_n85_), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(x08), .c(x02), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n84_), .c(new_n78_), .O(z14));
  nand2  g47(.a(new_n55_), .b(new_n44_), .O(new_n89_));
  nand2  g48(.a(x17), .b(x15), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(x16), .c(new_n47_), .O(new_n92_));
  nand3  g51(.a(new_n86_), .b(x08), .c(x03), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n78_), .O(z15));
  inv1   g53(.a(x04), .O(new_n95_));
  oai22  g54(.a(new_n90_), .b(x14), .c(x15), .d(new_n95_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(x10), .c(x08), .O(new_n97_));
  nand2  g56(.a(x15), .b(x14), .O(new_n98_));
  nand3  g57(.a(new_n98_), .b(x10), .c(x08), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x17), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n97_), .c(new_n78_), .O(z16));
  nand2  g60(.a(x18), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x17), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x15), .O(new_n104_));
  nand2  g63(.a(new_n55_), .b(x18), .O(new_n105_));
  nand4  g64(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n104_), .O(z17));
  oai21  g68(.a(new_n61_), .b(new_n78_), .c(x17), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x15), .O(new_n111_));
  nand2  g70(.a(new_n55_), .b(x19), .O(new_n112_));
  nand4  g71(.a(new_n44_), .b(x10), .c(x08), .d(x06), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x12), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n111_), .O(z18));
  nand2  g75(.a(new_n55_), .b(x20), .O(new_n117_));
  nand3  g76(.a(x10), .b(x08), .c(x07), .O(new_n118_));
  nand2  g77(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n44_), .O(new_n120_));
  nand3  g79(.a(x20), .b(x17), .c(x15), .O(new_n121_));
  aoi21  g80(.a(new_n121_), .b(new_n120_), .c(new_n78_), .O(z19));
  buf    g81(.a(x15), .O(z06));
  buf    g82(.a(x17), .O(z07));
endmodule


