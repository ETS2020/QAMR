// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_, new_n62_,
    new_n64_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x19), .b(x04), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x08), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nor2   g07(.a(new_n43_), .b(new_n48_), .O(z06));
  nand3  g08(.a(z06), .b(new_n47_), .c(x10), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(new_n46_), .O(z01));
  inv1   g10(.a(x11), .O(new_n52_));
  nand3  g11(.a(new_n48_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n44_), .c(x12), .d(new_n52_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand4  g15(.a(z06), .b(new_n47_), .c(x12), .d(x10), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n46_), .O(z03));
  nand2  g17(.a(new_n44_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x04), .O(z05));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n44_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n43_), .b(new_n64_), .O(z08));
  aoi21  g24(.a(x09), .b(x08), .c(new_n43_), .O(z10));
  inv1   g25(.a(z10), .O(z09));
  nor2   g26(.a(new_n43_), .b(new_n47_), .O(z11));
  nand2  g27(.a(new_n53_), .b(x13), .O(new_n69_));
  nand4  g28(.a(new_n48_), .b(x10), .c(x08), .d(x00), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n44_), .c(x12), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z12));
  aoi21  g32(.a(x10), .b(x08), .c(x14), .O(new_n74_));
  nand3  g33(.a(x10), .b(x08), .c(x01), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x14), .O(new_n76_));
  oai21  g35(.a(new_n74_), .b(new_n48_), .c(new_n76_), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n44_), .c(x12), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(z13));
  and2   g38(.a(x10), .b(x08), .O(new_n80_));
  nand4  g39(.a(new_n48_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  oai21  g40(.a(new_n80_), .b(new_n48_), .c(new_n81_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(new_n44_), .c(x12), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z14));
  nand3  g43(.a(new_n53_), .b(x16), .c(new_n47_), .O(new_n85_));
  nand4  g44(.a(new_n48_), .b(x10), .c(x08), .d(x03), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n44_), .O(z15));
  inv1   g48(.a(x12), .O(new_n90_));
  inv1   g49(.a(x10), .O(new_n91_));
  nor2   g50(.a(x15), .b(new_n91_), .O(new_n92_));
  and2   g51(.a(x08), .b(x04), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n92_), .c(new_n53_), .d(x17), .O(new_n94_));
  oai21  g53(.a(new_n94_), .b(new_n90_), .c(new_n44_), .O(z16));
  and2   g54(.a(x08), .b(x05), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n92_), .c(new_n53_), .d(x18), .O(new_n97_));
  oai21  g56(.a(new_n97_), .b(new_n90_), .c(new_n44_), .O(z17));
  nand4  g57(.a(new_n48_), .b(x10), .c(x08), .d(x04), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n60_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(x06), .O(new_n101_));
  oai21  g60(.a(new_n74_), .b(new_n48_), .c(new_n80_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x19), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n101_), .c(new_n90_), .O(z18));
  and2   g63(.a(x08), .b(x07), .O(new_n105_));
  aoi22  g64(.a(new_n105_), .b(new_n92_), .c(new_n53_), .d(x20), .O(new_n106_));
  oai21  g65(.a(new_n106_), .b(new_n90_), .c(new_n44_), .O(z19));
endmodule


