// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x13), .b(x04), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x15), .b(new_n48_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(z01));
  inv1   g09(.a(x15), .O(new_n51_));
  nand3  g10(.a(new_n51_), .b(x10), .c(x08), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n48_), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n46_), .c(x12), .d(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(x08), .O(new_n56_));
  nor2   g15(.a(new_n44_), .b(new_n51_), .O(new_n57_));
  nand4  g16(.a(new_n57_), .b(new_n48_), .c(x12), .d(x10), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(new_n56_), .O(z03));
  nand2  g18(.a(new_n46_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n46_), .b(new_n51_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n44_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n44_), .b(new_n68_), .c(new_n56_), .O(z09));
  nand3  g28(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n44_), .b(new_n48_), .O(z11));
  nand3  g30(.a(x15), .b(new_n48_), .c(x13), .O(new_n72_));
  nand2  g31(.a(new_n51_), .b(x00), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g33(.a(new_n74_), .b(x10), .c(x08), .O(new_n75_));
  nand2  g34(.a(x15), .b(x14), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x10), .c(x08), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x13), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n75_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n46_), .O(z12));
  aoi21  g40(.a(x10), .b(x08), .c(x14), .O(new_n82_));
  inv1   g41(.a(x01), .O(new_n83_));
  oai21  g42(.a(new_n47_), .b(new_n83_), .c(x14), .O(new_n84_));
  oai21  g43(.a(new_n82_), .b(new_n51_), .c(new_n84_), .O(new_n85_));
  nand3  g44(.a(new_n85_), .b(new_n46_), .c(x12), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z13));
  nand2  g46(.a(new_n47_), .b(x15), .O(new_n88_));
  inv1   g47(.a(x10), .O(new_n89_));
  nor2   g48(.a(x15), .b(new_n89_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(x08), .c(x02), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n46_), .O(z14));
  nand3  g53(.a(new_n52_), .b(x16), .c(new_n48_), .O(new_n95_));
  nand3  g54(.a(new_n90_), .b(x08), .c(x03), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(new_n46_), .c(x12), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z15));
  inv1   g58(.a(x12), .O(new_n100_));
  inv1   g59(.a(new_n52_), .O(new_n101_));
  oai21  g60(.a(new_n101_), .b(x17), .c(x04), .O(new_n102_));
  nand3  g61(.a(new_n52_), .b(x17), .c(x13), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n100_), .O(z16));
  nand2  g63(.a(new_n52_), .b(x18), .O(new_n105_));
  nand3  g64(.a(new_n90_), .b(x08), .c(x05), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand3  g66(.a(new_n107_), .b(new_n46_), .c(x12), .O(new_n108_));
  inv1   g67(.a(new_n108_), .O(z17));
  nand3  g68(.a(new_n90_), .b(x08), .c(x06), .O(new_n110_));
  oai21  g69(.a(new_n101_), .b(new_n61_), .c(new_n110_), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(new_n46_), .c(x12), .O(new_n112_));
  inv1   g71(.a(new_n112_), .O(z18));
  nand3  g72(.a(new_n90_), .b(x08), .c(x07), .O(new_n114_));
  oai21  g73(.a(new_n101_), .b(new_n43_), .c(new_n114_), .O(new_n115_));
  nand3  g74(.a(new_n115_), .b(new_n46_), .c(x12), .O(new_n116_));
  inv1   g75(.a(new_n116_), .O(z19));
endmodule


