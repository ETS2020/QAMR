// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n60_, new_n61_,
    new_n64_, new_n66_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x07), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x07), .O(new_n46_));
  nand3  g05(.a(x10), .b(x08), .c(new_n46_), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n45_), .c(x14), .O(z01));
  inv1   g07(.a(x08), .O(new_n49_));
  inv1   g08(.a(x14), .O(new_n50_));
  nand2  g09(.a(new_n45_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n49_), .c(new_n50_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n43_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor4   g14(.a(new_n47_), .b(new_n45_), .c(x14), .d(new_n55_), .O(z03));
  nand2  g15(.a(new_n43_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z05));
  nand2  g18(.a(x09), .b(new_n49_), .O(new_n60_));
  oai21  g19(.a(new_n60_), .b(new_n49_), .c(x15), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(x07), .O(z06));
  and2   g21(.a(new_n43_), .b(x17), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x15), .b(x07), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  aoi22  g27(.a(x15), .b(x07), .c(x09), .d(x08), .O(z10));
  aoi21  g28(.a(x15), .b(x07), .c(new_n50_), .O(z11));
  aoi21  g29(.a(x13), .b(x12), .c(x07), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  nand2  g32(.a(x10), .b(x08), .O(new_n74_));
  and2   g33(.a(x08), .b(x00), .O(new_n75_));
  aoi22  g34(.a(new_n75_), .b(new_n73_), .c(new_n74_), .d(x13), .O(new_n76_));
  oai22  g35(.a(new_n76_), .b(new_n55_), .c(new_n71_), .d(new_n45_), .O(z12));
  nand4  g36(.a(new_n50_), .b(x10), .c(x08), .d(new_n46_), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n50_), .c(new_n45_), .O(new_n79_));
  inv1   g38(.a(new_n74_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(x01), .c(new_n50_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n79_), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n43_), .O(z13));
  nand3  g42(.a(new_n74_), .b(x15), .c(new_n46_), .O(new_n84_));
  nand3  g43(.a(new_n73_), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n55_), .O(z14));
  nand2  g45(.a(x15), .b(new_n46_), .O(new_n87_));
  oai21  g46(.a(new_n80_), .b(x15), .c(new_n87_), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(x16), .c(new_n50_), .O(new_n89_));
  nand3  g48(.a(new_n73_), .b(x08), .c(x03), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n55_), .O(z15));
  inv1   g50(.a(x04), .O(new_n92_));
  nand2  g51(.a(new_n74_), .b(x17), .O(new_n93_));
  oai21  g52(.a(new_n74_), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n45_), .O(new_n95_));
  nand3  g54(.a(x17), .b(x15), .c(new_n46_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n55_), .O(z16));
  aoi21  g56(.a(x18), .b(x12), .c(x07), .O(new_n98_));
  and2   g57(.a(x08), .b(x05), .O(new_n99_));
  aoi22  g58(.a(new_n99_), .b(new_n73_), .c(new_n74_), .d(x18), .O(new_n100_));
  oai22  g59(.a(new_n100_), .b(new_n55_), .c(new_n98_), .d(new_n45_), .O(z17));
  inv1   g60(.a(x06), .O(new_n102_));
  nand2  g61(.a(new_n74_), .b(x19), .O(new_n103_));
  oai21  g62(.a(new_n74_), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand3  g64(.a(x19), .b(x15), .c(new_n46_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n105_), .c(new_n55_), .O(z18));
  nand2  g66(.a(new_n74_), .b(x20), .O(new_n108_));
  oai21  g67(.a(new_n74_), .b(new_n46_), .c(new_n108_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n45_), .O(new_n110_));
  nand3  g69(.a(x20), .b(x15), .c(new_n46_), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n55_), .O(z19));
endmodule


