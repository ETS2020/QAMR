// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n64_, new_n66_, new_n68_, new_n69_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x14), .O(new_n43_));
  nor2   g02(.a(x15), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(new_n47_), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x15), .c(new_n43_), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  nand2  g10(.a(x15), .b(x14), .O(new_n52_));
  inv1   g11(.a(x15), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n43_), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(new_n47_), .c(new_n52_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n51_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  nand3  g16(.a(x15), .b(new_n43_), .c(x12), .O(new_n58_));
  oai21  g17(.a(new_n58_), .b(new_n47_), .c(new_n45_), .O(z03));
  nand2  g18(.a(new_n45_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n53_), .b(new_n43_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n45_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n44_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x08), .O(new_n68_));
  oai21  g27(.a(x15), .b(new_n43_), .c(x09), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n68_), .O(z09));
  aoi22  g29(.a(new_n53_), .b(x14), .c(x09), .d(x08), .O(z10));
  nand2  g30(.a(x09), .b(new_n68_), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n68_), .c(x15), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n43_), .O(z11));
  nand4  g33(.a(x12), .b(x10), .c(x08), .d(x00), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(new_n43_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n53_), .O(new_n77_));
  nand3  g36(.a(new_n53_), .b(x10), .c(x08), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x13), .c(x12), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n77_), .O(z12));
  inv1   g39(.a(x12), .O(new_n81_));
  aoi21  g40(.a(x10), .b(x08), .c(x14), .O(new_n82_));
  nor3   g41(.a(new_n82_), .b(new_n53_), .c(new_n81_), .O(z13));
  nand4  g42(.a(new_n53_), .b(x10), .c(x08), .d(x02), .O(new_n84_));
  oai21  g43(.a(new_n48_), .b(new_n53_), .c(new_n84_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n45_), .O(z14));
  nand2  g46(.a(new_n78_), .b(x16), .O(new_n88_));
  nand4  g47(.a(new_n53_), .b(x10), .c(x08), .d(x03), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n43_), .c(x12), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z15));
  nand2  g51(.a(new_n47_), .b(x17), .O(new_n93_));
  nand4  g52(.a(new_n53_), .b(x10), .c(x08), .d(x04), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n43_), .O(new_n96_));
  nand2  g55(.a(x17), .b(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n81_), .O(z16));
  nand2  g57(.a(new_n47_), .b(x18), .O(new_n99_));
  nand4  g58(.a(new_n53_), .b(x10), .c(x08), .d(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n43_), .O(new_n102_));
  nand2  g61(.a(x18), .b(x15), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n81_), .O(z17));
  nand4  g63(.a(x12), .b(x10), .c(x08), .d(x06), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n43_), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n53_), .O(new_n107_));
  nand3  g66(.a(new_n78_), .b(x19), .c(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(z18));
  nand2  g68(.a(new_n47_), .b(x20), .O(new_n110_));
  nand4  g69(.a(new_n53_), .b(x10), .c(x08), .d(x07), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n43_), .O(new_n113_));
  nand2  g72(.a(x20), .b(x15), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n81_), .O(z19));
endmodule


