// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n63_, new_n65_, new_n67_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(x19), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x19), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n47_), .c(new_n43_), .d(x14), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  inv1   g10(.a(x14), .O(new_n52_));
  nand2  g11(.a(new_n43_), .b(x10), .O(new_n53_));
  oai22  g12(.a(new_n53_), .b(new_n51_), .c(new_n44_), .d(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  inv1   g15(.a(new_n48_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x12), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x19), .c(x15), .d(new_n52_), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z03));
  nand2  g20(.a(new_n45_), .b(x18), .O(z04));
  inv1   g21(.a(x17), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n44_), .b(new_n65_), .O(z08));
  inv1   g25(.a(x09), .O(new_n67_));
  nor3   g26(.a(new_n44_), .b(new_n67_), .c(new_n51_), .O(z09));
  nand3  g27(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n45_), .b(new_n52_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  nand2  g30(.a(x19), .b(x15), .O(new_n72_));
  oai21  g31(.a(new_n57_), .b(x15), .c(new_n72_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x13), .O(new_n74_));
  inv1   g33(.a(x10), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n75_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n74_), .c(new_n71_), .O(z12));
  oai21  g37(.a(new_n75_), .b(new_n51_), .c(new_n52_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x19), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n57_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n43_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n71_), .O(z13));
  nand3  g42(.a(new_n48_), .b(x19), .c(x15), .O(new_n84_));
  nand3  g43(.a(new_n76_), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n71_), .O(z14));
  nand3  g45(.a(x16), .b(new_n52_), .c(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x19), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x15), .O(new_n89_));
  nand3  g48(.a(new_n48_), .b(x16), .c(new_n52_), .O(new_n90_));
  nand2  g49(.a(x08), .b(x03), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n53_), .c(new_n90_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n89_), .O(z15));
  aoi21  g53(.a(x17), .b(x12), .c(new_n47_), .O(new_n95_));
  and2   g54(.a(x08), .b(x04), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n76_), .c(new_n48_), .d(x17), .O(new_n97_));
  oai22  g56(.a(new_n97_), .b(new_n71_), .c(new_n95_), .d(new_n43_), .O(z16));
  inv1   g57(.a(x05), .O(new_n99_));
  nand2  g58(.a(new_n48_), .b(x18), .O(new_n100_));
  oai21  g59(.a(new_n48_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n43_), .O(new_n102_));
  nand3  g61(.a(x19), .b(x18), .c(x15), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n71_), .O(z17));
  inv1   g63(.a(x06), .O(new_n105_));
  oai21  g64(.a(new_n72_), .b(x14), .c(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(x10), .c(x08), .O(new_n107_));
  nand2  g66(.a(x15), .b(x14), .O(new_n108_));
  nand3  g67(.a(new_n108_), .b(x10), .c(x08), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x19), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n107_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n45_), .O(z18));
  inv1   g72(.a(x07), .O(new_n114_));
  nand2  g73(.a(new_n48_), .b(x20), .O(new_n115_));
  oai21  g74(.a(new_n48_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n43_), .O(new_n117_));
  nand3  g76(.a(x20), .b(x19), .c(x15), .O(new_n118_));
  aoi21  g77(.a(new_n118_), .b(new_n117_), .c(new_n71_), .O(z19));
  buf    g78(.a(x19), .O(z05));
  buf    g79(.a(x15), .O(z06));
endmodule


