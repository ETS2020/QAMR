// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n55_, new_n58_, new_n60_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nor2   g01(.a(x15), .b(x14), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  aoi21  g05(.a(new_n46_), .b(x15), .c(x14), .O(z01));
  inv1   g06(.a(x11), .O(new_n48_));
  nand3  g07(.a(x14), .b(x12), .c(new_n48_), .O(new_n49_));
  inv1   g08(.a(new_n49_), .O(z02));
  inv1   g09(.a(x12), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  nor4   g11(.a(new_n46_), .b(new_n52_), .c(x14), .d(new_n51_), .O(z03));
  nand2  g12(.a(new_n44_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nand2  g14(.a(new_n44_), .b(new_n55_), .O(z05));
  nand2  g15(.a(new_n52_), .b(x14), .O(z06));
  inv1   g16(.a(x17), .O(new_n58_));
  nor2   g17(.a(new_n43_), .b(new_n58_), .O(z07));
  inv1   g18(.a(x16), .O(new_n60_));
  nor2   g19(.a(new_n43_), .b(new_n60_), .O(z08));
  aoi21  g20(.a(x09), .b(x08), .c(new_n43_), .O(z10));
  inv1   g21(.a(z10), .O(z09));
  inv1   g22(.a(x14), .O(new_n64_));
  nand2  g23(.a(x15), .b(new_n64_), .O(z11));
  nand2  g24(.a(new_n46_), .b(x14), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n52_), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(x13), .O(new_n68_));
  nand3  g27(.a(new_n52_), .b(x14), .c(x10), .O(new_n69_));
  inv1   g28(.a(new_n69_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(x08), .c(x00), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n68_), .c(new_n51_), .O(z12));
  aoi21  g31(.a(new_n52_), .b(x08), .c(new_n64_), .O(new_n73_));
  nand2  g32(.a(x10), .b(x01), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand4  g34(.a(x15), .b(new_n64_), .c(x10), .d(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n73_), .c(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n44_), .O(z13));
  nand2  g38(.a(new_n46_), .b(x15), .O(new_n80_));
  nand4  g39(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(z14));
  nand3  g43(.a(new_n70_), .b(x08), .c(x03), .O(new_n85_));
  nand3  g44(.a(x16), .b(x15), .c(new_n64_), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n51_), .O(z15));
  nand4  g46(.a(x12), .b(x10), .c(x08), .d(x04), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x14), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n52_), .O(new_n90_));
  nand3  g49(.a(new_n52_), .b(x10), .c(x08), .O(new_n91_));
  nand3  g50(.a(new_n91_), .b(x17), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n90_), .O(z16));
  nand2  g52(.a(new_n46_), .b(x18), .O(new_n94_));
  nand4  g53(.a(new_n52_), .b(x10), .c(x08), .d(x05), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x14), .O(new_n97_));
  nand2  g56(.a(x18), .b(x15), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n51_), .O(z17));
  nand2  g58(.a(new_n46_), .b(x19), .O(new_n100_));
  nand4  g59(.a(new_n52_), .b(x10), .c(x08), .d(x06), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x14), .O(new_n103_));
  nand2  g62(.a(x19), .b(x15), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n51_), .O(z18));
  nand2  g64(.a(new_n46_), .b(x20), .O(new_n106_));
  nand4  g65(.a(new_n52_), .b(x10), .c(x08), .d(x07), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x14), .O(new_n109_));
  nand2  g68(.a(x20), .b(x15), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n51_), .O(z19));
endmodule


