// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n63_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x12), .O(new_n43_));
  nor2   g02(.a(new_n43_), .b(x01), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  inv1   g07(.a(x01), .O(new_n49_));
  nand2  g08(.a(x12), .b(new_n49_), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(x15), .c(new_n48_), .d(x10), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n47_), .O(z01));
  inv1   g11(.a(x11), .O(new_n53_));
  inv1   g12(.a(x15), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x10), .c(x08), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(x01), .c(new_n43_), .O(z02));
  nor2   g17(.a(new_n47_), .b(new_n49_), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(x15), .c(new_n48_), .d(x10), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(x01), .c(new_n43_), .O(z03));
  nand2  g20(.a(new_n45_), .b(x18), .O(z04));
  inv1   g21(.a(x19), .O(new_n63_));
  nand2  g22(.a(new_n45_), .b(new_n63_), .O(z05));
  nand2  g23(.a(new_n45_), .b(new_n54_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nor2   g25(.a(new_n44_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nor2   g27(.a(new_n44_), .b(new_n68_), .O(z08));
  inv1   g28(.a(x09), .O(new_n70_));
  nor3   g29(.a(new_n44_), .b(new_n70_), .c(new_n47_), .O(z09));
  nand3  g30(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nand2  g31(.a(new_n45_), .b(new_n48_), .O(z11));
  nand2  g32(.a(new_n55_), .b(x13), .O(new_n74_));
  inv1   g33(.a(x10), .O(new_n75_));
  nor2   g34(.a(x15), .b(new_n75_), .O(new_n76_));
  and2   g35(.a(x08), .b(x00), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n49_), .O(new_n78_));
  aoi21  g37(.a(new_n78_), .b(new_n74_), .c(new_n43_), .O(z12));
  nand2  g38(.a(x10), .b(x08), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x14), .O(new_n81_));
  nand3  g40(.a(x15), .b(x10), .c(x08), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand3  g42(.a(new_n83_), .b(x12), .c(x01), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z13));
  nand2  g44(.a(new_n80_), .b(x15), .O(new_n86_));
  nand4  g45(.a(new_n54_), .b(x10), .c(x08), .d(x02), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(new_n86_), .c(x01), .O(new_n88_));
  and2   g47(.a(new_n88_), .b(x12), .O(z14));
  nand3  g48(.a(new_n55_), .b(x16), .c(new_n48_), .O(new_n90_));
  nand4  g49(.a(new_n54_), .b(x10), .c(x08), .d(x03), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x12), .c(x01), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z15));
  nand2  g53(.a(new_n55_), .b(x17), .O(new_n95_));
  and2   g54(.a(x08), .b(x04), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n76_), .c(new_n49_), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n95_), .c(new_n43_), .O(z16));
  nand2  g57(.a(new_n55_), .b(x18), .O(new_n99_));
  nand4  g58(.a(new_n54_), .b(x10), .c(x08), .d(x05), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(x12), .c(x01), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z17));
  nand2  g62(.a(new_n55_), .b(x19), .O(new_n104_));
  and2   g63(.a(x08), .b(x06), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n76_), .c(new_n49_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n104_), .c(new_n43_), .O(z18));
  nand2  g66(.a(new_n55_), .b(x20), .O(new_n108_));
  nand4  g67(.a(new_n54_), .b(x10), .c(x08), .d(x07), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand3  g69(.a(new_n110_), .b(x12), .c(x01), .O(new_n111_));
  inv1   g70(.a(new_n111_), .O(z19));
endmodule


