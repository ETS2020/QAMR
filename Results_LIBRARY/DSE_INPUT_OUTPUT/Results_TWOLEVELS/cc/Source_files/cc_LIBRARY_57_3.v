// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n64_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  inv1   g00(.a(x18), .O(new_n42_));
  nand3  g01(.a(x20), .b(new_n42_), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x20), .b(x18), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(x15), .c(new_n46_), .d(x10), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g08(.a(x15), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n47_), .O(z02));
  nand2  g13(.a(x10), .b(x08), .O(new_n55_));
  nand3  g14(.a(x15), .b(new_n46_), .c(x12), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n55_), .c(new_n47_), .O(z03));
  nand2  g16(.a(x09), .b(new_n45_), .O(new_n58_));
  oai21  g17(.a(new_n58_), .b(new_n45_), .c(new_n42_), .O(new_n59_));
  nand2  g18(.a(new_n59_), .b(new_n47_), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nand2  g20(.a(new_n47_), .b(new_n61_), .O(z05));
  nand2  g21(.a(new_n47_), .b(new_n50_), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n47_), .b(new_n64_), .O(z07));
  and2   g24(.a(new_n47_), .b(x16), .O(z08));
  nand3  g25(.a(new_n47_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  aoi22  g27(.a(x20), .b(x18), .c(x09), .d(x08), .O(z10));
  nand2  g28(.a(new_n47_), .b(new_n46_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x15), .b(new_n72_), .O(new_n73_));
  and2   g32(.a(x08), .b(x00), .O(new_n74_));
  aoi22  g33(.a(new_n74_), .b(new_n73_), .c(new_n51_), .d(x13), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n71_), .c(new_n47_), .O(z12));
  nand3  g35(.a(x10), .b(x08), .c(x01), .O(new_n77_));
  nand3  g36(.a(new_n77_), .b(new_n47_), .c(x14), .O(new_n78_));
  nand2  g37(.a(x20), .b(x18), .O(new_n79_));
  nand4  g38(.a(new_n79_), .b(x15), .c(x10), .d(x08), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n78_), .c(new_n71_), .O(z13));
  nand2  g40(.a(new_n55_), .b(x15), .O(new_n82_));
  nand3  g41(.a(new_n73_), .b(x08), .c(x02), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x12), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(new_n47_), .O(z14));
  nand3  g45(.a(new_n51_), .b(x16), .c(new_n46_), .O(new_n87_));
  nand3  g46(.a(new_n73_), .b(x08), .c(x03), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g48(.a(new_n89_), .b(new_n47_), .c(x12), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z15));
  nand2  g50(.a(new_n51_), .b(x17), .O(new_n92_));
  nand3  g51(.a(new_n73_), .b(x08), .c(x04), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand3  g53(.a(new_n94_), .b(new_n47_), .c(x12), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z16));
  nand2  g55(.a(new_n42_), .b(new_n50_), .O(new_n97_));
  oai22  g56(.a(new_n97_), .b(new_n55_), .c(x20), .d(new_n42_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x05), .O(new_n99_));
  inv1   g58(.a(x20), .O(new_n100_));
  nand3  g59(.a(new_n51_), .b(new_n100_), .c(x18), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n99_), .c(new_n71_), .O(z17));
  and2   g61(.a(x08), .b(x06), .O(new_n103_));
  aoi22  g62(.a(new_n103_), .b(new_n73_), .c(new_n51_), .d(x19), .O(new_n104_));
  oai21  g63(.a(new_n104_), .b(new_n71_), .c(new_n47_), .O(z18));
  nand2  g64(.a(new_n50_), .b(x07), .O(new_n106_));
  nand3  g65(.a(x20), .b(new_n42_), .c(x15), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n72_), .O(new_n108_));
  aoi21  g67(.a(x10), .b(x08), .c(new_n100_), .O(new_n109_));
  aoi21  g68(.a(new_n108_), .b(x08), .c(new_n109_), .O(new_n110_));
  oai21  g69(.a(new_n110_), .b(new_n71_), .c(new_n47_), .O(z19));
endmodule


