// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n62_, new_n64_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x17), .b(new_n44_), .O(new_n45_));
  nor3   g04(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x17), .c(new_n44_), .O(z01));
  inv1   g08(.a(new_n45_), .O(new_n50_));
  nand3  g09(.a(new_n44_), .b(x10), .c(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(new_n42_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n50_), .O(z02));
  nand2  g13(.a(x10), .b(x08), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(x17), .c(x15), .d(new_n47_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  nor2   g19(.a(new_n45_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n50_), .b(new_n62_), .O(z05));
  nand2  g22(.a(x17), .b(x15), .O(new_n64_));
  inv1   g23(.a(new_n64_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n44_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nand2  g27(.a(new_n50_), .b(new_n68_), .O(z08));
  nand2  g28(.a(x09), .b(x08), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n50_), .O(z09));
  nand3  g30(.a(new_n50_), .b(x09), .c(x08), .O(z10));
  nor2   g31(.a(new_n45_), .b(new_n47_), .O(z11));
  inv1   g32(.a(x12), .O(new_n74_));
  inv1   g33(.a(x13), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n74_), .c(x17), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x15), .O(new_n77_));
  nand4  g36(.a(new_n44_), .b(x10), .c(x08), .d(x00), .O(new_n78_));
  oai21  g37(.a(new_n56_), .b(new_n75_), .c(new_n78_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n77_), .O(z12));
  aoi21  g40(.a(x10), .b(x08), .c(x14), .O(new_n82_));
  nor2   g41(.a(new_n82_), .b(new_n66_), .O(new_n83_));
  nand3  g42(.a(x10), .b(x08), .c(x01), .O(new_n84_));
  and2   g43(.a(new_n84_), .b(x14), .O(new_n85_));
  aoi21  g44(.a(new_n83_), .b(x15), .c(new_n85_), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n74_), .c(new_n50_), .O(z13));
  nand4  g46(.a(new_n44_), .b(x10), .c(x08), .d(x02), .O(new_n88_));
  oai21  g47(.a(new_n56_), .b(new_n44_), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n50_), .O(z14));
  nand3  g50(.a(x16), .b(new_n47_), .c(x12), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x17), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x15), .O(new_n94_));
  nand3  g53(.a(new_n55_), .b(x16), .c(new_n47_), .O(new_n95_));
  nand4  g54(.a(new_n44_), .b(x10), .c(x08), .d(x03), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x12), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n94_), .O(z15));
  inv1   g58(.a(x04), .O(new_n100_));
  oai22  g59(.a(new_n64_), .b(x14), .c(x15), .d(new_n100_), .O(new_n101_));
  nand3  g60(.a(new_n101_), .b(x10), .c(x08), .O(new_n102_));
  nand2  g61(.a(x15), .b(x14), .O(new_n103_));
  nand3  g62(.a(new_n103_), .b(x10), .c(x08), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(x17), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n102_), .c(new_n74_), .O(z16));
  inv1   g65(.a(x18), .O(new_n107_));
  oai21  g66(.a(new_n107_), .b(new_n74_), .c(x17), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x15), .O(new_n109_));
  nand4  g68(.a(new_n44_), .b(x10), .c(x08), .d(x05), .O(new_n110_));
  oai21  g69(.a(new_n56_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x12), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n109_), .O(z17));
  inv1   g72(.a(x06), .O(new_n114_));
  nand2  g73(.a(new_n55_), .b(x19), .O(new_n115_));
  oai21  g74(.a(new_n55_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n44_), .O(new_n117_));
  nand3  g76(.a(x19), .b(x17), .c(x15), .O(new_n118_));
  aoi21  g77(.a(new_n118_), .b(new_n117_), .c(new_n74_), .O(z18));
  oai21  g78(.a(new_n43_), .b(new_n74_), .c(x17), .O(new_n120_));
  nand2  g79(.a(new_n120_), .b(x15), .O(new_n121_));
  nand4  g80(.a(new_n44_), .b(x10), .c(x08), .d(x07), .O(new_n122_));
  oai21  g81(.a(new_n56_), .b(new_n43_), .c(new_n122_), .O(new_n123_));
  nand2  g82(.a(new_n123_), .b(x12), .O(new_n124_));
  nand2  g83(.a(new_n124_), .b(new_n121_), .O(z19));
endmodule


