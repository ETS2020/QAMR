// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n58_, new_n61_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x09), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x09), .O(new_n46_));
  nand3  g05(.a(x10), .b(new_n46_), .c(x08), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n45_), .c(x14), .O(z01));
  inv1   g07(.a(x08), .O(new_n49_));
  inv1   g08(.a(x14), .O(new_n50_));
  nand2  g09(.a(new_n45_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n49_), .c(new_n50_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n43_), .O(z02));
  inv1   g13(.a(x12), .O(new_n55_));
  nor4   g14(.a(new_n47_), .b(new_n45_), .c(x14), .d(new_n55_), .O(z03));
  nand2  g15(.a(new_n43_), .b(x18), .O(z04));
  inv1   g16(.a(x19), .O(new_n58_));
  nand2  g17(.a(new_n43_), .b(new_n58_), .O(z05));
  and2   g18(.a(new_n43_), .b(x17), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nand2  g20(.a(new_n43_), .b(new_n61_), .O(z08));
  nor3   g21(.a(x15), .b(new_n46_), .c(new_n49_), .O(z09));
  oai21  g22(.a(x15), .b(x08), .c(x09), .O(z10));
  aoi21  g23(.a(x15), .b(x09), .c(new_n50_), .O(z11));
  aoi21  g24(.a(x13), .b(x12), .c(x09), .O(new_n66_));
  inv1   g25(.a(x10), .O(new_n67_));
  nor2   g26(.a(x15), .b(new_n67_), .O(new_n68_));
  nand2  g27(.a(x10), .b(x08), .O(new_n69_));
  and2   g28(.a(x08), .b(x00), .O(new_n70_));
  aoi22  g29(.a(new_n70_), .b(new_n68_), .c(new_n69_), .d(x13), .O(new_n71_));
  oai22  g30(.a(new_n71_), .b(new_n55_), .c(new_n66_), .d(new_n45_), .O(z12));
  nand4  g31(.a(new_n50_), .b(x10), .c(new_n46_), .d(x08), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n50_), .c(new_n45_), .O(new_n74_));
  inv1   g33(.a(new_n69_), .O(new_n75_));
  aoi21  g34(.a(new_n75_), .b(x01), .c(new_n50_), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n74_), .c(x12), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n43_), .O(z13));
  and2   g37(.a(x08), .b(x02), .O(new_n79_));
  aoi22  g38(.a(new_n79_), .b(new_n68_), .c(new_n69_), .d(x15), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n55_), .c(new_n43_), .O(z14));
  nand3  g40(.a(x16), .b(new_n50_), .c(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n46_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x15), .O(new_n84_));
  nand3  g43(.a(new_n69_), .b(x16), .c(new_n50_), .O(new_n85_));
  nand2  g44(.a(x08), .b(x03), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n51_), .c(new_n85_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n84_), .O(z15));
  inv1   g48(.a(x04), .O(new_n90_));
  nand2  g49(.a(new_n69_), .b(x17), .O(new_n91_));
  oai21  g50(.a(new_n69_), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n45_), .O(new_n93_));
  nand3  g52(.a(x17), .b(x15), .c(new_n46_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n55_), .O(z16));
  inv1   g54(.a(x05), .O(new_n96_));
  nand2  g55(.a(new_n69_), .b(x18), .O(new_n97_));
  oai21  g56(.a(new_n69_), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  nand3  g58(.a(x18), .b(x15), .c(new_n46_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n55_), .O(z17));
  inv1   g60(.a(x06), .O(new_n102_));
  nand2  g61(.a(new_n69_), .b(x19), .O(new_n103_));
  oai21  g62(.a(new_n69_), .b(new_n102_), .c(new_n103_), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n45_), .O(new_n105_));
  nand3  g64(.a(x19), .b(x15), .c(new_n46_), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n105_), .c(new_n55_), .O(z18));
  aoi21  g66(.a(x20), .b(x12), .c(x09), .O(new_n108_));
  and2   g67(.a(x08), .b(x07), .O(new_n109_));
  aoi22  g68(.a(new_n109_), .b(new_n68_), .c(new_n69_), .d(x20), .O(new_n110_));
  oai22  g69(.a(new_n110_), .b(new_n55_), .c(new_n108_), .d(new_n45_), .O(z19));
  buf    g70(.a(x15), .O(z06));
endmodule


