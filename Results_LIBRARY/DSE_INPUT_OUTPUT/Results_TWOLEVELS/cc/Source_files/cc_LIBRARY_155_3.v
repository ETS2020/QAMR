// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x18), .O(z04));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(z04), .c(new_n45_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(new_n43_), .b(x14), .O(new_n52_));
  nand2  g11(.a(new_n45_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand4  g15(.a(new_n47_), .b(x12), .c(x10), .d(x08), .O(new_n57_));
  aoi21  g16(.a(new_n57_), .b(z04), .c(new_n45_), .O(z03));
  and2   g17(.a(new_n43_), .b(x19), .O(z05));
  and2   g18(.a(new_n43_), .b(x17), .O(z07));
  and2   g19(.a(new_n43_), .b(x16), .O(z08));
  nand2  g20(.a(x09), .b(x08), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n43_), .O(z09));
  nand3  g22(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  inv1   g23(.a(new_n52_), .O(z11));
  inv1   g24(.a(x12), .O(new_n66_));
  aoi21  g25(.a(x10), .b(x08), .c(x15), .O(new_n67_));
  nor2   g26(.a(x18), .b(new_n45_), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n67_), .c(x13), .O(new_n69_));
  nand4  g28(.a(new_n45_), .b(x10), .c(x08), .d(x00), .O(new_n70_));
  aoi21  g29(.a(new_n70_), .b(new_n69_), .c(new_n66_), .O(z12));
  aoi21  g30(.a(x10), .b(x08), .c(x14), .O(new_n72_));
  nor2   g31(.a(new_n72_), .b(new_n45_), .O(new_n73_));
  nand3  g32(.a(x10), .b(x08), .c(x01), .O(new_n74_));
  and2   g33(.a(new_n74_), .b(x14), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n73_), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n43_), .O(z13));
  nand2  g36(.a(x10), .b(x08), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x15), .O(new_n79_));
  nand2  g38(.a(x08), .b(x02), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n53_), .c(new_n79_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x12), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n43_), .O(z14));
  nand3  g42(.a(x16), .b(new_n47_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(z04), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x15), .O(new_n86_));
  nand3  g45(.a(new_n78_), .b(x16), .c(new_n47_), .O(new_n87_));
  nand4  g46(.a(new_n45_), .b(x10), .c(x08), .d(x03), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n86_), .O(z15));
  nand2  g50(.a(new_n78_), .b(x17), .O(new_n92_));
  nand3  g51(.a(x10), .b(x08), .c(x04), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n45_), .O(new_n95_));
  nand3  g54(.a(z04), .b(x17), .c(x15), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n66_), .O(z16));
  inv1   g56(.a(x05), .O(new_n98_));
  nand2  g57(.a(new_n78_), .b(x18), .O(new_n99_));
  oai21  g58(.a(new_n78_), .b(new_n98_), .c(new_n99_), .O(new_n100_));
  nand3  g59(.a(new_n100_), .b(new_n45_), .c(x12), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z17));
  nand2  g61(.a(new_n78_), .b(x19), .O(new_n103_));
  nand3  g62(.a(x10), .b(x08), .c(x06), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n45_), .O(new_n106_));
  nand3  g65(.a(x19), .b(z04), .c(x15), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n66_), .O(z18));
  nand2  g67(.a(new_n78_), .b(x20), .O(new_n109_));
  nand3  g68(.a(x10), .b(x08), .c(x07), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n45_), .O(new_n112_));
  nand3  g71(.a(x20), .b(z04), .c(x15), .O(new_n113_));
  aoi21  g72(.a(new_n113_), .b(new_n112_), .c(new_n66_), .O(z19));
  buf    g73(.a(x15), .O(z06));
endmodule


