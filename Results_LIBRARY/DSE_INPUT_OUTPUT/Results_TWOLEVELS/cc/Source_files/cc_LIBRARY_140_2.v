// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x15), .O(new_n43_));
  nor2   g02(.a(x19), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(x19), .c(new_n43_), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(new_n43_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n45_), .O(z02));
  nand2  g13(.a(x10), .b(x08), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(x19), .c(x15), .d(new_n47_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  nand2  g19(.a(new_n45_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(new_n43_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nor2   g23(.a(new_n44_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n45_), .b(new_n66_), .O(z08));
  inv1   g26(.a(x09), .O(new_n68_));
  nor3   g27(.a(new_n44_), .b(new_n68_), .c(new_n50_), .O(z09));
  nand3  g28(.a(new_n45_), .b(x09), .c(x08), .O(z10));
  nor2   g29(.a(new_n44_), .b(new_n47_), .O(z11));
  inv1   g30(.a(x12), .O(new_n72_));
  aoi21  g31(.a(x13), .b(x12), .c(new_n62_), .O(new_n73_));
  inv1   g32(.a(x10), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n74_), .O(new_n75_));
  and2   g34(.a(x08), .b(x00), .O(new_n76_));
  aoi22  g35(.a(new_n76_), .b(new_n75_), .c(new_n55_), .d(x13), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n72_), .c(new_n73_), .d(new_n43_), .O(z12));
  nand2  g37(.a(new_n48_), .b(new_n47_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x19), .c(x15), .O(new_n80_));
  nand2  g39(.a(new_n56_), .b(x01), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(new_n43_), .c(x14), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n80_), .c(new_n72_), .O(z13));
  nand3  g42(.a(new_n55_), .b(x19), .c(x15), .O(new_n84_));
  nand3  g43(.a(new_n75_), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n84_), .c(new_n72_), .O(z14));
  nand3  g45(.a(x16), .b(new_n47_), .c(x12), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x19), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(x15), .O(new_n89_));
  nand3  g48(.a(new_n55_), .b(x16), .c(new_n47_), .O(new_n90_));
  nand2  g49(.a(x08), .b(x03), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n51_), .c(new_n90_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x12), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n89_), .O(z15));
  inv1   g53(.a(x04), .O(new_n95_));
  nand2  g54(.a(new_n55_), .b(x17), .O(new_n96_));
  oai21  g55(.a(new_n55_), .b(new_n95_), .c(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n43_), .O(new_n98_));
  nand3  g57(.a(x19), .b(x17), .c(x15), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n98_), .c(new_n72_), .O(z16));
  inv1   g59(.a(x05), .O(new_n101_));
  nand2  g60(.a(new_n55_), .b(x18), .O(new_n102_));
  oai21  g61(.a(new_n55_), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n43_), .O(new_n104_));
  nand3  g63(.a(x19), .b(x18), .c(x15), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n72_), .O(z17));
  inv1   g65(.a(x06), .O(new_n107_));
  nand2  g66(.a(x19), .b(x15), .O(new_n108_));
  oai21  g67(.a(new_n108_), .b(x14), .c(new_n107_), .O(new_n109_));
  nand3  g68(.a(new_n109_), .b(x10), .c(x08), .O(new_n110_));
  nand2  g69(.a(x15), .b(x14), .O(new_n111_));
  nand3  g70(.a(new_n111_), .b(x10), .c(x08), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(x19), .O(new_n113_));
  nand2  g72(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(x12), .O(new_n115_));
  nand2  g74(.a(new_n115_), .b(new_n45_), .O(z18));
  inv1   g75(.a(x07), .O(new_n117_));
  nand2  g76(.a(new_n55_), .b(x20), .O(new_n118_));
  oai21  g77(.a(new_n55_), .b(new_n117_), .c(new_n118_), .O(new_n119_));
  nand2  g78(.a(new_n119_), .b(new_n43_), .O(new_n120_));
  nand3  g79(.a(x20), .b(x19), .c(x15), .O(new_n121_));
  aoi21  g80(.a(new_n121_), .b(new_n120_), .c(new_n72_), .O(z19));
  buf    g81(.a(x15), .O(z06));
endmodule


