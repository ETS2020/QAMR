// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x10), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  nand2  g05(.a(x10), .b(x08), .O(new_n47_));
  inv1   g06(.a(x14), .O(new_n48_));
  nand2  g07(.a(x15), .b(new_n48_), .O(new_n49_));
  oai21  g08(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(z01));
  inv1   g09(.a(x10), .O(new_n51_));
  inv1   g10(.a(x15), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(x08), .c(x14), .O(new_n53_));
  nand2  g12(.a(x15), .b(x14), .O(new_n54_));
  oai21  g13(.a(new_n53_), .b(new_n51_), .c(new_n54_), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(x12), .c(new_n42_), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z02));
  inv1   g16(.a(x12), .O(new_n58_));
  nor4   g17(.a(new_n47_), .b(new_n52_), .c(x14), .d(new_n58_), .O(z03));
  nor2   g18(.a(new_n44_), .b(x18), .O(z04));
  inv1   g19(.a(x19), .O(new_n61_));
  nor2   g20(.a(new_n44_), .b(new_n61_), .O(z05));
  inv1   g21(.a(x09), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(x08), .O(new_n64_));
  aoi21  g23(.a(new_n64_), .b(x08), .c(new_n52_), .O(z06));
  inv1   g24(.a(x17), .O(new_n66_));
  nor2   g25(.a(new_n44_), .b(new_n66_), .O(z07));
  inv1   g26(.a(x16), .O(new_n68_));
  nand2  g27(.a(new_n46_), .b(new_n68_), .O(z08));
  inv1   g28(.a(x08), .O(new_n70_));
  nor3   g29(.a(new_n44_), .b(new_n63_), .c(new_n70_), .O(z09));
  aoi21  g30(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nand2  g31(.a(new_n46_), .b(new_n48_), .O(z11));
  inv1   g32(.a(x13), .O(new_n74_));
  aoi21  g33(.a(new_n52_), .b(x08), .c(new_n74_), .O(new_n75_));
  nand3  g34(.a(new_n52_), .b(x08), .c(x00), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n75_), .c(x12), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n46_), .O(z12));
  oai21  g38(.a(new_n51_), .b(new_n70_), .c(new_n48_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x15), .O(new_n81_));
  nand2  g40(.a(x08), .b(x01), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x14), .c(x10), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n81_), .c(new_n58_), .O(z13));
  oai21  g43(.a(new_n52_), .b(x12), .c(new_n51_), .O(new_n85_));
  inv1   g44(.a(x02), .O(new_n86_));
  nand2  g45(.a(new_n52_), .b(x08), .O(new_n87_));
  nand2  g46(.a(x15), .b(new_n70_), .O(new_n88_));
  oai21  g47(.a(new_n87_), .b(new_n86_), .c(new_n88_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x12), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(new_n85_), .O(z14));
  nand3  g50(.a(new_n52_), .b(x08), .c(x03), .O(new_n92_));
  nand3  g51(.a(x16), .b(new_n48_), .c(new_n70_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x10), .O(new_n95_));
  nand3  g54(.a(x16), .b(x15), .c(new_n48_), .O(new_n96_));
  aoi21  g55(.a(new_n96_), .b(new_n95_), .c(new_n58_), .O(z15));
  nand3  g56(.a(x12), .b(x08), .c(x04), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(x10), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n52_), .O(new_n100_));
  nand3  g59(.a(new_n87_), .b(x17), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(z16));
  nand3  g61(.a(x12), .b(x08), .c(x05), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x10), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n52_), .O(new_n105_));
  nand3  g64(.a(new_n87_), .b(x18), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(z17));
  inv1   g66(.a(x06), .O(new_n108_));
  nand2  g67(.a(x19), .b(new_n70_), .O(new_n109_));
  oai21  g68(.a(new_n87_), .b(new_n108_), .c(new_n109_), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(x10), .O(new_n111_));
  nand2  g70(.a(x19), .b(x15), .O(new_n112_));
  aoi21  g71(.a(new_n112_), .b(new_n111_), .c(new_n58_), .O(z18));
  inv1   g72(.a(x07), .O(new_n114_));
  nand2  g73(.a(x20), .b(new_n70_), .O(new_n115_));
  oai21  g74(.a(new_n87_), .b(new_n114_), .c(new_n115_), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(x10), .O(new_n117_));
  nand2  g76(.a(x20), .b(x15), .O(new_n118_));
  aoi21  g77(.a(new_n118_), .b(new_n117_), .c(new_n58_), .O(z19));
endmodule


