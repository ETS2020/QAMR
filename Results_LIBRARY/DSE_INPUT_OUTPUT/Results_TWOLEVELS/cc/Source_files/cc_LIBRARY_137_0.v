// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x15), .b(x01), .O(new_n44_));
  nor3   g03(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x15), .c(new_n47_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(z01));
  inv1   g10(.a(x01), .O(new_n52_));
  inv1   g11(.a(x15), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x10), .c(x08), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(new_n47_), .c(new_n52_), .O(new_n55_));
  nor2   g14(.a(new_n53_), .b(new_n47_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n55_), .c(x12), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x11), .O(z02));
  nand3  g17(.a(x15), .b(new_n47_), .c(x12), .O(new_n59_));
  oai21  g18(.a(new_n59_), .b(new_n48_), .c(new_n46_), .O(z03));
  nor2   g19(.a(new_n44_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n46_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x08), .O(new_n64_));
  nand2  g23(.a(x09), .b(new_n64_), .O(new_n65_));
  oai21  g24(.a(new_n65_), .b(new_n64_), .c(x15), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n46_), .O(z06));
  inv1   g26(.a(x17), .O(new_n68_));
  nand2  g27(.a(new_n46_), .b(new_n68_), .O(z07));
  inv1   g28(.a(x16), .O(new_n70_));
  nand2  g29(.a(new_n46_), .b(new_n70_), .O(z08));
  inv1   g30(.a(x09), .O(new_n72_));
  nor3   g31(.a(new_n44_), .b(new_n72_), .c(new_n64_), .O(z09));
  aoi21  g32(.a(x09), .b(x08), .c(new_n44_), .O(z10));
  nand2  g33(.a(new_n46_), .b(new_n47_), .O(z11));
  and2   g34(.a(x12), .b(x10), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(x08), .c(x00), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x01), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n53_), .O(new_n79_));
  nand3  g38(.a(new_n54_), .b(x13), .c(x12), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(new_n79_), .O(z12));
  inv1   g40(.a(x12), .O(new_n82_));
  oai21  g41(.a(new_n49_), .b(new_n52_), .c(new_n53_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x14), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n50_), .c(new_n82_), .O(z13));
  nand2  g44(.a(new_n48_), .b(x15), .O(new_n86_));
  inv1   g45(.a(new_n54_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x02), .c(x01), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n86_), .c(new_n82_), .O(z14));
  nand3  g48(.a(new_n48_), .b(x16), .c(new_n47_), .O(new_n90_));
  nand4  g49(.a(new_n53_), .b(x10), .c(x08), .d(x03), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x01), .O(new_n93_));
  nand3  g52(.a(x16), .b(x15), .c(new_n47_), .O(new_n94_));
  aoi21  g53(.a(new_n94_), .b(new_n93_), .c(new_n82_), .O(z15));
  nand4  g54(.a(new_n53_), .b(x10), .c(x08), .d(x04), .O(new_n96_));
  oai21  g55(.a(new_n49_), .b(new_n68_), .c(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x01), .O(new_n98_));
  nand2  g57(.a(x17), .b(x15), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n98_), .c(new_n82_), .O(z16));
  nand3  g59(.a(new_n76_), .b(x08), .c(x05), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(x01), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n53_), .O(new_n103_));
  nand3  g62(.a(new_n54_), .b(x18), .c(x12), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n103_), .O(z17));
  nand3  g64(.a(new_n76_), .b(x08), .c(x06), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(x01), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(new_n53_), .O(new_n108_));
  nand3  g67(.a(new_n54_), .b(x19), .c(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(z18));
  nand3  g69(.a(new_n76_), .b(x08), .c(x07), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(x01), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n53_), .O(new_n113_));
  nand3  g72(.a(new_n54_), .b(x20), .c(x12), .O(new_n114_));
  nand2  g73(.a(new_n114_), .b(new_n113_), .O(z19));
endmodule


