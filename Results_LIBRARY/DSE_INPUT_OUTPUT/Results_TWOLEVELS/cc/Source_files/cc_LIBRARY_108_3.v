// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n59_, new_n61_, new_n63_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x19), .b(x16), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  nand2  g03(.a(x10), .b(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x15), .b(new_n46_), .O(new_n47_));
  oai21  g06(.a(new_n47_), .b(new_n45_), .c(new_n43_), .O(z01));
  inv1   g07(.a(x15), .O(new_n49_));
  nand3  g08(.a(new_n49_), .b(x10), .c(x08), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x12), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(x11), .c(new_n43_), .O(z02));
  nand3  g12(.a(x15), .b(new_n46_), .c(x12), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(new_n45_), .c(new_n43_), .O(z03));
  inv1   g14(.a(new_n43_), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(x18), .O(z04));
  nand2  g16(.a(new_n43_), .b(new_n49_), .O(z06));
  inv1   g17(.a(x17), .O(new_n59_));
  nand2  g18(.a(new_n43_), .b(new_n59_), .O(z07));
  inv1   g19(.a(x16), .O(new_n61_));
  nor2   g20(.a(x19), .b(new_n61_), .O(z08));
  nand2  g21(.a(x09), .b(x08), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(new_n43_), .O(z09));
  nand3  g23(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nor2   g24(.a(new_n56_), .b(new_n46_), .O(z11));
  inv1   g25(.a(x12), .O(new_n67_));
  inv1   g26(.a(x10), .O(new_n68_));
  nor2   g27(.a(x15), .b(new_n68_), .O(new_n69_));
  and2   g28(.a(x08), .b(x00), .O(new_n70_));
  aoi22  g29(.a(new_n70_), .b(new_n69_), .c(new_n50_), .d(x13), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n67_), .c(new_n43_), .O(z12));
  nand3  g31(.a(x10), .b(x08), .c(x01), .O(new_n73_));
  nand3  g32(.a(new_n73_), .b(new_n43_), .c(x14), .O(new_n74_));
  nand2  g33(.a(x19), .b(x16), .O(new_n75_));
  nand4  g34(.a(new_n75_), .b(x15), .c(x10), .d(x08), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n74_), .c(new_n67_), .O(z13));
  nand2  g36(.a(new_n45_), .b(x15), .O(new_n78_));
  nand3  g37(.a(new_n69_), .b(x08), .c(x02), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n43_), .O(z14));
  nand3  g41(.a(new_n50_), .b(x16), .c(new_n46_), .O(new_n83_));
  nand3  g42(.a(new_n69_), .b(x08), .c(x03), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n43_), .O(z15));
  and2   g46(.a(x08), .b(x04), .O(new_n88_));
  aoi22  g47(.a(new_n88_), .b(new_n69_), .c(new_n50_), .d(x17), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n67_), .c(new_n43_), .O(z16));
  nand2  g49(.a(new_n50_), .b(x18), .O(new_n91_));
  nand3  g50(.a(new_n69_), .b(x08), .c(x05), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(new_n43_), .c(x12), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z17));
  nand2  g54(.a(new_n49_), .b(x06), .O(new_n96_));
  nand3  g55(.a(x19), .b(new_n61_), .c(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n68_), .O(new_n98_));
  and2   g57(.a(new_n45_), .b(x19), .O(new_n99_));
  aoi21  g58(.a(new_n98_), .b(x08), .c(new_n99_), .O(new_n100_));
  oai21  g59(.a(new_n100_), .b(new_n67_), .c(new_n43_), .O(z18));
  nand2  g60(.a(new_n50_), .b(x20), .O(new_n102_));
  nand3  g61(.a(new_n69_), .b(x08), .c(x07), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g63(.a(new_n104_), .b(new_n43_), .c(x12), .O(new_n105_));
  inv1   g64(.a(new_n105_), .O(z19));
  buf    g65(.a(x19), .O(z05));
endmodule


