// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n59_, new_n61_,
    new_n62_, new_n64_, new_n66_, new_n68_, new_n69_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_;
  nand2  g00(.a(x15), .b(x03), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x03), .O(new_n46_));
  nand3  g05(.a(x10), .b(x08), .c(new_n46_), .O(new_n47_));
  nor3   g06(.a(new_n47_), .b(new_n45_), .c(x14), .O(z01));
  inv1   g07(.a(x11), .O(new_n49_));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(new_n42_), .b(x14), .O(new_n51_));
  nand2  g10(.a(new_n45_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n50_), .c(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n49_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  nand4  g14(.a(x12), .b(x10), .c(x08), .d(new_n46_), .O(new_n56_));
  nor3   g15(.a(new_n56_), .b(new_n45_), .c(x14), .O(z03));
  aoi21  g16(.a(x15), .b(x03), .c(x18), .O(z04));
  inv1   g17(.a(x19), .O(new_n59_));
  nand2  g18(.a(new_n42_), .b(new_n59_), .O(z05));
  nand2  g19(.a(x09), .b(new_n50_), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(new_n50_), .c(x15), .O(new_n62_));
  nor2   g21(.a(new_n62_), .b(x03), .O(z06));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n42_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nand2  g25(.a(new_n42_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x15), .b(x03), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(x09), .c(x08), .O(new_n69_));
  inv1   g28(.a(new_n69_), .O(z09));
  aoi22  g29(.a(x15), .b(x03), .c(x09), .d(x08), .O(z10));
  inv1   g30(.a(x14), .O(new_n72_));
  nand2  g31(.a(new_n42_), .b(new_n72_), .O(z11));
  inv1   g32(.a(x12), .O(new_n74_));
  nand2  g33(.a(x15), .b(new_n46_), .O(new_n75_));
  nand2  g34(.a(x10), .b(x08), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n45_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x13), .O(new_n79_));
  inv1   g38(.a(new_n52_), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(x08), .c(x00), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n79_), .c(new_n74_), .O(z12));
  inv1   g41(.a(x10), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n50_), .c(new_n72_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x15), .c(new_n46_), .O(new_n85_));
  nand3  g44(.a(x10), .b(x08), .c(x01), .O(new_n86_));
  nand3  g45(.a(new_n86_), .b(new_n45_), .c(x14), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(new_n85_), .c(new_n74_), .O(z13));
  nand3  g47(.a(new_n76_), .b(x15), .c(new_n46_), .O(new_n89_));
  nand3  g48(.a(new_n80_), .b(x08), .c(x02), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n74_), .O(z14));
  nand3  g50(.a(new_n78_), .b(x16), .c(new_n72_), .O(new_n92_));
  nand3  g51(.a(new_n80_), .b(x08), .c(x03), .O(new_n93_));
  aoi21  g52(.a(new_n93_), .b(new_n92_), .c(new_n74_), .O(z15));
  oai21  g53(.a(new_n64_), .b(new_n74_), .c(new_n46_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x15), .O(new_n96_));
  nand2  g55(.a(new_n76_), .b(x17), .O(new_n97_));
  nand2  g56(.a(x08), .b(x04), .O(new_n98_));
  oai21  g57(.a(new_n98_), .b(new_n52_), .c(new_n97_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(x12), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n96_), .O(z16));
  nand2  g60(.a(x18), .b(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n46_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x15), .O(new_n104_));
  nand2  g63(.a(new_n76_), .b(x18), .O(new_n105_));
  nand2  g64(.a(x08), .b(x05), .O(new_n106_));
  oai21  g65(.a(new_n106_), .b(new_n52_), .c(new_n105_), .O(new_n107_));
  nand2  g66(.a(new_n107_), .b(x12), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n104_), .O(z17));
  nand2  g68(.a(new_n76_), .b(x19), .O(new_n110_));
  nand3  g69(.a(x10), .b(x08), .c(x06), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand2  g71(.a(new_n112_), .b(new_n45_), .O(new_n113_));
  nand3  g72(.a(x19), .b(x15), .c(new_n46_), .O(new_n114_));
  aoi21  g73(.a(new_n114_), .b(new_n113_), .c(new_n74_), .O(z18));
  nand2  g74(.a(x20), .b(x12), .O(new_n116_));
  nand2  g75(.a(new_n116_), .b(new_n46_), .O(new_n117_));
  nand2  g76(.a(new_n117_), .b(x15), .O(new_n118_));
  nand2  g77(.a(new_n76_), .b(x20), .O(new_n119_));
  nand2  g78(.a(x08), .b(x07), .O(new_n120_));
  oai21  g79(.a(new_n120_), .b(new_n52_), .c(new_n119_), .O(new_n121_));
  nand2  g80(.a(new_n121_), .b(x12), .O(new_n122_));
  nand2  g81(.a(new_n122_), .b(new_n118_), .O(z19));
endmodule


