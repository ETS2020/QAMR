// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n60_, new_n61_,
    new_n64_, new_n66_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_;
  nand2  g00(.a(x15), .b(x05), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x05), .O(new_n45_));
  inv1   g04(.a(x15), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n45_), .c(new_n46_), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  nand2  g09(.a(new_n46_), .b(x10), .O(new_n51_));
  oai21  g10(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(x12), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(x11), .c(new_n42_), .O(z02));
  nand4  g13(.a(x12), .b(x10), .c(x08), .d(new_n45_), .O(new_n55_));
  nor3   g14(.a(new_n55_), .b(new_n46_), .c(x14), .O(z03));
  inv1   g15(.a(new_n42_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x18), .O(z04));
  and2   g17(.a(new_n42_), .b(x19), .O(z05));
  nand2  g18(.a(x09), .b(new_n50_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n50_), .O(new_n61_));
  aoi21  g20(.a(new_n61_), .b(new_n45_), .c(new_n46_), .O(z06));
  and2   g21(.a(new_n42_), .b(x17), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n42_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x15), .b(x05), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  aoi22  g27(.a(x15), .b(x05), .c(x09), .d(x08), .O(z10));
  nor2   g28(.a(new_n57_), .b(new_n47_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  nand2  g30(.a(x15), .b(new_n45_), .O(new_n72_));
  nand2  g31(.a(x10), .b(x08), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(x15), .c(new_n72_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x13), .O(new_n76_));
  inv1   g35(.a(x10), .O(new_n77_));
  nor2   g36(.a(x15), .b(new_n77_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(x08), .c(x00), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n76_), .c(new_n71_), .O(z12));
  nand4  g39(.a(new_n47_), .b(x10), .c(x08), .d(new_n45_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n47_), .c(new_n46_), .O(new_n82_));
  aoi21  g41(.a(new_n74_), .b(x01), .c(new_n47_), .O(new_n83_));
  oai21  g42(.a(new_n83_), .b(new_n82_), .c(x12), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n42_), .O(z13));
  and2   g44(.a(x08), .b(x02), .O(new_n86_));
  aoi22  g45(.a(new_n86_), .b(new_n78_), .c(new_n73_), .d(x15), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n71_), .c(new_n42_), .O(z14));
  nand3  g47(.a(new_n75_), .b(x16), .c(new_n47_), .O(new_n89_));
  nand3  g48(.a(new_n78_), .b(x08), .c(x03), .O(new_n90_));
  aoi21  g49(.a(new_n90_), .b(new_n89_), .c(new_n71_), .O(z15));
  aoi21  g50(.a(x17), .b(x12), .c(x05), .O(new_n92_));
  and2   g51(.a(x08), .b(x04), .O(new_n93_));
  aoi22  g52(.a(new_n93_), .b(new_n78_), .c(new_n73_), .d(x17), .O(new_n94_));
  oai22  g53(.a(new_n94_), .b(new_n71_), .c(new_n92_), .d(new_n46_), .O(z16));
  nand2  g54(.a(new_n73_), .b(x18), .O(new_n96_));
  oai21  g55(.a(new_n73_), .b(new_n45_), .c(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n46_), .O(new_n98_));
  nand3  g57(.a(x18), .b(x15), .c(new_n45_), .O(new_n99_));
  aoi21  g58(.a(new_n99_), .b(new_n98_), .c(new_n71_), .O(z17));
  inv1   g59(.a(x06), .O(new_n101_));
  nand2  g60(.a(new_n73_), .b(x19), .O(new_n102_));
  oai21  g61(.a(new_n73_), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n46_), .O(new_n104_));
  nand3  g63(.a(x19), .b(x15), .c(new_n45_), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n71_), .O(z18));
  aoi21  g65(.a(x20), .b(x12), .c(x05), .O(new_n107_));
  and2   g66(.a(x08), .b(x07), .O(new_n108_));
  aoi22  g67(.a(new_n108_), .b(new_n78_), .c(new_n73_), .d(x20), .O(new_n109_));
  oai22  g68(.a(new_n109_), .b(new_n71_), .c(new_n107_), .d(new_n46_), .O(z19));
endmodule


