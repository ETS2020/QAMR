// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n63_, new_n65_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_;
  nand2  g00(.a(x17), .b(x15), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x17), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x10), .c(x08), .O(new_n48_));
  aoi21  g07(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x08), .O(new_n51_));
  nand2  g10(.a(new_n42_), .b(x14), .O(new_n52_));
  nand2  g11(.a(new_n45_), .b(x10), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n51_), .c(new_n52_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand3  g15(.a(x12), .b(x10), .c(x08), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n46_), .c(x15), .d(new_n47_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  nand2  g19(.a(new_n42_), .b(x18), .O(z04));
  and2   g20(.a(new_n42_), .b(x19), .O(z05));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n42_), .b(new_n63_), .O(z08));
  nand3  g23(.a(new_n42_), .b(x09), .c(x08), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(z09));
  nand3  g25(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  inv1   g26(.a(new_n52_), .O(z11));
  inv1   g27(.a(x12), .O(new_n69_));
  nand2  g28(.a(x10), .b(x08), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n45_), .O(new_n71_));
  nand2  g30(.a(new_n46_), .b(x15), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x13), .O(new_n74_));
  inv1   g33(.a(new_n53_), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x08), .c(x00), .O(new_n76_));
  aoi21  g35(.a(new_n76_), .b(new_n74_), .c(new_n69_), .O(z12));
  oai21  g36(.a(new_n70_), .b(x17), .c(new_n47_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  aoi22  g38(.a(new_n79_), .b(x14), .c(new_n78_), .d(x15), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n69_), .c(new_n42_), .O(z13));
  nand3  g40(.a(new_n70_), .b(new_n46_), .c(x15), .O(new_n82_));
  nand3  g41(.a(new_n75_), .b(x08), .c(x02), .O(new_n83_));
  aoi21  g42(.a(new_n83_), .b(new_n82_), .c(new_n69_), .O(z14));
  nand3  g43(.a(new_n73_), .b(x16), .c(new_n47_), .O(new_n85_));
  nand3  g44(.a(new_n75_), .b(x08), .c(x03), .O(new_n86_));
  aoi21  g45(.a(new_n86_), .b(new_n85_), .c(new_n69_), .O(z15));
  inv1   g46(.a(x04), .O(new_n88_));
  nand2  g47(.a(new_n70_), .b(x17), .O(new_n89_));
  oai21  g48(.a(new_n70_), .b(new_n88_), .c(new_n89_), .O(new_n90_));
  nand3  g49(.a(new_n90_), .b(new_n45_), .c(x12), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z16));
  inv1   g51(.a(x05), .O(new_n93_));
  nand2  g52(.a(new_n70_), .b(x18), .O(new_n94_));
  oai21  g53(.a(new_n70_), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n45_), .O(new_n96_));
  nand3  g55(.a(x18), .b(new_n46_), .c(x15), .O(new_n97_));
  aoi21  g56(.a(new_n97_), .b(new_n96_), .c(new_n69_), .O(z17));
  aoi21  g57(.a(x19), .b(x12), .c(x17), .O(new_n99_));
  nand2  g58(.a(new_n70_), .b(x19), .O(new_n100_));
  nand2  g59(.a(x08), .b(x06), .O(new_n101_));
  oai21  g60(.a(new_n101_), .b(new_n53_), .c(new_n100_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(x12), .O(new_n103_));
  oai21  g62(.a(new_n99_), .b(new_n45_), .c(new_n103_), .O(z18));
  aoi21  g63(.a(x20), .b(x12), .c(x17), .O(new_n105_));
  nand2  g64(.a(new_n70_), .b(x20), .O(new_n106_));
  nand2  g65(.a(x08), .b(x07), .O(new_n107_));
  oai21  g66(.a(new_n107_), .b(new_n53_), .c(new_n106_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(x12), .O(new_n109_));
  oai21  g68(.a(new_n105_), .b(new_n45_), .c(new_n109_), .O(z19));
  buf    g69(.a(x15), .O(z06));
  buf    g70(.a(x17), .O(z07));
endmodule


