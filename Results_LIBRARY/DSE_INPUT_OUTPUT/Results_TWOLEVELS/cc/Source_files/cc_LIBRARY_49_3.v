// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x15), .b(x14), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x14), .O(new_n45_));
  nand2  g04(.a(x10), .b(x08), .O(new_n46_));
  inv1   g05(.a(new_n46_), .O(new_n47_));
  nand3  g06(.a(new_n47_), .b(x15), .c(new_n45_), .O(new_n48_));
  inv1   g07(.a(new_n48_), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x15), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(x10), .O(new_n52_));
  oai21  g11(.a(new_n52_), .b(new_n50_), .c(new_n45_), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(x12), .O(new_n54_));
  oai21  g13(.a(new_n54_), .b(x11), .c(new_n43_), .O(z02));
  aoi21  g14(.a(new_n47_), .b(x12), .c(x14), .O(new_n56_));
  nor2   g15(.a(new_n56_), .b(new_n51_), .O(z03));
  inv1   g16(.a(new_n43_), .O(new_n58_));
  nor2   g17(.a(new_n58_), .b(x18), .O(z04));
  and2   g18(.a(new_n43_), .b(x19), .O(z05));
  nor2   g19(.a(new_n51_), .b(x14), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nand2  g21(.a(new_n43_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nor2   g23(.a(new_n58_), .b(new_n64_), .O(z08));
  nand3  g24(.a(new_n43_), .b(x09), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z09));
  nand3  g26(.a(new_n43_), .b(x09), .c(x08), .O(z10));
  nor2   g27(.a(x15), .b(new_n45_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  aoi21  g29(.a(x10), .b(x08), .c(x15), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(z06), .c(x13), .O(new_n72_));
  nand4  g31(.a(new_n51_), .b(x10), .c(x08), .d(x00), .O(new_n73_));
  aoi21  g32(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(z12));
  nand2  g33(.a(new_n47_), .b(x01), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(new_n51_), .c(x14), .O(new_n76_));
  nand2  g35(.a(z06), .b(new_n47_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n76_), .c(new_n70_), .O(z13));
  nand2  g37(.a(x08), .b(x02), .O(new_n79_));
  oai22  g38(.a(new_n79_), .b(new_n52_), .c(new_n47_), .d(new_n51_), .O(new_n80_));
  nand2  g39(.a(new_n80_), .b(x12), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n43_), .O(z14));
  aoi21  g41(.a(new_n46_), .b(new_n45_), .c(x15), .O(new_n83_));
  nand4  g42(.a(new_n51_), .b(x10), .c(x08), .d(x03), .O(new_n84_));
  oai21  g43(.a(new_n83_), .b(new_n64_), .c(new_n84_), .O(new_n85_));
  nand2  g44(.a(new_n85_), .b(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n43_), .O(z15));
  aoi21  g46(.a(x17), .b(x12), .c(x14), .O(new_n88_));
  nand2  g47(.a(new_n46_), .b(x17), .O(new_n89_));
  nand2  g48(.a(x08), .b(x04), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n52_), .c(new_n89_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x12), .O(new_n92_));
  oai21  g51(.a(new_n88_), .b(new_n51_), .c(new_n92_), .O(z16));
  inv1   g52(.a(x05), .O(new_n94_));
  nand2  g53(.a(new_n46_), .b(x18), .O(new_n95_));
  oai21  g54(.a(new_n46_), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n51_), .O(new_n97_));
  nand3  g56(.a(x18), .b(x15), .c(new_n45_), .O(new_n98_));
  aoi21  g57(.a(new_n98_), .b(new_n97_), .c(new_n70_), .O(z17));
  inv1   g58(.a(x06), .O(new_n100_));
  nand2  g59(.a(new_n46_), .b(x19), .O(new_n101_));
  oai21  g60(.a(new_n46_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n51_), .O(new_n103_));
  nand3  g62(.a(x19), .b(x15), .c(new_n45_), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n70_), .O(z18));
  inv1   g64(.a(x07), .O(new_n106_));
  nand2  g65(.a(new_n46_), .b(x20), .O(new_n107_));
  oai21  g66(.a(new_n46_), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n51_), .O(new_n109_));
  nand3  g68(.a(x20), .b(x15), .c(new_n45_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n70_), .O(z19));
endmodule


