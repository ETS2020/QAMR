// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n58_, new_n60_, new_n62_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_;
  nand2  g00(.a(x15), .b(x12), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(x15), .O(new_n45_));
  inv1   g04(.a(x12), .O(new_n46_));
  inv1   g05(.a(x14), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(new_n46_), .c(x10), .d(x08), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  nand2  g09(.a(x10), .b(x08), .O(new_n51_));
  nand2  g10(.a(new_n51_), .b(new_n47_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  aoi21  g12(.a(new_n53_), .b(new_n45_), .c(new_n46_), .O(z02));
  aoi21  g13(.a(x15), .b(x12), .c(x18), .O(z04));
  and2   g14(.a(new_n42_), .b(x19), .O(z05));
  inv1   g15(.a(x17), .O(new_n58_));
  nand2  g16(.a(new_n42_), .b(new_n58_), .O(z07));
  inv1   g17(.a(x16), .O(new_n60_));
  nand2  g18(.a(new_n42_), .b(new_n60_), .O(z08));
  nand2  g19(.a(x09), .b(x08), .O(new_n62_));
  nand2  g20(.a(new_n62_), .b(new_n42_), .O(z09));
  nand3  g21(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  nand2  g22(.a(new_n42_), .b(new_n47_), .O(z11));
  nand2  g23(.a(new_n51_), .b(x13), .O(new_n66_));
  nand3  g24(.a(x10), .b(x08), .c(x00), .O(new_n67_));
  and2   g25(.a(new_n67_), .b(new_n45_), .O(new_n68_));
  aoi21  g26(.a(new_n68_), .b(new_n66_), .c(new_n46_), .O(z12));
  nand3  g27(.a(x10), .b(x08), .c(x01), .O(new_n70_));
  nand4  g28(.a(new_n70_), .b(new_n45_), .c(x14), .d(x12), .O(new_n71_));
  inv1   g29(.a(new_n71_), .O(z13));
  nand4  g30(.a(x12), .b(x10), .c(x08), .d(x02), .O(new_n73_));
  nor2   g31(.a(new_n73_), .b(x15), .O(z14));
  nand3  g32(.a(new_n51_), .b(x16), .c(new_n47_), .O(new_n75_));
  nand3  g33(.a(x10), .b(x08), .c(x03), .O(new_n76_));
  and2   g34(.a(new_n76_), .b(new_n45_), .O(new_n77_));
  aoi21  g35(.a(new_n77_), .b(new_n75_), .c(new_n46_), .O(z15));
  nand2  g36(.a(new_n51_), .b(x17), .O(new_n79_));
  nand3  g37(.a(x10), .b(x08), .c(x04), .O(new_n80_));
  and2   g38(.a(new_n80_), .b(new_n45_), .O(new_n81_));
  aoi21  g39(.a(new_n81_), .b(new_n79_), .c(new_n46_), .O(z16));
  nand2  g40(.a(new_n51_), .b(x18), .O(new_n83_));
  nand3  g41(.a(x10), .b(x08), .c(x05), .O(new_n84_));
  and2   g42(.a(new_n84_), .b(new_n45_), .O(new_n85_));
  aoi21  g43(.a(new_n85_), .b(new_n83_), .c(new_n46_), .O(z17));
  nand2  g44(.a(new_n51_), .b(x19), .O(new_n87_));
  nand3  g45(.a(x10), .b(x08), .c(x06), .O(new_n88_));
  and2   g46(.a(new_n88_), .b(new_n45_), .O(new_n89_));
  aoi21  g47(.a(new_n89_), .b(new_n87_), .c(new_n46_), .O(z18));
  nand2  g48(.a(new_n51_), .b(x20), .O(new_n91_));
  nand3  g49(.a(x10), .b(x08), .c(x07), .O(new_n92_));
  and2   g50(.a(new_n92_), .b(new_n45_), .O(new_n93_));
  aoi21  g51(.a(new_n93_), .b(new_n91_), .c(new_n46_), .O(z19));
  zero   g52(.O(z03));
  buf    g53(.a(x15), .O(z06));
endmodule


