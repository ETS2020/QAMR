// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_, new_n62_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_;
  and2   g00(.a(x20), .b(x11), .O(z00));
  inv1   g01(.a(x15), .O(new_n43_));
  inv1   g02(.a(x14), .O(new_n44_));
  nand3  g03(.a(new_n44_), .b(x10), .c(x08), .O(new_n45_));
  aoi21  g04(.a(new_n45_), .b(x11), .c(new_n43_), .O(z01));
  inv1   g05(.a(x11), .O(new_n47_));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nand2  g07(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n43_), .c(x12), .d(new_n47_), .O(new_n50_));
  inv1   g09(.a(new_n50_), .O(z02));
  inv1   g10(.a(new_n48_), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  nor3   g12(.a(new_n53_), .b(new_n43_), .c(x14), .O(z03));
  nor2   g13(.a(new_n43_), .b(x11), .O(new_n55_));
  nor2   g14(.a(new_n55_), .b(x18), .O(z04));
  inv1   g15(.a(x19), .O(new_n57_));
  inv1   g16(.a(new_n55_), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n57_), .O(z05));
  nand2  g18(.a(x15), .b(x11), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(z06));
  inv1   g20(.a(x17), .O(new_n62_));
  nor2   g21(.a(new_n55_), .b(new_n62_), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n58_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n58_), .O(z09));
  inv1   g26(.a(z09), .O(z10));
  nand2  g27(.a(new_n58_), .b(new_n44_), .O(z11));
  inv1   g28(.a(x12), .O(new_n70_));
  inv1   g29(.a(x13), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n70_), .c(x11), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x15), .O(new_n73_));
  nand4  g32(.a(new_n43_), .b(x10), .c(x08), .d(x00), .O(new_n74_));
  oai21  g33(.a(new_n52_), .b(new_n71_), .c(new_n74_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n73_), .O(z12));
  oai21  g36(.a(new_n48_), .b(new_n47_), .c(new_n44_), .O(new_n78_));
  nand3  g37(.a(x10), .b(x08), .c(x01), .O(new_n79_));
  aoi22  g38(.a(new_n79_), .b(x14), .c(new_n78_), .d(x15), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(new_n70_), .c(new_n58_), .O(z13));
  nand3  g40(.a(new_n48_), .b(x15), .c(x11), .O(new_n82_));
  inv1   g41(.a(x10), .O(new_n83_));
  nor2   g42(.a(x15), .b(new_n83_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(x08), .c(x02), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n82_), .c(new_n70_), .O(z14));
  oai21  g45(.a(new_n52_), .b(x15), .c(new_n60_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x16), .c(new_n44_), .O(new_n88_));
  nand3  g47(.a(new_n84_), .b(x08), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n70_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  nand2  g50(.a(new_n48_), .b(x17), .O(new_n92_));
  oai21  g51(.a(new_n48_), .b(new_n91_), .c(new_n92_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n43_), .O(new_n94_));
  nand3  g53(.a(x17), .b(x15), .c(x11), .O(new_n95_));
  aoi21  g54(.a(new_n95_), .b(new_n94_), .c(new_n70_), .O(z16));
  inv1   g55(.a(x05), .O(new_n97_));
  nand2  g56(.a(new_n48_), .b(x18), .O(new_n98_));
  oai21  g57(.a(new_n48_), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n43_), .O(new_n100_));
  nand3  g59(.a(x18), .b(x15), .c(x11), .O(new_n101_));
  aoi21  g60(.a(new_n101_), .b(new_n100_), .c(new_n70_), .O(z17));
  inv1   g61(.a(x06), .O(new_n103_));
  nand2  g62(.a(new_n48_), .b(x19), .O(new_n104_));
  oai21  g63(.a(new_n48_), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n43_), .O(new_n106_));
  nand3  g65(.a(x19), .b(x15), .c(x11), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n70_), .O(z18));
  inv1   g67(.a(x07), .O(new_n109_));
  nand2  g68(.a(new_n48_), .b(x20), .O(new_n110_));
  oai21  g69(.a(new_n48_), .b(new_n109_), .c(new_n110_), .O(new_n111_));
  nand2  g70(.a(new_n111_), .b(new_n43_), .O(new_n112_));
  nand3  g71(.a(x20), .b(x15), .c(x11), .O(new_n113_));
  aoi21  g72(.a(new_n113_), .b(new_n112_), .c(new_n70_), .O(z19));
endmodule


