// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n58_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_;
  nand2  g00(.a(x15), .b(x10), .O(new_n42_));
  nand3  g01(.a(new_n42_), .b(x20), .c(x11), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(z00));
  inv1   g03(.a(new_n42_), .O(z01));
  inv1   g04(.a(x11), .O(new_n46_));
  nand2  g05(.a(new_n42_), .b(x14), .O(new_n47_));
  inv1   g06(.a(x15), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x10), .c(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand3  g09(.a(new_n50_), .b(x12), .c(new_n46_), .O(new_n51_));
  inv1   g10(.a(new_n51_), .O(z02));
  nor2   g11(.a(z01), .b(x18), .O(z04));
  and2   g12(.a(new_n42_), .b(x19), .O(z05));
  nor2   g13(.a(new_n48_), .b(x10), .O(z06));
  and2   g14(.a(new_n42_), .b(x17), .O(z07));
  and2   g15(.a(new_n42_), .b(x16), .O(z08));
  nand3  g16(.a(new_n42_), .b(x09), .c(x08), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(z09));
  nand3  g18(.a(new_n42_), .b(x09), .c(x08), .O(z10));
  inv1   g19(.a(x14), .O(new_n61_));
  nand2  g20(.a(new_n42_), .b(new_n61_), .O(z11));
  inv1   g21(.a(x00), .O(new_n63_));
  nand2  g22(.a(x12), .b(x08), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n63_), .c(new_n48_), .O(new_n65_));
  nand2  g24(.a(new_n65_), .b(x10), .O(new_n66_));
  nand2  g25(.a(x10), .b(x08), .O(new_n67_));
  nand3  g26(.a(new_n67_), .b(x13), .c(x12), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n66_), .O(z12));
  inv1   g28(.a(x12), .O(new_n70_));
  inv1   g29(.a(x10), .O(new_n71_));
  aoi21  g30(.a(x08), .b(x01), .c(x15), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(x14), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n70_), .O(z13));
  inv1   g33(.a(x02), .O(new_n75_));
  oai21  g34(.a(new_n64_), .b(new_n75_), .c(new_n48_), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(x10), .O(new_n77_));
  oai21  g36(.a(new_n48_), .b(new_n70_), .c(new_n77_), .O(z14));
  oai21  g37(.a(x15), .b(x08), .c(x10), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(x16), .c(new_n61_), .O(new_n80_));
  nand4  g39(.a(new_n48_), .b(x10), .c(x08), .d(x03), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(new_n80_), .c(new_n70_), .O(z15));
  inv1   g41(.a(x04), .O(new_n83_));
  oai21  g42(.a(new_n64_), .b(new_n83_), .c(new_n48_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(x10), .O(new_n85_));
  nand3  g44(.a(new_n67_), .b(x17), .c(x12), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n85_), .O(z16));
  inv1   g46(.a(x05), .O(new_n88_));
  oai21  g47(.a(new_n64_), .b(new_n88_), .c(new_n48_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x10), .O(new_n90_));
  nand3  g49(.a(new_n67_), .b(x18), .c(x12), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n90_), .O(z17));
  inv1   g51(.a(x06), .O(new_n93_));
  oai21  g52(.a(new_n64_), .b(new_n93_), .c(new_n48_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x10), .O(new_n95_));
  nand3  g54(.a(new_n67_), .b(x19), .c(x12), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n95_), .O(z18));
  inv1   g56(.a(x20), .O(new_n98_));
  nand3  g57(.a(x10), .b(x08), .c(x07), .O(new_n99_));
  oai21  g58(.a(new_n98_), .b(x08), .c(new_n99_), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n48_), .O(new_n101_));
  nand2  g60(.a(x20), .b(new_n71_), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n101_), .c(new_n70_), .O(z19));
  inv1   g62(.a(new_n42_), .O(z03));
endmodule


