// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n62_, new_n64_, new_n66_, new_n68_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(x20), .b(new_n44_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  oai21  g05(.a(new_n43_), .b(new_n42_), .c(new_n46_), .O(z00));
  nand2  g06(.a(x10), .b(x08), .O(new_n48_));
  nor4   g07(.a(new_n48_), .b(new_n43_), .c(new_n44_), .d(x14), .O(z01));
  inv1   g08(.a(x08), .O(new_n50_));
  inv1   g09(.a(x14), .O(new_n51_));
  nand2  g10(.a(new_n44_), .b(x10), .O(new_n52_));
  oai22  g11(.a(new_n52_), .b(new_n50_), .c(new_n45_), .d(new_n51_), .O(new_n53_));
  nand3  g12(.a(new_n53_), .b(x12), .c(new_n42_), .O(new_n54_));
  inv1   g13(.a(new_n54_), .O(z02));
  inv1   g14(.a(new_n48_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(x20), .c(x15), .d(new_n51_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  nand2  g19(.a(new_n46_), .b(x18), .O(z04));
  inv1   g20(.a(x19), .O(new_n62_));
  nand2  g21(.a(new_n46_), .b(new_n62_), .O(z05));
  inv1   g22(.a(x17), .O(new_n64_));
  nand2  g23(.a(new_n46_), .b(new_n64_), .O(z07));
  inv1   g24(.a(x16), .O(new_n66_));
  nor2   g25(.a(new_n45_), .b(new_n66_), .O(z08));
  nand2  g26(.a(x09), .b(x08), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n46_), .O(z09));
  nand3  g28(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nand2  g29(.a(new_n46_), .b(new_n51_), .O(z11));
  inv1   g30(.a(x12), .O(new_n72_));
  aoi21  g31(.a(x13), .b(x12), .c(new_n43_), .O(new_n73_));
  inv1   g32(.a(x10), .O(new_n74_));
  nor2   g33(.a(x15), .b(new_n74_), .O(new_n75_));
  and2   g34(.a(x08), .b(x00), .O(new_n76_));
  aoi22  g35(.a(new_n76_), .b(new_n75_), .c(new_n48_), .d(x13), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n72_), .c(new_n73_), .d(new_n44_), .O(z12));
  oai21  g37(.a(new_n48_), .b(new_n43_), .c(new_n51_), .O(new_n79_));
  nand2  g38(.a(new_n56_), .b(x01), .O(new_n80_));
  aoi22  g39(.a(new_n80_), .b(x14), .c(new_n79_), .d(x15), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n72_), .c(new_n46_), .O(z13));
  nand3  g41(.a(new_n48_), .b(x20), .c(x15), .O(new_n83_));
  nand3  g42(.a(new_n75_), .b(x08), .c(x02), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n72_), .O(z14));
  nand2  g44(.a(x20), .b(x15), .O(new_n86_));
  oai21  g45(.a(new_n56_), .b(x15), .c(new_n86_), .O(new_n87_));
  nand3  g46(.a(new_n87_), .b(x16), .c(new_n51_), .O(new_n88_));
  nand3  g47(.a(new_n75_), .b(x08), .c(x03), .O(new_n89_));
  aoi21  g48(.a(new_n89_), .b(new_n88_), .c(new_n72_), .O(z15));
  aoi21  g49(.a(x17), .b(x12), .c(new_n43_), .O(new_n91_));
  and2   g50(.a(x08), .b(x04), .O(new_n92_));
  aoi22  g51(.a(new_n92_), .b(new_n75_), .c(new_n48_), .d(x17), .O(new_n93_));
  oai22  g52(.a(new_n93_), .b(new_n72_), .c(new_n91_), .d(new_n44_), .O(z16));
  aoi21  g53(.a(x18), .b(x12), .c(new_n43_), .O(new_n95_));
  and2   g54(.a(x08), .b(x05), .O(new_n96_));
  aoi22  g55(.a(new_n96_), .b(new_n75_), .c(new_n48_), .d(x18), .O(new_n97_));
  oai22  g56(.a(new_n97_), .b(new_n72_), .c(new_n95_), .d(new_n44_), .O(z17));
  inv1   g57(.a(x06), .O(new_n99_));
  nand2  g58(.a(new_n48_), .b(x19), .O(new_n100_));
  oai21  g59(.a(new_n48_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n44_), .O(new_n102_));
  nand3  g61(.a(x20), .b(x19), .c(x15), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n72_), .O(z18));
  inv1   g63(.a(x07), .O(new_n105_));
  oai21  g64(.a(new_n86_), .b(x14), .c(new_n105_), .O(new_n106_));
  nand3  g65(.a(new_n106_), .b(x10), .c(x08), .O(new_n107_));
  aoi22  g66(.a(new_n48_), .b(x20), .c(x15), .d(x14), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(x12), .O(new_n110_));
  nand2  g69(.a(new_n110_), .b(new_n46_), .O(z19));
  buf    g70(.a(x15), .O(z06));
endmodule


