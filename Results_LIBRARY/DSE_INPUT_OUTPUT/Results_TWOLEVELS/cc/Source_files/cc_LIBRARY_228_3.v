// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n63_, new_n65_, new_n66_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_;
  inv1   g00(.a(x11), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  inv1   g02(.a(x10), .O(new_n44_));
  inv1   g03(.a(x15), .O(new_n45_));
  nor2   g04(.a(new_n45_), .b(new_n44_), .O(z01));
  inv1   g05(.a(z01), .O(new_n47_));
  oai21  g06(.a(new_n43_), .b(new_n42_), .c(new_n47_), .O(z00));
  nand3  g07(.a(x12), .b(new_n42_), .c(x08), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand2  g09(.a(new_n50_), .b(x10), .O(new_n51_));
  nand3  g10(.a(x14), .b(x12), .c(new_n42_), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(z02));
  nand2  g12(.a(new_n47_), .b(x18), .O(z04));
  inv1   g13(.a(x19), .O(new_n55_));
  nor2   g14(.a(z01), .b(new_n55_), .O(z05));
  inv1   g15(.a(x08), .O(new_n57_));
  nand2  g16(.a(x09), .b(new_n57_), .O(new_n58_));
  oai21  g17(.a(new_n58_), .b(new_n57_), .c(x15), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(x10), .O(z06));
  inv1   g19(.a(x17), .O(new_n61_));
  nor2   g20(.a(z01), .b(new_n61_), .O(z07));
  inv1   g21(.a(x16), .O(new_n63_));
  nand2  g22(.a(new_n47_), .b(new_n63_), .O(z08));
  nand2  g23(.a(x15), .b(x10), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(x09), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z09));
  aoi22  g26(.a(x15), .b(x10), .c(x09), .d(x08), .O(z10));
  inv1   g27(.a(x14), .O(new_n69_));
  nor2   g28(.a(z01), .b(new_n69_), .O(z11));
  inv1   g29(.a(x00), .O(new_n71_));
  nand2  g30(.a(x12), .b(x08), .O(new_n72_));
  oai21  g31(.a(new_n72_), .b(new_n71_), .c(new_n45_), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(x10), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  nand3  g34(.a(new_n75_), .b(x13), .c(x12), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n74_), .O(z12));
  inv1   g36(.a(x12), .O(new_n78_));
  aoi21  g37(.a(x08), .b(x01), .c(x15), .O(new_n79_));
  oai21  g38(.a(new_n79_), .b(new_n44_), .c(x14), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(new_n78_), .O(z13));
  inv1   g40(.a(x02), .O(new_n82_));
  oai21  g41(.a(new_n72_), .b(new_n82_), .c(new_n45_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x10), .O(new_n84_));
  oai21  g43(.a(new_n45_), .b(new_n78_), .c(new_n84_), .O(z14));
  inv1   g44(.a(x03), .O(new_n86_));
  oai21  g45(.a(new_n72_), .b(new_n86_), .c(new_n45_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x10), .O(new_n88_));
  nand4  g47(.a(new_n75_), .b(x16), .c(new_n69_), .d(x12), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n88_), .O(z15));
  inv1   g49(.a(x04), .O(new_n91_));
  oai21  g50(.a(new_n72_), .b(new_n91_), .c(new_n45_), .O(new_n92_));
  nand2  g51(.a(new_n92_), .b(x10), .O(new_n93_));
  nand3  g52(.a(new_n75_), .b(x17), .c(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n93_), .O(z16));
  inv1   g54(.a(x18), .O(new_n96_));
  nand3  g55(.a(x10), .b(x08), .c(x05), .O(new_n97_));
  oai21  g56(.a(new_n96_), .b(x08), .c(new_n97_), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  nand2  g58(.a(x18), .b(new_n44_), .O(new_n100_));
  aoi21  g59(.a(new_n100_), .b(new_n99_), .c(new_n78_), .O(z17));
  inv1   g60(.a(x06), .O(new_n102_));
  oai21  g61(.a(new_n72_), .b(new_n102_), .c(new_n45_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x10), .O(new_n104_));
  nand3  g63(.a(new_n75_), .b(x19), .c(x12), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n104_), .O(z18));
  nand3  g65(.a(x10), .b(x08), .c(x07), .O(new_n107_));
  oai21  g66(.a(new_n43_), .b(x08), .c(new_n107_), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n45_), .O(new_n109_));
  nand2  g68(.a(x20), .b(new_n44_), .O(new_n110_));
  aoi21  g69(.a(new_n110_), .b(new_n109_), .c(new_n78_), .O(z19));
  nor2   g70(.a(new_n45_), .b(new_n44_), .O(z03));
endmodule


