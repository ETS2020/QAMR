// Benchmark "FAU" written by ABC on Tue Aug 18 14:00:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n64_, new_n66_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  nand2  g01(.a(x18), .b(x12), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x08), .O(new_n45_));
  inv1   g04(.a(x14), .O(new_n46_));
  nand2  g05(.a(x18), .b(x12), .O(new_n47_));
  nand4  g06(.a(new_n47_), .b(x15), .c(new_n46_), .d(x10), .O(new_n48_));
  nor2   g07(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g08(.a(x11), .O(new_n50_));
  inv1   g09(.a(x18), .O(z04));
  inv1   g10(.a(x15), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x10), .c(x08), .O(new_n53_));
  nand2  g12(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(z04), .c(x12), .d(new_n50_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand3  g15(.a(x12), .b(x10), .c(x08), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(z04), .c(x15), .d(new_n46_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(z03));
  and2   g19(.a(new_n43_), .b(x19), .O(z05));
  nand2  g20(.a(new_n43_), .b(new_n52_), .O(z06));
  and2   g21(.a(new_n43_), .b(x17), .O(z07));
  inv1   g22(.a(x16), .O(new_n64_));
  nand2  g23(.a(new_n43_), .b(new_n64_), .O(z08));
  nand2  g24(.a(x09), .b(x08), .O(new_n66_));
  nand2  g25(.a(new_n66_), .b(new_n43_), .O(z09));
  inv1   g26(.a(z09), .O(z10));
  nand2  g27(.a(new_n43_), .b(new_n46_), .O(z11));
  nand2  g28(.a(new_n53_), .b(x13), .O(new_n70_));
  nand4  g29(.a(new_n52_), .b(x10), .c(x08), .d(x00), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g31(.a(new_n72_), .b(z04), .c(x12), .O(new_n73_));
  inv1   g32(.a(new_n73_), .O(z12));
  inv1   g33(.a(x01), .O(new_n75_));
  nand2  g34(.a(x10), .b(x08), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n75_), .c(x14), .O(new_n77_));
  nand3  g36(.a(x15), .b(x10), .c(x08), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(z04), .c(x12), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z13));
  nand2  g40(.a(new_n76_), .b(x15), .O(new_n82_));
  nand4  g41(.a(new_n52_), .b(x10), .c(x08), .d(x02), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand3  g43(.a(new_n84_), .b(z04), .c(x12), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(z14));
  inv1   g45(.a(x12), .O(new_n87_));
  nand3  g46(.a(new_n53_), .b(x16), .c(new_n46_), .O(new_n88_));
  inv1   g47(.a(x10), .O(new_n89_));
  nor2   g48(.a(x15), .b(new_n89_), .O(new_n90_));
  and2   g49(.a(x08), .b(x03), .O(new_n91_));
  aoi21  g50(.a(new_n91_), .b(new_n90_), .c(x18), .O(new_n92_));
  aoi21  g51(.a(new_n92_), .b(new_n88_), .c(new_n87_), .O(z15));
  nand2  g52(.a(new_n53_), .b(x17), .O(new_n94_));
  nand4  g53(.a(new_n52_), .b(x10), .c(x08), .d(x04), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g55(.a(new_n96_), .b(z04), .c(x12), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z16));
  nand4  g57(.a(x12), .b(x10), .c(x08), .d(x05), .O(new_n99_));
  nor3   g58(.a(new_n99_), .b(x18), .c(x15), .O(z17));
  nand2  g59(.a(new_n53_), .b(x19), .O(new_n101_));
  and2   g60(.a(x08), .b(x06), .O(new_n102_));
  aoi21  g61(.a(new_n102_), .b(new_n90_), .c(x18), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n101_), .c(new_n87_), .O(z18));
  nand2  g63(.a(new_n53_), .b(x20), .O(new_n105_));
  and2   g64(.a(x08), .b(x07), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n90_), .c(x18), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n105_), .c(new_n87_), .O(z19));
endmodule


