// Benchmark "FAU" written by ABC on Tue Aug 18 13:59:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n60_,
    new_n63_, new_n65_, new_n67_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_;
  nand2  g00(.a(x20), .b(x11), .O(new_n42_));
  inv1   g01(.a(x06), .O(new_n43_));
  inv1   g02(.a(x15), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  inv1   g06(.a(x14), .O(new_n48_));
  nand3  g07(.a(new_n48_), .b(x10), .c(x08), .O(new_n49_));
  aoi21  g08(.a(new_n49_), .b(new_n43_), .c(new_n44_), .O(z01));
  inv1   g09(.a(x11), .O(new_n51_));
  inv1   g10(.a(x08), .O(new_n52_));
  nand2  g11(.a(new_n44_), .b(x10), .O(new_n53_));
  oai22  g12(.a(new_n53_), .b(new_n52_), .c(new_n45_), .d(new_n48_), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x12), .c(new_n51_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z02));
  nand4  g15(.a(x12), .b(x10), .c(x08), .d(new_n43_), .O(new_n57_));
  nor3   g16(.a(new_n57_), .b(new_n44_), .c(x14), .O(z03));
  nand2  g17(.a(new_n46_), .b(x18), .O(z04));
  inv1   g18(.a(x19), .O(new_n60_));
  nand2  g19(.a(new_n46_), .b(new_n60_), .O(z05));
  nor2   g20(.a(new_n44_), .b(x06), .O(z06));
  inv1   g21(.a(x17), .O(new_n63_));
  nor2   g22(.a(new_n45_), .b(new_n63_), .O(z07));
  inv1   g23(.a(x16), .O(new_n65_));
  nor2   g24(.a(new_n45_), .b(new_n65_), .O(z08));
  nand3  g25(.a(new_n46_), .b(x09), .c(x08), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z09));
  nand3  g27(.a(new_n46_), .b(x09), .c(x08), .O(z10));
  nand2  g28(.a(new_n46_), .b(new_n48_), .O(z11));
  inv1   g29(.a(x12), .O(new_n71_));
  aoi21  g30(.a(x13), .b(x12), .c(x06), .O(new_n72_));
  inv1   g31(.a(x10), .O(new_n73_));
  nor2   g32(.a(x15), .b(new_n73_), .O(new_n74_));
  nand2  g33(.a(x10), .b(x08), .O(new_n75_));
  and2   g34(.a(x08), .b(x00), .O(new_n76_));
  aoi22  g35(.a(new_n76_), .b(new_n74_), .c(new_n75_), .d(x13), .O(new_n77_));
  oai22  g36(.a(new_n77_), .b(new_n71_), .c(new_n72_), .d(new_n44_), .O(z12));
  nand4  g37(.a(new_n48_), .b(x10), .c(x08), .d(new_n43_), .O(new_n79_));
  aoi21  g38(.a(new_n79_), .b(new_n48_), .c(new_n44_), .O(new_n80_));
  inv1   g39(.a(new_n75_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(x01), .c(new_n48_), .O(new_n82_));
  oai21  g41(.a(new_n82_), .b(new_n80_), .c(x12), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n46_), .O(z13));
  and2   g43(.a(x08), .b(x02), .O(new_n85_));
  aoi22  g44(.a(new_n85_), .b(new_n74_), .c(new_n75_), .d(x15), .O(new_n86_));
  oai21  g45(.a(new_n86_), .b(new_n71_), .c(new_n46_), .O(z14));
  nand3  g46(.a(x16), .b(new_n48_), .c(x12), .O(new_n88_));
  nand2  g47(.a(new_n88_), .b(new_n43_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x15), .O(new_n90_));
  nand3  g49(.a(new_n75_), .b(x16), .c(new_n48_), .O(new_n91_));
  nand2  g50(.a(x08), .b(x03), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n53_), .c(new_n91_), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(x12), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(new_n90_), .O(z15));
  aoi21  g54(.a(x17), .b(x12), .c(x06), .O(new_n96_));
  and2   g55(.a(x08), .b(x04), .O(new_n97_));
  aoi22  g56(.a(new_n97_), .b(new_n74_), .c(new_n75_), .d(x17), .O(new_n98_));
  oai22  g57(.a(new_n98_), .b(new_n71_), .c(new_n96_), .d(new_n44_), .O(z16));
  aoi21  g58(.a(x18), .b(x12), .c(x06), .O(new_n100_));
  and2   g59(.a(x08), .b(x05), .O(new_n101_));
  aoi22  g60(.a(new_n101_), .b(new_n74_), .c(new_n75_), .d(x18), .O(new_n102_));
  oai22  g61(.a(new_n102_), .b(new_n71_), .c(new_n100_), .d(new_n44_), .O(z17));
  nand2  g62(.a(new_n75_), .b(x19), .O(new_n104_));
  oai21  g63(.a(new_n75_), .b(new_n43_), .c(new_n104_), .O(new_n105_));
  nand2  g64(.a(new_n105_), .b(new_n44_), .O(new_n106_));
  nand3  g65(.a(x19), .b(x15), .c(new_n43_), .O(new_n107_));
  aoi21  g66(.a(new_n107_), .b(new_n106_), .c(new_n71_), .O(z18));
  aoi21  g67(.a(x20), .b(x12), .c(x06), .O(new_n109_));
  and2   g68(.a(x08), .b(x07), .O(new_n110_));
  aoi22  g69(.a(new_n110_), .b(new_n74_), .c(new_n75_), .d(x20), .O(new_n111_));
  oai22  g70(.a(new_n111_), .b(new_n71_), .c(new_n109_), .d(new_n44_), .O(z19));
endmodule


