// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n37_, new_n39_, new_n40_, new_n42_,
    new_n43_;
  inv1   g00(.a(x5), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nand3  g03(.a(x3), .b(new_n17_), .c(new_n16_), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n15_), .O(new_n19_));
  nor3   g05(.a(x5), .b(x4), .c(x1), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n19_), .O(z0));
  nand3  g07(.a(x3), .b(new_n17_), .c(x0), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n15_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n20_), .O(z1));
  inv1   g10(.a(x4), .O(new_n25_));
  inv1   g11(.a(x3), .O(new_n26_));
  nor2   g12(.a(new_n26_), .b(x2), .O(new_n27_));
  inv1   g13(.a(x1), .O(new_n28_));
  nor2   g14(.a(new_n28_), .b(x0), .O(new_n29_));
  nand4  g15(.a(new_n29_), .b(new_n27_), .c(new_n15_), .d(new_n25_), .O(z2));
  nand4  g16(.a(x3), .b(new_n17_), .c(x1), .d(x0), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n15_), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n25_), .O(z3));
  nand2  g19(.a(x3), .b(x2), .O(new_n34_));
  oai21  g20(.a(new_n34_), .b(x0), .c(new_n15_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n20_), .O(z4));
  oai21  g22(.a(new_n34_), .b(new_n16_), .c(new_n15_), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(new_n20_), .O(z5));
  nand4  g24(.a(x3), .b(x2), .c(x1), .d(new_n16_), .O(new_n39_));
  nand2  g25(.a(new_n39_), .b(new_n15_), .O(new_n40_));
  nand2  g26(.a(new_n40_), .b(new_n25_), .O(z6));
  nand2  g27(.a(x1), .b(x0), .O(new_n42_));
  oai21  g28(.a(new_n34_), .b(new_n42_), .c(new_n15_), .O(new_n43_));
  nand2  g29(.a(new_n43_), .b(new_n25_), .O(z7));
endmodule


