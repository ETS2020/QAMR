// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n37_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nand2  g03(.a(new_n17_), .b(new_n16_), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n15_), .O(new_n19_));
  inv1   g05(.a(x4), .O(new_n20_));
  inv1   g06(.a(x5), .O(new_n21_));
  nand4  g07(.a(new_n21_), .b(new_n20_), .c(x3), .d(new_n15_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n16_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n19_), .O(z0));
  nand4  g10(.a(new_n21_), .b(new_n20_), .c(x3), .d(new_n17_), .O(new_n25_));
  inv1   g11(.a(new_n25_), .O(new_n26_));
  nand3  g12(.a(new_n26_), .b(new_n18_), .c(new_n15_), .O(z1));
  nand2  g13(.a(new_n25_), .b(new_n16_), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(x1), .O(z2));
  nand2  g15(.a(x1), .b(x0), .O(z3));
  oai21  g16(.a(new_n17_), .b(x0), .c(new_n15_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n23_), .O(z4));
  oai21  g18(.a(x2), .b(x1), .c(new_n16_), .O(new_n33_));
  nand4  g19(.a(new_n21_), .b(new_n20_), .c(x3), .d(x2), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n15_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n33_), .O(z5));
  nand2  g22(.a(new_n34_), .b(new_n16_), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(x1), .O(z6));
  nand2  g24(.a(x1), .b(x0), .O(z7));
endmodule


