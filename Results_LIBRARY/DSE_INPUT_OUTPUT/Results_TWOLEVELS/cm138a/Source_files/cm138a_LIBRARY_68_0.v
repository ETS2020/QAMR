// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:20 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n33_,
    new_n34_, new_n35_, new_n37_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x2), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x0), .O(new_n18_));
  inv1   g04(.a(x4), .O(new_n19_));
  inv1   g05(.a(x5), .O(new_n20_));
  nand3  g06(.a(new_n20_), .b(new_n19_), .c(x3), .O(new_n21_));
  inv1   g07(.a(new_n21_), .O(new_n22_));
  nand4  g08(.a(new_n22_), .b(new_n16_), .c(x1), .d(new_n18_), .O(z2));
  nand4  g09(.a(new_n22_), .b(new_n16_), .c(x1), .d(x0), .O(z3));
  nand2  g10(.a(x1), .b(new_n18_), .O(new_n25_));
  oai21  g11(.a(x2), .b(x1), .c(x0), .O(new_n26_));
  nand3  g12(.a(new_n20_), .b(new_n19_), .c(x3), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(x2), .O(new_n28_));
  nand3  g14(.a(new_n28_), .b(new_n26_), .c(new_n25_), .O(z4));
  nand2  g15(.a(x1), .b(x0), .O(new_n30_));
  oai21  g16(.a(x2), .b(x1), .c(new_n18_), .O(new_n31_));
  nand3  g17(.a(new_n31_), .b(new_n28_), .c(new_n30_), .O(z5));
  oai21  g18(.a(new_n16_), .b(x0), .c(x1), .O(new_n33_));
  nand4  g19(.a(new_n20_), .b(new_n19_), .c(x3), .d(x1), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(x2), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n33_), .O(z6));
  oai21  g22(.a(new_n16_), .b(new_n18_), .c(x1), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(new_n35_), .O(z7));
  nand2  g24(.a(new_n16_), .b(new_n15_), .O(z1));
endmodule


