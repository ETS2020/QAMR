// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n24_, new_n25_, new_n26_, new_n29_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_;
  inv1   g00(.a(x1), .O(new_n15_));
  oai21  g01(.a(x2), .b(new_n15_), .c(x0), .O(new_n16_));
  inv1   g02(.a(x4), .O(new_n17_));
  inv1   g03(.a(x5), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(new_n17_), .c(x3), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n15_), .O(new_n20_));
  inv1   g06(.a(x0), .O(new_n21_));
  nand2  g07(.a(x2), .b(new_n21_), .O(new_n22_));
  nand3  g08(.a(new_n22_), .b(new_n20_), .c(new_n16_), .O(z0));
  nand4  g09(.a(new_n18_), .b(new_n17_), .c(x3), .d(x0), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n15_), .O(new_n25_));
  aoi21  g11(.a(x2), .b(x0), .c(x1), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n25_), .O(z1));
  inv1   g13(.a(x2), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(x1), .O(z3));
  nand4  g15(.a(new_n18_), .b(new_n17_), .c(x3), .d(x2), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(new_n15_), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(new_n26_), .O(z4));
  oai21  g18(.a(x1), .b(new_n21_), .c(x2), .O(new_n34_));
  nand2  g19(.a(new_n34_), .b(new_n32_), .O(z5));
  inv1   g20(.a(new_n26_), .O(new_n36_));
  nand4  g21(.a(new_n18_), .b(new_n17_), .c(x3), .d(new_n21_), .O(new_n37_));
  nand2  g22(.a(new_n37_), .b(x2), .O(new_n38_));
  nand2  g23(.a(new_n38_), .b(new_n36_), .O(z6));
  nand2  g24(.a(new_n22_), .b(new_n15_), .O(new_n40_));
  nand2  g25(.a(new_n24_), .b(x2), .O(new_n41_));
  nand2  g26(.a(new_n41_), .b(new_n40_), .O(z7));
  one    g27(.O(z2));
endmodule


