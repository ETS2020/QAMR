// Benchmark "FAU" written by ABC on Wed Aug 19 17:14:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n21_, new_n23_,
    new_n26_, new_n27_, new_n28_, new_n31_, new_n32_, new_n33_;
  inv1   g00(.a(x4), .O(new_n15_));
  inv1   g01(.a(x5), .O(new_n16_));
  nand3  g02(.a(new_n16_), .b(new_n15_), .c(x3), .O(new_n17_));
  inv1   g03(.a(new_n17_), .O(new_n18_));
  nor3   g04(.a(x2), .b(x1), .c(x0), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n18_), .O(z0));
  inv1   g06(.a(x2), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(x0), .O(z1));
  inv1   g08(.a(x0), .O(new_n23_));
  nand4  g09(.a(new_n18_), .b(new_n21_), .c(x1), .d(new_n23_), .O(z2));
  nand3  g10(.a(new_n16_), .b(new_n15_), .c(x3), .O(new_n26_));
  nand2  g11(.a(new_n26_), .b(x2), .O(new_n27_));
  nor2   g12(.a(new_n21_), .b(x1), .O(new_n28_));
  nand3  g13(.a(new_n28_), .b(new_n27_), .c(new_n23_), .O(z4));
  nand3  g14(.a(new_n28_), .b(new_n27_), .c(x0), .O(z5));
  nand4  g15(.a(new_n16_), .b(new_n15_), .c(x3), .d(x1), .O(new_n31_));
  nand2  g16(.a(new_n31_), .b(x2), .O(new_n32_));
  nor2   g17(.a(new_n21_), .b(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n32_), .O(z6));
  nand2  g19(.a(new_n32_), .b(x0), .O(z7));
  one    g20(.O(z3));
endmodule


