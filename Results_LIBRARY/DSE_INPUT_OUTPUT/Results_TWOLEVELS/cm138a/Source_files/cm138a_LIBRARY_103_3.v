// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n27_, new_n29_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n36_, new_n37_, new_n38_;
  inv1   g00(.a(x2), .O(new_n15_));
  oai21  g01(.a(x1), .b(x0), .c(new_n15_), .O(new_n16_));
  inv1   g02(.a(x0), .O(new_n17_));
  inv1   g03(.a(x4), .O(new_n18_));
  inv1   g04(.a(x5), .O(new_n19_));
  nand4  g05(.a(new_n19_), .b(new_n18_), .c(x3), .d(new_n15_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n16_), .O(z0));
  inv1   g08(.a(x1), .O(new_n23_));
  oai21  g09(.a(x2), .b(new_n23_), .c(new_n17_), .O(new_n24_));
  nand4  g10(.a(new_n19_), .b(new_n18_), .c(x3), .d(new_n23_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n15_), .O(new_n26_));
  nand2  g12(.a(x2), .b(x0), .O(new_n27_));
  nand3  g13(.a(new_n27_), .b(new_n26_), .c(new_n24_), .O(z1));
  oai21  g14(.a(new_n23_), .b(x0), .c(new_n15_), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n21_), .O(z2));
  nand2  g16(.a(new_n15_), .b(new_n23_), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n17_), .O(new_n32_));
  nand4  g18(.a(new_n19_), .b(new_n18_), .c(x3), .d(x1), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n15_), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n32_), .O(z3));
  nand2  g21(.a(new_n27_), .b(x1), .O(new_n36_));
  nand3  g22(.a(new_n19_), .b(new_n18_), .c(x3), .O(new_n37_));
  nand2  g23(.a(new_n37_), .b(new_n17_), .O(new_n38_));
  nand3  g24(.a(new_n38_), .b(new_n36_), .c(new_n31_), .O(z4));
  nand2  g25(.a(x2), .b(x0), .O(z5));
  nand4  g26(.a(new_n38_), .b(x2), .c(x1), .d(new_n17_), .O(z6));
  one    g27(.O(z7));
endmodule


