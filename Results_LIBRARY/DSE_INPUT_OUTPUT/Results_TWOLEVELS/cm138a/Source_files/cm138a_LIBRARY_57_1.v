// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n35_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_;
  inv1   g00(.a(x5), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  nand3  g03(.a(x3), .b(new_n17_), .c(new_n16_), .O(new_n18_));
  nand2  g04(.a(new_n18_), .b(new_n15_), .O(new_n19_));
  nor3   g05(.a(x5), .b(x4), .c(x1), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n19_), .O(z0));
  nand3  g07(.a(x3), .b(new_n17_), .c(x0), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n15_), .O(new_n23_));
  nand2  g09(.a(new_n23_), .b(new_n20_), .O(z1));
  inv1   g10(.a(x4), .O(new_n25_));
  nand4  g11(.a(x3), .b(new_n17_), .c(x1), .d(new_n16_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n15_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(new_n25_), .O(z2));
  nand4  g14(.a(x3), .b(new_n17_), .c(x1), .d(x0), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n15_), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(new_n25_), .O(z3));
  nand2  g17(.a(x3), .b(x2), .O(new_n32_));
  oai21  g18(.a(new_n32_), .b(x0), .c(new_n15_), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n20_), .O(z4));
  oai21  g20(.a(new_n32_), .b(new_n16_), .c(new_n15_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n20_), .O(z5));
  inv1   g22(.a(x1), .O(new_n37_));
  nor2   g23(.a(new_n37_), .b(x0), .O(new_n38_));
  inv1   g24(.a(new_n32_), .O(new_n39_));
  nand4  g25(.a(new_n39_), .b(new_n38_), .c(new_n15_), .d(new_n25_), .O(z6));
  nand2  g26(.a(x1), .b(x0), .O(new_n41_));
  oai21  g27(.a(new_n32_), .b(new_n41_), .c(new_n15_), .O(new_n42_));
  nand2  g28(.a(new_n42_), .b(new_n25_), .O(z7));
endmodule


