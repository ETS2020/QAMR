// Benchmark "FAU" written by ABC on Wed Aug 19 17:16:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n19_, new_n20_, new_n21_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n33_, new_n35_, new_n36_, new_n38_, new_n39_, new_n41_,
    new_n43_;
  inv1   g00(.a(x1), .O(new_n15_));
  nor2   g01(.a(x2), .b(x0), .O(new_n16_));
  nor2   g02(.a(x5), .b(x4), .O(new_n17_));
  nand4  g03(.a(new_n17_), .b(new_n16_), .c(x3), .d(new_n15_), .O(z0));
  inv1   g04(.a(x2), .O(new_n19_));
  inv1   g05(.a(x4), .O(new_n20_));
  inv1   g06(.a(x5), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(x3), .O(new_n23_));
  nand3  g09(.a(x3), .b(new_n15_), .c(x0), .O(new_n24_));
  inv1   g10(.a(new_n24_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n23_), .O(z1));
  inv1   g12(.a(x0), .O(new_n27_));
  nand2  g13(.a(x1), .b(new_n27_), .O(new_n28_));
  oai21  g14(.a(new_n28_), .b(new_n22_), .c(x3), .O(new_n29_));
  inv1   g15(.a(x3), .O(new_n30_));
  nand2  g16(.a(new_n30_), .b(x1), .O(new_n31_));
  nand2  g17(.a(new_n31_), .b(new_n29_), .O(z2));
  nor2   g18(.a(x2), .b(new_n15_), .O(new_n33_));
  nand4  g19(.a(new_n33_), .b(new_n17_), .c(x3), .d(x0), .O(z3));
  nand3  g20(.a(new_n17_), .b(x2), .c(new_n27_), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(x3), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(new_n15_), .O(z4));
  nand2  g23(.a(new_n17_), .b(x2), .O(new_n38_));
  nand2  g24(.a(new_n38_), .b(x3), .O(new_n39_));
  nand2  g25(.a(new_n39_), .b(new_n25_), .O(z5));
  inv1   g26(.a(new_n28_), .O(new_n41_));
  nand4  g27(.a(new_n41_), .b(new_n17_), .c(x3), .d(x2), .O(z6));
  nor2   g28(.a(new_n19_), .b(new_n15_), .O(new_n43_));
  nand4  g29(.a(new_n43_), .b(new_n17_), .c(x3), .d(x0), .O(z7));
endmodule


