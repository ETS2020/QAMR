// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n24_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n35_, new_n36_, new_n38_, new_n39_;
  inv1   g00(.a(x1), .O(new_n15_));
  inv1   g01(.a(x0), .O(new_n16_));
  inv1   g02(.a(x2), .O(new_n17_));
  inv1   g03(.a(x4), .O(new_n18_));
  inv1   g04(.a(x5), .O(new_n19_));
  nand4  g05(.a(new_n19_), .b(new_n18_), .c(x3), .d(new_n17_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n16_), .O(new_n21_));
  nand2  g07(.a(new_n21_), .b(new_n15_), .O(z0));
  nand4  g08(.a(new_n19_), .b(new_n18_), .c(x3), .d(x1), .O(new_n24_));
  inv1   g09(.a(new_n24_), .O(new_n25_));
  nand2  g10(.a(x2), .b(x1), .O(new_n26_));
  nand3  g11(.a(new_n26_), .b(new_n25_), .c(new_n16_), .O(z2));
  nand2  g12(.a(new_n26_), .b(new_n16_), .O(new_n28_));
  nand2  g13(.a(new_n20_), .b(x1), .O(new_n29_));
  nand2  g14(.a(new_n29_), .b(new_n28_), .O(z3));
  nand4  g15(.a(new_n19_), .b(new_n18_), .c(x3), .d(x2), .O(new_n31_));
  inv1   g16(.a(new_n31_), .O(new_n32_));
  nand3  g17(.a(new_n32_), .b(new_n26_), .c(new_n16_), .O(z4));
  nand2  g18(.a(new_n24_), .b(new_n16_), .O(new_n35_));
  oai21  g19(.a(new_n17_), .b(x0), .c(x1), .O(new_n36_));
  nand2  g20(.a(new_n36_), .b(new_n35_), .O(z6));
  oai21  g21(.a(x2), .b(new_n15_), .c(new_n16_), .O(new_n38_));
  nand2  g22(.a(new_n31_), .b(x1), .O(new_n39_));
  nand2  g23(.a(new_n39_), .b(new_n38_), .O(z7));
  one    g24(.O(z1));
  one    g25(.O(z5));
endmodule


