// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n22_, new_n24_, new_n25_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n39_, new_n40_, new_n41_, new_n42_;
  inv1   g00(.a(x4), .O(new_n15_));
  inv1   g01(.a(x5), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(new_n17_));
  oai21  g03(.a(new_n17_), .b(x0), .c(x3), .O(new_n18_));
  inv1   g04(.a(x1), .O(new_n19_));
  inv1   g05(.a(x2), .O(new_n20_));
  nand3  g06(.a(x3), .b(new_n20_), .c(new_n19_), .O(new_n21_));
  inv1   g07(.a(new_n21_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n18_), .O(z0));
  inv1   g09(.a(x0), .O(new_n24_));
  oai21  g10(.a(new_n17_), .b(new_n24_), .c(x3), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n22_), .O(z1));
  nor2   g12(.a(x5), .b(x4), .O(new_n27_));
  nor2   g13(.a(new_n19_), .b(x0), .O(new_n28_));
  nand4  g14(.a(new_n28_), .b(new_n27_), .c(x3), .d(new_n20_), .O(z2));
  nand2  g15(.a(x1), .b(x0), .O(new_n30_));
  inv1   g16(.a(new_n30_), .O(new_n31_));
  nand4  g17(.a(new_n31_), .b(new_n27_), .c(x3), .d(new_n20_), .O(z3));
  nand3  g18(.a(new_n27_), .b(x2), .c(new_n24_), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(x3), .O(new_n34_));
  nand2  g20(.a(new_n34_), .b(new_n19_), .O(z4));
  nor2   g21(.a(new_n20_), .b(new_n24_), .O(new_n36_));
  nand4  g22(.a(new_n36_), .b(new_n27_), .c(x3), .d(new_n19_), .O(z5));
  nand4  g23(.a(new_n28_), .b(new_n27_), .c(x3), .d(x2), .O(z6));
  nand3  g24(.a(new_n16_), .b(new_n15_), .c(x2), .O(new_n39_));
  oai21  g25(.a(new_n39_), .b(new_n30_), .c(x3), .O(new_n40_));
  inv1   g26(.a(x3), .O(new_n41_));
  nand2  g27(.a(new_n41_), .b(x1), .O(new_n42_));
  nand2  g28(.a(new_n42_), .b(new_n40_), .O(z7));
endmodule


