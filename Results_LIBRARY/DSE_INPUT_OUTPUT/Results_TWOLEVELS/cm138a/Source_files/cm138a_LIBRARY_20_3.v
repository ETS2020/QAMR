// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n21_,
    new_n23_, new_n24_, new_n25_, new_n27_, new_n28_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n38_, new_n39_;
  inv1   g00(.a(x2), .O(new_n15_));
  nor2   g01(.a(x1), .b(x0), .O(new_n16_));
  inv1   g02(.a(x0), .O(new_n17_));
  inv1   g03(.a(x4), .O(new_n18_));
  inv1   g04(.a(x5), .O(new_n19_));
  nand3  g05(.a(new_n19_), .b(new_n18_), .c(x3), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n17_), .O(new_n21_));
  nand3  g07(.a(new_n21_), .b(new_n16_), .c(new_n15_), .O(z0));
  inv1   g08(.a(x1), .O(new_n23_));
  nand4  g09(.a(new_n19_), .b(new_n18_), .c(x3), .d(new_n23_), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(new_n15_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(x0), .O(z1));
  nand2  g12(.a(x1), .b(new_n17_), .O(new_n27_));
  nand2  g13(.a(new_n27_), .b(new_n15_), .O(new_n28_));
  nand3  g14(.a(new_n28_), .b(new_n21_), .c(new_n15_), .O(z2));
  nor2   g15(.a(x5), .b(x4), .O(new_n30_));
  and2   g16(.a(x3), .b(x1), .O(new_n31_));
  nand4  g17(.a(new_n31_), .b(new_n27_), .c(new_n30_), .d(new_n15_), .O(z3));
  nand2  g18(.a(x2), .b(x0), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(x1), .O(new_n34_));
  nand2  g20(.a(new_n15_), .b(new_n23_), .O(new_n35_));
  nand3  g21(.a(new_n35_), .b(new_n34_), .c(new_n21_), .O(z4));
  nand2  g22(.a(x2), .b(x0), .O(z5));
  nand2  g23(.a(new_n33_), .b(new_n23_), .O(new_n38_));
  nand2  g24(.a(new_n15_), .b(x1), .O(new_n39_));
  nand3  g25(.a(new_n39_), .b(new_n38_), .c(new_n21_), .O(z6));
  nand2  g26(.a(x2), .b(x0), .O(z7));
endmodule


