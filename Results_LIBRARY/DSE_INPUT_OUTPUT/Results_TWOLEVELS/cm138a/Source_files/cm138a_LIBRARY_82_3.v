// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:26 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n18_, new_n19_, new_n20_, new_n22_,
    new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_,
    new_n42_;
  inv1   g00(.a(x1), .O(new_n15_));
  oai21  g01(.a(x2), .b(x0), .c(new_n15_), .O(new_n16_));
  inv1   g02(.a(x0), .O(new_n17_));
  nor2   g03(.a(x5), .b(x4), .O(new_n18_));
  nand3  g04(.a(new_n18_), .b(x3), .c(new_n15_), .O(new_n19_));
  nand2  g05(.a(new_n19_), .b(new_n17_), .O(new_n20_));
  nand2  g06(.a(new_n20_), .b(new_n16_), .O(z0));
  nand2  g07(.a(x2), .b(new_n15_), .O(new_n22_));
  nand2  g08(.a(new_n22_), .b(new_n17_), .O(new_n23_));
  inv1   g09(.a(x2), .O(new_n24_));
  nand3  g10(.a(new_n18_), .b(x3), .c(new_n24_), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(new_n15_), .O(new_n26_));
  nand2  g12(.a(new_n26_), .b(new_n23_), .O(z1));
  inv1   g13(.a(new_n25_), .O(new_n28_));
  nand3  g14(.a(new_n28_), .b(x1), .c(new_n17_), .O(z2));
  nand2  g15(.a(x1), .b(x0), .O(z3));
  inv1   g16(.a(x4), .O(new_n31_));
  inv1   g17(.a(x5), .O(new_n32_));
  nand3  g18(.a(new_n32_), .b(new_n31_), .c(x3), .O(new_n33_));
  oai21  g19(.a(new_n33_), .b(new_n22_), .c(new_n17_), .O(new_n34_));
  nand2  g20(.a(new_n15_), .b(x0), .O(new_n35_));
  nand2  g21(.a(new_n35_), .b(new_n34_), .O(z4));
  nand2  g22(.a(x1), .b(x0), .O(new_n37_));
  oai21  g23(.a(x2), .b(x1), .c(new_n17_), .O(new_n38_));
  nand3  g24(.a(new_n18_), .b(x3), .c(x2), .O(new_n39_));
  nand2  g25(.a(new_n39_), .b(new_n15_), .O(new_n40_));
  nand3  g26(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z5));
  nand2  g27(.a(new_n39_), .b(new_n17_), .O(new_n42_));
  nand2  g28(.a(new_n42_), .b(new_n16_), .O(z6));
  nand2  g29(.a(x1), .b(x0), .O(z7));
endmodule


