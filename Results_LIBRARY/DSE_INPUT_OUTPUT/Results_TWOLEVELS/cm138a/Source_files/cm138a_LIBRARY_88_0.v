// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:28 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n18_, new_n19_, new_n20_, new_n21_, new_n23_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n32_, new_n33_,
    new_n35_, new_n37_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  nand2  g02(.a(new_n16_), .b(new_n15_), .O(z0));
  inv1   g03(.a(x2), .O(new_n18_));
  inv1   g04(.a(x3), .O(new_n19_));
  nor2   g05(.a(new_n19_), .b(x1), .O(new_n20_));
  nor2   g06(.a(x5), .b(x4), .O(new_n21_));
  nand4  g07(.a(new_n21_), .b(new_n20_), .c(new_n18_), .d(x0), .O(z1));
  aoi21  g08(.a(new_n16_), .b(new_n15_), .c(x2), .O(new_n23_));
  nand4  g09(.a(new_n23_), .b(new_n21_), .c(x3), .d(new_n15_), .O(z2));
  oai21  g10(.a(x2), .b(new_n15_), .c(x1), .O(new_n25_));
  inv1   g11(.a(x4), .O(new_n26_));
  inv1   g12(.a(x5), .O(new_n27_));
  nand4  g13(.a(new_n27_), .b(new_n26_), .c(x3), .d(x1), .O(new_n28_));
  nand2  g14(.a(new_n28_), .b(x0), .O(new_n29_));
  nand2  g15(.a(new_n29_), .b(new_n25_), .O(z3));
  nand4  g16(.a(new_n27_), .b(new_n26_), .c(x3), .d(x2), .O(new_n32_));
  nand2  g17(.a(new_n32_), .b(x0), .O(new_n33_));
  nand2  g18(.a(new_n33_), .b(new_n16_), .O(z5));
  aoi21  g19(.a(new_n16_), .b(new_n15_), .c(new_n18_), .O(new_n35_));
  nand4  g20(.a(new_n35_), .b(new_n21_), .c(x3), .d(new_n15_), .O(z6));
  inv1   g21(.a(new_n28_), .O(new_n37_));
  nand3  g22(.a(new_n37_), .b(x2), .c(x0), .O(z7));
  one    g23(.O(z4));
endmodule


