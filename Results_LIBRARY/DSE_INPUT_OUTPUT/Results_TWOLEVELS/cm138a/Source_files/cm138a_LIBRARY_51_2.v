// Benchmark "FAU" written by ABC on Wed Aug 19 17:15:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5,
    z0, z1, z2, z3, z4, z5, z6, z7  );
  input  x0, x1, x2, x3, x4, x5;
  output z0, z1, z2, z3, z4, z5, z6, z7;
  wire new_n15_, new_n16_, new_n17_, new_n20_, new_n21_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n36_, new_n38_, new_n40_, new_n41_;
  inv1   g00(.a(x0), .O(new_n15_));
  inv1   g01(.a(x1), .O(new_n16_));
  nor3   g02(.a(x5), .b(x4), .c(x2), .O(new_n17_));
  nand4  g03(.a(new_n17_), .b(x3), .c(new_n16_), .d(new_n15_), .O(z0));
  nand4  g04(.a(new_n17_), .b(x3), .c(new_n16_), .d(x0), .O(z1));
  nor2   g05(.a(x5), .b(x4), .O(new_n20_));
  nor2   g06(.a(x2), .b(x0), .O(new_n21_));
  nand4  g07(.a(new_n21_), .b(new_n20_), .c(x3), .d(x1), .O(z2));
  inv1   g08(.a(x2), .O(new_n23_));
  nand3  g09(.a(new_n20_), .b(new_n23_), .c(x0), .O(new_n24_));
  nand2  g10(.a(new_n24_), .b(x3), .O(new_n25_));
  nand2  g11(.a(new_n25_), .b(x1), .O(z3));
  nand2  g12(.a(new_n16_), .b(new_n15_), .O(new_n27_));
  inv1   g13(.a(x4), .O(new_n28_));
  inv1   g14(.a(x5), .O(new_n29_));
  nand3  g15(.a(new_n29_), .b(new_n28_), .c(x2), .O(new_n30_));
  oai21  g16(.a(new_n30_), .b(new_n27_), .c(x3), .O(new_n31_));
  inv1   g17(.a(x3), .O(new_n32_));
  nand2  g18(.a(new_n32_), .b(new_n16_), .O(new_n33_));
  nand2  g19(.a(new_n33_), .b(new_n31_), .O(z4));
  nand2  g20(.a(new_n16_), .b(x0), .O(new_n35_));
  oai21  g21(.a(new_n30_), .b(new_n35_), .c(x3), .O(new_n36_));
  nand2  g22(.a(new_n36_), .b(new_n33_), .O(z5));
  nor2   g23(.a(new_n23_), .b(x0), .O(new_n38_));
  nand4  g24(.a(new_n38_), .b(new_n20_), .c(x3), .d(x1), .O(z6));
  nand3  g25(.a(new_n20_), .b(x2), .c(x0), .O(new_n40_));
  nand2  g26(.a(new_n40_), .b(x3), .O(new_n41_));
  nand2  g27(.a(new_n41_), .b(x1), .O(z7));
endmodule


