// Benchmark "FAU" written by ABC on Tue Aug 18 19:07:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n148_, new_n149_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n157_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n163_, new_n164_, new_n165_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n172_, new_n173_, new_n174_,
    new_n176_, new_n177_, new_n178_, new_n179_, new_n181_, new_n182_,
    new_n183_, new_n185_, new_n186_, new_n187_, new_n188_, new_n190_,
    new_n191_, new_n192_, new_n193_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n207_, new_n208_, new_n209_, new_n210_, new_n211_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n219_,
    new_n220_, new_n221_, new_n222_, new_n224_, new_n225_, new_n226_,
    new_n227_, new_n228_, new_n230_, new_n231_, new_n232_, new_n233_,
    new_n235_, new_n236_, new_n237_, new_n238_, new_n239_;
  inv1   g000(.a(x11), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x07), .O(new_n85_));
  aoi21  g002(.a(x07), .b(x04), .c(new_n85_), .O(new_n86_));
  inv1   g003(.a(x08), .O(new_n87_));
  nand2  g004(.a(x39), .b(new_n87_), .O(new_n88_));
  oai21  g005(.a(new_n86_), .b(x10), .c(new_n88_), .O(z00));
  inv1   g006(.a(x12), .O(new_n90_));
  nor2   g007(.a(new_n90_), .b(x07), .O(new_n91_));
  aoi21  g008(.a(x07), .b(x05), .c(new_n91_), .O(new_n92_));
  oai21  g009(.a(new_n92_), .b(x10), .c(new_n88_), .O(z01));
  inv1   g010(.a(x10), .O(new_n94_));
  inv1   g011(.a(x13), .O(new_n95_));
  nand2  g012(.a(x07), .b(x06), .O(new_n96_));
  oai21  g013(.a(new_n95_), .b(x07), .c(new_n96_), .O(new_n97_));
  nand3  g014(.a(new_n97_), .b(new_n88_), .c(new_n94_), .O(new_n98_));
  inv1   g015(.a(new_n98_), .O(z02));
  inv1   g016(.a(x14), .O(new_n100_));
  nor2   g017(.a(new_n100_), .b(x07), .O(new_n101_));
  aoi21  g018(.a(x07), .b(x01), .c(new_n101_), .O(new_n102_));
  oai21  g019(.a(new_n102_), .b(x10), .c(new_n88_), .O(z03));
  inv1   g020(.a(x15), .O(new_n104_));
  nand2  g021(.a(x07), .b(x02), .O(new_n105_));
  oai21  g022(.a(new_n104_), .b(x07), .c(new_n105_), .O(new_n106_));
  nand3  g023(.a(new_n106_), .b(new_n88_), .c(new_n94_), .O(new_n107_));
  inv1   g024(.a(new_n107_), .O(z04));
  inv1   g025(.a(x16), .O(new_n109_));
  nand2  g026(.a(x07), .b(x03), .O(new_n110_));
  oai21  g027(.a(new_n109_), .b(x07), .c(new_n110_), .O(new_n111_));
  nand3  g028(.a(new_n111_), .b(new_n88_), .c(new_n94_), .O(new_n112_));
  inv1   g029(.a(new_n112_), .O(z05));
  nand2  g030(.a(x18), .b(x08), .O(new_n114_));
  inv1   g031(.a(x39), .O(new_n115_));
  nand3  g032(.a(new_n115_), .b(x17), .c(new_n87_), .O(new_n116_));
  aoi21  g033(.a(new_n116_), .b(new_n114_), .c(x10), .O(z06));
  nand2  g034(.a(x19), .b(x08), .O(new_n118_));
  nand3  g035(.a(new_n115_), .b(x18), .c(new_n87_), .O(new_n119_));
  aoi21  g036(.a(new_n119_), .b(new_n118_), .c(x10), .O(z07));
  nand2  g037(.a(x20), .b(x08), .O(new_n121_));
  nand3  g038(.a(new_n115_), .b(x19), .c(new_n87_), .O(new_n122_));
  aoi21  g039(.a(new_n122_), .b(new_n121_), .c(x10), .O(z08));
  nand2  g040(.a(x21), .b(x08), .O(new_n124_));
  nand3  g041(.a(new_n115_), .b(x20), .c(new_n87_), .O(new_n125_));
  aoi21  g042(.a(new_n125_), .b(new_n124_), .c(x10), .O(z09));
  aoi21  g043(.a(x21), .b(new_n94_), .c(x39), .O(new_n127_));
  nand3  g044(.a(x22), .b(new_n94_), .c(x08), .O(new_n128_));
  oai21  g045(.a(new_n127_), .b(x08), .c(new_n128_), .O(z10));
  aoi21  g046(.a(x22), .b(new_n94_), .c(x39), .O(new_n130_));
  nand3  g047(.a(x23), .b(new_n94_), .c(x08), .O(new_n131_));
  oai21  g048(.a(new_n130_), .b(x08), .c(new_n131_), .O(z11));
  nand2  g049(.a(x24), .b(x08), .O(new_n133_));
  nand3  g050(.a(new_n115_), .b(x23), .c(new_n87_), .O(new_n134_));
  aoi21  g051(.a(new_n134_), .b(new_n133_), .c(x10), .O(z12));
  aoi21  g052(.a(x24), .b(new_n94_), .c(x39), .O(new_n136_));
  nand3  g053(.a(x25), .b(new_n94_), .c(x08), .O(new_n137_));
  oai21  g054(.a(new_n136_), .b(x08), .c(new_n137_), .O(z13));
  nand2  g055(.a(x26), .b(x08), .O(new_n139_));
  nand3  g056(.a(new_n115_), .b(x25), .c(new_n87_), .O(new_n140_));
  aoi21  g057(.a(new_n140_), .b(new_n139_), .c(x10), .O(z14));
  aoi21  g058(.a(x26), .b(new_n94_), .c(x39), .O(new_n142_));
  nand3  g059(.a(x27), .b(new_n94_), .c(x08), .O(new_n143_));
  oai21  g060(.a(new_n142_), .b(x08), .c(new_n143_), .O(z15));
  nand2  g061(.a(x28), .b(x08), .O(new_n145_));
  nand3  g062(.a(new_n115_), .b(x27), .c(new_n87_), .O(new_n146_));
  aoi21  g063(.a(new_n146_), .b(new_n145_), .c(x10), .O(z16));
  nand2  g064(.a(x29), .b(x08), .O(new_n148_));
  nand3  g065(.a(new_n115_), .b(x28), .c(new_n87_), .O(new_n149_));
  aoi21  g066(.a(new_n149_), .b(new_n148_), .c(x10), .O(z17));
  aoi21  g067(.a(x29), .b(new_n94_), .c(x39), .O(new_n151_));
  nand3  g068(.a(x30), .b(new_n94_), .c(x08), .O(new_n152_));
  oai21  g069(.a(new_n151_), .b(x08), .c(new_n152_), .O(z18));
  nand2  g070(.a(x08), .b(x00), .O(new_n154_));
  nand3  g071(.a(new_n115_), .b(x30), .c(new_n87_), .O(new_n155_));
  aoi21  g072(.a(new_n155_), .b(new_n154_), .c(x10), .O(z19));
  inv1   g073(.a(x09), .O(new_n157_));
  inv1   g074(.a(x32), .O(new_n158_));
  nand2  g075(.a(x31), .b(new_n157_), .O(new_n159_));
  oai21  g076(.a(new_n158_), .b(new_n157_), .c(new_n159_), .O(new_n160_));
  nand3  g077(.a(new_n160_), .b(new_n88_), .c(new_n94_), .O(new_n161_));
  inv1   g078(.a(new_n161_), .O(z20));
  nand2  g079(.a(x33), .b(x09), .O(new_n163_));
  oai21  g080(.a(new_n158_), .b(x09), .c(new_n163_), .O(new_n164_));
  nand2  g081(.a(new_n164_), .b(new_n94_), .O(new_n165_));
  nand2  g082(.a(new_n165_), .b(new_n88_), .O(z21));
  inv1   g083(.a(x34), .O(new_n167_));
  nand2  g084(.a(x33), .b(new_n157_), .O(new_n168_));
  oai21  g085(.a(new_n167_), .b(new_n157_), .c(new_n168_), .O(new_n169_));
  nand2  g086(.a(new_n169_), .b(new_n94_), .O(new_n170_));
  nand2  g087(.a(new_n170_), .b(new_n88_), .O(z22));
  nand2  g088(.a(x35), .b(x09), .O(new_n172_));
  oai21  g089(.a(new_n167_), .b(x09), .c(new_n172_), .O(new_n173_));
  nand2  g090(.a(new_n173_), .b(new_n94_), .O(new_n174_));
  nand2  g091(.a(new_n174_), .b(new_n88_), .O(z23));
  inv1   g092(.a(x36), .O(new_n176_));
  nand2  g093(.a(x35), .b(new_n157_), .O(new_n177_));
  oai21  g094(.a(new_n176_), .b(new_n157_), .c(new_n177_), .O(new_n178_));
  nand2  g095(.a(new_n178_), .b(new_n94_), .O(new_n179_));
  nand2  g096(.a(new_n179_), .b(new_n88_), .O(z24));
  nand2  g097(.a(x37), .b(x09), .O(new_n181_));
  oai21  g098(.a(new_n176_), .b(x09), .c(new_n181_), .O(new_n182_));
  nand2  g099(.a(new_n182_), .b(new_n94_), .O(new_n183_));
  nand2  g100(.a(new_n183_), .b(new_n88_), .O(z25));
  inv1   g101(.a(x38), .O(new_n185_));
  nand2  g102(.a(x37), .b(new_n157_), .O(new_n186_));
  oai21  g103(.a(new_n185_), .b(new_n157_), .c(new_n186_), .O(new_n187_));
  nand2  g104(.a(new_n187_), .b(new_n94_), .O(new_n188_));
  nand2  g105(.a(new_n188_), .b(new_n88_), .O(z26));
  nand2  g106(.a(x14), .b(x00), .O(new_n190_));
  oai21  g107(.a(new_n115_), .b(x14), .c(new_n190_), .O(new_n191_));
  nor2   g108(.a(new_n185_), .b(x09), .O(new_n192_));
  aoi21  g109(.a(new_n191_), .b(x09), .c(new_n192_), .O(new_n193_));
  oai21  g110(.a(new_n193_), .b(x10), .c(new_n88_), .O(z27));
  nand2  g111(.a(x39), .b(x08), .O(new_n195_));
  nand3  g112(.a(new_n115_), .b(new_n100_), .c(x09), .O(new_n196_));
  nand2  g113(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(x40), .O(new_n198_));
  nand2  g115(.a(new_n100_), .b(x09), .O(new_n199_));
  nand3  g116(.a(new_n199_), .b(x39), .c(x08), .O(new_n200_));
  aoi21  g117(.a(new_n200_), .b(new_n198_), .c(x10), .O(z28));
  oai21  g118(.a(x14), .b(new_n157_), .c(x40), .O(new_n202_));
  nand3  g119(.a(x41), .b(new_n100_), .c(x09), .O(new_n203_));
  nand2  g120(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand3  g121(.a(new_n204_), .b(new_n88_), .c(new_n94_), .O(new_n205_));
  inv1   g122(.a(new_n205_), .O(z29));
  inv1   g123(.a(x41), .O(new_n207_));
  aoi21  g124(.a(new_n100_), .b(x09), .c(new_n207_), .O(new_n208_));
  nand3  g125(.a(x42), .b(new_n100_), .c(x09), .O(new_n209_));
  inv1   g126(.a(new_n209_), .O(new_n210_));
  oai21  g127(.a(new_n210_), .b(new_n208_), .c(new_n94_), .O(new_n211_));
  nand2  g128(.a(new_n211_), .b(new_n88_), .O(z30));
  inv1   g129(.a(x42), .O(new_n213_));
  aoi21  g130(.a(new_n100_), .b(x09), .c(new_n213_), .O(new_n214_));
  nand3  g131(.a(x43), .b(new_n100_), .c(x09), .O(new_n215_));
  inv1   g132(.a(new_n215_), .O(new_n216_));
  oai21  g133(.a(new_n216_), .b(new_n214_), .c(new_n94_), .O(new_n217_));
  nand2  g134(.a(new_n217_), .b(new_n88_), .O(z31));
  oai21  g135(.a(x14), .b(new_n157_), .c(x43), .O(new_n219_));
  nand3  g136(.a(x44), .b(new_n100_), .c(x09), .O(new_n220_));
  nand2  g137(.a(new_n220_), .b(new_n219_), .O(new_n221_));
  nand3  g138(.a(new_n221_), .b(new_n88_), .c(new_n94_), .O(new_n222_));
  inv1   g139(.a(new_n222_), .O(z32));
  inv1   g140(.a(x44), .O(new_n224_));
  aoi21  g141(.a(new_n100_), .b(x09), .c(new_n224_), .O(new_n225_));
  nand3  g142(.a(x45), .b(new_n100_), .c(x09), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(new_n227_));
  oai21  g144(.a(new_n227_), .b(new_n225_), .c(new_n94_), .O(new_n228_));
  nand2  g145(.a(new_n228_), .b(new_n88_), .O(z33));
  oai21  g146(.a(x14), .b(new_n157_), .c(x45), .O(new_n230_));
  nand3  g147(.a(x46), .b(new_n100_), .c(x09), .O(new_n231_));
  nand2  g148(.a(new_n231_), .b(new_n230_), .O(new_n232_));
  nand3  g149(.a(new_n232_), .b(new_n88_), .c(new_n94_), .O(new_n233_));
  inv1   g150(.a(new_n233_), .O(z34));
  inv1   g151(.a(x46), .O(new_n235_));
  aoi21  g152(.a(new_n100_), .b(x09), .c(new_n235_), .O(new_n236_));
  nand3  g153(.a(new_n100_), .b(x09), .c(x00), .O(new_n237_));
  inv1   g154(.a(new_n237_), .O(new_n238_));
  oai21  g155(.a(new_n238_), .b(new_n236_), .c(new_n94_), .O(new_n239_));
  nand2  g156(.a(new_n239_), .b(new_n88_), .O(z35));
endmodule


