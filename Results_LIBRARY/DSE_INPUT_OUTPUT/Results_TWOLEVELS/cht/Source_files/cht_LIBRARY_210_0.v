// Benchmark "FAU" written by ABC on Tue Aug 18 19:07:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n149_, new_n150_, new_n151_, new_n153_,
    new_n154_, new_n155_, new_n156_, new_n158_, new_n159_, new_n160_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n171_, new_n172_, new_n173_, new_n175_, new_n176_,
    new_n177_, new_n179_, new_n180_, new_n181_, new_n183_, new_n184_,
    new_n185_, new_n186_, new_n187_, new_n189_, new_n190_, new_n191_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n198_, new_n199_,
    new_n200_, new_n202_, new_n203_, new_n204_, new_n205_, new_n207_,
    new_n208_, new_n209_, new_n211_, new_n212_, new_n213_, new_n214_,
    new_n215_, new_n216_, new_n218_, new_n219_, new_n220_, new_n221_,
    new_n223_, new_n224_, new_n225_, new_n226_, new_n228_, new_n229_,
    new_n230_, new_n231_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n239_, new_n240_, new_n241_, new_n242_, new_n243_,
    new_n245_, new_n246_, new_n247_, new_n248_, new_n250_, new_n251_,
    new_n252_, new_n253_, new_n255_, new_n256_, new_n257_, new_n258_,
    new_n259_;
  inv1   g000(.a(x11), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x07), .O(new_n85_));
  aoi21  g002(.a(x07), .b(x04), .c(new_n85_), .O(new_n86_));
  inv1   g003(.a(x30), .O(new_n87_));
  inv1   g004(.a(x31), .O(new_n88_));
  nand2  g005(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  oai21  g006(.a(new_n86_), .b(x10), .c(new_n89_), .O(z00));
  inv1   g007(.a(x10), .O(new_n91_));
  inv1   g008(.a(x12), .O(new_n92_));
  nand2  g009(.a(x07), .b(x05), .O(new_n93_));
  oai21  g010(.a(new_n92_), .b(x07), .c(new_n93_), .O(new_n94_));
  nand3  g011(.a(new_n94_), .b(new_n89_), .c(new_n91_), .O(new_n95_));
  inv1   g012(.a(new_n95_), .O(z01));
  inv1   g013(.a(x13), .O(new_n97_));
  nand2  g014(.a(x07), .b(x06), .O(new_n98_));
  oai21  g015(.a(new_n97_), .b(x07), .c(new_n98_), .O(new_n99_));
  nand3  g016(.a(new_n99_), .b(new_n89_), .c(new_n91_), .O(new_n100_));
  inv1   g017(.a(new_n100_), .O(z02));
  inv1   g018(.a(x14), .O(new_n102_));
  nand2  g019(.a(x07), .b(x01), .O(new_n103_));
  oai21  g020(.a(new_n102_), .b(x07), .c(new_n103_), .O(new_n104_));
  nand3  g021(.a(new_n104_), .b(new_n89_), .c(new_n91_), .O(new_n105_));
  inv1   g022(.a(new_n105_), .O(z03));
  inv1   g023(.a(x15), .O(new_n107_));
  nand2  g024(.a(x07), .b(x02), .O(new_n108_));
  oai21  g025(.a(new_n107_), .b(x07), .c(new_n108_), .O(new_n109_));
  nand3  g026(.a(new_n109_), .b(new_n89_), .c(new_n91_), .O(new_n110_));
  inv1   g027(.a(new_n110_), .O(z04));
  inv1   g028(.a(x16), .O(new_n112_));
  nor2   g029(.a(new_n112_), .b(x07), .O(new_n113_));
  aoi21  g030(.a(x07), .b(x03), .c(new_n113_), .O(new_n114_));
  oai21  g031(.a(new_n114_), .b(x10), .c(new_n89_), .O(z05));
  inv1   g032(.a(x08), .O(new_n116_));
  inv1   g033(.a(x18), .O(new_n117_));
  nand2  g034(.a(x17), .b(new_n116_), .O(new_n118_));
  oai21  g035(.a(new_n117_), .b(new_n116_), .c(new_n118_), .O(new_n119_));
  nand2  g036(.a(new_n119_), .b(new_n91_), .O(new_n120_));
  nand2  g037(.a(new_n120_), .b(new_n89_), .O(z06));
  nand2  g038(.a(x19), .b(x08), .O(new_n122_));
  oai21  g039(.a(new_n117_), .b(x08), .c(new_n122_), .O(new_n123_));
  nand2  g040(.a(new_n123_), .b(new_n91_), .O(new_n124_));
  nand2  g041(.a(new_n124_), .b(new_n89_), .O(z07));
  inv1   g042(.a(x20), .O(new_n126_));
  nand2  g043(.a(x19), .b(new_n116_), .O(new_n127_));
  oai21  g044(.a(new_n126_), .b(new_n116_), .c(new_n127_), .O(new_n128_));
  nand2  g045(.a(new_n128_), .b(new_n91_), .O(new_n129_));
  nand2  g046(.a(new_n129_), .b(new_n89_), .O(z08));
  nand2  g047(.a(x21), .b(x08), .O(new_n131_));
  oai21  g048(.a(new_n126_), .b(x08), .c(new_n131_), .O(new_n132_));
  nand2  g049(.a(new_n132_), .b(new_n91_), .O(new_n133_));
  nand2  g050(.a(new_n133_), .b(new_n89_), .O(z09));
  inv1   g051(.a(x22), .O(new_n135_));
  nand2  g052(.a(x21), .b(new_n116_), .O(new_n136_));
  oai21  g053(.a(new_n135_), .b(new_n116_), .c(new_n136_), .O(new_n137_));
  nand2  g054(.a(new_n137_), .b(new_n91_), .O(new_n138_));
  nand2  g055(.a(new_n138_), .b(new_n89_), .O(z10));
  nand2  g056(.a(x23), .b(x08), .O(new_n140_));
  oai21  g057(.a(new_n135_), .b(x08), .c(new_n140_), .O(new_n141_));
  nand2  g058(.a(new_n141_), .b(new_n91_), .O(new_n142_));
  nand2  g059(.a(new_n142_), .b(new_n89_), .O(z11));
  inv1   g060(.a(x24), .O(new_n144_));
  nand2  g061(.a(x23), .b(new_n116_), .O(new_n145_));
  oai21  g062(.a(new_n144_), .b(new_n116_), .c(new_n145_), .O(new_n146_));
  nand2  g063(.a(new_n146_), .b(new_n91_), .O(new_n147_));
  nand2  g064(.a(new_n147_), .b(new_n89_), .O(z12));
  nand2  g065(.a(x25), .b(x08), .O(new_n149_));
  oai21  g066(.a(new_n144_), .b(x08), .c(new_n149_), .O(new_n150_));
  nand2  g067(.a(new_n150_), .b(new_n91_), .O(new_n151_));
  nand2  g068(.a(new_n151_), .b(new_n89_), .O(z13));
  inv1   g069(.a(x26), .O(new_n153_));
  nand2  g070(.a(x25), .b(new_n116_), .O(new_n154_));
  oai21  g071(.a(new_n153_), .b(new_n116_), .c(new_n154_), .O(new_n155_));
  nand3  g072(.a(new_n155_), .b(new_n89_), .c(new_n91_), .O(new_n156_));
  inv1   g073(.a(new_n156_), .O(z14));
  nand2  g074(.a(x27), .b(x08), .O(new_n158_));
  oai21  g075(.a(new_n153_), .b(x08), .c(new_n158_), .O(new_n159_));
  nand3  g076(.a(new_n159_), .b(new_n89_), .c(new_n91_), .O(new_n160_));
  inv1   g077(.a(new_n160_), .O(z15));
  inv1   g078(.a(x28), .O(new_n162_));
  nand2  g079(.a(x27), .b(new_n116_), .O(new_n163_));
  oai21  g080(.a(new_n162_), .b(new_n116_), .c(new_n163_), .O(new_n164_));
  nand3  g081(.a(new_n164_), .b(new_n89_), .c(new_n91_), .O(new_n165_));
  inv1   g082(.a(new_n165_), .O(z16));
  nand2  g083(.a(x29), .b(x08), .O(new_n167_));
  oai21  g084(.a(new_n162_), .b(x08), .c(new_n167_), .O(new_n168_));
  nand3  g085(.a(new_n168_), .b(new_n89_), .c(new_n91_), .O(new_n169_));
  inv1   g086(.a(new_n169_), .O(z17));
  oai21  g087(.a(new_n88_), .b(x08), .c(new_n87_), .O(new_n171_));
  nand2  g088(.a(new_n171_), .b(x29), .O(new_n172_));
  nand2  g089(.a(x30), .b(x08), .O(new_n173_));
  aoi21  g090(.a(new_n173_), .b(new_n172_), .c(x10), .O(z18));
  oai21  g091(.a(new_n88_), .b(new_n116_), .c(new_n87_), .O(new_n175_));
  nand2  g092(.a(new_n175_), .b(x00), .O(new_n176_));
  nand2  g093(.a(x30), .b(new_n116_), .O(new_n177_));
  aoi21  g094(.a(new_n177_), .b(new_n176_), .c(x10), .O(z19));
  nand2  g095(.a(x32), .b(x09), .O(new_n179_));
  oai21  g096(.a(new_n88_), .b(x09), .c(new_n179_), .O(new_n180_));
  nand2  g097(.a(new_n180_), .b(new_n91_), .O(new_n181_));
  nand2  g098(.a(new_n181_), .b(new_n89_), .O(z20));
  inv1   g099(.a(x09), .O(new_n183_));
  inv1   g100(.a(x33), .O(new_n184_));
  nand2  g101(.a(x32), .b(new_n183_), .O(new_n185_));
  oai21  g102(.a(new_n184_), .b(new_n183_), .c(new_n185_), .O(new_n186_));
  nand3  g103(.a(new_n186_), .b(new_n89_), .c(new_n91_), .O(new_n187_));
  inv1   g104(.a(new_n187_), .O(z21));
  nand2  g105(.a(x34), .b(x09), .O(new_n189_));
  oai21  g106(.a(new_n184_), .b(x09), .c(new_n189_), .O(new_n190_));
  nand3  g107(.a(new_n190_), .b(new_n89_), .c(new_n91_), .O(new_n191_));
  inv1   g108(.a(new_n191_), .O(z22));
  inv1   g109(.a(x35), .O(new_n193_));
  nand2  g110(.a(x34), .b(new_n183_), .O(new_n194_));
  oai21  g111(.a(new_n193_), .b(new_n183_), .c(new_n194_), .O(new_n195_));
  nand3  g112(.a(new_n195_), .b(new_n89_), .c(new_n91_), .O(new_n196_));
  inv1   g113(.a(new_n196_), .O(z23));
  nand2  g114(.a(x36), .b(x09), .O(new_n198_));
  oai21  g115(.a(new_n193_), .b(x09), .c(new_n198_), .O(new_n199_));
  nand2  g116(.a(new_n199_), .b(new_n91_), .O(new_n200_));
  nand2  g117(.a(new_n200_), .b(new_n89_), .O(z24));
  inv1   g118(.a(x37), .O(new_n202_));
  nand2  g119(.a(x36), .b(new_n183_), .O(new_n203_));
  oai21  g120(.a(new_n202_), .b(new_n183_), .c(new_n203_), .O(new_n204_));
  nand2  g121(.a(new_n204_), .b(new_n91_), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(new_n89_), .O(z25));
  nand2  g123(.a(x38), .b(x09), .O(new_n207_));
  oai21  g124(.a(new_n202_), .b(x09), .c(new_n207_), .O(new_n208_));
  nand3  g125(.a(new_n208_), .b(new_n89_), .c(new_n91_), .O(new_n209_));
  inv1   g126(.a(new_n209_), .O(z26));
  inv1   g127(.a(x39), .O(new_n211_));
  nand2  g128(.a(x14), .b(x00), .O(new_n212_));
  oai21  g129(.a(new_n211_), .b(x14), .c(new_n212_), .O(new_n213_));
  inv1   g130(.a(x38), .O(new_n214_));
  nor2   g131(.a(new_n214_), .b(x09), .O(new_n215_));
  aoi21  g132(.a(new_n213_), .b(x09), .c(new_n215_), .O(new_n216_));
  oai21  g133(.a(new_n216_), .b(x10), .c(new_n89_), .O(z27));
  oai21  g134(.a(x14), .b(new_n183_), .c(x39), .O(new_n218_));
  nand3  g135(.a(x40), .b(new_n102_), .c(x09), .O(new_n219_));
  nand2  g136(.a(new_n219_), .b(new_n218_), .O(new_n220_));
  nand3  g137(.a(new_n220_), .b(new_n89_), .c(new_n91_), .O(new_n221_));
  inv1   g138(.a(new_n221_), .O(z28));
  oai21  g139(.a(x14), .b(new_n183_), .c(x40), .O(new_n223_));
  nand3  g140(.a(x41), .b(new_n102_), .c(x09), .O(new_n224_));
  nand2  g141(.a(new_n224_), .b(new_n223_), .O(new_n225_));
  nand3  g142(.a(new_n225_), .b(new_n89_), .c(new_n91_), .O(new_n226_));
  inv1   g143(.a(new_n226_), .O(z29));
  oai21  g144(.a(x14), .b(new_n183_), .c(x41), .O(new_n228_));
  nand3  g145(.a(x42), .b(new_n102_), .c(x09), .O(new_n229_));
  nand2  g146(.a(new_n229_), .b(new_n228_), .O(new_n230_));
  nand3  g147(.a(new_n230_), .b(new_n89_), .c(new_n91_), .O(new_n231_));
  inv1   g148(.a(new_n231_), .O(z30));
  inv1   g149(.a(x42), .O(new_n233_));
  aoi21  g150(.a(new_n102_), .b(x09), .c(new_n233_), .O(new_n234_));
  nand3  g151(.a(x43), .b(new_n102_), .c(x09), .O(new_n235_));
  inv1   g152(.a(new_n235_), .O(new_n236_));
  oai21  g153(.a(new_n236_), .b(new_n234_), .c(new_n91_), .O(new_n237_));
  nand2  g154(.a(new_n237_), .b(new_n89_), .O(z31));
  inv1   g155(.a(x43), .O(new_n239_));
  aoi21  g156(.a(new_n102_), .b(x09), .c(new_n239_), .O(new_n240_));
  nand3  g157(.a(x44), .b(new_n102_), .c(x09), .O(new_n241_));
  inv1   g158(.a(new_n241_), .O(new_n242_));
  oai21  g159(.a(new_n242_), .b(new_n240_), .c(new_n91_), .O(new_n243_));
  nand2  g160(.a(new_n243_), .b(new_n89_), .O(z32));
  oai21  g161(.a(x14), .b(new_n183_), .c(x44), .O(new_n245_));
  nand3  g162(.a(x45), .b(new_n102_), .c(x09), .O(new_n246_));
  nand2  g163(.a(new_n246_), .b(new_n245_), .O(new_n247_));
  nand3  g164(.a(new_n247_), .b(new_n89_), .c(new_n91_), .O(new_n248_));
  inv1   g165(.a(new_n248_), .O(z33));
  oai21  g166(.a(x14), .b(new_n183_), .c(x45), .O(new_n250_));
  nand3  g167(.a(x46), .b(new_n102_), .c(x09), .O(new_n251_));
  nand2  g168(.a(new_n251_), .b(new_n250_), .O(new_n252_));
  nand3  g169(.a(new_n252_), .b(new_n89_), .c(new_n91_), .O(new_n253_));
  inv1   g170(.a(new_n253_), .O(z34));
  inv1   g171(.a(x46), .O(new_n255_));
  aoi21  g172(.a(new_n102_), .b(x09), .c(new_n255_), .O(new_n256_));
  nand3  g173(.a(new_n102_), .b(x09), .c(x00), .O(new_n257_));
  inv1   g174(.a(new_n257_), .O(new_n258_));
  oai21  g175(.a(new_n258_), .b(new_n256_), .c(new_n91_), .O(new_n259_));
  nand2  g176(.a(new_n259_), .b(new_n89_), .O(z35));
endmodule


