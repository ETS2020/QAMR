// Benchmark "FAU" written by ABC on Tue Aug 18 19:06:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n146_, new_n147_,
    new_n148_, new_n149_, new_n151_, new_n152_, new_n153_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n160_, new_n161_, new_n162_,
    new_n164_, new_n165_, new_n166_, new_n168_, new_n169_, new_n170_,
    new_n172_, new_n173_, new_n174_, new_n175_, new_n177_, new_n178_,
    new_n179_, new_n180_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n190_, new_n191_, new_n192_, new_n193_,
    new_n195_, new_n196_, new_n197_, new_n199_, new_n200_, new_n201_,
    new_n203_, new_n204_, new_n205_, new_n206_, new_n207_, new_n209_,
    new_n210_, new_n211_, new_n212_, new_n213_, new_n214_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n221_, new_n222_, new_n223_,
    new_n224_, new_n226_, new_n227_, new_n228_, new_n229_, new_n231_,
    new_n232_, new_n233_, new_n234_, new_n236_, new_n237_, new_n238_,
    new_n239_, new_n241_, new_n242_, new_n243_, new_n244_, new_n246_,
    new_n247_, new_n248_;
  inv1   g000(.a(x10), .O(new_n84_));
  nand2  g001(.a(x07), .b(x04), .O(new_n85_));
  inv1   g002(.a(x07), .O(new_n86_));
  nand2  g003(.a(x11), .b(new_n86_), .O(new_n87_));
  nand3  g004(.a(new_n87_), .b(new_n85_), .c(x01), .O(new_n88_));
  and2   g005(.a(new_n88_), .b(new_n84_), .O(z00));
  inv1   g006(.a(x12), .O(new_n90_));
  nand2  g007(.a(x07), .b(x05), .O(new_n91_));
  oai21  g008(.a(new_n90_), .b(x07), .c(new_n91_), .O(new_n92_));
  nand3  g009(.a(new_n92_), .b(new_n84_), .c(x01), .O(new_n93_));
  inv1   g010(.a(new_n93_), .O(z01));
  nand2  g011(.a(x07), .b(x06), .O(new_n95_));
  nand2  g012(.a(x13), .b(new_n86_), .O(new_n96_));
  nand3  g013(.a(new_n96_), .b(new_n95_), .c(x01), .O(new_n97_));
  and2   g014(.a(new_n97_), .b(new_n84_), .O(z02));
  inv1   g015(.a(x01), .O(new_n99_));
  nor2   g016(.a(x14), .b(x07), .O(new_n100_));
  nor3   g017(.a(new_n100_), .b(x10), .c(new_n99_), .O(z03));
  inv1   g018(.a(x15), .O(new_n102_));
  nand2  g019(.a(x07), .b(x02), .O(new_n103_));
  oai21  g020(.a(new_n102_), .b(x07), .c(new_n103_), .O(new_n104_));
  nand3  g021(.a(new_n104_), .b(new_n84_), .c(x01), .O(new_n105_));
  inv1   g022(.a(new_n105_), .O(z04));
  nand2  g023(.a(x07), .b(x03), .O(new_n107_));
  nand2  g024(.a(x16), .b(new_n86_), .O(new_n108_));
  nand3  g025(.a(new_n108_), .b(new_n107_), .c(x01), .O(new_n109_));
  and2   g026(.a(new_n109_), .b(new_n84_), .O(z05));
  inv1   g027(.a(x17), .O(new_n111_));
  nand2  g028(.a(x18), .b(x08), .O(new_n112_));
  oai21  g029(.a(new_n111_), .b(x08), .c(new_n112_), .O(new_n113_));
  nand3  g030(.a(new_n113_), .b(new_n84_), .c(x01), .O(new_n114_));
  inv1   g031(.a(new_n114_), .O(z06));
  inv1   g032(.a(x08), .O(new_n116_));
  nand2  g033(.a(x18), .b(new_n116_), .O(new_n117_));
  nand2  g034(.a(x19), .b(x08), .O(new_n118_));
  nand3  g035(.a(new_n118_), .b(new_n117_), .c(x01), .O(new_n119_));
  and2   g036(.a(new_n119_), .b(new_n84_), .O(z07));
  nand2  g037(.a(x19), .b(new_n116_), .O(new_n121_));
  nand2  g038(.a(x20), .b(x08), .O(new_n122_));
  nand3  g039(.a(new_n122_), .b(new_n121_), .c(x01), .O(new_n123_));
  and2   g040(.a(new_n123_), .b(new_n84_), .O(z08));
  nand2  g041(.a(x20), .b(new_n116_), .O(new_n125_));
  nand2  g042(.a(x21), .b(x08), .O(new_n126_));
  nand3  g043(.a(new_n126_), .b(new_n125_), .c(x01), .O(new_n127_));
  and2   g044(.a(new_n127_), .b(new_n84_), .O(z09));
  nand2  g045(.a(x21), .b(new_n116_), .O(new_n129_));
  nand2  g046(.a(x22), .b(x08), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n129_), .c(x01), .O(new_n131_));
  and2   g048(.a(new_n131_), .b(new_n84_), .O(z10));
  nand2  g049(.a(x22), .b(new_n116_), .O(new_n133_));
  nand2  g050(.a(x23), .b(x08), .O(new_n134_));
  nand3  g051(.a(new_n134_), .b(new_n133_), .c(x01), .O(new_n135_));
  and2   g052(.a(new_n135_), .b(new_n84_), .O(z11));
  inv1   g053(.a(x23), .O(new_n137_));
  nand2  g054(.a(x24), .b(x08), .O(new_n138_));
  oai21  g055(.a(new_n137_), .b(x08), .c(new_n138_), .O(new_n139_));
  nand3  g056(.a(new_n139_), .b(new_n84_), .c(x01), .O(new_n140_));
  inv1   g057(.a(new_n140_), .O(z12));
  nand2  g058(.a(x24), .b(new_n116_), .O(new_n142_));
  nand2  g059(.a(x25), .b(x08), .O(new_n143_));
  nand3  g060(.a(new_n143_), .b(new_n142_), .c(x01), .O(new_n144_));
  and2   g061(.a(new_n144_), .b(new_n84_), .O(z13));
  inv1   g062(.a(x26), .O(new_n146_));
  nand2  g063(.a(x25), .b(new_n116_), .O(new_n147_));
  oai21  g064(.a(new_n146_), .b(new_n116_), .c(new_n147_), .O(new_n148_));
  nand3  g065(.a(new_n148_), .b(new_n84_), .c(x01), .O(new_n149_));
  inv1   g066(.a(new_n149_), .O(z14));
  nand2  g067(.a(x27), .b(x08), .O(new_n151_));
  oai21  g068(.a(new_n146_), .b(x08), .c(new_n151_), .O(new_n152_));
  nand3  g069(.a(new_n152_), .b(new_n84_), .c(x01), .O(new_n153_));
  inv1   g070(.a(new_n153_), .O(z15));
  inv1   g071(.a(x28), .O(new_n155_));
  nand2  g072(.a(x27), .b(new_n116_), .O(new_n156_));
  oai21  g073(.a(new_n155_), .b(new_n116_), .c(new_n156_), .O(new_n157_));
  nand3  g074(.a(new_n157_), .b(new_n84_), .c(x01), .O(new_n158_));
  inv1   g075(.a(new_n158_), .O(z16));
  nand2  g076(.a(x29), .b(x08), .O(new_n160_));
  oai21  g077(.a(new_n155_), .b(x08), .c(new_n160_), .O(new_n161_));
  nand3  g078(.a(new_n161_), .b(new_n84_), .c(x01), .O(new_n162_));
  inv1   g079(.a(new_n162_), .O(z17));
  nand2  g080(.a(x29), .b(new_n116_), .O(new_n164_));
  nand2  g081(.a(x30), .b(x08), .O(new_n165_));
  nand3  g082(.a(new_n165_), .b(new_n164_), .c(x01), .O(new_n166_));
  and2   g083(.a(new_n166_), .b(new_n84_), .O(z18));
  nand2  g084(.a(x08), .b(x00), .O(new_n168_));
  nand2  g085(.a(x30), .b(new_n116_), .O(new_n169_));
  nand3  g086(.a(new_n169_), .b(new_n168_), .c(x01), .O(new_n170_));
  and2   g087(.a(new_n170_), .b(new_n84_), .O(z19));
  inv1   g088(.a(x31), .O(new_n172_));
  nand2  g089(.a(x32), .b(x09), .O(new_n173_));
  oai21  g090(.a(new_n172_), .b(x09), .c(new_n173_), .O(new_n174_));
  nand3  g091(.a(new_n174_), .b(new_n84_), .c(x01), .O(new_n175_));
  inv1   g092(.a(new_n175_), .O(z20));
  inv1   g093(.a(x09), .O(new_n177_));
  nand2  g094(.a(x32), .b(new_n177_), .O(new_n178_));
  nand2  g095(.a(x33), .b(x09), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n178_), .c(x01), .O(new_n180_));
  and2   g097(.a(new_n180_), .b(new_n84_), .O(z21));
  nand2  g098(.a(x33), .b(new_n177_), .O(new_n182_));
  nand2  g099(.a(x34), .b(x09), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n182_), .c(x01), .O(new_n184_));
  and2   g101(.a(new_n184_), .b(new_n84_), .O(z22));
  nand2  g102(.a(x34), .b(new_n177_), .O(new_n186_));
  nand2  g103(.a(x35), .b(x09), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(x01), .O(new_n188_));
  and2   g105(.a(new_n188_), .b(new_n84_), .O(z23));
  inv1   g106(.a(x35), .O(new_n190_));
  nand2  g107(.a(x36), .b(x09), .O(new_n191_));
  oai21  g108(.a(new_n190_), .b(x09), .c(new_n191_), .O(new_n192_));
  nand3  g109(.a(new_n192_), .b(new_n84_), .c(x01), .O(new_n193_));
  inv1   g110(.a(new_n193_), .O(z24));
  nand2  g111(.a(x36), .b(new_n177_), .O(new_n195_));
  nand2  g112(.a(x37), .b(x09), .O(new_n196_));
  nand3  g113(.a(new_n196_), .b(new_n195_), .c(x01), .O(new_n197_));
  and2   g114(.a(new_n197_), .b(new_n84_), .O(z25));
  nand2  g115(.a(x37), .b(new_n177_), .O(new_n199_));
  nand2  g116(.a(x38), .b(x09), .O(new_n200_));
  nand3  g117(.a(new_n200_), .b(new_n199_), .c(x01), .O(new_n201_));
  and2   g118(.a(new_n201_), .b(new_n84_), .O(z26));
  inv1   g119(.a(x39), .O(new_n203_));
  nand2  g120(.a(x14), .b(x00), .O(new_n204_));
  oai21  g121(.a(new_n203_), .b(x14), .c(new_n204_), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(x09), .O(new_n206_));
  aoi21  g123(.a(x38), .b(new_n177_), .c(new_n99_), .O(new_n207_));
  aoi21  g124(.a(new_n207_), .b(new_n206_), .c(x10), .O(z27));
  inv1   g125(.a(x14), .O(new_n209_));
  nand2  g126(.a(new_n209_), .b(x09), .O(new_n210_));
  nand2  g127(.a(new_n210_), .b(x39), .O(new_n211_));
  inv1   g128(.a(x40), .O(new_n212_));
  nor2   g129(.a(new_n212_), .b(x14), .O(new_n213_));
  aoi21  g130(.a(new_n213_), .b(x09), .c(new_n99_), .O(new_n214_));
  aoi21  g131(.a(new_n214_), .b(new_n211_), .c(x10), .O(z28));
  nand2  g132(.a(new_n210_), .b(x40), .O(new_n216_));
  inv1   g133(.a(x41), .O(new_n217_));
  nor2   g134(.a(new_n217_), .b(x14), .O(new_n218_));
  aoi21  g135(.a(new_n218_), .b(x09), .c(new_n99_), .O(new_n219_));
  aoi21  g136(.a(new_n219_), .b(new_n216_), .c(x10), .O(z29));
  oai21  g137(.a(x14), .b(new_n177_), .c(x41), .O(new_n221_));
  nand3  g138(.a(x42), .b(new_n209_), .c(x09), .O(new_n222_));
  nand2  g139(.a(new_n222_), .b(new_n221_), .O(new_n223_));
  nand3  g140(.a(new_n223_), .b(new_n84_), .c(x01), .O(new_n224_));
  inv1   g141(.a(new_n224_), .O(z30));
  oai21  g142(.a(x14), .b(new_n177_), .c(x42), .O(new_n226_));
  nand3  g143(.a(x43), .b(new_n209_), .c(x09), .O(new_n227_));
  nand2  g144(.a(new_n227_), .b(new_n226_), .O(new_n228_));
  nand3  g145(.a(new_n228_), .b(new_n84_), .c(x01), .O(new_n229_));
  inv1   g146(.a(new_n229_), .O(z31));
  oai21  g147(.a(x14), .b(new_n177_), .c(x43), .O(new_n231_));
  nand3  g148(.a(x44), .b(new_n209_), .c(x09), .O(new_n232_));
  nand2  g149(.a(new_n232_), .b(new_n231_), .O(new_n233_));
  nand3  g150(.a(new_n233_), .b(new_n84_), .c(x01), .O(new_n234_));
  inv1   g151(.a(new_n234_), .O(z32));
  nand2  g152(.a(new_n210_), .b(x44), .O(new_n236_));
  inv1   g153(.a(x45), .O(new_n237_));
  nor2   g154(.a(new_n237_), .b(x14), .O(new_n238_));
  aoi21  g155(.a(new_n238_), .b(x09), .c(new_n99_), .O(new_n239_));
  aoi21  g156(.a(new_n239_), .b(new_n236_), .c(x10), .O(z33));
  nand2  g157(.a(new_n210_), .b(x45), .O(new_n241_));
  inv1   g158(.a(x46), .O(new_n242_));
  nor2   g159(.a(new_n242_), .b(x14), .O(new_n243_));
  aoi21  g160(.a(new_n243_), .b(x09), .c(new_n99_), .O(new_n244_));
  aoi21  g161(.a(new_n244_), .b(new_n241_), .c(x10), .O(z34));
  nand2  g162(.a(new_n210_), .b(x46), .O(new_n246_));
  nor2   g163(.a(x14), .b(new_n177_), .O(new_n247_));
  aoi21  g164(.a(new_n247_), .b(x00), .c(new_n99_), .O(new_n248_));
  aoi21  g165(.a(new_n248_), .b(new_n246_), .c(x10), .O(z35));
endmodule


