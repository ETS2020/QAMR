// Benchmark "FAU" written by ABC on Tue Aug 18 19:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n85_, new_n87_, new_n89_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_, new_n171_, new_n172_,
    new_n173_, new_n174_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n184_, new_n185_, new_n186_, new_n188_,
    new_n189_, new_n190_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n196_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n214_, new_n215_, new_n216_, new_n217_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n224_, new_n225_,
    new_n226_, new_n227_, new_n229_, new_n230_, new_n231_, new_n232_,
    new_n234_, new_n235_, new_n236_, new_n237_;
  nor2   g000(.a(x10), .b(x07), .O(new_n84_));
  nand2  g001(.a(new_n84_), .b(x11), .O(new_n85_));
  inv1   g002(.a(new_n85_), .O(z00));
  nand2  g003(.a(new_n84_), .b(x12), .O(new_n87_));
  inv1   g004(.a(new_n87_), .O(z01));
  nand2  g005(.a(new_n84_), .b(x13), .O(new_n89_));
  inv1   g006(.a(new_n89_), .O(z02));
  inv1   g007(.a(x07), .O(new_n91_));
  inv1   g008(.a(x14), .O(new_n92_));
  aoi21  g009(.a(new_n92_), .b(new_n91_), .c(x10), .O(z03));
  inv1   g010(.a(x15), .O(new_n94_));
  aoi21  g011(.a(new_n94_), .b(new_n91_), .c(x10), .O(z04));
  nand2  g012(.a(new_n84_), .b(x16), .O(new_n96_));
  inv1   g013(.a(new_n96_), .O(z05));
  inv1   g014(.a(x10), .O(new_n98_));
  inv1   g015(.a(x08), .O(new_n99_));
  nand2  g016(.a(x17), .b(new_n99_), .O(new_n100_));
  nand2  g017(.a(x18), .b(x08), .O(new_n101_));
  nand3  g018(.a(new_n101_), .b(new_n100_), .c(new_n91_), .O(new_n102_));
  and2   g019(.a(new_n102_), .b(new_n98_), .O(z06));
  nand2  g020(.a(x18), .b(new_n99_), .O(new_n104_));
  nand2  g021(.a(x19), .b(x08), .O(new_n105_));
  nand3  g022(.a(new_n105_), .b(new_n104_), .c(new_n91_), .O(new_n106_));
  and2   g023(.a(new_n106_), .b(new_n98_), .O(z07));
  nand2  g024(.a(x19), .b(new_n99_), .O(new_n108_));
  nand2  g025(.a(x20), .b(x08), .O(new_n109_));
  nand3  g026(.a(new_n109_), .b(new_n108_), .c(new_n91_), .O(new_n110_));
  and2   g027(.a(new_n110_), .b(new_n98_), .O(z08));
  inv1   g028(.a(x20), .O(new_n112_));
  nand2  g029(.a(x21), .b(x08), .O(new_n113_));
  oai21  g030(.a(new_n112_), .b(x08), .c(new_n113_), .O(new_n114_));
  nand3  g031(.a(new_n114_), .b(new_n98_), .c(new_n91_), .O(new_n115_));
  inv1   g032(.a(new_n115_), .O(z09));
  nand2  g033(.a(x21), .b(new_n99_), .O(new_n117_));
  nand2  g034(.a(x22), .b(x08), .O(new_n118_));
  nand3  g035(.a(new_n118_), .b(new_n117_), .c(new_n91_), .O(new_n119_));
  and2   g036(.a(new_n119_), .b(new_n98_), .O(z10));
  inv1   g037(.a(x23), .O(new_n121_));
  nand2  g038(.a(x22), .b(new_n99_), .O(new_n122_));
  oai21  g039(.a(new_n121_), .b(new_n99_), .c(new_n122_), .O(new_n123_));
  nand3  g040(.a(new_n123_), .b(new_n98_), .c(new_n91_), .O(new_n124_));
  inv1   g041(.a(new_n124_), .O(z11));
  nand2  g042(.a(x24), .b(x08), .O(new_n126_));
  oai21  g043(.a(new_n121_), .b(x08), .c(new_n126_), .O(new_n127_));
  nand3  g044(.a(new_n127_), .b(new_n98_), .c(new_n91_), .O(new_n128_));
  inv1   g045(.a(new_n128_), .O(z12));
  inv1   g046(.a(x25), .O(new_n130_));
  nand2  g047(.a(x24), .b(new_n99_), .O(new_n131_));
  oai21  g048(.a(new_n130_), .b(new_n99_), .c(new_n131_), .O(new_n132_));
  nand3  g049(.a(new_n132_), .b(new_n98_), .c(new_n91_), .O(new_n133_));
  inv1   g050(.a(new_n133_), .O(z13));
  nand2  g051(.a(x26), .b(x08), .O(new_n135_));
  oai21  g052(.a(new_n130_), .b(x08), .c(new_n135_), .O(new_n136_));
  nand3  g053(.a(new_n136_), .b(new_n98_), .c(new_n91_), .O(new_n137_));
  inv1   g054(.a(new_n137_), .O(z14));
  nand2  g055(.a(x26), .b(new_n99_), .O(new_n139_));
  nand2  g056(.a(x27), .b(x08), .O(new_n140_));
  nand3  g057(.a(new_n140_), .b(new_n139_), .c(new_n91_), .O(new_n141_));
  and2   g058(.a(new_n141_), .b(new_n98_), .O(z15));
  nand2  g059(.a(x27), .b(new_n99_), .O(new_n143_));
  nand2  g060(.a(x28), .b(x08), .O(new_n144_));
  nand3  g061(.a(new_n144_), .b(new_n143_), .c(new_n91_), .O(new_n145_));
  and2   g062(.a(new_n145_), .b(new_n98_), .O(z16));
  inv1   g063(.a(x28), .O(new_n147_));
  nand2  g064(.a(x29), .b(x08), .O(new_n148_));
  oai21  g065(.a(new_n147_), .b(x08), .c(new_n148_), .O(new_n149_));
  nand3  g066(.a(new_n149_), .b(new_n98_), .c(new_n91_), .O(new_n150_));
  inv1   g067(.a(new_n150_), .O(z17));
  nand2  g068(.a(x29), .b(new_n99_), .O(new_n152_));
  nand2  g069(.a(x30), .b(x08), .O(new_n153_));
  nand3  g070(.a(new_n153_), .b(new_n152_), .c(new_n91_), .O(new_n154_));
  and2   g071(.a(new_n154_), .b(new_n98_), .O(z18));
  inv1   g072(.a(x30), .O(new_n156_));
  nand2  g073(.a(x08), .b(x00), .O(new_n157_));
  oai21  g074(.a(new_n156_), .b(x08), .c(new_n157_), .O(new_n158_));
  nand3  g075(.a(new_n158_), .b(new_n98_), .c(new_n91_), .O(new_n159_));
  inv1   g076(.a(new_n159_), .O(z19));
  inv1   g077(.a(x31), .O(new_n161_));
  nand2  g078(.a(x32), .b(x09), .O(new_n162_));
  oai21  g079(.a(new_n161_), .b(x09), .c(new_n162_), .O(new_n163_));
  nand3  g080(.a(new_n163_), .b(new_n98_), .c(new_n91_), .O(new_n164_));
  inv1   g081(.a(new_n164_), .O(z20));
  inv1   g082(.a(x09), .O(new_n166_));
  nand2  g083(.a(x32), .b(new_n166_), .O(new_n167_));
  nand2  g084(.a(x33), .b(x09), .O(new_n168_));
  nand3  g085(.a(new_n168_), .b(new_n167_), .c(new_n91_), .O(new_n169_));
  and2   g086(.a(new_n169_), .b(new_n98_), .O(z21));
  inv1   g087(.a(x34), .O(new_n171_));
  nand2  g088(.a(x33), .b(new_n166_), .O(new_n172_));
  oai21  g089(.a(new_n171_), .b(new_n166_), .c(new_n172_), .O(new_n173_));
  nand3  g090(.a(new_n173_), .b(new_n98_), .c(new_n91_), .O(new_n174_));
  inv1   g091(.a(new_n174_), .O(z22));
  nand2  g092(.a(x35), .b(x09), .O(new_n176_));
  oai21  g093(.a(new_n171_), .b(x09), .c(new_n176_), .O(new_n177_));
  nand3  g094(.a(new_n177_), .b(new_n98_), .c(new_n91_), .O(new_n178_));
  inv1   g095(.a(new_n178_), .O(z23));
  nand2  g096(.a(x35), .b(new_n166_), .O(new_n180_));
  nand2  g097(.a(x36), .b(x09), .O(new_n181_));
  nand3  g098(.a(new_n181_), .b(new_n180_), .c(new_n91_), .O(new_n182_));
  and2   g099(.a(new_n182_), .b(new_n98_), .O(z24));
  nand2  g100(.a(x36), .b(new_n166_), .O(new_n184_));
  nand2  g101(.a(x37), .b(x09), .O(new_n185_));
  nand3  g102(.a(new_n185_), .b(new_n184_), .c(new_n91_), .O(new_n186_));
  and2   g103(.a(new_n186_), .b(new_n98_), .O(z25));
  nand2  g104(.a(x37), .b(new_n166_), .O(new_n188_));
  nand2  g105(.a(x38), .b(x09), .O(new_n189_));
  nand3  g106(.a(new_n189_), .b(new_n188_), .c(new_n91_), .O(new_n190_));
  and2   g107(.a(new_n190_), .b(new_n98_), .O(z26));
  inv1   g108(.a(x39), .O(new_n192_));
  nand2  g109(.a(x14), .b(x00), .O(new_n193_));
  oai21  g110(.a(new_n192_), .b(x14), .c(new_n193_), .O(new_n194_));
  nand2  g111(.a(new_n194_), .b(x09), .O(new_n195_));
  aoi21  g112(.a(x38), .b(new_n166_), .c(x07), .O(new_n196_));
  aoi21  g113(.a(new_n196_), .b(new_n195_), .c(x10), .O(z27));
  nand2  g114(.a(new_n92_), .b(x09), .O(new_n198_));
  nand2  g115(.a(new_n198_), .b(x39), .O(new_n199_));
  inv1   g116(.a(x40), .O(new_n200_));
  nor2   g117(.a(new_n200_), .b(x14), .O(new_n201_));
  aoi21  g118(.a(new_n201_), .b(x09), .c(x07), .O(new_n202_));
  aoi21  g119(.a(new_n202_), .b(new_n199_), .c(x10), .O(z28));
  oai21  g120(.a(x14), .b(new_n166_), .c(x40), .O(new_n204_));
  nand3  g121(.a(x41), .b(new_n92_), .c(x09), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n98_), .c(new_n91_), .O(new_n207_));
  inv1   g124(.a(new_n207_), .O(z29));
  nand2  g125(.a(new_n198_), .b(x41), .O(new_n209_));
  inv1   g126(.a(x42), .O(new_n210_));
  nor2   g127(.a(new_n210_), .b(x14), .O(new_n211_));
  aoi21  g128(.a(new_n211_), .b(x09), .c(x07), .O(new_n212_));
  aoi21  g129(.a(new_n212_), .b(new_n209_), .c(x10), .O(z30));
  nand2  g130(.a(new_n198_), .b(x42), .O(new_n214_));
  inv1   g131(.a(x43), .O(new_n215_));
  nor2   g132(.a(new_n215_), .b(x14), .O(new_n216_));
  aoi21  g133(.a(new_n216_), .b(x09), .c(x07), .O(new_n217_));
  aoi21  g134(.a(new_n217_), .b(new_n214_), .c(x10), .O(z31));
  nand2  g135(.a(new_n198_), .b(x43), .O(new_n219_));
  inv1   g136(.a(x44), .O(new_n220_));
  nor2   g137(.a(new_n220_), .b(x14), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(x09), .c(x07), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n219_), .c(x10), .O(z32));
  oai21  g140(.a(x14), .b(new_n166_), .c(x44), .O(new_n224_));
  nand3  g141(.a(x45), .b(new_n92_), .c(x09), .O(new_n225_));
  nand2  g142(.a(new_n225_), .b(new_n224_), .O(new_n226_));
  nand3  g143(.a(new_n226_), .b(new_n98_), .c(new_n91_), .O(new_n227_));
  inv1   g144(.a(new_n227_), .O(z33));
  oai21  g145(.a(x14), .b(new_n166_), .c(x45), .O(new_n229_));
  nand3  g146(.a(x46), .b(new_n92_), .c(x09), .O(new_n230_));
  nand2  g147(.a(new_n230_), .b(new_n229_), .O(new_n231_));
  nand3  g148(.a(new_n231_), .b(new_n98_), .c(new_n91_), .O(new_n232_));
  inv1   g149(.a(new_n232_), .O(z34));
  oai21  g150(.a(x14), .b(new_n166_), .c(x46), .O(new_n234_));
  nand3  g151(.a(new_n92_), .b(x09), .c(x00), .O(new_n235_));
  nand2  g152(.a(new_n235_), .b(new_n234_), .O(new_n236_));
  nand3  g153(.a(new_n236_), .b(new_n98_), .c(new_n91_), .O(new_n237_));
  inv1   g154(.a(new_n237_), .O(z35));
endmodule


