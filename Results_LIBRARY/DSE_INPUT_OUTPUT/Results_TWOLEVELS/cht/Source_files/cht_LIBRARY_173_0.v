// Benchmark "FAU" written by ABC on Tue Aug 18 19:07:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41,
    x42, x43, x44, x45, x46,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40,
    x41, x42, x43, x44, x45, x46;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17, z18, z19, z20, z21, z22, z23, z24, z25, z26, z27,
    z28, z29, z30, z31, z32, z33, z34, z35;
  wire new_n84_, new_n86_, new_n88_, new_n90_, new_n92_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n152_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_, new_n169_, new_n170_, new_n171_, new_n172_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n182_, new_n183_, new_n184_, new_n186_, new_n187_, new_n188_,
    new_n190_, new_n191_, new_n192_, new_n193_, new_n194_, new_n195_,
    new_n197_, new_n198_, new_n199_, new_n200_, new_n201_, new_n202_,
    new_n204_, new_n205_, new_n206_, new_n207_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n214_, new_n215_, new_n216_, new_n217_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n224_, new_n225_,
    new_n226_, new_n227_, new_n229_, new_n230_, new_n231_, new_n232_,
    new_n234_, new_n235_, new_n236_, new_n237_;
  nand2  g000(.a(x07), .b(x04), .O(new_n84_));
  nor2   g001(.a(new_n84_), .b(x10), .O(z00));
  nand2  g002(.a(x07), .b(x05), .O(new_n86_));
  nor2   g003(.a(new_n86_), .b(x10), .O(z01));
  nand2  g004(.a(x07), .b(x06), .O(new_n88_));
  nor2   g005(.a(new_n88_), .b(x10), .O(z02));
  inv1   g006(.a(x01), .O(new_n90_));
  aoi21  g007(.a(x07), .b(new_n90_), .c(x10), .O(z03));
  inv1   g008(.a(x02), .O(new_n92_));
  aoi21  g009(.a(x07), .b(new_n92_), .c(x10), .O(z04));
  nand2  g010(.a(x07), .b(x03), .O(new_n94_));
  nor2   g011(.a(new_n94_), .b(x10), .O(z05));
  inv1   g012(.a(x10), .O(new_n96_));
  inv1   g013(.a(x08), .O(new_n97_));
  nand2  g014(.a(x17), .b(new_n97_), .O(new_n98_));
  nand2  g015(.a(x18), .b(x08), .O(new_n99_));
  nand3  g016(.a(new_n99_), .b(new_n98_), .c(x07), .O(new_n100_));
  and2   g017(.a(new_n100_), .b(new_n96_), .O(z06));
  nand2  g018(.a(x18), .b(new_n97_), .O(new_n102_));
  nand2  g019(.a(x19), .b(x08), .O(new_n103_));
  nand3  g020(.a(new_n103_), .b(new_n102_), .c(x07), .O(new_n104_));
  and2   g021(.a(new_n104_), .b(new_n96_), .O(z07));
  nand2  g022(.a(x19), .b(new_n97_), .O(new_n106_));
  nand2  g023(.a(x20), .b(x08), .O(new_n107_));
  nand3  g024(.a(new_n107_), .b(new_n106_), .c(x07), .O(new_n108_));
  and2   g025(.a(new_n108_), .b(new_n96_), .O(z08));
  inv1   g026(.a(x20), .O(new_n110_));
  nand2  g027(.a(x21), .b(x08), .O(new_n111_));
  oai21  g028(.a(new_n110_), .b(x08), .c(new_n111_), .O(new_n112_));
  nand3  g029(.a(new_n112_), .b(new_n96_), .c(x07), .O(new_n113_));
  inv1   g030(.a(new_n113_), .O(z09));
  nand2  g031(.a(x21), .b(new_n97_), .O(new_n115_));
  nand2  g032(.a(x22), .b(x08), .O(new_n116_));
  nand3  g033(.a(new_n116_), .b(new_n115_), .c(x07), .O(new_n117_));
  and2   g034(.a(new_n117_), .b(new_n96_), .O(z10));
  inv1   g035(.a(x23), .O(new_n119_));
  nand2  g036(.a(x22), .b(new_n97_), .O(new_n120_));
  oai21  g037(.a(new_n119_), .b(new_n97_), .c(new_n120_), .O(new_n121_));
  nand3  g038(.a(new_n121_), .b(new_n96_), .c(x07), .O(new_n122_));
  inv1   g039(.a(new_n122_), .O(z11));
  nand2  g040(.a(x24), .b(x08), .O(new_n124_));
  oai21  g041(.a(new_n119_), .b(x08), .c(new_n124_), .O(new_n125_));
  nand3  g042(.a(new_n125_), .b(new_n96_), .c(x07), .O(new_n126_));
  inv1   g043(.a(new_n126_), .O(z12));
  inv1   g044(.a(x25), .O(new_n128_));
  nand2  g045(.a(x24), .b(new_n97_), .O(new_n129_));
  oai21  g046(.a(new_n128_), .b(new_n97_), .c(new_n129_), .O(new_n130_));
  nand3  g047(.a(new_n130_), .b(new_n96_), .c(x07), .O(new_n131_));
  inv1   g048(.a(new_n131_), .O(z13));
  nand2  g049(.a(x26), .b(x08), .O(new_n133_));
  oai21  g050(.a(new_n128_), .b(x08), .c(new_n133_), .O(new_n134_));
  nand3  g051(.a(new_n134_), .b(new_n96_), .c(x07), .O(new_n135_));
  inv1   g052(.a(new_n135_), .O(z14));
  nand2  g053(.a(x26), .b(new_n97_), .O(new_n137_));
  nand2  g054(.a(x27), .b(x08), .O(new_n138_));
  nand3  g055(.a(new_n138_), .b(new_n137_), .c(x07), .O(new_n139_));
  and2   g056(.a(new_n139_), .b(new_n96_), .O(z15));
  nand2  g057(.a(x27), .b(new_n97_), .O(new_n141_));
  nand2  g058(.a(x28), .b(x08), .O(new_n142_));
  nand3  g059(.a(new_n142_), .b(new_n141_), .c(x07), .O(new_n143_));
  and2   g060(.a(new_n143_), .b(new_n96_), .O(z16));
  inv1   g061(.a(x28), .O(new_n145_));
  nand2  g062(.a(x29), .b(x08), .O(new_n146_));
  oai21  g063(.a(new_n145_), .b(x08), .c(new_n146_), .O(new_n147_));
  nand3  g064(.a(new_n147_), .b(new_n96_), .c(x07), .O(new_n148_));
  inv1   g065(.a(new_n148_), .O(z17));
  nand2  g066(.a(x29), .b(new_n97_), .O(new_n150_));
  nand2  g067(.a(x30), .b(x08), .O(new_n151_));
  nand3  g068(.a(new_n151_), .b(new_n150_), .c(x07), .O(new_n152_));
  and2   g069(.a(new_n152_), .b(new_n96_), .O(z18));
  inv1   g070(.a(x30), .O(new_n154_));
  nand2  g071(.a(x08), .b(x00), .O(new_n155_));
  oai21  g072(.a(new_n154_), .b(x08), .c(new_n155_), .O(new_n156_));
  nand3  g073(.a(new_n156_), .b(new_n96_), .c(x07), .O(new_n157_));
  inv1   g074(.a(new_n157_), .O(z19));
  inv1   g075(.a(x31), .O(new_n159_));
  nand2  g076(.a(x32), .b(x09), .O(new_n160_));
  oai21  g077(.a(new_n159_), .b(x09), .c(new_n160_), .O(new_n161_));
  nand3  g078(.a(new_n161_), .b(new_n96_), .c(x07), .O(new_n162_));
  inv1   g079(.a(new_n162_), .O(z20));
  inv1   g080(.a(x09), .O(new_n164_));
  nand2  g081(.a(x32), .b(new_n164_), .O(new_n165_));
  nand2  g082(.a(x33), .b(x09), .O(new_n166_));
  nand3  g083(.a(new_n166_), .b(new_n165_), .c(x07), .O(new_n167_));
  and2   g084(.a(new_n167_), .b(new_n96_), .O(z21));
  inv1   g085(.a(x34), .O(new_n169_));
  nand2  g086(.a(x33), .b(new_n164_), .O(new_n170_));
  oai21  g087(.a(new_n169_), .b(new_n164_), .c(new_n170_), .O(new_n171_));
  nand3  g088(.a(new_n171_), .b(new_n96_), .c(x07), .O(new_n172_));
  inv1   g089(.a(new_n172_), .O(z22));
  nand2  g090(.a(x35), .b(x09), .O(new_n174_));
  oai21  g091(.a(new_n169_), .b(x09), .c(new_n174_), .O(new_n175_));
  nand3  g092(.a(new_n175_), .b(new_n96_), .c(x07), .O(new_n176_));
  inv1   g093(.a(new_n176_), .O(z23));
  nand2  g094(.a(x35), .b(new_n164_), .O(new_n178_));
  nand2  g095(.a(x36), .b(x09), .O(new_n179_));
  nand3  g096(.a(new_n179_), .b(new_n178_), .c(x07), .O(new_n180_));
  and2   g097(.a(new_n180_), .b(new_n96_), .O(z24));
  nand2  g098(.a(x36), .b(new_n164_), .O(new_n182_));
  nand2  g099(.a(x37), .b(x09), .O(new_n183_));
  nand3  g100(.a(new_n183_), .b(new_n182_), .c(x07), .O(new_n184_));
  and2   g101(.a(new_n184_), .b(new_n96_), .O(z25));
  nand2  g102(.a(x37), .b(new_n164_), .O(new_n186_));
  nand2  g103(.a(x38), .b(x09), .O(new_n187_));
  nand3  g104(.a(new_n187_), .b(new_n186_), .c(x07), .O(new_n188_));
  and2   g105(.a(new_n188_), .b(new_n96_), .O(z26));
  inv1   g106(.a(x39), .O(new_n190_));
  nand2  g107(.a(x14), .b(x00), .O(new_n191_));
  oai21  g108(.a(new_n190_), .b(x14), .c(new_n191_), .O(new_n192_));
  nand2  g109(.a(new_n192_), .b(x09), .O(new_n193_));
  inv1   g110(.a(x07), .O(new_n194_));
  aoi21  g111(.a(x38), .b(new_n164_), .c(new_n194_), .O(new_n195_));
  aoi21  g112(.a(new_n195_), .b(new_n193_), .c(x10), .O(z27));
  inv1   g113(.a(x14), .O(new_n197_));
  nand2  g114(.a(new_n197_), .b(x09), .O(new_n198_));
  nand2  g115(.a(new_n198_), .b(x39), .O(new_n199_));
  inv1   g116(.a(x40), .O(new_n200_));
  nor2   g117(.a(new_n200_), .b(x14), .O(new_n201_));
  aoi21  g118(.a(new_n201_), .b(x09), .c(new_n194_), .O(new_n202_));
  aoi21  g119(.a(new_n202_), .b(new_n199_), .c(x10), .O(z28));
  oai21  g120(.a(x14), .b(new_n164_), .c(x40), .O(new_n204_));
  nand3  g121(.a(x41), .b(new_n197_), .c(x09), .O(new_n205_));
  nand2  g122(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  nand3  g123(.a(new_n206_), .b(new_n96_), .c(x07), .O(new_n207_));
  inv1   g124(.a(new_n207_), .O(z29));
  nand2  g125(.a(new_n198_), .b(x41), .O(new_n209_));
  inv1   g126(.a(x42), .O(new_n210_));
  nor2   g127(.a(new_n210_), .b(x14), .O(new_n211_));
  aoi21  g128(.a(new_n211_), .b(x09), .c(new_n194_), .O(new_n212_));
  aoi21  g129(.a(new_n212_), .b(new_n209_), .c(x10), .O(z30));
  nand2  g130(.a(new_n198_), .b(x42), .O(new_n214_));
  inv1   g131(.a(x43), .O(new_n215_));
  nor2   g132(.a(new_n215_), .b(x14), .O(new_n216_));
  aoi21  g133(.a(new_n216_), .b(x09), .c(new_n194_), .O(new_n217_));
  aoi21  g134(.a(new_n217_), .b(new_n214_), .c(x10), .O(z31));
  nand2  g135(.a(new_n198_), .b(x43), .O(new_n219_));
  inv1   g136(.a(x44), .O(new_n220_));
  nor2   g137(.a(new_n220_), .b(x14), .O(new_n221_));
  aoi21  g138(.a(new_n221_), .b(x09), .c(new_n194_), .O(new_n222_));
  aoi21  g139(.a(new_n222_), .b(new_n219_), .c(x10), .O(z32));
  oai21  g140(.a(x14), .b(new_n164_), .c(x44), .O(new_n224_));
  nand3  g141(.a(x45), .b(new_n197_), .c(x09), .O(new_n225_));
  nand2  g142(.a(new_n225_), .b(new_n224_), .O(new_n226_));
  nand3  g143(.a(new_n226_), .b(new_n96_), .c(x07), .O(new_n227_));
  inv1   g144(.a(new_n227_), .O(z33));
  oai21  g145(.a(x14), .b(new_n164_), .c(x45), .O(new_n229_));
  nand3  g146(.a(x46), .b(new_n197_), .c(x09), .O(new_n230_));
  nand2  g147(.a(new_n230_), .b(new_n229_), .O(new_n231_));
  nand3  g148(.a(new_n231_), .b(new_n96_), .c(x07), .O(new_n232_));
  inv1   g149(.a(new_n232_), .O(z34));
  oai21  g150(.a(x14), .b(new_n164_), .c(x46), .O(new_n234_));
  nand3  g151(.a(new_n197_), .b(x09), .c(x00), .O(new_n235_));
  nand2  g152(.a(new_n235_), .b(new_n234_), .O(new_n236_));
  nand3  g153(.a(new_n236_), .b(new_n96_), .c(x07), .O(new_n237_));
  inv1   g154(.a(new_n237_), .O(z35));
endmodule


