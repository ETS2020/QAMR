// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n178_, new_n179_, new_n180_, new_n181_;
  nand2  g000(.a(x23), .b(x21), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x21), .O(new_n58_));
  inv1   g012(.a(x23), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n58_), .O(new_n60_));
  oai22  g014(.a(new_n60_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n48_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n48_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand2  g024(.a(new_n52_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n48_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  aoi21  g027(.a(x27), .b(new_n73_), .c(new_n48_), .O(new_n74_));
  oai21  g028(.a(x27), .b(x25), .c(new_n74_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x26), .O(new_n78_));
  nand2  g032(.a(new_n52_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n48_), .O(z07));
  nand2  g034(.a(new_n47_), .b(new_n52_), .O(z08));
  nand2  g035(.a(x18), .b(x00), .O(new_n82_));
  inv1   g036(.a(x18), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(x08), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n82_), .c(x16), .O(new_n85_));
  nand3  g039(.a(x19), .b(x17), .c(x16), .O(new_n86_));
  inv1   g040(.a(new_n86_), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n85_), .c(new_n47_), .O(new_n88_));
  inv1   g042(.a(x17), .O(new_n89_));
  inv1   g043(.a(x20), .O(new_n90_));
  oai21  g044(.a(x23), .b(new_n90_), .c(new_n58_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x23), .O(new_n92_));
  nand4  g046(.a(new_n92_), .b(new_n51_), .c(new_n89_), .d(x16), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n88_), .O(z09));
  inv1   g048(.a(x16), .O(new_n95_));
  nand2  g049(.a(x18), .b(x01), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n55_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nor2   g052(.a(x19), .b(x17), .O(new_n99_));
  nor2   g053(.a(new_n99_), .b(new_n90_), .O(new_n100_));
  nor3   g054(.a(x20), .b(x19), .c(x17), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n100_), .c(x16), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(new_n98_), .c(new_n48_), .O(z10));
  inv1   g057(.a(x10), .O(new_n104_));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n104_), .c(new_n105_), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n47_), .c(new_n95_), .O(new_n107_));
  nor2   g061(.a(x20), .b(x19), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n89_), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n59_), .c(x21), .O(new_n110_));
  nor2   g064(.a(x21), .b(x20), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n99_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n107_), .O(z11));
  nand2  g069(.a(x18), .b(x03), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n62_), .c(new_n116_), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n47_), .c(new_n95_), .O(new_n118_));
  inv1   g072(.a(x22), .O(new_n119_));
  nor2   g073(.a(x22), .b(x20), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n99_), .O(new_n121_));
  oai21  g075(.a(new_n101_), .b(new_n119_), .c(new_n121_), .O(new_n122_));
  nand3  g076(.a(new_n59_), .b(x22), .c(x21), .O(new_n123_));
  inv1   g077(.a(new_n123_), .O(new_n124_));
  aoi21  g078(.a(new_n122_), .b(new_n58_), .c(new_n124_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n95_), .c(new_n118_), .O(z12));
  nand2  g080(.a(x18), .b(x04), .O(new_n127_));
  oai21  g081(.a(x18), .b(new_n65_), .c(new_n127_), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n47_), .c(new_n95_), .O(new_n129_));
  nand2  g083(.a(x23), .b(x20), .O(new_n130_));
  nand3  g084(.a(new_n59_), .b(new_n119_), .c(new_n90_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n51_), .c(new_n89_), .O(new_n133_));
  nand3  g087(.a(new_n119_), .b(new_n51_), .c(new_n89_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(x23), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n58_), .c(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n129_), .O(z13));
  nand2  g092(.a(x18), .b(x05), .O(new_n139_));
  oai21  g093(.a(x18), .b(new_n68_), .c(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n95_), .O(new_n141_));
  oai21  g095(.a(new_n70_), .b(new_n95_), .c(new_n59_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x21), .O(new_n143_));
  nor2   g097(.a(x23), .b(x22), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n99_), .c(new_n90_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x24), .O(new_n146_));
  nor2   g100(.a(x22), .b(x21), .O(new_n147_));
  nor2   g101(.a(x24), .b(x23), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n147_), .c(new_n101_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n146_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n143_), .c(new_n141_), .O(z14));
  nand2  g106(.a(x18), .b(x06), .O(new_n153_));
  oai21  g107(.a(x18), .b(new_n73_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n95_), .O(new_n155_));
  inv1   g109(.a(x25), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n95_), .c(new_n59_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x21), .O(new_n158_));
  nor3   g112(.a(x24), .b(x23), .c(x22), .O(new_n159_));
  aoi21  g113(.a(new_n159_), .b(new_n101_), .c(new_n156_), .O(new_n160_));
  nor2   g114(.a(x25), .b(x24), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n144_), .O(new_n162_));
  nor2   g116(.a(new_n162_), .b(new_n112_), .O(new_n163_));
  oai21  g117(.a(new_n163_), .b(new_n160_), .c(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n158_), .c(new_n155_), .O(z15));
  nand2  g119(.a(x18), .b(x07), .O(new_n166_));
  oai21  g120(.a(x18), .b(new_n76_), .c(new_n166_), .O(new_n167_));
  nand3  g121(.a(new_n167_), .b(new_n47_), .c(new_n95_), .O(new_n168_));
  nand2  g122(.a(x26), .b(x23), .O(new_n169_));
  nor2   g123(.a(x26), .b(x25), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n148_), .c(new_n120_), .d(new_n99_), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n169_), .c(x21), .O(new_n172_));
  nand4  g126(.a(new_n161_), .b(new_n147_), .c(new_n108_), .d(new_n89_), .O(new_n173_));
  nand3  g127(.a(new_n173_), .b(x26), .c(new_n59_), .O(new_n174_));
  inv1   g128(.a(new_n174_), .O(new_n175_));
  oai21  g129(.a(new_n175_), .b(new_n172_), .c(x16), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(new_n168_), .O(z16));
  nand3  g131(.a(new_n47_), .b(x27), .c(x17), .O(new_n178_));
  nor2   g132(.a(new_n51_), .b(x17), .O(new_n179_));
  nor3   g133(.a(x26), .b(x25), .c(x24), .O(new_n180_));
  nand4  g134(.a(new_n180_), .b(new_n179_), .c(new_n144_), .d(new_n111_), .O(new_n181_));
  aoi21  g135(.a(new_n181_), .b(new_n178_), .c(new_n95_), .O(z17));
endmodule


