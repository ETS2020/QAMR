// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_,
    new_n172_;
  nor2   g000(.a(x18), .b(x09), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x18), .O(new_n54_));
  inv1   g008(.a(x09), .O(new_n55_));
  nor2   g009(.a(x27), .b(x20), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  nand2  g011(.a(new_n56_), .b(x09), .O(new_n58_));
  oai21  g012(.a(new_n57_), .b(new_n54_), .c(new_n58_), .O(z01));
  inv1   g013(.a(x10), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x21), .O(new_n62_));
  nand2  g016(.a(new_n51_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n47_), .O(z02));
  inv1   g018(.a(x11), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x22), .O(new_n67_));
  nand2  g021(.a(new_n51_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(z03));
  inv1   g023(.a(x12), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n47_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x23), .c(new_n71_), .O(z04));
  inv1   g026(.a(x13), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x24), .O(new_n75_));
  nand2  g029(.a(new_n51_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(z05));
  inv1   g031(.a(x14), .O(new_n78_));
  aoi21  g032(.a(x27), .b(new_n78_), .c(new_n47_), .O(new_n79_));
  oai21  g033(.a(x27), .b(x25), .c(new_n79_), .O(z06));
  inv1   g034(.a(x15), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  inv1   g036(.a(x26), .O(new_n83_));
  nand2  g037(.a(new_n51_), .b(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n82_), .c(new_n47_), .O(z07));
  inv1   g039(.a(new_n47_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n51_), .O(z08));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  nand2  g043(.a(new_n54_), .b(x09), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n48_), .c(new_n89_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nand2  g046(.a(x19), .b(x17), .O(new_n93_));
  nor2   g047(.a(x19), .b(x17), .O(new_n94_));
  inv1   g048(.a(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n86_), .c(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n92_), .O(z09));
  nand2  g052(.a(x18), .b(x01), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n90_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n88_), .O(new_n101_));
  oai21  g055(.a(new_n54_), .b(new_n88_), .c(new_n90_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n95_), .c(x20), .O(new_n103_));
  inv1   g057(.a(x17), .O(new_n104_));
  nor2   g058(.a(new_n47_), .b(x20), .O(new_n105_));
  nand4  g059(.a(new_n105_), .b(new_n50_), .c(new_n104_), .d(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n103_), .c(new_n101_), .O(z10));
  nand2  g061(.a(x18), .b(x02), .O(new_n108_));
  nand3  g062(.a(new_n54_), .b(x10), .c(x09), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n88_), .O(new_n111_));
  nor3   g065(.a(x20), .b(x19), .c(x17), .O(new_n112_));
  nor2   g066(.a(x21), .b(x20), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n94_), .O(new_n114_));
  oai21  g068(.a(new_n112_), .b(new_n62_), .c(new_n114_), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n86_), .c(x16), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n111_), .O(z11));
  nand2  g071(.a(x18), .b(x03), .O(new_n118_));
  nand3  g072(.a(new_n54_), .b(x11), .c(x09), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n88_), .O(new_n121_));
  nand2  g075(.a(new_n114_), .b(x22), .O(new_n122_));
  inv1   g076(.a(x20), .O(new_n123_));
  nor2   g077(.a(x22), .b(x21), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n94_), .c(new_n123_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n122_), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n86_), .c(x16), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n121_), .O(z12));
  nand2  g082(.a(x18), .b(x04), .O(new_n129_));
  oai21  g083(.a(x18), .b(new_n70_), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n88_), .O(new_n131_));
  nand2  g085(.a(new_n125_), .b(x23), .O(new_n132_));
  nor3   g086(.a(x23), .b(x22), .c(x21), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n112_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n131_), .c(new_n86_), .O(z13));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n73_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n88_), .O(new_n140_));
  aoi21  g094(.a(new_n133_), .b(new_n112_), .c(new_n75_), .O(new_n141_));
  nor2   g095(.a(x20), .b(x19), .O(new_n142_));
  nor2   g096(.a(x24), .b(x23), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n124_), .c(new_n142_), .d(new_n104_), .O(new_n144_));
  inv1   g098(.a(new_n144_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n141_), .c(x16), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n140_), .c(new_n86_), .O(z14));
  nand2  g101(.a(x18), .b(x06), .O(new_n148_));
  nand3  g102(.a(new_n54_), .b(x14), .c(x09), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n148_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n88_), .O(new_n151_));
  nand2  g105(.a(new_n144_), .b(x25), .O(new_n152_));
  nor2   g106(.a(x23), .b(x22), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n153_), .c(new_n113_), .d(new_n94_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n152_), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n86_), .c(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n151_), .O(z15));
  nand2  g112(.a(x18), .b(x07), .O(new_n159_));
  oai21  g113(.a(x18), .b(new_n81_), .c(new_n159_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n88_), .O(new_n161_));
  nand2  g115(.a(new_n155_), .b(x26), .O(new_n162_));
  nor3   g116(.a(x26), .b(x25), .c(x24), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n153_), .c(new_n113_), .d(new_n94_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x16), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(new_n161_), .c(new_n86_), .O(z16));
  nor2   g121(.a(x26), .b(x25), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n153_), .c(new_n75_), .O(new_n169_));
  nand3  g123(.a(new_n113_), .b(x19), .c(new_n104_), .O(new_n170_));
  oai22  g124(.a(new_n170_), .b(new_n169_), .c(new_n51_), .d(new_n104_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n86_), .O(z17));
endmodule


