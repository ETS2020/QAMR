// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n49_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x23), .c(new_n62_), .O(z04));
  inv1   g017(.a(x13), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x24), .c(new_n65_), .O(z05));
  inv1   g020(.a(x14), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x25), .c(new_n68_), .O(z06));
  inv1   g023(.a(x15), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x26), .O(new_n72_));
  inv1   g026(.a(x27), .O(new_n73_));
  nand2  g027(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n71_), .c(new_n49_), .O(z07));
  inv1   g029(.a(new_n49_), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(new_n73_), .O(z08));
  nand3  g031(.a(x19), .b(x18), .c(x17), .O(new_n78_));
  inv1   g032(.a(new_n78_), .O(new_n79_));
  nor2   g033(.a(x19), .b(x17), .O(new_n80_));
  oai21  g034(.a(new_n80_), .b(new_n79_), .c(x16), .O(new_n81_));
  nor2   g035(.a(x19), .b(x18), .O(new_n82_));
  aoi22  g036(.a(new_n82_), .b(x08), .c(x18), .d(x00), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(x16), .c(new_n81_), .O(z09));
  aoi22  g038(.a(new_n82_), .b(x09), .c(x18), .d(x01), .O(new_n85_));
  inv1   g039(.a(x17), .O(new_n86_));
  inv1   g040(.a(x20), .O(new_n87_));
  nor2   g041(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g042(.a(x20), .b(x17), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n88_), .c(new_n48_), .O(new_n90_));
  nand3  g044(.a(x20), .b(x19), .c(x18), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(x16), .O(new_n93_));
  oai21  g047(.a(new_n85_), .b(x16), .c(new_n93_), .O(z10));
  aoi22  g048(.a(new_n82_), .b(x10), .c(x18), .d(x02), .O(new_n95_));
  oai21  g049(.a(x20), .b(x17), .c(x21), .O(new_n96_));
  inv1   g050(.a(x21), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n87_), .c(new_n86_), .O(new_n98_));
  aoi21  g052(.a(new_n98_), .b(new_n96_), .c(x19), .O(new_n99_));
  nand3  g053(.a(x21), .b(x19), .c(x18), .O(new_n100_));
  inv1   g054(.a(new_n100_), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n99_), .c(x16), .O(new_n102_));
  oai21  g056(.a(new_n95_), .b(x16), .c(new_n102_), .O(z11));
  inv1   g057(.a(x16), .O(new_n104_));
  inv1   g058(.a(new_n82_), .O(new_n105_));
  nand2  g059(.a(x18), .b(x03), .O(new_n106_));
  oai21  g060(.a(new_n105_), .b(new_n58_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  inv1   g062(.a(x22), .O(new_n109_));
  nor3   g063(.a(x21), .b(x20), .c(x17), .O(new_n110_));
  nand3  g064(.a(new_n89_), .b(new_n109_), .c(new_n97_), .O(new_n111_));
  oai21  g065(.a(new_n110_), .b(new_n109_), .c(new_n111_), .O(new_n112_));
  nor2   g066(.a(new_n109_), .b(new_n48_), .O(new_n113_));
  aoi22  g067(.a(new_n113_), .b(x18), .c(new_n112_), .d(new_n48_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n104_), .c(new_n108_), .O(z12));
  nand2  g069(.a(x18), .b(x04), .O(new_n116_));
  oai21  g070(.a(new_n105_), .b(new_n61_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n104_), .O(new_n118_));
  nand2  g072(.a(new_n111_), .b(x23), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n89_), .c(new_n97_), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n119_), .c(x19), .O(new_n122_));
  nand3  g076(.a(x23), .b(x19), .c(x18), .O(new_n123_));
  inv1   g077(.a(new_n123_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n118_), .O(z13));
  nand2  g080(.a(x18), .b(x05), .O(new_n127_));
  oai21  g081(.a(new_n105_), .b(new_n64_), .c(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n104_), .O(new_n129_));
  nand2  g083(.a(new_n121_), .b(x24), .O(new_n130_));
  nor3   g084(.a(x24), .b(x23), .c(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n110_), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n130_), .c(x19), .O(new_n133_));
  nand3  g087(.a(x24), .b(x19), .c(x18), .O(new_n134_));
  inv1   g088(.a(new_n134_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n129_), .O(z14));
  nand2  g091(.a(x18), .b(x06), .O(new_n138_));
  oai21  g092(.a(new_n105_), .b(new_n67_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n104_), .O(new_n140_));
  inv1   g094(.a(x23), .O(new_n141_));
  inv1   g095(.a(x24), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n141_), .c(new_n109_), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n98_), .c(x25), .O(new_n144_));
  nor2   g098(.a(x25), .b(x24), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n120_), .c(new_n110_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n144_), .c(x19), .O(new_n147_));
  nand3  g101(.a(x25), .b(x19), .c(x18), .O(new_n148_));
  inv1   g102(.a(new_n148_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n147_), .c(x16), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n140_), .O(z15));
  nand2  g105(.a(x18), .b(x07), .O(new_n152_));
  oai21  g106(.a(x18), .b(new_n70_), .c(new_n152_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n104_), .O(new_n154_));
  oai21  g108(.a(new_n72_), .b(new_n104_), .c(x18), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x19), .O(new_n156_));
  nand2  g110(.a(new_n146_), .b(x26), .O(new_n157_));
  nor2   g111(.a(x21), .b(x20), .O(new_n158_));
  nor3   g112(.a(x26), .b(x25), .c(x24), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n120_), .c(new_n158_), .d(new_n80_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n157_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand3  g116(.a(new_n162_), .b(new_n156_), .c(new_n154_), .O(z16));
  nand2  g117(.a(new_n159_), .b(new_n120_), .O(new_n164_));
  nand3  g118(.a(new_n158_), .b(x19), .c(new_n86_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n164_), .c(new_n73_), .d(new_n86_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n76_), .O(z17));
endmodule


