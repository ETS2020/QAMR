// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n151_, new_n152_, new_n153_,
    new_n154_, new_n155_, new_n156_;
  inv1   g000(.a(x18), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x08), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(x27), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  nand2  g004(.a(x27), .b(new_n47_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g006(.a(new_n51_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g007(.a(new_n51_), .b(x11), .c(x27), .d(x22), .O(z03));
  nand2  g008(.a(new_n47_), .b(x12), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(x27), .O(new_n56_));
  oai21  g010(.a(x27), .b(x23), .c(new_n56_), .O(z04));
  inv1   g011(.a(x13), .O(new_n58_));
  oai21  g012(.a(x18), .b(new_n58_), .c(x27), .O(new_n59_));
  oai21  g013(.a(x27), .b(x24), .c(new_n59_), .O(z05));
  inv1   g014(.a(x14), .O(new_n61_));
  oai21  g015(.a(x18), .b(new_n61_), .c(x27), .O(new_n62_));
  oai21  g016(.a(x27), .b(x25), .c(new_n62_), .O(z06));
  inv1   g017(.a(x15), .O(new_n64_));
  oai21  g018(.a(x18), .b(new_n64_), .c(x27), .O(new_n65_));
  oai21  g019(.a(x27), .b(x26), .c(new_n65_), .O(z07));
  inv1   g020(.a(new_n51_), .O(z08));
  inv1   g021(.a(x17), .O(new_n68_));
  inv1   g022(.a(x19), .O(new_n69_));
  nor2   g023(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x16), .O(new_n73_));
  nand2  g027(.a(x18), .b(x00), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(new_n48_), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g030(.a(x27), .b(x18), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z09));
  inv1   g032(.a(x01), .O(new_n79_));
  nand2  g033(.a(new_n47_), .b(x09), .O(new_n80_));
  inv1   g034(.a(x27), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(x18), .O(new_n82_));
  oai21  g036(.a(new_n82_), .b(new_n79_), .c(new_n80_), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n73_), .O(new_n84_));
  inv1   g038(.a(x20), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n69_), .c(new_n68_), .O(new_n86_));
  oai21  g040(.a(new_n71_), .b(new_n85_), .c(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n77_), .c(x16), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n84_), .O(z10));
  inv1   g043(.a(x10), .O(new_n90_));
  nand2  g044(.a(x18), .b(x02), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n90_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n73_), .O(new_n93_));
  inv1   g047(.a(x21), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  nor2   g049(.a(x21), .b(x20), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n71_), .O(new_n97_));
  oai21  g051(.a(new_n95_), .b(new_n94_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x16), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n93_), .c(new_n77_), .O(z11));
  inv1   g054(.a(x11), .O(new_n101_));
  nand2  g055(.a(x18), .b(x03), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n101_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n73_), .O(new_n104_));
  inv1   g058(.a(x22), .O(new_n105_));
  aoi21  g059(.a(new_n96_), .b(new_n71_), .c(new_n105_), .O(new_n106_));
  nor2   g060(.a(x22), .b(x21), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n71_), .c(new_n85_), .O(new_n108_));
  inv1   g062(.a(new_n108_), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n104_), .c(new_n77_), .O(z12));
  nand2  g065(.a(x18), .b(x04), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n55_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n73_), .O(new_n114_));
  nand2  g068(.a(new_n108_), .b(x23), .O(new_n115_));
  nor2   g069(.a(x23), .b(x22), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n95_), .c(new_n94_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x16), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n114_), .c(new_n77_), .O(z13));
  inv1   g074(.a(x05), .O(new_n121_));
  oai22  g075(.a(new_n82_), .b(new_n121_), .c(x18), .d(new_n58_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n73_), .O(new_n123_));
  inv1   g077(.a(x23), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n105_), .c(new_n94_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n86_), .c(x24), .O(new_n126_));
  nor2   g080(.a(x20), .b(x19), .O(new_n127_));
  nor2   g081(.a(x24), .b(x23), .O(new_n128_));
  nand4  g082(.a(new_n128_), .b(new_n107_), .c(new_n127_), .d(new_n68_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n126_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n77_), .c(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n123_), .O(z14));
  inv1   g086(.a(x06), .O(new_n133_));
  oai22  g087(.a(new_n82_), .b(new_n133_), .c(x18), .d(new_n61_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n73_), .O(new_n135_));
  nand2  g089(.a(new_n129_), .b(x25), .O(new_n136_));
  nor2   g090(.a(x25), .b(x24), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n116_), .c(new_n96_), .d(new_n71_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n77_), .c(x16), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n135_), .O(z15));
  inv1   g095(.a(x07), .O(new_n142_));
  oai22  g096(.a(new_n82_), .b(new_n142_), .c(x18), .d(new_n64_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n73_), .O(new_n144_));
  nand2  g098(.a(new_n138_), .b(x26), .O(new_n145_));
  nor3   g099(.a(x26), .b(x25), .c(x24), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n116_), .c(new_n96_), .d(new_n71_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n145_), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n77_), .c(x16), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n144_), .O(z16));
  inv1   g104(.a(x24), .O(new_n151_));
  nor2   g105(.a(x26), .b(x25), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n116_), .c(new_n151_), .O(new_n153_));
  nand3  g107(.a(new_n96_), .b(x19), .c(new_n68_), .O(new_n154_));
  oai22  g108(.a(new_n154_), .b(new_n153_), .c(new_n81_), .d(new_n68_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n77_), .O(z17));
endmodule


