// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x25), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x23), .O(new_n63_));
  inv1   g017(.a(x27), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n62_), .c(new_n49_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x24), .c(new_n68_), .O(z05));
  inv1   g023(.a(x18), .O(new_n70_));
  inv1   g024(.a(x25), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n70_), .c(new_n71_), .O(new_n72_));
  oai22  g026(.a(new_n72_), .b(x14), .c(x27), .d(x25), .O(z06));
  inv1   g027(.a(x15), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n49_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x26), .c(new_n75_), .O(z07));
  nor2   g030(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g031(.a(x16), .O(new_n78_));
  inv1   g032(.a(x00), .O(new_n79_));
  nand2  g033(.a(new_n71_), .b(x18), .O(new_n80_));
  oai22  g034(.a(new_n80_), .b(new_n79_), .c(x18), .d(new_n47_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n78_), .O(new_n82_));
  xnor2a g036(.a(x19), .b(x17), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n48_), .c(x16), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n82_), .O(z09));
  nand2  g039(.a(x18), .b(x01), .O(new_n86_));
  oai21  g040(.a(x18), .b(new_n52_), .c(new_n86_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n78_), .O(new_n88_));
  inv1   g042(.a(x20), .O(new_n89_));
  nor2   g043(.a(x19), .b(x17), .O(new_n90_));
  nor2   g044(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor3   g045(.a(x20), .b(x19), .c(x17), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n91_), .c(x16), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n88_), .c(new_n48_), .O(z10));
  inv1   g048(.a(x02), .O(new_n95_));
  oai22  g049(.a(new_n80_), .b(new_n95_), .c(x18), .d(new_n55_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n78_), .O(new_n97_));
  inv1   g051(.a(x21), .O(new_n98_));
  nor2   g052(.a(x21), .b(x20), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n90_), .O(new_n100_));
  oai21  g054(.a(new_n92_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n48_), .c(x16), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n97_), .O(z11));
  nand2  g057(.a(x18), .b(x03), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n58_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n78_), .O(new_n106_));
  inv1   g060(.a(x22), .O(new_n107_));
  inv1   g061(.a(new_n100_), .O(new_n108_));
  nor2   g062(.a(x22), .b(x21), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n90_), .c(new_n89_), .O(new_n110_));
  oai21  g064(.a(new_n108_), .b(new_n107_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x16), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n106_), .c(new_n48_), .O(z12));
  inv1   g067(.a(x04), .O(new_n114_));
  oai22  g068(.a(new_n80_), .b(new_n114_), .c(x18), .d(new_n61_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n78_), .O(new_n116_));
  inv1   g070(.a(x17), .O(new_n117_));
  nand3  g071(.a(new_n110_), .b(new_n48_), .c(x23), .O(new_n118_));
  inv1   g072(.a(new_n118_), .O(new_n119_));
  inv1   g073(.a(x24), .O(new_n120_));
  oai21  g074(.a(x25), .b(new_n120_), .c(new_n70_), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(x25), .c(x23), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n107_), .c(new_n98_), .d(new_n89_), .O(new_n123_));
  nor2   g077(.a(new_n123_), .b(x19), .O(new_n124_));
  aoi21  g078(.a(new_n124_), .b(new_n117_), .c(new_n119_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n78_), .c(new_n116_), .O(z13));
  inv1   g080(.a(x05), .O(new_n127_));
  oai22  g081(.a(new_n80_), .b(new_n127_), .c(x18), .d(new_n67_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n78_), .O(new_n129_));
  nor2   g083(.a(x23), .b(x22), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n92_), .c(new_n98_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x24), .O(new_n132_));
  nand4  g086(.a(new_n109_), .b(new_n92_), .c(new_n120_), .d(new_n63_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n48_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n129_), .O(z14));
  inv1   g090(.a(x14), .O(new_n137_));
  nand2  g091(.a(x18), .b(x06), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n137_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n78_), .O(new_n140_));
  inv1   g094(.a(x19), .O(new_n141_));
  nand3  g095(.a(x25), .b(x24), .c(new_n70_), .O(new_n142_));
  nand2  g096(.a(new_n71_), .b(new_n120_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n63_), .c(new_n107_), .d(new_n98_), .O(new_n145_));
  inv1   g099(.a(new_n145_), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n89_), .c(new_n141_), .d(new_n117_), .O(new_n147_));
  aoi21  g101(.a(x23), .b(new_n70_), .c(x22), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n92_), .c(new_n98_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(x25), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n140_), .c(new_n48_), .O(z15));
  inv1   g107(.a(x07), .O(new_n154_));
  oai22  g108(.a(new_n80_), .b(new_n154_), .c(x18), .d(new_n74_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n78_), .O(new_n156_));
  nand2  g110(.a(new_n133_), .b(x26), .O(new_n157_));
  inv1   g111(.a(x26), .O(new_n158_));
  nand4  g112(.a(new_n130_), .b(new_n108_), .c(new_n158_), .d(new_n120_), .O(new_n159_));
  aoi21  g113(.a(new_n159_), .b(new_n157_), .c(x25), .O(new_n160_));
  nor3   g114(.a(new_n158_), .b(new_n71_), .c(x18), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n160_), .c(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n156_), .O(z16));
  nand2  g117(.a(z08), .b(x17), .O(new_n164_));
  nor2   g118(.a(new_n141_), .b(x17), .O(new_n165_));
  nor3   g119(.a(x26), .b(x25), .c(x24), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n165_), .c(new_n130_), .d(new_n99_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n164_), .c(new_n78_), .O(z17));
endmodule


