// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n164_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n177_,
    new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x17), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x09), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x20), .O(new_n58_));
  nand2  g012(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(z01));
  inv1   g014(.a(x10), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x21), .O(new_n63_));
  nand2  g017(.a(new_n53_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z02));
  inv1   g019(.a(x11), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x22), .c(new_n67_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n49_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x23), .c(new_n70_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n53_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  nand2  g034(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x26), .O(new_n82_));
  nand2  g036(.a(new_n53_), .b(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n81_), .c(new_n49_), .O(z07));
  inv1   g038(.a(new_n49_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n53_), .O(z08));
  nor2   g040(.a(new_n52_), .b(new_n47_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x16), .O(new_n90_));
  nand2  g044(.a(x18), .b(x00), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n50_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand3  g047(.a(new_n93_), .b(new_n89_), .c(new_n85_), .O(z09));
  inv1   g048(.a(x01), .O(new_n95_));
  nand2  g049(.a(x18), .b(new_n47_), .O(new_n96_));
  oai22  g050(.a(new_n96_), .b(new_n95_), .c(x18), .d(new_n56_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n90_), .O(new_n98_));
  nor2   g052(.a(new_n58_), .b(new_n52_), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n99_), .c(new_n47_), .O(new_n101_));
  nand3  g055(.a(x20), .b(new_n48_), .c(x17), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n98_), .O(z10));
  nand2  g059(.a(x18), .b(x02), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n61_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n90_), .O(new_n108_));
  oai21  g062(.a(new_n63_), .b(new_n90_), .c(new_n48_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x17), .O(new_n110_));
  nor2   g064(.a(x21), .b(x20), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n88_), .O(new_n112_));
  oai21  g066(.a(new_n100_), .b(new_n63_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n110_), .c(new_n108_), .O(z11));
  nand2  g069(.a(x18), .b(x03), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n66_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n90_), .O(new_n118_));
  inv1   g072(.a(x22), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n90_), .c(new_n48_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x17), .O(new_n121_));
  nand3  g075(.a(new_n63_), .b(new_n58_), .c(new_n52_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x22), .O(new_n123_));
  nor2   g077(.a(x22), .b(x21), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n88_), .c(new_n58_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n123_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x16), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n121_), .c(new_n118_), .O(z12));
  inv1   g082(.a(x04), .O(new_n129_));
  oai22  g083(.a(new_n96_), .b(new_n129_), .c(x18), .d(new_n69_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n90_), .O(new_n131_));
  nand2  g085(.a(new_n124_), .b(new_n100_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x23), .O(new_n133_));
  nor2   g087(.a(x23), .b(x22), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n100_), .c(new_n63_), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n133_), .c(x17), .O(new_n136_));
  nand3  g090(.a(x23), .b(new_n48_), .c(x17), .O(new_n137_));
  inv1   g091(.a(new_n137_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n131_), .O(z13));
  nand2  g094(.a(x18), .b(x05), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n72_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n90_), .O(new_n143_));
  inv1   g097(.a(x24), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n90_), .c(new_n48_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x17), .O(new_n146_));
  nand2  g100(.a(new_n135_), .b(x24), .O(new_n147_));
  nor2   g101(.a(x24), .b(x23), .O(new_n148_));
  nand4  g102(.a(new_n148_), .b(new_n124_), .c(new_n100_), .d(new_n47_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n146_), .c(new_n143_), .O(z14));
  inv1   g106(.a(x06), .O(new_n153_));
  oai22  g107(.a(new_n96_), .b(new_n153_), .c(x18), .d(new_n75_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n90_), .O(new_n155_));
  inv1   g109(.a(x23), .O(new_n156_));
  nand3  g110(.a(new_n144_), .b(new_n156_), .c(new_n119_), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n122_), .c(x25), .O(new_n158_));
  nor2   g112(.a(x25), .b(x24), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n134_), .c(new_n111_), .d(new_n52_), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(new_n158_), .c(x17), .O(new_n161_));
  nand3  g115(.a(x25), .b(new_n48_), .c(x17), .O(new_n162_));
  inv1   g116(.a(new_n162_), .O(new_n163_));
  oai21  g117(.a(new_n163_), .b(new_n161_), .c(x16), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n155_), .O(z15));
  inv1   g119(.a(x07), .O(new_n166_));
  oai22  g120(.a(new_n96_), .b(new_n166_), .c(x18), .d(new_n80_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n90_), .O(new_n168_));
  nand2  g122(.a(new_n160_), .b(x26), .O(new_n169_));
  nor2   g123(.a(x26), .b(x25), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n148_), .c(new_n124_), .d(new_n100_), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n169_), .c(x17), .O(new_n172_));
  nand3  g126(.a(x26), .b(new_n48_), .c(x17), .O(new_n173_));
  inv1   g127(.a(new_n173_), .O(new_n174_));
  oai21  g128(.a(new_n174_), .b(new_n172_), .c(x16), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(new_n168_), .O(z16));
  nor2   g130(.a(new_n52_), .b(x17), .O(new_n177_));
  nor3   g131(.a(x26), .b(x25), .c(x24), .O(new_n178_));
  nand4  g132(.a(new_n178_), .b(new_n177_), .c(new_n134_), .d(new_n111_), .O(new_n179_));
  nand3  g133(.a(x27), .b(new_n48_), .c(x17), .O(new_n180_));
  aoi21  g134(.a(new_n180_), .b(new_n179_), .c(new_n90_), .O(z17));
endmodule


