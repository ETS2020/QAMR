// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_, new_n180_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x23), .O(new_n63_));
  inv1   g017(.a(x27), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n62_), .c(new_n49_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x26), .O(new_n77_));
  nand2  g031(.a(new_n64_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g034(.a(x17), .O(new_n81_));
  nor2   g035(.a(new_n48_), .b(new_n81_), .O(new_n82_));
  nor2   g036(.a(x19), .b(x17), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n82_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x16), .O(new_n85_));
  nand2  g039(.a(x18), .b(x00), .O(new_n86_));
  oai21  g040(.a(x18), .b(new_n47_), .c(new_n86_), .O(new_n87_));
  aoi21  g041(.a(new_n87_), .b(new_n85_), .c(new_n49_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n84_), .O(z09));
  nand2  g043(.a(x18), .b(x01), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n52_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n85_), .c(x18), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(x19), .O(new_n95_));
  nor2   g049(.a(new_n93_), .b(new_n81_), .O(new_n96_));
  nor3   g050(.a(x20), .b(x19), .c(x17), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n96_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n95_), .c(new_n92_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  inv1   g054(.a(x18), .O(new_n101_));
  nand3  g055(.a(new_n48_), .b(new_n101_), .c(x10), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n100_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n85_), .O(new_n104_));
  oai21  g058(.a(x20), .b(x17), .c(x21), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(x19), .O(new_n108_));
  nand3  g062(.a(x21), .b(x19), .c(x18), .O(new_n109_));
  inv1   g063(.a(new_n109_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n104_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n58_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n85_), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n85_), .c(x18), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x19), .O(new_n118_));
  nor3   g072(.a(x21), .b(x20), .c(x17), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n83_), .c(new_n93_), .O(new_n121_));
  oai21  g075(.a(new_n119_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n118_), .c(new_n115_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n61_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n85_), .O(new_n127_));
  oai21  g081(.a(new_n63_), .b(new_n85_), .c(x18), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x19), .O(new_n129_));
  nor2   g083(.a(x20), .b(x17), .O(new_n130_));
  aoi21  g084(.a(new_n120_), .b(new_n130_), .c(new_n63_), .O(new_n131_));
  nand3  g085(.a(new_n93_), .b(new_n48_), .c(new_n81_), .O(new_n132_));
  inv1   g086(.a(x21), .O(new_n133_));
  nor2   g087(.a(x23), .b(x22), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nor2   g089(.a(new_n135_), .b(new_n132_), .O(new_n136_));
  oai21  g090(.a(new_n136_), .b(new_n131_), .c(x16), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n129_), .c(new_n127_), .O(z13));
  nand2  g092(.a(x18), .b(x05), .O(new_n139_));
  oai21  g093(.a(x18), .b(new_n67_), .c(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n85_), .O(new_n141_));
  oai21  g095(.a(new_n69_), .b(new_n85_), .c(x18), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x19), .O(new_n143_));
  nand3  g097(.a(new_n134_), .b(new_n130_), .c(new_n133_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x24), .O(new_n145_));
  nand4  g099(.a(new_n120_), .b(new_n97_), .c(new_n69_), .d(new_n63_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n145_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x16), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n143_), .c(new_n141_), .O(z14));
  nand2  g103(.a(x18), .b(x06), .O(new_n150_));
  oai21  g104(.a(x18), .b(new_n72_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n85_), .O(new_n152_));
  inv1   g106(.a(x25), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n85_), .c(x18), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x19), .O(new_n155_));
  nor3   g109(.a(x24), .b(x23), .c(x22), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n119_), .c(new_n153_), .O(new_n157_));
  nand2  g111(.a(new_n106_), .b(new_n83_), .O(new_n158_));
  nor2   g112(.a(x25), .b(x24), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n134_), .O(new_n160_));
  nor2   g114(.a(new_n160_), .b(new_n158_), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n157_), .c(x16), .O(new_n162_));
  nand3  g116(.a(new_n162_), .b(new_n155_), .c(new_n152_), .O(z15));
  nand2  g117(.a(x18), .b(x07), .O(new_n164_));
  oai21  g118(.a(x18), .b(new_n75_), .c(new_n164_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(new_n85_), .O(new_n166_));
  oai21  g120(.a(new_n77_), .b(new_n85_), .c(x18), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x19), .O(new_n168_));
  nand4  g122(.a(new_n159_), .b(new_n134_), .c(new_n106_), .d(new_n81_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x26), .O(new_n170_));
  nor3   g124(.a(x26), .b(x25), .c(x24), .O(new_n171_));
  nand4  g125(.a(new_n171_), .b(new_n134_), .c(new_n106_), .d(new_n83_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(x16), .O(new_n174_));
  nand3  g128(.a(new_n174_), .b(new_n168_), .c(new_n166_), .O(z16));
  nand2  g129(.a(z08), .b(x17), .O(new_n176_));
  nor2   g130(.a(new_n101_), .b(x17), .O(new_n177_));
  nand2  g131(.a(new_n106_), .b(x19), .O(new_n178_));
  inv1   g132(.a(new_n178_), .O(new_n179_));
  nand4  g133(.a(new_n179_), .b(new_n177_), .c(new_n171_), .d(new_n134_), .O(new_n180_));
  aoi21  g134(.a(new_n180_), .b(new_n176_), .c(new_n85_), .O(z17));
endmodule


