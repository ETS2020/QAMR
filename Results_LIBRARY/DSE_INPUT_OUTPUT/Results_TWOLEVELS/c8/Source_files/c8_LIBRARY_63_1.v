// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n130_, new_n131_, new_n132_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n148_, new_n149_, new_n150_, new_n151_, new_n152_, new_n153_,
    new_n154_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  oai21  g002(.a(new_n48_), .b(new_n47_), .c(x27), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  nand2  g004(.a(x27), .b(x18), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g006(.a(new_n51_), .b(x10), .c(x27), .d(x21), .O(z02));
  oai22  g007(.a(new_n51_), .b(x11), .c(x27), .d(x22), .O(z03));
  inv1   g008(.a(x12), .O(new_n55_));
  oai21  g009(.a(new_n48_), .b(new_n55_), .c(x27), .O(new_n56_));
  oai21  g010(.a(x27), .b(x23), .c(new_n56_), .O(z04));
  inv1   g011(.a(x13), .O(new_n58_));
  oai21  g012(.a(new_n48_), .b(new_n58_), .c(x27), .O(new_n59_));
  oai21  g013(.a(x27), .b(x24), .c(new_n59_), .O(z05));
  inv1   g014(.a(x14), .O(new_n61_));
  oai21  g015(.a(new_n48_), .b(new_n61_), .c(x27), .O(new_n62_));
  oai21  g016(.a(x27), .b(x25), .c(new_n62_), .O(z06));
  inv1   g017(.a(x15), .O(new_n64_));
  oai21  g018(.a(new_n48_), .b(new_n64_), .c(x27), .O(new_n65_));
  oai21  g019(.a(x27), .b(x26), .c(new_n65_), .O(z07));
  inv1   g020(.a(new_n51_), .O(z08));
  inv1   g021(.a(x17), .O(new_n68_));
  inv1   g022(.a(x19), .O(new_n69_));
  nor2   g023(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nor2   g024(.a(x19), .b(x17), .O(new_n71_));
  oai21  g025(.a(new_n71_), .b(new_n70_), .c(x16), .O(new_n72_));
  inv1   g026(.a(x16), .O(new_n73_));
  nand2  g027(.a(x18), .b(x00), .O(new_n74_));
  oai21  g028(.a(x18), .b(new_n47_), .c(new_n74_), .O(new_n75_));
  nand2  g029(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n48_), .O(new_n77_));
  nand3  g031(.a(new_n77_), .b(new_n76_), .c(new_n72_), .O(z09));
  nor2   g032(.a(x27), .b(x18), .O(new_n79_));
  aoi22  g033(.a(new_n79_), .b(x09), .c(x18), .d(x01), .O(new_n80_));
  inv1   g034(.a(x20), .O(new_n81_));
  nand3  g035(.a(new_n81_), .b(new_n69_), .c(new_n68_), .O(new_n82_));
  oai21  g036(.a(new_n71_), .b(new_n81_), .c(new_n82_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n77_), .c(x16), .O(new_n84_));
  oai21  g038(.a(new_n80_), .b(x16), .c(new_n84_), .O(z10));
  inv1   g039(.a(x10), .O(new_n86_));
  nand2  g040(.a(x18), .b(x02), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  inv1   g043(.a(x21), .O(new_n90_));
  nor3   g044(.a(x20), .b(x19), .c(x17), .O(new_n91_));
  nor2   g045(.a(x21), .b(x20), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n71_), .O(new_n93_));
  oai21  g047(.a(new_n91_), .b(new_n90_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(x16), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n89_), .c(new_n77_), .O(z11));
  inv1   g050(.a(x11), .O(new_n97_));
  nand2  g051(.a(x18), .b(x03), .O(new_n98_));
  oai21  g052(.a(x18), .b(new_n97_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n73_), .O(new_n100_));
  inv1   g054(.a(x22), .O(new_n101_));
  aoi21  g055(.a(new_n92_), .b(new_n71_), .c(new_n101_), .O(new_n102_));
  nor2   g056(.a(x22), .b(x21), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n71_), .c(new_n81_), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n102_), .c(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n100_), .c(new_n77_), .O(z12));
  nand2  g061(.a(x18), .b(x04), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n55_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n73_), .O(new_n110_));
  nand2  g064(.a(new_n104_), .b(x23), .O(new_n111_));
  nor2   g065(.a(x23), .b(x22), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n91_), .c(new_n90_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n110_), .c(new_n77_), .O(z13));
  inv1   g070(.a(new_n79_), .O(new_n117_));
  nand2  g071(.a(x18), .b(x05), .O(new_n118_));
  oai21  g072(.a(new_n117_), .b(new_n58_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n73_), .O(new_n120_));
  inv1   g074(.a(x23), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n101_), .c(new_n90_), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n82_), .c(x24), .O(new_n123_));
  nor2   g077(.a(x20), .b(x19), .O(new_n124_));
  nor2   g078(.a(x24), .b(x23), .O(new_n125_));
  nand4  g079(.a(new_n125_), .b(new_n103_), .c(new_n124_), .d(new_n68_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n123_), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n77_), .c(x16), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n120_), .O(z14));
  nand2  g083(.a(x18), .b(x06), .O(new_n130_));
  oai21  g084(.a(new_n117_), .b(new_n61_), .c(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n73_), .O(new_n132_));
  nand2  g086(.a(new_n126_), .b(x25), .O(new_n133_));
  nor2   g087(.a(x25), .b(x24), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n112_), .c(new_n92_), .d(new_n71_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n77_), .c(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n132_), .O(z15));
  nand2  g092(.a(x18), .b(x07), .O(new_n139_));
  oai21  g093(.a(new_n117_), .b(new_n64_), .c(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n73_), .O(new_n141_));
  nand2  g095(.a(new_n135_), .b(x26), .O(new_n142_));
  nor3   g096(.a(x26), .b(x25), .c(x24), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n112_), .c(new_n92_), .d(new_n71_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n77_), .c(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n141_), .O(z16));
  inv1   g101(.a(x24), .O(new_n148_));
  nor2   g102(.a(x26), .b(x25), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(new_n112_), .c(new_n148_), .O(new_n150_));
  nand3  g104(.a(new_n92_), .b(x19), .c(new_n68_), .O(new_n151_));
  nand2  g105(.a(x27), .b(x17), .O(new_n152_));
  oai21  g106(.a(new_n151_), .b(new_n150_), .c(new_n152_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n77_), .O(z17));
endmodule


