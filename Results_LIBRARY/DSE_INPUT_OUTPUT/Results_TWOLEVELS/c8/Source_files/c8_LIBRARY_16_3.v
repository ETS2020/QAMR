// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  nand2  g000(.a(x20), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x18), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n55_), .c(new_n56_), .O(new_n57_));
  oai22  g011(.a(new_n57_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n52_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n48_), .O(z02));
  inv1   g017(.a(x11), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x22), .O(new_n66_));
  nand2  g020(.a(new_n52_), .b(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n65_), .c(new_n48_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n48_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x23), .c(new_n70_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n48_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n52_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n48_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  nor2   g036(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g037(.a(x16), .O(new_n84_));
  inv1   g038(.a(x00), .O(new_n85_));
  nand2  g039(.a(new_n56_), .b(x18), .O(new_n86_));
  oai22  g040(.a(new_n86_), .b(new_n85_), .c(x18), .d(new_n49_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  xnor2a g042(.a(x19), .b(x17), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n47_), .c(x16), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(z09));
  inv1   g045(.a(x09), .O(new_n92_));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nor2   g049(.a(x19), .b(x17), .O(new_n96_));
  inv1   g050(.a(x17), .O(new_n97_));
  nand3  g051(.a(new_n56_), .b(new_n51_), .c(new_n97_), .O(new_n98_));
  oai21  g052(.a(new_n96_), .b(new_n56_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x16), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n95_), .c(new_n47_), .O(z10));
  nand2  g055(.a(x18), .b(x02), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n59_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n84_), .O(new_n104_));
  oai21  g058(.a(new_n61_), .b(new_n84_), .c(new_n55_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g060(.a(new_n96_), .b(new_n61_), .c(new_n56_), .O(new_n107_));
  oai21  g061(.a(new_n96_), .b(new_n61_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n106_), .c(new_n104_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n64_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n84_), .O(new_n113_));
  oai21  g067(.a(new_n66_), .b(new_n84_), .c(new_n55_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x20), .O(new_n115_));
  nor3   g069(.a(x21), .b(x19), .c(x17), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n96_), .c(new_n56_), .O(new_n118_));
  oai21  g072(.a(new_n116_), .b(new_n66_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n113_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n69_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n84_), .O(new_n124_));
  inv1   g078(.a(x23), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n84_), .c(new_n55_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x20), .O(new_n127_));
  aoi21  g081(.a(new_n117_), .b(new_n96_), .c(new_n125_), .O(new_n128_));
  nor2   g082(.a(x23), .b(x22), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n61_), .O(new_n130_));
  nor2   g084(.a(new_n130_), .b(new_n98_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n128_), .c(x16), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n127_), .c(new_n124_), .O(z13));
  inv1   g087(.a(x05), .O(new_n134_));
  oai22  g088(.a(new_n86_), .b(new_n134_), .c(x18), .d(new_n72_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n84_), .O(new_n136_));
  nand3  g090(.a(new_n129_), .b(new_n96_), .c(new_n61_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x24), .O(new_n138_));
  nor2   g092(.a(x24), .b(x23), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n116_), .c(new_n66_), .O(new_n140_));
  aoi21  g094(.a(new_n140_), .b(new_n138_), .c(x20), .O(new_n141_));
  inv1   g095(.a(x24), .O(new_n142_));
  nor3   g096(.a(new_n142_), .b(new_n56_), .c(x18), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n141_), .c(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n136_), .O(z14));
  inv1   g099(.a(x06), .O(new_n146_));
  oai22  g100(.a(new_n86_), .b(new_n146_), .c(x18), .d(new_n75_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n84_), .O(new_n148_));
  nand3  g102(.a(new_n61_), .b(new_n51_), .c(new_n97_), .O(new_n149_));
  nand3  g103(.a(new_n142_), .b(new_n125_), .c(new_n66_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n149_), .c(x25), .O(new_n151_));
  nor2   g105(.a(x21), .b(x19), .O(new_n152_));
  nor2   g106(.a(x25), .b(x24), .O(new_n153_));
  nand4  g107(.a(new_n153_), .b(new_n129_), .c(new_n152_), .d(new_n97_), .O(new_n154_));
  aoi21  g108(.a(new_n154_), .b(new_n151_), .c(x20), .O(new_n155_));
  nor3   g109(.a(new_n77_), .b(new_n56_), .c(x18), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n155_), .c(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n148_), .O(z15));
  inv1   g112(.a(x07), .O(new_n159_));
  oai22  g113(.a(new_n86_), .b(new_n159_), .c(x18), .d(new_n80_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n84_), .O(new_n161_));
  nand2  g115(.a(new_n154_), .b(x26), .O(new_n162_));
  nor2   g116(.a(x26), .b(x25), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n139_), .c(new_n117_), .d(new_n96_), .O(new_n164_));
  aoi21  g118(.a(new_n164_), .b(new_n162_), .c(x20), .O(new_n165_));
  nand2  g119(.a(x26), .b(x20), .O(new_n166_));
  nor2   g120(.a(new_n166_), .b(x18), .O(new_n167_));
  oai21  g121(.a(new_n167_), .b(new_n165_), .c(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n161_), .O(z16));
  nand4  g123(.a(new_n61_), .b(new_n56_), .c(x19), .d(new_n97_), .O(new_n170_));
  nand3  g124(.a(new_n163_), .b(new_n129_), .c(new_n142_), .O(new_n171_));
  oai22  g125(.a(new_n171_), .b(new_n170_), .c(new_n52_), .d(new_n97_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(new_n47_), .O(z17));
endmodule


