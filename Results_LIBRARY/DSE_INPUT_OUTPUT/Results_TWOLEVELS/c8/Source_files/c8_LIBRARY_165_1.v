// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n171_,
    new_n172_, new_n173_, new_n174_, new_n175_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x22), .O(new_n60_));
  inv1   g014(.a(x27), .O(new_n61_));
  nand2  g015(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n59_), .c(new_n49_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x23), .O(new_n66_));
  nand2  g020(.a(new_n61_), .b(new_n66_), .O(new_n67_));
  aoi21  g021(.a(new_n67_), .b(new_n65_), .c(new_n49_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n49_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x24), .c(new_n70_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n61_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n49_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n49_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n61_), .O(z08));
  inv1   g034(.a(x16), .O(new_n81_));
  xor2a  g035(.a(x19), .b(x17), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  oai21  g037(.a(x18), .b(new_n47_), .c(new_n83_), .O(new_n84_));
  aoi21  g038(.a(new_n84_), .b(new_n81_), .c(new_n49_), .O(new_n85_));
  oai21  g039(.a(new_n82_), .b(new_n81_), .c(new_n85_), .O(z09));
  nand2  g040(.a(x18), .b(x01), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n52_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n81_), .O(new_n89_));
  inv1   g043(.a(x20), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n81_), .c(x18), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(x19), .O(new_n92_));
  inv1   g046(.a(x17), .O(new_n93_));
  nor2   g047(.a(new_n90_), .b(new_n93_), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n89_), .O(z10));
  nand2  g051(.a(x18), .b(x02), .O(new_n98_));
  inv1   g052(.a(x18), .O(new_n99_));
  nand2  g053(.a(new_n48_), .b(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n55_), .c(new_n98_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  oai21  g056(.a(x20), .b(x17), .c(x21), .O(new_n103_));
  nor3   g057(.a(x21), .b(x20), .c(x17), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n103_), .c(x19), .O(new_n106_));
  nand3  g060(.a(x21), .b(x19), .c(x18), .O(new_n107_));
  inv1   g061(.a(new_n107_), .O(new_n108_));
  oai21  g062(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n102_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(new_n100_), .b(new_n58_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n81_), .O(new_n113_));
  nor2   g067(.a(x20), .b(x17), .O(new_n114_));
  nor2   g068(.a(x22), .b(x21), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  oai21  g070(.a(new_n104_), .b(new_n60_), .c(new_n116_), .O(new_n117_));
  nor2   g071(.a(new_n60_), .b(new_n48_), .O(new_n118_));
  aoi22  g072(.a(new_n118_), .b(x18), .c(new_n117_), .d(new_n48_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n81_), .c(new_n113_), .O(z12));
  nand2  g074(.a(x18), .b(x04), .O(new_n121_));
  oai21  g075(.a(new_n100_), .b(new_n64_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n81_), .O(new_n123_));
  nand2  g077(.a(new_n116_), .b(x23), .O(new_n124_));
  inv1   g078(.a(x21), .O(new_n125_));
  nor2   g079(.a(x23), .b(x22), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n114_), .c(new_n125_), .O(new_n127_));
  aoi21  g081(.a(new_n127_), .b(new_n124_), .c(x19), .O(new_n128_));
  nand3  g082(.a(x23), .b(x19), .c(x18), .O(new_n129_));
  inv1   g083(.a(new_n129_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n123_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  oai21  g087(.a(x18), .b(new_n69_), .c(new_n133_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n81_), .O(new_n135_));
  inv1   g089(.a(x24), .O(new_n136_));
  oai21  g090(.a(new_n136_), .b(new_n81_), .c(x18), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(x19), .O(new_n138_));
  nand2  g092(.a(new_n127_), .b(x24), .O(new_n139_));
  nor2   g093(.a(x24), .b(x23), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n115_), .c(new_n95_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n138_), .c(new_n135_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n72_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n81_), .O(new_n147_));
  oai21  g101(.a(new_n74_), .b(new_n81_), .c(x18), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x19), .O(new_n149_));
  nor3   g103(.a(x24), .b(x23), .c(x22), .O(new_n150_));
  aoi21  g104(.a(new_n150_), .b(new_n104_), .c(new_n74_), .O(new_n151_));
  nor2   g105(.a(x21), .b(x20), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n48_), .c(new_n93_), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n126_), .O(new_n155_));
  nor2   g109(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n151_), .c(x16), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n149_), .c(new_n147_), .O(z15));
  nand2  g112(.a(x18), .b(x07), .O(new_n159_));
  oai21  g113(.a(new_n100_), .b(new_n77_), .c(new_n159_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n81_), .O(new_n161_));
  nand4  g115(.a(new_n154_), .b(new_n126_), .c(new_n152_), .d(new_n93_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x26), .O(new_n163_));
  nor2   g117(.a(x26), .b(x25), .O(new_n164_));
  nand4  g118(.a(new_n164_), .b(new_n140_), .c(new_n115_), .d(new_n114_), .O(new_n165_));
  aoi21  g119(.a(new_n165_), .b(new_n163_), .c(x19), .O(new_n166_));
  nand3  g120(.a(x26), .b(x19), .c(x18), .O(new_n167_));
  inv1   g121(.a(new_n167_), .O(new_n168_));
  oai21  g122(.a(new_n168_), .b(new_n166_), .c(x16), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n161_), .O(z16));
  inv1   g124(.a(new_n49_), .O(new_n171_));
  nand3  g125(.a(new_n152_), .b(x19), .c(new_n93_), .O(new_n172_));
  nand3  g126(.a(new_n164_), .b(new_n126_), .c(new_n136_), .O(new_n173_));
  oai22  g127(.a(new_n173_), .b(new_n172_), .c(new_n61_), .d(new_n93_), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(x16), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(new_n171_), .O(z17));
endmodule


