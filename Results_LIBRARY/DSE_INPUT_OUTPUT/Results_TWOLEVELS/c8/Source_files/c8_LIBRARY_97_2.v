// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x23), .O(new_n48_));
  nor2   g002(.a(x26), .b(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n49_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n55_), .b(new_n60_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n59_), .c(new_n49_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n55_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n49_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n49_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x24), .c(new_n72_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n49_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x26), .O(new_n77_));
  oai21  g031(.a(new_n55_), .b(x23), .c(new_n77_), .O(new_n78_));
  oai21  g032(.a(new_n55_), .b(x15), .c(new_n78_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n55_), .O(z08));
  inv1   g034(.a(new_n49_), .O(new_n81_));
  inv1   g035(.a(x17), .O(new_n82_));
  inv1   g036(.a(x19), .O(new_n83_));
  nor2   g037(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nor2   g038(.a(x19), .b(x17), .O(new_n85_));
  oai21  g039(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  inv1   g040(.a(x16), .O(new_n87_));
  nand2  g041(.a(x18), .b(x00), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n47_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n86_), .c(new_n81_), .O(z09));
  nand2  g045(.a(x18), .b(x01), .O(new_n92_));
  oai21  g046(.a(x18), .b(new_n52_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n87_), .O(new_n94_));
  nor2   g048(.a(new_n85_), .b(new_n54_), .O(new_n95_));
  nor3   g049(.a(x20), .b(x19), .c(x17), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n95_), .c(x16), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n94_), .c(new_n81_), .O(z10));
  nand2  g052(.a(x18), .b(x02), .O(new_n99_));
  inv1   g053(.a(x18), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x10), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(new_n99_), .c(x16), .O(new_n102_));
  inv1   g056(.a(new_n96_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(x21), .c(x16), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n102_), .c(new_n81_), .O(new_n106_));
  oai21  g060(.a(x23), .b(x22), .c(x26), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(x23), .O(new_n108_));
  nand4  g062(.a(new_n108_), .b(new_n60_), .c(new_n54_), .d(new_n83_), .O(new_n109_));
  inv1   g063(.a(new_n109_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n82_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n106_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n63_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n87_), .O(new_n115_));
  aoi21  g069(.a(new_n77_), .b(x23), .c(x22), .O(new_n116_));
  nand4  g070(.a(new_n116_), .b(new_n60_), .c(new_n54_), .d(new_n83_), .O(new_n117_));
  nand3  g071(.a(new_n85_), .b(new_n60_), .c(new_n54_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x22), .O(new_n119_));
  oai21  g073(.a(new_n117_), .b(x17), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n115_), .c(new_n81_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n68_), .c(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n81_), .c(new_n87_), .O(new_n125_));
  nor2   g079(.a(x22), .b(x21), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n85_), .c(new_n54_), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(x26), .c(x23), .O(new_n128_));
  nand4  g082(.a(new_n96_), .b(new_n48_), .c(new_n65_), .d(new_n60_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n125_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  oai21  g087(.a(x18), .b(new_n71_), .c(new_n133_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n81_), .c(new_n87_), .O(new_n135_));
  nand2  g089(.a(new_n127_), .b(x24), .O(new_n136_));
  inv1   g090(.a(x24), .O(new_n137_));
  nand4  g091(.a(new_n96_), .b(new_n137_), .c(new_n65_), .d(new_n60_), .O(new_n138_));
  aoi21  g092(.a(new_n138_), .b(new_n136_), .c(x23), .O(new_n139_));
  nand3  g093(.a(x26), .b(x24), .c(x23), .O(new_n140_));
  inv1   g094(.a(new_n140_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n135_), .O(z14));
  nand2  g097(.a(x18), .b(x06), .O(new_n144_));
  oai21  g098(.a(x18), .b(new_n74_), .c(new_n144_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n81_), .c(new_n87_), .O(new_n146_));
  nand2  g100(.a(new_n138_), .b(x25), .O(new_n147_));
  inv1   g101(.a(x25), .O(new_n148_));
  nand4  g102(.a(new_n126_), .b(new_n96_), .c(new_n148_), .d(new_n137_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n147_), .c(x23), .O(new_n150_));
  nand3  g104(.a(x26), .b(x25), .c(x23), .O(new_n151_));
  inv1   g105(.a(new_n151_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n146_), .O(z15));
  inv1   g108(.a(x15), .O(new_n155_));
  nand2  g109(.a(x18), .b(x07), .O(new_n156_));
  oai21  g110(.a(x18), .b(new_n155_), .c(new_n156_), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n81_), .c(new_n87_), .O(new_n158_));
  nor2   g112(.a(x24), .b(x23), .O(new_n159_));
  nor2   g113(.a(x26), .b(x25), .O(new_n160_));
  aoi22  g114(.a(new_n160_), .b(new_n159_), .c(x26), .d(x23), .O(new_n161_));
  nand2  g115(.a(x26), .b(x22), .O(new_n162_));
  oai21  g116(.a(new_n161_), .b(x22), .c(new_n162_), .O(new_n163_));
  nand4  g117(.a(new_n163_), .b(new_n60_), .c(new_n54_), .d(new_n83_), .O(new_n164_));
  nand3  g118(.a(new_n148_), .b(new_n137_), .c(new_n60_), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n103_), .c(x26), .O(new_n166_));
  oai21  g120(.a(new_n164_), .b(x17), .c(new_n166_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n158_), .O(z16));
  nand4  g123(.a(new_n60_), .b(new_n54_), .c(x19), .d(new_n82_), .O(new_n170_));
  nand3  g124(.a(new_n160_), .b(new_n137_), .c(new_n65_), .O(new_n171_));
  oai22  g125(.a(new_n171_), .b(new_n170_), .c(new_n55_), .d(new_n82_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(new_n81_), .O(z17));
endmodule


