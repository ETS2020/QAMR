// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x06), .O(new_n52_));
  nand2  g006(.a(x18), .b(new_n52_), .O(new_n53_));
  nand3  g007(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g008(.a(x20), .O(new_n55_));
  nand2  g009(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  nand3  g012(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z01));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x10), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n53_), .O(z02));
  inv1   g018(.a(x11), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x22), .O(new_n67_));
  nand2  g021(.a(new_n48_), .b(new_n67_), .O(new_n68_));
  aoi22  g022(.a(new_n68_), .b(new_n66_), .c(x18), .d(new_n52_), .O(z03));
  inv1   g023(.a(x23), .O(new_n70_));
  nand2  g024(.a(new_n48_), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x12), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  nand3  g027(.a(new_n73_), .b(new_n71_), .c(new_n53_), .O(z04));
  inv1   g028(.a(x24), .O(new_n75_));
  nand2  g029(.a(new_n48_), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x13), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n76_), .c(new_n53_), .O(z05));
  inv1   g033(.a(x25), .O(new_n80_));
  nand2  g034(.a(new_n48_), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x14), .O(new_n82_));
  nand2  g036(.a(x27), .b(new_n82_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n81_), .c(new_n53_), .O(z06));
  inv1   g038(.a(x26), .O(new_n85_));
  nand2  g039(.a(new_n48_), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x15), .O(new_n87_));
  nand2  g041(.a(x27), .b(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n86_), .c(new_n53_), .O(z07));
  nand2  g043(.a(new_n53_), .b(new_n48_), .O(z08));
  inv1   g044(.a(x16), .O(new_n91_));
  inv1   g045(.a(x00), .O(new_n92_));
  nand2  g046(.a(x18), .b(x06), .O(new_n93_));
  oai22  g047(.a(new_n93_), .b(new_n92_), .c(x18), .d(new_n50_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n91_), .O(new_n95_));
  xnor2a g049(.a(x19), .b(x17), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n53_), .c(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(z09));
  nand2  g052(.a(x18), .b(x01), .O(new_n99_));
  oai21  g053(.a(x18), .b(new_n57_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n91_), .O(new_n101_));
  nor2   g055(.a(x19), .b(x17), .O(new_n102_));
  nor2   g056(.a(new_n102_), .b(new_n55_), .O(new_n103_));
  inv1   g057(.a(x17), .O(new_n104_));
  nand3  g058(.a(new_n55_), .b(new_n47_), .c(new_n104_), .O(new_n105_));
  inv1   g059(.a(new_n105_), .O(new_n106_));
  oai21  g060(.a(new_n106_), .b(new_n103_), .c(x16), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n101_), .c(new_n53_), .O(z10));
  inv1   g062(.a(x02), .O(new_n109_));
  oai22  g063(.a(new_n93_), .b(new_n109_), .c(x18), .d(new_n62_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n91_), .O(new_n111_));
  nor2   g065(.a(x21), .b(x20), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n102_), .O(new_n113_));
  oai21  g067(.a(new_n106_), .b(new_n60_), .c(new_n113_), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n53_), .c(x16), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n111_), .O(z11));
  nand2  g070(.a(x18), .b(x03), .O(new_n117_));
  oai21  g071(.a(x18), .b(new_n65_), .c(new_n117_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n91_), .O(new_n119_));
  inv1   g073(.a(new_n113_), .O(new_n120_));
  nand4  g074(.a(new_n102_), .b(new_n67_), .c(new_n60_), .d(new_n55_), .O(new_n121_));
  oai21  g075(.a(new_n120_), .b(new_n67_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n119_), .c(new_n53_), .O(z12));
  inv1   g078(.a(x04), .O(new_n125_));
  oai22  g079(.a(new_n93_), .b(new_n125_), .c(x18), .d(new_n72_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n91_), .O(new_n127_));
  nand2  g081(.a(new_n121_), .b(x23), .O(new_n128_));
  nor2   g082(.a(x23), .b(x22), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n60_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n105_), .c(new_n128_), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n53_), .c(x16), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n127_), .O(z13));
  nand2  g087(.a(x18), .b(x05), .O(new_n134_));
  oai21  g088(.a(x18), .b(new_n77_), .c(new_n134_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n91_), .O(new_n136_));
  nor2   g090(.a(new_n130_), .b(new_n105_), .O(new_n137_));
  nor2   g091(.a(new_n137_), .b(new_n75_), .O(new_n138_));
  nand4  g092(.a(new_n75_), .b(new_n70_), .c(new_n67_), .d(new_n60_), .O(new_n139_));
  nor2   g093(.a(new_n139_), .b(new_n105_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n136_), .c(new_n53_), .O(z14));
  oai21  g096(.a(x18), .b(new_n82_), .c(new_n93_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n91_), .O(new_n144_));
  oai21  g098(.a(x18), .b(new_n91_), .c(new_n93_), .O(new_n145_));
  nand3  g099(.a(new_n75_), .b(new_n67_), .c(new_n60_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n105_), .c(new_n145_), .O(new_n147_));
  nand3  g101(.a(new_n53_), .b(x23), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(x25), .O(new_n150_));
  aoi21  g104(.a(x18), .b(new_n52_), .c(x25), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n75_), .c(new_n70_), .d(new_n67_), .O(new_n152_));
  nor3   g106(.a(new_n152_), .b(x21), .c(x20), .O(new_n153_));
  nand4  g107(.a(new_n153_), .b(new_n47_), .c(new_n104_), .d(x16), .O(new_n154_));
  nand3  g108(.a(new_n154_), .b(new_n150_), .c(new_n144_), .O(z15));
  nand2  g109(.a(x18), .b(x07), .O(new_n156_));
  oai21  g110(.a(x18), .b(new_n87_), .c(new_n156_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n91_), .O(new_n158_));
  nand3  g112(.a(new_n129_), .b(new_n80_), .c(new_n75_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n113_), .c(x26), .O(new_n160_));
  nand4  g114(.a(new_n129_), .b(new_n85_), .c(new_n80_), .d(new_n75_), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(new_n113_), .c(new_n160_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(x16), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n158_), .c(new_n53_), .O(z16));
  nand3  g118(.a(new_n112_), .b(x19), .c(new_n104_), .O(new_n165_));
  oai22  g119(.a(new_n165_), .b(new_n161_), .c(new_n48_), .d(new_n104_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n53_), .O(z17));
endmodule


