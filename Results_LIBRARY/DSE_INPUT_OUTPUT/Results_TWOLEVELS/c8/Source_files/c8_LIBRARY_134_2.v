// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x24), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x21), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n53_), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x09), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  nand3  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g014(.a(x21), .O(new_n61_));
  aoi21  g015(.a(x27), .b(x24), .c(new_n61_), .O(new_n62_));
  oai22  g016(.a(new_n62_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n53_), .b(new_n64_), .O(new_n65_));
  inv1   g019(.a(x11), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  nand3  g021(.a(new_n67_), .b(new_n65_), .c(new_n48_), .O(z03));
  inv1   g022(.a(x12), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x23), .O(new_n71_));
  nand2  g025(.a(new_n53_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n49_), .O(z04));
  inv1   g027(.a(x13), .O(new_n74_));
  nor2   g028(.a(new_n49_), .b(new_n53_), .O(z08));
  nand2  g029(.a(z08), .b(new_n74_), .O(new_n76_));
  nand3  g030(.a(new_n53_), .b(new_n47_), .c(new_n61_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n76_), .O(z05));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n53_), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x14), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n80_), .c(new_n48_), .O(z06));
  inv1   g037(.a(x26), .O(new_n84_));
  nand2  g038(.a(new_n53_), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x15), .O(new_n86_));
  nand2  g040(.a(x27), .b(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n85_), .c(new_n48_), .O(z07));
  inv1   g042(.a(x17), .O(new_n89_));
  nor2   g043(.a(new_n52_), .b(new_n89_), .O(new_n90_));
  nor2   g044(.a(x19), .b(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  inv1   g046(.a(x16), .O(new_n93_));
  nand2  g047(.a(x18), .b(x00), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n50_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n48_), .O(z09));
  nand2  g051(.a(x18), .b(x01), .O(new_n98_));
  oai21  g052(.a(x18), .b(new_n58_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n93_), .O(new_n100_));
  nand2  g054(.a(new_n47_), .b(x21), .O(new_n101_));
  nand4  g055(.a(new_n101_), .b(new_n56_), .c(new_n52_), .d(new_n89_), .O(new_n102_));
  oai21  g056(.a(new_n91_), .b(new_n56_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n100_), .c(new_n48_), .O(z10));
  inv1   g059(.a(x10), .O(new_n106_));
  nor2   g060(.a(x18), .b(new_n106_), .O(new_n107_));
  aoi21  g061(.a(x18), .b(x02), .c(new_n107_), .O(new_n108_));
  nor3   g062(.a(x20), .b(x19), .c(x17), .O(new_n109_));
  nand3  g063(.a(new_n91_), .b(new_n61_), .c(new_n56_), .O(new_n110_));
  oai21  g064(.a(new_n109_), .b(new_n61_), .c(new_n110_), .O(new_n111_));
  aoi21  g065(.a(new_n111_), .b(x16), .c(new_n49_), .O(new_n112_));
  oai21  g066(.a(new_n108_), .b(x16), .c(new_n112_), .O(z11));
  nand2  g067(.a(x18), .b(x03), .O(new_n114_));
  oai21  g068(.a(x18), .b(new_n66_), .c(new_n114_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n93_), .O(new_n116_));
  oai21  g070(.a(new_n64_), .b(new_n93_), .c(x24), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x21), .O(new_n118_));
  nand4  g072(.a(new_n91_), .b(new_n64_), .c(new_n61_), .d(new_n56_), .O(new_n119_));
  oai21  g073(.a(new_n109_), .b(new_n64_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n118_), .c(new_n116_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n69_), .c(new_n123_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n48_), .c(new_n93_), .O(new_n125_));
  nor2   g079(.a(x22), .b(x20), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n91_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x23), .O(new_n128_));
  nor2   g082(.a(x23), .b(x22), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n91_), .c(new_n56_), .O(new_n130_));
  aoi21  g084(.a(new_n130_), .b(new_n128_), .c(x21), .O(new_n131_));
  nand3  g085(.a(x24), .b(x23), .c(x21), .O(new_n132_));
  inv1   g086(.a(new_n132_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n131_), .c(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n125_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n74_), .c(new_n136_), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n48_), .c(new_n93_), .O(new_n138_));
  nand2  g092(.a(x24), .b(x21), .O(new_n139_));
  nand4  g093(.a(new_n47_), .b(new_n71_), .c(new_n64_), .d(new_n61_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand4  g095(.a(new_n141_), .b(new_n56_), .c(new_n52_), .d(new_n89_), .O(new_n142_));
  nand2  g096(.a(new_n130_), .b(x24), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x16), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n138_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(x18), .b(new_n81_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n93_), .O(new_n149_));
  nor2   g103(.a(x25), .b(x23), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n126_), .c(new_n91_), .d(x16), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n61_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n47_), .O(new_n153_));
  nor2   g107(.a(x24), .b(x23), .O(new_n154_));
  nand3  g108(.a(new_n154_), .b(new_n109_), .c(new_n64_), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(x25), .c(x16), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n153_), .c(new_n149_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  oai21  g112(.a(x18), .b(new_n86_), .c(new_n158_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n93_), .O(new_n160_));
  nand4  g114(.a(new_n56_), .b(new_n52_), .c(new_n89_), .d(x16), .O(new_n161_));
  nand3  g115(.a(new_n129_), .b(new_n84_), .c(new_n79_), .O(new_n162_));
  oai21  g116(.a(new_n162_), .b(new_n161_), .c(new_n61_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n47_), .O(new_n164_));
  nand4  g118(.a(new_n129_), .b(new_n109_), .c(new_n79_), .d(new_n47_), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(x26), .c(x16), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(new_n164_), .c(new_n160_), .O(z16));
  nand3  g121(.a(new_n126_), .b(x19), .c(new_n89_), .O(new_n168_));
  nand3  g122(.a(new_n154_), .b(new_n84_), .c(new_n79_), .O(new_n169_));
  oai22  g123(.a(new_n169_), .b(new_n168_), .c(new_n53_), .d(new_n89_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n48_), .O(z17));
endmodule


