// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_;
  inv1   g000(.a(x26), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x09), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x20), .O(new_n58_));
  nand2  g012(.a(new_n53_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n49_), .O(z01));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n53_), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x10), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  nand3  g018(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z02));
  inv1   g019(.a(x22), .O(new_n66_));
  nand2  g020(.a(new_n53_), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x11), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  nand3  g023(.a(new_n69_), .b(new_n67_), .c(new_n48_), .O(z03));
  inv1   g024(.a(x12), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x23), .O(new_n73_));
  nand2  g027(.a(new_n53_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(z04));
  inv1   g029(.a(x24), .O(new_n76_));
  nand2  g030(.a(new_n53_), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x13), .O(new_n78_));
  nand2  g032(.a(x27), .b(new_n78_), .O(new_n79_));
  nand3  g033(.a(new_n79_), .b(new_n77_), .c(new_n48_), .O(z05));
  inv1   g034(.a(x25), .O(new_n81_));
  nand2  g035(.a(new_n53_), .b(new_n81_), .O(new_n82_));
  inv1   g036(.a(x14), .O(new_n83_));
  nand2  g037(.a(x27), .b(new_n83_), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n82_), .c(new_n48_), .O(z06));
  oai21  g039(.a(new_n53_), .b(x18), .c(new_n47_), .O(new_n86_));
  oai21  g040(.a(new_n53_), .b(x15), .c(new_n86_), .O(z07));
  nand2  g041(.a(new_n48_), .b(new_n53_), .O(z08));
  inv1   g042(.a(x16), .O(new_n89_));
  inv1   g043(.a(x00), .O(new_n90_));
  nand2  g044(.a(x26), .b(x18), .O(new_n91_));
  oai22  g045(.a(new_n91_), .b(new_n90_), .c(x18), .d(new_n50_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n89_), .O(new_n93_));
  nand2  g047(.a(x19), .b(x17), .O(new_n94_));
  nor2   g048(.a(x19), .b(x17), .O(new_n95_));
  inv1   g049(.a(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n94_), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n48_), .c(x16), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n93_), .O(z09));
  nand2  g053(.a(x18), .b(x01), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n56_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n89_), .O(new_n102_));
  nor2   g056(.a(new_n95_), .b(new_n58_), .O(new_n103_));
  nor3   g057(.a(x20), .b(x19), .c(x17), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n103_), .c(x16), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n102_), .c(new_n48_), .O(z10));
  inv1   g060(.a(x02), .O(new_n107_));
  oai22  g061(.a(new_n91_), .b(new_n107_), .c(x18), .d(new_n63_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n89_), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n95_), .O(new_n111_));
  oai21  g065(.a(new_n104_), .b(new_n61_), .c(new_n111_), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n48_), .c(x16), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n109_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n68_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n89_), .O(new_n117_));
  aoi21  g071(.a(new_n110_), .b(new_n95_), .c(new_n66_), .O(new_n118_));
  nor2   g072(.a(x22), .b(x21), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n58_), .O(new_n120_));
  nor2   g074(.a(new_n120_), .b(new_n96_), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(new_n118_), .c(x16), .O(new_n122_));
  nand3  g076(.a(new_n122_), .b(new_n117_), .c(new_n48_), .O(z12));
  nor2   g077(.a(x18), .b(new_n71_), .O(new_n124_));
  aoi21  g078(.a(x18), .b(x04), .c(new_n124_), .O(new_n125_));
  nor2   g079(.a(x23), .b(x22), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n104_), .c(new_n61_), .O(new_n127_));
  oai21  g081(.a(new_n121_), .b(new_n73_), .c(new_n127_), .O(new_n128_));
  aoi21  g082(.a(new_n128_), .b(x16), .c(new_n49_), .O(new_n129_));
  oai21  g083(.a(new_n125_), .b(x16), .c(new_n129_), .O(z13));
  inv1   g084(.a(x05), .O(new_n131_));
  oai22  g085(.a(new_n91_), .b(new_n131_), .c(x18), .d(new_n78_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n89_), .O(new_n133_));
  nand3  g087(.a(new_n127_), .b(new_n48_), .c(x24), .O(new_n134_));
  inv1   g088(.a(x17), .O(new_n135_));
  nand4  g089(.a(new_n48_), .b(new_n76_), .c(new_n73_), .d(new_n66_), .O(new_n136_));
  nor2   g090(.a(new_n136_), .b(x21), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n58_), .c(new_n52_), .d(new_n135_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n133_), .O(z14));
  inv1   g095(.a(x06), .O(new_n142_));
  oai22  g096(.a(new_n91_), .b(new_n142_), .c(x18), .d(new_n83_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n89_), .O(new_n144_));
  nand4  g098(.a(new_n119_), .b(new_n104_), .c(new_n76_), .d(new_n73_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n48_), .c(x25), .O(new_n146_));
  aoi21  g100(.a(new_n47_), .b(x18), .c(x25), .O(new_n147_));
  nand4  g101(.a(new_n147_), .b(new_n76_), .c(new_n73_), .d(new_n66_), .O(new_n148_));
  nor2   g102(.a(new_n148_), .b(x21), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n58_), .c(new_n52_), .d(new_n135_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n146_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n144_), .O(z15));
  inv1   g107(.a(x15), .O(new_n154_));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n154_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n89_), .O(new_n157_));
  nor3   g111(.a(x21), .b(x20), .c(x19), .O(new_n158_));
  nor2   g112(.a(new_n81_), .b(x24), .O(new_n159_));
  nand4  g113(.a(new_n159_), .b(new_n126_), .c(new_n110_), .d(new_n95_), .O(new_n160_));
  nand4  g114(.a(new_n160_), .b(new_n158_), .c(new_n126_), .d(new_n76_), .O(new_n161_));
  oai21  g115(.a(new_n161_), .b(x17), .c(x26), .O(new_n162_));
  nor2   g116(.a(x18), .b(x17), .O(new_n163_));
  nor3   g117(.a(x26), .b(x25), .c(x24), .O(new_n164_));
  and2   g118(.a(new_n164_), .b(new_n126_), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(new_n163_), .c(new_n158_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n162_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n157_), .c(new_n48_), .O(z16));
  nand3  g123(.a(new_n48_), .b(x27), .c(x17), .O(new_n170_));
  nand4  g124(.a(new_n165_), .b(new_n163_), .c(new_n110_), .d(x19), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n170_), .c(new_n89_), .O(z17));
endmodule


