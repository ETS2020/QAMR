// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_, new_n163_, new_n164_;
  nor2   g000(.a(x18), .b(x13), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  nand2  g013(.a(new_n51_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n47_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n47_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  inv1   g022(.a(x18), .O(new_n69_));
  oai21  g023(.a(x27), .b(new_n69_), .c(new_n68_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x24), .c(new_n70_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n51_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n47_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n47_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  aoi21  g033(.a(new_n69_), .b(new_n68_), .c(new_n51_), .O(z08));
  inv1   g034(.a(x16), .O(new_n81_));
  nand2  g035(.a(x18), .b(x00), .O(new_n82_));
  nand3  g036(.a(new_n69_), .b(x13), .c(x08), .O(new_n83_));
  nand2  g037(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n81_), .O(new_n85_));
  inv1   g039(.a(new_n47_), .O(new_n86_));
  nand2  g040(.a(x19), .b(x17), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  inv1   g042(.a(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n86_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n85_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n81_), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  nor2   g050(.a(new_n88_), .b(new_n96_), .O(new_n97_));
  nor3   g051(.a(x20), .b(x19), .c(x17), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n95_), .c(new_n86_), .O(z10));
  nand2  g054(.a(x18), .b(x02), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n57_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n81_), .O(new_n103_));
  nand3  g057(.a(new_n88_), .b(new_n59_), .c(new_n96_), .O(new_n104_));
  oai21  g058(.a(new_n98_), .b(new_n59_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n103_), .c(new_n86_), .O(z11));
  nand2  g061(.a(x18), .b(x03), .O(new_n108_));
  oai21  g062(.a(x18), .b(new_n62_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n81_), .O(new_n110_));
  and2   g064(.a(new_n104_), .b(x22), .O(new_n111_));
  inv1   g065(.a(x22), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n59_), .O(new_n113_));
  nor3   g067(.a(new_n113_), .b(new_n89_), .c(x20), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n111_), .c(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n110_), .c(new_n86_), .O(z12));
  nand2  g070(.a(x18), .b(x04), .O(new_n117_));
  oai21  g071(.a(x18), .b(new_n65_), .c(new_n117_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n81_), .O(new_n119_));
  inv1   g073(.a(x23), .O(new_n120_));
  nor2   g074(.a(new_n114_), .b(new_n120_), .O(new_n121_));
  nor3   g075(.a(x23), .b(x22), .c(x21), .O(new_n122_));
  and2   g076(.a(new_n122_), .b(new_n98_), .O(new_n123_));
  oai21  g077(.a(new_n123_), .b(new_n121_), .c(x16), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n119_), .c(new_n86_), .O(z13));
  oai21  g079(.a(new_n69_), .b(x05), .c(new_n81_), .O(new_n126_));
  inv1   g080(.a(x24), .O(new_n127_));
  nor2   g081(.a(new_n123_), .b(new_n127_), .O(new_n128_));
  inv1   g082(.a(new_n98_), .O(new_n129_));
  nor4   g083(.a(new_n113_), .b(new_n129_), .c(x24), .d(x23), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n128_), .c(x16), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n126_), .c(new_n86_), .O(z14));
  nand2  g086(.a(x18), .b(x06), .O(new_n133_));
  nand3  g087(.a(new_n69_), .b(x14), .c(x13), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n81_), .O(new_n136_));
  aoi22  g090(.a(new_n122_), .b(new_n98_), .c(new_n69_), .d(new_n68_), .O(new_n137_));
  nor2   g091(.a(new_n47_), .b(new_n127_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n137_), .c(x25), .O(new_n139_));
  nand4  g093(.a(new_n86_), .b(new_n74_), .c(new_n127_), .d(new_n120_), .O(new_n140_));
  nor2   g094(.a(new_n140_), .b(x22), .O(new_n141_));
  nand4  g095(.a(new_n141_), .b(new_n59_), .c(new_n96_), .d(new_n50_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(x17), .c(new_n139_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n136_), .O(z15));
  nand2  g099(.a(x18), .b(x07), .O(new_n146_));
  nand3  g100(.a(new_n69_), .b(x15), .c(x13), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n81_), .O(new_n149_));
  aoi21  g103(.a(new_n74_), .b(new_n127_), .c(new_n47_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(new_n137_), .c(x26), .O(new_n151_));
  aoi21  g105(.a(new_n69_), .b(new_n68_), .c(x26), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n74_), .c(new_n127_), .d(new_n120_), .O(new_n153_));
  nor2   g107(.a(new_n153_), .b(x22), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n59_), .c(new_n96_), .d(new_n50_), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(x17), .c(new_n151_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n149_), .O(z16));
  inv1   g112(.a(x17), .O(new_n159_));
  nand4  g113(.a(new_n59_), .b(new_n96_), .c(x19), .d(new_n159_), .O(new_n160_));
  nor2   g114(.a(x26), .b(x25), .O(new_n161_));
  nand4  g115(.a(new_n161_), .b(new_n127_), .c(new_n120_), .d(new_n112_), .O(new_n162_));
  oai22  g116(.a(new_n162_), .b(new_n160_), .c(new_n51_), .d(new_n159_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n86_), .O(z17));
endmodule


