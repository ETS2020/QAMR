// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x13), .O(new_n52_));
  inv1   g006(.a(x18), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(new_n54_), .O(new_n56_));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x20), .O(new_n59_));
  nand2  g013(.a(new_n48_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n56_), .O(z01));
  inv1   g015(.a(x21), .O(new_n62_));
  nand2  g016(.a(new_n48_), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x10), .O(new_n64_));
  nand2  g018(.a(x27), .b(new_n64_), .O(new_n65_));
  nand3  g019(.a(new_n65_), .b(new_n63_), .c(new_n54_), .O(z02));
  inv1   g020(.a(x22), .O(new_n67_));
  nand2  g021(.a(new_n48_), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x11), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  nand3  g024(.a(new_n70_), .b(new_n68_), .c(new_n54_), .O(z03));
  inv1   g025(.a(x23), .O(new_n72_));
  nand2  g026(.a(new_n48_), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x12), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  nand3  g029(.a(new_n75_), .b(new_n73_), .c(new_n54_), .O(z04));
  oai21  g030(.a(x27), .b(new_n53_), .c(new_n52_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x24), .c(new_n77_), .O(z05));
  inv1   g032(.a(x14), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x25), .O(new_n81_));
  nand2  g035(.a(new_n48_), .b(new_n81_), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n80_), .c(new_n56_), .O(z06));
  inv1   g037(.a(x26), .O(new_n84_));
  nand2  g038(.a(new_n48_), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x15), .O(new_n86_));
  nand2  g040(.a(x27), .b(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n85_), .c(new_n54_), .O(z07));
  aoi21  g042(.a(new_n53_), .b(new_n52_), .c(new_n48_), .O(z08));
  inv1   g043(.a(x16), .O(new_n90_));
  nand2  g044(.a(x18), .b(x00), .O(new_n91_));
  nand2  g045(.a(new_n53_), .b(x13), .O(new_n92_));
  oai21  g046(.a(new_n92_), .b(new_n50_), .c(new_n91_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nand2  g048(.a(x19), .b(x17), .O(new_n95_));
  nor2   g049(.a(x19), .b(x17), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n54_), .c(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n94_), .O(z09));
  nand2  g054(.a(x18), .b(x01), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n57_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n90_), .O(new_n103_));
  nor2   g057(.a(new_n96_), .b(new_n59_), .O(new_n104_));
  nor3   g058(.a(x20), .b(x19), .c(x17), .O(new_n105_));
  oai21  g059(.a(new_n105_), .b(new_n104_), .c(x16), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n103_), .c(new_n54_), .O(z10));
  nand2  g061(.a(x18), .b(x02), .O(new_n108_));
  oai21  g062(.a(new_n92_), .b(new_n64_), .c(new_n108_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n90_), .O(new_n110_));
  nor2   g064(.a(x21), .b(x20), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n96_), .O(new_n112_));
  oai21  g066(.a(new_n105_), .b(new_n62_), .c(new_n112_), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n54_), .c(x16), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n110_), .O(z11));
  nand2  g069(.a(x18), .b(x03), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n69_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n90_), .O(new_n118_));
  aoi21  g072(.a(new_n111_), .b(new_n96_), .c(new_n67_), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n59_), .O(new_n121_));
  nor2   g075(.a(new_n121_), .b(new_n97_), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n119_), .c(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n118_), .c(new_n54_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(new_n92_), .b(new_n74_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n90_), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n105_), .c(new_n62_), .O(new_n129_));
  oai21  g083(.a(new_n122_), .b(new_n72_), .c(new_n129_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n54_), .c(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n127_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n92_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n90_), .O(new_n135_));
  nand2  g089(.a(x18), .b(x16), .O(new_n136_));
  and2   g090(.a(new_n136_), .b(new_n92_), .O(new_n137_));
  nand3  g091(.a(new_n54_), .b(x23), .c(x16), .O(new_n138_));
  oai21  g092(.a(new_n137_), .b(new_n122_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x24), .O(new_n140_));
  inv1   g094(.a(x17), .O(new_n141_));
  aoi21  g095(.a(new_n53_), .b(new_n52_), .c(x24), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n72_), .c(new_n67_), .d(new_n62_), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(x20), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n47_), .c(new_n141_), .d(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n140_), .c(new_n135_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(x18), .b(new_n79_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n90_), .O(new_n149_));
  nor2   g103(.a(x24), .b(x23), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n120_), .c(new_n105_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x25), .O(new_n152_));
  nor2   g106(.a(x25), .b(x24), .O(new_n153_));
  nand4  g107(.a(new_n153_), .b(new_n128_), .c(new_n111_), .d(new_n96_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x16), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n149_), .c(new_n54_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  nand3  g112(.a(new_n53_), .b(x15), .c(x13), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n158_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n90_), .O(new_n161_));
  nand2  g115(.a(new_n154_), .b(x26), .O(new_n162_));
  inv1   g116(.a(x24), .O(new_n163_));
  nand4  g117(.a(new_n128_), .b(new_n84_), .c(new_n81_), .d(new_n163_), .O(new_n164_));
  oai21  g118(.a(new_n164_), .b(new_n112_), .c(new_n162_), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(new_n54_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n161_), .O(z16));
  nand3  g121(.a(new_n111_), .b(x19), .c(new_n141_), .O(new_n168_));
  oai22  g122(.a(new_n168_), .b(new_n164_), .c(new_n48_), .d(new_n141_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x16), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n54_), .O(z17));
endmodule


