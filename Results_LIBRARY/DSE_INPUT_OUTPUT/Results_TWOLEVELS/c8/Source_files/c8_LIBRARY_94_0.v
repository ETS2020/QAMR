// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_;
  nor2   g000(.a(x18), .b(x17), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  nand2  g013(.a(new_n51_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n47_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n51_), .b(new_n64_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n51_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n47_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n47_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n51_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n47_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  nand2  g034(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x26), .O(new_n82_));
  nand2  g036(.a(new_n51_), .b(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n81_), .c(new_n47_), .O(z07));
  nor2   g038(.a(new_n47_), .b(new_n51_), .O(z08));
  inv1   g039(.a(new_n47_), .O(new_n86_));
  inv1   g040(.a(x17), .O(new_n87_));
  nor2   g041(.a(new_n50_), .b(new_n87_), .O(new_n88_));
  nor2   g042(.a(x19), .b(x17), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  inv1   g044(.a(x16), .O(new_n91_));
  nand2  g045(.a(x18), .b(x00), .O(new_n92_));
  oai21  g046(.a(x18), .b(new_n48_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n90_), .c(new_n86_), .O(z09));
  nand2  g049(.a(x18), .b(x01), .O(new_n96_));
  inv1   g050(.a(x18), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(x17), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n54_), .c(new_n96_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n91_), .O(new_n100_));
  nand2  g054(.a(x20), .b(x19), .O(new_n101_));
  nor2   g055(.a(x20), .b(x19), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n87_), .O(new_n103_));
  aoi21  g057(.a(new_n103_), .b(new_n101_), .c(new_n97_), .O(new_n104_));
  inv1   g058(.a(x20), .O(new_n105_));
  nor2   g059(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  oai21  g060(.a(new_n106_), .b(new_n104_), .c(x16), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n100_), .O(z10));
  nand2  g062(.a(x18), .b(x02), .O(new_n109_));
  oai21  g063(.a(new_n98_), .b(new_n57_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n91_), .O(new_n111_));
  oai21  g065(.a(x20), .b(x19), .c(x21), .O(new_n112_));
  nor2   g066(.a(x21), .b(x20), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n89_), .O(new_n114_));
  aoi21  g068(.a(new_n114_), .b(new_n112_), .c(new_n97_), .O(new_n115_));
  nor2   g069(.a(new_n59_), .b(new_n87_), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n115_), .c(x16), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n111_), .O(z11));
  nand2  g072(.a(x18), .b(x03), .O(new_n119_));
  oai21  g073(.a(x18), .b(new_n62_), .c(new_n119_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n91_), .O(new_n121_));
  nand2  g075(.a(new_n50_), .b(x16), .O(new_n122_));
  nor2   g076(.a(x22), .b(x21), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n105_), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n122_), .c(x18), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n87_), .O(new_n126_));
  nand3  g080(.a(new_n114_), .b(x22), .c(x16), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n126_), .c(new_n121_), .O(z12));
  nand2  g082(.a(x18), .b(x04), .O(new_n129_));
  oai21  g083(.a(new_n98_), .b(new_n67_), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n91_), .O(new_n131_));
  nand2  g085(.a(new_n123_), .b(new_n102_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x23), .O(new_n133_));
  nor2   g087(.a(x23), .b(x22), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n102_), .c(new_n59_), .d(new_n87_), .O(new_n135_));
  aoi21  g089(.a(new_n135_), .b(new_n133_), .c(new_n97_), .O(new_n136_));
  nor2   g090(.a(new_n69_), .b(new_n87_), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n136_), .c(x16), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n131_), .O(z13));
  nand2  g093(.a(x18), .b(x05), .O(new_n140_));
  oai21  g094(.a(x18), .b(new_n72_), .c(new_n140_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n91_), .O(new_n142_));
  nor2   g096(.a(x24), .b(x23), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n123_), .c(new_n102_), .d(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x18), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n87_), .O(new_n146_));
  nand3  g100(.a(new_n135_), .b(x24), .c(x16), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n146_), .c(new_n142_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(new_n98_), .b(new_n75_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n91_), .O(new_n151_));
  nand3  g105(.a(new_n59_), .b(new_n105_), .c(new_n50_), .O(new_n152_));
  inv1   g106(.a(x24), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n69_), .c(new_n64_), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n152_), .c(x25), .O(new_n155_));
  nor2   g109(.a(x25), .b(x24), .O(new_n156_));
  nand4  g110(.a(new_n156_), .b(new_n134_), .c(new_n113_), .d(new_n89_), .O(new_n157_));
  aoi21  g111(.a(new_n157_), .b(new_n155_), .c(new_n97_), .O(new_n158_));
  nor2   g112(.a(new_n77_), .b(new_n87_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n158_), .c(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n151_), .O(z15));
  nand2  g115(.a(x18), .b(x07), .O(new_n162_));
  oai21  g116(.a(new_n98_), .b(new_n80_), .c(new_n162_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n91_), .O(new_n164_));
  nand4  g118(.a(new_n156_), .b(new_n134_), .c(new_n113_), .d(new_n50_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x26), .O(new_n166_));
  nor3   g120(.a(x26), .b(x25), .c(x24), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n134_), .c(new_n113_), .d(new_n89_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n166_), .c(new_n97_), .O(new_n169_));
  nor2   g123(.a(new_n82_), .b(new_n87_), .O(new_n170_));
  oai21  g124(.a(new_n170_), .b(new_n169_), .c(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n164_), .O(z16));
  nand2  g126(.a(new_n167_), .b(new_n134_), .O(new_n173_));
  nand3  g127(.a(new_n113_), .b(x19), .c(new_n87_), .O(new_n174_));
  oai22  g128(.a(new_n174_), .b(new_n173_), .c(new_n51_), .d(new_n87_), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(x16), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(new_n86_), .O(z17));
endmodule


