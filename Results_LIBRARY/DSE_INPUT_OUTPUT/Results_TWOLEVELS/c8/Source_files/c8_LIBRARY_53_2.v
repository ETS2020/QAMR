// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x14), .O(new_n48_));
  nor2   g002(.a(x18), .b(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n58_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  aoi21  g023(.a(x27), .b(new_n69_), .c(new_n49_), .O(new_n70_));
  oai21  g024(.a(x27), .b(x24), .c(new_n70_), .O(z05));
  inv1   g025(.a(x25), .O(new_n72_));
  oai21  g026(.a(x27), .b(new_n72_), .c(new_n48_), .O(new_n73_));
  nand3  g027(.a(new_n58_), .b(new_n72_), .c(x18), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(new_n73_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x26), .O(new_n78_));
  nand2  g032(.a(new_n58_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n49_), .O(z07));
  nor2   g034(.a(new_n49_), .b(new_n58_), .O(z08));
  nor2   g035(.a(x18), .b(x14), .O(new_n82_));
  aoi22  g036(.a(new_n82_), .b(x08), .c(x18), .d(x00), .O(new_n83_));
  inv1   g037(.a(new_n49_), .O(new_n84_));
  nand2  g038(.a(x19), .b(x17), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  inv1   g040(.a(new_n86_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n84_), .c(x16), .O(new_n89_));
  oai21  g043(.a(new_n83_), .b(x16), .c(new_n89_), .O(z09));
  aoi22  g044(.a(new_n82_), .b(x09), .c(x18), .d(x01), .O(new_n91_));
  inv1   g045(.a(x20), .O(new_n92_));
  inv1   g046(.a(x17), .O(new_n93_));
  inv1   g047(.a(x19), .O(new_n94_));
  nand3  g048(.a(new_n92_), .b(new_n94_), .c(new_n93_), .O(new_n95_));
  oai21  g049(.a(new_n86_), .b(new_n92_), .c(new_n95_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n84_), .c(x16), .O(new_n97_));
  oai21  g051(.a(new_n91_), .b(x16), .c(new_n97_), .O(z10));
  aoi22  g052(.a(new_n82_), .b(x10), .c(x18), .d(x02), .O(new_n99_));
  nor3   g053(.a(x20), .b(x19), .c(x17), .O(new_n100_));
  nor2   g054(.a(x21), .b(x20), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n86_), .O(new_n102_));
  oai21  g056(.a(new_n100_), .b(new_n57_), .c(new_n102_), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n84_), .c(x16), .O(new_n104_));
  oai21  g058(.a(new_n99_), .b(x16), .c(new_n104_), .O(z11));
  inv1   g059(.a(x16), .O(new_n106_));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n61_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  aoi21  g063(.a(new_n101_), .b(new_n86_), .c(new_n63_), .O(new_n110_));
  nor2   g064(.a(x22), .b(x21), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n86_), .c(new_n92_), .O(new_n112_));
  inv1   g066(.a(new_n112_), .O(new_n113_));
  oai21  g067(.a(new_n113_), .b(new_n110_), .c(x16), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n109_), .c(new_n84_), .O(z12));
  nand2  g069(.a(x18), .b(x04), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n66_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n106_), .O(new_n118_));
  nand2  g072(.a(new_n112_), .b(x23), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n100_), .c(new_n57_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n118_), .c(new_n84_), .O(z13));
  nand2  g078(.a(x18), .b(x05), .O(new_n125_));
  nand2  g079(.a(new_n82_), .b(x13), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n106_), .O(new_n128_));
  inv1   g082(.a(x23), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n63_), .c(new_n57_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n95_), .c(x24), .O(new_n131_));
  nor2   g085(.a(x20), .b(x19), .O(new_n132_));
  nor2   g086(.a(x24), .b(x23), .O(new_n133_));
  nand4  g087(.a(new_n133_), .b(new_n111_), .c(new_n132_), .d(new_n93_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n131_), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n84_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n128_), .O(z14));
  inv1   g091(.a(new_n102_), .O(new_n138_));
  nor2   g092(.a(x25), .b(x24), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n120_), .O(new_n140_));
  inv1   g094(.a(new_n140_), .O(new_n141_));
  aoi22  g095(.a(new_n141_), .b(new_n138_), .c(new_n134_), .d(x25), .O(new_n142_));
  inv1   g096(.a(x18), .O(new_n143_));
  nor2   g097(.a(new_n143_), .b(x16), .O(new_n144_));
  aoi21  g098(.a(new_n144_), .b(x06), .c(new_n49_), .O(new_n145_));
  oai21  g099(.a(new_n142_), .b(new_n106_), .c(new_n145_), .O(z15));
  nand2  g100(.a(x18), .b(x07), .O(new_n147_));
  nand3  g101(.a(new_n143_), .b(x15), .c(new_n48_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n106_), .O(new_n150_));
  nand4  g104(.a(new_n139_), .b(new_n120_), .c(new_n101_), .d(new_n86_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x26), .O(new_n152_));
  nor3   g106(.a(x26), .b(x25), .c(x24), .O(new_n153_));
  nand4  g107(.a(new_n153_), .b(new_n120_), .c(new_n101_), .d(new_n86_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n152_), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n84_), .c(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n150_), .O(z16));
  inv1   g111(.a(x24), .O(new_n158_));
  nand4  g112(.a(new_n120_), .b(new_n78_), .c(new_n72_), .d(new_n158_), .O(new_n159_));
  nand3  g113(.a(new_n101_), .b(x19), .c(new_n93_), .O(new_n160_));
  oai22  g114(.a(new_n160_), .b(new_n159_), .c(new_n58_), .d(new_n93_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n84_), .O(z17));
endmodule


