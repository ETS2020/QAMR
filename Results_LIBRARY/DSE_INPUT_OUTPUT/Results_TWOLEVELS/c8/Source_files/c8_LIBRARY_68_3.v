// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_;
  inv1   g000(.a(x18), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai22  g003(.a(new_n49_), .b(x08), .c(x27), .d(x19), .O(z00));
  nor2   g004(.a(new_n48_), .b(new_n47_), .O(new_n51_));
  inv1   g005(.a(x09), .O(new_n52_));
  nand2  g006(.a(x27), .b(new_n52_), .O(new_n53_));
  inv1   g007(.a(x20), .O(new_n54_));
  inv1   g008(.a(x27), .O(new_n55_));
  nand2  g009(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  aoi21  g010(.a(new_n56_), .b(new_n53_), .c(new_n51_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n51_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x21), .c(new_n59_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n55_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n51_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n51_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand2  g025(.a(new_n55_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n51_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n51_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n51_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  nor2   g033(.a(new_n51_), .b(new_n55_), .O(z08));
  inv1   g034(.a(new_n51_), .O(new_n81_));
  inv1   g035(.a(x17), .O(new_n82_));
  nor2   g036(.a(new_n48_), .b(new_n82_), .O(new_n83_));
  nor2   g037(.a(x19), .b(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  inv1   g040(.a(x08), .O(new_n87_));
  nand2  g041(.a(x18), .b(x00), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n87_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n85_), .c(new_n81_), .O(z09));
  nand2  g045(.a(x18), .b(x01), .O(new_n92_));
  oai21  g046(.a(x18), .b(new_n52_), .c(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n86_), .O(new_n94_));
  oai21  g048(.a(new_n54_), .b(new_n86_), .c(new_n47_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(x19), .O(new_n96_));
  nor2   g050(.a(new_n54_), .b(new_n82_), .O(new_n97_));
  nor3   g051(.a(x20), .b(x19), .c(x17), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n96_), .c(new_n94_), .O(z10));
  inv1   g054(.a(x02), .O(new_n101_));
  nand2  g055(.a(new_n48_), .b(x18), .O(new_n102_));
  oai22  g056(.a(new_n102_), .b(new_n101_), .c(x18), .d(new_n58_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  nor2   g058(.a(x20), .b(x17), .O(new_n105_));
  inv1   g059(.a(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(x21), .O(new_n107_));
  inv1   g061(.a(x21), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n54_), .c(new_n82_), .O(new_n109_));
  aoi21  g063(.a(new_n109_), .b(new_n107_), .c(x19), .O(new_n110_));
  nor3   g064(.a(new_n108_), .b(new_n48_), .c(x18), .O(new_n111_));
  oai21  g065(.a(new_n111_), .b(new_n110_), .c(x16), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n104_), .O(z11));
  nand2  g067(.a(x18), .b(x03), .O(new_n114_));
  oai21  g068(.a(x18), .b(new_n61_), .c(new_n114_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n86_), .O(new_n116_));
  oai21  g070(.a(new_n63_), .b(new_n86_), .c(new_n47_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x19), .O(new_n118_));
  nand2  g072(.a(new_n109_), .b(x22), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n84_), .c(new_n54_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n119_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n118_), .c(new_n116_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n66_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n86_), .O(new_n127_));
  inv1   g081(.a(x23), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n86_), .c(new_n47_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x19), .O(new_n130_));
  aoi21  g084(.a(new_n120_), .b(new_n105_), .c(new_n128_), .O(new_n131_));
  inv1   g085(.a(new_n98_), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n108_), .O(new_n134_));
  nor2   g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n130_), .c(new_n127_), .O(z13));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n69_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n86_), .O(new_n140_));
  oai21  g094(.a(new_n71_), .b(new_n86_), .c(new_n47_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x19), .O(new_n142_));
  oai21  g096(.a(new_n134_), .b(new_n106_), .c(x24), .O(new_n143_));
  nand4  g097(.a(new_n120_), .b(new_n98_), .c(new_n71_), .d(new_n128_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x16), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n142_), .c(new_n140_), .O(z14));
  inv1   g101(.a(x06), .O(new_n148_));
  oai22  g102(.a(new_n102_), .b(new_n148_), .c(x18), .d(new_n74_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n86_), .O(new_n150_));
  nand3  g104(.a(new_n71_), .b(new_n128_), .c(new_n63_), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n109_), .c(x25), .O(new_n152_));
  nor2   g106(.a(x21), .b(x20), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n133_), .c(new_n153_), .d(new_n82_), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n152_), .c(x19), .O(new_n156_));
  inv1   g110(.a(x25), .O(new_n157_));
  nor3   g111(.a(new_n157_), .b(new_n48_), .c(x18), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n156_), .c(x16), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n150_), .O(z15));
  nand2  g114(.a(x18), .b(x07), .O(new_n161_));
  oai21  g115(.a(x18), .b(new_n77_), .c(new_n161_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n86_), .O(new_n163_));
  inv1   g117(.a(x26), .O(new_n164_));
  oai21  g118(.a(new_n164_), .b(new_n86_), .c(new_n47_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x19), .O(new_n166_));
  nand2  g120(.a(new_n155_), .b(x26), .O(new_n167_));
  nand2  g121(.a(new_n153_), .b(new_n84_), .O(new_n168_));
  nand4  g122(.a(new_n133_), .b(new_n164_), .c(new_n157_), .d(new_n71_), .O(new_n169_));
  oai21  g123(.a(new_n169_), .b(new_n168_), .c(new_n167_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(x16), .O(new_n171_));
  nand3  g125(.a(new_n171_), .b(new_n166_), .c(new_n163_), .O(z16));
  nand3  g126(.a(new_n153_), .b(x19), .c(new_n82_), .O(new_n173_));
  oai22  g127(.a(new_n173_), .b(new_n169_), .c(new_n55_), .d(new_n82_), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(x16), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(new_n81_), .O(z17));
endmodule


