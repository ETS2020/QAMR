// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_;
  nand2  g000(.a(x21), .b(x20), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x20), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n58_), .c(new_n59_), .O(new_n60_));
  oai22  g014(.a(new_n60_), .b(x10), .c(x27), .d(x21), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n52_), .b(new_n64_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n63_), .c(new_n48_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n48_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x23), .c(new_n68_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x24), .O(new_n72_));
  nand2  g026(.a(new_n52_), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(new_n73_), .b(new_n71_), .c(new_n48_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n48_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x25), .c(new_n76_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  aoi21  g032(.a(x27), .b(new_n78_), .c(new_n48_), .O(new_n79_));
  oai21  g033(.a(x27), .b(x26), .c(new_n79_), .O(z07));
  nor2   g034(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g035(.a(x17), .O(new_n82_));
  nor2   g036(.a(new_n51_), .b(new_n82_), .O(new_n83_));
  nor2   g037(.a(x19), .b(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n49_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n85_), .c(new_n47_), .O(z09));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n55_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n86_), .O(new_n93_));
  nor2   g047(.a(new_n84_), .b(new_n58_), .O(new_n94_));
  nand3  g048(.a(new_n58_), .b(new_n51_), .c(new_n82_), .O(new_n95_));
  inv1   g049(.a(new_n95_), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n94_), .c(x16), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n93_), .c(new_n47_), .O(z10));
  inv1   g052(.a(x10), .O(new_n99_));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  oai21  g054(.a(x18), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n47_), .c(new_n86_), .O(new_n102_));
  nand3  g056(.a(new_n59_), .b(new_n51_), .c(new_n82_), .O(new_n103_));
  oai21  g057(.a(new_n84_), .b(new_n59_), .c(new_n103_), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n58_), .c(x16), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n102_), .O(z11));
  nand2  g060(.a(x18), .b(x03), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n62_), .c(new_n107_), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n47_), .c(new_n86_), .O(new_n109_));
  nand2  g063(.a(x22), .b(x21), .O(new_n110_));
  nor2   g064(.a(x22), .b(x21), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n84_), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n110_), .c(x20), .O(new_n113_));
  nand3  g067(.a(new_n95_), .b(x22), .c(new_n59_), .O(new_n114_));
  inv1   g068(.a(new_n114_), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n109_), .O(z12));
  nand2  g071(.a(x18), .b(x04), .O(new_n118_));
  oai21  g072(.a(x18), .b(new_n67_), .c(new_n118_), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n47_), .c(new_n86_), .O(new_n120_));
  inv1   g074(.a(x23), .O(new_n121_));
  oai21  g075(.a(x22), .b(x21), .c(new_n58_), .O(new_n122_));
  nand2  g076(.a(new_n95_), .b(new_n59_), .O(new_n123_));
  aoi21  g077(.a(new_n123_), .b(new_n122_), .c(new_n121_), .O(new_n124_));
  nor2   g078(.a(x23), .b(x22), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n59_), .O(new_n126_));
  nor2   g080(.a(new_n126_), .b(new_n95_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n124_), .c(x16), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n120_), .O(z13));
  nand2  g083(.a(x18), .b(x05), .O(new_n130_));
  oai21  g084(.a(x18), .b(new_n70_), .c(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n86_), .O(new_n132_));
  oai21  g086(.a(new_n72_), .b(new_n86_), .c(new_n59_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x20), .O(new_n134_));
  nand3  g088(.a(new_n125_), .b(new_n84_), .c(new_n59_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(x24), .O(new_n136_));
  nand3  g090(.a(new_n111_), .b(new_n72_), .c(new_n121_), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n95_), .c(new_n136_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n134_), .c(new_n132_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n75_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n86_), .O(new_n143_));
  inv1   g097(.a(x25), .O(new_n144_));
  oai21  g098(.a(new_n144_), .b(new_n86_), .c(new_n59_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x20), .O(new_n146_));
  nand3  g100(.a(new_n72_), .b(new_n121_), .c(new_n64_), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n103_), .c(x25), .O(new_n148_));
  nand3  g102(.a(new_n84_), .b(new_n59_), .c(new_n58_), .O(new_n149_));
  nor2   g103(.a(x25), .b(x24), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n125_), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n149_), .c(new_n148_), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(x16), .O(new_n153_));
  nand3  g107(.a(new_n153_), .b(new_n146_), .c(new_n143_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n78_), .c(new_n155_), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n47_), .c(new_n86_), .O(new_n157_));
  inv1   g111(.a(x26), .O(new_n158_));
  nand3  g112(.a(new_n150_), .b(new_n111_), .c(new_n121_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n58_), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(new_n123_), .c(new_n158_), .O(new_n161_));
  nand4  g115(.a(new_n125_), .b(new_n158_), .c(new_n144_), .d(new_n72_), .O(new_n162_));
  nor2   g116(.a(new_n162_), .b(new_n149_), .O(new_n163_));
  oai21  g117(.a(new_n163_), .b(new_n161_), .c(x16), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n157_), .O(z16));
  nand4  g119(.a(new_n59_), .b(new_n58_), .c(x19), .d(new_n82_), .O(new_n166_));
  oai22  g120(.a(new_n166_), .b(new_n162_), .c(new_n52_), .d(new_n82_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(new_n47_), .O(z17));
endmodule


