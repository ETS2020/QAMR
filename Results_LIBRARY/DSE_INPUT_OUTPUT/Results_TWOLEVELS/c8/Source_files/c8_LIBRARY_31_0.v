// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  oai21  g003(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  oai21  g004(.a(new_n47_), .b(x08), .c(new_n50_), .O(z00));
  inv1   g005(.a(x20), .O(new_n52_));
  nand3  g006(.a(new_n47_), .b(new_n52_), .c(x18), .O(new_n53_));
  oai21  g007(.a(new_n47_), .b(x09), .c(new_n53_), .O(z01));
  inv1   g008(.a(x21), .O(new_n55_));
  nand3  g009(.a(new_n47_), .b(new_n55_), .c(x18), .O(new_n56_));
  oai21  g010(.a(new_n47_), .b(x10), .c(new_n56_), .O(z02));
  inv1   g011(.a(x22), .O(new_n58_));
  oai21  g012(.a(new_n58_), .b(new_n48_), .c(new_n47_), .O(new_n59_));
  oai21  g013(.a(new_n47_), .b(x11), .c(new_n59_), .O(z03));
  inv1   g014(.a(x23), .O(new_n61_));
  oai21  g015(.a(new_n61_), .b(new_n48_), .c(new_n47_), .O(new_n62_));
  oai21  g016(.a(new_n47_), .b(x12), .c(new_n62_), .O(z04));
  inv1   g017(.a(x24), .O(new_n64_));
  oai21  g018(.a(new_n64_), .b(new_n48_), .c(new_n47_), .O(new_n65_));
  oai21  g019(.a(new_n47_), .b(x13), .c(new_n65_), .O(z05));
  inv1   g020(.a(x25), .O(new_n67_));
  oai21  g021(.a(new_n67_), .b(new_n48_), .c(new_n47_), .O(new_n68_));
  oai21  g022(.a(new_n47_), .b(x14), .c(new_n68_), .O(z06));
  inv1   g023(.a(x26), .O(new_n70_));
  oai21  g024(.a(new_n70_), .b(new_n48_), .c(new_n47_), .O(new_n71_));
  oai21  g025(.a(new_n47_), .b(x15), .c(new_n71_), .O(z07));
  inv1   g026(.a(x16), .O(new_n73_));
  inv1   g027(.a(x08), .O(new_n74_));
  nand2  g028(.a(x18), .b(x00), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n48_), .O(new_n76_));
  oai21  g030(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand2  g032(.a(new_n47_), .b(new_n48_), .O(new_n79_));
  nand2  g033(.a(x19), .b(x17), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  inv1   g035(.a(new_n81_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n79_), .c(x16), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n78_), .O(z09));
  inv1   g039(.a(x09), .O(new_n86_));
  nand2  g040(.a(x18), .b(x01), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n86_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n73_), .O(new_n89_));
  nor2   g043(.a(new_n81_), .b(new_n52_), .O(new_n90_));
  nor3   g044(.a(x20), .b(x19), .c(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n89_), .c(new_n79_), .O(z10));
  inv1   g047(.a(x10), .O(new_n94_));
  nand2  g048(.a(x18), .b(x02), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n73_), .O(new_n97_));
  nor2   g051(.a(x21), .b(x20), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n81_), .O(new_n99_));
  oai21  g053(.a(new_n91_), .b(new_n55_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x16), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n97_), .c(new_n79_), .O(z11));
  inv1   g056(.a(x11), .O(new_n103_));
  nand2  g057(.a(x18), .b(x03), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n103_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n73_), .O(new_n106_));
  aoi21  g060(.a(new_n98_), .b(new_n81_), .c(new_n58_), .O(new_n107_));
  nor2   g061(.a(x22), .b(x21), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n81_), .c(new_n52_), .O(new_n109_));
  inv1   g063(.a(new_n109_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n106_), .c(new_n79_), .O(z12));
  inv1   g066(.a(x12), .O(new_n113_));
  nand2  g067(.a(x18), .b(x04), .O(new_n114_));
  oai21  g068(.a(x18), .b(new_n113_), .c(new_n114_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n73_), .O(new_n116_));
  nand2  g070(.a(new_n109_), .b(x23), .O(new_n117_));
  nor2   g071(.a(x23), .b(x22), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n91_), .c(new_n55_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n116_), .c(new_n79_), .O(z13));
  inv1   g076(.a(x13), .O(new_n123_));
  nand2  g077(.a(x18), .b(x05), .O(new_n124_));
  oai21  g078(.a(new_n76_), .b(new_n123_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n73_), .O(new_n126_));
  inv1   g080(.a(x17), .O(new_n127_));
  nand3  g081(.a(new_n52_), .b(new_n49_), .c(new_n127_), .O(new_n128_));
  nand3  g082(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n128_), .c(x24), .O(new_n130_));
  nor2   g084(.a(x20), .b(x19), .O(new_n131_));
  nor2   g085(.a(x24), .b(x23), .O(new_n132_));
  nand4  g086(.a(new_n132_), .b(new_n108_), .c(new_n131_), .d(new_n127_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n79_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n126_), .O(z14));
  inv1   g090(.a(x14), .O(new_n137_));
  nand2  g091(.a(x18), .b(x06), .O(new_n138_));
  oai21  g092(.a(new_n76_), .b(new_n137_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n73_), .O(new_n140_));
  nand2  g094(.a(new_n133_), .b(x25), .O(new_n141_));
  nor2   g095(.a(x25), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n142_), .b(new_n118_), .c(new_n98_), .d(new_n81_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n141_), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n79_), .c(x16), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n140_), .O(z15));
  inv1   g100(.a(x15), .O(new_n147_));
  nand2  g101(.a(x18), .b(x07), .O(new_n148_));
  oai21  g102(.a(x18), .b(new_n147_), .c(new_n148_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n73_), .O(new_n150_));
  nand2  g104(.a(new_n143_), .b(x26), .O(new_n151_));
  nor3   g105(.a(x26), .b(x25), .c(x24), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n118_), .c(new_n98_), .d(new_n81_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x16), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n150_), .c(new_n79_), .O(z16));
  nand4  g110(.a(new_n118_), .b(new_n70_), .c(new_n67_), .d(new_n64_), .O(new_n157_));
  nand3  g111(.a(new_n98_), .b(x19), .c(new_n127_), .O(new_n158_));
  oai22  g112(.a(new_n158_), .b(new_n157_), .c(new_n47_), .d(new_n127_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n79_), .O(z17));
  buf    g115(.a(x27), .O(z08));
endmodule


