// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n143_, new_n144_, new_n145_, new_n146_;
  nor2   g000(.a(x18), .b(x16), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x21), .O(new_n61_));
  nand2  g015(.a(new_n51_), .b(new_n61_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n60_), .c(new_n47_), .O(z02));
  inv1   g017(.a(x11), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n47_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x22), .c(new_n65_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n51_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n47_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n47_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  nand2  g031(.a(new_n51_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n47_), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  nand2  g034(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x26), .O(new_n82_));
  nand2  g036(.a(new_n51_), .b(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n81_), .c(new_n47_), .O(z07));
  nor2   g038(.a(new_n47_), .b(new_n51_), .O(z08));
  inv1   g039(.a(x17), .O(new_n86_));
  nor2   g040(.a(new_n50_), .b(new_n86_), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n87_), .c(x16), .O(new_n89_));
  inv1   g043(.a(x16), .O(new_n90_));
  inv1   g044(.a(x18), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(x00), .c(new_n90_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n89_), .O(z09));
  oai21  g047(.a(new_n91_), .b(x01), .c(new_n90_), .O(new_n94_));
  nor2   g048(.a(new_n88_), .b(new_n56_), .O(new_n95_));
  nor3   g049(.a(x20), .b(x19), .c(x17), .O(new_n96_));
  oai21  g050(.a(new_n96_), .b(new_n95_), .c(x16), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n94_), .O(z10));
  oai21  g052(.a(new_n91_), .b(x02), .c(new_n90_), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n86_), .O(new_n101_));
  nand4  g055(.a(new_n61_), .b(new_n56_), .c(new_n50_), .d(new_n86_), .O(new_n102_));
  inv1   g056(.a(new_n102_), .O(new_n103_));
  aoi21  g057(.a(new_n101_), .b(x21), .c(new_n103_), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n90_), .c(new_n99_), .O(z11));
  oai21  g059(.a(new_n91_), .b(x03), .c(new_n90_), .O(new_n106_));
  nand2  g060(.a(new_n102_), .b(x22), .O(new_n107_));
  nor2   g061(.a(x22), .b(x21), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n88_), .c(new_n56_), .O(new_n109_));
  and2   g063(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n90_), .c(new_n106_), .O(z12));
  oai21  g065(.a(new_n91_), .b(x04), .c(new_n90_), .O(new_n112_));
  nor3   g066(.a(x23), .b(x22), .c(x21), .O(new_n113_));
  aoi22  g067(.a(new_n113_), .b(new_n96_), .c(new_n109_), .d(x23), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n90_), .c(new_n112_), .O(z13));
  inv1   g069(.a(x24), .O(new_n116_));
  aoi21  g070(.a(new_n113_), .b(new_n96_), .c(new_n116_), .O(new_n117_));
  nor2   g071(.a(x24), .b(x23), .O(new_n118_));
  nand4  g072(.a(new_n118_), .b(new_n108_), .c(new_n100_), .d(new_n86_), .O(new_n119_));
  inv1   g073(.a(new_n119_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nor2   g075(.a(new_n91_), .b(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x05), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n121_), .O(z14));
  inv1   g078(.a(x22), .O(new_n125_));
  nand4  g079(.a(new_n77_), .b(new_n116_), .c(new_n69_), .d(new_n125_), .O(new_n126_));
  inv1   g080(.a(new_n126_), .O(new_n127_));
  aoi22  g081(.a(new_n127_), .b(new_n103_), .c(new_n119_), .d(x25), .O(new_n128_));
  nand2  g082(.a(new_n122_), .b(x06), .O(new_n129_));
  oai21  g083(.a(new_n128_), .b(new_n90_), .c(new_n129_), .O(z15));
  nor2   g084(.a(x21), .b(x20), .O(new_n131_));
  nor2   g085(.a(x23), .b(x22), .O(new_n132_));
  nor2   g086(.a(x25), .b(x24), .O(new_n133_));
  nand4  g087(.a(new_n133_), .b(new_n132_), .c(new_n131_), .d(new_n88_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(x26), .O(new_n135_));
  nor3   g089(.a(x26), .b(x25), .c(x24), .O(new_n136_));
  nand4  g090(.a(new_n136_), .b(new_n132_), .c(new_n131_), .d(new_n88_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x16), .O(new_n139_));
  nand2  g093(.a(new_n122_), .b(x07), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n139_), .O(z16));
  inv1   g095(.a(new_n47_), .O(new_n142_));
  nand4  g096(.a(new_n132_), .b(new_n82_), .c(new_n77_), .d(new_n116_), .O(new_n143_));
  nand3  g097(.a(new_n131_), .b(x19), .c(new_n86_), .O(new_n144_));
  oai22  g098(.a(new_n144_), .b(new_n143_), .c(new_n51_), .d(new_n86_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n142_), .O(z17));
endmodule


