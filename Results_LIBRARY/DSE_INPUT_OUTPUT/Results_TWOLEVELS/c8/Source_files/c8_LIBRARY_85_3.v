// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x21), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x22), .O(new_n60_));
  inv1   g014(.a(x27), .O(new_n61_));
  nand2  g015(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g016(.a(new_n62_), .b(new_n59_), .c(new_n49_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n61_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n61_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n49_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n49_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n61_), .O(z08));
  inv1   g034(.a(x17), .O(new_n81_));
  inv1   g035(.a(x19), .O(new_n82_));
  nor2   g036(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g037(.a(x19), .b(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n47_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n85_), .c(new_n48_), .O(z09));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n52_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n86_), .O(new_n93_));
  inv1   g047(.a(x20), .O(new_n94_));
  nor2   g048(.a(new_n84_), .b(new_n94_), .O(new_n95_));
  nand3  g049(.a(new_n94_), .b(new_n82_), .c(new_n81_), .O(new_n96_));
  inv1   g050(.a(new_n96_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n93_), .c(new_n48_), .O(z10));
  inv1   g053(.a(x02), .O(new_n100_));
  inv1   g054(.a(x21), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(x18), .O(new_n102_));
  oai22  g056(.a(new_n102_), .b(new_n100_), .c(x18), .d(new_n55_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  inv1   g058(.a(x18), .O(new_n105_));
  nand3  g059(.a(new_n96_), .b(x21), .c(new_n105_), .O(new_n106_));
  nor2   g060(.a(x21), .b(x20), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n84_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n104_), .O(z11));
  inv1   g065(.a(x03), .O(new_n112_));
  oai22  g066(.a(new_n102_), .b(new_n112_), .c(x18), .d(new_n58_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n86_), .O(new_n114_));
  nor2   g068(.a(x22), .b(x20), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n84_), .O(new_n116_));
  oai21  g070(.a(new_n97_), .b(new_n60_), .c(new_n116_), .O(new_n117_));
  nor3   g071(.a(new_n60_), .b(new_n101_), .c(x18), .O(new_n118_));
  aoi21  g072(.a(new_n117_), .b(new_n101_), .c(new_n118_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n86_), .c(new_n114_), .O(z12));
  nand2  g074(.a(x18), .b(x04), .O(new_n121_));
  oai21  g075(.a(x18), .b(new_n64_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n86_), .O(new_n123_));
  inv1   g077(.a(x23), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n86_), .c(new_n105_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x21), .O(new_n126_));
  aoi21  g080(.a(new_n115_), .b(new_n84_), .c(new_n124_), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n101_), .O(new_n129_));
  nor2   g083(.a(new_n129_), .b(new_n96_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n127_), .c(x16), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n126_), .c(new_n123_), .O(z13));
  nand2  g086(.a(x18), .b(x05), .O(new_n133_));
  oai21  g087(.a(x18), .b(new_n67_), .c(new_n133_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n86_), .O(new_n135_));
  oai21  g089(.a(new_n69_), .b(new_n86_), .c(new_n105_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x21), .O(new_n137_));
  nand3  g091(.a(new_n128_), .b(new_n84_), .c(new_n94_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x24), .O(new_n139_));
  nor2   g093(.a(x24), .b(x23), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n60_), .c(new_n101_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n96_), .c(new_n139_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n137_), .c(new_n135_), .O(z14));
  inv1   g098(.a(x06), .O(new_n145_));
  oai22  g099(.a(new_n102_), .b(new_n145_), .c(x18), .d(new_n72_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n86_), .O(new_n147_));
  nand3  g101(.a(new_n69_), .b(new_n124_), .c(new_n60_), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n96_), .c(x25), .O(new_n149_));
  nor2   g103(.a(x20), .b(x19), .O(new_n150_));
  nor2   g104(.a(x25), .b(x24), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n128_), .c(new_n150_), .d(new_n81_), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n149_), .c(x21), .O(new_n153_));
  nor3   g107(.a(new_n74_), .b(new_n101_), .c(x18), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n153_), .c(x16), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n147_), .O(z15));
  inv1   g110(.a(x07), .O(new_n157_));
  oai22  g111(.a(new_n102_), .b(new_n157_), .c(x18), .d(new_n77_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n86_), .O(new_n159_));
  nand2  g113(.a(new_n152_), .b(x26), .O(new_n160_));
  nor2   g114(.a(x26), .b(x25), .O(new_n161_));
  nand4  g115(.a(new_n161_), .b(new_n140_), .c(new_n115_), .d(new_n84_), .O(new_n162_));
  aoi21  g116(.a(new_n162_), .b(new_n160_), .c(x21), .O(new_n163_));
  nand2  g117(.a(x26), .b(x21), .O(new_n164_));
  nor2   g118(.a(new_n164_), .b(x18), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n163_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n159_), .O(z16));
  nand2  g121(.a(z08), .b(x17), .O(new_n168_));
  nor2   g122(.a(new_n82_), .b(x17), .O(new_n169_));
  nor3   g123(.a(x26), .b(x25), .c(x24), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n169_), .c(new_n128_), .d(new_n107_), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n168_), .c(new_n86_), .O(z17));
endmodule


