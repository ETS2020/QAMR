// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n152_, new_n153_, new_n154_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_;
  inv1   g000(.a(x22), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n53_), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x09), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  nand3  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g014(.a(x10), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x21), .O(new_n63_));
  nand2  g017(.a(new_n53_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z02));
  inv1   g019(.a(x11), .O(new_n66_));
  nand3  g020(.a(new_n48_), .b(x27), .c(new_n66_), .O(new_n67_));
  inv1   g021(.a(x18), .O(new_n68_));
  nand3  g022(.a(new_n53_), .b(new_n47_), .c(new_n68_), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n67_), .O(z03));
  inv1   g024(.a(x12), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x23), .O(new_n73_));
  nand2  g027(.a(new_n53_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(z04));
  inv1   g029(.a(x13), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x24), .O(new_n78_));
  nand2  g032(.a(new_n53_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n49_), .O(z05));
  inv1   g034(.a(x25), .O(new_n81_));
  nand2  g035(.a(new_n53_), .b(new_n81_), .O(new_n82_));
  inv1   g036(.a(x14), .O(new_n83_));
  nand2  g037(.a(x27), .b(new_n83_), .O(new_n84_));
  nand3  g038(.a(new_n84_), .b(new_n82_), .c(new_n48_), .O(z06));
  inv1   g039(.a(x26), .O(new_n86_));
  nand2  g040(.a(new_n53_), .b(new_n86_), .O(new_n87_));
  inv1   g041(.a(x15), .O(new_n88_));
  nand2  g042(.a(x27), .b(new_n88_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n87_), .c(new_n48_), .O(z07));
  nand2  g044(.a(new_n48_), .b(new_n53_), .O(z08));
  inv1   g045(.a(x16), .O(new_n92_));
  inv1   g046(.a(x00), .O(new_n93_));
  nand2  g047(.a(x22), .b(x18), .O(new_n94_));
  oai22  g048(.a(new_n94_), .b(new_n93_), .c(x18), .d(new_n50_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n92_), .O(new_n96_));
  inv1   g050(.a(x17), .O(new_n97_));
  nor3   g051(.a(new_n49_), .b(new_n52_), .c(new_n97_), .O(new_n98_));
  nor3   g052(.a(new_n49_), .b(x19), .c(x17), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n96_), .O(z09));
  inv1   g055(.a(x01), .O(new_n102_));
  oai22  g056(.a(new_n94_), .b(new_n102_), .c(x18), .d(new_n58_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n92_), .O(new_n104_));
  nor2   g058(.a(x19), .b(x17), .O(new_n105_));
  inv1   g059(.a(new_n105_), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n48_), .c(x20), .O(new_n107_));
  nand4  g061(.a(new_n48_), .b(new_n56_), .c(new_n52_), .d(new_n97_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n104_), .O(z10));
  nand2  g065(.a(x18), .b(x02), .O(new_n112_));
  oai21  g066(.a(x18), .b(new_n61_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n92_), .O(new_n114_));
  aoi21  g068(.a(new_n47_), .b(x18), .c(x21), .O(new_n115_));
  nand4  g069(.a(new_n115_), .b(new_n56_), .c(new_n52_), .d(new_n97_), .O(new_n116_));
  nor3   g070(.a(x20), .b(x19), .c(x17), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n63_), .c(new_n116_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x16), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n114_), .c(new_n48_), .O(z11));
  inv1   g074(.a(x03), .O(new_n121_));
  oai22  g075(.a(new_n94_), .b(new_n121_), .c(x18), .d(new_n66_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n92_), .O(new_n123_));
  nand2  g077(.a(x22), .b(x21), .O(new_n124_));
  nand3  g078(.a(new_n47_), .b(new_n63_), .c(new_n68_), .O(new_n125_));
  aoi21  g079(.a(new_n125_), .b(new_n124_), .c(x20), .O(new_n126_));
  nor2   g080(.a(new_n47_), .b(new_n56_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n126_), .c(new_n52_), .O(new_n128_));
  nand2  g082(.a(new_n106_), .b(x22), .O(new_n129_));
  oai21  g083(.a(new_n128_), .b(x17), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n123_), .O(z12));
  inv1   g086(.a(x04), .O(new_n133_));
  oai22  g087(.a(new_n94_), .b(new_n133_), .c(x18), .d(new_n71_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n92_), .O(new_n135_));
  nand3  g089(.a(new_n105_), .b(new_n63_), .c(new_n56_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x23), .O(new_n137_));
  nor2   g091(.a(x23), .b(x22), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n117_), .c(new_n63_), .O(new_n139_));
  aoi21  g093(.a(new_n139_), .b(new_n137_), .c(x18), .O(new_n140_));
  nor2   g094(.a(new_n73_), .b(new_n47_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n140_), .c(x16), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n135_), .O(z13));
  inv1   g097(.a(x05), .O(new_n144_));
  oai22  g098(.a(new_n94_), .b(new_n144_), .c(x18), .d(new_n76_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n92_), .O(new_n146_));
  nor2   g100(.a(x23), .b(x21), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n105_), .c(new_n56_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(x24), .O(new_n149_));
  nor2   g103(.a(x22), .b(x21), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n117_), .c(new_n78_), .d(new_n73_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n149_), .c(x18), .O(new_n152_));
  nor2   g106(.a(new_n78_), .b(new_n47_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n152_), .c(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n146_), .O(z14));
  nand2  g109(.a(x18), .b(x06), .O(new_n156_));
  oai21  g110(.a(x18), .b(new_n83_), .c(new_n156_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n92_), .O(new_n158_));
  nand4  g112(.a(new_n56_), .b(new_n52_), .c(new_n97_), .d(x16), .O(new_n159_));
  nand3  g113(.a(new_n147_), .b(new_n81_), .c(new_n78_), .O(new_n160_));
  oai21  g114(.a(new_n160_), .b(new_n159_), .c(new_n68_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n47_), .O(new_n162_));
  nand3  g116(.a(new_n151_), .b(x25), .c(x16), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n162_), .c(new_n158_), .O(z15));
  nand2  g118(.a(x18), .b(x07), .O(new_n165_));
  oai21  g119(.a(x18), .b(new_n88_), .c(new_n165_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n92_), .O(new_n167_));
  nand4  g121(.a(new_n147_), .b(new_n86_), .c(new_n81_), .d(new_n78_), .O(new_n168_));
  oai21  g122(.a(new_n168_), .b(new_n159_), .c(new_n68_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n47_), .O(new_n170_));
  inv1   g124(.a(new_n136_), .O(new_n171_));
  nand4  g125(.a(new_n138_), .b(new_n171_), .c(new_n81_), .d(new_n78_), .O(new_n172_));
  nand3  g126(.a(new_n172_), .b(x26), .c(x16), .O(new_n173_));
  nand3  g127(.a(new_n173_), .b(new_n170_), .c(new_n167_), .O(z16));
  nand4  g128(.a(new_n63_), .b(new_n56_), .c(x19), .d(new_n97_), .O(new_n175_));
  nand4  g129(.a(new_n138_), .b(new_n86_), .c(new_n81_), .d(new_n78_), .O(new_n176_));
  oai22  g130(.a(new_n176_), .b(new_n175_), .c(new_n53_), .d(new_n97_), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(x16), .O(new_n178_));
  nand2  g132(.a(new_n178_), .b(new_n48_), .O(z17));
endmodule


