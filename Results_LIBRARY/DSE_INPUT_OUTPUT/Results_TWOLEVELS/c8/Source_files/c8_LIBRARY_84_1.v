// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n151_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_, new_n179_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x19), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x21), .O(new_n57_));
  inv1   g011(.a(x27), .O(new_n58_));
  nand2  g012(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n56_), .c(new_n49_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n58_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n49_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  inv1   g022(.a(x13), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand2  g025(.a(new_n58_), .b(new_n71_), .O(new_n72_));
  aoi21  g026(.a(new_n72_), .b(new_n70_), .c(new_n49_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x25), .O(new_n76_));
  nand2  g030(.a(new_n58_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n49_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n49_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x26), .c(new_n80_), .O(z07));
  nor2   g035(.a(new_n49_), .b(new_n58_), .O(z08));
  inv1   g036(.a(new_n49_), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  nor2   g038(.a(new_n48_), .b(new_n84_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n47_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n83_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  inv1   g047(.a(x18), .O(new_n94_));
  nand2  g048(.a(new_n48_), .b(new_n94_), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n52_), .c(new_n93_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  inv1   g051(.a(x20), .O(new_n98_));
  nor2   g052(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  nor2   g053(.a(x20), .b(x17), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n99_), .c(new_n48_), .O(new_n101_));
  nand3  g055(.a(x20), .b(x19), .c(x18), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n101_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(x16), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n97_), .O(z10));
  nand2  g059(.a(x18), .b(x02), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n55_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n88_), .O(new_n108_));
  oai21  g062(.a(new_n57_), .b(new_n88_), .c(x18), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x19), .O(new_n110_));
  nor2   g064(.a(x21), .b(x20), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n86_), .O(new_n112_));
  oai21  g066(.a(new_n100_), .b(new_n57_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(x16), .O(new_n114_));
  nand3  g068(.a(new_n114_), .b(new_n110_), .c(new_n108_), .O(z11));
  nand2  g069(.a(x18), .b(x03), .O(new_n116_));
  oai21  g070(.a(x18), .b(new_n61_), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n88_), .O(new_n118_));
  oai21  g072(.a(new_n63_), .b(new_n88_), .c(x18), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x19), .O(new_n120_));
  nand3  g074(.a(new_n57_), .b(new_n98_), .c(new_n84_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(x22), .O(new_n122_));
  nor2   g076(.a(x22), .b(x21), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n86_), .c(new_n98_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n122_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x16), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n120_), .c(new_n118_), .O(z12));
  nand2  g081(.a(x18), .b(x04), .O(new_n128_));
  oai21  g082(.a(x18), .b(new_n66_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n88_), .O(new_n130_));
  inv1   g084(.a(x23), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n88_), .c(x18), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x19), .O(new_n133_));
  aoi21  g087(.a(new_n123_), .b(new_n100_), .c(new_n131_), .O(new_n134_));
  nand3  g088(.a(new_n98_), .b(new_n48_), .c(new_n84_), .O(new_n135_));
  nor2   g089(.a(x23), .b(x22), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n57_), .O(new_n137_));
  nor2   g091(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n134_), .c(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n133_), .c(new_n130_), .O(z13));
  nand2  g094(.a(x18), .b(x05), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n69_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n88_), .O(new_n143_));
  oai21  g097(.a(new_n71_), .b(new_n88_), .c(x18), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x19), .O(new_n145_));
  nand3  g099(.a(new_n136_), .b(new_n100_), .c(new_n57_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(x24), .O(new_n147_));
  nor2   g101(.a(x24), .b(x23), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n123_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n135_), .c(new_n147_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n145_), .c(new_n143_), .O(z14));
  nand2  g106(.a(x18), .b(x06), .O(new_n153_));
  oai21  g107(.a(new_n95_), .b(new_n74_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n88_), .O(new_n155_));
  nand3  g109(.a(new_n71_), .b(new_n131_), .c(new_n63_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n121_), .c(x25), .O(new_n157_));
  nor2   g111(.a(x25), .b(x24), .O(new_n158_));
  nand4  g112(.a(new_n158_), .b(new_n136_), .c(new_n111_), .d(new_n84_), .O(new_n159_));
  aoi21  g113(.a(new_n159_), .b(new_n157_), .c(x19), .O(new_n160_));
  nand3  g114(.a(x25), .b(x19), .c(x18), .O(new_n161_));
  inv1   g115(.a(new_n161_), .O(new_n162_));
  oai21  g116(.a(new_n162_), .b(new_n160_), .c(x16), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n155_), .O(z15));
  nand2  g118(.a(x18), .b(x07), .O(new_n165_));
  oai21  g119(.a(new_n95_), .b(new_n79_), .c(new_n165_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n88_), .O(new_n167_));
  nand2  g121(.a(new_n159_), .b(x26), .O(new_n168_));
  nor2   g122(.a(x26), .b(x25), .O(new_n169_));
  nand4  g123(.a(new_n169_), .b(new_n148_), .c(new_n123_), .d(new_n100_), .O(new_n170_));
  aoi21  g124(.a(new_n170_), .b(new_n168_), .c(x19), .O(new_n171_));
  nand3  g125(.a(x26), .b(x19), .c(x18), .O(new_n172_));
  inv1   g126(.a(new_n172_), .O(new_n173_));
  oai21  g127(.a(new_n173_), .b(new_n171_), .c(x16), .O(new_n174_));
  nand2  g128(.a(new_n174_), .b(new_n167_), .O(z16));
  nand3  g129(.a(new_n111_), .b(x19), .c(new_n84_), .O(new_n176_));
  nand3  g130(.a(new_n169_), .b(new_n136_), .c(new_n71_), .O(new_n177_));
  oai22  g131(.a(new_n177_), .b(new_n176_), .c(new_n58_), .d(new_n84_), .O(new_n178_));
  nand2  g132(.a(new_n178_), .b(x16), .O(new_n179_));
  nand2  g133(.a(new_n179_), .b(new_n83_), .O(z17));
endmodule


