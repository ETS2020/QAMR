// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n167_, new_n168_, new_n169_, new_n170_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  nor2   g003(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  aoi21  g004(.a(x27), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  oai21  g005(.a(x27), .b(x19), .c(new_n51_), .O(z00));
  inv1   g006(.a(x09), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n53_), .c(new_n50_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n50_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n50_), .O(new_n60_));
  oai21  g014(.a(x27), .b(x22), .c(new_n60_), .O(z03));
  inv1   g015(.a(x12), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n50_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x23), .c(new_n63_), .O(z04));
  inv1   g018(.a(x13), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n50_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x24), .c(new_n66_), .O(z05));
  inv1   g021(.a(x14), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n50_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x25), .c(new_n69_), .O(z06));
  inv1   g024(.a(x15), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x26), .O(new_n73_));
  inv1   g027(.a(x27), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n72_), .c(new_n50_), .O(z07));
  inv1   g030(.a(new_n50_), .O(new_n77_));
  nand2  g031(.a(new_n77_), .b(new_n74_), .O(z08));
  nand3  g032(.a(x19), .b(new_n48_), .c(x17), .O(new_n79_));
  inv1   g033(.a(new_n79_), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g036(.a(x16), .O(new_n83_));
  inv1   g037(.a(x00), .O(new_n84_));
  nand2  g038(.a(new_n49_), .b(x18), .O(new_n85_));
  oai22  g039(.a(new_n85_), .b(new_n84_), .c(x18), .d(new_n47_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n82_), .O(z09));
  inv1   g042(.a(x01), .O(new_n89_));
  oai22  g043(.a(new_n85_), .b(new_n89_), .c(x18), .d(new_n53_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  inv1   g045(.a(x17), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nor2   g048(.a(x20), .b(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(new_n49_), .O(new_n96_));
  nand3  g050(.a(x20), .b(x19), .c(new_n48_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x16), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n91_), .O(z10));
  inv1   g054(.a(x02), .O(new_n101_));
  oai22  g055(.a(new_n85_), .b(new_n101_), .c(x18), .d(new_n56_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  oai21  g057(.a(x20), .b(x17), .c(x21), .O(new_n104_));
  inv1   g058(.a(x21), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n93_), .c(new_n92_), .O(new_n106_));
  aoi21  g060(.a(new_n106_), .b(new_n104_), .c(x19), .O(new_n107_));
  nor3   g061(.a(new_n105_), .b(new_n49_), .c(x18), .O(new_n108_));
  oai21  g062(.a(new_n108_), .b(new_n107_), .c(x16), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n103_), .O(z11));
  inv1   g064(.a(x03), .O(new_n111_));
  oai22  g065(.a(new_n85_), .b(new_n111_), .c(x18), .d(new_n59_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n83_), .O(new_n113_));
  inv1   g067(.a(x22), .O(new_n114_));
  nor3   g068(.a(x21), .b(x20), .c(x17), .O(new_n115_));
  nand3  g069(.a(new_n95_), .b(new_n114_), .c(new_n105_), .O(new_n116_));
  oai21  g070(.a(new_n115_), .b(new_n114_), .c(new_n116_), .O(new_n117_));
  nor3   g071(.a(new_n114_), .b(new_n49_), .c(x18), .O(new_n118_));
  aoi21  g072(.a(new_n117_), .b(new_n49_), .c(new_n118_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n83_), .c(new_n113_), .O(z12));
  inv1   g074(.a(x04), .O(new_n121_));
  oai22  g075(.a(new_n85_), .b(new_n121_), .c(x18), .d(new_n62_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n83_), .O(new_n123_));
  nand2  g077(.a(new_n116_), .b(x23), .O(new_n124_));
  nor2   g078(.a(x23), .b(x22), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(new_n95_), .c(new_n105_), .O(new_n126_));
  aoi21  g080(.a(new_n126_), .b(new_n124_), .c(x19), .O(new_n127_));
  inv1   g081(.a(x23), .O(new_n128_));
  nor3   g082(.a(new_n128_), .b(new_n49_), .c(x18), .O(new_n129_));
  oai21  g083(.a(new_n129_), .b(new_n127_), .c(x16), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n123_), .O(z13));
  inv1   g085(.a(x05), .O(new_n132_));
  oai22  g086(.a(new_n85_), .b(new_n132_), .c(x18), .d(new_n65_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n83_), .O(new_n134_));
  nand2  g088(.a(new_n126_), .b(x24), .O(new_n135_));
  nor3   g089(.a(x24), .b(x23), .c(x22), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n115_), .O(new_n137_));
  aoi21  g091(.a(new_n137_), .b(new_n135_), .c(x19), .O(new_n138_));
  inv1   g092(.a(x24), .O(new_n139_));
  nor3   g093(.a(new_n139_), .b(new_n49_), .c(x18), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n134_), .O(z14));
  inv1   g096(.a(x06), .O(new_n143_));
  oai22  g097(.a(new_n85_), .b(new_n143_), .c(x18), .d(new_n68_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n83_), .O(new_n145_));
  nand3  g099(.a(new_n139_), .b(new_n128_), .c(new_n114_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n106_), .c(x25), .O(new_n147_));
  nor2   g101(.a(x25), .b(x24), .O(new_n148_));
  nand3  g102(.a(new_n148_), .b(new_n125_), .c(new_n115_), .O(new_n149_));
  aoi21  g103(.a(new_n149_), .b(new_n147_), .c(x19), .O(new_n150_));
  nand2  g104(.a(x25), .b(x19), .O(new_n151_));
  nor2   g105(.a(new_n151_), .b(x18), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n145_), .O(z15));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n71_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n83_), .O(new_n157_));
  oai21  g111(.a(new_n73_), .b(new_n83_), .c(new_n48_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x19), .O(new_n159_));
  nand2  g113(.a(new_n149_), .b(x26), .O(new_n160_));
  nor2   g114(.a(x21), .b(x20), .O(new_n161_));
  nor3   g115(.a(x26), .b(x25), .c(x24), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n125_), .c(new_n161_), .d(new_n81_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n160_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x16), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(new_n159_), .c(new_n157_), .O(z16));
  nand2  g120(.a(new_n162_), .b(new_n125_), .O(new_n167_));
  nand3  g121(.a(new_n161_), .b(x19), .c(new_n92_), .O(new_n168_));
  oai22  g122(.a(new_n168_), .b(new_n167_), .c(new_n74_), .d(new_n92_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x16), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n77_), .O(z17));
endmodule


