// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n50_, new_n51_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n121_, new_n122_, new_n123_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n154_, new_n155_, new_n156_, new_n157_, new_n158_, new_n159_;
  inv1   g000(.a(x08), .O(new_n47_));
  oai21  g001(.a(x18), .b(new_n47_), .c(x27), .O(new_n48_));
  oai21  g002(.a(x27), .b(x19), .c(new_n48_), .O(z00));
  inv1   g003(.a(x18), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  oai22  g005(.a(new_n51_), .b(x09), .c(x27), .d(x20), .O(z01));
  oai22  g006(.a(new_n51_), .b(x10), .c(x27), .d(x21), .O(z02));
  nand2  g007(.a(new_n50_), .b(x11), .O(new_n54_));
  nand2  g008(.a(new_n54_), .b(x27), .O(new_n55_));
  oai21  g009(.a(x27), .b(x22), .c(new_n55_), .O(z03));
  nand2  g010(.a(new_n50_), .b(x12), .O(new_n57_));
  nand2  g011(.a(new_n57_), .b(x27), .O(new_n58_));
  oai21  g012(.a(x27), .b(x23), .c(new_n58_), .O(z04));
  inv1   g013(.a(x13), .O(new_n60_));
  oai21  g014(.a(x18), .b(new_n60_), .c(x27), .O(new_n61_));
  oai21  g015(.a(x27), .b(x24), .c(new_n61_), .O(z05));
  inv1   g016(.a(x14), .O(new_n63_));
  oai21  g017(.a(x18), .b(new_n63_), .c(x27), .O(new_n64_));
  oai21  g018(.a(x27), .b(x25), .c(new_n64_), .O(z06));
  nand2  g019(.a(new_n50_), .b(x15), .O(new_n66_));
  nand2  g020(.a(new_n66_), .b(x27), .O(new_n67_));
  oai21  g021(.a(x27), .b(x26), .c(new_n67_), .O(z07));
  inv1   g022(.a(new_n51_), .O(z08));
  inv1   g023(.a(x16), .O(new_n70_));
  inv1   g024(.a(x00), .O(new_n71_));
  inv1   g025(.a(x27), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(x18), .O(new_n73_));
  oai22  g027(.a(new_n73_), .b(new_n71_), .c(x18), .d(new_n47_), .O(new_n74_));
  nand2  g028(.a(new_n74_), .b(new_n70_), .O(new_n75_));
  nand2  g029(.a(x27), .b(x18), .O(new_n76_));
  nand2  g030(.a(x19), .b(x17), .O(new_n77_));
  nor2   g031(.a(x19), .b(x17), .O(new_n78_));
  inv1   g032(.a(new_n78_), .O(new_n79_));
  nand2  g033(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand3  g034(.a(new_n80_), .b(new_n76_), .c(x16), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n75_), .O(z09));
  inv1   g036(.a(x09), .O(new_n83_));
  nand2  g037(.a(x18), .b(x01), .O(new_n84_));
  oai21  g038(.a(x18), .b(new_n83_), .c(new_n84_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n70_), .O(new_n86_));
  inv1   g040(.a(x20), .O(new_n87_));
  nor2   g041(.a(new_n78_), .b(new_n87_), .O(new_n88_));
  nor3   g042(.a(x20), .b(x19), .c(x17), .O(new_n89_));
  oai21  g043(.a(new_n89_), .b(new_n88_), .c(x16), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n86_), .c(new_n76_), .O(z10));
  inv1   g045(.a(x10), .O(new_n92_));
  nand2  g046(.a(x18), .b(x02), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n92_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n70_), .O(new_n95_));
  inv1   g049(.a(x21), .O(new_n96_));
  nor2   g050(.a(x21), .b(x20), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n78_), .O(new_n98_));
  oai21  g052(.a(new_n89_), .b(new_n96_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(x16), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n95_), .c(new_n76_), .O(z11));
  nand2  g055(.a(x18), .b(x03), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n54_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n70_), .O(new_n104_));
  inv1   g058(.a(x22), .O(new_n105_));
  aoi21  g059(.a(new_n97_), .b(new_n78_), .c(new_n105_), .O(new_n106_));
  nor2   g060(.a(x22), .b(x21), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n78_), .c(new_n87_), .O(new_n108_));
  inv1   g062(.a(new_n108_), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n106_), .c(x16), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n104_), .c(new_n76_), .O(z12));
  nand2  g065(.a(x18), .b(x04), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n57_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n70_), .O(new_n114_));
  nand2  g068(.a(new_n108_), .b(x23), .O(new_n115_));
  nor2   g069(.a(x23), .b(x22), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n89_), .c(new_n96_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(x16), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n114_), .c(new_n76_), .O(z13));
  inv1   g074(.a(x05), .O(new_n121_));
  oai22  g075(.a(new_n73_), .b(new_n121_), .c(x18), .d(new_n60_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n70_), .O(new_n123_));
  inv1   g077(.a(x17), .O(new_n124_));
  inv1   g078(.a(x19), .O(new_n125_));
  nand3  g079(.a(new_n87_), .b(new_n125_), .c(new_n124_), .O(new_n126_));
  inv1   g080(.a(x23), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n105_), .c(new_n96_), .O(new_n128_));
  oai21  g082(.a(new_n128_), .b(new_n126_), .c(x24), .O(new_n129_));
  nor2   g083(.a(x20), .b(x19), .O(new_n130_));
  nor2   g084(.a(x24), .b(x23), .O(new_n131_));
  nand4  g085(.a(new_n131_), .b(new_n107_), .c(new_n130_), .d(new_n124_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n76_), .c(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n123_), .O(z14));
  inv1   g089(.a(x06), .O(new_n136_));
  oai22  g090(.a(new_n73_), .b(new_n136_), .c(x18), .d(new_n63_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n70_), .O(new_n138_));
  nand2  g092(.a(new_n132_), .b(x25), .O(new_n139_));
  nor2   g093(.a(x25), .b(x24), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n116_), .c(new_n97_), .d(new_n78_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n139_), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n76_), .c(x16), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n138_), .O(z15));
  nand2  g098(.a(x18), .b(x07), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n66_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n70_), .O(new_n147_));
  nand2  g101(.a(new_n141_), .b(x26), .O(new_n148_));
  nor3   g102(.a(x26), .b(x25), .c(x24), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n116_), .c(new_n97_), .d(new_n78_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n147_), .c(new_n76_), .O(z16));
  inv1   g107(.a(x24), .O(new_n154_));
  nor2   g108(.a(x26), .b(x25), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n116_), .c(new_n154_), .O(new_n156_));
  nand3  g110(.a(new_n97_), .b(x19), .c(new_n124_), .O(new_n157_));
  oai22  g111(.a(new_n157_), .b(new_n156_), .c(new_n72_), .d(new_n124_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x16), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n76_), .O(z17));
endmodule


