// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_, new_n176_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x20), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  aoi21  g005(.a(x27), .b(x18), .c(new_n48_), .O(new_n52_));
  oai22  g006(.a(new_n52_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n49_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x22), .O(new_n59_));
  inv1   g013(.a(x27), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n58_), .c(new_n49_), .O(z03));
  inv1   g016(.a(x12), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x23), .O(new_n65_));
  nand2  g019(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n49_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x24), .c(new_n69_), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x25), .O(new_n73_));
  nand2  g027(.a(new_n60_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n49_), .O(z06));
  inv1   g029(.a(x15), .O(new_n76_));
  nand2  g030(.a(x27), .b(new_n76_), .O(new_n77_));
  inv1   g031(.a(x26), .O(new_n78_));
  nand2  g032(.a(new_n60_), .b(new_n78_), .O(new_n79_));
  aoi21  g033(.a(new_n79_), .b(new_n77_), .c(new_n49_), .O(z07));
  inv1   g034(.a(new_n49_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n60_), .O(z08));
  inv1   g036(.a(x17), .O(new_n83_));
  inv1   g037(.a(x19), .O(new_n84_));
  nor2   g038(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n47_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n81_), .O(z09));
  inv1   g046(.a(x09), .O(new_n93_));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n88_), .O(new_n96_));
  nor2   g050(.a(new_n86_), .b(new_n48_), .O(new_n97_));
  nor3   g051(.a(x20), .b(x19), .c(x17), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n97_), .c(x16), .O(new_n99_));
  nand3  g053(.a(new_n99_), .b(new_n96_), .c(new_n81_), .O(z10));
  nand2  g054(.a(x18), .b(x02), .O(new_n101_));
  oai21  g055(.a(x18), .b(new_n54_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n88_), .O(new_n103_));
  inv1   g057(.a(x21), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n88_), .c(x18), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(x20), .O(new_n106_));
  nand3  g060(.a(new_n86_), .b(new_n104_), .c(new_n48_), .O(new_n107_));
  oai21  g061(.a(new_n86_), .b(new_n104_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n106_), .c(new_n103_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  inv1   g065(.a(x18), .O(new_n112_));
  nand2  g066(.a(new_n48_), .b(new_n112_), .O(new_n113_));
  oai21  g067(.a(new_n113_), .b(new_n57_), .c(new_n111_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n88_), .O(new_n115_));
  nand3  g069(.a(new_n104_), .b(new_n84_), .c(new_n83_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(x22), .O(new_n117_));
  nor2   g071(.a(x22), .b(x21), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n86_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  nor2   g074(.a(new_n59_), .b(new_n48_), .O(new_n121_));
  aoi22  g075(.a(new_n121_), .b(x18), .c(new_n120_), .d(new_n48_), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n88_), .c(new_n115_), .O(z12));
  nand2  g077(.a(x18), .b(x04), .O(new_n124_));
  oai21  g078(.a(x18), .b(new_n63_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n88_), .O(new_n126_));
  oai21  g080(.a(new_n65_), .b(new_n88_), .c(x18), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x20), .O(new_n128_));
  aoi21  g082(.a(new_n118_), .b(new_n86_), .c(new_n65_), .O(new_n129_));
  inv1   g083(.a(new_n98_), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n104_), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n129_), .c(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n128_), .c(new_n126_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n68_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n88_), .O(new_n138_));
  inv1   g092(.a(x24), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n88_), .c(x18), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x20), .O(new_n141_));
  nand3  g095(.a(new_n131_), .b(new_n86_), .c(new_n104_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x24), .O(new_n143_));
  nor2   g097(.a(x24), .b(x23), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n118_), .c(new_n98_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(x16), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n141_), .c(new_n138_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(new_n113_), .b(new_n71_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n88_), .O(new_n151_));
  nand3  g105(.a(new_n139_), .b(new_n65_), .c(new_n59_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n116_), .c(x25), .O(new_n153_));
  nor2   g107(.a(x21), .b(x19), .O(new_n154_));
  nor2   g108(.a(x25), .b(x24), .O(new_n155_));
  nand4  g109(.a(new_n155_), .b(new_n131_), .c(new_n154_), .d(new_n83_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n153_), .c(x20), .O(new_n157_));
  nand3  g111(.a(x25), .b(x20), .c(x18), .O(new_n158_));
  inv1   g112(.a(new_n158_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n151_), .O(z15));
  nand2  g115(.a(x18), .b(x07), .O(new_n162_));
  oai21  g116(.a(new_n113_), .b(new_n76_), .c(new_n162_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n88_), .O(new_n164_));
  nand2  g118(.a(new_n156_), .b(x26), .O(new_n165_));
  nor2   g119(.a(x26), .b(x25), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n144_), .c(new_n118_), .d(new_n86_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n165_), .c(x20), .O(new_n168_));
  nand3  g122(.a(x26), .b(x20), .c(x18), .O(new_n169_));
  inv1   g123(.a(new_n169_), .O(new_n170_));
  oai21  g124(.a(new_n170_), .b(new_n168_), .c(x16), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n164_), .O(z16));
  nand4  g126(.a(new_n104_), .b(new_n48_), .c(x19), .d(new_n83_), .O(new_n173_));
  nand3  g127(.a(new_n166_), .b(new_n131_), .c(new_n139_), .O(new_n174_));
  oai22  g128(.a(new_n174_), .b(new_n173_), .c(new_n60_), .d(new_n83_), .O(new_n175_));
  nand2  g129(.a(new_n175_), .b(x16), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(new_n81_), .O(z17));
endmodule


