// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_;
  inv1   g000(.a(x21), .O(new_n47_));
  nand2  g001(.a(x24), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n53_), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x09), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  nand3  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z01));
  inv1   g014(.a(x10), .O(new_n61_));
  nor2   g015(.a(new_n49_), .b(new_n53_), .O(z08));
  nand2  g016(.a(z08), .b(new_n61_), .O(new_n63_));
  inv1   g017(.a(x24), .O(new_n64_));
  nand3  g018(.a(new_n53_), .b(new_n64_), .c(new_n47_), .O(new_n65_));
  nand2  g019(.a(new_n65_), .b(new_n63_), .O(z02));
  inv1   g020(.a(x22), .O(new_n67_));
  nand2  g021(.a(new_n53_), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x11), .O(new_n69_));
  nand2  g023(.a(x27), .b(new_n69_), .O(new_n70_));
  nand3  g024(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(z03));
  inv1   g025(.a(x12), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x23), .O(new_n74_));
  nand2  g028(.a(new_n53_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n49_), .O(z04));
  aoi21  g030(.a(x27), .b(x21), .c(new_n64_), .O(new_n77_));
  oai22  g031(.a(new_n77_), .b(x13), .c(x27), .d(x24), .O(z05));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n53_), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x14), .O(new_n81_));
  nand2  g035(.a(x27), .b(new_n81_), .O(new_n82_));
  nand3  g036(.a(new_n82_), .b(new_n80_), .c(new_n48_), .O(z06));
  inv1   g037(.a(x26), .O(new_n84_));
  nand2  g038(.a(new_n53_), .b(new_n84_), .O(new_n85_));
  inv1   g039(.a(x15), .O(new_n86_));
  nand2  g040(.a(x27), .b(new_n86_), .O(new_n87_));
  nand3  g041(.a(new_n87_), .b(new_n85_), .c(new_n48_), .O(z07));
  inv1   g042(.a(x17), .O(new_n89_));
  nor2   g043(.a(new_n52_), .b(new_n89_), .O(new_n90_));
  nor2   g044(.a(x19), .b(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  inv1   g046(.a(x16), .O(new_n93_));
  nand2  g047(.a(x18), .b(x00), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n50_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n48_), .O(z09));
  nand2  g051(.a(x18), .b(x01), .O(new_n98_));
  oai21  g052(.a(x18), .b(new_n58_), .c(new_n98_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n93_), .O(new_n100_));
  nor2   g054(.a(new_n91_), .b(new_n56_), .O(new_n101_));
  nor2   g055(.a(x20), .b(x19), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n89_), .O(new_n103_));
  inv1   g057(.a(new_n103_), .O(new_n104_));
  oai21  g058(.a(new_n104_), .b(new_n101_), .c(x16), .O(new_n105_));
  nand3  g059(.a(new_n105_), .b(new_n100_), .c(new_n48_), .O(z10));
  nand2  g060(.a(x18), .b(x02), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n61_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n93_), .O(new_n109_));
  nand3  g063(.a(new_n102_), .b(new_n89_), .c(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n64_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n47_), .O(new_n112_));
  nand3  g066(.a(new_n103_), .b(x21), .c(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n112_), .c(new_n109_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  oai21  g069(.a(x18), .b(new_n69_), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n93_), .O(new_n117_));
  nand2  g071(.a(new_n89_), .b(x16), .O(new_n118_));
  nor2   g072(.a(x22), .b(x20), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n52_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n118_), .c(new_n64_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n47_), .O(new_n122_));
  nand3  g076(.a(new_n91_), .b(new_n47_), .c(new_n56_), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(x22), .c(x16), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n122_), .c(new_n117_), .O(z12));
  nand2  g079(.a(x18), .b(x04), .O(new_n126_));
  oai21  g080(.a(x18), .b(new_n72_), .c(new_n126_), .O(new_n127_));
  nand3  g081(.a(new_n127_), .b(new_n48_), .c(new_n93_), .O(new_n128_));
  nand4  g082(.a(new_n67_), .b(new_n56_), .c(new_n52_), .d(new_n89_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n64_), .O(new_n130_));
  aoi21  g084(.a(new_n130_), .b(new_n47_), .c(new_n74_), .O(new_n131_));
  nor2   g085(.a(x22), .b(x21), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n64_), .c(new_n74_), .O(new_n133_));
  nor2   g087(.a(new_n133_), .b(new_n103_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n131_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n128_), .O(z13));
  inv1   g090(.a(x13), .O(new_n137_));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n137_), .c(new_n138_), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n48_), .c(new_n93_), .O(new_n140_));
  aoi21  g094(.a(x24), .b(x21), .c(new_n134_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n93_), .c(new_n140_), .O(z14));
  nand2  g096(.a(x18), .b(x06), .O(new_n143_));
  oai21  g097(.a(x18), .b(new_n81_), .c(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n93_), .O(new_n145_));
  nor2   g099(.a(x25), .b(x23), .O(new_n146_));
  nand4  g100(.a(new_n146_), .b(new_n119_), .c(new_n91_), .d(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n64_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n47_), .O(new_n149_));
  nand4  g103(.a(new_n104_), .b(new_n74_), .c(new_n67_), .d(new_n47_), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(x25), .c(x16), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n149_), .c(new_n145_), .O(z15));
  nand2  g106(.a(x18), .b(x07), .O(new_n153_));
  oai21  g107(.a(x18), .b(new_n86_), .c(new_n153_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n93_), .O(new_n155_));
  nand4  g109(.a(new_n84_), .b(new_n79_), .c(new_n74_), .d(new_n67_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n110_), .c(new_n64_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n47_), .O(new_n158_));
  nand3  g112(.a(new_n146_), .b(new_n132_), .c(new_n104_), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(x26), .c(x16), .O(new_n160_));
  nand3  g114(.a(new_n160_), .b(new_n158_), .c(new_n155_), .O(z16));
  nand4  g115(.a(new_n47_), .b(new_n56_), .c(x19), .d(new_n89_), .O(new_n162_));
  oai22  g116(.a(new_n162_), .b(new_n156_), .c(new_n53_), .d(new_n89_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n48_), .O(z17));
endmodule


