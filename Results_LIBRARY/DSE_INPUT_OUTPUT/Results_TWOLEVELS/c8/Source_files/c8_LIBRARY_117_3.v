// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n169_, new_n170_, new_n171_,
    new_n172_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x20), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x18), .O(new_n52_));
  inv1   g006(.a(x20), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai22  g008(.a(new_n54_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n49_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  nand2  g013(.a(x27), .b(new_n59_), .O(new_n60_));
  inv1   g014(.a(x22), .O(new_n61_));
  inv1   g015(.a(x27), .O(new_n62_));
  nand2  g016(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n60_), .c(new_n49_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n49_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand2  g024(.a(new_n62_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n49_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x25), .O(new_n75_));
  nand2  g029(.a(new_n62_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n49_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  aoi21  g032(.a(x27), .b(new_n78_), .c(new_n49_), .O(new_n79_));
  oai21  g033(.a(x27), .b(x26), .c(new_n79_), .O(z07));
  nor2   g034(.a(new_n49_), .b(new_n62_), .O(z08));
  inv1   g035(.a(x16), .O(new_n82_));
  inv1   g036(.a(x00), .O(new_n83_));
  nand2  g037(.a(new_n53_), .b(x18), .O(new_n84_));
  oai22  g038(.a(new_n84_), .b(new_n83_), .c(x18), .d(new_n47_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g040(.a(x19), .b(x17), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  inv1   g042(.a(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n48_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n86_), .O(z09));
  inv1   g046(.a(x09), .O(new_n93_));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n93_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n82_), .O(new_n96_));
  inv1   g050(.a(x17), .O(new_n97_));
  inv1   g051(.a(x19), .O(new_n98_));
  nand3  g052(.a(new_n53_), .b(new_n98_), .c(new_n97_), .O(new_n99_));
  oai21  g053(.a(new_n88_), .b(new_n53_), .c(new_n99_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(x16), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n96_), .c(new_n48_), .O(z10));
  inv1   g056(.a(x02), .O(new_n103_));
  oai22  g057(.a(new_n84_), .b(new_n103_), .c(x18), .d(new_n56_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  nand2  g059(.a(new_n89_), .b(x21), .O(new_n106_));
  inv1   g060(.a(x21), .O(new_n107_));
  nand3  g061(.a(new_n107_), .b(new_n98_), .c(new_n97_), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n106_), .c(x20), .O(new_n109_));
  nor3   g063(.a(new_n107_), .b(new_n53_), .c(x18), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n109_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n105_), .O(z11));
  inv1   g066(.a(x03), .O(new_n113_));
  oai22  g067(.a(new_n84_), .b(new_n113_), .c(x18), .d(new_n59_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n82_), .O(new_n115_));
  nor3   g069(.a(x21), .b(x19), .c(x17), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n88_), .O(new_n118_));
  oai21  g072(.a(new_n116_), .b(new_n61_), .c(new_n118_), .O(new_n119_));
  nor3   g073(.a(new_n61_), .b(new_n53_), .c(x18), .O(new_n120_));
  aoi21  g074(.a(new_n119_), .b(new_n53_), .c(new_n120_), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(new_n82_), .c(new_n115_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n65_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n82_), .O(new_n125_));
  inv1   g079(.a(x23), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n82_), .c(new_n52_), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(x20), .O(new_n128_));
  aoi21  g082(.a(new_n117_), .b(new_n88_), .c(new_n126_), .O(new_n129_));
  nor2   g083(.a(x23), .b(x22), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n107_), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n99_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n129_), .c(x16), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n128_), .c(new_n125_), .O(z13));
  inv1   g088(.a(x05), .O(new_n135_));
  oai22  g089(.a(new_n84_), .b(new_n135_), .c(x18), .d(new_n68_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n82_), .O(new_n137_));
  nand3  g091(.a(new_n130_), .b(new_n88_), .c(new_n107_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x24), .O(new_n139_));
  nor2   g093(.a(x24), .b(x23), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n116_), .c(new_n61_), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(new_n139_), .c(x20), .O(new_n142_));
  nor3   g096(.a(new_n70_), .b(new_n53_), .c(x18), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n142_), .c(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n137_), .O(z14));
  inv1   g099(.a(x06), .O(new_n146_));
  oai22  g100(.a(new_n84_), .b(new_n146_), .c(x18), .d(new_n73_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n82_), .O(new_n148_));
  nand3  g102(.a(new_n70_), .b(new_n126_), .c(new_n61_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n108_), .c(x25), .O(new_n150_));
  nor2   g104(.a(x21), .b(x19), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n130_), .c(new_n151_), .d(new_n97_), .O(new_n153_));
  aoi21  g107(.a(new_n153_), .b(new_n150_), .c(x20), .O(new_n154_));
  nor3   g108(.a(new_n75_), .b(new_n53_), .c(x18), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(new_n154_), .c(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n148_), .O(z15));
  inv1   g111(.a(x07), .O(new_n158_));
  oai22  g112(.a(new_n84_), .b(new_n158_), .c(x18), .d(new_n78_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n82_), .O(new_n160_));
  nand2  g114(.a(new_n153_), .b(x26), .O(new_n161_));
  nor2   g115(.a(x26), .b(x25), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n140_), .c(new_n117_), .d(new_n88_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n161_), .c(x20), .O(new_n164_));
  nand2  g118(.a(x26), .b(x20), .O(new_n165_));
  nor2   g119(.a(new_n165_), .b(x18), .O(new_n166_));
  oai21  g120(.a(new_n166_), .b(new_n164_), .c(x16), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(new_n160_), .O(z16));
  nand4  g122(.a(new_n107_), .b(new_n53_), .c(x19), .d(new_n97_), .O(new_n169_));
  nand3  g123(.a(new_n162_), .b(new_n130_), .c(new_n70_), .O(new_n170_));
  oai22  g124(.a(new_n170_), .b(new_n169_), .c(new_n62_), .d(new_n97_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n48_), .O(z17));
endmodule


