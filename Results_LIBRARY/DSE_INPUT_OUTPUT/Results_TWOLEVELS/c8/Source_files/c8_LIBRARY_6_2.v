// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x20), .O(new_n47_));
  nand2  g001(.a(new_n47_), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x19), .O(new_n52_));
  inv1   g006(.a(x27), .O(new_n53_));
  nand2  g007(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g008(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(z00));
  nand2  g009(.a(new_n48_), .b(x27), .O(new_n56_));
  inv1   g010(.a(x18), .O(new_n57_));
  nand3  g011(.a(new_n53_), .b(new_n47_), .c(new_n57_), .O(new_n58_));
  oai21  g012(.a(new_n56_), .b(x09), .c(new_n58_), .O(z01));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n53_), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x10), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z02));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n53_), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x11), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n66_), .c(new_n48_), .O(z03));
  inv1   g023(.a(x12), .O(new_n70_));
  nand2  g024(.a(x27), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x23), .O(new_n72_));
  nand2  g026(.a(new_n53_), .b(new_n72_), .O(new_n73_));
  aoi21  g027(.a(new_n73_), .b(new_n71_), .c(new_n49_), .O(z04));
  inv1   g028(.a(x13), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x24), .O(new_n77_));
  nand2  g031(.a(new_n53_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z05));
  inv1   g033(.a(x25), .O(new_n80_));
  nand2  g034(.a(new_n53_), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x14), .O(new_n82_));
  nand2  g036(.a(x27), .b(new_n82_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n81_), .c(new_n48_), .O(z06));
  inv1   g038(.a(x15), .O(new_n85_));
  nand2  g039(.a(x27), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x26), .O(new_n87_));
  nand2  g041(.a(new_n53_), .b(new_n87_), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n49_), .O(z07));
  inv1   g043(.a(new_n56_), .O(z08));
  inv1   g044(.a(x17), .O(new_n91_));
  nor2   g045(.a(new_n52_), .b(new_n91_), .O(new_n92_));
  nor2   g046(.a(x19), .b(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  inv1   g048(.a(x16), .O(new_n95_));
  nand2  g049(.a(x18), .b(x00), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n50_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n94_), .c(new_n48_), .O(z09));
  nor2   g053(.a(new_n47_), .b(new_n57_), .O(new_n100_));
  aoi22  g054(.a(new_n100_), .b(x01), .c(new_n57_), .d(x09), .O(new_n101_));
  nor2   g055(.a(x18), .b(x17), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n47_), .c(new_n52_), .O(new_n103_));
  oai21  g057(.a(new_n93_), .b(new_n47_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(x16), .O(new_n105_));
  oai21  g059(.a(new_n101_), .b(x16), .c(new_n105_), .O(z10));
  nand2  g060(.a(x18), .b(x02), .O(new_n107_));
  oai21  g061(.a(x18), .b(new_n62_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n95_), .O(new_n109_));
  nand4  g063(.a(new_n60_), .b(new_n52_), .c(new_n91_), .d(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n57_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n47_), .O(new_n112_));
  nor3   g066(.a(x20), .b(x19), .c(x17), .O(new_n113_));
  nor2   g067(.a(new_n113_), .b(new_n60_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n112_), .c(new_n109_), .O(z11));
  nand2  g070(.a(x18), .b(x03), .O(new_n117_));
  oai21  g071(.a(x18), .b(new_n67_), .c(new_n117_), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n95_), .O(new_n119_));
  nand2  g073(.a(new_n91_), .b(x16), .O(new_n120_));
  nor2   g074(.a(x22), .b(x21), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n52_), .O(new_n122_));
  oai21  g076(.a(new_n122_), .b(new_n120_), .c(new_n57_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n47_), .O(new_n124_));
  nand3  g078(.a(new_n93_), .b(new_n60_), .c(new_n47_), .O(new_n125_));
  nand3  g079(.a(new_n125_), .b(x22), .c(x16), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n124_), .c(new_n119_), .O(z12));
  nand2  g081(.a(new_n100_), .b(x04), .O(new_n128_));
  oai21  g082(.a(x18), .b(new_n70_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n95_), .O(new_n130_));
  nand2  g084(.a(new_n121_), .b(new_n93_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x23), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n113_), .c(new_n60_), .O(new_n134_));
  aoi21  g088(.a(new_n134_), .b(new_n132_), .c(x18), .O(new_n135_));
  nor2   g089(.a(new_n72_), .b(new_n47_), .O(new_n136_));
  oai21  g090(.a(new_n136_), .b(new_n135_), .c(x16), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n130_), .O(z13));
  nand2  g092(.a(new_n100_), .b(x05), .O(new_n139_));
  oai21  g093(.a(x18), .b(new_n75_), .c(new_n139_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n95_), .O(new_n141_));
  nand3  g095(.a(new_n133_), .b(new_n93_), .c(new_n60_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x24), .O(new_n143_));
  nor2   g097(.a(x24), .b(x23), .O(new_n144_));
  nand3  g098(.a(new_n144_), .b(new_n121_), .c(new_n113_), .O(new_n145_));
  aoi21  g099(.a(new_n145_), .b(new_n143_), .c(x18), .O(new_n146_));
  nor2   g100(.a(new_n77_), .b(new_n47_), .O(new_n147_));
  oai21  g101(.a(new_n147_), .b(new_n146_), .c(x16), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n141_), .O(z14));
  nand2  g103(.a(x18), .b(x06), .O(new_n150_));
  oai21  g104(.a(x18), .b(new_n82_), .c(new_n150_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n95_), .O(new_n152_));
  nand3  g106(.a(new_n133_), .b(new_n80_), .c(new_n77_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n110_), .c(new_n57_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n47_), .O(new_n155_));
  nand3  g109(.a(new_n145_), .b(x25), .c(x16), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n155_), .c(new_n152_), .O(z15));
  nand2  g111(.a(x18), .b(x07), .O(new_n158_));
  oai21  g112(.a(x18), .b(new_n85_), .c(new_n158_), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n95_), .O(new_n160_));
  nor3   g114(.a(x26), .b(x25), .c(x24), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n133_), .O(new_n162_));
  oai21  g116(.a(new_n162_), .b(new_n110_), .c(new_n57_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n47_), .O(new_n164_));
  or2    g118(.a(new_n153_), .b(new_n125_), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(x26), .c(x16), .O(new_n166_));
  nand3  g120(.a(new_n166_), .b(new_n164_), .c(new_n160_), .O(z16));
  nand2  g121(.a(z08), .b(x17), .O(new_n168_));
  nand3  g122(.a(new_n60_), .b(new_n47_), .c(x19), .O(new_n169_));
  inv1   g123(.a(new_n169_), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n161_), .c(new_n133_), .d(new_n102_), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n168_), .c(new_n95_), .O(z17));
endmodule


