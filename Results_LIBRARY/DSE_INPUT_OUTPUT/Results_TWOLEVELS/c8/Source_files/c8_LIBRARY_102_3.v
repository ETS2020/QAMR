// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x20), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n49_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x23), .c(new_n62_), .O(z04));
  inv1   g017(.a(x13), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n49_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x24), .c(new_n65_), .O(z05));
  inv1   g020(.a(x14), .O(new_n67_));
  aoi21  g021(.a(x27), .b(new_n67_), .c(new_n49_), .O(new_n68_));
  oai21  g022(.a(x27), .b(x25), .c(new_n68_), .O(z06));
  inv1   g023(.a(x15), .O(new_n70_));
  aoi21  g024(.a(x27), .b(new_n70_), .c(new_n49_), .O(new_n71_));
  oai21  g025(.a(x27), .b(x26), .c(new_n71_), .O(z07));
  and2   g026(.a(new_n48_), .b(x27), .O(z08));
  inv1   g027(.a(x16), .O(new_n74_));
  inv1   g028(.a(x00), .O(new_n75_));
  inv1   g029(.a(x20), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(x18), .O(new_n77_));
  oai22  g031(.a(new_n77_), .b(new_n75_), .c(x18), .d(new_n47_), .O(new_n78_));
  nand2  g032(.a(new_n78_), .b(new_n74_), .O(new_n79_));
  nand2  g033(.a(x19), .b(x17), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  inv1   g035(.a(new_n81_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n48_), .c(x16), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n79_), .O(z09));
  inv1   g039(.a(x01), .O(new_n86_));
  oai22  g040(.a(new_n77_), .b(new_n86_), .c(x18), .d(new_n52_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  nor3   g042(.a(new_n81_), .b(new_n76_), .c(x18), .O(new_n89_));
  nor3   g043(.a(x20), .b(x19), .c(x17), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(z10));
  nand2  g046(.a(x18), .b(x02), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n55_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n74_), .O(new_n95_));
  inv1   g049(.a(x18), .O(new_n96_));
  inv1   g050(.a(x21), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n74_), .c(new_n96_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x20), .O(new_n99_));
  nor2   g053(.a(new_n81_), .b(new_n97_), .O(new_n100_));
  nor2   g054(.a(x21), .b(x20), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  inv1   g056(.a(new_n102_), .O(new_n103_));
  oai21  g057(.a(new_n103_), .b(new_n100_), .c(x16), .O(new_n104_));
  nand3  g058(.a(new_n104_), .b(new_n99_), .c(new_n95_), .O(z11));
  inv1   g059(.a(x03), .O(new_n106_));
  oai22  g060(.a(new_n77_), .b(new_n106_), .c(x18), .d(new_n58_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n74_), .O(new_n108_));
  inv1   g062(.a(x22), .O(new_n109_));
  nor3   g063(.a(x21), .b(x19), .c(x17), .O(new_n110_));
  nand3  g064(.a(new_n81_), .b(new_n109_), .c(new_n97_), .O(new_n111_));
  oai21  g065(.a(new_n110_), .b(new_n109_), .c(new_n111_), .O(new_n112_));
  nor3   g066(.a(new_n109_), .b(new_n76_), .c(x18), .O(new_n113_));
  aoi21  g067(.a(new_n112_), .b(new_n76_), .c(new_n113_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n74_), .c(new_n108_), .O(z12));
  inv1   g069(.a(x04), .O(new_n116_));
  oai22  g070(.a(new_n77_), .b(new_n116_), .c(x18), .d(new_n61_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n74_), .O(new_n118_));
  nand2  g072(.a(new_n111_), .b(x23), .O(new_n119_));
  nor2   g073(.a(x23), .b(x22), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n81_), .c(new_n97_), .O(new_n121_));
  aoi21  g075(.a(new_n121_), .b(new_n119_), .c(x20), .O(new_n122_));
  inv1   g076(.a(x23), .O(new_n123_));
  nor3   g077(.a(new_n123_), .b(new_n76_), .c(x18), .O(new_n124_));
  oai21  g078(.a(new_n124_), .b(new_n122_), .c(x16), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n118_), .O(z13));
  nand2  g080(.a(x18), .b(x05), .O(new_n127_));
  oai21  g081(.a(x18), .b(new_n64_), .c(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n74_), .O(new_n129_));
  inv1   g083(.a(x24), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n74_), .c(new_n96_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(x20), .O(new_n132_));
  nand2  g086(.a(new_n121_), .b(x24), .O(new_n133_));
  nor2   g087(.a(x24), .b(x23), .O(new_n134_));
  nand4  g088(.a(new_n134_), .b(new_n90_), .c(new_n109_), .d(new_n97_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(x16), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n132_), .c(new_n129_), .O(z14));
  inv1   g092(.a(x06), .O(new_n139_));
  oai22  g093(.a(new_n77_), .b(new_n139_), .c(x18), .d(new_n67_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n74_), .O(new_n141_));
  inv1   g095(.a(x17), .O(new_n142_));
  inv1   g096(.a(x19), .O(new_n143_));
  nand3  g097(.a(new_n97_), .b(new_n143_), .c(new_n142_), .O(new_n144_));
  nand3  g098(.a(new_n130_), .b(new_n123_), .c(new_n109_), .O(new_n145_));
  oai21  g099(.a(new_n145_), .b(new_n144_), .c(x25), .O(new_n146_));
  nor2   g100(.a(x25), .b(x24), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n120_), .c(new_n110_), .O(new_n148_));
  aoi21  g102(.a(new_n148_), .b(new_n146_), .c(x20), .O(new_n149_));
  nand2  g103(.a(x25), .b(x20), .O(new_n150_));
  nor2   g104(.a(new_n150_), .b(x18), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n149_), .c(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n141_), .O(z15));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  oai21  g108(.a(x18), .b(new_n70_), .c(new_n154_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n74_), .O(new_n156_));
  nand2  g110(.a(x26), .b(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n96_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(x20), .O(new_n159_));
  nand2  g113(.a(new_n148_), .b(x26), .O(new_n160_));
  nor3   g114(.a(x26), .b(x25), .c(x24), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n120_), .c(new_n103_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n159_), .c(new_n156_), .O(z16));
  nand2  g119(.a(z08), .b(x17), .O(new_n166_));
  nor2   g120(.a(new_n143_), .b(x17), .O(new_n167_));
  nand4  g121(.a(new_n167_), .b(new_n161_), .c(new_n120_), .d(new_n101_), .O(new_n168_));
  aoi21  g122(.a(new_n168_), .b(new_n166_), .c(new_n74_), .O(z17));
endmodule


