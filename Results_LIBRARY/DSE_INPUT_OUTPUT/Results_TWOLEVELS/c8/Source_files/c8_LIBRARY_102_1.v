// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n173_, new_n174_, new_n175_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  nand2  g006(.a(x20), .b(new_n52_), .O(new_n53_));
  nand3  g007(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g008(.a(x20), .O(new_n55_));
  nand2  g009(.a(new_n48_), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  nand3  g012(.a(new_n58_), .b(new_n56_), .c(new_n53_), .O(z01));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n48_), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x10), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  nand3  g017(.a(new_n63_), .b(new_n61_), .c(new_n53_), .O(z02));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n48_), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x11), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  nand3  g022(.a(new_n68_), .b(new_n66_), .c(new_n53_), .O(z03));
  inv1   g023(.a(x23), .O(new_n70_));
  nand2  g024(.a(new_n48_), .b(new_n70_), .O(new_n71_));
  inv1   g025(.a(x12), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  nand3  g027(.a(new_n73_), .b(new_n71_), .c(new_n53_), .O(z04));
  inv1   g028(.a(x24), .O(new_n75_));
  nand2  g029(.a(new_n48_), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x13), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  nand3  g032(.a(new_n78_), .b(new_n76_), .c(new_n53_), .O(z05));
  inv1   g033(.a(x25), .O(new_n80_));
  nand2  g034(.a(new_n48_), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x14), .O(new_n82_));
  nand2  g036(.a(x27), .b(new_n82_), .O(new_n83_));
  nand3  g037(.a(new_n83_), .b(new_n81_), .c(new_n53_), .O(z06));
  inv1   g038(.a(x26), .O(new_n85_));
  nand2  g039(.a(new_n48_), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x15), .O(new_n87_));
  nand2  g041(.a(x27), .b(new_n87_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n86_), .c(new_n53_), .O(z07));
  aoi21  g043(.a(x20), .b(new_n52_), .c(new_n48_), .O(z08));
  nor2   g044(.a(x20), .b(x18), .O(new_n91_));
  aoi22  g045(.a(new_n91_), .b(x08), .c(x18), .d(x00), .O(new_n92_));
  nand2  g046(.a(x19), .b(x17), .O(new_n93_));
  inv1   g047(.a(x17), .O(new_n94_));
  nand2  g048(.a(new_n47_), .b(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n53_), .c(x16), .O(new_n97_));
  oai21  g051(.a(new_n92_), .b(x16), .c(new_n97_), .O(z09));
  aoi22  g052(.a(new_n91_), .b(x09), .c(x18), .d(x01), .O(new_n99_));
  nor2   g053(.a(x19), .b(x17), .O(new_n100_));
  nor3   g054(.a(new_n100_), .b(new_n55_), .c(new_n52_), .O(new_n101_));
  nor3   g055(.a(x20), .b(x19), .c(x17), .O(new_n102_));
  oai21  g056(.a(new_n102_), .b(new_n101_), .c(x16), .O(new_n103_));
  oai21  g057(.a(new_n99_), .b(x16), .c(new_n103_), .O(z10));
  inv1   g058(.a(x16), .O(new_n105_));
  nand2  g059(.a(x18), .b(x02), .O(new_n106_));
  oai21  g060(.a(x18), .b(new_n62_), .c(new_n106_), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  oai21  g062(.a(new_n60_), .b(new_n105_), .c(x18), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(x20), .O(new_n110_));
  nor2   g064(.a(new_n100_), .b(new_n60_), .O(new_n111_));
  nor2   g065(.a(x21), .b(x20), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n100_), .O(new_n113_));
  inv1   g067(.a(new_n113_), .O(new_n114_));
  oai21  g068(.a(new_n114_), .b(new_n111_), .c(x16), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n110_), .c(new_n108_), .O(z11));
  inv1   g070(.a(new_n91_), .O(new_n117_));
  nand2  g071(.a(x18), .b(x03), .O(new_n118_));
  oai21  g072(.a(new_n117_), .b(new_n67_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n105_), .O(new_n120_));
  nor3   g074(.a(x21), .b(x19), .c(x17), .O(new_n121_));
  nand3  g075(.a(new_n100_), .b(new_n65_), .c(new_n60_), .O(new_n122_));
  oai21  g076(.a(new_n121_), .b(new_n65_), .c(new_n122_), .O(new_n123_));
  nor2   g077(.a(new_n65_), .b(new_n55_), .O(new_n124_));
  aoi22  g078(.a(new_n124_), .b(x18), .c(new_n123_), .d(new_n55_), .O(new_n125_));
  oai21  g079(.a(new_n125_), .b(new_n105_), .c(new_n120_), .O(z12));
  nand2  g080(.a(x18), .b(x04), .O(new_n127_));
  oai21  g081(.a(new_n117_), .b(new_n72_), .c(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n105_), .O(new_n129_));
  nand2  g083(.a(new_n122_), .b(x23), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n100_), .c(new_n60_), .O(new_n132_));
  aoi21  g086(.a(new_n132_), .b(new_n130_), .c(x20), .O(new_n133_));
  nand3  g087(.a(x23), .b(x20), .c(x18), .O(new_n134_));
  inv1   g088(.a(new_n134_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n129_), .O(z13));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n77_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n105_), .O(new_n140_));
  oai21  g094(.a(new_n75_), .b(new_n105_), .c(x18), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x20), .O(new_n142_));
  nand2  g096(.a(new_n132_), .b(x24), .O(new_n143_));
  nor2   g097(.a(x24), .b(x23), .O(new_n144_));
  nand4  g098(.a(new_n144_), .b(new_n102_), .c(new_n65_), .d(new_n60_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n143_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(x16), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n142_), .c(new_n140_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(new_n117_), .b(new_n82_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n105_), .O(new_n151_));
  nand3  g105(.a(new_n60_), .b(new_n47_), .c(new_n94_), .O(new_n152_));
  nand3  g106(.a(new_n75_), .b(new_n70_), .c(new_n65_), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n152_), .c(x25), .O(new_n154_));
  nor2   g108(.a(x25), .b(x24), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n131_), .c(new_n121_), .O(new_n156_));
  aoi21  g110(.a(new_n156_), .b(new_n154_), .c(x20), .O(new_n157_));
  nand3  g111(.a(x25), .b(x20), .c(x18), .O(new_n158_));
  inv1   g112(.a(new_n158_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(new_n151_), .O(z15));
  nand2  g115(.a(x18), .b(x07), .O(new_n162_));
  oai21  g116(.a(x18), .b(new_n87_), .c(new_n162_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(new_n105_), .O(new_n164_));
  oai21  g118(.a(new_n85_), .b(new_n105_), .c(x18), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x20), .O(new_n166_));
  nand2  g120(.a(new_n156_), .b(x26), .O(new_n167_));
  nor3   g121(.a(x26), .b(x25), .c(x24), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n131_), .c(new_n114_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(x16), .O(new_n171_));
  nand3  g125(.a(new_n171_), .b(new_n166_), .c(new_n164_), .O(z16));
  nand2  g126(.a(z08), .b(x17), .O(new_n173_));
  nor2   g127(.a(new_n47_), .b(x17), .O(new_n174_));
  nand4  g128(.a(new_n174_), .b(new_n168_), .c(new_n131_), .d(new_n112_), .O(new_n175_));
  aoi21  g129(.a(new_n175_), .b(new_n173_), .c(new_n105_), .O(z17));
endmodule


