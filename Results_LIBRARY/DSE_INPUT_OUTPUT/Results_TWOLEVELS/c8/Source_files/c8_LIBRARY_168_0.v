// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n164_, new_n165_,
    new_n166_;
  nor2   g000(.a(x18), .b(x13), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x20), .O(new_n56_));
  nand2  g010(.a(new_n51_), .b(new_n56_), .O(new_n57_));
  aoi21  g011(.a(new_n57_), .b(new_n55_), .c(new_n47_), .O(z01));
  inv1   g012(.a(x10), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n47_), .O(new_n60_));
  oai21  g014(.a(x27), .b(x21), .c(new_n60_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n47_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x22), .c(new_n63_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  nand2  g019(.a(x27), .b(new_n65_), .O(new_n66_));
  inv1   g020(.a(x23), .O(new_n67_));
  nand2  g021(.a(new_n51_), .b(new_n67_), .O(new_n68_));
  aoi21  g022(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(z04));
  inv1   g023(.a(x13), .O(new_n70_));
  inv1   g024(.a(x18), .O(new_n71_));
  oai21  g025(.a(x27), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x24), .c(new_n72_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  nand2  g028(.a(x27), .b(new_n74_), .O(new_n75_));
  inv1   g029(.a(x25), .O(new_n76_));
  nand2  g030(.a(new_n51_), .b(new_n76_), .O(new_n77_));
  aoi21  g031(.a(new_n77_), .b(new_n75_), .c(new_n47_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x26), .O(new_n81_));
  nand2  g035(.a(new_n51_), .b(new_n81_), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n80_), .c(new_n47_), .O(z07));
  inv1   g037(.a(new_n47_), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n51_), .O(z08));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  nand2  g041(.a(new_n71_), .b(x13), .O(new_n88_));
  oai21  g042(.a(new_n88_), .b(new_n48_), .c(new_n87_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand2  g044(.a(x19), .b(x17), .O(new_n91_));
  nor2   g045(.a(x19), .b(x17), .O(new_n92_));
  inv1   g046(.a(new_n92_), .O(new_n93_));
  nand2  g047(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand3  g048(.a(new_n94_), .b(new_n84_), .c(x16), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n90_), .O(z09));
  nand2  g050(.a(x18), .b(x01), .O(new_n97_));
  oai21  g051(.a(x18), .b(new_n54_), .c(new_n97_), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n86_), .O(new_n99_));
  nor2   g053(.a(new_n92_), .b(new_n56_), .O(new_n100_));
  nor3   g054(.a(x20), .b(x19), .c(x17), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n100_), .c(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n99_), .c(new_n84_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(new_n88_), .b(new_n59_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n86_), .O(new_n106_));
  inv1   g060(.a(x21), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n92_), .O(new_n109_));
  oai21  g063(.a(new_n101_), .b(new_n107_), .c(new_n109_), .O(new_n110_));
  nand3  g064(.a(new_n110_), .b(new_n84_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n106_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n62_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n86_), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  aoi21  g070(.a(new_n108_), .b(new_n92_), .c(new_n116_), .O(new_n117_));
  nor2   g071(.a(x22), .b(x21), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n56_), .O(new_n119_));
  nor2   g073(.a(new_n119_), .b(new_n93_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n117_), .c(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n115_), .c(new_n84_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(x18), .b(new_n65_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n86_), .O(new_n125_));
  nor2   g079(.a(x23), .b(x22), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n101_), .c(new_n107_), .O(new_n127_));
  oai21  g081(.a(new_n120_), .b(new_n67_), .c(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n125_), .c(new_n84_), .O(z13));
  nand2  g084(.a(x18), .b(x05), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n88_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n86_), .O(new_n133_));
  oai21  g087(.a(new_n71_), .b(new_n86_), .c(new_n88_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n127_), .c(x24), .O(new_n135_));
  inv1   g089(.a(x17), .O(new_n136_));
  aoi21  g090(.a(new_n71_), .b(new_n70_), .c(x24), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n67_), .c(new_n116_), .d(new_n107_), .O(new_n138_));
  nor2   g092(.a(new_n138_), .b(x20), .O(new_n139_));
  nand4  g093(.a(new_n139_), .b(new_n50_), .c(new_n136_), .d(x16), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n135_), .c(new_n133_), .O(z14));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  nand3  g096(.a(new_n71_), .b(x14), .c(x13), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n86_), .O(new_n145_));
  nor2   g099(.a(x24), .b(x23), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n118_), .c(new_n101_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(x25), .O(new_n148_));
  nor2   g102(.a(x25), .b(x24), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n126_), .c(new_n108_), .d(new_n92_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand3  g105(.a(new_n151_), .b(new_n84_), .c(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n145_), .O(z15));
  nand2  g107(.a(x18), .b(x07), .O(new_n154_));
  nand3  g108(.a(new_n71_), .b(x15), .c(x13), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n86_), .O(new_n157_));
  nand2  g111(.a(new_n150_), .b(x26), .O(new_n158_));
  inv1   g112(.a(x24), .O(new_n159_));
  nand4  g113(.a(new_n126_), .b(new_n81_), .c(new_n76_), .d(new_n159_), .O(new_n160_));
  oai21  g114(.a(new_n160_), .b(new_n109_), .c(new_n158_), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n84_), .c(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n157_), .O(z16));
  nand3  g117(.a(new_n108_), .b(x19), .c(new_n136_), .O(new_n164_));
  oai22  g118(.a(new_n164_), .b(new_n160_), .c(new_n51_), .d(new_n136_), .O(new_n165_));
  nand3  g119(.a(new_n165_), .b(new_n84_), .c(x16), .O(new_n166_));
  inv1   g120(.a(new_n166_), .O(z17));
endmodule


