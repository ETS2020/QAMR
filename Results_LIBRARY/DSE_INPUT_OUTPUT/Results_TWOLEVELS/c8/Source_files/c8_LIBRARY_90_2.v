// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n124_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n135_, new_n136_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n159_,
    new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x24), .O(new_n47_));
  nor2   g001(.a(x26), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n48_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x21), .c(new_n59_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n52_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  aoi21  g020(.a(x27), .b(new_n66_), .c(new_n48_), .O(new_n67_));
  oai21  g021(.a(x27), .b(x23), .c(new_n67_), .O(z04));
  aoi21  g022(.a(x27), .b(x26), .c(new_n47_), .O(new_n69_));
  oai22  g023(.a(new_n69_), .b(x13), .c(x27), .d(x24), .O(z05));
  inv1   g024(.a(x14), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n48_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x25), .c(new_n72_), .O(z06));
  inv1   g027(.a(x26), .O(new_n74_));
  oai21  g028(.a(new_n52_), .b(x24), .c(new_n74_), .O(new_n75_));
  oai21  g029(.a(new_n52_), .b(x15), .c(new_n75_), .O(z07));
  nor2   g030(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g031(.a(new_n48_), .O(new_n78_));
  inv1   g032(.a(x17), .O(new_n79_));
  nor2   g033(.a(new_n51_), .b(new_n79_), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g036(.a(x16), .O(new_n83_));
  nand2  g037(.a(x18), .b(x00), .O(new_n84_));
  oai21  g038(.a(x18), .b(new_n49_), .c(new_n84_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand3  g040(.a(new_n86_), .b(new_n82_), .c(new_n78_), .O(z09));
  nand2  g041(.a(x18), .b(x01), .O(new_n88_));
  oai21  g042(.a(x18), .b(new_n55_), .c(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n83_), .O(new_n90_));
  inv1   g044(.a(x20), .O(new_n91_));
  nor2   g045(.a(new_n81_), .b(new_n91_), .O(new_n92_));
  nand3  g046(.a(new_n91_), .b(new_n51_), .c(new_n79_), .O(new_n93_));
  inv1   g047(.a(new_n93_), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n92_), .c(x16), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n90_), .c(new_n78_), .O(z10));
  nor2   g050(.a(x18), .b(new_n58_), .O(new_n97_));
  aoi21  g051(.a(x18), .b(x02), .c(new_n97_), .O(new_n98_));
  inv1   g052(.a(x21), .O(new_n99_));
  nand3  g053(.a(new_n81_), .b(new_n99_), .c(new_n91_), .O(new_n100_));
  oai21  g054(.a(new_n94_), .b(new_n99_), .c(new_n100_), .O(new_n101_));
  aoi21  g055(.a(new_n101_), .b(x16), .c(new_n48_), .O(new_n102_));
  oai21  g056(.a(new_n98_), .b(x16), .c(new_n102_), .O(z11));
  nand2  g057(.a(x18), .b(x03), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n61_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n83_), .O(new_n106_));
  and2   g060(.a(new_n100_), .b(x22), .O(new_n107_));
  nor2   g061(.a(x22), .b(x21), .O(new_n108_));
  nand3  g062(.a(new_n108_), .b(new_n81_), .c(new_n91_), .O(new_n109_));
  inv1   g063(.a(new_n109_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n107_), .c(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n106_), .c(new_n78_), .O(z12));
  nor2   g066(.a(x18), .b(new_n66_), .O(new_n113_));
  aoi21  g067(.a(x18), .b(x04), .c(new_n113_), .O(new_n114_));
  nand3  g068(.a(new_n109_), .b(x23), .c(x16), .O(new_n115_));
  oai21  g069(.a(new_n114_), .b(x16), .c(new_n115_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n78_), .O(new_n117_));
  inv1   g071(.a(x23), .O(new_n118_));
  nand2  g072(.a(new_n74_), .b(x24), .O(new_n119_));
  nand4  g073(.a(new_n119_), .b(new_n118_), .c(new_n63_), .d(new_n99_), .O(new_n120_));
  nor2   g074(.a(new_n120_), .b(x20), .O(new_n121_));
  nand4  g075(.a(new_n121_), .b(new_n51_), .c(new_n79_), .d(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n117_), .O(z13));
  inv1   g077(.a(x13), .O(new_n124_));
  nand2  g078(.a(x18), .b(x05), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n124_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n83_), .O(new_n127_));
  nand2  g081(.a(new_n118_), .b(new_n63_), .O(new_n128_));
  nor3   g082(.a(new_n128_), .b(new_n93_), .c(x21), .O(new_n129_));
  nor2   g083(.a(new_n129_), .b(new_n47_), .O(new_n130_));
  nand3  g084(.a(new_n108_), .b(new_n47_), .c(new_n118_), .O(new_n131_));
  nor2   g085(.a(new_n131_), .b(new_n93_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n130_), .c(x16), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n127_), .c(new_n78_), .O(z14));
  nand2  g088(.a(x18), .b(x06), .O(new_n135_));
  oai21  g089(.a(x18), .b(new_n71_), .c(new_n135_), .O(new_n136_));
  nand2  g090(.a(new_n136_), .b(new_n83_), .O(new_n137_));
  inv1   g091(.a(x25), .O(new_n138_));
  oai21  g092(.a(new_n138_), .b(new_n83_), .c(x26), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x24), .O(new_n140_));
  nor2   g094(.a(new_n129_), .b(new_n138_), .O(new_n141_));
  nor4   g095(.a(new_n128_), .b(new_n100_), .c(x25), .d(x24), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n141_), .c(x16), .O(new_n143_));
  nand3  g097(.a(new_n143_), .b(new_n140_), .c(new_n137_), .O(z15));
  inv1   g098(.a(x15), .O(new_n145_));
  nand2  g099(.a(x18), .b(x07), .O(new_n146_));
  oai21  g100(.a(x18), .b(new_n145_), .c(new_n146_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n83_), .O(new_n148_));
  nand2  g102(.a(x26), .b(x24), .O(new_n149_));
  oai21  g103(.a(x26), .b(x25), .c(new_n149_), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n118_), .c(new_n63_), .d(new_n99_), .O(new_n151_));
  inv1   g105(.a(new_n151_), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n91_), .c(new_n51_), .d(new_n79_), .O(new_n153_));
  nand3  g107(.a(new_n108_), .b(new_n138_), .c(new_n118_), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n93_), .c(x26), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x16), .O(new_n157_));
  nand3  g111(.a(new_n157_), .b(new_n148_), .c(new_n78_), .O(z16));
  nand4  g112(.a(new_n99_), .b(new_n91_), .c(x19), .d(new_n79_), .O(new_n159_));
  nand4  g113(.a(new_n74_), .b(new_n138_), .c(new_n118_), .d(new_n63_), .O(new_n160_));
  oai22  g114(.a(new_n160_), .b(new_n159_), .c(new_n52_), .d(new_n79_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(x16), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n78_), .O(z17));
endmodule


