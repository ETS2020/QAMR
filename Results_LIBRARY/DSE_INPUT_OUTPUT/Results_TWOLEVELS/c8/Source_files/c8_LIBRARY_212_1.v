// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n163_, new_n164_, new_n165_;
  inv1   g000(.a(x19), .O(new_n47_));
  inv1   g001(.a(x27), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(x08), .O(new_n50_));
  nand2  g004(.a(x27), .b(new_n50_), .O(new_n51_));
  inv1   g005(.a(x13), .O(new_n52_));
  nand2  g006(.a(x18), .b(new_n52_), .O(new_n53_));
  nand3  g007(.a(new_n53_), .b(new_n51_), .c(new_n49_), .O(z00));
  inv1   g008(.a(new_n53_), .O(new_n55_));
  inv1   g009(.a(x09), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x20), .O(new_n58_));
  nand2  g012(.a(new_n48_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n55_), .O(z01));
  inv1   g014(.a(x10), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x21), .O(new_n63_));
  nand2  g017(.a(new_n48_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n55_), .O(z02));
  inv1   g019(.a(x11), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x22), .O(new_n68_));
  nand2  g022(.a(new_n48_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n55_), .O(z03));
  inv1   g024(.a(x12), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x23), .O(new_n73_));
  nand2  g027(.a(new_n48_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n55_), .O(z04));
  nor2   g029(.a(x27), .b(x24), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n52_), .c(new_n76_), .O(new_n77_));
  nand2  g031(.a(new_n76_), .b(x13), .O(new_n78_));
  oai21  g032(.a(new_n77_), .b(x18), .c(new_n78_), .O(z05));
  inv1   g033(.a(x14), .O(new_n80_));
  nand2  g034(.a(x27), .b(new_n80_), .O(new_n81_));
  inv1   g035(.a(x25), .O(new_n82_));
  nand2  g036(.a(new_n48_), .b(new_n82_), .O(new_n83_));
  aoi21  g037(.a(new_n83_), .b(new_n81_), .c(new_n55_), .O(z06));
  inv1   g038(.a(x15), .O(new_n85_));
  nand2  g039(.a(x27), .b(new_n85_), .O(new_n86_));
  inv1   g040(.a(x26), .O(new_n87_));
  nand2  g041(.a(new_n48_), .b(new_n87_), .O(new_n88_));
  aoi21  g042(.a(new_n88_), .b(new_n86_), .c(new_n55_), .O(z07));
  nand2  g043(.a(new_n53_), .b(new_n48_), .O(z08));
  inv1   g044(.a(x17), .O(new_n91_));
  nor2   g045(.a(new_n47_), .b(new_n91_), .O(new_n92_));
  nor2   g046(.a(x19), .b(x17), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n92_), .c(x16), .O(new_n94_));
  inv1   g048(.a(x16), .O(new_n95_));
  nand2  g049(.a(x18), .b(x00), .O(new_n96_));
  oai21  g050(.a(x18), .b(new_n50_), .c(new_n96_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand3  g052(.a(new_n98_), .b(new_n94_), .c(new_n53_), .O(z09));
  inv1   g053(.a(x01), .O(new_n100_));
  nand2  g054(.a(x18), .b(x13), .O(new_n101_));
  oai22  g055(.a(new_n101_), .b(new_n100_), .c(x18), .d(new_n56_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n95_), .O(new_n103_));
  nor3   g057(.a(x20), .b(x19), .c(x17), .O(new_n104_));
  inv1   g058(.a(new_n104_), .O(new_n105_));
  oai21  g059(.a(new_n93_), .b(new_n58_), .c(new_n105_), .O(new_n106_));
  nand3  g060(.a(new_n106_), .b(new_n53_), .c(x16), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n103_), .O(z10));
  nor2   g062(.a(x18), .b(new_n61_), .O(new_n109_));
  aoi21  g063(.a(x18), .b(x02), .c(new_n109_), .O(new_n110_));
  nor2   g064(.a(x21), .b(x20), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n93_), .O(new_n112_));
  oai21  g066(.a(new_n104_), .b(new_n63_), .c(new_n112_), .O(new_n113_));
  aoi21  g067(.a(new_n113_), .b(x16), .c(new_n55_), .O(new_n114_));
  oai21  g068(.a(new_n110_), .b(x16), .c(new_n114_), .O(z11));
  inv1   g069(.a(x03), .O(new_n116_));
  oai22  g070(.a(new_n101_), .b(new_n116_), .c(x18), .d(new_n66_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n95_), .O(new_n118_));
  nand2  g072(.a(new_n112_), .b(x22), .O(new_n119_));
  nand4  g073(.a(new_n93_), .b(new_n68_), .c(new_n63_), .d(new_n58_), .O(new_n120_));
  nand2  g074(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n53_), .c(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n118_), .O(z12));
  inv1   g077(.a(x04), .O(new_n124_));
  oai22  g078(.a(new_n101_), .b(new_n124_), .c(x18), .d(new_n71_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n95_), .O(new_n126_));
  nand2  g080(.a(new_n120_), .b(x23), .O(new_n127_));
  nor3   g081(.a(x23), .b(x22), .c(x21), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n104_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n53_), .c(x16), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n126_), .O(z13));
  inv1   g086(.a(x05), .O(new_n133_));
  nand2  g087(.a(x18), .b(new_n133_), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n95_), .c(x13), .O(new_n135_));
  inv1   g089(.a(x24), .O(new_n136_));
  aoi21  g090(.a(new_n128_), .b(new_n104_), .c(new_n136_), .O(new_n137_));
  nand4  g091(.a(new_n136_), .b(new_n73_), .c(new_n68_), .d(new_n63_), .O(new_n138_));
  nor2   g092(.a(new_n138_), .b(new_n105_), .O(new_n139_));
  oai21  g093(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  nand3  g094(.a(new_n140_), .b(new_n135_), .c(new_n53_), .O(z14));
  inv1   g095(.a(x06), .O(new_n142_));
  oai22  g096(.a(new_n101_), .b(new_n142_), .c(x18), .d(new_n80_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n95_), .O(new_n144_));
  aoi22  g098(.a(new_n128_), .b(new_n104_), .c(x18), .d(new_n52_), .O(new_n145_));
  nor2   g099(.a(new_n55_), .b(new_n136_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n145_), .c(x25), .O(new_n147_));
  nand4  g101(.a(new_n53_), .b(new_n82_), .c(new_n136_), .d(new_n73_), .O(new_n148_));
  nor2   g102(.a(new_n148_), .b(x22), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n63_), .c(new_n58_), .d(new_n47_), .O(new_n150_));
  oai21  g104(.a(new_n150_), .b(x17), .c(new_n147_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n144_), .O(z15));
  nor2   g107(.a(x18), .b(new_n85_), .O(new_n154_));
  aoi21  g108(.a(x18), .b(x07), .c(new_n154_), .O(new_n155_));
  nor2   g109(.a(x23), .b(x22), .O(new_n156_));
  nand3  g110(.a(new_n156_), .b(new_n82_), .c(new_n136_), .O(new_n157_));
  oai21  g111(.a(new_n157_), .b(new_n112_), .c(x26), .O(new_n158_));
  nand4  g112(.a(new_n156_), .b(new_n87_), .c(new_n82_), .d(new_n136_), .O(new_n159_));
  oai21  g113(.a(new_n159_), .b(new_n112_), .c(new_n158_), .O(new_n160_));
  aoi21  g114(.a(new_n160_), .b(x16), .c(new_n55_), .O(new_n161_));
  oai21  g115(.a(new_n155_), .b(x16), .c(new_n161_), .O(z16));
  nand3  g116(.a(new_n111_), .b(x19), .c(new_n91_), .O(new_n163_));
  oai22  g117(.a(new_n163_), .b(new_n159_), .c(new_n48_), .d(new_n91_), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n53_), .c(x16), .O(new_n165_));
  inv1   g119(.a(new_n165_), .O(z17));
endmodule


