// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_, new_n161_, new_n162_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x18), .b(x11), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n48_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  inv1   g011(.a(x18), .O(new_n58_));
  oai21  g012(.a(x27), .b(new_n58_), .c(new_n57_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x23), .O(new_n63_));
  inv1   g017(.a(x27), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n62_), .c(new_n48_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n48_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x25), .O(new_n74_));
  nand2  g028(.a(new_n64_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n48_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n77_), .c(new_n48_), .O(new_n78_));
  oai21  g032(.a(x27), .b(x26), .c(new_n78_), .O(z07));
  inv1   g033(.a(new_n48_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(new_n64_), .O(z08));
  inv1   g035(.a(x16), .O(new_n82_));
  nand2  g036(.a(x18), .b(x00), .O(new_n83_));
  nand3  g037(.a(new_n58_), .b(x11), .c(x08), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g039(.a(new_n85_), .b(new_n82_), .O(new_n86_));
  nand2  g040(.a(x19), .b(x17), .O(new_n87_));
  nor2   g041(.a(x19), .b(x17), .O(new_n88_));
  inv1   g042(.a(new_n88_), .O(new_n89_));
  nand2  g043(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand3  g044(.a(new_n90_), .b(new_n80_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n86_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n51_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n82_), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  nor2   g050(.a(new_n88_), .b(new_n96_), .O(new_n97_));
  inv1   g051(.a(x17), .O(new_n98_));
  inv1   g052(.a(x19), .O(new_n99_));
  nand3  g053(.a(new_n96_), .b(new_n99_), .c(new_n98_), .O(new_n100_));
  inv1   g054(.a(new_n100_), .O(new_n101_));
  oai21  g055(.a(new_n101_), .b(new_n97_), .c(x16), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n95_), .c(new_n80_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n54_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n82_), .O(new_n106_));
  inv1   g060(.a(x21), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n88_), .O(new_n109_));
  oai21  g063(.a(new_n101_), .b(new_n107_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n106_), .c(new_n80_), .O(z11));
  oai21  g066(.a(new_n58_), .b(x03), .c(new_n82_), .O(new_n113_));
  inv1   g067(.a(x22), .O(new_n114_));
  inv1   g068(.a(new_n109_), .O(new_n115_));
  nor2   g069(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand2  g070(.a(new_n114_), .b(new_n107_), .O(new_n117_));
  nor3   g071(.a(new_n117_), .b(new_n89_), .c(x20), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n116_), .c(x16), .O(new_n119_));
  nand3  g073(.a(new_n119_), .b(new_n113_), .c(new_n80_), .O(z12));
  nand2  g074(.a(x18), .b(x04), .O(new_n121_));
  oai21  g075(.a(x18), .b(new_n61_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n82_), .O(new_n123_));
  nor2   g077(.a(new_n118_), .b(new_n63_), .O(new_n124_));
  nor2   g078(.a(x23), .b(x22), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n107_), .O(new_n126_));
  nor2   g080(.a(new_n126_), .b(new_n100_), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n124_), .c(x16), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n123_), .c(new_n80_), .O(z13));
  nand2  g083(.a(x18), .b(x05), .O(new_n130_));
  oai21  g084(.a(x18), .b(new_n67_), .c(new_n130_), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n82_), .O(new_n132_));
  nor2   g086(.a(new_n127_), .b(new_n69_), .O(new_n133_));
  nor4   g087(.a(new_n117_), .b(new_n100_), .c(x24), .d(x23), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n133_), .c(x16), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n132_), .c(new_n80_), .O(z14));
  nand2  g090(.a(x18), .b(x06), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n72_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n82_), .O(new_n139_));
  nand3  g093(.a(new_n125_), .b(new_n74_), .c(new_n69_), .O(new_n140_));
  oai22  g094(.a(new_n140_), .b(new_n109_), .c(new_n134_), .d(new_n74_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x16), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n139_), .c(new_n80_), .O(z15));
  nand2  g097(.a(x18), .b(x07), .O(new_n144_));
  nand3  g098(.a(new_n58_), .b(x15), .c(x11), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n82_), .O(new_n147_));
  aoi21  g101(.a(new_n58_), .b(new_n57_), .c(new_n115_), .O(new_n148_));
  and2   g102(.a(new_n140_), .b(new_n80_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n148_), .c(x26), .O(new_n150_));
  aoi21  g104(.a(new_n58_), .b(new_n57_), .c(x26), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n74_), .c(new_n69_), .d(new_n63_), .O(new_n152_));
  nor2   g106(.a(new_n152_), .b(x22), .O(new_n153_));
  nand4  g107(.a(new_n153_), .b(new_n107_), .c(new_n96_), .d(new_n99_), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(x17), .c(new_n150_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x16), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n147_), .O(z16));
  nand3  g111(.a(new_n108_), .b(x19), .c(new_n98_), .O(new_n158_));
  nor2   g112(.a(x26), .b(x25), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n125_), .c(new_n69_), .O(new_n160_));
  oai22  g114(.a(new_n160_), .b(new_n158_), .c(new_n64_), .d(new_n98_), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n80_), .c(x16), .O(new_n162_));
  inv1   g116(.a(new_n162_), .O(z17));
endmodule


