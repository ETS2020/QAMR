// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n125_, new_n126_,
    new_n127_, new_n128_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n137_, new_n138_, new_n139_,
    new_n140_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_;
  nor2   g000(.a(x26), .b(x24), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  aoi21  g011(.a(x27), .b(new_n57_), .c(new_n47_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x21), .c(new_n58_), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x22), .O(new_n62_));
  nand2  g016(.a(new_n51_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n47_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nor2   g022(.a(new_n47_), .b(new_n51_), .O(z08));
  nand2  g023(.a(z08), .b(new_n68_), .O(new_n70_));
  inv1   g024(.a(x24), .O(new_n71_));
  nand3  g025(.a(new_n51_), .b(x26), .c(new_n71_), .O(new_n72_));
  nand2  g026(.a(new_n72_), .b(new_n70_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n47_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x26), .O(new_n77_));
  oai21  g031(.a(new_n51_), .b(new_n71_), .c(new_n77_), .O(new_n78_));
  oai21  g032(.a(new_n51_), .b(x15), .c(new_n78_), .O(z07));
  inv1   g033(.a(new_n47_), .O(new_n80_));
  inv1   g034(.a(x17), .O(new_n81_));
  nor2   g035(.a(new_n50_), .b(new_n81_), .O(new_n82_));
  nor2   g036(.a(x19), .b(x17), .O(new_n83_));
  oai21  g037(.a(new_n83_), .b(new_n82_), .c(x16), .O(new_n84_));
  inv1   g038(.a(x16), .O(new_n85_));
  nand2  g039(.a(x18), .b(x00), .O(new_n86_));
  oai21  g040(.a(x18), .b(new_n48_), .c(new_n86_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand3  g042(.a(new_n88_), .b(new_n84_), .c(new_n80_), .O(z09));
  nand2  g043(.a(x18), .b(x01), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n54_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n85_), .O(new_n92_));
  inv1   g046(.a(x20), .O(new_n93_));
  nor2   g047(.a(new_n83_), .b(new_n93_), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n80_), .O(z10));
  nor2   g051(.a(x18), .b(new_n57_), .O(new_n98_));
  aoi21  g052(.a(x18), .b(x02), .c(new_n98_), .O(new_n99_));
  inv1   g053(.a(x21), .O(new_n100_));
  nand3  g054(.a(new_n83_), .b(new_n100_), .c(new_n93_), .O(new_n101_));
  oai21  g055(.a(new_n95_), .b(new_n100_), .c(new_n101_), .O(new_n102_));
  aoi21  g056(.a(new_n102_), .b(x16), .c(new_n47_), .O(new_n103_));
  oai21  g057(.a(new_n99_), .b(x16), .c(new_n103_), .O(z11));
  nand2  g058(.a(x18), .b(x03), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n60_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n85_), .O(new_n107_));
  and2   g061(.a(new_n101_), .b(x22), .O(new_n108_));
  nor2   g062(.a(x22), .b(x21), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n83_), .c(new_n93_), .O(new_n110_));
  inv1   g064(.a(new_n110_), .O(new_n111_));
  oai21  g065(.a(new_n111_), .b(new_n108_), .c(x16), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n107_), .c(new_n80_), .O(z12));
  nor2   g067(.a(x18), .b(new_n65_), .O(new_n114_));
  aoi21  g068(.a(x18), .b(x04), .c(new_n114_), .O(new_n115_));
  nand3  g069(.a(new_n110_), .b(x23), .c(x16), .O(new_n116_));
  oai21  g070(.a(new_n115_), .b(x16), .c(new_n116_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n80_), .O(new_n118_));
  inv1   g072(.a(x23), .O(new_n119_));
  nand2  g073(.a(new_n77_), .b(new_n71_), .O(new_n120_));
  nand4  g074(.a(new_n120_), .b(new_n119_), .c(new_n62_), .d(new_n100_), .O(new_n121_));
  nor2   g075(.a(new_n121_), .b(x20), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n50_), .c(new_n81_), .d(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n118_), .O(z13));
  nand2  g078(.a(x18), .b(x05), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n68_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n85_), .O(new_n127_));
  nand4  g081(.a(x26), .b(new_n71_), .c(new_n119_), .d(new_n62_), .O(new_n128_));
  nor2   g082(.a(new_n128_), .b(x21), .O(new_n129_));
  nand4  g083(.a(new_n129_), .b(new_n93_), .c(new_n50_), .d(new_n81_), .O(new_n130_));
  inv1   g084(.a(new_n95_), .O(new_n131_));
  nand3  g085(.a(new_n119_), .b(new_n62_), .c(new_n100_), .O(new_n132_));
  oai21  g086(.a(new_n132_), .b(new_n131_), .c(x24), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n130_), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(x16), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n127_), .c(new_n80_), .O(z14));
  nand2  g090(.a(x18), .b(x06), .O(new_n137_));
  oai21  g091(.a(x18), .b(new_n74_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n85_), .O(new_n139_));
  nand4  g093(.a(new_n93_), .b(new_n50_), .c(new_n81_), .d(x16), .O(new_n140_));
  inv1   g094(.a(x25), .O(new_n141_));
  nand4  g095(.a(new_n109_), .b(x26), .c(new_n141_), .d(new_n119_), .O(new_n142_));
  oai21  g096(.a(new_n142_), .b(new_n140_), .c(x26), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n71_), .O(new_n144_));
  nand4  g098(.a(new_n109_), .b(new_n95_), .c(new_n71_), .d(new_n119_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(x25), .c(x16), .O(new_n146_));
  nand3  g100(.a(new_n146_), .b(new_n144_), .c(new_n139_), .O(z15));
  inv1   g101(.a(x15), .O(new_n148_));
  nand2  g102(.a(x18), .b(x07), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n148_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n85_), .O(new_n151_));
  nand4  g105(.a(new_n109_), .b(x26), .c(x25), .d(new_n119_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n140_), .c(x26), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n71_), .O(new_n154_));
  nand3  g108(.a(new_n145_), .b(x26), .c(x16), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n154_), .c(new_n151_), .O(z16));
  nand3  g110(.a(x27), .b(x17), .c(x16), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n80_), .O(z17));
endmodule


