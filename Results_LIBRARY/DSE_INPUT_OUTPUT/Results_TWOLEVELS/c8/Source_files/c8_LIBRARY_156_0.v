// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(new_n47_), .b(x08), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nor2   g006(.a(x19), .b(x18), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  aoi21  g010(.a(x27), .b(new_n56_), .c(new_n53_), .O(new_n57_));
  oai21  g011(.a(x27), .b(x21), .c(new_n57_), .O(z02));
  inv1   g012(.a(x11), .O(new_n59_));
  aoi21  g013(.a(x27), .b(new_n59_), .c(new_n53_), .O(new_n60_));
  oai21  g014(.a(x27), .b(x22), .c(new_n60_), .O(z03));
  inv1   g015(.a(x12), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n53_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x23), .c(new_n63_), .O(z04));
  inv1   g018(.a(x13), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n53_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x24), .c(new_n66_), .O(z05));
  inv1   g021(.a(x14), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n53_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x25), .c(new_n69_), .O(z06));
  inv1   g024(.a(x15), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x26), .O(new_n73_));
  nand2  g027(.a(new_n47_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n53_), .O(z07));
  inv1   g029(.a(new_n53_), .O(new_n76_));
  nand2  g030(.a(new_n76_), .b(new_n47_), .O(z08));
  inv1   g031(.a(x17), .O(new_n78_));
  nand3  g032(.a(new_n49_), .b(x18), .c(new_n78_), .O(new_n79_));
  oai21  g033(.a(new_n49_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  nand2  g034(.a(new_n80_), .b(x16), .O(new_n81_));
  inv1   g035(.a(x16), .O(new_n82_));
  inv1   g036(.a(x08), .O(new_n83_));
  nand2  g037(.a(x18), .b(x00), .O(new_n84_));
  nand2  g038(.a(x19), .b(new_n48_), .O(new_n85_));
  oai21  g039(.a(new_n85_), .b(new_n83_), .c(new_n84_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n82_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n81_), .O(z09));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  oai21  g043(.a(new_n85_), .b(new_n52_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n82_), .O(new_n91_));
  nand2  g045(.a(x20), .b(x17), .O(new_n92_));
  nor3   g046(.a(x20), .b(x19), .c(x17), .O(new_n93_));
  inv1   g047(.a(new_n93_), .O(new_n94_));
  aoi21  g048(.a(new_n94_), .b(new_n92_), .c(new_n48_), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  nor2   g050(.a(new_n96_), .b(new_n49_), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n95_), .c(x16), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(new_n91_), .O(z10));
  nand2  g053(.a(x18), .b(x02), .O(new_n100_));
  oai21  g054(.a(new_n85_), .b(new_n56_), .c(new_n100_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n82_), .O(new_n102_));
  oai21  g056(.a(x20), .b(x17), .c(x21), .O(new_n103_));
  nor2   g057(.a(x19), .b(x17), .O(new_n104_));
  nor2   g058(.a(x21), .b(x20), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g060(.a(new_n106_), .b(new_n103_), .c(new_n48_), .O(new_n107_));
  inv1   g061(.a(x21), .O(new_n108_));
  nor2   g062(.a(new_n108_), .b(new_n49_), .O(new_n109_));
  oai21  g063(.a(new_n109_), .b(new_n107_), .c(x16), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n102_), .O(z11));
  nand2  g065(.a(x18), .b(x03), .O(new_n112_));
  oai21  g066(.a(new_n85_), .b(new_n59_), .c(new_n112_), .O(new_n113_));
  nand2  g067(.a(new_n113_), .b(new_n82_), .O(new_n114_));
  nand3  g068(.a(new_n108_), .b(new_n96_), .c(new_n78_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(x22), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n104_), .c(new_n96_), .O(new_n118_));
  aoi21  g072(.a(new_n118_), .b(new_n116_), .c(new_n48_), .O(new_n119_));
  inv1   g073(.a(x22), .O(new_n120_));
  nor2   g074(.a(new_n120_), .b(new_n49_), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(new_n119_), .c(x16), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(new_n114_), .O(z12));
  nand2  g077(.a(x18), .b(x04), .O(new_n124_));
  oai21  g078(.a(new_n85_), .b(new_n62_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(new_n82_), .O(new_n126_));
  nor2   g080(.a(x20), .b(x17), .O(new_n127_));
  nand2  g081(.a(new_n117_), .b(new_n127_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x23), .O(new_n129_));
  nor2   g083(.a(x23), .b(x22), .O(new_n130_));
  nand3  g084(.a(new_n130_), .b(new_n93_), .c(new_n108_), .O(new_n131_));
  aoi21  g085(.a(new_n131_), .b(new_n129_), .c(new_n48_), .O(new_n132_));
  inv1   g086(.a(x23), .O(new_n133_));
  nor2   g087(.a(new_n133_), .b(new_n49_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n126_), .O(z13));
  nand2  g090(.a(x18), .b(x05), .O(new_n137_));
  oai21  g091(.a(new_n85_), .b(new_n65_), .c(new_n137_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n82_), .O(new_n139_));
  nand3  g093(.a(new_n130_), .b(new_n127_), .c(new_n108_), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(x24), .O(new_n141_));
  nor2   g095(.a(x24), .b(x23), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n117_), .c(new_n93_), .O(new_n143_));
  aoi21  g097(.a(new_n143_), .b(new_n141_), .c(new_n48_), .O(new_n144_));
  inv1   g098(.a(x24), .O(new_n145_));
  nor2   g099(.a(new_n145_), .b(new_n49_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n139_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(new_n85_), .b(new_n68_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n82_), .O(new_n151_));
  nand3  g105(.a(new_n145_), .b(new_n133_), .c(new_n120_), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n115_), .c(x25), .O(new_n153_));
  nor2   g107(.a(x25), .b(x24), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n130_), .c(new_n105_), .d(new_n104_), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n153_), .c(new_n48_), .O(new_n156_));
  inv1   g110(.a(x25), .O(new_n157_));
  nor2   g111(.a(new_n157_), .b(new_n49_), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n156_), .c(x16), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n151_), .O(z15));
  nand2  g114(.a(x18), .b(x07), .O(new_n161_));
  oai21  g115(.a(x18), .b(new_n71_), .c(new_n161_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n82_), .O(new_n163_));
  nand3  g117(.a(new_n105_), .b(new_n78_), .c(x16), .O(new_n164_));
  nand4  g118(.a(new_n130_), .b(new_n73_), .c(new_n157_), .d(new_n145_), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n164_), .c(x18), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n49_), .O(new_n167_));
  nand3  g121(.a(new_n155_), .b(x26), .c(x16), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n167_), .c(new_n163_), .O(z16));
  nand3  g123(.a(new_n105_), .b(x19), .c(new_n78_), .O(new_n170_));
  oai22  g124(.a(new_n170_), .b(new_n165_), .c(new_n47_), .d(new_n78_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n76_), .O(z17));
endmodule


