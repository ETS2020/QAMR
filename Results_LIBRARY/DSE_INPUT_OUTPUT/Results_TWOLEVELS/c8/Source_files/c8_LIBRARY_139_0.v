// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_,
    new_n178_;
  nor2   g000(.a(x26), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  aoi21  g011(.a(x27), .b(new_n57_), .c(new_n47_), .O(new_n58_));
  oai21  g012(.a(x27), .b(x21), .c(new_n58_), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x22), .O(new_n62_));
  nand2  g016(.a(new_n51_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n47_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  aoi21  g019(.a(x27), .b(new_n65_), .c(new_n47_), .O(new_n66_));
  oai21  g020(.a(x27), .b(x23), .c(new_n66_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand2  g024(.a(new_n51_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n47_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x25), .O(new_n75_));
  nand2  g029(.a(new_n51_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n47_), .O(z06));
  inv1   g031(.a(x18), .O(new_n78_));
  nor2   g032(.a(new_n51_), .b(new_n78_), .O(new_n79_));
  oai22  g033(.a(new_n79_), .b(x26), .c(new_n51_), .d(x15), .O(z07));
  inv1   g034(.a(new_n47_), .O(new_n81_));
  nand2  g035(.a(new_n81_), .b(new_n51_), .O(z08));
  inv1   g036(.a(x16), .O(new_n83_));
  nand2  g037(.a(x18), .b(x00), .O(new_n84_));
  nand2  g038(.a(x26), .b(new_n78_), .O(new_n85_));
  oai21  g039(.a(new_n85_), .b(new_n48_), .c(new_n84_), .O(new_n86_));
  nand2  g040(.a(new_n86_), .b(new_n83_), .O(new_n87_));
  inv1   g041(.a(x17), .O(new_n88_));
  nor3   g042(.a(new_n47_), .b(new_n50_), .c(new_n88_), .O(new_n89_));
  nor3   g043(.a(new_n47_), .b(x19), .c(x17), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n87_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  oai21  g047(.a(new_n85_), .b(new_n54_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n83_), .O(new_n95_));
  inv1   g049(.a(x20), .O(new_n96_));
  nor2   g050(.a(x19), .b(x17), .O(new_n97_));
  nor3   g051(.a(x20), .b(x19), .c(x17), .O(new_n98_));
  inv1   g052(.a(new_n98_), .O(new_n99_));
  oai21  g053(.a(new_n97_), .b(new_n96_), .c(new_n99_), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n81_), .c(x16), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n95_), .O(z10));
  nand2  g056(.a(x18), .b(x02), .O(new_n103_));
  oai21  g057(.a(x18), .b(new_n57_), .c(new_n103_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n83_), .O(new_n105_));
  inv1   g059(.a(x21), .O(new_n106_));
  nand3  g060(.a(new_n97_), .b(new_n106_), .c(new_n96_), .O(new_n107_));
  oai21  g061(.a(new_n98_), .b(new_n106_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n105_), .c(new_n81_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(new_n85_), .b(new_n60_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n83_), .O(new_n113_));
  nand3  g067(.a(new_n107_), .b(new_n81_), .c(x22), .O(new_n114_));
  nor2   g068(.a(new_n47_), .b(x22), .O(new_n115_));
  nand4  g069(.a(new_n115_), .b(new_n106_), .c(new_n96_), .d(new_n50_), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(x17), .c(new_n114_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x16), .O(new_n118_));
  nand2  g072(.a(new_n118_), .b(new_n113_), .O(z12));
  nand2  g073(.a(x18), .b(x04), .O(new_n120_));
  oai21  g074(.a(new_n85_), .b(new_n65_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n83_), .O(new_n122_));
  nor2   g076(.a(x22), .b(x21), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n97_), .c(new_n96_), .O(new_n124_));
  nand3  g078(.a(new_n124_), .b(new_n81_), .c(x23), .O(new_n125_));
  inv1   g079(.a(new_n125_), .O(new_n126_));
  nor2   g080(.a(new_n47_), .b(x23), .O(new_n127_));
  nand4  g081(.a(new_n127_), .b(new_n62_), .c(new_n106_), .d(new_n96_), .O(new_n128_));
  nor2   g082(.a(new_n128_), .b(x19), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n88_), .c(new_n126_), .O(new_n130_));
  oai21  g084(.a(new_n130_), .b(new_n83_), .c(new_n122_), .O(z13));
  nand2  g085(.a(x18), .b(x05), .O(new_n132_));
  oai21  g086(.a(new_n85_), .b(new_n68_), .c(new_n132_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n83_), .O(new_n134_));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  nand3  g089(.a(new_n135_), .b(new_n98_), .c(new_n106_), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n81_), .c(x24), .O(new_n137_));
  inv1   g091(.a(x23), .O(new_n138_));
  nand4  g092(.a(new_n81_), .b(new_n70_), .c(new_n138_), .d(new_n62_), .O(new_n139_));
  nor2   g093(.a(new_n139_), .b(x21), .O(new_n140_));
  nand4  g094(.a(new_n140_), .b(new_n96_), .c(new_n50_), .d(new_n88_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n137_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(x16), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n134_), .O(z14));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n73_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n83_), .O(new_n147_));
  nand4  g101(.a(new_n75_), .b(new_n70_), .c(new_n138_), .d(new_n62_), .O(new_n148_));
  nor2   g102(.a(new_n148_), .b(x21), .O(new_n149_));
  nand4  g103(.a(new_n149_), .b(new_n96_), .c(new_n50_), .d(new_n88_), .O(new_n150_));
  nand3  g104(.a(new_n123_), .b(new_n70_), .c(new_n138_), .O(new_n151_));
  oai21  g105(.a(new_n151_), .b(new_n99_), .c(x25), .O(new_n152_));
  nand2  g106(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x16), .O(new_n154_));
  nand3  g108(.a(new_n154_), .b(new_n147_), .c(new_n81_), .O(z15));
  inv1   g109(.a(x15), .O(new_n156_));
  nand2  g110(.a(x18), .b(x07), .O(new_n157_));
  oai21  g111(.a(x18), .b(new_n156_), .c(new_n157_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n83_), .O(new_n159_));
  xnor2a g113(.a(x26), .b(x25), .O(new_n160_));
  and2   g114(.a(x26), .b(x24), .O(new_n161_));
  aoi21  g115(.a(new_n160_), .b(new_n70_), .c(new_n161_), .O(new_n162_));
  nand2  g116(.a(x26), .b(x23), .O(new_n163_));
  oai21  g117(.a(new_n162_), .b(x23), .c(new_n163_), .O(new_n164_));
  nand4  g118(.a(new_n164_), .b(new_n62_), .c(new_n106_), .d(new_n96_), .O(new_n165_));
  nand2  g119(.a(x26), .b(x20), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n165_), .O(new_n167_));
  nand3  g121(.a(new_n167_), .b(new_n50_), .c(new_n88_), .O(new_n168_));
  nand2  g122(.a(new_n123_), .b(new_n97_), .O(new_n169_));
  nand2  g123(.a(new_n169_), .b(x26), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n168_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand3  g126(.a(new_n172_), .b(new_n159_), .c(new_n81_), .O(z16));
  nand3  g127(.a(new_n81_), .b(x27), .c(x17), .O(new_n174_));
  nand3  g128(.a(new_n106_), .b(new_n96_), .c(x19), .O(new_n175_));
  nor3   g129(.a(new_n175_), .b(new_n78_), .c(x17), .O(new_n176_));
  nor3   g130(.a(x26), .b(x25), .c(x24), .O(new_n177_));
  nand3  g131(.a(new_n177_), .b(new_n176_), .c(new_n135_), .O(new_n178_));
  aoi21  g132(.a(new_n178_), .b(new_n174_), .c(new_n83_), .O(z17));
endmodule


