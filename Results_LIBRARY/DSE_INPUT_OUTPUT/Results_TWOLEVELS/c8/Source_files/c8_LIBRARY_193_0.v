// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n141_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n162_, new_n163_, new_n164_, new_n165_,
    new_n166_, new_n167_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x23), .b(x18), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x09), .O(new_n51_));
  aoi21  g005(.a(x27), .b(new_n51_), .c(new_n48_), .O(new_n52_));
  oai21  g006(.a(x27), .b(x20), .c(new_n52_), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  nand2  g008(.a(x27), .b(new_n54_), .O(new_n55_));
  inv1   g009(.a(x21), .O(new_n56_));
  inv1   g010(.a(x27), .O(new_n57_));
  nand2  g011(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g012(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z02));
  inv1   g013(.a(x11), .O(new_n60_));
  nand2  g014(.a(x27), .b(new_n60_), .O(new_n61_));
  inv1   g015(.a(x22), .O(new_n62_));
  nand2  g016(.a(new_n57_), .b(new_n62_), .O(new_n63_));
  aoi21  g017(.a(new_n63_), .b(new_n61_), .c(new_n48_), .O(z03));
  inv1   g018(.a(x12), .O(new_n65_));
  inv1   g019(.a(new_n48_), .O(new_n66_));
  nand3  g020(.a(new_n66_), .b(x27), .c(new_n65_), .O(new_n67_));
  inv1   g021(.a(x23), .O(new_n68_));
  nand3  g022(.a(new_n57_), .b(new_n68_), .c(x18), .O(new_n69_));
  nand2  g023(.a(new_n69_), .b(new_n67_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n57_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x25), .c(new_n77_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  aoi21  g033(.a(x27), .b(new_n79_), .c(new_n48_), .O(new_n80_));
  oai21  g034(.a(x27), .b(x26), .c(new_n80_), .O(z07));
  nand2  g035(.a(new_n66_), .b(new_n57_), .O(z08));
  inv1   g036(.a(x17), .O(new_n83_));
  inv1   g037(.a(x19), .O(new_n84_));
  nor2   g038(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g039(.a(x19), .b(x17), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n85_), .c(x16), .O(new_n87_));
  inv1   g041(.a(x16), .O(new_n88_));
  nand2  g042(.a(x18), .b(x00), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n47_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nand3  g045(.a(new_n91_), .b(new_n87_), .c(new_n66_), .O(z09));
  nand2  g046(.a(x18), .b(x01), .O(new_n93_));
  inv1   g047(.a(x18), .O(new_n94_));
  nand3  g048(.a(x23), .b(new_n94_), .c(x09), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n88_), .O(new_n97_));
  inv1   g051(.a(x20), .O(new_n98_));
  nor2   g052(.a(x20), .b(x19), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n83_), .O(new_n100_));
  oai21  g054(.a(new_n86_), .b(new_n98_), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n66_), .c(x16), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n97_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n54_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n88_), .O(new_n106_));
  nor3   g060(.a(x20), .b(x19), .c(x17), .O(new_n107_));
  nor2   g061(.a(x21), .b(x20), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n86_), .O(new_n109_));
  oai21  g063(.a(new_n107_), .b(new_n56_), .c(new_n109_), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(x16), .O(new_n111_));
  nand3  g065(.a(new_n111_), .b(new_n106_), .c(new_n66_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n60_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n88_), .O(new_n115_));
  aoi21  g069(.a(new_n108_), .b(new_n86_), .c(new_n62_), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n86_), .c(new_n98_), .O(new_n118_));
  inv1   g072(.a(new_n118_), .O(new_n119_));
  oai21  g073(.a(new_n119_), .b(new_n116_), .c(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n66_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n65_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n88_), .O(new_n124_));
  nand3  g078(.a(new_n62_), .b(new_n56_), .c(new_n98_), .O(new_n125_));
  nand2  g079(.a(new_n86_), .b(x16), .O(new_n126_));
  oai21  g080(.a(new_n126_), .b(new_n125_), .c(x18), .O(new_n127_));
  nand2  g081(.a(new_n127_), .b(new_n68_), .O(new_n128_));
  nand3  g082(.a(new_n118_), .b(x23), .c(x16), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n128_), .c(new_n124_), .O(z13));
  nand2  g084(.a(x18), .b(x05), .O(new_n131_));
  oai21  g085(.a(x18), .b(new_n71_), .c(new_n131_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n88_), .O(new_n133_));
  nand3  g087(.a(new_n108_), .b(new_n73_), .c(new_n62_), .O(new_n134_));
  oai21  g088(.a(new_n134_), .b(new_n126_), .c(x18), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n68_), .O(new_n136_));
  nor2   g090(.a(x23), .b(x22), .O(new_n137_));
  nand3  g091(.a(new_n137_), .b(new_n107_), .c(new_n56_), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(x24), .c(x16), .O(new_n139_));
  nand3  g093(.a(new_n139_), .b(new_n136_), .c(new_n133_), .O(z14));
  nand2  g094(.a(x18), .b(x06), .O(new_n141_));
  oai21  g095(.a(x18), .b(new_n76_), .c(new_n141_), .O(new_n142_));
  nand2  g096(.a(new_n142_), .b(new_n88_), .O(new_n143_));
  nand3  g097(.a(new_n99_), .b(new_n83_), .c(x16), .O(new_n144_));
  nor2   g098(.a(x25), .b(x24), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n117_), .O(new_n146_));
  oai21  g100(.a(new_n146_), .b(new_n144_), .c(x18), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n68_), .O(new_n148_));
  nand4  g102(.a(new_n117_), .b(new_n107_), .c(new_n73_), .d(new_n68_), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(x25), .c(x16), .O(new_n150_));
  nand3  g104(.a(new_n150_), .b(new_n148_), .c(new_n143_), .O(z15));
  nand2  g105(.a(x18), .b(x07), .O(new_n152_));
  oai21  g106(.a(x18), .b(new_n79_), .c(new_n152_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n88_), .O(new_n154_));
  nor2   g108(.a(x26), .b(x25), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n117_), .c(new_n73_), .O(new_n156_));
  oai21  g110(.a(new_n156_), .b(new_n144_), .c(x18), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n68_), .O(new_n158_));
  nand4  g112(.a(new_n145_), .b(new_n137_), .c(new_n108_), .d(new_n86_), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(x26), .c(x16), .O(new_n160_));
  nand3  g114(.a(new_n160_), .b(new_n158_), .c(new_n154_), .O(z16));
  nand3  g115(.a(new_n66_), .b(x27), .c(x17), .O(new_n162_));
  nor3   g116(.a(x26), .b(x25), .c(x24), .O(new_n163_));
  nor2   g117(.a(new_n94_), .b(x17), .O(new_n164_));
  nand3  g118(.a(new_n56_), .b(new_n98_), .c(x19), .O(new_n165_));
  inv1   g119(.a(new_n165_), .O(new_n166_));
  nand4  g120(.a(new_n166_), .b(new_n164_), .c(new_n163_), .d(new_n137_), .O(new_n167_));
  aoi21  g121(.a(new_n167_), .b(new_n162_), .c(new_n88_), .O(z17));
endmodule


