// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n129_, new_n130_, new_n131_, new_n132_,
    new_n133_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n142_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_, new_n173_;
  nor2   g000(.a(x26), .b(x18), .O(new_n47_));
  inv1   g001(.a(x08), .O(new_n48_));
  nand2  g002(.a(x27), .b(new_n48_), .O(new_n49_));
  inv1   g003(.a(x19), .O(new_n50_));
  inv1   g004(.a(x27), .O(new_n51_));
  nand2  g005(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  aoi21  g006(.a(new_n52_), .b(new_n49_), .c(new_n47_), .O(z00));
  inv1   g007(.a(x09), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n47_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x20), .c(new_n55_), .O(z01));
  inv1   g010(.a(x10), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x21), .O(new_n59_));
  nand2  g013(.a(new_n51_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n47_), .O(z02));
  inv1   g015(.a(x11), .O(new_n62_));
  nand2  g016(.a(x27), .b(new_n62_), .O(new_n63_));
  inv1   g017(.a(x22), .O(new_n64_));
  nand2  g018(.a(new_n51_), .b(new_n64_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n63_), .c(new_n47_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n51_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n47_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n47_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x24), .c(new_n73_), .O(z05));
  inv1   g028(.a(x14), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n47_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x25), .c(new_n76_), .O(z06));
  inv1   g031(.a(x18), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  oai21  g033(.a(new_n51_), .b(new_n78_), .c(new_n79_), .O(new_n80_));
  oai21  g034(.a(new_n51_), .b(x15), .c(new_n80_), .O(z07));
  inv1   g035(.a(new_n47_), .O(new_n82_));
  nand2  g036(.a(new_n82_), .b(new_n51_), .O(z08));
  inv1   g037(.a(x16), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  nand2  g039(.a(x26), .b(new_n78_), .O(new_n86_));
  oai21  g040(.a(new_n86_), .b(new_n48_), .c(new_n85_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  inv1   g042(.a(x17), .O(new_n89_));
  nor3   g043(.a(new_n47_), .b(new_n50_), .c(new_n89_), .O(new_n90_));
  nor3   g044(.a(new_n47_), .b(x19), .c(x17), .O(new_n91_));
  oai21  g045(.a(new_n91_), .b(new_n90_), .c(x16), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n88_), .O(z09));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(new_n86_), .b(new_n54_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n84_), .O(new_n96_));
  inv1   g050(.a(x20), .O(new_n97_));
  nor2   g051(.a(x19), .b(x17), .O(new_n98_));
  nor3   g052(.a(x20), .b(x19), .c(x17), .O(new_n99_));
  inv1   g053(.a(new_n99_), .O(new_n100_));
  oai21  g054(.a(new_n98_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand3  g055(.a(new_n101_), .b(new_n82_), .c(x16), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(new_n96_), .O(z10));
  nand2  g057(.a(x18), .b(x02), .O(new_n104_));
  oai21  g058(.a(x18), .b(new_n57_), .c(new_n104_), .O(new_n105_));
  nand2  g059(.a(new_n105_), .b(new_n84_), .O(new_n106_));
  nand3  g060(.a(new_n98_), .b(new_n59_), .c(new_n97_), .O(new_n107_));
  oai21  g061(.a(new_n99_), .b(new_n59_), .c(new_n107_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(x16), .O(new_n109_));
  nand3  g063(.a(new_n109_), .b(new_n106_), .c(new_n82_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n62_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n84_), .O(new_n113_));
  and2   g067(.a(new_n107_), .b(x22), .O(new_n114_));
  nor2   g068(.a(x22), .b(x21), .O(new_n115_));
  nand3  g069(.a(new_n115_), .b(new_n98_), .c(new_n97_), .O(new_n116_));
  inv1   g070(.a(new_n116_), .O(new_n117_));
  oai21  g071(.a(new_n117_), .b(new_n114_), .c(x16), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n113_), .c(new_n82_), .O(z12));
  nand2  g073(.a(x18), .b(x04), .O(new_n120_));
  oai21  g074(.a(new_n86_), .b(new_n67_), .c(new_n120_), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n84_), .O(new_n122_));
  nor2   g076(.a(new_n117_), .b(new_n47_), .O(new_n123_));
  nor2   g077(.a(new_n47_), .b(x23), .O(new_n124_));
  nand4  g078(.a(new_n124_), .b(new_n64_), .c(new_n59_), .d(new_n97_), .O(new_n125_));
  nor2   g079(.a(new_n125_), .b(x19), .O(new_n126_));
  aoi22  g080(.a(new_n126_), .b(new_n89_), .c(new_n123_), .d(x23), .O(new_n127_));
  oai21  g081(.a(new_n127_), .b(new_n84_), .c(new_n122_), .O(z13));
  nand2  g082(.a(x18), .b(x05), .O(new_n129_));
  oai21  g083(.a(new_n86_), .b(new_n72_), .c(new_n129_), .O(new_n130_));
  nand2  g084(.a(new_n130_), .b(new_n84_), .O(new_n131_));
  nor2   g085(.a(x23), .b(x22), .O(new_n132_));
  nand3  g086(.a(new_n132_), .b(new_n99_), .c(new_n59_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n82_), .c(x24), .O(new_n134_));
  inv1   g088(.a(x24), .O(new_n135_));
  nand4  g089(.a(new_n82_), .b(new_n135_), .c(new_n69_), .d(new_n64_), .O(new_n136_));
  nor2   g090(.a(new_n136_), .b(x21), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n97_), .c(new_n50_), .d(new_n89_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x16), .O(new_n140_));
  nand2  g094(.a(new_n140_), .b(new_n131_), .O(z14));
  nand2  g095(.a(x18), .b(x06), .O(new_n142_));
  oai21  g096(.a(x18), .b(new_n75_), .c(new_n142_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n84_), .O(new_n144_));
  inv1   g098(.a(x25), .O(new_n145_));
  nand4  g099(.a(new_n145_), .b(new_n135_), .c(new_n69_), .d(new_n64_), .O(new_n146_));
  nor2   g100(.a(new_n146_), .b(x21), .O(new_n147_));
  nand4  g101(.a(new_n147_), .b(new_n97_), .c(new_n50_), .d(new_n89_), .O(new_n148_));
  nand4  g102(.a(new_n115_), .b(new_n99_), .c(new_n135_), .d(new_n69_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(x25), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(x16), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n144_), .c(new_n82_), .O(z15));
  inv1   g107(.a(x15), .O(new_n154_));
  nand2  g108(.a(x18), .b(x07), .O(new_n155_));
  oai21  g109(.a(x18), .b(new_n154_), .c(new_n155_), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n84_), .O(new_n157_));
  xor2a  g111(.a(x26), .b(x25), .O(new_n158_));
  nand2  g112(.a(x26), .b(x24), .O(new_n159_));
  oai21  g113(.a(new_n158_), .b(x24), .c(new_n159_), .O(new_n160_));
  nand4  g114(.a(new_n160_), .b(new_n69_), .c(new_n64_), .d(new_n59_), .O(new_n161_));
  nand2  g115(.a(x26), .b(x20), .O(new_n162_));
  oai21  g116(.a(new_n161_), .b(x20), .c(new_n162_), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n50_), .c(new_n89_), .O(new_n164_));
  nand3  g118(.a(new_n132_), .b(new_n98_), .c(new_n59_), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x26), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n164_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n157_), .c(new_n82_), .O(z16));
  nand4  g123(.a(new_n59_), .b(new_n97_), .c(x19), .d(new_n89_), .O(new_n170_));
  nand4  g124(.a(new_n132_), .b(new_n79_), .c(new_n145_), .d(new_n135_), .O(new_n171_));
  oai22  g125(.a(new_n171_), .b(new_n170_), .c(new_n51_), .d(new_n89_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand2  g127(.a(new_n173_), .b(new_n82_), .O(z17));
endmodule


