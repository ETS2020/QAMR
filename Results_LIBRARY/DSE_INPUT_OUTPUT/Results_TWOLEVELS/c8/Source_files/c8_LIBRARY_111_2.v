// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n148_, new_n149_, new_n150_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n162_, new_n163_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n176_, new_n177_,
    new_n178_;
  inv1   g000(.a(x17), .O(new_n47_));
  nor2   g001(.a(x18), .b(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n48_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x20), .c(new_n56_), .O(z01));
  inv1   g011(.a(x10), .O(new_n58_));
  nand2  g012(.a(x27), .b(new_n58_), .O(new_n59_));
  inv1   g013(.a(x21), .O(new_n60_));
  nand2  g014(.a(new_n52_), .b(new_n60_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n59_), .c(new_n48_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n52_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n48_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n48_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n52_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z05));
  inv1   g029(.a(x14), .O(new_n76_));
  aoi21  g030(.a(x27), .b(new_n76_), .c(new_n48_), .O(new_n77_));
  oai21  g031(.a(x27), .b(x25), .c(new_n77_), .O(z06));
  inv1   g032(.a(x15), .O(new_n79_));
  nand2  g033(.a(x27), .b(new_n79_), .O(new_n80_));
  inv1   g034(.a(x26), .O(new_n81_));
  nand2  g035(.a(new_n52_), .b(new_n81_), .O(new_n82_));
  aoi21  g036(.a(new_n82_), .b(new_n80_), .c(new_n48_), .O(z07));
  inv1   g037(.a(new_n48_), .O(new_n84_));
  nand2  g038(.a(new_n84_), .b(new_n52_), .O(z08));
  nor2   g039(.a(new_n51_), .b(new_n47_), .O(new_n86_));
  nor2   g040(.a(x19), .b(x17), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  inv1   g042(.a(x16), .O(new_n89_));
  nand2  g043(.a(x18), .b(x00), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n49_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n88_), .c(new_n84_), .O(z09));
  nand2  g047(.a(x18), .b(x01), .O(new_n94_));
  oai21  g048(.a(x18), .b(new_n55_), .c(new_n94_), .O(new_n95_));
  nand2  g049(.a(new_n95_), .b(new_n89_), .O(new_n96_));
  inv1   g050(.a(x20), .O(new_n97_));
  oai21  g051(.a(new_n97_), .b(new_n89_), .c(x18), .O(new_n98_));
  nand2  g052(.a(new_n98_), .b(x17), .O(new_n99_));
  nor2   g053(.a(x20), .b(x19), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n47_), .O(new_n101_));
  oai21  g055(.a(new_n97_), .b(new_n51_), .c(new_n101_), .O(new_n102_));
  nand2  g056(.a(new_n102_), .b(x16), .O(new_n103_));
  nand3  g057(.a(new_n103_), .b(new_n99_), .c(new_n96_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  inv1   g059(.a(x18), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n47_), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n58_), .c(new_n105_), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n89_), .O(new_n109_));
  oai21  g063(.a(x20), .b(x19), .c(x21), .O(new_n110_));
  nor3   g064(.a(x21), .b(x20), .c(x19), .O(new_n111_));
  inv1   g065(.a(new_n111_), .O(new_n112_));
  aoi21  g066(.a(new_n112_), .b(new_n110_), .c(x17), .O(new_n113_));
  nand3  g067(.a(x21), .b(x18), .c(x17), .O(new_n114_));
  inv1   g068(.a(new_n114_), .O(new_n115_));
  oai21  g069(.a(new_n115_), .b(new_n113_), .c(x16), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n109_), .O(z11));
  nand2  g071(.a(x18), .b(x03), .O(new_n118_));
  oai21  g072(.a(x18), .b(new_n63_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n89_), .O(new_n120_));
  oai21  g074(.a(new_n65_), .b(new_n89_), .c(x18), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(x17), .O(new_n122_));
  nor2   g076(.a(x22), .b(x21), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n87_), .c(new_n97_), .O(new_n124_));
  oai21  g078(.a(new_n111_), .b(new_n65_), .c(new_n124_), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x16), .O(new_n126_));
  nand3  g080(.a(new_n126_), .b(new_n122_), .c(new_n120_), .O(z12));
  nand2  g081(.a(x18), .b(x04), .O(new_n128_));
  oai21  g082(.a(x18), .b(new_n68_), .c(new_n128_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(new_n89_), .O(new_n130_));
  inv1   g084(.a(x23), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n89_), .c(x18), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(x17), .O(new_n133_));
  aoi21  g087(.a(new_n123_), .b(new_n100_), .c(new_n131_), .O(new_n134_));
  nor2   g088(.a(x23), .b(x22), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n60_), .O(new_n136_));
  nor2   g090(.a(new_n136_), .b(new_n101_), .O(new_n137_));
  oai21  g091(.a(new_n137_), .b(new_n134_), .c(x16), .O(new_n138_));
  nand3  g092(.a(new_n138_), .b(new_n133_), .c(new_n130_), .O(z13));
  nand2  g093(.a(x18), .b(x05), .O(new_n140_));
  oai21  g094(.a(new_n107_), .b(new_n71_), .c(new_n140_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(new_n89_), .O(new_n142_));
  nand3  g096(.a(new_n135_), .b(new_n100_), .c(new_n60_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x24), .O(new_n144_));
  nor3   g098(.a(x24), .b(x23), .c(x22), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n111_), .O(new_n146_));
  aoi21  g100(.a(new_n146_), .b(new_n144_), .c(x17), .O(new_n147_));
  nand3  g101(.a(x24), .b(x18), .c(x17), .O(new_n148_));
  inv1   g102(.a(new_n148_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n147_), .c(x16), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n142_), .O(z14));
  nand2  g105(.a(x18), .b(x06), .O(new_n152_));
  oai21  g106(.a(x18), .b(new_n76_), .c(new_n152_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n89_), .O(new_n154_));
  inv1   g108(.a(x25), .O(new_n155_));
  oai21  g109(.a(new_n155_), .b(new_n89_), .c(x18), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(x17), .O(new_n157_));
  aoi21  g111(.a(new_n145_), .b(new_n111_), .c(new_n155_), .O(new_n158_));
  nor2   g112(.a(x21), .b(x20), .O(new_n159_));
  nand2  g113(.a(new_n159_), .b(new_n87_), .O(new_n160_));
  nand3  g114(.a(new_n135_), .b(new_n155_), .c(new_n73_), .O(new_n161_));
  nor2   g115(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  oai21  g116(.a(new_n162_), .b(new_n158_), .c(x16), .O(new_n163_));
  nand3  g117(.a(new_n163_), .b(new_n157_), .c(new_n154_), .O(z15));
  nand2  g118(.a(x18), .b(x07), .O(new_n165_));
  oai21  g119(.a(x18), .b(new_n79_), .c(new_n165_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n89_), .O(new_n167_));
  oai21  g121(.a(new_n81_), .b(new_n89_), .c(x18), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x17), .O(new_n169_));
  inv1   g123(.a(new_n161_), .O(new_n170_));
  aoi21  g124(.a(new_n170_), .b(new_n111_), .c(new_n81_), .O(new_n171_));
  nand4  g125(.a(new_n135_), .b(new_n81_), .c(new_n155_), .d(new_n73_), .O(new_n172_));
  nor2   g126(.a(new_n172_), .b(new_n160_), .O(new_n173_));
  oai21  g127(.a(new_n173_), .b(new_n171_), .c(x16), .O(new_n174_));
  nand3  g128(.a(new_n174_), .b(new_n169_), .c(new_n167_), .O(z16));
  nand3  g129(.a(new_n159_), .b(x19), .c(new_n47_), .O(new_n176_));
  oai22  g130(.a(new_n176_), .b(new_n172_), .c(new_n52_), .d(new_n47_), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(x16), .O(new_n178_));
  nand2  g132(.a(new_n178_), .b(new_n84_), .O(z17));
endmodule


