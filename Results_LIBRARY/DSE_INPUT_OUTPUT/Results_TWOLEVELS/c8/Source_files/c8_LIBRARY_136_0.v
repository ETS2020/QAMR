// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n143_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_, new_n171_;
  inv1   g000(.a(x08), .O(new_n47_));
  nor2   g001(.a(x20), .b(x18), .O(new_n48_));
  aoi21  g002(.a(x27), .b(new_n47_), .c(new_n48_), .O(new_n49_));
  oai21  g003(.a(x27), .b(x19), .c(new_n49_), .O(z00));
  inv1   g004(.a(x27), .O(new_n51_));
  inv1   g005(.a(x18), .O(new_n52_));
  inv1   g006(.a(x20), .O(new_n53_));
  oai21  g007(.a(new_n51_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(new_n51_), .b(x09), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x21), .O(new_n58_));
  nand2  g012(.a(new_n51_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n48_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x22), .O(new_n63_));
  nand2  g017(.a(new_n51_), .b(new_n63_), .O(new_n64_));
  aoi21  g018(.a(new_n64_), .b(new_n62_), .c(new_n48_), .O(z03));
  inv1   g019(.a(x12), .O(new_n66_));
  nand2  g020(.a(x27), .b(new_n66_), .O(new_n67_));
  inv1   g021(.a(x23), .O(new_n68_));
  nand2  g022(.a(new_n51_), .b(new_n68_), .O(new_n69_));
  aoi21  g023(.a(new_n69_), .b(new_n67_), .c(new_n48_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  aoi21  g025(.a(x27), .b(new_n71_), .c(new_n48_), .O(new_n72_));
  oai21  g026(.a(x27), .b(x24), .c(new_n72_), .O(z05));
  inv1   g027(.a(x14), .O(new_n74_));
  aoi21  g028(.a(x27), .b(new_n74_), .c(new_n48_), .O(new_n75_));
  oai21  g029(.a(x27), .b(x25), .c(new_n75_), .O(z06));
  inv1   g030(.a(x15), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x26), .O(new_n79_));
  nand2  g033(.a(new_n51_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n48_), .O(z07));
  nor2   g035(.a(new_n48_), .b(new_n51_), .O(z08));
  inv1   g036(.a(new_n48_), .O(new_n83_));
  inv1   g037(.a(x17), .O(new_n84_));
  inv1   g038(.a(x19), .O(new_n85_));
  nor2   g039(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor2   g040(.a(x19), .b(x17), .O(new_n87_));
  oai21  g041(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  inv1   g042(.a(x16), .O(new_n89_));
  nand2  g043(.a(x18), .b(x00), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n47_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand3  g046(.a(new_n92_), .b(new_n88_), .c(new_n83_), .O(z09));
  inv1   g047(.a(x09), .O(new_n94_));
  nand2  g048(.a(x18), .b(x01), .O(new_n95_));
  oai21  g049(.a(x18), .b(new_n94_), .c(new_n95_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(new_n89_), .O(new_n97_));
  nor2   g051(.a(new_n87_), .b(new_n53_), .O(new_n98_));
  nor3   g052(.a(x20), .b(x19), .c(x17), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n97_), .c(new_n83_), .O(z10));
  nand2  g055(.a(x18), .b(x02), .O(new_n102_));
  nand2  g056(.a(x20), .b(new_n52_), .O(new_n103_));
  oai21  g057(.a(new_n103_), .b(new_n56_), .c(new_n102_), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n89_), .O(new_n105_));
  oai21  g059(.a(x19), .b(x17), .c(x21), .O(new_n106_));
  nor2   g060(.a(x21), .b(x20), .O(new_n107_));
  nand2  g061(.a(new_n107_), .b(new_n87_), .O(new_n108_));
  aoi21  g062(.a(new_n108_), .b(new_n106_), .c(new_n52_), .O(new_n109_));
  nor2   g063(.a(new_n58_), .b(new_n53_), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n109_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n105_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n61_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n89_), .O(new_n115_));
  nand2  g069(.a(new_n84_), .b(x16), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(new_n85_), .O(new_n118_));
  oai21  g072(.a(new_n118_), .b(new_n116_), .c(x18), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(new_n53_), .O(new_n120_));
  nand3  g074(.a(new_n108_), .b(x22), .c(x16), .O(new_n121_));
  nand3  g075(.a(new_n121_), .b(new_n120_), .c(new_n115_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(new_n103_), .b(new_n66_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n89_), .O(new_n125_));
  nand2  g079(.a(new_n117_), .b(new_n87_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x23), .O(new_n127_));
  nor2   g081(.a(x23), .b(x22), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n99_), .c(new_n58_), .O(new_n129_));
  aoi21  g083(.a(new_n129_), .b(new_n127_), .c(new_n52_), .O(new_n130_));
  nor2   g084(.a(new_n68_), .b(new_n53_), .O(new_n131_));
  oai21  g085(.a(new_n131_), .b(new_n130_), .c(x16), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n125_), .O(z13));
  nand2  g087(.a(x18), .b(x05), .O(new_n134_));
  oai21  g088(.a(x18), .b(new_n71_), .c(new_n134_), .O(new_n135_));
  nand2  g089(.a(new_n135_), .b(new_n89_), .O(new_n136_));
  nor2   g090(.a(x24), .b(x23), .O(new_n137_));
  nand4  g091(.a(new_n137_), .b(new_n117_), .c(new_n87_), .d(x16), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(x18), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n53_), .O(new_n140_));
  nand3  g094(.a(new_n129_), .b(x24), .c(x16), .O(new_n141_));
  nand3  g095(.a(new_n141_), .b(new_n140_), .c(new_n136_), .O(z14));
  nand2  g096(.a(x18), .b(x06), .O(new_n143_));
  oai21  g097(.a(new_n103_), .b(new_n74_), .c(new_n143_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n89_), .O(new_n145_));
  nand3  g099(.a(new_n58_), .b(new_n85_), .c(new_n84_), .O(new_n146_));
  inv1   g100(.a(x24), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n68_), .c(new_n63_), .O(new_n148_));
  oai21  g102(.a(new_n148_), .b(new_n146_), .c(x25), .O(new_n149_));
  nor2   g103(.a(x25), .b(x24), .O(new_n150_));
  nand4  g104(.a(new_n150_), .b(new_n128_), .c(new_n107_), .d(new_n87_), .O(new_n151_));
  aoi21  g105(.a(new_n151_), .b(new_n149_), .c(new_n52_), .O(new_n152_));
  and2   g106(.a(x25), .b(x20), .O(new_n153_));
  oai21  g107(.a(new_n153_), .b(new_n152_), .c(x16), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n145_), .O(z15));
  nand2  g109(.a(x18), .b(x07), .O(new_n156_));
  oai21  g110(.a(new_n103_), .b(new_n77_), .c(new_n156_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(new_n89_), .O(new_n158_));
  nor2   g112(.a(x21), .b(x19), .O(new_n159_));
  nand4  g113(.a(new_n150_), .b(new_n159_), .c(new_n128_), .d(new_n84_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(x26), .O(new_n161_));
  nor3   g115(.a(x26), .b(x25), .c(x24), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n128_), .c(new_n107_), .d(new_n87_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n161_), .c(new_n52_), .O(new_n164_));
  nor2   g118(.a(new_n79_), .b(new_n53_), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n164_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n158_), .O(z16));
  nand2  g121(.a(z08), .b(x17), .O(new_n168_));
  nand2  g122(.a(new_n107_), .b(x19), .O(new_n169_));
  nor3   g123(.a(new_n169_), .b(new_n52_), .c(x17), .O(new_n170_));
  nand3  g124(.a(new_n170_), .b(new_n162_), .c(new_n128_), .O(new_n171_));
  aoi21  g125(.a(new_n171_), .b(new_n168_), .c(new_n89_), .O(z17));
endmodule


