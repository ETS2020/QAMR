// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n54_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n174_, new_n175_, new_n176_, new_n177_;
  inv1   g000(.a(x08), .O(new_n47_));
  inv1   g001(.a(x20), .O(new_n48_));
  nor2   g002(.a(new_n48_), .b(x18), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  aoi21  g005(.a(x27), .b(x18), .c(new_n48_), .O(new_n52_));
  oai22  g006(.a(new_n52_), .b(x09), .c(x27), .d(x20), .O(z01));
  inv1   g007(.a(x10), .O(new_n54_));
  aoi21  g008(.a(x27), .b(new_n54_), .c(new_n49_), .O(new_n55_));
  oai21  g009(.a(x27), .b(x21), .c(new_n55_), .O(z02));
  inv1   g010(.a(x11), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x22), .O(new_n59_));
  inv1   g013(.a(x27), .O(new_n60_));
  nand2  g014(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  aoi21  g015(.a(new_n61_), .b(new_n58_), .c(new_n49_), .O(z03));
  inv1   g016(.a(x12), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x23), .O(new_n65_));
  nand2  g019(.a(new_n60_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n49_), .O(z04));
  inv1   g021(.a(x13), .O(new_n68_));
  nand2  g022(.a(x27), .b(new_n68_), .O(new_n69_));
  inv1   g023(.a(x24), .O(new_n70_));
  nand2  g024(.a(new_n60_), .b(new_n70_), .O(new_n71_));
  aoi21  g025(.a(new_n71_), .b(new_n69_), .c(new_n49_), .O(z05));
  inv1   g026(.a(x14), .O(new_n73_));
  nand2  g027(.a(x27), .b(new_n73_), .O(new_n74_));
  inv1   g028(.a(x25), .O(new_n75_));
  nand2  g029(.a(new_n60_), .b(new_n75_), .O(new_n76_));
  aoi21  g030(.a(new_n76_), .b(new_n74_), .c(new_n49_), .O(z06));
  inv1   g031(.a(x15), .O(new_n78_));
  aoi21  g032(.a(x27), .b(new_n78_), .c(new_n49_), .O(new_n79_));
  oai21  g033(.a(x27), .b(x26), .c(new_n79_), .O(z07));
  nor2   g034(.a(new_n49_), .b(new_n60_), .O(z08));
  nor2   g035(.a(x20), .b(x18), .O(new_n82_));
  aoi22  g036(.a(new_n82_), .b(x08), .c(x18), .d(x00), .O(new_n83_));
  inv1   g037(.a(new_n49_), .O(new_n84_));
  xnor2a g038(.a(x19), .b(x17), .O(new_n85_));
  nand3  g039(.a(new_n85_), .b(new_n84_), .c(x16), .O(new_n86_));
  oai21  g040(.a(new_n83_), .b(x16), .c(new_n86_), .O(z09));
  inv1   g041(.a(x16), .O(new_n88_));
  inv1   g042(.a(x09), .O(new_n89_));
  nand2  g043(.a(x18), .b(x01), .O(new_n90_));
  oai21  g044(.a(x18), .b(new_n89_), .c(new_n90_), .O(new_n91_));
  nand2  g045(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  nor2   g046(.a(x19), .b(x17), .O(new_n93_));
  nor2   g047(.a(new_n93_), .b(new_n48_), .O(new_n94_));
  nor3   g048(.a(x20), .b(x19), .c(x17), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n94_), .c(x16), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(new_n92_), .c(new_n84_), .O(z10));
  nand2  g051(.a(x18), .b(x02), .O(new_n98_));
  nand2  g052(.a(new_n82_), .b(x10), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g054(.a(new_n100_), .b(new_n88_), .O(new_n101_));
  oai21  g055(.a(x19), .b(x17), .c(x21), .O(new_n102_));
  inv1   g056(.a(x17), .O(new_n103_));
  nor2   g057(.a(x21), .b(x19), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n102_), .c(x20), .O(new_n106_));
  nand3  g060(.a(x21), .b(x20), .c(x18), .O(new_n107_));
  inv1   g061(.a(new_n107_), .O(new_n108_));
  oai21  g062(.a(new_n108_), .b(new_n106_), .c(x16), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n101_), .O(z11));
  nand2  g064(.a(x18), .b(x03), .O(new_n111_));
  oai21  g065(.a(x18), .b(new_n57_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(new_n88_), .O(new_n113_));
  oai21  g067(.a(new_n59_), .b(new_n88_), .c(x18), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x20), .O(new_n115_));
  nor3   g069(.a(x21), .b(x19), .c(x17), .O(new_n116_));
  nor2   g070(.a(x22), .b(x21), .O(new_n117_));
  nand3  g071(.a(new_n117_), .b(new_n93_), .c(new_n48_), .O(new_n118_));
  oai21  g072(.a(new_n116_), .b(new_n59_), .c(new_n118_), .O(new_n119_));
  nand2  g073(.a(new_n119_), .b(x16), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n115_), .c(new_n113_), .O(z12));
  nand2  g075(.a(x18), .b(x04), .O(new_n122_));
  oai21  g076(.a(x18), .b(new_n63_), .c(new_n122_), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n88_), .O(new_n124_));
  oai21  g078(.a(new_n65_), .b(new_n88_), .c(x18), .O(new_n125_));
  nand2  g079(.a(new_n125_), .b(x20), .O(new_n126_));
  aoi21  g080(.a(new_n117_), .b(new_n93_), .c(new_n65_), .O(new_n127_));
  inv1   g081(.a(x19), .O(new_n128_));
  nand3  g082(.a(new_n48_), .b(new_n128_), .c(new_n103_), .O(new_n129_));
  inv1   g083(.a(x21), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand2  g085(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n129_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n127_), .c(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n126_), .c(new_n124_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n68_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n88_), .O(new_n138_));
  oai21  g092(.a(new_n70_), .b(new_n88_), .c(x18), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x20), .O(new_n140_));
  nand3  g094(.a(new_n131_), .b(new_n93_), .c(new_n130_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x24), .O(new_n142_));
  nand4  g096(.a(new_n117_), .b(new_n95_), .c(new_n70_), .d(new_n65_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(new_n142_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(x16), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n140_), .c(new_n138_), .O(z14));
  nand2  g100(.a(x18), .b(x06), .O(new_n147_));
  oai21  g101(.a(x18), .b(new_n73_), .c(new_n147_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n88_), .O(new_n149_));
  oai21  g103(.a(new_n75_), .b(new_n88_), .c(x18), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(x20), .O(new_n151_));
  nor3   g105(.a(x24), .b(x23), .c(x22), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n116_), .c(new_n75_), .O(new_n153_));
  nor2   g107(.a(x21), .b(x20), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(new_n93_), .O(new_n155_));
  nor2   g109(.a(x25), .b(x24), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n131_), .O(new_n157_));
  nor2   g111(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  oai21  g112(.a(new_n158_), .b(new_n153_), .c(x16), .O(new_n159_));
  nand3  g113(.a(new_n159_), .b(new_n151_), .c(new_n149_), .O(z15));
  nand2  g114(.a(x18), .b(x07), .O(new_n161_));
  oai21  g115(.a(x18), .b(new_n78_), .c(new_n161_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n88_), .O(new_n163_));
  inv1   g117(.a(x26), .O(new_n164_));
  oai21  g118(.a(new_n164_), .b(new_n88_), .c(x18), .O(new_n165_));
  nand2  g119(.a(new_n165_), .b(x20), .O(new_n166_));
  nand4  g120(.a(new_n156_), .b(new_n131_), .c(new_n104_), .d(new_n103_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x26), .O(new_n168_));
  nor3   g122(.a(x26), .b(x25), .c(x24), .O(new_n169_));
  nand4  g123(.a(new_n169_), .b(new_n154_), .c(new_n131_), .d(new_n93_), .O(new_n170_));
  nand2  g124(.a(new_n170_), .b(new_n168_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand3  g126(.a(new_n172_), .b(new_n166_), .c(new_n163_), .O(z16));
  nand4  g127(.a(new_n131_), .b(new_n164_), .c(new_n75_), .d(new_n70_), .O(new_n174_));
  nand3  g128(.a(new_n154_), .b(x19), .c(new_n103_), .O(new_n175_));
  oai22  g129(.a(new_n175_), .b(new_n174_), .c(new_n60_), .d(new_n103_), .O(new_n176_));
  nand2  g130(.a(new_n176_), .b(x16), .O(new_n177_));
  nand2  g131(.a(new_n177_), .b(new_n84_), .O(z17));
endmodule


