// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n56_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n168_, new_n169_, new_n170_;
  inv1   g000(.a(x27), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  inv1   g002(.a(x19), .O(new_n49_));
  oai21  g003(.a(new_n47_), .b(new_n48_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(new_n47_), .b(x08), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  nor2   g006(.a(x19), .b(x18), .O(new_n53_));
  aoi21  g007(.a(x27), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  oai21  g008(.a(x27), .b(x20), .c(new_n54_), .O(z01));
  inv1   g009(.a(x10), .O(new_n56_));
  nand2  g010(.a(x27), .b(new_n56_), .O(new_n57_));
  inv1   g011(.a(x21), .O(new_n58_));
  nand2  g012(.a(new_n47_), .b(new_n58_), .O(new_n59_));
  aoi21  g013(.a(new_n59_), .b(new_n57_), .c(new_n53_), .O(z02));
  inv1   g014(.a(x11), .O(new_n61_));
  aoi21  g015(.a(x27), .b(new_n61_), .c(new_n53_), .O(new_n62_));
  oai21  g016(.a(x27), .b(x22), .c(new_n62_), .O(z03));
  inv1   g017(.a(x12), .O(new_n64_));
  aoi21  g018(.a(x27), .b(new_n64_), .c(new_n53_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x23), .c(new_n65_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n47_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n53_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n53_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  aoi21  g029(.a(x27), .b(new_n75_), .c(new_n53_), .O(new_n76_));
  oai21  g030(.a(x27), .b(x26), .c(new_n76_), .O(z07));
  nor2   g031(.a(new_n53_), .b(new_n47_), .O(z08));
  inv1   g032(.a(x17), .O(new_n79_));
  nor2   g033(.a(new_n49_), .b(new_n79_), .O(new_n80_));
  nor2   g034(.a(x19), .b(x17), .O(new_n81_));
  oai21  g035(.a(new_n81_), .b(new_n80_), .c(x16), .O(new_n82_));
  inv1   g036(.a(x16), .O(new_n83_));
  inv1   g037(.a(x08), .O(new_n84_));
  nand2  g038(.a(x18), .b(x00), .O(new_n85_));
  oai21  g039(.a(x18), .b(new_n84_), .c(new_n85_), .O(new_n86_));
  aoi21  g040(.a(new_n86_), .b(new_n83_), .c(new_n53_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n82_), .O(z09));
  nand2  g042(.a(x18), .b(x01), .O(new_n89_));
  oai21  g043(.a(x18), .b(new_n52_), .c(new_n89_), .O(new_n90_));
  nand2  g044(.a(new_n90_), .b(new_n83_), .O(new_n91_));
  nor2   g045(.a(x20), .b(x17), .O(new_n92_));
  inv1   g046(.a(new_n92_), .O(new_n93_));
  oai21  g047(.a(new_n93_), .b(new_n83_), .c(x18), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n49_), .O(new_n95_));
  inv1   g049(.a(new_n81_), .O(new_n96_));
  nand3  g050(.a(new_n96_), .b(x20), .c(x16), .O(new_n97_));
  nand3  g051(.a(new_n97_), .b(new_n95_), .c(new_n91_), .O(z10));
  nand2  g052(.a(x18), .b(x02), .O(new_n99_));
  nand2  g053(.a(x19), .b(new_n48_), .O(new_n100_));
  oai21  g054(.a(new_n100_), .b(new_n56_), .c(new_n99_), .O(new_n101_));
  nand2  g055(.a(new_n101_), .b(new_n83_), .O(new_n102_));
  nand2  g056(.a(new_n93_), .b(x21), .O(new_n103_));
  nor2   g057(.a(x21), .b(x20), .O(new_n104_));
  nand2  g058(.a(new_n104_), .b(new_n81_), .O(new_n105_));
  aoi21  g059(.a(new_n105_), .b(new_n103_), .c(new_n48_), .O(new_n106_));
  nor2   g060(.a(new_n58_), .b(new_n49_), .O(new_n107_));
  oai21  g061(.a(new_n107_), .b(new_n106_), .c(x16), .O(new_n108_));
  nand2  g062(.a(new_n108_), .b(new_n102_), .O(z11));
  nand2  g063(.a(x18), .b(x03), .O(new_n110_));
  oai21  g064(.a(new_n100_), .b(new_n61_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n83_), .O(new_n112_));
  inv1   g066(.a(x20), .O(new_n113_));
  nand3  g067(.a(new_n58_), .b(new_n113_), .c(new_n79_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(x22), .O(new_n115_));
  nor2   g069(.a(x22), .b(x21), .O(new_n116_));
  nand3  g070(.a(new_n116_), .b(new_n81_), .c(new_n113_), .O(new_n117_));
  aoi21  g071(.a(new_n117_), .b(new_n115_), .c(new_n48_), .O(new_n118_));
  inv1   g072(.a(x22), .O(new_n119_));
  nor2   g073(.a(new_n119_), .b(new_n49_), .O(new_n120_));
  oai21  g074(.a(new_n120_), .b(new_n118_), .c(x16), .O(new_n121_));
  nand2  g075(.a(new_n121_), .b(new_n112_), .O(z12));
  nand2  g076(.a(x18), .b(x04), .O(new_n123_));
  oai21  g077(.a(new_n100_), .b(new_n64_), .c(new_n123_), .O(new_n124_));
  nand2  g078(.a(new_n124_), .b(new_n83_), .O(new_n125_));
  nand2  g079(.a(new_n116_), .b(new_n92_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(x23), .O(new_n127_));
  nor3   g081(.a(x20), .b(x19), .c(x17), .O(new_n128_));
  nor2   g082(.a(x23), .b(x22), .O(new_n129_));
  nand3  g083(.a(new_n129_), .b(new_n128_), .c(new_n58_), .O(new_n130_));
  aoi21  g084(.a(new_n130_), .b(new_n127_), .c(new_n48_), .O(new_n131_));
  inv1   g085(.a(x23), .O(new_n132_));
  nor2   g086(.a(new_n132_), .b(new_n49_), .O(new_n133_));
  oai21  g087(.a(new_n133_), .b(new_n131_), .c(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n125_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(new_n100_), .b(new_n67_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n83_), .O(new_n138_));
  nand3  g092(.a(new_n129_), .b(new_n92_), .c(new_n58_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(x24), .O(new_n140_));
  nand4  g094(.a(new_n128_), .b(new_n116_), .c(new_n69_), .d(new_n132_), .O(new_n141_));
  aoi21  g095(.a(new_n141_), .b(new_n140_), .c(new_n48_), .O(new_n142_));
  nor2   g096(.a(new_n69_), .b(new_n49_), .O(new_n143_));
  oai21  g097(.a(new_n143_), .b(new_n142_), .c(x16), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n138_), .O(z14));
  nand2  g099(.a(x18), .b(x06), .O(new_n146_));
  oai21  g100(.a(new_n100_), .b(new_n72_), .c(new_n146_), .O(new_n147_));
  nand2  g101(.a(new_n147_), .b(new_n83_), .O(new_n148_));
  nand3  g102(.a(new_n69_), .b(new_n132_), .c(new_n119_), .O(new_n149_));
  oai21  g103(.a(new_n149_), .b(new_n114_), .c(x25), .O(new_n150_));
  nor2   g104(.a(x25), .b(x24), .O(new_n151_));
  nand4  g105(.a(new_n151_), .b(new_n129_), .c(new_n104_), .d(new_n81_), .O(new_n152_));
  aoi21  g106(.a(new_n152_), .b(new_n150_), .c(new_n48_), .O(new_n153_));
  and2   g107(.a(x25), .b(x19), .O(new_n154_));
  oai21  g108(.a(new_n154_), .b(new_n153_), .c(x16), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(new_n148_), .O(z15));
  nand2  g110(.a(x18), .b(x07), .O(new_n157_));
  oai21  g111(.a(new_n100_), .b(new_n75_), .c(new_n157_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n83_), .O(new_n159_));
  nand4  g113(.a(new_n151_), .b(new_n129_), .c(new_n104_), .d(new_n79_), .O(new_n160_));
  nand2  g114(.a(new_n160_), .b(x26), .O(new_n161_));
  nor3   g115(.a(x26), .b(x25), .c(x24), .O(new_n162_));
  nand4  g116(.a(new_n162_), .b(new_n129_), .c(new_n104_), .d(new_n81_), .O(new_n163_));
  aoi21  g117(.a(new_n163_), .b(new_n161_), .c(new_n48_), .O(new_n164_));
  and2   g118(.a(x26), .b(x19), .O(new_n165_));
  oai21  g119(.a(new_n165_), .b(new_n164_), .c(x16), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(new_n159_), .O(z16));
  nand2  g121(.a(z08), .b(x17), .O(new_n168_));
  nor2   g122(.a(new_n49_), .b(x17), .O(new_n169_));
  nand4  g123(.a(new_n169_), .b(new_n162_), .c(new_n129_), .d(new_n104_), .O(new_n170_));
  aoi21  g124(.a(new_n170_), .b(new_n168_), .c(new_n83_), .O(z17));
endmodule


