// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n166_, new_n167_, new_n168_, new_n169_;
  inv1   g000(.a(x11), .O(new_n47_));
  inv1   g001(.a(x18), .O(new_n48_));
  nand2  g002(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  inv1   g003(.a(new_n49_), .O(new_n50_));
  inv1   g004(.a(x08), .O(new_n51_));
  nand2  g005(.a(x27), .b(new_n51_), .O(new_n52_));
  inv1   g006(.a(x19), .O(new_n53_));
  inv1   g007(.a(x27), .O(new_n54_));
  nand2  g008(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  aoi21  g009(.a(new_n55_), .b(new_n52_), .c(new_n50_), .O(z00));
  inv1   g010(.a(x09), .O(new_n57_));
  nand2  g011(.a(x27), .b(new_n57_), .O(new_n58_));
  inv1   g012(.a(x20), .O(new_n59_));
  nand2  g013(.a(new_n54_), .b(new_n59_), .O(new_n60_));
  aoi21  g014(.a(new_n60_), .b(new_n58_), .c(new_n50_), .O(z01));
  inv1   g015(.a(x10), .O(new_n62_));
  aoi21  g016(.a(x27), .b(new_n62_), .c(new_n50_), .O(new_n63_));
  oai21  g017(.a(x27), .b(x21), .c(new_n63_), .O(z02));
  oai21  g018(.a(x27), .b(new_n48_), .c(new_n47_), .O(new_n65_));
  oai21  g019(.a(x27), .b(x22), .c(new_n65_), .O(z03));
  inv1   g020(.a(x12), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x23), .O(new_n69_));
  nand2  g023(.a(new_n54_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n50_), .O(z04));
  inv1   g025(.a(x13), .O(new_n72_));
  nand2  g026(.a(x27), .b(new_n72_), .O(new_n73_));
  inv1   g027(.a(x24), .O(new_n74_));
  nand2  g028(.a(new_n54_), .b(new_n74_), .O(new_n75_));
  aoi21  g029(.a(new_n75_), .b(new_n73_), .c(new_n50_), .O(z05));
  inv1   g030(.a(x14), .O(new_n77_));
  nand2  g031(.a(x27), .b(new_n77_), .O(new_n78_));
  inv1   g032(.a(x25), .O(new_n79_));
  nand2  g033(.a(new_n54_), .b(new_n79_), .O(new_n80_));
  aoi21  g034(.a(new_n80_), .b(new_n78_), .c(new_n50_), .O(z06));
  inv1   g035(.a(x15), .O(new_n82_));
  nand2  g036(.a(x27), .b(new_n82_), .O(new_n83_));
  inv1   g037(.a(x26), .O(new_n84_));
  nand2  g038(.a(new_n54_), .b(new_n84_), .O(new_n85_));
  aoi21  g039(.a(new_n85_), .b(new_n83_), .c(new_n50_), .O(z07));
  nand2  g040(.a(new_n49_), .b(new_n54_), .O(z08));
  inv1   g041(.a(x17), .O(new_n88_));
  nor2   g042(.a(new_n53_), .b(new_n88_), .O(new_n89_));
  nor2   g043(.a(x19), .b(x17), .O(new_n90_));
  oai21  g044(.a(new_n90_), .b(new_n89_), .c(x16), .O(new_n91_));
  inv1   g045(.a(x16), .O(new_n92_));
  nand2  g046(.a(x18), .b(x00), .O(new_n93_));
  oai21  g047(.a(x18), .b(new_n51_), .c(new_n93_), .O(new_n94_));
  nand2  g048(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand3  g049(.a(new_n95_), .b(new_n91_), .c(new_n49_), .O(z09));
  nand2  g050(.a(x18), .b(x01), .O(new_n97_));
  nand2  g051(.a(new_n48_), .b(x11), .O(new_n98_));
  oai21  g052(.a(new_n98_), .b(new_n57_), .c(new_n97_), .O(new_n99_));
  nand2  g053(.a(new_n99_), .b(new_n92_), .O(new_n100_));
  nand3  g054(.a(new_n59_), .b(new_n53_), .c(new_n88_), .O(new_n101_));
  oai21  g055(.a(new_n90_), .b(new_n59_), .c(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n49_), .c(x16), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n100_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n62_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n92_), .O(new_n107_));
  inv1   g061(.a(x21), .O(new_n108_));
  nor3   g062(.a(x20), .b(x19), .c(x17), .O(new_n109_));
  nor2   g063(.a(x21), .b(x20), .O(new_n110_));
  nand2  g064(.a(new_n110_), .b(new_n90_), .O(new_n111_));
  oai21  g065(.a(new_n109_), .b(new_n108_), .c(new_n111_), .O(new_n112_));
  nand2  g066(.a(new_n112_), .b(x16), .O(new_n113_));
  nand3  g067(.a(new_n113_), .b(new_n107_), .c(new_n49_), .O(z11));
  nand2  g068(.a(x18), .b(x03), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n98_), .O(new_n116_));
  nand2  g070(.a(new_n116_), .b(new_n92_), .O(new_n117_));
  oai21  g071(.a(new_n48_), .b(new_n92_), .c(new_n98_), .O(new_n118_));
  nand3  g072(.a(new_n118_), .b(new_n111_), .c(x22), .O(new_n119_));
  inv1   g073(.a(x22), .O(new_n120_));
  nand4  g074(.a(new_n49_), .b(new_n120_), .c(new_n108_), .d(new_n59_), .O(new_n121_));
  inv1   g075(.a(new_n121_), .O(new_n122_));
  nand4  g076(.a(new_n122_), .b(new_n53_), .c(new_n88_), .d(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n119_), .c(new_n117_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n67_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n92_), .O(new_n127_));
  nor2   g081(.a(x22), .b(x21), .O(new_n128_));
  nand3  g082(.a(new_n128_), .b(new_n90_), .c(new_n59_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x23), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n109_), .c(new_n108_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(x16), .O(new_n134_));
  nand3  g088(.a(new_n134_), .b(new_n127_), .c(new_n49_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  nand3  g090(.a(new_n48_), .b(x13), .c(x11), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand2  g092(.a(new_n138_), .b(new_n92_), .O(new_n139_));
  nand3  g093(.a(new_n69_), .b(new_n120_), .c(new_n108_), .O(new_n140_));
  oai21  g094(.a(new_n140_), .b(new_n101_), .c(x24), .O(new_n141_));
  nor2   g095(.a(x20), .b(x19), .O(new_n142_));
  nor2   g096(.a(x24), .b(x23), .O(new_n143_));
  nand4  g097(.a(new_n143_), .b(new_n128_), .c(new_n142_), .d(new_n88_), .O(new_n144_));
  nand2  g098(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand3  g099(.a(new_n145_), .b(new_n49_), .c(x16), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n139_), .O(z14));
  nand2  g101(.a(x18), .b(x06), .O(new_n148_));
  oai21  g102(.a(x18), .b(new_n77_), .c(new_n148_), .O(new_n149_));
  nand2  g103(.a(new_n149_), .b(new_n92_), .O(new_n150_));
  nand2  g104(.a(new_n144_), .b(x25), .O(new_n151_));
  nor2   g105(.a(x25), .b(x24), .O(new_n152_));
  nand4  g106(.a(new_n152_), .b(new_n131_), .c(new_n110_), .d(new_n90_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand2  g108(.a(new_n154_), .b(x16), .O(new_n155_));
  nand3  g109(.a(new_n155_), .b(new_n150_), .c(new_n49_), .O(z15));
  nand2  g110(.a(x18), .b(x07), .O(new_n157_));
  oai21  g111(.a(x18), .b(new_n82_), .c(new_n157_), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n92_), .O(new_n159_));
  nand2  g113(.a(new_n153_), .b(x26), .O(new_n160_));
  nor3   g114(.a(x26), .b(x25), .c(x24), .O(new_n161_));
  nand4  g115(.a(new_n161_), .b(new_n131_), .c(new_n110_), .d(new_n90_), .O(new_n162_));
  nand2  g116(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand2  g117(.a(new_n163_), .b(x16), .O(new_n164_));
  nand3  g118(.a(new_n164_), .b(new_n159_), .c(new_n49_), .O(z16));
  nand4  g119(.a(new_n131_), .b(new_n84_), .c(new_n79_), .d(new_n74_), .O(new_n166_));
  nand3  g120(.a(new_n110_), .b(x19), .c(new_n88_), .O(new_n167_));
  oai22  g121(.a(new_n167_), .b(new_n166_), .c(new_n54_), .d(new_n88_), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n49_), .c(x16), .O(new_n169_));
  inv1   g123(.a(new_n169_), .O(z17));
endmodule


