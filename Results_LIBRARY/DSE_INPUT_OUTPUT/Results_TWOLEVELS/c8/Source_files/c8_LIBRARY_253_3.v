// Benchmark "FAU" written by ABC on Tue Aug 18 15:55:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n144_, new_n145_,
    new_n146_, new_n147_, new_n148_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n170_, new_n171_,
    new_n172_;
  nand2  g000(.a(x25), .b(x18), .O(new_n47_));
  inv1   g001(.a(new_n47_), .O(new_n48_));
  inv1   g002(.a(x08), .O(new_n49_));
  nand2  g003(.a(x27), .b(new_n49_), .O(new_n50_));
  inv1   g004(.a(x19), .O(new_n51_));
  inv1   g005(.a(x27), .O(new_n52_));
  nand2  g006(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  aoi21  g007(.a(new_n53_), .b(new_n50_), .c(new_n48_), .O(z00));
  inv1   g008(.a(x09), .O(new_n55_));
  nand2  g009(.a(x27), .b(new_n55_), .O(new_n56_));
  inv1   g010(.a(x20), .O(new_n57_));
  nand2  g011(.a(new_n52_), .b(new_n57_), .O(new_n58_));
  aoi21  g012(.a(new_n58_), .b(new_n56_), .c(new_n48_), .O(z01));
  inv1   g013(.a(x10), .O(new_n60_));
  aoi21  g014(.a(x27), .b(new_n60_), .c(new_n48_), .O(new_n61_));
  oai21  g015(.a(x27), .b(x21), .c(new_n61_), .O(z02));
  inv1   g016(.a(x11), .O(new_n63_));
  nand2  g017(.a(x27), .b(new_n63_), .O(new_n64_));
  inv1   g018(.a(x22), .O(new_n65_));
  nand2  g019(.a(new_n52_), .b(new_n65_), .O(new_n66_));
  aoi21  g020(.a(new_n66_), .b(new_n64_), .c(new_n48_), .O(z03));
  inv1   g021(.a(x12), .O(new_n68_));
  aoi21  g022(.a(x27), .b(new_n68_), .c(new_n48_), .O(new_n69_));
  oai21  g023(.a(x27), .b(x23), .c(new_n69_), .O(z04));
  inv1   g024(.a(x13), .O(new_n71_));
  nand2  g025(.a(x27), .b(new_n71_), .O(new_n72_));
  inv1   g026(.a(x24), .O(new_n73_));
  nand2  g027(.a(new_n52_), .b(new_n73_), .O(new_n74_));
  aoi21  g028(.a(new_n74_), .b(new_n72_), .c(new_n48_), .O(z05));
  inv1   g029(.a(x18), .O(new_n76_));
  inv1   g030(.a(x25), .O(new_n77_));
  aoi21  g031(.a(x27), .b(new_n76_), .c(new_n77_), .O(new_n78_));
  oai22  g032(.a(new_n78_), .b(x14), .c(x27), .d(x25), .O(z06));
  inv1   g033(.a(x15), .O(new_n80_));
  aoi21  g034(.a(x27), .b(new_n80_), .c(new_n48_), .O(new_n81_));
  oai21  g035(.a(x27), .b(x26), .c(new_n81_), .O(z07));
  nor2   g036(.a(new_n48_), .b(new_n52_), .O(z08));
  inv1   g037(.a(x16), .O(new_n84_));
  inv1   g038(.a(x00), .O(new_n85_));
  nand2  g039(.a(new_n77_), .b(x18), .O(new_n86_));
  oai22  g040(.a(new_n86_), .b(new_n85_), .c(x18), .d(new_n49_), .O(new_n87_));
  nand2  g041(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  inv1   g042(.a(x17), .O(new_n89_));
  nand3  g043(.a(new_n47_), .b(x19), .c(x17), .O(new_n90_));
  inv1   g044(.a(new_n90_), .O(new_n91_));
  oai21  g045(.a(x25), .b(new_n57_), .c(new_n76_), .O(new_n92_));
  aoi21  g046(.a(new_n92_), .b(x25), .c(x19), .O(new_n93_));
  aoi21  g047(.a(new_n93_), .b(new_n89_), .c(new_n91_), .O(new_n94_));
  oai21  g048(.a(new_n94_), .b(new_n84_), .c(new_n88_), .O(z09));
  inv1   g049(.a(x01), .O(new_n96_));
  oai22  g050(.a(new_n86_), .b(new_n96_), .c(x18), .d(new_n55_), .O(new_n97_));
  nand2  g051(.a(new_n97_), .b(new_n84_), .O(new_n98_));
  nor2   g052(.a(x19), .b(x17), .O(new_n99_));
  nor3   g053(.a(x20), .b(x19), .c(x17), .O(new_n100_));
  inv1   g054(.a(new_n100_), .O(new_n101_));
  oai21  g055(.a(new_n99_), .b(new_n57_), .c(new_n101_), .O(new_n102_));
  nand3  g056(.a(new_n102_), .b(new_n47_), .c(x16), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n98_), .O(z10));
  nand2  g058(.a(x18), .b(x02), .O(new_n105_));
  oai21  g059(.a(x18), .b(new_n60_), .c(new_n105_), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n84_), .O(new_n107_));
  inv1   g061(.a(x21), .O(new_n108_));
  nor2   g062(.a(x21), .b(x20), .O(new_n109_));
  nand2  g063(.a(new_n109_), .b(new_n99_), .O(new_n110_));
  oai21  g064(.a(new_n100_), .b(new_n108_), .c(new_n110_), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(x16), .O(new_n112_));
  nand3  g066(.a(new_n112_), .b(new_n107_), .c(new_n47_), .O(z11));
  inv1   g067(.a(x03), .O(new_n114_));
  oai22  g068(.a(new_n86_), .b(new_n114_), .c(x18), .d(new_n63_), .O(new_n115_));
  nand2  g069(.a(new_n115_), .b(new_n84_), .O(new_n116_));
  nand3  g070(.a(new_n110_), .b(new_n47_), .c(x22), .O(new_n117_));
  inv1   g071(.a(x23), .O(new_n118_));
  oai21  g072(.a(x25), .b(new_n118_), .c(new_n76_), .O(new_n119_));
  aoi21  g073(.a(new_n119_), .b(x25), .c(x22), .O(new_n120_));
  nand4  g074(.a(new_n120_), .b(new_n108_), .c(new_n57_), .d(new_n51_), .O(new_n121_));
  oai21  g075(.a(new_n121_), .b(x17), .c(new_n117_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand2  g077(.a(new_n123_), .b(new_n116_), .O(z12));
  inv1   g078(.a(x04), .O(new_n125_));
  oai22  g079(.a(new_n86_), .b(new_n125_), .c(x18), .d(new_n68_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n84_), .O(new_n127_));
  nor3   g081(.a(x22), .b(x21), .c(x20), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(new_n99_), .O(new_n129_));
  nand2  g083(.a(new_n129_), .b(x23), .O(new_n130_));
  nor2   g084(.a(x23), .b(x22), .O(new_n131_));
  nand3  g085(.a(new_n131_), .b(new_n100_), .c(new_n108_), .O(new_n132_));
  nand2  g086(.a(new_n132_), .b(new_n130_), .O(new_n133_));
  nand3  g087(.a(new_n133_), .b(new_n47_), .c(x16), .O(new_n134_));
  nand2  g088(.a(new_n134_), .b(new_n127_), .O(z13));
  nand2  g089(.a(x18), .b(x05), .O(new_n136_));
  oai21  g090(.a(x18), .b(new_n71_), .c(new_n136_), .O(new_n137_));
  nand2  g091(.a(new_n137_), .b(new_n84_), .O(new_n138_));
  and2   g092(.a(new_n132_), .b(x24), .O(new_n139_));
  nand4  g093(.a(new_n73_), .b(new_n118_), .c(new_n65_), .d(new_n108_), .O(new_n140_));
  nor2   g094(.a(new_n140_), .b(new_n101_), .O(new_n141_));
  oai21  g095(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  nand3  g096(.a(new_n142_), .b(new_n138_), .c(new_n47_), .O(z14));
  inv1   g097(.a(x14), .O(new_n144_));
  nand2  g098(.a(x18), .b(x06), .O(new_n145_));
  oai21  g099(.a(x18), .b(new_n144_), .c(new_n145_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(new_n84_), .O(new_n147_));
  nand4  g101(.a(x23), .b(new_n65_), .c(new_n108_), .d(new_n57_), .O(new_n148_));
  nand2  g102(.a(new_n148_), .b(new_n57_), .O(new_n149_));
  nand3  g103(.a(new_n149_), .b(x25), .c(new_n76_), .O(new_n150_));
  nand4  g104(.a(new_n128_), .b(new_n77_), .c(new_n73_), .d(new_n118_), .O(new_n151_));
  nand2  g105(.a(new_n151_), .b(new_n150_), .O(new_n152_));
  nand3  g106(.a(new_n152_), .b(new_n51_), .c(new_n89_), .O(new_n153_));
  nand2  g107(.a(x22), .b(new_n76_), .O(new_n154_));
  nand4  g108(.a(new_n154_), .b(new_n99_), .c(new_n73_), .d(new_n108_), .O(new_n155_));
  nand2  g109(.a(new_n155_), .b(x25), .O(new_n156_));
  nand2  g110(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g111(.a(new_n157_), .b(x16), .O(new_n158_));
  nand3  g112(.a(new_n158_), .b(new_n147_), .c(new_n47_), .O(z15));
  nand2  g113(.a(x18), .b(x07), .O(new_n160_));
  oai21  g114(.a(x18), .b(new_n80_), .c(new_n160_), .O(new_n161_));
  nand2  g115(.a(new_n161_), .b(new_n84_), .O(new_n162_));
  inv1   g116(.a(x26), .O(new_n163_));
  oai21  g117(.a(new_n163_), .b(new_n84_), .c(new_n76_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(x25), .O(new_n165_));
  nand4  g119(.a(new_n131_), .b(new_n163_), .c(new_n77_), .d(new_n73_), .O(new_n166_));
  oai22  g120(.a(new_n166_), .b(new_n110_), .c(new_n141_), .d(new_n163_), .O(new_n167_));
  nand2  g121(.a(new_n167_), .b(x16), .O(new_n168_));
  nand3  g122(.a(new_n168_), .b(new_n165_), .c(new_n162_), .O(z16));
  nand3  g123(.a(new_n109_), .b(x19), .c(new_n89_), .O(new_n170_));
  oai22  g124(.a(new_n170_), .b(new_n166_), .c(new_n52_), .d(new_n89_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(x16), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(new_n47_), .O(z17));
endmodule


