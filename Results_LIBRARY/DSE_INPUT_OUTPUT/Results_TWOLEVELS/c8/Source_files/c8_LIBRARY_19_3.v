// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n55_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n138_,
    new_n139_, new_n140_, new_n141_, new_n142_, new_n143_, new_n144_,
    new_n145_, new_n146_, new_n147_, new_n149_, new_n150_, new_n151_,
    new_n152_, new_n153_, new_n154_, new_n155_, new_n156_, new_n157_,
    new_n158_, new_n159_, new_n160_, new_n161_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n175_, new_n176_, new_n177_,
    new_n178_, new_n179_;
  inv1   g000(.a(x08), .O(new_n47_));
  nand2  g001(.a(x19), .b(x18), .O(new_n48_));
  inv1   g002(.a(new_n48_), .O(new_n49_));
  aoi21  g003(.a(x27), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  oai21  g004(.a(x27), .b(x19), .c(new_n50_), .O(z00));
  inv1   g005(.a(x09), .O(new_n52_));
  aoi21  g006(.a(x27), .b(new_n52_), .c(new_n49_), .O(new_n53_));
  oai21  g007(.a(x27), .b(x20), .c(new_n53_), .O(z01));
  inv1   g008(.a(x10), .O(new_n55_));
  aoi21  g009(.a(x27), .b(new_n55_), .c(new_n49_), .O(new_n56_));
  oai21  g010(.a(x27), .b(x21), .c(new_n56_), .O(z02));
  inv1   g011(.a(x11), .O(new_n58_));
  aoi21  g012(.a(x27), .b(new_n58_), .c(new_n49_), .O(new_n59_));
  oai21  g013(.a(x27), .b(x22), .c(new_n59_), .O(z03));
  inv1   g014(.a(x12), .O(new_n61_));
  nand2  g015(.a(x27), .b(new_n61_), .O(new_n62_));
  inv1   g016(.a(x23), .O(new_n63_));
  inv1   g017(.a(x27), .O(new_n64_));
  nand2  g018(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  aoi21  g019(.a(new_n65_), .b(new_n62_), .c(new_n49_), .O(z04));
  inv1   g020(.a(x13), .O(new_n67_));
  nand2  g021(.a(x27), .b(new_n67_), .O(new_n68_));
  inv1   g022(.a(x24), .O(new_n69_));
  nand2  g023(.a(new_n64_), .b(new_n69_), .O(new_n70_));
  aoi21  g024(.a(new_n70_), .b(new_n68_), .c(new_n49_), .O(z05));
  inv1   g025(.a(x14), .O(new_n72_));
  aoi21  g026(.a(x27), .b(new_n72_), .c(new_n49_), .O(new_n73_));
  oai21  g027(.a(x27), .b(x25), .c(new_n73_), .O(z06));
  inv1   g028(.a(x15), .O(new_n75_));
  nand2  g029(.a(x27), .b(new_n75_), .O(new_n76_));
  inv1   g030(.a(x26), .O(new_n77_));
  nand2  g031(.a(new_n64_), .b(new_n77_), .O(new_n78_));
  aoi21  g032(.a(new_n78_), .b(new_n76_), .c(new_n49_), .O(z07));
  nor2   g033(.a(new_n49_), .b(new_n64_), .O(z08));
  inv1   g034(.a(x17), .O(new_n81_));
  inv1   g035(.a(x19), .O(new_n82_));
  nor2   g036(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nor2   g037(.a(x19), .b(x17), .O(new_n84_));
  oai21  g038(.a(new_n84_), .b(new_n83_), .c(x16), .O(new_n85_));
  inv1   g039(.a(x16), .O(new_n86_));
  nand2  g040(.a(x18), .b(x00), .O(new_n87_));
  oai21  g041(.a(x18), .b(new_n47_), .c(new_n87_), .O(new_n88_));
  nand2  g042(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand3  g043(.a(new_n89_), .b(new_n85_), .c(new_n48_), .O(z09));
  nand2  g044(.a(x18), .b(x01), .O(new_n91_));
  oai21  g045(.a(x18), .b(new_n52_), .c(new_n91_), .O(new_n92_));
  nand2  g046(.a(new_n92_), .b(new_n86_), .O(new_n93_));
  inv1   g047(.a(x18), .O(new_n94_));
  inv1   g048(.a(x20), .O(new_n95_));
  oai21  g049(.a(new_n95_), .b(new_n86_), .c(new_n94_), .O(new_n96_));
  nand2  g050(.a(new_n96_), .b(x19), .O(new_n97_));
  nor2   g051(.a(new_n95_), .b(new_n81_), .O(new_n98_));
  nor3   g052(.a(x20), .b(x19), .c(x17), .O(new_n99_));
  oai21  g053(.a(new_n99_), .b(new_n98_), .c(x16), .O(new_n100_));
  nand3  g054(.a(new_n100_), .b(new_n97_), .c(new_n93_), .O(z10));
  nand3  g055(.a(new_n82_), .b(x18), .c(x02), .O(new_n102_));
  oai21  g056(.a(x18), .b(new_n55_), .c(new_n102_), .O(new_n103_));
  nand2  g057(.a(new_n103_), .b(new_n86_), .O(new_n104_));
  oai21  g058(.a(x20), .b(x17), .c(x21), .O(new_n105_));
  nor2   g059(.a(x21), .b(x20), .O(new_n106_));
  nand2  g060(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  aoi21  g061(.a(new_n107_), .b(new_n105_), .c(x19), .O(new_n108_));
  inv1   g062(.a(x21), .O(new_n109_));
  nor3   g063(.a(new_n109_), .b(new_n82_), .c(x18), .O(new_n110_));
  oai21  g064(.a(new_n110_), .b(new_n108_), .c(x16), .O(new_n111_));
  nand2  g065(.a(new_n111_), .b(new_n104_), .O(z11));
  nand2  g066(.a(x18), .b(x03), .O(new_n113_));
  oai21  g067(.a(x18), .b(new_n58_), .c(new_n113_), .O(new_n114_));
  nand2  g068(.a(new_n114_), .b(new_n86_), .O(new_n115_));
  inv1   g069(.a(x22), .O(new_n116_));
  oai21  g070(.a(new_n116_), .b(new_n86_), .c(new_n94_), .O(new_n117_));
  nand2  g071(.a(new_n117_), .b(x19), .O(new_n118_));
  nor3   g072(.a(x21), .b(x20), .c(x17), .O(new_n119_));
  nor2   g073(.a(x22), .b(x21), .O(new_n120_));
  nand3  g074(.a(new_n120_), .b(new_n84_), .c(new_n95_), .O(new_n121_));
  oai21  g075(.a(new_n119_), .b(new_n116_), .c(new_n121_), .O(new_n122_));
  nand2  g076(.a(new_n122_), .b(x16), .O(new_n123_));
  nand3  g077(.a(new_n123_), .b(new_n118_), .c(new_n115_), .O(z12));
  nand2  g078(.a(x18), .b(x04), .O(new_n125_));
  oai21  g079(.a(x18), .b(new_n61_), .c(new_n125_), .O(new_n126_));
  nand2  g080(.a(new_n126_), .b(new_n86_), .O(new_n127_));
  oai21  g081(.a(new_n63_), .b(new_n86_), .c(new_n94_), .O(new_n128_));
  nand2  g082(.a(new_n128_), .b(x19), .O(new_n129_));
  nor2   g083(.a(x20), .b(x17), .O(new_n130_));
  aoi21  g084(.a(new_n120_), .b(new_n130_), .c(new_n63_), .O(new_n131_));
  nand3  g085(.a(new_n95_), .b(new_n82_), .c(new_n81_), .O(new_n132_));
  nor2   g086(.a(x23), .b(x22), .O(new_n133_));
  nand2  g087(.a(new_n133_), .b(new_n109_), .O(new_n134_));
  nor2   g088(.a(new_n134_), .b(new_n132_), .O(new_n135_));
  oai21  g089(.a(new_n135_), .b(new_n131_), .c(x16), .O(new_n136_));
  nand3  g090(.a(new_n136_), .b(new_n129_), .c(new_n127_), .O(z13));
  nand2  g091(.a(x18), .b(x05), .O(new_n138_));
  oai21  g092(.a(x18), .b(new_n67_), .c(new_n138_), .O(new_n139_));
  nand2  g093(.a(new_n139_), .b(new_n86_), .O(new_n140_));
  oai21  g094(.a(new_n69_), .b(new_n86_), .c(new_n94_), .O(new_n141_));
  nand2  g095(.a(new_n141_), .b(x19), .O(new_n142_));
  nand3  g096(.a(new_n133_), .b(new_n130_), .c(new_n109_), .O(new_n143_));
  nand2  g097(.a(new_n143_), .b(x24), .O(new_n144_));
  nand4  g098(.a(new_n120_), .b(new_n99_), .c(new_n69_), .d(new_n63_), .O(new_n145_));
  nand2  g099(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g100(.a(new_n146_), .b(x16), .O(new_n147_));
  nand3  g101(.a(new_n147_), .b(new_n142_), .c(new_n140_), .O(z14));
  nand2  g102(.a(x18), .b(x06), .O(new_n149_));
  oai21  g103(.a(x18), .b(new_n72_), .c(new_n149_), .O(new_n150_));
  nand2  g104(.a(new_n150_), .b(new_n86_), .O(new_n151_));
  inv1   g105(.a(x25), .O(new_n152_));
  oai21  g106(.a(new_n152_), .b(new_n86_), .c(new_n94_), .O(new_n153_));
  nand2  g107(.a(new_n153_), .b(x19), .O(new_n154_));
  nor3   g108(.a(x24), .b(x23), .c(x22), .O(new_n155_));
  aoi21  g109(.a(new_n155_), .b(new_n119_), .c(new_n152_), .O(new_n156_));
  nand2  g110(.a(new_n106_), .b(new_n84_), .O(new_n157_));
  nor2   g111(.a(x25), .b(x24), .O(new_n158_));
  nand2  g112(.a(new_n158_), .b(new_n133_), .O(new_n159_));
  nor2   g113(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  oai21  g114(.a(new_n160_), .b(new_n156_), .c(x16), .O(new_n161_));
  nand3  g115(.a(new_n161_), .b(new_n154_), .c(new_n151_), .O(z15));
  nand2  g116(.a(x18), .b(x07), .O(new_n163_));
  oai21  g117(.a(x18), .b(new_n75_), .c(new_n163_), .O(new_n164_));
  nand2  g118(.a(new_n164_), .b(new_n86_), .O(new_n165_));
  oai21  g119(.a(new_n77_), .b(new_n86_), .c(new_n94_), .O(new_n166_));
  nand2  g120(.a(new_n166_), .b(x19), .O(new_n167_));
  nand4  g121(.a(new_n158_), .b(new_n133_), .c(new_n106_), .d(new_n81_), .O(new_n168_));
  nand2  g122(.a(new_n168_), .b(x26), .O(new_n169_));
  nor3   g123(.a(x26), .b(x25), .c(x24), .O(new_n170_));
  nand4  g124(.a(new_n170_), .b(new_n133_), .c(new_n106_), .d(new_n84_), .O(new_n171_));
  nand2  g125(.a(new_n171_), .b(new_n169_), .O(new_n172_));
  nand2  g126(.a(new_n172_), .b(x16), .O(new_n173_));
  nand3  g127(.a(new_n173_), .b(new_n167_), .c(new_n165_), .O(z16));
  nand2  g128(.a(z08), .b(x17), .O(new_n175_));
  nor2   g129(.a(x18), .b(x17), .O(new_n176_));
  nand2  g130(.a(new_n106_), .b(x19), .O(new_n177_));
  inv1   g131(.a(new_n177_), .O(new_n178_));
  nand4  g132(.a(new_n178_), .b(new_n176_), .c(new_n170_), .d(new_n133_), .O(new_n179_));
  aoi21  g133(.a(new_n179_), .b(new_n175_), .c(new_n86_), .O(z17));
endmodule


