// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(x8), .c(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  oai21  g03(.a(new_n18_), .b(x3), .c(x9), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g05(.a(x8), .b(x3), .c(x9), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n19_), .O(z0));
  inv1   g07(.a(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x7), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n23_), .c(new_n19_), .O(z1));
  nor3   g10(.a(x9), .b(x8), .c(x7), .O(z2));
  oai21  g11(.a(x8), .b(x3), .c(x9), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(x7), .O(new_n30_));
  oai21  g13(.a(x9), .b(new_n20_), .c(new_n18_), .O(new_n31_));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  inv1   g18(.a(new_n35_), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n31_), .c(new_n30_), .d(new_n23_), .O(z3));
  nand2  g20(.a(x9), .b(x3), .O(new_n38_));
  nand2  g21(.a(new_n26_), .b(x8), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(new_n38_), .c(new_n31_), .d(x6), .O(z4));
  inv1   g23(.a(x3), .O(new_n41_));
  nand3  g24(.a(x9), .b(x8), .c(x7), .O(new_n42_));
  oai21  g25(.a(x8), .b(x7), .c(new_n42_), .O(new_n43_));
  nand4  g26(.a(new_n43_), .b(x2), .c(new_n33_), .d(new_n32_), .O(new_n44_));
  inv1   g27(.a(x6), .O(new_n45_));
  nor2   g28(.a(new_n25_), .b(new_n20_), .O(new_n46_));
  inv1   g29(.a(x5), .O(new_n47_));
  nor2   g30(.a(x7), .b(new_n47_), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n44_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n41_), .O(new_n51_));
  inv1   g34(.a(x4), .O(new_n52_));
  nand3  g35(.a(x7), .b(new_n52_), .c(x3), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(x8), .c(x6), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n51_), .O(z5));
  nand2  g39(.a(new_n38_), .b(new_n45_), .O(new_n57_));
  nor2   g40(.a(x9), .b(x8), .O(new_n58_));
  nand3  g41(.a(new_n46_), .b(x5), .c(new_n41_), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(new_n18_), .O(new_n61_));
  nand2  g44(.a(new_n35_), .b(x8), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(x9), .c(new_n41_), .O(new_n63_));
  nand4  g46(.a(new_n25_), .b(x8), .c(x4), .d(x3), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(x7), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n61_), .c(new_n57_), .O(z6));
endmodule


