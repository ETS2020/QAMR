// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  nand2  g00(.a(x9), .b(x7), .O(new_n18_));
  oai21  g01(.a(x7), .b(x0), .c(new_n18_), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x8), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nor2   g05(.a(x9), .b(new_n22_), .O(new_n23_));
  nor2   g06(.a(new_n23_), .b(x7), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n21_), .O(new_n25_));
  nand2  g08(.a(new_n22_), .b(x7), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n20_), .O(z0));
  inv1   g10(.a(x7), .O(new_n28_));
  inv1   g11(.a(x9), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n22_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(x0), .c(new_n28_), .O(new_n31_));
  oai21  g14(.a(new_n29_), .b(x8), .c(x7), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(z1));
  aoi21  g16(.a(new_n30_), .b(new_n21_), .c(x7), .O(z2));
  nor2   g17(.a(x2), .b(x1), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n23_), .c(new_n28_), .d(new_n21_), .O(z3));
  inv1   g19(.a(x6), .O(new_n37_));
  aoi21  g20(.a(x9), .b(x8), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(new_n28_), .b(new_n21_), .O(new_n39_));
  oai21  g22(.a(new_n38_), .b(new_n28_), .c(new_n39_), .O(z4));
  inv1   g23(.a(x1), .O(new_n41_));
  nand2  g24(.a(x2), .b(new_n41_), .O(new_n42_));
  nand3  g25(.a(x9), .b(x8), .c(x7), .O(new_n43_));
  nand3  g26(.a(new_n29_), .b(new_n22_), .c(new_n28_), .O(new_n44_));
  oai21  g27(.a(new_n43_), .b(new_n42_), .c(new_n44_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n21_), .O(new_n46_));
  inv1   g29(.a(x3), .O(new_n47_));
  oai21  g30(.a(x4), .b(new_n47_), .c(x8), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n29_), .c(x7), .O(new_n49_));
  nand3  g32(.a(new_n22_), .b(x2), .c(new_n41_), .O(new_n50_));
  nand3  g33(.a(x9), .b(x8), .c(x5), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n21_), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n28_), .c(new_n37_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n49_), .c(new_n46_), .O(z5));
  nand3  g37(.a(new_n35_), .b(x9), .c(x7), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n44_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand2  g40(.a(x9), .b(new_n22_), .O(new_n58_));
  nand4  g41(.a(new_n29_), .b(x8), .c(x4), .d(x3), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x7), .O(new_n61_));
  nand2  g44(.a(new_n51_), .b(new_n21_), .O(new_n62_));
  aoi21  g45(.a(new_n62_), .b(new_n28_), .c(new_n37_), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n61_), .c(new_n57_), .O(z6));
endmodule


