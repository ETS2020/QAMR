// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x3), .O(new_n19_));
  nand2  g02(.a(x9), .b(x7), .O(new_n20_));
  oai21  g03(.a(x9), .b(new_n19_), .c(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  inv1   g06(.a(x9), .O(new_n24_));
  oai21  g07(.a(new_n18_), .b(new_n19_), .c(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g09(.a(x9), .b(x8), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(z0));
  nand2  g11(.a(x8), .b(new_n23_), .O(new_n29_));
  nand2  g12(.a(new_n24_), .b(new_n18_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(new_n23_), .c(new_n29_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x3), .O(new_n32_));
  inv1   g15(.a(x4), .O(new_n33_));
  oai21  g16(.a(x8), .b(new_n33_), .c(x7), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x3), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n24_), .O(new_n36_));
  oai21  g19(.a(x8), .b(new_n23_), .c(x9), .O(new_n37_));
  nand3  g20(.a(new_n37_), .b(new_n36_), .c(new_n32_), .O(z1));
  nand4  g21(.a(new_n24_), .b(new_n18_), .c(new_n23_), .d(x3), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(z2));
  nand3  g23(.a(new_n34_), .b(x8), .c(x3), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n24_), .O(new_n42_));
  nor2   g25(.a(x8), .b(x7), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x7), .O(new_n44_));
  inv1   g27(.a(x0), .O(new_n45_));
  inv1   g28(.a(x1), .O(new_n46_));
  inv1   g29(.a(x2), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  aoi21  g31(.a(new_n44_), .b(x9), .c(new_n48_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n42_), .O(z3));
  aoi21  g33(.a(x9), .b(new_n18_), .c(new_n19_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(x9), .c(new_n23_), .O(new_n52_));
  nand2  g35(.a(new_n24_), .b(new_n19_), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n52_), .c(new_n27_), .d(x6), .O(z4));
  nand2  g37(.a(new_n18_), .b(new_n23_), .O(new_n55_));
  oai21  g38(.a(new_n27_), .b(new_n23_), .c(new_n55_), .O(new_n56_));
  nand4  g39(.a(new_n56_), .b(x2), .c(new_n46_), .d(new_n45_), .O(new_n57_));
  nand2  g40(.a(x7), .b(new_n33_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(x8), .c(x3), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n24_), .O(new_n60_));
  inv1   g43(.a(x6), .O(new_n61_));
  inv1   g44(.a(new_n27_), .O(new_n62_));
  inv1   g45(.a(x5), .O(new_n63_));
  nor2   g46(.a(x7), .b(new_n63_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n60_), .c(new_n57_), .O(z5));
  oai22  g49(.a(new_n30_), .b(new_n19_), .c(new_n27_), .d(new_n63_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n23_), .O(new_n68_));
  aoi21  g51(.a(new_n48_), .b(x8), .c(new_n24_), .O(new_n69_));
  nand3  g52(.a(new_n24_), .b(x8), .c(x4), .O(new_n70_));
  inv1   g53(.a(new_n70_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n69_), .c(x7), .O(new_n72_));
  aoi21  g55(.a(new_n24_), .b(new_n19_), .c(new_n61_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n72_), .c(new_n68_), .O(z6));
endmodule


