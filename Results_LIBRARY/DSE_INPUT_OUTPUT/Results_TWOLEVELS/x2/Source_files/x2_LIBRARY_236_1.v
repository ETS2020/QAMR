// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:10 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x8), .O(new_n19_));
  nor2   g02(.a(x9), .b(x8), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n19_), .O(z0));
  inv1   g05(.a(x8), .O(new_n23_));
  oai21  g06(.a(x9), .b(new_n18_), .c(new_n23_), .O(z1));
  nor3   g07(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g08(.a(x9), .O(new_n26_));
  oai21  g09(.a(new_n23_), .b(x7), .c(new_n26_), .O(new_n27_));
  inv1   g10(.a(x2), .O(new_n28_));
  nor2   g11(.a(x1), .b(x0), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n26_), .c(new_n28_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x8), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n27_), .O(z3));
  oai21  g15(.a(new_n26_), .b(x8), .c(new_n18_), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n26_), .c(x6), .O(z4));
  nand3  g17(.a(new_n29_), .b(x9), .c(x2), .O(new_n35_));
  inv1   g18(.a(x4), .O(new_n36_));
  nand3  g19(.a(new_n26_), .b(new_n36_), .c(x3), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x7), .O(new_n39_));
  nand3  g22(.a(x9), .b(new_n18_), .c(x5), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(x8), .d(x6), .O(z5));
  inv1   g24(.a(x6), .O(new_n42_));
  oai21  g25(.a(new_n26_), .b(x8), .c(new_n42_), .O(new_n43_));
  inv1   g26(.a(new_n40_), .O(new_n44_));
  inv1   g27(.a(x0), .O(new_n45_));
  inv1   g28(.a(x1), .O(new_n46_));
  nand4  g29(.a(x9), .b(new_n28_), .c(new_n46_), .d(new_n45_), .O(new_n47_));
  nand3  g30(.a(new_n26_), .b(x4), .c(x3), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n47_), .c(new_n18_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n44_), .c(x8), .O(new_n50_));
  nand2  g33(.a(new_n20_), .b(new_n18_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(new_n43_), .O(z6));
endmodule


