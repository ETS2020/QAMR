// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  nor2   g02(.a(x9), .b(new_n19_), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n18_), .c(x8), .O(z0));
  inv1   g04(.a(new_n20_), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n18_), .c(x8), .O(new_n23_));
  inv1   g06(.a(x9), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(x8), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(new_n19_), .c(new_n20_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n23_), .O(z1));
  nand2  g10(.a(x8), .b(new_n18_), .O(new_n28_));
  nor2   g11(.a(x9), .b(x8), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n19_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n28_), .O(z2));
  inv1   g14(.a(x8), .O(new_n32_));
  oai21  g15(.a(new_n24_), .b(new_n32_), .c(x7), .O(new_n33_));
  nand2  g16(.a(new_n32_), .b(new_n19_), .O(new_n34_));
  oai21  g17(.a(x9), .b(new_n18_), .c(x8), .O(new_n35_));
  nor3   g18(.a(x2), .b(x1), .c(x0), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n35_), .c(new_n34_), .d(new_n33_), .O(z3));
  inv1   g20(.a(x6), .O(new_n38_));
  nand2  g21(.a(new_n28_), .b(new_n38_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n34_), .c(new_n23_), .O(z4));
  inv1   g23(.a(x0), .O(new_n41_));
  inv1   g24(.a(x1), .O(new_n42_));
  nand2  g25(.a(x9), .b(x8), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n19_), .c(new_n34_), .O(new_n44_));
  nand4  g27(.a(new_n44_), .b(x2), .c(new_n42_), .d(new_n41_), .O(new_n45_));
  nand2  g28(.a(new_n19_), .b(x5), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n43_), .c(x6), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x4), .O(new_n48_));
  oai21  g31(.a(new_n24_), .b(new_n38_), .c(new_n32_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(new_n48_), .c(new_n45_), .d(new_n28_), .O(z5));
  nand4  g33(.a(x9), .b(x8), .c(x5), .d(x4), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n29_), .c(new_n19_), .O(new_n53_));
  inv1   g36(.a(x2), .O(new_n54_));
  nand4  g37(.a(x9), .b(new_n54_), .c(new_n42_), .d(new_n41_), .O(new_n55_));
  nand3  g38(.a(new_n24_), .b(x8), .c(x3), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n55_), .c(new_n18_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n25_), .c(x7), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n53_), .c(new_n39_), .O(z6));
endmodule


