// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(new_n18_), .c(x8), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x8), .O(z0));
  inv1   g03(.a(x8), .O(new_n21_));
  inv1   g04(.a(x9), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n18_), .O(new_n24_));
  nor2   g07(.a(new_n22_), .b(new_n21_), .O(new_n25_));
  aoi21  g08(.a(new_n22_), .b(x7), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z1));
  nor3   g10(.a(x9), .b(x8), .c(x7), .O(z2));
  inv1   g11(.a(x2), .O(new_n29_));
  oai21  g12(.a(x9), .b(new_n18_), .c(new_n21_), .O(new_n30_));
  inv1   g13(.a(x0), .O(new_n31_));
  inv1   g14(.a(x1), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g16(.a(new_n33_), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n30_), .c(new_n26_), .d(new_n29_), .O(z3));
  oai21  g18(.a(x9), .b(new_n21_), .c(new_n18_), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  nand2  g20(.a(x7), .b(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n19_), .O(z4));
  oai21  g22(.a(new_n22_), .b(x7), .c(new_n37_), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x2), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(new_n22_), .b(new_n42_), .c(x3), .O(new_n43_));
  oai21  g26(.a(new_n41_), .b(new_n33_), .c(new_n43_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x7), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n40_), .c(new_n23_), .O(z5));
  nor3   g29(.a(x2), .b(x1), .c(x0), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n21_), .c(x9), .O(new_n48_));
  nor2   g31(.a(x9), .b(new_n21_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(x7), .c(x4), .d(x3), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(new_n48_), .c(new_n40_), .d(new_n36_), .O(z6));
endmodule


