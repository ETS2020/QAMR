// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x7), .O(new_n18_));
  oai21  g01(.a(x9), .b(x8), .c(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x3), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(new_n21_), .c(x9), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  oai21  g06(.a(x8), .b(new_n21_), .c(x9), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n19_), .O(z0));
  nor2   g08(.a(x9), .b(new_n18_), .O(new_n26_));
  inv1   g09(.a(new_n26_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n24_), .c(new_n19_), .O(z1));
  nor3   g11(.a(x9), .b(x8), .c(x7), .O(z2));
  oai21  g12(.a(x8), .b(new_n21_), .c(x9), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x7), .O(new_n31_));
  oai21  g14(.a(x9), .b(new_n20_), .c(new_n18_), .O(new_n32_));
  nor3   g15(.a(x2), .b(x1), .c(x0), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n24_), .O(z3));
  nand2  g17(.a(x9), .b(new_n21_), .O(new_n35_));
  nand2  g18(.a(new_n27_), .b(x8), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n35_), .c(new_n32_), .d(x6), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  nand3  g22(.a(x9), .b(x8), .c(x7), .O(new_n40_));
  oai21  g23(.a(x8), .b(x7), .c(new_n40_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(x2), .c(new_n39_), .d(new_n38_), .O(new_n42_));
  inv1   g25(.a(x9), .O(new_n43_));
  nor2   g26(.a(new_n43_), .b(new_n20_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n18_), .c(x5), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  inv1   g29(.a(x6), .O(new_n47_));
  aoi21  g30(.a(new_n26_), .b(new_n46_), .c(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n45_), .c(new_n42_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x3), .O(new_n50_));
  oai21  g33(.a(new_n20_), .b(new_n47_), .c(new_n43_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(z5));
  nand2  g35(.a(new_n35_), .b(new_n47_), .O(new_n53_));
  nor2   g36(.a(x9), .b(x8), .O(new_n54_));
  nand3  g37(.a(new_n44_), .b(x5), .c(x3), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n54_), .c(new_n18_), .O(new_n57_));
  oai21  g40(.a(new_n33_), .b(new_n20_), .c(x9), .O(new_n58_));
  nand3  g41(.a(new_n43_), .b(x8), .c(x4), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(x7), .c(x3), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n57_), .c(new_n53_), .O(z6));
endmodule


