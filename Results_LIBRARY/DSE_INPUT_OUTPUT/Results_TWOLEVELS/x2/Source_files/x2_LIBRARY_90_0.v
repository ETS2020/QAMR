// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x3), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x8), .O(new_n21_));
  nand2  g04(.a(new_n18_), .b(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g06(.a(x4), .b(x3), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n20_), .c(new_n18_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x8), .O(new_n26_));
  inv1   g09(.a(x3), .O(new_n27_));
  aoi21  g10(.a(x8), .b(x4), .c(new_n20_), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n27_), .c(new_n18_), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n26_), .c(new_n23_), .O(z1));
  nand4  g13(.a(new_n18_), .b(new_n21_), .c(new_n20_), .d(x3), .O(new_n31_));
  inv1   g14(.a(new_n31_), .O(z2));
  oai21  g15(.a(x8), .b(new_n27_), .c(new_n18_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n20_), .O(new_n34_));
  nand2  g17(.a(x8), .b(x4), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n18_), .c(x3), .O(new_n36_));
  oai21  g19(.a(new_n18_), .b(x8), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x7), .O(new_n38_));
  nor2   g21(.a(x9), .b(x3), .O(new_n39_));
  nor4   g22(.a(new_n39_), .b(x2), .c(x1), .d(x0), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n38_), .c(new_n34_), .d(new_n26_), .O(z3));
  oai21  g24(.a(new_n22_), .b(x7), .c(x6), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x3), .O(new_n43_));
  nand2  g26(.a(new_n21_), .b(x6), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(x9), .c(new_n39_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n43_), .c(new_n23_), .O(z4));
  inv1   g29(.a(x0), .O(new_n47_));
  inv1   g30(.a(x1), .O(new_n48_));
  xnor2a g31(.a(x8), .b(x7), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(x2), .c(new_n48_), .d(new_n47_), .O(new_n50_));
  nand3  g33(.a(x8), .b(new_n20_), .c(x5), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n50_), .c(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x9), .O(new_n53_));
  inv1   g36(.a(x4), .O(new_n54_));
  aoi21  g37(.a(x7), .b(new_n54_), .c(new_n21_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(x9), .c(x6), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x3), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n53_), .O(z5));
  or2    g41(.a(new_n39_), .b(x6), .O(new_n59_));
  nand3  g42(.a(x8), .b(x7), .c(x4), .O(new_n60_));
  oai21  g43(.a(x8), .b(x7), .c(new_n60_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n18_), .c(x3), .O(new_n62_));
  inv1   g45(.a(new_n51_), .O(new_n63_));
  inv1   g46(.a(x2), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n48_), .c(new_n47_), .O(new_n65_));
  aoi21  g48(.a(new_n65_), .b(x8), .c(new_n20_), .O(new_n66_));
  oai21  g49(.a(new_n66_), .b(new_n63_), .c(x9), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n62_), .c(new_n59_), .O(z6));
endmodule


