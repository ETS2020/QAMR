// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x3), .O(new_n19_));
  nand2  g02(.a(x9), .b(x7), .O(new_n20_));
  oai21  g03(.a(x9), .b(new_n19_), .c(new_n20_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  inv1   g06(.a(x9), .O(new_n24_));
  oai21  g07(.a(new_n18_), .b(new_n19_), .c(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  nand2  g09(.a(x9), .b(x8), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(z0));
  nand2  g11(.a(x4), .b(x3), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n23_), .c(new_n24_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x8), .O(new_n31_));
  nand2  g14(.a(x8), .b(x4), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n24_), .c(x7), .d(x3), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n31_), .c(new_n26_), .O(z1));
  nand2  g17(.a(new_n18_), .b(new_n23_), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(x3), .c(x9), .O(z2));
  oai21  g19(.a(x8), .b(new_n19_), .c(new_n24_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  nand3  g21(.a(new_n32_), .b(new_n24_), .c(x3), .O(new_n39_));
  oai21  g22(.a(new_n24_), .b(x8), .c(new_n39_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x7), .O(new_n41_));
  inv1   g24(.a(x0), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  inv1   g27(.a(x2), .O(new_n45_));
  oai21  g28(.a(x9), .b(x3), .c(new_n45_), .O(new_n46_));
  nor2   g29(.a(new_n46_), .b(new_n44_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(new_n41_), .c(new_n38_), .d(new_n31_), .O(z3));
  inv1   g31(.a(x6), .O(new_n49_));
  oai21  g32(.a(x9), .b(x3), .c(new_n49_), .O(new_n50_));
  aoi21  g33(.a(x9), .b(new_n18_), .c(new_n19_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(x9), .c(new_n23_), .O(new_n52_));
  nand3  g35(.a(new_n52_), .b(new_n50_), .c(new_n27_), .O(z4));
  nand3  g36(.a(x9), .b(x8), .c(x7), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n35_), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(x2), .c(new_n43_), .d(new_n42_), .O(new_n56_));
  oai21  g39(.a(new_n23_), .b(x4), .c(x8), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n19_), .c(new_n24_), .O(new_n58_));
  nand4  g41(.a(x9), .b(x8), .c(new_n23_), .d(x5), .O(new_n59_));
  nand4  g42(.a(new_n59_), .b(new_n58_), .c(new_n56_), .d(new_n50_), .O(z5));
  nand3  g43(.a(x8), .b(x7), .c(x4), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n35_), .c(x3), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n24_), .O(new_n63_));
  nand3  g46(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(x8), .c(new_n23_), .O(new_n65_));
  nand3  g48(.a(x8), .b(new_n23_), .c(x5), .O(new_n66_));
  inv1   g49(.a(new_n66_), .O(new_n67_));
  oai21  g50(.a(new_n67_), .b(new_n65_), .c(x9), .O(new_n68_));
  nand3  g51(.a(new_n68_), .b(new_n63_), .c(new_n50_), .O(z6));
endmodule


