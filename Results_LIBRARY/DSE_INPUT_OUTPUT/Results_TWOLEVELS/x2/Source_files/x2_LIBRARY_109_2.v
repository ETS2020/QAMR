// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  inv1   g03(.a(x7), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nand2  g05(.a(new_n19_), .b(new_n22_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n21_), .O(new_n24_));
  nor2   g07(.a(new_n19_), .b(new_n22_), .O(new_n25_));
  inv1   g08(.a(new_n25_), .O(new_n26_));
  oai21  g09(.a(x7), .b(x0), .c(new_n19_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n24_), .O(z1));
  nand4  g11(.a(new_n19_), .b(new_n22_), .c(new_n21_), .d(new_n18_), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nor2   g13(.a(x9), .b(x0), .O(new_n31_));
  nor2   g14(.a(new_n19_), .b(x8), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(x7), .O(new_n33_));
  oai21  g16(.a(x8), .b(x0), .c(new_n19_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n21_), .O(new_n35_));
  oai21  g18(.a(x2), .b(x1), .c(new_n18_), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n35_), .c(new_n33_), .d(new_n26_), .O(z3));
  nand2  g20(.a(new_n19_), .b(x0), .O(new_n38_));
  inv1   g21(.a(x6), .O(new_n39_));
  nor2   g22(.a(new_n23_), .b(x7), .O(new_n40_));
  oai21  g23(.a(new_n40_), .b(new_n39_), .c(new_n18_), .O(new_n41_));
  oai21  g24(.a(x8), .b(new_n39_), .c(x9), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n38_), .d(new_n24_), .O(z4));
  inv1   g26(.a(x1), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(x7), .O(new_n45_));
  oai21  g28(.a(x8), .b(x7), .c(new_n45_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(x2), .c(new_n44_), .O(new_n47_));
  inv1   g30(.a(x4), .O(new_n48_));
  nand3  g31(.a(x7), .b(new_n48_), .c(x3), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x8), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n19_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n47_), .c(x6), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  nand3  g36(.a(x8), .b(new_n21_), .c(x5), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x6), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(x9), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n53_), .O(z5));
  nand2  g40(.a(new_n38_), .b(new_n39_), .O(new_n58_));
  inv1   g41(.a(x2), .O(new_n59_));
  nand4  g42(.a(x9), .b(x7), .c(new_n59_), .d(new_n44_), .O(new_n60_));
  inv1   g43(.a(new_n60_), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n40_), .c(new_n18_), .O(new_n62_));
  nand4  g45(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n63_));
  inv1   g46(.a(new_n63_), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n32_), .c(x7), .O(new_n65_));
  inv1   g48(.a(x5), .O(new_n66_));
  nor2   g49(.a(x7), .b(new_n66_), .O(new_n67_));
  aoi22  g50(.a(new_n67_), .b(new_n25_), .c(new_n19_), .d(x0), .O(new_n68_));
  nand4  g51(.a(new_n68_), .b(new_n65_), .c(new_n62_), .d(new_n58_), .O(z6));
endmodule


