// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:46 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x3), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n23_), .c(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n22_), .O(z0));
  aoi21  g09(.a(x8), .b(new_n18_), .c(x9), .O(new_n27_));
  inv1   g10(.a(x9), .O(new_n28_));
  nand2  g11(.a(new_n23_), .b(new_n18_), .O(new_n29_));
  nand2  g12(.a(x9), .b(x8), .O(new_n30_));
  inv1   g13(.a(new_n30_), .O(new_n31_));
  aoi21  g14(.a(new_n29_), .b(new_n28_), .c(new_n31_), .O(new_n32_));
  oai21  g15(.a(new_n27_), .b(x7), .c(new_n32_), .O(z1));
  nand4  g16(.a(new_n28_), .b(new_n19_), .c(new_n23_), .d(new_n18_), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(z2));
  oai21  g18(.a(new_n28_), .b(new_n19_), .c(x7), .O(new_n36_));
  oai21  g19(.a(x8), .b(x3), .c(new_n28_), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  nor2   g21(.a(x1), .b(x0), .O(new_n39_));
  nand2  g22(.a(new_n28_), .b(x3), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n30_), .d(new_n38_), .O(new_n41_));
  aoi21  g24(.a(new_n37_), .b(new_n23_), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n36_), .O(z3));
  aoi21  g26(.a(x9), .b(new_n19_), .c(x3), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(x9), .c(new_n23_), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n40_), .c(new_n30_), .d(x6), .O(z4));
  inv1   g29(.a(x0), .O(new_n47_));
  inv1   g30(.a(x1), .O(new_n48_));
  nand2  g31(.a(new_n19_), .b(new_n23_), .O(new_n49_));
  oai21  g32(.a(new_n30_), .b(new_n23_), .c(new_n49_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(x2), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  oai21  g34(.a(new_n19_), .b(x3), .c(new_n28_), .O(new_n52_));
  inv1   g35(.a(x6), .O(new_n53_));
  inv1   g36(.a(x5), .O(new_n54_));
  nor2   g37(.a(x7), .b(new_n54_), .O(new_n55_));
  aoi21  g38(.a(new_n55_), .b(new_n31_), .c(new_n53_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n52_), .c(new_n51_), .O(z5));
  nand3  g40(.a(new_n38_), .b(new_n48_), .c(new_n47_), .O(new_n58_));
  aoi21  g41(.a(new_n58_), .b(x8), .c(new_n23_), .O(new_n59_));
  nand3  g42(.a(x8), .b(new_n23_), .c(x5), .O(new_n60_));
  inv1   g43(.a(new_n60_), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n59_), .c(x9), .O(new_n62_));
  nand2  g45(.a(new_n49_), .b(new_n18_), .O(new_n63_));
  aoi21  g46(.a(new_n63_), .b(new_n28_), .c(new_n53_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


