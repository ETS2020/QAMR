// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n24_, new_n25_,
    new_n26_, new_n29_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  xnor2a g00(.a(x9), .b(x7), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n19_), .c(x7), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  or2    g05(.a(new_n22_), .b(new_n18_), .O(z0));
  oai21  g06(.a(new_n20_), .b(x8), .c(x7), .O(new_n24_));
  inv1   g07(.a(x7), .O(new_n25_));
  nand3  g08(.a(new_n20_), .b(x8), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n24_), .O(z1));
  nor3   g10(.a(x9), .b(x8), .c(x7), .O(z2));
  nor3   g11(.a(x2), .b(x1), .c(x0), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(new_n20_), .c(x8), .d(new_n25_), .O(z3));
  nand2  g13(.a(x9), .b(x8), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(x7), .c(x6), .O(z4));
  inv1   g15(.a(x6), .O(new_n33_));
  oai21  g16(.a(new_n20_), .b(x7), .c(new_n33_), .O(new_n34_));
  inv1   g17(.a(x4), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x3), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n25_), .c(x8), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n20_), .O(new_n38_));
  inv1   g21(.a(x0), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  inv1   g24(.a(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n38_), .c(new_n34_), .O(z5));
  nand2  g27(.a(new_n18_), .b(new_n19_), .O(new_n45_));
  inv1   g28(.a(x2), .O(new_n46_));
  nand4  g29(.a(x9), .b(new_n46_), .c(new_n40_), .d(new_n39_), .O(new_n47_));
  nand4  g30(.a(new_n20_), .b(x8), .c(x4), .d(x3), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x7), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n45_), .c(new_n34_), .O(z6));
endmodule


