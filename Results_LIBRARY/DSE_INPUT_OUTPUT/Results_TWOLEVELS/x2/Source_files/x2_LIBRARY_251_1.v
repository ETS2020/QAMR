// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n37_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n46_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(x7), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n20_), .c(x4), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  oai21  g08(.a(x9), .b(new_n18_), .c(new_n25_), .O(new_n26_));
  inv1   g09(.a(x4), .O(new_n27_));
  nand2  g10(.a(x6), .b(new_n27_), .O(new_n28_));
  inv1   g11(.a(x7), .O(new_n29_));
  aoi21  g12(.a(x9), .b(new_n18_), .c(new_n29_), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n28_), .c(new_n26_), .d(new_n24_), .O(z0));
  oai21  g14(.a(x8), .b(new_n27_), .c(x6), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n19_), .c(x7), .O(new_n33_));
  oai21  g16(.a(new_n19_), .b(x7), .c(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n28_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n33_), .O(z1));
  nand2  g19(.a(new_n21_), .b(new_n29_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n28_), .O(z2));
  inv1   g21(.a(x0), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  inv1   g23(.a(x2), .O(new_n41_));
  nand4  g24(.a(x8), .b(new_n41_), .c(new_n40_), .d(new_n39_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(new_n28_), .O(new_n43_));
  oai22  g26(.a(x9), .b(x7), .c(new_n25_), .d(x4), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(z3));
  oai21  g28(.a(new_n20_), .b(new_n29_), .c(x4), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x6), .O(z4));
  nand3  g30(.a(x9), .b(x8), .c(x7), .O(new_n48_));
  oai21  g31(.a(x8), .b(x7), .c(new_n48_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(x2), .c(new_n40_), .d(new_n39_), .O(new_n50_));
  nand3  g33(.a(x9), .b(x8), .c(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n21_), .c(new_n29_), .O(new_n53_));
  nor2   g36(.a(new_n29_), .b(new_n27_), .O(new_n54_));
  aoi22  g37(.a(new_n54_), .b(new_n21_), .c(x6), .d(new_n27_), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(new_n53_), .c(new_n50_), .d(x6), .O(z5));
  nand3  g39(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(x8), .c(new_n19_), .O(new_n58_));
  nand3  g41(.a(new_n19_), .b(x8), .c(x3), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(x7), .O(new_n61_));
  nand4  g44(.a(new_n61_), .b(new_n53_), .c(x6), .d(x4), .O(z6));
endmodule


