// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x7), .O(new_n18_));
  nor2   g01(.a(x9), .b(new_n18_), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(x4), .c(x8), .O(z0));
  inv1   g03(.a(new_n19_), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(x4), .c(x8), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand3  g06(.a(x9), .b(new_n23_), .c(new_n18_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n22_), .c(new_n21_), .O(z1));
  nand2  g08(.a(x8), .b(x4), .O(new_n26_));
  inv1   g09(.a(x9), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n23_), .c(new_n18_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n26_), .O(z2));
  oai21  g12(.a(new_n27_), .b(new_n23_), .c(x7), .O(new_n30_));
  nand2  g13(.a(new_n23_), .b(new_n18_), .O(new_n31_));
  oai21  g14(.a(x9), .b(x4), .c(x8), .O(new_n32_));
  nor3   g15(.a(x2), .b(x1), .c(x0), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z3));
  inv1   g17(.a(x6), .O(new_n35_));
  nand2  g18(.a(new_n26_), .b(new_n35_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n31_), .c(new_n22_), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  nand3  g22(.a(x9), .b(x8), .c(x7), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n31_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(x2), .c(new_n39_), .d(new_n38_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand4  g26(.a(x9), .b(x8), .c(new_n18_), .d(x5), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x6), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  oai21  g29(.a(new_n27_), .b(new_n35_), .c(new_n23_), .O(new_n47_));
  nand2  g30(.a(new_n19_), .b(x3), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n47_), .c(new_n26_), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n46_), .c(new_n42_), .O(z5));
  inv1   g34(.a(x2), .O(new_n52_));
  nand4  g35(.a(x7), .b(new_n52_), .c(new_n39_), .d(new_n38_), .O(new_n53_));
  nand3  g36(.a(x8), .b(new_n18_), .c(x5), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n53_), .c(x4), .O(new_n55_));
  nor2   g38(.a(x8), .b(new_n18_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n55_), .c(x9), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n36_), .c(new_n28_), .O(z6));
endmodule


