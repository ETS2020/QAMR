// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n23_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand3  g03(.a(x9), .b(new_n20_), .c(x7), .O(z1));
  nand2  g04(.a(x9), .b(x8), .O(new_n23_));
  nand2  g05(.a(new_n23_), .b(x7), .O(new_n24_));
  inv1   g06(.a(x7), .O(new_n25_));
  oai21  g07(.a(x8), .b(new_n25_), .c(x9), .O(new_n26_));
  nor2   g08(.a(x1), .b(x0), .O(new_n27_));
  nor2   g09(.a(x9), .b(x8), .O(new_n28_));
  nor2   g10(.a(new_n28_), .b(x2), .O(new_n29_));
  nand4  g11(.a(new_n29_), .b(new_n27_), .c(new_n26_), .d(new_n24_), .O(z3));
  inv1   g12(.a(x6), .O(new_n31_));
  nor2   g13(.a(new_n25_), .b(new_n31_), .O(new_n32_));
  oai21  g14(.a(new_n32_), .b(new_n28_), .c(new_n23_), .O(z4));
  inv1   g15(.a(x0), .O(new_n34_));
  inv1   g16(.a(x1), .O(new_n35_));
  nand3  g17(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  oai21  g18(.a(x8), .b(x7), .c(new_n36_), .O(new_n37_));
  nand4  g19(.a(new_n37_), .b(x2), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  oai21  g20(.a(x9), .b(x8), .c(new_n31_), .O(new_n39_));
  inv1   g21(.a(x4), .O(new_n40_));
  nand3  g22(.a(x7), .b(new_n40_), .c(x3), .O(new_n41_));
  nand2  g23(.a(new_n41_), .b(x8), .O(new_n42_));
  nand2  g24(.a(new_n42_), .b(new_n18_), .O(new_n43_));
  nand4  g25(.a(x9), .b(x8), .c(new_n25_), .d(x5), .O(new_n44_));
  nand4  g26(.a(new_n44_), .b(new_n43_), .c(new_n39_), .d(new_n38_), .O(z5));
  oai21  g27(.a(new_n18_), .b(x7), .c(new_n20_), .O(new_n46_));
  inv1   g28(.a(x2), .O(new_n47_));
  nand4  g29(.a(x9), .b(new_n47_), .c(new_n35_), .d(new_n34_), .O(new_n48_));
  nand3  g30(.a(new_n18_), .b(x4), .c(x3), .O(new_n49_));
  nand2  g31(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g32(.a(new_n50_), .b(x7), .O(new_n51_));
  nand4  g33(.a(new_n51_), .b(new_n44_), .c(new_n39_), .d(new_n46_), .O(z6));
  zero   g34(.O(z2));
endmodule


