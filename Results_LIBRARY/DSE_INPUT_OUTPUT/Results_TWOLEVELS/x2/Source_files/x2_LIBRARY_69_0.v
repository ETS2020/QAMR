// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x0), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  inv1   g03(.a(x0), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n21_), .c(new_n18_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n20_), .O(new_n24_));
  nand2  g07(.a(x9), .b(x8), .O(new_n25_));
  oai21  g08(.a(x7), .b(new_n21_), .c(new_n18_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z1));
  nand2  g10(.a(new_n22_), .b(new_n20_), .O(new_n28_));
  aoi21  g11(.a(new_n28_), .b(x0), .c(x9), .O(z2));
  nand2  g12(.a(new_n18_), .b(new_n21_), .O(new_n30_));
  aoi21  g13(.a(x9), .b(new_n22_), .c(new_n21_), .O(new_n31_));
  oai21  g14(.a(new_n31_), .b(x9), .c(new_n20_), .O(new_n32_));
  oai21  g15(.a(new_n18_), .b(new_n22_), .c(x7), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n25_), .d(new_n30_), .O(z3));
  inv1   g17(.a(x6), .O(new_n35_));
  nand2  g18(.a(new_n30_), .b(new_n35_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n32_), .c(new_n25_), .O(z4));
  inv1   g20(.a(x1), .O(new_n38_));
  xnor2a g21(.a(x8), .b(x7), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(x2), .c(new_n38_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x9), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n21_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x8), .O(new_n45_));
  nand4  g28(.a(x9), .b(x8), .c(new_n20_), .d(x5), .O(new_n46_));
  inv1   g29(.a(new_n46_), .O(new_n47_));
  aoi21  g30(.a(new_n45_), .b(new_n18_), .c(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n42_), .c(new_n36_), .O(z5));
  nand4  g32(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n28_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n18_), .c(x0), .O(new_n52_));
  inv1   g35(.a(x2), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n38_), .c(new_n21_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x8), .c(new_n20_), .O(new_n55_));
  nand3  g38(.a(x8), .b(new_n20_), .c(x5), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n55_), .c(x9), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n52_), .c(new_n36_), .O(z6));
endmodule


