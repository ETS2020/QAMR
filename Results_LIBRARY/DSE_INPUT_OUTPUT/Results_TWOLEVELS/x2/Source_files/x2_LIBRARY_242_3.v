// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:11 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x7), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  inv1   g02(.a(x9), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  oai21  g05(.a(new_n18_), .b(x2), .c(x9), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  oai21  g07(.a(x8), .b(x2), .c(x9), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(z0));
  nand2  g09(.a(new_n20_), .b(x7), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n22_), .O(z1));
  nand2  g11(.a(x9), .b(x2), .O(new_n29_));
  oai21  g12(.a(new_n21_), .b(x7), .c(new_n29_), .O(z2));
  oai21  g13(.a(x8), .b(x2), .c(x9), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x7), .O(new_n32_));
  oai21  g15(.a(x9), .b(new_n19_), .c(new_n18_), .O(new_n33_));
  nor2   g16(.a(x1), .b(x0), .O(new_n34_));
  aoi21  g17(.a(x9), .b(x8), .c(x2), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(new_n32_), .O(z3));
  nand2  g19(.a(new_n27_), .b(x8), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n33_), .c(new_n29_), .d(x6), .O(z4));
  inv1   g21(.a(x2), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(new_n18_), .d(x5), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x6), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x8), .c(x6), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n20_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n42_), .O(z5));
  inv1   g30(.a(x6), .O(new_n48_));
  nand2  g31(.a(new_n29_), .b(new_n48_), .O(new_n49_));
  nand4  g32(.a(x9), .b(x8), .c(x5), .d(new_n39_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n21_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  oai21  g35(.a(x1), .b(x0), .c(x8), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(x9), .c(new_n39_), .O(new_n54_));
  nand4  g37(.a(new_n20_), .b(x8), .c(x4), .d(x3), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x7), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n52_), .c(new_n49_), .O(z6));
endmodule


