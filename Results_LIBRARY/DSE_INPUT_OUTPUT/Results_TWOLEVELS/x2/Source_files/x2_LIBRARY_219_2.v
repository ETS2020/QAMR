// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x7), .O(new_n20_));
  inv1   g03(.a(x9), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  inv1   g05(.a(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n20_), .c(new_n18_), .O(new_n24_));
  inv1   g07(.a(x7), .O(new_n25_));
  nand2  g08(.a(new_n19_), .b(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n26_), .b(new_n25_), .c(x9), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n24_), .O(z0));
  nor2   g11(.a(x9), .b(new_n25_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n20_), .c(new_n18_), .O(new_n30_));
  oai21  g13(.a(x8), .b(new_n25_), .c(x9), .O(new_n31_));
  nand2  g14(.a(new_n21_), .b(x0), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z1));
  nand4  g16(.a(new_n21_), .b(new_n19_), .c(new_n25_), .d(new_n18_), .O(new_n34_));
  inv1   g17(.a(new_n34_), .O(z2));
  nor2   g18(.a(new_n21_), .b(x8), .O(new_n36_));
  nor2   g19(.a(x9), .b(x0), .O(new_n37_));
  oai21  g20(.a(new_n37_), .b(new_n36_), .c(x7), .O(new_n38_));
  nor2   g21(.a(new_n22_), .b(x0), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(x9), .c(new_n25_), .O(new_n40_));
  oai21  g23(.a(x2), .b(x1), .c(new_n18_), .O(new_n41_));
  nand2  g24(.a(x9), .b(x8), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .d(new_n38_), .O(z3));
  inv1   g26(.a(x6), .O(new_n44_));
  nand2  g27(.a(new_n32_), .b(new_n44_), .O(new_n45_));
  aoi21  g28(.a(x9), .b(new_n19_), .c(x0), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(x9), .c(new_n25_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n45_), .c(new_n42_), .O(z4));
  inv1   g31(.a(x1), .O(new_n49_));
  inv1   g32(.a(x2), .O(new_n50_));
  nand2  g33(.a(new_n19_), .b(new_n25_), .O(new_n51_));
  nand3  g34(.a(x9), .b(x8), .c(x7), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(new_n50_), .O(new_n53_));
  nand2  g36(.a(new_n22_), .b(x6), .O(new_n54_));
  aoi21  g37(.a(new_n53_), .b(new_n49_), .c(new_n54_), .O(new_n55_));
  nand3  g38(.a(x8), .b(new_n25_), .c(x5), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(x6), .c(new_n21_), .O(new_n57_));
  inv1   g40(.a(x4), .O(new_n58_));
  nand3  g41(.a(x7), .b(new_n58_), .c(x3), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n18_), .c(x9), .O(new_n60_));
  nor2   g43(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  oai21  g44(.a(new_n55_), .b(x0), .c(new_n61_), .O(z5));
  nand3  g45(.a(x9), .b(x8), .c(x5), .O(new_n63_));
  inv1   g46(.a(new_n63_), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n39_), .c(new_n25_), .O(new_n65_));
  nand3  g48(.a(x9), .b(new_n50_), .c(new_n49_), .O(new_n66_));
  nand4  g49(.a(new_n21_), .b(x8), .c(x4), .d(x3), .O(new_n67_));
  aoi21  g50(.a(new_n67_), .b(new_n66_), .c(x0), .O(new_n68_));
  oai21  g51(.a(new_n68_), .b(new_n36_), .c(x7), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n65_), .c(new_n45_), .O(z6));
endmodule


