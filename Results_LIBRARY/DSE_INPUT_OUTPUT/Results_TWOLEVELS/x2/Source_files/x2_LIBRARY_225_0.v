// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:08 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x8), .O(new_n18_));
  nor2   g01(.a(new_n18_), .b(x7), .O(new_n19_));
  nor2   g02(.a(x9), .b(x8), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n19_), .c(x1), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n18_), .b(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n22_), .c(x9), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n21_), .O(z0));
  inv1   g08(.a(x1), .O(new_n26_));
  inv1   g09(.a(x9), .O(new_n27_));
  oai21  g10(.a(new_n18_), .b(new_n26_), .c(new_n27_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n22_), .O(new_n29_));
  oai21  g12(.a(x7), .b(new_n26_), .c(new_n27_), .O(new_n30_));
  nand2  g13(.a(x9), .b(x8), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(z1));
  nand4  g15(.a(new_n27_), .b(new_n18_), .c(new_n22_), .d(x1), .O(new_n33_));
  inv1   g16(.a(new_n33_), .O(z2));
  aoi21  g17(.a(x9), .b(new_n18_), .c(new_n26_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(x9), .c(new_n22_), .O(new_n36_));
  oai21  g19(.a(new_n27_), .b(new_n18_), .c(x7), .O(new_n37_));
  nand2  g20(.a(new_n27_), .b(new_n26_), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(new_n31_), .O(z3));
  nand4  g22(.a(new_n38_), .b(new_n36_), .c(new_n31_), .d(x6), .O(z4));
  inv1   g23(.a(x0), .O(new_n41_));
  xnor2a g24(.a(x8), .b(x7), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(x2), .c(new_n41_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x9), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n26_), .O(new_n45_));
  inv1   g28(.a(x6), .O(new_n46_));
  oai21  g29(.a(new_n20_), .b(new_n46_), .c(x1), .O(new_n47_));
  nand3  g30(.a(x8), .b(new_n22_), .c(x5), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x6), .O(new_n49_));
  inv1   g32(.a(x4), .O(new_n50_));
  nand4  g33(.a(new_n27_), .b(x7), .c(new_n50_), .d(x3), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  aoi21  g35(.a(new_n49_), .b(x9), .c(new_n52_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n47_), .c(new_n45_), .O(z5));
  nand2  g37(.a(new_n38_), .b(new_n46_), .O(new_n55_));
  nand4  g38(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n56_));
  oai21  g39(.a(x8), .b(x7), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n27_), .c(x1), .O(new_n58_));
  inv1   g41(.a(new_n48_), .O(new_n59_));
  inv1   g42(.a(x2), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n26_), .c(new_n41_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(x8), .c(new_n22_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n59_), .c(x9), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n58_), .c(new_n55_), .O(z6));
endmodule


