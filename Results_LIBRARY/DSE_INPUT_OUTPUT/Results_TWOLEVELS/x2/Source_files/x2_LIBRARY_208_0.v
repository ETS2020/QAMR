// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n29_, new_n31_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x1), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  inv1   g04(.a(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  nand2  g06(.a(x9), .b(x8), .O(new_n24_));
  inv1   g07(.a(x1), .O(new_n25_));
  oai21  g08(.a(x7), .b(new_n25_), .c(new_n18_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n24_), .c(new_n23_), .O(z1));
  inv1   g10(.a(x8), .O(new_n28_));
  nand4  g11(.a(new_n18_), .b(new_n28_), .c(new_n20_), .d(x1), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nor2   g13(.a(new_n18_), .b(new_n28_), .O(new_n31_));
  nand4  g14(.a(new_n31_), .b(new_n24_), .c(new_n22_), .d(x7), .O(z3));
  oai21  g15(.a(x9), .b(new_n20_), .c(x8), .O(new_n33_));
  nand2  g16(.a(new_n21_), .b(x1), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n20_), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  aoi21  g20(.a(new_n18_), .b(new_n25_), .c(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n33_), .O(z4));
  inv1   g22(.a(x0), .O(new_n40_));
  xnor2a g23(.a(x8), .b(x7), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(x2), .c(new_n40_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x9), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n25_), .O(new_n44_));
  inv1   g27(.a(x4), .O(new_n45_));
  nand3  g28(.a(x7), .b(new_n45_), .c(x3), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(x8), .c(x9), .O(new_n47_));
  nand2  g30(.a(new_n20_), .b(x5), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n24_), .c(x6), .O(new_n49_));
  nor2   g32(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n44_), .O(z5));
  inv1   g34(.a(x2), .O(new_n52_));
  nand3  g35(.a(x7), .b(new_n52_), .c(new_n40_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(x9), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n25_), .O(new_n55_));
  nand4  g38(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  oai21  g39(.a(new_n18_), .b(x8), .c(new_n56_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x7), .O(new_n58_));
  nand3  g41(.a(x9), .b(x8), .c(x5), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n34_), .O(new_n60_));
  aoi21  g43(.a(new_n60_), .b(new_n20_), .c(new_n37_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n58_), .c(new_n55_), .O(z6));
endmodule


