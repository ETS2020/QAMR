// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:13 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_;
  inv1   g00(.a(x7), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x2), .O(new_n20_));
  oai21  g03(.a(x9), .b(new_n18_), .c(x8), .O(new_n21_));
  inv1   g04(.a(x2), .O(new_n22_));
  nand2  g05(.a(new_n19_), .b(new_n22_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(new_n20_), .O(z0));
  inv1   g07(.a(x9), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n19_), .c(x7), .O(new_n26_));
  oai21  g09(.a(new_n25_), .b(x7), .c(new_n26_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(x2), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n19_), .O(z1));
  nand2  g12(.a(new_n25_), .b(new_n18_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(x2), .c(x8), .O(z2));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n30_), .c(x8), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n20_), .O(z3));
  inv1   g19(.a(x6), .O(new_n37_));
  oai21  g20(.a(new_n18_), .b(new_n37_), .c(new_n23_), .O(new_n38_));
  oai21  g21(.a(new_n25_), .b(new_n19_), .c(new_n38_), .O(z4));
  nand2  g22(.a(new_n19_), .b(new_n18_), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n40_), .c(x1), .O(new_n42_));
  oai21  g25(.a(x9), .b(x8), .c(x6), .O(new_n43_));
  aoi21  g26(.a(new_n42_), .b(new_n32_), .c(new_n43_), .O(new_n44_));
  nand3  g27(.a(x9), .b(new_n18_), .c(x5), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand4  g29(.a(new_n25_), .b(x7), .c(new_n46_), .d(x3), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n45_), .c(x6), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x8), .O(new_n49_));
  oai21  g32(.a(new_n44_), .b(new_n22_), .c(new_n49_), .O(z5));
  nand2  g33(.a(x9), .b(x7), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n34_), .c(x8), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nand2  g36(.a(new_n23_), .b(new_n37_), .O(new_n54_));
  nand2  g37(.a(new_n51_), .b(new_n30_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n19_), .c(x2), .O(new_n56_));
  nand4  g39(.a(new_n25_), .b(x7), .c(x4), .d(x3), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n45_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(x8), .O(new_n59_));
  nand4  g42(.a(new_n59_), .b(new_n56_), .c(new_n54_), .d(new_n53_), .O(z6));
endmodule


