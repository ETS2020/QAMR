// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x8), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  aoi21  g06(.a(x9), .b(new_n23_), .c(new_n18_), .O(new_n24_));
  nor2   g07(.a(x9), .b(x7), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  nand2  g09(.a(x9), .b(new_n23_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n21_), .O(z0));
  nor2   g11(.a(x9), .b(x8), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n23_), .O(new_n31_));
  nor2   g14(.a(new_n19_), .b(new_n22_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x4), .O(new_n33_));
  nand3  g16(.a(new_n33_), .b(new_n31_), .c(new_n20_), .O(z1));
  nand2  g17(.a(x7), .b(new_n18_), .O(new_n35_));
  oai21  g18(.a(new_n30_), .b(x7), .c(new_n35_), .O(z2));
  nand2  g19(.a(x7), .b(new_n18_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x9), .O(new_n38_));
  nor2   g21(.a(x1), .b(x0), .O(new_n39_));
  aoi21  g22(.a(new_n29_), .b(new_n23_), .c(x2), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n39_), .c(new_n38_), .d(new_n20_), .O(z3));
  inv1   g24(.a(x6), .O(new_n42_));
  oai21  g25(.a(new_n32_), .b(new_n42_), .c(x4), .O(new_n43_));
  nand2  g26(.a(new_n19_), .b(x8), .O(new_n44_));
  oai21  g27(.a(new_n44_), .b(x8), .c(new_n23_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n43_), .O(z4));
  inv1   g29(.a(x0), .O(new_n47_));
  inv1   g30(.a(x1), .O(new_n48_));
  nand4  g31(.a(x9), .b(x8), .c(x7), .d(x4), .O(new_n49_));
  oai21  g32(.a(x8), .b(x7), .c(new_n49_), .O(new_n50_));
  nand4  g33(.a(new_n50_), .b(x2), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nand2  g34(.a(new_n30_), .b(x6), .O(new_n52_));
  inv1   g35(.a(x5), .O(new_n53_));
  nor2   g36(.a(x7), .b(new_n53_), .O(new_n54_));
  aoi22  g37(.a(new_n54_), .b(new_n32_), .c(new_n52_), .d(new_n35_), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n51_), .O(z5));
  nand2  g39(.a(new_n35_), .b(new_n42_), .O(new_n57_));
  nand3  g40(.a(x9), .b(x8), .c(x5), .O(new_n58_));
  inv1   g41(.a(new_n58_), .O(new_n59_));
  oai21  g42(.a(new_n59_), .b(new_n29_), .c(new_n23_), .O(new_n60_));
  nor2   g43(.a(x2), .b(x1), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n47_), .c(new_n22_), .O(new_n62_));
  nand3  g45(.a(new_n19_), .b(x8), .c(x3), .O(new_n63_));
  oai21  g46(.a(new_n62_), .b(new_n19_), .c(new_n63_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(x7), .c(x4), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n60_), .c(new_n57_), .O(z6));
endmodule


