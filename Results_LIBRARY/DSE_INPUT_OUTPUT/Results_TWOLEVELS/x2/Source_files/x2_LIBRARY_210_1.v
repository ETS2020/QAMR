// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_, new_n33_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x7), .O(new_n18_));
  nor2   g01(.a(x8), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  inv1   g03(.a(x9), .O(new_n21_));
  nor2   g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n19_), .c(x0), .O(new_n23_));
  nand2  g06(.a(new_n21_), .b(x8), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x8), .c(new_n18_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(z0));
  inv1   g09(.a(x0), .O(new_n27_));
  aoi21  g10(.a(new_n21_), .b(x7), .c(x8), .O(new_n28_));
  nand2  g11(.a(new_n21_), .b(new_n20_), .O(new_n29_));
  nor2   g12(.a(new_n18_), .b(x0), .O(new_n30_));
  aoi21  g13(.a(new_n29_), .b(new_n18_), .c(new_n30_), .O(new_n31_));
  oai21  g14(.a(new_n28_), .b(new_n27_), .c(new_n31_), .O(z1));
  inv1   g15(.a(new_n30_), .O(new_n33_));
  oai21  g16(.a(new_n29_), .b(x7), .c(new_n33_), .O(z2));
  inv1   g17(.a(x1), .O(new_n35_));
  nand2  g18(.a(new_n24_), .b(new_n18_), .O(new_n36_));
  oai21  g19(.a(x8), .b(x7), .c(x0), .O(new_n37_));
  nor2   g20(.a(new_n30_), .b(x2), .O(new_n38_));
  nand4  g21(.a(new_n38_), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(z3));
  inv1   g22(.a(x6), .O(new_n40_));
  oai21  g23(.a(new_n22_), .b(new_n40_), .c(x0), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n25_), .O(z4));
  nand3  g25(.a(new_n20_), .b(x2), .c(new_n35_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n27_), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x3), .O(new_n47_));
  nand2  g30(.a(new_n20_), .b(x0), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n47_), .c(new_n18_), .O(new_n49_));
  nor2   g32(.a(x8), .b(x7), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n49_), .c(new_n21_), .O(new_n51_));
  inv1   g34(.a(x5), .O(new_n52_));
  nor2   g35(.a(x7), .b(new_n52_), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n22_), .c(new_n40_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n51_), .c(new_n45_), .O(z5));
  nand3  g38(.a(x9), .b(x8), .c(x5), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  nand2  g41(.a(x9), .b(new_n20_), .O(new_n59_));
  nand4  g42(.a(new_n21_), .b(x8), .c(x4), .d(x3), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n59_), .c(x0), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x7), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n58_), .c(x6), .O(z6));
endmodule


