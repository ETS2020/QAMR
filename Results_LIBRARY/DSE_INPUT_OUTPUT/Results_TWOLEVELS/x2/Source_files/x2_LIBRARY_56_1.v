// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n24_, new_n25_, new_n26_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n32_, new_n33_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  oai21  g02(.a(new_n18_), .b(x8), .c(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(x7), .c(new_n20_), .O(z1));
  aoi21  g05(.a(new_n18_), .b(x8), .c(x7), .O(z2));
  inv1   g06(.a(x8), .O(new_n24_));
  oai21  g07(.a(new_n18_), .b(x7), .c(new_n24_), .O(new_n25_));
  inv1   g08(.a(x0), .O(new_n26_));
  inv1   g09(.a(x1), .O(new_n27_));
  inv1   g10(.a(x2), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(new_n27_), .c(new_n26_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n18_), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n25_), .c(new_n20_), .O(z3));
  oai21  g14(.a(new_n18_), .b(new_n24_), .c(x6), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x7), .O(new_n33_));
  oai21  g16(.a(x9), .b(x7), .c(new_n33_), .O(z4));
  nand2  g17(.a(new_n18_), .b(new_n24_), .O(new_n35_));
  inv1   g18(.a(x6), .O(new_n36_));
  oai21  g19(.a(new_n18_), .b(x7), .c(new_n36_), .O(new_n37_));
  nand2  g20(.a(new_n27_), .b(new_n26_), .O(new_n38_));
  nand3  g21(.a(x9), .b(x8), .c(x2), .O(new_n39_));
  inv1   g22(.a(x4), .O(new_n40_));
  nand3  g23(.a(new_n18_), .b(new_n40_), .c(x3), .O(new_n41_));
  oai21  g24(.a(new_n39_), .b(new_n38_), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x7), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n37_), .c(new_n35_), .O(z5));
  nand2  g27(.a(x9), .b(new_n24_), .O(new_n45_));
  nand4  g28(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x7), .O(new_n48_));
  inv1   g31(.a(new_n38_), .O(new_n49_));
  nor2   g32(.a(new_n18_), .b(x2), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n49_), .c(z2), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n48_), .c(new_n37_), .O(z6));
endmodule


