// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n29_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g03(.a(x2), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  oai21  g05(.a(new_n20_), .b(new_n21_), .c(new_n22_), .O(new_n23_));
  nand2  g06(.a(x9), .b(x7), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(new_n23_), .c(new_n20_), .O(z0));
  nand3  g08(.a(new_n20_), .b(new_n22_), .c(x2), .O(new_n26_));
  oai21  g09(.a(new_n19_), .b(x8), .c(x7), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n26_), .O(z1));
  nand4  g11(.a(new_n19_), .b(new_n18_), .c(new_n22_), .d(x2), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nand2  g13(.a(new_n22_), .b(new_n21_), .O(z3));
  oai21  g14(.a(x7), .b(new_n21_), .c(new_n24_), .O(new_n32_));
  nand2  g15(.a(new_n32_), .b(x8), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nand3  g17(.a(new_n19_), .b(x8), .c(x2), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n22_), .c(new_n34_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n33_), .O(z4));
  inv1   g20(.a(x0), .O(new_n38_));
  inv1   g21(.a(x1), .O(new_n39_));
  nand4  g22(.a(x9), .b(x8), .c(x7), .d(x2), .O(new_n40_));
  oai21  g23(.a(x8), .b(x7), .c(new_n40_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(new_n42_));
  nand3  g25(.a(x9), .b(x8), .c(x5), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x2), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n22_), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand3  g29(.a(x7), .b(new_n46_), .c(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x8), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(new_n45_), .c(new_n42_), .d(x6), .O(z5));
  nand3  g33(.a(x9), .b(new_n39_), .c(new_n38_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x7), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n21_), .O(new_n53_));
  nand2  g36(.a(x9), .b(new_n18_), .O(new_n54_));
  nand4  g37(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(x7), .O(new_n57_));
  nand3  g40(.a(new_n19_), .b(new_n18_), .c(x2), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n43_), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n22_), .c(new_n34_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n57_), .c(new_n53_), .O(z6));
endmodule


