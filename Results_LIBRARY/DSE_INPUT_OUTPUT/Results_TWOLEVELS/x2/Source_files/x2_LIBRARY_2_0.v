// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:31 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x1), .O(z0));
  inv1   g02(.a(x1), .O(new_n20_));
  nand2  g03(.a(new_n18_), .b(new_n20_), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(x7), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  nor2   g07(.a(x9), .b(new_n24_), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n23_), .c(x1), .O(new_n26_));
  oai21  g09(.a(x8), .b(new_n24_), .c(x9), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n21_), .O(z1));
  nand4  g11(.a(new_n18_), .b(new_n22_), .c(new_n24_), .d(x1), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nand2  g13(.a(x9), .b(x8), .O(new_n31_));
  nor2   g14(.a(new_n18_), .b(x8), .O(new_n32_));
  nor2   g15(.a(x9), .b(new_n20_), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n32_), .c(x7), .O(new_n34_));
  aoi21  g17(.a(x9), .b(new_n22_), .c(new_n20_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(x9), .c(new_n24_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n34_), .c(new_n31_), .O(z3));
  nand4  g20(.a(new_n36_), .b(new_n31_), .c(new_n21_), .d(x6), .O(z4));
  inv1   g21(.a(x6), .O(new_n39_));
  nand2  g22(.a(new_n21_), .b(new_n39_), .O(new_n40_));
  inv1   g23(.a(x0), .O(new_n41_));
  xnor2a g24(.a(x8), .b(x7), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(x2), .c(new_n41_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x9), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n20_), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand3  g29(.a(x7), .b(new_n46_), .c(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x8), .O(new_n48_));
  nand4  g31(.a(x9), .b(x8), .c(new_n24_), .d(x5), .O(new_n49_));
  inv1   g32(.a(new_n49_), .O(new_n50_));
  aoi21  g33(.a(new_n48_), .b(new_n18_), .c(new_n50_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n45_), .c(new_n40_), .O(z5));
  nand4  g35(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n53_));
  oai21  g36(.a(x8), .b(x7), .c(new_n53_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n18_), .c(x1), .O(new_n55_));
  inv1   g38(.a(x2), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n20_), .c(new_n41_), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(x8), .c(new_n24_), .O(new_n58_));
  nand3  g41(.a(x8), .b(new_n24_), .c(x5), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(x9), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n55_), .c(new_n40_), .O(z6));
endmodule


