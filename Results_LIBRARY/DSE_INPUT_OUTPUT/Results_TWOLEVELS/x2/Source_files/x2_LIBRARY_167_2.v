// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_;
  inv1   g00(.a(x3), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  nand2  g02(.a(x8), .b(new_n19_), .O(new_n20_));
  inv1   g03(.a(new_n20_), .O(new_n21_));
  nor2   g04(.a(x9), .b(x8), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n21_), .c(new_n18_), .O(new_n23_));
  inv1   g06(.a(x8), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n19_), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n19_), .c(x9), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n23_), .O(z0));
  nor2   g10(.a(x9), .b(new_n19_), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n21_), .c(new_n18_), .O(new_n29_));
  oai21  g12(.a(x8), .b(new_n19_), .c(x9), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(z1));
  nand2  g14(.a(new_n24_), .b(new_n19_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n18_), .c(x9), .O(z2));
  inv1   g16(.a(x9), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x8), .O(new_n35_));
  nor2   g18(.a(x9), .b(x3), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n35_), .c(x7), .O(new_n37_));
  oai21  g20(.a(x8), .b(x3), .c(new_n34_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n19_), .O(new_n39_));
  inv1   g22(.a(x0), .O(new_n40_));
  inv1   g23(.a(x1), .O(new_n41_));
  inv1   g24(.a(x2), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n18_), .O(new_n44_));
  nand2  g27(.a(x9), .b(x8), .O(new_n45_));
  nand4  g28(.a(new_n45_), .b(new_n44_), .c(new_n39_), .d(new_n37_), .O(z3));
  inv1   g29(.a(x6), .O(new_n47_));
  nand2  g30(.a(new_n34_), .b(x3), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  aoi21  g32(.a(x9), .b(new_n24_), .c(x3), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(x9), .c(new_n19_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n49_), .c(new_n45_), .O(z4));
  nand3  g35(.a(x9), .b(x8), .c(x7), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n32_), .O(new_n54_));
  nand4  g37(.a(new_n54_), .b(x2), .c(new_n41_), .d(new_n40_), .O(new_n55_));
  oai21  g38(.a(new_n22_), .b(new_n47_), .c(new_n18_), .O(new_n56_));
  inv1   g39(.a(x5), .O(new_n57_));
  oai21  g40(.a(new_n20_), .b(new_n57_), .c(x6), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(x9), .O(new_n59_));
  nand4  g42(.a(new_n59_), .b(new_n56_), .c(new_n55_), .d(new_n48_), .O(z5));
  nor3   g43(.a(x9), .b(x8), .c(x7), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n47_), .c(new_n18_), .O(new_n62_));
  aoi21  g45(.a(new_n43_), .b(x8), .c(new_n19_), .O(new_n63_));
  oai21  g46(.a(new_n63_), .b(new_n58_), .c(x9), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n62_), .O(z6));
endmodule


