// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:42 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n23_), .c(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n22_), .O(z0));
  nor2   g09(.a(x9), .b(new_n23_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n20_), .c(new_n18_), .O(new_n28_));
  oai21  g11(.a(x8), .b(new_n23_), .c(x9), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n28_), .O(z1));
  nand2  g13(.a(new_n19_), .b(new_n23_), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(new_n18_), .c(x9), .O(z2));
  inv1   g15(.a(x9), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(x8), .O(new_n34_));
  nor2   g17(.a(x9), .b(x1), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(new_n34_), .c(x7), .O(new_n36_));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  inv1   g21(.a(new_n21_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n18_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n36_), .c(new_n29_), .O(z3));
  inv1   g25(.a(x6), .O(new_n43_));
  oai21  g26(.a(x9), .b(new_n18_), .c(new_n43_), .O(new_n44_));
  aoi21  g27(.a(x9), .b(new_n19_), .c(x1), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(x9), .c(new_n23_), .O(new_n46_));
  nor2   g29(.a(new_n33_), .b(new_n19_), .O(new_n47_));
  inv1   g30(.a(new_n47_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n44_), .O(z4));
  nand3  g32(.a(x9), .b(x8), .c(x7), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n31_), .c(new_n38_), .O(new_n51_));
  nand2  g34(.a(new_n39_), .b(x6), .O(new_n52_));
  aoi21  g35(.a(new_n51_), .b(new_n37_), .c(new_n52_), .O(new_n53_));
  nand3  g36(.a(x8), .b(new_n23_), .c(x5), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(x6), .c(new_n33_), .O(new_n55_));
  inv1   g38(.a(x4), .O(new_n56_));
  nand3  g39(.a(x7), .b(new_n56_), .c(x3), .O(new_n57_));
  aoi21  g40(.a(new_n57_), .b(new_n18_), .c(x9), .O(new_n58_));
  nor2   g41(.a(new_n58_), .b(new_n55_), .O(new_n59_));
  oai21  g42(.a(new_n53_), .b(x1), .c(new_n59_), .O(z5));
  nand4  g43(.a(x9), .b(x7), .c(new_n38_), .d(new_n37_), .O(new_n61_));
  oai21  g44(.a(new_n39_), .b(x7), .c(new_n61_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n18_), .O(new_n63_));
  nand4  g46(.a(new_n33_), .b(x8), .c(x4), .d(x3), .O(new_n64_));
  inv1   g47(.a(new_n64_), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n34_), .c(x7), .O(new_n66_));
  inv1   g49(.a(x5), .O(new_n67_));
  nor2   g50(.a(x7), .b(new_n67_), .O(new_n68_));
  aoi22  g51(.a(new_n68_), .b(new_n47_), .c(new_n33_), .d(x1), .O(new_n69_));
  nand4  g52(.a(new_n69_), .b(new_n66_), .c(new_n63_), .d(new_n44_), .O(z6));
endmodule


