// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n29_, new_n30_, new_n31_, new_n32_,
    new_n34_, new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_,
    new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x8), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  aoi21  g06(.a(x9), .b(new_n23_), .c(new_n18_), .O(new_n24_));
  nor2   g07(.a(x9), .b(x7), .O(new_n25_));
  oai21  g08(.a(new_n25_), .b(new_n24_), .c(new_n22_), .O(new_n26_));
  nand2  g09(.a(x9), .b(new_n23_), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n26_), .c(new_n21_), .O(z0));
  nand2  g11(.a(new_n19_), .b(new_n22_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n23_), .O(new_n30_));
  nor2   g13(.a(new_n19_), .b(new_n22_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x1), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(new_n30_), .c(new_n20_), .O(z1));
  nand2  g16(.a(x7), .b(new_n18_), .O(new_n34_));
  oai21  g17(.a(new_n29_), .b(x7), .c(new_n34_), .O(z2));
  inv1   g18(.a(x0), .O(new_n36_));
  nand2  g19(.a(new_n19_), .b(x8), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n23_), .O(new_n38_));
  aoi21  g21(.a(x7), .b(new_n18_), .c(x2), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(new_n38_), .c(new_n18_), .d(new_n36_), .O(z3));
  inv1   g23(.a(x6), .O(new_n41_));
  oai21  g24(.a(new_n31_), .b(new_n41_), .c(x1), .O(new_n42_));
  oai21  g25(.a(new_n37_), .b(x8), .c(new_n23_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(new_n42_), .O(z4));
  nand3  g27(.a(new_n22_), .b(x2), .c(new_n36_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n23_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  nand3  g30(.a(x9), .b(x8), .c(x5), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n29_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n23_), .O(new_n50_));
  inv1   g33(.a(x4), .O(new_n51_));
  nand3  g34(.a(x7), .b(new_n51_), .c(x3), .O(new_n52_));
  nand2  g35(.a(new_n22_), .b(x1), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n19_), .O(new_n55_));
  nand4  g38(.a(new_n55_), .b(new_n50_), .c(new_n47_), .d(x6), .O(z5));
  nand2  g39(.a(x9), .b(new_n22_), .O(new_n57_));
  nand4  g40(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n57_), .c(x1), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x7), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n50_), .c(x6), .O(z6));
endmodule


