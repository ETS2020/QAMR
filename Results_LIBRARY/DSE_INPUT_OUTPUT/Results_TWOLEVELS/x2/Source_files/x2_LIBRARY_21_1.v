// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  inv1   g00(.a(x7), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n18_), .c(new_n19_), .O(new_n21_));
  oai21  g04(.a(new_n19_), .b(new_n18_), .c(new_n21_), .O(z0));
  oai21  g05(.a(new_n19_), .b(x8), .c(x7), .O(new_n23_));
  nand2  g06(.a(new_n19_), .b(new_n20_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n18_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n23_), .O(z1));
  aoi21  g09(.a(new_n19_), .b(x8), .c(x7), .O(z2));
  oai21  g10(.a(new_n19_), .b(x7), .c(new_n20_), .O(new_n28_));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  inv1   g14(.a(new_n31_), .O(new_n32_));
  aoi21  g15(.a(x9), .b(new_n18_), .c(x2), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n28_), .d(new_n23_), .O(z3));
  inv1   g17(.a(x6), .O(new_n35_));
  aoi21  g18(.a(x9), .b(x8), .c(new_n35_), .O(new_n36_));
  oai21  g19(.a(new_n24_), .b(new_n20_), .c(new_n18_), .O(new_n37_));
  oai21  g20(.a(new_n36_), .b(new_n18_), .c(new_n37_), .O(z4));
  oai21  g21(.a(new_n19_), .b(x7), .c(new_n35_), .O(new_n39_));
  nand3  g22(.a(x9), .b(x8), .c(x2), .O(new_n40_));
  inv1   g23(.a(x4), .O(new_n41_));
  nand3  g24(.a(new_n19_), .b(new_n41_), .c(x3), .O(new_n42_));
  oai21  g25(.a(new_n40_), .b(new_n31_), .c(new_n42_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x7), .O(new_n44_));
  nand2  g27(.a(new_n19_), .b(new_n20_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n44_), .c(new_n39_), .O(z5));
  nand2  g29(.a(new_n19_), .b(x8), .O(new_n47_));
  nand2  g30(.a(x9), .b(new_n20_), .O(new_n48_));
  nand2  g31(.a(x4), .b(x3), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n47_), .c(new_n48_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x7), .O(new_n51_));
  nor2   g34(.a(new_n19_), .b(x2), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n32_), .c(z2), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n51_), .c(new_n39_), .O(z6));
endmodule


