// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:05 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x4), .O(z0));
  nand2  g02(.a(x9), .b(x8), .O(new_n20_));
  nand2  g03(.a(new_n18_), .b(x7), .O(new_n21_));
  nand2  g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x4), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  oai21  g07(.a(x9), .b(x8), .c(new_n24_), .O(new_n25_));
  inv1   g08(.a(x4), .O(new_n26_));
  nand2  g09(.a(x8), .b(new_n26_), .O(new_n27_));
  nor2   g10(.a(x9), .b(x8), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x7), .O(new_n29_));
  nand4  g12(.a(new_n29_), .b(new_n27_), .c(new_n25_), .d(new_n23_), .O(z1));
  nand2  g13(.a(new_n28_), .b(new_n24_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n27_), .O(z2));
  inv1   g15(.a(x2), .O(new_n33_));
  nor2   g16(.a(x1), .b(x0), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n21_), .c(new_n20_), .d(new_n33_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(x4), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x8), .O(z3));
  nand2  g20(.a(new_n20_), .b(x6), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x4), .O(new_n39_));
  nand2  g22(.a(x7), .b(x4), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(x8), .O(new_n41_));
  inv1   g24(.a(x6), .O(new_n42_));
  inv1   g25(.a(x8), .O(new_n43_));
  oai21  g26(.a(new_n24_), .b(new_n42_), .c(new_n43_), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n41_), .c(new_n39_), .O(z4));
  inv1   g28(.a(x0), .O(new_n46_));
  inv1   g29(.a(x1), .O(new_n47_));
  oai22  g30(.a(new_n40_), .b(new_n20_), .c(x8), .d(x7), .O(new_n48_));
  nand4  g31(.a(new_n48_), .b(x2), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nand2  g32(.a(new_n24_), .b(x5), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n20_), .c(x6), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x4), .O(new_n52_));
  oai21  g35(.a(new_n18_), .b(new_n42_), .c(new_n43_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n49_), .O(z5));
  nand2  g37(.a(new_n27_), .b(new_n42_), .O(new_n55_));
  nand4  g38(.a(x9), .b(x8), .c(x5), .d(x4), .O(new_n56_));
  inv1   g39(.a(new_n56_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n28_), .c(new_n24_), .O(new_n58_));
  nor2   g41(.a(new_n18_), .b(x8), .O(new_n59_));
  nand4  g42(.a(x9), .b(new_n33_), .c(new_n47_), .d(new_n46_), .O(new_n60_));
  nand3  g43(.a(new_n18_), .b(x8), .c(x3), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n60_), .c(new_n26_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n59_), .c(x7), .O(new_n63_));
  nand3  g46(.a(new_n63_), .b(new_n58_), .c(new_n55_), .O(z6));
endmodule


