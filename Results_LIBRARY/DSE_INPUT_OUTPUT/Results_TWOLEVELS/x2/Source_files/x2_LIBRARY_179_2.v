// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  aoi21  g03(.a(new_n18_), .b(new_n20_), .c(new_n19_), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(x6), .c(new_n18_), .O(z0));
  aoi21  g05(.a(new_n20_), .b(x7), .c(x6), .O(new_n23_));
  aoi21  g06(.a(x9), .b(new_n19_), .c(x8), .O(new_n24_));
  oai21  g07(.a(new_n23_), .b(x9), .c(new_n24_), .O(z1));
  inv1   g08(.a(x6), .O(new_n26_));
  nand4  g09(.a(new_n18_), .b(new_n20_), .c(new_n19_), .d(new_n26_), .O(new_n27_));
  inv1   g10(.a(new_n27_), .O(z2));
  nor2   g11(.a(x8), .b(x7), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(x6), .c(new_n18_), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  nand4  g15(.a(new_n32_), .b(new_n18_), .c(new_n19_), .d(new_n31_), .O(new_n33_));
  inv1   g16(.a(new_n33_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n30_), .O(z3));
  nand4  g18(.a(x9), .b(new_n20_), .c(x7), .d(x6), .O(z4));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x1), .O(new_n38_));
  xnor2a g21(.a(x8), .b(x7), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(x2), .c(new_n38_), .d(new_n37_), .O(new_n40_));
  nand3  g23(.a(x8), .b(new_n19_), .c(x5), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(x9), .d(x6), .O(z5));
  nor3   g25(.a(x2), .b(x1), .c(x0), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(new_n20_), .c(x7), .O(new_n44_));
  nand3  g27(.a(new_n41_), .b(x9), .c(x6), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n44_), .O(z6));
endmodule


