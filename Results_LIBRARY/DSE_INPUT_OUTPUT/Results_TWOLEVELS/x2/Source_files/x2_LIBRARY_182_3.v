// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand3  g01(.a(new_n18_), .b(x8), .c(x7), .O(z0));
  inv1   g02(.a(x8), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x7), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(x4), .c(x9), .O(new_n22_));
  oai21  g05(.a(x8), .b(x7), .c(new_n18_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(z1));
  nand2  g07(.a(x9), .b(x4), .O(new_n25_));
  inv1   g08(.a(x7), .O(new_n26_));
  nand3  g09(.a(new_n18_), .b(new_n20_), .c(new_n26_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n25_), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n18_), .c(x8), .d(new_n26_), .O(z3));
  inv1   g17(.a(x6), .O(new_n35_));
  nand2  g18(.a(new_n25_), .b(new_n35_), .O(new_n36_));
  inv1   g19(.a(x4), .O(new_n37_));
  nand3  g20(.a(new_n21_), .b(x9), .c(new_n37_), .O(new_n38_));
  nand2  g21(.a(new_n18_), .b(new_n26_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n38_), .c(new_n36_), .O(z4));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  oai21  g24(.a(x8), .b(x7), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n30_), .d(new_n29_), .O(new_n43_));
  nand4  g26(.a(x9), .b(x8), .c(new_n26_), .d(x5), .O(new_n44_));
  nor2   g27(.a(x9), .b(new_n26_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(x3), .c(new_n35_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n37_), .O(new_n48_));
  oai21  g31(.a(new_n20_), .b(new_n35_), .c(new_n18_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n48_), .O(z5));
  nand3  g33(.a(x8), .b(x7), .c(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n18_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x4), .O(new_n53_));
  nand2  g36(.a(new_n44_), .b(x6), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n37_), .O(new_n55_));
  nand2  g38(.a(new_n32_), .b(x8), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(x9), .c(x7), .O(new_n57_));
  oai21  g40(.a(x8), .b(x7), .c(x6), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n18_), .O(new_n59_));
  nand4  g42(.a(new_n59_), .b(new_n57_), .c(new_n55_), .d(new_n53_), .O(z6));
endmodule


