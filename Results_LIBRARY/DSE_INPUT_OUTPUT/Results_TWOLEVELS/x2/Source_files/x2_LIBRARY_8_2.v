// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x2), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  aoi21  g03(.a(x8), .b(new_n18_), .c(x9), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nor2   g05(.a(new_n19_), .b(new_n22_), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  aoi21  g07(.a(new_n24_), .b(new_n18_), .c(x9), .O(new_n25_));
  nor2   g08(.a(new_n25_), .b(new_n23_), .O(new_n26_));
  oai21  g09(.a(new_n21_), .b(x7), .c(new_n26_), .O(z1));
  nand4  g10(.a(new_n19_), .b(new_n22_), .c(new_n24_), .d(new_n18_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  nor2   g12(.a(x9), .b(new_n18_), .O(new_n30_));
  nor2   g13(.a(new_n23_), .b(new_n30_), .O(new_n31_));
  oai21  g14(.a(new_n19_), .b(new_n22_), .c(x7), .O(new_n32_));
  oai21  g15(.a(x8), .b(x2), .c(new_n19_), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n24_), .O(new_n34_));
  nor2   g17(.a(x1), .b(x0), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n34_), .c(new_n32_), .d(new_n31_), .O(z3));
  inv1   g19(.a(new_n23_), .O(new_n37_));
  or2    g20(.a(new_n30_), .b(x6), .O(new_n38_));
  aoi21  g21(.a(x9), .b(new_n22_), .c(x2), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(x9), .c(new_n24_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n38_), .c(new_n37_), .O(z4));
  inv1   g24(.a(x0), .O(new_n42_));
  inv1   g25(.a(x1), .O(new_n43_));
  xnor2a g26(.a(x8), .b(x7), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x9), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x2), .O(new_n47_));
  inv1   g30(.a(x4), .O(new_n48_));
  nand3  g31(.a(x7), .b(new_n48_), .c(x3), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x8), .O(new_n50_));
  nand4  g33(.a(x9), .b(x8), .c(new_n24_), .d(x5), .O(new_n51_));
  inv1   g34(.a(new_n51_), .O(new_n52_));
  aoi21  g35(.a(new_n50_), .b(new_n19_), .c(new_n52_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n47_), .c(new_n38_), .O(z5));
  nand3  g37(.a(new_n35_), .b(x9), .c(x7), .O(new_n55_));
  nand3  g38(.a(new_n19_), .b(new_n22_), .c(new_n24_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n18_), .O(new_n58_));
  nand4  g41(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n59_));
  oai21  g42(.a(new_n19_), .b(x8), .c(new_n59_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x7), .O(new_n61_));
  nor2   g44(.a(new_n52_), .b(new_n30_), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n61_), .c(new_n58_), .d(new_n38_), .O(z6));
endmodule


