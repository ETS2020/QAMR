// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:12 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  oai21  g03(.a(new_n19_), .b(x5), .c(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  nor2   g07(.a(new_n19_), .b(new_n23_), .O(new_n25_));
  aoi21  g08(.a(new_n24_), .b(new_n22_), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n21_), .O(z1));
  nand2  g10(.a(x7), .b(x5), .O(new_n28_));
  oai21  g11(.a(new_n24_), .b(x7), .c(new_n28_), .O(z2));
  oai21  g12(.a(x9), .b(new_n23_), .c(new_n22_), .O(new_n30_));
  oai21  g13(.a(new_n22_), .b(x5), .c(new_n23_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x9), .O(new_n32_));
  nor3   g15(.a(x2), .b(x1), .c(x0), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n30_), .d(new_n21_), .O(z3));
  oai21  g17(.a(x9), .b(new_n22_), .c(x8), .O(new_n35_));
  inv1   g18(.a(x6), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n18_), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n35_), .c(new_n30_), .d(new_n28_), .O(z4));
  inv1   g21(.a(x0), .O(new_n39_));
  inv1   g22(.a(x1), .O(new_n40_));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  oai21  g24(.a(x8), .b(x7), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n40_), .d(new_n39_), .O(new_n43_));
  oai21  g26(.a(new_n25_), .b(x7), .c(x5), .O(new_n44_));
  nand2  g27(.a(new_n28_), .b(new_n36_), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand3  g29(.a(x7), .b(new_n46_), .c(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x8), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nand4  g32(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(z5));
  nand2  g33(.a(new_n25_), .b(x5), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n24_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n22_), .O(new_n53_));
  nor2   g36(.a(x2), .b(x1), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n39_), .c(new_n23_), .O(new_n55_));
  nand4  g38(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  oai21  g39(.a(new_n55_), .b(new_n19_), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(x7), .c(new_n18_), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n53_), .c(new_n45_), .O(z6));
endmodule


