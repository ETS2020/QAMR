// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  oai21  g04(.a(new_n20_), .b(x2), .c(new_n21_), .O(new_n22_));
  nand2  g05(.a(x9), .b(x7), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n22_), .c(new_n20_), .O(z0));
  inv1   g07(.a(x2), .O(new_n25_));
  nand3  g08(.a(new_n20_), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  oai21  g09(.a(new_n19_), .b(x8), .c(x7), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n26_), .O(z1));
  nand4  g11(.a(new_n19_), .b(new_n18_), .c(new_n21_), .d(new_n25_), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(z2));
  nand2  g13(.a(new_n19_), .b(x8), .O(new_n31_));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  aoi21  g17(.a(new_n31_), .b(new_n21_), .c(new_n34_), .O(new_n35_));
  oai21  g18(.a(new_n35_), .b(x2), .c(new_n21_), .O(z3));
  oai21  g19(.a(x7), .b(x2), .c(new_n23_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(x8), .O(new_n38_));
  inv1   g21(.a(x6), .O(new_n39_));
  nand3  g22(.a(new_n19_), .b(x8), .c(new_n25_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(new_n21_), .c(new_n39_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n38_), .O(z4));
  nand4  g25(.a(x9), .b(x8), .c(new_n33_), .d(new_n32_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x7), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x2), .O(new_n45_));
  inv1   g28(.a(x4), .O(new_n46_));
  nand3  g29(.a(x7), .b(new_n46_), .c(x3), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(x8), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n19_), .O(new_n49_));
  nand2  g32(.a(x9), .b(x8), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  inv1   g34(.a(x5), .O(new_n52_));
  nor2   g35(.a(x7), .b(new_n52_), .O(new_n53_));
  aoi21  g36(.a(new_n53_), .b(new_n51_), .c(new_n39_), .O(new_n54_));
  nand3  g37(.a(new_n54_), .b(new_n49_), .c(new_n45_), .O(z5));
  nand4  g38(.a(x9), .b(x7), .c(new_n33_), .d(new_n32_), .O(new_n56_));
  nand3  g39(.a(new_n19_), .b(new_n18_), .c(new_n21_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n25_), .O(new_n59_));
  nand2  g42(.a(x9), .b(new_n18_), .O(new_n60_));
  nand4  g43(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(x7), .O(new_n63_));
  oai21  g46(.a(new_n50_), .b(new_n52_), .c(new_n25_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(new_n21_), .c(new_n39_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(new_n63_), .c(new_n59_), .O(z6));
endmodule


