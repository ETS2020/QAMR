// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:51 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n26_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  inv1   g03(.a(x7), .O(new_n21_));
  nor2   g04(.a(x9), .b(new_n21_), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(x8), .c(new_n18_), .O(new_n23_));
  oai21  g06(.a(x8), .b(new_n21_), .c(x9), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n23_), .O(z1));
  inv1   g08(.a(x8), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(new_n21_), .O(new_n27_));
  aoi21  g10(.a(new_n27_), .b(new_n18_), .c(x9), .O(z2));
  oai21  g11(.a(new_n22_), .b(new_n26_), .c(new_n18_), .O(new_n29_));
  nand2  g12(.a(new_n26_), .b(new_n21_), .O(new_n30_));
  oai21  g13(.a(new_n30_), .b(new_n21_), .c(x9), .O(new_n31_));
  nor2   g14(.a(x1), .b(x0), .O(new_n32_));
  aoi21  g15(.a(new_n19_), .b(x6), .c(x2), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n29_), .O(z3));
  nand2  g17(.a(new_n24_), .b(x6), .O(z4));
  inv1   g18(.a(x0), .O(new_n36_));
  inv1   g19(.a(x1), .O(new_n37_));
  nand2  g20(.a(x8), .b(x7), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n27_), .O(new_n39_));
  nand4  g22(.a(new_n39_), .b(x2), .c(new_n37_), .d(new_n36_), .O(new_n40_));
  nand4  g23(.a(x9), .b(x8), .c(new_n21_), .d(x5), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(x9), .d(x6), .O(z5));
  inv1   g25(.a(x2), .O(new_n43_));
  nand3  g26(.a(new_n43_), .b(new_n37_), .c(new_n36_), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(x8), .c(new_n21_), .O(new_n45_));
  nand3  g28(.a(x8), .b(new_n21_), .c(x5), .O(new_n46_));
  inv1   g29(.a(new_n46_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n45_), .c(x9), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x6), .O(z6));
endmodule


