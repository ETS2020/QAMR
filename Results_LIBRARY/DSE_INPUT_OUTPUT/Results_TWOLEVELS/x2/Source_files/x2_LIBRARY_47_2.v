// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:39 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n25_,
    new_n26_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_;
  nand2  g00(.a(x9), .b(x8), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x7), .O(new_n19_));
  inv1   g02(.a(x7), .O(new_n20_));
  oai21  g03(.a(x9), .b(new_n20_), .c(x8), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  oai21  g05(.a(x9), .b(new_n22_), .c(new_n20_), .O(new_n23_));
  nand3  g06(.a(new_n23_), .b(new_n21_), .c(new_n19_), .O(z0));
  nor2   g07(.a(x9), .b(x8), .O(new_n25_));
  nand2  g08(.a(x9), .b(x8), .O(new_n26_));
  oai21  g09(.a(new_n25_), .b(x7), .c(new_n26_), .O(z1));
  aoi21  g10(.a(x8), .b(new_n20_), .c(x9), .O(z2));
  xnor2a g11(.a(x9), .b(x7), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n22_), .O(new_n30_));
  oai21  g13(.a(x8), .b(new_n20_), .c(x9), .O(new_n31_));
  inv1   g14(.a(x0), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  inv1   g16(.a(x2), .O(new_n34_));
  nand3  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n20_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n31_), .c(new_n30_), .O(z3));
  inv1   g20(.a(x6), .O(new_n38_));
  nand2  g21(.a(x9), .b(new_n38_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n23_), .c(new_n21_), .O(z4));
  nand2  g23(.a(new_n22_), .b(new_n20_), .O(new_n41_));
  oai21  g24(.a(new_n26_), .b(new_n20_), .c(new_n41_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n33_), .d(new_n32_), .O(new_n43_));
  oai21  g26(.a(x9), .b(new_n20_), .c(new_n38_), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(x5), .O(new_n45_));
  inv1   g28(.a(new_n45_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n25_), .c(new_n20_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n44_), .c(new_n43_), .O(z5));
  aoi21  g31(.a(new_n35_), .b(x8), .c(new_n20_), .O(new_n49_));
  nand3  g32(.a(x8), .b(new_n20_), .c(x5), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  oai21  g34(.a(new_n51_), .b(new_n49_), .c(x9), .O(new_n52_));
  nand2  g35(.a(new_n25_), .b(new_n20_), .O(new_n53_));
  nand3  g36(.a(new_n53_), .b(new_n52_), .c(new_n44_), .O(z6));
endmodule


