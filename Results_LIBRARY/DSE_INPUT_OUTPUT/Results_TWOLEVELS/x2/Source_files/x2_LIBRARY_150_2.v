// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:56 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x2), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x0), .O(new_n20_));
  nand4  g03(.a(new_n20_), .b(new_n18_), .c(x8), .d(x7), .O(z0));
  inv1   g04(.a(x8), .O(new_n22_));
  nand2  g05(.a(new_n18_), .b(x7), .O(new_n23_));
  inv1   g06(.a(x7), .O(new_n24_));
  nand2  g07(.a(x9), .b(new_n24_), .O(new_n25_));
  nand4  g08(.a(new_n25_), .b(new_n23_), .c(new_n20_), .d(new_n22_), .O(z1));
  nand3  g09(.a(new_n18_), .b(new_n22_), .c(new_n24_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n20_), .O(z2));
  nor2   g11(.a(new_n18_), .b(new_n22_), .O(new_n29_));
  oai21  g12(.a(new_n29_), .b(new_n24_), .c(x2), .O(new_n30_));
  oai21  g13(.a(new_n18_), .b(new_n22_), .c(x7), .O(new_n31_));
  oai21  g14(.a(x9), .b(new_n22_), .c(new_n24_), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  inv1   g16(.a(x0), .O(new_n34_));
  nand2  g17(.a(new_n29_), .b(new_n34_), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(new_n20_), .c(new_n33_), .O(new_n36_));
  inv1   g19(.a(new_n36_), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z3));
  nor2   g21(.a(new_n29_), .b(new_n24_), .O(new_n39_));
  aoi22  g22(.a(new_n39_), .b(x6), .c(new_n19_), .d(x0), .O(z4));
  nand3  g23(.a(x9), .b(x8), .c(x7), .O(new_n41_));
  oai21  g24(.a(x8), .b(x7), .c(new_n41_), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(x2), .c(new_n33_), .O(new_n43_));
  inv1   g26(.a(x4), .O(new_n44_));
  nand3  g27(.a(x7), .b(new_n44_), .c(x3), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x8), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(new_n18_), .O(new_n47_));
  inv1   g30(.a(x6), .O(new_n48_));
  inv1   g31(.a(x5), .O(new_n49_));
  nor2   g32(.a(x7), .b(new_n49_), .O(new_n50_));
  aoi21  g33(.a(new_n50_), .b(new_n29_), .c(new_n48_), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n47_), .c(new_n43_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand2  g36(.a(new_n51_), .b(new_n47_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x2), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n53_), .O(z5));
  nand2  g39(.a(new_n50_), .b(new_n29_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x6), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n20_), .O(new_n59_));
  nand2  g42(.a(x9), .b(x7), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(x1), .c(new_n34_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n19_), .O(new_n62_));
  nand4  g45(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n63_));
  oai21  g46(.a(new_n18_), .b(x8), .c(new_n63_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(x7), .O(new_n65_));
  nand4  g48(.a(new_n65_), .b(new_n62_), .c(new_n59_), .d(new_n27_), .O(z6));
endmodule


