// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_,
    new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x4), .O(z0));
  inv1   g02(.a(x4), .O(new_n20_));
  oai21  g03(.a(new_n18_), .b(new_n20_), .c(x7), .O(new_n21_));
  inv1   g04(.a(x7), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n18_), .b(new_n23_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n22_), .O(new_n25_));
  nor2   g08(.a(new_n18_), .b(new_n23_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x4), .O(new_n27_));
  nand3  g10(.a(new_n27_), .b(new_n25_), .c(new_n21_), .O(z1));
  nand2  g11(.a(x7), .b(new_n20_), .O(new_n29_));
  oai21  g12(.a(new_n24_), .b(x7), .c(new_n29_), .O(z2));
  nand2  g13(.a(new_n18_), .b(x8), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n22_), .O(new_n32_));
  nand2  g15(.a(x8), .b(x4), .O(new_n33_));
  oai21  g16(.a(x8), .b(new_n22_), .c(new_n33_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x9), .O(new_n35_));
  inv1   g18(.a(x0), .O(new_n36_));
  inv1   g19(.a(x1), .O(new_n37_));
  inv1   g20(.a(x2), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  inv1   g22(.a(new_n39_), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n35_), .c(new_n32_), .d(new_n21_), .O(z3));
  oai21  g24(.a(new_n31_), .b(x8), .c(new_n22_), .O(new_n42_));
  inv1   g25(.a(x6), .O(new_n43_));
  oai21  g26(.a(new_n26_), .b(new_n43_), .c(x4), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n42_), .O(z4));
  nand4  g28(.a(x9), .b(x8), .c(x7), .d(x4), .O(new_n46_));
  oai21  g29(.a(x8), .b(x7), .c(new_n46_), .O(new_n47_));
  nand4  g30(.a(new_n47_), .b(x2), .c(new_n37_), .d(new_n36_), .O(new_n48_));
  nand2  g31(.a(new_n24_), .b(x6), .O(new_n49_));
  inv1   g32(.a(x5), .O(new_n50_));
  nor2   g33(.a(x7), .b(new_n50_), .O(new_n51_));
  aoi22  g34(.a(new_n51_), .b(new_n26_), .c(new_n49_), .d(new_n29_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n48_), .O(z5));
  nand2  g36(.a(new_n29_), .b(new_n43_), .O(new_n54_));
  nand3  g37(.a(x9), .b(x8), .c(x5), .O(new_n55_));
  nand2  g38(.a(new_n55_), .b(new_n24_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n22_), .O(new_n57_));
  aoi21  g40(.a(new_n39_), .b(x8), .c(new_n18_), .O(new_n58_));
  nand3  g41(.a(new_n18_), .b(x8), .c(x3), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(x4), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n58_), .c(x7), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n57_), .c(new_n54_), .O(z6));
endmodule


