// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:00 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n31_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_;
  inv1   g00(.a(x7), .O(new_n18_));
  nand2  g01(.a(x8), .b(new_n18_), .O(new_n19_));
  inv1   g02(.a(new_n19_), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(x6), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n18_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n18_), .c(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n22_), .O(z0));
  nor2   g09(.a(x9), .b(new_n18_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n20_), .c(x6), .O(new_n28_));
  oai21  g11(.a(x8), .b(new_n18_), .c(x9), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n28_), .O(z1));
  nand2  g13(.a(new_n23_), .b(new_n18_), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(x6), .c(x9), .O(z2));
  inv1   g15(.a(x9), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(x8), .O(new_n34_));
  inv1   g17(.a(x6), .O(new_n35_));
  nor2   g18(.a(x9), .b(new_n35_), .O(new_n36_));
  oai21  g19(.a(new_n36_), .b(new_n34_), .c(x7), .O(new_n37_));
  oai21  g20(.a(x8), .b(new_n35_), .c(new_n33_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n18_), .O(new_n39_));
  inv1   g22(.a(x0), .O(new_n40_));
  inv1   g23(.a(x1), .O(new_n41_));
  inv1   g24(.a(x2), .O(new_n42_));
  nand3  g25(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(new_n43_));
  nor2   g26(.a(new_n33_), .b(new_n23_), .O(new_n44_));
  aoi21  g27(.a(new_n43_), .b(x6), .c(new_n44_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n39_), .c(new_n37_), .O(z3));
  aoi21  g29(.a(x9), .b(new_n23_), .c(new_n35_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(x9), .c(new_n18_), .O(new_n48_));
  nor2   g31(.a(x8), .b(new_n35_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n33_), .c(new_n48_), .O(z4));
  nand3  g33(.a(x9), .b(x8), .c(x7), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n31_), .O(new_n52_));
  nand4  g35(.a(new_n52_), .b(x2), .c(new_n41_), .d(new_n40_), .O(new_n53_));
  inv1   g36(.a(x4), .O(new_n54_));
  nand3  g37(.a(x7), .b(new_n54_), .c(x3), .O(new_n55_));
  inv1   g38(.a(new_n55_), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n49_), .c(new_n33_), .O(new_n57_));
  nand3  g40(.a(new_n44_), .b(new_n18_), .c(x5), .O(new_n58_));
  nand4  g41(.a(new_n58_), .b(new_n57_), .c(new_n53_), .d(x6), .O(z5));
  nand4  g42(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n31_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n33_), .c(x6), .O(new_n62_));
  aoi21  g45(.a(new_n43_), .b(x8), .c(new_n18_), .O(new_n63_));
  inv1   g46(.a(x5), .O(new_n64_));
  oai21  g47(.a(new_n19_), .b(new_n64_), .c(x6), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n63_), .c(x9), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n62_), .O(z6));
endmodule


