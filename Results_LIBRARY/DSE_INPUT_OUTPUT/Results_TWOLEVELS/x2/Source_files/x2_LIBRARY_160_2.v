// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:58 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_, new_n34_,
    new_n35_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x5), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nor2   g02(.a(new_n19_), .b(x7), .O(new_n20_));
  nor2   g03(.a(x9), .b(x8), .O(new_n21_));
  oai21  g04(.a(new_n21_), .b(new_n20_), .c(new_n18_), .O(new_n22_));
  inv1   g05(.a(x7), .O(new_n23_));
  nand2  g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(new_n23_), .c(x9), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n22_), .O(z0));
  nor2   g09(.a(x9), .b(new_n23_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n20_), .c(new_n18_), .O(new_n28_));
  oai21  g11(.a(x8), .b(new_n23_), .c(x9), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n28_), .O(z1));
  inv1   g13(.a(x9), .O(new_n31_));
  nand4  g14(.a(new_n31_), .b(new_n19_), .c(new_n23_), .d(new_n18_), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(z2));
  nand3  g16(.a(x8), .b(new_n23_), .c(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nor3   g18(.a(x2), .b(x1), .c(x0), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n35_), .c(new_n25_), .O(z3));
  aoi21  g20(.a(x9), .b(new_n19_), .c(x5), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(x9), .c(new_n23_), .O(new_n39_));
  nand2  g22(.a(new_n31_), .b(x5), .O(new_n40_));
  nand2  g23(.a(x9), .b(x8), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(new_n40_), .c(new_n39_), .d(x6), .O(z4));
  inv1   g25(.a(x0), .O(new_n43_));
  inv1   g26(.a(x1), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(x7), .O(new_n45_));
  oai21  g28(.a(x8), .b(x7), .c(new_n45_), .O(new_n46_));
  nand4  g29(.a(new_n46_), .b(x2), .c(new_n44_), .d(new_n43_), .O(new_n47_));
  oai21  g30(.a(new_n20_), .b(new_n31_), .c(x5), .O(new_n48_));
  inv1   g31(.a(x4), .O(new_n49_));
  nand3  g32(.a(x7), .b(new_n49_), .c(x3), .O(new_n50_));
  nand2  g33(.a(new_n19_), .b(new_n18_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n31_), .O(new_n53_));
  nand4  g36(.a(new_n53_), .b(new_n48_), .c(new_n47_), .d(x6), .O(z5));
  nor2   g37(.a(new_n31_), .b(new_n23_), .O(new_n55_));
  nor3   g38(.a(x9), .b(x7), .c(x5), .O(new_n56_));
  oai21  g39(.a(new_n56_), .b(new_n55_), .c(new_n19_), .O(new_n57_));
  inv1   g40(.a(x2), .O(new_n58_));
  nand4  g41(.a(x9), .b(new_n58_), .c(new_n44_), .d(new_n43_), .O(new_n59_));
  nand4  g42(.a(new_n31_), .b(x8), .c(x4), .d(x3), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(x7), .O(new_n62_));
  nand4  g45(.a(new_n62_), .b(new_n57_), .c(new_n48_), .d(x6), .O(z6));
endmodule


