// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n27_, new_n29_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand4  g01(.a(new_n18_), .b(x8), .c(x7), .d(x6), .O(z0));
  inv1   g02(.a(x7), .O(new_n20_));
  nand2  g03(.a(x8), .b(new_n20_), .O(new_n21_));
  oai21  g04(.a(x9), .b(new_n20_), .c(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x6), .O(new_n23_));
  inv1   g06(.a(x6), .O(new_n24_));
  inv1   g07(.a(x8), .O(new_n25_));
  aoi21  g08(.a(new_n25_), .b(x7), .c(new_n18_), .O(new_n26_));
  aoi21  g09(.a(new_n18_), .b(new_n24_), .c(new_n26_), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n23_), .O(z1));
  nand2  g11(.a(new_n25_), .b(new_n20_), .O(new_n29_));
  aoi21  g12(.a(new_n29_), .b(x6), .c(x9), .O(z2));
  nor2   g13(.a(new_n18_), .b(x8), .O(new_n31_));
  nor2   g14(.a(x9), .b(new_n24_), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(x7), .O(new_n33_));
  oai21  g16(.a(x8), .b(new_n24_), .c(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(new_n20_), .O(new_n35_));
  nor2   g18(.a(new_n18_), .b(new_n25_), .O(new_n36_));
  inv1   g19(.a(x0), .O(new_n37_));
  inv1   g20(.a(x1), .O(new_n38_));
  inv1   g21(.a(x2), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n38_), .c(new_n37_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x6), .c(new_n36_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n35_), .c(new_n33_), .O(z3));
  aoi21  g25(.a(x9), .b(new_n25_), .c(new_n24_), .O(new_n43_));
  oai21  g26(.a(new_n43_), .b(x9), .c(new_n20_), .O(new_n44_));
  oai21  g27(.a(x8), .b(new_n24_), .c(x9), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n44_), .O(z4));
  nand3  g29(.a(x9), .b(x8), .c(x7), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n29_), .O(new_n48_));
  nand4  g31(.a(new_n48_), .b(x2), .c(new_n38_), .d(new_n37_), .O(new_n49_));
  inv1   g32(.a(x4), .O(new_n50_));
  nand3  g33(.a(x7), .b(new_n50_), .c(x3), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x8), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n18_), .O(new_n53_));
  nand3  g36(.a(new_n36_), .b(new_n20_), .c(x5), .O(new_n54_));
  nand4  g37(.a(new_n54_), .b(new_n53_), .c(new_n49_), .d(x6), .O(z5));
  nand4  g38(.a(x8), .b(x7), .c(x4), .d(x3), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n29_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n18_), .c(x6), .O(new_n58_));
  aoi21  g41(.a(new_n40_), .b(x8), .c(new_n20_), .O(new_n59_));
  inv1   g42(.a(x5), .O(new_n60_));
  oai21  g43(.a(new_n21_), .b(new_n60_), .c(x6), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n59_), .c(x9), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n58_), .O(z6));
endmodule


