// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:47 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n23_, new_n24_, new_n25_,
    new_n27_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x7), .O(new_n19_));
  inv1   g02(.a(x8), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(x3), .c(new_n21_), .O(z0));
  oai21  g05(.a(x9), .b(x8), .c(new_n19_), .O(new_n23_));
  oai21  g06(.a(x8), .b(x3), .c(x9), .O(new_n24_));
  nand2  g07(.a(new_n18_), .b(x7), .O(new_n25_));
  nand3  g08(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(z1));
  nand2  g09(.a(new_n20_), .b(new_n19_), .O(new_n27_));
  nor2   g10(.a(new_n27_), .b(x9), .O(z2));
  inv1   g11(.a(x0), .O(new_n29_));
  inv1   g12(.a(x1), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n30_), .c(new_n29_), .O(new_n32_));
  inv1   g15(.a(new_n32_), .O(new_n33_));
  nand4  g16(.a(new_n33_), .b(new_n18_), .c(x8), .d(new_n19_), .O(z3));
  oai21  g17(.a(x9), .b(new_n19_), .c(x8), .O(new_n35_));
  oai21  g18(.a(x9), .b(new_n20_), .c(new_n19_), .O(new_n36_));
  inv1   g19(.a(x6), .O(new_n37_));
  aoi21  g20(.a(x9), .b(x3), .c(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(new_n36_), .c(new_n35_), .O(z4));
  nand3  g22(.a(x9), .b(x8), .c(x7), .O(new_n40_));
  nand2  g23(.a(new_n40_), .b(new_n27_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(x2), .c(new_n30_), .d(new_n29_), .O(new_n42_));
  oai21  g25(.a(new_n19_), .b(x4), .c(new_n18_), .O(new_n43_));
  nand2  g26(.a(new_n18_), .b(new_n20_), .O(new_n44_));
  nand4  g27(.a(x9), .b(x8), .c(new_n19_), .d(x5), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(new_n44_), .c(x6), .O(new_n46_));
  aoi21  g29(.a(new_n43_), .b(x3), .c(new_n46_), .O(new_n47_));
  nand2  g30(.a(new_n47_), .b(new_n42_), .O(z5));
  nand3  g31(.a(x8), .b(x7), .c(x4), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(new_n18_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x3), .O(new_n51_));
  aoi21  g34(.a(new_n32_), .b(x8), .c(new_n19_), .O(new_n52_));
  nand3  g35(.a(x8), .b(new_n19_), .c(x5), .O(new_n53_));
  inv1   g36(.a(new_n53_), .O(new_n54_));
  oai21  g37(.a(new_n54_), .b(new_n52_), .c(x9), .O(new_n55_));
  nor2   g38(.a(new_n44_), .b(x7), .O(new_n56_));
  nor2   g39(.a(new_n56_), .b(new_n37_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n55_), .c(new_n51_), .O(z6));
endmodule


