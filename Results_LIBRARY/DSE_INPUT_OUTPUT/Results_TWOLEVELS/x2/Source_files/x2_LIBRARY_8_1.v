// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n28_, new_n29_, new_n30_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_;
  inv1   g00(.a(x8), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  oai21  g02(.a(new_n19_), .b(new_n18_), .c(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(x2), .O(new_n21_));
  oai21  g04(.a(new_n19_), .b(x7), .c(new_n18_), .O(new_n22_));
  inv1   g05(.a(x2), .O(new_n23_));
  nand2  g06(.a(x9), .b(new_n23_), .O(new_n24_));
  inv1   g07(.a(x7), .O(new_n25_));
  nand3  g08(.a(new_n19_), .b(x8), .c(new_n25_), .O(new_n26_));
  nand4  g09(.a(new_n26_), .b(new_n24_), .c(new_n22_), .d(new_n21_), .O(z0));
  oai21  g10(.a(x9), .b(x8), .c(new_n25_), .O(new_n28_));
  oai21  g11(.a(x8), .b(new_n23_), .c(x9), .O(new_n29_));
  nand2  g12(.a(new_n19_), .b(x7), .O(new_n30_));
  nand3  g13(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(z1));
  nor3   g14(.a(x9), .b(x8), .c(x7), .O(z2));
  oai21  g15(.a(new_n19_), .b(new_n18_), .c(x7), .O(new_n33_));
  nor2   g16(.a(x1), .b(x0), .O(new_n34_));
  nor2   g17(.a(new_n19_), .b(x2), .O(new_n35_));
  nor3   g18(.a(x9), .b(x8), .c(x7), .O(new_n36_));
  nor2   g19(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n34_), .c(new_n33_), .d(new_n21_), .O(z3));
  inv1   g21(.a(x6), .O(new_n39_));
  nand2  g22(.a(new_n24_), .b(new_n39_), .O(new_n40_));
  nand2  g23(.a(new_n19_), .b(new_n25_), .O(new_n41_));
  nand3  g24(.a(new_n41_), .b(new_n40_), .c(new_n21_), .O(z4));
  inv1   g25(.a(x0), .O(new_n43_));
  inv1   g26(.a(x1), .O(new_n44_));
  nand3  g27(.a(x9), .b(x8), .c(x7), .O(new_n45_));
  oai21  g28(.a(x8), .b(x7), .c(new_n45_), .O(new_n46_));
  nand3  g29(.a(new_n46_), .b(new_n44_), .c(new_n43_), .O(new_n47_));
  nand3  g30(.a(x8), .b(new_n25_), .c(x5), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x2), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x9), .O(new_n50_));
  inv1   g33(.a(x4), .O(new_n51_));
  nand3  g34(.a(x7), .b(new_n51_), .c(x3), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(x8), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n19_), .O(new_n54_));
  nand4  g37(.a(new_n54_), .b(new_n50_), .c(new_n47_), .d(new_n40_), .O(z5));
  nand2  g38(.a(x9), .b(new_n18_), .O(new_n56_));
  nand4  g39(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(x7), .O(new_n59_));
  nand3  g42(.a(x9), .b(x8), .c(x5), .O(new_n60_));
  oai21  g43(.a(x9), .b(x8), .c(new_n60_), .O(new_n61_));
  aoi21  g44(.a(new_n61_), .b(new_n25_), .c(new_n35_), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(new_n59_), .c(new_n40_), .O(z6));
endmodule


