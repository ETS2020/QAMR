// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n24_, new_n25_, new_n27_,
    new_n28_, new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_;
  inv1   g00(.a(x9), .O(new_n18_));
  nand2  g01(.a(x2), .b(x0), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(new_n18_), .c(x8), .d(x7), .O(z0));
  inv1   g03(.a(x8), .O(new_n21_));
  xnor2a g04(.a(x9), .b(x7), .O(new_n22_));
  aoi22  g05(.a(new_n22_), .b(new_n21_), .c(x2), .d(x0), .O(z1));
  inv1   g06(.a(x7), .O(new_n24_));
  nand3  g07(.a(new_n18_), .b(new_n21_), .c(new_n24_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(new_n19_), .O(z2));
  nor2   g09(.a(new_n18_), .b(x8), .O(new_n27_));
  nor2   g10(.a(x9), .b(x0), .O(new_n28_));
  oai21  g11(.a(new_n28_), .b(new_n27_), .c(x7), .O(new_n29_));
  inv1   g12(.a(x0), .O(new_n30_));
  inv1   g13(.a(x2), .O(new_n31_));
  oai22  g14(.a(new_n18_), .b(x7), .c(new_n21_), .d(new_n31_), .O(new_n32_));
  inv1   g15(.a(x1), .O(new_n33_));
  nand2  g16(.a(x9), .b(x8), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n25_), .c(new_n33_), .d(new_n30_), .O(new_n35_));
  aoi21  g18(.a(new_n32_), .b(new_n30_), .c(new_n35_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(new_n29_), .O(z3));
  nand4  g20(.a(new_n34_), .b(new_n19_), .c(x7), .d(x6), .O(z4));
  nand3  g21(.a(x9), .b(x8), .c(x7), .O(new_n39_));
  oai21  g22(.a(x8), .b(x7), .c(new_n39_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x2), .c(new_n33_), .O(new_n41_));
  inv1   g24(.a(x4), .O(new_n42_));
  nand3  g25(.a(x7), .b(new_n42_), .c(x3), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x8), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n18_), .O(new_n45_));
  inv1   g28(.a(x6), .O(new_n46_));
  inv1   g29(.a(new_n34_), .O(new_n47_));
  inv1   g30(.a(x5), .O(new_n48_));
  nor2   g31(.a(x7), .b(new_n48_), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n47_), .c(new_n46_), .O(new_n50_));
  nand3  g33(.a(new_n50_), .b(new_n45_), .c(new_n41_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  nand2  g35(.a(new_n50_), .b(new_n45_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n31_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n52_), .O(z5));
  xnor2a g38(.a(x9), .b(x7), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n21_), .O(new_n57_));
  nand4  g40(.a(x9), .b(new_n31_), .c(new_n33_), .d(new_n30_), .O(new_n58_));
  nand4  g41(.a(new_n18_), .b(x8), .c(x4), .d(x3), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g43(.a(new_n60_), .b(x7), .O(new_n61_));
  nand4  g44(.a(new_n61_), .b(new_n57_), .c(new_n50_), .d(new_n19_), .O(z6));
endmodule


