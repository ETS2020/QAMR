// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:01 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n26_, new_n27_, new_n28_, new_n30_, new_n32_, new_n33_, new_n34_,
    new_n35_, new_n36_, new_n37_, new_n38_, new_n39_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_;
  inv1   g00(.a(x6), .O(new_n18_));
  nand2  g01(.a(x8), .b(x7), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(new_n18_), .O(new_n20_));
  inv1   g03(.a(x7), .O(new_n21_));
  inv1   g04(.a(x8), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n21_), .c(x9), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(new_n20_), .O(z0));
  inv1   g08(.a(x9), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(x7), .c(x8), .O(new_n27_));
  oai21  g10(.a(x8), .b(new_n21_), .c(x9), .O(new_n28_));
  oai21  g11(.a(new_n27_), .b(x6), .c(new_n28_), .O(z1));
  nand2  g12(.a(new_n22_), .b(new_n21_), .O(new_n30_));
  aoi21  g13(.a(new_n30_), .b(new_n18_), .c(x9), .O(z2));
  nor2   g14(.a(new_n26_), .b(x8), .O(new_n32_));
  nor2   g15(.a(x9), .b(x6), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n32_), .c(x7), .O(new_n34_));
  inv1   g17(.a(x0), .O(new_n35_));
  inv1   g18(.a(x1), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  nand4  g20(.a(x8), .b(new_n37_), .c(new_n36_), .d(new_n35_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(new_n18_), .O(new_n39_));
  nand3  g22(.a(new_n39_), .b(new_n34_), .c(new_n28_), .O(z3));
  nand2  g23(.a(new_n28_), .b(x6), .O(z4));
  nand2  g24(.a(new_n30_), .b(new_n19_), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(x2), .c(new_n36_), .d(new_n35_), .O(new_n43_));
  nand4  g26(.a(x9), .b(x8), .c(new_n21_), .d(x5), .O(new_n44_));
  nand4  g27(.a(new_n44_), .b(new_n43_), .c(x9), .d(x6), .O(z5));
  nand3  g28(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(x8), .c(new_n21_), .O(new_n47_));
  nand3  g30(.a(x8), .b(new_n21_), .c(x5), .O(new_n48_));
  inv1   g31(.a(new_n48_), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n47_), .c(x9), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(x6), .O(z6));
endmodule


