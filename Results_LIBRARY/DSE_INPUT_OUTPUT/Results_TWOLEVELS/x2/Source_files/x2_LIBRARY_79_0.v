// Benchmark "FAU" written by ABC on Wed Aug 19 19:36:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n20_, new_n22_, new_n23_, new_n26_, new_n28_,
    new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_;
  inv1   g00(.a(x9), .O(new_n18_));
  inv1   g01(.a(x8), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x7), .O(new_n20_));
  nand2  g03(.a(new_n20_), .b(new_n18_), .O(z0));
  inv1   g04(.a(x7), .O(new_n22_));
  nand2  g05(.a(new_n20_), .b(x9), .O(new_n23_));
  oai21  g06(.a(x9), .b(new_n22_), .c(new_n23_), .O(z1));
  oai21  g07(.a(new_n18_), .b(new_n19_), .c(x7), .O(new_n26_));
  nand2  g08(.a(new_n26_), .b(new_n23_), .O(z3));
  nand2  g09(.a(x9), .b(x8), .O(new_n28_));
  nand3  g10(.a(new_n28_), .b(x7), .c(x6), .O(z4));
  inv1   g11(.a(x0), .O(new_n30_));
  inv1   g12(.a(x1), .O(new_n31_));
  nand3  g13(.a(x9), .b(x8), .c(x7), .O(new_n32_));
  oai21  g14(.a(x8), .b(x7), .c(new_n32_), .O(new_n33_));
  nand4  g15(.a(new_n33_), .b(x2), .c(new_n31_), .d(new_n30_), .O(new_n34_));
  inv1   g16(.a(x6), .O(new_n35_));
  oai21  g17(.a(x9), .b(x7), .c(new_n35_), .O(new_n36_));
  inv1   g18(.a(x5), .O(new_n37_));
  oai21  g19(.a(new_n19_), .b(new_n37_), .c(x9), .O(new_n38_));
  nand2  g20(.a(new_n38_), .b(new_n22_), .O(new_n39_));
  nor2   g21(.a(x8), .b(new_n22_), .O(new_n40_));
  inv1   g22(.a(x3), .O(new_n41_));
  nor2   g23(.a(x4), .b(new_n41_), .O(new_n42_));
  oai21  g24(.a(new_n42_), .b(new_n40_), .c(new_n18_), .O(new_n43_));
  nand4  g25(.a(new_n43_), .b(new_n39_), .c(new_n36_), .d(new_n34_), .O(z5));
  nand3  g26(.a(x9), .b(new_n22_), .c(x5), .O(new_n45_));
  nand4  g27(.a(new_n18_), .b(x7), .c(x4), .d(x3), .O(new_n46_));
  nand2  g28(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g29(.a(new_n47_), .b(x8), .O(new_n48_));
  inv1   g30(.a(x2), .O(new_n49_));
  nand3  g31(.a(new_n49_), .b(new_n31_), .c(new_n30_), .O(new_n50_));
  nand2  g32(.a(new_n50_), .b(x8), .O(new_n51_));
  nand3  g33(.a(new_n51_), .b(x9), .c(x7), .O(new_n52_));
  nand3  g34(.a(new_n52_), .b(new_n48_), .c(new_n36_), .O(z6));
  zero   g35(.O(z2));
endmodule


