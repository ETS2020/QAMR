// Benchmark "FAU" written by ABC on Wed Aug 19 19:37:06 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5, z6  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5, z6;
  wire new_n18_, new_n19_, new_n21_, new_n22_, new_n23_, new_n24_, new_n25_,
    new_n26_, new_n28_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_;
  inv1   g00(.a(x1), .O(new_n18_));
  inv1   g01(.a(x9), .O(new_n19_));
  nand4  g02(.a(new_n19_), .b(x8), .c(x7), .d(new_n18_), .O(z0));
  inv1   g03(.a(x7), .O(new_n21_));
  oai21  g04(.a(x9), .b(x8), .c(new_n21_), .O(new_n22_));
  inv1   g05(.a(x8), .O(new_n23_));
  nor2   g06(.a(new_n19_), .b(new_n23_), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  oai21  g08(.a(x7), .b(x1), .c(new_n19_), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n25_), .c(new_n22_), .O(z1));
  nand4  g10(.a(new_n19_), .b(new_n23_), .c(new_n21_), .d(new_n18_), .O(new_n28_));
  inv1   g11(.a(new_n28_), .O(z2));
  oai21  g12(.a(new_n19_), .b(new_n23_), .c(x7), .O(new_n30_));
  oai21  g13(.a(x8), .b(x1), .c(new_n19_), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(new_n21_), .O(new_n32_));
  aoi21  g15(.a(new_n19_), .b(x1), .c(x0), .O(new_n33_));
  nor2   g16(.a(new_n24_), .b(x2), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(new_n33_), .c(new_n32_), .d(new_n30_), .O(z3));
  nand2  g18(.a(new_n19_), .b(x1), .O(new_n36_));
  oai21  g19(.a(x9), .b(new_n21_), .c(x8), .O(new_n37_));
  nand4  g20(.a(new_n37_), .b(new_n32_), .c(new_n36_), .d(x6), .O(z4));
  inv1   g21(.a(x0), .O(new_n39_));
  nand3  g22(.a(x9), .b(x8), .c(x7), .O(new_n40_));
  oai21  g23(.a(x8), .b(x7), .c(new_n40_), .O(new_n41_));
  nand4  g24(.a(new_n41_), .b(x2), .c(new_n18_), .d(new_n39_), .O(new_n42_));
  inv1   g25(.a(x4), .O(new_n43_));
  nand3  g26(.a(x7), .b(new_n43_), .c(x3), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(x8), .c(new_n18_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(new_n19_), .O(new_n46_));
  nand4  g29(.a(x9), .b(x8), .c(new_n21_), .d(x5), .O(new_n47_));
  and2   g30(.a(new_n47_), .b(x6), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n42_), .O(z5));
  inv1   g32(.a(x2), .O(new_n50_));
  nand4  g33(.a(x9), .b(x7), .c(new_n50_), .d(new_n39_), .O(new_n51_));
  nand3  g34(.a(new_n19_), .b(new_n23_), .c(new_n21_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  nand2  g37(.a(x9), .b(new_n23_), .O(new_n55_));
  nand4  g38(.a(new_n19_), .b(x8), .c(x4), .d(x3), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x7), .O(new_n58_));
  nand3  g41(.a(new_n47_), .b(new_n36_), .c(x6), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n58_), .c(new_n54_), .O(z6));
endmodule


