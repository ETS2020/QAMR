// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n150_,
    new_n151_, new_n154_, new_n155_, new_n156_, new_n157_, new_n158_,
    new_n159_, new_n160_, new_n161_, new_n162_, new_n163_, new_n164_,
    new_n165_, new_n166_, new_n167_, new_n168_, new_n169_, new_n170_,
    new_n171_, new_n172_, new_n173_, new_n174_, new_n175_, new_n176_,
    new_n177_, new_n178_, new_n179_, new_n180_, new_n182_, new_n183_,
    new_n184_;
  inv1   g000(.a(x4), .O(new_n17_));
  inv1   g001(.a(x9), .O(new_n18_));
  inv1   g002(.a(x6), .O(new_n19_));
  inv1   g003(.a(x7), .O(new_n20_));
  oai21  g004(.a(new_n19_), .b(x4), .c(new_n20_), .O(new_n21_));
  nand3  g005(.a(new_n21_), .b(x5), .c(x2), .O(new_n22_));
  inv1   g006(.a(x2), .O(new_n23_));
  nor2   g007(.a(new_n19_), .b(new_n17_), .O(new_n24_));
  nor2   g008(.a(new_n20_), .b(x5), .O(new_n25_));
  oai21  g009(.a(new_n25_), .b(new_n24_), .c(new_n23_), .O(new_n26_));
  nand2  g010(.a(x7), .b(x6), .O(new_n27_));
  nand3  g011(.a(new_n27_), .b(new_n26_), .c(new_n22_), .O(new_n28_));
  nand2  g012(.a(new_n28_), .b(x0), .O(new_n29_));
  inv1   g013(.a(x0), .O(new_n30_));
  oai21  g014(.a(new_n19_), .b(x4), .c(x5), .O(new_n31_));
  nand3  g015(.a(new_n31_), .b(new_n20_), .c(new_n30_), .O(new_n32_));
  oai21  g016(.a(new_n19_), .b(x5), .c(new_n32_), .O(new_n33_));
  nand2  g017(.a(new_n33_), .b(new_n23_), .O(new_n34_));
  oai21  g018(.a(new_n25_), .b(new_n17_), .c(new_n19_), .O(new_n35_));
  oai22  g019(.a(new_n35_), .b(new_n23_), .c(x5), .d(x4), .O(new_n36_));
  nand2  g020(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand3  g021(.a(x7), .b(new_n17_), .c(x2), .O(new_n38_));
  nand4  g022(.a(new_n38_), .b(new_n37_), .c(new_n34_), .d(new_n29_), .O(new_n39_));
  nand2  g023(.a(new_n39_), .b(new_n18_), .O(new_n40_));
  inv1   g024(.a(x8), .O(new_n41_));
  nand2  g025(.a(new_n20_), .b(new_n19_), .O(new_n42_));
  oai21  g026(.a(new_n41_), .b(x0), .c(new_n42_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(x5), .O(new_n44_));
  nand2  g028(.a(x8), .b(x7), .O(new_n45_));
  inv1   g029(.a(x5), .O(new_n46_));
  nand4  g030(.a(new_n41_), .b(new_n20_), .c(x6), .d(new_n46_), .O(new_n47_));
  aoi21  g031(.a(new_n47_), .b(new_n45_), .c(new_n23_), .O(new_n48_));
  nor2   g032(.a(new_n41_), .b(new_n19_), .O(new_n49_));
  nor2   g033(.a(x8), .b(x6), .O(new_n50_));
  oai21  g034(.a(new_n50_), .b(new_n49_), .c(new_n20_), .O(new_n51_));
  nand2  g035(.a(new_n41_), .b(x7), .O(new_n52_));
  inv1   g036(.a(new_n52_), .O(new_n53_));
  nand2  g037(.a(new_n53_), .b(new_n46_), .O(new_n54_));
  aoi21  g038(.a(new_n54_), .b(new_n51_), .c(x2), .O(new_n55_));
  oai21  g039(.a(new_n55_), .b(new_n48_), .c(new_n30_), .O(new_n56_));
  nor2   g040(.a(x6), .b(x2), .O(new_n57_));
  aoi21  g041(.a(new_n20_), .b(x2), .c(new_n57_), .O(new_n58_));
  oai22  g042(.a(new_n58_), .b(new_n30_), .c(new_n42_), .d(new_n23_), .O(new_n59_));
  nor2   g043(.a(new_n23_), .b(new_n30_), .O(new_n60_));
  aoi22  g044(.a(new_n60_), .b(new_n53_), .c(new_n59_), .d(x8), .O(new_n61_));
  nand3  g045(.a(new_n61_), .b(new_n56_), .c(new_n44_), .O(new_n62_));
  nor2   g046(.a(x2), .b(new_n30_), .O(new_n63_));
  nor2   g047(.a(x8), .b(new_n19_), .O(new_n64_));
  aoi22  g048(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(x9), .O(new_n65_));
  oai21  g049(.a(new_n65_), .b(new_n17_), .c(new_n40_), .O(z0));
  inv1   g050(.a(x3), .O(new_n67_));
  inv1   g051(.a(x1), .O(new_n68_));
  nand2  g052(.a(x2), .b(new_n68_), .O(new_n69_));
  nand3  g053(.a(x9), .b(x8), .c(new_n20_), .O(new_n70_));
  nor2   g054(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  oai21  g055(.a(new_n71_), .b(new_n25_), .c(x0), .O(new_n72_));
  nand3  g056(.a(new_n46_), .b(x2), .c(x1), .O(new_n73_));
  nand4  g057(.a(x9), .b(x8), .c(new_n23_), .d(new_n68_), .O(new_n74_));
  aoi21  g058(.a(new_n74_), .b(new_n73_), .c(x0), .O(new_n75_));
  nand3  g059(.a(new_n41_), .b(new_n46_), .c(x1), .O(new_n76_));
  inv1   g060(.a(new_n76_), .O(new_n77_));
  oai21  g061(.a(new_n77_), .b(new_n75_), .c(new_n20_), .O(new_n78_));
  nor2   g062(.a(x5), .b(new_n23_), .O(new_n79_));
  oai21  g063(.a(x8), .b(new_n46_), .c(x9), .O(new_n80_));
  aoi22  g064(.a(new_n80_), .b(x1), .c(new_n79_), .d(new_n53_), .O(new_n81_));
  nand3  g065(.a(new_n81_), .b(new_n78_), .c(new_n72_), .O(new_n82_));
  nand3  g066(.a(x3), .b(new_n23_), .c(new_n68_), .O(new_n83_));
  nand3  g067(.a(x9), .b(new_n20_), .c(new_n46_), .O(new_n84_));
  oai22  g068(.a(new_n84_), .b(new_n83_), .c(new_n20_), .d(new_n68_), .O(new_n85_));
  nand2  g069(.a(new_n85_), .b(x0), .O(new_n86_));
  nor2   g070(.a(x5), .b(new_n67_), .O(new_n87_));
  nor2   g071(.a(new_n18_), .b(x8), .O(new_n88_));
  nand4  g072(.a(new_n88_), .b(new_n87_), .c(new_n20_), .d(new_n68_), .O(new_n89_));
  nand2  g073(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  aoi21  g074(.a(new_n82_), .b(new_n67_), .c(new_n90_), .O(new_n91_));
  nand3  g075(.a(x5), .b(new_n17_), .c(x3), .O(new_n92_));
  aoi21  g076(.a(new_n92_), .b(new_n20_), .c(new_n68_), .O(new_n93_));
  nand3  g077(.a(new_n20_), .b(new_n17_), .c(new_n68_), .O(new_n94_));
  aoi21  g078(.a(new_n94_), .b(x5), .c(x3), .O(new_n95_));
  oai21  g079(.a(new_n95_), .b(new_n93_), .c(new_n18_), .O(new_n96_));
  oai21  g080(.a(new_n91_), .b(new_n17_), .c(new_n96_), .O(new_n97_));
  nand2  g081(.a(new_n97_), .b(x6), .O(new_n98_));
  nand2  g082(.a(x7), .b(x2), .O(new_n99_));
  oai21  g083(.a(x8), .b(x6), .c(new_n99_), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(x0), .O(new_n101_));
  nand2  g085(.a(new_n50_), .b(x2), .O(new_n102_));
  aoi21  g086(.a(new_n102_), .b(new_n101_), .c(x3), .O(new_n103_));
  nand4  g087(.a(x7), .b(x3), .c(new_n23_), .d(new_n30_), .O(new_n104_));
  inv1   g088(.a(new_n104_), .O(new_n105_));
  oai21  g089(.a(new_n105_), .b(new_n103_), .c(new_n46_), .O(new_n106_));
  aoi21  g090(.a(x3), .b(x2), .c(x5), .O(new_n107_));
  nand2  g091(.a(new_n46_), .b(new_n67_), .O(new_n108_));
  nand3  g092(.a(new_n108_), .b(new_n19_), .c(new_n23_), .O(new_n109_));
  oai21  g093(.a(new_n107_), .b(x0), .c(new_n109_), .O(new_n110_));
  nand2  g094(.a(new_n110_), .b(x8), .O(new_n111_));
  nand2  g095(.a(new_n111_), .b(new_n106_), .O(new_n112_));
  nand3  g096(.a(new_n112_), .b(x9), .c(x4), .O(new_n113_));
  aoi21  g097(.a(new_n20_), .b(new_n67_), .c(new_n17_), .O(new_n114_));
  oai22  g098(.a(new_n114_), .b(x5), .c(new_n35_), .d(new_n67_), .O(new_n115_));
  nand2  g099(.a(new_n115_), .b(new_n18_), .O(new_n116_));
  nand2  g100(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand2  g101(.a(new_n117_), .b(new_n68_), .O(new_n118_));
  nand2  g102(.a(x8), .b(x2), .O(new_n119_));
  aoi21  g103(.a(new_n119_), .b(new_n52_), .c(new_n30_), .O(new_n120_));
  nand2  g104(.a(new_n23_), .b(new_n30_), .O(new_n121_));
  nand2  g105(.a(x8), .b(new_n20_), .O(new_n122_));
  oai22  g106(.a(new_n122_), .b(new_n121_), .c(new_n52_), .d(new_n23_), .O(new_n123_));
  oai21  g107(.a(new_n123_), .b(new_n120_), .c(x3), .O(new_n124_));
  oai21  g108(.a(x5), .b(x2), .c(new_n41_), .O(new_n125_));
  nand3  g109(.a(new_n125_), .b(new_n19_), .c(new_n30_), .O(new_n126_));
  nand3  g110(.a(x8), .b(new_n23_), .c(x0), .O(new_n127_));
  nand2  g111(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g112(.a(new_n128_), .b(new_n67_), .O(new_n129_));
  nand3  g113(.a(x8), .b(new_n20_), .c(new_n19_), .O(new_n130_));
  nand3  g114(.a(new_n130_), .b(new_n129_), .c(new_n124_), .O(new_n131_));
  nand3  g115(.a(new_n53_), .b(x5), .c(x3), .O(new_n132_));
  inv1   g116(.a(new_n132_), .O(new_n133_));
  aoi21  g117(.a(new_n131_), .b(x9), .c(new_n133_), .O(new_n134_));
  nand2  g118(.a(x5), .b(x3), .O(new_n135_));
  nand2  g119(.a(new_n135_), .b(new_n108_), .O(new_n136_));
  nand3  g120(.a(new_n136_), .b(new_n18_), .c(x7), .O(new_n137_));
  oai21  g121(.a(new_n134_), .b(new_n17_), .c(new_n137_), .O(new_n138_));
  nand2  g122(.a(new_n138_), .b(x1), .O(new_n139_));
  nand3  g123(.a(x8), .b(x3), .c(x2), .O(new_n140_));
  nand2  g124(.a(new_n140_), .b(x6), .O(new_n141_));
  nand2  g125(.a(new_n141_), .b(x5), .O(new_n142_));
  nand2  g126(.a(new_n121_), .b(new_n41_), .O(new_n143_));
  nand3  g127(.a(new_n143_), .b(new_n19_), .c(x3), .O(new_n144_));
  nand2  g128(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand4  g129(.a(new_n145_), .b(x9), .c(new_n20_), .d(x4), .O(new_n146_));
  nand4  g130(.a(new_n18_), .b(x7), .c(new_n17_), .d(x3), .O(new_n147_));
  and2   g131(.a(new_n147_), .b(new_n146_), .O(new_n148_));
  nand4  g132(.a(new_n148_), .b(new_n139_), .c(new_n118_), .d(new_n98_), .O(z1));
  nand2  g133(.a(x9), .b(new_n17_), .O(new_n150_));
  xor2a  g134(.a(x3), .b(x1), .O(new_n151_));
  nand2  g135(.a(new_n151_), .b(new_n150_), .O(z2));
  oai21  g136(.a(new_n67_), .b(new_n68_), .c(new_n150_), .O(z3));
  aoi21  g137(.a(new_n67_), .b(x1), .c(new_n23_), .O(new_n154_));
  oai21  g138(.a(new_n67_), .b(x0), .c(x6), .O(new_n155_));
  oai21  g139(.a(new_n155_), .b(new_n154_), .c(new_n20_), .O(new_n156_));
  oai21  g140(.a(x6), .b(x3), .c(x1), .O(new_n157_));
  nand2  g141(.a(new_n157_), .b(new_n30_), .O(new_n158_));
  oai21  g142(.a(new_n41_), .b(x1), .c(x3), .O(new_n159_));
  nand3  g143(.a(new_n159_), .b(new_n19_), .c(new_n23_), .O(new_n160_));
  nand3  g144(.a(new_n160_), .b(new_n158_), .c(new_n156_), .O(new_n161_));
  nand2  g145(.a(new_n161_), .b(x5), .O(new_n162_));
  nand3  g146(.a(new_n20_), .b(x6), .c(new_n46_), .O(new_n163_));
  nand2  g147(.a(new_n63_), .b(new_n50_), .O(new_n164_));
  aoi21  g148(.a(new_n164_), .b(new_n163_), .c(x1), .O(new_n165_));
  oai21  g149(.a(new_n57_), .b(new_n30_), .c(new_n68_), .O(new_n166_));
  nand2  g150(.a(new_n166_), .b(x7), .O(new_n167_));
  nand2  g151(.a(new_n41_), .b(x2), .O(new_n168_));
  aoi21  g152(.a(new_n168_), .b(new_n167_), .c(x5), .O(new_n169_));
  oai21  g153(.a(new_n169_), .b(new_n165_), .c(x3), .O(new_n170_));
  aoi21  g154(.a(new_n99_), .b(x8), .c(new_n30_), .O(new_n171_));
  oai21  g155(.a(new_n19_), .b(x3), .c(new_n168_), .O(new_n172_));
  oai21  g156(.a(new_n172_), .b(new_n171_), .c(x1), .O(new_n173_));
  nor2   g157(.a(new_n63_), .b(new_n41_), .O(new_n174_));
  oai21  g158(.a(new_n23_), .b(x0), .c(new_n174_), .O(new_n175_));
  nand3  g159(.a(new_n175_), .b(new_n20_), .c(x6), .O(new_n176_));
  nand2  g160(.a(new_n176_), .b(new_n173_), .O(new_n177_));
  nand2  g161(.a(new_n177_), .b(new_n46_), .O(new_n178_));
  nand3  g162(.a(new_n178_), .b(new_n170_), .c(new_n162_), .O(new_n179_));
  nand3  g163(.a(new_n179_), .b(x9), .c(x4), .O(new_n180_));
  inv1   g164(.a(new_n180_), .O(z4));
  inv1   g165(.a(new_n60_), .O(new_n182_));
  aoi21  g166(.a(new_n121_), .b(new_n182_), .c(new_n151_), .O(new_n183_));
  nand2  g167(.a(new_n183_), .b(new_n150_), .O(new_n184_));
  inv1   g168(.a(new_n184_), .O(z5));
endmodule


