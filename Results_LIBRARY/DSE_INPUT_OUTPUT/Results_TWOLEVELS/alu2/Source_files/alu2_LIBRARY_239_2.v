// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:52 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n199_, new_n201_, new_n203_, new_n204_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n254_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n260_;
  inv1   g000(.a(x5), .O(new_n17_));
  inv1   g001(.a(x0), .O(new_n18_));
  inv1   g002(.a(x6), .O(new_n19_));
  inv1   g003(.a(x8), .O(new_n20_));
  nor2   g004(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  nand2  g005(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  inv1   g006(.a(x7), .O(new_n23_));
  nand3  g007(.a(x9), .b(new_n23_), .c(new_n19_), .O(new_n24_));
  aoi21  g008(.a(new_n24_), .b(new_n22_), .c(new_n17_), .O(new_n25_));
  nor2   g009(.a(x8), .b(x7), .O(new_n26_));
  nand3  g010(.a(new_n26_), .b(x6), .c(new_n17_), .O(new_n27_));
  inv1   g011(.a(x9), .O(new_n28_));
  nor2   g012(.a(new_n28_), .b(new_n20_), .O(new_n29_));
  nand2  g013(.a(new_n29_), .b(x7), .O(new_n30_));
  aoi21  g014(.a(new_n30_), .b(new_n27_), .c(x0), .O(new_n31_));
  nand3  g015(.a(new_n29_), .b(new_n23_), .c(new_n19_), .O(new_n32_));
  inv1   g016(.a(new_n32_), .O(new_n33_));
  oai21  g017(.a(new_n33_), .b(new_n31_), .c(x2), .O(new_n34_));
  nor2   g018(.a(x8), .b(new_n23_), .O(new_n35_));
  nand3  g019(.a(new_n35_), .b(x6), .c(x0), .O(new_n36_));
  nand2  g020(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  oai21  g021(.a(new_n37_), .b(new_n25_), .c(x4), .O(new_n38_));
  inv1   g022(.a(x2), .O(new_n39_));
  nand2  g023(.a(x7), .b(new_n17_), .O(new_n40_));
  nand3  g024(.a(new_n40_), .b(new_n20_), .c(new_n39_), .O(new_n41_));
  inv1   g025(.a(x4), .O(new_n42_));
  nand2  g026(.a(x9), .b(x7), .O(new_n43_));
  oai21  g027(.a(new_n43_), .b(x5), .c(new_n39_), .O(new_n44_));
  nand3  g028(.a(new_n44_), .b(x8), .c(new_n42_), .O(new_n45_));
  aoi21  g029(.a(new_n45_), .b(new_n41_), .c(new_n19_), .O(new_n46_));
  nand2  g030(.a(new_n29_), .b(new_n19_), .O(new_n47_));
  nand2  g031(.a(new_n28_), .b(x7), .O(new_n48_));
  oai21  g032(.a(new_n48_), .b(x5), .c(new_n47_), .O(new_n49_));
  nand2  g033(.a(new_n49_), .b(new_n39_), .O(new_n50_));
  nand3  g034(.a(x9), .b(new_n20_), .c(new_n19_), .O(new_n51_));
  nand2  g035(.a(new_n28_), .b(x5), .O(new_n52_));
  aoi21  g036(.a(new_n52_), .b(new_n51_), .c(new_n23_), .O(new_n53_));
  nand2  g037(.a(new_n29_), .b(new_n23_), .O(new_n54_));
  inv1   g038(.a(new_n54_), .O(new_n55_));
  oai21  g039(.a(new_n55_), .b(new_n53_), .c(x2), .O(new_n56_));
  nand2  g040(.a(new_n56_), .b(new_n50_), .O(new_n57_));
  oai21  g041(.a(new_n57_), .b(new_n46_), .c(x0), .O(new_n58_));
  nand2  g042(.a(new_n20_), .b(new_n42_), .O(new_n59_));
  nand2  g043(.a(new_n29_), .b(new_n39_), .O(new_n60_));
  oai21  g044(.a(new_n59_), .b(new_n39_), .c(new_n60_), .O(new_n61_));
  nand2  g045(.a(new_n61_), .b(x5), .O(new_n62_));
  aoi21  g046(.a(new_n40_), .b(x4), .c(new_n39_), .O(new_n63_));
  nor3   g047(.a(x7), .b(x5), .c(x2), .O(new_n64_));
  oai21  g048(.a(new_n64_), .b(new_n63_), .c(new_n28_), .O(new_n65_));
  nand2  g049(.a(new_n23_), .b(x6), .O(new_n66_));
  oai21  g050(.a(new_n28_), .b(x2), .c(x4), .O(new_n67_));
  nand4  g051(.a(new_n67_), .b(new_n66_), .c(new_n20_), .d(new_n17_), .O(new_n68_));
  nand2  g052(.a(x6), .b(new_n39_), .O(new_n69_));
  nand2  g053(.a(x8), .b(new_n23_), .O(new_n70_));
  or2    g054(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g055(.a(new_n71_), .b(new_n68_), .c(new_n65_), .d(new_n62_), .O(new_n72_));
  nand2  g056(.a(x6), .b(x2), .O(new_n73_));
  oai22  g057(.a(new_n73_), .b(new_n54_), .c(new_n48_), .d(x6), .O(new_n74_));
  nand3  g058(.a(new_n74_), .b(new_n17_), .c(new_n42_), .O(new_n75_));
  nand2  g059(.a(new_n28_), .b(x6), .O(new_n76_));
  nand2  g060(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  aoi21  g061(.a(new_n72_), .b(new_n18_), .c(new_n77_), .O(new_n78_));
  nand3  g062(.a(new_n78_), .b(new_n58_), .c(new_n38_), .O(z0));
  nand2  g063(.a(new_n20_), .b(new_n39_), .O(new_n80_));
  nand2  g064(.a(new_n80_), .b(x9), .O(new_n81_));
  nand3  g065(.a(new_n81_), .b(x5), .c(x3), .O(new_n82_));
  inv1   g066(.a(x3), .O(new_n83_));
  nand3  g067(.a(new_n28_), .b(new_n17_), .c(new_n83_), .O(new_n84_));
  nand2  g068(.a(new_n84_), .b(new_n82_), .O(new_n85_));
  nand2  g069(.a(new_n85_), .b(x7), .O(new_n86_));
  nor2   g070(.a(x5), .b(x0), .O(new_n87_));
  oai21  g071(.a(new_n87_), .b(x8), .c(new_n39_), .O(new_n88_));
  oai21  g072(.a(x4), .b(new_n39_), .c(new_n20_), .O(new_n89_));
  nand2  g073(.a(new_n89_), .b(new_n18_), .O(new_n90_));
  aoi21  g074(.a(new_n90_), .b(new_n88_), .c(x3), .O(new_n91_));
  nand3  g075(.a(new_n17_), .b(new_n42_), .c(new_n18_), .O(new_n92_));
  aoi21  g076(.a(new_n92_), .b(new_n20_), .c(x7), .O(new_n93_));
  oai21  g077(.a(new_n93_), .b(new_n91_), .c(x9), .O(new_n94_));
  aoi21  g078(.a(new_n94_), .b(new_n86_), .c(x6), .O(new_n95_));
  inv1   g079(.a(new_n35_), .O(new_n96_));
  nor2   g080(.a(x7), .b(new_n17_), .O(new_n97_));
  nand2  g081(.a(new_n97_), .b(new_n42_), .O(new_n98_));
  oai21  g082(.a(new_n96_), .b(new_n19_), .c(new_n98_), .O(new_n99_));
  nand2  g083(.a(new_n99_), .b(x2), .O(new_n100_));
  oai22  g084(.a(x7), .b(x2), .c(new_n19_), .d(x4), .O(new_n101_));
  nand2  g085(.a(new_n101_), .b(x8), .O(new_n102_));
  aoi21  g086(.a(new_n102_), .b(new_n100_), .c(x0), .O(new_n103_));
  nand3  g087(.a(x8), .b(x2), .c(x0), .O(new_n104_));
  oai21  g088(.a(new_n96_), .b(new_n42_), .c(new_n104_), .O(new_n105_));
  nand2  g089(.a(new_n105_), .b(x5), .O(new_n106_));
  nand3  g090(.a(x8), .b(x4), .c(x2), .O(new_n107_));
  oai21  g091(.a(new_n96_), .b(x5), .c(new_n107_), .O(new_n108_));
  nand2  g092(.a(new_n108_), .b(x0), .O(new_n109_));
  nand3  g093(.a(new_n35_), .b(x4), .c(x2), .O(new_n110_));
  nand3  g094(.a(new_n110_), .b(new_n109_), .c(new_n106_), .O(new_n111_));
  oai21  g095(.a(new_n111_), .b(new_n103_), .c(x3), .O(new_n112_));
  nand2  g096(.a(x7), .b(new_n42_), .O(new_n113_));
  nand4  g097(.a(new_n23_), .b(x4), .c(new_n83_), .d(x2), .O(new_n114_));
  aoi21  g098(.a(new_n114_), .b(new_n113_), .c(x0), .O(new_n115_));
  inv1   g099(.a(new_n26_), .O(new_n116_));
  nor2   g100(.a(new_n116_), .b(x3), .O(new_n117_));
  oai21  g101(.a(new_n117_), .b(new_n115_), .c(new_n17_), .O(new_n118_));
  nand2  g102(.a(x5), .b(new_n39_), .O(new_n119_));
  aoi21  g103(.a(new_n119_), .b(x8), .c(new_n18_), .O(new_n120_));
  nand2  g104(.a(x4), .b(x2), .O(new_n121_));
  aoi21  g105(.a(new_n119_), .b(new_n121_), .c(x8), .O(new_n122_));
  oai21  g106(.a(new_n122_), .b(new_n120_), .c(new_n83_), .O(new_n123_));
  nand3  g107(.a(x7), .b(x4), .c(x0), .O(new_n124_));
  nand3  g108(.a(new_n124_), .b(new_n123_), .c(new_n118_), .O(new_n125_));
  nor4   g109(.a(new_n70_), .b(x3), .c(x2), .d(new_n18_), .O(new_n126_));
  aoi21  g110(.a(new_n125_), .b(x6), .c(new_n126_), .O(new_n127_));
  aoi21  g111(.a(new_n127_), .b(new_n112_), .c(new_n28_), .O(new_n128_));
  oai21  g112(.a(new_n128_), .b(new_n95_), .c(x1), .O(new_n129_));
  inv1   g113(.a(x1), .O(new_n130_));
  nand3  g114(.a(new_n29_), .b(x4), .c(new_n18_), .O(new_n131_));
  nor2   g115(.a(x9), .b(x6), .O(new_n132_));
  nand2  g116(.a(new_n132_), .b(new_n17_), .O(new_n133_));
  nand2  g117(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g118(.a(new_n134_), .b(x7), .O(new_n135_));
  nand3  g119(.a(x8), .b(x6), .c(x2), .O(new_n136_));
  inv1   g120(.a(new_n136_), .O(new_n137_));
  nor3   g121(.a(x8), .b(x5), .c(x2), .O(new_n138_));
  oai21  g122(.a(new_n138_), .b(new_n137_), .c(new_n18_), .O(new_n139_));
  inv1   g123(.a(new_n27_), .O(new_n140_));
  nand4  g124(.a(new_n23_), .b(x6), .c(new_n17_), .d(x0), .O(new_n141_));
  nand2  g125(.a(x8), .b(new_n19_), .O(new_n142_));
  nand2  g126(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  aoi21  g127(.a(new_n143_), .b(new_n39_), .c(new_n140_), .O(new_n144_));
  aoi21  g128(.a(new_n144_), .b(new_n139_), .c(new_n42_), .O(new_n145_));
  aoi21  g129(.a(x6), .b(new_n39_), .c(new_n20_), .O(new_n146_));
  oai21  g130(.a(new_n146_), .b(new_n18_), .c(new_n80_), .O(new_n147_));
  nand3  g131(.a(new_n147_), .b(x5), .c(new_n42_), .O(new_n148_));
  inv1   g132(.a(new_n148_), .O(new_n149_));
  oai21  g133(.a(new_n149_), .b(new_n145_), .c(x9), .O(new_n150_));
  nand2  g134(.a(new_n132_), .b(new_n42_), .O(new_n151_));
  nand3  g135(.a(new_n151_), .b(new_n150_), .c(new_n135_), .O(new_n152_));
  nand2  g136(.a(new_n152_), .b(x3), .O(new_n153_));
  oai21  g137(.a(x7), .b(x6), .c(new_n40_), .O(new_n154_));
  nand3  g138(.a(new_n154_), .b(new_n20_), .c(x4), .O(new_n155_));
  nand3  g139(.a(x8), .b(x6), .c(x0), .O(new_n156_));
  oai21  g140(.a(new_n17_), .b(x0), .c(new_n156_), .O(new_n157_));
  nand2  g141(.a(new_n157_), .b(new_n42_), .O(new_n158_));
  aoi21  g142(.a(new_n66_), .b(new_n40_), .c(new_n18_), .O(new_n159_));
  oai21  g143(.a(new_n159_), .b(new_n97_), .c(x8), .O(new_n160_));
  nand3  g144(.a(new_n160_), .b(new_n158_), .c(new_n155_), .O(new_n161_));
  oai21  g145(.a(new_n66_), .b(x2), .c(new_n17_), .O(new_n162_));
  nand3  g146(.a(new_n162_), .b(x8), .c(new_n18_), .O(new_n163_));
  nor2   g147(.a(x8), .b(x6), .O(new_n164_));
  nand3  g148(.a(new_n164_), .b(new_n17_), .c(x0), .O(new_n165_));
  nand2  g149(.a(new_n165_), .b(new_n163_), .O(new_n166_));
  aoi21  g150(.a(new_n161_), .b(x2), .c(new_n166_), .O(new_n167_));
  xor2a  g151(.a(x7), .b(x0), .O(new_n168_));
  nor2   g152(.a(new_n168_), .b(new_n20_), .O(new_n169_));
  nand4  g153(.a(new_n169_), .b(x6), .c(new_n17_), .d(new_n42_), .O(new_n170_));
  oai21  g154(.a(new_n167_), .b(x3), .c(new_n170_), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(x9), .O(new_n172_));
  nand3  g156(.a(new_n28_), .b(new_n23_), .c(new_n83_), .O(new_n173_));
  oai21  g157(.a(new_n59_), .b(new_n18_), .c(new_n173_), .O(new_n174_));
  nand3  g158(.a(new_n174_), .b(new_n19_), .c(new_n17_), .O(new_n175_));
  nand3  g159(.a(new_n175_), .b(new_n172_), .c(new_n153_), .O(new_n176_));
  nand2  g160(.a(new_n176_), .b(new_n130_), .O(new_n177_));
  nand2  g161(.a(x3), .b(x2), .O(new_n178_));
  nand2  g162(.a(new_n19_), .b(new_n42_), .O(new_n179_));
  nand2  g163(.a(x4), .b(new_n83_), .O(new_n180_));
  nand3  g164(.a(x9), .b(x6), .c(new_n17_), .O(new_n181_));
  oai22  g165(.a(new_n181_), .b(new_n180_), .c(new_n179_), .d(new_n178_), .O(new_n182_));
  nand2  g166(.a(new_n182_), .b(x0), .O(new_n183_));
  nand3  g167(.a(new_n132_), .b(new_n17_), .c(new_n42_), .O(new_n184_));
  aoi21  g168(.a(new_n184_), .b(new_n183_), .c(new_n23_), .O(new_n185_));
  oai22  g169(.a(new_n20_), .b(new_n17_), .c(x6), .d(x2), .O(new_n186_));
  nand2  g170(.a(x5), .b(x2), .O(new_n187_));
  aoi21  g171(.a(new_n187_), .b(x6), .c(new_n20_), .O(new_n188_));
  aoi21  g172(.a(new_n186_), .b(new_n18_), .c(new_n188_), .O(new_n189_));
  nand4  g173(.a(new_n21_), .b(new_n17_), .c(new_n42_), .d(x2), .O(new_n190_));
  oai21  g174(.a(new_n189_), .b(new_n42_), .c(new_n190_), .O(new_n191_));
  nor2   g175(.a(x6), .b(new_n17_), .O(new_n192_));
  aoi22  g176(.a(new_n192_), .b(x4), .c(new_n191_), .d(x3), .O(new_n193_));
  inv1   g177(.a(new_n142_), .O(new_n194_));
  nand4  g178(.a(new_n194_), .b(x5), .c(new_n83_), .d(new_n39_), .O(new_n195_));
  oai21  g179(.a(new_n193_), .b(x7), .c(new_n195_), .O(new_n196_));
  aoi21  g180(.a(new_n196_), .b(x9), .c(new_n185_), .O(new_n197_));
  nand3  g181(.a(new_n197_), .b(new_n177_), .c(new_n129_), .O(z1));
  xor2a  g182(.a(x3), .b(x1), .O(new_n199_));
  nand2  g183(.a(new_n199_), .b(new_n76_), .O(z2));
  nand2  g184(.a(x3), .b(x1), .O(new_n201_));
  nand2  g185(.a(new_n201_), .b(new_n76_), .O(z3));
  xor2a  g186(.a(x2), .b(x0), .O(new_n203_));
  nor2   g187(.a(new_n203_), .b(new_n199_), .O(new_n204_));
  nand2  g188(.a(new_n204_), .b(x6), .O(new_n205_));
  aoi21  g189(.a(new_n178_), .b(x6), .c(new_n42_), .O(new_n206_));
  nor2   g190(.a(x2), .b(new_n130_), .O(new_n207_));
  aoi21  g191(.a(new_n207_), .b(new_n164_), .c(new_n206_), .O(new_n208_));
  aoi21  g192(.a(new_n208_), .b(new_n205_), .c(x7), .O(new_n209_));
  nand2  g193(.a(new_n19_), .b(x2), .O(new_n210_));
  nand2  g194(.a(new_n210_), .b(new_n69_), .O(new_n211_));
  aoi22  g195(.a(new_n211_), .b(x0), .c(x6), .d(x1), .O(new_n212_));
  oai22  g196(.a(new_n212_), .b(x3), .c(new_n80_), .d(new_n130_), .O(new_n213_));
  nand3  g197(.a(new_n164_), .b(new_n83_), .c(x0), .O(new_n214_));
  inv1   g198(.a(new_n214_), .O(new_n215_));
  aoi21  g199(.a(new_n213_), .b(x7), .c(new_n215_), .O(new_n216_));
  nor2   g200(.a(x8), .b(x3), .O(new_n217_));
  aoi21  g201(.a(new_n217_), .b(new_n39_), .c(x4), .O(new_n218_));
  nand3  g202(.a(new_n19_), .b(x4), .c(new_n83_), .O(new_n219_));
  oai21  g203(.a(new_n218_), .b(x1), .c(new_n219_), .O(new_n220_));
  nand4  g204(.a(new_n201_), .b(new_n19_), .c(x4), .d(new_n39_), .O(new_n221_));
  inv1   g205(.a(new_n221_), .O(new_n222_));
  aoi21  g206(.a(new_n220_), .b(new_n18_), .c(new_n222_), .O(new_n223_));
  oai21  g207(.a(new_n216_), .b(x4), .c(new_n223_), .O(new_n224_));
  oai21  g208(.a(new_n224_), .b(new_n209_), .c(x5), .O(new_n225_));
  aoi21  g209(.a(x7), .b(x2), .c(new_n20_), .O(new_n226_));
  aoi21  g210(.a(x4), .b(x3), .c(x1), .O(new_n227_));
  nand3  g211(.a(x7), .b(x4), .c(x3), .O(new_n228_));
  inv1   g212(.a(new_n228_), .O(new_n229_));
  oai21  g213(.a(x7), .b(x2), .c(new_n59_), .O(new_n230_));
  oai21  g214(.a(new_n230_), .b(new_n229_), .c(x6), .O(new_n231_));
  oai21  g215(.a(new_n227_), .b(new_n226_), .c(new_n231_), .O(new_n232_));
  nand2  g216(.a(new_n232_), .b(new_n17_), .O(new_n233_));
  or2    g217(.a(new_n146_), .b(new_n130_), .O(new_n234_));
  nand3  g218(.a(new_n20_), .b(x6), .c(new_n83_), .O(new_n235_));
  aoi21  g219(.a(new_n235_), .b(new_n234_), .c(new_n23_), .O(new_n236_));
  nor2   g220(.a(x6), .b(new_n130_), .O(new_n237_));
  aoi22  g221(.a(new_n237_), .b(new_n26_), .c(new_n236_), .d(new_n42_), .O(new_n238_));
  aoi21  g222(.a(new_n238_), .b(new_n233_), .c(new_n18_), .O(new_n239_));
  nand2  g223(.a(x5), .b(x4), .O(new_n240_));
  nand3  g224(.a(new_n240_), .b(new_n19_), .c(x3), .O(new_n241_));
  nand3  g225(.a(x6), .b(new_n17_), .c(x4), .O(new_n242_));
  aoi21  g226(.a(new_n242_), .b(new_n241_), .c(new_n23_), .O(new_n243_));
  nor2   g227(.a(x7), .b(new_n19_), .O(new_n244_));
  nand2  g228(.a(new_n244_), .b(new_n83_), .O(new_n245_));
  nand3  g229(.a(new_n20_), .b(x4), .c(x2), .O(new_n246_));
  aoi21  g230(.a(new_n246_), .b(new_n245_), .c(x5), .O(new_n247_));
  oai21  g231(.a(new_n247_), .b(new_n243_), .c(x1), .O(new_n248_));
  nand2  g232(.a(new_n244_), .b(new_n18_), .O(new_n249_));
  nand2  g233(.a(new_n20_), .b(x3), .O(new_n250_));
  aoi21  g234(.a(new_n250_), .b(new_n249_), .c(new_n39_), .O(new_n251_));
  nand3  g235(.a(new_n244_), .b(x3), .c(new_n130_), .O(new_n252_));
  inv1   g236(.a(new_n252_), .O(new_n253_));
  oai21  g237(.a(new_n253_), .b(new_n251_), .c(x4), .O(new_n254_));
  oai21  g238(.a(new_n116_), .b(new_n19_), .c(new_n254_), .O(new_n255_));
  nand2  g239(.a(new_n255_), .b(new_n17_), .O(new_n256_));
  nand2  g240(.a(new_n256_), .b(new_n248_), .O(new_n257_));
  nor2   g241(.a(new_n257_), .b(new_n239_), .O(new_n258_));
  aoi21  g242(.a(new_n258_), .b(new_n225_), .c(new_n28_), .O(z4));
  nand2  g243(.a(new_n204_), .b(new_n76_), .O(new_n260_));
  inv1   g244(.a(new_n260_), .O(z5));
endmodule


