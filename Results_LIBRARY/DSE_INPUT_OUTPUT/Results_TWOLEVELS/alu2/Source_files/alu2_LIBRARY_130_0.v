// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:18 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n200_, new_n202_, new_n203_, new_n205_, new_n206_,
    new_n207_, new_n208_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n254_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n259_, new_n261_,
    new_n262_, new_n263_, new_n264_;
  inv1   g000(.a(x9), .O(new_n17_));
  inv1   g001(.a(x0), .O(new_n18_));
  inv1   g002(.a(x8), .O(new_n19_));
  inv1   g003(.a(x2), .O(new_n20_));
  inv1   g004(.a(x7), .O(new_n21_));
  nand2  g005(.a(new_n21_), .b(x6), .O(new_n22_));
  nand2  g006(.a(new_n22_), .b(new_n20_), .O(new_n23_));
  inv1   g007(.a(x4), .O(new_n24_));
  inv1   g008(.a(x6), .O(new_n25_));
  nand2  g009(.a(new_n25_), .b(new_n24_), .O(new_n26_));
  inv1   g010(.a(new_n22_), .O(new_n27_));
  nor2   g011(.a(new_n24_), .b(new_n20_), .O(new_n28_));
  nand2  g012(.a(new_n28_), .b(new_n27_), .O(new_n29_));
  nand3  g013(.a(new_n29_), .b(new_n26_), .c(new_n23_), .O(new_n30_));
  nor2   g014(.a(new_n25_), .b(x4), .O(new_n31_));
  nor2   g015(.a(new_n19_), .b(x7), .O(new_n32_));
  aoi22  g016(.a(new_n32_), .b(new_n31_), .c(new_n30_), .d(new_n19_), .O(new_n33_));
  nand3  g017(.a(x8), .b(x7), .c(x4), .O(new_n34_));
  nand3  g018(.a(new_n19_), .b(x5), .c(new_n24_), .O(new_n35_));
  aoi21  g019(.a(new_n35_), .b(new_n34_), .c(new_n20_), .O(new_n36_));
  oai21  g020(.a(new_n27_), .b(x5), .c(new_n20_), .O(new_n37_));
  nand2  g021(.a(x5), .b(x4), .O(new_n38_));
  nand2  g022(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g023(.a(new_n39_), .b(x8), .c(new_n36_), .O(new_n40_));
  oai21  g024(.a(new_n33_), .b(x5), .c(new_n40_), .O(new_n41_));
  nand2  g025(.a(x8), .b(new_n20_), .O(new_n42_));
  nor2   g026(.a(x8), .b(new_n21_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(x2), .O(new_n44_));
  aoi21  g028(.a(new_n44_), .b(new_n42_), .c(new_n18_), .O(new_n45_));
  aoi21  g029(.a(x8), .b(x2), .c(x5), .O(new_n46_));
  nor3   g030(.a(new_n46_), .b(x7), .c(new_n24_), .O(new_n47_));
  oai21  g031(.a(new_n47_), .b(new_n45_), .c(new_n25_), .O(new_n48_));
  oai21  g032(.a(new_n25_), .b(x4), .c(x7), .O(new_n49_));
  nand3  g033(.a(new_n49_), .b(x8), .c(x2), .O(new_n50_));
  inv1   g034(.a(x5), .O(new_n51_));
  nand2  g035(.a(x7), .b(new_n51_), .O(new_n52_));
  nand4  g036(.a(new_n52_), .b(new_n19_), .c(x6), .d(new_n20_), .O(new_n53_));
  nand2  g037(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g038(.a(new_n54_), .b(x0), .O(new_n55_));
  nand2  g039(.a(new_n55_), .b(new_n48_), .O(new_n56_));
  aoi21  g040(.a(new_n41_), .b(new_n18_), .c(new_n56_), .O(new_n57_));
  nand2  g041(.a(new_n19_), .b(new_n24_), .O(new_n58_));
  nand3  g042(.a(new_n17_), .b(new_n25_), .c(x2), .O(new_n59_));
  nand2  g043(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand2  g044(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nor2   g045(.a(x6), .b(x0), .O(new_n62_));
  oai21  g046(.a(new_n62_), .b(x2), .c(x4), .O(new_n63_));
  nand2  g047(.a(x8), .b(x6), .O(new_n64_));
  nor2   g048(.a(new_n64_), .b(x4), .O(new_n65_));
  aoi22  g049(.a(new_n65_), .b(x0), .c(new_n63_), .d(new_n17_), .O(new_n66_));
  aoi21  g050(.a(new_n66_), .b(new_n61_), .c(x5), .O(new_n67_));
  nand2  g051(.a(x5), .b(x2), .O(new_n68_));
  aoi21  g052(.a(new_n68_), .b(new_n25_), .c(new_n18_), .O(new_n69_));
  nor2   g053(.a(x4), .b(new_n20_), .O(new_n70_));
  oai21  g054(.a(new_n70_), .b(new_n69_), .c(new_n17_), .O(new_n71_));
  nand2  g055(.a(x4), .b(x0), .O(new_n72_));
  nand2  g056(.a(new_n19_), .b(x6), .O(new_n73_));
  oai21  g057(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  oai21  g058(.a(new_n74_), .b(new_n67_), .c(x7), .O(new_n75_));
  oai21  g059(.a(new_n57_), .b(new_n17_), .c(new_n75_), .O(z0));
  inv1   g060(.a(x3), .O(new_n77_));
  inv1   g061(.a(x1), .O(new_n78_));
  nand3  g062(.a(x8), .b(x7), .c(x2), .O(new_n79_));
  nand2  g063(.a(new_n19_), .b(new_n25_), .O(new_n80_));
  aoi21  g064(.a(new_n80_), .b(new_n79_), .c(new_n18_), .O(new_n81_));
  nand2  g065(.a(new_n43_), .b(new_n28_), .O(new_n82_));
  inv1   g066(.a(new_n82_), .O(new_n83_));
  oai21  g067(.a(new_n83_), .b(new_n81_), .c(new_n78_), .O(new_n84_));
  nor2   g068(.a(x6), .b(x2), .O(new_n85_));
  inv1   g069(.a(new_n85_), .O(new_n86_));
  aoi21  g070(.a(new_n86_), .b(new_n29_), .c(x0), .O(new_n87_));
  nor2   g071(.a(x8), .b(x7), .O(new_n88_));
  nand2  g072(.a(new_n88_), .b(x6), .O(new_n89_));
  inv1   g073(.a(new_n89_), .O(new_n90_));
  oai21  g074(.a(new_n90_), .b(new_n87_), .c(x1), .O(new_n91_));
  aoi21  g075(.a(new_n91_), .b(new_n84_), .c(new_n17_), .O(new_n92_));
  nand2  g076(.a(new_n72_), .b(x9), .O(new_n93_));
  nand2  g077(.a(new_n93_), .b(x6), .O(new_n94_));
  nand2  g078(.a(new_n17_), .b(x1), .O(new_n95_));
  aoi21  g079(.a(new_n95_), .b(new_n94_), .c(new_n21_), .O(new_n96_));
  oai21  g080(.a(new_n96_), .b(new_n92_), .c(new_n51_), .O(new_n97_));
  nand2  g081(.a(x7), .b(x4), .O(new_n98_));
  nand4  g082(.a(new_n98_), .b(x8), .c(x6), .d(x0), .O(new_n99_));
  nand3  g083(.a(x5), .b(new_n24_), .c(new_n18_), .O(new_n100_));
  nand3  g084(.a(new_n88_), .b(new_n25_), .c(x4), .O(new_n101_));
  nand3  g085(.a(new_n101_), .b(new_n100_), .c(new_n99_), .O(new_n102_));
  nand2  g086(.a(new_n102_), .b(new_n78_), .O(new_n103_));
  oai22  g087(.a(new_n73_), .b(new_n24_), .c(new_n26_), .d(x0), .O(new_n104_));
  nand2  g088(.a(new_n104_), .b(x1), .O(new_n105_));
  aoi21  g089(.a(new_n105_), .b(new_n103_), .c(new_n20_), .O(new_n106_));
  nand2  g090(.a(new_n32_), .b(new_n20_), .O(new_n107_));
  aoi21  g091(.a(new_n107_), .b(new_n73_), .c(new_n18_), .O(new_n108_));
  nand2  g092(.a(x8), .b(new_n25_), .O(new_n109_));
  oai21  g093(.a(new_n73_), .b(new_n51_), .c(new_n109_), .O(new_n110_));
  nand2  g094(.a(new_n110_), .b(new_n20_), .O(new_n111_));
  oai21  g095(.a(new_n109_), .b(x0), .c(new_n111_), .O(new_n112_));
  oai21  g096(.a(new_n112_), .b(new_n108_), .c(x1), .O(new_n113_));
  oai21  g097(.a(new_n22_), .b(x2), .c(new_n51_), .O(new_n114_));
  nand3  g098(.a(new_n114_), .b(new_n78_), .c(new_n18_), .O(new_n115_));
  nand2  g099(.a(x7), .b(x2), .O(new_n116_));
  nand3  g100(.a(new_n116_), .b(new_n25_), .c(x5), .O(new_n117_));
  nand2  g101(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand2  g102(.a(new_n118_), .b(x8), .O(new_n119_));
  nand2  g103(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  oai21  g104(.a(new_n120_), .b(new_n106_), .c(x9), .O(new_n121_));
  nor2   g105(.a(x2), .b(new_n78_), .O(new_n122_));
  nor2   g106(.a(new_n21_), .b(new_n25_), .O(new_n123_));
  nand4  g107(.a(new_n123_), .b(new_n122_), .c(x5), .d(x0), .O(new_n124_));
  nand3  g108(.a(new_n124_), .b(new_n121_), .c(new_n97_), .O(new_n125_));
  nand2  g109(.a(new_n125_), .b(new_n77_), .O(new_n126_));
  nand3  g110(.a(new_n43_), .b(new_n25_), .c(x5), .O(new_n127_));
  nor2   g111(.a(new_n17_), .b(new_n19_), .O(new_n128_));
  nand3  g112(.a(new_n128_), .b(new_n21_), .c(new_n18_), .O(new_n129_));
  nand2  g113(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand2  g114(.a(new_n130_), .b(new_n20_), .O(new_n131_));
  nand2  g115(.a(x6), .b(new_n18_), .O(new_n132_));
  nand2  g116(.a(x9), .b(x4), .O(new_n133_));
  aoi21  g117(.a(new_n133_), .b(new_n132_), .c(new_n20_), .O(new_n134_));
  nand3  g118(.a(x9), .b(new_n51_), .c(x0), .O(new_n135_));
  nand2  g119(.a(new_n135_), .b(new_n38_), .O(new_n136_));
  oai21  g120(.a(new_n136_), .b(new_n134_), .c(new_n19_), .O(new_n137_));
  oai21  g121(.a(x9), .b(new_n51_), .c(new_n137_), .O(new_n138_));
  nand3  g122(.a(new_n21_), .b(x5), .c(x2), .O(new_n139_));
  nand2  g123(.a(new_n139_), .b(new_n64_), .O(new_n140_));
  nand3  g124(.a(new_n140_), .b(new_n24_), .c(new_n18_), .O(new_n141_));
  nand2  g125(.a(new_n123_), .b(new_n51_), .O(new_n142_));
  nand4  g126(.a(new_n142_), .b(x8), .c(x2), .d(x0), .O(new_n143_));
  aoi21  g127(.a(new_n143_), .b(new_n141_), .c(new_n17_), .O(new_n144_));
  aoi21  g128(.a(new_n138_), .b(x7), .c(new_n144_), .O(new_n145_));
  aoi21  g129(.a(new_n145_), .b(new_n131_), .c(new_n77_), .O(new_n146_));
  inv1   g130(.a(new_n123_), .O(new_n147_));
  nor2   g131(.a(new_n17_), .b(x7), .O(new_n148_));
  nand2  g132(.a(new_n148_), .b(new_n25_), .O(new_n149_));
  nand2  g133(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand4  g134(.a(new_n150_), .b(new_n51_), .c(new_n24_), .d(new_n18_), .O(new_n151_));
  nand3  g135(.a(new_n93_), .b(x7), .c(x6), .O(new_n152_));
  nand3  g136(.a(new_n128_), .b(new_n21_), .c(new_n25_), .O(new_n153_));
  nand3  g137(.a(new_n153_), .b(new_n152_), .c(new_n151_), .O(new_n154_));
  oai21  g138(.a(new_n154_), .b(new_n146_), .c(x1), .O(new_n155_));
  nand4  g139(.a(x7), .b(new_n25_), .c(new_n24_), .d(x0), .O(new_n156_));
  nand4  g140(.a(new_n128_), .b(x4), .c(new_n78_), .d(new_n18_), .O(new_n157_));
  nand2  g141(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  nand2  g142(.a(new_n158_), .b(x2), .O(new_n159_));
  nand3  g143(.a(new_n128_), .b(x4), .c(new_n18_), .O(new_n160_));
  nand3  g144(.a(new_n17_), .b(new_n25_), .c(new_n51_), .O(new_n161_));
  aoi21  g145(.a(new_n161_), .b(new_n160_), .c(x1), .O(new_n162_));
  nor2   g146(.a(x9), .b(x4), .O(new_n163_));
  oai21  g147(.a(new_n163_), .b(new_n162_), .c(x7), .O(new_n164_));
  nand3  g148(.a(new_n21_), .b(x6), .c(x0), .O(new_n165_));
  nand2  g149(.a(new_n19_), .b(new_n18_), .O(new_n166_));
  aoi21  g150(.a(new_n166_), .b(new_n165_), .c(x2), .O(new_n167_));
  oai21  g151(.a(new_n167_), .b(new_n90_), .c(new_n51_), .O(new_n168_));
  nand3  g152(.a(x8), .b(new_n25_), .c(new_n20_), .O(new_n169_));
  aoi21  g153(.a(new_n169_), .b(new_n168_), .c(new_n24_), .O(new_n170_));
  oai21  g154(.a(new_n25_), .b(x2), .c(x8), .O(new_n171_));
  nand2  g155(.a(new_n171_), .b(x0), .O(new_n172_));
  nand2  g156(.a(new_n19_), .b(new_n20_), .O(new_n173_));
  nand2  g157(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand3  g158(.a(new_n174_), .b(x5), .c(new_n24_), .O(new_n175_));
  inv1   g159(.a(new_n175_), .O(new_n176_));
  oai21  g160(.a(new_n176_), .b(new_n170_), .c(new_n78_), .O(new_n177_));
  aoi21  g161(.a(x8), .b(x5), .c(new_n85_), .O(new_n178_));
  oai21  g162(.a(new_n178_), .b(x0), .c(new_n109_), .O(new_n179_));
  nand3  g163(.a(new_n179_), .b(new_n21_), .c(x4), .O(new_n180_));
  nand2  g164(.a(new_n180_), .b(new_n177_), .O(new_n181_));
  nand2  g165(.a(new_n181_), .b(x9), .O(new_n182_));
  nand3  g166(.a(new_n182_), .b(new_n164_), .c(new_n159_), .O(new_n183_));
  nand2  g167(.a(new_n183_), .b(x3), .O(new_n184_));
  aoi21  g168(.a(x9), .b(x2), .c(x7), .O(new_n185_));
  nand2  g169(.a(new_n148_), .b(new_n18_), .O(new_n186_));
  oai21  g170(.a(new_n185_), .b(new_n18_), .c(new_n186_), .O(new_n187_));
  nand4  g171(.a(new_n187_), .b(x6), .c(new_n51_), .d(new_n24_), .O(new_n188_));
  nand3  g172(.a(new_n148_), .b(new_n28_), .c(x5), .O(new_n189_));
  nand2  g173(.a(new_n189_), .b(new_n188_), .O(new_n190_));
  nand2  g174(.a(new_n190_), .b(x8), .O(new_n191_));
  nor2   g175(.a(x5), .b(x4), .O(new_n192_));
  nor2   g176(.a(new_n17_), .b(x8), .O(new_n193_));
  nand4  g177(.a(new_n193_), .b(new_n192_), .c(new_n25_), .d(x0), .O(new_n194_));
  nand2  g178(.a(new_n194_), .b(new_n191_), .O(new_n195_));
  nand3  g179(.a(new_n192_), .b(new_n17_), .c(x7), .O(new_n196_));
  oai21  g180(.a(new_n149_), .b(new_n38_), .c(new_n196_), .O(new_n197_));
  aoi21  g181(.a(new_n195_), .b(new_n78_), .c(new_n197_), .O(new_n198_));
  nand4  g182(.a(new_n198_), .b(new_n184_), .c(new_n155_), .d(new_n126_), .O(z1));
  nor2   g183(.a(x9), .b(x7), .O(new_n200_));
  nor3   g184(.a(new_n200_), .b(new_n77_), .c(new_n78_), .O(z3));
  inv1   g185(.a(z3), .O(new_n202_));
  aoi21  g186(.a(new_n77_), .b(new_n78_), .c(new_n200_), .O(new_n203_));
  nand2  g187(.a(new_n203_), .b(new_n202_), .O(z2));
  nand2  g188(.a(x4), .b(new_n18_), .O(new_n205_));
  oai21  g189(.a(new_n19_), .b(x4), .c(new_n20_), .O(new_n206_));
  nor2   g190(.a(new_n20_), .b(new_n18_), .O(new_n207_));
  nand3  g191(.a(new_n207_), .b(x7), .c(new_n24_), .O(new_n208_));
  nand3  g192(.a(new_n208_), .b(new_n206_), .c(new_n205_), .O(new_n209_));
  nand2  g193(.a(new_n209_), .b(new_n25_), .O(new_n210_));
  nand2  g194(.a(new_n58_), .b(new_n22_), .O(new_n211_));
  nand3  g195(.a(new_n211_), .b(x2), .c(x0), .O(new_n212_));
  nand2  g196(.a(new_n22_), .b(x8), .O(new_n213_));
  nand3  g197(.a(new_n213_), .b(new_n20_), .c(new_n18_), .O(new_n214_));
  nand2  g198(.a(new_n214_), .b(new_n212_), .O(new_n215_));
  oai21  g199(.a(x2), .b(new_n18_), .c(new_n78_), .O(new_n216_));
  nand4  g200(.a(new_n216_), .b(x7), .c(x6), .d(new_n24_), .O(new_n217_));
  inv1   g201(.a(new_n217_), .O(new_n218_));
  aoi21  g202(.a(new_n215_), .b(new_n78_), .c(new_n218_), .O(new_n219_));
  aoi21  g203(.a(new_n219_), .b(new_n210_), .c(x3), .O(new_n220_));
  aoi21  g204(.a(new_n86_), .b(x0), .c(x1), .O(new_n221_));
  aoi21  g205(.a(x3), .b(x2), .c(new_n25_), .O(new_n222_));
  nor2   g206(.a(new_n222_), .b(x7), .O(new_n223_));
  oai21  g207(.a(new_n223_), .b(new_n221_), .c(x4), .O(new_n224_));
  nand3  g208(.a(new_n211_), .b(new_n20_), .c(new_n18_), .O(new_n225_));
  nand2  g209(.a(new_n207_), .b(new_n27_), .O(new_n226_));
  nand2  g210(.a(new_n226_), .b(new_n225_), .O(new_n227_));
  nand3  g211(.a(new_n227_), .b(x3), .c(x1), .O(new_n228_));
  nand2  g212(.a(new_n228_), .b(new_n224_), .O(new_n229_));
  oai21  g213(.a(new_n229_), .b(new_n220_), .c(x5), .O(new_n230_));
  nand2  g214(.a(new_n116_), .b(x8), .O(new_n231_));
  oai21  g215(.a(new_n24_), .b(new_n77_), .c(new_n78_), .O(new_n232_));
  nand2  g216(.a(new_n232_), .b(new_n231_), .O(new_n233_));
  nand3  g217(.a(x7), .b(x4), .c(x3), .O(new_n234_));
  inv1   g218(.a(new_n234_), .O(new_n235_));
  oai21  g219(.a(x7), .b(x2), .c(new_n58_), .O(new_n236_));
  oai21  g220(.a(new_n236_), .b(new_n235_), .c(x6), .O(new_n237_));
  aoi21  g221(.a(new_n237_), .b(new_n233_), .c(x5), .O(new_n238_));
  nand3  g222(.a(new_n171_), .b(x7), .c(new_n24_), .O(new_n239_));
  nand2  g223(.a(new_n88_), .b(new_n25_), .O(new_n240_));
  aoi21  g224(.a(new_n240_), .b(new_n239_), .c(new_n78_), .O(new_n241_));
  oai21  g225(.a(new_n241_), .b(new_n238_), .c(x0), .O(new_n242_));
  nand3  g226(.a(new_n38_), .b(new_n25_), .c(x3), .O(new_n243_));
  nand3  g227(.a(x6), .b(new_n51_), .c(x4), .O(new_n244_));
  aoi21  g228(.a(new_n244_), .b(new_n243_), .c(new_n21_), .O(new_n245_));
  nand2  g229(.a(new_n27_), .b(new_n77_), .O(new_n246_));
  nand3  g230(.a(new_n19_), .b(x4), .c(x2), .O(new_n247_));
  aoi21  g231(.a(new_n247_), .b(new_n246_), .c(x5), .O(new_n248_));
  oai21  g232(.a(new_n248_), .b(new_n245_), .c(x1), .O(new_n249_));
  nand2  g233(.a(new_n27_), .b(new_n18_), .O(new_n250_));
  nand2  g234(.a(new_n19_), .b(x3), .O(new_n251_));
  aoi21  g235(.a(new_n251_), .b(new_n250_), .c(new_n20_), .O(new_n252_));
  nand3  g236(.a(new_n27_), .b(x3), .c(new_n78_), .O(new_n253_));
  inv1   g237(.a(new_n253_), .O(new_n254_));
  oai21  g238(.a(new_n254_), .b(new_n252_), .c(x4), .O(new_n255_));
  nand2  g239(.a(new_n255_), .b(new_n89_), .O(new_n256_));
  nand2  g240(.a(new_n256_), .b(new_n51_), .O(new_n257_));
  nand3  g241(.a(new_n257_), .b(new_n249_), .c(new_n242_), .O(new_n258_));
  inv1   g242(.a(new_n258_), .O(new_n259_));
  aoi21  g243(.a(new_n259_), .b(new_n230_), .c(new_n17_), .O(z4));
  inv1   g244(.a(new_n200_), .O(new_n261_));
  xor2a  g245(.a(x3), .b(x1), .O(new_n262_));
  nor2   g246(.a(x2), .b(x0), .O(new_n263_));
  nor2   g247(.a(new_n263_), .b(new_n207_), .O(new_n264_));
  oai21  g248(.a(new_n264_), .b(new_n262_), .c(new_n261_), .O(z5));
endmodule


