// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:15 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n136_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n156_, new_n158_, new_n159_,
    new_n160_;
  inv1   g000(.a(x0), .O(new_n17_));
  inv1   g001(.a(x2), .O(new_n18_));
  inv1   g002(.a(x4), .O(new_n19_));
  inv1   g003(.a(x8), .O(new_n20_));
  nand2  g004(.a(x9), .b(new_n20_), .O(new_n21_));
  oai21  g005(.a(x9), .b(x6), .c(new_n21_), .O(new_n22_));
  nand2  g006(.a(new_n22_), .b(new_n19_), .O(new_n23_));
  nor2   g007(.a(x6), .b(x5), .O(new_n24_));
  inv1   g008(.a(x9), .O(new_n25_));
  nand2  g009(.a(new_n25_), .b(x7), .O(new_n26_));
  inv1   g010(.a(new_n26_), .O(new_n27_));
  nor2   g011(.a(new_n25_), .b(new_n20_), .O(new_n28_));
  aoi22  g012(.a(new_n28_), .b(x4), .c(new_n27_), .d(new_n24_), .O(new_n29_));
  aoi21  g013(.a(new_n29_), .b(new_n23_), .c(new_n18_), .O(new_n30_));
  inv1   g014(.a(x7), .O(new_n31_));
  nand3  g015(.a(new_n31_), .b(x6), .c(new_n18_), .O(new_n32_));
  inv1   g016(.a(x5), .O(new_n33_));
  nand2  g017(.a(new_n25_), .b(new_n33_), .O(new_n34_));
  nand2  g018(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand2  g019(.a(new_n35_), .b(new_n19_), .O(new_n36_));
  oai21  g020(.a(x9), .b(x7), .c(new_n21_), .O(new_n37_));
  nand3  g021(.a(new_n37_), .b(new_n33_), .c(new_n18_), .O(new_n38_));
  nand2  g022(.a(new_n38_), .b(new_n36_), .O(new_n39_));
  oai21  g023(.a(new_n39_), .b(new_n30_), .c(new_n17_), .O(new_n40_));
  nand3  g024(.a(x6), .b(new_n18_), .c(x0), .O(new_n41_));
  nand3  g025(.a(x9), .b(new_n31_), .c(x5), .O(new_n42_));
  nand2  g026(.a(new_n42_), .b(new_n41_), .O(new_n43_));
  nand2  g027(.a(new_n43_), .b(x4), .O(new_n44_));
  nand3  g028(.a(x6), .b(x5), .c(x0), .O(new_n45_));
  aoi21  g029(.a(new_n45_), .b(new_n26_), .c(x4), .O(new_n46_));
  oai21  g030(.a(x9), .b(new_n33_), .c(new_n21_), .O(new_n47_));
  nand3  g031(.a(new_n47_), .b(x7), .c(x0), .O(new_n48_));
  inv1   g032(.a(new_n48_), .O(new_n49_));
  oai21  g033(.a(new_n49_), .b(new_n46_), .c(x2), .O(new_n50_));
  inv1   g034(.a(new_n28_), .O(new_n51_));
  nand2  g035(.a(new_n27_), .b(new_n33_), .O(new_n52_));
  nand2  g036(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g037(.a(new_n53_), .b(x0), .O(new_n54_));
  nand2  g038(.a(new_n28_), .b(x5), .O(new_n55_));
  nand2  g039(.a(x6), .b(new_n33_), .O(new_n56_));
  nand3  g040(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand2  g041(.a(x7), .b(x6), .O(new_n58_));
  oai21  g042(.a(new_n51_), .b(x7), .c(new_n58_), .O(new_n59_));
  nand2  g043(.a(new_n59_), .b(x0), .O(new_n60_));
  inv1   g044(.a(x6), .O(new_n61_));
  nor2   g045(.a(new_n25_), .b(new_n61_), .O(new_n62_));
  inv1   g046(.a(new_n62_), .O(new_n63_));
  nand2  g047(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  aoi21  g048(.a(new_n57_), .b(new_n18_), .c(new_n64_), .O(new_n65_));
  nand4  g049(.a(new_n65_), .b(new_n50_), .c(new_n44_), .d(new_n40_), .O(z0));
  nor2   g050(.a(x5), .b(x2), .O(new_n67_));
  oai21  g051(.a(new_n67_), .b(x8), .c(new_n17_), .O(new_n68_));
  nand2  g052(.a(x8), .b(new_n18_), .O(new_n69_));
  aoi21  g053(.a(new_n69_), .b(new_n68_), .c(new_n19_), .O(new_n70_));
  oai21  g054(.a(new_n33_), .b(x2), .c(new_n17_), .O(new_n71_));
  nand3  g055(.a(new_n71_), .b(new_n20_), .c(new_n19_), .O(new_n72_));
  inv1   g056(.a(new_n72_), .O(new_n73_));
  oai21  g057(.a(new_n73_), .b(new_n70_), .c(x9), .O(new_n74_));
  oai21  g058(.a(new_n31_), .b(x5), .c(x4), .O(new_n75_));
  nand3  g059(.a(new_n75_), .b(new_n25_), .c(new_n61_), .O(new_n76_));
  aoi21  g060(.a(new_n76_), .b(new_n74_), .c(x1), .O(new_n77_));
  nor2   g061(.a(x8), .b(new_n31_), .O(new_n78_));
  nand3  g062(.a(new_n78_), .b(x5), .c(x1), .O(new_n79_));
  nand4  g063(.a(x9), .b(new_n31_), .c(x4), .d(new_n17_), .O(new_n80_));
  nand2  g064(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g065(.a(new_n81_), .b(new_n18_), .O(new_n82_));
  inv1   g066(.a(x1), .O(new_n83_));
  nand2  g067(.a(x8), .b(x2), .O(new_n84_));
  oai21  g068(.a(x8), .b(new_n31_), .c(new_n84_), .O(new_n85_));
  nand2  g069(.a(new_n85_), .b(x0), .O(new_n86_));
  nand3  g070(.a(new_n78_), .b(x4), .c(x2), .O(new_n87_));
  aoi21  g071(.a(new_n87_), .b(new_n86_), .c(new_n83_), .O(new_n88_));
  nand3  g072(.a(x8), .b(new_n31_), .c(x4), .O(new_n89_));
  inv1   g073(.a(new_n89_), .O(new_n90_));
  oai21  g074(.a(new_n90_), .b(new_n88_), .c(x9), .O(new_n91_));
  nand2  g075(.a(x6), .b(new_n19_), .O(new_n92_));
  aoi21  g076(.a(new_n92_), .b(new_n26_), .c(new_n33_), .O(new_n93_));
  nand2  g077(.a(x2), .b(x0), .O(new_n94_));
  aoi21  g078(.a(new_n94_), .b(x9), .c(new_n31_), .O(new_n95_));
  aoi22  g079(.a(new_n95_), .b(new_n19_), .c(new_n93_), .d(x1), .O(new_n96_));
  nand3  g080(.a(new_n96_), .b(new_n91_), .c(new_n82_), .O(new_n97_));
  oai21  g081(.a(new_n97_), .b(new_n77_), .c(x3), .O(new_n98_));
  inv1   g082(.a(x3), .O(new_n99_));
  nand2  g083(.a(x7), .b(x2), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(x8), .O(new_n101_));
  nand2  g085(.a(new_n101_), .b(x0), .O(new_n102_));
  nand3  g086(.a(new_n20_), .b(x4), .c(x2), .O(new_n103_));
  aoi21  g087(.a(new_n103_), .b(new_n102_), .c(x1), .O(new_n104_));
  nand2  g088(.a(x4), .b(x2), .O(new_n105_));
  nand3  g089(.a(new_n105_), .b(x1), .c(new_n17_), .O(new_n106_));
  inv1   g090(.a(new_n106_), .O(new_n107_));
  oai21  g091(.a(new_n107_), .b(new_n104_), .c(new_n33_), .O(new_n108_));
  oai21  g092(.a(x5), .b(x1), .c(new_n94_), .O(new_n109_));
  nand2  g093(.a(new_n31_), .b(x5), .O(new_n110_));
  nand2  g094(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g095(.a(x5), .b(new_n19_), .c(x2), .d(new_n17_), .O(new_n112_));
  inv1   g096(.a(new_n112_), .O(new_n113_));
  aoi21  g097(.a(new_n111_), .b(x8), .c(new_n113_), .O(new_n114_));
  aoi21  g098(.a(new_n114_), .b(new_n108_), .c(new_n25_), .O(new_n115_));
  oai21  g099(.a(new_n61_), .b(new_n19_), .c(new_n52_), .O(new_n116_));
  nand2  g100(.a(new_n116_), .b(x1), .O(new_n117_));
  nand2  g101(.a(new_n92_), .b(new_n34_), .O(new_n118_));
  nand3  g102(.a(new_n118_), .b(new_n31_), .c(new_n83_), .O(new_n119_));
  nand3  g103(.a(new_n119_), .b(new_n117_), .c(new_n56_), .O(new_n120_));
  oai21  g104(.a(new_n120_), .b(new_n115_), .c(new_n99_), .O(new_n121_));
  and2   g105(.a(x5), .b(x2), .O(new_n122_));
  nor2   g106(.a(x5), .b(x4), .O(new_n123_));
  oai21  g107(.a(new_n123_), .b(new_n122_), .c(new_n17_), .O(new_n124_));
  aoi21  g108(.a(new_n124_), .b(new_n20_), .c(new_n83_), .O(new_n125_));
  nand2  g109(.a(x5), .b(x4), .O(new_n126_));
  inv1   g110(.a(new_n126_), .O(new_n127_));
  oai21  g111(.a(new_n127_), .b(new_n125_), .c(new_n31_), .O(new_n128_));
  nand2  g112(.a(new_n128_), .b(new_n61_), .O(new_n129_));
  nand2  g113(.a(new_n19_), .b(new_n83_), .O(new_n130_));
  oai22  g114(.a(new_n130_), .b(new_n34_), .c(new_n58_), .d(new_n83_), .O(new_n131_));
  aoi21  g115(.a(new_n129_), .b(x9), .c(new_n131_), .O(new_n132_));
  nand3  g116(.a(new_n132_), .b(new_n121_), .c(new_n98_), .O(z1));
  xor2a  g117(.a(x3), .b(x1), .O(new_n134_));
  nor2   g118(.a(new_n134_), .b(new_n62_), .O(z2));
  nand3  g119(.a(new_n63_), .b(x3), .c(x1), .O(new_n136_));
  inv1   g120(.a(new_n136_), .O(z3));
  oai21  g121(.a(x8), .b(new_n18_), .c(new_n102_), .O(new_n138_));
  nand2  g122(.a(new_n138_), .b(x3), .O(new_n139_));
  nand3  g123(.a(new_n20_), .b(x2), .c(x1), .O(new_n140_));
  aoi21  g124(.a(new_n140_), .b(new_n139_), .c(x5), .O(new_n141_));
  oai21  g125(.a(new_n99_), .b(new_n83_), .c(new_n94_), .O(new_n142_));
  aoi21  g126(.a(new_n142_), .b(x7), .c(new_n33_), .O(new_n143_));
  oai21  g127(.a(new_n143_), .b(new_n141_), .c(x4), .O(new_n144_));
  nand3  g128(.a(x5), .b(new_n19_), .c(new_n99_), .O(new_n145_));
  oai21  g129(.a(x5), .b(new_n83_), .c(new_n145_), .O(new_n146_));
  nand3  g130(.a(new_n146_), .b(x7), .c(x2), .O(new_n147_));
  oai21  g131(.a(new_n31_), .b(new_n33_), .c(x1), .O(new_n148_));
  oai21  g132(.a(new_n110_), .b(x3), .c(new_n148_), .O(new_n149_));
  nand2  g133(.a(new_n149_), .b(new_n20_), .O(new_n150_));
  nand2  g134(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  oai21  g135(.a(x7), .b(new_n83_), .c(x3), .O(new_n152_));
  nand4  g136(.a(new_n152_), .b(new_n20_), .c(x5), .d(new_n18_), .O(new_n153_));
  nand4  g137(.a(new_n126_), .b(x7), .c(x3), .d(x1), .O(new_n154_));
  nand3  g138(.a(new_n154_), .b(new_n153_), .c(new_n61_), .O(new_n155_));
  aoi21  g139(.a(new_n151_), .b(x0), .c(new_n155_), .O(new_n156_));
  aoi21  g140(.a(new_n156_), .b(new_n144_), .c(new_n25_), .O(z4));
  nand2  g141(.a(new_n18_), .b(new_n17_), .O(new_n158_));
  aoi21  g142(.a(new_n158_), .b(new_n94_), .c(new_n134_), .O(new_n159_));
  nand2  g143(.a(new_n159_), .b(new_n63_), .O(new_n160_));
  inv1   g144(.a(new_n160_), .O(z5));
endmodule


