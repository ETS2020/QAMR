// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:57 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n128_, new_n129_, new_n130_, new_n131_, new_n133_,
    new_n134_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n143_, new_n144_, new_n145_, new_n146_,
    new_n147_, new_n148_, new_n149_, new_n150_, new_n151_, new_n152_,
    new_n153_, new_n154_, new_n155_, new_n157_;
  inv1   g000(.a(x6), .O(new_n17_));
  inv1   g001(.a(x9), .O(new_n18_));
  inv1   g002(.a(x0), .O(new_n19_));
  nand3  g003(.a(x8), .b(x4), .c(x2), .O(new_n20_));
  inv1   g004(.a(new_n20_), .O(new_n21_));
  nor3   g005(.a(x8), .b(x5), .c(x2), .O(new_n22_));
  oai21  g006(.a(new_n22_), .b(new_n21_), .c(new_n19_), .O(new_n23_));
  inv1   g007(.a(x5), .O(new_n24_));
  nand2  g008(.a(x7), .b(x2), .O(new_n25_));
  nand2  g009(.a(new_n25_), .b(x0), .O(new_n26_));
  oai21  g010(.a(new_n24_), .b(x2), .c(new_n26_), .O(new_n27_));
  inv1   g011(.a(x4), .O(new_n28_));
  inv1   g012(.a(x7), .O(new_n29_));
  nand2  g013(.a(new_n29_), .b(x5), .O(new_n30_));
  inv1   g014(.a(x8), .O(new_n31_));
  inv1   g015(.a(x2), .O(new_n32_));
  nor2   g016(.a(new_n32_), .b(new_n19_), .O(new_n33_));
  nand3  g017(.a(new_n33_), .b(new_n31_), .c(x7), .O(new_n34_));
  oai21  g018(.a(new_n30_), .b(new_n28_), .c(new_n34_), .O(new_n35_));
  aoi21  g019(.a(new_n27_), .b(x8), .c(new_n35_), .O(new_n36_));
  aoi21  g020(.a(new_n36_), .b(new_n23_), .c(new_n18_), .O(new_n37_));
  oai21  g021(.a(new_n18_), .b(new_n31_), .c(new_n28_), .O(new_n38_));
  nand3  g022(.a(new_n18_), .b(x7), .c(new_n24_), .O(new_n39_));
  nand2  g023(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand3  g024(.a(new_n40_), .b(x2), .c(new_n19_), .O(new_n41_));
  inv1   g025(.a(new_n41_), .O(new_n42_));
  oai21  g026(.a(new_n42_), .b(new_n37_), .c(new_n17_), .O(new_n43_));
  nor2   g027(.a(new_n17_), .b(x4), .O(new_n44_));
  nor2   g028(.a(new_n44_), .b(x7), .O(new_n45_));
  nor2   g029(.a(new_n45_), .b(new_n24_), .O(new_n46_));
  nand2  g030(.a(new_n46_), .b(x2), .O(new_n47_));
  nand2  g031(.a(x7), .b(new_n24_), .O(new_n48_));
  oai21  g032(.a(new_n17_), .b(new_n28_), .c(new_n48_), .O(new_n49_));
  nand2  g033(.a(new_n49_), .b(new_n32_), .O(new_n50_));
  nand2  g034(.a(x7), .b(x6), .O(new_n51_));
  nand3  g035(.a(new_n51_), .b(new_n50_), .c(new_n47_), .O(new_n52_));
  nand2  g036(.a(new_n52_), .b(x0), .O(new_n53_));
  oai21  g037(.a(new_n44_), .b(new_n24_), .c(new_n29_), .O(new_n54_));
  nand2  g038(.a(x6), .b(new_n24_), .O(new_n55_));
  oai21  g039(.a(new_n54_), .b(x0), .c(new_n55_), .O(new_n56_));
  nand2  g040(.a(new_n56_), .b(new_n32_), .O(new_n57_));
  oai21  g041(.a(x5), .b(x0), .c(new_n25_), .O(new_n58_));
  nand2  g042(.a(new_n58_), .b(new_n28_), .O(new_n59_));
  nand3  g043(.a(new_n59_), .b(new_n57_), .c(new_n53_), .O(new_n60_));
  nand2  g044(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nand2  g045(.a(new_n61_), .b(new_n43_), .O(z0));
  nand2  g046(.a(new_n46_), .b(x3), .O(new_n63_));
  inv1   g047(.a(x3), .O(new_n64_));
  nand2  g048(.a(new_n49_), .b(new_n64_), .O(new_n65_));
  nand3  g049(.a(new_n65_), .b(new_n63_), .c(new_n51_), .O(new_n66_));
  nand2  g050(.a(new_n66_), .b(x1), .O(new_n67_));
  oai21  g051(.a(new_n54_), .b(x1), .c(new_n55_), .O(new_n68_));
  nand2  g052(.a(new_n68_), .b(new_n64_), .O(new_n69_));
  inv1   g053(.a(x1), .O(new_n70_));
  nand2  g054(.a(new_n48_), .b(x4), .O(new_n71_));
  nand3  g055(.a(new_n71_), .b(new_n17_), .c(x3), .O(new_n72_));
  oai21  g056(.a(x5), .b(x4), .c(new_n72_), .O(new_n73_));
  nand2  g057(.a(new_n73_), .b(new_n70_), .O(new_n74_));
  nand3  g058(.a(x7), .b(new_n28_), .c(x3), .O(new_n75_));
  nand4  g059(.a(new_n75_), .b(new_n74_), .c(new_n69_), .d(new_n67_), .O(new_n76_));
  nand2  g060(.a(new_n76_), .b(new_n18_), .O(new_n77_));
  nor2   g061(.a(x3), .b(x1), .O(new_n78_));
  nand3  g062(.a(new_n78_), .b(x7), .c(new_n24_), .O(new_n79_));
  nand3  g063(.a(x8), .b(x3), .c(x1), .O(new_n80_));
  nand2  g064(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g065(.a(new_n81_), .b(x0), .O(new_n82_));
  nand3  g066(.a(new_n24_), .b(new_n64_), .c(new_n70_), .O(new_n83_));
  nand3  g067(.a(x7), .b(x3), .c(x1), .O(new_n84_));
  nand2  g068(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand3  g069(.a(new_n85_), .b(new_n31_), .c(x4), .O(new_n86_));
  nand2  g070(.a(new_n28_), .b(new_n64_), .O(new_n87_));
  aoi21  g071(.a(new_n87_), .b(new_n30_), .c(new_n70_), .O(new_n88_));
  nand3  g072(.a(x5), .b(new_n28_), .c(new_n64_), .O(new_n89_));
  inv1   g073(.a(new_n89_), .O(new_n90_));
  oai21  g074(.a(new_n90_), .b(new_n88_), .c(new_n19_), .O(new_n91_));
  nand3  g075(.a(new_n91_), .b(new_n86_), .c(new_n82_), .O(new_n92_));
  nand2  g076(.a(new_n92_), .b(x9), .O(new_n93_));
  nand4  g077(.a(x7), .b(new_n28_), .c(x3), .d(x0), .O(new_n94_));
  nand2  g078(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g079(.a(new_n95_), .b(x2), .O(new_n96_));
  nand2  g080(.a(new_n28_), .b(new_n70_), .O(new_n97_));
  nand3  g081(.a(x9), .b(x7), .c(x1), .O(new_n98_));
  aoi21  g082(.a(new_n98_), .b(new_n97_), .c(new_n19_), .O(new_n99_));
  oai21  g083(.a(new_n29_), .b(new_n70_), .c(new_n97_), .O(new_n100_));
  nand3  g084(.a(new_n100_), .b(x5), .c(new_n32_), .O(new_n101_));
  inv1   g085(.a(new_n101_), .O(new_n102_));
  oai21  g086(.a(new_n102_), .b(new_n99_), .c(new_n31_), .O(new_n103_));
  nor2   g087(.a(x2), .b(x0), .O(new_n104_));
  oai21  g088(.a(new_n104_), .b(x8), .c(new_n29_), .O(new_n105_));
  oai21  g089(.a(x5), .b(x2), .c(new_n31_), .O(new_n106_));
  aoi22  g090(.a(new_n106_), .b(new_n19_), .c(x8), .d(new_n32_), .O(new_n107_));
  oai21  g091(.a(new_n107_), .b(x1), .c(new_n105_), .O(new_n108_));
  nand3  g092(.a(new_n108_), .b(x9), .c(x4), .O(new_n109_));
  nand2  g093(.a(new_n109_), .b(new_n103_), .O(new_n110_));
  nand2  g094(.a(new_n110_), .b(x3), .O(new_n111_));
  nand3  g095(.a(new_n32_), .b(x1), .c(new_n19_), .O(new_n112_));
  nand3  g096(.a(new_n31_), .b(new_n70_), .c(x0), .O(new_n113_));
  nand2  g097(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand2  g098(.a(new_n114_), .b(new_n24_), .O(new_n115_));
  inv1   g099(.a(new_n30_), .O(new_n116_));
  aoi21  g100(.a(new_n24_), .b(new_n70_), .c(new_n33_), .O(new_n117_));
  oai21  g101(.a(new_n117_), .b(new_n116_), .c(x8), .O(new_n118_));
  aoi21  g102(.a(new_n118_), .b(new_n115_), .c(x3), .O(new_n119_));
  nor3   g103(.a(x5), .b(x4), .c(x0), .O(new_n120_));
  oai21  g104(.a(new_n120_), .b(x8), .c(x1), .O(new_n121_));
  nand2  g105(.a(x5), .b(x4), .O(new_n122_));
  aoi21  g106(.a(new_n122_), .b(new_n121_), .c(x7), .O(new_n123_));
  oai21  g107(.a(new_n123_), .b(new_n119_), .c(x9), .O(new_n124_));
  nand3  g108(.a(new_n124_), .b(new_n111_), .c(new_n96_), .O(new_n125_));
  nand2  g109(.a(new_n125_), .b(new_n17_), .O(new_n126_));
  nand2  g110(.a(new_n126_), .b(new_n77_), .O(z1));
  nor2   g111(.a(new_n18_), .b(new_n17_), .O(new_n128_));
  nand2  g112(.a(x3), .b(x1), .O(new_n129_));
  inv1   g113(.a(new_n129_), .O(new_n130_));
  nor2   g114(.a(new_n130_), .b(new_n78_), .O(new_n131_));
  nor2   g115(.a(new_n131_), .b(new_n128_), .O(z2));
  inv1   g116(.a(new_n128_), .O(new_n133_));
  nand3  g117(.a(new_n133_), .b(x3), .c(x1), .O(new_n134_));
  inv1   g118(.a(new_n134_), .O(z3));
  aoi21  g119(.a(new_n25_), .b(x8), .c(new_n19_), .O(new_n136_));
  nor2   g120(.a(x8), .b(new_n32_), .O(new_n137_));
  oai21  g121(.a(new_n137_), .b(new_n136_), .c(x3), .O(new_n138_));
  nand2  g122(.a(new_n137_), .b(x1), .O(new_n139_));
  aoi21  g123(.a(new_n139_), .b(new_n138_), .c(x5), .O(new_n140_));
  oai21  g124(.a(new_n32_), .b(new_n19_), .c(new_n129_), .O(new_n141_));
  aoi21  g125(.a(new_n141_), .b(x7), .c(new_n24_), .O(new_n142_));
  oai21  g126(.a(new_n142_), .b(new_n140_), .c(x4), .O(new_n143_));
  oai21  g127(.a(x5), .b(new_n70_), .c(new_n89_), .O(new_n144_));
  nand3  g128(.a(new_n144_), .b(x7), .c(x2), .O(new_n145_));
  oai21  g129(.a(new_n29_), .b(new_n24_), .c(x1), .O(new_n146_));
  oai21  g130(.a(new_n30_), .b(x3), .c(new_n146_), .O(new_n147_));
  nand2  g131(.a(new_n147_), .b(new_n31_), .O(new_n148_));
  nand2  g132(.a(new_n148_), .b(new_n145_), .O(new_n149_));
  nand2  g133(.a(new_n149_), .b(x0), .O(new_n150_));
  oai21  g134(.a(x7), .b(new_n70_), .c(x3), .O(new_n151_));
  nand4  g135(.a(new_n151_), .b(new_n31_), .c(x5), .d(new_n32_), .O(new_n152_));
  nand4  g136(.a(new_n122_), .b(x7), .c(x3), .d(x1), .O(new_n153_));
  nand4  g137(.a(new_n153_), .b(new_n152_), .c(new_n150_), .d(new_n143_), .O(new_n154_));
  nand3  g138(.a(new_n154_), .b(x9), .c(new_n17_), .O(new_n155_));
  inv1   g139(.a(new_n155_), .O(z4));
  nor2   g140(.a(new_n104_), .b(new_n33_), .O(new_n157_));
  oai21  g141(.a(new_n157_), .b(new_n131_), .c(new_n133_), .O(z5));
endmodule


