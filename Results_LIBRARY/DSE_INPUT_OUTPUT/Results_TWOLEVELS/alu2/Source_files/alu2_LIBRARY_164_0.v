// Benchmark "FAU" written by ABC on Tue Aug 18 18:38:29 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6, x7, x8, x9,
    z0, z1, z2, z3, z4, z5  );
  input  x0, x1, x2, x3, x4, x5, x6, x7, x8, x9;
  output z0, z1, z2, z3, z4, z5;
  wire new_n17_, new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_,
    new_n24_, new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_,
    new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n194_, new_n195_, new_n196_, new_n197_,
    new_n198_, new_n199_, new_n200_, new_n201_, new_n202_, new_n203_,
    new_n205_, new_n207_, new_n209_, new_n210_, new_n211_, new_n212_,
    new_n213_, new_n214_, new_n215_, new_n216_, new_n217_, new_n218_,
    new_n219_, new_n220_, new_n221_, new_n222_, new_n223_, new_n224_,
    new_n225_, new_n226_, new_n227_, new_n228_, new_n229_, new_n230_,
    new_n231_, new_n232_, new_n233_, new_n234_, new_n235_, new_n236_,
    new_n237_, new_n238_, new_n239_, new_n240_, new_n241_, new_n242_,
    new_n243_, new_n244_, new_n245_, new_n246_, new_n247_, new_n248_,
    new_n249_, new_n250_, new_n251_, new_n252_, new_n253_, new_n254_,
    new_n255_, new_n256_, new_n257_, new_n258_, new_n259_, new_n260_,
    new_n261_;
  inv1   g000(.a(x4), .O(new_n17_));
  inv1   g001(.a(x7), .O(new_n18_));
  nor2   g002(.a(x8), .b(new_n18_), .O(new_n19_));
  nand2  g003(.a(new_n19_), .b(x6), .O(new_n20_));
  inv1   g004(.a(x2), .O(new_n21_));
  inv1   g005(.a(x9), .O(new_n22_));
  nand2  g006(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  aoi21  g007(.a(new_n23_), .b(new_n20_), .c(new_n17_), .O(new_n24_));
  inv1   g008(.a(x8), .O(new_n25_));
  nand4  g009(.a(x9), .b(new_n25_), .c(x6), .d(new_n21_), .O(new_n26_));
  nand3  g010(.a(new_n22_), .b(new_n17_), .c(x2), .O(new_n27_));
  nand2  g011(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g012(.a(new_n28_), .b(x5), .O(new_n29_));
  inv1   g013(.a(x5), .O(new_n30_));
  nor2   g014(.a(x8), .b(x7), .O(new_n31_));
  nand3  g015(.a(new_n31_), .b(x6), .c(new_n30_), .O(new_n32_));
  oai21  g016(.a(new_n25_), .b(x6), .c(new_n32_), .O(new_n33_));
  nand2  g017(.a(new_n33_), .b(new_n21_), .O(new_n34_));
  nand4  g018(.a(x8), .b(x6), .c(new_n30_), .d(new_n17_), .O(new_n35_));
  inv1   g019(.a(x6), .O(new_n36_));
  nand3  g020(.a(new_n25_), .b(new_n36_), .c(x2), .O(new_n37_));
  nand3  g021(.a(new_n37_), .b(new_n35_), .c(x9), .O(new_n38_));
  nand2  g022(.a(new_n38_), .b(x7), .O(new_n39_));
  nand2  g023(.a(x6), .b(new_n17_), .O(new_n40_));
  nand2  g024(.a(new_n40_), .b(x7), .O(new_n41_));
  nand4  g025(.a(new_n41_), .b(x9), .c(x8), .d(x2), .O(new_n42_));
  nand4  g026(.a(new_n42_), .b(new_n39_), .c(new_n34_), .d(new_n29_), .O(new_n43_));
  oai21  g027(.a(new_n43_), .b(new_n24_), .c(x0), .O(new_n44_));
  inv1   g028(.a(x0), .O(new_n45_));
  nand2  g029(.a(new_n18_), .b(x6), .O(new_n46_));
  oai21  g030(.a(new_n17_), .b(new_n21_), .c(new_n46_), .O(new_n47_));
  nor2   g031(.a(new_n17_), .b(new_n21_), .O(new_n48_));
  nand4  g032(.a(new_n48_), .b(x9), .c(new_n18_), .d(x6), .O(new_n49_));
  aoi21  g033(.a(new_n49_), .b(new_n47_), .c(x8), .O(new_n50_));
  nor2   g034(.a(new_n25_), .b(x7), .O(new_n51_));
  nand2  g035(.a(new_n51_), .b(x6), .O(new_n52_));
  aoi21  g036(.a(new_n52_), .b(x9), .c(x4), .O(new_n53_));
  oai21  g037(.a(new_n53_), .b(new_n50_), .c(new_n30_), .O(new_n54_));
  nand3  g038(.a(x8), .b(x7), .c(x4), .O(new_n55_));
  nand2  g039(.a(new_n25_), .b(x5), .O(new_n56_));
  inv1   g040(.a(new_n56_), .O(new_n57_));
  nand2  g041(.a(new_n57_), .b(new_n17_), .O(new_n58_));
  aoi21  g042(.a(new_n58_), .b(new_n55_), .c(new_n21_), .O(new_n59_));
  inv1   g043(.a(new_n46_), .O(new_n60_));
  oai21  g044(.a(new_n60_), .b(x5), .c(new_n21_), .O(new_n61_));
  nand2  g045(.a(x5), .b(x4), .O(new_n62_));
  aoi21  g046(.a(new_n62_), .b(new_n61_), .c(new_n25_), .O(new_n63_));
  oai21  g047(.a(new_n63_), .b(new_n59_), .c(x9), .O(new_n64_));
  nand4  g048(.a(new_n22_), .b(new_n18_), .c(new_n17_), .d(new_n21_), .O(new_n65_));
  nand3  g049(.a(new_n65_), .b(new_n64_), .c(new_n54_), .O(new_n66_));
  nand2  g050(.a(new_n66_), .b(new_n45_), .O(new_n67_));
  nand3  g051(.a(new_n51_), .b(new_n36_), .c(x4), .O(new_n68_));
  nand3  g052(.a(new_n22_), .b(x7), .c(new_n17_), .O(new_n69_));
  aoi21  g053(.a(new_n69_), .b(new_n68_), .c(new_n21_), .O(new_n70_));
  nand3  g054(.a(new_n18_), .b(x5), .c(x4), .O(new_n71_));
  aoi21  g055(.a(new_n71_), .b(x9), .c(x6), .O(new_n72_));
  nor3   g056(.a(x9), .b(x5), .c(x2), .O(new_n73_));
  nor3   g057(.a(new_n73_), .b(new_n72_), .c(new_n70_), .O(new_n74_));
  nand3  g058(.a(new_n74_), .b(new_n67_), .c(new_n44_), .O(z0));
  inv1   g059(.a(x1), .O(new_n76_));
  nand3  g060(.a(new_n51_), .b(x6), .c(new_n30_), .O(new_n77_));
  inv1   g061(.a(x3), .O(new_n78_));
  nor2   g062(.a(new_n22_), .b(new_n30_), .O(new_n79_));
  nand3  g063(.a(new_n79_), .b(new_n78_), .c(x2), .O(new_n80_));
  nand2  g064(.a(new_n80_), .b(new_n77_), .O(new_n81_));
  nand2  g065(.a(new_n81_), .b(new_n45_), .O(new_n82_));
  nand2  g066(.a(x7), .b(new_n30_), .O(new_n83_));
  nand3  g067(.a(x9), .b(new_n78_), .c(x2), .O(new_n84_));
  aoi21  g068(.a(new_n84_), .b(new_n83_), .c(new_n45_), .O(new_n85_));
  nor2   g069(.a(x7), .b(x5), .O(new_n86_));
  nand2  g070(.a(new_n86_), .b(x2), .O(new_n87_));
  inv1   g071(.a(new_n87_), .O(new_n88_));
  oai21  g072(.a(new_n88_), .b(new_n85_), .c(x8), .O(new_n89_));
  nor2   g073(.a(x2), .b(new_n45_), .O(new_n90_));
  nand3  g074(.a(x9), .b(x5), .c(x3), .O(new_n91_));
  inv1   g075(.a(new_n91_), .O(new_n92_));
  nand2  g076(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand2  g077(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  nand2  g078(.a(new_n36_), .b(new_n30_), .O(new_n95_));
  aoi21  g079(.a(new_n95_), .b(new_n91_), .c(new_n45_), .O(new_n96_));
  nand4  g080(.a(x9), .b(x5), .c(x3), .d(new_n21_), .O(new_n97_));
  inv1   g081(.a(new_n97_), .O(new_n98_));
  oai21  g082(.a(new_n98_), .b(new_n96_), .c(new_n25_), .O(new_n99_));
  oai21  g083(.a(x7), .b(x3), .c(x5), .O(new_n100_));
  nand2  g084(.a(new_n100_), .b(new_n22_), .O(new_n101_));
  nand2  g085(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  aoi21  g086(.a(new_n94_), .b(x6), .c(new_n102_), .O(new_n103_));
  aoi21  g087(.a(new_n103_), .b(new_n82_), .c(x4), .O(new_n104_));
  nand4  g088(.a(new_n30_), .b(x4), .c(x3), .d(new_n21_), .O(new_n105_));
  nand3  g089(.a(x8), .b(new_n78_), .c(x2), .O(new_n106_));
  aoi21  g090(.a(new_n106_), .b(new_n105_), .c(new_n45_), .O(new_n107_));
  nand2  g091(.a(x8), .b(new_n78_), .O(new_n108_));
  nand2  g092(.a(new_n21_), .b(new_n45_), .O(new_n109_));
  nand2  g093(.a(x4), .b(x3), .O(new_n110_));
  nand2  g094(.a(new_n25_), .b(new_n30_), .O(new_n111_));
  oai22  g095(.a(new_n111_), .b(new_n110_), .c(new_n109_), .d(new_n108_), .O(new_n112_));
  oai21  g096(.a(new_n112_), .b(new_n107_), .c(x6), .O(new_n113_));
  nand3  g097(.a(new_n48_), .b(x8), .c(x5), .O(new_n114_));
  aoi21  g098(.a(new_n114_), .b(new_n113_), .c(x7), .O(new_n115_));
  oai21  g099(.a(x7), .b(x2), .c(x8), .O(new_n116_));
  oai21  g100(.a(new_n111_), .b(x2), .c(new_n116_), .O(new_n117_));
  nand3  g101(.a(new_n117_), .b(x4), .c(x3), .O(new_n118_));
  nand3  g102(.a(x8), .b(x5), .c(new_n78_), .O(new_n119_));
  aoi21  g103(.a(new_n119_), .b(new_n118_), .c(x0), .O(new_n120_));
  oai21  g104(.a(new_n120_), .b(new_n115_), .c(x9), .O(new_n121_));
  nand2  g105(.a(new_n25_), .b(new_n36_), .O(new_n122_));
  nand3  g106(.a(x8), .b(x7), .c(x2), .O(new_n123_));
  aoi21  g107(.a(new_n123_), .b(new_n122_), .c(new_n45_), .O(new_n124_));
  nand2  g108(.a(new_n48_), .b(new_n19_), .O(new_n125_));
  inv1   g109(.a(new_n125_), .O(new_n126_));
  oai21  g110(.a(new_n126_), .b(new_n124_), .c(new_n30_), .O(new_n127_));
  nand3  g111(.a(new_n48_), .b(new_n31_), .c(new_n36_), .O(new_n128_));
  nand2  g112(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g113(.a(x8), .b(new_n36_), .c(x4), .O(new_n130_));
  nor3   g114(.a(new_n130_), .b(new_n78_), .c(x2), .O(new_n131_));
  aoi21  g115(.a(new_n129_), .b(new_n78_), .c(new_n131_), .O(new_n132_));
  nand2  g116(.a(new_n132_), .b(new_n121_), .O(new_n133_));
  oai21  g117(.a(new_n133_), .b(new_n104_), .c(new_n76_), .O(new_n134_));
  nand3  g118(.a(new_n19_), .b(new_n36_), .c(x5), .O(new_n135_));
  nor2   g119(.a(new_n22_), .b(new_n25_), .O(new_n136_));
  nand3  g120(.a(new_n136_), .b(new_n18_), .c(new_n45_), .O(new_n137_));
  nand2  g121(.a(new_n137_), .b(new_n135_), .O(new_n138_));
  nand2  g122(.a(new_n138_), .b(new_n21_), .O(new_n139_));
  inv1   g123(.a(new_n111_), .O(new_n140_));
  nor2   g124(.a(x6), .b(new_n21_), .O(new_n141_));
  oai21  g125(.a(new_n141_), .b(new_n140_), .c(x0), .O(new_n142_));
  oai22  g126(.a(new_n40_), .b(x0), .c(x8), .d(new_n17_), .O(new_n143_));
  nand2  g127(.a(new_n143_), .b(x2), .O(new_n144_));
  nand2  g128(.a(new_n57_), .b(x4), .O(new_n145_));
  nand3  g129(.a(new_n145_), .b(new_n144_), .c(new_n142_), .O(new_n146_));
  nand2  g130(.a(new_n146_), .b(x7), .O(new_n147_));
  nand4  g131(.a(new_n83_), .b(x9), .c(x8), .d(x0), .O(new_n148_));
  nand4  g132(.a(new_n18_), .b(x5), .c(new_n17_), .d(new_n45_), .O(new_n149_));
  aoi21  g133(.a(new_n149_), .b(new_n148_), .c(new_n21_), .O(new_n150_));
  nand3  g134(.a(new_n136_), .b(x6), .c(new_n45_), .O(new_n151_));
  nand2  g135(.a(new_n22_), .b(x5), .O(new_n152_));
  aoi21  g136(.a(new_n152_), .b(new_n151_), .c(x4), .O(new_n153_));
  nor2   g137(.a(new_n153_), .b(new_n150_), .O(new_n154_));
  nand3  g138(.a(new_n154_), .b(new_n147_), .c(new_n139_), .O(new_n155_));
  nand2  g139(.a(new_n155_), .b(x1), .O(new_n156_));
  nand2  g140(.a(new_n79_), .b(new_n45_), .O(new_n157_));
  nand2  g141(.a(new_n157_), .b(x6), .O(new_n158_));
  nand4  g142(.a(new_n158_), .b(x8), .c(new_n18_), .d(x4), .O(new_n159_));
  nand2  g143(.a(new_n141_), .b(x0), .O(new_n160_));
  nand2  g144(.a(new_n160_), .b(x9), .O(new_n161_));
  nand3  g145(.a(new_n161_), .b(x7), .c(new_n17_), .O(new_n162_));
  nand3  g146(.a(new_n162_), .b(new_n159_), .c(new_n156_), .O(new_n163_));
  nand2  g147(.a(new_n163_), .b(x3), .O(new_n164_));
  aoi21  g148(.a(x5), .b(new_n21_), .c(new_n25_), .O(new_n165_));
  oai22  g149(.a(new_n165_), .b(new_n45_), .c(new_n56_), .d(x2), .O(new_n166_));
  nand2  g150(.a(new_n166_), .b(x9), .O(new_n167_));
  nand3  g151(.a(new_n18_), .b(new_n30_), .c(new_n45_), .O(new_n168_));
  aoi21  g152(.a(new_n168_), .b(x8), .c(new_n17_), .O(new_n169_));
  nor3   g153(.a(x8), .b(x7), .c(x5), .O(new_n170_));
  aoi21  g154(.a(new_n169_), .b(x2), .c(new_n170_), .O(new_n171_));
  aoi21  g155(.a(new_n171_), .b(new_n167_), .c(new_n36_), .O(new_n172_));
  oai21  g156(.a(x5), .b(x0), .c(new_n25_), .O(new_n173_));
  oai21  g157(.a(x4), .b(new_n21_), .c(new_n25_), .O(new_n174_));
  aoi22  g158(.a(new_n174_), .b(new_n45_), .c(new_n173_), .d(new_n21_), .O(new_n175_));
  nor3   g159(.a(new_n22_), .b(new_n25_), .c(x7), .O(new_n176_));
  aoi22  g160(.a(new_n176_), .b(new_n90_), .c(new_n22_), .d(x4), .O(new_n177_));
  oai21  g161(.a(new_n175_), .b(x6), .c(new_n177_), .O(new_n178_));
  oai21  g162(.a(new_n178_), .b(new_n172_), .c(new_n78_), .O(new_n179_));
  nor2   g163(.a(new_n18_), .b(new_n36_), .O(new_n180_));
  nor2   g164(.a(x7), .b(x6), .O(new_n181_));
  oai21  g165(.a(new_n181_), .b(new_n180_), .c(new_n17_), .O(new_n182_));
  nand2  g166(.a(new_n181_), .b(new_n21_), .O(new_n183_));
  aoi21  g167(.a(new_n183_), .b(new_n182_), .c(x5), .O(new_n184_));
  nand3  g168(.a(x6), .b(x4), .c(x0), .O(new_n185_));
  nand2  g169(.a(new_n185_), .b(x9), .O(new_n186_));
  nand2  g170(.a(new_n186_), .b(x7), .O(new_n187_));
  nand2  g171(.a(new_n51_), .b(new_n36_), .O(new_n188_));
  nand2  g172(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  aoi21  g173(.a(new_n184_), .b(new_n45_), .c(new_n189_), .O(new_n190_));
  nand2  g174(.a(new_n190_), .b(new_n179_), .O(new_n191_));
  nand2  g175(.a(new_n78_), .b(x0), .O(new_n192_));
  nand2  g176(.a(new_n180_), .b(new_n30_), .O(new_n193_));
  nand2  g177(.a(new_n181_), .b(x5), .O(new_n194_));
  oai21  g178(.a(new_n193_), .b(new_n192_), .c(new_n194_), .O(new_n195_));
  nand2  g179(.a(new_n195_), .b(x4), .O(new_n196_));
  nand2  g180(.a(x7), .b(x2), .O(new_n197_));
  nand4  g181(.a(new_n197_), .b(x8), .c(new_n36_), .d(x5), .O(new_n198_));
  oai21  g182(.a(x9), .b(x5), .c(new_n198_), .O(new_n199_));
  nor2   g183(.a(x9), .b(x6), .O(new_n200_));
  aoi21  g184(.a(new_n199_), .b(new_n78_), .c(new_n200_), .O(new_n201_));
  nand2  g185(.a(new_n201_), .b(new_n196_), .O(new_n202_));
  aoi21  g186(.a(new_n191_), .b(x1), .c(new_n202_), .O(new_n203_));
  nand3  g187(.a(new_n203_), .b(new_n164_), .c(new_n134_), .O(z1));
  xor2a  g188(.a(x3), .b(x1), .O(new_n205_));
  nor2   g189(.a(new_n205_), .b(new_n200_), .O(z2));
  inv1   g190(.a(new_n200_), .O(new_n207_));
  oai21  g191(.a(new_n78_), .b(new_n76_), .c(new_n207_), .O(z3));
  nand3  g192(.a(new_n18_), .b(x3), .c(x2), .O(new_n209_));
  oai21  g193(.a(x1), .b(x0), .c(new_n209_), .O(new_n210_));
  nand2  g194(.a(new_n210_), .b(x4), .O(new_n211_));
  nand2  g195(.a(x2), .b(x0), .O(new_n212_));
  aoi21  g196(.a(new_n212_), .b(new_n109_), .c(new_n205_), .O(new_n213_));
  inv1   g197(.a(new_n213_), .O(new_n214_));
  oai21  g198(.a(x2), .b(new_n45_), .c(new_n76_), .O(new_n215_));
  nand4  g199(.a(new_n215_), .b(x7), .c(new_n17_), .d(new_n78_), .O(new_n216_));
  oai21  g200(.a(new_n214_), .b(x7), .c(new_n216_), .O(new_n217_));
  nand2  g201(.a(new_n217_), .b(x6), .O(new_n218_));
  nand3  g202(.a(new_n78_), .b(new_n76_), .c(new_n45_), .O(new_n219_));
  nand3  g203(.a(x7), .b(new_n17_), .c(x1), .O(new_n220_));
  nand2  g204(.a(new_n220_), .b(new_n219_), .O(new_n221_));
  nand3  g205(.a(new_n221_), .b(new_n25_), .c(new_n21_), .O(new_n222_));
  nand3  g206(.a(new_n222_), .b(new_n218_), .c(new_n211_), .O(new_n223_));
  nand2  g207(.a(new_n223_), .b(x5), .O(new_n224_));
  inv1   g208(.a(new_n220_), .O(new_n225_));
  oai21  g209(.a(new_n225_), .b(new_n86_), .c(new_n21_), .O(new_n226_));
  oai21  g210(.a(new_n18_), .b(x3), .c(x5), .O(new_n227_));
  nand3  g211(.a(new_n227_), .b(new_n25_), .c(new_n17_), .O(new_n228_));
  or2    g212(.a(new_n110_), .b(new_n83_), .O(new_n229_));
  nand3  g213(.a(new_n229_), .b(new_n228_), .c(new_n226_), .O(new_n230_));
  nand2  g214(.a(new_n230_), .b(x6), .O(new_n231_));
  nand2  g215(.a(new_n197_), .b(x8), .O(new_n232_));
  nand2  g216(.a(new_n110_), .b(new_n76_), .O(new_n233_));
  nand3  g217(.a(new_n233_), .b(new_n232_), .c(new_n30_), .O(new_n234_));
  nor2   g218(.a(x8), .b(x4), .O(new_n235_));
  nand4  g219(.a(new_n235_), .b(x3), .c(x2), .d(x1), .O(new_n236_));
  nand3  g220(.a(new_n236_), .b(new_n234_), .c(new_n231_), .O(new_n237_));
  nand2  g221(.a(new_n237_), .b(x0), .O(new_n238_));
  nor2   g222(.a(x8), .b(new_n21_), .O(new_n239_));
  oai21  g223(.a(new_n239_), .b(new_n180_), .c(x1), .O(new_n240_));
  oai22  g224(.a(new_n46_), .b(x0), .c(x8), .d(new_n78_), .O(new_n241_));
  nor2   g225(.a(new_n78_), .b(x1), .O(new_n242_));
  aoi22  g226(.a(new_n242_), .b(new_n60_), .c(new_n241_), .d(x2), .O(new_n243_));
  aoi21  g227(.a(new_n243_), .b(new_n240_), .c(new_n17_), .O(new_n244_));
  oai21  g228(.a(x3), .b(new_n76_), .c(x8), .O(new_n245_));
  nand3  g229(.a(new_n245_), .b(new_n18_), .c(x6), .O(new_n246_));
  inv1   g230(.a(new_n246_), .O(new_n247_));
  oai21  g231(.a(new_n247_), .b(new_n244_), .c(new_n30_), .O(new_n248_));
  nand3  g232(.a(new_n248_), .b(new_n238_), .c(new_n224_), .O(new_n249_));
  nand2  g233(.a(new_n249_), .b(x9), .O(new_n250_));
  nand3  g234(.a(new_n232_), .b(new_n17_), .c(x0), .O(new_n251_));
  nand2  g235(.a(new_n212_), .b(x4), .O(new_n252_));
  nand2  g236(.a(new_n252_), .b(new_n251_), .O(new_n253_));
  nand2  g237(.a(new_n253_), .b(new_n78_), .O(new_n254_));
  nand2  g238(.a(new_n235_), .b(x1), .O(new_n255_));
  oai21  g239(.a(new_n17_), .b(x1), .c(new_n255_), .O(new_n256_));
  aoi22  g240(.a(new_n256_), .b(new_n21_), .c(new_n18_), .d(x4), .O(new_n257_));
  aoi21  g241(.a(new_n257_), .b(new_n254_), .c(new_n30_), .O(new_n258_));
  nand4  g242(.a(new_n62_), .b(x7), .c(x3), .d(x1), .O(new_n259_));
  nand2  g243(.a(new_n259_), .b(x9), .O(new_n260_));
  oai21  g244(.a(new_n260_), .b(new_n258_), .c(new_n36_), .O(new_n261_));
  nand2  g245(.a(new_n261_), .b(new_n250_), .O(z4));
  nand2  g246(.a(new_n214_), .b(new_n207_), .O(z5));
endmodule


