// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  nor2   g000(.a(x30), .b(x11), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(x03), .b(x02), .O(new_n59_));
  nand2  g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n49_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(new_n52_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n52_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x05), .c(new_n49_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n63_), .c(x01), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n61_), .c(new_n57_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  xnor2a g024(.a(x03), .b(x02), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  nand2  g026(.a(new_n64_), .b(new_n62_), .O(new_n71_));
  nand4  g027(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n68_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n67_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n46_), .O(z2));
  inv1   g033(.a(x26), .O(new_n78_));
  inv1   g034(.a(x27), .O(new_n79_));
  oai22  g035(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n80_));
  nor2   g036(.a(x22), .b(x17), .O(new_n81_));
  nor2   g037(.a(x23), .b(x18), .O(new_n82_));
  nor2   g038(.a(x24), .b(x19), .O(new_n83_));
  nor4   g039(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(new_n80_), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(new_n59_), .c(new_n46_), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n79_), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n78_), .c(x25), .d(x01), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(z3));
  xor2a  g047(.a(x28), .b(x27), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n87_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n78_), .c(x25), .d(x01), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z4));
  inv1   g052(.a(x28), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n79_), .c(x29), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x28), .c(x27), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n85_), .c(new_n59_), .d(new_n46_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n78_), .c(x25), .d(x01), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(z5));
  nand2  g061(.a(x30), .b(new_n99_), .O(new_n106_));
  inv1   g062(.a(x30), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x29), .c(x11), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(new_n106_), .c(new_n97_), .O(new_n109_));
  nor2   g065(.a(new_n107_), .b(x28), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n109_), .c(x27), .O(new_n111_));
  nand2  g067(.a(x30), .b(new_n79_), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(new_n111_), .c(new_n84_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n59_), .c(new_n78_), .d(x25), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n58_), .O(z6));
  nor4   g071(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(new_n80_), .O(new_n116_));
  nand4  g072(.a(new_n59_), .b(new_n78_), .c(x25), .d(x01), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n116_), .c(new_n46_), .O(new_n118_));
  nand3  g074(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x31), .O(new_n120_));
  inv1   g076(.a(x31), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(x29), .c(x28), .d(x27), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nor2   g079(.a(new_n121_), .b(x30), .O(new_n124_));
  aoi22  g080(.a(new_n124_), .b(x11), .c(new_n123_), .d(x30), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n118_), .O(z7));
  inv1   g082(.a(x11), .O(new_n127_));
  inv1   g083(.a(x16), .O(new_n128_));
  nand4  g084(.a(x32), .b(new_n78_), .c(new_n128_), .d(x00), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x30), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n127_), .O(new_n131_));
  nor2   g087(.a(x28), .b(x27), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x29), .O(new_n134_));
  inv1   g090(.a(x18), .O(new_n135_));
  nand3  g091(.a(x19), .b(new_n135_), .c(x13), .O(new_n136_));
  inv1   g092(.a(x19), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(x18), .c(x14), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n136_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x17), .O(new_n140_));
  inv1   g096(.a(x17), .O(new_n141_));
  nor2   g097(.a(new_n137_), .b(new_n135_), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(new_n141_), .c(x12), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n140_), .c(new_n128_), .O(new_n144_));
  nand2  g100(.a(new_n142_), .b(x17), .O(new_n145_));
  nor3   g101(.a(new_n145_), .b(x16), .c(new_n127_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(x15), .O(new_n147_));
  inv1   g103(.a(x15), .O(new_n148_));
  inv1   g104(.a(new_n145_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(x16), .c(new_n148_), .d(x10), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n147_), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(new_n134_), .c(x30), .O(new_n152_));
  nand3  g108(.a(new_n142_), .b(x17), .c(new_n128_), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n144_), .c(x15), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n150_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n133_), .c(new_n107_), .d(x29), .O(new_n157_));
  aoi21  g113(.a(new_n157_), .b(new_n152_), .c(x32), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x31), .O(new_n159_));
  nand3  g115(.a(new_n134_), .b(new_n107_), .c(x11), .O(new_n160_));
  aoi21  g116(.a(new_n137_), .b(x14), .c(new_n135_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(x17), .c(x16), .d(x15), .O(new_n162_));
  nand2  g118(.a(new_n97_), .b(new_n79_), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(x30), .c(x29), .O(new_n164_));
  nand3  g120(.a(new_n142_), .b(x17), .c(x15), .O(new_n165_));
  nand2  g121(.a(x18), .b(x17), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n148_), .c(new_n137_), .O(new_n167_));
  inv1   g123(.a(x10), .O(new_n168_));
  oai21  g124(.a(new_n166_), .b(new_n168_), .c(new_n148_), .O(new_n169_));
  nand2  g125(.a(x18), .b(x12), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n141_), .O(new_n171_));
  inv1   g127(.a(x13), .O(new_n172_));
  aoi21  g128(.a(new_n135_), .b(new_n172_), .c(new_n121_), .O(new_n173_));
  nand4  g129(.a(new_n173_), .b(new_n171_), .c(new_n169_), .d(new_n167_), .O(new_n174_));
  aoi21  g130(.a(new_n165_), .b(new_n128_), .c(new_n174_), .O(new_n175_));
  nand4  g131(.a(new_n175_), .b(new_n164_), .c(new_n162_), .d(new_n160_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(x32), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n159_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n78_), .c(x00), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n131_), .O(z8));
  nand2  g136(.a(new_n135_), .b(new_n172_), .O(new_n181_));
  nand4  g137(.a(new_n181_), .b(new_n171_), .c(new_n169_), .d(new_n167_), .O(new_n182_));
  inv1   g138(.a(new_n182_), .O(new_n183_));
  aoi21  g139(.a(new_n183_), .b(new_n162_), .c(new_n45_), .O(new_n184_));
  nand3  g140(.a(new_n133_), .b(new_n121_), .c(x29), .O(new_n185_));
  aoi21  g141(.a(new_n165_), .b(new_n128_), .c(new_n124_), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n185_), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(x11), .O(new_n188_));
  oai21  g144(.a(x16), .b(x11), .c(new_n133_), .O(new_n189_));
  nand2  g145(.a(x31), .b(x29), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n189_), .c(x30), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n188_), .O(new_n192_));
  oai21  g148(.a(new_n192_), .b(new_n184_), .c(x33), .O(new_n193_));
  inv1   g149(.a(x33), .O(new_n194_));
  nand4  g150(.a(new_n156_), .b(new_n134_), .c(new_n121_), .d(new_n107_), .O(new_n195_));
  nor2   g151(.a(new_n132_), .b(new_n121_), .O(new_n196_));
  nand4  g152(.a(new_n196_), .b(x30), .c(x29), .d(x19), .O(new_n197_));
  nor2   g153(.a(new_n197_), .b(new_n135_), .O(new_n198_));
  nand4  g154(.a(new_n198_), .b(x17), .c(new_n128_), .d(x15), .O(new_n199_));
  aoi21  g155(.a(new_n199_), .b(new_n195_), .c(new_n127_), .O(new_n200_));
  nand2  g156(.a(new_n143_), .b(new_n140_), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(x15), .O(new_n202_));
  nand3  g158(.a(new_n149_), .b(new_n148_), .c(x10), .O(new_n203_));
  aoi21  g159(.a(new_n203_), .b(new_n202_), .c(new_n132_), .O(new_n204_));
  nand4  g160(.a(new_n204_), .b(x31), .c(x30), .d(x29), .O(new_n205_));
  nor2   g161(.a(new_n205_), .b(new_n128_), .O(new_n206_));
  oai21  g162(.a(new_n206_), .b(new_n200_), .c(new_n194_), .O(new_n207_));
  nand2  g163(.a(new_n207_), .b(new_n193_), .O(new_n208_));
  nand3  g164(.a(new_n208_), .b(new_n78_), .c(x00), .O(new_n209_));
  inv1   g165(.a(new_n209_), .O(z9));
endmodule


