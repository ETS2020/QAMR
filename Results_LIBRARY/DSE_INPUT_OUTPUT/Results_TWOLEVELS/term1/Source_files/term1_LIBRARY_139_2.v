// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  inv1   g000(.a(x30), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x24), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nor2   g004(.a(x32), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nor2   g009(.a(x32), .b(new_n50_), .O(new_n54_));
  nor2   g010(.a(x33), .b(x03), .O(new_n55_));
  oai21  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n47_), .O(z1));
  inv1   g013(.a(x09), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n50_), .b(new_n53_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n50_), .b(new_n53_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n50_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n53_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n53_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n50_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n46_), .c(new_n58_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x17), .O(new_n86_));
  inv1   g042(.a(x22), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x18), .O(new_n89_));
  inv1   g045(.a(x23), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n46_), .O(new_n93_));
  inv1   g049(.a(x19), .O(new_n94_));
  inv1   g050(.a(x24), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  aoi21  g052(.a(new_n96_), .b(new_n93_), .c(new_n61_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n60_), .O(z3));
  xor2a  g055(.a(x28), .b(x27), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n97_), .c(new_n81_), .d(x25), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n60_), .O(z4));
  inv1   g058(.a(x28), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n82_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n92_), .O(new_n108_));
  aoi21  g064(.a(x30), .b(new_n103_), .c(new_n82_), .O(new_n109_));
  oai22  g065(.a(new_n109_), .b(x24), .c(x30), .d(x28), .O(new_n110_));
  nand4  g066(.a(new_n105_), .b(x28), .c(x27), .d(new_n95_), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(new_n112_));
  aoi21  g068(.a(new_n110_), .b(x29), .c(new_n112_), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(x19), .c(new_n108_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n60_), .c(new_n46_), .O(z5));
  nand2  g072(.a(x30), .b(new_n103_), .O(new_n117_));
  nand4  g073(.a(new_n45_), .b(x29), .c(x28), .d(new_n95_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n82_), .O(new_n119_));
  aoi21  g075(.a(x29), .b(x27), .c(new_n45_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n92_), .O(new_n121_));
  nand3  g077(.a(new_n45_), .b(x28), .c(x27), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n117_), .c(new_n105_), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n120_), .c(new_n95_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(x19), .c(new_n121_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n60_), .O(z6));
  nand4  g083(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n46_), .O(new_n129_));
  nand3  g085(.a(x29), .b(x28), .c(x27), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x31), .O(new_n131_));
  nand3  g087(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x24), .O(new_n134_));
  inv1   g090(.a(x31), .O(new_n135_));
  nand4  g091(.a(new_n135_), .b(x29), .c(x28), .d(x27), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n134_), .c(new_n131_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x30), .O(new_n138_));
  nand2  g094(.a(x31), .b(new_n45_), .O(new_n139_));
  oai21  g095(.a(new_n132_), .b(new_n94_), .c(new_n139_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n95_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n138_), .c(new_n129_), .O(z7));
  inv1   g098(.a(x15), .O(new_n143_));
  nand2  g099(.a(x16), .b(x10), .O(new_n144_));
  nand3  g100(.a(x19), .b(x18), .c(x17), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n144_), .c(new_n143_), .O(new_n146_));
  inv1   g102(.a(x16), .O(new_n147_));
  nand2  g103(.a(x19), .b(x18), .O(new_n148_));
  nand2  g104(.a(x17), .b(x11), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n148_), .c(new_n147_), .O(new_n150_));
  inv1   g106(.a(x12), .O(new_n151_));
  oai21  g107(.a(new_n148_), .b(new_n151_), .c(new_n86_), .O(new_n152_));
  aoi21  g108(.a(x19), .b(x13), .c(x18), .O(new_n153_));
  nor2   g109(.a(x19), .b(x14), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n153_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n152_), .c(new_n150_), .d(new_n146_), .O(new_n156_));
  oai21  g112(.a(x30), .b(new_n95_), .c(new_n156_), .O(new_n157_));
  nand2  g113(.a(x16), .b(x15), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n145_), .c(x31), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n46_), .O(new_n160_));
  nand2  g116(.a(new_n103_), .b(new_n82_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x29), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n45_), .c(new_n95_), .O(new_n163_));
  nand2  g119(.a(new_n103_), .b(new_n82_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(x30), .c(x29), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(new_n163_), .c(new_n160_), .d(new_n157_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x32), .O(new_n167_));
  inv1   g123(.a(x32), .O(new_n168_));
  nand4  g124(.a(new_n161_), .b(new_n45_), .c(x29), .d(new_n95_), .O(new_n169_));
  nand2  g125(.a(new_n162_), .b(x30), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand3  g127(.a(x19), .b(new_n89_), .c(x13), .O(new_n172_));
  nand3  g128(.a(new_n94_), .b(x18), .c(x14), .O(new_n173_));
  aoi21  g129(.a(new_n173_), .b(new_n172_), .c(new_n86_), .O(new_n174_));
  nand4  g130(.a(x19), .b(x18), .c(new_n86_), .d(x12), .O(new_n175_));
  inv1   g131(.a(new_n175_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n174_), .c(x16), .O(new_n177_));
  inv1   g133(.a(new_n145_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n147_), .c(x11), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n177_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(x15), .O(new_n181_));
  nand4  g137(.a(new_n178_), .b(x16), .c(new_n143_), .d(x10), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n171_), .c(new_n168_), .d(x31), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n167_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n81_), .c(x00), .O(new_n186_));
  inv1   g142(.a(new_n186_), .O(z8));
  nand2  g143(.a(new_n94_), .b(x14), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(x18), .c(x17), .d(x16), .O(new_n189_));
  nor2   g145(.a(new_n189_), .b(new_n143_), .O(new_n190_));
  nand2  g146(.a(x18), .b(x17), .O(new_n191_));
  oai21  g147(.a(new_n191_), .b(new_n158_), .c(new_n94_), .O(new_n192_));
  oai21  g148(.a(new_n191_), .b(new_n144_), .c(new_n143_), .O(new_n193_));
  inv1   g149(.a(x11), .O(new_n194_));
  oai21  g150(.a(new_n191_), .b(new_n194_), .c(new_n147_), .O(new_n195_));
  aoi21  g151(.a(x18), .b(x12), .c(x17), .O(new_n196_));
  nor2   g152(.a(x18), .b(x13), .O(new_n197_));
  nor2   g153(.a(new_n197_), .b(new_n196_), .O(new_n198_));
  nand4  g154(.a(new_n198_), .b(new_n195_), .c(new_n193_), .d(new_n192_), .O(new_n199_));
  oai21  g155(.a(new_n199_), .b(new_n190_), .c(new_n46_), .O(new_n200_));
  nand3  g156(.a(new_n161_), .b(new_n135_), .c(x29), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n139_), .O(new_n202_));
  nor2   g158(.a(new_n135_), .b(new_n105_), .O(new_n203_));
  aoi21  g159(.a(new_n203_), .b(new_n161_), .c(new_n45_), .O(new_n204_));
  aoi21  g160(.a(new_n202_), .b(new_n95_), .c(new_n204_), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n200_), .O(new_n206_));
  nand2  g162(.a(new_n206_), .b(x33), .O(new_n207_));
  aoi21  g163(.a(new_n179_), .b(new_n177_), .c(new_n143_), .O(new_n208_));
  inv1   g164(.a(new_n182_), .O(new_n209_));
  nand4  g165(.a(new_n162_), .b(new_n135_), .c(new_n45_), .d(new_n95_), .O(new_n210_));
  nand4  g166(.a(new_n161_), .b(x31), .c(x30), .d(x29), .O(new_n211_));
  nand2  g167(.a(new_n211_), .b(new_n210_), .O(new_n212_));
  oai21  g168(.a(new_n209_), .b(new_n208_), .c(new_n212_), .O(new_n213_));
  oai21  g169(.a(new_n213_), .b(x33), .c(new_n207_), .O(new_n214_));
  nand3  g170(.a(new_n214_), .b(new_n81_), .c(x00), .O(new_n215_));
  inv1   g171(.a(new_n215_), .O(z9));
endmodule


