// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_;
  inv1   g000(.a(x30), .O(new_n45_));
  nand2  g001(.a(new_n45_), .b(x00), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nor2   g013(.a(new_n49_), .b(new_n52_), .O(new_n58_));
  nor2   g014(.a(x03), .b(x02), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n49_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(new_n52_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n52_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x05), .c(new_n49_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n60_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(new_n57_), .c(new_n46_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(x01), .O(new_n72_));
  and2   g028(.a(x06), .b(x05), .O(new_n73_));
  aoi22  g029(.a(new_n73_), .b(new_n59_), .c(new_n64_), .d(new_n62_), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(x01), .c(new_n72_), .O(new_n75_));
  nand3  g031(.a(new_n75_), .b(new_n70_), .c(new_n46_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n69_), .c(x09), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  nor2   g039(.a(x22), .b(x17), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(new_n85_));
  nor2   g041(.a(x23), .b(x18), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(new_n87_), .c(new_n85_), .d(new_n83_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n58_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n93_));
  oai21  g049(.a(new_n93_), .b(new_n61_), .c(new_n46_), .O(z3));
  inv1   g050(.a(new_n58_), .O(new_n95_));
  xor2a  g051(.a(x28), .b(x27), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n90_), .c(new_n95_), .d(new_n46_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n80_), .c(x25), .d(x01), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z4));
  inv1   g056(.a(x29), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n81_), .O(new_n103_));
  nand3  g059(.a(new_n101_), .b(x28), .c(x27), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n101_), .c(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n90_), .c(new_n95_), .d(new_n46_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n80_), .c(x25), .d(x01), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(z5));
  inv1   g065(.a(x00), .O(new_n110_));
  nand4  g066(.a(new_n45_), .b(x28), .c(x27), .d(new_n110_), .O(new_n111_));
  oai21  g067(.a(new_n103_), .b(new_n45_), .c(new_n111_), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x29), .O(new_n113_));
  nand2  g069(.a(x30), .b(new_n101_), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n113_), .c(new_n91_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n95_), .c(new_n80_), .d(x25), .O(new_n116_));
  nor2   g072(.a(new_n116_), .b(new_n61_), .O(z6));
  oai21  g073(.a(x31), .b(x00), .c(new_n45_), .O(new_n118_));
  nand3  g074(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x31), .O(new_n120_));
  nand4  g076(.a(new_n89_), .b(new_n87_), .c(new_n85_), .d(new_n83_), .O(new_n121_));
  nor2   g077(.a(new_n58_), .b(new_n61_), .O(new_n122_));
  inv1   g078(.a(x31), .O(new_n123_));
  nand4  g079(.a(new_n103_), .b(new_n123_), .c(x30), .d(x29), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n122_), .c(new_n80_), .d(x25), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n121_), .c(new_n120_), .d(new_n118_), .O(z7));
  inv1   g083(.a(x32), .O(new_n128_));
  nand2  g084(.a(new_n102_), .b(new_n81_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x29), .O(new_n130_));
  inv1   g086(.a(x17), .O(new_n131_));
  inv1   g087(.a(x18), .O(new_n132_));
  nand3  g088(.a(x19), .b(new_n132_), .c(x13), .O(new_n133_));
  inv1   g089(.a(x19), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(x18), .c(x14), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n131_), .O(new_n136_));
  nand4  g092(.a(x19), .b(x18), .c(new_n131_), .d(x12), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n136_), .c(x16), .O(new_n139_));
  inv1   g095(.a(x16), .O(new_n140_));
  nand2  g096(.a(x19), .b(x18), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x17), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n140_), .c(x11), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n139_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x15), .O(new_n147_));
  inv1   g103(.a(x15), .O(new_n148_));
  nand4  g104(.a(new_n144_), .b(x16), .c(new_n148_), .d(x10), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n147_), .O(new_n150_));
  nand4  g106(.a(new_n150_), .b(new_n130_), .c(new_n128_), .d(x31), .O(new_n151_));
  aoi21  g107(.a(new_n102_), .b(new_n81_), .c(new_n101_), .O(new_n152_));
  nand2  g108(.a(x16), .b(x10), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n143_), .c(new_n148_), .O(new_n154_));
  nand2  g110(.a(x17), .b(x11), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n141_), .c(new_n140_), .O(new_n156_));
  inv1   g112(.a(x12), .O(new_n157_));
  oai21  g113(.a(new_n141_), .b(new_n157_), .c(new_n131_), .O(new_n158_));
  aoi21  g114(.a(x19), .b(x13), .c(x18), .O(new_n159_));
  nor2   g115(.a(x19), .b(x14), .O(new_n160_));
  nand2  g116(.a(x16), .b(x15), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n143_), .c(x31), .O(new_n162_));
  nor3   g118(.a(new_n162_), .b(new_n160_), .c(new_n159_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n158_), .c(new_n156_), .d(new_n154_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(new_n152_), .c(x32), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n151_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n80_), .O(new_n167_));
  aoi21  g123(.a(new_n167_), .b(x30), .c(new_n110_), .O(z8));
  inv1   g124(.a(x33), .O(new_n169_));
  nand4  g125(.a(new_n150_), .b(new_n129_), .c(new_n169_), .d(x31), .O(new_n170_));
  aoi21  g126(.a(new_n134_), .b(x14), .c(new_n132_), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(x17), .c(x16), .d(x15), .O(new_n172_));
  nor2   g128(.a(new_n132_), .b(new_n131_), .O(new_n173_));
  inv1   g129(.a(new_n173_), .O(new_n174_));
  oai21  g130(.a(new_n174_), .b(new_n161_), .c(new_n134_), .O(new_n175_));
  oai21  g131(.a(new_n174_), .b(new_n153_), .c(new_n148_), .O(new_n176_));
  aoi21  g132(.a(new_n173_), .b(x11), .c(x16), .O(new_n177_));
  oai21  g133(.a(new_n132_), .b(new_n157_), .c(new_n131_), .O(new_n178_));
  inv1   g134(.a(x13), .O(new_n179_));
  nand2  g135(.a(new_n132_), .b(new_n179_), .O(new_n180_));
  nor2   g136(.a(new_n123_), .b(new_n101_), .O(new_n181_));
  nand4  g137(.a(new_n181_), .b(new_n180_), .c(new_n178_), .d(new_n129_), .O(new_n182_));
  nor2   g138(.a(new_n182_), .b(new_n177_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n176_), .c(new_n175_), .d(new_n172_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(x33), .O(new_n185_));
  oai21  g141(.a(new_n170_), .b(new_n101_), .c(new_n185_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(x30), .c(new_n80_), .d(x00), .O(new_n187_));
  inv1   g143(.a(new_n187_), .O(z9));
endmodule


