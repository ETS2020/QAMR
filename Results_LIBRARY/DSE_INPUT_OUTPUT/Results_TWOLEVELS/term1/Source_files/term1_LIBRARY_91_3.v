// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_;
  inv1   g000(.a(x17), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n53_), .c(new_n48_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n48_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x18), .O(new_n86_));
  inv1   g042(.a(x23), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x19), .O(new_n89_));
  inv1   g045(.a(x24), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n88_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n85_), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(new_n95_));
  nor2   g051(.a(x22), .b(x17), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  oai21  g053(.a(new_n95_), .b(new_n47_), .c(new_n97_), .O(new_n98_));
  and2   g054(.a(new_n98_), .b(new_n62_), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n60_), .O(z3));
  xor2a  g057(.a(x28), .b(x27), .O(new_n102_));
  nor2   g058(.a(new_n96_), .b(new_n92_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n85_), .c(new_n61_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n102_), .c(new_n81_), .d(x25), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n60_), .c(new_n48_), .O(z4));
  inv1   g062(.a(x28), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(new_n82_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nor2   g065(.a(new_n46_), .b(new_n45_), .O(new_n110_));
  oai21  g066(.a(new_n110_), .b(new_n95_), .c(new_n97_), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n109_), .c(x29), .O(new_n112_));
  inv1   g068(.a(x29), .O(new_n113_));
  nand4  g069(.a(new_n98_), .b(new_n113_), .c(x28), .d(x27), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n116_));
  nor2   g072(.a(new_n116_), .b(new_n60_), .O(z5));
  inv1   g073(.a(x22), .O(new_n118_));
  aoi21  g074(.a(new_n94_), .b(x29), .c(new_n118_), .O(new_n119_));
  nand3  g075(.a(new_n93_), .b(new_n85_), .c(x22), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n113_), .O(new_n121_));
  oai21  g077(.a(new_n119_), .b(new_n108_), .c(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n46_), .b(x29), .c(x28), .O(new_n123_));
  nor3   g079(.a(new_n123_), .b(new_n82_), .c(x22), .O(new_n124_));
  aoi21  g080(.a(new_n122_), .b(x30), .c(new_n124_), .O(new_n125_));
  nor2   g081(.a(new_n95_), .b(x30), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(x29), .c(x28), .d(x27), .O(new_n127_));
  oai21  g083(.a(new_n125_), .b(x17), .c(new_n127_), .O(new_n128_));
  nand4  g084(.a(new_n128_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n129_));
  nor2   g085(.a(new_n129_), .b(new_n60_), .O(z6));
  nand4  g086(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n48_), .O(new_n132_));
  nand3  g088(.a(x29), .b(x28), .c(x27), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x31), .O(new_n134_));
  nand3  g090(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x22), .O(new_n137_));
  inv1   g093(.a(x31), .O(new_n138_));
  nand4  g094(.a(new_n108_), .b(new_n138_), .c(x30), .d(x29), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(new_n134_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n45_), .O(new_n141_));
  oai21  g097(.a(new_n135_), .b(new_n45_), .c(new_n138_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n46_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n141_), .c(new_n132_), .O(z7));
  inv1   g100(.a(x00), .O(new_n145_));
  inv1   g101(.a(x32), .O(new_n146_));
  nand2  g102(.a(new_n107_), .b(new_n82_), .O(new_n147_));
  nand3  g103(.a(x19), .b(new_n86_), .c(x13), .O(new_n148_));
  nand3  g104(.a(new_n89_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n148_), .c(new_n45_), .O(new_n150_));
  nand2  g106(.a(new_n45_), .b(x12), .O(new_n151_));
  nand3  g107(.a(new_n46_), .b(x19), .c(x18), .O(new_n152_));
  nor2   g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n150_), .c(x16), .O(new_n154_));
  inv1   g110(.a(x16), .O(new_n155_));
  nand3  g111(.a(x19), .b(x18), .c(x17), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n155_), .c(x11), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(x15), .O(new_n160_));
  inv1   g116(.a(x15), .O(new_n161_));
  nand4  g117(.a(new_n157_), .b(x16), .c(new_n161_), .d(x10), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n147_), .c(new_n146_), .d(x31), .O(new_n164_));
  oai21  g120(.a(new_n82_), .b(x17), .c(new_n107_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(x32), .c(x30), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n164_), .c(new_n113_), .O(new_n167_));
  nand2  g123(.a(new_n147_), .b(x29), .O(new_n168_));
  nand4  g124(.a(x18), .b(x16), .c(x15), .d(x12), .O(new_n169_));
  nand4  g125(.a(new_n146_), .b(x31), .c(x30), .d(x19), .O(new_n170_));
  oai22  g126(.a(new_n170_), .b(new_n169_), .c(new_n146_), .d(x30), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand3  g128(.a(x16), .b(x15), .c(x13), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n86_), .O(new_n174_));
  inv1   g130(.a(x10), .O(new_n175_));
  nand2  g131(.a(x19), .b(x16), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n175_), .c(new_n161_), .O(new_n177_));
  nand2  g133(.a(x19), .b(x11), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n155_), .O(new_n179_));
  inv1   g135(.a(x14), .O(new_n180_));
  nand2  g136(.a(new_n89_), .b(new_n180_), .O(new_n181_));
  nand4  g137(.a(new_n181_), .b(new_n179_), .c(new_n177_), .d(new_n174_), .O(new_n182_));
  oai21  g138(.a(new_n89_), .b(new_n45_), .c(new_n86_), .O(new_n183_));
  nand2  g139(.a(x15), .b(x12), .O(new_n184_));
  oai21  g140(.a(new_n176_), .b(new_n184_), .c(new_n45_), .O(new_n185_));
  nand3  g141(.a(new_n157_), .b(x16), .c(x15), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n185_), .c(new_n183_), .d(x31), .O(new_n187_));
  aoi21  g143(.a(new_n182_), .b(new_n46_), .c(new_n187_), .O(new_n188_));
  oai21  g144(.a(new_n188_), .b(new_n146_), .c(new_n172_), .O(new_n189_));
  oai21  g145(.a(new_n189_), .b(new_n167_), .c(new_n81_), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n145_), .c(new_n48_), .O(z8));
  inv1   g147(.a(x33), .O(new_n192_));
  nand3  g148(.a(new_n163_), .b(new_n192_), .c(new_n138_), .O(new_n193_));
  nand2  g149(.a(x33), .b(x31), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n193_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(new_n168_), .O(new_n196_));
  nand4  g152(.a(new_n192_), .b(x31), .c(x30), .d(x19), .O(new_n197_));
  oai22  g153(.a(new_n197_), .b(new_n169_), .c(new_n192_), .d(x31), .O(new_n198_));
  nand3  g154(.a(new_n198_), .b(new_n147_), .c(x29), .O(new_n199_));
  inv1   g155(.a(new_n199_), .O(new_n200_));
  nand2  g156(.a(x16), .b(x10), .O(new_n201_));
  oai21  g157(.a(new_n201_), .b(new_n156_), .c(new_n161_), .O(new_n202_));
  nor2   g158(.a(new_n89_), .b(new_n86_), .O(new_n203_));
  nand3  g159(.a(new_n203_), .b(x17), .c(x11), .O(new_n204_));
  nand2  g160(.a(new_n204_), .b(new_n155_), .O(new_n205_));
  aoi21  g161(.a(new_n203_), .b(x12), .c(x17), .O(new_n206_));
  aoi21  g162(.a(x19), .b(x13), .c(x18), .O(new_n207_));
  xnor2a g163(.a(x31), .b(x30), .O(new_n208_));
  nand3  g164(.a(new_n208_), .b(new_n186_), .c(new_n181_), .O(new_n209_));
  nor3   g165(.a(new_n209_), .b(new_n207_), .c(new_n206_), .O(new_n210_));
  nand3  g166(.a(new_n210_), .b(new_n205_), .c(new_n202_), .O(new_n211_));
  aoi21  g167(.a(new_n211_), .b(x33), .c(new_n200_), .O(new_n212_));
  nand2  g168(.a(new_n212_), .b(new_n196_), .O(new_n213_));
  nand3  g169(.a(new_n213_), .b(new_n81_), .c(x00), .O(new_n214_));
  nand2  g170(.a(new_n214_), .b(new_n48_), .O(z9));
endmodule


