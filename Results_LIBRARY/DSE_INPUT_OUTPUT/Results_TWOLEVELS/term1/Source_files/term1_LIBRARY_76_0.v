// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_;
  nor2   g000(.a(x30), .b(x21), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n49_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n58_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xnor2a g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g028(.a(new_n66_), .b(new_n64_), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(new_n57_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x15), .O(new_n81_));
  inv1   g037(.a(x20), .O(new_n82_));
  nor2   g038(.a(x22), .b(x17), .O(new_n83_));
  aoi21  g039(.a(new_n82_), .b(new_n81_), .c(new_n83_), .O(new_n84_));
  nor2   g040(.a(x23), .b(x18), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x24), .b(x19), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  inv1   g044(.a(x16), .O(new_n89_));
  inv1   g045(.a(x21), .O(new_n90_));
  nand3  g046(.a(x30), .b(new_n90_), .c(new_n89_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n88_), .c(new_n86_), .d(new_n84_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n61_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(x27), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n80_), .c(x25), .d(x01), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n47_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nor2   g053(.a(new_n87_), .b(new_n85_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n84_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n47_), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(new_n91_), .c(new_n60_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n97_), .c(new_n80_), .d(x25), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n59_), .O(z4));
  nand2  g059(.a(x28), .b(x27), .O(new_n104_));
  inv1   g060(.a(new_n83_), .O(new_n105_));
  aoi22  g061(.a(new_n90_), .b(new_n89_), .c(new_n82_), .d(new_n81_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(new_n98_), .c(new_n105_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n104_), .c(x29), .O(new_n108_));
  inv1   g064(.a(x29), .O(new_n109_));
  nand4  g065(.a(new_n92_), .b(new_n109_), .c(x28), .d(x27), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n60_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n80_), .c(x25), .d(x01), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n47_), .O(z5));
  inv1   g069(.a(x30), .O(new_n114_));
  nand2  g070(.a(x27), .b(x21), .O(new_n115_));
  nand3  g071(.a(new_n114_), .b(x29), .c(x28), .O(new_n116_));
  oai22  g072(.a(new_n116_), .b(new_n115_), .c(new_n114_), .d(x29), .O(new_n117_));
  nand3  g073(.a(x29), .b(x28), .c(x27), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(x30), .c(new_n90_), .d(new_n89_), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  aoi21  g076(.a(new_n117_), .b(new_n99_), .c(new_n120_), .O(new_n121_));
  aoi22  g077(.a(x28), .b(x27), .c(x03), .d(x02), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(new_n99_), .c(x30), .O(new_n123_));
  oai21  g079(.a(new_n121_), .b(new_n60_), .c(new_n123_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n80_), .c(x25), .d(x01), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(z6));
  nand4  g082(.a(new_n61_), .b(new_n80_), .c(x25), .d(x01), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n47_), .O(new_n128_));
  nand3  g084(.a(x29), .b(x28), .c(x27), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x31), .O(new_n130_));
  nand3  g086(.a(new_n88_), .b(new_n86_), .c(new_n84_), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x16), .O(new_n133_));
  inv1   g089(.a(x31), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(x29), .c(x28), .d(x27), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n133_), .c(new_n130_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x30), .O(new_n137_));
  nor2   g093(.a(new_n134_), .b(x30), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n132_), .c(x21), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(new_n128_), .O(z7));
  inv1   g096(.a(x27), .O(new_n141_));
  inv1   g097(.a(x28), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n141_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(x29), .O(new_n144_));
  inv1   g100(.a(x32), .O(new_n145_));
  inv1   g101(.a(x17), .O(new_n146_));
  inv1   g102(.a(x18), .O(new_n147_));
  nand3  g103(.a(x19), .b(new_n147_), .c(x13), .O(new_n148_));
  inv1   g104(.a(x19), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(x18), .c(x14), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n148_), .c(new_n146_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n146_), .d(x12), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n89_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n154_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  nand4  g115(.a(new_n156_), .b(x16), .c(new_n81_), .d(x10), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n159_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n145_), .c(x31), .d(x30), .O(new_n162_));
  nand3  g118(.a(x32), .b(new_n114_), .c(x21), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n162_), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n144_), .O(new_n165_));
  nand4  g121(.a(new_n161_), .b(new_n143_), .c(new_n145_), .d(x31), .O(new_n166_));
  nand2  g122(.a(new_n142_), .b(new_n141_), .O(new_n167_));
  nand3  g123(.a(new_n167_), .b(x32), .c(x30), .O(new_n168_));
  oai21  g124(.a(new_n166_), .b(x30), .c(new_n168_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(x29), .O(new_n170_));
  and2   g126(.a(x16), .b(x10), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(new_n156_), .c(x15), .O(new_n172_));
  nor2   g128(.a(new_n149_), .b(new_n147_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(x17), .c(x11), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n89_), .O(new_n175_));
  nand2  g131(.a(new_n173_), .b(x12), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n146_), .O(new_n177_));
  nand2  g133(.a(x19), .b(x13), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n147_), .O(new_n179_));
  nor2   g135(.a(x19), .b(x14), .O(new_n180_));
  nor3   g136(.a(new_n155_), .b(new_n89_), .c(new_n81_), .O(new_n181_));
  nor3   g137(.a(new_n181_), .b(new_n180_), .c(new_n134_), .O(new_n182_));
  nand4  g138(.a(new_n182_), .b(new_n179_), .c(new_n177_), .d(new_n175_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n172_), .c(x32), .O(new_n184_));
  nand3  g140(.a(new_n184_), .b(new_n170_), .c(new_n165_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n80_), .c(x00), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n47_), .O(z8));
  inv1   g143(.a(x33), .O(new_n188_));
  nand4  g144(.a(new_n161_), .b(new_n188_), .c(x31), .d(x30), .O(new_n189_));
  nand2  g145(.a(x33), .b(new_n134_), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n189_), .O(new_n191_));
  nand3  g147(.a(new_n191_), .b(new_n143_), .c(x29), .O(new_n192_));
  nand4  g148(.a(new_n161_), .b(new_n188_), .c(new_n134_), .d(new_n114_), .O(new_n193_));
  oai21  g149(.a(new_n188_), .b(new_n114_), .c(new_n193_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n144_), .O(new_n195_));
  nor3   g151(.a(new_n181_), .b(new_n180_), .c(new_n138_), .O(new_n196_));
  nand4  g152(.a(new_n196_), .b(new_n179_), .c(new_n177_), .d(new_n175_), .O(new_n197_));
  oai21  g153(.a(new_n197_), .b(new_n172_), .c(x33), .O(new_n198_));
  nand3  g154(.a(new_n198_), .b(new_n195_), .c(new_n192_), .O(new_n199_));
  nand3  g155(.a(new_n199_), .b(new_n80_), .c(x00), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(new_n47_), .O(z9));
endmodule


