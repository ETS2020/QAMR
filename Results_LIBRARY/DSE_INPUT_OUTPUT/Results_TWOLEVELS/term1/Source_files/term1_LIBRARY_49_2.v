// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_;
  inv1   g000(.a(x26), .O(new_n45_));
  nor2   g001(.a(x29), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  nor2   g004(.a(x32), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nor2   g009(.a(x32), .b(new_n50_), .O(new_n54_));
  nor2   g010(.a(x33), .b(x03), .O(new_n55_));
  oai21  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n46_), .O(z1));
  inv1   g013(.a(x09), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n50_), .b(new_n53_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n50_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n53_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n53_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n50_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n59_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xnor2a g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n60_), .O(new_n72_));
  nand2  g028(.a(new_n66_), .b(new_n64_), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n47_), .c(new_n58_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x25), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  nor2   g037(.a(x20), .b(x15), .O(new_n82_));
  nor2   g038(.a(x21), .b(x16), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g040(.a(x17), .O(new_n85_));
  inv1   g041(.a(x22), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x18), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  inv1   g047(.a(x24), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n84_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n61_), .c(new_n81_), .d(new_n45_), .O(new_n95_));
  nor3   g051(.a(new_n95_), .b(new_n80_), .c(new_n60_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n61_), .d(new_n45_), .O(new_n98_));
  nor3   g054(.a(new_n98_), .b(new_n80_), .c(new_n60_), .O(z4));
  inv1   g055(.a(x29), .O(new_n100_));
  inv1   g056(.a(x28), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n81_), .O(new_n102_));
  nand3  g058(.a(new_n100_), .b(x28), .c(x27), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n94_), .c(new_n61_), .d(new_n45_), .O(new_n105_));
  nor3   g061(.a(new_n105_), .b(new_n80_), .c(new_n60_), .O(z5));
  nand3  g062(.a(x29), .b(x28), .c(x27), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x30), .O(new_n108_));
  inv1   g064(.a(x30), .O(new_n109_));
  nand3  g065(.a(new_n102_), .b(new_n109_), .c(x29), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n94_), .c(new_n61_), .d(new_n45_), .O(new_n112_));
  nor3   g068(.a(new_n112_), .b(new_n80_), .c(new_n60_), .O(z6));
  nand3  g069(.a(x30), .b(x28), .c(x27), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x31), .O(new_n115_));
  inv1   g071(.a(x31), .O(new_n116_));
  nand3  g072(.a(new_n102_), .b(new_n116_), .c(x30), .O(new_n117_));
  nand3  g073(.a(new_n117_), .b(new_n115_), .c(new_n45_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x29), .O(new_n119_));
  nand4  g075(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n84_), .O(new_n120_));
  aoi21  g076(.a(x31), .b(new_n100_), .c(new_n80_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n120_), .c(new_n61_), .d(x01), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n45_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n119_), .O(z7));
  inv1   g080(.a(x32), .O(new_n125_));
  nor2   g081(.a(x28), .b(x27), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n109_), .c(x29), .O(new_n128_));
  oai21  g084(.a(new_n126_), .b(new_n100_), .c(x30), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g086(.a(x19), .b(new_n88_), .c(x13), .O(new_n131_));
  nand3  g087(.a(new_n91_), .b(x18), .c(x14), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n131_), .c(new_n85_), .O(new_n133_));
  nand4  g089(.a(x19), .b(x18), .c(new_n85_), .d(x12), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  inv1   g092(.a(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(x11), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x15), .O(new_n142_));
  inv1   g098(.a(x15), .O(new_n143_));
  nand4  g099(.a(new_n139_), .b(x16), .c(new_n143_), .d(x10), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n130_), .c(new_n125_), .d(x31), .O(new_n146_));
  nor2   g102(.a(new_n109_), .b(new_n100_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x27), .O(new_n148_));
  oai21  g104(.a(x30), .b(x27), .c(new_n148_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n101_), .O(new_n150_));
  nand2  g106(.a(x16), .b(x10), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n138_), .c(new_n143_), .O(new_n152_));
  nor2   g108(.a(new_n91_), .b(new_n88_), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(x17), .c(x11), .O(new_n154_));
  nand2  g110(.a(new_n153_), .b(x12), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n85_), .O(new_n156_));
  inv1   g112(.a(x13), .O(new_n157_));
  oai21  g113(.a(new_n91_), .b(new_n157_), .c(new_n88_), .O(new_n158_));
  nor2   g114(.a(x19), .b(x14), .O(new_n159_));
  nor2   g115(.a(new_n137_), .b(new_n143_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n139_), .c(new_n159_), .O(new_n161_));
  oai21  g117(.a(x30), .b(x29), .c(x31), .O(new_n162_));
  aoi21  g118(.a(new_n147_), .b(x28), .c(new_n162_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n161_), .c(new_n158_), .d(new_n156_), .O(new_n164_));
  aoi21  g120(.a(new_n154_), .b(new_n137_), .c(new_n164_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n152_), .c(new_n150_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x32), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n146_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n45_), .c(x00), .O(new_n169_));
  inv1   g125(.a(new_n169_), .O(z8));
  inv1   g126(.a(x33), .O(new_n171_));
  nand4  g127(.a(new_n145_), .b(new_n171_), .c(x31), .d(x30), .O(new_n172_));
  nand2  g128(.a(x33), .b(new_n116_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n127_), .c(x29), .O(new_n175_));
  nand3  g131(.a(new_n81_), .b(new_n88_), .c(x13), .O(new_n176_));
  nand4  g132(.a(new_n171_), .b(new_n116_), .c(new_n109_), .d(new_n101_), .O(new_n177_));
  oai22  g133(.a(new_n177_), .b(new_n176_), .c(new_n171_), .d(new_n88_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x19), .O(new_n179_));
  nand3  g135(.a(new_n81_), .b(new_n91_), .c(x14), .O(new_n180_));
  oai22  g136(.a(new_n180_), .b(new_n177_), .c(new_n171_), .d(x14), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(x18), .O(new_n182_));
  aoi21  g138(.a(new_n182_), .b(new_n179_), .c(new_n85_), .O(new_n183_));
  nor3   g139(.a(x33), .b(x31), .c(x30), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n126_), .O(new_n185_));
  nor2   g141(.a(new_n185_), .b(new_n134_), .O(new_n186_));
  oai21  g142(.a(new_n186_), .b(new_n183_), .c(x16), .O(new_n187_));
  or2    g143(.a(new_n185_), .b(new_n140_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand2  g145(.a(x33), .b(x30), .O(new_n190_));
  nand4  g146(.a(x17), .b(x16), .c(new_n143_), .d(x10), .O(new_n191_));
  nand2  g147(.a(new_n184_), .b(new_n153_), .O(new_n192_));
  oai21  g148(.a(new_n192_), .b(new_n191_), .c(new_n190_), .O(new_n193_));
  nand3  g149(.a(new_n193_), .b(new_n101_), .c(new_n81_), .O(new_n194_));
  nand2  g150(.a(x18), .b(x17), .O(new_n195_));
  inv1   g151(.a(new_n195_), .O(new_n196_));
  aoi21  g152(.a(new_n196_), .b(new_n160_), .c(x19), .O(new_n197_));
  oai21  g153(.a(new_n195_), .b(new_n151_), .c(new_n143_), .O(new_n198_));
  inv1   g154(.a(x11), .O(new_n199_));
  oai21  g155(.a(new_n195_), .b(new_n199_), .c(new_n137_), .O(new_n200_));
  nand2  g156(.a(x18), .b(x12), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n85_), .O(new_n202_));
  aoi22  g158(.a(x31), .b(new_n109_), .c(new_n88_), .d(new_n157_), .O(new_n203_));
  nand4  g159(.a(new_n203_), .b(new_n202_), .c(new_n200_), .d(new_n198_), .O(new_n204_));
  oai21  g160(.a(new_n204_), .b(new_n197_), .c(x33), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n194_), .O(new_n206_));
  aoi21  g162(.a(new_n189_), .b(x15), .c(new_n206_), .O(new_n207_));
  aoi21  g163(.a(new_n207_), .b(new_n175_), .c(x26), .O(new_n208_));
  nand4  g164(.a(new_n145_), .b(new_n171_), .c(new_n116_), .d(new_n109_), .O(new_n209_));
  aoi21  g165(.a(new_n209_), .b(new_n190_), .c(x29), .O(new_n210_));
  oai21  g166(.a(new_n210_), .b(new_n208_), .c(x00), .O(new_n211_));
  nand2  g167(.a(new_n211_), .b(new_n47_), .O(z9));
endmodule


