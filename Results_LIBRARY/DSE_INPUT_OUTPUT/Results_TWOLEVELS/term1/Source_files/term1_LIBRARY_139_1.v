// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_, new_n220_;
  inv1   g000(.a(x24), .O(new_n45_));
  nand2  g001(.a(x30), .b(new_n45_), .O(new_n46_));
  inv1   g002(.a(new_n46_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nor2   g004(.a(x32), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nor2   g009(.a(x32), .b(new_n50_), .O(new_n54_));
  nor2   g010(.a(x33), .b(x03), .O(new_n55_));
  oai21  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n47_), .O(z1));
  inv1   g013(.a(x09), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n50_), .b(new_n53_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n50_), .b(new_n53_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n50_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n53_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n53_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n50_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n46_), .c(new_n58_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  inv1   g038(.a(x30), .O(new_n83_));
  nor2   g039(.a(x20), .b(x15), .O(new_n84_));
  nor2   g040(.a(x21), .b(x16), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  nor2   g042(.a(x22), .b(x17), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nor2   g044(.a(x23), .b(x18), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(new_n91_));
  oai21  g047(.a(new_n83_), .b(x24), .c(new_n91_), .O(new_n92_));
  inv1   g048(.a(x19), .O(new_n93_));
  nand3  g049(.a(new_n83_), .b(new_n45_), .c(new_n93_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(new_n92_), .c(new_n61_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n60_), .O(z3));
  nand2  g053(.a(x28), .b(new_n82_), .O(new_n98_));
  inv1   g054(.a(x28), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x27), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  aoi21  g057(.a(new_n45_), .b(new_n93_), .c(new_n89_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n88_), .c(new_n86_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n101_), .c(new_n62_), .d(new_n83_), .O(new_n104_));
  nand2  g060(.a(x30), .b(new_n99_), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n82_), .c(new_n98_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n91_), .c(new_n62_), .d(x24), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n104_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n81_), .c(x25), .d(x01), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(z4));
  inv1   g066(.a(x29), .O(new_n111_));
  nor2   g067(.a(new_n99_), .b(new_n82_), .O(new_n112_));
  nand3  g068(.a(new_n111_), .b(x28), .c(x27), .O(new_n113_));
  oai21  g069(.a(new_n112_), .b(new_n111_), .c(new_n113_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n103_), .c(new_n62_), .d(new_n81_), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x25), .c(x01), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n46_), .O(z5));
  inv1   g074(.a(new_n91_), .O(new_n119_));
  nand3  g075(.a(new_n83_), .b(x29), .c(x28), .O(new_n120_));
  oai21  g076(.a(new_n105_), .b(new_n45_), .c(new_n120_), .O(new_n121_));
  aoi21  g077(.a(x29), .b(x27), .c(new_n83_), .O(new_n122_));
  aoi22  g078(.a(new_n122_), .b(x24), .c(new_n121_), .d(x27), .O(new_n123_));
  nand3  g079(.a(x27), .b(new_n45_), .c(new_n93_), .O(new_n124_));
  oai22  g080(.a(new_n124_), .b(new_n120_), .c(new_n123_), .d(new_n119_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n126_));
  nor2   g082(.a(new_n126_), .b(new_n60_), .O(z6));
  nand4  g083(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n46_), .O(new_n129_));
  nand3  g085(.a(x29), .b(x28), .c(x27), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x31), .O(new_n131_));
  nand3  g087(.a(new_n90_), .b(new_n88_), .c(new_n86_), .O(new_n132_));
  inv1   g088(.a(x31), .O(new_n133_));
  nand4  g089(.a(new_n112_), .b(new_n133_), .c(x30), .d(x29), .O(new_n134_));
  nand3  g090(.a(new_n134_), .b(new_n132_), .c(new_n131_), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x24), .O(new_n136_));
  oai21  g092(.a(new_n132_), .b(new_n93_), .c(new_n133_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n83_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n136_), .c(new_n129_), .O(z7));
  nand2  g095(.a(x16), .b(x10), .O(new_n140_));
  nand3  g096(.a(x19), .b(x18), .c(x17), .O(new_n141_));
  nor2   g097(.a(new_n141_), .b(new_n140_), .O(new_n142_));
  nor2   g098(.a(new_n142_), .b(x15), .O(new_n143_));
  inv1   g099(.a(x16), .O(new_n144_));
  nand2  g100(.a(x19), .b(x18), .O(new_n145_));
  nand2  g101(.a(x17), .b(x11), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n145_), .c(new_n144_), .O(new_n147_));
  inv1   g103(.a(x12), .O(new_n148_));
  inv1   g104(.a(x17), .O(new_n149_));
  oai21  g105(.a(new_n145_), .b(new_n148_), .c(new_n149_), .O(new_n150_));
  inv1   g106(.a(x18), .O(new_n151_));
  nand2  g107(.a(x19), .b(x13), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n151_), .O(new_n153_));
  inv1   g109(.a(x14), .O(new_n154_));
  nand2  g110(.a(x16), .b(x15), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n141_), .c(x31), .O(new_n156_));
  aoi21  g112(.a(new_n93_), .b(new_n154_), .c(new_n156_), .O(new_n157_));
  nand4  g113(.a(new_n157_), .b(new_n153_), .c(new_n150_), .d(new_n147_), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n143_), .c(new_n46_), .O(new_n159_));
  nand2  g115(.a(new_n99_), .b(new_n82_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(x30), .O(new_n161_));
  nor2   g117(.a(new_n161_), .b(new_n111_), .O(new_n162_));
  oai21  g118(.a(x28), .b(x27), .c(x29), .O(new_n163_));
  aoi22  g119(.a(new_n163_), .b(new_n83_), .c(new_n162_), .d(x24), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n159_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(x32), .O(new_n166_));
  inv1   g122(.a(x32), .O(new_n167_));
  nand3  g123(.a(new_n163_), .b(x30), .c(x24), .O(new_n168_));
  nand3  g124(.a(new_n160_), .b(new_n83_), .c(x29), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand3  g126(.a(x19), .b(new_n151_), .c(x13), .O(new_n171_));
  nand3  g127(.a(new_n93_), .b(x18), .c(x14), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n171_), .c(new_n149_), .O(new_n173_));
  nand4  g129(.a(x19), .b(x18), .c(new_n149_), .d(x12), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n173_), .c(x16), .O(new_n176_));
  inv1   g132(.a(new_n141_), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n144_), .c(x11), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n176_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(x15), .O(new_n180_));
  inv1   g136(.a(x15), .O(new_n181_));
  nand4  g137(.a(new_n177_), .b(x16), .c(new_n181_), .d(x10), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n180_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n170_), .c(new_n167_), .d(x31), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n166_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n81_), .c(x00), .O(new_n186_));
  inv1   g142(.a(new_n186_), .O(z8));
  inv1   g143(.a(new_n155_), .O(new_n188_));
  nand2  g144(.a(x18), .b(x17), .O(new_n189_));
  inv1   g145(.a(new_n189_), .O(new_n190_));
  aoi21  g146(.a(new_n190_), .b(new_n188_), .c(x19), .O(new_n191_));
  oai21  g147(.a(new_n189_), .b(new_n140_), .c(new_n181_), .O(new_n192_));
  inv1   g148(.a(x11), .O(new_n193_));
  oai21  g149(.a(new_n189_), .b(new_n193_), .c(new_n144_), .O(new_n194_));
  oai21  g150(.a(new_n151_), .b(new_n148_), .c(new_n149_), .O(new_n195_));
  nor2   g151(.a(x18), .b(x13), .O(new_n196_));
  aoi21  g152(.a(new_n188_), .b(new_n177_), .c(new_n196_), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(new_n195_), .c(new_n194_), .d(new_n192_), .O(new_n198_));
  oai21  g154(.a(new_n198_), .b(new_n191_), .c(new_n46_), .O(new_n199_));
  nand2  g155(.a(x24), .b(new_n93_), .O(new_n200_));
  nand3  g156(.a(new_n83_), .b(x18), .c(x17), .O(new_n201_));
  oai21  g157(.a(new_n201_), .b(new_n155_), .c(new_n200_), .O(new_n202_));
  nand2  g158(.a(new_n202_), .b(new_n154_), .O(new_n203_));
  nand2  g159(.a(new_n163_), .b(new_n133_), .O(new_n204_));
  nand2  g160(.a(new_n204_), .b(new_n83_), .O(new_n205_));
  nand2  g161(.a(new_n133_), .b(x30), .O(new_n206_));
  nand2  g162(.a(new_n163_), .b(x31), .O(new_n207_));
  nand2  g163(.a(new_n207_), .b(new_n206_), .O(new_n208_));
  nand2  g164(.a(new_n208_), .b(x24), .O(new_n209_));
  nand3  g165(.a(new_n209_), .b(new_n205_), .c(new_n203_), .O(new_n210_));
  inv1   g166(.a(new_n210_), .O(new_n211_));
  nand2  g167(.a(new_n211_), .b(new_n199_), .O(new_n212_));
  nand2  g168(.a(new_n212_), .b(x33), .O(new_n213_));
  aoi21  g169(.a(new_n178_), .b(new_n176_), .c(new_n181_), .O(new_n214_));
  inv1   g170(.a(new_n182_), .O(new_n215_));
  nand4  g171(.a(new_n160_), .b(x31), .c(x30), .d(x29), .O(new_n216_));
  oai22  g172(.a(new_n216_), .b(new_n45_), .c(new_n204_), .d(x30), .O(new_n217_));
  oai21  g173(.a(new_n215_), .b(new_n214_), .c(new_n217_), .O(new_n218_));
  oai21  g174(.a(new_n218_), .b(x33), .c(new_n213_), .O(new_n219_));
  nand3  g175(.a(new_n219_), .b(new_n81_), .c(x00), .O(new_n220_));
  inv1   g176(.a(new_n220_), .O(z9));
endmodule


