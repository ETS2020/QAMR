// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n146_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_;
  nor2   g000(.a(x26), .b(x00), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nand2  g013(.a(new_n49_), .b(x01), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand3  g015(.a(x06), .b(x03), .c(new_n59_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n58_), .c(new_n45_), .O(new_n61_));
  inv1   g017(.a(x05), .O(new_n62_));
  nand2  g018(.a(x06), .b(new_n62_), .O(new_n63_));
  inv1   g019(.a(x06), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(x05), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x26), .c(new_n49_), .O(new_n67_));
  nand2  g023(.a(new_n59_), .b(x00), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n67_), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n61_), .c(new_n52_), .O(new_n70_));
  nand2  g026(.a(x03), .b(x01), .O(new_n71_));
  nand3  g027(.a(x05), .b(new_n49_), .c(new_n59_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(new_n45_), .O(new_n73_));
  nor3   g029(.a(new_n68_), .b(new_n65_), .c(x03), .O(new_n74_));
  aoi21  g030(.a(new_n73_), .b(x02), .c(new_n74_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n57_), .O(new_n77_));
  xor2a  g033(.a(x07), .b(x04), .O(new_n78_));
  xor2a  g034(.a(x03), .b(x02), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x01), .O(new_n80_));
  nor2   g036(.a(new_n49_), .b(new_n52_), .O(new_n81_));
  nand4  g037(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n82_));
  oai21  g038(.a(x06), .b(x05), .c(new_n82_), .O(new_n83_));
  oai21  g039(.a(new_n83_), .b(new_n81_), .c(new_n59_), .O(new_n84_));
  aoi21  g040(.a(new_n84_), .b(new_n80_), .c(new_n45_), .O(new_n85_));
  oai22  g041(.a(x06), .b(new_n49_), .c(x05), .d(new_n52_), .O(new_n86_));
  nand3  g042(.a(new_n86_), .b(new_n59_), .c(x00), .O(new_n87_));
  nand3  g043(.a(new_n62_), .b(new_n49_), .c(x02), .O(new_n88_));
  nand3  g044(.a(new_n64_), .b(x03), .c(new_n52_), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(x26), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n87_), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n85_), .c(new_n78_), .O(new_n93_));
  aoi21  g049(.a(new_n93_), .b(new_n77_), .c(x09), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x08), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z2));
  inv1   g052(.a(x27), .O(new_n97_));
  nor2   g053(.a(x20), .b(x15), .O(new_n98_));
  nor2   g054(.a(x21), .b(x16), .O(new_n99_));
  nor2   g055(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nor2   g056(.a(x22), .b(x17), .O(new_n101_));
  nor2   g057(.a(x23), .b(x18), .O(new_n102_));
  nor2   g058(.a(x24), .b(x19), .O(new_n103_));
  nor3   g059(.a(new_n103_), .b(new_n102_), .c(new_n101_), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n100_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n81_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n97_), .c(x25), .d(x01), .O(new_n108_));
  aoi21  g064(.a(new_n108_), .b(x00), .c(x26), .O(z3));
  inv1   g065(.a(x26), .O(new_n110_));
  xor2a  g066(.a(x28), .b(x27), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n107_), .c(new_n110_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(x25), .c(x01), .d(x00), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(z4));
  inv1   g071(.a(new_n81_), .O(new_n116_));
  inv1   g072(.a(x28), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n97_), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x29), .O(new_n120_));
  inv1   g076(.a(x29), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x28), .c(x27), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n120_), .c(new_n106_), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n116_), .c(x25), .d(x01), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(x00), .c(x26), .O(z5));
  nand3  g081(.a(x29), .b(x28), .c(x27), .O(new_n126_));
  nand2  g082(.a(new_n126_), .b(x30), .O(new_n127_));
  inv1   g083(.a(x30), .O(new_n128_));
  nand3  g084(.a(new_n118_), .b(new_n128_), .c(x29), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n127_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n105_), .c(new_n116_), .d(new_n110_), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x25), .c(x01), .d(x00), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(z6));
  nand3  g090(.a(new_n118_), .b(x30), .c(x29), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(x31), .O(new_n136_));
  inv1   g092(.a(new_n101_), .O(new_n137_));
  inv1   g093(.a(new_n102_), .O(new_n138_));
  inv1   g094(.a(new_n103_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n138_), .c(new_n137_), .d(new_n100_), .O(new_n140_));
  nand3  g096(.a(new_n116_), .b(x25), .c(x01), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  inv1   g098(.a(x31), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x30), .c(x29), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n119_), .c(new_n46_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(x26), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n142_), .c(new_n140_), .d(new_n136_), .O(z7));
  nand2  g103(.a(new_n117_), .b(new_n97_), .O(new_n148_));
  inv1   g104(.a(x32), .O(new_n149_));
  inv1   g105(.a(x17), .O(new_n150_));
  inv1   g106(.a(x18), .O(new_n151_));
  nand3  g107(.a(x19), .b(new_n151_), .c(x13), .O(new_n152_));
  inv1   g108(.a(x19), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(x18), .c(x14), .O(new_n154_));
  aoi21  g110(.a(new_n154_), .b(new_n152_), .c(new_n150_), .O(new_n155_));
  nand4  g111(.a(x19), .b(x18), .c(new_n150_), .d(x12), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n155_), .c(x16), .O(new_n158_));
  inv1   g114(.a(x16), .O(new_n159_));
  nand3  g115(.a(x19), .b(x18), .c(x17), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(new_n159_), .c(x11), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n158_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(x15), .O(new_n164_));
  inv1   g120(.a(x15), .O(new_n165_));
  nand4  g121(.a(new_n161_), .b(x16), .c(new_n165_), .d(x10), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(new_n164_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n149_), .c(x31), .d(new_n128_), .O(new_n168_));
  nand2  g124(.a(x32), .b(x30), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand3  g126(.a(new_n170_), .b(new_n148_), .c(x29), .O(new_n171_));
  nand2  g127(.a(new_n148_), .b(x29), .O(new_n172_));
  nand4  g128(.a(new_n167_), .b(new_n149_), .c(x31), .d(x30), .O(new_n173_));
  nand2  g129(.a(x32), .b(new_n128_), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  and2   g132(.a(x16), .b(x10), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n161_), .c(x15), .O(new_n178_));
  nor2   g134(.a(new_n153_), .b(new_n151_), .O(new_n179_));
  nand3  g135(.a(new_n179_), .b(x17), .c(x11), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n159_), .O(new_n181_));
  nand2  g137(.a(new_n179_), .b(x12), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n150_), .O(new_n183_));
  nand2  g139(.a(x19), .b(x13), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n151_), .O(new_n185_));
  nor2   g141(.a(x19), .b(x14), .O(new_n186_));
  nor3   g142(.a(new_n160_), .b(new_n159_), .c(new_n165_), .O(new_n187_));
  nor3   g143(.a(new_n187_), .b(new_n186_), .c(new_n143_), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(new_n185_), .c(new_n183_), .d(new_n181_), .O(new_n189_));
  oai21  g145(.a(new_n189_), .b(new_n178_), .c(x32), .O(new_n190_));
  nand3  g146(.a(new_n190_), .b(new_n176_), .c(new_n171_), .O(new_n191_));
  nand3  g147(.a(new_n191_), .b(new_n110_), .c(x00), .O(new_n192_));
  inv1   g148(.a(new_n192_), .O(z8));
  inv1   g149(.a(x33), .O(new_n194_));
  nand4  g150(.a(new_n167_), .b(new_n194_), .c(x31), .d(x30), .O(new_n195_));
  nand2  g151(.a(x33), .b(new_n143_), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand3  g153(.a(new_n197_), .b(new_n148_), .c(x29), .O(new_n198_));
  nand4  g154(.a(new_n167_), .b(new_n194_), .c(new_n143_), .d(new_n128_), .O(new_n199_));
  nand2  g155(.a(x33), .b(x31), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(new_n199_), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n172_), .O(new_n202_));
  xor2a  g158(.a(x31), .b(x30), .O(new_n203_));
  nor3   g159(.a(new_n203_), .b(new_n187_), .c(new_n186_), .O(new_n204_));
  nand4  g160(.a(new_n204_), .b(new_n185_), .c(new_n183_), .d(new_n181_), .O(new_n205_));
  oai21  g161(.a(new_n205_), .b(new_n178_), .c(x33), .O(new_n206_));
  nand3  g162(.a(new_n206_), .b(new_n202_), .c(new_n198_), .O(new_n207_));
  nand3  g163(.a(new_n207_), .b(new_n110_), .c(x00), .O(new_n208_));
  inv1   g164(.a(new_n208_), .O(z9));
endmodule


