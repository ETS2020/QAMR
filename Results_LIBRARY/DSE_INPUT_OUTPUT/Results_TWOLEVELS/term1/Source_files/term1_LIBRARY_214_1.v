// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x02), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  inv1   g008(.a(x32), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(x03), .O(new_n54_));
  oai21  g010(.a(x33), .b(x03), .c(new_n54_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n45_), .c(new_n52_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n51_), .O(z1));
  inv1   g013(.a(new_n46_), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nand2  g016(.a(x03), .b(x02), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n49_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n59_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  nand2  g026(.a(x03), .b(new_n52_), .O(new_n71_));
  nand2  g027(.a(new_n49_), .b(x02), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n64_), .b(x02), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n75_));
  inv1   g031(.a(x05), .O(new_n76_));
  inv1   g032(.a(x06), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n75_), .c(new_n74_), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n60_), .O(new_n80_));
  nand3  g036(.a(new_n77_), .b(x03), .c(new_n52_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(new_n82_));
  oai21  g038(.a(new_n82_), .b(new_n73_), .c(new_n70_), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n69_), .c(x09), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(x08), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n58_), .O(z2));
  inv1   g042(.a(x26), .O(new_n87_));
  inv1   g043(.a(x27), .O(new_n88_));
  oai22  g044(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nor2   g046(.a(x22), .b(x17), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nor2   g048(.a(x23), .b(x18), .O(new_n93_));
  nor2   g049(.a(x24), .b(x19), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n92_), .c(new_n90_), .O(new_n96_));
  and2   g052(.a(new_n96_), .b(new_n61_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n88_), .c(new_n87_), .d(x25), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n60_), .c(new_n58_), .O(z3));
  xor2a  g055(.a(x28), .b(x27), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n97_), .c(new_n87_), .d(x25), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n60_), .c(new_n58_), .O(z4));
  inv1   g058(.a(x25), .O(new_n103_));
  oai21  g059(.a(x30), .b(x02), .c(new_n72_), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  inv1   g061(.a(x28), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n88_), .O(new_n107_));
  nand3  g063(.a(new_n105_), .b(x28), .c(x27), .O(new_n108_));
  oai21  g064(.a(new_n107_), .b(new_n105_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n104_), .c(new_n96_), .d(new_n87_), .O(new_n110_));
  nor3   g066(.a(new_n110_), .b(new_n103_), .c(new_n60_), .O(z5));
  oai21  g067(.a(x30), .b(x03), .c(x02), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(x29), .c(x28), .d(x27), .O(new_n113_));
  nand2  g069(.a(new_n106_), .b(x02), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x29), .c(x27), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x30), .c(new_n49_), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n113_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n96_), .c(new_n87_), .d(x25), .O(new_n118_));
  oai21  g074(.a(new_n118_), .b(new_n60_), .c(new_n58_), .O(z6));
  nor4   g075(.a(new_n94_), .b(new_n93_), .c(new_n91_), .d(new_n89_), .O(new_n120_));
  nand3  g076(.a(new_n87_), .b(x25), .c(x01), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n120_), .c(new_n58_), .O(new_n122_));
  nand3  g078(.a(x29), .b(x28), .c(x27), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(x31), .O(new_n124_));
  inv1   g080(.a(x31), .O(new_n125_));
  nand4  g081(.a(new_n107_), .b(new_n125_), .c(x30), .d(x29), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n124_), .c(new_n49_), .O(new_n127_));
  aoi22  g083(.a(new_n127_), .b(x02), .c(x31), .d(new_n45_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n122_), .O(z7));
  nand2  g085(.a(new_n106_), .b(new_n88_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n45_), .c(x29), .O(new_n131_));
  nand2  g087(.a(new_n130_), .b(x29), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(x30), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  inv1   g090(.a(x17), .O(new_n135_));
  inv1   g091(.a(x18), .O(new_n136_));
  nand3  g092(.a(x19), .b(new_n136_), .c(x13), .O(new_n137_));
  inv1   g093(.a(x19), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x18), .c(x14), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n137_), .c(new_n135_), .O(new_n140_));
  nand4  g096(.a(x19), .b(x18), .c(new_n135_), .d(x12), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  inv1   g099(.a(x16), .O(new_n144_));
  nand3  g100(.a(x19), .b(x18), .c(x17), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n144_), .c(x11), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x15), .O(new_n149_));
  inv1   g105(.a(x15), .O(new_n150_));
  nand3  g106(.a(x16), .b(new_n150_), .c(x10), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n145_), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n149_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n134_), .c(new_n53_), .d(x31), .O(new_n155_));
  oai21  g111(.a(new_n88_), .b(new_n52_), .c(new_n106_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x30), .O(new_n157_));
  nor2   g113(.a(new_n157_), .b(new_n105_), .O(new_n158_));
  nand2  g114(.a(new_n132_), .b(new_n45_), .O(new_n159_));
  nand2  g115(.a(x16), .b(x10), .O(new_n160_));
  oai21  g116(.a(new_n160_), .b(new_n145_), .c(new_n150_), .O(new_n161_));
  nand2  g117(.a(x19), .b(x18), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  and2   g119(.a(x17), .b(x11), .O(new_n164_));
  aoi21  g120(.a(new_n164_), .b(new_n163_), .c(x16), .O(new_n165_));
  inv1   g121(.a(x12), .O(new_n166_));
  oai21  g122(.a(new_n162_), .b(new_n166_), .c(new_n135_), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  nand2  g124(.a(x19), .b(x13), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n136_), .O(new_n170_));
  inv1   g126(.a(x14), .O(new_n171_));
  nand2  g127(.a(new_n138_), .b(new_n171_), .O(new_n172_));
  nand3  g128(.a(new_n146_), .b(x16), .c(x15), .O(new_n173_));
  nand4  g129(.a(new_n173_), .b(new_n172_), .c(new_n170_), .d(x31), .O(new_n174_));
  nor3   g130(.a(new_n174_), .b(new_n168_), .c(new_n165_), .O(new_n175_));
  nand3  g131(.a(new_n175_), .b(new_n161_), .c(new_n159_), .O(new_n176_));
  oai21  g132(.a(new_n176_), .b(new_n158_), .c(x32), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n155_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n87_), .c(x00), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n58_), .O(z8));
  aoi21  g136(.a(new_n147_), .b(new_n143_), .c(new_n150_), .O(new_n181_));
  nand4  g137(.a(new_n156_), .b(x31), .c(x30), .d(x29), .O(new_n182_));
  nand3  g138(.a(new_n132_), .b(new_n125_), .c(new_n45_), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n182_), .O(new_n184_));
  oai21  g140(.a(new_n152_), .b(new_n181_), .c(new_n184_), .O(new_n185_));
  nand2  g141(.a(new_n159_), .b(new_n125_), .O(new_n186_));
  nand3  g142(.a(new_n130_), .b(x30), .c(x29), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(x31), .O(new_n188_));
  nand4  g144(.a(new_n173_), .b(new_n172_), .c(new_n170_), .d(new_n167_), .O(new_n189_));
  nor2   g145(.a(new_n189_), .b(new_n165_), .O(new_n190_));
  nand4  g146(.a(new_n190_), .b(new_n188_), .c(new_n186_), .d(new_n161_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(x33), .O(new_n192_));
  oai21  g148(.a(new_n185_), .b(x33), .c(new_n192_), .O(new_n193_));
  nand3  g149(.a(new_n193_), .b(new_n87_), .c(x00), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n58_), .O(z9));
endmodule


