// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_;
  inv1   g000(.a(x30), .O(new_n45_));
  nor2   g001(.a(x32), .b(new_n45_), .O(z0));
  inv1   g002(.a(x03), .O(new_n47_));
  nand2  g003(.a(z0), .b(new_n47_), .O(new_n48_));
  inv1   g004(.a(x33), .O(new_n49_));
  nand3  g005(.a(new_n49_), .b(x32), .c(x03), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  nand2  g007(.a(new_n51_), .b(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nand2  g009(.a(z0), .b(x03), .O(new_n54_));
  nand3  g010(.a(new_n49_), .b(x32), .c(new_n47_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  inv1   g013(.a(x32), .O(new_n58_));
  nand3  g014(.a(new_n49_), .b(new_n58_), .c(x30), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n57_), .c(new_n52_), .O(z1));
  inv1   g016(.a(x09), .O(new_n61_));
  nand2  g017(.a(new_n58_), .b(new_n45_), .O(new_n62_));
  xnor2a g018(.a(x07), .b(x04), .O(new_n63_));
  inv1   g019(.a(x01), .O(new_n64_));
  nor2   g020(.a(new_n47_), .b(new_n53_), .O(new_n65_));
  inv1   g021(.a(new_n65_), .O(new_n66_));
  nand2  g022(.a(new_n47_), .b(new_n53_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  nand2  g024(.a(x05), .b(new_n47_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(x06), .c(new_n53_), .O(new_n70_));
  nand2  g026(.a(x06), .b(new_n53_), .O(new_n71_));
  nand3  g027(.a(new_n71_), .b(x05), .c(new_n47_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n70_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n68_), .c(new_n63_), .O(new_n74_));
  xor2a  g030(.a(x07), .b(x04), .O(new_n75_));
  xnor2a g031(.a(x03), .b(x02), .O(new_n76_));
  nor2   g032(.a(new_n76_), .b(new_n64_), .O(new_n77_));
  nand2  g033(.a(new_n71_), .b(new_n69_), .O(new_n78_));
  nand4  g034(.a(x06), .b(x05), .c(new_n47_), .d(new_n53_), .O(new_n79_));
  aoi21  g035(.a(new_n79_), .b(new_n78_), .c(x01), .O(new_n80_));
  oai21  g036(.a(new_n80_), .b(new_n77_), .c(new_n75_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n74_), .O(new_n82_));
  nand4  g038(.a(new_n82_), .b(new_n62_), .c(new_n61_), .d(x08), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(z2));
  inv1   g040(.a(x26), .O(new_n85_));
  inv1   g041(.a(x27), .O(new_n86_));
  oai22  g042(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n87_));
  inv1   g043(.a(x17), .O(new_n88_));
  inv1   g044(.a(x22), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x18), .O(new_n91_));
  inv1   g047(.a(x23), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  inv1   g049(.a(x19), .O(new_n94_));
  inv1   g050(.a(x24), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n94_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n93_), .c(new_n90_), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n87_), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n65_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(new_n62_), .c(new_n86_), .O(new_n100_));
  inv1   g056(.a(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n85_), .c(x25), .d(x01), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(z3));
  xor2a  g059(.a(x28), .b(x27), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n99_), .c(new_n85_), .d(x25), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n64_), .c(new_n62_), .O(z4));
  inv1   g062(.a(new_n98_), .O(new_n107_));
  inv1   g063(.a(x29), .O(new_n108_));
  inv1   g064(.a(x28), .O(new_n109_));
  nor2   g065(.a(new_n109_), .b(new_n86_), .O(new_n110_));
  nand3  g066(.a(new_n108_), .b(x28), .c(x27), .O(new_n111_));
  oai21  g067(.a(new_n110_), .b(new_n108_), .c(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n107_), .c(new_n66_), .d(new_n85_), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x25), .c(x01), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n62_), .O(z5));
  inv1   g072(.a(x25), .O(new_n117_));
  inv1   g073(.a(new_n110_), .O(new_n118_));
  nand3  g074(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x30), .O(new_n120_));
  nand3  g076(.a(x32), .b(new_n45_), .c(x29), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n118_), .c(new_n120_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n107_), .c(new_n66_), .d(new_n85_), .O(new_n123_));
  nor3   g079(.a(new_n123_), .b(new_n117_), .c(new_n64_), .O(z6));
  oai21  g080(.a(new_n58_), .b(x31), .c(new_n45_), .O(new_n125_));
  nand2  g081(.a(new_n119_), .b(x31), .O(new_n126_));
  inv1   g082(.a(new_n87_), .O(new_n127_));
  nand4  g083(.a(new_n96_), .b(new_n93_), .c(new_n90_), .d(new_n127_), .O(new_n128_));
  nor2   g084(.a(new_n65_), .b(new_n64_), .O(new_n129_));
  inv1   g085(.a(x31), .O(new_n130_));
  nand4  g086(.a(new_n110_), .b(new_n130_), .c(x30), .d(x29), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n129_), .c(new_n85_), .d(x25), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(new_n128_), .c(new_n126_), .d(new_n125_), .O(z7));
  inv1   g090(.a(x00), .O(new_n135_));
  nand2  g091(.a(x32), .b(new_n45_), .O(new_n136_));
  nand2  g092(.a(new_n109_), .b(new_n86_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(x29), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(x19), .b(new_n91_), .c(x13), .O(new_n140_));
  nand3  g096(.a(new_n94_), .b(x18), .c(x14), .O(new_n141_));
  aoi21  g097(.a(new_n141_), .b(new_n140_), .c(new_n88_), .O(new_n142_));
  nand4  g098(.a(x19), .b(x18), .c(new_n88_), .d(x12), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  oai21  g100(.a(new_n144_), .b(new_n142_), .c(x16), .O(new_n145_));
  inv1   g101(.a(x16), .O(new_n146_));
  nand3  g102(.a(x19), .b(x18), .c(x17), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(new_n146_), .c(x11), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x15), .O(new_n151_));
  inv1   g107(.a(x15), .O(new_n152_));
  nand4  g108(.a(new_n148_), .b(x16), .c(new_n152_), .d(x10), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n151_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n58_), .c(x31), .d(x30), .O(new_n155_));
  aoi21  g111(.a(new_n155_), .b(new_n136_), .c(new_n139_), .O(new_n156_));
  nand2  g112(.a(new_n109_), .b(new_n86_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(x30), .c(x29), .O(new_n158_));
  nand2  g114(.a(x16), .b(x10), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  aoi21  g116(.a(new_n160_), .b(new_n148_), .c(x15), .O(new_n161_));
  nand2  g117(.a(x19), .b(x18), .O(new_n162_));
  nand2  g118(.a(x17), .b(x11), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n162_), .c(new_n146_), .O(new_n164_));
  inv1   g120(.a(x12), .O(new_n165_));
  oai21  g121(.a(new_n162_), .b(new_n165_), .c(new_n88_), .O(new_n166_));
  inv1   g122(.a(x13), .O(new_n167_));
  oai21  g123(.a(new_n94_), .b(new_n167_), .c(new_n91_), .O(new_n168_));
  nor2   g124(.a(x19), .b(x14), .O(new_n169_));
  nand2  g125(.a(x16), .b(x15), .O(new_n170_));
  nor2   g126(.a(new_n170_), .b(new_n147_), .O(new_n171_));
  nor3   g127(.a(new_n171_), .b(new_n169_), .c(new_n130_), .O(new_n172_));
  nand4  g128(.a(new_n172_), .b(new_n168_), .c(new_n166_), .d(new_n164_), .O(new_n173_));
  nor2   g129(.a(new_n173_), .b(new_n161_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n158_), .c(new_n58_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n156_), .c(new_n85_), .O(new_n176_));
  nor2   g132(.a(new_n176_), .b(new_n135_), .O(z8));
  inv1   g133(.a(new_n170_), .O(new_n178_));
  nand2  g134(.a(x18), .b(x17), .O(new_n179_));
  inv1   g135(.a(new_n179_), .O(new_n180_));
  aoi21  g136(.a(new_n180_), .b(new_n178_), .c(x19), .O(new_n181_));
  oai21  g137(.a(new_n179_), .b(new_n159_), .c(new_n152_), .O(new_n182_));
  inv1   g138(.a(x11), .O(new_n183_));
  oai21  g139(.a(new_n179_), .b(new_n183_), .c(new_n146_), .O(new_n184_));
  oai21  g140(.a(new_n91_), .b(new_n165_), .c(new_n88_), .O(new_n185_));
  aoi21  g141(.a(new_n91_), .b(new_n167_), .c(new_n171_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n185_), .c(new_n184_), .d(new_n182_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n181_), .c(new_n62_), .O(new_n188_));
  inv1   g144(.a(x14), .O(new_n189_));
  nand3  g145(.a(x30), .b(x18), .c(x17), .O(new_n190_));
  oai22  g146(.a(new_n190_), .b(new_n170_), .c(new_n58_), .d(x19), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n189_), .O(new_n192_));
  nand3  g148(.a(new_n137_), .b(x32), .c(x29), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(new_n45_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n130_), .O(new_n195_));
  nor3   g151(.a(new_n58_), .b(new_n130_), .c(x30), .O(new_n196_));
  aoi21  g152(.a(new_n138_), .b(x30), .c(new_n196_), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(new_n195_), .c(new_n192_), .d(new_n188_), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(x33), .O(new_n199_));
  nand4  g155(.a(new_n137_), .b(x31), .c(x30), .d(x29), .O(new_n200_));
  nand4  g156(.a(new_n138_), .b(x32), .c(new_n130_), .d(new_n45_), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n200_), .O(new_n202_));
  nand3  g158(.a(new_n202_), .b(new_n154_), .c(new_n49_), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(new_n199_), .O(new_n204_));
  nand3  g160(.a(new_n204_), .b(new_n85_), .c(x00), .O(new_n205_));
  inv1   g161(.a(new_n205_), .O(z9));
endmodule


