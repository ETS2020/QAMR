// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:24 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_;
  inv1   g000(.a(x27), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand2  g017(.a(x03), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n61_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n48_), .c(new_n59_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x25), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x17), .O(new_n86_));
  inv1   g042(.a(x22), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x18), .O(new_n89_));
  inv1   g045(.a(x23), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  inv1   g047(.a(x19), .O(new_n92_));
  inv1   g048(.a(x24), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n91_), .c(new_n88_), .d(new_n85_), .O(new_n95_));
  and2   g051(.a(new_n95_), .b(new_n62_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n45_), .c(new_n82_), .O(new_n97_));
  nor3   g053(.a(new_n97_), .b(new_n81_), .c(new_n61_), .O(z3));
  inv1   g054(.a(x28), .O(new_n99_));
  nand3  g055(.a(new_n46_), .b(new_n99_), .c(x27), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(x27), .c(new_n100_), .O(new_n101_));
  nand3  g057(.a(new_n101_), .b(new_n96_), .c(new_n82_), .O(new_n102_));
  nor3   g058(.a(new_n102_), .b(new_n81_), .c(new_n61_), .O(z4));
  oai21  g059(.a(new_n99_), .b(new_n45_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n95_), .c(new_n62_), .d(new_n82_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x25), .c(x01), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n48_), .O(z5));
  nand2  g067(.a(x29), .b(x28), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n45_), .c(new_n46_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n96_), .c(new_n82_), .d(x25), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n61_), .c(new_n48_), .O(z6));
  nand4  g071(.a(new_n94_), .b(new_n91_), .c(new_n88_), .d(new_n85_), .O(new_n116_));
  nand2  g072(.a(new_n62_), .b(x01), .O(new_n117_));
  nor4   g073(.a(new_n117_), .b(x31), .c(x26), .d(new_n81_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n116_), .c(new_n47_), .O(z7));
  inv1   g075(.a(x10), .O(new_n120_));
  inv1   g076(.a(x16), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  nand3  g078(.a(x19), .b(x18), .c(x17), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(x15), .O(new_n125_));
  nand2  g081(.a(x19), .b(x18), .O(new_n126_));
  nand2  g082(.a(x17), .b(x11), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n126_), .c(new_n121_), .O(new_n128_));
  inv1   g084(.a(x12), .O(new_n129_));
  oai21  g085(.a(new_n126_), .b(new_n129_), .c(new_n86_), .O(new_n130_));
  aoi21  g086(.a(x19), .b(x13), .c(x18), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nor2   g088(.a(x19), .b(x14), .O(new_n133_));
  nand2  g089(.a(x16), .b(x15), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n123_), .c(x31), .O(new_n135_));
  nor2   g091(.a(new_n135_), .b(new_n133_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n132_), .c(new_n130_), .d(new_n128_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n125_), .c(new_n48_), .O(new_n138_));
  nand3  g094(.a(x30), .b(x29), .c(x28), .O(new_n139_));
  oai21  g095(.a(x30), .b(x28), .c(new_n139_), .O(new_n140_));
  aoi22  g096(.a(new_n140_), .b(new_n45_), .c(new_n46_), .d(new_n105_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n138_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(x32), .O(new_n143_));
  inv1   g099(.a(x32), .O(new_n144_));
  nand2  g100(.a(new_n112_), .b(x30), .O(new_n145_));
  nand2  g101(.a(new_n99_), .b(new_n45_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n46_), .c(x29), .O(new_n147_));
  oai21  g103(.a(new_n145_), .b(x27), .c(new_n147_), .O(new_n148_));
  nand3  g104(.a(x19), .b(new_n89_), .c(x13), .O(new_n149_));
  nand3  g105(.a(new_n92_), .b(x18), .c(x14), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(new_n86_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n86_), .d(x12), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  nand3  g110(.a(new_n124_), .b(new_n121_), .c(x11), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n154_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x15), .O(new_n157_));
  nor4   g113(.a(new_n123_), .b(new_n121_), .c(x15), .d(new_n120_), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n148_), .c(new_n144_), .d(x31), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n143_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n82_), .c(x00), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(z8));
  nand4  g120(.a(x33), .b(x29), .c(new_n82_), .d(x00), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n46_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x27), .O(new_n167_));
  inv1   g123(.a(x15), .O(new_n168_));
  aoi21  g124(.a(new_n155_), .b(new_n154_), .c(new_n168_), .O(new_n169_));
  inv1   g125(.a(x31), .O(new_n170_));
  nand2  g126(.a(new_n146_), .b(x29), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n170_), .c(new_n46_), .O(new_n172_));
  nand2  g128(.a(x31), .b(x30), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n112_), .c(new_n172_), .O(new_n174_));
  oai21  g130(.a(new_n158_), .b(new_n169_), .c(new_n174_), .O(new_n175_));
  inv1   g131(.a(new_n145_), .O(new_n176_));
  inv1   g132(.a(new_n125_), .O(new_n177_));
  oai22  g133(.a(new_n134_), .b(new_n123_), .c(x19), .d(x14), .O(new_n178_));
  nand2  g134(.a(x31), .b(new_n46_), .O(new_n179_));
  nand3  g135(.a(new_n170_), .b(x29), .c(x28), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n179_), .O(new_n181_));
  nor3   g137(.a(new_n181_), .b(new_n178_), .c(new_n131_), .O(new_n182_));
  nand4  g138(.a(new_n182_), .b(new_n130_), .c(new_n128_), .d(new_n177_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n176_), .c(x33), .O(new_n184_));
  oai21  g140(.a(new_n175_), .b(x33), .c(new_n184_), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n82_), .c(x00), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n167_), .O(z9));
endmodule


