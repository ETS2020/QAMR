// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_, new_n219_;
  inv1   g000(.a(x16), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n49_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  inv1   g042(.a(new_n86_), .O(new_n87_));
  nor2   g043(.a(x23), .b(x18), .O(new_n88_));
  inv1   g044(.a(new_n88_), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n87_), .c(new_n85_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n62_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n60_), .c(new_n49_), .O(z3));
  nand2  g053(.a(x28), .b(new_n82_), .O(new_n98_));
  inv1   g054(.a(x28), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x27), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nor2   g057(.a(new_n86_), .b(new_n83_), .O(new_n102_));
  nand2  g058(.a(new_n102_), .b(new_n93_), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n46_), .O(new_n104_));
  oai21  g060(.a(x21), .b(x16), .c(new_n104_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n101_), .c(new_n62_), .O(new_n106_));
  nand3  g062(.a(x30), .b(new_n99_), .c(x27), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n98_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n103_), .c(new_n62_), .d(new_n45_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n106_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n81_), .c(x25), .d(x01), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z4));
  inv1   g068(.a(x29), .O(new_n113_));
  nor2   g069(.a(new_n99_), .b(new_n82_), .O(new_n114_));
  nand3  g070(.a(new_n113_), .b(x28), .c(x27), .O(new_n115_));
  oai21  g071(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n94_), .c(new_n62_), .d(new_n81_), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x25), .c(x01), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n49_), .O(z5));
  nand3  g076(.a(x30), .b(new_n99_), .c(new_n45_), .O(new_n121_));
  nand3  g077(.a(new_n46_), .b(x29), .c(x28), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n121_), .c(new_n82_), .O(new_n123_));
  aoi21  g079(.a(x29), .b(x27), .c(new_n46_), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n123_), .c(new_n103_), .O(new_n125_));
  nand3  g081(.a(x29), .b(x28), .c(x27), .O(new_n126_));
  nor3   g082(.a(new_n122_), .b(new_n82_), .c(x16), .O(new_n127_));
  aoi21  g083(.a(new_n126_), .b(x30), .c(new_n127_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(x21), .c(new_n125_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n60_), .c(new_n49_), .O(z6));
  inv1   g087(.a(x31), .O(new_n132_));
  nand3  g088(.a(new_n114_), .b(new_n132_), .c(x29), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(new_n45_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x30), .O(new_n135_));
  nand3  g091(.a(new_n114_), .b(x30), .c(x29), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x31), .O(new_n137_));
  nand4  g093(.a(new_n91_), .b(new_n89_), .c(new_n87_), .d(new_n85_), .O(new_n138_));
  nand4  g094(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  nand4  g096(.a(new_n140_), .b(new_n138_), .c(new_n137_), .d(new_n135_), .O(z7));
  inv1   g097(.a(x00), .O(new_n142_));
  inv1   g098(.a(x32), .O(new_n143_));
  nand2  g099(.a(new_n99_), .b(new_n82_), .O(new_n144_));
  inv1   g100(.a(x17), .O(new_n145_));
  inv1   g101(.a(x18), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n146_), .c(x13), .O(new_n147_));
  inv1   g103(.a(x19), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n145_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n145_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  nor2   g109(.a(new_n145_), .b(x16), .O(new_n154_));
  nor2   g110(.a(x30), .b(new_n148_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n154_), .c(x18), .d(x11), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(new_n153_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x15), .O(new_n158_));
  nor2   g114(.a(new_n148_), .b(new_n146_), .O(new_n159_));
  nor2   g115(.a(new_n45_), .b(x15), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n159_), .c(x17), .d(x10), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n158_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n144_), .c(new_n143_), .d(x31), .O(new_n163_));
  nand2  g119(.a(new_n99_), .b(new_n82_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(x32), .c(x30), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n163_), .c(new_n113_), .O(new_n166_));
  nand2  g122(.a(new_n144_), .b(x29), .O(new_n167_));
  nor2   g123(.a(new_n146_), .b(new_n145_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(x15), .c(x11), .O(new_n169_));
  nand4  g125(.a(new_n143_), .b(x31), .c(x30), .d(x19), .O(new_n170_));
  oai22  g126(.a(new_n170_), .b(new_n169_), .c(new_n143_), .d(x30), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n167_), .O(new_n172_));
  aoi21  g128(.a(x15), .b(x12), .c(x17), .O(new_n173_));
  inv1   g129(.a(x13), .O(new_n174_));
  inv1   g130(.a(x15), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n174_), .c(new_n146_), .O(new_n176_));
  nand2  g132(.a(x19), .b(x10), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  inv1   g134(.a(x14), .O(new_n179_));
  nand2  g135(.a(new_n148_), .b(new_n179_), .O(new_n180_));
  nand3  g136(.a(new_n159_), .b(x17), .c(x15), .O(new_n181_));
  nand4  g137(.a(new_n181_), .b(new_n180_), .c(new_n178_), .d(new_n176_), .O(new_n182_));
  oai21  g138(.a(new_n182_), .b(new_n173_), .c(x16), .O(new_n183_));
  nand2  g139(.a(new_n159_), .b(x16), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n145_), .O(new_n185_));
  oai21  g141(.a(new_n148_), .b(new_n45_), .c(new_n146_), .O(new_n186_));
  nand3  g142(.a(x19), .b(x15), .c(x11), .O(new_n187_));
  aoi21  g143(.a(new_n187_), .b(new_n45_), .c(new_n132_), .O(new_n188_));
  nand4  g144(.a(new_n188_), .b(new_n186_), .c(new_n185_), .d(new_n183_), .O(new_n189_));
  nand2  g145(.a(new_n189_), .b(x32), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n172_), .O(new_n191_));
  oai21  g147(.a(new_n191_), .b(new_n166_), .c(new_n81_), .O(new_n192_));
  oai21  g148(.a(new_n192_), .b(new_n142_), .c(new_n49_), .O(z8));
  inv1   g149(.a(x33), .O(new_n194_));
  nand3  g150(.a(new_n162_), .b(new_n194_), .c(new_n132_), .O(new_n195_));
  nand2  g151(.a(x33), .b(x31), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(new_n167_), .O(new_n198_));
  nand4  g154(.a(new_n194_), .b(x31), .c(x30), .d(x19), .O(new_n199_));
  oai22  g155(.a(new_n199_), .b(new_n169_), .c(new_n194_), .d(x31), .O(new_n200_));
  nand3  g156(.a(new_n200_), .b(new_n144_), .c(x29), .O(new_n201_));
  inv1   g157(.a(new_n201_), .O(new_n202_));
  aoi21  g158(.a(new_n148_), .b(x14), .c(new_n146_), .O(new_n203_));
  nand4  g159(.a(new_n203_), .b(x17), .c(x16), .d(x15), .O(new_n204_));
  nand3  g160(.a(new_n168_), .b(x16), .c(x15), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n148_), .O(new_n206_));
  nand3  g162(.a(new_n168_), .b(x16), .c(x10), .O(new_n207_));
  nand2  g163(.a(new_n168_), .b(x11), .O(new_n208_));
  nand2  g164(.a(new_n208_), .b(new_n45_), .O(new_n209_));
  nand2  g165(.a(x18), .b(x12), .O(new_n210_));
  nand2  g166(.a(new_n210_), .b(new_n145_), .O(new_n211_));
  nand2  g167(.a(new_n146_), .b(new_n174_), .O(new_n212_));
  xnor2a g168(.a(x31), .b(x30), .O(new_n213_));
  nand4  g169(.a(new_n213_), .b(new_n212_), .c(new_n211_), .d(new_n209_), .O(new_n214_));
  aoi21  g170(.a(new_n207_), .b(new_n175_), .c(new_n214_), .O(new_n215_));
  nand3  g171(.a(new_n215_), .b(new_n206_), .c(new_n204_), .O(new_n216_));
  aoi21  g172(.a(new_n216_), .b(x33), .c(new_n202_), .O(new_n217_));
  nand2  g173(.a(new_n217_), .b(new_n198_), .O(new_n218_));
  nand3  g174(.a(new_n218_), .b(new_n81_), .c(x00), .O(new_n219_));
  nand2  g175(.a(new_n219_), .b(new_n49_), .O(z9));
endmodule


