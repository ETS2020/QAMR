// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nor2   g004(.a(x32), .b(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  nor2   g009(.a(x32), .b(new_n50_), .O(new_n54_));
  nor2   g010(.a(x33), .b(x03), .O(new_n55_));
  oai21  g011(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  aoi21  g012(.a(new_n56_), .b(new_n52_), .c(new_n47_), .O(z1));
  inv1   g013(.a(new_n47_), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n50_), .b(new_n53_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n50_), .b(new_n53_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n50_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n53_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n53_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n50_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n58_), .O(z2));
  inv1   g036(.a(x27), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  inv1   g038(.a(x17), .O(new_n83_));
  inv1   g039(.a(x22), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x18), .O(new_n86_));
  inv1   g042(.a(x23), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x19), .O(new_n89_));
  inv1   g045(.a(x24), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n82_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n81_), .c(new_n46_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n60_), .c(new_n58_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n94_), .c(new_n46_), .d(x25), .O(new_n98_));
  oai21  g054(.a(new_n98_), .b(new_n60_), .c(new_n58_), .O(z4));
  inv1   g055(.a(new_n93_), .O(new_n100_));
  inv1   g056(.a(x29), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n81_), .O(new_n103_));
  nand3  g059(.a(new_n101_), .b(x28), .c(x27), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n101_), .c(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n100_), .c(new_n62_), .d(new_n46_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x25), .c(x01), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n58_), .O(z5));
  inv1   g065(.a(x25), .O(new_n110_));
  nand3  g066(.a(x29), .b(x28), .c(x27), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(x30), .O(new_n112_));
  inv1   g068(.a(x30), .O(new_n113_));
  nand3  g069(.a(new_n103_), .b(new_n113_), .c(x29), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n100_), .c(new_n62_), .d(new_n46_), .O(new_n116_));
  nor3   g072(.a(new_n116_), .b(new_n110_), .c(new_n60_), .O(z6));
  inv1   g073(.a(x31), .O(new_n118_));
  nor2   g074(.a(new_n113_), .b(new_n101_), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n103_), .c(new_n118_), .O(new_n120_));
  inv1   g076(.a(new_n82_), .O(new_n121_));
  nand4  g077(.a(new_n91_), .b(new_n88_), .c(new_n85_), .d(new_n121_), .O(new_n122_));
  nor2   g078(.a(new_n61_), .b(new_n60_), .O(new_n123_));
  nand3  g079(.a(new_n118_), .b(x30), .c(x29), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n103_), .c(new_n110_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n123_), .c(new_n122_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n120_), .c(new_n46_), .O(new_n128_));
  oai21  g084(.a(new_n46_), .b(x15), .c(new_n128_), .O(z7));
  inv1   g085(.a(x32), .O(new_n130_));
  nor2   g086(.a(x28), .b(x27), .O(new_n131_));
  inv1   g087(.a(new_n131_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n113_), .c(x29), .O(new_n133_));
  nor2   g089(.a(new_n131_), .b(new_n101_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n113_), .c(new_n133_), .O(new_n135_));
  nand3  g091(.a(x19), .b(new_n86_), .c(x13), .O(new_n136_));
  nand3  g092(.a(new_n89_), .b(x18), .c(x14), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n136_), .c(new_n83_), .O(new_n138_));
  nand4  g094(.a(x19), .b(x18), .c(new_n83_), .d(x12), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  inv1   g097(.a(x16), .O(new_n142_));
  nor2   g098(.a(new_n89_), .b(new_n86_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(x17), .c(new_n142_), .d(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n141_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x15), .O(new_n146_));
  nand3  g102(.a(x16), .b(new_n45_), .c(x10), .O(new_n147_));
  nand4  g103(.a(new_n46_), .b(x19), .c(x18), .d(x17), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n147_), .c(new_n146_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n135_), .c(new_n130_), .d(x31), .O(new_n150_));
  nand2  g106(.a(new_n119_), .b(x27), .O(new_n151_));
  oai21  g107(.a(x30), .b(x27), .c(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n143_), .b(x17), .c(x15), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n142_), .O(new_n154_));
  nand2  g110(.a(new_n143_), .b(x15), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n83_), .O(new_n156_));
  aoi21  g112(.a(x19), .b(x15), .c(x18), .O(new_n157_));
  inv1   g113(.a(new_n157_), .O(new_n158_));
  nand2  g114(.a(x19), .b(x10), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n45_), .O(new_n160_));
  nand3  g116(.a(x30), .b(x29), .c(x28), .O(new_n161_));
  nand2  g117(.a(new_n113_), .b(new_n101_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n161_), .c(new_n160_), .d(x31), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(new_n164_));
  nand4  g120(.a(new_n164_), .b(new_n158_), .c(new_n156_), .d(new_n154_), .O(new_n165_));
  aoi21  g121(.a(new_n152_), .b(new_n102_), .c(new_n165_), .O(new_n166_));
  oai22  g122(.a(x17), .b(x12), .c(x16), .d(x11), .O(new_n167_));
  inv1   g123(.a(x13), .O(new_n168_));
  nand2  g124(.a(new_n86_), .b(new_n168_), .O(new_n169_));
  nand3  g125(.a(new_n143_), .b(x17), .c(x16), .O(new_n170_));
  inv1   g126(.a(x14), .O(new_n171_));
  nand2  g127(.a(new_n89_), .b(new_n171_), .O(new_n172_));
  nand3  g128(.a(new_n172_), .b(new_n170_), .c(new_n169_), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n167_), .c(x15), .O(new_n174_));
  oai21  g130(.a(new_n166_), .b(x26), .c(new_n174_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(x32), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n150_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(x00), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n58_), .O(z8));
  inv1   g135(.a(x33), .O(new_n180_));
  nand4  g136(.a(x17), .b(x16), .c(new_n45_), .d(x10), .O(new_n181_));
  nand4  g137(.a(new_n143_), .b(new_n180_), .c(x31), .d(x30), .O(new_n182_));
  oai22  g138(.a(new_n182_), .b(new_n181_), .c(new_n180_), .d(x31), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n46_), .O(new_n184_));
  aoi21  g140(.a(new_n144_), .b(new_n141_), .c(x33), .O(new_n185_));
  nand4  g141(.a(new_n185_), .b(x31), .c(x30), .d(x15), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n132_), .c(x29), .O(new_n188_));
  inv1   g144(.a(new_n134_), .O(new_n189_));
  nand4  g145(.a(new_n143_), .b(new_n180_), .c(new_n118_), .d(new_n113_), .O(new_n190_));
  oai22  g146(.a(new_n190_), .b(new_n181_), .c(new_n180_), .d(new_n118_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n46_), .O(new_n192_));
  nand4  g148(.a(new_n185_), .b(new_n118_), .c(new_n113_), .d(x15), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(new_n192_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n189_), .O(new_n195_));
  inv1   g151(.a(new_n154_), .O(new_n196_));
  xnor2a g152(.a(x31), .b(x30), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(new_n160_), .c(new_n158_), .d(new_n156_), .O(new_n198_));
  oai21  g154(.a(new_n198_), .b(new_n196_), .c(new_n46_), .O(new_n199_));
  nand2  g155(.a(new_n199_), .b(new_n174_), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(x33), .O(new_n201_));
  nand3  g157(.a(new_n201_), .b(new_n195_), .c(new_n188_), .O(new_n202_));
  nand2  g158(.a(new_n202_), .b(x00), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(new_n58_), .O(z9));
endmodule


