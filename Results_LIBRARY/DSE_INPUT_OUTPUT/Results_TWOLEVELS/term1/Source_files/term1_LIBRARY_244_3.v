// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:19 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_;
  inv1   g000(.a(x27), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n53_), .c(new_n49_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n51_), .b(new_n54_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n51_), .b(new_n54_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n51_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n54_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n54_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n51_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n60_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g030(.a(new_n68_), .b(new_n66_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n49_), .c(new_n59_), .d(x08), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x22), .b(x17), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  and2   g046(.a(new_n90_), .b(new_n63_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n45_), .c(new_n82_), .d(x25), .O(new_n92_));
  oai21  g048(.a(new_n92_), .b(new_n61_), .c(new_n49_), .O(z3));
  xor2a  g049(.a(x28), .b(x27), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n91_), .c(new_n82_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n61_), .c(new_n49_), .O(z4));
  inv1   g052(.a(x25), .O(new_n97_));
  inv1   g053(.a(x29), .O(new_n98_));
  inv1   g054(.a(x28), .O(new_n99_));
  aoi21  g055(.a(new_n46_), .b(new_n99_), .c(new_n45_), .O(new_n100_));
  nand4  g056(.a(new_n46_), .b(new_n98_), .c(x28), .d(x27), .O(new_n101_));
  oai21  g057(.a(new_n100_), .b(new_n98_), .c(new_n101_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n90_), .c(new_n63_), .d(new_n82_), .O(new_n103_));
  nor3   g059(.a(new_n103_), .b(new_n97_), .c(new_n61_), .O(z5));
  nand2  g060(.a(x29), .b(x28), .O(new_n105_));
  oai21  g061(.a(new_n105_), .b(new_n45_), .c(new_n46_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n91_), .c(new_n82_), .d(x25), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n61_), .c(new_n49_), .O(z6));
  inv1   g064(.a(new_n87_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n86_), .c(new_n84_), .O(new_n110_));
  nand3  g066(.a(new_n63_), .b(x25), .c(x01), .O(new_n111_));
  nor4   g067(.a(new_n111_), .b(new_n47_), .c(x31), .d(x26), .O(new_n112_));
  oai21  g068(.a(new_n110_), .b(new_n88_), .c(new_n112_), .O(z7));
  inv1   g069(.a(x32), .O(new_n114_));
  inv1   g070(.a(new_n105_), .O(new_n115_));
  aoi21  g071(.a(new_n46_), .b(x28), .c(x27), .O(new_n116_));
  oai22  g072(.a(new_n116_), .b(new_n98_), .c(new_n115_), .d(new_n46_), .O(new_n117_));
  inv1   g073(.a(x17), .O(new_n118_));
  inv1   g074(.a(x18), .O(new_n119_));
  nand3  g075(.a(x19), .b(new_n119_), .c(x13), .O(new_n120_));
  inv1   g076(.a(x19), .O(new_n121_));
  nand3  g077(.a(new_n121_), .b(x18), .c(x14), .O(new_n122_));
  aoi21  g078(.a(new_n122_), .b(new_n120_), .c(new_n118_), .O(new_n123_));
  nand4  g079(.a(x19), .b(x18), .c(new_n118_), .d(x12), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n123_), .c(x16), .O(new_n126_));
  inv1   g082(.a(x16), .O(new_n127_));
  nand3  g083(.a(x19), .b(x18), .c(x17), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(new_n127_), .c(x11), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x15), .O(new_n132_));
  inv1   g088(.a(x15), .O(new_n133_));
  nand3  g089(.a(x16), .b(new_n133_), .c(x10), .O(new_n134_));
  nor2   g090(.a(new_n134_), .b(new_n128_), .O(new_n135_));
  inv1   g091(.a(new_n135_), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n117_), .c(new_n114_), .d(x31), .O(new_n138_));
  oai21  g094(.a(x28), .b(x27), .c(x29), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n46_), .O(new_n140_));
  nand2  g096(.a(x16), .b(x10), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n128_), .c(new_n133_), .O(new_n142_));
  nor2   g098(.a(new_n121_), .b(new_n119_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(x17), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n127_), .O(new_n145_));
  aoi21  g101(.a(new_n143_), .b(x12), .c(x17), .O(new_n146_));
  inv1   g102(.a(x13), .O(new_n147_));
  oai21  g103(.a(new_n121_), .b(new_n147_), .c(new_n119_), .O(new_n148_));
  inv1   g104(.a(x14), .O(new_n149_));
  nand2  g105(.a(new_n121_), .b(new_n149_), .O(new_n150_));
  nand3  g106(.a(new_n129_), .b(x16), .c(x15), .O(new_n151_));
  inv1   g107(.a(x31), .O(new_n152_));
  nor2   g108(.a(new_n46_), .b(new_n98_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(x28), .c(new_n152_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n151_), .c(new_n150_), .d(new_n148_), .O(new_n155_));
  nor2   g111(.a(new_n155_), .b(new_n146_), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n145_), .c(new_n142_), .d(new_n140_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x32), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n138_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n82_), .c(x00), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n49_), .O(z8));
  nand4  g117(.a(x33), .b(x29), .c(new_n82_), .d(x00), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n46_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(x27), .O(new_n164_));
  aoi21  g120(.a(new_n130_), .b(new_n126_), .c(new_n133_), .O(new_n165_));
  nand3  g121(.a(new_n139_), .b(new_n152_), .c(new_n46_), .O(new_n166_));
  nand3  g122(.a(new_n115_), .b(x31), .c(x30), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  oai21  g124(.a(new_n135_), .b(new_n165_), .c(new_n168_), .O(new_n169_));
  nand2  g125(.a(new_n121_), .b(x14), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(x18), .c(x17), .d(x16), .O(new_n171_));
  nor2   g127(.a(new_n171_), .b(new_n133_), .O(new_n172_));
  nand2  g128(.a(new_n105_), .b(x31), .O(new_n173_));
  nor2   g129(.a(new_n119_), .b(new_n118_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(x16), .c(x15), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n121_), .O(new_n176_));
  inv1   g132(.a(new_n174_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n141_), .c(new_n133_), .O(new_n178_));
  nand2  g134(.a(new_n174_), .b(x11), .O(new_n179_));
  nand2  g135(.a(x18), .b(x12), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n118_), .O(new_n181_));
  nand2  g137(.a(new_n119_), .b(new_n147_), .O(new_n182_));
  nand3  g138(.a(new_n46_), .b(x29), .c(x28), .O(new_n183_));
  nand2  g139(.a(new_n152_), .b(x30), .O(new_n184_));
  nand4  g140(.a(new_n184_), .b(new_n183_), .c(new_n182_), .d(new_n181_), .O(new_n185_));
  aoi21  g141(.a(new_n179_), .b(new_n127_), .c(new_n185_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n178_), .c(new_n176_), .d(new_n173_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n172_), .c(x33), .O(new_n188_));
  oai21  g144(.a(new_n169_), .b(x33), .c(new_n188_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n82_), .c(x00), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n164_), .O(z9));
endmodule


