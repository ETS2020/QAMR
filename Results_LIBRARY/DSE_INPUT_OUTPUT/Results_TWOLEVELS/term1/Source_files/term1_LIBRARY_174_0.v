// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  nor2   g000(.a(x29), .b(x26), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  nor2   g014(.a(new_n49_), .b(new_n52_), .O(new_n59_));
  nor2   g015(.a(x03), .b(x02), .O(new_n60_));
  oai21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nand2  g018(.a(x03), .b(new_n62_), .O(new_n63_));
  oai21  g019(.a(x05), .b(x03), .c(new_n63_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand4  g022(.a(new_n66_), .b(x05), .c(new_n49_), .d(new_n62_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n65_), .c(new_n61_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  xnor2a g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n64_), .b(x02), .O(new_n72_));
  aoi21  g028(.a(x05), .b(new_n49_), .c(x06), .O(new_n73_));
  nand3  g029(.a(new_n60_), .b(x06), .c(x05), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n73_), .c(new_n62_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  aoi21  g033(.a(new_n71_), .b(x01), .c(new_n77_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n70_), .c(new_n69_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n57_), .c(x08), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n47_), .O(z2));
  inv1   g037(.a(x27), .O(new_n82_));
  inv1   g038(.a(new_n59_), .O(new_n83_));
  nor2   g039(.a(x20), .b(x15), .O(new_n84_));
  nor2   g040(.a(x21), .b(x16), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x17), .O(new_n87_));
  inv1   g043(.a(x22), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(x18), .O(new_n90_));
  inv1   g046(.a(x23), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g048(.a(x19), .O(new_n93_));
  inv1   g049(.a(x24), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n92_), .c(new_n89_), .d(new_n86_), .O(new_n96_));
  and2   g052(.a(new_n96_), .b(new_n83_), .O(new_n97_));
  nand3  g053(.a(new_n97_), .b(x29), .c(new_n82_), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(new_n99_));
  nand3  g055(.a(new_n99_), .b(x25), .c(x01), .O(new_n100_));
  aoi21  g056(.a(new_n100_), .b(x29), .c(x26), .O(z3));
  inv1   g057(.a(x26), .O(new_n102_));
  xor2a  g058(.a(x28), .b(x27), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(new_n97_), .c(x29), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n102_), .c(x25), .d(x01), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(z4));
  nand2  g063(.a(x28), .b(x27), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(new_n97_), .c(x29), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n102_), .c(x25), .d(x01), .O(new_n111_));
  inv1   g067(.a(new_n111_), .O(z5));
  inv1   g068(.a(x28), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n82_), .c(x30), .O(new_n114_));
  inv1   g070(.a(x30), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x28), .c(x27), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n96_), .c(new_n83_), .d(x29), .O(new_n118_));
  inv1   g074(.a(new_n118_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n102_), .c(x25), .d(x01), .O(new_n120_));
  inv1   g076(.a(new_n120_), .O(z6));
  nand3  g077(.a(x30), .b(x28), .c(x27), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x31), .O(new_n123_));
  nand4  g079(.a(new_n95_), .b(new_n92_), .c(new_n89_), .d(new_n86_), .O(new_n124_));
  inv1   g080(.a(x31), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(x30), .c(x28), .d(x27), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n83_), .c(x25), .d(x01), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n124_), .c(new_n123_), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(x29), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n102_), .O(z7));
  inv1   g087(.a(x32), .O(new_n132_));
  nand2  g088(.a(new_n113_), .b(new_n82_), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand3  g090(.a(x30), .b(new_n113_), .c(new_n82_), .O(new_n135_));
  oai21  g091(.a(new_n134_), .b(x30), .c(new_n135_), .O(new_n136_));
  nand3  g092(.a(x19), .b(new_n90_), .c(x13), .O(new_n137_));
  nand3  g093(.a(new_n93_), .b(x18), .c(x14), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n137_), .c(new_n87_), .O(new_n139_));
  nand4  g095(.a(x19), .b(x18), .c(new_n87_), .d(x12), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n139_), .c(x16), .O(new_n142_));
  inv1   g098(.a(x16), .O(new_n143_));
  nand2  g099(.a(x19), .b(x18), .O(new_n144_));
  inv1   g100(.a(new_n144_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x17), .O(new_n146_));
  inv1   g102(.a(new_n146_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(new_n143_), .c(x11), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n142_), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(x15), .O(new_n150_));
  inv1   g106(.a(x15), .O(new_n151_));
  nand4  g107(.a(new_n147_), .b(x16), .c(new_n151_), .d(x10), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n150_), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n136_), .c(new_n132_), .d(x31), .O(new_n154_));
  xnor2a g110(.a(x30), .b(x27), .O(new_n155_));
  nand2  g111(.a(x16), .b(x10), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n146_), .c(new_n151_), .O(new_n157_));
  nand2  g113(.a(x17), .b(x11), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n144_), .c(new_n143_), .O(new_n159_));
  inv1   g115(.a(x12), .O(new_n160_));
  oai21  g116(.a(new_n144_), .b(new_n160_), .c(new_n87_), .O(new_n161_));
  nand2  g117(.a(x19), .b(x13), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n90_), .O(new_n163_));
  inv1   g119(.a(x14), .O(new_n164_));
  nand2  g120(.a(new_n93_), .b(new_n164_), .O(new_n165_));
  nand3  g121(.a(new_n147_), .b(x16), .c(x15), .O(new_n166_));
  aoi21  g122(.a(x30), .b(x28), .c(new_n125_), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(new_n166_), .c(new_n165_), .d(new_n163_), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n161_), .c(new_n159_), .d(new_n157_), .O(new_n170_));
  aoi21  g126(.a(new_n155_), .b(new_n113_), .c(new_n170_), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n132_), .c(new_n154_), .O(new_n172_));
  nand4  g128(.a(new_n172_), .b(x29), .c(new_n102_), .d(x00), .O(new_n173_));
  inv1   g129(.a(new_n173_), .O(z8));
  inv1   g130(.a(x33), .O(new_n175_));
  nand4  g131(.a(new_n153_), .b(new_n175_), .c(x31), .d(x30), .O(new_n176_));
  nand2  g132(.a(x33), .b(new_n125_), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n176_), .c(new_n134_), .O(new_n178_));
  nand2  g134(.a(x33), .b(x19), .O(new_n179_));
  nor2   g135(.a(x27), .b(x19), .O(new_n180_));
  nor2   g136(.a(x30), .b(x28), .O(new_n181_));
  nor2   g137(.a(x33), .b(x31), .O(new_n182_));
  nand4  g138(.a(new_n182_), .b(new_n181_), .c(new_n180_), .d(x14), .O(new_n183_));
  aoi21  g139(.a(new_n183_), .b(new_n179_), .c(new_n90_), .O(new_n184_));
  nand2  g140(.a(new_n182_), .b(new_n181_), .O(new_n185_));
  nand4  g141(.a(new_n82_), .b(x19), .c(new_n90_), .d(x13), .O(new_n186_));
  nor2   g142(.a(new_n186_), .b(new_n185_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n184_), .c(x17), .O(new_n188_));
  nor3   g144(.a(x33), .b(x31), .c(x30), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n141_), .c(new_n134_), .O(new_n190_));
  aoi21  g146(.a(new_n190_), .b(new_n188_), .c(new_n143_), .O(new_n191_));
  nand2  g147(.a(new_n189_), .b(new_n134_), .O(new_n192_));
  nor2   g148(.a(new_n192_), .b(new_n148_), .O(new_n193_));
  oai21  g149(.a(new_n193_), .b(new_n191_), .c(x15), .O(new_n194_));
  nand2  g150(.a(x33), .b(x30), .O(new_n195_));
  inv1   g151(.a(x10), .O(new_n196_));
  nor2   g152(.a(x15), .b(new_n196_), .O(new_n197_));
  nor2   g153(.a(new_n87_), .b(new_n143_), .O(new_n198_));
  nand4  g154(.a(new_n198_), .b(new_n197_), .c(new_n189_), .d(new_n145_), .O(new_n199_));
  aoi21  g155(.a(new_n199_), .b(new_n195_), .c(x28), .O(new_n200_));
  nand2  g156(.a(x31), .b(new_n115_), .O(new_n201_));
  nand3  g157(.a(new_n201_), .b(new_n165_), .c(new_n163_), .O(new_n202_));
  inv1   g158(.a(new_n202_), .O(new_n203_));
  nand4  g159(.a(new_n203_), .b(new_n161_), .c(new_n159_), .d(new_n157_), .O(new_n204_));
  aoi22  g160(.a(new_n204_), .b(x33), .c(new_n200_), .d(new_n82_), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n194_), .O(new_n206_));
  oai21  g162(.a(new_n206_), .b(new_n178_), .c(x00), .O(new_n207_));
  aoi21  g163(.a(new_n207_), .b(x29), .c(x26), .O(z9));
endmodule


