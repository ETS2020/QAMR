// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n94_, new_n95_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_;
  inv1   g000(.a(x27), .O(new_n45_));
  nor2   g001(.a(x30), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  inv1   g013(.a(new_n46_), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n49_), .b(new_n52_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n49_), .b(new_n52_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n49_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n52_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n52_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n49_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n58_), .c(new_n57_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x25), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x22), .b(x17), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  and2   g046(.a(new_n90_), .b(new_n62_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n45_), .c(new_n82_), .O(new_n92_));
  nor3   g048(.a(new_n92_), .b(new_n81_), .c(new_n60_), .O(z3));
  xor2a  g049(.a(x28), .b(x27), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n91_), .c(new_n82_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n60_), .c(new_n58_), .O(z4));
  inv1   g052(.a(x29), .O(new_n97_));
  inv1   g053(.a(x28), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n45_), .O(new_n99_));
  nand3  g055(.a(new_n97_), .b(x28), .c(x27), .O(new_n100_));
  oai21  g056(.a(new_n99_), .b(new_n97_), .c(new_n100_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n90_), .c(new_n62_), .d(new_n82_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x25), .c(x01), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n58_), .O(z5));
  inv1   g061(.a(x30), .O(new_n106_));
  oai21  g062(.a(new_n97_), .b(new_n98_), .c(x27), .O(new_n107_));
  oai21  g063(.a(new_n106_), .b(x27), .c(new_n107_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n90_), .c(new_n62_), .d(new_n82_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand3  g066(.a(new_n110_), .b(x25), .c(x01), .O(new_n111_));
  nand2  g067(.a(new_n111_), .b(new_n58_), .O(z6));
  nor4   g068(.a(new_n88_), .b(new_n87_), .c(new_n85_), .d(new_n83_), .O(new_n113_));
  nand4  g069(.a(new_n62_), .b(new_n82_), .c(x25), .d(x01), .O(new_n114_));
  oai21  g070(.a(new_n114_), .b(new_n113_), .c(new_n58_), .O(new_n115_));
  nand2  g071(.a(x29), .b(x28), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x31), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  nand3  g074(.a(new_n99_), .b(new_n118_), .c(x29), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(new_n117_), .O(new_n120_));
  aoi22  g076(.a(new_n120_), .b(x30), .c(x31), .d(new_n45_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n115_), .O(z7));
  and2   g078(.a(x16), .b(x10), .O(new_n123_));
  nand3  g079(.a(x19), .b(x18), .c(x17), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n123_), .c(x15), .O(new_n126_));
  inv1   g082(.a(x16), .O(new_n127_));
  nand2  g083(.a(x19), .b(x18), .O(new_n128_));
  nand2  g084(.a(x17), .b(x11), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n128_), .c(new_n127_), .O(new_n130_));
  inv1   g086(.a(new_n128_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(x12), .c(x17), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  inv1   g089(.a(x18), .O(new_n134_));
  nand2  g090(.a(x19), .b(x13), .O(new_n135_));
  nand2  g091(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  inv1   g092(.a(x14), .O(new_n137_));
  inv1   g093(.a(x19), .O(new_n138_));
  nand2  g094(.a(x16), .b(x15), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n124_), .c(x31), .O(new_n140_));
  aoi21  g096(.a(new_n138_), .b(new_n137_), .c(new_n140_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n136_), .c(new_n133_), .d(new_n130_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n126_), .c(new_n58_), .O(new_n143_));
  nand3  g099(.a(new_n116_), .b(new_n106_), .c(new_n45_), .O(new_n144_));
  nand2  g100(.a(new_n98_), .b(new_n45_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(x30), .c(x29), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n144_), .c(new_n143_), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(x32), .O(new_n148_));
  inv1   g104(.a(x32), .O(new_n149_));
  nand3  g105(.a(new_n106_), .b(x29), .c(x28), .O(new_n150_));
  oai21  g106(.a(new_n106_), .b(x28), .c(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n45_), .O(new_n152_));
  oai21  g108(.a(new_n106_), .b(x29), .c(new_n152_), .O(new_n153_));
  inv1   g109(.a(x17), .O(new_n154_));
  nand3  g110(.a(x19), .b(new_n134_), .c(x13), .O(new_n155_));
  nand3  g111(.a(new_n138_), .b(x18), .c(x14), .O(new_n156_));
  aoi21  g112(.a(new_n156_), .b(new_n155_), .c(new_n154_), .O(new_n157_));
  nand4  g113(.a(x19), .b(x18), .c(new_n154_), .d(x12), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n157_), .c(x16), .O(new_n160_));
  nand3  g116(.a(new_n125_), .b(new_n127_), .c(x11), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(x15), .O(new_n163_));
  nor2   g119(.a(new_n127_), .b(x15), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n125_), .c(x10), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n163_), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(new_n153_), .c(new_n149_), .d(x31), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n148_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n82_), .c(x00), .O(new_n169_));
  inv1   g125(.a(new_n169_), .O(z8));
  inv1   g126(.a(x33), .O(new_n171_));
  nand4  g127(.a(new_n145_), .b(x31), .c(x30), .d(x29), .O(new_n172_));
  nand3  g128(.a(new_n116_), .b(new_n118_), .c(new_n106_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n172_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n166_), .c(new_n171_), .O(new_n175_));
  aoi21  g131(.a(x31), .b(x29), .c(new_n106_), .O(new_n176_));
  inv1   g132(.a(new_n126_), .O(new_n177_));
  nand2  g133(.a(new_n145_), .b(x30), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x31), .O(new_n179_));
  nand2  g135(.a(new_n138_), .b(new_n137_), .O(new_n180_));
  nand3  g136(.a(new_n125_), .b(x16), .c(x15), .O(new_n181_));
  nand3  g137(.a(new_n118_), .b(x29), .c(x28), .O(new_n182_));
  nand4  g138(.a(new_n182_), .b(new_n181_), .c(new_n180_), .d(new_n136_), .O(new_n183_));
  nor2   g139(.a(new_n183_), .b(new_n132_), .O(new_n184_));
  nand4  g140(.a(new_n184_), .b(new_n179_), .c(new_n130_), .d(new_n177_), .O(new_n185_));
  oai21  g141(.a(new_n185_), .b(new_n176_), .c(x33), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n175_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n82_), .c(x00), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n58_), .O(z9));
endmodule


