// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_;
  nor2   g000(.a(x26), .b(x09), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nor2   g002(.a(x32), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nor2   g004(.a(x33), .b(new_n48_), .O(new_n49_));
  oai21  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(x32), .b(new_n48_), .O(new_n52_));
  nor2   g008(.a(x33), .b(x03), .O(new_n53_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x09), .O(new_n56_));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  nor2   g013(.a(new_n48_), .b(new_n51_), .O(new_n58_));
  nor2   g014(.a(x03), .b(x02), .O(new_n59_));
  oai21  g015(.a(new_n59_), .b(new_n58_), .c(x01), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand3  g017(.a(x05), .b(new_n48_), .c(x02), .O(new_n62_));
  nand3  g018(.a(x06), .b(x03), .c(new_n51_), .O(new_n63_));
  nand2  g019(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g020(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  xor2a  g021(.a(x06), .b(x05), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(new_n48_), .c(new_n51_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n65_), .c(new_n60_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(x01), .O(new_n72_));
  nor2   g028(.a(new_n48_), .b(x01), .O(new_n73_));
  nor2   g029(.a(x05), .b(x03), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n73_), .c(x02), .O(new_n75_));
  nand3  g031(.a(new_n59_), .b(x06), .c(x05), .O(new_n76_));
  oai21  g032(.a(x06), .b(x05), .c(new_n76_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n61_), .O(new_n78_));
  inv1   g034(.a(x06), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(x03), .c(new_n51_), .O(new_n80_));
  nand4  g036(.a(new_n80_), .b(new_n78_), .c(new_n75_), .d(new_n72_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n69_), .O(new_n83_));
  nand4  g039(.a(new_n83_), .b(x26), .c(new_n56_), .d(x08), .O(new_n84_));
  inv1   g040(.a(new_n84_), .O(z2));
  inv1   g041(.a(x26), .O(new_n86_));
  inv1   g042(.a(x27), .O(new_n87_));
  nor2   g043(.a(x20), .b(x15), .O(new_n88_));
  nor2   g044(.a(x21), .b(x16), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nor2   g046(.a(x22), .b(x17), .O(new_n91_));
  nor2   g047(.a(x23), .b(x18), .O(new_n92_));
  nor2   g048(.a(x24), .b(x19), .O(new_n93_));
  nor3   g049(.a(new_n93_), .b(new_n92_), .c(new_n91_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n90_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n58_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n87_), .c(new_n86_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(x25), .c(x09), .d(x01), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z3));
  xor2a  g056(.a(x28), .b(x27), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n96_), .c(x25), .d(x01), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(x09), .c(x26), .O(z4));
  inv1   g059(.a(new_n58_), .O(new_n104_));
  inv1   g060(.a(x28), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n87_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(x29), .O(new_n108_));
  inv1   g064(.a(x29), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x28), .c(x27), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n108_), .c(new_n95_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n104_), .c(x25), .d(x01), .O(new_n112_));
  aoi21  g068(.a(new_n112_), .b(x09), .c(x26), .O(z5));
  nand3  g069(.a(x29), .b(x28), .c(x27), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x30), .O(new_n115_));
  inv1   g071(.a(x30), .O(new_n116_));
  nand3  g072(.a(new_n106_), .b(new_n116_), .c(x29), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n115_), .c(new_n95_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n104_), .c(x25), .d(x01), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(x09), .c(x26), .O(z6));
  nor2   g076(.a(new_n116_), .b(new_n109_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n106_), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x31), .O(new_n123_));
  inv1   g079(.a(new_n91_), .O(new_n124_));
  inv1   g080(.a(new_n92_), .O(new_n125_));
  inv1   g081(.a(new_n93_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n125_), .c(new_n124_), .d(new_n90_), .O(new_n127_));
  nand3  g083(.a(new_n104_), .b(x25), .c(x01), .O(new_n128_));
  inv1   g084(.a(new_n128_), .O(new_n129_));
  nor4   g085(.a(new_n107_), .b(x31), .c(new_n116_), .d(new_n109_), .O(new_n130_));
  nor3   g086(.a(new_n130_), .b(x26), .c(new_n56_), .O(new_n131_));
  nand4  g087(.a(new_n131_), .b(new_n129_), .c(new_n127_), .d(new_n123_), .O(z7));
  inv1   g088(.a(x32), .O(new_n133_));
  nor2   g089(.a(x28), .b(x27), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n116_), .c(x29), .O(new_n136_));
  nor2   g092(.a(new_n134_), .b(new_n109_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n116_), .c(new_n136_), .O(new_n138_));
  inv1   g094(.a(x17), .O(new_n139_));
  inv1   g095(.a(x18), .O(new_n140_));
  nand3  g096(.a(x19), .b(new_n140_), .c(x13), .O(new_n141_));
  inv1   g097(.a(x19), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x18), .c(x14), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n141_), .c(new_n139_), .O(new_n144_));
  nand4  g100(.a(x19), .b(x18), .c(new_n139_), .d(x12), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n144_), .c(x16), .O(new_n147_));
  inv1   g103(.a(x16), .O(new_n148_));
  nand2  g104(.a(x19), .b(x18), .O(new_n149_));
  inv1   g105(.a(new_n149_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x17), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n148_), .c(x11), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n147_), .O(new_n154_));
  nand2  g110(.a(new_n154_), .b(x15), .O(new_n155_));
  inv1   g111(.a(x15), .O(new_n156_));
  nand4  g112(.a(new_n152_), .b(x16), .c(new_n156_), .d(x10), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n138_), .c(new_n133_), .d(x31), .O(new_n159_));
  nand2  g115(.a(new_n121_), .b(x27), .O(new_n160_));
  oai21  g116(.a(x30), .b(x27), .c(new_n160_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n105_), .O(new_n162_));
  nand2  g118(.a(x16), .b(x10), .O(new_n163_));
  oai21  g119(.a(new_n163_), .b(new_n151_), .c(new_n156_), .O(new_n164_));
  nand2  g120(.a(x17), .b(x11), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n149_), .c(new_n148_), .O(new_n166_));
  inv1   g122(.a(x12), .O(new_n167_));
  oai21  g123(.a(new_n149_), .b(new_n167_), .c(new_n139_), .O(new_n168_));
  inv1   g124(.a(x13), .O(new_n169_));
  oai21  g125(.a(new_n142_), .b(new_n169_), .c(new_n140_), .O(new_n170_));
  nor2   g126(.a(x19), .b(x14), .O(new_n171_));
  nor2   g127(.a(new_n148_), .b(new_n156_), .O(new_n172_));
  aoi21  g128(.a(new_n172_), .b(new_n152_), .c(new_n171_), .O(new_n173_));
  oai21  g129(.a(x30), .b(x29), .c(x31), .O(new_n174_));
  aoi21  g130(.a(new_n121_), .b(x28), .c(new_n174_), .O(new_n175_));
  nand4  g131(.a(new_n175_), .b(new_n173_), .c(new_n170_), .d(new_n168_), .O(new_n176_));
  inv1   g132(.a(new_n176_), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(new_n166_), .c(new_n164_), .d(new_n162_), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(x32), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n159_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n86_), .c(x09), .d(x00), .O(new_n181_));
  inv1   g137(.a(new_n181_), .O(z8));
  inv1   g138(.a(x33), .O(new_n183_));
  nand4  g139(.a(new_n158_), .b(new_n183_), .c(x31), .d(x30), .O(new_n184_));
  inv1   g140(.a(x31), .O(new_n185_));
  nand2  g141(.a(x33), .b(new_n185_), .O(new_n186_));
  nand2  g142(.a(new_n186_), .b(new_n184_), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n135_), .c(x29), .O(new_n188_));
  inv1   g144(.a(new_n137_), .O(new_n189_));
  nand4  g145(.a(new_n158_), .b(new_n183_), .c(new_n185_), .d(new_n116_), .O(new_n190_));
  nand2  g146(.a(x33), .b(x31), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n189_), .O(new_n193_));
  aoi21  g149(.a(new_n142_), .b(x14), .c(new_n140_), .O(new_n194_));
  nand4  g150(.a(new_n194_), .b(x17), .c(x16), .d(x15), .O(new_n195_));
  inv1   g151(.a(new_n172_), .O(new_n196_));
  nand2  g152(.a(x18), .b(x17), .O(new_n197_));
  oai21  g153(.a(new_n197_), .b(new_n196_), .c(new_n142_), .O(new_n198_));
  oai21  g154(.a(new_n197_), .b(new_n163_), .c(new_n156_), .O(new_n199_));
  inv1   g155(.a(x11), .O(new_n200_));
  oai21  g156(.a(new_n197_), .b(new_n200_), .c(new_n148_), .O(new_n201_));
  oai21  g157(.a(new_n140_), .b(new_n167_), .c(new_n139_), .O(new_n202_));
  nand2  g158(.a(new_n140_), .b(new_n169_), .O(new_n203_));
  xnor2a g159(.a(x31), .b(x30), .O(new_n204_));
  nand4  g160(.a(new_n204_), .b(new_n203_), .c(new_n202_), .d(new_n201_), .O(new_n205_));
  inv1   g161(.a(new_n205_), .O(new_n206_));
  nand4  g162(.a(new_n206_), .b(new_n199_), .c(new_n198_), .d(new_n195_), .O(new_n207_));
  nand2  g163(.a(new_n207_), .b(x33), .O(new_n208_));
  nand3  g164(.a(new_n208_), .b(new_n193_), .c(new_n188_), .O(new_n209_));
  nand2  g165(.a(new_n209_), .b(x00), .O(new_n210_));
  aoi21  g166(.a(new_n210_), .b(x09), .c(x26), .O(z9));
endmodule


