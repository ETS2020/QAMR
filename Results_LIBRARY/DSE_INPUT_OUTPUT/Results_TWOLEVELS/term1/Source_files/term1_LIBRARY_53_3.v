// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  nand2  g000(.a(x30), .b(x27), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n49_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n58_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xnor2a g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g028(.a(new_n66_), .b(new_n64_), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n45_), .c(new_n57_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x25), .O(new_n80_));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x18), .O(new_n87_));
  inv1   g043(.a(x23), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(x19), .O(new_n90_));
  inv1   g046(.a(x24), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand3  g048(.a(new_n92_), .b(new_n89_), .c(new_n86_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n83_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n60_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n82_), .c(new_n81_), .O(new_n96_));
  nor3   g052(.a(new_n96_), .b(new_n80_), .c(new_n59_), .O(z3));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n95_), .c(new_n81_), .d(x25), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n59_), .c(new_n45_), .O(z4));
  oai21  g056(.a(x30), .b(x28), .c(x27), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x29), .O(new_n102_));
  nor2   g058(.a(x30), .b(x29), .O(new_n103_));
  nand3  g059(.a(new_n103_), .b(x28), .c(x27), .O(new_n104_));
  aoi21  g060(.a(new_n104_), .b(new_n102_), .c(new_n94_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(new_n61_), .c(new_n81_), .O(new_n106_));
  nor3   g062(.a(new_n106_), .b(new_n80_), .c(new_n59_), .O(z5));
  inv1   g063(.a(x30), .O(new_n108_));
  nand2  g064(.a(x29), .b(x28), .O(new_n109_));
  oai21  g065(.a(new_n109_), .b(new_n82_), .c(new_n108_), .O(new_n110_));
  nand4  g066(.a(new_n110_), .b(new_n95_), .c(new_n81_), .d(x25), .O(new_n111_));
  oai21  g067(.a(new_n111_), .b(new_n59_), .c(new_n45_), .O(z6));
  inv1   g068(.a(new_n83_), .O(new_n113_));
  nand4  g069(.a(new_n92_), .b(new_n89_), .c(new_n86_), .d(new_n113_), .O(new_n114_));
  inv1   g070(.a(x31), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(new_n81_), .c(x25), .O(new_n116_));
  nor3   g072(.a(new_n116_), .b(new_n60_), .c(new_n59_), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n114_), .c(new_n46_), .O(z7));
  nand2  g074(.a(x16), .b(x10), .O(new_n119_));
  nand3  g075(.a(x19), .b(x18), .c(x17), .O(new_n120_));
  nor2   g076(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nor2   g077(.a(new_n121_), .b(x15), .O(new_n122_));
  inv1   g078(.a(x16), .O(new_n123_));
  nand2  g079(.a(x19), .b(x18), .O(new_n124_));
  nand2  g080(.a(x17), .b(x11), .O(new_n125_));
  oai21  g081(.a(new_n125_), .b(new_n124_), .c(new_n123_), .O(new_n126_));
  inv1   g082(.a(x12), .O(new_n127_));
  oai21  g083(.a(new_n124_), .b(new_n127_), .c(new_n84_), .O(new_n128_));
  inv1   g084(.a(x13), .O(new_n129_));
  oai21  g085(.a(new_n90_), .b(new_n129_), .c(new_n87_), .O(new_n130_));
  nor2   g086(.a(x19), .b(x14), .O(new_n131_));
  nand2  g087(.a(x16), .b(x15), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n120_), .c(x31), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n130_), .c(new_n128_), .d(new_n126_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n122_), .c(new_n45_), .O(new_n136_));
  nand3  g092(.a(x30), .b(x29), .c(x28), .O(new_n137_));
  oai21  g093(.a(x30), .b(x28), .c(new_n137_), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n82_), .c(new_n103_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n136_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x32), .O(new_n141_));
  inv1   g097(.a(x32), .O(new_n142_));
  nand3  g098(.a(new_n109_), .b(x30), .c(new_n82_), .O(new_n143_));
  inv1   g099(.a(x28), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n82_), .O(new_n145_));
  nand3  g101(.a(new_n145_), .b(new_n108_), .c(x29), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n143_), .O(new_n147_));
  nand3  g103(.a(x19), .b(new_n87_), .c(x13), .O(new_n148_));
  nand3  g104(.a(new_n90_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n148_), .c(new_n84_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n84_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  inv1   g109(.a(new_n120_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(new_n123_), .c(x11), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n153_), .O(new_n156_));
  nand2  g112(.a(new_n156_), .b(x15), .O(new_n157_));
  inv1   g113(.a(x15), .O(new_n158_));
  nand4  g114(.a(new_n154_), .b(x16), .c(new_n158_), .d(x10), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n147_), .c(new_n142_), .d(x31), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n141_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(new_n81_), .c(x00), .O(new_n163_));
  inv1   g119(.a(new_n163_), .O(z8));
  nand2  g120(.a(new_n90_), .b(x14), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(x18), .c(x17), .d(x16), .O(new_n166_));
  nor2   g122(.a(new_n166_), .b(new_n158_), .O(new_n167_));
  nand2  g123(.a(x18), .b(x17), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n132_), .c(new_n90_), .O(new_n169_));
  oai21  g125(.a(new_n168_), .b(new_n119_), .c(new_n158_), .O(new_n170_));
  inv1   g126(.a(x11), .O(new_n171_));
  oai21  g127(.a(new_n168_), .b(new_n171_), .c(new_n123_), .O(new_n172_));
  nand2  g128(.a(x18), .b(x12), .O(new_n173_));
  aoi22  g129(.a(new_n173_), .b(new_n84_), .c(new_n87_), .d(new_n129_), .O(new_n174_));
  nand4  g130(.a(new_n174_), .b(new_n172_), .c(new_n170_), .d(new_n169_), .O(new_n175_));
  oai21  g131(.a(new_n175_), .b(new_n167_), .c(new_n45_), .O(new_n176_));
  nand2  g132(.a(new_n145_), .b(x29), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n115_), .O(new_n178_));
  nand2  g134(.a(new_n109_), .b(x31), .O(new_n179_));
  oai21  g135(.a(x31), .b(new_n108_), .c(new_n179_), .O(new_n180_));
  aoi22  g136(.a(new_n180_), .b(new_n82_), .c(new_n178_), .d(new_n108_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n176_), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(x33), .O(new_n183_));
  aoi21  g139(.a(new_n155_), .b(new_n153_), .c(new_n158_), .O(new_n184_));
  inv1   g140(.a(new_n159_), .O(new_n185_));
  nand2  g141(.a(new_n115_), .b(new_n108_), .O(new_n186_));
  nand4  g142(.a(x31), .b(x30), .c(x29), .d(x28), .O(new_n187_));
  oai21  g143(.a(new_n186_), .b(x28), .c(new_n187_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n82_), .O(new_n189_));
  oai21  g145(.a(new_n186_), .b(x29), .c(new_n189_), .O(new_n190_));
  oai21  g146(.a(new_n185_), .b(new_n184_), .c(new_n190_), .O(new_n191_));
  oai21  g147(.a(new_n191_), .b(x33), .c(new_n183_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n81_), .c(x00), .O(new_n193_));
  inv1   g149(.a(new_n193_), .O(z9));
endmodule


