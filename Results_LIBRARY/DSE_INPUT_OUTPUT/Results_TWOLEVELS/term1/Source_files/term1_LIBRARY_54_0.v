// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_;
  nor2   g000(.a(x26), .b(x00), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  inv1   g002(.a(new_n45_), .O(new_n47_));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n47_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  nor2   g014(.a(new_n49_), .b(new_n52_), .O(new_n59_));
  nor2   g015(.a(x03), .b(x02), .O(new_n60_));
  oai21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nand2  g018(.a(x03), .b(new_n62_), .O(new_n63_));
  oai21  g019(.a(x05), .b(x03), .c(new_n63_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand4  g022(.a(new_n66_), .b(x05), .c(new_n49_), .d(new_n62_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n65_), .c(new_n61_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  xnor2a g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n64_), .b(x02), .O(new_n72_));
  aoi21  g028(.a(x05), .b(new_n49_), .c(x06), .O(new_n73_));
  nand3  g029(.a(new_n60_), .b(x06), .c(x05), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n73_), .c(new_n62_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  aoi21  g033(.a(new_n71_), .b(x01), .c(new_n77_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n70_), .c(new_n69_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n57_), .c(x08), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n47_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  oai22  g039(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n84_));
  inv1   g040(.a(x17), .O(new_n85_));
  inv1   g041(.a(x22), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x18), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  inv1   g047(.a(x24), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n90_), .c(new_n87_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n59_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n83_), .c(new_n82_), .O(new_n97_));
  inv1   g053(.a(new_n97_), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(x25), .c(x01), .d(x00), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(z3));
  xor2a  g056(.a(x28), .b(x27), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n96_), .c(x25), .d(x01), .O(new_n102_));
  aoi21  g058(.a(new_n102_), .b(x00), .c(x26), .O(z4));
  inv1   g059(.a(new_n59_), .O(new_n104_));
  inv1   g060(.a(new_n95_), .O(new_n105_));
  inv1   g061(.a(x29), .O(new_n106_));
  and2   g062(.a(x28), .b(x27), .O(new_n107_));
  nand3  g063(.a(new_n106_), .b(x28), .c(x27), .O(new_n108_));
  oai21  g064(.a(new_n107_), .b(new_n106_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n105_), .c(new_n104_), .d(new_n82_), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(x25), .c(x01), .d(x00), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(z5));
  nand3  g069(.a(x29), .b(x28), .c(x27), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x30), .O(new_n115_));
  inv1   g071(.a(x30), .O(new_n116_));
  nand3  g072(.a(new_n107_), .b(new_n116_), .c(x29), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n105_), .c(new_n104_), .d(new_n82_), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(x25), .c(x01), .d(x00), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(z6));
  inv1   g078(.a(x31), .O(new_n123_));
  nor2   g079(.a(new_n116_), .b(new_n106_), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n107_), .c(new_n123_), .O(new_n125_));
  inv1   g081(.a(new_n84_), .O(new_n126_));
  nand4  g082(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n126_), .O(new_n127_));
  nor2   g083(.a(new_n59_), .b(new_n62_), .O(new_n128_));
  inv1   g084(.a(x25), .O(new_n129_));
  nand3  g085(.a(new_n123_), .b(x30), .c(x29), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n107_), .c(new_n129_), .O(new_n132_));
  nand3  g088(.a(new_n132_), .b(new_n128_), .c(new_n127_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n125_), .c(x00), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n82_), .O(z7));
  nand2  g091(.a(new_n124_), .b(x27), .O(new_n136_));
  nand2  g092(.a(new_n116_), .b(new_n83_), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n136_), .c(x28), .O(new_n138_));
  nor2   g094(.a(x30), .b(x29), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n138_), .c(x00), .O(new_n140_));
  inv1   g096(.a(x15), .O(new_n141_));
  nand2  g097(.a(x16), .b(x10), .O(new_n142_));
  nand3  g098(.a(x19), .b(x18), .c(x17), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(new_n141_), .O(new_n144_));
  inv1   g100(.a(x16), .O(new_n145_));
  nand2  g101(.a(x19), .b(x18), .O(new_n146_));
  nand2  g102(.a(x17), .b(x11), .O(new_n147_));
  oai21  g103(.a(new_n147_), .b(new_n146_), .c(new_n145_), .O(new_n148_));
  inv1   g104(.a(x12), .O(new_n149_));
  oai21  g105(.a(new_n146_), .b(new_n149_), .c(new_n85_), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  nand2  g107(.a(x19), .b(x13), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n88_), .O(new_n153_));
  inv1   g109(.a(x14), .O(new_n154_));
  nand2  g110(.a(new_n91_), .b(new_n154_), .O(new_n155_));
  inv1   g111(.a(new_n143_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(x16), .c(x15), .O(new_n157_));
  aoi21  g113(.a(new_n124_), .b(x28), .c(new_n123_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n157_), .c(new_n155_), .d(new_n153_), .O(new_n159_));
  nor2   g115(.a(new_n159_), .b(new_n151_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n148_), .c(new_n144_), .d(new_n140_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x32), .O(new_n162_));
  inv1   g118(.a(x32), .O(new_n163_));
  nor2   g119(.a(x28), .b(x27), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n116_), .c(x29), .O(new_n166_));
  nor2   g122(.a(new_n164_), .b(new_n106_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n116_), .c(new_n166_), .O(new_n168_));
  nand3  g124(.a(x19), .b(new_n88_), .c(x13), .O(new_n169_));
  nand3  g125(.a(new_n91_), .b(x18), .c(x14), .O(new_n170_));
  aoi21  g126(.a(new_n170_), .b(new_n169_), .c(new_n85_), .O(new_n171_));
  nand4  g127(.a(x19), .b(x18), .c(new_n85_), .d(x12), .O(new_n172_));
  inv1   g128(.a(new_n172_), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n171_), .c(x16), .O(new_n174_));
  nand3  g130(.a(new_n156_), .b(new_n145_), .c(x11), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(x15), .O(new_n177_));
  nand4  g133(.a(new_n156_), .b(x16), .c(new_n141_), .d(x10), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n177_), .O(new_n179_));
  nand4  g135(.a(new_n179_), .b(new_n168_), .c(new_n163_), .d(x31), .O(new_n180_));
  nand3  g136(.a(new_n180_), .b(new_n162_), .c(x00), .O(new_n181_));
  and2   g137(.a(new_n181_), .b(new_n82_), .O(z8));
  inv1   g138(.a(x33), .O(new_n183_));
  nand4  g139(.a(new_n179_), .b(new_n183_), .c(x31), .d(x30), .O(new_n184_));
  nand2  g140(.a(x33), .b(new_n123_), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n184_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n165_), .c(x29), .O(new_n187_));
  inv1   g143(.a(new_n167_), .O(new_n188_));
  nand4  g144(.a(new_n179_), .b(new_n183_), .c(new_n123_), .d(new_n116_), .O(new_n189_));
  nand2  g145(.a(x33), .b(x31), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(new_n189_), .O(new_n191_));
  inv1   g147(.a(new_n144_), .O(new_n192_));
  nand2  g148(.a(new_n157_), .b(new_n155_), .O(new_n193_));
  xor2a  g149(.a(x31), .b(x30), .O(new_n194_));
  nor2   g150(.a(new_n194_), .b(new_n193_), .O(new_n195_));
  nand4  g151(.a(new_n195_), .b(new_n153_), .c(new_n150_), .d(new_n148_), .O(new_n196_));
  oai21  g152(.a(new_n196_), .b(new_n192_), .c(x33), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(x00), .O(new_n198_));
  aoi21  g154(.a(new_n191_), .b(new_n188_), .c(new_n198_), .O(new_n199_));
  aoi21  g155(.a(new_n199_), .b(new_n187_), .c(x26), .O(z9));
endmodule


