// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n109_, new_n110_, new_n111_, new_n113_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x27), .O(new_n45_));
  inv1   g001(.a(x29), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand2  g017(.a(x03), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n61_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n48_), .c(new_n59_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x25), .O(new_n81_));
  inv1   g037(.a(x26), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x17), .O(new_n86_));
  inv1   g042(.a(x22), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  inv1   g044(.a(x18), .O(new_n89_));
  inv1   g045(.a(x23), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nor2   g047(.a(x24), .b(x19), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n91_), .c(new_n88_), .d(new_n85_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n62_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n45_), .c(new_n82_), .O(new_n96_));
  nor3   g052(.a(new_n96_), .b(new_n81_), .c(new_n61_), .O(z3));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n95_), .c(new_n82_), .d(x25), .O(new_n99_));
  oai21  g055(.a(new_n99_), .b(new_n61_), .c(new_n48_), .O(z4));
  nand3  g056(.a(new_n46_), .b(x28), .c(x27), .O(new_n101_));
  oai21  g057(.a(new_n46_), .b(x27), .c(new_n101_), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n95_), .c(new_n82_), .O(new_n103_));
  nor3   g059(.a(new_n103_), .b(new_n81_), .c(new_n61_), .O(z5));
  nand3  g060(.a(new_n95_), .b(new_n48_), .c(x30), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n82_), .c(x25), .d(x01), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(z6));
  nand3  g064(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n109_));
  nand3  g065(.a(new_n62_), .b(x25), .c(x01), .O(new_n110_));
  nor4   g066(.a(new_n110_), .b(new_n47_), .c(x31), .d(x26), .O(new_n111_));
  oai21  g067(.a(new_n109_), .b(new_n92_), .c(new_n111_), .O(z7));
  inv1   g068(.a(x00), .O(new_n113_));
  inv1   g069(.a(x28), .O(new_n114_));
  nor2   g070(.a(new_n46_), .b(new_n114_), .O(new_n115_));
  inv1   g071(.a(x32), .O(new_n116_));
  nand3  g072(.a(x19), .b(new_n89_), .c(x13), .O(new_n117_));
  inv1   g073(.a(x19), .O(new_n118_));
  nand3  g074(.a(new_n118_), .b(x18), .c(x14), .O(new_n119_));
  aoi21  g075(.a(new_n119_), .b(new_n117_), .c(new_n86_), .O(new_n120_));
  nand4  g076(.a(x19), .b(x18), .c(new_n86_), .d(x12), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n120_), .c(x16), .O(new_n123_));
  inv1   g079(.a(x16), .O(new_n124_));
  nand3  g080(.a(x19), .b(x18), .c(x17), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n124_), .c(x11), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x15), .O(new_n129_));
  inv1   g085(.a(x15), .O(new_n130_));
  nand4  g086(.a(new_n126_), .b(x16), .c(new_n130_), .d(x10), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n129_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n116_), .c(x31), .d(x30), .O(new_n133_));
  inv1   g089(.a(x30), .O(new_n134_));
  nand2  g090(.a(x32), .b(new_n134_), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n133_), .c(new_n115_), .O(new_n136_));
  nand3  g092(.a(x28), .b(new_n118_), .c(x14), .O(new_n137_));
  nand4  g093(.a(new_n116_), .b(x31), .c(new_n134_), .d(x29), .O(new_n138_));
  oai22  g094(.a(new_n138_), .b(new_n137_), .c(new_n116_), .d(new_n118_), .O(new_n139_));
  nand4  g095(.a(x28), .b(x19), .c(new_n89_), .d(x13), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n138_), .O(new_n141_));
  aoi21  g097(.a(new_n139_), .b(x18), .c(new_n141_), .O(new_n142_));
  nand3  g098(.a(new_n116_), .b(x31), .c(new_n134_), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n115_), .O(new_n145_));
  oai22  g101(.a(new_n145_), .b(new_n121_), .c(new_n142_), .d(new_n86_), .O(new_n146_));
  nor2   g102(.a(new_n145_), .b(new_n127_), .O(new_n147_));
  aoi21  g103(.a(new_n146_), .b(x16), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(x32), .b(x30), .O(new_n149_));
  nor2   g105(.a(new_n118_), .b(new_n89_), .O(new_n150_));
  nand4  g106(.a(x17), .b(x16), .c(new_n130_), .d(x10), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(new_n144_), .c(new_n150_), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n149_), .c(new_n46_), .O(new_n154_));
  nand2  g110(.a(x16), .b(x10), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n125_), .c(new_n130_), .O(new_n156_));
  nand3  g112(.a(new_n150_), .b(x17), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n124_), .O(new_n158_));
  nand2  g114(.a(new_n150_), .b(x12), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n86_), .O(new_n160_));
  aoi21  g116(.a(x19), .b(x13), .c(x18), .O(new_n161_));
  oai21  g117(.a(x19), .b(x14), .c(x31), .O(new_n162_));
  nor2   g118(.a(new_n162_), .b(new_n161_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n160_), .c(new_n158_), .d(new_n156_), .O(new_n164_));
  aoi22  g120(.a(new_n164_), .b(x32), .c(new_n154_), .d(x28), .O(new_n165_));
  oai21  g121(.a(new_n148_), .b(new_n130_), .c(new_n165_), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n136_), .c(new_n82_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n113_), .c(new_n48_), .O(z8));
  nand2  g124(.a(new_n118_), .b(x14), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(x18), .c(x17), .d(x16), .O(new_n170_));
  nor2   g126(.a(new_n170_), .b(new_n130_), .O(new_n171_));
  nand2  g127(.a(x16), .b(x15), .O(new_n172_));
  nand2  g128(.a(x18), .b(x17), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n172_), .c(new_n118_), .O(new_n174_));
  oai21  g130(.a(new_n173_), .b(new_n155_), .c(new_n130_), .O(new_n175_));
  inv1   g131(.a(x11), .O(new_n176_));
  oai21  g132(.a(new_n173_), .b(new_n176_), .c(new_n124_), .O(new_n177_));
  aoi21  g133(.a(x18), .b(x12), .c(x17), .O(new_n178_));
  nor2   g134(.a(x18), .b(x13), .O(new_n179_));
  nor2   g135(.a(new_n179_), .b(new_n178_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n177_), .c(new_n175_), .d(new_n174_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n171_), .c(new_n48_), .O(new_n182_));
  inv1   g138(.a(x31), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n134_), .O(new_n184_));
  nand2  g140(.a(x30), .b(new_n114_), .O(new_n185_));
  nand2  g141(.a(x31), .b(new_n134_), .O(new_n186_));
  nand3  g142(.a(new_n183_), .b(x29), .c(x28), .O(new_n187_));
  nand3  g143(.a(new_n187_), .b(new_n186_), .c(new_n185_), .O(new_n188_));
  aoi22  g144(.a(new_n188_), .b(new_n45_), .c(new_n184_), .d(new_n46_), .O(new_n189_));
  nand2  g145(.a(new_n189_), .b(new_n182_), .O(new_n190_));
  nand2  g146(.a(new_n190_), .b(x33), .O(new_n191_));
  nand3  g147(.a(new_n115_), .b(x31), .c(x30), .O(new_n192_));
  oai21  g148(.a(new_n184_), .b(x28), .c(new_n192_), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(new_n45_), .O(new_n194_));
  oai21  g150(.a(new_n184_), .b(x29), .c(new_n194_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(new_n132_), .O(new_n196_));
  oai21  g152(.a(new_n196_), .b(x33), .c(new_n191_), .O(new_n197_));
  nand3  g153(.a(new_n197_), .b(new_n82_), .c(x00), .O(new_n198_));
  inv1   g154(.a(new_n198_), .O(z9));
endmodule


