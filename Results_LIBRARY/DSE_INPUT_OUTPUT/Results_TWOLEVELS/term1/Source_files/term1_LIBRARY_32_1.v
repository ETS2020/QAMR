// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_;
  inv1   g000(.a(x32), .O(z0));
  inv1   g001(.a(x33), .O(new_n46_));
  nand3  g002(.a(new_n46_), .b(x30), .c(x03), .O(new_n47_));
  oai21  g003(.a(x32), .b(x03), .c(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nand3  g007(.a(new_n46_), .b(x30), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(x32), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g010(.a(new_n46_), .b(z0), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(z1));
  inv1   g012(.a(x08), .O(new_n57_));
  inv1   g013(.a(x30), .O(new_n58_));
  nand2  g014(.a(x32), .b(new_n58_), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nand2  g017(.a(x03), .b(x02), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n50_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n50_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n50_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n60_), .O(new_n70_));
  xnor2a g026(.a(x07), .b(x04), .O(new_n71_));
  inv1   g027(.a(new_n71_), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n51_), .d(new_n50_), .O(new_n74_));
  inv1   g030(.a(x05), .O(new_n75_));
  inv1   g031(.a(x06), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n74_), .c(new_n62_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n61_), .O(new_n79_));
  oai21  g035(.a(new_n73_), .b(new_n61_), .c(new_n79_), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n72_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n70_), .O(new_n82_));
  oai22  g038(.a(x06), .b(new_n51_), .c(x05), .d(new_n50_), .O(new_n83_));
  nand3  g039(.a(new_n83_), .b(x30), .c(new_n61_), .O(new_n84_));
  nand3  g040(.a(new_n75_), .b(new_n51_), .c(x02), .O(new_n85_));
  nand3  g041(.a(new_n76_), .b(x03), .c(new_n50_), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(z0), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n84_), .c(new_n71_), .O(new_n89_));
  aoi21  g045(.a(new_n82_), .b(new_n59_), .c(new_n89_), .O(new_n90_));
  nor3   g046(.a(new_n90_), .b(x09), .c(new_n57_), .O(z2));
  inv1   g047(.a(x26), .O(new_n92_));
  inv1   g048(.a(x27), .O(new_n93_));
  inv1   g049(.a(new_n62_), .O(new_n94_));
  oai22  g050(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n95_));
  inv1   g051(.a(x17), .O(new_n96_));
  inv1   g052(.a(x22), .O(new_n97_));
  nand2  g053(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  inv1   g054(.a(x18), .O(new_n99_));
  inv1   g055(.a(x23), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  inv1   g057(.a(x19), .O(new_n102_));
  inv1   g058(.a(x24), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n102_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n101_), .c(new_n98_), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n95_), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n94_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(new_n59_), .c(new_n93_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n92_), .c(x25), .d(x01), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(z3));
  xor2a  g067(.a(x28), .b(x27), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n107_), .c(new_n92_), .d(x25), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n61_), .c(new_n59_), .O(z4));
  inv1   g070(.a(new_n106_), .O(new_n115_));
  inv1   g071(.a(x29), .O(new_n116_));
  inv1   g072(.a(x28), .O(new_n117_));
  nor2   g073(.a(new_n117_), .b(new_n93_), .O(new_n118_));
  nand3  g074(.a(new_n116_), .b(x28), .c(x27), .O(new_n119_));
  oai21  g075(.a(new_n118_), .b(new_n116_), .c(new_n119_), .O(new_n120_));
  nand4  g076(.a(new_n120_), .b(new_n115_), .c(new_n62_), .d(new_n92_), .O(new_n121_));
  inv1   g077(.a(new_n121_), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x25), .c(x01), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n59_), .O(z5));
  inv1   g080(.a(x25), .O(new_n125_));
  inv1   g081(.a(new_n118_), .O(new_n126_));
  nand3  g082(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x30), .O(new_n128_));
  nand3  g084(.a(z0), .b(new_n58_), .c(x29), .O(new_n129_));
  oai21  g085(.a(new_n129_), .b(new_n126_), .c(new_n128_), .O(new_n130_));
  nand4  g086(.a(new_n130_), .b(new_n115_), .c(new_n62_), .d(new_n92_), .O(new_n131_));
  nor3   g087(.a(new_n131_), .b(new_n125_), .c(new_n61_), .O(z6));
  oai21  g088(.a(x32), .b(x31), .c(new_n58_), .O(new_n133_));
  nand2  g089(.a(new_n127_), .b(x31), .O(new_n134_));
  inv1   g090(.a(new_n95_), .O(new_n135_));
  nand4  g091(.a(new_n104_), .b(new_n101_), .c(new_n98_), .d(new_n135_), .O(new_n136_));
  nor2   g092(.a(new_n94_), .b(new_n61_), .O(new_n137_));
  inv1   g093(.a(x31), .O(new_n138_));
  nand4  g094(.a(new_n118_), .b(new_n138_), .c(x30), .d(x29), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n137_), .c(new_n92_), .d(x25), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n136_), .c(new_n134_), .d(new_n133_), .O(z7));
  nand2  g098(.a(new_n117_), .b(new_n93_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n58_), .c(x29), .O(new_n144_));
  nand2  g100(.a(new_n143_), .b(x29), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x30), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n144_), .O(new_n147_));
  nand3  g103(.a(x19), .b(new_n99_), .c(x13), .O(new_n148_));
  nand3  g104(.a(new_n102_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n148_), .c(new_n96_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n96_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  inv1   g109(.a(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  inv1   g115(.a(x15), .O(new_n160_));
  nand4  g116(.a(new_n156_), .b(x16), .c(new_n160_), .d(x10), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n159_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n147_), .c(z0), .d(x31), .O(new_n163_));
  oai21  g119(.a(x28), .b(x27), .c(x29), .O(new_n164_));
  and2   g120(.a(x16), .b(x10), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n156_), .c(x15), .O(new_n166_));
  inv1   g122(.a(new_n166_), .O(new_n167_));
  nand2  g123(.a(x19), .b(x18), .O(new_n168_));
  nand2  g124(.a(x17), .b(x11), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n168_), .c(new_n154_), .O(new_n170_));
  inv1   g126(.a(new_n168_), .O(new_n171_));
  aoi21  g127(.a(new_n171_), .b(x12), .c(x17), .O(new_n172_));
  nand2  g128(.a(x19), .b(x13), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n99_), .O(new_n174_));
  nor2   g130(.a(x19), .b(x14), .O(new_n175_));
  inv1   g131(.a(new_n175_), .O(new_n176_));
  nor2   g132(.a(new_n154_), .b(new_n160_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n156_), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(new_n176_), .c(new_n174_), .d(x31), .O(new_n179_));
  nor2   g135(.a(new_n179_), .b(new_n172_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n170_), .c(new_n167_), .d(new_n164_), .O(new_n181_));
  nand3  g137(.a(new_n181_), .b(x32), .c(x30), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n163_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n92_), .c(x00), .O(new_n184_));
  inv1   g140(.a(new_n184_), .O(z8));
  inv1   g141(.a(new_n172_), .O(new_n186_));
  aoi21  g142(.a(new_n177_), .b(new_n156_), .c(new_n175_), .O(new_n187_));
  nand4  g143(.a(new_n187_), .b(new_n174_), .c(new_n186_), .d(new_n170_), .O(new_n188_));
  oai21  g144(.a(new_n188_), .b(new_n166_), .c(new_n59_), .O(new_n189_));
  nand3  g145(.a(new_n143_), .b(z0), .c(x29), .O(new_n190_));
  aoi21  g146(.a(new_n190_), .b(new_n58_), .c(x31), .O(new_n191_));
  nand3  g147(.a(z0), .b(x31), .c(new_n58_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n146_), .O(new_n193_));
  nor2   g149(.a(new_n193_), .b(new_n191_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n189_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x33), .O(new_n196_));
  nand4  g152(.a(new_n143_), .b(x31), .c(x30), .d(x29), .O(new_n197_));
  nand4  g153(.a(new_n145_), .b(z0), .c(new_n138_), .d(new_n58_), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  nand3  g155(.a(new_n199_), .b(new_n162_), .c(new_n46_), .O(new_n200_));
  nand2  g156(.a(new_n200_), .b(new_n196_), .O(new_n201_));
  nand3  g157(.a(new_n201_), .b(new_n92_), .c(x00), .O(new_n202_));
  inv1   g158(.a(new_n202_), .O(z9));
endmodule


