// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  nor2   g000(.a(x32), .b(x30), .O(z0));
  inv1   g001(.a(x03), .O(new_n46_));
  nand2  g002(.a(z0), .b(new_n46_), .O(new_n47_));
  inv1   g003(.a(x33), .O(new_n48_));
  nand3  g004(.a(new_n48_), .b(x32), .c(x03), .O(new_n49_));
  nand2  g005(.a(new_n49_), .b(new_n47_), .O(new_n50_));
  nand2  g006(.a(new_n50_), .b(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nand2  g008(.a(z0), .b(x03), .O(new_n53_));
  nand3  g009(.a(new_n48_), .b(x32), .c(new_n46_), .O(new_n54_));
  nand2  g010(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand2  g011(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  inv1   g012(.a(x30), .O(new_n57_));
  inv1   g013(.a(x32), .O(new_n58_));
  nand3  g014(.a(new_n48_), .b(new_n58_), .c(new_n57_), .O(new_n59_));
  nand3  g015(.a(new_n59_), .b(new_n56_), .c(new_n51_), .O(z1));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n46_), .b(new_n52_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nand2  g020(.a(new_n46_), .b(new_n52_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(new_n66_));
  nand2  g022(.a(x05), .b(new_n46_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x06), .c(new_n52_), .O(new_n68_));
  nand2  g024(.a(x06), .b(new_n52_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(x05), .c(new_n46_), .O(new_n70_));
  aoi21  g026(.a(new_n70_), .b(new_n68_), .c(x01), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n66_), .c(new_n61_), .O(new_n72_));
  xor2a  g028(.a(x07), .b(x04), .O(new_n73_));
  xnor2a g029(.a(x03), .b(x02), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n62_), .O(new_n75_));
  nand2  g031(.a(new_n69_), .b(new_n67_), .O(new_n76_));
  nand4  g032(.a(x06), .b(x05), .c(new_n46_), .d(new_n52_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n76_), .c(x01), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n75_), .c(new_n73_), .O(new_n79_));
  aoi21  g035(.a(new_n79_), .b(new_n72_), .c(x09), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(x08), .O(new_n81_));
  nand2  g037(.a(new_n58_), .b(x30), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(new_n81_), .O(z2));
  inv1   g039(.a(x26), .O(new_n84_));
  inv1   g040(.a(x27), .O(new_n85_));
  oai22  g041(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n86_));
  nor2   g042(.a(x22), .b(x17), .O(new_n87_));
  nor2   g043(.a(x23), .b(x18), .O(new_n88_));
  nor2   g044(.a(x24), .b(x19), .O(new_n89_));
  nor4   g045(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n63_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n82_), .c(new_n85_), .O(new_n92_));
  inv1   g048(.a(new_n92_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n84_), .c(x25), .d(x01), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z3));
  xor2a  g051(.a(x28), .b(x27), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n91_), .c(new_n84_), .d(x25), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n62_), .c(new_n82_), .O(z4));
  inv1   g054(.a(x29), .O(new_n99_));
  nand4  g055(.a(new_n82_), .b(new_n99_), .c(x28), .d(x27), .O(new_n100_));
  nand2  g056(.a(new_n58_), .b(x30), .O(new_n101_));
  nand2  g057(.a(x28), .b(x27), .O(new_n102_));
  nand3  g058(.a(new_n102_), .b(new_n101_), .c(x29), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n100_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n91_), .c(new_n84_), .d(x25), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n62_), .O(z5));
  nand3  g062(.a(new_n102_), .b(x32), .c(x30), .O(new_n107_));
  nand3  g063(.a(new_n57_), .b(x28), .c(x27), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(x29), .O(new_n110_));
  nand3  g066(.a(x32), .b(x30), .c(new_n99_), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n110_), .c(new_n90_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n64_), .c(new_n84_), .d(x25), .O(new_n113_));
  nor2   g069(.a(new_n113_), .b(new_n62_), .O(z6));
  nor4   g070(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n115_));
  nand4  g071(.a(new_n64_), .b(new_n84_), .c(x25), .d(x01), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n115_), .c(new_n82_), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  nand3  g074(.a(x29), .b(x28), .c(x27), .O(new_n119_));
  nand2  g075(.a(new_n119_), .b(x32), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(x30), .c(new_n118_), .O(new_n121_));
  nor4   g077(.a(new_n119_), .b(new_n58_), .c(x31), .d(new_n57_), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n117_), .O(z7));
  inv1   g080(.a(x00), .O(new_n125_));
  nor2   g081(.a(x28), .b(x27), .O(new_n126_));
  inv1   g082(.a(x17), .O(new_n127_));
  inv1   g083(.a(x18), .O(new_n128_));
  nand3  g084(.a(x19), .b(new_n128_), .c(x13), .O(new_n129_));
  inv1   g085(.a(x19), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(x18), .c(x14), .O(new_n131_));
  aoi21  g087(.a(new_n131_), .b(new_n129_), .c(new_n127_), .O(new_n132_));
  nand4  g088(.a(x19), .b(x18), .c(new_n127_), .d(x12), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n132_), .c(x16), .O(new_n135_));
  inv1   g091(.a(x16), .O(new_n136_));
  nand3  g092(.a(x19), .b(x18), .c(x17), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(new_n136_), .c(x11), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(x15), .O(new_n141_));
  inv1   g097(.a(x15), .O(new_n142_));
  nand4  g098(.a(new_n138_), .b(x16), .c(new_n142_), .d(x10), .O(new_n143_));
  aoi21  g099(.a(new_n143_), .b(new_n141_), .c(new_n126_), .O(new_n144_));
  nand4  g100(.a(new_n144_), .b(new_n58_), .c(x31), .d(new_n57_), .O(new_n145_));
  inv1   g101(.a(x28), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(new_n85_), .O(new_n147_));
  nand3  g103(.a(new_n147_), .b(x32), .c(x30), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n145_), .c(new_n99_), .O(new_n149_));
  inv1   g105(.a(new_n126_), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(x29), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n57_), .O(new_n152_));
  and2   g108(.a(x16), .b(x10), .O(new_n153_));
  aoi21  g109(.a(new_n153_), .b(new_n138_), .c(x15), .O(new_n154_));
  nand2  g110(.a(x19), .b(x18), .O(new_n155_));
  nand2  g111(.a(x17), .b(x11), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n155_), .c(new_n136_), .O(new_n157_));
  inv1   g113(.a(x12), .O(new_n158_));
  oai21  g114(.a(new_n155_), .b(new_n158_), .c(new_n127_), .O(new_n159_));
  nand2  g115(.a(x19), .b(x13), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n128_), .O(new_n161_));
  nor2   g117(.a(x19), .b(x14), .O(new_n162_));
  nor3   g118(.a(new_n137_), .b(new_n136_), .c(new_n142_), .O(new_n163_));
  nor3   g119(.a(new_n163_), .b(new_n162_), .c(new_n118_), .O(new_n164_));
  nand4  g120(.a(new_n164_), .b(new_n161_), .c(new_n159_), .d(new_n157_), .O(new_n165_));
  nor2   g121(.a(new_n165_), .b(new_n154_), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n152_), .c(new_n58_), .O(new_n167_));
  oai21  g123(.a(new_n167_), .b(new_n149_), .c(new_n84_), .O(new_n168_));
  nor2   g124(.a(new_n168_), .b(new_n125_), .O(z8));
  nand2  g125(.a(new_n143_), .b(new_n141_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(new_n48_), .c(x31), .d(x30), .O(new_n171_));
  nand2  g127(.a(x33), .b(new_n118_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n150_), .c(x29), .O(new_n174_));
  nand4  g130(.a(new_n170_), .b(new_n48_), .c(new_n118_), .d(new_n57_), .O(new_n175_));
  nand2  g131(.a(x33), .b(x31), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n175_), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n151_), .O(new_n178_));
  xor2a  g134(.a(x31), .b(x30), .O(new_n179_));
  nor3   g135(.a(new_n179_), .b(new_n163_), .c(new_n162_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n161_), .c(new_n159_), .d(new_n157_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n154_), .c(x33), .O(new_n182_));
  nand3  g138(.a(new_n182_), .b(new_n178_), .c(new_n174_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n84_), .c(x00), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n82_), .O(z9));
endmodule


