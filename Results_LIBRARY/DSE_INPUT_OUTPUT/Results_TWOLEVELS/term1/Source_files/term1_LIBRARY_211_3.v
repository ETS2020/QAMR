// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_;
  inv1   g000(.a(x00), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  nor2   g015(.a(new_n51_), .b(new_n54_), .O(new_n60_));
  nor2   g016(.a(x03), .b(x02), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  inv1   g018(.a(x01), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n51_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n54_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n54_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n51_), .O(new_n67_));
  nand2  g023(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n63_), .O(new_n69_));
  nand2  g025(.a(new_n69_), .b(new_n62_), .O(new_n70_));
  nand3  g026(.a(new_n70_), .b(new_n59_), .c(new_n48_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xor2a  g028(.a(x03), .b(x02), .O(new_n73_));
  nand2  g029(.a(new_n73_), .b(x01), .O(new_n74_));
  and2   g030(.a(x06), .b(x05), .O(new_n75_));
  aoi22  g031(.a(new_n75_), .b(new_n61_), .c(new_n66_), .d(new_n64_), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(x01), .c(new_n74_), .O(new_n77_));
  nand3  g033(.a(new_n77_), .b(new_n72_), .c(new_n48_), .O(new_n78_));
  aoi21  g034(.a(new_n78_), .b(new_n71_), .c(x09), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x08), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z2));
  inv1   g037(.a(x25), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  oai22  g039(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n84_));
  inv1   g040(.a(x17), .O(new_n85_));
  inv1   g041(.a(x22), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x18), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  inv1   g047(.a(x24), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand3  g049(.a(new_n93_), .b(new_n90_), .c(new_n87_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n84_), .O(new_n95_));
  nor2   g051(.a(new_n95_), .b(new_n60_), .O(new_n96_));
  nand3  g052(.a(new_n96_), .b(new_n83_), .c(new_n46_), .O(new_n97_));
  nor3   g053(.a(new_n97_), .b(new_n82_), .c(new_n63_), .O(z3));
  xor2a  g054(.a(x28), .b(x27), .O(new_n99_));
  nand4  g055(.a(new_n99_), .b(new_n96_), .c(new_n46_), .d(x25), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n63_), .c(new_n48_), .O(z4));
  inv1   g057(.a(new_n60_), .O(new_n102_));
  inv1   g058(.a(new_n95_), .O(new_n103_));
  inv1   g059(.a(x29), .O(new_n104_));
  and2   g060(.a(x28), .b(x27), .O(new_n105_));
  nand3  g061(.a(new_n104_), .b(x28), .c(x27), .O(new_n106_));
  oai21  g062(.a(new_n105_), .b(new_n104_), .c(new_n106_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n103_), .c(new_n102_), .d(new_n46_), .O(new_n108_));
  nor3   g064(.a(new_n108_), .b(new_n82_), .c(new_n63_), .O(z5));
  nand3  g065(.a(x29), .b(x28), .c(x27), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x30), .O(new_n111_));
  inv1   g067(.a(x30), .O(new_n112_));
  nand3  g068(.a(new_n105_), .b(new_n112_), .c(x29), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n103_), .c(new_n102_), .d(new_n46_), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(new_n116_));
  nand3  g072(.a(new_n116_), .b(x25), .c(x01), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n48_), .O(z6));
  inv1   g074(.a(x31), .O(new_n119_));
  nor2   g075(.a(new_n112_), .b(new_n104_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n105_), .c(new_n119_), .O(new_n121_));
  inv1   g077(.a(new_n84_), .O(new_n122_));
  nand4  g078(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n122_), .O(new_n123_));
  nor2   g079(.a(new_n60_), .b(new_n63_), .O(new_n124_));
  nand3  g080(.a(new_n119_), .b(x30), .c(x29), .O(new_n125_));
  inv1   g081(.a(new_n125_), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(new_n105_), .c(new_n82_), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n124_), .c(new_n123_), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n121_), .c(new_n46_), .O(new_n129_));
  oai21  g085(.a(new_n46_), .b(x00), .c(new_n129_), .O(z7));
  inv1   g086(.a(x15), .O(new_n131_));
  nand2  g087(.a(new_n91_), .b(x14), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(x18), .c(x17), .d(x16), .O(new_n133_));
  nor2   g089(.a(new_n133_), .b(new_n131_), .O(new_n134_));
  nand2  g090(.a(new_n120_), .b(x27), .O(new_n135_));
  nand2  g091(.a(new_n112_), .b(new_n83_), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n135_), .c(x28), .O(new_n137_));
  nor2   g093(.a(x30), .b(x29), .O(new_n138_));
  oai21  g094(.a(new_n138_), .b(new_n137_), .c(new_n46_), .O(new_n139_));
  nand2  g095(.a(x16), .b(x15), .O(new_n140_));
  nand2  g096(.a(x18), .b(x17), .O(new_n141_));
  oai21  g097(.a(new_n141_), .b(new_n140_), .c(new_n91_), .O(new_n142_));
  nand2  g098(.a(x16), .b(x10), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n141_), .c(new_n131_), .O(new_n144_));
  inv1   g100(.a(x11), .O(new_n145_));
  inv1   g101(.a(x16), .O(new_n146_));
  oai21  g102(.a(new_n141_), .b(new_n145_), .c(new_n146_), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  nand2  g104(.a(x18), .b(x12), .O(new_n149_));
  nand2  g105(.a(new_n149_), .b(new_n85_), .O(new_n150_));
  inv1   g106(.a(x13), .O(new_n151_));
  nand2  g107(.a(new_n88_), .b(new_n151_), .O(new_n152_));
  nand2  g108(.a(new_n120_), .b(x28), .O(new_n153_));
  nand4  g109(.a(new_n153_), .b(new_n152_), .c(new_n150_), .d(x31), .O(new_n154_));
  nor2   g110(.a(new_n154_), .b(new_n148_), .O(new_n155_));
  nand4  g111(.a(new_n155_), .b(new_n144_), .c(new_n142_), .d(new_n139_), .O(new_n156_));
  oai21  g112(.a(new_n156_), .b(new_n134_), .c(x32), .O(new_n157_));
  inv1   g113(.a(x32), .O(new_n158_));
  nor2   g114(.a(x28), .b(x27), .O(new_n159_));
  inv1   g115(.a(new_n159_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(new_n112_), .c(x29), .O(new_n161_));
  nor2   g117(.a(new_n159_), .b(new_n104_), .O(new_n162_));
  oai21  g118(.a(new_n162_), .b(new_n112_), .c(new_n161_), .O(new_n163_));
  nand3  g119(.a(x19), .b(new_n88_), .c(x13), .O(new_n164_));
  nand3  g120(.a(new_n91_), .b(x18), .c(x14), .O(new_n165_));
  aoi21  g121(.a(new_n165_), .b(new_n164_), .c(new_n85_), .O(new_n166_));
  nand4  g122(.a(x19), .b(x18), .c(new_n85_), .d(x12), .O(new_n167_));
  inv1   g123(.a(new_n167_), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n166_), .c(x16), .O(new_n169_));
  nand3  g125(.a(x19), .b(x18), .c(x17), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n146_), .c(x11), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n169_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x15), .O(new_n174_));
  nand4  g130(.a(new_n171_), .b(x16), .c(new_n131_), .d(x10), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n163_), .c(new_n158_), .d(x31), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n157_), .c(new_n46_), .O(new_n178_));
  and2   g134(.a(new_n178_), .b(x00), .O(z8));
  inv1   g135(.a(x33), .O(new_n180_));
  nand4  g136(.a(new_n176_), .b(new_n180_), .c(x31), .d(x30), .O(new_n181_));
  nand2  g137(.a(x33), .b(new_n119_), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n181_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n160_), .c(x29), .O(new_n184_));
  inv1   g140(.a(new_n162_), .O(new_n185_));
  nand4  g141(.a(new_n176_), .b(new_n180_), .c(new_n119_), .d(new_n112_), .O(new_n186_));
  nand2  g142(.a(x33), .b(x31), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n185_), .O(new_n189_));
  xnor2a g145(.a(x31), .b(x30), .O(new_n190_));
  nand3  g146(.a(new_n190_), .b(new_n152_), .c(new_n150_), .O(new_n191_));
  inv1   g147(.a(new_n191_), .O(new_n192_));
  nand4  g148(.a(new_n192_), .b(new_n147_), .c(new_n144_), .d(new_n142_), .O(new_n193_));
  oai21  g149(.a(new_n193_), .b(new_n134_), .c(x33), .O(new_n194_));
  nand3  g150(.a(new_n194_), .b(new_n189_), .c(new_n184_), .O(new_n195_));
  nand3  g151(.a(new_n195_), .b(new_n46_), .c(x00), .O(new_n196_));
  inv1   g152(.a(new_n196_), .O(z9));
endmodule


