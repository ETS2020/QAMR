// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_;
  nor2   g000(.a(x30), .b(x23), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nor2   g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  inv1   g016(.a(new_n60_), .O(new_n61_));
  nand2  g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  aoi21  g018(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(new_n63_));
  nand2  g019(.a(x05), .b(new_n49_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x05), .c(new_n49_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n65_), .c(x01), .O(new_n68_));
  oai21  g024(.a(new_n68_), .b(new_n63_), .c(new_n58_), .O(new_n69_));
  xor2a  g025(.a(x07), .b(x04), .O(new_n70_));
  xnor2a g026(.a(x03), .b(x02), .O(new_n71_));
  nor2   g027(.a(new_n71_), .b(new_n59_), .O(new_n72_));
  nand2  g028(.a(new_n66_), .b(new_n64_), .O(new_n73_));
  nand4  g029(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n73_), .c(x01), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n72_), .c(new_n70_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n69_), .O(new_n77_));
  nand4  g033(.a(new_n77_), .b(new_n46_), .c(new_n57_), .d(x08), .O(new_n78_));
  inv1   g034(.a(new_n78_), .O(z2));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  inv1   g038(.a(new_n82_), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x19), .O(new_n87_));
  inv1   g043(.a(x24), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n46_), .O(new_n91_));
  inv1   g047(.a(x18), .O(new_n92_));
  inv1   g048(.a(x23), .O(new_n93_));
  nand3  g049(.a(x30), .b(new_n93_), .c(new_n92_), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(new_n91_), .c(new_n60_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n81_), .c(new_n80_), .d(x25), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n59_), .O(z3));
  nand2  g053(.a(x28), .b(new_n81_), .O(new_n98_));
  inv1   g054(.a(x28), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x27), .O(new_n100_));
  nand2  g056(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n90_), .O(new_n102_));
  nand3  g058(.a(x30), .b(new_n99_), .c(x27), .O(new_n103_));
  nand2  g059(.a(new_n103_), .b(new_n98_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(new_n93_), .c(new_n92_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n61_), .c(new_n80_), .d(x25), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n59_), .c(new_n46_), .O(z4));
  nor2   g064(.a(new_n99_), .b(new_n81_), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(new_n110_));
  nand2  g066(.a(new_n93_), .b(new_n92_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n89_), .c(new_n86_), .d(new_n83_), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n110_), .c(x29), .O(new_n113_));
  inv1   g069(.a(x29), .O(new_n114_));
  nand4  g070(.a(new_n94_), .b(new_n89_), .c(new_n86_), .d(new_n83_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n114_), .c(x28), .d(x27), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n113_), .c(new_n60_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n80_), .c(x25), .d(x01), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n46_), .O(z5));
  nand3  g075(.a(x29), .b(x28), .c(x27), .O(new_n120_));
  nand3  g076(.a(new_n120_), .b(new_n93_), .c(new_n92_), .O(new_n121_));
  nand3  g077(.a(x29), .b(x28), .c(x27), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(new_n90_), .O(new_n123_));
  nand2  g079(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x30), .O(new_n125_));
  inv1   g081(.a(x30), .O(new_n126_));
  and2   g082(.a(new_n90_), .b(new_n126_), .O(new_n127_));
  nand4  g083(.a(new_n127_), .b(x29), .c(x28), .d(x27), .O(new_n128_));
  oai21  g084(.a(new_n128_), .b(new_n93_), .c(new_n125_), .O(new_n129_));
  nand4  g085(.a(new_n129_), .b(new_n61_), .c(new_n80_), .d(x25), .O(new_n130_));
  nor2   g086(.a(new_n130_), .b(new_n59_), .O(z6));
  oai21  g087(.a(x31), .b(new_n93_), .c(new_n126_), .O(new_n132_));
  nand2  g088(.a(new_n122_), .b(x31), .O(new_n133_));
  nand4  g089(.a(new_n111_), .b(new_n89_), .c(new_n86_), .d(new_n83_), .O(new_n134_));
  nor2   g090(.a(new_n60_), .b(new_n59_), .O(new_n135_));
  inv1   g091(.a(x31), .O(new_n136_));
  nand4  g092(.a(new_n109_), .b(new_n136_), .c(x30), .d(x29), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n135_), .c(new_n80_), .d(x25), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n134_), .c(new_n133_), .d(new_n132_), .O(z7));
  inv1   g096(.a(x32), .O(new_n141_));
  nor2   g097(.a(x28), .b(x27), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n126_), .c(x29), .O(new_n144_));
  nor2   g100(.a(new_n142_), .b(new_n114_), .O(new_n145_));
  oai21  g101(.a(new_n145_), .b(new_n126_), .c(new_n144_), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n92_), .c(x13), .O(new_n147_));
  nand3  g103(.a(new_n87_), .b(x18), .c(x14), .O(new_n148_));
  aoi21  g104(.a(new_n148_), .b(new_n147_), .c(new_n84_), .O(new_n149_));
  nand4  g105(.a(x19), .b(x18), .c(new_n84_), .d(x12), .O(new_n150_));
  inv1   g106(.a(new_n150_), .O(new_n151_));
  oai21  g107(.a(new_n151_), .b(new_n149_), .c(x16), .O(new_n152_));
  inv1   g108(.a(x16), .O(new_n153_));
  nand2  g109(.a(x19), .b(x18), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x17), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n153_), .c(x11), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n152_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(x15), .O(new_n160_));
  inv1   g116(.a(x15), .O(new_n161_));
  nand4  g117(.a(new_n157_), .b(x16), .c(new_n161_), .d(x10), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n146_), .c(new_n141_), .d(x31), .O(new_n164_));
  nor2   g120(.a(new_n126_), .b(new_n114_), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(x28), .O(new_n166_));
  oai21  g122(.a(x30), .b(x28), .c(new_n166_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n81_), .O(new_n168_));
  nand2  g124(.a(x16), .b(x10), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n156_), .c(new_n161_), .O(new_n170_));
  nand2  g126(.a(x17), .b(x11), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n154_), .c(new_n153_), .O(new_n172_));
  inv1   g128(.a(x12), .O(new_n173_));
  oai21  g129(.a(new_n154_), .b(new_n173_), .c(new_n84_), .O(new_n174_));
  inv1   g130(.a(x13), .O(new_n175_));
  oai21  g131(.a(new_n87_), .b(new_n175_), .c(new_n92_), .O(new_n176_));
  nor2   g132(.a(x19), .b(x14), .O(new_n177_));
  nor2   g133(.a(new_n153_), .b(new_n161_), .O(new_n178_));
  aoi21  g134(.a(new_n178_), .b(new_n157_), .c(new_n177_), .O(new_n179_));
  oai21  g135(.a(x30), .b(x29), .c(x31), .O(new_n180_));
  aoi21  g136(.a(new_n165_), .b(x27), .c(new_n180_), .O(new_n181_));
  nand4  g137(.a(new_n181_), .b(new_n179_), .c(new_n176_), .d(new_n174_), .O(new_n182_));
  inv1   g138(.a(new_n182_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n172_), .c(new_n170_), .d(new_n168_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(x32), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n164_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n80_), .c(x00), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n46_), .O(z8));
  inv1   g144(.a(x33), .O(new_n189_));
  nand4  g145(.a(new_n163_), .b(new_n189_), .c(x31), .d(x30), .O(new_n190_));
  nand2  g146(.a(x33), .b(new_n136_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n190_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n143_), .c(x29), .O(new_n193_));
  inv1   g149(.a(new_n145_), .O(new_n194_));
  nand4  g150(.a(new_n163_), .b(new_n189_), .c(new_n136_), .d(new_n126_), .O(new_n195_));
  nand2  g151(.a(x33), .b(x31), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n195_), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(new_n194_), .O(new_n198_));
  aoi21  g154(.a(new_n87_), .b(x14), .c(new_n92_), .O(new_n199_));
  nand4  g155(.a(new_n199_), .b(x17), .c(x16), .d(x15), .O(new_n200_));
  inv1   g156(.a(new_n178_), .O(new_n201_));
  nand2  g157(.a(x18), .b(x17), .O(new_n202_));
  oai21  g158(.a(new_n202_), .b(new_n201_), .c(new_n87_), .O(new_n203_));
  oai21  g159(.a(new_n202_), .b(new_n169_), .c(new_n161_), .O(new_n204_));
  inv1   g160(.a(x11), .O(new_n205_));
  oai21  g161(.a(new_n202_), .b(new_n205_), .c(new_n153_), .O(new_n206_));
  oai21  g162(.a(new_n92_), .b(new_n173_), .c(new_n84_), .O(new_n207_));
  nand2  g163(.a(new_n92_), .b(new_n175_), .O(new_n208_));
  xnor2a g164(.a(x31), .b(x30), .O(new_n209_));
  nand4  g165(.a(new_n209_), .b(new_n208_), .c(new_n207_), .d(new_n206_), .O(new_n210_));
  inv1   g166(.a(new_n210_), .O(new_n211_));
  nand4  g167(.a(new_n211_), .b(new_n204_), .c(new_n203_), .d(new_n200_), .O(new_n212_));
  nand2  g168(.a(new_n212_), .b(x33), .O(new_n213_));
  nand3  g169(.a(new_n213_), .b(new_n198_), .c(new_n193_), .O(new_n214_));
  nand3  g170(.a(new_n214_), .b(new_n80_), .c(x00), .O(new_n215_));
  nand2  g171(.a(new_n215_), .b(new_n46_), .O(z9));
endmodule


