// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_;
  inv1   g000(.a(x20), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n47_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  inv1   g038(.a(x16), .O(new_n83_));
  inv1   g039(.a(x21), .O(new_n84_));
  nand2  g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  inv1   g041(.a(x17), .O(new_n86_));
  inv1   g042(.a(x22), .O(new_n87_));
  nand2  g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g044(.a(x23), .b(x18), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  nor2   g046(.a(new_n90_), .b(new_n89_), .O(new_n91_));
  nand3  g047(.a(new_n91_), .b(new_n88_), .c(new_n85_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n47_), .O(new_n93_));
  inv1   g049(.a(x15), .O(new_n94_));
  nand3  g050(.a(x30), .b(new_n45_), .c(new_n94_), .O(new_n95_));
  aoi21  g051(.a(new_n95_), .b(new_n93_), .c(new_n61_), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n97_));
  nor2   g053(.a(new_n97_), .b(new_n60_), .O(z3));
  xor2a  g054(.a(x28), .b(x27), .O(new_n99_));
  aoi22  g055(.a(new_n84_), .b(new_n83_), .c(new_n45_), .d(new_n94_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n91_), .c(new_n88_), .O(new_n101_));
  and2   g057(.a(new_n101_), .b(new_n62_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n99_), .c(new_n81_), .d(x25), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n60_), .c(new_n47_), .O(z4));
  inv1   g060(.a(x29), .O(new_n105_));
  inv1   g061(.a(x28), .O(new_n106_));
  nor2   g062(.a(new_n106_), .b(new_n82_), .O(new_n107_));
  nand3  g063(.a(new_n105_), .b(x28), .c(x27), .O(new_n108_));
  oai21  g064(.a(new_n107_), .b(new_n105_), .c(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n101_), .c(new_n62_), .d(new_n81_), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(x25), .c(x01), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n47_), .O(z5));
  nand2  g069(.a(x29), .b(x28), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(new_n45_), .c(new_n94_), .O(new_n115_));
  nand4  g071(.a(new_n92_), .b(new_n46_), .c(x29), .d(x28), .O(new_n116_));
  aoi21  g072(.a(new_n116_), .b(new_n115_), .c(new_n82_), .O(new_n117_));
  nand3  g073(.a(x29), .b(x28), .c(x27), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n92_), .O(new_n119_));
  nand3  g075(.a(new_n82_), .b(new_n45_), .c(new_n94_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n119_), .c(new_n46_), .O(new_n121_));
  or2    g077(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand4  g078(.a(new_n122_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n123_));
  oai21  g079(.a(new_n123_), .b(new_n60_), .c(new_n47_), .O(z6));
  oai21  g080(.a(x31), .b(new_n45_), .c(new_n46_), .O(new_n125_));
  nand2  g081(.a(new_n118_), .b(x31), .O(new_n126_));
  inv1   g082(.a(new_n89_), .O(new_n127_));
  inv1   g083(.a(new_n90_), .O(new_n128_));
  nand4  g084(.a(new_n100_), .b(new_n128_), .c(new_n127_), .d(new_n88_), .O(new_n129_));
  nor2   g085(.a(new_n61_), .b(new_n60_), .O(new_n130_));
  inv1   g086(.a(x31), .O(new_n131_));
  nand4  g087(.a(new_n107_), .b(new_n131_), .c(x30), .d(x29), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n130_), .c(new_n81_), .d(x25), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand4  g090(.a(new_n134_), .b(new_n129_), .c(new_n126_), .d(new_n125_), .O(z7));
  nand2  g091(.a(x16), .b(x10), .O(new_n136_));
  nand3  g092(.a(x19), .b(x18), .c(x17), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n136_), .c(new_n94_), .O(new_n138_));
  nand2  g094(.a(x19), .b(x18), .O(new_n139_));
  nand2  g095(.a(x17), .b(x11), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n139_), .c(new_n83_), .O(new_n141_));
  inv1   g097(.a(x12), .O(new_n142_));
  oai21  g098(.a(new_n139_), .b(new_n142_), .c(new_n86_), .O(new_n143_));
  aoi21  g099(.a(x19), .b(x13), .c(x18), .O(new_n144_));
  nor2   g100(.a(x19), .b(x14), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand4  g102(.a(new_n146_), .b(new_n143_), .c(new_n141_), .d(new_n138_), .O(new_n147_));
  oai21  g103(.a(x30), .b(x20), .c(new_n147_), .O(new_n148_));
  nand2  g104(.a(x16), .b(x15), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n137_), .c(x31), .O(new_n150_));
  nand2  g106(.a(new_n150_), .b(new_n47_), .O(new_n151_));
  nand2  g107(.a(new_n106_), .b(new_n82_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x29), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(new_n46_), .c(x20), .O(new_n154_));
  nand2  g110(.a(new_n106_), .b(new_n82_), .O(new_n155_));
  nand3  g111(.a(new_n155_), .b(x30), .c(x29), .O(new_n156_));
  nand4  g112(.a(new_n156_), .b(new_n154_), .c(new_n151_), .d(new_n148_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x32), .O(new_n158_));
  inv1   g114(.a(x32), .O(new_n159_));
  nand4  g115(.a(new_n152_), .b(new_n46_), .c(x29), .d(x20), .O(new_n160_));
  nand2  g116(.a(new_n153_), .b(x30), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n160_), .O(new_n162_));
  inv1   g118(.a(x18), .O(new_n163_));
  nand3  g119(.a(x19), .b(new_n163_), .c(x13), .O(new_n164_));
  inv1   g120(.a(x19), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(x18), .c(x14), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n164_), .c(new_n86_), .O(new_n167_));
  nand4  g123(.a(x19), .b(x18), .c(new_n86_), .d(x12), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  oai21  g125(.a(new_n169_), .b(new_n167_), .c(x16), .O(new_n170_));
  inv1   g126(.a(new_n137_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n83_), .c(x11), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n170_), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(x15), .O(new_n174_));
  nand4  g130(.a(new_n171_), .b(x16), .c(new_n94_), .d(x10), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n174_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n162_), .c(new_n159_), .d(x31), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n158_), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n81_), .c(x00), .O(new_n179_));
  inv1   g135(.a(new_n179_), .O(z8));
  nand4  g136(.a(x19), .b(x18), .c(x17), .d(x16), .O(new_n181_));
  inv1   g137(.a(new_n181_), .O(new_n182_));
  aoi22  g138(.a(new_n182_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n183_));
  nand2  g139(.a(new_n183_), .b(new_n94_), .O(new_n184_));
  aoi21  g140(.a(new_n165_), .b(x14), .c(new_n163_), .O(new_n185_));
  nand4  g141(.a(new_n185_), .b(x17), .c(x16), .d(x15), .O(new_n186_));
  nand2  g142(.a(x18), .b(x17), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n83_), .c(new_n165_), .O(new_n188_));
  inv1   g144(.a(x11), .O(new_n189_));
  oai21  g145(.a(new_n187_), .b(new_n189_), .c(new_n83_), .O(new_n190_));
  oai21  g146(.a(new_n163_), .b(new_n142_), .c(new_n86_), .O(new_n191_));
  inv1   g147(.a(x13), .O(new_n192_));
  nand2  g148(.a(new_n163_), .b(new_n192_), .O(new_n193_));
  nand3  g149(.a(new_n193_), .b(new_n191_), .c(new_n190_), .O(new_n194_));
  inv1   g150(.a(new_n194_), .O(new_n195_));
  nand3  g151(.a(new_n195_), .b(new_n188_), .c(new_n186_), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n47_), .O(new_n197_));
  nand3  g153(.a(new_n152_), .b(new_n131_), .c(x29), .O(new_n198_));
  oai21  g154(.a(new_n131_), .b(x30), .c(new_n198_), .O(new_n199_));
  nor2   g155(.a(new_n131_), .b(new_n105_), .O(new_n200_));
  aoi21  g156(.a(new_n200_), .b(new_n152_), .c(new_n46_), .O(new_n201_));
  aoi21  g157(.a(new_n199_), .b(x20), .c(new_n201_), .O(new_n202_));
  nand3  g158(.a(new_n202_), .b(new_n197_), .c(new_n184_), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(x33), .O(new_n204_));
  aoi21  g160(.a(new_n172_), .b(new_n170_), .c(new_n94_), .O(new_n205_));
  inv1   g161(.a(new_n175_), .O(new_n206_));
  nand4  g162(.a(new_n153_), .b(new_n131_), .c(new_n46_), .d(x20), .O(new_n207_));
  nand4  g163(.a(new_n152_), .b(x31), .c(x30), .d(x29), .O(new_n208_));
  nand2  g164(.a(new_n208_), .b(new_n207_), .O(new_n209_));
  oai21  g165(.a(new_n206_), .b(new_n205_), .c(new_n209_), .O(new_n210_));
  oai21  g166(.a(new_n210_), .b(x33), .c(new_n204_), .O(new_n211_));
  nand3  g167(.a(new_n211_), .b(new_n81_), .c(x00), .O(new_n212_));
  inv1   g168(.a(new_n212_), .O(z9));
endmodule


