// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x28), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand2  g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n47_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  nor2   g039(.a(x22), .b(x17), .O(new_n84_));
  nor2   g040(.a(x23), .b(x18), .O(new_n85_));
  nor2   g041(.a(x24), .b(x19), .O(new_n86_));
  nor4   g042(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n61_), .O(new_n88_));
  nand3  g044(.a(new_n88_), .b(new_n47_), .c(new_n82_), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n81_), .c(x25), .d(x01), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z3));
  nand2  g048(.a(x28), .b(new_n82_), .O(new_n93_));
  nand2  g049(.a(x30), .b(new_n45_), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n82_), .c(new_n93_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n88_), .c(new_n81_), .d(x25), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n60_), .c(new_n47_), .O(z4));
  inv1   g053(.a(new_n87_), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  oai21  g055(.a(new_n46_), .b(x28), .c(new_n99_), .O(new_n100_));
  nor2   g056(.a(new_n45_), .b(new_n82_), .O(new_n101_));
  oai22  g057(.a(new_n101_), .b(new_n99_), .c(new_n100_), .d(new_n82_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n98_), .c(new_n62_), .d(new_n81_), .O(new_n103_));
  inv1   g059(.a(new_n103_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x25), .c(x01), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n47_), .O(z5));
  xnor2a g062(.a(x30), .b(x29), .O(new_n107_));
  oai21  g063(.a(new_n107_), .b(new_n45_), .c(new_n94_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(x27), .O(new_n109_));
  nand2  g065(.a(x30), .b(new_n82_), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(new_n109_), .c(new_n87_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n60_), .O(z6));
  oai21  g069(.a(x31), .b(new_n46_), .c(new_n45_), .O(new_n114_));
  nand2  g070(.a(x30), .b(x29), .O(new_n115_));
  oai21  g071(.a(new_n115_), .b(new_n82_), .c(x31), .O(new_n116_));
  inv1   g072(.a(new_n85_), .O(new_n117_));
  inv1   g073(.a(new_n86_), .O(new_n118_));
  nor2   g074(.a(new_n84_), .b(new_n83_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(new_n118_), .c(new_n117_), .O(new_n120_));
  nor2   g076(.a(new_n61_), .b(new_n60_), .O(new_n121_));
  inv1   g077(.a(x31), .O(new_n122_));
  nand4  g078(.a(new_n101_), .b(new_n122_), .c(x30), .d(x29), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(new_n121_), .c(new_n81_), .d(x25), .O(new_n124_));
  inv1   g080(.a(new_n124_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n120_), .c(new_n116_), .d(new_n114_), .O(z7));
  nand2  g082(.a(new_n46_), .b(new_n45_), .O(new_n127_));
  inv1   g083(.a(x15), .O(new_n128_));
  nand2  g084(.a(x16), .b(x10), .O(new_n129_));
  nand3  g085(.a(x19), .b(x18), .c(x17), .O(new_n130_));
  oai21  g086(.a(new_n130_), .b(new_n129_), .c(new_n128_), .O(new_n131_));
  inv1   g087(.a(x16), .O(new_n132_));
  nand2  g088(.a(x19), .b(x18), .O(new_n133_));
  nand2  g089(.a(x17), .b(x11), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n133_), .c(new_n132_), .O(new_n135_));
  inv1   g091(.a(x12), .O(new_n136_));
  inv1   g092(.a(x17), .O(new_n137_));
  oai21  g093(.a(new_n133_), .b(new_n136_), .c(new_n137_), .O(new_n138_));
  aoi21  g094(.a(x19), .b(x13), .c(x18), .O(new_n139_));
  nor2   g095(.a(x19), .b(x14), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand4  g097(.a(new_n141_), .b(new_n138_), .c(new_n135_), .d(new_n131_), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n127_), .O(new_n143_));
  inv1   g099(.a(new_n130_), .O(new_n144_));
  nor2   g100(.a(new_n132_), .b(new_n128_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(new_n144_), .O(new_n146_));
  nand2  g102(.a(new_n146_), .b(x31), .O(new_n147_));
  xor2a  g103(.a(x30), .b(x29), .O(new_n148_));
  nand2  g104(.a(new_n45_), .b(x27), .O(new_n149_));
  oai22  g105(.a(new_n149_), .b(new_n115_), .c(new_n148_), .d(new_n45_), .O(new_n150_));
  aoi21  g106(.a(new_n147_), .b(new_n47_), .c(new_n150_), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n143_), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(x32), .O(new_n153_));
  inv1   g109(.a(x32), .O(new_n154_));
  nor2   g110(.a(x28), .b(x27), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n99_), .c(x30), .O(new_n156_));
  nand3  g112(.a(new_n46_), .b(x29), .c(x28), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n156_), .O(new_n158_));
  inv1   g114(.a(x18), .O(new_n159_));
  nand3  g115(.a(x19), .b(new_n159_), .c(x13), .O(new_n160_));
  inv1   g116(.a(x19), .O(new_n161_));
  nand3  g117(.a(new_n161_), .b(x18), .c(x14), .O(new_n162_));
  aoi21  g118(.a(new_n162_), .b(new_n160_), .c(new_n137_), .O(new_n163_));
  nand4  g119(.a(x19), .b(x18), .c(new_n137_), .d(x12), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(new_n165_));
  oai21  g121(.a(new_n165_), .b(new_n163_), .c(x16), .O(new_n166_));
  nand3  g122(.a(new_n144_), .b(new_n132_), .c(x11), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x15), .O(new_n169_));
  nand4  g125(.a(new_n144_), .b(x16), .c(new_n128_), .d(x10), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(new_n158_), .c(new_n154_), .d(x31), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n153_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n81_), .c(x00), .O(new_n174_));
  inv1   g130(.a(new_n174_), .O(z8));
  nand2  g131(.a(x18), .b(x17), .O(new_n176_));
  inv1   g132(.a(new_n176_), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n145_), .c(x19), .O(new_n178_));
  oai21  g134(.a(new_n176_), .b(new_n129_), .c(new_n128_), .O(new_n179_));
  inv1   g135(.a(x11), .O(new_n180_));
  oai21  g136(.a(new_n176_), .b(new_n180_), .c(new_n132_), .O(new_n181_));
  oai21  g137(.a(new_n159_), .b(new_n136_), .c(new_n137_), .O(new_n182_));
  nor2   g138(.a(x18), .b(x13), .O(new_n183_));
  nor2   g139(.a(new_n128_), .b(x14), .O(new_n184_));
  nand3  g140(.a(x18), .b(x17), .c(x16), .O(new_n185_));
  inv1   g141(.a(new_n185_), .O(new_n186_));
  aoi21  g142(.a(new_n186_), .b(new_n184_), .c(new_n183_), .O(new_n187_));
  nand4  g143(.a(new_n187_), .b(new_n182_), .c(new_n181_), .d(new_n179_), .O(new_n188_));
  oai21  g144(.a(new_n188_), .b(new_n178_), .c(new_n127_), .O(new_n189_));
  nor3   g145(.a(new_n49_), .b(new_n161_), .c(new_n159_), .O(new_n190_));
  nand4  g146(.a(new_n190_), .b(x17), .c(x16), .d(x15), .O(new_n191_));
  nand2  g147(.a(new_n122_), .b(new_n99_), .O(new_n192_));
  nand3  g148(.a(new_n192_), .b(new_n46_), .c(x28), .O(new_n193_));
  nand2  g149(.a(x31), .b(x29), .O(new_n194_));
  oai21  g150(.a(new_n194_), .b(new_n155_), .c(x30), .O(new_n195_));
  nand4  g151(.a(new_n195_), .b(new_n193_), .c(new_n191_), .d(new_n189_), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(x33), .O(new_n197_));
  inv1   g153(.a(x33), .O(new_n198_));
  nand3  g154(.a(x31), .b(x30), .c(x29), .O(new_n199_));
  inv1   g155(.a(new_n199_), .O(new_n200_));
  nor3   g156(.a(x31), .b(x30), .c(x29), .O(new_n201_));
  oai21  g157(.a(new_n201_), .b(new_n200_), .c(x28), .O(new_n202_));
  nand4  g158(.a(x31), .b(x30), .c(x29), .d(x27), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(new_n202_), .O(new_n204_));
  nand3  g160(.a(new_n204_), .b(new_n171_), .c(new_n198_), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n197_), .O(new_n206_));
  nand3  g162(.a(new_n206_), .b(new_n81_), .c(x00), .O(new_n207_));
  inv1   g163(.a(new_n207_), .O(z9));
endmodule


