// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_;
  inv1   g000(.a(x30), .O(new_n45_));
  inv1   g001(.a(x33), .O(new_n46_));
  aoi21  g002(.a(new_n46_), .b(new_n45_), .c(x32), .O(z0));
  inv1   g003(.a(x03), .O(new_n48_));
  nand2  g004(.a(new_n46_), .b(x30), .O(new_n49_));
  inv1   g005(.a(x32), .O(new_n50_));
  nand3  g006(.a(x33), .b(new_n50_), .c(new_n48_), .O(new_n51_));
  oai21  g007(.a(new_n49_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand2  g008(.a(new_n52_), .b(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nand3  g010(.a(x33), .b(new_n50_), .c(x03), .O(new_n55_));
  oai21  g011(.a(new_n49_), .b(x03), .c(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n54_), .O(new_n57_));
  nand3  g013(.a(new_n46_), .b(new_n50_), .c(x30), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n57_), .c(new_n53_), .O(z1));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n48_), .b(new_n54_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n48_), .b(new_n54_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n48_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n54_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n54_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n48_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n60_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g030(.a(new_n68_), .b(new_n66_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n48_), .d(new_n54_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  aoi21  g034(.a(new_n78_), .b(new_n71_), .c(x09), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(x08), .O(new_n80_));
  nand2  g036(.a(new_n46_), .b(new_n45_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n80_), .O(z2));
  inv1   g038(.a(x26), .O(new_n83_));
  inv1   g039(.a(x27), .O(new_n84_));
  oai22  g040(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  nor4   g044(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n89_));
  nor2   g045(.a(new_n89_), .b(new_n62_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n81_), .c(new_n84_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n83_), .c(x25), .d(x01), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(z3));
  xor2a  g050(.a(x28), .b(x27), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n90_), .c(new_n83_), .d(x25), .O(new_n96_));
  oai21  g052(.a(new_n96_), .b(new_n61_), .c(new_n81_), .O(z4));
  inv1   g053(.a(new_n89_), .O(new_n98_));
  inv1   g054(.a(x29), .O(new_n99_));
  inv1   g055(.a(x28), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n84_), .O(new_n101_));
  nand3  g057(.a(new_n99_), .b(x28), .c(x27), .O(new_n102_));
  oai21  g058(.a(new_n101_), .b(new_n99_), .c(new_n102_), .O(new_n103_));
  nand4  g059(.a(new_n103_), .b(new_n98_), .c(new_n63_), .d(new_n83_), .O(new_n104_));
  inv1   g060(.a(new_n104_), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x25), .c(x01), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n81_), .O(z5));
  nand3  g063(.a(x29), .b(x28), .c(x27), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand3  g065(.a(new_n101_), .b(new_n45_), .c(x29), .O(new_n110_));
  oai21  g066(.a(new_n109_), .b(new_n45_), .c(new_n110_), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(new_n98_), .c(new_n63_), .d(new_n83_), .O(new_n112_));
  inv1   g068(.a(new_n112_), .O(new_n113_));
  nand3  g069(.a(new_n113_), .b(x25), .c(x01), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n81_), .O(z6));
  nor4   g071(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n85_), .O(new_n116_));
  nand3  g072(.a(new_n83_), .b(x25), .c(x01), .O(new_n117_));
  oai21  g073(.a(new_n117_), .b(new_n116_), .c(new_n81_), .O(new_n118_));
  nand2  g074(.a(new_n46_), .b(new_n45_), .O(new_n119_));
  nand3  g075(.a(new_n119_), .b(x03), .c(x02), .O(new_n120_));
  inv1   g076(.a(x31), .O(new_n121_));
  nand3  g077(.a(new_n101_), .b(new_n121_), .c(x29), .O(new_n122_));
  oai21  g078(.a(new_n109_), .b(new_n121_), .c(new_n122_), .O(new_n123_));
  nor3   g079(.a(new_n46_), .b(new_n121_), .c(x30), .O(new_n124_));
  aoi21  g080(.a(new_n123_), .b(x30), .c(new_n124_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n120_), .c(new_n118_), .O(z7));
  nor2   g082(.a(x28), .b(x27), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand3  g084(.a(new_n128_), .b(new_n45_), .c(x29), .O(new_n129_));
  oai21  g085(.a(new_n127_), .b(new_n99_), .c(x30), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  inv1   g087(.a(x15), .O(new_n132_));
  inv1   g088(.a(x17), .O(new_n133_));
  inv1   g089(.a(x18), .O(new_n134_));
  nand3  g090(.a(x19), .b(new_n134_), .c(x13), .O(new_n135_));
  inv1   g091(.a(x19), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x18), .c(x14), .O(new_n137_));
  aoi21  g093(.a(new_n137_), .b(new_n135_), .c(new_n133_), .O(new_n138_));
  nand4  g094(.a(x19), .b(x18), .c(new_n133_), .d(x12), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  oai21  g096(.a(new_n140_), .b(new_n138_), .c(x16), .O(new_n141_));
  inv1   g097(.a(x16), .O(new_n142_));
  nand2  g098(.a(x19), .b(x18), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x17), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n142_), .c(x11), .O(new_n147_));
  aoi21  g103(.a(new_n147_), .b(new_n141_), .c(new_n132_), .O(new_n148_));
  nand3  g104(.a(x16), .b(new_n132_), .c(x10), .O(new_n149_));
  nor2   g105(.a(new_n149_), .b(new_n145_), .O(new_n150_));
  or2    g106(.a(new_n150_), .b(new_n148_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(new_n131_), .c(new_n50_), .d(x31), .O(new_n152_));
  nor2   g108(.a(new_n45_), .b(new_n99_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x27), .O(new_n154_));
  oai21  g110(.a(x30), .b(x27), .c(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n100_), .O(new_n156_));
  nand2  g112(.a(x16), .b(x10), .O(new_n157_));
  oai21  g113(.a(new_n157_), .b(new_n145_), .c(new_n132_), .O(new_n158_));
  nand2  g114(.a(x17), .b(x11), .O(new_n159_));
  oai21  g115(.a(new_n159_), .b(new_n143_), .c(new_n142_), .O(new_n160_));
  inv1   g116(.a(x12), .O(new_n161_));
  oai21  g117(.a(new_n143_), .b(new_n161_), .c(new_n133_), .O(new_n162_));
  inv1   g118(.a(x13), .O(new_n163_));
  oai21  g119(.a(new_n136_), .b(new_n163_), .c(new_n134_), .O(new_n164_));
  nor2   g120(.a(x19), .b(x14), .O(new_n165_));
  nor2   g121(.a(new_n142_), .b(new_n132_), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n146_), .c(new_n165_), .O(new_n167_));
  oai21  g123(.a(x30), .b(x29), .c(x31), .O(new_n168_));
  aoi21  g124(.a(new_n153_), .b(x28), .c(new_n168_), .O(new_n169_));
  nand4  g125(.a(new_n169_), .b(new_n167_), .c(new_n164_), .d(new_n162_), .O(new_n170_));
  inv1   g126(.a(new_n170_), .O(new_n171_));
  nand4  g127(.a(new_n171_), .b(new_n160_), .c(new_n158_), .d(new_n156_), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(x32), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n152_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n83_), .c(x00), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n81_), .O(z8));
  oai21  g132(.a(new_n150_), .b(new_n148_), .c(new_n46_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n121_), .c(x30), .O(new_n178_));
  nand3  g134(.a(new_n178_), .b(new_n128_), .c(x29), .O(new_n179_));
  aoi21  g135(.a(new_n136_), .b(x14), .c(new_n134_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(x17), .c(x16), .d(x15), .O(new_n181_));
  oai21  g137(.a(new_n127_), .b(new_n99_), .c(x31), .O(new_n182_));
  nand2  g138(.a(x18), .b(x17), .O(new_n183_));
  inv1   g139(.a(new_n183_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n166_), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n136_), .O(new_n186_));
  nand3  g142(.a(new_n184_), .b(x16), .c(x10), .O(new_n187_));
  inv1   g143(.a(x11), .O(new_n188_));
  oai21  g144(.a(new_n183_), .b(new_n188_), .c(new_n142_), .O(new_n189_));
  oai21  g145(.a(new_n134_), .b(new_n161_), .c(new_n133_), .O(new_n190_));
  nand2  g146(.a(new_n134_), .b(new_n163_), .O(new_n191_));
  nand2  g147(.a(new_n121_), .b(x30), .O(new_n192_));
  nand4  g148(.a(new_n192_), .b(new_n191_), .c(new_n190_), .d(new_n189_), .O(new_n193_));
  aoi21  g149(.a(new_n187_), .b(new_n132_), .c(new_n193_), .O(new_n194_));
  nand4  g150(.a(new_n194_), .b(new_n186_), .c(new_n182_), .d(new_n181_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x33), .O(new_n196_));
  nand2  g152(.a(new_n196_), .b(new_n179_), .O(new_n197_));
  nand3  g153(.a(new_n197_), .b(new_n83_), .c(x00), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n81_), .O(z9));
endmodule


