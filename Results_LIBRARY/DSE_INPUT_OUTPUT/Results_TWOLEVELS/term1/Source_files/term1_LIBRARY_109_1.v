// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_;
  inv1   g000(.a(x32), .O(z0));
  inv1   g001(.a(x33), .O(new_n46_));
  nand3  g002(.a(new_n46_), .b(x26), .c(x03), .O(new_n47_));
  oai21  g003(.a(x32), .b(x03), .c(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x02), .O(new_n49_));
  inv1   g005(.a(x02), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nand3  g007(.a(new_n46_), .b(x26), .c(new_n51_), .O(new_n52_));
  oai21  g008(.a(x32), .b(new_n51_), .c(new_n52_), .O(new_n53_));
  nand2  g009(.a(new_n53_), .b(new_n50_), .O(new_n54_));
  nand2  g010(.a(new_n46_), .b(z0), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n54_), .c(new_n49_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  nor2   g013(.a(z0), .b(x26), .O(new_n58_));
  xor2a  g014(.a(x07), .b(x04), .O(new_n59_));
  nor2   g015(.a(new_n51_), .b(new_n50_), .O(new_n60_));
  nor2   g016(.a(x03), .b(x02), .O(new_n61_));
  oai21  g017(.a(new_n61_), .b(new_n60_), .c(x01), .O(new_n62_));
  inv1   g018(.a(x01), .O(new_n63_));
  nand3  g019(.a(x05), .b(new_n51_), .c(x02), .O(new_n64_));
  nand3  g020(.a(x06), .b(x03), .c(new_n50_), .O(new_n65_));
  nand2  g021(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand2  g022(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  aoi21  g023(.a(new_n67_), .b(new_n62_), .c(new_n59_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xor2a  g025(.a(x03), .b(x02), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(x01), .O(new_n71_));
  nor2   g027(.a(new_n51_), .b(x01), .O(new_n72_));
  nor2   g028(.a(x05), .b(x03), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n72_), .c(x02), .O(new_n74_));
  nand3  g030(.a(new_n61_), .b(x06), .c(x05), .O(new_n75_));
  oai21  g031(.a(x06), .b(x05), .c(new_n75_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n63_), .O(new_n77_));
  inv1   g033(.a(x06), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(x03), .c(new_n50_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n77_), .c(new_n74_), .d(new_n71_), .O(new_n80_));
  aoi21  g036(.a(new_n80_), .b(new_n69_), .c(new_n68_), .O(new_n81_));
  inv1   g037(.a(x05), .O(new_n82_));
  nand2  g038(.a(x06), .b(new_n82_), .O(new_n83_));
  nand2  g039(.a(new_n78_), .b(x05), .O(new_n84_));
  oai22  g040(.a(new_n84_), .b(x03), .c(new_n83_), .d(x02), .O(new_n85_));
  nand3  g041(.a(new_n85_), .b(z0), .c(new_n63_), .O(new_n86_));
  nand2  g042(.a(new_n84_), .b(new_n83_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(x26), .c(new_n51_), .d(new_n50_), .O(new_n88_));
  and2   g044(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  oai22  g045(.a(new_n89_), .b(new_n59_), .c(new_n81_), .d(new_n58_), .O(new_n90_));
  nand3  g046(.a(new_n90_), .b(new_n57_), .c(x08), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(z2));
  inv1   g048(.a(x26), .O(new_n93_));
  inv1   g049(.a(x27), .O(new_n94_));
  nor2   g050(.a(x20), .b(x15), .O(new_n95_));
  nor2   g051(.a(x21), .b(x16), .O(new_n96_));
  nor2   g052(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nor2   g053(.a(x22), .b(x17), .O(new_n98_));
  nor2   g054(.a(x23), .b(x18), .O(new_n99_));
  nor2   g055(.a(x24), .b(x19), .O(new_n100_));
  nor3   g056(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n97_), .O(new_n102_));
  inv1   g058(.a(new_n102_), .O(new_n103_));
  nor2   g059(.a(new_n103_), .b(new_n60_), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(z0), .c(new_n94_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n93_), .c(x25), .d(x01), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(z3));
  xor2a  g064(.a(x28), .b(x27), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n104_), .c(x25), .d(x01), .O(new_n110_));
  aoi21  g066(.a(new_n110_), .b(z0), .c(x26), .O(z4));
  inv1   g067(.a(new_n60_), .O(new_n112_));
  inv1   g068(.a(x29), .O(new_n113_));
  and2   g069(.a(x28), .b(x27), .O(new_n114_));
  nand3  g070(.a(new_n113_), .b(x28), .c(x27), .O(new_n115_));
  oai21  g071(.a(new_n114_), .b(new_n113_), .c(new_n115_), .O(new_n116_));
  nand4  g072(.a(new_n116_), .b(new_n102_), .c(new_n112_), .d(z0), .O(new_n117_));
  inv1   g073(.a(new_n117_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n93_), .c(x25), .d(x01), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(z5));
  nand3  g076(.a(x29), .b(x28), .c(x27), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(x30), .O(new_n122_));
  inv1   g078(.a(x30), .O(new_n123_));
  nand3  g079(.a(new_n114_), .b(new_n123_), .c(x29), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(new_n103_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n112_), .c(x25), .d(x01), .O(new_n126_));
  aoi21  g082(.a(new_n126_), .b(z0), .c(x26), .O(z6));
  nand3  g083(.a(new_n114_), .b(x30), .c(x29), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x31), .O(new_n129_));
  inv1   g085(.a(new_n98_), .O(new_n130_));
  inv1   g086(.a(new_n99_), .O(new_n131_));
  inv1   g087(.a(new_n100_), .O(new_n132_));
  nand4  g088(.a(new_n132_), .b(new_n131_), .c(new_n130_), .d(new_n97_), .O(new_n133_));
  nor2   g089(.a(new_n60_), .b(new_n63_), .O(new_n134_));
  inv1   g090(.a(x25), .O(new_n135_));
  inv1   g091(.a(x31), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(x30), .c(x29), .O(new_n137_));
  inv1   g093(.a(new_n137_), .O(new_n138_));
  aoi21  g094(.a(new_n138_), .b(new_n114_), .c(new_n135_), .O(new_n139_));
  nand4  g095(.a(new_n139_), .b(new_n134_), .c(new_n133_), .d(new_n129_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(z0), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(new_n93_), .O(z7));
  nor2   g098(.a(x28), .b(x27), .O(new_n143_));
  inv1   g099(.a(new_n143_), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(new_n123_), .c(x29), .O(new_n145_));
  nor2   g101(.a(new_n143_), .b(new_n113_), .O(new_n146_));
  oai21  g102(.a(new_n146_), .b(new_n123_), .c(new_n145_), .O(new_n147_));
  inv1   g103(.a(x17), .O(new_n148_));
  inv1   g104(.a(x18), .O(new_n149_));
  nand3  g105(.a(x19), .b(new_n149_), .c(x13), .O(new_n150_));
  inv1   g106(.a(x19), .O(new_n151_));
  nand3  g107(.a(new_n151_), .b(x18), .c(x14), .O(new_n152_));
  aoi21  g108(.a(new_n152_), .b(new_n150_), .c(new_n148_), .O(new_n153_));
  nand4  g109(.a(x19), .b(x18), .c(new_n148_), .d(x12), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n153_), .c(x16), .O(new_n156_));
  inv1   g112(.a(x16), .O(new_n157_));
  nand3  g113(.a(x19), .b(x18), .c(x17), .O(new_n158_));
  inv1   g114(.a(new_n158_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n157_), .c(x11), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n156_), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(x15), .O(new_n162_));
  nor2   g118(.a(new_n157_), .b(x15), .O(new_n163_));
  nand3  g119(.a(new_n163_), .b(new_n159_), .c(x10), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n162_), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(new_n147_), .c(x31), .d(x00), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(z0), .c(x26), .O(z8));
  nand4  g123(.a(new_n165_), .b(new_n46_), .c(x31), .d(x30), .O(new_n168_));
  nand2  g124(.a(x33), .b(new_n136_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n168_), .O(new_n170_));
  nand3  g126(.a(new_n170_), .b(new_n144_), .c(x29), .O(new_n171_));
  inv1   g127(.a(new_n146_), .O(new_n172_));
  nand4  g128(.a(new_n165_), .b(new_n46_), .c(new_n136_), .d(new_n123_), .O(new_n173_));
  nand2  g129(.a(x33), .b(x31), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n173_), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n172_), .O(new_n176_));
  and2   g132(.a(x16), .b(x10), .O(new_n177_));
  aoi21  g133(.a(new_n177_), .b(new_n159_), .c(x15), .O(new_n178_));
  nor2   g134(.a(new_n151_), .b(new_n149_), .O(new_n179_));
  nand3  g135(.a(new_n179_), .b(x17), .c(x11), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n157_), .O(new_n181_));
  nand2  g137(.a(new_n179_), .b(x12), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n148_), .O(new_n183_));
  nand2  g139(.a(x19), .b(x13), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n149_), .O(new_n185_));
  nand2  g141(.a(x16), .b(x15), .O(new_n186_));
  oai22  g142(.a(new_n186_), .b(new_n158_), .c(x19), .d(x14), .O(new_n187_));
  xor2a  g143(.a(x31), .b(x30), .O(new_n188_));
  nor2   g144(.a(new_n188_), .b(new_n187_), .O(new_n189_));
  nand4  g145(.a(new_n189_), .b(new_n185_), .c(new_n183_), .d(new_n181_), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n178_), .c(x33), .O(new_n191_));
  nand3  g147(.a(new_n191_), .b(new_n176_), .c(new_n171_), .O(new_n192_));
  nand4  g148(.a(new_n192_), .b(z0), .c(new_n93_), .d(x00), .O(new_n193_));
  inv1   g149(.a(new_n193_), .O(z9));
endmodule


