// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_;
  nor2   g000(.a(x30), .b(x27), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n45_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  nor2   g014(.a(new_n49_), .b(new_n52_), .O(new_n59_));
  nor2   g015(.a(x03), .b(x02), .O(new_n60_));
  oai21  g016(.a(new_n60_), .b(new_n59_), .c(x01), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nand2  g018(.a(x03), .b(new_n62_), .O(new_n63_));
  oai21  g019(.a(x05), .b(x03), .c(new_n63_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x06), .c(new_n52_), .O(new_n65_));
  nand2  g021(.a(x06), .b(new_n52_), .O(new_n66_));
  nand4  g022(.a(new_n66_), .b(x05), .c(new_n49_), .d(new_n62_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(new_n65_), .c(new_n61_), .O(new_n68_));
  nand2  g024(.a(new_n68_), .b(new_n58_), .O(new_n69_));
  xnor2a g025(.a(x07), .b(x04), .O(new_n70_));
  xor2a  g026(.a(x03), .b(x02), .O(new_n71_));
  nand2  g027(.a(new_n64_), .b(x02), .O(new_n72_));
  aoi21  g028(.a(x05), .b(new_n49_), .c(x06), .O(new_n73_));
  nand3  g029(.a(new_n60_), .b(x06), .c(x05), .O(new_n74_));
  inv1   g030(.a(new_n74_), .O(new_n75_));
  oai21  g031(.a(new_n75_), .b(new_n73_), .c(new_n62_), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  aoi21  g033(.a(new_n71_), .b(x01), .c(new_n77_), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n70_), .c(new_n69_), .O(new_n79_));
  nand3  g035(.a(new_n79_), .b(new_n57_), .c(x08), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(new_n46_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x22), .b(x17), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  nor2   g043(.a(x24), .b(x19), .O(new_n88_));
  nor2   g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand3  g045(.a(new_n89_), .b(new_n86_), .c(new_n84_), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n59_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n82_), .c(x25), .d(x01), .O(new_n93_));
  aoi21  g049(.a(new_n93_), .b(x30), .c(x27), .O(z3));
  inv1   g050(.a(x25), .O(new_n95_));
  inv1   g051(.a(x27), .O(new_n96_));
  nand3  g052(.a(x30), .b(x28), .c(new_n96_), .O(new_n97_));
  oai21  g053(.a(x28), .b(new_n96_), .c(new_n97_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n92_), .c(new_n82_), .O(new_n99_));
  nor3   g055(.a(new_n99_), .b(new_n95_), .c(new_n62_), .O(z4));
  inv1   g056(.a(new_n59_), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  oai21  g058(.a(new_n102_), .b(new_n96_), .c(x29), .O(new_n103_));
  inv1   g059(.a(x29), .O(new_n104_));
  nand3  g060(.a(new_n104_), .b(x28), .c(x27), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n90_), .c(new_n101_), .d(new_n82_), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  nand3  g064(.a(new_n108_), .b(x25), .c(x01), .O(new_n109_));
  nand2  g065(.a(new_n109_), .b(new_n46_), .O(z5));
  nand2  g066(.a(x30), .b(new_n96_), .O(new_n111_));
  inv1   g067(.a(x30), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x29), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n96_), .c(new_n111_), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(x28), .O(new_n115_));
  nand2  g071(.a(x29), .b(x28), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(x30), .O(new_n117_));
  aoi21  g073(.a(new_n117_), .b(new_n115_), .c(new_n91_), .O(new_n118_));
  nand4  g074(.a(new_n118_), .b(new_n101_), .c(new_n82_), .d(x25), .O(new_n119_));
  nor2   g075(.a(new_n119_), .b(new_n62_), .O(z6));
  nor4   g076(.a(new_n88_), .b(new_n87_), .c(new_n85_), .d(new_n83_), .O(new_n121_));
  nand4  g077(.a(new_n101_), .b(new_n82_), .c(x25), .d(x01), .O(new_n122_));
  oai21  g078(.a(new_n122_), .b(new_n121_), .c(new_n46_), .O(new_n123_));
  inv1   g079(.a(x31), .O(new_n124_));
  inv1   g080(.a(new_n116_), .O(new_n125_));
  nand3  g081(.a(new_n125_), .b(new_n124_), .c(x30), .O(new_n126_));
  oai21  g082(.a(new_n124_), .b(x30), .c(new_n126_), .O(new_n127_));
  aoi21  g083(.a(new_n125_), .b(x27), .c(new_n124_), .O(new_n128_));
  aoi22  g084(.a(new_n128_), .b(x30), .c(new_n127_), .d(x27), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n123_), .O(z7));
  nor3   g086(.a(new_n112_), .b(new_n104_), .c(x28), .O(new_n131_));
  nor2   g087(.a(x30), .b(x29), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n131_), .c(x27), .O(new_n133_));
  inv1   g089(.a(x15), .O(new_n134_));
  nand2  g090(.a(x16), .b(x10), .O(new_n135_));
  nand3  g091(.a(x19), .b(x18), .c(x17), .O(new_n136_));
  oai21  g092(.a(new_n136_), .b(new_n135_), .c(new_n134_), .O(new_n137_));
  inv1   g093(.a(x16), .O(new_n138_));
  inv1   g094(.a(x18), .O(new_n139_));
  inv1   g095(.a(x19), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n139_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(x17), .c(x11), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n138_), .O(new_n143_));
  aoi21  g099(.a(new_n141_), .b(x12), .c(x17), .O(new_n144_));
  inv1   g100(.a(x13), .O(new_n145_));
  oai21  g101(.a(new_n140_), .b(new_n145_), .c(new_n139_), .O(new_n146_));
  inv1   g102(.a(x14), .O(new_n147_));
  nand2  g103(.a(new_n140_), .b(new_n147_), .O(new_n148_));
  inv1   g104(.a(new_n136_), .O(new_n149_));
  nand3  g105(.a(new_n149_), .b(x16), .c(x15), .O(new_n150_));
  nor2   g106(.a(new_n112_), .b(new_n104_), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(x28), .c(new_n124_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n150_), .c(new_n148_), .d(new_n146_), .O(new_n153_));
  nor2   g109(.a(new_n153_), .b(new_n144_), .O(new_n154_));
  nand4  g110(.a(new_n154_), .b(new_n143_), .c(new_n137_), .d(new_n133_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(x32), .O(new_n156_));
  inv1   g112(.a(x32), .O(new_n157_));
  nand2  g113(.a(new_n102_), .b(new_n96_), .O(new_n158_));
  nand2  g114(.a(x30), .b(new_n104_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n158_), .c(new_n113_), .O(new_n160_));
  inv1   g116(.a(x17), .O(new_n161_));
  nand3  g117(.a(x19), .b(new_n139_), .c(x13), .O(new_n162_));
  nand3  g118(.a(new_n140_), .b(x18), .c(x14), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n162_), .c(new_n161_), .O(new_n164_));
  nand4  g120(.a(x19), .b(x18), .c(new_n161_), .d(x12), .O(new_n165_));
  inv1   g121(.a(new_n165_), .O(new_n166_));
  oai21  g122(.a(new_n166_), .b(new_n164_), .c(x16), .O(new_n167_));
  nand3  g123(.a(new_n149_), .b(new_n138_), .c(x11), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n167_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(x15), .O(new_n170_));
  nand4  g126(.a(new_n149_), .b(x16), .c(new_n134_), .d(x10), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n170_), .O(new_n172_));
  nand4  g128(.a(new_n172_), .b(new_n160_), .c(new_n157_), .d(x31), .O(new_n173_));
  nand2  g129(.a(new_n173_), .b(new_n156_), .O(new_n174_));
  nand3  g130(.a(new_n174_), .b(new_n82_), .c(x00), .O(new_n175_));
  nand2  g131(.a(new_n175_), .b(new_n46_), .O(z8));
  nand2  g132(.a(new_n140_), .b(x14), .O(new_n177_));
  nand4  g133(.a(new_n177_), .b(x18), .c(x17), .d(x16), .O(new_n178_));
  nor2   g134(.a(new_n178_), .b(new_n134_), .O(new_n179_));
  nand2  g135(.a(x16), .b(x15), .O(new_n180_));
  nand2  g136(.a(x18), .b(x17), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n180_), .c(new_n140_), .O(new_n182_));
  oai21  g138(.a(new_n181_), .b(new_n135_), .c(new_n134_), .O(new_n183_));
  inv1   g139(.a(x11), .O(new_n184_));
  oai21  g140(.a(new_n181_), .b(new_n184_), .c(new_n138_), .O(new_n185_));
  nand2  g141(.a(x18), .b(x12), .O(new_n186_));
  aoi22  g142(.a(new_n186_), .b(new_n161_), .c(new_n139_), .d(new_n145_), .O(new_n187_));
  nand4  g143(.a(new_n187_), .b(new_n185_), .c(new_n183_), .d(new_n182_), .O(new_n188_));
  oai21  g144(.a(new_n188_), .b(new_n179_), .c(new_n46_), .O(new_n189_));
  aoi21  g145(.a(new_n124_), .b(new_n104_), .c(x30), .O(new_n190_));
  nand3  g146(.a(new_n158_), .b(x31), .c(x29), .O(new_n191_));
  aoi22  g147(.a(new_n191_), .b(x30), .c(new_n190_), .d(x27), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(new_n189_), .O(new_n193_));
  nand2  g149(.a(new_n193_), .b(x33), .O(new_n194_));
  aoi21  g150(.a(new_n168_), .b(new_n167_), .c(new_n134_), .O(new_n195_));
  inv1   g151(.a(new_n171_), .O(new_n196_));
  nand3  g152(.a(x31), .b(x30), .c(x29), .O(new_n197_));
  inv1   g153(.a(new_n197_), .O(new_n198_));
  nor3   g154(.a(x31), .b(x30), .c(x29), .O(new_n199_));
  oai21  g155(.a(new_n199_), .b(new_n198_), .c(x27), .O(new_n200_));
  nand3  g156(.a(new_n125_), .b(x31), .c(x30), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n200_), .O(new_n202_));
  oai21  g158(.a(new_n196_), .b(new_n195_), .c(new_n202_), .O(new_n203_));
  oai21  g159(.a(new_n203_), .b(x33), .c(new_n194_), .O(new_n204_));
  nand3  g160(.a(new_n204_), .b(new_n82_), .c(x00), .O(new_n205_));
  inv1   g161(.a(new_n205_), .O(z9));
endmodule


