// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_;
  nand2  g000(.a(x30), .b(x24), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nor2   g014(.a(new_n49_), .b(new_n52_), .O(new_n59_));
  inv1   g015(.a(new_n59_), .O(new_n60_));
  nand2  g016(.a(new_n49_), .b(new_n52_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n58_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n49_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n52_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n52_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n49_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n57_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xnor2a g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n58_), .O(new_n71_));
  nand2  g027(.a(new_n65_), .b(new_n63_), .O(new_n72_));
  nand4  g028(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n68_), .c(x09), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(x08), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n45_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  oai22  g036(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n81_));
  inv1   g037(.a(new_n81_), .O(new_n82_));
  nor2   g038(.a(x22), .b(x17), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x23), .b(x18), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x24), .b(x19), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nand4  g044(.a(new_n88_), .b(new_n86_), .c(new_n84_), .d(new_n82_), .O(new_n89_));
  and2   g045(.a(new_n89_), .b(new_n60_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n80_), .c(new_n79_), .d(x25), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n58_), .c(new_n45_), .O(z3));
  nand2  g048(.a(x28), .b(new_n80_), .O(new_n93_));
  inv1   g049(.a(x28), .O(new_n94_));
  nand2  g050(.a(new_n94_), .b(x27), .O(new_n95_));
  nand2  g051(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  inv1   g052(.a(x30), .O(new_n97_));
  nand3  g053(.a(new_n86_), .b(new_n84_), .c(new_n82_), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(new_n88_), .O(new_n100_));
  nand3  g056(.a(new_n100_), .b(new_n96_), .c(new_n60_), .O(new_n101_));
  inv1   g057(.a(x24), .O(new_n102_));
  nand2  g058(.a(x30), .b(new_n94_), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n80_), .c(new_n93_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n98_), .c(new_n60_), .d(new_n102_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n101_), .O(new_n106_));
  nand4  g062(.a(new_n106_), .b(new_n79_), .c(x25), .d(x01), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(z4));
  inv1   g064(.a(x29), .O(new_n109_));
  nor2   g065(.a(new_n94_), .b(new_n80_), .O(new_n110_));
  nand3  g066(.a(new_n109_), .b(x28), .c(x27), .O(new_n111_));
  oai21  g067(.a(new_n110_), .b(new_n109_), .c(new_n111_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n89_), .c(new_n60_), .d(new_n79_), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand3  g070(.a(new_n114_), .b(x25), .c(x01), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n45_), .O(z5));
  nand3  g072(.a(x30), .b(new_n94_), .c(new_n102_), .O(new_n117_));
  nand3  g073(.a(new_n97_), .b(x29), .c(x28), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(new_n80_), .O(new_n119_));
  aoi21  g075(.a(x29), .b(x27), .c(new_n97_), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n98_), .O(new_n121_));
  inv1   g077(.a(x19), .O(new_n122_));
  nand3  g078(.a(new_n110_), .b(new_n97_), .c(x29), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n103_), .c(x24), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n120_), .c(new_n122_), .O(new_n125_));
  nand2  g081(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n60_), .c(new_n79_), .d(x25), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n58_), .c(new_n45_), .O(z6));
  nand4  g084(.a(new_n60_), .b(new_n79_), .c(x25), .d(x01), .O(new_n129_));
  nand2  g085(.a(new_n129_), .b(new_n45_), .O(new_n130_));
  nand3  g086(.a(x29), .b(x28), .c(x27), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(x31), .O(new_n132_));
  nand3  g088(.a(new_n86_), .b(new_n84_), .c(new_n82_), .O(new_n133_));
  inv1   g089(.a(new_n133_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(x19), .O(new_n135_));
  inv1   g091(.a(x31), .O(new_n136_));
  nand4  g092(.a(new_n110_), .b(new_n136_), .c(x30), .d(x29), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(new_n132_), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n102_), .O(new_n139_));
  oai21  g095(.a(new_n133_), .b(new_n102_), .c(new_n136_), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n97_), .O(new_n141_));
  nand3  g097(.a(new_n141_), .b(new_n139_), .c(new_n130_), .O(z7));
  nand2  g098(.a(x16), .b(x10), .O(new_n143_));
  nand3  g099(.a(x19), .b(x18), .c(x17), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(new_n143_), .O(new_n145_));
  nor2   g101(.a(new_n145_), .b(x15), .O(new_n146_));
  inv1   g102(.a(x16), .O(new_n147_));
  nand2  g103(.a(x19), .b(x18), .O(new_n148_));
  nand2  g104(.a(x17), .b(x11), .O(new_n149_));
  oai21  g105(.a(new_n149_), .b(new_n148_), .c(new_n147_), .O(new_n150_));
  inv1   g106(.a(x12), .O(new_n151_));
  inv1   g107(.a(x17), .O(new_n152_));
  oai21  g108(.a(new_n148_), .b(new_n151_), .c(new_n152_), .O(new_n153_));
  inv1   g109(.a(x13), .O(new_n154_));
  inv1   g110(.a(x18), .O(new_n155_));
  oai21  g111(.a(new_n122_), .b(new_n154_), .c(new_n155_), .O(new_n156_));
  nor2   g112(.a(x19), .b(x14), .O(new_n157_));
  nand2  g113(.a(x16), .b(x15), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n144_), .c(x31), .O(new_n159_));
  nor2   g115(.a(new_n159_), .b(new_n157_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n156_), .c(new_n153_), .d(new_n150_), .O(new_n161_));
  oai21  g117(.a(new_n161_), .b(new_n146_), .c(new_n45_), .O(new_n162_));
  nand2  g118(.a(new_n94_), .b(new_n80_), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(x30), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n109_), .O(new_n165_));
  nand2  g121(.a(new_n163_), .b(x29), .O(new_n166_));
  aoi22  g122(.a(new_n166_), .b(new_n97_), .c(new_n165_), .d(new_n102_), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n162_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(x32), .O(new_n169_));
  inv1   g125(.a(x32), .O(new_n170_));
  nand3  g126(.a(new_n166_), .b(x30), .c(new_n102_), .O(new_n171_));
  nand3  g127(.a(new_n163_), .b(new_n97_), .c(x29), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n171_), .O(new_n173_));
  nand3  g129(.a(x19), .b(new_n155_), .c(x13), .O(new_n174_));
  nand3  g130(.a(new_n122_), .b(x18), .c(x14), .O(new_n175_));
  aoi21  g131(.a(new_n175_), .b(new_n174_), .c(new_n152_), .O(new_n176_));
  nand4  g132(.a(x19), .b(x18), .c(new_n152_), .d(x12), .O(new_n177_));
  inv1   g133(.a(new_n177_), .O(new_n178_));
  oai21  g134(.a(new_n178_), .b(new_n176_), .c(x16), .O(new_n179_));
  inv1   g135(.a(new_n144_), .O(new_n180_));
  nand3  g136(.a(new_n180_), .b(new_n147_), .c(x11), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n179_), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(x15), .O(new_n183_));
  inv1   g139(.a(x15), .O(new_n184_));
  nand4  g140(.a(new_n180_), .b(x16), .c(new_n184_), .d(x10), .O(new_n185_));
  nand2  g141(.a(new_n185_), .b(new_n183_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n173_), .c(new_n170_), .d(x31), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n169_), .O(new_n188_));
  nand3  g144(.a(new_n188_), .b(new_n79_), .c(x00), .O(new_n189_));
  inv1   g145(.a(new_n189_), .O(z8));
  nand2  g146(.a(new_n122_), .b(x14), .O(new_n191_));
  nand4  g147(.a(new_n191_), .b(x18), .c(x17), .d(x16), .O(new_n192_));
  nor2   g148(.a(new_n192_), .b(new_n184_), .O(new_n193_));
  nand2  g149(.a(x18), .b(x17), .O(new_n194_));
  oai21  g150(.a(new_n194_), .b(new_n158_), .c(new_n122_), .O(new_n195_));
  oai21  g151(.a(new_n194_), .b(new_n143_), .c(new_n184_), .O(new_n196_));
  inv1   g152(.a(x11), .O(new_n197_));
  oai21  g153(.a(new_n194_), .b(new_n197_), .c(new_n147_), .O(new_n198_));
  nand2  g154(.a(x18), .b(x12), .O(new_n199_));
  aoi22  g155(.a(new_n199_), .b(new_n152_), .c(new_n155_), .d(new_n154_), .O(new_n200_));
  nand4  g156(.a(new_n200_), .b(new_n198_), .c(new_n196_), .d(new_n195_), .O(new_n201_));
  oai21  g157(.a(new_n201_), .b(new_n193_), .c(new_n45_), .O(new_n202_));
  nand2  g158(.a(new_n166_), .b(new_n136_), .O(new_n203_));
  nand2  g159(.a(new_n136_), .b(x30), .O(new_n204_));
  nand2  g160(.a(new_n166_), .b(x31), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  aoi22  g162(.a(new_n206_), .b(new_n102_), .c(new_n203_), .d(new_n97_), .O(new_n207_));
  nand2  g163(.a(new_n207_), .b(new_n202_), .O(new_n208_));
  nand2  g164(.a(new_n208_), .b(x33), .O(new_n209_));
  aoi21  g165(.a(new_n181_), .b(new_n179_), .c(new_n184_), .O(new_n210_));
  inv1   g166(.a(new_n185_), .O(new_n211_));
  nand4  g167(.a(new_n163_), .b(x31), .c(x30), .d(x29), .O(new_n212_));
  oai22  g168(.a(new_n212_), .b(x24), .c(new_n203_), .d(x30), .O(new_n213_));
  oai21  g169(.a(new_n211_), .b(new_n210_), .c(new_n213_), .O(new_n214_));
  oai21  g170(.a(new_n214_), .b(x33), .c(new_n209_), .O(new_n215_));
  nand3  g171(.a(new_n215_), .b(new_n79_), .c(x00), .O(new_n216_));
  inv1   g172(.a(new_n216_), .O(z9));
endmodule


