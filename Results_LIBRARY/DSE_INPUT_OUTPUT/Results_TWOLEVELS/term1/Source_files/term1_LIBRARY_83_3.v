// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x19), .O(new_n45_));
  inv1   g001(.a(x26), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n51_), .b(new_n54_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n51_), .b(new_n54_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n51_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n54_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n54_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n51_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n60_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g030(.a(new_n68_), .b(new_n66_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n48_), .c(new_n59_), .d(x08), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z2));
  inv1   g037(.a(x25), .O(new_n82_));
  inv1   g038(.a(x27), .O(new_n83_));
  nor2   g039(.a(x20), .b(x15), .O(new_n84_));
  nor2   g040(.a(x21), .b(x16), .O(new_n85_));
  nor2   g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x17), .O(new_n87_));
  inv1   g043(.a(x22), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  inv1   g045(.a(x18), .O(new_n90_));
  inv1   g046(.a(x23), .O(new_n91_));
  nand2  g047(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  inv1   g048(.a(x24), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(new_n45_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n92_), .c(new_n89_), .d(new_n86_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n63_), .c(new_n83_), .d(new_n46_), .O(new_n96_));
  nor3   g052(.a(new_n96_), .b(new_n82_), .c(new_n61_), .O(z3));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand4  g054(.a(new_n98_), .b(new_n95_), .c(new_n63_), .d(new_n46_), .O(new_n99_));
  nor3   g055(.a(new_n99_), .b(new_n82_), .c(new_n61_), .O(z4));
  inv1   g056(.a(x29), .O(new_n101_));
  inv1   g057(.a(x28), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n83_), .O(new_n103_));
  nand3  g059(.a(new_n101_), .b(x28), .c(x27), .O(new_n104_));
  oai21  g060(.a(new_n103_), .b(new_n101_), .c(new_n104_), .O(new_n105_));
  nand4  g061(.a(new_n105_), .b(new_n95_), .c(new_n63_), .d(new_n46_), .O(new_n106_));
  inv1   g062(.a(new_n106_), .O(new_n107_));
  nand3  g063(.a(new_n107_), .b(x25), .c(x01), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n48_), .O(z5));
  nand3  g065(.a(x29), .b(x28), .c(x27), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(x30), .O(new_n111_));
  inv1   g067(.a(x30), .O(new_n112_));
  nand3  g068(.a(new_n103_), .b(new_n112_), .c(x29), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n95_), .c(new_n63_), .d(new_n46_), .O(new_n115_));
  nor3   g071(.a(new_n115_), .b(new_n82_), .c(new_n61_), .O(z6));
  nor2   g072(.a(new_n112_), .b(new_n101_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(new_n103_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(x31), .O(new_n119_));
  nand4  g075(.a(new_n94_), .b(new_n92_), .c(new_n89_), .d(new_n86_), .O(new_n120_));
  nor2   g076(.a(new_n62_), .b(new_n61_), .O(new_n121_));
  inv1   g077(.a(new_n103_), .O(new_n122_));
  inv1   g078(.a(x31), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x30), .c(x29), .O(new_n124_));
  oai21  g080(.a(new_n124_), .b(new_n122_), .c(new_n46_), .O(new_n125_));
  nor2   g081(.a(new_n125_), .b(new_n82_), .O(new_n126_));
  nand4  g082(.a(new_n126_), .b(new_n121_), .c(new_n120_), .d(new_n119_), .O(z7));
  inv1   g083(.a(x32), .O(new_n128_));
  nor2   g084(.a(x28), .b(x27), .O(new_n129_));
  inv1   g085(.a(new_n129_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n112_), .c(x29), .O(new_n131_));
  nor2   g087(.a(new_n129_), .b(new_n101_), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n112_), .c(new_n131_), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n90_), .c(x13), .O(new_n134_));
  nand4  g090(.a(new_n46_), .b(new_n45_), .c(x18), .d(x14), .O(new_n135_));
  aoi21  g091(.a(new_n135_), .b(new_n134_), .c(new_n87_), .O(new_n136_));
  nor2   g092(.a(new_n45_), .b(new_n90_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n87_), .c(x12), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n136_), .c(x16), .O(new_n140_));
  inv1   g096(.a(x16), .O(new_n141_));
  nand2  g097(.a(new_n137_), .b(x17), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n141_), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x15), .O(new_n146_));
  inv1   g102(.a(x15), .O(new_n147_));
  nand4  g103(.a(new_n143_), .b(x16), .c(new_n147_), .d(x10), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n133_), .c(new_n128_), .d(x31), .O(new_n150_));
  nand2  g106(.a(new_n117_), .b(x27), .O(new_n151_));
  oai21  g107(.a(x30), .b(x27), .c(new_n151_), .O(new_n152_));
  nand3  g108(.a(new_n137_), .b(x17), .c(x16), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n147_), .O(new_n154_));
  nand2  g110(.a(new_n142_), .b(new_n141_), .O(new_n155_));
  nor2   g111(.a(new_n137_), .b(x17), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand2  g113(.a(x18), .b(x14), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n45_), .O(new_n159_));
  nand3  g115(.a(x30), .b(x29), .c(x28), .O(new_n160_));
  nand2  g116(.a(new_n112_), .b(new_n101_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(new_n160_), .c(new_n159_), .d(x31), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n157_), .c(new_n155_), .d(new_n154_), .O(new_n164_));
  aoi21  g120(.a(new_n152_), .b(new_n102_), .c(new_n164_), .O(new_n165_));
  inv1   g121(.a(x10), .O(new_n166_));
  nand2  g122(.a(new_n147_), .b(new_n166_), .O(new_n167_));
  inv1   g123(.a(x11), .O(new_n168_));
  nand2  g124(.a(new_n141_), .b(new_n168_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n167_), .O(new_n170_));
  inv1   g126(.a(x12), .O(new_n171_));
  nand2  g127(.a(new_n87_), .b(new_n171_), .O(new_n172_));
  nor2   g128(.a(new_n90_), .b(new_n87_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(x16), .c(x15), .O(new_n174_));
  inv1   g130(.a(x13), .O(new_n175_));
  nand2  g131(.a(new_n90_), .b(new_n175_), .O(new_n176_));
  nand3  g132(.a(new_n176_), .b(new_n174_), .c(new_n172_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n170_), .c(x19), .O(new_n178_));
  oai21  g134(.a(new_n165_), .b(x26), .c(new_n178_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(x32), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n150_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(x00), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n48_), .O(z8));
  inv1   g139(.a(x33), .O(new_n184_));
  nand4  g140(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n185_));
  nor2   g141(.a(x19), .b(new_n90_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n184_), .c(x31), .d(x30), .O(new_n187_));
  oai22  g143(.a(new_n187_), .b(new_n185_), .c(new_n184_), .d(x31), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n46_), .O(new_n189_));
  nand3  g145(.a(x18), .b(new_n87_), .c(x12), .O(new_n190_));
  nand3  g146(.a(new_n90_), .b(x17), .c(x13), .O(new_n191_));
  aoi21  g147(.a(new_n191_), .b(new_n190_), .c(new_n141_), .O(new_n192_));
  nand4  g148(.a(x18), .b(x17), .c(new_n141_), .d(x11), .O(new_n193_));
  inv1   g149(.a(new_n193_), .O(new_n194_));
  oai21  g150(.a(new_n194_), .b(new_n192_), .c(x15), .O(new_n195_));
  nand4  g151(.a(new_n173_), .b(x16), .c(new_n147_), .d(x10), .O(new_n196_));
  aoi21  g152(.a(new_n196_), .b(new_n195_), .c(x33), .O(new_n197_));
  nand4  g153(.a(new_n197_), .b(x31), .c(x30), .d(x19), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n189_), .O(new_n199_));
  nand3  g155(.a(new_n199_), .b(new_n130_), .c(x29), .O(new_n200_));
  inv1   g156(.a(new_n132_), .O(new_n201_));
  nand4  g157(.a(new_n186_), .b(new_n184_), .c(new_n123_), .d(new_n112_), .O(new_n202_));
  oai22  g158(.a(new_n202_), .b(new_n185_), .c(new_n184_), .d(new_n123_), .O(new_n203_));
  nand2  g159(.a(new_n203_), .b(new_n46_), .O(new_n204_));
  nand4  g160(.a(new_n197_), .b(new_n123_), .c(new_n112_), .d(x19), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n204_), .O(new_n206_));
  nand2  g162(.a(new_n206_), .b(new_n201_), .O(new_n207_));
  nand2  g163(.a(new_n90_), .b(x13), .O(new_n208_));
  nand4  g164(.a(new_n208_), .b(x17), .c(x16), .d(x15), .O(new_n209_));
  nand4  g165(.a(new_n209_), .b(new_n172_), .c(new_n169_), .d(new_n167_), .O(new_n210_));
  and2   g166(.a(new_n210_), .b(x19), .O(new_n211_));
  xnor2a g167(.a(x31), .b(x30), .O(new_n212_));
  nand4  g168(.a(new_n212_), .b(new_n159_), .c(new_n157_), .d(new_n155_), .O(new_n213_));
  inv1   g169(.a(new_n213_), .O(new_n214_));
  aoi21  g170(.a(new_n214_), .b(new_n154_), .c(x26), .O(new_n215_));
  oai21  g171(.a(new_n215_), .b(new_n211_), .c(x33), .O(new_n216_));
  nand3  g172(.a(new_n216_), .b(new_n207_), .c(new_n200_), .O(new_n217_));
  nand2  g173(.a(new_n217_), .b(x00), .O(new_n218_));
  nand2  g174(.a(new_n218_), .b(new_n48_), .O(z9));
endmodule


