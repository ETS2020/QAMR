// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:20 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n123_,
    new_n124_, new_n125_, new_n126_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n191_,
    new_n192_, new_n193_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_, new_n218_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  inv1   g004(.a(new_n47_), .O(new_n49_));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand3  g013(.a(new_n57_), .b(new_n53_), .c(new_n49_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n49_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nor2   g044(.a(x24), .b(x19), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  and2   g048(.a(new_n92_), .b(new_n85_), .O(new_n93_));
  nor2   g049(.a(new_n93_), .b(new_n61_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n60_), .c(new_n49_), .O(z3));
  xor2a  g052(.a(x28), .b(x27), .O(new_n97_));
  nor2   g053(.a(new_n86_), .b(new_n84_), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n90_), .c(new_n88_), .O(new_n99_));
  aoi21  g055(.a(new_n99_), .b(new_n49_), .c(new_n83_), .O(new_n100_));
  nor2   g056(.a(new_n100_), .b(new_n61_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n97_), .c(new_n81_), .d(x25), .O(new_n102_));
  nor2   g058(.a(new_n102_), .b(new_n60_), .O(z4));
  inv1   g059(.a(x28), .O(new_n104_));
  nor2   g060(.a(new_n104_), .b(new_n82_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  inv1   g062(.a(new_n83_), .O(new_n107_));
  oai21  g063(.a(new_n46_), .b(new_n45_), .c(new_n99_), .O(new_n108_));
  nand2  g064(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(new_n106_), .c(x29), .O(new_n110_));
  inv1   g066(.a(x29), .O(new_n111_));
  inv1   g067(.a(new_n100_), .O(new_n112_));
  nand4  g068(.a(new_n112_), .b(new_n111_), .c(x28), .d(x27), .O(new_n113_));
  nand2  g069(.a(new_n113_), .b(new_n110_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n115_));
  nor2   g071(.a(new_n115_), .b(new_n60_), .O(z5));
  nand3  g072(.a(new_n99_), .b(x29), .c(new_n45_), .O(new_n117_));
  nand2  g073(.a(new_n117_), .b(x20), .O(new_n118_));
  inv1   g074(.a(x20), .O(new_n119_));
  nor2   g075(.a(new_n84_), .b(new_n119_), .O(new_n120_));
  aoi21  g076(.a(new_n120_), .b(new_n92_), .c(x29), .O(new_n121_));
  aoi21  g077(.a(new_n118_), .b(new_n106_), .c(new_n121_), .O(new_n122_));
  nor2   g078(.a(new_n93_), .b(x30), .O(new_n123_));
  nand4  g079(.a(new_n123_), .b(x29), .c(x28), .d(x27), .O(new_n124_));
  oai21  g080(.a(new_n122_), .b(new_n46_), .c(new_n124_), .O(new_n125_));
  nand4  g081(.a(new_n125_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n126_));
  oai21  g082(.a(new_n126_), .b(new_n60_), .c(new_n49_), .O(z6));
  nand4  g083(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n49_), .O(new_n129_));
  nand3  g085(.a(x29), .b(x28), .c(x27), .O(new_n130_));
  nand2  g086(.a(new_n130_), .b(x31), .O(new_n131_));
  nand3  g087(.a(new_n98_), .b(new_n90_), .c(new_n88_), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x20), .O(new_n134_));
  inv1   g090(.a(x31), .O(new_n135_));
  nand4  g091(.a(new_n105_), .b(new_n135_), .c(x30), .d(x29), .O(new_n136_));
  nand3  g092(.a(new_n136_), .b(new_n134_), .c(new_n131_), .O(new_n137_));
  nand2  g093(.a(new_n137_), .b(new_n45_), .O(new_n138_));
  oai21  g094(.a(new_n132_), .b(new_n45_), .c(new_n135_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(new_n46_), .O(new_n140_));
  nand3  g096(.a(new_n140_), .b(new_n138_), .c(new_n129_), .O(z7));
  inv1   g097(.a(x00), .O(new_n142_));
  inv1   g098(.a(x32), .O(new_n143_));
  nand2  g099(.a(new_n104_), .b(new_n82_), .O(new_n144_));
  inv1   g100(.a(x17), .O(new_n145_));
  inv1   g101(.a(x18), .O(new_n146_));
  nand3  g102(.a(x19), .b(new_n146_), .c(x13), .O(new_n147_));
  inv1   g103(.a(x19), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x18), .c(x14), .O(new_n149_));
  aoi21  g105(.a(new_n149_), .b(new_n147_), .c(new_n145_), .O(new_n150_));
  nand4  g106(.a(x19), .b(x18), .c(new_n145_), .d(x12), .O(new_n151_));
  inv1   g107(.a(new_n151_), .O(new_n152_));
  oai21  g108(.a(new_n152_), .b(new_n150_), .c(x16), .O(new_n153_));
  inv1   g109(.a(x16), .O(new_n154_));
  nand3  g110(.a(x19), .b(x18), .c(x17), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n154_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n153_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  nand4  g115(.a(new_n46_), .b(x19), .c(x18), .d(x17), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(new_n161_));
  nand4  g117(.a(new_n161_), .b(x16), .c(new_n45_), .d(x10), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n159_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n144_), .c(new_n143_), .d(x31), .O(new_n164_));
  oai21  g120(.a(new_n104_), .b(x15), .c(new_n82_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(x32), .c(x30), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n164_), .c(new_n111_), .O(new_n167_));
  nand2  g123(.a(new_n144_), .b(x29), .O(new_n168_));
  nand4  g124(.a(x18), .b(x17), .c(x16), .d(x10), .O(new_n169_));
  nand4  g125(.a(new_n143_), .b(x31), .c(x30), .d(x19), .O(new_n170_));
  oai22  g126(.a(new_n170_), .b(new_n169_), .c(new_n143_), .d(x30), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  inv1   g128(.a(x11), .O(new_n173_));
  inv1   g129(.a(x12), .O(new_n174_));
  aoi22  g130(.a(new_n145_), .b(new_n174_), .c(new_n154_), .d(new_n173_), .O(new_n175_));
  inv1   g131(.a(x13), .O(new_n176_));
  inv1   g132(.a(x14), .O(new_n177_));
  aoi22  g133(.a(new_n148_), .b(new_n177_), .c(new_n146_), .d(new_n176_), .O(new_n178_));
  aoi21  g134(.a(new_n178_), .b(new_n175_), .c(x30), .O(new_n179_));
  nor2   g135(.a(new_n148_), .b(new_n146_), .O(new_n180_));
  nand3  g136(.a(new_n180_), .b(x17), .c(x15), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n154_), .O(new_n182_));
  inv1   g138(.a(new_n180_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n45_), .c(new_n145_), .O(new_n184_));
  oai21  g140(.a(new_n148_), .b(new_n45_), .c(new_n146_), .O(new_n185_));
  aoi21  g141(.a(x19), .b(x10), .c(x15), .O(new_n186_));
  nand3  g142(.a(new_n156_), .b(x16), .c(x15), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(x31), .O(new_n188_));
  nor2   g144(.a(new_n188_), .b(new_n186_), .O(new_n189_));
  nand4  g145(.a(new_n189_), .b(new_n185_), .c(new_n184_), .d(new_n182_), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n179_), .c(x32), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n172_), .O(new_n192_));
  oai21  g148(.a(new_n192_), .b(new_n167_), .c(new_n81_), .O(new_n193_));
  oai21  g149(.a(new_n193_), .b(new_n142_), .c(new_n49_), .O(z8));
  inv1   g150(.a(x33), .O(new_n195_));
  nand3  g151(.a(new_n163_), .b(new_n195_), .c(new_n135_), .O(new_n196_));
  nand2  g152(.a(x33), .b(x31), .O(new_n197_));
  nand2  g153(.a(new_n197_), .b(new_n196_), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n168_), .O(new_n199_));
  nand4  g155(.a(x17), .b(x16), .c(new_n45_), .d(x10), .O(new_n200_));
  nand4  g156(.a(new_n180_), .b(new_n195_), .c(x31), .d(x30), .O(new_n201_));
  oai22  g157(.a(new_n201_), .b(new_n200_), .c(new_n195_), .d(x31), .O(new_n202_));
  nand3  g158(.a(new_n202_), .b(new_n144_), .c(x29), .O(new_n203_));
  inv1   g159(.a(new_n203_), .O(new_n204_));
  nand2  g160(.a(x16), .b(x10), .O(new_n205_));
  oai21  g161(.a(new_n205_), .b(new_n155_), .c(new_n45_), .O(new_n206_));
  nand3  g162(.a(new_n180_), .b(x17), .c(x11), .O(new_n207_));
  nand2  g163(.a(new_n207_), .b(new_n154_), .O(new_n208_));
  oai21  g164(.a(new_n183_), .b(new_n174_), .c(new_n145_), .O(new_n209_));
  nand2  g165(.a(new_n148_), .b(new_n177_), .O(new_n210_));
  oai21  g166(.a(new_n148_), .b(new_n176_), .c(new_n146_), .O(new_n211_));
  xnor2a g167(.a(x31), .b(x30), .O(new_n212_));
  nand4  g168(.a(new_n212_), .b(new_n211_), .c(new_n187_), .d(new_n210_), .O(new_n213_));
  inv1   g169(.a(new_n213_), .O(new_n214_));
  nand4  g170(.a(new_n214_), .b(new_n209_), .c(new_n208_), .d(new_n206_), .O(new_n215_));
  aoi21  g171(.a(new_n215_), .b(x33), .c(new_n204_), .O(new_n216_));
  nand2  g172(.a(new_n216_), .b(new_n199_), .O(new_n217_));
  nand3  g173(.a(new_n217_), .b(new_n81_), .c(x00), .O(new_n218_));
  nand2  g174(.a(new_n218_), .b(new_n49_), .O(z9));
endmodule


