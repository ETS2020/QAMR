// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n97_, new_n98_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_;
  inv1   g000(.a(x30), .O(new_n45_));
  inv1   g001(.a(x33), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nor2   g003(.a(new_n47_), .b(x32), .O(z0));
  nor3   g004(.a(x32), .b(x30), .c(x03), .O(new_n49_));
  inv1   g005(.a(x03), .O(new_n50_));
  nor2   g006(.a(x33), .b(new_n50_), .O(new_n51_));
  oai21  g007(.a(new_n51_), .b(new_n49_), .c(x02), .O(new_n52_));
  inv1   g008(.a(x02), .O(new_n53_));
  inv1   g009(.a(x32), .O(new_n54_));
  nand3  g010(.a(new_n54_), .b(new_n45_), .c(x03), .O(new_n55_));
  oai21  g011(.a(x33), .b(x03), .c(new_n55_), .O(new_n56_));
  nand2  g012(.a(new_n56_), .b(new_n53_), .O(new_n57_));
  nand2  g013(.a(new_n46_), .b(new_n54_), .O(new_n58_));
  nand3  g014(.a(new_n58_), .b(new_n57_), .c(new_n52_), .O(z1));
  inv1   g015(.a(new_n47_), .O(new_n60_));
  xnor2a g016(.a(x07), .b(x04), .O(new_n61_));
  inv1   g017(.a(x01), .O(new_n62_));
  nor2   g018(.a(new_n50_), .b(new_n53_), .O(new_n63_));
  inv1   g019(.a(new_n63_), .O(new_n64_));
  nand2  g020(.a(new_n50_), .b(new_n53_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(new_n66_));
  nand2  g022(.a(x05), .b(new_n50_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x06), .c(new_n53_), .O(new_n68_));
  nand2  g024(.a(x06), .b(new_n53_), .O(new_n69_));
  nand3  g025(.a(new_n69_), .b(x05), .c(new_n50_), .O(new_n70_));
  aoi21  g026(.a(new_n70_), .b(new_n68_), .c(x01), .O(new_n71_));
  oai21  g027(.a(new_n71_), .b(new_n66_), .c(new_n61_), .O(new_n72_));
  xor2a  g028(.a(x07), .b(x04), .O(new_n73_));
  xnor2a g029(.a(x03), .b(x02), .O(new_n74_));
  nor2   g030(.a(new_n74_), .b(new_n62_), .O(new_n75_));
  nand2  g031(.a(new_n69_), .b(new_n67_), .O(new_n76_));
  nand4  g032(.a(x06), .b(x05), .c(new_n50_), .d(new_n53_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n76_), .c(x01), .O(new_n78_));
  oai21  g034(.a(new_n78_), .b(new_n75_), .c(new_n73_), .O(new_n79_));
  aoi21  g035(.a(new_n79_), .b(new_n72_), .c(x09), .O(new_n80_));
  nand2  g036(.a(new_n80_), .b(x08), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n60_), .O(z2));
  inv1   g038(.a(x26), .O(new_n83_));
  inv1   g039(.a(x27), .O(new_n84_));
  nand2  g040(.a(x33), .b(x30), .O(new_n85_));
  nor2   g041(.a(x20), .b(x15), .O(new_n86_));
  nor2   g042(.a(x21), .b(x16), .O(new_n87_));
  nor2   g043(.a(new_n87_), .b(new_n86_), .O(new_n88_));
  nor2   g044(.a(x22), .b(x17), .O(new_n89_));
  nor2   g045(.a(x23), .b(x18), .O(new_n90_));
  nor2   g046(.a(x24), .b(x19), .O(new_n91_));
  nor3   g047(.a(new_n91_), .b(new_n90_), .c(new_n89_), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n88_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nor2   g050(.a(new_n94_), .b(new_n63_), .O(new_n95_));
  nand3  g051(.a(new_n95_), .b(new_n85_), .c(new_n84_), .O(new_n96_));
  inv1   g052(.a(new_n96_), .O(new_n97_));
  nand4  g053(.a(new_n97_), .b(new_n83_), .c(x25), .d(x01), .O(new_n98_));
  inv1   g054(.a(new_n98_), .O(z3));
  xor2a  g055(.a(x28), .b(x27), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n95_), .c(new_n83_), .d(x25), .O(new_n101_));
  oai21  g057(.a(new_n101_), .b(new_n62_), .c(new_n60_), .O(z4));
  inv1   g058(.a(x28), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n84_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n93_), .c(new_n64_), .d(new_n83_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand3  g065(.a(new_n109_), .b(x25), .c(x01), .O(new_n110_));
  nand2  g066(.a(new_n110_), .b(new_n60_), .O(z5));
  oai21  g067(.a(x33), .b(x27), .c(x29), .O(new_n112_));
  oai21  g068(.a(new_n112_), .b(new_n103_), .c(x30), .O(new_n113_));
  nand4  g069(.a(new_n45_), .b(x29), .c(x28), .d(x27), .O(new_n114_));
  aoi21  g070(.a(new_n114_), .b(new_n113_), .c(new_n94_), .O(new_n115_));
  nand4  g071(.a(new_n115_), .b(new_n64_), .c(new_n83_), .d(x25), .O(new_n116_));
  oai21  g072(.a(new_n116_), .b(new_n62_), .c(new_n60_), .O(z6));
  inv1   g073(.a(new_n89_), .O(new_n118_));
  nand2  g074(.a(new_n118_), .b(new_n88_), .O(new_n119_));
  nor3   g075(.a(new_n119_), .b(new_n91_), .c(new_n90_), .O(new_n120_));
  nand4  g076(.a(new_n64_), .b(new_n83_), .c(x25), .d(x01), .O(new_n121_));
  oai21  g077(.a(new_n121_), .b(new_n120_), .c(new_n60_), .O(new_n122_));
  inv1   g078(.a(x31), .O(new_n123_));
  nand3  g079(.a(x29), .b(x28), .c(x27), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n46_), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(x30), .c(new_n123_), .O(new_n126_));
  nor4   g082(.a(new_n124_), .b(x33), .c(x31), .d(new_n45_), .O(new_n127_));
  nor2   g083(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(new_n122_), .O(z7));
  nor2   g085(.a(x28), .b(x27), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n45_), .c(x29), .O(new_n132_));
  nor2   g088(.a(new_n130_), .b(new_n105_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n45_), .c(new_n132_), .O(new_n134_));
  inv1   g090(.a(x17), .O(new_n135_));
  inv1   g091(.a(x18), .O(new_n136_));
  nand3  g092(.a(x19), .b(new_n136_), .c(x13), .O(new_n137_));
  inv1   g093(.a(x19), .O(new_n138_));
  nand3  g094(.a(new_n138_), .b(x18), .c(x14), .O(new_n139_));
  aoi21  g095(.a(new_n139_), .b(new_n137_), .c(new_n135_), .O(new_n140_));
  nand4  g096(.a(x19), .b(x18), .c(new_n135_), .d(x12), .O(new_n141_));
  inv1   g097(.a(new_n141_), .O(new_n142_));
  oai21  g098(.a(new_n142_), .b(new_n140_), .c(x16), .O(new_n143_));
  inv1   g099(.a(x16), .O(new_n144_));
  nand3  g100(.a(x19), .b(x18), .c(x17), .O(new_n145_));
  inv1   g101(.a(new_n145_), .O(new_n146_));
  nand3  g102(.a(new_n146_), .b(new_n144_), .c(x11), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n143_), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(x15), .O(new_n149_));
  inv1   g105(.a(x15), .O(new_n150_));
  nand4  g106(.a(new_n146_), .b(x16), .c(new_n150_), .d(x10), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n134_), .c(new_n54_), .d(x31), .O(new_n153_));
  oai21  g109(.a(x33), .b(new_n103_), .c(new_n84_), .O(new_n154_));
  nand3  g110(.a(new_n154_), .b(x30), .c(x29), .O(new_n155_));
  inv1   g111(.a(new_n155_), .O(new_n156_));
  oai21  g112(.a(new_n130_), .b(new_n105_), .c(new_n45_), .O(new_n157_));
  nand2  g113(.a(x16), .b(x10), .O(new_n158_));
  oai21  g114(.a(new_n158_), .b(new_n145_), .c(new_n150_), .O(new_n159_));
  nor2   g115(.a(new_n138_), .b(new_n136_), .O(new_n160_));
  nand3  g116(.a(new_n160_), .b(x17), .c(x11), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n144_), .O(new_n162_));
  aoi21  g118(.a(new_n160_), .b(x12), .c(x17), .O(new_n163_));
  nand2  g119(.a(x19), .b(x13), .O(new_n164_));
  nand2  g120(.a(new_n164_), .b(new_n136_), .O(new_n165_));
  nor2   g121(.a(x19), .b(x14), .O(new_n166_));
  nor3   g122(.a(new_n145_), .b(new_n144_), .c(new_n150_), .O(new_n167_));
  nor3   g123(.a(new_n167_), .b(new_n166_), .c(new_n123_), .O(new_n168_));
  nand2  g124(.a(new_n168_), .b(new_n165_), .O(new_n169_));
  nor2   g125(.a(new_n169_), .b(new_n163_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(new_n162_), .c(new_n159_), .d(new_n157_), .O(new_n171_));
  oai21  g127(.a(new_n171_), .b(new_n156_), .c(x32), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n153_), .O(new_n173_));
  nand3  g129(.a(new_n173_), .b(new_n83_), .c(x00), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n60_), .O(z8));
  nand4  g131(.a(new_n152_), .b(new_n46_), .c(x31), .d(x30), .O(new_n176_));
  nand2  g132(.a(new_n176_), .b(new_n46_), .O(new_n177_));
  nand3  g133(.a(new_n177_), .b(new_n131_), .c(x29), .O(new_n178_));
  aoi21  g134(.a(new_n151_), .b(new_n149_), .c(new_n133_), .O(new_n179_));
  nand4  g135(.a(new_n179_), .b(new_n46_), .c(new_n123_), .d(new_n45_), .O(new_n180_));
  inv1   g136(.a(new_n159_), .O(new_n181_));
  inv1   g137(.a(new_n163_), .O(new_n182_));
  nor3   g138(.a(new_n167_), .b(new_n166_), .c(x31), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n165_), .c(new_n182_), .d(new_n162_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n181_), .c(x33), .O(new_n185_));
  nand3  g141(.a(new_n185_), .b(new_n180_), .c(new_n178_), .O(new_n186_));
  nand3  g142(.a(new_n186_), .b(new_n83_), .c(x00), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n60_), .O(z9));
endmodule


