// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n107_, new_n108_, new_n109_,
    new_n111_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  inv1   g000(.a(x26), .O(new_n45_));
  nand2  g001(.a(x28), .b(new_n45_), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  nand3  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  xor2a  g013(.a(x03), .b(x02), .O(new_n58_));
  nand2  g014(.a(new_n58_), .b(x01), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n49_), .b(new_n52_), .O(new_n61_));
  nor2   g017(.a(x03), .b(x02), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(x05), .O(new_n63_));
  oai21  g019(.a(x06), .b(x05), .c(new_n63_), .O(new_n64_));
  oai21  g020(.a(new_n64_), .b(new_n61_), .c(new_n60_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n59_), .c(new_n57_), .O(new_n66_));
  xor2a  g022(.a(x07), .b(x04), .O(new_n67_));
  oai21  g023(.a(new_n62_), .b(new_n61_), .c(x01), .O(new_n68_));
  nand3  g024(.a(x05), .b(new_n49_), .c(x02), .O(new_n69_));
  nand3  g025(.a(x06), .b(x03), .c(new_n52_), .O(new_n70_));
  nand2  g026(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n60_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n68_), .c(new_n67_), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n66_), .c(new_n46_), .O(new_n74_));
  inv1   g030(.a(x28), .O(new_n75_));
  oai22  g031(.a(x06), .b(new_n49_), .c(x05), .d(new_n52_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n75_), .c(new_n60_), .O(new_n77_));
  inv1   g033(.a(x05), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n49_), .c(x02), .O(new_n79_));
  inv1   g035(.a(x06), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x03), .c(new_n52_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x26), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n77_), .c(new_n57_), .O(new_n84_));
  nand2  g040(.a(x06), .b(new_n78_), .O(new_n85_));
  nand2  g041(.a(new_n80_), .b(x05), .O(new_n86_));
  oai22  g042(.a(new_n86_), .b(x03), .c(new_n85_), .d(x02), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n75_), .c(new_n60_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(x26), .c(new_n49_), .d(new_n52_), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n88_), .c(new_n67_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n74_), .c(x09), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x08), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z2));
  inv1   g051(.a(x27), .O(new_n96_));
  nor2   g052(.a(x20), .b(x15), .O(new_n97_));
  nor2   g053(.a(x21), .b(x16), .O(new_n98_));
  nor2   g054(.a(new_n98_), .b(new_n97_), .O(new_n99_));
  nor2   g055(.a(x22), .b(x17), .O(new_n100_));
  nor2   g056(.a(x23), .b(x18), .O(new_n101_));
  nor2   g057(.a(x24), .b(x19), .O(new_n102_));
  nor3   g058(.a(new_n102_), .b(new_n101_), .c(new_n100_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n99_), .c(new_n61_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n96_), .c(x25), .d(x01), .O(new_n105_));
  aoi21  g061(.a(new_n105_), .b(new_n75_), .c(x26), .O(z3));
  nand3  g062(.a(new_n104_), .b(new_n75_), .c(x27), .O(new_n107_));
  inv1   g063(.a(new_n107_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n45_), .c(x25), .d(x01), .O(new_n109_));
  inv1   g065(.a(new_n109_), .O(z4));
  nand4  g066(.a(new_n104_), .b(x29), .c(x25), .d(x01), .O(new_n111_));
  aoi21  g067(.a(new_n111_), .b(new_n75_), .c(x26), .O(z5));
  nand4  g068(.a(new_n104_), .b(x30), .c(x25), .d(x01), .O(new_n113_));
  aoi21  g069(.a(new_n113_), .b(new_n75_), .c(x26), .O(z6));
  inv1   g070(.a(new_n100_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n99_), .O(new_n116_));
  nor3   g072(.a(new_n116_), .b(new_n102_), .c(new_n101_), .O(new_n117_));
  inv1   g073(.a(x31), .O(new_n118_));
  inv1   g074(.a(new_n61_), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(new_n118_), .c(x25), .d(x01), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n117_), .c(new_n75_), .O(new_n121_));
  nand2  g077(.a(new_n121_), .b(new_n45_), .O(z7));
  nand2  g078(.a(x29), .b(x27), .O(new_n123_));
  inv1   g079(.a(new_n123_), .O(new_n124_));
  inv1   g080(.a(x32), .O(new_n125_));
  inv1   g081(.a(x17), .O(new_n126_));
  inv1   g082(.a(x18), .O(new_n127_));
  nand3  g083(.a(x19), .b(new_n127_), .c(x13), .O(new_n128_));
  inv1   g084(.a(x19), .O(new_n129_));
  nand3  g085(.a(new_n129_), .b(x18), .c(x14), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n128_), .c(new_n126_), .O(new_n131_));
  nand4  g087(.a(x19), .b(x18), .c(new_n126_), .d(x12), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  oai21  g089(.a(new_n133_), .b(new_n131_), .c(x16), .O(new_n134_));
  inv1   g090(.a(x16), .O(new_n135_));
  nand3  g091(.a(x19), .b(x18), .c(x17), .O(new_n136_));
  inv1   g092(.a(new_n136_), .O(new_n137_));
  nand3  g093(.a(new_n137_), .b(new_n135_), .c(x11), .O(new_n138_));
  nand2  g094(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x15), .O(new_n140_));
  inv1   g096(.a(x15), .O(new_n141_));
  nand4  g097(.a(new_n137_), .b(x16), .c(new_n141_), .d(x10), .O(new_n142_));
  nand2  g098(.a(new_n142_), .b(new_n140_), .O(new_n143_));
  nand4  g099(.a(new_n143_), .b(new_n125_), .c(x31), .d(x30), .O(new_n144_));
  inv1   g100(.a(x30), .O(new_n145_));
  nand3  g101(.a(x32), .b(new_n145_), .c(new_n75_), .O(new_n146_));
  aoi21  g102(.a(new_n146_), .b(new_n144_), .c(new_n124_), .O(new_n147_));
  nand3  g103(.a(x27), .b(new_n129_), .c(x14), .O(new_n148_));
  nand4  g104(.a(new_n125_), .b(x31), .c(new_n145_), .d(x29), .O(new_n149_));
  oai22  g105(.a(new_n149_), .b(new_n148_), .c(new_n125_), .d(new_n129_), .O(new_n150_));
  nand4  g106(.a(x27), .b(x19), .c(new_n127_), .d(x13), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  aoi21  g108(.a(new_n150_), .b(x18), .c(new_n152_), .O(new_n153_));
  nand3  g109(.a(new_n125_), .b(x31), .c(new_n145_), .O(new_n154_));
  inv1   g110(.a(new_n154_), .O(new_n155_));
  nand2  g111(.a(new_n155_), .b(new_n124_), .O(new_n156_));
  oai22  g112(.a(new_n156_), .b(new_n132_), .c(new_n153_), .d(new_n126_), .O(new_n157_));
  nor2   g113(.a(new_n156_), .b(new_n138_), .O(new_n158_));
  aoi21  g114(.a(new_n157_), .b(x16), .c(new_n158_), .O(new_n159_));
  inv1   g115(.a(x29), .O(new_n160_));
  nand3  g116(.a(x32), .b(x30), .c(new_n75_), .O(new_n161_));
  nand2  g117(.a(x19), .b(x18), .O(new_n162_));
  inv1   g118(.a(new_n162_), .O(new_n163_));
  nand4  g119(.a(x17), .b(x16), .c(new_n141_), .d(x10), .O(new_n164_));
  inv1   g120(.a(new_n164_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n155_), .c(new_n163_), .O(new_n166_));
  aoi21  g122(.a(new_n166_), .b(new_n161_), .c(new_n160_), .O(new_n167_));
  nand2  g123(.a(x16), .b(x10), .O(new_n168_));
  oai21  g124(.a(new_n168_), .b(new_n136_), .c(new_n141_), .O(new_n169_));
  nand2  g125(.a(x17), .b(x11), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n162_), .c(new_n135_), .O(new_n171_));
  inv1   g127(.a(x12), .O(new_n172_));
  oai21  g128(.a(new_n162_), .b(new_n172_), .c(new_n126_), .O(new_n173_));
  nand2  g129(.a(x19), .b(x13), .O(new_n174_));
  nand2  g130(.a(new_n174_), .b(new_n127_), .O(new_n175_));
  inv1   g131(.a(x14), .O(new_n176_));
  nand2  g132(.a(new_n129_), .b(new_n176_), .O(new_n177_));
  and2   g133(.a(new_n177_), .b(new_n175_), .O(new_n178_));
  nand4  g134(.a(new_n178_), .b(new_n173_), .c(new_n171_), .d(new_n169_), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n75_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(x31), .O(new_n181_));
  aoi22  g137(.a(new_n181_), .b(x32), .c(new_n167_), .d(x27), .O(new_n182_));
  oai21  g138(.a(new_n159_), .b(new_n141_), .c(new_n182_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n147_), .c(x00), .O(new_n184_));
  aoi21  g140(.a(new_n184_), .b(new_n75_), .c(x26), .O(z8));
  inv1   g141(.a(x33), .O(new_n186_));
  nand4  g142(.a(new_n143_), .b(new_n186_), .c(new_n118_), .d(new_n145_), .O(new_n187_));
  nand2  g143(.a(x33), .b(x31), .O(new_n188_));
  aoi21  g144(.a(new_n188_), .b(new_n187_), .c(new_n124_), .O(new_n189_));
  nand4  g145(.a(new_n186_), .b(x31), .c(x30), .d(x29), .O(new_n190_));
  oai22  g146(.a(new_n190_), .b(new_n148_), .c(new_n186_), .d(new_n129_), .O(new_n191_));
  nor2   g147(.a(new_n190_), .b(new_n151_), .O(new_n192_));
  aoi21  g148(.a(new_n191_), .b(x18), .c(new_n192_), .O(new_n193_));
  nand3  g149(.a(new_n186_), .b(x31), .c(x30), .O(new_n194_));
  inv1   g150(.a(new_n194_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(new_n124_), .O(new_n196_));
  oai22  g152(.a(new_n196_), .b(new_n132_), .c(new_n193_), .d(new_n126_), .O(new_n197_));
  nor2   g153(.a(new_n196_), .b(new_n138_), .O(new_n198_));
  aoi21  g154(.a(new_n197_), .b(x16), .c(new_n198_), .O(new_n199_));
  nand2  g155(.a(x33), .b(new_n145_), .O(new_n200_));
  nand3  g156(.a(new_n195_), .b(new_n165_), .c(new_n163_), .O(new_n201_));
  aoi21  g157(.a(new_n201_), .b(new_n200_), .c(new_n160_), .O(new_n202_));
  nand2  g158(.a(new_n118_), .b(x30), .O(new_n203_));
  nand3  g159(.a(new_n203_), .b(new_n177_), .c(new_n175_), .O(new_n204_));
  inv1   g160(.a(new_n204_), .O(new_n205_));
  nand4  g161(.a(new_n205_), .b(new_n173_), .c(new_n171_), .d(new_n169_), .O(new_n206_));
  aoi22  g162(.a(new_n206_), .b(x33), .c(new_n202_), .d(x27), .O(new_n207_));
  oai21  g163(.a(new_n199_), .b(new_n141_), .c(new_n207_), .O(new_n208_));
  oai21  g164(.a(new_n208_), .b(new_n189_), .c(x00), .O(new_n209_));
  aoi21  g165(.a(new_n209_), .b(new_n75_), .c(x26), .O(z9));
endmodule


