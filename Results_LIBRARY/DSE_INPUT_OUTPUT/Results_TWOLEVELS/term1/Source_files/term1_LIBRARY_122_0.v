// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_;
  nor2   g000(.a(x29), .b(x27), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x32), .O(z0));
  nor2   g002(.a(x32), .b(x03), .O(new_n47_));
  inv1   g003(.a(x03), .O(new_n48_));
  nor2   g004(.a(x33), .b(new_n48_), .O(new_n49_));
  oai21  g005(.a(new_n49_), .b(new_n47_), .c(x02), .O(new_n50_));
  inv1   g006(.a(x02), .O(new_n51_));
  nor2   g007(.a(x32), .b(new_n48_), .O(new_n52_));
  nor2   g008(.a(x33), .b(x03), .O(new_n53_));
  oai21  g009(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n54_));
  aoi21  g010(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z1));
  inv1   g011(.a(x09), .O(new_n56_));
  inv1   g012(.a(new_n45_), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  inv1   g014(.a(x01), .O(new_n59_));
  nand2  g015(.a(x03), .b(x02), .O(new_n60_));
  nand2  g016(.a(new_n48_), .b(new_n51_), .O(new_n61_));
  aoi21  g017(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nand2  g018(.a(x05), .b(new_n48_), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(new_n51_), .O(new_n64_));
  nand2  g020(.a(x06), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x05), .c(new_n48_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n64_), .c(x01), .O(new_n67_));
  oai21  g023(.a(new_n67_), .b(new_n62_), .c(new_n58_), .O(new_n68_));
  xor2a  g024(.a(x07), .b(x04), .O(new_n69_));
  xnor2a g025(.a(x03), .b(x02), .O(new_n70_));
  nor2   g026(.a(new_n70_), .b(new_n59_), .O(new_n71_));
  nand2  g027(.a(new_n65_), .b(new_n63_), .O(new_n72_));
  nand4  g028(.a(x06), .b(x05), .c(new_n48_), .d(new_n51_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n72_), .c(x01), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n71_), .c(new_n69_), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand4  g032(.a(new_n76_), .b(new_n57_), .c(new_n56_), .d(x08), .O(new_n77_));
  inv1   g033(.a(new_n77_), .O(z2));
  inv1   g034(.a(x26), .O(new_n79_));
  inv1   g035(.a(x27), .O(new_n80_));
  nor2   g036(.a(x20), .b(x15), .O(new_n81_));
  nor2   g037(.a(x21), .b(x16), .O(new_n82_));
  nor2   g038(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  inv1   g039(.a(x17), .O(new_n84_));
  inv1   g040(.a(x22), .O(new_n85_));
  nand2  g041(.a(new_n85_), .b(new_n84_), .O(new_n86_));
  inv1   g042(.a(x18), .O(new_n87_));
  inv1   g043(.a(x23), .O(new_n88_));
  nand2  g044(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nor2   g045(.a(x24), .b(x19), .O(new_n90_));
  inv1   g046(.a(new_n90_), .O(new_n91_));
  nand4  g047(.a(new_n91_), .b(new_n89_), .c(new_n86_), .d(new_n83_), .O(new_n92_));
  nand4  g048(.a(new_n92_), .b(new_n60_), .c(x29), .d(new_n80_), .O(new_n93_));
  inv1   g049(.a(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n79_), .c(x25), .d(x01), .O(new_n95_));
  inv1   g051(.a(new_n95_), .O(z3));
  inv1   g052(.a(x25), .O(new_n97_));
  inv1   g053(.a(x28), .O(new_n98_));
  nand2  g054(.a(new_n98_), .b(x27), .O(new_n99_));
  nand2  g055(.a(x29), .b(x28), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(x27), .c(new_n99_), .O(new_n101_));
  nand4  g057(.a(new_n101_), .b(new_n92_), .c(new_n60_), .d(new_n79_), .O(new_n102_));
  nor3   g058(.a(new_n102_), .b(new_n97_), .c(new_n59_), .O(z4));
  oai21  g059(.a(new_n98_), .b(new_n80_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n92_), .c(new_n60_), .d(new_n79_), .O(new_n108_));
  nor3   g064(.a(new_n108_), .b(new_n97_), .c(new_n59_), .O(z5));
  oai21  g065(.a(new_n100_), .b(new_n80_), .c(x30), .O(new_n110_));
  inv1   g066(.a(x30), .O(new_n111_));
  nand4  g067(.a(new_n111_), .b(x29), .c(x28), .d(x27), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(new_n110_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n92_), .c(new_n60_), .d(new_n79_), .O(new_n114_));
  inv1   g070(.a(new_n114_), .O(new_n115_));
  nand3  g071(.a(new_n115_), .b(x25), .c(x01), .O(new_n116_));
  nand2  g072(.a(new_n116_), .b(new_n57_), .O(z6));
  nand3  g073(.a(new_n89_), .b(new_n86_), .c(new_n83_), .O(new_n118_));
  nor2   g074(.a(new_n118_), .b(new_n90_), .O(new_n119_));
  nand4  g075(.a(new_n60_), .b(new_n79_), .c(x25), .d(x01), .O(new_n120_));
  oai21  g076(.a(new_n120_), .b(new_n119_), .c(new_n57_), .O(new_n121_));
  inv1   g077(.a(x31), .O(new_n122_));
  nand2  g078(.a(new_n122_), .b(x30), .O(new_n123_));
  oai22  g079(.a(new_n123_), .b(new_n100_), .c(new_n122_), .d(x29), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(x27), .O(new_n125_));
  nand3  g081(.a(x30), .b(x28), .c(x27), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(x31), .c(x29), .O(new_n127_));
  nand3  g083(.a(new_n127_), .b(new_n125_), .c(new_n121_), .O(z7));
  inv1   g084(.a(x32), .O(new_n129_));
  nand2  g085(.a(new_n98_), .b(new_n80_), .O(new_n130_));
  nand3  g086(.a(new_n130_), .b(new_n111_), .c(x29), .O(new_n131_));
  aoi21  g087(.a(new_n98_), .b(new_n80_), .c(new_n105_), .O(new_n132_));
  oai21  g088(.a(new_n132_), .b(new_n111_), .c(new_n131_), .O(new_n133_));
  nand3  g089(.a(x19), .b(new_n87_), .c(x13), .O(new_n134_));
  inv1   g090(.a(x19), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(x18), .c(x14), .O(new_n136_));
  aoi21  g092(.a(new_n136_), .b(new_n134_), .c(new_n84_), .O(new_n137_));
  nand4  g093(.a(x19), .b(x18), .c(new_n84_), .d(x12), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  oai21  g095(.a(new_n139_), .b(new_n137_), .c(x16), .O(new_n140_));
  inv1   g096(.a(x16), .O(new_n141_));
  nand3  g097(.a(x19), .b(x18), .c(x17), .O(new_n142_));
  inv1   g098(.a(new_n142_), .O(new_n143_));
  nand3  g099(.a(new_n143_), .b(new_n141_), .c(x11), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n140_), .O(new_n145_));
  nand2  g101(.a(new_n145_), .b(x15), .O(new_n146_));
  inv1   g102(.a(x15), .O(new_n147_));
  nand4  g103(.a(new_n143_), .b(x16), .c(new_n147_), .d(x10), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n146_), .O(new_n149_));
  nand4  g105(.a(new_n149_), .b(new_n133_), .c(new_n129_), .d(x31), .O(new_n150_));
  xnor2a g106(.a(x30), .b(x29), .O(new_n151_));
  nand2  g107(.a(new_n151_), .b(x27), .O(new_n152_));
  xnor2a g108(.a(x30), .b(x28), .O(new_n153_));
  nand3  g109(.a(new_n153_), .b(x29), .c(new_n80_), .O(new_n154_));
  nand2  g110(.a(x16), .b(x10), .O(new_n155_));
  oai21  g111(.a(new_n155_), .b(new_n142_), .c(new_n147_), .O(new_n156_));
  nand4  g112(.a(x19), .b(x18), .c(x17), .d(x11), .O(new_n157_));
  nand3  g113(.a(x19), .b(x18), .c(x12), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n84_), .O(new_n159_));
  inv1   g115(.a(x13), .O(new_n160_));
  oai21  g116(.a(new_n135_), .b(new_n160_), .c(new_n87_), .O(new_n161_));
  inv1   g117(.a(x14), .O(new_n162_));
  nand2  g118(.a(new_n135_), .b(new_n162_), .O(new_n163_));
  nand2  g119(.a(x16), .b(x15), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n142_), .O(new_n165_));
  nor2   g121(.a(new_n165_), .b(new_n122_), .O(new_n166_));
  nand4  g122(.a(new_n166_), .b(new_n163_), .c(new_n161_), .d(new_n159_), .O(new_n167_));
  aoi21  g123(.a(new_n157_), .b(new_n141_), .c(new_n167_), .O(new_n168_));
  nand4  g124(.a(new_n168_), .b(new_n156_), .c(new_n154_), .d(new_n152_), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(x32), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n150_), .O(new_n171_));
  nand3  g127(.a(new_n171_), .b(new_n79_), .c(x00), .O(new_n172_));
  nand2  g128(.a(new_n172_), .b(new_n57_), .O(z8));
  nand2  g129(.a(new_n135_), .b(x14), .O(new_n174_));
  nand4  g130(.a(new_n174_), .b(x18), .c(x17), .d(x16), .O(new_n175_));
  nor2   g131(.a(new_n175_), .b(new_n147_), .O(new_n176_));
  nand2  g132(.a(x18), .b(x17), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n164_), .c(new_n135_), .O(new_n178_));
  oai21  g134(.a(new_n177_), .b(new_n155_), .c(new_n147_), .O(new_n179_));
  inv1   g135(.a(x11), .O(new_n180_));
  oai21  g136(.a(new_n177_), .b(new_n180_), .c(new_n141_), .O(new_n181_));
  nand2  g137(.a(x18), .b(x12), .O(new_n182_));
  aoi22  g138(.a(new_n182_), .b(new_n84_), .c(new_n87_), .d(new_n160_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n181_), .c(new_n179_), .d(new_n178_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n176_), .c(new_n57_), .O(new_n185_));
  nand2  g141(.a(new_n105_), .b(x27), .O(new_n186_));
  nand2  g142(.a(new_n122_), .b(x29), .O(new_n187_));
  aoi21  g143(.a(new_n187_), .b(new_n186_), .c(new_n111_), .O(new_n188_));
  nand2  g144(.a(x31), .b(new_n111_), .O(new_n189_));
  aoi21  g145(.a(new_n189_), .b(new_n187_), .c(new_n80_), .O(new_n190_));
  nand2  g146(.a(new_n111_), .b(x28), .O(new_n191_));
  nand3  g147(.a(x31), .b(new_n98_), .c(new_n80_), .O(new_n192_));
  aoi21  g148(.a(new_n192_), .b(new_n191_), .c(new_n105_), .O(new_n193_));
  nor3   g149(.a(new_n193_), .b(new_n190_), .c(new_n188_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n185_), .O(new_n195_));
  nand2  g151(.a(new_n195_), .b(x33), .O(new_n196_));
  aoi21  g152(.a(new_n144_), .b(new_n140_), .c(new_n147_), .O(new_n197_));
  inv1   g153(.a(new_n148_), .O(new_n198_));
  nand3  g154(.a(x31), .b(x30), .c(x29), .O(new_n199_));
  nand2  g155(.a(new_n122_), .b(new_n111_), .O(new_n200_));
  oai21  g156(.a(new_n200_), .b(x29), .c(new_n199_), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(x27), .O(new_n202_));
  nand3  g158(.a(x31), .b(x30), .c(x28), .O(new_n203_));
  oai21  g159(.a(new_n200_), .b(new_n130_), .c(new_n203_), .O(new_n204_));
  nand2  g160(.a(new_n204_), .b(x29), .O(new_n205_));
  nand2  g161(.a(new_n205_), .b(new_n202_), .O(new_n206_));
  oai21  g162(.a(new_n198_), .b(new_n197_), .c(new_n206_), .O(new_n207_));
  oai21  g163(.a(new_n207_), .b(x33), .c(new_n196_), .O(new_n208_));
  nand3  g164(.a(new_n208_), .b(new_n79_), .c(x00), .O(new_n209_));
  inv1   g165(.a(new_n209_), .O(z9));
endmodule


