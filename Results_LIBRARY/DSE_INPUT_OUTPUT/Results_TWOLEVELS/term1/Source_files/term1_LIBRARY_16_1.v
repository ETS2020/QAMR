// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n119_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n162_,
    new_n163_, new_n164_, new_n165_, new_n166_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_;
  inv1   g000(.a(x29), .O(new_n45_));
  nor2   g001(.a(new_n45_), .b(x27), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(x09), .O(new_n57_));
  inv1   g013(.a(new_n46_), .O(new_n58_));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n49_), .b(new_n52_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n49_), .b(new_n52_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n49_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n52_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n52_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n49_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  nand2  g033(.a(new_n77_), .b(new_n70_), .O(new_n78_));
  nand4  g034(.a(new_n78_), .b(new_n58_), .c(new_n57_), .d(x08), .O(new_n79_));
  inv1   g035(.a(new_n79_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  oai22  g037(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n82_));
  nor2   g038(.a(x22), .b(x17), .O(new_n83_));
  nor2   g039(.a(x23), .b(x18), .O(new_n84_));
  nor2   g040(.a(x24), .b(x19), .O(new_n85_));
  nor4   g041(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n86_));
  nor2   g042(.a(new_n86_), .b(new_n61_), .O(new_n87_));
  nand4  g043(.a(new_n87_), .b(new_n81_), .c(x25), .d(x01), .O(new_n88_));
  aoi21  g044(.a(new_n88_), .b(new_n45_), .c(x27), .O(z3));
  xor2a  g045(.a(x28), .b(x27), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n87_), .c(new_n81_), .d(x25), .O(new_n91_));
  oai21  g047(.a(new_n91_), .b(new_n60_), .c(new_n58_), .O(z4));
  nand3  g048(.a(new_n45_), .b(x28), .c(x27), .O(new_n93_));
  oai21  g049(.a(new_n45_), .b(x28), .c(new_n93_), .O(new_n94_));
  nand4  g050(.a(new_n94_), .b(new_n87_), .c(new_n81_), .d(x25), .O(new_n95_));
  oai21  g051(.a(new_n95_), .b(new_n60_), .c(new_n58_), .O(z5));
  inv1   g052(.a(x28), .O(new_n97_));
  nand2  g053(.a(x30), .b(new_n97_), .O(new_n98_));
  inv1   g054(.a(x30), .O(new_n99_));
  nand2  g055(.a(new_n99_), .b(x29), .O(new_n100_));
  oai21  g056(.a(new_n100_), .b(new_n97_), .c(new_n98_), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(x27), .O(new_n102_));
  nand2  g058(.a(x30), .b(new_n45_), .O(new_n103_));
  aoi21  g059(.a(new_n103_), .b(new_n102_), .c(new_n86_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n105_));
  nor2   g061(.a(new_n105_), .b(new_n60_), .O(z6));
  nor4   g062(.a(new_n85_), .b(new_n84_), .c(new_n83_), .d(new_n82_), .O(new_n107_));
  nand4  g063(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n108_));
  oai21  g064(.a(new_n108_), .b(new_n107_), .c(new_n58_), .O(new_n109_));
  oai21  g065(.a(new_n99_), .b(new_n97_), .c(x31), .O(new_n110_));
  nand2  g066(.a(x29), .b(x28), .O(new_n111_));
  inv1   g067(.a(x31), .O(new_n112_));
  nand2  g068(.a(new_n112_), .b(x30), .O(new_n113_));
  oai21  g069(.a(new_n113_), .b(new_n111_), .c(new_n110_), .O(new_n114_));
  aoi22  g070(.a(new_n114_), .b(x27), .c(x31), .d(new_n45_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n109_), .O(z7));
  inv1   g072(.a(x32), .O(new_n117_));
  inv1   g073(.a(x27), .O(new_n118_));
  oai21  g074(.a(new_n100_), .b(new_n118_), .c(new_n103_), .O(new_n119_));
  inv1   g075(.a(x17), .O(new_n120_));
  inv1   g076(.a(x18), .O(new_n121_));
  nand3  g077(.a(x19), .b(new_n121_), .c(x13), .O(new_n122_));
  inv1   g078(.a(x19), .O(new_n123_));
  nand3  g079(.a(new_n123_), .b(x18), .c(x14), .O(new_n124_));
  aoi21  g080(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  nand4  g081(.a(x19), .b(x18), .c(new_n120_), .d(x12), .O(new_n126_));
  inv1   g082(.a(new_n126_), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n125_), .c(x16), .O(new_n128_));
  inv1   g084(.a(x16), .O(new_n129_));
  nand3  g085(.a(x19), .b(x18), .c(x17), .O(new_n130_));
  inv1   g086(.a(new_n130_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n129_), .c(x11), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand2  g089(.a(new_n133_), .b(x15), .O(new_n134_));
  inv1   g090(.a(x15), .O(new_n135_));
  nand4  g091(.a(new_n131_), .b(x16), .c(new_n135_), .d(x10), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(new_n134_), .O(new_n137_));
  nand4  g093(.a(new_n137_), .b(new_n119_), .c(new_n117_), .d(x31), .O(new_n138_));
  nand2  g094(.a(x16), .b(x10), .O(new_n139_));
  nor2   g095(.a(new_n139_), .b(new_n130_), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(x15), .O(new_n141_));
  nand2  g097(.a(x19), .b(x18), .O(new_n142_));
  nand2  g098(.a(x17), .b(x11), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n142_), .c(new_n129_), .O(new_n144_));
  inv1   g100(.a(x12), .O(new_n145_));
  oai21  g101(.a(new_n142_), .b(new_n145_), .c(new_n120_), .O(new_n146_));
  inv1   g102(.a(x13), .O(new_n147_));
  oai21  g103(.a(new_n123_), .b(new_n147_), .c(new_n121_), .O(new_n148_));
  nor2   g104(.a(x19), .b(x14), .O(new_n149_));
  nand2  g105(.a(x16), .b(x15), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n130_), .c(x31), .O(new_n151_));
  nor2   g107(.a(new_n151_), .b(new_n149_), .O(new_n152_));
  nand4  g108(.a(new_n152_), .b(new_n148_), .c(new_n146_), .d(new_n144_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n141_), .c(new_n58_), .O(new_n154_));
  nand3  g110(.a(x30), .b(x29), .c(x27), .O(new_n155_));
  nand2  g111(.a(new_n99_), .b(new_n45_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n155_), .c(new_n154_), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(x32), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n138_), .O(new_n159_));
  nand3  g115(.a(new_n159_), .b(new_n81_), .c(x00), .O(new_n160_));
  inv1   g116(.a(new_n160_), .O(z8));
  inv1   g117(.a(x33), .O(new_n162_));
  nand4  g118(.a(x31), .b(x30), .c(x29), .d(x27), .O(new_n163_));
  nand2  g119(.a(new_n112_), .b(new_n99_), .O(new_n164_));
  oai21  g120(.a(new_n164_), .b(x29), .c(new_n163_), .O(new_n165_));
  nand3  g121(.a(new_n165_), .b(new_n137_), .c(new_n162_), .O(new_n166_));
  nand2  g122(.a(new_n123_), .b(x14), .O(new_n167_));
  nand4  g123(.a(new_n167_), .b(x18), .c(x17), .d(x16), .O(new_n168_));
  nor2   g124(.a(new_n168_), .b(new_n135_), .O(new_n169_));
  nand2  g125(.a(x18), .b(x17), .O(new_n170_));
  oai21  g126(.a(new_n170_), .b(new_n150_), .c(new_n123_), .O(new_n171_));
  oai21  g127(.a(new_n170_), .b(new_n139_), .c(new_n135_), .O(new_n172_));
  inv1   g128(.a(x11), .O(new_n173_));
  oai21  g129(.a(new_n170_), .b(new_n173_), .c(new_n129_), .O(new_n174_));
  nand2  g130(.a(x18), .b(x12), .O(new_n175_));
  aoi22  g131(.a(new_n175_), .b(new_n120_), .c(new_n121_), .d(new_n147_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n174_), .c(new_n172_), .d(new_n171_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n169_), .c(new_n58_), .O(new_n178_));
  nand2  g134(.a(new_n113_), .b(new_n100_), .O(new_n179_));
  aoi22  g135(.a(new_n179_), .b(x27), .c(new_n164_), .d(new_n45_), .O(new_n180_));
  nand2  g136(.a(new_n180_), .b(new_n178_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(x33), .O(new_n182_));
  nand2  g138(.a(new_n182_), .b(new_n166_), .O(new_n183_));
  nand3  g139(.a(new_n183_), .b(new_n81_), .c(x00), .O(new_n184_));
  inv1   g140(.a(new_n184_), .O(z9));
endmodule


