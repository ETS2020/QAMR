// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_,
    new_n98_, new_n99_, new_n100_, new_n101_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n167_, new_n168_,
    new_n169_, new_n170_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_;
  nand2  g000(.a(x30), .b(x25), .O(new_n45_));
  inv1   g001(.a(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  xnor2a g012(.a(x07), .b(x04), .O(new_n57_));
  inv1   g013(.a(x01), .O(new_n58_));
  nand2  g014(.a(x03), .b(x02), .O(new_n59_));
  nand2  g015(.a(new_n49_), .b(new_n52_), .O(new_n60_));
  aoi21  g016(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g017(.a(x05), .b(new_n49_), .O(new_n62_));
  nand3  g018(.a(new_n62_), .b(x06), .c(new_n52_), .O(new_n63_));
  nand2  g019(.a(x06), .b(new_n52_), .O(new_n64_));
  nand3  g020(.a(new_n64_), .b(x05), .c(new_n49_), .O(new_n65_));
  aoi21  g021(.a(new_n65_), .b(new_n63_), .c(x01), .O(new_n66_));
  oai21  g022(.a(new_n66_), .b(new_n61_), .c(new_n57_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  xnor2a g024(.a(x03), .b(x02), .O(new_n69_));
  nor2   g025(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  nand2  g026(.a(new_n64_), .b(new_n62_), .O(new_n71_));
  nand4  g027(.a(x06), .b(x05), .c(new_n49_), .d(new_n52_), .O(new_n72_));
  aoi21  g028(.a(new_n72_), .b(new_n71_), .c(x01), .O(new_n73_));
  oai21  g029(.a(new_n73_), .b(new_n70_), .c(new_n68_), .O(new_n74_));
  aoi21  g030(.a(new_n74_), .b(new_n67_), .c(x09), .O(new_n75_));
  nand2  g031(.a(new_n75_), .b(x08), .O(new_n76_));
  nand2  g032(.a(new_n76_), .b(new_n45_), .O(z2));
  inv1   g033(.a(x25), .O(new_n78_));
  inv1   g034(.a(x30), .O(new_n79_));
  inv1   g035(.a(x26), .O(new_n80_));
  inv1   g036(.a(x27), .O(new_n81_));
  nor2   g037(.a(x20), .b(x15), .O(new_n82_));
  nor2   g038(.a(x21), .b(x16), .O(new_n83_));
  nor2   g039(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  inv1   g040(.a(x17), .O(new_n85_));
  inv1   g041(.a(x22), .O(new_n86_));
  nand2  g042(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  inv1   g043(.a(x18), .O(new_n88_));
  inv1   g044(.a(x23), .O(new_n89_));
  nand2  g045(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  inv1   g046(.a(x19), .O(new_n91_));
  inv1   g047(.a(x24), .O(new_n92_));
  nand2  g048(.a(new_n92_), .b(new_n91_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n90_), .c(new_n87_), .d(new_n84_), .O(new_n94_));
  and2   g050(.a(new_n94_), .b(new_n59_), .O(new_n95_));
  nand4  g051(.a(new_n95_), .b(new_n81_), .c(new_n80_), .d(x01), .O(new_n96_));
  aoi21  g052(.a(new_n96_), .b(new_n79_), .c(new_n78_), .O(z3));
  xor2a  g053(.a(x28), .b(x27), .O(new_n98_));
  nand3  g054(.a(new_n98_), .b(new_n95_), .c(new_n79_), .O(new_n99_));
  inv1   g055(.a(new_n99_), .O(new_n100_));
  nand4  g056(.a(new_n100_), .b(new_n80_), .c(x25), .d(x01), .O(new_n101_));
  inv1   g057(.a(new_n101_), .O(z4));
  inv1   g058(.a(x28), .O(new_n103_));
  oai21  g059(.a(new_n103_), .b(new_n81_), .c(x29), .O(new_n104_));
  inv1   g060(.a(x29), .O(new_n105_));
  nand3  g061(.a(new_n105_), .b(x28), .c(x27), .O(new_n106_));
  nand2  g062(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand4  g063(.a(new_n107_), .b(new_n94_), .c(new_n59_), .d(new_n79_), .O(new_n108_));
  inv1   g064(.a(new_n108_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n80_), .c(x25), .d(x01), .O(new_n110_));
  inv1   g066(.a(new_n110_), .O(z5));
  nand3  g067(.a(new_n95_), .b(new_n79_), .c(x29), .O(new_n112_));
  nor2   g068(.a(new_n112_), .b(new_n103_), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(x27), .c(new_n80_), .d(x25), .O(new_n114_));
  nor2   g070(.a(new_n114_), .b(new_n58_), .O(z6));
  inv1   g071(.a(new_n93_), .O(new_n116_));
  nand3  g072(.a(new_n90_), .b(new_n87_), .c(new_n84_), .O(new_n117_));
  nand3  g073(.a(new_n59_), .b(x25), .c(x01), .O(new_n118_));
  nor4   g074(.a(new_n118_), .b(new_n46_), .c(x31), .d(x26), .O(new_n119_));
  oai21  g075(.a(new_n117_), .b(new_n116_), .c(new_n119_), .O(z7));
  nand2  g076(.a(x16), .b(x10), .O(new_n121_));
  nand3  g077(.a(x19), .b(x18), .c(x17), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nor2   g079(.a(new_n123_), .b(x15), .O(new_n124_));
  inv1   g080(.a(x16), .O(new_n125_));
  nand2  g081(.a(x19), .b(x18), .O(new_n126_));
  nand2  g082(.a(x17), .b(x11), .O(new_n127_));
  oai21  g083(.a(new_n127_), .b(new_n126_), .c(new_n125_), .O(new_n128_));
  inv1   g084(.a(x12), .O(new_n129_));
  oai21  g085(.a(new_n126_), .b(new_n129_), .c(new_n85_), .O(new_n130_));
  nand2  g086(.a(x19), .b(x13), .O(new_n131_));
  nand2  g087(.a(new_n131_), .b(new_n88_), .O(new_n132_));
  inv1   g088(.a(x14), .O(new_n133_));
  nand2  g089(.a(x16), .b(x15), .O(new_n134_));
  oai21  g090(.a(new_n134_), .b(new_n122_), .c(x31), .O(new_n135_));
  aoi21  g091(.a(new_n91_), .b(new_n133_), .c(new_n135_), .O(new_n136_));
  nand4  g092(.a(new_n136_), .b(new_n132_), .c(new_n130_), .d(new_n128_), .O(new_n137_));
  oai21  g093(.a(new_n137_), .b(new_n124_), .c(new_n45_), .O(new_n138_));
  nand2  g094(.a(new_n103_), .b(new_n81_), .O(new_n139_));
  nand2  g095(.a(new_n139_), .b(x30), .O(new_n140_));
  nor2   g096(.a(new_n140_), .b(new_n105_), .O(new_n141_));
  oai21  g097(.a(x28), .b(x27), .c(x29), .O(new_n142_));
  aoi22  g098(.a(new_n142_), .b(new_n79_), .c(new_n141_), .d(new_n78_), .O(new_n143_));
  nand2  g099(.a(new_n143_), .b(new_n138_), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(x32), .O(new_n145_));
  inv1   g101(.a(x32), .O(new_n146_));
  nand3  g102(.a(new_n142_), .b(x30), .c(new_n78_), .O(new_n147_));
  nand3  g103(.a(new_n139_), .b(new_n79_), .c(x29), .O(new_n148_));
  nand2  g104(.a(new_n148_), .b(new_n147_), .O(new_n149_));
  nand3  g105(.a(x19), .b(new_n88_), .c(x13), .O(new_n150_));
  nand3  g106(.a(new_n91_), .b(x18), .c(x14), .O(new_n151_));
  aoi21  g107(.a(new_n151_), .b(new_n150_), .c(new_n85_), .O(new_n152_));
  nand4  g108(.a(x19), .b(x18), .c(new_n85_), .d(x12), .O(new_n153_));
  inv1   g109(.a(new_n153_), .O(new_n154_));
  oai21  g110(.a(new_n154_), .b(new_n152_), .c(x16), .O(new_n155_));
  inv1   g111(.a(new_n122_), .O(new_n156_));
  nand3  g112(.a(new_n156_), .b(new_n125_), .c(x11), .O(new_n157_));
  nand2  g113(.a(new_n157_), .b(new_n155_), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(x15), .O(new_n159_));
  inv1   g115(.a(x15), .O(new_n160_));
  nand4  g116(.a(new_n156_), .b(x16), .c(new_n160_), .d(x10), .O(new_n161_));
  nand2  g117(.a(new_n161_), .b(new_n159_), .O(new_n162_));
  nand4  g118(.a(new_n162_), .b(new_n149_), .c(new_n146_), .d(x31), .O(new_n163_));
  nand2  g119(.a(new_n163_), .b(new_n145_), .O(new_n164_));
  nand3  g120(.a(new_n164_), .b(new_n80_), .c(x00), .O(new_n165_));
  inv1   g121(.a(new_n165_), .O(z8));
  inv1   g122(.a(new_n134_), .O(new_n167_));
  nand2  g123(.a(x18), .b(x17), .O(new_n168_));
  inv1   g124(.a(new_n168_), .O(new_n169_));
  aoi21  g125(.a(new_n169_), .b(new_n167_), .c(x19), .O(new_n170_));
  oai21  g126(.a(new_n168_), .b(new_n121_), .c(new_n160_), .O(new_n171_));
  inv1   g127(.a(x11), .O(new_n172_));
  oai21  g128(.a(new_n168_), .b(new_n172_), .c(new_n125_), .O(new_n173_));
  oai21  g129(.a(new_n88_), .b(new_n129_), .c(new_n85_), .O(new_n174_));
  nor2   g130(.a(x18), .b(x13), .O(new_n175_));
  aoi21  g131(.a(new_n167_), .b(new_n156_), .c(new_n175_), .O(new_n176_));
  nand4  g132(.a(new_n176_), .b(new_n174_), .c(new_n173_), .d(new_n171_), .O(new_n177_));
  oai21  g133(.a(new_n177_), .b(new_n170_), .c(new_n45_), .O(new_n178_));
  nand2  g134(.a(new_n78_), .b(new_n91_), .O(new_n179_));
  nand3  g135(.a(new_n79_), .b(x18), .c(x17), .O(new_n180_));
  oai21  g136(.a(new_n180_), .b(new_n134_), .c(new_n179_), .O(new_n181_));
  nand2  g137(.a(new_n181_), .b(new_n133_), .O(new_n182_));
  inv1   g138(.a(x31), .O(new_n183_));
  nand2  g139(.a(new_n142_), .b(new_n183_), .O(new_n184_));
  nand2  g140(.a(new_n184_), .b(new_n79_), .O(new_n185_));
  nand2  g141(.a(new_n142_), .b(x31), .O(new_n186_));
  nand2  g142(.a(new_n183_), .b(x30), .O(new_n187_));
  nand2  g143(.a(new_n187_), .b(new_n186_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n78_), .O(new_n189_));
  nand3  g145(.a(new_n189_), .b(new_n185_), .c(new_n182_), .O(new_n190_));
  inv1   g146(.a(new_n190_), .O(new_n191_));
  nand2  g147(.a(new_n191_), .b(new_n178_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(x33), .O(new_n193_));
  aoi21  g149(.a(new_n157_), .b(new_n155_), .c(new_n160_), .O(new_n194_));
  inv1   g150(.a(new_n161_), .O(new_n195_));
  nand4  g151(.a(new_n139_), .b(x31), .c(x30), .d(x29), .O(new_n196_));
  oai22  g152(.a(new_n196_), .b(x25), .c(new_n184_), .d(x30), .O(new_n197_));
  oai21  g153(.a(new_n195_), .b(new_n194_), .c(new_n197_), .O(new_n198_));
  oai21  g154(.a(new_n198_), .b(x33), .c(new_n193_), .O(new_n199_));
  nand3  g155(.a(new_n199_), .b(new_n80_), .c(x00), .O(new_n200_));
  inv1   g156(.a(new_n200_), .O(z9));
endmodule


