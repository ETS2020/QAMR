// Benchmark "FAU" written by ABC on Thu Aug 20 01:02:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n135_, new_n136_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n171_, new_n172_, new_n173_, new_n174_,
    new_n175_, new_n176_, new_n177_, new_n178_, new_n179_, new_n180_,
    new_n181_, new_n182_, new_n183_, new_n184_, new_n185_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_;
  inv1   g000(.a(x27), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nand2  g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nor2   g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n48_), .O(z1));
  inv1   g014(.a(x09), .O(new_n59_));
  xnor2a g015(.a(x07), .b(x04), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n51_), .b(new_n54_), .O(new_n62_));
  inv1   g018(.a(new_n62_), .O(new_n63_));
  nand2  g019(.a(new_n51_), .b(new_n54_), .O(new_n64_));
  aoi21  g020(.a(new_n64_), .b(new_n63_), .c(new_n61_), .O(new_n65_));
  nand2  g021(.a(x05), .b(new_n51_), .O(new_n66_));
  nand3  g022(.a(new_n66_), .b(x06), .c(new_n54_), .O(new_n67_));
  nand2  g023(.a(x06), .b(new_n54_), .O(new_n68_));
  nand3  g024(.a(new_n68_), .b(x05), .c(new_n51_), .O(new_n69_));
  aoi21  g025(.a(new_n69_), .b(new_n67_), .c(x01), .O(new_n70_));
  oai21  g026(.a(new_n70_), .b(new_n65_), .c(new_n60_), .O(new_n71_));
  xor2a  g027(.a(x07), .b(x04), .O(new_n72_));
  xnor2a g028(.a(x03), .b(x02), .O(new_n73_));
  nor2   g029(.a(new_n73_), .b(new_n61_), .O(new_n74_));
  nand2  g030(.a(new_n68_), .b(new_n66_), .O(new_n75_));
  nand4  g031(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n76_));
  aoi21  g032(.a(new_n76_), .b(new_n75_), .c(x01), .O(new_n77_));
  oai21  g033(.a(new_n77_), .b(new_n74_), .c(new_n72_), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(new_n71_), .O(new_n79_));
  nand4  g035(.a(new_n79_), .b(new_n47_), .c(new_n59_), .d(x08), .O(new_n80_));
  inv1   g036(.a(new_n80_), .O(z2));
  inv1   g037(.a(x26), .O(new_n82_));
  oai22  g038(.a(x21), .b(x16), .c(x20), .d(x15), .O(new_n83_));
  inv1   g039(.a(new_n83_), .O(new_n84_));
  nor2   g040(.a(x22), .b(x17), .O(new_n85_));
  inv1   g041(.a(new_n85_), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nor2   g044(.a(x24), .b(x19), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand4  g046(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n84_), .O(new_n91_));
  inv1   g047(.a(new_n91_), .O(new_n92_));
  nor2   g048(.a(new_n92_), .b(new_n62_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n82_), .c(x25), .d(x01), .O(new_n94_));
  aoi21  g050(.a(new_n94_), .b(x30), .c(x27), .O(z3));
  xor2a  g051(.a(x28), .b(x27), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n93_), .c(new_n82_), .d(x25), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n61_), .c(new_n47_), .O(z4));
  inv1   g054(.a(x25), .O(new_n99_));
  xnor2a g055(.a(x29), .b(x28), .O(new_n100_));
  nand3  g056(.a(x30), .b(x29), .c(new_n45_), .O(new_n101_));
  oai21  g057(.a(new_n100_), .b(new_n45_), .c(new_n101_), .O(new_n102_));
  nand4  g058(.a(new_n102_), .b(new_n91_), .c(new_n63_), .d(new_n82_), .O(new_n103_));
  nor3   g059(.a(new_n103_), .b(new_n99_), .c(new_n61_), .O(z5));
  nand3  g060(.a(new_n46_), .b(x28), .c(x27), .O(new_n105_));
  oai21  g061(.a(new_n46_), .b(x27), .c(new_n105_), .O(new_n106_));
  nand2  g062(.a(x29), .b(x28), .O(new_n107_));
  aoi22  g063(.a(new_n107_), .b(x30), .c(new_n106_), .d(x29), .O(new_n108_));
  nor2   g064(.a(new_n108_), .b(new_n92_), .O(new_n109_));
  nand4  g065(.a(new_n109_), .b(new_n63_), .c(new_n82_), .d(x25), .O(new_n110_));
  nor2   g066(.a(new_n110_), .b(new_n61_), .O(z6));
  nor2   g067(.a(x31), .b(new_n46_), .O(new_n112_));
  nand3  g068(.a(x30), .b(x29), .c(x28), .O(new_n113_));
  nand4  g069(.a(new_n90_), .b(new_n88_), .c(new_n86_), .d(new_n84_), .O(new_n114_));
  nand4  g070(.a(new_n112_), .b(x29), .c(x28), .d(x27), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n82_), .O(new_n116_));
  nor2   g072(.a(new_n116_), .b(new_n99_), .O(new_n117_));
  nand4  g073(.a(new_n117_), .b(new_n114_), .c(new_n63_), .d(x01), .O(new_n118_));
  aoi21  g074(.a(new_n113_), .b(x31), .c(new_n118_), .O(new_n119_));
  oai21  g075(.a(new_n112_), .b(x27), .c(new_n119_), .O(z7));
  inv1   g076(.a(x32), .O(new_n121_));
  nor2   g077(.a(x28), .b(x27), .O(new_n122_));
  inv1   g078(.a(new_n122_), .O(new_n123_));
  inv1   g079(.a(x29), .O(new_n124_));
  nand2  g080(.a(x30), .b(new_n124_), .O(new_n125_));
  nand2  g081(.a(new_n46_), .b(x29), .O(new_n126_));
  nand3  g082(.a(new_n126_), .b(new_n125_), .c(new_n123_), .O(new_n127_));
  inv1   g083(.a(x17), .O(new_n128_));
  inv1   g084(.a(x18), .O(new_n129_));
  nand3  g085(.a(x19), .b(new_n129_), .c(x13), .O(new_n130_));
  inv1   g086(.a(x19), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(x18), .c(x14), .O(new_n132_));
  aoi21  g088(.a(new_n132_), .b(new_n130_), .c(new_n128_), .O(new_n133_));
  nand4  g089(.a(x19), .b(x18), .c(new_n128_), .d(x12), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(new_n135_));
  oai21  g091(.a(new_n135_), .b(new_n133_), .c(x16), .O(new_n136_));
  inv1   g092(.a(x16), .O(new_n137_));
  nand3  g093(.a(x19), .b(x18), .c(x17), .O(new_n138_));
  inv1   g094(.a(new_n138_), .O(new_n139_));
  nand3  g095(.a(new_n139_), .b(new_n137_), .c(x11), .O(new_n140_));
  nand2  g096(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand2  g097(.a(new_n141_), .b(x15), .O(new_n142_));
  inv1   g098(.a(x15), .O(new_n143_));
  nand4  g099(.a(new_n139_), .b(x16), .c(new_n143_), .d(x10), .O(new_n144_));
  nand2  g100(.a(new_n144_), .b(new_n142_), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n127_), .c(new_n121_), .d(x31), .O(new_n146_));
  inv1   g102(.a(x28), .O(new_n147_));
  nand2  g103(.a(new_n147_), .b(new_n45_), .O(new_n148_));
  nand3  g104(.a(new_n148_), .b(x30), .c(x29), .O(new_n149_));
  nand2  g105(.a(x16), .b(x10), .O(new_n150_));
  oai21  g106(.a(new_n150_), .b(new_n138_), .c(new_n143_), .O(new_n151_));
  nor2   g107(.a(new_n131_), .b(new_n129_), .O(new_n152_));
  nand3  g108(.a(new_n152_), .b(x17), .c(x11), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(new_n137_), .O(new_n154_));
  aoi21  g110(.a(new_n152_), .b(x12), .c(x17), .O(new_n155_));
  inv1   g111(.a(x13), .O(new_n156_));
  oai21  g112(.a(new_n131_), .b(new_n156_), .c(new_n129_), .O(new_n157_));
  inv1   g113(.a(x14), .O(new_n158_));
  nand2  g114(.a(new_n131_), .b(new_n158_), .O(new_n159_));
  nor2   g115(.a(new_n137_), .b(new_n143_), .O(new_n160_));
  nand2  g116(.a(new_n160_), .b(new_n139_), .O(new_n161_));
  inv1   g117(.a(x31), .O(new_n162_));
  aoi21  g118(.a(new_n46_), .b(new_n124_), .c(new_n162_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n161_), .c(new_n159_), .d(new_n157_), .O(new_n164_));
  nor2   g120(.a(new_n164_), .b(new_n155_), .O(new_n165_));
  nand4  g121(.a(new_n165_), .b(new_n154_), .c(new_n151_), .d(new_n149_), .O(new_n166_));
  nand2  g122(.a(new_n166_), .b(x32), .O(new_n167_));
  nand2  g123(.a(new_n167_), .b(new_n146_), .O(new_n168_));
  nand3  g124(.a(new_n168_), .b(new_n82_), .c(x00), .O(new_n169_));
  nand2  g125(.a(new_n169_), .b(new_n47_), .O(z8));
  nand2  g126(.a(new_n46_), .b(new_n45_), .O(new_n171_));
  nand2  g127(.a(x18), .b(x17), .O(new_n172_));
  inv1   g128(.a(new_n172_), .O(new_n173_));
  aoi21  g129(.a(new_n173_), .b(new_n160_), .c(x19), .O(new_n174_));
  oai21  g130(.a(new_n172_), .b(new_n150_), .c(new_n143_), .O(new_n175_));
  inv1   g131(.a(x11), .O(new_n176_));
  oai21  g132(.a(new_n172_), .b(new_n176_), .c(new_n137_), .O(new_n177_));
  nand2  g133(.a(x18), .b(x12), .O(new_n178_));
  nand2  g134(.a(new_n178_), .b(new_n128_), .O(new_n179_));
  nor2   g135(.a(new_n143_), .b(x14), .O(new_n180_));
  nand3  g136(.a(x18), .b(x17), .c(x16), .O(new_n181_));
  inv1   g137(.a(new_n181_), .O(new_n182_));
  aoi22  g138(.a(new_n182_), .b(new_n180_), .c(new_n129_), .d(new_n156_), .O(new_n183_));
  nand4  g139(.a(new_n183_), .b(new_n179_), .c(new_n177_), .d(new_n175_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n174_), .c(new_n171_), .O(new_n185_));
  nor3   g141(.a(new_n48_), .b(new_n131_), .c(new_n129_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(x17), .c(x16), .d(x15), .O(new_n187_));
  nand2  g143(.a(new_n162_), .b(new_n124_), .O(new_n188_));
  nand3  g144(.a(new_n188_), .b(new_n46_), .c(x27), .O(new_n189_));
  nand2  g145(.a(x31), .b(x29), .O(new_n190_));
  oai21  g146(.a(new_n190_), .b(new_n122_), .c(x30), .O(new_n191_));
  nand4  g147(.a(new_n191_), .b(new_n189_), .c(new_n187_), .d(new_n185_), .O(new_n192_));
  nand2  g148(.a(new_n192_), .b(x33), .O(new_n193_));
  inv1   g149(.a(x33), .O(new_n194_));
  nand2  g150(.a(x31), .b(x30), .O(new_n195_));
  nand3  g151(.a(x31), .b(x30), .c(x29), .O(new_n196_));
  inv1   g152(.a(new_n196_), .O(new_n197_));
  nor3   g153(.a(x31), .b(x30), .c(x29), .O(new_n198_));
  oai21  g154(.a(new_n198_), .b(new_n197_), .c(x27), .O(new_n199_));
  oai21  g155(.a(new_n195_), .b(new_n107_), .c(new_n199_), .O(new_n200_));
  nand3  g156(.a(new_n200_), .b(new_n145_), .c(new_n194_), .O(new_n201_));
  nand2  g157(.a(new_n201_), .b(new_n193_), .O(new_n202_));
  nand3  g158(.a(new_n202_), .b(new_n82_), .c(x00), .O(new_n203_));
  inv1   g159(.a(new_n203_), .O(z9));
endmodule


