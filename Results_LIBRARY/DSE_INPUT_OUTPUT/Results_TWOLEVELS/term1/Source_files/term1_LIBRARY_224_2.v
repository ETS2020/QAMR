// Benchmark "FAU" written by ABC on Thu Aug 20 01:04:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_,
    new_n97_, new_n98_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n127_, new_n128_, new_n129_,
    new_n130_, new_n131_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n138_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n185_,
    new_n186_, new_n187_, new_n188_, new_n189_, new_n190_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_, new_n208_, new_n209_, new_n210_,
    new_n211_, new_n212_, new_n213_, new_n214_, new_n215_, new_n216_,
    new_n217_;
  inv1   g000(.a(x22), .O(new_n45_));
  nor2   g001(.a(x26), .b(new_n45_), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(x32), .O(z0));
  nor2   g003(.a(x32), .b(x03), .O(new_n48_));
  inv1   g004(.a(x03), .O(new_n49_));
  nor2   g005(.a(x33), .b(new_n49_), .O(new_n50_));
  oai21  g006(.a(new_n50_), .b(new_n48_), .c(x02), .O(new_n51_));
  inv1   g007(.a(x02), .O(new_n52_));
  nor2   g008(.a(x32), .b(new_n49_), .O(new_n53_));
  nor2   g009(.a(x33), .b(x03), .O(new_n54_));
  oai21  g010(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(new_n55_));
  aoi21  g011(.a(new_n55_), .b(new_n51_), .c(new_n46_), .O(z1));
  inv1   g012(.a(new_n46_), .O(new_n57_));
  xnor2a g013(.a(x07), .b(x04), .O(new_n58_));
  xor2a  g014(.a(x03), .b(x02), .O(new_n59_));
  nand2  g015(.a(new_n59_), .b(x01), .O(new_n60_));
  inv1   g016(.a(x01), .O(new_n61_));
  nor2   g017(.a(new_n49_), .b(new_n52_), .O(new_n62_));
  nor2   g018(.a(x03), .b(x02), .O(new_n63_));
  nand3  g019(.a(new_n63_), .b(x06), .c(x05), .O(new_n64_));
  oai21  g020(.a(x06), .b(x05), .c(new_n64_), .O(new_n65_));
  oai21  g021(.a(new_n65_), .b(new_n62_), .c(new_n61_), .O(new_n66_));
  aoi21  g022(.a(new_n66_), .b(new_n60_), .c(new_n58_), .O(new_n67_));
  xor2a  g023(.a(x07), .b(x04), .O(new_n68_));
  oai21  g024(.a(new_n63_), .b(new_n62_), .c(x01), .O(new_n69_));
  nand3  g025(.a(x05), .b(new_n49_), .c(x02), .O(new_n70_));
  nand3  g026(.a(x06), .b(x03), .c(new_n52_), .O(new_n71_));
  nand2  g027(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand2  g028(.a(new_n72_), .b(new_n61_), .O(new_n73_));
  aoi21  g029(.a(new_n73_), .b(new_n69_), .c(new_n68_), .O(new_n74_));
  oai21  g030(.a(new_n74_), .b(new_n67_), .c(new_n57_), .O(new_n75_));
  oai22  g031(.a(x06), .b(new_n49_), .c(x05), .d(new_n52_), .O(new_n76_));
  nand3  g032(.a(new_n76_), .b(new_n45_), .c(new_n61_), .O(new_n77_));
  inv1   g033(.a(x05), .O(new_n78_));
  nand3  g034(.a(new_n78_), .b(new_n49_), .c(x02), .O(new_n79_));
  inv1   g035(.a(x06), .O(new_n80_));
  nand3  g036(.a(new_n80_), .b(x03), .c(new_n52_), .O(new_n81_));
  nand2  g037(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g038(.a(new_n82_), .b(x26), .O(new_n83_));
  aoi21  g039(.a(new_n83_), .b(new_n77_), .c(new_n58_), .O(new_n84_));
  nand2  g040(.a(x06), .b(new_n78_), .O(new_n85_));
  nand2  g041(.a(new_n80_), .b(x05), .O(new_n86_));
  oai22  g042(.a(new_n86_), .b(x03), .c(new_n85_), .d(x02), .O(new_n87_));
  nand3  g043(.a(new_n87_), .b(new_n45_), .c(new_n61_), .O(new_n88_));
  nand2  g044(.a(new_n86_), .b(new_n85_), .O(new_n89_));
  nand4  g045(.a(new_n89_), .b(x26), .c(new_n49_), .d(new_n52_), .O(new_n90_));
  aoi21  g046(.a(new_n90_), .b(new_n88_), .c(new_n68_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n84_), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n75_), .c(x09), .O(new_n93_));
  nand2  g049(.a(new_n93_), .b(x08), .O(new_n94_));
  inv1   g050(.a(new_n94_), .O(z2));
  inv1   g051(.a(x27), .O(new_n96_));
  inv1   g052(.a(x17), .O(new_n97_));
  nor2   g053(.a(x20), .b(x15), .O(new_n98_));
  nor2   g054(.a(x21), .b(x16), .O(new_n99_));
  inv1   g055(.a(x18), .O(new_n100_));
  inv1   g056(.a(x23), .O(new_n101_));
  nand2  g057(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  inv1   g058(.a(x19), .O(new_n103_));
  inv1   g059(.a(x24), .O(new_n104_));
  nand2  g060(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g061(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nor4   g062(.a(new_n106_), .b(new_n99_), .c(new_n98_), .d(new_n97_), .O(new_n107_));
  nor2   g063(.a(new_n107_), .b(new_n62_), .O(new_n108_));
  nand4  g064(.a(new_n108_), .b(new_n96_), .c(x25), .d(x01), .O(new_n109_));
  aoi21  g065(.a(new_n109_), .b(new_n45_), .c(x26), .O(z3));
  inv1   g066(.a(x26), .O(new_n111_));
  xor2a  g067(.a(x28), .b(x27), .O(new_n112_));
  nand3  g068(.a(new_n112_), .b(new_n108_), .c(new_n111_), .O(new_n113_));
  inv1   g069(.a(new_n113_), .O(new_n114_));
  nand4  g070(.a(new_n114_), .b(x25), .c(new_n45_), .d(x01), .O(new_n115_));
  inv1   g071(.a(new_n115_), .O(z4));
  inv1   g072(.a(new_n62_), .O(new_n117_));
  inv1   g073(.a(x28), .O(new_n118_));
  nor2   g074(.a(new_n118_), .b(new_n96_), .O(new_n119_));
  inv1   g075(.a(new_n119_), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(x29), .O(new_n121_));
  inv1   g077(.a(x29), .O(new_n122_));
  nand3  g078(.a(new_n122_), .b(x28), .c(x27), .O(new_n123_));
  aoi21  g079(.a(new_n123_), .b(new_n121_), .c(new_n107_), .O(new_n124_));
  nand4  g080(.a(new_n124_), .b(new_n117_), .c(x25), .d(x01), .O(new_n125_));
  aoi21  g081(.a(new_n125_), .b(new_n45_), .c(x26), .O(z5));
  nand3  g082(.a(x29), .b(x28), .c(x27), .O(new_n127_));
  nand2  g083(.a(new_n127_), .b(x30), .O(new_n128_));
  inv1   g084(.a(x30), .O(new_n129_));
  nand3  g085(.a(new_n119_), .b(new_n129_), .c(x29), .O(new_n130_));
  aoi21  g086(.a(new_n130_), .b(new_n128_), .c(new_n107_), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n117_), .c(new_n111_), .O(new_n132_));
  inv1   g088(.a(new_n132_), .O(new_n133_));
  nand4  g089(.a(new_n133_), .b(x25), .c(new_n45_), .d(x01), .O(new_n134_));
  inv1   g090(.a(new_n134_), .O(z6));
  nand3  g091(.a(new_n119_), .b(x30), .c(x29), .O(new_n136_));
  nand2  g092(.a(new_n136_), .b(x31), .O(new_n137_));
  nor2   g093(.a(new_n99_), .b(new_n98_), .O(new_n138_));
  nand4  g094(.a(new_n138_), .b(new_n105_), .c(new_n102_), .d(x17), .O(new_n139_));
  nand3  g095(.a(new_n117_), .b(x25), .c(x01), .O(new_n140_));
  inv1   g096(.a(new_n140_), .O(new_n141_));
  inv1   g097(.a(x31), .O(new_n142_));
  nand3  g098(.a(new_n142_), .b(x30), .c(x29), .O(new_n143_));
  oai21  g099(.a(new_n143_), .b(new_n120_), .c(new_n57_), .O(new_n144_));
  nor2   g100(.a(new_n144_), .b(x26), .O(new_n145_));
  nand4  g101(.a(new_n145_), .b(new_n141_), .c(new_n139_), .d(new_n137_), .O(z7));
  nand2  g102(.a(new_n118_), .b(new_n96_), .O(new_n147_));
  inv1   g103(.a(x32), .O(new_n148_));
  nand3  g104(.a(x19), .b(new_n100_), .c(x13), .O(new_n149_));
  nand3  g105(.a(new_n103_), .b(x18), .c(x14), .O(new_n150_));
  aoi21  g106(.a(new_n150_), .b(new_n149_), .c(new_n97_), .O(new_n151_));
  nand4  g107(.a(x19), .b(x18), .c(new_n97_), .d(x12), .O(new_n152_));
  inv1   g108(.a(new_n152_), .O(new_n153_));
  oai21  g109(.a(new_n153_), .b(new_n151_), .c(x16), .O(new_n154_));
  inv1   g110(.a(x16), .O(new_n155_));
  nand3  g111(.a(x19), .b(x18), .c(x17), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nand3  g113(.a(new_n157_), .b(new_n155_), .c(x11), .O(new_n158_));
  nand2  g114(.a(new_n158_), .b(new_n154_), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(x15), .O(new_n160_));
  inv1   g116(.a(x15), .O(new_n161_));
  nand4  g117(.a(new_n157_), .b(x16), .c(new_n161_), .d(x10), .O(new_n162_));
  nand2  g118(.a(new_n162_), .b(new_n160_), .O(new_n163_));
  nand4  g119(.a(new_n163_), .b(new_n148_), .c(x31), .d(new_n129_), .O(new_n164_));
  nand2  g120(.a(x32), .b(x30), .O(new_n165_));
  nand2  g121(.a(new_n165_), .b(new_n164_), .O(new_n166_));
  nand3  g122(.a(new_n166_), .b(new_n147_), .c(x29), .O(new_n167_));
  nand2  g123(.a(new_n147_), .b(x29), .O(new_n168_));
  nand4  g124(.a(new_n163_), .b(new_n148_), .c(x31), .d(x30), .O(new_n169_));
  nand2  g125(.a(x32), .b(new_n129_), .O(new_n170_));
  nand2  g126(.a(new_n170_), .b(new_n169_), .O(new_n171_));
  nand2  g127(.a(new_n171_), .b(new_n168_), .O(new_n172_));
  nand2  g128(.a(x16), .b(x10), .O(new_n173_));
  inv1   g129(.a(new_n173_), .O(new_n174_));
  aoi21  g130(.a(new_n174_), .b(new_n157_), .c(x15), .O(new_n175_));
  nor2   g131(.a(new_n103_), .b(new_n100_), .O(new_n176_));
  nand3  g132(.a(new_n176_), .b(x17), .c(x11), .O(new_n177_));
  nand2  g133(.a(new_n177_), .b(new_n155_), .O(new_n178_));
  nand2  g134(.a(new_n176_), .b(x12), .O(new_n179_));
  nand2  g135(.a(new_n179_), .b(new_n97_), .O(new_n180_));
  inv1   g136(.a(x13), .O(new_n181_));
  oai21  g137(.a(new_n103_), .b(new_n181_), .c(new_n100_), .O(new_n182_));
  nor2   g138(.a(x19), .b(x14), .O(new_n183_));
  nand2  g139(.a(x16), .b(x15), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n156_), .c(x31), .O(new_n185_));
  nor2   g141(.a(new_n185_), .b(new_n183_), .O(new_n186_));
  nand4  g142(.a(new_n186_), .b(new_n182_), .c(new_n180_), .d(new_n178_), .O(new_n187_));
  oai21  g143(.a(new_n187_), .b(new_n175_), .c(x32), .O(new_n188_));
  nand3  g144(.a(new_n188_), .b(new_n172_), .c(new_n167_), .O(new_n189_));
  nand2  g145(.a(new_n189_), .b(x00), .O(new_n190_));
  aoi21  g146(.a(new_n190_), .b(new_n45_), .c(x26), .O(z8));
  inv1   g147(.a(x33), .O(new_n192_));
  nand4  g148(.a(new_n163_), .b(new_n192_), .c(x31), .d(x30), .O(new_n193_));
  nand2  g149(.a(x33), .b(new_n142_), .O(new_n194_));
  nand2  g150(.a(new_n194_), .b(new_n193_), .O(new_n195_));
  nand3  g151(.a(new_n195_), .b(new_n147_), .c(x29), .O(new_n196_));
  nand4  g152(.a(new_n163_), .b(new_n192_), .c(new_n142_), .d(new_n129_), .O(new_n197_));
  nand2  g153(.a(x33), .b(x31), .O(new_n198_));
  nand2  g154(.a(new_n198_), .b(new_n197_), .O(new_n199_));
  nand2  g155(.a(new_n199_), .b(new_n168_), .O(new_n200_));
  aoi21  g156(.a(new_n103_), .b(x14), .c(new_n100_), .O(new_n201_));
  nand4  g157(.a(new_n201_), .b(x17), .c(x16), .d(x15), .O(new_n202_));
  nand2  g158(.a(x18), .b(x17), .O(new_n203_));
  oai21  g159(.a(new_n203_), .b(new_n184_), .c(new_n103_), .O(new_n204_));
  oai21  g160(.a(new_n203_), .b(new_n173_), .c(new_n161_), .O(new_n205_));
  inv1   g161(.a(x11), .O(new_n206_));
  oai21  g162(.a(new_n203_), .b(new_n206_), .c(new_n155_), .O(new_n207_));
  nand2  g163(.a(x18), .b(x12), .O(new_n208_));
  nand2  g164(.a(new_n208_), .b(new_n97_), .O(new_n209_));
  nand2  g165(.a(new_n100_), .b(new_n181_), .O(new_n210_));
  xnor2a g166(.a(x31), .b(x30), .O(new_n211_));
  nand4  g167(.a(new_n211_), .b(new_n210_), .c(new_n209_), .d(new_n207_), .O(new_n212_));
  inv1   g168(.a(new_n212_), .O(new_n213_));
  nand4  g169(.a(new_n213_), .b(new_n205_), .c(new_n204_), .d(new_n202_), .O(new_n214_));
  nand2  g170(.a(new_n214_), .b(x33), .O(new_n215_));
  nand3  g171(.a(new_n215_), .b(new_n200_), .c(new_n196_), .O(new_n216_));
  nand4  g172(.a(new_n216_), .b(new_n111_), .c(new_n45_), .d(x00), .O(new_n217_));
  inv1   g173(.a(new_n217_), .O(z9));
endmodule


