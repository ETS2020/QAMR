// Benchmark "FAU" written by ABC on Thu Aug 20 01:03:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27,
    x28, x29, x30, x31, x32, x33,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26,
    x27, x28, x29, x30, x31, x32, x33;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n90_, new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n131_, new_n132_, new_n133_, new_n134_, new_n135_, new_n137_,
    new_n138_, new_n139_, new_n140_, new_n141_, new_n142_, new_n143_,
    new_n144_, new_n145_, new_n146_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n151_, new_n152_, new_n153_, new_n154_, new_n155_,
    new_n156_, new_n157_, new_n158_, new_n159_, new_n160_, new_n161_,
    new_n162_, new_n163_, new_n164_, new_n165_, new_n166_, new_n167_,
    new_n168_, new_n169_, new_n170_, new_n171_, new_n172_, new_n173_,
    new_n174_, new_n175_, new_n176_, new_n177_, new_n178_, new_n179_,
    new_n180_, new_n181_, new_n182_, new_n183_, new_n184_, new_n186_,
    new_n187_, new_n188_, new_n189_, new_n190_, new_n191_, new_n192_,
    new_n193_, new_n194_, new_n195_, new_n196_, new_n197_, new_n198_,
    new_n199_, new_n200_, new_n201_, new_n202_, new_n203_, new_n204_,
    new_n205_, new_n206_, new_n207_;
  inv1   g000(.a(x15), .O(new_n45_));
  inv1   g001(.a(x30), .O(new_n46_));
  nor2   g002(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  inv1   g003(.a(new_n47_), .O(new_n48_));
  nand2  g004(.a(new_n48_), .b(x32), .O(z0));
  nor2   g005(.a(x32), .b(x03), .O(new_n50_));
  inv1   g006(.a(x03), .O(new_n51_));
  nor2   g007(.a(x33), .b(new_n51_), .O(new_n52_));
  oai21  g008(.a(new_n52_), .b(new_n50_), .c(x02), .O(new_n53_));
  inv1   g009(.a(x02), .O(new_n54_));
  nor2   g010(.a(x32), .b(new_n51_), .O(new_n55_));
  nor2   g011(.a(x33), .b(x03), .O(new_n56_));
  oai21  g012(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  aoi21  g013(.a(new_n57_), .b(new_n53_), .c(new_n47_), .O(z1));
  xnor2a g014(.a(x07), .b(x04), .O(new_n59_));
  inv1   g015(.a(x01), .O(new_n60_));
  nor2   g016(.a(new_n51_), .b(new_n54_), .O(new_n61_));
  inv1   g017(.a(new_n61_), .O(new_n62_));
  nand2  g018(.a(new_n51_), .b(new_n54_), .O(new_n63_));
  aoi21  g019(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  nand2  g020(.a(x05), .b(new_n51_), .O(new_n65_));
  nand3  g021(.a(new_n65_), .b(x06), .c(new_n54_), .O(new_n66_));
  nand2  g022(.a(x06), .b(new_n54_), .O(new_n67_));
  nand3  g023(.a(new_n67_), .b(x05), .c(new_n51_), .O(new_n68_));
  aoi21  g024(.a(new_n68_), .b(new_n66_), .c(x01), .O(new_n69_));
  oai21  g025(.a(new_n69_), .b(new_n64_), .c(new_n59_), .O(new_n70_));
  xor2a  g026(.a(x07), .b(x04), .O(new_n71_));
  xnor2a g027(.a(x03), .b(x02), .O(new_n72_));
  nor2   g028(.a(new_n72_), .b(new_n60_), .O(new_n73_));
  nand2  g029(.a(new_n67_), .b(new_n65_), .O(new_n74_));
  nand4  g030(.a(x06), .b(x05), .c(new_n51_), .d(new_n54_), .O(new_n75_));
  aoi21  g031(.a(new_n75_), .b(new_n74_), .c(x01), .O(new_n76_));
  oai21  g032(.a(new_n76_), .b(new_n73_), .c(new_n71_), .O(new_n77_));
  aoi21  g033(.a(new_n77_), .b(new_n70_), .c(x09), .O(new_n78_));
  nand2  g034(.a(new_n78_), .b(x08), .O(new_n79_));
  nand2  g035(.a(new_n79_), .b(new_n48_), .O(z2));
  inv1   g036(.a(x26), .O(new_n81_));
  inv1   g037(.a(x27), .O(new_n82_));
  nor2   g038(.a(x20), .b(x15), .O(new_n83_));
  nor2   g039(.a(x21), .b(x16), .O(new_n84_));
  nor2   g040(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nor2   g041(.a(x22), .b(x17), .O(new_n86_));
  nor2   g042(.a(x23), .b(x18), .O(new_n87_));
  inv1   g043(.a(new_n87_), .O(new_n88_));
  nor2   g044(.a(x24), .b(x19), .O(new_n89_));
  inv1   g045(.a(new_n89_), .O(new_n90_));
  nand2  g046(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  nor2   g047(.a(new_n91_), .b(new_n86_), .O(new_n92_));
  aoi21  g048(.a(new_n92_), .b(new_n85_), .c(new_n61_), .O(new_n93_));
  nand4  g049(.a(new_n93_), .b(new_n82_), .c(new_n81_), .d(x25), .O(new_n94_));
  oai21  g050(.a(new_n94_), .b(new_n60_), .c(new_n48_), .O(z3));
  xor2a  g051(.a(x28), .b(x27), .O(new_n96_));
  nand4  g052(.a(new_n96_), .b(new_n93_), .c(new_n81_), .d(x25), .O(new_n97_));
  oai21  g053(.a(new_n97_), .b(new_n60_), .c(new_n48_), .O(z4));
  nand2  g054(.a(new_n92_), .b(new_n85_), .O(new_n99_));
  inv1   g055(.a(x29), .O(new_n100_));
  inv1   g056(.a(x28), .O(new_n101_));
  nor2   g057(.a(new_n101_), .b(new_n82_), .O(new_n102_));
  nand3  g058(.a(new_n100_), .b(x28), .c(x27), .O(new_n103_));
  oai21  g059(.a(new_n102_), .b(new_n100_), .c(new_n103_), .O(new_n104_));
  nand4  g060(.a(new_n104_), .b(new_n99_), .c(new_n62_), .d(new_n81_), .O(new_n105_));
  inv1   g061(.a(new_n105_), .O(new_n106_));
  nand3  g062(.a(new_n106_), .b(x25), .c(x01), .O(new_n107_));
  nand2  g063(.a(new_n107_), .b(new_n48_), .O(z5));
  nand3  g064(.a(x29), .b(x28), .c(x27), .O(new_n109_));
  inv1   g065(.a(new_n84_), .O(new_n110_));
  nand3  g066(.a(new_n92_), .b(new_n110_), .c(x20), .O(new_n111_));
  nand3  g067(.a(new_n111_), .b(new_n109_), .c(x30), .O(new_n112_));
  nor2   g068(.a(new_n82_), .b(x20), .O(new_n113_));
  nand4  g069(.a(new_n113_), .b(new_n46_), .c(x29), .d(x28), .O(new_n114_));
  nand2  g070(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  nand2  g071(.a(new_n115_), .b(new_n45_), .O(new_n116_));
  inv1   g072(.a(new_n91_), .O(new_n117_));
  nor2   g073(.a(new_n86_), .b(new_n84_), .O(new_n118_));
  aoi21  g074(.a(new_n118_), .b(new_n117_), .c(x30), .O(new_n119_));
  nand4  g075(.a(new_n119_), .b(x29), .c(x28), .d(x27), .O(new_n120_));
  nand2  g076(.a(new_n120_), .b(new_n116_), .O(new_n121_));
  nand4  g077(.a(new_n121_), .b(new_n62_), .c(new_n81_), .d(x25), .O(new_n122_));
  nor2   g078(.a(new_n122_), .b(new_n60_), .O(z6));
  nand4  g079(.a(new_n62_), .b(new_n81_), .c(x25), .d(x01), .O(new_n124_));
  nand2  g080(.a(new_n124_), .b(new_n48_), .O(new_n125_));
  nand2  g081(.a(new_n109_), .b(x31), .O(new_n126_));
  nand3  g082(.a(new_n118_), .b(new_n90_), .c(new_n88_), .O(new_n127_));
  inv1   g083(.a(new_n127_), .O(new_n128_));
  nand2  g084(.a(new_n128_), .b(x20), .O(new_n129_));
  inv1   g085(.a(x31), .O(new_n130_));
  nand4  g086(.a(new_n102_), .b(new_n130_), .c(x30), .d(x29), .O(new_n131_));
  nand3  g087(.a(new_n131_), .b(new_n129_), .c(new_n126_), .O(new_n132_));
  nand2  g088(.a(new_n132_), .b(new_n45_), .O(new_n133_));
  oai21  g089(.a(new_n127_), .b(new_n45_), .c(new_n130_), .O(new_n134_));
  nand2  g090(.a(new_n134_), .b(new_n46_), .O(new_n135_));
  nand3  g091(.a(new_n135_), .b(new_n133_), .c(new_n125_), .O(z7));
  inv1   g092(.a(x00), .O(new_n137_));
  inv1   g093(.a(x32), .O(new_n138_));
  nor2   g094(.a(x28), .b(x27), .O(new_n139_));
  inv1   g095(.a(new_n139_), .O(new_n140_));
  inv1   g096(.a(x17), .O(new_n141_));
  inv1   g097(.a(x18), .O(new_n142_));
  nand3  g098(.a(x19), .b(new_n142_), .c(x13), .O(new_n143_));
  inv1   g099(.a(x19), .O(new_n144_));
  nand3  g100(.a(new_n144_), .b(x18), .c(x14), .O(new_n145_));
  aoi21  g101(.a(new_n145_), .b(new_n143_), .c(new_n141_), .O(new_n146_));
  nand4  g102(.a(x19), .b(x18), .c(new_n141_), .d(x12), .O(new_n147_));
  inv1   g103(.a(new_n147_), .O(new_n148_));
  oai21  g104(.a(new_n148_), .b(new_n146_), .c(x16), .O(new_n149_));
  inv1   g105(.a(x16), .O(new_n150_));
  nor2   g106(.a(new_n144_), .b(new_n142_), .O(new_n151_));
  nand4  g107(.a(new_n151_), .b(x17), .c(new_n150_), .d(x11), .O(new_n152_));
  nand2  g108(.a(new_n152_), .b(new_n149_), .O(new_n153_));
  nand2  g109(.a(new_n153_), .b(x15), .O(new_n154_));
  nor2   g110(.a(new_n150_), .b(x15), .O(new_n155_));
  nand2  g111(.a(x18), .b(x17), .O(new_n156_));
  inv1   g112(.a(new_n156_), .O(new_n157_));
  nor2   g113(.a(x30), .b(new_n144_), .O(new_n158_));
  nand4  g114(.a(new_n158_), .b(new_n157_), .c(new_n155_), .d(x10), .O(new_n159_));
  nand2  g115(.a(new_n159_), .b(new_n154_), .O(new_n160_));
  nand4  g116(.a(new_n160_), .b(new_n140_), .c(new_n138_), .d(x31), .O(new_n161_));
  oai21  g117(.a(new_n82_), .b(x15), .c(new_n101_), .O(new_n162_));
  nand3  g118(.a(new_n162_), .b(x32), .c(x30), .O(new_n163_));
  aoi21  g119(.a(new_n163_), .b(new_n161_), .c(new_n100_), .O(new_n164_));
  nor2   g120(.a(new_n139_), .b(new_n100_), .O(new_n165_));
  nand3  g121(.a(new_n157_), .b(x16), .c(x10), .O(new_n166_));
  nand4  g122(.a(new_n138_), .b(x31), .c(x30), .d(x19), .O(new_n167_));
  nor2   g123(.a(new_n167_), .b(new_n166_), .O(new_n168_));
  aoi21  g124(.a(x32), .b(new_n46_), .c(new_n168_), .O(new_n169_));
  oai21  g125(.a(x30), .b(x14), .c(new_n144_), .O(new_n170_));
  nand4  g126(.a(new_n170_), .b(x18), .c(x17), .d(x16), .O(new_n171_));
  nor2   g127(.a(new_n171_), .b(new_n45_), .O(new_n172_));
  nand2  g128(.a(x16), .b(x15), .O(new_n173_));
  oai21  g129(.a(new_n173_), .b(new_n156_), .c(new_n144_), .O(new_n174_));
  nand2  g130(.a(new_n166_), .b(new_n45_), .O(new_n175_));
  inv1   g131(.a(x11), .O(new_n176_));
  oai21  g132(.a(new_n156_), .b(new_n176_), .c(new_n150_), .O(new_n177_));
  aoi21  g133(.a(x18), .b(x12), .c(x17), .O(new_n178_));
  nor2   g134(.a(x18), .b(x13), .O(new_n179_));
  nor3   g135(.a(new_n179_), .b(new_n178_), .c(new_n130_), .O(new_n180_));
  nand4  g136(.a(new_n180_), .b(new_n177_), .c(new_n175_), .d(new_n174_), .O(new_n181_));
  oai21  g137(.a(new_n181_), .b(new_n172_), .c(x32), .O(new_n182_));
  oai21  g138(.a(new_n169_), .b(new_n165_), .c(new_n182_), .O(new_n183_));
  oai21  g139(.a(new_n183_), .b(new_n164_), .c(new_n81_), .O(new_n184_));
  oai21  g140(.a(new_n184_), .b(new_n137_), .c(new_n48_), .O(z8));
  inv1   g141(.a(x33), .O(new_n186_));
  nand3  g142(.a(new_n158_), .b(new_n186_), .c(new_n130_), .O(new_n187_));
  oai22  g143(.a(new_n187_), .b(new_n166_), .c(new_n186_), .d(new_n130_), .O(new_n188_));
  nand2  g144(.a(new_n188_), .b(new_n45_), .O(new_n189_));
  aoi21  g145(.a(new_n152_), .b(new_n149_), .c(x33), .O(new_n190_));
  nand4  g146(.a(new_n190_), .b(new_n130_), .c(new_n46_), .d(x15), .O(new_n191_));
  aoi21  g147(.a(new_n191_), .b(new_n189_), .c(new_n165_), .O(new_n192_));
  nor2   g148(.a(new_n141_), .b(new_n150_), .O(new_n193_));
  nand3  g149(.a(new_n193_), .b(new_n45_), .c(x10), .O(new_n194_));
  nand4  g150(.a(new_n151_), .b(new_n186_), .c(x31), .d(x30), .O(new_n195_));
  oai22  g151(.a(new_n195_), .b(new_n194_), .c(new_n186_), .d(x30), .O(new_n196_));
  nand3  g152(.a(new_n196_), .b(new_n140_), .c(x29), .O(new_n197_));
  aoi21  g153(.a(new_n144_), .b(x14), .c(new_n142_), .O(new_n198_));
  nand4  g154(.a(new_n198_), .b(x17), .c(x16), .d(x15), .O(new_n199_));
  oai21  g155(.a(new_n156_), .b(new_n150_), .c(new_n144_), .O(new_n200_));
  nor3   g156(.a(new_n179_), .b(new_n178_), .c(x31), .O(new_n201_));
  nand4  g157(.a(new_n201_), .b(new_n200_), .c(new_n199_), .d(new_n177_), .O(new_n202_));
  aoi21  g158(.a(new_n130_), .b(x30), .c(new_n144_), .O(new_n203_));
  nand4  g159(.a(new_n203_), .b(new_n193_), .c(x18), .d(x10), .O(new_n204_));
  aoi22  g160(.a(new_n204_), .b(new_n45_), .c(new_n202_), .d(new_n46_), .O(new_n205_));
  oai21  g161(.a(new_n205_), .b(new_n186_), .c(new_n197_), .O(new_n206_));
  oai21  g162(.a(new_n206_), .b(new_n192_), .c(new_n81_), .O(new_n207_));
  nor2   g163(.a(new_n207_), .b(new_n137_), .O(z9));
endmodule


