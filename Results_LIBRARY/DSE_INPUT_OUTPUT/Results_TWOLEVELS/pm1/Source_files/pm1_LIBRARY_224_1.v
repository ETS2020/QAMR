// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x14), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x14), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  nand2  g08(.a(x12), .b(new_n37_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n36_), .c(new_n31_), .O(z00));
  inv1   g10(.a(x10), .O(new_n40_));
  nand3  g11(.a(x11), .b(new_n40_), .c(x00), .O(new_n41_));
  aoi21  g12(.a(new_n41_), .b(new_n30_), .c(x11), .O(new_n42_));
  oai22  g13(.a(new_n42_), .b(x14), .c(new_n34_), .d(new_n37_), .O(z01));
  inv1   g14(.a(x01), .O(new_n44_));
  inv1   g15(.a(x14), .O(z04));
  nand4  g16(.a(x11), .b(new_n40_), .c(x01), .d(x00), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(z04), .c(new_n44_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n30_), .O(new_n49_));
  nand4  g20(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n37_), .c(x12), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n49_), .c(x09), .O(z02));
  inv1   g23(.a(x09), .O(new_n53_));
  nand3  g24(.a(x11), .b(x08), .c(x07), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n32_), .c(x11), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(x12), .c(new_n53_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n49_), .O(z03));
  nand2  g28(.a(x14), .b(new_n30_), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(x13), .O(z05));
  and2   g31(.a(x03), .b(x02), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n38_), .c(x04), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(x09), .c(x01), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n58_), .O(z06));
  nor2   g35(.a(new_n59_), .b(x15), .O(z07));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n66_));
  nand3  g37(.a(x12), .b(x11), .c(x09), .O(new_n67_));
  nand3  g38(.a(z04), .b(new_n30_), .c(new_n37_), .O(new_n68_));
  oai21  g39(.a(new_n67_), .b(new_n66_), .c(new_n68_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n40_), .c(x00), .O(new_n70_));
  inv1   g41(.a(x00), .O(new_n71_));
  oai22  g42(.a(z04), .b(x12), .c(x10), .d(new_n71_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n70_), .c(new_n38_), .O(z08));
  nand4  g44(.a(new_n66_), .b(new_n61_), .c(x09), .d(x04), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x12), .c(x11), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n68_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(new_n40_), .c(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z09));
  nand4  g49(.a(new_n66_), .b(x12), .c(x11), .d(new_n40_), .O(new_n79_));
  nor3   g50(.a(new_n79_), .b(new_n53_), .c(new_n71_), .O(z10));
  nand4  g51(.a(new_n66_), .b(x12), .c(x09), .d(x01), .O(new_n81_));
  nand3  g52(.a(z04), .b(new_n30_), .c(new_n44_), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(x11), .c(new_n40_), .d(x00), .O(new_n84_));
  inv1   g55(.a(new_n84_), .O(z11));
  nand2  g56(.a(new_n53_), .b(x00), .O(new_n86_));
  nand3  g57(.a(x12), .b(x11), .c(new_n40_), .O(new_n87_));
  oai21  g58(.a(new_n87_), .b(new_n86_), .c(new_n58_), .O(z12));
endmodule


