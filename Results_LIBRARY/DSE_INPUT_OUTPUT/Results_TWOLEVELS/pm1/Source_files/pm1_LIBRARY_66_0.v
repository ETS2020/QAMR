// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n53_, new_n56_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x14), .O(new_n31_));
  inv1   g02(.a(x15), .O(z07));
  nor2   g03(.a(x12), .b(x11), .O(new_n33_));
  aoi22  g04(.a(new_n33_), .b(new_n30_), .c(z07), .d(new_n31_), .O(z00));
  nand2  g05(.a(z07), .b(new_n31_), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  and2   g08(.a(new_n37_), .b(new_n35_), .O(z01));
  inv1   g09(.a(x09), .O(new_n39_));
  nand2  g10(.a(z07), .b(new_n31_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g12(.a(x12), .b(x11), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  and2   g14(.a(x06), .b(x05), .O(new_n44_));
  inv1   g15(.a(x07), .O(new_n45_));
  nand2  g16(.a(new_n40_), .b(x08), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(new_n45_), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n44_), .c(new_n43_), .d(new_n41_), .O(z02));
  nand3  g19(.a(new_n44_), .b(x08), .c(x07), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x09), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n40_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n43_), .O(z03));
  oai21  g23(.a(new_n42_), .b(x11), .c(x15), .O(new_n53_));
  nor2   g24(.a(new_n53_), .b(x14), .O(z04));
  nand2  g25(.a(new_n40_), .b(x13), .O(z05));
  nand4  g26(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x01), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n40_), .O(z06));
  inv1   g29(.a(x10), .O(new_n59_));
  inv1   g30(.a(x12), .O(new_n60_));
  nand4  g31(.a(new_n40_), .b(new_n60_), .c(new_n59_), .d(x00), .O(new_n61_));
  nand2  g32(.a(new_n35_), .b(x12), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n61_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n36_), .O(new_n64_));
  nand2  g35(.a(new_n59_), .b(x00), .O(new_n65_));
  nand2  g36(.a(x03), .b(x02), .O(new_n66_));
  nand3  g37(.a(x12), .b(x09), .c(x04), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n66_), .c(new_n40_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n64_), .O(z08));
  xnor2a g41(.a(x12), .b(x11), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n40_), .c(new_n59_), .d(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z09));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x12), .c(x11), .d(new_n59_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n40_), .O(z10));
  nand4  g49(.a(new_n74_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  nand2  g50(.a(new_n60_), .b(new_n30_), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x11), .c(new_n59_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n40_), .O(z11));
  nand2  g54(.a(new_n39_), .b(x00), .O(new_n84_));
  nand3  g55(.a(x12), .b(x11), .c(new_n59_), .O(new_n85_));
  oai21  g56(.a(new_n85_), .b(new_n84_), .c(new_n40_), .O(z12));
endmodule


