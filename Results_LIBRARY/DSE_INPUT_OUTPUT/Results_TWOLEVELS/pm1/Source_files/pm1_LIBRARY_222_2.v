// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_;
  inv1   g00(.a(x15), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x10), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  inv1   g06(.a(x12), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(x11), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(x01), .O(new_n38_));
  aoi21  g09(.a(new_n38_), .b(new_n35_), .c(new_n32_), .O(z00));
  aoi21  g10(.a(new_n35_), .b(x12), .c(new_n32_), .O(z01));
  nand2  g11(.a(x08), .b(x07), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n33_), .c(x11), .O(new_n42_));
  inv1   g13(.a(x11), .O(new_n43_));
  nor2   g14(.a(new_n36_), .b(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x09), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  aoi21  g17(.a(new_n46_), .b(new_n42_), .c(new_n32_), .O(z02));
  inv1   g18(.a(x09), .O(new_n48_));
  nand3  g19(.a(x11), .b(x08), .c(x07), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(new_n33_), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g22(.a(new_n51_), .b(new_n44_), .c(new_n32_), .O(z03));
  nand2  g23(.a(new_n31_), .b(x14), .O(z04));
  nor2   g24(.a(new_n32_), .b(x13), .O(z05));
  inv1   g25(.a(new_n37_), .O(new_n55_));
  and2   g26(.a(x03), .b(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n55_), .c(x04), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(x09), .c(x01), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n31_), .O(z06));
  nor2   g30(.a(x15), .b(x10), .O(z07));
  inv1   g31(.a(x00), .O(new_n61_));
  nand3  g32(.a(x04), .b(x03), .c(x02), .O(new_n62_));
  nand3  g33(.a(x12), .b(x11), .c(x09), .O(new_n63_));
  oai22  g34(.a(new_n63_), .b(new_n62_), .c(x12), .d(x11), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n37_), .c(new_n61_), .O(new_n65_));
  nand2  g36(.a(x15), .b(x10), .O(new_n66_));
  oai21  g37(.a(new_n65_), .b(x10), .c(new_n66_), .O(z08));
  inv1   g38(.a(x10), .O(new_n68_));
  nand2  g39(.a(new_n36_), .b(new_n43_), .O(new_n69_));
  nand4  g40(.a(new_n62_), .b(new_n56_), .c(x09), .d(x04), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n68_), .c(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand4  g45(.a(new_n62_), .b(x12), .c(x11), .d(new_n68_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n31_), .O(z10));
  nand4  g49(.a(new_n62_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n68_), .d(x00), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(z11));
  nand2  g53(.a(new_n48_), .b(x00), .O(new_n83_));
  nand3  g54(.a(x12), .b(x11), .c(new_n68_), .O(new_n84_));
  oai21  g55(.a(new_n84_), .b(new_n83_), .c(new_n31_), .O(z12));
endmodule


