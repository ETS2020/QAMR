// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  inv1   g05(.a(x11), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n35_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x00), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n31_), .O(z00));
  nand3  g10(.a(new_n34_), .b(x12), .c(x00), .O(z01));
  inv1   g11(.a(x09), .O(new_n41_));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  aoi21  g13(.a(new_n42_), .b(x11), .c(new_n41_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(x12), .c(x11), .d(x00), .O(z02));
  nand2  g15(.a(x05), .b(x00), .O(new_n45_));
  nand3  g16(.a(x08), .b(x07), .c(x06), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(new_n45_), .c(x12), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x11), .O(new_n48_));
  nand2  g19(.a(new_n36_), .b(x09), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x00), .O(new_n50_));
  nand2  g21(.a(new_n30_), .b(new_n35_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n50_), .c(new_n48_), .O(z03));
  nor2   g23(.a(new_n30_), .b(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x14), .O(z04));
  nand2  g26(.a(new_n54_), .b(x13), .O(z05));
  inv1   g27(.a(x00), .O(new_n57_));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  oai22  g30(.a(new_n59_), .b(new_n53_), .c(new_n36_), .d(new_n57_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(x09), .c(x01), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z06));
  nor2   g33(.a(new_n53_), .b(x15), .O(z07));
  oai21  g34(.a(new_n30_), .b(x00), .c(new_n35_), .O(new_n64_));
  inv1   g35(.a(x10), .O(new_n65_));
  nand4  g36(.a(x12), .b(x11), .c(new_n65_), .d(x09), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n58_), .c(new_n65_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x00), .O(new_n68_));
  nand2  g39(.a(new_n30_), .b(new_n57_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n68_), .c(new_n64_), .O(z08));
  and2   g41(.a(x03), .b(x02), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n58_), .c(x09), .d(x04), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x12), .c(x11), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n51_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(new_n65_), .c(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z09));
  nand4  g47(.a(new_n58_), .b(x11), .c(new_n65_), .d(x09), .O(new_n77_));
  aoi21  g48(.a(new_n77_), .b(x00), .c(new_n30_), .O(z10));
  nand4  g49(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n65_), .d(x00), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(z11));
  nand4  g53(.a(x11), .b(new_n65_), .c(new_n41_), .d(x00), .O(new_n83_));
  aoi21  g54(.a(new_n83_), .b(x00), .c(new_n30_), .O(z12));
endmodule


