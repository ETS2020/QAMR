// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(x15), .b(x08), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  inv1   g05(.a(new_n33_), .O(new_n35_));
  aoi21  g06(.a(x12), .b(new_n31_), .c(new_n35_), .O(z01));
  inv1   g07(.a(x15), .O(z07));
  nand3  g08(.a(x07), .b(x06), .c(x05), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nor2   g10(.a(new_n32_), .b(new_n31_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n39_), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(z07), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x08), .O(z02));
  oai21  g14(.a(new_n39_), .b(x15), .c(x08), .O(new_n44_));
  oai21  g15(.a(new_n32_), .b(new_n31_), .c(new_n33_), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n44_), .c(x09), .O(z03));
  nand2  g17(.a(new_n33_), .b(x14), .O(z04));
  nand2  g18(.a(new_n33_), .b(x13), .O(z05));
  nand2  g19(.a(x12), .b(new_n31_), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(x04), .c(x03), .d(x02), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n33_), .O(z06));
  inv1   g23(.a(x00), .O(new_n53_));
  nor2   g24(.a(x10), .b(new_n53_), .O(new_n54_));
  nand4  g25(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(new_n40_), .b(new_n56_), .c(x09), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n31_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n54_), .c(new_n35_), .O(z08));
  nand4  g31(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x09), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n33_), .O(new_n63_));
  nand3  g34(.a(x04), .b(x03), .c(x02), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x09), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(x12), .c(x11), .O(new_n67_));
  nand2  g38(.a(new_n32_), .b(new_n31_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n56_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n33_), .O(z09));
  and2   g42(.a(new_n64_), .b(x12), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x11), .c(new_n56_), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x09), .c(x00), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n33_), .O(z10));
  nand3  g47(.a(new_n72_), .b(x09), .c(x01), .O(new_n77_));
  oai21  g48(.a(x12), .b(x01), .c(new_n77_), .O(new_n78_));
  nand4  g49(.a(new_n78_), .b(x11), .c(new_n56_), .d(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n33_), .O(z11));
  nand4  g51(.a(new_n33_), .b(x12), .c(x11), .d(new_n56_), .O(new_n81_));
  nor3   g52(.a(new_n81_), .b(x09), .c(new_n53_), .O(z12));
endmodule


