// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x15), .O(z07));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  nor3   g04(.a(new_n33_), .b(new_n32_), .c(z07), .O(new_n34_));
  aoi21  g05(.a(x12), .b(new_n30_), .c(x01), .O(new_n35_));
  oai21  g06(.a(new_n34_), .b(new_n30_), .c(new_n35_), .O(z00));
  oai21  g07(.a(new_n33_), .b(new_n32_), .c(x15), .O(new_n37_));
  inv1   g08(.a(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n30_), .O(new_n39_));
  oai21  g10(.a(new_n37_), .b(new_n30_), .c(new_n39_), .O(z01));
  inv1   g11(.a(new_n32_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x08), .c(x07), .O(new_n42_));
  nand2  g13(.a(x12), .b(x09), .O(new_n43_));
  aoi21  g14(.a(new_n42_), .b(x11), .c(new_n43_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(z07), .c(x11), .O(z02));
  inv1   g16(.a(new_n43_), .O(new_n46_));
  nand4  g17(.a(new_n41_), .b(x15), .c(x08), .d(x07), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n46_), .c(x15), .d(x11), .O(z03));
  oai21  g19(.a(x15), .b(new_n30_), .c(x14), .O(z04));
  nor2   g20(.a(x15), .b(new_n30_), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(x13), .O(z05));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai22  g24(.a(new_n53_), .b(new_n50_), .c(new_n38_), .d(x11), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(z06));
  inv1   g27(.a(x10), .O(new_n57_));
  nand4  g28(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nor2   g30(.a(z07), .b(new_n38_), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n59_), .c(new_n57_), .d(x09), .O(new_n61_));
  inv1   g32(.a(x00), .O(new_n62_));
  nor2   g33(.a(x10), .b(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n61_), .c(x15), .d(x11), .O(z08));
  and2   g35(.a(x03), .b(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n52_), .c(x09), .d(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x15), .c(x12), .d(x11), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n39_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n57_), .c(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z09));
  nand4  g41(.a(new_n52_), .b(x15), .c(x12), .d(new_n57_), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x09), .c(x00), .O(new_n73_));
  aoi21  g44(.a(new_n73_), .b(x15), .c(new_n30_), .O(z10));
  nand4  g45(.a(new_n52_), .b(x12), .c(x09), .d(x01), .O(new_n75_));
  oai21  g46(.a(x12), .b(x01), .c(new_n75_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x15), .c(x11), .d(new_n57_), .O(new_n77_));
  nor2   g48(.a(new_n77_), .b(new_n62_), .O(z11));
  inv1   g49(.a(x09), .O(new_n79_));
  nand4  g50(.a(x11), .b(new_n57_), .c(new_n79_), .d(x00), .O(new_n80_));
  nor3   g51(.a(new_n80_), .b(z07), .c(new_n38_), .O(z12));
endmodule


