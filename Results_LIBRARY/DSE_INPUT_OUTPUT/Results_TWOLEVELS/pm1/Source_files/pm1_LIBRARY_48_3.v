// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n66_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x00), .c(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x00), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n31_), .c(new_n34_), .O(z01));
  nand2  g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(new_n34_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n32_), .c(x12), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand2  g11(.a(x12), .b(new_n31_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z03));
  nand2  g15(.a(x12), .b(x00), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nor2   g17(.a(new_n46_), .b(x14), .O(z04));
  nor2   g18(.a(new_n46_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  nand3  g21(.a(x12), .b(new_n31_), .c(new_n34_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z06));
  nand2  g25(.a(new_n45_), .b(x15), .O(z07));
  inv1   g26(.a(x12), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  oai21  g28(.a(x11), .b(new_n34_), .c(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand3  g30(.a(new_n36_), .b(x08), .c(x07), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n36_), .c(new_n34_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n59_), .O(z08));
  aoi21  g33(.a(new_n31_), .b(new_n57_), .c(x12), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n34_), .O(z09));
  nand4  g35(.a(x11), .b(new_n57_), .c(new_n30_), .d(x00), .O(new_n66_));
  nor2   g36(.a(new_n66_), .b(x12), .O(z11));
  zero   g37(.O(z10));
  zero   g38(.O(z12));
endmodule


