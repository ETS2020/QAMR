// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_;
  inv1   g00(.a(x13), .O(new_n30_));
  inv1   g01(.a(x15), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x11), .c(new_n31_), .O(new_n32_));
  and2   g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  inv1   g07(.a(x01), .O(new_n37_));
  inv1   g08(.a(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n36_), .c(new_n32_), .O(z00));
  nand2  g12(.a(new_n31_), .b(new_n30_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n38_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n36_), .c(new_n32_), .O(z01));
  inv1   g15(.a(x11), .O(new_n45_));
  oai21  g16(.a(new_n31_), .b(new_n45_), .c(new_n30_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n34_), .O(new_n47_));
  inv1   g18(.a(x09), .O(new_n48_));
  oai21  g19(.a(new_n38_), .b(new_n48_), .c(new_n42_), .O(new_n49_));
  oai21  g20(.a(x15), .b(x13), .c(new_n45_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(z02));
  nand4  g22(.a(new_n33_), .b(x11), .c(x08), .d(x07), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(new_n50_), .c(new_n49_), .d(new_n42_), .O(z03));
  nand2  g24(.a(new_n42_), .b(x14), .O(z04));
  nor2   g25(.a(new_n31_), .b(x13), .O(z05));
  and2   g26(.a(x03), .b(x02), .O(new_n56_));
  nand2  g27(.a(x12), .b(new_n45_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n56_), .c(x04), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n42_), .c(x09), .d(x01), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z06));
  nor2   g31(.a(x15), .b(new_n30_), .O(z07));
  inv1   g32(.a(x00), .O(new_n62_));
  inv1   g33(.a(x10), .O(new_n63_));
  nand4  g34(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n64_));
  nand4  g35(.a(x12), .b(x11), .c(new_n63_), .d(x09), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n64_), .c(new_n63_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n62_), .c(new_n42_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n50_), .O(z08));
  nand3  g39(.a(x04), .b(x03), .c(x02), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n56_), .c(x09), .d(x04), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  nand2  g42(.a(new_n38_), .b(new_n45_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n42_), .c(new_n63_), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z09));
  nand4  g46(.a(new_n69_), .b(new_n42_), .c(x12), .d(x11), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(new_n63_), .c(x09), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z10));
  nand4  g50(.a(new_n69_), .b(x12), .c(x09), .d(x01), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n39_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x11), .c(new_n63_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n42_), .O(z11));
  aoi21  g54(.a(new_n31_), .b(new_n30_), .c(new_n38_), .O(new_n84_));
  nand4  g55(.a(new_n84_), .b(x11), .c(new_n63_), .d(new_n48_), .O(new_n85_));
  nor2   g56(.a(new_n85_), .b(new_n62_), .O(z12));
endmodule


