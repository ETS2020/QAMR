// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n49_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x02), .O(new_n32_));
  nor2   g03(.a(x12), .b(new_n32_), .O(new_n33_));
  aoi21  g04(.a(x12), .b(new_n31_), .c(new_n33_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g06(.a(x06), .b(x05), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n36_), .c(x12), .O(new_n38_));
  inv1   g09(.a(x12), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n32_), .O(new_n40_));
  oai21  g11(.a(new_n38_), .b(new_n31_), .c(new_n40_), .O(z01));
  inv1   g12(.a(x09), .O(new_n42_));
  nand2  g13(.a(x08), .b(x07), .O(new_n43_));
  nor4   g14(.a(new_n43_), .b(new_n36_), .c(new_n31_), .d(new_n42_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n39_), .c(new_n40_), .O(z02));
  nand3  g16(.a(x11), .b(x08), .c(x07), .O(new_n46_));
  or2    g17(.a(new_n46_), .b(new_n36_), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g19(.a(new_n33_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x14), .O(z04));
  nand2  g21(.a(new_n49_), .b(x13), .O(z05));
  nand3  g22(.a(x11), .b(x04), .c(x03), .O(new_n52_));
  aoi21  g23(.a(new_n52_), .b(x12), .c(new_n32_), .O(new_n53_));
  nor3   g24(.a(new_n53_), .b(new_n42_), .c(new_n30_), .O(z06));
  nor2   g25(.a(new_n33_), .b(x15), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(new_n60_));
  nor2   g31(.a(x12), .b(x11), .O(new_n61_));
  aoi22  g32(.a(new_n61_), .b(new_n32_), .c(new_n60_), .d(new_n58_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n34_), .c(new_n56_), .d(x00), .O(z08));
  oai21  g34(.a(new_n61_), .b(new_n60_), .c(new_n32_), .O(new_n64_));
  nand4  g35(.a(x09), .b(x04), .c(x03), .d(new_n32_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x12), .c(x11), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n56_), .c(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z09));
  nand4  g40(.a(new_n57_), .b(x12), .c(x11), .d(new_n56_), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(x09), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n49_), .O(z10));
  nand3  g44(.a(x12), .b(x09), .c(x01), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(new_n75_));
  nor2   g46(.a(x12), .b(x01), .O(new_n76_));
  oai21  g47(.a(new_n76_), .b(new_n75_), .c(new_n32_), .O(new_n77_));
  nand2  g48(.a(x04), .b(x03), .O(new_n78_));
  nand4  g49(.a(new_n78_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n77_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n56_), .d(x00), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(z11));
  nand2  g53(.a(new_n42_), .b(x00), .O(new_n83_));
  nand3  g54(.a(x12), .b(x11), .c(new_n56_), .O(new_n84_));
  oai21  g55(.a(new_n84_), .b(new_n83_), .c(new_n49_), .O(z12));
endmodule


