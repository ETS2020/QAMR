// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n55_, new_n56_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x14), .O(z04));
  inv1   g02(.a(x15), .O(new_n32_));
  nor2   g03(.a(x12), .b(x11), .O(new_n33_));
  aoi22  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .d(z04), .O(z00));
  nand2  g05(.a(new_n32_), .b(z04), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  and2   g08(.a(new_n37_), .b(new_n35_), .O(z01));
  inv1   g09(.a(x09), .O(new_n39_));
  nand2  g10(.a(new_n32_), .b(z04), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g12(.a(x12), .b(x11), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n35_), .O(new_n43_));
  and2   g14(.a(x06), .b(x05), .O(new_n44_));
  inv1   g15(.a(x07), .O(new_n45_));
  inv1   g16(.a(x08), .O(new_n46_));
  nor2   g17(.a(x15), .b(x14), .O(new_n47_));
  nor3   g18(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(new_n44_), .c(new_n43_), .d(new_n41_), .O(z02));
  nand3  g20(.a(new_n44_), .b(x08), .c(x07), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n40_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n43_), .O(z03));
  nor2   g24(.a(new_n47_), .b(x13), .O(z05));
  nand4  g25(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n40_), .O(z06));
  nor2   g28(.a(new_n42_), .b(x11), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(x14), .c(x15), .O(z07));
  inv1   g30(.a(x10), .O(new_n60_));
  inv1   g31(.a(x12), .O(new_n61_));
  nand4  g32(.a(new_n40_), .b(new_n61_), .c(new_n60_), .d(x00), .O(new_n62_));
  nand2  g33(.a(new_n35_), .b(x12), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand2  g36(.a(new_n60_), .b(x00), .O(new_n66_));
  nand2  g37(.a(x03), .b(x02), .O(new_n67_));
  nand3  g38(.a(x12), .b(x09), .c(x04), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n67_), .c(new_n40_), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n65_), .O(z08));
  xnor2a g42(.a(x12), .b(x11), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n40_), .c(new_n60_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand3  g45(.a(x04), .b(x03), .c(x02), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(new_n40_), .c(x12), .d(x11), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(new_n60_), .c(x09), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z10));
  nand4  g50(.a(new_n75_), .b(x12), .c(x09), .d(x01), .O(new_n80_));
  oai21  g51(.a(x12), .b(x01), .c(new_n80_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x11), .c(new_n60_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n40_), .O(z11));
  inv1   g54(.a(x00), .O(new_n84_));
  nor2   g55(.a(new_n47_), .b(new_n61_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(x11), .c(new_n60_), .d(new_n39_), .O(new_n86_));
  nor2   g57(.a(new_n86_), .b(new_n84_), .O(z12));
endmodule


