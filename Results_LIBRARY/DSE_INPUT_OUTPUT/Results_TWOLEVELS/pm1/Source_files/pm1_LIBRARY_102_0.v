// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n74_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x11), .O(z00));
  nand2  g04(.a(new_n32_), .b(x12), .O(z01));
  nand2  g05(.a(x08), .b(x07), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n30_), .c(x11), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  inv1   g08(.a(x12), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n36_), .c(x09), .O(z02));
  inv1   g11(.a(new_n30_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x11), .c(x08), .d(x07), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(new_n39_), .c(x09), .O(z03));
  nor2   g14(.a(x12), .b(x01), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n44_), .b(x13), .O(z05));
  and2   g18(.a(x03), .b(x02), .O(new_n48_));
  nand2  g19(.a(x12), .b(new_n37_), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n48_), .c(x04), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n45_), .O(z06));
  nand2  g23(.a(new_n45_), .b(x15), .O(z07));
  nor2   g24(.a(new_n37_), .b(x10), .O(new_n54_));
  inv1   g25(.a(x10), .O(new_n55_));
  nand4  g26(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n39_), .c(new_n55_), .d(x09), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n54_), .c(new_n45_), .d(x00), .O(z08));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n48_), .c(x09), .d(x04), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x12), .c(x11), .O(new_n62_));
  nand3  g33(.a(new_n38_), .b(new_n37_), .c(x01), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n55_), .c(x00), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z09));
  inv1   g37(.a(x09), .O(new_n67_));
  nand4  g38(.a(new_n60_), .b(x12), .c(x11), .d(new_n55_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n45_), .O(z10));
  nand3  g42(.a(new_n69_), .b(x01), .c(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z11));
  nand4  g44(.a(x11), .b(new_n55_), .c(new_n67_), .d(x00), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(new_n38_), .O(z12));
endmodule


