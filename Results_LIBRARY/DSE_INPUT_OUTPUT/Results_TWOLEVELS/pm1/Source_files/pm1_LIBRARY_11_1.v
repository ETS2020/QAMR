// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n88_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  inv1   g02(.a(x08), .O(new_n32_));
  oai21  g03(.a(x15), .b(x11), .c(new_n32_), .O(new_n33_));
  and2   g04(.a(x06), .b(x05), .O(new_n34_));
  nand3  g05(.a(x07), .b(x06), .c(x05), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(new_n34_), .c(x07), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x11), .c(x08), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n33_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g09(.a(x15), .b(new_n32_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n37_), .c(new_n33_), .O(z01));
  inv1   g12(.a(x11), .O(new_n42_));
  nand2  g13(.a(new_n39_), .b(new_n42_), .O(new_n43_));
  nand2  g14(.a(new_n35_), .b(x11), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x12), .c(x09), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x08), .O(new_n46_));
  inv1   g17(.a(x15), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x11), .c(new_n32_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(z02));
  nor2   g20(.a(new_n31_), .b(new_n42_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n39_), .O(new_n52_));
  nand4  g23(.a(new_n34_), .b(x11), .c(x08), .d(x07), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n52_), .O(z03));
  nand2  g25(.a(new_n39_), .b(x14), .O(z04));
  nand2  g26(.a(new_n39_), .b(x13), .O(z05));
  and2   g27(.a(x03), .b(x02), .O(new_n57_));
  nand2  g28(.a(x12), .b(new_n42_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n57_), .c(x04), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x09), .c(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n39_), .O(z06));
  inv1   g32(.a(x10), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(new_n64_));
  nand3  g35(.a(new_n63_), .b(x12), .c(x11), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n64_), .c(x15), .O(z07));
  nand3  g38(.a(x12), .b(x09), .c(x04), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n57_), .c(new_n42_), .O(new_n70_));
  aoi22  g41(.a(new_n70_), .b(new_n64_), .c(x15), .d(new_n32_), .O(z08));
  xnor2a g42(.a(x12), .b(x11), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n39_), .c(new_n62_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand3  g45(.a(x04), .b(x03), .c(x02), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x12), .c(x11), .d(new_n62_), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(new_n77_));
  nand3  g48(.a(new_n77_), .b(x09), .c(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n39_), .O(z10));
  inv1   g50(.a(x00), .O(new_n80_));
  nand2  g51(.a(new_n31_), .b(new_n30_), .O(new_n81_));
  nand4  g52(.a(new_n75_), .b(x12), .c(x09), .d(x01), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(new_n39_), .c(x11), .d(new_n62_), .O(new_n84_));
  nor2   g55(.a(new_n84_), .b(new_n80_), .O(z11));
  inv1   g56(.a(x09), .O(new_n86_));
  nand2  g57(.a(new_n86_), .b(x00), .O(new_n87_));
  nand2  g58(.a(new_n50_), .b(new_n62_), .O(new_n88_));
  oai21  g59(.a(new_n88_), .b(new_n87_), .c(new_n39_), .O(z12));
endmodule


