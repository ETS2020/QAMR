// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n88_,
    new_n89_, new_n90_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_,
    new_n98_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  inv1   g02(.a(x15), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(x11), .c(new_n31_), .O(new_n33_));
  nand2  g04(.a(x06), .b(x05), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand3  g06(.a(new_n34_), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n35_), .c(x11), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n33_), .c(new_n30_), .O(z00));
  inv1   g11(.a(x11), .O(new_n41_));
  inv1   g12(.a(x10), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x00), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  nand3  g15(.a(new_n41_), .b(new_n42_), .c(x00), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x15), .c(new_n31_), .O(new_n47_));
  nand2  g18(.a(new_n37_), .b(new_n35_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x12), .c(x11), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n47_), .O(z01));
  oai21  g21(.a(new_n43_), .b(new_n32_), .c(new_n31_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nand3  g23(.a(new_n45_), .b(x15), .c(new_n31_), .O(new_n53_));
  nand2  g24(.a(x08), .b(x07), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n34_), .c(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x09), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x12), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n53_), .c(new_n52_), .O(z02));
  nand2  g29(.a(x15), .b(new_n31_), .O(new_n59_));
  nand3  g30(.a(x12), .b(x08), .c(x07), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n34_), .c(new_n59_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x11), .O(new_n62_));
  oai21  g33(.a(new_n43_), .b(new_n32_), .c(new_n31_), .O(new_n63_));
  inv1   g34(.a(x09), .O(new_n64_));
  nand2  g35(.a(x12), .b(new_n64_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n63_), .c(new_n62_), .d(new_n52_), .O(z03));
  aoi21  g37(.a(new_n32_), .b(new_n31_), .c(x14), .O(z04));
  nand2  g38(.a(new_n32_), .b(new_n31_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(x13), .O(z05));
  and2   g40(.a(x03), .b(x02), .O(new_n70_));
  nand2  g41(.a(x12), .b(new_n41_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n70_), .c(x04), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x09), .c(x01), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n68_), .O(z06));
  nor2   g45(.a(x15), .b(new_n31_), .O(z07));
  nand3  g46(.a(x04), .b(x03), .c(x02), .O(new_n76_));
  nand3  g47(.a(x12), .b(x11), .c(x09), .O(new_n77_));
  oai22  g48(.a(new_n77_), .b(new_n76_), .c(new_n59_), .d(x11), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(new_n42_), .c(x00), .O(new_n79_));
  oai21  g50(.a(x15), .b(x12), .c(new_n43_), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(new_n79_), .c(new_n71_), .O(z08));
  inv1   g52(.a(x00), .O(new_n82_));
  oai21  g53(.a(new_n43_), .b(x11), .c(x15), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n31_), .O(new_n84_));
  nand4  g55(.a(new_n76_), .b(new_n70_), .c(x09), .d(x04), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(x12), .c(x11), .d(new_n42_), .O(new_n86_));
  oai21  g57(.a(new_n86_), .b(new_n82_), .c(new_n84_), .O(z09));
  nand4  g58(.a(new_n76_), .b(x12), .c(x11), .d(new_n42_), .O(new_n88_));
  inv1   g59(.a(new_n88_), .O(new_n89_));
  nand3  g60(.a(new_n89_), .b(x09), .c(x00), .O(new_n90_));
  nand2  g61(.a(new_n90_), .b(new_n68_), .O(z10));
  nand4  g62(.a(new_n76_), .b(x12), .c(x09), .d(x01), .O(new_n92_));
  nand3  g63(.a(x15), .b(new_n31_), .c(new_n30_), .O(new_n93_));
  nand2  g64(.a(new_n93_), .b(new_n92_), .O(new_n94_));
  nand4  g65(.a(new_n94_), .b(x11), .c(new_n42_), .d(x00), .O(new_n95_));
  inv1   g66(.a(new_n95_), .O(z11));
  nand2  g67(.a(new_n64_), .b(x00), .O(new_n97_));
  nand3  g68(.a(x12), .b(x11), .c(new_n42_), .O(new_n98_));
  oai21  g69(.a(new_n98_), .b(new_n97_), .c(new_n68_), .O(z12));
endmodule


