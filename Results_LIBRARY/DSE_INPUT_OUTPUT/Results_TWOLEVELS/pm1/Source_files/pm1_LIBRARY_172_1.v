// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_;
  inv1   g00(.a(x11), .O(new_n30_));
  aoi21  g01(.a(x12), .b(new_n30_), .c(x01), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  oai21  g05(.a(new_n31_), .b(x15), .c(new_n34_), .O(z00));
  nand2  g06(.a(x15), .b(new_n30_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n34_), .c(x12), .O(z01));
  oai21  g08(.a(x15), .b(x12), .c(new_n30_), .O(new_n38_));
  nand2  g09(.a(x08), .b(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n32_), .c(x11), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(x12), .d(x09), .O(z02));
  nand3  g12(.a(x11), .b(x08), .c(x07), .O(new_n42_));
  or2    g13(.a(new_n42_), .b(new_n32_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n38_), .c(x12), .d(x09), .O(z03));
  aoi21  g15(.a(x15), .b(new_n30_), .c(x14), .O(z04));
  nand2  g16(.a(new_n36_), .b(x13), .O(z05));
  inv1   g17(.a(x15), .O(new_n47_));
  nand3  g18(.a(x12), .b(x09), .c(x01), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n30_), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n50_), .O(z06));
  nand2  g24(.a(x15), .b(x11), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  nor2   g27(.a(new_n56_), .b(new_n51_), .O(new_n57_));
  nor3   g28(.a(x15), .b(x12), .c(x11), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n57_), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n38_), .c(new_n55_), .d(x00), .O(z08));
  inv1   g31(.a(new_n58_), .O(new_n61_));
  and2   g32(.a(x03), .b(x02), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n51_), .c(x09), .d(x04), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(x12), .c(x11), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n55_), .c(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z09));
  inv1   g38(.a(x00), .O(new_n68_));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(new_n51_), .b(x12), .c(x11), .d(new_n55_), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z10));
  nand4  g42(.a(new_n51_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  oai21  g43(.a(x12), .b(x01), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n55_), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z11));
  nand2  g46(.a(new_n69_), .b(x00), .O(new_n76_));
  nand3  g47(.a(x12), .b(x11), .c(new_n55_), .O(new_n77_));
  oai21  g48(.a(new_n77_), .b(new_n76_), .c(new_n36_), .O(z12));
endmodule


