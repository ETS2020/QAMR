// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n58_, new_n59_, new_n60_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_;
  inv1   g00(.a(x06), .O(new_n30_));
  inv1   g01(.a(x15), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x11), .c(new_n30_), .O(new_n32_));
  and2   g03(.a(x07), .b(x05), .O(new_n33_));
  nand3  g04(.a(x08), .b(x07), .c(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n33_), .c(x08), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x11), .c(x06), .O(new_n36_));
  nor2   g07(.a(x12), .b(x01), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n36_), .c(new_n32_), .O(z00));
  inv1   g09(.a(x12), .O(new_n39_));
  nand2  g10(.a(new_n31_), .b(new_n30_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n36_), .c(new_n32_), .O(z01));
  inv1   g13(.a(x11), .O(new_n43_));
  nand2  g14(.a(new_n40_), .b(new_n43_), .O(new_n44_));
  nand2  g15(.a(new_n34_), .b(x11), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(x12), .c(x09), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x06), .O(new_n47_));
  nand3  g18(.a(x15), .b(x11), .c(new_n30_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n47_), .c(new_n44_), .O(z02));
  nor2   g20(.a(new_n39_), .b(new_n43_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x09), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n40_), .O(new_n52_));
  nand2  g23(.a(x06), .b(x05), .O(new_n53_));
  nand3  g24(.a(x11), .b(x08), .c(x07), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n53_), .c(new_n52_), .O(z03));
  nand2  g26(.a(new_n40_), .b(x14), .O(z04));
  nand2  g27(.a(new_n40_), .b(x13), .O(z05));
  nand2  g28(.a(x12), .b(new_n43_), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(x04), .c(x03), .d(x02), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x09), .c(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n40_), .O(z06));
  nor2   g32(.a(x15), .b(new_n30_), .O(z07));
  inv1   g33(.a(x00), .O(new_n63_));
  nor2   g34(.a(x10), .b(new_n63_), .O(new_n64_));
  inv1   g35(.a(x10), .O(new_n65_));
  nand4  g36(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n50_), .c(new_n65_), .d(x09), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n64_), .c(new_n40_), .d(x11), .O(z08));
  nand4  g40(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n40_), .O(new_n71_));
  nand4  g42(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(x12), .c(x11), .O(new_n74_));
  nand2  g45(.a(new_n39_), .b(new_n43_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(new_n65_), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n40_), .O(z09));
  nand3  g49(.a(x04), .b(x03), .c(x02), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(new_n40_), .c(x12), .d(x11), .O(new_n80_));
  inv1   g51(.a(new_n80_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(new_n65_), .c(x09), .d(x00), .O(new_n82_));
  inv1   g53(.a(new_n82_), .O(z10));
  nand4  g54(.a(new_n79_), .b(x12), .c(x09), .d(x01), .O(new_n84_));
  oai21  g55(.a(x12), .b(x01), .c(new_n84_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(new_n40_), .c(x11), .d(new_n65_), .O(new_n86_));
  nor2   g57(.a(new_n86_), .b(new_n63_), .O(z11));
  nand4  g58(.a(new_n40_), .b(x12), .c(x11), .d(new_n65_), .O(new_n88_));
  nor3   g59(.a(new_n88_), .b(x09), .c(new_n63_), .O(z12));
endmodule


