// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x11), .O(new_n30_));
  and2   g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(x07), .b(x06), .c(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(x07), .O(new_n33_));
  aoi21  g04(.a(new_n33_), .b(x08), .c(x15), .O(new_n34_));
  inv1   g05(.a(x01), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n30_), .O(new_n36_));
  inv1   g07(.a(x08), .O(new_n37_));
  inv1   g08(.a(x15), .O(z07));
  nand2  g09(.a(z07), .b(new_n37_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n36_), .c(new_n35_), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  oai21  g12(.a(new_n34_), .b(new_n30_), .c(new_n41_), .O(z00));
  inv1   g13(.a(x12), .O(new_n43_));
  nor2   g14(.a(x15), .b(x08), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  oai21  g16(.a(new_n34_), .b(new_n30_), .c(new_n45_), .O(z01));
  nand3  g17(.a(x12), .b(x11), .c(x09), .O(new_n47_));
  aoi21  g18(.a(new_n32_), .b(x11), .c(new_n47_), .O(new_n48_));
  nand2  g19(.a(x15), .b(new_n37_), .O(new_n49_));
  oai21  g20(.a(new_n48_), .b(new_n37_), .c(new_n49_), .O(z02));
  inv1   g21(.a(x09), .O(new_n51_));
  nor2   g22(.a(new_n43_), .b(new_n51_), .O(new_n52_));
  nand4  g23(.a(new_n31_), .b(x11), .c(x08), .d(x07), .O(new_n53_));
  nand4  g24(.a(new_n53_), .b(new_n52_), .c(new_n39_), .d(new_n36_), .O(z03));
  nor2   g25(.a(new_n44_), .b(x14), .O(z04));
  nand2  g26(.a(new_n39_), .b(x13), .O(z05));
  nand4  g27(.a(new_n36_), .b(x04), .c(x03), .d(x02), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n39_), .c(x09), .d(x01), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z06));
  inv1   g30(.a(x10), .O(new_n60_));
  nand3  g31(.a(x04), .b(x03), .c(x02), .O(new_n61_));
  nand2  g32(.a(new_n43_), .b(new_n30_), .O(new_n62_));
  oai21  g33(.a(new_n61_), .b(new_n47_), .c(new_n62_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  nor2   g35(.a(new_n44_), .b(x10), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n64_), .c(new_n36_), .d(x00), .O(z08));
  nand3  g37(.a(new_n61_), .b(new_n44_), .c(x09), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n62_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n60_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n39_), .O(z09));
  nand4  g42(.a(new_n61_), .b(new_n39_), .c(x12), .d(x11), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n60_), .c(x09), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z10));
  nand4  g46(.a(new_n61_), .b(x12), .c(x09), .d(x01), .O(new_n76_));
  oai21  g47(.a(x12), .b(x01), .c(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n60_), .d(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n39_), .O(z11));
  nand2  g50(.a(new_n51_), .b(x00), .O(new_n80_));
  nand3  g51(.a(x12), .b(x11), .c(new_n60_), .O(new_n81_));
  oai21  g52(.a(new_n81_), .b(new_n80_), .c(new_n39_), .O(z12));
endmodule


