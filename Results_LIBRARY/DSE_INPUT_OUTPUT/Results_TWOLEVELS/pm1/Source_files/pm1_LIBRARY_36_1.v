// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x10), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x03), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  nand2  g08(.a(x12), .b(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(new_n33_), .d(new_n30_), .O(z00));
  nand3  g10(.a(new_n36_), .b(new_n33_), .c(x12), .O(z01));
  inv1   g11(.a(new_n31_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x08), .c(x07), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  inv1   g14(.a(x09), .O(new_n44_));
  nor2   g15(.a(new_n35_), .b(new_n44_), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(new_n43_), .c(x12), .d(x11), .O(z02));
  inv1   g17(.a(x12), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand4  g19(.a(new_n41_), .b(x11), .c(x08), .d(x07), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(new_n48_), .c(new_n38_), .d(new_n36_), .O(z03));
  nand2  g21(.a(new_n36_), .b(x14), .O(z04));
  nand2  g22(.a(new_n36_), .b(x13), .O(z05));
  aoi21  g23(.a(x09), .b(x01), .c(x10), .O(new_n53_));
  nand3  g24(.a(new_n38_), .b(x04), .c(x02), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  oai21  g26(.a(new_n53_), .b(x03), .c(new_n55_), .O(z06));
  nand2  g27(.a(new_n36_), .b(x15), .O(z07));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand2  g29(.a(x12), .b(x11), .O(new_n59_));
  nor3   g30(.a(new_n59_), .b(new_n58_), .c(new_n44_), .O(new_n60_));
  aoi21  g31(.a(new_n47_), .b(new_n37_), .c(new_n60_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n38_), .c(new_n34_), .d(x00), .O(z08));
  nand2  g33(.a(new_n47_), .b(new_n37_), .O(new_n63_));
  and2   g34(.a(x03), .b(x02), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n58_), .c(x09), .d(x04), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x12), .c(x11), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n34_), .c(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z09));
  inv1   g40(.a(x03), .O(new_n70_));
  nand2  g41(.a(x09), .b(x00), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n59_), .c(new_n34_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n70_), .O(new_n73_));
  nand2  g44(.a(x04), .b(x02), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x12), .c(x11), .d(new_n34_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n73_), .O(z10));
  nand4  g49(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n34_), .d(x00), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(z11));
  nand4  g53(.a(x11), .b(new_n34_), .c(new_n44_), .d(x00), .O(new_n83_));
  nor2   g54(.a(new_n83_), .b(new_n47_), .O(z12));
endmodule


