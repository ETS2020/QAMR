// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nand3  g05(.a(x11), .b(new_n34_), .c(x00), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  inv1   g07(.a(x15), .O(z07));
  nor2   g08(.a(z07), .b(new_n32_), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n36_), .c(new_n31_), .O(z01));
  nor2   g11(.a(new_n31_), .b(new_n30_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n34_), .c(new_n30_), .d(x00), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n32_), .O(new_n43_));
  and2   g14(.a(x07), .b(x06), .O(new_n44_));
  inv1   g15(.a(x09), .O(new_n45_));
  aoi21  g16(.a(x12), .b(new_n31_), .c(new_n45_), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(new_n44_), .c(x08), .d(x05), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(z07), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n43_), .O(z02));
  oai21  g20(.a(x15), .b(new_n31_), .c(x12), .O(new_n50_));
  and2   g21(.a(x06), .b(x05), .O(new_n51_));
  and2   g22(.a(x08), .b(x07), .O(new_n52_));
  aoi22  g23(.a(new_n52_), .b(new_n51_), .c(z07), .d(new_n45_), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(new_n43_), .c(new_n50_), .O(z03));
  nor2   g25(.a(new_n38_), .b(x14), .O(z04));
  nor2   g26(.a(new_n38_), .b(x13), .O(z05));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  nand3  g29(.a(z07), .b(x12), .c(new_n31_), .O(new_n59_));
  oai21  g30(.a(new_n58_), .b(new_n38_), .c(new_n59_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(x09), .c(x01), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z06));
  nand2  g33(.a(new_n32_), .b(new_n31_), .O(new_n63_));
  nand4  g34(.a(z07), .b(x12), .c(x11), .d(x09), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n57_), .c(new_n63_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n34_), .c(x00), .O(new_n66_));
  nand2  g37(.a(new_n34_), .b(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n39_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n66_), .c(new_n59_), .O(z08));
  and2   g40(.a(x03), .b(x02), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n57_), .c(x09), .d(x04), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(z07), .c(x12), .d(x11), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n63_), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(new_n34_), .c(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z09));
  nand4  g46(.a(new_n57_), .b(z07), .c(x12), .d(x11), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(new_n34_), .c(x09), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z10));
  nand4  g50(.a(new_n57_), .b(x12), .c(x09), .d(x01), .O(new_n80_));
  oai21  g51(.a(x12), .b(x01), .c(new_n80_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x11), .c(new_n34_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n39_), .O(z11));
  nor2   g54(.a(x15), .b(new_n31_), .O(new_n84_));
  nand4  g55(.a(new_n84_), .b(new_n34_), .c(new_n45_), .d(x00), .O(new_n85_));
  aoi21  g56(.a(new_n85_), .b(z07), .c(new_n32_), .O(z12));
endmodule


