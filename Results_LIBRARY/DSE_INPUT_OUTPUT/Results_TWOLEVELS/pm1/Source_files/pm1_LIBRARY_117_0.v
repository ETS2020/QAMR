// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n97_;
  inv1   g00(.a(x15), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  aoi21  g02(.a(new_n31_), .b(x11), .c(x01), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x06), .b(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x08), .c(x07), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x06), .c(x05), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n33_), .c(x12), .O(new_n38_));
  oai21  g09(.a(new_n32_), .b(new_n30_), .c(new_n38_), .O(z00));
  inv1   g10(.a(x00), .O(new_n40_));
  nor2   g11(.a(x10), .b(new_n40_), .O(new_n41_));
  inv1   g12(.a(x10), .O(new_n42_));
  nand3  g13(.a(new_n33_), .b(new_n42_), .c(x00), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(x15), .d(new_n33_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n31_), .O(new_n45_));
  nand3  g16(.a(new_n37_), .b(x12), .c(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n45_), .O(z01));
  inv1   g18(.a(new_n41_), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n30_), .c(new_n31_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n33_), .O(new_n50_));
  nand3  g21(.a(new_n43_), .b(x15), .c(new_n31_), .O(new_n51_));
  nand2  g22(.a(x08), .b(x07), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n34_), .c(x11), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x09), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n51_), .c(new_n50_), .O(z02));
  nor2   g27(.a(new_n30_), .b(x12), .O(new_n57_));
  nand3  g28(.a(x12), .b(x08), .c(x07), .O(new_n58_));
  nor2   g29(.a(new_n58_), .b(new_n34_), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n57_), .c(x11), .O(new_n60_));
  nand3  g31(.a(new_n48_), .b(x15), .c(new_n31_), .O(new_n61_));
  inv1   g32(.a(x09), .O(new_n62_));
  nand2  g33(.a(x12), .b(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n61_), .c(new_n60_), .d(new_n50_), .O(z03));
  nor2   g35(.a(x15), .b(x12), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(x14), .O(z04));
  nor2   g37(.a(new_n65_), .b(x13), .O(z05));
  inv1   g38(.a(new_n65_), .O(new_n68_));
  and2   g39(.a(x03), .b(x02), .O(new_n69_));
  nand2  g40(.a(x12), .b(new_n33_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n69_), .c(x04), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(x09), .c(x01), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n68_), .O(z06));
  nor2   g44(.a(x15), .b(new_n31_), .O(z07));
  nand3  g45(.a(x04), .b(x03), .c(x02), .O(new_n75_));
  nand3  g46(.a(x12), .b(x11), .c(x09), .O(new_n76_));
  nand2  g47(.a(new_n57_), .b(new_n33_), .O(new_n77_));
  oai21  g48(.a(new_n76_), .b(new_n75_), .c(new_n77_), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(new_n42_), .c(x00), .O(new_n79_));
  oai21  g50(.a(new_n57_), .b(x12), .c(new_n48_), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(new_n79_), .c(new_n70_), .O(z08));
  nand4  g52(.a(new_n75_), .b(new_n69_), .c(x09), .d(x04), .O(new_n82_));
  nand3  g53(.a(new_n82_), .b(x12), .c(x11), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n77_), .O(new_n84_));
  nand3  g55(.a(new_n84_), .b(new_n42_), .c(x00), .O(new_n85_));
  inv1   g56(.a(new_n85_), .O(z09));
  nand4  g57(.a(new_n75_), .b(x12), .c(x11), .d(new_n42_), .O(new_n87_));
  nor2   g58(.a(new_n87_), .b(new_n62_), .O(new_n88_));
  nand2  g59(.a(new_n88_), .b(x00), .O(new_n89_));
  nand2  g60(.a(new_n89_), .b(new_n68_), .O(z10));
  inv1   g61(.a(x01), .O(new_n91_));
  nand4  g62(.a(x11), .b(new_n42_), .c(new_n91_), .d(x00), .O(new_n92_));
  nand2  g63(.a(new_n92_), .b(x15), .O(new_n93_));
  nand2  g64(.a(new_n93_), .b(new_n31_), .O(new_n94_));
  nand3  g65(.a(new_n88_), .b(x01), .c(x00), .O(new_n95_));
  nand2  g66(.a(new_n95_), .b(new_n94_), .O(z11));
  nand4  g67(.a(x11), .b(new_n42_), .c(new_n62_), .d(x00), .O(new_n97_));
  nor2   g68(.a(new_n97_), .b(new_n31_), .O(z12));
endmodule


