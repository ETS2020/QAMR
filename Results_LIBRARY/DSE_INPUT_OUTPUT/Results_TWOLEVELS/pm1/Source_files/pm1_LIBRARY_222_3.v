// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_;
  nand2  g00(.a(x15), .b(x10), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n32_), .b(x08), .c(x07), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  aoi21  g07(.a(new_n36_), .b(x11), .c(x01), .O(new_n37_));
  inv1   g08(.a(x15), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x10), .O(new_n39_));
  oai21  g10(.a(x11), .b(x10), .c(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(new_n41_));
  oai21  g12(.a(new_n37_), .b(new_n31_), .c(new_n41_), .O(z00));
  nand3  g13(.a(new_n36_), .b(new_n30_), .c(x11), .O(new_n43_));
  inv1   g14(.a(x10), .O(new_n44_));
  inv1   g15(.a(x12), .O(new_n45_));
  oai21  g16(.a(new_n38_), .b(new_n44_), .c(new_n45_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n43_), .O(z01));
  inv1   g18(.a(x09), .O(new_n48_));
  nand3  g19(.a(new_n33_), .b(x08), .c(x07), .O(new_n49_));
  aoi21  g20(.a(new_n49_), .b(x11), .c(new_n48_), .O(new_n50_));
  inv1   g21(.a(x11), .O(new_n51_));
  oai22  g22(.a(new_n38_), .b(new_n44_), .c(new_n45_), .d(new_n51_), .O(new_n52_));
  oai21  g23(.a(new_n50_), .b(new_n31_), .c(new_n52_), .O(z02));
  nand3  g24(.a(x11), .b(x08), .c(x07), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n32_), .c(x09), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n30_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n52_), .O(z03));
  nand2  g28(.a(new_n30_), .b(x14), .O(z04));
  nor2   g29(.a(new_n31_), .b(x13), .O(z05));
  nand2  g30(.a(x12), .b(new_n51_), .O(new_n60_));
  and2   g31(.a(x03), .b(x02), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n60_), .c(x04), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(x09), .c(x01), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n30_), .O(z06));
  nor2   g35(.a(new_n36_), .b(new_n51_), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(x15), .O(z07));
  nand3  g37(.a(x04), .b(x03), .c(x02), .O(new_n67_));
  nand3  g38(.a(x12), .b(x11), .c(x09), .O(new_n68_));
  nand2  g39(.a(new_n45_), .b(new_n51_), .O(new_n69_));
  oai21  g40(.a(new_n68_), .b(new_n67_), .c(new_n69_), .O(new_n70_));
  nand2  g41(.a(new_n60_), .b(x00), .O(new_n71_));
  aoi21  g42(.a(new_n70_), .b(x00), .c(new_n71_), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(x10), .c(new_n39_), .O(z08));
  nand4  g44(.a(new_n67_), .b(new_n61_), .c(x09), .d(x04), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x12), .c(x11), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n69_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(new_n44_), .c(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z09));
  nand4  g49(.a(new_n67_), .b(x12), .c(x11), .d(new_n44_), .O(new_n79_));
  inv1   g50(.a(new_n79_), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(x09), .c(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n30_), .O(z10));
  nand4  g53(.a(new_n67_), .b(x12), .c(x09), .d(x01), .O(new_n83_));
  oai21  g54(.a(x12), .b(x01), .c(new_n83_), .O(new_n84_));
  nand4  g55(.a(new_n84_), .b(x11), .c(new_n44_), .d(x00), .O(new_n85_));
  inv1   g56(.a(new_n85_), .O(z11));
  nand2  g57(.a(new_n48_), .b(x00), .O(new_n87_));
  nand3  g58(.a(x12), .b(x11), .c(new_n44_), .O(new_n88_));
  oai21  g59(.a(new_n88_), .b(new_n87_), .c(new_n30_), .O(z12));
endmodule


