// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(x14), .b(x11), .c(x08), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x14), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x07), .O(new_n33_));
  nand3  g04(.a(x08), .b(x06), .c(x05), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x14), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x07), .O(new_n36_));
  inv1   g07(.a(x01), .O(new_n37_));
  inv1   g08(.a(x12), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  aoi21  g10(.a(new_n36_), .b(x11), .c(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n33_), .O(z00));
  inv1   g12(.a(x14), .O(new_n42_));
  inv1   g13(.a(x07), .O(new_n43_));
  inv1   g14(.a(x11), .O(new_n44_));
  nand2  g15(.a(x12), .b(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n43_), .O(new_n46_));
  nand4  g17(.a(x08), .b(new_n43_), .c(x06), .d(x05), .O(new_n47_));
  aoi21  g18(.a(new_n47_), .b(x11), .c(new_n38_), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n42_), .c(new_n46_), .O(z01));
  oai21  g20(.a(new_n42_), .b(x11), .c(new_n43_), .O(new_n50_));
  nand3  g21(.a(x12), .b(x11), .c(x09), .O(new_n51_));
  aoi21  g22(.a(new_n34_), .b(x11), .c(new_n51_), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n42_), .c(new_n50_), .O(z02));
  inv1   g24(.a(new_n51_), .O(new_n54_));
  nor2   g25(.a(x14), .b(new_n43_), .O(new_n55_));
  nand3  g26(.a(x07), .b(x06), .c(x05), .O(new_n56_));
  oai22  g27(.a(new_n56_), .b(new_n31_), .c(new_n55_), .d(new_n54_), .O(z03));
  nor2   g28(.a(x14), .b(x07), .O(z04));
  inv1   g29(.a(new_n55_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x13), .O(z05));
  and2   g31(.a(x03), .b(x02), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n45_), .c(x04), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n59_), .c(x09), .d(x01), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z06));
  nor2   g35(.a(new_n55_), .b(x15), .O(z07));
  inv1   g36(.a(x00), .O(new_n66_));
  nor2   g37(.a(x10), .b(new_n66_), .O(new_n67_));
  nand4  g38(.a(new_n61_), .b(x12), .c(x09), .d(x04), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n67_), .c(new_n59_), .d(x11), .O(z08));
  inv1   g40(.a(x10), .O(new_n70_));
  xnor2a g41(.a(x12), .b(x11), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n59_), .c(new_n70_), .d(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z09));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x12), .c(x11), .d(new_n70_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n59_), .O(z10));
  nand4  g49(.a(new_n74_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n39_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n70_), .d(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n59_), .O(z11));
  inv1   g53(.a(x09), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(x00), .O(new_n84_));
  nand3  g55(.a(x12), .b(x11), .c(new_n70_), .O(new_n85_));
  oai21  g56(.a(new_n85_), .b(new_n84_), .c(new_n59_), .O(z12));
endmodule


