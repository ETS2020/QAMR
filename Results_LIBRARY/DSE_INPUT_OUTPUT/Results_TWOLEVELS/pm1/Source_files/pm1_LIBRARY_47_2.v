// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x09), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x11), .O(new_n34_));
  inv1   g05(.a(x01), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  inv1   g08(.a(x09), .O(new_n38_));
  nor2   g09(.a(x12), .b(new_n38_), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n34_), .O(z00));
  inv1   g14(.a(x12), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n38_), .O(new_n45_));
  oai21  g16(.a(new_n32_), .b(new_n36_), .c(new_n45_), .O(z01));
  nand2  g17(.a(new_n37_), .b(new_n38_), .O(new_n47_));
  nand4  g18(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n36_), .c(x12), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n47_), .O(z02));
  nand3  g21(.a(x11), .b(x08), .c(x07), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n30_), .c(x11), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n47_), .O(z03));
  nand2  g25(.a(new_n40_), .b(x14), .O(z04));
  nor2   g26(.a(new_n39_), .b(x13), .O(z05));
  nand2  g27(.a(x03), .b(x02), .O(new_n57_));
  nand2  g28(.a(x11), .b(x04), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(x01), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(x12), .c(new_n38_), .O(z06));
  nand2  g31(.a(new_n40_), .b(x15), .O(z07));
  inv1   g32(.a(x10), .O(new_n62_));
  nand3  g33(.a(x11), .b(x09), .c(x04), .O(new_n63_));
  oai22  g34(.a(new_n63_), .b(new_n57_), .c(x12), .d(x11), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  aoi21  g36(.a(x12), .b(new_n36_), .c(x10), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(new_n65_), .c(new_n40_), .d(x00), .O(z08));
  nand3  g38(.a(new_n36_), .b(new_n62_), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n38_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n44_), .O(new_n70_));
  nand3  g41(.a(x04), .b(x03), .c(x02), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n38_), .c(new_n44_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n62_), .d(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n70_), .O(z09));
  inv1   g45(.a(x00), .O(new_n75_));
  nand4  g46(.a(new_n71_), .b(x12), .c(x11), .d(new_n62_), .O(new_n76_));
  nor3   g47(.a(new_n76_), .b(new_n38_), .c(new_n75_), .O(z10));
  nand4  g48(.a(new_n71_), .b(x12), .c(x09), .d(x01), .O(new_n78_));
  oai21  g49(.a(new_n45_), .b(x01), .c(new_n78_), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(x11), .c(new_n62_), .d(x00), .O(new_n80_));
  inv1   g51(.a(new_n80_), .O(z11));
  nand2  g52(.a(new_n38_), .b(x00), .O(new_n82_));
  nand3  g53(.a(x12), .b(x11), .c(new_n62_), .O(new_n83_));
  oai21  g54(.a(new_n83_), .b(new_n82_), .c(new_n40_), .O(z12));
endmodule


