// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n43_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n71_;
  inv1   g00(.a(x11), .O(new_n30_));
  oai21  g01(.a(x12), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x12), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n30_), .c(new_n31_), .O(z00));
  oai21  g06(.a(new_n34_), .b(new_n30_), .c(x12), .O(z01));
  inv1   g07(.a(new_n32_), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(x08), .c(x07), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g11(.a(new_n37_), .b(x11), .c(x08), .d(x07), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z03));
  inv1   g13(.a(x12), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x11), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x14), .O(z04));
  aoi21  g16(.a(new_n43_), .b(x11), .c(x13), .O(z05));
  nand2  g17(.a(x12), .b(new_n30_), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n44_), .O(z06));
  nand2  g21(.a(new_n44_), .b(x15), .O(z07));
  nor2   g22(.a(new_n43_), .b(x10), .O(new_n52_));
  nand3  g23(.a(x03), .b(x02), .c(x00), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nor2   g25(.a(new_n30_), .b(x10), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(new_n54_), .c(x09), .d(x04), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n52_), .c(new_n47_), .d(x00), .O(z08));
  inv1   g28(.a(x10), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(x00), .c(x11), .O(new_n59_));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n43_), .c(x09), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x11), .c(new_n58_), .d(x00), .O(new_n62_));
  oai21  g33(.a(new_n59_), .b(x12), .c(new_n62_), .O(z09));
  inv1   g34(.a(x00), .O(new_n64_));
  inv1   g35(.a(x09), .O(new_n65_));
  nand4  g36(.a(new_n60_), .b(x12), .c(x11), .d(new_n58_), .O(new_n66_));
  nor3   g37(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(z10));
  inv1   g38(.a(new_n66_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x09), .c(x01), .d(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z11));
  nand3  g41(.a(new_n58_), .b(new_n65_), .c(x00), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x12), .c(new_n30_), .O(z12));
endmodule


