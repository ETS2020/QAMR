// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x04), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n32_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n31_), .O(z00));
  nand3  g11(.a(new_n35_), .b(x12), .c(new_n32_), .O(z01));
  nand2  g12(.a(x08), .b(x07), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(new_n33_), .c(x11), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n37_), .c(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n32_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(z02));
  inv1   g17(.a(x09), .O(new_n47_));
  nand3  g18(.a(x11), .b(x08), .c(x07), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n33_), .c(new_n37_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n47_), .c(new_n32_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x12), .O(z03));
  nand2  g22(.a(x12), .b(x04), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x14), .O(z04));
  nand2  g24(.a(new_n52_), .b(x13), .O(z05));
  inv1   g25(.a(x01), .O(new_n55_));
  aoi21  g26(.a(x03), .b(x02), .c(x12), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n32_), .c(x09), .O(new_n57_));
  nor2   g28(.a(new_n57_), .b(new_n55_), .O(z06));
  aoi21  g29(.a(x12), .b(x04), .c(x15), .O(z07));
  nor2   g30(.a(x12), .b(x10), .O(new_n60_));
  aoi22  g31(.a(new_n60_), .b(x00), .c(x12), .d(new_n32_), .O(new_n61_));
  inv1   g32(.a(x10), .O(new_n62_));
  nand3  g33(.a(new_n52_), .b(new_n62_), .c(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(new_n64_));
  oai21  g35(.a(new_n61_), .b(x11), .c(new_n64_), .O(z08));
  nand3  g36(.a(x12), .b(x11), .c(new_n32_), .O(new_n66_));
  oai21  g37(.a(x12), .b(x11), .c(new_n66_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n62_), .c(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z09));
  inv1   g40(.a(x00), .O(new_n70_));
  nor2   g41(.a(x04), .b(new_n70_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n62_), .c(x09), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(new_n30_), .c(new_n36_), .O(z10));
  nand4  g44(.a(x12), .b(x09), .c(new_n32_), .d(x01), .O(new_n74_));
  oai21  g45(.a(x12), .b(x01), .c(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x11), .c(new_n62_), .d(x00), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(z11));
  nand4  g48(.a(new_n71_), .b(x11), .c(new_n62_), .d(new_n47_), .O(new_n78_));
  aoi21  g49(.a(new_n78_), .b(new_n32_), .c(new_n30_), .O(z12));
endmodule


