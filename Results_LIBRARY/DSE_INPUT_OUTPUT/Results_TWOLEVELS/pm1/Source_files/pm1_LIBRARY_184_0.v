// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:02 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n39_, new_n40_,
    new_n42_, new_n44_, new_n45_, new_n46_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(x13), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand4  g05(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g08(.a(x13), .O(z05));
  nand3  g09(.a(new_n35_), .b(x11), .c(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(new_n40_));
  oai21  g11(.a(z05), .b(x12), .c(new_n40_), .O(z03));
  nand2  g12(.a(z05), .b(new_n32_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  and2   g14(.a(x03), .b(x02), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(z01), .c(x04), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(x09), .c(x01), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n42_), .O(z06));
  nand2  g18(.a(new_n42_), .b(x15), .O(z07));
  inv1   g19(.a(x10), .O(new_n49_));
  nand3  g20(.a(x04), .b(x03), .c(x02), .O(new_n50_));
  nand3  g21(.a(x12), .b(x11), .c(x09), .O(new_n51_));
  oai22  g22(.a(new_n51_), .b(new_n50_), .c(x12), .d(x11), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n49_), .O(new_n53_));
  aoi21  g24(.a(z05), .b(new_n32_), .c(x10), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n53_), .c(z01), .d(x00), .O(z08));
  inv1   g26(.a(x00), .O(new_n56_));
  nand3  g27(.a(new_n31_), .b(new_n49_), .c(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x13), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n32_), .O(new_n59_));
  nand4  g30(.a(new_n50_), .b(new_n44_), .c(x09), .d(x04), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x12), .c(x11), .d(new_n49_), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n56_), .c(new_n59_), .O(z09));
  inv1   g33(.a(x09), .O(new_n63_));
  nand4  g34(.a(new_n50_), .b(x12), .c(x11), .d(new_n49_), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n63_), .c(new_n56_), .O(z10));
  nand4  g36(.a(x11), .b(new_n49_), .c(new_n30_), .d(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x13), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n32_), .O(new_n68_));
  inv1   g39(.a(new_n64_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x09), .c(x01), .d(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n68_), .O(z11));
  nand2  g42(.a(new_n63_), .b(x00), .O(new_n72_));
  nand3  g43(.a(x12), .b(x11), .c(new_n49_), .O(new_n73_));
  oai21  g44(.a(new_n73_), .b(new_n72_), .c(new_n42_), .O(z12));
endmodule


