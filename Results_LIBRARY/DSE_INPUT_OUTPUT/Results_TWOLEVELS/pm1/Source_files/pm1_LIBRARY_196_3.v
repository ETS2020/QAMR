// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n35_, new_n37_, new_n38_, new_n40_,
    new_n41_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_;
  nor2   g00(.a(x12), .b(x11), .O(new_n30_));
  nor2   g01(.a(x11), .b(x01), .O(new_n31_));
  oai22  g02(.a(new_n31_), .b(x12), .c(new_n30_), .d(x08), .O(z00));
  inv1   g03(.a(x11), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x08), .c(x12), .O(z01));
  inv1   g05(.a(x08), .O(new_n35_));
  nand3  g06(.a(new_n30_), .b(x12), .c(new_n35_), .O(z02));
  inv1   g07(.a(x09), .O(new_n37_));
  aoi21  g08(.a(x12), .b(new_n33_), .c(new_n37_), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(x08), .c(x12), .O(z03));
  inv1   g10(.a(x12), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n35_), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(x14), .O(z04));
  nor2   g13(.a(new_n41_), .b(x13), .O(z05));
  nand3  g14(.a(x04), .b(x03), .c(x02), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand3  g16(.a(x12), .b(new_n33_), .c(new_n35_), .O(new_n46_));
  oai21  g17(.a(new_n45_), .b(new_n41_), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(z06));
  inv1   g20(.a(new_n41_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x12), .b(x11), .c(x09), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  aoi22  g25(.a(new_n54_), .b(new_n45_), .c(new_n40_), .d(new_n33_), .O(new_n55_));
  oai21  g26(.a(new_n33_), .b(x08), .c(x12), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n55_), .c(new_n52_), .d(x00), .O(z08));
  nand2  g28(.a(new_n40_), .b(new_n33_), .O(new_n58_));
  nand3  g29(.a(new_n44_), .b(x09), .c(x08), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x12), .c(x11), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n52_), .c(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n50_), .O(z09));
  nand4  g34(.a(new_n44_), .b(x12), .c(x11), .d(new_n52_), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x09), .c(new_n35_), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z10));
  nand4  g38(.a(new_n44_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  inv1   g39(.a(x01), .O(new_n69_));
  nand2  g40(.a(new_n40_), .b(new_n69_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(x11), .c(new_n52_), .d(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n50_), .O(z11));
  nand4  g44(.a(x11), .b(new_n52_), .c(new_n37_), .d(x00), .O(new_n74_));
  aoi21  g45(.a(new_n74_), .b(new_n35_), .c(new_n40_), .O(z12));
endmodule


