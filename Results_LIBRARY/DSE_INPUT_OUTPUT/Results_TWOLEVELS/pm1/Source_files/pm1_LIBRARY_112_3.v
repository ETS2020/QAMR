// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_;
  inv1   g00(.a(x07), .O(new_n30_));
  oai21  g01(.a(x12), .b(x11), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  oai21  g03(.a(x11), .b(x01), .c(new_n32_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n31_), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(x07), .c(x12), .O(z01));
  nand2  g07(.a(new_n31_), .b(x12), .O(z02));
  inv1   g08(.a(x09), .O(new_n38_));
  aoi21  g09(.a(x12), .b(new_n35_), .c(new_n38_), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(x07), .c(x12), .O(z03));
  nor2   g11(.a(new_n32_), .b(new_n30_), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x14), .O(z04));
  nand2  g14(.a(new_n42_), .b(x13), .O(z05));
  nand3  g15(.a(x04), .b(x03), .c(x02), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand3  g17(.a(x12), .b(new_n35_), .c(new_n30_), .O(new_n47_));
  oai21  g18(.a(new_n46_), .b(new_n41_), .c(new_n47_), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(z06));
  nand2  g21(.a(new_n42_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand4  g23(.a(x12), .b(x11), .c(x09), .d(new_n30_), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  aoi22  g25(.a(new_n54_), .b(new_n46_), .c(new_n32_), .d(new_n35_), .O(new_n55_));
  oai21  g26(.a(new_n35_), .b(x07), .c(x12), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n55_), .c(new_n52_), .d(x00), .O(z08));
  nand2  g28(.a(new_n32_), .b(new_n35_), .O(new_n58_));
  and2   g29(.a(x03), .b(x02), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n45_), .c(x09), .d(x04), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x12), .c(x11), .d(new_n30_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n58_), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n52_), .c(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z09));
  nand4  g35(.a(new_n45_), .b(x12), .c(x11), .d(new_n52_), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x09), .c(new_n30_), .d(x00), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(z10));
  inv1   g39(.a(x01), .O(new_n69_));
  nand4  g40(.a(new_n45_), .b(x12), .c(x09), .d(new_n30_), .O(new_n70_));
  nand2  g41(.a(new_n32_), .b(new_n69_), .O(new_n71_));
  oai21  g42(.a(new_n70_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n52_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z11));
  nand4  g45(.a(new_n52_), .b(new_n38_), .c(new_n30_), .d(x00), .O(new_n75_));
  nor3   g46(.a(new_n75_), .b(new_n32_), .c(new_n35_), .O(z12));
endmodule


