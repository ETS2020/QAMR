// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  nor2   g05(.a(new_n30_), .b(x11), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n34_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x09), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n31_), .O(z00));
  nand3  g10(.a(new_n34_), .b(x12), .c(x09), .O(z01));
  inv1   g11(.a(x09), .O(new_n41_));
  inv1   g12(.a(new_n32_), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(x08), .c(x07), .O(new_n43_));
  aoi21  g14(.a(new_n43_), .b(x11), .c(new_n35_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n41_), .c(x12), .O(z02));
  nand4  g16(.a(new_n42_), .b(x11), .c(x08), .d(x07), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g18(.a(new_n30_), .b(x09), .O(z12));
  nor2   g19(.a(z12), .b(x14), .O(z04));
  oai21  g20(.a(new_n30_), .b(x09), .c(x13), .O(z05));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n36_), .c(x04), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z06));
  nor2   g25(.a(z12), .b(x15), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  inv1   g27(.a(x11), .O(new_n57_));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  aoi21  g31(.a(new_n30_), .b(new_n57_), .c(new_n60_), .O(new_n61_));
  oai21  g32(.a(new_n57_), .b(new_n41_), .c(x12), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n56_), .d(x00), .O(z08));
  nand2  g34(.a(new_n30_), .b(new_n57_), .O(new_n64_));
  nand3  g35(.a(new_n58_), .b(new_n51_), .c(x04), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x12), .c(x11), .d(x09), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n64_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n56_), .c(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z09));
  inv1   g40(.a(x00), .O(new_n70_));
  nand4  g41(.a(new_n58_), .b(x12), .c(x11), .d(new_n56_), .O(new_n71_));
  nor3   g42(.a(new_n71_), .b(new_n41_), .c(new_n70_), .O(z10));
  nand4  g43(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n73_));
  oai21  g44(.a(x12), .b(x01), .c(new_n73_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x11), .c(new_n56_), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z11));
endmodule


