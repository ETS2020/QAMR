// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n43_, new_n45_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n78_;
  inv1   g00(.a(x11), .O(new_n30_));
  oai21  g01(.a(x12), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n32_), .b(x08), .c(x07), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x12), .c(x11), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n31_), .O(z00));
  oai21  g09(.a(x12), .b(x11), .c(new_n37_), .O(z01));
  nand3  g10(.a(new_n33_), .b(x08), .c(x07), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g13(.a(new_n33_), .b(x12), .c(x08), .d(x07), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g15(.a(x12), .b(new_n30_), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(new_n46_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(x12), .b(new_n30_), .O(new_n51_));
  oai21  g22(.a(new_n50_), .b(new_n45_), .c(new_n51_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z06));
  nor2   g25(.a(new_n45_), .b(x15), .O(z07));
  inv1   g26(.a(x00), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x03), .b(x02), .c(x00), .O(new_n58_));
  nand4  g29(.a(x11), .b(new_n57_), .c(x09), .d(x04), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n56_), .c(x12), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x11), .O(z08));
  inv1   g33(.a(x12), .O(new_n63_));
  oai21  g34(.a(x10), .b(new_n56_), .c(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g36(.a(x03), .b(x02), .O(new_n66_));
  nand2  g37(.a(new_n49_), .b(x04), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n66_), .c(x12), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(x09), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n57_), .d(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n65_), .O(z09));
  inv1   g42(.a(x09), .O(new_n72_));
  nand4  g43(.a(new_n49_), .b(x12), .c(x11), .d(new_n57_), .O(new_n73_));
  nor3   g44(.a(new_n73_), .b(new_n72_), .c(new_n56_), .O(z10));
  nor2   g45(.a(new_n50_), .b(x10), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x09), .c(x01), .d(x00), .O(new_n76_));
  aoi21  g47(.a(new_n76_), .b(x12), .c(new_n30_), .O(z11));
  nand3  g48(.a(new_n57_), .b(new_n72_), .c(x00), .O(new_n78_));
  aoi21  g49(.a(new_n78_), .b(x12), .c(new_n30_), .O(z12));
endmodule


