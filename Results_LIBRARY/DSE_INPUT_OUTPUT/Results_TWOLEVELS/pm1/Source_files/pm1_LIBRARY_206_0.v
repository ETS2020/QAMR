// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_;
  inv1   g00(.a(x12), .O(new_n30_));
  and2   g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x11), .c(x07), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(x01), .c(x08), .O(new_n34_));
  nand3  g05(.a(new_n31_), .b(x08), .c(x07), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n34_), .O(z00));
  nand3  g08(.a(new_n35_), .b(new_n31_), .c(x07), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x11), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(x12), .c(x08), .O(z01));
  nand3  g11(.a(x07), .b(x06), .c(x05), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(new_n42_));
  inv1   g13(.a(x11), .O(new_n43_));
  aoi21  g14(.a(new_n43_), .b(x08), .c(new_n30_), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(new_n42_), .c(x09), .d(x08), .O(z02));
  nand2  g16(.a(new_n32_), .b(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x08), .O(new_n47_));
  nor2   g18(.a(x11), .b(x08), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(new_n47_), .c(x12), .d(x09), .O(z03));
  nand2  g21(.a(new_n49_), .b(x14), .O(z04));
  nand2  g22(.a(new_n49_), .b(x13), .O(z05));
  nand3  g23(.a(x12), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x08), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n43_), .O(new_n55_));
  nand3  g26(.a(x04), .b(x03), .c(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x01), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n55_), .O(z06));
  nor2   g29(.a(new_n48_), .b(x15), .O(z07));
  nand2  g30(.a(new_n43_), .b(x08), .O(new_n60_));
  inv1   g31(.a(x00), .O(new_n61_));
  inv1   g32(.a(x10), .O(new_n62_));
  and2   g33(.a(x03), .b(x02), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(x00), .O(new_n64_));
  nand4  g35(.a(x12), .b(new_n62_), .c(x09), .d(x04), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n64_), .c(new_n62_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n61_), .c(x11), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n60_), .O(z08));
  nand4  g39(.a(new_n63_), .b(new_n56_), .c(x09), .d(x04), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  nand3  g41(.a(new_n30_), .b(new_n43_), .c(x08), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n62_), .c(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand4  g45(.a(new_n56_), .b(x12), .c(x11), .d(new_n62_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n49_), .O(z10));
  nand4  g49(.a(new_n56_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n62_), .d(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n49_), .O(z11));
  inv1   g53(.a(x09), .O(new_n83_));
  nand4  g54(.a(x11), .b(new_n62_), .c(new_n83_), .d(x00), .O(new_n84_));
  nor2   g55(.a(new_n84_), .b(new_n30_), .O(z12));
endmodule


