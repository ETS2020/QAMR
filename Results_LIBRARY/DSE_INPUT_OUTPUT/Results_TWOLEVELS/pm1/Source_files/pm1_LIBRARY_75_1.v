// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nor2   g04(.a(x12), .b(x11), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x12), .O(z01));
  nand2  g06(.a(x12), .b(new_n31_), .O(new_n36_));
  nand3  g07(.a(x12), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  and2   g09(.a(x06), .b(x05), .O(new_n39_));
  inv1   g10(.a(x09), .O(new_n40_));
  nand2  g11(.a(x11), .b(new_n40_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n39_), .c(new_n38_), .d(new_n36_), .O(z02));
  nand3  g13(.a(new_n39_), .b(x08), .c(x07), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x11), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(z03));
  nand2  g17(.a(new_n36_), .b(x14), .O(z04));
  aoi21  g18(.a(x12), .b(new_n31_), .c(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n36_), .O(z06));
  nand2  g22(.a(new_n36_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand2  g24(.a(x03), .b(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x09), .c(x04), .O(new_n55_));
  or2    g26(.a(new_n55_), .b(new_n54_), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  inv1   g28(.a(x00), .O(new_n58_));
  oai21  g29(.a(x10), .b(new_n58_), .c(new_n32_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n31_), .O(new_n60_));
  nand2  g31(.a(new_n49_), .b(x04), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n54_), .c(x09), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n41_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x12), .c(new_n53_), .d(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n60_), .O(z09));
  nand4  g36(.a(new_n49_), .b(new_n53_), .c(x09), .d(x00), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(x11), .c(new_n32_), .O(z10));
  nand4  g38(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  nand3  g39(.a(new_n32_), .b(x11), .c(new_n30_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n53_), .c(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n36_), .O(z11));
  nand4  g43(.a(x11), .b(new_n53_), .c(new_n40_), .d(x00), .O(new_n73_));
  nor2   g44(.a(new_n73_), .b(new_n32_), .O(z12));
endmodule


