// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n49_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x15), .b(x12), .c(new_n31_), .O(z01));
  inv1   g05(.a(x15), .O(z07));
  oai21  g06(.a(z07), .b(new_n31_), .c(x12), .O(new_n36_));
  nand3  g07(.a(x07), .b(x06), .c(x05), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  aoi21  g10(.a(x15), .b(new_n39_), .c(new_n32_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(new_n36_), .d(x08), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand2  g13(.a(x12), .b(new_n31_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n42_), .c(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x15), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(z03));
  nor2   g17(.a(x15), .b(new_n32_), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(x14), .O(z04));
  inv1   g19(.a(new_n47_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x13), .O(z05));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(x15), .b(x12), .c(new_n31_), .O(new_n53_));
  oai21  g24(.a(new_n52_), .b(new_n47_), .c(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(z06));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x12), .b(x11), .c(x09), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nor2   g30(.a(x12), .b(x11), .O(new_n60_));
  aoi21  g31(.a(new_n59_), .b(new_n52_), .c(new_n60_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n36_), .c(new_n57_), .d(x00), .O(z08));
  inv1   g33(.a(new_n60_), .O(new_n63_));
  nand2  g34(.a(x15), .b(new_n39_), .O(new_n64_));
  nand2  g35(.a(x03), .b(x02), .O(new_n65_));
  nand2  g36(.a(new_n51_), .b(x04), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n65_), .c(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n63_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n57_), .c(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n49_), .O(z09));
  nor2   g43(.a(new_n52_), .b(new_n31_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n57_), .c(x09), .d(x00), .O(new_n74_));
  aoi21  g45(.a(new_n74_), .b(x15), .c(new_n32_), .O(z10));
  nand4  g46(.a(new_n51_), .b(x12), .c(x09), .d(x01), .O(new_n76_));
  oai21  g47(.a(x12), .b(x01), .c(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n57_), .d(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n49_), .O(z11));
  nand4  g50(.a(x11), .b(new_n57_), .c(new_n39_), .d(x00), .O(new_n80_));
  nor3   g51(.a(new_n80_), .b(z07), .c(new_n32_), .O(z12));
endmodule


