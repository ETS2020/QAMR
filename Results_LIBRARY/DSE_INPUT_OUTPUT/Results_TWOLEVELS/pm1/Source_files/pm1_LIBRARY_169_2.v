// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n47_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_;
  inv1   g00(.a(x12), .O(new_n30_));
  inv1   g01(.a(x09), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x06), .b(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x08), .c(x07), .O(new_n35_));
  nor3   g06(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n30_), .c(new_n32_), .O(z00));
  nor2   g08(.a(new_n35_), .b(new_n34_), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n33_), .c(x12), .O(z01));
  nor2   g10(.a(new_n30_), .b(x11), .O(new_n40_));
  inv1   g11(.a(new_n34_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x08), .c(x07), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(new_n33_), .c(x12), .O(new_n43_));
  oai21  g14(.a(new_n40_), .b(x09), .c(new_n43_), .O(z02));
  nand4  g15(.a(new_n41_), .b(x11), .c(x08), .d(x07), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g17(.a(x12), .b(new_n31_), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(x14), .O(z04));
  inv1   g19(.a(new_n47_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x13), .O(z05));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x11), .c(x04), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(x12), .c(x09), .d(x01), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z06));
  nor2   g25(.a(new_n47_), .b(x15), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  nand2  g28(.a(x12), .b(x11), .O(new_n58_));
  nor3   g29(.a(new_n58_), .b(new_n57_), .c(new_n31_), .O(new_n59_));
  nor2   g30(.a(x12), .b(x11), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n31_), .c(new_n59_), .O(new_n61_));
  nor2   g32(.a(new_n47_), .b(new_n40_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n56_), .d(x00), .O(z08));
  inv1   g34(.a(new_n58_), .O(new_n64_));
  oai21  g35(.a(new_n60_), .b(new_n64_), .c(new_n31_), .O(new_n65_));
  nand3  g36(.a(new_n57_), .b(new_n51_), .c(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x11), .d(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n56_), .c(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z09));
  inv1   g41(.a(x00), .O(new_n71_));
  nand4  g42(.a(new_n57_), .b(x12), .c(x11), .d(new_n56_), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(new_n31_), .c(new_n71_), .O(z10));
  nand4  g44(.a(new_n57_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  nand2  g45(.a(new_n30_), .b(new_n31_), .O(new_n75_));
  oai21  g46(.a(new_n75_), .b(x01), .c(new_n74_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x11), .c(new_n56_), .d(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z11));
  nand2  g49(.a(new_n31_), .b(x00), .O(new_n79_));
  nand2  g50(.a(new_n64_), .b(new_n56_), .O(new_n80_));
  oai21  g51(.a(new_n80_), .b(new_n79_), .c(new_n49_), .O(z12));
endmodule


