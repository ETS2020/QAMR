// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  oai21  g04(.a(x12), .b(new_n30_), .c(new_n33_), .O(z00));
  nand3  g05(.a(new_n33_), .b(x12), .c(x11), .O(z01));
  inv1   g06(.a(new_n31_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g10(.a(x11), .O(new_n40_));
  nand2  g11(.a(x12), .b(new_n40_), .O(new_n41_));
  inv1   g12(.a(x09), .O(new_n42_));
  inv1   g13(.a(x12), .O(new_n43_));
  nor2   g14(.a(new_n43_), .b(new_n42_), .O(new_n44_));
  nand4  g15(.a(new_n36_), .b(x11), .c(x08), .d(x07), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n44_), .c(new_n41_), .O(z03));
  nand2  g17(.a(new_n41_), .b(x14), .O(z04));
  aoi21  g18(.a(x12), .b(new_n40_), .c(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n41_), .O(z06));
  nand2  g22(.a(new_n41_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand4  g24(.a(new_n44_), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  aoi21  g26(.a(new_n53_), .b(x00), .c(x12), .O(new_n56_));
  nand3  g27(.a(new_n49_), .b(new_n40_), .c(x09), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x12), .c(new_n53_), .d(x00), .O(new_n58_));
  oai21  g29(.a(new_n56_), .b(x11), .c(new_n58_), .O(z09));
  inv1   g30(.a(x00), .O(new_n60_));
  nand4  g31(.a(new_n49_), .b(x12), .c(x11), .d(new_n53_), .O(new_n61_));
  nor3   g32(.a(new_n61_), .b(new_n42_), .c(new_n60_), .O(z10));
  nand4  g33(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n63_));
  nand2  g34(.a(new_n43_), .b(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x11), .c(new_n53_), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z11));
  nand3  g38(.a(new_n53_), .b(new_n42_), .c(x00), .O(new_n68_));
  aoi21  g39(.a(new_n68_), .b(x11), .c(new_n43_), .O(z12));
endmodule


