// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n42_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n33_), .O(new_n34_));
  inv1   g05(.a(x01), .O(new_n35_));
  nor2   g06(.a(x12), .b(new_n35_), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n34_), .c(new_n32_), .O(z00));
  nand2  g09(.a(new_n32_), .b(x12), .O(z01));
  inv1   g10(.a(new_n30_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x08), .c(x07), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g14(.a(new_n40_), .b(x11), .c(x08), .d(x07), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g16(.a(new_n37_), .b(x14), .O(z04));
  nor2   g17(.a(new_n36_), .b(x13), .O(z05));
  and2   g18(.a(x03), .b(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x11), .c(x04), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x09), .O(new_n50_));
  aoi21  g21(.a(new_n50_), .b(x12), .c(new_n35_), .O(z06));
  nand2  g22(.a(new_n37_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x11), .c(x09), .O(new_n55_));
  inv1   g26(.a(x12), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n33_), .c(new_n35_), .O(new_n57_));
  oai21  g28(.a(new_n55_), .b(new_n54_), .c(new_n57_), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n53_), .O(new_n59_));
  nor2   g30(.a(new_n36_), .b(x10), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n59_), .c(new_n34_), .d(x00), .O(z08));
  nand4  g32(.a(new_n54_), .b(new_n48_), .c(x09), .d(x04), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n57_), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n53_), .c(x00), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z09));
  nand4  g37(.a(new_n54_), .b(x12), .c(x11), .d(new_n53_), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x09), .c(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n37_), .O(z10));
  nand4  g41(.a(new_n54_), .b(x12), .c(x09), .d(x01), .O(new_n71_));
  oai21  g42(.a(x12), .b(x01), .c(new_n71_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n53_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z11));
  inv1   g45(.a(x09), .O(new_n75_));
  nand4  g46(.a(x11), .b(new_n53_), .c(new_n75_), .d(x00), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n56_), .O(z12));
endmodule


