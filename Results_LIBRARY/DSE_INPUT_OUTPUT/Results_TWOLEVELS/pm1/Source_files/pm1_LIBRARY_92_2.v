// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n42_, new_n43_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand4  g01(.a(x11), .b(x07), .c(x06), .d(x05), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(x15), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x08), .O(new_n33_));
  nand4  g04(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x11), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z00));
  nand3  g09(.a(new_n35_), .b(new_n33_), .c(x12), .O(z01));
  inv1   g10(.a(x09), .O(new_n40_));
  aoi21  g11(.a(x12), .b(new_n36_), .c(new_n40_), .O(new_n41_));
  inv1   g12(.a(x15), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x08), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(new_n35_), .d(x12), .O(z02));
  nand4  g15(.a(new_n33_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g16(.a(new_n43_), .b(x14), .O(z04));
  nand2  g17(.a(new_n43_), .b(x13), .O(z05));
  nand4  g18(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(new_n43_), .c(x09), .d(x01), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(z06));
  nor2   g21(.a(x15), .b(x08), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  nand3  g24(.a(x12), .b(x11), .c(x09), .O(new_n54_));
  inv1   g25(.a(x12), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n36_), .O(new_n56_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n52_), .O(new_n58_));
  aoi21  g29(.a(new_n42_), .b(x08), .c(x10), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n58_), .c(new_n37_), .d(x00), .O(z08));
  nand4  g31(.a(new_n53_), .b(new_n42_), .c(x09), .d(x08), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x12), .c(x11), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n56_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n52_), .c(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n43_), .O(z09));
  nand4  g36(.a(new_n53_), .b(new_n43_), .c(x12), .d(x11), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n52_), .c(x09), .d(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z10));
  nand4  g40(.a(new_n53_), .b(x12), .c(x09), .d(x01), .O(new_n70_));
  oai21  g41(.a(x12), .b(x01), .c(new_n70_), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(x11), .c(new_n52_), .d(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n43_), .O(z11));
  nand2  g44(.a(new_n40_), .b(x00), .O(new_n74_));
  nand3  g45(.a(x12), .b(x11), .c(new_n52_), .O(new_n75_));
  oai21  g46(.a(new_n75_), .b(new_n74_), .c(new_n43_), .O(z12));
endmodule


