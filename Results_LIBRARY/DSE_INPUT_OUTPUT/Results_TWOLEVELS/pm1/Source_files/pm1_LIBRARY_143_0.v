// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(x13), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g04(.a(x12), .b(new_n31_), .O(z01));
  nand3  g05(.a(x07), .b(x06), .c(x05), .O(new_n35_));
  nand3  g06(.a(x11), .b(x09), .c(x08), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x12), .O(new_n37_));
  nand2  g08(.a(x13), .b(new_n32_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x11), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n38_), .O(z03));
  nor2   g14(.a(x13), .b(x12), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(x14), .O(z04));
  nor2   g16(.a(x13), .b(new_n32_), .O(z05));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  inv1   g18(.a(new_n47_), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n44_), .c(z01), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(z06));
  oai21  g22(.a(x13), .b(x12), .c(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x12), .b(x11), .c(x09), .O(new_n54_));
  oai22  g25(.a(new_n54_), .b(new_n47_), .c(x12), .d(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(new_n56_));
  nor2   g27(.a(new_n44_), .b(x10), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n56_), .c(z01), .d(x00), .O(z08));
  inv1   g29(.a(x00), .O(new_n59_));
  nand3  g30(.a(new_n31_), .b(new_n53_), .c(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x13), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n32_), .O(new_n62_));
  and2   g33(.a(x03), .b(x02), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n47_), .c(x09), .d(x04), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(x12), .c(x11), .d(new_n53_), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n59_), .c(new_n62_), .O(z09));
  inv1   g37(.a(x09), .O(new_n67_));
  nand4  g38(.a(new_n47_), .b(x12), .c(x11), .d(new_n53_), .O(new_n68_));
  nor3   g39(.a(new_n68_), .b(new_n67_), .c(new_n59_), .O(z10));
  nand4  g40(.a(x11), .b(new_n53_), .c(new_n30_), .d(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(x13), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n32_), .O(new_n72_));
  inv1   g43(.a(new_n68_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x09), .c(x01), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n72_), .O(z11));
  nand4  g46(.a(x11), .b(new_n53_), .c(new_n67_), .d(x00), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n32_), .O(z12));
endmodule


