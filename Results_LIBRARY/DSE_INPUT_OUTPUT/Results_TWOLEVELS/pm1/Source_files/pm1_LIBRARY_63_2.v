// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n34_), .O(new_n35_));
  inv1   g06(.a(x15), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x13), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z00));
  inv1   g09(.a(x12), .O(new_n39_));
  inv1   g10(.a(new_n37_), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n33_), .O(z01));
  nand2  g13(.a(x08), .b(x07), .O(new_n43_));
  oai21  g14(.a(new_n43_), .b(new_n31_), .c(x11), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(new_n41_), .c(new_n35_), .d(x09), .O(z02));
  inv1   g16(.a(x09), .O(new_n46_));
  nor2   g17(.a(new_n39_), .b(new_n46_), .O(new_n47_));
  nand3  g18(.a(x11), .b(x08), .c(x07), .O(new_n48_));
  or2    g19(.a(new_n48_), .b(new_n31_), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(new_n47_), .c(new_n37_), .d(new_n35_), .O(z03));
  nand2  g21(.a(new_n37_), .b(x14), .O(z04));
  nand2  g22(.a(x15), .b(x13), .O(z05));
  and2   g23(.a(x03), .b(x02), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(new_n35_), .c(x04), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n37_), .O(z06));
  nor2   g27(.a(x15), .b(x13), .O(z07));
  inv1   g28(.a(x00), .O(new_n58_));
  nor2   g29(.a(x10), .b(new_n58_), .O(new_n59_));
  nand4  g30(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n60_));
  inv1   g31(.a(x10), .O(new_n61_));
  nand4  g32(.a(x12), .b(x11), .c(new_n61_), .d(x09), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n34_), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(new_n59_), .c(new_n40_), .O(z08));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(new_n53_), .c(x09), .d(x04), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g39(.a(new_n39_), .b(new_n34_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n37_), .c(new_n61_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  nand4  g43(.a(new_n66_), .b(new_n37_), .c(x12), .d(x11), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(new_n61_), .c(x09), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z10));
  nand4  g47(.a(new_n66_), .b(x12), .c(x09), .d(x01), .O(new_n77_));
  oai21  g48(.a(x12), .b(x01), .c(new_n77_), .O(new_n78_));
  nand4  g49(.a(new_n78_), .b(x11), .c(new_n61_), .d(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n37_), .O(z11));
  nand4  g51(.a(new_n41_), .b(x11), .c(new_n61_), .d(new_n46_), .O(new_n81_));
  nor2   g52(.a(new_n81_), .b(new_n58_), .O(z12));
endmodule


