// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x15), .O(z07));
  and2   g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(x08), .b(x06), .c(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n32_), .c(x08), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(x07), .c(z07), .O(new_n35_));
  inv1   g06(.a(x01), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n30_), .O(new_n37_));
  inv1   g08(.a(x07), .O(new_n38_));
  nand2  g09(.a(x15), .b(new_n38_), .O(new_n39_));
  nand3  g10(.a(new_n39_), .b(new_n37_), .c(new_n36_), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  oai21  g12(.a(new_n35_), .b(new_n30_), .c(new_n41_), .O(z00));
  inv1   g13(.a(x12), .O(new_n43_));
  nand2  g14(.a(new_n39_), .b(new_n43_), .O(new_n44_));
  oai21  g15(.a(new_n35_), .b(new_n30_), .c(new_n44_), .O(z01));
  nand3  g16(.a(x12), .b(x11), .c(x09), .O(new_n46_));
  aoi21  g17(.a(new_n33_), .b(x11), .c(new_n46_), .O(new_n47_));
  nand2  g18(.a(z07), .b(new_n38_), .O(new_n48_));
  oai21  g19(.a(new_n47_), .b(new_n38_), .c(new_n48_), .O(z02));
  nand4  g20(.a(x11), .b(x08), .c(x06), .d(x05), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x12), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x07), .O(new_n52_));
  inv1   g23(.a(new_n39_), .O(new_n53_));
  aoi21  g24(.a(z07), .b(new_n43_), .c(new_n53_), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(new_n52_), .c(new_n37_), .d(x09), .O(z03));
  nor2   g26(.a(new_n53_), .b(x14), .O(z04));
  nor2   g27(.a(new_n53_), .b(x13), .O(z05));
  nand4  g28(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x09), .c(x01), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n39_), .O(z06));
  inv1   g31(.a(x10), .O(new_n61_));
  nand4  g32(.a(new_n39_), .b(new_n43_), .c(new_n61_), .d(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n43_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n30_), .O(new_n64_));
  nand2  g35(.a(new_n61_), .b(x00), .O(new_n65_));
  nand2  g36(.a(x03), .b(x02), .O(new_n66_));
  nand3  g37(.a(x12), .b(x09), .c(x04), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n66_), .c(new_n39_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n65_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n64_), .O(z08));
  xnor2a g41(.a(x12), .b(x11), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n39_), .c(new_n61_), .d(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z09));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x12), .c(x11), .d(new_n61_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n39_), .O(z10));
  nand4  g49(.a(new_n74_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  nand2  g50(.a(new_n43_), .b(new_n36_), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x11), .c(new_n61_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n39_), .O(z11));
  inv1   g54(.a(x09), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(x00), .O(new_n85_));
  nand3  g56(.a(x12), .b(x11), .c(new_n61_), .O(new_n86_));
  oai21  g57(.a(new_n86_), .b(new_n85_), .c(new_n39_), .O(z12));
endmodule


