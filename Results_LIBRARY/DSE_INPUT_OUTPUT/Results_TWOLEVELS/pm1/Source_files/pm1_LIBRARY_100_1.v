// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x06), .O(new_n31_));
  nand2  g02(.a(x15), .b(new_n31_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  inv1   g04(.a(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  and2   g07(.a(x07), .b(x05), .O(new_n37_));
  nand3  g08(.a(x08), .b(x07), .c(x05), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n37_), .c(x08), .O(new_n39_));
  nor2   g10(.a(x15), .b(x06), .O(new_n40_));
  aoi21  g11(.a(new_n39_), .b(x06), .c(new_n40_), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n30_), .c(new_n36_), .O(z00));
  oai21  g13(.a(x15), .b(x11), .c(new_n31_), .O(new_n43_));
  nand3  g14(.a(new_n39_), .b(x11), .c(x06), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n43_), .c(x12), .O(z01));
  nand3  g16(.a(new_n38_), .b(x11), .c(x06), .O(new_n46_));
  nor2   g17(.a(new_n34_), .b(new_n30_), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n46_), .c(new_n43_), .d(x09), .O(z02));
  nand3  g19(.a(x11), .b(x08), .c(x07), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x06), .c(x05), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(new_n47_), .c(new_n32_), .d(x09), .O(z03));
  nand2  g23(.a(new_n32_), .b(x14), .O(z04));
  nand2  g24(.a(new_n32_), .b(x13), .O(z05));
  nand2  g25(.a(x12), .b(new_n30_), .O(new_n55_));
  nand4  g26(.a(new_n55_), .b(x04), .c(x03), .d(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x01), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n32_), .O(z06));
  inv1   g29(.a(x15), .O(z07));
  nor2   g30(.a(new_n30_), .b(x10), .O(new_n60_));
  inv1   g31(.a(x10), .O(new_n61_));
  nand4  g32(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n47_), .c(new_n61_), .d(x09), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n60_), .c(new_n32_), .d(x00), .O(z08));
  nand4  g36(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n32_), .O(new_n67_));
  nand4  g38(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  nand2  g41(.a(new_n34_), .b(new_n30_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n70_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n61_), .c(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n32_), .O(z09));
  nand3  g45(.a(x04), .b(x03), .c(x02), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(new_n32_), .c(x12), .d(x11), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(new_n61_), .c(x09), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z10));
  inv1   g50(.a(x00), .O(new_n80_));
  nand4  g51(.a(new_n75_), .b(x12), .c(x09), .d(x01), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n35_), .O(new_n82_));
  nand4  g53(.a(new_n82_), .b(new_n32_), .c(x11), .d(new_n61_), .O(new_n83_));
  nor2   g54(.a(new_n83_), .b(new_n80_), .O(z11));
  nand4  g55(.a(new_n32_), .b(x12), .c(x11), .d(new_n61_), .O(new_n85_));
  nor3   g56(.a(new_n85_), .b(x09), .c(new_n80_), .O(z12));
endmodule


