// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  nor2   g05(.a(x13), .b(x11), .O(new_n35_));
  inv1   g06(.a(new_n35_), .O(new_n36_));
  nand4  g07(.a(new_n36_), .b(new_n34_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g08(.a(x13), .b(new_n31_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n34_), .O(z01));
  nand3  g10(.a(x07), .b(x06), .c(x05), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand2  g12(.a(x12), .b(x09), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(x11), .d(x08), .O(z02));
  nand2  g15(.a(x08), .b(x07), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n32_), .c(new_n43_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x11), .O(new_n47_));
  inv1   g18(.a(x11), .O(new_n48_));
  nand2  g19(.a(x13), .b(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n47_), .O(z03));
  nor2   g21(.a(new_n35_), .b(x14), .O(z04));
  nor2   g22(.a(x13), .b(new_n48_), .O(z05));
  oai21  g23(.a(new_n42_), .b(new_n30_), .c(x13), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand3  g25(.a(x04), .b(x03), .c(x02), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n54_), .O(z06));
  nor2   g28(.a(new_n35_), .b(x15), .O(z07));
  inv1   g29(.a(x00), .O(new_n59_));
  inv1   g30(.a(x10), .O(new_n60_));
  and2   g31(.a(x03), .b(x02), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x00), .O(new_n62_));
  nand4  g33(.a(x12), .b(new_n60_), .c(x09), .d(x04), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n62_), .c(new_n60_), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n59_), .c(x11), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n49_), .O(z08));
  nand4  g37(.a(new_n61_), .b(new_n55_), .c(x09), .d(x04), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand3  g39(.a(x13), .b(new_n31_), .c(new_n48_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n60_), .c(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  inv1   g43(.a(x09), .O(new_n73_));
  nand4  g44(.a(new_n55_), .b(x12), .c(x11), .d(new_n60_), .O(new_n74_));
  nor3   g45(.a(new_n74_), .b(new_n73_), .c(new_n59_), .O(z10));
  nand4  g46(.a(new_n55_), .b(x12), .c(x09), .d(x01), .O(new_n76_));
  oai21  g47(.a(x12), .b(x01), .c(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n60_), .d(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n36_), .O(z11));
  nand4  g50(.a(x11), .b(new_n60_), .c(new_n73_), .d(x00), .O(new_n80_));
  nor2   g51(.a(new_n80_), .b(new_n31_), .O(z12));
endmodule


