// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  nand3  g02(.a(new_n30_), .b(x08), .c(x07), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(x11), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  aoi21  g06(.a(new_n33_), .b(new_n31_), .c(new_n35_), .O(z01));
  nand3  g07(.a(new_n31_), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g10(.a(new_n31_), .b(x11), .c(x08), .d(x07), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g12(.a(x12), .b(x11), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x14), .O(z04));
  inv1   g14(.a(new_n42_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x13), .O(z05));
  inv1   g16(.a(x12), .O(new_n46_));
  aoi21  g17(.a(x09), .b(x01), .c(new_n46_), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x09), .c(x01), .O(new_n49_));
  oai21  g20(.a(new_n47_), .b(x11), .c(new_n49_), .O(z06));
  nor2   g21(.a(new_n42_), .b(x15), .O(z07));
  nand3  g22(.a(x03), .b(x02), .c(x00), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand4  g24(.a(x11), .b(new_n53_), .c(x09), .d(x04), .O(new_n54_));
  oai21  g25(.a(new_n54_), .b(new_n52_), .c(x11), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x12), .O(new_n56_));
  inv1   g27(.a(x00), .O(new_n57_));
  oai21  g28(.a(x10), .b(new_n57_), .c(x11), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n56_), .O(z08));
  and2   g30(.a(x03), .b(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n48_), .c(x09), .d(x04), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x12), .c(x11), .d(new_n53_), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n57_), .O(z09));
  inv1   g34(.a(x09), .O(new_n64_));
  nand4  g35(.a(new_n48_), .b(x12), .c(x11), .d(new_n53_), .O(new_n65_));
  nor3   g36(.a(new_n65_), .b(new_n64_), .c(new_n57_), .O(z10));
  nand4  g37(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  inv1   g38(.a(x01), .O(new_n68_));
  nand2  g39(.a(new_n46_), .b(new_n68_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x11), .c(new_n53_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z11));
  nand2  g43(.a(new_n64_), .b(x00), .O(new_n73_));
  nand3  g44(.a(x12), .b(x11), .c(new_n53_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n73_), .c(new_n44_), .O(z12));
endmodule


