// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n56_, new_n58_, new_n59_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x14), .O(new_n31_));
  inv1   g02(.a(x15), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(x11), .c(new_n32_), .O(new_n33_));
  and2   g04(.a(x06), .b(x05), .O(new_n34_));
  nand4  g05(.a(new_n34_), .b(new_n32_), .c(x08), .d(x07), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  nand2  g08(.a(x12), .b(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n36_), .c(new_n33_), .d(new_n30_), .O(z00));
  inv1   g10(.a(x12), .O(new_n40_));
  nand2  g11(.a(new_n32_), .b(new_n31_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(new_n36_), .c(new_n33_), .O(z01));
  inv1   g14(.a(x09), .O(new_n44_));
  oai21  g15(.a(new_n40_), .b(new_n44_), .c(new_n41_), .O(new_n45_));
  nand3  g16(.a(new_n34_), .b(x08), .c(x07), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x11), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n45_), .c(new_n41_), .d(new_n38_), .O(z02));
  inv1   g19(.a(x08), .O(new_n49_));
  nand2  g20(.a(x15), .b(x11), .O(new_n50_));
  aoi21  g21(.a(new_n50_), .b(new_n31_), .c(new_n49_), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(x07), .c(x06), .d(x05), .O(new_n52_));
  oai21  g23(.a(x15), .b(x14), .c(new_n37_), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(new_n52_), .c(new_n45_), .O(z03));
  nor2   g25(.a(new_n32_), .b(x14), .O(z04));
  inv1   g26(.a(new_n41_), .O(new_n56_));
  nor2   g27(.a(new_n56_), .b(x13), .O(z05));
  nand4  g28(.a(new_n38_), .b(x04), .c(x03), .d(x02), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x09), .c(x01), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n41_), .O(z06));
  nor2   g31(.a(new_n31_), .b(new_n37_), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n37_), .c(x15), .O(z07));
  inv1   g33(.a(x00), .O(new_n63_));
  nor2   g34(.a(x10), .b(new_n63_), .O(new_n64_));
  inv1   g35(.a(x10), .O(new_n65_));
  nand4  g36(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nor2   g38(.a(new_n40_), .b(new_n37_), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n67_), .c(new_n65_), .d(x09), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n64_), .c(new_n53_), .d(new_n41_), .O(z08));
  nand4  g41(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n41_), .O(new_n72_));
  nand4  g43(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x12), .c(x11), .O(new_n75_));
  nand2  g46(.a(new_n40_), .b(new_n37_), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g48(.a(new_n77_), .b(new_n65_), .c(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n41_), .O(z09));
  nand3  g50(.a(x04), .b(x03), .c(x02), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(new_n41_), .c(x12), .d(x11), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(new_n82_));
  nand4  g53(.a(new_n82_), .b(new_n65_), .c(x09), .d(x00), .O(new_n83_));
  inv1   g54(.a(new_n83_), .O(z10));
  nand4  g55(.a(new_n80_), .b(x12), .c(x09), .d(x01), .O(new_n85_));
  oai21  g56(.a(x12), .b(x01), .c(new_n85_), .O(new_n86_));
  nand4  g57(.a(new_n86_), .b(x11), .c(new_n65_), .d(x00), .O(new_n87_));
  nand2  g58(.a(new_n87_), .b(new_n41_), .O(z11));
  nor2   g59(.a(new_n56_), .b(new_n40_), .O(new_n89_));
  nand4  g60(.a(new_n89_), .b(x11), .c(new_n65_), .d(new_n44_), .O(new_n90_));
  nor2   g61(.a(new_n90_), .b(new_n63_), .O(z12));
endmodule


