// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n79_;
  inv1   g00(.a(x01), .O(new_n30_));
  and2   g01(.a(x06), .b(x05), .O(new_n31_));
  nand4  g02(.a(new_n31_), .b(x11), .c(x08), .d(x07), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x11), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x12), .O(new_n34_));
  nand4  g05(.a(new_n31_), .b(x12), .c(x08), .d(x07), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x11), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n34_), .c(new_n30_), .O(z00));
  inv1   g08(.a(x12), .O(new_n38_));
  nand4  g09(.a(new_n31_), .b(new_n38_), .c(x08), .d(x07), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x11), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z01));
  inv1   g12(.a(x09), .O(new_n42_));
  nand2  g13(.a(x12), .b(new_n42_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n36_), .c(x11), .O(z02));
  nand2  g15(.a(new_n32_), .b(x09), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x11), .O(z03));
  inv1   g18(.a(x11), .O(new_n48_));
  nor2   g19(.a(x12), .b(new_n48_), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x14), .O(z04));
  nor2   g22(.a(new_n49_), .b(x13), .O(z05));
  nand2  g23(.a(x03), .b(x02), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand2  g25(.a(x12), .b(new_n48_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n54_), .c(x04), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x01), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n50_), .O(z06));
  nand2  g29(.a(new_n50_), .b(x15), .O(z07));
  nor2   g30(.a(x10), .b(new_n42_), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n54_), .c(x04), .d(x00), .O(new_n61_));
  inv1   g32(.a(x00), .O(new_n62_));
  nor2   g33(.a(x10), .b(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n61_), .c(x12), .d(x11), .O(z08));
  oai21  g35(.a(new_n63_), .b(x11), .c(new_n38_), .O(new_n65_));
  inv1   g36(.a(x10), .O(new_n66_));
  nand3  g37(.a(x04), .b(x03), .c(x02), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x04), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n53_), .c(x09), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n43_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x11), .c(new_n66_), .d(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n65_), .O(z09));
  nand3  g43(.a(new_n67_), .b(new_n66_), .c(x09), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(x00), .O(new_n75_));
  aoi21  g46(.a(new_n75_), .b(x12), .c(new_n48_), .O(z10));
  nand3  g47(.a(new_n74_), .b(x01), .c(x00), .O(new_n77_));
  aoi21  g48(.a(new_n77_), .b(x12), .c(new_n48_), .O(z11));
  nand4  g49(.a(x11), .b(new_n66_), .c(new_n42_), .d(x00), .O(new_n79_));
  nor2   g50(.a(new_n79_), .b(new_n38_), .O(z12));
endmodule


