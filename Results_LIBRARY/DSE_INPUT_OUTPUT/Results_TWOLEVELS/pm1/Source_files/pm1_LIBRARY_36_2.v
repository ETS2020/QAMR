// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n52_, new_n53_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n69_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x10), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x03), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z00));
  nand3  g09(.a(new_n35_), .b(new_n33_), .c(x12), .O(z01));
  nand2  g10(.a(x08), .b(x07), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n31_), .c(x11), .O(new_n41_));
  inv1   g12(.a(x12), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(new_n36_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(new_n35_), .d(x09), .O(z02));
  inv1   g15(.a(x09), .O(new_n45_));
  nor2   g16(.a(new_n42_), .b(new_n45_), .O(new_n46_));
  nand3  g17(.a(x11), .b(x08), .c(x07), .O(new_n47_));
  or2    g18(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(new_n46_), .c(new_n37_), .d(new_n35_), .O(z03));
  nand2  g20(.a(new_n35_), .b(x14), .O(z04));
  nand2  g21(.a(new_n35_), .b(x13), .O(z05));
  nand4  g22(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n35_), .O(z06));
  nand2  g25(.a(new_n35_), .b(x15), .O(z07));
  inv1   g26(.a(x03), .O(new_n56_));
  nand4  g27(.a(x11), .b(new_n34_), .c(new_n56_), .d(x00), .O(z08));
  xnor2a g28(.a(x12), .b(x11), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n34_), .c(new_n56_), .d(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z09));
  inv1   g31(.a(x00), .O(new_n61_));
  nand3  g32(.a(x12), .b(x11), .c(x09), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n56_), .O(new_n63_));
  and2   g34(.a(new_n63_), .b(new_n34_), .O(z10));
  nand3  g35(.a(x12), .b(x09), .c(x01), .O(new_n65_));
  oai21  g36(.a(x12), .b(x01), .c(new_n65_), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x11), .c(new_n34_), .d(new_n56_), .O(new_n67_));
  nor2   g38(.a(new_n67_), .b(new_n61_), .O(z11));
  nand4  g39(.a(new_n34_), .b(new_n45_), .c(new_n56_), .d(x00), .O(new_n69_));
  nor3   g40(.a(new_n69_), .b(new_n42_), .c(new_n36_), .O(z12));
endmodule


