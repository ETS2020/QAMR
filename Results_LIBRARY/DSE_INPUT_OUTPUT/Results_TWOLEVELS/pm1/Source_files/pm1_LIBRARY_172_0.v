// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_;
  inv1   g00(.a(x15), .O(z07));
  inv1   g01(.a(x11), .O(new_n31_));
  aoi21  g02(.a(x12), .b(new_n31_), .c(x01), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  oai21  g06(.a(new_n32_), .b(z07), .c(new_n35_), .O(z00));
  nor2   g07(.a(x15), .b(x11), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n35_), .c(x12), .O(z01));
  oai21  g10(.a(z07), .b(x12), .c(new_n31_), .O(new_n40_));
  nand2  g11(.a(x08), .b(x07), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n33_), .c(x11), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n40_), .c(x12), .d(x09), .O(z02));
  nand3  g14(.a(x11), .b(x08), .c(x07), .O(new_n44_));
  or2    g15(.a(new_n44_), .b(new_n33_), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(new_n40_), .c(x12), .d(x09), .O(z03));
  nor2   g17(.a(new_n37_), .b(x14), .O(z04));
  nand2  g18(.a(new_n38_), .b(x13), .O(z05));
  nand3  g19(.a(x12), .b(x09), .c(x01), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x15), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n31_), .O(new_n51_));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n51_), .O(z06));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  nor2   g27(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  inv1   g28(.a(x12), .O(new_n58_));
  nand3  g29(.a(x15), .b(new_n58_), .c(new_n31_), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n57_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n40_), .c(new_n55_), .d(x00), .O(z08));
  and2   g33(.a(x03), .b(x02), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n52_), .c(x09), .d(x04), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(x12), .c(x11), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n59_), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(new_n55_), .c(x00), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(z09));
  inv1   g39(.a(x00), .O(new_n69_));
  inv1   g40(.a(x09), .O(new_n70_));
  nand4  g41(.a(new_n52_), .b(x12), .c(x11), .d(new_n55_), .O(new_n71_));
  nor3   g42(.a(new_n71_), .b(new_n70_), .c(new_n69_), .O(z10));
  nand4  g43(.a(new_n52_), .b(x12), .c(x09), .d(x01), .O(new_n73_));
  oai21  g44(.a(x12), .b(x01), .c(new_n73_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x11), .c(new_n55_), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z11));
  nand2  g47(.a(new_n70_), .b(x00), .O(new_n77_));
  nand3  g48(.a(x12), .b(x11), .c(new_n55_), .O(new_n78_));
  oai21  g49(.a(new_n78_), .b(new_n77_), .c(new_n38_), .O(z12));
endmodule


