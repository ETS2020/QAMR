// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_;
  oai21  g00(.a(x14), .b(x00), .c(x11), .O(new_n30_));
  nor2   g01(.a(x14), .b(x00), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  nor2   g03(.a(x12), .b(x01), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n32_), .c(new_n30_), .O(z00));
  inv1   g05(.a(x00), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  inv1   g07(.a(x14), .O(z04));
  aoi22  g08(.a(z04), .b(new_n35_), .c(x12), .d(new_n36_), .O(z01));
  inv1   g09(.a(x09), .O(new_n39_));
  inv1   g10(.a(x12), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(new_n32_), .O(new_n41_));
  oai21  g12(.a(x14), .b(x00), .c(new_n36_), .O(new_n42_));
  inv1   g13(.a(x05), .O(new_n43_));
  nand3  g14(.a(x08), .b(x07), .c(x06), .O(new_n44_));
  nor3   g15(.a(new_n44_), .b(new_n31_), .c(new_n43_), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n42_), .c(new_n41_), .O(z02));
  nand4  g17(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x12), .c(x09), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n32_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n42_), .O(z03));
  nand2  g21(.a(new_n32_), .b(x13), .O(z05));
  nand2  g22(.a(x12), .b(new_n36_), .O(new_n52_));
  and2   g23(.a(x03), .b(x02), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(new_n52_), .c(x04), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n32_), .O(z06));
  nor2   g27(.a(new_n31_), .b(x15), .O(z07));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(x04), .b(x03), .c(x02), .O(new_n59_));
  nand4  g30(.a(x12), .b(x11), .c(new_n58_), .d(x09), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(x00), .O(new_n62_));
  nand2  g33(.a(x14), .b(new_n35_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n62_), .c(new_n42_), .O(z08));
  nand4  g35(.a(new_n59_), .b(new_n53_), .c(x09), .d(x04), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x12), .c(x11), .O(new_n66_));
  nand2  g37(.a(new_n40_), .b(new_n36_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n58_), .c(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n32_), .O(z09));
  nand4  g41(.a(new_n59_), .b(x12), .c(x11), .d(new_n58_), .O(new_n71_));
  nor3   g42(.a(new_n71_), .b(new_n39_), .c(new_n35_), .O(z10));
  nand4  g43(.a(new_n59_), .b(x12), .c(x09), .d(x01), .O(new_n73_));
  oai21  g44(.a(x12), .b(x01), .c(new_n73_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x11), .c(new_n58_), .d(x00), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n32_), .O(z11));
  nand4  g47(.a(x11), .b(new_n58_), .c(new_n39_), .d(x00), .O(new_n77_));
  nor2   g48(.a(new_n77_), .b(new_n40_), .O(z12));
endmodule


