// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n60_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n86_;
  inv1   g00(.a(x15), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x09), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x11), .O(new_n35_));
  inv1   g06(.a(x01), .O(new_n36_));
  inv1   g07(.a(x11), .O(new_n37_));
  nand2  g08(.a(x12), .b(new_n37_), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nor2   g10(.a(new_n30_), .b(new_n39_), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n38_), .c(new_n36_), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n35_), .O(z00));
  nand2  g15(.a(new_n38_), .b(new_n39_), .O(new_n45_));
  oai21  g16(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(new_n30_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g20(.a(x12), .O(new_n50_));
  nand2  g21(.a(x08), .b(x07), .O(new_n51_));
  nor4   g22(.a(new_n51_), .b(new_n31_), .c(new_n50_), .d(new_n37_), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(x15), .c(x09), .O(z02));
  nand3  g24(.a(x11), .b(x08), .c(x07), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(new_n31_), .O(new_n55_));
  nor2   g26(.a(new_n55_), .b(new_n50_), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n38_), .c(new_n30_), .d(x09), .O(z03));
  nor2   g28(.a(new_n40_), .b(x14), .O(z04));
  nor2   g29(.a(new_n40_), .b(x13), .O(z05));
  nand4  g30(.a(new_n38_), .b(x04), .c(x03), .d(x02), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x01), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n30_), .c(new_n39_), .O(z06));
  aoi21  g33(.a(new_n46_), .b(x11), .c(x15), .O(z07));
  inv1   g34(.a(x10), .O(new_n64_));
  nand3  g35(.a(x03), .b(x02), .c(x00), .O(new_n65_));
  nand4  g36(.a(x12), .b(x11), .c(new_n64_), .d(x04), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n65_), .c(new_n30_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x09), .O(new_n68_));
  nand2  g39(.a(new_n41_), .b(new_n37_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n68_), .c(new_n64_), .d(x00), .O(z08));
  nand3  g41(.a(x04), .b(x03), .c(x02), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(x15), .c(x09), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x12), .c(x11), .O(new_n73_));
  nand2  g44(.a(new_n50_), .b(new_n37_), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n73_), .O(new_n75_));
  nand3  g46(.a(new_n75_), .b(new_n64_), .c(x00), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n41_), .O(z09));
  nand4  g48(.a(new_n71_), .b(new_n30_), .c(x12), .d(x11), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(new_n64_), .c(x09), .d(x00), .O(new_n80_));
  inv1   g51(.a(new_n80_), .O(z10));
  nand4  g52(.a(new_n71_), .b(x12), .c(x09), .d(x01), .O(new_n82_));
  oai21  g53(.a(x12), .b(x01), .c(new_n82_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(x11), .c(new_n64_), .d(x00), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n41_), .O(z11));
  nand4  g56(.a(x11), .b(new_n64_), .c(new_n39_), .d(x00), .O(new_n86_));
  nor2   g57(.a(new_n86_), .b(new_n50_), .O(z12));
endmodule


