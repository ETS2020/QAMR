// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x00), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  nor3   g05(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n30_), .c(new_n31_), .O(z00));
  nor2   g07(.a(new_n34_), .b(new_n33_), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n32_), .c(x12), .O(z01));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g12(.a(x00), .O(new_n42_));
  inv1   g13(.a(x09), .O(new_n43_));
  nand3  g14(.a(x11), .b(x08), .c(x07), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n33_), .c(x11), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n43_), .c(x12), .O(new_n46_));
  oai21  g17(.a(x12), .b(new_n42_), .c(new_n46_), .O(z03));
  nor2   g18(.a(x12), .b(x00), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(x14), .O(z04));
  inv1   g20(.a(new_n48_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x13), .O(z05));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand2  g24(.a(x12), .b(new_n32_), .O(new_n54_));
  oai21  g25(.a(new_n53_), .b(new_n48_), .c(new_n54_), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(z06));
  nand2  g28(.a(new_n50_), .b(x15), .O(z07));
  inv1   g29(.a(x10), .O(new_n59_));
  nand3  g30(.a(x03), .b(x02), .c(x00), .O(new_n60_));
  nand4  g31(.a(x12), .b(x11), .c(x09), .d(x04), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  aoi21  g33(.a(new_n30_), .b(new_n32_), .c(new_n62_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(new_n54_), .c(new_n59_), .d(x00), .O(z08));
  oai21  g35(.a(x11), .b(x10), .c(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  and2   g37(.a(x03), .b(x02), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n52_), .c(x09), .d(x04), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(x12), .c(x11), .d(new_n59_), .O(new_n69_));
  oai21  g40(.a(new_n69_), .b(new_n42_), .c(new_n66_), .O(z09));
  nand4  g41(.a(new_n52_), .b(x12), .c(x11), .d(new_n59_), .O(new_n71_));
  nor2   g42(.a(new_n71_), .b(new_n43_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n50_), .O(z10));
  nand2  g45(.a(x11), .b(new_n59_), .O(new_n75_));
  oai21  g46(.a(new_n75_), .b(x01), .c(x00), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n30_), .O(new_n77_));
  nand3  g48(.a(new_n72_), .b(x01), .c(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n77_), .O(z11));
  nand4  g50(.a(x11), .b(new_n59_), .c(new_n43_), .d(x00), .O(new_n80_));
  nor2   g51(.a(new_n80_), .b(new_n30_), .O(z12));
endmodule


