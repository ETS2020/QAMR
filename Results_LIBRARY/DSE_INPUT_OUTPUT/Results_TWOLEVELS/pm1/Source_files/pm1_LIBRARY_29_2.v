// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:35 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x08), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  nand4  g03(.a(new_n30_), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(z00));
  nand2  g06(.a(x12), .b(new_n32_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand3  g08(.a(new_n33_), .b(x12), .c(x11), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(z01));
  inv1   g10(.a(x12), .O(new_n40_));
  nand3  g11(.a(x07), .b(x06), .c(x05), .O(new_n41_));
  nand2  g12(.a(x11), .b(x09), .O(new_n42_));
  aoi21  g13(.a(new_n41_), .b(x11), .c(new_n42_), .O(new_n43_));
  oai21  g14(.a(new_n43_), .b(new_n40_), .c(new_n37_), .O(z02));
  nand2  g15(.a(x06), .b(x05), .O(new_n45_));
  nand2  g16(.a(x12), .b(x11), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x07), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n45_), .c(x12), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x08), .O(new_n50_));
  nor2   g21(.a(x12), .b(x08), .O(new_n51_));
  aoi21  g22(.a(new_n42_), .b(x12), .c(new_n51_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n50_), .O(z03));
  nor2   g24(.a(x12), .b(new_n30_), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(x14), .O(z04));
  inv1   g26(.a(new_n54_), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x13), .O(z05));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n54_), .c(new_n36_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(x09), .c(x01), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z06));
  nand2  g33(.a(new_n56_), .b(x15), .O(z07));
  inv1   g34(.a(x10), .O(new_n64_));
  inv1   g35(.a(x09), .O(new_n65_));
  nor3   g36(.a(new_n58_), .b(new_n46_), .c(new_n65_), .O(new_n66_));
  nor3   g37(.a(x12), .b(x11), .c(x08), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n66_), .c(new_n64_), .O(new_n68_));
  nor2   g39(.a(new_n54_), .b(x10), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n68_), .c(new_n36_), .d(x00), .O(z08));
  inv1   g41(.a(new_n67_), .O(new_n71_));
  and2   g42(.a(x03), .b(x02), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n58_), .c(x09), .d(x04), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(x12), .c(x11), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n71_), .O(new_n75_));
  nand3  g46(.a(new_n75_), .b(new_n64_), .c(x00), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(z09));
  nand4  g48(.a(new_n58_), .b(x12), .c(x11), .d(new_n64_), .O(new_n78_));
  nor2   g49(.a(new_n78_), .b(new_n65_), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(x00), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n56_), .O(z10));
  inv1   g52(.a(x01), .O(new_n82_));
  nand4  g53(.a(x11), .b(new_n64_), .c(new_n82_), .d(x00), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n30_), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n40_), .O(new_n85_));
  nand3  g56(.a(new_n79_), .b(x01), .c(x00), .O(new_n86_));
  nand2  g57(.a(new_n86_), .b(new_n85_), .O(z11));
  nand2  g58(.a(new_n65_), .b(x00), .O(new_n88_));
  nand2  g59(.a(new_n47_), .b(new_n64_), .O(new_n89_));
  oai21  g60(.a(new_n89_), .b(new_n88_), .c(new_n56_), .O(z12));
endmodule


