// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n41_, new_n42_, new_n43_, new_n45_, new_n49_,
    new_n50_, new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n34_), .O(new_n35_));
  inv1   g06(.a(x12), .O(new_n36_));
  inv1   g07(.a(x13), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z00));
  nand2  g10(.a(new_n33_), .b(x12), .O(z01));
  inv1   g11(.a(new_n31_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x08), .c(x07), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g15(.a(new_n41_), .b(x11), .c(x08), .d(x07), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(x12), .c(x11), .d(x09), .O(z03));
  aoi21  g17(.a(new_n37_), .b(new_n36_), .c(x14), .O(z04));
  nor2   g18(.a(x13), .b(new_n36_), .O(z05));
  and2   g19(.a(x03), .b(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n35_), .c(x04), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n38_), .O(z06));
  nand2  g23(.a(new_n38_), .b(x15), .O(z07));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x04), .b(x03), .c(x02), .O(new_n55_));
  nand3  g26(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  nand3  g27(.a(x13), .b(new_n36_), .c(new_n34_), .O(new_n57_));
  oai21  g28(.a(new_n56_), .b(new_n55_), .c(new_n57_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n54_), .c(x00), .O(new_n59_));
  nand2  g30(.a(new_n54_), .b(x00), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n38_), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(new_n59_), .c(new_n35_), .O(z08));
  inv1   g33(.a(x00), .O(new_n63_));
  oai21  g34(.a(new_n60_), .b(x11), .c(x13), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n36_), .O(new_n65_));
  nand4  g36(.a(new_n55_), .b(new_n49_), .c(x09), .d(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x11), .d(new_n54_), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n63_), .c(new_n65_), .O(z09));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(new_n55_), .b(x12), .c(x11), .d(new_n54_), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n69_), .c(new_n63_), .O(z10));
  nand4  g42(.a(new_n55_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  nand3  g43(.a(x13), .b(new_n36_), .c(new_n30_), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x11), .c(new_n54_), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z11));
  nand2  g47(.a(new_n69_), .b(x00), .O(new_n77_));
  nand3  g48(.a(x12), .b(x11), .c(new_n54_), .O(new_n78_));
  oai21  g49(.a(new_n78_), .b(new_n77_), .c(new_n38_), .O(z12));
endmodule


