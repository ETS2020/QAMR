// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:41 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_;
  nor2   g00(.a(x15), .b(x14), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nor2   g04(.a(new_n33_), .b(x01), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n30_), .O(z00));
  aoi21  g06(.a(x12), .b(new_n31_), .c(new_n30_), .O(z01));
  inv1   g07(.a(new_n30_), .O(new_n37_));
  nor2   g08(.a(new_n32_), .b(new_n31_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n37_), .O(new_n40_));
  and2   g11(.a(x06), .b(x05), .O(new_n41_));
  inv1   g12(.a(x08), .O(new_n42_));
  nor2   g13(.a(new_n30_), .b(new_n42_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(new_n40_), .d(x07), .O(z02));
  nand3  g15(.a(new_n41_), .b(x08), .c(x07), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n38_), .c(x09), .O(new_n46_));
  and2   g17(.a(new_n46_), .b(new_n37_), .O(z03));
  inv1   g18(.a(x14), .O(z04));
  nand2  g19(.a(new_n37_), .b(x13), .O(z05));
  nand2  g20(.a(x12), .b(new_n31_), .O(new_n50_));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(new_n50_), .c(x04), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(new_n37_), .c(x09), .d(x01), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(z06));
  inv1   g25(.a(x15), .O(z07));
  inv1   g26(.a(x00), .O(new_n56_));
  nor2   g27(.a(x10), .b(new_n56_), .O(new_n57_));
  nand4  g28(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n58_));
  inv1   g29(.a(x10), .O(new_n59_));
  nand3  g30(.a(new_n38_), .b(new_n59_), .c(x09), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(new_n31_), .O(new_n62_));
  aoi21  g33(.a(new_n62_), .b(new_n57_), .c(new_n30_), .O(z08));
  nand3  g34(.a(x04), .b(x03), .c(x02), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n51_), .c(x09), .d(x04), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n37_), .c(x12), .d(x11), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n33_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n59_), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n37_), .O(z09));
  nand4  g40(.a(new_n64_), .b(new_n37_), .c(x12), .d(x11), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n59_), .c(x09), .d(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z10));
  nand4  g44(.a(new_n64_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  oai21  g45(.a(x12), .b(x01), .c(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x11), .c(new_n59_), .d(x00), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n37_), .O(z11));
  nand4  g48(.a(new_n37_), .b(x12), .c(x11), .d(new_n59_), .O(new_n78_));
  nor3   g49(.a(new_n78_), .b(x09), .c(new_n56_), .O(z12));
endmodule


