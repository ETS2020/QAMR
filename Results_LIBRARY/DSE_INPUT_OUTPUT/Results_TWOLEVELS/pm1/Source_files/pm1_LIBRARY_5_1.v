// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  oai21  g04(.a(x12), .b(new_n30_), .c(new_n33_), .O(z00));
  nand3  g05(.a(new_n33_), .b(x12), .c(x11), .O(z01));
  inv1   g06(.a(new_n31_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x08), .c(x07), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g10(.a(x12), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x11), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  inv1   g13(.a(x09), .O(new_n43_));
  nor2   g14(.a(new_n40_), .b(new_n43_), .O(new_n44_));
  nand4  g15(.a(new_n36_), .b(x11), .c(x08), .d(x07), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n44_), .c(new_n42_), .O(z03));
  nor2   g17(.a(new_n41_), .b(x14), .O(z04));
  nand2  g18(.a(new_n42_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n42_), .O(z06));
  nor2   g22(.a(new_n41_), .b(x15), .O(z07));
  inv1   g23(.a(x10), .O(new_n53_));
  nand4  g24(.a(new_n44_), .b(x04), .c(x03), .d(x02), .O(new_n54_));
  nand4  g25(.a(new_n54_), .b(x11), .c(new_n53_), .d(x00), .O(z08));
  aoi21  g26(.a(new_n53_), .b(x00), .c(x12), .O(new_n56_));
  or2    g27(.a(new_n56_), .b(x11), .O(new_n57_));
  nand2  g28(.a(x03), .b(x02), .O(new_n58_));
  nand2  g29(.a(new_n49_), .b(x04), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(x09), .O(new_n60_));
  nand2  g31(.a(x11), .b(new_n43_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x12), .c(new_n53_), .d(x00), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n57_), .O(z09));
  nand4  g35(.a(new_n49_), .b(new_n53_), .c(x09), .d(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(x11), .c(new_n40_), .O(z10));
  nand4  g37(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n67_));
  nand3  g38(.a(new_n40_), .b(x11), .c(new_n30_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n53_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n42_), .O(z11));
  nand4  g42(.a(x11), .b(new_n53_), .c(new_n43_), .d(x00), .O(new_n72_));
  nor2   g43(.a(new_n72_), .b(new_n40_), .O(z12));
endmodule


