// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x01), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n30_), .O(new_n34_));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x08), .c(x07), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x14), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n30_), .c(new_n34_), .O(z00));
  inv1   g09(.a(x14), .O(z04));
  nor3   g10(.a(new_n36_), .b(new_n35_), .c(z04), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n30_), .c(x12), .O(z01));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(z04), .c(x11), .O(new_n43_));
  nand3  g14(.a(x12), .b(x11), .c(x09), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n43_), .O(z02));
  nand3  g17(.a(x14), .b(x08), .c(x07), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n35_), .c(x14), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x11), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n45_), .O(z03));
  nor2   g21(.a(x14), .b(new_n30_), .O(new_n51_));
  nor2   g22(.a(new_n51_), .b(x13), .O(z05));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  oai22  g25(.a(new_n54_), .b(new_n51_), .c(new_n32_), .d(x11), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(z06));
  oai21  g28(.a(x14), .b(new_n30_), .c(x15), .O(z07));
  inv1   g29(.a(x00), .O(new_n59_));
  inv1   g30(.a(x10), .O(new_n60_));
  nand4  g31(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n61_));
  nand4  g32(.a(x12), .b(x11), .c(new_n60_), .d(x09), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n59_), .c(x14), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x11), .O(z08));
  and2   g36(.a(x03), .b(x02), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(new_n53_), .c(x09), .d(x04), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(x14), .c(x12), .d(x11), .O(new_n68_));
  nand2  g39(.a(new_n32_), .b(new_n30_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n60_), .c(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  nand4  g43(.a(new_n53_), .b(x14), .c(x12), .d(new_n60_), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x09), .c(x00), .O(new_n75_));
  aoi21  g46(.a(new_n75_), .b(x14), .c(new_n30_), .O(z10));
  nand4  g47(.a(new_n53_), .b(x12), .c(x09), .d(x01), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n33_), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(new_n60_), .c(x00), .O(new_n79_));
  aoi21  g50(.a(new_n79_), .b(x14), .c(new_n30_), .O(z11));
  nor2   g51(.a(x09), .b(new_n59_), .O(new_n81_));
  nand4  g52(.a(new_n81_), .b(x14), .c(x12), .d(new_n60_), .O(new_n82_));
  aoi21  g53(.a(new_n82_), .b(x14), .c(new_n30_), .O(z12));
endmodule


