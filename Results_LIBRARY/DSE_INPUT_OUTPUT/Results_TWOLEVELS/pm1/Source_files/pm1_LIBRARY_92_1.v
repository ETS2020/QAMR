// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x08), .O(new_n31_));
  oai21  g02(.a(x15), .b(x11), .c(new_n31_), .O(new_n32_));
  and2   g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(x07), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x11), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(new_n32_), .d(new_n30_), .O(z00));
  nand3  g09(.a(new_n35_), .b(new_n32_), .c(x12), .O(z01));
  nand3  g10(.a(new_n33_), .b(x08), .c(x07), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(x12), .c(x11), .d(x09), .O(z02));
  inv1   g13(.a(x09), .O(new_n43_));
  inv1   g14(.a(x12), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand2  g16(.a(x15), .b(new_n31_), .O(new_n46_));
  nand4  g17(.a(new_n33_), .b(x11), .c(x08), .d(x07), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n37_), .O(z03));
  nand2  g19(.a(new_n46_), .b(x14), .O(z04));
  nand2  g20(.a(new_n46_), .b(x13), .O(z05));
  nand4  g21(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(new_n46_), .c(x09), .d(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  inv1   g24(.a(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x04), .b(x03), .c(x02), .O(new_n56_));
  nand3  g27(.a(x12), .b(x11), .c(x09), .O(new_n57_));
  nand2  g28(.a(new_n44_), .b(new_n36_), .O(new_n58_));
  oai21  g29(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  aoi21  g31(.a(x15), .b(new_n31_), .c(x10), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n60_), .c(new_n37_), .d(x00), .O(z08));
  nand4  g33(.a(new_n56_), .b(x15), .c(x09), .d(new_n31_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(x12), .c(x11), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n58_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n55_), .c(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n46_), .O(z09));
  nand4  g38(.a(new_n56_), .b(new_n46_), .c(x12), .d(x11), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n55_), .c(x09), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z10));
  nand4  g42(.a(new_n56_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  oai21  g43(.a(x12), .b(x01), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n55_), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n46_), .O(z11));
  nand2  g46(.a(new_n43_), .b(x00), .O(new_n76_));
  nand3  g47(.a(x12), .b(x11), .c(new_n55_), .O(new_n77_));
  oai21  g48(.a(new_n77_), .b(new_n76_), .c(new_n46_), .O(z12));
endmodule


