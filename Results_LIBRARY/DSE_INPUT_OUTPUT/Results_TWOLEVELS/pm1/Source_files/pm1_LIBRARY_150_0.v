// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n83_, new_n85_, new_n86_, new_n87_, new_n89_;
  inv1   g00(.a(x11), .O(new_n30_));
  nor2   g01(.a(x15), .b(x07), .O(new_n31_));
  inv1   g02(.a(new_n31_), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  inv1   g04(.a(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  and2   g07(.a(x06), .b(x05), .O(new_n37_));
  nand3  g08(.a(x08), .b(x06), .c(x05), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(new_n37_), .c(x08), .O(new_n39_));
  inv1   g10(.a(x15), .O(z07));
  nor2   g11(.a(z07), .b(x07), .O(new_n41_));
  aoi21  g12(.a(new_n39_), .b(x07), .c(new_n41_), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(new_n30_), .c(new_n36_), .O(z00));
  oai22  g14(.a(new_n42_), .b(new_n30_), .c(new_n31_), .d(x12), .O(z01));
  nand2  g15(.a(new_n32_), .b(new_n30_), .O(new_n45_));
  nand2  g16(.a(new_n38_), .b(x11), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x12), .c(x09), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x07), .O(new_n48_));
  inv1   g19(.a(x07), .O(new_n49_));
  nand3  g20(.a(x15), .b(x11), .c(new_n49_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n48_), .c(new_n45_), .O(z02));
  inv1   g22(.a(new_n37_), .O(new_n52_));
  nand3  g23(.a(x12), .b(x11), .c(x09), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand3  g25(.a(x11), .b(x08), .c(x07), .O(new_n55_));
  oai22  g26(.a(new_n55_), .b(new_n52_), .c(new_n54_), .d(new_n31_), .O(z03));
  nor2   g27(.a(new_n31_), .b(x14), .O(z04));
  nor2   g28(.a(new_n31_), .b(x13), .O(z05));
  nand2  g29(.a(x12), .b(new_n30_), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x04), .c(x03), .d(x02), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(new_n32_), .c(x09), .d(x01), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(z06));
  inv1   g33(.a(x00), .O(new_n63_));
  nor2   g34(.a(x10), .b(new_n63_), .O(new_n64_));
  nand4  g35(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n65_));
  inv1   g36(.a(x10), .O(new_n66_));
  nand4  g37(.a(x12), .b(x11), .c(new_n66_), .d(x09), .O(new_n67_));
  nor2   g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n30_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n64_), .c(new_n31_), .O(z08));
  nand4  g41(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x09), .c(new_n31_), .O(new_n72_));
  nand3  g43(.a(x04), .b(x03), .c(x02), .O(new_n73_));
  and2   g44(.a(new_n73_), .b(x09), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n72_), .c(x12), .O(new_n75_));
  nand2  g46(.a(new_n34_), .b(new_n30_), .O(new_n76_));
  oai21  g47(.a(new_n75_), .b(new_n30_), .c(new_n76_), .O(new_n77_));
  nand3  g48(.a(new_n77_), .b(new_n66_), .c(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n32_), .O(z09));
  and2   g50(.a(new_n73_), .b(x12), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(x11), .c(new_n66_), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(new_n82_));
  nand3  g53(.a(new_n82_), .b(x09), .c(x00), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n32_), .O(z10));
  nand3  g55(.a(new_n80_), .b(x09), .c(x01), .O(new_n85_));
  nand2  g56(.a(new_n85_), .b(new_n35_), .O(new_n86_));
  nand4  g57(.a(new_n86_), .b(new_n32_), .c(x11), .d(new_n66_), .O(new_n87_));
  nor2   g58(.a(new_n87_), .b(new_n63_), .O(z11));
  nand4  g59(.a(new_n32_), .b(x12), .c(x11), .d(new_n66_), .O(new_n89_));
  nor3   g60(.a(new_n89_), .b(x09), .c(new_n63_), .O(z12));
endmodule


