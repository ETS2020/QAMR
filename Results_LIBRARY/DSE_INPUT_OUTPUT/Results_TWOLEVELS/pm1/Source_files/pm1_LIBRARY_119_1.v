// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n44_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x12), .O(new_n30_));
  inv1   g01(.a(x13), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x06), .b(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x08), .c(x07), .O(new_n35_));
  nor3   g06(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n30_), .c(new_n32_), .O(z00));
  nor2   g08(.a(new_n35_), .b(new_n34_), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n33_), .c(x12), .O(z01));
  inv1   g10(.a(new_n34_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x08), .c(x07), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g14(.a(new_n40_), .b(x11), .c(x08), .d(x07), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g16(.a(x13), .b(new_n30_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(x13), .b(x12), .O(z05));
  and2   g19(.a(x03), .b(x02), .O(new_n49_));
  nand2  g20(.a(x12), .b(new_n33_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n49_), .c(x04), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n46_), .O(z06));
  aoi21  g24(.a(x13), .b(new_n30_), .c(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x04), .b(x03), .c(x02), .O(new_n56_));
  nand3  g27(.a(x12), .b(x11), .c(x09), .O(new_n57_));
  nand3  g28(.a(new_n31_), .b(new_n30_), .c(new_n33_), .O(new_n58_));
  oai21  g29(.a(new_n57_), .b(new_n56_), .c(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n55_), .c(x00), .O(new_n60_));
  nand2  g31(.a(new_n55_), .b(x00), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n46_), .O(new_n62_));
  nand3  g33(.a(new_n62_), .b(new_n60_), .c(new_n50_), .O(z08));
  inv1   g34(.a(x00), .O(new_n64_));
  oai21  g35(.a(new_n61_), .b(x11), .c(new_n31_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n30_), .O(new_n66_));
  nand4  g37(.a(new_n56_), .b(new_n49_), .c(x09), .d(x04), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(x12), .c(x11), .d(new_n55_), .O(new_n68_));
  oai21  g39(.a(new_n68_), .b(new_n64_), .c(new_n66_), .O(z09));
  nand4  g40(.a(new_n56_), .b(x12), .c(x11), .d(new_n55_), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(x09), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n46_), .O(z10));
  nand4  g44(.a(new_n56_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  inv1   g45(.a(x01), .O(new_n75_));
  nand3  g46(.a(new_n31_), .b(new_n30_), .c(new_n75_), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n74_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n55_), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z11));
  inv1   g50(.a(x09), .O(new_n80_));
  nand4  g51(.a(x11), .b(new_n55_), .c(new_n80_), .d(x00), .O(new_n81_));
  nor2   g52(.a(new_n81_), .b(new_n30_), .O(z12));
endmodule


