// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n51_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x11), .O(new_n30_));
  oai21  g01(.a(new_n30_), .b(x04), .c(x12), .O(new_n31_));
  nor2   g02(.a(x11), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(z00));
  nand2  g04(.a(x06), .b(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x08), .c(x07), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n34_), .c(x11), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(x04), .c(x12), .O(z01));
  inv1   g08(.a(x04), .O(new_n38_));
  nand2  g09(.a(x08), .b(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n34_), .c(x11), .O(new_n40_));
  inv1   g11(.a(x09), .O(new_n41_));
  aoi21  g12(.a(x12), .b(new_n30_), .c(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n40_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n38_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(z02));
  inv1   g16(.a(x12), .O(new_n46_));
  nand3  g17(.a(x11), .b(x08), .c(x07), .O(new_n47_));
  oai22  g18(.a(new_n47_), .b(new_n34_), .c(new_n46_), .d(x11), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n41_), .c(new_n38_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(z03));
  nand2  g21(.a(x12), .b(x04), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x14), .O(z04));
  nand2  g23(.a(new_n51_), .b(x13), .O(z05));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n51_), .O(z06));
  nand2  g27(.a(new_n51_), .b(x15), .O(z07));
  nor2   g28(.a(x12), .b(x10), .O(new_n58_));
  aoi22  g29(.a(new_n58_), .b(x00), .c(x12), .d(new_n38_), .O(new_n59_));
  inv1   g30(.a(x10), .O(new_n60_));
  nand3  g31(.a(new_n51_), .b(new_n60_), .c(x00), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  oai21  g33(.a(new_n59_), .b(x11), .c(new_n62_), .O(z08));
  nand2  g34(.a(x09), .b(x04), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(x12), .c(x11), .O(new_n65_));
  nand2  g36(.a(new_n46_), .b(new_n30_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n60_), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n51_), .O(z09));
  nand4  g40(.a(new_n60_), .b(x09), .c(new_n38_), .d(x00), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n46_), .c(new_n30_), .O(z10));
  nand3  g42(.a(x12), .b(x09), .c(x01), .O(new_n72_));
  oai21  g43(.a(x12), .b(x01), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n60_), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n51_), .O(z11));
  nand4  g46(.a(x11), .b(new_n60_), .c(new_n41_), .d(x00), .O(new_n76_));
  aoi21  g47(.a(new_n76_), .b(new_n38_), .c(new_n46_), .O(z12));
endmodule


