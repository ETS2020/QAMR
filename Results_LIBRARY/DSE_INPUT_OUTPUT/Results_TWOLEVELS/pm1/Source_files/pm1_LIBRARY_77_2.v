// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n34_), .O(new_n35_));
  inv1   g06(.a(x12), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x00), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n35_), .c(new_n33_), .d(new_n30_), .O(z00));
  nand2  g09(.a(new_n33_), .b(x12), .O(z01));
  inv1   g10(.a(new_n31_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x08), .c(x07), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g14(.a(new_n40_), .b(x11), .c(x08), .d(x07), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g16(.a(new_n37_), .b(x14), .O(z04));
  inv1   g17(.a(new_n37_), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(x13), .O(z05));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(new_n37_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n35_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  nor2   g24(.a(new_n47_), .b(x15), .O(z07));
  nand3  g25(.a(x03), .b(x02), .c(x00), .O(new_n55_));
  inv1   g26(.a(x10), .O(new_n56_));
  nand4  g27(.a(x11), .b(new_n56_), .c(x09), .d(x04), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n55_), .c(x11), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(x10), .c(x12), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x00), .O(z08));
  inv1   g31(.a(x00), .O(new_n61_));
  and2   g32(.a(x03), .b(x02), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n49_), .c(x09), .d(x04), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x12), .c(x11), .d(new_n56_), .O(new_n64_));
  aoi21  g35(.a(new_n64_), .b(x12), .c(new_n61_), .O(z09));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(new_n49_), .b(x12), .c(x11), .d(new_n56_), .O(new_n67_));
  nor3   g38(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z10));
  nand3  g39(.a(new_n49_), .b(x11), .c(new_n56_), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(x09), .c(x01), .O(new_n71_));
  aoi21  g42(.a(new_n71_), .b(x12), .c(new_n61_), .O(z11));
  nand4  g43(.a(x11), .b(new_n56_), .c(new_n66_), .d(x00), .O(new_n73_));
  nor2   g44(.a(new_n73_), .b(new_n36_), .O(z12));
endmodule


