// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n61_, new_n63_,
    new_n64_, new_n65_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_,
    new_n90_, new_n92_, new_n93_, new_n94_;
  inv1   g00(.a(x14), .O(new_n30_));
  nand2  g01(.a(x15), .b(new_n30_), .O(new_n31_));
  nand2  g02(.a(new_n31_), .b(x01), .O(new_n32_));
  inv1   g03(.a(x11), .O(new_n33_));
  nand2  g04(.a(x15), .b(new_n30_), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x12), .c(new_n33_), .O(new_n35_));
  nor2   g06(.a(x15), .b(x14), .O(new_n36_));
  nand2  g07(.a(x06), .b(x05), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(x08), .c(x07), .O(new_n38_));
  nor2   g09(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n30_), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n36_), .c(x11), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n35_), .c(new_n32_), .O(z00));
  inv1   g13(.a(x12), .O(new_n43_));
  nand2  g14(.a(new_n34_), .b(new_n43_), .O(new_n44_));
  oai21  g15(.a(x15), .b(x11), .c(new_n30_), .O(new_n45_));
  nand2  g16(.a(new_n40_), .b(x11), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(new_n45_), .c(new_n44_), .O(z01));
  nand2  g18(.a(x08), .b(x07), .O(new_n48_));
  oai21  g19(.a(new_n30_), .b(new_n33_), .c(x15), .O(new_n49_));
  oai21  g20(.a(new_n48_), .b(new_n37_), .c(new_n49_), .O(new_n50_));
  inv1   g21(.a(x09), .O(new_n51_));
  inv1   g22(.a(x15), .O(new_n52_));
  nand2  g23(.a(x12), .b(x11), .O(new_n53_));
  oai22  g24(.a(new_n53_), .b(new_n51_), .c(new_n52_), .d(x14), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n50_), .O(z02));
  inv1   g26(.a(x05), .O(new_n56_));
  nand4  g27(.a(new_n49_), .b(x08), .c(x07), .d(x06), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n56_), .c(new_n54_), .O(z03));
  oai21  g29(.a(new_n53_), .b(x11), .c(new_n52_), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(x14), .O(z04));
  inv1   g31(.a(new_n31_), .O(new_n61_));
  nor2   g32(.a(new_n61_), .b(x13), .O(z05));
  nand2  g33(.a(x12), .b(new_n33_), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x04), .c(x03), .d(x02), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n31_), .c(x09), .d(x01), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z06));
  nor2   g37(.a(new_n39_), .b(x15), .O(z07));
  inv1   g38(.a(x10), .O(new_n68_));
  nand3  g39(.a(x04), .b(x03), .c(x02), .O(new_n69_));
  nand3  g40(.a(x12), .b(x11), .c(x09), .O(new_n70_));
  nand2  g41(.a(new_n43_), .b(new_n33_), .O(new_n71_));
  oai21  g42(.a(new_n70_), .b(new_n69_), .c(new_n71_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n68_), .O(new_n73_));
  nor2   g44(.a(new_n61_), .b(x10), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(new_n73_), .c(new_n35_), .d(x00), .O(z08));
  nand4  g46(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n31_), .O(new_n77_));
  nand4  g48(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n77_), .O(new_n79_));
  nand3  g50(.a(new_n79_), .b(x12), .c(x11), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n71_), .O(new_n81_));
  nand3  g52(.a(new_n81_), .b(new_n68_), .c(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n31_), .O(z09));
  nand4  g54(.a(new_n69_), .b(new_n31_), .c(x12), .d(x11), .O(new_n84_));
  inv1   g55(.a(new_n84_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(new_n68_), .c(x09), .d(x00), .O(new_n86_));
  inv1   g57(.a(new_n86_), .O(z10));
  nand4  g58(.a(new_n69_), .b(x12), .c(x09), .d(x01), .O(new_n88_));
  oai21  g59(.a(x12), .b(x01), .c(new_n88_), .O(new_n89_));
  nand4  g60(.a(new_n89_), .b(x11), .c(new_n68_), .d(x00), .O(new_n90_));
  nand2  g61(.a(new_n90_), .b(new_n31_), .O(z11));
  inv1   g62(.a(x00), .O(new_n92_));
  nor2   g63(.a(new_n61_), .b(new_n43_), .O(new_n93_));
  nand4  g64(.a(new_n93_), .b(x11), .c(new_n68_), .d(new_n51_), .O(new_n94_));
  nor2   g65(.a(new_n94_), .b(new_n92_), .O(z12));
endmodule


