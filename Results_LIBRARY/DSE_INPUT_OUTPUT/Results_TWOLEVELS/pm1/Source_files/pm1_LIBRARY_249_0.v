// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n41_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n76_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x12), .c(x11), .O(z00));
  nand2  g04(.a(new_n32_), .b(x12), .O(z01));
  inv1   g05(.a(new_n30_), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g09(.a(new_n35_), .b(x11), .c(x08), .d(x07), .O(new_n39_));
  nand4  g10(.a(new_n39_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g11(.a(x12), .b(x01), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(x14), .O(z04));
  nor2   g13(.a(new_n41_), .b(x13), .O(z05));
  inv1   g14(.a(x11), .O(new_n44_));
  nand2  g15(.a(x12), .b(new_n44_), .O(new_n45_));
  and2   g16(.a(x03), .b(x02), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(new_n45_), .c(x04), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(z06));
  inv1   g20(.a(new_n41_), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x15), .O(z07));
  inv1   g22(.a(x10), .O(new_n52_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  nand3  g24(.a(x12), .b(x11), .c(x09), .O(new_n54_));
  inv1   g25(.a(x12), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n44_), .c(x01), .O(new_n56_));
  oai21  g27(.a(new_n54_), .b(new_n53_), .c(new_n56_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(new_n52_), .c(x00), .O(new_n58_));
  inv1   g29(.a(x00), .O(new_n59_));
  oai21  g30(.a(x10), .b(new_n59_), .c(new_n50_), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n58_), .c(new_n45_), .O(z08));
  nand2  g32(.a(new_n44_), .b(new_n52_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n59_), .c(x01), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n55_), .O(new_n64_));
  nand4  g35(.a(new_n53_), .b(new_n46_), .c(x09), .d(x04), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x12), .c(x11), .d(new_n52_), .O(new_n66_));
  oai21  g37(.a(new_n66_), .b(new_n59_), .c(new_n64_), .O(z09));
  inv1   g38(.a(x09), .O(new_n68_));
  nand4  g39(.a(new_n53_), .b(x12), .c(x11), .d(new_n52_), .O(new_n69_));
  nor2   g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n50_), .O(z10));
  nand3  g43(.a(new_n70_), .b(x01), .c(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n50_), .O(z11));
  nand2  g45(.a(new_n68_), .b(x00), .O(new_n75_));
  nand3  g46(.a(x12), .b(x11), .c(new_n52_), .O(new_n76_));
  oai21  g47(.a(new_n76_), .b(new_n75_), .c(new_n50_), .O(z12));
endmodule


