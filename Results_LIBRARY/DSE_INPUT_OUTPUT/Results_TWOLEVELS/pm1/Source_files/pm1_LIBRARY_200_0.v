// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n46_, new_n47_, new_n48_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x11), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  inv1   g05(.a(x12), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n32_), .O(z01));
  nand3  g08(.a(x07), .b(x06), .c(x05), .O(new_n38_));
  nand3  g09(.a(x12), .b(x09), .c(x08), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x11), .O(new_n40_));
  oai21  g11(.a(new_n35_), .b(x11), .c(new_n40_), .O(z02));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z03));
  nand2  g14(.a(new_n36_), .b(x14), .O(z04));
  nand2  g15(.a(new_n36_), .b(x13), .O(z05));
  aoi21  g16(.a(x09), .b(x01), .c(new_n35_), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(x09), .c(x01), .O(new_n48_));
  oai21  g19(.a(new_n46_), .b(x11), .c(new_n48_), .O(z06));
  aoi21  g20(.a(new_n35_), .b(new_n34_), .c(x15), .O(z07));
  nand3  g21(.a(x03), .b(x02), .c(x00), .O(new_n51_));
  inv1   g22(.a(x10), .O(new_n52_));
  nand4  g23(.a(x11), .b(new_n52_), .c(x09), .d(x04), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n51_), .c(x11), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x12), .O(new_n55_));
  inv1   g26(.a(x00), .O(new_n56_));
  oai21  g27(.a(x10), .b(new_n56_), .c(x11), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n55_), .O(z08));
  and2   g29(.a(x03), .b(x02), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n47_), .c(x09), .d(x04), .O(new_n60_));
  nand4  g31(.a(new_n60_), .b(x12), .c(x11), .d(new_n52_), .O(new_n61_));
  oai21  g32(.a(new_n61_), .b(new_n56_), .c(new_n36_), .O(z09));
  inv1   g33(.a(x09), .O(new_n63_));
  nand4  g34(.a(new_n47_), .b(x12), .c(x11), .d(new_n52_), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n63_), .c(new_n56_), .O(z10));
  inv1   g36(.a(x01), .O(new_n66_));
  nand3  g37(.a(new_n52_), .b(new_n66_), .c(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(x11), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n35_), .O(new_n69_));
  inv1   g40(.a(new_n64_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x09), .c(x01), .d(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(z11));
  nand2  g43(.a(new_n63_), .b(x00), .O(new_n73_));
  nand3  g44(.a(x12), .b(x11), .c(new_n52_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n73_), .c(new_n36_), .O(z12));
endmodule


