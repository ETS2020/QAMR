// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x08), .O(new_n30_));
  nand2  g01(.a(x07), .b(x05), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x06), .O(new_n33_));
  nand2  g04(.a(x12), .b(x08), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n31_), .c(x11), .O(new_n35_));
  inv1   g06(.a(x06), .O(new_n36_));
  aoi21  g07(.a(x12), .b(new_n36_), .c(x01), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n35_), .c(new_n33_), .O(z00));
  inv1   g09(.a(x10), .O(new_n39_));
  and2   g10(.a(x03), .b(x02), .O(new_n40_));
  nand3  g11(.a(x04), .b(x03), .c(x02), .O(new_n41_));
  nand4  g12(.a(new_n41_), .b(new_n40_), .c(x09), .d(x04), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n39_), .c(x00), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x11), .O(new_n45_));
  inv1   g16(.a(x12), .O(new_n46_));
  nor2   g17(.a(x11), .b(new_n36_), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(new_n45_), .O(z01));
  inv1   g20(.a(new_n47_), .O(new_n50_));
  inv1   g21(.a(x11), .O(new_n51_));
  oai21  g22(.a(x12), .b(new_n51_), .c(new_n36_), .O(new_n52_));
  nand3  g23(.a(x12), .b(x09), .c(x08), .O(new_n53_));
  oai21  g24(.a(new_n53_), .b(new_n31_), .c(x11), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(new_n50_), .O(z02));
  nand4  g26(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x12), .c(x09), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x11), .O(new_n58_));
  nand2  g29(.a(new_n51_), .b(new_n36_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n58_), .O(z03));
  nor2   g31(.a(new_n47_), .b(x14), .O(z04));
  nor2   g32(.a(new_n47_), .b(x13), .O(z05));
  inv1   g33(.a(new_n41_), .O(new_n63_));
  nand3  g34(.a(x12), .b(new_n51_), .c(new_n36_), .O(new_n64_));
  oai21  g35(.a(new_n47_), .b(new_n63_), .c(new_n64_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x09), .c(x01), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(z06));
  nand2  g38(.a(new_n50_), .b(x15), .O(z07));
  inv1   g39(.a(x00), .O(new_n69_));
  nand2  g40(.a(new_n40_), .b(x00), .O(new_n70_));
  nand4  g41(.a(x12), .b(new_n39_), .c(x09), .d(x04), .O(new_n71_));
  oai21  g42(.a(new_n71_), .b(new_n70_), .c(new_n39_), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(new_n69_), .c(x11), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n59_), .O(z08));
  nand3  g45(.a(new_n42_), .b(x12), .c(x11), .O(new_n75_));
  nand3  g46(.a(new_n46_), .b(new_n51_), .c(new_n36_), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g48(.a(new_n77_), .b(new_n39_), .c(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z09));
  nand4  g50(.a(new_n41_), .b(x12), .c(x11), .d(new_n39_), .O(new_n80_));
  inv1   g51(.a(new_n80_), .O(new_n81_));
  nand3  g52(.a(new_n81_), .b(x09), .c(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n50_), .O(z10));
  nand4  g54(.a(new_n41_), .b(x12), .c(x09), .d(x01), .O(new_n84_));
  oai21  g55(.a(x12), .b(x01), .c(new_n84_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(x11), .c(new_n39_), .d(x00), .O(new_n86_));
  inv1   g57(.a(new_n86_), .O(z11));
  inv1   g58(.a(x09), .O(new_n88_));
  nand2  g59(.a(new_n88_), .b(x00), .O(new_n89_));
  nand3  g60(.a(x12), .b(x11), .c(new_n39_), .O(new_n90_));
  oai21  g61(.a(new_n90_), .b(new_n89_), .c(new_n50_), .O(z12));
endmodule


