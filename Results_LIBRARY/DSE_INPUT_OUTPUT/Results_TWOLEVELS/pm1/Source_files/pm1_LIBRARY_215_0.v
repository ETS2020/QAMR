// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  inv1   g01(.a(new_n30_), .O(new_n31_));
  nand3  g02(.a(new_n30_), .b(x08), .c(x07), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(x11), .O(z00));
  inv1   g05(.a(x11), .O(new_n35_));
  aoi21  g06(.a(new_n33_), .b(new_n31_), .c(new_n35_), .O(z01));
  inv1   g07(.a(x12), .O(new_n37_));
  nand3  g08(.a(x07), .b(x06), .c(x05), .O(new_n38_));
  nand3  g09(.a(x12), .b(x09), .c(x08), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x11), .O(new_n40_));
  oai21  g11(.a(new_n37_), .b(x11), .c(new_n40_), .O(z02));
  nand3  g12(.a(new_n31_), .b(x08), .c(x07), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z03));
  nor2   g14(.a(x12), .b(x11), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nor2   g17(.a(new_n44_), .b(x13), .O(z05));
  aoi21  g18(.a(x09), .b(x01), .c(new_n37_), .O(new_n48_));
  nand3  g19(.a(x04), .b(x03), .c(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  oai21  g21(.a(new_n48_), .b(x11), .c(new_n50_), .O(z06));
  nor2   g22(.a(new_n44_), .b(x15), .O(z07));
  nand3  g23(.a(x03), .b(x02), .c(x00), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand4  g25(.a(x11), .b(new_n54_), .c(x09), .d(x04), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(new_n53_), .c(x11), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x12), .O(new_n57_));
  inv1   g28(.a(x00), .O(new_n58_));
  oai21  g29(.a(x10), .b(new_n58_), .c(x11), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(z08));
  and2   g31(.a(x03), .b(x02), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n49_), .c(x09), .d(x04), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(x12), .c(x11), .d(new_n54_), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n58_), .O(z09));
  nand4  g35(.a(new_n49_), .b(x12), .c(x11), .d(new_n54_), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(x09), .c(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n45_), .O(z10));
  nand4  g39(.a(new_n49_), .b(x12), .c(x09), .d(x01), .O(new_n69_));
  inv1   g40(.a(x01), .O(new_n70_));
  nand2  g41(.a(new_n37_), .b(new_n70_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x11), .c(new_n54_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z11));
  inv1   g45(.a(x09), .O(new_n75_));
  nand4  g46(.a(x11), .b(new_n54_), .c(new_n75_), .d(x00), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n37_), .O(z12));
endmodule


