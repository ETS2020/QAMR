// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x00), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n35_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n32_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n31_), .O(z00));
  nand3  g11(.a(new_n35_), .b(x12), .c(new_n32_), .O(z01));
  inv1   g12(.a(x09), .O(new_n42_));
  inv1   g13(.a(new_n33_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(x08), .c(x07), .O(new_n44_));
  aoi21  g15(.a(new_n44_), .b(x11), .c(new_n42_), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(x12), .c(x11), .d(new_n32_), .O(z02));
  nand2  g17(.a(x05), .b(new_n32_), .O(new_n47_));
  nand3  g18(.a(x08), .b(x07), .c(x06), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n47_), .c(x12), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x11), .O(new_n50_));
  nand2  g21(.a(new_n37_), .b(x09), .O(new_n51_));
  aoi22  g22(.a(new_n51_), .b(new_n32_), .c(new_n30_), .d(new_n36_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n50_), .O(z03));
  nand2  g24(.a(x12), .b(x00), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(x14), .O(z04));
  nand2  g26(.a(new_n54_), .b(x13), .O(z05));
  nand3  g27(.a(x04), .b(x03), .c(x02), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  oai21  g29(.a(new_n37_), .b(x00), .c(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x09), .c(x01), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(z06));
  inv1   g32(.a(new_n54_), .O(z10));
  nor2   g33(.a(z10), .b(x15), .O(z07));
  inv1   g34(.a(new_n34_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n43_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x11), .c(new_n32_), .O(new_n66_));
  oai21  g37(.a(new_n30_), .b(new_n32_), .c(new_n36_), .O(new_n67_));
  nand2  g38(.a(new_n30_), .b(x10), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(z08));
  inv1   g40(.a(x10), .O(new_n70_));
  nand4  g41(.a(new_n30_), .b(new_n36_), .c(new_n70_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  inv1   g43(.a(x01), .O(new_n73_));
  nand4  g44(.a(x11), .b(new_n70_), .c(new_n73_), .d(x00), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(x12), .O(z11));
  inv1   g46(.a(new_n54_), .O(z12));
endmodule


