// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:01 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nand2  g02(.a(x12), .b(new_n31_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x02), .O(new_n34_));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x08), .c(x07), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x11), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n34_), .O(z00));
  inv1   g09(.a(x02), .O(new_n39_));
  inv1   g10(.a(x12), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n39_), .c(new_n31_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n37_), .O(z01));
  nand2  g13(.a(new_n31_), .b(x02), .O(new_n43_));
  nand3  g14(.a(x07), .b(x06), .c(x05), .O(new_n44_));
  nand3  g15(.a(x12), .b(x09), .c(x08), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n44_), .c(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n43_), .O(z02));
  nand4  g18(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x12), .c(x09), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x11), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n43_), .O(z03));
  nor2   g22(.a(x11), .b(x02), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(x14), .O(z04));
  nor2   g24(.a(new_n52_), .b(x13), .O(z05));
  inv1   g25(.a(x09), .O(new_n55_));
  nand3  g26(.a(new_n32_), .b(x04), .c(x03), .O(new_n56_));
  nor2   g27(.a(new_n31_), .b(x02), .O(new_n57_));
  aoi21  g28(.a(new_n56_), .b(x02), .c(new_n57_), .O(new_n58_));
  nor3   g29(.a(new_n58_), .b(new_n55_), .c(new_n30_), .O(z06));
  oai21  g30(.a(x11), .b(x02), .c(x15), .O(z07));
  nand3  g31(.a(x04), .b(x03), .c(x00), .O(new_n61_));
  inv1   g32(.a(x10), .O(new_n62_));
  nand4  g33(.a(x12), .b(x11), .c(new_n62_), .d(x09), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n61_), .c(x11), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x02), .O(new_n65_));
  inv1   g36(.a(x00), .O(new_n66_));
  oai21  g37(.a(x10), .b(new_n66_), .c(x11), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(z08));
  nand2  g39(.a(x09), .b(x00), .O(new_n69_));
  nand3  g40(.a(x12), .b(x11), .c(new_n62_), .O(new_n70_));
  oai21  g41(.a(new_n70_), .b(new_n69_), .c(x11), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n39_), .O(new_n72_));
  nand4  g43(.a(x09), .b(x04), .c(x03), .d(new_n39_), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(x12), .c(x11), .O(new_n74_));
  nand2  g45(.a(new_n40_), .b(new_n31_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(new_n62_), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n72_), .O(z09));
  nand3  g49(.a(x04), .b(x03), .c(x02), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(x12), .c(x11), .d(new_n62_), .O(new_n80_));
  nor3   g51(.a(new_n80_), .b(new_n55_), .c(new_n66_), .O(z10));
  nand2  g52(.a(x01), .b(x00), .O(new_n82_));
  nand3  g53(.a(x12), .b(new_n62_), .c(x09), .O(new_n83_));
  oai21  g54(.a(new_n83_), .b(new_n82_), .c(x11), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n39_), .O(new_n85_));
  nand2  g56(.a(x04), .b(x03), .O(new_n86_));
  nand4  g57(.a(new_n86_), .b(x12), .c(x09), .d(x01), .O(new_n87_));
  oai21  g58(.a(x12), .b(x01), .c(new_n87_), .O(new_n88_));
  nand4  g59(.a(new_n88_), .b(x11), .c(new_n62_), .d(x00), .O(new_n89_));
  nand2  g60(.a(new_n89_), .b(new_n85_), .O(z11));
  nand4  g61(.a(x11), .b(new_n62_), .c(new_n55_), .d(x00), .O(new_n91_));
  nor2   g62(.a(new_n91_), .b(new_n40_), .O(z12));
endmodule


