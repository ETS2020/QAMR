// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n37_,
    new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n56_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_;
  inv1   g00(.a(x08), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  nand4  g03(.a(new_n30_), .b(x07), .c(x06), .d(x05), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x12), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n31_), .O(z00));
  nand2  g06(.a(x12), .b(new_n32_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand3  g08(.a(new_n33_), .b(x12), .c(x11), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n37_), .O(z01));
  nand4  g10(.a(x11), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(new_n41_));
  nor2   g12(.a(x12), .b(new_n30_), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n41_), .c(new_n37_), .d(x09), .O(z02));
  nand2  g15(.a(x06), .b(x05), .O(new_n45_));
  nand2  g16(.a(x12), .b(x11), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x07), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n45_), .c(x12), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x08), .O(new_n50_));
  inv1   g21(.a(x12), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n30_), .O(new_n52_));
  nand4  g23(.a(new_n52_), .b(new_n50_), .c(new_n36_), .d(x09), .O(z03));
  nor2   g24(.a(new_n42_), .b(x14), .O(z04));
  nor2   g25(.a(new_n42_), .b(x13), .O(z05));
  and2   g26(.a(x03), .b(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n36_), .c(x04), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(x09), .c(x01), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n43_), .O(z06));
  nand2  g30(.a(new_n43_), .b(x15), .O(z07));
  inv1   g31(.a(x10), .O(new_n61_));
  inv1   g32(.a(x09), .O(new_n62_));
  nand3  g33(.a(x04), .b(x03), .c(x02), .O(new_n63_));
  nor3   g34(.a(new_n63_), .b(new_n46_), .c(new_n62_), .O(new_n64_));
  nor3   g35(.a(x12), .b(x11), .c(x08), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n64_), .c(new_n61_), .O(new_n66_));
  nor2   g37(.a(new_n42_), .b(x10), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n66_), .c(new_n36_), .d(x00), .O(z08));
  inv1   g39(.a(new_n65_), .O(new_n69_));
  nand4  g40(.a(new_n63_), .b(new_n56_), .c(x09), .d(x04), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(new_n61_), .c(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand4  g45(.a(new_n63_), .b(x12), .c(x11), .d(new_n61_), .O(new_n75_));
  nor2   g46(.a(new_n75_), .b(new_n62_), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n43_), .O(z10));
  inv1   g49(.a(x01), .O(new_n79_));
  nand4  g50(.a(x11), .b(new_n61_), .c(new_n79_), .d(x00), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n30_), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n51_), .O(new_n82_));
  nand3  g53(.a(new_n76_), .b(x01), .c(x00), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n82_), .O(z11));
  nand2  g55(.a(new_n62_), .b(x00), .O(new_n85_));
  nand2  g56(.a(new_n47_), .b(new_n61_), .O(new_n86_));
  oai21  g57(.a(new_n86_), .b(new_n85_), .c(new_n43_), .O(z12));
endmodule


