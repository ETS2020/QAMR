// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x08), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  nand2  g02(.a(x15), .b(new_n31_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n30_), .O(new_n33_));
  nand4  g04(.a(new_n30_), .b(x07), .c(x06), .d(x05), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(x12), .b(x01), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n35_), .c(new_n33_), .O(z00));
  nand3  g08(.a(new_n35_), .b(new_n33_), .c(x12), .O(z01));
  nor2   g09(.a(x15), .b(x08), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n31_), .O(new_n41_));
  nand3  g12(.a(x07), .b(x06), .c(x05), .O(new_n42_));
  nand2  g13(.a(x12), .b(x09), .O(new_n43_));
  aoi21  g14(.a(new_n42_), .b(x11), .c(new_n43_), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(new_n41_), .c(new_n33_), .O(z02));
  inv1   g16(.a(new_n43_), .O(new_n46_));
  nand4  g17(.a(x11), .b(x07), .c(x06), .d(x05), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x11), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x08), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(new_n46_), .c(new_n40_), .d(new_n32_), .O(z03));
  nor2   g21(.a(new_n39_), .b(x14), .O(z04));
  nor2   g22(.a(new_n39_), .b(x13), .O(z05));
  and2   g23(.a(x03), .b(x02), .O(new_n53_));
  nand2  g24(.a(x12), .b(new_n31_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n53_), .c(x04), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n40_), .O(z06));
  inv1   g28(.a(x15), .O(z07));
  inv1   g29(.a(x00), .O(new_n59_));
  nor2   g30(.a(x10), .b(new_n59_), .O(new_n60_));
  nand4  g31(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n61_));
  inv1   g32(.a(x10), .O(new_n62_));
  nand4  g33(.a(x12), .b(x11), .c(new_n62_), .d(x09), .O(new_n63_));
  nor2   g34(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n31_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n60_), .c(new_n39_), .O(z08));
  nand3  g37(.a(x04), .b(x03), .c(x02), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n53_), .c(x09), .d(x04), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  inv1   g40(.a(x12), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n31_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n40_), .c(new_n62_), .d(x00), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(z09));
  nand4  g45(.a(new_n67_), .b(new_n40_), .c(x12), .d(x11), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(new_n62_), .c(x09), .d(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z10));
  nand4  g49(.a(new_n67_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n62_), .d(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n40_), .O(z11));
  nand4  g53(.a(new_n40_), .b(x12), .c(x11), .d(new_n62_), .O(new_n83_));
  nor3   g54(.a(new_n83_), .b(x09), .c(new_n59_), .O(z12));
endmodule


