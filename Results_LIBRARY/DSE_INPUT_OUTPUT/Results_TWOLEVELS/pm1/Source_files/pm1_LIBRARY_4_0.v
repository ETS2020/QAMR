// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n87_;
  inv1   g00(.a(x07), .O(new_n30_));
  inv1   g01(.a(x15), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(x11), .c(new_n30_), .O(new_n32_));
  and2   g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(x08), .b(x06), .c(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n33_), .c(x08), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x11), .c(x07), .O(new_n36_));
  nor2   g07(.a(x12), .b(x01), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(new_n36_), .c(new_n32_), .O(z00));
  inv1   g09(.a(x11), .O(new_n39_));
  nor2   g10(.a(x15), .b(x07), .O(new_n40_));
  nor2   g11(.a(new_n31_), .b(x07), .O(new_n41_));
  aoi21  g12(.a(new_n35_), .b(x07), .c(new_n41_), .O(new_n42_));
  oai22  g13(.a(new_n42_), .b(new_n39_), .c(new_n40_), .d(x12), .O(z01));
  inv1   g14(.a(new_n40_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n39_), .O(new_n45_));
  nand2  g16(.a(new_n34_), .b(x11), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x12), .c(x09), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x07), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n45_), .c(new_n32_), .O(z02));
  inv1   g20(.a(new_n33_), .O(new_n50_));
  nand3  g21(.a(x12), .b(x11), .c(x09), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nand3  g23(.a(x11), .b(x08), .c(x07), .O(new_n53_));
  oai22  g24(.a(new_n53_), .b(new_n50_), .c(new_n52_), .d(new_n40_), .O(z03));
  nand2  g25(.a(new_n44_), .b(x14), .O(z04));
  nor2   g26(.a(new_n40_), .b(x13), .O(z05));
  and2   g27(.a(x03), .b(x02), .O(new_n57_));
  nand2  g28(.a(x12), .b(new_n39_), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(new_n57_), .c(x04), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x09), .c(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n44_), .O(z06));
  nor2   g32(.a(x15), .b(new_n30_), .O(z07));
  inv1   g33(.a(x00), .O(new_n63_));
  nor2   g34(.a(x10), .b(new_n63_), .O(new_n64_));
  nand4  g35(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n65_));
  inv1   g36(.a(x10), .O(new_n66_));
  nand4  g37(.a(x12), .b(x11), .c(new_n66_), .d(x09), .O(new_n67_));
  nor2   g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nor2   g39(.a(new_n68_), .b(new_n39_), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(new_n64_), .c(new_n40_), .O(z08));
  nand3  g41(.a(x04), .b(x03), .c(x02), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n57_), .c(x09), .d(x04), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x12), .c(x11), .O(new_n73_));
  inv1   g44(.a(x12), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n39_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(new_n44_), .c(new_n66_), .d(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z09));
  nand4  g49(.a(new_n71_), .b(new_n44_), .c(x12), .d(x11), .O(new_n79_));
  inv1   g50(.a(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(new_n66_), .c(x09), .d(x00), .O(new_n81_));
  inv1   g52(.a(new_n81_), .O(z10));
  nand4  g53(.a(new_n71_), .b(x12), .c(x09), .d(x01), .O(new_n83_));
  oai21  g54(.a(x12), .b(x01), .c(new_n83_), .O(new_n84_));
  nand4  g55(.a(new_n84_), .b(x11), .c(new_n66_), .d(x00), .O(new_n85_));
  nand2  g56(.a(new_n85_), .b(new_n44_), .O(z11));
  nand4  g57(.a(new_n44_), .b(x12), .c(x11), .d(new_n66_), .O(new_n87_));
  nor3   g58(.a(new_n87_), .b(x09), .c(new_n63_), .O(z12));
endmodule


