// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n49_, new_n51_, new_n52_, new_n53_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_;
  inv1   g00(.a(x11), .O(new_n30_));
  inv1   g01(.a(x14), .O(new_n31_));
  nor2   g02(.a(x12), .b(x01), .O(new_n32_));
  aoi22  g03(.a(new_n32_), .b(new_n30_), .c(new_n31_), .d(x07), .O(z00));
  aoi22  g04(.a(new_n31_), .b(x07), .c(x12), .d(new_n30_), .O(z01));
  nand3  g05(.a(x08), .b(x06), .c(x05), .O(new_n35_));
  nand3  g06(.a(x12), .b(x11), .c(x09), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x14), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x07), .O(z02));
  inv1   g09(.a(x09), .O(new_n39_));
  inv1   g10(.a(x12), .O(new_n40_));
  nand2  g11(.a(new_n31_), .b(x07), .O(new_n41_));
  oai21  g12(.a(new_n40_), .b(new_n39_), .c(new_n41_), .O(new_n42_));
  nor2   g13(.a(x14), .b(x07), .O(z04));
  oai21  g14(.a(z04), .b(x14), .c(new_n30_), .O(new_n44_));
  nand3  g15(.a(x14), .b(x08), .c(x07), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x06), .c(x05), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(new_n44_), .c(new_n42_), .O(z03));
  inv1   g19(.a(new_n41_), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(x13), .O(z05));
  nand2  g21(.a(x12), .b(new_n30_), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n41_), .O(z06));
  nand2  g25(.a(new_n41_), .b(x15), .O(z07));
  inv1   g26(.a(x00), .O(new_n56_));
  inv1   g27(.a(x10), .O(new_n57_));
  nand4  g28(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n58_));
  nand4  g29(.a(x12), .b(x11), .c(new_n57_), .d(x09), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(new_n60_));
  oai21  g31(.a(new_n60_), .b(new_n56_), .c(new_n41_), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n44_), .O(z08));
  nand4  g33(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(x09), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n41_), .O(new_n65_));
  nand3  g36(.a(x04), .b(x03), .c(x02), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  nand2  g40(.a(new_n40_), .b(new_n30_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n57_), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n41_), .O(z09));
  and2   g44(.a(new_n66_), .b(x12), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x11), .c(new_n57_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n41_), .O(z10));
  nand3  g49(.a(new_n74_), .b(x09), .c(x01), .O(new_n79_));
  oai21  g50(.a(x12), .b(x01), .c(new_n79_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n57_), .d(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n41_), .O(z11));
  nor2   g53(.a(new_n49_), .b(new_n40_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(x11), .c(new_n57_), .d(new_n39_), .O(new_n84_));
  nor2   g55(.a(new_n84_), .b(new_n56_), .O(z12));
endmodule


