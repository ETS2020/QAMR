// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_;
  inv1   g00(.a(x11), .O(new_n30_));
  oai21  g01(.a(new_n30_), .b(x10), .c(x12), .O(new_n31_));
  nor2   g02(.a(x11), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x08), .c(x07), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x11), .O(new_n37_));
  nand3  g08(.a(new_n37_), .b(x12), .c(new_n34_), .O(z01));
  nand2  g09(.a(x08), .b(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n35_), .c(x11), .O(new_n40_));
  nand2  g11(.a(x12), .b(new_n30_), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(new_n40_), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x12), .O(z02));
  inv1   g15(.a(x09), .O(new_n45_));
  nand3  g16(.a(x11), .b(x08), .c(x07), .O(new_n46_));
  oai21  g17(.a(new_n46_), .b(new_n35_), .c(new_n41_), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n45_), .c(new_n34_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(z03));
  inv1   g20(.a(x12), .O(new_n50_));
  nor2   g21(.a(new_n50_), .b(new_n34_), .O(new_n51_));
  nor2   g22(.a(new_n51_), .b(x14), .O(z04));
  nor2   g23(.a(new_n51_), .b(x13), .O(z05));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  oai22  g26(.a(new_n55_), .b(new_n51_), .c(new_n41_), .d(x10), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(x09), .c(x01), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(z06));
  nor2   g29(.a(new_n51_), .b(x15), .O(z07));
  nand3  g30(.a(x12), .b(x11), .c(x09), .O(new_n60_));
  nand2  g31(.a(new_n50_), .b(new_n30_), .O(new_n61_));
  oai21  g32(.a(new_n60_), .b(new_n54_), .c(new_n61_), .O(new_n62_));
  nand2  g33(.a(new_n41_), .b(x00), .O(new_n63_));
  aoi21  g34(.a(new_n62_), .b(x00), .c(new_n63_), .O(new_n64_));
  nand2  g35(.a(new_n50_), .b(x10), .O(new_n65_));
  oai21  g36(.a(new_n64_), .b(x10), .c(new_n65_), .O(z08));
  and2   g37(.a(x03), .b(x02), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n54_), .c(x09), .d(x04), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n61_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n34_), .c(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  nor2   g43(.a(new_n55_), .b(new_n30_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n34_), .c(x09), .d(x00), .O(new_n74_));
  aoi21  g45(.a(new_n74_), .b(new_n34_), .c(new_n50_), .O(z10));
  nand4  g46(.a(new_n54_), .b(x12), .c(x09), .d(x01), .O(new_n76_));
  oai21  g47(.a(x12), .b(x01), .c(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n34_), .d(x00), .O(new_n78_));
  inv1   g49(.a(new_n78_), .O(z11));
  nand4  g50(.a(x11), .b(new_n34_), .c(new_n45_), .d(x00), .O(new_n80_));
  nor2   g51(.a(new_n80_), .b(new_n50_), .O(z12));
endmodule


