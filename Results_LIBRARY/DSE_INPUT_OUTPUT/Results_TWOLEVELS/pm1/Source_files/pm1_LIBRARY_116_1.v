// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x01), .O(new_n34_));
  nand2  g05(.a(new_n34_), .b(new_n32_), .O(z00));
  nand3  g06(.a(new_n32_), .b(x12), .c(x11), .O(z01));
  nand3  g07(.a(x09), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n30_), .c(x11), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nand2  g10(.a(x08), .b(x07), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n30_), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x11), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  nor2   g14(.a(new_n33_), .b(x11), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n45_), .b(x13), .O(z05));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  and2   g19(.a(new_n48_), .b(x09), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n45_), .O(z06));
  nor2   g22(.a(new_n44_), .b(x15), .O(z07));
  inv1   g23(.a(x00), .O(new_n53_));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x03), .b(x02), .c(x00), .O(new_n55_));
  nand4  g26(.a(x12), .b(new_n54_), .c(x09), .d(x04), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  oai21  g28(.a(new_n57_), .b(new_n53_), .c(x11), .O(new_n58_));
  oai21  g29(.a(x12), .b(x11), .c(new_n58_), .O(z08));
  inv1   g30(.a(x11), .O(new_n60_));
  nand4  g31(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(x09), .c(new_n60_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n49_), .c(x12), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(x11), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n54_), .c(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n45_), .O(z09));
  nand4  g37(.a(new_n48_), .b(new_n54_), .c(x09), .d(x00), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(x11), .c(new_n33_), .O(z10));
  nand4  g39(.a(new_n48_), .b(x12), .c(x09), .d(x01), .O(new_n69_));
  oai21  g40(.a(x12), .b(x01), .c(new_n69_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(x11), .c(new_n54_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z11));
  inv1   g43(.a(x09), .O(new_n73_));
  nand4  g44(.a(x11), .b(new_n54_), .c(new_n73_), .d(x00), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(new_n33_), .O(z12));
endmodule


