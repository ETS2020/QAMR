// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n46_, new_n49_, new_n50_,
    new_n51_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n68_, new_n69_,
    new_n70_;
  nand2  g00(.a(x11), .b(x09), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x12), .O(new_n31_));
  nor2   g02(.a(x11), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n34_), .c(x09), .O(z01));
  nand3  g06(.a(x12), .b(x08), .c(x07), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n31_), .c(x06), .d(x05), .O(z02));
  nand4  g09(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  nand2  g10(.a(x12), .b(new_n34_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  inv1   g14(.a(x12), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(x09), .O(z12));
  inv1   g16(.a(z12), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(new_n46_), .b(x13), .O(z05));
  and2   g19(.a(x03), .b(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(new_n40_), .c(x04), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x01), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(z06));
  nand2  g23(.a(new_n46_), .b(x15), .O(z07));
  inv1   g24(.a(x10), .O(new_n54_));
  nand3  g25(.a(x12), .b(x11), .c(x04), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(new_n56_));
  aoi22  g27(.a(new_n56_), .b(new_n49_), .c(new_n44_), .d(new_n34_), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n31_), .c(new_n54_), .d(x00), .O(z08));
  nand2  g29(.a(new_n44_), .b(new_n34_), .O(new_n59_));
  nand3  g30(.a(x04), .b(x03), .c(x02), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n49_), .c(x04), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x12), .c(x11), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n59_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n54_), .c(x00), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n46_), .O(z09));
  nand4  g36(.a(new_n60_), .b(x11), .c(new_n54_), .d(x00), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(x09), .c(new_n44_), .O(z10));
  nand4  g38(.a(new_n60_), .b(x12), .c(x09), .d(x01), .O(new_n68_));
  oai21  g39(.a(x12), .b(x01), .c(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x11), .c(new_n54_), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z11));
endmodule


