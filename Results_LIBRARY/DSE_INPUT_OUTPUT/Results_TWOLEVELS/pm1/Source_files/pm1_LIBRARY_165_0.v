// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x11), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  nor2   g03(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  nand2  g04(.a(x12), .b(new_n30_), .O(new_n34_));
  oai21  g05(.a(new_n33_), .b(new_n30_), .c(new_n34_), .O(z00));
  nor2   g06(.a(new_n33_), .b(new_n30_), .O(z01));
  nand3  g07(.a(x07), .b(x06), .c(x05), .O(new_n37_));
  nand3  g08(.a(x12), .b(x09), .c(x08), .O(new_n38_));
  oai21  g09(.a(new_n38_), .b(new_n37_), .c(x11), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n34_), .O(z02));
  nand4  g11(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n41_));
  nand3  g12(.a(new_n41_), .b(x12), .c(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x11), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n34_), .O(z03));
  nor2   g15(.a(x12), .b(x11), .O(new_n45_));
  inv1   g16(.a(new_n45_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x14), .O(z04));
  nand2  g18(.a(new_n46_), .b(x13), .O(z05));
  inv1   g19(.a(x12), .O(new_n49_));
  aoi21  g20(.a(x09), .b(x01), .c(new_n49_), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  oai21  g23(.a(new_n50_), .b(x11), .c(new_n52_), .O(z06));
  nor2   g24(.a(new_n45_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand4  g26(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(new_n57_));
  nor2   g28(.a(new_n49_), .b(new_n30_), .O(new_n58_));
  nand4  g29(.a(new_n58_), .b(new_n57_), .c(new_n55_), .d(x09), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(x11), .c(new_n55_), .d(x00), .O(z08));
  inv1   g31(.a(x00), .O(new_n61_));
  and2   g32(.a(x03), .b(x02), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n51_), .c(x09), .d(x04), .O(new_n63_));
  nand4  g34(.a(new_n63_), .b(x12), .c(x11), .d(new_n55_), .O(new_n64_));
  nor2   g35(.a(new_n64_), .b(new_n61_), .O(z09));
  inv1   g36(.a(x09), .O(new_n66_));
  nand4  g37(.a(new_n51_), .b(x12), .c(x11), .d(new_n55_), .O(new_n67_));
  nor3   g38(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z10));
  inv1   g39(.a(x01), .O(new_n69_));
  nand3  g40(.a(new_n55_), .b(new_n69_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(x11), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n49_), .O(new_n72_));
  inv1   g43(.a(new_n67_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x09), .c(x01), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n72_), .O(z11));
  nand4  g46(.a(x11), .b(new_n55_), .c(new_n66_), .d(x00), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n49_), .O(z12));
endmodule


