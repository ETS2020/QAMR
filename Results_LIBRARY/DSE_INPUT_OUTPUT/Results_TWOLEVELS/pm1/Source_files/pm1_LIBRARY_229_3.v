// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_;
  inv1   g00(.a(x11), .O(new_n30_));
  oai21  g01(.a(new_n30_), .b(x10), .c(x12), .O(new_n31_));
  nor2   g02(.a(x11), .b(x01), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(new_n31_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nand3  g05(.a(x12), .b(new_n30_), .c(new_n34_), .O(z01));
  nand2  g06(.a(x12), .b(new_n30_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x09), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n34_), .O(new_n38_));
  nand3  g09(.a(x07), .b(x06), .c(x05), .O(new_n39_));
  nand3  g10(.a(x12), .b(new_n34_), .c(x08), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n38_), .O(z02));
  nand4  g13(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n37_), .c(new_n34_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(z03));
  inv1   g17(.a(x12), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n34_), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x14), .O(z04));
  nand2  g21(.a(new_n49_), .b(x13), .O(z05));
  nand3  g22(.a(new_n30_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n34_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand3  g25(.a(x04), .b(x03), .c(x02), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n54_), .O(z06));
  nor2   g28(.a(new_n48_), .b(x15), .O(z07));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  aoi21  g31(.a(new_n47_), .b(new_n30_), .c(new_n60_), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n36_), .c(new_n34_), .d(x00), .O(z08));
  nand2  g33(.a(new_n47_), .b(new_n30_), .O(new_n63_));
  and2   g34(.a(x03), .b(x02), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n55_), .c(x09), .d(x04), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(x12), .c(x11), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(new_n34_), .c(x00), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(z09));
  inv1   g40(.a(x00), .O(new_n70_));
  inv1   g41(.a(x09), .O(new_n71_));
  nand4  g42(.a(new_n55_), .b(x12), .c(x11), .d(new_n34_), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z10));
  nand4  g44(.a(new_n55_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  oai21  g45(.a(x12), .b(x01), .c(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x11), .c(new_n34_), .d(x00), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(z11));
  nand4  g48(.a(x11), .b(new_n34_), .c(new_n71_), .d(x00), .O(new_n78_));
  nor2   g49(.a(new_n78_), .b(new_n47_), .O(z12));
endmodule


