// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_,
    new_n48_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_;
  inv1   g00(.a(x08), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  and2   g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(x07), .b(x06), .c(x05), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(new_n33_), .c(x07), .O(new_n35_));
  nor2   g06(.a(new_n31_), .b(x11), .O(new_n36_));
  aoi21  g07(.a(new_n35_), .b(x11), .c(new_n36_), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n30_), .c(new_n32_), .O(z00));
  nand3  g09(.a(new_n35_), .b(x11), .c(x08), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x12), .O(z01));
  inv1   g11(.a(x11), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n30_), .c(x12), .O(new_n42_));
  nand3  g13(.a(new_n34_), .b(x11), .c(x08), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n42_), .c(x12), .d(x09), .O(z02));
  and2   g15(.a(x11), .b(x07), .O(new_n45_));
  aoi21  g16(.a(new_n45_), .b(new_n33_), .c(new_n36_), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(x12), .c(x09), .d(x08), .O(z03));
  nor2   g18(.a(new_n31_), .b(x08), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(x14), .O(z04));
  nor2   g20(.a(new_n48_), .b(x13), .O(z05));
  nand3  g21(.a(new_n41_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x08), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(z06));
  inv1   g27(.a(new_n48_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x15), .O(z07));
  inv1   g29(.a(x10), .O(new_n59_));
  nand4  g30(.a(x12), .b(x11), .c(x09), .d(x08), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n54_), .O(new_n61_));
  aoi21  g32(.a(new_n31_), .b(new_n41_), .c(new_n61_), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n42_), .c(new_n59_), .d(x00), .O(z08));
  nand2  g34(.a(new_n31_), .b(new_n41_), .O(new_n64_));
  and2   g35(.a(x03), .b(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n54_), .c(x09), .d(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x11), .d(x08), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n59_), .c(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z09));
  inv1   g41(.a(x00), .O(new_n71_));
  nand4  g42(.a(new_n54_), .b(x11), .c(new_n59_), .d(x09), .O(new_n72_));
  oai21  g43(.a(new_n72_), .b(new_n71_), .c(x08), .O(new_n73_));
  and2   g44(.a(new_n73_), .b(x12), .O(z10));
  nand4  g45(.a(new_n54_), .b(x12), .c(x09), .d(x01), .O(new_n75_));
  oai21  g46(.a(x12), .b(x01), .c(new_n75_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x11), .c(new_n59_), .d(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n57_), .O(z11));
  inv1   g49(.a(x09), .O(new_n79_));
  nor2   g50(.a(new_n30_), .b(new_n71_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n59_), .d(new_n79_), .O(new_n81_));
  aoi21  g52(.a(new_n81_), .b(x08), .c(new_n31_), .O(z12));
endmodule


