// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x00), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(new_n32_), .c(x12), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g05(.a(x06), .b(x05), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x08), .c(x07), .O(new_n36_));
  oai21  g07(.a(new_n36_), .b(new_n35_), .c(x11), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n32_), .c(x12), .O(z01));
  nand2  g09(.a(x08), .b(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n35_), .c(x11), .O(new_n40_));
  inv1   g11(.a(x09), .O(new_n41_));
  inv1   g12(.a(x12), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(x11), .O(new_n43_));
  nor2   g14(.a(new_n43_), .b(new_n41_), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n40_), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x00), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(z02));
  nand3  g18(.a(x11), .b(x08), .c(x07), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n35_), .c(x09), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x00), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n33_), .c(x12), .O(z03));
  nor2   g22(.a(new_n42_), .b(x00), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x14), .O(z04));
  nand2  g25(.a(new_n53_), .b(x13), .O(z05));
  nand3  g26(.a(new_n31_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(x00), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(x12), .O(new_n58_));
  nand3  g29(.a(x04), .b(x03), .c(x02), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(x09), .c(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n58_), .O(z06));
  nor2   g32(.a(new_n52_), .b(x15), .O(z07));
  nand3  g33(.a(x12), .b(x11), .c(x09), .O(new_n63_));
  oai22  g34(.a(new_n63_), .b(new_n59_), .c(x12), .d(x11), .O(new_n64_));
  nor3   g35(.a(new_n64_), .b(new_n43_), .c(x10), .O(new_n65_));
  nand2  g36(.a(new_n42_), .b(new_n32_), .O(new_n66_));
  oai21  g37(.a(new_n65_), .b(new_n32_), .c(new_n66_), .O(z08));
  inv1   g38(.a(x10), .O(new_n68_));
  nand2  g39(.a(new_n42_), .b(new_n31_), .O(new_n69_));
  and2   g40(.a(x03), .b(x02), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n59_), .c(x09), .d(x04), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(x12), .c(x11), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n69_), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(new_n68_), .c(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z09));
  nand4  g46(.a(new_n59_), .b(x12), .c(x11), .d(new_n68_), .O(new_n76_));
  nor3   g47(.a(new_n76_), .b(new_n41_), .c(new_n32_), .O(z10));
  nand4  g48(.a(new_n59_), .b(x12), .c(x09), .d(x01), .O(new_n78_));
  nand3  g49(.a(new_n42_), .b(new_n30_), .c(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(x11), .c(new_n68_), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n53_), .O(z11));
  nand4  g53(.a(x11), .b(new_n68_), .c(new_n41_), .d(x00), .O(new_n83_));
  aoi21  g54(.a(new_n83_), .b(x00), .c(new_n42_), .O(z12));
endmodule


