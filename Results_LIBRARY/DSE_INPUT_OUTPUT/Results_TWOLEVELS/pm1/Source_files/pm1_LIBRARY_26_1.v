// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n48_, new_n49_,
    new_n50_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  inv1   g03(.a(x14), .O(new_n33_));
  nand2  g04(.a(x15), .b(new_n33_), .O(new_n34_));
  nand4  g05(.a(new_n34_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand2  g06(.a(x12), .b(new_n31_), .O(new_n36_));
  and2   g07(.a(new_n36_), .b(new_n34_), .O(z01));
  inv1   g08(.a(new_n34_), .O(new_n38_));
  nand3  g09(.a(x07), .b(x06), .c(x05), .O(new_n39_));
  nor2   g10(.a(new_n32_), .b(new_n31_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x09), .c(x08), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nor2   g13(.a(new_n42_), .b(new_n38_), .O(z02));
  nand4  g14(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(new_n40_), .c(new_n34_), .d(x09), .O(z03));
  nor2   g16(.a(x15), .b(x14), .O(z04));
  nor2   g17(.a(new_n38_), .b(x13), .O(z05));
  and2   g18(.a(x03), .b(x02), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(new_n36_), .c(x04), .O(new_n49_));
  nand4  g20(.a(new_n49_), .b(new_n34_), .c(x09), .d(x01), .O(new_n50_));
  inv1   g21(.a(new_n50_), .O(z06));
  inv1   g22(.a(x10), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x00), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x12), .c(x11), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  oai21  g26(.a(new_n55_), .b(x15), .c(new_n34_), .O(z07));
  inv1   g27(.a(new_n53_), .O(new_n57_));
  nand3  g28(.a(x12), .b(x09), .c(x04), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  aoi21  g30(.a(new_n59_), .b(new_n48_), .c(new_n31_), .O(new_n60_));
  aoi21  g31(.a(new_n60_), .b(new_n57_), .c(new_n38_), .O(z08));
  xnor2a g32(.a(x12), .b(x11), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n34_), .c(new_n52_), .d(x00), .O(new_n63_));
  inv1   g34(.a(new_n63_), .O(z09));
  nand3  g35(.a(x04), .b(x03), .c(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(x12), .c(x11), .d(new_n52_), .O(new_n66_));
  inv1   g37(.a(new_n66_), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x09), .c(x00), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n34_), .O(z10));
  inv1   g40(.a(x00), .O(new_n70_));
  nand4  g41(.a(new_n65_), .b(x12), .c(x09), .d(x01), .O(new_n71_));
  nand2  g42(.a(new_n32_), .b(new_n30_), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n34_), .c(x11), .d(new_n52_), .O(new_n74_));
  nor2   g45(.a(new_n74_), .b(new_n70_), .O(z11));
  nand4  g46(.a(new_n34_), .b(x12), .c(x11), .d(new_n52_), .O(new_n76_));
  nor3   g47(.a(new_n76_), .b(x09), .c(new_n70_), .O(z12));
endmodule


