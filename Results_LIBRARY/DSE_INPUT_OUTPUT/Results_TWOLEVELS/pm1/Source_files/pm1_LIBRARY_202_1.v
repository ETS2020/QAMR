// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  oai21  g05(.a(new_n30_), .b(x11), .c(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x09), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n31_), .O(z00));
  nand3  g08(.a(new_n34_), .b(x12), .c(x09), .O(z01));
  inv1   g09(.a(x09), .O(new_n39_));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nor2   g11(.a(new_n30_), .b(x11), .O(new_n41_));
  aoi21  g12(.a(new_n40_), .b(x11), .c(new_n41_), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(new_n39_), .c(x12), .O(z02));
  nand3  g14(.a(x11), .b(x08), .c(x07), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(new_n32_), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n41_), .c(x09), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(z03));
  nor2   g18(.a(new_n30_), .b(x09), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x14), .O(z04));
  nor2   g21(.a(new_n48_), .b(x13), .O(z05));
  inv1   g22(.a(x11), .O(new_n52_));
  aoi21  g23(.a(new_n52_), .b(x01), .c(new_n39_), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  oai21  g26(.a(new_n53_), .b(new_n30_), .c(new_n55_), .O(z06));
  nand2  g27(.a(new_n49_), .b(x15), .O(z07));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n54_), .O(new_n60_));
  aoi21  g31(.a(new_n30_), .b(new_n52_), .c(new_n60_), .O(new_n61_));
  oai21  g32(.a(new_n52_), .b(new_n39_), .c(x12), .O(new_n62_));
  nand4  g33(.a(new_n62_), .b(new_n61_), .c(new_n58_), .d(x00), .O(z08));
  nand2  g34(.a(new_n30_), .b(new_n52_), .O(new_n64_));
  and2   g35(.a(x03), .b(x02), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n54_), .c(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x11), .d(x09), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(new_n58_), .c(x00), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(z09));
  inv1   g41(.a(x00), .O(new_n71_));
  nand4  g42(.a(new_n54_), .b(x12), .c(x11), .d(new_n58_), .O(new_n72_));
  nor3   g43(.a(new_n72_), .b(new_n39_), .c(new_n71_), .O(z10));
  nand3  g44(.a(new_n54_), .b(x12), .c(x01), .O(new_n74_));
  oai21  g45(.a(x12), .b(x01), .c(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x11), .c(new_n58_), .d(x00), .O(new_n76_));
  nand2  g47(.a(new_n76_), .b(new_n49_), .O(z11));
  zero   g48(.O(z12));
endmodule


