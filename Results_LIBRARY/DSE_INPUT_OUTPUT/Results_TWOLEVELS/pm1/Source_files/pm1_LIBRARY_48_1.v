// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x00), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x12), .O(new_n32_));
  nor2   g03(.a(x11), .b(x01), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(new_n32_), .O(z00));
  nand3  g05(.a(x12), .b(new_n31_), .c(x00), .O(z01));
  inv1   g06(.a(x12), .O(new_n36_));
  nor2   g07(.a(new_n36_), .b(x00), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(x12), .b(new_n31_), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(x09), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x00), .O(new_n41_));
  and2   g12(.a(x06), .b(x05), .O(new_n42_));
  nand3  g13(.a(x12), .b(x08), .c(x07), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(new_n42_), .c(new_n41_), .d(new_n38_), .O(z02));
  nand3  g16(.a(new_n42_), .b(x08), .c(x07), .O(new_n46_));
  inv1   g17(.a(new_n46_), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n40_), .c(x00), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x12), .O(z03));
  nor2   g20(.a(new_n37_), .b(x14), .O(z04));
  nor2   g21(.a(new_n37_), .b(x13), .O(z05));
  nand3  g22(.a(x04), .b(x03), .c(x02), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(new_n53_));
  oai22  g24(.a(new_n53_), .b(new_n37_), .c(new_n39_), .d(new_n30_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  inv1   g26(.a(new_n55_), .O(z06));
  nand2  g27(.a(new_n38_), .b(x15), .O(z07));
  inv1   g28(.a(x10), .O(new_n58_));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  nand2  g30(.a(new_n36_), .b(new_n31_), .O(new_n60_));
  oai21  g31(.a(new_n59_), .b(new_n52_), .c(new_n60_), .O(new_n61_));
  nand2  g32(.a(new_n39_), .b(new_n58_), .O(new_n62_));
  aoi21  g33(.a(new_n61_), .b(new_n58_), .c(new_n62_), .O(new_n63_));
  nand2  g34(.a(new_n36_), .b(new_n30_), .O(new_n64_));
  oai21  g35(.a(new_n63_), .b(new_n30_), .c(new_n64_), .O(z08));
  and2   g36(.a(x03), .b(x02), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(new_n52_), .c(x09), .d(x04), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n60_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n58_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n38_), .O(z09));
  inv1   g42(.a(x09), .O(new_n72_));
  nand4  g43(.a(new_n52_), .b(x12), .c(x11), .d(new_n58_), .O(new_n73_));
  nor3   g44(.a(new_n73_), .b(new_n72_), .c(new_n30_), .O(z10));
  nand4  g45(.a(new_n52_), .b(x12), .c(x09), .d(x01), .O(new_n75_));
  oai21  g46(.a(x12), .b(x01), .c(new_n75_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x11), .c(new_n58_), .d(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z11));
  nand4  g49(.a(x11), .b(new_n58_), .c(new_n72_), .d(x00), .O(new_n79_));
  nor2   g50(.a(new_n79_), .b(new_n36_), .O(z12));
endmodule


