// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:11 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n50_, new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x15), .O(new_n31_));
  nor2   g02(.a(x12), .b(x11), .O(new_n32_));
  aoi22  g03(.a(new_n32_), .b(new_n30_), .c(new_n31_), .d(x04), .O(z00));
  inv1   g04(.a(x11), .O(new_n34_));
  inv1   g05(.a(x04), .O(new_n35_));
  nor2   g06(.a(x15), .b(new_n35_), .O(new_n36_));
  aoi21  g07(.a(x12), .b(new_n34_), .c(new_n36_), .O(z01));
  nand2  g08(.a(new_n31_), .b(x04), .O(new_n38_));
  inv1   g09(.a(new_n38_), .O(new_n39_));
  nand2  g10(.a(x06), .b(x05), .O(new_n40_));
  nand3  g11(.a(x09), .b(x08), .c(x07), .O(new_n41_));
  nor2   g12(.a(new_n41_), .b(new_n40_), .O(new_n42_));
  nand2  g13(.a(x12), .b(x11), .O(new_n43_));
  oai21  g14(.a(x15), .b(new_n35_), .c(new_n43_), .O(new_n44_));
  oai21  g15(.a(new_n42_), .b(new_n39_), .c(new_n44_), .O(z02));
  nand4  g16(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(new_n44_), .c(new_n38_), .d(x09), .O(z03));
  nand2  g18(.a(new_n38_), .b(x14), .O(z04));
  nor2   g19(.a(new_n39_), .b(x13), .O(z05));
  nand2  g20(.a(x12), .b(new_n34_), .O(new_n50_));
  nand4  g21(.a(new_n50_), .b(x04), .c(x03), .d(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n38_), .O(z06));
  oai21  g24(.a(new_n43_), .b(x11), .c(new_n31_), .O(new_n54_));
  nor2   g25(.a(new_n54_), .b(x04), .O(z07));
  inv1   g26(.a(x10), .O(new_n56_));
  nand4  g27(.a(x09), .b(x03), .c(x02), .d(x00), .O(new_n57_));
  nand4  g28(.a(x15), .b(x12), .c(x11), .d(new_n56_), .O(new_n58_));
  oai21  g29(.a(new_n58_), .b(new_n57_), .c(x15), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x04), .O(new_n60_));
  inv1   g31(.a(x12), .O(new_n61_));
  nand4  g32(.a(new_n38_), .b(new_n61_), .c(new_n56_), .d(x00), .O(new_n62_));
  oai21  g33(.a(new_n36_), .b(new_n61_), .c(new_n62_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n34_), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n60_), .c(new_n56_), .d(x00), .O(z08));
  inv1   g36(.a(x09), .O(new_n66_));
  nand3  g37(.a(x12), .b(x11), .c(new_n66_), .O(new_n67_));
  oai21  g38(.a(x12), .b(x11), .c(new_n67_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n38_), .O(new_n69_));
  nand3  g40(.a(new_n35_), .b(x03), .c(x02), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(x15), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(x04), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x12), .c(x11), .d(x09), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n69_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(new_n56_), .c(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z09));
  inv1   g47(.a(x00), .O(new_n77_));
  nand2  g48(.a(x03), .b(x02), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(x15), .O(new_n79_));
  aoi21  g50(.a(new_n79_), .b(x04), .c(new_n61_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(x11), .c(new_n56_), .d(x09), .O(new_n81_));
  nor2   g52(.a(new_n81_), .b(new_n77_), .O(z10));
  nand3  g53(.a(new_n38_), .b(new_n61_), .c(new_n30_), .O(new_n83_));
  nand3  g54(.a(new_n80_), .b(x09), .c(x01), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(x11), .c(new_n56_), .d(x00), .O(new_n86_));
  inv1   g57(.a(new_n86_), .O(z11));
  nor2   g58(.a(new_n39_), .b(new_n61_), .O(new_n88_));
  nand4  g59(.a(new_n88_), .b(x11), .c(new_n56_), .d(new_n66_), .O(new_n89_));
  nor2   g60(.a(new_n89_), .b(new_n77_), .O(z12));
endmodule


