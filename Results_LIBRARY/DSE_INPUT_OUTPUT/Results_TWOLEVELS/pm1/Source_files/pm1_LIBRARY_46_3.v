// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:38 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n48_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(new_n30_), .O(new_n31_));
  oai21  g02(.a(new_n30_), .b(x08), .c(new_n31_), .O(z00));
  inv1   g03(.a(x08), .O(new_n33_));
  inv1   g04(.a(x10), .O(new_n34_));
  nand2  g05(.a(x03), .b(x02), .O(new_n35_));
  nand3  g06(.a(x04), .b(x03), .c(x02), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x04), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n35_), .c(x09), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x12), .O(new_n42_));
  nand3  g13(.a(new_n42_), .b(new_n34_), .c(x00), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(x11), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x12), .c(new_n33_), .O(z01));
  nand2  g16(.a(x12), .b(x08), .O(z02));
  nand4  g17(.a(x12), .b(x11), .c(x09), .d(new_n33_), .O(z03));
  nand2  g18(.a(x12), .b(x08), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x14), .O(z04));
  nand2  g20(.a(new_n48_), .b(x13), .O(z05));
  inv1   g21(.a(x11), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n33_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x12), .O(new_n54_));
  nand3  g25(.a(new_n36_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n54_), .O(z06));
  nand2  g27(.a(new_n48_), .b(x15), .O(z07));
  nand3  g28(.a(x03), .b(x02), .c(x00), .O(new_n58_));
  nand4  g29(.a(x11), .b(new_n34_), .c(x09), .d(x04), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n58_), .c(new_n33_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x12), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(x11), .c(new_n34_), .d(x00), .O(z08));
  nand3  g33(.a(new_n41_), .b(x12), .c(x11), .O(new_n63_));
  nand2  g34(.a(new_n30_), .b(new_n51_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand3  g36(.a(new_n65_), .b(new_n34_), .c(x00), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n48_), .O(z09));
  nand3  g38(.a(new_n36_), .b(x11), .c(new_n34_), .O(new_n68_));
  inv1   g39(.a(new_n68_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(x09), .c(x00), .O(new_n70_));
  aoi21  g41(.a(new_n70_), .b(new_n33_), .c(new_n30_), .O(z10));
  nand4  g42(.a(new_n36_), .b(x12), .c(x09), .d(x01), .O(new_n72_));
  oai21  g43(.a(x12), .b(x01), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n34_), .d(x00), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n48_), .O(z11));
  nand4  g46(.a(new_n34_), .b(new_n39_), .c(new_n33_), .d(x00), .O(new_n76_));
  nor3   g47(.a(new_n76_), .b(new_n30_), .c(new_n51_), .O(z12));
endmodule


