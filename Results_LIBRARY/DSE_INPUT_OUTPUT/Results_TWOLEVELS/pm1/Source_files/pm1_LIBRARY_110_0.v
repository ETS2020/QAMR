// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_;
  nor2   g00(.a(x15), .b(x13), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x12), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x11), .O(new_n35_));
  nor2   g06(.a(new_n35_), .b(x01), .O(new_n36_));
  aoi21  g07(.a(new_n36_), .b(new_n33_), .c(new_n30_), .O(z00));
  aoi21  g08(.a(new_n33_), .b(x12), .c(new_n30_), .O(z01));
  nand2  g09(.a(x08), .b(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n31_), .c(x11), .O(new_n40_));
  inv1   g11(.a(x11), .O(new_n41_));
  nor2   g12(.a(new_n34_), .b(new_n41_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x09), .O(new_n43_));
  inv1   g14(.a(new_n43_), .O(new_n44_));
  aoi21  g15(.a(new_n44_), .b(new_n40_), .c(new_n30_), .O(z02));
  inv1   g16(.a(x09), .O(new_n46_));
  nand3  g17(.a(x11), .b(x08), .c(x07), .O(new_n47_));
  nor2   g18(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nor2   g19(.a(new_n48_), .b(new_n46_), .O(new_n49_));
  aoi21  g20(.a(new_n49_), .b(new_n42_), .c(new_n30_), .O(z03));
  nor2   g21(.a(new_n30_), .b(x14), .O(z04));
  inv1   g22(.a(x13), .O(z05));
  inv1   g23(.a(new_n30_), .O(new_n53_));
  inv1   g24(.a(new_n35_), .O(new_n54_));
  and2   g25(.a(x03), .b(x02), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(new_n54_), .c(x04), .O(new_n56_));
  nand4  g27(.a(new_n56_), .b(new_n53_), .c(x09), .d(x01), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(z06));
  nor2   g29(.a(x15), .b(z05), .O(z07));
  inv1   g30(.a(x10), .O(new_n60_));
  nand3  g31(.a(x04), .b(x03), .c(x02), .O(new_n61_));
  nand3  g32(.a(x12), .b(x11), .c(x09), .O(new_n62_));
  nand2  g33(.a(new_n34_), .b(new_n41_), .O(new_n63_));
  oai21  g34(.a(new_n62_), .b(new_n61_), .c(new_n63_), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n60_), .c(x00), .O(new_n65_));
  inv1   g36(.a(x00), .O(new_n66_));
  nor3   g37(.a(new_n35_), .b(x10), .c(new_n66_), .O(new_n67_));
  aoi21  g38(.a(new_n67_), .b(new_n65_), .c(new_n30_), .O(z08));
  nand4  g39(.a(new_n61_), .b(new_n55_), .c(x09), .d(x04), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n63_), .O(new_n71_));
  nand4  g42(.a(new_n71_), .b(new_n53_), .c(new_n60_), .d(x00), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(z09));
  nand4  g44(.a(new_n61_), .b(new_n53_), .c(x12), .d(x11), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(new_n60_), .c(x09), .d(x00), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(z10));
  nand4  g48(.a(new_n61_), .b(x12), .c(x09), .d(x01), .O(new_n78_));
  oai21  g49(.a(x12), .b(x01), .c(new_n78_), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(x11), .c(new_n60_), .d(x00), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n53_), .O(z11));
  nor2   g52(.a(new_n30_), .b(new_n34_), .O(new_n82_));
  nand4  g53(.a(new_n82_), .b(x11), .c(new_n60_), .d(new_n46_), .O(new_n83_));
  nor2   g54(.a(new_n83_), .b(new_n66_), .O(z12));
endmodule


