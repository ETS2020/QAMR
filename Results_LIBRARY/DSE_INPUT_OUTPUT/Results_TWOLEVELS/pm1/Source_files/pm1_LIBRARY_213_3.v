// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(new_n31_), .b(x08), .c(x07), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x11), .O(new_n33_));
  inv1   g04(.a(x11), .O(new_n34_));
  nand2  g05(.a(x12), .b(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  oai21  g08(.a(x14), .b(new_n30_), .c(new_n37_), .O(z00));
  nand2  g09(.a(x14), .b(x01), .O(new_n39_));
  inv1   g10(.a(new_n39_), .O(new_n40_));
  aoi21  g11(.a(new_n33_), .b(x12), .c(new_n40_), .O(z01));
  nand4  g12(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n42_));
  nand2  g13(.a(x12), .b(x09), .O(new_n43_));
  aoi21  g14(.a(new_n42_), .b(x11), .c(new_n43_), .O(new_n44_));
  inv1   g15(.a(x09), .O(new_n45_));
  oai21  g16(.a(x14), .b(new_n45_), .c(x01), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(x12), .c(new_n34_), .O(new_n47_));
  oai21  g18(.a(new_n44_), .b(new_n40_), .c(new_n47_), .O(z02));
  nand3  g19(.a(x11), .b(x08), .c(x07), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n31_), .c(x12), .O(new_n50_));
  oai21  g21(.a(new_n50_), .b(new_n45_), .c(new_n39_), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n47_), .O(z03));
  aoi21  g23(.a(new_n33_), .b(x11), .c(x14), .O(z04));
  nand2  g24(.a(new_n39_), .b(x13), .O(z05));
  inv1   g25(.a(x14), .O(new_n55_));
  and2   g26(.a(x03), .b(x02), .O(new_n56_));
  nand3  g27(.a(new_n56_), .b(new_n35_), .c(x04), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(new_n55_), .c(x09), .d(x01), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(z06));
  nor2   g30(.a(new_n40_), .b(x15), .O(z07));
  inv1   g31(.a(x10), .O(new_n61_));
  nand3  g32(.a(x04), .b(x03), .c(x02), .O(new_n62_));
  nand3  g33(.a(x12), .b(x11), .c(x09), .O(new_n63_));
  inv1   g34(.a(x12), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(new_n34_), .O(new_n65_));
  oai21  g36(.a(new_n63_), .b(new_n62_), .c(new_n65_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  oai21  g38(.a(new_n64_), .b(x01), .c(x14), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n34_), .O(new_n69_));
  nor2   g40(.a(new_n40_), .b(x10), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n69_), .c(new_n67_), .d(x00), .O(z08));
  nand4  g42(.a(new_n62_), .b(new_n56_), .c(x09), .d(x04), .O(new_n72_));
  nand3  g43(.a(new_n72_), .b(x12), .c(x11), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n65_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(new_n61_), .c(x00), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n39_), .O(z09));
  nand4  g47(.a(new_n62_), .b(x12), .c(x11), .d(new_n61_), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(x09), .c(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n39_), .O(z10));
  nand4  g51(.a(new_n62_), .b(new_n55_), .c(x12), .d(x09), .O(new_n81_));
  nand2  g52(.a(new_n64_), .b(new_n30_), .O(new_n82_));
  oai21  g53(.a(new_n81_), .b(new_n30_), .c(new_n82_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(x11), .c(new_n61_), .d(x00), .O(new_n84_));
  inv1   g55(.a(new_n84_), .O(z11));
  nand2  g56(.a(new_n45_), .b(x00), .O(new_n86_));
  nand3  g57(.a(x12), .b(x11), .c(new_n61_), .O(new_n87_));
  oai21  g58(.a(new_n87_), .b(new_n86_), .c(new_n39_), .O(z12));
endmodule


