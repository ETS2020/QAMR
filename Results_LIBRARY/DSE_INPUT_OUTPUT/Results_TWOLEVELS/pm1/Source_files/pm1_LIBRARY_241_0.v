// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_;
  inv1   g00(.a(x15), .O(z07));
  inv1   g01(.a(x11), .O(new_n31_));
  aoi21  g02(.a(x12), .b(new_n31_), .c(x01), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  oai21  g06(.a(new_n32_), .b(z07), .c(new_n35_), .O(z00));
  aoi21  g07(.a(x15), .b(x12), .c(x11), .O(new_n37_));
  inv1   g08(.a(new_n37_), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(new_n35_), .O(z01));
  nand2  g10(.a(x15), .b(new_n31_), .O(new_n40_));
  nand3  g11(.a(x07), .b(x06), .c(x05), .O(new_n41_));
  nand3  g12(.a(x12), .b(x09), .c(x08), .O(new_n42_));
  oai21  g13(.a(new_n42_), .b(new_n41_), .c(x11), .O(new_n43_));
  nand2  g14(.a(new_n43_), .b(new_n40_), .O(z02));
  inv1   g15(.a(x12), .O(new_n45_));
  oai21  g16(.a(x15), .b(x11), .c(new_n45_), .O(new_n46_));
  nand2  g17(.a(x08), .b(x07), .O(new_n47_));
  oai21  g18(.a(new_n47_), .b(new_n33_), .c(x09), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x11), .O(new_n49_));
  oai21  g20(.a(z07), .b(x12), .c(new_n31_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(new_n49_), .c(new_n46_), .O(z03));
  aoi21  g22(.a(z07), .b(new_n31_), .c(x14), .O(z04));
  nand2  g23(.a(z07), .b(new_n31_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x13), .O(z05));
  nand3  g25(.a(x12), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(x15), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n31_), .O(new_n57_));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x09), .c(x01), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(z06));
  nand3  g31(.a(x03), .b(x02), .c(x00), .O(new_n61_));
  inv1   g32(.a(x10), .O(new_n62_));
  nand4  g33(.a(x11), .b(new_n62_), .c(x09), .d(x04), .O(new_n63_));
  oai21  g34(.a(new_n63_), .b(new_n61_), .c(new_n40_), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x12), .O(new_n65_));
  inv1   g36(.a(x00), .O(new_n66_));
  nor3   g37(.a(new_n37_), .b(x10), .c(new_n66_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(z08));
  nand3  g39(.a(new_n45_), .b(new_n62_), .c(x00), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(x15), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n31_), .O(new_n71_));
  and2   g42(.a(x03), .b(x02), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n58_), .c(x09), .d(x04), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x12), .c(x11), .d(new_n62_), .O(new_n74_));
  oai21  g45(.a(new_n74_), .b(new_n66_), .c(new_n71_), .O(z09));
  inv1   g46(.a(x09), .O(new_n76_));
  nand4  g47(.a(new_n58_), .b(x12), .c(x11), .d(new_n62_), .O(new_n77_));
  nor3   g48(.a(new_n77_), .b(new_n76_), .c(new_n66_), .O(z10));
  nand4  g49(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n79_));
  inv1   g50(.a(x01), .O(new_n80_));
  nand2  g51(.a(new_n45_), .b(new_n80_), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand4  g53(.a(new_n82_), .b(x11), .c(new_n62_), .d(x00), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n53_), .O(z11));
  nand2  g55(.a(new_n76_), .b(x00), .O(new_n85_));
  nand3  g56(.a(x12), .b(x11), .c(new_n62_), .O(new_n86_));
  oai21  g57(.a(new_n86_), .b(new_n85_), .c(new_n53_), .O(z12));
endmodule


