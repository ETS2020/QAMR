// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_;
  inv1   g00(.a(x12), .O(new_n30_));
  oai21  g01(.a(x11), .b(x01), .c(x14), .O(new_n31_));
  inv1   g02(.a(x11), .O(new_n32_));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  nor3   g05(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  oai21  g06(.a(new_n35_), .b(new_n30_), .c(new_n31_), .O(z00));
  inv1   g07(.a(x14), .O(z04));
  oai21  g08(.a(new_n34_), .b(new_n33_), .c(x12), .O(new_n38_));
  oai22  g09(.a(new_n38_), .b(new_n32_), .c(z04), .d(x12), .O(z01));
  inv1   g10(.a(new_n33_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x08), .c(x07), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(x12), .c(x11), .d(x09), .O(z02));
  nand4  g14(.a(new_n40_), .b(x11), .c(x08), .d(x07), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(x12), .c(x11), .d(x09), .O(z03));
  oai21  g16(.a(x14), .b(x12), .c(x13), .O(z05));
  nor2   g17(.a(x14), .b(x12), .O(new_n47_));
  nand3  g18(.a(x04), .b(x03), .c(x02), .O(new_n48_));
  inv1   g19(.a(new_n48_), .O(new_n49_));
  nand2  g20(.a(x12), .b(new_n32_), .O(new_n50_));
  oai21  g21(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  inv1   g23(.a(new_n52_), .O(z06));
  nor2   g24(.a(new_n47_), .b(x15), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(x12), .b(x11), .c(x09), .O(new_n56_));
  oai22  g27(.a(new_n56_), .b(new_n48_), .c(x12), .d(x11), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nor2   g29(.a(new_n47_), .b(x10), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n58_), .c(new_n50_), .d(x00), .O(z08));
  inv1   g31(.a(x00), .O(new_n61_));
  nand3  g32(.a(new_n32_), .b(new_n55_), .c(x00), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(x14), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n30_), .O(new_n64_));
  and2   g35(.a(x03), .b(x02), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n48_), .c(x09), .d(x04), .O(new_n66_));
  nand4  g37(.a(new_n66_), .b(x12), .c(x11), .d(new_n55_), .O(new_n67_));
  oai21  g38(.a(new_n67_), .b(new_n61_), .c(new_n64_), .O(z09));
  inv1   g39(.a(x09), .O(new_n69_));
  nand4  g40(.a(new_n48_), .b(x12), .c(x11), .d(new_n55_), .O(new_n70_));
  nor3   g41(.a(new_n70_), .b(new_n69_), .c(new_n61_), .O(z10));
  inv1   g42(.a(x01), .O(new_n72_));
  nand4  g43(.a(x11), .b(new_n55_), .c(new_n72_), .d(x00), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(x14), .O(new_n74_));
  nand2  g45(.a(new_n74_), .b(new_n30_), .O(new_n75_));
  inv1   g46(.a(new_n70_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x09), .c(x01), .d(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n75_), .O(z11));
  nand4  g49(.a(x11), .b(new_n55_), .c(new_n69_), .d(x00), .O(new_n79_));
  nor2   g50(.a(new_n79_), .b(new_n30_), .O(z12));
endmodule


