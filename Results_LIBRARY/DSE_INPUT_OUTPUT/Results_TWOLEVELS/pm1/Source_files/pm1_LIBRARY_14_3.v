// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  inv1   g04(.a(x15), .O(new_n34_));
  nand3  g05(.a(new_n34_), .b(x12), .c(new_n31_), .O(z01));
  inv1   g06(.a(x08), .O(new_n36_));
  oai21  g07(.a(new_n32_), .b(x11), .c(x09), .O(new_n37_));
  nor2   g08(.a(new_n37_), .b(new_n36_), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(x07), .c(x06), .d(x05), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n34_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(x12), .O(z02));
  nand2  g12(.a(x06), .b(x05), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(x08), .c(x07), .O(new_n44_));
  inv1   g15(.a(new_n44_), .O(new_n45_));
  oai21  g16(.a(new_n45_), .b(new_n37_), .c(new_n34_), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(x12), .O(z03));
  nand2  g18(.a(x15), .b(x12), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x14), .O(z04));
  nand2  g20(.a(new_n48_), .b(x13), .O(z05));
  nand3  g21(.a(new_n31_), .b(x09), .c(x01), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(new_n34_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(x12), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(x09), .c(x01), .O(new_n55_));
  nand2  g26(.a(new_n55_), .b(new_n53_), .O(z06));
  nand3  g27(.a(new_n42_), .b(x08), .c(x07), .O(new_n57_));
  inv1   g28(.a(new_n57_), .O(new_n58_));
  aoi21  g29(.a(new_n58_), .b(new_n43_), .c(x15), .O(z07));
  inv1   g30(.a(x10), .O(new_n60_));
  oai21  g31(.a(x15), .b(new_n31_), .c(x12), .O(new_n61_));
  nand4  g32(.a(new_n34_), .b(x12), .c(x11), .d(x09), .O(new_n62_));
  nor2   g33(.a(new_n62_), .b(new_n54_), .O(new_n63_));
  aoi21  g34(.a(new_n32_), .b(new_n31_), .c(new_n63_), .O(new_n64_));
  nand4  g35(.a(new_n64_), .b(new_n61_), .c(new_n60_), .d(x00), .O(z08));
  nand2  g36(.a(new_n32_), .b(new_n31_), .O(new_n66_));
  and2   g37(.a(x03), .b(x02), .O(new_n67_));
  nand4  g38(.a(new_n67_), .b(new_n54_), .c(x09), .d(x04), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n34_), .c(x12), .d(x11), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n66_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n60_), .c(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  nand4  g43(.a(new_n54_), .b(new_n34_), .c(x11), .d(new_n60_), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x09), .c(x00), .O(new_n75_));
  aoi21  g46(.a(new_n75_), .b(new_n34_), .c(new_n32_), .O(z10));
  nand4  g47(.a(new_n54_), .b(new_n34_), .c(x12), .d(x09), .O(new_n77_));
  nand2  g48(.a(new_n32_), .b(new_n30_), .O(new_n78_));
  oai21  g49(.a(new_n77_), .b(new_n30_), .c(new_n78_), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(x11), .c(new_n60_), .d(x00), .O(new_n80_));
  inv1   g51(.a(new_n80_), .O(z11));
  inv1   g52(.a(x09), .O(new_n82_));
  nand4  g53(.a(x11), .b(new_n60_), .c(new_n82_), .d(x00), .O(new_n83_));
  nor3   g54(.a(new_n83_), .b(x15), .c(new_n32_), .O(z12));
endmodule


