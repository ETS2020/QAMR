// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:52 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n58_, new_n59_, new_n60_, new_n61_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x06), .b(x05), .O(new_n31_));
  nand3  g02(.a(x12), .b(x11), .c(x08), .O(new_n32_));
  oai21  g03(.a(new_n32_), .b(new_n31_), .c(x12), .O(new_n33_));
  nand2  g04(.a(new_n33_), .b(x07), .O(new_n34_));
  nand3  g05(.a(x08), .b(x06), .c(x05), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(x12), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(x07), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(x11), .O(new_n38_));
  inv1   g09(.a(x11), .O(new_n39_));
  nand2  g10(.a(x12), .b(new_n39_), .O(new_n40_));
  nand4  g11(.a(new_n40_), .b(new_n38_), .c(new_n34_), .d(new_n30_), .O(z00));
  inv1   g12(.a(x07), .O(new_n42_));
  nand2  g13(.a(new_n40_), .b(new_n42_), .O(new_n43_));
  nand4  g14(.a(x08), .b(new_n42_), .c(x06), .d(x05), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x12), .c(x11), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n43_), .O(z01));
  inv1   g17(.a(x12), .O(new_n47_));
  inv1   g18(.a(x09), .O(new_n48_));
  nor3   g19(.a(new_n35_), .b(new_n39_), .c(new_n48_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n47_), .c(new_n43_), .O(z02));
  nand3  g21(.a(x11), .b(x08), .c(x07), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n31_), .c(x11), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n48_), .c(x12), .O(new_n53_));
  oai21  g24(.a(x12), .b(x07), .c(new_n53_), .O(z03));
  nor2   g25(.a(x12), .b(new_n42_), .O(new_n55_));
  nor2   g26(.a(new_n55_), .b(x14), .O(z04));
  nor2   g27(.a(new_n55_), .b(x13), .O(z05));
  inv1   g28(.a(new_n55_), .O(new_n58_));
  and2   g29(.a(x03), .b(x02), .O(new_n59_));
  nand3  g30(.a(new_n59_), .b(new_n40_), .c(x04), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(x09), .c(x01), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n58_), .O(z06));
  nor2   g33(.a(new_n55_), .b(x15), .O(z07));
  inv1   g34(.a(x10), .O(new_n64_));
  nand3  g35(.a(x04), .b(x03), .c(x02), .O(new_n65_));
  nand3  g36(.a(x12), .b(x11), .c(x09), .O(new_n66_));
  oai22  g37(.a(new_n66_), .b(new_n65_), .c(x12), .d(x11), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nor2   g39(.a(new_n55_), .b(x10), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n68_), .c(new_n40_), .d(x00), .O(z08));
  inv1   g41(.a(x00), .O(new_n71_));
  nand3  g42(.a(new_n39_), .b(new_n64_), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n42_), .O(new_n73_));
  nand2  g44(.a(new_n73_), .b(new_n47_), .O(new_n74_));
  nand4  g45(.a(new_n65_), .b(new_n59_), .c(x09), .d(x04), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(x12), .c(x11), .d(new_n64_), .O(new_n76_));
  oai21  g47(.a(new_n76_), .b(new_n71_), .c(new_n74_), .O(z09));
  nand4  g48(.a(new_n65_), .b(x12), .c(x11), .d(new_n64_), .O(new_n78_));
  nor2   g49(.a(new_n78_), .b(new_n48_), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(x00), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n58_), .O(z10));
  nand4  g52(.a(x11), .b(new_n64_), .c(new_n30_), .d(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n42_), .O(new_n83_));
  nand2  g54(.a(new_n83_), .b(new_n47_), .O(new_n84_));
  nand3  g55(.a(new_n79_), .b(x01), .c(x00), .O(new_n85_));
  nand2  g56(.a(new_n85_), .b(new_n84_), .O(z11));
  nand4  g57(.a(x11), .b(new_n64_), .c(new_n48_), .d(x00), .O(new_n87_));
  nor2   g58(.a(new_n87_), .b(new_n47_), .O(z12));
endmodule


