// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n33_, new_n34_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n81_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  oai21  g02(.a(x12), .b(new_n31_), .c(new_n30_), .O(z00));
  nand2  g03(.a(x06), .b(x05), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(x08), .c(x07), .O(new_n34_));
  oai21  g05(.a(new_n34_), .b(new_n33_), .c(x11), .O(new_n35_));
  nand3  g06(.a(new_n35_), .b(x12), .c(x01), .O(z01));
  nand3  g07(.a(x12), .b(x08), .c(x07), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n33_), .c(x11), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  inv1   g10(.a(x12), .O(new_n40_));
  nor2   g11(.a(new_n40_), .b(x01), .O(new_n41_));
  nor3   g12(.a(new_n41_), .b(new_n31_), .c(new_n39_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n38_), .O(z02));
  nand2  g14(.a(x08), .b(x07), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n33_), .c(x12), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x11), .O(new_n46_));
  nand2  g17(.a(new_n46_), .b(new_n42_), .O(z03));
  inv1   g18(.a(new_n41_), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x14), .O(z04));
  nand2  g20(.a(new_n48_), .b(x13), .O(z05));
  and2   g21(.a(x03), .b(x02), .O(new_n51_));
  nand2  g22(.a(x12), .b(new_n31_), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(new_n51_), .c(x04), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(z06));
  nand2  g26(.a(new_n48_), .b(x15), .O(z07));
  nand2  g27(.a(new_n51_), .b(x00), .O(new_n57_));
  inv1   g28(.a(x10), .O(new_n58_));
  nand4  g29(.a(x11), .b(new_n58_), .c(x09), .d(x04), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(new_n57_), .c(x01), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x12), .O(new_n61_));
  nand3  g32(.a(x11), .b(new_n58_), .c(x00), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n61_), .O(z08));
  nand4  g35(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x01), .O(new_n66_));
  nand4  g37(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  nand2  g40(.a(new_n40_), .b(new_n31_), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n58_), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n48_), .O(z09));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(x12), .c(x11), .d(new_n58_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand4  g47(.a(new_n76_), .b(x09), .c(x01), .d(x00), .O(new_n77_));
  inv1   g48(.a(new_n77_), .O(z10));
  oai21  g49(.a(new_n63_), .b(x12), .c(new_n30_), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n77_), .O(z11));
  nand4  g51(.a(new_n58_), .b(new_n39_), .c(x01), .d(x00), .O(new_n81_));
  nor3   g52(.a(new_n81_), .b(new_n40_), .c(new_n31_), .O(z12));
endmodule


