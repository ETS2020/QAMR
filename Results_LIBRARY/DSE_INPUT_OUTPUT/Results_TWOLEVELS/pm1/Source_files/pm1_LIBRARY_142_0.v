// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n36_, new_n37_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n49_, new_n52_, new_n53_, new_n54_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x13), .O(new_n31_));
  inv1   g02(.a(x14), .O(new_n32_));
  nor2   g03(.a(x12), .b(x11), .O(new_n33_));
  aoi22  g04(.a(new_n33_), .b(new_n30_), .c(new_n32_), .d(new_n31_), .O(z00));
  nand2  g05(.a(new_n32_), .b(new_n31_), .O(new_n35_));
  inv1   g06(.a(x11), .O(new_n36_));
  nand2  g07(.a(x12), .b(new_n36_), .O(new_n37_));
  and2   g08(.a(new_n37_), .b(new_n35_), .O(z01));
  nand2  g09(.a(x12), .b(x11), .O(new_n39_));
  nand2  g10(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  nand3  g11(.a(x07), .b(x06), .c(x05), .O(new_n41_));
  inv1   g12(.a(new_n41_), .O(new_n42_));
  inv1   g13(.a(x09), .O(new_n43_));
  nor2   g14(.a(x14), .b(x13), .O(new_n44_));
  nor2   g15(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(new_n42_), .c(new_n40_), .d(x08), .O(z02));
  nand4  g17(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n47_));
  nand3  g18(.a(new_n47_), .b(new_n45_), .c(new_n40_), .O(z03));
  oai21  g19(.a(new_n39_), .b(x11), .c(new_n32_), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(new_n31_), .O(z04));
  nor2   g21(.a(new_n32_), .b(x13), .O(z05));
  nand2  g22(.a(new_n32_), .b(new_n31_), .O(new_n52_));
  nand4  g23(.a(new_n37_), .b(x04), .c(x03), .d(x02), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n52_), .O(z06));
  nor2   g26(.a(new_n44_), .b(x15), .O(z07));
  inv1   g27(.a(x10), .O(new_n57_));
  nand3  g28(.a(x04), .b(x03), .c(x02), .O(new_n58_));
  nand3  g29(.a(x12), .b(x11), .c(x09), .O(new_n59_));
  inv1   g30(.a(x12), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n36_), .O(new_n61_));
  oai21  g32(.a(new_n59_), .b(new_n58_), .c(new_n61_), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n57_), .O(new_n63_));
  nand3  g34(.a(new_n35_), .b(x12), .c(new_n36_), .O(new_n64_));
  nor2   g35(.a(new_n44_), .b(x10), .O(new_n65_));
  nand4  g36(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(x00), .O(z08));
  nand3  g37(.a(new_n58_), .b(new_n44_), .c(x09), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n61_), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(new_n57_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n52_), .O(z09));
  nand4  g42(.a(new_n58_), .b(new_n52_), .c(x12), .d(x11), .O(new_n72_));
  inv1   g43(.a(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n57_), .c(x09), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z10));
  nand4  g46(.a(new_n58_), .b(x12), .c(x09), .d(x01), .O(new_n76_));
  oai21  g47(.a(x12), .b(x01), .c(new_n76_), .O(new_n77_));
  nand4  g48(.a(new_n77_), .b(x11), .c(new_n57_), .d(x00), .O(new_n78_));
  nand2  g49(.a(new_n78_), .b(new_n52_), .O(z11));
  nand2  g50(.a(new_n43_), .b(x00), .O(new_n80_));
  nand3  g51(.a(x12), .b(x11), .c(new_n57_), .O(new_n81_));
  oai21  g52(.a(new_n81_), .b(new_n80_), .c(new_n52_), .O(z12));
endmodule


