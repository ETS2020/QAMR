// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n32_, new_n33_, new_n34_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_;
  inv1   g00(.a(x13), .O(new_n30_));
  inv1   g01(.a(x15), .O(z07));
  nand2  g02(.a(z07), .b(new_n30_), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nor3   g04(.a(x12), .b(x11), .c(x01), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(new_n33_), .O(z00));
  inv1   g06(.a(x11), .O(new_n36_));
  nand3  g07(.a(new_n32_), .b(x12), .c(new_n36_), .O(z01));
  inv1   g08(.a(x12), .O(new_n38_));
  nor2   g09(.a(new_n33_), .b(new_n38_), .O(new_n39_));
  nand3  g10(.a(x07), .b(x06), .c(x05), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  and2   g12(.a(x09), .b(x08), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n41_), .c(new_n39_), .d(x11), .O(z02));
  nand4  g14(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n44_));
  nand4  g15(.a(new_n44_), .b(new_n39_), .c(x11), .d(x09), .O(z03));
  nor2   g16(.a(new_n33_), .b(x14), .O(z04));
  nor2   g17(.a(z07), .b(x13), .O(z05));
  nand2  g18(.a(x12), .b(new_n36_), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(x04), .c(x03), .d(x02), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n32_), .O(z06));
  nand2  g22(.a(new_n39_), .b(new_n36_), .O(new_n52_));
  inv1   g23(.a(x10), .O(new_n53_));
  nand3  g24(.a(x04), .b(x03), .c(x02), .O(new_n54_));
  nand3  g25(.a(x12), .b(x11), .c(x09), .O(new_n55_));
  nand2  g26(.a(new_n38_), .b(new_n36_), .O(new_n56_));
  oai21  g27(.a(new_n55_), .b(new_n54_), .c(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n53_), .O(new_n58_));
  nor2   g29(.a(new_n33_), .b(x10), .O(new_n59_));
  nand4  g30(.a(new_n59_), .b(new_n58_), .c(new_n52_), .d(x00), .O(z08));
  nand4  g31(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n61_));
  nand2  g32(.a(new_n61_), .b(new_n32_), .O(new_n62_));
  nand4  g33(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n63_));
  nand2  g34(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(x12), .c(x11), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n56_), .O(new_n66_));
  nand3  g37(.a(new_n66_), .b(new_n53_), .c(x00), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n32_), .O(z09));
  nand4  g39(.a(new_n54_), .b(new_n32_), .c(x12), .d(x11), .O(new_n69_));
  inv1   g40(.a(new_n69_), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n53_), .c(x09), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z10));
  inv1   g43(.a(x00), .O(new_n73_));
  nand4  g44(.a(new_n54_), .b(x12), .c(x09), .d(x01), .O(new_n74_));
  oai21  g45(.a(x12), .b(x01), .c(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(new_n32_), .c(x11), .d(new_n53_), .O(new_n76_));
  nor2   g47(.a(new_n76_), .b(new_n73_), .O(z11));
  nand3  g48(.a(new_n39_), .b(x11), .c(new_n53_), .O(new_n78_));
  nor3   g49(.a(new_n78_), .b(x09), .c(new_n73_), .O(z12));
endmodule


