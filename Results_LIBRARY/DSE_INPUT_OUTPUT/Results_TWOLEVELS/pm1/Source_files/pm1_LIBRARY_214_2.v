// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n35_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n57_,
    new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_,
    new_n84_;
  inv1   g00(.a(x15), .O(new_n30_));
  nand2  g01(.a(new_n30_), .b(x14), .O(new_n31_));
  oai21  g02(.a(x11), .b(x01), .c(new_n31_), .O(new_n32_));
  nor2   g03(.a(new_n30_), .b(x11), .O(new_n33_));
  nor2   g04(.a(x15), .b(x14), .O(z07));
  oai21  g05(.a(z07), .b(new_n33_), .c(x12), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n32_), .O(z00));
  inv1   g07(.a(x11), .O(new_n37_));
  inv1   g08(.a(x12), .O(new_n38_));
  inv1   g09(.a(x14), .O(new_n39_));
  oai21  g10(.a(x15), .b(new_n39_), .c(new_n38_), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n31_), .c(new_n37_), .O(z01));
  nand2  g12(.a(x06), .b(x05), .O(new_n42_));
  nand3  g13(.a(x09), .b(x08), .c(x07), .O(new_n43_));
  oai21  g14(.a(new_n43_), .b(new_n42_), .c(new_n31_), .O(new_n44_));
  oai21  g15(.a(new_n30_), .b(new_n38_), .c(x14), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n37_), .O(new_n46_));
  nand3  g17(.a(new_n46_), .b(new_n44_), .c(new_n40_), .O(z02));
  inv1   g18(.a(x09), .O(new_n48_));
  nand2  g19(.a(new_n31_), .b(new_n48_), .O(new_n49_));
  nor2   g20(.a(x15), .b(new_n39_), .O(new_n50_));
  nand4  g21(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n51_));
  inv1   g22(.a(new_n51_), .O(new_n52_));
  nor2   g23(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand4  g24(.a(new_n53_), .b(new_n49_), .c(new_n46_), .d(new_n40_), .O(z03));
  nand2  g25(.a(x15), .b(x14), .O(z04));
  nand2  g26(.a(new_n31_), .b(x13), .O(z05));
  nand2  g27(.a(x12), .b(new_n37_), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x04), .c(x03), .d(x02), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x09), .c(x01), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n31_), .O(z06));
  nand4  g31(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n61_));
  inv1   g32(.a(new_n61_), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n33_), .c(x12), .O(new_n63_));
  inv1   g34(.a(x10), .O(new_n64_));
  nand4  g35(.a(x15), .b(new_n38_), .c(new_n64_), .d(x00), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(x14), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n37_), .O(new_n67_));
  nor2   g38(.a(new_n50_), .b(x10), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n67_), .c(new_n63_), .d(x00), .O(z08));
  xnor2a g40(.a(x12), .b(x11), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n31_), .c(new_n64_), .d(x00), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z09));
  nand3  g43(.a(x04), .b(x03), .c(x02), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(new_n31_), .c(x12), .d(x11), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(new_n75_));
  nand4  g46(.a(new_n75_), .b(new_n64_), .c(x09), .d(x00), .O(new_n76_));
  inv1   g47(.a(new_n76_), .O(z10));
  nand4  g48(.a(new_n73_), .b(x12), .c(x09), .d(x01), .O(new_n78_));
  oai21  g49(.a(x12), .b(x01), .c(new_n78_), .O(new_n79_));
  nand4  g50(.a(new_n79_), .b(x11), .c(new_n64_), .d(x00), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n31_), .O(z11));
  inv1   g52(.a(x00), .O(new_n82_));
  nor2   g53(.a(new_n50_), .b(new_n38_), .O(new_n83_));
  nand4  g54(.a(new_n83_), .b(x11), .c(new_n64_), .d(new_n48_), .O(new_n84_));
  nor2   g55(.a(new_n84_), .b(new_n82_), .O(z12));
endmodule


