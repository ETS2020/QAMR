// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n34_, new_n35_, new_n36_, new_n37_,
    new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n88_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand4  g03(.a(new_n32_), .b(new_n31_), .c(x03), .d(new_n30_), .O(z00));
  inv1   g04(.a(x10), .O(new_n34_));
  nor2   g05(.a(new_n31_), .b(new_n30_), .O(new_n35_));
  nand4  g06(.a(new_n35_), .b(new_n34_), .c(new_n30_), .d(x00), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n32_), .c(x03), .O(new_n37_));
  nand2  g08(.a(x06), .b(x05), .O(new_n38_));
  nand3  g09(.a(new_n38_), .b(x08), .c(x07), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n38_), .c(x12), .O(new_n40_));
  oai21  g11(.a(new_n40_), .b(new_n31_), .c(new_n37_), .O(z01));
  inv1   g12(.a(new_n38_), .O(new_n42_));
  and2   g13(.a(x08), .b(x07), .O(new_n43_));
  nand4  g14(.a(new_n43_), .b(new_n42_), .c(x11), .d(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(new_n37_), .O(z02));
  inv1   g17(.a(x09), .O(new_n47_));
  nand3  g18(.a(x11), .b(x08), .c(x07), .O(new_n48_));
  oai21  g19(.a(new_n48_), .b(new_n38_), .c(x11), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n47_), .c(x12), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(new_n37_), .O(z03));
  inv1   g22(.a(x03), .O(new_n52_));
  nand2  g23(.a(new_n32_), .b(new_n52_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(x14), .O(z04));
  nand2  g25(.a(new_n53_), .b(x13), .O(z05));
  aoi21  g26(.a(x09), .b(x01), .c(new_n32_), .O(new_n56_));
  nand2  g27(.a(x12), .b(new_n31_), .O(new_n57_));
  nand3  g28(.a(new_n57_), .b(x04), .c(x02), .O(new_n58_));
  nand3  g29(.a(new_n58_), .b(x09), .c(x01), .O(new_n59_));
  oai21  g30(.a(new_n56_), .b(x03), .c(new_n59_), .O(z06));
  nand2  g31(.a(new_n53_), .b(x15), .O(z07));
  nand2  g32(.a(x04), .b(x02), .O(new_n62_));
  nand3  g33(.a(x12), .b(x11), .c(x09), .O(new_n63_));
  oai22  g34(.a(new_n63_), .b(new_n62_), .c(x12), .d(x11), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n34_), .c(x00), .O(new_n65_));
  inv1   g36(.a(x00), .O(new_n66_));
  oai21  g37(.a(x10), .b(new_n66_), .c(new_n32_), .O(new_n67_));
  nand2  g38(.a(new_n67_), .b(new_n65_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(x03), .O(new_n69_));
  nand3  g40(.a(x11), .b(new_n34_), .c(x00), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(x12), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n69_), .O(z08));
  nand2  g43(.a(new_n64_), .b(x03), .O(new_n73_));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(x09), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x12), .c(x11), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n73_), .O(new_n78_));
  nand3  g49(.a(new_n78_), .b(new_n34_), .c(x00), .O(new_n79_));
  inv1   g50(.a(new_n79_), .O(z09));
  nand4  g51(.a(new_n74_), .b(x12), .c(x11), .d(new_n34_), .O(new_n81_));
  nor3   g52(.a(new_n81_), .b(new_n47_), .c(new_n66_), .O(z10));
  nand4  g53(.a(new_n74_), .b(x12), .c(x09), .d(x01), .O(new_n83_));
  nand3  g54(.a(new_n32_), .b(x03), .c(new_n30_), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand4  g56(.a(new_n85_), .b(x11), .c(new_n34_), .d(x00), .O(new_n86_));
  inv1   g57(.a(new_n86_), .O(z11));
  nand4  g58(.a(x11), .b(new_n34_), .c(new_n47_), .d(x00), .O(new_n88_));
  nor2   g59(.a(new_n88_), .b(new_n32_), .O(z12));
endmodule


