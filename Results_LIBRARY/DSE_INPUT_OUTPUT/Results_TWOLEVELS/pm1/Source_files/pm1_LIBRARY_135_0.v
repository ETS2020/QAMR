// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n37_, new_n39_,
    new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(x08), .c(x07), .O(new_n33_));
  oai21  g04(.a(new_n33_), .b(new_n32_), .c(x11), .O(new_n34_));
  inv1   g05(.a(x13), .O(z05));
  inv1   g06(.a(x15), .O(z07));
  nand2  g07(.a(z07), .b(z05), .O(new_n37_));
  nand4  g08(.a(new_n37_), .b(new_n34_), .c(new_n31_), .d(new_n30_), .O(z00));
  inv1   g09(.a(new_n37_), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n31_), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n34_), .O(z01));
  inv1   g12(.a(x11), .O(new_n42_));
  nand2  g13(.a(new_n37_), .b(new_n42_), .O(new_n43_));
  nand2  g14(.a(x08), .b(x07), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n32_), .c(x11), .O(new_n45_));
  nand4  g16(.a(new_n45_), .b(new_n43_), .c(new_n40_), .d(x09), .O(z02));
  inv1   g17(.a(new_n32_), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(x11), .c(x08), .d(x07), .O(new_n48_));
  nand4  g19(.a(new_n48_), .b(new_n43_), .c(new_n40_), .d(x09), .O(z03));
  nor2   g20(.a(new_n39_), .b(x14), .O(z04));
  nand2  g21(.a(x12), .b(new_n42_), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(x04), .c(x03), .d(x02), .O(new_n52_));
  nand3  g23(.a(new_n52_), .b(x09), .c(x01), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n37_), .O(z06));
  inv1   g25(.a(x00), .O(new_n55_));
  nor2   g26(.a(x10), .b(new_n55_), .O(new_n56_));
  nand4  g27(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n57_));
  inv1   g28(.a(x10), .O(new_n58_));
  nand4  g29(.a(x12), .b(x11), .c(new_n58_), .d(x09), .O(new_n59_));
  nor2   g30(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nor2   g31(.a(new_n60_), .b(new_n42_), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n56_), .c(new_n39_), .O(z08));
  nand2  g33(.a(x03), .b(x02), .O(new_n63_));
  nand3  g34(.a(x04), .b(x03), .c(x02), .O(new_n64_));
  nand2  g35(.a(new_n64_), .b(x04), .O(new_n65_));
  oai21  g36(.a(new_n65_), .b(new_n63_), .c(new_n37_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(x09), .O(new_n67_));
  nand3  g38(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  nand2  g39(.a(new_n31_), .b(new_n42_), .O(new_n69_));
  nand2  g40(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand3  g41(.a(new_n70_), .b(new_n58_), .c(x00), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n37_), .O(z09));
  nand4  g43(.a(new_n64_), .b(new_n37_), .c(x12), .d(x11), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand4  g45(.a(new_n74_), .b(new_n58_), .c(x09), .d(x00), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(z10));
  nand4  g47(.a(new_n64_), .b(x12), .c(x09), .d(x01), .O(new_n77_));
  oai21  g48(.a(x12), .b(x01), .c(new_n77_), .O(new_n78_));
  nand4  g49(.a(new_n78_), .b(new_n37_), .c(x11), .d(new_n58_), .O(new_n79_));
  nor2   g50(.a(new_n79_), .b(new_n55_), .O(z11));
  inv1   g51(.a(x09), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(x00), .O(new_n82_));
  nand3  g53(.a(x12), .b(x11), .c(new_n58_), .O(new_n83_));
  oai21  g54(.a(new_n83_), .b(new_n82_), .c(new_n37_), .O(z12));
endmodule


