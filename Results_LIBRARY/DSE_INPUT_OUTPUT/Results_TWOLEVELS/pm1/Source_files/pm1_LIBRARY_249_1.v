// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_;
  nand2  g00(.a(x06), .b(x05), .O(new_n30_));
  nand3  g01(.a(new_n30_), .b(x08), .c(x07), .O(new_n31_));
  oai21  g02(.a(new_n31_), .b(new_n30_), .c(x11), .O(new_n32_));
  inv1   g03(.a(x01), .O(new_n33_));
  nor2   g04(.a(x11), .b(new_n33_), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n33_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n35_), .c(new_n32_), .O(z00));
  nand3  g08(.a(new_n32_), .b(x12), .c(x01), .O(z01));
  inv1   g09(.a(x12), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n33_), .O(new_n40_));
  nand2  g11(.a(x08), .b(x07), .O(new_n41_));
  oai21  g12(.a(new_n41_), .b(new_n30_), .c(x11), .O(new_n42_));
  nand4  g13(.a(new_n42_), .b(new_n40_), .c(new_n35_), .d(x09), .O(z02));
  inv1   g14(.a(x09), .O(new_n44_));
  nor2   g15(.a(new_n39_), .b(new_n44_), .O(new_n45_));
  inv1   g16(.a(new_n30_), .O(new_n46_));
  nand4  g17(.a(new_n46_), .b(x11), .c(x08), .d(x07), .O(new_n47_));
  nand4  g18(.a(new_n47_), .b(new_n45_), .c(new_n36_), .d(new_n35_), .O(z03));
  inv1   g19(.a(new_n36_), .O(new_n49_));
  nor2   g20(.a(new_n49_), .b(x14), .O(z04));
  nor2   g21(.a(new_n49_), .b(x13), .O(z05));
  and2   g22(.a(x03), .b(x02), .O(new_n52_));
  inv1   g23(.a(x11), .O(new_n53_));
  nand2  g24(.a(x12), .b(new_n53_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n52_), .c(x04), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  inv1   g27(.a(new_n56_), .O(z06));
  nand2  g28(.a(new_n36_), .b(x15), .O(z07));
  inv1   g29(.a(x10), .O(new_n59_));
  nand2  g30(.a(new_n52_), .b(x01), .O(new_n60_));
  nand4  g31(.a(x12), .b(x11), .c(x09), .d(x04), .O(new_n61_));
  nand2  g32(.a(new_n39_), .b(new_n53_), .O(new_n62_));
  oai21  g33(.a(new_n61_), .b(new_n60_), .c(new_n62_), .O(new_n63_));
  nand3  g34(.a(new_n63_), .b(new_n59_), .c(x00), .O(new_n64_));
  nand2  g35(.a(new_n59_), .b(x00), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n36_), .c(new_n34_), .O(new_n66_));
  nand2  g37(.a(new_n66_), .b(new_n64_), .O(z08));
  nand4  g38(.a(x04), .b(x03), .c(x02), .d(x01), .O(new_n68_));
  nand4  g39(.a(new_n68_), .b(new_n52_), .c(x09), .d(x04), .O(new_n69_));
  nand3  g40(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  nand2  g41(.a(new_n70_), .b(new_n62_), .O(new_n71_));
  nand3  g42(.a(new_n71_), .b(new_n59_), .c(x00), .O(new_n72_));
  nand2  g43(.a(new_n72_), .b(new_n36_), .O(z09));
  nand3  g44(.a(x04), .b(x03), .c(x02), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x11), .c(new_n59_), .O(new_n75_));
  inv1   g46(.a(new_n75_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(x09), .c(x00), .O(new_n77_));
  aoi21  g48(.a(new_n77_), .b(x01), .c(new_n39_), .O(z10));
  nand3  g49(.a(x11), .b(new_n59_), .c(x00), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n39_), .O(new_n80_));
  nand2  g51(.a(new_n80_), .b(new_n33_), .O(new_n81_));
  nand4  g52(.a(new_n74_), .b(x12), .c(x11), .d(new_n59_), .O(new_n82_));
  inv1   g53(.a(new_n82_), .O(new_n83_));
  nand3  g54(.a(new_n83_), .b(x09), .c(x00), .O(new_n84_));
  nand2  g55(.a(new_n84_), .b(new_n81_), .O(z11));
  nand4  g56(.a(x11), .b(new_n59_), .c(new_n44_), .d(x00), .O(new_n86_));
  aoi21  g57(.a(new_n86_), .b(x01), .c(new_n39_), .O(z12));
endmodule


