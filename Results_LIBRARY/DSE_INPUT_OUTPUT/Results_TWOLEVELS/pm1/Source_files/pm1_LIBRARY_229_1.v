// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n57_, new_n59_,
    new_n62_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x10), .O(new_n32_));
  oai21  g03(.a(new_n31_), .b(new_n32_), .c(x12), .O(new_n33_));
  nand3  g04(.a(new_n33_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g05(.a(x12), .b(new_n31_), .c(x10), .O(z01));
  inv1   g06(.a(x09), .O(new_n36_));
  aoi21  g07(.a(x12), .b(new_n31_), .c(new_n36_), .O(new_n37_));
  nand3  g08(.a(x07), .b(x06), .c(x05), .O(new_n38_));
  nand3  g09(.a(x12), .b(x10), .c(x08), .O(new_n39_));
  nor2   g10(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  oai21  g11(.a(new_n37_), .b(new_n32_), .c(new_n40_), .O(z02));
  nand2  g12(.a(x12), .b(new_n31_), .O(new_n42_));
  nand4  g13(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n42_), .c(x09), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x10), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x12), .O(z03));
  nand2  g17(.a(x12), .b(new_n32_), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x14), .O(z04));
  nand2  g19(.a(new_n47_), .b(x13), .O(z05));
  nand3  g20(.a(new_n31_), .b(x09), .c(x01), .O(new_n50_));
  nand2  g21(.a(new_n50_), .b(x10), .O(new_n51_));
  nand2  g22(.a(new_n51_), .b(x12), .O(new_n52_));
  nand3  g23(.a(x04), .b(x03), .c(x02), .O(new_n53_));
  nand3  g24(.a(new_n53_), .b(x09), .c(x01), .O(new_n54_));
  nand2  g25(.a(new_n54_), .b(new_n52_), .O(z06));
  aoi21  g26(.a(x12), .b(new_n32_), .c(x15), .O(z07));
  inv1   g27(.a(x12), .O(new_n57_));
  nand4  g28(.a(new_n57_), .b(x11), .c(new_n32_), .d(x00), .O(z08));
  nand4  g29(.a(new_n57_), .b(new_n31_), .c(new_n32_), .d(x00), .O(new_n59_));
  inv1   g30(.a(new_n59_), .O(z09));
  nand4  g31(.a(x11), .b(new_n32_), .c(new_n30_), .d(x00), .O(new_n62_));
  nor2   g32(.a(new_n62_), .b(x12), .O(z11));
  zero   g33(.O(z10));
  zero   g34(.O(z12));
endmodule


