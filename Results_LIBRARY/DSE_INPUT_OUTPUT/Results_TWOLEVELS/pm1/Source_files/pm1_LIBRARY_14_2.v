// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand3  g03(.a(new_n32_), .b(new_n31_), .c(new_n30_), .O(z00));
  nand3  g04(.a(x15), .b(x12), .c(new_n31_), .O(z01));
  nand3  g05(.a(x07), .b(x06), .c(x05), .O(new_n35_));
  nand2  g06(.a(x12), .b(new_n31_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(x09), .c(x08), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n35_), .c(x15), .O(new_n38_));
  nand2  g09(.a(new_n38_), .b(x12), .O(z02));
  nand4  g10(.a(x08), .b(x07), .c(x06), .d(x05), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(new_n36_), .c(x09), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(x15), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x12), .O(z03));
  inv1   g14(.a(x15), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(x12), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x14), .O(z04));
  nand2  g17(.a(new_n45_), .b(x13), .O(z05));
  nand3  g18(.a(new_n31_), .b(x09), .c(x01), .O(new_n48_));
  nand2  g19(.a(new_n48_), .b(x15), .O(new_n49_));
  nand2  g20(.a(new_n49_), .b(x12), .O(new_n50_));
  nand3  g21(.a(x04), .b(x03), .c(x02), .O(new_n51_));
  nand3  g22(.a(new_n51_), .b(x09), .c(x01), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n50_), .O(z06));
  nor2   g24(.a(x15), .b(x12), .O(z07));
  inv1   g25(.a(x10), .O(new_n55_));
  nand3  g26(.a(new_n32_), .b(new_n55_), .c(x00), .O(new_n56_));
  oai21  g27(.a(new_n44_), .b(new_n32_), .c(new_n56_), .O(new_n57_));
  nand2  g28(.a(new_n57_), .b(new_n31_), .O(new_n58_));
  nand4  g29(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(x15), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(x12), .O(new_n61_));
  nand4  g32(.a(new_n61_), .b(new_n58_), .c(new_n55_), .d(x00), .O(z08));
  nand3  g33(.a(x15), .b(x12), .c(x11), .O(new_n63_));
  oai21  g34(.a(x12), .b(x11), .c(new_n63_), .O(new_n64_));
  nand3  g35(.a(new_n64_), .b(new_n55_), .c(x00), .O(new_n65_));
  inv1   g36(.a(new_n65_), .O(z09));
  nand3  g37(.a(new_n51_), .b(x11), .c(new_n55_), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(new_n68_));
  nand3  g39(.a(new_n68_), .b(x09), .c(x00), .O(new_n69_));
  aoi21  g40(.a(new_n69_), .b(x15), .c(new_n32_), .O(z10));
  nand4  g41(.a(new_n51_), .b(x15), .c(x12), .d(x09), .O(new_n71_));
  nand2  g42(.a(new_n32_), .b(new_n30_), .O(new_n72_));
  oai21  g43(.a(new_n71_), .b(new_n30_), .c(new_n72_), .O(new_n73_));
  nand4  g44(.a(new_n73_), .b(x11), .c(new_n55_), .d(x00), .O(new_n74_));
  inv1   g45(.a(new_n74_), .O(z11));
  inv1   g46(.a(x09), .O(new_n76_));
  nand4  g47(.a(x11), .b(new_n55_), .c(new_n76_), .d(x00), .O(new_n77_));
  nor3   g48(.a(new_n77_), .b(new_n44_), .c(new_n32_), .O(z12));
endmodule


