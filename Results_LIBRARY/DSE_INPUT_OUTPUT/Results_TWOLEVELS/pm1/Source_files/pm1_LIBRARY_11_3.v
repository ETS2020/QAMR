// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n47_, new_n48_, new_n49_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_, new_n65_,
    new_n66_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x11), .O(new_n31_));
  inv1   g02(.a(x12), .O(new_n32_));
  nand2  g03(.a(x15), .b(x08), .O(new_n33_));
  nand4  g04(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g05(.a(new_n33_), .b(x12), .c(new_n31_), .O(z01));
  nand2  g06(.a(x12), .b(x11), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n33_), .O(new_n37_));
  inv1   g08(.a(x08), .O(new_n38_));
  inv1   g09(.a(x10), .O(new_n39_));
  nand3  g10(.a(new_n36_), .b(new_n39_), .c(x00), .O(new_n40_));
  nand2  g11(.a(new_n40_), .b(new_n38_), .O(new_n41_));
  inv1   g12(.a(x15), .O(new_n42_));
  nand2  g13(.a(x06), .b(x05), .O(new_n43_));
  nand2  g14(.a(x09), .b(x07), .O(new_n44_));
  oai21  g15(.a(new_n44_), .b(new_n43_), .c(new_n42_), .O(new_n45_));
  nand3  g16(.a(new_n45_), .b(new_n41_), .c(new_n37_), .O(z02));
  inv1   g17(.a(x09), .O(new_n47_));
  oai21  g18(.a(new_n36_), .b(new_n47_), .c(new_n33_), .O(new_n48_));
  nand3  g19(.a(new_n42_), .b(x08), .c(x07), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n43_), .c(new_n48_), .O(z03));
  nand2  g21(.a(new_n33_), .b(x14), .O(z04));
  nand2  g22(.a(new_n33_), .b(x13), .O(z05));
  and2   g23(.a(x03), .b(x02), .O(new_n53_));
  nand2  g24(.a(x12), .b(new_n31_), .O(new_n54_));
  nand3  g25(.a(new_n54_), .b(new_n53_), .c(x04), .O(new_n55_));
  nand3  g26(.a(new_n55_), .b(x09), .c(x01), .O(new_n56_));
  nand2  g27(.a(new_n56_), .b(new_n33_), .O(z06));
  nand2  g28(.a(new_n39_), .b(x00), .O(new_n58_));
  inv1   g29(.a(new_n58_), .O(new_n59_));
  nand3  g30(.a(new_n58_), .b(x12), .c(x11), .O(new_n60_));
  inv1   g31(.a(new_n60_), .O(new_n61_));
  aoi21  g32(.a(new_n61_), .b(new_n59_), .c(x15), .O(z07));
  inv1   g33(.a(new_n33_), .O(new_n63_));
  nand3  g34(.a(x12), .b(x09), .c(x04), .O(new_n64_));
  inv1   g35(.a(new_n64_), .O(new_n65_));
  aoi21  g36(.a(new_n65_), .b(new_n53_), .c(new_n31_), .O(new_n66_));
  aoi21  g37(.a(new_n66_), .b(new_n59_), .c(new_n63_), .O(z08));
  nand2  g38(.a(new_n32_), .b(new_n31_), .O(new_n68_));
  nand2  g39(.a(new_n36_), .b(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n33_), .c(new_n39_), .d(x00), .O(new_n70_));
  inv1   g41(.a(new_n70_), .O(z09));
  nand3  g42(.a(x04), .b(x03), .c(x02), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(x12), .c(x11), .d(new_n39_), .O(new_n73_));
  inv1   g44(.a(new_n73_), .O(new_n74_));
  nand3  g45(.a(new_n74_), .b(x09), .c(x00), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n33_), .O(z10));
  inv1   g47(.a(x00), .O(new_n77_));
  nand4  g48(.a(new_n72_), .b(x12), .c(x09), .d(x01), .O(new_n78_));
  nand2  g49(.a(new_n32_), .b(new_n30_), .O(new_n79_));
  nand2  g50(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand4  g51(.a(new_n80_), .b(new_n33_), .c(x11), .d(new_n39_), .O(new_n81_));
  nor2   g52(.a(new_n81_), .b(new_n77_), .O(z11));
  nand2  g53(.a(new_n47_), .b(x00), .O(new_n83_));
  nand3  g54(.a(x12), .b(x11), .c(new_n39_), .O(new_n84_));
  oai21  g55(.a(new_n84_), .b(new_n83_), .c(new_n33_), .O(z12));
endmodule


