// Benchmark "FAU" written by ABC on Wed Aug 19 14:48:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n55_, new_n56_, new_n59_, new_n60_, new_n61_, new_n62_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_;
  inv1   g00(.a(x01), .O(new_n30_));
  nand2  g01(.a(x14), .b(new_n30_), .O(new_n31_));
  nand2  g02(.a(x06), .b(x05), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  nand3  g04(.a(new_n32_), .b(x08), .c(x07), .O(new_n34_));
  inv1   g05(.a(new_n34_), .O(new_n35_));
  nand2  g06(.a(new_n35_), .b(new_n33_), .O(new_n36_));
  nand3  g07(.a(new_n36_), .b(new_n31_), .c(x11), .O(new_n37_));
  inv1   g08(.a(x11), .O(new_n38_));
  inv1   g09(.a(x09), .O(new_n39_));
  oai21  g10(.a(new_n39_), .b(new_n30_), .c(x14), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x12), .c(new_n38_), .O(new_n41_));
  nand2  g12(.a(x12), .b(x09), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(x01), .O(new_n43_));
  nand3  g14(.a(new_n43_), .b(new_n41_), .c(new_n37_), .O(z00));
  inv1   g15(.a(new_n31_), .O(new_n45_));
  nand2  g16(.a(new_n36_), .b(x11), .O(new_n46_));
  aoi21  g17(.a(new_n46_), .b(x12), .c(new_n45_), .O(z01));
  nand3  g18(.a(new_n33_), .b(x08), .c(x07), .O(new_n48_));
  aoi21  g19(.a(new_n48_), .b(x11), .c(new_n42_), .O(new_n49_));
  oai21  g20(.a(new_n49_), .b(new_n45_), .c(new_n41_), .O(z02));
  nand3  g21(.a(x11), .b(x08), .c(x07), .O(new_n51_));
  oai21  g22(.a(new_n51_), .b(new_n32_), .c(x12), .O(new_n52_));
  oai21  g23(.a(new_n52_), .b(new_n39_), .c(new_n31_), .O(new_n53_));
  nand2  g24(.a(new_n53_), .b(new_n41_), .O(z03));
  inv1   g25(.a(x12), .O(new_n55_));
  nor2   g26(.a(new_n55_), .b(new_n38_), .O(new_n56_));
  aoi21  g27(.a(new_n56_), .b(new_n46_), .c(x14), .O(z04));
  nand2  g28(.a(new_n31_), .b(x13), .O(z05));
  nand2  g29(.a(x12), .b(new_n38_), .O(new_n59_));
  and2   g30(.a(x03), .b(x02), .O(new_n60_));
  nand3  g31(.a(new_n60_), .b(new_n59_), .c(x04), .O(new_n61_));
  nand3  g32(.a(new_n61_), .b(x09), .c(x01), .O(new_n62_));
  inv1   g33(.a(new_n62_), .O(z06));
  nor2   g34(.a(new_n45_), .b(x15), .O(z07));
  nor2   g35(.a(new_n38_), .b(x10), .O(new_n65_));
  inv1   g36(.a(x10), .O(new_n66_));
  nand4  g37(.a(x04), .b(x03), .c(x02), .d(x00), .O(new_n67_));
  inv1   g38(.a(new_n67_), .O(new_n68_));
  nor2   g39(.a(new_n55_), .b(new_n38_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(new_n68_), .c(new_n66_), .d(x09), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n65_), .c(new_n31_), .d(x00), .O(z08));
  nand3  g42(.a(x04), .b(x03), .c(x02), .O(new_n72_));
  nand4  g43(.a(new_n72_), .b(new_n60_), .c(x09), .d(x04), .O(new_n73_));
  nand3  g44(.a(new_n73_), .b(x12), .c(x11), .O(new_n74_));
  nand2  g45(.a(new_n55_), .b(new_n38_), .O(new_n75_));
  nand2  g46(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand3  g47(.a(new_n76_), .b(new_n66_), .c(x00), .O(new_n77_));
  nand2  g48(.a(new_n77_), .b(new_n31_), .O(z09));
  nand4  g49(.a(new_n72_), .b(x12), .c(x11), .d(new_n66_), .O(new_n79_));
  inv1   g50(.a(new_n79_), .O(new_n80_));
  nand3  g51(.a(new_n80_), .b(x09), .c(x00), .O(new_n81_));
  nand2  g52(.a(new_n81_), .b(new_n31_), .O(z10));
  nand4  g53(.a(new_n72_), .b(x12), .c(x09), .d(x01), .O(new_n83_));
  inv1   g54(.a(x14), .O(new_n84_));
  nand3  g55(.a(new_n84_), .b(new_n55_), .c(new_n30_), .O(new_n85_));
  nand2  g56(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nand4  g57(.a(new_n86_), .b(x11), .c(new_n66_), .d(x00), .O(new_n87_));
  inv1   g58(.a(new_n87_), .O(z11));
  nand2  g59(.a(new_n39_), .b(x00), .O(new_n89_));
  nand2  g60(.a(new_n69_), .b(new_n66_), .O(new_n90_));
  oai21  g61(.a(new_n90_), .b(new_n89_), .c(new_n31_), .O(z12));
endmodule


