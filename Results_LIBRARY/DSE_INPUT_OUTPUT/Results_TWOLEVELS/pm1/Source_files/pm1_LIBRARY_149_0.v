// Benchmark "FAU" written by ABC on Wed Aug 19 14:47:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_;
  inv1   g00(.a(x01), .O(new_n30_));
  inv1   g01(.a(x12), .O(new_n31_));
  nor2   g02(.a(x15), .b(x14), .O(new_n32_));
  inv1   g03(.a(new_n32_), .O(new_n33_));
  inv1   g04(.a(x10), .O(new_n34_));
  aoi21  g05(.a(new_n34_), .b(x00), .c(new_n32_), .O(new_n35_));
  nand4  g06(.a(new_n31_), .b(new_n34_), .c(new_n30_), .d(x00), .O(new_n36_));
  inv1   g07(.a(new_n36_), .O(new_n37_));
  oai21  g08(.a(new_n37_), .b(new_n35_), .c(x11), .O(new_n38_));
  nand4  g09(.a(new_n38_), .b(new_n33_), .c(new_n31_), .d(new_n30_), .O(z00));
  nand3  g10(.a(x03), .b(x02), .c(x00), .O(new_n40_));
  inv1   g11(.a(new_n40_), .O(new_n41_));
  nand4  g12(.a(x12), .b(new_n34_), .c(x09), .d(x04), .O(new_n42_));
  inv1   g13(.a(new_n42_), .O(new_n43_));
  aoi21  g14(.a(new_n43_), .b(new_n41_), .c(x10), .O(new_n44_));
  nand3  g15(.a(new_n44_), .b(x09), .c(x00), .O(new_n45_));
  nand2  g16(.a(new_n45_), .b(x11), .O(new_n46_));
  nand3  g17(.a(x04), .b(x03), .c(x02), .O(new_n47_));
  and2   g18(.a(new_n47_), .b(x12), .O(new_n48_));
  nand3  g19(.a(new_n48_), .b(x11), .c(new_n34_), .O(new_n49_));
  inv1   g20(.a(new_n49_), .O(new_n50_));
  nand3  g21(.a(new_n50_), .b(x09), .c(x00), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(new_n46_), .c(new_n33_), .d(x12), .O(z01));
  nor2   g23(.a(x15), .b(x14), .O(new_n53_));
  inv1   g24(.a(new_n53_), .O(new_n54_));
  nand2  g25(.a(x06), .b(x05), .O(new_n55_));
  nand2  g26(.a(x08), .b(x07), .O(new_n56_));
  oai21  g27(.a(new_n56_), .b(new_n55_), .c(new_n54_), .O(new_n57_));
  nand3  g28(.a(x12), .b(x11), .c(x09), .O(new_n58_));
  nand2  g29(.a(new_n58_), .b(new_n33_), .O(new_n59_));
  nand2  g30(.a(new_n59_), .b(new_n57_), .O(z02));
  inv1   g31(.a(x05), .O(new_n61_));
  nand4  g32(.a(new_n54_), .b(x08), .c(x07), .d(x06), .O(new_n62_));
  oai21  g33(.a(new_n62_), .b(new_n61_), .c(new_n59_), .O(z03));
  nand3  g34(.a(new_n55_), .b(x08), .c(x07), .O(new_n64_));
  oai21  g35(.a(new_n64_), .b(new_n55_), .c(x15), .O(new_n65_));
  nor2   g36(.a(new_n65_), .b(x14), .O(z04));
  nand2  g37(.a(new_n33_), .b(x13), .O(z05));
  inv1   g38(.a(x11), .O(new_n68_));
  nand2  g39(.a(x12), .b(new_n68_), .O(new_n69_));
  nand4  g40(.a(new_n69_), .b(x04), .c(x03), .d(x02), .O(new_n70_));
  nand4  g41(.a(new_n70_), .b(new_n33_), .c(x09), .d(x01), .O(new_n71_));
  inv1   g42(.a(new_n71_), .O(z06));
  inv1   g43(.a(x15), .O(z07));
  nand3  g44(.a(new_n44_), .b(x11), .c(x00), .O(new_n74_));
  and2   g45(.a(new_n74_), .b(new_n33_), .O(z08));
  nand4  g46(.a(x09), .b(x04), .c(x03), .d(x02), .O(new_n76_));
  aoi21  g47(.a(new_n76_), .b(x09), .c(new_n32_), .O(new_n77_));
  and2   g48(.a(new_n47_), .b(x09), .O(new_n78_));
  oai21  g49(.a(new_n78_), .b(new_n77_), .c(x12), .O(new_n79_));
  nand2  g50(.a(new_n31_), .b(new_n68_), .O(new_n80_));
  oai21  g51(.a(new_n79_), .b(new_n68_), .c(new_n80_), .O(new_n81_));
  nand3  g52(.a(new_n81_), .b(new_n34_), .c(x00), .O(new_n82_));
  nand2  g53(.a(new_n82_), .b(new_n33_), .O(z09));
  nand2  g54(.a(new_n51_), .b(new_n33_), .O(z10));
  nand3  g55(.a(new_n48_), .b(x09), .c(x01), .O(new_n85_));
  oai21  g56(.a(x12), .b(x01), .c(new_n85_), .O(new_n86_));
  nand4  g57(.a(new_n86_), .b(x11), .c(new_n34_), .d(x00), .O(new_n87_));
  nand2  g58(.a(new_n87_), .b(new_n33_), .O(z11));
  inv1   g59(.a(x00), .O(new_n89_));
  nand4  g60(.a(new_n33_), .b(x12), .c(x11), .d(new_n34_), .O(new_n90_));
  nor3   g61(.a(new_n90_), .b(x09), .c(new_n89_), .O(z12));
endmodule


