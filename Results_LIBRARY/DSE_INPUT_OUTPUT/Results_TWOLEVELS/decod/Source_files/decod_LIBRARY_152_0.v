// Benchmark "FAU" written by ABC on Wed Aug 19 17:05:25 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n24_, new_n25_, new_n26_, new_n28_, new_n30_,
    new_n31_, new_n34_, new_n35_, new_n37_, new_n41_, new_n42_, new_n44_,
    new_n46_, new_n49_, new_n51_;
  inv1   g00(.a(x2), .O(new_n22_));
  inv1   g01(.a(x3), .O(new_n23_));
  nand2  g02(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand2  g03(.a(x1), .b(x0), .O(new_n25_));
  nand3  g04(.a(x4), .b(x3), .c(x2), .O(new_n26_));
  oai21  g05(.a(new_n26_), .b(new_n25_), .c(new_n24_), .O(z00));
  nand3  g06(.a(x4), .b(x1), .c(x0), .O(new_n28_));
  aoi21  g07(.a(new_n28_), .b(x2), .c(x3), .O(z01));
  inv1   g08(.a(x4), .O(new_n30_));
  nand4  g09(.a(x3), .b(new_n22_), .c(x1), .d(x0), .O(new_n31_));
  nor2   g10(.a(new_n31_), .b(new_n30_), .O(z02));
  inv1   g11(.a(x1), .O(new_n34_));
  nand2  g12(.a(new_n34_), .b(x0), .O(new_n35_));
  oai21  g13(.a(new_n35_), .b(new_n26_), .c(new_n24_), .O(z04));
  nand3  g14(.a(x4), .b(new_n34_), .c(x0), .O(new_n37_));
  aoi21  g15(.a(new_n37_), .b(x2), .c(x3), .O(z05));
  aoi21  g16(.a(new_n37_), .b(x3), .c(x2), .O(z06));
  inv1   g17(.a(x0), .O(new_n41_));
  nand2  g18(.a(x1), .b(new_n41_), .O(new_n42_));
  oai21  g19(.a(new_n42_), .b(new_n26_), .c(new_n24_), .O(z08));
  nand3  g20(.a(x4), .b(x1), .c(new_n41_), .O(new_n44_));
  aoi21  g21(.a(new_n44_), .b(x2), .c(x3), .O(z09));
  nand4  g22(.a(x3), .b(new_n22_), .c(x1), .d(new_n41_), .O(new_n46_));
  nor2   g23(.a(new_n46_), .b(new_n30_), .O(z10));
  inv1   g24(.a(new_n24_), .O(z11));
  nand2  g25(.a(new_n34_), .b(new_n41_), .O(new_n49_));
  oai21  g26(.a(new_n49_), .b(new_n26_), .c(new_n24_), .O(z12));
  nand3  g27(.a(x4), .b(new_n34_), .c(new_n41_), .O(new_n51_));
  aoi21  g28(.a(new_n51_), .b(x2), .c(x3), .O(z13));
  aoi21  g29(.a(new_n51_), .b(x3), .c(x2), .O(z14));
  zero   g30(.O(z03));
  zero   g31(.O(z07));
  zero   g32(.O(z15));
endmodule


