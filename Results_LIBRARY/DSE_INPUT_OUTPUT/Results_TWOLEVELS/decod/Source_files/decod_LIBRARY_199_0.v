// Benchmark "FAU" written by ABC on Wed Aug 19 17:05:37 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n27_, new_n28_, new_n29_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n37_, new_n39_, new_n41_, new_n43_,
    new_n44_, new_n47_, new_n50_, new_n53_;
  nand2  g00(.a(x1), .b(x0), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x2), .O(new_n23_));
  oai22  g02(.a(new_n23_), .b(new_n22_), .c(x3), .d(x0), .O(z00));
  nand3  g03(.a(x4), .b(x2), .c(x1), .O(new_n25_));
  aoi21  g04(.a(new_n25_), .b(x0), .c(x3), .O(z01));
  inv1   g05(.a(x4), .O(new_n27_));
  inv1   g06(.a(x2), .O(new_n28_));
  nand4  g07(.a(x3), .b(new_n28_), .c(x1), .d(x0), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n27_), .O(z02));
  inv1   g09(.a(x3), .O(new_n31_));
  nand4  g10(.a(new_n31_), .b(new_n28_), .c(x1), .d(x0), .O(new_n32_));
  nor2   g11(.a(new_n32_), .b(new_n27_), .O(z03));
  inv1   g12(.a(x1), .O(new_n34_));
  nand4  g13(.a(x3), .b(x2), .c(new_n34_), .d(x0), .O(new_n35_));
  nor2   g14(.a(new_n35_), .b(new_n27_), .O(z04));
  nand4  g15(.a(new_n31_), .b(x2), .c(new_n34_), .d(x0), .O(new_n37_));
  nor2   g16(.a(new_n37_), .b(new_n27_), .O(z05));
  nand4  g17(.a(x3), .b(new_n28_), .c(new_n34_), .d(x0), .O(new_n39_));
  nor2   g18(.a(new_n39_), .b(new_n27_), .O(z06));
  nand4  g19(.a(new_n31_), .b(new_n28_), .c(new_n34_), .d(x0), .O(new_n41_));
  nor2   g20(.a(new_n41_), .b(new_n27_), .O(z07));
  inv1   g21(.a(x0), .O(new_n43_));
  nand4  g22(.a(x3), .b(x2), .c(x1), .d(new_n43_), .O(new_n44_));
  nor2   g23(.a(new_n44_), .b(new_n27_), .O(z08));
  nor2   g24(.a(x3), .b(x0), .O(z09));
  nand4  g25(.a(x3), .b(new_n28_), .c(x1), .d(new_n43_), .O(new_n47_));
  nor2   g26(.a(new_n47_), .b(new_n27_), .O(z10));
  nand4  g27(.a(x3), .b(x2), .c(new_n34_), .d(new_n43_), .O(new_n50_));
  nor2   g28(.a(new_n50_), .b(new_n27_), .O(z12));
  nand3  g29(.a(x4), .b(new_n28_), .c(new_n34_), .O(new_n53_));
  aoi21  g30(.a(new_n53_), .b(x3), .c(x0), .O(z14));
  zero   g31(.O(z11));
  zero   g32(.O(z13));
  zero   g33(.O(z15));
endmodule


