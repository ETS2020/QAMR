// Benchmark "FAU" written by ABC on Wed Aug 19 17:05:27 2020

module FAU ( 
    x0, x1, x2, x3, x4,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x0, x1, x2, x3, x4;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n22_, new_n23_, new_n25_, new_n26_, new_n28_, new_n29_, new_n31_,
    new_n33_, new_n34_, new_n36_, new_n38_, new_n39_, new_n40_, new_n41_,
    new_n43_, new_n47_, new_n49_, new_n52_, new_n54_;
  inv1   g00(.a(x2), .O(new_n22_));
  nand3  g01(.a(x4), .b(x3), .c(x1), .O(new_n23_));
  aoi21  g02(.a(new_n23_), .b(x0), .c(new_n22_), .O(z00));
  inv1   g03(.a(x3), .O(new_n25_));
  nand3  g04(.a(x4), .b(new_n25_), .c(x1), .O(new_n26_));
  aoi21  g05(.a(new_n26_), .b(x0), .c(new_n22_), .O(z01));
  inv1   g06(.a(x4), .O(new_n28_));
  nand4  g07(.a(x3), .b(new_n22_), .c(x1), .d(x0), .O(new_n29_));
  nor2   g08(.a(new_n29_), .b(new_n28_), .O(z02));
  nand4  g09(.a(new_n25_), .b(new_n22_), .c(x1), .d(x0), .O(new_n31_));
  nor2   g10(.a(new_n31_), .b(new_n28_), .O(z03));
  inv1   g11(.a(x1), .O(new_n33_));
  nand3  g12(.a(x4), .b(x3), .c(new_n33_), .O(new_n34_));
  aoi21  g13(.a(new_n34_), .b(x0), .c(new_n22_), .O(z04));
  nand3  g14(.a(x4), .b(new_n25_), .c(new_n33_), .O(new_n36_));
  aoi21  g15(.a(new_n36_), .b(x0), .c(new_n22_), .O(z05));
  inv1   g16(.a(x0), .O(new_n38_));
  nand2  g17(.a(x2), .b(new_n38_), .O(new_n39_));
  nand2  g18(.a(new_n33_), .b(x0), .O(new_n40_));
  nand3  g19(.a(x4), .b(x3), .c(new_n22_), .O(new_n41_));
  oai21  g20(.a(new_n41_), .b(new_n40_), .c(new_n39_), .O(z06));
  nand3  g21(.a(x4), .b(new_n25_), .c(new_n22_), .O(new_n43_));
  oai21  g22(.a(new_n43_), .b(new_n40_), .c(new_n39_), .O(z07));
  inv1   g23(.a(new_n39_), .O(z08));
  nand4  g24(.a(x3), .b(new_n22_), .c(x1), .d(new_n38_), .O(new_n47_));
  nor2   g25(.a(new_n47_), .b(new_n28_), .O(z10));
  nand4  g26(.a(new_n25_), .b(new_n22_), .c(x1), .d(new_n38_), .O(new_n49_));
  nor2   g27(.a(new_n49_), .b(new_n28_), .O(z11));
  nand4  g28(.a(x3), .b(new_n22_), .c(new_n33_), .d(new_n38_), .O(new_n52_));
  nor2   g29(.a(new_n52_), .b(new_n28_), .O(z14));
  nand4  g30(.a(new_n25_), .b(new_n22_), .c(new_n33_), .d(new_n38_), .O(new_n54_));
  nor2   g31(.a(new_n54_), .b(new_n28_), .O(z15));
  zero   g32(.O(z09));
  zero   g33(.O(z12));
  inv1   g34(.a(new_n39_), .O(z13));
endmodule


