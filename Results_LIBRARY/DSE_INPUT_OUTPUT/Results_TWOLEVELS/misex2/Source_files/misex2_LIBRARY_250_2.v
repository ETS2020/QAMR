// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n129_,
    new_n131_, new_n132_, new_n135_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nand4  g10(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nor2   g17(.a(x09), .b(new_n48_), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  inv1   g19(.a(new_n62_), .O(new_n63_));
  aoi21  g20(.a(new_n63_), .b(new_n61_), .c(new_n47_), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nor2   g22(.a(x02), .b(x01), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n44_), .O(new_n67_));
  oai21  g24(.a(new_n64_), .b(new_n46_), .c(new_n67_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n61_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x01), .c(new_n46_), .O(z04));
  nand3  g29(.a(x10), .b(x09), .c(x02), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(x01), .c(new_n46_), .O(z05));
  nand4  g31(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x01), .c(new_n46_), .O(z06));
  nand4  g33(.a(new_n61_), .b(new_n70_), .c(x11), .d(x10), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x01), .c(new_n46_), .O(z07));
  nor2   g35(.a(x03), .b(new_n48_), .O(new_n79_));
  nor2   g36(.a(x06), .b(x05), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(x04), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(new_n82_));
  nor2   g39(.a(x08), .b(x07), .O(new_n83_));
  nand3  g40(.a(x19), .b(new_n45_), .c(x17), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n83_), .c(new_n82_), .d(new_n79_), .O(new_n86_));
  aoi21  g43(.a(new_n86_), .b(new_n46_), .c(x01), .O(z08));
  inv1   g44(.a(x21), .O(new_n88_));
  inv1   g45(.a(x22), .O(new_n89_));
  nand4  g46(.a(x12), .b(new_n69_), .c(x02), .d(new_n47_), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nor2   g49(.a(x14), .b(x13), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(x20), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x18), .c(x01), .O(new_n96_));
  oai21  g53(.a(new_n94_), .b(new_n90_), .c(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n89_), .c(new_n88_), .d(new_n46_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(z09));
  nor3   g56(.a(new_n89_), .b(new_n88_), .c(x20), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n65_), .c(x18), .d(x01), .O(new_n101_));
  nor2   g58(.a(new_n48_), .b(x01), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n69_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nor3   g62(.a(new_n92_), .b(new_n91_), .c(x14), .O(new_n106_));
  nor2   g63(.a(x22), .b(x21), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x20), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n106_), .c(new_n105_), .d(new_n102_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n101_), .c(x00), .O(z10));
  nand4  g68(.a(new_n93_), .b(x12), .c(new_n69_), .d(x02), .O(new_n112_));
  nand4  g69(.a(new_n107_), .b(x20), .c(new_n92_), .d(x15), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n112_), .c(new_n46_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n47_), .O(new_n115_));
  nor3   g72(.a(new_n45_), .b(new_n47_), .c(x00), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n95_), .c(new_n89_), .d(x21), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n115_), .O(z11));
  inv1   g75(.a(x24), .O(new_n119_));
  aoi21  g76(.a(x10), .b(x02), .c(new_n46_), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n65_), .b(x17), .c(new_n48_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(x01), .O(new_n123_));
  oai21  g80(.a(new_n123_), .b(new_n120_), .c(new_n119_), .O(new_n124_));
  nand2  g81(.a(new_n47_), .b(x00), .O(new_n125_));
  oai21  g82(.a(new_n124_), .b(new_n49_), .c(new_n125_), .O(z12));
  nand3  g83(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n127_));
  nor3   g84(.a(new_n127_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g85(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n129_));
  nor2   g86(.a(new_n129_), .b(x19), .O(z14));
  nand2  g87(.a(x19), .b(new_n48_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(new_n46_), .c(x01), .O(new_n132_));
  or2    g89(.a(new_n132_), .b(new_n120_), .O(z15));
  nor2   g90(.a(new_n47_), .b(x00), .O(z16));
  nand3  g91(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n135_));
  inv1   g92(.a(new_n135_), .O(z17));
endmodule


