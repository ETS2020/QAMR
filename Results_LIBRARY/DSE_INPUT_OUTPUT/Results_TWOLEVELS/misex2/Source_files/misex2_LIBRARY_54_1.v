// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n104_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n116_, new_n118_,
    new_n119_, new_n121_, new_n122_, new_n125_;
  inv1   g00(.a(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor3   g08(.a(new_n51_), .b(new_n47_), .c(x09), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  inv1   g11(.a(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x02), .O(new_n56_));
  nor2   g13(.a(x17), .b(new_n47_), .O(new_n57_));
  nor2   g14(.a(x19), .b(x18), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n48_), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x01), .O(z01));
  nor3   g17(.a(new_n51_), .b(x10), .c(new_n55_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z02));
  nand3  g20(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(x18), .c(new_n45_), .d(new_n44_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nand4  g24(.a(x12), .b(x11), .c(x10), .d(new_n55_), .O(new_n68_));
  oai22  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(new_n64_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n70_), .c(x10), .d(new_n55_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x12), .O(z04));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand2  g32(.a(x12), .b(new_n49_), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n75_), .c(new_n76_), .O(z05));
  nor3   g35(.a(new_n70_), .b(new_n47_), .c(x09), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n76_), .O(z06));
  nand3  g38(.a(new_n44_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n76_), .O(z07));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x07), .O(new_n88_));
  inv1   g45(.a(x04), .O(new_n89_));
  inv1   g46(.a(x03), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x02), .c(new_n49_), .d(new_n48_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x08), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n46_), .c(x17), .d(new_n44_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n65_), .O(z08));
  nand3  g53(.a(x18), .b(x01), .c(new_n48_), .O(new_n97_));
  nor2   g54(.a(x20), .b(x19), .O(new_n98_));
  nor2   g55(.a(x22), .b(x21), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n97_), .c(new_n76_), .O(z09));
  nand3  g58(.a(new_n98_), .b(x22), .c(x21), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n97_), .c(new_n76_), .O(z10));
  inv1   g60(.a(x22), .O(new_n104_));
  nand3  g61(.a(new_n98_), .b(new_n104_), .c(x21), .O(new_n105_));
  oai21  g62(.a(new_n105_), .b(new_n97_), .c(new_n76_), .O(z11));
  aoi21  g63(.a(x10), .b(x02), .c(new_n49_), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(x00), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n109_));
  nand3  g66(.a(new_n65_), .b(x17), .c(new_n50_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n44_), .c(new_n49_), .d(new_n48_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n108_), .c(x24), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(x09), .O(new_n114_));
  inv1   g71(.a(new_n114_), .O(z12));
  nand4  g72(.a(new_n65_), .b(x17), .c(new_n50_), .d(new_n48_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n44_), .c(x01), .O(z13));
  nor3   g74(.a(x09), .b(x02), .c(x00), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n58_), .c(new_n45_), .d(new_n47_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g77(.a(new_n44_), .b(new_n50_), .c(new_n107_), .O(new_n121_));
  nand4  g78(.a(x19), .b(new_n44_), .c(new_n50_), .d(new_n49_), .O(new_n122_));
  oai21  g79(.a(new_n121_), .b(new_n48_), .c(new_n122_), .O(z15));
  nor2   g80(.a(new_n49_), .b(x00), .O(z16));
  nand2  g81(.a(x02), .b(new_n48_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n44_), .c(x01), .O(z17));
endmodule


