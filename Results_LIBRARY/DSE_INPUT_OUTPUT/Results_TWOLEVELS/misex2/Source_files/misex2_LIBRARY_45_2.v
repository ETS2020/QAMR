// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n147_;
  inv1   g000(.a(x01), .O(new_n44_));
  nor3   g001(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g002(.a(x19), .b(x17), .O(new_n46_));
  nand3  g003(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g004(.a(new_n47_), .b(new_n44_), .c(x18), .O(z00));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x00), .O(new_n51_));
  inv1   g008(.a(x02), .O(new_n52_));
  nand4  g009(.a(x09), .b(new_n52_), .c(new_n44_), .d(new_n51_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n54_), .b(new_n50_), .c(new_n49_), .d(new_n57_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z02));
  inv1   g016(.a(x09), .O(new_n60_));
  nand3  g017(.a(new_n60_), .b(x02), .c(x00), .O(new_n61_));
  nand4  g018(.a(x18), .b(x12), .c(x11), .d(x10), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n61_), .c(x18), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nor3   g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n49_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n64_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(new_n50_), .O(z04));
  nand4  g031(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n75_));
  aoi21  g032(.a(new_n75_), .b(x18), .c(new_n44_), .O(z05));
  nand3  g033(.a(x18), .b(x11), .c(x10), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(new_n60_), .c(x02), .d(x01), .O(new_n79_));
  nor2   g036(.a(new_n79_), .b(new_n51_), .O(z06));
  nand2  g037(.a(x10), .b(new_n60_), .O(new_n81_));
  nand3  g038(.a(x18), .b(new_n70_), .c(x11), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(new_n81_), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n50_), .b(x01), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n84_), .O(z07));
  nand2  g043(.a(x02), .b(new_n51_), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(x04), .O(new_n89_));
  nor3   g046(.a(new_n89_), .b(new_n87_), .c(x03), .O(new_n90_));
  nor2   g047(.a(x07), .b(x06), .O(new_n91_));
  nor3   g048(.a(new_n65_), .b(new_n49_), .c(x08), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(new_n91_), .c(new_n90_), .O(new_n93_));
  aoi21  g050(.a(new_n93_), .b(new_n44_), .c(x18), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n69_), .c(x02), .d(new_n44_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n65_), .c(x18), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n51_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  nor2   g063(.a(x19), .b(new_n44_), .O(new_n107_));
  nor3   g064(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n44_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n69_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nand3  g070(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g071(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi22  g073(.a(new_n116_), .b(new_n112_), .c(new_n108_), .d(new_n107_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n85_), .O(z10));
  nor2   g075(.a(new_n52_), .b(x01), .O(new_n119_));
  nor2   g076(.a(x13), .b(new_n70_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n69_), .O(new_n121_));
  nand3  g078(.a(new_n107_), .b(x21), .c(new_n102_), .O(new_n122_));
  nor2   g079(.a(new_n98_), .b(x14), .O(new_n123_));
  nor2   g080(.a(x21), .b(new_n102_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n123_), .c(new_n99_), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(new_n121_), .c(new_n122_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n96_), .c(new_n51_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n85_), .O(z11));
  inv1   g085(.a(x24), .O(new_n129_));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n65_), .b(x17), .c(new_n52_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n44_), .c(new_n51_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n129_), .c(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n85_), .O(z12));
  nand3  g095(.a(new_n52_), .b(new_n44_), .c(new_n51_), .O(new_n139_));
  nor3   g096(.a(new_n139_), .b(x19), .c(new_n49_), .O(z13));
  nand3  g097(.a(new_n46_), .b(new_n45_), .c(new_n57_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(new_n44_), .c(x18), .O(z14));
  aoi22  g099(.a(new_n85_), .b(x00), .c(x19), .d(new_n44_), .O(new_n143_));
  nand4  g100(.a(x18), .b(new_n57_), .c(x01), .d(x00), .O(new_n144_));
  oai21  g101(.a(new_n143_), .b(x02), .c(new_n144_), .O(z15));
  aoi21  g102(.a(x18), .b(x00), .c(new_n44_), .O(z16));
  nand3  g103(.a(x02), .b(new_n44_), .c(new_n51_), .O(new_n147_));
  inv1   g104(.a(new_n147_), .O(z17));
endmodule


