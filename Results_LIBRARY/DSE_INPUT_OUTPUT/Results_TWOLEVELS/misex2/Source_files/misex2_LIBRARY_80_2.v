// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n125_, new_n126_, new_n127_, new_n130_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x00), .O(new_n53_));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n46_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n53_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n46_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n53_), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nand3  g17(.a(new_n46_), .b(new_n60_), .c(new_n53_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n44_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x10), .c(new_n47_), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(x12), .c(x11), .O(z04));
  nor3   g27(.a(new_n67_), .b(new_n57_), .c(new_n47_), .O(z05));
  nand4  g28(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n53_), .O(z06));
  inv1   g30(.a(x11), .O(new_n74_));
  nor3   g31(.a(new_n69_), .b(x12), .c(new_n74_), .O(z07));
  inv1   g32(.a(x08), .O(new_n76_));
  inv1   g33(.a(x05), .O(new_n77_));
  inv1   g34(.a(x06), .O(new_n78_));
  inv1   g35(.a(x03), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x02), .c(new_n60_), .d(new_n53_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(x04), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n45_), .c(x17), .d(new_n76_), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(new_n62_), .O(z08));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x12), .c(new_n74_), .d(x02), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x22), .b(x21), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  oai21  g48(.a(new_n91_), .b(new_n87_), .c(new_n53_), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n60_), .O(new_n93_));
  nor3   g50(.a(new_n45_), .b(new_n60_), .c(x00), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n90_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n93_), .O(z09));
  nand4  g54(.a(new_n90_), .b(x20), .c(x16), .d(x15), .O(new_n98_));
  oai21  g55(.a(new_n98_), .b(new_n87_), .c(new_n53_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n60_), .O(new_n100_));
  nand4  g57(.a(new_n95_), .b(new_n94_), .c(x22), .d(x21), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n100_), .O(z10));
  inv1   g59(.a(x22), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(x20), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n62_), .c(x18), .d(x01), .O(new_n106_));
  inv1   g63(.a(x13), .O(new_n107_));
  nor2   g64(.a(new_n46_), .b(x01), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n107_), .c(x12), .d(new_n74_), .O(new_n109_));
  nor2   g66(.a(new_n88_), .b(x14), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n104_), .c(x20), .d(new_n89_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n109_), .c(new_n106_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n103_), .c(new_n53_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n53_), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n62_), .b(x17), .c(new_n46_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n117_), .c(x01), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n115_), .O(new_n120_));
  oai22  g77(.a(new_n120_), .b(new_n47_), .c(x01), .d(new_n53_), .O(z12));
  aoi21  g78(.a(new_n118_), .b(new_n53_), .c(x01), .O(z13));
  nand4  g79(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(x19), .O(z14));
  nand2  g81(.a(x10), .b(x02), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  nand3  g83(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(z15));
  xor2a  g85(.a(x01), .b(x00), .O(z16));
  nand2  g86(.a(new_n48_), .b(x02), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z17));
endmodule


