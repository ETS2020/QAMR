// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:27 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n136_,
    new_n138_, new_n139_, new_n142_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x04), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nand3  g10(.a(x09), .b(new_n48_), .c(new_n46_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nor2   g12(.a(x19), .b(x18), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n55_), .c(new_n44_), .d(x10), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n49_), .c(x01), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n55_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n49_), .c(x01), .O(z02));
  nand2  g18(.a(new_n48_), .b(new_n46_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(x18), .c(new_n44_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n62_), .c(new_n49_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n47_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(x09), .O(new_n68_));
  nand4  g25(.a(x12), .b(x11), .c(x10), .d(new_n68_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  inv1   g27(.a(x11), .O(new_n71_));
  nand3  g28(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g29(.a(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n71_), .c(x10), .d(new_n68_), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(x12), .O(z04));
  nand2  g32(.a(x01), .b(x00), .O(new_n76_));
  nand2  g33(.a(x04), .b(new_n47_), .O(new_n77_));
  nand3  g34(.a(x10), .b(x09), .c(x02), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(new_n76_), .c(new_n77_), .O(z05));
  nand4  g36(.a(x11), .b(x10), .c(new_n68_), .d(x02), .O(new_n80_));
  nor3   g37(.a(new_n80_), .b(new_n47_), .c(new_n46_), .O(z06));
  nand2  g38(.a(new_n68_), .b(x01), .O(new_n82_));
  inv1   g39(.a(x12), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x11), .c(x10), .O(new_n84_));
  oai22  g41(.a(new_n84_), .b(new_n82_), .c(x04), .d(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z07));
  inv1   g44(.a(x21), .O(new_n89_));
  inv1   g45(.a(x22), .O(new_n90_));
  nor2   g46(.a(new_n48_), .b(x01), .O(new_n91_));
  nor2   g47(.a(new_n83_), .b(x11), .O(new_n92_));
  nand3  g48(.a(new_n92_), .b(new_n91_), .c(new_n49_), .O(new_n93_));
  inv1   g49(.a(x15), .O(new_n94_));
  inv1   g50(.a(x16), .O(new_n95_));
  nor2   g51(.a(x14), .b(x13), .O(new_n96_));
  nand4  g52(.a(new_n96_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g53(.a(x20), .O(new_n98_));
  nor2   g54(.a(new_n45_), .b(new_n47_), .O(new_n99_));
  nand3  g55(.a(new_n99_), .b(new_n98_), .c(new_n63_), .O(new_n100_));
  oai21  g56(.a(new_n97_), .b(new_n93_), .c(new_n100_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n90_), .c(new_n89_), .d(new_n46_), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(z09));
  nand3  g59(.a(new_n63_), .b(x18), .c(x01), .O(new_n104_));
  nand3  g60(.a(x22), .b(x21), .c(new_n98_), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g62(.a(x02), .b(new_n47_), .O(new_n107_));
  inv1   g63(.a(x13), .O(new_n108_));
  nand3  g64(.a(new_n108_), .b(x12), .c(new_n71_), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  inv1   g66(.a(x14), .O(new_n111_));
  nand3  g67(.a(x16), .b(x15), .c(new_n111_), .O(new_n112_));
  nand3  g68(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n113_));
  nor2   g69(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  aoi21  g70(.a(new_n114_), .b(new_n110_), .c(new_n106_), .O(new_n115_));
  oai21  g71(.a(new_n115_), .b(x00), .c(new_n77_), .O(z10));
  nand4  g72(.a(new_n99_), .b(x21), .c(new_n98_), .d(new_n63_), .O(new_n117_));
  nor2   g73(.a(new_n94_), .b(x14), .O(new_n118_));
  nor2   g74(.a(x21), .b(new_n98_), .O(new_n119_));
  nand4  g75(.a(new_n119_), .b(new_n118_), .c(new_n95_), .d(new_n108_), .O(new_n120_));
  oai21  g76(.a(new_n120_), .b(new_n93_), .c(new_n117_), .O(new_n121_));
  nand3  g77(.a(new_n121_), .b(new_n90_), .c(new_n46_), .O(new_n122_));
  inv1   g78(.a(new_n122_), .O(z11));
  inv1   g79(.a(x24), .O(new_n124_));
  nand2  g80(.a(x10), .b(x02), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  oai21  g82(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g83(.a(new_n63_), .b(x17), .c(new_n48_), .O(new_n128_));
  nand2  g84(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g85(.a(new_n129_), .b(new_n47_), .c(new_n46_), .O(new_n130_));
  nand2  g86(.a(new_n130_), .b(new_n126_), .O(new_n131_));
  nand3  g87(.a(new_n131_), .b(new_n124_), .c(x09), .O(new_n132_));
  nand2  g88(.a(new_n132_), .b(new_n77_), .O(z12));
  nand4  g89(.a(new_n63_), .b(x17), .c(new_n48_), .d(new_n46_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(new_n49_), .c(x01), .O(z13));
  nand4  g91(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n136_));
  nor2   g92(.a(new_n136_), .b(x19), .O(z14));
  aoi21  g93(.a(new_n59_), .b(x01), .c(new_n48_), .O(new_n138_));
  aoi21  g94(.a(x19), .b(new_n48_), .c(x04), .O(new_n139_));
  oai22  g95(.a(new_n139_), .b(x01), .c(new_n138_), .d(new_n46_), .O(z15));
  nor2   g96(.a(new_n47_), .b(x00), .O(z16));
  nand4  g97(.a(new_n49_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n142_));
  inv1   g98(.a(new_n142_), .O(z17));
  zero   g99(.O(z08));
endmodule


