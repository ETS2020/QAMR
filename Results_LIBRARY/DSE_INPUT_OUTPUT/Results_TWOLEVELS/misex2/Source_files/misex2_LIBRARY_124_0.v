// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:18 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n105_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n117_,
    new_n120_, new_n121_, new_n122_;
  inv1   g00(.a(x13), .O(new_n44_));
  inv1   g01(.a(x22), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  inv1   g04(.a(x09), .O(new_n48_));
  nor2   g05(.a(x01), .b(x00), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n46_), .O(z00));
  nand3  g12(.a(new_n49_), .b(x09), .c(new_n47_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n46_), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n58_), .O(new_n59_));
  oai21  g16(.a(new_n59_), .b(new_n56_), .c(new_n46_), .O(z02));
  nor3   g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  inv1   g22(.a(x12), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n64_), .c(x10), .d(new_n48_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(new_n62_), .c(new_n46_), .O(z03));
  inv1   g26(.a(x00), .O(new_n70_));
  nand4  g27(.a(new_n46_), .b(new_n66_), .c(new_n65_), .d(x10), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n48_), .c(x02), .d(x01), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n70_), .O(z04));
  nor2   g31(.a(x22), .b(x13), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(new_n58_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x09), .c(x02), .d(x01), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n70_), .O(z05));
  nand2  g35(.a(new_n75_), .b(new_n66_), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x10), .d(new_n48_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(x02), .c(x01), .d(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n46_), .O(z06));
  nand3  g40(.a(new_n66_), .b(x11), .c(x10), .O(new_n84_));
  oai21  g41(.a(new_n84_), .b(x09), .c(x01), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n46_), .c(x02), .d(x00), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z07));
  inv1   g44(.a(x01), .O(new_n88_));
  nand3  g45(.a(x02), .b(new_n88_), .c(new_n70_), .O(new_n89_));
  inv1   g46(.a(x05), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x04), .O(new_n91_));
  nor3   g48(.a(new_n91_), .b(new_n89_), .c(x03), .O(new_n92_));
  nor3   g49(.a(x08), .b(x07), .c(x06), .O(new_n93_));
  nor2   g50(.a(new_n53_), .b(x18), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(x17), .O(new_n95_));
  nand2  g52(.a(new_n95_), .b(new_n46_), .O(z08));
  inv1   g53(.a(x20), .O(new_n97_));
  nand4  g54(.a(x18), .b(x13), .c(x01), .d(new_n70_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n97_), .c(new_n53_), .O(new_n100_));
  nor3   g57(.a(new_n100_), .b(x22), .c(x21), .O(z09));
  nand3  g58(.a(x18), .b(x01), .c(new_n70_), .O(new_n102_));
  nand4  g59(.a(x22), .b(x21), .c(new_n97_), .d(new_n53_), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n102_), .c(new_n46_), .O(z10));
  inv1   g61(.a(x21), .O(new_n105_));
  nor3   g62(.a(new_n100_), .b(x22), .c(new_n105_), .O(z11));
  inv1   g63(.a(x24), .O(new_n107_));
  nand2  g64(.a(x10), .b(x02), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x01), .c(x00), .O(new_n109_));
  oai21  g66(.a(x19), .b(x02), .c(x23), .O(new_n110_));
  nand3  g67(.a(new_n53_), .b(x17), .c(new_n47_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n88_), .c(new_n70_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n109_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n107_), .c(x09), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n46_), .O(z12));
  inv1   g73(.a(new_n49_), .O(new_n117_));
  oai21  g74(.a(new_n111_), .b(new_n117_), .c(new_n46_), .O(z13));
  oai21  g75(.a(new_n59_), .b(new_n50_), .c(new_n46_), .O(z14));
  oai21  g76(.a(x10), .b(new_n88_), .c(x02), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x00), .O(new_n121_));
  nand3  g78(.a(x19), .b(new_n47_), .c(new_n88_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n121_), .c(new_n75_), .O(z15));
  nor3   g80(.a(new_n75_), .b(new_n88_), .c(x00), .O(z16));
  nand2  g81(.a(new_n89_), .b(new_n46_), .O(z17));
endmodule


