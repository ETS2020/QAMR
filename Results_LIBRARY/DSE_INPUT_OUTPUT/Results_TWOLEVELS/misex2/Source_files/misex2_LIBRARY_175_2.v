// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n142_, new_n144_;
  inv1   g000(.a(x24), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nor3   g011(.a(x02), .b(x01), .c(x00), .O(new_n55_));
  nand2  g012(.a(new_n55_), .b(x09), .O(new_n56_));
  inv1   g013(.a(new_n56_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z01));
  nand3  g016(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n60_));
  inv1   g017(.a(x10), .O(new_n61_));
  nand4  g018(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n61_), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n60_), .c(new_n45_), .O(z02));
  nand3  g020(.a(new_n47_), .b(x02), .c(x00), .O(new_n64_));
  nand3  g021(.a(x12), .b(x11), .c(x10), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(x24), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(x01), .O(new_n67_));
  nor3   g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g025(.a(x19), .b(new_n51_), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n68_), .c(new_n50_), .O(new_n70_));
  nand2  g027(.a(new_n70_), .b(new_n67_), .O(z03));
  inv1   g028(.a(x01), .O(new_n72_));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  inv1   g031(.a(new_n64_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(x10), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(x24), .c(new_n72_), .O(z04));
  nand4  g034(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n78_));
  aoi21  g035(.a(new_n78_), .b(x24), .c(new_n72_), .O(z05));
  nand3  g036(.a(x02), .b(x01), .c(x00), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(x11), .c(x10), .d(new_n47_), .O(new_n82_));
  nor2   g039(.a(new_n82_), .b(new_n44_), .O(z06));
  nand3  g040(.a(new_n74_), .b(x11), .c(x10), .O(new_n84_));
  oai21  g041(.a(new_n84_), .b(x09), .c(x01), .O(new_n85_));
  nand3  g042(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n45_), .O(z07));
  inv1   g044(.a(x08), .O(new_n88_));
  inv1   g045(.a(x05), .O(new_n89_));
  inv1   g046(.a(x06), .O(new_n90_));
  inv1   g047(.a(x00), .O(new_n91_));
  inv1   g048(.a(x03), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(x02), .c(new_n72_), .d(new_n91_), .O(new_n93_));
  inv1   g050(.a(new_n93_), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(x04), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(x07), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n51_), .c(x17), .d(new_n88_), .O(new_n97_));
  nor2   g054(.a(new_n97_), .b(new_n52_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  nor2   g057(.a(new_n46_), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x12), .c(new_n73_), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nor2   g061(.a(x14), .b(x13), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(x20), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  nor2   g063(.a(new_n51_), .b(new_n72_), .O(new_n107_));
  nor2   g064(.a(x20), .b(x19), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  oai21  g066(.a(new_n106_), .b(new_n102_), .c(new_n109_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n100_), .c(new_n99_), .d(new_n91_), .O(new_n111_));
  nand2  g068(.a(new_n111_), .b(new_n45_), .O(z09));
  nor2   g069(.a(x13), .b(new_n74_), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(new_n101_), .c(new_n73_), .O(new_n114_));
  nor3   g071(.a(new_n104_), .b(new_n103_), .c(x14), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n100_), .c(new_n99_), .d(x20), .O(new_n116_));
  or2    g073(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  nor2   g074(.a(new_n99_), .b(x20), .O(new_n118_));
  nor2   g075(.a(new_n44_), .b(new_n100_), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n118_), .c(new_n69_), .d(x01), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n117_), .c(x00), .O(z10));
  nand3  g078(.a(new_n118_), .b(new_n107_), .c(new_n52_), .O(new_n122_));
  nor2   g079(.a(new_n103_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n99_), .c(x20), .d(new_n104_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n114_), .c(new_n122_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n100_), .c(new_n91_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n45_), .O(z11));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand4  g087(.a(new_n130_), .b(new_n44_), .c(x09), .d(new_n72_), .O(new_n131_));
  nor2   g088(.a(new_n131_), .b(x00), .O(z12));
  nand2  g089(.a(new_n55_), .b(x17), .O(new_n133_));
  nor2   g090(.a(new_n133_), .b(x19), .O(z13));
  nand2  g091(.a(new_n55_), .b(new_n47_), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(new_n136_));
  nand4  g093(.a(new_n136_), .b(new_n51_), .c(new_n50_), .d(new_n61_), .O(new_n137_));
  nor2   g094(.a(new_n137_), .b(x19), .O(z14));
  aoi22  g095(.a(new_n45_), .b(x00), .c(x19), .d(new_n72_), .O(new_n139_));
  nand4  g096(.a(x24), .b(new_n61_), .c(x01), .d(x00), .O(new_n140_));
  oai21  g097(.a(new_n139_), .b(x02), .c(new_n140_), .O(z15));
  nand3  g098(.a(x24), .b(x01), .c(new_n91_), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z16));
  nand2  g100(.a(new_n48_), .b(x02), .O(new_n144_));
  inv1   g101(.a(new_n144_), .O(z17));
endmodule


