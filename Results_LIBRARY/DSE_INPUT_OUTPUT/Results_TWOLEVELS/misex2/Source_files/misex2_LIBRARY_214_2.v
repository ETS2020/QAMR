// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x13), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  inv1   g002(.a(x18), .O(new_n46_));
  inv1   g003(.a(x10), .O(new_n47_));
  inv1   g004(.a(x00), .O(new_n48_));
  inv1   g005(.a(x01), .O(new_n49_));
  inv1   g006(.a(x02), .O(new_n50_));
  inv1   g007(.a(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(new_n47_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z00));
  nor3   g012(.a(x02), .b(x01), .c(x00), .O(new_n56_));
  nor2   g013(.a(x19), .b(x18), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n56_), .c(x10), .d(x09), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(new_n44_), .c(x17), .O(z01));
  nand2  g016(.a(new_n49_), .b(new_n48_), .O(new_n60_));
  nor4   g017(.a(new_n60_), .b(x10), .c(new_n51_), .d(x02), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  inv1   g020(.a(x19), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(x18), .c(new_n50_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n60_), .c(new_n44_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n45_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nor2   g025(.a(new_n47_), .b(x09), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(x12), .c(x11), .O(new_n70_));
  oai21  g027(.a(new_n70_), .b(new_n68_), .c(new_n67_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  nor2   g030(.a(x17), .b(new_n44_), .O(new_n74_));
  inv1   g031(.a(new_n74_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n73_), .c(new_n72_), .d(x10), .O(new_n76_));
  inv1   g033(.a(new_n76_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(new_n51_), .c(x02), .d(x01), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n48_), .O(z04));
  nand2  g036(.a(x01), .b(x00), .O(new_n80_));
  nand3  g037(.a(x10), .b(x09), .c(x02), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n80_), .c(new_n75_), .O(z05));
  nor2   g039(.a(new_n74_), .b(new_n72_), .O(new_n83_));
  nand4  g040(.a(new_n83_), .b(x10), .c(new_n51_), .d(x02), .O(new_n84_));
  nor3   g041(.a(new_n84_), .b(new_n49_), .c(new_n48_), .O(z06));
  nand3  g042(.a(new_n69_), .b(new_n73_), .c(x11), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(x01), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x02), .c(x00), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(new_n75_), .O(z07));
  nor2   g046(.a(new_n50_), .b(x01), .O(new_n90_));
  nand2  g047(.a(new_n90_), .b(new_n48_), .O(new_n91_));
  inv1   g048(.a(x05), .O(new_n92_));
  nand2  g049(.a(new_n92_), .b(x04), .O(new_n93_));
  nor3   g050(.a(new_n93_), .b(new_n91_), .c(x03), .O(new_n94_));
  nor3   g051(.a(x08), .b(x07), .c(x06), .O(new_n95_));
  nor2   g052(.a(new_n64_), .b(x18), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x17), .O(new_n97_));
  nand2  g054(.a(new_n97_), .b(new_n75_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  nor2   g057(.a(new_n74_), .b(x20), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(new_n64_), .c(x18), .d(x01), .O(new_n102_));
  nor2   g059(.a(new_n73_), .b(x11), .O(new_n103_));
  inv1   g060(.a(x15), .O(new_n104_));
  inv1   g061(.a(x16), .O(new_n105_));
  nand3  g062(.a(x20), .b(new_n105_), .c(new_n104_), .O(new_n106_));
  nor3   g063(.a(new_n106_), .b(x14), .c(x13), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n103_), .c(new_n90_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n102_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n100_), .c(new_n99_), .d(new_n48_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(z09));
  inv1   g068(.a(x20), .O(new_n112_));
  nand4  g069(.a(new_n75_), .b(x22), .c(x21), .d(new_n112_), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n64_), .c(x18), .d(x01), .O(new_n115_));
  nand4  g072(.a(new_n90_), .b(new_n44_), .c(x12), .d(new_n72_), .O(new_n116_));
  nor3   g073(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n117_));
  nand4  g074(.a(new_n117_), .b(new_n100_), .c(new_n99_), .d(x20), .O(new_n118_));
  or2    g075(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n115_), .c(x00), .O(z10));
  nor2   g077(.a(new_n74_), .b(new_n99_), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n112_), .c(new_n64_), .d(x18), .O(new_n122_));
  nor2   g079(.a(new_n104_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n99_), .c(x20), .d(new_n105_), .O(new_n124_));
  oai22  g081(.a(new_n124_), .b(new_n116_), .c(new_n122_), .d(new_n49_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n100_), .c(new_n48_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand4  g086(.a(new_n129_), .b(new_n75_), .c(x01), .d(x00), .O(new_n130_));
  nor2   g087(.a(x19), .b(x02), .O(new_n131_));
  oai21  g088(.a(new_n131_), .b(x13), .c(new_n45_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(x23), .O(new_n133_));
  nand3  g090(.a(new_n64_), .b(x17), .c(new_n50_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n49_), .c(new_n48_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n130_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n128_), .c(x09), .O(new_n138_));
  inv1   g095(.a(new_n138_), .O(z12));
  oai21  g096(.a(new_n134_), .b(new_n60_), .c(new_n75_), .O(z13));
  nor2   g097(.a(new_n52_), .b(x10), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z14));
  oai21  g100(.a(x10), .b(new_n49_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n50_), .c(new_n49_), .O(new_n146_));
  nand3  g103(.a(new_n146_), .b(new_n145_), .c(new_n75_), .O(z15));
  oai21  g104(.a(new_n49_), .b(x00), .c(new_n75_), .O(z16));
  nand2  g105(.a(new_n91_), .b(new_n75_), .O(z17));
endmodule


