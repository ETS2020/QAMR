// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n129_,
    new_n130_, new_n132_, new_n133_, new_n136_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x03), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(x03), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n44_), .d(x03), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n50_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n58_), .c(new_n50_), .O(z05));
  nand4  g29(.a(x11), .b(x10), .c(new_n50_), .d(x02), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(new_n52_), .c(new_n51_), .O(z06));
  inv1   g31(.a(x12), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x11), .c(x10), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(x09), .c(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  inv1   g35(.a(x03), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n78_), .O(z07));
  inv1   g38(.a(x21), .O(new_n83_));
  inv1   g39(.a(x22), .O(new_n84_));
  nor2   g40(.a(new_n53_), .b(x01), .O(new_n85_));
  nor2   g41(.a(new_n75_), .b(x11), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(new_n85_), .c(x03), .O(new_n87_));
  inv1   g43(.a(x15), .O(new_n88_));
  inv1   g44(.a(x16), .O(new_n89_));
  nor2   g45(.a(x14), .b(x13), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  inv1   g47(.a(x20), .O(new_n92_));
  nor2   g48(.a(new_n49_), .b(new_n52_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n92_), .c(new_n62_), .O(new_n94_));
  oai21  g50(.a(new_n91_), .b(new_n87_), .c(new_n94_), .O(new_n95_));
  nand4  g51(.a(new_n95_), .b(new_n84_), .c(new_n83_), .d(new_n51_), .O(new_n96_));
  inv1   g52(.a(new_n96_), .O(z09));
  nand3  g53(.a(new_n62_), .b(x18), .c(x01), .O(new_n98_));
  nand3  g54(.a(x22), .b(x21), .c(new_n92_), .O(new_n99_));
  nor2   g55(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  nand2  g56(.a(x02), .b(new_n52_), .O(new_n101_));
  inv1   g57(.a(x13), .O(new_n102_));
  nand3  g58(.a(new_n102_), .b(x12), .c(new_n67_), .O(new_n103_));
  nor2   g59(.a(new_n103_), .b(new_n101_), .O(new_n104_));
  inv1   g60(.a(x14), .O(new_n105_));
  nand3  g61(.a(x16), .b(x15), .c(new_n105_), .O(new_n106_));
  nand3  g62(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n107_));
  nor2   g63(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  aoi21  g64(.a(new_n108_), .b(new_n104_), .c(new_n100_), .O(new_n109_));
  oai21  g65(.a(new_n109_), .b(x00), .c(new_n80_), .O(z10));
  nand4  g66(.a(new_n93_), .b(x21), .c(new_n92_), .d(new_n62_), .O(new_n111_));
  nor2   g67(.a(new_n88_), .b(x14), .O(new_n112_));
  nor2   g68(.a(x21), .b(new_n92_), .O(new_n113_));
  nand4  g69(.a(new_n113_), .b(new_n112_), .c(new_n89_), .d(new_n102_), .O(new_n114_));
  oai21  g70(.a(new_n114_), .b(new_n87_), .c(new_n111_), .O(new_n115_));
  nand3  g71(.a(new_n115_), .b(new_n84_), .c(new_n51_), .O(new_n116_));
  inv1   g72(.a(new_n116_), .O(z11));
  inv1   g73(.a(x24), .O(new_n118_));
  nand2  g74(.a(x10), .b(x02), .O(new_n119_));
  nand3  g75(.a(new_n119_), .b(x01), .c(x00), .O(new_n120_));
  oai21  g76(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g77(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n122_));
  nand2  g78(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g79(.a(new_n123_), .b(new_n52_), .c(new_n51_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(new_n120_), .O(new_n125_));
  nand3  g81(.a(new_n125_), .b(new_n118_), .c(x09), .O(new_n126_));
  nand2  g82(.a(new_n126_), .b(new_n80_), .O(z12));
  nor3   g83(.a(new_n54_), .b(x19), .c(new_n44_), .O(z13));
  nor2   g84(.a(new_n54_), .b(x09), .O(new_n129_));
  nand4  g85(.a(new_n129_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n130_));
  nor2   g86(.a(new_n130_), .b(x19), .O(z14));
  aoi21  g87(.a(new_n58_), .b(x01), .c(new_n53_), .O(new_n132_));
  aoi21  g88(.a(x19), .b(new_n53_), .c(new_n79_), .O(new_n133_));
  oai22  g89(.a(new_n133_), .b(x01), .c(new_n132_), .d(new_n51_), .O(z15));
  nor2   g90(.a(new_n52_), .b(x00), .O(z16));
  nand2  g91(.a(x02), .b(new_n51_), .O(new_n136_));
  aoi21  g92(.a(new_n136_), .b(x03), .c(x01), .O(z17));
  zero   g93(.O(z08));
endmodule


