// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n148_;
  inv1   g000(.a(x10), .O(new_n44_));
  inv1   g001(.a(x23), .O(new_n45_));
  nand2  g002(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g003(.a(x02), .O(new_n47_));
  inv1   g004(.a(x09), .O(new_n48_));
  nor2   g005(.a(x01), .b(x00), .O(new_n49_));
  nand3  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  inv1   g007(.a(x17), .O(new_n51_));
  nor2   g008(.a(x19), .b(x18), .O(new_n52_));
  nand3  g009(.a(new_n52_), .b(new_n51_), .c(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n50_), .c(new_n46_), .O(z00));
  inv1   g011(.a(x18), .O(new_n55_));
  nand3  g012(.a(new_n49_), .b(x09), .c(new_n47_), .O(new_n56_));
  inv1   g013(.a(new_n56_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n55_), .c(new_n51_), .d(x10), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z01));
  nor3   g016(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n52_), .c(new_n51_), .d(x09), .O(new_n61_));
  aoi21  g018(.a(new_n61_), .b(x23), .c(x10), .O(z02));
  inv1   g019(.a(x00), .O(new_n63_));
  inv1   g020(.a(x01), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nand4  g022(.a(new_n46_), .b(new_n65_), .c(x18), .d(new_n51_), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n47_), .c(new_n64_), .d(new_n63_), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(new_n44_), .b(x09), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nand3  g031(.a(new_n70_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n69_), .c(new_n46_), .O(z04));
  nand2  g033(.a(x01), .b(x00), .O(new_n77_));
  nand3  g034(.a(x10), .b(x09), .c(x02), .O(new_n78_));
  oai21  g035(.a(new_n78_), .b(new_n77_), .c(new_n46_), .O(z05));
  nand4  g036(.a(x11), .b(x10), .c(new_n48_), .d(x02), .O(new_n80_));
  nor3   g037(.a(new_n80_), .b(new_n64_), .c(new_n63_), .O(z06));
  nand3  g038(.a(new_n74_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n46_), .O(z07));
  inv1   g042(.a(x03), .O(new_n86_));
  inv1   g043(.a(x04), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  inv1   g045(.a(x06), .O(new_n89_));
  inv1   g046(.a(x07), .O(new_n90_));
  nand4  g047(.a(new_n46_), .b(x19), .c(new_n55_), .d(x17), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(x08), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n87_), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n86_), .c(x02), .d(new_n64_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(x00), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nor2   g055(.a(new_n47_), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(x12), .c(new_n73_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nor2   g062(.a(new_n55_), .b(new_n64_), .O(new_n106_));
  nand3  g063(.a(new_n106_), .b(new_n105_), .c(new_n65_), .O(new_n107_));
  oai21  g064(.a(new_n104_), .b(new_n100_), .c(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n98_), .c(new_n97_), .d(new_n63_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n46_), .O(z09));
  nand3  g067(.a(new_n65_), .b(x18), .c(x01), .O(new_n111_));
  nor4   g068(.a(new_n111_), .b(new_n98_), .c(new_n97_), .d(x20), .O(new_n112_));
  nor2   g069(.a(x13), .b(new_n74_), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(new_n99_), .c(new_n73_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand3  g072(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n116_));
  nor4   g073(.a(new_n116_), .b(new_n102_), .c(new_n101_), .d(x14), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n115_), .c(new_n112_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n46_), .O(z10));
  nand4  g076(.a(new_n106_), .b(x21), .c(new_n105_), .d(new_n65_), .O(new_n120_));
  nor2   g077(.a(new_n101_), .b(x14), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n97_), .c(x20), .d(new_n102_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n114_), .c(new_n120_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n46_), .c(new_n98_), .d(new_n63_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(new_n47_), .O(new_n127_));
  oai21  g084(.a(new_n45_), .b(x10), .c(new_n127_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  nand3  g086(.a(new_n65_), .b(x10), .c(new_n47_), .O(new_n130_));
  aoi21  g087(.a(new_n130_), .b(new_n45_), .c(new_n51_), .O(new_n131_));
  aoi21  g088(.a(new_n65_), .b(new_n47_), .c(new_n45_), .O(new_n132_));
  oai21  g089(.a(new_n132_), .b(new_n131_), .c(new_n64_), .O(new_n133_));
  oai21  g090(.a(new_n133_), .b(x00), .c(new_n129_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n126_), .c(x09), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(z12));
  inv1   g093(.a(new_n49_), .O(new_n137_));
  nand3  g094(.a(new_n65_), .b(x17), .c(new_n47_), .O(new_n138_));
  oai21  g095(.a(new_n138_), .b(new_n137_), .c(new_n46_), .O(z13));
  nand4  g096(.a(new_n60_), .b(new_n52_), .c(new_n51_), .d(new_n48_), .O(new_n140_));
  aoi21  g097(.a(new_n140_), .b(x23), .c(x10), .O(z14));
  oai21  g098(.a(x10), .b(new_n64_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n47_), .c(new_n64_), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n143_), .c(new_n46_), .O(z15));
  nand2  g102(.a(new_n46_), .b(x01), .O(new_n146_));
  nor2   g103(.a(new_n146_), .b(x00), .O(z16));
  nand2  g104(.a(new_n99_), .b(new_n63_), .O(new_n148_));
  nand2  g105(.a(new_n148_), .b(new_n46_), .O(z17));
endmodule


