// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n143_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n151_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g009(.a(new_n52_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x19), .O(z01));
  inv1   g012(.a(x03), .O(new_n56_));
  nand2  g013(.a(new_n56_), .b(x01), .O(new_n57_));
  inv1   g014(.a(x00), .O(new_n58_));
  inv1   g015(.a(x01), .O(new_n59_));
  inv1   g016(.a(x02), .O(new_n60_));
  nand4  g017(.a(x09), .b(new_n60_), .c(new_n59_), .d(new_n58_), .O(new_n61_));
  inv1   g018(.a(x10), .O(new_n62_));
  inv1   g019(.a(x19), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(new_n45_), .c(new_n44_), .d(new_n62_), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n61_), .c(new_n57_), .O(z02));
  nand3  g022(.a(new_n60_), .b(new_n59_), .c(new_n58_), .O(new_n66_));
  nand3  g023(.a(new_n63_), .b(x18), .c(new_n44_), .O(new_n67_));
  nand4  g024(.a(x03), .b(x02), .c(x01), .d(x00), .O(new_n68_));
  nand4  g025(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n69_));
  oai22  g026(.a(new_n69_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(z03));
  nor2   g027(.a(x09), .b(new_n60_), .O(new_n71_));
  nor2   g028(.a(x12), .b(x11), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n71_), .c(x10), .d(x00), .O(new_n73_));
  aoi21  g030(.a(new_n73_), .b(x03), .c(new_n59_), .O(z04));
  nand4  g031(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n75_));
  aoi21  g032(.a(new_n75_), .b(x03), .c(new_n59_), .O(z05));
  inv1   g033(.a(x11), .O(new_n77_));
  nor3   g034(.a(new_n77_), .b(new_n62_), .c(x09), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x03), .c(x02), .d(x01), .O(new_n79_));
  nor2   g036(.a(new_n79_), .b(new_n58_), .O(z06));
  nand2  g037(.a(new_n46_), .b(x03), .O(new_n81_));
  inv1   g038(.a(x12), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(new_n81_), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n57_), .O(z07));
  nor2   g043(.a(new_n60_), .b(x00), .O(new_n87_));
  nor2   g044(.a(x06), .b(x05), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(x04), .O(new_n89_));
  inv1   g046(.a(new_n89_), .O(new_n90_));
  nand3  g047(.a(x19), .b(new_n45_), .c(x17), .O(new_n91_));
  nor3   g048(.a(new_n91_), .b(x08), .c(x07), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(new_n90_), .c(new_n87_), .O(new_n93_));
  aoi21  g050(.a(new_n93_), .b(new_n59_), .c(x03), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n77_), .c(x02), .d(new_n59_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  nor2   g058(.a(x20), .b(x19), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x18), .c(x03), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n58_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  nand3  g063(.a(new_n63_), .b(x18), .c(x01), .O(new_n107_));
  inv1   g064(.a(x20), .O(new_n108_));
  nand3  g065(.a(x22), .b(x21), .c(new_n108_), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n107_), .O(new_n110_));
  nand2  g067(.a(x02), .b(new_n59_), .O(new_n111_));
  inv1   g068(.a(x13), .O(new_n112_));
  nand3  g069(.a(new_n112_), .b(x12), .c(new_n77_), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  inv1   g071(.a(x14), .O(new_n115_));
  nand3  g072(.a(x16), .b(x15), .c(new_n115_), .O(new_n116_));
  nand3  g073(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n117_));
  nor2   g074(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n114_), .c(new_n110_), .O(new_n119_));
  oai21  g076(.a(new_n119_), .b(x00), .c(new_n57_), .O(z10));
  nor2   g077(.a(new_n60_), .b(x01), .O(new_n121_));
  nor2   g078(.a(x13), .b(new_n82_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n121_), .c(new_n77_), .O(new_n123_));
  nand3  g080(.a(x18), .b(x03), .c(x01), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(x21), .c(new_n108_), .d(new_n63_), .O(new_n126_));
  nor2   g083(.a(new_n98_), .b(x14), .O(new_n127_));
  nor2   g084(.a(x21), .b(new_n108_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n127_), .c(new_n99_), .O(new_n129_));
  oai21  g086(.a(new_n129_), .b(new_n123_), .c(new_n126_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n96_), .c(new_n58_), .O(new_n131_));
  inv1   g088(.a(new_n131_), .O(z11));
  inv1   g089(.a(x24), .O(new_n133_));
  nand2  g090(.a(x10), .b(x02), .O(new_n134_));
  nand4  g091(.a(new_n134_), .b(x03), .c(x01), .d(x00), .O(new_n135_));
  oai21  g092(.a(x19), .b(x02), .c(x23), .O(new_n136_));
  nand3  g093(.a(new_n63_), .b(x17), .c(new_n60_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n136_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n59_), .c(new_n58_), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n135_), .O(new_n140_));
  nand3  g097(.a(new_n140_), .b(new_n133_), .c(x09), .O(new_n141_));
  inv1   g098(.a(new_n141_), .O(z12));
  nand2  g099(.a(new_n47_), .b(x17), .O(new_n143_));
  nor2   g100(.a(new_n143_), .b(x19), .O(z13));
  nand4  g101(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n62_), .O(new_n145_));
  nor2   g102(.a(new_n145_), .b(x19), .O(z14));
  oai21  g103(.a(x10), .b(new_n59_), .c(x02), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(x00), .O(new_n148_));
  nand3  g105(.a(x19), .b(new_n60_), .c(new_n59_), .O(new_n149_));
  nand3  g106(.a(new_n149_), .b(new_n148_), .c(new_n57_), .O(z15));
  nand3  g107(.a(x03), .b(x01), .c(new_n58_), .O(new_n151_));
  inv1   g108(.a(new_n151_), .O(z16));
  oai21  g109(.a(new_n111_), .b(x00), .c(new_n57_), .O(z17));
endmodule


