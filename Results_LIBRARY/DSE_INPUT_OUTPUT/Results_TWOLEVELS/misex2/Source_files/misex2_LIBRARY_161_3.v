// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:29 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n142_, new_n144_, new_n146_, new_n147_, new_n150_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand3  g009(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z02));
  inv1   g016(.a(x15), .O(new_n60_));
  nand3  g017(.a(new_n47_), .b(x02), .c(x00), .O(new_n61_));
  nand4  g018(.a(new_n60_), .b(x12), .c(x11), .d(x10), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nor3   g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n64_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor2   g028(.a(new_n71_), .b(x09), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(x15), .O(z04));
  inv1   g031(.a(x01), .O(new_n75_));
  nand4  g032(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(new_n60_), .c(new_n75_), .O(z05));
  nand4  g034(.a(new_n60_), .b(x11), .c(x10), .d(new_n47_), .O(new_n78_));
  inv1   g035(.a(new_n78_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(z06));
  nand2  g038(.a(x10), .b(new_n47_), .O(new_n82_));
  nand3  g039(.a(new_n60_), .b(new_n70_), .c(x11), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(new_n82_), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(x15), .b(x01), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n85_), .O(z07));
  inv1   g044(.a(x00), .O(new_n88_));
  inv1   g045(.a(x03), .O(new_n89_));
  nor2   g046(.a(new_n46_), .b(x01), .O(new_n90_));
  inv1   g047(.a(x04), .O(new_n91_));
  nor2   g048(.a(x05), .b(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n90_), .c(new_n89_), .d(new_n88_), .O(new_n93_));
  nor3   g050(.a(x08), .b(x07), .c(x06), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(x19), .c(new_n45_), .d(x17), .O(new_n95_));
  oai21  g052(.a(new_n95_), .b(new_n93_), .c(new_n86_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nand4  g055(.a(x12), .b(new_n69_), .c(x02), .d(new_n75_), .O(new_n99_));
  inv1   g056(.a(x16), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x20), .c(new_n100_), .d(new_n60_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n65_), .c(x18), .d(x01), .O(new_n104_));
  oai21  g061(.a(new_n102_), .b(new_n99_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n98_), .c(new_n97_), .d(new_n88_), .O(new_n106_));
  nand2  g063(.a(new_n106_), .b(new_n86_), .O(z09));
  nand3  g064(.a(new_n65_), .b(x18), .c(x01), .O(new_n108_));
  nand3  g065(.a(x22), .b(x21), .c(new_n103_), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g067(.a(new_n69_), .b(x02), .O(new_n111_));
  nand2  g068(.a(new_n101_), .b(x12), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand2  g070(.a(x16), .b(x15), .O(new_n114_));
  nand3  g071(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n113_), .c(new_n110_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n86_), .O(z10));
  nor2   g075(.a(new_n45_), .b(x15), .O(new_n119_));
  nor2   g076(.a(new_n97_), .b(x20), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n119_), .c(new_n65_), .d(x01), .O(new_n121_));
  inv1   g078(.a(x13), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(x12), .c(new_n69_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(new_n124_));
  nor2   g081(.a(new_n60_), .b(x14), .O(new_n125_));
  nand3  g082(.a(new_n97_), .b(x20), .c(new_n100_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(new_n125_), .c(new_n124_), .d(new_n90_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n121_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n98_), .c(new_n88_), .O(new_n130_));
  inv1   g087(.a(new_n130_), .O(z11));
  inv1   g088(.a(x24), .O(new_n132_));
  nand2  g089(.a(x10), .b(x02), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(x01), .c(x00), .O(new_n134_));
  oai21  g091(.a(x19), .b(x02), .c(x23), .O(new_n135_));
  nand3  g092(.a(new_n65_), .b(x17), .c(new_n46_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n135_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n75_), .c(new_n88_), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n134_), .O(new_n139_));
  nand3  g096(.a(new_n139_), .b(new_n132_), .c(x09), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(new_n86_), .O(z12));
  inv1   g098(.a(new_n48_), .O(new_n142_));
  oai21  g099(.a(new_n136_), .b(new_n142_), .c(new_n86_), .O(z13));
  nand4  g100(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n144_));
  nor2   g101(.a(new_n144_), .b(x19), .O(z14));
  aoi22  g102(.a(new_n86_), .b(x00), .c(x19), .d(new_n75_), .O(new_n146_));
  nand4  g103(.a(new_n60_), .b(new_n57_), .c(x01), .d(x00), .O(new_n147_));
  oai21  g104(.a(new_n146_), .b(x02), .c(new_n147_), .O(z15));
  aoi21  g105(.a(new_n60_), .b(x00), .c(new_n75_), .O(z16));
  nand2  g106(.a(new_n48_), .b(x02), .O(new_n150_));
  inv1   g107(.a(new_n150_), .O(z17));
endmodule


