// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n102_, new_n103_,
    new_n104_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n114_, new_n115_, new_n117_, new_n118_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x21), .c(x01), .O(z00));
  inv1   g05(.a(x21), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x00), .O(new_n53_));
  inv1   g10(.a(x01), .O(new_n54_));
  inv1   g11(.a(x02), .O(new_n55_));
  nand4  g12(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(new_n52_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(new_n49_), .O(z01));
  nor2   g16(.a(new_n56_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(new_n49_), .O(z02));
  nand3  g19(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n63_));
  nand4  g20(.a(x21), .b(new_n51_), .c(x18), .d(new_n44_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g22(.a(x09), .O(new_n66_));
  nand4  g23(.a(x12), .b(x11), .c(x10), .d(new_n66_), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(z03));
  nand2  g25(.a(new_n49_), .b(new_n54_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n66_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n65_), .c(new_n69_), .O(z04));
  nand4  g30(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n52_), .O(z05));
  nand4  g32(.a(x11), .b(x10), .c(new_n66_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n54_), .c(new_n53_), .O(z06));
  nand3  g34(.a(new_n71_), .b(x11), .c(x10), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n69_), .O(z07));
  nand2  g38(.a(x02), .b(new_n53_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x04), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(x03), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nor2   g43(.a(new_n51_), .b(x18), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x17), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(x21), .c(x01), .O(z08));
  nor2   g46(.a(x22), .b(x20), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n51_), .c(x18), .d(new_n53_), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(x01), .c(x21), .O(z09));
  inv1   g49(.a(x20), .O(new_n93_));
  nand4  g50(.a(new_n51_), .b(x18), .c(x01), .d(new_n53_), .O(new_n94_));
  inv1   g51(.a(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(x22), .c(x21), .d(new_n93_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z10));
  nand3  g54(.a(x18), .b(x01), .c(new_n53_), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x21), .c(new_n93_), .d(new_n51_), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n98_), .c(new_n69_), .O(z11));
  inv1   g58(.a(x24), .O(new_n102_));
  aoi21  g59(.a(x10), .b(x02), .c(new_n54_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(x00), .O(new_n104_));
  oai21  g61(.a(x19), .b(x02), .c(x23), .O(new_n105_));
  nand3  g62(.a(new_n51_), .b(x17), .c(new_n55_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n105_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(x21), .c(new_n54_), .d(new_n53_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n104_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n102_), .c(x09), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z12));
  nand3  g68(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n112_));
  nor4   g69(.a(new_n112_), .b(new_n49_), .c(x19), .d(new_n44_), .O(z13));
  nor3   g70(.a(new_n112_), .b(x10), .c(x09), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n49_), .O(z14));
  aoi21  g73(.a(x21), .b(new_n55_), .c(new_n103_), .O(new_n117_));
  nand4  g74(.a(x21), .b(x19), .c(new_n55_), .d(new_n54_), .O(new_n118_));
  oai21  g75(.a(new_n117_), .b(new_n53_), .c(new_n118_), .O(z15));
  oai21  g76(.a(new_n54_), .b(x00), .c(new_n69_), .O(z16));
  aoi21  g77(.a(new_n82_), .b(x21), .c(x01), .O(z17));
endmodule


