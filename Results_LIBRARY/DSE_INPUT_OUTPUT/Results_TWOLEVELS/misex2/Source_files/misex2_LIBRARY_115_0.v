// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n135_, new_n138_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x17), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(x15), .c(x01), .O(z00));
  nand3  g09(.a(x09), .b(new_n45_), .c(new_n44_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(x15), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n48_), .c(new_n47_), .d(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(x15), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nand3  g17(.a(new_n45_), .b(new_n60_), .c(new_n44_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n47_), .d(x15), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n46_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nor3   g28(.a(new_n68_), .b(new_n57_), .c(new_n46_), .O(z05));
  nand4  g29(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(new_n60_), .c(new_n44_), .O(z06));
  inv1   g31(.a(x12), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x11), .c(x10), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(x09), .c(x01), .O(new_n77_));
  nand3  g34(.a(new_n77_), .b(x02), .c(x00), .O(new_n78_));
  inv1   g35(.a(x15), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n60_), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n78_), .O(z07));
  inv1   g38(.a(x18), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x07), .O(new_n85_));
  inv1   g42(.a(x04), .O(new_n86_));
  inv1   g43(.a(x03), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x02), .c(new_n60_), .d(new_n44_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n86_), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n85_), .c(new_n84_), .d(new_n83_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(x08), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n82_), .c(x17), .d(x15), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n62_), .O(z08));
  inv1   g50(.a(x20), .O(new_n94_));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(new_n62_), .b(x18), .c(x01), .d(new_n44_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(z09));
  nand3  g57(.a(new_n62_), .b(x18), .c(x01), .O(new_n101_));
  nor4   g58(.a(new_n101_), .b(new_n96_), .c(new_n95_), .d(x20), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n67_), .O(new_n104_));
  nor3   g61(.a(new_n104_), .b(new_n45_), .c(x01), .O(new_n105_));
  inv1   g62(.a(x16), .O(new_n106_));
  nand3  g63(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n107_));
  nor3   g64(.a(new_n107_), .b(new_n106_), .c(x14), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n105_), .c(new_n102_), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(x00), .c(new_n80_), .O(z10));
  inv1   g67(.a(new_n105_), .O(new_n111_));
  nor2   g68(.a(new_n82_), .b(new_n60_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(x21), .c(new_n94_), .d(new_n62_), .O(new_n113_));
  nor2   g70(.a(new_n79_), .b(x14), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n95_), .c(x20), .d(new_n106_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n111_), .c(new_n113_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n96_), .c(new_n44_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(z11));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(x01), .c(x00), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n62_), .b(x17), .c(new_n45_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n60_), .c(new_n44_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n120_), .c(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n80_), .O(z12));
  nand4  g84(.a(new_n62_), .b(x17), .c(new_n45_), .d(new_n44_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(x15), .c(x01), .O(z13));
  nand4  g86(.a(new_n46_), .b(new_n45_), .c(new_n60_), .d(new_n44_), .O(new_n130_));
  nor2   g87(.a(new_n130_), .b(x10), .O(new_n131_));
  nand4  g88(.a(new_n131_), .b(new_n82_), .c(new_n47_), .d(x15), .O(new_n132_));
  nor2   g89(.a(new_n132_), .b(x19), .O(z14));
  aoi21  g90(.a(new_n57_), .b(x01), .c(new_n45_), .O(new_n134_));
  aoi21  g91(.a(x19), .b(new_n45_), .c(new_n79_), .O(new_n135_));
  oai22  g92(.a(new_n135_), .b(x01), .c(new_n134_), .d(new_n44_), .O(z15));
  oai21  g93(.a(new_n60_), .b(x00), .c(new_n80_), .O(z16));
  nand2  g94(.a(x02), .b(new_n44_), .O(new_n138_));
  aoi21  g95(.a(new_n138_), .b(x15), .c(x01), .O(z17));
endmodule


