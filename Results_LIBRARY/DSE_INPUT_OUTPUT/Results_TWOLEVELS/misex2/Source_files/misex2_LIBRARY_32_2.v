// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n68_, new_n70_,
    new_n72_, new_n73_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n101_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n120_, new_n122_, new_n123_, new_n126_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g06(.a(x17), .O(new_n50_));
  nor3   g07(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(x10), .d(x09), .O(new_n52_));
  nor3   g09(.a(new_n52_), .b(x19), .c(x18), .O(z01));
  inv1   g10(.a(x10), .O(new_n54_));
  nand4  g11(.a(new_n47_), .b(new_n54_), .c(x09), .d(new_n45_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  nand3  g14(.a(new_n45_), .b(new_n57_), .c(new_n44_), .O(new_n58_));
  inv1   g15(.a(x19), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x18), .c(new_n50_), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nand4  g18(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n62_));
  oai22  g19(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n58_), .O(z03));
  nor2   g20(.a(x09), .b(new_n45_), .O(new_n64_));
  nor2   g21(.a(x12), .b(x11), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(new_n64_), .c(x10), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(x01), .c(new_n44_), .O(z04));
  nand4  g24(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(new_n54_), .O(z05));
  nand4  g26(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n70_));
  nor3   g27(.a(new_n70_), .b(new_n57_), .c(new_n44_), .O(z06));
  inv1   g28(.a(x11), .O(new_n72_));
  nor2   g29(.a(x12), .b(new_n72_), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(new_n64_), .c(x10), .d(x01), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x01), .c(new_n44_), .O(z07));
  nor2   g32(.a(x03), .b(new_n45_), .O(new_n76_));
  nor2   g33(.a(x06), .b(x05), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(x04), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n80_), .c(x17), .O(new_n81_));
  nor3   g38(.a(new_n81_), .b(x08), .c(x07), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(new_n79_), .c(new_n76_), .O(new_n83_));
  aoi21  g40(.a(new_n83_), .b(new_n44_), .c(x01), .O(z08));
  nor2   g41(.a(x14), .b(x13), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x12), .c(new_n72_), .d(x02), .O(new_n86_));
  nor2   g43(.a(x16), .b(x15), .O(new_n87_));
  nor2   g44(.a(x22), .b(x21), .O(new_n88_));
  nand3  g45(.a(new_n88_), .b(new_n87_), .c(x20), .O(new_n89_));
  oai21  g46(.a(new_n89_), .b(new_n86_), .c(new_n44_), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(new_n57_), .O(new_n91_));
  nor3   g48(.a(new_n80_), .b(new_n57_), .c(x00), .O(new_n92_));
  nor2   g49(.a(x20), .b(x19), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n88_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n91_), .O(z09));
  nand4  g52(.a(new_n88_), .b(x20), .c(x16), .d(x15), .O(new_n96_));
  oai21  g53(.a(new_n96_), .b(new_n86_), .c(new_n44_), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n57_), .O(new_n98_));
  nand4  g55(.a(new_n93_), .b(new_n92_), .c(x22), .d(x21), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n98_), .O(z10));
  inv1   g57(.a(x16), .O(new_n101_));
  nand4  g58(.a(new_n88_), .b(x20), .c(new_n101_), .d(x15), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n86_), .c(new_n44_), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n57_), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand4  g62(.a(new_n93_), .b(new_n92_), .c(new_n105_), .d(x21), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n104_), .O(z11));
  inv1   g64(.a(x24), .O(new_n108_));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n59_), .b(x17), .c(new_n45_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n57_), .c(new_n44_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n108_), .c(x09), .O(new_n116_));
  inv1   g73(.a(new_n116_), .O(z12));
  nand2  g74(.a(new_n51_), .b(x17), .O(new_n118_));
  nor2   g75(.a(new_n118_), .b(x19), .O(z13));
  nand4  g76(.a(new_n51_), .b(new_n50_), .c(new_n54_), .d(new_n46_), .O(new_n120_));
  nor3   g77(.a(new_n120_), .b(x19), .c(x18), .O(z14));
  nor2   g78(.a(x01), .b(x00), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(x19), .c(new_n45_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n110_), .O(z15));
  xor2a  g81(.a(x01), .b(x00), .O(z16));
  nand2  g82(.a(new_n122_), .b(x02), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z17));
endmodule


