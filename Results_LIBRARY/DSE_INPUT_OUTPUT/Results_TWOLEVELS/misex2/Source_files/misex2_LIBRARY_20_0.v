// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n122_;
  nor2   g00(.a(x02), .b(x01), .O(z00));
  inv1   g01(.a(z00), .O(new_n45_));
  nand3  g02(.a(x02), .b(x01), .c(x00), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nand4  g04(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n48_));
  oai21  g05(.a(new_n48_), .b(new_n46_), .c(new_n45_), .O(z03));
  inv1   g06(.a(x11), .O(new_n50_));
  inv1   g07(.a(x12), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(x10), .d(new_n47_), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n46_), .c(new_n45_), .O(z04));
  nand2  g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(x10), .b(x09), .c(x02), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n54_), .c(new_n45_), .O(z05));
  nand2  g13(.a(x11), .b(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x09), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(x02), .c(x01), .d(x00), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n45_), .O(z06));
  nand2  g17(.a(new_n47_), .b(x02), .O(new_n61_));
  nand3  g18(.a(new_n51_), .b(x11), .c(x10), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(x01), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x00), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n45_), .O(z07));
  inv1   g22(.a(x19), .O(new_n66_));
  inv1   g23(.a(x08), .O(new_n67_));
  inv1   g24(.a(x18), .O(new_n68_));
  inv1   g25(.a(x05), .O(new_n69_));
  inv1   g26(.a(x06), .O(new_n70_));
  inv1   g27(.a(x00), .O(new_n71_));
  inv1   g28(.a(x01), .O(new_n72_));
  inv1   g29(.a(x03), .O(new_n73_));
  nand4  g30(.a(new_n73_), .b(x02), .c(new_n72_), .d(new_n71_), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(new_n70_), .c(new_n69_), .d(x04), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(x07), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(new_n68_), .c(x17), .d(new_n67_), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(new_n66_), .O(z08));
  inv1   g36(.a(x21), .O(new_n80_));
  inv1   g37(.a(x22), .O(new_n81_));
  inv1   g38(.a(x02), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x12), .c(new_n50_), .O(new_n84_));
  inv1   g41(.a(x15), .O(new_n85_));
  inv1   g42(.a(x16), .O(new_n86_));
  nor2   g43(.a(x14), .b(x13), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(x20), .c(new_n86_), .d(new_n85_), .O(new_n88_));
  inv1   g45(.a(x20), .O(new_n89_));
  nor2   g46(.a(new_n68_), .b(new_n72_), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(new_n89_), .c(new_n66_), .O(new_n91_));
  oai21  g48(.a(new_n88_), .b(new_n84_), .c(new_n91_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n81_), .c(new_n80_), .d(new_n71_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(z09));
  nand3  g51(.a(new_n66_), .b(x18), .c(x01), .O(new_n95_));
  nand3  g52(.a(x22), .b(x21), .c(new_n89_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g54(.a(new_n87_), .b(x12), .O(new_n98_));
  nor3   g55(.a(new_n98_), .b(x11), .c(x01), .O(new_n99_));
  nand3  g56(.a(new_n81_), .b(new_n80_), .c(x20), .O(new_n100_));
  nor3   g57(.a(new_n100_), .b(new_n86_), .c(new_n85_), .O(new_n101_));
  aoi21  g58(.a(new_n101_), .b(new_n99_), .c(new_n97_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(x00), .c(new_n45_), .O(z10));
  nand4  g60(.a(new_n90_), .b(x21), .c(new_n89_), .d(new_n66_), .O(new_n104_));
  nor2   g61(.a(x13), .b(new_n51_), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(new_n83_), .c(new_n50_), .O(new_n106_));
  nor2   g63(.a(new_n85_), .b(x14), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n80_), .c(x20), .d(new_n86_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n106_), .c(new_n104_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n81_), .c(new_n71_), .O(new_n110_));
  inv1   g67(.a(new_n110_), .O(z11));
  oai21  g68(.a(x10), .b(new_n72_), .c(x02), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(x00), .O(new_n113_));
  nand3  g70(.a(x23), .b(new_n72_), .c(new_n71_), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n113_), .c(x24), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x09), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n45_), .O(z12));
  nand2  g74(.a(new_n113_), .b(new_n45_), .O(z15));
  oai21  g75(.a(new_n72_), .b(x00), .c(new_n45_), .O(z16));
  nand3  g76(.a(x02), .b(new_n72_), .c(new_n71_), .O(new_n122_));
  inv1   g77(.a(new_n122_), .O(z17));
  zero   g78(.O(z13));
  zero   g79(.O(z14));
  nor2   g80(.a(x02), .b(x01), .O(z01));
  nor2   g81(.a(x02), .b(x01), .O(z02));
endmodule


