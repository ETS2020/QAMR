// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n148_;
  nand2  g000(.a(x11), .b(x09), .O(new_n44_));
  inv1   g001(.a(x00), .O(new_n45_));
  inv1   g002(.a(x01), .O(new_n46_));
  inv1   g003(.a(x02), .O(new_n47_));
  inv1   g004(.a(x09), .O(new_n48_));
  nand4  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n44_), .O(z00));
  inv1   g011(.a(x11), .O(new_n55_));
  inv1   g012(.a(new_n53_), .O(new_n56_));
  nor3   g013(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z01));
  nand3  g016(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n60_));
  nor3   g017(.a(new_n60_), .b(x10), .c(new_n48_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n51_), .c(new_n50_), .d(new_n55_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  nand2  g020(.a(x01), .b(x00), .O(new_n64_));
  nand3  g021(.a(x12), .b(x10), .c(x02), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n64_), .c(new_n48_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(x11), .O(new_n67_));
  nand4  g024(.a(new_n57_), .b(new_n52_), .c(x18), .d(new_n50_), .O(new_n68_));
  nand2  g025(.a(new_n68_), .b(new_n67_), .O(z03));
  inv1   g026(.a(new_n64_), .O(new_n70_));
  nand2  g027(.a(new_n70_), .b(x02), .O(new_n71_));
  inv1   g028(.a(new_n71_), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n55_), .c(x10), .d(new_n48_), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(x12), .O(z04));
  nand2  g031(.a(x10), .b(x02), .O(new_n75_));
  inv1   g032(.a(new_n75_), .O(new_n76_));
  nand2  g033(.a(new_n76_), .b(new_n70_), .O(new_n77_));
  aoi21  g034(.a(new_n77_), .b(new_n55_), .c(new_n48_), .O(z05));
  nand3  g035(.a(new_n76_), .b(x01), .c(x00), .O(new_n79_));
  aoi21  g036(.a(new_n79_), .b(new_n48_), .c(new_n55_), .O(z06));
  inv1   g037(.a(x10), .O(new_n81_));
  inv1   g038(.a(x12), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x11), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(new_n81_), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n44_), .O(z07));
  inv1   g043(.a(x03), .O(new_n87_));
  inv1   g044(.a(x04), .O(new_n88_));
  inv1   g045(.a(x05), .O(new_n89_));
  inv1   g046(.a(x06), .O(new_n90_));
  inv1   g047(.a(x07), .O(new_n91_));
  nand4  g048(.a(new_n44_), .b(x19), .c(new_n51_), .d(x17), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x08), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n87_), .c(x02), .d(new_n46_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(x00), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nor2   g056(.a(new_n47_), .b(x01), .O(new_n100_));
  nand3  g057(.a(new_n100_), .b(x12), .c(new_n55_), .O(new_n101_));
  inv1   g058(.a(x15), .O(new_n102_));
  inv1   g059(.a(x16), .O(new_n103_));
  nor2   g060(.a(x14), .b(x13), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(x20), .c(new_n103_), .d(new_n102_), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n52_), .c(x18), .d(x01), .O(new_n107_));
  oai21  g064(.a(new_n105_), .b(new_n101_), .c(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n99_), .c(new_n98_), .d(new_n45_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n44_), .O(z09));
  nand4  g067(.a(new_n44_), .b(x22), .c(x21), .d(new_n106_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n52_), .c(x18), .d(x01), .O(new_n113_));
  nor2   g070(.a(x13), .b(new_n82_), .O(new_n114_));
  nand3  g071(.a(new_n114_), .b(new_n100_), .c(new_n55_), .O(new_n115_));
  nor3   g072(.a(new_n103_), .b(new_n102_), .c(x14), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n99_), .c(new_n98_), .d(x20), .O(new_n117_));
  or2    g074(.a(new_n117_), .b(new_n115_), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n113_), .c(x00), .O(z10));
  nand4  g076(.a(new_n44_), .b(x21), .c(new_n106_), .d(new_n52_), .O(new_n120_));
  inv1   g077(.a(new_n120_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(x18), .c(x01), .O(new_n122_));
  nor2   g079(.a(new_n102_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n98_), .c(x20), .d(new_n103_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n115_), .c(new_n122_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n99_), .c(new_n45_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  nand3  g084(.a(new_n75_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n52_), .b(x17), .c(new_n47_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n46_), .c(new_n45_), .O(new_n132_));
  aoi21  g089(.a(new_n132_), .b(new_n128_), .c(x24), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n55_), .c(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  nand3  g092(.a(new_n44_), .b(new_n52_), .c(x17), .O(new_n136_));
  nor4   g093(.a(new_n136_), .b(x02), .c(x01), .d(x00), .O(z13));
  or2    g094(.a(new_n60_), .b(x09), .O(new_n138_));
  inv1   g095(.a(new_n138_), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n51_), .c(new_n50_), .d(new_n81_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x19), .O(z14));
  oai21  g098(.a(x10), .b(new_n46_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n143_), .c(new_n44_), .O(z15));
  nand3  g102(.a(new_n44_), .b(x01), .c(new_n45_), .O(new_n146_));
  inv1   g103(.a(new_n146_), .O(z16));
  nand4  g104(.a(new_n44_), .b(x02), .c(new_n46_), .d(new_n45_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


