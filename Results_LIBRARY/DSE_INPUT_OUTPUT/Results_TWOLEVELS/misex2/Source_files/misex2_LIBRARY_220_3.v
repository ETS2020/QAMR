// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n142_, new_n143_,
    new_n144_, new_n146_, new_n148_;
  nand2  g000(.a(x06), .b(x04), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x10), .O(new_n55_));
  nand4  g012(.a(new_n44_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(new_n55_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(x09), .c(new_n45_), .d(new_n54_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x00), .O(z01));
  nand3  g016(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n60_));
  nand4  g017(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n55_), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n60_), .c(new_n44_), .O(z02));
  nor3   g019(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nor2   g020(.a(x19), .b(new_n50_), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(new_n63_), .c(new_n49_), .O(new_n65_));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  inv1   g024(.a(x11), .O(new_n68_));
  inv1   g025(.a(x12), .O(new_n69_));
  nor2   g026(.a(new_n69_), .b(new_n68_), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n67_), .c(x10), .d(new_n46_), .O(new_n71_));
  nand3  g028(.a(new_n71_), .b(new_n65_), .c(new_n44_), .O(z03));
  inv1   g029(.a(x00), .O(new_n73_));
  nand4  g030(.a(new_n44_), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n74_));
  inv1   g031(.a(new_n74_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n46_), .c(x02), .d(x01), .O(new_n76_));
  nor2   g033(.a(new_n76_), .b(new_n73_), .O(z04));
  nand4  g034(.a(new_n44_), .b(x10), .c(x09), .d(x02), .O(new_n78_));
  nor3   g035(.a(new_n78_), .b(new_n54_), .c(new_n73_), .O(z05));
  nor3   g036(.a(new_n68_), .b(new_n55_), .c(x09), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x02), .c(x01), .d(x00), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(new_n44_), .O(z06));
  nand3  g039(.a(new_n69_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n44_), .O(z07));
  inv1   g043(.a(x04), .O(new_n87_));
  inv1   g044(.a(x06), .O(new_n88_));
  inv1   g045(.a(new_n47_), .O(new_n89_));
  nor2   g046(.a(x05), .b(x03), .O(new_n90_));
  nand2  g047(.a(new_n90_), .b(x02), .O(new_n91_));
  nor2   g048(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nor2   g049(.a(x08), .b(x07), .O(new_n93_));
  nor2   g050(.a(new_n51_), .b(x18), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n93_), .c(new_n92_), .d(x17), .O(new_n95_));
  aoi21  g052(.a(new_n95_), .b(new_n88_), .c(new_n87_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  inv1   g054(.a(x22), .O(new_n98_));
  nand4  g055(.a(x12), .b(new_n68_), .c(x02), .d(new_n54_), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n51_), .c(x18), .d(x01), .O(new_n105_));
  oai21  g062(.a(new_n103_), .b(new_n99_), .c(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n44_), .c(new_n98_), .d(new_n97_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(x00), .O(z09));
  nand2  g065(.a(x22), .b(x21), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(x20), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(new_n64_), .c(x01), .O(new_n111_));
  nor2   g068(.a(new_n45_), .b(x01), .O(new_n112_));
  nor2   g069(.a(x13), .b(new_n69_), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(new_n112_), .c(new_n68_), .O(new_n114_));
  inv1   g071(.a(x14), .O(new_n115_));
  and2   g072(.a(x16), .b(x15), .O(new_n116_));
  nor2   g073(.a(x22), .b(x21), .O(new_n117_));
  nand4  g074(.a(new_n117_), .b(new_n116_), .c(x20), .d(new_n115_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(new_n114_), .c(new_n111_), .O(new_n119_));
  nand3  g076(.a(new_n119_), .b(new_n44_), .c(new_n73_), .O(new_n120_));
  inv1   g077(.a(new_n120_), .O(z10));
  and2   g078(.a(x18), .b(x01), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(x21), .c(new_n104_), .d(new_n51_), .O(new_n123_));
  nor2   g080(.a(new_n100_), .b(x14), .O(new_n124_));
  nor2   g081(.a(x21), .b(new_n104_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n124_), .c(new_n101_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n114_), .c(new_n123_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n98_), .c(new_n73_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n44_), .O(z11));
  inv1   g086(.a(x24), .O(new_n130_));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n54_), .c(new_n73_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n130_), .c(x09), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n44_), .O(z12));
  oai21  g096(.a(new_n134_), .b(new_n89_), .c(new_n44_), .O(z13));
  oai21  g097(.a(new_n61_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g098(.a(x10), .b(new_n54_), .c(x02), .O(new_n142_));
  nand2  g099(.a(new_n142_), .b(x00), .O(new_n143_));
  nand3  g100(.a(x19), .b(new_n45_), .c(new_n54_), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n143_), .c(new_n44_), .O(z15));
  nand3  g102(.a(new_n44_), .b(x01), .c(new_n73_), .O(new_n146_));
  inv1   g103(.a(new_n146_), .O(z16));
  nand2  g104(.a(new_n112_), .b(new_n73_), .O(new_n148_));
  nand2  g105(.a(new_n148_), .b(new_n44_), .O(z17));
endmodule


