// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n138_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x19), .O(new_n46_));
  nor3   g03(.a(x09), .b(x02), .c(x00), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(x10), .c(x01), .O(z00));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x01), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(x09), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  nor2   g13(.a(x10), .b(x01), .O(z02));
  inv1   g14(.a(x10), .O(new_n58_));
  inv1   g15(.a(x09), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x12), .c(x11), .d(new_n59_), .O(new_n62_));
  nor3   g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n46_), .c(x18), .d(new_n44_), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(new_n62_), .c(new_n58_), .O(z03));
  inv1   g22(.a(z02), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(x10), .d(new_n59_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n60_), .c(new_n66_), .O(z04));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n66_), .O(z05));
  nor3   g30(.a(new_n67_), .b(new_n58_), .c(x09), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(x02), .c(x01), .d(x00), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n66_), .O(z06));
  nand2  g33(.a(new_n68_), .b(x11), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(x09), .c(x01), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x10), .c(x02), .d(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  nand2  g37(.a(x02), .b(new_n50_), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(x04), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(new_n81_), .c(x03), .O(new_n84_));
  nor3   g41(.a(x08), .b(x07), .c(x06), .O(new_n85_));
  nor2   g42(.a(new_n46_), .b(x18), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n85_), .c(new_n84_), .d(x17), .O(new_n87_));
  aoi21  g44(.a(new_n87_), .b(x10), .c(x01), .O(z08));
  inv1   g45(.a(x21), .O(new_n89_));
  inv1   g46(.a(x22), .O(new_n90_));
  nor2   g47(.a(new_n52_), .b(x01), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(x12), .c(new_n67_), .O(new_n92_));
  inv1   g49(.a(x15), .O(new_n93_));
  inv1   g50(.a(x16), .O(new_n94_));
  nor2   g51(.a(x14), .b(x13), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(x20), .c(new_n94_), .d(new_n93_), .O(new_n96_));
  inv1   g53(.a(x20), .O(new_n97_));
  nor2   g54(.a(new_n45_), .b(new_n51_), .O(new_n98_));
  nand3  g55(.a(new_n98_), .b(new_n97_), .c(new_n46_), .O(new_n99_));
  oai21  g56(.a(new_n96_), .b(new_n92_), .c(new_n99_), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n90_), .c(new_n89_), .d(new_n50_), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n66_), .O(z09));
  nand3  g59(.a(new_n46_), .b(x18), .c(x01), .O(new_n103_));
  nand3  g60(.a(x22), .b(x21), .c(new_n97_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g62(.a(x02), .b(new_n51_), .O(new_n106_));
  inv1   g63(.a(x13), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(x12), .c(new_n67_), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  inv1   g66(.a(x14), .O(new_n110_));
  nand3  g67(.a(x16), .b(x15), .c(new_n110_), .O(new_n111_));
  nand3  g68(.a(new_n90_), .b(new_n89_), .c(x20), .O(new_n112_));
  nor2   g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n109_), .c(new_n105_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(x00), .c(new_n66_), .O(z10));
  nand4  g72(.a(new_n91_), .b(new_n107_), .c(x12), .d(new_n67_), .O(new_n116_));
  nand4  g73(.a(new_n98_), .b(x21), .c(new_n97_), .d(new_n46_), .O(new_n117_));
  nor2   g74(.a(new_n93_), .b(x14), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n89_), .c(x20), .d(new_n94_), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n117_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n90_), .c(new_n50_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n66_), .O(z11));
  inv1   g79(.a(x24), .O(new_n123_));
  nand2  g80(.a(x10), .b(x02), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(x01), .c(x00), .O(new_n125_));
  oai21  g82(.a(x19), .b(x02), .c(x23), .O(new_n126_));
  nand3  g83(.a(new_n46_), .b(x17), .c(new_n52_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g85(.a(new_n128_), .b(x10), .c(new_n51_), .d(new_n50_), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand3  g87(.a(new_n130_), .b(new_n123_), .c(x09), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z12));
  nand4  g89(.a(new_n46_), .b(x17), .c(new_n52_), .d(new_n50_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(x10), .c(x01), .O(z13));
  nor2   g91(.a(new_n58_), .b(x02), .O(new_n135_));
  nor2   g92(.a(x10), .b(new_n51_), .O(new_n136_));
  oai21  g93(.a(new_n136_), .b(new_n135_), .c(x00), .O(new_n137_));
  nand4  g94(.a(x19), .b(x10), .c(new_n52_), .d(new_n51_), .O(new_n138_));
  nand2  g95(.a(new_n138_), .b(new_n137_), .O(z15));
  oai21  g96(.a(new_n51_), .b(x00), .c(new_n66_), .O(z16));
  aoi21  g97(.a(new_n81_), .b(x10), .c(x01), .O(z17));
  nor2   g98(.a(x10), .b(x01), .O(z14));
endmodule


