// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n139_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n145_;
  nor3   g000(.a(x02), .b(x01), .c(x00), .O(new_n44_));
  inv1   g001(.a(x10), .O(new_n45_));
  nor2   g002(.a(new_n45_), .b(x09), .O(new_n46_));
  nor2   g003(.a(x19), .b(x17), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n48_));
  aoi21  g005(.a(new_n48_), .b(x21), .c(x18), .O(z00));
  inv1   g006(.a(x21), .O(new_n50_));
  inv1   g007(.a(x17), .O(new_n51_));
  inv1   g008(.a(x18), .O(new_n52_));
  inv1   g009(.a(x19), .O(new_n53_));
  inv1   g010(.a(x02), .O(new_n54_));
  nor2   g011(.a(x01), .b(x00), .O(new_n55_));
  nand3  g012(.a(new_n55_), .b(x09), .c(new_n54_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(new_n45_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(new_n50_), .O(z01));
  nand4  g016(.a(new_n47_), .b(new_n44_), .c(new_n45_), .d(x09), .O(new_n60_));
  aoi21  g017(.a(new_n60_), .b(x21), .c(x18), .O(z02));
  nor2   g018(.a(x21), .b(x18), .O(new_n62_));
  inv1   g019(.a(new_n62_), .O(new_n63_));
  nand4  g020(.a(new_n44_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n46_), .c(x12), .d(x11), .O(new_n67_));
  nand3  g024(.a(new_n67_), .b(new_n64_), .c(new_n63_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(new_n46_), .b(new_n70_), .c(new_n69_), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n65_), .c(new_n63_), .O(z04));
  inv1   g029(.a(x00), .O(new_n73_));
  nor2   g030(.a(new_n62_), .b(new_n45_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x09), .c(x02), .d(x01), .O(new_n75_));
  nor2   g032(.a(new_n75_), .b(new_n73_), .O(z05));
  nor3   g033(.a(new_n69_), .b(new_n45_), .c(x09), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(x02), .c(x01), .d(x00), .O(new_n78_));
  nand2  g035(.a(new_n78_), .b(new_n63_), .O(z06));
  nand3  g036(.a(new_n70_), .b(x11), .c(x10), .O(new_n80_));
  oai21  g037(.a(new_n80_), .b(x09), .c(x01), .O(new_n81_));
  nand3  g038(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n63_), .O(z07));
  inv1   g040(.a(x03), .O(new_n84_));
  nand3  g041(.a(x04), .b(new_n84_), .c(x02), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(new_n86_));
  nor3   g043(.a(x07), .b(x06), .c(x05), .O(new_n87_));
  nor3   g044(.a(new_n53_), .b(new_n51_), .c(x08), .O(new_n88_));
  nand4  g045(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n55_), .O(new_n89_));
  aoi21  g046(.a(new_n89_), .b(x21), .c(x18), .O(z08));
  inv1   g047(.a(x22), .O(new_n91_));
  inv1   g048(.a(x01), .O(new_n92_));
  nand4  g049(.a(x12), .b(new_n69_), .c(x02), .d(new_n92_), .O(new_n93_));
  inv1   g050(.a(x15), .O(new_n94_));
  inv1   g051(.a(x16), .O(new_n95_));
  nor2   g052(.a(x14), .b(x13), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(x20), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g054(.a(x20), .O(new_n98_));
  nand3  g055(.a(new_n98_), .b(new_n53_), .c(x01), .O(new_n99_));
  oai21  g056(.a(new_n97_), .b(new_n93_), .c(new_n99_), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(new_n91_), .c(new_n50_), .d(x18), .O(new_n101_));
  nor2   g058(.a(new_n101_), .b(x00), .O(z09));
  nand3  g059(.a(new_n53_), .b(x18), .c(x01), .O(new_n103_));
  nand3  g060(.a(x22), .b(x21), .c(new_n98_), .O(new_n104_));
  nor2   g061(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand2  g062(.a(x02), .b(new_n92_), .O(new_n106_));
  inv1   g063(.a(x13), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(x12), .c(new_n69_), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(new_n106_), .O(new_n109_));
  inv1   g066(.a(x14), .O(new_n110_));
  nand3  g067(.a(x16), .b(x15), .c(new_n110_), .O(new_n111_));
  nand3  g068(.a(new_n91_), .b(new_n50_), .c(x20), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  aoi21  g070(.a(new_n113_), .b(new_n109_), .c(new_n105_), .O(new_n114_));
  oai21  g071(.a(new_n114_), .b(x00), .c(new_n63_), .O(z10));
  nor2   g072(.a(new_n54_), .b(x01), .O(new_n116_));
  nor2   g073(.a(x13), .b(new_n70_), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(new_n116_), .c(new_n69_), .O(new_n118_));
  nand4  g075(.a(x21), .b(new_n98_), .c(new_n53_), .d(x01), .O(new_n119_));
  nor2   g076(.a(new_n94_), .b(x14), .O(new_n120_));
  nor2   g077(.a(x21), .b(new_n98_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n120_), .c(new_n95_), .O(new_n122_));
  oai21  g079(.a(new_n122_), .b(new_n118_), .c(new_n119_), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n91_), .c(x18), .d(new_n73_), .O(new_n124_));
  inv1   g081(.a(new_n124_), .O(z11));
  inv1   g082(.a(x24), .O(new_n126_));
  nand2  g083(.a(x10), .b(x02), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g085(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g086(.a(new_n53_), .b(x17), .c(new_n54_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n92_), .c(new_n73_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand4  g090(.a(new_n133_), .b(new_n63_), .c(new_n126_), .d(x09), .O(new_n134_));
  inv1   g091(.a(new_n134_), .O(z12));
  inv1   g092(.a(new_n55_), .O(new_n136_));
  oai21  g093(.a(new_n130_), .b(new_n136_), .c(new_n63_), .O(z13));
  inv1   g094(.a(x09), .O(new_n138_));
  nand3  g095(.a(new_n55_), .b(new_n138_), .c(new_n54_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x10), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(new_n50_), .O(z14));
  oai21  g099(.a(x10), .b(new_n92_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n54_), .c(new_n92_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n63_), .O(z15));
  oai21  g103(.a(new_n92_), .b(x00), .c(new_n63_), .O(z16));
  nor4   g104(.a(new_n62_), .b(new_n54_), .c(x01), .d(x00), .O(z17));
endmodule


