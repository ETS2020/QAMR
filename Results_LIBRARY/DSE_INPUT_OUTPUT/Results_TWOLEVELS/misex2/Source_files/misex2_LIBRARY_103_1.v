// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n139_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x13), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor3   g002(.a(x09), .b(x02), .c(x00), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g005(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x10), .O(new_n51_));
  inv1   g008(.a(x00), .O(new_n52_));
  inv1   g009(.a(x01), .O(new_n53_));
  inv1   g010(.a(x02), .O(new_n54_));
  nand4  g011(.a(x09), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  nand4  g013(.a(new_n56_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(x19), .O(z01));
  nor2   g015(.a(new_n55_), .b(x10), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z02));
  nand2  g018(.a(new_n54_), .b(new_n52_), .O(new_n62_));
  inv1   g019(.a(x19), .O(new_n63_));
  nand3  g020(.a(new_n63_), .b(x18), .c(new_n45_), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n62_), .c(new_n44_), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(new_n53_), .O(new_n66_));
  nand3  g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g024(.a(new_n51_), .b(x09), .O(new_n68_));
  nand3  g025(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g026(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nand2  g027(.a(x13), .b(new_n53_), .O(new_n71_));
  inv1   g028(.a(x11), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  nand3  g030(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n67_), .c(new_n71_), .O(z04));
  nand2  g032(.a(x01), .b(x00), .O(new_n76_));
  nand3  g033(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z05));
  nor3   g035(.a(new_n72_), .b(new_n51_), .c(x09), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(new_n71_), .O(z06));
  nand3  g038(.a(new_n73_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n71_), .O(z07));
  nand2  g042(.a(x02), .b(new_n52_), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(x04), .O(new_n88_));
  nor3   g045(.a(new_n88_), .b(new_n86_), .c(x03), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand3  g047(.a(x19), .b(new_n50_), .c(x17), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  aoi21  g050(.a(new_n93_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n72_), .c(x02), .d(new_n53_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  and2   g059(.a(x18), .b(x01), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n102_), .c(new_n63_), .O(new_n104_));
  oai21  g061(.a(new_n101_), .b(new_n97_), .c(new_n104_), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n96_), .c(new_n95_), .d(new_n52_), .O(new_n106_));
  inv1   g063(.a(new_n106_), .O(z09));
  nand3  g064(.a(new_n63_), .b(x18), .c(x01), .O(new_n108_));
  nand3  g065(.a(x22), .b(x21), .c(new_n102_), .O(new_n109_));
  nor2   g066(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  nand2  g067(.a(x02), .b(new_n53_), .O(new_n111_));
  inv1   g068(.a(x14), .O(new_n112_));
  nand3  g069(.a(new_n112_), .b(x12), .c(new_n72_), .O(new_n113_));
  nor2   g070(.a(new_n113_), .b(new_n111_), .O(new_n114_));
  nand2  g071(.a(x16), .b(x15), .O(new_n115_));
  nand3  g072(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n116_));
  nor2   g073(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n114_), .c(new_n110_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n71_), .O(z10));
  nand4  g076(.a(new_n103_), .b(x21), .c(new_n102_), .d(new_n63_), .O(new_n120_));
  nor2   g077(.a(new_n98_), .b(x14), .O(new_n121_));
  nor2   g078(.a(x21), .b(new_n102_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n121_), .c(new_n99_), .O(new_n123_));
  oai21  g080(.a(new_n123_), .b(new_n97_), .c(new_n120_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n96_), .c(new_n52_), .O(new_n125_));
  nand2  g082(.a(new_n125_), .b(new_n71_), .O(z11));
  inv1   g083(.a(x24), .O(new_n127_));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  oai21  g086(.a(x19), .b(x02), .c(x23), .O(new_n130_));
  nand3  g087(.a(new_n63_), .b(x17), .c(new_n54_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n53_), .c(new_n52_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n127_), .c(x09), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n71_), .O(z12));
  nand4  g093(.a(new_n63_), .b(x17), .c(new_n54_), .d(new_n52_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(new_n44_), .c(x01), .O(z13));
  inv1   g095(.a(x09), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x10), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n50_), .c(new_n45_), .d(new_n44_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z14));
  aoi21  g100(.a(new_n51_), .b(x01), .c(new_n54_), .O(new_n144_));
  aoi21  g101(.a(x19), .b(new_n54_), .c(x13), .O(new_n145_));
  oai22  g102(.a(new_n145_), .b(x01), .c(new_n144_), .d(new_n52_), .O(z15));
  oai21  g103(.a(new_n53_), .b(x00), .c(new_n71_), .O(z16));
  nand4  g104(.a(new_n44_), .b(x02), .c(new_n53_), .d(new_n52_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


