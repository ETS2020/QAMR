// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n134_, new_n135_,
    new_n136_, new_n138_, new_n140_, new_n141_, new_n142_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand4  g010(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z02));
  nor3   g017(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  inv1   g019(.a(x19), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(x18), .c(new_n44_), .d(x09), .O(new_n64_));
  nand4  g021(.a(new_n49_), .b(x02), .c(x01), .d(x00), .O(new_n65_));
  nand4  g022(.a(new_n45_), .b(x12), .c(x11), .d(x10), .O(new_n66_));
  oai22  g023(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  inv1   g024(.a(x11), .O(new_n68_));
  inv1   g025(.a(x12), .O(new_n69_));
  nand3  g026(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g027(.a(new_n70_), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n72_));
  aoi21  g029(.a(new_n72_), .b(new_n45_), .c(x09), .O(z04));
  nand2  g030(.a(x01), .b(x00), .O(new_n74_));
  nand2  g031(.a(x18), .b(new_n49_), .O(new_n75_));
  nand3  g032(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(z05));
  nand4  g034(.a(new_n45_), .b(x11), .c(x10), .d(new_n49_), .O(new_n78_));
  inv1   g035(.a(new_n78_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(z06));
  nand2  g038(.a(x10), .b(new_n49_), .O(new_n82_));
  nand3  g039(.a(new_n45_), .b(new_n69_), .c(x11), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(new_n82_), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n75_), .O(z07));
  inv1   g043(.a(x04), .O(new_n87_));
  nand3  g044(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n88_));
  nor4   g045(.a(new_n88_), .b(x05), .c(new_n87_), .d(x03), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nor2   g047(.a(new_n63_), .b(x18), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n90_), .c(new_n89_), .d(x17), .O(new_n92_));
  nand2  g049(.a(new_n92_), .b(new_n75_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  inv1   g051(.a(x22), .O(new_n95_));
  nand4  g052(.a(x12), .b(new_n68_), .c(x02), .d(new_n47_), .O(new_n96_));
  inv1   g053(.a(x15), .O(new_n97_));
  inv1   g054(.a(x16), .O(new_n98_));
  nor2   g055(.a(x14), .b(x13), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(x20), .c(new_n98_), .d(new_n97_), .O(new_n100_));
  nor2   g057(.a(x20), .b(x19), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x18), .c(x01), .O(new_n102_));
  oai21  g059(.a(new_n100_), .b(new_n96_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n95_), .c(new_n94_), .d(new_n46_), .O(new_n104_));
  nand2  g061(.a(new_n104_), .b(new_n75_), .O(z09));
  inv1   g062(.a(x14), .O(new_n106_));
  nand4  g063(.a(new_n75_), .b(new_n95_), .c(new_n94_), .d(x20), .O(new_n107_));
  inv1   g064(.a(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(x16), .c(x15), .d(new_n106_), .O(new_n109_));
  nor3   g066(.a(new_n109_), .b(x13), .c(new_n69_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n68_), .c(x02), .d(new_n47_), .O(new_n111_));
  nand3  g068(.a(x18), .b(x09), .c(x01), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n101_), .c(x22), .d(x21), .O(new_n114_));
  aoi21  g071(.a(new_n114_), .b(new_n111_), .c(x00), .O(z10));
  aoi21  g072(.a(x18), .b(new_n49_), .c(x21), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(x20), .c(new_n98_), .d(x15), .O(new_n117_));
  nor3   g074(.a(new_n117_), .b(x14), .c(x13), .O(new_n118_));
  nand4  g075(.a(new_n118_), .b(x12), .c(new_n68_), .d(x02), .O(new_n119_));
  nor2   g076(.a(new_n94_), .b(x20), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n113_), .c(new_n63_), .O(new_n121_));
  oai21  g078(.a(new_n119_), .b(x01), .c(new_n121_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n95_), .c(new_n46_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(z11));
  nand2  g081(.a(x10), .b(x02), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  oai21  g083(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g084(.a(new_n63_), .b(x17), .c(new_n48_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n47_), .c(new_n46_), .O(new_n130_));
  aoi21  g087(.a(new_n130_), .b(new_n126_), .c(x24), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(x09), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n75_), .O(z12));
  inv1   g090(.a(new_n75_), .O(new_n134_));
  nor2   g091(.a(new_n134_), .b(x19), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x00), .O(z13));
  nand4  g094(.a(new_n61_), .b(new_n63_), .c(new_n44_), .d(new_n58_), .O(new_n138_));
  aoi21  g095(.a(new_n138_), .b(new_n45_), .c(x09), .O(z14));
  oai21  g096(.a(x10), .b(new_n47_), .c(x02), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(x00), .O(new_n141_));
  nand3  g098(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n142_));
  nand3  g099(.a(new_n142_), .b(new_n141_), .c(new_n75_), .O(z15));
  nor3   g100(.a(new_n134_), .b(new_n47_), .c(x00), .O(z16));
  nand2  g101(.a(new_n88_), .b(new_n75_), .O(z17));
endmodule


