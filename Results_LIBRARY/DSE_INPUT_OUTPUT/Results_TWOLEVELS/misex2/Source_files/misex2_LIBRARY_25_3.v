// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n136_, new_n139_, new_n140_, new_n141_, new_n143_, new_n145_;
  nand2  g000(.a(x24), .b(x22), .O(new_n44_));
  inv1   g001(.a(x00), .O(new_n45_));
  inv1   g002(.a(x01), .O(new_n46_));
  inv1   g003(.a(x02), .O(new_n47_));
  inv1   g004(.a(x09), .O(new_n48_));
  nand4  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand4  g011(.a(x09), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n55_));
  oai21  g012(.a(new_n55_), .b(new_n53_), .c(new_n44_), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n55_), .c(new_n44_), .O(z02));
  inv1   g016(.a(new_n44_), .O(new_n60_));
  nand3  g017(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  inv1   g019(.a(x11), .O(new_n63_));
  inv1   g020(.a(x12), .O(new_n64_));
  nor2   g021(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n62_), .c(x10), .d(new_n48_), .O(new_n66_));
  nor3   g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g024(.a(x19), .b(new_n51_), .O(new_n68_));
  nand3  g025(.a(new_n68_), .b(new_n67_), .c(new_n50_), .O(new_n69_));
  aoi21  g026(.a(new_n69_), .b(new_n66_), .c(new_n60_), .O(z03));
  nand4  g027(.a(new_n44_), .b(new_n64_), .c(new_n63_), .d(x10), .O(new_n71_));
  inv1   g028(.a(new_n71_), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n48_), .c(x02), .d(x01), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(new_n45_), .O(z04));
  nand4  g031(.a(new_n44_), .b(x10), .c(x09), .d(x02), .O(new_n75_));
  nor3   g032(.a(new_n75_), .b(new_n46_), .c(new_n45_), .O(z05));
  nand4  g033(.a(new_n44_), .b(x11), .c(x10), .d(new_n48_), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x02), .c(x01), .d(x00), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z06));
  nand3  g037(.a(new_n64_), .b(x11), .c(x10), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(new_n44_), .c(x02), .d(x00), .O(new_n83_));
  inv1   g040(.a(new_n83_), .O(z07));
  inv1   g041(.a(x03), .O(new_n85_));
  nor2   g042(.a(new_n47_), .b(x01), .O(new_n86_));
  inv1   g043(.a(x04), .O(new_n87_));
  nor2   g044(.a(x05), .b(new_n87_), .O(new_n88_));
  nand4  g045(.a(new_n88_), .b(new_n86_), .c(new_n85_), .d(new_n45_), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand4  g047(.a(new_n90_), .b(x19), .c(new_n51_), .d(x17), .O(new_n91_));
  oai21  g048(.a(new_n91_), .b(new_n89_), .c(new_n44_), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n63_), .c(x02), .d(new_n46_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  and2   g057(.a(x18), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(new_n52_), .O(new_n102_));
  oai21  g059(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n94_), .c(new_n93_), .d(new_n45_), .O(new_n104_));
  inv1   g061(.a(new_n104_), .O(z09));
  nor2   g062(.a(x13), .b(new_n64_), .O(new_n106_));
  nand2  g063(.a(new_n106_), .b(new_n63_), .O(new_n107_));
  inv1   g064(.a(new_n107_), .O(new_n108_));
  nor3   g065(.a(new_n97_), .b(new_n96_), .c(x14), .O(new_n109_));
  nand3  g066(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n109_), .c(new_n108_), .d(new_n86_), .O(new_n112_));
  nor2   g069(.a(new_n93_), .b(x20), .O(new_n113_));
  nor2   g070(.a(x24), .b(new_n94_), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n113_), .c(new_n68_), .d(x01), .O(new_n115_));
  aoi21  g072(.a(new_n115_), .b(new_n112_), .c(x00), .O(z10));
  nand3  g073(.a(new_n106_), .b(new_n86_), .c(new_n63_), .O(new_n117_));
  nand3  g074(.a(new_n113_), .b(new_n101_), .c(new_n52_), .O(new_n118_));
  nor2   g075(.a(new_n96_), .b(x14), .O(new_n119_));
  nor2   g076(.a(x21), .b(new_n100_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n97_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n117_), .c(new_n118_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n94_), .c(new_n45_), .O(new_n123_));
  nand2  g080(.a(new_n123_), .b(new_n44_), .O(z11));
  inv1   g081(.a(x24), .O(new_n125_));
  nand2  g082(.a(x10), .b(x02), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(x01), .c(x00), .O(new_n127_));
  oai21  g084(.a(x19), .b(x02), .c(x23), .O(new_n128_));
  nand3  g085(.a(new_n52_), .b(x17), .c(new_n47_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n128_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n46_), .c(new_n45_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n127_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n125_), .c(x09), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n44_), .O(z12));
  nor2   g091(.a(new_n60_), .b(x19), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(x17), .c(new_n47_), .d(new_n46_), .O(new_n136_));
  nor2   g093(.a(new_n136_), .b(x00), .O(z13));
  oai21  g094(.a(new_n58_), .b(new_n49_), .c(new_n44_), .O(z14));
  oai21  g095(.a(x10), .b(new_n46_), .c(x02), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(x00), .O(new_n140_));
  nand3  g097(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n141_));
  nand3  g098(.a(new_n141_), .b(new_n140_), .c(new_n44_), .O(z15));
  nand3  g099(.a(new_n44_), .b(x01), .c(new_n45_), .O(new_n143_));
  inv1   g100(.a(new_n143_), .O(z16));
  nand4  g101(.a(new_n44_), .b(x02), .c(new_n46_), .d(new_n45_), .O(new_n145_));
  inv1   g102(.a(new_n145_), .O(z17));
endmodule


