// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:06 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n150_;
  inv1   g000(.a(x24), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g011(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g012(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand3  g014(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  inv1   g015(.a(new_n58_), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z02));
  nand4  g018(.a(new_n48_), .b(new_n52_), .c(new_n50_), .d(new_n46_), .O(new_n62_));
  nand2  g019(.a(new_n62_), .b(x24), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(x18), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nor2   g022(.a(new_n57_), .b(x09), .O(new_n66_));
  nand3  g023(.a(new_n66_), .b(x12), .c(x11), .O(new_n67_));
  oai21  g024(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand3  g027(.a(new_n66_), .b(new_n70_), .c(new_n69_), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n65_), .c(new_n45_), .O(z04));
  nand2  g029(.a(x01), .b(x00), .O(new_n73_));
  nand3  g030(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n73_), .c(new_n45_), .O(z05));
  nor3   g032(.a(new_n69_), .b(new_n57_), .c(x09), .O(new_n76_));
  nand4  g033(.a(new_n76_), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nand2  g034(.a(new_n77_), .b(new_n45_), .O(z06));
  nand3  g035(.a(new_n70_), .b(x11), .c(x10), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nand3  g037(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(new_n45_), .O(z07));
  inv1   g039(.a(x08), .O(new_n83_));
  inv1   g040(.a(x05), .O(new_n84_));
  inv1   g041(.a(x03), .O(new_n85_));
  nand3  g042(.a(new_n48_), .b(new_n85_), .c(x02), .O(new_n86_));
  inv1   g043(.a(new_n86_), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(new_n84_), .c(x04), .O(new_n88_));
  nor3   g045(.a(new_n88_), .b(x07), .c(x06), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n51_), .c(x17), .d(new_n83_), .O(new_n90_));
  nor2   g047(.a(new_n90_), .b(new_n52_), .O(z08));
  inv1   g048(.a(x00), .O(new_n92_));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  inv1   g051(.a(x14), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  inv1   g054(.a(x20), .O(new_n98_));
  aoi21  g055(.a(new_n44_), .b(x18), .c(new_n98_), .O(new_n99_));
  nand4  g056(.a(new_n99_), .b(new_n97_), .c(new_n96_), .d(new_n95_), .O(new_n100_));
  nor2   g057(.a(new_n100_), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x12), .c(new_n69_), .d(x02), .O(new_n102_));
  inv1   g059(.a(x01), .O(new_n103_));
  nor2   g060(.a(new_n51_), .b(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(x24), .c(new_n98_), .d(new_n52_), .O(new_n105_));
  oai21  g062(.a(new_n102_), .b(x01), .c(new_n105_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n94_), .c(new_n93_), .d(new_n92_), .O(new_n107_));
  inv1   g064(.a(new_n107_), .O(z09));
  inv1   g065(.a(x13), .O(new_n109_));
  nand4  g066(.a(new_n45_), .b(new_n94_), .c(new_n93_), .d(x20), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(new_n97_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(x15), .c(new_n95_), .d(new_n109_), .O(new_n112_));
  nor2   g069(.a(new_n112_), .b(new_n70_), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n69_), .c(x02), .d(new_n103_), .O(new_n114_));
  nor2   g071(.a(x19), .b(new_n51_), .O(new_n115_));
  nor2   g072(.a(new_n93_), .b(x20), .O(new_n116_));
  nor2   g073(.a(new_n44_), .b(new_n94_), .O(new_n117_));
  nand4  g074(.a(new_n117_), .b(new_n116_), .c(new_n115_), .d(x01), .O(new_n118_));
  aoi21  g075(.a(new_n118_), .b(new_n114_), .c(x00), .O(z10));
  nand3  g076(.a(new_n116_), .b(new_n104_), .c(new_n52_), .O(new_n120_));
  nor2   g077(.a(new_n46_), .b(x01), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n109_), .c(x12), .d(new_n69_), .O(new_n122_));
  nor2   g079(.a(new_n96_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n93_), .c(x20), .d(new_n97_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n122_), .c(new_n120_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n94_), .c(new_n92_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n45_), .O(z11));
  nand2  g084(.a(x10), .b(x02), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(x01), .c(x00), .O(new_n129_));
  oai21  g086(.a(x19), .b(x02), .c(x23), .O(new_n130_));
  nand3  g087(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(new_n130_), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(new_n103_), .c(new_n92_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n129_), .O(new_n134_));
  nand4  g091(.a(new_n134_), .b(new_n44_), .c(new_n51_), .d(x09), .O(new_n135_));
  inv1   g092(.a(new_n135_), .O(z12));
  inv1   g093(.a(new_n45_), .O(new_n137_));
  nor2   g094(.a(new_n137_), .b(x19), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(x17), .c(new_n46_), .d(new_n103_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x00), .O(z13));
  nand3  g097(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n141_));
  inv1   g098(.a(new_n141_), .O(new_n142_));
  nand4  g099(.a(new_n142_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n143_));
  nor2   g100(.a(new_n143_), .b(x19), .O(z14));
  oai21  g101(.a(x10), .b(new_n103_), .c(x02), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(x00), .O(new_n146_));
  nand3  g103(.a(x19), .b(new_n46_), .c(new_n103_), .O(new_n147_));
  aoi21  g104(.a(new_n147_), .b(new_n146_), .c(new_n137_), .O(z15));
  oai21  g105(.a(new_n103_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g106(.a(new_n121_), .b(new_n92_), .O(new_n150_));
  nand2  g107(.a(new_n150_), .b(new_n45_), .O(z17));
endmodule


