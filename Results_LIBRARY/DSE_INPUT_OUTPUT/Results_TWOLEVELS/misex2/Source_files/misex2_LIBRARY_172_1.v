// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:32 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n131_, new_n132_, new_n133_, new_n135_, new_n136_,
    new_n137_, new_n140_;
  inv1   g00(.a(x08), .O(new_n44_));
  nand2  g01(.a(x19), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand4  g16(.a(new_n45_), .b(x12), .c(x11), .d(x10), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x09), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x02), .c(x01), .d(x00), .O(new_n62_));
  nor3   g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n52_), .c(x18), .d(new_n50_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n62_), .O(z03));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(x10), .d(new_n47_), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n66_), .c(new_n45_), .O(z04));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n45_), .O(z05));
  inv1   g30(.a(x00), .O(new_n74_));
  inv1   g31(.a(x01), .O(new_n75_));
  nand4  g32(.a(new_n45_), .b(x11), .c(x10), .d(new_n47_), .O(new_n76_));
  nor4   g33(.a(new_n76_), .b(new_n46_), .c(new_n75_), .d(new_n74_), .O(z06));
  nand3  g34(.a(new_n68_), .b(x11), .c(x10), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(new_n45_), .c(x02), .d(x00), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z07));
  inv1   g38(.a(new_n45_), .O(z08));
  inv1   g39(.a(x21), .O(new_n83_));
  inv1   g40(.a(x22), .O(new_n84_));
  nand4  g41(.a(x12), .b(new_n67_), .c(x02), .d(new_n75_), .O(new_n85_));
  inv1   g42(.a(x15), .O(new_n86_));
  inv1   g43(.a(x16), .O(new_n87_));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x20), .c(new_n87_), .d(new_n86_), .O(new_n89_));
  inv1   g46(.a(x20), .O(new_n90_));
  and2   g47(.a(x18), .b(x01), .O(new_n91_));
  nand3  g48(.a(new_n91_), .b(new_n90_), .c(new_n52_), .O(new_n92_));
  oai21  g49(.a(new_n89_), .b(new_n85_), .c(new_n92_), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n84_), .c(new_n83_), .d(new_n74_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n45_), .O(z09));
  nand3  g52(.a(new_n52_), .b(x18), .c(x01), .O(new_n96_));
  nand3  g53(.a(x22), .b(x21), .c(new_n90_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n96_), .O(new_n98_));
  nand2  g55(.a(x02), .b(new_n75_), .O(new_n99_));
  inv1   g56(.a(x13), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(x12), .c(new_n67_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n99_), .O(new_n102_));
  inv1   g59(.a(x14), .O(new_n103_));
  nand3  g60(.a(x16), .b(x15), .c(new_n103_), .O(new_n104_));
  nand3  g61(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n102_), .c(new_n98_), .O(new_n107_));
  oai21  g64(.a(new_n107_), .b(x00), .c(new_n45_), .O(z10));
  nor2   g65(.a(new_n46_), .b(x01), .O(new_n109_));
  nor2   g66(.a(x13), .b(new_n68_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n109_), .c(new_n67_), .O(new_n111_));
  nand4  g68(.a(new_n91_), .b(x21), .c(new_n90_), .d(new_n52_), .O(new_n112_));
  nor2   g69(.a(new_n86_), .b(x14), .O(new_n113_));
  nor2   g70(.a(x21), .b(new_n90_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n113_), .c(new_n87_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n111_), .c(new_n112_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n84_), .c(new_n74_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n45_), .O(z11));
  inv1   g75(.a(x24), .O(new_n119_));
  nand2  g76(.a(x10), .b(x02), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(x01), .c(x00), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand3  g81(.a(new_n124_), .b(new_n75_), .c(new_n74_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n119_), .c(x09), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n45_), .O(z12));
  nand3  g85(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n129_));
  nor2   g86(.a(new_n129_), .b(x19), .O(z13));
  nand3  g87(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(new_n132_));
  nand4  g89(.a(new_n132_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n133_));
  nor2   g90(.a(new_n133_), .b(x19), .O(z14));
  oai21  g91(.a(x10), .b(new_n75_), .c(x02), .O(new_n135_));
  nand3  g92(.a(new_n135_), .b(new_n45_), .c(x00), .O(new_n136_));
  nand4  g93(.a(x19), .b(x08), .c(new_n46_), .d(new_n75_), .O(new_n137_));
  nand2  g94(.a(new_n137_), .b(new_n136_), .O(z15));
  nor3   g95(.a(z08), .b(new_n75_), .c(x00), .O(z16));
  nand4  g96(.a(new_n45_), .b(x02), .c(new_n75_), .d(new_n74_), .O(new_n140_));
  inv1   g97(.a(new_n140_), .O(z17));
endmodule


