// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n136_, new_n137_,
    new_n138_;
  nand2  g00(.a(x12), .b(x01), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x01), .O(new_n46_));
  inv1   g03(.a(x02), .O(new_n47_));
  inv1   g04(.a(x09), .O(new_n48_));
  nand4  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  nor2   g07(.a(x19), .b(x18), .O(new_n51_));
  nand3  g08(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n49_), .c(new_n44_), .O(z00));
  inv1   g10(.a(x18), .O(new_n54_));
  nand4  g11(.a(x09), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n54_), .c(new_n50_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n54_), .c(new_n50_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand3  g20(.a(new_n63_), .b(x18), .c(new_n50_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n62_), .c(new_n44_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(x10), .d(new_n48_), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(x12), .O(z04));
  inv1   g27(.a(x12), .O(new_n71_));
  nor2   g28(.a(new_n47_), .b(new_n45_), .O(new_n72_));
  nand3  g29(.a(new_n72_), .b(x10), .c(x09), .O(new_n73_));
  aoi21  g30(.a(new_n73_), .b(new_n71_), .c(new_n46_), .O(z05));
  nand2  g31(.a(x11), .b(x10), .O(new_n75_));
  nor2   g32(.a(new_n75_), .b(x09), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n72_), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(new_n71_), .c(new_n46_), .O(z06));
  oai21  g35(.a(new_n75_), .b(x09), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n44_), .O(z07));
  nand3  g38(.a(x02), .b(new_n46_), .c(new_n45_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x04), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(x03), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nor2   g43(.a(new_n63_), .b(x18), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x17), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(new_n44_), .O(z08));
  inv1   g46(.a(x21), .O(new_n90_));
  inv1   g47(.a(x22), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x12), .c(new_n66_), .d(x02), .O(new_n93_));
  inv1   g50(.a(x14), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nand4  g53(.a(x20), .b(new_n96_), .c(new_n95_), .d(new_n94_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  and2   g55(.a(x18), .b(x01), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(new_n63_), .O(new_n100_));
  oai21  g57(.a(new_n97_), .b(new_n93_), .c(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n91_), .c(new_n90_), .d(new_n45_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n44_), .O(z09));
  nand3  g60(.a(new_n63_), .b(x18), .c(x01), .O(new_n104_));
  nand3  g61(.a(x22), .b(x21), .c(new_n98_), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n104_), .O(new_n106_));
  nand2  g63(.a(new_n66_), .b(x02), .O(new_n107_));
  nand3  g64(.a(new_n94_), .b(new_n92_), .c(x12), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g66(.a(x16), .b(x15), .O(new_n110_));
  nand3  g67(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n109_), .c(new_n106_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(x00), .c(new_n44_), .O(z10));
  nand4  g71(.a(new_n99_), .b(x21), .c(new_n98_), .d(new_n63_), .O(new_n115_));
  nor2   g72(.a(new_n95_), .b(x14), .O(new_n116_));
  nor2   g73(.a(x21), .b(new_n98_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n116_), .c(new_n96_), .O(new_n118_));
  oai21  g75(.a(new_n118_), .b(new_n93_), .c(new_n115_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n91_), .c(new_n45_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n44_), .O(z11));
  inv1   g78(.a(x24), .O(new_n122_));
  nand2  g79(.a(x10), .b(x02), .O(new_n123_));
  nand4  g80(.a(new_n123_), .b(new_n71_), .c(x01), .d(x00), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand3  g82(.a(new_n63_), .b(x17), .c(new_n47_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n46_), .c(new_n45_), .O(new_n128_));
  nand2  g85(.a(new_n128_), .b(new_n124_), .O(new_n129_));
  nand3  g86(.a(new_n129_), .b(new_n122_), .c(x09), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z12));
  nand3  g88(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n132_));
  nor3   g89(.a(new_n132_), .b(x19), .c(new_n50_), .O(z13));
  nand3  g90(.a(new_n51_), .b(new_n50_), .c(new_n59_), .O(new_n134_));
  oai21  g91(.a(new_n134_), .b(new_n49_), .c(new_n44_), .O(z14));
  oai21  g92(.a(x10), .b(new_n46_), .c(x02), .O(new_n136_));
  nand2  g93(.a(new_n136_), .b(x00), .O(new_n137_));
  nand3  g94(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n138_));
  nand3  g95(.a(new_n138_), .b(new_n137_), .c(new_n44_), .O(z15));
  aoi21  g96(.a(new_n71_), .b(x00), .c(new_n46_), .O(z16));
  nand2  g97(.a(new_n82_), .b(new_n44_), .O(z17));
endmodule


