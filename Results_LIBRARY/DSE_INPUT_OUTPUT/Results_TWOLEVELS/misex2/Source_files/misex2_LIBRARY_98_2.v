// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n143_,
    new_n144_, new_n147_;
  inv1   g000(.a(x07), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g011(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g012(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  nand4  g014(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  nand3  g016(.a(x07), .b(x02), .c(x00), .O(new_n60_));
  nand4  g017(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n60_), .c(x07), .O(new_n62_));
  nand2  g019(.a(new_n62_), .b(x01), .O(new_n63_));
  nor3   g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g021(.a(new_n64_), .b(new_n52_), .c(x18), .d(new_n50_), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(new_n63_), .O(z03));
  inv1   g023(.a(x01), .O(new_n67_));
  nor2   g024(.a(x09), .b(new_n46_), .O(new_n68_));
  nor2   g025(.a(x12), .b(x11), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n68_), .c(x10), .d(x00), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(x07), .c(new_n67_), .O(z04));
  nand4  g028(.a(x07), .b(x02), .c(x01), .d(x00), .O(new_n72_));
  nor3   g029(.a(new_n72_), .b(new_n57_), .c(new_n47_), .O(z05));
  inv1   g030(.a(x00), .O(new_n74_));
  inv1   g031(.a(x11), .O(new_n75_));
  nor3   g032(.a(new_n75_), .b(new_n57_), .c(x09), .O(new_n76_));
  nand4  g033(.a(new_n76_), .b(x07), .c(x02), .d(x01), .O(new_n77_));
  nor2   g034(.a(new_n77_), .b(new_n74_), .O(z06));
  nand2  g035(.a(new_n47_), .b(x07), .O(new_n79_));
  inv1   g036(.a(x12), .O(new_n80_));
  nand3  g037(.a(new_n80_), .b(x11), .c(x10), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n79_), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  inv1   g040(.a(new_n83_), .O(z07));
  nand2  g041(.a(x02), .b(new_n74_), .O(new_n85_));
  inv1   g042(.a(x05), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(x04), .O(new_n87_));
  nor3   g044(.a(new_n87_), .b(new_n85_), .c(x03), .O(new_n88_));
  nand3  g045(.a(x19), .b(new_n51_), .c(x17), .O(new_n89_));
  nor3   g046(.a(new_n89_), .b(x08), .c(x06), .O(new_n90_));
  nand2  g047(.a(new_n90_), .b(new_n88_), .O(new_n91_));
  aoi21  g048(.a(new_n91_), .b(new_n67_), .c(x07), .O(z08));
  inv1   g049(.a(x21), .O(new_n93_));
  inv1   g050(.a(x22), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n75_), .c(x02), .d(new_n67_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  nor2   g056(.a(x20), .b(x19), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x18), .c(x07), .d(x01), .O(new_n101_));
  oai21  g058(.a(new_n99_), .b(new_n95_), .c(new_n101_), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n94_), .c(new_n93_), .d(new_n74_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(z09));
  nand3  g061(.a(new_n52_), .b(x18), .c(x01), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nand3  g063(.a(x22), .b(x21), .c(new_n106_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n105_), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n67_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n75_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nand3  g070(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g071(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n112_), .c(new_n108_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n45_), .O(z10));
  nor2   g075(.a(new_n46_), .b(x01), .O(new_n119_));
  nor2   g076(.a(x13), .b(new_n80_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n75_), .O(new_n121_));
  and2   g078(.a(x18), .b(x01), .O(new_n122_));
  nand4  g079(.a(new_n122_), .b(x21), .c(new_n106_), .d(new_n52_), .O(new_n123_));
  nor2   g080(.a(new_n96_), .b(x14), .O(new_n124_));
  nor2   g081(.a(x21), .b(new_n106_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n124_), .c(new_n97_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n121_), .c(new_n123_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n94_), .c(new_n74_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n45_), .O(z11));
  inv1   g086(.a(x24), .O(new_n130_));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand4  g088(.a(new_n131_), .b(x07), .c(x01), .d(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n67_), .c(new_n74_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n132_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n130_), .c(x09), .O(new_n138_));
  inv1   g095(.a(new_n138_), .O(z12));
  nand3  g096(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x19), .O(z13));
  oai21  g098(.a(new_n58_), .b(new_n49_), .c(new_n45_), .O(z14));
  aoi22  g099(.a(new_n45_), .b(x00), .c(x19), .d(new_n67_), .O(new_n143_));
  nand4  g100(.a(new_n57_), .b(x07), .c(x01), .d(x00), .O(new_n144_));
  oai21  g101(.a(new_n143_), .b(x02), .c(new_n144_), .O(z15));
  aoi21  g102(.a(x07), .b(x00), .c(new_n67_), .O(z16));
  nand2  g103(.a(new_n48_), .b(x02), .O(new_n147_));
  inv1   g104(.a(new_n147_), .O(z17));
endmodule


