// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n112_, new_n113_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n140_, new_n141_, new_n143_,
    new_n144_, new_n147_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x10), .O(new_n46_));
  inv1   g003(.a(x00), .O(new_n47_));
  inv1   g004(.a(x01), .O(new_n48_));
  inv1   g005(.a(x02), .O(new_n49_));
  nand3  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n50_));
  nor3   g007(.a(new_n50_), .b(new_n46_), .c(x09), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x12), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  inv1   g010(.a(x09), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(x02), .O(new_n55_));
  nor2   g012(.a(x17), .b(new_n46_), .O(new_n56_));
  nor2   g013(.a(x19), .b(x18), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n56_), .c(new_n55_), .d(new_n47_), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(x12), .c(x01), .O(z01));
  nor3   g016(.a(new_n50_), .b(x10), .c(new_n54_), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n45_), .c(new_n44_), .d(x12), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x19), .O(z02));
  nand2  g019(.a(new_n49_), .b(new_n47_), .O(new_n63_));
  inv1   g020(.a(x19), .O(new_n64_));
  nand3  g021(.a(new_n64_), .b(x18), .c(new_n44_), .O(new_n65_));
  oai21  g022(.a(new_n65_), .b(new_n63_), .c(x12), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n48_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nand4  g025(.a(x12), .b(x11), .c(x10), .d(new_n54_), .O(new_n69_));
  oai21  g026(.a(new_n69_), .b(new_n68_), .c(new_n67_), .O(z03));
  inv1   g027(.a(x11), .O(new_n71_));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n71_), .c(x10), .d(new_n54_), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(x12), .O(z04));
  nand2  g032(.a(x01), .b(x00), .O(new_n76_));
  inv1   g033(.a(x12), .O(new_n77_));
  nand2  g034(.a(new_n77_), .b(new_n48_), .O(new_n78_));
  nand3  g035(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n76_), .c(new_n78_), .O(z05));
  nor3   g037(.a(new_n71_), .b(new_n46_), .c(x09), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(x02), .c(x01), .d(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n78_), .O(z06));
  nand2  g040(.a(new_n54_), .b(x01), .O(new_n84_));
  nand3  g041(.a(new_n77_), .b(x11), .c(x10), .O(new_n85_));
  oai22  g042(.a(new_n85_), .b(new_n84_), .c(new_n77_), .d(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  inv1   g044(.a(new_n87_), .O(z07));
  inv1   g045(.a(x05), .O(new_n89_));
  inv1   g046(.a(x06), .O(new_n90_));
  inv1   g047(.a(x07), .O(new_n91_));
  inv1   g048(.a(x04), .O(new_n92_));
  inv1   g049(.a(x03), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n92_), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(x08), .O(new_n97_));
  nand4  g054(.a(new_n97_), .b(new_n45_), .c(x17), .d(x12), .O(new_n98_));
  nor2   g055(.a(new_n98_), .b(new_n64_), .O(z08));
  inv1   g056(.a(x21), .O(new_n100_));
  inv1   g057(.a(x22), .O(new_n101_));
  inv1   g058(.a(x13), .O(new_n102_));
  nor2   g059(.a(new_n49_), .b(x01), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n102_), .c(new_n71_), .O(new_n104_));
  inv1   g061(.a(x14), .O(new_n105_));
  inv1   g062(.a(x15), .O(new_n106_));
  inv1   g063(.a(x16), .O(new_n107_));
  nand4  g064(.a(x20), .b(new_n107_), .c(new_n106_), .d(new_n105_), .O(new_n108_));
  inv1   g065(.a(x20), .O(new_n109_));
  nor2   g066(.a(new_n45_), .b(new_n48_), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(new_n109_), .c(new_n64_), .O(new_n111_));
  oai21  g068(.a(new_n108_), .b(new_n104_), .c(new_n111_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n101_), .c(new_n100_), .d(new_n47_), .O(new_n113_));
  nand2  g070(.a(new_n113_), .b(new_n78_), .O(z09));
  nand3  g071(.a(new_n64_), .b(x18), .c(x01), .O(new_n115_));
  nor4   g072(.a(new_n115_), .b(new_n101_), .c(new_n100_), .d(x20), .O(new_n116_));
  nor3   g073(.a(x14), .b(x13), .c(x11), .O(new_n117_));
  and2   g074(.a(new_n117_), .b(new_n103_), .O(new_n118_));
  nand3  g075(.a(new_n101_), .b(new_n100_), .c(x20), .O(new_n119_));
  nor3   g076(.a(new_n119_), .b(new_n107_), .c(new_n106_), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n118_), .c(new_n116_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(x00), .c(new_n78_), .O(z10));
  nand4  g079(.a(new_n110_), .b(x21), .c(new_n109_), .d(new_n64_), .O(new_n123_));
  nand4  g080(.a(new_n103_), .b(new_n102_), .c(x12), .d(new_n71_), .O(new_n124_));
  nor2   g081(.a(new_n106_), .b(x14), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n100_), .c(x20), .d(new_n107_), .O(new_n126_));
  oai21  g083(.a(new_n126_), .b(new_n124_), .c(new_n123_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n101_), .c(new_n47_), .O(new_n128_));
  inv1   g085(.a(new_n128_), .O(z11));
  aoi21  g086(.a(x10), .b(x02), .c(new_n48_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n64_), .b(x17), .c(new_n49_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n48_), .c(new_n47_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n131_), .c(x24), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n78_), .O(z12));
  nor4   g095(.a(new_n50_), .b(x19), .c(new_n44_), .d(new_n77_), .O(z13));
  nor3   g096(.a(x09), .b(x02), .c(x00), .O(new_n140_));
  nand4  g097(.a(new_n140_), .b(new_n57_), .c(new_n44_), .d(new_n46_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(x12), .c(x01), .O(z14));
  aoi21  g099(.a(x12), .b(new_n49_), .c(new_n130_), .O(new_n143_));
  nand4  g100(.a(x19), .b(x12), .c(new_n49_), .d(new_n48_), .O(new_n144_));
  oai21  g101(.a(new_n143_), .b(new_n47_), .c(new_n144_), .O(z15));
  oai21  g102(.a(new_n48_), .b(x00), .c(new_n78_), .O(z16));
  nand4  g103(.a(x12), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n147_));
  inv1   g104(.a(new_n147_), .O(z17));
endmodule


