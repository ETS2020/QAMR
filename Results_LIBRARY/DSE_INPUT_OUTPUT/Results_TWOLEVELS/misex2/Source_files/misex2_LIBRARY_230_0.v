// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n127_, new_n129_, new_n130_;
  nor3   g00(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(x16), .c(x01), .O(z00));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(new_n48_), .b(x09), .c(new_n52_), .d(new_n51_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(x16), .c(x01), .O(z01));
  inv1   g11(.a(x18), .O(new_n55_));
  inv1   g12(.a(x09), .O(new_n56_));
  inv1   g13(.a(x01), .O(new_n57_));
  nand3  g14(.a(new_n52_), .b(new_n57_), .c(new_n51_), .O(new_n58_));
  nor3   g15(.a(new_n58_), .b(x10), .c(new_n56_), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n55_), .c(new_n45_), .d(x16), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n52_), .b(new_n57_), .c(new_n51_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x18), .c(new_n45_), .d(x16), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n56_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  inv1   g24(.a(x16), .O(new_n68_));
  nand2  g25(.a(new_n68_), .b(new_n57_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n56_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n65_), .c(new_n69_), .O(z04));
  nand2  g30(.a(x01), .b(x00), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z05));
  inv1   g33(.a(x10), .O(new_n77_));
  nor3   g34(.a(new_n70_), .b(new_n77_), .c(x09), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x02), .c(x01), .d(x00), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n69_), .O(z06));
  nand3  g37(.a(new_n71_), .b(x11), .c(x10), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(new_n69_), .O(z07));
  nand2  g41(.a(x02), .b(new_n51_), .O(new_n85_));
  inv1   g42(.a(x05), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(x04), .O(new_n87_));
  nor3   g44(.a(new_n87_), .b(new_n85_), .c(x03), .O(new_n88_));
  nor3   g45(.a(x08), .b(x07), .c(x06), .O(new_n89_));
  nor2   g46(.a(new_n63_), .b(x18), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n89_), .c(new_n88_), .d(x17), .O(new_n91_));
  aoi21  g48(.a(new_n91_), .b(x16), .c(x01), .O(z08));
  inv1   g49(.a(x20), .O(new_n93_));
  inv1   g50(.a(x21), .O(new_n94_));
  inv1   g51(.a(x22), .O(new_n95_));
  nand4  g52(.a(new_n63_), .b(x18), .c(x01), .d(new_n51_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n95_), .c(new_n94_), .d(new_n93_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(z09));
  nand3  g56(.a(new_n63_), .b(x18), .c(x01), .O(new_n100_));
  nand3  g57(.a(x22), .b(x21), .c(new_n93_), .O(new_n101_));
  nor2   g58(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g59(.a(x02), .b(new_n57_), .O(new_n103_));
  inv1   g60(.a(x13), .O(new_n104_));
  nand3  g61(.a(new_n104_), .b(x12), .c(new_n70_), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  inv1   g63(.a(x14), .O(new_n107_));
  nand2  g64(.a(x15), .b(new_n107_), .O(new_n108_));
  nand3  g65(.a(new_n95_), .b(new_n94_), .c(x20), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n106_), .c(new_n102_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n69_), .O(z10));
  nand3  g69(.a(x18), .b(x01), .c(new_n51_), .O(new_n113_));
  nand4  g70(.a(new_n95_), .b(x21), .c(new_n93_), .d(new_n63_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n113_), .c(new_n69_), .O(z11));
  inv1   g72(.a(x24), .O(new_n116_));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(x01), .c(x00), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n63_), .b(x17), .c(new_n52_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n119_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(x16), .c(new_n57_), .d(new_n51_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n118_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n116_), .c(x09), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z12));
  nor4   g82(.a(new_n58_), .b(x19), .c(new_n45_), .d(new_n68_), .O(z13));
  nand4  g83(.a(new_n46_), .b(new_n44_), .c(new_n45_), .d(new_n77_), .O(new_n127_));
  aoi21  g84(.a(new_n127_), .b(x16), .c(x01), .O(z14));
  aoi21  g85(.a(new_n77_), .b(x01), .c(new_n52_), .O(new_n129_));
  aoi21  g86(.a(x19), .b(new_n52_), .c(new_n68_), .O(new_n130_));
  oai22  g87(.a(new_n130_), .b(x01), .c(new_n129_), .d(new_n51_), .O(z15));
  oai21  g88(.a(new_n57_), .b(x00), .c(new_n69_), .O(z16));
  aoi21  g89(.a(new_n85_), .b(x16), .c(x01), .O(z17));
endmodule


