// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n62_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n121_, new_n123_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n47_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x02), .O(new_n50_));
  nand2  g07(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  inv1   g08(.a(new_n51_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  nand2  g11(.a(new_n50_), .b(x09), .O(new_n55_));
  inv1   g12(.a(new_n55_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand4  g16(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  nand3  g18(.a(new_n50_), .b(x18), .c(new_n44_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z03));
  nor2   g20(.a(new_n48_), .b(new_n47_), .O(z04));
  nand3  g21(.a(x02), .b(new_n48_), .c(x00), .O(new_n66_));
  inv1   g22(.a(new_n66_), .O(z07));
  inv1   g23(.a(x19), .O(new_n68_));
  inv1   g24(.a(x08), .O(new_n69_));
  inv1   g25(.a(x05), .O(new_n70_));
  inv1   g26(.a(x06), .O(new_n71_));
  inv1   g27(.a(x03), .O(new_n72_));
  nand4  g28(.a(new_n72_), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n73_));
  inv1   g29(.a(new_n73_), .O(new_n74_));
  nand4  g30(.a(new_n74_), .b(new_n71_), .c(new_n70_), .d(x04), .O(new_n75_));
  nor2   g31(.a(new_n75_), .b(x07), .O(new_n76_));
  nand4  g32(.a(new_n76_), .b(new_n45_), .c(x17), .d(new_n69_), .O(new_n77_));
  nor2   g33(.a(new_n77_), .b(new_n68_), .O(z08));
  inv1   g34(.a(x21), .O(new_n79_));
  inv1   g35(.a(x22), .O(new_n80_));
  inv1   g36(.a(x11), .O(new_n81_));
  inv1   g37(.a(x02), .O(new_n82_));
  nor2   g38(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g39(.a(new_n83_), .b(x12), .c(new_n81_), .O(new_n84_));
  inv1   g40(.a(x15), .O(new_n85_));
  inv1   g41(.a(x16), .O(new_n86_));
  nor2   g42(.a(x14), .b(x13), .O(new_n87_));
  nand4  g43(.a(new_n87_), .b(x20), .c(new_n86_), .d(new_n85_), .O(new_n88_));
  inv1   g44(.a(x20), .O(new_n89_));
  nand4  g45(.a(new_n89_), .b(new_n68_), .c(x18), .d(x01), .O(new_n90_));
  oai21  g46(.a(new_n88_), .b(new_n84_), .c(new_n90_), .O(new_n91_));
  nand4  g47(.a(new_n91_), .b(new_n80_), .c(new_n79_), .d(new_n47_), .O(new_n92_));
  inv1   g48(.a(new_n92_), .O(z09));
  nor3   g49(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n94_));
  nand4  g50(.a(new_n94_), .b(new_n68_), .c(x18), .d(x01), .O(new_n95_));
  inv1   g51(.a(x13), .O(new_n96_));
  nand3  g52(.a(new_n96_), .b(x12), .c(new_n81_), .O(new_n97_));
  inv1   g53(.a(new_n97_), .O(new_n98_));
  nor3   g54(.a(new_n86_), .b(new_n85_), .c(x14), .O(new_n99_));
  nand3  g55(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n100_));
  inv1   g56(.a(new_n100_), .O(new_n101_));
  nand4  g57(.a(new_n101_), .b(new_n99_), .c(new_n98_), .d(new_n83_), .O(new_n102_));
  aoi21  g58(.a(new_n102_), .b(new_n95_), .c(x00), .O(z10));
  nand2  g59(.a(new_n68_), .b(x18), .O(new_n104_));
  nand3  g60(.a(new_n80_), .b(x21), .c(new_n89_), .O(new_n105_));
  oai21  g61(.a(new_n105_), .b(new_n104_), .c(new_n47_), .O(new_n106_));
  nand2  g62(.a(new_n106_), .b(x01), .O(new_n107_));
  inv1   g63(.a(x14), .O(new_n108_));
  nand2  g64(.a(new_n83_), .b(new_n47_), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n97_), .O(new_n110_));
  nor2   g66(.a(x16), .b(new_n85_), .O(new_n111_));
  nand4  g67(.a(new_n111_), .b(new_n110_), .c(new_n101_), .d(new_n108_), .O(new_n112_));
  nand2  g68(.a(new_n112_), .b(new_n107_), .O(z11));
  inv1   g69(.a(z04), .O(new_n114_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g71(.a(new_n68_), .b(x17), .c(new_n82_), .O(new_n116_));
  aoi21  g72(.a(new_n116_), .b(new_n115_), .c(x24), .O(new_n117_));
  nand4  g73(.a(new_n117_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n114_), .O(z12));
  oai21  g75(.a(new_n116_), .b(new_n49_), .c(new_n114_), .O(z13));
  nand4  g76(.a(new_n52_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n121_));
  nor2   g77(.a(new_n121_), .b(x19), .O(z14));
  nor2   g78(.a(x19), .b(x00), .O(new_n123_));
  nor3   g79(.a(new_n123_), .b(x02), .c(x01), .O(z15));
  nand2  g80(.a(new_n109_), .b(new_n114_), .O(z17));
  zero   g81(.O(z06));
  nor2   g82(.a(new_n48_), .b(new_n47_), .O(z05));
  buf    g83(.a(x01), .O(z16));
endmodule


