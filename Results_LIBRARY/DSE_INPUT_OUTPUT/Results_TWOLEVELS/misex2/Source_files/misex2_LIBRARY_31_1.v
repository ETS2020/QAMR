// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n123_,
    new_n125_, new_n127_, new_n128_, new_n131_;
  inv1   g00(.a(x16), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  inv1   g02(.a(x17), .O(new_n46_));
  nor2   g03(.a(x19), .b(x18), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand2  g06(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g08(.a(x00), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(new_n49_), .b(x09), .c(new_n53_), .d(new_n52_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g12(.a(x01), .O(new_n56_));
  nand4  g13(.a(x09), .b(new_n53_), .c(new_n56_), .d(new_n52_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x10), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(new_n46_), .c(new_n44_), .O(new_n59_));
  nor3   g16(.a(new_n59_), .b(x19), .c(x18), .O(z02));
  nand3  g17(.a(new_n53_), .b(new_n56_), .c(new_n52_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n46_), .d(new_n44_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(x09), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n65_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  nand2  g24(.a(x16), .b(new_n56_), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n65_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n64_), .c(new_n68_), .O(z04));
  inv1   g29(.a(x10), .O(new_n73_));
  nand4  g30(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n73_), .O(z05));
  nor3   g32(.a(new_n69_), .b(new_n73_), .c(x09), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n68_), .O(z06));
  nand2  g35(.a(new_n65_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n70_), .b(x11), .c(x10), .O(new_n80_));
  oai22  g37(.a(new_n80_), .b(new_n79_), .c(x16), .d(x01), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(z07));
  nand2  g40(.a(x02), .b(new_n52_), .O(new_n84_));
  inv1   g41(.a(x05), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(x04), .O(new_n86_));
  nor3   g43(.a(new_n86_), .b(new_n84_), .c(x03), .O(new_n87_));
  nor3   g44(.a(x08), .b(x07), .c(x06), .O(new_n88_));
  nor2   g45(.a(new_n62_), .b(x18), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n88_), .c(new_n87_), .d(x17), .O(new_n90_));
  aoi21  g47(.a(new_n90_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g48(.a(x21), .O(new_n92_));
  inv1   g49(.a(x22), .O(new_n93_));
  nand4  g50(.a(x12), .b(new_n69_), .c(x02), .d(new_n56_), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  nor2   g52(.a(x14), .b(x13), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(x20), .c(new_n44_), .d(new_n95_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  nand2  g55(.a(x18), .b(x01), .O(new_n99_));
  inv1   g56(.a(new_n99_), .O(new_n100_));
  nand3  g57(.a(new_n100_), .b(new_n98_), .c(new_n62_), .O(new_n101_));
  oai21  g58(.a(new_n97_), .b(new_n94_), .c(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n93_), .c(new_n92_), .d(new_n52_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(z09));
  nand2  g61(.a(new_n100_), .b(new_n52_), .O(new_n105_));
  nand4  g62(.a(x22), .b(x21), .c(new_n98_), .d(new_n62_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n105_), .c(new_n68_), .O(z10));
  nand3  g64(.a(x21), .b(new_n98_), .c(new_n62_), .O(new_n108_));
  nand4  g65(.a(new_n96_), .b(new_n92_), .c(x20), .d(x15), .O(new_n109_));
  oai22  g66(.a(new_n109_), .b(new_n94_), .c(new_n108_), .d(new_n99_), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(new_n93_), .c(new_n52_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n68_), .O(z11));
  inv1   g69(.a(x24), .O(new_n113_));
  nand2  g70(.a(x10), .b(x02), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(x01), .c(x00), .O(new_n115_));
  oai21  g72(.a(x19), .b(x02), .c(x23), .O(new_n116_));
  nand3  g73(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n44_), .c(new_n56_), .d(new_n52_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n113_), .c(x09), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z12));
  nand4  g79(.a(new_n62_), .b(x17), .c(new_n53_), .d(new_n52_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g81(.a(new_n47_), .b(new_n45_), .c(new_n46_), .d(new_n73_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g83(.a(new_n73_), .b(x01), .c(new_n53_), .O(new_n127_));
  aoi21  g84(.a(x19), .b(new_n53_), .c(x16), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(x01), .c(new_n127_), .d(new_n52_), .O(z15));
  nor2   g86(.a(new_n56_), .b(x00), .O(z16));
  nand4  g87(.a(new_n44_), .b(x02), .c(new_n56_), .d(new_n52_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z17));
endmodule


