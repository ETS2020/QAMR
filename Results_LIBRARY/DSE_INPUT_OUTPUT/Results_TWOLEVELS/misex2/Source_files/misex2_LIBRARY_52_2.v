// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n127_, new_n129_, new_n130_,
    new_n132_, new_n134_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x00), .O(new_n53_));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n46_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n53_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n58_));
  inv1   g15(.a(new_n58_), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n60_));
  nor2   g17(.a(new_n60_), .b(x19), .O(z02));
  inv1   g18(.a(x01), .O(new_n62_));
  nor2   g19(.a(x09), .b(new_n46_), .O(new_n63_));
  nand3  g20(.a(x12), .b(x11), .c(x10), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  aoi21  g22(.a(new_n65_), .b(new_n63_), .c(new_n62_), .O(new_n66_));
  inv1   g23(.a(x19), .O(new_n67_));
  nor2   g24(.a(x02), .b(x01), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n67_), .c(x18), .d(new_n44_), .O(new_n69_));
  oai21  g26(.a(new_n66_), .b(new_n53_), .c(new_n69_), .O(z03));
  nor2   g27(.a(x12), .b(x11), .O(new_n71_));
  nand3  g28(.a(new_n71_), .b(new_n63_), .c(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x01), .c(new_n53_), .O(z04));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  nor3   g31(.a(new_n74_), .b(new_n57_), .c(new_n47_), .O(z05));
  nand4  g32(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n62_), .c(new_n53_), .O(z06));
  inv1   g34(.a(new_n74_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n47_), .O(new_n79_));
  nor2   g36(.a(new_n79_), .b(x12), .O(z07));
  nor2   g37(.a(x03), .b(new_n46_), .O(new_n81_));
  nor2   g38(.a(x06), .b(x05), .O(new_n82_));
  nand3  g39(.a(x19), .b(new_n45_), .c(x17), .O(new_n83_));
  nor3   g40(.a(new_n83_), .b(x08), .c(x07), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n82_), .c(new_n81_), .d(x04), .O(new_n85_));
  aoi21  g42(.a(new_n85_), .b(new_n53_), .c(x01), .O(z08));
  inv1   g43(.a(x21), .O(new_n87_));
  inv1   g44(.a(x22), .O(new_n88_));
  inv1   g45(.a(x11), .O(new_n89_));
  nand4  g46(.a(x12), .b(new_n89_), .c(x02), .d(new_n62_), .O(new_n90_));
  inv1   g47(.a(x15), .O(new_n91_));
  inv1   g48(.a(x16), .O(new_n92_));
  nor2   g49(.a(x14), .b(x13), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(x20), .c(new_n92_), .d(new_n91_), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(x18), .c(x01), .O(new_n96_));
  oai21  g53(.a(new_n94_), .b(new_n90_), .c(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n88_), .c(new_n87_), .d(new_n53_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(z09));
  nand4  g56(.a(new_n93_), .b(x12), .c(new_n89_), .d(x02), .O(new_n100_));
  nand3  g57(.a(new_n88_), .b(new_n87_), .c(x20), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand3  g59(.a(new_n102_), .b(x16), .c(x15), .O(new_n103_));
  oai21  g60(.a(new_n103_), .b(new_n100_), .c(new_n53_), .O(new_n104_));
  nand2  g61(.a(new_n104_), .b(new_n62_), .O(new_n105_));
  nor3   g62(.a(new_n45_), .b(new_n62_), .c(x00), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n95_), .c(x22), .d(x21), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n105_), .O(z10));
  nand3  g65(.a(new_n102_), .b(new_n92_), .c(x15), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n100_), .c(new_n53_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n62_), .O(new_n111_));
  nand4  g68(.a(new_n106_), .b(new_n95_), .c(new_n88_), .d(x21), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(z11));
  inv1   g70(.a(x24), .O(new_n114_));
  nand2  g71(.a(x10), .b(x02), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x00), .O(new_n116_));
  nand2  g73(.a(x23), .b(x19), .O(new_n117_));
  oai21  g74(.a(x19), .b(new_n44_), .c(new_n117_), .O(new_n118_));
  and2   g75(.a(x23), .b(x02), .O(new_n119_));
  aoi21  g76(.a(new_n118_), .b(new_n46_), .c(new_n119_), .O(new_n120_));
  oai21  g77(.a(new_n120_), .b(x01), .c(new_n116_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n114_), .c(x09), .O(new_n122_));
  nand2  g79(.a(new_n62_), .b(x00), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(z12));
  nand3  g81(.a(new_n67_), .b(x17), .c(new_n46_), .O(new_n125_));
  aoi21  g82(.a(new_n125_), .b(new_n53_), .c(x01), .O(z13));
  nand4  g83(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(x19), .O(z14));
  nand3  g85(.a(new_n115_), .b(x01), .c(x00), .O(new_n129_));
  nand3  g86(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n129_), .O(z15));
  nand2  g88(.a(x01), .b(new_n53_), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(new_n123_), .O(z16));
  nand2  g90(.a(new_n48_), .b(x02), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z17));
endmodule


