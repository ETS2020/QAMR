// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n121_, new_n123_, new_n124_, new_n127_;
  inv1   g00(.a(x12), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  inv1   g03(.a(x10), .O(new_n47_));
  inv1   g04(.a(x00), .O(new_n48_));
  inv1   g05(.a(x01), .O(new_n49_));
  inv1   g06(.a(x02), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n51_));
  nor3   g08(.a(new_n51_), .b(new_n47_), .c(x09), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  inv1   g11(.a(x09), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x02), .O(new_n56_));
  nor2   g13(.a(x17), .b(new_n47_), .O(new_n57_));
  nor2   g14(.a(x19), .b(x18), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(new_n48_), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x01), .O(z01));
  nor3   g17(.a(new_n51_), .b(x10), .c(new_n55_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z02));
  nand2  g20(.a(new_n50_), .b(new_n48_), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nand3  g22(.a(new_n65_), .b(x18), .c(new_n45_), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n49_), .O(new_n68_));
  nand3  g25(.a(new_n55_), .b(x02), .c(x00), .O(new_n69_));
  nand3  g26(.a(x12), .b(x11), .c(x10), .O(new_n70_));
  oai21  g27(.a(new_n70_), .b(new_n69_), .c(new_n68_), .O(z03));
  nor2   g28(.a(new_n49_), .b(new_n48_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(x10), .c(new_n55_), .d(x02), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(x12), .c(x11), .O(z04));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand2  g32(.a(x12), .b(new_n49_), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n75_), .c(new_n76_), .O(z05));
  oai21  g35(.a(x12), .b(x01), .c(x11), .O(new_n79_));
  nor2   g36(.a(new_n79_), .b(new_n47_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n55_), .c(x02), .d(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n76_), .O(z06));
  nand2  g39(.a(x11), .b(x10), .O(new_n83_));
  oai21  g40(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(new_n44_), .c(x02), .d(x00), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z07));
  inv1   g43(.a(x05), .O(new_n87_));
  inv1   g44(.a(x06), .O(new_n88_));
  inv1   g45(.a(x07), .O(new_n89_));
  inv1   g46(.a(x04), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x02), .c(new_n49_), .d(new_n48_), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(new_n87_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(x08), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n46_), .c(x17), .d(new_n44_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n65_), .O(z08));
  nand3  g54(.a(x18), .b(x01), .c(new_n48_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n65_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n98_), .c(new_n76_), .O(z09));
  nand4  g60(.a(x22), .b(x21), .c(new_n99_), .d(new_n65_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n98_), .c(new_n76_), .O(z10));
  nand4  g62(.a(new_n65_), .b(x18), .c(x01), .d(new_n48_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n101_), .c(x21), .d(new_n99_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(z11));
  aoi21  g66(.a(x10), .b(x02), .c(new_n49_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(x00), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g69(.a(new_n65_), .b(x17), .c(new_n50_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n49_), .c(new_n48_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n111_), .c(x24), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(x09), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n76_), .O(z12));
  nor4   g75(.a(new_n51_), .b(x19), .c(new_n45_), .d(x12), .O(z13));
  nor3   g76(.a(x09), .b(x02), .c(x00), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n58_), .c(new_n45_), .d(new_n47_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g79(.a(new_n44_), .b(new_n50_), .c(new_n110_), .O(new_n123_));
  nand4  g80(.a(x19), .b(new_n44_), .c(new_n50_), .d(new_n49_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n48_), .c(new_n124_), .O(z15));
  oai21  g82(.a(new_n49_), .b(x00), .c(new_n76_), .O(z16));
  nand4  g83(.a(new_n44_), .b(x02), .c(new_n49_), .d(new_n48_), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z17));
endmodule


