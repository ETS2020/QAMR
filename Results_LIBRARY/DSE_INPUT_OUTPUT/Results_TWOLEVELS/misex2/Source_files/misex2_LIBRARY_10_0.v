// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n142_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x22), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  inv1   g002(.a(x18), .O(new_n46_));
  inv1   g003(.a(x19), .O(new_n47_));
  inv1   g004(.a(x10), .O(new_n48_));
  inv1   g005(.a(x00), .O(new_n49_));
  inv1   g006(.a(x01), .O(new_n50_));
  inv1   g007(.a(x02), .O(new_n51_));
  inv1   g008(.a(x09), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor2   g010(.a(new_n53_), .b(new_n48_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(new_n44_), .O(z00));
  nand4  g013(.a(x09), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n57_));
  inv1   g014(.a(new_n57_), .O(new_n58_));
  nand4  g015(.a(new_n58_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z01));
  nor2   g017(.a(x22), .b(x09), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(x09), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n63_));
  nand4  g020(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(new_n48_), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z02));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g023(.a(x22), .b(x12), .c(x11), .d(x10), .O(new_n67_));
  oai21  g024(.a(new_n67_), .b(new_n66_), .c(x22), .O(new_n68_));
  nand2  g025(.a(new_n68_), .b(new_n52_), .O(new_n69_));
  nor3   g026(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n47_), .c(x18), .d(new_n45_), .O(new_n71_));
  nand2  g028(.a(new_n71_), .b(new_n69_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  inv1   g031(.a(new_n66_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(x10), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(x22), .c(x09), .O(z04));
  nand4  g034(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n48_), .O(z05));
  nand3  g036(.a(x22), .b(x11), .c(x10), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(new_n52_), .c(x02), .d(x01), .O(new_n82_));
  nor2   g039(.a(new_n82_), .b(new_n49_), .O(z06));
  nand3  g040(.a(x10), .b(new_n52_), .c(x01), .O(new_n84_));
  nand3  g041(.a(x22), .b(new_n74_), .c(x11), .O(new_n85_));
  oai22  g042(.a(new_n85_), .b(new_n84_), .c(new_n61_), .d(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  inv1   g044(.a(new_n87_), .O(z07));
  inv1   g045(.a(x03), .O(new_n89_));
  nor2   g046(.a(new_n51_), .b(x01), .O(new_n90_));
  inv1   g047(.a(x04), .O(new_n91_));
  nor2   g048(.a(x05), .b(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n90_), .c(new_n89_), .d(new_n49_), .O(new_n93_));
  nor3   g050(.a(x08), .b(x07), .c(x06), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(x19), .c(new_n46_), .d(x17), .O(new_n95_));
  oai21  g052(.a(new_n95_), .b(new_n93_), .c(new_n62_), .O(z08));
  inv1   g053(.a(x21), .O(new_n97_));
  nand4  g054(.a(x12), .b(new_n73_), .c(x02), .d(new_n50_), .O(new_n98_));
  inv1   g055(.a(x15), .O(new_n99_));
  inv1   g056(.a(x16), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x20), .c(new_n100_), .d(new_n99_), .O(new_n102_));
  inv1   g059(.a(x20), .O(new_n103_));
  and2   g060(.a(x18), .b(x01), .O(new_n104_));
  nand3  g061(.a(new_n104_), .b(new_n103_), .c(new_n47_), .O(new_n105_));
  oai21  g062(.a(new_n102_), .b(new_n98_), .c(new_n105_), .O(new_n106_));
  nand3  g063(.a(new_n106_), .b(new_n97_), .c(new_n49_), .O(new_n107_));
  aoi21  g064(.a(new_n107_), .b(x09), .c(x22), .O(z09));
  nand3  g065(.a(new_n47_), .b(x18), .c(x01), .O(new_n109_));
  nand3  g066(.a(x22), .b(x21), .c(new_n103_), .O(new_n110_));
  nor2   g067(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g068(.a(x02), .b(new_n50_), .O(new_n112_));
  inv1   g069(.a(x13), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(x12), .c(new_n73_), .O(new_n114_));
  nor2   g071(.a(new_n114_), .b(new_n112_), .O(new_n115_));
  inv1   g072(.a(x14), .O(new_n116_));
  nand3  g073(.a(x16), .b(x15), .c(new_n116_), .O(new_n117_));
  nand3  g074(.a(new_n44_), .b(new_n97_), .c(x20), .O(new_n118_));
  nor2   g075(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n115_), .c(new_n111_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(x00), .c(new_n62_), .O(z10));
  nor2   g078(.a(x13), .b(new_n74_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n90_), .c(new_n73_), .O(new_n123_));
  nand4  g080(.a(new_n104_), .b(x21), .c(new_n103_), .d(new_n47_), .O(new_n124_));
  nor2   g081(.a(new_n99_), .b(x14), .O(new_n125_));
  nor2   g082(.a(x21), .b(new_n103_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n125_), .c(new_n100_), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(new_n123_), .c(new_n124_), .O(new_n128_));
  nand4  g085(.a(new_n128_), .b(new_n44_), .c(x09), .d(new_n49_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n47_), .b(x17), .c(new_n51_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n50_), .c(new_n49_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  inv1   g096(.a(new_n139_), .O(z12));
  nor2   g097(.a(new_n61_), .b(x19), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(x17), .c(new_n51_), .d(new_n50_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x00), .O(z13));
  nor2   g100(.a(new_n53_), .b(x10), .O(new_n144_));
  nand4  g101(.a(new_n144_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n145_));
  nor2   g102(.a(new_n145_), .b(new_n44_), .O(z14));
  oai21  g103(.a(x10), .b(new_n50_), .c(x02), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(x00), .O(new_n148_));
  nand3  g105(.a(x19), .b(new_n51_), .c(new_n50_), .O(new_n149_));
  nand3  g106(.a(new_n149_), .b(new_n148_), .c(new_n62_), .O(z15));
  oai21  g107(.a(new_n50_), .b(x00), .c(new_n62_), .O(z16));
  nor4   g108(.a(new_n61_), .b(new_n51_), .c(x01), .d(x00), .O(z17));
endmodule


