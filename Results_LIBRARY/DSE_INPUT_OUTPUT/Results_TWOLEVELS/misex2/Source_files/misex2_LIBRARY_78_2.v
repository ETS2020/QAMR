// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x16), .O(new_n44_));
  inv1   g001(.a(x09), .O(new_n45_));
  nor3   g002(.a(x02), .b(x01), .c(x00), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand4  g004(.a(new_n47_), .b(new_n46_), .c(x10), .d(new_n45_), .O(new_n48_));
  aoi21  g005(.a(new_n48_), .b(new_n44_), .c(x17), .O(z00));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x10), .O(new_n52_));
  inv1   g009(.a(x00), .O(new_n53_));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nand3  g012(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nor3   g013(.a(new_n56_), .b(new_n52_), .c(new_n45_), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x19), .O(z01));
  nand4  g016(.a(new_n47_), .b(new_n46_), .c(new_n52_), .d(x09), .O(new_n60_));
  aoi21  g017(.a(new_n60_), .b(new_n44_), .c(x17), .O(z02));
  nand2  g018(.a(new_n50_), .b(x16), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(x12), .c(x11), .d(x10), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x09), .O(new_n64_));
  nand4  g021(.a(new_n64_), .b(x02), .c(x01), .d(x00), .O(new_n65_));
  nor2   g022(.a(x19), .b(new_n51_), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n46_), .c(new_n50_), .d(new_n44_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n65_), .O(z03));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  inv1   g026(.a(x11), .O(new_n70_));
  inv1   g027(.a(x12), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n45_), .O(new_n72_));
  oai21  g029(.a(new_n72_), .b(new_n69_), .c(new_n62_), .O(z04));
  nor2   g030(.a(x17), .b(new_n44_), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(new_n52_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(x09), .c(x02), .d(x01), .O(new_n76_));
  nor2   g033(.a(new_n76_), .b(new_n53_), .O(z05));
  nor2   g034(.a(new_n74_), .b(new_n70_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x10), .c(new_n45_), .d(x02), .O(new_n79_));
  nor3   g036(.a(new_n79_), .b(new_n54_), .c(new_n53_), .O(z06));
  nand4  g037(.a(new_n62_), .b(new_n71_), .c(x11), .d(x10), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n62_), .O(z07));
  nor2   g041(.a(new_n55_), .b(x01), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n53_), .O(new_n86_));
  inv1   g043(.a(x03), .O(new_n87_));
  inv1   g044(.a(x05), .O(new_n88_));
  nand3  g045(.a(new_n88_), .b(x04), .c(new_n87_), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  inv1   g048(.a(x19), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x18), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n94_));
  nand2  g051(.a(new_n94_), .b(new_n62_), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  aoi21  g054(.a(new_n50_), .b(x16), .c(x20), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(new_n92_), .c(x18), .d(x01), .O(new_n99_));
  nor2   g056(.a(new_n71_), .b(x11), .O(new_n100_));
  nor2   g057(.a(x14), .b(x13), .O(new_n101_));
  inv1   g058(.a(x15), .O(new_n102_));
  nand3  g059(.a(x20), .b(new_n44_), .c(new_n102_), .O(new_n103_));
  inv1   g060(.a(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n101_), .c(new_n100_), .d(new_n85_), .O(new_n105_));
  nand2  g062(.a(new_n105_), .b(new_n99_), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(new_n97_), .c(new_n96_), .d(new_n53_), .O(new_n107_));
  inv1   g064(.a(new_n107_), .O(z09));
  inv1   g065(.a(x20), .O(new_n109_));
  nand4  g066(.a(new_n62_), .b(x22), .c(x21), .d(new_n109_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n92_), .c(x18), .d(x01), .O(new_n112_));
  nand3  g069(.a(new_n70_), .b(x02), .c(new_n54_), .O(new_n113_));
  inv1   g070(.a(new_n113_), .O(new_n114_));
  nand3  g071(.a(x17), .b(x16), .c(x15), .O(new_n115_));
  nor4   g072(.a(new_n115_), .b(x22), .c(x21), .d(new_n109_), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n114_), .c(new_n101_), .d(x12), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n112_), .c(x00), .O(z10));
  aoi21  g075(.a(new_n50_), .b(x16), .c(new_n96_), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n109_), .c(new_n92_), .d(x18), .O(new_n120_));
  inv1   g077(.a(x13), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(x12), .c(new_n70_), .O(new_n122_));
  inv1   g079(.a(new_n122_), .O(new_n123_));
  nor2   g080(.a(new_n102_), .b(x14), .O(new_n124_));
  nand3  g081(.a(new_n96_), .b(x20), .c(new_n44_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n124_), .c(new_n123_), .d(new_n85_), .O(new_n127_));
  oai21  g084(.a(new_n120_), .b(new_n54_), .c(new_n127_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n97_), .c(new_n53_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n92_), .b(x17), .c(new_n55_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n54_), .c(new_n53_), .O(new_n136_));
  aoi21  g093(.a(new_n136_), .b(new_n132_), .c(x24), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(x09), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n62_), .O(z12));
  nor3   g096(.a(new_n56_), .b(x19), .c(new_n50_), .O(z13));
  nor3   g097(.a(new_n56_), .b(x10), .c(x09), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n51_), .c(new_n50_), .d(new_n44_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z14));
  oai21  g100(.a(x10), .b(new_n54_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n55_), .c(new_n54_), .O(new_n146_));
  aoi21  g103(.a(new_n146_), .b(new_n145_), .c(new_n74_), .O(z15));
  nor3   g104(.a(new_n74_), .b(new_n54_), .c(x00), .O(z16));
  nand2  g105(.a(new_n86_), .b(new_n62_), .O(z17));
endmodule


