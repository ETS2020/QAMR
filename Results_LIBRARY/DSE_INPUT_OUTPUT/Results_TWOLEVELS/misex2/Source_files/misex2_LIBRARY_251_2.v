// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n121_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n130_;
  inv1   g00(.a(x14), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x12), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z00));
  nand4  g12(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n56_));
  oai21  g13(.a(new_n56_), .b(new_n54_), .c(new_n45_), .O(z01));
  aoi21  g14(.a(new_n44_), .b(x12), .c(x19), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(new_n52_), .c(new_n51_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x00), .O(z02));
  nand3  g19(.a(new_n58_), .b(x18), .c(new_n51_), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  nand4  g21(.a(new_n64_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n65_));
  nor2   g22(.a(new_n47_), .b(new_n46_), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(new_n49_), .c(x02), .O(new_n67_));
  nand4  g24(.a(x14), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n67_), .c(new_n65_), .O(z03));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n49_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n70_), .c(new_n45_), .O(z04));
  inv1   g31(.a(new_n66_), .O(new_n75_));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n45_), .O(z05));
  nand2  g34(.a(new_n44_), .b(x12), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x11), .c(x10), .d(new_n49_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(x02), .c(x01), .d(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n45_), .O(z06));
  nand3  g39(.a(new_n72_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g40(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(new_n45_), .O(z07));
  inv1   g43(.a(x03), .O(new_n87_));
  inv1   g44(.a(x04), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  inv1   g46(.a(x06), .O(new_n90_));
  inv1   g47(.a(x07), .O(new_n91_));
  nand4  g48(.a(new_n45_), .b(x19), .c(new_n52_), .d(x17), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x08), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n91_), .c(new_n90_), .d(new_n89_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n88_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n87_), .c(x02), .d(new_n47_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(x00), .O(z08));
  nand3  g54(.a(x18), .b(x01), .c(new_n46_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x22), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n100_), .c(new_n99_), .d(new_n53_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n98_), .c(new_n45_), .O(z09));
  nand4  g60(.a(new_n45_), .b(x22), .c(x21), .d(new_n99_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n53_), .c(x18), .d(x01), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(x00), .O(z10));
  nand4  g64(.a(new_n45_), .b(new_n101_), .c(x21), .d(new_n99_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n53_), .c(x18), .d(x01), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(x00), .O(z11));
  nand2  g68(.a(x10), .b(x02), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(x01), .c(x00), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n53_), .b(x17), .c(new_n48_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n47_), .c(new_n46_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n113_), .c(x24), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(x09), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n45_), .O(z12));
  nand4  g77(.a(new_n58_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n121_));
  nor2   g78(.a(new_n121_), .b(x00), .O(z13));
  nand4  g79(.a(new_n60_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n123_));
  nor2   g80(.a(new_n123_), .b(x00), .O(z14));
  oai21  g81(.a(x10), .b(new_n47_), .c(x02), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x00), .O(new_n126_));
  nand3  g83(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n126_), .c(new_n45_), .O(z15));
  oai21  g85(.a(new_n47_), .b(x00), .c(new_n45_), .O(z16));
  nand3  g86(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n45_), .O(z17));
endmodule


