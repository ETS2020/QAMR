// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n71_, new_n73_, new_n74_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n99_, new_n100_, new_n101_, new_n102_,
    new_n103_, new_n105_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n114_, new_n116_, new_n118_,
    new_n120_, new_n121_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  nor2   g03(.a(new_n46_), .b(x09), .O(new_n47_));
  nor3   g04(.a(x19), .b(x18), .c(x17), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n45_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(new_n44_), .c(x00), .O(z00));
  nand4  g07(.a(new_n48_), .b(x10), .c(x09), .d(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x00), .O(z01));
  nand4  g09(.a(new_n48_), .b(new_n46_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  inv1   g12(.a(x17), .O(new_n56_));
  inv1   g13(.a(x19), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(x18), .c(new_n56_), .d(new_n45_), .O(new_n58_));
  nand2  g15(.a(new_n58_), .b(new_n44_), .O(new_n59_));
  nand2  g16(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nand3  g17(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nand3  g18(.a(new_n47_), .b(x12), .c(x11), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n60_), .O(z03));
  inv1   g20(.a(x09), .O(new_n64_));
  inv1   g21(.a(x11), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(x10), .d(new_n64_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(x12), .O(z04));
  nor3   g26(.a(new_n66_), .b(new_n46_), .c(new_n64_), .O(z05));
  nand4  g27(.a(x11), .b(x10), .c(new_n64_), .d(x02), .O(new_n71_));
  nor3   g28(.a(new_n71_), .b(new_n44_), .c(new_n55_), .O(z06));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n73_), .b(x11), .c(x10), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(x09), .c(x01), .O(new_n75_));
  nand3  g32(.a(new_n75_), .b(x02), .c(x00), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(z07));
  nor2   g34(.a(x03), .b(new_n45_), .O(new_n78_));
  nor2   g35(.a(x06), .b(x05), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n80_), .c(x17), .O(new_n81_));
  nor3   g38(.a(new_n81_), .b(x08), .c(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n79_), .c(new_n78_), .d(x04), .O(new_n83_));
  aoi21  g40(.a(new_n83_), .b(new_n44_), .c(x00), .O(z08));
  inv1   g41(.a(x16), .O(new_n85_));
  inv1   g42(.a(x21), .O(new_n86_));
  inv1   g43(.a(x13), .O(new_n87_));
  inv1   g44(.a(x14), .O(new_n88_));
  nand4  g45(.a(new_n65_), .b(x02), .c(new_n44_), .d(new_n55_), .O(new_n89_));
  inv1   g46(.a(new_n89_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n88_), .c(new_n87_), .d(x12), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(x15), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n86_), .c(x20), .d(new_n85_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x22), .O(z09));
  inv1   g51(.a(x15), .O(new_n95_));
  nor2   g52(.a(new_n91_), .b(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n86_), .c(x20), .d(x16), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(x22), .O(z10));
  nand3  g55(.a(new_n88_), .b(new_n87_), .c(x12), .O(new_n99_));
  nor3   g56(.a(new_n99_), .b(x11), .c(new_n45_), .O(new_n100_));
  nor2   g57(.a(x16), .b(new_n95_), .O(new_n101_));
  nor2   g58(.a(x22), .b(x21), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n101_), .c(new_n100_), .d(x20), .O(new_n103_));
  aoi21  g60(.a(new_n103_), .b(new_n44_), .c(x00), .O(z11));
  inv1   g61(.a(x24), .O(new_n105_));
  oai21  g62(.a(new_n46_), .b(new_n45_), .c(x01), .O(new_n106_));
  nand2  g63(.a(x23), .b(x19), .O(new_n107_));
  oai21  g64(.a(x19), .b(new_n56_), .c(new_n107_), .O(new_n108_));
  and2   g65(.a(x23), .b(x02), .O(new_n109_));
  aoi21  g66(.a(new_n108_), .b(new_n45_), .c(new_n109_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n106_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n105_), .c(x09), .O(new_n112_));
  nor2   g69(.a(new_n44_), .b(x00), .O(z16));
  inv1   g70(.a(z16), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(z12));
  nand3  g72(.a(new_n57_), .b(x17), .c(new_n45_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g74(.a(new_n48_), .b(new_n46_), .c(new_n64_), .d(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n44_), .c(x00), .O(z14));
  aoi21  g76(.a(new_n46_), .b(x01), .c(new_n45_), .O(new_n120_));
  nand3  g77(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n121_));
  oai21  g78(.a(new_n120_), .b(new_n55_), .c(new_n121_), .O(z15));
  aoi21  g79(.a(new_n45_), .b(new_n44_), .c(x00), .O(z17));
endmodule


