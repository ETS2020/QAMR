// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n66_, new_n68_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n125_, new_n126_, new_n127_, new_n129_, new_n130_,
    new_n133_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand4  g09(.a(new_n47_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g11(.a(x01), .O(new_n55_));
  nand3  g12(.a(new_n45_), .b(new_n55_), .c(new_n44_), .O(new_n56_));
  inv1   g13(.a(x17), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x18), .c(new_n57_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand4  g17(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n61_));
  oai22  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n56_), .O(z03));
  nor2   g19(.a(x12), .b(x11), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x10), .c(new_n46_), .d(x02), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(x01), .c(new_n44_), .O(z04));
  nand3  g22(.a(x10), .b(x09), .c(x02), .O(new_n66_));
  aoi21  g23(.a(new_n66_), .b(x01), .c(new_n44_), .O(z05));
  nand4  g24(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n68_));
  aoi21  g25(.a(new_n68_), .b(x01), .c(new_n44_), .O(z06));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(new_n70_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(x11), .c(x10), .d(new_n46_), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(x12), .O(z07));
  inv1   g30(.a(x08), .O(new_n74_));
  inv1   g31(.a(x18), .O(new_n75_));
  inv1   g32(.a(x05), .O(new_n76_));
  inv1   g33(.a(x06), .O(new_n77_));
  inv1   g34(.a(x03), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x02), .c(new_n55_), .d(new_n44_), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n77_), .c(new_n76_), .d(x04), .O(new_n81_));
  nor2   g38(.a(new_n81_), .b(x07), .O(new_n82_));
  nand4  g39(.a(new_n82_), .b(new_n75_), .c(x17), .d(new_n74_), .O(new_n83_));
  nor2   g40(.a(new_n83_), .b(new_n58_), .O(z08));
  inv1   g41(.a(x11), .O(new_n85_));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x12), .c(new_n85_), .d(x02), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x22), .b(x21), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  oai21  g48(.a(new_n91_), .b(new_n87_), .c(new_n44_), .O(new_n92_));
  nand2  g49(.a(new_n92_), .b(new_n55_), .O(new_n93_));
  nor3   g50(.a(new_n75_), .b(new_n55_), .c(x00), .O(new_n94_));
  nor2   g51(.a(x20), .b(x19), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(new_n90_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n93_), .O(z09));
  nand4  g54(.a(new_n90_), .b(x20), .c(x16), .d(x15), .O(new_n98_));
  oai21  g55(.a(new_n98_), .b(new_n87_), .c(new_n44_), .O(new_n99_));
  nand2  g56(.a(new_n99_), .b(new_n55_), .O(new_n100_));
  nand4  g57(.a(new_n95_), .b(new_n94_), .c(x22), .d(x21), .O(new_n101_));
  nand2  g58(.a(new_n101_), .b(new_n100_), .O(z10));
  inv1   g59(.a(x22), .O(new_n103_));
  inv1   g60(.a(x21), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(x20), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n58_), .c(x18), .d(x01), .O(new_n106_));
  inv1   g63(.a(x13), .O(new_n107_));
  nor2   g64(.a(new_n45_), .b(x01), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n107_), .c(x12), .d(new_n85_), .O(new_n109_));
  nor2   g66(.a(new_n88_), .b(x14), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n104_), .c(x20), .d(new_n89_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n109_), .c(new_n106_), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(new_n103_), .c(new_n44_), .O(new_n113_));
  inv1   g70(.a(new_n113_), .O(z11));
  nand2  g71(.a(x10), .b(x02), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(x01), .c(x00), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n58_), .b(x17), .c(new_n45_), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n55_), .c(new_n44_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n116_), .c(x24), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x09), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z12));
  aoi21  g80(.a(new_n118_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g81(.a(new_n46_), .b(new_n45_), .c(new_n55_), .d(new_n44_), .O(new_n125_));
  inv1   g82(.a(new_n125_), .O(new_n126_));
  nand4  g83(.a(new_n126_), .b(new_n75_), .c(new_n57_), .d(new_n52_), .O(new_n127_));
  nor2   g84(.a(new_n127_), .b(x19), .O(z14));
  aoi21  g85(.a(x19), .b(new_n45_), .c(x00), .O(new_n129_));
  nand2  g86(.a(new_n115_), .b(x00), .O(new_n130_));
  oai21  g87(.a(new_n129_), .b(x01), .c(new_n130_), .O(z15));
  xor2a  g88(.a(x01), .b(x00), .O(z16));
  nand3  g89(.a(x02), .b(new_n55_), .c(new_n44_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


