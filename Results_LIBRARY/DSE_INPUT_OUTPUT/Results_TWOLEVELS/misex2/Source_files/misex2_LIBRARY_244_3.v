// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n136_, new_n137_,
    new_n138_, new_n141_;
  nand2  g00(.a(x12), .b(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  nor2   g06(.a(x19), .b(x18), .O(new_n50_));
  nand3  g07(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  oai21  g08(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g09(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n51_), .c(new_n44_), .O(z01));
  inv1   g11(.a(x10), .O(new_n55_));
  inv1   g12(.a(x18), .O(new_n56_));
  nand3  g13(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n56_), .c(new_n49_), .d(new_n55_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  inv1   g17(.a(x00), .O(new_n61_));
  inv1   g18(.a(x01), .O(new_n62_));
  nand3  g19(.a(new_n45_), .b(new_n62_), .c(new_n61_), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nand3  g21(.a(new_n64_), .b(x18), .c(new_n49_), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n63_), .c(new_n44_), .O(z03));
  inv1   g23(.a(x12), .O(new_n67_));
  inv1   g24(.a(x11), .O(new_n68_));
  nor2   g25(.a(new_n45_), .b(new_n61_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n68_), .c(x10), .d(new_n46_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n67_), .c(new_n62_), .O(z04));
  nand4  g28(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(x12), .c(new_n55_), .O(z05));
  nand3  g30(.a(new_n67_), .b(x11), .c(x10), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(new_n46_), .c(x02), .d(x00), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(new_n67_), .c(new_n62_), .O(z06));
  oai21  g34(.a(new_n74_), .b(x09), .c(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x02), .c(new_n62_), .d(new_n61_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n83_), .c(new_n82_), .d(x04), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n56_), .c(x17), .d(new_n81_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n64_), .O(z08));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nor2   g49(.a(new_n45_), .b(x01), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(x12), .c(new_n68_), .O(new_n94_));
  inv1   g51(.a(x15), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nor2   g53(.a(x14), .b(x13), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(x20), .c(new_n96_), .d(new_n95_), .O(new_n98_));
  nor2   g55(.a(x20), .b(x19), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x18), .c(new_n67_), .d(x01), .O(new_n100_));
  oai21  g57(.a(new_n98_), .b(new_n94_), .c(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n92_), .c(new_n91_), .d(new_n61_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z09));
  nand3  g60(.a(new_n64_), .b(x18), .c(x01), .O(new_n104_));
  nand2  g61(.a(x22), .b(x21), .O(new_n105_));
  nor3   g62(.a(new_n105_), .b(new_n104_), .c(x20), .O(new_n106_));
  nand2  g63(.a(new_n97_), .b(x12), .O(new_n107_));
  nor3   g64(.a(new_n107_), .b(x11), .c(new_n45_), .O(new_n108_));
  nand3  g65(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n109_));
  nor3   g66(.a(new_n109_), .b(new_n96_), .c(new_n95_), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n108_), .c(new_n106_), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(x00), .c(new_n44_), .O(z10));
  nor2   g69(.a(new_n56_), .b(x12), .O(new_n113_));
  nor2   g70(.a(new_n91_), .b(x20), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n113_), .c(new_n64_), .d(x01), .O(new_n115_));
  nor2   g72(.a(x13), .b(new_n67_), .O(new_n116_));
  nand3  g73(.a(new_n91_), .b(x20), .c(new_n96_), .O(new_n117_));
  nor3   g74(.a(new_n117_), .b(new_n95_), .c(x14), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n116_), .c(new_n93_), .d(new_n68_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n115_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n92_), .c(new_n61_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z11));
  nand2  g79(.a(x10), .b(x02), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(x01), .c(x00), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand3  g82(.a(new_n64_), .b(x17), .c(new_n45_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n62_), .c(new_n61_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n124_), .c(x24), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x09), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n44_), .O(z12));
  inv1   g88(.a(new_n47_), .O(new_n132_));
  oai21  g89(.a(new_n126_), .b(new_n132_), .c(new_n44_), .O(z13));
  nand3  g90(.a(new_n50_), .b(new_n49_), .c(new_n55_), .O(new_n134_));
  oai21  g91(.a(new_n134_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g92(.a(x10), .b(new_n62_), .c(x02), .O(new_n136_));
  nand2  g93(.a(new_n136_), .b(x00), .O(new_n137_));
  nand3  g94(.a(x19), .b(new_n45_), .c(new_n62_), .O(new_n138_));
  nand3  g95(.a(new_n138_), .b(new_n137_), .c(new_n44_), .O(z15));
  aoi21  g96(.a(new_n67_), .b(x00), .c(new_n62_), .O(z16));
  nand2  g97(.a(new_n93_), .b(new_n61_), .O(new_n141_));
  nand2  g98(.a(new_n141_), .b(new_n44_), .O(z17));
endmodule


