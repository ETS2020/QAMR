// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_,
    new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n119_, new_n121_, new_n122_, new_n123_,
    new_n125_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  nand4  g15(.a(new_n47_), .b(x18), .c(new_n49_), .d(new_n45_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z03));
  inv1   g17(.a(new_n44_), .O(z06));
  inv1   g18(.a(x00), .O(new_n64_));
  inv1   g19(.a(x01), .O(new_n65_));
  aoi21  g20(.a(new_n45_), .b(new_n65_), .c(new_n64_), .O(z07));
  nor2   g21(.a(new_n45_), .b(x01), .O(new_n67_));
  nand2  g22(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(new_n69_));
  inv1   g24(.a(x03), .O(new_n70_));
  inv1   g25(.a(x05), .O(new_n71_));
  nand3  g26(.a(new_n71_), .b(x04), .c(new_n70_), .O(new_n72_));
  inv1   g27(.a(new_n72_), .O(new_n73_));
  nor3   g28(.a(x08), .b(x07), .c(x06), .O(new_n74_));
  nand3  g29(.a(x19), .b(new_n50_), .c(x17), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  nand4  g31(.a(new_n76_), .b(new_n74_), .c(new_n73_), .d(new_n69_), .O(new_n77_));
  nand2  g32(.a(new_n77_), .b(new_n44_), .O(z08));
  inv1   g33(.a(x21), .O(new_n79_));
  inv1   g34(.a(x22), .O(new_n80_));
  inv1   g35(.a(x11), .O(new_n81_));
  nand4  g36(.a(x12), .b(new_n81_), .c(x02), .d(new_n65_), .O(new_n82_));
  inv1   g37(.a(x15), .O(new_n83_));
  inv1   g38(.a(x16), .O(new_n84_));
  nor2   g39(.a(x14), .b(x13), .O(new_n85_));
  nand4  g40(.a(new_n85_), .b(x20), .c(new_n84_), .d(new_n83_), .O(new_n86_));
  inv1   g41(.a(x20), .O(new_n87_));
  and2   g42(.a(x18), .b(x01), .O(new_n88_));
  nand3  g43(.a(new_n88_), .b(new_n87_), .c(new_n51_), .O(new_n89_));
  oai21  g44(.a(new_n86_), .b(new_n82_), .c(new_n89_), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n80_), .c(new_n79_), .d(new_n64_), .O(new_n91_));
  inv1   g46(.a(new_n91_), .O(z09));
  nand2  g47(.a(new_n51_), .b(x18), .O(new_n93_));
  nand3  g48(.a(x22), .b(x21), .c(new_n87_), .O(new_n94_));
  oai21  g49(.a(new_n94_), .b(new_n93_), .c(new_n64_), .O(new_n95_));
  nand2  g50(.a(new_n95_), .b(x01), .O(new_n96_));
  inv1   g51(.a(x12), .O(new_n97_));
  nor2   g52(.a(x13), .b(new_n97_), .O(new_n98_));
  nand2  g53(.a(new_n98_), .b(new_n81_), .O(new_n99_));
  inv1   g54(.a(new_n99_), .O(new_n100_));
  nor3   g55(.a(new_n84_), .b(new_n83_), .c(x14), .O(new_n101_));
  nand3  g56(.a(new_n80_), .b(new_n79_), .c(x20), .O(new_n102_));
  inv1   g57(.a(new_n102_), .O(new_n103_));
  nand4  g58(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n69_), .O(new_n104_));
  nand2  g59(.a(new_n104_), .b(new_n96_), .O(z10));
  nand4  g60(.a(new_n88_), .b(x21), .c(new_n87_), .d(new_n51_), .O(new_n106_));
  nand3  g61(.a(new_n98_), .b(new_n67_), .c(new_n81_), .O(new_n107_));
  nor2   g62(.a(new_n83_), .b(x14), .O(new_n108_));
  nor2   g63(.a(x21), .b(new_n87_), .O(new_n109_));
  nand3  g64(.a(new_n109_), .b(new_n108_), .c(new_n84_), .O(new_n110_));
  oai21  g65(.a(new_n110_), .b(new_n107_), .c(new_n106_), .O(new_n111_));
  nand3  g66(.a(new_n111_), .b(new_n80_), .c(new_n64_), .O(new_n112_));
  inv1   g67(.a(new_n112_), .O(z11));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g69(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n115_));
  aoi21  g70(.a(new_n115_), .b(new_n114_), .c(x24), .O(new_n116_));
  nand3  g71(.a(new_n116_), .b(x09), .c(new_n65_), .O(new_n117_));
  nor2   g72(.a(new_n117_), .b(x00), .O(z12));
  inv1   g73(.a(new_n47_), .O(new_n119_));
  oai21  g74(.a(new_n115_), .b(new_n119_), .c(new_n44_), .O(z13));
  nand3  g75(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n121_));
  inv1   g76(.a(new_n121_), .O(new_n122_));
  nand4  g77(.a(new_n122_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n123_));
  nor2   g78(.a(new_n123_), .b(x19), .O(z14));
  nand3  g79(.a(x19), .b(new_n45_), .c(new_n65_), .O(new_n125_));
  oai21  g80(.a(new_n67_), .b(new_n64_), .c(new_n125_), .O(z15));
  nor2   g81(.a(new_n65_), .b(x00), .O(z16));
  nand2  g82(.a(new_n68_), .b(new_n44_), .O(z17));
  zero   g83(.O(z04));
  zero   g84(.O(z05));
endmodule


