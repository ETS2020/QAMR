// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n104_, new_n105_, new_n106_, new_n108_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n121_, new_n123_, new_n124_, new_n126_;
  nor3   g00(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g04(.a(new_n47_), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g06(.a(new_n49_), .b(x11), .c(x01), .O(z00));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x02), .O(new_n52_));
  nand4  g09(.a(new_n48_), .b(x09), .c(new_n52_), .d(new_n51_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(x11), .c(x01), .O(z01));
  inv1   g11(.a(x18), .O(new_n55_));
  inv1   g12(.a(x01), .O(new_n56_));
  nand4  g13(.a(x09), .b(new_n52_), .c(new_n56_), .d(new_n51_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x10), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .d(x11), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand2  g17(.a(new_n52_), .b(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n45_), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n61_), .c(x11), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n56_), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(x09), .O(new_n67_));
  nand4  g24(.a(x12), .b(x11), .c(x10), .d(new_n67_), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n70_), .c(x10), .d(new_n67_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x12), .O(z04));
  inv1   g31(.a(x10), .O(new_n75_));
  nor3   g32(.a(new_n71_), .b(new_n75_), .c(new_n67_), .O(z05));
  nor3   g33(.a(new_n70_), .b(new_n75_), .c(x09), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x02), .c(x01), .d(x00), .O(new_n78_));
  nand2  g35(.a(new_n70_), .b(new_n56_), .O(new_n79_));
  nand2  g36(.a(new_n79_), .b(new_n78_), .O(z06));
  inv1   g37(.a(x12), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(x10), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(x11), .c(x02), .d(x00), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z07));
  inv1   g42(.a(x05), .O(new_n86_));
  inv1   g43(.a(x06), .O(new_n87_));
  inv1   g44(.a(x07), .O(new_n88_));
  inv1   g45(.a(x04), .O(new_n89_));
  inv1   g46(.a(x03), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x02), .c(new_n56_), .d(new_n51_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n89_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(new_n86_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x08), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n55_), .c(x17), .d(x11), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n62_), .O(z08));
  nand3  g53(.a(x18), .b(x01), .c(new_n51_), .O(new_n97_));
  inv1   g54(.a(x20), .O(new_n98_));
  inv1   g55(.a(x21), .O(new_n99_));
  inv1   g56(.a(x22), .O(new_n100_));
  nand4  g57(.a(new_n100_), .b(new_n99_), .c(new_n98_), .d(new_n62_), .O(new_n101_));
  oai21  g58(.a(new_n101_), .b(new_n97_), .c(new_n79_), .O(z09));
  nor2   g59(.a(new_n56_), .b(x00), .O(z16));
  nand3  g60(.a(z16), .b(new_n62_), .c(x18), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(x22), .c(x21), .d(new_n98_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(z10));
  nand4  g64(.a(new_n100_), .b(x21), .c(new_n98_), .d(new_n62_), .O(new_n108_));
  oai21  g65(.a(new_n108_), .b(new_n97_), .c(new_n79_), .O(z11));
  aoi21  g66(.a(x10), .b(x02), .c(new_n56_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(x00), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g69(.a(new_n62_), .b(x17), .c(new_n52_), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n112_), .O(new_n114_));
  nand3  g71(.a(new_n114_), .b(new_n56_), .c(new_n51_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n111_), .c(x24), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(x09), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n79_), .O(z12));
  nand4  g75(.a(new_n62_), .b(x17), .c(new_n52_), .d(new_n51_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(x11), .c(x01), .O(z13));
  nand4  g77(.a(new_n46_), .b(new_n44_), .c(new_n45_), .d(new_n75_), .O(new_n121_));
  aoi21  g78(.a(new_n121_), .b(x11), .c(x01), .O(z14));
  aoi21  g79(.a(x11), .b(new_n52_), .c(new_n110_), .O(new_n123_));
  nand4  g80(.a(x19), .b(x11), .c(new_n52_), .d(new_n56_), .O(new_n124_));
  oai21  g81(.a(new_n123_), .b(new_n51_), .c(new_n124_), .O(z15));
  nand4  g82(.a(x11), .b(x02), .c(new_n56_), .d(new_n51_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z17));
endmodule


