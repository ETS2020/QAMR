// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n110_, new_n112_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n120_, new_n122_, new_n123_, new_n126_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  nor3   g10(.a(x19), .b(x18), .c(x17), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(x10), .c(x09), .d(new_n48_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n47_), .c(x00), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n57_), .c(x09), .d(new_n48_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n47_), .c(x00), .O(z02));
  inv1   g16(.a(x19), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x18), .c(new_n44_), .d(new_n48_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n46_), .O(new_n63_));
  nand2  g20(.a(new_n49_), .b(x02), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  inv1   g22(.a(x11), .O(new_n66_));
  inv1   g23(.a(x12), .O(new_n67_));
  nor2   g24(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n65_), .c(x10), .d(x01), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n63_), .O(z03));
  nand4  g27(.a(new_n65_), .b(new_n67_), .c(new_n66_), .d(x10), .O(new_n71_));
  aoi21  g28(.a(new_n71_), .b(x00), .c(new_n47_), .O(z04));
  nand4  g29(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n57_), .O(z05));
  nand4  g31(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x00), .c(new_n47_), .O(z06));
  nand3  g33(.a(new_n67_), .b(x11), .c(x10), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n64_), .c(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(x01), .O(new_n79_));
  nand3  g36(.a(x02), .b(new_n47_), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n79_), .O(z07));
  inv1   g38(.a(x08), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  inv1   g40(.a(x06), .O(new_n84_));
  inv1   g41(.a(x03), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n84_), .c(new_n83_), .d(x04), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(x07), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(new_n45_), .c(x17), .d(new_n82_), .O(new_n90_));
  nor2   g47(.a(new_n90_), .b(new_n60_), .O(z08));
  inv1   g48(.a(x15), .O(new_n92_));
  inv1   g49(.a(x16), .O(new_n93_));
  inv1   g50(.a(x13), .O(new_n94_));
  inv1   g51(.a(x14), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n94_), .c(x12), .O(new_n96_));
  nor3   g53(.a(new_n96_), .b(x11), .c(new_n48_), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x22), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n97_), .c(new_n93_), .d(new_n92_), .O(new_n102_));
  aoi21  g59(.a(new_n102_), .b(new_n47_), .c(x00), .O(z09));
  nand4  g60(.a(new_n66_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n95_), .c(new_n94_), .d(x12), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n92_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n98_), .c(x20), .d(x16), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(x22), .O(z10));
  nand4  g66(.a(new_n101_), .b(new_n97_), .c(new_n93_), .d(x15), .O(new_n110_));
  aoi21  g67(.a(new_n110_), .b(new_n47_), .c(x00), .O(z11));
  inv1   g68(.a(x24), .O(new_n112_));
  aoi21  g69(.a(x10), .b(x02), .c(new_n47_), .O(new_n113_));
  oai21  g70(.a(x19), .b(x02), .c(x23), .O(new_n114_));
  nand3  g71(.a(new_n60_), .b(x17), .c(new_n48_), .O(new_n115_));
  aoi21  g72(.a(new_n115_), .b(new_n114_), .c(x00), .O(new_n116_));
  oai21  g73(.a(new_n116_), .b(new_n113_), .c(new_n112_), .O(new_n117_));
  oai22  g74(.a(new_n117_), .b(new_n49_), .c(new_n47_), .d(x00), .O(z12));
  aoi21  g75(.a(new_n115_), .b(new_n47_), .c(x00), .O(z13));
  nand4  g76(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(x19), .O(z14));
  aoi21  g78(.a(new_n57_), .b(x01), .c(new_n48_), .O(new_n122_));
  nand3  g79(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n46_), .c(new_n123_), .O(z15));
  nor2   g81(.a(new_n47_), .b(x00), .O(z16));
  nand3  g82(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z17));
endmodule


