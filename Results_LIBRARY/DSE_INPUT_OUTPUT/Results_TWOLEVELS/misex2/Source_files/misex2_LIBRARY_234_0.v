// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n55_,
    new_n57_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_, new_n66_,
    new_n67_, new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n82_, new_n83_, new_n84_;
  inv1   g00(.a(x11), .O(new_n47_));
  inv1   g01(.a(x12), .O(new_n48_));
  inv1   g02(.a(x09), .O(new_n49_));
  nand3  g03(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  inv1   g04(.a(new_n50_), .O(new_n51_));
  nand3  g05(.a(new_n51_), .b(x10), .c(new_n49_), .O(new_n52_));
  nor3   g06(.a(new_n52_), .b(new_n48_), .c(new_n47_), .O(z03));
  nor3   g07(.a(new_n52_), .b(x12), .c(x11), .O(z04));
  inv1   g08(.a(x10), .O(new_n55_));
  nor3   g09(.a(new_n50_), .b(new_n55_), .c(new_n49_), .O(z05));
  inv1   g10(.a(x00), .O(new_n57_));
  inv1   g11(.a(x01), .O(new_n58_));
  nand4  g12(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n59_));
  nor3   g13(.a(new_n59_), .b(new_n58_), .c(new_n57_), .O(z06));
  oai21  g14(.a(x02), .b(new_n57_), .c(new_n58_), .O(new_n61_));
  nand3  g15(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nand4  g16(.a(new_n48_), .b(x11), .c(x10), .d(new_n49_), .O(new_n63_));
  oai21  g17(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(z07));
  nor2   g18(.a(x01), .b(x00), .O(z08));
  inv1   g19(.a(x19), .O(new_n66_));
  inv1   g20(.a(x20), .O(new_n67_));
  nor2   g21(.a(x22), .b(x21), .O(new_n68_));
  nand4  g22(.a(new_n68_), .b(new_n67_), .c(new_n66_), .d(x18), .O(new_n69_));
  aoi21  g23(.a(new_n69_), .b(x01), .c(x00), .O(z09));
  nand4  g24(.a(new_n66_), .b(x18), .c(x01), .d(new_n57_), .O(new_n71_));
  inv1   g25(.a(new_n71_), .O(new_n72_));
  nand4  g26(.a(new_n72_), .b(x22), .c(x21), .d(new_n67_), .O(new_n73_));
  inv1   g27(.a(new_n73_), .O(z10));
  inv1   g28(.a(x22), .O(new_n75_));
  nand4  g29(.a(new_n72_), .b(new_n75_), .c(x21), .d(new_n67_), .O(new_n76_));
  inv1   g30(.a(new_n76_), .O(z11));
  aoi21  g31(.a(x10), .b(x02), .c(x24), .O(new_n78_));
  nand3  g32(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nor2   g33(.a(new_n79_), .b(new_n57_), .O(z12));
  inv1   g34(.a(z08), .O(new_n82_));
  inv1   g35(.a(x02), .O(new_n83_));
  aoi21  g36(.a(new_n55_), .b(x01), .c(new_n83_), .O(new_n84_));
  oai21  g37(.a(new_n84_), .b(new_n57_), .c(new_n82_), .O(z15));
  nor2   g38(.a(new_n58_), .b(x00), .O(z16));
  zero   g39(.O(z00));
  zero   g40(.O(z01));
  zero   g41(.O(z02));
  zero   g42(.O(z14));
  zero   g43(.O(z17));
  nor2   g44(.a(x01), .b(x00), .O(z13));
endmodule


