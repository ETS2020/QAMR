// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n85_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x01), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(z00));
  nand3  g04(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  inv1   g05(.a(x09), .O(new_n51_));
  nand4  g06(.a(x12), .b(x11), .c(x10), .d(new_n51_), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(new_n50_), .c(new_n46_), .O(z03));
  nand3  g08(.a(x02), .b(x01), .c(x00), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(x10), .c(new_n51_), .O(new_n56_));
  nor3   g11(.a(new_n56_), .b(x12), .c(x11), .O(z04));
  inv1   g12(.a(x10), .O(new_n58_));
  nor3   g13(.a(new_n54_), .b(new_n58_), .c(new_n51_), .O(z05));
  nand2  g14(.a(x11), .b(x10), .O(new_n60_));
  nor2   g15(.a(new_n60_), .b(x09), .O(new_n61_));
  nand4  g16(.a(new_n61_), .b(x02), .c(x01), .d(x00), .O(new_n62_));
  nand2  g17(.a(new_n62_), .b(new_n46_), .O(z06));
  inv1   g18(.a(x12), .O(new_n64_));
  nand3  g19(.a(new_n64_), .b(x11), .c(x10), .O(new_n65_));
  oai21  g20(.a(new_n65_), .b(x09), .c(x01), .O(new_n66_));
  nand3  g21(.a(new_n66_), .b(x02), .c(x00), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z07));
  inv1   g23(.a(x19), .O(new_n69_));
  inv1   g24(.a(x20), .O(new_n70_));
  nor2   g25(.a(x22), .b(x21), .O(new_n71_));
  nand4  g26(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x18), .O(new_n72_));
  aoi21  g27(.a(new_n72_), .b(x01), .c(x00), .O(z09));
  nand4  g28(.a(new_n69_), .b(x18), .c(x01), .d(new_n44_), .O(new_n74_));
  inv1   g29(.a(new_n74_), .O(new_n75_));
  nand4  g30(.a(new_n75_), .b(x22), .c(x21), .d(new_n70_), .O(new_n76_));
  inv1   g31(.a(new_n76_), .O(z10));
  inv1   g32(.a(x22), .O(new_n78_));
  nand4  g33(.a(new_n75_), .b(new_n78_), .c(x21), .d(new_n70_), .O(new_n79_));
  inv1   g34(.a(new_n79_), .O(z11));
  aoi21  g35(.a(x10), .b(x02), .c(x24), .O(new_n81_));
  nand3  g36(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nor2   g37(.a(new_n82_), .b(new_n44_), .O(z12));
  nand2  g38(.a(new_n58_), .b(x01), .O(new_n85_));
  aoi21  g39(.a(new_n85_), .b(x02), .c(new_n44_), .O(z15));
  nor2   g40(.a(new_n45_), .b(x00), .O(z16));
  zero   g41(.O(z01));
  zero   g42(.O(z02));
  zero   g43(.O(z13));
  inv1   g44(.a(new_n46_), .O(z08));
  inv1   g45(.a(new_n46_), .O(z14));
  inv1   g46(.a(new_n46_), .O(z17));
endmodule


