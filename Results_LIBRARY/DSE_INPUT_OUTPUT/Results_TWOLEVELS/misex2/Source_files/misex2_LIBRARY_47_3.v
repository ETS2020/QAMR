// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n149_, new_n151_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor3   g003(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g004(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g007(.a(new_n50_), .b(x19), .O(z00));
  nand2  g008(.a(x17), .b(x08), .O(new_n52_));
  inv1   g009(.a(x00), .O(new_n53_));
  inv1   g010(.a(x01), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nand4  g012(.a(x09), .b(new_n55_), .c(new_n54_), .d(new_n53_), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  nand4  g014(.a(new_n57_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n52_), .O(z01));
  inv1   g016(.a(x10), .O(new_n60_));
  nand2  g017(.a(new_n47_), .b(x09), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x19), .O(z02));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n57_), .c(x18), .d(new_n44_), .O(new_n66_));
  nand3  g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g024(.a(new_n67_), .O(new_n68_));
  nor2   g025(.a(new_n60_), .b(x09), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n68_), .c(x12), .d(x11), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(new_n66_), .c(new_n52_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  nand3  g030(.a(new_n69_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n67_), .c(new_n52_), .O(z04));
  nand4  g032(.a(new_n52_), .b(x10), .c(x09), .d(x02), .O(new_n76_));
  nor3   g033(.a(new_n76_), .b(new_n54_), .c(new_n53_), .O(z05));
  nand4  g034(.a(new_n52_), .b(x11), .c(x10), .d(new_n46_), .O(new_n78_));
  inv1   g035(.a(new_n78_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(z06));
  nand3  g038(.a(new_n69_), .b(new_n73_), .c(x11), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n52_), .O(z07));
  inv1   g042(.a(x08), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  inv1   g044(.a(x06), .O(new_n88_));
  inv1   g045(.a(x03), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n90_));
  inv1   g047(.a(new_n90_), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n88_), .c(new_n87_), .d(x04), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x07), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n45_), .c(x17), .d(new_n86_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n57_), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  nor2   g054(.a(new_n55_), .b(x01), .O(new_n98_));
  nand3  g055(.a(new_n98_), .b(x12), .c(new_n72_), .O(new_n99_));
  inv1   g056(.a(x15), .O(new_n100_));
  inv1   g057(.a(x16), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n45_), .b(new_n54_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n57_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n52_), .c(new_n97_), .d(new_n96_), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(x00), .O(z09));
  nand3  g066(.a(new_n57_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n104_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  inv1   g069(.a(x13), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(x12), .c(new_n72_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand2  g072(.a(new_n115_), .b(new_n98_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand2  g074(.a(x16), .b(x15), .O(new_n118_));
  nand3  g075(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n119_));
  nor3   g076(.a(new_n119_), .b(new_n118_), .c(x14), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n117_), .c(new_n112_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(x00), .c(new_n52_), .O(z10));
  nand4  g079(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n57_), .O(new_n123_));
  nor2   g080(.a(new_n100_), .b(x14), .O(new_n124_));
  nand4  g081(.a(new_n124_), .b(new_n96_), .c(x20), .d(new_n101_), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(new_n116_), .c(new_n123_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n97_), .c(new_n53_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n52_), .O(z11));
  inv1   g085(.a(x24), .O(new_n129_));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand4  g087(.a(new_n130_), .b(new_n52_), .c(x01), .d(x00), .O(new_n131_));
  nor2   g088(.a(x19), .b(x02), .O(new_n132_));
  oai21  g089(.a(new_n132_), .b(x23), .c(x17), .O(new_n133_));
  nand2  g090(.a(new_n57_), .b(new_n55_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(x23), .c(new_n44_), .O(new_n135_));
  oai21  g092(.a(new_n133_), .b(x08), .c(new_n135_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n54_), .c(new_n53_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n131_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n129_), .c(x09), .O(new_n139_));
  inv1   g096(.a(new_n139_), .O(z12));
  nand3  g097(.a(new_n47_), .b(x17), .c(new_n86_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z13));
  nand4  g099(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n143_));
  nor2   g100(.a(new_n143_), .b(x19), .O(z14));
  oai21  g101(.a(x10), .b(new_n54_), .c(x02), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(x00), .O(new_n146_));
  nand3  g103(.a(x19), .b(new_n55_), .c(new_n54_), .O(new_n147_));
  nand3  g104(.a(new_n147_), .b(new_n146_), .c(new_n52_), .O(z15));
  nand3  g105(.a(new_n52_), .b(x01), .c(new_n53_), .O(new_n149_));
  inv1   g106(.a(new_n149_), .O(z16));
  nand4  g107(.a(new_n52_), .b(x02), .c(new_n54_), .d(new_n53_), .O(new_n151_));
  inv1   g108(.a(new_n151_), .O(z17));
endmodule


