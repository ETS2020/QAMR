// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:50 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n59_, new_n60_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n72_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n83_;
  nor2   g00(.a(x01), .b(x00), .O(z00));
  inv1   g01(.a(x12), .O(new_n46_));
  inv1   g02(.a(x09), .O(new_n47_));
  nand3  g03(.a(x02), .b(x01), .c(x00), .O(new_n48_));
  inv1   g04(.a(new_n48_), .O(new_n49_));
  nand4  g05(.a(new_n49_), .b(x11), .c(x10), .d(new_n47_), .O(new_n50_));
  nor2   g06(.a(new_n50_), .b(new_n46_), .O(z03));
  nand3  g07(.a(x02), .b(x01), .c(x00), .O(new_n52_));
  inv1   g08(.a(x11), .O(new_n53_));
  nand4  g09(.a(new_n46_), .b(new_n53_), .c(x10), .d(new_n47_), .O(new_n54_));
  oai22  g10(.a(new_n54_), .b(new_n52_), .c(x01), .d(x00), .O(z04));
  inv1   g11(.a(x10), .O(new_n56_));
  nor3   g12(.a(new_n48_), .b(new_n56_), .c(new_n47_), .O(z05));
  inv1   g13(.a(x00), .O(z16));
  inv1   g14(.a(x01), .O(new_n59_));
  nand4  g15(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n60_));
  nor3   g16(.a(new_n60_), .b(new_n59_), .c(z16), .O(z06));
  oai21  g17(.a(x02), .b(z16), .c(new_n59_), .O(new_n62_));
  nand4  g18(.a(new_n46_), .b(x11), .c(x10), .d(new_n47_), .O(new_n63_));
  oai21  g19(.a(new_n63_), .b(new_n52_), .c(new_n62_), .O(z07));
  inv1   g20(.a(x18), .O(new_n65_));
  nor2   g21(.a(x19), .b(new_n65_), .O(new_n66_));
  nor3   g22(.a(x22), .b(x21), .c(x20), .O(new_n67_));
  nand2  g23(.a(new_n67_), .b(new_n66_), .O(new_n68_));
  aoi21  g24(.a(new_n68_), .b(x01), .c(x00), .O(z09));
  inv1   g25(.a(x20), .O(new_n70_));
  and2   g26(.a(x22), .b(x21), .O(new_n71_));
  nand3  g27(.a(new_n71_), .b(new_n66_), .c(new_n70_), .O(new_n72_));
  aoi21  g28(.a(new_n72_), .b(x01), .c(x00), .O(z10));
  inv1   g29(.a(x21), .O(new_n74_));
  nor2   g30(.a(x22), .b(new_n74_), .O(new_n75_));
  nand3  g31(.a(new_n75_), .b(new_n66_), .c(new_n70_), .O(new_n76_));
  aoi21  g32(.a(new_n76_), .b(x01), .c(x00), .O(z11));
  aoi21  g33(.a(x10), .b(x02), .c(x24), .O(new_n78_));
  nand3  g34(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nor2   g35(.a(new_n79_), .b(z16), .O(z12));
  nand2  g36(.a(new_n56_), .b(x01), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(x02), .c(z16), .O(z15));
  zero   g38(.O(z01));
  zero   g39(.O(z13));
  zero   g40(.O(z14));
  zero   g41(.O(z17));
  nor2   g42(.a(x01), .b(x00), .O(z02));
  nor2   g43(.a(x01), .b(x00), .O(z08));
endmodule


