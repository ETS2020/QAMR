// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n136_, new_n138_, new_n139_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  nor2   g12(.a(x10), .b(x01), .O(z02));
  inv1   g13(.a(x10), .O(new_n57_));
  inv1   g14(.a(x11), .O(new_n58_));
  inv1   g15(.a(x12), .O(new_n59_));
  nor3   g16(.a(new_n59_), .b(new_n58_), .c(x09), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nor3   g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(new_n62_), .c(x18), .d(new_n44_), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(new_n61_), .c(new_n57_), .O(z03));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g23(.a(new_n66_), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n58_), .c(x10), .d(new_n46_), .O(new_n68_));
  nor2   g25(.a(new_n68_), .b(x12), .O(z04));
  inv1   g26(.a(z02), .O(new_n70_));
  nand2  g27(.a(x01), .b(x00), .O(new_n71_));
  nand3  g28(.a(x10), .b(x09), .c(x02), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n71_), .c(new_n70_), .O(z05));
  inv1   g30(.a(x00), .O(new_n74_));
  inv1   g31(.a(x01), .O(new_n75_));
  nand4  g32(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n75_), .c(new_n74_), .O(z06));
  nand3  g34(.a(new_n59_), .b(x11), .c(x10), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n70_), .O(z07));
  nand2  g38(.a(x02), .b(new_n74_), .O(new_n82_));
  inv1   g39(.a(x05), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x04), .O(new_n84_));
  nor3   g41(.a(new_n84_), .b(new_n82_), .c(x03), .O(new_n85_));
  nor3   g42(.a(x08), .b(x07), .c(x06), .O(new_n86_));
  nor2   g43(.a(new_n62_), .b(x18), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n86_), .c(new_n85_), .d(x17), .O(new_n88_));
  aoi21  g45(.a(new_n88_), .b(x10), .c(x01), .O(z08));
  inv1   g46(.a(x21), .O(new_n90_));
  inv1   g47(.a(x22), .O(new_n91_));
  inv1   g48(.a(x02), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x01), .O(new_n93_));
  nor2   g50(.a(new_n59_), .b(x11), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x10), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nor2   g57(.a(new_n45_), .b(new_n75_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n62_), .O(new_n102_));
  oai21  g59(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n91_), .c(new_n90_), .d(new_n74_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  nand3  g62(.a(new_n62_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g63(.a(x22), .b(x21), .c(new_n100_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(x02), .b(new_n75_), .O(new_n109_));
  inv1   g66(.a(x13), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x12), .c(new_n58_), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g69(.a(x14), .O(new_n113_));
  nand3  g70(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n91_), .b(new_n90_), .c(x20), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n112_), .c(new_n108_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(x00), .c(new_n70_), .O(z10));
  nand4  g75(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n62_), .O(new_n119_));
  nor2   g76(.a(new_n96_), .b(x14), .O(new_n120_));
  nor2   g77(.a(x21), .b(new_n100_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n120_), .c(new_n97_), .d(new_n110_), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(new_n95_), .c(new_n119_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n91_), .c(new_n74_), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z11));
  inv1   g82(.a(x24), .O(new_n126_));
  nand2  g83(.a(x10), .b(x02), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g85(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g86(.a(new_n62_), .b(x17), .c(new_n92_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand4  g88(.a(new_n131_), .b(x10), .c(new_n75_), .d(new_n74_), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g90(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z12));
  nand3  g92(.a(new_n47_), .b(x17), .c(x10), .O(new_n136_));
  nor2   g93(.a(new_n136_), .b(x19), .O(z13));
  nand2  g94(.a(new_n127_), .b(x00), .O(new_n138_));
  aoi21  g95(.a(x19), .b(new_n92_), .c(new_n57_), .O(new_n139_));
  oai21  g96(.a(new_n139_), .b(x01), .c(new_n138_), .O(z15));
  oai21  g97(.a(new_n75_), .b(x00), .c(new_n70_), .O(z16));
  aoi21  g98(.a(new_n82_), .b(x10), .c(x01), .O(z17));
  nor2   g99(.a(x10), .b(x01), .O(z14));
endmodule


