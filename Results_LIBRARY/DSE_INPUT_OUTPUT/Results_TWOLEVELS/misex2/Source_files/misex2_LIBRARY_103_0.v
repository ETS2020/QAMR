// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:12 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n106_, new_n107_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n118_, new_n119_, new_n120_, new_n121_, new_n123_, new_n124_,
    new_n127_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x13), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x10), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(x09), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x13), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  nor2   g14(.a(new_n54_), .b(x10), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n49_), .c(new_n44_), .d(x13), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand2  g17(.a(new_n53_), .b(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n44_), .O(new_n63_));
  oai21  g20(.a(new_n63_), .b(new_n61_), .c(x13), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n52_), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nor2   g23(.a(new_n50_), .b(x09), .O(new_n67_));
  nand3  g24(.a(new_n67_), .b(x12), .c(x11), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n66_), .c(new_n65_), .O(z03));
  inv1   g26(.a(x13), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n52_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n67_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n66_), .c(new_n71_), .O(z04));
  nand2  g32(.a(x01), .b(x00), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z05));
  nor3   g35(.a(new_n72_), .b(new_n50_), .c(x09), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n71_), .O(z06));
  nand3  g38(.a(new_n73_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n71_), .O(z07));
  nand2  g42(.a(x02), .b(new_n51_), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  nand2  g44(.a(new_n87_), .b(x04), .O(new_n88_));
  nor3   g45(.a(new_n88_), .b(new_n86_), .c(x03), .O(new_n89_));
  nor3   g46(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand3  g47(.a(x19), .b(new_n49_), .c(x17), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  aoi21  g50(.a(new_n93_), .b(x13), .c(x01), .O(z08));
  inv1   g51(.a(x22), .O(new_n95_));
  nand4  g52(.a(new_n62_), .b(x18), .c(x01), .d(new_n51_), .O(new_n96_));
  nor3   g53(.a(new_n96_), .b(x21), .c(x20), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(z09));
  nand3  g56(.a(x18), .b(x01), .c(new_n51_), .O(new_n100_));
  inv1   g57(.a(x20), .O(new_n101_));
  nand4  g58(.a(x22), .b(x21), .c(new_n101_), .d(new_n62_), .O(new_n102_));
  oai21  g59(.a(new_n102_), .b(new_n100_), .c(new_n71_), .O(z10));
  nand4  g60(.a(new_n95_), .b(x21), .c(new_n101_), .d(new_n62_), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n100_), .c(new_n71_), .O(z11));
  inv1   g62(.a(x24), .O(new_n106_));
  nand2  g63(.a(x10), .b(x02), .O(new_n107_));
  nand3  g64(.a(new_n107_), .b(x01), .c(x00), .O(new_n108_));
  oai21  g65(.a(x19), .b(x02), .c(x23), .O(new_n109_));
  nand3  g66(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n52_), .c(new_n51_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n108_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n106_), .c(x09), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n71_), .O(z12));
  nand4  g72(.a(new_n62_), .b(x17), .c(new_n53_), .d(new_n51_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(x13), .c(x01), .O(z13));
  inv1   g74(.a(x09), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(x10), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n49_), .c(new_n44_), .d(x13), .O(new_n121_));
  nor2   g78(.a(new_n121_), .b(x19), .O(z14));
  aoi21  g79(.a(new_n50_), .b(x01), .c(new_n53_), .O(new_n123_));
  aoi21  g80(.a(x19), .b(new_n53_), .c(new_n70_), .O(new_n124_));
  oai22  g81(.a(new_n124_), .b(x01), .c(new_n123_), .d(new_n51_), .O(z15));
  oai21  g82(.a(new_n52_), .b(x00), .c(new_n71_), .O(z16));
  nand4  g83(.a(x13), .b(x02), .c(new_n52_), .d(new_n51_), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z17));
endmodule


