// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:33 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n140_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n148_, new_n150_;
  nand2  g000(.a(x19), .b(x08), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g010(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n52_), .c(new_n44_), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  nand4  g013(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n57_));
  oai21  g014(.a(new_n57_), .b(new_n54_), .c(new_n44_), .O(z02));
  nand4  g015(.a(new_n44_), .b(x12), .c(x11), .d(x10), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x09), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(x02), .c(x01), .d(x00), .O(new_n61_));
  nor3   g018(.a(x02), .b(x01), .c(x00), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(new_n61_), .O(z03));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(x11), .O(new_n66_));
  inv1   g023(.a(x12), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n66_), .c(x10), .d(new_n46_), .O(new_n68_));
  oai21  g025(.a(new_n68_), .b(new_n65_), .c(new_n44_), .O(z04));
  nand2  g026(.a(x01), .b(x00), .O(new_n70_));
  nand3  g027(.a(x10), .b(x09), .c(x02), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n70_), .c(new_n44_), .O(z05));
  nand4  g029(.a(new_n44_), .b(x11), .c(x10), .d(new_n46_), .O(new_n73_));
  inv1   g030(.a(new_n73_), .O(new_n74_));
  nand4  g031(.a(new_n74_), .b(x02), .c(x01), .d(x00), .O(new_n75_));
  inv1   g032(.a(new_n75_), .O(z06));
  nand3  g033(.a(new_n67_), .b(x11), .c(x10), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(x09), .c(x01), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(new_n44_), .c(x02), .d(x00), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z07));
  inv1   g037(.a(x08), .O(new_n81_));
  inv1   g038(.a(x07), .O(new_n82_));
  inv1   g039(.a(new_n47_), .O(new_n83_));
  inv1   g040(.a(x03), .O(new_n84_));
  nand3  g041(.a(x04), .b(new_n84_), .c(x02), .O(new_n85_));
  nor2   g042(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nor2   g043(.a(x06), .b(x05), .O(new_n87_));
  nor2   g044(.a(x18), .b(new_n49_), .O(new_n88_));
  nand4  g045(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(new_n82_), .O(new_n89_));
  aoi21  g046(.a(new_n89_), .b(new_n81_), .c(new_n51_), .O(z08));
  inv1   g047(.a(x00), .O(new_n91_));
  inv1   g048(.a(x21), .O(new_n92_));
  inv1   g049(.a(x22), .O(new_n93_));
  inv1   g050(.a(x01), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n66_), .c(x02), .d(new_n94_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  and2   g057(.a(x18), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(new_n51_), .O(new_n102_));
  oai21  g059(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(new_n93_), .c(new_n92_), .d(new_n91_), .O(new_n104_));
  nand2  g061(.a(new_n104_), .b(new_n44_), .O(z09));
  nand3  g062(.a(new_n51_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g063(.a(x22), .b(x21), .c(new_n100_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n94_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n66_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nand3  g070(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g071(.a(new_n93_), .b(new_n92_), .c(x20), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n112_), .c(new_n108_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n44_), .O(z10));
  nor2   g075(.a(new_n45_), .b(x01), .O(new_n119_));
  nor2   g076(.a(x13), .b(new_n67_), .O(new_n120_));
  nand3  g077(.a(new_n120_), .b(new_n119_), .c(new_n66_), .O(new_n121_));
  nand4  g078(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n51_), .O(new_n122_));
  nor2   g079(.a(new_n96_), .b(x14), .O(new_n123_));
  nor2   g080(.a(x21), .b(new_n100_), .O(new_n124_));
  nand3  g081(.a(new_n124_), .b(new_n123_), .c(new_n97_), .O(new_n125_));
  oai21  g082(.a(new_n125_), .b(new_n121_), .c(new_n122_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n93_), .c(new_n91_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n44_), .O(z11));
  inv1   g085(.a(x24), .O(new_n129_));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n94_), .c(new_n91_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n131_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n129_), .c(x09), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n44_), .O(z12));
  nor4   g095(.a(new_n83_), .b(x19), .c(new_n49_), .d(x02), .O(z13));
  nand3  g096(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n140_));
  inv1   g097(.a(new_n140_), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n50_), .c(new_n49_), .d(new_n56_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x19), .O(z14));
  oai21  g100(.a(x10), .b(new_n94_), .c(x02), .O(new_n144_));
  nand3  g101(.a(new_n144_), .b(new_n44_), .c(x00), .O(new_n145_));
  nand4  g102(.a(x19), .b(new_n81_), .c(new_n45_), .d(new_n94_), .O(new_n146_));
  nand2  g103(.a(new_n146_), .b(new_n145_), .O(z15));
  nand3  g104(.a(new_n44_), .b(x01), .c(new_n91_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z16));
  nand4  g106(.a(new_n44_), .b(x02), .c(new_n94_), .d(new_n91_), .O(new_n150_));
  inv1   g107(.a(new_n150_), .O(z17));
endmodule


