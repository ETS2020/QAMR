// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:30 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n128_, new_n130_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n138_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x20), .O(new_n53_));
  nand2  g10(.a(new_n53_), .b(x01), .O(new_n54_));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n54_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand3  g16(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z02));
  nand3  g20(.a(new_n47_), .b(x02), .c(x00), .O(new_n64_));
  nand3  g21(.a(x12), .b(x11), .c(x10), .O(new_n65_));
  oai21  g22(.a(new_n65_), .b(new_n64_), .c(x20), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(x01), .O(new_n67_));
  nor3   g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n56_), .c(x18), .d(new_n44_), .O(new_n69_));
  nand2  g26(.a(new_n69_), .b(new_n67_), .O(z03));
  inv1   g27(.a(x01), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  inv1   g30(.a(new_n64_), .O(new_n74_));
  nand4  g31(.a(new_n74_), .b(new_n73_), .c(new_n72_), .d(x10), .O(new_n75_));
  aoi21  g32(.a(new_n75_), .b(x20), .c(new_n71_), .O(z04));
  nand4  g33(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x20), .c(new_n71_), .O(z05));
  oai21  g35(.a(x20), .b(x12), .c(x11), .O(new_n79_));
  nor2   g36(.a(new_n79_), .b(new_n59_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n47_), .c(x02), .d(x00), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(x20), .c(new_n71_), .O(z06));
  nand2  g39(.a(x10), .b(new_n47_), .O(new_n83_));
  nand3  g40(.a(x20), .b(new_n73_), .c(x11), .O(new_n84_));
  oai21  g41(.a(new_n84_), .b(new_n83_), .c(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  inv1   g43(.a(new_n86_), .O(z07));
  inv1   g44(.a(x08), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  inv1   g46(.a(x06), .O(new_n90_));
  inv1   g47(.a(x00), .O(new_n91_));
  inv1   g48(.a(x03), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(x02), .c(new_n71_), .d(new_n91_), .O(new_n93_));
  inv1   g50(.a(new_n93_), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n90_), .c(new_n89_), .d(x04), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(x07), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n45_), .c(x17), .d(new_n88_), .O(new_n97_));
  nor2   g54(.a(new_n97_), .b(new_n56_), .O(z08));
  inv1   g55(.a(x16), .O(new_n99_));
  inv1   g56(.a(x21), .O(new_n100_));
  inv1   g57(.a(x13), .O(new_n101_));
  inv1   g58(.a(x14), .O(new_n102_));
  nand4  g59(.a(new_n72_), .b(x02), .c(new_n71_), .d(new_n91_), .O(new_n103_));
  inv1   g60(.a(new_n103_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(x12), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(x15), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n100_), .c(x20), .d(new_n99_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(x22), .O(z09));
  nand3  g65(.a(x02), .b(new_n71_), .c(new_n91_), .O(new_n109_));
  nor4   g66(.a(new_n109_), .b(x13), .c(new_n73_), .d(x11), .O(new_n110_));
  inv1   g67(.a(x15), .O(new_n111_));
  nor3   g68(.a(new_n99_), .b(new_n111_), .c(x14), .O(new_n112_));
  nor2   g69(.a(x22), .b(x21), .O(new_n113_));
  nand4  g70(.a(new_n113_), .b(new_n112_), .c(new_n110_), .d(x20), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n54_), .O(z10));
  nor2   g72(.a(new_n105_), .b(new_n111_), .O(new_n116_));
  nand4  g73(.a(new_n116_), .b(new_n100_), .c(x20), .d(new_n99_), .O(new_n117_));
  nor2   g74(.a(new_n117_), .b(x22), .O(z11));
  nand2  g75(.a(x10), .b(x02), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(x01), .c(x00), .O(new_n120_));
  oai21  g77(.a(x19), .b(x02), .c(x23), .O(new_n121_));
  nand3  g78(.a(new_n56_), .b(x17), .c(new_n46_), .O(new_n122_));
  nand2  g79(.a(new_n122_), .b(new_n121_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n71_), .c(new_n91_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n120_), .c(x24), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(x09), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n54_), .O(z12));
  inv1   g84(.a(new_n48_), .O(new_n128_));
  oai21  g85(.a(new_n122_), .b(new_n128_), .c(new_n54_), .O(z13));
  nand4  g86(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n130_));
  nor2   g87(.a(new_n130_), .b(x19), .O(z14));
  oai21  g88(.a(x10), .b(new_n71_), .c(x02), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(x00), .O(new_n133_));
  nand3  g90(.a(x19), .b(new_n46_), .c(new_n71_), .O(new_n134_));
  nand3  g91(.a(new_n134_), .b(new_n133_), .c(new_n54_), .O(z15));
  nand3  g92(.a(x20), .b(x01), .c(new_n91_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z16));
  nand2  g94(.a(new_n48_), .b(x02), .O(new_n138_));
  inv1   g95(.a(new_n138_), .O(z17));
endmodule


