// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n102_,
    new_n103_, new_n104_, new_n105_, new_n106_, new_n108_, new_n110_,
    new_n111_, new_n112_, new_n113_, new_n114_, new_n115_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n126_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x00), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x00), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  nand4  g09(.a(new_n47_), .b(new_n52_), .c(x09), .d(new_n45_), .O(new_n53_));
  aoi21  g10(.a(new_n53_), .b(new_n44_), .c(x00), .O(z02));
  inv1   g11(.a(x00), .O(new_n55_));
  nand3  g12(.a(new_n45_), .b(new_n44_), .c(new_n55_), .O(new_n56_));
  inv1   g13(.a(x17), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand3  g15(.a(new_n58_), .b(x18), .c(new_n57_), .O(new_n59_));
  nand3  g16(.a(x02), .b(x01), .c(x00), .O(new_n60_));
  nand4  g17(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n61_));
  oai22  g18(.a(new_n61_), .b(new_n60_), .c(new_n59_), .d(new_n56_), .O(z03));
  inv1   g19(.a(x11), .O(new_n63_));
  inv1   g20(.a(x12), .O(new_n64_));
  nand2  g21(.a(new_n46_), .b(x02), .O(new_n65_));
  inv1   g22(.a(new_n65_), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n64_), .c(new_n63_), .d(x10), .O(new_n67_));
  aoi21  g24(.a(new_n67_), .b(x00), .c(new_n44_), .O(z04));
  nand3  g25(.a(x10), .b(x09), .c(x02), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(x00), .c(new_n44_), .O(z05));
  aoi21  g27(.a(x12), .b(new_n55_), .c(new_n63_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(x10), .c(new_n46_), .d(x02), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x00), .c(new_n44_), .O(z06));
  nand3  g30(.a(new_n64_), .b(x11), .c(x10), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n65_), .c(x00), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(x01), .O(new_n76_));
  nand3  g33(.a(x02), .b(new_n44_), .c(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n76_), .O(z07));
  inv1   g35(.a(x08), .O(new_n79_));
  inv1   g36(.a(x18), .O(new_n80_));
  inv1   g37(.a(x05), .O(new_n81_));
  inv1   g38(.a(x06), .O(new_n82_));
  inv1   g39(.a(x03), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(x02), .c(new_n44_), .d(new_n55_), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(new_n85_));
  nand4  g42(.a(new_n85_), .b(new_n82_), .c(new_n81_), .d(x04), .O(new_n86_));
  nor2   g43(.a(new_n86_), .b(x07), .O(new_n87_));
  nand4  g44(.a(new_n87_), .b(new_n80_), .c(x17), .d(new_n79_), .O(new_n88_));
  nor2   g45(.a(new_n88_), .b(new_n58_), .O(z08));
  inv1   g46(.a(x15), .O(new_n90_));
  inv1   g47(.a(x16), .O(new_n91_));
  inv1   g48(.a(x13), .O(new_n92_));
  inv1   g49(.a(x14), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(x12), .O(new_n94_));
  nor3   g51(.a(new_n94_), .b(x11), .c(new_n45_), .O(new_n95_));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n98_));
  inv1   g55(.a(new_n98_), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(new_n95_), .c(new_n91_), .d(new_n90_), .O(new_n100_));
  aoi21  g57(.a(new_n100_), .b(new_n44_), .c(x00), .O(z09));
  nand4  g58(.a(new_n63_), .b(x02), .c(new_n44_), .d(new_n55_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n93_), .c(new_n92_), .d(x12), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n90_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n96_), .c(x20), .d(x16), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(x22), .O(z10));
  nand4  g64(.a(new_n99_), .b(new_n95_), .c(new_n91_), .d(x15), .O(new_n108_));
  aoi21  g65(.a(new_n108_), .b(new_n44_), .c(x00), .O(z11));
  inv1   g66(.a(x24), .O(new_n110_));
  aoi21  g67(.a(x10), .b(x02), .c(new_n44_), .O(new_n111_));
  oai21  g68(.a(x19), .b(x02), .c(x23), .O(new_n112_));
  nand3  g69(.a(new_n58_), .b(x17), .c(new_n45_), .O(new_n113_));
  aoi21  g70(.a(new_n113_), .b(new_n112_), .c(x00), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n111_), .c(new_n110_), .O(new_n115_));
  oai22  g72(.a(new_n115_), .b(new_n46_), .c(new_n44_), .d(x00), .O(z12));
  aoi21  g73(.a(new_n113_), .b(new_n44_), .c(x00), .O(z13));
  nand4  g74(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n55_), .O(new_n118_));
  inv1   g75(.a(new_n118_), .O(new_n119_));
  nand4  g76(.a(new_n119_), .b(new_n80_), .c(new_n57_), .d(new_n52_), .O(new_n120_));
  nor2   g77(.a(new_n120_), .b(x19), .O(z14));
  aoi21  g78(.a(new_n52_), .b(x01), .c(new_n45_), .O(new_n122_));
  nand3  g79(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n123_));
  oai21  g80(.a(new_n122_), .b(new_n55_), .c(new_n123_), .O(z15));
  nor2   g81(.a(new_n44_), .b(x00), .O(z16));
  nand3  g82(.a(x02), .b(new_n44_), .c(new_n55_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(z17));
endmodule


