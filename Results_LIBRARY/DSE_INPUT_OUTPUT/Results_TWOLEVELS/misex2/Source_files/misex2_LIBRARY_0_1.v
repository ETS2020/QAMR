// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n145_, new_n146_;
  inv1   g000(.a(x03), .O(new_n44_));
  nor3   g001(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  inv1   g002(.a(x17), .O(new_n46_));
  nor2   g003(.a(x19), .b(x18), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(x10), .O(new_n48_));
  inv1   g005(.a(new_n48_), .O(new_n49_));
  nand2  g006(.a(new_n49_), .b(new_n45_), .O(new_n50_));
  aoi21  g007(.a(new_n50_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g008(.a(x00), .O(new_n52_));
  inv1   g009(.a(x02), .O(new_n53_));
  nand4  g010(.a(new_n49_), .b(x09), .c(new_n53_), .d(new_n52_), .O(new_n54_));
  aoi21  g011(.a(new_n54_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g012(.a(x10), .O(new_n56_));
  inv1   g013(.a(x18), .O(new_n57_));
  inv1   g014(.a(x09), .O(new_n58_));
  inv1   g015(.a(x01), .O(new_n59_));
  nand4  g016(.a(new_n44_), .b(new_n53_), .c(new_n59_), .d(new_n52_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n57_), .c(new_n46_), .d(new_n56_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  nand2  g020(.a(new_n53_), .b(new_n52_), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nand3  g022(.a(new_n65_), .b(x18), .c(new_n46_), .O(new_n66_));
  oai21  g023(.a(new_n66_), .b(new_n64_), .c(new_n44_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n59_), .O(new_n68_));
  nand3  g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g026(.a(new_n56_), .b(x09), .O(new_n70_));
  nand3  g027(.a(new_n70_), .b(x12), .c(x11), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n69_), .c(new_n68_), .O(z03));
  nand2  g029(.a(x03), .b(new_n59_), .O(new_n73_));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n70_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n69_), .c(new_n73_), .O(z04));
  nand4  g034(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n56_), .O(z05));
  nor3   g036(.a(new_n74_), .b(new_n56_), .c(x09), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x02), .c(x01), .d(x00), .O(new_n81_));
  nand2  g038(.a(new_n81_), .b(new_n73_), .O(z06));
  nand3  g039(.a(new_n75_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n73_), .O(z07));
  nand2  g043(.a(x02), .b(new_n52_), .O(new_n87_));
  nor2   g044(.a(x06), .b(x05), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(x04), .O(new_n89_));
  nor2   g046(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nor2   g047(.a(x08), .b(x07), .O(new_n91_));
  nor2   g048(.a(new_n65_), .b(x18), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  aoi21  g050(.a(new_n93_), .b(new_n44_), .c(x01), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n74_), .c(x02), .d(new_n59_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  inv1   g058(.a(x20), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(new_n65_), .c(x18), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n52_), .O(new_n105_));
  nand2  g062(.a(new_n105_), .b(new_n73_), .O(z09));
  nor3   g063(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n65_), .c(x18), .d(x01), .O(new_n108_));
  nand3  g065(.a(new_n44_), .b(x02), .c(new_n59_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n74_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nor3   g069(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n113_));
  nor2   g070(.a(x22), .b(x21), .O(new_n114_));
  nand4  g071(.a(new_n114_), .b(new_n113_), .c(new_n112_), .d(x20), .O(new_n115_));
  aoi21  g072(.a(new_n115_), .b(new_n108_), .c(x00), .O(z10));
  nor2   g073(.a(new_n57_), .b(new_n59_), .O(new_n117_));
  nand4  g074(.a(new_n117_), .b(x21), .c(new_n102_), .d(new_n65_), .O(new_n118_));
  nor2   g075(.a(new_n53_), .b(x01), .O(new_n119_));
  nand3  g076(.a(x12), .b(new_n74_), .c(new_n44_), .O(new_n120_));
  inv1   g077(.a(new_n120_), .O(new_n121_));
  inv1   g078(.a(x14), .O(new_n122_));
  nand3  g079(.a(x15), .b(new_n122_), .c(new_n110_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(new_n124_));
  nand3  g081(.a(new_n95_), .b(x20), .c(new_n99_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n124_), .c(new_n121_), .d(new_n119_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n118_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n96_), .c(new_n52_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n65_), .b(x17), .c(new_n53_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n59_), .c(new_n52_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n73_), .O(z12));
  nand4  g097(.a(new_n65_), .b(x17), .c(new_n53_), .d(new_n52_), .O(new_n141_));
  aoi21  g098(.a(new_n141_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g099(.a(new_n47_), .b(new_n45_), .c(new_n46_), .d(new_n56_), .O(new_n143_));
  aoi21  g100(.a(new_n143_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g101(.a(new_n56_), .b(x01), .c(new_n53_), .O(new_n145_));
  aoi21  g102(.a(x19), .b(new_n53_), .c(x03), .O(new_n146_));
  oai22  g103(.a(new_n146_), .b(x01), .c(new_n145_), .d(new_n52_), .O(z15));
  nor2   g104(.a(new_n59_), .b(x00), .O(z16));
  aoi21  g105(.a(new_n87_), .b(new_n44_), .c(x01), .O(z17));
endmodule


