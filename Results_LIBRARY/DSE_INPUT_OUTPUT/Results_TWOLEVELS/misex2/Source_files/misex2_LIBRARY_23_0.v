// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n73_, new_n75_, new_n76_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n129_, new_n131_, new_n133_, new_n134_, new_n137_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x07), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(x07), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n44_), .d(x07), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  nor2   g23(.a(x07), .b(x01), .O(z08));
  inv1   g24(.a(z08), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n50_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n64_), .c(new_n68_), .O(z04));
  nand4  g29(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(new_n58_), .O(z05));
  nor3   g31(.a(new_n69_), .b(new_n58_), .c(x09), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(x02), .c(x01), .d(x00), .O(new_n76_));
  nand2  g33(.a(new_n76_), .b(new_n68_), .O(z06));
  nand2  g34(.a(x07), .b(new_n52_), .O(new_n78_));
  nand2  g35(.a(new_n50_), .b(x01), .O(new_n79_));
  nand3  g36(.a(new_n70_), .b(x11), .c(x10), .O(new_n80_));
  oai21  g37(.a(new_n80_), .b(new_n79_), .c(new_n78_), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x02), .c(x00), .O(new_n82_));
  inv1   g39(.a(new_n82_), .O(z07));
  inv1   g40(.a(x21), .O(new_n84_));
  inv1   g41(.a(x22), .O(new_n85_));
  nor2   g42(.a(new_n53_), .b(x01), .O(new_n86_));
  nor2   g43(.a(new_n70_), .b(x11), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n86_), .c(x07), .O(new_n88_));
  inv1   g45(.a(x15), .O(new_n89_));
  inv1   g46(.a(x16), .O(new_n90_));
  nor2   g47(.a(x14), .b(x13), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x20), .c(new_n90_), .d(new_n89_), .O(new_n92_));
  inv1   g49(.a(x20), .O(new_n93_));
  nor2   g50(.a(new_n49_), .b(new_n52_), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(new_n62_), .O(new_n95_));
  oai21  g52(.a(new_n92_), .b(new_n88_), .c(new_n95_), .O(new_n96_));
  nand4  g53(.a(new_n96_), .b(new_n85_), .c(new_n84_), .d(new_n51_), .O(new_n97_));
  inv1   g54(.a(new_n97_), .O(z09));
  nand3  g55(.a(new_n62_), .b(x18), .c(x01), .O(new_n99_));
  nand3  g56(.a(x22), .b(x21), .c(new_n93_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  nand2  g58(.a(x02), .b(new_n52_), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n69_), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n102_), .O(new_n105_));
  inv1   g62(.a(x14), .O(new_n106_));
  nand3  g63(.a(x16), .b(x15), .c(new_n106_), .O(new_n107_));
  nand3  g64(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  aoi21  g66(.a(new_n109_), .b(new_n105_), .c(new_n101_), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(x00), .c(new_n68_), .O(z10));
  nand4  g68(.a(new_n86_), .b(new_n103_), .c(x12), .d(new_n69_), .O(new_n112_));
  nand4  g69(.a(new_n94_), .b(x21), .c(new_n93_), .d(new_n62_), .O(new_n113_));
  nor2   g70(.a(new_n89_), .b(x14), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n84_), .c(x20), .d(new_n90_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n112_), .c(new_n113_), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(new_n85_), .c(new_n51_), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(new_n68_), .O(z11));
  inv1   g75(.a(x24), .O(new_n119_));
  aoi21  g76(.a(x10), .b(x02), .c(new_n52_), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x00), .O(new_n121_));
  oai21  g78(.a(x19), .b(x02), .c(x23), .O(new_n122_));
  nand3  g79(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n122_), .O(new_n124_));
  nand4  g81(.a(new_n124_), .b(x07), .c(new_n52_), .d(new_n51_), .O(new_n125_));
  nand2  g82(.a(new_n125_), .b(new_n121_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n119_), .c(x09), .O(new_n127_));
  inv1   g84(.a(new_n127_), .O(z12));
  nand4  g85(.a(new_n62_), .b(x17), .c(new_n53_), .d(new_n51_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(x07), .c(x01), .O(z13));
  nand4  g87(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n131_));
  aoi21  g88(.a(new_n131_), .b(x07), .c(x01), .O(z14));
  aoi21  g89(.a(x07), .b(new_n53_), .c(new_n120_), .O(new_n133_));
  nand4  g90(.a(x19), .b(x07), .c(new_n53_), .d(new_n52_), .O(new_n134_));
  oai21  g91(.a(new_n133_), .b(new_n51_), .c(new_n134_), .O(z15));
  nor2   g92(.a(new_n52_), .b(x00), .O(z16));
  nand2  g93(.a(x02), .b(new_n51_), .O(new_n137_));
  aoi21  g94(.a(new_n137_), .b(x07), .c(x01), .O(z17));
endmodule


