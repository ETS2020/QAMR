// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n124_, new_n126_, new_n127_, new_n128_, new_n130_,
    new_n131_, new_n132_, new_n134_, new_n136_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  inv1   g05(.a(x18), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x19), .O(z00));
  nand4  g10(.a(new_n51_), .b(x09), .c(new_n46_), .d(new_n45_), .O(new_n54_));
  aoi21  g11(.a(new_n54_), .b(new_n44_), .c(x19), .O(z01));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(x09), .b(new_n46_), .c(new_n44_), .d(new_n45_), .O(new_n57_));
  inv1   g14(.a(new_n57_), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(new_n49_), .c(new_n48_), .d(new_n56_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n47_), .b(x02), .c(x00), .O(new_n61_));
  nand3  g18(.a(x12), .b(x11), .c(x10), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(x19), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g21(.a(x19), .O(new_n65_));
  nor2   g22(.a(x02), .b(x00), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n48_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(new_n64_), .O(z03));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  inv1   g27(.a(new_n61_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n72_));
  aoi21  g29(.a(new_n72_), .b(x19), .c(new_n44_), .O(z04));
  nand4  g30(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x19), .c(new_n44_), .O(z05));
  nor2   g32(.a(new_n69_), .b(new_n56_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(new_n47_), .c(x02), .d(x00), .O(new_n77_));
  aoi21  g34(.a(new_n77_), .b(x19), .c(new_n44_), .O(z06));
  nand3  g35(.a(new_n70_), .b(x11), .c(x10), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n65_), .b(x01), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n81_), .O(z07));
  inv1   g40(.a(x04), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n44_), .c(new_n45_), .O(new_n85_));
  nor4   g42(.a(new_n85_), .b(x05), .c(new_n84_), .d(x03), .O(new_n86_));
  nor3   g43(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  nor2   g44(.a(new_n65_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(new_n82_), .O(z08));
  nor4   g47(.a(new_n85_), .b(x13), .c(new_n70_), .d(x11), .O(new_n91_));
  nor3   g48(.a(x16), .b(x15), .c(x14), .O(new_n92_));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand3  g53(.a(new_n96_), .b(new_n92_), .c(new_n91_), .O(new_n97_));
  nand2  g54(.a(new_n97_), .b(new_n82_), .O(z09));
  inv1   g55(.a(x15), .O(new_n99_));
  inv1   g56(.a(x16), .O(new_n100_));
  nor3   g57(.a(new_n100_), .b(new_n99_), .c(x14), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n96_), .c(new_n91_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n82_), .O(z10));
  inv1   g60(.a(x13), .O(new_n104_));
  inv1   g61(.a(x14), .O(new_n105_));
  nand4  g62(.a(new_n69_), .b(x02), .c(new_n44_), .d(new_n45_), .O(new_n106_));
  inv1   g63(.a(new_n106_), .O(new_n107_));
  nand4  g64(.a(new_n107_), .b(new_n105_), .c(new_n104_), .d(x12), .O(new_n108_));
  nor2   g65(.a(new_n108_), .b(new_n99_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n93_), .c(x20), .d(new_n100_), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(x22), .O(z11));
  nand2  g68(.a(x10), .b(x02), .O(new_n112_));
  nand3  g69(.a(new_n112_), .b(x01), .c(x00), .O(new_n113_));
  nand3  g70(.a(x23), .b(new_n44_), .c(new_n45_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand2  g72(.a(new_n115_), .b(x19), .O(new_n116_));
  nand3  g73(.a(new_n65_), .b(x17), .c(new_n46_), .O(new_n117_));
  nand2  g74(.a(x23), .b(x02), .O(new_n118_));
  nand2  g75(.a(new_n118_), .b(new_n117_), .O(new_n119_));
  nand3  g76(.a(new_n119_), .b(new_n44_), .c(new_n45_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n116_), .c(x24), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(x09), .O(new_n122_));
  inv1   g79(.a(new_n122_), .O(z12));
  nand3  g80(.a(x17), .b(new_n46_), .c(new_n45_), .O(new_n124_));
  aoi21  g81(.a(new_n124_), .b(new_n44_), .c(x19), .O(z13));
  nand4  g82(.a(new_n47_), .b(new_n46_), .c(new_n44_), .d(new_n45_), .O(new_n126_));
  inv1   g83(.a(new_n126_), .O(new_n127_));
  nand4  g84(.a(new_n127_), .b(new_n49_), .c(new_n48_), .d(new_n56_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(x19), .O(z14));
  oai21  g86(.a(x10), .b(new_n44_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n46_), .c(new_n44_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n82_), .O(z15));
  nand3  g90(.a(x19), .b(x01), .c(new_n45_), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z16));
  nand3  g92(.a(x02), .b(new_n44_), .c(new_n45_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z17));
endmodule


