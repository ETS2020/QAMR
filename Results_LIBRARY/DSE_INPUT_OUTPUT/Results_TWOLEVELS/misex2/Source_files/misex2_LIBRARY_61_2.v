// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n137_, new_n139_, new_n141_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand4  g010(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  inv1   g014(.a(x10), .O(new_n58_));
  nand4  g015(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x19), .O(z02));
  nand2  g017(.a(x02), .b(x00), .O(new_n61_));
  nand3  g018(.a(x12), .b(x11), .c(new_n49_), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n61_), .c(x10), .O(new_n63_));
  nand2  g020(.a(new_n63_), .b(x01), .O(new_n64_));
  inv1   g021(.a(x19), .O(new_n65_));
  nor3   g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g023(.a(new_n66_), .b(new_n65_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand2  g024(.a(new_n67_), .b(new_n64_), .O(z03));
  nor2   g025(.a(x12), .b(x11), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n49_), .c(x02), .d(x00), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(x10), .c(new_n47_), .O(z04));
  nand4  g028(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n72_));
  nor2   g029(.a(new_n72_), .b(new_n58_), .O(z05));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  aoi21  g032(.a(new_n75_), .b(new_n58_), .c(new_n74_), .O(new_n76_));
  nand4  g033(.a(new_n76_), .b(new_n49_), .c(x02), .d(x00), .O(new_n77_));
  aoi21  g034(.a(new_n77_), .b(x10), .c(new_n47_), .O(z06));
  nand3  g035(.a(new_n75_), .b(x11), .c(x10), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nand3  g037(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g038(.a(new_n81_), .O(z07));
  nand2  g039(.a(new_n58_), .b(x01), .O(new_n83_));
  nor2   g040(.a(new_n48_), .b(x01), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n46_), .O(new_n85_));
  inv1   g042(.a(x03), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x04), .c(new_n86_), .O(new_n88_));
  nor2   g045(.a(new_n88_), .b(new_n85_), .O(new_n89_));
  nor3   g046(.a(x08), .b(x07), .c(x06), .O(new_n90_));
  nand3  g047(.a(x19), .b(new_n45_), .c(x17), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(new_n90_), .c(new_n89_), .O(new_n93_));
  nand2  g050(.a(new_n93_), .b(new_n83_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  nand4  g053(.a(x12), .b(new_n74_), .c(x02), .d(new_n47_), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x16), .O(new_n99_));
  nor2   g056(.a(x14), .b(x13), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(x20), .c(new_n99_), .d(new_n98_), .O(new_n101_));
  nor2   g058(.a(x20), .b(x19), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x18), .c(x10), .d(x01), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n97_), .c(new_n103_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n46_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  nand3  g063(.a(x18), .b(x10), .c(x01), .O(new_n107_));
  inv1   g064(.a(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n102_), .c(x22), .d(x21), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n74_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nor3   g069(.a(new_n99_), .b(new_n98_), .c(x14), .O(new_n113_));
  nand3  g070(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n113_), .c(new_n112_), .d(new_n84_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n109_), .c(x00), .O(z10));
  nor2   g074(.a(x13), .b(new_n75_), .O(new_n118_));
  nand3  g075(.a(new_n118_), .b(new_n84_), .c(new_n74_), .O(new_n119_));
  inv1   g076(.a(x20), .O(new_n120_));
  nand4  g077(.a(new_n108_), .b(x21), .c(new_n120_), .d(new_n65_), .O(new_n121_));
  nor2   g078(.a(new_n98_), .b(x14), .O(new_n122_));
  nor2   g079(.a(x21), .b(new_n120_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n122_), .c(new_n99_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n119_), .c(new_n121_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n96_), .c(new_n46_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand4  g085(.a(new_n128_), .b(x09), .c(new_n48_), .d(x00), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(x10), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(x01), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n65_), .b(x17), .c(new_n48_), .O(new_n133_));
  aoi21  g090(.a(new_n133_), .b(new_n132_), .c(x24), .O(new_n134_));
  nand4  g091(.a(new_n134_), .b(x09), .c(new_n47_), .d(new_n46_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n131_), .O(z12));
  nand3  g093(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n137_));
  nor3   g094(.a(new_n137_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g095(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x19), .O(z14));
  aoi21  g097(.a(x19), .b(new_n47_), .c(x00), .O(new_n141_));
  oai21  g098(.a(new_n141_), .b(x02), .c(new_n83_), .O(z15));
  aoi21  g099(.a(x10), .b(x00), .c(new_n47_), .O(z16));
  nand2  g100(.a(new_n85_), .b(new_n83_), .O(z17));
endmodule


