// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n111_, new_n113_, new_n114_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n122_, new_n124_,
    new_n125_, new_n127_, new_n128_, new_n131_;
  inv1   g00(.a(x15), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  inv1   g04(.a(x17), .O(new_n48_));
  nor2   g05(.a(x19), .b(x18), .O(new_n49_));
  nand3  g06(.a(new_n49_), .b(new_n48_), .c(x10), .O(new_n50_));
  inv1   g07(.a(new_n50_), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g10(.a(x09), .b(new_n46_), .c(new_n45_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand2  g12(.a(new_n55_), .b(new_n51_), .O(new_n56_));
  aoi21  g13(.a(new_n56_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n49_), .c(new_n48_), .d(new_n58_), .O(new_n59_));
  aoi21  g16(.a(new_n59_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g17(.a(x01), .O(new_n61_));
  nand3  g18(.a(new_n46_), .b(new_n61_), .c(new_n45_), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x18), .c(new_n48_), .d(new_n44_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g22(.a(x12), .b(x11), .c(x10), .d(new_n47_), .O(new_n66_));
  oai22  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .d(new_n62_), .O(z03));
  nand2  g24(.a(x15), .b(new_n61_), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n47_), .O(new_n71_));
  oai21  g28(.a(new_n71_), .b(new_n65_), .c(new_n68_), .O(z04));
  nand2  g29(.a(x01), .b(x00), .O(new_n73_));
  nand3  g30(.a(x10), .b(x09), .c(x02), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n73_), .c(new_n68_), .O(z05));
  nand4  g32(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n61_), .c(new_n45_), .O(z06));
  nand2  g34(.a(new_n47_), .b(x01), .O(new_n78_));
  nand3  g35(.a(new_n70_), .b(x11), .c(x10), .O(new_n79_));
  oai22  g36(.a(new_n79_), .b(new_n78_), .c(x15), .d(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  inv1   g39(.a(x18), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x07), .O(new_n86_));
  inv1   g43(.a(x04), .O(new_n87_));
  inv1   g44(.a(x03), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x02), .c(new_n61_), .d(new_n45_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n86_), .c(new_n85_), .d(new_n84_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(x08), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n83_), .c(x17), .d(new_n44_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(new_n63_), .O(z08));
  inv1   g51(.a(x21), .O(new_n95_));
  inv1   g52(.a(x22), .O(new_n96_));
  nand4  g53(.a(x12), .b(new_n69_), .c(x02), .d(new_n61_), .O(new_n97_));
  inv1   g54(.a(x16), .O(new_n98_));
  nor2   g55(.a(x14), .b(x13), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x20), .c(new_n98_), .O(new_n100_));
  nand2  g57(.a(x18), .b(x01), .O(new_n101_));
  inv1   g58(.a(x20), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n63_), .O(new_n103_));
  oai22  g60(.a(new_n103_), .b(new_n101_), .c(new_n100_), .d(new_n97_), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n45_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n68_), .O(z09));
  nand3  g63(.a(x18), .b(x01), .c(new_n45_), .O(new_n107_));
  inv1   g64(.a(new_n103_), .O(new_n108_));
  nand3  g65(.a(new_n108_), .b(x22), .c(x21), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n107_), .c(new_n68_), .O(z10));
  nand3  g67(.a(new_n108_), .b(new_n96_), .c(x21), .O(new_n111_));
  oai21  g68(.a(new_n111_), .b(new_n107_), .c(new_n68_), .O(z11));
  nand2  g69(.a(x10), .b(x02), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(x01), .c(x00), .O(new_n114_));
  oai21  g71(.a(x19), .b(x02), .c(x23), .O(new_n115_));
  nand3  g72(.a(new_n63_), .b(x17), .c(new_n46_), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n115_), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(new_n61_), .c(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n114_), .c(x24), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(x09), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(new_n68_), .O(z12));
  nand3  g78(.a(new_n46_), .b(new_n61_), .c(new_n45_), .O(new_n122_));
  nor4   g79(.a(new_n122_), .b(x19), .c(new_n48_), .d(x15), .O(z13));
  nor3   g80(.a(new_n122_), .b(x10), .c(x09), .O(new_n124_));
  nand4  g81(.a(new_n124_), .b(new_n83_), .c(new_n48_), .d(new_n44_), .O(new_n125_));
  nor2   g82(.a(new_n125_), .b(x19), .O(z14));
  aoi21  g83(.a(new_n58_), .b(x01), .c(new_n46_), .O(new_n127_));
  aoi21  g84(.a(x19), .b(new_n46_), .c(x15), .O(new_n128_));
  oai22  g85(.a(new_n128_), .b(x01), .c(new_n127_), .d(new_n45_), .O(z15));
  nor2   g86(.a(new_n61_), .b(x00), .O(z16));
  nand4  g87(.a(new_n44_), .b(x02), .c(new_n61_), .d(new_n45_), .O(new_n131_));
  inv1   g88(.a(new_n131_), .O(z17));
endmodule


