// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:34 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n141_, new_n142_,
    new_n144_, new_n145_, new_n146_;
  inv1   g000(.a(x01), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  inv1   g003(.a(x10), .O(new_n47_));
  inv1   g004(.a(x17), .O(new_n48_));
  inv1   g005(.a(x18), .O(new_n49_));
  inv1   g006(.a(x19), .O(new_n50_));
  nor2   g007(.a(x23), .b(x21), .O(new_n51_));
  inv1   g008(.a(new_n51_), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n53_));
  nor2   g010(.a(new_n53_), .b(new_n47_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x00), .O(z00));
  inv1   g013(.a(x00), .O(new_n57_));
  nand4  g014(.a(x09), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nand4  g015(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n59_));
  oai21  g016(.a(new_n59_), .b(new_n58_), .c(new_n52_), .O(z01));
  nand4  g017(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n61_));
  oai21  g018(.a(new_n61_), .b(new_n58_), .c(new_n52_), .O(z02));
  nand3  g019(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  inv1   g020(.a(new_n63_), .O(new_n64_));
  inv1   g021(.a(x11), .O(new_n65_));
  inv1   g022(.a(x12), .O(new_n66_));
  nor2   g023(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n64_), .c(x10), .d(new_n46_), .O(new_n68_));
  nor3   g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n50_), .c(x18), .d(new_n48_), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(new_n68_), .c(new_n51_), .O(z03));
  nand4  g028(.a(new_n52_), .b(new_n66_), .c(new_n65_), .d(x10), .O(new_n72_));
  inv1   g029(.a(new_n72_), .O(new_n73_));
  nand4  g030(.a(new_n73_), .b(new_n46_), .c(x02), .d(x01), .O(new_n74_));
  nor2   g031(.a(new_n74_), .b(new_n57_), .O(z04));
  nand2  g032(.a(x01), .b(x00), .O(new_n76_));
  nand3  g033(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n76_), .c(new_n52_), .O(z05));
  nor3   g035(.a(new_n51_), .b(new_n65_), .c(new_n47_), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(new_n46_), .c(x02), .d(x01), .O(new_n80_));
  nor2   g037(.a(new_n80_), .b(new_n57_), .O(z06));
  nand4  g038(.a(new_n52_), .b(new_n66_), .c(x11), .d(x10), .O(new_n82_));
  oai21  g039(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n52_), .O(z07));
  inv1   g042(.a(x03), .O(new_n86_));
  nor2   g043(.a(new_n45_), .b(x01), .O(new_n87_));
  inv1   g044(.a(x04), .O(new_n88_));
  nor2   g045(.a(x05), .b(new_n88_), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(new_n57_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(x19), .c(new_n49_), .d(x17), .O(new_n92_));
  oai21  g049(.a(new_n92_), .b(new_n90_), .c(new_n52_), .O(z08));
  inv1   g050(.a(x22), .O(new_n94_));
  nand3  g051(.a(new_n87_), .b(x12), .c(new_n65_), .O(new_n95_));
  inv1   g052(.a(x13), .O(new_n96_));
  inv1   g053(.a(x14), .O(new_n97_));
  inv1   g054(.a(x15), .O(new_n98_));
  inv1   g055(.a(x20), .O(new_n99_));
  nor2   g056(.a(new_n99_), .b(x16), .O(new_n100_));
  nand4  g057(.a(new_n100_), .b(new_n98_), .c(new_n97_), .d(new_n96_), .O(new_n101_));
  nor2   g058(.a(new_n49_), .b(new_n44_), .O(new_n102_));
  nand3  g059(.a(new_n102_), .b(new_n99_), .c(new_n50_), .O(new_n103_));
  oai21  g060(.a(new_n101_), .b(new_n95_), .c(new_n103_), .O(new_n104_));
  nand3  g061(.a(new_n104_), .b(new_n94_), .c(new_n57_), .O(new_n105_));
  aoi21  g062(.a(new_n105_), .b(x23), .c(x21), .O(z09));
  nand3  g063(.a(new_n50_), .b(x18), .c(x01), .O(new_n107_));
  nand3  g064(.a(x22), .b(x21), .c(new_n99_), .O(new_n108_));
  nor2   g065(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand3  g066(.a(new_n96_), .b(x12), .c(new_n65_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand2  g068(.a(new_n111_), .b(new_n87_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nand3  g070(.a(x16), .b(x15), .c(new_n97_), .O(new_n114_));
  inv1   g071(.a(x21), .O(new_n115_));
  nand3  g072(.a(new_n94_), .b(new_n115_), .c(x20), .O(new_n116_));
  nor2   g073(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n113_), .c(new_n109_), .O(new_n118_));
  oai21  g075(.a(new_n118_), .b(x00), .c(new_n52_), .O(z10));
  nand4  g076(.a(new_n102_), .b(x21), .c(new_n99_), .d(new_n50_), .O(new_n120_));
  nor2   g077(.a(x16), .b(new_n98_), .O(new_n121_));
  inv1   g078(.a(x23), .O(new_n122_));
  nor2   g079(.a(new_n122_), .b(x21), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n121_), .c(x20), .d(new_n97_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n112_), .c(new_n120_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n94_), .c(new_n57_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand4  g086(.a(new_n129_), .b(new_n52_), .c(x01), .d(x00), .O(new_n130_));
  nand3  g087(.a(x21), .b(new_n50_), .c(new_n45_), .O(new_n131_));
  aoi21  g088(.a(new_n131_), .b(new_n122_), .c(new_n48_), .O(new_n132_));
  aoi21  g089(.a(new_n50_), .b(new_n45_), .c(new_n122_), .O(new_n133_));
  oai21  g090(.a(new_n133_), .b(new_n132_), .c(new_n44_), .O(new_n134_));
  oai21  g091(.a(new_n134_), .b(x00), .c(new_n130_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n128_), .c(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  nand2  g094(.a(new_n44_), .b(new_n57_), .O(new_n138_));
  nand3  g095(.a(new_n50_), .b(x17), .c(new_n45_), .O(new_n139_));
  oai21  g096(.a(new_n139_), .b(new_n138_), .c(new_n52_), .O(z13));
  nor2   g097(.a(new_n53_), .b(x10), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x00), .O(z14));
  oai21  g100(.a(x10), .b(new_n44_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n146_));
  aoi21  g103(.a(new_n146_), .b(new_n145_), .c(new_n51_), .O(z15));
  nor3   g104(.a(new_n51_), .b(new_n44_), .c(x00), .O(z16));
  nor4   g105(.a(new_n51_), .b(new_n45_), .c(x01), .d(x00), .O(z17));
endmodule


