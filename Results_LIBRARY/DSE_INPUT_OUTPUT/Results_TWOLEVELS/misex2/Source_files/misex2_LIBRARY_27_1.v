// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n124_, new_n125_, new_n127_, new_n128_, new_n131_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x00), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x19), .b(x18), .O(new_n48_));
  nand2  g05(.a(new_n48_), .b(x10), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n51_));
  aoi21  g08(.a(new_n51_), .b(new_n44_), .c(x01), .O(z00));
  nand3  g09(.a(x09), .b(new_n46_), .c(new_n45_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand2  g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand3  g14(.a(new_n54_), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  aoi21  g15(.a(new_n58_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x18), .c(new_n46_), .d(new_n45_), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(new_n44_), .O(new_n63_));
  nand2  g20(.a(new_n63_), .b(new_n60_), .O(new_n64_));
  nand3  g21(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nor2   g22(.a(new_n57_), .b(x09), .O(new_n66_));
  nand3  g23(.a(new_n66_), .b(x12), .c(x11), .O(new_n67_));
  oai21  g24(.a(new_n67_), .b(new_n65_), .c(new_n64_), .O(z03));
  nand2  g25(.a(x17), .b(new_n60_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand3  g28(.a(new_n66_), .b(new_n71_), .c(new_n70_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n65_), .c(new_n69_), .O(z04));
  nand4  g30(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(new_n57_), .O(z05));
  nor3   g32(.a(new_n70_), .b(new_n57_), .c(x09), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nand2  g34(.a(new_n77_), .b(new_n69_), .O(z06));
  nand3  g35(.a(new_n71_), .b(x11), .c(x10), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n69_), .O(z07));
  inv1   g39(.a(new_n69_), .O(z08));
  inv1   g40(.a(x21), .O(new_n84_));
  inv1   g41(.a(x22), .O(new_n85_));
  nor2   g42(.a(new_n46_), .b(x01), .O(new_n86_));
  nor2   g43(.a(x13), .b(new_n71_), .O(new_n87_));
  nand3  g44(.a(new_n87_), .b(new_n86_), .c(new_n70_), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x15), .b(x14), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x20), .c(new_n44_), .d(new_n89_), .O(new_n91_));
  inv1   g48(.a(x20), .O(new_n92_));
  and2   g49(.a(x18), .b(x01), .O(new_n93_));
  nand3  g50(.a(new_n93_), .b(new_n92_), .c(new_n61_), .O(new_n94_));
  oai21  g51(.a(new_n91_), .b(new_n88_), .c(new_n94_), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n85_), .c(new_n84_), .d(new_n45_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z09));
  nor3   g54(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(new_n61_), .c(x18), .d(x01), .O(new_n99_));
  nand3  g56(.a(new_n70_), .b(x02), .c(new_n60_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nor2   g58(.a(x14), .b(x13), .O(new_n102_));
  nand3  g59(.a(new_n44_), .b(x16), .c(x15), .O(new_n103_));
  nand3  g60(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n102_), .c(new_n101_), .d(x12), .O(new_n106_));
  aoi21  g63(.a(new_n106_), .b(new_n99_), .c(x00), .O(z10));
  nand4  g64(.a(new_n93_), .b(x21), .c(new_n92_), .d(new_n61_), .O(new_n108_));
  inv1   g65(.a(x15), .O(new_n109_));
  nor2   g66(.a(new_n109_), .b(x14), .O(new_n110_));
  nor2   g67(.a(x21), .b(new_n92_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n110_), .c(new_n89_), .O(new_n112_));
  oai21  g69(.a(new_n112_), .b(new_n88_), .c(new_n108_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n85_), .c(new_n45_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n69_), .O(z11));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(x01), .c(x00), .O(new_n117_));
  nand2  g74(.a(new_n61_), .b(new_n46_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(x23), .c(new_n60_), .d(new_n45_), .O(new_n119_));
  aoi21  g76(.a(new_n119_), .b(new_n117_), .c(x24), .O(new_n120_));
  nand2  g77(.a(new_n120_), .b(x09), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n69_), .O(z12));
  nor3   g79(.a(x02), .b(x01), .c(x00), .O(new_n124_));
  nand4  g80(.a(new_n124_), .b(new_n44_), .c(new_n57_), .d(new_n47_), .O(new_n125_));
  nor3   g81(.a(new_n125_), .b(x19), .c(x18), .O(z14));
  aoi21  g82(.a(new_n57_), .b(x01), .c(new_n46_), .O(new_n127_));
  aoi21  g83(.a(x19), .b(new_n46_), .c(x17), .O(new_n128_));
  oai22  g84(.a(new_n128_), .b(x01), .c(new_n127_), .d(new_n45_), .O(z15));
  oai21  g85(.a(new_n60_), .b(x00), .c(new_n69_), .O(z16));
  nand4  g86(.a(new_n44_), .b(x02), .c(new_n60_), .d(new_n45_), .O(new_n131_));
  inv1   g87(.a(new_n131_), .O(z17));
  zero   g88(.O(z13));
endmodule


