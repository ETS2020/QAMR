// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n73_, new_n75_, new_n76_, new_n77_, new_n78_,
    new_n79_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n128_, new_n130_,
    new_n133_;
  inv1   g00(.a(x19), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x18), .b(x17), .O(new_n46_));
  nand3  g03(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(new_n44_), .c(x01), .O(z00));
  inv1   g05(.a(x00), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x02), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n46_), .c(x10), .d(new_n49_), .O(new_n52_));
  aoi21  g09(.a(new_n52_), .b(new_n44_), .c(x01), .O(z01));
  nor3   g10(.a(x18), .b(x17), .c(x10), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(new_n51_), .c(new_n49_), .O(new_n55_));
  aoi21  g12(.a(new_n55_), .b(new_n44_), .c(x01), .O(z02));
  inv1   g13(.a(x01), .O(new_n57_));
  inv1   g14(.a(x02), .O(new_n58_));
  inv1   g15(.a(x17), .O(new_n59_));
  nand4  g16(.a(x18), .b(new_n59_), .c(new_n58_), .d(new_n49_), .O(new_n60_));
  nand2  g17(.a(new_n60_), .b(new_n44_), .O(new_n61_));
  nand2  g18(.a(new_n61_), .b(new_n57_), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n64_));
  oai21  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(x10), .d(new_n50_), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(x12), .O(z04));
  inv1   g27(.a(x10), .O(new_n71_));
  nor3   g28(.a(new_n67_), .b(new_n71_), .c(new_n50_), .O(z05));
  nand4  g29(.a(x11), .b(x10), .c(new_n50_), .d(x02), .O(new_n73_));
  nor3   g30(.a(new_n73_), .b(new_n57_), .c(new_n49_), .O(z06));
  nand2  g31(.a(new_n50_), .b(x01), .O(new_n75_));
  inv1   g32(.a(x12), .O(new_n76_));
  nand3  g33(.a(new_n76_), .b(x11), .c(x10), .O(new_n77_));
  oai22  g34(.a(new_n77_), .b(new_n75_), .c(x19), .d(x01), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x02), .c(x00), .O(new_n79_));
  inv1   g36(.a(new_n79_), .O(z07));
  nand2  g37(.a(x19), .b(new_n57_), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z08));
  inv1   g39(.a(x21), .O(new_n83_));
  inv1   g40(.a(x22), .O(new_n84_));
  nand4  g41(.a(x12), .b(new_n66_), .c(x02), .d(new_n57_), .O(new_n85_));
  inv1   g42(.a(x15), .O(new_n86_));
  inv1   g43(.a(x16), .O(new_n87_));
  nor2   g44(.a(x14), .b(x13), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(x20), .c(new_n87_), .d(new_n86_), .O(new_n89_));
  inv1   g46(.a(x20), .O(new_n90_));
  nand3  g47(.a(new_n90_), .b(x18), .c(x01), .O(new_n91_));
  oai21  g48(.a(new_n89_), .b(new_n85_), .c(new_n91_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n84_), .c(new_n83_), .d(new_n44_), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x00), .O(z09));
  nand3  g51(.a(new_n44_), .b(x18), .c(x01), .O(new_n95_));
  nand3  g52(.a(x22), .b(x21), .c(new_n90_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g54(.a(x02), .b(new_n57_), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x12), .c(new_n66_), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(new_n98_), .O(new_n101_));
  inv1   g58(.a(x14), .O(new_n102_));
  nand3  g59(.a(x16), .b(x15), .c(new_n102_), .O(new_n103_));
  nand3  g60(.a(new_n84_), .b(new_n83_), .c(x20), .O(new_n104_));
  nor2   g61(.a(new_n104_), .b(new_n103_), .O(new_n105_));
  aoi21  g62(.a(new_n105_), .b(new_n101_), .c(new_n97_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(x00), .c(new_n81_), .O(z10));
  nor2   g64(.a(new_n58_), .b(x01), .O(new_n108_));
  nor2   g65(.a(x13), .b(new_n76_), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(new_n108_), .c(new_n66_), .O(new_n110_));
  nand4  g67(.a(x21), .b(new_n90_), .c(x18), .d(x01), .O(new_n111_));
  nor2   g68(.a(new_n86_), .b(x14), .O(new_n112_));
  nor2   g69(.a(x21), .b(new_n90_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n112_), .c(new_n87_), .O(new_n114_));
  oai21  g71(.a(new_n114_), .b(new_n110_), .c(new_n111_), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n84_), .c(new_n44_), .d(new_n49_), .O(new_n116_));
  inv1   g73(.a(new_n116_), .O(z11));
  nand2  g74(.a(x10), .b(x02), .O(new_n118_));
  nand3  g75(.a(new_n118_), .b(x01), .c(x00), .O(new_n119_));
  nand2  g76(.a(x23), .b(x02), .O(new_n120_));
  oai21  g77(.a(new_n59_), .b(x02), .c(new_n120_), .O(new_n121_));
  nand3  g78(.a(new_n121_), .b(new_n57_), .c(new_n49_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n119_), .c(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n81_), .O(z12));
  nand3  g82(.a(x17), .b(new_n58_), .c(new_n49_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n44_), .c(x01), .O(z13));
  nand2  g84(.a(new_n54_), .b(new_n45_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n44_), .c(x01), .O(z14));
  aoi21  g86(.a(new_n71_), .b(x01), .c(new_n58_), .O(new_n130_));
  oai21  g87(.a(new_n130_), .b(new_n49_), .c(new_n81_), .O(z15));
  nor2   g88(.a(new_n57_), .b(x00), .O(z16));
  nand4  g89(.a(new_n44_), .b(x02), .c(new_n57_), .d(new_n49_), .O(new_n133_));
  inv1   g90(.a(new_n133_), .O(z17));
endmodule


