// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x08), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  nor2   g006(.a(new_n49_), .b(x09), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nor3   g009(.a(x02), .b(x01), .c(x00), .O(new_n53_));
  nor2   g010(.a(x19), .b(x18), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n53_), .c(x10), .d(x09), .O(new_n55_));
  aoi21  g012(.a(new_n55_), .b(new_n47_), .c(x17), .O(z01));
  inv1   g013(.a(x10), .O(new_n57_));
  inv1   g014(.a(x09), .O(new_n58_));
  nor2   g015(.a(new_n49_), .b(new_n58_), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x19), .O(z02));
  inv1   g018(.a(new_n48_), .O(new_n62_));
  inv1   g019(.a(x19), .O(new_n63_));
  nand3  g020(.a(new_n63_), .b(x18), .c(new_n46_), .O(new_n64_));
  oai21  g021(.a(new_n64_), .b(new_n62_), .c(new_n47_), .O(new_n65_));
  nand2  g022(.a(new_n65_), .b(new_n44_), .O(new_n66_));
  nand3  g023(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g024(.a(new_n57_), .b(x09), .O(new_n68_));
  nand3  g025(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g026(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nor2   g027(.a(x17), .b(new_n47_), .O(new_n71_));
  inv1   g028(.a(new_n71_), .O(new_n72_));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nand3  g031(.a(new_n68_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n67_), .c(new_n72_), .O(z04));
  inv1   g033(.a(x00), .O(new_n77_));
  nor2   g034(.a(new_n71_), .b(new_n57_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x09), .c(x02), .d(x01), .O(new_n79_));
  nor2   g036(.a(new_n79_), .b(new_n77_), .O(z05));
  inv1   g037(.a(x01), .O(new_n81_));
  nor2   g038(.a(new_n71_), .b(new_n73_), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(x10), .c(new_n58_), .d(x02), .O(new_n83_));
  nor3   g040(.a(new_n83_), .b(new_n81_), .c(new_n77_), .O(z06));
  nand3  g041(.a(new_n68_), .b(new_n74_), .c(x11), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n72_), .O(z07));
  inv1   g045(.a(x05), .O(new_n89_));
  inv1   g046(.a(x06), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  nand3  g048(.a(new_n48_), .b(new_n91_), .c(x02), .O(new_n92_));
  inv1   g049(.a(new_n92_), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n90_), .c(new_n89_), .d(x04), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x07), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n45_), .c(x17), .d(new_n47_), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(new_n63_), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nand2  g056(.a(x02), .b(new_n81_), .O(new_n100_));
  inv1   g057(.a(new_n100_), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x12), .c(new_n73_), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nor2   g061(.a(x14), .b(x13), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(x20), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  inv1   g063(.a(x20), .O(new_n107_));
  nor2   g064(.a(new_n45_), .b(new_n81_), .O(new_n108_));
  nand3  g065(.a(new_n108_), .b(new_n107_), .c(new_n63_), .O(new_n109_));
  oai21  g066(.a(new_n106_), .b(new_n102_), .c(new_n109_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n72_), .c(new_n99_), .d(new_n98_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(x00), .O(z09));
  nand3  g069(.a(new_n63_), .b(x18), .c(x01), .O(new_n113_));
  nor4   g070(.a(new_n113_), .b(new_n99_), .c(new_n98_), .d(x20), .O(new_n114_));
  nor2   g071(.a(x13), .b(new_n74_), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(new_n101_), .c(new_n73_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand3  g074(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n118_));
  nor4   g075(.a(new_n118_), .b(new_n104_), .c(new_n103_), .d(x14), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n117_), .c(new_n114_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(x00), .c(new_n72_), .O(z10));
  nand4  g078(.a(new_n108_), .b(x21), .c(new_n107_), .d(new_n63_), .O(new_n122_));
  nor2   g079(.a(new_n103_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n98_), .c(x20), .d(new_n104_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n116_), .c(new_n122_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n99_), .c(new_n77_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n72_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand4  g086(.a(new_n129_), .b(new_n72_), .c(x01), .d(x00), .O(new_n130_));
  nor2   g087(.a(x19), .b(x02), .O(new_n131_));
  oai21  g088(.a(new_n131_), .b(x08), .c(new_n44_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(x23), .O(new_n133_));
  nand3  g090(.a(new_n63_), .b(x17), .c(new_n46_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n81_), .c(new_n77_), .O(new_n136_));
  nand2  g093(.a(new_n136_), .b(new_n130_), .O(new_n137_));
  nand3  g094(.a(new_n137_), .b(new_n128_), .c(x09), .O(new_n138_));
  inv1   g095(.a(new_n138_), .O(z12));
  nor4   g096(.a(new_n62_), .b(x19), .c(new_n44_), .d(x02), .O(z13));
  nand4  g097(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z14));
  oai21  g099(.a(x10), .b(new_n81_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n46_), .c(new_n81_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n72_), .O(z15));
  nor3   g103(.a(new_n71_), .b(new_n81_), .c(x00), .O(z16));
  nand4  g104(.a(new_n72_), .b(x02), .c(new_n81_), .d(new_n77_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


