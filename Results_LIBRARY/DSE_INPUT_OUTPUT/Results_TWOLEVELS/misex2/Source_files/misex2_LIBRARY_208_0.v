// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_,
    new_n59_, new_n60_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n132_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n141_, new_n142_, new_n145_, new_n147_, new_n148_, new_n149_;
  inv1   g000(.a(x01), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  inv1   g003(.a(x10), .O(new_n47_));
  inv1   g004(.a(x17), .O(new_n48_));
  inv1   g005(.a(x18), .O(new_n49_));
  inv1   g006(.a(x19), .O(new_n50_));
  inv1   g007(.a(x05), .O(new_n51_));
  inv1   g008(.a(x21), .O(new_n52_));
  nand2  g009(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n54_));
  nor2   g011(.a(new_n54_), .b(new_n47_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x00), .O(z00));
  nor2   g014(.a(x01), .b(x00), .O(new_n58_));
  nand3  g015(.a(new_n58_), .b(x09), .c(new_n45_), .O(new_n59_));
  nand4  g016(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(x10), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n59_), .c(new_n53_), .O(z01));
  nor2   g018(.a(new_n54_), .b(x10), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(x09), .c(new_n45_), .d(new_n44_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x00), .O(z02));
  nor2   g021(.a(x21), .b(x05), .O(new_n65_));
  nand3  g022(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  inv1   g023(.a(new_n66_), .O(new_n67_));
  nor2   g024(.a(new_n47_), .b(x09), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(new_n67_), .c(x12), .d(x11), .O(new_n69_));
  nor3   g026(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  nor2   g027(.a(x19), .b(new_n49_), .O(new_n71_));
  nand3  g028(.a(new_n71_), .b(new_n70_), .c(new_n48_), .O(new_n72_));
  aoi21  g029(.a(new_n72_), .b(new_n69_), .c(new_n65_), .O(z03));
  inv1   g030(.a(x11), .O(new_n74_));
  inv1   g031(.a(x12), .O(new_n75_));
  nand3  g032(.a(new_n68_), .b(new_n75_), .c(new_n74_), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n66_), .c(new_n53_), .O(z04));
  nand2  g034(.a(x01), .b(x00), .O(new_n78_));
  nand3  g035(.a(x10), .b(x09), .c(x02), .O(new_n79_));
  oai21  g036(.a(new_n79_), .b(new_n78_), .c(new_n53_), .O(z05));
  nand2  g037(.a(new_n65_), .b(x12), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(x11), .c(x10), .d(new_n46_), .O(new_n82_));
  inv1   g039(.a(new_n82_), .O(new_n83_));
  nand4  g040(.a(new_n83_), .b(x02), .c(x01), .d(x00), .O(new_n84_));
  nand2  g041(.a(new_n84_), .b(new_n53_), .O(z06));
  nand3  g042(.a(new_n75_), .b(x11), .c(x10), .O(new_n86_));
  oai21  g043(.a(new_n86_), .b(x09), .c(x01), .O(new_n87_));
  nand3  g044(.a(new_n87_), .b(x02), .c(x00), .O(new_n88_));
  nand2  g045(.a(new_n88_), .b(new_n53_), .O(z07));
  inv1   g046(.a(new_n58_), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  nand3  g048(.a(x04), .b(new_n91_), .c(x02), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nor3   g050(.a(x08), .b(x07), .c(x06), .O(new_n94_));
  nor2   g051(.a(new_n50_), .b(x18), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(new_n94_), .c(new_n93_), .d(x17), .O(new_n96_));
  aoi21  g053(.a(new_n96_), .b(x21), .c(x05), .O(z08));
  inv1   g054(.a(x00), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nand4  g056(.a(x12), .b(new_n74_), .c(x02), .d(new_n44_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n50_), .c(x18), .d(x01), .O(new_n106_));
  oai21  g063(.a(new_n104_), .b(new_n100_), .c(new_n106_), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n99_), .c(new_n98_), .O(new_n108_));
  aoi21  g065(.a(new_n108_), .b(x05), .c(x21), .O(z09));
  nor2   g066(.a(new_n99_), .b(new_n52_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n71_), .c(new_n105_), .d(x01), .O(new_n111_));
  nor3   g068(.a(new_n51_), .b(new_n45_), .c(x01), .O(new_n112_));
  inv1   g069(.a(x13), .O(new_n113_));
  nand3  g070(.a(new_n113_), .b(x12), .c(new_n74_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nor3   g072(.a(new_n102_), .b(new_n101_), .c(x14), .O(new_n116_));
  nand3  g073(.a(new_n99_), .b(new_n52_), .c(x20), .O(new_n117_));
  inv1   g074(.a(new_n117_), .O(new_n118_));
  nand4  g075(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n112_), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n111_), .c(x00), .O(z10));
  nor2   g077(.a(new_n49_), .b(new_n44_), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(x21), .c(new_n105_), .d(new_n50_), .O(new_n122_));
  nand2  g079(.a(x02), .b(new_n44_), .O(new_n123_));
  nand3  g080(.a(x12), .b(new_n74_), .c(x05), .O(new_n124_));
  nor2   g081(.a(new_n124_), .b(new_n123_), .O(new_n125_));
  inv1   g082(.a(x14), .O(new_n126_));
  nand3  g083(.a(x15), .b(new_n126_), .c(new_n113_), .O(new_n127_));
  nand3  g084(.a(new_n52_), .b(x20), .c(new_n102_), .O(new_n128_));
  nor2   g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n122_), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(new_n99_), .c(new_n98_), .O(new_n132_));
  inv1   g089(.a(new_n132_), .O(z11));
  inv1   g090(.a(x24), .O(new_n134_));
  nand2  g091(.a(x10), .b(x02), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(x01), .c(x00), .O(new_n136_));
  oai21  g093(.a(x19), .b(x02), .c(x23), .O(new_n137_));
  nand3  g094(.a(new_n50_), .b(x17), .c(new_n45_), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n137_), .O(new_n139_));
  nand3  g096(.a(new_n139_), .b(new_n44_), .c(new_n98_), .O(new_n140_));
  nand2  g097(.a(new_n140_), .b(new_n136_), .O(new_n141_));
  nand4  g098(.a(new_n141_), .b(new_n53_), .c(new_n134_), .d(x09), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z12));
  oai21  g100(.a(new_n138_), .b(new_n90_), .c(new_n53_), .O(z13));
  nand4  g101(.a(new_n62_), .b(new_n46_), .c(new_n45_), .d(new_n44_), .O(new_n145_));
  nor2   g102(.a(new_n145_), .b(x00), .O(z14));
  oai21  g103(.a(x10), .b(new_n44_), .c(x02), .O(new_n147_));
  nand2  g104(.a(new_n147_), .b(x00), .O(new_n148_));
  nand3  g105(.a(x19), .b(new_n45_), .c(new_n44_), .O(new_n149_));
  aoi21  g106(.a(new_n149_), .b(new_n148_), .c(new_n65_), .O(z15));
  nor3   g107(.a(new_n65_), .b(new_n44_), .c(x00), .O(z16));
  oai21  g108(.a(new_n123_), .b(x00), .c(new_n53_), .O(z17));
endmodule


