// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n140_, new_n142_,
    new_n144_, new_n145_, new_n146_, new_n149_;
  nand2  g000(.a(x14), .b(x12), .O(new_n44_));
  inv1   g001(.a(x00), .O(new_n45_));
  inv1   g002(.a(x01), .O(new_n46_));
  inv1   g003(.a(x02), .O(new_n47_));
  inv1   g004(.a(x09), .O(new_n48_));
  nand4  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  inv1   g007(.a(x18), .O(new_n51_));
  inv1   g008(.a(x19), .O(new_n52_));
  nand4  g009(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g010(.a(new_n53_), .b(new_n49_), .c(new_n44_), .O(z00));
  nand4  g011(.a(x09), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n55_));
  oai21  g012(.a(new_n55_), .b(new_n53_), .c(new_n44_), .O(z01));
  aoi21  g013(.a(x14), .b(x12), .c(x19), .O(new_n57_));
  nand3  g014(.a(new_n57_), .b(new_n51_), .c(new_n50_), .O(new_n58_));
  nor2   g015(.a(new_n58_), .b(x10), .O(new_n59_));
  nand4  g016(.a(new_n59_), .b(x09), .c(new_n47_), .d(new_n46_), .O(new_n60_));
  nor2   g017(.a(new_n60_), .b(x00), .O(z02));
  nand3  g018(.a(new_n57_), .b(x18), .c(new_n50_), .O(new_n62_));
  inv1   g019(.a(new_n62_), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n64_));
  nor2   g021(.a(new_n46_), .b(new_n45_), .O(new_n65_));
  nor2   g022(.a(x09), .b(new_n47_), .O(new_n66_));
  and2   g023(.a(x11), .b(x10), .O(new_n67_));
  inv1   g024(.a(x12), .O(new_n68_));
  nor2   g025(.a(x14), .b(new_n68_), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n67_), .c(new_n66_), .d(new_n65_), .O(new_n70_));
  nand2  g027(.a(new_n70_), .b(new_n64_), .O(z03));
  nand3  g028(.a(x02), .b(x01), .c(x00), .O(new_n72_));
  inv1   g029(.a(x11), .O(new_n73_));
  nand4  g030(.a(new_n68_), .b(new_n73_), .c(x10), .d(new_n48_), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n72_), .c(new_n44_), .O(z04));
  inv1   g032(.a(new_n65_), .O(new_n76_));
  nand3  g033(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g034(.a(new_n77_), .b(new_n76_), .c(new_n44_), .O(z05));
  nand2  g035(.a(x14), .b(x12), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x11), .c(x10), .d(new_n48_), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(x02), .c(x01), .d(x00), .O(new_n82_));
  nand2  g039(.a(new_n82_), .b(new_n44_), .O(z06));
  nand3  g040(.a(new_n68_), .b(x11), .c(x10), .O(new_n84_));
  oai21  g041(.a(new_n84_), .b(x09), .c(x01), .O(new_n85_));
  nand3  g042(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g043(.a(new_n86_), .b(new_n44_), .O(z07));
  inv1   g044(.a(x03), .O(new_n88_));
  inv1   g045(.a(x04), .O(new_n89_));
  inv1   g046(.a(x05), .O(new_n90_));
  inv1   g047(.a(x06), .O(new_n91_));
  inv1   g048(.a(x07), .O(new_n92_));
  nand4  g049(.a(new_n44_), .b(x19), .c(new_n51_), .d(x17), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(x08), .O(new_n94_));
  nand4  g051(.a(new_n94_), .b(new_n92_), .c(new_n91_), .d(new_n90_), .O(new_n95_));
  nor2   g052(.a(new_n95_), .b(new_n89_), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n88_), .c(x02), .d(new_n46_), .O(new_n97_));
  nor2   g054(.a(new_n97_), .b(x00), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  nor2   g057(.a(new_n47_), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x12), .c(new_n73_), .O(new_n102_));
  inv1   g059(.a(x13), .O(new_n103_));
  inv1   g060(.a(x15), .O(new_n104_));
  inv1   g061(.a(x16), .O(new_n105_));
  nand4  g062(.a(x20), .b(new_n105_), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  inv1   g063(.a(x20), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n52_), .c(x18), .d(x01), .O(new_n108_));
  oai21  g065(.a(new_n106_), .b(new_n102_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n100_), .c(new_n99_), .d(new_n45_), .O(new_n110_));
  nand2  g067(.a(new_n110_), .b(new_n44_), .O(z09));
  nand4  g068(.a(new_n44_), .b(x22), .c(x21), .d(new_n107_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n52_), .c(x18), .d(x01), .O(new_n114_));
  nand3  g071(.a(new_n103_), .b(x12), .c(new_n73_), .O(new_n115_));
  inv1   g072(.a(new_n115_), .O(new_n116_));
  nor3   g073(.a(new_n105_), .b(new_n104_), .c(x14), .O(new_n117_));
  nand3  g074(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(new_n119_));
  nand4  g076(.a(new_n119_), .b(new_n117_), .c(new_n116_), .d(new_n101_), .O(new_n120_));
  aoi21  g077(.a(new_n120_), .b(new_n114_), .c(x00), .O(z10));
  nand4  g078(.a(new_n44_), .b(x21), .c(new_n107_), .d(new_n52_), .O(new_n122_));
  inv1   g079(.a(new_n122_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(x18), .c(x01), .O(new_n124_));
  nand3  g081(.a(new_n99_), .b(x20), .c(new_n105_), .O(new_n125_));
  nor3   g082(.a(new_n125_), .b(new_n104_), .c(x14), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n116_), .c(new_n101_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n124_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n100_), .c(new_n45_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  nand2  g087(.a(x10), .b(x02), .O(new_n131_));
  nand3  g088(.a(new_n131_), .b(x01), .c(x00), .O(new_n132_));
  oai21  g089(.a(x19), .b(x02), .c(x23), .O(new_n133_));
  nand3  g090(.a(new_n52_), .b(x17), .c(new_n47_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n133_), .O(new_n135_));
  nand3  g092(.a(new_n135_), .b(new_n46_), .c(new_n45_), .O(new_n136_));
  aoi21  g093(.a(new_n136_), .b(new_n132_), .c(x24), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(x09), .O(new_n138_));
  nand2  g095(.a(new_n138_), .b(new_n44_), .O(z12));
  nand4  g096(.a(new_n57_), .b(x17), .c(new_n47_), .d(new_n46_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x00), .O(z13));
  nand4  g098(.a(new_n59_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n142_));
  nor2   g099(.a(new_n142_), .b(x00), .O(z14));
  oai21  g100(.a(x10), .b(new_n46_), .c(x02), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(x00), .O(new_n145_));
  nand3  g102(.a(x19), .b(new_n47_), .c(new_n46_), .O(new_n146_));
  nand3  g103(.a(new_n146_), .b(new_n145_), .c(new_n44_), .O(z15));
  oai21  g104(.a(new_n46_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g105(.a(new_n101_), .b(new_n45_), .O(new_n149_));
  nand2  g106(.a(new_n149_), .b(new_n44_), .O(z17));
endmodule


