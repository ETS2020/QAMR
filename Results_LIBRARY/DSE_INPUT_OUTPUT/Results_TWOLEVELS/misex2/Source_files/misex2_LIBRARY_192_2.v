// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n76_, new_n78_,
    new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  nand3  g09(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g10(.a(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x01), .O(new_n61_));
  nor2   g18(.a(x09), .b(new_n46_), .O(new_n62_));
  nand3  g19(.a(x12), .b(x11), .c(x10), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(new_n64_));
  aoi21  g21(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  inv1   g22(.a(x19), .O(new_n66_));
  nor2   g23(.a(x02), .b(x01), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n66_), .c(x18), .d(new_n44_), .O(new_n68_));
  oai21  g25(.a(new_n65_), .b(new_n60_), .c(new_n68_), .O(z03));
  inv1   g26(.a(x11), .O(new_n70_));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n70_), .c(x10), .d(new_n47_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x12), .O(z04));
  nor3   g31(.a(new_n71_), .b(new_n57_), .c(new_n47_), .O(z05));
  nand4  g32(.a(x11), .b(x10), .c(new_n47_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n61_), .c(new_n60_), .O(z06));
  nor2   g34(.a(x12), .b(new_n70_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(new_n62_), .c(x10), .d(x01), .O(new_n79_));
  aoi21  g36(.a(new_n79_), .b(x01), .c(new_n60_), .O(z07));
  inv1   g37(.a(x08), .O(new_n81_));
  inv1   g38(.a(x05), .O(new_n82_));
  inv1   g39(.a(x06), .O(new_n83_));
  inv1   g40(.a(x03), .O(new_n84_));
  nand4  g41(.a(new_n84_), .b(x02), .c(new_n61_), .d(new_n60_), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(new_n83_), .c(new_n82_), .d(x04), .O(new_n87_));
  nor2   g44(.a(new_n87_), .b(x07), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n45_), .c(x17), .d(new_n81_), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(new_n66_), .O(z08));
  nor2   g47(.a(x14), .b(x13), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x12), .c(new_n70_), .d(x02), .O(new_n92_));
  nor2   g49(.a(x16), .b(x15), .O(new_n93_));
  nor2   g50(.a(x22), .b(x21), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(x20), .O(new_n95_));
  inv1   g52(.a(new_n95_), .O(new_n96_));
  nand2  g53(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  oai21  g54(.a(new_n97_), .b(new_n92_), .c(new_n60_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n61_), .O(new_n99_));
  nor3   g56(.a(new_n45_), .b(new_n61_), .c(x00), .O(new_n100_));
  nor2   g57(.a(x20), .b(x19), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n94_), .O(new_n102_));
  nand2  g59(.a(new_n102_), .b(new_n99_), .O(z09));
  nand3  g60(.a(new_n96_), .b(x16), .c(x15), .O(new_n104_));
  oai21  g61(.a(new_n104_), .b(new_n92_), .c(new_n60_), .O(new_n105_));
  nand2  g62(.a(new_n105_), .b(new_n61_), .O(new_n106_));
  nand4  g63(.a(new_n101_), .b(new_n100_), .c(x22), .d(x21), .O(new_n107_));
  nand2  g64(.a(new_n107_), .b(new_n106_), .O(z10));
  inv1   g65(.a(x16), .O(new_n109_));
  nand3  g66(.a(new_n96_), .b(new_n109_), .c(x15), .O(new_n110_));
  oai21  g67(.a(new_n110_), .b(new_n92_), .c(new_n60_), .O(new_n111_));
  nand2  g68(.a(new_n111_), .b(new_n61_), .O(new_n112_));
  inv1   g69(.a(x22), .O(new_n113_));
  nand4  g70(.a(new_n101_), .b(new_n100_), .c(new_n113_), .d(x21), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n112_), .O(z11));
  inv1   g72(.a(x24), .O(new_n116_));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand2  g74(.a(new_n117_), .b(x00), .O(new_n118_));
  nand2  g75(.a(x23), .b(x19), .O(new_n119_));
  oai21  g76(.a(x19), .b(new_n44_), .c(new_n119_), .O(new_n120_));
  and2   g77(.a(x23), .b(x02), .O(new_n121_));
  aoi21  g78(.a(new_n120_), .b(new_n46_), .c(new_n121_), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(x01), .c(new_n118_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n116_), .c(x09), .O(new_n124_));
  oai21  g81(.a(x01), .b(new_n60_), .c(new_n124_), .O(z12));
  nand3  g82(.a(new_n66_), .b(x17), .c(new_n46_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n60_), .c(x01), .O(z13));
  nand4  g84(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n128_));
  nor2   g85(.a(new_n128_), .b(x19), .O(z14));
  nand3  g86(.a(new_n117_), .b(x01), .c(x00), .O(new_n130_));
  nand3  g87(.a(new_n48_), .b(x19), .c(new_n46_), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n130_), .O(z15));
  nor2   g89(.a(new_n61_), .b(x00), .O(z16));
  aoi21  g90(.a(new_n46_), .b(new_n60_), .c(x01), .O(z17));
endmodule


