// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n99_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n112_, new_n115_, new_n116_, new_n118_, new_n119_,
    new_n121_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x10), .O(new_n46_));
  inv1   g03(.a(x00), .O(new_n47_));
  inv1   g04(.a(x01), .O(new_n48_));
  inv1   g05(.a(x02), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(new_n46_), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n53_));
  nor2   g10(.a(new_n53_), .b(x19), .O(z00));
  nand3  g11(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n55_));
  nor3   g12(.a(new_n55_), .b(new_n46_), .c(new_n50_), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(x19), .O(z01));
  inv1   g15(.a(x19), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n46_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x09), .c(new_n49_), .d(new_n47_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(x11), .c(x01), .O(z02));
  inv1   g20(.a(x11), .O(new_n64_));
  inv1   g21(.a(x12), .O(new_n65_));
  nor3   g22(.a(new_n65_), .b(new_n46_), .c(x09), .O(new_n66_));
  nand4  g23(.a(new_n66_), .b(x02), .c(x01), .d(x00), .O(new_n67_));
  nor3   g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n59_), .c(x18), .d(new_n44_), .O(new_n69_));
  aoi21  g26(.a(new_n69_), .b(new_n67_), .c(new_n64_), .O(z03));
  nand3  g27(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g28(.a(new_n71_), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n64_), .c(x10), .d(new_n50_), .O(new_n73_));
  nor2   g30(.a(new_n73_), .b(x12), .O(z04));
  nor3   g31(.a(new_n71_), .b(new_n46_), .c(new_n50_), .O(z05));
  nand4  g32(.a(x11), .b(x10), .c(new_n50_), .d(x02), .O(new_n76_));
  nor3   g33(.a(new_n76_), .b(new_n48_), .c(new_n47_), .O(z06));
  nand3  g34(.a(new_n65_), .b(x11), .c(x10), .O(new_n78_));
  oai21  g35(.a(new_n78_), .b(x09), .c(x01), .O(new_n79_));
  nand3  g36(.a(new_n79_), .b(x02), .c(x00), .O(new_n80_));
  nand2  g37(.a(new_n64_), .b(new_n48_), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n80_), .O(z07));
  nand2  g39(.a(x02), .b(new_n47_), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x04), .O(new_n85_));
  nor3   g42(.a(new_n85_), .b(new_n83_), .c(x03), .O(new_n86_));
  nor3   g43(.a(x08), .b(x07), .c(x06), .O(new_n87_));
  nor2   g44(.a(new_n59_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(x11), .c(x01), .O(z08));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nor2   g49(.a(new_n48_), .b(x00), .O(z16));
  nand3  g50(.a(z16), .b(new_n59_), .c(x18), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(x20), .O(new_n95_));
  nand3  g52(.a(new_n95_), .b(new_n92_), .c(new_n91_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(z09));
  nand3  g54(.a(x18), .b(x01), .c(new_n47_), .O(new_n98_));
  inv1   g55(.a(x20), .O(new_n99_));
  nand4  g56(.a(x22), .b(x21), .c(new_n99_), .d(new_n59_), .O(new_n100_));
  oai21  g57(.a(new_n100_), .b(new_n98_), .c(new_n81_), .O(z10));
  nand3  g58(.a(new_n95_), .b(new_n92_), .c(x21), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z11));
  inv1   g60(.a(x24), .O(new_n104_));
  nand2  g61(.a(x10), .b(x02), .O(new_n105_));
  nand3  g62(.a(new_n105_), .b(x01), .c(x00), .O(new_n106_));
  oai21  g63(.a(x19), .b(x02), .c(x23), .O(new_n107_));
  nand3  g64(.a(new_n59_), .b(x17), .c(new_n49_), .O(new_n108_));
  nand2  g65(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(x11), .c(new_n48_), .d(new_n47_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n106_), .O(new_n111_));
  nand3  g68(.a(new_n111_), .b(new_n104_), .c(x09), .O(new_n112_));
  inv1   g69(.a(new_n112_), .O(z12));
  nor4   g70(.a(new_n55_), .b(x19), .c(new_n44_), .d(new_n64_), .O(z13));
  nor2   g71(.a(new_n51_), .b(x10), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n45_), .c(new_n44_), .d(x11), .O(new_n116_));
  nor2   g73(.a(new_n116_), .b(x19), .O(z14));
  aoi21  g74(.a(new_n46_), .b(x01), .c(new_n49_), .O(new_n118_));
  aoi21  g75(.a(x19), .b(new_n49_), .c(new_n64_), .O(new_n119_));
  oai22  g76(.a(new_n119_), .b(x01), .c(new_n118_), .d(new_n47_), .O(z15));
  nand4  g77(.a(x11), .b(x02), .c(new_n48_), .d(new_n47_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z17));
endmodule


