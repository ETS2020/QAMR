// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n114_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n124_, new_n126_, new_n128_, new_n129_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(new_n47_), .b(x09), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  inv1   g12(.a(x01), .O(new_n56_));
  inv1   g13(.a(x02), .O(new_n57_));
  inv1   g14(.a(x10), .O(new_n58_));
  nor3   g15(.a(x19), .b(x18), .c(x17), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n58_), .c(x09), .d(new_n57_), .O(new_n60_));
  aoi21  g17(.a(new_n60_), .b(new_n56_), .c(x00), .O(z02));
  inv1   g18(.a(x00), .O(new_n62_));
  inv1   g19(.a(x19), .O(new_n63_));
  nand4  g20(.a(new_n63_), .b(x18), .c(new_n44_), .d(new_n57_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n56_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n62_), .O(new_n66_));
  nand2  g23(.a(new_n46_), .b(x02), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  inv1   g25(.a(x11), .O(new_n69_));
  inv1   g26(.a(x12), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n68_), .c(x10), .d(x01), .O(new_n72_));
  nand2  g29(.a(new_n72_), .b(new_n66_), .O(z03));
  nand4  g30(.a(new_n68_), .b(new_n70_), .c(new_n69_), .d(x10), .O(new_n74_));
  aoi21  g31(.a(new_n74_), .b(x00), .c(new_n56_), .O(z04));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  aoi21  g33(.a(new_n76_), .b(x00), .c(new_n56_), .O(z05));
  nand3  g34(.a(x02), .b(x01), .c(x00), .O(new_n78_));
  inv1   g35(.a(new_n78_), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x10), .d(new_n46_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(z06));
  nand3  g38(.a(new_n70_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(new_n67_), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n83_), .b(x01), .O(new_n84_));
  nand3  g41(.a(x02), .b(new_n56_), .c(x00), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(new_n84_), .O(z07));
  inv1   g43(.a(x08), .O(new_n87_));
  inv1   g44(.a(x05), .O(new_n88_));
  inv1   g45(.a(x06), .O(new_n89_));
  inv1   g46(.a(x03), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x02), .c(new_n56_), .d(new_n62_), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x07), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n63_), .O(z08));
  inv1   g53(.a(x16), .O(new_n97_));
  inv1   g54(.a(x21), .O(new_n98_));
  inv1   g55(.a(x13), .O(new_n99_));
  inv1   g56(.a(x14), .O(new_n100_));
  nand4  g57(.a(new_n69_), .b(x02), .c(new_n56_), .d(new_n62_), .O(new_n101_));
  inv1   g58(.a(new_n101_), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(x12), .O(new_n103_));
  nor2   g60(.a(new_n103_), .b(x15), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n98_), .c(x20), .d(new_n97_), .O(new_n105_));
  nor2   g62(.a(new_n105_), .b(x22), .O(z09));
  nand3  g63(.a(new_n100_), .b(new_n99_), .c(x12), .O(new_n107_));
  nor3   g64(.a(new_n107_), .b(x11), .c(new_n57_), .O(new_n108_));
  inv1   g65(.a(x15), .O(new_n109_));
  nor2   g66(.a(new_n97_), .b(new_n109_), .O(new_n110_));
  nor2   g67(.a(x22), .b(x21), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n110_), .c(new_n108_), .d(x20), .O(new_n112_));
  aoi21  g69(.a(new_n112_), .b(new_n56_), .c(x00), .O(z10));
  nor2   g70(.a(new_n103_), .b(new_n109_), .O(new_n114_));
  nand4  g71(.a(new_n114_), .b(new_n98_), .c(x20), .d(new_n97_), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(x22), .O(z11));
  inv1   g73(.a(x24), .O(new_n117_));
  aoi21  g74(.a(x10), .b(x02), .c(new_n56_), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n63_), .b(x17), .c(new_n57_), .O(new_n120_));
  aoi21  g77(.a(new_n120_), .b(new_n119_), .c(x00), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n118_), .c(new_n117_), .O(new_n122_));
  oai22  g79(.a(new_n122_), .b(new_n46_), .c(new_n56_), .d(x00), .O(z12));
  nand2  g80(.a(new_n47_), .b(x17), .O(new_n124_));
  nor2   g81(.a(new_n124_), .b(x19), .O(z13));
  nand4  g82(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(x19), .O(z14));
  aoi21  g84(.a(new_n58_), .b(x01), .c(new_n57_), .O(new_n128_));
  nand3  g85(.a(x19), .b(new_n57_), .c(new_n56_), .O(new_n129_));
  oai21  g86(.a(new_n128_), .b(new_n62_), .c(new_n129_), .O(z15));
  aoi21  g87(.a(new_n57_), .b(new_n56_), .c(x00), .O(z17));
  zero   g88(.O(z16));
endmodule


