// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:26 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n145_, new_n146_, new_n149_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(new_n49_), .O(new_n50_));
  nand4  g007(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g008(.a(new_n51_), .b(x19), .O(z00));
  nand3  g009(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n53_));
  inv1   g010(.a(new_n53_), .O(new_n54_));
  nand4  g011(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g012(.a(new_n55_), .b(x19), .O(z01));
  inv1   g013(.a(x14), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(x01), .O(new_n58_));
  nand3  g015(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n59_));
  inv1   g016(.a(x10), .O(new_n60_));
  inv1   g017(.a(x19), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n62_));
  oai21  g019(.a(new_n62_), .b(new_n59_), .c(new_n58_), .O(z02));
  inv1   g020(.a(x00), .O(new_n64_));
  inv1   g021(.a(x01), .O(new_n65_));
  nand3  g022(.a(new_n46_), .b(new_n65_), .c(new_n64_), .O(new_n66_));
  nand3  g023(.a(new_n61_), .b(x18), .c(new_n44_), .O(new_n67_));
  nand2  g024(.a(x01), .b(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n47_), .c(x02), .O(new_n70_));
  nand4  g027(.a(x14), .b(x12), .c(x11), .d(x10), .O(new_n71_));
  oai22  g028(.a(new_n71_), .b(new_n70_), .c(new_n67_), .d(new_n66_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nor3   g031(.a(new_n68_), .b(x09), .c(new_n46_), .O(new_n75_));
  nand4  g032(.a(new_n75_), .b(new_n74_), .c(new_n73_), .d(x10), .O(new_n76_));
  nor2   g033(.a(new_n76_), .b(new_n57_), .O(z04));
  nand4  g034(.a(x10), .b(x09), .c(x02), .d(x00), .O(new_n78_));
  aoi21  g035(.a(new_n78_), .b(x14), .c(new_n65_), .O(z05));
  nand3  g036(.a(x14), .b(x11), .c(x10), .O(new_n80_));
  inv1   g037(.a(new_n80_), .O(new_n81_));
  nand4  g038(.a(new_n81_), .b(new_n47_), .c(x02), .d(x01), .O(new_n82_));
  nor2   g039(.a(new_n82_), .b(new_n64_), .O(z06));
  nand2  g040(.a(x10), .b(new_n47_), .O(new_n84_));
  nand3  g041(.a(x14), .b(new_n74_), .c(x11), .O(new_n85_));
  oai21  g042(.a(new_n85_), .b(new_n84_), .c(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n58_), .O(z07));
  nor2   g045(.a(new_n46_), .b(x01), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(new_n64_), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  inv1   g048(.a(x05), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(x04), .c(new_n91_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nor3   g051(.a(x08), .b(x07), .c(x06), .O(new_n95_));
  nor2   g052(.a(new_n61_), .b(x18), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x17), .O(new_n97_));
  nand2  g054(.a(new_n97_), .b(new_n58_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  inv1   g057(.a(x13), .O(new_n101_));
  nand4  g058(.a(new_n101_), .b(x12), .c(new_n73_), .d(x02), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nand4  g061(.a(x20), .b(new_n104_), .c(new_n103_), .d(new_n57_), .O(new_n105_));
  nor2   g062(.a(x20), .b(x19), .O(new_n106_));
  nand3  g063(.a(new_n106_), .b(x18), .c(x01), .O(new_n107_));
  oai21  g064(.a(new_n105_), .b(new_n102_), .c(new_n107_), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n100_), .c(new_n99_), .d(new_n64_), .O(new_n109_));
  nand2  g066(.a(new_n109_), .b(new_n58_), .O(z09));
  nand3  g067(.a(x18), .b(x14), .c(x01), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nand4  g069(.a(new_n112_), .b(new_n106_), .c(x22), .d(x21), .O(new_n113_));
  nand3  g070(.a(new_n101_), .b(x12), .c(new_n73_), .O(new_n114_));
  inv1   g071(.a(new_n114_), .O(new_n115_));
  nor3   g072(.a(new_n104_), .b(new_n103_), .c(x14), .O(new_n116_));
  nand3  g073(.a(new_n100_), .b(new_n99_), .c(x20), .O(new_n117_));
  inv1   g074(.a(new_n117_), .O(new_n118_));
  nand4  g075(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(new_n89_), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n113_), .c(x00), .O(z10));
  nor2   g077(.a(x13), .b(new_n74_), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(new_n89_), .c(new_n73_), .O(new_n122_));
  inv1   g079(.a(x20), .O(new_n123_));
  nand4  g080(.a(new_n112_), .b(x21), .c(new_n123_), .d(new_n61_), .O(new_n124_));
  nor2   g081(.a(new_n103_), .b(x14), .O(new_n125_));
  nor2   g082(.a(x21), .b(new_n123_), .O(new_n126_));
  nand3  g083(.a(new_n126_), .b(new_n125_), .c(new_n104_), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(new_n122_), .c(new_n124_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n100_), .c(new_n64_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand4  g089(.a(new_n132_), .b(x14), .c(x01), .d(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n61_), .b(x17), .c(new_n46_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n65_), .c(new_n64_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  inv1   g096(.a(new_n139_), .O(z12));
  nand3  g097(.a(new_n48_), .b(x17), .c(new_n46_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z13));
  nand3  g099(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n143_));
  oai21  g100(.a(new_n143_), .b(new_n62_), .c(new_n58_), .O(z14));
  aoi22  g101(.a(new_n58_), .b(x00), .c(x19), .d(new_n65_), .O(new_n145_));
  nand3  g102(.a(new_n69_), .b(x14), .c(new_n60_), .O(new_n146_));
  oai21  g103(.a(new_n145_), .b(x02), .c(new_n146_), .O(z15));
  aoi21  g104(.a(x14), .b(x00), .c(new_n65_), .O(z16));
  nand2  g105(.a(new_n48_), .b(x02), .O(new_n149_));
  inv1   g106(.a(new_n149_), .O(z17));
endmodule


