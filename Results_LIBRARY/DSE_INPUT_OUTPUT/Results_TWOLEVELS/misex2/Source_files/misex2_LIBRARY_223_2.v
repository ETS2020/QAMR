// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n124_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n136_;
  inv1   g00(.a(x01), .O(new_n44_));
  inv1   g01(.a(x17), .O(new_n45_));
  inv1   g02(.a(x18), .O(new_n46_));
  nor3   g03(.a(x09), .b(x02), .c(x00), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x19), .O(z00));
  inv1   g06(.a(x00), .O(new_n50_));
  inv1   g07(.a(x02), .O(new_n51_));
  nand4  g08(.a(x09), .b(new_n51_), .c(new_n44_), .d(new_n50_), .O(new_n52_));
  inv1   g09(.a(new_n52_), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n46_), .c(new_n45_), .d(x10), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(x19), .O(z01));
  nor3   g12(.a(x18), .b(x17), .c(x10), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(x09), .c(new_n51_), .d(new_n50_), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n44_), .c(x19), .O(z02));
  inv1   g15(.a(x09), .O(new_n59_));
  nand3  g16(.a(new_n59_), .b(x02), .c(x00), .O(new_n60_));
  nand4  g17(.a(x19), .b(x12), .c(x11), .d(x10), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(new_n60_), .c(x19), .O(new_n62_));
  nand2  g19(.a(new_n62_), .b(x01), .O(new_n63_));
  inv1   g20(.a(x19), .O(new_n64_));
  nor2   g21(.a(x02), .b(x00), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n64_), .c(x18), .d(new_n45_), .O(new_n66_));
  nand2  g23(.a(new_n66_), .b(new_n63_), .O(z03));
  inv1   g24(.a(x11), .O(new_n68_));
  inv1   g25(.a(x12), .O(new_n69_));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x09), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n69_), .c(new_n68_), .d(x10), .O(new_n72_));
  nor2   g29(.a(new_n72_), .b(new_n64_), .O(z04));
  inv1   g30(.a(x10), .O(new_n74_));
  nor4   g31(.a(new_n70_), .b(new_n64_), .c(new_n74_), .d(new_n59_), .O(z05));
  nand3  g32(.a(x19), .b(x11), .c(x10), .O(new_n76_));
  inv1   g33(.a(new_n76_), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(new_n59_), .c(x02), .d(x01), .O(new_n78_));
  nor2   g35(.a(new_n78_), .b(new_n50_), .O(z06));
  nand2  g36(.a(x10), .b(new_n59_), .O(new_n80_));
  nand3  g37(.a(x19), .b(new_n69_), .c(x11), .O(new_n81_));
  oai21  g38(.a(new_n81_), .b(new_n80_), .c(x01), .O(new_n82_));
  nand3  g39(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g40(.a(new_n64_), .b(x01), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(new_n83_), .O(z07));
  inv1   g42(.a(x08), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  inv1   g44(.a(x06), .O(new_n88_));
  inv1   g45(.a(x03), .O(new_n89_));
  nand4  g46(.a(new_n89_), .b(x02), .c(new_n44_), .d(new_n50_), .O(new_n90_));
  inv1   g47(.a(new_n90_), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(new_n88_), .c(new_n87_), .d(x04), .O(new_n92_));
  nor2   g49(.a(new_n92_), .b(x07), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n46_), .c(x17), .d(new_n86_), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(new_n64_), .O(z08));
  nor2   g52(.a(new_n51_), .b(x01), .O(new_n96_));
  nor2   g53(.a(x13), .b(new_n69_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n96_), .c(new_n68_), .d(new_n50_), .O(new_n98_));
  inv1   g55(.a(x14), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  inv1   g58(.a(x21), .O(new_n102_));
  inv1   g59(.a(x22), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(new_n102_), .c(x20), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n101_), .c(new_n100_), .d(new_n99_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n98_), .c(new_n84_), .O(z09));
  inv1   g64(.a(x13), .O(new_n108_));
  nand4  g65(.a(new_n68_), .b(x02), .c(new_n44_), .d(new_n50_), .O(new_n109_));
  inv1   g66(.a(new_n109_), .O(new_n110_));
  nand4  g67(.a(new_n110_), .b(new_n99_), .c(new_n108_), .d(x12), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n100_), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n102_), .c(x20), .d(x16), .O(new_n113_));
  nor2   g70(.a(new_n113_), .b(x22), .O(z10));
  nand4  g71(.a(new_n105_), .b(new_n101_), .c(x15), .d(new_n99_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(new_n98_), .c(new_n84_), .O(z11));
  nand2  g73(.a(x10), .b(x02), .O(new_n117_));
  nand3  g74(.a(new_n117_), .b(x01), .c(x00), .O(new_n118_));
  oai21  g75(.a(x19), .b(x02), .c(x23), .O(new_n119_));
  nand3  g76(.a(new_n64_), .b(x17), .c(new_n51_), .O(new_n120_));
  oai21  g77(.a(new_n119_), .b(x01), .c(new_n120_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n50_), .O(new_n122_));
  aoi21  g79(.a(new_n122_), .b(new_n118_), .c(x24), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(x09), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(new_n84_), .O(z12));
  nand3  g82(.a(x17), .b(new_n51_), .c(new_n50_), .O(new_n126_));
  aoi21  g83(.a(new_n126_), .b(new_n44_), .c(x19), .O(z13));
  nand2  g84(.a(new_n56_), .b(new_n47_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n44_), .c(x19), .O(z14));
  oai21  g86(.a(x10), .b(new_n44_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n51_), .c(new_n44_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n84_), .O(z15));
  nand3  g90(.a(x19), .b(x01), .c(new_n50_), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z16));
  nand3  g92(.a(x02), .b(new_n44_), .c(new_n50_), .O(new_n136_));
  inv1   g93(.a(new_n136_), .O(z17));
endmodule


