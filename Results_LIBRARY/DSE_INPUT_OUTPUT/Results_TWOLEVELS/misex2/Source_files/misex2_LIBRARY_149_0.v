// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:25 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n74_, new_n75_, new_n77_, new_n79_, new_n80_, new_n82_;
  inv1   g00(.a(x00), .O(z16));
  inv1   g01(.a(x01), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(z16), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(z00));
  nand3  g04(.a(x02), .b(x01), .c(x00), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(x12), .b(x11), .c(x10), .d(new_n49_), .O(new_n50_));
  oai21  g07(.a(new_n50_), .b(new_n48_), .c(new_n46_), .O(z03));
  inv1   g08(.a(x11), .O(new_n52_));
  inv1   g09(.a(x12), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(x10), .d(new_n49_), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n48_), .c(new_n46_), .O(z04));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n56_), .O(z05));
  nand4  g15(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n59_));
  nor3   g16(.a(new_n59_), .b(new_n45_), .c(z16), .O(z06));
  nand3  g17(.a(new_n53_), .b(x11), .c(x10), .O(new_n61_));
  oai21  g18(.a(new_n61_), .b(x09), .c(x01), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x02), .c(x00), .O(new_n63_));
  inv1   g20(.a(new_n63_), .O(z07));
  inv1   g21(.a(x20), .O(new_n66_));
  inv1   g22(.a(x21), .O(new_n67_));
  inv1   g23(.a(x22), .O(new_n68_));
  inv1   g24(.a(x19), .O(new_n69_));
  nand4  g25(.a(new_n69_), .b(x18), .c(x01), .d(z16), .O(new_n70_));
  inv1   g26(.a(new_n70_), .O(new_n71_));
  nand4  g27(.a(new_n71_), .b(new_n68_), .c(new_n67_), .d(new_n66_), .O(new_n72_));
  inv1   g28(.a(new_n72_), .O(z09));
  nor2   g29(.a(new_n68_), .b(new_n67_), .O(new_n74_));
  nand4  g30(.a(new_n74_), .b(new_n66_), .c(new_n69_), .d(x18), .O(new_n75_));
  aoi21  g31(.a(new_n75_), .b(x01), .c(x00), .O(z10));
  nand4  g32(.a(new_n71_), .b(new_n68_), .c(x21), .d(new_n66_), .O(new_n77_));
  inv1   g33(.a(new_n77_), .O(z11));
  aoi21  g34(.a(x10), .b(x02), .c(x24), .O(new_n79_));
  nand3  g35(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nor2   g36(.a(new_n80_), .b(z16), .O(z12));
  nand2  g37(.a(new_n56_), .b(x01), .O(new_n82_));
  aoi21  g38(.a(new_n82_), .b(x02), .c(z16), .O(z15));
  zero   g39(.O(z08));
  zero   g40(.O(z17));
  inv1   g41(.a(new_n46_), .O(z01));
  inv1   g42(.a(new_n46_), .O(z02));
  inv1   g43(.a(new_n46_), .O(z13));
  inv1   g44(.a(new_n46_), .O(z14));
endmodule


