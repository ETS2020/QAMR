// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_,
    new_n52_, new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n135_,
    new_n137_, new_n139_, new_n140_, new_n143_;
  nor3   g000(.a(x09), .b(x02), .c(x00), .O(new_n44_));
  inv1   g001(.a(x17), .O(new_n45_));
  nor2   g002(.a(x19), .b(x18), .O(new_n46_));
  nand3  g003(.a(new_n46_), .b(new_n45_), .c(x10), .O(new_n47_));
  inv1   g004(.a(new_n47_), .O(new_n48_));
  nand2  g005(.a(new_n48_), .b(new_n44_), .O(new_n49_));
  aoi21  g006(.a(new_n49_), .b(x23), .c(x01), .O(z00));
  inv1   g007(.a(x00), .O(new_n51_));
  inv1   g008(.a(x02), .O(new_n52_));
  nand4  g009(.a(new_n48_), .b(x09), .c(new_n52_), .d(new_n51_), .O(new_n53_));
  aoi21  g010(.a(new_n53_), .b(x23), .c(x01), .O(z01));
  inv1   g011(.a(x23), .O(new_n55_));
  inv1   g012(.a(x18), .O(new_n56_));
  inv1   g013(.a(x19), .O(new_n57_));
  inv1   g014(.a(x01), .O(new_n58_));
  nand4  g015(.a(x09), .b(new_n52_), .c(new_n58_), .d(new_n51_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(new_n57_), .c(new_n56_), .d(new_n45_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(new_n55_), .O(z02));
  nand3  g019(.a(new_n52_), .b(new_n58_), .c(new_n51_), .O(new_n63_));
  nand4  g020(.a(x23), .b(new_n57_), .c(x18), .d(new_n45_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(x09), .O(new_n66_));
  nand4  g023(.a(x12), .b(x11), .c(x10), .d(new_n66_), .O(new_n67_));
  oai22  g024(.a(new_n67_), .b(new_n65_), .c(new_n64_), .d(new_n63_), .O(z03));
  inv1   g025(.a(x11), .O(new_n69_));
  nand3  g026(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g027(.a(new_n70_), .O(new_n71_));
  nand4  g028(.a(new_n71_), .b(new_n69_), .c(x10), .d(new_n66_), .O(new_n72_));
  nor2   g029(.a(new_n72_), .b(x12), .O(z04));
  nand2  g030(.a(x01), .b(x00), .O(new_n74_));
  nand2  g031(.a(new_n55_), .b(new_n58_), .O(new_n75_));
  nand3  g032(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g033(.a(new_n76_), .b(new_n74_), .c(new_n75_), .O(z05));
  inv1   g034(.a(x10), .O(new_n78_));
  nor3   g035(.a(new_n69_), .b(new_n78_), .c(x09), .O(new_n79_));
  nand4  g036(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  nand2  g037(.a(new_n80_), .b(new_n75_), .O(z06));
  inv1   g038(.a(x12), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g040(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand3  g041(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g042(.a(new_n85_), .b(new_n75_), .O(z07));
  inv1   g043(.a(x05), .O(new_n87_));
  inv1   g044(.a(x06), .O(new_n88_));
  inv1   g045(.a(x07), .O(new_n89_));
  inv1   g046(.a(x04), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(x02), .c(new_n58_), .d(new_n51_), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(new_n90_), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(new_n87_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x08), .O(new_n95_));
  nand4  g052(.a(new_n95_), .b(x19), .c(new_n56_), .d(x17), .O(new_n96_));
  nor2   g053(.a(new_n96_), .b(new_n55_), .O(z08));
  inv1   g054(.a(x21), .O(new_n98_));
  inv1   g055(.a(x22), .O(new_n99_));
  nand2  g056(.a(x02), .b(new_n58_), .O(new_n100_));
  inv1   g057(.a(new_n100_), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(x12), .c(new_n69_), .O(new_n102_));
  inv1   g059(.a(x15), .O(new_n103_));
  inv1   g060(.a(x16), .O(new_n104_));
  nor2   g061(.a(x14), .b(x13), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(x20), .c(new_n104_), .d(new_n103_), .O(new_n106_));
  inv1   g063(.a(x20), .O(new_n107_));
  nor2   g064(.a(new_n56_), .b(new_n58_), .O(new_n108_));
  nand3  g065(.a(new_n108_), .b(new_n107_), .c(new_n57_), .O(new_n109_));
  oai21  g066(.a(new_n106_), .b(new_n102_), .c(new_n109_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n99_), .c(new_n98_), .d(new_n51_), .O(new_n111_));
  nand2  g068(.a(new_n111_), .b(new_n75_), .O(z09));
  nand3  g069(.a(new_n57_), .b(x18), .c(x01), .O(new_n113_));
  nor4   g070(.a(new_n113_), .b(new_n99_), .c(new_n98_), .d(x20), .O(new_n114_));
  nor2   g071(.a(x13), .b(new_n82_), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(new_n101_), .c(new_n69_), .O(new_n116_));
  inv1   g073(.a(new_n116_), .O(new_n117_));
  nand3  g074(.a(new_n99_), .b(new_n98_), .c(x20), .O(new_n118_));
  nor4   g075(.a(new_n118_), .b(new_n104_), .c(new_n103_), .d(x14), .O(new_n119_));
  aoi21  g076(.a(new_n119_), .b(new_n117_), .c(new_n114_), .O(new_n120_));
  oai21  g077(.a(new_n120_), .b(x00), .c(new_n75_), .O(z10));
  nand4  g078(.a(new_n108_), .b(x21), .c(new_n107_), .d(new_n57_), .O(new_n122_));
  nor2   g079(.a(new_n103_), .b(x14), .O(new_n123_));
  nand4  g080(.a(new_n123_), .b(new_n98_), .c(x20), .d(new_n104_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n116_), .c(new_n122_), .O(new_n125_));
  nand3  g082(.a(new_n125_), .b(new_n99_), .c(new_n51_), .O(new_n126_));
  nand2  g083(.a(new_n126_), .b(new_n75_), .O(z11));
  aoi21  g084(.a(x10), .b(x02), .c(new_n58_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(x00), .O(new_n129_));
  nand3  g086(.a(new_n57_), .b(new_n45_), .c(new_n52_), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(new_n58_), .c(new_n51_), .O(new_n131_));
  aoi21  g088(.a(new_n131_), .b(new_n129_), .c(x24), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(x09), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n75_), .O(z12));
  nand4  g091(.a(new_n57_), .b(x17), .c(new_n52_), .d(new_n51_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(x23), .c(x01), .O(z13));
  nand4  g093(.a(new_n46_), .b(new_n44_), .c(new_n45_), .d(new_n78_), .O(new_n137_));
  aoi21  g094(.a(new_n137_), .b(x23), .c(x01), .O(z14));
  aoi21  g095(.a(x23), .b(new_n52_), .c(new_n128_), .O(new_n139_));
  nand4  g096(.a(x23), .b(x19), .c(new_n52_), .d(new_n58_), .O(new_n140_));
  oai21  g097(.a(new_n139_), .b(new_n51_), .c(new_n140_), .O(z15));
  oai21  g098(.a(new_n58_), .b(x00), .c(new_n75_), .O(z16));
  nand2  g099(.a(x02), .b(new_n51_), .O(new_n143_));
  aoi21  g100(.a(new_n143_), .b(x23), .c(x01), .O(z17));
endmodule


