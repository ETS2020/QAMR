// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n139_, new_n140_, new_n142_,
    new_n143_, new_n144_, new_n146_, new_n147_, new_n148_, new_n151_;
  inv1   g000(.a(x11), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x09), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(x17), .O(new_n51_));
  inv1   g008(.a(x18), .O(new_n52_));
  inv1   g009(.a(x19), .O(new_n53_));
  nand4  g010(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g012(.a(new_n54_), .O(new_n56_));
  nor3   g013(.a(x02), .b(x01), .c(x00), .O(new_n57_));
  nand2  g014(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  aoi21  g015(.a(new_n58_), .b(x11), .c(new_n49_), .O(z01));
  nand3  g016(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n60_));
  nor3   g017(.a(new_n60_), .b(x10), .c(new_n49_), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(new_n52_), .c(new_n51_), .d(x11), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x19), .O(z02));
  nand4  g020(.a(new_n57_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  inv1   g023(.a(x12), .O(new_n67_));
  nor2   g024(.a(new_n67_), .b(new_n44_), .O(new_n68_));
  nand4  g025(.a(new_n68_), .b(new_n66_), .c(x10), .d(new_n49_), .O(new_n69_));
  nand3  g026(.a(new_n69_), .b(new_n64_), .c(new_n45_), .O(z03));
  nand3  g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  inv1   g028(.a(new_n71_), .O(new_n72_));
  nand4  g029(.a(new_n72_), .b(new_n44_), .c(x10), .d(new_n49_), .O(new_n73_));
  nor2   g030(.a(new_n73_), .b(x12), .O(z04));
  nand4  g031(.a(x10), .b(x02), .c(x01), .d(x00), .O(new_n75_));
  aoi21  g032(.a(new_n75_), .b(x11), .c(new_n49_), .O(z05));
  inv1   g033(.a(x10), .O(new_n77_));
  nor3   g034(.a(new_n44_), .b(new_n77_), .c(x09), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x02), .c(x01), .d(x00), .O(new_n79_));
  nand2  g036(.a(new_n79_), .b(new_n45_), .O(z06));
  nand3  g037(.a(new_n67_), .b(x11), .c(x10), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n45_), .O(z07));
  inv1   g041(.a(x03), .O(new_n85_));
  inv1   g042(.a(x04), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  inv1   g044(.a(x06), .O(new_n88_));
  inv1   g045(.a(x07), .O(new_n89_));
  nand4  g046(.a(new_n45_), .b(x19), .c(new_n52_), .d(x17), .O(new_n90_));
  nor2   g047(.a(new_n90_), .b(x08), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n89_), .c(new_n88_), .d(new_n87_), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(new_n86_), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n85_), .c(x02), .d(new_n47_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(x00), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  nor2   g054(.a(new_n48_), .b(x01), .O(new_n98_));
  nor2   g055(.a(new_n67_), .b(x11), .O(new_n99_));
  nand2  g056(.a(new_n99_), .b(new_n98_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nor2   g059(.a(x14), .b(x13), .O(new_n103_));
  nand4  g060(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g061(.a(x20), .O(new_n105_));
  nand4  g062(.a(new_n105_), .b(new_n53_), .c(x18), .d(x01), .O(new_n106_));
  oai21  g063(.a(new_n104_), .b(new_n100_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n97_), .c(new_n96_), .d(new_n46_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n45_), .O(z09));
  nand4  g066(.a(new_n45_), .b(x22), .c(x21), .d(new_n105_), .O(new_n110_));
  inv1   g067(.a(new_n110_), .O(new_n111_));
  nand4  g068(.a(new_n111_), .b(new_n53_), .c(x18), .d(x01), .O(new_n112_));
  nand3  g069(.a(new_n49_), .b(x02), .c(new_n47_), .O(new_n113_));
  nor4   g070(.a(new_n113_), .b(x13), .c(new_n67_), .d(x11), .O(new_n114_));
  nor3   g071(.a(new_n102_), .b(new_n101_), .c(x14), .O(new_n115_));
  nor2   g072(.a(x22), .b(x21), .O(new_n116_));
  nand4  g073(.a(new_n116_), .b(new_n115_), .c(new_n114_), .d(x20), .O(new_n117_));
  aoi21  g074(.a(new_n117_), .b(new_n112_), .c(x00), .O(z10));
  inv1   g075(.a(new_n45_), .O(new_n119_));
  nor2   g076(.a(new_n119_), .b(new_n96_), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n105_), .c(new_n53_), .d(x18), .O(new_n121_));
  inv1   g078(.a(x14), .O(new_n122_));
  nand2  g079(.a(x15), .b(new_n122_), .O(new_n123_));
  nand3  g080(.a(new_n96_), .b(x20), .c(new_n102_), .O(new_n124_));
  nor3   g081(.a(new_n124_), .b(new_n123_), .c(x13), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n99_), .c(new_n98_), .d(new_n49_), .O(new_n126_));
  oai21  g083(.a(new_n121_), .b(new_n47_), .c(new_n126_), .O(new_n127_));
  nand3  g084(.a(new_n127_), .b(new_n97_), .c(new_n46_), .O(new_n128_));
  inv1   g085(.a(new_n128_), .O(z11));
  nand2  g086(.a(x10), .b(x02), .O(new_n130_));
  nand3  g087(.a(new_n130_), .b(x01), .c(x00), .O(new_n131_));
  oai21  g088(.a(x19), .b(x02), .c(x23), .O(new_n132_));
  nand3  g089(.a(new_n53_), .b(x17), .c(new_n48_), .O(new_n133_));
  nand2  g090(.a(new_n133_), .b(new_n132_), .O(new_n134_));
  nand3  g091(.a(new_n134_), .b(new_n47_), .c(new_n46_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n131_), .c(x24), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(x11), .c(x09), .O(new_n137_));
  inv1   g094(.a(new_n137_), .O(z12));
  nor2   g095(.a(new_n119_), .b(x19), .O(new_n139_));
  nand4  g096(.a(new_n139_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(x00), .O(z13));
  or2    g098(.a(new_n60_), .b(x09), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(new_n143_));
  nand4  g100(.a(new_n143_), .b(new_n52_), .c(new_n51_), .d(new_n77_), .O(new_n144_));
  nor2   g101(.a(new_n144_), .b(x19), .O(z14));
  oai21  g102(.a(x10), .b(new_n47_), .c(x02), .O(new_n146_));
  nand2  g103(.a(new_n146_), .b(x00), .O(new_n147_));
  nand3  g104(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n148_));
  nand3  g105(.a(new_n148_), .b(new_n147_), .c(new_n45_), .O(z15));
  nor3   g106(.a(new_n119_), .b(new_n47_), .c(x00), .O(z16));
  nand4  g107(.a(new_n45_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n151_));
  inv1   g108(.a(new_n151_), .O(z17));
endmodule


