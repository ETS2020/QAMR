// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n61_, new_n62_, new_n63_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n71_, new_n72_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n77_, new_n79_, new_n80_, new_n81_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n89_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x01), .O(new_n45_));
  nand2  g02(.a(new_n45_), .b(new_n44_), .O(new_n46_));
  inv1   g03(.a(new_n46_), .O(z00));
  nand3  g04(.a(x02), .b(x01), .c(x00), .O(new_n50_));
  inv1   g05(.a(x09), .O(new_n51_));
  nand4  g06(.a(x12), .b(x11), .c(x10), .d(new_n51_), .O(new_n52_));
  oai21  g07(.a(new_n52_), .b(new_n50_), .c(new_n46_), .O(z03));
  nand3  g08(.a(x02), .b(x01), .c(x00), .O(new_n54_));
  inv1   g09(.a(new_n54_), .O(new_n55_));
  nand3  g10(.a(new_n55_), .b(x10), .c(new_n51_), .O(new_n56_));
  nor3   g11(.a(new_n56_), .b(x12), .c(x11), .O(z04));
  nand2  g12(.a(x01), .b(x00), .O(new_n58_));
  nand3  g13(.a(x10), .b(x09), .c(x02), .O(new_n59_));
  oai21  g14(.a(new_n59_), .b(new_n58_), .c(new_n46_), .O(z05));
  nand2  g15(.a(x11), .b(x10), .O(new_n61_));
  nor2   g16(.a(new_n61_), .b(x09), .O(new_n62_));
  nand4  g17(.a(new_n62_), .b(x02), .c(x01), .d(x00), .O(new_n63_));
  nand2  g18(.a(new_n63_), .b(new_n46_), .O(z06));
  inv1   g19(.a(x12), .O(new_n65_));
  nand3  g20(.a(new_n65_), .b(x11), .c(x10), .O(new_n66_));
  oai21  g21(.a(new_n66_), .b(x09), .c(x01), .O(new_n67_));
  nand3  g22(.a(new_n67_), .b(x02), .c(x00), .O(new_n68_));
  inv1   g23(.a(new_n68_), .O(z07));
  inv1   g24(.a(x20), .O(new_n71_));
  inv1   g25(.a(x21), .O(new_n72_));
  inv1   g26(.a(x22), .O(new_n73_));
  inv1   g27(.a(x19), .O(new_n74_));
  nand4  g28(.a(new_n74_), .b(x18), .c(x01), .d(new_n44_), .O(new_n75_));
  inv1   g29(.a(new_n75_), .O(new_n76_));
  nand4  g30(.a(new_n76_), .b(new_n73_), .c(new_n72_), .d(new_n71_), .O(new_n77_));
  inv1   g31(.a(new_n77_), .O(z09));
  inv1   g32(.a(x18), .O(new_n79_));
  nor2   g33(.a(x19), .b(new_n79_), .O(new_n80_));
  nand4  g34(.a(new_n80_), .b(x22), .c(x21), .d(new_n71_), .O(new_n81_));
  aoi21  g35(.a(new_n81_), .b(x01), .c(x00), .O(z10));
  nand4  g36(.a(new_n80_), .b(new_n73_), .c(x21), .d(new_n71_), .O(new_n83_));
  aoi21  g37(.a(new_n83_), .b(x01), .c(x00), .O(z11));
  aoi21  g38(.a(x10), .b(x02), .c(x24), .O(new_n85_));
  nand4  g39(.a(new_n85_), .b(x09), .c(x01), .d(x00), .O(new_n86_));
  nand2  g40(.a(new_n86_), .b(new_n46_), .O(z12));
  inv1   g41(.a(x10), .O(new_n88_));
  nand2  g42(.a(new_n88_), .b(x01), .O(new_n89_));
  aoi21  g43(.a(new_n89_), .b(x02), .c(new_n44_), .O(z15));
  nor2   g44(.a(new_n45_), .b(x00), .O(z16));
  zero   g45(.O(z01));
  zero   g46(.O(z02));
  zero   g47(.O(z08));
  zero   g48(.O(z17));
  inv1   g49(.a(new_n46_), .O(z13));
  inv1   g50(.a(new_n46_), .O(z14));
endmodule


