// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n72_, new_n74_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n123_,
    new_n125_, new_n126_, new_n127_;
  inv1   g00(.a(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x19), .b(x18), .c(x17), .O(new_n47_));
  nand4  g04(.a(new_n47_), .b(x10), .c(new_n46_), .d(new_n45_), .O(new_n48_));
  aoi21  g05(.a(new_n48_), .b(new_n44_), .c(x01), .O(z00));
  nand4  g06(.a(new_n47_), .b(x10), .c(x09), .d(new_n45_), .O(new_n50_));
  aoi21  g07(.a(new_n50_), .b(new_n44_), .c(x01), .O(z01));
  inv1   g08(.a(x10), .O(new_n52_));
  inv1   g09(.a(x17), .O(new_n53_));
  inv1   g10(.a(x18), .O(new_n54_));
  nor2   g11(.a(x01), .b(x00), .O(new_n55_));
  nand3  g12(.a(new_n55_), .b(x09), .c(new_n45_), .O(new_n56_));
  inv1   g13(.a(new_n56_), .O(new_n57_));
  nand4  g14(.a(new_n57_), .b(new_n54_), .c(new_n53_), .d(new_n52_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z02));
  inv1   g16(.a(x01), .O(new_n60_));
  nand3  g17(.a(new_n45_), .b(new_n60_), .c(new_n44_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand3  g19(.a(new_n62_), .b(x18), .c(new_n53_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n46_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x10), .c(new_n46_), .O(new_n69_));
  nor3   g26(.a(new_n69_), .b(x12), .c(x11), .O(z04));
  nor3   g27(.a(new_n67_), .b(new_n52_), .c(new_n46_), .O(z05));
  nand4  g28(.a(x11), .b(x10), .c(new_n46_), .d(x02), .O(new_n72_));
  nor3   g29(.a(new_n72_), .b(new_n60_), .c(new_n44_), .O(z06));
  inv1   g30(.a(x11), .O(new_n74_));
  nor3   g31(.a(new_n69_), .b(x12), .c(new_n74_), .O(z07));
  inv1   g32(.a(x08), .O(new_n76_));
  inv1   g33(.a(x05), .O(new_n77_));
  inv1   g34(.a(x06), .O(new_n78_));
  inv1   g35(.a(x03), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x02), .c(new_n60_), .d(new_n44_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(new_n78_), .c(new_n77_), .d(x04), .O(new_n82_));
  nor2   g39(.a(new_n82_), .b(x07), .O(new_n83_));
  nand4  g40(.a(new_n83_), .b(new_n54_), .c(x17), .d(new_n76_), .O(new_n84_));
  nor2   g41(.a(new_n84_), .b(new_n62_), .O(z08));
  nor2   g42(.a(x14), .b(x13), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x12), .c(new_n74_), .d(x02), .O(new_n87_));
  inv1   g44(.a(x15), .O(new_n88_));
  inv1   g45(.a(x16), .O(new_n89_));
  nor2   g46(.a(x22), .b(x21), .O(new_n90_));
  nand2  g47(.a(new_n90_), .b(x20), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand3  g49(.a(new_n92_), .b(new_n89_), .c(new_n88_), .O(new_n93_));
  oai21  g50(.a(new_n93_), .b(new_n87_), .c(new_n44_), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n60_), .O(new_n95_));
  nor3   g52(.a(new_n54_), .b(new_n60_), .c(x00), .O(new_n96_));
  nor2   g53(.a(x20), .b(x19), .O(new_n97_));
  nand3  g54(.a(new_n97_), .b(new_n96_), .c(new_n90_), .O(new_n98_));
  nand2  g55(.a(new_n98_), .b(new_n95_), .O(z09));
  nand2  g56(.a(x22), .b(x21), .O(new_n100_));
  nor2   g57(.a(new_n100_), .b(x20), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n62_), .c(x18), .d(x01), .O(new_n102_));
  inv1   g59(.a(x13), .O(new_n103_));
  nand3  g60(.a(new_n103_), .b(x12), .c(new_n74_), .O(new_n104_));
  nor3   g61(.a(new_n104_), .b(new_n45_), .c(x01), .O(new_n105_));
  nor3   g62(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n92_), .O(new_n107_));
  aoi21  g64(.a(new_n107_), .b(new_n102_), .c(x00), .O(z10));
  nand3  g65(.a(new_n92_), .b(new_n89_), .c(x15), .O(new_n109_));
  oai21  g66(.a(new_n109_), .b(new_n87_), .c(new_n44_), .O(new_n110_));
  nand2  g67(.a(new_n110_), .b(new_n60_), .O(new_n111_));
  inv1   g68(.a(x22), .O(new_n112_));
  nand4  g69(.a(new_n97_), .b(new_n96_), .c(new_n112_), .d(x21), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n111_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  aoi21  g72(.a(x10), .b(x02), .c(new_n44_), .O(new_n116_));
  oai21  g73(.a(x19), .b(x02), .c(x23), .O(new_n117_));
  nand3  g74(.a(new_n62_), .b(x17), .c(new_n45_), .O(new_n118_));
  aoi21  g75(.a(new_n118_), .b(new_n117_), .c(x01), .O(new_n119_));
  oai21  g76(.a(new_n119_), .b(new_n116_), .c(new_n115_), .O(new_n120_));
  oai22  g77(.a(new_n120_), .b(new_n46_), .c(x01), .d(new_n44_), .O(z12));
  aoi21  g78(.a(new_n118_), .b(new_n44_), .c(x01), .O(z13));
  nand4  g79(.a(new_n47_), .b(new_n52_), .c(new_n46_), .d(new_n45_), .O(new_n123_));
  aoi21  g80(.a(new_n123_), .b(new_n44_), .c(x01), .O(z14));
  nand2  g81(.a(x10), .b(x02), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  nand3  g83(.a(new_n55_), .b(x19), .c(new_n45_), .O(new_n127_));
  nand2  g84(.a(new_n127_), .b(new_n126_), .O(z15));
  xor2a  g85(.a(x01), .b(x00), .O(z16));
  aoi21  g86(.a(new_n45_), .b(new_n44_), .c(x01), .O(z17));
endmodule


