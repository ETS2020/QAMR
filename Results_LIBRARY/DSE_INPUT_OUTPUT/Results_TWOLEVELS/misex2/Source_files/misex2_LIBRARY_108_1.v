// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n137_, new_n139_, new_n140_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x03), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x09), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g09(.a(new_n52_), .b(x19), .O(z00));
  inv1   g10(.a(x09), .O(new_n54_));
  nor2   g11(.a(new_n50_), .b(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  inv1   g15(.a(x19), .O(new_n59_));
  nand4  g16(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x09), .c(new_n48_), .d(new_n46_), .O(new_n62_));
  aoi21  g19(.a(new_n62_), .b(new_n49_), .c(x01), .O(z02));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n64_));
  nand4  g21(.a(new_n59_), .b(x18), .c(new_n44_), .d(new_n49_), .O(new_n65_));
  nand3  g22(.a(x02), .b(x01), .c(x00), .O(new_n66_));
  nand4  g23(.a(x12), .b(x11), .c(x10), .d(new_n54_), .O(new_n67_));
  oai22  g24(.a(new_n67_), .b(new_n66_), .c(new_n65_), .d(new_n64_), .O(z03));
  nand2  g25(.a(x03), .b(new_n47_), .O(new_n69_));
  inv1   g26(.a(x11), .O(new_n70_));
  inv1   g27(.a(x12), .O(new_n71_));
  nand4  g28(.a(new_n71_), .b(new_n70_), .c(x10), .d(new_n54_), .O(new_n72_));
  oai21  g29(.a(new_n72_), .b(new_n66_), .c(new_n69_), .O(z04));
  nand2  g30(.a(x01), .b(x00), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n74_), .c(new_n69_), .O(z05));
  nand4  g33(.a(x11), .b(x10), .c(new_n54_), .d(x02), .O(new_n77_));
  nor3   g34(.a(new_n77_), .b(new_n47_), .c(new_n46_), .O(z06));
  nand3  g35(.a(new_n71_), .b(x11), .c(x10), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(x09), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n69_), .O(z07));
  nand2  g39(.a(x02), .b(new_n46_), .O(new_n83_));
  nor2   g40(.a(x06), .b(x05), .O(new_n84_));
  nand2  g41(.a(new_n84_), .b(x04), .O(new_n85_));
  nor2   g42(.a(new_n85_), .b(new_n83_), .O(new_n86_));
  nor2   g43(.a(x08), .b(x07), .O(new_n87_));
  nor2   g44(.a(new_n59_), .b(x18), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n87_), .c(new_n86_), .d(x17), .O(new_n89_));
  aoi21  g46(.a(new_n89_), .b(new_n49_), .c(x01), .O(z08));
  inv1   g47(.a(x21), .O(new_n91_));
  inv1   g48(.a(x22), .O(new_n92_));
  nor2   g49(.a(new_n48_), .b(x01), .O(new_n93_));
  nor2   g50(.a(new_n71_), .b(x11), .O(new_n94_));
  nand3  g51(.a(new_n94_), .b(new_n93_), .c(new_n49_), .O(new_n95_));
  inv1   g52(.a(x15), .O(new_n96_));
  inv1   g53(.a(x16), .O(new_n97_));
  nor2   g54(.a(x14), .b(x13), .O(new_n98_));
  nand4  g55(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g56(.a(x20), .O(new_n100_));
  nor2   g57(.a(new_n45_), .b(new_n47_), .O(new_n101_));
  nand3  g58(.a(new_n101_), .b(new_n100_), .c(new_n59_), .O(new_n102_));
  oai21  g59(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(new_n92_), .c(new_n91_), .d(new_n46_), .O(new_n104_));
  inv1   g61(.a(new_n104_), .O(z09));
  nand3  g62(.a(new_n59_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g63(.a(x22), .b(x21), .c(new_n100_), .O(new_n107_));
  nor2   g64(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g65(.a(x02), .b(new_n47_), .O(new_n109_));
  inv1   g66(.a(x13), .O(new_n110_));
  nand3  g67(.a(new_n110_), .b(x12), .c(new_n70_), .O(new_n111_));
  nor2   g68(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g69(.a(x14), .O(new_n113_));
  nand3  g70(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g71(.a(new_n92_), .b(new_n91_), .c(x20), .O(new_n115_));
  nor2   g72(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g73(.a(new_n116_), .b(new_n112_), .c(new_n108_), .O(new_n117_));
  oai21  g74(.a(new_n117_), .b(x00), .c(new_n69_), .O(z10));
  nand4  g75(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n59_), .O(new_n119_));
  nor2   g76(.a(new_n96_), .b(x14), .O(new_n120_));
  nor2   g77(.a(x21), .b(new_n100_), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n120_), .c(new_n97_), .d(new_n110_), .O(new_n122_));
  oai21  g79(.a(new_n122_), .b(new_n95_), .c(new_n119_), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(new_n92_), .c(new_n46_), .O(new_n124_));
  inv1   g81(.a(new_n124_), .O(z11));
  inv1   g82(.a(x24), .O(new_n126_));
  nand2  g83(.a(x10), .b(x02), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(x01), .c(x00), .O(new_n128_));
  oai21  g85(.a(x19), .b(x02), .c(x23), .O(new_n129_));
  nand3  g86(.a(new_n59_), .b(x17), .c(new_n48_), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n129_), .O(new_n131_));
  nand4  g88(.a(new_n131_), .b(new_n49_), .c(new_n47_), .d(new_n46_), .O(new_n132_));
  nand2  g89(.a(new_n132_), .b(new_n128_), .O(new_n133_));
  nand3  g90(.a(new_n133_), .b(new_n126_), .c(x09), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(z12));
  nor3   g92(.a(new_n50_), .b(x19), .c(new_n44_), .O(z13));
  nand4  g93(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n58_), .O(new_n137_));
  nor2   g94(.a(new_n137_), .b(x19), .O(z14));
  aoi21  g95(.a(new_n58_), .b(x01), .c(new_n48_), .O(new_n139_));
  aoi21  g96(.a(x19), .b(new_n48_), .c(x03), .O(new_n140_));
  oai22  g97(.a(new_n140_), .b(x01), .c(new_n139_), .d(new_n46_), .O(z15));
  nor2   g98(.a(new_n47_), .b(x00), .O(z16));
  aoi21  g99(.a(new_n83_), .b(new_n49_), .c(x01), .O(z17));
endmodule


