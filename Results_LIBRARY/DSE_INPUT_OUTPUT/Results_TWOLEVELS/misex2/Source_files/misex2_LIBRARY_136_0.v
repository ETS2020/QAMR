// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:22 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n131_, new_n134_, new_n135_,
    new_n137_, new_n138_, new_n141_;
  inv1   g00(.a(x17), .O(new_n44_));
  nor3   g01(.a(x09), .b(x02), .c(x00), .O(new_n45_));
  nor2   g02(.a(x19), .b(x18), .O(new_n46_));
  nand4  g03(.a(new_n46_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n47_));
  aoi21  g04(.a(new_n47_), .b(x03), .c(x01), .O(z00));
  inv1   g05(.a(x18), .O(new_n49_));
  inv1   g06(.a(x09), .O(new_n50_));
  inv1   g07(.a(x00), .O(new_n51_));
  inv1   g08(.a(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nand4  g10(.a(x03), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n54_));
  nor2   g11(.a(new_n54_), .b(new_n50_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand3  g17(.a(new_n53_), .b(new_n52_), .c(new_n51_), .O(new_n61_));
  inv1   g18(.a(x19), .O(new_n62_));
  nand4  g19(.a(new_n62_), .b(x18), .c(new_n44_), .d(x03), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  nand4  g21(.a(x12), .b(x11), .c(x10), .d(new_n50_), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n61_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  nand3  g24(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g25(.a(new_n68_), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n50_), .O(new_n70_));
  nor2   g27(.a(new_n70_), .b(x12), .O(z04));
  nand2  g28(.a(x01), .b(x00), .O(new_n72_));
  inv1   g29(.a(x03), .O(new_n73_));
  nand2  g30(.a(new_n73_), .b(new_n52_), .O(new_n74_));
  nand3  g31(.a(x10), .b(x09), .c(x02), .O(new_n75_));
  oai21  g32(.a(new_n75_), .b(new_n72_), .c(new_n74_), .O(z05));
  nor3   g33(.a(new_n67_), .b(new_n58_), .c(x09), .O(new_n77_));
  nand4  g34(.a(new_n77_), .b(x02), .c(x01), .d(x00), .O(new_n78_));
  nand2  g35(.a(new_n78_), .b(new_n74_), .O(z06));
  nand2  g36(.a(new_n50_), .b(x01), .O(new_n80_));
  inv1   g37(.a(x12), .O(new_n81_));
  nand3  g38(.a(new_n81_), .b(x11), .c(x10), .O(new_n82_));
  oai22  g39(.a(new_n82_), .b(new_n80_), .c(new_n73_), .d(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z07));
  inv1   g42(.a(x21), .O(new_n87_));
  inv1   g43(.a(x22), .O(new_n88_));
  nand4  g44(.a(x12), .b(new_n67_), .c(x02), .d(new_n52_), .O(new_n89_));
  inv1   g45(.a(x15), .O(new_n90_));
  inv1   g46(.a(x16), .O(new_n91_));
  nor2   g47(.a(x14), .b(x13), .O(new_n92_));
  nand4  g48(.a(new_n92_), .b(x20), .c(new_n91_), .d(new_n90_), .O(new_n93_));
  inv1   g49(.a(x20), .O(new_n94_));
  nor2   g50(.a(new_n49_), .b(new_n52_), .O(new_n95_));
  nand3  g51(.a(new_n95_), .b(new_n94_), .c(new_n62_), .O(new_n96_));
  oai21  g52(.a(new_n93_), .b(new_n89_), .c(new_n96_), .O(new_n97_));
  nand4  g53(.a(new_n97_), .b(new_n88_), .c(new_n87_), .d(new_n51_), .O(new_n98_));
  nand2  g54(.a(new_n98_), .b(new_n74_), .O(z09));
  nand3  g55(.a(new_n62_), .b(x18), .c(x01), .O(new_n100_));
  nand3  g56(.a(x22), .b(x21), .c(new_n94_), .O(new_n101_));
  nor2   g57(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nand2  g58(.a(x02), .b(new_n52_), .O(new_n103_));
  inv1   g59(.a(x13), .O(new_n104_));
  nand3  g60(.a(new_n104_), .b(x12), .c(new_n67_), .O(new_n105_));
  nor2   g61(.a(new_n105_), .b(new_n103_), .O(new_n106_));
  inv1   g62(.a(x14), .O(new_n107_));
  nand3  g63(.a(x16), .b(x15), .c(new_n107_), .O(new_n108_));
  nand3  g64(.a(new_n88_), .b(new_n87_), .c(x20), .O(new_n109_));
  nor2   g65(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  aoi21  g66(.a(new_n110_), .b(new_n106_), .c(new_n102_), .O(new_n111_));
  oai21  g67(.a(new_n111_), .b(x00), .c(new_n74_), .O(z10));
  nand4  g68(.a(new_n95_), .b(x21), .c(new_n94_), .d(new_n62_), .O(new_n113_));
  nand3  g69(.a(x12), .b(new_n67_), .c(x03), .O(new_n114_));
  nor2   g70(.a(new_n114_), .b(new_n103_), .O(new_n115_));
  nand3  g71(.a(x15), .b(new_n107_), .c(new_n104_), .O(new_n116_));
  nand3  g72(.a(new_n87_), .b(x20), .c(new_n91_), .O(new_n117_));
  nor2   g73(.a(new_n117_), .b(new_n116_), .O(new_n118_));
  nand2  g74(.a(new_n118_), .b(new_n115_), .O(new_n119_));
  nand2  g75(.a(new_n119_), .b(new_n113_), .O(new_n120_));
  nand3  g76(.a(new_n120_), .b(new_n88_), .c(new_n51_), .O(new_n121_));
  inv1   g77(.a(new_n121_), .O(z11));
  inv1   g78(.a(x24), .O(new_n123_));
  aoi21  g79(.a(x10), .b(x02), .c(new_n52_), .O(new_n124_));
  nand2  g80(.a(new_n124_), .b(x00), .O(new_n125_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n126_));
  nand3  g82(.a(new_n62_), .b(x17), .c(new_n53_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n126_), .O(new_n128_));
  nand4  g84(.a(new_n128_), .b(x03), .c(new_n52_), .d(new_n51_), .O(new_n129_));
  nand2  g85(.a(new_n129_), .b(new_n125_), .O(new_n130_));
  nand3  g86(.a(new_n130_), .b(new_n123_), .c(x09), .O(new_n131_));
  inv1   g87(.a(new_n131_), .O(z12));
  nor3   g88(.a(new_n54_), .b(x19), .c(new_n44_), .O(z13));
  nor2   g89(.a(new_n54_), .b(x09), .O(new_n134_));
  nand4  g90(.a(new_n134_), .b(new_n49_), .c(new_n44_), .d(new_n58_), .O(new_n135_));
  nor2   g91(.a(new_n135_), .b(x19), .O(z14));
  aoi21  g92(.a(x03), .b(new_n53_), .c(new_n124_), .O(new_n137_));
  nand4  g93(.a(x19), .b(x03), .c(new_n53_), .d(new_n52_), .O(new_n138_));
  oai21  g94(.a(new_n137_), .b(new_n51_), .c(new_n138_), .O(z15));
  oai21  g95(.a(new_n52_), .b(x00), .c(new_n74_), .O(z16));
  nand4  g96(.a(x03), .b(x02), .c(new_n52_), .d(new_n51_), .O(new_n141_));
  inv1   g97(.a(new_n141_), .O(z17));
  zero   g98(.O(z08));
endmodule


