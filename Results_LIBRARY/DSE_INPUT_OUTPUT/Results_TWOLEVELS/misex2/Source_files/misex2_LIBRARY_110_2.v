// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n145_, new_n146_, new_n147_;
  inv1   g000(.a(x14), .O(new_n44_));
  nand2  g001(.a(new_n44_), .b(x08), .O(new_n45_));
  inv1   g002(.a(x02), .O(new_n46_));
  inv1   g003(.a(x09), .O(new_n47_));
  nor2   g004(.a(x01), .b(x00), .O(new_n48_));
  nand3  g005(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g006(.a(x17), .O(new_n50_));
  nor2   g007(.a(x19), .b(x18), .O(new_n51_));
  nand3  g008(.a(new_n51_), .b(new_n50_), .c(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g010(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n54_));
  oai21  g011(.a(new_n54_), .b(new_n52_), .c(new_n45_), .O(z01));
  inv1   g012(.a(x01), .O(new_n56_));
  inv1   g013(.a(x18), .O(new_n57_));
  inv1   g014(.a(x19), .O(new_n58_));
  nand4  g015(.a(new_n45_), .b(new_n58_), .c(new_n57_), .d(new_n50_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g017(.a(new_n60_), .b(x09), .c(new_n46_), .d(new_n56_), .O(new_n61_));
  nor2   g018(.a(new_n61_), .b(x00), .O(z02));
  inv1   g019(.a(new_n45_), .O(new_n63_));
  nand3  g020(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g021(.a(new_n64_), .O(new_n65_));
  inv1   g022(.a(x10), .O(new_n66_));
  nor2   g023(.a(new_n66_), .b(x09), .O(new_n67_));
  nand4  g024(.a(new_n67_), .b(new_n65_), .c(x12), .d(x11), .O(new_n68_));
  nor3   g025(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n58_), .c(x18), .d(new_n50_), .O(new_n70_));
  aoi21  g027(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z03));
  inv1   g028(.a(x11), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  nand3  g030(.a(new_n67_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g031(.a(new_n74_), .b(new_n64_), .c(new_n45_), .O(z04));
  inv1   g032(.a(x00), .O(new_n76_));
  nor2   g033(.a(new_n63_), .b(new_n66_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(x09), .c(x02), .d(x01), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n76_), .O(z05));
  nand3  g036(.a(new_n44_), .b(x12), .c(x08), .O(new_n80_));
  nand4  g037(.a(new_n80_), .b(x11), .c(x10), .d(new_n47_), .O(new_n81_));
  inv1   g038(.a(new_n81_), .O(new_n82_));
  nand4  g039(.a(new_n82_), .b(x02), .c(x01), .d(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n45_), .O(z06));
  nand3  g041(.a(new_n73_), .b(x11), .c(x10), .O(new_n85_));
  oai21  g042(.a(new_n85_), .b(x09), .c(x01), .O(new_n86_));
  nand3  g043(.a(new_n86_), .b(x02), .c(x00), .O(new_n87_));
  nand2  g044(.a(new_n87_), .b(new_n45_), .O(z07));
  nor2   g045(.a(new_n46_), .b(x01), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(new_n76_), .O(new_n90_));
  inv1   g047(.a(x03), .O(new_n91_));
  inv1   g048(.a(x05), .O(new_n92_));
  nand3  g049(.a(new_n92_), .b(x04), .c(new_n91_), .O(new_n93_));
  nor2   g050(.a(new_n93_), .b(new_n90_), .O(new_n94_));
  nor3   g051(.a(x08), .b(x07), .c(x06), .O(new_n95_));
  nor2   g052(.a(new_n58_), .b(x18), .O(new_n96_));
  nand4  g053(.a(new_n96_), .b(new_n95_), .c(new_n94_), .d(x17), .O(new_n97_));
  nand2  g054(.a(new_n97_), .b(new_n45_), .O(z08));
  inv1   g055(.a(x21), .O(new_n99_));
  inv1   g056(.a(x22), .O(new_n100_));
  nand3  g057(.a(new_n89_), .b(x12), .c(new_n72_), .O(new_n101_));
  inv1   g058(.a(x15), .O(new_n102_));
  inv1   g059(.a(x16), .O(new_n103_));
  nor2   g060(.a(x14), .b(x13), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(x20), .c(new_n103_), .d(new_n102_), .O(new_n105_));
  inv1   g062(.a(x20), .O(new_n106_));
  nor2   g063(.a(new_n57_), .b(new_n56_), .O(new_n107_));
  nand3  g064(.a(new_n107_), .b(new_n106_), .c(new_n58_), .O(new_n108_));
  oai21  g065(.a(new_n105_), .b(new_n101_), .c(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n100_), .c(new_n99_), .d(new_n76_), .O(new_n110_));
  nand2  g067(.a(new_n110_), .b(new_n45_), .O(z09));
  nand4  g068(.a(new_n45_), .b(x22), .c(x21), .d(new_n106_), .O(new_n112_));
  inv1   g069(.a(new_n112_), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n58_), .c(x18), .d(x01), .O(new_n114_));
  inv1   g071(.a(x08), .O(new_n115_));
  nand3  g072(.a(new_n115_), .b(x02), .c(new_n56_), .O(new_n116_));
  inv1   g073(.a(x13), .O(new_n117_));
  nand3  g074(.a(new_n117_), .b(x12), .c(new_n72_), .O(new_n118_));
  nor2   g075(.a(new_n118_), .b(new_n116_), .O(new_n119_));
  nor3   g076(.a(new_n103_), .b(new_n102_), .c(x14), .O(new_n120_));
  nor2   g077(.a(x22), .b(x21), .O(new_n121_));
  nand4  g078(.a(new_n121_), .b(new_n120_), .c(new_n119_), .d(x20), .O(new_n122_));
  aoi21  g079(.a(new_n122_), .b(new_n114_), .c(x00), .O(z10));
  nand4  g080(.a(new_n107_), .b(x21), .c(new_n106_), .d(new_n58_), .O(new_n124_));
  nand4  g081(.a(new_n89_), .b(new_n117_), .c(x12), .d(new_n72_), .O(new_n125_));
  nor2   g082(.a(new_n102_), .b(x14), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n99_), .c(x20), .d(new_n103_), .O(new_n127_));
  oai21  g084(.a(new_n127_), .b(new_n125_), .c(new_n124_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n100_), .c(new_n76_), .O(new_n129_));
  nand2  g086(.a(new_n129_), .b(new_n45_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n58_), .b(x17), .c(new_n46_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n56_), .c(new_n76_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n45_), .O(z12));
  inv1   g097(.a(new_n48_), .O(new_n141_));
  oai21  g098(.a(new_n135_), .b(new_n141_), .c(new_n45_), .O(z13));
  nand3  g099(.a(new_n51_), .b(new_n50_), .c(new_n66_), .O(new_n143_));
  oai21  g100(.a(new_n143_), .b(new_n49_), .c(new_n45_), .O(z14));
  oai21  g101(.a(x10), .b(new_n56_), .c(x02), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(x00), .O(new_n146_));
  nand3  g103(.a(x19), .b(new_n46_), .c(new_n56_), .O(new_n147_));
  aoi21  g104(.a(new_n147_), .b(new_n146_), .c(new_n63_), .O(z15));
  nor3   g105(.a(new_n63_), .b(new_n56_), .c(x00), .O(z16));
  nand2  g106(.a(new_n90_), .b(new_n45_), .O(z17));
endmodule


