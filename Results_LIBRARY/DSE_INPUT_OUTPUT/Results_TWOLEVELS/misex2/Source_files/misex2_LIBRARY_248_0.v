// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n54_, new_n55_,
    new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n67_, new_n68_, new_n69_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x12), .O(new_n47_));
  inv1   g01(.a(x09), .O(new_n48_));
  nand3  g02(.a(x02), .b(x01), .c(x00), .O(new_n49_));
  inv1   g03(.a(new_n49_), .O(new_n50_));
  nand4  g04(.a(new_n50_), .b(x11), .c(x10), .d(new_n48_), .O(new_n51_));
  nor2   g05(.a(new_n51_), .b(new_n47_), .O(z03));
  inv1   g06(.a(x00), .O(z16));
  inv1   g07(.a(x01), .O(new_n54_));
  nand2  g08(.a(new_n54_), .b(z16), .O(new_n55_));
  nand3  g09(.a(x02), .b(x01), .c(x00), .O(new_n56_));
  inv1   g10(.a(x11), .O(new_n57_));
  nand4  g11(.a(new_n47_), .b(new_n57_), .c(x10), .d(new_n48_), .O(new_n58_));
  oai21  g12(.a(new_n58_), .b(new_n56_), .c(new_n55_), .O(z04));
  nand2  g13(.a(x01), .b(x00), .O(new_n60_));
  nand3  g14(.a(x10), .b(x09), .c(x02), .O(new_n61_));
  oai21  g15(.a(new_n61_), .b(new_n60_), .c(new_n55_), .O(z05));
  nand2  g16(.a(x11), .b(x10), .O(new_n63_));
  nor2   g17(.a(new_n63_), .b(x09), .O(new_n64_));
  nand4  g18(.a(new_n64_), .b(x02), .c(x01), .d(x00), .O(new_n65_));
  nand2  g19(.a(new_n65_), .b(new_n55_), .O(z06));
  nand3  g20(.a(new_n47_), .b(x11), .c(x10), .O(new_n67_));
  oai21  g21(.a(new_n67_), .b(x09), .c(x01), .O(new_n68_));
  nand3  g22(.a(new_n68_), .b(x02), .c(x00), .O(new_n69_));
  inv1   g23(.a(new_n69_), .O(z07));
  inv1   g24(.a(x20), .O(new_n72_));
  inv1   g25(.a(x21), .O(new_n73_));
  inv1   g26(.a(x22), .O(new_n74_));
  inv1   g27(.a(x19), .O(new_n75_));
  nand4  g28(.a(new_n75_), .b(x18), .c(x01), .d(z16), .O(new_n76_));
  inv1   g29(.a(new_n76_), .O(new_n77_));
  nand4  g30(.a(new_n77_), .b(new_n74_), .c(new_n73_), .d(new_n72_), .O(new_n78_));
  inv1   g31(.a(new_n78_), .O(z09));
  nand4  g32(.a(new_n77_), .b(x22), .c(x21), .d(new_n72_), .O(new_n80_));
  inv1   g33(.a(new_n80_), .O(z10));
  nor2   g34(.a(x22), .b(new_n73_), .O(new_n82_));
  nand4  g35(.a(new_n82_), .b(new_n72_), .c(new_n75_), .d(x18), .O(new_n83_));
  aoi21  g36(.a(new_n83_), .b(x01), .c(x00), .O(z11));
  aoi21  g37(.a(x10), .b(x02), .c(x24), .O(new_n85_));
  nand3  g38(.a(new_n85_), .b(x09), .c(x01), .O(new_n86_));
  nor2   g39(.a(new_n86_), .b(z16), .O(z12));
  inv1   g40(.a(new_n55_), .O(z13));
  inv1   g41(.a(x02), .O(new_n90_));
  inv1   g42(.a(x10), .O(new_n91_));
  aoi21  g43(.a(new_n91_), .b(x01), .c(new_n90_), .O(new_n92_));
  oai21  g44(.a(new_n92_), .b(z16), .c(new_n55_), .O(z15));
  zero   g45(.O(z00));
  zero   g46(.O(z01));
  zero   g47(.O(z02));
  zero   g48(.O(z08));
  zero   g49(.O(z14));
  zero   g50(.O(z17));
endmodule


