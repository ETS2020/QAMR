// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n74_, new_n75_, new_n76_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n135_, new_n136_,
    new_n138_, new_n139_, new_n140_;
  inv1   g00(.a(x15), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x01), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(x17), .O(new_n50_));
  inv1   g07(.a(x18), .O(new_n51_));
  inv1   g08(.a(x19), .O(new_n52_));
  nand4  g09(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n49_), .c(new_n45_), .O(z00));
  nand3  g11(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n58_));
  oai21  g15(.a(new_n58_), .b(new_n55_), .c(new_n45_), .O(z02));
  inv1   g16(.a(x00), .O(new_n60_));
  inv1   g17(.a(x01), .O(new_n61_));
  nand3  g18(.a(new_n46_), .b(new_n61_), .c(new_n60_), .O(new_n62_));
  nand3  g19(.a(new_n52_), .b(x18), .c(new_n50_), .O(new_n63_));
  nand4  g20(.a(new_n47_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  nand4  g21(.a(x15), .b(x12), .c(x11), .d(x10), .O(new_n65_));
  oai22  g22(.a(new_n65_), .b(new_n64_), .c(new_n63_), .d(new_n62_), .O(z03));
  inv1   g23(.a(x11), .O(new_n67_));
  inv1   g24(.a(x12), .O(new_n68_));
  nand3  g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(x09), .O(new_n70_));
  nand4  g27(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(x10), .O(new_n71_));
  nor2   g28(.a(new_n71_), .b(new_n44_), .O(z04));
  nor4   g29(.a(new_n69_), .b(new_n44_), .c(new_n57_), .d(new_n47_), .O(z05));
  nand3  g30(.a(x15), .b(x11), .c(x10), .O(new_n74_));
  inv1   g31(.a(new_n74_), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(new_n47_), .c(x02), .d(x01), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(new_n60_), .O(z06));
  nand2  g34(.a(x10), .b(new_n47_), .O(new_n78_));
  nand3  g35(.a(x15), .b(new_n68_), .c(x11), .O(new_n79_));
  oai21  g36(.a(new_n79_), .b(new_n78_), .c(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  nand2  g38(.a(new_n81_), .b(new_n45_), .O(z07));
  inv1   g39(.a(x08), .O(new_n83_));
  inv1   g40(.a(x05), .O(new_n84_));
  inv1   g41(.a(x06), .O(new_n85_));
  inv1   g42(.a(x03), .O(new_n86_));
  nand4  g43(.a(new_n86_), .b(x02), .c(new_n61_), .d(new_n60_), .O(new_n87_));
  inv1   g44(.a(new_n87_), .O(new_n88_));
  nand4  g45(.a(new_n88_), .b(new_n85_), .c(new_n84_), .d(x04), .O(new_n89_));
  nor2   g46(.a(new_n89_), .b(x07), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(new_n51_), .c(x17), .d(new_n83_), .O(new_n91_));
  nor2   g48(.a(new_n91_), .b(new_n52_), .O(z08));
  inv1   g49(.a(x21), .O(new_n93_));
  inv1   g50(.a(x22), .O(new_n94_));
  nand4  g51(.a(x12), .b(new_n67_), .c(x02), .d(new_n61_), .O(new_n95_));
  inv1   g52(.a(x16), .O(new_n96_));
  nor2   g53(.a(x14), .b(x13), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(x20), .c(new_n96_), .d(new_n44_), .O(new_n98_));
  nor2   g55(.a(x20), .b(x19), .O(new_n99_));
  nand4  g56(.a(new_n99_), .b(x18), .c(x15), .d(x01), .O(new_n100_));
  oai21  g57(.a(new_n98_), .b(new_n95_), .c(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(new_n94_), .c(new_n93_), .d(new_n60_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z09));
  nand3  g60(.a(new_n52_), .b(x18), .c(x01), .O(new_n104_));
  inv1   g61(.a(x20), .O(new_n105_));
  nand3  g62(.a(x22), .b(x21), .c(new_n105_), .O(new_n106_));
  nor2   g63(.a(new_n106_), .b(new_n104_), .O(new_n107_));
  nand2  g64(.a(x02), .b(new_n61_), .O(new_n108_));
  inv1   g65(.a(x13), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x12), .c(new_n67_), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(new_n108_), .O(new_n111_));
  nand2  g68(.a(x16), .b(x15), .O(new_n112_));
  nand3  g69(.a(new_n94_), .b(new_n93_), .c(x20), .O(new_n113_));
  nor3   g70(.a(new_n113_), .b(new_n112_), .c(x14), .O(new_n114_));
  aoi21  g71(.a(new_n114_), .b(new_n111_), .c(new_n107_), .O(new_n115_));
  oai21  g72(.a(new_n115_), .b(x00), .c(new_n45_), .O(z10));
  nand2  g73(.a(x18), .b(x01), .O(new_n117_));
  nand3  g74(.a(x21), .b(new_n105_), .c(new_n52_), .O(new_n118_));
  nand4  g75(.a(new_n97_), .b(new_n93_), .c(x20), .d(new_n96_), .O(new_n119_));
  oai22  g76(.a(new_n119_), .b(new_n95_), .c(new_n118_), .d(new_n117_), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n94_), .c(x15), .d(new_n60_), .O(new_n121_));
  inv1   g78(.a(new_n121_), .O(z11));
  nand2  g79(.a(x10), .b(x02), .O(new_n123_));
  nand3  g80(.a(new_n123_), .b(x01), .c(x00), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand3  g82(.a(new_n52_), .b(x17), .c(new_n46_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n61_), .c(new_n60_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n124_), .c(x24), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x09), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(new_n45_), .O(z12));
  inv1   g88(.a(new_n48_), .O(new_n132_));
  oai21  g89(.a(new_n126_), .b(new_n132_), .c(new_n45_), .O(z13));
  nand3  g90(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n134_));
  inv1   g91(.a(new_n134_), .O(new_n135_));
  nand4  g92(.a(new_n135_), .b(new_n51_), .c(new_n50_), .d(new_n57_), .O(new_n136_));
  nor2   g93(.a(new_n136_), .b(x19), .O(z14));
  oai21  g94(.a(x10), .b(new_n61_), .c(x02), .O(new_n138_));
  nand2  g95(.a(new_n138_), .b(x00), .O(new_n139_));
  nand3  g96(.a(x19), .b(new_n46_), .c(new_n61_), .O(new_n140_));
  nand3  g97(.a(new_n140_), .b(new_n139_), .c(new_n45_), .O(z15));
  aoi21  g98(.a(x15), .b(x00), .c(new_n61_), .O(z16));
  oai21  g99(.a(new_n108_), .b(x00), .c(new_n45_), .O(z17));
endmodule


