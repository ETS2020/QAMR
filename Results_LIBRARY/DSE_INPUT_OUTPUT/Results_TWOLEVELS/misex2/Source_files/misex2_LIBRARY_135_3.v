// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:21 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n52_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n123_, new_n124_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n132_, new_n134_, new_n136_, new_n137_,
    new_n139_, new_n141_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor3   g03(.a(x02), .b(x01), .c(x00), .O(new_n47_));
  nand2  g04(.a(new_n47_), .b(new_n46_), .O(new_n48_));
  inv1   g05(.a(new_n48_), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n50_));
  nor2   g07(.a(new_n50_), .b(x19), .O(z00));
  nand2  g08(.a(x21), .b(x01), .O(new_n52_));
  inv1   g09(.a(x02), .O(new_n53_));
  nor2   g10(.a(x01), .b(x00), .O(new_n54_));
  nand3  g11(.a(new_n54_), .b(x09), .c(new_n53_), .O(new_n55_));
  inv1   g12(.a(x19), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n57_));
  oai21  g14(.a(new_n57_), .b(new_n55_), .c(new_n52_), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand2  g16(.a(new_n47_), .b(x09), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z02));
  inv1   g20(.a(x21), .O(new_n64_));
  nand3  g21(.a(new_n46_), .b(x02), .c(x00), .O(new_n65_));
  nand3  g22(.a(x12), .b(x11), .c(x10), .O(new_n66_));
  oai21  g23(.a(new_n66_), .b(new_n65_), .c(new_n64_), .O(new_n67_));
  nand2  g24(.a(new_n67_), .b(x01), .O(new_n68_));
  nor3   g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n56_), .c(x18), .d(new_n44_), .O(new_n70_));
  nand2  g27(.a(new_n70_), .b(new_n68_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(x02), .b(x01), .c(x00), .O(new_n74_));
  nor2   g31(.a(new_n74_), .b(x09), .O(new_n75_));
  nand4  g32(.a(new_n75_), .b(new_n73_), .c(new_n72_), .d(x10), .O(new_n76_));
  nor2   g33(.a(new_n76_), .b(x21), .O(z04));
  nor4   g34(.a(new_n74_), .b(x21), .c(new_n59_), .d(new_n46_), .O(z05));
  inv1   g35(.a(x01), .O(new_n79_));
  nor2   g36(.a(new_n72_), .b(new_n59_), .O(new_n80_));
  nand4  g37(.a(new_n80_), .b(new_n46_), .c(x02), .d(x00), .O(new_n81_));
  aoi21  g38(.a(new_n81_), .b(new_n64_), .c(new_n79_), .O(z06));
  nand3  g39(.a(new_n73_), .b(x11), .c(x10), .O(new_n83_));
  oai21  g40(.a(new_n83_), .b(x09), .c(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  nand2  g42(.a(new_n85_), .b(new_n52_), .O(z07));
  inv1   g43(.a(x08), .O(new_n87_));
  inv1   g44(.a(x05), .O(new_n88_));
  inv1   g45(.a(x06), .O(new_n89_));
  inv1   g46(.a(x00), .O(new_n90_));
  inv1   g47(.a(x03), .O(new_n91_));
  nand4  g48(.a(new_n91_), .b(x02), .c(new_n79_), .d(new_n90_), .O(new_n92_));
  inv1   g49(.a(new_n92_), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n89_), .c(new_n88_), .d(x04), .O(new_n94_));
  nor2   g51(.a(new_n94_), .b(x07), .O(new_n95_));
  nand4  g52(.a(new_n95_), .b(new_n45_), .c(x17), .d(new_n87_), .O(new_n96_));
  nor2   g53(.a(new_n96_), .b(new_n56_), .O(z08));
  inv1   g54(.a(x22), .O(new_n98_));
  nand4  g55(.a(x12), .b(new_n72_), .c(x02), .d(new_n79_), .O(new_n99_));
  inv1   g56(.a(x15), .O(new_n100_));
  inv1   g57(.a(x16), .O(new_n101_));
  nor2   g58(.a(x14), .b(x13), .O(new_n102_));
  nand4  g59(.a(new_n102_), .b(x20), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g60(.a(x20), .O(new_n104_));
  nand4  g61(.a(new_n104_), .b(new_n56_), .c(x18), .d(x01), .O(new_n105_));
  oai21  g62(.a(new_n103_), .b(new_n99_), .c(new_n105_), .O(new_n106_));
  nand4  g63(.a(new_n106_), .b(new_n98_), .c(new_n64_), .d(new_n90_), .O(new_n107_));
  inv1   g64(.a(new_n107_), .O(z09));
  nand3  g65(.a(x02), .b(new_n79_), .c(new_n90_), .O(new_n109_));
  nor4   g66(.a(new_n109_), .b(x13), .c(new_n73_), .d(x11), .O(new_n110_));
  nor3   g67(.a(new_n101_), .b(new_n100_), .c(x14), .O(new_n111_));
  nor2   g68(.a(x22), .b(x21), .O(new_n112_));
  nand4  g69(.a(new_n112_), .b(new_n111_), .c(new_n110_), .d(x20), .O(new_n113_));
  nand2  g70(.a(new_n113_), .b(new_n52_), .O(z10));
  inv1   g71(.a(x13), .O(new_n115_));
  inv1   g72(.a(x14), .O(new_n116_));
  nand4  g73(.a(new_n72_), .b(x02), .c(new_n79_), .d(new_n90_), .O(new_n117_));
  inv1   g74(.a(new_n117_), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(new_n116_), .c(new_n115_), .d(x12), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(new_n100_), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n64_), .c(x20), .d(new_n101_), .O(new_n121_));
  nor2   g78(.a(new_n121_), .b(x22), .O(z11));
  nand2  g79(.a(x10), .b(x02), .O(new_n123_));
  nand4  g80(.a(new_n123_), .b(new_n64_), .c(x01), .d(x00), .O(new_n124_));
  oai21  g81(.a(x19), .b(x02), .c(x23), .O(new_n125_));
  nand3  g82(.a(new_n56_), .b(x17), .c(new_n53_), .O(new_n126_));
  nand2  g83(.a(new_n126_), .b(new_n125_), .O(new_n127_));
  nand3  g84(.a(new_n127_), .b(new_n79_), .c(new_n90_), .O(new_n128_));
  aoi21  g85(.a(new_n128_), .b(new_n124_), .c(x24), .O(new_n129_));
  nand2  g86(.a(new_n129_), .b(x09), .O(new_n130_));
  inv1   g87(.a(new_n130_), .O(z12));
  nand2  g88(.a(new_n47_), .b(x17), .O(new_n132_));
  nor2   g89(.a(new_n132_), .b(x19), .O(z13));
  nand4  g90(.a(new_n49_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n134_));
  nor2   g91(.a(new_n134_), .b(x19), .O(z14));
  aoi22  g92(.a(new_n52_), .b(x00), .c(x19), .d(new_n79_), .O(new_n136_));
  nand4  g93(.a(new_n64_), .b(new_n59_), .c(x01), .d(x00), .O(new_n137_));
  oai21  g94(.a(new_n136_), .b(x02), .c(new_n137_), .O(z15));
  nand3  g95(.a(new_n64_), .b(x01), .c(new_n90_), .O(new_n139_));
  inv1   g96(.a(new_n139_), .O(z16));
  nand2  g97(.a(new_n54_), .b(x02), .O(new_n141_));
  inv1   g98(.a(new_n141_), .O(z17));
endmodule


