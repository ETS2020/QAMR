// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n138_, new_n139_, new_n141_, new_n143_,
    new_n144_, new_n145_, new_n148_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x00), .O(new_n46_));
  inv1   g003(.a(x01), .O(new_n47_));
  inv1   g004(.a(x02), .O(new_n48_));
  inv1   g005(.a(x09), .O(new_n49_));
  nand4  g006(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  inv1   g010(.a(x22), .O(new_n54_));
  inv1   g011(.a(x19), .O(new_n55_));
  inv1   g012(.a(x10), .O(new_n56_));
  nand3  g013(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n57_));
  nor3   g014(.a(new_n57_), .b(new_n56_), .c(new_n49_), .O(new_n58_));
  nand4  g015(.a(new_n58_), .b(new_n55_), .c(new_n45_), .d(new_n44_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(new_n54_), .O(z01));
  nor3   g017(.a(x02), .b(x01), .c(x00), .O(new_n61_));
  nor2   g018(.a(x19), .b(x18), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n61_), .c(new_n44_), .d(new_n56_), .O(new_n63_));
  aoi21  g020(.a(new_n63_), .b(x22), .c(new_n49_), .O(z02));
  nor2   g021(.a(x22), .b(new_n49_), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  nand4  g023(.a(new_n61_), .b(new_n55_), .c(x18), .d(new_n44_), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nor2   g026(.a(new_n56_), .b(x09), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n69_), .c(x12), .d(x11), .O(new_n71_));
  nand3  g028(.a(new_n71_), .b(new_n67_), .c(new_n66_), .O(z03));
  inv1   g029(.a(x11), .O(new_n73_));
  inv1   g030(.a(x12), .O(new_n74_));
  nand3  g031(.a(new_n70_), .b(new_n74_), .c(new_n73_), .O(new_n75_));
  oai21  g032(.a(new_n75_), .b(new_n68_), .c(new_n66_), .O(z04));
  nand4  g033(.a(x09), .b(x02), .c(x01), .d(x00), .O(new_n77_));
  nor3   g034(.a(new_n77_), .b(new_n54_), .c(new_n56_), .O(z05));
  nand4  g035(.a(x11), .b(x10), .c(new_n49_), .d(x02), .O(new_n79_));
  nor3   g036(.a(new_n79_), .b(new_n47_), .c(new_n46_), .O(z06));
  nand2  g037(.a(new_n49_), .b(x01), .O(new_n81_));
  nand3  g038(.a(new_n74_), .b(x11), .c(x10), .O(new_n82_));
  oai22  g039(.a(new_n82_), .b(new_n81_), .c(new_n65_), .d(x01), .O(new_n83_));
  nand3  g040(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g041(.a(new_n84_), .O(z07));
  inv1   g042(.a(x03), .O(new_n86_));
  nor2   g043(.a(new_n48_), .b(x01), .O(new_n87_));
  inv1   g044(.a(x04), .O(new_n88_));
  nor2   g045(.a(x05), .b(new_n88_), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(new_n87_), .c(new_n86_), .d(new_n46_), .O(new_n90_));
  nor3   g047(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(x19), .c(new_n45_), .d(x17), .O(new_n92_));
  oai21  g049(.a(new_n92_), .b(new_n90_), .c(new_n66_), .O(z08));
  inv1   g050(.a(x21), .O(new_n94_));
  nand4  g051(.a(x12), .b(new_n73_), .c(x02), .d(new_n47_), .O(new_n95_));
  inv1   g052(.a(x15), .O(new_n96_));
  inv1   g053(.a(x16), .O(new_n97_));
  nor2   g054(.a(x14), .b(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x20), .c(new_n97_), .d(new_n96_), .O(new_n99_));
  inv1   g056(.a(x20), .O(new_n100_));
  and2   g057(.a(x18), .b(x01), .O(new_n101_));
  nand3  g058(.a(new_n101_), .b(new_n100_), .c(new_n55_), .O(new_n102_));
  oai21  g059(.a(new_n99_), .b(new_n95_), .c(new_n102_), .O(new_n103_));
  nand3  g060(.a(new_n103_), .b(new_n94_), .c(new_n46_), .O(new_n104_));
  aoi21  g061(.a(new_n104_), .b(new_n49_), .c(x22), .O(z09));
  nand3  g062(.a(new_n55_), .b(x18), .c(x01), .O(new_n106_));
  nand3  g063(.a(x22), .b(x21), .c(new_n100_), .O(new_n107_));
  nor2   g064(.a(new_n107_), .b(new_n106_), .O(new_n108_));
  nand2  g065(.a(x02), .b(new_n47_), .O(new_n109_));
  inv1   g066(.a(x13), .O(new_n110_));
  nand3  g067(.a(new_n110_), .b(x12), .c(new_n73_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  inv1   g069(.a(x14), .O(new_n113_));
  nand3  g070(.a(x16), .b(x15), .c(new_n113_), .O(new_n114_));
  nand3  g071(.a(new_n54_), .b(new_n94_), .c(x20), .O(new_n115_));
  nor2   g072(.a(new_n115_), .b(new_n114_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n112_), .c(new_n108_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n66_), .O(z10));
  nor2   g075(.a(x13), .b(new_n74_), .O(new_n119_));
  nand3  g076(.a(new_n119_), .b(new_n87_), .c(new_n73_), .O(new_n120_));
  nand4  g077(.a(new_n101_), .b(x21), .c(new_n100_), .d(new_n55_), .O(new_n121_));
  nor2   g078(.a(new_n96_), .b(x14), .O(new_n122_));
  nor2   g079(.a(x21), .b(new_n100_), .O(new_n123_));
  nand3  g080(.a(new_n123_), .b(new_n122_), .c(new_n97_), .O(new_n124_));
  oai21  g081(.a(new_n124_), .b(new_n120_), .c(new_n121_), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n54_), .c(new_n49_), .d(new_n46_), .O(new_n126_));
  inv1   g083(.a(new_n126_), .O(z11));
  inv1   g084(.a(x24), .O(new_n128_));
  nand2  g085(.a(x10), .b(x02), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(x01), .c(x00), .O(new_n130_));
  oai21  g087(.a(x19), .b(x02), .c(x23), .O(new_n131_));
  nand3  g088(.a(new_n55_), .b(x17), .c(new_n48_), .O(new_n132_));
  nand2  g089(.a(new_n132_), .b(new_n131_), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(new_n47_), .c(new_n46_), .O(new_n134_));
  nand2  g091(.a(new_n134_), .b(new_n130_), .O(new_n135_));
  nand4  g092(.a(new_n135_), .b(new_n128_), .c(x22), .d(x09), .O(new_n136_));
  inv1   g093(.a(new_n136_), .O(z12));
  nor2   g094(.a(new_n65_), .b(x19), .O(new_n138_));
  nand4  g095(.a(new_n138_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n139_));
  nor2   g096(.a(new_n139_), .b(x00), .O(z13));
  nand4  g097(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n56_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z14));
  oai21  g099(.a(x10), .b(new_n47_), .c(x02), .O(new_n143_));
  nand2  g100(.a(new_n143_), .b(x00), .O(new_n144_));
  nand3  g101(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n145_));
  nand3  g102(.a(new_n145_), .b(new_n144_), .c(new_n66_), .O(z15));
  oai21  g103(.a(new_n47_), .b(x00), .c(new_n66_), .O(z16));
  nand4  g104(.a(new_n66_), .b(x02), .c(new_n47_), .d(new_n46_), .O(new_n148_));
  inv1   g105(.a(new_n148_), .O(z17));
endmodule


