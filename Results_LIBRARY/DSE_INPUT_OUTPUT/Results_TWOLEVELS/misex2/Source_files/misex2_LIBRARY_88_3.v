// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n120_,
    new_n121_, new_n122_, new_n123_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n130_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n140_,
    new_n143_, new_n145_, new_n146_, new_n147_, new_n150_;
  nand2  g000(.a(x23), .b(x04), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  nor2   g006(.a(x19), .b(x18), .O(new_n50_));
  nand3  g007(.a(new_n50_), .b(new_n49_), .c(x10), .O(new_n51_));
  oai21  g008(.a(new_n51_), .b(new_n48_), .c(new_n44_), .O(z00));
  inv1   g009(.a(x01), .O(new_n53_));
  inv1   g010(.a(x10), .O(new_n54_));
  inv1   g011(.a(x18), .O(new_n55_));
  inv1   g012(.a(x19), .O(new_n56_));
  nand4  g013(.a(new_n44_), .b(new_n56_), .c(new_n55_), .d(new_n49_), .O(new_n57_));
  nor2   g014(.a(new_n57_), .b(new_n54_), .O(new_n58_));
  nand4  g015(.a(new_n58_), .b(x09), .c(new_n45_), .d(new_n53_), .O(new_n59_));
  nor2   g016(.a(new_n59_), .b(x00), .O(z01));
  nor2   g017(.a(new_n57_), .b(x10), .O(new_n61_));
  nand4  g018(.a(new_n61_), .b(x09), .c(new_n45_), .d(new_n53_), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x00), .O(z02));
  inv1   g020(.a(new_n44_), .O(new_n64_));
  nand3  g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  inv1   g022(.a(new_n65_), .O(new_n66_));
  inv1   g023(.a(x11), .O(new_n67_));
  inv1   g024(.a(x12), .O(new_n68_));
  nor2   g025(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n66_), .c(x10), .d(new_n46_), .O(new_n70_));
  nor3   g027(.a(x02), .b(x01), .c(x00), .O(new_n71_));
  nor2   g028(.a(x19), .b(new_n55_), .O(new_n72_));
  nand3  g029(.a(new_n72_), .b(new_n71_), .c(new_n49_), .O(new_n73_));
  aoi21  g030(.a(new_n73_), .b(new_n70_), .c(new_n64_), .O(z03));
  inv1   g031(.a(x00), .O(new_n75_));
  nand4  g032(.a(new_n44_), .b(new_n68_), .c(new_n67_), .d(x10), .O(new_n76_));
  inv1   g033(.a(new_n76_), .O(new_n77_));
  nand4  g034(.a(new_n77_), .b(new_n46_), .c(x02), .d(x01), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(new_n75_), .O(z04));
  nand2  g036(.a(x01), .b(x00), .O(new_n80_));
  nand3  g037(.a(x10), .b(x09), .c(x02), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(new_n80_), .c(new_n44_), .O(z05));
  nand4  g039(.a(new_n44_), .b(x11), .c(x10), .d(new_n46_), .O(new_n83_));
  inv1   g040(.a(new_n83_), .O(new_n84_));
  nand4  g041(.a(new_n84_), .b(x02), .c(x01), .d(x00), .O(new_n85_));
  inv1   g042(.a(new_n85_), .O(z06));
  nand4  g043(.a(new_n44_), .b(new_n68_), .c(x11), .d(x10), .O(new_n87_));
  oai21  g044(.a(new_n87_), .b(x09), .c(x01), .O(new_n88_));
  nand3  g045(.a(new_n88_), .b(x02), .c(x00), .O(new_n89_));
  nand2  g046(.a(new_n89_), .b(new_n44_), .O(z07));
  inv1   g047(.a(x04), .O(new_n91_));
  inv1   g048(.a(x23), .O(new_n92_));
  nor2   g049(.a(x05), .b(x03), .O(new_n93_));
  nand2  g050(.a(new_n93_), .b(x02), .O(new_n94_));
  inv1   g051(.a(new_n94_), .O(new_n95_));
  nor3   g052(.a(x08), .b(x07), .c(x06), .O(new_n96_));
  nand3  g053(.a(x19), .b(new_n55_), .c(x17), .O(new_n97_));
  inv1   g054(.a(new_n97_), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(new_n96_), .c(new_n95_), .d(new_n47_), .O(new_n99_));
  aoi21  g056(.a(new_n99_), .b(new_n92_), .c(new_n91_), .O(z08));
  inv1   g057(.a(x21), .O(new_n101_));
  inv1   g058(.a(x22), .O(new_n102_));
  nand4  g059(.a(x12), .b(new_n67_), .c(x02), .d(new_n53_), .O(new_n103_));
  inv1   g060(.a(x15), .O(new_n104_));
  inv1   g061(.a(x16), .O(new_n105_));
  nor2   g062(.a(x14), .b(x13), .O(new_n106_));
  nand4  g063(.a(new_n106_), .b(x20), .c(new_n105_), .d(new_n104_), .O(new_n107_));
  inv1   g064(.a(x20), .O(new_n108_));
  nand4  g065(.a(new_n108_), .b(new_n56_), .c(x18), .d(x01), .O(new_n109_));
  oai21  g066(.a(new_n107_), .b(new_n103_), .c(new_n109_), .O(new_n110_));
  nand4  g067(.a(new_n110_), .b(new_n44_), .c(new_n102_), .d(new_n101_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(x00), .O(z09));
  nor2   g069(.a(new_n102_), .b(new_n101_), .O(new_n113_));
  nand4  g070(.a(new_n113_), .b(new_n72_), .c(new_n108_), .d(x01), .O(new_n114_));
  nor2   g071(.a(new_n45_), .b(x01), .O(new_n115_));
  nor2   g072(.a(x13), .b(new_n68_), .O(new_n116_));
  nand3  g073(.a(new_n116_), .b(new_n115_), .c(new_n67_), .O(new_n117_));
  inv1   g074(.a(x14), .O(new_n118_));
  nor2   g075(.a(new_n105_), .b(new_n104_), .O(new_n119_));
  nor2   g076(.a(x22), .b(x21), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n119_), .c(x20), .d(new_n118_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n117_), .c(new_n114_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n44_), .c(new_n75_), .O(new_n123_));
  inv1   g080(.a(new_n123_), .O(z10));
  nor2   g081(.a(new_n55_), .b(new_n53_), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(x21), .c(new_n108_), .d(new_n56_), .O(new_n126_));
  nor2   g083(.a(new_n104_), .b(x14), .O(new_n127_));
  nand4  g084(.a(new_n127_), .b(new_n101_), .c(x20), .d(new_n105_), .O(new_n128_));
  oai21  g085(.a(new_n128_), .b(new_n117_), .c(new_n126_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n102_), .c(new_n75_), .O(new_n130_));
  nand2  g087(.a(new_n130_), .b(new_n44_), .O(z11));
  inv1   g088(.a(x24), .O(new_n132_));
  nand2  g089(.a(x10), .b(x02), .O(new_n133_));
  nand3  g090(.a(new_n133_), .b(x01), .c(x00), .O(new_n134_));
  nand4  g091(.a(new_n47_), .b(new_n56_), .c(x17), .d(new_n45_), .O(new_n135_));
  aoi21  g092(.a(new_n135_), .b(new_n134_), .c(new_n64_), .O(new_n136_));
  nand2  g093(.a(new_n56_), .b(new_n45_), .O(new_n137_));
  nand4  g094(.a(new_n137_), .b(x23), .c(new_n91_), .d(new_n53_), .O(new_n138_));
  nor2   g095(.a(new_n138_), .b(x00), .O(new_n139_));
  oai21  g096(.a(new_n139_), .b(new_n136_), .c(new_n132_), .O(new_n140_));
  nor2   g097(.a(new_n140_), .b(new_n46_), .O(z12));
  nand2  g098(.a(new_n135_), .b(new_n44_), .O(z13));
  nand3  g099(.a(new_n50_), .b(new_n49_), .c(new_n54_), .O(new_n143_));
  oai21  g100(.a(new_n143_), .b(new_n48_), .c(new_n44_), .O(z14));
  oai21  g101(.a(x10), .b(new_n53_), .c(x02), .O(new_n145_));
  nand2  g102(.a(new_n145_), .b(x00), .O(new_n146_));
  nand3  g103(.a(x19), .b(new_n45_), .c(new_n53_), .O(new_n147_));
  aoi21  g104(.a(new_n147_), .b(new_n146_), .c(new_n64_), .O(z15));
  oai21  g105(.a(new_n53_), .b(x00), .c(new_n44_), .O(z16));
  nand2  g106(.a(new_n115_), .b(new_n75_), .O(new_n150_));
  nand2  g107(.a(new_n150_), .b(new_n44_), .O(z17));
endmodule


