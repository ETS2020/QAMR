// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n81_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n96_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n118_, new_n119_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n132_, new_n133_, new_n134_, new_n136_,
    new_n137_, new_n140_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  nand4  g05(.a(x05), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nor2   g06(.a(new_n49_), .b(x09), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x09), .O(new_n53_));
  nor2   g10(.a(new_n49_), .b(new_n53_), .O(new_n54_));
  nand4  g11(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n55_));
  nor2   g12(.a(new_n55_), .b(x19), .O(z01));
  inv1   g13(.a(x10), .O(new_n57_));
  nand4  g14(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n57_), .O(new_n58_));
  nor2   g15(.a(new_n58_), .b(x19), .O(z02));
  nand3  g16(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n60_));
  inv1   g17(.a(x19), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(x18), .c(new_n44_), .d(x05), .O(new_n62_));
  nand3  g19(.a(x02), .b(x01), .c(x00), .O(new_n63_));
  nand4  g20(.a(x12), .b(x11), .c(x10), .d(new_n53_), .O(new_n64_));
  oai22  g21(.a(new_n64_), .b(new_n63_), .c(new_n62_), .d(new_n60_), .O(z03));
  inv1   g22(.a(x11), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  inv1   g24(.a(new_n67_), .O(new_n68_));
  nand4  g25(.a(new_n68_), .b(new_n66_), .c(x10), .d(new_n53_), .O(new_n69_));
  nor2   g26(.a(new_n69_), .b(x12), .O(z04));
  nor3   g27(.a(new_n67_), .b(new_n57_), .c(new_n53_), .O(z05));
  nor3   g28(.a(new_n66_), .b(new_n57_), .c(x09), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(x02), .c(x01), .d(x00), .O(new_n73_));
  inv1   g30(.a(x05), .O(new_n74_));
  nand2  g31(.a(new_n74_), .b(new_n47_), .O(new_n75_));
  nand2  g32(.a(new_n75_), .b(new_n73_), .O(z06));
  nand2  g33(.a(new_n53_), .b(x01), .O(new_n77_));
  inv1   g34(.a(x12), .O(new_n78_));
  nand3  g35(.a(new_n78_), .b(x11), .c(x10), .O(new_n79_));
  oai22  g36(.a(new_n79_), .b(new_n77_), .c(new_n74_), .d(x01), .O(new_n80_));
  nand3  g37(.a(new_n80_), .b(x02), .c(x00), .O(new_n81_));
  inv1   g38(.a(new_n81_), .O(z07));
  inv1   g39(.a(x21), .O(new_n84_));
  inv1   g40(.a(x22), .O(new_n85_));
  nor2   g41(.a(new_n48_), .b(x01), .O(new_n86_));
  nand3  g42(.a(new_n86_), .b(x12), .c(new_n66_), .O(new_n87_));
  inv1   g43(.a(x15), .O(new_n88_));
  inv1   g44(.a(x16), .O(new_n89_));
  nor2   g45(.a(x14), .b(x13), .O(new_n90_));
  nand4  g46(.a(new_n90_), .b(x20), .c(new_n89_), .d(new_n88_), .O(new_n91_));
  inv1   g47(.a(x20), .O(new_n92_));
  nor2   g48(.a(new_n45_), .b(new_n47_), .O(new_n93_));
  nand3  g49(.a(new_n93_), .b(new_n92_), .c(new_n61_), .O(new_n94_));
  oai21  g50(.a(new_n91_), .b(new_n87_), .c(new_n94_), .O(new_n95_));
  nand4  g51(.a(new_n95_), .b(new_n85_), .c(new_n84_), .d(new_n46_), .O(new_n96_));
  nand2  g52(.a(new_n96_), .b(new_n75_), .O(z09));
  nor3   g53(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n98_));
  nand4  g54(.a(new_n98_), .b(new_n61_), .c(x18), .d(x01), .O(new_n99_));
  nor3   g55(.a(new_n74_), .b(new_n48_), .c(x01), .O(new_n100_));
  inv1   g56(.a(x13), .O(new_n101_));
  nand3  g57(.a(new_n101_), .b(x12), .c(new_n66_), .O(new_n102_));
  inv1   g58(.a(new_n102_), .O(new_n103_));
  nor3   g59(.a(new_n89_), .b(new_n88_), .c(x14), .O(new_n104_));
  nand3  g60(.a(new_n85_), .b(new_n84_), .c(x20), .O(new_n105_));
  inv1   g61(.a(new_n105_), .O(new_n106_));
  nand4  g62(.a(new_n106_), .b(new_n104_), .c(new_n103_), .d(new_n100_), .O(new_n107_));
  aoi21  g63(.a(new_n107_), .b(new_n99_), .c(x00), .O(z10));
  nand4  g64(.a(new_n93_), .b(x21), .c(new_n92_), .d(new_n61_), .O(new_n109_));
  nand3  g65(.a(x12), .b(new_n66_), .c(x05), .O(new_n110_));
  inv1   g66(.a(new_n110_), .O(new_n111_));
  inv1   g67(.a(x14), .O(new_n112_));
  nand3  g68(.a(x15), .b(new_n112_), .c(new_n101_), .O(new_n113_));
  inv1   g69(.a(new_n113_), .O(new_n114_));
  nand3  g70(.a(new_n84_), .b(x20), .c(new_n89_), .O(new_n115_));
  inv1   g71(.a(new_n115_), .O(new_n116_));
  nand4  g72(.a(new_n116_), .b(new_n114_), .c(new_n111_), .d(new_n86_), .O(new_n117_));
  nand2  g73(.a(new_n117_), .b(new_n109_), .O(new_n118_));
  nand3  g74(.a(new_n118_), .b(new_n85_), .c(new_n46_), .O(new_n119_));
  inv1   g75(.a(new_n119_), .O(z11));
  inv1   g76(.a(x24), .O(new_n121_));
  nand2  g77(.a(x10), .b(x02), .O(new_n122_));
  nand3  g78(.a(new_n122_), .b(x01), .c(x00), .O(new_n123_));
  oai21  g79(.a(x19), .b(x02), .c(x23), .O(new_n124_));
  nand3  g80(.a(new_n61_), .b(x17), .c(new_n48_), .O(new_n125_));
  nand2  g81(.a(new_n125_), .b(new_n124_), .O(new_n126_));
  nand4  g82(.a(new_n126_), .b(x05), .c(new_n47_), .d(new_n46_), .O(new_n127_));
  nand2  g83(.a(new_n127_), .b(new_n123_), .O(new_n128_));
  nand3  g84(.a(new_n128_), .b(new_n121_), .c(x09), .O(new_n129_));
  inv1   g85(.a(new_n129_), .O(z12));
  nor3   g86(.a(new_n49_), .b(x19), .c(new_n44_), .O(z13));
  nor3   g87(.a(x09), .b(x02), .c(x00), .O(new_n132_));
  nor2   g88(.a(x19), .b(x18), .O(new_n133_));
  nand4  g89(.a(new_n133_), .b(new_n132_), .c(new_n44_), .d(new_n57_), .O(new_n134_));
  aoi21  g90(.a(new_n134_), .b(x05), .c(x01), .O(z14));
  aoi21  g91(.a(new_n57_), .b(x01), .c(new_n48_), .O(new_n136_));
  aoi21  g92(.a(x19), .b(new_n48_), .c(new_n74_), .O(new_n137_));
  oai22  g93(.a(new_n137_), .b(x01), .c(new_n136_), .d(new_n46_), .O(z15));
  nor2   g94(.a(new_n47_), .b(x00), .O(z16));
  nand2  g95(.a(x02), .b(new_n46_), .O(new_n140_));
  aoi21  g96(.a(new_n140_), .b(x05), .c(x01), .O(z17));
  zero   g97(.O(z08));
endmodule


