// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n60_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n118_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_, new_n126_, new_n127_,
    new_n128_, new_n129_, new_n131_, new_n132_, new_n133_, new_n134_,
    new_n135_, new_n136_, new_n137_, new_n138_, new_n139_, new_n141_,
    new_n143_, new_n144_, new_n145_, new_n147_, new_n148_, new_n149_,
    new_n150_, new_n152_, new_n154_;
  nand2  g000(.a(x17), .b(x16), .O(new_n44_));
  inv1   g001(.a(x02), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  nand3  g004(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g005(.a(x17), .O(new_n49_));
  inv1   g006(.a(x18), .O(new_n50_));
  inv1   g007(.a(x19), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g009(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g010(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  inv1   g011(.a(new_n54_), .O(new_n55_));
  nand4  g012(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n56_));
  nor2   g013(.a(new_n56_), .b(x19), .O(z01));
  nand3  g014(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n58_));
  inv1   g015(.a(x10), .O(new_n59_));
  nand4  g016(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n59_), .O(new_n60_));
  oai21  g017(.a(new_n60_), .b(new_n58_), .c(new_n44_), .O(z02));
  nand4  g018(.a(new_n44_), .b(x12), .c(x11), .d(x10), .O(new_n62_));
  nor2   g019(.a(new_n62_), .b(x09), .O(new_n63_));
  nand4  g020(.a(new_n63_), .b(x02), .c(x01), .d(x00), .O(new_n64_));
  nor3   g021(.a(x02), .b(x01), .c(x00), .O(new_n65_));
  nand4  g022(.a(new_n65_), .b(new_n51_), .c(x18), .d(new_n49_), .O(new_n66_));
  nand2  g023(.a(new_n66_), .b(new_n64_), .O(z03));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g025(.a(x11), .O(new_n69_));
  inv1   g026(.a(x12), .O(new_n70_));
  nand4  g027(.a(new_n70_), .b(new_n69_), .c(x10), .d(new_n46_), .O(new_n71_));
  oai21  g028(.a(new_n71_), .b(new_n68_), .c(new_n44_), .O(z04));
  inv1   g029(.a(x00), .O(new_n73_));
  inv1   g030(.a(x01), .O(new_n74_));
  nand4  g031(.a(new_n44_), .b(x10), .c(x09), .d(x02), .O(new_n75_));
  nor3   g032(.a(new_n75_), .b(new_n74_), .c(new_n73_), .O(z05));
  nand4  g033(.a(new_n44_), .b(x11), .c(x10), .d(new_n46_), .O(new_n77_));
  inv1   g034(.a(new_n77_), .O(new_n78_));
  nand4  g035(.a(new_n78_), .b(x02), .c(x01), .d(x00), .O(new_n79_));
  inv1   g036(.a(new_n79_), .O(z06));
  nand4  g037(.a(new_n44_), .b(new_n70_), .c(x11), .d(x10), .O(new_n81_));
  oai21  g038(.a(new_n81_), .b(x09), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n44_), .O(z07));
  inv1   g041(.a(x16), .O(new_n85_));
  inv1   g042(.a(x03), .O(new_n86_));
  nand3  g043(.a(x04), .b(new_n86_), .c(x02), .O(new_n87_));
  inv1   g044(.a(new_n87_), .O(new_n88_));
  nor3   g045(.a(x07), .b(x06), .c(x05), .O(new_n89_));
  inv1   g046(.a(x08), .O(new_n90_));
  nand3  g047(.a(x19), .b(new_n50_), .c(new_n90_), .O(new_n91_));
  inv1   g048(.a(new_n91_), .O(new_n92_));
  nand4  g049(.a(new_n92_), .b(new_n89_), .c(new_n88_), .d(new_n47_), .O(new_n93_));
  aoi21  g050(.a(new_n93_), .b(new_n85_), .c(new_n49_), .O(z08));
  inv1   g051(.a(x21), .O(new_n95_));
  inv1   g052(.a(x22), .O(new_n96_));
  aoi21  g053(.a(x17), .b(x16), .c(x20), .O(new_n97_));
  nand4  g054(.a(new_n97_), .b(new_n51_), .c(x18), .d(x01), .O(new_n98_));
  nor2   g055(.a(new_n45_), .b(x01), .O(new_n99_));
  nand3  g056(.a(new_n99_), .b(x12), .c(new_n69_), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  nor2   g058(.a(x14), .b(x13), .O(new_n102_));
  nand4  g059(.a(new_n102_), .b(x20), .c(new_n85_), .d(new_n101_), .O(new_n103_));
  oai21  g060(.a(new_n103_), .b(new_n100_), .c(new_n98_), .O(new_n104_));
  nand4  g061(.a(new_n104_), .b(new_n96_), .c(new_n95_), .d(new_n73_), .O(new_n105_));
  inv1   g062(.a(new_n105_), .O(z09));
  inv1   g063(.a(x20), .O(new_n107_));
  nand4  g064(.a(new_n44_), .b(x22), .c(x21), .d(new_n107_), .O(new_n108_));
  inv1   g065(.a(new_n108_), .O(new_n109_));
  nand4  g066(.a(new_n109_), .b(new_n51_), .c(x18), .d(x01), .O(new_n110_));
  nand3  g067(.a(new_n69_), .b(x02), .c(new_n74_), .O(new_n111_));
  inv1   g068(.a(new_n111_), .O(new_n112_));
  nand3  g069(.a(new_n49_), .b(x16), .c(x15), .O(new_n113_));
  nand3  g070(.a(new_n96_), .b(new_n95_), .c(x20), .O(new_n114_));
  nor2   g071(.a(new_n114_), .b(new_n113_), .O(new_n115_));
  nand4  g072(.a(new_n115_), .b(new_n112_), .c(new_n102_), .d(x12), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n110_), .c(x00), .O(z10));
  nand4  g074(.a(new_n44_), .b(x21), .c(new_n107_), .d(new_n51_), .O(new_n118_));
  inv1   g075(.a(new_n118_), .O(new_n119_));
  nand3  g076(.a(new_n119_), .b(x18), .c(x01), .O(new_n120_));
  inv1   g077(.a(x13), .O(new_n121_));
  nand3  g078(.a(new_n121_), .b(x12), .c(new_n69_), .O(new_n122_));
  inv1   g079(.a(new_n122_), .O(new_n123_));
  nor2   g080(.a(new_n101_), .b(x14), .O(new_n124_));
  nand3  g081(.a(new_n95_), .b(x20), .c(new_n85_), .O(new_n125_));
  inv1   g082(.a(new_n125_), .O(new_n126_));
  nand4  g083(.a(new_n126_), .b(new_n124_), .c(new_n123_), .d(new_n99_), .O(new_n127_));
  nand2  g084(.a(new_n127_), .b(new_n120_), .O(new_n128_));
  nand3  g085(.a(new_n128_), .b(new_n96_), .c(new_n73_), .O(new_n129_));
  inv1   g086(.a(new_n129_), .O(z11));
  inv1   g087(.a(x24), .O(new_n131_));
  nand2  g088(.a(x10), .b(x02), .O(new_n132_));
  nand3  g089(.a(new_n132_), .b(x01), .c(x00), .O(new_n133_));
  oai21  g090(.a(x19), .b(x02), .c(x23), .O(new_n134_));
  nand3  g091(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n135_));
  nand2  g092(.a(new_n135_), .b(new_n134_), .O(new_n136_));
  nand3  g093(.a(new_n136_), .b(new_n74_), .c(new_n73_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n133_), .O(new_n138_));
  nand3  g095(.a(new_n138_), .b(new_n131_), .c(x09), .O(new_n139_));
  nand2  g096(.a(new_n139_), .b(new_n44_), .O(z12));
  nand4  g097(.a(new_n47_), .b(x17), .c(new_n85_), .d(new_n45_), .O(new_n141_));
  nor2   g098(.a(new_n141_), .b(x19), .O(z13));
  nand3  g099(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n143_));
  inv1   g100(.a(new_n143_), .O(new_n144_));
  nand4  g101(.a(new_n144_), .b(new_n50_), .c(new_n49_), .d(new_n59_), .O(new_n145_));
  nor2   g102(.a(new_n145_), .b(x19), .O(z14));
  inv1   g103(.a(new_n44_), .O(new_n147_));
  oai21  g104(.a(x10), .b(new_n74_), .c(x02), .O(new_n148_));
  nand2  g105(.a(new_n148_), .b(x00), .O(new_n149_));
  nand3  g106(.a(x19), .b(new_n45_), .c(new_n74_), .O(new_n150_));
  aoi21  g107(.a(new_n150_), .b(new_n149_), .c(new_n147_), .O(z15));
  nand3  g108(.a(new_n44_), .b(x01), .c(new_n73_), .O(new_n152_));
  inv1   g109(.a(new_n152_), .O(z16));
  nand2  g110(.a(new_n99_), .b(new_n73_), .O(new_n154_));
  nand2  g111(.a(new_n154_), .b(new_n44_), .O(z17));
endmodule


