// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n55_, new_n57_, new_n58_, new_n59_,
    new_n60_, new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n108_,
    new_n109_, new_n110_, new_n112_, new_n113_, new_n115_, new_n116_,
    new_n117_, new_n118_, new_n119_, new_n120_, new_n121_, new_n122_,
    new_n123_, new_n125_, new_n126_, new_n128_, new_n130_, new_n131_,
    new_n132_;
  inv1   g00(.a(x13), .O(new_n44_));
  nand2  g01(.a(x15), .b(new_n44_), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  nor2   g08(.a(x19), .b(x18), .O(new_n52_));
  nand3  g09(.a(new_n52_), .b(new_n51_), .c(x10), .O(new_n53_));
  oai21  g10(.a(new_n53_), .b(new_n50_), .c(new_n45_), .O(z00));
  nand4  g11(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  oai21  g12(.a(new_n55_), .b(new_n53_), .c(new_n45_), .O(z01));
  inv1   g13(.a(x18), .O(new_n57_));
  inv1   g14(.a(x19), .O(new_n58_));
  nand4  g15(.a(new_n45_), .b(new_n58_), .c(new_n57_), .d(new_n51_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x10), .O(new_n60_));
  nand4  g17(.a(new_n60_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x00), .O(z02));
  inv1   g19(.a(new_n45_), .O(new_n63_));
  nand3  g20(.a(x02), .b(x01), .c(x00), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  inv1   g22(.a(x10), .O(new_n66_));
  nor2   g23(.a(new_n66_), .b(x09), .O(new_n67_));
  nand4  g24(.a(new_n67_), .b(new_n65_), .c(x12), .d(x11), .O(new_n68_));
  nor3   g25(.a(x02), .b(x01), .c(x00), .O(new_n69_));
  nand4  g26(.a(new_n69_), .b(new_n58_), .c(x18), .d(new_n51_), .O(new_n70_));
  aoi21  g27(.a(new_n70_), .b(new_n68_), .c(new_n63_), .O(z03));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n67_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n64_), .c(new_n45_), .O(z04));
  nor2   g32(.a(new_n63_), .b(new_n66_), .O(new_n76_));
  nand4  g33(.a(new_n76_), .b(x09), .c(x02), .d(x01), .O(new_n77_));
  nor2   g34(.a(new_n77_), .b(new_n46_), .O(z05));
  nand3  g35(.a(x15), .b(new_n44_), .c(x12), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x11), .c(x10), .d(new_n49_), .O(new_n80_));
  inv1   g37(.a(new_n80_), .O(new_n81_));
  nand4  g38(.a(new_n81_), .b(x02), .c(x01), .d(x00), .O(new_n82_));
  nand2  g39(.a(new_n82_), .b(new_n45_), .O(z06));
  nand3  g40(.a(new_n73_), .b(x11), .c(x10), .O(new_n84_));
  oai21  g41(.a(new_n84_), .b(x09), .c(x01), .O(new_n85_));
  nand3  g42(.a(new_n85_), .b(x02), .c(x00), .O(new_n86_));
  nand2  g43(.a(new_n86_), .b(new_n45_), .O(z07));
  nand3  g44(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n88_));
  inv1   g45(.a(x05), .O(new_n89_));
  nand2  g46(.a(new_n89_), .b(x04), .O(new_n90_));
  nor3   g47(.a(new_n90_), .b(new_n88_), .c(x03), .O(new_n91_));
  nor3   g48(.a(x08), .b(x07), .c(x06), .O(new_n92_));
  nor2   g49(.a(new_n58_), .b(x18), .O(new_n93_));
  nand4  g50(.a(new_n93_), .b(new_n92_), .c(new_n91_), .d(x17), .O(new_n94_));
  nand2  g51(.a(new_n94_), .b(new_n45_), .O(z08));
  inv1   g52(.a(x21), .O(new_n96_));
  inv1   g53(.a(x22), .O(new_n97_));
  nand4  g54(.a(x12), .b(new_n72_), .c(x02), .d(new_n47_), .O(new_n98_));
  inv1   g55(.a(x14), .O(new_n99_));
  inv1   g56(.a(x16), .O(new_n100_));
  nand4  g57(.a(x20), .b(new_n100_), .c(new_n99_), .d(new_n44_), .O(new_n101_));
  nand2  g58(.a(x18), .b(x01), .O(new_n102_));
  inv1   g59(.a(x20), .O(new_n103_));
  nand2  g60(.a(new_n103_), .b(new_n58_), .O(new_n104_));
  oai22  g61(.a(new_n104_), .b(new_n102_), .c(new_n101_), .d(new_n98_), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(new_n97_), .c(new_n96_), .d(new_n46_), .O(new_n106_));
  nand2  g63(.a(new_n106_), .b(new_n45_), .O(z09));
  nand4  g64(.a(new_n45_), .b(x22), .c(x21), .d(new_n103_), .O(new_n108_));
  inv1   g65(.a(new_n108_), .O(new_n109_));
  nand4  g66(.a(new_n109_), .b(new_n58_), .c(x18), .d(x01), .O(new_n110_));
  nor2   g67(.a(new_n110_), .b(x00), .O(z10));
  nand3  g68(.a(x18), .b(x01), .c(new_n46_), .O(new_n112_));
  nand4  g69(.a(new_n97_), .b(x21), .c(new_n103_), .d(new_n58_), .O(new_n113_));
  oai21  g70(.a(new_n113_), .b(new_n112_), .c(new_n45_), .O(z11));
  inv1   g71(.a(x24), .O(new_n115_));
  nand2  g72(.a(x10), .b(x02), .O(new_n116_));
  nand3  g73(.a(new_n116_), .b(x01), .c(x00), .O(new_n117_));
  oai21  g74(.a(x19), .b(x02), .c(x23), .O(new_n118_));
  nand3  g75(.a(new_n58_), .b(x17), .c(new_n48_), .O(new_n119_));
  nand2  g76(.a(new_n119_), .b(new_n118_), .O(new_n120_));
  nand3  g77(.a(new_n120_), .b(new_n47_), .c(new_n46_), .O(new_n121_));
  nand2  g78(.a(new_n121_), .b(new_n117_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n115_), .c(x09), .O(new_n123_));
  nand2  g80(.a(new_n123_), .b(new_n45_), .O(z12));
  nor2   g81(.a(new_n63_), .b(x19), .O(new_n125_));
  nand4  g82(.a(new_n125_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n126_));
  nor2   g83(.a(new_n126_), .b(x00), .O(z13));
  nand3  g84(.a(new_n52_), .b(new_n51_), .c(new_n66_), .O(new_n128_));
  oai21  g85(.a(new_n128_), .b(new_n50_), .c(new_n45_), .O(z14));
  oai21  g86(.a(x10), .b(new_n47_), .c(x02), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x00), .O(new_n131_));
  nand3  g88(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n132_));
  nand3  g89(.a(new_n132_), .b(new_n131_), .c(new_n45_), .O(z15));
  oai21  g90(.a(new_n47_), .b(x00), .c(new_n45_), .O(z16));
  nand2  g91(.a(new_n88_), .b(new_n45_), .O(z17));
endmodule


