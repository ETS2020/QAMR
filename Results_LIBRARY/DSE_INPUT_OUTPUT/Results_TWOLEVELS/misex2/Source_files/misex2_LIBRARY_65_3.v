// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_,
    new_n61_, new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_,
    new_n73_, new_n74_, new_n75_, new_n76_, new_n77_, new_n78_, new_n79_,
    new_n80_, new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_,
    new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_, new_n95_,
    new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n105_, new_n106_, new_n107_, new_n108_,
    new_n109_, new_n110_, new_n111_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n120_, new_n121_, new_n122_, new_n124_;
  nand2  g00(.a(x01), .b(x00), .O(new_n44_));
  inv1   g01(.a(x02), .O(new_n45_));
  inv1   g02(.a(x09), .O(new_n46_));
  nor2   g03(.a(x01), .b(x00), .O(new_n47_));
  nand3  g04(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n48_));
  inv1   g05(.a(x17), .O(new_n49_));
  inv1   g06(.a(x18), .O(new_n50_));
  inv1   g07(.a(x19), .O(new_n51_));
  nand4  g08(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n52_));
  oai21  g09(.a(new_n52_), .b(new_n48_), .c(new_n44_), .O(z00));
  nand3  g10(.a(new_n47_), .b(x09), .c(new_n45_), .O(new_n54_));
  inv1   g11(.a(new_n54_), .O(new_n55_));
  nand4  g12(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(x10), .O(new_n56_));
  nor2   g13(.a(new_n56_), .b(x19), .O(z01));
  inv1   g14(.a(x10), .O(new_n58_));
  nand4  g15(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x19), .O(z02));
  nand4  g17(.a(new_n47_), .b(x18), .c(new_n49_), .d(new_n45_), .O(new_n61_));
  nor2   g18(.a(new_n61_), .b(x19), .O(z03));
  inv1   g19(.a(new_n44_), .O(z04));
  inv1   g20(.a(x01), .O(new_n66_));
  nand3  g21(.a(x02), .b(new_n66_), .c(x00), .O(new_n67_));
  inv1   g22(.a(new_n67_), .O(z07));
  inv1   g23(.a(x00), .O(new_n69_));
  nor2   g24(.a(new_n45_), .b(x01), .O(new_n70_));
  nand2  g25(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  inv1   g26(.a(new_n71_), .O(new_n72_));
  inv1   g27(.a(x03), .O(new_n73_));
  inv1   g28(.a(x05), .O(new_n74_));
  nand3  g29(.a(new_n74_), .b(x04), .c(new_n73_), .O(new_n75_));
  inv1   g30(.a(new_n75_), .O(new_n76_));
  nor3   g31(.a(x08), .b(x07), .c(x06), .O(new_n77_));
  nand3  g32(.a(x19), .b(new_n50_), .c(x17), .O(new_n78_));
  inv1   g33(.a(new_n78_), .O(new_n79_));
  nand4  g34(.a(new_n79_), .b(new_n77_), .c(new_n76_), .d(new_n72_), .O(new_n80_));
  nand2  g35(.a(new_n80_), .b(new_n44_), .O(z08));
  inv1   g36(.a(x21), .O(new_n82_));
  inv1   g37(.a(x22), .O(new_n83_));
  inv1   g38(.a(x11), .O(new_n84_));
  nand4  g39(.a(x12), .b(new_n84_), .c(x02), .d(new_n66_), .O(new_n85_));
  inv1   g40(.a(x15), .O(new_n86_));
  inv1   g41(.a(x16), .O(new_n87_));
  nor2   g42(.a(x14), .b(x13), .O(new_n88_));
  nand4  g43(.a(new_n88_), .b(x20), .c(new_n87_), .d(new_n86_), .O(new_n89_));
  inv1   g44(.a(x20), .O(new_n90_));
  nand4  g45(.a(new_n90_), .b(new_n51_), .c(x18), .d(x01), .O(new_n91_));
  oai21  g46(.a(new_n89_), .b(new_n85_), .c(new_n91_), .O(new_n92_));
  nand4  g47(.a(new_n92_), .b(new_n83_), .c(new_n82_), .d(new_n69_), .O(new_n93_));
  inv1   g48(.a(new_n93_), .O(z09));
  nor3   g49(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n95_));
  nand4  g50(.a(new_n95_), .b(new_n51_), .c(x18), .d(x01), .O(new_n96_));
  inv1   g51(.a(x13), .O(new_n97_));
  nand3  g52(.a(new_n97_), .b(x12), .c(new_n84_), .O(new_n98_));
  inv1   g53(.a(new_n98_), .O(new_n99_));
  nor3   g54(.a(new_n87_), .b(new_n86_), .c(x14), .O(new_n100_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(x20), .O(new_n101_));
  inv1   g56(.a(new_n101_), .O(new_n102_));
  nand4  g57(.a(new_n102_), .b(new_n100_), .c(new_n99_), .d(new_n70_), .O(new_n103_));
  aoi21  g58(.a(new_n103_), .b(new_n96_), .c(x00), .O(z10));
  nand2  g59(.a(new_n51_), .b(x18), .O(new_n105_));
  nand3  g60(.a(new_n83_), .b(x21), .c(new_n90_), .O(new_n106_));
  oai21  g61(.a(new_n106_), .b(new_n105_), .c(new_n69_), .O(new_n107_));
  nand2  g62(.a(new_n107_), .b(x01), .O(new_n108_));
  nand2  g63(.a(new_n87_), .b(x15), .O(new_n109_));
  nor3   g64(.a(new_n109_), .b(new_n101_), .c(x14), .O(new_n110_));
  nand3  g65(.a(new_n110_), .b(new_n99_), .c(new_n72_), .O(new_n111_));
  nand2  g66(.a(new_n111_), .b(new_n108_), .O(z11));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n113_));
  nand3  g68(.a(new_n51_), .b(x17), .c(new_n45_), .O(new_n114_));
  aoi21  g69(.a(new_n114_), .b(new_n113_), .c(x24), .O(new_n115_));
  nand4  g70(.a(new_n115_), .b(x09), .c(new_n66_), .d(new_n69_), .O(new_n116_));
  nand2  g71(.a(new_n116_), .b(new_n44_), .O(z12));
  inv1   g72(.a(new_n47_), .O(new_n118_));
  oai21  g73(.a(new_n114_), .b(new_n118_), .c(new_n44_), .O(z13));
  nand3  g74(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n120_));
  inv1   g75(.a(new_n120_), .O(new_n121_));
  nand4  g76(.a(new_n121_), .b(new_n50_), .c(new_n49_), .d(new_n58_), .O(new_n122_));
  nor2   g77(.a(new_n122_), .b(x19), .O(z14));
  nor2   g78(.a(x19), .b(x00), .O(new_n124_));
  nor3   g79(.a(new_n124_), .b(x02), .c(x01), .O(z15));
  nand2  g80(.a(new_n71_), .b(new_n44_), .O(z17));
  zero   g81(.O(z05));
  zero   g82(.O(z06));
  buf    g83(.a(x01), .O(z16));
endmodule


