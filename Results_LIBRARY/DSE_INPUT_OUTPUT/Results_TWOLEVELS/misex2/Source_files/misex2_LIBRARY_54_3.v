// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_,
    new_n60_, new_n61_, new_n62_, new_n63_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n85_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n132_, new_n135_, new_n136_,
    new_n137_, new_n139_, new_n140_, new_n142_, new_n144_;
  inv1   g000(.a(x17), .O(new_n44_));
  inv1   g001(.a(x18), .O(new_n45_));
  inv1   g002(.a(x09), .O(new_n46_));
  nor2   g003(.a(x01), .b(x00), .O(new_n47_));
  inv1   g004(.a(new_n47_), .O(new_n48_));
  nor2   g005(.a(new_n48_), .b(x02), .O(new_n49_));
  nand2  g006(.a(new_n49_), .b(new_n46_), .O(new_n50_));
  inv1   g007(.a(new_n50_), .O(new_n51_));
  nand4  g008(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n52_));
  nor2   g009(.a(new_n52_), .b(x19), .O(z00));
  nand2  g010(.a(x12), .b(x01), .O(new_n54_));
  inv1   g011(.a(x02), .O(new_n55_));
  nand3  g012(.a(new_n47_), .b(x09), .c(new_n55_), .O(new_n56_));
  nor2   g013(.a(x19), .b(x18), .O(new_n57_));
  nand3  g014(.a(new_n57_), .b(new_n44_), .c(x10), .O(new_n58_));
  oai21  g015(.a(new_n58_), .b(new_n56_), .c(new_n54_), .O(z01));
  inv1   g016(.a(x10), .O(new_n60_));
  nand2  g017(.a(new_n49_), .b(x09), .O(new_n61_));
  inv1   g018(.a(new_n61_), .O(new_n62_));
  nand4  g019(.a(new_n62_), .b(new_n45_), .c(new_n44_), .d(new_n60_), .O(new_n63_));
  nor2   g020(.a(new_n63_), .b(x19), .O(z02));
  nand3  g021(.a(new_n49_), .b(x18), .c(new_n44_), .O(new_n65_));
  nor2   g022(.a(new_n65_), .b(x19), .O(z03));
  inv1   g023(.a(x11), .O(new_n67_));
  nand3  g024(.a(x02), .b(x01), .c(x00), .O(new_n68_));
  inv1   g025(.a(new_n68_), .O(new_n69_));
  nand4  g026(.a(new_n69_), .b(new_n67_), .c(x10), .d(new_n46_), .O(new_n70_));
  nor2   g027(.a(new_n70_), .b(x12), .O(z04));
  inv1   g028(.a(x01), .O(new_n72_));
  inv1   g029(.a(x12), .O(new_n73_));
  inv1   g030(.a(x00), .O(new_n74_));
  nor2   g031(.a(new_n55_), .b(new_n74_), .O(new_n75_));
  nand3  g032(.a(new_n75_), .b(x10), .c(x09), .O(new_n76_));
  aoi21  g033(.a(new_n76_), .b(new_n73_), .c(new_n72_), .O(z05));
  nand2  g034(.a(x11), .b(x10), .O(new_n78_));
  nor2   g035(.a(new_n78_), .b(x09), .O(new_n79_));
  nand2  g036(.a(new_n79_), .b(new_n75_), .O(new_n80_));
  aoi21  g037(.a(new_n80_), .b(new_n73_), .c(new_n72_), .O(z06));
  oai21  g038(.a(new_n78_), .b(x09), .c(x01), .O(new_n82_));
  nand3  g039(.a(new_n82_), .b(x02), .c(x00), .O(new_n83_));
  nand2  g040(.a(new_n83_), .b(new_n54_), .O(z07));
  inv1   g041(.a(x19), .O(new_n85_));
  inv1   g042(.a(x08), .O(new_n86_));
  inv1   g043(.a(x05), .O(new_n87_));
  inv1   g044(.a(x06), .O(new_n88_));
  inv1   g045(.a(x03), .O(new_n89_));
  nand4  g046(.a(new_n89_), .b(x02), .c(new_n72_), .d(new_n74_), .O(new_n90_));
  inv1   g047(.a(new_n90_), .O(new_n91_));
  nand4  g048(.a(new_n91_), .b(new_n88_), .c(new_n87_), .d(x04), .O(new_n92_));
  nor2   g049(.a(new_n92_), .b(x07), .O(new_n93_));
  nand4  g050(.a(new_n93_), .b(new_n45_), .c(x17), .d(new_n86_), .O(new_n94_));
  nor2   g051(.a(new_n94_), .b(new_n85_), .O(z08));
  inv1   g052(.a(x21), .O(new_n96_));
  inv1   g053(.a(x22), .O(new_n97_));
  inv1   g054(.a(x13), .O(new_n98_));
  nand4  g055(.a(new_n98_), .b(x12), .c(new_n67_), .d(x02), .O(new_n99_));
  inv1   g056(.a(x14), .O(new_n100_));
  inv1   g057(.a(x15), .O(new_n101_));
  inv1   g058(.a(x16), .O(new_n102_));
  nand4  g059(.a(x20), .b(new_n102_), .c(new_n101_), .d(new_n100_), .O(new_n103_));
  inv1   g060(.a(x20), .O(new_n104_));
  nor2   g061(.a(new_n45_), .b(new_n72_), .O(new_n105_));
  nand3  g062(.a(new_n105_), .b(new_n104_), .c(new_n85_), .O(new_n106_));
  oai21  g063(.a(new_n103_), .b(new_n99_), .c(new_n106_), .O(new_n107_));
  nand4  g064(.a(new_n107_), .b(new_n97_), .c(new_n96_), .d(new_n74_), .O(new_n108_));
  nand2  g065(.a(new_n108_), .b(new_n54_), .O(z09));
  nand3  g066(.a(new_n85_), .b(x18), .c(x01), .O(new_n110_));
  nand3  g067(.a(x22), .b(x21), .c(new_n104_), .O(new_n111_));
  nor2   g068(.a(new_n111_), .b(new_n110_), .O(new_n112_));
  nand3  g069(.a(new_n100_), .b(new_n98_), .c(x12), .O(new_n113_));
  nor3   g070(.a(new_n113_), .b(x11), .c(new_n55_), .O(new_n114_));
  nand3  g071(.a(new_n97_), .b(new_n96_), .c(x20), .O(new_n115_));
  nor3   g072(.a(new_n115_), .b(new_n102_), .c(new_n101_), .O(new_n116_));
  aoi21  g073(.a(new_n116_), .b(new_n114_), .c(new_n112_), .O(new_n117_));
  oai21  g074(.a(new_n117_), .b(x00), .c(new_n54_), .O(z10));
  nand4  g075(.a(new_n105_), .b(x21), .c(new_n104_), .d(new_n85_), .O(new_n119_));
  nor2   g076(.a(new_n101_), .b(x14), .O(new_n120_));
  nand4  g077(.a(new_n120_), .b(new_n96_), .c(x20), .d(new_n102_), .O(new_n121_));
  oai21  g078(.a(new_n121_), .b(new_n99_), .c(new_n119_), .O(new_n122_));
  nand3  g079(.a(new_n122_), .b(new_n97_), .c(new_n74_), .O(new_n123_));
  nand2  g080(.a(new_n123_), .b(new_n54_), .O(z11));
  nand2  g081(.a(x10), .b(x02), .O(new_n125_));
  nand4  g082(.a(new_n125_), .b(new_n73_), .c(x01), .d(x00), .O(new_n126_));
  oai21  g083(.a(x19), .b(x02), .c(x23), .O(new_n127_));
  nand3  g084(.a(new_n85_), .b(x17), .c(new_n55_), .O(new_n128_));
  nand2  g085(.a(new_n128_), .b(new_n127_), .O(new_n129_));
  nand3  g086(.a(new_n129_), .b(new_n72_), .c(new_n74_), .O(new_n130_));
  aoi21  g087(.a(new_n130_), .b(new_n126_), .c(x24), .O(new_n131_));
  nand2  g088(.a(new_n131_), .b(x09), .O(new_n132_));
  inv1   g089(.a(new_n132_), .O(z12));
  oai21  g090(.a(new_n128_), .b(new_n48_), .c(new_n54_), .O(z13));
  nor2   g091(.a(x09), .b(x02), .O(new_n135_));
  nor2   g092(.a(x17), .b(x10), .O(new_n136_));
  nand4  g093(.a(new_n136_), .b(new_n135_), .c(new_n57_), .d(new_n47_), .O(new_n137_));
  nand2  g094(.a(new_n137_), .b(new_n54_), .O(z14));
  aoi22  g095(.a(new_n54_), .b(x00), .c(x19), .d(new_n72_), .O(new_n139_));
  nand4  g096(.a(new_n73_), .b(new_n60_), .c(x01), .d(x00), .O(new_n140_));
  oai21  g097(.a(new_n139_), .b(x02), .c(new_n140_), .O(z15));
  nand3  g098(.a(new_n73_), .b(x01), .c(new_n74_), .O(new_n142_));
  inv1   g099(.a(new_n142_), .O(z16));
  nand3  g100(.a(x02), .b(new_n72_), .c(new_n74_), .O(new_n144_));
  nand2  g101(.a(new_n144_), .b(new_n54_), .O(z17));
endmodule


