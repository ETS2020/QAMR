// Benchmark "FAU" written by ABC on Wed Aug 19 14:56:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_,
    new_n60_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_, new_n86_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_, new_n101_,
    new_n102_, new_n103_, new_n104_, new_n105_, new_n106_, new_n107_,
    new_n108_, new_n109_, new_n111_, new_n112_, new_n113_, new_n114_,
    new_n115_, new_n116_, new_n117_, new_n119_, new_n120_, new_n121_,
    new_n122_, new_n123_, new_n125_, new_n126_, new_n127_, new_n128_,
    new_n129_, new_n130_, new_n131_, new_n133_, new_n135_, new_n136_,
    new_n138_, new_n141_;
  inv1   g00(.a(x17), .O(new_n44_));
  inv1   g01(.a(x18), .O(new_n45_));
  inv1   g02(.a(x02), .O(new_n46_));
  inv1   g03(.a(x09), .O(new_n47_));
  nor2   g04(.a(x01), .b(x00), .O(new_n48_));
  nand3  g05(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n49_));
  inv1   g06(.a(new_n49_), .O(new_n50_));
  nand4  g07(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(x10), .O(new_n51_));
  nor2   g08(.a(new_n51_), .b(x19), .O(z00));
  inv1   g09(.a(x19), .O(new_n53_));
  inv1   g10(.a(x00), .O(new_n54_));
  nor2   g11(.a(new_n47_), .b(x01), .O(new_n55_));
  nor2   g12(.a(x18), .b(x17), .O(new_n56_));
  nand4  g13(.a(new_n56_), .b(new_n55_), .c(x10), .d(new_n54_), .O(new_n57_));
  aoi21  g14(.a(new_n57_), .b(new_n53_), .c(x02), .O(z01));
  inv1   g15(.a(x10), .O(new_n59_));
  nand3  g16(.a(new_n48_), .b(x09), .c(new_n46_), .O(new_n60_));
  inv1   g17(.a(new_n60_), .O(new_n61_));
  nand4  g18(.a(new_n61_), .b(new_n45_), .c(new_n44_), .d(new_n59_), .O(new_n62_));
  nor2   g19(.a(new_n62_), .b(x19), .O(z02));
  nand3  g20(.a(new_n48_), .b(x18), .c(new_n44_), .O(new_n64_));
  nand2  g21(.a(new_n64_), .b(new_n53_), .O(new_n65_));
  nand2  g22(.a(new_n65_), .b(new_n46_), .O(new_n66_));
  nand3  g23(.a(x02), .b(x01), .c(x00), .O(new_n67_));
  nor2   g24(.a(new_n59_), .b(x09), .O(new_n68_));
  nand3  g25(.a(new_n68_), .b(x12), .c(x11), .O(new_n69_));
  oai21  g26(.a(new_n69_), .b(new_n67_), .c(new_n66_), .O(z03));
  nand2  g27(.a(x19), .b(new_n46_), .O(new_n71_));
  inv1   g28(.a(x11), .O(new_n72_));
  inv1   g29(.a(x12), .O(new_n73_));
  nand3  g30(.a(new_n68_), .b(new_n73_), .c(new_n72_), .O(new_n74_));
  oai21  g31(.a(new_n74_), .b(new_n67_), .c(new_n71_), .O(z04));
  nand2  g32(.a(x01), .b(x00), .O(new_n76_));
  nand3  g33(.a(x10), .b(x09), .c(x02), .O(new_n77_));
  oai21  g34(.a(new_n77_), .b(new_n76_), .c(new_n71_), .O(z05));
  nor3   g35(.a(new_n72_), .b(new_n59_), .c(x09), .O(new_n79_));
  nand4  g36(.a(new_n79_), .b(x02), .c(x01), .d(x00), .O(new_n80_));
  nand2  g37(.a(new_n80_), .b(new_n71_), .O(z06));
  nand3  g38(.a(new_n73_), .b(x11), .c(x10), .O(new_n82_));
  oai21  g39(.a(new_n82_), .b(x09), .c(x01), .O(new_n83_));
  nand3  g40(.a(new_n83_), .b(x02), .c(x00), .O(new_n84_));
  inv1   g41(.a(new_n84_), .O(z07));
  inv1   g42(.a(x08), .O(new_n86_));
  inv1   g43(.a(x05), .O(new_n87_));
  inv1   g44(.a(x06), .O(new_n88_));
  inv1   g45(.a(x01), .O(new_n89_));
  inv1   g46(.a(x03), .O(new_n90_));
  nand4  g47(.a(new_n90_), .b(x02), .c(new_n89_), .d(new_n54_), .O(new_n91_));
  inv1   g48(.a(new_n91_), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n88_), .c(new_n87_), .d(x04), .O(new_n93_));
  nor2   g50(.a(new_n93_), .b(x07), .O(new_n94_));
  nand4  g51(.a(new_n94_), .b(new_n45_), .c(x17), .d(new_n86_), .O(new_n95_));
  nor2   g52(.a(new_n95_), .b(new_n53_), .O(z08));
  inv1   g53(.a(x21), .O(new_n97_));
  inv1   g54(.a(x22), .O(new_n98_));
  nor2   g55(.a(new_n46_), .b(x01), .O(new_n99_));
  nand3  g56(.a(new_n99_), .b(x12), .c(new_n72_), .O(new_n100_));
  inv1   g57(.a(x15), .O(new_n101_));
  inv1   g58(.a(x16), .O(new_n102_));
  nor2   g59(.a(x14), .b(x13), .O(new_n103_));
  nand4  g60(.a(new_n103_), .b(x20), .c(new_n102_), .d(new_n101_), .O(new_n104_));
  inv1   g61(.a(x20), .O(new_n105_));
  nor2   g62(.a(new_n45_), .b(new_n89_), .O(new_n106_));
  nand3  g63(.a(new_n106_), .b(new_n105_), .c(new_n53_), .O(new_n107_));
  oai21  g64(.a(new_n104_), .b(new_n100_), .c(new_n107_), .O(new_n108_));
  nand4  g65(.a(new_n108_), .b(new_n98_), .c(new_n97_), .d(new_n54_), .O(new_n109_));
  nand2  g66(.a(new_n109_), .b(new_n71_), .O(z09));
  nor3   g67(.a(new_n98_), .b(new_n97_), .c(x20), .O(new_n111_));
  nand4  g68(.a(new_n111_), .b(new_n53_), .c(x18), .d(x01), .O(new_n112_));
  nor2   g69(.a(x13), .b(new_n73_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n99_), .c(new_n72_), .O(new_n114_));
  nor3   g71(.a(new_n102_), .b(new_n101_), .c(x14), .O(new_n115_));
  nand4  g72(.a(new_n115_), .b(new_n98_), .c(new_n97_), .d(x20), .O(new_n116_));
  or2    g73(.a(new_n116_), .b(new_n114_), .O(new_n117_));
  aoi21  g74(.a(new_n117_), .b(new_n112_), .c(x00), .O(z10));
  nand4  g75(.a(new_n106_), .b(x21), .c(new_n105_), .d(new_n53_), .O(new_n119_));
  nor2   g76(.a(new_n101_), .b(x14), .O(new_n120_));
  nand4  g77(.a(new_n120_), .b(new_n97_), .c(x20), .d(new_n102_), .O(new_n121_));
  oai21  g78(.a(new_n121_), .b(new_n114_), .c(new_n119_), .O(new_n122_));
  nand3  g79(.a(new_n122_), .b(new_n98_), .c(new_n54_), .O(new_n123_));
  inv1   g80(.a(new_n123_), .O(z11));
  nand2  g81(.a(x10), .b(x02), .O(new_n125_));
  nand3  g82(.a(new_n125_), .b(x01), .c(x00), .O(new_n126_));
  nand2  g83(.a(x23), .b(x02), .O(new_n127_));
  oai21  g84(.a(new_n44_), .b(x02), .c(new_n127_), .O(new_n128_));
  nand3  g85(.a(new_n128_), .b(new_n89_), .c(new_n54_), .O(new_n129_));
  aoi21  g86(.a(new_n129_), .b(new_n126_), .c(x24), .O(new_n130_));
  nand2  g87(.a(new_n130_), .b(x09), .O(new_n131_));
  nand2  g88(.a(new_n131_), .b(new_n71_), .O(z12));
  nand3  g89(.a(x17), .b(new_n89_), .c(new_n54_), .O(new_n133_));
  aoi21  g90(.a(new_n133_), .b(new_n53_), .c(x02), .O(z13));
  nor2   g91(.a(x09), .b(x01), .O(new_n135_));
  nand4  g92(.a(new_n135_), .b(new_n56_), .c(new_n59_), .d(new_n54_), .O(new_n136_));
  aoi21  g93(.a(new_n136_), .b(new_n53_), .c(x02), .O(z14));
  aoi21  g94(.a(new_n59_), .b(x01), .c(new_n46_), .O(new_n138_));
  oai21  g95(.a(new_n138_), .b(new_n54_), .c(new_n71_), .O(z15));
  oai21  g96(.a(new_n89_), .b(x00), .c(new_n71_), .O(z16));
  nand2  g97(.a(new_n48_), .b(x02), .O(new_n141_));
  inv1   g98(.a(new_n141_), .O(z17));
endmodule


