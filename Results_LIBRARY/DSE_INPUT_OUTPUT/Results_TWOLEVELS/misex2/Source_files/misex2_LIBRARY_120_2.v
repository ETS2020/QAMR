// Benchmark "FAU" written by ABC on Wed Aug 19 14:57:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15, z16, z17;
  wire new_n44_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n58_,
    new_n59_, new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_, new_n93_,
    new_n95_, new_n96_, new_n97_, new_n98_, new_n100_, new_n101_,
    new_n102_, new_n104_, new_n105_, new_n106_, new_n108_, new_n109_,
    new_n110_, new_n111_, new_n112_, new_n113_, new_n114_, new_n115_,
    new_n116_, new_n118_, new_n119_, new_n121_, new_n122_, new_n124_,
    new_n125_, new_n126_;
  inv1   g00(.a(x21), .O(new_n44_));
  nand2  g01(.a(new_n44_), .b(x12), .O(new_n45_));
  inv1   g02(.a(x00), .O(new_n46_));
  inv1   g03(.a(x01), .O(new_n47_));
  inv1   g04(.a(x02), .O(new_n48_));
  inv1   g05(.a(x09), .O(new_n49_));
  nand4  g06(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g07(.a(x17), .O(new_n51_));
  inv1   g08(.a(x18), .O(new_n52_));
  inv1   g09(.a(x19), .O(new_n53_));
  nand4  g10(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(x10), .O(new_n54_));
  oai21  g11(.a(new_n54_), .b(new_n50_), .c(new_n45_), .O(z00));
  inv1   g12(.a(x10), .O(new_n56_));
  nand4  g13(.a(new_n45_), .b(new_n53_), .c(new_n52_), .d(new_n51_), .O(new_n57_));
  nor2   g14(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand4  g15(.a(new_n58_), .b(x09), .c(new_n48_), .d(new_n47_), .O(new_n59_));
  nor2   g16(.a(new_n59_), .b(x00), .O(z01));
  nand4  g17(.a(x09), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n61_));
  nand4  g18(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n56_), .O(new_n62_));
  oai21  g19(.a(new_n62_), .b(new_n61_), .c(new_n45_), .O(z02));
  nand4  g20(.a(new_n45_), .b(new_n53_), .c(x18), .d(new_n51_), .O(new_n64_));
  inv1   g21(.a(new_n64_), .O(new_n65_));
  nand4  g22(.a(new_n65_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n66_));
  nand4  g23(.a(new_n49_), .b(x02), .c(x01), .d(x00), .O(new_n67_));
  nand4  g24(.a(x21), .b(x12), .c(x11), .d(x10), .O(new_n68_));
  oai21  g25(.a(new_n68_), .b(new_n67_), .c(new_n66_), .O(z03));
  nand3  g26(.a(x02), .b(x01), .c(x00), .O(new_n70_));
  inv1   g27(.a(x11), .O(new_n71_));
  inv1   g28(.a(x12), .O(new_n72_));
  nand4  g29(.a(new_n72_), .b(new_n71_), .c(x10), .d(new_n49_), .O(new_n73_));
  oai21  g30(.a(new_n73_), .b(new_n70_), .c(new_n45_), .O(z04));
  nand2  g31(.a(x01), .b(x00), .O(new_n75_));
  nand3  g32(.a(x10), .b(x09), .c(x02), .O(new_n76_));
  oai21  g33(.a(new_n76_), .b(new_n75_), .c(new_n45_), .O(z05));
  aoi21  g34(.a(new_n44_), .b(x12), .c(new_n71_), .O(new_n78_));
  nand4  g35(.a(new_n78_), .b(x10), .c(new_n49_), .d(x02), .O(new_n79_));
  nor3   g36(.a(new_n79_), .b(new_n47_), .c(new_n46_), .O(z06));
  inv1   g37(.a(new_n45_), .O(new_n81_));
  nand2  g38(.a(new_n49_), .b(x01), .O(new_n82_));
  nand3  g39(.a(new_n72_), .b(x11), .c(x10), .O(new_n83_));
  oai22  g40(.a(new_n83_), .b(new_n82_), .c(new_n81_), .d(x01), .O(new_n84_));
  nand3  g41(.a(new_n84_), .b(x02), .c(x00), .O(new_n85_));
  inv1   g42(.a(new_n85_), .O(z07));
  nand3  g43(.a(x02), .b(new_n47_), .c(new_n46_), .O(new_n87_));
  inv1   g44(.a(x05), .O(new_n88_));
  nand2  g45(.a(new_n88_), .b(x04), .O(new_n89_));
  nor3   g46(.a(new_n89_), .b(new_n87_), .c(x03), .O(new_n90_));
  nor3   g47(.a(x08), .b(x07), .c(x06), .O(new_n91_));
  nor2   g48(.a(new_n53_), .b(x18), .O(new_n92_));
  nand4  g49(.a(new_n92_), .b(new_n91_), .c(new_n90_), .d(x17), .O(new_n93_));
  nand2  g50(.a(new_n93_), .b(new_n45_), .O(z08));
  inv1   g51(.a(x20), .O(new_n95_));
  nand4  g52(.a(x18), .b(new_n72_), .c(x01), .d(new_n46_), .O(new_n96_));
  inv1   g53(.a(new_n96_), .O(new_n97_));
  nand4  g54(.a(new_n97_), .b(new_n44_), .c(new_n95_), .d(new_n53_), .O(new_n98_));
  nor2   g55(.a(new_n98_), .b(x22), .O(z09));
  nand4  g56(.a(new_n53_), .b(x18), .c(x01), .d(new_n46_), .O(new_n100_));
  inv1   g57(.a(new_n100_), .O(new_n101_));
  nand4  g58(.a(new_n101_), .b(x22), .c(x21), .d(new_n95_), .O(new_n102_));
  inv1   g59(.a(new_n102_), .O(z10));
  nand3  g60(.a(x18), .b(x01), .c(new_n46_), .O(new_n104_));
  inv1   g61(.a(x22), .O(new_n105_));
  nand4  g62(.a(new_n105_), .b(x21), .c(new_n95_), .d(new_n53_), .O(new_n106_));
  oai21  g63(.a(new_n106_), .b(new_n104_), .c(new_n45_), .O(z11));
  inv1   g64(.a(x24), .O(new_n108_));
  nand2  g65(.a(x10), .b(x02), .O(new_n109_));
  nand3  g66(.a(new_n109_), .b(x01), .c(x00), .O(new_n110_));
  oai21  g67(.a(x19), .b(x02), .c(x23), .O(new_n111_));
  nand3  g68(.a(new_n53_), .b(x17), .c(new_n48_), .O(new_n112_));
  nand2  g69(.a(new_n112_), .b(new_n111_), .O(new_n113_));
  nand3  g70(.a(new_n113_), .b(new_n47_), .c(new_n46_), .O(new_n114_));
  nand2  g71(.a(new_n114_), .b(new_n110_), .O(new_n115_));
  nand3  g72(.a(new_n115_), .b(new_n108_), .c(x09), .O(new_n116_));
  nand2  g73(.a(new_n116_), .b(new_n45_), .O(z12));
  nor2   g74(.a(new_n81_), .b(x19), .O(new_n118_));
  nand4  g75(.a(new_n118_), .b(x17), .c(new_n48_), .d(new_n47_), .O(new_n119_));
  nor2   g76(.a(new_n119_), .b(x00), .O(z13));
  nor2   g77(.a(new_n57_), .b(x10), .O(new_n121_));
  nand4  g78(.a(new_n121_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n122_));
  nor2   g79(.a(new_n122_), .b(x00), .O(z14));
  oai21  g80(.a(x10), .b(new_n47_), .c(x02), .O(new_n124_));
  nand2  g81(.a(new_n124_), .b(x00), .O(new_n125_));
  nand3  g82(.a(x19), .b(new_n48_), .c(new_n47_), .O(new_n126_));
  nand3  g83(.a(new_n126_), .b(new_n125_), .c(new_n45_), .O(z15));
  nor3   g84(.a(new_n81_), .b(new_n47_), .c(x00), .O(z16));
  nand2  g85(.a(new_n87_), .b(new_n45_), .O(z17));
endmodule


