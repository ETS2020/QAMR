// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n125_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n136_;
  inv1   g000(.a(x1), .O(new_n18_));
  inv1   g001(.a(x5), .O(new_n19_));
  nand2  g002(.a(new_n19_), .b(x4), .O(new_n20_));
  inv1   g003(.a(x4), .O(new_n21_));
  nand2  g004(.a(new_n21_), .b(x3), .O(new_n22_));
  nand2  g005(.a(x6), .b(x5), .O(new_n23_));
  oai22  g006(.a(new_n23_), .b(new_n22_), .c(new_n20_), .d(new_n18_), .O(new_n24_));
  nand2  g007(.a(new_n24_), .b(x2), .O(new_n25_));
  nand2  g008(.a(x3), .b(x0), .O(new_n26_));
  oai22  g009(.a(new_n26_), .b(new_n20_), .c(new_n23_), .d(x4), .O(new_n27_));
  nand2  g010(.a(new_n27_), .b(x1), .O(new_n28_));
  nand3  g011(.a(x5), .b(new_n21_), .c(x0), .O(new_n29_));
  nand2  g012(.a(new_n29_), .b(new_n20_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(x6), .O(new_n31_));
  inv1   g014(.a(x6), .O(new_n32_));
  oai21  g015(.a(x3), .b(x2), .c(x1), .O(new_n33_));
  nand3  g016(.a(new_n33_), .b(new_n19_), .c(x4), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(x0), .O(new_n35_));
  nand2  g018(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  nand4  g019(.a(new_n36_), .b(new_n31_), .c(new_n28_), .d(new_n25_), .O(z0));
  oai21  g020(.a(x2), .b(x1), .c(x0), .O(new_n38_));
  aoi21  g021(.a(new_n38_), .b(new_n21_), .c(x5), .O(new_n39_));
  nand2  g022(.a(x3), .b(x2), .O(new_n40_));
  nand4  g023(.a(new_n40_), .b(x5), .c(new_n21_), .d(new_n18_), .O(new_n41_));
  nor2   g024(.a(new_n41_), .b(x0), .O(new_n42_));
  oai21  g025(.a(new_n42_), .b(new_n39_), .c(x6), .O(new_n43_));
  nand2  g026(.a(x3), .b(x1), .O(new_n44_));
  nand3  g027(.a(new_n32_), .b(x5), .c(new_n21_), .O(new_n45_));
  oai21  g028(.a(new_n44_), .b(new_n20_), .c(new_n45_), .O(new_n46_));
  inv1   g029(.a(x0), .O(new_n47_));
  nand2  g030(.a(new_n32_), .b(new_n47_), .O(new_n48_));
  nand2  g031(.a(x2), .b(x1), .O(new_n49_));
  oai21  g032(.a(new_n49_), .b(new_n20_), .c(new_n48_), .O(new_n50_));
  aoi21  g033(.a(new_n46_), .b(x0), .c(new_n50_), .O(new_n51_));
  nand2  g034(.a(new_n51_), .b(new_n43_), .O(z1));
  nand2  g035(.a(x4), .b(x2), .O(new_n53_));
  nor2   g036(.a(x6), .b(x4), .O(new_n54_));
  nand3  g037(.a(new_n54_), .b(x1), .c(x0), .O(new_n55_));
  oai21  g038(.a(new_n53_), .b(new_n23_), .c(new_n55_), .O(new_n56_));
  nand2  g039(.a(new_n56_), .b(x3), .O(new_n57_));
  nor2   g040(.a(new_n32_), .b(new_n21_), .O(new_n58_));
  nand2  g041(.a(new_n54_), .b(x2), .O(new_n59_));
  inv1   g042(.a(new_n59_), .O(new_n60_));
  oai21  g043(.a(new_n60_), .b(new_n58_), .c(x1), .O(new_n61_));
  oai21  g044(.a(new_n58_), .b(new_n54_), .c(x5), .O(new_n62_));
  nand3  g045(.a(new_n33_), .b(new_n32_), .c(new_n19_), .O(new_n63_));
  nand2  g046(.a(x6), .b(x2), .O(new_n64_));
  nand2  g047(.a(new_n64_), .b(new_n63_), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(x4), .O(new_n66_));
  nand3  g049(.a(new_n66_), .b(new_n62_), .c(new_n61_), .O(new_n67_));
  nand2  g050(.a(new_n67_), .b(x0), .O(new_n68_));
  nand3  g051(.a(new_n40_), .b(x5), .c(new_n47_), .O(new_n69_));
  inv1   g052(.a(x2), .O(new_n70_));
  nand2  g053(.a(new_n19_), .b(new_n70_), .O(new_n71_));
  aoi21  g054(.a(new_n71_), .b(new_n69_), .c(x1), .O(new_n72_));
  nor2   g055(.a(x5), .b(x0), .O(new_n73_));
  oai21  g056(.a(new_n73_), .b(new_n72_), .c(new_n21_), .O(new_n74_));
  nand3  g057(.a(x5), .b(x4), .c(x1), .O(new_n75_));
  nand2  g058(.a(new_n75_), .b(new_n74_), .O(new_n76_));
  nand2  g059(.a(new_n76_), .b(x6), .O(new_n77_));
  nand3  g060(.a(new_n77_), .b(new_n68_), .c(new_n57_), .O(z2));
  nand3  g061(.a(x5), .b(x1), .c(x0), .O(new_n79_));
  nand4  g062(.a(x6), .b(new_n19_), .c(x2), .d(new_n47_), .O(new_n80_));
  nand2  g063(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g064(.a(new_n81_), .b(x3), .O(new_n82_));
  oai21  g065(.a(x6), .b(x2), .c(x1), .O(new_n83_));
  aoi21  g066(.a(new_n83_), .b(new_n64_), .c(new_n19_), .O(new_n84_));
  nand2  g067(.a(new_n64_), .b(new_n18_), .O(new_n85_));
  inv1   g068(.a(x3), .O(new_n86_));
  nand3  g069(.a(new_n32_), .b(new_n86_), .c(new_n70_), .O(new_n87_));
  aoi21  g070(.a(new_n87_), .b(new_n85_), .c(x5), .O(new_n88_));
  oai21  g071(.a(new_n88_), .b(new_n84_), .c(x0), .O(new_n89_));
  nand3  g072(.a(new_n40_), .b(x5), .c(new_n18_), .O(new_n90_));
  oai21  g073(.a(x5), .b(new_n18_), .c(new_n90_), .O(new_n91_));
  nand3  g074(.a(new_n91_), .b(x6), .c(new_n47_), .O(new_n92_));
  nand3  g075(.a(new_n92_), .b(new_n89_), .c(new_n82_), .O(z3));
  nor2   g076(.a(new_n86_), .b(x2), .O(new_n94_));
  inv1   g077(.a(new_n94_), .O(new_n95_));
  aoi21  g078(.a(new_n95_), .b(new_n49_), .c(new_n47_), .O(new_n96_));
  oai21  g079(.a(x3), .b(x0), .c(x2), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(new_n18_), .O(new_n98_));
  oai21  g081(.a(new_n40_), .b(new_n18_), .c(new_n98_), .O(new_n99_));
  oai21  g082(.a(new_n99_), .b(new_n96_), .c(x6), .O(new_n100_));
  nand3  g083(.a(new_n86_), .b(new_n70_), .c(x1), .O(new_n101_));
  oai21  g084(.a(new_n70_), .b(x1), .c(new_n101_), .O(new_n102_));
  nand3  g085(.a(new_n102_), .b(new_n32_), .c(x0), .O(new_n103_));
  nand2  g086(.a(new_n103_), .b(new_n100_), .O(z4));
  nor2   g087(.a(new_n32_), .b(x3), .O(new_n105_));
  nand2  g088(.a(new_n105_), .b(new_n47_), .O(new_n106_));
  nand2  g089(.a(new_n106_), .b(new_n26_), .O(new_n107_));
  nand2  g090(.a(new_n107_), .b(x2), .O(new_n108_));
  oai21  g091(.a(new_n86_), .b(new_n18_), .c(x0), .O(new_n109_));
  nand2  g092(.a(x1), .b(new_n47_), .O(new_n110_));
  nand2  g093(.a(x6), .b(x3), .O(new_n111_));
  oai21  g094(.a(new_n111_), .b(new_n110_), .c(new_n109_), .O(new_n112_));
  nand2  g095(.a(new_n112_), .b(new_n70_), .O(new_n113_));
  nand3  g096(.a(new_n113_), .b(new_n108_), .c(new_n48_), .O(z5));
  nand2  g097(.a(x3), .b(new_n18_), .O(new_n115_));
  nand3  g098(.a(new_n32_), .b(new_n86_), .c(x1), .O(new_n116_));
  aoi21  g099(.a(new_n116_), .b(new_n115_), .c(new_n47_), .O(new_n117_));
  nor2   g100(.a(new_n111_), .b(x1), .O(new_n118_));
  oai21  g101(.a(new_n118_), .b(new_n117_), .c(new_n70_), .O(new_n119_));
  oai21  g102(.a(new_n54_), .b(x5), .c(x0), .O(new_n120_));
  and2   g103(.a(new_n111_), .b(new_n20_), .O(new_n121_));
  aoi21  g104(.a(new_n121_), .b(new_n120_), .c(new_n70_), .O(new_n122_));
  oai21  g105(.a(new_n122_), .b(new_n105_), .c(x1), .O(new_n123_));
  nand3  g106(.a(new_n123_), .b(new_n119_), .c(new_n48_), .O(z6));
  nand2  g107(.a(new_n86_), .b(x2), .O(new_n125_));
  inv1   g108(.a(new_n125_), .O(new_n126_));
  oai21  g109(.a(new_n126_), .b(new_n94_), .c(x0), .O(new_n127_));
  nand2  g110(.a(new_n94_), .b(x1), .O(new_n128_));
  aoi21  g111(.a(new_n128_), .b(new_n125_), .c(x0), .O(new_n129_));
  nand2  g112(.a(new_n94_), .b(new_n18_), .O(new_n130_));
  inv1   g113(.a(new_n130_), .O(new_n131_));
  oai21  g114(.a(new_n131_), .b(new_n129_), .c(x6), .O(new_n132_));
  nand2  g115(.a(new_n132_), .b(new_n127_), .O(z7));
  oai21  g116(.a(x1), .b(new_n47_), .c(x6), .O(new_n134_));
  aoi21  g117(.a(new_n134_), .b(new_n47_), .c(x3), .O(z8));
  nand2  g118(.a(new_n48_), .b(x5), .O(new_n136_));
  nor2   g119(.a(new_n136_), .b(new_n21_), .O(z9));
endmodule


