// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n110_, new_n111_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  oai21  g03(.a(new_n20_), .b(new_n18_), .c(new_n19_), .O(new_n21_));
  nand2  g04(.a(x3), .b(x2), .O(new_n22_));
  oai21  g05(.a(new_n22_), .b(new_n20_), .c(new_n18_), .O(new_n23_));
  aoi21  g06(.a(new_n23_), .b(new_n21_), .c(x6), .O(new_n24_));
  inv1   g07(.a(x6), .O(new_n25_));
  nand2  g08(.a(x1), .b(x0), .O(new_n26_));
  aoi21  g09(.a(new_n26_), .b(new_n25_), .c(x5), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n24_), .c(x4), .O(new_n28_));
  nor2   g11(.a(new_n25_), .b(x4), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(new_n28_), .O(z0));
  inv1   g14(.a(x4), .O(new_n32_));
  oai21  g15(.a(x3), .b(x2), .c(x1), .O(new_n33_));
  oai21  g16(.a(new_n33_), .b(new_n18_), .c(new_n25_), .O(new_n34_));
  nand3  g17(.a(x3), .b(x2), .c(x1), .O(new_n35_));
  nand4  g18(.a(new_n35_), .b(new_n25_), .c(x5), .d(new_n18_), .O(new_n36_));
  inv1   g19(.a(new_n36_), .O(new_n37_));
  aoi21  g20(.a(new_n34_), .b(new_n20_), .c(new_n37_), .O(new_n38_));
  nand3  g21(.a(new_n25_), .b(x5), .c(new_n32_), .O(new_n39_));
  oai21  g22(.a(new_n38_), .b(new_n32_), .c(new_n39_), .O(z1));
  inv1   g23(.a(x2), .O(new_n41_));
  inv1   g24(.a(x3), .O(new_n42_));
  nand3  g25(.a(new_n20_), .b(new_n42_), .c(new_n41_), .O(new_n43_));
  nand2  g26(.a(x6), .b(x3), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  nand3  g28(.a(new_n45_), .b(x1), .c(x0), .O(new_n46_));
  inv1   g29(.a(new_n46_), .O(new_n47_));
  oai21  g30(.a(new_n47_), .b(new_n24_), .c(x4), .O(new_n48_));
  oai21  g31(.a(x6), .b(new_n42_), .c(new_n41_), .O(new_n49_));
  nand3  g32(.a(x5), .b(x3), .c(x2), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  aoi21  g34(.a(new_n49_), .b(x0), .c(new_n51_), .O(new_n52_));
  aoi21  g35(.a(x5), .b(x0), .c(x6), .O(new_n53_));
  oai21  g36(.a(new_n52_), .b(new_n19_), .c(new_n53_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n32_), .O(new_n55_));
  aoi21  g38(.a(new_n20_), .b(new_n41_), .c(new_n18_), .O(new_n56_));
  aoi21  g39(.a(new_n22_), .b(new_n19_), .c(new_n20_), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n56_), .c(x6), .O(new_n58_));
  nand3  g41(.a(new_n58_), .b(new_n55_), .c(new_n48_), .O(z2));
  aoi21  g42(.a(new_n22_), .b(new_n25_), .c(new_n19_), .O(new_n60_));
  nand3  g43(.a(x6), .b(x3), .c(x2), .O(new_n61_));
  inv1   g44(.a(new_n61_), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(new_n60_), .c(new_n20_), .O(new_n63_));
  aoi21  g46(.a(x6), .b(x1), .c(x2), .O(new_n64_));
  aoi21  g47(.a(x6), .b(x3), .c(x1), .O(new_n65_));
  oai21  g48(.a(new_n65_), .b(new_n64_), .c(x5), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n63_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n18_), .O(new_n68_));
  oai21  g51(.a(x6), .b(x3), .c(x1), .O(new_n69_));
  nand2  g52(.a(x6), .b(x2), .O(new_n70_));
  aoi21  g53(.a(new_n70_), .b(new_n69_), .c(new_n20_), .O(new_n71_));
  nand2  g54(.a(new_n70_), .b(new_n19_), .O(new_n72_));
  nand3  g55(.a(new_n25_), .b(new_n42_), .c(new_n41_), .O(new_n73_));
  aoi21  g56(.a(new_n73_), .b(new_n72_), .c(x5), .O(new_n74_));
  oai21  g57(.a(new_n74_), .b(new_n71_), .c(x0), .O(new_n75_));
  nand3  g58(.a(new_n25_), .b(x5), .c(new_n42_), .O(new_n76_));
  nor3   g59(.a(new_n76_), .b(new_n41_), .c(new_n19_), .O(new_n77_));
  nor2   g60(.a(new_n77_), .b(new_n29_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(new_n75_), .c(new_n68_), .O(z3));
  nor2   g62(.a(x3), .b(x2), .O(new_n80_));
  oai21  g63(.a(new_n80_), .b(new_n18_), .c(new_n22_), .O(new_n81_));
  nand3  g64(.a(new_n81_), .b(x6), .c(x4), .O(new_n82_));
  aoi21  g65(.a(new_n22_), .b(new_n18_), .c(new_n80_), .O(new_n83_));
  oai21  g66(.a(new_n83_), .b(x6), .c(new_n82_), .O(new_n84_));
  nand2  g67(.a(new_n84_), .b(x1), .O(new_n85_));
  nand2  g68(.a(new_n42_), .b(new_n18_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n25_), .c(x2), .O(new_n87_));
  nand2  g70(.a(new_n86_), .b(x2), .O(new_n88_));
  nand3  g71(.a(new_n88_), .b(x6), .c(x4), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n87_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n19_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n85_), .O(z4));
  nor2   g75(.a(new_n42_), .b(x2), .O(new_n93_));
  inv1   g76(.a(new_n93_), .O(new_n94_));
  nand2  g77(.a(new_n42_), .b(x2), .O(new_n95_));
  oai21  g78(.a(new_n94_), .b(new_n19_), .c(new_n95_), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n18_), .O(new_n97_));
  inv1   g80(.a(new_n22_), .O(new_n98_));
  aoi21  g81(.a(x3), .b(x1), .c(x2), .O(new_n99_));
  oai21  g82(.a(new_n99_), .b(new_n98_), .c(x0), .O(new_n100_));
  nand3  g83(.a(new_n100_), .b(new_n97_), .c(new_n30_), .O(z5));
  oai21  g84(.a(new_n20_), .b(new_n32_), .c(x0), .O(new_n102_));
  oai21  g85(.a(x5), .b(new_n18_), .c(x3), .O(new_n103_));
  aoi21  g86(.a(new_n103_), .b(new_n102_), .c(new_n41_), .O(new_n104_));
  oai21  g87(.a(new_n104_), .b(new_n42_), .c(x1), .O(new_n105_));
  aoi21  g88(.a(new_n93_), .b(new_n19_), .c(new_n29_), .O(new_n106_));
  nand2  g89(.a(new_n106_), .b(new_n105_), .O(z6));
  aoi21  g90(.a(new_n95_), .b(new_n94_), .c(new_n29_), .O(z7));
  nand2  g91(.a(new_n30_), .b(x3), .O(z8));
  nand3  g92(.a(new_n35_), .b(new_n25_), .c(new_n18_), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x5), .c(x4), .O(new_n111_));
  inv1   g94(.a(new_n111_), .O(z9));
endmodule


