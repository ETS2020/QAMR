// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:59 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n110_, new_n111_, new_n113_, new_n114_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x2), .O(new_n21_));
  inv1   g04(.a(x3), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x1), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand4  g09(.a(new_n23_), .b(new_n26_), .c(x1), .d(x0), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(new_n19_), .O(new_n28_));
  nand2  g11(.a(x3), .b(x2), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x1), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n26_), .c(new_n20_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n28_), .c(new_n18_), .O(z0));
  nor2   g16(.a(x5), .b(new_n18_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x0), .O(new_n35_));
  nand4  g18(.a(new_n26_), .b(x5), .c(new_n18_), .d(x3), .O(new_n36_));
  aoi21  g19(.a(new_n36_), .b(new_n35_), .c(new_n21_), .O(new_n37_));
  nand2  g20(.a(new_n34_), .b(x3), .O(new_n38_));
  aoi21  g21(.a(new_n38_), .b(new_n26_), .c(new_n20_), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(new_n37_), .c(x1), .O(new_n40_));
  nand3  g23(.a(new_n26_), .b(x5), .c(new_n18_), .O(new_n41_));
  oai21  g24(.a(new_n26_), .b(new_n21_), .c(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  aoi21  g26(.a(new_n19_), .b(new_n18_), .c(new_n26_), .O(new_n44_));
  aoi21  g27(.a(new_n26_), .b(new_n22_), .c(new_n21_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(x1), .c(new_n19_), .O(new_n46_));
  aoi21  g29(.a(new_n46_), .b(new_n20_), .c(new_n44_), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n43_), .c(new_n40_), .O(z1));
  inv1   g31(.a(x1), .O(new_n49_));
  nand2  g32(.a(x5), .b(x0), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n49_), .O(new_n51_));
  oai21  g34(.a(x5), .b(x3), .c(x0), .O(new_n52_));
  nand2  g35(.a(x5), .b(x3), .O(new_n53_));
  aoi22  g36(.a(new_n53_), .b(new_n20_), .c(new_n52_), .d(new_n21_), .O(new_n54_));
  aoi21  g37(.a(new_n54_), .b(new_n51_), .c(new_n18_), .O(new_n55_));
  oai21  g38(.a(x3), .b(x2), .c(x0), .O(new_n56_));
  nand3  g39(.a(x5), .b(x3), .c(x2), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(x1), .O(new_n59_));
  aoi21  g42(.a(new_n59_), .b(new_n50_), .c(x4), .O(new_n60_));
  oai21  g43(.a(new_n60_), .b(new_n55_), .c(new_n26_), .O(new_n61_));
  oai21  g44(.a(x2), .b(x1), .c(x4), .O(new_n62_));
  nor2   g45(.a(x2), .b(x1), .O(new_n63_));
  oai21  g46(.a(new_n63_), .b(new_n20_), .c(new_n18_), .O(new_n64_));
  oai21  g47(.a(new_n62_), .b(new_n20_), .c(new_n64_), .O(new_n65_));
  nand3  g48(.a(new_n65_), .b(x6), .c(new_n19_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(new_n61_), .O(z2));
  nand3  g50(.a(new_n24_), .b(new_n19_), .c(x0), .O(new_n68_));
  nor2   g51(.a(new_n21_), .b(new_n49_), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(x5), .c(new_n22_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(new_n26_), .O(new_n72_));
  nand3  g55(.a(x6), .b(new_n21_), .c(new_n49_), .O(new_n73_));
  oai21  g56(.a(new_n53_), .b(new_n49_), .c(new_n73_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(x0), .O(new_n75_));
  nand3  g58(.a(new_n19_), .b(x3), .c(x2), .O(new_n76_));
  aoi21  g59(.a(new_n76_), .b(new_n26_), .c(new_n49_), .O(new_n77_));
  nand3  g60(.a(x6), .b(x3), .c(x2), .O(new_n78_));
  oai21  g61(.a(new_n69_), .b(new_n19_), .c(new_n78_), .O(new_n79_));
  oai21  g62(.a(new_n79_), .b(new_n77_), .c(new_n20_), .O(new_n80_));
  nand2  g63(.a(x6), .b(x5), .O(new_n81_));
  nand4  g64(.a(new_n81_), .b(new_n80_), .c(new_n75_), .d(new_n72_), .O(z3));
  aoi21  g65(.a(new_n56_), .b(new_n29_), .c(new_n26_), .O(new_n83_));
  nand2  g66(.a(new_n29_), .b(new_n20_), .O(new_n84_));
  aoi21  g67(.a(new_n84_), .b(new_n23_), .c(x6), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n83_), .c(x1), .O(new_n86_));
  nand2  g69(.a(new_n22_), .b(new_n20_), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n26_), .c(x2), .O(new_n88_));
  aoi21  g71(.a(new_n22_), .b(new_n20_), .c(new_n21_), .O(new_n89_));
  oai21  g72(.a(new_n89_), .b(new_n26_), .c(new_n88_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n49_), .O(new_n91_));
  nand3  g74(.a(new_n91_), .b(new_n86_), .c(new_n81_), .O(z4));
  nor2   g75(.a(new_n22_), .b(x2), .O(new_n93_));
  nand2  g76(.a(new_n93_), .b(x1), .O(new_n94_));
  oai21  g77(.a(x3), .b(new_n21_), .c(new_n94_), .O(new_n95_));
  nand2  g78(.a(new_n95_), .b(new_n20_), .O(new_n96_));
  aoi21  g79(.a(x3), .b(x1), .c(x2), .O(new_n97_));
  oai21  g80(.a(new_n97_), .b(new_n30_), .c(x0), .O(new_n98_));
  nand3  g81(.a(new_n98_), .b(new_n96_), .c(new_n81_), .O(z5));
  nor2   g82(.a(x6), .b(x4), .O(new_n100_));
  aoi21  g83(.a(new_n19_), .b(x4), .c(new_n100_), .O(new_n101_));
  nand3  g84(.a(new_n26_), .b(new_n19_), .c(x0), .O(new_n102_));
  nand2  g85(.a(new_n102_), .b(x3), .O(new_n103_));
  oai21  g86(.a(new_n101_), .b(new_n20_), .c(new_n103_), .O(new_n104_));
  aoi21  g87(.a(new_n104_), .b(x2), .c(new_n22_), .O(new_n105_));
  aoi22  g88(.a(new_n93_), .b(new_n49_), .c(x6), .d(x5), .O(new_n106_));
  oai21  g89(.a(new_n105_), .b(new_n49_), .c(new_n106_), .O(z6));
  nand2  g90(.a(new_n81_), .b(x3), .O(z8));
  nand2  g91(.a(x6), .b(x5), .O(new_n110_));
  nand3  g92(.a(new_n110_), .b(new_n22_), .c(x2), .O(new_n111_));
  oai21  g93(.a(z8), .b(x2), .c(new_n111_), .O(z7));
  nand2  g94(.a(new_n31_), .b(new_n20_), .O(new_n113_));
  nand4  g95(.a(new_n113_), .b(new_n26_), .c(x5), .d(x4), .O(new_n114_));
  inv1   g96(.a(new_n114_), .O(z9));
endmodule


