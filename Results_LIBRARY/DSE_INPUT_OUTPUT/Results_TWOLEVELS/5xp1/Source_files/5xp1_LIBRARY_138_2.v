// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:54 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n70_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n98_, new_n99_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x5), .O(new_n21_));
  nor2   g04(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nor2   g05(.a(new_n22_), .b(x6), .O(new_n23_));
  nand2  g06(.a(x6), .b(new_n21_), .O(new_n24_));
  inv1   g07(.a(new_n24_), .O(new_n25_));
  aoi21  g08(.a(new_n23_), .b(new_n19_), .c(new_n25_), .O(new_n26_));
  aoi21  g09(.a(x3), .b(x2), .c(x1), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n20_), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(x6), .c(x5), .d(new_n18_), .O(new_n29_));
  oai21  g12(.a(new_n26_), .b(new_n18_), .c(new_n29_), .O(z0));
  oai21  g13(.a(x5), .b(new_n20_), .c(x6), .O(new_n31_));
  nand2  g14(.a(new_n31_), .b(x1), .O(new_n32_));
  inv1   g15(.a(x2), .O(new_n33_));
  inv1   g16(.a(x6), .O(new_n34_));
  nand4  g17(.a(new_n34_), .b(x5), .c(new_n18_), .d(new_n19_), .O(new_n35_));
  oai21  g18(.a(new_n24_), .b(new_n33_), .c(new_n35_), .O(new_n36_));
  nand2  g19(.a(new_n36_), .b(x0), .O(new_n37_));
  nand2  g20(.a(x3), .b(x2), .O(new_n38_));
  nand3  g21(.a(new_n38_), .b(x6), .c(new_n18_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x6), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x5), .c(new_n19_), .d(new_n20_), .O(new_n41_));
  nand2  g24(.a(new_n25_), .b(x4), .O(new_n42_));
  nand4  g25(.a(new_n42_), .b(new_n41_), .c(new_n37_), .d(new_n32_), .O(z1));
  oai21  g26(.a(x5), .b(x0), .c(x1), .O(new_n44_));
  nand2  g27(.a(new_n21_), .b(new_n33_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x0), .O(new_n46_));
  nand3  g29(.a(x5), .b(x3), .c(x2), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(new_n46_), .c(new_n44_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x4), .O(new_n49_));
  nor2   g32(.a(x5), .b(x0), .O(new_n50_));
  nand3  g33(.a(new_n38_), .b(x5), .c(new_n20_), .O(new_n51_));
  aoi21  g34(.a(new_n51_), .b(new_n45_), .c(x1), .O(new_n52_));
  oai21  g35(.a(new_n52_), .b(new_n50_), .c(new_n18_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n49_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x6), .O(new_n55_));
  nand3  g38(.a(x5), .b(new_n18_), .c(x0), .O(new_n56_));
  oai21  g39(.a(new_n22_), .b(new_n18_), .c(new_n56_), .O(new_n57_));
  nand3  g40(.a(new_n57_), .b(new_n34_), .c(new_n19_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n55_), .O(z2));
  oai21  g42(.a(x2), .b(x1), .c(x0), .O(new_n60_));
  nor2   g43(.a(x3), .b(new_n33_), .O(new_n61_));
  nand2  g44(.a(new_n61_), .b(new_n19_), .O(new_n62_));
  aoi21  g45(.a(new_n62_), .b(new_n60_), .c(new_n21_), .O(new_n63_));
  nor3   g46(.a(new_n27_), .b(x5), .c(x0), .O(new_n64_));
  oai21  g47(.a(new_n64_), .b(new_n63_), .c(x6), .O(new_n65_));
  nand2  g48(.a(x6), .b(x2), .O(new_n66_));
  xor2a  g49(.a(x5), .b(x0), .O(new_n67_));
  nand3  g50(.a(new_n67_), .b(new_n66_), .c(new_n19_), .O(new_n68_));
  nand2  g51(.a(new_n68_), .b(new_n65_), .O(z3));
  inv1   g52(.a(x3), .O(new_n70_));
  aoi21  g53(.a(new_n70_), .b(new_n20_), .c(new_n33_), .O(new_n71_));
  nand3  g54(.a(x3), .b(new_n33_), .c(x0), .O(new_n72_));
  oai21  g55(.a(new_n71_), .b(x1), .c(new_n72_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(x6), .O(new_n74_));
  aoi22  g57(.a(x6), .b(new_n19_), .c(new_n70_), .d(new_n20_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(x2), .O(new_n76_));
  nand2  g59(.a(new_n34_), .b(x1), .O(new_n77_));
  nand3  g60(.a(new_n77_), .b(new_n76_), .c(new_n74_), .O(z4));
  nand3  g61(.a(new_n77_), .b(new_n70_), .c(x2), .O(new_n79_));
  nand4  g62(.a(x6), .b(x3), .c(new_n33_), .d(x1), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n79_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n20_), .O(new_n82_));
  nor2   g65(.a(new_n61_), .b(x1), .O(new_n83_));
  nand2  g66(.a(new_n70_), .b(new_n33_), .O(new_n84_));
  aoi21  g67(.a(new_n84_), .b(new_n38_), .c(new_n34_), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n83_), .c(x0), .O(new_n86_));
  nand2  g69(.a(new_n86_), .b(new_n82_), .O(z5));
  xor2a  g70(.a(x2), .b(x1), .O(new_n88_));
  oai21  g71(.a(new_n34_), .b(new_n70_), .c(x1), .O(new_n89_));
  oai21  g72(.a(new_n88_), .b(new_n70_), .c(new_n89_), .O(z6));
  nor2   g73(.a(x2), .b(x0), .O(new_n91_));
  aoi21  g74(.a(new_n91_), .b(x3), .c(new_n34_), .O(new_n92_));
  nand2  g75(.a(x6), .b(x0), .O(new_n93_));
  aoi21  g76(.a(new_n93_), .b(x1), .c(new_n70_), .O(new_n94_));
  aoi21  g77(.a(new_n94_), .b(new_n33_), .c(new_n61_), .O(new_n95_));
  oai21  g78(.a(new_n92_), .b(new_n19_), .c(new_n95_), .O(z7));
  aoi21  g79(.a(new_n34_), .b(x1), .c(x3), .O(z8));
  oai21  g80(.a(x1), .b(new_n20_), .c(new_n34_), .O(new_n98_));
  nand3  g81(.a(new_n98_), .b(x5), .c(x4), .O(new_n99_));
  inv1   g82(.a(new_n99_), .O(z9));
endmodule


