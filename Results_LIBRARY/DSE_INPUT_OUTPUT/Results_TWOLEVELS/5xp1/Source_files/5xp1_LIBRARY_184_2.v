// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:02 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n89_, new_n90_, new_n92_, new_n93_, new_n94_,
    new_n95_, new_n98_, new_n99_;
  inv1   g00(.a(x5), .O(new_n18_));
  oai21  g01(.a(new_n18_), .b(x4), .c(x6), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x1), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  inv1   g04(.a(x0), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(x5), .c(new_n21_), .O(new_n25_));
  oai21  g08(.a(x5), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x6), .O(new_n27_));
  aoi21  g10(.a(x5), .b(x0), .c(x6), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(x4), .O(new_n29_));
  nand3  g12(.a(new_n29_), .b(new_n27_), .c(new_n20_), .O(z0));
  inv1   g13(.a(x1), .O(new_n31_));
  inv1   g14(.a(x6), .O(new_n32_));
  aoi21  g15(.a(new_n23_), .b(new_n21_), .c(new_n32_), .O(new_n33_));
  nand3  g16(.a(new_n32_), .b(new_n21_), .c(x0), .O(new_n34_));
  oai21  g17(.a(new_n33_), .b(x0), .c(new_n34_), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x5), .c(new_n31_), .O(new_n36_));
  inv1   g19(.a(x2), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n31_), .O(new_n38_));
  nand2  g21(.a(new_n38_), .b(x0), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(new_n21_), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(x6), .c(new_n18_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n36_), .O(z1));
  oai21  g25(.a(x5), .b(x0), .c(x1), .O(new_n43_));
  nor2   g26(.a(x5), .b(new_n37_), .O(new_n44_));
  nor2   g27(.a(new_n32_), .b(new_n18_), .O(new_n45_));
  oai21  g28(.a(new_n45_), .b(new_n44_), .c(x0), .O(new_n46_));
  inv1   g29(.a(new_n23_), .O(new_n47_));
  aoi21  g30(.a(new_n45_), .b(new_n47_), .c(new_n28_), .O(new_n48_));
  nand3  g31(.a(new_n48_), .b(new_n46_), .c(new_n43_), .O(new_n49_));
  nand2  g32(.a(new_n49_), .b(x4), .O(new_n50_));
  nand3  g33(.a(new_n23_), .b(x6), .c(new_n22_), .O(new_n51_));
  nand2  g34(.a(new_n32_), .b(x0), .O(new_n52_));
  aoi21  g35(.a(new_n52_), .b(new_n51_), .c(new_n18_), .O(new_n53_));
  nand3  g36(.a(x6), .b(new_n18_), .c(new_n37_), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  oai21  g38(.a(new_n55_), .b(new_n53_), .c(new_n31_), .O(new_n56_));
  nand3  g39(.a(x6), .b(new_n18_), .c(new_n22_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n21_), .O(new_n59_));
  nor2   g42(.a(x6), .b(new_n31_), .O(new_n60_));
  inv1   g43(.a(new_n60_), .O(new_n61_));
  nand3  g44(.a(new_n61_), .b(new_n59_), .c(new_n50_), .O(z2));
  nand3  g45(.a(new_n38_), .b(x5), .c(x0), .O(new_n63_));
  nand2  g46(.a(new_n23_), .b(new_n31_), .O(new_n64_));
  nand3  g47(.a(new_n64_), .b(new_n18_), .c(new_n22_), .O(new_n65_));
  nand2  g48(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x6), .O(new_n67_));
  nor2   g50(.a(new_n32_), .b(new_n37_), .O(new_n68_));
  xnor2a g51(.a(x5), .b(x0), .O(new_n69_));
  inv1   g52(.a(x3), .O(new_n70_));
  nand3  g53(.a(x5), .b(new_n70_), .c(new_n22_), .O(new_n71_));
  oai21  g54(.a(new_n69_), .b(new_n68_), .c(new_n71_), .O(new_n72_));
  nand2  g55(.a(new_n72_), .b(new_n31_), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n67_), .O(z3));
  oai21  g57(.a(x3), .b(x0), .c(x2), .O(new_n75_));
  nor2   g58(.a(new_n70_), .b(x2), .O(new_n76_));
  aoi22  g59(.a(new_n76_), .b(x0), .c(new_n75_), .d(new_n31_), .O(new_n77_));
  aoi22  g60(.a(x6), .b(new_n31_), .c(new_n70_), .d(new_n22_), .O(new_n78_));
  aoi21  g61(.a(new_n78_), .b(x2), .c(new_n60_), .O(new_n79_));
  oai21  g62(.a(new_n77_), .b(new_n32_), .c(new_n79_), .O(z4));
  inv1   g63(.a(new_n76_), .O(new_n81_));
  nand2  g64(.a(new_n70_), .b(x2), .O(new_n82_));
  oai21  g65(.a(new_n81_), .b(new_n31_), .c(new_n82_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n22_), .O(new_n84_));
  aoi21  g67(.a(new_n37_), .b(x1), .c(new_n70_), .O(new_n85_));
  nor2   g68(.a(x3), .b(x2), .O(new_n86_));
  oai21  g69(.a(new_n86_), .b(new_n85_), .c(x0), .O(new_n87_));
  nand3  g70(.a(new_n87_), .b(new_n84_), .c(new_n61_), .O(z5));
  xor2a  g71(.a(x2), .b(x1), .O(new_n89_));
  oai21  g72(.a(new_n32_), .b(new_n70_), .c(x1), .O(new_n90_));
  oai21  g73(.a(new_n89_), .b(new_n70_), .c(new_n90_), .O(z6));
  oai21  g74(.a(new_n81_), .b(x0), .c(x6), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(x1), .O(new_n93_));
  oai21  g76(.a(new_n32_), .b(new_n22_), .c(x1), .O(new_n94_));
  nand3  g77(.a(new_n94_), .b(x3), .c(new_n37_), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(new_n93_), .c(new_n82_), .O(z7));
  nand2  g79(.a(new_n61_), .b(x3), .O(z8));
  oai21  g80(.a(x1), .b(new_n22_), .c(new_n32_), .O(new_n98_));
  nand3  g81(.a(new_n98_), .b(x5), .c(x4), .O(new_n99_));
  inv1   g82(.a(new_n99_), .O(z9));
endmodule


