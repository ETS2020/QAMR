// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:35 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n108_;
  inv1   g00(.a(x4), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  inv1   g02(.a(x0), .O(new_n20_));
  inv1   g03(.a(x2), .O(new_n21_));
  inv1   g04(.a(x3), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(x1), .O(new_n24_));
  nor2   g07(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  inv1   g08(.a(x6), .O(new_n26_));
  nand2  g09(.a(new_n25_), .b(new_n26_), .O(new_n27_));
  oai21  g10(.a(new_n27_), .b(new_n25_), .c(new_n19_), .O(new_n28_));
  nand2  g11(.a(x3), .b(x2), .O(new_n29_));
  inv1   g12(.a(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x1), .O(new_n31_));
  nand4  g14(.a(new_n31_), .b(new_n26_), .c(x5), .d(new_n20_), .O(new_n32_));
  aoi21  g15(.a(new_n32_), .b(new_n28_), .c(new_n18_), .O(z0));
  inv1   g16(.a(x1), .O(new_n34_));
  aoi21  g17(.a(x4), .b(x1), .c(x6), .O(new_n35_));
  aoi21  g18(.a(x4), .b(x3), .c(x6), .O(new_n36_));
  oai22  g19(.a(new_n36_), .b(new_n34_), .c(new_n35_), .d(new_n21_), .O(new_n37_));
  aoi22  g20(.a(new_n37_), .b(x0), .c(x6), .d(x4), .O(new_n38_));
  inv1   g21(.a(new_n31_), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(x0), .c(x4), .O(new_n40_));
  nand3  g23(.a(new_n40_), .b(new_n26_), .c(x5), .O(new_n41_));
  oai21  g24(.a(new_n38_), .b(x5), .c(new_n41_), .O(z1));
  nand3  g25(.a(new_n23_), .b(new_n26_), .c(x0), .O(new_n43_));
  nand3  g26(.a(x5), .b(x3), .c(x2), .O(new_n44_));
  aoi21  g27(.a(new_n44_), .b(new_n43_), .c(new_n34_), .O(new_n45_));
  nor2   g28(.a(x2), .b(x1), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n20_), .c(x6), .O(new_n47_));
  oai21  g30(.a(new_n19_), .b(new_n20_), .c(new_n47_), .O(new_n48_));
  oai21  g31(.a(new_n48_), .b(new_n45_), .c(new_n18_), .O(new_n49_));
  nor3   g32(.a(new_n46_), .b(new_n26_), .c(new_n20_), .O(new_n50_));
  oai21  g33(.a(new_n19_), .b(new_n20_), .c(new_n34_), .O(new_n51_));
  nand2  g34(.a(x5), .b(new_n20_), .O(new_n52_));
  nand2  g35(.a(new_n19_), .b(new_n22_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  aoi21  g37(.a(x5), .b(x3), .c(x0), .O(new_n55_));
  aoi21  g38(.a(new_n54_), .b(new_n21_), .c(new_n55_), .O(new_n56_));
  aoi21  g39(.a(new_n56_), .b(new_n51_), .c(x6), .O(new_n57_));
  oai21  g40(.a(new_n57_), .b(new_n50_), .c(x4), .O(new_n58_));
  nor2   g41(.a(new_n26_), .b(new_n19_), .O(new_n59_));
  inv1   g42(.a(new_n59_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n58_), .c(new_n49_), .O(z2));
  aoi21  g44(.a(x2), .b(x1), .c(x0), .O(new_n62_));
  nand2  g45(.a(new_n22_), .b(x2), .O(new_n63_));
  oai21  g46(.a(new_n22_), .b(new_n20_), .c(new_n63_), .O(new_n64_));
  aoi21  g47(.a(new_n64_), .b(x1), .c(new_n62_), .O(new_n65_));
  nand3  g48(.a(new_n24_), .b(new_n19_), .c(x0), .O(new_n66_));
  oai21  g49(.a(new_n65_), .b(new_n19_), .c(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n26_), .O(new_n68_));
  aoi21  g51(.a(new_n29_), .b(new_n26_), .c(new_n34_), .O(new_n69_));
  nand3  g52(.a(x6), .b(x3), .c(x2), .O(new_n70_));
  inv1   g53(.a(new_n70_), .O(new_n71_));
  oai21  g54(.a(new_n71_), .b(new_n69_), .c(new_n20_), .O(new_n72_));
  nand2  g55(.a(new_n46_), .b(x0), .O(new_n73_));
  nand2  g56(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g57(.a(new_n74_), .b(new_n19_), .O(new_n75_));
  nand2  g58(.a(new_n75_), .b(new_n68_), .O(z3));
  inv1   g59(.a(new_n23_), .O(new_n77_));
  oai21  g60(.a(new_n77_), .b(new_n20_), .c(new_n29_), .O(new_n78_));
  nand3  g61(.a(new_n78_), .b(x6), .c(new_n19_), .O(new_n79_));
  oai21  g62(.a(new_n30_), .b(x0), .c(new_n23_), .O(new_n80_));
  nand2  g63(.a(new_n80_), .b(new_n26_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n79_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(x1), .O(new_n83_));
  nand2  g66(.a(new_n22_), .b(new_n20_), .O(new_n84_));
  nand3  g67(.a(new_n84_), .b(new_n26_), .c(x2), .O(new_n85_));
  nand2  g68(.a(new_n84_), .b(x2), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(x6), .c(new_n19_), .O(new_n87_));
  nand2  g70(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g71(.a(new_n88_), .b(new_n34_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n83_), .O(z4));
  nand2  g73(.a(x3), .b(new_n21_), .O(new_n91_));
  oai21  g74(.a(new_n91_), .b(new_n34_), .c(new_n63_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(new_n20_), .O(new_n93_));
  aoi21  g76(.a(new_n21_), .b(x1), .c(new_n22_), .O(new_n94_));
  oai21  g77(.a(new_n94_), .b(new_n77_), .c(x0), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(new_n93_), .c(new_n60_), .O(z5));
  nor2   g79(.a(x6), .b(x4), .O(new_n97_));
  aoi21  g80(.a(new_n19_), .b(x4), .c(new_n97_), .O(new_n98_));
  nand3  g81(.a(new_n26_), .b(new_n19_), .c(x0), .O(new_n99_));
  nand2  g82(.a(new_n99_), .b(x3), .O(new_n100_));
  oai21  g83(.a(new_n98_), .b(new_n20_), .c(new_n100_), .O(new_n101_));
  aoi21  g84(.a(new_n101_), .b(x2), .c(new_n22_), .O(new_n102_));
  inv1   g85(.a(new_n91_), .O(new_n103_));
  aoi21  g86(.a(new_n103_), .b(new_n34_), .c(new_n59_), .O(new_n104_));
  oai21  g87(.a(new_n102_), .b(new_n34_), .c(new_n104_), .O(z6));
  nand3  g88(.a(new_n91_), .b(new_n63_), .c(new_n60_), .O(z7));
  nor2   g89(.a(new_n59_), .b(x3), .O(z8));
  oai21  g90(.a(new_n39_), .b(x0), .c(x4), .O(new_n108_));
  aoi21  g91(.a(new_n108_), .b(new_n26_), .c(new_n19_), .O(z9));
endmodule


