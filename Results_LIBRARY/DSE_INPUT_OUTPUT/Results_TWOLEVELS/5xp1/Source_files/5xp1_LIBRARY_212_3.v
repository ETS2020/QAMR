// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n102_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n120_, new_n121_, new_n122_, new_n123_, new_n126_, new_n127_,
    new_n128_;
  inv1   g000(.a(x4), .O(new_n18_));
  inv1   g001(.a(x5), .O(new_n19_));
  inv1   g002(.a(x0), .O(new_n20_));
  inv1   g003(.a(x2), .O(new_n21_));
  inv1   g004(.a(x3), .O(new_n22_));
  nand2  g005(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand2  g006(.a(new_n23_), .b(x1), .O(new_n24_));
  nor2   g007(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  inv1   g008(.a(x6), .O(new_n26_));
  nand2  g009(.a(new_n25_), .b(new_n26_), .O(new_n27_));
  oai21  g010(.a(new_n27_), .b(new_n25_), .c(new_n19_), .O(new_n28_));
  nand2  g011(.a(x3), .b(x2), .O(new_n29_));
  inv1   g012(.a(new_n29_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(x1), .O(new_n31_));
  nand3  g014(.a(new_n31_), .b(new_n26_), .c(new_n20_), .O(new_n32_));
  aoi21  g015(.a(new_n32_), .b(new_n28_), .c(new_n18_), .O(z0));
  nor2   g016(.a(x5), .b(new_n18_), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(x0), .O(new_n35_));
  nand4  g018(.a(new_n26_), .b(x5), .c(new_n18_), .d(x3), .O(new_n36_));
  aoi21  g019(.a(new_n36_), .b(new_n35_), .c(new_n21_), .O(new_n37_));
  nand2  g020(.a(new_n34_), .b(x3), .O(new_n38_));
  aoi21  g021(.a(new_n38_), .b(new_n26_), .c(new_n20_), .O(new_n39_));
  oai21  g022(.a(new_n39_), .b(new_n37_), .c(x1), .O(new_n40_));
  nand2  g023(.a(new_n26_), .b(x5), .O(new_n41_));
  oai22  g024(.a(new_n41_), .b(x4), .c(new_n26_), .d(new_n21_), .O(new_n42_));
  nand2  g025(.a(new_n42_), .b(x0), .O(new_n43_));
  aoi21  g026(.a(new_n19_), .b(new_n18_), .c(new_n26_), .O(new_n44_));
  nand2  g027(.a(x3), .b(x1), .O(new_n45_));
  nand2  g028(.a(new_n45_), .b(new_n26_), .O(new_n46_));
  aoi21  g029(.a(new_n46_), .b(x2), .c(new_n19_), .O(new_n47_));
  aoi21  g030(.a(new_n47_), .b(new_n20_), .c(new_n44_), .O(new_n48_));
  nand3  g031(.a(new_n48_), .b(new_n43_), .c(new_n40_), .O(z1));
  inv1   g032(.a(x1), .O(new_n50_));
  nand2  g033(.a(x5), .b(x0), .O(new_n51_));
  nand2  g034(.a(new_n51_), .b(new_n50_), .O(new_n52_));
  oai21  g035(.a(x5), .b(x3), .c(x0), .O(new_n53_));
  nand2  g036(.a(x5), .b(x3), .O(new_n54_));
  aoi22  g037(.a(new_n54_), .b(new_n20_), .c(new_n53_), .d(new_n21_), .O(new_n55_));
  aoi21  g038(.a(new_n55_), .b(new_n52_), .c(new_n18_), .O(new_n56_));
  nand2  g039(.a(new_n23_), .b(x0), .O(new_n57_));
  oai21  g040(.a(new_n54_), .b(new_n21_), .c(new_n57_), .O(new_n58_));
  nand2  g041(.a(new_n58_), .b(x1), .O(new_n59_));
  aoi21  g042(.a(new_n59_), .b(new_n51_), .c(x4), .O(new_n60_));
  oai21  g043(.a(new_n60_), .b(new_n56_), .c(new_n26_), .O(new_n61_));
  nor2   g044(.a(x2), .b(x1), .O(new_n62_));
  inv1   g045(.a(new_n62_), .O(new_n63_));
  nand3  g046(.a(new_n63_), .b(x4), .c(x0), .O(new_n64_));
  oai21  g047(.a(new_n62_), .b(new_n20_), .c(new_n18_), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(new_n64_), .O(new_n66_));
  nand3  g049(.a(new_n66_), .b(x6), .c(new_n19_), .O(new_n67_));
  nand2  g050(.a(new_n67_), .b(new_n61_), .O(z2));
  nand2  g051(.a(new_n45_), .b(new_n20_), .O(new_n69_));
  nand2  g052(.a(x2), .b(x0), .O(new_n70_));
  oai21  g053(.a(new_n22_), .b(x2), .c(new_n70_), .O(new_n71_));
  nand2  g054(.a(new_n71_), .b(x1), .O(new_n72_));
  aoi21  g055(.a(new_n72_), .b(new_n69_), .c(new_n19_), .O(new_n73_));
  nand3  g056(.a(new_n24_), .b(new_n19_), .c(x0), .O(new_n74_));
  inv1   g057(.a(new_n74_), .O(new_n75_));
  oai21  g058(.a(new_n75_), .b(new_n73_), .c(new_n26_), .O(new_n76_));
  aoi21  g059(.a(new_n29_), .b(new_n26_), .c(new_n50_), .O(new_n77_));
  nand3  g060(.a(x6), .b(x3), .c(x2), .O(new_n78_));
  inv1   g061(.a(new_n78_), .O(new_n79_));
  oai21  g062(.a(new_n79_), .b(new_n77_), .c(new_n20_), .O(new_n80_));
  oai21  g063(.a(new_n63_), .b(new_n20_), .c(new_n80_), .O(new_n81_));
  nand2  g064(.a(new_n81_), .b(new_n19_), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(new_n76_), .O(z3));
  nand3  g066(.a(new_n19_), .b(x3), .c(x2), .O(new_n84_));
  aoi21  g067(.a(new_n84_), .b(new_n57_), .c(new_n26_), .O(new_n85_));
  nand2  g068(.a(new_n29_), .b(new_n20_), .O(new_n86_));
  aoi21  g069(.a(new_n86_), .b(new_n23_), .c(x6), .O(new_n87_));
  oai21  g070(.a(new_n87_), .b(new_n85_), .c(x1), .O(new_n88_));
  nor2   g071(.a(x3), .b(x0), .O(new_n89_));
  nor3   g072(.a(new_n89_), .b(x6), .c(new_n21_), .O(new_n90_));
  inv1   g073(.a(new_n89_), .O(new_n91_));
  aoi21  g074(.a(new_n91_), .b(x2), .c(new_n26_), .O(new_n92_));
  oai21  g075(.a(new_n92_), .b(new_n90_), .c(new_n50_), .O(new_n93_));
  nand2  g076(.a(x6), .b(x5), .O(new_n94_));
  nand3  g077(.a(new_n94_), .b(new_n93_), .c(new_n88_), .O(z4));
  nand3  g078(.a(x3), .b(new_n21_), .c(x1), .O(new_n96_));
  oai21  g079(.a(x3), .b(new_n21_), .c(new_n96_), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(new_n20_), .O(new_n98_));
  aoi21  g081(.a(x3), .b(x1), .c(x2), .O(new_n99_));
  oai21  g082(.a(new_n99_), .b(new_n30_), .c(x0), .O(new_n100_));
  nand3  g083(.a(new_n100_), .b(new_n98_), .c(new_n94_), .O(z5));
  oai21  g084(.a(x6), .b(x3), .c(new_n84_), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(new_n20_), .O(new_n103_));
  oai21  g086(.a(x6), .b(x2), .c(x5), .O(new_n104_));
  nand2  g087(.a(new_n104_), .b(new_n22_), .O(new_n105_));
  nand2  g088(.a(new_n19_), .b(x0), .O(new_n106_));
  oai21  g089(.a(new_n41_), .b(new_n22_), .c(new_n106_), .O(new_n107_));
  nand2  g090(.a(new_n107_), .b(x4), .O(new_n108_));
  aoi21  g091(.a(new_n19_), .b(x4), .c(new_n20_), .O(new_n109_));
  nand3  g092(.a(x5), .b(new_n18_), .c(x3), .O(new_n110_));
  inv1   g093(.a(new_n110_), .O(new_n111_));
  oai21  g094(.a(new_n111_), .b(new_n109_), .c(new_n26_), .O(new_n112_));
  nand3  g095(.a(x6), .b(new_n19_), .c(x3), .O(new_n113_));
  nand3  g096(.a(new_n113_), .b(new_n112_), .c(new_n108_), .O(new_n114_));
  nand2  g097(.a(new_n114_), .b(x2), .O(new_n115_));
  nand3  g098(.a(new_n115_), .b(new_n105_), .c(new_n103_), .O(new_n116_));
  nand2  g099(.a(new_n116_), .b(x1), .O(new_n117_));
  nand4  g100(.a(new_n94_), .b(x3), .c(new_n21_), .d(new_n50_), .O(new_n118_));
  nand2  g101(.a(new_n118_), .b(new_n117_), .O(z6));
  nand3  g102(.a(new_n94_), .b(new_n22_), .c(x2), .O(new_n120_));
  oai21  g103(.a(x5), .b(new_n50_), .c(new_n26_), .O(new_n121_));
  nand2  g104(.a(new_n121_), .b(x5), .O(new_n122_));
  nand3  g105(.a(new_n122_), .b(x3), .c(new_n21_), .O(new_n123_));
  nand2  g106(.a(new_n123_), .b(new_n120_), .O(z7));
  nand2  g107(.a(new_n94_), .b(x3), .O(z8));
  nand4  g108(.a(new_n26_), .b(x3), .c(x2), .d(x1), .O(new_n126_));
  nand2  g109(.a(new_n126_), .b(new_n20_), .O(new_n127_));
  nand2  g110(.a(new_n127_), .b(x4), .O(new_n128_));
  aoi21  g111(.a(new_n128_), .b(new_n26_), .c(new_n19_), .O(z9));
endmodule


