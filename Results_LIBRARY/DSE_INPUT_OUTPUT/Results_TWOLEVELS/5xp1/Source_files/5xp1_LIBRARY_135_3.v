// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_;
  inv1   g000(.a(x5), .O(new_n18_));
  nor2   g001(.a(x3), .b(x2), .O(new_n19_));
  inv1   g002(.a(x2), .O(new_n20_));
  inv1   g003(.a(x3), .O(new_n21_));
  nand2  g004(.a(new_n21_), .b(new_n20_), .O(new_n22_));
  nand4  g005(.a(new_n22_), .b(new_n19_), .c(x1), .d(x0), .O(new_n23_));
  inv1   g006(.a(x0), .O(new_n24_));
  nand3  g007(.a(x3), .b(x2), .c(x1), .O(new_n25_));
  nand3  g008(.a(new_n25_), .b(x5), .c(new_n24_), .O(new_n26_));
  inv1   g009(.a(new_n26_), .O(new_n27_));
  aoi21  g010(.a(new_n23_), .b(new_n18_), .c(new_n27_), .O(new_n28_));
  inv1   g011(.a(x1), .O(new_n29_));
  nand3  g012(.a(x6), .b(new_n18_), .c(new_n29_), .O(new_n30_));
  oai21  g013(.a(new_n28_), .b(x6), .c(new_n30_), .O(new_n31_));
  nand2  g014(.a(new_n31_), .b(x4), .O(new_n32_));
  inv1   g015(.a(x4), .O(new_n33_));
  inv1   g016(.a(x6), .O(new_n34_));
  nand2  g017(.a(x3), .b(x2), .O(new_n35_));
  aoi21  g018(.a(new_n35_), .b(new_n24_), .c(new_n34_), .O(new_n36_));
  nand4  g019(.a(new_n36_), .b(x5), .c(new_n33_), .d(new_n29_), .O(new_n37_));
  nand2  g020(.a(new_n37_), .b(new_n32_), .O(z0));
  inv1   g021(.a(new_n30_), .O(new_n39_));
  nor2   g022(.a(new_n19_), .b(x5), .O(new_n40_));
  nand3  g023(.a(new_n40_), .b(x1), .c(x0), .O(new_n41_));
  aoi21  g024(.a(new_n41_), .b(new_n26_), .c(x6), .O(new_n42_));
  oai21  g025(.a(new_n42_), .b(new_n39_), .c(x4), .O(new_n43_));
  nand4  g026(.a(new_n35_), .b(x5), .c(new_n33_), .d(new_n24_), .O(new_n44_));
  nand3  g027(.a(new_n18_), .b(x2), .c(x0), .O(new_n45_));
  aoi21  g028(.a(new_n45_), .b(new_n44_), .c(new_n34_), .O(new_n46_));
  nor2   g029(.a(x6), .b(new_n18_), .O(new_n47_));
  aoi22  g030(.a(new_n47_), .b(new_n33_), .c(new_n46_), .d(new_n29_), .O(new_n48_));
  nand2  g031(.a(new_n48_), .b(new_n43_), .O(z1));
  nand3  g032(.a(new_n35_), .b(x6), .c(new_n29_), .O(new_n50_));
  inv1   g033(.a(new_n50_), .O(new_n51_));
  aoi21  g034(.a(new_n25_), .b(new_n24_), .c(x6), .O(new_n52_));
  aoi21  g035(.a(new_n51_), .b(new_n24_), .c(new_n52_), .O(new_n53_));
  nor2   g036(.a(new_n53_), .b(new_n18_), .O(new_n54_));
  oai21  g037(.a(x6), .b(new_n20_), .c(new_n21_), .O(new_n55_));
  nand3  g038(.a(new_n55_), .b(x1), .c(x0), .O(new_n56_));
  nand2  g039(.a(x2), .b(x0), .O(new_n57_));
  nand3  g040(.a(new_n57_), .b(x6), .c(new_n18_), .O(new_n58_));
  nand2  g041(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  oai21  g042(.a(new_n59_), .b(new_n54_), .c(new_n33_), .O(new_n60_));
  oai21  g043(.a(x5), .b(x2), .c(x0), .O(new_n61_));
  nand2  g044(.a(x5), .b(x3), .O(new_n62_));
  inv1   g045(.a(new_n62_), .O(new_n63_));
  nand2  g046(.a(new_n63_), .b(x2), .O(new_n64_));
  aoi21  g047(.a(new_n64_), .b(new_n61_), .c(new_n34_), .O(new_n65_));
  oai21  g048(.a(new_n18_), .b(new_n24_), .c(new_n29_), .O(new_n66_));
  nand2  g049(.a(x5), .b(new_n24_), .O(new_n67_));
  nand2  g050(.a(new_n18_), .b(new_n21_), .O(new_n68_));
  nand2  g051(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  aoi21  g052(.a(x5), .b(x3), .c(x0), .O(new_n70_));
  aoi21  g053(.a(new_n69_), .b(new_n20_), .c(new_n70_), .O(new_n71_));
  aoi21  g054(.a(new_n71_), .b(new_n66_), .c(x6), .O(new_n72_));
  oai21  g055(.a(new_n72_), .b(new_n65_), .c(x4), .O(new_n73_));
  nand2  g056(.a(x6), .b(x1), .O(new_n74_));
  nand3  g057(.a(new_n74_), .b(new_n73_), .c(new_n60_), .O(z2));
  nand2  g058(.a(x5), .b(new_n21_), .O(new_n76_));
  nand2  g059(.a(x6), .b(new_n29_), .O(new_n77_));
  nand2  g060(.a(new_n34_), .b(x1), .O(new_n78_));
  nand3  g061(.a(new_n18_), .b(x3), .c(new_n24_), .O(new_n79_));
  aoi22  g062(.a(new_n79_), .b(new_n76_), .c(new_n78_), .d(new_n77_), .O(new_n80_));
  nand4  g063(.a(x6), .b(x5), .c(new_n29_), .d(x0), .O(new_n81_));
  inv1   g064(.a(new_n81_), .O(new_n82_));
  oai21  g065(.a(new_n82_), .b(new_n80_), .c(x2), .O(new_n83_));
  nand2  g066(.a(x6), .b(x2), .O(new_n84_));
  nand2  g067(.a(new_n18_), .b(x0), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(new_n67_), .O(new_n86_));
  nand3  g069(.a(new_n86_), .b(new_n84_), .c(new_n29_), .O(new_n87_));
  nand2  g070(.a(new_n63_), .b(x1), .O(new_n88_));
  nor2   g071(.a(x5), .b(x3), .O(new_n89_));
  nand2  g072(.a(new_n89_), .b(new_n20_), .O(new_n90_));
  aoi21  g073(.a(new_n90_), .b(new_n88_), .c(new_n24_), .O(new_n91_));
  nand3  g074(.a(x5), .b(new_n20_), .c(new_n24_), .O(new_n92_));
  inv1   g075(.a(new_n92_), .O(new_n93_));
  oai21  g076(.a(new_n93_), .b(new_n91_), .c(new_n34_), .O(new_n94_));
  nand3  g077(.a(new_n94_), .b(new_n87_), .c(new_n83_), .O(z3));
  nand2  g078(.a(x2), .b(new_n29_), .O(new_n96_));
  inv1   g079(.a(new_n22_), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(x1), .O(new_n98_));
  aoi21  g081(.a(new_n98_), .b(new_n96_), .c(new_n24_), .O(new_n99_));
  nand2  g082(.a(x3), .b(x2), .O(new_n100_));
  nand3  g083(.a(new_n100_), .b(x1), .c(new_n24_), .O(new_n101_));
  oai21  g084(.a(new_n35_), .b(x1), .c(new_n101_), .O(new_n102_));
  oai21  g085(.a(new_n102_), .b(new_n99_), .c(new_n34_), .O(new_n103_));
  oai21  g086(.a(x3), .b(x0), .c(x2), .O(new_n104_));
  nand3  g087(.a(new_n104_), .b(x6), .c(new_n29_), .O(new_n105_));
  nand2  g088(.a(new_n105_), .b(new_n103_), .O(z4));
  nand2  g089(.a(x6), .b(x1), .O(new_n107_));
  nand3  g090(.a(new_n107_), .b(new_n21_), .c(x2), .O(new_n108_));
  nand4  g091(.a(new_n34_), .b(x3), .c(new_n20_), .d(x1), .O(new_n109_));
  aoi21  g092(.a(new_n109_), .b(new_n108_), .c(x0), .O(new_n110_));
  nand2  g093(.a(new_n35_), .b(new_n22_), .O(new_n111_));
  nand3  g094(.a(new_n111_), .b(new_n34_), .c(x1), .O(new_n112_));
  oai21  g095(.a(x3), .b(new_n20_), .c(new_n29_), .O(new_n113_));
  aoi21  g096(.a(new_n113_), .b(new_n112_), .c(new_n24_), .O(new_n114_));
  or2    g097(.a(new_n114_), .b(new_n110_), .O(z5));
  oai21  g098(.a(x5), .b(new_n20_), .c(x3), .O(new_n116_));
  nand2  g099(.a(new_n116_), .b(new_n24_), .O(new_n117_));
  nand2  g100(.a(new_n89_), .b(x0), .O(new_n118_));
  aoi21  g101(.a(new_n118_), .b(new_n62_), .c(new_n33_), .O(new_n119_));
  oai21  g102(.a(new_n33_), .b(x3), .c(x0), .O(new_n120_));
  oai21  g103(.a(new_n33_), .b(new_n21_), .c(x5), .O(new_n121_));
  nand2  g104(.a(new_n121_), .b(new_n120_), .O(new_n122_));
  oai21  g105(.a(new_n122_), .b(new_n119_), .c(x2), .O(new_n123_));
  nand2  g106(.a(new_n97_), .b(x0), .O(new_n124_));
  nand3  g107(.a(new_n124_), .b(new_n123_), .c(new_n117_), .O(new_n125_));
  nand3  g108(.a(new_n125_), .b(new_n34_), .c(x1), .O(new_n126_));
  nand3  g109(.a(x3), .b(new_n20_), .c(new_n29_), .O(new_n127_));
  nand2  g110(.a(new_n127_), .b(new_n126_), .O(z6));
  nor2   g111(.a(x1), .b(new_n24_), .O(new_n129_));
  oai21  g112(.a(new_n129_), .b(x6), .c(x1), .O(new_n130_));
  nand3  g113(.a(new_n130_), .b(new_n21_), .c(x2), .O(new_n131_));
  nand3  g114(.a(new_n74_), .b(x3), .c(new_n20_), .O(new_n132_));
  nand2  g115(.a(new_n132_), .b(new_n131_), .O(z7));
  nor2   g116(.a(new_n18_), .b(new_n33_), .O(new_n134_));
  nand4  g117(.a(new_n134_), .b(new_n18_), .c(x2), .d(x0), .O(new_n135_));
  nand2  g118(.a(new_n135_), .b(new_n34_), .O(new_n136_));
  aoi21  g119(.a(new_n136_), .b(x1), .c(x3), .O(z8));
  inv1   g120(.a(new_n77_), .O(new_n138_));
  oai21  g121(.a(new_n138_), .b(new_n52_), .c(x5), .O(new_n139_));
  nor2   g122(.a(new_n139_), .b(new_n33_), .O(z9));
endmodule


