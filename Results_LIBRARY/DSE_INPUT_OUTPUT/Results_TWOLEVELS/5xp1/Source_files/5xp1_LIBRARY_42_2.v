// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n122_, new_n123_, new_n124_, new_n125_,
    new_n128_;
  inv1   g000(.a(x2), .O(new_n18_));
  nand2  g001(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g002(.a(x4), .O(new_n20_));
  nor2   g003(.a(x5), .b(new_n20_), .O(new_n21_));
  nand2  g004(.a(new_n21_), .b(x3), .O(new_n22_));
  nand3  g005(.a(x6), .b(x5), .c(new_n20_), .O(new_n23_));
  oai21  g006(.a(new_n22_), .b(new_n19_), .c(new_n23_), .O(new_n24_));
  nand2  g007(.a(new_n24_), .b(x0), .O(new_n25_));
  nand3  g008(.a(x5), .b(new_n20_), .c(x3), .O(new_n26_));
  nand2  g009(.a(new_n26_), .b(x6), .O(new_n27_));
  nand2  g010(.a(new_n27_), .b(x2), .O(new_n28_));
  nand3  g011(.a(x5), .b(new_n20_), .c(x1), .O(new_n29_));
  inv1   g012(.a(new_n29_), .O(new_n30_));
  oai21  g013(.a(new_n30_), .b(new_n21_), .c(x6), .O(new_n31_));
  inv1   g014(.a(x6), .O(new_n32_));
  inv1   g015(.a(x1), .O(new_n33_));
  inv1   g016(.a(x3), .O(new_n34_));
  inv1   g017(.a(x5), .O(new_n35_));
  oai21  g018(.a(new_n34_), .b(new_n33_), .c(new_n35_), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(x0), .O(new_n37_));
  nand3  g020(.a(new_n37_), .b(new_n32_), .c(x4), .O(new_n38_));
  nand4  g021(.a(new_n38_), .b(new_n31_), .c(new_n28_), .d(new_n25_), .O(z0));
  nand2  g022(.a(x4), .b(x3), .O(new_n40_));
  oai21  g023(.a(new_n40_), .b(x2), .c(new_n32_), .O(new_n41_));
  nand2  g024(.a(new_n41_), .b(x1), .O(new_n42_));
  nand2  g025(.a(x6), .b(x2), .O(new_n43_));
  aoi21  g026(.a(new_n43_), .b(new_n42_), .c(x5), .O(new_n44_));
  nand4  g027(.a(new_n32_), .b(x5), .c(new_n20_), .d(new_n18_), .O(new_n45_));
  inv1   g028(.a(new_n45_), .O(new_n46_));
  oai21  g029(.a(new_n46_), .b(new_n44_), .c(x0), .O(new_n47_));
  inv1   g030(.a(x0), .O(new_n48_));
  nand2  g031(.a(x3), .b(x2), .O(new_n49_));
  nand4  g032(.a(new_n49_), .b(x6), .c(new_n20_), .d(new_n33_), .O(new_n50_));
  nand2  g033(.a(new_n32_), .b(new_n18_), .O(new_n51_));
  aoi21  g034(.a(new_n51_), .b(new_n50_), .c(new_n35_), .O(new_n52_));
  nor2   g035(.a(new_n32_), .b(x5), .O(new_n53_));
  aoi22  g036(.a(new_n53_), .b(x4), .c(new_n52_), .d(new_n48_), .O(new_n54_));
  nand2  g037(.a(new_n54_), .b(new_n47_), .O(z1));
  nand3  g038(.a(x5), .b(x4), .c(x2), .O(new_n56_));
  nand4  g039(.a(new_n32_), .b(new_n20_), .c(x1), .d(x0), .O(new_n57_));
  nand2  g040(.a(new_n57_), .b(new_n56_), .O(new_n58_));
  nand2  g041(.a(new_n58_), .b(x3), .O(new_n59_));
  nand2  g042(.a(new_n37_), .b(new_n32_), .O(new_n60_));
  aoi21  g043(.a(new_n35_), .b(new_n33_), .c(new_n48_), .O(new_n61_));
  nor2   g044(.a(new_n35_), .b(new_n33_), .O(new_n62_));
  oai21  g045(.a(new_n62_), .b(new_n61_), .c(x6), .O(new_n63_));
  nand2  g046(.a(x2), .b(x0), .O(new_n64_));
  nand3  g047(.a(new_n64_), .b(new_n63_), .c(new_n60_), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(x4), .O(new_n66_));
  nand3  g049(.a(new_n49_), .b(x5), .c(new_n48_), .O(new_n67_));
  nand2  g050(.a(new_n35_), .b(new_n18_), .O(new_n68_));
  aoi21  g051(.a(new_n68_), .b(new_n67_), .c(x1), .O(new_n69_));
  nor2   g052(.a(x5), .b(x0), .O(new_n70_));
  oai21  g053(.a(new_n70_), .b(new_n69_), .c(x6), .O(new_n71_));
  nand4  g054(.a(new_n32_), .b(x5), .c(new_n18_), .d(x0), .O(new_n72_));
  nand2  g055(.a(new_n72_), .b(new_n71_), .O(new_n73_));
  nand2  g056(.a(new_n73_), .b(new_n20_), .O(new_n74_));
  nor2   g057(.a(x6), .b(new_n18_), .O(new_n75_));
  inv1   g058(.a(new_n75_), .O(new_n76_));
  nand4  g059(.a(new_n76_), .b(new_n74_), .c(new_n66_), .d(new_n59_), .O(z2));
  nand4  g060(.a(x5), .b(new_n18_), .c(x1), .d(x0), .O(new_n78_));
  nand3  g061(.a(new_n35_), .b(x2), .c(new_n48_), .O(new_n79_));
  nand2  g062(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand2  g063(.a(new_n80_), .b(x3), .O(new_n81_));
  xnor2a g064(.a(x5), .b(x0), .O(new_n82_));
  nand3  g065(.a(new_n82_), .b(x6), .c(x1), .O(new_n83_));
  nand2  g066(.a(x6), .b(x1), .O(new_n84_));
  nand3  g067(.a(new_n84_), .b(new_n18_), .c(new_n48_), .O(new_n85_));
  nor2   g068(.a(x3), .b(x1), .O(new_n86_));
  oai21  g069(.a(new_n86_), .b(x0), .c(x2), .O(new_n87_));
  nand2  g070(.a(new_n87_), .b(new_n85_), .O(new_n88_));
  nand2  g071(.a(new_n88_), .b(x5), .O(new_n89_));
  nand2  g072(.a(new_n32_), .b(new_n34_), .O(new_n90_));
  oai21  g073(.a(x2), .b(x1), .c(new_n90_), .O(new_n91_));
  nand3  g074(.a(new_n91_), .b(new_n35_), .c(x0), .O(new_n92_));
  and2   g075(.a(new_n92_), .b(new_n76_), .O(new_n93_));
  nand4  g076(.a(new_n93_), .b(new_n89_), .c(new_n83_), .d(new_n81_), .O(z3));
  nor2   g077(.a(new_n32_), .b(new_n34_), .O(new_n95_));
  nand2  g078(.a(new_n95_), .b(new_n18_), .O(new_n96_));
  oai21  g079(.a(new_n18_), .b(new_n33_), .c(new_n96_), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(x0), .O(new_n98_));
  nand3  g081(.a(x6), .b(new_n34_), .c(new_n33_), .O(new_n99_));
  oai21  g082(.a(x6), .b(new_n33_), .c(new_n99_), .O(new_n100_));
  nand2  g083(.a(new_n100_), .b(new_n48_), .O(new_n101_));
  nand2  g084(.a(new_n95_), .b(x2), .O(new_n102_));
  inv1   g085(.a(new_n102_), .O(new_n103_));
  nor2   g086(.a(new_n90_), .b(x2), .O(new_n104_));
  oai21  g087(.a(new_n104_), .b(new_n103_), .c(x1), .O(new_n105_));
  nor2   g088(.a(new_n32_), .b(x2), .O(new_n106_));
  aoi21  g089(.a(new_n106_), .b(new_n33_), .c(new_n75_), .O(new_n107_));
  nand4  g090(.a(new_n107_), .b(new_n105_), .c(new_n101_), .d(new_n98_), .O(z4));
  nand3  g091(.a(x3), .b(new_n18_), .c(x1), .O(new_n109_));
  nand3  g092(.a(x6), .b(new_n34_), .c(x2), .O(new_n110_));
  nand2  g093(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(new_n48_), .O(new_n112_));
  aoi21  g095(.a(x3), .b(x1), .c(x2), .O(new_n113_));
  oai21  g096(.a(new_n113_), .b(new_n103_), .c(x0), .O(new_n114_));
  nand2  g097(.a(new_n114_), .b(new_n112_), .O(z5));
  nand2  g098(.a(x3), .b(new_n33_), .O(new_n116_));
  oai21  g099(.a(new_n90_), .b(new_n33_), .c(new_n116_), .O(new_n117_));
  nand2  g100(.a(new_n117_), .b(new_n18_), .O(new_n118_));
  nand2  g101(.a(x3), .b(new_n18_), .O(new_n119_));
  nand3  g102(.a(new_n119_), .b(x6), .c(x1), .O(new_n120_));
  nand2  g103(.a(new_n120_), .b(new_n118_), .O(z6));
  inv1   g104(.a(new_n57_), .O(new_n122_));
  nor2   g105(.a(x5), .b(x4), .O(new_n123_));
  nand4  g106(.a(new_n123_), .b(new_n32_), .c(x1), .d(x0), .O(new_n124_));
  aoi21  g107(.a(new_n124_), .b(new_n18_), .c(new_n122_), .O(new_n125_));
  oai22  g108(.a(new_n125_), .b(new_n34_), .c(new_n95_), .d(new_n18_), .O(z7));
  nand2  g109(.a(new_n76_), .b(x3), .O(z8));
  oai21  g110(.a(x6), .b(x0), .c(x5), .O(new_n128_));
  oai21  g111(.a(new_n128_), .b(new_n20_), .c(new_n76_), .O(z9));
endmodule


