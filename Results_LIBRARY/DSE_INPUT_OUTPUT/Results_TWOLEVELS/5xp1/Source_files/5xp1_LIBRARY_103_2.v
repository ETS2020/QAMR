// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:48 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n105_, new_n106_, new_n109_,
    new_n110_, new_n111_;
  inv1   g00(.a(x0), .O(new_n18_));
  inv1   g01(.a(x5), .O(new_n19_));
  oai21  g02(.a(x6), .b(new_n18_), .c(new_n19_), .O(new_n20_));
  inv1   g03(.a(x1), .O(new_n21_));
  inv1   g04(.a(x6), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  oai21  g06(.a(new_n23_), .b(new_n21_), .c(new_n22_), .O(new_n24_));
  oai21  g07(.a(new_n24_), .b(x0), .c(new_n20_), .O(new_n25_));
  nand2  g08(.a(new_n25_), .b(x4), .O(new_n26_));
  inv1   g09(.a(x4), .O(new_n27_));
  nand3  g10(.a(new_n23_), .b(new_n21_), .c(new_n18_), .O(new_n28_));
  nand4  g11(.a(new_n28_), .b(x6), .c(x5), .d(new_n27_), .O(new_n29_));
  nand2  g12(.a(new_n29_), .b(new_n26_), .O(z0));
  oai21  g13(.a(x2), .b(x1), .c(x0), .O(new_n31_));
  aoi21  g14(.a(new_n31_), .b(new_n27_), .c(x5), .O(new_n32_));
  nand4  g15(.a(new_n23_), .b(x5), .c(new_n27_), .d(new_n21_), .O(new_n33_));
  nor2   g16(.a(new_n33_), .b(x0), .O(new_n34_));
  oai21  g17(.a(new_n34_), .b(new_n32_), .c(x6), .O(new_n35_));
  nand4  g18(.a(x4), .b(x3), .c(x2), .d(x1), .O(new_n36_));
  nand4  g19(.a(new_n36_), .b(new_n22_), .c(x5), .d(new_n18_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n35_), .O(z1));
  nand2  g21(.a(x6), .b(x4), .O(new_n39_));
  nor2   g22(.a(x6), .b(x4), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(x3), .c(x2), .d(new_n18_), .O(new_n41_));
  aoi21  g24(.a(new_n41_), .b(new_n39_), .c(new_n21_), .O(new_n42_));
  nand4  g25(.a(new_n23_), .b(new_n27_), .c(new_n21_), .d(new_n18_), .O(new_n43_));
  nand2  g26(.a(new_n23_), .b(new_n18_), .O(new_n44_));
  nand2  g27(.a(new_n44_), .b(x4), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n43_), .c(new_n22_), .O(new_n46_));
  oai21  g29(.a(new_n46_), .b(new_n42_), .c(x5), .O(new_n47_));
  inv1   g30(.a(x2), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(new_n21_), .O(new_n49_));
  nand3  g32(.a(new_n49_), .b(x6), .c(x0), .O(new_n50_));
  nand4  g33(.a(x5), .b(x3), .c(x2), .d(x1), .O(new_n51_));
  nand3  g34(.a(new_n51_), .b(new_n22_), .c(new_n18_), .O(new_n52_));
  nand2  g35(.a(new_n52_), .b(new_n50_), .O(new_n53_));
  nand4  g36(.a(new_n31_), .b(x6), .c(new_n19_), .d(new_n27_), .O(new_n54_));
  inv1   g37(.a(new_n54_), .O(new_n55_));
  aoi21  g38(.a(new_n53_), .b(x4), .c(new_n55_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n47_), .O(z2));
  nand2  g40(.a(x6), .b(new_n18_), .O(new_n58_));
  nand2  g41(.a(new_n22_), .b(x1), .O(new_n59_));
  nand2  g42(.a(new_n59_), .b(new_n58_), .O(new_n60_));
  nand3  g43(.a(new_n60_), .b(new_n19_), .c(x3), .O(new_n61_));
  nor2   g44(.a(x3), .b(x1), .O(new_n62_));
  oai21  g45(.a(new_n62_), .b(x0), .c(x5), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(x2), .O(new_n65_));
  nand3  g48(.a(x6), .b(new_n19_), .c(new_n18_), .O(new_n66_));
  oai21  g49(.a(new_n19_), .b(new_n18_), .c(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(x1), .O(new_n68_));
  nand3  g51(.a(new_n19_), .b(new_n48_), .c(new_n21_), .O(new_n69_));
  aoi21  g52(.a(new_n69_), .b(x6), .c(new_n18_), .O(new_n70_));
  oai21  g53(.a(new_n22_), .b(new_n48_), .c(new_n21_), .O(new_n71_));
  nand2  g54(.a(new_n23_), .b(new_n22_), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n71_), .c(new_n19_), .O(new_n73_));
  aoi21  g56(.a(new_n73_), .b(new_n18_), .c(new_n70_), .O(new_n74_));
  nand3  g57(.a(new_n74_), .b(new_n68_), .c(new_n65_), .O(z3));
  inv1   g58(.a(x3), .O(new_n76_));
  nand3  g59(.a(x6), .b(new_n21_), .c(new_n18_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n59_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand3  g62(.a(new_n22_), .b(x3), .c(x2), .O(new_n80_));
  oai21  g63(.a(new_n22_), .b(x2), .c(new_n80_), .O(new_n81_));
  nand2  g64(.a(new_n81_), .b(new_n21_), .O(new_n82_));
  nand2  g65(.a(x6), .b(x3), .O(new_n83_));
  aoi21  g66(.a(new_n83_), .b(new_n18_), .c(new_n48_), .O(new_n84_));
  nor2   g67(.a(x6), .b(x2), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n84_), .c(x1), .O(new_n86_));
  nor2   g69(.a(new_n76_), .b(x2), .O(new_n87_));
  oai21  g70(.a(new_n87_), .b(new_n22_), .c(x0), .O(new_n88_));
  nand4  g71(.a(new_n88_), .b(new_n86_), .c(new_n82_), .d(new_n79_), .O(z4));
  nand2  g72(.a(new_n87_), .b(x1), .O(new_n90_));
  nand2  g73(.a(new_n76_), .b(x2), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(new_n90_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(new_n18_), .O(new_n93_));
  oai21  g76(.a(new_n76_), .b(new_n21_), .c(new_n48_), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n23_), .O(new_n95_));
  nand3  g78(.a(new_n95_), .b(x6), .c(x0), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n93_), .O(z5));
  nand2  g80(.a(x6), .b(new_n27_), .O(new_n98_));
  nand3  g81(.a(new_n98_), .b(x5), .c(new_n18_), .O(new_n99_));
  nor2   g82(.a(x6), .b(new_n19_), .O(new_n100_));
  nand2  g83(.a(new_n100_), .b(new_n99_), .O(new_n101_));
  aoi21  g84(.a(new_n101_), .b(x2), .c(new_n76_), .O(new_n102_));
  aoi22  g85(.a(new_n87_), .b(new_n21_), .c(new_n22_), .d(x0), .O(new_n103_));
  oai21  g86(.a(new_n102_), .b(new_n21_), .c(new_n103_), .O(z6));
  nand2  g87(.a(x1), .b(x0), .O(new_n105_));
  nand3  g88(.a(new_n105_), .b(x3), .c(new_n48_), .O(new_n106_));
  nand3  g89(.a(new_n106_), .b(new_n91_), .c(new_n88_), .O(z7));
  oai21  g90(.a(x6), .b(new_n18_), .c(x3), .O(z8));
  nand2  g91(.a(x1), .b(new_n18_), .O(new_n109_));
  oai21  g92(.a(new_n109_), .b(new_n23_), .c(new_n22_), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x5), .c(x4), .O(new_n111_));
  inv1   g94(.a(new_n111_), .O(z9));
endmodule


