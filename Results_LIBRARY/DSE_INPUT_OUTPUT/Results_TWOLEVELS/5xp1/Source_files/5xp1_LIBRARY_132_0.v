// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n126_, new_n127_, new_n128_, new_n129_, new_n130_, new_n131_,
    new_n132_, new_n133_, new_n134_, new_n136_, new_n137_;
  inv1   g000(.a(x4), .O(new_n18_));
  nor2   g001(.a(x5), .b(new_n18_), .O(new_n19_));
  nand3  g002(.a(new_n19_), .b(x1), .c(x0), .O(new_n20_));
  inv1   g003(.a(x5), .O(new_n21_));
  inv1   g004(.a(x6), .O(new_n22_));
  nor2   g005(.a(new_n22_), .b(new_n21_), .O(new_n23_));
  nand3  g006(.a(new_n23_), .b(new_n18_), .c(x3), .O(new_n24_));
  nand2  g007(.a(new_n24_), .b(new_n20_), .O(new_n25_));
  nand2  g008(.a(new_n25_), .b(x2), .O(new_n26_));
  nand2  g009(.a(x3), .b(x0), .O(new_n27_));
  inv1   g010(.a(new_n27_), .O(new_n28_));
  nand2  g011(.a(new_n28_), .b(new_n19_), .O(new_n29_));
  nand2  g012(.a(new_n23_), .b(new_n18_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(new_n29_), .O(new_n31_));
  nand2  g014(.a(new_n31_), .b(x1), .O(new_n32_));
  nand3  g015(.a(x5), .b(new_n18_), .c(x0), .O(new_n33_));
  inv1   g016(.a(new_n33_), .O(new_n34_));
  oai21  g017(.a(new_n34_), .b(new_n19_), .c(x6), .O(new_n35_));
  oai21  g018(.a(x3), .b(x2), .c(x1), .O(new_n36_));
  nand3  g019(.a(new_n36_), .b(new_n22_), .c(new_n21_), .O(new_n37_));
  inv1   g020(.a(new_n37_), .O(new_n38_));
  nand3  g021(.a(new_n38_), .b(x4), .c(x0), .O(new_n39_));
  nand4  g022(.a(new_n39_), .b(new_n35_), .c(new_n32_), .d(new_n26_), .O(z0));
  oai21  g023(.a(x2), .b(x1), .c(x0), .O(new_n41_));
  aoi21  g024(.a(new_n41_), .b(new_n18_), .c(x5), .O(new_n42_));
  inv1   g025(.a(x1), .O(new_n43_));
  nand2  g026(.a(x3), .b(x2), .O(new_n44_));
  nand4  g027(.a(new_n44_), .b(x5), .c(new_n18_), .d(new_n43_), .O(new_n45_));
  nor2   g028(.a(new_n45_), .b(x0), .O(new_n46_));
  oai21  g029(.a(new_n46_), .b(new_n42_), .c(x6), .O(new_n47_));
  inv1   g030(.a(x2), .O(new_n48_));
  inv1   g031(.a(x3), .O(new_n49_));
  nand2  g032(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand4  g033(.a(new_n50_), .b(new_n21_), .c(x4), .d(x1), .O(new_n51_));
  nand3  g034(.a(new_n22_), .b(x5), .c(new_n18_), .O(new_n52_));
  nand2  g035(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  nand2  g036(.a(new_n53_), .b(x0), .O(new_n54_));
  nand2  g037(.a(new_n54_), .b(new_n47_), .O(z1));
  nand3  g038(.a(new_n23_), .b(x4), .c(x2), .O(new_n56_));
  nor2   g039(.a(x6), .b(x4), .O(new_n57_));
  nand3  g040(.a(new_n57_), .b(x1), .c(x0), .O(new_n58_));
  nand2  g041(.a(new_n58_), .b(new_n56_), .O(new_n59_));
  nand2  g042(.a(new_n59_), .b(x3), .O(new_n60_));
  nor2   g043(.a(new_n22_), .b(new_n18_), .O(new_n61_));
  nand2  g044(.a(new_n57_), .b(x2), .O(new_n62_));
  inv1   g045(.a(new_n62_), .O(new_n63_));
  oai21  g046(.a(new_n63_), .b(new_n61_), .c(x1), .O(new_n64_));
  oai21  g047(.a(new_n61_), .b(new_n57_), .c(x5), .O(new_n65_));
  nand2  g048(.a(x6), .b(x2), .O(new_n66_));
  nand2  g049(.a(new_n66_), .b(new_n37_), .O(new_n67_));
  nand2  g050(.a(new_n67_), .b(x4), .O(new_n68_));
  nand3  g051(.a(new_n68_), .b(new_n65_), .c(new_n64_), .O(new_n69_));
  nand2  g052(.a(new_n69_), .b(x0), .O(new_n70_));
  inv1   g053(.a(x0), .O(new_n71_));
  nand3  g054(.a(new_n44_), .b(x5), .c(new_n71_), .O(new_n72_));
  nand2  g055(.a(new_n21_), .b(new_n48_), .O(new_n73_));
  aoi21  g056(.a(new_n73_), .b(new_n72_), .c(x1), .O(new_n74_));
  nor2   g057(.a(x5), .b(x0), .O(new_n75_));
  oai21  g058(.a(new_n75_), .b(new_n74_), .c(new_n18_), .O(new_n76_));
  nand3  g059(.a(x5), .b(x4), .c(x1), .O(new_n77_));
  nand2  g060(.a(new_n77_), .b(new_n76_), .O(new_n78_));
  nand2  g061(.a(new_n78_), .b(x6), .O(new_n79_));
  nand3  g062(.a(new_n79_), .b(new_n70_), .c(new_n60_), .O(z2));
  nand3  g063(.a(x5), .b(x1), .c(x0), .O(new_n81_));
  nand4  g064(.a(x6), .b(new_n21_), .c(x2), .d(new_n71_), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(new_n81_), .O(new_n83_));
  nand2  g066(.a(new_n83_), .b(x3), .O(new_n84_));
  oai21  g067(.a(x6), .b(x2), .c(x1), .O(new_n85_));
  aoi21  g068(.a(new_n85_), .b(new_n66_), .c(new_n21_), .O(new_n86_));
  nand2  g069(.a(new_n66_), .b(new_n43_), .O(new_n87_));
  nand3  g070(.a(new_n22_), .b(new_n49_), .c(new_n48_), .O(new_n88_));
  aoi21  g071(.a(new_n88_), .b(new_n87_), .c(x5), .O(new_n89_));
  oai21  g072(.a(new_n89_), .b(new_n86_), .c(x0), .O(new_n90_));
  nand3  g073(.a(new_n44_), .b(x5), .c(new_n43_), .O(new_n91_));
  oai21  g074(.a(x5), .b(new_n43_), .c(new_n91_), .O(new_n92_));
  nand3  g075(.a(new_n92_), .b(x6), .c(new_n71_), .O(new_n93_));
  nand3  g076(.a(new_n93_), .b(new_n90_), .c(new_n84_), .O(z3));
  oai21  g077(.a(x3), .b(x1), .c(x6), .O(new_n95_));
  nand2  g078(.a(new_n95_), .b(new_n71_), .O(new_n96_));
  xor2a  g079(.a(x6), .b(x2), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(new_n43_), .O(new_n98_));
  aoi21  g081(.a(new_n88_), .b(new_n66_), .c(new_n71_), .O(new_n99_));
  nor2   g082(.a(new_n22_), .b(new_n49_), .O(new_n100_));
  nand2  g083(.a(new_n100_), .b(x2), .O(new_n101_));
  inv1   g084(.a(new_n101_), .O(new_n102_));
  oai21  g085(.a(new_n102_), .b(new_n99_), .c(x1), .O(new_n103_));
  nand3  g086(.a(new_n100_), .b(new_n48_), .c(x0), .O(new_n104_));
  nand4  g087(.a(new_n104_), .b(new_n103_), .c(new_n98_), .d(new_n96_), .O(z4));
  nand3  g088(.a(x6), .b(new_n49_), .c(new_n71_), .O(new_n106_));
  nand2  g089(.a(new_n106_), .b(new_n27_), .O(new_n107_));
  nand2  g090(.a(new_n107_), .b(x2), .O(new_n108_));
  oai21  g091(.a(new_n49_), .b(new_n43_), .c(x0), .O(new_n109_));
  nand3  g092(.a(new_n100_), .b(x1), .c(new_n71_), .O(new_n110_));
  nand2  g093(.a(new_n110_), .b(new_n109_), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(new_n48_), .O(new_n112_));
  nand2  g095(.a(new_n22_), .b(new_n71_), .O(new_n113_));
  nand3  g096(.a(new_n113_), .b(new_n112_), .c(new_n108_), .O(z5));
  nand2  g097(.a(x3), .b(new_n43_), .O(new_n115_));
  nand3  g098(.a(new_n22_), .b(new_n49_), .c(x1), .O(new_n116_));
  aoi21  g099(.a(new_n116_), .b(new_n115_), .c(new_n71_), .O(new_n117_));
  nor3   g100(.a(new_n22_), .b(new_n49_), .c(x1), .O(new_n118_));
  oai21  g101(.a(new_n118_), .b(new_n117_), .c(new_n48_), .O(new_n119_));
  oai21  g102(.a(new_n49_), .b(x2), .c(x6), .O(new_n120_));
  nand3  g103(.a(x6), .b(new_n21_), .c(new_n18_), .O(new_n121_));
  nand3  g104(.a(new_n121_), .b(x2), .c(x0), .O(new_n122_));
  nand2  g105(.a(new_n122_), .b(new_n120_), .O(new_n123_));
  nand2  g106(.a(new_n123_), .b(x1), .O(new_n124_));
  nand2  g107(.a(new_n124_), .b(new_n119_), .O(z6));
  nor2   g108(.a(new_n49_), .b(x2), .O(new_n126_));
  nand2  g109(.a(new_n49_), .b(x2), .O(new_n127_));
  inv1   g110(.a(new_n127_), .O(new_n128_));
  oai21  g111(.a(new_n128_), .b(new_n126_), .c(x0), .O(new_n129_));
  nand2  g112(.a(new_n126_), .b(x1), .O(new_n130_));
  aoi21  g113(.a(new_n130_), .b(new_n127_), .c(x0), .O(new_n131_));
  nand2  g114(.a(new_n126_), .b(new_n43_), .O(new_n132_));
  inv1   g115(.a(new_n132_), .O(new_n133_));
  oai21  g116(.a(new_n133_), .b(new_n131_), .c(x6), .O(new_n134_));
  nand2  g117(.a(new_n134_), .b(new_n129_), .O(z7));
  nand3  g118(.a(new_n22_), .b(x1), .c(new_n71_), .O(new_n136_));
  nand2  g119(.a(new_n136_), .b(new_n49_), .O(new_n137_));
  nand2  g120(.a(new_n137_), .b(new_n113_), .O(z8));
  oai21  g121(.a(new_n21_), .b(new_n18_), .c(new_n113_), .O(z9));
endmodule


