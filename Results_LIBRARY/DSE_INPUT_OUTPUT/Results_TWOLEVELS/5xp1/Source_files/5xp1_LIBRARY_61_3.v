// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:40 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n42_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n111_,
    new_n112_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n119_, new_n120_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n135_, new_n136_, new_n137_;
  inv1   g000(.a(x4), .O(new_n18_));
  nand2  g001(.a(new_n18_), .b(x2), .O(new_n19_));
  nand2  g002(.a(x6), .b(x5), .O(new_n20_));
  nand2  g003(.a(x1), .b(x0), .O(new_n21_));
  inv1   g004(.a(x5), .O(new_n22_));
  inv1   g005(.a(x6), .O(new_n23_));
  nand3  g006(.a(new_n23_), .b(new_n22_), .c(x4), .O(new_n24_));
  oai22  g007(.a(new_n24_), .b(new_n21_), .c(new_n20_), .d(new_n19_), .O(new_n25_));
  nand2  g008(.a(new_n25_), .b(x3), .O(new_n26_));
  nand2  g009(.a(x2), .b(x0), .O(new_n27_));
  oai22  g010(.a(new_n27_), .b(new_n24_), .c(new_n20_), .d(x4), .O(new_n28_));
  nand2  g011(.a(new_n28_), .b(x1), .O(new_n29_));
  inv1   g012(.a(x2), .O(new_n30_));
  aoi21  g013(.a(new_n30_), .b(x1), .c(new_n23_), .O(new_n31_));
  nand4  g014(.a(new_n31_), .b(x5), .c(new_n18_), .d(x0), .O(new_n32_));
  inv1   g015(.a(x1), .O(new_n33_));
  nand2  g016(.a(x5), .b(x0), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  oai21  g018(.a(x5), .b(x3), .c(x0), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  inv1   g020(.a(x0), .O(new_n38_));
  inv1   g021(.a(x3), .O(new_n39_));
  oai21  g022(.a(new_n22_), .b(new_n39_), .c(new_n38_), .O(new_n40_));
  nand3  g023(.a(new_n40_), .b(new_n37_), .c(new_n35_), .O(new_n41_));
  nand3  g024(.a(new_n41_), .b(new_n23_), .c(x4), .O(new_n42_));
  nand4  g025(.a(new_n42_), .b(new_n32_), .c(new_n29_), .d(new_n26_), .O(z0));
  oai21  g026(.a(x4), .b(x1), .c(x6), .O(new_n44_));
  nand2  g027(.a(new_n44_), .b(new_n30_), .O(new_n45_));
  oai21  g028(.a(x4), .b(x3), .c(x6), .O(new_n46_));
  aoi22  g029(.a(new_n46_), .b(new_n33_), .c(new_n23_), .d(new_n39_), .O(new_n47_));
  aoi21  g030(.a(new_n47_), .b(new_n45_), .c(x0), .O(new_n48_));
  nand3  g031(.a(x3), .b(x2), .c(x1), .O(new_n49_));
  nand2  g032(.a(new_n49_), .b(new_n38_), .O(new_n50_));
  nand3  g033(.a(new_n50_), .b(new_n23_), .c(new_n18_), .O(new_n51_));
  inv1   g034(.a(new_n51_), .O(new_n52_));
  oai21  g035(.a(new_n52_), .b(new_n48_), .c(x5), .O(new_n53_));
  aoi21  g036(.a(x4), .b(x1), .c(x6), .O(new_n54_));
  aoi21  g037(.a(x4), .b(x3), .c(x6), .O(new_n55_));
  oai22  g038(.a(new_n55_), .b(new_n33_), .c(new_n54_), .d(new_n30_), .O(new_n56_));
  nand3  g039(.a(new_n56_), .b(new_n22_), .c(x0), .O(new_n57_));
  nand2  g040(.a(x6), .b(x4), .O(new_n58_));
  nand3  g041(.a(new_n58_), .b(new_n57_), .c(new_n53_), .O(z1));
  nand2  g042(.a(new_n41_), .b(x4), .O(new_n60_));
  inv1   g043(.a(new_n34_), .O(new_n61_));
  oai21  g044(.a(x3), .b(x2), .c(x0), .O(new_n62_));
  nand3  g045(.a(x5), .b(x3), .c(x2), .O(new_n63_));
  aoi21  g046(.a(new_n63_), .b(new_n62_), .c(new_n33_), .O(new_n64_));
  oai21  g047(.a(new_n64_), .b(new_n61_), .c(new_n18_), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(new_n60_), .O(new_n66_));
  nand2  g049(.a(new_n66_), .b(new_n23_), .O(new_n67_));
  oai21  g050(.a(new_n22_), .b(x2), .c(x3), .O(new_n68_));
  aoi22  g051(.a(new_n68_), .b(new_n38_), .c(new_n22_), .d(new_n30_), .O(new_n69_));
  nand2  g052(.a(x3), .b(x2), .O(new_n70_));
  nand2  g053(.a(new_n70_), .b(new_n33_), .O(new_n71_));
  nand3  g054(.a(new_n71_), .b(new_n22_), .c(new_n38_), .O(new_n72_));
  oai21  g055(.a(new_n69_), .b(x1), .c(new_n72_), .O(new_n73_));
  nand3  g056(.a(new_n73_), .b(x6), .c(new_n18_), .O(new_n74_));
  nand2  g057(.a(new_n74_), .b(new_n67_), .O(z2));
  nand2  g058(.a(x6), .b(new_n18_), .O(new_n76_));
  oai21  g059(.a(x6), .b(new_n33_), .c(new_n76_), .O(new_n77_));
  nand3  g060(.a(new_n22_), .b(x3), .c(new_n38_), .O(new_n78_));
  nand2  g061(.a(new_n78_), .b(new_n34_), .O(new_n79_));
  nand3  g062(.a(new_n79_), .b(new_n77_), .c(x2), .O(new_n80_));
  nand3  g063(.a(new_n22_), .b(new_n30_), .c(new_n33_), .O(new_n81_));
  nand3  g064(.a(x6), .b(x5), .c(x1), .O(new_n82_));
  aoi21  g065(.a(new_n82_), .b(new_n81_), .c(new_n38_), .O(new_n83_));
  oai21  g066(.a(new_n23_), .b(x2), .c(x3), .O(new_n84_));
  nand3  g067(.a(new_n84_), .b(x5), .c(new_n33_), .O(new_n85_));
  nand3  g068(.a(x6), .b(new_n22_), .c(x1), .O(new_n86_));
  aoi21  g069(.a(new_n86_), .b(new_n85_), .c(x0), .O(new_n87_));
  oai21  g070(.a(new_n87_), .b(new_n83_), .c(new_n18_), .O(new_n88_));
  nand2  g071(.a(new_n39_), .b(new_n30_), .O(new_n89_));
  nand2  g072(.a(new_n89_), .b(x1), .O(new_n90_));
  nand3  g073(.a(new_n90_), .b(new_n22_), .c(x0), .O(new_n91_));
  inv1   g074(.a(new_n91_), .O(new_n92_));
  oai21  g075(.a(new_n39_), .b(new_n33_), .c(new_n38_), .O(new_n93_));
  nor2   g076(.a(new_n39_), .b(x2), .O(new_n94_));
  nand2  g077(.a(new_n94_), .b(x1), .O(new_n95_));
  aoi21  g078(.a(new_n95_), .b(new_n93_), .c(new_n22_), .O(new_n96_));
  oai21  g079(.a(new_n96_), .b(new_n92_), .c(new_n23_), .O(new_n97_));
  nand3  g080(.a(new_n97_), .b(new_n88_), .c(new_n80_), .O(z3));
  oai21  g081(.a(x3), .b(x0), .c(x2), .O(new_n99_));
  nand2  g082(.a(new_n99_), .b(new_n33_), .O(new_n100_));
  nor2   g083(.a(new_n30_), .b(new_n33_), .O(new_n101_));
  oai21  g084(.a(new_n101_), .b(new_n94_), .c(x0), .O(new_n102_));
  nand4  g085(.a(new_n102_), .b(new_n100_), .c(new_n49_), .d(new_n18_), .O(new_n103_));
  nand2  g086(.a(new_n103_), .b(x6), .O(new_n104_));
  nand2  g087(.a(new_n70_), .b(new_n38_), .O(new_n105_));
  aoi21  g088(.a(new_n105_), .b(new_n89_), .c(new_n33_), .O(new_n106_));
  oai21  g089(.a(x3), .b(x0), .c(x2), .O(new_n107_));
  nor2   g090(.a(new_n107_), .b(x1), .O(new_n108_));
  oai21  g091(.a(new_n108_), .b(new_n106_), .c(new_n23_), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(new_n104_), .O(z4));
  nor2   g093(.a(x3), .b(new_n30_), .O(new_n111_));
  inv1   g094(.a(new_n111_), .O(new_n112_));
  nand2  g095(.a(new_n112_), .b(new_n95_), .O(new_n113_));
  nand2  g096(.a(new_n113_), .b(new_n38_), .O(new_n114_));
  inv1   g097(.a(new_n70_), .O(new_n115_));
  aoi21  g098(.a(x3), .b(x1), .c(x2), .O(new_n116_));
  oai21  g099(.a(new_n116_), .b(new_n115_), .c(x0), .O(new_n117_));
  nand3  g100(.a(new_n117_), .b(new_n114_), .c(new_n58_), .O(z5));
  oai21  g101(.a(x5), .b(new_n30_), .c(x3), .O(new_n119_));
  nand2  g102(.a(new_n119_), .b(new_n38_), .O(new_n120_));
  oai21  g103(.a(x3), .b(x0), .c(x5), .O(new_n121_));
  nand2  g104(.a(x5), .b(x4), .O(new_n122_));
  nand2  g105(.a(new_n122_), .b(x0), .O(new_n123_));
  nand2  g106(.a(new_n123_), .b(new_n121_), .O(new_n124_));
  nand2  g107(.a(new_n124_), .b(x2), .O(new_n125_));
  nand3  g108(.a(new_n125_), .b(new_n120_), .c(new_n89_), .O(new_n126_));
  aoi21  g109(.a(x6), .b(x2), .c(new_n39_), .O(new_n127_));
  nor2   g110(.a(new_n127_), .b(x4), .O(new_n128_));
  aoi21  g111(.a(new_n126_), .b(new_n23_), .c(new_n128_), .O(new_n129_));
  nand4  g112(.a(new_n58_), .b(x3), .c(new_n30_), .d(new_n33_), .O(new_n130_));
  oai21  g113(.a(new_n129_), .b(new_n33_), .c(new_n130_), .O(z6));
  inv1   g114(.a(new_n94_), .O(new_n132_));
  nand3  g115(.a(new_n112_), .b(new_n132_), .c(new_n58_), .O(z7));
  aoi21  g116(.a(x6), .b(x4), .c(x3), .O(z8));
  nand3  g117(.a(new_n101_), .b(new_n23_), .c(x3), .O(new_n135_));
  nand2  g118(.a(new_n135_), .b(new_n38_), .O(new_n136_));
  nand2  g119(.a(new_n136_), .b(x5), .O(new_n137_));
  aoi21  g120(.a(new_n137_), .b(new_n23_), .c(new_n18_), .O(z9));
endmodule


