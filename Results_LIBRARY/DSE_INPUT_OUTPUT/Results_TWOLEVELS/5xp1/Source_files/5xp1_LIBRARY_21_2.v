// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:33 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n114_, new_n115_, new_n116_, new_n117_, new_n118_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g000(.a(x0), .O(new_n18_));
  inv1   g001(.a(x1), .O(new_n19_));
  inv1   g002(.a(x5), .O(new_n20_));
  oai21  g003(.a(new_n20_), .b(new_n18_), .c(new_n19_), .O(new_n21_));
  nand2  g004(.a(x3), .b(x2), .O(new_n22_));
  nand3  g005(.a(new_n22_), .b(x5), .c(new_n18_), .O(new_n23_));
  aoi21  g006(.a(new_n23_), .b(new_n21_), .c(x6), .O(new_n24_));
  inv1   g007(.a(x6), .O(new_n25_));
  nor2   g008(.a(new_n25_), .b(x5), .O(new_n26_));
  nand2  g009(.a(new_n26_), .b(new_n19_), .O(new_n27_));
  inv1   g010(.a(new_n27_), .O(new_n28_));
  oai21  g011(.a(new_n28_), .b(new_n24_), .c(x4), .O(new_n29_));
  inv1   g012(.a(x4), .O(new_n30_));
  nand3  g013(.a(new_n22_), .b(new_n19_), .c(new_n18_), .O(new_n31_));
  nand4  g014(.a(new_n31_), .b(x6), .c(x5), .d(new_n30_), .O(new_n32_));
  nand2  g015(.a(new_n32_), .b(new_n29_), .O(z0));
  nand3  g016(.a(new_n26_), .b(x2), .c(new_n19_), .O(new_n34_));
  nand3  g017(.a(new_n25_), .b(x5), .c(new_n30_), .O(new_n35_));
  nand2  g018(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(x0), .O(new_n37_));
  nand3  g020(.a(x6), .b(new_n30_), .c(new_n19_), .O(new_n38_));
  nand2  g021(.a(new_n25_), .b(x1), .O(new_n39_));
  nand2  g022(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nor2   g023(.a(x6), .b(x1), .O(new_n41_));
  aoi21  g024(.a(new_n40_), .b(new_n22_), .c(new_n41_), .O(new_n42_));
  nand2  g025(.a(new_n25_), .b(new_n30_), .O(new_n43_));
  inv1   g026(.a(new_n43_), .O(new_n44_));
  nand4  g027(.a(new_n44_), .b(x3), .c(x2), .d(x1), .O(new_n45_));
  oai21  g028(.a(new_n42_), .b(x0), .c(new_n45_), .O(new_n46_));
  nand2  g029(.a(new_n46_), .b(x5), .O(new_n47_));
  nand3  g030(.a(new_n26_), .b(x4), .c(new_n19_), .O(new_n48_));
  nand3  g031(.a(new_n48_), .b(new_n47_), .c(new_n37_), .O(z1));
  nand2  g032(.a(x6), .b(x4), .O(new_n50_));
  inv1   g033(.a(new_n50_), .O(new_n51_));
  oai21  g034(.a(new_n51_), .b(new_n44_), .c(x0), .O(new_n52_));
  oai21  g035(.a(new_n43_), .b(new_n22_), .c(new_n50_), .O(new_n53_));
  nand2  g036(.a(new_n53_), .b(x1), .O(new_n54_));
  nand2  g037(.a(new_n25_), .b(x4), .O(new_n55_));
  nand2  g038(.a(new_n55_), .b(new_n38_), .O(new_n56_));
  nand3  g039(.a(new_n56_), .b(new_n22_), .c(new_n18_), .O(new_n57_));
  inv1   g040(.a(new_n22_), .O(new_n58_));
  nand2  g041(.a(new_n51_), .b(new_n58_), .O(new_n59_));
  nand4  g042(.a(new_n59_), .b(new_n57_), .c(new_n54_), .d(new_n52_), .O(new_n60_));
  nand2  g043(.a(new_n60_), .b(x5), .O(new_n61_));
  nand2  g044(.a(new_n26_), .b(new_n30_), .O(new_n62_));
  aoi21  g045(.a(new_n62_), .b(new_n55_), .c(x0), .O(new_n63_));
  inv1   g046(.a(x2), .O(new_n64_));
  oai21  g047(.a(new_n64_), .b(new_n18_), .c(x6), .O(new_n65_));
  nand2  g048(.a(new_n65_), .b(x4), .O(new_n66_));
  nand3  g049(.a(x6), .b(new_n30_), .c(new_n64_), .O(new_n67_));
  aoi21  g050(.a(new_n67_), .b(new_n66_), .c(x5), .O(new_n68_));
  oai21  g051(.a(new_n68_), .b(new_n63_), .c(new_n19_), .O(new_n69_));
  nand2  g052(.a(new_n69_), .b(new_n61_), .O(z2));
  nand3  g053(.a(x5), .b(x1), .c(x0), .O(new_n71_));
  nand2  g054(.a(new_n19_), .b(new_n18_), .O(new_n72_));
  nand2  g055(.a(new_n26_), .b(x2), .O(new_n73_));
  oai21  g056(.a(new_n73_), .b(new_n72_), .c(new_n71_), .O(new_n74_));
  nand2  g057(.a(new_n74_), .b(x3), .O(new_n75_));
  nand2  g058(.a(new_n64_), .b(new_n19_), .O(new_n76_));
  nand3  g059(.a(new_n76_), .b(x6), .c(x0), .O(new_n77_));
  oai21  g060(.a(new_n64_), .b(new_n19_), .c(new_n18_), .O(new_n78_));
  inv1   g061(.a(x3), .O(new_n79_));
  nand2  g062(.a(new_n79_), .b(x2), .O(new_n80_));
  oai21  g063(.a(new_n80_), .b(new_n19_), .c(new_n78_), .O(new_n81_));
  nand2  g064(.a(new_n81_), .b(new_n25_), .O(new_n82_));
  nand3  g065(.a(new_n82_), .b(new_n77_), .c(new_n31_), .O(new_n83_));
  nand2  g066(.a(new_n83_), .b(x5), .O(new_n84_));
  nand2  g067(.a(x6), .b(x2), .O(new_n85_));
  nand4  g068(.a(new_n85_), .b(new_n20_), .c(new_n19_), .d(x0), .O(new_n86_));
  nand3  g069(.a(new_n86_), .b(new_n84_), .c(new_n75_), .O(z3));
  nor2   g070(.a(x3), .b(x2), .O(new_n88_));
  oai21  g071(.a(new_n88_), .b(new_n18_), .c(new_n22_), .O(new_n89_));
  nand2  g072(.a(new_n89_), .b(x6), .O(new_n90_));
  aoi21  g073(.a(new_n22_), .b(new_n18_), .c(new_n88_), .O(new_n91_));
  oai21  g074(.a(new_n91_), .b(x6), .c(new_n90_), .O(new_n92_));
  nand3  g075(.a(new_n92_), .b(x5), .c(x1), .O(new_n93_));
  nor2   g076(.a(x3), .b(x0), .O(new_n94_));
  nor3   g077(.a(new_n94_), .b(x6), .c(new_n64_), .O(new_n95_));
  nor2   g078(.a(new_n94_), .b(new_n64_), .O(new_n96_));
  nor2   g079(.a(new_n96_), .b(new_n25_), .O(new_n97_));
  oai21  g080(.a(new_n97_), .b(new_n95_), .c(new_n19_), .O(new_n98_));
  nand2  g081(.a(new_n98_), .b(new_n93_), .O(z4));
  nor2   g082(.a(new_n20_), .b(new_n79_), .O(new_n100_));
  nand3  g083(.a(new_n100_), .b(new_n64_), .c(x1), .O(new_n101_));
  nand2  g084(.a(new_n101_), .b(new_n80_), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(new_n18_), .O(new_n103_));
  aoi21  g086(.a(x3), .b(x1), .c(x2), .O(new_n104_));
  oai21  g087(.a(new_n104_), .b(new_n58_), .c(x0), .O(new_n105_));
  nand2  g088(.a(new_n20_), .b(x1), .O(new_n106_));
  nand3  g089(.a(new_n106_), .b(new_n105_), .c(new_n103_), .O(z5));
  oai21  g090(.a(x5), .b(x4), .c(x3), .O(new_n108_));
  oai21  g091(.a(new_n108_), .b(new_n64_), .c(new_n100_), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(x1), .O(new_n110_));
  nor2   g093(.a(new_n79_), .b(x2), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(new_n19_), .O(new_n112_));
  nand2  g095(.a(new_n112_), .b(new_n110_), .O(z6));
  inv1   g096(.a(new_n80_), .O(new_n114_));
  oai21  g097(.a(new_n111_), .b(new_n114_), .c(new_n19_), .O(new_n115_));
  oai21  g098(.a(x1), .b(x0), .c(x3), .O(new_n116_));
  oai21  g099(.a(new_n116_), .b(x2), .c(new_n80_), .O(new_n117_));
  nand2  g100(.a(new_n117_), .b(x5), .O(new_n118_));
  nand2  g101(.a(new_n118_), .b(new_n115_), .O(z7));
  nand2  g102(.a(new_n106_), .b(x3), .O(z8));
  nand3  g103(.a(x4), .b(x3), .c(x2), .O(new_n121_));
  nand2  g104(.a(new_n121_), .b(x5), .O(new_n122_));
  nand2  g105(.a(new_n122_), .b(x1), .O(new_n123_));
  nand2  g106(.a(new_n25_), .b(new_n18_), .O(new_n124_));
  nand3  g107(.a(new_n124_), .b(x5), .c(x4), .O(new_n125_));
  nand2  g108(.a(new_n125_), .b(new_n123_), .O(z9));
endmodule


