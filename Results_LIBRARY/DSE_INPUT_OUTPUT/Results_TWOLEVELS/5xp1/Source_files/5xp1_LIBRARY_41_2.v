// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:37 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_, new_n100_,
    new_n101_, new_n102_, new_n103_, new_n104_, new_n106_, new_n107_,
    new_n108_, new_n110_, new_n112_, new_n113_, new_n114_;
  inv1   g00(.a(x5), .O(new_n18_));
  oai21  g01(.a(new_n18_), .b(x4), .c(x6), .O(new_n19_));
  nand2  g02(.a(new_n19_), .b(x0), .O(new_n20_));
  inv1   g03(.a(x4), .O(new_n21_));
  inv1   g04(.a(x1), .O(new_n22_));
  nand2  g05(.a(x3), .b(x2), .O(new_n23_));
  nand2  g06(.a(new_n23_), .b(new_n22_), .O(new_n24_));
  nand3  g07(.a(new_n24_), .b(x5), .c(new_n21_), .O(new_n25_));
  oai21  g08(.a(x5), .b(new_n21_), .c(new_n25_), .O(new_n26_));
  nand2  g09(.a(new_n26_), .b(x6), .O(new_n27_));
  inv1   g10(.a(x0), .O(new_n28_));
  inv1   g11(.a(x6), .O(new_n29_));
  nand4  g12(.a(x5), .b(x3), .c(x2), .d(x1), .O(new_n30_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(x4), .d(new_n28_), .O(new_n31_));
  nand3  g14(.a(new_n31_), .b(new_n27_), .c(new_n20_), .O(z0));
  inv1   g15(.a(x2), .O(new_n33_));
  nand2  g16(.a(new_n33_), .b(new_n22_), .O(new_n34_));
  nand2  g17(.a(new_n34_), .b(x0), .O(new_n35_));
  aoi21  g18(.a(new_n35_), .b(new_n21_), .c(x5), .O(new_n36_));
  nand4  g19(.a(new_n23_), .b(x5), .c(new_n21_), .d(new_n22_), .O(new_n37_));
  nor2   g20(.a(new_n37_), .b(x0), .O(new_n38_));
  oai21  g21(.a(new_n38_), .b(new_n36_), .c(x6), .O(new_n39_));
  nand4  g22(.a(x4), .b(x3), .c(x2), .d(x1), .O(new_n40_));
  nand4  g23(.a(new_n40_), .b(new_n29_), .c(x5), .d(new_n28_), .O(new_n41_));
  nand2  g24(.a(new_n41_), .b(new_n39_), .O(z1));
  nand2  g25(.a(x6), .b(x4), .O(new_n43_));
  nor2   g26(.a(x6), .b(x4), .O(new_n44_));
  nand4  g27(.a(new_n44_), .b(x3), .c(x2), .d(new_n28_), .O(new_n45_));
  aoi21  g28(.a(new_n45_), .b(new_n43_), .c(new_n22_), .O(new_n46_));
  nand4  g29(.a(new_n23_), .b(new_n21_), .c(new_n22_), .d(new_n28_), .O(new_n47_));
  nand2  g30(.a(new_n23_), .b(new_n28_), .O(new_n48_));
  nand2  g31(.a(new_n48_), .b(x4), .O(new_n49_));
  aoi21  g32(.a(new_n49_), .b(new_n47_), .c(new_n29_), .O(new_n50_));
  oai21  g33(.a(new_n50_), .b(new_n46_), .c(x5), .O(new_n51_));
  nand3  g34(.a(new_n34_), .b(x6), .c(x0), .O(new_n52_));
  nand3  g35(.a(new_n30_), .b(new_n29_), .c(new_n28_), .O(new_n53_));
  nand2  g36(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x4), .O(new_n55_));
  nand4  g38(.a(new_n35_), .b(x6), .c(new_n18_), .d(new_n21_), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n55_), .c(new_n51_), .O(z2));
  nand2  g40(.a(new_n29_), .b(x1), .O(new_n58_));
  oai21  g41(.a(new_n29_), .b(x0), .c(new_n58_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(new_n18_), .c(x3), .O(new_n60_));
  nor2   g43(.a(x3), .b(x1), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(x0), .c(x5), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n60_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x2), .O(new_n64_));
  nand3  g47(.a(x6), .b(new_n18_), .c(new_n28_), .O(new_n65_));
  oai21  g48(.a(new_n18_), .b(new_n28_), .c(new_n65_), .O(new_n66_));
  nand2  g49(.a(new_n66_), .b(x1), .O(new_n67_));
  nand3  g50(.a(new_n18_), .b(new_n33_), .c(new_n22_), .O(new_n68_));
  aoi21  g51(.a(new_n68_), .b(x6), .c(new_n28_), .O(new_n69_));
  oai21  g52(.a(new_n29_), .b(new_n33_), .c(new_n22_), .O(new_n70_));
  nand2  g53(.a(new_n23_), .b(new_n29_), .O(new_n71_));
  aoi21  g54(.a(new_n71_), .b(new_n70_), .c(new_n18_), .O(new_n72_));
  aoi21  g55(.a(new_n72_), .b(new_n28_), .c(new_n69_), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(new_n67_), .c(new_n64_), .O(z3));
  inv1   g57(.a(x3), .O(new_n75_));
  nand3  g58(.a(x6), .b(new_n22_), .c(new_n28_), .O(new_n76_));
  nand2  g59(.a(new_n76_), .b(new_n58_), .O(new_n77_));
  nand2  g60(.a(new_n77_), .b(new_n75_), .O(new_n78_));
  nand2  g61(.a(x6), .b(x3), .O(new_n79_));
  aoi21  g62(.a(new_n79_), .b(new_n28_), .c(new_n33_), .O(new_n80_));
  nor2   g63(.a(x6), .b(x2), .O(new_n81_));
  oai21  g64(.a(new_n81_), .b(new_n80_), .c(x1), .O(new_n82_));
  oai21  g65(.a(new_n23_), .b(x1), .c(new_n28_), .O(new_n83_));
  nand2  g66(.a(new_n83_), .b(new_n29_), .O(new_n84_));
  oai21  g67(.a(x3), .b(new_n22_), .c(x0), .O(new_n85_));
  oai21  g68(.a(new_n75_), .b(x1), .c(new_n85_), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(x6), .c(new_n33_), .O(new_n87_));
  nand4  g70(.a(new_n87_), .b(new_n84_), .c(new_n82_), .d(new_n78_), .O(z4));
  nand3  g71(.a(x3), .b(new_n33_), .c(x1), .O(new_n89_));
  oai21  g72(.a(x3), .b(new_n33_), .c(new_n89_), .O(new_n90_));
  nand2  g73(.a(new_n90_), .b(new_n28_), .O(new_n91_));
  oai21  g74(.a(new_n75_), .b(new_n22_), .c(new_n33_), .O(new_n92_));
  nand2  g75(.a(new_n92_), .b(new_n23_), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(x6), .c(x0), .O(new_n94_));
  nand2  g77(.a(new_n94_), .b(new_n91_), .O(z5));
  nand2  g78(.a(new_n29_), .b(x0), .O(new_n96_));
  nand2  g79(.a(new_n96_), .b(new_n33_), .O(new_n97_));
  nor2   g80(.a(new_n97_), .b(x1), .O(new_n98_));
  nand2  g81(.a(x6), .b(new_n21_), .O(new_n99_));
  nand3  g82(.a(new_n99_), .b(x5), .c(new_n28_), .O(new_n100_));
  nor2   g83(.a(x6), .b(new_n18_), .O(new_n101_));
  aoi21  g84(.a(new_n101_), .b(new_n100_), .c(new_n33_), .O(new_n102_));
  aoi21  g85(.a(new_n102_), .b(x1), .c(new_n98_), .O(new_n103_));
  aoi22  g86(.a(new_n29_), .b(x0), .c(new_n75_), .d(x1), .O(new_n104_));
  oai21  g87(.a(new_n103_), .b(new_n75_), .c(new_n104_), .O(z6));
  nand3  g88(.a(new_n96_), .b(new_n75_), .c(x2), .O(new_n106_));
  nand2  g89(.a(new_n29_), .b(x0), .O(new_n107_));
  nand3  g90(.a(new_n107_), .b(x3), .c(new_n33_), .O(new_n108_));
  nand2  g91(.a(new_n108_), .b(new_n106_), .O(z7));
  oai21  g92(.a(x2), .b(x0), .c(x6), .O(new_n110_));
  aoi21  g93(.a(new_n110_), .b(x0), .c(x3), .O(z8));
  nand2  g94(.a(x1), .b(new_n28_), .O(new_n112_));
  oai21  g95(.a(new_n112_), .b(new_n23_), .c(new_n29_), .O(new_n113_));
  nand3  g96(.a(new_n113_), .b(x5), .c(x4), .O(new_n114_));
  inv1   g97(.a(new_n114_), .O(z9));
endmodule


