// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:44 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n126_, new_n127_, new_n128_, new_n129_, new_n130_,
    new_n132_, new_n134_, new_n135_, new_n136_, new_n138_, new_n139_;
  inv1   g000(.a(x5), .O(new_n18_));
  nand3  g001(.a(new_n18_), .b(x4), .c(x1), .O(new_n19_));
  inv1   g002(.a(x4), .O(new_n20_));
  nand4  g003(.a(x6), .b(x5), .c(new_n20_), .d(x2), .O(new_n21_));
  nand2  g004(.a(new_n21_), .b(new_n19_), .O(new_n22_));
  nand2  g005(.a(new_n22_), .b(x3), .O(new_n23_));
  inv1   g006(.a(x2), .O(new_n24_));
  inv1   g007(.a(x6), .O(new_n25_));
  nand4  g008(.a(new_n25_), .b(x3), .c(new_n24_), .d(x1), .O(new_n26_));
  nand3  g009(.a(new_n26_), .b(new_n18_), .c(x0), .O(new_n27_));
  inv1   g010(.a(x0), .O(new_n28_));
  nand3  g011(.a(x3), .b(x2), .c(x1), .O(new_n29_));
  and2   g012(.a(new_n29_), .b(new_n25_), .O(new_n30_));
  nand3  g013(.a(new_n30_), .b(x5), .c(new_n28_), .O(new_n31_));
  nand2  g014(.a(new_n31_), .b(new_n27_), .O(new_n32_));
  nand2  g015(.a(new_n32_), .b(x4), .O(new_n33_));
  oai21  g016(.a(x1), .b(x0), .c(x6), .O(new_n34_));
  nor2   g017(.a(new_n34_), .b(new_n18_), .O(new_n35_));
  nor2   g018(.a(x5), .b(x0), .O(new_n36_));
  aoi21  g019(.a(new_n35_), .b(new_n20_), .c(new_n36_), .O(new_n37_));
  nand3  g020(.a(new_n37_), .b(new_n33_), .c(new_n23_), .O(z0));
  nand2  g021(.a(new_n24_), .b(new_n28_), .O(new_n39_));
  nand3  g022(.a(new_n20_), .b(x3), .c(x2), .O(new_n40_));
  nand2  g023(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g024(.a(new_n41_), .b(x1), .O(new_n42_));
  aoi21  g025(.a(x3), .b(x1), .c(x0), .O(new_n43_));
  aoi21  g026(.a(new_n20_), .b(x0), .c(new_n43_), .O(new_n44_));
  aoi21  g027(.a(new_n44_), .b(new_n42_), .c(x6), .O(new_n45_));
  inv1   g028(.a(x1), .O(new_n46_));
  nand2  g029(.a(x3), .b(x2), .O(new_n47_));
  nand4  g030(.a(new_n47_), .b(x6), .c(new_n20_), .d(new_n46_), .O(new_n48_));
  nor2   g031(.a(new_n48_), .b(x0), .O(new_n49_));
  oai21  g032(.a(new_n49_), .b(new_n45_), .c(x5), .O(new_n50_));
  nand2  g033(.a(x2), .b(x1), .O(new_n51_));
  nand3  g034(.a(x6), .b(new_n24_), .c(new_n46_), .O(new_n52_));
  aoi21  g035(.a(new_n52_), .b(new_n51_), .c(new_n28_), .O(new_n53_));
  inv1   g036(.a(x3), .O(new_n54_));
  nor2   g037(.a(new_n54_), .b(new_n46_), .O(new_n55_));
  oai21  g038(.a(new_n55_), .b(new_n53_), .c(x4), .O(new_n56_));
  nor2   g039(.a(x2), .b(x1), .O(new_n57_));
  nor2   g040(.a(new_n57_), .b(new_n25_), .O(new_n58_));
  nor2   g041(.a(new_n58_), .b(new_n28_), .O(new_n59_));
  nand2  g042(.a(new_n59_), .b(new_n56_), .O(new_n60_));
  nand2  g043(.a(new_n60_), .b(new_n18_), .O(new_n61_));
  nand2  g044(.a(new_n61_), .b(new_n50_), .O(z1));
  oai21  g045(.a(x3), .b(x2), .c(x1), .O(new_n63_));
  nand3  g046(.a(new_n63_), .b(new_n18_), .c(x0), .O(new_n64_));
  nand3  g047(.a(new_n29_), .b(x5), .c(new_n28_), .O(new_n65_));
  aoi21  g048(.a(new_n65_), .b(new_n64_), .c(new_n20_), .O(new_n66_));
  oai21  g049(.a(x3), .b(x2), .c(x0), .O(new_n67_));
  nand3  g050(.a(x5), .b(x3), .c(x2), .O(new_n68_));
  nand2  g051(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand2  g052(.a(new_n69_), .b(x1), .O(new_n70_));
  nand2  g053(.a(x5), .b(x0), .O(new_n71_));
  aoi21  g054(.a(new_n71_), .b(new_n70_), .c(x4), .O(new_n72_));
  oai21  g055(.a(new_n72_), .b(new_n66_), .c(new_n25_), .O(new_n73_));
  nand4  g056(.a(new_n47_), .b(new_n20_), .c(new_n46_), .d(new_n28_), .O(new_n74_));
  nand3  g057(.a(new_n47_), .b(new_n46_), .c(new_n28_), .O(new_n75_));
  nand2  g058(.a(new_n75_), .b(x4), .O(new_n76_));
  aoi21  g059(.a(new_n76_), .b(new_n74_), .c(new_n18_), .O(new_n77_));
  nand3  g060(.a(new_n20_), .b(new_n24_), .c(new_n46_), .O(new_n78_));
  oai21  g061(.a(new_n57_), .b(new_n20_), .c(new_n78_), .O(new_n79_));
  nand3  g062(.a(new_n79_), .b(new_n18_), .c(x0), .O(new_n80_));
  inv1   g063(.a(new_n80_), .O(new_n81_));
  oai21  g064(.a(new_n81_), .b(new_n77_), .c(x6), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(new_n73_), .O(z2));
  oai21  g066(.a(new_n25_), .b(new_n46_), .c(new_n54_), .O(new_n84_));
  nand2  g067(.a(x6), .b(x2), .O(new_n85_));
  nand3  g068(.a(new_n85_), .b(x3), .c(new_n46_), .O(new_n86_));
  nand3  g069(.a(new_n25_), .b(new_n24_), .c(x1), .O(new_n87_));
  nand3  g070(.a(new_n87_), .b(new_n86_), .c(new_n84_), .O(new_n88_));
  oai21  g071(.a(x6), .b(x1), .c(x2), .O(new_n89_));
  oai21  g072(.a(x6), .b(x3), .c(x1), .O(new_n90_));
  aoi21  g073(.a(new_n90_), .b(new_n89_), .c(new_n28_), .O(new_n91_));
  aoi21  g074(.a(new_n88_), .b(new_n28_), .c(new_n91_), .O(new_n92_));
  oai21  g075(.a(new_n25_), .b(new_n24_), .c(new_n46_), .O(new_n93_));
  nand3  g076(.a(new_n25_), .b(new_n54_), .c(new_n24_), .O(new_n94_));
  nand2  g077(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand3  g078(.a(new_n95_), .b(new_n18_), .c(x0), .O(new_n96_));
  oai21  g079(.a(new_n92_), .b(new_n18_), .c(new_n96_), .O(z3));
  xor2a  g080(.a(x6), .b(x2), .O(new_n98_));
  nand3  g081(.a(new_n98_), .b(new_n46_), .c(new_n28_), .O(new_n99_));
  nand3  g082(.a(x6), .b(x2), .c(x1), .O(new_n100_));
  aoi21  g083(.a(new_n100_), .b(new_n99_), .c(new_n54_), .O(new_n101_));
  nand3  g084(.a(new_n47_), .b(new_n25_), .c(x1), .O(new_n102_));
  nand3  g085(.a(x6), .b(new_n54_), .c(new_n46_), .O(new_n103_));
  aoi21  g086(.a(new_n103_), .b(new_n102_), .c(x0), .O(new_n104_));
  oai21  g087(.a(new_n104_), .b(new_n101_), .c(x5), .O(new_n105_));
  nand2  g088(.a(x6), .b(x2), .O(new_n106_));
  aoi21  g089(.a(new_n94_), .b(new_n106_), .c(new_n46_), .O(new_n107_));
  nand2  g090(.a(new_n98_), .b(new_n46_), .O(new_n108_));
  nand3  g091(.a(x6), .b(x3), .c(new_n24_), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(new_n108_), .O(new_n110_));
  oai21  g093(.a(new_n110_), .b(new_n107_), .c(x0), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(new_n105_), .O(z4));
  nand2  g095(.a(x3), .b(new_n24_), .O(new_n113_));
  nand2  g096(.a(new_n54_), .b(x2), .O(new_n114_));
  oai21  g097(.a(new_n113_), .b(new_n46_), .c(new_n114_), .O(new_n115_));
  nand3  g098(.a(new_n115_), .b(x5), .c(new_n28_), .O(new_n116_));
  inv1   g099(.a(new_n47_), .O(new_n117_));
  aoi21  g100(.a(x3), .b(x1), .c(x2), .O(new_n118_));
  oai21  g101(.a(new_n118_), .b(new_n117_), .c(x0), .O(new_n119_));
  nand2  g102(.a(new_n119_), .b(new_n116_), .O(z5));
  nand2  g103(.a(new_n57_), .b(new_n28_), .O(new_n121_));
  nand2  g104(.a(x2), .b(x1), .O(new_n122_));
  aoi21  g105(.a(new_n122_), .b(new_n121_), .c(new_n18_), .O(new_n123_));
  nand2  g106(.a(new_n57_), .b(x0), .O(new_n124_));
  inv1   g107(.a(new_n124_), .O(new_n125_));
  oai21  g108(.a(new_n125_), .b(new_n123_), .c(x3), .O(new_n126_));
  nand3  g109(.a(new_n25_), .b(x5), .c(x4), .O(new_n127_));
  nand3  g110(.a(new_n127_), .b(x2), .c(x0), .O(new_n128_));
  oai21  g111(.a(new_n36_), .b(x3), .c(new_n128_), .O(new_n129_));
  nand2  g112(.a(new_n129_), .b(x1), .O(new_n130_));
  nand2  g113(.a(new_n130_), .b(new_n126_), .O(z6));
  inv1   g114(.a(new_n36_), .O(new_n132_));
  nand3  g115(.a(new_n114_), .b(new_n113_), .c(new_n132_), .O(z7));
  aoi21  g116(.a(new_n46_), .b(x0), .c(new_n18_), .O(new_n134_));
  nand2  g117(.a(new_n24_), .b(new_n28_), .O(new_n135_));
  oai21  g118(.a(new_n135_), .b(new_n134_), .c(new_n54_), .O(new_n136_));
  nand2  g119(.a(new_n136_), .b(new_n132_), .O(z8));
  nand2  g120(.a(new_n30_), .b(new_n28_), .O(new_n138_));
  nand3  g121(.a(new_n138_), .b(x5), .c(x4), .O(new_n139_));
  inv1   g122(.a(new_n139_), .O(z9));
endmodule


