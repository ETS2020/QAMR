// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:36 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n116_, new_n117_, new_n118_,
    new_n119_, new_n120_, new_n121_, new_n123_, new_n126_, new_n127_;
  inv1   g000(.a(x5), .O(new_n18_));
  inv1   g001(.a(x0), .O(new_n19_));
  oai21  g002(.a(x3), .b(x2), .c(x1), .O(new_n20_));
  nor2   g003(.a(new_n20_), .b(new_n19_), .O(new_n21_));
  inv1   g004(.a(x6), .O(new_n22_));
  inv1   g005(.a(x2), .O(new_n23_));
  inv1   g006(.a(x3), .O(new_n24_));
  nand2  g007(.a(new_n24_), .b(new_n23_), .O(new_n25_));
  nand4  g008(.a(new_n25_), .b(new_n22_), .c(x1), .d(x0), .O(new_n26_));
  oai21  g009(.a(new_n26_), .b(new_n21_), .c(new_n18_), .O(new_n27_));
  nand3  g010(.a(x3), .b(x2), .c(x1), .O(new_n28_));
  nand3  g011(.a(new_n28_), .b(new_n22_), .c(new_n19_), .O(new_n29_));
  nand2  g012(.a(new_n29_), .b(new_n27_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(x4), .O(new_n31_));
  inv1   g014(.a(x1), .O(new_n32_));
  inv1   g015(.a(x4), .O(new_n33_));
  nand2  g016(.a(x3), .b(x2), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(new_n19_), .O(new_n35_));
  nand3  g018(.a(new_n35_), .b(x5), .c(new_n33_), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(new_n32_), .O(new_n37_));
  nand2  g020(.a(new_n37_), .b(x6), .O(new_n38_));
  nand2  g021(.a(new_n38_), .b(new_n31_), .O(z0));
  oai21  g022(.a(x4), .b(x1), .c(x6), .O(new_n40_));
  nor2   g023(.a(x6), .b(x1), .O(new_n41_));
  aoi21  g024(.a(new_n40_), .b(new_n34_), .c(new_n41_), .O(new_n42_));
  aoi21  g025(.a(new_n28_), .b(new_n19_), .c(x6), .O(new_n43_));
  nand2  g026(.a(new_n43_), .b(new_n33_), .O(new_n44_));
  oai21  g027(.a(new_n42_), .b(x0), .c(new_n44_), .O(new_n45_));
  nand2  g028(.a(new_n45_), .b(x5), .O(new_n46_));
  nand3  g029(.a(new_n25_), .b(x4), .c(x1), .O(new_n47_));
  nand2  g030(.a(x6), .b(x2), .O(new_n48_));
  aoi21  g031(.a(new_n48_), .b(new_n47_), .c(new_n19_), .O(new_n49_));
  nor2   g032(.a(new_n22_), .b(new_n33_), .O(new_n50_));
  oai21  g033(.a(new_n50_), .b(new_n49_), .c(new_n18_), .O(new_n51_));
  nand2  g034(.a(x6), .b(x1), .O(new_n52_));
  nand3  g035(.a(new_n52_), .b(new_n51_), .c(new_n46_), .O(z1));
  nand2  g036(.a(x5), .b(x0), .O(new_n54_));
  nand2  g037(.a(new_n54_), .b(new_n32_), .O(new_n55_));
  oai21  g038(.a(x5), .b(x3), .c(x0), .O(new_n56_));
  nand2  g039(.a(x5), .b(x3), .O(new_n57_));
  aoi22  g040(.a(new_n57_), .b(new_n19_), .c(new_n56_), .d(new_n23_), .O(new_n58_));
  aoi21  g041(.a(new_n58_), .b(new_n55_), .c(new_n33_), .O(new_n59_));
  oai21  g042(.a(x3), .b(x2), .c(x0), .O(new_n60_));
  nand3  g043(.a(x5), .b(x3), .c(x2), .O(new_n61_));
  nand2  g044(.a(new_n61_), .b(new_n60_), .O(new_n62_));
  nand2  g045(.a(new_n62_), .b(x1), .O(new_n63_));
  aoi21  g046(.a(new_n63_), .b(new_n54_), .c(x4), .O(new_n64_));
  oai21  g047(.a(new_n64_), .b(new_n59_), .c(new_n22_), .O(new_n65_));
  oai21  g048(.a(x5), .b(x2), .c(x0), .O(new_n66_));
  nand2  g049(.a(new_n66_), .b(new_n61_), .O(new_n67_));
  nand2  g050(.a(new_n67_), .b(x4), .O(new_n68_));
  aoi21  g051(.a(x5), .b(x0), .c(x2), .O(new_n69_));
  aoi21  g052(.a(x5), .b(x3), .c(x0), .O(new_n70_));
  oai21  g053(.a(new_n70_), .b(new_n69_), .c(new_n33_), .O(new_n71_));
  nand2  g054(.a(new_n71_), .b(new_n68_), .O(new_n72_));
  nand3  g055(.a(new_n72_), .b(x6), .c(new_n32_), .O(new_n73_));
  nand2  g056(.a(new_n73_), .b(new_n65_), .O(z2));
  nand2  g057(.a(x5), .b(new_n19_), .O(new_n75_));
  nand2  g058(.a(new_n18_), .b(x0), .O(new_n76_));
  nand2  g059(.a(new_n76_), .b(new_n75_), .O(new_n77_));
  nand3  g060(.a(new_n77_), .b(new_n22_), .c(new_n32_), .O(new_n78_));
  nand2  g061(.a(x5), .b(new_n24_), .O(new_n79_));
  nand3  g062(.a(new_n18_), .b(x3), .c(new_n19_), .O(new_n80_));
  aoi21  g063(.a(new_n80_), .b(new_n79_), .c(new_n41_), .O(new_n81_));
  nand3  g064(.a(x6), .b(x5), .c(x0), .O(new_n82_));
  inv1   g065(.a(new_n82_), .O(new_n83_));
  oai21  g066(.a(new_n83_), .b(new_n81_), .c(x2), .O(new_n84_));
  oai21  g067(.a(new_n57_), .b(new_n19_), .c(new_n22_), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(x1), .O(new_n86_));
  nand2  g069(.a(new_n22_), .b(x3), .O(new_n87_));
  nand3  g070(.a(new_n87_), .b(new_n18_), .c(x0), .O(new_n88_));
  nand2  g071(.a(new_n88_), .b(new_n75_), .O(new_n89_));
  nand2  g072(.a(new_n89_), .b(new_n23_), .O(new_n90_));
  nand4  g073(.a(new_n90_), .b(new_n86_), .c(new_n84_), .d(new_n78_), .O(z3));
  nand2  g074(.a(x2), .b(new_n32_), .O(new_n92_));
  nand3  g075(.a(new_n24_), .b(new_n23_), .c(x1), .O(new_n93_));
  aoi21  g076(.a(new_n93_), .b(new_n92_), .c(new_n19_), .O(new_n94_));
  nand3  g077(.a(new_n23_), .b(x1), .c(new_n19_), .O(new_n95_));
  aoi21  g078(.a(new_n95_), .b(new_n92_), .c(new_n24_), .O(new_n96_));
  oai21  g079(.a(new_n96_), .b(new_n94_), .c(new_n22_), .O(new_n97_));
  aoi21  g080(.a(new_n48_), .b(new_n32_), .c(x3), .O(new_n98_));
  aoi22  g081(.a(new_n98_), .b(new_n19_), .c(new_n92_), .d(x6), .O(new_n99_));
  nand2  g082(.a(new_n99_), .b(new_n97_), .O(z4));
  aoi21  g083(.a(new_n93_), .b(new_n34_), .c(new_n19_), .O(new_n101_));
  nor2   g084(.a(new_n24_), .b(x2), .O(new_n102_));
  nand2  g085(.a(new_n102_), .b(x1), .O(new_n103_));
  nand2  g086(.a(new_n24_), .b(x2), .O(new_n104_));
  aoi21  g087(.a(new_n104_), .b(new_n103_), .c(x0), .O(new_n105_));
  oai21  g088(.a(new_n105_), .b(new_n101_), .c(new_n22_), .O(new_n106_));
  nand2  g089(.a(new_n104_), .b(x0), .O(new_n107_));
  nand4  g090(.a(x6), .b(new_n24_), .c(x2), .d(new_n19_), .O(new_n108_));
  nand2  g091(.a(new_n108_), .b(new_n107_), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(new_n32_), .O(new_n110_));
  nand2  g093(.a(new_n110_), .b(new_n106_), .O(z5));
  nor2   g094(.a(x5), .b(new_n33_), .O(new_n112_));
  nor2   g095(.a(x6), .b(x4), .O(new_n113_));
  oai21  g096(.a(new_n113_), .b(new_n112_), .c(x0), .O(new_n114_));
  nor2   g097(.a(x6), .b(new_n18_), .O(new_n115_));
  nor2   g098(.a(x5), .b(x0), .O(new_n116_));
  oai21  g099(.a(new_n116_), .b(new_n115_), .c(x3), .O(new_n117_));
  aoi21  g100(.a(new_n117_), .b(new_n114_), .c(new_n23_), .O(new_n118_));
  nand2  g101(.a(new_n22_), .b(x3), .O(new_n119_));
  oai21  g102(.a(new_n119_), .b(new_n118_), .c(x1), .O(new_n120_));
  nand2  g103(.a(new_n102_), .b(new_n32_), .O(new_n121_));
  nand2  g104(.a(new_n121_), .b(new_n120_), .O(z6));
  inv1   g105(.a(new_n102_), .O(new_n123_));
  nand3  g106(.a(new_n104_), .b(new_n123_), .c(new_n52_), .O(z7));
  aoi21  g107(.a(x6), .b(x1), .c(x3), .O(z8));
  nor2   g108(.a(new_n22_), .b(x1), .O(new_n126_));
  oai21  g109(.a(new_n126_), .b(new_n43_), .c(x5), .O(new_n127_));
  nor2   g110(.a(new_n127_), .b(new_n33_), .O(z9));
endmodule


