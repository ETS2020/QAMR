// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:14 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n101_, new_n102_, new_n103_, new_n104_, new_n106_,
    new_n107_, new_n110_, new_n111_, new_n112_, new_n113_, new_n115_,
    new_n116_, new_n117_;
  inv1   g000(.a(x4), .O(new_n18_));
  inv1   g001(.a(x0), .O(new_n19_));
  inv1   g002(.a(x6), .O(new_n20_));
  nand2  g003(.a(x3), .b(x2), .O(new_n21_));
  nand3  g004(.a(new_n21_), .b(new_n20_), .c(new_n19_), .O(new_n22_));
  aoi21  g005(.a(new_n22_), .b(x5), .c(new_n18_), .O(new_n23_));
  inv1   g006(.a(x5), .O(new_n24_));
  nor3   g007(.a(new_n20_), .b(new_n24_), .c(x4), .O(new_n25_));
  oai21  g008(.a(new_n25_), .b(new_n23_), .c(x1), .O(new_n26_));
  nand2  g009(.a(new_n21_), .b(new_n19_), .O(new_n27_));
  nand3  g010(.a(new_n27_), .b(x5), .c(new_n18_), .O(new_n28_));
  oai21  g011(.a(x5), .b(new_n18_), .c(new_n28_), .O(new_n29_));
  nand2  g012(.a(new_n29_), .b(x6), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(new_n26_), .O(z0));
  oai21  g014(.a(x4), .b(x1), .c(x6), .O(new_n32_));
  nand3  g015(.a(new_n32_), .b(new_n21_), .c(new_n19_), .O(new_n33_));
  nand3  g016(.a(new_n27_), .b(new_n20_), .c(new_n18_), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand2  g018(.a(new_n35_), .b(x5), .O(new_n36_));
  nand2  g019(.a(new_n20_), .b(new_n18_), .O(new_n37_));
  nand2  g020(.a(new_n37_), .b(x2), .O(new_n38_));
  aoi22  g021(.a(x6), .b(x1), .c(x4), .d(x3), .O(new_n39_));
  aoi21  g022(.a(new_n39_), .b(new_n38_), .c(new_n19_), .O(new_n40_));
  nand2  g023(.a(x6), .b(x4), .O(new_n41_));
  inv1   g024(.a(new_n41_), .O(new_n42_));
  oai21  g025(.a(new_n42_), .b(new_n40_), .c(new_n24_), .O(new_n43_));
  inv1   g026(.a(x1), .O(new_n44_));
  nand2  g027(.a(new_n20_), .b(new_n44_), .O(new_n45_));
  nand3  g028(.a(new_n45_), .b(new_n43_), .c(new_n36_), .O(z1));
  nand2  g029(.a(new_n41_), .b(new_n37_), .O(new_n47_));
  oai21  g030(.a(x5), .b(x2), .c(x0), .O(new_n48_));
  nand3  g031(.a(x5), .b(x3), .c(x2), .O(new_n49_));
  nand2  g032(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand2  g033(.a(new_n50_), .b(new_n47_), .O(new_n51_));
  inv1   g034(.a(x3), .O(new_n52_));
  oai22  g035(.a(new_n41_), .b(new_n44_), .c(new_n37_), .d(new_n52_), .O(new_n53_));
  nand2  g036(.a(new_n53_), .b(x0), .O(new_n54_));
  nand3  g037(.a(x5), .b(x4), .c(x1), .O(new_n55_));
  inv1   g038(.a(new_n55_), .O(new_n56_));
  nor3   g039(.a(x5), .b(x4), .c(x0), .O(new_n57_));
  oai21  g040(.a(new_n57_), .b(new_n56_), .c(x6), .O(new_n58_));
  nand3  g041(.a(x5), .b(new_n18_), .c(new_n44_), .O(new_n59_));
  nand3  g042(.a(new_n20_), .b(x4), .c(x1), .O(new_n60_));
  nand2  g043(.a(new_n60_), .b(new_n59_), .O(new_n61_));
  nand2  g044(.a(new_n61_), .b(new_n21_), .O(new_n62_));
  nand3  g045(.a(new_n20_), .b(new_n24_), .c(x4), .O(new_n63_));
  nand2  g046(.a(new_n63_), .b(new_n62_), .O(new_n64_));
  inv1   g047(.a(x2), .O(new_n65_));
  nor2   g048(.a(x5), .b(x4), .O(new_n66_));
  aoi21  g049(.a(new_n66_), .b(new_n65_), .c(new_n20_), .O(new_n67_));
  nand2  g050(.a(new_n52_), .b(new_n65_), .O(new_n68_));
  oai22  g051(.a(new_n68_), .b(new_n63_), .c(new_n67_), .d(x1), .O(new_n69_));
  aoi21  g052(.a(new_n64_), .b(new_n19_), .c(new_n69_), .O(new_n70_));
  nand4  g053(.a(new_n70_), .b(new_n58_), .c(new_n54_), .d(new_n51_), .O(z2));
  nand3  g054(.a(x5), .b(x1), .c(x0), .O(new_n72_));
  nand3  g055(.a(new_n24_), .b(x2), .c(new_n19_), .O(new_n73_));
  nand2  g056(.a(new_n73_), .b(new_n72_), .O(new_n74_));
  nand2  g057(.a(new_n74_), .b(x3), .O(new_n75_));
  xnor2a g058(.a(x5), .b(x0), .O(new_n76_));
  nand3  g059(.a(new_n76_), .b(x6), .c(x1), .O(new_n77_));
  nand2  g060(.a(new_n65_), .b(new_n19_), .O(new_n78_));
  nand2  g061(.a(new_n52_), .b(x2), .O(new_n79_));
  aoi21  g062(.a(new_n79_), .b(new_n78_), .c(x1), .O(new_n80_));
  oai21  g063(.a(new_n65_), .b(new_n19_), .c(new_n22_), .O(new_n81_));
  oai21  g064(.a(new_n81_), .b(new_n80_), .c(x5), .O(new_n82_));
  oai21  g065(.a(x6), .b(x3), .c(x1), .O(new_n83_));
  nand4  g066(.a(new_n83_), .b(new_n24_), .c(new_n65_), .d(x0), .O(new_n84_));
  and2   g067(.a(new_n84_), .b(new_n45_), .O(new_n85_));
  nand4  g068(.a(new_n85_), .b(new_n82_), .c(new_n77_), .d(new_n75_), .O(z3));
  nand2  g069(.a(x2), .b(x1), .O(new_n87_));
  nand2  g070(.a(x3), .b(new_n65_), .O(new_n88_));
  aoi21  g071(.a(new_n88_), .b(new_n87_), .c(new_n19_), .O(new_n89_));
  nand2  g072(.a(new_n65_), .b(new_n44_), .O(new_n90_));
  aoi21  g073(.a(new_n90_), .b(new_n87_), .c(new_n52_), .O(new_n91_));
  oai21  g074(.a(new_n91_), .b(new_n89_), .c(x6), .O(new_n92_));
  aoi21  g075(.a(x6), .b(x1), .c(x3), .O(new_n93_));
  nor2   g076(.a(x6), .b(x2), .O(new_n94_));
  oai21  g077(.a(new_n94_), .b(new_n93_), .c(new_n19_), .O(new_n95_));
  nand2  g078(.a(new_n65_), .b(x0), .O(new_n96_));
  nand2  g079(.a(new_n96_), .b(x6), .O(new_n97_));
  nand2  g080(.a(new_n97_), .b(new_n44_), .O(new_n98_));
  nand3  g081(.a(new_n20_), .b(new_n52_), .c(new_n65_), .O(new_n99_));
  nand4  g082(.a(new_n99_), .b(new_n98_), .c(new_n95_), .d(new_n92_), .O(z4));
  oai21  g083(.a(new_n88_), .b(new_n44_), .c(new_n79_), .O(new_n101_));
  nand2  g084(.a(new_n101_), .b(new_n19_), .O(new_n102_));
  nand2  g085(.a(new_n68_), .b(new_n21_), .O(new_n103_));
  nand2  g086(.a(new_n103_), .b(x0), .O(new_n104_));
  nand3  g087(.a(new_n104_), .b(new_n102_), .c(new_n98_), .O(z5));
  nand2  g088(.a(new_n88_), .b(x1), .O(new_n106_));
  nand2  g089(.a(x6), .b(x3), .O(new_n107_));
  oai21  g090(.a(new_n107_), .b(new_n90_), .c(new_n106_), .O(z6));
  aoi22  g091(.a(new_n88_), .b(new_n79_), .c(new_n20_), .d(new_n44_), .O(z7));
  aoi21  g092(.a(new_n52_), .b(new_n19_), .c(new_n20_), .O(new_n110_));
  nand2  g093(.a(x6), .b(x2), .O(new_n111_));
  nand3  g094(.a(new_n111_), .b(new_n96_), .c(new_n44_), .O(new_n112_));
  nand2  g095(.a(new_n112_), .b(new_n52_), .O(new_n113_));
  oai21  g096(.a(new_n110_), .b(x1), .c(new_n113_), .O(z8));
  nand2  g097(.a(new_n27_), .b(x1), .O(new_n115_));
  nand2  g098(.a(new_n115_), .b(new_n20_), .O(new_n116_));
  nand3  g099(.a(new_n116_), .b(x5), .c(x4), .O(new_n117_));
  inv1   g100(.a(new_n117_), .O(z9));
endmodule


