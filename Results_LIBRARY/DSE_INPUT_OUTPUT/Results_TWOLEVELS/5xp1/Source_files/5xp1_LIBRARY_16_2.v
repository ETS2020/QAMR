// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:32 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n104_, new_n105_, new_n107_,
    new_n108_, new_n109_, new_n110_, new_n111_, new_n114_, new_n115_;
  inv1   g00(.a(x2), .O(new_n18_));
  nand2  g01(.a(new_n18_), .b(x1), .O(new_n19_));
  inv1   g02(.a(x5), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(x4), .c(x3), .O(new_n21_));
  inv1   g04(.a(x4), .O(new_n22_));
  nand3  g05(.a(x6), .b(x5), .c(new_n22_), .O(new_n23_));
  oai21  g06(.a(new_n21_), .b(new_n19_), .c(new_n23_), .O(new_n24_));
  nand2  g07(.a(new_n24_), .b(x0), .O(new_n25_));
  inv1   g08(.a(x1), .O(new_n26_));
  nand2  g09(.a(x3), .b(x2), .O(new_n27_));
  nand2  g10(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand3  g11(.a(new_n28_), .b(x5), .c(new_n22_), .O(new_n29_));
  oai21  g12(.a(x5), .b(new_n22_), .c(new_n29_), .O(new_n30_));
  nand2  g13(.a(new_n30_), .b(x6), .O(new_n31_));
  inv1   g14(.a(x3), .O(new_n32_));
  oai21  g15(.a(new_n32_), .b(new_n26_), .c(new_n20_), .O(new_n33_));
  and2   g16(.a(new_n33_), .b(x0), .O(new_n34_));
  nor2   g17(.a(new_n34_), .b(x6), .O(new_n35_));
  nand3  g18(.a(new_n35_), .b(x4), .c(new_n18_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(new_n31_), .c(new_n25_), .O(z0));
  inv1   g20(.a(x6), .O(new_n38_));
  nand2  g21(.a(x4), .b(x3), .O(new_n39_));
  oai21  g22(.a(new_n39_), .b(x2), .c(new_n38_), .O(new_n40_));
  aoi21  g23(.a(new_n40_), .b(x1), .c(x2), .O(new_n41_));
  nand4  g24(.a(new_n38_), .b(x5), .c(new_n22_), .d(new_n18_), .O(new_n42_));
  oai21  g25(.a(new_n41_), .b(x5), .c(new_n42_), .O(new_n43_));
  nand2  g26(.a(new_n43_), .b(x0), .O(new_n44_));
  inv1   g27(.a(x0), .O(new_n45_));
  nand4  g28(.a(new_n27_), .b(x6), .c(new_n22_), .d(new_n26_), .O(new_n46_));
  nand2  g29(.a(new_n46_), .b(x6), .O(new_n47_));
  nand3  g30(.a(new_n47_), .b(x5), .c(new_n45_), .O(new_n48_));
  nand3  g31(.a(x6), .b(new_n20_), .c(x4), .O(new_n49_));
  nor2   g32(.a(x6), .b(new_n18_), .O(new_n50_));
  inv1   g33(.a(new_n50_), .O(new_n51_));
  nand4  g34(.a(new_n51_), .b(new_n49_), .c(new_n48_), .d(new_n44_), .O(z1));
  oai21  g35(.a(x5), .b(x0), .c(x1), .O(new_n53_));
  nand2  g36(.a(new_n20_), .b(new_n18_), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(x0), .O(new_n55_));
  nand3  g38(.a(x5), .b(x3), .c(x2), .O(new_n56_));
  nand3  g39(.a(new_n56_), .b(new_n55_), .c(new_n53_), .O(new_n57_));
  nand2  g40(.a(new_n57_), .b(x4), .O(new_n58_));
  nor2   g41(.a(x5), .b(x0), .O(new_n59_));
  nand3  g42(.a(new_n27_), .b(x5), .c(new_n45_), .O(new_n60_));
  aoi21  g43(.a(new_n60_), .b(new_n54_), .c(x1), .O(new_n61_));
  oai21  g44(.a(new_n61_), .b(new_n59_), .c(new_n22_), .O(new_n62_));
  nand2  g45(.a(new_n62_), .b(new_n58_), .O(new_n63_));
  nand2  g46(.a(new_n63_), .b(x6), .O(new_n64_));
  nand3  g47(.a(new_n33_), .b(new_n22_), .c(x0), .O(new_n65_));
  oai21  g48(.a(new_n34_), .b(new_n22_), .c(new_n65_), .O(new_n66_));
  nand3  g49(.a(new_n66_), .b(new_n38_), .c(new_n18_), .O(new_n67_));
  nand2  g50(.a(new_n67_), .b(new_n64_), .O(z2));
  nand4  g51(.a(x5), .b(new_n18_), .c(x1), .d(x0), .O(new_n69_));
  nand3  g52(.a(new_n20_), .b(x2), .c(new_n45_), .O(new_n70_));
  nand2  g53(.a(new_n70_), .b(new_n69_), .O(new_n71_));
  nand2  g54(.a(new_n71_), .b(x3), .O(new_n72_));
  xnor2a g55(.a(x5), .b(x0), .O(new_n73_));
  nand3  g56(.a(new_n73_), .b(x6), .c(x1), .O(new_n74_));
  aoi21  g57(.a(new_n18_), .b(new_n26_), .c(new_n38_), .O(new_n75_));
  nor2   g58(.a(x3), .b(x1), .O(new_n76_));
  oai21  g59(.a(new_n76_), .b(x0), .c(x2), .O(new_n77_));
  oai21  g60(.a(new_n75_), .b(x0), .c(new_n77_), .O(new_n78_));
  nand2  g61(.a(new_n78_), .b(x5), .O(new_n79_));
  oai22  g62(.a(x6), .b(x3), .c(x2), .d(x1), .O(new_n80_));
  nand3  g63(.a(new_n80_), .b(new_n20_), .c(x0), .O(new_n81_));
  and2   g64(.a(new_n81_), .b(new_n51_), .O(new_n82_));
  nand4  g65(.a(new_n82_), .b(new_n79_), .c(new_n74_), .d(new_n72_), .O(z3));
  nand3  g66(.a(x6), .b(x3), .c(new_n18_), .O(new_n84_));
  oai21  g67(.a(new_n18_), .b(new_n26_), .c(new_n84_), .O(new_n85_));
  nand2  g68(.a(new_n85_), .b(x0), .O(new_n86_));
  nand2  g69(.a(x6), .b(new_n32_), .O(new_n87_));
  nand2  g70(.a(new_n38_), .b(x1), .O(new_n88_));
  oai21  g71(.a(new_n87_), .b(x1), .c(new_n88_), .O(new_n89_));
  nand2  g72(.a(new_n89_), .b(new_n45_), .O(new_n90_));
  oai21  g73(.a(x6), .b(x3), .c(new_n27_), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(x1), .O(new_n92_));
  nor2   g75(.a(new_n38_), .b(x2), .O(new_n93_));
  aoi21  g76(.a(new_n93_), .b(new_n26_), .c(new_n50_), .O(new_n94_));
  nand4  g77(.a(new_n94_), .b(new_n92_), .c(new_n90_), .d(new_n86_), .O(z4));
  nand3  g78(.a(x3), .b(new_n18_), .c(x1), .O(new_n96_));
  oai21  g79(.a(new_n87_), .b(new_n18_), .c(new_n96_), .O(new_n97_));
  nand2  g80(.a(new_n97_), .b(new_n45_), .O(new_n98_));
  aoi21  g81(.a(x3), .b(x1), .c(x2), .O(new_n99_));
  nand3  g82(.a(x6), .b(x3), .c(x2), .O(new_n100_));
  inv1   g83(.a(new_n100_), .O(new_n101_));
  oai21  g84(.a(new_n101_), .b(new_n99_), .c(x0), .O(new_n102_));
  nand2  g85(.a(new_n102_), .b(new_n98_), .O(z5));
  oai21  g86(.a(new_n32_), .b(x2), .c(x1), .O(new_n104_));
  nand3  g87(.a(x3), .b(new_n18_), .c(new_n26_), .O(new_n105_));
  nand3  g88(.a(new_n105_), .b(new_n104_), .c(new_n51_), .O(z6));
  oai21  g89(.a(new_n38_), .b(new_n32_), .c(x2), .O(new_n107_));
  nand3  g90(.a(x6), .b(new_n20_), .c(new_n22_), .O(new_n108_));
  inv1   g91(.a(new_n108_), .O(new_n109_));
  nand4  g92(.a(new_n109_), .b(new_n38_), .c(x1), .d(x0), .O(new_n110_));
  nand3  g93(.a(new_n110_), .b(x3), .c(new_n18_), .O(new_n111_));
  nand2  g94(.a(new_n111_), .b(new_n107_), .O(z7));
  aoi21  g95(.a(new_n38_), .b(x2), .c(x3), .O(z8));
  oai21  g96(.a(x2), .b(new_n45_), .c(new_n38_), .O(new_n114_));
  nand3  g97(.a(new_n114_), .b(x5), .c(x4), .O(new_n115_));
  inv1   g98(.a(new_n115_), .O(z9));
endmodule


