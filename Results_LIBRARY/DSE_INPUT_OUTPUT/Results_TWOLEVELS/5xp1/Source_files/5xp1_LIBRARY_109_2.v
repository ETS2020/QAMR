// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:49 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n30_, new_n31_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n39_, new_n40_,
    new_n41_, new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_, new_n69_,
    new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n88_, new_n89_, new_n91_, new_n92_, new_n93_, new_n94_,
    new_n97_, new_n98_;
  inv1   g00(.a(x6), .O(new_n18_));
  inv1   g01(.a(x1), .O(new_n19_));
  nand2  g02(.a(x5), .b(x0), .O(new_n20_));
  nand3  g03(.a(new_n20_), .b(new_n18_), .c(new_n19_), .O(new_n21_));
  oai21  g04(.a(new_n18_), .b(x5), .c(new_n21_), .O(new_n22_));
  nand2  g05(.a(new_n22_), .b(x4), .O(new_n23_));
  inv1   g06(.a(x4), .O(new_n24_));
  inv1   g07(.a(x0), .O(new_n25_));
  nand2  g08(.a(x3), .b(x2), .O(new_n26_));
  nand3  g09(.a(new_n26_), .b(new_n19_), .c(new_n25_), .O(new_n27_));
  nand4  g10(.a(new_n27_), .b(x6), .c(x5), .d(new_n24_), .O(new_n28_));
  nand2  g11(.a(new_n28_), .b(new_n23_), .O(z0));
  aoi21  g12(.a(new_n26_), .b(new_n24_), .c(new_n18_), .O(new_n30_));
  nand3  g13(.a(new_n18_), .b(new_n24_), .c(x0), .O(new_n31_));
  oai21  g14(.a(new_n30_), .b(x0), .c(new_n31_), .O(new_n32_));
  nand3  g15(.a(new_n32_), .b(x5), .c(new_n19_), .O(new_n33_));
  inv1   g16(.a(x5), .O(new_n34_));
  oai21  g17(.a(x2), .b(x1), .c(x0), .O(new_n35_));
  nand2  g18(.a(new_n35_), .b(new_n24_), .O(new_n36_));
  nand3  g19(.a(new_n36_), .b(x6), .c(new_n34_), .O(new_n37_));
  nand2  g20(.a(new_n37_), .b(new_n33_), .O(z1));
  nand2  g21(.a(new_n34_), .b(new_n25_), .O(new_n39_));
  nand2  g22(.a(new_n39_), .b(x1), .O(new_n40_));
  inv1   g23(.a(x2), .O(new_n41_));
  nand2  g24(.a(new_n34_), .b(new_n41_), .O(new_n42_));
  nand2  g25(.a(new_n42_), .b(x0), .O(new_n43_));
  nand3  g26(.a(x5), .b(x3), .c(x2), .O(new_n44_));
  nand3  g27(.a(new_n44_), .b(new_n43_), .c(new_n40_), .O(new_n45_));
  nand2  g28(.a(new_n45_), .b(x4), .O(new_n46_));
  inv1   g29(.a(new_n39_), .O(new_n47_));
  nand3  g30(.a(new_n26_), .b(x5), .c(new_n25_), .O(new_n48_));
  aoi21  g31(.a(new_n48_), .b(new_n42_), .c(x1), .O(new_n49_));
  oai21  g32(.a(new_n49_), .b(new_n47_), .c(new_n24_), .O(new_n50_));
  nand2  g33(.a(new_n50_), .b(new_n46_), .O(new_n51_));
  nand2  g34(.a(new_n51_), .b(x6), .O(new_n52_));
  nand2  g35(.a(new_n20_), .b(x4), .O(new_n53_));
  nand3  g36(.a(x5), .b(new_n24_), .c(x0), .O(new_n54_));
  nand2  g37(.a(new_n54_), .b(new_n53_), .O(new_n55_));
  nand3  g38(.a(new_n55_), .b(new_n18_), .c(new_n19_), .O(new_n56_));
  nand2  g39(.a(new_n56_), .b(new_n52_), .O(z2));
  nand3  g40(.a(new_n34_), .b(x3), .c(new_n25_), .O(new_n58_));
  nand2  g41(.a(new_n58_), .b(new_n20_), .O(new_n59_));
  nand3  g42(.a(new_n59_), .b(x6), .c(x2), .O(new_n60_));
  xnor2a g43(.a(x5), .b(x0), .O(new_n61_));
  nand2  g44(.a(x6), .b(x3), .O(new_n62_));
  nand3  g45(.a(new_n62_), .b(x5), .c(new_n25_), .O(new_n63_));
  oai21  g46(.a(new_n61_), .b(x2), .c(new_n63_), .O(new_n64_));
  nand2  g47(.a(new_n64_), .b(new_n19_), .O(new_n65_));
  nand2  g48(.a(x5), .b(x1), .O(new_n66_));
  oai21  g49(.a(x6), .b(x5), .c(new_n66_), .O(new_n67_));
  nand2  g50(.a(new_n39_), .b(x6), .O(new_n68_));
  aoi22  g51(.a(new_n68_), .b(x1), .c(new_n67_), .d(x0), .O(new_n69_));
  nand3  g52(.a(new_n69_), .b(new_n65_), .c(new_n60_), .O(z3));
  oai21  g53(.a(x3), .b(x0), .c(x2), .O(new_n71_));
  inv1   g54(.a(x3), .O(new_n72_));
  nor2   g55(.a(new_n72_), .b(x2), .O(new_n73_));
  aoi22  g56(.a(new_n73_), .b(x0), .c(new_n71_), .d(new_n19_), .O(new_n74_));
  aoi22  g57(.a(x6), .b(new_n19_), .c(new_n72_), .d(new_n25_), .O(new_n75_));
  nor2   g58(.a(x6), .b(new_n19_), .O(new_n76_));
  aoi21  g59(.a(new_n75_), .b(x2), .c(new_n76_), .O(new_n77_));
  oai21  g60(.a(new_n74_), .b(new_n18_), .c(new_n77_), .O(z4));
  inv1   g61(.a(new_n76_), .O(new_n79_));
  inv1   g62(.a(new_n73_), .O(new_n80_));
  nand2  g63(.a(new_n72_), .b(x2), .O(new_n81_));
  oai21  g64(.a(new_n80_), .b(new_n19_), .c(new_n81_), .O(new_n82_));
  nand2  g65(.a(new_n82_), .b(new_n25_), .O(new_n83_));
  aoi21  g66(.a(new_n41_), .b(x1), .c(new_n72_), .O(new_n84_));
  nor2   g67(.a(x3), .b(x2), .O(new_n85_));
  oai21  g68(.a(new_n85_), .b(new_n84_), .c(x0), .O(new_n86_));
  nand3  g69(.a(new_n86_), .b(new_n83_), .c(new_n79_), .O(z5));
  xor2a  g70(.a(x2), .b(x1), .O(new_n88_));
  oai21  g71(.a(new_n18_), .b(new_n72_), .c(x1), .O(new_n89_));
  oai21  g72(.a(new_n88_), .b(new_n72_), .c(new_n89_), .O(z6));
  oai21  g73(.a(new_n80_), .b(x0), .c(x6), .O(new_n91_));
  nand2  g74(.a(new_n91_), .b(x1), .O(new_n92_));
  oai21  g75(.a(new_n18_), .b(new_n25_), .c(x1), .O(new_n93_));
  nand3  g76(.a(new_n93_), .b(x3), .c(new_n41_), .O(new_n94_));
  nand3  g77(.a(new_n94_), .b(new_n92_), .c(new_n81_), .O(z7));
  aoi21  g78(.a(new_n18_), .b(x1), .c(x3), .O(z8));
  oai21  g79(.a(x1), .b(new_n25_), .c(new_n18_), .O(new_n97_));
  nand3  g80(.a(new_n97_), .b(x5), .c(x4), .O(new_n98_));
  inv1   g81(.a(new_n98_), .O(z9));
endmodule


