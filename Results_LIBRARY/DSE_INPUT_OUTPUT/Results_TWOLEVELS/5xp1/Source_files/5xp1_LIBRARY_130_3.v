// Benchmark "FAU" written by ABC on Tue Aug 18 15:53:53 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n32_,
    new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_, new_n39_,
    new_n40_, new_n41_, new_n42_, new_n43_, new_n44_, new_n46_, new_n47_,
    new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_, new_n54_,
    new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n103_, new_n104_, new_n105_, new_n106_,
    new_n107_, new_n108_, new_n109_, new_n110_, new_n111_, new_n112_,
    new_n113_, new_n114_, new_n115_, new_n117_, new_n118_, new_n121_,
    new_n122_, new_n123_, new_n124_, new_n125_;
  inv1   g000(.a(x0), .O(new_n18_));
  inv1   g001(.a(x1), .O(new_n19_));
  nand2  g002(.a(x3), .b(x2), .O(new_n20_));
  oai21  g003(.a(new_n20_), .b(new_n19_), .c(new_n18_), .O(new_n21_));
  aoi21  g004(.a(new_n21_), .b(x5), .c(x6), .O(new_n22_));
  inv1   g005(.a(x5), .O(new_n23_));
  nand3  g006(.a(x6), .b(new_n23_), .c(new_n19_), .O(new_n24_));
  inv1   g007(.a(new_n24_), .O(new_n25_));
  oai21  g008(.a(new_n25_), .b(new_n22_), .c(x4), .O(new_n26_));
  inv1   g009(.a(x4), .O(new_n27_));
  inv1   g010(.a(x6), .O(new_n28_));
  aoi21  g011(.a(new_n20_), .b(new_n18_), .c(new_n28_), .O(new_n29_));
  nand4  g012(.a(new_n29_), .b(x5), .c(new_n27_), .d(new_n19_), .O(new_n30_));
  nand2  g013(.a(new_n30_), .b(new_n26_), .O(z0));
  oai21  g014(.a(x4), .b(x1), .c(x6), .O(new_n32_));
  aoi22  g015(.a(new_n32_), .b(new_n20_), .c(new_n28_), .d(new_n19_), .O(new_n33_));
  nand3  g016(.a(new_n21_), .b(new_n28_), .c(new_n27_), .O(new_n34_));
  oai21  g017(.a(new_n33_), .b(x0), .c(new_n34_), .O(new_n35_));
  nand2  g018(.a(new_n35_), .b(x5), .O(new_n36_));
  nor2   g019(.a(x3), .b(x2), .O(new_n37_));
  inv1   g020(.a(new_n37_), .O(new_n38_));
  nand4  g021(.a(new_n38_), .b(new_n28_), .c(x4), .d(x1), .O(new_n39_));
  nand2  g022(.a(x6), .b(x2), .O(new_n40_));
  aoi21  g023(.a(new_n40_), .b(new_n39_), .c(new_n18_), .O(new_n41_));
  nor3   g024(.a(new_n28_), .b(new_n27_), .c(x1), .O(new_n42_));
  oai21  g025(.a(new_n42_), .b(new_n41_), .c(new_n23_), .O(new_n43_));
  nand2  g026(.a(x6), .b(x1), .O(new_n44_));
  nand3  g027(.a(new_n44_), .b(new_n43_), .c(new_n36_), .O(z1));
  nand2  g028(.a(x5), .b(x0), .O(new_n46_));
  nand2  g029(.a(new_n46_), .b(new_n19_), .O(new_n47_));
  inv1   g030(.a(x2), .O(new_n48_));
  oai21  g031(.a(x5), .b(x3), .c(x0), .O(new_n49_));
  nand2  g032(.a(x5), .b(x3), .O(new_n50_));
  aoi22  g033(.a(new_n50_), .b(new_n18_), .c(new_n49_), .d(new_n48_), .O(new_n51_));
  aoi21  g034(.a(new_n51_), .b(new_n47_), .c(new_n27_), .O(new_n52_));
  oai22  g035(.a(new_n50_), .b(new_n48_), .c(new_n37_), .d(new_n18_), .O(new_n53_));
  nand2  g036(.a(new_n53_), .b(x1), .O(new_n54_));
  aoi21  g037(.a(new_n54_), .b(new_n46_), .c(x4), .O(new_n55_));
  oai21  g038(.a(new_n55_), .b(new_n52_), .c(new_n28_), .O(new_n56_));
  nand2  g039(.a(x6), .b(x5), .O(new_n57_));
  oai21  g040(.a(x5), .b(new_n48_), .c(new_n57_), .O(new_n58_));
  nand2  g041(.a(new_n58_), .b(x0), .O(new_n59_));
  or2    g042(.a(new_n57_), .b(new_n20_), .O(new_n60_));
  aoi21  g043(.a(new_n60_), .b(new_n59_), .c(new_n27_), .O(new_n61_));
  aoi21  g044(.a(x5), .b(x0), .c(x2), .O(new_n62_));
  aoi21  g045(.a(x5), .b(x3), .c(x0), .O(new_n63_));
  oai21  g046(.a(new_n63_), .b(new_n62_), .c(x6), .O(new_n64_));
  nor2   g047(.a(new_n64_), .b(x4), .O(new_n65_));
  oai21  g048(.a(new_n65_), .b(new_n61_), .c(new_n19_), .O(new_n66_));
  nand2  g049(.a(new_n66_), .b(new_n56_), .O(z2));
  nand2  g050(.a(new_n23_), .b(x3), .O(new_n68_));
  oai21  g051(.a(new_n68_), .b(x0), .c(new_n46_), .O(new_n69_));
  nand3  g052(.a(new_n69_), .b(x6), .c(x2), .O(new_n70_));
  xor2a  g053(.a(x5), .b(x0), .O(new_n71_));
  nand2  g054(.a(new_n71_), .b(new_n40_), .O(new_n72_));
  nor2   g055(.a(new_n23_), .b(x3), .O(new_n73_));
  nand2  g056(.a(new_n73_), .b(new_n18_), .O(new_n74_));
  nand3  g057(.a(new_n74_), .b(new_n72_), .c(new_n70_), .O(new_n75_));
  nand2  g058(.a(new_n75_), .b(new_n19_), .O(new_n76_));
  nand3  g059(.a(x5), .b(x3), .c(x1), .O(new_n77_));
  inv1   g060(.a(new_n77_), .O(new_n78_));
  nor3   g061(.a(x5), .b(x3), .c(x2), .O(new_n79_));
  oai21  g062(.a(new_n79_), .b(new_n78_), .c(x0), .O(new_n80_));
  nand2  g063(.a(x2), .b(x1), .O(new_n81_));
  oai22  g064(.a(new_n81_), .b(new_n68_), .c(new_n23_), .d(x2), .O(new_n82_));
  nand2  g065(.a(new_n82_), .b(new_n18_), .O(new_n83_));
  nand3  g066(.a(new_n73_), .b(x2), .c(x1), .O(new_n84_));
  nand3  g067(.a(new_n84_), .b(new_n83_), .c(new_n80_), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(new_n28_), .O(new_n86_));
  nand2  g069(.a(new_n86_), .b(new_n76_), .O(z3));
  inv1   g070(.a(x3), .O(new_n88_));
  oai21  g071(.a(new_n88_), .b(new_n48_), .c(new_n18_), .O(new_n89_));
  aoi21  g072(.a(new_n89_), .b(new_n38_), .c(new_n19_), .O(new_n90_));
  oai21  g073(.a(x3), .b(x0), .c(x2), .O(new_n91_));
  nor2   g074(.a(new_n91_), .b(x1), .O(new_n92_));
  oai21  g075(.a(new_n92_), .b(new_n90_), .c(new_n28_), .O(new_n93_));
  nand3  g076(.a(new_n91_), .b(x6), .c(new_n19_), .O(new_n94_));
  nand2  g077(.a(new_n94_), .b(new_n93_), .O(z4));
  nor2   g078(.a(x3), .b(new_n48_), .O(new_n96_));
  nand4  g079(.a(new_n28_), .b(x3), .c(new_n48_), .d(x1), .O(new_n97_));
  inv1   g080(.a(new_n97_), .O(new_n98_));
  oai21  g081(.a(new_n98_), .b(new_n96_), .c(new_n18_), .O(new_n99_));
  aoi21  g082(.a(new_n48_), .b(x1), .c(new_n88_), .O(new_n100_));
  oai21  g083(.a(new_n100_), .b(new_n37_), .c(x0), .O(new_n101_));
  nand3  g084(.a(new_n101_), .b(new_n99_), .c(new_n44_), .O(z5));
  nand2  g085(.a(x5), .b(x4), .O(new_n103_));
  nand2  g086(.a(new_n103_), .b(x0), .O(new_n104_));
  nand2  g087(.a(x5), .b(new_n27_), .O(new_n105_));
  nand2  g088(.a(new_n23_), .b(new_n18_), .O(new_n106_));
  nand4  g089(.a(new_n106_), .b(new_n105_), .c(new_n104_), .d(x3), .O(new_n107_));
  nand3  g090(.a(x5), .b(x4), .c(x3), .O(new_n108_));
  inv1   g091(.a(new_n108_), .O(new_n109_));
  aoi21  g092(.a(new_n107_), .b(new_n28_), .c(new_n109_), .O(new_n110_));
  aoi21  g093(.a(new_n88_), .b(new_n48_), .c(x6), .O(new_n111_));
  oai21  g094(.a(new_n110_), .b(new_n48_), .c(new_n111_), .O(new_n112_));
  nand2  g095(.a(new_n112_), .b(x1), .O(new_n113_));
  nor2   g096(.a(new_n88_), .b(x2), .O(new_n114_));
  nand2  g097(.a(new_n114_), .b(new_n19_), .O(new_n115_));
  nand2  g098(.a(new_n115_), .b(new_n113_), .O(z6));
  inv1   g099(.a(new_n114_), .O(new_n117_));
  inv1   g100(.a(new_n96_), .O(new_n118_));
  aoi22  g101(.a(new_n118_), .b(new_n117_), .c(x6), .d(x1), .O(z7));
  aoi21  g102(.a(x6), .b(x1), .c(x3), .O(z8));
  nand2  g103(.a(x5), .b(x4), .O(new_n121_));
  oai21  g104(.a(new_n121_), .b(new_n20_), .c(new_n28_), .O(new_n122_));
  nand2  g105(.a(new_n122_), .b(x1), .O(new_n123_));
  nand2  g106(.a(new_n28_), .b(new_n18_), .O(new_n124_));
  nand3  g107(.a(new_n124_), .b(x5), .c(x4), .O(new_n125_));
  nand2  g108(.a(new_n125_), .b(new_n123_), .O(z9));
endmodule


