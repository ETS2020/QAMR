// Benchmark "FAU" written by ABC on Tue Aug 18 15:54:07 2020

module FAU ( 
    x0, x1, x2, x3, x4, x5, x6,
    z0, z1, z2, z3, z4, z5, z6, z7, z8, z9  );
  input  x0, x1, x2, x3, x4, x5, x6;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8, z9;
  wire new_n18_, new_n19_, new_n20_, new_n21_, new_n22_, new_n23_, new_n24_,
    new_n25_, new_n26_, new_n27_, new_n28_, new_n29_, new_n30_, new_n31_,
    new_n32_, new_n33_, new_n34_, new_n35_, new_n36_, new_n37_, new_n38_,
    new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_, new_n46_,
    new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_,
    new_n91_, new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_,
    new_n98_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n105_, new_n106_, new_n107_, new_n108_, new_n109_, new_n110_,
    new_n111_, new_n113_, new_n114_, new_n115_, new_n116_, new_n117_,
    new_n118_, new_n119_, new_n121_, new_n122_, new_n123_, new_n124_,
    new_n125_, new_n128_, new_n129_, new_n131_, new_n132_, new_n133_;
  inv1   g000(.a(x5), .O(new_n18_));
  nand3  g001(.a(new_n18_), .b(x4), .c(x0), .O(new_n19_));
  inv1   g002(.a(x4), .O(new_n20_));
  inv1   g003(.a(x6), .O(new_n21_));
  nor2   g004(.a(new_n21_), .b(new_n18_), .O(new_n22_));
  nand3  g005(.a(new_n22_), .b(new_n20_), .c(x3), .O(new_n23_));
  nand2  g006(.a(new_n23_), .b(new_n19_), .O(new_n24_));
  nand2  g007(.a(new_n24_), .b(x2), .O(new_n25_));
  nand3  g008(.a(new_n18_), .b(x4), .c(x3), .O(new_n26_));
  nand2  g009(.a(new_n22_), .b(new_n20_), .O(new_n27_));
  nand2  g010(.a(new_n27_), .b(new_n26_), .O(new_n28_));
  nand2  g011(.a(new_n28_), .b(x0), .O(new_n29_));
  inv1   g012(.a(x2), .O(new_n30_));
  oai21  g013(.a(x5), .b(x3), .c(x0), .O(new_n31_));
  nand2  g014(.a(new_n31_), .b(new_n30_), .O(new_n32_));
  aoi21  g015(.a(x5), .b(x3), .c(x0), .O(new_n33_));
  inv1   g016(.a(new_n33_), .O(new_n34_));
  nand2  g017(.a(new_n34_), .b(new_n32_), .O(new_n35_));
  nand3  g018(.a(new_n35_), .b(new_n21_), .c(x4), .O(new_n36_));
  nand2  g019(.a(new_n36_), .b(new_n27_), .O(new_n37_));
  nand2  g020(.a(new_n37_), .b(x1), .O(new_n38_));
  nor2   g021(.a(new_n21_), .b(x5), .O(new_n39_));
  nor2   g022(.a(x6), .b(x1), .O(new_n40_));
  aoi21  g023(.a(new_n39_), .b(x4), .c(new_n40_), .O(new_n41_));
  nand4  g024(.a(new_n41_), .b(new_n38_), .c(new_n29_), .d(new_n25_), .O(z0));
  inv1   g025(.a(x0), .O(new_n43_));
  inv1   g026(.a(x1), .O(new_n44_));
  nand2  g027(.a(x3), .b(x2), .O(new_n45_));
  nand4  g028(.a(new_n45_), .b(x6), .c(new_n44_), .d(new_n43_), .O(new_n46_));
  nand2  g029(.a(new_n45_), .b(new_n43_), .O(new_n47_));
  nand3  g030(.a(new_n47_), .b(new_n21_), .c(x1), .O(new_n48_));
  aoi21  g031(.a(new_n48_), .b(new_n46_), .c(x4), .O(new_n49_));
  nand3  g032(.a(new_n45_), .b(new_n21_), .c(new_n43_), .O(new_n50_));
  inv1   g033(.a(new_n50_), .O(new_n51_));
  oai21  g034(.a(new_n51_), .b(new_n49_), .c(x5), .O(new_n52_));
  oai21  g035(.a(x6), .b(x4), .c(x2), .O(new_n53_));
  nand2  g036(.a(x6), .b(x1), .O(new_n54_));
  nand2  g037(.a(x4), .b(x3), .O(new_n55_));
  nand3  g038(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  nand2  g039(.a(new_n56_), .b(x0), .O(new_n57_));
  oai21  g040(.a(new_n21_), .b(new_n20_), .c(new_n57_), .O(new_n58_));
  aoi21  g041(.a(new_n58_), .b(new_n18_), .c(new_n40_), .O(new_n59_));
  nand2  g042(.a(new_n59_), .b(new_n52_), .O(z1));
  inv1   g043(.a(new_n45_), .O(new_n61_));
  nand3  g044(.a(new_n22_), .b(new_n20_), .c(new_n44_), .O(new_n62_));
  nand3  g045(.a(new_n21_), .b(x4), .c(x1), .O(new_n63_));
  aoi21  g046(.a(new_n63_), .b(new_n62_), .c(new_n61_), .O(new_n64_));
  nand2  g047(.a(x6), .b(new_n20_), .O(new_n65_));
  aoi21  g048(.a(new_n65_), .b(new_n63_), .c(x5), .O(new_n66_));
  oai21  g049(.a(new_n66_), .b(new_n64_), .c(new_n43_), .O(new_n67_));
  inv1   g050(.a(x3), .O(new_n68_));
  nand4  g051(.a(new_n21_), .b(x4), .c(new_n68_), .d(x1), .O(new_n69_));
  oai21  g052(.a(new_n65_), .b(x1), .c(new_n69_), .O(new_n70_));
  nand3  g053(.a(new_n70_), .b(new_n18_), .c(new_n30_), .O(new_n71_));
  oai21  g054(.a(new_n33_), .b(new_n30_), .c(new_n31_), .O(new_n72_));
  nand3  g055(.a(new_n72_), .b(new_n21_), .c(new_n20_), .O(new_n73_));
  nand2  g056(.a(new_n18_), .b(new_n43_), .O(new_n74_));
  nand3  g057(.a(new_n74_), .b(x6), .c(x4), .O(new_n75_));
  nand2  g058(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand2  g059(.a(new_n76_), .b(x1), .O(new_n77_));
  oai21  g060(.a(x5), .b(x2), .c(x0), .O(new_n78_));
  nand3  g061(.a(x5), .b(x3), .c(x2), .O(new_n79_));
  nand2  g062(.a(new_n79_), .b(new_n78_), .O(new_n80_));
  nand3  g063(.a(new_n80_), .b(x6), .c(x4), .O(new_n81_));
  nand4  g064(.a(new_n81_), .b(new_n77_), .c(new_n71_), .d(new_n67_), .O(z2));
  nand3  g065(.a(x5), .b(x1), .c(x0), .O(new_n83_));
  nand3  g066(.a(new_n18_), .b(x2), .c(new_n43_), .O(new_n84_));
  nand2  g067(.a(new_n84_), .b(new_n83_), .O(new_n85_));
  nand2  g068(.a(new_n85_), .b(x3), .O(new_n86_));
  nand2  g069(.a(x5), .b(x0), .O(new_n87_));
  nand2  g070(.a(new_n87_), .b(new_n74_), .O(new_n88_));
  nand3  g071(.a(new_n88_), .b(x6), .c(x1), .O(new_n89_));
  nand2  g072(.a(new_n30_), .b(new_n43_), .O(new_n90_));
  nand2  g073(.a(new_n68_), .b(x2), .O(new_n91_));
  aoi21  g074(.a(new_n91_), .b(new_n90_), .c(x1), .O(new_n92_));
  oai21  g075(.a(new_n30_), .b(new_n43_), .c(new_n50_), .O(new_n93_));
  oai21  g076(.a(new_n93_), .b(new_n92_), .c(x5), .O(new_n94_));
  oai21  g077(.a(x6), .b(x3), .c(x1), .O(new_n95_));
  nand3  g078(.a(new_n95_), .b(new_n18_), .c(new_n30_), .O(new_n96_));
  inv1   g079(.a(new_n96_), .O(new_n97_));
  aoi21  g080(.a(new_n97_), .b(x0), .c(new_n40_), .O(new_n98_));
  nand4  g081(.a(new_n98_), .b(new_n94_), .c(new_n89_), .d(new_n86_), .O(z3));
  nand2  g082(.a(x3), .b(new_n30_), .O(new_n100_));
  oai21  g083(.a(new_n30_), .b(new_n44_), .c(new_n100_), .O(new_n101_));
  nand2  g084(.a(new_n101_), .b(x0), .O(new_n102_));
  nor2   g085(.a(new_n68_), .b(new_n44_), .O(new_n103_));
  nor3   g086(.a(x3), .b(x1), .c(x0), .O(new_n104_));
  oai21  g087(.a(new_n104_), .b(new_n103_), .c(x2), .O(new_n105_));
  nand2  g088(.a(new_n30_), .b(new_n44_), .O(new_n106_));
  nand3  g089(.a(new_n106_), .b(new_n105_), .c(new_n102_), .O(new_n107_));
  nand2  g090(.a(new_n107_), .b(x6), .O(new_n108_));
  nand2  g091(.a(new_n68_), .b(new_n30_), .O(new_n109_));
  nand2  g092(.a(new_n109_), .b(new_n47_), .O(new_n110_));
  nand3  g093(.a(new_n110_), .b(new_n21_), .c(x1), .O(new_n111_));
  nand2  g094(.a(new_n111_), .b(new_n108_), .O(z4));
  aoi21  g095(.a(new_n106_), .b(new_n45_), .c(new_n43_), .O(new_n113_));
  nand4  g096(.a(new_n68_), .b(x2), .c(new_n44_), .d(new_n43_), .O(new_n114_));
  inv1   g097(.a(new_n114_), .O(new_n115_));
  oai21  g098(.a(new_n115_), .b(new_n113_), .c(x6), .O(new_n116_));
  aoi21  g099(.a(new_n109_), .b(new_n45_), .c(new_n43_), .O(new_n117_));
  aoi21  g100(.a(new_n100_), .b(new_n91_), .c(x0), .O(new_n118_));
  oai21  g101(.a(new_n118_), .b(new_n117_), .c(x1), .O(new_n119_));
  nand2  g102(.a(new_n119_), .b(new_n116_), .O(z5));
  oai21  g103(.a(new_n21_), .b(new_n30_), .c(x3), .O(new_n121_));
  nand2  g104(.a(new_n121_), .b(x1), .O(new_n122_));
  oai21  g105(.a(new_n68_), .b(x2), .c(x6), .O(new_n123_));
  nand2  g106(.a(new_n123_), .b(new_n44_), .O(new_n124_));
  nand2  g107(.a(new_n21_), .b(x2), .O(new_n125_));
  nand3  g108(.a(new_n125_), .b(new_n124_), .c(new_n122_), .O(z6));
  aoi22  g109(.a(new_n100_), .b(new_n91_), .c(new_n21_), .d(new_n44_), .O(z7));
  aoi21  g110(.a(new_n68_), .b(new_n30_), .c(new_n21_), .O(new_n128_));
  aoi21  g111(.a(x6), .b(x2), .c(x1), .O(new_n129_));
  oai22  g112(.a(new_n129_), .b(x3), .c(new_n128_), .d(x1), .O(z8));
  nand2  g113(.a(new_n47_), .b(x1), .O(new_n131_));
  nand2  g114(.a(new_n131_), .b(new_n21_), .O(new_n132_));
  nand3  g115(.a(new_n132_), .b(x5), .c(x4), .O(new_n133_));
  inv1   g116(.a(new_n133_), .O(z9));
endmodule


