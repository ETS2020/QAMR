// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:48 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x17), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  inv1   g06(.a(x16), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nand2  g08(.a(x14), .b(x13), .O(new_n37_));
  nor4   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n33_), .c(x09), .d(new_n31_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g12(.a(x17), .b(x10), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n47_));
  nand2  g19(.a(x08), .b(x01), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(new_n47_), .c(new_n41_), .O(z2));
  inv1   g21(.a(x02), .O(new_n50_));
  nand2  g22(.a(new_n41_), .b(x08), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  oai21  g25(.a(new_n33_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n55_));
  oai21  g27(.a(new_n51_), .b(new_n50_), .c(new_n55_), .O(z3));
  inv1   g28(.a(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n58_), .b(x13), .O(new_n61_));
  oai22  g33(.a(new_n61_), .b(new_n32_), .c(new_n60_), .d(new_n58_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n51_), .b(new_n57_), .c(new_n63_), .O(z4));
  inv1   g36(.a(x04), .O(new_n65_));
  oai21  g37(.a(new_n37_), .b(new_n32_), .c(x15), .O(new_n66_));
  nand3  g38(.a(new_n34_), .b(x14), .c(x13), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n32_), .c(new_n66_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n69_));
  oai21  g41(.a(new_n51_), .b(new_n65_), .c(new_n69_), .O(z5));
  inv1   g42(.a(x05), .O(new_n71_));
  nand3  g43(.a(x15), .b(x14), .c(x13), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n32_), .c(x16), .O(new_n73_));
  nand3  g45(.a(new_n35_), .b(x15), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n59_), .c(new_n73_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n76_));
  oai21  g48(.a(new_n51_), .b(new_n71_), .c(new_n76_), .O(z6));
  nand3  g49(.a(x16), .b(x12), .c(x11), .O(new_n78_));
  aoi21  g50(.a(new_n72_), .b(new_n29_), .c(new_n78_), .O(new_n79_));
  nand4  g51(.a(x13), .b(x12), .c(x11), .d(new_n29_), .O(new_n80_));
  and2   g52(.a(x15), .b(x14), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(new_n30_), .c(x16), .O(new_n82_));
  oai22  g54(.a(new_n82_), .b(new_n80_), .c(new_n79_), .d(new_n30_), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x09), .c(new_n31_), .O(new_n84_));
  inv1   g56(.a(new_n41_), .O(new_n85_));
  aoi21  g57(.a(x08), .b(x06), .c(new_n85_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n84_), .O(z7));
  nand3  g59(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n59_), .c(x18), .O(new_n89_));
  nand4  g61(.a(new_n81_), .b(new_n60_), .c(new_n36_), .d(x16), .O(new_n90_));
  aoi21  g62(.a(new_n90_), .b(new_n89_), .c(new_n30_), .O(new_n91_));
  nand3  g63(.a(x18), .b(new_n30_), .c(new_n29_), .O(new_n92_));
  inv1   g64(.a(new_n92_), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n91_), .c(x09), .O(new_n94_));
  aoi21  g66(.a(x08), .b(x07), .c(new_n85_), .O(new_n95_));
  oai21  g67(.a(new_n94_), .b(x08), .c(new_n95_), .O(z8));
endmodule


