// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:04 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_, new_n77_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_, new_n85_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x09), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand2  g03(.a(x13), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n33_), .c(new_n31_), .d(new_n30_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x12), .c(new_n29_), .O(z0));
  inv1   g10(.a(x12), .O(new_n39_));
  nor3   g11(.a(x11), .b(x10), .c(x08), .O(new_n40_));
  oai21  g12(.a(new_n40_), .b(new_n39_), .c(x09), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z1));
  nand2  g15(.a(x08), .b(x01), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n41_), .O(z2));
  xor2a  g17(.a(x13), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n31_), .c(new_n30_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x12), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(x09), .O(new_n49_));
  nand2  g21(.a(x08), .b(x02), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z3));
  nand2  g23(.a(new_n32_), .b(x14), .O(new_n52_));
  inv1   g24(.a(x14), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(x13), .c(x11), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n52_), .c(x10), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n30_), .c(new_n39_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x03), .O(new_n57_));
  oai21  g29(.a(new_n56_), .b(new_n29_), .c(new_n57_), .O(z4));
  nand3  g30(.a(x14), .b(x13), .c(x11), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x15), .O(new_n60_));
  inv1   g32(.a(x15), .O(new_n61_));
  nand3  g33(.a(new_n33_), .b(new_n61_), .c(x14), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n60_), .c(x10), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n30_), .c(new_n39_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x04), .O(new_n65_));
  oai21  g37(.a(new_n64_), .b(new_n29_), .c(new_n65_), .O(z5));
  oai21  g38(.a(new_n34_), .b(new_n32_), .c(x16), .O(new_n67_));
  inv1   g39(.a(x16), .O(new_n68_));
  nand4  g40(.a(new_n33_), .b(new_n68_), .c(x15), .d(x14), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n67_), .c(x10), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n30_), .c(new_n39_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x05), .O(new_n72_));
  oai21  g44(.a(new_n71_), .b(new_n29_), .c(new_n72_), .O(z6));
  nand2  g45(.a(new_n39_), .b(x09), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(x08), .c(x06), .O(new_n75_));
  nand3  g47(.a(x16), .b(x15), .c(x14), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n32_), .c(x17), .O(new_n77_));
  inv1   g49(.a(x17), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(x16), .c(x15), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n59_), .c(new_n77_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(x12), .c(new_n31_), .d(x09), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(x08), .c(new_n75_), .O(z7));
  nand3  g54(.a(x17), .b(x16), .c(x15), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n59_), .c(x18), .O(new_n84_));
  inv1   g56(.a(new_n59_), .O(new_n85_));
  nor2   g57(.a(new_n68_), .b(new_n61_), .O(new_n86_));
  nor2   g58(.a(x18), .b(new_n78_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n85_), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(new_n84_), .c(x10), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n30_), .c(new_n39_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x07), .O(new_n91_));
  oai21  g63(.a(new_n90_), .b(new_n29_), .c(new_n91_), .O(z8));
endmodule


