// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand2  g01(.a(x13), .b(x12), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x16), .O(new_n32_));
  inv1   g04(.a(x17), .O(new_n33_));
  inv1   g05(.a(x18), .O(new_n34_));
  nand2  g06(.a(x15), .b(x14), .O(new_n35_));
  nor4   g07(.a(new_n35_), .b(new_n34_), .c(new_n33_), .d(new_n32_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n31_), .c(x09), .d(new_n29_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x11), .c(x10), .O(z0));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n39_), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(new_n41_), .O(z1));
  inv1   g15(.a(x01), .O(new_n44_));
  nand2  g16(.a(x09), .b(new_n29_), .O(new_n45_));
  nand2  g17(.a(new_n41_), .b(x08), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x11), .c(new_n39_), .O(new_n48_));
  oai22  g20(.a(new_n48_), .b(new_n45_), .c(new_n46_), .d(new_n44_), .O(z2));
  xor2a  g21(.a(x13), .b(x12), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(x09), .c(new_n29_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n39_), .O(new_n53_));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n53_), .O(z3));
  inv1   g27(.a(x09), .O(new_n56_));
  nand2  g28(.a(new_n30_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(x13), .c(x12), .O(new_n59_));
  aoi21  g31(.a(new_n59_), .b(new_n57_), .c(new_n56_), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n29_), .c(new_n40_), .O(new_n61_));
  nand2  g33(.a(x08), .b(x03), .O(new_n62_));
  oai21  g34(.a(new_n61_), .b(x10), .c(new_n62_), .O(z4));
  inv1   g35(.a(x04), .O(new_n64_));
  nand3  g36(.a(x14), .b(x13), .c(x12), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x15), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand3  g39(.a(new_n31_), .b(new_n67_), .c(x14), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n66_), .c(new_n40_), .O(new_n69_));
  nand4  g41(.a(new_n69_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n70_));
  oai21  g42(.a(new_n46_), .b(new_n64_), .c(new_n70_), .O(z5));
  inv1   g43(.a(x05), .O(new_n72_));
  oai21  g44(.a(new_n35_), .b(new_n30_), .c(x16), .O(new_n73_));
  nand3  g45(.a(new_n32_), .b(x15), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n30_), .c(new_n73_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(x11), .c(new_n39_), .d(x09), .O(new_n76_));
  oai22  g48(.a(new_n76_), .b(x08), .c(new_n46_), .d(new_n72_), .O(z6));
  nand3  g49(.a(x16), .b(x13), .c(x12), .O(new_n78_));
  aoi21  g50(.a(new_n35_), .b(x11), .c(new_n78_), .O(new_n79_));
  nand3  g51(.a(new_n33_), .b(x16), .c(x15), .O(new_n80_));
  oai22  g52(.a(new_n80_), .b(new_n65_), .c(new_n79_), .d(new_n33_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(x09), .c(new_n29_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x11), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n39_), .O(new_n84_));
  nand2  g56(.a(x08), .b(x06), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n84_), .O(z7));
  inv1   g58(.a(x07), .O(new_n87_));
  nand3  g59(.a(x17), .b(x16), .c(x15), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n65_), .c(x18), .O(new_n89_));
  nand4  g61(.a(new_n34_), .b(x17), .c(x16), .d(x15), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n65_), .c(new_n89_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(x11), .c(new_n39_), .d(x09), .O(new_n92_));
  oai22  g64(.a(new_n92_), .b(x08), .c(new_n46_), .d(new_n87_), .O(z8));
endmodule


