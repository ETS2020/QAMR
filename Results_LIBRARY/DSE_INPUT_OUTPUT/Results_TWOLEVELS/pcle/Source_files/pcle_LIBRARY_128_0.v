// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_, new_n108_, new_n109_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(x09), .O(new_n39_));
  nand2  g11(.a(x12), .b(new_n39_), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n38_), .c(new_n31_), .d(new_n30_), .O(new_n41_));
  nor2   g13(.a(x12), .b(x09), .O(new_n42_));
  aoi21  g14(.a(x08), .b(x00), .c(new_n42_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n41_), .O(z1));
  nand3  g16(.a(x12), .b(new_n38_), .c(x09), .O(new_n45_));
  oai21  g17(.a(x12), .b(new_n38_), .c(new_n45_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n31_), .c(new_n30_), .O(new_n47_));
  aoi21  g19(.a(x08), .b(x01), .c(new_n42_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  nand2  g21(.a(x13), .b(new_n38_), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(x12), .c(x11), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n50_), .c(new_n39_), .O(new_n53_));
  nor2   g25(.a(new_n51_), .b(x12), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n53_), .c(new_n31_), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n42_), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(x08), .c(new_n56_), .O(z3));
  inv1   g29(.a(x03), .O(new_n58_));
  oai21  g30(.a(x12), .b(x09), .c(x08), .O(new_n59_));
  nand3  g31(.a(x13), .b(x12), .c(x11), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x14), .O(new_n61_));
  nand2  g33(.a(x12), .b(x11), .O(new_n62_));
  inv1   g34(.a(x14), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x13), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n62_), .c(new_n61_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n66_));
  oai21  g38(.a(new_n59_), .b(new_n58_), .c(new_n66_), .O(z4));
  inv1   g39(.a(x12), .O(new_n68_));
  nand3  g40(.a(x15), .b(new_n31_), .c(new_n30_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(x09), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n68_), .O(new_n71_));
  nand3  g43(.a(x14), .b(x13), .c(x11), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x15), .O(new_n73_));
  inv1   g45(.a(x15), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(x14), .c(x13), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n62_), .c(new_n73_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n77_));
  nand2  g49(.a(x08), .b(x04), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(new_n77_), .c(new_n71_), .O(z5));
  inv1   g51(.a(x05), .O(new_n80_));
  nand3  g52(.a(x15), .b(x14), .c(x13), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n62_), .c(x16), .O(new_n82_));
  inv1   g54(.a(x16), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x15), .c(x14), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n60_), .c(new_n82_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n86_));
  oai21  g58(.a(new_n59_), .b(new_n80_), .c(new_n86_), .O(z6));
  nand3  g59(.a(x17), .b(new_n31_), .c(new_n30_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x09), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n68_), .O(new_n90_));
  nand2  g62(.a(x13), .b(x11), .O(new_n91_));
  nand3  g63(.a(x16), .b(x15), .c(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n91_), .c(x17), .O(new_n93_));
  inv1   g65(.a(x17), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(x16), .c(x15), .d(x14), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n60_), .c(new_n93_), .O(new_n96_));
  nand4  g68(.a(new_n96_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n97_));
  nand2  g69(.a(x08), .b(x06), .O(new_n98_));
  nand3  g70(.a(new_n98_), .b(new_n97_), .c(new_n90_), .O(z7));
  nand3  g71(.a(x18), .b(new_n31_), .c(new_n30_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(x09), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n68_), .O(new_n102_));
  nand3  g74(.a(x17), .b(x16), .c(x15), .O(new_n103_));
  oai21  g75(.a(new_n103_), .b(new_n72_), .c(x18), .O(new_n104_));
  nand4  g76(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n105_));
  nand4  g77(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n106_));
  oai21  g78(.a(new_n106_), .b(new_n105_), .c(new_n104_), .O(new_n107_));
  nand4  g79(.a(new_n107_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n108_));
  nand2  g80(.a(x08), .b(x07), .O(new_n109_));
  nand3  g81(.a(new_n109_), .b(new_n108_), .c(new_n102_), .O(z8));
endmodule


