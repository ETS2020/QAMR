// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  and2   g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(x13), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor3   g07(.a(new_n35_), .b(new_n34_), .c(new_n33_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g10(.a(new_n30_), .b(new_n29_), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand4  g14(.a(new_n42_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(new_n46_), .c(new_n40_), .O(z2));
  nand3  g20(.a(new_n33_), .b(x12), .c(x11), .O(new_n49_));
  oai21  g21(.a(new_n32_), .b(new_n33_), .c(new_n49_), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x02), .c(new_n39_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(new_n51_), .O(z3));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x14), .O(new_n55_));
  nand2  g27(.a(x11), .b(new_n29_), .O(new_n56_));
  nand3  g28(.a(new_n30_), .b(x13), .c(x12), .O(new_n57_));
  oai21  g29(.a(new_n57_), .b(new_n56_), .c(new_n55_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(x09), .c(new_n31_), .O(new_n59_));
  aoi21  g31(.a(x08), .b(x03), .c(new_n39_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n59_), .O(z4));
  nand2  g33(.a(new_n54_), .b(x15), .O(new_n62_));
  nand4  g34(.a(new_n34_), .b(x13), .c(x12), .d(x11), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n62_), .c(new_n30_), .O(new_n64_));
  nand3  g36(.a(x15), .b(new_n30_), .c(new_n29_), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n64_), .c(x09), .O(new_n67_));
  aoi21  g39(.a(x08), .b(x04), .c(new_n39_), .O(new_n68_));
  oai21  g40(.a(new_n67_), .b(x08), .c(new_n68_), .O(z5));
  inv1   g41(.a(x16), .O(new_n70_));
  nand2  g42(.a(x15), .b(x14), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n29_), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n55_), .c(new_n70_), .O(new_n73_));
  nand3  g45(.a(new_n70_), .b(x15), .c(x14), .O(new_n74_));
  nor2   g46(.a(new_n74_), .b(new_n54_), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n73_), .c(x09), .O(new_n76_));
  aoi21  g48(.a(x08), .b(x05), .c(new_n39_), .O(new_n77_));
  oai21  g49(.a(new_n76_), .b(x08), .c(new_n77_), .O(z6));
  nand3  g50(.a(new_n40_), .b(x08), .c(x06), .O(new_n79_));
  nand3  g51(.a(x16), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n54_), .c(x17), .O(new_n81_));
  inv1   g53(.a(x17), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(x16), .c(x15), .d(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n54_), .c(new_n81_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(new_n79_), .O(z7));
  inv1   g58(.a(x18), .O(new_n87_));
  nand4  g59(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n88_));
  aoi22  g60(.a(new_n88_), .b(new_n29_), .c(new_n54_), .d(x14), .O(new_n89_));
  nor2   g61(.a(new_n70_), .b(new_n34_), .O(new_n90_));
  nand4  g62(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n91_));
  inv1   g63(.a(new_n91_), .O(new_n92_));
  nor2   g64(.a(x18), .b(new_n82_), .O(new_n93_));
  nand3  g65(.a(new_n93_), .b(new_n92_), .c(new_n90_), .O(new_n94_));
  oai21  g66(.a(new_n89_), .b(new_n87_), .c(new_n94_), .O(new_n95_));
  nand3  g67(.a(new_n95_), .b(x09), .c(new_n31_), .O(new_n96_));
  aoi21  g68(.a(x08), .b(x07), .c(new_n39_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n96_), .O(z8));
endmodule


