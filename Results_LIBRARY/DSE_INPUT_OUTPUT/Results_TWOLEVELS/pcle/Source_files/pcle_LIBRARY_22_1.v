// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x09), .O(new_n38_));
  nand2  g10(.a(x12), .b(new_n38_), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n40_), .c(new_n39_), .O(z1));
  inv1   g15(.a(x12), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(x11), .c(x09), .O(new_n45_));
  oai21  g17(.a(new_n44_), .b(x11), .c(new_n45_), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(new_n31_), .c(new_n30_), .O(new_n47_));
  inv1   g19(.a(new_n39_), .O(new_n48_));
  aoi21  g20(.a(x08), .b(x01), .c(new_n48_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n47_), .O(z2));
  xnor2a g22(.a(x13), .b(x11), .O(new_n51_));
  nand3  g23(.a(x13), .b(new_n44_), .c(x09), .O(new_n52_));
  oai21  g24(.a(new_n51_), .b(new_n44_), .c(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(new_n31_), .c(new_n30_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n48_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  inv1   g28(.a(x03), .O(new_n57_));
  nand2  g29(.a(new_n39_), .b(x08), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  nand3  g31(.a(x13), .b(x12), .c(x11), .O(new_n60_));
  inv1   g32(.a(new_n60_), .O(new_n61_));
  nand2  g33(.a(x12), .b(x11), .O(new_n62_));
  nand2  g34(.a(new_n59_), .b(x13), .O(new_n63_));
  oai22  g35(.a(new_n63_), .b(new_n62_), .c(new_n61_), .d(new_n59_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n65_));
  oai21  g37(.a(new_n58_), .b(new_n57_), .c(new_n65_), .O(z4));
  inv1   g38(.a(x04), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand4  g40(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n68_), .b(x14), .c(x13), .O(new_n71_));
  oai22  g43(.a(new_n71_), .b(new_n62_), .c(new_n70_), .d(new_n68_), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n73_));
  oai21  g45(.a(new_n58_), .b(new_n67_), .c(new_n73_), .O(z5));
  inv1   g46(.a(x05), .O(new_n75_));
  nand3  g47(.a(x15), .b(x14), .c(x13), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n62_), .c(x16), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n60_), .c(new_n77_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n81_));
  oai21  g53(.a(new_n58_), .b(new_n75_), .c(new_n81_), .O(z6));
  inv1   g54(.a(x17), .O(new_n83_));
  nand2  g55(.a(x16), .b(x11), .O(new_n84_));
  nand4  g56(.a(x15), .b(x14), .c(x13), .d(x12), .O(new_n85_));
  aoi22  g57(.a(new_n85_), .b(x09), .c(new_n84_), .d(x12), .O(new_n86_));
  nor2   g58(.a(new_n68_), .b(new_n59_), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(new_n61_), .c(new_n83_), .d(x16), .O(new_n88_));
  oai21  g60(.a(new_n86_), .b(new_n83_), .c(new_n88_), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(new_n31_), .c(new_n30_), .O(new_n90_));
  aoi21  g62(.a(x08), .b(x06), .c(new_n48_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n90_), .O(z7));
  inv1   g64(.a(x07), .O(new_n93_));
  nand4  g65(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n60_), .c(x18), .O(new_n95_));
  nor2   g67(.a(x18), .b(new_n83_), .O(new_n96_));
  nand4  g68(.a(new_n96_), .b(new_n70_), .c(x16), .d(x15), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n95_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n99_));
  oai21  g71(.a(new_n58_), .b(new_n93_), .c(new_n99_), .O(z8));
endmodule


