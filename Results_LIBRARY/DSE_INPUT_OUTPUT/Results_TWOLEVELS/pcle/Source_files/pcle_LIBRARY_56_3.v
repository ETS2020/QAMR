// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:45 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_;
  inv1   g00(.a(x08), .O(new_n29_));
  inv1   g01(.a(x09), .O(new_n30_));
  inv1   g02(.a(x11), .O(new_n31_));
  nor2   g03(.a(new_n31_), .b(x10), .O(new_n32_));
  inv1   g04(.a(x12), .O(new_n33_));
  inv1   g05(.a(x13), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  and2   g07(.a(x15), .b(x14), .O(new_n36_));
  nand3  g08(.a(x18), .b(x17), .c(x16), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n36_), .c(new_n35_), .d(new_n32_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g12(.a(x09), .b(new_n29_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  inv1   g14(.a(x10), .O(new_n43_));
  nand4  g15(.a(new_n31_), .b(new_n43_), .c(x09), .d(new_n29_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n42_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n43_), .c(x09), .d(new_n29_), .O(new_n47_));
  nand2  g19(.a(new_n41_), .b(x01), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n47_), .O(z2));
  oai21  g21(.a(x09), .b(x02), .c(x08), .O(new_n50_));
  nand2  g22(.a(x12), .b(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(x13), .O(new_n52_));
  nand3  g24(.a(new_n34_), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(new_n43_), .c(x09), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n50_), .O(z3));
  inv1   g28(.a(x14), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  inv1   g30(.a(new_n58_), .O(new_n59_));
  nand2  g31(.a(new_n57_), .b(x13), .O(new_n60_));
  oai22  g32(.a(new_n60_), .b(new_n51_), .c(new_n59_), .d(new_n57_), .O(new_n61_));
  nand4  g33(.a(new_n61_), .b(new_n43_), .c(x09), .d(new_n29_), .O(new_n62_));
  nand2  g34(.a(new_n41_), .b(x03), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z4));
  inv1   g36(.a(x15), .O(new_n65_));
  nand4  g37(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n66_));
  inv1   g38(.a(new_n66_), .O(new_n67_));
  nand3  g39(.a(new_n65_), .b(x14), .c(x13), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(new_n51_), .c(new_n67_), .d(new_n65_), .O(new_n69_));
  nand4  g41(.a(new_n69_), .b(new_n43_), .c(x09), .d(new_n29_), .O(new_n70_));
  nand2  g42(.a(new_n41_), .b(x04), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z5));
  oai21  g44(.a(x09), .b(x05), .c(x08), .O(new_n73_));
  aoi21  g45(.a(x14), .b(x13), .c(x08), .O(new_n74_));
  nand3  g46(.a(x15), .b(x12), .c(x11), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n74_), .c(x16), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand4  g49(.a(new_n59_), .b(new_n77_), .c(x15), .d(x14), .O(new_n78_));
  nand2  g50(.a(new_n78_), .b(new_n76_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(new_n43_), .c(x09), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(new_n73_), .O(z6));
  nand3  g53(.a(x16), .b(x15), .c(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n58_), .c(x17), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand3  g56(.a(new_n36_), .b(new_n84_), .c(x16), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n58_), .c(new_n83_), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(new_n43_), .c(x09), .d(new_n29_), .O(new_n87_));
  nand2  g59(.a(new_n41_), .b(x06), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(z7));
  oai21  g61(.a(x09), .b(x07), .c(x08), .O(new_n90_));
  nand2  g62(.a(new_n77_), .b(new_n29_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(new_n59_), .c(new_n36_), .d(x17), .O(new_n92_));
  nand2  g64(.a(new_n92_), .b(x18), .O(new_n93_));
  nor2   g65(.a(x18), .b(new_n84_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n67_), .c(x16), .d(x15), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n43_), .c(x09), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n90_), .O(z8));
endmodule


