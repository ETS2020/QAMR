// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:36 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_;
  nand2  g00(.a(x17), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  nand2  g02(.a(x08), .b(x00), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x11), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(new_n31_), .c(z0), .O(z1));
  xor2a  g08(.a(x12), .b(x11), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n38_));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n38_), .c(new_n29_), .O(z2));
  inv1   g12(.a(x13), .O(new_n41_));
  and2   g13(.a(x12), .b(x11), .O(new_n42_));
  nand3  g14(.a(new_n41_), .b(x12), .c(x11), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(new_n41_), .c(new_n43_), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n45_));
  aoi21  g17(.a(x08), .b(x02), .c(z0), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n45_), .O(z3));
  nand3  g19(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n48_));
  inv1   g20(.a(x14), .O(new_n49_));
  nand3  g21(.a(new_n42_), .b(new_n49_), .c(x13), .O(new_n50_));
  nor2   g22(.a(new_n50_), .b(new_n48_), .O(new_n51_));
  aoi21  g23(.a(x08), .b(x03), .c(new_n51_), .O(new_n52_));
  nand2  g24(.a(x17), .b(x15), .O(new_n53_));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n53_), .c(x14), .d(new_n33_), .O(new_n55_));
  inv1   g27(.a(new_n55_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x09), .c(new_n32_), .O(new_n57_));
  oai21  g29(.a(new_n52_), .b(z0), .c(new_n57_), .O(z4));
  nand3  g30(.a(new_n29_), .b(x08), .c(x04), .O(new_n59_));
  inv1   g31(.a(x17), .O(new_n60_));
  nand3  g32(.a(new_n54_), .b(new_n60_), .c(x15), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(x13), .c(x12), .d(x11), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x14), .O(new_n65_));
  nand3  g37(.a(new_n60_), .b(x15), .c(new_n49_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n59_), .O(z5));
  nand3  g41(.a(new_n29_), .b(x08), .c(x05), .O(new_n70_));
  inv1   g42(.a(x16), .O(new_n71_));
  nand4  g43(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(new_n60_), .c(new_n62_), .O(new_n73_));
  nand4  g45(.a(new_n60_), .b(new_n71_), .c(x15), .d(x14), .O(new_n74_));
  oai22  g46(.a(new_n74_), .b(new_n54_), .c(new_n73_), .d(new_n71_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n70_), .O(z6));
  nand3  g49(.a(x16), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n54_), .c(new_n60_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n80_));
  aoi21  g52(.a(x08), .b(x06), .c(z0), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n80_), .O(z7));
  nand2  g54(.a(x08), .b(x07), .O(new_n83_));
  nand4  g55(.a(x18), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(new_n29_), .O(z8));
endmodule


