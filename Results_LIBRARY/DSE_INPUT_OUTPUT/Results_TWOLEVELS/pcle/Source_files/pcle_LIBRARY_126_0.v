// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:57 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(x09), .c(new_n31_), .d(x07), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x13), .c(x12), .d(x11), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x17), .c(x16), .d(x15), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n29_), .O(z0));
  nand2  g10(.a(new_n31_), .b(x07), .O(new_n39_));
  inv1   g11(.a(x07), .O(new_n40_));
  aoi21  g12(.a(new_n32_), .b(new_n40_), .c(new_n31_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n32_), .c(x09), .O(new_n44_));
  oai21  g16(.a(new_n44_), .b(new_n39_), .c(new_n42_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x09), .c(new_n31_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(x07), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n32_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g23(.a(new_n41_), .b(x02), .O(new_n52_));
  and2   g24(.a(x12), .b(x11), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x13), .O(new_n55_));
  inv1   g27(.a(x13), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x12), .c(x11), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n55_), .c(x10), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(x09), .c(new_n31_), .d(x07), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n52_), .O(z3));
  inv1   g32(.a(x09), .O(new_n61_));
  inv1   g33(.a(x12), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x07), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(x13), .c(x11), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x14), .O(new_n65_));
  nand3  g37(.a(new_n53_), .b(new_n30_), .c(x13), .O(new_n66_));
  aoi21  g38(.a(new_n66_), .b(new_n65_), .c(new_n61_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n31_), .c(new_n40_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x03), .O(new_n69_));
  oai21  g41(.a(new_n68_), .b(x10), .c(new_n69_), .O(z4));
  nand2  g42(.a(new_n41_), .b(x04), .O(new_n71_));
  nand3  g43(.a(new_n53_), .b(x14), .c(x13), .O(new_n72_));
  nand2  g44(.a(new_n72_), .b(x15), .O(new_n73_));
  inv1   g45(.a(x15), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(x14), .c(x13), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n54_), .c(new_n73_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n40_), .c(new_n71_), .O(z5));
  nand2  g50(.a(new_n30_), .b(x07), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n53_), .c(x15), .d(x13), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x16), .O(new_n81_));
  nand3  g53(.a(x13), .b(x12), .c(x11), .O(new_n82_));
  inv1   g54(.a(new_n82_), .O(new_n83_));
  nor2   g55(.a(x16), .b(new_n74_), .O(new_n84_));
  nand3  g56(.a(new_n84_), .b(new_n83_), .c(x14), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n81_), .c(new_n61_), .O(new_n86_));
  aoi21  g58(.a(new_n86_), .b(new_n31_), .c(new_n40_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x05), .O(new_n88_));
  oai21  g60(.a(new_n87_), .b(x10), .c(new_n88_), .O(z6));
  and2   g61(.a(x16), .b(x15), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n83_), .c(new_n79_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(x17), .O(new_n92_));
  inv1   g64(.a(x17), .O(new_n93_));
  nor2   g65(.a(new_n74_), .b(new_n30_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n83_), .c(new_n93_), .d(x16), .O(new_n95_));
  aoi21  g67(.a(new_n95_), .b(new_n92_), .c(new_n61_), .O(new_n96_));
  aoi21  g68(.a(new_n96_), .b(new_n31_), .c(new_n40_), .O(new_n97_));
  nand2  g69(.a(x08), .b(x06), .O(new_n98_));
  oai21  g70(.a(new_n97_), .b(x10), .c(new_n98_), .O(z7));
  nand3  g71(.a(x14), .b(x13), .c(x12), .O(new_n100_));
  nand4  g72(.a(x17), .b(x16), .c(x15), .d(x11), .O(new_n101_));
  aoi21  g73(.a(new_n100_), .b(x07), .c(new_n101_), .O(new_n102_));
  nand3  g74(.a(new_n90_), .b(new_n29_), .c(x17), .O(new_n103_));
  oai22  g75(.a(new_n103_), .b(new_n72_), .c(new_n102_), .d(new_n29_), .O(new_n104_));
  aoi21  g76(.a(new_n104_), .b(x09), .c(new_n40_), .O(new_n105_));
  oai22  g77(.a(new_n105_), .b(x10), .c(new_n31_), .d(new_n40_), .O(z8));
endmodule


