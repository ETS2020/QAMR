// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:51 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(x12), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  nand2  g04(.a(x09), .b(new_n32_), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand2  g06(.a(x12), .b(x11), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand3  g08(.a(x15), .b(x14), .c(x13), .O(new_n37_));
  nand3  g09(.a(x18), .b(x17), .c(x16), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g11(.a(new_n39_), .b(new_n36_), .c(new_n34_), .d(new_n29_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n31_), .O(z0));
  nor2   g13(.a(new_n30_), .b(new_n32_), .O(new_n42_));
  nand2  g14(.a(new_n42_), .b(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand3  g16(.a(new_n34_), .b(new_n44_), .c(new_n29_), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n43_), .O(z1));
  nand3  g18(.a(x12), .b(new_n44_), .c(new_n29_), .O(new_n47_));
  oai21  g19(.a(x12), .b(new_n44_), .c(new_n47_), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(x09), .c(new_n32_), .O(new_n49_));
  aoi21  g21(.a(x08), .b(x01), .c(new_n30_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  nand2  g23(.a(new_n42_), .b(x02), .O(new_n52_));
  inv1   g24(.a(x13), .O(new_n53_));
  nand3  g25(.a(new_n53_), .b(x12), .c(x11), .O(new_n54_));
  oai21  g26(.a(new_n36_), .b(new_n53_), .c(new_n54_), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(new_n52_), .O(z3));
  inv1   g29(.a(x12), .O(new_n58_));
  inv1   g30(.a(x14), .O(new_n59_));
  oai21  g31(.a(new_n33_), .b(new_n59_), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand2  g33(.a(x13), .b(x11), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x14), .O(new_n63_));
  nand2  g35(.a(new_n59_), .b(x13), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n35_), .c(new_n63_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n66_));
  nand2  g38(.a(x08), .b(x03), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n66_), .c(new_n61_), .O(z4));
  nand2  g40(.a(new_n42_), .b(x04), .O(new_n69_));
  nand4  g41(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(x15), .O(new_n71_));
  inv1   g43(.a(x15), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(x14), .c(x13), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n35_), .c(new_n71_), .O(new_n74_));
  nand4  g46(.a(new_n74_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n69_), .O(z5));
  nand2  g48(.a(new_n42_), .b(x05), .O(new_n77_));
  oai21  g49(.a(new_n37_), .b(new_n35_), .c(x16), .O(new_n78_));
  nand3  g50(.a(x13), .b(x12), .c(x11), .O(new_n79_));
  inv1   g51(.a(x16), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(x15), .c(x14), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n77_), .O(z6));
  nand2  g56(.a(x17), .b(x09), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(x08), .c(new_n29_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n58_), .O(new_n87_));
  nand3  g59(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n62_), .c(x17), .O(new_n89_));
  inv1   g61(.a(x17), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(x16), .c(x15), .d(x14), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n79_), .c(new_n89_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n93_));
  nand2  g65(.a(x08), .b(x06), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(new_n93_), .c(new_n87_), .O(z7));
  nand2  g67(.a(x18), .b(x09), .O(new_n96_));
  oai21  g68(.a(new_n96_), .b(x08), .c(new_n29_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n58_), .O(new_n98_));
  nand3  g70(.a(x14), .b(x13), .c(x11), .O(new_n99_));
  nand3  g71(.a(x17), .b(x16), .c(x15), .O(new_n100_));
  oai21  g72(.a(new_n100_), .b(new_n99_), .c(x18), .O(new_n101_));
  inv1   g73(.a(x18), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(x17), .c(x16), .d(x15), .O(new_n103_));
  oai21  g75(.a(new_n103_), .b(new_n70_), .c(new_n101_), .O(new_n104_));
  nand4  g76(.a(new_n104_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n105_));
  nand2  g77(.a(x08), .b(x07), .O(new_n106_));
  nand3  g78(.a(new_n106_), .b(new_n105_), .c(new_n98_), .O(z8));
endmodule


