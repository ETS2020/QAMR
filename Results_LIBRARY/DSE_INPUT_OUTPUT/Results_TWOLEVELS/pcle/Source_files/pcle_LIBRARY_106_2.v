// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x15), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(x14), .O(new_n39_));
  nand2  g11(.a(x08), .b(x00), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(x08), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n41_), .c(new_n31_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n40_), .c(new_n39_), .O(z1));
  inv1   g17(.a(new_n39_), .O(new_n46_));
  xor2a  g18(.a(x12), .b(x11), .O(new_n47_));
  nand4  g19(.a(new_n47_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n48_), .c(new_n46_), .O(z2));
  nand3  g22(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  nor2   g25(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n54_), .O(new_n55_));
  nand2  g27(.a(x12), .b(x11), .O(new_n56_));
  inv1   g28(.a(new_n56_), .O(new_n57_));
  aoi21  g29(.a(new_n38_), .b(x14), .c(new_n57_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(x13), .c(new_n31_), .d(x09), .O(new_n59_));
  oai22  g31(.a(new_n59_), .b(x08), .c(new_n55_), .d(new_n46_), .O(z3));
  nand3  g32(.a(new_n56_), .b(x15), .c(x14), .O(new_n61_));
  inv1   g33(.a(x14), .O(new_n62_));
  nand3  g34(.a(new_n62_), .b(x12), .c(x11), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x13), .O(new_n65_));
  nand3  g37(.a(x15), .b(x14), .c(new_n52_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n65_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n68_));
  aoi21  g40(.a(x08), .b(x03), .c(new_n46_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z4));
  nand3  g42(.a(new_n39_), .b(x08), .c(x04), .O(new_n71_));
  nand3  g43(.a(new_n57_), .b(x14), .c(x13), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(x15), .c(new_n31_), .d(x09), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(x08), .c(new_n71_), .O(z5));
  inv1   g46(.a(x16), .O(new_n75_));
  nand3  g47(.a(new_n57_), .b(new_n75_), .c(x13), .O(new_n76_));
  oai21  g48(.a(new_n76_), .b(new_n51_), .c(x15), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(x14), .O(new_n78_));
  nand3  g50(.a(new_n57_), .b(x14), .c(x13), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(x16), .c(new_n31_), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n81_), .b(x09), .c(new_n30_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x05), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(new_n82_), .c(new_n78_), .O(z6));
  nand3  g56(.a(new_n43_), .b(x11), .c(new_n31_), .O(new_n85_));
  inv1   g57(.a(x17), .O(new_n86_));
  and2   g58(.a(x13), .b(x12), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(x16), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n85_), .c(x15), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(x14), .O(new_n90_));
  nand3  g62(.a(x16), .b(x14), .c(x13), .O(new_n91_));
  inv1   g63(.a(new_n91_), .O(new_n92_));
  aoi21  g64(.a(new_n92_), .b(new_n57_), .c(new_n86_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n94_));
  nand2  g66(.a(x08), .b(x06), .O(new_n95_));
  nand3  g67(.a(new_n95_), .b(new_n94_), .c(new_n90_), .O(z7));
  nand4  g68(.a(new_n87_), .b(new_n29_), .c(x17), .d(x16), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n85_), .c(x15), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(x14), .O(new_n99_));
  nand2  g71(.a(new_n87_), .b(x11), .O(new_n100_));
  nand3  g72(.a(x17), .b(x16), .c(x14), .O(new_n101_));
  nor2   g73(.a(new_n101_), .b(new_n100_), .O(new_n102_));
  nor2   g74(.a(new_n102_), .b(new_n29_), .O(new_n103_));
  nand4  g75(.a(new_n103_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n104_));
  nand2  g76(.a(x08), .b(x07), .O(new_n105_));
  nand3  g77(.a(new_n105_), .b(new_n104_), .c(new_n99_), .O(z8));
endmodule


