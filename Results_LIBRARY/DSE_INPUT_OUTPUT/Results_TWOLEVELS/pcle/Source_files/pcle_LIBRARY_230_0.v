// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_, new_n69_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand2  g01(.a(x13), .b(x12), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x14), .O(new_n32_));
  inv1   g04(.a(x15), .O(new_n33_));
  nand3  g05(.a(x18), .b(x17), .c(x16), .O(new_n34_));
  nor3   g06(.a(new_n34_), .b(new_n33_), .c(new_n32_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n31_), .c(x09), .d(new_n29_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(x11), .c(x10), .O(z0));
  inv1   g09(.a(x10), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n38_), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(new_n40_), .O(z1));
  inv1   g14(.a(x12), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(x09), .c(new_n29_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x11), .O(new_n45_));
  nand2  g17(.a(new_n45_), .b(new_n38_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z2));
  inv1   g20(.a(x02), .O(new_n49_));
  nand2  g21(.a(new_n40_), .b(x08), .O(new_n50_));
  xor2a  g22(.a(x13), .b(x12), .O(new_n51_));
  nand4  g23(.a(new_n51_), .b(x11), .c(new_n38_), .d(x09), .O(new_n52_));
  oai22  g24(.a(new_n52_), .b(x08), .c(new_n50_), .d(new_n49_), .O(z3));
  inv1   g25(.a(x03), .O(new_n54_));
  nand2  g26(.a(new_n30_), .b(x14), .O(new_n55_));
  nand3  g27(.a(new_n32_), .b(x13), .c(x12), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n55_), .c(new_n39_), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(new_n38_), .c(x09), .d(new_n29_), .O(new_n58_));
  oai21  g30(.a(new_n50_), .b(new_n54_), .c(new_n58_), .O(z4));
  inv1   g31(.a(x04), .O(new_n60_));
  nand3  g32(.a(x14), .b(x13), .c(x12), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x15), .O(new_n62_));
  nand3  g34(.a(new_n31_), .b(new_n33_), .c(x14), .O(new_n63_));
  aoi21  g35(.a(new_n63_), .b(new_n62_), .c(new_n39_), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n38_), .c(x09), .d(new_n29_), .O(new_n65_));
  oai21  g37(.a(new_n50_), .b(new_n60_), .c(new_n65_), .O(z5));
  inv1   g38(.a(x16), .O(new_n67_));
  nand2  g39(.a(x14), .b(x13), .O(new_n68_));
  nand2  g40(.a(x15), .b(x12), .O(new_n69_));
  aoi21  g41(.a(new_n68_), .b(x11), .c(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n67_), .b(x15), .c(x14), .O(new_n71_));
  oai22  g43(.a(new_n71_), .b(new_n30_), .c(new_n70_), .d(new_n67_), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(x09), .c(new_n29_), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x11), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(new_n38_), .O(new_n75_));
  nand2  g47(.a(x08), .b(x05), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n75_), .O(z6));
  inv1   g49(.a(x17), .O(new_n78_));
  nand2  g50(.a(new_n68_), .b(x11), .O(new_n79_));
  nand3  g51(.a(x16), .b(x15), .c(x12), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(new_n79_), .c(new_n78_), .O(new_n82_));
  nand3  g54(.a(new_n78_), .b(x16), .c(x15), .O(new_n83_));
  nor2   g55(.a(new_n83_), .b(new_n61_), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n82_), .c(x09), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(x08), .c(x11), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n38_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x06), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n87_), .O(z7));
  inv1   g61(.a(x07), .O(new_n90_));
  nand3  g62(.a(x17), .b(x16), .c(x15), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n61_), .c(x18), .O(new_n92_));
  inv1   g64(.a(x18), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(x17), .c(x16), .d(x15), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n61_), .c(new_n92_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(x11), .c(new_n38_), .d(x09), .O(new_n96_));
  oai22  g68(.a(new_n96_), .b(x08), .c(new_n50_), .d(new_n90_), .O(z8));
endmodule


