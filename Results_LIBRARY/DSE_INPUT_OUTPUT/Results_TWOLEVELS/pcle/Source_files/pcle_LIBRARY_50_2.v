// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  oai21  g09(.a(new_n31_), .b(x00), .c(x08), .O(new_n38_));
  inv1   g10(.a(x11), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n31_), .c(x09), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(new_n38_), .O(z1));
  inv1   g13(.a(x01), .O(new_n42_));
  xor2a  g14(.a(x12), .b(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n44_));
  nand2  g16(.a(x10), .b(x08), .O(new_n45_));
  oai21  g17(.a(new_n45_), .b(new_n42_), .c(new_n44_), .O(z2));
  inv1   g18(.a(x02), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  inv1   g21(.a(new_n49_), .O(new_n50_));
  nand3  g22(.a(new_n48_), .b(x12), .c(x11), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(new_n48_), .c(new_n51_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  oai21  g25(.a(new_n45_), .b(new_n47_), .c(new_n53_), .O(z3));
  oai21  g26(.a(new_n31_), .b(x03), .c(x08), .O(new_n55_));
  oai21  g27(.a(x12), .b(x08), .c(x13), .O(new_n56_));
  oai21  g28(.a(new_n56_), .b(new_n39_), .c(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(new_n50_), .b(new_n58_), .c(x13), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n57_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n31_), .c(x09), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n55_), .O(z4));
  oai21  g34(.a(new_n31_), .b(x04), .c(x08), .O(new_n63_));
  oai21  g35(.a(x12), .b(x08), .c(x11), .O(new_n64_));
  nor2   g36(.a(new_n58_), .b(new_n48_), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n64_), .c(x15), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand4  g40(.a(new_n50_), .b(new_n68_), .c(x14), .d(x13), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n67_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(new_n31_), .c(x09), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n63_), .O(z5));
  inv1   g44(.a(x05), .O(new_n73_));
  nand3  g45(.a(x15), .b(x14), .c(x13), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n49_), .c(x16), .O(new_n75_));
  nand3  g47(.a(x13), .b(x12), .c(x11), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n76_), .c(new_n75_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n80_));
  oai21  g52(.a(new_n45_), .b(new_n73_), .c(new_n80_), .O(z6));
  oai21  g53(.a(new_n31_), .b(x06), .c(x08), .O(new_n82_));
  inv1   g54(.a(new_n76_), .O(new_n83_));
  nand2  g55(.a(new_n68_), .b(new_n30_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n83_), .c(x16), .d(x14), .O(new_n85_));
  nand2  g57(.a(new_n85_), .b(x17), .O(new_n86_));
  inv1   g58(.a(x17), .O(new_n87_));
  nor2   g59(.a(new_n68_), .b(new_n58_), .O(new_n88_));
  nand4  g60(.a(new_n83_), .b(new_n88_), .c(new_n87_), .d(x16), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n86_), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n31_), .c(x09), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n82_), .O(z7));
  oai21  g64(.a(new_n31_), .b(x07), .c(x08), .O(new_n93_));
  nor2   g65(.a(new_n87_), .b(new_n77_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n84_), .c(new_n83_), .d(x14), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(x18), .O(new_n96_));
  nor2   g68(.a(new_n77_), .b(new_n68_), .O(new_n97_));
  nor2   g69(.a(x18), .b(new_n87_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(new_n97_), .c(new_n65_), .d(new_n50_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n96_), .O(new_n100_));
  nand3  g72(.a(new_n100_), .b(new_n31_), .c(x09), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n93_), .O(z8));
endmodule


