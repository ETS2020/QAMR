// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:59 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(x09), .c(new_n31_), .d(x04), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x13), .c(x12), .d(x11), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x17), .c(x16), .d(x15), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n29_), .O(z0));
  inv1   g10(.a(x04), .O(new_n39_));
  aoi21  g11(.a(x09), .b(new_n39_), .c(new_n31_), .O(new_n40_));
  nand2  g12(.a(new_n40_), .b(x00), .O(new_n41_));
  nor2   g13(.a(x11), .b(x10), .O(new_n42_));
  nand4  g14(.a(new_n42_), .b(x09), .c(new_n31_), .d(x04), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(new_n41_), .O(z1));
  nand2  g16(.a(new_n40_), .b(x01), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n47_));
  oai21  g19(.a(new_n47_), .b(new_n39_), .c(new_n45_), .O(z2));
  inv1   g20(.a(x09), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x13), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n51_), .c(x10), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n31_), .c(new_n39_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x02), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(new_n49_), .c(new_n56_), .O(z3));
  nand2  g29(.a(new_n40_), .b(x03), .O(new_n58_));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(x14), .O(new_n60_));
  inv1   g32(.a(new_n50_), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n30_), .c(x13), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n60_), .c(x10), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(x09), .c(new_n31_), .d(x04), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n58_), .O(z4));
  inv1   g37(.a(x15), .O(new_n66_));
  nand2  g38(.a(x13), .b(x12), .O(new_n67_));
  nand2  g39(.a(x14), .b(x11), .O(new_n68_));
  aoi21  g40(.a(new_n67_), .b(x04), .c(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n66_), .b(x14), .c(x13), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n50_), .c(new_n69_), .d(new_n66_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n32_), .c(new_n39_), .O(new_n72_));
  oai22  g44(.a(new_n72_), .b(new_n49_), .c(new_n31_), .d(new_n39_), .O(z5));
  nand2  g45(.a(new_n40_), .b(x05), .O(new_n74_));
  nand3  g46(.a(x15), .b(x14), .c(x13), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n50_), .c(x16), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(x15), .c(x14), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n59_), .c(new_n76_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n39_), .c(new_n74_), .O(z6));
  inv1   g53(.a(new_n59_), .O(new_n82_));
  nand2  g54(.a(new_n66_), .b(x04), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n82_), .c(x16), .d(x14), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x17), .O(new_n85_));
  nor2   g57(.a(new_n66_), .b(new_n30_), .O(new_n86_));
  nor2   g58(.a(x17), .b(new_n77_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n82_), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(new_n85_), .c(x10), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n31_), .c(new_n39_), .O(new_n90_));
  nand2  g62(.a(x08), .b(x06), .O(new_n91_));
  oai21  g63(.a(new_n90_), .b(new_n49_), .c(new_n91_), .O(z7));
  nand2  g64(.a(new_n40_), .b(x07), .O(new_n93_));
  nand3  g65(.a(new_n86_), .b(x17), .c(x16), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n59_), .c(x18), .O(new_n95_));
  nand4  g67(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n96_));
  inv1   g68(.a(new_n96_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n61_), .c(x14), .d(x13), .O(new_n98_));
  aoi21  g70(.a(new_n98_), .b(new_n95_), .c(x10), .O(new_n99_));
  nand4  g71(.a(new_n99_), .b(x09), .c(new_n31_), .d(x04), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n93_), .O(z8));
endmodule


