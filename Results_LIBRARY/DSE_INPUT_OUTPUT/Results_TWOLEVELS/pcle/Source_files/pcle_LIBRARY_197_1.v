// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x17), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x12), .O(new_n34_));
  inv1   g06(.a(x13), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  inv1   g08(.a(x16), .O(new_n37_));
  inv1   g09(.a(x18), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n37_), .O(new_n39_));
  nand4  g11(.a(new_n39_), .b(new_n36_), .c(new_n33_), .d(x14), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(x15), .c(new_n29_), .O(z0));
  nor2   g13(.a(new_n29_), .b(x15), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  inv1   g15(.a(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n43_), .c(new_n42_), .O(z1));
  inv1   g18(.a(new_n42_), .O(new_n47_));
  xor2a  g19(.a(x12), .b(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(new_n49_), .c(new_n47_), .O(z2));
  nand2  g23(.a(x12), .b(x11), .O(new_n52_));
  inv1   g24(.a(new_n52_), .O(new_n53_));
  nand3  g25(.a(new_n35_), .b(x12), .c(x11), .O(new_n54_));
  oai21  g26(.a(new_n53_), .b(new_n35_), .c(new_n54_), .O(new_n55_));
  nand4  g27(.a(new_n55_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n56_));
  nand2  g28(.a(x08), .b(x02), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n56_), .c(new_n42_), .O(z3));
  nand3  g30(.a(new_n31_), .b(x09), .c(new_n30_), .O(new_n59_));
  nor4   g31(.a(new_n59_), .b(new_n52_), .c(x14), .d(new_n35_), .O(new_n60_));
  aoi21  g32(.a(x08), .b(x03), .c(new_n60_), .O(new_n61_));
  inv1   g33(.a(x15), .O(new_n62_));
  nand2  g34(.a(x17), .b(new_n62_), .O(new_n63_));
  nand3  g35(.a(x13), .b(x12), .c(x11), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(new_n63_), .c(x14), .d(new_n31_), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(x09), .c(new_n30_), .O(new_n67_));
  oai21  g39(.a(new_n61_), .b(new_n42_), .c(new_n67_), .O(z4));
  xnor2a g40(.a(x15), .b(x14), .O(new_n69_));
  nand2  g41(.a(x15), .b(new_n35_), .O(new_n70_));
  oai21  g42(.a(new_n69_), .b(new_n35_), .c(new_n70_), .O(new_n71_));
  nand3  g43(.a(new_n71_), .b(x12), .c(x11), .O(new_n72_));
  oai21  g44(.a(new_n53_), .b(new_n62_), .c(new_n72_), .O(new_n73_));
  nand4  g45(.a(new_n73_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n74_));
  aoi21  g46(.a(x08), .b(x04), .c(new_n42_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n74_), .O(z5));
  nand3  g48(.a(new_n47_), .b(x08), .c(x05), .O(new_n77_));
  nand3  g49(.a(new_n53_), .b(x14), .c(x13), .O(new_n78_));
  nand3  g50(.a(new_n37_), .b(x14), .c(x13), .O(new_n79_));
  nor2   g51(.a(new_n79_), .b(new_n52_), .O(new_n80_));
  aoi21  g52(.a(new_n78_), .b(x16), .c(new_n80_), .O(new_n81_));
  nand3  g53(.a(new_n29_), .b(x16), .c(new_n62_), .O(new_n82_));
  oai21  g54(.a(new_n81_), .b(new_n62_), .c(new_n82_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n77_), .O(z6));
  nand2  g57(.a(new_n64_), .b(x17), .O(new_n86_));
  nand3  g58(.a(new_n29_), .b(x16), .c(x13), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n52_), .c(new_n86_), .O(new_n88_));
  nor2   g60(.a(new_n29_), .b(x16), .O(new_n89_));
  aoi21  g61(.a(new_n88_), .b(x14), .c(new_n89_), .O(new_n90_));
  inv1   g62(.a(x14), .O(new_n91_));
  nand2  g63(.a(x17), .b(new_n91_), .O(new_n92_));
  oai21  g64(.a(new_n90_), .b(new_n62_), .c(new_n92_), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n94_));
  aoi21  g66(.a(x08), .b(x06), .c(new_n42_), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(new_n94_), .O(z7));
  nand4  g68(.a(new_n36_), .b(new_n38_), .c(x16), .d(x14), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n32_), .c(x15), .O(new_n98_));
  nand2  g70(.a(new_n98_), .b(x17), .O(new_n99_));
  nor4   g71(.a(new_n64_), .b(new_n29_), .c(new_n37_), .d(new_n91_), .O(new_n100_));
  nor2   g72(.a(new_n100_), .b(new_n38_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n102_));
  nand2  g74(.a(x08), .b(x07), .O(new_n103_));
  nand3  g75(.a(new_n103_), .b(new_n102_), .c(new_n99_), .O(z8));
endmodule


