// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_, new_n61_,
    new_n62_, new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n71_;
  inv1   g00(.a(x08), .O(new_n30_));
  inv1   g01(.a(x10), .O(new_n31_));
  aoi21  g02(.a(x14), .b(new_n31_), .c(new_n30_), .O(new_n32_));
  nand2  g03(.a(new_n32_), .b(x00), .O(new_n33_));
  inv1   g04(.a(x09), .O(new_n34_));
  nor2   g05(.a(new_n34_), .b(x08), .O(new_n35_));
  nor3   g06(.a(x14), .b(x11), .c(x10), .O(new_n36_));
  nand2  g07(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand2  g08(.a(new_n37_), .b(new_n33_), .O(z1));
  inv1   g09(.a(x14), .O(new_n39_));
  xor2a  g10(.a(x12), .b(x11), .O(new_n40_));
  nand3  g11(.a(new_n40_), .b(x09), .c(new_n30_), .O(new_n41_));
  nand2  g12(.a(new_n41_), .b(new_n39_), .O(new_n42_));
  nand2  g13(.a(new_n42_), .b(new_n31_), .O(new_n43_));
  nand2  g14(.a(x08), .b(x01), .O(new_n44_));
  nand2  g15(.a(new_n44_), .b(new_n43_), .O(z2));
  nand2  g16(.a(new_n32_), .b(x02), .O(new_n46_));
  nand2  g17(.a(x12), .b(x11), .O(new_n47_));
  nand2  g18(.a(new_n47_), .b(x13), .O(new_n48_));
  inv1   g19(.a(x13), .O(new_n49_));
  nand3  g20(.a(new_n49_), .b(x12), .c(x11), .O(new_n50_));
  aoi21  g21(.a(new_n50_), .b(new_n48_), .c(x14), .O(new_n51_));
  nand4  g22(.a(new_n51_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n52_));
  nand2  g23(.a(new_n52_), .b(new_n46_), .O(z3));
  nand3  g24(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  inv1   g25(.a(new_n54_), .O(new_n55_));
  aoi21  g26(.a(new_n55_), .b(new_n35_), .c(x14), .O(new_n56_));
  nand2  g27(.a(x08), .b(x03), .O(new_n57_));
  oai21  g28(.a(new_n56_), .b(x10), .c(new_n57_), .O(z4));
  nand2  g29(.a(x15), .b(x09), .O(new_n59_));
  oai21  g30(.a(new_n59_), .b(x08), .c(new_n39_), .O(new_n60_));
  nand2  g31(.a(new_n60_), .b(new_n31_), .O(new_n61_));
  nand2  g32(.a(x08), .b(x04), .O(new_n62_));
  nand2  g33(.a(new_n62_), .b(new_n61_), .O(z5));
  nand2  g34(.a(new_n32_), .b(x05), .O(new_n64_));
  nand4  g35(.a(new_n35_), .b(x16), .c(new_n39_), .d(new_n31_), .O(new_n65_));
  nand2  g36(.a(new_n65_), .b(new_n64_), .O(z6));
  nand2  g37(.a(new_n32_), .b(x06), .O(new_n67_));
  nand4  g38(.a(new_n35_), .b(x17), .c(new_n39_), .d(new_n31_), .O(new_n68_));
  nand2  g39(.a(new_n68_), .b(new_n67_), .O(z7));
  nand2  g40(.a(new_n32_), .b(x07), .O(new_n70_));
  nand4  g41(.a(new_n35_), .b(x18), .c(new_n39_), .d(new_n31_), .O(new_n71_));
  nand2  g42(.a(new_n71_), .b(new_n70_), .O(z8));
  zero   g43(.O(z0));
endmodule


