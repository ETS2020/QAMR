// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:42 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand2  g01(.a(x09), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  nand2  g03(.a(x13), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  and2   g05(.a(x15), .b(x14), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  inv1   g07(.a(new_n35_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n34_), .c(new_n33_), .d(new_n31_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(x12), .c(x10), .O(z0));
  inv1   g10(.a(x00), .O(new_n39_));
  oai21  g11(.a(x12), .b(x10), .c(x08), .O(new_n40_));
  inv1   g12(.a(x10), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(x12), .b(new_n42_), .c(new_n41_), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(new_n30_), .c(new_n40_), .d(new_n39_), .O(z1));
  inv1   g16(.a(x12), .O(new_n45_));
  aoi21  g17(.a(new_n31_), .b(new_n42_), .c(new_n45_), .O(new_n46_));
  nand2  g18(.a(x08), .b(x01), .O(new_n47_));
  oai21  g19(.a(new_n46_), .b(x10), .c(new_n47_), .O(z2));
  xor2a  g20(.a(x13), .b(x11), .O(new_n49_));
  nand3  g21(.a(new_n49_), .b(x09), .c(new_n29_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x12), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n41_), .O(new_n52_));
  nand2  g24(.a(x08), .b(x02), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n52_), .O(z3));
  inv1   g26(.a(x03), .O(new_n55_));
  nand2  g27(.a(new_n32_), .b(x14), .O(new_n56_));
  inv1   g28(.a(x14), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(x13), .c(x11), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n56_), .c(new_n45_), .O(new_n59_));
  nand4  g31(.a(new_n59_), .b(new_n41_), .c(x09), .d(new_n29_), .O(new_n60_));
  oai21  g32(.a(new_n40_), .b(new_n55_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x09), .O(new_n62_));
  inv1   g34(.a(x13), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(x12), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x14), .c(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x15), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand3  g39(.a(new_n33_), .b(new_n67_), .c(x14), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n66_), .c(new_n62_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n29_), .c(new_n45_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x04), .O(new_n71_));
  oai21  g43(.a(new_n70_), .b(x10), .c(new_n71_), .O(z5));
  nand3  g44(.a(new_n64_), .b(new_n34_), .c(x11), .O(new_n73_));
  nand2  g45(.a(new_n73_), .b(x16), .O(new_n74_));
  inv1   g46(.a(x16), .O(new_n75_));
  nand4  g47(.a(new_n33_), .b(new_n75_), .c(x15), .d(x14), .O(new_n76_));
  aoi21  g48(.a(new_n76_), .b(new_n74_), .c(new_n62_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n29_), .c(new_n45_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x05), .O(new_n79_));
  oai21  g51(.a(new_n78_), .b(x10), .c(new_n79_), .O(z6));
  inv1   g52(.a(x06), .O(new_n81_));
  nand3  g53(.a(x16), .b(x15), .c(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n32_), .c(x17), .O(new_n83_));
  nand3  g55(.a(x14), .b(x13), .c(x11), .O(new_n84_));
  inv1   g56(.a(x17), .O(new_n85_));
  nand3  g57(.a(new_n85_), .b(x16), .c(x15), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n84_), .c(new_n83_), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(x12), .c(new_n41_), .d(x09), .O(new_n88_));
  oai22  g60(.a(new_n88_), .b(x08), .c(new_n40_), .d(new_n81_), .O(z7));
  inv1   g61(.a(x07), .O(new_n90_));
  nand3  g62(.a(x17), .b(x16), .c(x15), .O(new_n91_));
  oai21  g63(.a(new_n91_), .b(new_n84_), .c(x18), .O(new_n92_));
  inv1   g64(.a(x18), .O(new_n93_));
  nand4  g65(.a(new_n93_), .b(x17), .c(x16), .d(x15), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n84_), .c(new_n92_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(x12), .c(new_n41_), .d(x09), .O(new_n96_));
  oai22  g68(.a(new_n96_), .b(x08), .c(new_n40_), .d(new_n90_), .O(z8));
endmodule


