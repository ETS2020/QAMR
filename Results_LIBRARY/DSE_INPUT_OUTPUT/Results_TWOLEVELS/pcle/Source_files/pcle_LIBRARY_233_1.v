// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:15 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_;
  inv1   g00(.a(x10), .O(new_n29_));
  nand2  g01(.a(x18), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(z0));
  inv1   g03(.a(x18), .O(new_n32_));
  inv1   g04(.a(x08), .O(new_n33_));
  inv1   g05(.a(x11), .O(new_n34_));
  nand3  g06(.a(new_n34_), .b(x09), .c(new_n33_), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n32_), .O(new_n36_));
  nand2  g08(.a(new_n36_), .b(new_n29_), .O(new_n37_));
  nand2  g09(.a(x08), .b(x00), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n37_), .O(z1));
  inv1   g11(.a(x01), .O(new_n40_));
  nand2  g12(.a(new_n30_), .b(x08), .O(new_n41_));
  xor2a  g13(.a(x12), .b(x11), .O(new_n42_));
  nand4  g14(.a(new_n42_), .b(new_n32_), .c(new_n29_), .d(x09), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(x08), .c(new_n41_), .d(new_n40_), .O(z2));
  inv1   g16(.a(x02), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x13), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(x12), .c(x11), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n47_), .c(x18), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n29_), .c(x09), .d(new_n33_), .O(new_n51_));
  oai21  g23(.a(new_n41_), .b(new_n45_), .c(new_n51_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  nand3  g25(.a(x13), .b(x12), .c(x11), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x14), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  inv1   g28(.a(new_n46_), .O(new_n57_));
  nand3  g29(.a(new_n57_), .b(new_n56_), .c(x13), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n55_), .c(x18), .O(new_n59_));
  nand4  g31(.a(new_n59_), .b(new_n29_), .c(x09), .d(new_n33_), .O(new_n60_));
  oai21  g32(.a(new_n41_), .b(new_n53_), .c(new_n60_), .O(z4));
  inv1   g33(.a(x04), .O(new_n62_));
  nand2  g34(.a(x14), .b(x13), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n46_), .c(x15), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x14), .c(x13), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n46_), .c(new_n64_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n32_), .c(new_n29_), .d(x09), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(x08), .c(new_n41_), .d(new_n62_), .O(z5));
  inv1   g41(.a(x16), .O(new_n70_));
  nand3  g42(.a(x14), .b(x13), .c(x12), .O(new_n71_));
  nand2  g43(.a(x15), .b(x11), .O(new_n72_));
  aoi21  g44(.a(new_n71_), .b(new_n32_), .c(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n70_), .b(x15), .c(x14), .O(new_n74_));
  oai22  g46(.a(new_n74_), .b(new_n54_), .c(new_n73_), .d(new_n70_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(x09), .c(new_n33_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n32_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n29_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x05), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z6));
  inv1   g52(.a(x06), .O(new_n81_));
  nand3  g53(.a(x16), .b(x15), .c(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n54_), .c(x17), .O(new_n83_));
  inv1   g55(.a(x17), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(x16), .c(x15), .d(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n54_), .c(new_n83_), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(new_n32_), .c(new_n29_), .d(x09), .O(new_n87_));
  oai22  g59(.a(new_n87_), .b(x08), .c(new_n41_), .d(new_n81_), .O(z7));
  inv1   g60(.a(x07), .O(new_n89_));
  nand4  g61(.a(new_n57_), .b(new_n29_), .c(x09), .d(new_n33_), .O(new_n90_));
  nand3  g62(.a(x15), .b(x14), .c(x13), .O(new_n91_));
  inv1   g63(.a(new_n91_), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(new_n32_), .c(x17), .d(x16), .O(new_n93_));
  oai22  g65(.a(new_n93_), .b(new_n90_), .c(new_n41_), .d(new_n89_), .O(z8));
endmodule


