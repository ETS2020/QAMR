// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:05 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_;
  inv1   g00(.a(x08), .O(new_n29_));
  nand2  g01(.a(x12), .b(x11), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(x10), .O(new_n31_));
  nand3  g03(.a(x15), .b(x14), .c(x13), .O(new_n32_));
  nand3  g04(.a(x18), .b(x17), .c(x16), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(new_n31_), .c(x09), .d(new_n29_), .O(new_n35_));
  oai21  g07(.a(x18), .b(new_n29_), .c(new_n35_), .O(z0));
  inv1   g08(.a(x00), .O(new_n37_));
  nand2  g09(.a(x18), .b(x08), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n41_));
  oai21  g13(.a(new_n38_), .b(new_n37_), .c(new_n41_), .O(z1));
  inv1   g14(.a(x18), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(x01), .c(x08), .O(new_n44_));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n44_), .O(z2));
  inv1   g19(.a(x02), .O(new_n48_));
  nand2  g20(.a(new_n30_), .b(x13), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n53_));
  oai21  g25(.a(new_n38_), .b(new_n48_), .c(new_n53_), .O(z3));
  inv1   g26(.a(x03), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x13), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(new_n30_), .c(new_n57_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n61_));
  oai21  g33(.a(new_n38_), .b(new_n55_), .c(new_n61_), .O(z4));
  oai21  g34(.a(new_n43_), .b(x04), .c(x08), .O(new_n63_));
  nand4  g35(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x15), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(x14), .c(x13), .O(new_n67_));
  oai21  g39(.a(new_n67_), .b(new_n30_), .c(new_n65_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n63_), .O(z5));
  oai21  g42(.a(new_n43_), .b(x05), .c(x08), .O(new_n71_));
  oai21  g43(.a(new_n32_), .b(new_n30_), .c(x16), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nand3  g45(.a(new_n73_), .b(x15), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n56_), .c(new_n72_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n71_), .O(z6));
  oai21  g49(.a(new_n43_), .b(x06), .c(x08), .O(new_n78_));
  nand3  g50(.a(x16), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n56_), .c(x17), .O(new_n80_));
  inv1   g52(.a(x17), .O(new_n81_));
  nand4  g53(.a(new_n81_), .b(x16), .c(x15), .d(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n56_), .c(new_n80_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n39_), .c(x09), .d(new_n29_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(new_n78_), .O(z7));
  oai21  g57(.a(new_n43_), .b(x07), .c(x08), .O(new_n86_));
  nand4  g58(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n56_), .c(x18), .O(new_n88_));
  nand4  g60(.a(new_n43_), .b(x17), .c(x16), .d(x15), .O(new_n89_));
  oai22  g61(.a(new_n89_), .b(new_n64_), .c(new_n88_), .d(x08), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n39_), .c(x09), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(new_n86_), .O(z8));
endmodule


