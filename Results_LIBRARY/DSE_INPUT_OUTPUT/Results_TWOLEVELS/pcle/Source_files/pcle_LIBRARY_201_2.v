// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:10 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_,
    new_n106_, new_n107_;
  inv1   g00(.a(x10), .O(new_n29_));
  nor2   g01(.a(x12), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  nand2  g04(.a(x12), .b(x11), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x10), .O(new_n34_));
  inv1   g06(.a(x13), .O(new_n35_));
  nand2  g07(.a(x15), .b(x14), .O(new_n36_));
  nand3  g08(.a(x18), .b(x17), .c(x16), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n34_), .c(x09), .d(new_n32_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n31_), .O(z0));
  nor2   g12(.a(new_n30_), .b(new_n32_), .O(new_n41_));
  nand2  g13(.a(new_n41_), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n42_), .O(z1));
  nand3  g17(.a(x12), .b(new_n43_), .c(new_n29_), .O(new_n46_));
  oai21  g18(.a(x12), .b(new_n43_), .c(new_n46_), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x09), .c(new_n32_), .O(new_n48_));
  aoi21  g20(.a(x08), .b(x01), .c(new_n30_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z2));
  nand2  g22(.a(x13), .b(new_n43_), .O(new_n51_));
  nand3  g23(.a(new_n35_), .b(x12), .c(x11), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n51_), .c(x10), .O(new_n53_));
  nor2   g25(.a(new_n35_), .b(x12), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n53_), .c(x09), .O(new_n55_));
  aoi21  g27(.a(x08), .b(x02), .c(new_n30_), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(x08), .c(new_n56_), .O(z3));
  inv1   g29(.a(x12), .O(new_n58_));
  nand2  g30(.a(x14), .b(x09), .O(new_n59_));
  oai21  g31(.a(new_n59_), .b(x08), .c(new_n29_), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nand2  g33(.a(x13), .b(x11), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(x14), .O(new_n63_));
  inv1   g35(.a(x14), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(x13), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n33_), .c(new_n63_), .O(new_n66_));
  nand4  g38(.a(new_n66_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n67_));
  nand2  g39(.a(x08), .b(x03), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(new_n67_), .c(new_n61_), .O(z4));
  nand2  g41(.a(new_n41_), .b(x04), .O(new_n70_));
  inv1   g42(.a(x15), .O(new_n71_));
  nand4  g43(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n72_));
  inv1   g44(.a(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n71_), .b(x14), .c(x13), .O(new_n74_));
  oai22  g46(.a(new_n74_), .b(new_n33_), .c(new_n73_), .d(new_n71_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(new_n70_), .O(z5));
  nand2  g49(.a(x16), .b(x09), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(x08), .c(new_n29_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n58_), .O(new_n80_));
  oai21  g52(.a(new_n62_), .b(new_n36_), .c(x16), .O(new_n81_));
  nand3  g53(.a(x13), .b(x12), .c(x11), .O(new_n82_));
  inv1   g54(.a(x16), .O(new_n83_));
  nand3  g55(.a(new_n83_), .b(x15), .c(x14), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n82_), .c(new_n81_), .O(new_n85_));
  nand4  g57(.a(new_n85_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n86_));
  nand2  g58(.a(x08), .b(x05), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(new_n86_), .c(new_n80_), .O(z6));
  nand2  g60(.a(x17), .b(x09), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(x08), .c(new_n29_), .O(new_n90_));
  nand2  g62(.a(new_n90_), .b(new_n58_), .O(new_n91_));
  nand3  g63(.a(x16), .b(x15), .c(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n62_), .c(x17), .O(new_n93_));
  inv1   g65(.a(x17), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(x16), .c(x15), .d(x14), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n82_), .c(new_n93_), .O(new_n96_));
  nand4  g68(.a(new_n96_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n97_));
  nand2  g69(.a(x08), .b(x06), .O(new_n98_));
  nand3  g70(.a(new_n98_), .b(new_n97_), .c(new_n91_), .O(z7));
  nand2  g71(.a(new_n41_), .b(x07), .O(new_n100_));
  nand4  g72(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n101_));
  oai21  g73(.a(new_n101_), .b(new_n82_), .c(x18), .O(new_n102_));
  nor2   g74(.a(new_n83_), .b(new_n71_), .O(new_n103_));
  nor2   g75(.a(x18), .b(new_n94_), .O(new_n104_));
  nand3  g76(.a(new_n104_), .b(new_n103_), .c(new_n73_), .O(new_n105_));
  nand2  g77(.a(new_n105_), .b(new_n102_), .O(new_n106_));
  nand4  g78(.a(new_n106_), .b(new_n29_), .c(x09), .d(new_n32_), .O(new_n107_));
  nand2  g79(.a(new_n107_), .b(new_n100_), .O(z8));
endmodule


