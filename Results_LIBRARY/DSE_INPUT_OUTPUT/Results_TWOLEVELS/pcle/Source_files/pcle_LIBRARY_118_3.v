// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:56 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_, new_n44_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n69_, new_n70_,
    new_n72_, new_n73_;
  inv1   g00(.a(x11), .O(new_n29_));
  inv1   g01(.a(x15), .O(new_n30_));
  nor2   g02(.a(new_n30_), .b(new_n29_), .O(z0));
  inv1   g03(.a(x08), .O(new_n32_));
  nor2   g04(.a(z0), .b(new_n32_), .O(new_n33_));
  nand2  g05(.a(new_n33_), .b(x00), .O(new_n34_));
  inv1   g06(.a(x10), .O(new_n35_));
  inv1   g07(.a(x09), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(x08), .O(new_n37_));
  nand3  g09(.a(new_n37_), .b(new_n29_), .c(new_n35_), .O(new_n38_));
  nand2  g10(.a(new_n38_), .b(new_n34_), .O(z1));
  nand2  g11(.a(new_n33_), .b(x01), .O(new_n40_));
  inv1   g12(.a(x12), .O(new_n41_));
  nand3  g13(.a(new_n30_), .b(new_n41_), .c(x11), .O(new_n42_));
  oai21  g14(.a(new_n41_), .b(x11), .c(new_n42_), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n35_), .c(x09), .d(new_n32_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(new_n40_), .O(z2));
  nand2  g17(.a(new_n33_), .b(x02), .O(new_n46_));
  inv1   g18(.a(x13), .O(new_n47_));
  aoi21  g19(.a(new_n30_), .b(new_n41_), .c(new_n29_), .O(new_n48_));
  nand4  g20(.a(new_n30_), .b(new_n47_), .c(x12), .d(x11), .O(new_n49_));
  oai21  g21(.a(new_n48_), .b(new_n47_), .c(new_n49_), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n35_), .c(x09), .d(new_n32_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n46_), .O(z3));
  nand2  g24(.a(new_n33_), .b(x03), .O(new_n53_));
  aoi21  g25(.a(x13), .b(x12), .c(x15), .O(new_n54_));
  oai21  g26(.a(new_n54_), .b(new_n29_), .c(x14), .O(new_n55_));
  nor2   g27(.a(x15), .b(x14), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(x13), .c(x12), .d(x11), .O(new_n57_));
  nand2  g29(.a(new_n57_), .b(new_n55_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n35_), .c(x09), .d(new_n32_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n53_), .O(z4));
  nand4  g32(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n30_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n35_), .c(x09), .d(new_n32_), .O(new_n63_));
  aoi21  g35(.a(x08), .b(x04), .c(z0), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n63_), .O(z5));
  nand2  g37(.a(x08), .b(x05), .O(new_n66_));
  nand3  g38(.a(new_n37_), .b(x16), .c(new_n35_), .O(new_n67_));
  aoi21  g39(.a(new_n67_), .b(new_n66_), .c(z0), .O(z6));
  nand2  g40(.a(x08), .b(x06), .O(new_n69_));
  nand3  g41(.a(new_n37_), .b(x17), .c(new_n35_), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n69_), .c(z0), .O(z7));
  nand2  g43(.a(x08), .b(x07), .O(new_n72_));
  nand3  g44(.a(new_n37_), .b(x18), .c(new_n35_), .O(new_n73_));
  aoi21  g45(.a(new_n73_), .b(new_n72_), .c(z0), .O(z8));
endmodule


