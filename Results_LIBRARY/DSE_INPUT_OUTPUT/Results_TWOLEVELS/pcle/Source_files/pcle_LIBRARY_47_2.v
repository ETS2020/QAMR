// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:44 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x06), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  nand3  g02(.a(x13), .b(x12), .c(x11), .O(new_n31_));
  inv1   g03(.a(new_n31_), .O(new_n32_));
  nand2  g04(.a(x15), .b(x14), .O(new_n33_));
  nand3  g05(.a(x18), .b(x17), .c(x16), .O(new_n34_));
  nor2   g06(.a(new_n34_), .b(new_n33_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n32_), .c(x09), .d(new_n30_), .O(new_n36_));
  aoi21  g08(.a(new_n36_), .b(new_n29_), .c(x10), .O(z0));
  inv1   g09(.a(x00), .O(new_n38_));
  oai21  g10(.a(x10), .b(new_n29_), .c(x08), .O(new_n39_));
  nor2   g11(.a(x11), .b(x10), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(x09), .c(new_n30_), .d(new_n29_), .O(new_n41_));
  oai21  g13(.a(new_n39_), .b(new_n38_), .c(new_n41_), .O(z1));
  inv1   g14(.a(x01), .O(new_n43_));
  inv1   g15(.a(x10), .O(new_n44_));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n44_), .c(x09), .d(new_n30_), .O(new_n46_));
  oai22  g18(.a(new_n46_), .b(x06), .c(new_n39_), .d(new_n43_), .O(z2));
  inv1   g19(.a(x09), .O(new_n48_));
  nand2  g20(.a(x12), .b(x11), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(x13), .O(new_n50_));
  inv1   g22(.a(x13), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(x12), .c(x11), .O(new_n52_));
  aoi21  g24(.a(new_n52_), .b(new_n50_), .c(new_n48_), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n30_), .c(x06), .O(new_n54_));
  nand2  g26(.a(x08), .b(x02), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(x10), .c(new_n55_), .O(z3));
  nand2  g28(.a(new_n31_), .b(x14), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  inv1   g30(.a(new_n49_), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(new_n58_), .c(x13), .O(new_n60_));
  aoi21  g32(.a(new_n60_), .b(new_n57_), .c(new_n48_), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n30_), .c(x06), .O(new_n62_));
  nand2  g34(.a(x08), .b(x03), .O(new_n63_));
  oai21  g35(.a(new_n62_), .b(x10), .c(new_n63_), .O(z4));
  inv1   g36(.a(x04), .O(new_n65_));
  inv1   g37(.a(x15), .O(new_n66_));
  nand4  g38(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n67_));
  inv1   g39(.a(new_n67_), .O(new_n68_));
  nand3  g40(.a(new_n66_), .b(x14), .c(x13), .O(new_n69_));
  oai22  g41(.a(new_n69_), .b(new_n49_), .c(new_n68_), .d(new_n66_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n44_), .c(x09), .d(new_n30_), .O(new_n71_));
  oai22  g43(.a(new_n71_), .b(x06), .c(new_n39_), .d(new_n65_), .O(z5));
  inv1   g44(.a(x05), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  nor3   g46(.a(new_n49_), .b(new_n33_), .c(new_n51_), .O(new_n75_));
  nand3  g47(.a(new_n74_), .b(x15), .c(x14), .O(new_n76_));
  oai22  g48(.a(new_n76_), .b(new_n31_), .c(new_n75_), .d(new_n74_), .O(new_n77_));
  nand4  g49(.a(new_n77_), .b(new_n44_), .c(x09), .d(new_n30_), .O(new_n78_));
  oai22  g50(.a(new_n78_), .b(x06), .c(new_n39_), .d(new_n73_), .O(z6));
  oai21  g51(.a(new_n44_), .b(x08), .c(x06), .O(new_n80_));
  nand2  g52(.a(new_n33_), .b(new_n29_), .O(new_n81_));
  inv1   g53(.a(new_n81_), .O(new_n82_));
  nand3  g54(.a(new_n59_), .b(x16), .c(x13), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n82_), .c(x17), .O(new_n84_));
  inv1   g56(.a(x17), .O(new_n85_));
  inv1   g57(.a(new_n33_), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(new_n32_), .c(new_n85_), .d(x16), .O(new_n87_));
  nand2  g59(.a(new_n87_), .b(new_n84_), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(new_n44_), .c(x09), .d(new_n30_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n80_), .O(z7));
  nand3  g62(.a(x17), .b(x16), .c(x13), .O(new_n91_));
  inv1   g63(.a(new_n91_), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n81_), .c(new_n59_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(x18), .O(new_n94_));
  nor2   g66(.a(x18), .b(new_n85_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n68_), .c(x16), .d(x15), .O(new_n96_));
  aoi21  g68(.a(new_n96_), .b(new_n94_), .c(new_n48_), .O(new_n97_));
  aoi21  g69(.a(new_n97_), .b(new_n30_), .c(x06), .O(new_n98_));
  nand2  g70(.a(x08), .b(x07), .O(new_n99_));
  oai21  g71(.a(new_n98_), .b(x10), .c(new_n99_), .O(z8));
endmodule


