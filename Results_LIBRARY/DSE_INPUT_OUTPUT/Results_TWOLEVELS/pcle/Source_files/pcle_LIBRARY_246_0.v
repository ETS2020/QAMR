// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:17 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_;
  inv1   g00(.a(x11), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x08), .O(new_n30_));
  inv1   g02(.a(x12), .O(new_n31_));
  inv1   g03(.a(x13), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  inv1   g05(.a(x14), .O(new_n34_));
  inv1   g06(.a(x15), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand3  g08(.a(x18), .b(x17), .c(x16), .O(new_n37_));
  inv1   g09(.a(new_n37_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n36_), .c(new_n33_), .d(new_n30_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(x09), .c(x10), .O(z0));
  inv1   g12(.a(x08), .O(new_n41_));
  inv1   g13(.a(x09), .O(new_n42_));
  aoi21  g14(.a(new_n29_), .b(new_n41_), .c(new_n42_), .O(new_n43_));
  nand2  g15(.a(x08), .b(x00), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(x10), .c(new_n44_), .O(z1));
  inv1   g17(.a(x01), .O(new_n46_));
  oai21  g18(.a(x10), .b(x09), .c(x08), .O(new_n47_));
  inv1   g19(.a(x10), .O(new_n48_));
  xor2a  g20(.a(x12), .b(x11), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n50_));
  oai21  g22(.a(new_n47_), .b(new_n46_), .c(new_n50_), .O(z2));
  inv1   g23(.a(x02), .O(new_n52_));
  nand2  g24(.a(x12), .b(x11), .O(new_n53_));
  inv1   g25(.a(new_n53_), .O(new_n54_));
  nand3  g26(.a(new_n32_), .b(x12), .c(x11), .O(new_n55_));
  oai21  g27(.a(new_n54_), .b(new_n32_), .c(new_n55_), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n57_));
  oai21  g29(.a(new_n47_), .b(new_n52_), .c(new_n57_), .O(z3));
  nand2  g30(.a(new_n31_), .b(x09), .O(new_n59_));
  nand3  g31(.a(new_n59_), .b(x13), .c(x11), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x14), .O(new_n61_));
  nand3  g33(.a(new_n54_), .b(new_n34_), .c(x13), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n61_), .c(x08), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n42_), .c(new_n48_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x03), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z4));
  inv1   g38(.a(x04), .O(new_n67_));
  nand4  g39(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n35_), .b(x14), .c(x13), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n53_), .c(new_n69_), .d(new_n35_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n72_));
  oai21  g44(.a(new_n47_), .b(new_n67_), .c(new_n72_), .O(z5));
  inv1   g45(.a(x05), .O(new_n74_));
  nand3  g46(.a(x15), .b(x14), .c(x13), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n53_), .c(x16), .O(new_n76_));
  nand3  g48(.a(x13), .b(x12), .c(x11), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  nand3  g50(.a(new_n78_), .b(x15), .c(x14), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n77_), .c(new_n76_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n81_));
  oai21  g53(.a(new_n47_), .b(new_n74_), .c(new_n81_), .O(z6));
  inv1   g54(.a(x06), .O(new_n83_));
  nand3  g55(.a(x16), .b(x15), .c(x14), .O(new_n84_));
  oai21  g56(.a(new_n84_), .b(new_n77_), .c(x17), .O(new_n85_));
  nor2   g57(.a(x17), .b(new_n78_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n36_), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n77_), .c(new_n85_), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n89_));
  oai21  g61(.a(new_n47_), .b(new_n83_), .c(new_n89_), .O(z7));
  inv1   g62(.a(x07), .O(new_n91_));
  nand4  g63(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n77_), .c(x18), .O(new_n93_));
  inv1   g65(.a(x18), .O(new_n94_));
  nor2   g66(.a(new_n78_), .b(new_n35_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n69_), .c(new_n94_), .d(x17), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n93_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n48_), .c(x09), .d(new_n41_), .O(new_n98_));
  oai21  g70(.a(new_n47_), .b(new_n91_), .c(new_n98_), .O(z8));
endmodule


