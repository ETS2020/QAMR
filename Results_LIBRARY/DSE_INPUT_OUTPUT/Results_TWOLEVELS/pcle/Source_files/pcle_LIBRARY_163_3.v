// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:03 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n40_, new_n42_, new_n43_,
    new_n44_, new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x15), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand3  g05(.a(new_n33_), .b(x09), .c(new_n31_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  inv1   g07(.a(x13), .O(new_n36_));
  inv1   g08(.a(x14), .O(new_n37_));
  nand3  g09(.a(x18), .b(x17), .c(x16), .O(new_n38_));
  nor3   g10(.a(new_n38_), .b(new_n37_), .c(new_n36_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(new_n35_), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(new_n29_), .c(new_n30_), .O(z0));
  nor2   g13(.a(new_n30_), .b(new_n29_), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(new_n31_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x00), .O(new_n44_));
  inv1   g16(.a(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n44_), .O(z1));
  nand2  g19(.a(new_n43_), .b(x01), .O(new_n48_));
  xor2a  g20(.a(x12), .b(x11), .O(new_n49_));
  nand4  g21(.a(new_n49_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n48_), .O(z2));
  nand3  g23(.a(new_n36_), .b(x12), .c(x11), .O(new_n52_));
  oai21  g24(.a(new_n33_), .b(new_n36_), .c(new_n52_), .O(new_n53_));
  nand4  g25(.a(new_n53_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n42_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n54_), .O(z3));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n37_), .b(x13), .O(new_n59_));
  oai22  g31(.a(new_n59_), .b(new_n32_), .c(new_n58_), .d(new_n37_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n61_));
  aoi21  g33(.a(x08), .b(x03), .c(new_n42_), .O(new_n62_));
  nand2  g34(.a(new_n62_), .b(new_n61_), .O(z4));
  nand2  g35(.a(new_n43_), .b(x04), .O(new_n64_));
  nand4  g36(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  inv1   g37(.a(new_n65_), .O(new_n66_));
  nand3  g38(.a(new_n30_), .b(x14), .c(x13), .O(new_n67_));
  oai22  g39(.a(new_n67_), .b(new_n32_), .c(new_n66_), .d(new_n30_), .O(new_n68_));
  nand4  g40(.a(new_n68_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n64_), .O(z5));
  nand3  g42(.a(x11), .b(x09), .c(new_n31_), .O(new_n71_));
  inv1   g43(.a(x16), .O(new_n72_));
  nand4  g44(.a(new_n72_), .b(x14), .c(x13), .d(x12), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n71_), .c(new_n29_), .O(new_n74_));
  nand2  g46(.a(new_n74_), .b(x15), .O(new_n75_));
  nand3  g47(.a(x15), .b(x14), .c(x13), .O(new_n76_));
  inv1   g48(.a(new_n76_), .O(new_n77_));
  aoi21  g49(.a(new_n77_), .b(new_n33_), .c(new_n72_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n79_));
  nand2  g51(.a(x08), .b(x05), .O(new_n80_));
  nand3  g52(.a(new_n80_), .b(new_n79_), .c(new_n75_), .O(z6));
  inv1   g53(.a(x17), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(x16), .c(x14), .d(x13), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n34_), .c(new_n29_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x15), .O(new_n85_));
  nand3  g57(.a(x16), .b(x15), .c(x14), .O(new_n86_));
  inv1   g58(.a(new_n86_), .O(new_n87_));
  aoi21  g59(.a(new_n87_), .b(new_n58_), .c(new_n82_), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n89_));
  nand2  g61(.a(x08), .b(x06), .O(new_n90_));
  nand3  g62(.a(new_n90_), .b(new_n89_), .c(new_n85_), .O(z7));
  nand2  g63(.a(new_n43_), .b(x07), .O(new_n92_));
  nand4  g64(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n57_), .c(x18), .O(new_n94_));
  nor2   g66(.a(new_n72_), .b(new_n30_), .O(new_n95_));
  nor2   g67(.a(x18), .b(new_n82_), .O(new_n96_));
  nand3  g68(.a(new_n96_), .b(new_n95_), .c(new_n66_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n94_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(new_n92_), .O(z8));
endmodule


