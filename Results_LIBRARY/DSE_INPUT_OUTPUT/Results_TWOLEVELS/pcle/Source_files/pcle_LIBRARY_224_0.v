// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n36_,
    new_n37_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_;
  inv1   g00(.a(x09), .O(new_n29_));
  nand3  g01(.a(x13), .b(x12), .c(x11), .O(new_n30_));
  nor3   g02(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n31_));
  and2   g03(.a(x15), .b(x14), .O(new_n32_));
  and2   g04(.a(x18), .b(x17), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(new_n32_), .c(new_n31_), .d(x16), .O(new_n34_));
  aoi21  g06(.a(new_n34_), .b(x01), .c(x10), .O(z0));
  inv1   g07(.a(x01), .O(new_n36_));
  inv1   g08(.a(x08), .O(new_n37_));
  inv1   g09(.a(x10), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n36_), .c(new_n37_), .O(new_n39_));
  nand2  g11(.a(new_n39_), .b(x00), .O(new_n40_));
  nand2  g12(.a(new_n37_), .b(x01), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n38_), .c(x09), .O(new_n43_));
  oai21  g15(.a(new_n43_), .b(new_n41_), .c(new_n40_), .O(z1));
  nand4  g16(.a(x12), .b(new_n42_), .c(new_n38_), .d(x09), .O(new_n45_));
  aoi21  g17(.a(new_n45_), .b(new_n37_), .c(new_n36_), .O(new_n46_));
  inv1   g18(.a(x12), .O(new_n47_));
  nand3  g19(.a(new_n47_), .b(x11), .c(x09), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(x01), .c(x10), .O(new_n49_));
  or2    g21(.a(new_n49_), .b(new_n46_), .O(z2));
  oai21  g22(.a(new_n47_), .b(new_n42_), .c(x13), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  aoi21  g25(.a(new_n53_), .b(new_n51_), .c(new_n29_), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n37_), .c(new_n36_), .O(new_n55_));
  nand2  g27(.a(x08), .b(x02), .O(new_n56_));
  oai21  g28(.a(new_n55_), .b(x10), .c(new_n56_), .O(z3));
  nand2  g29(.a(new_n39_), .b(x03), .O(new_n58_));
  nand2  g30(.a(new_n30_), .b(x14), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  and2   g32(.a(x12), .b(x11), .O(new_n61_));
  nand3  g33(.a(new_n61_), .b(new_n60_), .c(x13), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n59_), .c(x10), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(x09), .c(new_n37_), .d(x01), .O(new_n64_));
  nand2  g36(.a(new_n64_), .b(new_n58_), .O(z4));
  nand2  g37(.a(new_n52_), .b(x01), .O(new_n66_));
  nand3  g38(.a(new_n66_), .b(new_n61_), .c(x14), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x15), .O(new_n68_));
  inv1   g40(.a(x15), .O(new_n69_));
  nand4  g41(.a(new_n61_), .b(new_n69_), .c(x14), .d(x13), .O(new_n70_));
  aoi21  g42(.a(new_n70_), .b(new_n68_), .c(new_n29_), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n37_), .c(new_n36_), .O(new_n72_));
  nand2  g44(.a(x08), .b(x04), .O(new_n73_));
  oai21  g45(.a(new_n72_), .b(x10), .c(new_n73_), .O(z5));
  nand3  g46(.a(new_n66_), .b(new_n61_), .c(new_n32_), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(x16), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  inv1   g49(.a(new_n30_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(new_n77_), .c(x15), .d(x14), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n76_), .c(new_n29_), .O(new_n80_));
  aoi21  g52(.a(new_n80_), .b(new_n37_), .c(new_n36_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x05), .O(new_n82_));
  oai21  g54(.a(new_n81_), .b(x10), .c(new_n82_), .O(z6));
  nand2  g55(.a(new_n39_), .b(x06), .O(new_n84_));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n30_), .c(x17), .O(new_n86_));
  inv1   g58(.a(x17), .O(new_n87_));
  nand3  g59(.a(new_n32_), .b(new_n87_), .c(x16), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n30_), .c(new_n86_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n38_), .c(x09), .d(new_n37_), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n36_), .c(new_n84_), .O(z7));
  nand2  g63(.a(new_n39_), .b(x07), .O(new_n92_));
  nand3  g64(.a(new_n32_), .b(x17), .c(x16), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n30_), .c(x18), .O(new_n94_));
  nor2   g66(.a(new_n77_), .b(new_n69_), .O(new_n95_));
  nor2   g67(.a(new_n60_), .b(new_n52_), .O(new_n96_));
  nor2   g68(.a(x18), .b(new_n87_), .O(new_n97_));
  nand4  g69(.a(new_n97_), .b(new_n96_), .c(new_n95_), .d(new_n61_), .O(new_n98_));
  aoi21  g70(.a(new_n98_), .b(new_n94_), .c(x10), .O(new_n99_));
  nand4  g71(.a(new_n99_), .b(x09), .c(new_n37_), .d(x01), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n92_), .O(z8));
endmodule


