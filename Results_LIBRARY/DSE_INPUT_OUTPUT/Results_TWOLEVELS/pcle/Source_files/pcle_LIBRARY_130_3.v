// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:58 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n37_,
    new_n38_, new_n39_, new_n41_, new_n42_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n61_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_;
  nand2  g00(.a(x18), .b(x15), .O(new_n29_));
  inv1   g01(.a(new_n29_), .O(z0));
  nand2  g02(.a(x08), .b(x00), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  inv1   g05(.a(x11), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n35_));
  aoi21  g07(.a(new_n35_), .b(new_n31_), .c(z0), .O(z1));
  xor2a  g08(.a(x12), .b(x11), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n38_));
  nand2  g10(.a(x08), .b(x01), .O(new_n39_));
  nand3  g11(.a(new_n39_), .b(new_n38_), .c(new_n29_), .O(z2));
  inv1   g12(.a(x13), .O(new_n41_));
  inv1   g13(.a(x12), .O(new_n42_));
  nor2   g14(.a(new_n42_), .b(new_n34_), .O(new_n43_));
  nand3  g15(.a(new_n41_), .b(x12), .c(x11), .O(new_n44_));
  oai21  g16(.a(new_n43_), .b(new_n41_), .c(new_n44_), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n46_));
  aoi21  g18(.a(x08), .b(x02), .c(z0), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(z3));
  nand2  g20(.a(x08), .b(x03), .O(new_n49_));
  nand3  g21(.a(new_n33_), .b(x09), .c(new_n32_), .O(new_n50_));
  inv1   g22(.a(x14), .O(new_n51_));
  nand3  g23(.a(new_n43_), .b(new_n51_), .c(x13), .O(new_n52_));
  oai21  g24(.a(new_n52_), .b(new_n50_), .c(new_n49_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n29_), .O(new_n54_));
  nand2  g26(.a(x18), .b(x15), .O(new_n55_));
  nand3  g27(.a(x13), .b(x12), .c(x11), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(new_n55_), .c(x14), .d(new_n33_), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand3  g30(.a(new_n58_), .b(x09), .c(new_n32_), .O(new_n59_));
  nand2  g31(.a(new_n59_), .b(new_n54_), .O(z4));
  nand3  g32(.a(new_n29_), .b(x08), .c(x04), .O(new_n61_));
  inv1   g33(.a(x18), .O(new_n62_));
  nand3  g34(.a(new_n56_), .b(new_n62_), .c(x15), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand4  g36(.a(new_n64_), .b(x13), .c(x12), .d(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x14), .O(new_n67_));
  nand3  g39(.a(new_n62_), .b(x15), .c(new_n51_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(new_n67_), .O(new_n69_));
  nand4  g41(.a(new_n69_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n61_), .O(z5));
  nand4  g43(.a(x11), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n72_));
  inv1   g44(.a(x16), .O(new_n73_));
  nor2   g45(.a(new_n41_), .b(new_n42_), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(new_n73_), .c(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n72_), .c(new_n62_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x15), .O(new_n77_));
  nand3  g49(.a(x15), .b(x14), .c(x13), .O(new_n78_));
  inv1   g50(.a(new_n78_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n43_), .c(new_n73_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x05), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n77_), .O(z6));
  inv1   g55(.a(x17), .O(new_n84_));
  nand4  g56(.a(new_n74_), .b(new_n84_), .c(x16), .d(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n72_), .c(new_n62_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(x15), .O(new_n87_));
  nand3  g59(.a(x16), .b(x15), .c(x14), .O(new_n88_));
  nor2   g60(.a(new_n88_), .b(new_n56_), .O(new_n89_));
  nor2   g61(.a(new_n89_), .b(new_n84_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n91_));
  nand2  g63(.a(x08), .b(x06), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(new_n91_), .c(new_n87_), .O(z7));
  nand4  g65(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n56_), .c(new_n62_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n96_));
  aoi21  g68(.a(x08), .b(x07), .c(z0), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n96_), .O(z8));
endmodule


