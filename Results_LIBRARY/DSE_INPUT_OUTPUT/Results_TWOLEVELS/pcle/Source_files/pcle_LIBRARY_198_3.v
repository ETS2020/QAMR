// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x16), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  inv1   g05(.a(x15), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nand2  g08(.a(x14), .b(x13), .O(new_n37_));
  nor4   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .d(new_n34_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n33_), .c(x09), .d(new_n31_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g12(.a(x16), .b(x10), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  inv1   g14(.a(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n44_));
  nand3  g16(.a(new_n44_), .b(new_n42_), .c(new_n41_), .O(z1));
  inv1   g17(.a(x01), .O(new_n46_));
  nand2  g18(.a(new_n41_), .b(x08), .O(new_n47_));
  xor2a  g19(.a(x12), .b(x11), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n49_));
  oai21  g21(.a(new_n47_), .b(new_n46_), .c(new_n49_), .O(z2));
  inv1   g22(.a(x02), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  oai21  g25(.a(new_n33_), .b(new_n52_), .c(new_n53_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n55_));
  oai21  g27(.a(new_n47_), .b(new_n51_), .c(new_n55_), .O(z3));
  inv1   g28(.a(x03), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x13), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n32_), .c(new_n59_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n47_), .b(new_n57_), .c(new_n63_), .O(z4));
  oai21  g36(.a(new_n37_), .b(new_n32_), .c(x15), .O(new_n65_));
  nand3  g37(.a(new_n34_), .b(x14), .c(x13), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n32_), .c(new_n65_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n68_));
  inv1   g40(.a(new_n41_), .O(new_n69_));
  aoi21  g41(.a(x08), .b(x04), .c(new_n69_), .O(new_n70_));
  nand2  g42(.a(new_n70_), .b(new_n68_), .O(z5));
  inv1   g43(.a(x05), .O(new_n72_));
  nand3  g44(.a(x15), .b(x14), .c(x13), .O(new_n73_));
  oai21  g45(.a(new_n73_), .b(new_n32_), .c(x16), .O(new_n74_));
  nand3  g46(.a(new_n30_), .b(x15), .c(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n58_), .c(new_n74_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n77_));
  oai21  g49(.a(new_n47_), .b(new_n72_), .c(new_n77_), .O(z6));
  inv1   g50(.a(x06), .O(new_n79_));
  nand3  g51(.a(x16), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n58_), .c(x17), .O(new_n81_));
  nand4  g53(.a(new_n35_), .b(x16), .c(x15), .d(x14), .O(new_n82_));
  oai21  g54(.a(new_n82_), .b(new_n58_), .c(new_n81_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n84_));
  oai21  g56(.a(new_n47_), .b(new_n79_), .c(new_n84_), .O(z7));
  nand2  g57(.a(x18), .b(new_n43_), .O(new_n86_));
  nand4  g58(.a(new_n36_), .b(x17), .c(x15), .d(x14), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n58_), .c(new_n86_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(x16), .O(new_n89_));
  nand3  g61(.a(x17), .b(x16), .c(x15), .O(new_n90_));
  inv1   g62(.a(new_n90_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(x14), .c(x13), .d(x12), .O(new_n92_));
  nand3  g64(.a(new_n92_), .b(x18), .c(new_n29_), .O(new_n93_));
  nand2  g65(.a(new_n93_), .b(new_n89_), .O(new_n94_));
  nand3  g66(.a(new_n94_), .b(x09), .c(new_n31_), .O(new_n95_));
  aoi21  g67(.a(x08), .b(x07), .c(new_n69_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n95_), .O(z8));
endmodule


