// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:39 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x11), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand4  g03(.a(x13), .b(x12), .c(x09), .d(new_n31_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  and2   g05(.a(x15), .b(x14), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n34_), .c(new_n33_), .d(x16), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g11(.a(x11), .b(x10), .O(new_n40_));
  nand2  g12(.a(x08), .b(x00), .O(new_n41_));
  nand4  g13(.a(new_n30_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n41_), .c(new_n40_), .O(z1));
  aoi21  g15(.a(x11), .b(x10), .c(new_n31_), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x01), .O(new_n45_));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand4  g18(.a(new_n46_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n45_), .O(z2));
  nand2  g20(.a(new_n44_), .b(x02), .O(new_n49_));
  nand2  g21(.a(x12), .b(x11), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(x13), .O(new_n51_));
  inv1   g23(.a(x13), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x12), .c(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(new_n54_));
  nand4  g26(.a(new_n54_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n49_), .O(z3));
  nand2  g28(.a(new_n44_), .b(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  nand3  g30(.a(x13), .b(x12), .c(x11), .O(new_n59_));
  inv1   g31(.a(new_n59_), .O(new_n60_));
  nand2  g32(.a(new_n58_), .b(x13), .O(new_n61_));
  oai22  g33(.a(new_n61_), .b(new_n50_), .c(new_n60_), .d(new_n58_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n57_), .O(z4));
  nand2  g36(.a(new_n44_), .b(x04), .O(new_n65_));
  nand2  g37(.a(x14), .b(x13), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n50_), .c(x15), .O(new_n67_));
  inv1   g39(.a(x15), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(x14), .c(x13), .O(new_n69_));
  oai21  g41(.a(new_n69_), .b(new_n50_), .c(new_n67_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n65_), .O(z5));
  nand2  g44(.a(new_n44_), .b(x05), .O(new_n73_));
  nand3  g45(.a(x15), .b(x14), .c(x13), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n50_), .c(x16), .O(new_n75_));
  inv1   g47(.a(x16), .O(new_n76_));
  nand3  g48(.a(new_n76_), .b(x15), .c(x14), .O(new_n77_));
  oai21  g49(.a(new_n77_), .b(new_n59_), .c(new_n75_), .O(new_n78_));
  nand4  g50(.a(new_n78_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n73_), .O(z6));
  nand3  g52(.a(new_n34_), .b(new_n35_), .c(x16), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n32_), .c(new_n29_), .O(new_n82_));
  nand2  g54(.a(new_n82_), .b(x11), .O(new_n83_));
  nand3  g55(.a(x16), .b(x15), .c(x14), .O(new_n84_));
  inv1   g56(.a(new_n84_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n60_), .c(new_n35_), .O(new_n86_));
  nand4  g58(.a(new_n86_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n87_));
  nand2  g59(.a(x08), .b(x06), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(new_n83_), .O(z7));
  nand4  g61(.a(new_n34_), .b(new_n36_), .c(x17), .d(x16), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n32_), .c(new_n29_), .O(new_n91_));
  nand2  g63(.a(new_n91_), .b(x11), .O(new_n92_));
  nand4  g64(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n93_));
  inv1   g65(.a(new_n93_), .O(new_n94_));
  aoi21  g66(.a(new_n94_), .b(new_n60_), .c(new_n36_), .O(new_n95_));
  nand4  g67(.a(new_n95_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n96_));
  nand2  g68(.a(x08), .b(x07), .O(new_n97_));
  nand3  g69(.a(new_n97_), .b(new_n96_), .c(new_n92_), .O(z8));
endmodule


