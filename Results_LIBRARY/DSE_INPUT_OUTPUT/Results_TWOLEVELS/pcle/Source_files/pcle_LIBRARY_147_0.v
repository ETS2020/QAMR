// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_, new_n105_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x10), .O(new_n32_));
  nand4  g04(.a(new_n32_), .b(x09), .c(new_n31_), .d(x03), .O(new_n33_));
  inv1   g05(.a(new_n33_), .O(new_n34_));
  nand4  g06(.a(new_n34_), .b(x13), .c(x12), .d(x11), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n30_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(x17), .c(x16), .d(x15), .O(new_n37_));
  nor2   g09(.a(new_n37_), .b(new_n29_), .O(z0));
  inv1   g10(.a(x00), .O(new_n39_));
  nand2  g11(.a(new_n31_), .b(x03), .O(new_n40_));
  oai21  g12(.a(x10), .b(x03), .c(x08), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nand3  g14(.a(new_n42_), .b(new_n32_), .c(x09), .O(new_n43_));
  oai22  g15(.a(new_n43_), .b(new_n40_), .c(new_n41_), .d(new_n39_), .O(z1));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(x09), .c(new_n31_), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x03), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n32_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z2));
  inv1   g22(.a(x02), .O(new_n51_));
  nand2  g23(.a(x12), .b(x11), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x13), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(x12), .c(x11), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n53_), .c(x10), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(x09), .c(new_n31_), .d(x03), .O(new_n57_));
  oai21  g29(.a(new_n41_), .b(new_n51_), .c(new_n57_), .O(z3));
  inv1   g30(.a(x03), .O(new_n59_));
  nand3  g31(.a(x13), .b(x12), .c(x11), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x14), .O(new_n61_));
  nand2  g33(.a(new_n30_), .b(x13), .O(new_n62_));
  oai21  g34(.a(new_n62_), .b(new_n52_), .c(new_n61_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n32_), .c(x09), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n31_), .c(new_n59_), .O(z4));
  inv1   g37(.a(x04), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand4  g39(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n68_));
  inv1   g40(.a(new_n68_), .O(new_n69_));
  nand3  g41(.a(new_n67_), .b(x14), .c(x13), .O(new_n70_));
  oai22  g42(.a(new_n70_), .b(new_n52_), .c(new_n69_), .d(new_n67_), .O(new_n71_));
  nand4  g43(.a(new_n71_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n72_));
  oai22  g44(.a(new_n72_), .b(new_n59_), .c(new_n41_), .d(new_n66_), .O(z5));
  inv1   g45(.a(x16), .O(new_n74_));
  nand3  g46(.a(x14), .b(x13), .c(x12), .O(new_n75_));
  nand2  g47(.a(x15), .b(x11), .O(new_n76_));
  aoi21  g48(.a(new_n75_), .b(x03), .c(new_n76_), .O(new_n77_));
  nand3  g49(.a(new_n74_), .b(x15), .c(x14), .O(new_n78_));
  oai22  g50(.a(new_n78_), .b(new_n60_), .c(new_n77_), .d(new_n74_), .O(new_n79_));
  nand3  g51(.a(new_n79_), .b(x09), .c(new_n31_), .O(new_n80_));
  nand2  g52(.a(new_n80_), .b(x03), .O(new_n81_));
  nand2  g53(.a(new_n81_), .b(new_n32_), .O(new_n82_));
  nand2  g54(.a(x08), .b(x05), .O(new_n83_));
  nand2  g55(.a(new_n83_), .b(new_n82_), .O(z6));
  inv1   g56(.a(x06), .O(new_n85_));
  nand3  g57(.a(x16), .b(x15), .c(x14), .O(new_n86_));
  oai21  g58(.a(new_n86_), .b(new_n60_), .c(x17), .O(new_n87_));
  inv1   g59(.a(x17), .O(new_n88_));
  nand4  g60(.a(new_n88_), .b(x16), .c(x15), .d(x14), .O(new_n89_));
  oai21  g61(.a(new_n89_), .b(new_n60_), .c(new_n87_), .O(new_n90_));
  nand4  g62(.a(new_n90_), .b(new_n32_), .c(x09), .d(new_n31_), .O(new_n91_));
  oai22  g63(.a(new_n91_), .b(new_n59_), .c(new_n41_), .d(new_n85_), .O(z7));
  inv1   g64(.a(x09), .O(new_n93_));
  inv1   g65(.a(new_n52_), .O(new_n94_));
  nand2  g66(.a(x16), .b(x14), .O(new_n95_));
  nand2  g67(.a(new_n95_), .b(x03), .O(new_n96_));
  nand3  g68(.a(x17), .b(x15), .c(x13), .O(new_n97_));
  inv1   g69(.a(new_n97_), .O(new_n98_));
  nand3  g70(.a(new_n98_), .b(new_n96_), .c(new_n94_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(x18), .O(new_n100_));
  nor2   g72(.a(new_n74_), .b(new_n67_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n69_), .c(new_n29_), .d(x17), .O(new_n102_));
  aoi21  g74(.a(new_n102_), .b(new_n100_), .c(new_n93_), .O(new_n103_));
  aoi21  g75(.a(new_n103_), .b(new_n31_), .c(new_n59_), .O(new_n104_));
  nand2  g76(.a(x08), .b(x07), .O(new_n105_));
  oai21  g77(.a(new_n104_), .b(x10), .c(new_n105_), .O(z8));
endmodule


