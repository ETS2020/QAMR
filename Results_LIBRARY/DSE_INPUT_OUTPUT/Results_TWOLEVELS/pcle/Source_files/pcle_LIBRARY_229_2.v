// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:14 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x14), .O(new_n30_));
  inv1   g02(.a(x04), .O(new_n31_));
  inv1   g03(.a(x08), .O(new_n32_));
  inv1   g04(.a(x10), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x09), .c(new_n32_), .d(new_n31_), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x13), .c(x12), .d(x11), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n30_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(x17), .c(x16), .d(x15), .O(new_n38_));
  nor2   g10(.a(new_n38_), .b(new_n29_), .O(z0));
  inv1   g11(.a(x00), .O(new_n40_));
  oai21  g12(.a(x10), .b(new_n31_), .c(x08), .O(new_n41_));
  inv1   g13(.a(x11), .O(new_n42_));
  nor2   g14(.a(x08), .b(x04), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n42_), .c(new_n33_), .d(x09), .O(new_n44_));
  oai21  g16(.a(new_n41_), .b(new_n40_), .c(new_n44_), .O(z1));
  xor2a  g17(.a(x12), .b(x11), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x09), .c(new_n32_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n31_), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(new_n33_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(z2));
  inv1   g23(.a(x02), .O(new_n52_));
  nand2  g24(.a(x12), .b(x11), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(x13), .O(new_n54_));
  inv1   g26(.a(x13), .O(new_n55_));
  nand3  g27(.a(new_n55_), .b(x12), .c(x11), .O(new_n56_));
  aoi21  g28(.a(new_n56_), .b(new_n54_), .c(x10), .O(new_n57_));
  nand4  g29(.a(new_n57_), .b(x09), .c(new_n32_), .d(new_n31_), .O(new_n58_));
  oai21  g30(.a(new_n41_), .b(new_n52_), .c(new_n58_), .O(z3));
  inv1   g31(.a(x09), .O(new_n60_));
  oai21  g32(.a(x12), .b(x04), .c(x13), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n42_), .c(x14), .O(new_n62_));
  inv1   g34(.a(new_n53_), .O(new_n63_));
  nand3  g35(.a(new_n63_), .b(new_n30_), .c(x13), .O(new_n64_));
  aoi21  g36(.a(new_n64_), .b(new_n62_), .c(new_n60_), .O(new_n65_));
  aoi21  g37(.a(new_n65_), .b(new_n32_), .c(x04), .O(new_n66_));
  nand2  g38(.a(x08), .b(x03), .O(new_n67_));
  oai21  g39(.a(new_n66_), .b(x10), .c(new_n67_), .O(z4));
  oai21  g40(.a(new_n33_), .b(x08), .c(x04), .O(new_n69_));
  oai21  g41(.a(x12), .b(x04), .c(x11), .O(new_n70_));
  nor2   g42(.a(new_n30_), .b(new_n55_), .O(new_n71_));
  inv1   g43(.a(new_n71_), .O(new_n72_));
  oai21  g44(.a(new_n72_), .b(new_n70_), .c(x15), .O(new_n73_));
  inv1   g45(.a(x15), .O(new_n74_));
  nand4  g46(.a(new_n63_), .b(new_n74_), .c(x14), .d(x13), .O(new_n75_));
  nand2  g47(.a(new_n75_), .b(new_n73_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n69_), .O(z5));
  nand3  g50(.a(x15), .b(x14), .c(x13), .O(new_n79_));
  oai21  g51(.a(new_n79_), .b(new_n53_), .c(x16), .O(new_n80_));
  inv1   g52(.a(x16), .O(new_n81_));
  nand3  g53(.a(x13), .b(x12), .c(x11), .O(new_n82_));
  inv1   g54(.a(new_n82_), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(new_n81_), .c(x15), .d(x14), .O(new_n84_));
  aoi21  g56(.a(new_n84_), .b(new_n80_), .c(new_n60_), .O(new_n85_));
  aoi21  g57(.a(new_n85_), .b(new_n32_), .c(x04), .O(new_n86_));
  nand2  g58(.a(x08), .b(x05), .O(new_n87_));
  oai21  g59(.a(new_n86_), .b(x10), .c(new_n87_), .O(z6));
  inv1   g60(.a(x06), .O(new_n89_));
  nand3  g61(.a(x16), .b(x15), .c(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n82_), .c(x17), .O(new_n91_));
  inv1   g63(.a(x17), .O(new_n92_));
  nand4  g64(.a(new_n92_), .b(x16), .c(x15), .d(x14), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n82_), .c(new_n91_), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(new_n33_), .c(x09), .d(new_n32_), .O(new_n95_));
  oai22  g67(.a(new_n95_), .b(x04), .c(new_n41_), .d(new_n89_), .O(z7));
  inv1   g68(.a(x07), .O(new_n97_));
  nand4  g69(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n98_));
  oai21  g70(.a(new_n98_), .b(new_n82_), .c(x18), .O(new_n99_));
  nor2   g71(.a(new_n81_), .b(new_n74_), .O(new_n100_));
  nor2   g72(.a(x18), .b(new_n92_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n100_), .c(new_n71_), .d(new_n63_), .O(new_n102_));
  aoi21  g74(.a(new_n102_), .b(new_n99_), .c(x10), .O(new_n103_));
  nand4  g75(.a(new_n103_), .b(x09), .c(new_n32_), .d(new_n31_), .O(new_n104_));
  oai21  g76(.a(new_n41_), .b(new_n97_), .c(new_n104_), .O(z8));
endmodule


