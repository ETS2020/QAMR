// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_;
  inv1   g00(.a(x07), .O(new_n29_));
  inv1   g01(.a(x10), .O(new_n30_));
  nand2  g02(.a(new_n30_), .b(x09), .O(new_n31_));
  nand3  g03(.a(x13), .b(x12), .c(x11), .O(new_n32_));
  nor2   g04(.a(new_n32_), .b(new_n31_), .O(new_n33_));
  and2   g05(.a(x15), .b(x14), .O(new_n34_));
  inv1   g06(.a(x17), .O(new_n35_));
  inv1   g07(.a(x18), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n35_), .O(new_n37_));
  nand4  g09(.a(new_n37_), .b(new_n34_), .c(new_n33_), .d(x16), .O(new_n38_));
  aoi21  g10(.a(new_n38_), .b(new_n29_), .c(x08), .O(z0));
  nor2   g11(.a(x11), .b(x10), .O(new_n40_));
  aoi21  g12(.a(new_n40_), .b(x09), .c(x07), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  oai21  g14(.a(new_n41_), .b(x08), .c(new_n42_), .O(z1));
  inv1   g15(.a(x08), .O(new_n44_));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n30_), .c(x09), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(new_n29_), .O(new_n47_));
  nand2  g19(.a(new_n47_), .b(new_n44_), .O(new_n48_));
  nand2  g20(.a(x08), .b(x01), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(z2));
  and2   g22(.a(x12), .b(x11), .O(new_n51_));
  inv1   g23(.a(new_n51_), .O(new_n52_));
  nand2  g24(.a(new_n52_), .b(x13), .O(new_n53_));
  inv1   g25(.a(x13), .O(new_n54_));
  nand3  g26(.a(new_n54_), .b(x12), .c(x11), .O(new_n55_));
  aoi21  g27(.a(new_n55_), .b(new_n53_), .c(x10), .O(new_n56_));
  nand4  g28(.a(new_n56_), .b(x09), .c(new_n44_), .d(new_n29_), .O(new_n57_));
  nand2  g29(.a(x08), .b(x02), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(new_n57_), .O(z3));
  nand2  g31(.a(new_n32_), .b(x14), .O(new_n60_));
  inv1   g32(.a(x14), .O(new_n61_));
  nand3  g33(.a(new_n51_), .b(new_n61_), .c(x13), .O(new_n62_));
  aoi21  g34(.a(new_n62_), .b(new_n60_), .c(x10), .O(new_n63_));
  nand4  g35(.a(new_n63_), .b(x09), .c(new_n44_), .d(new_n29_), .O(new_n64_));
  nand2  g36(.a(x08), .b(x03), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(new_n64_), .O(z4));
  nand2  g38(.a(new_n54_), .b(new_n29_), .O(new_n67_));
  nand3  g39(.a(new_n67_), .b(new_n51_), .c(x14), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x15), .O(new_n69_));
  inv1   g41(.a(x15), .O(new_n70_));
  nand4  g42(.a(new_n51_), .b(new_n70_), .c(x14), .d(x13), .O(new_n71_));
  aoi21  g43(.a(new_n71_), .b(new_n69_), .c(x10), .O(new_n72_));
  aoi21  g44(.a(new_n72_), .b(x09), .c(x07), .O(new_n73_));
  nand2  g45(.a(x08), .b(x04), .O(new_n74_));
  oai21  g46(.a(new_n73_), .b(x08), .c(new_n74_), .O(z5));
  nand3  g47(.a(new_n67_), .b(new_n51_), .c(new_n34_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x16), .O(new_n77_));
  inv1   g49(.a(x16), .O(new_n78_));
  inv1   g50(.a(new_n32_), .O(new_n79_));
  nand4  g51(.a(new_n79_), .b(new_n78_), .c(x15), .d(x14), .O(new_n80_));
  aoi21  g52(.a(new_n80_), .b(new_n77_), .c(x10), .O(new_n81_));
  aoi21  g53(.a(new_n81_), .b(x09), .c(x07), .O(new_n82_));
  nand2  g54(.a(x08), .b(x05), .O(new_n83_));
  oai21  g55(.a(new_n82_), .b(x08), .c(new_n83_), .O(z6));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n32_), .c(x17), .O(new_n86_));
  nand4  g58(.a(new_n34_), .b(new_n79_), .c(new_n35_), .d(x16), .O(new_n87_));
  aoi21  g59(.a(new_n87_), .b(new_n86_), .c(x10), .O(new_n88_));
  aoi21  g60(.a(new_n88_), .b(x09), .c(x07), .O(new_n89_));
  nand2  g61(.a(x08), .b(x06), .O(new_n90_));
  oai21  g62(.a(new_n89_), .b(x08), .c(new_n90_), .O(z7));
  nand3  g63(.a(new_n34_), .b(x17), .c(x16), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n32_), .c(x18), .O(new_n93_));
  nand3  g65(.a(new_n51_), .b(x14), .c(x13), .O(new_n94_));
  nand4  g66(.a(new_n36_), .b(x17), .c(x16), .d(x15), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n94_), .c(new_n93_), .O(new_n96_));
  nand4  g68(.a(new_n96_), .b(new_n30_), .c(x09), .d(new_n44_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(new_n29_), .O(z8));
endmodule


