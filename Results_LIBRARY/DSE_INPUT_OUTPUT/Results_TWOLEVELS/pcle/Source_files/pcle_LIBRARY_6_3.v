// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n91_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x13), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  nand2  g03(.a(x12), .b(x11), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand2  g05(.a(x15), .b(x14), .O(new_n34_));
  nand3  g06(.a(x18), .b(x17), .c(x16), .O(new_n35_));
  nor2   g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  nand4  g08(.a(new_n36_), .b(new_n33_), .c(x09), .d(new_n31_), .O(new_n37_));
  aoi21  g09(.a(new_n37_), .b(new_n29_), .c(new_n30_), .O(z0));
  inv1   g10(.a(x00), .O(new_n39_));
  oai21  g11(.a(new_n30_), .b(new_n29_), .c(x08), .O(new_n40_));
  inv1   g12(.a(x11), .O(new_n41_));
  nand4  g13(.a(new_n41_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n42_));
  oai21  g14(.a(new_n40_), .b(new_n39_), .c(new_n42_), .O(z1));
  inv1   g15(.a(x01), .O(new_n44_));
  xor2a  g16(.a(x12), .b(x11), .O(new_n45_));
  nand4  g17(.a(new_n45_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n46_));
  oai21  g18(.a(new_n40_), .b(new_n44_), .c(new_n46_), .O(z2));
  nand2  g19(.a(new_n32_), .b(x13), .O(new_n48_));
  nand4  g20(.a(new_n30_), .b(x12), .c(x11), .d(new_n29_), .O(new_n49_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(x09), .c(new_n31_), .O(new_n51_));
  nor2   g23(.a(new_n30_), .b(new_n29_), .O(new_n52_));
  aoi21  g24(.a(x08), .b(x02), .c(new_n52_), .O(new_n53_));
  nand2  g25(.a(new_n53_), .b(new_n51_), .O(z3));
  inv1   g26(.a(x03), .O(new_n55_));
  inv1   g27(.a(x14), .O(new_n56_));
  nand3  g28(.a(x13), .b(x12), .c(x11), .O(new_n57_));
  inv1   g29(.a(new_n57_), .O(new_n58_));
  nand2  g30(.a(new_n56_), .b(x13), .O(new_n59_));
  oai22  g31(.a(new_n59_), .b(new_n32_), .c(new_n58_), .d(new_n56_), .O(new_n60_));
  nand4  g32(.a(new_n60_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n61_));
  oai21  g33(.a(new_n40_), .b(new_n55_), .c(new_n61_), .O(z4));
  nand3  g34(.a(x11), .b(x09), .c(new_n31_), .O(new_n63_));
  inv1   g35(.a(x15), .O(new_n64_));
  nand3  g36(.a(new_n64_), .b(x14), .c(x12), .O(new_n65_));
  oai21  g37(.a(new_n65_), .b(new_n63_), .c(new_n29_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x13), .O(new_n67_));
  nand4  g39(.a(x14), .b(x13), .c(x12), .d(x11), .O(new_n68_));
  nand3  g40(.a(new_n68_), .b(x15), .c(new_n29_), .O(new_n69_));
  inv1   g41(.a(new_n69_), .O(new_n70_));
  nand3  g42(.a(new_n70_), .b(x09), .c(new_n31_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x04), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n71_), .c(new_n67_), .O(z5));
  inv1   g45(.a(x16), .O(new_n74_));
  oai21  g46(.a(new_n34_), .b(new_n30_), .c(new_n29_), .O(new_n75_));
  aoi21  g47(.a(new_n75_), .b(new_n48_), .c(new_n74_), .O(new_n76_));
  nand3  g48(.a(new_n74_), .b(x15), .c(x14), .O(new_n77_));
  nor2   g49(.a(new_n77_), .b(new_n57_), .O(new_n78_));
  oai21  g50(.a(new_n78_), .b(new_n76_), .c(x09), .O(new_n79_));
  aoi21  g51(.a(x08), .b(x05), .c(new_n52_), .O(new_n80_));
  oai21  g52(.a(new_n79_), .b(x08), .c(new_n80_), .O(z6));
  inv1   g53(.a(x17), .O(new_n82_));
  nand4  g54(.a(x16), .b(x15), .c(x14), .d(x13), .O(new_n83_));
  aoi22  g55(.a(new_n83_), .b(new_n29_), .c(new_n32_), .d(x13), .O(new_n84_));
  inv1   g56(.a(new_n34_), .O(new_n85_));
  nand4  g57(.a(new_n58_), .b(new_n85_), .c(new_n82_), .d(x16), .O(new_n86_));
  oai21  g58(.a(new_n84_), .b(new_n82_), .c(new_n86_), .O(new_n87_));
  nand3  g59(.a(new_n87_), .b(x09), .c(new_n31_), .O(new_n88_));
  aoi21  g60(.a(x08), .b(x06), .c(new_n52_), .O(new_n89_));
  nand2  g61(.a(new_n89_), .b(new_n88_), .O(z7));
  inv1   g62(.a(x18), .O(new_n91_));
  nand2  g63(.a(x14), .b(x13), .O(new_n92_));
  nand3  g64(.a(x17), .b(x16), .c(x15), .O(new_n93_));
  oai21  g65(.a(new_n93_), .b(new_n92_), .c(new_n29_), .O(new_n94_));
  aoi21  g66(.a(new_n94_), .b(new_n48_), .c(new_n91_), .O(new_n95_));
  nand4  g67(.a(new_n91_), .b(x17), .c(x16), .d(x15), .O(new_n96_));
  nor2   g68(.a(new_n96_), .b(new_n68_), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n95_), .c(x09), .O(new_n98_));
  aoi21  g70(.a(x08), .b(x07), .c(new_n52_), .O(new_n99_));
  oai21  g71(.a(new_n98_), .b(x08), .c(new_n99_), .O(z8));
endmodule


