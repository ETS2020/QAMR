// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:47 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  inv1   g10(.a(x16), .O(new_n39_));
  inv1   g11(.a(x09), .O(new_n40_));
  nor2   g12(.a(new_n40_), .b(x08), .O(new_n41_));
  aoi21  g13(.a(new_n41_), .b(new_n38_), .c(new_n39_), .O(new_n42_));
  nand2  g14(.a(x08), .b(x00), .O(new_n43_));
  oai21  g15(.a(new_n42_), .b(x10), .c(new_n43_), .O(z1));
  nand2  g16(.a(x12), .b(new_n38_), .O(new_n45_));
  inv1   g17(.a(x12), .O(new_n46_));
  nand3  g18(.a(x16), .b(new_n46_), .c(x11), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(new_n40_), .O(new_n48_));
  aoi21  g20(.a(new_n48_), .b(new_n30_), .c(new_n39_), .O(new_n49_));
  nand2  g21(.a(x08), .b(x01), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(x10), .c(new_n50_), .O(z2));
  inv1   g23(.a(x02), .O(new_n52_));
  oai21  g24(.a(x16), .b(x10), .c(x08), .O(new_n53_));
  nand2  g25(.a(x12), .b(x11), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(x13), .O(new_n55_));
  inv1   g27(.a(x13), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(x12), .c(x11), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n55_), .c(new_n39_), .O(new_n58_));
  nand4  g30(.a(new_n58_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n59_));
  oai21  g31(.a(new_n53_), .b(new_n52_), .c(new_n59_), .O(z3));
  nand3  g32(.a(x13), .b(x12), .c(x11), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(x14), .O(new_n62_));
  inv1   g34(.a(x14), .O(new_n63_));
  nand3  g35(.a(x16), .b(new_n63_), .c(x13), .O(new_n64_));
  oai21  g36(.a(new_n64_), .b(new_n54_), .c(new_n62_), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x09), .c(new_n30_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(x16), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(new_n31_), .O(new_n68_));
  nand2  g40(.a(x08), .b(x03), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n68_), .O(z4));
  nand2  g42(.a(x14), .b(x13), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n54_), .c(x15), .O(new_n72_));
  inv1   g44(.a(x15), .O(new_n73_));
  nand3  g45(.a(x16), .b(new_n73_), .c(x14), .O(new_n74_));
  oai21  g46(.a(new_n74_), .b(new_n61_), .c(new_n72_), .O(new_n75_));
  nand3  g47(.a(new_n75_), .b(x09), .c(new_n30_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x16), .O(new_n77_));
  nand2  g49(.a(new_n77_), .b(new_n31_), .O(new_n78_));
  nand2  g50(.a(x08), .b(x04), .O(new_n79_));
  nand2  g51(.a(new_n79_), .b(new_n78_), .O(z5));
  inv1   g52(.a(x05), .O(new_n81_));
  nor2   g53(.a(new_n73_), .b(new_n63_), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(x13), .c(x12), .d(x11), .O(new_n83_));
  nand4  g55(.a(new_n83_), .b(x16), .c(new_n31_), .d(x09), .O(new_n84_));
  oai22  g56(.a(new_n84_), .b(x08), .c(new_n53_), .d(new_n81_), .O(z6));
  inv1   g57(.a(x06), .O(new_n86_));
  nand3  g58(.a(x15), .b(x14), .c(x13), .O(new_n87_));
  oai21  g59(.a(new_n87_), .b(new_n54_), .c(x17), .O(new_n88_));
  inv1   g60(.a(x17), .O(new_n89_));
  nand3  g61(.a(new_n89_), .b(x15), .c(x14), .O(new_n90_));
  oai21  g62(.a(new_n90_), .b(new_n61_), .c(new_n88_), .O(new_n91_));
  nand4  g63(.a(new_n91_), .b(x16), .c(new_n31_), .d(x09), .O(new_n92_));
  oai22  g64(.a(new_n92_), .b(x08), .c(new_n53_), .d(new_n86_), .O(z7));
  inv1   g65(.a(x07), .O(new_n94_));
  nand3  g66(.a(x17), .b(x15), .c(x14), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n61_), .c(x18), .O(new_n96_));
  nand4  g68(.a(new_n29_), .b(x17), .c(x15), .d(x14), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n61_), .c(new_n96_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(x16), .c(new_n31_), .d(x09), .O(new_n99_));
  oai22  g71(.a(new_n99_), .b(x08), .c(new_n53_), .d(new_n94_), .O(z8));
endmodule


