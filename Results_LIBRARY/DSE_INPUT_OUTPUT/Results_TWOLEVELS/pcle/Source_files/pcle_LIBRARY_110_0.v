// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:54 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n42_, new_n44_,
    new_n45_, new_n46_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_,
    new_n76_, new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_,
    new_n99_, new_n100_, new_n101_, new_n102_, new_n103_, new_n104_,
    new_n106_, new_n107_, new_n108_, new_n109_, new_n110_, new_n111_,
    new_n112_, new_n113_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  inv1   g09(.a(x11), .O(new_n38_));
  nor2   g10(.a(x17), .b(x14), .O(new_n39_));
  inv1   g11(.a(new_n39_), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n38_), .c(new_n31_), .d(x09), .O(new_n41_));
  aoi21  g13(.a(x08), .b(x00), .c(new_n39_), .O(new_n42_));
  oai21  g14(.a(new_n41_), .b(x08), .c(new_n42_), .O(z1));
  xor2a  g15(.a(x12), .b(x11), .O(new_n44_));
  nand4  g16(.a(new_n44_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n45_));
  nand2  g17(.a(x08), .b(x01), .O(new_n46_));
  aoi21  g18(.a(new_n46_), .b(new_n45_), .c(new_n39_), .O(z2));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  nand2  g20(.a(new_n48_), .b(x13), .O(new_n49_));
  inv1   g21(.a(x13), .O(new_n50_));
  nand3  g22(.a(new_n50_), .b(x12), .c(x11), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n49_), .O(new_n52_));
  nand4  g24(.a(new_n52_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n53_));
  nand2  g25(.a(x08), .b(x02), .O(new_n54_));
  aoi21  g26(.a(new_n54_), .b(new_n53_), .c(new_n39_), .O(z3));
  nor2   g27(.a(new_n39_), .b(new_n30_), .O(new_n56_));
  nand2  g28(.a(new_n56_), .b(x03), .O(new_n57_));
  inv1   g29(.a(x14), .O(new_n58_));
  inv1   g30(.a(x12), .O(new_n59_));
  nand2  g31(.a(x14), .b(new_n50_), .O(new_n60_));
  nand3  g32(.a(x17), .b(new_n58_), .c(x13), .O(new_n61_));
  aoi21  g33(.a(new_n61_), .b(new_n60_), .c(new_n59_), .O(new_n62_));
  nor2   g34(.a(new_n58_), .b(x12), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n62_), .c(x11), .O(new_n64_));
  oai21  g36(.a(new_n58_), .b(x11), .c(new_n64_), .O(new_n65_));
  nand4  g37(.a(new_n65_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n66_));
  nand2  g38(.a(new_n66_), .b(new_n57_), .O(z4));
  nand4  g39(.a(x15), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n68_));
  nand2  g40(.a(new_n68_), .b(x17), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n58_), .O(new_n70_));
  inv1   g42(.a(x15), .O(new_n71_));
  nand3  g43(.a(x13), .b(x12), .c(x11), .O(new_n72_));
  inv1   g44(.a(new_n72_), .O(new_n73_));
  nand3  g45(.a(new_n71_), .b(x14), .c(x13), .O(new_n74_));
  oai22  g46(.a(new_n74_), .b(new_n48_), .c(new_n73_), .d(new_n71_), .O(new_n75_));
  nand4  g47(.a(new_n75_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n76_));
  nand2  g48(.a(x08), .b(x04), .O(new_n77_));
  nand3  g49(.a(new_n77_), .b(new_n76_), .c(new_n70_), .O(z5));
  nand2  g50(.a(new_n56_), .b(x05), .O(new_n79_));
  nand2  g51(.a(x15), .b(x13), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n48_), .c(x16), .O(new_n81_));
  inv1   g53(.a(x16), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(x15), .c(x13), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n48_), .c(new_n81_), .O(new_n84_));
  nand2  g56(.a(new_n84_), .b(x14), .O(new_n85_));
  nand3  g57(.a(x17), .b(x16), .c(new_n58_), .O(new_n86_));
  nand2  g58(.a(new_n86_), .b(new_n85_), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n88_));
  nand2  g60(.a(new_n88_), .b(new_n79_), .O(z6));
  nand2  g61(.a(x17), .b(new_n58_), .O(new_n90_));
  inv1   g62(.a(x17), .O(new_n91_));
  nand3  g63(.a(new_n91_), .b(x16), .c(x15), .O(new_n92_));
  aoi21  g64(.a(new_n92_), .b(new_n90_), .c(new_n50_), .O(new_n93_));
  nor2   g65(.a(new_n91_), .b(x13), .O(new_n94_));
  oai21  g66(.a(new_n94_), .b(new_n93_), .c(x12), .O(new_n95_));
  nand2  g67(.a(x17), .b(new_n59_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(new_n95_), .O(new_n97_));
  nand2  g69(.a(new_n97_), .b(x11), .O(new_n98_));
  nor2   g70(.a(new_n82_), .b(new_n71_), .O(new_n99_));
  nand2  g71(.a(new_n99_), .b(x11), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(x17), .O(new_n101_));
  nand2  g73(.a(new_n101_), .b(new_n98_), .O(new_n102_));
  nand4  g74(.a(new_n102_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n103_));
  aoi21  g75(.a(x08), .b(x06), .c(new_n39_), .O(new_n104_));
  nand2  g76(.a(new_n104_), .b(new_n103_), .O(z7));
  nand2  g77(.a(new_n56_), .b(x07), .O(new_n106_));
  nand3  g78(.a(new_n99_), .b(new_n29_), .c(x17), .O(new_n107_));
  oai22  g79(.a(new_n107_), .b(new_n72_), .c(new_n29_), .d(x17), .O(new_n108_));
  nand2  g80(.a(new_n108_), .b(x14), .O(new_n109_));
  nand3  g81(.a(new_n99_), .b(new_n73_), .c(x14), .O(new_n110_));
  nand3  g82(.a(new_n110_), .b(x18), .c(x17), .O(new_n111_));
  nand2  g83(.a(new_n111_), .b(new_n109_), .O(new_n112_));
  nand4  g84(.a(new_n112_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n113_));
  nand2  g85(.a(new_n113_), .b(new_n106_), .O(z8));
endmodule


