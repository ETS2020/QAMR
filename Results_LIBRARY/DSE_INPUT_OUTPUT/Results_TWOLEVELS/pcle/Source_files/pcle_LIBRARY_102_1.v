// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n53_,
    new_n54_, new_n55_, new_n57_, new_n58_, new_n60_, new_n61_, new_n62_,
    new_n63_, new_n65_, new_n66_, new_n68_, new_n69_, new_n70_, new_n71_;
  inv1   g00(.a(x14), .O(new_n29_));
  nor2   g01(.a(new_n29_), .b(x10), .O(z0));
  inv1   g02(.a(x00), .O(new_n31_));
  oai21  g03(.a(new_n29_), .b(x10), .c(x08), .O(new_n32_));
  inv1   g04(.a(x09), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(x08), .O(new_n34_));
  nor3   g06(.a(x14), .b(x11), .c(x10), .O(new_n35_));
  nand2  g07(.a(new_n35_), .b(new_n34_), .O(new_n36_));
  oai21  g08(.a(new_n32_), .b(new_n31_), .c(new_n36_), .O(z1));
  inv1   g09(.a(x01), .O(new_n38_));
  inv1   g10(.a(x10), .O(new_n39_));
  xor2a  g11(.a(x12), .b(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n29_), .c(new_n39_), .d(x09), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(x08), .c(new_n32_), .d(new_n38_), .O(z2));
  inv1   g14(.a(new_n32_), .O(new_n43_));
  nand2  g15(.a(new_n43_), .b(x02), .O(new_n44_));
  inv1   g16(.a(x08), .O(new_n45_));
  nand2  g17(.a(x12), .b(x11), .O(new_n46_));
  nand2  g18(.a(new_n46_), .b(x13), .O(new_n47_));
  inv1   g19(.a(x13), .O(new_n48_));
  nand3  g20(.a(new_n48_), .b(x12), .c(x11), .O(new_n49_));
  aoi21  g21(.a(new_n49_), .b(new_n47_), .c(x14), .O(new_n50_));
  nand4  g22(.a(new_n50_), .b(new_n39_), .c(x09), .d(new_n45_), .O(new_n51_));
  nand2  g23(.a(new_n51_), .b(new_n44_), .O(z3));
  inv1   g24(.a(x03), .O(new_n53_));
  nand3  g25(.a(new_n39_), .b(x09), .c(new_n45_), .O(new_n54_));
  nand4  g26(.a(new_n29_), .b(x13), .c(x12), .d(x11), .O(new_n55_));
  oai22  g27(.a(new_n55_), .b(new_n54_), .c(new_n32_), .d(new_n53_), .O(z4));
  inv1   g28(.a(x04), .O(new_n57_));
  nand4  g29(.a(new_n34_), .b(x15), .c(new_n29_), .d(new_n39_), .O(new_n58_));
  oai21  g30(.a(new_n32_), .b(new_n57_), .c(new_n58_), .O(z5));
  nand2  g31(.a(x16), .b(x09), .O(new_n60_));
  oai21  g32(.a(new_n60_), .b(x08), .c(new_n29_), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n39_), .O(new_n62_));
  nand2  g34(.a(x08), .b(x05), .O(new_n63_));
  nand2  g35(.a(new_n63_), .b(new_n62_), .O(z6));
  inv1   g36(.a(x06), .O(new_n65_));
  nand4  g37(.a(new_n34_), .b(x17), .c(new_n29_), .d(new_n39_), .O(new_n66_));
  oai21  g38(.a(new_n32_), .b(new_n65_), .c(new_n66_), .O(z7));
  nand2  g39(.a(x18), .b(x09), .O(new_n68_));
  oai21  g40(.a(new_n68_), .b(x08), .c(new_n29_), .O(new_n69_));
  nand2  g41(.a(new_n69_), .b(new_n39_), .O(new_n70_));
  nand2  g42(.a(x08), .b(x07), .O(new_n71_));
  nand2  g43(.a(new_n71_), .b(new_n70_), .O(z8));
endmodule


