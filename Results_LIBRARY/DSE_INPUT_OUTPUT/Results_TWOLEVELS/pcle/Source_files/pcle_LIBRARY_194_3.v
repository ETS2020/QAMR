// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n37_, new_n38_, new_n39_, new_n41_, new_n42_, new_n43_,
    new_n45_, new_n46_, new_n47_, new_n48_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n55_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n84_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_,
    new_n93_, new_n94_, new_n95_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_, new_n103_;
  inv1   g00(.a(x10), .O(new_n29_));
  inv1   g01(.a(x12), .O(new_n30_));
  inv1   g02(.a(x08), .O(new_n31_));
  inv1   g03(.a(x11), .O(new_n32_));
  inv1   g04(.a(x13), .O(new_n33_));
  nor2   g05(.a(new_n33_), .b(new_n32_), .O(new_n34_));
  inv1   g06(.a(x14), .O(new_n35_));
  inv1   g07(.a(x15), .O(new_n36_));
  nand3  g08(.a(x18), .b(x17), .c(x16), .O(new_n37_));
  nor3   g09(.a(new_n37_), .b(new_n36_), .c(new_n35_), .O(new_n38_));
  nand4  g10(.a(new_n38_), .b(new_n34_), .c(x09), .d(new_n31_), .O(new_n39_));
  aoi21  g11(.a(new_n39_), .b(new_n29_), .c(new_n30_), .O(z0));
  nand2  g12(.a(x12), .b(x10), .O(new_n41_));
  nand2  g13(.a(x08), .b(x00), .O(new_n42_));
  nand4  g14(.a(new_n32_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n43_));
  nand3  g15(.a(new_n43_), .b(new_n42_), .c(new_n41_), .O(z1));
  inv1   g16(.a(x01), .O(new_n45_));
  nand2  g17(.a(new_n41_), .b(x08), .O(new_n46_));
  xor2a  g18(.a(x12), .b(x11), .O(new_n47_));
  nand4  g19(.a(new_n47_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n48_));
  oai21  g20(.a(new_n46_), .b(new_n45_), .c(new_n48_), .O(z2));
  oai21  g21(.a(new_n30_), .b(new_n32_), .c(x13), .O(new_n50_));
  nand3  g22(.a(new_n33_), .b(x12), .c(x11), .O(new_n51_));
  oai21  g23(.a(new_n50_), .b(x10), .c(new_n51_), .O(new_n52_));
  nand3  g24(.a(new_n52_), .b(x09), .c(new_n31_), .O(new_n53_));
  inv1   g25(.a(new_n41_), .O(new_n54_));
  aoi21  g26(.a(x08), .b(x02), .c(new_n54_), .O(new_n55_));
  nand2  g27(.a(new_n55_), .b(new_n53_), .O(z3));
  inv1   g28(.a(x03), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  nor2   g31(.a(new_n30_), .b(new_n32_), .O(new_n60_));
  nand3  g32(.a(new_n60_), .b(new_n35_), .c(x13), .O(new_n61_));
  nand2  g33(.a(new_n61_), .b(new_n59_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n63_));
  oai21  g35(.a(new_n46_), .b(new_n57_), .c(new_n63_), .O(z4));
  nand3  g36(.a(x11), .b(x09), .c(new_n31_), .O(new_n65_));
  nand3  g37(.a(new_n36_), .b(x14), .c(x13), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n65_), .c(new_n29_), .O(new_n67_));
  nand2  g39(.a(new_n67_), .b(x12), .O(new_n68_));
  nor2   g40(.a(new_n35_), .b(new_n33_), .O(new_n69_));
  aoi21  g41(.a(new_n69_), .b(new_n60_), .c(new_n36_), .O(new_n70_));
  nand4  g42(.a(new_n70_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n71_));
  nand2  g43(.a(x08), .b(x04), .O(new_n72_));
  nand3  g44(.a(new_n72_), .b(new_n71_), .c(new_n68_), .O(z5));
  inv1   g45(.a(x16), .O(new_n74_));
  nand3  g46(.a(new_n69_), .b(new_n74_), .c(x15), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n65_), .c(new_n29_), .O(new_n76_));
  nand2  g48(.a(new_n76_), .b(x12), .O(new_n77_));
  nand3  g49(.a(x15), .b(x14), .c(x13), .O(new_n78_));
  inv1   g50(.a(new_n78_), .O(new_n79_));
  aoi21  g51(.a(new_n79_), .b(new_n60_), .c(new_n74_), .O(new_n80_));
  nand4  g52(.a(new_n80_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n81_));
  nand2  g53(.a(x08), .b(x05), .O(new_n82_));
  nand3  g54(.a(new_n82_), .b(new_n81_), .c(new_n77_), .O(z6));
  inv1   g55(.a(x06), .O(new_n84_));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n58_), .c(x17), .O(new_n86_));
  inv1   g58(.a(x17), .O(new_n87_));
  nand4  g59(.a(new_n87_), .b(x16), .c(x15), .d(x14), .O(new_n88_));
  oai21  g60(.a(new_n88_), .b(new_n58_), .c(new_n86_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n29_), .c(x09), .d(new_n31_), .O(new_n90_));
  oai21  g62(.a(new_n46_), .b(new_n84_), .c(new_n90_), .O(z7));
  nand2  g63(.a(x18), .b(new_n32_), .O(new_n92_));
  nand3  g64(.a(x14), .b(x13), .c(x11), .O(new_n93_));
  inv1   g65(.a(x18), .O(new_n94_));
  nand4  g66(.a(new_n94_), .b(x17), .c(x16), .d(x15), .O(new_n95_));
  oai21  g67(.a(new_n95_), .b(new_n93_), .c(new_n92_), .O(new_n96_));
  nand2  g68(.a(new_n96_), .b(x12), .O(new_n97_));
  nor2   g69(.a(new_n87_), .b(new_n74_), .O(new_n98_));
  nand4  g70(.a(new_n98_), .b(new_n69_), .c(x15), .d(x12), .O(new_n99_));
  nand3  g71(.a(new_n99_), .b(x18), .c(new_n29_), .O(new_n100_));
  nand2  g72(.a(new_n100_), .b(new_n97_), .O(new_n101_));
  nand3  g73(.a(new_n101_), .b(x09), .c(new_n31_), .O(new_n102_));
  aoi21  g74(.a(x08), .b(x07), .c(new_n54_), .O(new_n103_));
  nand2  g75(.a(new_n103_), .b(new_n102_), .O(z8));
endmodule


