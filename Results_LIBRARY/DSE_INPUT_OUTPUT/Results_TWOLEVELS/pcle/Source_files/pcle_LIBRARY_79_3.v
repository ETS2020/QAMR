// Benchmark "FAU" written by ABC on Wed Aug 19 15:24:49 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n31_, new_n32_, new_n33_, new_n34_, new_n35_,
    new_n36_, new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_,
    new_n45_, new_n47_, new_n48_, new_n49_, new_n50_, new_n51_, new_n52_,
    new_n53_, new_n54_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n61_, new_n62_, new_n63_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n69_, new_n70_, new_n71_, new_n72_, new_n73_, new_n75_, new_n76_,
    new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_,
    new_n85_, new_n86_, new_n87_, new_n88_, new_n89_, new_n90_, new_n91_,
    new_n92_, new_n93_, new_n94_, new_n96_, new_n97_, new_n98_, new_n99_,
    new_n100_, new_n101_, new_n102_;
  inv1   g00(.a(x18), .O(new_n29_));
  inv1   g01(.a(x08), .O(new_n30_));
  inv1   g02(.a(x10), .O(new_n31_));
  nand4  g03(.a(x11), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n32_));
  inv1   g04(.a(new_n32_), .O(new_n33_));
  nand4  g05(.a(new_n33_), .b(x14), .c(x13), .d(x12), .O(new_n34_));
  inv1   g06(.a(new_n34_), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(x17), .c(x16), .d(x15), .O(new_n36_));
  nor2   g08(.a(new_n36_), .b(new_n29_), .O(z0));
  nand2  g09(.a(x13), .b(x10), .O(new_n38_));
  nand2  g10(.a(x08), .b(x00), .O(new_n39_));
  inv1   g11(.a(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n41_));
  nand3  g13(.a(new_n41_), .b(new_n39_), .c(new_n38_), .O(z1));
  xor2a  g14(.a(x12), .b(x11), .O(new_n43_));
  nand4  g15(.a(new_n43_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n44_));
  nand2  g16(.a(x08), .b(x01), .O(new_n45_));
  nand3  g17(.a(new_n45_), .b(new_n44_), .c(new_n38_), .O(z2));
  inv1   g18(.a(x13), .O(new_n47_));
  nand2  g19(.a(x12), .b(x11), .O(new_n48_));
  inv1   g20(.a(new_n48_), .O(new_n49_));
  nand4  g21(.a(new_n47_), .b(x12), .c(x11), .d(new_n31_), .O(new_n50_));
  oai21  g22(.a(new_n49_), .b(new_n47_), .c(new_n50_), .O(new_n51_));
  nand3  g23(.a(new_n51_), .b(x09), .c(new_n30_), .O(new_n52_));
  inv1   g24(.a(new_n38_), .O(new_n53_));
  aoi21  g25(.a(x08), .b(x02), .c(new_n53_), .O(new_n54_));
  nand2  g26(.a(new_n54_), .b(new_n52_), .O(z3));
  inv1   g27(.a(x03), .O(new_n56_));
  nand2  g28(.a(new_n38_), .b(x08), .O(new_n57_));
  nand3  g29(.a(x13), .b(x12), .c(x11), .O(new_n58_));
  nand2  g30(.a(new_n58_), .b(x14), .O(new_n59_));
  inv1   g31(.a(x14), .O(new_n60_));
  nand2  g32(.a(new_n60_), .b(x13), .O(new_n61_));
  oai21  g33(.a(new_n61_), .b(new_n48_), .c(new_n59_), .O(new_n62_));
  nand4  g34(.a(new_n62_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n63_));
  oai21  g35(.a(new_n57_), .b(new_n56_), .c(new_n63_), .O(z4));
  nand3  g36(.a(x14), .b(x12), .c(x11), .O(new_n65_));
  nand2  g37(.a(new_n65_), .b(x15), .O(new_n66_));
  inv1   g38(.a(x15), .O(new_n67_));
  nand3  g39(.a(new_n49_), .b(new_n67_), .c(x14), .O(new_n68_));
  aoi21  g40(.a(new_n68_), .b(new_n66_), .c(new_n47_), .O(new_n69_));
  nand3  g41(.a(x15), .b(new_n47_), .c(new_n31_), .O(new_n70_));
  inv1   g42(.a(new_n70_), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n69_), .c(x09), .O(new_n72_));
  aoi21  g44(.a(x08), .b(x04), .c(new_n53_), .O(new_n73_));
  oai21  g45(.a(new_n72_), .b(x08), .c(new_n73_), .O(z5));
  nand2  g46(.a(x15), .b(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n48_), .c(x16), .O(new_n76_));
  inv1   g48(.a(x16), .O(new_n77_));
  nand4  g49(.a(new_n49_), .b(new_n77_), .c(x15), .d(x14), .O(new_n78_));
  aoi21  g50(.a(new_n78_), .b(new_n76_), .c(new_n47_), .O(new_n79_));
  nand3  g51(.a(x16), .b(new_n47_), .c(new_n31_), .O(new_n80_));
  inv1   g52(.a(new_n80_), .O(new_n81_));
  oai21  g53(.a(new_n81_), .b(new_n79_), .c(x09), .O(new_n82_));
  aoi21  g54(.a(x08), .b(x05), .c(new_n53_), .O(new_n83_));
  oai21  g55(.a(new_n82_), .b(x08), .c(new_n83_), .O(z6));
  nand3  g56(.a(x16), .b(x15), .c(x14), .O(new_n85_));
  oai21  g57(.a(new_n85_), .b(new_n48_), .c(x17), .O(new_n86_));
  inv1   g58(.a(new_n65_), .O(new_n87_));
  nor2   g59(.a(x17), .b(new_n77_), .O(new_n88_));
  nand3  g60(.a(new_n88_), .b(new_n87_), .c(x15), .O(new_n89_));
  aoi21  g61(.a(new_n89_), .b(new_n86_), .c(new_n47_), .O(new_n90_));
  nand3  g62(.a(x17), .b(new_n47_), .c(new_n31_), .O(new_n91_));
  inv1   g63(.a(new_n91_), .O(new_n92_));
  oai21  g64(.a(new_n92_), .b(new_n90_), .c(x09), .O(new_n93_));
  aoi21  g65(.a(x08), .b(x06), .c(new_n53_), .O(new_n94_));
  oai21  g66(.a(new_n93_), .b(x08), .c(new_n94_), .O(z7));
  inv1   g67(.a(x07), .O(new_n96_));
  nand4  g68(.a(x17), .b(x16), .c(x15), .d(x14), .O(new_n97_));
  oai21  g69(.a(new_n97_), .b(new_n58_), .c(x18), .O(new_n98_));
  nand3  g70(.a(new_n49_), .b(x14), .c(x13), .O(new_n99_));
  nand4  g71(.a(new_n29_), .b(x17), .c(x16), .d(x15), .O(new_n100_));
  oai21  g72(.a(new_n100_), .b(new_n99_), .c(new_n98_), .O(new_n101_));
  nand4  g73(.a(new_n101_), .b(new_n31_), .c(x09), .d(new_n30_), .O(new_n102_));
  oai21  g74(.a(new_n57_), .b(new_n96_), .c(new_n102_), .O(z8));
endmodule


