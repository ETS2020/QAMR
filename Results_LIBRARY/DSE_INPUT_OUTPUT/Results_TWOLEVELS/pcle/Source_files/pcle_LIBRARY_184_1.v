// Benchmark "FAU" written by ABC on Wed Aug 19 15:25:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18,
    z0, z1, z2, z3, z4, z5, z6, z7, z8  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18;
  output z0, z1, z2, z3, z4, z5, z6, z7, z8;
  wire new_n29_, new_n30_, new_n32_, new_n33_, new_n34_, new_n35_, new_n36_,
    new_n38_, new_n39_, new_n40_, new_n41_, new_n43_, new_n44_, new_n45_,
    new_n46_, new_n47_, new_n48_, new_n49_, new_n51_, new_n52_, new_n53_,
    new_n54_, new_n55_, new_n56_, new_n57_, new_n58_, new_n59_, new_n60_,
    new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_, new_n68_,
    new_n70_, new_n71_, new_n72_, new_n73_, new_n74_, new_n75_, new_n76_,
    new_n77_, new_n79_, new_n80_, new_n81_, new_n82_, new_n83_, new_n84_,
    new_n85_, new_n87_, new_n88_, new_n89_, new_n90_;
  inv1   g00(.a(x10), .O(new_n29_));
  nand2  g01(.a(x18), .b(new_n29_), .O(new_n30_));
  inv1   g02(.a(new_n30_), .O(z0));
  inv1   g03(.a(x00), .O(new_n32_));
  nand2  g04(.a(new_n30_), .b(x08), .O(new_n33_));
  inv1   g05(.a(x08), .O(new_n34_));
  nor2   g06(.a(x18), .b(x11), .O(new_n35_));
  nand4  g07(.a(new_n35_), .b(new_n29_), .c(x09), .d(new_n34_), .O(new_n36_));
  oai21  g08(.a(new_n33_), .b(new_n32_), .c(new_n36_), .O(z1));
  inv1   g09(.a(x01), .O(new_n38_));
  inv1   g10(.a(x18), .O(new_n39_));
  xor2a  g11(.a(x12), .b(x11), .O(new_n40_));
  nand4  g12(.a(new_n40_), .b(new_n39_), .c(new_n29_), .d(x09), .O(new_n41_));
  oai22  g13(.a(new_n41_), .b(x08), .c(new_n33_), .d(new_n38_), .O(z2));
  inv1   g14(.a(x02), .O(new_n43_));
  nand2  g15(.a(x12), .b(x11), .O(new_n44_));
  nand2  g16(.a(new_n44_), .b(x13), .O(new_n45_));
  inv1   g17(.a(x13), .O(new_n46_));
  nand3  g18(.a(new_n46_), .b(x12), .c(x11), .O(new_n47_));
  aoi21  g19(.a(new_n47_), .b(new_n45_), .c(x18), .O(new_n48_));
  nand4  g20(.a(new_n48_), .b(new_n29_), .c(x09), .d(new_n34_), .O(new_n49_));
  oai21  g21(.a(new_n33_), .b(new_n43_), .c(new_n49_), .O(z3));
  inv1   g22(.a(x09), .O(new_n51_));
  inv1   g23(.a(x11), .O(new_n52_));
  oai21  g24(.a(x18), .b(x12), .c(x13), .O(new_n53_));
  oai21  g25(.a(new_n53_), .b(new_n52_), .c(x14), .O(new_n54_));
  inv1   g26(.a(x14), .O(new_n55_));
  inv1   g27(.a(new_n44_), .O(new_n56_));
  nand3  g28(.a(new_n56_), .b(new_n55_), .c(x13), .O(new_n57_));
  aoi21  g29(.a(new_n57_), .b(new_n54_), .c(new_n51_), .O(new_n58_));
  aoi21  g30(.a(new_n58_), .b(new_n34_), .c(x18), .O(new_n59_));
  nand2  g31(.a(x08), .b(x03), .O(new_n60_));
  oai21  g32(.a(new_n59_), .b(x10), .c(new_n60_), .O(z4));
  inv1   g33(.a(x04), .O(new_n62_));
  nand2  g34(.a(x14), .b(x13), .O(new_n63_));
  oai21  g35(.a(new_n63_), .b(new_n44_), .c(x15), .O(new_n64_));
  inv1   g36(.a(x15), .O(new_n65_));
  nand3  g37(.a(new_n65_), .b(x14), .c(x13), .O(new_n66_));
  oai21  g38(.a(new_n66_), .b(new_n44_), .c(new_n64_), .O(new_n67_));
  nand4  g39(.a(new_n67_), .b(new_n39_), .c(new_n29_), .d(x09), .O(new_n68_));
  oai22  g40(.a(new_n68_), .b(x08), .c(new_n33_), .d(new_n62_), .O(z5));
  inv1   g41(.a(x05), .O(new_n70_));
  nand3  g42(.a(x15), .b(x14), .c(x13), .O(new_n71_));
  oai21  g43(.a(new_n71_), .b(new_n44_), .c(x16), .O(new_n72_));
  nand3  g44(.a(x13), .b(x12), .c(x11), .O(new_n73_));
  inv1   g45(.a(x16), .O(new_n74_));
  nand3  g46(.a(new_n74_), .b(x15), .c(x14), .O(new_n75_));
  oai21  g47(.a(new_n75_), .b(new_n73_), .c(new_n72_), .O(new_n76_));
  nand4  g48(.a(new_n76_), .b(new_n39_), .c(new_n29_), .d(x09), .O(new_n77_));
  oai22  g49(.a(new_n77_), .b(x08), .c(new_n33_), .d(new_n70_), .O(z6));
  inv1   g50(.a(x06), .O(new_n79_));
  nand3  g51(.a(x16), .b(x15), .c(x14), .O(new_n80_));
  oai21  g52(.a(new_n80_), .b(new_n73_), .c(x17), .O(new_n81_));
  inv1   g53(.a(x17), .O(new_n82_));
  nand4  g54(.a(new_n82_), .b(x16), .c(x15), .d(x14), .O(new_n83_));
  oai21  g55(.a(new_n83_), .b(new_n73_), .c(new_n81_), .O(new_n84_));
  nand4  g56(.a(new_n84_), .b(new_n39_), .c(new_n29_), .d(x09), .O(new_n85_));
  oai22  g57(.a(new_n85_), .b(x08), .c(new_n33_), .d(new_n79_), .O(z7));
  inv1   g58(.a(x07), .O(new_n87_));
  nand4  g59(.a(new_n56_), .b(new_n29_), .c(x09), .d(new_n34_), .O(new_n88_));
  inv1   g60(.a(new_n71_), .O(new_n89_));
  nand4  g61(.a(new_n89_), .b(new_n39_), .c(x17), .d(x16), .O(new_n90_));
  oai22  g62(.a(new_n90_), .b(new_n88_), .c(new_n33_), .d(new_n87_), .O(z8));
endmodule


