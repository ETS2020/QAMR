// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:46 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n87_, new_n89_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n103_, new_n104_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x17), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(x04), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  aoi21  g14(.a(new_n55_), .b(x00), .c(new_n43_), .O(z01));
  nor2   g15(.a(x03), .b(x02), .O(new_n57_));
  nor2   g16(.a(x05), .b(x04), .O(new_n58_));
  nor2   g17(.a(x07), .b(x06), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n46_), .O(new_n60_));
  and2   g19(.a(new_n60_), .b(x00), .O(new_n61_));
  inv1   g20(.a(x04), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n63_));
  nand2  g22(.a(new_n50_), .b(new_n49_), .O(new_n64_));
  inv1   g23(.a(x09), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(x08), .c(new_n51_), .O(new_n66_));
  nor3   g25(.a(new_n66_), .b(new_n64_), .c(new_n63_), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n61_), .c(x19), .O(new_n68_));
  nand2  g27(.a(new_n68_), .b(new_n44_), .O(z02));
  inv1   g28(.a(x20), .O(new_n70_));
  inv1   g29(.a(new_n44_), .O(new_n71_));
  nor2   g30(.a(new_n71_), .b(new_n70_), .O(z03));
  inv1   g31(.a(x21), .O(new_n73_));
  nand3  g32(.a(new_n44_), .b(new_n73_), .c(new_n70_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  inv1   g34(.a(new_n63_), .O(new_n76_));
  inv1   g35(.a(new_n64_), .O(new_n77_));
  nor2   g36(.a(new_n43_), .b(x08), .O(new_n78_));
  nand4  g37(.a(new_n78_), .b(new_n77_), .c(new_n76_), .d(new_n51_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n44_), .c(new_n75_), .O(z05));
  nand3  g39(.a(new_n59_), .b(new_n49_), .c(new_n62_), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(new_n82_));
  nand4  g41(.a(new_n82_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n83_));
  nor2   g42(.a(new_n60_), .b(new_n42_), .O(new_n84_));
  aoi21  g43(.a(new_n84_), .b(new_n83_), .c(new_n43_), .O(z06));
  inv1   g44(.a(x18), .O(new_n86_));
  nand2  g45(.a(new_n44_), .b(x24), .O(new_n87_));
  nor2   g46(.a(new_n87_), .b(new_n86_), .O(z07));
  inv1   g47(.a(x11), .O(new_n89_));
  nand2  g48(.a(new_n44_), .b(new_n89_), .O(z08));
  nor2   g49(.a(new_n87_), .b(new_n89_), .O(z09));
  inv1   g50(.a(x24), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x22), .c(x14), .O(new_n93_));
  nand2  g52(.a(new_n93_), .b(new_n44_), .O(z10));
  nand3  g53(.a(x22), .b(x19), .c(x17), .O(new_n95_));
  nor2   g54(.a(new_n95_), .b(x24), .O(z11));
  nand4  g55(.a(new_n44_), .b(new_n92_), .c(x23), .d(x14), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z12));
  nand3  g57(.a(x23), .b(x19), .c(x17), .O(new_n99_));
  nor2   g58(.a(new_n99_), .b(x24), .O(z13));
  nand2  g59(.a(new_n92_), .b(x16), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n44_), .O(z14));
  nor2   g61(.a(x13), .b(x12), .O(new_n103_));
  nor2   g62(.a(x15), .b(x14), .O(new_n104_));
  aoi21  g63(.a(new_n104_), .b(new_n103_), .c(new_n71_), .O(z15));
endmodule


