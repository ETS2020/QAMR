// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n68_, new_n69_, new_n71_, new_n73_, new_n75_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n87_, new_n89_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n100_, new_n102_, new_n103_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x08), .O(new_n43_));
  nand2  g02(.a(x19), .b(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x04), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  nor2   g10(.a(x06), .b(x05), .O(new_n52_));
  nand3  g11(.a(new_n52_), .b(x09), .c(new_n51_), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n50_), .c(x00), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x19), .c(x08), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z01));
  nor2   g15(.a(x03), .b(x02), .O(new_n57_));
  nor2   g16(.a(x05), .b(x04), .O(new_n58_));
  nor2   g17(.a(x07), .b(x06), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n58_), .c(new_n57_), .d(new_n46_), .O(new_n60_));
  nand2  g19(.a(new_n60_), .b(x00), .O(new_n61_));
  inv1   g20(.a(new_n50_), .O(new_n62_));
  nor2   g21(.a(x09), .b(x07), .O(new_n63_));
  nand3  g22(.a(new_n63_), .b(new_n52_), .c(new_n62_), .O(new_n64_));
  nand2  g23(.a(new_n64_), .b(new_n61_), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(x19), .c(x08), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(z02));
  inv1   g26(.a(x20), .O(new_n68_));
  inv1   g27(.a(new_n44_), .O(new_n69_));
  nor2   g28(.a(new_n69_), .b(new_n68_), .O(z03));
  inv1   g29(.a(x21), .O(new_n71_));
  nand3  g30(.a(new_n44_), .b(new_n71_), .c(new_n68_), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  nand2  g32(.a(new_n44_), .b(new_n73_), .O(z05));
  inv1   g33(.a(x19), .O(new_n75_));
  inv1   g34(.a(x05), .O(new_n76_));
  nand4  g35(.a(new_n59_), .b(new_n76_), .c(new_n49_), .d(new_n48_), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(x02), .O(new_n78_));
  nor3   g37(.a(x03), .b(x02), .c(x01), .O(new_n79_));
  nand4  g38(.a(new_n59_), .b(new_n58_), .c(new_n79_), .d(x00), .O(new_n80_));
  aoi21  g39(.a(new_n78_), .b(new_n46_), .c(new_n80_), .O(new_n81_));
  aoi21  g40(.a(new_n81_), .b(x08), .c(new_n75_), .O(z06));
  nand2  g41(.a(x24), .b(x18), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n44_), .O(z07));
  inv1   g43(.a(x11), .O(new_n85_));
  nand2  g44(.a(new_n44_), .b(new_n85_), .O(z08));
  inv1   g45(.a(x24), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n85_), .c(new_n44_), .O(z09));
  nand3  g47(.a(new_n87_), .b(x22), .c(x14), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n44_), .O(z10));
  nand4  g49(.a(new_n44_), .b(new_n87_), .c(x22), .d(x17), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z11));
  inv1   g51(.a(x23), .O(new_n93_));
  nand2  g52(.a(new_n44_), .b(new_n87_), .O(new_n94_));
  nor2   g53(.a(new_n94_), .b(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x14), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z12));
  nand2  g56(.a(new_n95_), .b(x17), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z13));
  inv1   g58(.a(x16), .O(new_n100_));
  nor2   g59(.a(new_n94_), .b(new_n100_), .O(z14));
  nor2   g60(.a(x15), .b(x14), .O(new_n102_));
  nor3   g61(.a(new_n69_), .b(x13), .c(x12), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n102_), .O(z15));
endmodule


