// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:40 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n72_, new_n74_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_,
    new_n82_, new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_,
    new_n89_, new_n91_, new_n92_, new_n94_, new_n97_, new_n98_, new_n100_,
    new_n102_, new_n103_, new_n104_, new_n106_, new_n108_, new_n110_,
    new_n111_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x20), .O(new_n43_));
  nor2   g02(.a(x22), .b(new_n43_), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(new_n44_), .O(new_n46_));
  inv1   g05(.a(x01), .O(new_n47_));
  inv1   g06(.a(x02), .O(new_n48_));
  inv1   g07(.a(x03), .O(new_n49_));
  inv1   g08(.a(x05), .O(new_n50_));
  inv1   g09(.a(x06), .O(new_n51_));
  inv1   g10(.a(x07), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x04), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(x00), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x19), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(new_n46_), .O(z01));
  nand3  g18(.a(new_n49_), .b(new_n48_), .c(new_n47_), .O(new_n60_));
  inv1   g19(.a(x04), .O(new_n61_));
  nor2   g20(.a(x07), .b(x06), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n50_), .c(new_n61_), .O(new_n63_));
  oai21  g22(.a(new_n63_), .b(new_n60_), .c(x00), .O(new_n64_));
  nand4  g23(.a(new_n61_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n65_));
  inv1   g24(.a(new_n65_), .O(new_n66_));
  nor2   g25(.a(x06), .b(x05), .O(new_n67_));
  inv1   g26(.a(x08), .O(new_n68_));
  nor2   g27(.a(x09), .b(new_n68_), .O(new_n69_));
  nand4  g28(.a(new_n69_), .b(new_n67_), .c(new_n66_), .d(new_n52_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(new_n64_), .O(new_n71_));
  nand3  g30(.a(new_n71_), .b(new_n46_), .c(x19), .O(new_n72_));
  inv1   g31(.a(new_n72_), .O(z02));
  inv1   g32(.a(x21), .O(new_n74_));
  nand2  g33(.a(x22), .b(x20), .O(new_n75_));
  oai21  g34(.a(new_n74_), .b(x20), .c(new_n75_), .O(z04));
  inv1   g35(.a(x10), .O(new_n77_));
  nand3  g36(.a(x19), .b(new_n68_), .c(new_n52_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n67_), .c(new_n66_), .O(new_n80_));
  aoi21  g39(.a(new_n80_), .b(new_n77_), .c(new_n44_), .O(z05));
  nand2  g40(.a(x09), .b(x08), .O(new_n82_));
  nand4  g41(.a(new_n82_), .b(new_n52_), .c(new_n51_), .d(new_n50_), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(x04), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(new_n49_), .c(new_n48_), .d(new_n47_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n64_), .c(new_n44_), .O(new_n86_));
  nand4  g45(.a(new_n67_), .b(x09), .c(x08), .d(new_n52_), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n65_), .c(x00), .O(new_n88_));
  oai21  g47(.a(new_n88_), .b(new_n86_), .c(x19), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(new_n46_), .O(z06));
  inv1   g49(.a(x18), .O(new_n91_));
  nand2  g50(.a(new_n46_), .b(x24), .O(new_n92_));
  nor2   g51(.a(new_n92_), .b(new_n91_), .O(z07));
  inv1   g52(.a(x11), .O(new_n94_));
  nor2   g53(.a(new_n44_), .b(new_n94_), .O(z08));
  nor2   g54(.a(new_n92_), .b(new_n94_), .O(z09));
  inv1   g55(.a(x24), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(x22), .c(x14), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n46_), .O(z10));
  nand2  g58(.a(x22), .b(x17), .O(new_n100_));
  nor2   g59(.a(new_n100_), .b(x24), .O(z11));
  inv1   g60(.a(x23), .O(new_n102_));
  nor3   g61(.a(new_n44_), .b(x24), .c(new_n102_), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(x14), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z12));
  nand2  g64(.a(new_n103_), .b(x17), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z13));
  nand2  g66(.a(new_n97_), .b(x16), .O(new_n108_));
  nand2  g67(.a(new_n108_), .b(new_n46_), .O(z14));
  nor2   g68(.a(x13), .b(x12), .O(new_n110_));
  nor2   g69(.a(x15), .b(x14), .O(new_n111_));
  aoi21  g70(.a(new_n111_), .b(new_n110_), .c(new_n44_), .O(z15));
  buf    g71(.a(x20), .O(z03));
endmodule


