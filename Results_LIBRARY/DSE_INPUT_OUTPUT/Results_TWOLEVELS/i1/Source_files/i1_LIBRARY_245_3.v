// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:37 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n89_,
    new_n91_, new_n93_, new_n95_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n108_, new_n109_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x21), .b(x15), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x05), .O(new_n48_));
  inv1   g07(.a(x06), .O(new_n49_));
  inv1   g08(.a(x07), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x04), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x00), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n43_), .c(x19), .O(new_n56_));
  inv1   g15(.a(new_n56_), .O(z01));
  nand3  g16(.a(new_n47_), .b(new_n46_), .c(new_n45_), .O(new_n58_));
  inv1   g17(.a(x04), .O(new_n59_));
  nor2   g18(.a(x07), .b(x06), .O(new_n60_));
  nand3  g19(.a(new_n60_), .b(new_n48_), .c(new_n59_), .O(new_n61_));
  oai21  g20(.a(new_n61_), .b(new_n58_), .c(x00), .O(new_n62_));
  nand4  g21(.a(new_n59_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(new_n64_));
  nor2   g23(.a(x06), .b(x05), .O(new_n65_));
  inv1   g24(.a(x09), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(x08), .c(new_n50_), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(new_n65_), .c(new_n64_), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n62_), .O(new_n70_));
  nand2  g29(.a(new_n70_), .b(x19), .O(new_n71_));
  nand2  g30(.a(new_n71_), .b(new_n43_), .O(z02));
  inv1   g31(.a(x20), .O(new_n73_));
  nand2  g32(.a(new_n43_), .b(new_n73_), .O(z03));
  inv1   g33(.a(x21), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x20), .O(new_n76_));
  oai21  g35(.a(new_n75_), .b(x15), .c(new_n76_), .O(z04));
  inv1   g36(.a(x10), .O(new_n78_));
  inv1   g37(.a(new_n43_), .O(new_n79_));
  inv1   g38(.a(x19), .O(new_n80_));
  nor2   g39(.a(new_n80_), .b(x08), .O(new_n81_));
  nand4  g40(.a(new_n81_), .b(new_n65_), .c(new_n64_), .d(new_n50_), .O(new_n82_));
  aoi21  g41(.a(new_n82_), .b(new_n78_), .c(new_n79_), .O(z05));
  nand2  g42(.a(new_n66_), .b(x08), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n85_));
  nor2   g44(.a(new_n85_), .b(x04), .O(new_n86_));
  nand4  g45(.a(new_n86_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x00), .O(new_n88_));
  aoi21  g47(.a(new_n88_), .b(new_n43_), .c(new_n70_), .O(new_n89_));
  oai21  g48(.a(new_n89_), .b(new_n80_), .c(new_n43_), .O(z06));
  nand3  g49(.a(new_n43_), .b(x24), .c(x18), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z07));
  inv1   g51(.a(x11), .O(new_n93_));
  nor2   g52(.a(new_n79_), .b(new_n93_), .O(z08));
  inv1   g53(.a(x24), .O(new_n95_));
  oai21  g54(.a(new_n95_), .b(new_n93_), .c(new_n43_), .O(z09));
  nor2   g55(.a(new_n79_), .b(x24), .O(new_n97_));
  nand3  g56(.a(new_n97_), .b(x22), .c(x14), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z10));
  nand3  g58(.a(new_n95_), .b(x22), .c(x17), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n43_), .O(z11));
  nand3  g60(.a(new_n97_), .b(x23), .c(x14), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z12));
  nand3  g62(.a(new_n97_), .b(x23), .c(x17), .O(new_n104_));
  inv1   g63(.a(new_n104_), .O(z13));
  nand2  g64(.a(new_n97_), .b(x16), .O(new_n106_));
  inv1   g65(.a(new_n106_), .O(z14));
  nor3   g66(.a(x14), .b(x13), .c(x12), .O(new_n108_));
  nand2  g67(.a(new_n75_), .b(x15), .O(new_n109_));
  oai21  g68(.a(new_n108_), .b(x15), .c(new_n109_), .O(z15));
endmodule


