// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_,
    new_n66_, new_n67_, new_n69_, new_n70_, new_n71_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n85_, new_n88_, new_n89_, new_n90_, new_n91_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n101_, new_n102_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  inv1   g02(.a(x02), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x06), .O(new_n46_));
  inv1   g05(.a(x07), .O(new_n47_));
  xnor2a g06(.a(x09), .b(x08), .O(new_n48_));
  nand4  g07(.a(new_n48_), .b(x19), .c(new_n47_), .d(new_n46_), .O(new_n49_));
  nor3   g08(.a(new_n49_), .b(x05), .c(x04), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n51_));
  nor2   g10(.a(new_n51_), .b(new_n42_), .O(z01));
  inv1   g11(.a(x19), .O(new_n53_));
  inv1   g12(.a(x04), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n55_));
  nor2   g14(.a(x06), .b(x05), .O(new_n56_));
  inv1   g15(.a(x09), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x08), .O(new_n58_));
  nand3  g17(.a(new_n58_), .b(new_n56_), .c(new_n47_), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n55_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n53_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(x00), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z02));
  nand2  g22(.a(x19), .b(new_n42_), .O(new_n64_));
  and2   g23(.a(new_n64_), .b(x20), .O(z03));
  inv1   g24(.a(new_n64_), .O(new_n66_));
  nor2   g25(.a(x21), .b(x20), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n66_), .O(z04));
  inv1   g27(.a(x10), .O(new_n69_));
  inv1   g28(.a(x08), .O(new_n70_));
  nand4  g29(.a(new_n56_), .b(x19), .c(new_n70_), .d(new_n47_), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n55_), .c(new_n69_), .O(new_n72_));
  nand2  g31(.a(new_n72_), .b(x00), .O(new_n73_));
  oai21  g32(.a(x19), .b(new_n69_), .c(new_n73_), .O(z05));
  inv1   g33(.a(x05), .O(new_n75_));
  nand2  g34(.a(new_n57_), .b(x08), .O(new_n76_));
  nand4  g35(.a(new_n76_), .b(new_n47_), .c(new_n46_), .d(new_n75_), .O(new_n77_));
  nor2   g36(.a(new_n77_), .b(x04), .O(new_n78_));
  nand4  g37(.a(new_n78_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n79_));
  nand3  g38(.a(new_n79_), .b(new_n60_), .c(x00), .O(new_n80_));
  and2   g39(.a(new_n80_), .b(x19), .O(z06));
  inv1   g40(.a(x18), .O(new_n82_));
  inv1   g41(.a(x24), .O(new_n83_));
  nor3   g42(.a(new_n66_), .b(new_n83_), .c(new_n82_), .O(z07));
  inv1   g43(.a(x11), .O(new_n85_));
  nand2  g44(.a(new_n64_), .b(new_n85_), .O(z08));
  oai21  g45(.a(new_n83_), .b(new_n85_), .c(new_n64_), .O(z09));
  inv1   g46(.a(x22), .O(new_n88_));
  nand2  g47(.a(new_n64_), .b(new_n83_), .O(new_n89_));
  nor2   g48(.a(new_n89_), .b(new_n88_), .O(new_n90_));
  nand2  g49(.a(new_n90_), .b(x14), .O(new_n91_));
  inv1   g50(.a(new_n91_), .O(z10));
  nand2  g51(.a(new_n90_), .b(x17), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z11));
  nand4  g53(.a(new_n64_), .b(new_n83_), .c(x23), .d(x14), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z12));
  nand3  g55(.a(new_n83_), .b(x23), .c(x17), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(new_n64_), .O(z13));
  inv1   g57(.a(x16), .O(new_n99_));
  nor2   g58(.a(new_n89_), .b(new_n99_), .O(z14));
  nor2   g59(.a(x15), .b(x14), .O(new_n101_));
  nor3   g60(.a(new_n66_), .b(x13), .c(x12), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n101_), .O(z15));
  buf    g62(.a(x00), .O(z00));
endmodule


