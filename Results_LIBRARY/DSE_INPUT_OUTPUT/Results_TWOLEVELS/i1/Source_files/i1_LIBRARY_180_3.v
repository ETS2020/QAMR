// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:08 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n64_, new_n66_,
    new_n68_, new_n69_, new_n71_, new_n73_, new_n75_, new_n77_, new_n79_,
    new_n81_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_, new_n91_,
    new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x08), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  nor2   g06(.a(x02), .b(x01), .O(new_n48_));
  nor2   g07(.a(x04), .b(x03), .O(new_n49_));
  nand2  g08(.a(new_n49_), .b(new_n48_), .O(new_n50_));
  nor2   g09(.a(x06), .b(x05), .O(new_n51_));
  nor2   g10(.a(x09), .b(x07), .O(new_n52_));
  nand2  g11(.a(new_n52_), .b(new_n51_), .O(new_n53_));
  oai21  g12(.a(new_n53_), .b(new_n50_), .c(x00), .O(new_n54_));
  nand3  g13(.a(new_n54_), .b(x19), .c(new_n43_), .O(new_n55_));
  inv1   g14(.a(new_n55_), .O(z01));
  inv1   g15(.a(x01), .O(new_n57_));
  nor2   g16(.a(x03), .b(x02), .O(new_n58_));
  nor2   g17(.a(x05), .b(x04), .O(new_n59_));
  nor2   g18(.a(x07), .b(x06), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n59_), .c(new_n58_), .d(new_n57_), .O(new_n61_));
  nand4  g20(.a(new_n61_), .b(x19), .c(new_n43_), .d(x00), .O(new_n62_));
  inv1   g21(.a(new_n62_), .O(z02));
  inv1   g22(.a(x20), .O(new_n64_));
  nor2   g23(.a(new_n45_), .b(new_n64_), .O(z03));
  inv1   g24(.a(x21), .O(new_n66_));
  nand3  g25(.a(new_n46_), .b(new_n66_), .c(new_n64_), .O(z04));
  inv1   g26(.a(x10), .O(new_n68_));
  and2   g27(.a(new_n61_), .b(new_n43_), .O(new_n69_));
  oai21  g28(.a(new_n69_), .b(new_n44_), .c(new_n68_), .O(z05));
  oai21  g29(.a(new_n61_), .b(new_n42_), .c(new_n43_), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n69_), .c(new_n44_), .O(z06));
  nand2  g31(.a(x24), .b(x18), .O(new_n73_));
  nand2  g32(.a(new_n73_), .b(new_n46_), .O(z07));
  inv1   g33(.a(x11), .O(new_n75_));
  nand2  g34(.a(new_n46_), .b(new_n75_), .O(z08));
  inv1   g35(.a(x24), .O(new_n77_));
  oai21  g36(.a(new_n77_), .b(new_n75_), .c(new_n46_), .O(z09));
  nand3  g37(.a(new_n77_), .b(x22), .c(x14), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n46_), .O(z10));
  nor2   g39(.a(new_n45_), .b(x24), .O(new_n81_));
  nand3  g40(.a(new_n81_), .b(x22), .c(x17), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z11));
  nand3  g42(.a(new_n81_), .b(x23), .c(x14), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z12));
  nand3  g44(.a(new_n81_), .b(x23), .c(x17), .O(new_n86_));
  inv1   g45(.a(new_n86_), .O(z13));
  nand2  g46(.a(new_n81_), .b(x16), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  inv1   g48(.a(x12), .O(new_n90_));
  inv1   g49(.a(x13), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  nand4  g51(.a(new_n92_), .b(new_n46_), .c(new_n91_), .d(new_n90_), .O(z15));
endmodule


