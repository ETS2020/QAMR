// Benchmark "FAU" written by ABC on Wed Aug 19 17:33:53 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n73_, new_n75_, new_n77_, new_n78_, new_n79_, new_n80_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n88_, new_n90_,
    new_n91_, new_n93_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n106_, new_n108_, new_n109_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(x08), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  nand2  g10(.a(x09), .b(x08), .O(new_n52_));
  inv1   g11(.a(x08), .O(new_n53_));
  inv1   g12(.a(x09), .O(new_n54_));
  nand3  g13(.a(x19), .b(new_n54_), .c(new_n53_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(new_n52_), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(x04), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n59_));
  inv1   g18(.a(new_n44_), .O(new_n60_));
  aoi21  g19(.a(x19), .b(new_n42_), .c(new_n60_), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n59_), .O(z01));
  inv1   g21(.a(x04), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n64_));
  nor2   g23(.a(x06), .b(x05), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(new_n54_), .c(new_n51_), .O(new_n66_));
  oai21  g25(.a(new_n66_), .b(new_n64_), .c(x19), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(x08), .O(new_n68_));
  nand3  g27(.a(new_n48_), .b(new_n47_), .c(new_n46_), .O(new_n69_));
  nand4  g28(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n63_), .O(new_n70_));
  oai21  g29(.a(new_n70_), .b(new_n69_), .c(x19), .O(new_n71_));
  oai21  g30(.a(new_n71_), .b(new_n42_), .c(new_n68_), .O(z02));
  inv1   g31(.a(x20), .O(new_n73_));
  nand2  g32(.a(new_n44_), .b(new_n73_), .O(z03));
  nor2   g33(.a(x21), .b(x20), .O(new_n75_));
  nor2   g34(.a(new_n75_), .b(new_n60_), .O(z04));
  inv1   g35(.a(x10), .O(new_n77_));
  nand3  g36(.a(new_n65_), .b(x19), .c(new_n51_), .O(new_n78_));
  oai21  g37(.a(new_n78_), .b(new_n64_), .c(new_n77_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(new_n53_), .O(new_n80_));
  oai21  g39(.a(new_n43_), .b(new_n77_), .c(new_n80_), .O(z05));
  nand2  g40(.a(new_n43_), .b(new_n53_), .O(new_n82_));
  nand4  g41(.a(new_n82_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n83_));
  nor2   g42(.a(new_n83_), .b(x04), .O(new_n84_));
  nand4  g43(.a(new_n84_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n85_));
  nor2   g44(.a(new_n70_), .b(new_n69_), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(x00), .O(new_n87_));
  nand2  g46(.a(new_n87_), .b(x19), .O(new_n88_));
  nand3  g47(.a(new_n88_), .b(new_n85_), .c(new_n44_), .O(z06));
  inv1   g48(.a(x18), .O(new_n90_));
  nand2  g49(.a(new_n44_), .b(x24), .O(new_n91_));
  nor2   g50(.a(new_n91_), .b(new_n90_), .O(z07));
  inv1   g51(.a(x11), .O(new_n93_));
  nor2   g52(.a(new_n60_), .b(new_n93_), .O(z08));
  nor2   g53(.a(new_n91_), .b(new_n93_), .O(z09));
  inv1   g54(.a(x14), .O(new_n96_));
  inv1   g55(.a(x24), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x22), .O(new_n98_));
  oai21  g57(.a(new_n98_), .b(new_n96_), .c(new_n44_), .O(z10));
  inv1   g58(.a(x17), .O(new_n100_));
  oai21  g59(.a(new_n98_), .b(new_n100_), .c(new_n44_), .O(z11));
  nand4  g60(.a(new_n44_), .b(new_n97_), .c(x23), .d(x14), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z12));
  nand3  g62(.a(new_n97_), .b(x23), .c(x17), .O(new_n104_));
  nand2  g63(.a(new_n104_), .b(new_n44_), .O(z13));
  inv1   g64(.a(x16), .O(new_n106_));
  nor3   g65(.a(new_n60_), .b(x24), .c(new_n106_), .O(z14));
  nor2   g66(.a(x15), .b(x14), .O(new_n108_));
  nor3   g67(.a(new_n60_), .b(x13), .c(x12), .O(new_n109_));
  nand2  g68(.a(new_n109_), .b(new_n108_), .O(z15));
endmodule


