// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:16 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n71_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n89_, new_n91_, new_n93_,
    new_n94_, new_n96_, new_n98_, new_n100_, new_n102_, new_n104_,
    new_n105_, new_n106_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x09), .O(new_n43_));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(new_n43_), .O(new_n45_));
  inv1   g04(.a(new_n45_), .O(new_n46_));
  nand2  g05(.a(new_n46_), .b(new_n42_), .O(z00));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  inv1   g08(.a(x03), .O(new_n50_));
  inv1   g09(.a(x04), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  inv1   g11(.a(x07), .O(new_n53_));
  inv1   g12(.a(x08), .O(new_n54_));
  nor2   g13(.a(x06), .b(x05), .O(new_n55_));
  nand3  g14(.a(new_n55_), .b(new_n54_), .c(new_n53_), .O(new_n56_));
  oai21  g15(.a(new_n56_), .b(new_n52_), .c(x00), .O(new_n57_));
  nand3  g16(.a(new_n57_), .b(x19), .c(new_n43_), .O(new_n58_));
  inv1   g17(.a(new_n58_), .O(z01));
  nor2   g18(.a(x03), .b(x02), .O(new_n60_));
  nor2   g19(.a(x05), .b(x04), .O(new_n61_));
  nor2   g20(.a(x07), .b(x06), .O(new_n62_));
  nand4  g21(.a(new_n62_), .b(new_n61_), .c(new_n60_), .d(new_n48_), .O(new_n63_));
  nand2  g22(.a(new_n63_), .b(x00), .O(new_n64_));
  inv1   g23(.a(new_n52_), .O(new_n65_));
  nor2   g24(.a(new_n54_), .b(x07), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(new_n55_), .c(new_n65_), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  nand3  g27(.a(new_n68_), .b(x19), .c(new_n43_), .O(new_n69_));
  inv1   g28(.a(new_n69_), .O(z02));
  inv1   g29(.a(x20), .O(new_n71_));
  nand2  g30(.a(new_n46_), .b(new_n71_), .O(z03));
  nor2   g31(.a(x21), .b(x20), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n45_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  nand4  g34(.a(new_n55_), .b(x19), .c(new_n54_), .d(new_n53_), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(new_n52_), .c(new_n75_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(new_n43_), .O(new_n78_));
  oai21  g37(.a(x19), .b(new_n75_), .c(new_n78_), .O(z05));
  inv1   g38(.a(x05), .O(new_n80_));
  nand4  g39(.a(new_n62_), .b(new_n80_), .c(new_n51_), .d(new_n50_), .O(new_n81_));
  nor2   g40(.a(new_n81_), .b(x02), .O(new_n82_));
  nor3   g41(.a(x03), .b(x02), .c(x01), .O(new_n83_));
  nand4  g42(.a(new_n62_), .b(new_n61_), .c(new_n83_), .d(x00), .O(new_n84_));
  aoi21  g43(.a(new_n82_), .b(new_n48_), .c(new_n84_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n43_), .c(new_n44_), .O(z06));
  nand3  g45(.a(new_n46_), .b(x24), .c(x18), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z07));
  inv1   g47(.a(x11), .O(new_n89_));
  nor2   g48(.a(new_n45_), .b(new_n89_), .O(z08));
  inv1   g49(.a(x24), .O(new_n91_));
  oai21  g50(.a(new_n91_), .b(new_n89_), .c(new_n46_), .O(z09));
  nor2   g51(.a(new_n45_), .b(x24), .O(new_n93_));
  nand3  g52(.a(new_n93_), .b(x22), .c(x14), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z10));
  nand3  g54(.a(new_n93_), .b(x22), .c(x17), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z11));
  nand3  g56(.a(new_n91_), .b(x23), .c(x14), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n46_), .O(z12));
  nand3  g58(.a(new_n93_), .b(x23), .c(x17), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z13));
  nand2  g60(.a(new_n93_), .b(x16), .O(new_n102_));
  inv1   g61(.a(new_n102_), .O(z14));
  inv1   g62(.a(x12), .O(new_n104_));
  inv1   g63(.a(x13), .O(new_n105_));
  nor2   g64(.a(x15), .b(x14), .O(new_n106_));
  nand4  g65(.a(new_n106_), .b(new_n46_), .c(new_n105_), .d(new_n104_), .O(z15));
endmodule


