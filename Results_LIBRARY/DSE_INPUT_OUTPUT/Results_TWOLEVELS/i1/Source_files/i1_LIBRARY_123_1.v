// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:43 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n55_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n65_,
    new_n66_, new_n67_, new_n69_, new_n71_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n85_, new_n86_, new_n88_, new_n91_, new_n92_, new_n93_,
    new_n94_, new_n95_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n106_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x06), .O(new_n48_));
  inv1   g07(.a(x07), .O(new_n49_));
  xnor2a g08(.a(x09), .b(x08), .O(new_n50_));
  nand4  g09(.a(new_n50_), .b(x19), .c(new_n49_), .d(new_n48_), .O(new_n51_));
  nor3   g10(.a(new_n51_), .b(x05), .c(x04), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n53_));
  nor2   g12(.a(new_n53_), .b(new_n42_), .O(z01));
  inv1   g13(.a(x04), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n56_));
  inv1   g15(.a(x05), .O(new_n57_));
  inv1   g16(.a(x09), .O(new_n58_));
  nand2  g17(.a(new_n58_), .b(x08), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n49_), .c(new_n48_), .d(new_n57_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n56_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(new_n43_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x00), .O(new_n63_));
  inv1   g22(.a(new_n63_), .O(z02));
  inv1   g23(.a(x20), .O(new_n65_));
  nand2  g24(.a(x19), .b(new_n42_), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n65_), .O(z03));
  inv1   g27(.a(x21), .O(new_n69_));
  nand3  g28(.a(new_n66_), .b(new_n69_), .c(new_n65_), .O(z04));
  inv1   g29(.a(x10), .O(new_n71_));
  nand4  g30(.a(new_n47_), .b(new_n46_), .c(new_n45_), .d(x00), .O(new_n72_));
  nor2   g31(.a(x08), .b(x07), .O(new_n73_));
  nand4  g32(.a(new_n73_), .b(new_n48_), .c(new_n57_), .d(new_n55_), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(new_n72_), .c(x00), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x19), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n71_), .O(z05));
  nand2  g36(.a(new_n58_), .b(x08), .O(new_n78_));
  nand4  g37(.a(new_n78_), .b(new_n49_), .c(new_n48_), .d(new_n57_), .O(new_n79_));
  nor2   g38(.a(new_n79_), .b(x04), .O(new_n80_));
  nand4  g39(.a(new_n80_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(new_n61_), .O(new_n82_));
  nand3  g41(.a(new_n82_), .b(x19), .c(x00), .O(new_n83_));
  inv1   g42(.a(new_n83_), .O(z06));
  inv1   g43(.a(x18), .O(new_n85_));
  nand2  g44(.a(new_n66_), .b(x24), .O(new_n86_));
  nor2   g45(.a(new_n86_), .b(new_n85_), .O(z07));
  inv1   g46(.a(x11), .O(new_n88_));
  nand2  g47(.a(new_n66_), .b(new_n88_), .O(z08));
  nor2   g48(.a(new_n86_), .b(new_n88_), .O(z09));
  inv1   g49(.a(x22), .O(new_n91_));
  inv1   g50(.a(x24), .O(new_n92_));
  nand2  g51(.a(new_n66_), .b(new_n92_), .O(new_n93_));
  nor2   g52(.a(new_n93_), .b(new_n91_), .O(new_n94_));
  nand2  g53(.a(new_n94_), .b(x14), .O(new_n95_));
  inv1   g54(.a(new_n95_), .O(z10));
  nand2  g55(.a(new_n94_), .b(x17), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z11));
  nand3  g57(.a(new_n92_), .b(x23), .c(x14), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n66_), .O(z12));
  nand4  g59(.a(new_n66_), .b(new_n92_), .c(x23), .d(x17), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z13));
  inv1   g61(.a(x16), .O(new_n103_));
  nor2   g62(.a(new_n93_), .b(new_n103_), .O(z14));
  nor2   g63(.a(x13), .b(x12), .O(new_n105_));
  nor2   g64(.a(x15), .b(x14), .O(new_n106_));
  aoi21  g65(.a(new_n106_), .b(new_n105_), .c(new_n67_), .O(z15));
endmodule


