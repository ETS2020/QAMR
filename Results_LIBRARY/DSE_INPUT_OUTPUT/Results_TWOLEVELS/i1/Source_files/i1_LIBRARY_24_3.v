// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_, new_n50_,
    new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_, new_n59_,
    new_n61_, new_n62_, new_n63_, new_n64_, new_n65_, new_n66_, new_n67_,
    new_n68_, new_n70_, new_n71_, new_n73_, new_n75_, new_n77_, new_n78_,
    new_n79_, new_n80_, new_n82_, new_n84_, new_n86_, new_n88_, new_n90_,
    new_n91_, new_n92_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x19), .b(new_n42_), .O(z00));
  inv1   g02(.a(x19), .O(new_n44_));
  nor2   g03(.a(new_n44_), .b(x00), .O(z01));
  inv1   g04(.a(x07), .O(new_n46_));
  inv1   g05(.a(x09), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x04), .O(new_n49_));
  inv1   g08(.a(x05), .O(new_n50_));
  nor3   g09(.a(x02), .b(x01), .c(x00), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x06), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n47_), .c(x08), .d(new_n46_), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(new_n44_), .O(z02));
  inv1   g14(.a(x20), .O(new_n56_));
  nor2   g15(.a(new_n44_), .b(new_n42_), .O(new_n57_));
  nor2   g16(.a(new_n57_), .b(new_n56_), .O(z03));
  nor2   g17(.a(x21), .b(x20), .O(new_n59_));
  nor2   g18(.a(new_n59_), .b(new_n57_), .O(z04));
  inv1   g19(.a(x10), .O(new_n61_));
  nor2   g20(.a(x02), .b(x01), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(new_n49_), .c(new_n48_), .O(new_n63_));
  inv1   g22(.a(x08), .O(new_n64_));
  nor2   g23(.a(x06), .b(x05), .O(new_n65_));
  nand4  g24(.a(new_n65_), .b(x19), .c(new_n64_), .d(new_n46_), .O(new_n66_));
  oai21  g25(.a(new_n66_), .b(new_n63_), .c(new_n61_), .O(new_n67_));
  nand2  g26(.a(new_n67_), .b(new_n42_), .O(new_n68_));
  oai21  g27(.a(x19), .b(new_n61_), .c(new_n68_), .O(z05));
  inv1   g28(.a(new_n57_), .O(new_n70_));
  nand3  g29(.a(new_n70_), .b(x24), .c(x18), .O(new_n71_));
  inv1   g30(.a(new_n71_), .O(z07));
  inv1   g31(.a(x11), .O(new_n73_));
  nand2  g32(.a(new_n70_), .b(new_n73_), .O(z08));
  inv1   g33(.a(x24), .O(new_n75_));
  oai21  g34(.a(new_n75_), .b(new_n73_), .c(new_n70_), .O(z09));
  nor2   g35(.a(new_n57_), .b(x24), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x22), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(new_n79_));
  nand2  g38(.a(new_n79_), .b(x14), .O(new_n80_));
  inv1   g39(.a(new_n80_), .O(z10));
  nand2  g40(.a(new_n79_), .b(x17), .O(new_n82_));
  inv1   g41(.a(new_n82_), .O(z11));
  nand3  g42(.a(new_n77_), .b(x23), .c(x14), .O(new_n84_));
  inv1   g43(.a(new_n84_), .O(z12));
  nand3  g44(.a(new_n75_), .b(x23), .c(x17), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n70_), .O(z13));
  nand2  g46(.a(new_n77_), .b(x16), .O(new_n88_));
  inv1   g47(.a(new_n88_), .O(z14));
  inv1   g48(.a(x12), .O(new_n90_));
  inv1   g49(.a(x13), .O(new_n91_));
  nor2   g50(.a(x15), .b(x14), .O(new_n92_));
  nand4  g51(.a(new_n92_), .b(new_n70_), .c(new_n91_), .d(new_n90_), .O(z15));
  buf    g52(.a(x19), .O(z06));
endmodule


