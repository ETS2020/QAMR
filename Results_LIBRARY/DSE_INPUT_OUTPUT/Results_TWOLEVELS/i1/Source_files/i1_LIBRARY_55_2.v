// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n70_, new_n71_,
    new_n74_, new_n76_, new_n77_, new_n78_, new_n79_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n86_, new_n87_, new_n89_, new_n90_,
    new_n92_, new_n95_, new_n96_, new_n98_, new_n100_, new_n102_,
    new_n105_, new_n106_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x05), .O(new_n43_));
  nor2   g02(.a(x20), .b(new_n43_), .O(new_n44_));
  inv1   g03(.a(new_n44_), .O(new_n45_));
  nand2  g04(.a(new_n45_), .b(new_n42_), .O(z00));
  inv1   g05(.a(x19), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  inv1   g08(.a(x03), .O(new_n50_));
  inv1   g09(.a(x06), .O(new_n51_));
  inv1   g10(.a(x07), .O(new_n52_));
  xnor2a g11(.a(x09), .b(x08), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n52_), .c(new_n51_), .d(new_n43_), .O(new_n54_));
  nor2   g13(.a(new_n54_), .b(x04), .O(new_n55_));
  nand4  g14(.a(new_n55_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n56_));
  aoi21  g15(.a(new_n56_), .b(z00), .c(new_n47_), .O(z01));
  nand3  g16(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n58_));
  inv1   g17(.a(x04), .O(new_n59_));
  nand3  g18(.a(new_n52_), .b(new_n51_), .c(new_n59_), .O(new_n60_));
  nor2   g19(.a(new_n60_), .b(new_n58_), .O(new_n61_));
  nor2   g20(.a(new_n61_), .b(x05), .O(new_n62_));
  inv1   g21(.a(x20), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n43_), .O(new_n64_));
  oai21  g23(.a(new_n64_), .b(new_n62_), .c(x00), .O(new_n65_));
  nand4  g24(.a(new_n59_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(new_n67_));
  nor2   g26(.a(x06), .b(x05), .O(new_n68_));
  inv1   g27(.a(x08), .O(new_n69_));
  nor2   g28(.a(x09), .b(new_n69_), .O(new_n70_));
  nand4  g29(.a(new_n70_), .b(new_n68_), .c(new_n67_), .d(new_n52_), .O(new_n71_));
  aoi21  g30(.a(new_n71_), .b(new_n65_), .c(new_n47_), .O(z02));
  nand2  g31(.a(new_n63_), .b(new_n43_), .O(z03));
  inv1   g32(.a(x21), .O(new_n74_));
  oai21  g33(.a(new_n74_), .b(x05), .c(new_n63_), .O(z04));
  nand4  g34(.a(x19), .b(new_n69_), .c(new_n52_), .d(new_n51_), .O(new_n76_));
  inv1   g35(.a(new_n76_), .O(new_n77_));
  aoi21  g36(.a(new_n77_), .b(new_n67_), .c(x10), .O(new_n78_));
  nand2  g37(.a(x20), .b(x10), .O(new_n79_));
  oai21  g38(.a(new_n78_), .b(x05), .c(new_n79_), .O(z05));
  nor3   g39(.a(x07), .b(x06), .c(x04), .O(new_n81_));
  nand4  g40(.a(new_n81_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n82_));
  oai21  g41(.a(new_n60_), .b(new_n58_), .c(x00), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n82_), .O(new_n84_));
  nand2  g43(.a(new_n64_), .b(x00), .O(new_n85_));
  inv1   g44(.a(new_n85_), .O(new_n86_));
  aoi21  g45(.a(new_n84_), .b(new_n43_), .c(new_n86_), .O(new_n87_));
  aoi21  g46(.a(new_n87_), .b(z00), .c(new_n47_), .O(z06));
  inv1   g47(.a(x18), .O(new_n89_));
  inv1   g48(.a(x24), .O(new_n90_));
  nor3   g49(.a(new_n44_), .b(new_n90_), .c(new_n89_), .O(z07));
  inv1   g50(.a(x11), .O(new_n92_));
  nor2   g51(.a(new_n44_), .b(new_n92_), .O(z08));
  oai21  g52(.a(new_n90_), .b(new_n92_), .c(new_n45_), .O(z09));
  nor2   g53(.a(new_n44_), .b(x24), .O(new_n95_));
  nand3  g54(.a(new_n95_), .b(x22), .c(x14), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z10));
  nand3  g56(.a(new_n90_), .b(x22), .c(x17), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n45_), .O(z11));
  nand3  g58(.a(new_n95_), .b(x23), .c(x14), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z12));
  nand3  g60(.a(new_n90_), .b(x23), .c(x17), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n45_), .O(z13));
  and2   g62(.a(new_n95_), .b(x16), .O(z14));
  nor2   g63(.a(x15), .b(x14), .O(new_n105_));
  nor3   g64(.a(new_n44_), .b(x13), .c(x12), .O(new_n106_));
  nand2  g65(.a(new_n106_), .b(new_n105_), .O(z15));
endmodule


