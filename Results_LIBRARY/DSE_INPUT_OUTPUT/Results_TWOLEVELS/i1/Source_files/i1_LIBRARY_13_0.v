// Benchmark "FAU" written by ABC on Wed Aug 19 17:33:55 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_,
    new_n74_, new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_,
    new_n82_, new_n84_, new_n85_, new_n87_, new_n90_, new_n91_, new_n92_,
    new_n94_, new_n96_, new_n97_, new_n98_, new_n100_, new_n102_,
    new_n104_, new_n105_;
  inv1   g00(.a(x19), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  inv1   g02(.a(x02), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x05), .O(new_n46_));
  inv1   g05(.a(x06), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x04), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(x00), .c(new_n42_), .O(z01));
  nor2   g12(.a(x03), .b(x02), .O(new_n54_));
  nor2   g13(.a(x05), .b(x04), .O(new_n55_));
  nor2   g14(.a(x07), .b(x06), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n55_), .c(new_n54_), .d(new_n43_), .O(new_n57_));
  inv1   g16(.a(x04), .O(new_n58_));
  nand4  g17(.a(new_n58_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n59_));
  inv1   g18(.a(new_n59_), .O(new_n60_));
  nand2  g19(.a(new_n47_), .b(new_n46_), .O(new_n61_));
  inv1   g20(.a(x09), .O(new_n62_));
  nand3  g21(.a(new_n62_), .b(x08), .c(new_n48_), .O(new_n63_));
  nor2   g22(.a(new_n63_), .b(new_n61_), .O(new_n64_));
  aoi22  g23(.a(new_n64_), .b(new_n60_), .c(new_n57_), .d(x00), .O(new_n65_));
  nor2   g24(.a(x19), .b(x00), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(new_n67_));
  oai21  g26(.a(new_n65_), .b(new_n42_), .c(new_n67_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nor2   g28(.a(new_n66_), .b(new_n69_), .O(z03));
  inv1   g29(.a(x21), .O(new_n71_));
  nand3  g30(.a(new_n67_), .b(new_n71_), .c(new_n69_), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  inv1   g32(.a(new_n61_), .O(new_n74_));
  nor2   g33(.a(new_n42_), .b(x08), .O(new_n75_));
  nand4  g34(.a(new_n75_), .b(new_n74_), .c(new_n60_), .d(new_n48_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n67_), .c(new_n73_), .O(z05));
  nand2  g36(.a(new_n57_), .b(x00), .O(new_n78_));
  nor3   g37(.a(x07), .b(x06), .c(x05), .O(new_n79_));
  nand4  g38(.a(new_n79_), .b(new_n58_), .c(new_n45_), .d(new_n44_), .O(new_n80_));
  oai21  g39(.a(new_n80_), .b(x01), .c(new_n78_), .O(new_n81_));
  nand2  g40(.a(new_n81_), .b(x19), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(x00), .O(z06));
  inv1   g42(.a(x18), .O(new_n84_));
  nand2  g43(.a(new_n67_), .b(x24), .O(new_n85_));
  nor2   g44(.a(new_n85_), .b(new_n84_), .O(z07));
  inv1   g45(.a(x11), .O(new_n87_));
  nand2  g46(.a(new_n67_), .b(new_n87_), .O(z08));
  nor2   g47(.a(new_n85_), .b(new_n87_), .O(z09));
  inv1   g48(.a(x14), .O(new_n90_));
  inv1   g49(.a(x24), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x22), .O(new_n92_));
  oai21  g51(.a(new_n92_), .b(new_n90_), .c(new_n67_), .O(z10));
  inv1   g52(.a(x17), .O(new_n94_));
  oai21  g53(.a(new_n92_), .b(new_n94_), .c(new_n67_), .O(z11));
  inv1   g54(.a(x23), .O(new_n96_));
  nor3   g55(.a(new_n66_), .b(x24), .c(new_n96_), .O(new_n97_));
  nand2  g56(.a(new_n97_), .b(x14), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z12));
  nand2  g58(.a(new_n97_), .b(x17), .O(new_n100_));
  inv1   g59(.a(new_n100_), .O(z13));
  nand2  g60(.a(new_n91_), .b(x16), .O(new_n102_));
  nand2  g61(.a(new_n102_), .b(new_n67_), .O(z14));
  nor2   g62(.a(x13), .b(x12), .O(new_n104_));
  nor2   g63(.a(x15), .b(x14), .O(new_n105_));
  aoi21  g64(.a(new_n105_), .b(new_n104_), .c(new_n66_), .O(z15));
  buf    g65(.a(x00), .O(z00));
endmodule


