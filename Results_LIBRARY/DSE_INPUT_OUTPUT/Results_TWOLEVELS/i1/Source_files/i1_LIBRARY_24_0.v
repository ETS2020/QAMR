// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:00 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n45_, new_n46_, new_n47_, new_n48_,
    new_n49_, new_n50_, new_n51_, new_n52_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_,
    new_n64_, new_n65_, new_n67_, new_n68_, new_n70_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n83_,
    new_n84_, new_n87_, new_n88_, new_n89_, new_n90_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n101_;
  inv1   g00(.a(x19), .O(new_n42_));
  inv1   g01(.a(x01), .O(new_n43_));
  inv1   g02(.a(x02), .O(new_n44_));
  inv1   g03(.a(x03), .O(new_n45_));
  inv1   g04(.a(x05), .O(new_n46_));
  inv1   g05(.a(x06), .O(new_n47_));
  inv1   g06(.a(x07), .O(new_n48_));
  xnor2a g07(.a(x09), .b(x08), .O(new_n49_));
  nand4  g08(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n50_));
  nor2   g09(.a(new_n50_), .b(x04), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n52_));
  aoi21  g11(.a(new_n52_), .b(x00), .c(new_n42_), .O(z01));
  nor2   g12(.a(x03), .b(x02), .O(new_n54_));
  nor2   g13(.a(x05), .b(x04), .O(new_n55_));
  nor2   g14(.a(x07), .b(x06), .O(new_n56_));
  nand4  g15(.a(new_n56_), .b(new_n55_), .c(new_n54_), .d(new_n43_), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(x00), .O(new_n58_));
  inv1   g17(.a(x04), .O(new_n59_));
  nand4  g18(.a(new_n59_), .b(new_n45_), .c(new_n44_), .d(new_n43_), .O(new_n60_));
  inv1   g19(.a(new_n60_), .O(new_n61_));
  nor2   g20(.a(x06), .b(x05), .O(new_n62_));
  inv1   g21(.a(x08), .O(new_n63_));
  nor2   g22(.a(x09), .b(new_n63_), .O(new_n64_));
  nand4  g23(.a(new_n64_), .b(new_n62_), .c(new_n61_), .d(new_n48_), .O(new_n65_));
  aoi21  g24(.a(new_n65_), .b(new_n58_), .c(new_n42_), .O(z02));
  inv1   g25(.a(x20), .O(new_n67_));
  nor2   g26(.a(x19), .b(x00), .O(new_n68_));
  nor2   g27(.a(new_n68_), .b(new_n67_), .O(z03));
  nor2   g28(.a(x21), .b(x20), .O(new_n70_));
  nor2   g29(.a(new_n70_), .b(new_n68_), .O(z04));
  inv1   g30(.a(x10), .O(new_n72_));
  nand4  g31(.a(new_n62_), .b(x19), .c(new_n63_), .d(new_n48_), .O(new_n73_));
  oai22  g32(.a(new_n73_), .b(new_n60_), .c(new_n68_), .d(new_n72_), .O(z05));
  nor3   g33(.a(x07), .b(x06), .c(x05), .O(new_n75_));
  nand4  g34(.a(new_n75_), .b(new_n59_), .c(new_n45_), .d(new_n44_), .O(new_n76_));
  oai21  g35(.a(new_n76_), .b(x01), .c(new_n58_), .O(new_n77_));
  nand2  g36(.a(new_n77_), .b(x19), .O(new_n78_));
  nand2  g37(.a(new_n78_), .b(x00), .O(z06));
  inv1   g38(.a(x18), .O(new_n80_));
  inv1   g39(.a(x24), .O(new_n81_));
  nor3   g40(.a(new_n68_), .b(new_n81_), .c(new_n80_), .O(z07));
  inv1   g41(.a(x11), .O(new_n83_));
  inv1   g42(.a(new_n68_), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n83_), .O(z08));
  oai21  g44(.a(new_n81_), .b(new_n83_), .c(new_n84_), .O(z09));
  inv1   g45(.a(x22), .O(new_n87_));
  nand2  g46(.a(new_n84_), .b(new_n81_), .O(new_n88_));
  nor2   g47(.a(new_n88_), .b(new_n87_), .O(new_n89_));
  nand2  g48(.a(new_n89_), .b(x14), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(z10));
  nand2  g50(.a(new_n89_), .b(x17), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z11));
  nand4  g52(.a(new_n84_), .b(new_n81_), .c(x23), .d(x14), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z12));
  nand3  g54(.a(new_n81_), .b(x23), .c(x17), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(new_n84_), .O(z13));
  inv1   g56(.a(x16), .O(new_n98_));
  nor2   g57(.a(new_n88_), .b(new_n98_), .O(z14));
  nor2   g58(.a(x15), .b(x14), .O(new_n100_));
  nor3   g59(.a(new_n68_), .b(x13), .c(x12), .O(new_n101_));
  nand2  g60(.a(new_n101_), .b(new_n100_), .O(z15));
  buf    g61(.a(x00), .O(z00));
endmodule


