// Benchmark "FAU" written by ABC on Wed Aug 19 17:35:07 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n69_, new_n72_, new_n73_,
    new_n75_, new_n76_, new_n77_, new_n78_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n85_, new_n87_, new_n88_, new_n90_, new_n93_,
    new_n94_, new_n95_, new_n96_, new_n97_, new_n99_, new_n101_, new_n103_,
    new_n105_, new_n107_, new_n108_;
  inv1   g00(.a(x00), .O(new_n42_));
  inv1   g01(.a(x19), .O(new_n43_));
  aoi21  g02(.a(new_n43_), .b(x05), .c(new_n42_), .O(z00));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x05), .O(new_n48_));
  inv1   g07(.a(x06), .O(new_n49_));
  inv1   g08(.a(x07), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x04), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n54_));
  nand2  g13(.a(new_n54_), .b(x00), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x19), .O(new_n56_));
  nand2  g15(.a(new_n43_), .b(x05), .O(new_n57_));
  nand2  g16(.a(new_n57_), .b(new_n56_), .O(z01));
  oai21  g17(.a(new_n43_), .b(x00), .c(x05), .O(new_n59_));
  nor3   g18(.a(x03), .b(x02), .c(x01), .O(new_n60_));
  nor3   g19(.a(x07), .b(x06), .c(x04), .O(new_n61_));
  aoi21  g20(.a(new_n61_), .b(new_n60_), .c(new_n42_), .O(new_n62_));
  inv1   g21(.a(x04), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n64_));
  inv1   g23(.a(x09), .O(new_n65_));
  nor2   g24(.a(x06), .b(x05), .O(new_n66_));
  nand4  g25(.a(new_n66_), .b(new_n65_), .c(x08), .d(new_n50_), .O(new_n67_));
  nor2   g26(.a(new_n67_), .b(new_n64_), .O(new_n68_));
  oai21  g27(.a(new_n68_), .b(new_n62_), .c(x19), .O(new_n69_));
  nand2  g28(.a(new_n69_), .b(new_n59_), .O(z02));
  and2   g29(.a(new_n57_), .b(x20), .O(z03));
  inv1   g30(.a(new_n57_), .O(new_n72_));
  nor2   g31(.a(x21), .b(x20), .O(new_n73_));
  nor2   g32(.a(new_n73_), .b(new_n72_), .O(z04));
  inv1   g33(.a(x10), .O(new_n75_));
  inv1   g34(.a(new_n64_), .O(new_n76_));
  nor2   g35(.a(new_n43_), .b(x08), .O(new_n77_));
  nand4  g36(.a(new_n77_), .b(new_n66_), .c(new_n76_), .d(new_n50_), .O(new_n78_));
  nand3  g37(.a(new_n78_), .b(new_n57_), .c(new_n75_), .O(z05));
  nor2   g38(.a(x07), .b(x06), .O(new_n80_));
  nand3  g39(.a(new_n80_), .b(new_n48_), .c(new_n63_), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(new_n82_));
  nand4  g41(.a(new_n82_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n83_));
  nand4  g42(.a(new_n80_), .b(new_n60_), .c(new_n48_), .d(new_n63_), .O(new_n84_));
  nor2   g43(.a(new_n84_), .b(new_n42_), .O(new_n85_));
  aoi21  g44(.a(new_n85_), .b(new_n83_), .c(new_n43_), .O(z06));
  inv1   g45(.a(x18), .O(new_n87_));
  nand2  g46(.a(new_n57_), .b(x24), .O(new_n88_));
  nor2   g47(.a(new_n88_), .b(new_n87_), .O(z07));
  inv1   g48(.a(x11), .O(new_n90_));
  nor2   g49(.a(new_n72_), .b(new_n90_), .O(z08));
  nor2   g50(.a(new_n88_), .b(new_n90_), .O(z09));
  inv1   g51(.a(x22), .O(new_n93_));
  inv1   g52(.a(x24), .O(new_n94_));
  nand2  g53(.a(new_n57_), .b(new_n94_), .O(new_n95_));
  nor2   g54(.a(new_n95_), .b(new_n93_), .O(new_n96_));
  nand2  g55(.a(new_n96_), .b(x14), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z10));
  nand2  g57(.a(new_n96_), .b(x17), .O(new_n99_));
  inv1   g58(.a(new_n99_), .O(z11));
  nand4  g59(.a(new_n57_), .b(new_n94_), .c(x23), .d(x14), .O(new_n101_));
  inv1   g60(.a(new_n101_), .O(z12));
  nand3  g61(.a(new_n94_), .b(x23), .c(x17), .O(new_n103_));
  nand2  g62(.a(new_n103_), .b(new_n57_), .O(z13));
  inv1   g63(.a(x16), .O(new_n105_));
  nor2   g64(.a(new_n95_), .b(new_n105_), .O(z14));
  nor2   g65(.a(x13), .b(x12), .O(new_n107_));
  nor2   g66(.a(x15), .b(x14), .O(new_n108_));
  aoi21  g67(.a(new_n108_), .b(new_n107_), .c(new_n72_), .O(z15));
endmodule


