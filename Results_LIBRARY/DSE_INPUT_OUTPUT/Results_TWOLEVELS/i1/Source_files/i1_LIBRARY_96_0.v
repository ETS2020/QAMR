// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:31 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n44_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n68_, new_n70_, new_n72_, new_n74_,
    new_n76_, new_n77_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n83_, new_n84_, new_n86_, new_n88_, new_n90_, new_n92_, new_n93_,
    new_n95_, new_n97_, new_n99_, new_n102_, new_n103_;
  inv1   g00(.a(x00), .O(new_n42_));
  nor2   g01(.a(x08), .b(x05), .O(new_n43_));
  inv1   g02(.a(new_n43_), .O(new_n44_));
  nand2  g03(.a(new_n44_), .b(new_n42_), .O(z00));
  inv1   g04(.a(x19), .O(new_n46_));
  inv1   g05(.a(x05), .O(new_n47_));
  inv1   g06(.a(x01), .O(new_n48_));
  inv1   g07(.a(x02), .O(new_n49_));
  inv1   g08(.a(x03), .O(new_n50_));
  inv1   g09(.a(x04), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  inv1   g11(.a(x06), .O(new_n53_));
  inv1   g12(.a(x07), .O(new_n54_));
  nand4  g13(.a(x19), .b(x09), .c(new_n54_), .d(new_n53_), .O(new_n55_));
  oai21  g14(.a(new_n55_), .b(new_n52_), .c(x08), .O(new_n56_));
  nand2  g15(.a(new_n56_), .b(new_n47_), .O(new_n57_));
  oai21  g16(.a(new_n46_), .b(x00), .c(new_n57_), .O(z01));
  nand3  g17(.a(new_n50_), .b(new_n49_), .c(new_n48_), .O(new_n59_));
  nand3  g18(.a(new_n54_), .b(new_n53_), .c(new_n51_), .O(new_n60_));
  oai21  g19(.a(new_n60_), .b(new_n59_), .c(x08), .O(new_n61_));
  nand2  g20(.a(new_n61_), .b(new_n47_), .O(new_n62_));
  nand2  g21(.a(new_n62_), .b(x00), .O(new_n63_));
  inv1   g22(.a(new_n52_), .O(new_n64_));
  inv1   g23(.a(x09), .O(new_n65_));
  nand3  g24(.a(new_n65_), .b(x08), .c(new_n54_), .O(new_n66_));
  inv1   g25(.a(new_n66_), .O(new_n67_));
  nand4  g26(.a(new_n67_), .b(new_n64_), .c(new_n53_), .d(new_n47_), .O(new_n68_));
  aoi21  g27(.a(new_n68_), .b(new_n63_), .c(new_n46_), .O(z02));
  inv1   g28(.a(x20), .O(new_n70_));
  nor2   g29(.a(new_n43_), .b(new_n70_), .O(z03));
  nor2   g30(.a(x21), .b(x20), .O(new_n72_));
  nor2   g31(.a(new_n72_), .b(new_n43_), .O(z04));
  inv1   g32(.a(x10), .O(new_n74_));
  nand2  g33(.a(new_n44_), .b(new_n74_), .O(z05));
  inv1   g34(.a(x08), .O(new_n76_));
  nand2  g35(.a(new_n65_), .b(new_n76_), .O(new_n77_));
  nand4  g36(.a(new_n77_), .b(new_n54_), .c(new_n53_), .d(new_n47_), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(x04), .O(new_n79_));
  nand4  g38(.a(new_n79_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n80_));
  nand3  g39(.a(new_n61_), .b(new_n47_), .c(x00), .O(new_n81_));
  inv1   g40(.a(new_n81_), .O(new_n82_));
  nand2  g41(.a(new_n82_), .b(new_n80_), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(x19), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n44_), .O(z06));
  nand2  g44(.a(x24), .b(x18), .O(new_n86_));
  nand2  g45(.a(new_n86_), .b(new_n44_), .O(z07));
  inv1   g46(.a(x11), .O(new_n88_));
  nand2  g47(.a(new_n44_), .b(new_n88_), .O(z08));
  inv1   g48(.a(x24), .O(new_n90_));
  oai21  g49(.a(new_n90_), .b(new_n88_), .c(new_n44_), .O(z09));
  nor2   g50(.a(new_n43_), .b(x24), .O(new_n92_));
  nand3  g51(.a(new_n92_), .b(x22), .c(x14), .O(new_n93_));
  inv1   g52(.a(new_n93_), .O(z10));
  nand3  g53(.a(new_n90_), .b(x22), .c(x17), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(new_n44_), .O(z11));
  nand3  g55(.a(new_n92_), .b(x23), .c(x14), .O(new_n97_));
  inv1   g56(.a(new_n97_), .O(z12));
  nand3  g57(.a(new_n90_), .b(x23), .c(x17), .O(new_n99_));
  nand2  g58(.a(new_n99_), .b(new_n44_), .O(z13));
  and2   g59(.a(new_n92_), .b(x16), .O(z14));
  nor2   g60(.a(x13), .b(x12), .O(new_n102_));
  nor2   g61(.a(x15), .b(x14), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n43_), .O(z15));
endmodule


