// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:09 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n56_, new_n57_,
    new_n58_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n69_, new_n71_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n83_,
    new_n85_, new_n87_, new_n89_, new_n91_, new_n93_, new_n94_, new_n95_,
    new_n96_, new_n98_, new_n100_, new_n102_, new_n103_;
  inv1   g00(.a(x19), .O(new_n42_));
  nand2  g01(.a(x21), .b(new_n42_), .O(new_n43_));
  and2   g02(.a(new_n43_), .b(x00), .O(z00));
  inv1   g03(.a(x01), .O(new_n45_));
  inv1   g04(.a(x02), .O(new_n46_));
  inv1   g05(.a(x03), .O(new_n47_));
  inv1   g06(.a(x05), .O(new_n48_));
  inv1   g07(.a(x06), .O(new_n49_));
  inv1   g08(.a(x07), .O(new_n50_));
  xnor2a g09(.a(x09), .b(x08), .O(new_n51_));
  nand4  g10(.a(new_n51_), .b(new_n50_), .c(new_n49_), .d(new_n48_), .O(new_n52_));
  nor2   g11(.a(new_n52_), .b(x04), .O(new_n53_));
  nand4  g12(.a(new_n53_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n54_));
  aoi21  g13(.a(new_n54_), .b(x00), .c(new_n42_), .O(z01));
  nor3   g14(.a(x03), .b(x02), .c(x01), .O(new_n56_));
  nor2   g15(.a(x05), .b(x04), .O(new_n57_));
  nor2   g16(.a(x07), .b(x06), .O(new_n58_));
  nand3  g17(.a(new_n58_), .b(new_n57_), .c(new_n56_), .O(new_n59_));
  inv1   g18(.a(x04), .O(new_n60_));
  nand4  g19(.a(new_n60_), .b(new_n47_), .c(new_n46_), .d(new_n45_), .O(new_n61_));
  inv1   g20(.a(new_n61_), .O(new_n62_));
  nand2  g21(.a(new_n49_), .b(new_n48_), .O(new_n63_));
  inv1   g22(.a(x09), .O(new_n64_));
  nand3  g23(.a(new_n64_), .b(x08), .c(new_n50_), .O(new_n65_));
  nor2   g24(.a(new_n65_), .b(new_n63_), .O(new_n66_));
  aoi22  g25(.a(new_n66_), .b(new_n62_), .c(new_n59_), .d(x00), .O(new_n67_));
  oai21  g26(.a(new_n67_), .b(new_n42_), .c(new_n43_), .O(z02));
  inv1   g27(.a(x20), .O(new_n69_));
  nand2  g28(.a(new_n43_), .b(new_n69_), .O(z03));
  nand2  g29(.a(x21), .b(x19), .O(new_n71_));
  oai21  g30(.a(x21), .b(new_n69_), .c(new_n71_), .O(z04));
  inv1   g31(.a(x10), .O(new_n73_));
  inv1   g32(.a(new_n63_), .O(new_n74_));
  nor2   g33(.a(new_n42_), .b(x08), .O(new_n75_));
  nand4  g34(.a(new_n75_), .b(new_n74_), .c(new_n62_), .d(new_n50_), .O(new_n76_));
  nand3  g35(.a(new_n76_), .b(new_n43_), .c(new_n73_), .O(z05));
  nand4  g36(.a(new_n58_), .b(new_n48_), .c(new_n60_), .d(new_n47_), .O(new_n78_));
  nor2   g37(.a(new_n78_), .b(x02), .O(new_n79_));
  nand4  g38(.a(new_n58_), .b(new_n57_), .c(new_n56_), .d(x00), .O(new_n80_));
  aoi21  g39(.a(new_n79_), .b(new_n45_), .c(new_n80_), .O(new_n81_));
  oai21  g40(.a(new_n81_), .b(new_n42_), .c(new_n43_), .O(z06));
  nand2  g41(.a(x24), .b(x18), .O(new_n83_));
  nand2  g42(.a(new_n83_), .b(new_n43_), .O(z07));
  inv1   g43(.a(x11), .O(new_n85_));
  nand2  g44(.a(new_n43_), .b(new_n85_), .O(z08));
  inv1   g45(.a(x24), .O(new_n87_));
  oai21  g46(.a(new_n87_), .b(new_n85_), .c(new_n43_), .O(z09));
  nand4  g47(.a(new_n43_), .b(new_n87_), .c(x22), .d(x14), .O(new_n89_));
  inv1   g48(.a(new_n89_), .O(z10));
  nand3  g49(.a(new_n87_), .b(x22), .c(x17), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(new_n43_), .O(z11));
  inv1   g51(.a(x23), .O(new_n93_));
  inv1   g52(.a(new_n43_), .O(new_n94_));
  nor3   g53(.a(new_n94_), .b(x24), .c(new_n93_), .O(new_n95_));
  nand2  g54(.a(new_n95_), .b(x14), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z12));
  nand2  g56(.a(new_n95_), .b(x17), .O(new_n98_));
  inv1   g57(.a(new_n98_), .O(z13));
  nand2  g58(.a(new_n87_), .b(x16), .O(new_n100_));
  nand2  g59(.a(new_n100_), .b(new_n43_), .O(z14));
  nor2   g60(.a(x13), .b(x12), .O(new_n102_));
  nor2   g61(.a(x15), .b(x14), .O(new_n103_));
  aoi21  g62(.a(new_n103_), .b(new_n102_), .c(new_n94_), .O(z15));
endmodule


