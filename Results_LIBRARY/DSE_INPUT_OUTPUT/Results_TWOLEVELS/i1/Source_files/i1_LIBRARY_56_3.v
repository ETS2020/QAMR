// Benchmark "FAU" written by ABC on Wed Aug 19 17:34:13 2020

module FAU ( 
    x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12, x13,
    x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24,
    z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15  );
  input  x00, x01, x02, x03, x04, x05, x06, x07, x08, x09, x10, x11, x12,
    x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24;
  output z00, z01, z02, z03, z04, z05, z06, z07, z08, z09, z10, z11, z12, z13,
    z14, z15;
  wire new_n42_, new_n43_, new_n45_, new_n46_, new_n47_, new_n48_, new_n49_,
    new_n50_, new_n51_, new_n52_, new_n53_, new_n54_, new_n55_, new_n56_,
    new_n57_, new_n59_, new_n60_, new_n61_, new_n62_, new_n63_, new_n64_,
    new_n65_, new_n66_, new_n67_, new_n70_, new_n72_, new_n73_, new_n74_,
    new_n75_, new_n76_, new_n78_, new_n79_, new_n80_, new_n81_, new_n82_,
    new_n84_, new_n87_, new_n89_, new_n90_, new_n91_, new_n92_, new_n94_,
    new_n96_, new_n98_, new_n100_, new_n102_, new_n103_, new_n104_;
  inv1   g00(.a(x00), .O(new_n42_));
  nand2  g01(.a(x19), .b(x04), .O(new_n43_));
  nand2  g02(.a(new_n43_), .b(new_n42_), .O(z00));
  inv1   g03(.a(x04), .O(new_n45_));
  inv1   g04(.a(x01), .O(new_n46_));
  inv1   g05(.a(x02), .O(new_n47_));
  inv1   g06(.a(x03), .O(new_n48_));
  inv1   g07(.a(x05), .O(new_n49_));
  inv1   g08(.a(x06), .O(new_n50_));
  inv1   g09(.a(x07), .O(new_n51_));
  xnor2a g10(.a(x09), .b(x08), .O(new_n52_));
  nand4  g11(.a(new_n52_), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n53_));
  inv1   g12(.a(new_n53_), .O(new_n54_));
  nand4  g13(.a(new_n54_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n55_));
  nand2  g14(.a(new_n55_), .b(x00), .O(new_n56_));
  nand3  g15(.a(new_n56_), .b(x19), .c(new_n45_), .O(new_n57_));
  inv1   g16(.a(new_n57_), .O(z01));
  nor3   g17(.a(x03), .b(x02), .c(x01), .O(new_n59_));
  nor3   g18(.a(x07), .b(x06), .c(x05), .O(new_n60_));
  aoi21  g19(.a(new_n60_), .b(new_n59_), .c(new_n42_), .O(new_n61_));
  nand4  g20(.a(new_n49_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n62_));
  inv1   g21(.a(x09), .O(new_n63_));
  nand4  g22(.a(new_n63_), .b(x08), .c(new_n51_), .d(new_n50_), .O(new_n64_));
  nor2   g23(.a(new_n64_), .b(new_n62_), .O(new_n65_));
  or2    g24(.a(new_n65_), .b(new_n61_), .O(new_n66_));
  nand3  g25(.a(new_n66_), .b(x19), .c(new_n45_), .O(new_n67_));
  inv1   g26(.a(new_n67_), .O(z02));
  and2   g27(.a(new_n43_), .b(x20), .O(z03));
  nor2   g28(.a(x21), .b(x20), .O(new_n70_));
  aoi21  g29(.a(x19), .b(x04), .c(new_n70_), .O(z04));
  inv1   g30(.a(x10), .O(new_n72_));
  nor2   g31(.a(x08), .b(x07), .O(new_n73_));
  nand4  g32(.a(new_n73_), .b(new_n59_), .c(new_n50_), .d(new_n49_), .O(new_n74_));
  nand2  g33(.a(new_n74_), .b(new_n45_), .O(new_n75_));
  nand2  g34(.a(new_n75_), .b(x19), .O(new_n76_));
  nand2  g35(.a(new_n76_), .b(new_n72_), .O(z05));
  nand4  g36(.a(x08), .b(new_n51_), .c(new_n50_), .d(new_n49_), .O(new_n78_));
  inv1   g37(.a(new_n78_), .O(new_n79_));
  nand4  g38(.a(new_n79_), .b(new_n48_), .c(new_n47_), .d(new_n46_), .O(new_n80_));
  nor2   g39(.a(new_n61_), .b(new_n42_), .O(new_n81_));
  nand4  g40(.a(new_n81_), .b(new_n80_), .c(new_n74_), .d(new_n45_), .O(new_n82_));
  and2   g41(.a(new_n82_), .b(x19), .O(z06));
  nand2  g42(.a(x24), .b(x18), .O(new_n84_));
  nand2  g43(.a(new_n84_), .b(new_n43_), .O(z07));
  and2   g44(.a(new_n43_), .b(x11), .O(z08));
  nand3  g45(.a(new_n43_), .b(x24), .c(x11), .O(new_n87_));
  inv1   g46(.a(new_n87_), .O(z09));
  inv1   g47(.a(x24), .O(new_n89_));
  nand3  g48(.a(new_n43_), .b(new_n89_), .c(x22), .O(new_n90_));
  inv1   g49(.a(new_n90_), .O(new_n91_));
  nand2  g50(.a(new_n91_), .b(x14), .O(new_n92_));
  inv1   g51(.a(new_n92_), .O(z10));
  nand2  g52(.a(new_n91_), .b(x17), .O(new_n94_));
  inv1   g53(.a(new_n94_), .O(z11));
  nand4  g54(.a(new_n43_), .b(new_n89_), .c(x23), .d(x14), .O(new_n96_));
  inv1   g55(.a(new_n96_), .O(z12));
  nand3  g56(.a(new_n89_), .b(x23), .c(x17), .O(new_n98_));
  nand2  g57(.a(new_n98_), .b(new_n43_), .O(z13));
  inv1   g58(.a(x16), .O(new_n100_));
  oai21  g59(.a(x24), .b(new_n100_), .c(new_n43_), .O(z14));
  inv1   g60(.a(x12), .O(new_n102_));
  inv1   g61(.a(x13), .O(new_n103_));
  nor2   g62(.a(x15), .b(x14), .O(new_n104_));
  nand4  g63(.a(new_n104_), .b(new_n43_), .c(new_n103_), .d(new_n102_), .O(z15));
endmodule


